...
public:
    explicit WebTrack(emscripten::val player)
...
public:
    ...
    sk_sp<skottie::ImageAsset> loadImageAsset(const char[] /* path */, const char name[], const char[] /* id */) const override
    {
    ...
    sk_sp<skresources::ExternalTrackAsset> loadAudioAsset(const char[] /* path */, const char[] /* name */, const char id[]) override
    {
...
public:
    ...
    {
        return logger.as<bool>() && logger.hasOwnProperty(kWrnFunc) && logger.hasOwnProperty(kErrFunc) ? sk_sp<JSLogger>(new JSLogger(std::move(logger)))
                                                                                                       : nullptr;
    }

private:
    explicit JSLogger(emscripten::val logger)
...
public:
    static sk_sp<ManagedAnimation> Make(const std::string& json, sk_sp<skottie::ResourceProvider> rp, std::string prop_prefix, emscripten::val logger)
    {
        auto mgr = std::make_unique<skottie_utils::CustomPropertyManager>(skottie_utils::CustomPropertyManager::Mode::kCollapseProperties, prop_prefix.c_str());
        static constexpr char kInterceptPrefix[] = "__";
        auto pinterceptor = sk_make_sp<skottie_utils::ExternalAnimationPrecompInterceptor>(rp, kInterceptPrefix);
        ...
        return animation ? sk_sp<ManagedAnimation>(new ManagedAnimation(std::move(animation), std::move(mgr), std::move(slotManager), std::move(rp))) : nullptr;
    }
    ...
    {
        return fAnimation->duration();
    }
    double fps() const
    {
        return fAnimation->fps();
    }
    const SkSize& size() const
    {
        return fAnimation->size();
    }
    std::string version() const
    {
        return std::string(fAnimation->version().c_str());
    }
    ...
    bool setTransform(const std::string& key, SkScalar anchorX, SkScalar anchorY, SkScalar posX, SkScalar posY, SkScalar scaleX, SkScalar scaleY,
    ...
    JSObject getTextSlot(const std::string& slotID) const
    {
        if (auto textProp = fSlotMgr->getTextSlot(SkString(slotID))) {
            ...
            text_val.set("fillColor", MakeTypedArray(4, SkColor4f::FromColor(textProp->fFillColor).vec()));

            text_val.set("strokeColor", MakeTypedArray(4, SkColor4f::FromColor(textProp->fStrokeColor).vec()));

            const float box[] = { textProp->fBox.fLeft, textProp->fBox.fTop, textProp->fBox.fRight, textProp->fBox.fBottom };
    ...
    bool setImageSlot(const std::string& slotID, const std::string& assetName)
    {
        ...
        return fSlotMgr->setImageSlot(SkString(slotID), fResourceProvider->loadImageAsset(nullptr, assetName.data(), nullptr));
    }
    ...
    bool attachEditor(const std::string& layerID, size_t layerIndex)
    {
        ...
        fTextEditor = sk_make_sp<skottie_utils::TextEditor>(std::move(txt_handle), std::move(deps));
        return true;
    }
    ...
    {
        return fTextEditor ? fTextEditor->onMouseInput(x, y, state, mod) : false;
    }
...
private:
    ManagedAnimation(sk_sp<skottie::Animation> animation, std::unique_ptr<skottie_utils::CustomPropertyManager> propMgr, sk_sp<skottie::SlotManager> slotMgr,
...
EMSCRIPTEN_BINDINGS(Skottie)
{
    ...
    class_<skottie::Animation>("Animation")
        ...
        .function("version", optional_override([](skottie::Animation& self) -> std::string { return std::string(self.version().c_str()); }))
        .function("_size", optional_override([](skottie::Animation& self, WASMPointerF32 oPtr) -> void {
            SkSize* output = reinterpret_cast<SkSize*>(oPtr);
            *output = self.size();
        }))
        .function("duration", &skottie::Animation::duration)
        .function("fps", &skottie::Animation::fps)
        .function("seek", optional_override([](skottie::Animation& self, SkScalar t) -> void { self.seek(t); }))
        .function("seekFrame", optional_override([](skottie::Animation& self, double t) -> void { self.seekFrame(t); }))
        .function("_render", optional_override([](skottie::Animation& self, SkCanvas* canvas, WASMPointerF32 fPtr) -> void {
    ...
    function("MakeAnimation",
        optional_override([](std::string json) -> sk_sp<skottie::Animation> { return skottie::Animation::Make(json.c_str(), json.length()); }));
    ...
    class_<ManagedAnimation>("ManagedAnimation")
        ...
        .function("_render", optional_override([](ManagedAnimation& self, SkCanvas* canvas, WASMPointerF32 fPtr) -> void {
            const SkRect* dst = reinterpret_cast<const SkRect*>(fPtr);
            self.render(canvas, dst);
        }),
            allow_raw_pointers())
        .function("_seek", optional_override([](ManagedAnimation& self, SkScalar t, WASMPointerF32 fPtr) {
            SkRect* damageRect = reinterpret_cast<SkRect*>(fPtr);
            damageRect[0] = self.seek(t);
        }))
        .function("_seekFrame", optional_override([](ManagedAnimation& self, double frame, WASMPointerF32 fPtr) {
            SkRect* damageRect = reinterpret_cast<SkRect*>(fPtr);
        ...
        .function("_setTransform", optional_override([](ManagedAnimation& self, const std::string& key, WASMPointerF32 transformData) {
            ...
            return self.setTransform(
                key, transform[0], transform[1], transform[2], transform[3], transform[4], transform[5], transform[6], transform[7], transform[8]);
        }))
    ...
    function("_MakeManagedAnimation",
        optional_override([](std::string json, size_t assetCount, WASMPointerU32 nptr, WASMPointerU32 dptr, WASMPointerU32 sptr, std::string prop_prefix,
                              emscripten::val soundMap, emscripten::val logger) -> sk_sp<ManagedAnimation> {
            ...
            return ManagedAnimation::Make(json,
                skresources::DataURIResourceProviderProxy::Make(
                    SkottieAssetProvider::Make(std::move(assets), std::move(soundMap)), skresources::ImageDecodeStrategy::kPreDecode, std::move(fontmgr)),
