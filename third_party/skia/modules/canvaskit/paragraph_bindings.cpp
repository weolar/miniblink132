...
para::TextStyle toTextStyle(const SimpleTextStyle& s)
{
    ...
    if (s.fontVariationLen > 0) {
        ...
        for (int i = 0; i < s.fontVariationLen; i++) {
            ...
            coordinates.push_back({ SkSetFourByteTag(axis[0], axis[1], axis[2], axis[3]), fontVariationValues[i] });
        }
...
JSObject GetLineMetricsAt(para::Paragraph& self, size_t lineNumber)
{
    ...
    return self.getLineMetricsAt(lineNumber, &metrics) ? JSObjectFromLineMetrics(metrics) : emscripten::val::null();
}

JSObject GetGlyphInfoAt(para::Paragraph& self, size_t index)
{
    skia::textlayout::Paragraph::GlyphInfo glyphInfo;
    return self.getGlyphInfoAtUTF16Offset(index, &glyphInfo) ? JSObjectFromGlyphInfo(glyphInfo) : emscripten::val::null();
}

JSObject GetClosestGlyphInfoAtCoordinate(para::Paragraph& self, SkScalar dx, SkScalar dy)
{
    skia::textlayout::Paragraph::GlyphInfo glyphInfo;
    return self.getClosestUTF16GlyphInfoAt(dx, dy, &glyphInfo) ? JSObjectFromGlyphInfo(glyphInfo) : emscripten::val::null();
}
...
EMSCRIPTEN_BINDINGS(Paragraph)
{
    ...
    class_<para::ParagraphBuilderImpl>("ParagraphBuilder")
        .class_function("_Make", optional_override([](SimpleParagraphStyle style, sk_sp<SkFontMgr> fontMgr) -> std::unique_ptr<para::ParagraphBuilderImpl> {
            ...
            allow_raw_pointers())
        .class_function("_MakeFromFontProvider",
            optional_override([](SimpleParagraphStyle style, sk_sp<para::TypefaceFontProvider> fontProvider) -> std::unique_ptr<para::ParagraphBuilderImpl> {
                auto fc = sk_make_sp<para::FontCollection>();
        ...
        .class_function("_MakeFromFontCollection",
            optional_override([](SimpleParagraphStyle style, sk_sp<para::FontCollection> fontCollection) -> std::unique_ptr<para::ParagraphBuilderImpl> {
                auto ps = toParagraphStyle(style);
                auto pb = para::ParagraphBuilderImpl::make(ps, fontCollection, get_unicode());
                return std::unique_ptr<para::ParagraphBuilderImpl>(static_cast<para::ParagraphBuilderImpl*>(pb.release()));
            }),
            allow_raw_pointers())
        .class_function("_ShapeText", optional_override([](JSString jtext, JSArray jruns, float width) -> JSArray {
        ...
        .function("addText", optional_override([](para::ParagraphBuilderImpl& self, std::string text) { return self.addText(text.c_str(), text.length()); }))
        ...
        .function("_pushStyle", optional_override([](para::ParagraphBuilderImpl& self, SimpleTextStyle textStyle) {
        ...
        .function("_pushPaintStyle", optional_override([](para::ParagraphBuilderImpl& self, SimpleTextStyle textStyle, SkPaint foreground, SkPaint background) {
            auto ts = toTextStyle(textStyle);
        ...
        .function("_addPlaceholder",
            optional_override([](para::ParagraphBuilderImpl& self, SkScalar width, SkScalar height, para::PlaceholderAlignment alignment,
                                  para::TextBaseline baseline, SkScalar offset) {
                para::PlaceholderStyle ps(width, height, alignment, baseline, offset);
                self.addPlaceholder(ps);
            }))
        .function("getText", optional_override([](para::ParagraphBuilderImpl& self) -> JSString {
            auto text = self.getText();
            return emscripten::val(std::string(text.data(), text.size()).c_str());
        }))
        .function("_setWordsUtf8", optional_override([](para::ParagraphBuilderImpl& self, WASMPointerU32 clientWords, size_t wordsNum) {
#if defined(SK_UNICODE_CLIENT_IMPLEMENTATION)
            self.setWordsUtf8(convertArrayU32(clientWords, wordsNum));
#endif
        }))
        .function("_setWordsUtf16", optional_override([](para::ParagraphBuilderImpl& self, WASMPointerU32 clientWords, size_t wordsNum) {
#if defined(SK_UNICODE_CLIENT_IMPLEMENTATION)
            self.setWordsUtf16(convertArrayU32(clientWords, wordsNum));
#endif
        }))
        .function("_setGraphemeBreaksUtf8", optional_override([](para::ParagraphBuilderImpl& self, WASMPointerU32 clientGraphemes, size_t graphemesNum) {
#if defined(SK_UNICODE_CLIENT_IMPLEMENTATION)
            self.setGraphemeBreaksUtf8(convertArrayU32(clientGraphemes, graphemesNum));
#endif
        }))
        .function("_setGraphemeBreaksUtf16", optional_override([](para::ParagraphBuilderImpl& self, WASMPointerU32 clientGraphemes, size_t graphemesNum) {
#if defined(SK_UNICODE_CLIENT_IMPLEMENTATION)
            self.setGraphemeBreaksUtf16(convertArrayU32(clientGraphemes, graphemesNum));
#endif
        }))
        .function("_setLineBreaksUtf8", optional_override([](para::ParagraphBuilderImpl& self, WASMPointerU32 clientLineBreaks, size_t lineBreaksNum) {
#if defined(SK_UNICODE_CLIENT_IMPLEMENTATION)
        ...
        .function("_setLineBreaksUtf16", optional_override([](para::ParagraphBuilderImpl& self, WASMPointerU32 clientLineBreaks, size_t lineBreaksNum) {
#if defined(SK_UNICODE_CLIENT_IMPLEMENTATION)
            SkUnicode::Position* lineBreakData = reinterpret_cast<SkUnicode::Position*>(clientLineBreaks);
            std::vector<SkUnicode::LineBreakBefore> lineBreaks;
            for (size_t i = 0; i < lineBreaksNum; i += 2) {
    ...
    class_<para::TypefaceFontProvider, base<SkFontMgr>>("TypefaceFontProvider")
        ...
        .class_function("Make", optional_override([]() -> sk_sp<para::TypefaceFontProvider> { return sk_make_sp<para::TypefaceFontProvider>(); }))
        .function("_registerFont", optional_override([](para::TypefaceFontProvider& self, sk_sp<SkTypeface> typeface, WASMPointerU8 familyPtr) {
    ...
    class_<para::FontCollection>("FontCollection")
        ...
        .class_function("Make", optional_override([]() -> sk_sp<para::FontCollection> { return sk_make_sp<para::FontCollection>(); }))
        .function("setDefaultFontManager", optional_override([](para::FontCollection& self, const sk_sp<para::TypefaceFontProvider>& fontManager) {
    ...
    value_object<para::SkRange<size_t>>("URange").field("start", &para::SkRange<size_t>::start).field("end", &para::SkRange<size_t>::end);
