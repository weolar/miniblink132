...
void Node::observeInval(const sk_sp<Node>& node)
{
    ...
    SkASSERT(std::find(node->fInvalObserverArray->begin(), node->fInvalObserverArray->end(), this) == node->fInvalObserverArray->end());
...
void Node::unobserveInval(const sk_sp<Node>& node)
{
    ...
    node->fInvalObserverArray->erase(std::remove(node->fInvalObserverArray->begin(), node->fInvalObserverArray->end(), this), node->fInvalObserverArray->end());
...
const SkRect& Node::revalidate(InvalidationController* ic, const SkMatrix& ctm)
{
    ...
    const auto generate_damage = ic && ((fFlags & kDamage_Flag) || (fInvalTraits & kOverrideDamage_Trait));
    if (!generate_damage) {
