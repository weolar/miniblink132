...
void MaskEffect::onRender(SkCanvas* canvas, const RenderContext* ctx) const
{
...

    {
...

        {
            ...
            content_layer_paint.setBlendMode(is_inverted(fMaskMode) ? SkBlendMode::kSrcOut : SkBlendMode::kSrcIn);
...
SkRect MaskEffect::onRevalidate(InvalidationController* ic, const SkMatrix& ctm)
{
    ...
    return (is_inverted(fMaskMode) || childBounds.intersect(maskBounds)) ? childBounds : SkRect::MakeEmpty();
}
