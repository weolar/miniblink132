...
bool RenderNode::RenderContext::requiresIsolation() const
{
    ...
    return ScaleAlpha(SK_AlphaOPAQUE, fOpacity) != SK_AlphaOPAQUE || fColorFilter || fMaskShader || fBlender;
}

void RenderNode::RenderContext::modulatePaint(const SkMatrix& ctm, SkPaint* paint, bool is_layer_paint) const
{
    ...
    if (!is_layer_paint && fMaskShader) {
        paint->setShader(SkShaders::Blend(SkBlendMode::kSrcIn, LocalShader(fMaskShader, fMaskCTM, ctm), paint->refShader()));
    }
}
...
RenderNode::ScopedRenderContext&& RenderNode::ScopedRenderContext::modulateColorFilter(sk_sp<SkColorFilter> cf)
{
    fCtx.fColorFilter = SkColorFilters::Compose(std::move(fCtx.fColorFilter), std::move(cf));
    return std::move(*this);
}

RenderNode::ScopedRenderContext&& RenderNode::ScopedRenderContext::modulateShader(sk_sp<SkShader> sh, const SkMatrix& shader_ctm)
{
...
RenderNode::ScopedRenderContext&& RenderNode::ScopedRenderContext::modulateMaskShader(sk_sp<SkShader> ms, const SkMatrix& ctm)
{
    if (fCtx.fMaskShader) {
        ...
        if (ms && fCtx.fMaskCTM.invert(&invMaskCTM)) {
            ...
            fCtx.fMaskShader = SkShaders::Blend(SkBlendMode::kSrcIn, std::move(fCtx.fMaskShader), ms->makeWithLocalMatrix(relative_transform));
        }
    } else {
...
RenderNode::ScopedRenderContext&& RenderNode::ScopedRenderContext::modulateBlender(sk_sp<SkBlender> blender)
{
    fCtx.fBlender = std::move(blender);
    return std::move(*this);
}

RenderNode::ScopedRenderContext&& RenderNode::ScopedRenderContext::setIsolation(const SkRect& bounds, const SkMatrix& ctm, bool isolation)
{
...
RenderNode::ScopedRenderContext&& RenderNode::ScopedRenderContext::setFilterIsolation(const SkRect& bounds, const SkMatrix& ctm, sk_sp<SkImageFilter> filter)
{
    if (filter) {
        SkPaint layer_paint;
        ...
        if (layer_paint.getShader()) {
            filter = SkImageFilters::Blend(SkBlendMode::kSrcIn, std::move(filter), SkImageFilters::Shader(layer_paint.refShader()));
        }
