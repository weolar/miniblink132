...
bool RRect::onContains(const SkPoint& p) const
{
    ...
    return fRRect.contains(
        SkRect::MakeLTRB(p.x() - SK_ScalarNearlyZero, p.y() - SK_ScalarNearlyZero, p.x() + SK_ScalarNearlyZero, p.y() + SK_ScalarNearlyZero));
}
