...
sk_sp<ModeColorFilter> ModeColorFilter::Make(sk_sp<RenderNode> child, sk_sp<Color> color, SkBlendMode mode)
{
    return (child && color) ? sk_sp<ModeColorFilter>(new ModeColorFilter(std::move(child), std::move(color), mode)) : nullptr;
}
...
sk_sp<GradientColorFilter> GradientColorFilter::Make(sk_sp<RenderNode> child, sk_sp<Color> c0, sk_sp<Color> c1)
{
    return Make(std::move(child), { std::move(c0), std::move(c1) });
}

sk_sp<GradientColorFilter> GradientColorFilter::Make(sk_sp<RenderNode> child, std::vector<sk_sp<Color>> colors)
{
    return (child && colors.size() > 1) ? sk_sp<GradientColorFilter>(new GradientColorFilter(std::move(child), std::move(colors))) : nullptr;
}
...
{
    const auto c0 = SkColor4f::FromColor(color0->getColor()), c1 = SkColor4f::FromColor(color1->getColor());
...
sk_sp<SkColorFilter> MakeNColorGradient(const std::vector<sk_sp<Color>>& colors)
{
    ...
    for (size_t i = 0; i < span_count; ++i) {
        const auto span_stop = static_cast<size_t>(std::round((i + 1) * 255.0f / span_count)), span_size = span_stop - span_start;
        ...
        const float dR = (SkColorGetR(c1) - r) / span_size, dG = (SkColorGetG(c1) - g) / span_size, dB = (SkColorGetB(c1) - b) / span_size;
    ...
    return SkColorFilters::TableARGB(nullptr, rTable, gTable, bTable)->makeComposed(SkColorFilters::Matrix(luminance_matrix));
}
...
sk_sp<SkColorFilter> GradientColorFilter::onRevalidateFilter()
{
    ...
    auto gradientCF = (fColors.size() > 2) ? MakeNColorGradient(fColors) : Make2ColorGradient(fColors[0], fColors[1]);
