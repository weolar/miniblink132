...
{
    return child ? sk_sp<MaskShaderEffect>(new MaskShaderEffect(std::move(child), std::move(sh))) : nullptr;
}
...
{
    const auto local_ctx = ScopedRenderContext(canvas, ctx).modulateMaskShader(fShader, canvas->getTotalMatrix());
...
{
    return child ? sk_sp<ShaderEffect>(new ShaderEffect(std::move(child), std::move(shader))) : nullptr;
}
...
{
    const auto local_ctx = ScopedRenderContext(canvas, ctx).modulateShader(fShader ? fShader->getShader() : nullptr, canvas->getTotalMatrix());
...
{
    return filter ? sk_sp<RenderNode>(new ImageFilterEffect(std::move(child), std::move(filter))) : child;
}
...
void ImageFilterEffect::onRender(SkCanvas* canvas, const RenderContext* ctx) const
{
    ...
    const auto filter_ctx
        = ScopedRenderContext(canvas, ctx).setFilterIsolation(this->getChild()->bounds(), canvas->getTotalMatrix(), fImageFilter->getFilter());
...
sk_sp<SkImageFilter> DropShadowImageFilter::onRevalidateFilter()
{
    if (fMode == Mode::kShadowOnly) {
        return SkImageFilters::DropShadowOnly(fOffset.x(), fOffset.y(), fSigma.x(), fSigma.y(), fColor, nullptr, this->getCropRect());
    } else {
        return SkImageFilters::DropShadow(fOffset.x(), fOffset.y(), fSigma.x(), fSigma.y(), fColor, nullptr, this->getCropRect());
    }
}
...
{
    return child ? sk_sp<BlenderEffect>(new BlenderEffect(std::move(child), std::move(blender))) : nullptr;
}
...
void LayerEffect::onRender(SkCanvas* canvas, const RenderContext* ctx) const
{
    ...
    const auto local_ctx = ScopedRenderContext(canvas, ctx).setIsolation(this->bounds(), canvas->getTotalMatrix(), true);
