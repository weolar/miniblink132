...
public:
    template <typename = std::enable_if<std::is_same<T, SkMatrix>::value || std::is_same<T, SkM44>::value>> static sk_sp<Matrix> Make(const T& m)
    {
...
public:
    ...
    {
        return child && transform ? sk_sp<TransformEffect>(new TransformEffect(std::move(child), std::move(transform))) : nullptr;
    }
