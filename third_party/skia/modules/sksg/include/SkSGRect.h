...
private:
    ...
    struct AttrContainer {
        ...
        void setDirection(SkPathDirection dir)
        {
            fDirection = SkTo<uint8_t>(dir);
        }

        uint8_t getInitialPointIndex() const
        {
            return fInitialPointIndex;
        }
        void setInitialPointIndex(uint8_t idx)
        {
            fInitialPointIndex = idx;
        }
    };
...
public:
    ...
    {
        return sk_sp<RRect>(new RRect(SkRRect()));
    }
