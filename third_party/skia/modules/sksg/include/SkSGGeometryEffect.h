...
private:
    explicit TrimEffect(sk_sp<GeometryNode> child)
...
public:
    ...
    {
        return child && transform ? sk_sp<GeometryTransform>(new GeometryTransform(std::move(child), std::move(transform))) : nullptr;
    }
...
private:
    explicit DashEffect(sk_sp<GeometryNode> child)
        : INHERITED(std::move(child))
    {
    }
...
private:
    explicit RoundEffect(sk_sp<GeometryNode> child)
        : INHERITED(std::move(child))
    {
    }
...
private:
    explicit OffsetEffect(sk_sp<GeometryNode> child)
        : INHERITED(std::move(child))
    {
    }
