...
static void check_inval(skiatest::Reporter* reporter, const sk_sp<sksg::Node>& root, const SkRect& expected_bounds, const SkRect& expected_inval_bounds,
    const std::vector<SkRect>* expected_damage)
{
    ...
    if ((false)) {
        SkDebugf("** bbox: [%f %f %f %f], ibbox: [%f %f %f %f]\n", bbox.fLeft, bbox.fTop, bbox.fRight, bbox.fBottom, ic.bounds().left(), ic.bounds().top(),
    ...
    if (expected_damage) {
        ...
        for (size_t i = 0; i < std::min(expected_damage->size(), damage_count); ++i) {
            ...
            if ((false)) {
                SkDebugf("*** expected inval: [%f %f %f %f], actual: [%f %f %f %f]\n", r1.left(), r1.top(), r1.right(), r1.bottom(), r2.left(), r2.top(),
                    r2.right(), r2.bottom());
            }
...
static void inval_test1(skiatest::Reporter* reporter)
{
    ...
    auto r1 = sksg::Rect::Make(SkRect::MakeWH(100, 100)), r2 = sksg::Rect::Make(SkRect::MakeWH(100, 100));
...

    {
        ...
        check_inval(reporter, root, SkRect::MakeWH(500, 400), SkRect::MakeLTRB(400, 200, 600, 400), &damage);
...
static void inval_test2(skiatest::Reporter* reporter)
{
    ...
    auto m1 = sksg::Matrix<SkMatrix>::Make(SkMatrix::I()), m2 = sksg::Matrix<SkMatrix>::Make(SkMatrix::I());
    auto t1 = sksg::TransformEffect::Make(sksg::Draw::Make(rect, color), sksg::Transform::MakeConcat(m1, m2)),
...

    {
        ...
        check_inval(reporter, root, SkRect::MakeLTRB(100, 100, 300, 300), SkRect::MakeLTRB(0, 0, 300, 300), &damage);
    }

    {
        ...
        check_inval(reporter, root, SkRect::MakeLTRB(100, 100, 200, 300), SkRect::MakeLTRB(100, 100, 300, 300), &damage);
    }
}
...
static void inval_test3(skiatest::Reporter* reporter)
{
    ...
    {
        // Initial revalidation.
        check_inval(reporter, root, SkRect::MakeXYWH(0, 0, 350, 175), SkRectPriv::MakeLargeS32(), nullptr);
    }

    {
        ...
        check_inval(reporter, root, SkRect::MakeLTRB(50, 75, 350, 175), SkRect::MakeLTRB(0, 0, 350, 175), &damage);
    }

    {
        ...
        check_inval(reporter, root, SkRect::MakeLTRB(50, 75, 350, 175), SkRect::MakeLTRB(50, 75, 350, 175), &damage);
    }
...
{
    auto draw = sksg::Draw::Make(sksg::Rect::Make(SkRect::MakeWH(100, 100)), sksg::Color::Make(SK_ColorBLACK));
    auto grp = sksg::Group::Make();
