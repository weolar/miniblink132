...
sk_sp<MultiFrameImageAsset> MultiFrameImageAsset::Make(sk_sp<SkData> data, ImageDecodeStrategy strat)
{
    if (auto codec = SkCodec::MakeFromData(std::move(data))) {
        return sk_sp<MultiFrameImageAsset>(new MultiFrameImageAsset(std::make_unique<SkAnimCodecPlayer>(std::move(codec)), strat));
    }
    ...
    auto decode = [](sk_sp<SkImage> image) {
        ...
        if (image_area > kMaxArea) {
            ...
            if (bm.tryAllocPixels(info, info.minRowBytes())
                && image->scalePixels(bm.pixmap(), SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNearest), SkImage::kDisallow_CachingHint)) {
                image = bm.asImage();
            }
        } else {
...
{
    return sk_isdir(base_dir.c_str()) ? sk_sp<FileResourceProvider>(new FileResourceProvider(std::move(base_dir), strat)) : nullptr;
}
...
{
    const auto full_dir = SkOSPath::Join(fDir.c_str(), resource_path), full_path = SkOSPath::Join(full_dir.c_str(), resource_name);
    return SkData::MakeFromFileName(full_path.c_str());
}

sk_sp<ImageAsset> FileResourceProvider::loadImageAsset(const char resource_path[], const char resource_name[], const char[]) const
{
...
sk_sp<ImageAsset> ResourceProviderProxyBase::loadImageAsset(const char rpath[], const char rname[], const char rid[]) const
{
...
sk_sp<ExternalTrackAsset> ResourceProviderProxyBase::loadAudioAsset(const char path[], const char name[], const char id[])
{
...
sk_sp<ImageAsset> CachingResourceProvider::loadImageAsset(const char resource_path[], const char resource_name[], const char resource_id[]) const
{
...
sk_sp<DataURIResourceProviderProxy> DataURIResourceProviderProxy::Make(sk_sp<ResourceProvider> rp, ImageDecodeStrategy strat, sk_sp<const SkFontMgr> mgr)
{
    return sk_sp<DataURIResourceProviderProxy>(new DataURIResourceProviderProxy(std::move(rp), strat, std::move(mgr)));
}

DataURIResourceProviderProxy::DataURIResourceProviderProxy(sk_sp<ResourceProvider> rp, ImageDecodeStrategy strat, sk_sp<const SkFontMgr> mgr)
    : INHERITED(std::move(rp))
    , fStrategy(strat)
    , fFontMgr(std::move(mgr))
{
}
...
sk_sp<ImageAsset> DataURIResourceProviderProxy::loadImageAsset(const char rpath[], const char rname[], const char rid[]) const
{
