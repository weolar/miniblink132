    ...
    : fCodec(std::move(codec))
{
    ...
    if (!fTotalDuration) {
        ...
        fFrameInfos.clear();
        fImages.clear();
        fImages.push_back(SkImages::DeferredFromGenerator(SkCodecImageGenerator::MakeFromCodec(std::move(fCodec))));
    }
}
...
sk_sp<SkImage> SkAnimCodecPlayer::getFrameAt(int index)
{
    ...
    const auto originMatrix = SkEncodedOriginToMatrix(origin, orientedDims.width(), orientedDims.height());
...
bool SkAnimCodecPlayer::seek(uint32_t msec)
{
    ...
    auto lower = std::lower_bound(
        fFrameInfos.begin(), fFrameInfos.end(), msec, [](const SkCodec::FrameInfo& info, uint32_t msec) { return (uint32_t)info.fDuration <= msec; });
