...
public:
    explicit PropertyInterceptor(CustomPropertyManager* mgr)
        : fMgr(mgr)
    {
    }
    ...
    void onEnterNode(const char node_name[], PropertyObserver::NodeType node_type) override
    {
        ...
        fMgr->fCurrentNode = fMgr->fCurrentNode.empty() ? node_name : fMgr->fCurrentNode + "." + node_name;
    }
    ...
    void onLeavingNode(const char node_name[], PropertyObserver::NodeType node_type) override
    {
        ...
        fMgr->fCurrentNode = fMgr->fCurrentNode.length() > length ? fMgr->fCurrentNode.substr(0, fMgr->fCurrentNode.length() - strlen(node_name) - 1) : "";
    }
...
public:
    explicit MarkerInterceptor(CustomPropertyManager* mgr)
        : fMgr(mgr)
    {
    }
...
template <typename T> std::vector<CustomPropertyManager::PropKey> CustomPropertyManager::getProps(const PropMap<T>& container) const
{
...
template <typename V, typename T> V CustomPropertyManager::get(const PropKey& key, const PropMap<T>& container) const
{
...
template <typename T> std::unique_ptr<T> CustomPropertyManager::getHandle(const PropKey& key, size_t index, const PropMap<T>& container) const
{
    auto prop_group = container.find(key);
...
template <typename V, typename T> bool CustomPropertyManager::set(const PropKey& key, const V& val, const PropMap<T>& container)
{
    auto prop_group = container.find(key);
...
std::unique_ptr<skottie::ColorPropertyHandle> CustomPropertyManager::getColorHandle(const PropKey& key, size_t index) const
{
...
std::unique_ptr<skottie::OpacityPropertyHandle> CustomPropertyManager::getOpacityHandle(const PropKey& key, size_t index) const
{
...
std::unique_ptr<skottie::TransformPropertyHandle> CustomPropertyManager::getTransformHandle(const PropKey& key, size_t index) const
{
    return this->getHandle(key, index, fTransformMap);
}

bool CustomPropertyManager::setTransform(const PropKey& key, const skottie::TransformPropertyValue& t)
{
...
std::unique_ptr<skottie::TextPropertyHandle> CustomPropertyManager::getTextHandle(const PropKey& key, size_t index) const
{
...
ExternalAnimationPrecompInterceptor::ExternalAnimationPrecompInterceptor(sk_sp<skresources::ResourceProvider> rprovider, const char prefixp[])
...
sk_sp<skottie::ExternalLayer> ExternalAnimationPrecompInterceptor::onLoadPrecomp(const char[], const char name[], const SkSize& size)
{
