...
SkPath make_cursor_path()
{
    ...
    constexpr float kWidth = 0.2f, kHeight = 0.75f;
...
TextEditor::TextEditor(std::unique_ptr<skottie::TextPropertyHandle>&& prop, std::vector<std::unique_ptr<skottie::TextPropertyHandle>>&& deps)
...
std::tuple<size_t, size_t> TextEditor::currentSelection() const
{
    ...
    return std::make_tuple(std::min(std::get<0>(fSelection), std::get<1>(fSelection)), std::max(std::get<0>(fSelection), std::get<1>(fSelection)));
}
...
void TextEditor::drawCursor(SkCanvas* canvas, const TextInfo& tinfo) const
{
    ...
    const auto now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - fTimeBase).count();
    ...
    const auto cscale = txt_prop.fTextSize * tinfo.fScale,
               cxpos = (fCursorIndex ? tinfo.fGlyphs[glyph_index].fAdvance : 0) - fCursorBounds.width() * cscale * 0.5f,
               cypos = -fCursorBounds.height() * cscale;
    const auto cpath = fCursorPath.makeTransform(SkMatrix::Translate(cxpos, cypos) * SkMatrix::Scale(cscale, cscale));
    ...
    const auto inner_width = cscale * fCursorWeight * 0.05f, outer_width = inner_width * 3 / 2;
...
bool TextEditor::deleteSelection()
{
    ...
    const auto utf8_sel_start = fGlyphData[glyph_sel_start].fCluster, utf8_sel_end = fGlyphData[glyph_sel_end].fCluster;
...
void TextEditor::onDecorate(SkCanvas* canvas, const TextInfo& tinfo)
{
    ...
    for (size_t i = 0; i < tinfo.fGlyphs.size(); ++i) {
        ...
        fGlyphData.push_back({ canvas->getLocalToDevice().asM33().mapRect(ginfo.fBounds), ginfo.fCluster });
...
bool TextEditor::onMouseInput(SkScalar x, SkScalar y, skui::InputState state, skui::ModifierKey)
{
    ...
    case skui::InputState::kMove:
        if (fMouseDown) {
            const auto closest = this->closestGlyph({ x, y });
            std::get<1>(fSelection) = closest < std::get<0>(fSelection) ? closest : closest + 1;
        }
        break;
    default:
        break;
...
bool TextEditor::onCharInput(SkUnichar c)
{
    ...
    case '\\': { // delete
        if (!this->deleteSelection() && fCursorIndex > 0) {
            ...
            const auto del_index = prev_utf8(txt_str, fCursorIndex), del_count = fCursorIndex - del_index;
