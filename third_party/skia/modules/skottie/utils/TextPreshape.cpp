...
public:
    ...
    void addGlyph(const std::string_view& font_name, SkUnichar id, const SkFont& font, SkGlyphID glyph)
    {
        std::vector<GlyphRec>& font_glyphs = fFontGlyphs.emplace(font_name, std::vector<GlyphRec>()).first->second;
        ...
        font_glyphs.push_back({ id, width * scale, path.makeTransform(SkMatrix::Scale(scale, scale)) });
    }
    ...
    std::tuple<Value, Value> toLottie(SkArenaAlloc& alloc, const Value& orig_fonts) const
    {
        ...
        for (const auto& font : fFontGlyphs) {
            ...
            orig_font->writable("fName", alloc) = StringValue(font_name.c_str(), font_name.size(), alloc);
        ...
        return std::make_tuple(ObjectValue(fonts_fields, std::size(fonts_fields), alloc), ArrayValue(chars.data(), chars.size(), alloc));
    }
...
public:
    Preshaper(sk_sp<ResourceProvider> rp, sk_sp<SkFontMgr> fontmgr, sk_sp<SkShapers::Factory> sfact)
        : fFontMgr(fontmgr)
        , fShapersFact(sfact)
        , fBuilder(rp ? std::move(rp) : sk_make_sp<NullResourceProvider>(), std::move(fontmgr), nullptr, nullptr, nullptr, nullptr, nullptr, std::move(sfact),
              &fStats, { 0, 0 }, 1, 1, 0)
        , fAlloc(4096)
    {
    }
    ...
    void preshapeLayer(const Value& jlayer)
    {
        ...
        for (const auto& jtxt : *jtxts) {
            ...
            const skottie::Shaper::TextDesc text_desc = {
                ...
                txt_val.fMaxLines,
                skottie::Shaper::Flags::kFragmentGlyphs | skottie::Shaper::Flags::kTrackFragmentAdvanceAscent | skottie::Shaper::Flags::kClusters,
                txt_val.fLocale.isEmpty() ? nullptr : txt_val.fLocale.c_str(),
                txt_val.fFontFamily.isEmpty() ? nullptr : txt_val.fFontFamily.c_str(),
            };

            auto shape_result = skottie::Shaper::Shape(txt_val.fText, text_desc, txt_val.fBox, fFontMgr, fShapersFact);

            auto shaped_glyph_info = [this](SkUnichar ch, const SkPoint& pos, float advance, size_t line, size_t cluster) -> Value {
            ...
            for (const auto& frag : shape_result.fFragments) {
                ...
                for (const auto& runrec : frag.fGlyphs.fRuns) {
                    ...
                    for (size_t i = 0; i < runrec.fSize; ++i) {
                        ...
                        shaped_info.push_back(shaped_glyph_info(ch, frag.fOrigin + glyph_pos[i], frag.fAdvance, frag.fLineIndex, clusters[i]));
                    }
                    offset += runrec.fSize;
                }
            }
            ...
            // Preshaped glyphs.
            jtxt_val.as<ObjectValue>().writable("gl", fAlloc) = ArrayValue(shaped_info.data(), shaped_info.size(), fAlloc);
            // Effecive size for preshaped glyphs, accounting for auto-sizing scale.
            jtxt_val.as<ObjectValue>().writable("gs", fAlloc) = NumberValue(text_desc.fTextSize * shape_result.fScale);
            // Updated font name.
            jtxt_val.as<ObjectValue>().writable("f", fAlloc) = StringValue(preshapedFontName(font_name).c_str(), fAlloc);
        }
    }
...
bool Preshape(const char* json, size_t size, SkWStream* stream, const sk_sp<SkFontMgr>& fmgr, const sk_sp<SkShapers::Factory>& sfact,
...
bool Preshape(
    const sk_sp<SkData>& json, SkWStream* stream, const sk_sp<SkFontMgr>& fmgr, const sk_sp<SkShapers::Factory>& sfact, const sk_sp<ResourceProvider>& rp)
{
