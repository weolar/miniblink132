...
template <typename T> bool ParseIntegral(const Value& v, T* result)
{
    if (const skjson::NumberValue* num = v) {
        const auto dbl = **num;
        if (dbl > static_cast<double>(std::numeric_limits<T>::max()) || dbl < static_cast<double>(std::numeric_limits<T>::min())) {
            return false;
        }
...
template <> bool Parse<int>(const Value& v, int* i)
{
    return ParseIntegral(v, i);
}

template <> bool Parse<size_t>(const Value& v, size_t* sz)
{
...
template <> bool Parse<SkV2>(const Value& v, SkV2* v2)
{
    if (!v.is<ArrayValue>())
        return false;
...
template <> bool Parse<VectorValue>(const Value& v, VectorValue* vec)
{
    if (!v.is<ArrayValue>())
        return false;
    const auto& av = v.as<ArrayValue>();
