...
static bool parse_encoding_data(const skjson::Value& jv, size_t data_len, float data[])
{
    ...
    for (size_t i = 0; i < jvs->size(); ++i) {
        ...
        if (!parse_point(jvs, i, dst + kX_Index, dst + kY_Index) || !parse_optional_point(jis, i, dst + kInX_Index, dst + kInY_Index)
...
ShapeValue::operator SkPath() const
{
    ...
    auto addCubic = [&](size_t from_vertex, size_t to_vertex) {
        ...
        const SkPoint p0 = SkPoint { (*this)[from_index + kX_Index], (*this)[from_index + kY_Index] },
                      ...
                      c0 = SkPoint { (*this)[from_index + kOutX_Index], (*this)[from_index + kOutY_Index] } + p0,
...
template <> bool AnimatablePropertyContainer::bind<ShapeValue>(const AnimationBuilder& abuilder, const skjson::ObjectValue* jprop, ShapeValue* v)
{
