...
private:
    ...
    StateChanged onSeek(float t) override
    {
        ...
        if (v0.cmeasure) {
            ...
            SkPoint pos;
            SkVector tan;
            const float len = v0.cmeasure->length(), distance = len * lerp_info.weight;
...
public:
    ...
    sk_sp<KeyframeAnimator> makeFromKeyframes(const AnimationBuilder& abuilder, const skjson::ArrayValue& jkfs) override
    {
        ...
        return sk_sp<Vec2KeyframeAnimator>(new Vec2KeyframeAnimator(std::move(fKFs), std::move(fCMs), std::move(fValues), fVecTarget, fRotTarget));
    }
...
private:
    ...
    bool parseKFValue(const AnimationBuilder&, const skjson::ObjectValue& jkf, const skjson::Value& jv, Keyframe::Value* v) override
    {
...
bool AnimatablePropertyContainer::bindAutoOrientable(const AnimationBuilder& abuilder, const skjson::ObjectValue* jprop, Vec2Value* v, float* orientation)
{
    if (!jprop) {
        return false;
    }
...
template <> bool AnimatablePropertyContainer::bind<Vec2Value>(const AnimationBuilder& abuilder, const skjson::ObjectValue* jprop, Vec2Value* v)
{
