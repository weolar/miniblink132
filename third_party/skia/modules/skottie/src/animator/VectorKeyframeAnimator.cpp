...
ColorValue::operator SkColor4f() const
{
    ...
    const auto r = this->size() > 0 ? SkTPin((*this)[0], 0.0f, 1.0f) : 0, g = this->size() > 1 ? SkTPin((*this)[1], 0.0f, 1.0f) : 0,
               b = this->size() > 2 ? SkTPin((*this)[2], 0.0f, 1.0f) : 0, a = this->size() > 3 ? SkTPin((*this)[3], 0.0f, 1.0f) : 1;
...
public:
    VectorKeyframeAnimator(std::vector<Keyframe> kfs, std::vector<SkCubicMap> cms, std::vector<float> storage, size_t vec_len, std::vector<float>* target_value)
...
private:
    StateChanged onSeek(float t) override
    {
        ...
        while (count >= 4) {
            const auto old_val = skvx::float4::Load(dst), new_val = Lerp(skvx::float4::Load(v0), skvx::float4::Load(v1), lerp_info.weight);
...
public:
    VectorExpressionAnimator(sk_sp<ExpressionEvaluator<std::vector<float>>> expression_evaluator, std::vector<float>* target_value)
...
VectorAnimatorBuilder::VectorAnimatorBuilder(std::vector<float>* target, VectorLenParser parse_len, VectorDataParser parse_data)
...
sk_sp<KeyframeAnimator> VectorAnimatorBuilder::makeFromKeyframes(const AnimationBuilder& abuilder, const skjson::ArrayValue& jkfs)
{
    ...
    return sk_sp<VectorKeyframeAnimator>(new VectorKeyframeAnimator(std::move(fKFs), std::move(fCMs), std::move(fStorage), fVecLen, fTarget));
}

sk_sp<Animator> VectorAnimatorBuilder::makeFromExpression(ExpressionManager& em, const char* expr)
{
    sk_sp<ExpressionEvaluator<std::vector<SkScalar>>> expression_evaluator = em.createArrayExpressionEvaluator(expr);
...
bool VectorAnimatorBuilder::parseKFValue(const AnimationBuilder&, const skjson::ObjectValue&, const skjson::Value& jv, Keyframe::Value* kfv)
{
...
template <> bool AnimatablePropertyContainer::bind<VectorValue>(const AnimationBuilder& abuilder, const skjson::ObjectValue* jprop, VectorValue* v)
{
    if (!jprop) {
        return false;
    }
    ...
    if (!ParseDefault<bool>((*jprop)["s"], false)) {
        ...
        VectorAnimatorBuilder builder(
            ...
            // Data parser.
            [](const skjson::Value& jv, size_t len, float* data) { return parse_array(jv, data, len); });
...
template <> bool AnimatablePropertyContainer::bind<ColorValue>(const AnimationBuilder& abuilder, const skjson::ObjectValue* jprop, ColorValue* v)
{
