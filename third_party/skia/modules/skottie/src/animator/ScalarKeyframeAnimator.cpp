...
public:
    ScalarKeyframeAnimator(std::vector<Keyframe> kfs, std::vector<SkCubicMap> cms, ScalarValue* target_value)
...
public:
    ScalarExpressionAnimator(sk_sp<ExpressionEvaluator<ScalarValue>> expression_evaluator, ScalarValue* target_value)
...
public:
    ...
    sk_sp<KeyframeAnimator> makeFromKeyframes(const AnimationBuilder& abuilder, const skjson::ArrayValue& jkfs) override
    {
        ...
        return sk_sp<ScalarKeyframeAnimator>(new ScalarKeyframeAnimator(std::move(fKFs), std::move(fCMs), fTarget));
    }

    sk_sp<Animator> makeFromExpression(ExpressionManager& em, const char* expr) override
    {
        sk_sp<ExpressionEvaluator<ScalarValue>> expression_evaluator = em.createNumberExpressionEvaluator(expr);
...
private:
    bool parseKFValue(const AnimationBuilder&, const skjson::ObjectValue&, const skjson::Value& jv, Keyframe::Value* v) override
    {
...
template <> bool AnimatablePropertyContainer::bind<ScalarValue>(const AnimationBuilder& abuilder, const skjson::ObjectValue* jprop, ScalarValue* v)
{
