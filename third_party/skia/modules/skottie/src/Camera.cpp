...
SkM44 ComputeCameraMatrix(const SkV3& position, const SkV3& poi, const SkV3& rotation, const SkSize& viewport_size, float zoom)
{

    // Initial camera vector.
    const auto cam_t = SkM44::Rotate({ 0, 0, 1 }, SkDegreesToRadians(-rotation.z)) * SkM44::Rotate({ 0, 1, 0 }, SkDegreesToRadians(rotation.y))
        * SkM44::Rotate({ 1, 0, 0 }, SkDegreesToRadians(rotation.x))
        * SkM44::LookAt({ position.x, position.y, -position.z }, { poi.x, poi.y, poi.z }, { 0, 1, 0 }) * SkM44::Scale(1, 1, -1);
    ...
    //
    const auto view_size = std::max(viewport_size.width(), viewport_size.height()), view_distance = zoom,
               view_angle = std::atan(sk_ieee_float_divide(view_size * 0.5f, view_distance));

    const auto persp_t = SkM44::Scale(view_size * 0.5f, view_size * 0.5f, 1) * SkM44::Perspective(0, view_distance, 2 * view_angle);

    return SkM44::Translate(viewport_size.width() * 0.5f, viewport_size.height() * 0.5f, 0) * persp_t * cam_t;
}

} // namespace

CameraAdaper::CameraAdaper(
    const skjson::ObjectValue& jlayer, const skjson::ObjectValue& jtransform, const AnimationBuilder& abuilder, const SkSize& viewport_size)
...
sk_sp<sksg::Transform> CameraAdaper::DefaultCameraTransform(const SkSize& viewport_size)
{
    ...
    const SkV3 pos = { center.fX, center.fY, -kDefaultAEZoom }, poi = { pos.x, pos.y, -pos.z - 1 }, rot = { 0, 0, 0 };

    return sksg::Matrix<SkM44>::Make(ComputeCameraMatrix(pos, poi, rot, viewport_size, kDefaultAEZoom));
}

sk_sp<sksg::Transform> AnimationBuilder::attachCamera(
    const skjson::ObjectValue& jlayer, const skjson::ObjectValue& jtransform, sk_sp<sksg::Transform> parent, const SkSize& viewport_size) const
{
