...
public:
    ...
    void finalize(double fps) override
    {
        ...
        printf("Encoder starved stats: "
            ...
            starved_ms[0], starved_ms[fFrames.size() / 2], sum / fFrames.size(), starved_ms.back(), sum, first, first == starved_ms.back() ? "ok" : "BAD");
    }
...
protected:
    explicit FrameGenerator(FrameSink* sink)
        : fSink(sink)
    {
    }
...
public:
    ...
    void generateFrame(const skottie::Animation* anim, size_t frame_index) override
    {
        ...
        if (frame && stream) {
            ...
            sProcs.fImageProc
                = [](SkImage* img, void*) -> sk_sp<SkData> { return SkPngEncoder::Encode(as_IB(img)->directContext(), img, SkPngEncoder::Options {}); };
...
public:
    ...
    static std::unique_ptr<FrameGenerator> Make(FrameSink* sink, const SkMatrix& matrix)
    {
        ...
        return gpu_generator->isValid() ? std::unique_ptr<FrameGenerator>(gpu_generator.release()) : nullptr;
    }
    ...
    void generateFrame(const skottie::Animation* anim, size_t frame_index) override
    {
        ...
        fSurface->asyncRescaleAndReadPixels(SkImageInfo::MakeN32Premul(FLAGS_width, FLAGS_height), { 0, 0, FLAGS_width, FLAGS_height },
            SkSurface::RescaleGamma::kSrc, SkImage::RescaleMode::kNearest, AsyncCallback, rec.release());
...
private:
        ...
        : FrameGenerator(sink)
    {
        ...
        fSurface = SkSurfaces::RenderTarget(
            fCtx, skgpu::Budgeted::kNo, SkImageInfo::MakeN32Premul(FLAGS_width, FLAGS_height), 0, GrSurfaceOrigin::kTopLeft_GrSurfaceOrigin, nullptr);
        if (fSurface) {
    ...
    struct AsyncRec {
        ...
        AsyncRec(FrameSink* sink, size_t index)
            : sink(sink)
            , index(index)
        {
        }
    ...
    static void AsyncCallback(SkSurface::ReadPixelsContext ctx, std::unique_ptr<const SkSurface::AsyncReadResult> result)
    {
        std::unique_ptr<const AsyncRec> rec(reinterpret_cast<const AsyncRec*>(ctx));
        if (result && result->count() == 1) {
            SkPixmap pm(SkImageInfo::MakeN32Premul(FLAGS_width, FLAGS_height), result->data(0), result->rowBytes(0));

            auto release_proc = [](const void*, SkImages::ReleaseContext ctx) {
                std::unique_ptr<const SkSurface::AsyncReadResult> adopted(reinterpret_cast<const SkSurface::AsyncReadResult*>(ctx));
            };

            auto frame_image = SkImages::RasterFromPixmap(pm, release_proc, (void*)result.release());
...
std::unique_ptr<FrameGenerator> FrameGenerator::Make(FrameSink* sink, OutputFormat fmt, const SkMatrix& matrix)
{
    if (fmt == OutputFormat::kSKP) {
        return SKPGenerator::Make(sink, matrix);
    }

    return FLAGS_gpu ? GPUGenerator::Make(sink, matrix) : CPUGenerator::Make(sink, matrix);
}
...
int main(int argc, char** argv)
{
    ...
    auto rp = skresources::CachingResourceProvider::Make(skresources::DataURIResourceProviderProxy::Make(
        skresources::FileResourceProvider::Make(SkOSPath::Dirname(FLAGS_input[0]), predecode), predecode, fontMgr));
    auto data = SkData::MakeFromFileName(FLAGS_input[0]);
    auto precomp_interceptor = sk_make_sp<skottie_utils::ExternalAnimationPrecompInterceptor>(rp, "__");
    ...
    const auto build_animation = [&fontMgr, &rp, &data](const sk_sp<Logger>& logger, const sk_sp<skottie::PrecompInterceptor>& pint) {
    ...
    const auto scale_matrix = SkMatrix::RectToRect(SkRect::MakeSize(anim->size()), SkRect::MakeIWH(FLAGS_width, FLAGS_height), SkMatrix::kCenter_ScaleToFit);
    logger->report();

    const auto t0 = SkTPin(FLAGS_t0, 0.0, 1.0), t1 = SkTPin(FLAGS_t1, t0, 1.0), native_fps = anim->fps(), frame0 = anim->duration() * t0 * native_fps,
    ...
    SkTaskGroup tg;
    {
        ...
        tg.batch(frame_count, [&](int i) {
            ...
            const auto start = std::chrono::steady_clock::now();
            thread_local static auto* anim = build_animation(nullptr, precomp_interceptor).release();
            thread_local static auto* gen = singleton_generator ? singleton_generator.get() : FrameGenerator::Make(sink.get(), fmt, scale_matrix).release();
