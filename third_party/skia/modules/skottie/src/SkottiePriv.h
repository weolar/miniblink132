...
public:
    AnimationBuilder(sk_sp<ResourceProvider>, sk_sp<SkFontMgr>, sk_sp<PropertyObserver>, sk_sp<Logger>, sk_sp<MarkerObserver>, sk_sp<PrecompInterceptor>,
        sk_sp<ExpressionManager>, sk_sp<SkShapers::Factory>, Animation::Builder::Stats*, const SkSize& comp_size, float duration, float framerate,
        uint32_t flags);
    ...
    sk_sp<sksg::Transform> attachMatrix2D(const skjson::ObjectValue&, sk_sp<sksg::Transform>, bool auto_orient = false) const;
    ...
    {
        return fHasNontrivialBlending;
    }

    class AutoScope final {
    public:
        explicit AutoScope(const AnimationBuilder* builder)
    ...
    template <typename T> void attachDiscardableAdapter(sk_sp<T> adapter) const
    {
        if (adapter->isStatic()) {
    ...
    {
        using NodeType = typename std::decay<decltype(T::Make(std::forward<Args>(args)...)->node())>::type;
...
private:
    ...
    sk_sp<sksg::RenderNode> attachBlendMode(const skjson::ObjectValue&, sk_sp<sksg::RenderNode>) const;

    sk_sp<sksg::RenderNode> attachShape(const skjson::ArrayValue*, AttachShapeContext*, bool suppress_draws = false) const;
    ...
    sk_sp<sksg::RenderNode> attachExternalPrecompLayer(const skjson::ObjectValue&, const LayerInfo&) const;
    ...
    public:
        ...
        const skjson::ObjectValue& operator*() const
        {
            return *fInfo->fAsset;
        }
