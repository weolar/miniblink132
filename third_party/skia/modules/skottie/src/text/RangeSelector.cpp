...
template <typename T, typename TArray> T ParseEnum(const TArray& arr, const skjson::Value& jenum, const AnimationBuilder* abuilder, const char* warn_name)
{
...
template <RangeSelector::Units> struct UnitTraits;

template <> struct UnitTraits<RangeSelector::Units::kPercentage> {
...
template <> struct UnitTraits<RangeSelector::Units::kIndex> {
    static constexpr auto Defaults()
    {
...
public:
    CoverageProcessor(const TextAnimator::DomainMaps& maps, RangeSelector::Domain domain, RangeSelector::Mode mode, TextAnimator::ModulatorBuffer& dst)
        : fDst(dst)
...
sk_sp<RangeSelector> RangeSelector::Make(const skjson::ObjectValue* jrange, const AnimationBuilder* abuilder, AnimatablePropertyContainer* acontainer)
{
    if (!jrange) {
        return nullptr;
    }
    ...
    auto selector = sk_sp<RangeSelector>(
        new RangeSelector(ParseEnum<Units>(gUnitMap, (*jrange)["r"], abuilder, "units"), ParseEnum<Domain>(gDomainMap, (*jrange)["b"], abuilder, "domain"),
            ParseEnum<Mode>(gModeMap, (*jrange)["m"], abuilder, "mode"), ParseEnum<Shape>(gShapeMap, (*jrange)["sh"], abuilder, "shape")));
...
std::tuple<float, float> RangeSelector::resolve(size_t len) const
{
    ...
    const auto resolver = (fUnits == Units::kPercentage) ? UnitTraits<Units::kPercentage>::Resolve : UnitTraits<Units::kIndex>::Resolve;
 ...
 */
void RangeSelector::modulateCoverage(const TextAnimator::DomainMaps& maps, TextAnimator::ModulatorBuffer& mbuf) const
{
    ...
    const auto amount = SkTPin<float>(fAmount / 100, -1, 1), ease_lo = SkTPin<float>(fEaseLo / 100, -1, 1), ease_hi = SkTPin<float>(fEaseHi / 100, -1, 1);

    // Resolve to a float range in the given domain.
    const auto range = this->resolve(coverage_proc.size());
    auto r0 = std::get<0>(range), len = std::max(std::get<1>(range) - r0, std::numeric_limits<float>::epsilon());
