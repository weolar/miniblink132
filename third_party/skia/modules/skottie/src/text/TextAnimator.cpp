 ...
 * }
 */
sk_sp<TextAnimator> TextAnimator::Make(const skjson::ObjectValue* janimator, const AnimationBuilder* abuilder, AnimatablePropertyContainer* acontainer)
{
    if (!janimator) {
        return nullptr;
    }
...
TextAnimator::ResolvedProps TextAnimator::modulateProps(const ResolvedProps& props, float amount) const
{
    ...
    const auto lerp_color = [](SkColor c0, SkColor c1, float t) {
        const auto c0_4f = Sk4f_fromL32(c0), c1_4f = Sk4f_fromL32(c1), c_4f = c0_4f + (c1_4f - c0_4f) * t;
    ...
    if (fHasFillColor) {
        modulated_props.fill_color = lerp_color(props.fill_color, fTextProps.fill_color, clamped_amount);
    }
    if (fHasStrokeColor) {
        modulated_props.stroke_color = lerp_color(props.stroke_color, fTextProps.stroke_color, clamped_amount);
    }
    if (fHasFillOpacity) {
        // 255-based
        const auto alpha = lerp(SkColorGetA(props.fill_color), fTextProps.fill_opacity * 2.55f, clamped_amount);
        modulated_props.fill_color = SkColorSetA(modulated_props.fill_color, static_cast<U8CPU>(std::round(alpha)));
    }
    if (fHasStrokeOpacity) {
        // 255-based
        const auto alpha = lerp(SkColorGetA(props.stroke_color), fTextProps.stroke_opacity * 2.55f, clamped_amount);
        modulated_props.stroke_color = SkColorSetA(modulated_props.stroke_color, static_cast<U8CPU>(std::round(alpha)));
    }
    if (fHasOpacity) {
...
TextAnimator::TextAnimator(
    std::vector<sk_sp<RangeSelector>>&& selectors, const skjson::ObjectValue& jprops, const AnimationBuilder* abuilder, AnimatablePropertyContainer* acontainer)
