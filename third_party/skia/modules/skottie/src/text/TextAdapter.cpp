...
public:
    explicit GlyphTextNode(Shaper::ShapedGlyphs&& glyphs)
    ...
    const Shaper::ShapedGlyphs* glyphs() const
    {
        return &fGlyphs;
    }
...
public:
    ...
    SkRect onRevalidate(sksg::InvalidationController* ic, const SkMatrix& ctm) override
    {
        ...
        for (size_t i = 0; i < fFragCount; ++i) {
            ...
            fDecoratorInfo[i].fBounds = glyphs->computeBounds(Shaper::ShapedGlyphs::BoundsType::kTight);
    ...
    void onRender(SkCanvas* canvas, const RenderContext* ctx) const override
    {
        ...
        fDecorator->onDecorate(canvas, { SkSpan(fDecoratorInfo.get(), fFragCount), fScale });
    }
...
struct TextAdapter::PathInfo {
    ...
    SkM44 getMatrix(float distance, SkTextUtils::Align alignment) const
    {
        ...
        case SkTextUtils::Align::kLeft_Align:
            distance += fPathFMargin;
            break;
        case SkTextUtils::Align::kCenter_Align:
            distance += fPathFMargin + fPathLMargin;
            break;
        case SkTextUtils::Align::kRight_Align:
            distance += fPathLMargin;
            break;
        }
...
sk_sp<TextAdapter> TextAdapter::Make(const skjson::ObjectValue& jlayer, const AnimationBuilder* abuilder, sk_sp<SkFontMgr> fontmgr,
    sk_sp<CustomFont::GlyphCompMapper> custom_glyph_mapper, sk_sp<Logger> logger, sk_sp<::SkShapers::Factory> factory)
{
    ...
    const skjson::ObjectValue* jd = jt ? static_cast<const skjson::ObjectValue*>((*jt)["d"]) : nullptr;
    if (!jd) {
    ...
    const auto apg = jm ? SkTPin<int>(ParseDefault<int>((*jm)["g"], 1), 1, std::size(gGroupingMap)) : 1;

    auto adapter = sk_sp<TextAdapter>(
        new TextAdapter(std::move(fontmgr), std::move(custom_glyph_mapper), std::move(logger), std::move(factory), gGroupingMap[SkToSizeT(apg - 1)]));
...
TextAdapter::TextAdapter(sk_sp<SkFontMgr> fontmgr, sk_sp<CustomFont::GlyphCompMapper> custom_glyph_mapper, sk_sp<Logger> logger,
    sk_sp<SkShapers::Factory> factory, AnchorPointGrouping apg)
...
std::vector<sk_sp<sksg::RenderNode>> TextAdapter::buildGlyphCompNodes(Shaper::ShapedGlyphs& glyphs) const
{
            ...
            for (size_t i = 0; i < run.fSize; ++i) {
                ...
                if (auto gcomp = fCustomGlyphMapper->getGlyphComp(run.fFont.getTypeface(), gid)) {
                    ...
                    const auto m = SkMatrix::Translate(glyphs.fGlyphPos[goffset])
                        * SkMatrix::Scale(fText->fTextSize * fTextShapingScale, fText->fTextSize * fTextShapingScale);
...
void TextAdapter::addFragment(Shaper::Fragment& frag, sksg::Group* container)
{
    ...
    rec.fMatrixNode = sksg::Matrix<SkM44>::Make(SkM44::Translate(frag.fOrigin.x(), frag.fOrigin.y()));
    ...
    draws.reserve(draws.size() + static_cast<size_t>(fText->fHasFill) + static_cast<size_t>(fText->fHasStroke));
...
void TextAdapter::reshape()
{
    ...
    static constexpr float kMinSize = 0.1f, kMaxSize = 1296.0f;
    ...
    auto shape_result = Shaper::Shape(fText->fText, text_desc, fText->fBox, fFontMgr, fShapingFactory);
    ...
    if (fLogger) {
        ...
        if (shape_result.fMissingGlyphCount > 0) {
            const auto msg = SkStringPrintf("Missing %zu glyphs for '%s'.", shape_result.fMissingGlyphCount, fText->fText.c_str());
    ...
    if (SHOW_LAYOUT_BOXES) {
        ...
        fRoot->addChild(sksg::Draw::Make(sksg::Rect::Make(shape_result.computeVisualBounds()), std::move(bounds_color)));
        ...
        if (fPathInfo) {
            ...
            fRoot->addChild(sksg::Draw::Make(sksg::Path::Make(static_cast<SkPath>(fPathInfo->fPath)), std::move(path_color)));
        }
    }
...
void TextAdapter::onSync()
{
    ...
    auto compute_linewide_props = [this](const TextAnimator::ModulatorBuffer& buf, const TextAnimator::DomainSpan& line_span) {
    ...
    for (const auto& line_span : fMaps.fLinesMap) {
        ...
        for (size_t i = line_span.fOffset; i < line_span.fOffset + line_span.fCount; ++i) {
            ...
            const auto track_before = i > line_span.fOffset ? props.tracking * 0.5f : 0.0f,
                       track_after = i < line_span.fOffset + line_span.fCount - 1 ? props.tracking * 0.5f : 0.0f;

            const auto frag_offset = current_line_offset + SkV2 { align_offset + tracking_acc + track_before, 0 };
...
SkV2 TextAdapter::fragmentAnchorPoint(const FragmentRec& rec, const SkV2& grouping_alignment, const TextAnimator::DomainSpan* grouping_span) const
{
    ...
    auto anchor_box = [&]() -> SkRect {
        ...
        case AnchorPointGrouping::kLine: {
            ...
            return make_box(first_span_fragment.fOrigin, grouping_span->fAdvance, grouping_span->fAscent);
        }
    ...
    const auto ap = SkV2 { ab.centerX() + ab.width() * 0.5f * grouping_alignment.x, ab.centerY() + ab.height() * 0.5f * grouping_alignment.y };

    // The anchor point is relative to the fragment position.
    return ap - SkV2 { rec.fOrigin.fX, rec.fOrigin.fY };
}

SkM44 TextAdapter::fragmentMatrix(const TextAnimator::ResolvedProps& props, const FragmentRec& rec, const SkV2& frag_offset) const
{
    const SkV3 pos = { props.position.x + rec.fOrigin.fX + frag_offset.x, props.position.y + rec.fOrigin.fY + frag_offset.y, props.position.z };
    ...
    const auto align_offset = align_factor(fText->fHAlign) * (fPathInfo->pathLength() - fText->fBox.width());
    ...
    return fPathInfo->getMatrix(path_distance, fText->fHAlign) * SkM44::Translate(0, rel_pos.y, pos.z);
}

void TextAdapter::pushPropsToFragment(const TextAnimator::ResolvedProps& props, const FragmentRec& rec, const SkV2& frag_offset, const SkV2& grouping_alignment,
    const TextAnimator::DomainSpan* grouping_span) const
{
    ...
    rec.fMatrixNode->setMatrix(this->fragmentMatrix(props, rec, anchor_point + frag_offset) * SkM44::Rotate({ 1, 0, 0 }, SkDegreesToRadians(props.rotation.x))
        * SkM44::Rotate({ 0, 1, 0 }, SkDegreesToRadians(props.rotation.y)) * SkM44::Rotate({ 0, 0, 1 }, SkDegreesToRadians(props.rotation.z))
        * SkM44::Scale(props.scale.x, props.scale.y, props.scale.z) * SkM44::Translate(-anchor_point.x, -anchor_point.y, 0));

    const auto scale_alpha = [](SkColor c, float o) { return SkColorSetA(c, SkScalarRoundToInt(o * SkColorGetA(c))); };
