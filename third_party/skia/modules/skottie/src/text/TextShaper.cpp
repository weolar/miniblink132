...
public:
    ResultBuilder(const Shaper::TextDesc& desc, const SkRect& box, const sk_sp<SkFontMgr>& fontmgr, const sk_sp<SkShapers::Factory>& shapingFactory)
        : fDesc(desc)
        , fBox(box)
    ...
    Buffer runBuffer(const RunInfo& info) override
    {
        ...
        return { fLineGlyphs.get() + run_start_index, fLinePos.get() + run_start_index, nullptr, fLineClusters.get() + run_start_index,
    ...
    void commitLine() override
    {
        ...
        auto adjust_trailing_whitespace = [this]() {
            ...
            fLineRuns.back().fFont.getWidths(fLineGlyphs.data() + fLineGlyphCount - ws_count, SkToInt(ws_count), fAdvanceBuffer.data(), nullptr);
            ...
            std::transform(fLinePos.data(), fLinePos.data() + fLineGlyphCount, fLinePos.data(), [&offset](SkPoint pos) {
        ...
        const auto commit_proc = (fDesc.fFlags & Shaper::Flags::kFragmentGlyphs) ? &ResultBuilder::commitFragementedRun : &ResultBuilder::commitConsolidatedRun;

        size_t run_offset = 0;
        for (const auto& rec : fLineRuns) {
            SkASSERT(run_offset < fLineGlyphCount);
            (this->*commit_proc)(rec, fLineGlyphs.get() + run_offset, fLinePos.get() + run_offset, fLineClusters.get() + run_offset, fLineCount);
    ...
    void shapeLine(const char* start, const char* end, size_t utf8_offset)
    {
        ...
        if (start == end) {
            ...
            if (fDesc.fFlags & Shaper::Flags::kFragmentGlyphs) {
                fResult.fFragments.push_back({ Shaper::ShapedGlyphs(), { fBox.x(), fBox.y() }, 0, 0, fLineCount - 1, false });
            }
        ...
        const auto shape_width = fDesc.fLinebreak == Shaper::LinebreakPolicy::kExplicit ? SK_ScalarMax : fBox.width();
        const auto shape_ltr = fDesc.fDirection == Shaper::Direction::kLTR;
        const size_t utf8_bytes = SkToSizeT(end - start);

        static constexpr uint8_t kBidiLevelLTR = 0, kBidiLevelRTL = 1;
        const auto lang_iter = fDesc.fLocale ? std::make_unique<SkShaper::TrivialLanguageRunIterator>(fDesc.fLocale, utf8_bytes)
        ...
        const auto font_iter = std::make_unique<SkShaper::TrivialFontRunIterator>(fFont, utf8_bytes);
#else
        const auto font_iter = SkShaper::MakeFontMgrRunIterator(start, utf8_bytes, fFont,
            fFontMgr ? fFontMgr : SkFontMgr::RefEmpty(), // used as fallback
            fDesc.fFontFamily, fFont.getTypeface()->fontStyle(), lang_iter.get());
#endif

        std::unique_ptr<SkShaper::BiDiRunIterator> bidi_iter
            = fShapingFactory->makeBidiRunIterator(start, utf8_bytes, shape_ltr ? kBidiLevelLTR : kBidiLevelRTL);
        if (!bidi_iter) {
            bidi_iter = std::make_unique<SkShaper::TrivialBiDiRunIterator>(shape_ltr ? kBidiLevelLTR : kBidiLevelRTL, utf8_bytes);
        }

        constexpr SkFourByteTag unknownScript = SkSetFourByteTag('Z', 'z', 'z', 'z');
        std::unique_ptr<SkShaper::ScriptRunIterator> scpt_iter = fShapingFactory->makeScriptRunIterator(start, utf8_bytes, unknownScript);
        if (!scpt_iter) {
        ...
        fShaper->shape(start, utf8_bytes, *font_iter, *bidi_iter, *scpt_iter, *lang_iter, nullptr, 0, shape_width, this);
        fUTF8 = nullptr;
    }

private:
    void commitFragementedRun(const skottie::Shaper::RunRec& run, const SkGlyphID* glyphs, const SkPoint* pos, const uint32_t* clusters, uint32_t line_index)
    {
        float ascent = 0;
        ...
        for (size_t i = 0; i < run.fSize; ++i) {
            const auto advance = (fDesc.fFlags & Shaper::Flags::kTrackFragmentAdvanceAscent) ? fAdvanceBuffer[SkToInt(i)] : 0.0f;

            fResult.fFragments.push_back({ {
                                               { { run.fFont, 1 } },
                                               { glyphs[i] },
                                               { { 0, 0 } },
                                               fDesc.fFlags & Shaper::kClusters ? std::vector<size_t> { fUTF8Offset + clusters[i] } : std::vector<size_t>({}),
                                           },
                { fBox.x() + pos[i].fX, fBox.y() + pos[i].fY }, advance, ascent, line_index, is_whitespace(fUTF8[clusters[i]]) });
    ...
    void commitConsolidatedRun(const skottie::Shaper::RunRec& run, const SkGlyphID* glyphs, const SkPoint* pos, const uint32_t* clusters, uint32_t)
    {
...
Shaper::Result ShapeImpl(const SkString& txt, const Shaper::TextDesc& desc, const SkRect& box, const sk_sp<SkFontMgr>& fontmgr,
    const sk_sp<SkShapers::Factory>& shapingFactory, SkSize* shaped_size)
{
...
bool result_fits(const Shaper::Result& res, const SkSize& res_size, const SkRect& box, const Shaper::TextDesc& desc)
{
...
Shaper::Result ShapeToFit(
    const SkString& txt, const Shaper::TextDesc& orig_desc, const SkRect& box, const sk_sp<SkFontMgr>& fontmgr, const sk_sp<SkShapers::Factory>& shapingFactory)
{
    ...
    const auto min_scale = std::max(desc.fMinTextSize / desc.fTextSize, 0.0f), max_scale = std::max(desc.fMaxTextSize / desc.fTextSize, min_scale);
...
Shaper::Result Shaper::Shape(
    const SkString& text, const TextDesc& desc, const SkPoint& point, const sk_sp<SkFontMgr>& fontmgr, const sk_sp<SkShapers::Factory>& shapingFactory)
{
    ...
    return (desc.fResize == ResizePolicy::kScaleToFit || desc.fResize == ResizePolicy::kDownscaleToFit) // makes no sense in point mode
        ? Result()
        : ShapeImpl(adjText, desc, SkRect::MakeEmpty().makeOffset(point.x(), point.y()), fontmgr, shapingFactory, nullptr);
}

Shaper::Result Shaper::Shape(
    const SkString& text, const TextDesc& desc, const SkRect& box, const sk_sp<SkFontMgr>& fontmgr, const sk_sp<SkShapers::Factory>& shapingFactory)
{
    const AdjustedText adjText(text, desc, shapingFactory->getUnicode());
    ...
    case ResizePolicy::kDownscaleToFit: {
        ...
        return result_fits(result, size, box, desc) ? result : ShapeToFit(adjText, desc, box, fontmgr, shapingFactory);
    }
    }
...
SkRect Shaper::ShapedGlyphs::computeBounds(BoundsType btype) const
{
    ...
    for (const auto& run : fRuns) {
        ...
        case BoundsType::kTight: {
            ...
            run.fFont.getBounds(fGlyphIDs.data() + offset, SkToInt(run.fSize), glyphBounds.data(), nullptr);
...
void Shaper::ShapedGlyphs::draw(SkCanvas* canvas, const SkPoint& origin, const SkPaint& paint) const
{
    size_t offset = 0;
    for (const auto& run : fRuns) {
        canvas->drawGlyphs(SkToInt(run.fSize), fGlyphIDs.data() + offset, fGlyphPos.data() + offset, origin, run.fFont, paint);
        offset += run.fSize;
    }
}
...
SkRect Shaper::Result::computeVisualBounds() const
{
    ...
    for (const auto& frag : fFragments) {
        bounds.join(frag.fGlyphs.computeBounds(ShapedGlyphs::BoundsType::kTight).makeOffset(frag.fOrigin));
    }
...
#if !defined(SK_DISABLE_LEGACY_SHAPER_FACTORY)
Shaper::Result Shaper::Shape(const SkString& text, const TextDesc& desc, const SkPoint& point, const sk_sp<SkFontMgr>& fontmgr)
{
    return Shaper::Shape(text, desc, point, fontmgr, SkShapers::BestAvailable());
}

Shaper::Result Shaper::Shape(const SkString& text, const TextDesc& desc, const SkRect& box, const sk_sp<SkFontMgr>& fontmgr)
{
