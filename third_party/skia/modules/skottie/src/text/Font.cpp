...
bool CustomFont::Builder::parseGlyph(const AnimationBuilder* abuilder, const skjson::ObjectValue& jchar)
{
    // Glyph encoding:
    //     {
    //         "ch": "t",
...
bool CustomFont::Builder::ParseGlyphPath(const skottie::internal::AnimationBuilder* abuilder, const skjson::ObjectValue& jdata, SkPath* path)
{
    // Glyph path encoding:
    //
    //   "data": {
...
sk_sp<sksg::RenderNode> CustomFont::Builder::ParseGlyphComp(const AnimationBuilder* abuilder, const skjson::ObjectValue& jdata, SkSize* glyph_size)
{
    // Glyph comp encoding:
    //
    ...
    AnimationBuilder::LayerInfo linfo { { 0, 0 }, ParseDefault<float>(jdata["ip"], 0.0f), ParseDefault<float>(jdata["op"], 0.0f) };
    ...
    *glyph_size = { linfo.fSize.fWidth * kPtScale, linfo.fSize.fHeight * kPtScale };
    ...
    sk_sp<sksg::Transform> glyph_transform = sksg::Matrix<SkMatrix>::Make(SkMatrix::Scale(kPtScale, kPtScale));
    ...
    return sksg::TransformEffect::Make(abuilder->attachPrecompLayer(jdata, &linfo), std::move(glyph_transform));
}

std::unique_ptr<CustomFont> CustomFont::Builder::detach()
{
    return std::unique_ptr<CustomFont>(new CustomFont(std::move(fGlyphComps), fCustomBuilder.detach()));
}
...
sk_sp<sksg::RenderNode> CustomFont::GlyphCompMapper::getGlyphComp(const SkTypeface* tf, SkGlyphID gid) const
{
