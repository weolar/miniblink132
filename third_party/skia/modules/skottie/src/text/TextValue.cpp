...
bool Parse(const skjson::Value& jv, const internal::AnimationBuilder& abuilder, TextValue* v)
{
    ...
    v->fHAlign = gAlignMap[std::min<size_t>(ParseDefault<size_t>((*jtxt)["j"], 0), std::size(gAlignMap) - 1)];
    ...
    v->fDirection = gDirectionMap[std::min(ParseDefault<size_t>((*jtxt)["d"], 0), std::size(gDirectionMap) - 1)];
    ...
    v->fResize = gResizeMap[std::min(std::max(ParseDefault<size_t>((*jtxt)["rs"], 0), ParseDefault<size_t>((*jtxt)["sk_rs"], 0)), std::size(gResizeMap) - 1)];
    ...
    v->fLineBreak = v->fBox.isEmpty() ? Shaper::LinebreakPolicy::kExplicit : Shaper::LinebreakPolicy::kParagraph;
    ...
    if (text_mode >= 0) {
        // Explicit text mode.
        v->fLineBreak = (text_mode == 0) ? Shaper::LinebreakPolicy::kExplicit // 'm': 0 -> point text
    ...
    v->fCapitalization = gCapMap[std::min<size_t>(ParseDefault<size_t>((*jtxt)["ca"], 0), std::size(gCapMap) - 1)];
    ...
    if (skottie::Parse((*jtxt)["vj"], &vj)) {
        ...
        case 4:
            ...
            break;
        default:
            abuilder.log(Logger::Level::kWarning, nullptr, "Ignoring unknown 'sk_vj' value: %zu", vj);
            break;
        }
    }
    ...
    if (v->fHasStroke) {
        ...
        v->fPaintOrder = ParseDefault((*jtxt)["of"], true) ? TextPaintOrder::kFillStroke : TextPaintOrder::kStrokeFill;
        ...
        v->fStrokeJoin = gJoins[std::min<size_t>(ParseDefault<size_t>((*jtxt)["lj"], 1) - 1, std::size(gJoins) - 1)];
    }
