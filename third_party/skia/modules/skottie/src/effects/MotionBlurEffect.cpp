...
sk_sp<MotionBlurEffect> MotionBlurEffect::Make(
    sk_sp<Animator> animator, sk_sp<sksg::RenderNode> child, size_t samples_per_frame, float shutter_angle, float shutter_phase)
{
    ...
    const auto samples_duration = shutter_angle / 360, phase = shutter_phase / 360, dt = samples_duration / (samples_per_frame - 1);

    return sk_sp<MotionBlurEffect>(new MotionBlurEffect(std::move(animator), std::move(child), samples_per_frame, phase, dt));
}

MotionBlurEffect::MotionBlurEffect(sk_sp<Animator> animator, sk_sp<sksg::RenderNode> child, size_t samples, float phase, float dt)
...
void MotionBlurEffect::renderToRaster8888Pow2Samples(SkCanvas* canvas, const RenderContext* ctx) const
{
...
void MotionBlurEffect::onRender(SkCanvas* canvas, const RenderContext* ctx) const
{
    ...
    if (isolate_frames) {
        ...
        frame_ctx = frame_ctx.modulateOpacity(frame_alpha).modulateBlender(SkBlender::Mode(SkBlendMode::kPlus));
    }
