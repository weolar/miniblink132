...
static constexpr char gDisplacementSkSL[] = "uniform shader child;"
...
{
    static const SkRuntimeEffect* effect = SkRuntimeEffect::MakeForShader(SkString(gDisplacementSkSL)).effect.release();
    if (0 && !effect) {
...
public:
    ...
    static sk_sp<DisplacementNode> Make(sk_sp<RenderNode> child, const SkSize& child_size, sk_sp<RenderNode> displ, const SkSize& displ_size)
    {
        if (!child || !displ) {
            return nullptr;
        }
...
private:
    ...
    sk_sp<SkShader> buildEffectShader(sksg::InvalidationController* ic, const SkMatrix& ctm)
    {
        ...
        auto get_content_picture = [](const sk_sp<sksg::RenderNode>& node, sksg::InvalidationController* ic, const SkMatrix& ctm) {
            if (!node) {
        ...
        const auto child_content = get_content_picture(this->children()[0], ic, ctm), displ_content = get_content_picture(fDisplSource, ic, ctm);
        if (!child_content || !displ_content) {
            return nullptr;
        }

        const auto child_tile = SkRect::MakeSize(fChildSize);
        auto child_shader = child_content->makeShader(fChildTileMode, fChildTileMode, SkFilterMode::kLinear, nullptr, &child_tile);
        ...
        auto displ_shader = displ_content->makeShader(displ_mode, displ_mode, SkFilterMode::kLinear, &displ_matrix, &displ_tile);
    ...
    SkMatrix displacementMatrix() const
    {
        switch (fPos) {
        case Pos::kCenter:
            return SkMatrix::Translate((fChildSize.fWidth - fDisplSize.fWidth) / 2, (fChildSize.fHeight - fDisplSize.fHeight) / 2);
        case Pos::kStretch:
            return SkMatrix::Scale(fChildSize.fWidth / fDisplSize.fWidth, fChildSize.fHeight / fDisplSize.fHeight);
        case Pos::kTile:
            return SkMatrix::I();
        }
        SkUNREACHABLE;
...
class DisplacementMapAdapter final : public DiscardableAdapterBase<DisplacementMapAdapter, DisplacementNode> {
public:
    DisplacementMapAdapter(const skjson::ArrayValue& jprops, const AnimationBuilder* abuilder, sk_sp<DisplacementNode> node)
    ...
    static std::tuple<sk_sp<sksg::RenderNode>, SkSize> GetDisplacementSource(const skjson::ArrayValue& jprops, const EffectBuilder* ebuilder)
    {
...
private:
    ...
    template <typename E> E ToEnum(float v)
    {
    ...
    ScalarValue fHorizontalSelector = 0, fVerticalSelector = 0, fMaxHorizontal = 0, fMaxVertical = 0, fMapBehavior = 0, fEdgeBehavior = 0, fExpandOutput = 0;
...
sk_sp<sksg::RenderNode> EffectBuilder::attachDisplacementMapEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
    ...
    return fBuilder->attachDiscardableAdapter<DisplacementMapAdapter>(jprops, fBuilder, std::move(displ_node));
}
