...
{
    static const SkRuntimeEffect* effect = SkRuntimeEffect::MakeForShader(SkStringPrintf(gSphereSkSL, gFancyLightSkSL), {}).effect.release();
    if (0 && !effect) {
...
{
    static const SkRuntimeEffect* effect = SkRuntimeEffect::MakeForShader(SkStringPrintf(gSphereSkSL, gBasicLightSkSL), {}).effect.release();
    SkASSERT(effect);
...
private:
    ...
    SkRect onRevalidate(sksg::InvalidationController* ic, const SkMatrix& ctm) override
    {
        ...
        if (fSide != RenderSide::kInside) {
            ...
            fSphereShader = fSphereShader ? SkShaders::Blend(SkBlendMode::kSrcOver, std::move(fSphereShader), std::move(outside)) : std::move(outside);
        }
...
private:
    void onSync() override
    {
        ...
        const auto rotation = [](ScalarValue order, ScalarValue x, ScalarValue y, ScalarValue z) {
            const SkM44 rx = SkM44::Rotate({ 1, 0, 0 }, SkDegreesToRadians(x)), ry = SkM44::Rotate({ 0, 1, 0 }, SkDegreesToRadians(y)),
        ...
        const auto light_vec = [](float height, float direction) {
            float z = std::sin(height * SK_ScalarPI / 2), r = std::sqrt(1 - z * z), x = std::cos(direction) * r, y = std::sin(direction) * r;
        ...
        sph->setLightVec(light_vec(SkTPin(fLightHeight * 0.01f, -1.0f, 1.0f), SkDegreesToRadians(fLightDirection - 90)));
    ...
    ScalarValue fLightIntensity = 0, fLightHeight = 0, fLightDirection = 0, fAmbient = 100, fDiffuse = 0, fSpecular = 0, fRoughness = 0.5f;
...
sk_sp<sksg::RenderNode> EffectBuilder::attachSphereEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
