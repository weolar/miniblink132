...
public:
    static sk_sp<TintAdapter> Make(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer, const AnimationBuilder& abuilder)
    {
...
private:
    TintAdapter(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer, const AnimationBuilder& abuilder)
        ...
        , fFilterNode(sksg::GradientColorFilter::Make(std::move(layer), fColorNode0, fColorNode1))
    {
        ...
        EffectBinder(jprops, abuilder, this).bind(kMapBlackTo_Index, fMapBlackTo).bind(kMapWhiteTo_Index, fMapWhiteTo).bind(kAmount_Index, fAmount);
    }
    ...
    ColorValue fMapBlackTo, fMapWhiteTo;
    ScalarValue fAmount = 0;
};

} // namespace

sk_sp<sksg::RenderNode> EffectBuilder::attachTintEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
