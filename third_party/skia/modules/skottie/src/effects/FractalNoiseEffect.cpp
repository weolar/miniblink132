...
static constexpr char gNoiseEffectSkSL[] = "uniform float3x3 u_submatrix;" // sublayer transform
...
static constexpr char gFilterNearestSkSL[] = "float filter(float2 xy) {"
                                             "return sample_noise(xy);"
                                             "}";

static constexpr char gFilterLinearSkSL[] = "float filter(float2 xy) {"
                                            "xy -= 0.5;"
...
static constexpr char gFilterSoftLinearSkSL[] = "float filter(float2 xy) {"
                                                "xy -= 0.5;"
...
static constexpr char gFractalBasicSkSL[] = "float fractal(float n) {"
                                            "return n;"
                                            "}";

static constexpr char gFractalTurbulentBasicSkSL[] = "float fractal(float n) {"
                                                     "return 2*abs(0.5 - n);"
                                                     "}";

static constexpr char gFractalTurbulentSmoothSkSL[] = "float fractal(float n) {"
                                                      "n = 2*abs(0.5 - n);"
                                                      "return n*n;"
                                                      "}";

static constexpr char gFractalTurbulentSharpSkSL[] = "float fractal(float n) {"
...
{
    auto result = SkRuntimeEffect::MakeForShader(SkStringPrintf(gNoiseEffectSkSL, filter, fractal, loops), {});
...
{
    static constexpr char const* gFilters[] = { gFilterNearestSkSL, gFilterLinearSkSL, gFilterSoftLinearSkSL };

    static constexpr char const* gFractals[] = { gFractalBasicSkSL, gFractalTurbulentBasicSkSL, gFractalTurbulentSmoothSkSL, gFractalTurbulentSharpSkSL };
    ...
    static constexpr BinInfo kLoopBins[] = { { 8, 20 }, { 4, 8 }, { 3, 4 }, { 2, 3 }, { 1, 2 }, { 0, 1 } };
    ...
    static SkRuntimeEffect* kEffectCache[std::size(kLoopBins)][std::size(gFilters)][std::size(gFractals)];

    const size_t bin = bin_index(octaves);

    auto& effect = kEffectCache[bin][static_cast<size_t>(filter)][static_cast<size_t>(fractal)];
    if (!effect) {
        effect = make_noise_effect(kLoopBins[bin].loops, gFilters[static_cast<size_t>(filter)], gFractals[static_cast<size_t>(fractal)]).release();
    }
...
public:
    explicit FractalNoiseNode(sk_sp<RenderNode> child)
...
private:
    ...
    void onRender(SkCanvas* canvas, const RenderContext* ctx) const override
    {
        ...
        const auto local_ctx = ScopedRenderContext(canvas, ctx).setIsolation(bounds, canvas->getTotalMatrix(), true);
...
class FractalNoiseAdapter final : public DiscardableAdapterBase<FractalNoiseAdapter, FractalNoiseNode> {
public:
    FractalNoiseAdapter(const skjson::ArrayValue& jprops, const AnimationBuilder* abuilder, sk_sp<FractalNoiseNode> node)
...
private:
    std::tuple<SkV2, float> noise() const
    {
        ...
        // must be integral.
        const float evo_rad = SkDegreesToRadians(fEvolution), rev_rad = std::max(fCycleRevolutions, 1.0f) * SK_FloatPI * 2,
                    cycle = fCycleEvolution ? SkScalarRoundToScalar(rev_rad * kEvolutionScale) : SK_ScalarMax,
                    // Adjust scale when cycling to ensure an integral period (post scaling).
            scale = fCycleEvolution ? cycle / rev_rad : kEvolutionScale, offset = SkRandom(static_cast<uint32_t>(fRandomSeed)).nextRangeU(0, 100),
    ...
    SkMatrix shaderMatrix() const
    {
        ...
        const auto scale = (SkScalarRoundToInt(fUniformScaling) == 1) ? SkV2 { fScale, fScale } : SkV2 { fScaleWidth, fScaleHeight };

        return SkMatrix::Translate(fOffset.x, fOffset.y) * SkMatrix::Scale(SkTPin(scale.x, 1.0f, 10000.0f) * 0.01f, SkTPin(scale.y, 1.0f, 10000.0f) * 0.01f)
    ...
    SkMatrix subMatrix() const
    {
        ...
        return SkMatrix::Translate(-fSubOffset.x * 0.01f, -fSubOffset.y * 0.01f) * SkMatrix::RotateDeg(-fSubRotation) * SkMatrix::Scale(scale, scale);
    }
...
sk_sp<sksg::RenderNode> EffectBuilder::attachFractalNoiseEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
    auto fractal_noise = sk_make_sp<FractalNoiseNode>(std::move(layer));

    return fBuilder->attachDiscardableAdapter<FractalNoiseAdapter>(jprops, fBuilder, std::move(fractal_noise));
}
