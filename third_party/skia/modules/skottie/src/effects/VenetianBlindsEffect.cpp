...
public:
    static sk_sp<VenetianBlindsAdapter> Make(
        const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer, const SkSize& layer_size, const AnimationBuilder* abuilder)
    {
        return sk_sp<VenetianBlindsAdapter>(new VenetianBlindsAdapter(jprops, std::move(layer), layer_size, abuilder));
    }

private:
    VenetianBlindsAdapter(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer, const SkSize& ls, const AnimationBuilder* abuilder)
    ...
    MaskInfo onMakeMask() const override
    {
        ...
        const auto t = fCompletion * 0.01f, size = std::max(1.0f, fWidth), angle = SkDegreesToRadians(-fDirection),
        ...
        const auto g01 = std::max(0.0f, 0.5f * (1 + sk_ieee_float_divide(0 - t, df))), g23 = std::min(1.0f, 0.5f * (1 + sk_ieee_float_divide(1 - t, df)));
        SkASSERT(0 <= g01 && g01 <= g23 && g23 <= 1);

        const SkColor c01 = SkColorSetA(SK_ColorWHITE, SkScalarRoundToInt(g01 * 0xff)), c23 = SkColorSetA(SK_ColorWHITE, SkScalarRoundToInt(g23 * 0xff)),
        ...
        const auto center = SkPoint::Make(0.5f * this->layerSize().width(), 0.5f * this->layerSize().height()),
        ...
        return { SkGradientShader::MakeLinear(pts, colors, pos, std::size(colors), SkTileMode::kRepeat), true };
    }
...
sk_sp<sksg::RenderNode> EffectBuilder::attachVenetianBlindsEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
    return fBuilder->attachDiscardableAdapter<VenetianBlindsAdapter>(jprops, std::move(layer), fLayerSize, fBuilder);
}
