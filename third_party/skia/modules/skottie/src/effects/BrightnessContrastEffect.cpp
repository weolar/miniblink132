...
static constexpr char CONTRAST_EFFECT[] = "uniform half a;"
                                          "uniform half b;"
                                          "uniform half c;"
...
static constexpr char BRIGHTNESS_EFFECT[] = "uniform half a;"
...
class BrightnessContrastAdapter final : public DiscardableAdapterBase<BrightnessContrastAdapter, sksg::ExternalColorFilter> {
public:
    BrightnessContrastAdapter(const skjson::ArrayValue& jprops, const AnimationBuilder& abuilder, sk_sp<sksg::RenderNode> layer)
        ...
        , fContrastEffect(SkRuntimeEffect::MakeForColorFilter(SkString(CONTRAST_EFFECT)).effect)
    {
        ...
        EffectBinder(jprops, abuilder, this).bind(kBrightness_Index, fBrightness).bind(kContrast_Index, fContrast).bind(kUseLegacy_Index, fUseLegacy);
    }

private:
    void onSync() override
    {
        this->node()->setColorFilter(SkScalarRoundToInt(fUseLegacy) ? this->makeLegacyCF() : this->makeCF());
    }
    ...
    sk_sp<SkColorFilter> makeCF() const
    {
        ...
        auto b_eff = SkScalarNearlyZero(brightness) ? nullptr : fBrightnessEffect->makeColorFilter(make_brightness_coeffs(brightness)),
             c_eff = SkScalarNearlyZero(fContrast) ? nullptr : fContrastEffect->makeColorFilter(make_contrast_coeffs(contrast));
...
sk_sp<sksg::RenderNode> EffectBuilder::attachBrightnessContrastEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
    return fBuilder->attachDiscardableAdapter<BrightnessContrastAdapter>(jprops, *fBuilder, std::move(layer));
}
