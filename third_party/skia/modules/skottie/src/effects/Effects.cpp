...
EffectBuilder::EffectBuilder(const AnimationBuilder* abuilder, const SkSize& layer_size, CompositionBuilder* cbuilder)
    : fBuilder(abuilder)
...
EffectBuilder::EffectBuilderT EffectBuilder::findBuilder(const skjson::ObjectValue& jeffect) const
{
    ...
    if (mn) {
        ...
        const auto* binfo = std::lower_bound(
            std::begin(gBuilderInfo), std::end(gBuilderInfo), key, [](const BuilderInfo& a, const BuilderInfo& b) { return strcmp(a.fName, b.fName) < 0; });
...
sk_sp<sksg::RenderNode> EffectBuilder::attachEffects(const skjson::ArrayValue& jeffects, sk_sp<sksg::RenderNode> layer) const
{
    if (!layer) {
        return nullptr;
    }
...
sk_sp<sksg::RenderNode> EffectBuilder::attachStyles(const skjson::ArrayValue& jstyles, sk_sp<sksg::RenderNode> layer) const
{
    ...
    using StyleBuilder = sk_sp<sksg::RenderNode> (EffectBuilder::*)(const skjson::ObjectValue&, sk_sp<sksg::RenderNode>) const;
    ...
    for (const skjson::ObjectValue* jstyle : jstyles) {
        ...
        const auto style_type = ParseDefault<size_t>((*jstyle)["ty"], std::numeric_limits<size_t>::max());
        auto builder = style_type < std::size(gStyleBuilders) ? gStyleBuilders[style_type] : nullptr;
...
const skjson::Value& EffectBuilder::GetPropValue(const skjson::ArrayValue& jprops, size_t prop_index)
{
