...
struct ChannelMapper {
    ...
    {
        auto in_0 = fInBlack, in_1 = fInWhite, out_0 = fOutBlack, out_1 = fOutWhite, g = sk_ieee_float_divide(1, std::max(fGamma, 0.0f));
...
class EasyLevelsEffectAdapter final : public DiscardableAdapterBase<EasyLevelsEffectAdapter, sksg::ExternalColorFilter> {
public:
    EasyLevelsEffectAdapter(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer, const AnimationBuilder* abuilder)
...
private:
    void onSync() override
    {
        ...
        this->node()->setColorFilter(SkColorFilters::TableARGB(channel == kA_Channel ? lut.data() : nullptr,
            channel == kR_Channel || channel == kRGB_Channel ? lut.data() : nullptr, channel == kG_Channel || channel == kRGB_Channel ? lut.data() : nullptr,
...
class ProLevelsEffectAdapter final : public DiscardableAdapterBase<ProLevelsEffectAdapter, sksg::ExternalColorFilter> {
public:
    ProLevelsEffectAdapter(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer, const AnimationBuilder* abuilder)
        : INHERITED(sksg::ExternalColorFilter::Make(std::move(layer)))
    {
        enum : size_t {
            //    kHistChan_Index =  0,
...
private:
    void onSync() override
    {
        ...
        auto cf = SkColorFilters::TableARGB(fAMapper.build_lut(a_lut_storage, fClip), fRMapper.build_lut(r_lut_storage, fClip),
        ...
        if (const auto* rgb_lut = fRGBMapper.build_lut(a_lut_storage, fClip)) {
            cf = SkColorFilters::Compose(SkColorFilters::TableARGB(nullptr, rgb_lut, rgb_lut, rgb_lut), std::move(cf));
        }
...
sk_sp<sksg::RenderNode> EffectBuilder::attachEasyLevelsEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
    return fBuilder->attachDiscardableAdapter<EasyLevelsEffectAdapter>(jprops, std::move(layer), fBuilder);
}

sk_sp<sksg::RenderNode> EffectBuilder::attachProLevelsEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
    return fBuilder->attachDiscardableAdapter<ProLevelsEffectAdapter>(jprops, std::move(layer), fBuilder);
}
