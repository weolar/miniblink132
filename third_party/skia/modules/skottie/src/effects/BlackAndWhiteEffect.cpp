...
{
    static constexpr char BLACK_AND_WHITE_EFFECT[] = "uniform half kR, kY, kG, kC, kB, kM;"
    ...
    static const SkRuntimeEffect* effect = SkRuntimeEffect::MakeForColorFilter(SkString(BLACK_AND_WHITE_EFFECT)).effect.release();
    SkASSERT(effect);

    return sk_ref_sp(effect);
}

class BlackAndWhiteAdapter final : public DiscardableAdapterBase<BlackAndWhiteAdapter, sksg::ExternalColorFilter> {
public:
    BlackAndWhiteAdapter(const skjson::ArrayValue& jprops, const AnimationBuilder& abuilder, sk_sp<sksg::RenderNode> layer)
...
private:
    void onSync() override
    {
        ...
        this->node()->setColorFilter(fEffect->makeColorFilter(SkData::MakeWithCopy(&coeffs, sizeof(coeffs))));
    }
...
sk_sp<sksg::RenderNode> EffectBuilder::attachBlackAndWhiteEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
    return fBuilder->attachDiscardableAdapter<BlackAndWhiteAdapter>(jprops, *fBuilder, std::move(layer));
}
