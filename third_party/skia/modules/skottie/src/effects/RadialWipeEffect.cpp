...
protected:
    ...
    SkRect onRevalidate(sksg::InvalidationController* ic, const SkMatrix& ctm) override
    {
        ...
        if (fCompletion <= 0) {
            ...
            auto a0 = sanitize_angle(fStartAngle - 90 + t * this->wipeAlignment()), a1 = sanitize_angle(a0 + t * 360);
            if (a0 > a1) {
                std::swap(a0, a1);
                std::swap(c0, c1);
            }
            ...
            fMaskShader = SkGradientShader::MakeSweep(
                fWipeCenter.x(), fWipeCenter.y(), grad_colors, grad_pos, std::size(grad_colors), SkTileMode::kClamp, a0, a1, 0, nullptr);
    ...
    void onRender(SkCanvas* canvas, const RenderContext* ctx) const override
    {
        ...
        const auto local_ctx = ScopedRenderContext(canvas, ctx).modulateMaskShader(fMaskShader, canvas->getTotalMatrix());
...
public:
    RadialWipeAdapter(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer, const AnimationBuilder& abuilder)
...
sk_sp<sksg::RenderNode> EffectBuilder::attachRadialWipeEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
