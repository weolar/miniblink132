...
public:
    static sk_sp<TritoneAdapter> Make(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer, const AnimationBuilder& abuilder)
    {
...
private:
    TritoneAdapter(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer, const AnimationBuilder& abuilder)
        ...
        , fCF(sksg::GradientColorFilter::Make(std::move(layer), { fLoColorNode, fMiColorNode, fHiColorNode }))
    {
        enum : size_t {
            kHiColor_Index = 0,
            kMiColor_Index = 1,
    ...
    ColorValue fLoColor, fMiColor, fHiColor;
    ScalarValue fWeight = 0;
};

} // namespace

sk_sp<sksg::RenderNode> EffectBuilder::attachTritoneEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
