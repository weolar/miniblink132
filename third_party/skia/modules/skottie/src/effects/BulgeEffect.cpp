...
static constexpr char gBulgeDisplacementSkSL[] = "uniform shader u_layer;"
...
{
    static const SkRuntimeEffect* effect = SkRuntimeEffect::MakeForShader(SkString(gBulgeDisplacementSkSL), {}).effect.release();
    SkASSERT(effect);
...
class BulgeEffectAdapter final : public DiscardableAdapterBase<BulgeEffectAdapter, BulgeNode> {
public:
    BulgeEffectAdapter(const skjson::ArrayValue& jprops, const AnimationBuilder& abuilder, sk_sp<BulgeNode> node)
        : INHERITED(std::move(node))
    {
        ...
        EffectBinder(jprops, abuilder, this)
            .bind(kHorizontalRadius_Index, fHorizontalRadius)
...
private:
    ...
    Vec2Value fCenter;
    ScalarValue fHorizontalRadius, fVerticalRadius, fBulgeHeight;
    using INHERITED = DiscardableAdapterBase<BulgeEffectAdapter, BulgeNode>;
};

} // namespace

sk_sp<sksg::RenderNode> EffectBuilder::attachBulgeEffect(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
