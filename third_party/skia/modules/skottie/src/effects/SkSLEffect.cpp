...
public:
    ...
    sk_sp<SkShader> contentShader()
    {
        if (!fContentShader || this->hasChildrenInval()) {
            ...
            fContentShader = recorder.finishRecordingAsPicture()->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkFilterMode::kLinear, nullptr, nullptr);
        }
...
public:
    SkSLEffectBase(const skjson::ArrayValue& jprops, const AnimationBuilder& abuilder)
    {
        ...
        if (!result.effect) {
            abuilder.log(Logger::Level::kError, nullptr, "Failed to parse SkSL shader: %s", result.errorText.c_str());
            return;
        }
...
protected:
    ...
    void bindUniforms(const skjson::ArrayValue& jprops, const AnimationBuilder& abuilder, AnimatablePropertyContainer* const& container)
    {
        ...
        for (size_t i = kFirstUniform_index; i < jprops.size(); i++) {
            ...
            if (type == kSkSLProp_uniform) {
                auto uniformTuple = std::make_tuple(SkString(uniformName->begin(), uniformName->size()), std::make_unique<VectorValue>());
                ...
                if (asset_info && asset_info->fAsset) {
                    ...
                    fChildren.push_back({ type, SkString(uniformName->begin(), uniformName->size()), frameData.image->makeShader(sampling) });
                } else {
                    SkDebugf("cannot find asset for custom shader effect");
                }
            } else if (type == kSkSLProp_layer) { /* layer content */
                fChildren.push_back({ type, SkString(uniformName->begin(), uniformName->size()), SkRuntimeEffect::ChildPtr() });
            }
        }
    }
...
class SkSLShaderAdapter final : public DiscardableAdapterBase<SkSLShaderAdapter, SkSLShaderNode>, public SkSLEffectBase {
public:
    SkSLShaderAdapter(const skjson::ArrayValue& jprops, const AnimationBuilder& abuilder, sk_sp<SkSLShaderNode> node)
...
private:
    void onSync() override
    {
        ...
        sk_sp<SkShader> shader = fEffect->makeShader(buildUniformData(), SkSpan(buildChildrenData(this->node())));
        this->node()->setShader(std::move(shader));
    }
};

class SkSLColorFilterAdapter final : public DiscardableAdapterBase<SkSLColorFilterAdapter, sksg::ExternalColorFilter>, public SkSLEffectBase {
public:
    SkSLColorFilterAdapter(const skjson::ArrayValue& jprops, const AnimationBuilder& abuilder, sk_sp<sksg::ExternalColorFilter> node)
...
sk_sp<sksg::RenderNode> EffectBuilder::attachSkSLShader(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
#if defined(SK_ENABLE_SKOTTIE_SKSLEFFECT)
    auto shaderNode = sk_make_sp<SkSLShaderNode>(std::move(layer), fLayerSize);
    return fBuilder->attachDiscardableAdapter<SkSLShaderAdapter>(jprops, *fBuilder, std::move(shaderNode));
#else
    return layer;
#endif
}

sk_sp<sksg::RenderNode> EffectBuilder::attachSkSLColorFilter(const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const
{
#if defined(SK_ENABLE_SKOTTIE_SKSLEFFECT)
