...
public:
    RoundCornersAdapter(const skjson::ObjectValue& jround, const AnimationBuilder& abuilder, sk_sp<sksg::GeometryNode> child)
    ...
    const skjson::ObjectValue& jround, const AnimationBuilder* abuilder, std::vector<sk_sp<sksg::GeometryNode>>&& geos)
{
    ...
    for (auto& g : geos) {
        rounded.push_back(abuilder->attachDiscardableAdapter<RoundCornersAdapter>(jround, *abuilder, std::move(g)));
    }
