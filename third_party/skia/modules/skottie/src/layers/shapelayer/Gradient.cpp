...
public:
    static sk_sp<GradientAdapter> Make(const skjson::ObjectValue& jgrad, const AnimationBuilder& abuilder)
    {
        ...
        auto gradient_node
            = (type == Type::kLinear) ? sk_sp<sksg::Gradient>(sksg::LinearGradient::Make()) : sk_sp<sksg::Gradient>(sksg::RadialGradient::Make());

        return sk_sp<GradientAdapter>(new GradientAdapter(std::move(gradient_node), type, SkToSizeT(stopCount), jgrad, *jstops, abuilder));
    }

    const sk_sp<sksg::Gradient>& node() const
    {
        return fGradient;
    }
...
private:
    ...
    GradientAdapter(sk_sp<sksg::Gradient> gradient, Type type, size_t stop_count, const skjson::ObjectValue& jgrad, const skjson::ObjectValue& jstops,
    ...
    {
        const auto s_point = SkPoint { fStartPoint.x, fStartPoint.y }, e_point = SkPoint { fEndPoint.x, fEndPoint.y };
        ...
        case Type::kRadial: {
            ...
            const float eps = SK_ScalarNearlyZero * 2, h_len = SkTPin(fHighlightLength * 0.01f, -1 + eps, 1 - eps);
        ...
        const auto* o_rec = o_count > 0 ? reinterpret_cast<const OpacityRec*>(fStops.data() + c_size) : nullptr;
        const auto* c_end = c_rec + c_count;
        const auto* o_end = o_rec + o_count;

        sksg::Gradient::ColorStop current_stop = { 0.0f,
            {
                c_rec ? c_rec->r : 0,
                c_rec ? c_rec->g : 0,
                c_rec ? c_rec->b : 0,
        ...
        while (c_rec || o_rec) {
            ...
            const auto& cs = c_rec ? *c_rec : ColorRec { o_rec->t, current_stop.fColor.fR, current_stop.fColor.fG, current_stop.fColor.fB };
            ...
            const auto c_pos = std::max(cs.t, current_stop.fPosition), o_pos = std::max(os.t, current_stop.fPosition),
                       c_pos_rel = c_pos - current_stop.fPosition, o_pos_rel = o_pos - current_stop.fPosition,
            ...
            current_stop = { std::min(c_pos, o_pos),
                { lerp(current_stop.fColor.fR, cs.r, t_c), lerp(current_stop.fColor.fG, cs.g, t_c), lerp(current_stop.fColor.fB, cs.b, t_c),
...
private:
    template <typename T> const T* next_rec(const T* rec, const T* end_rec) const
    {
        if (!rec)
            return nullptr;
    ...
    float fHighlightLength = 0, fHighlightAngle = 0;
};

} // namespace

sk_sp<sksg::PaintNode> ShapeBuilder::AttachGradientFill(const skjson::ObjectValue& jgrad, const AnimationBuilder* abuilder)
{
    auto adapter = GradientAdapter::Make(jgrad, *abuilder);

    return adapter ? AttachFill(jgrad, abuilder, sksg::ShaderPaint::Make(adapter->node()), adapter) : nullptr;
}

sk_sp<sksg::PaintNode> ShapeBuilder::AttachGradientStroke(const skjson::ObjectValue& jgrad, const AnimationBuilder* abuilder)
{
    auto adapter = GradientAdapter::Make(jgrad, *abuilder);

    return adapter ? AttachStroke(jgrad, abuilder, sksg::ShaderPaint::Make(adapter->node()), adapter) : nullptr;
}
