...
private:
    ...
    SkMatrix instanceTransform(size_t i) const
    {
        ...
        return SkMatrix::Translate(t * fPosition.x + fAnchorPoint.x, t * fPosition.y + fAnchorPoint.y) * SkMatrix::RotateDeg(t * fRotation)
            * SkMatrix::Scale(std::pow(fScale.x, t), std::pow(fScale.y, t)) * SkMatrix::Translate(-fAnchorPoint.x, -fAnchorPoint.y);
    }
    ...
    void onRender(SkCanvas* canvas, const RenderContext* ctx) const override
    {
        ...
        for (size_t i = 0; i < fCount; ++i) {
            ...
            const auto local_ctx
                = ScopedRenderContext(canvas, ctx).modulateOpacity(opacity).setIsolation(fChildrenBounds, canvas->getTotalMatrix(), children.size() > 1);
...
public:
    RepeaterAdapter(const skjson::ObjectValue& jrepeater, const skjson::ObjectValue& jtransform, const AnimationBuilder& abuilder,
        std::vector<sk_sp<sksg::RenderNode>>&& draws)
        : INHERITED(sk_make_sp<RepeaterRenderNode>(
            std::move(draws), (ParseDefault(jrepeater["m"], 1) == 1) ? RepeaterRenderNode::CompositeMode::kBelow : RepeaterRenderNode::CompositeMode::kAbove))
    {
...
std::vector<sk_sp<sksg::RenderNode>> ShapeBuilder::AttachRepeaterDrawEffect(
    const skjson::ObjectValue& jrepeater, const AnimationBuilder* abuilder, std::vector<sk_sp<sksg::RenderNode>>&& draws)
{
    ...
    if (const skjson::ObjectValue* jtransform = jrepeater["tr"]) {
        ...
        repeater_draws.push_back(abuilder->attachDiscardableAdapter<RepeaterAdapter>(jrepeater, *jtransform, *abuilder, std::move(draws)));
    } else {
