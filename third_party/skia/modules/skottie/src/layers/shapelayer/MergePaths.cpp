...
sk_sp<sksg::Merge> ShapeBuilder::MergeGeometry(std::vector<sk_sp<sksg::GeometryNode>>&& geos, sksg::Merge::Mode mode)
{
...
std::vector<sk_sp<sksg::GeometryNode>> ShapeBuilder::AttachMergeGeometryEffect(
    const skjson::ObjectValue& jmerge, const AnimationBuilder*, std::vector<sk_sp<sksg::GeometryNode>>&& geos)
{
    ...
    const auto mode = gModes[std::min<size_t>(ParseDefault<size_t>(jmerge["mm"], 1) - 1, std::size(gModes) - 1)];
