...
using GeometryAttacherT = sk_sp<sksg::GeometryNode> (*)(const skjson::ObjectValue&, const AnimationBuilder*);
...
using GeometryEffectAttacherT
    = std::vector<sk_sp<sksg::GeometryNode>> (*)(const skjson::ObjectValue&, const AnimationBuilder*, std::vector<sk_sp<sksg::GeometryNode>>&&);
...
using PaintAttacherT = sk_sp<sksg::PaintNode> (*)(const skjson::ObjectValue&, const AnimationBuilder*);
...
using DrawEffectAttacherT
    = std::vector<sk_sp<sksg::RenderNode>> (*)(const skjson::ObjectValue&, const AnimationBuilder*, std::vector<sk_sp<sksg::RenderNode>>&&);
...
const ShapeInfo* FindShapeInfo(const skjson::ObjectValue& jshape)
{
    ...
    const auto* info = bsearch(type->begin(), gShapeInfo, std::size(gShapeInfo), sizeof(ShapeInfo),
        [](const void* key, const void* info) { return strcmp(static_cast<const char*>(key), static_cast<const ShapeInfo*>(info)->fTypeString); });
...
sk_sp<sksg::GeometryNode> ShapeBuilder::AttachPathGeometry(const skjson::ObjectValue& jpath, const AnimationBuilder* abuilder)
{
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachShape(const skjson::ArrayValue* jshape, AttachShapeContext* ctx, bool suppress_draws) const
{
    if (!jshape)
        return nullptr;
    ...
    for (size_t i = 0; i < jshape->size(); ++i) {
        ...
        case ShapeType::kGeometryEffect:
            ...
            ctx->fGeometryEffectStack->push_back({ *shape, gGeometryEffectAttachers[info->fAttacherIndex] });
            break;
        default:
            break;
        }
    ...
    for (auto rec = recs.rbegin(); rec != recs.rend(); ++rec) {
        ...
        case ShapeType::kGeometryEffect: {
            ...
            if (!geos.empty()) {
                geos = gGeometryEffectAttachers[rec->fInfo.fAttacherIndex](rec->fJson, this, std::move(geos));
            }
        ...
        case ShapeType::kGroup: {
            AttachShapeContext groupShapeCtx(&geos, ctx->fGeometryEffectStack, ctx->fCommittedAnimators);
        ...
        case ShapeType::kPaint: {
            ...
            for (auto it = ctx->fGeometryEffectStack->rbegin(); it != ctx->fGeometryEffectStack->rend(); ++it) {
            ...
            auto geo = drawGeos.size() > 1 ? ShapeBuilder::MergeGeometry(std::move(drawGeos), sksg::Merge::Mode::kMerge) : drawGeos[0];
        ...
        case ShapeType::kDrawEffect: {
            ...
            if (!draws.empty()) {
                draws = gDrawEffectAttachers[rec->fInfo.fAttacherIndex](rec->fJson, this, std::move(draws));
                ctx->fCommittedAnimators = fCurrentAnimatorScope->size();
            }
        } break;
        default:
    ...
    if (jtransform) {
        ...
        fCurrentAnimatorScope->insert(fCurrentAnimatorScope->begin() + ctx->fCommittedAnimators, std::make_move_iterator(local_scope.begin()),
    ...
    for (auto& geo : geos) {
        ctx->fGeometryStack->push_back(shape_transform ? sksg::GeometryTransform::Make(std::move(geo), shape_transform) : std::move(geo));
    }

    return shape_wrapper;
}

sk_sp<sksg::RenderNode> AnimationBuilder::attachShapeLayer(const skjson::ObjectValue& layer, LayerInfo*) const
{
