...
public:
    explicit PuckerBloatEffect(sk_sp<sksg::GeometryNode> geo)
        : INHERITED({ std::move(geo) })
    {
    }
    ...
    SkPath onRevalidateEffect(const sk_sp<GeometryNode>& geo) override
    {
        ...
        auto commit_contour = [&]() {
            ...
            for (const auto& c : cubics) {
                path.cubicTo(lerp(c.ctrl0, center, -fAmount), lerp(c.ctrl1, center, -fAmount), lerp(c.pt, center, fAmount));
            }
            path.close();
        ...
        while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
            ...
            case SkPath::kLine_Verb: {
                ...
                cubics.push_back({ lerp(line_start, line_end, kCtrlPosFraction), lerp(line_start, line_end, 1 - kCtrlPosFraction), line_end });
            } break;
            ...
            case SkPath::kConic_Verb: {
                ...
                const auto conic_start = cubics.empty() ? contour_start : cubics.back().pt, conic_end = pts[2];

                cubics.push_back({ lerp(pts[1], conic_start, kCubicCircleCoeff), lerp(pts[1], conic_end, kCubicCircleCoeff), conic_end });
            } break;
...
public:
    PuckerBloatAdapter(const skjson::ObjectValue& joffset, const AnimationBuilder& abuilder, sk_sp<sksg::GeometryNode> child)
