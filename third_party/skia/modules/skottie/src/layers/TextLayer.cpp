...
template <typename T, typename TMap> const char* parse_map(const TMap& map, const char* str, T* result)
{
...
void AnimationBuilder::parseFonts(const skjson::ObjectValue* jfonts, const skjson::ArrayValue* jchars)
{
    ...
    //        ]
    //    },
    const skjson::ArrayValue* jlist = jfonts ? static_cast<const skjson::ArrayValue*>((*jfonts)["list"]) : nullptr;
    if (!jlist) {
        return;
    }
    ...
    for (const skjson::ObjectValue* jfont : *jlist) {
        ...
        fFonts.set(SkString(jname->begin(), jname->size()),
            { SkString(jfamily->begin(), jfamily->size()), SkString(jstyle->begin(), jstyle->size()),
                jpath ? SkString(jpath->begin(), jpath->size()) : SkString(), ParseDefault((*jfont)["ascent"], 0.0f),
                nullptr, // placeholder
                CustomFont::Builder() });
    }
    ...
    const auto prioritize_embedded_fonts = (fFlags & Animation::Builder::kPreferEmbeddedFonts) || has_comp_glyphs(jchars);
...
bool AnimationBuilder::resolveNativeTypefaces()
{
    ...
    fFonts.foreach ([&](const SkString& name, FontInfo* finfo) {
        ...
        if (!finfo->fTypeface && fFontMgr) {
            finfo->fTypeface = fFontMgr->makeFromData(fResourceProvider->loadFont(name.c_str(), finfo->fPath.c_str()));
        }

        if (!finfo->fTypeface && fFontMgr) {
            finfo->fTypeface = fFontMgr->matchFamilyStyle(finfo->fFamily.c_str(), FontStyle(this, finfo->fStyle.c_str()));

            if (!finfo->fTypeface) {
                this->log(Logger::Level::kError, nullptr, "Could not create typeface for %s|%s.", finfo->fFamily.c_str(), finfo->fStyle.c_str());
                // Last resort.
                finfo->fTypeface = fFontMgr->legacyMakeTypeface(nullptr, FontStyle(this, finfo->fStyle.c_str()));

                has_unresolved |= !finfo->fTypeface;
            }
        }
        if (!finfo->fTypeface && !fFontMgr) {
            this->log(Logger::Level::kError, nullptr, "Could not load typeface for %s|%s because no SkFontMgr provided.", finfo->fFamily.c_str(),
                finfo->fStyle.c_str());
        }
    });
...
bool AnimationBuilder::resolveEmbeddedTypefaces(const skjson::ArrayValue& jchars)
{
    ...
    for (const skjson::ObjectValue* jchar : jchars) {
        ...
        if (!current_font || !current_font->matches(family, style)) {
            ...
            if (!current_font) {
                this->log(Logger::Level::kError, nullptr, "Font not found (%s, %s).", family, style);
                continue;
            }
        }
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachTextLayer(const skjson::ObjectValue& jlayer, LayerInfo*) const
{
    return this->attachDiscardableAdapter<TextAdapter>(jlayer, this, fFontMgr, fCustomGlyphMapper, fLogger, fShapingFactory);
}
