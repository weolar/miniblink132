...
SkMatrix image_matrix(const ImageAsset::FrameData& frame_data, const SkISize& dest_size)
{
    ...
    const auto size_fit_matrix = frame_data.scaling == ImageAsset::SizeFit::kNone
        ? SkMatrix::I()
        : SkMatrix::RectToRect(SkRect::Make(frame_data.image->bounds()), SkRect::Make(dest_size), static_cast<SkMatrix::ScaleToFit>(frame_data.scaling));
...
public:
    FootageAnimator(sk_sp<ImageAsset> asset, sk_sp<sksg::Image> image_node, sk_sp<sksg::Matrix<SkMatrix>> image_transform_node, const SkISize& asset_size,
    ...
    StateChanged onSeek(float t) override
    {
        ...
        if (frame_data.image != fImageNode->getImage() || frame_data.sampling != fImageNode->getSamplingOptions() || m != fImageTransformNode->getMatrix()) {
...
const AnimationBuilder::FootageAssetInfo* AnimationBuilder::loadFootageAsset(const skjson::ObjectValue& defaultJImage) const
{
    const skjson::ObjectValue* jimage = &defaultJImage;
    const skjson::StringValue* slotID = defaultJImage["sid"];
    if (slotID) {
        if (!(fSlotsRoot)) {
            this->log(Logger::Level::kWarning, nullptr, "Slotid found but no slots were found in the json. Using default asset.");
        } else {
            const skjson::ObjectValue* slot = (*(fSlotsRoot))[slotID->begin()];
            if (!slot) {
                this->log(Logger::Level::kWarning, nullptr, "Specified slotID not found in 'slots'. Using default asset.");
            } else {
                jimage = (*slot)["p"];
            }
        }
    ...
    const auto size = SkISize::Make(ParseDefault<int>((*jimage)["w"], 0), ParseDefault<int>((*jimage)["h"], 0));
    return fImageAssetCache.set(res_id, { std::move(asset), size });
}

sk_sp<sksg::RenderNode> AnimationBuilder::attachFootageAsset(const skjson::ObjectValue& jimage, LayerInfo* layer_info) const
{
    ...
    const auto requires_animator = (fFlags & Animation::Builder::kDeferImageLoading) || asset_info->fAsset->isMultiFrame();
    if (requires_animator) {
        ...
        fCurrentAnimatorScope->push_back(
            sk_make_sp<FootageAnimator>(asset_info->fAsset, image_node, image_transform, asset_info->fSize, -layer_info->fInPoint, 1 / fFrameRate));
    } else {
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachFootageLayer(const skjson::ObjectValue& jlayer, LayerInfo* layer_info) const
{
