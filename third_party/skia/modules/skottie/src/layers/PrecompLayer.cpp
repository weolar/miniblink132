...
public:
    CompTimeMapper(AnimatorScope&& layer_animators, sk_sp<TimeRemapper> remapper, float time_bias, float time_scale)
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachExternalPrecompLayer(const skjson::ObjectValue& jlayer, const LayerInfo& layer_info) const
{
    ...
    auto external_layer = fPrecompInterceptor->onLoadPrecomp(id->begin(), nm->begin(), layer_info.fSize);
    if (!external_layer) {
        return nullptr;
    }
    ...
    private:
        ...
        void onRender(SkCanvas* canvas, const RenderContext* ctx) const override
        {
            ...
            const auto local_scope = ScopedRenderContext(canvas, ctx).setIsolation(this->bounds(), canvas->getTotalMatrix(), true);
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachPrecompLayer(const skjson::ObjectValue& jlayer, LayerInfo* layer_info) const
{
    ...
    const auto start_time = ParseDefault<float>(jlayer["st"], 0.0f), stretch_time = ParseDefault<float>(jlayer["sr"], 1.0f);
    ...
    auto parse_size
        = [](const skjson::ObjectValue& jlayer) { return SkSize::Make(ParseDefault<float>(jlayer["w"], 0.0f), ParseDefault<float>(jlayer["h"], 0.0f)); };
    ...
    if (requires_time_mapping) {
        ...
        auto time_mapper = sk_make_sp<CompTimeMapper>(local_scope->release(), std::move(time_remapper), t_bias, std::isfinite(t_scale) ? t_scale : 0);
