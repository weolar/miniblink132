...
public:
    ...
    {
        return !this->isStatic() || fOpacity < 100 || fFeather != SkV2 { 0, 0 };
    }
...
private:
    ...
    {
        SkASSERT(fBlendMode == SkBlendMode::kSrc || fBlendMode == SkBlendMode::kSrcOver || fBlendMode == SkBlendMode::kSrcIn
            || fBlendMode == SkBlendMode::kDstOut || fBlendMode == SkBlendMode::kXor);
...
sk_sp<sksg::RenderNode> AttachMask(const skjson::ArrayValue* jmask, const AnimationBuilder* abuilder, sk_sp<sksg::RenderNode> childNode)
{
    if (!jmask)
        return childNode;
    ...
    for (const skjson::ObjectValue* m : *jmask) {
        ...
        mask_path->setFillType(mask_inverted ? SkPathFillType::kInverseWinding : SkPathFillType::kWinding);
...
public:
    LayerController(AnimatorScope&& layer_animators, sk_sp<sksg::RenderNode> layer, size_t tanim_count, float in, float out)
...
LayerBuilder::LayerBuilder(const skjson::ObjectValue& jlayer, const SkSize& comp_size)
    ...
    , fInfo { comp_size, ParseDefault<float>(jlayer["ip"], 0.0f), ParseDefault<float>(jlayer["op"], 0.0f) }
{
...
sk_sp<sksg::Transform> LayerBuilder::buildTransform(const AnimationBuilder& abuilder, CompositionBuilder* cbuilder)
{
...
sk_sp<sksg::Transform> LayerBuilder::getTransform(const AnimationBuilder& abuilder, CompositionBuilder* cbuilder, TransformType ttype)
{
...
sk_sp<sksg::Transform> LayerBuilder::getParentTransform(const AnimationBuilder& abuilder, CompositionBuilder* cbuilder, TransformType ttype)
{
...
sk_sp<sksg::Transform> LayerBuilder::doAttachTransform(const AnimationBuilder& abuilder, CompositionBuilder* cbuilder, TransformType ttype)
{
    ...
    if (this->isCamera()) {
        ...
        //
        return abuilder.attachCamera(fJlayer, *jtransform, sksg::Transform::MakeInverse(std::move(parent_transform)), cbuilder->fSize);
    }

    return this->is3D() ? abuilder.attachMatrix3D(*jtransform, std::move(parent_transform), fAutoOrient)
...
{
    return cbuilder->fMotionBlurSamples > 1 && cbuilder->fMotionBlurAngle > 0 && ParseDefault(fJlayer["mb"], false);
}

sk_sp<sksg::RenderNode> LayerBuilder::buildRenderTree(const AnimationBuilder& abuilder, CompositionBuilder* cbuilder, const LayerBuilder* prev_layer)
{
    const AnimationBuilder::AutoPropertyTracker apt(&abuilder, fJlayer, PropertyObserver::NodeType::LAYER);

    using LayerBuilder = sk_sp<sksg::RenderNode> (AnimationBuilder::*)(const skjson::ObjectValue&, AnimationBuilder::LayerInfo*) const;
    ...
    if (const skjson::ArrayValue* jeffects = fJlayer["ef"]) {
        layer = EffectBuilder(&abuilder, fInfo.fSize, cbuilder).attachEffects(*jeffects, std::move(layer));
    }
    ...
    if (const skjson::ArrayValue* jstyles = fJlayer["sy"]) {
        layer = EffectBuilder(&abuilder, fInfo.fSize, cbuilder).attachStyles(*jstyles, std::move(layer));
    }
    ...
    const auto force_seek_count = build_info.fFlags & kForceSeek ? abuilder.fCurrentAnimatorScope->size() : fTransformAnimatorCount;

    sk_sp<Animator> controller = sk_make_sp<LayerController>(ascope.release(), layer, force_seek_count, fInfo.fInPoint, fInfo.fOutPoint);
    ...
    if (layer && has_animators && this->hasMotionBlur(cbuilder)) {
        ...
        auto motion_blur = MotionBlurEffect::Make(
            std::move(controller), std::move(layer), cbuilder->fMotionBlurSamples, cbuilder->fMotionBlurAngle, cbuilder->fMotionBlurPhase);
    ...
    if (matte_mode > 0) {
        ...
        if (matte_mode <= std::size(gMatteModes)) {
            ...
            layer = sksg::MaskEffect::Make(std::move(layer), prev_layer->fContentTree, gMatteModes[matte_mode - 1]);
        } else {
