...
public:
    ...
    FrameData getFrameData(float t) override
    {
        ...
        return { nullptr, SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNearest), SkMatrix::I(), SizeFit::kCenter };
    }
...
bool skottie::SlotManager::setImageSlot(const SlotID& slotID, const sk_sp<skresources::ImageAsset>& i)
{
...
{
    const auto valueGroup = fColorMap.find(slotID);
    return valueGroup && !valueGroup->empty() ? std::optional<SkColor>(*(valueGroup->at(0).value)) : std::nullopt;
}
...
{
    const auto valueGroup = fScalarMap.find(slotID);
    return valueGroup && !valueGroup->empty() ? std::optional<float>(*(valueGroup->at(0).value)) : std::nullopt;
}

std::optional<SkV2> skottie::SlotManager::getVec2Slot(const SlotID& slotID) const
{
    const auto valueGroup = fVec2Map.find(slotID);
    return valueGroup && !valueGroup->empty() ? std::optional<SkV2>(*(valueGroup->at(0).value)) : std::nullopt;
}

std::optional<skottie::TextPropertyValue> skottie::SlotManager::getTextSlot(const SlotID& slotID) const
{
    const auto adapterGroup = fTextMap.find(slotID);
    return adapterGroup && !adapterGroup->empty() ? std::optional<TextPropertyValue>(adapterGroup->at(0)->getText()) : std::nullopt;
}

void skottie::SlotManager::trackColorValue(const SlotID& slotID, ColorValue* colorValue, sk_sp<internal::AnimatablePropertyContainer> adapter)
{
    fColorMap[slotID].push_back({ colorValue, std::move(adapter) });
}

sk_sp<skresources::ImageAsset> skottie::SlotManager::trackImageValue(const SlotID& slotID, sk_sp<skresources::ImageAsset> imageAsset)
{
...
void skottie::SlotManager::trackScalarValue(const SlotID& slotID, ScalarValue* scalarValue, sk_sp<internal::AnimatablePropertyContainer> adapter)
{
    fScalarMap[slotID].push_back({ scalarValue, std::move(adapter) });
}

void skottie::SlotManager::trackVec2Value(const SlotID& slotID, Vec2Value* vec2Value, sk_sp<internal::AnimatablePropertyContainer> adapter)
{
    fVec2Map[slotID].push_back({ vec2Value, std::move(adapter) });
}

void skottie::SlotManager::trackTextValue(const SlotID& slotID, sk_sp<internal::TextAdapter> adapter)
{
