...
sk_sp<sksg::RenderNode> AnimationBuilder::attachOpacity(const skjson::ObjectValue& jobject, sk_sp<sksg::RenderNode> child_node) const
{
    if (!child_node)
        return nullptr;
...
AnimationBuilder::AnimationBuilder(sk_sp<ResourceProvider> rp, sk_sp<SkFontMgr> fontmgr, sk_sp<PropertyObserver> pobserver, sk_sp<Logger> logger,
    sk_sp<MarkerObserver> mobserver, sk_sp<PrecompInterceptor> pi, sk_sp<ExpressionManager> expressionmgr, sk_sp<SkShapers::Factory> shapingFactory,
    Animation::Builder::Stats* stats, const SkSize& comp_size, float duration, float framerate, uint32_t flags)
...
bool AnimationBuilder::dispatchTextProperty(const sk_sp<TextAdapter>& t, const skjson::ObjectValue* jtext) const
{
    bool dispatched = false;
...
void AnimationBuilder::AutoPropertyTracker::updateContext(PropertyObserver* observer, const skjson::ObjectValue& obj)
{
...
sk_sp<Animation> Animation::Builder::make(const char* data, size_t data_len)
{
    ...
    auto resolvedProvider = fResourceProvider ? fResourceProvider : sk_make_sp<NullResourceProvider>();
    ...
    const auto size = SkSize::Make(ParseDefault<float>(json["w"], 0.0f), ParseDefault<float>(json["h"], 0.0f));
    const auto fps = ParseDefault<float>(json["fr"], -1.0f), inPoint = ParseDefault<float>(json["ip"], 0.0f),
               outPoint = std::max(ParseDefault<float>(json["op"], SK_ScalarMax), inPoint), duration = sk_ieee_float_divide(outPoint - inPoint, fps);

    if (size.isEmpty() || version.isEmpty() || fps <= 0 || !SkIsFinite(inPoint, outPoint, duration)) {
        if (fLogger) {
            const auto msg = SkStringPrintf("Invalid animation params (version: %s, size: [%f %f], frame rate: %f, "
    ...
    internal::AnimationBuilder builder(std::move(resolvedProvider), fFontMgr, std::move(fPropertyObserver), std::move(fLogger), std::move(fMarkerObserver),
        std::move(fPrecompInterceptor), std::move(fExpressionManager), std::move(factory), &fStats, size, duration, fps, fFlags);
    ...
    return sk_sp<Animation>(
        new Animation(std::move(ainfo.fSceneRoot), std::move(ainfo.fAnimators), std::move(version), size, inPoint, outPoint, duration, fps, flags));
}
...
Animation::Animation(sk_sp<sksg::RenderNode> scene_root, std::vector<sk_sp<internal::Animator>>&& animators, SkString version, const SkSize& size,
...
void Animation::seekFrame(double t, sksg::InvalidationController* ic)
{
    ...
    const auto kLastValidFrame = std::nextafterf(fOutPoint, fInPoint), comp_time = SkTPin<float>(fInPoint + t, fInPoint, kLastValidFrame);
