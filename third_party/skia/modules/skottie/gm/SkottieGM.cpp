...
public:
    FakeWebFontProvider()
        : fTypeface(ToolUtils::CreateTypefaceFromResource(kWebFontResource))
    {
    }
...
protected:
    ...
    void onOnceBeforeDraw() override
    {
        if (auto stream = GetResourceAsStream("skottie/skottie_sample_multiframe.json")) {
            fAnimation = skottie::Animation::Builder().setResourceProvider(sk_make_sp<MultiFrameResourceProvider>()).make(stream.get());
            fAnimation->seek(0);
        }
    }
