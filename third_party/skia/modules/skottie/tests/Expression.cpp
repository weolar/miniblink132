...
public:
    ...
    {
        return SkString("Hello, world!");
    }
};

class FakeExpressionManager : public ExpressionManager {
public:
    sk_sp<ExpressionEvaluator<float>> createNumberExpressionEvaluator(const char expression[]) override
    {
        return sk_make_sp<FakeScalarExpressionEvaluator>();
    }

    sk_sp<ExpressionEvaluator<SkString>> createStringExpressionEvaluator(const char expression[]) override
    {
        return sk_make_sp<FakeStringExpressionEvaluator>();
    }

    sk_sp<ExpressionEvaluator<std::vector<float>>> createArrayExpressionEvaluator(const char expression[]) override
    {
...
public:
    void onOpacityProperty(const char node_name[], const LazyHandle<OpacityPropertyHandle>& opacity_handle) override
    {
        opacity_handle_ = opacity_handle();
    }

    void onTransformProperty(const char node_name[], const LazyHandle<TransformPropertyHandle>& transform_handle) override
    {
        transform_handle_ = transform_handle();
    }

    void onColorProperty(const char node_name[], const LazyHandle<ColorPropertyHandle>& color_handle) override
    {
        color_handle_ = color_handle();
    }

    void onTextProperty(const char node_name[], const LazyHandle<TextPropertyHandle>& text_handle) override
    {
...
DEF_TEST(Skottie_Expression, r)
{
    ...
    auto anim = Animation::Builder().setExpressionManager(em).setFontManager(ToolUtils::TestFontMgr()).setPropertyObserver(observer).make(&stream);
