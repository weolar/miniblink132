...
DEF_TEST(Skottie_Text_Style, r)
{
    ...
    auto anim = Animation::Builder().setFontManager(fmgr).setTextShapingFactory(SkShapers::BestAvailable()).make(&stream);
...
DEF_TEST(Skottie_Text_LayoutError, r)
{
    ...
    public:
        const std::vector<SkString>& errors() const
        {
            return fErrors;
        }
    ...
    auto anim = Animation::Builder().setLogger(logger).setResourceProvider(sk_make_sp<PortableRP>()).make(&stream);
...
DEF_TEST(Skottie_Text_FontFamily, r)
{
    ...
    public:
        const std::unique_ptr<TextPropertyHandle>& text() const
        {
            return fText;
        }

    private:
        void onTextProperty(const char node_name[], const LazyHandle<TextPropertyHandle>& lh) override
        {
    ...
    SkMemoryStream stream(json, strlen(json));
    auto anim = Animation::Builder().setFontManager(fmgr).setPropertyObserver(prop_observer).setTextShapingFactory(SkShapers::BestAvailable()).make(&stream);
    REPORTER_ASSERT(r, anim);
