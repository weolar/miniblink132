...
DEF_TEST(Skottie_Image_CustomTransform, r)
{
    ...
    static const struct TestData {
        ...
        { 2, SkMatrix::Translate(-50, 0), { 0xff00ff00, 0xffff0000, 0xff00ff00, 0xff00ff00, 0xff00ff00 } },
        { 3, SkMatrix::Translate(0, -25), { 0xff00ff00, 0xff00ff00, 0xffff0000, 0xff00ff00, 0xff00ff00 } },
    ...
    auto anim = Animation::Builder().setResourceProvider(sk_make_sp<TestResourceProvider>(tests, r)).make(&stream);

    REPORTER_ASSERT(r, anim);

    static constexpr SkSize render_size { 100, 100 };
    auto surf = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(render_size.width(), render_size.height()));
    ...
    for (const auto& tst : tests) {
        ...
        REPORTER_ASSERT(r, tst.c[0] == pmap.getColor(render_size.width() / 2, render_size.height() / 2));
        ...
        REPORTER_ASSERT(r, tst.c[3] == pmap.getColor(render_size.width() - 1, render_size.height() / 2));
        REPORTER_ASSERT(r, tst.c[4] == pmap.getColor(render_size.width() / 2, render_size.height() - 1));
    }
}
