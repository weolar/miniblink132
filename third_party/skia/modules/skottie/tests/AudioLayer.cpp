...
DEF_TEST(Skottie_AudioLayer, r)
{
    ...
    public:
        explicit MockResourceProvider(skiatest::Reporter* r)
            : fReporter(r)
        {
        }

        const std::vector<sk_sp<MockTracker>>& tracks() const
        {
            return fTracks;
        }

    private:
        sk_sp<ExternalTrackAsset> loadAudioAsset(const char path[], const char name[], const char id[]) override
        {
