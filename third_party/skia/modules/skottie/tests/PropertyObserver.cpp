...
public:
    explicit MockPropertyObserver(bool override_props = false)
        : fOverrideProps(override_props)
    {
    }
    ...
    void onColorProperty(const char node_name[], const PropertyObserver::LazyHandle<ColorPropertyHandle>& lh) override
    {
        auto prop_handle = lh();
        if (fOverrideProps) {
    ...
    void onOpacityProperty(const char node_name[], const PropertyObserver::LazyHandle<OpacityPropertyHandle>& lh) override
    {
        auto prop_handle = lh();
        if (fOverrideProps) {
    ...
    void onTextProperty(const char node_name[], const PropertyObserver::LazyHandle<TextPropertyHandle>& lh) override
    {
        auto prop_handle = lh();
        if (fOverrideProps) {
    ...
    void onTransformProperty(const char node_name[], const PropertyObserver::LazyHandle<TransformPropertyHandle>& lh) override
    {
        auto prop_handle = lh();
        if (fOverrideProps) {
    ...
    void onLeavingNode(const char node_name[], PropertyObserver::NodeType node_type) override
    {
        ...
        fCurrentNode = fCurrentNode.length() > length ? fCurrentNode.substr(0, fCurrentNode.length() - strlen(node_name) - 1) : "";
    }

    const std::vector<ColorInfo>& colors() const
    {
        return fColors;
    }
    const std::vector<OpacityInfo>& opacities() const
    {
        return fOpacities;
    }
    ...
    const std::vector<TransformInfo>& transforms() const
    {
        return fTransforms;
    }
...
public:
    MockFontMgr(sk_sp<SkTypeface> test_font)
        : fTestFont(std::move(test_font))
    {
    }
...
DEF_TEST(Skottie_Props, reporter)
{
    ...
    auto animation = skottie::Animation::Builder().setPropertyObserver(observer).setFontManager(test_font_manager).make(&stream);
    ...
    REPORTER_ASSERT(reporter,
        transforms[0].handle->get() == skottie::TransformPropertyValue({ SkPoint::Make(0, 0), SkPoint::Make(0, 0), SkVector::Make(100, 100), 0, 0, 0 }));
    REPORTER_ASSERT(reporter, transforms[1].node_name.equals("layer_1"));
    REPORTER_ASSERT(reporter,
        transforms[1].handle->get() == skottie::TransformPropertyValue({ SkPoint::Make(0, 0), SkPoint::Make(25, 25), SkVector::Make(100, 100), 0, 0, 0 }));
    REPORTER_ASSERT(reporter, transforms[2].node_name.equals("shape_transform_0"));
    REPORTER_ASSERT(reporter,
        transforms[2].handle->get() == skottie::TransformPropertyValue({ SkPoint::Make(0, 0), SkPoint::Make(0, 0), SkVector::Make(50, 50), 0, 0, 0 }));
    ...
    skottie::TextPropertyValue text_prop({ test_typeface, SkString("inline_text"), 100, 0, 100, 0, 120, 12, 0, 0, SkTextUtils::kLeft_Align,
        Shaper::VAlign::kTopBaseline, Shaper::ResizePolicy::kNone, Shaper::LinebreakPolicy::kExplicit, Shaper::Direction::kLTR, Shaper::Capitalization::kNone,
        SkRect::MakeEmpty(), SK_ColorTRANSPARENT, SK_ColorTRANSPARENT, TextPaintOrder::kFillStroke, SkPaint::Join::kDefault_Join, false, false, nullptr,
        SkString(), SkString("test-family") });
