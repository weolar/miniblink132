...
DEF_TEST(Skottie_Shaper_Clusters, r)
{
    ...
    auto check_clusters = [](skiatest::Reporter* r, const SkString& text, Shaper::Flags flags, const std::vector<size_t>& expected_clusters) {
        ...
        const auto result = Shaper::Shape(text, desc, SkRect::MakeWH(1000, 1000), ToolUtils::TestFontMgr(), SkShapers::BestAvailable());
    ...
    check_clusters(r, text, Shaper::kClusters, { 0, 1, 2, 3, 5, 6, 7, 8, 10, 11, 12, 13 });
    check_clusters(r, text, (Shaper::Flags)(Shaper::kClusters | Shaper::kFragmentGlyphs), { 0, 1, 2, 3, 5, 6, 7, 8, 10, 11, 12, 13 });
}
    ...
    for (const auto& tsize : kTestSizes) {
        for (const auto& talign : kTestAligns) {
            const skottie::Shaper::TextDesc desc
                = { typeface, tsize.text_size, 0, tsize.text_size, tsize.text_size, 0, 0, talign.align, Shaper::VAlign::kTopBaseline,
                      Shaper::ResizePolicy::kNone, Shaper::LinebreakPolicy::kExplicit, Shaper::Direction::kLTR, Shaper::Capitalization::kNone, 0, 0, nullptr };

            const auto shape_result = Shaper::Shape(text, desc, text_point, ToolUtils::TestFontMgr(), SkShapers::BestAvailable());
            ...
            REPORTER_ASSERT(reporter, std::fabs(shape_bounds.left() - expected_l) < tsize.tolerance, "%f %f %f %f %d", shape_bounds.left(), expected_l,
                tsize.tolerance, tsize.text_size, talign.align);

            const auto expected_r = text_point.x() + shape_bounds.width() * talign.r_selector;
            REPORTER_ASSERT(reporter, std::fabs(shape_bounds.right() - expected_r) < tsize.tolerance, "%f %f %f %f %d", shape_bounds.right(), expected_r,
                tsize.tolerance, tsize.text_size, talign.align);
        }
    }
}
...
DEF_TEST(Skottie_Shaper_VAlign, reporter)
{
    ...
    struct {
        ...
        { skottie::Shaper::VAlign::kHybridTop, 0.0f }, { skottie::Shaper::VAlign::kHybridCenter, 0.5f },
    ...
    for (const auto& tsize : kTestSizes) {
        for (const auto& talign : kTestAligns) {
            const skottie::Shaper::TextDesc desc
                = { typeface, tsize.text_size, 0, tsize.text_size, tsize.text_size, 0, 0, SkTextUtils::Align::kCenter_Align, talign.align,
                      Shaper::ResizePolicy::kNone, Shaper::LinebreakPolicy::kParagraph, Shaper::Direction::kLTR, Shaper::Capitalization::kNone, 0, 0, nullptr };
            ...
            REPORTER_ASSERT(reporter, std::fabs(shape_bounds.top() - expected_t) < tsize.tolerance, "%f %f %f %f %u", shape_bounds.top(), expected_t,
                tsize.tolerance, tsize.text_size, SkToU32(talign.align));

            const auto expected_b = text_box.bottom() - v_diff * (1 - talign.topFactor);
            REPORTER_ASSERT(reporter, std::fabs(shape_bounds.bottom() - expected_b) < tsize.tolerance, "%f %f %f %f %u", shape_bounds.bottom(), expected_b,
                tsize.tolerance, tsize.text_size, SkToU32(talign.align));
        }
    }
}
...
{
    skottie::Shaper::TextDesc desc = { ToolUtils::DefaultTypeface(), 18, 0, 18, 18, 0, 0, SkTextUtils::Align::kCenter_Align, Shaper::VAlign::kTop,
        Shaper::ResizePolicy::kNone, Shaper::LinebreakPolicy::kParagraph, Shaper::Direction::kLTR, Shaper::Capitalization::kNone, 0, 0, nullptr };
...

    {
        ...
        const auto shape_result = skottie::Shaper::Shape(text, desc, text_box, ToolUtils::TestFontMgr(), SkShapers::BestAvailable());
...
DEF_TEST(Skottie_Shaper_ExplicitFontMgr, reporter)
{
    ...
    skottie::Shaper::TextDesc desc = { ToolUtils::DefaultPortableTypeface(), 18, 0, 18, 18, 0, 0, SkTextUtils::Align::kCenter_Align, Shaper::VAlign::kTop,
        Shaper::ResizePolicy::kNone, Shaper::LinebreakPolicy::kParagraph, Shaper::Direction::kLTR, Shaper::Capitalization::kNone, 0, 0, nullptr };
...

    {
        ...
        const auto shape_result = skottie::Shaper::Shape(SkString("foo\U000DFFFFbar"), desc, text_box, fontmgr, SkShapers::BestAvailable());
