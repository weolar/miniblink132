...
JSArray EMSCRIPTEN_KEEPALIVE ToCmds(const SkPath& path)
{
    ...
    for (auto [verb, pts, w] : SkPathPriv::Iterate(path)) {
        ...
        case SkPathVerb::kCubic:
            cmd.call<void>("push", CUBIC, pts[1].x(), pts[1].y(), pts[2].x(), pts[2].y(), pts[3].x(), pts[3].y());
            break;
...
void EMSCRIPTEN_KEEPALIVE ToCanvas(const SkPath& path, emscripten::val /* Path2D or Canvas*/ ctx)
{
    ...
    while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
        ...
        case SkPath::kCubic_Verb:
            ctx.call<void>("bezierCurveTo", pts[1].x(), pts[1].y(), pts[2].x(), pts[2].y(), pts[3].x(), pts[3].y());
            break;
...
{
    return SkMatrix::MakeAll(sm.scaleX, sm.skewX, sm.transX, sm.skewY, sm.scaleY, sm.transY, sm.pers0, sm.pers1, sm.pers2);
}
...
EMSCRIPTEN_BINDINGS(skia)
{
    class_<SkPath>("SkPath")
        ...
        .function("_transform",
            select_overload<void(SkPath & orig, SkScalar, SkScalar, SkScalar, SkScalar, SkScalar, SkScalar, SkScalar, SkScalar, SkScalar)>(&ApplyTransform))
