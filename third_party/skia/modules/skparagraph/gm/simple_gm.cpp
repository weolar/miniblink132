...
static const char* gSpeech = "Five score years ago, a great American, in whose symbolic shadow we stand today, signed the Emancipation Proclamation. This "
                             "momentous decree came as a great beacon light of hope to millions of Negro slaves who had been seared in the flames of withering "
...
public:
    ...
    void buildParagraph()
    {
        ...
        auto builder = skia::textlayout::ParagraphBuilderImpl::make(paraStyle, collection, unicode);
        if (nullptr == builder) {
            fPara = nullptr;
            return;
        }
...
protected:
    ...
    {
        SkString name;
        name.printf("paragraph%s_%s", fFlags & kTimeLayout ? "_layout" : "", fFlags & kUseUnderline ? "_underline" : "");
        ...
        para->visit([&](int, const skia::textlayout::Paragraph::VisitorInfo* info) {
            ...
            if (fFlags & kUseUnderline) {
                ...
                for (int i = 0; i < info->count; ++i) {
                    pos.push_back({ info->origin.fX + info->positions[i].fX, info->origin.fY + info->positions[i].fY });
                }
