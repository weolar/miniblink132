...
std::unique_ptr<ParagraphBuilder> ParagraphBuilder::make(const ParagraphStyle& style, sk_sp<FontCollection> fontCollection)
{
    return ParagraphBuilderImpl::make(style, std::move(fontCollection), get_unicode());
}

std::unique_ptr<ParagraphBuilder> ParagraphBuilderImpl::make(const ParagraphStyle& style, sk_sp<FontCollection> fontCollection)
{
    return std::make_unique<ParagraphBuilderImpl>(style, std::move(fontCollection), get_unicode());
}

ParagraphBuilderImpl::ParagraphBuilderImpl(const ParagraphStyle& style, sk_sp<FontCollection> fontCollection)
...
std::unique_ptr<ParagraphBuilder> ParagraphBuilder::make(const ParagraphStyle& style, sk_sp<FontCollection> fontCollection, sk_sp<SkUnicode> unicode)
{
    return ParagraphBuilderImpl::make(style, std::move(fontCollection), std::move(unicode));
}

std::unique_ptr<ParagraphBuilder> ParagraphBuilderImpl::make(const ParagraphStyle& style, sk_sp<FontCollection> fontCollection, sk_sp<SkUnicode> unicode)
{
    return std::make_unique<ParagraphBuilderImpl>(style, std::move(fontCollection), std::move(unicode));
}

ParagraphBuilderImpl::ParagraphBuilderImpl(const ParagraphStyle& style, sk_sp<FontCollection> fontCollection, sk_sp<SkUnicode> unicode)
    : ParagraphBuilder()
    , fUtf8()
...
void ParagraphBuilderImpl::addPlaceholder(const PlaceholderStyle& placeholderStyle, bool lastOne)
{
    ...
    BlockRange stylesBefore(fPlaceholders.empty() ? 0 : fPlaceholders.back().fBlocksBefore.end + 1, fStyledBlocks.size());
...
std::unique_ptr<Paragraph> ParagraphBuilderImpl::Build()
{
    ...
    if (fUsingClientInfo && !fUnicode) {
        ...
        fUnicode = SkUnicodes::Client::Make(this->getText(), std::move(fWordsUtf16), std::move(fGraphemeBreaksUtf8), std::move(fLineBreaksUtf8));
    }
#endif

    SkASSERT_RELEASE(fUnicode);
    return std::make_unique<ParagraphImpl>(fUtf8, fParagraphStyle, fStyledBlocks, fPlaceholders, fFontCollection, fUnicode);
}
...
void ParagraphBuilderImpl::ensureUTF16Mapping()
{
    fillUTF16MappingOnce([&] {
        SkUnicode::extractUtfConversionMapping(
            this->getText(), [&](size_t index) { fUTF8IndexForUTF16Index.emplace_back(index); },
...
void ParagraphBuilderImpl::setLineBreaksUtf16(std::vector<SkUnicode::LineBreakBefore> lineBreaksUtf16)
{
    ...
    for (SkUnicode::LineBreakBefore lineBreakUtf16 : lineBreaksUtf16) {
        lineBreaksUtf8.emplace_back(SkUnicode::LineBreakBefore(fUTF8IndexForUTF16Index[lineBreakUtf16.pos], lineBreakUtf16.breakType));
    }
