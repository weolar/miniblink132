...
class TextWrapper {
    ...
    public:
        ...
        ClusterPos(Cluster* cluster, size_t pos)
            : fCluster(cluster)
            , fPos(pos)
        {
        }
    ...
    public:
        ...
        SkScalar widthWithGhostSpaces() const
        {
            return fWidthWithGhostSpaces;
        }
        inline Cluster* startCluster() const
        {
            return fStart.cluster();
        }
        inline Cluster* endCluster() const
        {
            return fEnd.cluster();
        }
        inline Cluster* breakCluster() const
        {
            return fBreak.cluster();
        }
        inline InternalLineMetrics& metrics()
        {
            return fMetrics;
        }
        inline size_t startPos() const
        {
        ...
        void setMetrics(const InternalLineMetrics& metrics)
        {
            fMetrics = metrics;
        }
        ...
        void trim()
        {
            ...
            if (fEnd.cluster() != nullptr && fEnd.cluster()->owner() != nullptr && fEnd.cluster()->runOrNull() != nullptr
...
public:
    ...
    using AddLineToParagraph = std::function<void(TextRange textExcludingSpaces, TextRange text, TextRange textIncludingNewlines, ClusterRange clusters,
        ClusterRange clustersWithGhosts, SkScalar AddLineToParagraph, size_t startClip, size_t endClip, SkVector offset, SkVector advance,
