...
void OneLineShaper::finish(const Block& block, SkScalar height, SkScalar& advanceX)
{
    ...
    std::sort(fResolvedBlocks.begin(), fResolvedBlocks.end(), [](const RunBlock& a, const RunBlock& b) { return a.fText.start < b.fText.start; });
    ...
    for (auto& resolvedBlock : fResolvedBlocks) {
        ...
        const SkShaper::RunHandler::RunInfo info
            = { run->fFont, run->fBidiLevel, runAdvance, glyphs.width(), SkShaper::RunHandler::Range(text.start - run->fClusterStart, text.width()) };
        this->fParagraph->fRuns.emplace_back(this->fParagraph, info, run->fClusterStart, height, block.fStyle.getHalfLeading(), block.fStyle.getBaselineShift(),
            this->fParagraph->fRuns.size(), advanceX);
...
TextRange OneLineShaper::normalizeTextRange(GlyphRange glyphRange)
{
    ...
    if (fCurrentRun->leftToRight()) {
        ...
        return TextRange(clusterIndex(glyphRange.end - 1), glyphRange.start > 0 ? clusterIndex(glyphRange.start - 1) : fCurrentRun->fTextRange.end);
    }
}
...
void OneLineShaper::addFullyResolved()
{
    ...
    RunBlock resolved(fCurrentRun, this->fCurrentRun->fTextRange, GlyphRange(0, this->fCurrentRun->size()), this->fCurrentRun->size());
    fResolvedBlocks.emplace_back(resolved);
}
...
void OneLineShaper::sortOutGlyphs(std::function<void(GlyphRange)>&& sortOutUnresolvedBLock)
{
    ...
    for (size_t i = 0; i < fCurrentRun->size(); ++i) {
        ...
        if ((fCurrentRun->leftToRight() ? gi > graphemeStart : gi < graphemeStart) || graphemeStart == EMPTY_INDEX) {
            ...
            bool isControl8 = fParagraph->codeUnitHasProperty(ci, SkUnicode::CodeUnitFlags::kControl);
...
void OneLineShaper::iterateThroughFontStyles(TextRange textRange, SkSpan<Block> styleSpan, const ShapeSingleFontVisitor& visitor)
{
    Block combinedBlock;
    ...
    auto addFeatures = [&features](const Block& block) {
        for (auto& ff : block.fStyle.getFontFeatures()) {
            ...
            SkShaper::Feature feature
                = { SkSetFourByteTag(ff.fName[0], ff.fName[1], ff.fName[2], ff.fName[3]), SkToU32(ff.fValue), block.fRange.start, block.fRange.end };
            features.emplace_back(feature);
        }
        // Disable ligatures if letter spacing is enabled.
        if (block.fStyle.getLetterSpacing() > 0) {
            features.emplace_back(SkShaper::Feature { SkSetFourByteTag('l', 'i', 'g', 'a'), 0, block.fRange.start, block.fRange.end });
        }
    };
...
void OneLineShaper::matchResolvedFonts(const TextStyle& textStyle, const TypefaceVisitor& visitor)
{
    std::vector<sk_sp<SkTypeface>> typefaces
        = fParagraph->fFontCollection->findTypefaces(textStyle.getFontFamilies(), textStyle.getFontStyle(), textStyle.getFontArguments());
        ...
        while (!fUnresolvedBlocks.empty()) {
            ...
            while (true) {
                ...
                while (ch != unresolvedText.end()) {
                    emojiStart = OneLineShaper::getEmojiSequenceStart(fParagraph->fUnicode.get(), &ch, unresolvedText.end());
                    if (emojiStart != -1) {
                ...
                if (emojiStart == -1) {
                    ...
                    if (typeface == nullptr) {
                        typeface = fParagraph->fFontCollection->defaultFallback(codepoint, textStyle.getFontStyle(), textStyle.getLocale());
                        if (typeface != nullptr) {
                            fFallbackFonts.set(fontKey, typeface);
                        }
                    }
                } else {
                    typeface = fParagraph->fFontCollection->defaultEmojiFallback(emojiStart, textStyle.getFontStyle(), textStyle.getLocale());
                }
...
bool OneLineShaper::iterateThroughShapingRegions(const ShapeVisitor& shape)
{
    ...
    for (auto& placeholder : fParagraph->fPlaceholders) {
        ...
        std::vector<sk_sp<SkTypeface>> typefaces = fParagraph->fFontCollection->findTypefaces(
            placeholder.fTextStyle.getFontFamilies(), placeholder.fTextStyle.getFontStyle(), placeholder.fTextStyle.getFontArguments());
        ...
        uint8_t bidiLevel = (bidiIndex < fParagraph->fBidiRegions.size()) ? fParagraph->fBidiRegions[bidiIndex].level : 2;
        const SkShaper::RunHandler::RunInfo runInfo = { font, bidiLevel, SkPoint::Make(placeholder.fStyle.fWidth, placeholder.fStyle.fHeight), 1,
            SkShaper::RunHandler::Range(0, placeholder.fRange.width()) };
        auto& run = fParagraph->fRuns.emplace_back(this->fParagraph, runInfo, placeholder.fRange.start, 0.0f, 0.0f, false, fParagraph->fRuns.size(), advanceX);
    ...
    auto result = iterateThroughShapingRegions(
        [this, limitlessWidth](TextRange textRange, SkSpan<Block> styleSpan, SkScalar& advanceX, TextIndex textStart, uint8_t defaultBidiLevel) {
            ...
            iterateThroughFontStyles(
                textRange, styleSpan, [this, &shaper, defaultBidiLevel, limitlessWidth, &advanceX](Block block, TArray<SkShaper::Feature> features) {
                    ...
                    this->matchResolvedFonts(block.fStyle, [&](sk_sp<SkTypeface> typeface) {
                        ...
                        bool fakeBold = wantedWeight >= SkFontStyle::kSemiBold_Weight && wantedWeight - font.getTypeface()->fontStyle().weight() >= 200;
                        ...
                        while (unresolvedCount-- > 0) {
                            ...
                            auto scriptIter = SkShapers::HB::ScriptRunIterator(unresolvedText.begin(), unresolvedText.size());
                            ...
                            shaper->shape(unresolvedText.begin(), unresolvedText.size(), fontIter, bidiIter, *scriptIter, langIter, adjustedFeatures.data(),
                                adjustedFeatures.size(), limitlessWidth, this);
    ...
    auto findBaseChar = [&](TextIndex index, Dir dir) -> TextIndex {
        if (dir == Dir::right) {
            while (index < fCurrentRun->fTextRange.end) {
                if (this->fParagraph->codeUnitHasProperty(index, SkUnicode::CodeUnitFlags::kGraphemeStart)) {
                    return index;
                }
                ++index;
            }
...
{
    return SkGoodHash()(key.fUnicode) ^ SkGoodHash()(key.fFontStyle) ^ SkGoodHash()(key.fLocale);
}
