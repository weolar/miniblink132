...
ParagraphPainter::DecorationStyle::DecorationStyle(SkColor color, SkScalar strokeWidth, std::optional<DashPathEffect> dashPathEffect)
    : fColor(color)
    , fStrokeWidth(strokeWidth)
    , fDashPathEffect(dashPathEffect)
{
    ...
    if (fDashPathEffect) {
        const std::array<SkScalar, 4> intervals
            = { fDashPathEffect->fOnLength, fDashPathEffect->fOffLength, fDashPathEffect->fOnLength, fDashPathEffect->fOffLength };
        fPaint.setPathEffect(SkPathEffect::MakeCompose(SkDashPathEffect::Make(intervals.data(), intervals.size(), 0.0f), SkDiscretePathEffect::Make(0, 0)));
    }
}
