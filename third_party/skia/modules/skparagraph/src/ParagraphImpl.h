...
template <typename TStyle> struct StyleBlock {
    StyleBlock()
        : fRange(EMPTY_RANGE)
        , fStyle()
    {
...
public:
    ParagraphImpl(const SkString& text, ParagraphStyle style, skia_private::TArray<Block, true> blocks, skia_private::TArray<Placeholder, true> placeholders,
        sk_sp<FontCollection> fonts, sk_sp<SkUnicode> unicode);

    ParagraphImpl(const std::u16string& utf16text, ParagraphStyle style, skia_private::TArray<Block, true> blocks,
        skia_private::TArray<Placeholder, true> placeholders, sk_sp<FontCollection> fonts, sk_sp<SkUnicode> unicode);
    ...
    {
        return fParagraphStyle.getApplyRoundingHack();
    }
    ...
    TextLine& addLine(SkVector offset, SkVector advance, TextRange textExcludingSpaces, TextRange text, TextRange textIncludingNewlines, ClusterRange clusters,
        ClusterRange clustersWithGhosts, SkScalar widthWithSpaces, InternalLineMetrics sizes);
    ...
    SkSpan<Run> runs()
    {
        return SkSpan<Run>(fRuns.data(), fRuns.size());
    }
    SkSpan<Block> styles()
    {
    ...
    {
        return SkSpan<TextLine>(fLines.data(), fLines.size());
    }
    ...
    {
        return paragraphStyle().getStrutStyle().getStrutEnabled();
    }
    ...
    {
        return fMaxWidthWithTrailingSpaces;
    }
