...
class DirText {
    DirText(bool dir, size_t s, size_t e)
        : start(s)
        , end(e)
    {
    }
...
class Run {
public:
    Run(ParagraphImpl* owner, const SkShaper::RunHandler::RunInfo& info, size_t firstChar, SkScalar heightMultiplier, bool useHalfLeading,
    ...
    {
        auto ascent = ascentStyle == LineMetricStyle::Typographic ? this->ascent() : this->correctAscent();
        auto descent = descentStyle == LineMetricStyle::Typographic ? this->descent() : this->correctDescent();
    ...
    template <typename Visitor> void iterateThroughClustersInTextOrder(Visitor visitor);
...
template <typename Visitor> void Run::iterateThroughClustersInTextOrder(Visitor visitor)
{
    ...
    if (leftToRight()) {
        ...
        for (size_t glyph = 1; glyph <= this->size(); ++glyph) {
            ...
            visitor(start, glyph, fClusterStart + cluster, fClusterStart + nextCluster, this->calculateWidth(start, glyph, glyph == size()),
        ...
        for (int32_t start = this->size() - 1; start >= 0; --start) {
            ...
            visitor(start, glyph, fClusterStart + cluster, fClusterStart + nextCluster, this->calculateWidth(start, glyph, glyph == 0),
                this->calculateHeight(LineMetricStyle::CSS, LineMetricStyle::CSS));
...
public:
    ...
    {
        return fIsWhiteSpaceBreak;
    }
    bool isIntraWordBreak() const
    {
        return fIsIntraWordBreak;
    }
    ...
    {
        return isHardBreak() || isSoftBreak();
    }
...
public:
    ...
    bool isClean()
    {
        return (fAscent == SK_ScalarMax && fDescent == SK_ScalarMin && fLeading == 0 && fRawAscent == SK_ScalarMax && fRawDescent == SK_ScalarMin
            && fRawLeading == 0);
    }

    SkScalar delta() const
    {
        return height() - ideographicBaseline();
    }
    ...
    {
        return fLeading / 2 - fAscent + (ascentStyle == LineMetricStyle::Typographic ? run->ascent() : run->correctAscent()) + delta();
    }
