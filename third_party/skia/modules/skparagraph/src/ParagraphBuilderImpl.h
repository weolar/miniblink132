...
public:
    ...
    std::tuple<std::vector<SkUnicode::Position>, std::vector<SkUnicode::Position>, std::vector<SkUnicode::LineBreakBefore>> getClientICUData() const override
    {
...
#if !defined(SK_DISABLE_LEGACY_PARAGRAPH_UNICODE)
    // Just until we fix all the code; calls icu::make inside
    static std::unique_ptr<ParagraphBuilder> make(const ParagraphStyle& style, sk_sp<FontCollection> fontCollection);
#endif
