...
ParagraphImpl::ParagraphImpl(const SkString& text, ParagraphStyle style, TArray<Block, true> blocks, TArray<Placeholder, true> placeholders,
...
ParagraphImpl::ParagraphImpl(const std::u16string& utf16text, ParagraphStyle style, TArray<Block, true> blocks, TArray<Placeholder, true> placeholders,
    sk_sp<FontCollection> fonts, sk_sp<SkUnicode> unicode)
    : ParagraphImpl(SkString(), std::move(style), std::move(blocks), std::move(placeholders), std::move(fonts), std::move(unicode))
{
    SkASSERT(fUnicode);
...
{
    fUnicode->forEachCodepoint(&fText[textRange.start], textRange.width(),
        [&](SkUnichar unichar, int32_t start, int32_t end, int32_t count) { fUnresolvedCodepoints.emplace(unichar); });
}
...
void ParagraphImpl::layout(SkScalar rawWidth)
{
    ...
    if ((!SkIsFinite(rawWidth) || fLongestLine <= floorWidth) && fState >= kLineBroken && fLines.size() == 1 && fLines.front().ellipsis() == nullptr) {
...
bool ParagraphImpl::computeCodeUnitProperties()
{
    ...
    // Get bidi regions
    auto textDirection = fParagraphStyle.getTextDirection() == TextDirection::kLtr ? SkUnicode::TextDirection::kLTR : SkUnicode::TextDirection::kRTL;
    ...
    if (!fUnicode->computeCodeUnitFlags(&fText[0], fText.size(), this->paragraphStyle().getReplaceTabCharacters(), &fCodeUnitProperties)) {
        return false;
    }
...
Cluster::Cluster(ParagraphImpl* owner, RunIndex runIndex, size_t start, size_t end, SkSpan<const char> text, SkScalar width, SkScalar height)
    : fOwner(owner)
    , fRunIndex(runIndex)
...
void ParagraphImpl::applySpacingAndBuildClusterTable()
{
    ...
    if (letterSpacingStyles == 1 && !hasWordSpacing && fTextStyles.size() == 1 && fTextStyles[0].fRange.width() == fText.size() && fRuns.size() == 1) {
    ...
    for (auto& run : fRuns) {
        ...
        if (run.isPlaceholder()) {
            ...
            run.iterateThroughClustersInTextOrder(
                [runIndex, this](size_t glyphStart, size_t glyphEnd, size_t charStart, size_t charEnd, SkScalar width, SkScalar height) {
...
void ParagraphImpl::breakShapedTextIntoLines(SkScalar maxWidth)
{
    ...
    if (!fHasLineBreaks && !fHasWhitespacesInside && fPlaceholders.size() == 1 && fRuns.size() == 1 && fRuns[0].fAdvance.fX <= maxWidth) {
        ...
        this->addLine(
            SkPoint::Make(0, 0), advance, textExcludingSpaces, textRange, textRange, clusterRange, clusterRangeWithGhosts, run.advance().x(), metrics);
    ...
    textWrapper.breakTextIntoLines(this, maxWidth,
        [&](TextRange textExcludingSpaces, TextRange text, TextRange textWithNewlines, ClusterRange clusters, ClusterRange clustersWithGhosts,
            SkScalar widthWithSpaces, size_t startPos, size_t endPos, SkVector offset, SkVector advance, InternalLineMetrics metrics, bool addEllipsis) {
...
void ParagraphImpl::formatLines(SkScalar maxWidth)
{
    ...
    const bool isLeftAligned
        = effectiveAlign == TextAlign::kLeft || (effectiveAlign == TextAlign::kJustify && fParagraphStyle.getTextDirection() == TextDirection::kLtr);
...
void ParagraphImpl::resolveStrut()
{
    ...
    if (strutStyle.getHeightOverride()) {
        ...
        if (strutStyle.getHalfLeading()) {
            ...
            const auto strutHeightMultiplier
                = strutMetricsHeight == 0 ? strutStyle.getHeight() : strutStyle.getHeight() * strutStyle.getFontSize() / strutMetricsHeight;
            strutAscent = metrics.fAscent * strutHeightMultiplier;
            strutDescent = metrics.fDescent * strutHeightMultiplier;
        }
        fStrutMetrics = InternalLineMetrics(strutAscent, strutDescent, strutLeading, metrics.fAscent, metrics.fDescent, metrics.fLeading);
    } else {
        fStrutMetrics = InternalLineMetrics(metrics.fAscent, metrics.fDescent, strutLeading);
    }
...
TextLine& ParagraphImpl::addLine(SkVector offset, SkVector advance, TextRange textExcludingSpaces, TextRange text, TextRange textIncludingNewLines,
    ClusterRange clusters, ClusterRange clustersWithGhosts, SkScalar widthWithSpaces, InternalLineMetrics sizes)
{
    // Define a list of styles that covers the line
    auto blocks = findAllBlocks(textExcludingSpaces);
    return fLines.emplace_back(
        this, offset, advance, blocks, textExcludingSpaces, text, textIncludingNewLines, clusters, clustersWithGhosts, widthWithSpaces, sizes);
}

// Returns a vector of bounding boxes that enclose all text between
// start and end glyph indexes, including start and excluding end
std::vector<TextBox> ParagraphImpl::getRectsForRange(unsigned start, unsigned end, RectHeightStyle rectHeightStyle, RectWidthStyle rectWidthStyle)
{
...
{
    SkASSERT(clusterRange.start < SkToSizeT(fClusters.size()) && clusterRange.end <= SkToSizeT(fClusters.size()));
...
{
    SkASSERT(blockRange.start < SkToSizeT(fTextStyles.size()) && blockRange.end <= SkToSizeT(fTextStyles.size()));
...
void ParagraphImpl::computeEmptyMetrics()
{
    ...
    auto typefaces = fontCollection()->findTypefaces(textStyle.getFontFamilies(), textStyle.getFontStyle(), textStyle.getFontArguments());
    ...
    if (!paragraphStyle().getStrutStyle().getForceStrutHeight() && textStyle.getHeightOverride()) {
        ...
        if (paragraphStyle().getStrutStyle().getHalfLeading()) {
            fEmptyMetrics.update(fEmptyMetrics.ascent(), fEmptyMetrics.descent(), fEmptyMetrics.leading() + strutHeight - intrinsicHeight);
        } else {
            const auto multiplier = strutHeight / intrinsicHeight;
            fEmptyMetrics.update(fEmptyMetrics.ascent() * multiplier, fEmptyMetrics.descent() * multiplier, fEmptyMetrics.leading() * multiplier);
        }
    }
    ...
    if (emptyParagraph) {
        ...
        auto disableFirstAscent
            = (paragraphStyle().getTextHeightBehavior() & TextHeightBehavior::kDisableFirstAscent) == TextHeightBehavior::kDisableFirstAscent;
        auto disableLastDescent
            = (paragraphStyle().getTextHeightBehavior() & TextHeightBehavior::kDisableLastDescent) == TextHeightBehavior::kDisableLastDescent;
        fEmptyMetrics.update(disableFirstAscent ? fEmptyMetrics.rawAscent() : fEmptyMetrics.ascent(),
            disableLastDescent ? fEmptyMetrics.rawDescent() : fEmptyMetrics.descent(), fEmptyMetrics.leading());
    }
...
{
    while (utf8 > 0 && (fCodeUnitProperties[utf8] & SkUnicode::CodeUnitFlags::kGraphemeStart) == 0) {
        --utf8;
    }
    return utf8;
}
...
{
    while (utf8 < fText.size() && (fCodeUnitProperties[utf8] & SkUnicode::CodeUnitFlags::kGraphemeStart) == 0) {
        ++utf8;
    }
    return utf8;
}
...
{
    while (utf8 < fText.size() && (fCodeUnitProperties[utf8] & SkUnicode::CodeUnitFlags::kGlyphClusterStart) == 0) {
        ++utf8;
    }
    return utf8;
}
...
{
    while (utf8 > 0 && (fCodeUnitProperties[utf8] & SkUnicode::CodeUnitFlags::kGlyphClusterStart) == 0) {
        --utf8;
    }
    return utf8;
}
...
void ParagraphImpl::ensureUTF16Mapping()
{
    fillUTF16MappingOnce([&] {
        SkUnicode::extractUtfConversionMapping(
            this->text(), [&](size_t index) { fUTF8IndexForUTF16Index.emplace_back(index); },
        ...
        for (auto& rec : line.fTextBlobCache) {
            ...
            while (iter.experimentalNext(&run)) {
                const Paragraph::VisitorInfo info = {
                    run.font, rec.fOffset, rec.fClipRect.fRight, run.count, run.glyphs, run.positions, clusterPtr,
                    0, // flags
                };
...
bool ParagraphImpl::getGlyphClusterAt(TextIndex codeUnitIndex, GlyphClusterInfo* glyphInfo)
{
    ...
    for (auto c = line.clustersWithSpaces().start; c < line.clustersWithSpaces().end; ++c) {
        ...
        if (cluster.contains(codeUnitIndex)) {
            std::vector<TextBox> boxes;
            line.getRectsForRange(cluster.textRange(), RectHeightStyle::kTight, RectWidthStyle::kTight, boxes);
            if (!boxes.empty()) {
                if (glyphInfo) {
...
bool ParagraphImpl::getClosestGlyphClusterAt(SkScalar dx, SkScalar dy, GlyphClusterInfo* glyphInfo)
{
...
void ParagraphImpl::extendedVisit(const ExtendedVisitor& visitor)
{
    int lineNumber = 0;
    for (auto& line : fLines) {
        line.iterateThroughVisualRuns(false, [&](const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
            *runWidthInLine = line.iterateThroughSingleRunByStyles(TextLine::TextAdjustment::GlyphCluster, run, runOffsetInLine, textRange, StyleType::kNone,
                [&](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
                    ...
                    SkPoint offset = SkPoint::Make(line.offset().fX + context.fTextShift, line.offset().fY + correctedBaseline);
                    SkRect rect = context.clip.makeOffset(line.offset());
                    AutoSTArray<16, SkRect> glyphBounds;
                    glyphBounds.reset(SkToInt(run->size()));
                    run->font().getBounds(run->glyphs().data(), SkToInt(run->size()), glyphBounds.data(), nullptr);
                    STArray<128, uint32_t> clusterStorage;
                    const uint32_t* clusterPtr = run->clusterIndexes().data();
                    if (run->fClusterStart > 0) {
                        clusterStorage.reset(context.size);
                        for (size_t i = 0; i < context.size; ++i) {
                            clusterStorage[i] = run->fClusterStart + run->fClusterIndexes[i];
                        }
                        clusterPtr = &clusterStorage[0];
                    }
                    const Paragraph::ExtendedVisitorInfo info = {
                        run->font(), offset, SkSize::Make(rect.width(), rect.height()), SkToS16(context.size), &run->glyphs()[context.pos],
                        &run->fPositions[context.pos], &glyphBounds[context.pos], clusterPtr,
                        0, // flags
                    };
                    visitor(lineNumber, &info);
                });
        ...
        *runWidthInLine = line.iterateThroughSingleRunByStyles(TextLine::TextAdjustment::GlyphCluster, run, runOffsetInLine, textRange, StyleType::kNone,
            [&](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
                ...
                struct Rec {
                    ...
                    const SkPoint* fPos;
                    int fNotConverted;
                } rec = { dest, SkPoint::Make(rect.left(), rect.top()), &run->positions()[context.pos], 0 };
                font.getPaths(
                    &run->glyphs()[context.pos], context.size,
...
SkPath Paragraph::GetPath(SkTextBlob* textBlob)
{
    ...
    while (!iter.done()) {
        ...
        struct Rec {
            ...
            SkPoint fOffset;
            const SkPoint* fPos;
        } rec = { &path, { textBlob->bounds().left(), textBlob->bounds().top() }, iter.points() };
        font.getPaths(
            iter.glyphs(), iter.glyphCount(),
            [](const SkPath* src, const SkMatrix& mx, void* ctx) {
                Rec* rec = (Rec*)ctx;
                if (src) {
                    SkMatrix tmp(mx);
                    tmp.postTranslate(rec->fPos->fX - rec->fOffset.fX, rec->fPos->fY - rec->fOffset.fY);
...
bool ParagraphImpl::containsEmoji(SkTextBlob* textBlob)
{
    ...
    while (!iter.done() && !result) {
        ...
        this->getUnicode()->forEachCodepoint(iter.text(), iter.textSize(), [&](SkUnichar unichar, int32_t start, int32_t end, int32_t count) {
...
bool ParagraphImpl::containsColorFontOrBitmap(SkTextBlob* textBlob)
{
    ...
    while (!iter.done() && !flag) {
        iter.font().getPaths((const SkGlyphID*)iter.glyphs(), iter.glyphCount(),
            [](const SkPath* path, const SkMatrix& mx, void* ctx) {
                if (path == nullptr) {
                    bool* flag1 = (bool*)ctx;
                    *flag1 = true;
