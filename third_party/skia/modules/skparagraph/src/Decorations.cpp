...
namespace {
void draw_line_as_rect(ParagraphPainter* painter, SkScalar x, SkScalar y, SkScalar width, const ParagraphPainter::DecorationStyle& decorStyle)
{
...
void Decorations::paint(ParagraphPainter* painter, const TextStyle& textStyle, const TextLine::ClipContext& context, SkScalar baseline)
{
    ...
    for (auto decoration : AllTextDecorations) {
        ...
        calculatePosition(
            decoration, decoration == TextDecoration::kOverline ? context.run->correctAscent() - context.run->ascent() : context.run->correctAscent());
...
void Decorations::calculatePosition(TextDecoration decoration, SkScalar ascent)
{
    ...
    case TextDecoration::kLineThrough: {
        fPosition = (fFontMetrics.fFlags & SkFontMetrics::FontMetricsFlags::kStrikeoutPositionIsValid_Flag) ? fFontMetrics.fStrikeoutPosition
...
void Decorations::calculatePaint(const TextStyle& textStyle)
{
    ...
    SkColor color = (textStyle.getDecorationColor() == SK_ColorTRANSPARENT) ? textStyle.getColor() : textStyle.getDecorationColor();
...
void Decorations::calculateWaves(const TextStyle& textStyle, SkRect clip)
{
    ...
    while (x_start + quarterWave * 2 < clip.width()) {
        fPath.rQuadTo(quarterWave, wave_count % 2 != 0 ? quarterWave : -quarterWave, quarterWave * 2, 0);
