...
DEF_TEST(SkShaperTest_cluster, reporter)
{
    ...
    for (auto& oneCase : cases) {
        ...
        auto checker = [&](size_t glyphStartIndex, size_t glyphEndIndex, uint32_t utf8StartIndex, uint32_t utf8EndIndex) {
...
DEF_TEST(SkShaperTest_VisualizeCluster, reporter)
{
    ...
    std::vector<TestCase> cases = {
        ...
        { "المادة", { 246, 268, 241, 205, 240 }, { 10, 8, 6, 2, 0 }, { { "ادة", { 246, 268, 241 } }, { "لم", { 205 } }, { "ا", { 240 } } } },
    };

    for (auto& oneCase : cases) {
        size_t answerCount = 0;
        auto checker = [&](int codePointCount, SkSpan<const char> utf1to1, SkSpan<const SkGlyphID> glyph1to1) {
            if (answerCount < oneCase.answers.size()) {
                Answer a = oneCase.answers[answerCount];
        ...
        SkShaperJSONWriter::VisualizeClusters(oneCase.utf8.c_str(), 0, oneCase.utf8.size(), SkSpan(oneCase.glyphIDs), SkSpan(oneCase.clusters), checker);
    }
}
