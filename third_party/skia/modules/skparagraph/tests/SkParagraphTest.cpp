    ...
    static const std::vector<sk_sp<SkTypeface>>& getTypefaces()
    {
        static std::vector<sk_sp<SkTypeface>> typefaces = []() -> std::vector<sk_sp<SkTypeface>> {
            ...
            TArray<SkString> paths;
            {
                ...
                if (paths.size()) {
                    SkTQSort(paths.begin(), paths.end(), [](const SkString& a, const SkString& b) { return strcmp(a.c_str(), b.c_str()) < 0; });
                }
            }
            ...
            for (auto&& path : paths) {
                ...
                if (typeface) {
                    if ((false)) {
                        ...
                        SkDebugf("Creating: %s size: %zu\n", familyName.c_str(), typeface->openExistingStream(nullptr)->getLength());
                    }
            ...
            SkASSERTF_RELEASE(typefaces.size(),
                "--paragraph_fonts set but no fonts found."
...
class TestCanvas {
public:
    TestCanvas(const char* testName)
        : name(testName)
    {
...
UNIX_ONLY_TEST(SkParagraph_SimpleParagraph, reporter)
{
    ...
    for (auto& line : impl->lines()) {
        line.scanStyles(StyleType::kDecorations, [&index, reporter](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
...
UNIX_ONLY_TEST(SkParagraph_SimpleRedParagraph, reporter)
{
    ...
    size_t index = 0;
    for (auto& line : impl->lines()) {
        line.scanStyles(StyleType::kDecorations, [reporter, &index](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
            REPORTER_ASSERT(reporter, index == 0);
...
UNIX_ONLY_TEST(SkParagraph_RainbowParagraph, reporter)
{
    ...
    const char* text5 = "Continue Last Style With lots of words to check if it overlaps "
                        "properly or not"; // [68:)
    const char* text42 = "Roboto"
    ...
    size_t index = 0;
    impl->lines()[0].scanStyles(StyleType::kAllAttributes, [&](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
        switch (index) {
        case 0:
    ...
    impl->lines()[1].scanStyles(StyleType::kAllAttributes, [&](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
        switch (index) {
        case 4:
            REPORTER_ASSERT(reporter, style.equals(text_style4));
...
UNIX_ONLY_TEST(SkParagraph_LeftAlignParagraph, reporter)
{
    ...
    const char* text = "This is a very long sentence to test if the text will properly wrap "
...
UNIX_ONLY_TEST(SkParagraph_RightAlignParagraph, reporter)
{
    ...
    auto calculate = [](const TextLine& line) -> SkScalar { return TestCanvasWidth - 100 - line.offset().fX - line.width(); };
...
UNIX_ONLY_TEST(SkParagraph_CenterAlignParagraph, reporter)
{
    ...
    auto calculate = [](const TextLine& line) -> SkScalar { return TestCanvasWidth - 100 - (line.offset().fX * 2 + line.width()); };
...
UNIX_ONLY_TEST(SkParagraph_JustifyAlignParagraph, reporter)
{
    ...
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(impl->lines()[0].offset().fY, expected_y, EPSILON100));
    expected_y += 30;
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(impl->lines()[1].offset().fY, expected_y, EPSILON100));
    expected_y += 30;
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(impl->lines()[2].offset().fY, expected_y, EPSILON100));
    expected_y += 30;
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(impl->lines()[3].offset().fY, expected_y, EPSILON100));
    expected_y += 30 * 9;
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(impl->lines()[12].offset().fY, expected_y, EPSILON100));
...
UNIX_ONLY_TEST(SkParagraph_JustifyRTL, reporter)
{
    ...
    auto calculate = [](const TextLine& line) -> SkScalar { return TestCanvasWidth - 100 - line.width(); };
    for (auto& line : impl->lines()) {
...
UNIX_ONLY_TEST(SkParagraph_JustifyRTLNewLine, reporter)
{
    ...
    for (auto& line : impl->lines()) {
        ...
        if (num == 0 || num == 5) {
            ...
            REPORTER_ASSERT(
                reporter, SkScalarNearlyEqual(line.width(), TestCanvasWidth - 100, EPSILON100), "#%zd: %f <= %d\n", num, line.width(), TestCanvasWidth - 100);
        }
    }
}
...
UNIX_ONLY_TEST(SkParagraph_DecorationsParagraph, reporter)
{
    ...
    size_t index = 0;
    for (auto& line : impl->lines()) {
        line.scanStyles(StyleType::kDecorations, [&](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
...
UNIX_ONLY_TEST(SkParagraph_ItalicsParagraph, reporter)
{
    ...
    auto& line = impl->lines()[0];
    size_t index = 0;
    line.scanStyles(StyleType::kForeground, [&](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
        switch (index) {
        case 0:
...
UNIX_ONLY_TEST(SkParagraph_ChineseParagraph, reporter)
{
    ...
    const char* text = "     左線読設重説切後碁給能上目秘使約。満毎冠行来昼本可必図将発確年。今属場育"
...
UNIX_ONLY_TEST(SkParagraph_ArabicParagraph, reporter)
{
    ...
    const char* text = "من أسر وإعلان الخاصّة وهولندا،, عل قائمة الضغوط بالمطالبة تلك. الصفحة "
...
UNIX_ONLY_TEST(SkParagraph_SpacingParagraph, reporter)
{
    ...
    REPORTER_ASSERT(reporter, impl->lines().size() == 1);
    size_t index = 0;
    impl->lines().begin()->scanStyles(StyleType::kLetterSpacing, [&](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
        ++index;
        return true;
    });
    REPORTER_ASSERT(reporter, index == 4);
    index = 0;
    impl->lines().begin()->scanStyles(StyleType::kWordSpacing, [&](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
        ++index;
        return true;
    });
    REPORTER_ASSERT(reporter, index == 4);
...
UNIX_ONLY_TEST(SkParagraph_EmojiParagraph, reporter)
{
    ...
    for (auto& line : impl->lines()) {
        if (&line != impl->lines().end() - 1) {
            ...
            REPORTER_ASSERT(reporter, line.width() == 998.25f || (998.1 < line.width() && line.width() < 998.2), "width: %f", line.width());
        } else {
...
UNIX_ONLY_TEST(SkParagraph_RepeatLayoutParagraph, reporter)
{
    ...
    const char* text = "Sentence to layout at diff widths to get diff line counts. short words "
...
UNIX_ONLY_TEST(SkParagraph_SimpleShadow, reporter)
{
    ...
    size_t index = 0;
    for (auto& line : impl->lines()) {
        line.scanStyles(StyleType::kShadow, [&](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
...
UNIX_ONLY_TEST(SkParagraph_BaselineParagraph, reporter)
{
    ...
    const char* text = "左線読設Byg後碁給能上目秘使約。満毎冠行来昼本可必図将発確年。今属場育"
    ...
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(paragraph->getIdeographicBaseline(), 79.035f, EPSILON100));
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(paragraph->getAlphabeticBaseline(), 63.305f, EPSILON100));
}
...
UNIX_ONLY_TEST(SkParagraph_WhitespacesInMultipleFonts, reporter)
{
    ...
    TextStyle text_style;
    text_style.setFontFamilies({ SkString("Roboto"), SkString("Noto Color Emoji"), SkString("Source Han Serif CN") });
    text_style.setFontSize(60);
    builder.pushStyle(text_style);
    builder.addText(text, len);
    builder.pop();
...
DEF_TEST_DISABLED(SkParagraph_JSON2, reporter)
{
    ...
    auto cluster = 0;
    for (auto& run : impl->runs()) {
        SkShaperJSONWriter::VisualizeClusters(impl->text().begin() + run.textRange().start, 0, run.textRange().width(), run.glyphs(), run.clusterIndexes(),
...
UNIX_ONLY_TEST(SkParagraph_CacheFontRanges, reporter)
{
    ...
    auto test = [&](const char* text1, const char* text2, const char* font1, const char* font2, int count, bool expectedToBeFound) {
        ParagraphBuilderImpl builder(paragraph_style, fontCollection, get_unicode());
...
UNIX_ONLY_TEST(SkParagraph_FontResolutions, reporter)
{
    ...
    sk_sp<TestFontCollection> fontCollection = sk_make_sp<TestFontCollection>(GetResourcePath("fonts").c_str(), false);
    SKIP_IF_FONTS_NOT_FOUND(reporter, fontCollection)
...
UNIX_ONLY_TEST(SkParagraph_FontStyle, reporter)
{
    ...
    text_style.setColor(SK_ColorBLACK);
    text_style.setFontSize(20);
    SkFontStyle fs = SkFontStyle(SkFontStyle::Weight::kLight_Weight, SkFontStyle::Width::kNormal_Width, SkFontStyle::Slant::kUpright_Slant);
    ...
    SkFontStyle bi = SkFontStyle(SkFontStyle::Weight::kBold_Weight, SkFontStyle::Width::kNormal_Width, SkFontStyle::Slant::kItalic_Slant);
...
UNIX_ONLY_TEST(SkParagraph_Shaping, reporter)
{
    ...
    sk_sp<TestFontCollection> fontCollection = sk_make_sp<TestFontCollection>(GetResourcePath("fonts").c_str(), true);
    SKIP_IF_FONTS_NOT_FOUND(reporter, fontCollection)
    ...
    SkFontStyle b = SkFontStyle(SkFontStyle::Weight::kNormal_Weight, SkFontStyle::Width::kNormal_Width, SkFontStyle::Slant::kUpright_Slant);
...
UNIX_ONLY_TEST(SkParagraph_LineMetrics, reporter)
{
    ...
    SkDEBUGCODE(auto impl = static_cast<ParagraphImpl*>(paragraph.get());) SkASSERT(metrics.size() == impl->lines().size());
    for (size_t i = 0; i < metrics.size(); ++i) {
        SkDEBUGCODE(auto& line = impl->lines()[i];) SkDEBUGCODE(auto baseline = metrics[i].fBaseline;) SkDEBUGCODE(auto top = line.offset().fY;)
            SkDEBUGCODE(auto bottom = line.offset().fY + line.height();) SkASSERT(baseline > top && baseline <= bottom);
    }
...
UNIX_ONLY_TEST(SkParagraph_RTLFollowedByLTR, reporter)
{
    ...
    REPORTER_ASSERT(reporter, boxes[0].direction == TextDirection::kRtl && boxes[1].direction == TextDirection::kLtr);
...
UNIX_ONLY_TEST(SkParagraph_NonMonotonicGlyphsLTR, reporter)
{
    ...
    std::u16string text = u"\u0068\u0301\u0350\u0312\u0357\u030C\u0369\u0305\u036C\u0304\u0310\u033F\u0366\u0350 ";
    /*
            u"\u0343\u0364\u0369\u0311\u0309\u030E\u0365\u031B\u0340\u0337\u0335\u035E\u0334\u0328"
            u"\u0360\u0360\u0315\u035F\u0340\u0340\u0362\u0360\u0322\u031B\u031B\u0337\u0340\u031E"
            u"\u031F\u032A\u0331\u0345\u032F\u0332\u032E\u0333\u0353\u0320\u0345\u031C\u031F\u033C"
...
UNIX_ONLY_TEST(SkParagraph_MultiStyle_Logo, reporter)
{
    ...
    size_t index = 0;
    SkScalar left = 0.0f;
    impl->lines().data()->scanStyles(
        StyleType::kDecorations, [&index, &left, reporter](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
            switch (index) {
            case 0:
                REPORTER_ASSERT(reporter, context.pos == 0 && context.size == 1);
...
UNIX_ONLY_TEST(SkParagraph_MultiStyle_Zalgo, reporter)
{
    ...
    std::u16string text16
        = u">S\u035B\u036D\u030B\u0346\u0308\u0314\u0307\u0357\u030D\u0351\u030E\u036A\u036E\u0367\u0363\u033D\u036B\u0363\u0301\u036C\u0300\u034C\u0351\u0342"
          u"\u0357\u0352\u030D\u0314\u0304\u0367\u030F\u031A\u0309\u030C\u030A\u030A\u033F\u0300\u030C\u0303\u0304\u0350\u0313\u031A\u0313\u0363\u0321\u035C"
          u"\u035D\u035F\u0360\u0335\u034F\u0321\u0327\u0338\u035C\u0335\u0363\u0337\u0321\u0347\u031C\u0318\u033B\u033A\u0318\u031F\u031D\u0359\u032C\u0318"
          u"\u0329\u0347\u032D\u033C\u0325\u0316\u0324\u0326\u034Ek\u0302\u0368\u0309\u036A\u0312\u034B\u035B\u0300\u030D\u034A\u0367\u033F\u0305\u0346\u0313"
          u"\u0314\u0314\u036C\u0307\u0311\u033F\u0369\u0357\u036E\u030E\u034C\u033F\u0304\u0349\u0329\u0318\u035A\u031C\u0339\u0317\u0317\u034D\u0324\u0325"
          u"\u0331\u0349\u0333\u0355\u0345\u0356\u0324\u0332\u0323\u035A\u032E\u031E\u032C\u0332\u034D\u0354\u032F\u033B\u032E\u031E\u032D\u0348\u0317\u032B"
          u"\u0353P\u031A\u0351\u0350\u030E\u0303\u0312\u0350\u034B\u0334\u031B\u035E\u0358\u0321\u0335\u0321\u032A\u0359\u033C\u0323\u031F\u0329\u032D\u032B"
          u"\u0331\u0359\u032C\u0354\u0349\u034D\u0318\u0320\u0349\u0326\u031D\u0318\u0325\u031F\u0317\u0356\u032B\u0324\u0355\u0319\u032C\u0326\u034D\u0331"
          u"\u0316\u032E\u0331a\u0313\u033F\u0346\u030E\u034B\u0313\u0366\u0344\u0366\u030C\u0301\u0304\u0357\u030C\u0313\u034C\u035C\u0336\u035C\u0321\u0336"
          u"\u035D\u0315\u0335\u0335\u0335\u035F\u0336\u0336\u0327\u0322\u0361\u0362\u0322\u033A\u0354\u0323\u0356\u032D\u033A\u034D\u0324\u035A\u0331\u031C"
          u"\u0330\u0325\u0355\u032C\u0325\u0332\u031E\u0325\u0318\u0347\u035A\u033A\u0330\u035A\u032A\u033A\u0354\u0324\u034Dr\u0301\u0361\u0315\u034E\u031D"
          u"\u0355\u0349\u033B\u034E\u0324\u0345\u0345\u032D\u035A\u0317\u0333\u0316\u0319\u0318\u035A\u0345\u032B\u0356\u0353\u035A\u0349\u0354\u0348\u031F"
          u"\u0330\u031F\u032C\u0317\u0353\u031F\u035A\u0331a\u033D\u036E\u0369\u0305\u0352\u031A\u036B\u0357\u0342\u030E\u0366\u0344\u0343\u0338\u035C\u0361"
          u"\u0322\u031B\u0358\u031B\u0362\u0336\u0363\u0337\u035C\u0322\u035C\u0321\u0335\u0336\u0345\u031C\u032E\u0326\u031C\u0325\u031C\u032F\u0319\u0353"
          u"\u0354\u033C\u0317\u033Bg\u0304\u0368\u0368\u030A\u0352\u0369\u036D\u0364\u030D\u0305\u0303\u036A\u030B\u030F\u0313\u030D\u031A\u030B\u0357\u030B"
          u"\u0368\u030F\u033D\u0344\u0314\u0300\u030B\u0309\u036B\u0305\u0302\u036D\u036B\u030F\u0352\u034B\u0365\u0322\u031B\u035C\u0355\u0317\u0356\u0316"
          u"\u0324\u0326\u0318\u0354r\u0300\u0302\u0312\u036A\u034C\u033F\u034C\u0305\u035B\u0368\u036A\u0352\u030D\u0365\u0309\u0364\u030C\u033F\u0306\u0301"
          u"\u036D\u0346\u0303\u0312\u0364\u035B\u030A\u0367\u033D\u035D\u0360\u0322\u0358\u0327\u0327\u0336\u0325\u0324\u033Ca\u0351\u0351\u0313\u0351\u036E"
          u"\u0303\u0342\u0304\u035B\u0301\u0344\u034B\u0302\u034C\u033D\u0304\u0352\u0314\u0301\u0307\u0368\u0367\u036D\u0350\u0366\u031A\u030B\u0368\u030D"
          u"\u0366\u030D\u030B\u0346\u0314\u0367\u0351\u034B\u034C\u0308\u0343\u035B\u035B\u0327\u0322\u0327\u0362\u035C\u035C\u0322\u034F\u0322\u031B\u0334"
          u"\u0327\u0333\u034D\u0339\u035A\u0330\u0339\u033B\u0354p\u0346\u0366\u031A\u0363\u034A\u033D\u0314\u0342\u0309\u0363\u0314\u0363\u030C\u030C\u0309"
          u"\u0303\u030B\u0302\u0352\u036B\u0304\u030E\u0310\u0357\u0309\u030C\u0303\u033D\u033D\u0328\u0341\u0358\u0340\u0321\u035C\u035F\u0363\u0331\u0349h"
          u"\u035C\u035D\u032D\u032E\u0318\u0317\u0354\u031C\u032F\u0354\u0348\u032F\u033A\u0354\u0317\u0323\u032D\u035A\u0331\u0330\u0319\u033C\u0339\u035A"
          u"\u0323\u033B\u0325\u0332\u032E\u034D\u0324<";
    ...
    auto resSK = paragraph->getRectsForRange(0, K, RectHeightStyle::kTight, RectWidthStyle::kTight);
    REPORTER_ASSERT(reporter, resSK.size() != 0);
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(resSK[0].rect.width(), 10.45f, EPSILON100));

    auto resKP = paragraph->getRectsForRange(K, P, RectHeightStyle::kTight, RectWidthStyle::kTight);
    ...
    auto resPh = paragraph->getRectsForRange(P, h, RectHeightStyle::kTight, RectWidthStyle::kTight);
...
UNIX_ONLY_TEST(SkParagraph_RtlEllipsis1, reporter)
{
    ...
    auto& line = impl->lines()[0];
    bool first = true;
    line.iterateThroughVisualRuns(true, [&](const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
...
UNIX_ONLY_TEST(SkParagraph_TextEditingFunctionality, reporter)
{
    ...
    REPORTER_ASSERT(reporter, foundClosest && glyphInfo.fClusterTextRange.start == 0 && glyphInfo.fClusterTextRange.end == 1);
    foundClosest = paragraph->getClosestGlyphClusterAt(TestCanvasWidth / 2, 20, &glyphInfo);
    REPORTER_ASSERT(reporter, foundClosest && glyphInfo.fClusterTextRange.start == 61 && glyphInfo.fClusterTextRange.end == 62);
    foundClosest = paragraph->getClosestGlyphClusterAt(TestCanvasWidth + 10, 30, &glyphInfo);

    REPORTER_ASSERT(reporter, foundClosest && glyphInfo.fClusterTextRange.start == 228 && glyphInfo.fClusterTextRange.end == 229);
    ...
    auto test = [&](const char* text, SkUnichar expected) {
        ...
        if ((false)) {
            ...
            while (begin != end) {
                ...
                SkDebugf("  %d: %s %s\n", unicode, icu->isEmoji(unicode) ? "isEmoji" : "", icu->isEmojiComponent(unicode) ? "isEmojiComponent" : "");
            }
