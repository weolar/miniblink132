...
template <typename C, typename UnaryFunction> UnaryFunction directional_for_each(C& c, bool forwards, UnaryFunction f)
{
    return forwards ? std::for_each(std::begin(c), std::end(c), f) : std::for_each(std::rbegin(c), std::rend(c), f);
}
