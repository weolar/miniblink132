...
std::pair<sk_sp<SkColorFilter>, sk_sp<PrecompileColorFilter>> create_blend_colorfilter(Fuzz* fuzz)
{
    ...
    while (!cf && !fuzz->exhausted()) {
        cf = SkColorFilters::Blend(random_color4f(fuzz), create_random_colorspace(fuzz), random_blend_mode(fuzz));
    }
...
{
    sk_sp<SkColorFilter> cf = SkColorFilters::Matrix(SkColorMatrix::RGBtoYUV(SkYUVColorSpace::kJPEG_Full_SkYUVColorSpace));
...
{
    sk_sp<SkColorFilter> cf = SkColorFilters::HSLAMatrix(SkColorMatrix::RGBtoYUV(SkYUVColorSpace::kJPEG_Full_SkYUVColorSpace));
    sk_sp<PrecompileColorFilter> o = PrecompileColorFilters::HSLAMatrix();

    return { cf, o };
}

std::pair<sk_sp<SkColorFilter>, sk_sp<PrecompileColorFilter>> create_colorfilter(Fuzz* fuzz, ColorFilterType type, int depth)
{
    if (depth <= 0) {
        return {};
    }
...
std::pair<sk_sp<SkColorFilter>, sk_sp<PrecompileColorFilter>> create_random_colorfilter(Fuzz* fuzz, int depth)
{
...
//--------------------------------------------------------------------------------------------------
void check_draw(Context* context, Recorder* recorder, const SkPaint& paint, DrawTypeFlags dt, const SkPath& path)
{
...
void fuzz_graphite(Fuzz* fuzz, Context* context, int depth = 9)
{
    ...
    SkColorInfo ci = SkColorInfo(kRGBA_8888_SkColorType, kPremul_SkAlphaType, SkColorSpace::MakeSRGB());
    ...
    Layout layout = context->backend() == skgpu::BackendApi::kMetal ? Layout::kMetal : Layout::kStd140;
    ...
    constexpr Coverage coverageOptions[3] = { Coverage::kNone, Coverage::kSingleChannel, Coverage::kLCD };
    uint32_t temp;
    fuzz->next(&temp);
    ...
    if (blender) {
        dstReadReq = GetDstReadRequirement(recorder->priv().caps(), blender->asBlendMode(), coverage);
    }

    UniquePaintParamsID paintID = ExtractPaintData(recorder.get(), &gatherer, &builder, layout, {},
        PaintParams(paint,
            /* primitiveBlender= */ nullptr,
            /* analyticClip= */ {},
            /* clipShader= */ nullptr, dstReadReq,
    ...
    paintOptions.priv().buildCombinations(precompileKeyContext, &gatherer, DrawTypeFlags::kNone,
        /* withPrimitiveBlender= */ false, coverage, unusedRenderPassDesc,
        [&](UniquePaintParamsID id, DrawTypeFlags, bool /* withPrimitiveBlender */, Coverage, const RenderPassDesc&) { precompileIDs.push_back(id); });
