...
static SkImageInfo gen_fuzzed_imageinfo(Fuzz* fuzz, SkColorType surfaceType)
{
    ...
    return SkImageInfo::Make(width, height, surfaceType, alphaType, SkColorSpace::MakeRGB(skcmsFn, skcmsMat));
}
...
static GrSurfaceCharacterization make_characterization(Fuzz* fuzz, GrDirectContext* dContext, SkImageInfo& ii, SkColorType surfaceType, GrSurfaceOrigin origin)
{
    if (!dContext->colorTypeSupportedAsSurface(surfaceType)) {
        SkDebugf("Couldn't create backend texture in the backend %s", GrBackendApiToStr(dContext->backend()));
        return {};
    }

    GrBackendFormat backendFormat = dContext->defaultBackendFormat(surfaceType, GrRenderable::kYes);
    if (!backendFormat.isValid()) {
        SkDebugf("Color Type is not supported in the backend %s", GrBackendApiToStr(dContext->backend()));
        return {};
    }
    ...
    c = dContext->threadSafeProxy()->createCharacterization(
        maxResourceBytes, ii, backendFormat, kSampleCount, origin, gen_fuzzed_surface_props(fuzz), skgpu::Mipmapped::kYes, false, true, protect);
    if (!c.isValid()) {
        SkDebugf("Could not create Characterization in the backend %s", GrBackendApiToStr(dContext->backend()));
        return {};
    }
    return c;
}
...
static sk_sp<SkSurface> make_surface(Fuzz* fuzz, GrDirectContext* dContext, const SkImageInfo& ii, GrSurfaceOrigin origin)
{
    ...
    auto surface = SkSurfaces::RenderTarget(dContext, budgeted, ii, kSampleCount, origin, &surfaceProps);
    return surface;
}
