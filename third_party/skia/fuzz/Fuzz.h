...
public:
    ...
    template <typename Arg, typename... Args> void next(Arg* first, Args... rest);

    // nextRange returns values only in [min, max].
    template <typename T, typename Min, typename Max> void nextRange(T*, Min, Max);

    // nextEnum is a wrapper around nextRange for enums.
    template <typename T> void nextEnum(T* ptr, T max);

    // nextN loads n * sizeof(T) bytes into ptr
    template <typename T> void nextN(T* ptr, int n);
...
template <typename Arg, typename... Args> inline void Fuzz::next(Arg* first, Args... rest)
{
    this->next(first);
    this->next(rest...);
}

template <typename T, typename Min, typename Max> inline void Fuzz::nextRange(T* value, Min min, Max max)
{
...
template <typename T> inline void Fuzz::nextEnum(T* value, T max)
{
...
template <typename T> inline void Fuzz::nextN(T* ptr, int n)
{
    for (int i = 0; i < n; i++) {
        this->next(ptr + i);
    }
