...
struct SK_API SkSamplingOptions {
    ...
    {
        return maxAniso == other.maxAniso && useCubic == other.useCubic && cubic.B == other.cubic.B && cubic.C == other.cubic.C && filter == other.filter
            && mipmap == other.mipmap;
    }
    bool operator!=(const SkSamplingOptions& other) const
    {
...
private:
    constexpr SkSamplingOptions(int maxAniso)
        : maxAniso(maxAniso)
    {
    }
};
