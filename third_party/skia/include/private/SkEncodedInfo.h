...
public:
    ...
    {
        return Make(width, height, color, alpha, /*bitsPerComponent*/ bitsPerComponent, std::move(profile), /*colorDepth*/ bitsPerComponent);
    }
    ...
    static SkEncodedInfo Make(int width, int height, Color color, Alpha alpha, int bitsPerComponent, std::unique_ptr<ICCProfile> profile, int colorDepth)
    {
        ...
        return SkEncodedInfo(width, height, color, alpha, SkToU8(bitsPerComponent), SkToU8(colorDepth), std::move(profile));
    }
    ...
    {
        auto copy = SkEncodedInfo::Make(fWidth, fHeight, fColor, fAlpha, fBitsPerComponent, nullptr, fColorDepth);
        if (fProfile) {
...
private:
    SkEncodedInfo(int width, int height, Color color, Alpha alpha, uint8_t bitsPerComponent, uint8_t colorDepth, std::unique_ptr<ICCProfile> profile)
        : fWidth(width)
        , fHeight(height)
        , fColor(color)
        , fAlpha(alpha)
