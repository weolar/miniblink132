...
//    }
#if defined(__clang__)
#define SkASSERT_RELEASE(cond) static_cast<void>(__builtin_expect(static_cast<bool>(cond), 1) ? static_cast<void>(0) : [] { SK_ABORT("check(%s)", #cond); }())

#define SkASSERTF_RELEASE(cond, fmt, ...)                                                                                                                      \
    static_cast<void>(__builtin_expect(static_cast<bool>(cond), 1) ? static_cast<void>(0) : [&] { SK_ABORT("assertf(%s): " fmt, #cond, ##__VA_ARGS__); }())
#else
#define SkASSERT_RELEASE(cond) static_cast<void>((cond) ? static_cast<void>(0) : [] { SK_ABORT("check(%s)", #cond); }())

#define SkASSERTF_RELEASE(cond, fmt, ...) static_cast<void>((cond) ? static_cast<void>(0) : [&] { SK_ABORT("assertf(%s): " fmt, #cond, ##__VA_ARGS__); }())
#endif
...
[[noreturn]] static inline void sk_fast_fail()
{
