...
public:
    ...
    explicit TArray(int reserveCount)
        : TArray()
    {
        this->reserve_exact(reserveCount);
    }
    ...
    TArray(SkSpan<const T> data)
        : TArray(data.begin(), static_cast<int>(data.size()))
    {
    }
...
protected:
    ...
    template <int InitialCapacity> TArray(SkAlignedSTStorage<InitialCapacity, T>* storage, int size = 0)
    {
...
private:
    ...
    void move(int dst, int src)
    {
        if constexpr (MEM_MOVE) {
            memcpy(static_cast<void*>(&fData[dst]), static_cast<const void*>(&fData[src]), sizeof(T));
        } else {
    ...
    template <typename... Args> SK_ALWAYS_INLINE T* growAndConstructAtEnd(Args&&... args)
    {
...
class STArray : private SkAlignedSTStorage<SkContainerAllocator::RoundUp<T>(Nreq), T>, public TArray<T, MEM_MOVE> {
