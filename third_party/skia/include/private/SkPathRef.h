...
public:
    ...
    public:
         ...
         */
        SkPoint* growForRepeatedVerb(int /*SkPath::Verb*/ verb, int numVbs, SkScalar** weights = nullptr)
        {
...
private:
    ...
    void resetToSize(int verbCount, int pointCount, int conicCount, int reserveVerbs = 0, int reservePoints = 0, int reserveConics = 0)
    {
        this->commonReset();
    ...
    {
        SkDEBUGCODE(this->validate();) fType = PathType::kGeneral;
        return fPoints.begin();
    }

    const SkPoint* getPoints() const
    {
        SkDEBUGCODE(this->validate();) return fPoints.begin();
    }
