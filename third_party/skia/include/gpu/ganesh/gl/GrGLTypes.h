...
typedef void(GR_GL_FUNCTION_TYPE* GRGLDEBUGPROC)(
    GrGLenum source, GrGLenum type, GrGLuint id, GrGLenum severity, GrGLsizei length, const GrGLchar* message, const void* userParam);
...
struct GrGLTextureInfo {
    ...
    {
        return fTarget == that.fTarget && fID == that.fID && fFormat == that.fFormat && fProtected == that.fProtected;
    }

    bool isProtected() const
    {
        return fProtected == skgpu::Protected::kYes;
    }
};
...
struct GrGLFramebufferInfo {
    ...
    {
        return fFBOID == that.fFBOID && fFormat == that.fFormat && fProtected == that.fProtected;
    }
