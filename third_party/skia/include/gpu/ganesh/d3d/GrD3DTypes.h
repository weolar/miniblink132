...
public:
    ...
    gr_cp(const gr_cp<T>& that)
        : fObject(GrSafeComAddRef(that.get()))
    {
    }
...
template <typename T> inline bool operator==(const gr_cp<T>& a, const gr_cp<T>& b)
{
    return a.get() == b.get();
}
...
public:
    virtual gr_cp<ID3D12Resource> createResource(
        D3D12_HEAP_TYPE, const D3D12_RESOURCE_DESC*, D3D12_RESOURCE_STATES initialResourceState, sk_sp<GrD3DAlloc>* allocation, const D3D12_CLEAR_VALUE*)
        = 0;
    virtual gr_cp<ID3D12Resource> createAliasingResource(
        sk_sp<GrD3DAlloc>& allocation, uint64_t localOffset, const D3D12_RESOURCE_DESC*, D3D12_RESOURCE_STATES initialResourceState, const D3D12_CLEAR_VALUE*)
        = 0;
};
...
struct GrD3DTextureResourceInfo {
    ...
    GrD3DTextureResourceInfo(ID3D12Resource* resource, const sk_sp<GrD3DAlloc> alloc, D3D12_RESOURCE_STATES resourceState, DXGI_FORMAT format,
        uint32_t sampleCount, uint32_t levelCount, unsigned int sampleQualityLevel, skgpu::Protected isProtected = skgpu::Protected::kNo)
        : fResource(resource)
        , fAlloc(alloc)
    ...
    {
        return fResource == that.fResource && fResourceState == that.fResourceState && fFormat == that.fFormat && fSampleCount == that.fSampleCount
            && fLevelCount == that.fLevelCount && fSampleQualityPattern == that.fSampleQualityPattern && fProtected == that.fProtected;
    }
#endif
};
