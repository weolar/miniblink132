...
public:
    ...
    /**
      ...
      @param cropBox The size of the recommended view port in physical units.
                     ...
                     The default is to coincide with the mediaBox.
     */
    bool beginSheet(const SkVector& unitsPerMeter, const SkVector& pixelsPerMeter, const SkSize& trimSize, const SkRect* mediaBox = nullptr,
        const SkRect* bleedBox = nullptr, const SkRect* artBox = nullptr, const SkRect* cropBox = nullptr);
    ...
    void drawImageRect(
        const SkImage*, const SkRect* srcOrNull, const SkRect& dst, const SkSamplingOptions&, const SkPaint& paint, SkCanvas::SrcRectConstraint) override;
...
private:
    class TypefaceUse {
    public:
        TypefaceUse(SkTypefaceID id, int index, std::unique_ptr<SkStream> data, SkTScopedComPtr<IXpsOMFontResource> xps, size_t numGlyphs)
            : typefaceId(id)
            , ttcIndex(index)
    ...
    HRESULT createXpsBrush(const SkPaint& skPaint, IXpsOMBrush** xpsBrush, const SkMatrix* parentTransform = nullptr);
    ...
    HRESULT createXpsLinearGradient(
        SkShaderBase::GradientInfo info, const SkAlpha alpha, const SkMatrix& localMatrix, IXpsOMMatrixTransform* xpsMatrixToUse, IXpsOMBrush** xpsBrush);
    ...
    HRESULT AddGlyphs(IXpsOMObjectFactory* xpsFactory, IXpsOMCanvas* canvas, const TypefaceUse* font, LPCWSTR text, XPS_GLYPH_INDEX* xpsGlyphs,
