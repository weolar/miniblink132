...
template <SkAlphaType kDstAT = kPremul_SkAlphaType> static SkRGBA4f<kDstAT> map_color(const SkColor4f& c, SkColorSpace* src, SkColorSpace* dst)
{
...
sk_sp<SkColorFilter> SkColorFilters::Blend(const SkColor4f& color, sk_sp<SkColorSpace> colorSpace, SkBlendMode mode)
{
    ...
    if (SkBlendMode::kDst == mode
        || (0.f == alpha
            && (SkBlendMode::kSrcOver == mode || SkBlendMode::kDstOver == mode || SkBlendMode::kDstOut == mode || SkBlendMode::kSrcATop == mode
