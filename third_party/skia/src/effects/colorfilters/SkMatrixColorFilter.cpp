...
static bool is_alpha_unchanged(const float matrix[20])
{
    ...
    return SkScalarNearlyZero(srcA[0]) && SkScalarNearlyZero(srcA[1]) && SkScalarNearlyZero(srcA[2]) && SkScalarNearlyEqual(srcA[3], 1)
...
sk_sp<SkFlattenable> SkMatrixColorFilter::CreateProc(SkReadBuffer& buffer)
{
    ...
    Clamp clamp = buffer.isVersionLT(SkPicturePriv::kUnclampedMatrixColorFilter) ? Clamp::kYes : (buffer.readBool() ? Clamp::kYes : Clamp::kNo);
...
{
    const bool willStayOpaque = shaderIsOpaque && fAlphaIsUnchanged, hsla = fDomain == Domain::kHSLA, clamp = fClamp == Clamp::kYes;
...
static sk_sp<SkColorFilter> MakeMatrix(const float array[20], SkMatrixColorFilter::Domain domain, SkColorFilters::Clamp clamp)
{
