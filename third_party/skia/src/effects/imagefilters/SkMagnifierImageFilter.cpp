...
public:
    SkMagnifierImageFilter(const SkRect& lensBounds, float zoomAmount, float inset, const SkSamplingOptions& sampling, sk_sp<SkImageFilter> input)
...
sk_sp<SkImageFilter> SkImageFilters::Magnifier(
    const SkRect& lensBounds, SkScalar zoomAmount, SkScalar inset, const SkSamplingOptions& sampling, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    if (lensBounds.isEmpty() || !lensBounds.isFinite() || zoomAmount <= 0.f || inset < 0.f || !SkIsFinite(zoomAmount, inset)) {
    ...
    if (zoomAmount > 1.f) {
        return sk_sp<SkImageFilter>(new SkMagnifierImageFilter(lensBounds, zoomAmount, inset, sampling, std::move(input)));
    } else {
...
static sk_sp<SkShader> make_magnifier_shader(
    sk_sp<SkShader> input, const skif::LayerSpace<SkRect>& lensBounds, const skif::LayerSpace<SkMatrix>& zoomXform, const skif::LayerSpace<SkSize>& inset)
{
    const SkRuntimeEffect* magEffect = GetKnownRuntimeEffect(SkKnownRuntimeEffects::StableKey::kMagnifier);
    ...
    builder.uniform("invInset") = SkV2 { 1.f / inset.width(), 1.f / inset.height() };
...
skif::FilterResult SkMagnifierImageFilter::onFilterImage(const skif::Context& context) const
{
    ...
    if (std::optional<skif::LayerSpace<SkIRect>> output = this->getChildOutputLayerBounds(0, context.mapping(), context.source().layerBounds())) {
    ...
    skif::LayerSpace<SkRect> srcRect { { lensBounds.left() * invZoom + zoomCenter.x() * (1.f - invZoom),
        lensBounds.top() * invZoom + zoomCenter.y() * (1.f - invZoom), lensBounds.right() * invZoom + zoomCenter.x() * (1.f - invZoom),
    ...
    if (!expectedChildOutput.contains(visibleLensBounds)) {
        ...
        if (expectedChildOutput.width() >= srcRect.width() && expectedChildOutput.height() >= srcRect.height()) {
            float left = srcRect.left() < expectedChildOutput.left() ? expectedChildOutput.left()
                                                                     : std::min(srcRect.right(), expectedChildOutput.right()) - srcRect.width();
            float top = srcRect.top() < expectedChildOutput.top() ? expectedChildOutput.top()
                                                                  : std::min(srcRect.bottom(), expectedChildOutput.bottom()) - srcRect.height();

            // Update transform to reflect fitted src
            srcRect = skif::LayerSpace<SkRect>(SkRect::MakeXYWH(left, top, srcRect.width(), srcRect.height()));
    ...
    skif::LayerSpace<SkSize> inset = context.mapping().paramToLayer(skif::ParameterSpace<SkSize>({ fInset, fInset }));
    if (inset.width() <= 0.f || inset.height() <= 0.f) {
        ...
        skif::FilterResult childOutput = this->getChildOutput(0, context.withNewDesiredOutput(srcRect.roundOut()));
        return childOutput.applyTransform(context, invZoomXform, fSampling).applyCrop(context, lensBounds.roundOut());
    }

    using ShaderFlags = skif::FilterResult::ShaderFlags;
    skif::FilterResult::Builder builder { context };
    builder.add(this->getChildOutput(0, context.withNewDesiredOutput(visibleLensBounds.roundOut())), {}, ShaderFlags::kNonTrivialSampling, fSampling);
    return builder.eval(
        [&](SkSpan<sk_sp<SkShader>> inputs) {
            // If the input resolved to a null shader, the magnified output will be transparent too
            return inputs[0] ? make_magnifier_shader(inputs[0], lensBounds, zoomXform, inset) : nullptr;
        },
        lensBounds.roundOut());
}
