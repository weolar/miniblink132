...
namespace skif {
template <> class LayerSpace<ZValue> {
public:
    LayerSpace() = default;
    explicit LayerSpace(ZValue z)
        : fData(z)
    {
    }
    ...
    static LayerSpace<ZValue> Map(const Mapping& mapping, ParameterSpace<ZValue> z)
    {
        ...
        skif::LayerSpace<skif::Vector> z2d = mapping.paramToLayer(skif::ParameterSpace<skif::Vector>({ ZValue(z), ZValue(z) }));
...
struct Light {
    enum class Type { kDistant, kPoint, kSpot, kLast = kSpot };
    ...
    {
        return { Type::kPoint, color, skif::ParameterSpace<SkPoint>({ location.fX, location.fY }), skif::ParameterSpace<ZValue>(location.fZ),
            /*directionXY=*/ {},
            /*directionZ=*/ {},
            /*falloffExponent=*/0.f,
            /*cutoffAngle=*/0.f };
    ...
    {
        return { Type::kDistant, color,
            /*locationXY=*/ {},
            /*locationZ=*/ {}, skif::ParameterSpace<skif::Vector>({ direction.fX, direction.fY }), skif::ParameterSpace<ZValue>(direction.fZ),
            /*falloffExponent=*/0.f,
            /*cutoffAngle=*/0.f };
    }

    static Light Spot(SkColor color, const SkPoint3& location, const SkPoint3& direction, float falloffExponent, float cosCutoffAngle)
    {
        return { Type::kSpot, color, skif::ParameterSpace<SkPoint>({ location.fX, location.fY }), skif::ParameterSpace<ZValue>(location.fZ),
            skif::ParameterSpace<skif::Vector>({ direction.fX, direction.fY }), skif::ParameterSpace<ZValue>(direction.fZ), falloffExponent, cosCutoffAngle };
    }
};

struct Material {
    enum class Type { kDiffuse, kSpecular, kLast = kSpecular };
...
private:
    ...
    skif::LayerSpace<SkIRect> onGetInputLayerBounds(
        const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const override;

    std::optional<skif::LayerSpace<SkIRect>> onGetOutputLayerBounds(
        const skif::Mapping& mapping, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const override;
...
sk_sp<SkShader> make_normal_shader(sk_sp<SkShader> alphaMap, const skif::LayerSpace<SkIRect>& edgeBounds, skif::LayerSpace<ZValue> surfaceDepth)
{
...
sk_sp<SkShader> make_lighting_shader(sk_sp<SkShader> normalMap, Light::Type lightType, SkColor lightColor, skif::LayerSpace<SkPoint> locationXY,
    skif::LayerSpace<ZValue> locationZ, skif::LayerSpace<skif::Vector> directionXY, skif::LayerSpace<ZValue> directionZ, float falloffExponent,
    float cosCutoffAngle, Material::Type matType, skif::LayerSpace<ZValue> surfaceDepth, float k, float shininess)
{
    ...
    builder.uniform("materialAndLightType") = SkV4 { surfaceDepth.val(), shininess, matType == Material::Type::kDiffuse ? 0.f : 1.f,
        lightType == Light::Type::kPoint ? 0.f : (lightType == Light::Type::kDistant ? -1.f : 1.f) };
    builder.uniform("lightPosAndSpotFalloff") = SkV4 { locationXY.x(), locationXY.y(), locationZ.val(), falloffExponent };
    ...
    builder.uniform("lightDirAndSpotCutoff") = SkV4 { invDirLen * dir.x, invDirLen * dir.y, invDirLen * dir.z, cosCutoffAngle };
    ...
    builder.uniform("lightColor") = SkV3 { SkColorGetR(lightColor) * colorScale, SkColorGetG(lightColor) * colorScale, SkColorGetB(lightColor) * colorScale };

    return builder.makeShader();
}

sk_sp<SkImageFilter> make_lighting(const Light& light, const Material& material, sk_sp<SkImageFilter> input, const SkImageFilters::CropRect& cropRect)
{
    ...
    if (!SkIsFinite(material.fK, material.fShininess, ZValue(material.fSurfaceDepth)) || material.fK < 0.f) {
        return nullptr;
    }
    ...
    if (!SkPoint(light.fLocationXY).isFinite() || !skif::Vector(light.fDirectionXY).isFinite()
        || !SkIsFinite(light.fFalloffExponent, light.fCosCutoffAngle, ZValue(light.fLocationZ), ZValue(light.fDirectionZ)) || light.fCosCutoffAngle < -1.f
        || light.fCosCutoffAngle > 1.f) {
        return nullptr;
    }
    ...
    filter = sk_sp<SkImageFilter>(new SkLightingImageFilter(light, material, std::move(filter)));
    if (cropRect) {
        filter = SkImageFilters::Crop(*cropRect, std::move(filter));
    }
    return filter;
...
sk_sp<SkImageFilter> SkImageFilters::DistantLitDiffuse(
    const SkPoint3& direction, SkColor lightColor, SkScalar surfaceScale, SkScalar kd, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    return make_lighting(Light::Distant(lightColor, direction), Material::Diffuse(kd, surfaceScale), std::move(input), cropRect);
}

sk_sp<SkImageFilter> SkImageFilters::PointLitDiffuse(
    const SkPoint3& location, SkColor lightColor, SkScalar surfaceScale, SkScalar kd, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    return make_lighting(Light::Point(lightColor, location), Material::Diffuse(kd, surfaceScale), std::move(input), cropRect);
}

sk_sp<SkImageFilter> SkImageFilters::SpotLitDiffuse(const SkPoint3& location, const SkPoint3& target, SkScalar falloffExponent, SkScalar cutoffAngle,
    SkColor lightColor, SkScalar surfaceScale, SkScalar kd, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    SkPoint3 dir = target - location;
    float cosCutoffAngle = SkScalarCos(SkDegreesToRadians(cutoffAngle));
    return make_lighting(
        Light::Spot(lightColor, location, dir, falloffExponent, cosCutoffAngle), Material::Diffuse(kd, surfaceScale), std::move(input), cropRect);
}

sk_sp<SkImageFilter> SkImageFilters::DistantLitSpecular(
    const SkPoint3& direction, SkColor lightColor, SkScalar surfaceScale, SkScalar ks, SkScalar shininess, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    return make_lighting(Light::Distant(lightColor, direction), Material::Specular(ks, shininess, surfaceScale), std::move(input), cropRect);
}

sk_sp<SkImageFilter> SkImageFilters::PointLitSpecular(
    const SkPoint3& location, SkColor lightColor, SkScalar surfaceScale, SkScalar ks, SkScalar shininess, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    return make_lighting(Light::Point(lightColor, location), Material::Specular(ks, shininess, surfaceScale), std::move(input), cropRect);
}

sk_sp<SkImageFilter> SkImageFilters::SpotLitSpecular(const SkPoint3& location, const SkPoint3& target, SkScalar falloffExponent, SkScalar cutoffAngle,
    SkColor lightColor, SkScalar surfaceScale, SkScalar ks, SkScalar shininess, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    SkPoint3 dir = target - location;
    float cosCutoffAngle = SkScalarCos(SkDegreesToRadians(cutoffAngle));
    return make_lighting(
        Light::Spot(lightColor, location, dir, falloffExponent, cosCutoffAngle), Material::Specular(ks, shininess, surfaceScale), std::move(input), cropRect);
}
...
void SkRegisterLightingImageFilterFlattenables()
{
    ...
    SkFlattenable::Register("SkDiffuseLightingImageFilter", SkLightingImageFilter::LegacyDiffuseCreateProc);
    SkFlattenable::Register("SkSpecularLightingImageFilter", SkLightingImageFilter::LegacySpecularCreateProc);
}
...
skif::FilterResult SkLightingImageFilter::onFilterImage(const skif::Context& ctx) const
{
    ...
    auto mapZToLayer = [&ctx](skif::ParameterSpace<ZValue> z) { return skif::LayerSpace<ZValue>::Map(ctx.mapping(), z); };
    ...
    skif::FilterResult childOutput = this->getChildOutput(0, ctx.withNewDesiredOutput(requiredInput));
    ...
    if (!childOutput.layerBounds().contains(requiredInput)) {
        ...
        auto edgeClamp
            = [](int actualEdgeValue, int requestedEdgeValue, int outputEdge) { return actualEdgeValue == outputEdge ? outputEdge : requestedEdgeValue; };
        auto inputRect = childOutput.layerBounds();
        auto clampTo = ctx.desiredOutput();
        clampRect = skif::LayerSpace<SkIRect>({ edgeClamp(inputRect.left(), requiredInput.left(), clampTo.left()),
            edgeClamp(inputRect.top(), requiredInput.top(), clampTo.top()), edgeClamp(inputRect.right(), requiredInput.right(), clampTo.right()),
    ...
    return builder.eval([&](SkSpan<sk_sp<SkShader>> input) {
        ...
        return make_lighting_shader(std::move(normals),
            ...
            fLight.fType, fLight.fLightColor, lightLocationXY, lightLocationZ, lightDirXY, lightDirZ, fLight.fFalloffExponent, fLight.fCosCutoffAngle,
            // Material in layer space
            fMaterial.fType, surfaceDepth, fMaterial.fK, fMaterial.fShininess);
    });
}

skif::LayerSpace<SkIRect> SkLightingImageFilter::onGetInputLayerBounds(
    const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const
{
...
std::optional<skif::LayerSpace<SkIRect>> SkLightingImageFilter::onGetOutputLayerBounds(
    const skif::Mapping& mapping, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const
{
