...
public:
    SkBlendImageFilter(sk_sp<SkBlender> blender, const std::optional<SkV4>& coefficients, bool enforcePremul, sk_sp<SkImageFilter> inputs[2])
...
private:
    ...
    bool onAffectsTransparentBlack() const override
    {
        ...
        // transparent black.
        return !as_BB(fBlender)->asBlendMode().has_value() && (!fArithmeticCoefficients.has_value() || (*fArithmeticCoefficients)[3] != 0.f);
    }
...
sk_sp<SkImageFilter> make_blend(sk_sp<SkBlender> blender, sk_sp<SkImageFilter> background, sk_sp<SkImageFilter> foreground,
    const SkImageFilters::CropRect& cropRect, std::optional<SkV4> coefficients = {}, bool enforcePremul = false)
{
    if (!blender) {
    ...
    sk_sp<SkImageFilter> filter { new SkBlendImageFilter(blender, coefficients, enforcePremul, inputs) };
    return cropped(std::move(filter));
}

} // anonymous namespace

sk_sp<SkImageFilter> SkImageFilters::Blend(SkBlendMode mode, sk_sp<SkImageFilter> background, sk_sp<SkImageFilter> foreground, const CropRect& cropRect)
{
    return make_blend(SkBlender::Mode(mode), std::move(background), std::move(foreground), cropRect);
}

sk_sp<SkImageFilter> SkImageFilters::Blend(sk_sp<SkBlender> blender, sk_sp<SkImageFilter> background, sk_sp<SkImageFilter> foreground, const CropRect& cropRect)
{
    return make_blend(std::move(blender), std::move(background), std::move(foreground), cropRect);
}

sk_sp<SkImageFilter> SkImageFilters::Arithmetic(SkScalar k1, SkScalar k2, SkScalar k3, SkScalar k4, bool enforcePMColor, sk_sp<SkImageFilter> background,
    sk_sp<SkImageFilter> foreground, const CropRect& cropRect)
{
    ...
    return make_blend(std::move(blender), std::move(background), std::move(foreground), cropRect,
        ...
        SkV4 { k1, k2, k3, k4 }, enforcePMColor);
}
...
void SkRegisterBlendImageFilterFlattenable()
{
    ...
    SkFlattenable::Register("ArithmeticImageFilterImpl", SkBlendImageFilter::LegacyArithmeticCreateProc);
    SkFlattenable::Register("SkArithmeticImageFilter", SkBlendImageFilter::LegacyArithmeticCreateProc);
}
...
sk_sp<SkFlattenable> SkBlendImageFilter::LegacyArithmeticCreateProc(SkReadBuffer& buffer)
{
    ...
    return SkImageFilters::Arithmetic(k[0], k[1], k[2], k[3], enforcePremul, common.getInput(0), common.getInput(1), common.cropRect());
}
...
sk_sp<SkFlattenable> SkBlendImageFilter::CreateProc(SkReadBuffer& buffer)
{
    ...
    return make_blend(std::move(blender), common.getInput(kBackground), common.getInput(kForeground), common.cropRect(), coefficients, enforcePremul);
}
    ...
    if (!bg || !fg) {
        ...
        if (auto bm = as_BB(fBlender)->asBlendMode()) {
            ...
            if (SkBlendMode_AsCoeff(*bm, &src, &dst)) {
                if (bg && (dst == SkBlendModeCoeff::kOne || dst == SkBlendModeCoeff::kISA || dst == SkBlendModeCoeff::kISC)) {
                    return bg;
                }
                if (fg && (src == SkBlendModeCoeff::kOne || src == SkBlendModeCoeff::kIDA)) {
                    return fg;
                }
            }
        }
        ...
        if (!bg) {
            bg = SkShaders::Color(SK_ColorTRANSPARENT);
        }
        if (!fg) {
            fg = SkShaders::Color(SK_ColorTRANSPARENT);
        }
    }
...
skif::FilterResult SkBlendImageFilter::onFilterImage(const skif::Context& ctx) const
{
    ...
    return builder.eval(
        [&](SkSpan<sk_sp<SkShader>> inputs) -> sk_sp<SkShader> { return this->makeBlendShader(inputs[kBackground], inputs[kForeground]); }, requiredInput);
}
    ...
    const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const
{
    ...
    skif::LayerSpace<SkIRect> bgInput = this->getChildInputLayerBounds(kBackground, mapping, requiredInput, contentBounds);
    skif::LayerSpace<SkIRect> fgInput = this->getChildInputLayerBounds(kForeground, mapping, requiredInput, contentBounds);
    ...
    const skif::Mapping& mapping, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const
{
    ...
    if (auto bm = as_BB(fBlender)->asBlendMode()) {
        ...
        if (SkBlendMode_AsCoeff(*bm, &src, &dst)) {
            ...
            transparentOutsideFG = dst == SkBlendModeCoeff::kZero || dst == SkBlendModeCoeff::kSA || dst == SkBlendModeCoeff::kSC;
...
SkRect SkBlendImageFilter::computeFastBounds(const SkRect& bounds) const
{
    ...
    SkRect foregroundBounds = this->getInput(kForeground) ? this->getInput(kForeground)->computeFastBounds(bounds) : bounds;
    SkRect backgroundBounds = this->getInput(kBackground) ? this->getInput(kBackground)->computeFastBounds(bounds) : bounds;
    if (transparentOutsideFG) {
        if (transparentOutsideBG) {
            // Output is the intersection of both
