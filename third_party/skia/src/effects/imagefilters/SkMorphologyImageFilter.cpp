...
private:
    ...
    skif::LayerSpace<SkISize> radii(const skif::Mapping& mapping) const
    {
        ...
        return skif::LayerSpace<SkISize>({ std::min(radii.width(), kMaxRadii), std::min(radii.height(), kMaxRadii) });
    }

    skif::LayerSpace<SkIRect> requiredInput(const skif::Mapping& mapping, skif::LayerSpace<SkIRect> bounds) const
    {
    ...
    skif::LayerSpace<SkIRect> kernelOutputBounds(const skif::Mapping& mapping, skif::LayerSpace<SkIRect> bounds) const
    {
...
sk_sp<SkImageFilter> make_morphology(MorphType type, SkSize radii, sk_sp<SkImageFilter> input, const SkImageFilters::CropRect& cropRect)
{
...
sk_sp<SkShader> make_linear_morphology(sk_sp<SkShader> input, MorphType type, MorphDirection direction, int radius)
{
    SkASSERT(radius <= kMaxLinearRadius);

    const SkRuntimeEffect* linearMorphologyEffect = GetKnownRuntimeEffect(SkKnownRuntimeEffects::StableKey::kLinearMorphology);
...
sk_sp<SkShader> make_sparse_morphology(sk_sp<SkShader> input, MorphType type, MorphDirection direction, int radius)
{

    const SkRuntimeEffect* sparseMorphologyEffect = GetKnownRuntimeEffect(SkKnownRuntimeEffects::StableKey::kSparseMorphology);

    SkRuntimeShaderBuilder builder(sk_ref_sp(sparseMorphologyEffect));
    builder.child("child") = std::move(input);
    builder.uniform("offset") = direction == MorphDirection::kX ? SkV2 { (float)radius, 0.f } : SkV2 { 0.f, (float)radius };
    builder.uniform("flip") = (type == MorphType::kDilate) ? 1.f : -1.f;

    return builder.makeShader();
}

skif::FilterResult morphology_pass(const skif::Context& ctx, const skif::FilterResult& input, MorphType type, MorphDirection dir, int radius)
{
    using ShaderFlags = skif::FilterResult::ShaderFlags;

    auto axisDelta = [dir](int step) { return skif::LayerSpace<SkISize>({ dir == MorphDirection::kX ? step : 0, dir == MorphDirection::kY ? step : 0 }); };
    ...
    while (radius > appliedRadius) {
        ...
        int stepRadius = appliedRadius == 0 ? std::min(kMaxLinearRadius, radius) : std::min(radius - appliedRadius, appliedRadius);
        ...
        childOutput = builder.eval([&](SkSpan<sk_sp<SkShader>> inputs) {
            if (appliedRadius == 0) {
...
sk_sp<SkImageFilter> SkImageFilters::Dilate(SkScalar radiusX, SkScalar radiusY, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    return make_morphology(MorphType::kDilate, { radiusX, radiusY }, std::move(input), cropRect);
}

sk_sp<SkImageFilter> SkImageFilters::Erode(SkScalar radiusX, SkScalar radiusY, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
...
{
    skif::LayerSpace<SkIRect> requiredInput = this->requiredInput(ctx.mapping(), ctx.desiredOutput());
    ...
    skif::LayerSpace<SkIRect> maxOutput = this->kernelOutputBounds(ctx.mapping(), childOutput.layerBounds());
    ...
    childOutput = morphology_pass(ctx.withNewDesiredOutput(maxOutputX), childOutput, fType, MorphDirection::kX, radii.width());
    childOutput = morphology_pass(ctx.withNewDesiredOutput(maxOutput), childOutput, fType, MorphDirection::kY, radii.height());
    return childOutput;
}
