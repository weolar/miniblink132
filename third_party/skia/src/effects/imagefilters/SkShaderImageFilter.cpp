...
sk_sp<SkImageFilter> SkImageFilters::Shader(sk_sp<SkShader> shader, Dither dither, const CropRect& cropRect)
{
    if (!shader) {
...
sk_sp<SkFlattenable> SkShaderImageFilter::CreateProc(SkReadBuffer& buffer)
{
    ...
    if (buffer.isVersionLT(SkPicturePriv::kShaderImageFilterSerializeShader)) {
        ...
        shader = paint.getShader() ? paint.refShader() : SkShaders::Color(paint.getColor4f(), nullptr);
        dither = paint.isDither();
    } else {
        shader = buffer.readShader();
        dither = buffer.readBool();
    }
    return SkImageFilters::Shader(std::move(shader), SkImageFilters::Dither(dither), common.cropRect());
}
...
skif::LayerSpace<SkIRect> SkShaderImageFilter::onGetInputLayerBounds(
    const skif::Mapping&, const skif::LayerSpace<SkIRect>&, std::optional<skif::LayerSpace<SkIRect>>) const
{
    // This is a leaf filter, it requires no input and no further recursion
    return skif::LayerSpace<SkIRect>::Empty();
}

std::optional<skif::LayerSpace<SkIRect>> SkShaderImageFilter::onGetOutputLayerBounds(const skif::Mapping&, std::optional<skif::LayerSpace<SkIRect>>) const
{
