...
SkBitmap create_kernel_bitmap(const SkISize& kernelSize, const float* kernel, float* innerGain, float* innerBias);
...
skif::LayerSpace<SkIRect> adjust(const skif::LayerSpace<SkIRect>& rect, int dl, int dt, int dr, int db)
{
...
SkBitmap create_kernel_bitmap(const SkISize& kernelSize, const float* kernel, float* innerGain, float* innerBias)
{
    ...
    *innerBias = min;
    ...
    SkBitmap kernelBM;
    if (!kernelBM.tryAllocPixels(SkImageInfo::Make({ quantizedKernelSize, 1 }, kAlpha_8_SkColorType, kPremul_SkAlphaType))) {
...
sk_sp<SkImageFilter> SkImageFilters::MatrixConvolution(const SkISize& kernelSize, const SkScalar kernel[], SkScalar gain, SkScalar bias,
    const SkIPoint& kernelOffset, SkTileMode tileMode, bool convolveAlpha, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    ...
    filter = sk_sp<SkImageFilter>(new SkMatrixConvolutionImageFilter(kernelSize, kernel, gain, bias, kernelOffset, convolveAlpha, &filter));
    if (cropRect) {
...
void SkRegisterMatrixConvolutionImageFilterFlattenable()
{
    ...
    SkFlattenable::Register("SkMatrixConvolutionImageFilterImpl", SkMatrixConvolutionImageFilter::CreateProc);
}
...
sk_sp<SkFlattenable> SkMatrixConvolutionImageFilter::CreateProc(SkReadBuffer& buffer)
{
    ...
    return SkImageFilters::MatrixConvolution(
        kernelSize, kernel.get(), gain, bias, kernelOffset, tileMode, convolveAlpha, common.getInput(0), common.cropRect());
}
...
skif::LayerSpace<SkIRect> SkMatrixConvolutionImageFilter::boundsSampledByKernel(const skif::LayerSpace<SkIRect>& bounds) const
{
    return adjust(bounds, -fKernelOffset.x(), -fKernelOffset.y(), fKernelSize.width() - fKernelOffset.x() - 1, fKernelSize.height() - fKernelOffset.y() - 1);
}

skif::LayerSpace<SkIRect> SkMatrixConvolutionImageFilter::boundsAffectedByKernel(const skif::LayerSpace<SkIRect>& bounds) const
{
    return adjust(bounds, fKernelOffset.x() - fKernelSize.width() + 1, fKernelOffset.y() - fKernelSize.height() + 1, fKernelOffset.x(), fKernelOffset.y());
}

sk_sp<SkShader> SkMatrixConvolutionImageFilter::createShader(const skif::Context& ctx, sk_sp<SkShader> input) const
{
...
skif::FilterResult SkMatrixConvolutionImageFilter::onFilterImage(const skif::Context& context) const
{
    ...
    skif::FilterResult childOutput = this->getChildOutput(0, context.withNewDesiredOutput(requiredInput));
    ...
    builder.add(childOutput, this->boundsSampledByKernel(outputBounds), ShaderFlags::kSampledRepeatedly);
    return builder.eval([&](SkSpan<sk_sp<SkShader>> inputs) { return this->createShader(context, inputs[0]); }, outputBounds);
}
