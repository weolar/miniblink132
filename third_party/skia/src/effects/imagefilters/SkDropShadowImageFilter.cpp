...
static sk_sp<SkImageFilter> make_drop_shadow_graph(SkVector offset, SkSize sigma, SkColor4f color, sk_sp<SkColorSpace> colorSpace, bool shadowOnly,
    sk_sp<SkImageFilter> input, const std::optional<SkRect>& crop)
{
    ...
    filter = SkImageFilters::ColorFilter(SkColorFilters::Blend(color, std::move(colorSpace), SkBlendMode::kSrcIn), std::move(filter));
    ...
    filter = SkImageFilters::MatrixTransform(SkMatrix::Translate(offset.fX, offset.fY), SkFilterMode::kLinear, std::move(filter));
    if (!shadowOnly) {
#if defined(SK_LEGACY_BLEND_FOR_DROP_SHADOWS)
        filter = SkImageFilters::Blend(SkBlendMode::kSrcOver, std::move(filter), std::move(input));
#else
...
sk_sp<SkFlattenable> legacy_drop_shadow_create_proc(SkReadBuffer& buffer)
{
    ...
    return make_drop_shadow_graph({ dx, dy }, { sigmaX, sigmaY }, color, /*colorSpace=*/nullptr, shadowOnly, std::move(child), cropRect);
}

} // anonymous namespace

sk_sp<SkImageFilter> SkImageFilters::DropShadow(SkScalar dx, SkScalar dy, SkScalar sigmaX, SkScalar sigmaY, SkColor4f color, sk_sp<SkColorSpace> colorSpace,
...
sk_sp<SkImageFilter> SkImageFilters::DropShadowOnly(SkScalar dx, SkScalar dy, SkScalar sigmaX, SkScalar sigmaY, SkColor4f color, sk_sp<SkColorSpace> colorSpace,
    sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    return make_drop_shadow_graph({ dx, dy }, { sigmaX, sigmaY }, color, std::move(colorSpace),
