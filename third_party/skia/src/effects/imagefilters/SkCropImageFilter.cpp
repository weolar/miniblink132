...
private:
    ...
    bool ignoreInputsAffectsTransparentBlack() const override
    {
        return true;
    }
    ...
    // be tiled across.
    skif::LayerSpace<SkIRect> requiredInput(const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& outputBounds) const
    {
...
sk_sp<SkImageFilter> SkImageFilters::Crop(const SkRect& rect, SkTileMode tileMode, sk_sp<SkImageFilter> input)
{
...
sk_sp<SkImageFilter> SkImageFilters::Tile(const SkRect& src, const SkRect& dst, sk_sp<SkImageFilter> input)
{
...
{
    skif::LayerSpace<SkIRect> cropInput = this->requiredInput(context.mapping(), context.desiredOutput());
    skif::FilterResult childOutput = this->getChildOutput(0, context.withNewDesiredOutput(cropInput));
