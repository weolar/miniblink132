...
sk_sp<SkImageFilter> SkImageFilters::ColorFilter(sk_sp<SkColorFilter> cf, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    if (cf) {
    ...
    if (cf) {
        filter = sk_sp<SkImageFilter>(new SkColorFilterImageFilter(std::move(cf), std::move(filter)));
    }
    if (cropRect) {
