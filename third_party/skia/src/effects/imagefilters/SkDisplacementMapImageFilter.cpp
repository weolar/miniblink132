...
public:
    SkDisplacementMapImageFilter(SkColorChannel xChannel, SkColorChannel yChannel, SkScalar scale, sk_sp<SkImageFilter> inputs[2])
...
private:
    ...
    skif::LayerSpace<SkIRect> outsetByMaxDisplacement(const skif::Mapping& mapping, skif::LayerSpace<SkIRect> bounds) const
    {
        ...
        skif::LayerSpace<SkSize> maxDisplacement = mapping.paramToLayer(skif::ParameterSpace<SkSize>({ 0.5f * fScale, 0.5f * fScale }));
    ...
    sk_sp<SkShader> displacement, sk_sp<SkShader> color, skif::LayerSpace<skif::Vector> scale, SkColorChannel xChannel, SkColorChannel yChannel)
{
    ...
    const SkRuntimeEffect* displacementEffect = GetKnownRuntimeEffect(SkKnownRuntimeEffects::StableKey::kDisplacement);

    auto channelSelector = [](SkColorChannel c) {
        return SkV4 { c == SkColorChannel::kR ? 1.f : 0.f, c == SkColorChannel::kG ? 1.f : 0.f, c == SkColorChannel::kB ? 1.f : 0.f,
...
sk_sp<SkImageFilter> SkImageFilters::DisplacementMap(SkColorChannel xChannelSelector, SkColorChannel yChannelSelector, SkScalar scale,
    sk_sp<SkImageFilter> displacement, sk_sp<SkImageFilter> color, const CropRect& cropRect)
{
    ...
    sk_sp<SkImageFilter> filter(new SkDisplacementMapImageFilter(xChannelSelector, yChannelSelector, scale, inputs));
    if (cropRect) {
...
sk_sp<SkFlattenable> SkDisplacementMapImageFilter::CreateProc(SkReadBuffer& buffer)
{
    ...
    return SkImageFilters::DisplacementMap(xsel, ysel, scale, common.getInput(0), common.getInput(1), common.cropRect());
}
...
{
    skif::LayerSpace<SkIRect> requiredColorInput = this->outsetByMaxDisplacement(ctx.mapping(), ctx.desiredOutput());
    skif::FilterResult colorOutput = this->getChildOutput(kColor, ctx.withNewDesiredOutput(requiredColorInput));
    if (!colorOutput) {
    ...
    skif::LayerSpace<SkIRect> outputBounds = this->outsetByMaxDisplacement(ctx.mapping(), colorOutput.layerBounds());
    ...
    skif::FilterResult displacementOutput = this->getChildOutput(kDisplacement, ctx.withNewDesiredOutput(outputBounds).withNewColorSpace(/*cs=*/nullptr));

    // NOTE: The scale is a "vector" not a "size" since we want to preserve negations on the final
    // displacement vector.
    const skif::LayerSpace<skif::Vector> scale = ctx.mapping().paramToLayer(skif::ParameterSpace<skif::Vector>({ fScale, fScale }));
    if (!displacementOutput) {
        ...
        // simple transform.
        skif::LayerSpace<SkMatrix> constantDisplacement { SkMatrix::Translate(-0.5f * scale.x(), -0.5f * scale.y()) };
    ...
    builder.add(colorOutput,
        /*sampleBounds=*/requiredColorInput, ShaderFlags::kNonTrivialSampling, kDisplacementSampling);
    return builder.eval(
        [&](SkSpan<sk_sp<SkShader>> inputs) { return make_displacement_shader(inputs[kDisplacement], inputs[kColor], scale, fXChannel, fYChannel); },
        outputBounds);
}
    ...
    const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const
{
    ...
    requiredInput.join(this->getChildInputLayerBounds(kDisplacement, mapping, desiredOutput, contentBounds));
...
{
    SkRect colorBounds = this->getInput(kColor) ? this->getInput(kColor)->computeFastBounds(src) : src;
