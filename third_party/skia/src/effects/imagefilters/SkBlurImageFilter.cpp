...
private:
    ...
    skif::LayerSpace<SkIRect> kernelBounds(const skif::Mapping& mapping, skif::LayerSpace<SkIRect> bounds, bool useBlurEngine) const
    {
...
sk_sp<SkImageFilter> SkImageFilters::Blur(SkScalar sigmaX, SkScalar sigmaY, SkTileMode tileMode, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
...
skif::FilterResult SkBlurImageFilter::onFilterImage(const skif::Context& ctx) const
{
    ...
    SkASSERT(sigma.width() >= 0.f && sigma.width() <= kMaxSigma && sigma.height() >= 0.f && sigma.height() <= kMaxSigma);
    ...
    if (fLegacyTileMode != SkTileMode::kDecal) {
        ...
        childOutput = childOutput.applyCrop(inputCtx, childOutput.layerBounds(), fLegacyTileMode);
    }
    ...
    sk_sp<SkSpecialImage> blurResult = legacyBlur->blur(SkSize(sigma), std::move(resolvedChildOutput), srcRect, SkTileMode::kDecal, srcRelativeOutput);
    return skif::FilterResult { std::move(blurResult), maxOutput.topLeft() };
}

skif::LayerSpace<SkSize> SkBlurImageFilter::mapSigma(const skif::Mapping& mapping, bool useBlurEngine) const
{
    skif::LayerSpace<SkSize> sigma = mapping.paramToLayer(fSigma);
    // Clamp to the maximum sigma
    sigma = skif::LayerSpace<SkSize>({ std::min(sigma.width(), kMaxSigma), std::min(sigma.height(), kMaxSigma) });
    ...
    if (!SkIsFinite(sigma.width()) || (!useBlurEngine && SkBlurEngine::BoxBlurWindow(sigma.width()) <= 1)
        || (useBlurEngine && SkBlurEngine::IsEffectivelyIdentity(sigma.width()))) {
        sigma = skif::LayerSpace<SkSize>({ 0.f, sigma.height() });
    }

    if (!SkIsFinite(sigma.height()) || (!useBlurEngine && SkBlurEngine::BoxBlurWindow(sigma.height()) <= 1)
    ...
    const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const
{
    ...
    skif::LayerSpace<SkIRect> requiredInput = this->kernelBounds(mapping, desiredOutput, /*useBlurEngine=*/true);
