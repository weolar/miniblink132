...
public:
    SkRuntimeImageFilter(
        const SkRuntimeShaderBuilder& builder, float maxSampleRadius, std::string_view childShaderNames[], const sk_sp<SkImageFilter> inputs[], int inputCount)
...
private:
    ...
    {
        return MatrixCapability::kTranslate;
    }
    ...
    skif::LayerSpace<SkIRect> applyMaxSampleRadius(const skif::Mapping& mapping, skif::LayerSpace<SkIRect> bounds) const
    {
        skif::LayerSpace<SkISize> maxSampleRadius = mapping.paramToLayer(skif::ParameterSpace<SkSize>({ fMaxSampleRadius, fMaxSampleRadius })).ceil();
...
sk_sp<SkImageFilter> SkImageFilters::RuntimeShader(
    const SkRuntimeShaderBuilder& builder, SkScalar sampleRadius, std::string_view childShaderName, sk_sp<SkImageFilter> input)
{
...
sk_sp<SkImageFilter> SkImageFilters::RuntimeShader(
    const SkRuntimeShaderBuilder& builder, SkScalar maxSampleRadius, std::string_view childShaderNames[], const sk_sp<SkImageFilter> inputs[], int inputCount)
{
    if (maxSampleRadius < 0.f) {
        return nullptr; // invalid sample radius
    }

    auto child_is_shader = [](const SkRuntimeEffect::Child* child) { return child && child->type == SkRuntimeEffect::ChildType::kShader; };
    ...
    return sk_sp<SkImageFilter>(new SkRuntimeImageFilter(builder, maxSampleRadius, childShaderNames, inputs, inputCount));
}
...
sk_sp<SkFlattenable> SkRuntimeImageFilter::CreateProc(SkReadBuffer& buffer)
{
    ...
    return SkImageFilters::RuntimeShader(builder, maxSampleRadius, childShaderNames.data(), common.inputs(), common.inputCount());
}
...
skif::FilterResult SkRuntimeImageFilter::onFilterImage(const skif::Context& ctx) const
{
    ...
    skif::Context inputCtx = ctx.withNewDesiredOutput(this->applyMaxSampleRadius(ctx.mapping(), ctx.desiredOutput()));
    ...
    for (int i = 0; i < inputCount; ++i) {
        ...
        builder.add(this->getChildOutput(i, inputCtx), inputCtx.desiredOutput(), ShaderFlags::kNonTrivialSampling);
    }
    return builder.eval(
    ...
    const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const
{
    ...
    if (inputCount <= 0) {
        ...
        skif::LayerSpace<SkIRect> requiredInput = this->applyMaxSampleRadius(mapping, desiredOutput);

        // Union of all child input bounds so that one source image can provide for all of them.
        return skif::LayerSpace<SkIRect>::Union(inputCount, [&](int i) { return this->getChildInputLayerBounds(i, mapping, requiredInput, contentBounds); });
    }
}
