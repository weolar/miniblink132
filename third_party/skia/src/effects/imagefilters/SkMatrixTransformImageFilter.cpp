...
public:
    ...
    SkMatrixTransformImageFilter(const SkMatrix& transform, const SkSamplingOptions& sampling, sk_sp<SkImageFilter> input)
...
private:
    ...
    skif::LayerSpace<SkIRect> requiredInput(const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput) const;
...
sk_sp<SkImageFilter> SkImageFilters::MatrixTransform(const SkMatrix& transform, const SkSamplingOptions& sampling, sk_sp<SkImageFilter> input)
{
    if (!transform.invert(/*inverse=*/nullptr)) {
        return nullptr;
    }
    return sk_sp<SkImageFilter>(new SkMatrixTransformImageFilter(transform, sampling, std::move(input)));
}

sk_sp<SkImageFilter> SkImageFilters::Offset(SkScalar dx, SkScalar dy, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    ...
    sk_sp<SkImageFilter> offset = SkImageFilters::MatrixTransform(SkMatrix::Translate(dx, dy), SkFilterMode::kNearest, std::move(input));
...
void SkRegisterMatrixTransformImageFilterFlattenable()
{
    ...
    SkFlattenable::Register("SkOffsetImageFilter", SkMatrixTransformImageFilter::LegacyOffsetCreateProc);
    SkFlattenable::Register("SkOffsetImageFilterImpl", SkMatrixTransformImageFilter::LegacyOffsetCreateProc);
}
...
{
    skif::LayerSpace<SkIRect> requiredInput = this->requiredInput(context.mapping(), context.desiredOutput());
...
skif::LayerSpace<SkIRect> SkMatrixTransformImageFilter::requiredInput(const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput) const
{
