...
sk_sp<SkImageFilter> SkImageFilters::Compose(sk_sp<SkImageFilter> outer, sk_sp<SkImageFilter> inner)
{
    if (!outer) {
        return inner;
    }
...
skif::FilterResult SkComposeImageFilter::onFilterImage(const skif::Context& ctx) const
{
    ...
    auto innerOutputBounds = this->getChildOutputLayerBounds(kInner, ctx.mapping(), ctx.source().layerBounds());
    // Get the required input for the outer filter, that it needs to cover the desired output.
    skif::LayerSpace<SkIRect> outerRequiredInput = this->getChildInputLayerBounds(kOuter, ctx.mapping(), ctx.desiredOutput(), innerOutputBounds);

    // Evalute the inner filter and pass that to the outer filter.
    skif::FilterResult innerResult = this->getChildOutput(kInner, ctx.withNewDesiredOutput(outerRequiredInput));
    ...
    const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const
{
    ...
    skif::LayerSpace<SkIRect> innerDesiredOutput = this->getChildInputLayerBounds(kOuter, mapping, desiredOutput, outerContentBounds);
...
{
    return this->getInput(kOuter)->computeFastBounds(this->getInput(kInner)->computeFastBounds(src));
}
