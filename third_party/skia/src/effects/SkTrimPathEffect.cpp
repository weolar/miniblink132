...
static size_t add_segments(const SkPath& src, SkScalar start, SkScalar stop, SkPath* dst, bool requires_moveto = true)
{
    SkASSERT(start < stop);
    ...
    do {
        ...
        if (start < next_offset) {
            measure.getSegment(start - current_segment_offset, stop - current_segment_offset, dst, requires_moveto);
...
bool SkTrimPE::onFilterPath(SkPath* dst, const SkPath& src, SkStrokeRec*, const SkRect*, const SkMatrix&) const
{
    if (fStartT >= fStopT) {
...
sk_sp<SkFlattenable> SkTrimPE::CreateProc(SkReadBuffer& buffer)
{
    ...
    return SkTrimPathEffect::Make(start, stop, (mode & 1) ? SkTrimPathEffect::Mode::kInverted : SkTrimPathEffect::Mode::kNormal);
}
