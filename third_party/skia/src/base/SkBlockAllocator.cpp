...
SkBlockAllocator::SkBlockAllocator(GrowthPolicy policy, size_t blockIncrementBytes, size_t additionalPreallocBytes)
    : fTail(&fHead)
    ...
    , fBlockIncrement(SkTo<uint16_t>(std::min(SkAlignTo(blockIncrementBytes, kAddressAlign) / kAddressAlign, (size_t)std::numeric_limits<uint16_t>::max())))
...
void SkBlockAllocator::reset()
{
    ...
    SkASSERT(fTail == &fHead && fHead.fNext == nullptr && fHead.fPrev == nullptr && fHead.metadata() == 0 && fHead.fCursor == kDataStart);
