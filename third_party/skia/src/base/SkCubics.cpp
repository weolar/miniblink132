...
int SkCubics::RootsReal(double A, double B, double C, double D, double solution[3])
{
    ...
    if (R2MinusQ3 < 0) { // we have 3 real roots
        ...
        *roots++ = r;
        ...
        r = neg2RootQ * cos((theta - 2 * PI) / 3) - adiv3;
        if (!nearly_equal(solution[0], r) && (roots - solution == 1 || !nearly_equal(solution[1], r))) {
            *roots++ = r;
        }
        ...
        r = A - adiv3;
        *roots++ = r;
        if (!sk_double_nearly_zero(R2) && sk_doubles_nearly_equal_ulps(R2, Q3)) {
            r = -A / 2 - adiv3;
            if (!nearly_equal(solution[0], r)) {
                *roots++ = r;
            }
...
int SkCubics::RootsValidT(double A, double B, double C, double D, double solution[3])
{
    ...
    for (int index = 0; index < realRoots; ++index) {
        ...
        if (tValue >= 1.0 && tValue <= 1.00005) {
            ...
            if ((foundRoots < 1 || !sk_doubles_nearly_equal_ulps(solution[0], 1)) && (foundRoots < 2 || !sk_doubles_nearly_equal_ulps(solution[1], 1))) {
            ...
            if ((foundRoots < 1 || !sk_double_nearly_zero(solution[0])) && (foundRoots < 2 || !sk_double_nearly_zero(solution[1]))) {
...
int SkCubics::BinarySearchRootsValidT(double A, double B, double C, double D, double solution[3])
{
    ...
    for (; startIndex < 3; startIndex++) {
        ...
        if (root >= 0) {
            ...
            if ((foundRoots < 1 || !approximately_zero(solution[0] - root)) && (foundRoots < 2 || !approximately_zero(solution[1] - root))) {
