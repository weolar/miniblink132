...
std::array<double, 2> SkBezierCubic::EvalAt(const double curve[8], double t)
{
    ...
    return { a * in_X(0) + b * in_X(1) + c * in_X(2) + d * in_X(3), a * in_Y(0) + b * in_Y(1) + c * in_Y(2) + d * in_Y(3) };
}
...
SkSpan<const float> SkBezierCubic::IntersectWithHorizontalLine(SkSpan<const SkPoint> controlPoints, float yIntercept, float* intersectionStorage)
{
    SkASSERT(controlPoints.size() >= 4);
    const DPoint P0 = controlPoints[0], P1 = controlPoints[1], P2 = controlPoints[2], P3 = controlPoints[3];

    const DPoint A = -P0 + 3 * P1 - 3 * P2 + P3, B = 3 * P0 - 6 * P1 + 3 * P2, C = -3 * P0 + 3 * P1, D = P0;

    return Intersect(A.x, B.x, C.x, D.x, A.y, B.y, C.y, D.y, yIntercept, intersectionStorage);
}

SkSpan<const float> SkBezierCubic::Intersect(
    double AX, double BX, double CX, double DX, double AY, double BY, double CY, double DY, float toIntersect, float intersectionsStorage[3])
{
    double roots[3];
    SkSpan<double> ts = SkSpan(roots, SkCubics::RootsReal(AY, BY, CY, DY - toIntersect, roots));
...
SkSpan<const float> SkBezierQuad::IntersectWithHorizontalLine(SkSpan<const SkPoint> controlPoints, float yIntercept, float intersectionStorage[2])
{
    SkASSERT(controlPoints.size() >= 3);
    const DPoint p0 = controlPoints[0], p1 = controlPoints[1], p2 = controlPoints[2];
...
SkSpan<const float> SkBezierQuad::Intersect(double AX, double BX, double CX, double AY, double BY, double CY, double yIntercept, float intersectionStorage[2])
{
