...
struct SkSemaphore::OSSemaphore {
    ...
    OSSemaphore()
    {
        fSemaphore = CreateSemaphore(nullptr /*security attributes, optional*/, 0 /*initial count*/, MAXLONG /*max count*/, nullptr /*name, optional*/);
    }
    ~OSSemaphore()
    {
    ...
    void wait()
    {
        WaitForSingleObject(fSemaphore, INFINITE /*timeout in ms*/);
    }
};
#else
...
struct SkSemaphore::OSSemaphore {
    ...
    OSSemaphore()
    {
        sem_init(&fSemaphore, 0 /*cross process?*/, 0 /*initial count*/);
    }
    ~OSSemaphore()
    {
