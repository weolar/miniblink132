...
template <typename... Ts> class SkZip {
    ...
    public:
        ...
        constexpr Iterator(const SkZip* zip, size_t index)
            : fZip(zip)
            , fIndex(index)
        {
        }
    ...
    template <typename T> inline static constexpr T* nullify = nullptr;

public:
    constexpr SkZip()
        : fPointers(nullify<Ts>...)
        , fSize(0)
    {
    }
...
private:
    ...
    template <std::size_t... Is> constexpr ReturnTuple indexDetail(size_t i, std::index_sequence<Is...>) const
    {
    ...
    template <std::size_t... Is> constexpr std::tuple<Ts*...> pointersAtDetail(size_t i, std::index_sequence<Is...>) const
    {
...
class SkMakeZipDetail {
    ...
    template <typename T> struct ContiguousMemory<T*> {
        using value_type = T;
        static constexpr value_type* Data(T* t)
        {
            return t;
        }
        static constexpr size_t Size(T* s)
        {
            return SIZE_MAX;
        }
    };
    template <typename T, size_t N> struct ContiguousMemory<T (&)[N]> {
        using value_type = T;
        static constexpr value_type* Data(T (&t)[N])
        {
            return t;
        }
    ...
    template <typename T> struct ContiguousMemory<SkSpan<T>> {
        using value_type = T;
        static constexpr value_type* Data(SkSpan<T> s)
        {
            return s.data();
        }
        static constexpr size_t Size(SkSpan<T> s)
        {
            return s.size();
        }
    };
    ...
    template <typename C> struct ContiguousMemory<C&> {
        ...
        static constexpr value_type* Data(C& c)
        {
            return c.data();
        }
        static constexpr size_t Size(C& c)
        {
            return c.size();
        }
    };
...
public:
    template <typename... Ts> static constexpr auto MakeZip(Ts&&... ts)
    {
...
template <typename... Ts> SkZip(size_t size, Ts*... ts) -> SkZip<Ts...>;

template <typename... Ts> inline constexpr auto SkMakeZip(Ts&&... ts)
{
