...
public:
    ...
    SkFibBlockSizes(uint32_t staticBlockSize, uint32_t firstAllocationSize)
        : fIndex { 0 }
    {
        fBlockUnitSize = firstAllocationSize > 0 ? firstAllocationSize : staticBlockSize > 0 ? staticBlockSize : 1024;
    ...
    uint32_t nextBlockSize()
    {
        ...
        if (SkTo<size_t>(fIndex + 1) < SkFibonacci47.size() && SkFibonacci47[fIndex + 1] < kMaxSize / fBlockUnitSize) {
            fIndex += 1;
        }
...
public:
    ...
    {
        return this->make([&](void* objStart) { return new (objStart) T(std::forward<Args>(args)...); });
    }
    ...
    template <typename T> T* make()
    {
        if constexpr (std::is_standard_layout<T>::value && std::is_trivial<T>::value) {
            ...
            return this->make([&](void* objStart) { return new (objStart) T(); });
        }
    }
    ...
    template <typename T, typename Initializer> T* makeInitializedArray(size_t count, Initializer initializer)
    {
        T* array = this->allocUninitializedArray<T>(count);
        for (size_t i = 0; i < count; i++) {
    ...
    template <typename T> T* makeArrayCopy(SkSpan<const T> toCopy)
    {
...
template <size_t InlineStorageSize> class SkSTArenaAlloc : private std::array<char, InlineStorageSize>, public SkArenaAlloc {
public:
...
template <size_t InlineStorageSize> class SkSTArenaAllocWithReset : private std::array<char, InlineStorageSize>, public SkArenaAllocWithReset {
public:
