...
public:
    ...
    public:
        ...
        template <size_t Align = 1, size_t Padding = 0> int avail() const
        {
        ...
        template <size_t Align = 1, size_t Padding = 0> int firstAlignedOffset() const
        {
    ...
    private:
        ...
        template <size_t Align, size_t Padding> int cursor() const
        {
     ...
     */
    template <size_t Align, size_t Padding = 0> ByteRange allocate(size_t size);
     ...
     */
    template <size_t Align, size_t Padding = 0> Block* owningBlock(const void* ptr, int start);

    template <size_t Align, size_t Padding = 0> const Block* owningBlock(const void* ptr, int start) const
    {
...
private:
    ...
    template <size_t Align, size_t Padding> static constexpr size_t MaxBlockSize();
...
template <size_t N> class SkSBlockAllocator : SkNoncopyable {
public:
    ...
    {
        return reinterpret_cast<SkBlockAllocator*>(fStorage);
    }
...
template <size_t Align, size_t Padding> constexpr size_t SkBlockAllocator::BlockOverhead()
{
    static_assert(SkAlignTo(kDataStart + Padding, Align) >= sizeof(Block));
    return SkAlignTo(kDataStart + Padding, Align);
}

template <size_t Align, size_t Padding> constexpr size_t SkBlockAllocator::Overhead()
{
    ...
    return std::max(sizeof(SkBlockAllocator), offsetof(SkBlockAllocator, fHead) + BlockOverhead<Align, Padding>());
}

template <size_t Align, size_t Padding> constexpr size_t SkBlockAllocator::MaxBlockSize()
{
...
template <size_t Align, size_t Padding> void SkBlockAllocator::reserve(size_t size, ReserveFlags flags)
{
    if (size > kMaxAllocationSize) {
        SK_ABORT("Allocation too large (%zu bytes requested)", size);
    }
    int iSize = (int)size;
    if ((flags & kIgnoreExistingBytes_Flag) || this->currentBlock()->avail<Align, Padding>() < iSize) {
...
template <size_t Align, size_t Padding> SkBlockAllocator::ByteRange SkBlockAllocator::allocate(size_t size)
{
    ...
    static_assert((kMaxAllocationSize + SkAlignTo(MaxBlockSize<Align, Padding>(), Align)) <= (size_t)std::numeric_limits<int32_t>::max());
    ...
    SkASSERT(offset - fTail->fCursor >= (int)Padding && offset - fTail->fCursor <= (int)(Padding + Align - 1));
...
template <size_t Align, size_t Padding> SkBlockAllocator::Block* SkBlockAllocator::owningBlock(const void* p, int start)
{
    ...
    if /* constexpr */ (Align <= kAddressAlign) {
        Block* block = reinterpret_cast<Block*>((reinterpret_cast<uintptr_t>(p) - start - Padding) & ~(Align - 1));
...
template <size_t Align, size_t Padding> int SkBlockAllocator::Block::alignedOffset(int offset) const
{
    ...
    static_assert(MaxBlockSize<Align, Padding>() + Padding + Align - 1 <= (size_t)std::numeric_limits<int32_t>::max());
...
///////// Block iteration
template <bool Forward, bool Const> class SkBlockAllocator::BlockIter {
private:
    using BlockT = typename std::conditional<Const, const Block, Block>::type;
    using AllocatorT = typename std::conditional<Const, const SkBlockAllocator, SkBlockAllocator>::type;
