...
#define SINTU template <int N, typename T, typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>> SI

namespace skvx {

template <int N, typename T> struct alignas(N * sizeof(T)) Vec;

template <int... Ix, int N, typename T> SI Vec<sizeof...(Ix), T> shuffle(const Vec<N, T>&);
...
template <typename T> struct alignas(2 * sizeof(T)) Vec<2, T> {
    static_assert(sizeof(T) >= alignof(T), "What kind of unusual T is this?");
...
{
    return sk_bit_cast<Vec<N, T>>((cond & sk_bit_cast<Vec<N, M<T>>>(t)) | (~cond & sk_bit_cast<Vec<N, M<T>>>(e)));
}
...
SIT Vec<1, T> if_then_else(const Vec<1, M<T>>& cond, const Vec<1, T>& t, const Vec<1, T>& e)
{
    ...
    return sk_bit_cast<Vec<1, T>>((cond & sk_bit_cast<Vec<1, M<T>>>(t)) | (~cond & sk_bit_cast<Vec<1, M<T>>>(e)));
}
SINT Vec<N, T> if_then_else(const Vec<N, M<T>>& cond, const Vec<N, T>& t, const Vec<N, T>& e)
{
    ...
    if constexpr (N * sizeof(T) == 32) {
        return sk_bit_cast<Vec<N, T>>(_mm256_blendv_epi8(sk_bit_cast<__m256i>(e), sk_bit_cast<__m256i>(t), sk_bit_cast<__m256i>(cond)));
    }
#endif
#if SKVX_USE_SIMD && SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_SSE41
    if constexpr (N * sizeof(T) == 16) {
        return sk_bit_cast<Vec<N, T>>(_mm_blendv_epi8(sk_bit_cast<__m128i>(e), sk_bit_cast<__m128i>(t), sk_bit_cast<__m128i>(cond)));
    }
#endif
#if SKVX_USE_SIMD && defined(SK_ARM_HAS_NEON)
    if constexpr (N * sizeof(T) == 16) {
        return sk_bit_cast<Vec<N, T>>(vbslq_u8(sk_bit_cast<uint8x16_t>(cond), sk_bit_cast<uint8x16_t>(t), sk_bit_cast<uint8x16_t>(e)));
    }
#endif
#if SKVX_USE_SIMD && SK_CPU_LSX_LEVEL >= SK_CPU_LSX_LEVEL_LASX
    if constexpr (N * sizeof(T) == 32) {
        return sk_bit_cast<Vec<N, T>>(__lasx_xvbitsel_v(sk_bit_cast<__m256i>(e), sk_bit_cast<__m256i>(t), sk_bit_cast<__m256i>(cond)));
    }
#endif
#if SKVX_USE_SIMD && SK_CPU_LSX_LEVEL >= SK_CPU_LSX_LEVEL_LSX
    if constexpr (N * sizeof(T) == 16) {
        return sk_bit_cast<Vec<N, T>>(__lsx_vbitsel_v(sk_bit_cast<__m128i>(e), sk_bit_cast<__m128i>(t), sk_bit_cast<__m128i>(cond)));
    }
#endif
...
SINT bool any(const Vec<N, T>& x)
{
    ...
    if constexpr (N * sizeof(T) == 8) {
        return vmaxv_u8(sk_bit_cast<uint8x8_t>(x)) > 0;
    }
    if constexpr (N * sizeof(T) == 16) {
        return vmaxvq_u8(sk_bit_cast<uint8x16_t>(x)) > 0;
    }
#endif
...
#if SKVX_USE_SIMD && SK_CPU_LSX_LEVEL >= SK_CPU_LSX_LEVEL_LASX
    if constexpr (N * sizeof(T) == 32) {
        v8i32 retv = (v8i32)__lasx_xvmskltz_w(__lasx_xvslt_wu(__lasx_xvldi(0), sk_bit_cast<__m256i>(x)));
        return (retv[0] | retv[4]) != 0b0000;
    }
#endif
#if SKVX_USE_SIMD && SK_CPU_LSX_LEVEL >= SK_CPU_LSX_LEVEL_LSX
    if constexpr (N * sizeof(T) == 16) {
        v4i32 retv = (v4i32)__lsx_vmskltz_w(__lsx_vslt_wu(__lsx_vldi(0), sk_bit_cast<__m128i>(x)));
...
// true lane has a mix of 0 and 1 bits.
    ...
    if constexpr (sizeof(T) == 1 && N == 8) {
        return vminv_u8(sk_bit_cast<uint8x8_t>(x)) > 0;
    }
    if constexpr (sizeof(T) == 1 && N == 16) {
        return vminvq_u8(sk_bit_cast<uint8x16_t>(x)) > 0;
    }
    if constexpr (sizeof(T) == 2 && N == 4) {
        return vminv_u16(sk_bit_cast<uint16x4_t>(x)) > 0;
    }
    if constexpr (sizeof(T) == 2 && N == 8) {
        return vminvq_u16(sk_bit_cast<uint16x8_t>(x)) > 0;
    }
    if constexpr (sizeof(T) == 4 && N == 2) {
        return vminv_u32(sk_bit_cast<uint32x2_t>(x)) > 0;
    }
    if constexpr (sizeof(T) == 4 && N == 4) {
        return vminvq_u32(sk_bit_cast<uint32x4_t>(x)) > 0;
    }
#endif
#if SKVX_USE_SIMD && defined(__wasm_simd128__)
    if constexpr (N == 4 && sizeof(T) == 4) {
...
template <typename D, typename S> SI Vec<1, D> cast(const Vec<1, S>& src)
{
    return (D)src.val;
}

template <typename D, int N, typename S> SI Vec<N, D> cast(const Vec<N, S>& src)
{
...
template <int... Ix, int N, typename T> SI Vec<sizeof...(Ix), T> shuffle(const Vec<N, T>& x)
{
#if SKVX_USE_SIMD && defined(__clang__)
...
template <typename Fn, int N, typename T, typename... Rest> auto map(Fn&& fn, const Vec<N, T>& first, const Rest&... rest)
{
...
#define F(x) sk_bit_cast<Vec<N, float>>(x)
    Vec<N, int32_t> sem = I(x), s = sem & 0x8000'0000, em = min(sem ^ s, 0x4780'0000), // |x| clamped to f16 infinity
...
{
#if SKVX_USE_SIMD && (SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_SSE1 || defined(SK_ARM_HAS_NEON) || SK_CPU_LSX_LEVEL >= SK_CPU_LSX_LEVEL_LSX)
    ...
    if constexpr (N == 16 && sizeof(T) == 1) {
        ...
        return sk_bit_cast<Vec<N, T>>(_mm_adds_epu8(sk_bit_cast<__m128i>(x), sk_bit_cast<__m128i>(y)));
#elif SK_CPU_LSX_LEVEL >= SK_CPU_LSX_LEVEL_LSX
        return sk_bit_cast<Vec<N, T>>(__lsx_vsadd_bu(sk_bit_cast<__m128i>(x), sk_bit_cast<__m128i>(y)));
#else // SK_ARM_HAS_NEON
        return sk_bit_cast<Vec<N, T>>(vqaddq_u8(sk_bit_cast<uint8x16_t>(x), sk_bit_cast<uint8x16_t>(y)));
#endif
...
SIN Vec<N, uint16_t> mulhi(const Vec<N, uint16_t>& x, const Vec<N, uint16_t>& y)
{
    ...
    if constexpr (N == 8) {
        return sk_bit_cast<Vec<8, uint16_t>>(_mm_mulhi_epu16(sk_bit_cast<__m128i>(x), sk_bit_cast<__m128i>(y)));
    } else if constexpr (N < 8) {
...
#define IMPL_LOAD4_TRANSPOSED(N, T, VLD)                                                                                                                       \
    SI void strided_load4(const T* v, Vec<N, T>& a, Vec<N, T>& b, Vec<N, T>& c, Vec<N, T>& d)                                                                  \
    {                                                                                                                                                          \
        auto mat = VLD(v);                                                                                                                                     \
...
SI void strided_load4(const int* v, Vec<4, int>& a, Vec<4, int>& b, Vec<4, int>& c, Vec<4, int>& d)
{
