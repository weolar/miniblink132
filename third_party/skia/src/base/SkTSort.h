...
template <typename T, typename C> void SkTInsertionSort(T* left, int count, const C& lessThan)
{
...
template <typename T, typename C> T* SkTQSort_Partition(T* left, int count, T* pivot, const C& lessThan)
{
    T* right = left + count - 1;
    using std::swap;
    T pivotValue = *pivot;
 ...
 */
template <typename T, typename C> void SkTIntroSort(int depth, T* left, int count, const C& lessThan)
{
    for (;;) {
        if (count <= 32) {
