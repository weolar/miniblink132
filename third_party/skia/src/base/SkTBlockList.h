...
template <typename T, bool Forward, bool Const, IndexFn Start, IndexFn End, NextFn Next,
    ItemFn<T, typename std::conditional<Const, const SkBlockAllocator::Block, SkBlockAllocator::Block>::type> Resolve>
 ...
 */
template <typename T, int StartingItems = 1> class SkTBlockList {
public:
    /**
    ...
    */
     ...
     */
    explicit SkTBlockList(int itemsPerBlock, SkBlockAllocator::GrowthPolicy policy = SkBlockAllocator::GrowthPolicy::kFixed)
        : fAllocator(policy, SkBlockAllocator::BlockOverhead<alignof(T)>() + sizeof(T) * itemsPerBlock)
    {
    }
    ...
    void reserve(int n)
    {
        ...
        if (n > avail) {
            ...
            fAllocator->template reserve<alignof(T)>(reserved * sizeof(T), SkBlockAllocator::kIgnoreExistingBytes_Flag);
        }
    }
...
private:
    ...
    inline static constexpr size_t StartingSize = SkBlockAllocator::Overhead<alignof(T)>() + StartingItems * sizeof(T);
    ...
    void* pushItem()
    {
        ...
        SkASSERT(br.fStart == br.fAlignedOffset || br.fAlignedOffset == First(fAllocator->currentBlock()));
...
template <typename T, int SI1> template <int SI2> void SkTBlockList<T, SI1>::concat(SkTBlockList<T, SI2>&& other)
{
    ...
    SkDEBUGCODE(int oldCount = this->count();) if (headBlock->metadata() > 0)
    {
        ...
        if (headItemCount > avail) {
            ...
            fAllocator->template reserve<alignof(T)>(
                (headItemCount - avail) * sizeof(T), SkBlockAllocator::kIgnoreExistingBytes_Flag | SkBlockAllocator::kIgnoreGrowthPolicy_Flag);
        }
        ...
        if constexpr (std::is_trivially_copy_constructible<T>::value) {
            ...
            if (headItemCount > avail) {
                ...
                copy(headBlock, headStart + avail * sizeof(T), fAllocator.allocator(), headItemCount - avail);
            }
    ...
    SkASSERT(other.fAllocator->headBlock()->metadata() == 0 && fAllocator->metadata() == oldCount + headItemCount);
...
public:
    ...
    public:
        ...
        Item& operator++()
        {
            ...
            SkASSERT((Forward && Next(block, fIndex) > fIndex) || (!Forward && Next(block, fIndex) < fIndex));
    ...
    Item begin() const
    {
        return Item(fBlockIter.begin());
    }
    Item end() const
    {
        return Item(fBlockIter.end());
    }
