...
private:
    ...
    static void load_directory_fonts(const SkFontScanner* scanner, const SkString& directory, const char* suffix, SkFontMgr_Custom::Families* families)
    {
        ...
        while (iter.next(&name, false)) {
            ...
            for (int faceIndex = 0; faceIndex < numFaces; ++faceIndex) {
                ...
                for (int instanceIndex = 0; instanceIndex <= numInstances; ++instanceIndex) {
                    ...
                    if (!scanner->scanInstance(stream.get(), faceIndex, instanceIndex, &realname, &style, &isFixedPitch, nullptr)) {
                    ...
                    addTo->appendTypeface(
                        sk_make_sp<SkTypeface_File>(style, isFixedPitch, true, realname, filename.c_str(), (instanceIndex << 16) + faceIndex));
                }
            }
        }
