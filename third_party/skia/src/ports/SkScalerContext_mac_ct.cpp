...
static void sk_memset_rect32(uint32_t* ptr, uint32_t value, int width, int height, size_t rowBytes)
{
    SkASSERT(width);
...
{
    return CGAffineTransformMake(SkScalarToCGFloat(matrix[SkMatrix::kMScaleX]), -SkScalarToCGFloat(matrix[SkMatrix::kMSkewY]),
        -SkScalarToCGFloat(matrix[SkMatrix::kMSkewX]), SkScalarToCGFloat(matrix[SkMatrix::kMScaleY]), SkScalarToCGFloat(matrix[SkMatrix::kMTransX]),
        SkScalarToCGFloat(matrix[SkMatrix::kMTransY]));
}

SkScalerContext_Mac::SkScalerContext_Mac(sk_sp<SkTypeface_Mac> typeface, const SkScalerContextEffects& effects, const SkDescriptor* desc)
...

{
    ...
    bool invertible = fRec.computeMatrices(SkScalerContextRec::PreMatrixScale::kVertical, &scale, &skTransform, nullptr, nullptr, nullptr);
    ...
    fCTFont = SkCTFontCreateExactCopy(ctFont, textSize, ((SkTypeface_Mac*)this->getTypeface())->fOpszVariation);
...
CGRGBPixel* SkScalerContext_Mac::Offscreen::getCG(
    const SkScalerContext_Mac& context, const SkGlyph& glyph, CGGlyph glyphID, size_t* rowBytesPtr, bool generateA8FromLCD)
{
    if (!fRGBSpace) {
    ...
    if (!fCG || fSize.fWidth < glyph.width() || fSize.fHeight < glyph.height()) {
        ...
        const CGImageAlphaInfo alpha = (glyph.isColor()) ? kCGImageAlphaPremultipliedFirst : kCGImageAlphaNoneSkipFirst;
        const CGBitmapInfo bitmapInfo = kCGBitmapByteOrder32Host | (CGBitmapInfo)alpha;
        fCG.reset(CGBitmapContextCreate(image, fSize.fWidth, fSize.fHeight, 8, rowBytes, fRGBSpace.get(), bitmapInfo));
...
SkScalerContext::GlyphMetrics SkScalerContext_Mac::generateMetrics(const SkGlyph& glyph, SkArenaAlloc*)
{
    ...
    // vertical metrics.
    {
        ...
        CGRect cgBounds;
        CTFontGetBoundingRectsForGlyphs(fCTFont.get(), kCTFontOrientationHorizontal, &cgGlyph, &cgBounds, 1);
        ...
        skBounds = SkRect::MakeXYWH(cgBounds.origin.x, -cgBounds.origin.y - cgBounds.size.height, cgBounds.size.width, cgBounds.size.height);
    }
...
template <bool APPLY_PREBLEND> static inline uint8_t rgb_to_a8(CGRGBPixel rgb, const uint8_t* table8)
{
...
static void RGBToA8(const CGRGBPixel* SK_RESTRICT cgPixels, size_t cgRowBytes, const SkGlyph& glyph, void* glyphImage, const uint8_t* table8)
{
...
template <bool APPLY_PREBLEND> static uint16_t RGBToLcd16(CGRGBPixel rgb, const uint8_t* tableR, const uint8_t* tableG, const uint8_t* tableB)
{
...
template <bool APPLY_PREBLEND>
static void RGBToLcd16(const CGRGBPixel* SK_RESTRICT cgPixels, size_t cgRowBytes, const SkGlyph& glyph, void* glyphImage, const uint8_t* tableR,
...
void SkScalerContext_Mac::generateImage(const SkGlyph& glyph, void* imageBuffer)
{
    ...
    if ((glyph.maskFormat() == SkMask::kLCD16_Format)
        || (glyph.maskFormat() == SkMask::kA8_Format && requestSmooth && SkCTFontGetSmoothBehavior() != SkCTFontSmoothBehavior::none)) {
    ...
    case SkMask::kLCD16_Format: {
        if (fPreBlend.isApplicable()) {
            RGBToLcd16<true>(cgPixels, cgRowBytes, glyph, imageBuffer, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
        } else {
            RGBToLcd16<false>(cgPixels, cgRowBytes, glyph, imageBuffer, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
        }
    } break;
...
public:
    ...
    static void ApplyElement(void* ctx, const CGPathElement* element)
    {
        ...
        case kCGPathElementAddCurveToPoint:
            if (self.currentIsNot(points[0]) || self.currentIsNot(points[1]) || self.currentIsNot(points[2])) {
                self.goingTo(points[2]);
                self.fBuilder.cubicTo(points[0].x, -points[0].y, points[1].x, -points[1].y, points[2].x, -points[2].y);
            }
            break;
...
bool SkScalerContext_Mac::generatePath(const SkGlyph& glyph, SkPath* path, bool* modified)
{
    ...
    if (fDoSubPosition) {
        ...
        CGAffineTransform scale(CGAffineTransformMakeScale(SkScalarToCGFloat(scaleX), SkScalarToCGFloat(scaleY)));
...
void SkScalerContext_Mac::generateFontMetrics(SkFontMetrics* metrics)
{
    ...
    if ((ctAxes && CFArrayGetCount(ctAxes) > 0) || ((SkTypeface_Mac*)this->getTypeface())->fHasColorGlyphs) {
