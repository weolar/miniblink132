...
template <typename T, T* (*C)(), void (*D)(T*)> class SkAutoFc : public SkAutoTCallVProc<T, FcTDestroy<T, D>> {
...
static SkWeakReturn is_weak(FcPattern* pattern, const char object[], int id)
{
    ...
    SkAutoFcPattern match(FcFontSetMatch(config, fontSets, std::size(fontSets), minimal, &result));

    FcLangSet* matchLangSet;
    FcPatternGetLangSet(match, FC_LANG, 0, &matchLangSet);
    return FcLangEqual == FcLangSetHasLang(matchLangSet, (const FcChar8*)"matchlang") ? kIsWeak_WeakReturn : kIsStrong_WeakReturn;
}
...
static SkScalar map_ranges(SkScalar val, MapRanges const ranges[], int rangesCount)
{
    ...
    for (int i = 0; i < rangesCount - 1; ++i) {
        if (val < ranges[i + 1].old_val) {
            return map_range(val, ranges[i].old_val, ranges[i + 1].old_val, ranges[i].new_val, ranges[i + 1].new_val);
        }
    }
...
static SkFontStyle skfontstyle_from_fcpattern(FcPattern* pattern)
{
    ...
    SkScalar weight = map_ranges(get_int(pattern, FC_WEIGHT, FC_WEIGHT_REGULAR), weightRanges, std::size(weightRanges));
    ...
    SkScalar width = map_ranges(get_int(pattern, FC_WIDTH, FC_WIDTH_NORMAL), widthRanges, std::size(widthRanges));
...
public:
    static sk_sp<SkTypeface_fontconfig> Make(SkAutoFcPattern pattern, SkString sysroot, SkFontScanner* scanner)
    {
        return sk_sp<SkTypeface_fontconfig>(new SkTypeface_fontconfig(std::move(pattern), std::move(sysroot), scanner));
    }
...
private:
    SkTypeface_fontconfig(SkAutoFcPattern pattern, SkString sysroot, SkFontScanner* fontScanner)
        : SkTypeface(skfontstyle_from_fcpattern(pattern), FC_PROPORTIONAL != get_int(pattern, FC_SPACING, FC_PROPORTIONAL))
        ...
        , fSysroot(std::move(sysroot))
    {
        ...
        fProxy = fontScanner->MakeFromStream(SkStream::MakeFromFile(filename), SkFontArguments().setCollectionIndex(ttcIndex));
    }
...
public:
    SkScalerContext_fontconfig(
        std::unique_ptr<SkScalerContext> proxy, sk_sp<SkTypeface_fontconfig> typeface, const SkScalerContextEffects& effects, const SkDescriptor* desc)
...
std::unique_ptr<SkScalerContext> SkTypeface_fontconfig::onCreateScalerContext(const SkScalerContextEffects& effects, const SkDescriptor* descr) const
{
    auto proxyScaler = fProxy->onCreateScalerContextAsProxyTypeface(effects, descr, sk_ref_sp(this));
    auto scaler = std::make_unique<SkScalerContext_fontconfig>(std::move(proxyScaler), sk_ref_sp(const_cast<SkTypeface_fontconfig*>(this)), effects, descr);
    return scaler;
}
...
class SkFontMgr_fontconfig : public SkFontMgr {
    ...
    public:
        ...
        sk_sp<SkTypeface> matchStyle(const SkFontStyle& style) override
        {
            SkAutoFcPattern match([this, &style]() {
                ...
                return FcFontSetMatch(fFontMgr->fFC, fontSets, std::size(fontSets), pattern, &result);
            }());
            return fFontMgr->createTypefaceFromFcPattern(std::move(match));
        }
    ...
    static sk_sp<SkDataTable> GetFamilyNames(FcConfig* fcconfig)
    {
        ...
        return SkDataTable::MakeCopyArrays((void const* const*)names.begin(), sizes.begin(), names.size());
    }
