...
static bool has_bitmap_strike(DWriteFontTypeface* typeface, GaspRange range)
{
    Exclusive l(maybe_dw_mutex(*typeface));
    {
        ...
        for (uint32_t i = 0; i < numSizes; ++i, ++sizeTable) {
            if (sizeTable->ppemX == sizeTable->ppemY && range.fMin <= sizeTable->ppemX && sizeTable->ppemX <= range.fMax) {
...

    {
        ...
        for (uint32_t i = 0; i < numSizes; ++i, ++scaleTable) {
            if (scaleTable->ppemX == scaleTable->ppemY && range.fMin <= scaleTable->ppemX && scaleTable->ppemX <= range.fMax) {
...
SkScalerContext_DW::SkScalerContext_DW(sk_sp<DWriteFontTypeface> typefaceRef, const SkScalerContextEffects& effects, const SkDescriptor* desc)
    : SkScalerContext(std::move(typefaceRef), effects, desc)
{
    ...
    } else if (get_gasp_range(typeface, SkScalarRoundToInt(gdiTextSize), &range) && range.fVersion >= 1) {
        fTextSizeRender = realTextSize;
        fRenderingMode = range.fFlags.field.SymmetricSmoothing ? DWRITE_RENDERING_MODE_NATURAL_SYMMETRIC : DWRITE_RENDERING_MODE_NATURAL;
        fTextureType = DWRITE_TEXTURE_CLEARTYPE_3x1;
    ...
    if (typeface->fFactory2 && typeface->fDWriteFontFace2 && SkMask::kA8_Format == fRec.fMaskFormat && !(fRec.fFlags & SkScalerContext::kGenA8FromLCD_Flag)) {
...
SkMatrix sk_matrix_from(DWRITE_MATRIX const& m)
{
    ...
    return SkMatrix::MakeAll(m.m11, m.m21, m.dx, m.m12, m.m22, m.dy, 0, 0, 1);
}
...
D2D1_COLOR_F lerpSkColor(D2D1_COLOR_F c0, D2D1_COLOR_F c1, float t)
{
    ...
    const auto c0_4f = skvx::float4(c0.r, c0.g, c0.b, c0.a), c1_4f = skvx::float4(c1.r, c1.g, c1.b, c1.a), c_4f = c0_4f + (c1_4f - c0_4f) * t;
    D2D1_COLOR_F r;
    c_4f.store(&r);
    return r;
}
...
void truncateToStopInterpolating(SkScalar zeroRadiusStop, std::vector<D2D1_GRADIENT_STOP>& stops, TruncateStops truncateStops)
{
    if (stops.size() <= 1u || zeroRadiusStop < stops.front().position || stops.back().position < zeroRadiusStop) {
        return;
    }

    auto lcmp = [](D2D1_GRADIENT_STOP const& stop, SkScalar position) { return stop.position < position; };
    auto ucmp = [](SkScalar position, D2D1_GRADIENT_STOP const& stop) { return position < stop.position; };
    size_t afterIndex = (truncateStops == TruncateStart) ? std::lower_bound(stops.begin(), stops.end(), zeroRadiusStop, lcmp) - stops.begin()
                                                         : std::upper_bound(stops.begin(), stops.end(), zeroRadiusStop, ucmp) - stops.begin();

    const float t = (zeroRadiusStop - stops[afterIndex - 1].position) / (stops[afterIndex].position - stops[afterIndex - 1].position);
...
bool SkScalerContext_DW::drawColorV1Paint(SkCanvas& canvas, IDWritePaintReader& reader, DWRITE_PAINT_ELEMENT const& element)
{
    ...
    case DWRITE_PAINT_TYPE_SOLID_GLYPH: {
        ...
        UINT16 glyphId = SkTo<UINT16>(solidGlyph.glyphIndex);
        {
            ...
            HRBM(this->getDWriteTypeface()->fDWriteFontFace->GetGlyphRunOutline(SkScalarToFloat(fTextSizeRender), &glyphId,
                     nullptr, //advances
                     nullptr, //offsets
                     1, //num glyphs
                     FALSE, //sideways
    ...
    case DWRITE_PAINT_TYPE_LINEAR_GRADIENT: {
        ...
        SkPoint linePositions[2] = { { linearGradient.x0, linearGradient.y0 }, { linearGradient.x1, linearGradient.y1 } };
        ...
        sk_sp<SkShader> shader(SkGradientShader::MakeLinear(linePositions, skColors.get(), SkColorSpace::MakeSRGB(), skStops.get(), stops.size(), tileMode,
    ...
    case DWRITE_PAINT_TYPE_RADIAL_GRADIENT: {
        ...
        // An opaque color is needed to ensure the gradient is not modulated by alpha.
        skPaint.setColor(SK_ColorBLACK);
        skPaint.setShader(SkGradientShader::MakeTwoPointConical(start, startRadius, end, endRadius, skColors.get(), SkColorSpace::MakeSRGB(), skStops.get(),
            stops.size(), tileMode,
            SkGradientShader::Interpolation { SkGradientShader::Interpolation::InPremul::kNo, SkGradientShader::Interpolation::ColorSpace::kSRGB,
                SkGradientShader::Interpolation::HueMethod::kShorter },
            nullptr));
    ...
    case DWRITE_PAINT_TYPE_SWEEP_GRADIENT: {
        ...
        * start angle being larger than end angle. */
        ...
        skPaint.setShader(SkGradientShader::MakeSweep(center.x(), center.y(), skColors.get(), SkColorSpace::MakeSRGB(), skStops.get(), stops.size(), tileMode,
...
bool SkScalerContext_DW::drawColorV1Image(const SkGlyph& glyph, SkCanvas& canvas)
{
    ...
    HRBM(fontFace->CreatePaintReader(DWRITE_GLYPH_IMAGE_FORMATS_COLR_PAINT_TREE, DWRITE_PAINT_FEATURE_LEVEL_COLR_V1, &paintReader),
...
bool SkScalerContext_DW::generateColorV1PaintBounds(SkMatrix* ctm, SkRect* bounds, IDWritePaintReader& reader, DWRITE_PAINT_ELEMENT const& element)
{
...
bool SkScalerContext_DW::generateColorV1Metrics(const SkGlyph& glyph, SkRect* bounds)
{
    ...
    hr = fontFace->CreatePaintReader(DWRITE_GLYPH_IMAGE_FORMATS_COLR_PAINT_TREE, DWRITE_PAINT_FEATURE_LEVEL_COLR_V1, &paintReader);
    if (FAILED(hr)) {
        return false;
    }
...
bool SkScalerContext_DW::generateColorV1Metrics(const SkGlyph&, SkRect*)
{
    return false;
}
bool SkScalerContext_DW::generateColorV1Image(const SkGlyph&, void*)
{
    return false;
}
bool SkScalerContext_DW::drawColorV1Image(const SkGlyph&, SkCanvas&)
{
    return false;
}
...
bool SkScalerContext_DW::setAdvance(const SkGlyph& glyph, SkVector* advance)
{
    *advance = { 0, 0 };
    ...
    if (DWRITE_MEASURING_MODE_GDI_CLASSIC == fMeasuringMode || DWRITE_MEASURING_MODE_GDI_NATURAL == fMeasuringMode) {
        Exclusive l(maybe_dw_mutex(*typeface));
        HRBM(typeface->fDWriteFontFace->GetGdiCompatibleGlyphMetrics(fTextSizeMeasure,
                 1.0f, // pixelsPerDip
...
bool SkScalerContext_DW::generateDWMetrics(const SkGlyph& glyph, DWRITE_RENDERING_MODE renderingMode, DWRITE_TEXTURE_TYPE textureType, SkRect* bounds)
{
    DWriteFontTypeface* typeface = this->getDWriteTypeface();
    ...
    SkTScopedComPtr<IDWriteGlyphRunAnalysis> glyphRunAnalysis;
    {
        ...
        if (typeface->fFactory2 && (fGridFitMode == DWRITE_GRID_FIT_MODE_DISABLED || fAntiAliasMode == DWRITE_TEXT_ANTIALIAS_MODE_GRAYSCALE)) {
            HRBM(typeface->fFactory2->CreateGlyphRunAnalysis(&run, &fXform, renderingMode, fMeasuringMode, fGridFitMode, fAntiAliasMode,
    ...
    {
        Shared l(maybe_dw_mutex(*typeface));
        HRBM(glyphRunAnalysis->GetAlphaTextureBounds(textureType, &bbox), "Could not get texture bounds.");
    }
...
bool SkScalerContext_DW::getColorGlyphRun(const SkGlyph& glyph, IDWriteColorGlyphRunEnumerator** colorGlyph)
{
    FLOAT advance = 0;
    UINT16 glyphId = glyph.getGlyphID();
    ...
    HRESULT hr = this->getDWriteTypeface()->fFactory2->TranslateColorGlyphRun(0, 0, &run, nullptr, fMeasuringMode, &fXform, 0, colorGlyph);
...
bool SkScalerContext_DW::generateColorMetrics(const SkGlyph& glyph, SkRect* bounds)
{
    ...
    *bounds = SkRect::MakeEmpty();
    ...
    while (SUCCEEDED(colorLayers->MoveNext(&hasNextRun)) && hasNextRun) {
        ...
        {
            Exclusive l(maybe_dw_mutex(*this->getDWriteTypeface()));
            HRBM(colorGlyph->glyphRun.fontFace->GetGlyphRunOutline(colorGlyph->glyphRun.fontEmSize, colorGlyph->glyphRun.glyphIndices,
                     colorGlyph->glyphRun.glyphAdvances, colorGlyph->glyphRun.glyphOffsets, colorGlyph->glyphRun.glyphCount, colorGlyph->glyphRun.isSideways,
...
bool SkScalerContext_DW::generateSVGMetrics(const SkGlyph& glyph, SkRect* bounds)
{
    ...
    SkRect infiniteRect = SkRect::MakeLTRB(-SK_ScalarInfinity, -SK_ScalarInfinity, SK_ScalarInfinity, SK_ScalarInfinity);
...
bool SkScalerContext_DW::generatePngMetrics(const SkGlyph& glyph, SkRect* bounds)
{
    ...
    HRBM(fontFace4->GetGlyphImageFormats(glyph.getGlyphID(), 0, UINT32_MAX, &imageFormats), "Cannot get glyph image formats.");
    ...
    void* glyphDataContext;
    HRBM(fontFace4->GetGlyphImageData(glyph.getGlyphID(), fTextSizeRender, DWRITE_GLYPH_IMAGE_FORMATS_PNG, &glyphData, &glyphDataContext),
        "Glyph image data could not be acquired.");

    Context* context = new Context(fontFace4, glyphDataContext);
    sk_sp<SkData> data = SkData::MakeWithProc(glyphData.imageData, glyphData.imageDataSize, &ReleaseProc, context);
...
SkScalerContext::GlyphMetrics SkScalerContext_DW::generateMetrics(const SkGlyph& glyph, SkArenaAlloc* alloc)
{
    ...
    if (DWRITE_TEXTURE_ALIASED_1x1 != fTextureType || DWRITE_TEXT_ANTIALIAS_MODE_GRAYSCALE == fAntiAliasMode) {
        if (this->generateDWMetrics(glyph, DWRITE_RENDERING_MODE_ALIASED, DWRITE_TEXTURE_ALIASED_1x1, &mx.bounds)) {
...
void SkScalerContext_DW::BilevelToBW(const uint8_t* SK_RESTRICT src, const SkGlyph& glyph, void* imageBuffer)
{
...
void SkScalerContext_DW::GrayscaleToA8(const uint8_t* SK_RESTRICT src, const SkGlyph& glyph, void* imageBuffer, const uint8_t* table8)
{
...
template <bool APPLY_PREBLEND> void SkScalerContext_DW::RGBToA8(const uint8_t* SK_RESTRICT src, const SkGlyph& glyph, void* imageBuffer, const uint8_t* table8)
{
    const size_t dstRB = glyph.rowBytes();
    const int width = glyph.width();
    uint8_t* SK_RESTRICT dst = static_cast<uint8_t*>(imageBuffer);
...
void SkScalerContext_DW::RGBToLcd16(
    const uint8_t* SK_RESTRICT src, const SkGlyph& glyph, void* imageBuffer, const uint8_t* tableR, const uint8_t* tableG, const uint8_t* tableB)
{
    const size_t dstRB = glyph.rowBytes();
    const int width = glyph.width();
...
const void* SkScalerContext_DW::getDWMaskBits(const SkGlyph& glyph, DWRITE_RENDERING_MODE renderingMode, DWRITE_TEXTURE_TYPE textureType)
{
    DWriteFontTypeface* typeface = this->getDWriteTypeface();
    ...
    run.glyphOffsets = &offset;
    {
        SkTScopedComPtr<IDWriteGlyphRunAnalysis> glyphRunAnalysis;
        {
            ...
            // IDWriteFactory2::CreateGlyphRunAnalysis is very bad at aliased glyphs.
            if (typeface->fFactory2 && (fGridFitMode == DWRITE_GRID_FIT_MODE_DISABLED || fAntiAliasMode == DWRITE_TEXT_ANTIALIAS_MODE_GRAYSCALE)) {
                HRNM(typeface->fFactory2->CreateGlyphRunAnalysis(&run, &fXform, renderingMode, fMeasuringMode, fGridFitMode, fAntiAliasMode,
                         0.0f, // baselineOriginX,
                         0.0f, // baselineOriginY,
                         &glyphRunAnalysis),
                    "Could not create DW2 glyph run analysis.");
        ...
        {
            Shared l(maybe_dw_mutex(*typeface));
            HRNM(glyphRunAnalysis->CreateAlphaTexture(textureType, &bbox, fBits.begin(), sizeNeeded), "Could not draw mask.");
        }
    }
    return fBits.begin();
}
    ...
    if (DWRITE_RENDERING_MODE_ALIASED == renderingMode) {
        ...
        if (fPreBlend.isApplicable()) {
            if (fRec.fFlags & SkScalerContext::kLCD_BGROrder_Flag) {
                RGBToLcd16<true, false>(src, glyph, imageBuffer, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
            } else {
                RGBToLcd16<true, true>(src, glyph, imageBuffer, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
            }
        } else {
            if (fRec.fFlags & SkScalerContext::kLCD_BGROrder_Flag) {
                RGBToLcd16<false, false>(src, glyph, imageBuffer, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
            } else {
                RGBToLcd16<false, true>(src, glyph, imageBuffer, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
            }
        }
    }
    return true;
...
bool SkScalerContext_DW::drawSVGImage(const SkGlyph& glyph, SkCanvas& canvas)
{
    ...
    DWRITE_GLYPH_IMAGE_DATA glyphData;
    void* glyphDataContext;
    HRBM(fontFace4->GetGlyphImageData(glyph.getGlyphID(), fTextSizeRender, DWRITE_GLYPH_IMAGE_FORMATS_SVG, &glyphData, &glyphDataContext),
    ...
    return svgDecoder->render(canvas, upem, glyph.getGlyphID(), fRec.fForegroundColor, SkSpan(palette, paletteEntryCount));
}
...
{
    ScalerContextBits::value_type format = glyph.extraBits();
    if (format == ScalerContextBits::DW || format == ScalerContextBits::DW_1) {
...
sk_sp<SkDrawable> SkScalerContext_DW::generateDrawable(const SkGlyph& glyph)
{
    struct GlyphDrawable : public SkDrawable {
        ...
        SkGlyph fGlyph;
        GlyphDrawable(SkScalerContext_DW* self, const SkGlyph& glyph)
            : fSelf(self)
            , fGlyph(glyph)
        {
        }
