...
struct FamilyData {
    FamilyData(
        XML_Parser parser, SkTDArray<FontFamily*>& families, const SkString& basePath, bool isFallback, const char* filename, const TagHandler* topLevelHandler)
        : fParser(parser)
        , fFamilies(families)
                ...
                if (valueLen == 4) {
                    ...
                    for (int j = 0; j < file.fVariationDesignPosition.size() - 1; ++j) {
                        if (file.fVariationDesignPosition[j].axis == axisTag) {
                            ...
                            SK_FONTCONFIGPARSER_WARNING("'%c%c%c%c' axis specified more than once", (char)((axisTag >> 24) & 0xFF),
                                (char)((axisTag >> 16) & 0xFF), (char)((axisTag >> 8) & 0xFF), (char)((axisTag)&0xFF));
                        }
                    }
                } else {
...
static const TagHandler fontHandler = {
    /*start*/ [](FamilyData* self, const char* tag, const char** attributes) {
        ...
        if (!fallbackFor.isEmpty()) {
            std::unique_ptr<FontFamily>* fallbackFamily = self->fCurrentFamily->fallbackFamilies.find(fallbackFor);
            if (!fallbackFamily) {
                std::unique_ptr<FontFamily> newFallbackFamily(new FontFamily(self->fCurrentFamily->fBasePath, true));
                fallbackFamily = self->fCurrentFamily->fallbackFamilies.set(fallbackFor, std::move(newFallbackFamily));
...
static void XMLCALL xml_entity_decl_handler(void* data, const XML_Char* entityName, int is_parameter_entity, const XML_Char* value, int value_length,
...
static int parse_config_file(const char* filename, SkTDArray<FontFamily*>& families, const SkString& basePath, bool isFallback)
{
    ...
    SkAutoTCallVProc<std::remove_pointer_t<XML_Parser>, XML_ParserFree> parser(XML_ParserCreate_MM(nullptr, &sk_XML_alloc, nullptr));
    if (!parser) {
...
static void append_fallback_font_families_for_locale(SkTDArray<FontFamily*>& fallbackFonts, const char* dir, const SkString& basePath)
{
    ...
    while (iter.next(&fileName, false)) {
        ...
        static const size_t fixedLen = sizeof(LOCALE_FALLBACK_FONTS_PREFIX) - 1 + sizeof(LOCALE_FALLBACK_FONTS_SUFFIX) - 1;
        ...
        SkString locale(fileName.c_str() + sizeof(LOCALE_FALLBACK_FONTS_PREFIX) - 1, fileName.size() - fixedLen);
...
static void append_system_fallback_font_families(SkTDArray<FontFamily*>& fallbackFonts, const SkString& basePath)
{
    ...
    append_fallback_font_families_for_locale(fallbackFonts, LOCALE_FALLBACK_FONTS_SYSTEM_DIR, basePath);
}
...
void SkFontMgr_Android_Parser::GetCustomFontFamilies(
    SkTDArray<FontFamily*>& fontFamilies, const SkString& basePath, const char* fontsXml, const char* fallbackFontsXml, const char* langFallbackFontsDir)
{
    if (fontsXml) {
