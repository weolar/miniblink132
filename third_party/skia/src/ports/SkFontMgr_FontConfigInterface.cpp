...
std::unique_ptr<SkFontData> SkTypeface_FCI::onMakeFontData() const
{
    ...
    return std::make_unique<SkFontData>(std::unique_ptr<SkStreamAsset>(fFCI->openStream(id)), id.fTTCIndex, 0, nullptr, 0, nullptr, 0);
}
...
private:
    struct Result : public SkResourceCache::Rec {
        ...
        {
            return fRequest->size() + sizeof(fFace);
        }
...
protected:
    ...
    sk_sp<SkTypeface> onMatchFamilyStyle(const char requestedFamilyName[], const SkFontStyle& requestedStyle) const override
    {
        ...
        if (!fFCI->matchFamilyName(requestedFamilyName, requestedStyle, &identity, &outFamilyName, &outStyle)) {
            return nullptr;
        }
