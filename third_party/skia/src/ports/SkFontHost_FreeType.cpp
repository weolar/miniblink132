...
#if defined(SK_BUILD_FOR_ANDROID_FRAMEWORK) || defined(SK_BUILD_FOR_GOOGLE3) || defined(SK_FREETYPE_MINIMUM_RUNTIME_VERSION_IS_BUILD_VERSION)
...
static_assert(std::is_same<FT_Alloc_size_t, long>::value || std::is_same<FT_Alloc_size_t, size_t>::value, "");
...
void SkTypeface_FreeType::FaceRec::setupPalette(const SkFontData& data)
{
    ...
    if (SkTFitsIn<FT_UShort>(data.getPaletteIndex()) && SkTo<FT_UShort>(data.getPaletteIndex()) < paletteData.num_palettes) {
    ...
    for (int i = 0; i < fFTPaletteEntryCount; ++i) {
        fSkPalette[i] = SkColorSetARGB(ftPalette[i].alpha, ftPalette[i].red, ftPalette[i].green, ftPalette[i].blue);
    }
#endif
}
...
public:
    ...
    FT_Face face()
    {
        return fFaceRec ? fFaceRec->fFace.get() : nullptr;
    }
...
public:
    SkScalerContext_FreeType(sk_sp<SkTypeface_FreeType>, const SkScalerContextEffects&, const SkDescriptor* desc, sk_sp<SkTypeface>);
...
static bool is_opentype_font_data_standard_format(const SkTypeface& typeface)
{
    ...
    return otTag == SkSFNTHeader::fontType_WindowsTrueType::TAG || otTag == SkSFNTHeader::fontType_MacTrueType::TAG
        || otTag == SkSFNTHeader::fontType_PostScript::TAG || otTag == SkSFNTHeader::fontType_OpenTypeCFF::TAG || otTag == SkTTCFHeader::TAG;
}
...
std::unique_ptr<SkAdvancedTypefaceMetrics> SkTypeface_FreeType::onGetAdvancedMetrics() const
{
    ...
    if ((info->fType == SkAdvancedTypefaceMetrics::kTrueType_Font || info->fType == SkAdvancedTypefaceMetrics::kCFF_Font)
    ...
    info->fBBox = SkIRect::MakeLTRB(face->bbox.xMin, face->bbox.yMax, face->bbox.xMax, face->bbox.yMin);
    return info;
}
...
std::unique_ptr<SkScalerContext> SkTypeface_FreeType::onCreateScalerContext(const SkScalerContextEffects& effects, const SkDescriptor* desc) const
{
    ...
    const SkScalerContextEffects& effects, const SkDescriptor* desc, sk_sp<SkTypeface> realTypeface) const
{
    auto c = std::make_unique<SkScalerContext_FreeType>(
        sk_ref_sp(const_cast<SkTypeface_FreeType*>(this)), effects, desc, realTypeface ? realTypeface : sk_ref_sp(const_cast<SkTypeface_FreeType*>(this)));
    if (c->success()) {
        return c;
    }
    return SkScalerContext::MakeEmpty(sk_ref_sp(const_cast<SkTypeface_FreeType*>(this)), effects, desc);
}
...
std::unique_ptr<SkFontData> SkTypeface_FreeType::cloneFontData(const SkFontArguments& args, SkFontStyle* style) const
{
    AutoFTAccess fta(this);
    FT_Face face = fta.face();
    if (!face) {
    ...
    SkFontScanner_FreeType::computeAxisValues(
        axisDefinitions, args.getVariationDesignPosition(), axisValues, name, style, currentAxisCount == axisCount ? currentPosition.get() : nullptr);

    int ttcIndex;
    std::unique_ptr<SkStreamAsset> stream = this->openStream(&ttcIndex);

    return std::make_unique<SkFontData>(
        std::move(stream), ttcIndex, args.getPalette().index, axisValues.get(), axisCount, args.getPalette().overrides, args.getPalette().overrideCount);
}
...
bool SkTypeface_FreeType::onGetKerningPairAdjustments(const uint16_t glyphs[], int count, int32_t adjustments[]) const
{
    AutoFTAccess fta(this);
    FT_Face face = fta.face();
...
SkScalerContext_FreeType::SkScalerContext_FreeType(
    sk_sp<SkTypeface_FreeType> proxyTypeface, const SkScalerContextEffects& effects, const SkDescriptor* desc, sk_sp<SkTypeface> realTypeface)
    ...
    , fStrikeIndex(-1)
{
    ...
    if (FT_IS_SCALABLE(fFaceRec->fFace)) {
        ...
        if (err != 0) {
            SK_TRACEFTR(err, "FT_Select_Size(%s, %d) failed.", fFaceRec->fFace->family_name, fStrikeIndex);
            fStrikeIndex = -1;
            return;
        }
        ...
        fMatrix22Scalar.preScale(fScale.x() / fFaceRec->fFace->size->metrics.x_ppem, fScale.y() / fFaceRec->fFace->size->metrics.y_ppem);
...
bool SkScalerContext_FreeType::getBoundsOfCurrentOutlineGlyph(FT_GlyphSlot glyph, SkRect* bounds)
{
    ...
    *bounds = SkRect::MakeLTRB(SkFDot6ToScalar(bbox.xMin), -SkFDot6ToScalar(bbox.yMax), SkFDot6ToScalar(bbox.xMax), -SkFDot6ToScalar(bbox.yMin));
    return true;
}
...
void SkScalerContext_FreeType::updateGlyphBoundsIfSubpixel(const SkGlyph& glyph, SkRect* bounds, bool subpixel)
{
    if (subpixel && !bounds->isEmpty()) {
        bounds->offset(SkFixedToScalar(glyph.getSubXFixed()), SkFixedToScalar(glyph.getSubYFixed()));
    }
}
...
bool SkScalerContext_FreeType::shouldSubpixelBitmap(const SkGlyph& glyph, const SkMatrix& matrix)
{
    ...
    bool mechanism = fFace->glyph->format == FT_GLYPH_FORMAT_BITMAP && this->isSubpixel() && (glyph.getSubXFixed() || glyph.getSubYFixed());
...
SkScalerContext::GlyphMetrics SkScalerContext_FreeType::generateMetrics(const SkGlyph& glyph, SkArenaAlloc* alloc)
{
    ...
    if (FT_IS_SCALABLE(fFace)) {
        ...
        if (FT_Get_Color_Glyph_Paint(fFace, glyph.getGlyphID(), FT_COLOR_INCLUDE_ROOT_TRANSFORM, &opaqueLayerPaint)) {
            haveLayers = true;
            ...
            if (FT_Get_Color_Glyph_ClipBox(fFace, glyph.getGlyphID(), &clipBox)) {
                ...
                if (!SkScalerContextFTUtils::computeColrV1GlyphBoundingBox(fFace, glyph.getGlyphID(), &bounds) || this->setupSize()) {
                    return mx;
                }
            }
        }
        ...
        if (!haveLayers) {
            ...
            while (FT_Get_Color_Glyph_Layer(fFace, glyph.getGlyphID(), &layerGlyphIndex, &layerColorIndex, &layerIterator)) {
                haveLayers = true;
    ...
    if (!haveLayers) {
        ...
        if (fFace->glyph->format == FT_GLYPH_FORMAT_OUTLINE) {
            ...
            mx.bounds = SkRect::MakeXYWH(SkIntToScalar(fFace->glyph->bitmap_left), -SkIntToScalar(fFace->glyph->bitmap_top),
                SkIntToScalar(fFace->glyph->bitmap.width), SkIntToScalar(fFace->glyph->bitmap.rows));
            fMatrix22Scalar.mapRect(&mx.bounds);
            updateGlyphBoundsIfSubpixel(glyph, &mx.bounds, this->shouldSubpixelBitmap(glyph, fMatrix22Scalar));
...
void SkScalerContext_FreeType::generateImage(const SkGlyph& glyph, void* imageBuffer)
{
    ...
    if (glyph.extraBits() == ScalerContextBits::COLRv0 || glyph.extraBits() == ScalerContextBits::COLRv1 || glyph.extraBits() == ScalerContextBits::SVG) {
    ...
    if (this->shouldSubpixelBitmap(glyph, *bitmapMatrix)) {
        ...
        subpixelBitmapMatrix.postTranslate(SkFixedToScalar(glyph.getSubXFixed()), SkFixedToScalar(glyph.getSubYFixed()));
...
void SkTypeface_FreeType::onCharsToGlyphs(const SkUnichar uni[], int count, SkGlyphID glyphs[]) const
{
...
int SkTypeface_FreeType::onGetVariationDesignPosition(SkFontArguments::VariationPosition::Coordinate coordinates[], int coordinateCount) const
{
    AutoFTAccess fta(this);
    return GetVariationDesignPosition(fta, coordinates, coordinateCount);
}

int SkTypeface_FreeType::onGetVariationDesignParameters(SkFontParameters::Variation::Axis parameters[], int parameterCount) const
{
    AutoFTAccess fta(this);
    FT_Face face = fta.face();
    if (!face) {
    ...
    for (FT_UInt i = 0; i < variations->num_axis; ++i) {
        ...
        FT_UInt flags = 0;
        bool hidden = !FT_Get_Var_Axis_Flags(variations, i, &flags) && (flags & FT_VAR_AXIS_FLAG_HIDDEN);
...
size_t SkTypeface_FreeType::onGetTableData(SkFontTableTag tag, size_t offset, size_t length, void* data) const
{
    AutoFTAccess fta(this);
    FT_Face face = fta.face();
    if (!face) {
...
sk_sp<SkData> SkTypeface_FreeType::onCopyTableData(SkFontTableTag tag) const
{
    ...
    if (data) {
        error = FT_Load_Sfnt_Table(face, tag, 0, reinterpret_cast<FT_Byte*>(data->writable_data()), &tableLength);
        if (error) {
            data.reset();
        }
    }
...
void SkTypeface_FreeType::FontDataPaletteToDescriptorPalette(const SkFontData& fontData, SkFontDescriptor* desc)
{
...
SkTypeface_FreeTypeStream::SkTypeface_FreeTypeStream(
    std::unique_ptr<SkFontData> fontData, const SkString familyName, const SkFontStyle& style, bool isFixedPitch)
...
sk_sp<SkTypeface> SkTypeface_FreeTypeStream::onMakeClone(const SkFontArguments& args) const
{
    ...
    return sk_make_sp<SkTypeface_FreeTypeStream>(std::move(data), familyName, style, this->isFixedPitch());
}
...
sk_sp<SkTypeface> SkTypeface_FreeType::MakeFromStream(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args)
{
    ...
    SkString name;
    SkFontScanner::AxisDefinitions axisDefinitions;
    if (!scanner.scanInstance(stream.get(), args.getCollectionIndex(), 0, &name, &style, &isFixedPitch, &axisDefinitions)) {
        return nullptr;
    }
    ...
    auto data = std::make_unique<SkFontData>(std::move(stream), args.getCollectionIndex(), args.getPalette().index, axisValues.get(), axisDefinitions.size(),
...
FT_Face SkFontScanner_FreeType::openFace(SkStreamAsset* stream, int ttcIndex, FT_Stream ftStream) const
{
...
bool SkFontScanner_FreeType::scanFace(SkStreamAsset* stream, int faceIndex, int* numInstances) const
{
    SkAutoMutexExclusive libraryLock(fLibraryMutex);
...
bool SkFontScanner_FreeType::scanInstance(
    SkStreamAsset* stream, int faceIndex, int instanceIndex, SkString* name, SkFontStyle* style, bool* isFixedPitch, AxisDefinitions* axes) const
{
    ...
    if (hasAxes) {
        ...
        if (GetAxes(face.get(), &axisDefinitions)) {
            ...
            if ((wghtIndex || wdthIndex || slntIndex) && !FT_Get_Var_Design_Coordinates(face.get(), numAxes, coords.get())) {
                if (wghtIndex) {
    ...
    if (!hasOs2 && !hasAxes && 0 == FT_Get_PS_Font_Info(face.get(), &psFontInfo) && psFontInfo.weight) {
        ...
        int const index = SkStrLCSearch(&commonWeights[0].name, std::size(commonWeights), psFontInfo.weight, sizeof(commonWeights[0]));
        if (index >= 0) {
...
sk_sp<SkTypeface> SkFontScanner_FreeType::MakeFromStream(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args) const
{
...
/*static*/ void SkFontScanner_FreeType::computeAxisValues(AxisDefinitions axisDefinitions, const SkFontArguments::VariationPosition position,
    SkFixed* axisValues, const SkString& name, SkFontStyle* style, const SkFontArguments::VariationPosition::Coordinate* current)
{
    static constexpr SkFourByteTag wghtTag = SkSetFourByteTag('w', 'g', 'h', 't');
    static constexpr SkFourByteTag wdthTag = SkSetFourByteTag('w', 'd', 't', 'h');
    static constexpr SkFourByteTag slntTag = SkSetFourByteTag('s', 'l', 'n', 't');
            ...
            if (axisDefinition.fTag == coordinate.axis) {
                ...
                if (coordinate.value != axisValue) {
                    LOG_INFO("Requested font axis value out of range: "
                        ...
                        name.c_str(), (axisDefinition.fTag >> 24) & 0xFF, (axisDefinition.fTag >> 16) & 0xFF, (axisDefinition.fTag >> 8) & 0xFF,
                        (axisDefinition.fTag) & 0xFF, SkScalarToDouble(coordinate.value), SkScalarToDouble(axisValue));
                }
                axisValues[i] = SkScalarToFixed(axisValue);
                break;
            }
    ...
    SkDEBUGCODE(
        ...
        for (int i = 0; i < position.coordinateCount; ++i) {
            ...
            if (!found) {
                LOG_INFO("Requested font axis not found: %s '%c%c%c%c'\n", name.c_str(), (skTag >> 24) & 0xFF, (skTag >> 16) & 0xFF, (skTag >> 8) & 0xFF,
                    (skTag)&0xFF);
            }
        })
}
