...
bool HasBitmapStrikes(const SkTScopedComPtr<IDWriteFont>& font)
{
    ...
    AutoDWriteTable ebdtTable(fontFace.get(), SkEndian_SwapBE32(SkSetFourByteTag('E', 'B', 'D', 'T')));
...
HRESULT FirstMatchingFontWithoutSimulations(const SkTScopedComPtr<IDWriteFontFamily>& family, DWriteStyle dwStyle, SkTScopedComPtr<IDWriteFont>& font)
{
    ...
    while (!noSimulations) {
        ...
        noSimulations = simulations == DWRITE_FONT_SIMULATIONS_NONE
            || (dwStyle.fWeight == DWRITE_FONT_WEIGHT_REGULAR && dwStyle.fSlant == DWRITE_FONT_STYLE_NORMAL) || HasBitmapStrikes(searchFont);
#else
        noSimulations = true;
#endif
        if (noSimulations) {
...
public:
    ...
    SkFontMgr_DirectWrite(IDWriteFactory* factory, IDWriteFontCollection* fontCollection, IDWriteFontFallback* fallback, const WCHAR* localeName,
        int localeNameLength, const WCHAR* defaultFamilyName, int defaultFamilyNameLength)
...
protected:
    ...
    sk_sp<SkTypeface> onMatchFamilyStyleCharacter(
        const char familyName[], const SkFontStyle&, const char* bcp47[], int bcp47Count, SkUnichar character) const override;
...
private:
    ...
    sk_sp<SkTypeface> fallback(const WCHAR* dwFamilyName, DWriteStyle, const WCHAR* dwBcp47, UINT32 character) const;
    sk_sp<SkTypeface> layoutFallback(const WCHAR* dwFamilyName, DWriteStyle, const WCHAR* dwBcp47, UINT32 character) const;

    /** Creates a typeface using a typeface cache. */
    sk_sp<SkTypeface> makeTypefaceFromDWriteFont(IDWriteFontFace* fontFace, IDWriteFont* font, IDWriteFontFamily* fontFamily) const;
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::makeTypefaceFromDWriteFont(IDWriteFontFace* fontFace, IDWriteFont* font, IDWriteFontFamily* fontFamily) const
{
    ...
    if (nullptr == face) {
        face = DWriteFontTypeface::Make(fFactory.get(), fontFace, font, fontFamily, nullptr, SkFontArguments::Palette { 0, nullptr, 0 });
        if (face) {
            fTFCache.add(face);
        }
    }
...
sk_sp<SkFontStyleSet> SkFontMgr_DirectWrite::onMatchFamily(const char familyName[]) const
{
    ...
    UINT32 index;
    BOOL exists;
    HRNM(fFontCollection->FindFamilyName(dwFamilyName.get(), &index, &exists), "Failed while finding family by name.");
    if (!exists) {
        return nullptr;
    }
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::onMatchFamilyStyle(const char familyName[], const SkFontStyle& fontstyle) const
{
...
public:
    ...
    SK_STDMETHODIMP DrawGlyphRun(void* clientDrawingContext, FLOAT baselineOriginX, FLOAT baselineOriginY, DWRITE_MEASURING_MODE measuringMode,
        DWRITE_GLYPH_RUN const* glyphRun, DWRITE_GLYPH_RUN_DESCRIPTION const* glyphRunDescription, IUnknown* clientDrawingEffect) override
    {
        ...
        if (exists) {
            ...
            fResolvedTypeface = fOuter->makeTypefaceFromDWriteFont(glyphRun->fontFace, font.get(), fontFamily.get());
            fHasSimulations = (font->GetSimulations() != DWRITE_FONT_SIMULATIONS_NONE) && !HasBitmapStrikes(font);
        }
    ...
    SK_STDMETHODIMP DrawUnderline(
        void* clientDrawingContext, FLOAT baselineOriginX, FLOAT baselineOriginY, DWRITE_UNDERLINE const* underline, IUnknown* clientDrawingEffect) override
    {
        return E_NOTIMPL;
    }

    SK_STDMETHODIMP DrawStrikethrough(void* clientDrawingContext, FLOAT baselineOriginX, FLOAT baselineOriginY, DWRITE_STRIKETHROUGH const* strikethrough,
        IUnknown* clientDrawingEffect) override
    {
        return E_NOTIMPL;
    }

    SK_STDMETHODIMP DrawInlineObject(void* clientDrawingContext, FLOAT originX, FLOAT originY, IDWriteInlineObject* inlineObject, BOOL isSideways,
    ...
    SK_STDMETHODIMP GetCurrentTransform(void* clientDrawingContext, DWRITE_MATRIX* transform) override
    {
    ...
    sk_sp<SkTypeface> ConsumeFallbackTypeface()
    {
...
public:
    FontFallbackSource(const WCHAR* string, UINT32 length, const WCHAR* locale, IDWriteNumberSubstitution* numberSubstitution)
        : fRefCount(1)
        , fString(string)
        , fLength(length)
        , fLocale(locale)
    ...
    SK_STDMETHODIMP GetTextAtPosition(UINT32 textPosition, WCHAR const** textString, UINT32* textLength) override
    {
    ...
    SK_STDMETHODIMP GetTextBeforePosition(UINT32 textPosition, WCHAR const** textString, UINT32* textLength) override
    {
    ...
    SK_STDMETHODIMP GetLocaleName(UINT32 textPosition, UINT32* textLength, WCHAR const** localeName) override
    {
        *localeName = fLocale;
        return S_OK;
    }

    SK_STDMETHODIMP GetNumberSubstitution(UINT32 textPosition, UINT32* textLength, IDWriteNumberSubstitution** numberSubstitution) override
    {
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::fallback(const WCHAR* dwFamilyName, DWriteStyle dwStyle, const WCHAR* dwBcp47, UINT32 character) const
{
    WCHAR str[16];
    ...
    HRNM(fFactory->CreateNumberSubstitution(DWRITE_NUMBER_SUBSTITUTION_METHOD_NONE, dwBcp47, TRUE, &numberSubstitution),
        "Could not create number substitution.");
    SkTScopedComPtr<FontFallbackSource> fontFallbackSource(new FontFallbackSource(str, strLen, dwBcp47, numberSubstitution.get()));
    ...
    while (!noSimulations) {
        ...
        HRNM(fFontFallback->MapCharacters(fontFallbackSource.get(),
                 0, // textPosition,
                 strLen, fFontCollection.get(), dwFamilyName, dwStyle.fWeight, dwStyle.fSlant, dwStyle.fWidth, &mappedLength, &font, &scale),
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::layoutFallback(const WCHAR* dwFamilyName, DWriteStyle dwStyle, const WCHAR* dwBcp47, UINT32 character) const
{
    WCHAR str[16];
    UINT32 strLen = SkTo<UINT32>(SkUTF::ToUTF16(character, reinterpret_cast<uint16_t*>(str)));
    ...
    while (!noSimulations) {
        ...
        HRNM(fFactory->CreateTextFormat(
                 dwFamilyName ? dwFamilyName : L"", fFontCollection.get(), dwStyle.fWeight, dwStyle.fSlant, dwStyle.fWidth, 72.0f, dwBcp47, &fallbackFormat),
        ...
        HRNM(fFactory->CreateTextLayout(str, strLen, fallbackFormat.get(), 200.0f, 200.0f, &fallbackLayout), "Could not create text layout.");
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::onMakeFromStreamIndex(std::unique_ptr<SkStreamAsset> stream, int ttcIndex) const
{
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::onMakeFromStreamArgs(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args) const
{
...
HRESULT SkFontMgr_DirectWrite::getByFamilyName(const WCHAR wideFamilyName[], IDWriteFontFamily** fontFamily) const
{
    UINT32 index;
    BOOL exists;
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::onLegacyMakeTypeface(const char familyName[], SkFontStyle style) const
{
    SkTScopedComPtr<IDWriteFontFamily> fontFamily;
    DWriteStyle dwStyle(style);
    if (familyName) {
...
sk_sp<SkTypeface> SkFontStyleSet_DirectWrite::matchStyle(const SkFontStyle& pattern)
{
    ...
    HRNM(FirstMatchingFontWithoutSimulations(fFontFamily, dwStyle, font), "No font found from family.");
...
sk_sp<SkFontMgr> SkFontMgr_New_DirectWrite(IDWriteFactory* factory, IDWriteFontCollection* collection)
{
    return SkFontMgr_New_DirectWrite(factory, collection, nullptr);
}

sk_sp<SkFontMgr> SkFontMgr_New_DirectWrite(IDWriteFactory* factory, IDWriteFontCollection* collection, IDWriteFontFallback* fallback)
{
    if (nullptr == factory) {
    ...
    return sk_make_sp<SkFontMgr_DirectWrite>(factory, collection, fallback, localeName, localeNameLen, defaultFamilyName, defaultFamilyNameLen);
}
