...
{
    return SkFontStyle(lf.lfWeight, SkFontStyle::kNormal_Width, lf.lfItalic ? SkFontStyle::kItalic_Slant : SkFontStyle::kUpright_Slant);
}
...
static unsigned calculateGlyphCount(HDC hdc, const LOGFONT& lf)
{
    ...
    while (min < max) {
        ...
        if (GetGlyphOutlineW(hdc, mid, GGO_METRICS | GGO_GLYPH_INDEX, &gm, 0, nullptr, &mat2) == GDI_ERROR) {
            max = mid;
        } else {
            min = mid + 1;
        }
...
public:
    ...
    {
        return sk_sp<FontMemResourceTypeface>(new FontMemResourceTypeface(get_style(lf), lf, fontMemResource));
    }
...
static void populate_glyph_to_unicode(HDC fontHdc, const unsigned glyphCount, SkUnichar* glyphToUnicode)
{
...
public:
    SkScalerContext_GDI(sk_sp<LogFontTypeface>, const SkScalerContextEffects&, const SkDescriptor* desc);
...
private:
    DWORD getGDIGlyphPath(SkGlyphID glyph, UINT flags, AutoSTMalloc<BUFFERSIZE, uint8_t>* glyphbuf);
    template <bool APPLY_PREBLEND>
    static void RGBToA8(const SkGdiRGB* SK_RESTRICT src, size_t srcRB, const SkGlyph& glyph, void* imageBuffer, const uint8_t* table8);

    template <bool APPLY_PREBLEND>
    static void RGBToLcd16(const SkGdiRGB* SK_RESTRICT src, size_t srcRB, const SkGlyph& glyph, void* imageBuffer, const uint8_t* tableR, const uint8_t* tableG,
...
SkScalerContext_GDI::SkScalerContext_GDI(sk_sp<LogFontTypeface> rawTypeface, const SkScalerContextEffects& effects, const SkDescriptor* desc)
    ...
    , fSC(nullptr)
{
    ...
    SkScalerContextRec::PreMatrixScale scaleConstraints = (fRec.getHinting() == SkFontHinting::kNone || fRec.getHinting() == SkFontHinting::kSlight)
...
SkScalerContext::GlyphMetrics SkScalerContext_GDI::generateMetrics(const SkGlyph& glyph, SkArenaAlloc*)
{
    SkASSERT(fDDC);
    ...
    if (fType == SkScalerContext_GDI::kBitmap_Type || fType == SkScalerContext_GDI::kLine_Type) {
        ...
        if (fType == SkScalerContext_GDI::kLine_Type) {
            ...
            SkMatrix m;
            m.setAll(SkFIXEDToScalar(fMat22.eM11), -SkFIXEDToScalar(fMat22.eM21), 0, -SkFIXEDToScalar(fMat22.eM12), SkFIXEDToScalar(fMat22.eM22), 0, 0, 0, 1);
            m.mapRect(&bounds);
    ...
    if ((fTM.tmPitchAndFamily & TMPF_VECTOR) && this->isLinearMetrics()) {
        ...
        if (GDI_ERROR != status) {
            mx.advance = fHiResMatrix.mapXY(SkIntToScalar(gm.gmCellIncX), SkIntToScalar(gm.gmCellIncY));
        }
...
template <bool APPLY_PREBLEND> static inline uint8_t rgb_to_a8(SkGdiRGB rgb, const uint8_t* table8)
{
    U8CPU r = (rgb >> 16) & 0xFF;
    U8CPU g = (rgb >> 8) & 0xFF;
    U8CPU b = (rgb >> 0) & 0xFF;
...
template <bool APPLY_PREBLEND> static inline uint16_t rgb_to_lcd16(SkGdiRGB rgb, const uint8_t* tableR, const uint8_t* tableG, const uint8_t* tableB)
{
...
template <bool APPLY_PREBLEND>
void SkScalerContext_GDI::RGBToA8(const SkGdiRGB* SK_RESTRICT src, size_t srcRB, const SkGlyph& glyph, void* imageBuffer, const uint8_t* table8)
{
...
void SkScalerContext_GDI::RGBToLcd16(
    const SkGdiRGB* SK_RESTRICT src, size_t srcRB, const SkGlyph& glyph, void* imageBuffer, const uint8_t* tableR, const uint8_t* tableG, const uint8_t* tableB)
{
    const size_t dstRB = glyph.rowBytes();
    const int width = glyph.width();
...
void SkScalerContext_GDI::generateImage(const SkGlyph& glyph, void* imageBuffer)
{
    ...
    if (isBW) {
        ...
        if (fPreBlend.isApplicable()) {
            RGBToLcd16<true>(src, srcRB, glyph, imageBuffer, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
        } else {
            RGBToLcd16<false>(src, srcRB, glyph, imageBuffer, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
        }
    }
}
...
class SkGDIGeometrySink {
    ...
    {
        return fCurrent.x.value != pt.x.value || fCurrent.x.fract != pt.x.fract || fCurrent.y.value != pt.y.value || fCurrent.y.fract != pt.y.fract;
    }

public:
    SkGDIGeometrySink(SkPath* path)
        : fPath(path)
    {
    }
            ...
            if (pc->wType == TT_PRIM_QSPLINE) {
                for (uint16_t u = 0; u < cpfx - 1; u++) { // Walk through points in spline
                    ...
                    if (this->currentIsNot(pnt_b) || this->currentIsNot(pnt_c)) {
                        this->goingTo(pnt_c);
                        fPath->quadTo(SkFIXEDToScalar(pnt_b.x), -SkFIXEDToScalar(pnt_b.y), SkFIXEDToScalar(pnt_c.x), -SkFIXEDToScalar(pnt_c.y));
                    }
                }
            }
...
bool SkGDIGeometrySink::process(const uint8_t* glyphbuf, DWORD total_size, GDIGlyphbufferPointIter hintedYs)
{
    const uint8_t* cur_glyph = glyphbuf;
    const uint8_t* end_glyph = glyphbuf + total_size;
...
DWORD SkScalerContext_GDI::getGDIGlyphPath(SkGlyphID glyph, UINT flags, AutoSTMalloc<BUFFERSIZE, uint8_t>* glyphbuf)
{
    GLYPHMETRICS gm;
...
bool SkScalerContext_GDI::generatePath(const SkGlyph& glyph, SkPath* path, bool* modified)
{
    ...
    if (fRec.getHinting() != SkFontHinting::kSlight) {
        ...
        //GDI only uses hinted outlines when axis aligned.
        DWORD hinted_total_size = getGDIGlyphPath(glyphID, GGO_NATIVE | GGO_GLYPH_INDEX, &hintedGlyphbuf);
        if (0 == hinted_total_size) {
            return false;
        }

        SkGDIGeometrySink sinkXBufYIter(path);
        if (!sinkXBufYIter.process(glyphbuf, total_size, GDIGlyphbufferPointIter(hintedGlyphbuf, hinted_total_size))) {
...
std::unique_ptr<SkAdvancedTypefaceMetrics> LogFontTypeface::onGetAdvancedMetrics() const
{
    ...
    info->fBBox = SkIRect::MakeLTRB(otm.otmrcFontBox.left, otm.otmrcFontBox.top, otm.otmrcFontBox.right, otm.otmrcFontBox.bottom);
...
static HANDLE activate_font(SkData* fontData)
{
    ...
    HANDLE fontHandle = AddFontMemResourceEx(const_cast<void*>(fontData->data()), static_cast<DWORD>(fontData->size()), nullptr, &numFonts);
...
static void bmpCharsToGlyphs(HDC hdc, const WCHAR* bmpChars, int count, uint16_t* glyphs, bool Ox1FHack)
{
...
static uint16_t nonBmpCharToGlyph(HDC hdc, SCRIPT_CACHE* scriptCache, const WCHAR utf16[2])
{
    ...
    HRZM(ScriptShape(hdc, scriptCache, utf16, numWCHAR, maxGlyphs, &script, outGlyphs, logClust, vsa, &numGlyphs), "Could not shape character.");
    if (1 == numGlyphs) {
        index = outGlyphs[0];
    }
    return index;
...
void LogFontTypeface::onCharsToGlyphs(const SkUnichar* uni, int glyphCount, SkGlyphID glyphs[]) const
{
    SkAutoHDC hdc(fLogFont);
...
size_t LogFontTypeface::onGetTableData(SkFontTableTag tag, size_t offset, size_t length, void* data) const
{
    LOGFONT lf = fLogFont;
    SkAutoHDC hdc(lf);
...
std::unique_ptr<SkScalerContext> LogFontTypeface::onCreateScalerContext(const SkScalerContextEffects& effects, const SkDescriptor* desc) const
{
    auto ctx = std::make_unique<SkScalerContext_GDI>(sk_ref_sp(const_cast<LogFontTypeface*>(this)), effects, desc);
    if (ctx->isValid()) {
        return std::move(ctx);
    }
    ...
    ctx = std::make_unique<SkScalerContext_GDI>(sk_ref_sp(const_cast<LogFontTypeface*>(this)), effects, desc);
    if (ctx->isValid()) {
        return std::move(ctx);
    }
...
void LogFontTypeface::onFilterRec(SkScalerContextRec* rec) const
{
    ...
    unsigned flagsWeDontSupport = SkScalerContext::kForceAutohinting_Flag | SkScalerContext::kEmbeddedBitmapText_Flag | SkScalerContext::kEmbolden_Flag
        | SkScalerContext::kLCD_BGROrder_Flag | SkScalerContext::kLCD_Vertical_Flag;
 ...
 */
static int CALLBACK enum_family_proc(const LOGFONT* lf, const TEXTMETRIC*, DWORD fontType, LPARAM builderParam)
{
...
protected:
    ...
    {
        SkASSERT(index < fLogFontArray.size());
        return sk_sp<SkFontStyleSet>(new SkFontStyleSetGDI(fLogFontArray[index].elfLogFont.lfFaceName));
    }
