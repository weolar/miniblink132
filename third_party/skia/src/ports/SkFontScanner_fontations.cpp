...
bool SkFontScanner_Fontations::scanFace(SkStreamAsset* stream, int faceIndex, int* numInstances) const
{
    sk_sp<SkData> fontData = make_data_avoiding_copy(stream);
    rust::Box<fontations_ffi::BridgeFontRef> fontRef = make_bridge_font_ref(fontData.get(), faceIndex);
    stream->rewind();
...
bool SkFontScanner_Fontations::scanInstance(
...
{
    sk_sp<SkData> fontData = make_data_avoiding_copy(stream);
    rust::Box<fontations_ffi::BridgeFontRef> bridgeFontFaceRef = make_bridge_font_ref(fontData.get(), faceIndex);
    stream->rewind();
    ...
    if (style != nullptr) {
        ...
        if (instanceIndex > num) {
            ...
            rust::Slice<const fontations_ffi::SkiaDesignCoordinate> coordinates;
            rust::Box<fontations_ffi::BridgeNormalizedCoords> normalizedCoords = resolve_into_normalized_coords(*bridgeFontFaceRef, coordinates);
            ...
            std::unique_ptr<SkFontArguments::VariationPosition::Coordinate[]> extractedCoords = nullptr;
            size_t numNamedInstanceCoords = fontations_ffi::coordinates_for_shifted_named_instance_index(
                *bridgeFontFaceRef, instanceIndex << 16, rust::cxxbridge1::Slice<fontations_ffi::SkiaDesignCoordinate>());
            ...
            rust::cxxbridge1::Slice<fontations_ffi::SkiaDesignCoordinate> targetSlice(
                reinterpret_cast<fontations_ffi::SkiaDesignCoordinate*>(extractedCoords.get()), numNamedInstanceCoords);
            size_t retrievedNamedInstanceCoords
                = fontations_ffi::coordinates_for_shifted_named_instance_index(*bridgeFontFaceRef, faceIndex + (instanceIndex << 16), targetSlice);
            ...
            rust::Box<fontations_ffi::BridgeNormalizedCoords> normalizedCoords = make_normalized_coords(*bridgeFontFaceRef, variationPosition);
            fontations_ffi::BridgeFontStyle fontStyle;
            if (fontations_ffi::get_font_style(*bridgeFontFaceRef, *normalizedCoords, fontStyle)) {
    ...
    if (axes != nullptr) {
        rust::Box<fontations_ffi::BridgeFontRef> bridgeFontNamedInstanceRef = make_bridge_font_ref(fontData.get(), faceIndex + (instanceIndex << 16));
        stream->rewind();
...
sk_sp<SkTypeface> SkFontScanner_Fontations::MakeFromStream(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args) const
{
