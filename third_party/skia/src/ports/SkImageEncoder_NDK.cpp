...
static AndroidBitmapInfo info_for_pixmap(const SkPixmap& pmap)
{
    ...
    AndroidBitmapInfo info { .width = SkTFitsIn<uint32_t>(pmap.width()) ? SkToU32(pmap.width()) : 0,
...
static bool write_image_to_stream(SkWStream* stream, const SkPixmap& pmap, AndroidBitmapCompressFormat androidFormat, int quality)
{
    ...
    auto write_to_stream = [](void* userContext, const void* data, size_t size) { return reinterpret_cast<SkWStream*>(userContext)->write(data, size); };

    return ANDROID_BITMAP_RESULT_SUCCESS
        == AndroidBitmap_compress(
            &info, SkNDKConversions::toDataSpace(pmap.colorSpace()), pmap.addr(), androidFormat, quality, reinterpret_cast<void*>(stream), write_to_stream);
}
...
bool Encode(SkWStream* dst, const SkPixmap& src, const Options& options)
{
    if (options.fCompression == Compression::kLossless) {
        return write_image_to_stream(dst, src, ANDROID_BITMAP_COMPRESS_FORMAT_WEBP_LOSSLESS, options.fQuality);
    }
    return write_image_to_stream(dst, src, ANDROID_BITMAP_COMPRESS_FORMAT_WEBP_LOSSY, options.fQuality);
}
