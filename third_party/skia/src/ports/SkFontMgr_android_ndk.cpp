...
static const AndroidFontAPI* GetAndroidFontAPI()
{
    ...
    if constexpr (kSkFontMgrVerbose) {
        SkDebugf("SKIA: GetAndroidFontAPI direct\n");
    }
...
#undef SK_DLSYM_ANDROID_FONT_API
    ...
    if constexpr (kSkFontMgrVerbose) {
        SkDebugf("SKIA: GetAndroidFontAPI dlsym\n");
    }
...
struct SkAFont {
    SkAFont(const AndroidFontAPI& api, AFont* font)
        : fAPI(api)
        , fFont(font)
    {
    }
    SkAFont(SkAFont&& that)
        : fAPI(that.fAPI)
        , fFont(that.fFont)
    {
        that.fFont = nullptr;
    ...
    {
        return fAPI.AFont_getFontFilePath(fFont);
    }
    uint16_t getWeight() const
    {
        return fAPI.AFont_getWeight(fFont);
    }
    bool isItalic() const
    {
        return fAPI.AFont_isItalic(fFont);
    }
    const char* getLocale() const
    {
        return fAPI.AFont_getLocale(fFont);
    }
    size_t getCollectionIndex() const
    {
        return fAPI.AFont_getCollectionIndex(fFont);
    }
    size_t getAxisCount() const
    {
        return fAPI.AFont_getAxisCount(fFont);
    }
    uint32_t getAxisTag(uint32_t index) const
    {
        return fAPI.AFont_getAxisTag(fFont, index);
    }
    float getAxisValue(uint32_t index) const
    {
        return fAPI.AFont_getAxisValue(fFont, index);
    }
...
struct SkASystemFontIterator {
    ...
    {
        if constexpr (kSkFontMgrVerbose) {
            SkDebugf("SKIA: ASystemFontIterator_open\n");
        }
    }
    ...
    {
        if constexpr (kSkFontMgrVerbose) {
            SkDebugf("SKIA: ASystemFontIterator_close\n");
        }
    ...
    SkAFont next()
    {
        return SkAFont(fAPI, fAPI.ASystemFontIterator_next(fIterator));
    }
...
public:
    SkTypeface_AndroidNDK(std::unique_ptr<SkStreamAsset> file, const SkString& pathName, const bool cacheFontFiles, int index, const SkFixed* axes,
        int axesCount, const SkFontStyle& style, bool isFixedPitch, const SkString& familyName, TArray<SkLanguage>&& lang)
    ...
    sk_sp<SkTypeface> onMakeClone(const SkFontArguments& args) const override
    {
        ...
        return sk_sp(new SkTypeface_AndroidNDK(fFile ? fFile->duplicate() : nullptr, fPathName, fCacheFontFiles, fIndex, data->getAxis(), data->getAxisCount(),
            newStyle, this->isFixedPitch(), fFamilyName, TArray<SkLanguage>()));
    }

    sk_sp<SkTypeface_AndroidNDK> makeNamedClone(const SkString& name) const
    {
        return sk_sp(new SkTypeface_AndroidNDK(fFile ? fFile->duplicate() : nullptr, fPathName, fCacheFontFiles, fIndex, fAxes.data(), fAxes.size(),
            this->fontStyle(), this->isFixedPitch(), name, STArray<4, SkLanguage>(fLang)));
    }
...
public:
    ...
    sk_sp<SkTypeface> matchStyle(const SkFontStyle& pattern) override
    {
        ...
        if constexpr (kSkFontMgrVerbose) {
            ...
            SkDebugf("SKIA: Search for [%d, %d, %d] matched %s [%d, %d, %d] %s#%d [%s]\n", pattern.weight(), pattern.width(), pattern.slant(), name.c_str(),
                fontStyle.weight(), fontStyle.width(), fontStyle.slant(), amatch->fPathName.c_str(), amatch->fIndex, axes.c_str());
        }
        return match;
    }
...
class SkFontMgr_AndroidNDK : public SkFontMgr {
    void addSystemTypeface(sk_sp<SkTypeface_AndroidNDK> typeface, const SkString& name)
    {
        ...
        if (!nameToFamily) {
            ...
            nameToFamily = &fNameToFamilyMap.emplace_back(NameToFamily { name, SkString(tolc.lc(), tolc.length()), newSet.get() });
            fStyleSets.push_back(std::move(newSet));
        }
        if constexpr (kSkFontMgrVerbose) {
            SkDebugf("SKIA: Adding member to %s\n", name.c_str());
        }
        nameToFamily->styleSet->fStyles.push_back(typeface);
    }

public:
    SkFontMgr_AndroidNDK(const AndroidFontAPI& androidFontAPI, bool const cacheFontFiles, std::unique_ptr<SkFontScanner> scanner)
        : fAPI(androidFontAPI)
        , fScanner(std::move(scanner))
    {
        ...
        if (!fontIter) {
            if constexpr (kSkFontMgrVerbose) {
                SkDebugf("SKIA: No ASystemFontIterator");
            }
            return;
        }

        if constexpr (kSkFontMgrVerbose) {
            SkDebugf("SKIA: Iterating over AFonts\n");
        }
...
protected:
    ...
    sk_sp<SkTypeface_AndroidNDK> make(SkAFont font, bool cacheFontFiles) const
    {
        ...
        if (!SkTFitsIn<int>(collectionIndex)) {
            if constexpr (kSkFontMgrVerbose) {
                SkDebugf("SKIA: Collection index invalid!");
            }
            return nullptr;
        }
        ...
        if (!fScanner->scanInstance(stream.get(), ttcIndex, 0, &familyName, &style, &isFixedWidth, &axisDefinitions)) {
            if constexpr (kSkFontMgrVerbose) {
        ...
        if (!SkTFitsIn<int>(requestAxisCount)) {
            if constexpr (kSkFontMgrVerbose) {
                SkDebugf("SKIA: Axis count unreasonable!");
            }
            return nullptr;
        }
        ...
        SkFontArguments::VariationPosition position = { requestAxisValues.get(), SkTo<int>(requestAxisCount) };
        SkFontScanner_FreeType::computeAxisValues(axisDefinitions, position, axisValues, familyName, nullptr);
        ...
        return sk_sp<SkTypeface_AndroidNDK>(new SkTypeface_AndroidNDK(std::move(stream), SkString(filePath), cacheFontFiles, ttcIndex, axisValues.get(),
            axisDefinitions.size(), style, isFixedWidth, familyName, std::move(skLangs)));
    }

    static bool has_locale_and_character(SkTypeface_AndroidNDK* face, const SkString& langTag, SkUnichar character, const char* scope, size_t* step)
    {
        ++*step;
        if (!langTag.isEmpty()
            && std::none_of(face->fLang.begin(), face->fLang.end(), [&](SkLanguage lang) { return lang.getTag().startsWith(langTag.c_str()); })) {
            return false;
        }
    ...
    sk_sp<SkTypeface> findByCharacterLocaleFamily(
        SkTypeface_AndroidNDK* familyFace, const SkFontStyle& style, const SkString& langTag, SkUnichar character) const
    {
        size_t step = 0;
    ...
    sk_sp<SkTypeface> onMatchFamilyStyleCharacter(
        const char familyName[], const SkFontStyle& style, const char* bcp47[], int bcp47Count, SkUnichar character) const override
    {
        ...
        for (int bcp47Index = bcp47Count; bcp47Index-- > 0;) {
            ...
            while (!lang.getTag().isEmpty()) {
                sk_sp<SkTypeface> typeface = findByCharacterLocaleFamily(afamilyFace, style, lang.getTag(), character);
                if (typeface) {
                    return typeface;
                }
                lang = lang.getParent();
        ...
        sk_sp<SkTypeface> typeface = findByCharacterLocaleFamily(afamilyFace, style, SkString(), character);
        if (typeface) {
            return typeface;
        }
...
sk_sp<SkFontMgr> SkFontMgr_New_AndroidNDK(bool cacheFontFiles, std::unique_ptr<SkFontScanner> scanner)
{
