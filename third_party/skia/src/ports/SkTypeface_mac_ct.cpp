...
{
    static CFStringRef* kCTFontPaletteAttributePtr = static_cast<CFStringRef*>(dlsym(RTLD_DEFAULT, "kCTFontPaletteAttribute"));
    return *kCTFontPaletteAttributePtr;
}
static CFStringRef getCTFontPaletteColorsAttribute()
{
    static CFStringRef* kCTFontPaletteColorsAttributePtr = static_cast<CFStringRef*>(dlsym(RTLD_DEFAULT, "kCTFontPaletteColorsAttribute"));
...
void SkStringFromCFString(CFStringRef src, SkString* dst)
{
    ...
    CFIndex length = CFStringGetMaximumSizeForEncoding(CFStringGetLength(src), kCFStringEncodingUTF8) + 1;
    dst->resize(length);
...
#define SK_GETCFTYPEID(cf)                                                                                                                                     \
    template <> CFTypeID SkCFGetTypeID<cf##Ref>()                                                                                                              \
    {                                                                                                                                                          \
        return cf##GetTypeID();                                                                                                                                \
    }
 ...
 */
template <typename CF> static bool SkCFDynamicCast(CFTypeRef cf, CF* cfAsCF, char const* name)
{
    ...
    if (CFGetTypeID(cf) != SkCFGetTypeID<CF>()) {
        if (name) {
            SkDEBUGF(
                "%s is a %s but expected a %s\n", name, SkCFTypeIDDescription(CFGetTypeID(cf)).c_str(), SkCFTypeIDDescription(SkCFGetTypeID<CF>()).c_str());
        }
        return false;
    }
...
template <typename T> static bool SkCFNumberDynamicCast(CFTypeRef cf, T* number, CFNumberRef* cfNumber, char const* name)
{
...
sk_sp<SkTypeface> SkTypeface_Mac::Make(SkUniqueCFRef<CTFontRef> font, OpszVariation opszVariation, std::unique_ptr<SkStreamAsset> providedData)
{
    ...
    auto makeTypeface = [&]() {
        ...
        return sk_sp<SkTypeface>(new SkTypeface_Mac(std::move(font), style, isFixedPitch, opszVariation, std::move(providedData)));
    };
...
{
    CFNumberRef num;
    return CFDictionaryGetValueIfPresent(dict, name, (const void**)&num) && CFNumberIsFloatType(num) && CFNumberGetValue(num, kCFNumberCGFloatType, value);
}
    ...
    D map(S val) const
    {
        ...
        for (int i = 0; i < fMappingCount - 1; ++i) {
            if (val < fMapping[i + 1].src_val) {
                return map(val, fMapping[i].src_val, fMapping[i + 1].src_val, fMapping[i].dst_val, fMapping[i + 1].dst_val);
            }
        }
...
struct RoundCGFloatToInt {
    int operator()(CGFloat s)
    {
        return s + 0.5;
    }
};
struct CGFloatIdentity {
    CGFloat operator()(CGFloat s)
    {
        return s;
    }
};
...
static int ct_weight_to_fontstyle(CGFloat cgWeight, bool fromDataProvider)
{
    ...
    static constexpr Interpolator nativeInterpolator(nativeWeightMappings, std::size(nativeWeightMappings));
    static constexpr Interpolator dataProviderInterpolator(dataProviderWeightMappings, std::size(dataProviderWeightMappings));

    return fromDataProvider ? dataProviderInterpolator.map(cgWeight) : nativeInterpolator.map(cgWeight);
}
...
SkFontStyle SkCTFontDescriptorGetSkFontStyle(CTFontDescriptorRef desc, bool fromDataProvider)
{
    ...
    return SkFontStyle(
        ct_weight_to_fontstyle(weight, fromDataProvider), ct_width_to_fontstyle(width), slant ? SkFontStyle::kItalic_Slant : SkFontStyle::kUpright_Slant);
}
...
static void get_plane_glyph_map(const uint8_t* bits, CTFontRef ctFont, CFIndex glyphCount, SkUnichar* glyphToUnicode, uint8_t planeIndex)
{
...
static void populate_glyph_to_unicode(CTFontRef ctFont, CFIndex glyphCount, SkUnichar* glyphToUnicode)
{
...
{
    SkUniqueCFRef<CTFontRef> ctFont = SkCTFontCreateExactCopy(fFontRef.get(), CTFontGetUnitsPerEm(fFontRef.get()), fOpszVariation);
...
std::unique_ptr<SkAdvancedTypefaceMetrics> SkTypeface_Mac::onGetAdvancedMetrics() const
{
    ...
    if (sizeof(fsType) == this->getTableData(SkTEndian_SwapBE32(SkOTTableOS2::TAG), offsetof(SkOTTableOS2_V4, fsType), sizeof(fsType), &fsType)) {
    ...
    if (CTFontGetGlyphsForCharacters(ctFont.get(), stem_chars, glyphs, count)) {
        CTFontGetBoundingRectsForGlyphs(ctFont.get(), kCTFontOrientationHorizontal, glyphs, boundingRects, count);
...
{
    SkUniqueCFRef<CFNumberRef> fontFormatRef(static_cast<CFNumberRef>(CTFontCopyAttribute(ctFont, kCTFontFormatAttribute)));
    if (!fontFormatRef) {
        return 0;
    }
    ...
    *ttcIndex = 0;
    ...
    fInitStream([this] {
        ...
        for (int tableIndex = 0; tableIndex < numTables; ++tableIndex) {
            ...
            entry->checksum = SkEndian_SwapBE32(SkOTUtils::CalcTableChecksum((SK_OT_ULONG*)dataPtr, tableSize));
...
CFArrayRef SkTypeface_Mac::getVariationAxes() const
{
    fInitVariationAxes([this] {
        ...
        SkUniqueCFRef<CFTypeRef> cf(CTFontDescriptorCopyAttribute(desc.get(), kCTFontVariationAxesAttribute));
        CFArrayRef array;
...
int SkTypeface_Mac::onGetVariationDesignPosition(SkFontArguments::VariationPosition::Coordinate coordinates[], int coordinateCount) const
{
    CFArrayRef ctAxes = this->getVariationAxes();
    if (!ctAxes) {
        return -1;
...
SkTypeface::LocalizedStrings* SkTypeface_Mac::onCreateFamilyNameIterator() const
{
    ...
    if (!nameIter) {
        ...
        SkUniqueCFRef<CFStringRef> cfFamilyName(CTFontCopyLocalizedName(fFontRef.get(), kCTFontFamilyNameKey, &cfLanguageRaw));
...
{
    SkUniqueCFRef<CFArrayRef> cfArray(CTFontCopyAvailableTables(fFontRef.get(), kCTFontTableOptionNoOptions));
    if (!cfArray) {
        return 0;
    }
...
{
    SkUniqueCFRef<CFDataRef> data(CTFontCopyTable(ctFont, (CTFontTableTag)tag, kCTFontTableOptionNoOptions));
    if (!data) {
...
size_t SkTypeface_Mac::onGetTableData(SkFontTableTag tag, size_t offset, size_t length, void* dstData) const
{
...
sk_sp<SkData> SkTypeface_Mac::onCopyTableData(SkFontTableTag tag) const
{
    ...
    return SkData::MakeWithProc(
        data, length, [](const void*, void* ctx) { CFRelease((CFDataRef)ctx); }, (void*)srcData.release());
}

std::unique_ptr<SkScalerContext> SkTypeface_Mac::onCreateScalerContext(const SkScalerContextEffects& effects, const SkDescriptor* desc) const
{
    return std::make_unique<SkScalerContext_Mac>(sk_ref_sp(const_cast<SkTypeface_Mac*>(this)), effects, desc);
}
...
void SkTypeface_Mac::onFilterRec(SkScalerContextRec* rec) const
{
    ...
    unsigned flagsWeDontSupport = SkScalerContext::kForceAutohinting_Flag | SkScalerContext::kLCD_BGROrder_Flag | SkScalerContext::kLCD_Vertical_Flag;
    ...
    if (SkMask::kA8_Format == rec->fMaskFormat && SkFontHinting::kNone == rec->getHinting()) {
        ...
        if (smoothBehavior == SkCTFontSmoothBehavior::some) {
            ...
            color = SkColorSetRGB(SkColorGetR(color) * 1 / 2, SkColorGetG(color) * 1 / 2, SkColorGetB(color) * 1 / 2);
            ...
            color = SkColorSetRGB(SkColorGetR(color) * 3 / 4, SkColorGetG(color) * 3 / 4, SkColorGetB(color) * 3 / 4);
        }
...
void SkTypeface_Mac::onGetFontDescriptor(SkFontDescriptor* desc, bool* isLocalStream) const
{
    ...
    CFTypeRef paletteOverrides = CFDictionaryGetValue(attributes.get(), getCTFontPaletteColorsAttribute());
    if (paletteOverrides) {
        ...
        if (SkCFDynamicCast(paletteOverrides, &paletteOverrideDict, "Palette")) {
            ...
            CFDictionaryApplyFunction(
                ...
                [](const void* key, const void* value, void* ctx) -> void {
                    ...
                    SkColor4f color4f = { (float)components[0], (float)components[1], (float)components[2], (float)components[3] };
...
static CTFontVariation ctvariation_from_SkFontArguments(CTFontRef ct, CFArrayRef ctAxes, const SkFontArguments& args)
{
    OpszVariation opsz;
    ...
    SkUniqueCFRef<CFMutableDictionaryRef> newCtVariation(
        CFDictionaryCreateMutable(kCFAllocatorDefault, axisCount, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks));
    ...
    for (int i = 0; i < axisCount; ++i) {
        ...
        CFTypeRef def = CFDictionaryGetValue(axisInfoDict, kCTFontVariationAxisDefaultValueKey);
        if (!SkCFNumberDynamicCast(min, &minDouble, nullptr, "Axis min") || !SkCFNumberDynamicCast(max, &maxDouble, nullptr, "Axis max")
        ...
        if (tagLong == opszTag) {
            ...
            if (haveCurrentDouble && value == currentDouble) {
                ...
                wrongOpszVariation.reset(CFDictionaryCreateMutable(kCFAllocatorDefault, 0, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks));
                SkUniqueCFRef<CFNumberRef> wrongOpszNumber(CFNumberCreate(kCFAllocatorDefault, kCFNumberDoubleType, &wrongOpszDouble));
                CFDictionarySetValue(wrongOpszVariation.get(), tagNumber, wrongOpszNumber.get());
            }
        }
        SkUniqueCFRef<CFNumberRef> valueNumber(CFNumberCreate(kCFAllocatorDefault, kCFNumberDoubleType, &value));
        CFDictionaryAddValue(newCtVariation.get(), tagNumber, valueNumber.get());
    }
    return { SkUniqueCFRef<CFDictionaryRef>(std::move(newCtVariation)), SkUniqueCFRef<CFDictionaryRef>(std::move(wrongOpszVariation)), opsz };
}
...
static bool apply_palette(CFMutableDictionaryRef attributes, const SkFontArguments::Palette& palette)
{
    bool changedAttributes = false;
    if (palette.index != 0 || palette.overrideCount) {
        SkUniqueCFRef<CFNumberRef> paletteIndex(CFNumberCreate(kCFAllocatorDefault, kCFNumberIntType, &palette.index));
    ...
    if (palette.overrideCount) {
        ...
        for (auto&& paletteOverride : SkSpan(palette.overrides, palette.overrideCount)) {
            ...
            SkUniqueCFRef<CFNumberRef> index(CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt16Type, &paletteOverrideIndex));
...
sk_sp<SkTypeface> SkTypeface_Mac::onMakeClone(const SkFontArguments& args) const
{
    ...
    CTFontVariation ctVariation = ctvariation_from_SkFontArguments(fFontRef.get(), this->getVariationAxes(), args);
    ...
    if (ctVariation.variation) {
        if (ctVariation.wrongOpszVariation) {
            ...
            CFDictionarySetValue(attributes.get(), kCTFontVariationAttribute, ctVariation.wrongOpszVariation.get());
            SkUniqueCFRef<CTFontDescriptorRef> varDesc(CTFontDescriptorCreateWithAttributes(attributes.get()));
    ...
    if (changedAttributes) {
        SkUniqueCFRef<CTFontDescriptorRef> desc(CTFontDescriptorCreateWithAttributes(attributes.get()));
    ...
    return SkTypeface_Mac::Make(std::move(ctVariant), ctVariation.opsz, fStream ? fStream->duplicate() : nullptr);
}
...
static sk_sp<SkData> skdata_from_skstreamasset(std::unique_ptr<SkStreamAsset> stream)
{
    ...
    if (const void* base = stream->getMemoryBase()) {
        return SkData::MakeWithProc(
            base, size, [](const void*, void* ctx) -> void { delete (SkStreamAsset*)ctx; }, stream.release());
    }
...
static SkUniqueCFRef<CFDataRef> cfdata_from_skdata(sk_sp<SkData> data)
{
    ...
    return SkUniqueCFRef<CFDataRef>(CFDataCreateWithBytesNoCopy(kCFAllocatorDefault, (const UInt8*)addr, size, alloc.get()));
}
...
sk_sp<SkTypeface> SkTypeface_Mac::MakeFromStream(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args)
{
    ...
    if (args.getVariationDesignPosition().coordinateCount != 0) {
        ...
        if (ctVariation.variation) {
            CFDictionaryAddValue(attributes.get(), kCTFontVariationAttribute, ctVariation.variation.get());
            changedAttributes = true;
        }
    }
    if (changedAttributes) {
...
int SkTypeface_Mac::onGetVariationDesignParameters(SkFontParameters::Variation::Axis parameters[], int parameterCount) const
{
    CFArrayRef ctAxes = this->getVariationAxes();
    if (!ctAxes) {
        return -1;
    ...
    // Added in 10.13
    static CFStringRef* kCTFontVariationAxisHiddenKeyPtr = static_cast<CFStringRef*>(dlsym(RTLD_DEFAULT, "kCTFontVariationAxisHiddenKey"));
