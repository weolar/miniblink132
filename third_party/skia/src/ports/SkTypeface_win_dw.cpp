...
SkFontStyle DWriteFontTypeface::GetStyle(IDWriteFont* font, IDWriteFontFace* fontFace)
{
    ...
    [&weight, &width, &slant, fontFace]() -> void {
        ...
        for (UINT32 axisIndex = 0; axisIndex < fontAxisCount; ++axisIndex) {
            ...
            if (fontAxisValue[axisIndex].axisTag == DWRITE_FONT_AXIS_TAG_SLANT && slant != SkFontStyle::kItalic_Slant) {
...
sk_sp<DWriteFontTypeface> DWriteFontTypeface::Make(IDWriteFactory* factory, IDWriteFontFace* fontFace, IDWriteFont* font, IDWriteFontFamily* fontFamily,
    sk_sp<Loaders> loaders, const SkFontArguments::Palette& palette)
{
    return sk_sp<DWriteFontTypeface>(new DWriteFontTypeface(GetStyle(font, fontFace), factory, fontFace, font, fontFamily, std::move(loaders), palette));
}
...
HRESULT DWriteFontTypeface::initializePalette()
{
    ...
    if (SkTFitsIn<UINT32>(fRequestedPalette.index) && SkTo<UINT32>(fRequestedPalette.index) < dwPaletteCount) {
    ...
    HRM(fDWriteFontFace2->GetPaletteEntries(basePaletteIndex, 0, dwPaletteEntryCount, dwPaletteEntry), "Could not retrieve palette entries.");
    ...
    for (UINT32 i = 0; i < dwPaletteEntryCount; ++i) {
        fPalette[i] = SkColorSetARGB(sk_float_round2int(dwPaletteEntry[i].a * 255), sk_float_round2int(dwPaletteEntry[i].r * 255),
            sk_float_round2int(dwPaletteEntry[i].g * 255), sk_float_round2int(dwPaletteEntry[i].b * 255));
    ...
    for (int i = 0; i < fRequestedPalette.overrideCount; ++i) {
        ...
        if (SkTFitsIn<UINT32>(paletteOverride.index) && SkTo<UINT32>(paletteOverride.index) < dwPaletteEntryCount) {
...
DWriteFontTypeface::DWriteFontTypeface(const SkFontStyle& style, IDWriteFactory* factory, IDWriteFontFace* fontFace, IDWriteFont* font,
    ...
    , fRequestedPaletteEntryOverrides(palette.overrideCount
              ? (SkFontArguments::Palette::Override*)memcpy(
                  new SkFontArguments::Palette::Override[palette.overrideCount], palette.overrides, palette.overrideCount * sizeof(palette.overrides[0]))
              : nullptr)
    , fRequestedPalette { palette.index, fRequestedPaletteEntryOverrides.get(), palette.overrideCount }
...
bool DWriteFontTypeface::onGetPostScriptName(SkString* skPostScriptName) const
{
    ...
    BOOL exists = FALSE;
    if (FAILED(fDWriteFont->GetInformationalStrings(DWRITE_INFORMATIONAL_STRING_POSTSCRIPT_NAME, &postScriptNames, &exists)) || !exists
...
void DWriteFontTypeface::onGetFontDescriptor(SkFontDescriptor* desc, bool* serialize) const
{
    // Get the family name.
    SkTScopedComPtr<IDWriteLocalizedStrings> familyNames;
    HRV(fDWriteFontFamily->GetFamilyNames(&familyNames));
    ...
    sk_careful_memcpy(desc->setPaletteEntryOverrides(fRequestedPalette.overrideCount), fRequestedPalette.overrides,
...
void DWriteFontTypeface::onCharsToGlyphs(const SkUnichar* uni, int count, SkGlyphID glyphs[]) const
{
...
{
    sk_sp<SkTypeface::LocalizedStrings> nameIter = SkOTUtils::LocalizedStrings_NameTable::MakeForFamilyNames(*this);
    if (!nameIter) {
        SkTScopedComPtr<IDWriteLocalizedStrings> familyNames;
...
int DWriteFontTypeface::onGetVariationDesignPosition(SkFontArguments::VariationPosition::Coordinate coordinates[], int coordinateCount) const
{
...
int DWriteFontTypeface::onGetVariationDesignParameters(SkFontParameters::Variation::Axis parameters[], int parameterCount) const
{
    ...
    for (UINT32 axisIndex = 0; axisIndex < fontAxisCount; ++axisIndex) {
        if (fontResource->GetFontAxisAttributes(axisIndex) & DWRITE_FONT_AXIS_ATTRIBUTES_VARIABLE) {
            ...
            parameters[coordIndex].setHidden(fontResource->GetFontAxisAttributes(axisIndex) & DWRITE_FONT_AXIS_ATTRIBUTES_HIDDEN);
            ++coordIndex;
        }
    }
...
size_t DWriteFontTypeface::onGetTableData(SkFontTableTag tag, size_t offset, size_t length, void* data) const
{
...
sk_sp<SkData> DWriteFontTypeface::onCopyTableData(SkFontTableTag tag) const
{
    ...
    void* lock;
    BOOL exists;
    fDWriteFontFace->TryGetFontTable(SkEndian_SwapBE32(tag), reinterpret_cast<const void**>(&data), &size, &lock, &exists);
    if (!exists) {
        return nullptr;
    }
    struct Context {
        Context(void* lock, IDWriteFontFace* face)
            : fLock(lock)
            , fFontFace(SkRefComPtr(face))
        {
        }
        ~Context()
        {
    ...
    return SkData::MakeWithProc(
        data, size, [](const void*, void* ctx) { delete (Context*)ctx; }, new Context(lock, fDWriteFontFace.get()));
}
    ...
    if (SUCCEEDED(fDWriteFontFace->QueryInterface(&fontFace5)) && fontFace5->HasVariations()) {
        ...
        for (UINT32 fontIndex = 0; fontIndex < fontAxisCount; ++fontIndex) {
            for (UINT32 argsIndex = 0; argsIndex < argsCoordCount; ++argsIndex) {
                if (SkEndian_SwapBE32(fontAxisValue[fontIndex].axisTag) == args.getVariationDesignPosition().coordinates[argsIndex].axis) {
                    fontAxisValue[fontIndex].value = args.getVariationDesignPosition().coordinates[argsIndex].value;
                }
            }
        }
        SkTScopedComPtr<IDWriteFontResource> fontResource;
        ...
        HRN(fontResource->CreateFontFace(fDWriteFont->GetSimulations(), fontAxisValue.get(), fontAxisCount, &newFontFace5));

        SkTScopedComPtr<IDWriteFontFace> newFontFace;
        HRN(newFontFace5->QueryInterface(&newFontFace));
        return DWriteFontTypeface::Make(fFactory.get(), newFontFace.get(), fDWriteFont.get(), fDWriteFontFamily.get(), fLoaders, args.getPalette());
    }

#endif

    // If the palette args have changed, a new font will need to be created.
    if (args.getPalette().index != fRequestedPalette.index || args.getPalette().overrideCount != fRequestedPalette.overrideCount
        || memcmp(args.getPalette().overrides, fRequestedPalette.overrides, fRequestedPalette.overrideCount * sizeof(fRequestedPalette.overrides[0]))) {
        return DWriteFontTypeface::Make(fFactory.get(), fDWriteFontFace.get(), fDWriteFont.get(), fDWriteFontFamily.get(), fLoaders, args.getPalette());
    }
...
std::unique_ptr<SkStreamAsset> DWriteFontTypeface::onOpenStream(int* ttcIndex) const
{
    *ttcIndex = fDWriteFontFace->GetIndex();
    ...
    HRNM(fontFile->GetReferenceKey(&fontFileKey, &fontFileKeySize), "Could not get font file reference key.");
    ...
    HRNM(fontFileLoader->CreateStreamFromKey(fontFileKey, fontFileKeySize, &fontFileStream), "Could not create font file stream.");

    return std::unique_ptr<SkStreamAsset>(new SkDWriteFontFileStream(fontFileStream.get()));
}

std::unique_ptr<SkScalerContext> DWriteFontTypeface::onCreateScalerContext(const SkScalerContextEffects& effects, const SkDescriptor* desc) const
{
    return std::make_unique<SkScalerContext_DW>(sk_ref_sp(const_cast<DWriteFontTypeface*>(this)), effects, desc);
}
...
void DWriteFontTypeface::onFilterRec(SkScalerContextRec* rec) const
{
    ...
    unsigned flagsWeDontSupport = SkScalerContext::kForceAutohinting_Flag | SkScalerContext::kEmbolden_Flag | SkScalerContext::kLCD_Vertical_Flag;
...
static void glyph_to_unicode_map(
    IDWriteFontFace* fontFace, DWRITE_UNICODE_RANGE range, UINT32* remainingGlyphCount, UINT32 numGlyphs, SkUnichar* glyphToUnicode)
{
    ...
    for (UINT32 c = range.first; c <= range.last && *remainingGlyphCount != 0; c += batchSize) {
        ...
        HRVM(fontFace->GetGlyphIndices(codepoints, numBatchedCodePoints, glyphs), "Failed to get glyph indexes.");
        for (UINT32 i = 0; i < numBatchedCodePoints; ++i) {
...
std::unique_ptr<SkAdvancedTypefaceMetrics> DWriteFontTypeface::onGetAdvancedMetrics() const
{
    ...
    } else if (fontType == DWRITE_FONT_FACE_TYPE_CFF || fontType == DWRITE_FONT_FACE_TYPE_OPENTYPE_COLLECTION) {
    ...
    if (SkPanose::FamilyType::TextAndDisplay == os2Table->version.v0.panose.bFamilyType) {
        if (SerifStyle::Cove == serifStyle || SerifStyle::ObtuseCove == serifStyle || SerifStyle::SquareCove == serifStyle
            || SerifStyle::ObtuseSquareCove == serifStyle || SerifStyle::Square == serifStyle || SerifStyle::Thin == serifStyle
            || SerifStyle::Bone == serifStyle || SerifStyle::Exaggerated == serifStyle || SerifStyle::Triangle == serifStyle) {
    ...
    info->fBBox = SkIRect::MakeLTRB((int32_t)SkEndian_SwapBE16((uint16_t)headTable->xMin), (int32_t)SkEndian_SwapBE16((uint16_t)headTable->yMax),
        (int32_t)SkEndian_SwapBE16((uint16_t)headTable->xMax), (int32_t)SkEndian_SwapBE16((uint16_t)headTable->yMin));
    return info;
}
...
public:
    ...
    SK_STDMETHODIMP CreateStreamFromKey(void const* fontFileReferenceKey, UINT32 fontFileReferenceKeySize, IDWriteFontFileStream** fontFileStream) override;

    // Takes ownership of stream.
    static HRESULT Create(std::unique_ptr<SkStreamAsset> stream, StreamFontFileLoader** streamFontFileLoader)
    {
...
SK_STDMETHODIMP StreamFontFileLoader::CreateStreamFromKey(
    void const* fontFileReferenceKey, UINT32 fontFileReferenceKeySize, IDWriteFontFileStream** fontFileStream)
{
...
public:
    ...
    static HRESULT Create(IDWriteFactory* factory, IDWriteFontFileLoader* fontFileLoader, StreamFontFileEnumerator** streamFontFileEnumerator)
    {
...
StreamFontFileEnumerator::StreamFontFileEnumerator(IDWriteFactory* factory, IDWriteFontFileLoader* fontFileLoader)
    : fRefCount(1)
    , fFactory(SkRefComPtr(factory))
    , fCurrentFile()
...
public:
    ...
    SK_STDMETHODIMP CreateEnumeratorFromKey(
        IDWriteFactory* factory, void const* collectionKey, UINT32 collectionKeySize, IDWriteFontFileEnumerator** fontFileEnumerator) override;

    static HRESULT Create(IDWriteFontFileLoader* fontFileLoader, StreamFontCollectionLoader** streamFontCollectionLoader)
    {
...
SK_STDMETHODIMP StreamFontCollectionLoader::CreateEnumeratorFromKey(
    IDWriteFactory* factory, void const* collectionKey, UINT32 collectionKeySize, IDWriteFontFileEnumerator** fontFileEnumerator)
{
...
static HRESULT apply_fontargument_variation(SkTScopedComPtr<IDWriteFontFace>& fontFace, const SkFontArguments& args)
{
#if defined(NTDDI_WIN10_RS3) && NTDDI_VERSION >= NTDDI_WIN10_RS3
    ...
    for (UINT32 fontAxisIndex = 0; fontAxisIndex < fontAxisCount; ++fontAxisIndex) {
        ...
        for (UINT32 argsCoordIndex = argsCoordCount; argsCoordIndex-- > 0;) {
            const SkFontArguments::VariationPosition::Coordinate& argsCoordinate = args.getVariationDesignPosition().coordinates[argsCoordIndex];
    ...
    HR(fontResource->CreateFontFace(DWRITE_FONT_SIMULATIONS_NONE, variation.get(), fontAxisCount, &fontFace5_Out));
    fontFace.reset();
...
sk_sp<SkTypeface> DWriteFontTypeface::MakeFromStream(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args)
{
    ...
    SkAutoIDWriteUnregister<StreamFontFileLoader> autoUnregisterFontFileLoader(factory, fontFileLoader.get());
    ...
    SkAutoIDWriteUnregister<StreamFontCollectionLoader> autoUnregisterFontCollectionLoader(factory, fontCollectionLoader.get());

    SkTScopedComPtr<IDWriteFontCollection> fontCollection;
    HRN(factory->CreateCustomFontCollection(fontCollectionLoader.get(), nullptr, 0, &fontCollection));
    ...
    for (UINT32 familyIndex = 0; familyIndex < familyCount; ++familyIndex) {
        ...
        for (UINT32 fontIndex = 0; fontIndex < fontCount; ++fontIndex) {
            ...
            return DWriteFontTypeface::Make(factory, fontFace.get(), font.get(), fontFamily.get(),
                sk_make_sp<DWriteFontTypeface::Loaders>(factory, autoUnregisterFontFileLoader.detatch(), autoUnregisterFontCollectionLoader.detatch()),
                args.getPalette());
        }
    }
