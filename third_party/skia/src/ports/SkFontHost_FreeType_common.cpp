...
#if (((FREETYPE_MAJOR) < 2) || ((FREETYPE_MAJOR) == 2 && (FREETYPE_MINOR) < 11) || ((FREETYPE_MAJOR) == 2 && (FREETYPE_MINOR) == 11 && (FREETYPE_PATCH) < 1))  \
...
void copyFT2LCD16(const FT_Bitmap& bitmap, SkMaskBuilder* dstMask, int lcdIsBGR, const uint8_t* tableR, const uint8_t* tableG, const uint8_t* tableB)
{
    ...
    case FT_PIXEL_MODE_LCD:
        ...
        for (int y = height; y-- > 0;) {
            ...
            if (lcdIsBGR) {
                for (int x = 0; x < width; x++) {
                    dst[x] = packTriple(sk_apply_lut_if<APPLY_PREBLEND>(triple[2], tableR), sk_apply_lut_if<APPLY_PREBLEND>(triple[1], tableG),
                        sk_apply_lut_if<APPLY_PREBLEND>(triple[0], tableB));
                    triple += 3;
                }
            } else {
                for (int x = 0; x < width; x++) {
                    dst[x] = packTriple(sk_apply_lut_if<APPLY_PREBLEND>(triple[0], tableR), sk_apply_lut_if<APPLY_PREBLEND>(triple[1], tableG),
    ...
    case FT_PIXEL_MODE_LCD_V:
        ...
        for (int y = height; y-- > 0;) {
            ...
            for (int x = 0; x < width; x++) {
                dst[x] = packTriple(sk_apply_lut_if<APPLY_PREBLEND>(*srcR++, tableR), sk_apply_lut_if<APPLY_PREBLEND>(*srcG++, tableG),
...
inline SkMatrix ToSkMatrix(FT_Affine23 affine23)
{
    ...
    return SkMatrix::MakeAll(SkFixedToScalar(affine23.xx), -SkFixedToScalar(affine23.xy), SkFixedToScalar(affine23.dx), -SkFixedToScalar(affine23.yx),
        SkFixedToScalar(affine23.yy), -SkFixedToScalar(affine23.dy), 0, 0, 1);
}
...
bool colrv1_configure_skpaint(FT_Face face, const SkSpan<SkColor>& palette, const SkColor foregroundColor, const FT_COLR_Paint& colrPaint, SkPaint* paint)
{
    auto fetchColorStops = [&face, &palette, &foregroundColor](
                               const FT_ColorStopIterator& colorStopIterator, std::vector<SkScalar>& stops, std::vector<SkColor4f>& colors) -> bool {
        ...
        std::stable_sort(colorStopsSorted.begin(), colorStopsSorted.end(), [](const ColorStop& a, const ColorStop& b) { return a.pos < b.pos; });
    ...
    case FT_COLR_PAINTFORMAT_LINEAR_GRADIENT: {
        ...
        SkPoint linePositions[2] = { SkPoint::Make(SkFixedToScalar(linearGradient.p0.x), -SkFixedToScalar(linearGradient.p0.y)),
            SkPoint::Make(SkFixedToScalar(linearGradient.p1.x), -SkFixedToScalar(linearGradient.p1.y)) };
        SkPoint p0 = linePositions[0];
        SkPoint p1 = linePositions[1];
        SkPoint p2 = SkPoint::Make(SkFixedToScalar(linearGradient.p2.x), -SkFixedToScalar(linearGradient.p2.y));
    ...
    case FT_COLR_PAINTFORMAT_RADIAL_GRADIENT: {
        ...
        SkPoint start = SkPoint::Make(SkFixedToScalar(radialGradient.c0.x), -SkFixedToScalar(radialGradient.c0.y));
        SkScalar startRadius = SkFixedToScalar(radialGradient.r0);
        SkPoint end = SkPoint::Make(SkFixedToScalar(radialGradient.c1.x), -SkFixedToScalar(radialGradient.c1.y));
        ...
        paint->setShader(SkGradientShader::MakeTwoPointConical(start, startRadius, end, endRadius, colors.data(), SkColorSpace::MakeSRGB(), stops.data(),
            stops.size(), tileMode,
            SkGradientShader::Interpolation { SkGradientShader::Interpolation::InPremul::kNo, SkGradientShader::Interpolation::ColorSpace::kSRGB,
                SkGradientShader::Interpolation::HueMethod::kShorter },
            nullptr));
    ...
    case FT_COLR_PAINTFORMAT_SWEEP_GRADIENT: {
        ...
        SkPoint center = SkPoint::Make(SkFixedToScalar(sweepGradient.center.x), -SkFixedToScalar(sweepGradient.center.y));
...
bool colrv1_draw_paint(SkCanvas* canvas, const SkSpan<SkColor>& palette, const SkColor foregroundColor, FT_Face face, const FT_COLR_Paint& colrPaint)
{
    switch (colrPaint.format) {
...
bool colrv1_draw_glyph_with_path(
    SkCanvas* canvas, const SkSpan<SkColor>& palette, SkColor foregroundColor, FT_Face face, const FT_COLR_Paint& glyphPaint, const FT_COLR_Paint& fillPaint)
{
    SkASSERT(glyphPaint.format == FT_COLR_PAINTFORMAT_GLYPH);
    SkASSERT(fillPaint.format == FT_COLR_PAINTFORMAT_SOLID || fillPaint.format == FT_COLR_PAINTFORMAT_LINEAR_GRADIENT
        || fillPaint.format == FT_COLR_PAINTFORMAT_RADIAL_GRADIENT || fillPaint.format == FT_COLR_PAINTFORMAT_SWEEP_GRADIENT);
...
void colrv1_transform(FT_Face face, const FT_COLR_Paint& colrPaint, SkCanvas* canvas, SkMatrix* outTransform = nullptr)
{
    SkMatrix transform;
    ...
    case FT_COLR_PAINTFORMAT_SCALE: {
        transform.setScale(SkFixedToScalar(colrPaint.u.scale.scale_x), SkFixedToScalar(colrPaint.u.scale.scale_y), SkFixedToScalar(colrPaint.u.scale.center_x),
    ...
    case FT_COLR_PAINTFORMAT_ROTATE: {
        ...
        transform = SkMatrix::RotateDeg(-SkFixedToScalar(colrPaint.u.rotate.angle) * 180.0f,
            SkPoint::Make(SkFixedToScalar(colrPaint.u.rotate.center_x), -SkFixedToScalar(colrPaint.u.rotate.center_y)));
        break;
    }
    case FT_COLR_PAINTFORMAT_SKEW: {
        ...
        transform.setSkew(xTan, yTan, SkFixedToScalar(colrPaint.u.skew.center_x), -SkFixedToScalar(colrPaint.u.skew.center_y));
        break;
    }
    default: {
...
bool colrv1_start_glyph(SkCanvas* canvas, const SkSpan<SkColor>& palette, const SkColor foregroundColor, FT_Face face, uint16_t glyphId,
    FT_Color_Root_Transform rootTransform, VisitedSet* activePaints);

bool colrv1_traverse_paint(
    SkCanvas* canvas, const SkSpan<SkColor>& palette, const SkColor foregroundColor, FT_Face face, FT_OpaquePaint opaquePaint, VisitedSet* activePaints)
{
    ...
    case FT_COLR_PAINTFORMAT_COLR_LAYERS: {
        ...
        while (FT_Get_Paint_Layers(face, &layerIterator, &layerPaint)) {
            if (!colrv1_traverse_paint(canvas, palette, foregroundColor, face, layerPaint, activePaints)) {
                return false;
            }
        }
        return true;
    ...
    case FT_COLR_PAINTFORMAT_GLYPH:
        ...
        if (fillPaint.format == FT_COLR_PAINTFORMAT_SOLID || fillPaint.format == FT_COLR_PAINTFORMAT_LINEAR_GRADIENT
            || fillPaint.format == FT_COLR_PAINTFORMAT_RADIAL_GRADIENT || fillPaint.format == FT_COLR_PAINTFORMAT_SWEEP_GRADIENT) {
            return colrv1_draw_glyph_with_path(canvas, palette, foregroundColor, face, paint, fillPaint);
        }
        if (!colrv1_draw_paint(canvas, palette, foregroundColor, face, paint)) {
            return false;
        }
        return colrv1_traverse_paint(canvas, palette, foregroundColor, face, paint.u.glyph.paint, activePaints);
    case FT_COLR_PAINTFORMAT_COLR_GLYPH:
        return colrv1_start_glyph(canvas, palette, foregroundColor, face, paint.u.colr_glyph.glyphID, FT_COLOR_NO_ROOT_TRANSFORM, activePaints);
    case FT_COLR_PAINTFORMAT_TRANSFORM:
        colrv1_transform(face, paint, canvas);
        return colrv1_traverse_paint(canvas, palette, foregroundColor, face, paint.u.transform.paint, activePaints);
    case FT_COLR_PAINTFORMAT_TRANSLATE:
        colrv1_transform(face, paint, canvas);
        return colrv1_traverse_paint(canvas, palette, foregroundColor, face, paint.u.translate.paint, activePaints);
    case FT_COLR_PAINTFORMAT_SCALE:
        colrv1_transform(face, paint, canvas);
        return colrv1_traverse_paint(canvas, palette, foregroundColor, face, paint.u.scale.paint, activePaints);
    case FT_COLR_PAINTFORMAT_ROTATE:
        colrv1_transform(face, paint, canvas);
        return colrv1_traverse_paint(canvas, palette, foregroundColor, face, paint.u.rotate.paint, activePaints);
    case FT_COLR_PAINTFORMAT_SKEW:
        colrv1_transform(face, paint, canvas);
        return colrv1_traverse_paint(canvas, palette, foregroundColor, face, paint.u.skew.paint, activePaints);
    case FT_COLR_PAINTFORMAT_COMPOSITE: {
        SkAutoCanvasRestore acr(canvas, false);
        canvas->saveLayer(nullptr, nullptr);
        if (!colrv1_traverse_paint(canvas, palette, foregroundColor, face, paint.u.composite.backdrop_paint, activePaints)) {
            return false;
        }
        ...
        return colrv1_traverse_paint(canvas, palette, foregroundColor, face, paint.u.composite.source_paint, activePaints);
    }
    case FT_COLR_PAINTFORMAT_SOLID:
    case FT_COLR_PAINTFORMAT_LINEAR_GRADIENT:
    case FT_COLR_PAINTFORMAT_RADIAL_GRADIENT:
...
SkPath GetClipBoxPath(FT_Face face, uint16_t glyphId, bool untransformed)
{
    ...
    if (FT_Get_Color_Glyph_ClipBox(face, glyphId, &colrGlyphClipBox)) {
        resultPath = SkPath::Polygon({ { SkFDot6ToScalar(colrGlyphClipBox.bottom_left.x), -SkFDot6ToScalar(colrGlyphClipBox.bottom_left.y) },
                                         { SkFDot6ToScalar(colrGlyphClipBox.top_left.x), -SkFDot6ToScalar(colrGlyphClipBox.top_left.y) },
                                         { SkFDot6ToScalar(colrGlyphClipBox.top_right.x), -SkFDot6ToScalar(colrGlyphClipBox.top_right.y) },
                                         { SkFDot6ToScalar(colrGlyphClipBox.bottom_right.x), -SkFDot6ToScalar(colrGlyphClipBox.bottom_right.y) } },
            true);
    }
...
bool colrv1_start_glyph_bounds(SkMatrix* ctm, SkRect* bounds, FT_Face face, uint16_t glyphId, FT_Color_Root_Transform rootTransform, VisitedSet* activePaints);

bool colrv1_traverse_paint_bounds(SkMatrix* ctm, SkRect* bounds, FT_Face face, FT_OpaquePaint opaquePaint, VisitedSet* activePaints)
{
    // Cycle detection, see section "5.7.11.1.9 Color glyphs as a directed acyclic graph".
    if (activePaints->contains(opaquePaint)) {
        return false;
    ...
    case FT_COLR_PAINTFORMAT_COLR_GLYPH: {
        ...
        return colrv1_start_glyph_bounds(ctm, bounds, face, glyphID, FT_COLOR_NO_ROOT_TRANSFORM, activePaints);
    }
    case FT_COLR_PAINTFORMAT_TRANSFORM: {
...
{
    FT_OpaquePaint opaquePaint { nullptr, 1 };
    return FT_Get_Color_Glyph_Paint(face, glyphId, rootTransform, &opaquePaint) && colrv1_traverse_paint_bounds(ctm, bounds, face, opaquePaint, activePaints);
}
...
#ifdef TT_SUPPORT_COLRV1
bool SkScalerContextFTUtils::drawCOLRv1Glyph(FT_Face face, const SkGlyph& glyph, uint32_t loadGlyphFlags, SkSpan<SkColor> palette, SkCanvas* canvas) const
{
    if (this->isSubpixel()) {
    ...
    return colrv1_start_glyph(canvas, palette, fForegroundColor, face, glyph.getGlyphID(), FT_COLOR_INCLUDE_ROOT_TRANSFORM, &activePaints);
}
#endif // TT_SUPPORT_COLRV1

#ifdef FT_COLOR_H
bool SkScalerContextFTUtils::drawCOLRv0Glyph(FT_Face face, const SkGlyph& glyph, LoadGlyphFlags flags, SkSpan<SkColor> palette, SkCanvas* canvas) const
{
    if (this->isSubpixel()) {
        canvas->translate(SkFixedToScalar(glyph.getSubXFixed()), SkFixedToScalar(glyph.getSubYFixed()));
    }
    ...
    while (FT_Get_Color_Glyph_Layer(face, glyph.getGlyphID(), &layerGlyphIndex, &layerColorIndex, &layerIterator)) {
        haveLayers = true;
...
#if defined(FT_CONFIG_OPTION_SVG)
bool SkScalerContextFTUtils::drawSVGGlyph(FT_Face face, const SkGlyph& glyph, LoadGlyphFlags flags, SkSpan<SkColor> palette, SkCanvas* canvas) const
{
    ...
    m.setAll(SkFixedToFloat(ftMatrix.xx), -SkFixedToFloat(ftMatrix.xy), SkFixedToFloat(ftOffset.x), -SkFixedToFloat(ftMatrix.yx), SkFixedToFloat(ftMatrix.yy),
        -SkFixedToFloat(ftOffset.y), 0, 0, 1);
    m.postScale(SkFixedToFloat(ftSvg->metrics.x_scale) / 64.0f, SkFixedToFloat(ftSvg->metrics.y_scale) / 64.0f);
    if (this->isSubpixel()) {
    ...
    return svgDecoder->render(*canvas, ftSvg->units_per_EM, glyph.getGlyphID(), fForegroundColor, palette);
}
#endif // FT_CONFIG_OPTION_SVG

void SkScalerContextFTUtils::generateGlyphImage(
    FT_Face face, const SkGlyph& glyph, void* imageBuffer, const SkMatrix& bitmapTransform, const SkMaskGamma::PreBlend& preBlend) const
{
    ...
    case FT_GLYPH_FORMAT_OUTLINE: {
        ...
        if (SkMask::kLCD16_Format == glyph.maskFormat()) {
            ...
            if (!SkIRect::Intersects(mask.fBounds, SkIRect::MakeXYWH(ftGlyph.bitmap_left, -ftGlyph.bitmap_top, ftGlyph.bitmap.width, ftGlyph.bitmap.rows))) {
                return;
            }
            ...
            if (preBlend.isApplicable()) {
                copyFT2LCD16<true>(ftGlyph.bitmap, &mask, doBGR, preBlend.fR, preBlend.fG, preBlend.fB);
            } else {
                copyFT2LCD16<false>(ftGlyph.bitmap, &mask, doBGR, preBlend.fR, preBlend.fG, preBlend.fB);
            }
    ...
    case FT_GLYPH_FORMAT_BITMAP: {
        ...
        SkASSERT(SkMask::kBW_Format == maskFormat || SkMask::kA8_Format == maskFormat || SkMask::kARGB32_Format == maskFormat
            || SkMask::kLCD16_Format == maskFormat);

        // If no scaling needed, directly copy glyph bitmap.
        if (bitmapTransform.isIdentity()) {
            SkMaskBuilder dstMask = SkMaskBuilder(static_cast<uint8_t*>(imageBuffer), glyph.iRect(), glyph.rowBytes(), glyph.maskFormat());
        ...
        unscaledBitmap.setInfo(
            SkImageInfo::Make(face->glyph->bitmap.width, face->glyph->bitmap.rows, SkColorType_for_FTPixelMode(pixel_mode), kPremul_SkAlphaType));
        ...
        SkMaskBuilder unscaledBitmapAlias(reinterpret_cast<uint8_t*>(unscaledBitmap.getPixels()),
            SkIRect::MakeWH(unscaledBitmap.width(), unscaledBitmap.height()), unscaledBitmap.rowBytes(),
        ...
        SkBitmap dstBitmap;
        // TODO: mark this as sRGB when the blits will be sRGB.
        dstBitmap.setInfo(SkImageInfo::Make(glyph.width(), glyph.height(), SkColorType_for_SkMaskFormat(maskFormat), kPremul_SkAlphaType), bitmapRowBytes);
        if (SkMask::kBW_Format == maskFormat || SkMask::kLCD16_Format == maskFormat) {
        ...
        if (SkMask::kBW_Format == maskFormat) {
            ...
            SkMaskBuilder dstMask(static_cast<uint8_t*>(imageBuffer), glyph.iRect(), glyph.rowBytes(), glyph.maskFormat());
...
class SkFTGeometrySink {
    ...
    static int Quad(const FT_Vector* pt0, const FT_Vector* pt1, void* ctx)
    {
        ...
        if (self.currentIsNot(pt0) || self.currentIsNot(pt1)) {
            self.goingTo(pt1);
            self.fPath->quadTo(SkFDot6ToScalar(pt0->x), -SkFDot6ToScalar(pt0->y), SkFDot6ToScalar(pt1->x), -SkFDot6ToScalar(pt1->y));
        }
        return 0;
    }
    ...
    static int Cubic(const FT_Vector* pt0, const FT_Vector* pt1, const FT_Vector* pt2, void* ctx)
    {
        ...
        if (self.currentIsNot(pt0) || self.currentIsNot(pt1) || self.currentIsNot(pt2)) {
            self.goingTo(pt2);
            self.fPath->cubicTo(SkFDot6ToScalar(pt0->x), -SkFDot6ToScalar(pt0->y), SkFDot6ToScalar(pt1->x), -SkFDot6ToScalar(pt1->y), SkFDot6ToScalar(pt2->x),
                -SkFDot6ToScalar(pt2->y));
        }
        return 0;
    }

public:
    SkFTGeometrySink(SkPath* path)
        : fPath { path }
        , fStarted { false }
        , fCurrent { 0, 0 }
    {
...
bool generateGlyphPathStatic(FT_Face face, SkPath* path)
{
    ...
    if (face->glyph->format != FT_GLYPH_FORMAT_OUTLINE || FT_Outline_Decompose(&face->glyph->outline, &SkFTGeometrySink::Funcs, &sink)) {
        path->reset();
        return false;
    }
    path->close();
...
bool generateFacePathStatic(FT_Face face, SkGlyphID glyphID, SkScalerContextFTUtils::LoadGlyphFlags flags, SkPath* path)
{
...
bool generateFacePathCOLRv1(FT_Face face, SkGlyphID glyphID, SkPath* path)
{
    ...
    auto tryGeneratePath = [face, &unscaledFtSize, glyphID, flags, path]() {
        ...
        err = FT_Set_Char_Size(face, SkIntToFDot6(face->units_per_EM), SkIntToFDot6(face->units_per_EM), 72, 72);
        if (err != 0) {
            return false;
        }
...
bool SkScalerContextFTUtils::generateFacePath(FT_Face face, SkGlyphID glyphID, LoadGlyphFlags flags, SkPath* path) const
{
    return generateFacePathStatic(face, glyphID, flags, path);
}

#ifdef TT_SUPPORT_COLRV1
bool SkScalerContextFTUtils::computeColrV1GlyphBoundingBox(FT_Face face, SkGlyphID glyphID, SkRect* bounds)
{
    SkMatrix ctm;
    *bounds = SkRect::MakeEmpty();
    VisitedSet activePaints;
    return colrv1_start_glyph_bounds(&ctm, bounds, face, glyphID, FT_COLOR_INCLUDE_ROOT_TRANSFORM, &activePaints);
}
#endif
