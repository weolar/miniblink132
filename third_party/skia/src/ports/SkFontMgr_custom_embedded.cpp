...
static void load_font_from_data(const SkFontScanner* scanner, std::unique_ptr<SkMemoryStream> stream, int index, SkFontMgr_Custom::Families* families)
{
    ...
    for (int faceIndex = 0; faceIndex < numFaces; ++faceIndex) {
        ...
        for (int instanceIndex = 0; instanceIndex <= numInstances; ++instanceIndex) {
            ...
            auto data = std::make_unique<SkFontData>(stream->duplicate(), faceIndex, 0, nullptr, 0, nullptr, 0);
            addTo->appendTypeface(sk_make_sp<SkTypeface_FreeTypeStream>(std::move(data), realname, style, isFixedPitch));
        }
    }
}
