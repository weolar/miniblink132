...
public:
    SkTypeface_AndroidSystem(const SkString& pathName, const bool cacheFontFiles, int index, const SkFixed* axes, int axesCount, const SkFontStyle& style,
        bool isFixedPitch, const SkString& familyName, const TArray<SkLanguage, true>& lang, FontVariant variantStyle)
    ...
    sk_sp<SkTypeface> onMakeClone(const SkFontArguments& args) const override
    {
        ...
        return sk_make_sp<SkTypeface_AndroidSystem>(
            fPathName, fFile, fIndex, data->getAxis(), data->getAxisCount(), style, this->isFixedPitch(), fFamilyName, fLang, fVariantStyle);
    }
...
public:
    explicit SkFontStyleSet_Android(const FontFamily& family, const SkFontScanner* scanner, const bool cacheFontFiles)
    {
        ...
        for (int i = 0; i < family.fFonts.size(); ++i) {
            ...
            if (!scanner->scanInstance(stream.get(), ttcIndex, 0, &familyName, &style, &isFixedWidth, &axisDefinitions)) {
                SkDEBUGF("Requested font file %s exists, but is not a valid font.\n", pathName.c_str());
                continue;
            }

            AutoSTMalloc<4, SkFixed> axisValues(axisDefinitions.size());
            SkFontArguments::VariationPosition position = { fontFile.fVariationDesignPosition.begin(), fontFile.fVariationDesignPosition.size() };
            SkFontScanner_FreeType::computeAxisValues(axisDefinitions, position, axisValues, familyName, &style);
            ...
            fStyles.push_back().reset(new SkTypeface_AndroidSystem(
                pathName, cacheFontFiles, ttcIndex, axisValues.get(), axisDefinitions.size(), style, isFixedWidth, familyName, family.fLanguages, variant));
        }
    }
...
public:
    SkFontMgr_Android(const SkFontMgr_Android_CustomFonts* custom, std::unique_ptr<SkFontScanner> scanner)
        : fScanner(std::move(scanner))
    {
        ...
        if (custom && SkFontMgr_Android_CustomFonts::kPreferSystem != custom->fSystemFontUse) {
            ...
            SkFontMgr_Android_Parser::GetCustomFontFamilies(families, base, custom->fFontsXml, custom->fFallbackFontsXml);
        }
...
protected:
    ...
    static sk_sp<SkTypeface_AndroidSystem> find_family_style_character(const SkString& familyName, const TArray<NameToFamily, true>& fallbackNameToFamilyMap,
        const SkFontStyle& style, bool elegant, const SkString& langTag, SkUnichar character)
    {
        for (int i = 0; i < fallbackNameToFamilyMap.size(); ++i) {
            ...
            if (!langTag.isEmpty()
                && std::none_of(face->fLang.begin(), face->fLang.end(), [&](const SkLanguage& lang) { return lang.getTag().startsWith(langTag.c_str()); })) {
                continue;
            }
        ...
        const char familyName[], const SkFontStyle& style, const char* bcp47[], int bcp47Count, SkUnichar character) const override
    {
        ...
        for (const SkString& currentFamilyName : { familyNameString, SkString() }) {
            ...
            for (int elegant = 2; elegant-- > 0;) {
                for (int bcp47Index = bcp47Count; bcp47Index-- > 0;) {
                    ...
                    while (!lang.getTag().isEmpty()) {
                        sk_sp<SkTypeface_AndroidSystem> matchingTypeface
                            = find_family_style_character(currentFamilyName, fFallbackNameToFamilyMap, style, SkToBool(elegant), lang.getTag(), character);
                        if (matchingTypeface) {
                ...
                sk_sp<SkTypeface_AndroidSystem> matchingTypeface
                    = find_family_style_character(currentFamilyName, fFallbackNameToFamilyMap, style, SkToBool(elegant), SkString(), character);
                if (matchingTypeface) {
                    return matchingTypeface;
                }
            }
...
private:
    ...
    void addFamily(FontFamily& family, const bool isolated, int familyIndex)
    {
        ...
        sk_sp<SkFontStyleSet_Android> newSet = sk_make_sp<SkFontStyleSet_Android>(family, fScanner.get(), isolated);
        if (0 == newSet->count()) {
            return;
        }
...
sk_sp<SkFontMgr> SkFontMgr_New_Android(const SkFontMgr_Android_CustomFonts* custom, std::unique_ptr<SkFontScanner> scanner)
{
    if (custom) {
        ...
        SkDEBUGF("SystemFontUse: %s BasePath: %s Fonts: %s FallbackFonts: %s\n", gSystemFontUseStrings[custom->fSystemFontUse], custom->fBasePath,
