...
bool SkFontConfigInterfaceDirect::isValidPattern(FcPattern* pattern)
{
    ...
    if (font_format && 0 != strcmp(font_format, kFontFormatTrueType) && 0 != strcmp(font_format, kFontFormatCFF)) {
        return false;
    }
#endif
...
FcPattern* SkFontConfigInterfaceDirect::MatchFont(FcFontSet* font_set, const char* post_config_family, const SkString& family)
{
...
bool SkFontConfigInterfaceDirect::matchFamilyName(
    const char familyName[], SkFontStyle style, FontIdentity* outIdentity, SkString* outFamilyName, SkFontStyle* outStyle)
{
