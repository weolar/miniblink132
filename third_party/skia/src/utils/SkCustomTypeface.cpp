...
private:
    ...
    explicit SkUserTypeface(SkFontStyle style, const SkFontMetrics& metrics, std::vector<SkCustomTypefaceBuilder::GlyphRec>&& recs)
        : SkTypeface(style)
...
void SkCustomTypefaceBuilder::setGlyph(SkGlyphID index, float advance, sk_sp<SkDrawable> drawable, const SkRect& bounds)
{
    auto& rec = this->ensureStorage(index);
    rec.fAdvance = advance;
...
sk_sp<SkTypeface> SkCustomTypefaceBuilder::detach()
{
    ...
    for (const auto& rec : fGlyphRecs) {
        bounds.join(rec.isDrawable() ? rec.fBounds : rec.fPath.getBounds());
    }
...
protected:
    ...
    void generateImage(const SkGlyph& glyph, void* imageBuffer) override
    {
        ...
        auto canvas = SkCanvas::MakeRasterDirectN32(glyph.width(), glyph.height(), static_cast<SkPMColor*>(imageBuffer), glyph.rowBytes());
    ...
    sk_sp<SkDrawable> generateDrawable(const SkGlyph& glyph) override
    {
        ...
        return rec.fDrawable ? sk_make_sp<DrawableMatrixWrapper>(rec.fDrawable, fMatrix) : nullptr;
    }
...
std::unique_ptr<SkScalerContext> SkUserTypeface::onCreateScalerContext(const SkScalerContextEffects& effects, const SkDescriptor* desc) const
{
    return std::make_unique<SkUserScalerContext>(sk_ref_sp(const_cast<SkUserTypeface*>(this)), effects, desc);
}
...
std::unique_ptr<SkStreamAsset> SkUserTypeface::onOpenStream(int* ttcIndex) const
{
    ...
    for (const auto& rec : fGlyphRecs) {
        ...
        auto data = rec.isDrawable() ? rec.fDrawable->serialize() : rec.fPath.serialize();
...
sk_sp<SkTypeface> SkCustomTypefaceBuilder::Deserialize(SkStream* stream)
{
    ...
    for (int i = 0; i < glyphCount; ++i) {
        uint32_t gtype;
        if (!stream->readU32(&gtype) || (gtype != GlyphType::kDrawable && gtype != GlyphType::kPath)) {
            return nullptr;
        }
...
sk_sp<SkTypeface> SkCustomTypefaceBuilder::MakeFromStream(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments&)
{
