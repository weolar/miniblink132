...
struct SpotVerticesFactory {
    ...
    {
        if (fOccluderHeight != that.fOccluderHeight || fDevLightPos.fZ != that.fDevLightPos.fZ || fLightRadius != that.fLightRadius
    ...
    sk_sp<SkVertices> makeVertices(const SkPath& path, const SkMatrix& ctm, SkVector* translate) const
    {
        bool transparent = fOccluderType == OccluderType::kPointTransparent || fOccluderType == OccluderType::kDirectionalTransparent;
        ...
        if (directional) {
            ...
            *translate = fOffset;
            return SkShadowTessellator::MakeSpot(path, noTrans, zParams, centerLightPos, fLightRadius, transparent, false);
        }
    }
};
...
public:
    ...
    sk_sp<SkVertices> add(const SkPath& devPath, const AmbientVerticesFactory& ambient, const SkMatrix& matrix, SkVector* translate)
    {
    ...
    sk_sp<SkVertices> add(const SkPath& devPath, const SpotVerticesFactory& spot, const SkMatrix& matrix, SkVector* translate)
    {
        ...
        sk_sp<SkVertices> find(const FACTORY& factory, const SkMatrix& matrix, SkVector* translate) const
        {
            for (int i = 0; i < MAX_ENTRIES; ++i) {
                if (fEntries[i].fFactory.isCompatible(factory, translate)) {
                    ...
                    } else if (matrix.getScaleX() != m.getScaleX() || matrix.getSkewX() != m.getSkewX() || matrix.getScaleY() != m.getScaleY()
...
public:
    ...
    template <typename FACTORY> sk_sp<SkVertices> find(const FACTORY& factory, const SkMatrix& matrix, SkVector* translate) const
    {
...
public:
    ...
    {
        return fShapeForKey.asRRect(rrect, nullptr);
    }
#else
    int keyBytes() const
    {
...
bool draw_shadow(const FACTORY& factory, std::function<void(const SkVertices*, SkBlendMode, const SkPaint&, SkScalar tx, SkScalar ty, bool)> drawProc,
    ShadowedPath& path, SkColor color)
{
    ...
    paint.setColorFilter(SkColorFilters::Blend(color, SkBlendMode::kModulate)->makeComposed(SkColorFilterPriv::MakeGaussian()));

    drawProc(vertices.get(), SkBlendMode::kModulate, paint, context.fTranslate.fX, context.fTranslate.fY, path.viewMatrix().hasPerspective());
...
void SkShadowUtils::ComputeTonalColors(SkColor inAmbientColor, SkColor inSpotColor, SkColor* outAmbientColor, SkColor* outSpotColor)
{
    ...
    *outSpotColor = SkColorSetARGB(tonalAlpha * 255.999f, unPremulScale * spotR, unPremulScale * spotG, unPremulScale * spotB);
}

static bool fill_shadow_rec(const SkPath& path, const SkPoint3& zPlaneParams, const SkPoint3& lightPos, SkScalar lightRadius, SkColor ambientColor,
    SkColor spotColor, uint32_t flags, const SkMatrix& ctm, SkDrawShadowRec* rec)
{
...
void SkShadowUtils::DrawShadow(SkCanvas* canvas, const SkPath& path, const SkPoint3& zPlaneParams, const SkPoint3& lightPos, SkScalar lightRadius,
    SkColor ambientColor, SkColor spotColor, uint32_t flags)
{
    SkDrawShadowRec rec;
    if (!fill_shadow_rec(path, zPlaneParams, lightPos, lightRadius, ambientColor, spotColor, flags, canvas->getTotalMatrix(), &rec)) {
        return;
    }
...
bool SkShadowUtils::GetLocalBounds(
    const SkMatrix& ctm, const SkPath& path, const SkPoint3& zPlaneParams, const SkPoint3& lightPos, SkScalar lightRadius, uint32_t flags, SkRect* bounds)
{
    SkDrawShadowRec rec;
...
{
    return rec.fLightPos.isFinite() && rec.fZPlaneParams.isFinite() && SkIsFinite(rec.fLightRadius);
}
...
#if !defined(SK_ENABLE_OPTIMIZE_SIZE)
    auto drawVertsProc = [this](const SkVertices* vertices, SkBlendMode mode, const SkPaint& paint, SkScalar tx, SkScalar ty, bool hasPerspective) {
        if (vertices->priv().vertexCount()) {
            ...
            SkAutoDeviceTransformRestore adr(this, hasPerspective ? SkMatrix::I() : this->localToDevice() * SkMatrix::Translate(tx, ty));
    ...
    if (SkColorGetA(rec.fAmbientColor) > 0) {
        ...
        if (uncached && !useBlur) {
            ...
            if (vertices) {
                ...
                paint.setColorFilter(SkColorFilters::Blend(rec.fAmbientColor, SkBlendMode::kModulate)->makeComposed(SkColorFilterPriv::MakeGaussian()));
                // The vertex colors for a tesselated shadow polygon are always either opaque black
                // or transparent and their real contribution to the final blended color is via
                // their alpha. We can skip expensive per-vertex color conversion for this.
    ...
    if (SkColorGetA(rec.fSpotColor) > 0) {
...
#if !defined(SK_ENABLE_OPTIMIZE_SIZE)
        if (uncached && !useBlur) {
            sk_sp<SkVertices> vertices = SkShadowTessellator::MakeSpot(path, viewMatrix, zPlaneParams, devLightPos, lightRadius, transparent, directional);
            if (vertices) {
                SkPaint paint;
                // Run the vertex color through a GaussianColorFilter and then modulate the
                // grayscale result of that against our 'color' param.
                paint.setColorFilter(SkColorFilters::Blend(rec.fSpotColor, SkBlendMode::kModulate)->makeComposed(SkColorFilterPriv::MakeGaussian()));
                // The vertex colors for a tesselated shadow polygon are always either opaque black
                // or transparent and their real contribution to the final blended color is via
                // their alpha. We can skip expensive per-vertex color conversion for this.
                this->drawVertices(vertices.get(), SkBlender::Mode(SkBlendMode::kModulate), paint,
        ...
        if (!success && !useBlur) {
            ...
            if (SkToBool(rec.fFlags & kDirectionalLight_ShadowFlag)) {
                SkDrawShadowMetrics::GetDirectionalParams(
                    zPlaneParams.fZ, devLightPos.fX, devLightPos.fY, devLightPos.fZ, lightRadius, &radius, &scale, &factory.fOffset);
            } else {
                SkDrawShadowMetrics::GetSpotParams(
                    zPlaneParams.fZ, devLightPos.fX - center.fX, devLightPos.fY - center.fY, devLightPos.fZ, lightRadius, &radius, &scale, &factory.fOffset);
            }

            SkRect devBounds;
            viewMatrix.mapRect(&devBounds, path.getBounds());
            if (transparent || SkTAbs(factory.fOffset.fX) > 0.5f * devBounds.width() || SkTAbs(factory.fOffset.fY) > 0.5f * devBounds.height()) {
        ...
        if (!success) {
            ...
            if (!SkDrawShadowMetrics::GetSpotShadowTransform(
                    devLightPos, lightRadius, viewMatrix, zPlaneParams, path.getBounds(), directional, &shadowMatrix, &radius)) {
                return;
            }
