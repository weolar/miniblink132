...
public:
    ...
    template <class T, std::enable_if_t<std::is_same_v<T, std::string>, bool> = false> void appendString(const T& value)
    {
    ...
    // Avoid the non-explicit converting constructor from char*
    template <class T, std::enable_if_t<std::is_same_v<T, std::string>, bool> = false> void appendString(const char* name, const T& value)
    {
        this->appendName(name);
        this->appendString(value.data(), value.size());
    }
