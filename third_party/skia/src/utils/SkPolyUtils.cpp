...
{
    return (denomPositive && (numer < 0 || numer > denom)) || (!denomPositive && (numer > 0 || numer < denom));
}
...
static bool compute_intersection(const OffsetSegment& s0, const OffsetSegment& s1, SkPoint* p, SkScalar* s, SkScalar* t)
{
    ...
    if (SkScalarNearlyZero(denom, kCrossTolerance)) {
        ...
        if (!SkScalarNearlyZero(w.cross(v0), kCrossTolerance) || !SkScalarNearlyZero(w.cross(v1), kCrossTolerance)) {
            return false;
        }
...
bool SkInsetConvexPolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize, SkScalar inset, SkTDArray<SkPoint>* insetPolygon)
{
    ...
    for (int curr = 0; curr < inputPolygonSize; prev = curr, ++curr) {
        ...
        if (compute_side(inputPolygonVerts[prev], inputPolygonVerts[curr] - inputPolygonVerts[prev], inputPolygonVerts[next]) * winding < 0) {
            return false;
        }
    ...
    while (head && prevEdge != currEdge) {
        ...
        if (compute_intersection(prevEdge->fOffset, currEdge->fOffset, &intersection, &s, &t)) {
            ...
            } else if (currEdge->fTValue > SK_ScalarMin && SkPointPriv::EqualsWithinTolerance(intersection, currEdge->fIntersection, 1.0e-6f)) {
                break;
            } else {
                // add intersection
            ...
            int side = winding * compute_side(currEdge->fOffset.fP0, currEdge->fOffset.fV, prevEdge->fOffset.fP0);
            if (side < 0 && side == winding * compute_side(currEdge->fOffset.fP0, currEdge->fOffset.fV, prevEdge->fOffset.fP0 + prevEdge->fOffset.fV)) {
                // no point in considering this one again
                remove_node(prevEdge, &head);
                --insetVertexCount;
                // go back one segment
    ...
    *insetPolygon->append() = head->fIntersection;
    currEdge = head->fNext;
    while (currEdge != head) {
        if (!SkPointPriv::EqualsWithinTolerance(currEdge->fIntersection, (*insetPolygon)[currIndex], kCleanupTolerance)) {
    ...
    if (currIndex >= 1 && SkPointPriv::EqualsWithinTolerance((*insetPolygon)[0], (*insetPolygon)[currIndex], kCleanupTolerance)) {
...
struct ActiveEdge {
    ...
    bool intersect(const SkPoint& q0, const SkVector& w, uint16_t index0, uint16_t index1) const
    {
        ...
        if (this->fIndex0 == index0 || this->fIndex1 == index0 || this->fIndex0 == index1 || this->fIndex1 == index1) {
            return false;
        }
    ...
    bool insert(const SkPoint& p0, const SkPoint& p1, uint16_t index0, uint16_t index1)
    {
        ...
        while (true) {
            if (!curr) {
                ...
                if ((pred && pred->intersect(p0, v, index0, index1)) || (succ && succ->intersect(p0, v, index0, index1))) {
                    return false;
                }
    ...
    while (vertexQueue.count() > 0) {
        ...
        if (v.fFlags == 0) {
            ...
            if (v.fFlags & kPrevLeft_VertexFlag) {
                if (!sweepLine.replace(polygon[v.fPrevIndex], v.fPosition, polygon[v.fNextIndex], v.fPrevIndex, v.fIndex, v.fNextIndex)) {
                    break;
                }
            } else {
                SkASSERT(v.fFlags & kNextLeft_VertexFlag);
                if (!sweepLine.replace(polygon[v.fNextIndex], v.fPosition, polygon[v.fPrevIndex], v.fNextIndex, v.fIndex, v.fPrevIndex)) {
                    break;
                }
            }
        }
...
{
    int side = compute_side(inputPolygonVerts[prevIndex], inputPolygonVerts[currIndex] - inputPolygonVerts[prevIndex], inputPolygonVerts[nextIndex]);
    // if reflex point, we need to add extra edges
    return (side * winding * offset < 0);
}

bool SkOffsetSimplePolygon(const SkPoint* inputPolygonVerts, int inputPolygonSize, const SkRect& bounds, SkScalar offset, SkTDArray<SkPoint>* offsetPolygon,
    SkTDArray<int>* polygonIndices)
{
    ...
    if (offset > std::min(SkTAbs(SkRectPriv::HalfWidth(bounds)), SkTAbs(SkRectPriv::HalfHeight(bounds)))) {
        return false;
    }
    ...
    for (int currIndex = 0, prevIndex = inputPolygonSize - 1; currIndex < inputPolygonSize; prevIndex = currIndex, ++currIndex) {
        if (!inputPolygonVerts[currIndex].isFinite()) {
            return false;
        }
        int nextIndex = (currIndex + 1) % inputPolygonSize;
        if (!compute_offset_vector(inputPolygonVerts[currIndex], inputPolygonVerts[nextIndex], offset, winding, &normals[currIndex])) {
            return false;
        }
        if (currIndex > 0) {
            // if reflex point, we need to add extra edges
            if (is_reflex_vertex(inputPolygonVerts, winding, offset, prevIndex, currIndex, nextIndex)) {
                ...
                int numSteps;
                if (!SkComputeRadialSteps(normals[prevIndex], normals[currIndex], offset, &rotSin, &rotCos, &numSteps)) {
                    return false;
                }
    ...
    if (is_reflex_vertex(inputPolygonVerts, winding, offset, inputPolygonSize - 1, 0, 1)) {
        SkScalar rotSin, rotCos;
        int numSteps;
        if (!SkComputeRadialSteps(normals[inputPolygonSize - 1], normals[0], offset, &rotSin, &rotCos, &numSteps)) {
            return false;
        }
        numEdges += std::max(numSteps, 1);
    }
    ...
    for (int currIndex = 0, prevIndex = inputPolygonSize - 1; currIndex < inputPolygonSize; prevIndex = currIndex, ++currIndex) {
        ...
        if (is_reflex_vertex(inputPolygonVerts, winding, offset, prevIndex, currIndex, nextIndex)) {
            ...
            if (!SkComputeRadialSteps(prevNormal, normals[currIndex], offset, &rotSin, &rotCos, &numSteps)) {
                return false;
            }
            auto currEdge = edgeData.push_back_n(std::max(numSteps, 1));
            for (int i = 0; i < numSteps - 1; ++i) {
                SkVector currNormal = SkVector::Make(prevNormal.fX * rotCos - prevNormal.fY * rotSin, prevNormal.fY * rotCos + prevNormal.fX * rotSin);
                setup_offset_edge(currEdge, inputPolygonVerts[currIndex] + prevNormal, inputPolygonVerts[currIndex] + currNormal, currIndex, currIndex);
            ...
            setup_offset_edge(currEdge, inputPolygonVerts[currIndex] + prevNormal, inputPolygonVerts[currIndex] + normals[currIndex], currIndex, currIndex);
            currEdge->fPrev = prevEdge;
            if (prevEdge) {
                prevEdge->fNext = currEdge;
            }
        ...
        setup_offset_edge(currEdge, inputPolygonVerts[currIndex] + normals[currIndex], inputPolygonVerts[nextIndex] + normals[currIndex], currIndex, nextIndex);
        currEdge->fPrev = prevEdge;
        if (prevEdge) {
            prevEdge->fNext = currEdge;
        }
    ...
    // TODO: look at the main algorithm and see if we can detect these better
    offsetPolygon->reset();
    if (!head || offsetVertexCount == 0 || offsetVertexCount >= std::numeric_limits<uint16_t>::max()) {
        return false;
    }
    ...
    *offsetPolygon->append() = head->fIntersection;
    ...
    currEdge = head->fNext;
    while (currEdge != head) {
        if (!SkPointPriv::EqualsWithinTolerance(currEdge->fIntersection, (*offsetPolygon)[currIndex], kCleanupTolerance)) {
    ...
    // make sure the first and last points aren't coincident
    if (currIndex >= 1 && SkPointPriv::EqualsWithinTolerance((*offsetPolygon)[0], (*offsetPolygon)[currIndex], kCleanupTolerance)) {
    ...
    return (winding * offsetWinding > 0 && SkIsSimplePolygon(offsetPolygon->begin(), offsetPolygon->size()));
}
...
static void compute_triangle_bounds(const SkPoint& p0, const SkPoint& p1, const SkPoint& p2, SkRect* bounds)
{
    ...
    min = skvx::min(min, xy);
    max = skvx::max(max, xy);
    bounds->setLTRB(std::min(min[0], min[2]), std::min(min[1], min[3]), std::max(max[0], max[2]), std::max(max[1], max[3]));
}
...
public:
    bool init(const SkRect& bounds, int vertexCount)
    {
        ...
        fGridConversion.set(sk_ieee_float_divide(fHCount - 0.001f, width), sk_ieee_float_divide(fVCount - 0.001f, height));
    ...
    bool checkTriangle(const SkPoint& p0, const SkPoint& p1, const SkPoint& p2, uint16_t ignoreIndex0, uint16_t ignoreIndex1) const
    {
        ...
        for (int v = v0; v <= v1; ++v) {
            for (int h = h0; h <= h1; ++h) {
                int i = v * fHCount + h;
                for (SkTInternalLList<TriangulationVertex>::Iter reflexIter = fGrid[i].begin(); reflexIter != fGrid[i].end(); ++reflexIter) {
...
static void reclassify_vertex(
    TriangulationVertex* p, const SkPoint* polygonVerts, int winding, ReflexHash* reflexHash, SkTInternalLList<TriangulationVertex>* convexList)
{
...
bool SkTriangulateSimplePolygon(const SkPoint* polygonVerts, uint16_t* indexMap, int polygonSize, SkTDArray<uint16_t>* triangleIndices)
{
    ...
    for (int currIndex = 0; currIndex < polygonSize; prevIndex = currIndex, ++currIndex) {
        ...
        if (TriangulationVertex::VertexType::kConvex == currType) {
            ...
            if (TriangulationVertex::VertexType::kReflex == prevType || TriangulationVertex::VertexType::kReflex == nextType) {
    ...
    while (vertexCount > 3) {
        ...
        for (SkTInternalLList<TriangulationVertex>::Iter convexIter = convexList.begin(); convexIter != convexList.end(); ++convexIter) {
            ...
            bool failed = reflexHash.checkTriangle(p0->fPosition, earVertex->fPosition, p2->fPosition, p0->fIndex, p2->fIndex);
            if (failed) {
                continue;
            }
    ...
    // output indices
    for (SkTInternalLList<TriangulationVertex>::Iter vertexIter = convexList.begin(); vertexIter != convexList.end(); ++vertexIter) {
