...
public:
    ...
    sk_sp<SkVertices> releaseVertices()
    {
        ...
        return SkVertices::MakeCopy(
            SkVertices::kTriangles_VertexMode, this->vertexCount(), fPositions.begin(), nullptr, fColors.begin(), this->indexCount(), fIndices.begin());
    }
...
protected:
    ...
    void stitchConcaveRings(
        const SkTDArray<SkPoint>& umbraPolygon, SkTDArray<int>* umbraIndices, const SkTDArray<SkPoint>& penumbraPolygon, SkTDArray<int>* penumbraIndices);
...
SkBaseShadowTessellator::SkBaseShadowTessellator(const SkPoint3& zPlaneParams, const SkRect& bounds, bool transparent)
...
void SkBaseShadowTessellator::finishPathPolygon()
{
    ...
    if (fPathPolygon.size() > 2) {
        ...
        if (!checkConvexity(fPathPolygon[fPathPolygon.size() - 2], fPathPolygon[fPathPolygon.size() - 1], fPathPolygon[0])) {
...
bool SkBaseShadowTessellator::computeConvexShadow(SkScalar inset, SkScalar outset, bool doClip)
{
    ...
    SkScalar minDistSq = SkPointPriv::DistanceToLineSegmentBetweenSqd(fCentroid, fPathPolygon[0], fPathPolygon[1]);
    SkRect bounds;
    ...
    if (inset > SK_ScalarNearlyZero) {
        ...
        // generate inner ring
        if (!SkInsetConvexPolygon(&fPathPolygon[0], fPathPolygon.size(), inset, &insetPolygon)) {
            // not ideal, but in this case we'll inset using the centroid
            fValidUmbra = false;
        }
    }
    const SkTDArray<SkPoint>& umbraPolygon = (inset > SK_ScalarNearlyZero) ? insetPolygon : fPathPolygon;
    ...
    if (this->addArc(fFirstOutset, outset, false)) {
        if (fFirstUmbraOutside) {
            this->appendTriangle(fFirstVertexIndex, fPositions.size() - 1, fFirstVertexIndex + 2);
        } else {
            this->appendTriangle(fFirstVertexIndex, fPositions.size() - 1, fFirstVertexIndex + 1);
        }
    } else {
...
void SkBaseShadowTessellator::addEdge(
    const SkPoint& nextPoint, const SkVector& nextNormal, SkColor umbraColor, const SkTDArray<SkPoint>& umbraPolygon, bool lastEdge, bool doClip)
{
    ...
    int prevPenumbraIndex = duplicate || (currUmbraIndex == fFirstVertexIndex) ? fPositions.size() - 1 : fPositions.size() - 2;
    if (!duplicate) {
        ...
        if (fTransparent) {
            ...
            SkPoint clipPoint;
            bool isOutside = lastEdge ? fFirstUmbraOutside : this->clipUmbraPoint(fPositions[currUmbraIndex], fCentroid, &clipPoint);
            if (isOutside) {
                if (!lastEdge) {
                    fPositions.push_back(clipPoint);
                    fColors.push_back(umbraColor);
...
bool SkBaseShadowTessellator::computeConcaveShadow(SkScalar inset, SkScalar outset)
{
    ...
    inset = std::min(inset, std::min(SkTAbs(SkRectPriv::HalfWidth(fPathBounds)), SkTAbs(SkRectPriv::HalfHeight(fPathBounds))));
    // generate inner ring
    SkTDArray<SkPoint> umbraPolygon;
    SkTDArray<int> umbraIndices;
    umbraIndices.reserve(fPathPolygon.size());
    if (!SkOffsetSimplePolygon(&fPathPolygon[0], fPathPolygon.size(), fPathBounds, inset, &umbraPolygon, &umbraIndices)) {
    ...
    if (!SkOffsetSimplePolygon(&fPathPolygon[0], fPathPolygon.size(), fPathBounds, -outset, &penumbraPolygon, &penumbraIndices)) {
        // TODO: figure out how to handle this case
        return false;
    }
    ...
    const SkTDArray<SkPoint>& umbraPolygon, SkTDArray<int>* umbraIndices, const SkTDArray<SkPoint>& penumbraPolygon, SkTDArray<int>* penumbraIndices)
{
    ...
    while ((*penumbraIndices)[nextPenumbra] <= maxPenumbraIndex || (*umbraIndices)[nextUmbra] <= maxUmbraIndex) {
        ...
        while ((*penumbraIndices)[nextPenumbra] < (*umbraIndices)[nextUmbra] && (*penumbraIndices)[nextPenumbra] <= maxPenumbraIndex) {
        ...
        while ((*umbraIndices)[nextUmbra] < (*penumbraIndices)[nextPenumbra] && (*umbraIndices)[nextUmbra] <= maxUmbraIndex) {
            // fill out umbra arc
            fPositions.push_back(umbraPolygon[nextUmbra]);
            fColors.push_back(kUmbraColor);
            int currUmbraIndex = fPositions.size() - 1;
    ...
    if (fTransparent) {
        SkTriangulateSimplePolygon(umbraPolygon.begin(), indexMap, umbraPolygon.size(), &fIndices);
    }
}
...
void SkBaseShadowTessellator::handleLine(const SkPoint& p)
{
    ...
    if (fPathPolygon.size() > 1) {
        if (!checkConvexity(fPathPolygon[fPathPolygon.size() - 2], fPathPolygon[fPathPolygon.size() - 1], pSanitized)) {
            // remove collinear point
            fPathPolygon.pop_back();
...
public:
    SkSpotShadowTessellator(const SkPath& path, const SkMatrix& ctm, const SkPoint3& zPlaneParams, const SkPoint3& lightPos, SkScalar lightRadius,
        bool transparent, bool directional);

private:
    bool computeClipAndPathPolygons(const SkPath& path, const SkMatrix& ctm, const SkMatrix& shadowTransform);
...
SkSpotShadowTessellator::SkSpotShadowTessellator(
    const SkPath& path, const SkMatrix& ctm, const SkPoint3& zPlaneParams, const SkPoint3& lightPos, SkScalar lightRadius, bool transparent, bool directional)
    : INHERITED(zPlaneParams, path.getBounds(), transparent)
{
    ...
    SkScalar outset;
    if (!SkDrawShadowMetrics::GetSpotShadowTransform(lightPos, lightRadius, ctm, zPlaneParams, path.getBounds(), directional, &shadowTransform, &outset)) {
        return;
    }
...
bool SkSpotShadowTessellator::computeClipAndPathPolygons(const SkPath& path, const SkMatrix& ctm, const SkMatrix& shadowTransform)
{
    ...
    while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
        ...
        case SkPath::kCubic_Verb:
            ...
            // point at t = 5/16
            curvePoint.fX = kA * clipPts[0].fX + kB * clipPts[1].fX + kC * clipPts[2].fX + kD * clipPts[3].fX;
            curvePoint.fY = kA * clipPts[0].fY + kB * clipPts[1].fY + kC * clipPts[2].fY + kD * clipPts[3].fY;
            this->addToClip(curvePoint);
            // point at t = 11/16
            curvePoint.fX = kD * clipPts[0].fX + kC * clipPts[1].fX + kB * clipPts[2].fX + kA * clipPts[3].fX;
            curvePoint.fY = kD * clipPts[0].fY + kC * clipPts[1].fY + kB * clipPts[2].fY + kA * clipPts[3].fY;
            this->addToClip(curvePoint);
...
sk_sp<SkVertices> SkShadowTessellator::MakeSpot(
    const SkPath& path, const SkMatrix& ctm, const SkPoint3& zPlane, const SkPoint3& lightPos, SkScalar lightRadius, bool transparent, bool directional)
{
    if (!ctm.mapRect(path.getBounds()).isFinite() || !zPlane.isFinite() || !lightPos.isFinite() || !(lightPos.fZ >= SK_ScalarNearlyZero)
        || !SkIsFinite(lightRadius) || !(lightRadius >= SK_ScalarNearlyZero)) {
        return nullptr;
    }
    SkSpotShadowTessellator spotTess(path, ctm, zPlane, lightPos, lightRadius, transparent, directional);
