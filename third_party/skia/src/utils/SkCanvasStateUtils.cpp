...
static void setup_canvas_from_MC_state(const SkMCState& state, SkCanvas* canvas)
{
    ...
    if (state.clipRectCount > 0) {
        bounds.setLTRB(state.clipRects[0].left, state.clipRects[0].top, state.clipRects[0].right, state.clipRects[0].bottom);
        for (int i = 1; i < state.clipRectCount; ++i) {
            bounds.join({ state.clipRects[i].left, state.clipRects[i].top, state.clipRects[i].right, state.clipRects[i].bottom });
        }
    }
...
static std::unique_ptr<SkCanvas> make_canvas_from_canvas_layer(const SkCanvasLayerState& layerState)
{
    ...
    bitmap.installPixels(
        SkImageInfo::Make(layerState.width, layerState.height, colorType, kPremul_SkAlphaType), layerState.raster.pixels, (size_t)layerState.raster.rowBytes);
...
std::unique_ptr<SkCanvas> SkCanvasStateUtils::MakeFromCanvasState(const SkCanvasState* state)
{
    ...
    for (int i = state_v1->layerCount - 1; i >= 0; --i) {
        ...
        canvas->pushCanvas(std::move(canvasLayer), SkIPoint::Make(state_v1->layers[i].x, state_v1->layers[i].y));
    }
