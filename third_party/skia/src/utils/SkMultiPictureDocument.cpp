...
struct MultiPictureDocument final : public SkDocument {
    ...
    MultiPictureDocument(SkWStream* s, const SkSerialProcs* procs, std::function<void(const SkPicture*)> onEndPage)
        : SkDocument(s)
...
struct PagerCanvas : public SkNWayCanvas {
    ...
    PagerCanvas(SkISize wh, SkDocumentPage* dst, int count)
        : SkNWayCanvas(wh.width(), wh.height())
        , fDst(dst)
        , fCount(count)
    {
        this->nextCanvas();
...
sk_sp<SkDocument> Make(SkWStream* dst, const SkSerialProcs* procs, std::function<void(const SkPicture*)> onEndPage)
{
...
bool Read(SkStreamSeekable* src, SkDocumentPage* dstArray, int dstArrayCount, const SkDeserialProcs* procs)
{
    ...
    for (int i = 0; i < dstArrayCount; ++i) {
        joined = SkSize { std::max(joined.width(), dstArray[i].fSize.width()), std::max(joined.height(), dstArray[i].fSize.height()) };
    }
