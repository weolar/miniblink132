...
{
    SkUniqueCFRef<CFNumberRef> opszValueNumber(CFNumberCreate(kCFAllocatorDefault, kCFNumberDoubleType, &opsz));
...
{
    int zero = 0;
    SkUniqueCFRef<CFNumberRef> unscaledTrackingNumber(CFNumberCreate(kCFAllocatorDefault, kCFNumberIntType, &zero));
...
SkUniqueCFRef<CTFontRef> SkCTFontCreateExactCopy(CTFontRef baseFont, CGFloat textSize, OpszVariation opszVariation)
{
    ...
    if (opszVariation.isSet) {
        ...
        double opsz_val;
        if (!opsz || CFGetTypeID(opsz.get()) != CFNumberGetTypeID() || !CFNumberGetValue(static_cast<CFNumberRef>(opsz.get()), kCFNumberDoubleType, &opsz_val)
            || opsz_val <= 0) {
    ...
    return SkUniqueCFRef<CTFontRef>(CTFontCreateCopyWithAttributes(baseFont, textSize, nullptr, desc.get()));
}
