...
static bool get_bitmap_info(SkColorType skColorType, SkAlphaType skAlphaType, size_t* bitsPerComponent, CGBitmapInfo* info, bool* upscaleTo32)
{
    if (upscaleTo32) {
        *upscaleTo32 = false;
    }
...
static std::unique_ptr<SkBitmap> prepare_for_image_ref(const SkBitmap& bm, size_t* bitsPerComponent, CGBitmapInfo* info)
{
    bool upscaleTo32;
...
CGImageRef SkCreateCGImageRef(const SkBitmap& bm)
{
    ...
    SkUniqueCFRef<CGDataProviderRef> dataRef(
        CGDataProviderCreateWithData(bitmap.release(), pixels, s, [](void* p, const void*, size_t) { delete reinterpret_cast<SkBitmap*>(p); }));

    SkUniqueCFRef<CGColorSpaceRef> colorSpace(SkCreateCGColorSpace(bm.colorSpace()));
    return CGImageCreate(pm.width(), pm.height(), bitsPerComponent, pm.info().bytesPerPixel() * CHAR_BIT, pm.rowBytes(), colorSpace.get(), info, dataRef.get(),
...
CGContextRef SkCreateCGContext(const SkPixmap& pmap)
{
    ...
    CGContextRef cg = CGBitmapContextCreate(pmap.writable_addr(), pmap.width(), pmap.height(), bitsPerComponent, rb, cs.get(), cg_bitmap_info);
    return cg;
}
...
bool SkCopyPixelsFromCGImage(const SkImageInfo& info, size_t rowBytes, void* pixels, CGImageRef image)
{
    ...
    SkUniqueCFRef<CGContextRef> cg(CGBitmapContextCreate(pixels, info.width(), info.height(), bitsPerComponent, rowBytes, cs.get(), cg_bitmap_info));
    if (!cg) {
        return false;
    }
...
CGDataProviderRef SkCreateCGDataProvider(sk_sp<SkData> data)
{
    ...
    CGDataProviderRef result = CGDataProviderCreateWithData(
        data.get(), data->data(), data->size(), [](void* info, const void*, size_t) { reinterpret_cast<SkData*>(info)->unref(); });
    if (!result) {
        return nullptr;
    }
...
sk_sp<SkColorSpace> SkMakeColorSpaceFromCGColorSpace(CGColorSpaceRef cgColorSpace)
{
    ...
    if (!skcms_Parse(CFDataGetBytePtr(iccData.get()), CFDataGetLength(iccData.get()), &iccProfile)) {
        return nullptr;
    }
...
CGColorSpaceRef SkCreateCGColorSpace(const SkColorSpace* space)
{
    ...
    CGColorSpaceRef result = CGColorSpaceCreateICCBased(kNumComponents, kComponentRanges, iccDataProvider.get(), cgSRGB);
    if (!result) {
        return cgSRGB;
    }
