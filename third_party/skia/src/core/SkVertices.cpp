...
struct SkVertices::Sizes {
    Sizes(const Desc& desc)
    {
        ...
        fTotal = safe.add(sizeof(SkVertices), safe.add(fVSize, safe.add(fTSize, safe.add(fCSize, fISize))));
...
sk_sp<SkVertices> SkVertices::MakeCopy(
    VertexMode mode, int vertexCount, const SkPoint pos[], const SkPoint texs[], const SkColor colors[], int indexCount, const uint16_t indices[])
{
...
sk_sp<SkVertices> SkVerticesPriv::Decode(SkReadBuffer& buffer)
{
    auto decode = [](SkReadBuffer& buffer) -> sk_sp<SkVertices> {
        ...
        const SkVertices::VertexMode mode = safe.checkLE<SkVertices::VertexMode>(packed & kMode_Mask, SkVertices::kLast_VertexMode);
        ...
        if (!safe || !buffer.isValid() || attrCount || mode == SkVertices::kTriangleFan_VertexMode) {
            return nullptr;
        }
