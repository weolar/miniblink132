...
sk_sp<SkPicture> SkPicture::Forwardport(const SkPictInfo& info, const SkPictureData* data, SkReadBuffer* buffer)
{
    if (!data) {
        return nullptr;
    }
...
sk_sp<SkPicture> SkPicture::MakeFromStreamPriv(SkStream* stream, const SkDeserialProcs* procsPtr, SkTypefacePlayback* typefaces, int recursionLimit)
{
    if (recursionLimit <= 0) {
        return nullptr;
    }
    ...
    case kPictureData_TrailingStreamByteAfterPictInfo: {
        std::unique_ptr<SkPictureData> data(SkPictureData::CreateFromStream(stream, info, procs, typefaces, recursionLimit));
...
sk_sp<SkPicture> SkPicture::MakePlaceholder(SkRect cull)
{
    struct Placeholder : public SkPicture {
        explicit Placeholder(SkRect cull)
            : fCull(cull)
        {
        }
