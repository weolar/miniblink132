...
SkScalerContextRec SkScalerContext::PreprocessRec(const SkTypeface& typeface, const SkScalerContextEffects& effects, const SkDescriptor& desc)
{
    SkScalerContextRec rec = *static_cast<const SkScalerContextRec*>(desc.findEntry(kRec_SkDescriptorTag, nullptr));
    ...
    if (rec.fMaskFormat == SkMask::kA8_Format) {
        U8CPU lum = SkComputeLuminance(SkColorGetR(lumColor), SkColorGetG(lumColor), SkColorGetB(lumColor));
...
const SkMaskGamma& SkScalerContextRec::CachedMaskGamma(uint8_t contrast, uint8_t gamma)
{
    ...
    if (defaultContrast == contrast && defaultGamma == gamma) {
        if (!gDefaultMaskGamma) {
            gDefaultMaskGamma = new SkMaskGamma(ExternalContrastFromInternal(contrast), ExternalGammaFromInternal(gamma));
        }
        return *gDefaultMaskGamma;
    }
    if (!gMaskGamma || gContrast != contrast || gGamma != gamma) {
        SkSafeUnref(gMaskGamma);
        gMaskGamma = new SkMaskGamma(ExternalContrastFromInternal(contrast), ExternalGammaFromInternal(gamma));
        gContrast = contrast;
        gGamma = gamma;
    }
    return *gMaskGamma;
...
size_t SkScalerContext::GetGammaLUTSize(SkScalar contrast, SkScalar deviceGamma, int* width, int* height)
{
    SkAutoMutexExclusive ama(mask_gamma_cache_mutex());
    const SkMaskGamma& maskGamma = SkScalerContextRec::CachedMaskGamma(
        SkScalerContextRec::InternalContrastFromExternal(contrast), SkScalerContextRec::InternalGammaFromExternal(deviceGamma));
...
void SkScalerContext::GenerateMetricsFromPath(
    SkGlyph* glyph, const SkPath& devPath, SkMask::Format format, const bool verticalLCD, const bool a8FromLCD, const bool hairline)
{
    // Only BW, A8, and LCD16 can be produced from paths.
    if (glyph->fMaskFormat != SkMask::kBW_Format && glyph->fMaskFormat != SkMask::kA8_Format && glyph->fMaskFormat != SkMask::kLCD16_Format) {
...
SkGlyph SkScalerContext::internalMakeGlyph(SkPackedGlyphID packedID, SkMask::Format format, SkArenaAlloc* alloc)
{
    ...
    if (mx.computeFromPath || (fGenerateImageFromPath && !mx.neverRequestPath)) {
        SkDEBUGCODE(glyph.fAdvancesBoundsFormatAndInitialPathDone = true;) this->internalGetPath(glyph, alloc);
...
#define LCD_PER_PIXEL 3
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (int sample_x = -4; sample_x < sample_width + 4; sample_x += 4) {
            ...
            for (int sample_index = std::max(0, sample_x - 4), coeff_index = sample_index - (sample_x - 4); sample_index < std::min(sample_x + 8, sample_width);
...
void SkScalerContext::GenerateImageFromPath(SkMaskBuilder& dstMask, const SkPath& path, const SkMaskGamma::PreBlend& maskPreBlend, const bool doBGR,
    const bool verticalLCD, const bool a8FromLCD, const bool hairline)
{
    SkASSERT(dstMask.fFormat == SkMask::kBW_Format || dstMask.fFormat == SkMask::kA8_Format || dstMask.fFormat == SkMask::kLCD16_Format);
    ...
    SkMatrix matrix;
    matrix.setTranslate(-SkIntToScalar(dstMask.fBounds.fLeft), -SkIntToScalar(dstMask.fBounds.fTop));

    paint.setStroke(hairline);
    paint.setAntiAlias(SkMask::kBW_Format != dstMask.fFormat);

    const bool fromLCD = (dstMask.fFormat == SkMask::kLCD16_Format) || (dstMask.fFormat == SkMask::kA8_Format && a8FromLCD);
    const bool intermediateDst = fromLCD || dstMask.fFormat == SkMask::kBW_Format;
    if (fromLCD) {
        if (verticalLCD) {
            dstW = 4 * dstH - 8;
            dstH = srcW;
            matrix.setAll(0, 4, -SkIntToScalar(dstMask.fBounds.fTop + 1) * 4, 1, 0, -SkIntToScalar(dstMask.fBounds.fLeft), 0, 0, 1);
        } else {
            dstW = 4 * dstW - 8;
            matrix.setAll(4, 0, -SkIntToScalar(dstMask.fBounds.fLeft + 1) * 4, 0, 1, -SkIntToScalar(dstMask.fBounds.fTop), 0, 0, 1);
        }
...
void SkScalerContext::getImage(const SkGlyph& origGlyph)
{
    ...
    if (fMaskFilter) {
        ...
        if (tmpGlyph.fMaskFormat == origGlyph.fMaskFormat && tmpGlyph.imageSize() <= origGlyph.imageSize()) {
    ...
    if (!fGenerateImageFromPath) {
        ...
        if (!devPath) {
            ...
            SkMaskBuilder mask(
                static_cast<uint8_t*>(unfilteredGlyph->fImage), unfilteredGlyph->iRect(), unfilteredGlyph->rowBytes(), unfilteredGlyph->maskFormat());
    ...
    if (fMaskFilter) {
        ...
        SkMaskBuilder dstMask = SkMaskBuilder(static_cast<uint8_t*>(origGlyph.fImage), origGlyph.iRect(), origGlyph.rowBytes(), origGlyph.maskFormat());
...
void SkScalerContext::internalGetPath(SkGlyph& glyph, SkArenaAlloc* alloc)
{
    ...
    if (fRec.fFrameWidth < 0 && fPathEffect == nullptr) {
        ...
        if (fRec.fFrameWidth >= 0) {
            ...
            rec.setStrokeParams((SkPaint::Cap)fRec.fStrokeCap, (SkPaint::Join)fRec.fStrokeJoin, fRec.fMiterLimit);
        }
...
bool SkScalerContextRec::computeMatrices(PreMatrixScale preMatrixScale, SkVector* s, SkMatrix* sA, SkMatrix* GsA, SkMatrix* G_inv, SkMatrix* A_out)
{
    ...
    if (skewedOrFlipped) {
        ...
        if (G_inv) {
            G_inv->setAll(G.get(SkMatrix::kMScaleX), -G.get(SkMatrix::kMSkewX), G.get(SkMatrix::kMTransX), -G.get(SkMatrix::kMSkewY), G.get(SkMatrix::kMScaleY),
                G.get(SkMatrix::kMTransY), G.get(SkMatrix::kMPersp0), G.get(SkMatrix::kMPersp1), G.get(SkMatrix::kMPersp2));
        }
    } else {
        GA = A;
        if (G_inv) {
    ...
    if (!skewedOrFlipped && ((PreMatrixScale::kFull == preMatrixScale) || (PreMatrixScale::kVertical == preMatrixScale && A.getScaleX() == A.getScaleY()))) {
...
{
    fLumBits = SkMaskGamma::CanonicalColor(SkColorSetRGB(SkColorGetR(c), SkColorGetG(c), SkColorGetB(c)));
}
...
void SkScalerContextRec::useStrokeForFakeBold()
{
    ...
    SkScalar fakeBoldScale = SkScalarInterpFunc(fTextSize, kStdFakeBoldInterpKeys, kStdFakeBoldInterpValues, kStdFakeBoldInterpLength);
...
void SkScalerContext::MakeRecAndEffects(const SkFont& font, const SkPaint& paint, const SkSurfaceProps& surfaceProps, SkScalerContextFlags scalerContextFlags,
    const SkMatrix& deviceMatrix, SkScalerContextRec* rec, SkScalerContextEffects* effects)
{
...
SkDescriptor* SkScalerContext::CreateDescriptorAndEffectsUsingPaint(const SkFont& font, const SkPaint& paint, const SkSurfaceProps& surfaceProps,
    SkScalerContextFlags scalerContextFlags, const SkMatrix& deviceMatrix, SkAutoDescriptor* ad, SkScalerContextEffects* effects)
{
...
static void generate_descriptor(const SkScalerContextRec& rec, const SkBinaryWriteBuffer& effectBuffer, SkDescriptor* desc)
{
    ...
    if (effectBuffer.bytesWritten() > 0) {
        effectBuffer.writeToMemory(desc->addEntry(kEffects_SkDescriptorTag, effectBuffer.bytesWritten(), nullptr));
    }

    desc->computeChecksum();
}

SkDescriptor* SkScalerContext::AutoDescriptorGivenRecAndEffects(const SkScalerContextRec& rec, const SkScalerContextEffects& effects, SkAutoDescriptor* ad)
{
...
std::unique_ptr<SkDescriptor> SkScalerContext::DescriptorGivenRecAndEffects(const SkScalerContextRec& rec, const SkScalerContextEffects& effects)
{
    SkBinaryWriteBuffer buf({});
...
bool SkScalerContext::CheckBufferSizeForRec(const SkScalerContextRec& rec, const SkScalerContextEffects& effects, size_t size)
{
    SkBinaryWriteBuffer buf({});
    return size >= calculate_size_and_flatten(rec, effects, &buf);
}

std::unique_ptr<SkScalerContext> SkScalerContext::MakeEmpty(sk_sp<SkTypeface> typeface, const SkScalerContextEffects& effects, const SkDescriptor* desc)
{
