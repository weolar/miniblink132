...
struct SkAAClip::RunHead {
    ...
    static Iter Iterate(const SkAAClip& clip)
    {
        ...
        return Iter(clip.getBounds().fTop, head->data(), head->yoffsets(), head->yoffsets() + head->fRowCount);
    }
};
...
bool SkAAClip::trimLeftRight()
{
    ...
    while (yoff < stop) {
        ...
        SkDEBUGCODE((void)compute_row_length(row, width);) yoff->fOffset += trim_row_left_right(row, width, leftZeros, riteZeros);
        SkDEBUGCODE((void)compute_row_length(base + yoff->fOffset, width - leftZeros - riteZeros);) yoff += 1;
    }
    return true;
}
...
bool SkAAClip::op(const SkRect& rect, SkClipOp op, bool doAA)
{
    if (!doAA) {
        ...
        if (!pixelBounds.intersect(rect.roundOut())) {
            ...
            SkAAClip rectClip;
            rectClip.setPath(SkPath::Rect(rect), op == SkClipOp::kDifference ? fBounds : pixelBounds,
                /*doAA=*/true);
...
static void merge(const uint8_t* SK_RESTRICT row, int rowN, const SkAlpha* SK_RESTRICT srcAA, const int16_t* SK_RESTRICT srcRuns, SkAlpha* SK_RESTRICT dstAA,
...
static inline uint16_t mergeOne(uint16_t value, unsigned alpha)
{
    ...
    return SkPackRGB16(SkMulDiv255Round(r, alpha), SkMulDiv255Round(g, alpha), SkMulDiv255Round(b, alpha));
}

template <typename T> void mergeT(const void* inSrc, int srcN, const uint8_t* SK_RESTRICT row, int rowN, void* inDst)
{
...
void SkAAClipBlitter::blitMask(const SkMask& origMask, const SkIRect& clip)
{
    ...
    if (SkMask::kBW_Format == origMask.fFormat) {
        ...
        grayMask.image() = reinterpret_cast<uint8_t*>(fGrayMaskScratch.reset(size, SkAutoMalloc::kReuse_OnShrink));
