...
SkColorSpace::SkColorSpace(const skcms_TransferFunction& transferFn, const skcms_Matrix3x3& toXYZD50)
...
{
    static SkColorSpace* cs = SkColorSpaceSingletonFactory::Make(SkNamedTransferFn::kSRGB, SkNamedGamut::kSRGB);
    return cs;
}

SkColorSpace* sk_srgb_linear_singleton()
{
    static SkColorSpace* cs = SkColorSpaceSingletonFactory::Make(SkNamedTransferFn::kLinear, SkNamedGamut::kSRGB);
    return cs;
}
...
bool SkColorSpace::Equals(const SkColorSpace* x, const SkColorSpace* y)
{
    ...
    if (x->hash() == y->hash()) {
        ...
        auto equiv = [](float X, float Y) { return (X == Y) || (std::isnan(X) && std::isnan(Y)); };
