...
{
    SkASSERTF_RELEASE(this->tryAllocPixelsFlags(info, flags), "ColorType:%d AlphaType:%d [w:%d h:%d] rb:%zu flags: 0x%x", info.colorType(), info.alphaType(),
        info.width(), info.height(), this->rowBytes(), flags);
}

void SkBitmap::allocPixels(const SkImageInfo& info, size_t rowBytes)
{
    SkASSERTF_RELEASE(this->tryAllocPixels(info, rowBytes), "ColorType:%d AlphaType:%d [w:%d h:%d] rb:%zu", info.colorType(), info.alphaType(), info.width(),
        info.height(), this->rowBytes());
}
...
bool SkBitmap::installPixels(const SkImageInfo& requestedInfo, void* pixels, size_t rb, void (*releaseProc)(void* addr, void* context), void* context)
{
    ...
    // setInfo may have corrected info (e.g. 565 is always opaque).
    const SkImageInfo& correctedInfo = this->info();
    this->setPixelRef(SkMakePixelRefWithProc(correctedInfo.width(), correctedInfo.height(), rb, pixels, releaseProc, context), 0, 0);
    SkDEBUGCODE(this->validate();) return true;
}
...
bool SkBitmap::installMaskPixels(SkMaskBuilder& mask)
{
    ...
    return this->installPixels(SkImageInfo::MakeA8(mask.fBounds.width(), mask.fBounds.height()), mask.image(), mask.fRowBytes);
}
...
bool SkBitmap::writePixels(const SkPixmap& src, int dstX, int dstY)
{
    ...
    if (!SkConvertPixels(dstInfo, dstPixels, this->rowBytes(), rec.fInfo, rec.fPixels, rec.fRowBytes)) {
        return false;
    }
    this->notifyPixelsChanged();
    return true;
...
static bool GetBitmapAlpha(const SkBitmap& src, uint8_t* SK_RESTRICT alpha, int alphaRowBytes)
{
    ...
    return SkConvertPixels(SkImageInfo::MakeA8(pmap.width(), pmap.height()), alpha, alphaRowBytes, pmap.info(), pmap.addr(), pmap.rowBytes());
}
...
bool SkBitmap::extractAlpha(SkBitmap* dst, const SkPaint* paint, Allocator* allocator, SkIPoint* offset) const
{
    ...
    tmpBitmap.setInfo(SkImageInfo::MakeA8(dstM.fBounds.width(), dstM.fBounds.height()), dstM.fRowBytes);
    if (!tmpBitmap.tryAllocPixels(allocator)) {
        // Allocation of pixels for alpha bitmap failed.
        SkDebugf("extractAlpha failed to allocate (%d,%d) alpha bitmap\n", tmpBitmap.width(), tmpBitmap.height());
        return false;
...
{
    return SkImages::RasterFromBitmap(*this);
}

sk_sp<SkShader> SkBitmap::makeShader(const SkSamplingOptions& sampling, const SkMatrix& lm) const
{
...
sk_sp<SkShader> SkBitmap::makeShader(SkTileMode tmx, SkTileMode tmy, const SkSamplingOptions& sampling, const SkMatrix& lm) const
{
    if (!lm.invert(nullptr)) {
        return nullptr;
    }
    return SkImageShader::Make(SkMakeImageFromRasterBitmap(*this, kIfMutable_SkCopyPixelsMode), tmx, tmy, sampling, &lm);
}
