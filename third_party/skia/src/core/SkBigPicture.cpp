...
SkBigPicture::SkBigPicture(
    const SkRect& cull, sk_sp<SkRecord> record, std::unique_ptr<SnapshotArray> drawablePicts, sk_sp<SkBBoxHierarchy> bbh, size_t approxBytesUsedBySubPictures)
    : fCullRect(cull)
...
void SkBigPicture::playback(SkCanvas* canvas, AbortCallback* callback) const
{
    ...
    SkRecordDraw(*fRecord, canvas, this->drawablePicts(), nullptr, this->drawableCount(), useBBH ? fBBH.get() : nullptr, callback);
}
