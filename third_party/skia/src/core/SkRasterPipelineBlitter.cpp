...
public:
    ...
    static SkBlitter* Create(const SkPixmap& dst, const SkPaint& paint, const SkColor4f& dstPaintColor, SkArenaAlloc* alloc,
        const SkRasterPipeline& shaderPipeline, bool is_opaque, bool is_constant, const SkShader* clipShader);
...
private:
    ...
    std::function<void(size_t, size_t, size_t, size_t)> fBlitRect, fBlitAntiH, fBlitMaskA8, fBlitMaskLCD16, fBlitMask3D;
...
static SkColor4f paint_color_to_dst(const SkPaint& paint, const SkPixmap& dst)
{
    ...
    SkColorSpaceXformSteps(sk_srgb_singleton(), kUnpremul_SkAlphaType, dst.colorSpace(), kUnpremul_SkAlphaType).apply(paintColor.vec());
    return paintColor;
}

SkBlitter* SkCreateRasterPipelineBlitter(
    const SkPixmap& dst, const SkPaint& paint, const SkMatrix& ctm, SkArenaAlloc* alloc, sk_sp<SkShader> clipShader, const SkSurfaceProps& props)
{
    ...
    if (!shader) {
        ...
        bool is_opaque = dstPaintColor.fA == 1.0f, is_constant = true;
        return SkRasterPipelineBlitter::Create(dst, paint, dstPaintColor, alloc, shaderPipeline, is_opaque, is_constant, clipShader.get());
    }

    bool is_opaque = shader->isOpaque() && dstPaintColor.fA == 1.0f;
    bool is_constant = shader->isConstant();

    if (shader->appendRootStages({ &shaderPipeline, alloc, dstCT, dstCS, dstPaintColor, props }, ctm)) {
        if (dstPaintColor.fA != 1.0f) {
            shaderPipeline.append(SkRasterPipelineOp::scale_1_float, alloc->make<float>(dstPaintColor.fA));
        }
        return SkRasterPipelineBlitter::Create(dst, paint, dstPaintColor, alloc, shaderPipeline, is_opaque, is_constant, clipShader.get());
    }
...
SkBlitter* SkCreateRasterPipelineBlitter(
    const SkPixmap& dst, const SkPaint& paint, const SkRasterPipeline& shaderPipeline, bool is_opaque, SkArenaAlloc* alloc, sk_sp<SkShader> clipShader)
{
    bool is_constant = false; // If this were the case, it'd be better to just set a paint color.
    return SkRasterPipelineBlitter::Create(dst, paint, paint_color_to_dst(paint, dst), alloc, shaderPipeline, is_opaque, is_constant, clipShader.get());
}

SkBlitter* SkRasterPipelineBlitter::Create(const SkPixmap& dst, const SkPaint& paint, const SkColor4f& dstPaintColor, SkArenaAlloc* alloc,
    const SkRasterPipeline& shaderPipeline, bool is_opaque, bool is_constant, const SkShader* clipShader)
{
    ...
    if (auto colorFilter = paint.getColorFilter()) {
        ...
        SkStageRec rec = { colorPipeline, alloc, dst.colorType(), dst.colorSpace(), dstPaintColor, props };
    ...
    if (is_constant && as_BB(blender)->asBlendMode() == SkBlendMode::kSrc && dst.info().bytesPerPixel() <= static_cast<int>(sizeof(blitter->fMemsetColor))) {
        ...
        case 1:
            blitter->fMemset2D
                = [](SkPixmap* dst, int x, int y, int w, int h, uint64_t c) { SkOpts::rect_memset16(dst->writable_addr16(x, y), c, w, dst->rowBytes(), h); };
            break;

        case 2:
            blitter->fMemset2D
                = [](SkPixmap* dst, int x, int y, int w, int h, uint64_t c) { SkOpts::rect_memset32(dst->writable_addr32(x, y), c, w, dst->rowBytes(), h); };
            break;

        case 3:
            blitter->fMemset2D
                = [](SkPixmap* dst, int x, int y, int w, int h, uint64_t c) { SkOpts::rect_memset64(dst->writable_addr64(x, y), c, w, dst->rowBytes(), h); };
            break;
...

    {
        ...
        SkStageRec rec = { blendPipeline, alloc, dst.colorType(), dst.colorSpace(), dstPaintColor, props };
...
void SkRasterPipelineBlitter::blitRectWithTrace(int x, int y, int w, int h, bool trace)
{
    ...
    if (!fBlitRect) {
        ...
        if (fBlendMode == SkBlendMode::kSrcOver && (fDst.info().colorType() == kRGBA_8888_SkColorType || fDst.info().colorType() == kBGRA_8888_SkColorType)
            && !fDst.colorSpace() && fDst.info().alphaType() != kUnpremul_SkAlphaType && fDitherRate == 0.0f) {
...
void SkRasterPipelineBlitter::blitAntiH(int x, int y, const SkAlpha aa[], const int16_t runs[])
{
    if (!fBlitAntiH) {
        ...
        p.extend(fColorPipeline);
        p.appendClampIfNormalized(fDst.info());
        if (fBlendMode.has_value() && SkBlendMode_ShouldPreScaleCoverage(*fBlendMode, /*rgb_coverage=*/false)) {
    ...
    for (int16_t run = *runs; run > 0; run = *runs) {
        ...
        case 0x00:
            break;
        case 0xff:
            this->blitRectWithTrace(x, y, run, 1, false);
            break;
        default:
...
void SkRasterPipelineBlitter::blitMask(const SkMask& mask, const SkIRect& clip)
{
    ...
    SkASSERT(mask.fFormat == SkMask::kA8_Format || mask.fFormat == SkMask::kLCD16_Format || mask.fFormat == SkMask::k3D_Format);
    ...
    if (mask.fFormat == SkMask::kLCD16_Format && !fBlitMaskLCD16) {
        ...
        p.extend(fColorPipeline);
        p.appendClampIfNormalized(fDst.info());
        if (fBlendMode.has_value() && SkBlendMode_ShouldPreScaleCoverage(*fBlendMode, /*rgb_coverage=*/true)) {
