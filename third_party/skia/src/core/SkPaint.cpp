...
#define EQUAL(field) (a.field == b.field)
    return EQUAL(fPathEffect) && EQUAL(fShader) && EQUAL(fMaskFilter) && EQUAL(fColorFilter) && EQUAL(fBlender) && EQUAL(fImageFilter) && EQUAL(fColor4f)
        && EQUAL(fWidth) && EQUAL(fMiterLimit) && EQUAL(fBitfieldsUInt);
#undef EQUAL
}

#define DEFINE_FIELD_REF(type)                                                                                                                                 \
    sk_sp<Sk##type> SkPaint::ref##type() const                                                                                                                 \
    {                                                                                                                                                          \
        return f##type;                                                                                                                                        \
    }
...
#define DEFINE_FIELD_SET(Field)                                                                                                                                \
    void SkPaint::set##Field(sk_sp<Sk##Field> f)                                                                                                               \
    {                                                                                                                                                          \
        f##Field = std::move(f);                                                                                                                               \
    }
...
const SkRect& SkPaint::doComputeFastBounds(const SkRect& origSrc, SkRect* storage, Style style) const
{
    SkASSERT(storage);
