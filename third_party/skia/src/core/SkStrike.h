...
public:
    SkStrike(SkStrikeCache* strikeCache, const SkStrikeSpec& strikeSpec, std::unique_ptr<SkScalerContext> scaler, const SkFontMetrics* metrics,
    ...
    const SkPath* mergePath(SkGlyph* glyph, const SkPath* path, bool hairline, bool modified) SK_EXCLUDES(fStrikeLock);
...
private:
    ...
    SkSpan<const SkGlyph*> internalPrepare(SkSpan<const SkGlyphID> glyphIDs, PathDetail pathDetail, const SkGlyph** results) SK_REQUIRES(fStrikeLock);
    ...
    skia_private::THashTable<SkGlyphDigest, SkPackedGlyphID, SkGlyphDigest> fDigestForPackedGlyphID SK_GUARDED_BY(fStrikeLock);
