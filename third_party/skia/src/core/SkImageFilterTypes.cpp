...
std::pair<bool, bool> are_axes_nearly_integer_aligned(const LayerSpace<SkMatrix>& m, LayerSpace<SkIPoint>* out = nullptr)
{
    ...
    //            [0 1 ty]
    //            [0 0  1]
    bool affine = SkScalarNearlyEqual(m.rc(2, 0) * invW, 0.f, kRoundEpsilon) && SkScalarNearlyEqual(m.rc(2, 1) * invW, 0.f, kRoundEpsilon);
    if (!affine) {
        return { false, false };
    }

    bool xAxis = SkScalarNearlyEqual(1.f, m.rc(0, 0) * invW, kRoundEpsilon) && SkScalarNearlyEqual(0.f, m.rc(0, 1) * invW, kRoundEpsilon)
        && SkScalarNearlyEqual(tx, m.rc(0, 2) * invW, kRoundEpsilon);
    bool yAxis = SkScalarNearlyEqual(0.f, m.rc(1, 0) * invW, kRoundEpsilon) && SkScalarNearlyEqual(1.f, m.rc(1, 1) * invW, kRoundEpsilon)
...
bool is_nearly_integer_translation(const LayerSpace<SkMatrix>& m, LayerSpace<SkIPoint>* out = nullptr)
{
    auto [axisX, axisY] = are_axes_nearly_integer_aligned(m, out);
    return axisX && axisY;
}

void decompose_transform(const SkMatrix& transform, SkPoint representativePoint, SkMatrix* postScaling, SkMatrix* scaling)
{
    SkSize scale;
...
std::optional<LayerSpace<SkMatrix>> periodic_axis_transform(SkTileMode tileMode, const LayerSpace<SkIRect>& crop, const LayerSpace<SkIRect>& output)
{
    ...
    if (periodR - periodL <= 1.0 && periodB - periodT <= 1.0) {
        ...
        if (sk_double_saturate2int(tx) != (float)tx || sk_double_saturate2int(ty) != (float)ty) {
            return {};
        }
...
#else
    bool useLegacyFilterResultBlur() const override
    {
        return false;
    }
...
Backend::Backend(sk_sp<SkImageFilterCache> cache, const SkSurfaceProps& surfaceProps, const SkColorType colorType)
...
void Stats::dumpStats() const
{
    SkDebugf("ImageFilter Stats:\n"
        ...
        fNumVisitedImageFilters, fNumCacheHits, fNumOffscreenSurfaces, fNumShaderClampedDraws, fNumShaderBasedTilingDraws);
}

void Stats::reportStats() const
{
    TRACE_EVENT_INSTANT2("skia", "ImageFilter Graph Size", TRACE_EVENT_SCOPE_THREAD, "count", fNumVisitedImageFilters, "cache hits", fNumCacheHits);
    TRACE_EVENT_INSTANT1("skia", "ImageFilter Surfaces", TRACE_EVENT_SCOPE_THREAD, "count", fNumOffscreenSurfaces);
    TRACE_EVENT_INSTANT2("skia", "ImageFilter Shader Tiling", TRACE_EVENT_SCOPE_THREAD, "clamp", fNumShaderClampedDraws, "other", fNumShaderBasedTilingDraws);
}
...
bool Mapping::decomposeCTM(const SkMatrix& ctm, MatrixCapability capability, const skif::ParameterSpace<SkPoint>& representativePt)
{
...
bool Mapping::decomposeCTM(const SkMatrix& ctm, const SkImageFilter* filter, const skif::ParameterSpace<SkPoint>& representativePt)
{
    return this->decomposeCTM(ctm, filter ? as_IFB(filter)->getCTMCapability() : MatrixCapability::kComplex, representativePt);
}
...
template <> SkRect Mapping::map<SkRect>(const SkRect& geom, const SkMatrix& matrix)
{
    return geom.isEmpty() ? SkRect::MakeEmpty() : matrix.mapRect(geom);
}

template <> SkIRect Mapping::map<SkIRect>(const SkIRect& geom, const SkMatrix& matrix)
{
    if (geom.isEmpty()) {
    ...
    if (matrix.isScaleTranslate()) {
        ...
        return { sk_double_saturate2int(std::floor(std::min(l, r) + kRoundEpsilon)), sk_double_saturate2int(std::floor(std::min(t, b) + kRoundEpsilon)),
            sk_double_saturate2int(std::ceil(std::max(l, r) - kRoundEpsilon)), sk_double_saturate2int(std::ceil(std::max(t, b) - kRoundEpsilon)) };
    } else {
        return RoundOut(matrix.mapRect(SkRect::Make(geom)));
    }
}

template <> SkIPoint Mapping::map<SkIPoint>(const SkIPoint& geom, const SkMatrix& matrix)
{
...
template <> SkPoint Mapping::map<SkPoint>(const SkPoint& geom, const SkMatrix& matrix)
{
    SkPoint p;
...
template <> Vector Mapping::map<Vector>(const Vector& geom, const SkMatrix& matrix)
{
...
template <> IVector Mapping::map<IVector>(const IVector& geom, const SkMatrix& matrix)
{
...
template <> SkSize Mapping::map<SkSize>(const SkSize& geom, const SkMatrix& matrix)
{
    if (matrix.isScaleTranslate()) {
...
template <> SkISize Mapping::map<SkISize>(const SkISize& geom, const SkMatrix& matrix)
{
    SkSize size = map(SkSize::Make(geom), matrix);
    return SkISize::Make(SkScalarCeilToInt(size.fWidth - kRoundEpsilon), SkScalarCeilToInt(size.fHeight - kRoundEpsilon));
}

template <> SkMatrix Mapping::map<SkMatrix>(const SkMatrix& m, const SkMatrix& matrix)
{
...
LayerSpace<SkIRect> LayerSpace<SkIRect>::relevantSubset(const LayerSpace<SkIRect> dstRect, SkTileMode tileMode) const
{
...
{
    return LayerSpace<SkISize>({ SkScalarCeilToInt(fData.fWidth - kRoundEpsilon), SkScalarCeilToInt(fData.fHeight - kRoundEpsilon) });
}
LayerSpace<SkISize> LayerSpace<SkSize>::floor() const
{
    return LayerSpace<SkISize>({ SkScalarFloorToInt(fData.fWidth + kRoundEpsilon), SkScalarFloorToInt(fData.fHeight + kRoundEpsilon) });
}
...
bool LayerSpace<SkMatrix>::inverseMapRect(const LayerSpace<SkRect>& r, LayerSpace<SkRect>* out) const
{
    SkRect mapped;
    if (r.isEmpty()) {
...
bool LayerSpace<SkMatrix>::inverseMapRect(const LayerSpace<SkIRect>& rect, LayerSpace<SkIRect>* out) const
{
    if (rect.isEmpty()) {
        // An empty input always inverse maps to an empty rect "successfully"
        *out = LayerSpace<SkIRect>::Empty();
        ...
        SkIRect mapped { sk_double_saturate2int(std::floor(std::min(l, r) + kRoundEpsilon)), sk_double_saturate2int(std::floor(std::min(t, b) + kRoundEpsilon)),
            sk_double_saturate2int(std::ceil(std::max(l, r) - kRoundEpsilon)), sk_double_saturate2int(std::ceil(std::max(t, b) - kRoundEpsilon)) };
...
public:
    AutoSurface(
        const Context& ctx, const LayerSpace<SkIRect>& dstBounds, PixelBoundary boundary, bool renderInParameterSpace, const SkSurfaceProps* props = nullptr)
        ...
        , fBoundary(boundary)
    {
        ...
        if (!dstBounds.isEmpty()) {
            ...
            if (padding) {
                ...
                if (fDstBounds.left() >= dstBounds.left() || fDstBounds.right() <= dstBounds.right() || fDstBounds.top() >= dstBounds.top()
                    || fDstBounds.bottom() <= dstBounds.bottom()) {
                    return;
                }
            }
            device = ctx.backend()->makeDevice(SkISize(fDstBounds.size()), ctx.refColorSpace(), props);
        }
    ...
    FilterResult snap()
    {
        if (fCanvas.has_value()) {
            ...
            if (image && fBoundary != PixelBoundary::kUnknown) {
                ...
                LayerSpace<SkIPoint> origin { { fDstBounds.left() + padding, fDstBounds.top() + padding } };
...
std::pair<sk_sp<SkSpecialImage>, LayerSpace<SkIPoint>> FilterResult::imageAndOffset(const Context& ctx) const
{
...
FilterResult FilterResult::insetForSaveLayer() const
{
    ...
    SkASSERT(inset.fBoundary == PixelBoundary::kInitialized && inset.fTileMode == SkTileMode::kDecal);
...
SkEnumBitMask<FilterResult::BoundsAnalysis> FilterResult::analyzeBounds(const SkMatrix& xtraTransform, const SkIRect& dstBounds, BoundsScope scope) const
{
    ...
    const bool fillsLayerBounds = fTileMode != SkTileMode::kDecal || (fColorFilter && as_CFB(fColorFilter)->affectsTransparentBlack());
    ...
    if (!SkRectPriv::QuadContainsRect(xtraTransform, SkIRect(fLayerBounds), dstBounds, kRoundEpsilon)) {
        ...
        if (!fillsLayerBounds) {
            LayerSpace<SkIRect> imageBounds = fTransform.mapRect(LayerSpace<SkIRect> { fImage->dimensions() });
    ...
    const bool decalLeaks = scope != BoundsScope::kRescale && fTileMode == SkTileMode::kDecal && fSamplingOptions != kNearestNeighbor && !isPixelAligned;
    ...
    if (fSamplingOptions == kDefaultSampling && !isPixelAligned) {
        ...
        safeImageBounds.inset(xAxisAligned ? 0.f : kRoundEpsilon, yAxisAligned ? 0.f : kRoundEpsilon);
    }
    bool hasPixelPadding = fBoundary != PixelBoundary::kUnknown;

    if (!SkRectPriv::QuadContainsRect(netM44, decalLeaks ? safeImageBounds : imageBounds, pixelCenterBounds, kRoundEpsilon)) {
        ...
        if (decalLeaks) {
            ...
            if (!(SkMatrix(netTransform).getMinMaxScales(scaleFactors) && SkScalarNearlyEqual(scaleFactors[0], 1.f, 0.2f)
    ...
    if (scope == BoundsScope::kDeferred) {
    ...
    } else if (scope == BoundsScope::kCanDrawDirectly && !(analysis & BoundsAnalysis::kHasLayerFillingEffect)) {
        ...
        const bool nnOrBilerp = fSamplingOptions == kDefaultSampling || fSamplingOptions == kNearestNeighbor;
    ...
    skvx::int4 edgeMask = SkRectPriv::QuadContainsRectMask(netM44, safeImageBounds, pixelCenterBounds, kRoundEpsilon);
    if (!all(edgeMask)) {
        ...
        skvx::int4 hwEdge { fImage->subset().fTop == 0, fImage->subset().fRight == fImage->backingStoreDimensions().fWidth,
            fImage->subset().fBottom == fImage->backingStoreDimensions().fHeight, fImage->subset().fLeft == 0 };
...
FilterResult FilterResult::applyCrop(const Context& ctx, const LayerSpace<SkIRect>& crop, SkTileMode tileMode) const
{
    ...
    if (!fImage || !cropContent.intersect(fLayerBounds)) {
    ...
    if (!preserveTransparencyInCrop && is_nearly_integer_translation(fTransform, &origin)
        && (doubleClamp || !(this->analyzeBounds(fittedCrop) & BoundsAnalysis::kHasLayerFillingEffect))) {
        ...
        if (restrictedOutput.fBoundary == PixelBoundary::kInitialized || tileMode != SkTileMode::kDecal) {
...
FilterResult FilterResult::applyColorFilter(const Context& ctx, sk_sp<SkColorFilter> colorFilter) const
{
    ...
    if (as_CFB(colorFilter)->affectsTransparentBlack()) {
        if (!fImage || !newLayerBounds.intersect(ctx.desiredOutput())) {
            ...
            AutoSurface surface { ctx, LayerSpace<SkIRect> { SkIRect::MakeXYWH(ctx.desiredOutput().left(), ctx.desiredOutput().top(), 1, 1) },
...
static bool compatible_sampling(
    const SkSamplingOptions& currentSampling, bool currentXformWontAffectNearest, SkSamplingOptions* nextSampling, bool nextXformWontAffectNearest)
{
    ...
    if (currentSampling.isAniso() && nextSampling->isAniso()) {
        ...
        *nextSampling = SkSamplingOptions::Aniso(std::max(currentSampling.maxAniso, nextSampling->maxAniso));
        return true;
        ...
        *nextSampling = currentSampling;
        ...
        return true;
    } else if (currentSampling.useCubic
        && (nextSampling->filter == SkFilterMode::kLinear
            || (nextSampling->useCubic && currentSampling.cubic.B == nextSampling->cubic.B && currentSampling.cubic.C == nextSampling->cubic.C))) {
        ...
        *nextSampling = currentSampling;
        ...
        return true;
    } else if (currentSampling.filter == SkFilterMode::kLinear && nextSampling->filter == SkFilterMode::kLinear) {
...
FilterResult FilterResult::applyTransform(const Context& ctx, const LayerSpace<SkMatrix>& transform, const SkSamplingOptions& sampling) const
{
    ...
    bool isCropped = !nextXformIsInteger && (this->analyzeBounds(SkMatrix(transform), SkIRect(ctx.desiredOutput())) & BoundsAnalysis::kRequiresLayerCrop);

    FilterResult transformed;
    if (!isCropped && compatible_sampling(fSamplingOptions, currentXformIsInteger, &nextSampling, nextXformIsInteger)) {
...
FilterResult FilterResult::resolve(const Context& ctx, LayerSpace<SkIRect> dstBounds, bool preserveDstBounds) const
{
    ...
    const bool subsetCompatible = !fColorFilter && fTileMode == SkTileMode::kDecal && !preserveDstBounds;
    ...
    PixelBoundary boundary = preserveDstBounds ? PixelBoundary::kUnknown : PixelBoundary::kTransparent;
...
FilterResult FilterResult::subset(const LayerSpace<SkIPoint>& knownOrigin, const LayerSpace<SkIRect>& subsetBounds, bool clampSrcIfDisjoint) const
{
    SkDEBUGCODE(LayerSpace<SkIPoint> actualOrigin;)
        SkASSERT(is_nearly_integer_translation(fTransform, &actualOrigin) && SkIPoint(actualOrigin) == SkIPoint(knownOrigin));

    LayerSpace<SkIRect> imageBounds(SkIRect::MakeXYWH(knownOrigin.x(), knownOrigin.y(), fImage->width(), fImage->height()));
    imageBounds = imageBounds.relevantSubset(subsetBounds, clampSrcIfDisjoint ? SkTileMode::kClamp : SkTileMode::kDecal);
    ...
    // origin is INT_MIN).
    SkIRect subset = { imageBounds.left() - knownOrigin.x(), imageBounds.top() - knownOrigin.y(), imageBounds.right() - knownOrigin.x(),
        imageBounds.bottom() - knownOrigin.y() };
    SkASSERT(subset.fLeft >= 0 && subset.fTop >= 0 && subset.fRight <= fImage->width() && subset.fBottom <= fImage->height());
...
void FilterResult::draw(const Context& ctx, SkDevice* device, bool preserveDeviceState, const SkBlender* blender) const
{
    ...
    BoundsScope scope = blendAffectsTransparentBlack ? BoundsScope::kShaderOnly : BoundsScope::kCanDrawDirectly;
    SkEnumBitMask<BoundsAnalysis> analysis = this->analyzeBounds(device->localToDevice(), device->devClipBounds(), scope);
    ...
    if (analysis & BoundsAnalysis::kRequiresLayerCrop) {
        if (blendAffectsTransparentBlack) {
            ...
            if (!LayerSpace<SkMatrix>(device->localToDevice()).inverseMapRect(LayerSpace<SkIRect>(device->devClipBounds()), &dstBounds)) {
                return;
            }
    ...
    const bool pixelAligned = is_nearly_integer_translation(fTransform) && is_nearly_integer_translation(skif::LayerSpace<SkMatrix>(device->localToDevice()));
    SkSamplingOptions sampling = fSamplingOptions;
    if (sampling == kDefaultSampling && pixelAligned) {
        sampling = {};
    }

    if (analysis & BoundsAnalysis::kHasLayerFillingEffect || (blendAffectsTransparentBlack && (analysis & BoundsAnalysis::kDstBoundsNotCovered))) {
        ...
        if (this->canClampToTransparentBoundary(analysis) && fSamplingOptions == kDefaultSampling) {
            ...
            device->drawSpecial(fImage->makePixelOutset().get(), netTransform, sampling, paint, SkCanvas::kFast_SrcRectConstraint);
        } else {
...
sk_sp<SkShader> FilterResult::asShader(
    const Context& ctx, const SkSamplingOptions& xtraSampling, SkEnumBitMask<ShaderFlags> flags, const LayerSpace<SkIRect>& sampleBounds) const
{
    if (!fImage) {
        return nullptr;
    }
    ...
    SkEnumBitMask<BoundsAnalysis> analysis = this->analyzeBounds(sampleBounds, BoundsScope::kShaderOnly);
    ...
    const bool needsResolve =
        ...
        (flags & ShaderFlags::kSampledRepeatedly
            && ((fColorFilter && (!fColorFilter->asAColorMode(nullptr, &colorFilterMode) || colorFilterMode > SkBlendMode::kLastCoeffMode))
        ...
        !compatible_sampling(fSamplingOptions, currentXformIsInteger, &sampling, nextXformIsInteger) ||
    ...
    if (sampling == kDefaultSampling && nextXformIsInteger && (needsResolve || currentXformIsInteger)) {
        sampling = {};
    }
...
sk_sp<SkShader> FilterResult::getAnalyzedShaderView(const Context& ctx, const SkSamplingOptions& finalSampling, SkEnumBitMask<BoundsAnalysis> analysis) const
{
    ...
    if (localMatrix.rectStaysRect() || !(analysis & BoundsAnalysis::kRequiresDecalInLayerSpace)) {
    ...
    if (strict && decalClampToTransparent) {
        ...
        imageShader = fImage->makePixelOutset()->asShader(SkTileMode::kClamp, finalSampling, preDecal, strict);
        effectiveTileMode = SkTileMode::kClamp;
    } else {
        if (!(analysis & BoundsAnalysis::kDstBoundsNotCovered) || (analysis & BoundsAnalysis::kRequiresDecalInLayerSpace)) {
            effectiveTileMode = SkTileMode::kClamp;
        }
    ...
    if (analysis & BoundsAnalysis::kRequiresDecalInLayerSpace) {
        ...
        // decal-subset shader.
        const SkRuntimeEffect* decalEffect = GetKnownRuntimeEffect(SkKnownRuntimeEffects::StableKey::kDecal);
...
template <typename T> using PixelSpace = LayerSpace<T>;
...
PixelSpace<SkRect> scale_about_center(const PixelSpace<SkRect> src, float sx, float sy)
{
    ...
    return LayerSpace<SkRect>({ (src.left() - cx) * sx, (src.top() - cy) * sy, (src.right() - cx) * sx, (src.bottom() - cy) * sy });
}

void draw_color_filtered_border(SkCanvas* canvas, PixelSpace<SkIRect> border, sk_sp<SkColorFilter> colorFilter)
{
    SkPaint cfOnly;
    ...
    canvas->drawIRect({ border.left(), border.top(), border.right(), border.top() + 1 },
        cfOnly); // Top (with corners)
    canvas->drawIRect({ border.left(), border.bottom() - 1, border.right(), border.bottom() },
        cfOnly); // Bottom (with corners)
    canvas->drawIRect({ border.left(), border.top() + 1, border.left() + 1, border.bottom() - 1 },
        cfOnly); // Left (no corners)
    canvas->drawIRect({ border.right() - 1, border.top() + 1, border.right(), border.bottom() - 1 },
        cfOnly); // Right (no corners)
}

void draw_tiled_border(SkCanvas* canvas, SkTileMode tileMode, const SkPaint& paint, const PixelSpace<SkMatrix>& srcToDst, PixelSpace<SkRect> srcBorder,
    PixelSpace<SkRect> dstBorder)
{
    ...
    auto drawCorner
        = [&](const SkPoint& src, const SkPoint& dst) { drawEdge(SkRect::MakeXYWH(src.fX, src.fY, 1.f, 1.f), SkRect::MakeXYWH(dst.fX, dst.fY, 1.f, 1.f)); };
    ...
    if (tileMode == SkTileMode::kRepeat) {
        dstBorder = PixelSpace<SkRect>({ dstBorder.right() - 1.f, dstBorder.bottom() - 1.f, dstBorder.left() + 1.f, dstBorder.top() + 1.f });
    }

    // Edges (excluding corners)
    drawEdge({ srcBorder.left(), srcSampleBounds.top(), srcBorder.left() + 1.f, srcSampleBounds.bottom() },
        { dstBorder.left(), dstSampleBounds.top(), dstBorder.left() + 1.f, dstSampleBounds.bottom() }); // Left

    drawEdge({ srcBorder.right() - 1.f, srcSampleBounds.top(), srcBorder.right(), srcSampleBounds.bottom() },
        { dstBorder.right() - 1.f, dstSampleBounds.top(), dstBorder.right(), dstSampleBounds.bottom() }); // Right

    drawEdge({ srcSampleBounds.left(), srcBorder.top(), srcSampleBounds.right(), srcBorder.top() + 1.f },
        { dstSampleBounds.left(), dstBorder.top(), dstSampleBounds.right(), dstBorder.top() + 1.f }); // Top

    drawEdge({ srcSampleBounds.left(), srcBorder.bottom() - 1.f, srcSampleBounds.right(), srcBorder.bottom() },
        { dstSampleBounds.left(), dstBorder.bottom() - 1.f, dstSampleBounds.right(), dstBorder.bottom() }); // Bottom

    // Corners (sampled directly to preserve their value since they can dominate the
    // output of a clamped blur with a large radius).
    drawCorner({ srcBorder.left(), srcBorder.top() }, { dstBorder.left(), dstBorder.top() }); // TL
    drawCorner({ srcBorder.right() - 1.f, srcBorder.top() }, { dstBorder.right() - 1.f, dstBorder.top() }); // TR
    drawCorner({ srcBorder.right() - 1.f, srcBorder.bottom() - 1.f }, { dstBorder.right() - 1.f, dstBorder.bottom() - 1.f }); // BR
    drawCorner({ srcBorder.left(), srcBorder.bottom() - 1.f }, { dstBorder.left(), dstBorder.bottom() - 1.f }); // BL
}

} // anonymous namespace

FilterResult FilterResult::rescale(const Context& ctx, const LayerSpace<SkSize>& scale, bool enforceDecal) const
{
    LayerSpace<SkIRect> visibleLayerBounds = fLayerBounds;
    if (!fImage || !visibleLayerBounds.intersect(ctx.desiredOutput()) || scale.width() <= 0.f || scale.height() <= 0.f) {
        return {};
    }
    ...
    SkEnumBitMask<BoundsAnalysis> analysis = this->analyzeBounds(ctx.desiredOutput(), BoundsScope::kRescale);
    ...
    const bool canDeferTiling
        = pixelAligned && !(analysis & BoundsAnalysis::kRequiresLayerCrop) && !(enforceDecal && (analysis & BoundsAnalysis::kHasLayerFillingEffect));

    // To match legacy color space conversion logic, treat a null src as sRGB and a null dst as
    // as the src CS.
    const SkColorSpace* srcCS = fImage->getColorSpace() ? fImage->getColorSpace() : sk_srgb_singleton();
    const SkColorSpace* dstCS = ctx.colorSpace() ? ctx.colorSpace() : srcCS;
    const bool hasEffectsToApply
        = !canDeferTiling || SkToBool(fColorFilter) || fImage->colorType() != ctx.backend()->colorType() || !SkColorSpace::Equals(srcCS, dstCS);
    ...
    if (canDeferTiling && (analysis & BoundsAnalysis::kHasLayerFillingEffect)) {
        ...
        srcRect = LayerSpace<SkIRect>(SkIRect::MakeXYWH(origin.x(), origin.y(), fImage->width(), fImage->height()));
        if (fTileMode == SkTileMode::kDecal && (analysis & BoundsAnalysis::kHasLayerFillingEffect)) {
            // Like in applyColorFilter() evaluate the transparent CF'ed border and clamp to it.
            tileMode = SkTileMode::kClamp;
            cfBorder = true;
        } else {
            tileMode = fTileMode;
            deferPeriodicTiling = tileMode == SkTileMode::kRepeat || tileMode == SkTileMode::kMirror;
        }
    } else {
    ...
    if (!pixelAligned && (xSteps > 0 || ySteps > 0)) {
        ...
        int nextXSteps = std::isfinite(netScale.width()) ? downscale_step_count(netScale.width()) : std::numeric_limits<int>::max();
        int nextYSteps = std::isfinite(netScale.height()) ? downscale_step_count(netScale.height()) : std::numeric_limits<int>::max();
    ...
    if (deferPeriodicTiling) {
        ...
        dstBoundsF = PixelSpace<SkRect>({ (float)std::min(dstCenterX, innerDstPixels.left()), (float)std::min(dstCenterY, innerDstPixels.top()),
            (float)std::max(dstCenterX + 1, innerDstPixels.right()), (float)std::max(dstCenterY + 1, innerDstPixels.bottom()) });
    ...
    do {
        ...
        if (surface) {
            ...
            analysis = image.analyzeBounds(SkMatrix(scaleXform), SkIRect(sampleBounds), BoundsScope::kRescale);
            ...
            SkAssertResult(scaleXform.inverseMapRect(PixelSpace<SkRect>(sampleBounds), &srcSampled));
            ...
            if (cfBorder) {
                ...
                draw_tiled_border(surface.canvas(), tileMode, paint, scaleXform, stepPixelBounds, PixelSpace<SkRect>(dstPixelBounds));
            }
        } else {
    ...
    image.fTileMode = tileMode;
    image.fTransform.postConcat(LayerSpace<SkMatrix>::RectToRect(stepBoundsF, LayerSpace<SkRect> { srcRect }));
    image.fLayerBounds = visibleLayerBounds;

    SkASSERT(!enforceDecal || image.fTileMode == SkTileMode::kDecal);
    SkASSERT(image.fTileMode != SkTileMode::kDecal || image.fBoundary == PixelBoundary::kTransparent);
    SkASSERT(!deferPeriodicTiling || image.fBoundary == PixelBoundary::kInitialized);
    return image;
}

FilterResult FilterResult::MakeFromPicture(const Context& ctx, sk_sp<SkPicture> pic, ParameterSpace<SkRect> cullRect)
{
    SkASSERT(pic);
    ...
    SkSurfaceProps props = ctx.backend()->surfaceProps().cloneWithPixelGeometry(kUnknown_SkPixelGeometry);
...
FilterResult FilterResult::MakeFromShader(const Context& ctx, sk_sp<SkShader> shader, bool dither)
{
    SkASSERT(shader);
...
FilterResult FilterResult::MakeFromImage(
    const Context& ctx, sk_sp<SkImage> image, SkRect srcRect, ParameterSpace<SkRect> dstRect, const SkSamplingOptions& sampling)
{
    SkASSERT(image);
    ...
    if (SkRect::Make(srcSubset) == srcRect) {
        ...
        skif::FilterResult subset { std::move(specialImage), skif::LayerSpace<SkIPoint>(srcSubset.topLeft()) };
        SkMatrix transform = SkMatrix::Concat(ctx.mapping().layerMatrix(), SkMatrix::RectToRect(srcRect, SkRect(dstRect)));
    ...
    if (surface) {
        SkPaint paint;
        paint.setAntiAlias(true);
        surface->drawImageRect(std::move(image), srcRect, SkRect(dstRect), sampling, &paint, SkCanvas::kStrict_SrcRectConstraint);
    }
    return surface.snap();
}
...
SkSpan<sk_sp<SkShader>> FilterResult::Builder::createInputShaders(const LayerSpace<SkIRect>& outputBounds, bool evaluateInParameterSpace)
{
    ...
    for (const SampledFilterResult& input : fInputs) {
        ...
        auto shader = input.fImage.asShader(fContext, input.fSampling, input.fFlags | xtraFlags, sampleBounds);
...
LayerSpace<SkIRect> FilterResult::Builder::outputBounds(std::optional<LayerSpace<SkIRect>> explicitOutput) const
{
...
FilterResult FilterResult::Builder::drawShader(sk_sp<SkShader> shader, const LayerSpace<SkIRect>& outputBounds, bool evaluateInParameterSpace) const
{
    ...
    AutoSurface surface { fContext, outputBounds, PixelBoundary::kTransparent, evaluateInParameterSpace };
    if (surface) {
        SkPaint paint;
...
FilterResult FilterResult::Builder::merge()
{
    ...
    if (fInputs.size() == 1) {
        SkASSERT(!fInputs[0].fSampleBounds.has_value() && fInputs[0].fSampling == kDefaultSampling && fInputs[0].fFlags == ShaderFlags::kNone);
        return fInputs[0].fImage;
    }

    const auto mergedBounds = LayerSpace<SkIRect>::Union((int)fInputs.size(), [this](int i) { return fInputs[i].fImage.layerBounds(); });
    ...
    if (surface) {
        for (const SampledFilterResult& input : fInputs) {
            SkASSERT(!input.fSampleBounds.has_value() && input.fSampling == kDefaultSampling && input.fFlags == ShaderFlags::kNone);
...
FilterResult FilterResult::Builder::blur(const LayerSpace<SkSize>& sigma)
{
    ...
    const SkBlurEngine::Algorithm* algorithm = blurEngine->findAlgorithm(SkSize(sigma), fContext.backend()->colorType());
    if (!algorithm) {
        return {};
    }

    // TODO: De-duplicate this logic between SkBlurImageFilter, here, and skgpu::BlurUtils.
    LayerSpace<SkISize> radii = LayerSpace<SkSize>({ 3.f * sigma.width(), 3.f * sigma.height() }).ceil();
    ...
    if (fContext.backend()->useLegacyFilterResultBlur()) {
        ...
        resolved = { algorithm->blur(
                         SkSize(sigma), resolved.fImage, SkIRect::MakeSize(resolved.fImage->dimensions()), SkTileMode::kDecal, SkIRect(srcRelativeOutput)),
    ...
    FilterResult lowResImage
        = fInputs[0].fImage.rescale(fContext.withNewDesiredOutput(sampleBounds), LayerSpace<SkSize>({ sx, sy }), algorithm->supportsOnlyDecalTiling());
    if (!lowResImage) {
        return {};
    }
    SkASSERT(lowResImage.tileMode() == SkTileMode::kDecal || !algorithm->supportsOnlyDecalTiling());
    ...
    lowResSigma = PixelSpace<SkSize> { { std::min(algorithm->maxSigma(), lowResSigma.width()), std::min(algorithm->maxSigma(), lowResSigma.height()) } };
    PixelSpace<SkIRect> lowResMaxOutput { SkISize { lowResImage.fImage->width(), lowResImage.fImage->height() } };

    PixelSpace<SkIRect> srcRelativeOutput;
    if (lowResImage.tileMode() == SkTileMode::kRepeat || lowResImage.tileMode() == SkTileMode::kMirror) {
        ...
        lowResMaxOutput.outset(PixelSpace<SkSize>({ 3.f * lowResSigma.width(), 3.f * lowResSigma.height() }).ceil());
        srcRelativeOutput = lowResMaxOutput.relevantSubset(srcRelativeOutput, lowResImage.tileMode());
    ...
    if (!algorithm->supportsOnlyDecalTiling() && lowResImage.canClampToTransparentBoundary(BoundsAnalysis::kSimple)) {
    ...
    lowResBlur = algorithm->blur(SkSize(lowResSigma), lowResBlur, SkIRect::MakeSize(lowResBlur->dimensions()), tileMode, blurOutputBounds);
    if (!lowResBlur) {
    ...
    if (lowResImage.tileMode() == SkTileMode::kClamp || lowResImage.tileMode() == SkTileMode::kDecal) {
    ...
    if (lowResImage.tileMode() == SkTileMode::kDecal) {
        ...
        outputBounds = this->outputBounds(result.fTransform.mapRect(LayerSpace<SkIRect>(result.fImage->dimensions())));
    }
