...
SkMipmap::SkMipmap(void* malloc, size_t size)
    : SkCachedData(malloc, size)
{
}
SkMipmap::SkMipmap(size_t size, SkDiscardableMemory* dm)
    : SkCachedData(size, dm)
{
}
...
SkMipmap* SkMipmap::Build(const SkPixmap& src, SkDiscardableFactoryProc fact, bool computeContents)
{
    ...
    for (int i = 0; i < countLevels; ++i) {
        ...
        levels[i].fScale = SkSize::Make(SkIntToScalar(width) / src.width(), SkIntToScalar(height) / src.height());
...
bool SkMipmap::validForRootLevel(const SkImageInfo& root) const
{
    ...
    if (fLevels[0].fPixmap.width() != std::max(1, dimension.width() >> 1) || fLevels[0].fPixmap.height() != std::max(1, dimension.height() >> 1)) {
        return false;
    }

    for (int i = 0; i < this->countLevels(); ++i) {
        if (fLevels[i].fPixmap.colorType() != root.colorType() || fLevels[i].fPixmap.alphaType() != root.alphaType()) {
            return false;
        }
    }
    return true;
