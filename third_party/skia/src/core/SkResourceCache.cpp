...
static inline bool SkShouldPostMessageToBus(const SkResourceCache::PurgeSharedIDMessage&, uint32_t)
{
...
void SkResourceCache::Key::init(void* nameSpace, uint64_t sharedID, size_t dataSize)
{
    ...
    static_assert(sizeof(Key) == offsetof(Key, fNamespace) + sizeof(fNamespace), "namespace_field_must_be_last");
    ...
    fHash = SkChecksum::Hash32(this->as32() + kUnhashedLocal32s, (fCount32 - kUnhashedLocal32s) << 2);
}

namespace {
struct HashTraits {
    static uint32_t Hash(const SkResourceCache::Key& key)
    {
        return key.hash();
    }
...
class SkResourceCache::Hash : public THashTable<SkResourceCache::Rec*, SkResourceCache::Key, HashTraits> { };
