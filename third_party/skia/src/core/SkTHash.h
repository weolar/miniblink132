...
template <typename T, typename K, typename Traits = T> class THashTable {
public:
    THashTable() = default;
    ~THashTable() = default;
    ...
    template <typename SlotVal> class Iter {
    public:
...
private:
    ...
    void removeSlot(int index)
    {
        ...
        for (;;) {
            ...
            } while ((index <= originalIndex && originalIndex < emptyIndex) || (originalIndex < emptyIndex && emptyIndex < index)
...
template <typename K, typename V, typename HashK = SkGoodHash> class THashMap {
public:
    ...
    {
        int capacity = pairs.size() >= 4 ? SkNextPow2(pairs.size() * 4 / 3) : 4;
    ...
    {
        fTable.swap(that.fTable);
    }
    void swap(THashMap&& that)
    {
        fTable.swap(std::move(that.fTable));
    }
...
template <typename T, typename HashT = SkGoodHash> class THashSet {
public:
    // Allow default construction and assignment.
    THashSet() = default;
    ...
    {
        int capacity = vals.size() >= 4 ? SkNextPow2(vals.size() * 4 / 3) : 4;
        fTable.resize(capacity);
