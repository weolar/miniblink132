...
template <typename Iter, typename C = std::monostate> class SkEnumerate {
    ...
    public:
        ...
        constexpr Iterator(ptrdiff_t index, Iter it)
            : fIndex { index }
            , fIt { it }
        {
        }
...
public:
    ...
    {
        return Iterator { fBeginIndex + this->ssize(), fEnd };
    }
...
template <typename C, typename Iter = decltype(std::begin(std::declval<C>()))> inline constexpr SkEnumerate<Iter> SkMakeEnumerate(C& c)
{
    return SkEnumerate<Iter> { std::begin(c), std::end(c) };
}
template <typename C, typename Iter = decltype(std::begin(std::declval<C>()))> inline constexpr SkEnumerate<Iter, C> SkMakeEnumerate(C&& c)
{
    return SkEnumerate<Iter, C> { std::forward<C>(c) };
}

template <class T, std::size_t N, typename Iter = decltype(std::begin(std::declval<T (&)[N]>()))> inline constexpr SkEnumerate<Iter> SkMakeEnumerate(T (&a)[N])
{
