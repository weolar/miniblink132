...
void SkPictureRecord::onDrawImage2(const SkImage* image, SkScalar x, SkScalar y, const SkSamplingOptions& sampling, const SkPaint* paint)
{
    ...
    const SkImage* image, const SkRect& src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint* paint, SrcRectConstraint constraint)
{
    ...
    size_t size = 3 * kUInt32Size + 2 * sizeof(dst) + SkSamplingPriv::FlatSize(sampling) + kUInt32Size;
...
void SkPictureRecord::onDrawImageLattice2(const SkImage* image, const Lattice& lattice, const SkRect& dst, SkFilterMode filter, const SkPaint* paint)
{
...
void SkPictureRecord::onDrawVerticesObject(const SkVertices* vertices, SkBlendMode mode, const SkPaint& paint)
{
...
void SkPictureRecord::onDrawAtlas2(const SkImage* atlas, const SkRSXform xform[], const SkRect tex[], const SkColor colors[], int count, SkBlendMode mode,
...
void SkPictureRecord::onDrawEdgeAAImageSet2(const SkCanvas::ImageSetEntry set[], int count, const SkPoint dstClips[], const SkMatrix preViewMatrices[],
    const SkSamplingOptions& sampling, const SkPaint* paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    size_t size = 6 * kUInt32Size + sizeof(SkPoint) * totalDstClipCount + kMatrixSize * totalMatrixCount
        + (4 * kUInt32Size + 2 * sizeof(SkRect) + sizeof(SkScalar)) * count + SkSamplingPriv::FlatSize(sampling);
...
template <typename T> static bool equals(T* a, T* b)
{
...
template <typename T> static int find_or_append(TArray<sk_sp<T>>& array, T* obj)
{
