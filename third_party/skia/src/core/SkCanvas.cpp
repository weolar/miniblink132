 ...
 */
bool SkCanvas::wouldOverwriteEntireSurface(const SkRect* rect, const SkPaint* paint, SkEnumBitMask<PredrawFlags> flags) const
{
    // Convert flags to a ShaderOverrideOpacity enum
    auto overrideOpacity = (flags & PredrawFlags::kOpaqueShaderOverride) ? SkPaintPriv::kOpaque_ShaderOverrideOpacity
        : (flags & PredrawFlags::kNonOpaqueShaderOverride)               ? SkPaintPriv::kNotOpaque_ShaderOverrideOpacity
...
{
    if (fSurfaceBase) {
        if (!fSurfaceBase->aboutToDraw(willOverwritesEntireSurface ? SkSurface::kDiscard_ContentChangeMode : SkSurface::kRetain_ContentChangeMode)) {
            return false;
        }
    }
    return true;
...
bool SkCanvas::predrawNotify(const SkRect* rect, const SkPaint* paint, SkEnumBitMask<PredrawFlags> flags)
{
    if (fSurfaceBase) {
...
SkCanvas::Layer::Layer(sk_sp<SkDevice> device, FilterSpan imageFilters, const SkPaint& paint, bool isCoverage, bool includesPadding)
...
SkCanvas::MCRec::MCRec(SkDevice* device)
    : fDevice(device)
{
    SkASSERT(fDevice);
}

SkCanvas::MCRec::MCRec(const MCRec* prev)
...
void SkCanvas::MCRec::newLayer(sk_sp<SkDevice> layerDevice, FilterSpan filters, const SkPaint& restorePaint, bool layerIsCoverage, bool includesPadding)
{
    SkASSERT(!fBackImage);
    fLayer = std::make_unique<Layer>(std::move(layerDevice), filters, restorePaint, layerIsCoverage, includesPadding);
...
public:
    explicit AutoUpdateQRBounds(SkCanvas* canvas)
        : fCanvas(canvas)
    {
...
std::optional<AutoLayerForImageFilter> SkCanvas::aboutToDraw(const SkPaint& paint, const SkRect* rawBounds, SkEnumBitMask<PredrawFlags> flags)
{
    ...
    const bool skipMaskFilterLayer = (flags & PredrawFlags::kSkipMaskFilterAutoLayer) || !this->topDevice()->useDrawCoverageMaskForMaskFilters();
    return std::optional<AutoLayerForImageFilter>(std::in_place, this, paint, rawBounds, skipMaskFilterLayer);
}

std::optional<AutoLayerForImageFilter> SkCanvas::aboutToDraw(const SkPaint& paint, const SkRect* rawBounds)
{
...
void SkCanvas::resetForNextPicture(const SkIRect& bounds)
{
    ...
    if (!asNoPixelsDevice->resetForNextPicture(bounds)) {
        fRootDevice = sk_make_sp<SkNoPixelsDevice>(bounds, fRootDevice->surfaceProps(), fRootDevice->imageInfo().refColorSpace());
    }
...
{
    this->init(sk_make_sp<SkNoPixelsDevice>(SkIRect::MakeWH(std::max(width, 0), std::max(height, 0)), fProps));
}
...
static skif::ParameterSpace<SkPoint> compute_decomposition_center(
    const SkMatrix& dstToLocal, std::optional<skif::ParameterSpace<SkRect>> contentBounds, const skif::DeviceSpace<SkIRect>& targetOutput)
{
...
static std::optional<std::pair<skif::Mapping, skif::LayerSpace<SkIRect>>> get_layer_mapping_and_bounds(SkCanvas::FilterSpan filters, const SkMatrix& localToDst,
    const skif::DeviceSpace<SkIRect>& targetOutput, std::optional<skif::ParameterSpace<SkRect>> contentBounds = {}, SkScalar scaleFactor = 1.0f)
{
    SkMatrix dstToLocal;
    ...
    skif::ParameterSpace<SkPoint> center = compute_decomposition_center(dstToLocal, contentBounds, targetOutput);
    ...
    int maxLayerDim = std::max(Sk64_pin_to_s32(2 * std::max(SkIRect(targetOutput).width64(), SkIRect(targetOutput).height64())), kMinDimThreshold);
    ...
    if (!filters.empty()) {
        layerBounds = skif::LayerSpace<SkIRect>::Union(
            filters.size(), [&](int i) { return filters[i] ? as_IFB(filters[i])->getInputBounds(mapping, targetOutput, contentBounds) : baseLayerBounds; });
        ...
        if (layerBounds.width() > maxLayerDim || layerBounds.height() > maxLayerDim) {
            ...
            for (const sk_sp<SkImageFilter>& filter : filters) {
                if (filter) {
                    auto idealLayerBounds = as_IFB(filter)->getInputBounds(idealMapping, targetOutput, contentBounds);
                    maxLayerDim = std::max(std::max(idealLayerBounds.width(), idealLayerBounds.height()), maxLayerDim);
                }
            }
        }
    } else {
    ...
    if (layerBounds.width() > maxLayerDim || layerBounds.height() > maxLayerDim) {
        skif::LayerSpace<SkIRect> newLayerBounds(SkIRect::MakeWH(std::min(layerBounds.width(), maxLayerDim), std::min(layerBounds.height(), maxLayerDim)));
        SkMatrix adjust = SkMatrix::MakeRectToRect(SkRect::Make(SkIRect(layerBounds)), SkRect::Make(SkIRect(newLayerBounds)), SkMatrix::kFill_ScaleToFit);
...
{
    if (dstInfo.bytesPerPixel() <= 4 && dstInfo.colorType() != kRGBA_8888_SkColorType && dstInfo.colorType() != kBGRA_8888_SkColorType) {
...
static skif::FilterResult apply_alpha_and_colorfilter(const skif::Context& ctx, const skif::FilterResult& image, const SkPaint& paint)
{
    ...
    if (paint.getAlphaf() < 1.f) {
        result = result.applyColorFilter(ctx,
            SkColorFilters::Blend(paint.getColor4f(),
                /*colorSpace=*/nullptr, SkBlendMode::kDstIn));
    }
...
void SkCanvas::internalDrawDeviceWithFilter(SkDevice* src, SkDevice* dst, FilterSpan filters, const SkPaint& paint, DeviceCompatibleWithFilter compat,
    const SkColorInfo& filterColorInfo, SkScalar scaleFactor, SkTileMode srcTileMode, bool srcIsCoverageLayer)
{
    ...
    const SkColorType filterColorType = srcIsCoverageLayer ? kAlpha_8_SkColorType : image_filter_color_type(filterColorInfo);
    ...
    if (compat != DeviceCompatibleWithFilter::kUnknown) {
        ...
        mapping = skif::Mapping(src->getRelativeTransform(*dst), dst->getRelativeTransform(*src), localToSrc);
        ...
        auto mappingAndBounds = get_layer_mapping_and_bounds(filters, dst->localToDevice(), outputBounds, {}, SkTPin(scaleFactor, 0.f, 1.f));
        if (!mappingAndBounds) {
            return;
        }
        ...
        if (src) {
            if (!requiredInput.isEmpty()) {
                ...
                srcToLayer = skif::LayerSpace<SkMatrix>(SkMatrix::Concat(mapping.layerMatrix(), srcToLocal));
    ...
    auto backend = dst->createImageFilteringBackend(src ? src->surfaceProps() : dst->surfaceProps(), filterColorType);
    skif::Stats stats;
    skif::Context ctx { std::move(backend), mapping, requiredInput, skif::FilterResult {}, filterColorSpace.get(), &stats };
    ...
    if (src && !requiredInput.isEmpty()) {
        ...
        auto availSrc = skif::LayerSpace<SkIRect>(src->size()).relevantSubset(srcSubset, srcTileMode);
        ...
        if (SkMatrix(srcToLayer).isScaleTranslate()) {
            ...
            if (requiredSubset.width() == availSrc.width() && requiredSubset.height() == availSrc.height()) {
                ...
                source = { src->snapSpecialScaled(SkIRect(availSrc), SkISize(requiredSubset.size())), requiredSubset.topLeft() };
                ctx.markNewSurface();
            }
        }
        ...
        if (compat == DeviceCompatibleWithFilter::kYesWithPadding) {
            ...
            source = source.applyCrop(ctx.withNewDesiredOutput(srcSubset), source.layerBounds(), srcTileMode)
    ...
    ctx = ctx.withNewDesiredOutput(mapping.deviceToLayer(outputBounds)).withNewSource(source);
    ...
    for (const sk_sp<SkImageFilter>& filter : filtersOrNull) {
        ...
        if (srcIsCoverageLayer) {
            ...
            if (coverageMask) {
                ...
                dst->drawCoverageMask(coverageMask.get(), deviceMatrixWithOffset, result.sampling(), paint);
            }
        } else {
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy, bool coverageOnly)
{
    TRACE_EVENT0("skia", TRACE_FUNC);
    ...
    filtersPriorDevice |= ((rec.fSaveLayerFlags & kInitWithPrevious_SaveLayerFlag) && (!filters.empty() || cf || blender || restorePaint.getAlphaf() < 1.f));
#endif
    ...
    const bool drawDeviceMustFillClip
        = filters.empty() && ((cf && as_CFB(cf)->affectsTransparentBlack()) || (blender && as_BB(blender)->affectsTransparentBlack()));
    ...
    auto mappingAndBounds = get_layer_mapping_and_bounds(filters, priorDevice->localToDevice(), outputBounds, contentBounds);
    ...
    if (layerBounds.isEmpty()) {
        ...
        if (!filters.empty() && !priorDevice->isNoPixelsDevice()) {
            ...
            this->internalDrawDeviceWithFilter(/*src=*/nullptr, priorDevice, filters, restorePaint, DeviceCompatibleWithFilter::kUnknown, filterColorInfo);
        }
        ...
        if (!filters.empty()) {
            ...
            if (paddedLayerBounds.left() < layerBounds.left() && paddedLayerBounds.top() < layerBounds.top() && paddedLayerBounds.right() > layerBounds.right()
    ...
    if (strategy == kFullLayer_SaveLayerStrategy) {
        ...
        if (coverageOnly) {
            ...
            layerColorType
                = SkToBool(rec.fSaveLayerFlags & kF16ColorType) ? kRGBA_F16_SkColorType : image_filter_color_type(priorDevice->imageInfo().colorInfo());
        }
        SkImageInfo info = SkImageInfo::Make(layerBounds.width(), layerBounds.height(), layerColorType, kPremul_SkAlphaType,
            rec.fColorSpace ? sk_ref_sp(rec.fColorSpace) : priorDevice->imageInfo().refColorSpace());

        SkPixelGeometry geo = rec.fSaveLayerFlags & kPreserveLCDText_SaveLayerFlag ? fProps.pixelGeometry() : kUnknown_SkPixelGeometry;
    ...
    if (!newDevice) {
        ...
        // doesn't exist.
        newDevice = sk_make_sp<SkNoPixelsDevice>(SkIRect::MakeWH(layerBounds.width(), layerBounds.height()), fProps, this->imageInfo().refColorSpace());
        initBackdrop = false;
    }
    ...
    if (paddedLayer) {
        newDevice->clipRect(SkRect::Make(newDevice->devClipBounds().makeInset(1, 1)), SkClipOp::kIntersect, /*aa=*/false);
    }
    ...
    newDevice->setDeviceCoordinateSystem(priorDevice->deviceToGlobal() * SkM44(newLayerMapping.layerToDevice()),
        SkM44(newLayerMapping.deviceToLayer()) * priorDevice->globalToDevice(), SkM44(newLayerMapping.layerMatrix()), layerBounds.left(), layerBounds.top());
    ...
    if (initBackdrop) {
        ...
        auto compat = (!filters.empty() || rec.fBackdrop || scaleBackdrop) ? DeviceCompatibleWithFilter::kUnknown : DeviceCompatibleWithFilter::kYes;
        ...
        this->internalDrawDeviceWithFilter(priorDevice, // src
            newDevice.get(), // dst
            backdropAsSpan, backdropPaint, compat, newDevice->imageInfo().colorInfo(), rec.fExperimentalBackdropScale, rec.fBackdropTileMode);
    }
...
void SkCanvas::internalSaveBehind(const SkRect* localBounds)
{
    ...
    fMCRec->fBackImage = std::make_unique<BackImage>(BackImage { std::move(backImage), devBounds.topLeft() });
...
void SkCanvas::internalRestore()
{
    ...
    if (backImage) {
        ...
        this->topDevice()->drawSpecial(backImage->fImage.get(), SkMatrix::Translate(backImage->fLoc), SkSamplingOptions(), paint);
    }
    ...
    if (layer && !layer->fDevice->isNoPixelsDevice() && !layer->fDiscard) {
        ...
        if (this->predrawNotify()) {
            ...
            if (!layer->fImageFilters.empty()) {
                auto compat = layer->fIncludesPadding ? DeviceCompatibleWithFilter::kYesWithPadding : DeviceCompatibleWithFilter::kYes;
                this->internalDrawDeviceWithFilter(layer->fDevice.get(), // src
                    dstDev, // dst
                    layer->fImageFilters, layer->fPaint, compat, layer->fDevice->imageInfo().colorInfo(),
                    /*scaleFactor=*/1.0f,
                    /*srcTileMode=*/SkTileMode::kDecal, layer->fIsCoverage);
            } else {
...
bool SkCanvas::internalQuickReject(const SkRect& bounds, const SkPaint& paint, const SkMatrix* matrix)
{
...
SkRect SkCanvas::computeDeviceClipBounds(bool outsetForAA) const
{
    ...
    if (dev->isClipEmpty()) {
        ...
        SkRect devClipBounds = SkMatrixPriv::MapRect(dev->deviceToGlobal(), SkRect::Make(dev->devClipBounds()));
        if (outsetForAA) {
...
void SkCanvas::drawImageNine(const SkImage* image, const SkIRect& center, const SkRect& dst, SkFilterMode filter, const SkPaint* paint)
{
...
void SkCanvas::drawImageLattice(const SkImage* image, const Lattice& lattice, const SkRect& dst, SkFilterMode filter, const SkPaint* paint)
{
    TRACE_EVENT0("skia", TRACE_FUNC);
    RETURN_ON_NULL(image);
    if (dst.isEmpty()) {
    ...
    if (SkLatticeIter::Valid(image->width(), image->height(), latticePlusBounds)) {
        ...
        this->drawImageRect(image, SkRect::MakeIWH(image->width(), image->height()), dst, SkSamplingOptions(filter), &latticePaint, kStrict_SrcRectConstraint);
    }
}

void SkCanvas::drawAtlas(const SkImage* atlas, const SkRSXform xform[], const SkRect tex[], const SkColor colors[], int count, SkBlendMode mode,
...
void SkCanvas::experimental_DrawEdgeAAImageSet(const ImageSetEntry imageSet[], int cnt, const SkPoint dstClips[], const SkMatrix preViewMatrices[],
    const SkSamplingOptions& sampling, const SkPaint* paint, SrcRectConstraint constraint)
{
    ...
    if (paint && paint->getImageFilter() && cnt == 1) {
        ...
        const bool canMapDstRect = entry.fMatrixIndex < 0
            || (preViewMatrices[entry.fMatrixIndex].isScaleTranslate() && preViewMatrices[entry.fMatrixIndex].getScaleX() > 0.f
        ...
        if (!entry.fHasClip && canMapDstRect) {
            ...
            this->drawImageRect(entry.fImage.get(), entry.fSrcRect, dst, sampling, paint, constraint);
            return;
    ...
    this->onDrawEdgeAAImageSet2(imageSet, cnt, dstClips, preViewMatrices, sampling, paint, constraint);
}
...
static const SkBlurMaskFilterImpl* can_attempt_blurred_rrect_draw(const SkPaint& paint)
{
    ...
    const SkBlurMaskFilterImpl* blurMaskFilter = static_cast<const SkBlurMaskFilterImpl*>(maskFilter);
...
std::optional<AutoLayerForImageFilter> SkCanvas::attemptBlurredRRectDraw(const SkRRect& rrect, const SkPaint& paint, SkEnumBitMask<PredrawFlags> flags)
{
...
void SkCanvas::onDrawRect(const SkRect& r, const SkPaint& paint)
{
    ...
    std::optional<AutoLayerForImageFilter> layer = this->attemptBlurredRRectDraw(SkRRect::MakeRect(r), paint, PredrawFlags::kCheckForOverwrite);
...
void SkCanvas::onDrawOval(const SkRect& oval, const SkPaint& paint)
{
    ...
    // Returns a layer if a blurred draw is not applicable or was unsuccessful.
    std::optional<AutoLayerForImageFilter> layer = this->attemptBlurredRRectDraw(SkRRect::MakeOval(oval), paint, PredrawFlags::kNone);
...
void SkCanvas::onDrawArc(const SkRect& oval, SkScalar startAngle, SkScalar sweepAngle, bool useCenter, const SkPaint& paint)
{
    ...
    if (layer) {
        this->topDevice()->drawArc(SkArc::Make(oval, startAngle, sweepAngle, useCenter), layer->paint());
    }
}
...
void SkCanvas::onDrawRRect(const SkRRect& rrect, const SkPaint& paint)
{
    ...
    // Returns a layer if a blurred draw is not applicable or was unsuccessful.
    std::optional<AutoLayerForImageFilter> layer = this->attemptBlurredRRectDraw(rrect, paint, PredrawFlags::kNone);
...
void SkCanvas::onDrawImage2(const SkImage* image, SkScalar x, SkScalar y, const SkSamplingOptions& sampling, const SkPaint* paint)
{
    SkUNREACHABLE;
}
...
void SkCanvas::onDrawImageRect2(
    const SkImage* image, const SkRect& src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint* paint, SrcRectConstraint constraint)
{
    ...
    if (this->topDevice()->shouldDrawAsTiledImageRect()) {
        if (this->topDevice()->drawAsTiledImageRect(this, image, &src, dst, realSampling, realPaint, constraint)) {
            return;
        }
    }
    ...
    if (realPaint.getImageFilter() && !image->isAlphaOnly() && !realPaint.getMaskFilter()) {
        ...
        auto mappingAndBounds = get_layer_mapping_and_bounds(filterAsSpan, device->localToDevice(), outputBounds, imageBounds);
        if (!mappingAndBounds) {
            return;
        }
        if (!this->predrawNotify()) {
        ...
        auto backend = device->createImageFilteringBackend(device->surfaceProps(), image_filter_color_type(device->imageInfo().colorInfo()));
        auto [mapping, srcBounds] = *mappingAndBounds;
        skif::Stats stats;
        skif::Context ctx { std::move(backend), mapping, srcBounds, skif::FilterResult {}, device->imageInfo().colorSpace(), &stats };

        auto source = skif::FilterResult::MakeFromImage(ctx, sk_ref_sp(image), src, imageBounds, sampling);
    ...
    if (realPaint.getMaskFilter() && this->topDevice()->useDrawCoverageMaskForMaskFilters()) {
        ...
        SkRect drawDst = SkModifyPaintAndDstForDrawImageRect(image, sampling, src, dst, constraint == kStrict_SrcRectConstraint, &realPaint);
        if (drawDst.isEmpty()) {
            return;
        } else {
    ...
    auto layer = this->aboutToDraw(
        realPaint, &dst, PredrawFlags::kCheckForOverwrite | (image->isOpaque() ? PredrawFlags::kOpaqueShaderOverride : PredrawFlags::kNonOpaqueShaderOverride));
    if (layer) {
        this->topDevice()->drawImageRect(image, &src, dst, realSampling, layer->paint(), constraint);
    }
}

void SkCanvas::onDrawImageLattice2(const SkImage* image, const Lattice& lattice, const SkRect& dst, SkFilterMode filter, const SkPaint* paint)
{
    SkPaint realPaint = clean_paint_for_drawImage(paint);
...
void SkCanvas::drawImage(const SkImage* image, SkScalar x, SkScalar y, const SkSamplingOptions& sampling, const SkPaint* paint)
{
    TRACE_EVENT0("skia", TRACE_FUNC);
    RETURN_ON_NULL(image);
    ...
    this->drawImageRect(image,
        ...
        /*dst=*/SkRect::MakeXYWH(x, y, image->width(), image->height()), sampling, paint, kFast_SrcRectConstraint);
}
...
void SkCanvas::drawImageRect(const SkImage* image, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint* paint)
{
    RETURN_ON_NULL(image);
    this->drawImageRect(image, SkRect::MakeIWH(image->width(), image->height()), dst, sampling, paint, kFast_SrcRectConstraint);
}
...
sk_sp<Slug> SkCanvas::convertBlobToSlug(const SkTextBlob& blob, SkPoint origin, const SkPaint& paint)
{
    TRACE_EVENT0("skia", TRACE_FUNC);
...
sk_sp<Slug> SkCanvas::onConvertGlyphRunListToSlug(const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
    SkRect bounds = glyphRunList.sourceBoundsWithOrigin();
...
void SkCanvas::drawSimpleText(const void* text, size_t byteLength, SkTextEncoding encoding, SkScalar x, SkScalar y, const SkFont& font, const SkPaint& paint)
{
    ...
    if (byteLength) {
        ...
        const sktext::GlyphRunList& glyphRunList = fScratchGlyphRunBuilder->textToGlyphRunList(font, paint, text, byteLength, { x, y }, encoding);
...
void SkCanvas::drawGlyphs(int count, const SkGlyphID* glyphs, const SkPoint* positions, const uint32_t* clusters, int textByteCount, const char* utf8text,
    SkPoint origin, const SkFont& font, const SkPaint& paint)
{
    ...
    sktext::GlyphRun glyphRun { font, SkSpan(positions, count), SkSpan(glyphs, count), SkSpan(utf8text, textByteCount), SkSpan(clusters, count),
        SkSpan<SkVector>() };

    sktext::GlyphRunList glyphRunList = fScratchGlyphRunBuilder->makeGlyphRunList(glyphRun, paint, origin);
    this->onDrawGlyphRunList(glyphRunList, paint);
}

void SkCanvas::drawGlyphs(int count, const SkGlyphID glyphs[], const SkPoint positions[], SkPoint origin, const SkFont& font, const SkPaint& paint)
{
    if (count <= 0) {
        return;
    }

    sktext::GlyphRun glyphRun { font, SkSpan(positions, count), SkSpan(glyphs, count), SkSpan<const char>(), SkSpan<const uint32_t>(), SkSpan<SkVector>() };

    sktext::GlyphRunList glyphRunList = fScratchGlyphRunBuilder->makeGlyphRunList(glyphRun, paint, origin);
    this->onDrawGlyphRunList(glyphRunList, paint);
}

void SkCanvas::drawGlyphs(int count, const SkGlyphID glyphs[], const SkRSXform xforms[], SkPoint origin, const SkFont& font, const SkPaint& paint)
{
    if (count <= 0) {
        return;
    }
    ...
    sktext::GlyphRun glyphRun { font, positions, SkSpan(glyphs, count), SkSpan<const char>(), SkSpan<const uint32_t>(), rotateScales };
    sktext::GlyphRunList glyphRunList = fScratchGlyphRunBuilder->makeGlyphRunList(glyphRun, paint, origin);
    this->onDrawGlyphRunList(glyphRunList, paint);
}
...
void SkCanvas::onDrawAtlas2(const SkImage* atlas, const SkRSXform xform[], const SkRect tex[], const SkColor colors[], int count, SkBlendMode bmode,
    const SkSamplingOptions& sampling, const SkRect* cull, const SkPaint* paint)
{
...
void SkCanvas::onDrawEdgeAAImageSet2(const ImageSetEntry imageSet[], int count, const SkPoint dstClips[], const SkMatrix preViewMatrices[],
    const SkSamplingOptions& sampling, const SkPaint* paint, SrcRectConstraint constraint)
{
    if (count <= 0) {
        // Nothing to draw
    ...
    if (layer) {
        this->topDevice()->drawEdgeAAImageSet(imageSet, count, dstClips, preViewMatrices, realSampling, layer->paint(), constraint);
    }
}
...
SkCanvas::ImageSetEntry::ImageSetEntry(
    sk_sp<const SkImage> image, const SkRect& srcRect, const SkRect& dstRect, int matrixIndex, float alpha, unsigned aaFlags, bool hasClip)
...
SkRasterHandleAllocator::Handle SkRasterHandleAllocator::allocBitmap(const SkImageInfo& info, SkBitmap* bm)
{
...
std::unique_ptr<SkCanvas> SkRasterHandleAllocator::MakeCanvas(
    std::unique_ptr<SkRasterHandleAllocator> alloc, const SkImageInfo& info, const Rec* rec, const SkSurfaceProps* props)
{
    ...
    return hndl ? std::unique_ptr<SkCanvas>(new SkCanvas(bm, std::move(alloc), hndl, props)) : nullptr;
}
