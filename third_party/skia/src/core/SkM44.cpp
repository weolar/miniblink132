...
static SkRect map_rect_affine(const SkRect& src, const float mat[16])
{
    ...
    auto minMax = c3
        + flip
            * min(min(c0 * src.fLeft + c1 * src.fTop, c0 * src.fRight + c1 * src.fTop),
...
static SkRect map_rect_perspective(const SkRect& src, const float mat[16])
{
        ...
        if (w0 >= SkPathPriv::kW0PlaneDistance) {
            ...
            auto clip = [&](const skvx::float4& p) {
                ...
                if (w >= SkPathPriv::kW0PlaneDistance) {
                    ...
                    auto c = (t * skvx::shuffle<0, 1>(p) + (1.f - t) * skvx::shuffle<0, 1>(p0)) / SkPathPriv::kW0PlaneDistance;
    ...
    auto minMax = flip * min(min(project(tl, tr, bl), project(tr, br, tl)), min(project(br, bl, tr), project(bl, tl, br)));
...
{
    const bool hasPerspective = m.fMat[3] != 0 || m.fMat[7] != 0 || m.fMat[11] != 0 || m.fMat[15] != 1;
    if (hasPerspective) {
...
SkM44& SkM44::setRotateUnitSinCos(SkV3 axis, SkScalar sinAngle, SkScalar cosAngle)
{
    ...
    *this = { t * x * x + c, t * x * y - s * z, t * x * z + s * y, 0, t * x * y + s * z, t * y * y + c, t * y * z - s * x, 0, t * x * z - s * y,
        t * y * z + s * x, t * z * z + c, 0, 0, 0, 0, 1 };
    return *this;
}
...
void SkM44::dump() const
{
    SkDebugf("|%g %g %g %g|\n"
             ...
             "|%g %g %g %g|\n"
             "|%g %g %g %g|\n",
        fMat[0], fMat[4], fMat[8], fMat[12], fMat[1], fMat[5], fMat[9], fMat[13], fMat[2], fMat[6], fMat[10], fMat[14], fMat[3], fMat[7], fMat[11], fMat[15]);
}
