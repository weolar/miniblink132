...
static_assert(std::size(kRecursiveLimits) == kQuad_RecursiveLimit + 1, "recursive_limits_mismatch");
...
static struct DepthHistogram {
    ...
    DepthHistogram()
    {
        memset(fCubicDepths, 0, sizeof(fCubicDepths));
    }
...
private:
    ...
    void conicPerpRay(const SkConic&, SkScalar t, SkPoint* tPt, SkPoint* onPt, SkVector* tangent) const;
    ...
    void cubicPerpRay(const SkPoint cubic[4], SkScalar t, SkPoint* tPt, SkPoint* onPt, SkVector* tangent) const;
    ...
    void setConicEndNormal(const SkConic&, const SkVector& normalAB, const SkVector& unitNormalAB, SkVector* normalBC, SkVector* unitNormalBC);
    void setCubicEndNormal(const SkPoint cubic[4], const SkVector& normalAB, const SkVector& unitNormalAB, SkVector* normalCD, SkVector* unitNormalCD);
    void setQuadEndNormal(const SkPoint quad[3], const SkVector& normalAB, const SkVector& unitNormalAB, SkVector* normalBC, SkVector* unitNormalBC);
...
SkPathStroker::SkPathStroker(
    const SkPath& src, SkScalar radius, SkScalar miterLimit, SkPaint::Cap cap, SkPaint::Join join, SkScalar resScale, bool canIgnoreCenter)
    : fRadius(radius)
    , fResScale(resScale)
...
void SkPathStroker::setCubicEndNormal(
    const SkPoint cubic[4], const SkVector& normalAB, const SkVector& unitNormalAB, SkVector* normalCD, SkVector* unitNormalCD)
{
...
// returns the distance squared from the point to the line
static SkScalar pt_to_tangent_line(const SkPoint& pt, const SkPoint& lineStart, const SkVector& tangent)
{
...
static bool cubic_in_line(const SkPoint cubic[4])
{
    ...
    return pt_to_line(cubic[mid1], cubic[outer1], cubic[outer2]) <= lineSlop && pt_to_line(cubic[mid2], cubic[outer1], cubic[outer2]) <= lineSlop;
}
...
SkPathStroker::ReductionType SkPathStroker::CheckCubicLinear(const SkPoint cubic[4], SkPoint reduction[3], const SkPoint** tangentPtPtr)
{
...
SkPathStroker::ReductionType SkPathStroker::CheckConicLinear(const SkConic& conic, SkPoint* reduction)
{
...
SkPathStroker::ReductionType SkPathStroker::CheckQuadLinear(const SkPoint quad[3], SkPoint* reduction)
{
...
void SkPathStroker::setRayPts(const SkPoint& tPt, SkVector* dxy, SkPoint* onPt, SkVector* tangent) const
{
...
void SkPathStroker::conicPerpRay(const SkConic& conic, SkScalar t, SkPoint* tPt, SkPoint* onPt, SkVector* tangent) const
{
    SkVector dxy;
...
void SkPathStroker::cubicPerpRay(const SkPoint cubic[4], SkScalar t, SkPoint* tPt, SkPoint* onPt, SkVector* tangent) const
{
    SkVector dxy;
    SkPoint chopped[7];
...
void SkPathStroker::quadPerpRay(const SkPoint quad[3], SkScalar t, SkPoint* tPt, SkPoint* onPt, SkVector* tangent) const
{
    SkVector dxy;
...
SkPathStroker::ResultType SkPathStroker::intersectRay(SkQuadConstruct* quadPts, IntersectRayType intersectRayType STROKER_DEBUG_PARAMS(int depth)) const
{
...
SkPathStroker::ResultType SkPathStroker::tangentsMeet(const SkPoint cubic[4], SkQuadConstruct* quadPts)
{
...
SkPathStroker::ResultType SkPathStroker::strokeCloseEnough(
    const SkPoint stroke[3], const SkPoint ray[2], SkQuadConstruct* quadPts STROKER_DEBUG_PARAMS(int depth)) const
{
    ...
    if (points_within_dist(ray[0], strokeMid, fInvResScale)) { // if the difference is small
        if (sharp_angle(quadPts->fQuad)) {
            return STROKER_RESULT(kSplit_ResultType, depth, quadPts, "sharp_angle (1) =%g,%g, %g,%g, %g,%g", quadPts->fQuad[0].fX, quadPts->fQuad[0].fY,
                quadPts->fQuad[1].fX, quadPts->fQuad[1].fY, quadPts->fQuad[2].fX, quadPts->fQuad[2].fY);
        }
        return STROKER_RESULT(kQuad_ResultType, depth, quadPts, "points_within_dist(ray[0]=%g,%g, strokeMid=%g,%g, fInvResScale=%g)", ray[0].fX, ray[0].fY,
    ...
    if (!ptInQuadBounds(stroke, ray[0])) { // if far, subdivide
        return STROKER_RESULT(kSplit_ResultType, depth, quadPts, "!pt_in_quad_bounds(stroke=(%g,%g %g,%g %g,%g), ray[0]=%g,%g)", stroke[0].fX, stroke[0].fY,
    ...
    if (points_within_dist(ray[0], quadPt, error)) { // if the difference is small, we're done
        if (sharp_angle(quadPts->fQuad)) {
            return STROKER_RESULT(kSplit_ResultType, depth, quadPts, "sharp_angle (2) =%g,%g, %g,%g, %g,%g", quadPts->fQuad[0].fX, quadPts->fQuad[0].fY,
                quadPts->fQuad[1].fX, quadPts->fQuad[1].fY, quadPts->fQuad[2].fX, quadPts->fQuad[2].fY);
        }
        return STROKER_RESULT(
            kQuad_ResultType, depth, quadPts, "points_within_dist(ray[0]=%g,%g, quadPt=%g,%g, error=%g)", ray[0].fX, ray[0].fY, quadPt.fX, quadPt.fY, error);
    }
    // otherwise, subdivide
    return STROKER_RESULT(kSplit_ResultType, depth, quadPts, "%s", "fall through");
}

SkPathStroker::ResultType SkPathStroker::compareQuadCubic(const SkPoint cubic[4], SkQuadConstruct* quadPts)
{
...
SkPathStroker::ResultType SkPathStroker::compareQuadConic(const SkConic& conic, SkQuadConstruct* quadPts) const
{
    // get the quadratic approximation of the stroke
...
SkPathStroker::ResultType SkPathStroker::compareQuadQuad(const SkPoint quad[3], SkQuadConstruct* quadPts)
{
    // get the quadratic approximation of the stroke
    if (!quadPts->fStartSet) {
        SkPoint quadStartPt;
...
bool SkPathStroker::cubicStroke(const SkPoint cubic[4], SkQuadConstruct* quadPts)
{
    if (!fFoundTangents) {
        ...
        if (kQuad_ResultType != resultType) {
            if ((kDegenerate_ResultType == resultType || points_within_dist(quadPts->fQuad[0], quadPts->fQuad[2], fInvResScale))
    ...
    SkDEBUGCODE(gMaxRecursion[fFoundTangents] = std::max(gMaxRecursion[fFoundTangents], fRecursionDepth + 1));
#endif
...
#if QUAD_STROKE_APPROX_EXTENDED_DEBUGGING
    SkDEBUGCODE(gMaxRecursion[kConic_RecursiveLimit] = std::max(gMaxRecursion[kConic_RecursiveLimit], fRecursionDepth + 1));
#endif
...
#if QUAD_STROKE_APPROX_EXTENDED_DEBUGGING
    SkDEBUGCODE(gMaxRecursion[kQuad_RecursiveLimit] = std::max(gMaxRecursion[kQuad_RecursiveLimit], fRecursionDepth + 1));
#endif
