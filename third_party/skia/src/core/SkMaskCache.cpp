...
struct RRectBlurRec : public SkResourceCache::Rec {
    RRectBlurRec(RRectBlurKey key, const SkMask& mask, SkCachedData* data)
        : fKey(key)
        , fValue({ { nullptr, mask.fBounds, mask.fRowBytes, mask.fFormat }, data })
    {
...
SkCachedData* SkMaskCache::FindAndRef(SkScalar sigma, SkBlurStyle style, const SkRRect& rrect, SkTLazy<SkMask>* mask, SkResourceCache* localCache)
{
    ...
    mask->init(static_cast<const uint8_t*>(result->fData->data()), result->fMask.fBounds, result->fMask.fRowBytes, result->fMask.fFormat);
    return result->fData;
}
...
SkCachedData* SkMaskCache::FindAndRef(SkScalar sigma, SkBlurStyle style, const SkRect rects[], int count, SkTLazy<SkMask>* mask, SkResourceCache* localCache)
{
    SkTLazy<MaskValue> result;
