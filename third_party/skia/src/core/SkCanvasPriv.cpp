...
size_t SkCanvasPriv::WriteLattice(void* buffer, const SkCanvas::Lattice& lattice)
{
    ...
    const size_t size = (1 + lattice.fXCount + 1 + lattice.fYCount + 1) * sizeof(int32_t) + SkAlign4(flagCount * sizeof(SkCanvas::Lattice::RectType))
...
AutoLayerForImageFilter::AutoLayerForImageFilter(SkCanvas* canvas, const SkPaint& paint, const SkRect* rawBounds, bool skipMaskFilterLayer)
    : fPaint(paint)
    , fCanvas(canvas)
...
void AutoLayerForImageFilter::addMaskFilterLayer(const SkRect* drawBounds)
{
    ...
    sk_sp<SkImageFilter> maskFilterAsImageFilter = as_MFB(fPaint.getMaskFilter())->asImageFilter(fCanvas->getTotalMatrix());
...
void AutoLayerForImageFilter::addLayer(const SkPaint& restorePaint, const SkRect* drawBounds, bool coverageOnly)
{
    ...
    fCanvas->fSaveCount += 1;
    fCanvas->internalSaveLayer(SkCanvas::SaveLayerRec(contentBounds, &restorePaint), SkCanvas::kFullLayer_SaveLayerStrategy, coverageOnly);
