...
MaskAdditiveBlitter::MaskAdditiveBlitter(SkBlitter* realBlitter, const SkIRect& ir, const SkIRect& clipBounds, bool isInverse)
...
void MaskAdditiveBlitter::blitAntiRect(int x, int y, int width, int height, SkAlpha leftAlpha, SkAlpha rightAlpha)
{
...
protected:
    ...
    int getRunsSz() const
    {
        return (fWidth + 1 + (fWidth + 2) / 2) * sizeof(int16_t);
    }
...
RunBasedAdditiveBlitter::RunBasedAdditiveBlitter(SkBlitter* realBlitter, const SkIRect& ir, const SkIRect& clipBounds, bool isInverse)
{
    fRealBlitter = realBlitter;
...
static void blit_aaa_trapezoid_row(AdditiveBlitter* blitter, int y, SkFixed ul, SkFixed ur, SkFixed ll, SkFixed lr, SkFixed lDY, SkFixed rDY, SkAlpha fullAlpha,
    SkAlpha* maskRow, bool noRealBlitter)
{
    ...
    if (uL + 2 == lL) { // We only need to compute two triangles, accelerate this special case
        ...
        compute_alpha_below_line(tempAlphas + uL - L, ul - SkIntToFixed(uL), ll - SkIntToFixed(uL), lDY, fullAlpha);
        for (int i = uL; i < lL; ++i) {
    ...
    if (uR + 2 == lR) { // We only need to compute two triangles, accelerate this special case
        ...
        compute_alpha_above_line(tempAlphas + uR - L, ur - SkIntToFixed(uR), lr - SkIntToFixed(uR), rDY, fullAlpha);
        for (int i = uR; i < lR; ++i) {
            if (alphas[i - L] > tempAlphas[i - L]) {
                alphas[i - L] -= tempAlphas[i - L];
            } else {
...
static void blit_trapezoid_row(AdditiveBlitter* blitter, int y, SkFixed ul, SkFixed ur, SkFixed ll, SkFixed lr, SkFixed lDY, SkFixed rDY, SkAlpha fullAlpha,
    SkAlpha* maskRow, bool noRealBlitter)
{
    ...
    if (joinLeft <= joinRite) { // There's a rect from joinLeft to joinRite that we can blit
        if (ul < joinLeft) {
            ...
            if (len == 1) {
                ...
                blit_single_alpha(blitter, y, ul >> 16, alpha, fullAlpha, maskRow, noRealBlitter);
            } else if (len == 2) {
                ...
                blit_two_alphas(blitter, y, ul >> 16, a1, a2, fullAlpha, maskRow, noRealBlitter);
            } else {
                blit_aaa_trapezoid_row(blitter, y, ul, joinLeft, ll, joinLeft, lDY, SK_MaxS32, fullAlpha, maskRow, noRealBlitter);
            }
        }
        ...
        if (joinLeft < joinRite) {
            blit_full_alpha(blitter, y, SkFixedFloorToInt(joinLeft), SkFixedFloorToInt(joinRite - joinLeft), fullAlpha, maskRow, noRealBlitter);
        }
        if (lr > joinRite) {
            int len = SkFixedCeilToInt(lr - joinRite);
            if (len == 1) {
                SkAlpha alpha = trapezoid_to_alpha(ur - joinRite, lr - joinRite);
                blit_single_alpha(blitter, y, joinRite >> 16, alpha, fullAlpha, maskRow, noRealBlitter);
            } else if (len == 2) {
                ...
                blit_two_alphas(blitter, y, joinRite >> 16, a1, a2, fullAlpha, maskRow, noRealBlitter);
            } else {
                blit_aaa_trapezoid_row(blitter, y, joinRite, ur, joinRite, lr, SK_MaxS32, rDY, fullAlpha, maskRow, noRealBlitter);
            }
        }
    } else {
        blit_aaa_trapezoid_row(blitter, y, ul, ur, ll, lr, lDY, rDY, fullAlpha, maskRow, noRealBlitter);
    }
}
...
static void aaa_walk_convex_edges(
    SkAnalyticEdge* prevHead, AdditiveBlitter* blitter, int start_y, int stop_y, SkFixed leftBound, SkFixed riteBound, bool isUsingMask)
{
        ...
        if (0 == (dLeft | dRite)) {
            ...
            if (fullRite >= fullLeft) {
                if (partialTop > 0) { // blit first partial row
                    if (partialLeft > 0) {
                        blitter->blitAntiH(fullLeft - 1, fullTop - 1, fixed_to_alpha(SkFixedMul(partialTop, partialLeft)));
                    }
                    blitter->blitAntiH(fullLeft, fullTop - 1, fullRite - fullLeft, fixed_to_alpha(partialTop));
                    if (partialRite > 0) {
                        blitter->blitAntiH(fullRite, fullTop - 1, fixed_to_alpha(SkFixedMul(partialTop, partialRite)));
                    }
                ...
                if (fullBot > fullTop &&
                    ...
                    blitter->getRealBlitter()->blitAntiRect(
                        fullLeft - 1, fullTop, fullRite - fullLeft, fullBot - fullTop, fixed_to_alpha(partialLeft), fixed_to_alpha(partialRite));
                }

                if (partialBot > 0) { // blit last partial row
                    if (partialLeft > 0) {
                        blitter->blitAntiH(fullLeft - 1, fullBot, fixed_to_alpha(SkFixedMul(partialBot, partialLeft)));
                    }
                    blitter->blitAntiH(fullLeft, fullBot, fullRite - fullLeft, fixed_to_alpha(partialBot));
                    if (partialRite > 0) {
                        blitter->blitAntiH(fullRite, fullBot, fixed_to_alpha(SkFixedMul(partialBot, partialRite)));
                    }
                }
            } else {
                ...
                if (width > 0) {
                    if (partialTop > 0) {
                        blitter->blitAntiH(fullLeft - 1, fullTop - 1, 1, fixed_to_alpha(SkFixedMul(partialTop, width)));
                        blitter->flush_if_y_changed(y, y + partialTop);
                    }
                    if (fullBot > fullTop) {
                        blitter->getRealBlitter()->blitV(fullLeft - 1, fullTop, fullBot - fullTop, fixed_to_alpha(width));
                    }
                    if (partialBot > 0) {
                        blitter->blitAntiH(fullLeft - 1, fullBot, 1, fixed_to_alpha(SkFixedMul(partialBot, width)));
                    }
                }
            }
            ...
            SkAlpha* maskRow = isUsingMask ? static_cast<MaskAdditiveBlitter*>(blitter)->getRow(y >> 16) : nullptr;
            ...
            if (count > 1) {
                if ((int)(y & 0xFFFF0000) != y) { // There's a partial-row on the top
                    ...
                    SkASSERT((left & kSnapMask) >= leftBound && (rite & kSnapMask) <= riteBound && (nextLeft & kSnapMask) >= leftBound
                        && (nextRite & kSnapMask) <= riteBound);
                    blit_trapezoid_row(blitter, y >> 16, left & kSnapMask, rite & kSnapMask, nextLeft & kSnapMask, nextRite & kSnapMask, leftE->fDY, riteE->fDY,
                        get_partial_alpha(0xFF, dY), maskRow,
                        /*noRealBlitter=*/false);
...
static void aaa_walk_edges(SkAnalyticEdge* prevHead, SkAnalyticEdge* nextTail, SkPathFillType fillType, AdditiveBlitter* blitter, int start_y, int stop_y,
    SkFixed leftClip, SkFixed rightClip, bool isUsingMask, bool forceRLE, bool skipIntersect)
{
    ...
    if (isInverse && SkIntToFixed(start_y) != y) {
        ...
        if (SkFixedFloorToInt(y) != start_y) {
            blitter->getRealBlitter()->blitRect(SkFixedFloorToInt(leftClip), start_y, width, SkFixedFloorToInt(y) - start_y);
            start_y = SkFixedFloorToInt(y);
        }
        SkAlpha* maskRow = isUsingMask ? static_cast<MaskAdditiveBlitter*>(blitter)->getRow(start_y) : nullptr;
        blit_full_alpha(blitter, start_y, SkFixedFloorToInt(leftClip), width, fixed_to_alpha(y - SkIntToFixed(start_y)), maskRow, false);
    }
    ...
    while (true) {
        ...
        // If we're using mask blitter, we advance the mask row in this function
        // to save some "if" condition checks.
        SkAlpha* maskRow = isUsingMask ? static_cast<MaskAdditiveBlitter*>(blitter)->getRow(SkFixedFloorToInt(y)) : nullptr;
        ...
        while (currE->fUpperY <= y) {
            ...
            if (isRite) {
                ...
                blit_trapezoid_row(blitter, y >> 16, left, rite, nextLeft, nextRite, leftDY, currE->fDY, fullAlpha, maskRow,
        ...
        if (in_interval) {
            blit_trapezoid_row(blitter, y >> 16, left, rightClip, std::max(leftClip, leftE->fX), rightClip, leftDY, 0, fullAlpha, maskRow,
...
static void aaa_fill_path(
    const SkPath& path, const SkIRect& clipRect, AdditiveBlitter* blitter, int start_y, int stop_y, bool pathContainedInClip, bool isUsingMask, bool forceRLE)
{ // forceRLE implies that SkAAClip is calling us
    ...
    if (!path.isInverseFillType() && path.isConvex() && count >= 2) {
        ...
        aaa_walk_edges(&headEdge, &tailEdge, path.getFillType(), blitter, start_y, stop_y, leftBound, rightBound, isUsingMask, forceRLE, skipIntersect);
    }
}

// Check if the path is a rect and fat enough after clipping; if so, blit it.
static inline bool try_blit_fat_anti_rect(SkBlitter* blitter, const SkPath& path, const SkIRect& clip)
{
    SkRect rect;
    if (!path.isRect(&rect)) {
...
void SkScan::AAAFillPath(const SkPath& path, SkBlitter* blitter, const SkIRect& ir, const SkIRect& clipBounds, bool forceRLE)
{
    ...
    if (MaskAdditiveBlitter::CanHandleRect(ir) && !isInverse && !forceRLE) {
        ...
        if (!try_blit_fat_anti_rect(blitter, path, clipBounds)) {
            ...
            aaa_fill_path(path, clipBounds, &additiveBlitter, ir.fTop, ir.fBottom, containedInClip, true, forceRLE);
        }
        ...
        aaa_fill_path(path, clipBounds, &additiveBlitter, ir.fTop, ir.fBottom, containedInClip, false, forceRLE);
    } else {
