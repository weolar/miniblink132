...
size_t SkRecordedDrawable::onApproximateBytesUsed()
{
    ...
    return sizeof(*this) + (fRecord ? fRecord->bytesUsed() : 0) + (fBBH ? fBBH->bytesUsed() : 0) + drawablesSize;
}
...
sk_sp<SkPicture> SkRecordedDrawable::onMakePictureSnapshot()
{
    ...
    //       PictureRecorder?
    std::unique_ptr<SkBigPicture::SnapshotArray> pictList { fDrawableList ? fDrawableList->newDrawableSnapshot() : nullptr };
