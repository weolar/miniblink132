...
static bool rect_memcpy(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB, const SkImageInfo& srcInfo, const void* srcPixels, size_t srcRB,
...
static bool swizzle_or_premul(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB, const SkImageInfo& srcInfo, const void* srcPixels, size_t srcRB,
    const SkColorSpaceXformSteps& steps)
{
    auto is_8888 = [](SkColorType ct) { return ct == kRGBA_8888_SkColorType || ct == kBGRA_8888_SkColorType; };
    if (!is_8888(dstInfo.colorType()) || !is_8888(srcInfo.colorType()) || steps.flags.linearize || steps.flags.gamut_transform ||
...
static bool convert_to_alpha8(
    const SkImageInfo& dstInfo, void* vdst, size_t dstRB, const SkImageInfo& srcInfo, const void* src, size_t srcRB, const SkColorSpaceXformSteps&)
{
    ...
    case kBGRA_10101010_XR_SkColorType: {
        ...
        for (int y = 0; y < srcInfo.height(); y++) {
            for (int x = 0; x < srcInfo.width(); x++) {
                ...
                // f(384) = 0
                // f(894) = 255
                int64_t alpha = SkTPin((raw_alpha - kZero) / kDivisor, kMinU8, kMaxU8);
...
static void convert_with_pipeline(
    const SkImageInfo& dstInfo, void* dstRow, int dstStride, const SkImageInfo& srcInfo, const void* srcRow, int srcStride, const SkColorSpaceXformSteps& steps)
{
    SkRasterPipeline_MemoryCtx src = { const_cast<void*>(srcRow), srcStride }, dst = { dstRow, dstStride };
...
bool SkConvertPixels(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB, const SkImageInfo& srcInfo, const void* srcPixels, size_t srcRB)
{
    ...
    if ((size_t)srcStride * srcInfo.bytesPerPixel() != srcRB || (size_t)dstStride * dstInfo.bytesPerPixel() != dstRB) {
        return false;
    }
