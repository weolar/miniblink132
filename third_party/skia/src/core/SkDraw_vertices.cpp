...
[[nodiscard]] static bool texture_to_matrix(const VertState& state, const SkPoint verts[], const SkPoint texs[], SkMatrix* matrix)
{
    SkPoint src[3], dst[3];
...
static SkPMColor4f* convert_colors(const SkColor src[], int count, SkColorSpace* deviceCS, SkArenaAlloc* alloc, bool skipColorXform)
{
    ...
    SkImageInfo dstInfo = SkImageInfo::Make(count, 1, kRGBA_F32_SkColorType, kPremul_SkAlphaType, dstCS);
...
void SkDraw::drawFixedVertices(const SkVertices* vertices, sk_sp<SkBlender> blender, const SkPaint& paint, const SkMatrix& ctmInverse, const SkPoint* dev2,
    const SkPoint3* dev3, SkArenaAlloc* outerAlloc, bool skipColorXform) const
{
    ...
    if (colors) {
        ...
        triColorShader = outerAlloc->make<SkTriColorShader>(compute_is_opaque(colors, vertexCount), usePerspective);
    }
    ...
    auto applyShaderColorBlend = [&](SkShader* shader) -> sk_sp<SkShader> {
        ...
        return SkShaders::Blend(blender, sk_ref_sp(triColorShader), std::move(shaderWithWhichToBlend));
    };
    ...
    if (texCoords && texCoords != positions) {
        paintShader = transformShader = outerAlloc->make<SkTransformShader>(*as_SB(paintShader), usePerspective);
        ctm = &SkMatrix::I();
    }
    ...
    auto blitter = SkCreateRasterPipelineBlitter(fDst, finalPaint, *ctm, outerAlloc, fRC->clipShader(), props);
    if (!blitter) {
        return;
    }
    while (vertProc(&state)) {
        if (triColorShader && !triColorShader->update(ctmInverse, positions, dstColors, state.f0, state.f1, state.f2)) {
            continue;
        }

        SkMatrix localM;
        if (!transformShader || (texture_to_matrix(state, positions, texCoords, &localM) && transformShader->update(SkMatrix::Concat(localM, ctmInverse)))) {
            fill_triangle(state, blitter, *fRC, dev2, dev3);
        }
    }
}

void SkDraw::drawVertices(const SkVertices* vertices, sk_sp<SkBlender> blender, const SkPaint& paint, bool skipColorXform) const
{
    SkVerticesPriv info(vertices->priv());
    const int vertexCount = info.vertexCount();
    const int indexCount = info.indexCount();
    ...
    constexpr size_t kOuterSize = sizeof(SkTriColorShader) + (2 * sizeof(SkPoint) + sizeof(SkColor4f)) * kDefVertexCount;
    ...
    this->drawFixedVertices(vertices, std::move(blender), paint, ctmInv, dev2, dev3, &outerAlloc, skipColorXform);
}
