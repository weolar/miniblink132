...
struct SkPackedGlyphID {
    ...
    inline static const constexpr SkScalar kSubpixelRound = 1.f / (1u << (SkPackedGlyphID::kSubPixelPosLen + 1));

    inline static const constexpr SkIPoint kXYFieldMask { kSubPixelPosMask << kSubPixelX, kSubPixelPosMask << kSubPixelY };
    ...
    constexpr explicit SkPackedGlyphID(uint32_t v)
        : fID { v & kMaskAll }
    {
    }
    ...
    {
        SkString str;
        str.appendf("0x%x|%1u|%1u", this->glyphID(), this->subPixelField(kSubPixelX), this->subPixelField(kSubPixelY));
        return str;
    }
...
#else
        const float magicX = 1.f * (1u << (kSubPixelPosLen + kSubPixelX)), magicY = 1.f * (1u << (kSubPixelPosLen + kSubPixelY));
...
public:
    ...
    {
        return -this->negLeftTop();
    }
    SkPoint rightBottom() const
    {
        return { fRect[2], fRect[3] };
    }
    SkPoint widthHeight() const
    {
        return this->rightBottom() + negLeftTop();
    }
...
public:
    ...
    bool isColor() const
    {
        return fFormat == SkMask::kARGB32_Format;
    }
    SkMask::Format maskFormat() const
    {
        return static_cast<SkMask::Format>(fFormat);
    }
