...
bool SkBlurMaskFilterImpl::filterMask(SkMaskBuilder* dst, const SkMask& src, const SkMatrix& matrix, SkIPoint* margin) const
{
...
bool SkBlurMaskFilterImpl::filterRectMask(
    SkMaskBuilder* dst, const SkRect& r, const SkMatrix& matrix, SkIPoint* margin, SkMaskBuilder::CreateMode createMode) const
{
...
bool SkBlurMaskFilterImpl::filterRRectMask(
    SkMaskBuilder* dst, const SkRRect& r, const SkMatrix& matrix, SkIPoint* margin, SkMaskBuilder::CreateMode createMode) const
{
    SkScalar sigma = computeXformedSigma(matrix);
...
{
    return draw_into_mask(mask, rrect.rect(), [&](SkDrawBase& draw, const SkPaint& paint) { draw.drawRRect(rrect, paint); });
}
...
SkMaskFilterBase::FilterReturn SkBlurMaskFilterImpl::filterRRectToNine(
    const SkRRect& rrect, const SkMatrix& matrix, const SkIRect& clipBounds, SkTLazy<NinePatch>* patch) const
{
    ...
    if (c_analyticBlurRRect) {
        ...
        filterResult = this->filterRRectMask(&dstM, rrect, matrix, &margin, SkMaskBuilder::kJustComputeBounds_CreateMode);
    }
    ...
    if (!cache) {
        ...
        if (c_analyticBlurRRect) {
            analyticBlurWorked = this->filterRRectMask(&filterM, smallRR, matrix, &margin, SkMaskBuilder::kComputeBoundsAndRenderImage_CreateMode);
        }
    ...
    bounds.offsetTo(0, 0);
    patch->init(SkMask { cachedMask->fImage, bounds, cachedMask->fRowBytes, cachedMask->fFormat }, dstM.fBounds,
...
SkMaskFilterBase::FilterReturn SkBlurMaskFilterImpl::filterRectsToNine(
    const SkRect rects[], int count, const SkMatrix& matrix, const SkIRect& clipBounds, SkTLazy<NinePatch>* patch) const
{
    if (count < 1 || count > 2) {
        return kUnimplemented_FilterReturn;
    }
    ...
    if (count == 1 && c_analyticBlurNinepatch) {
        ...
        // don't actually do the blur the first time, just compute the correct size
        filterResult = this->filterRectMask(&dstM, rects[0], matrix, &margin, SkMaskBuilder::kJustComputeBounds_CreateMode);
    } else {
        filterResult = this->filterMask(&dstM, srcM, matrix, &margin);
    }
    ...
    if (1 == count) {
        ...
        center.set(smallW + (innerIR.left() - srcM.fBounds.left()), smallH + (innerIR.top() - srcM.fBounds.top()));
    }
    ...
    if (!cache) {
        ...
        if (count > 1 || !c_analyticBlurNinepatch) {
            ...
            if (!this->filterRectMask(&filterM, smallR[0], matrix, &margin, SkMaskBuilder::kComputeBoundsAndRenderImage_CreateMode)) {
                return kFalse_FilterReturn;
            }
        }
    ...
    SkIRect bounds = cachedMask->fBounds;
    bounds.offsetTo(0, 0);
    patch->init(SkMask { cachedMask->fImage, bounds, cachedMask->fRowBytes, cachedMask->fFormat }, dstM.fBounds, center, cache); // transfer ownership to patch
    return kTrue_FilterReturn;
}
