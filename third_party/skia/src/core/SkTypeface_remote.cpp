...
SkScalerContextProxy::SkScalerContextProxy(
    sk_sp<SkTypeface> tf, const SkScalerContextEffects& effects, const SkDescriptor* desc, sk_sp<SkStrikeClient::DiscardableHandleManager> manager)
    : SkScalerContext { std::move(tf), effects, desc }
    , fDiscardableManager { std::move(manager) }
{
}

SkScalerContext::GlyphMetrics SkScalerContextProxy::generateMetrics(const SkGlyph& glyph, SkArenaAlloc*)
{
    ...
    fDiscardableManager->notifyCacheMiss(SkStrikeClient::CacheMissType::kGlyphMetrics, fRec.fTextSize);
...
sk_sp<SkDrawable> SkScalerContextProxy::generateDrawable(const SkGlyph&)
{
    ...
    fDiscardableManager->notifyCacheMiss(SkStrikeClient::CacheMissType::kGlyphDrawable, fRec.fTextSize);
    return nullptr;
}
...
std::optional<SkTypefaceProxyPrototype> SkTypefaceProxyPrototype::MakeFromBuffer(SkReadBuffer& buffer)
{
    ...
    if (buffer.isValid()) {
        return SkTypefaceProxyPrototype { typefaceID, glyphCount, styleValue, isFixedPitch, glyphMaskNeedsCurrentColor };
    }
...
SkTypefaceProxyPrototype::SkTypefaceProxyPrototype(
    SkTypefaceID typefaceID, int glyphCount, int32_t styleValue, bool isFixedPitch, bool glyphMaskNeedsCurrentColor)
...
SkTypefaceProxy::SkTypefaceProxy(const SkTypefaceProxyPrototype& prototype, sk_sp<SkStrikeClient::DiscardableHandleManager> manager, bool isLogging)
...
SkTypefaceProxy::SkTypefaceProxy(SkTypefaceID typefaceID, int glyphCount, const SkFontStyle& style, bool isFixedPitch, bool glyphMaskNeedsCurrentColor,
    sk_sp<SkStrikeClient::DiscardableHandleManager> manager, bool isLogging)
