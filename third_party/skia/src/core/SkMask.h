...
struct SkMaskBuilder : public SkMask {
    SkMaskBuilder()
        : SkMask(nullptr, { 0 }, 0, SkMask::Format::kBW_Format)
    {
    }
    ...
    uint8_t*& image()
    {
        return *const_cast<uint8_t**>(&fImage);
    }
    SkIRect& bounds()
    {
        return *const_cast<SkIRect*>(&fBounds);
    }
    uint32_t& rowBytes()
    {
        return *const_cast<uint32_t*>(&fRowBytes);
    }
    Format& format()
    {
        return *const_cast<Format*>(&fFormat);
    }
