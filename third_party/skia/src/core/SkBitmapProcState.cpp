...
static bool just_trans_general(const SkMatrix& matrix)
{
    ...
    return SkScalarNearlyZero(matrix[SkMatrix::kMScaleX] - SK_Scalar1, tol) && SkScalarNearlyZero(matrix[SkMatrix::kMScaleY] - SK_Scalar1, tol);
}
...
static bool just_trans_integral(const SkMatrix& m)
{
    ...
    return m.getType() <= SkMatrix::kTranslate_Mask && SkScalarNearlyEqual(m.getTranslateX(), SkScalarRoundToScalar(m.getTranslateX()), tol)
...
bool SkBitmapProcState::init(const SkMatrix& inv, SkAlpha paintAlpha, const SkSamplingOptions& sampling)
{
    ...
    if (fBilerp && (!valid_for_filtering(fPixmap.width() | fPixmap.height()) || integral_translate_only)) {
        fBilerp = false;
    }
...
bool SkBitmapProcState::chooseProcs()
{
    ...
    if (fAlphaScale == 256 && !fBilerp && SkTileMode::kClamp == fTileModeX && SkTileMode::kClamp == fTileModeY && fInvMatrix.isScaleTranslate()) {
...
static inline void filter_32_alpha(unsigned t, SkPMColor color0, SkPMColor color1, SkPMColor* dstColor, unsigned alphaScale)
{
...
void SkBitmapProcState::DebugMatrixProc(const SkBitmapProcState& state, uint32_t bitmapXY[], int count, int x, int y)
{
    SkASSERT(bitmapXY);
    SkASSERT(count > 0);
