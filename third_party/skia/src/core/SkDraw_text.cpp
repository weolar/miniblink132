...
static bool check_glyph_position(SkPoint position)
{
    ...
    return !(gt(position.fX, INT_MAX - (INT16_MAX + SkTo<int>(UINT16_MAX))) || lt(position.fX, INT_MIN - (INT16_MIN + 0 /*UINT16_MIN*/))
...
void SkDraw::paintMasks(SkZip<const SkGlyph*, SkPoint> accepted, const SkPaint& paint) const
{
    ...
    SkBlitter* blitter = SkBlitter::Choose(fDst, *fCTM, paint, &alloc, false, fRC->clipShader(), SkSurfacePropsCopyOrDefault(fProps));
...
void SkDraw::drawGlyphRunList(SkCanvas* canvas, SkGlyphRunListPainterCPU* glyphPainter, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint) const
{
