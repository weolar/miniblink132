...
public:
    ...
    static_assert(
        AllowCopyableMessage || is_sk_sp<Message>::value || !std::is_copy_constructible<Message>::value, "The message type must be sk_sp or non copyable.");
...
#define DECLARE_SKMESSAGEBUS_MESSAGE(Message, IDType, AllowCopyableMessage)                                                                                    \
    template <> SkMessageBus<Message, IDType, AllowCopyableMessage>* SkMessageBus<Message, IDType, AllowCopyableMessage>::Get()                                \
    {                                                                                                                                                          \
        static SkOnce once;                                                                                                                                    \
...
template <typename Message, typename IDType, bool AllowCopyableMessage> SkMessageBus<Message, IDType, AllowCopyableMessage>::Inbox::~Inbox()
{
...
template <typename Message, typename IDType, bool AllowCopyableMessage> void SkMessageBus<Message, IDType, AllowCopyableMessage>::Inbox::receive(Message m)
{
...
template <typename Message, typename IDType, bool AllowCopyableMessage>
void SkMessageBus<Message, IDType, AllowCopyableMessage>::Inbox::poll(skia_private::TArray<Message>* messages)
{
    SkASSERT(messages);
    messages->clear();
    SkAutoMutexExclusive lock(fMessagesMutex);
...
template <typename Message, typename IDType, bool AllowCopyableMessage> SkMessageBus<Message, IDType, AllowCopyableMessage>::SkMessageBus() = default;
