...
template <typename T> class DeviceSpace {
public:
    DeviceSpace() = default;
...
template <typename T> class LayerSpace { };

// Layer-space specialization for integerized direction vectors.
template <> class LayerSpace<IVector> {
public:
    LayerSpace() = default;
    ...
    explicit operator const IVector&() const
    {
        return fData;
    }

    explicit operator SkIVector() const
    {
...
template <> class LayerSpace<Vector> {
public:
    LayerSpace() = default;
    ...
    explicit operator SkVector() const
    {
...
template <> class LayerSpace<SkIPoint> {
public:
    LayerSpace() = default;
    ...
    explicit operator const SkIPoint&() const
    {
        return fData;
    }
...
template <> class LayerSpace<SkPoint> {
public:
    LayerSpace() = default;
    ...
    explicit operator const SkPoint&() const
    {
        return fData;
    }
...
template <> class LayerSpace<SkISize> {
public:
    LayerSpace() = default;
    ...
    explicit operator const SkISize&() const
    {
        return fData;
    }
...
template <> class LayerSpace<SkSize> {
public:
    LayerSpace() = default;
    ...
    explicit operator const SkSize&() const
    {
        return fData;
    }
...
template <> class LayerSpace<SkIRect> {
public:
    LayerSpace() = default;
    ...
    explicit LayerSpace(const SkISize& size)
        : fData(SkIRect::MakeSize(size))
    {
    }
    explicit operator const SkIRect&() const
    {
        return fData;
    }
    ...
    {
        return LayerSpace<SkIRect>(SkIRect::MakeEmpty());
    }
    ...
    // boundsCount-1.
    template <typename BoundsFn> static LayerSpace<SkIRect> Union(int boundsCount, BoundsFn boundsFn)
    {
        if (boundsCount <= 0) {
    ...
    {
        fData.inset(delta.width(), delta.height());
    }
...
template <> class LayerSpace<SkRect> {
public:
    LayerSpace() = default;
    ...
    explicit LayerSpace(const LayerSpace<SkIRect>& rect)
        : fData(SkRect::Make(SkIRect(rect)))
    {
    }
    explicit operator const SkRect&() const
    {
        return fData;
    }
    ...
    {
        return LayerSpace<SkRect>(SkRect::MakeEmpty());
    }
    ...
    {
        return LayerSpace<SkIRect>(fData.round());
    }
    LayerSpace<SkIRect> roundIn() const
    {
        return LayerSpace<SkIRect>(RoundIn(fData));
    }
    LayerSpace<SkIRect> roundOut() const
    {
        return LayerSpace<SkIRect>(RoundOut(fData));
    }
    ...
    {
        return LayerSpace<SkPoint>(SkPoint::Make(SkTPin(pt.x(), fData.fLeft, fData.fRight), SkTPin(pt.y(), fData.fTop, fData.fBottom)));
    }
...
template <> class LayerSpace<SkMatrix> {
public:
    LayerSpace() = default;
    explicit LayerSpace(const SkMatrix& m)
        : fData(m)
    {
    }
    explicit LayerSpace(SkMatrix&& m)
        : fData(std::move(m))
    {
    }
    explicit operator const SkMatrix&() const
    {
        return fData;
    }
...
public:
    ...
    [[nodiscard]] bool decomposeCTM(const SkMatrix& ctm, const SkImageFilter* filter, const skif::ParameterSpace<SkPoint>& representativePt);
    [[nodiscard]] bool decomposeCTM(const SkMatrix& ctm, MatrixCapability, const skif::ParameterSpace<SkPoint>& representativePt);
    ...
    {
        fParamToLayerMatrix.preConcat(local);
    }
    ...
    template <typename T> LayerSpace<T> paramToLayer(const ParameterSpace<T>& paramGeometry) const
    {
        return LayerSpace<T>(map(static_cast<const T&>(paramGeometry), fParamToLayerMatrix));
    }

    template <typename T> LayerSpace<T> deviceToLayer(const DeviceSpace<T>& devGeometry) const
    {
        return LayerSpace<T>(map(static_cast<const T&>(devGeometry), fDevToLayerMatrix));
    }

    template <typename T> DeviceSpace<T> layerToDevice(const LayerSpace<T>& layerGeometry) const
    {
...
private:
    ...
    template <typename T> static T map(const T& geom, const SkMatrix& matrix);
};
...
public:
    ...
    static FilterResult MakeFromImage(
...
private:
    ...
    FilterResult(sk_sp<SkSpecialImage> image, const LayerSpace<SkIPoint>& origin, PixelBoundary boundary)
        ...
        , fLayerBounds(fTransform.mapRect(LayerSpace<SkIRect>(fImage ? fImage->dimensions() : SkISize { 0, 0 })))
    {
    }
    ...
    // is true.
    FilterResult resolve(const Context& ctx, LayerSpace<SkIRect> dstBounds, bool preserveDstBounds = false) const;
    ...
    FilterResult subset(const LayerSpace<SkIPoint>& knownOrigin, const LayerSpace<SkIRect>& subsetBounds, bool clampSrcIfDisjoint = false) const;
    ...
    SkEnumBitMask<BoundsAnalysis> analyzeBounds(const SkMatrix& xtraTransform, const SkIRect& dstBounds, BoundsScope scope = BoundsScope::kDeferred) const;
    SkEnumBitMask<BoundsAnalysis> analyzeBounds(const LayerSpace<SkIRect>& dstBounds, BoundsScope scope = BoundsScope::kDeferred) const
    {
    ...
    {
        return fTileMode == SkTileMode::kDecal && fBoundary == PixelBoundary::kTransparent && !(analysis & BoundsAnalysis::kRequiresDecalInLayerSpace);
    }
    ...
    FilterResult rescale(const Context& ctx, const LayerSpace<SkSize>& scale, bool enforceDecal) const;
    ...
    void draw(const Context& ctx, SkDevice* device, bool preserveDeviceState, const SkBlender* blender = nullptr) const;
    ...
    sk_sp<SkShader> getAnalyzedShaderView(const Context& ctx, const SkSamplingOptions& finalSampling, SkEnumBitMask<BoundsAnalysis> analysis) const;
...
public:
    ...
    Builder& add(const FilterResult& input, std::optional<LayerSpace<SkIRect>> sampleBounds = {}, SkEnumBitMask<ShaderFlags> inputFlags = ShaderFlags::kNone,
...
private:
    ...
    SkSpan<sk_sp<SkShader>> createInputShaders(const LayerSpace<SkIRect>& outputBounds, bool evaluateInParameterSpace);

    LayerSpace<SkIRect> outputBounds(std::optional<LayerSpace<SkIRect>> explicitOutput) const;

    FilterResult drawShader(sk_sp<SkShader> shader, const LayerSpace<SkIRect>& outputBounds, bool evaluateInParameterSpace) const;
...
class Context {
public:
    Context(sk_sp<Backend> backend, const Mapping& mapping, const LayerSpace<SkIRect>& desiredOutput, const FilterResult& source,
