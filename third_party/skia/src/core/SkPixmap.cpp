...
bool SkPixmap::reset(const SkMask& src)
{
    if (SkMask::kA8_Format == src.fFormat) {
        this->reset(SkImageInfo::MakeA8(src.fBounds.width(), src.fBounds.height()), src.fImage, src.fRowBytes);
        return true;
    }
    this->reset();
    return false;
...
bool SkPixmap::readPixels(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB, int x, int y) const
{
    ...
    return SkConvertPixels(rec.fInfo, rec.fPixels, rec.fRowBytes, srcInfo, srcPixels, this->rowBytes());
}
...
SkColor SkPixmap::getColor(int x, int y) const
{
    ...
    auto toColor = [needsUnpremul](uint32_t maybePremulColor) {
        return needsUnpremul ? SkUnPreMultiply::PMColorToColor(maybePremulColor) : SkSwizzle_BGRA_to_PMColor(maybePremulColor);
    };
    ...
    case kR8G8_unorm_SkColorType: {
        uint16_t value = *this->addr16(x, y);
        return (uint32_t)(((value >> 0) & 0xff)) << 16 | (uint32_t)(((value >> 8) & 0xff)) << 8 | 0xff000000;
    }
    case kR16G16_unorm_SkColorType: {
        uint32_t value = *this->addr32(x, y);
        return (uint32_t)(((value >> 0) & 0xffff) * (255 / 65535.0f)) << 16 | (uint32_t)(((value >> 16) & 0xffff) * (255 / 65535.0f)) << 8 | 0xff000000;
    }
    case kR16G16_float_SkColorType: {
        uint32_t value = *this->addr32(x, y);
    ...
    case kSRGBA_8888_SkColorType: {
        auto srgb_to_linear = [](float x) { return (x <= 0.04045f) ? x * (1 / 12.92f) : std::pow(x * (1 / 1.055f) + (0.055f / 1.055f), 2.4f); };

        uint32_t value = *this->addr32(x, y);
        float r = ((value >> 0) & 0xff) * (1 / 255.0f), g = ((value >> 8) & 0xff) * (1 / 255.0f), b = ((value >> 16) & 0xff) * (1 / 255.0f),
        ...
        return (uint32_t)(r * 255.0f) << 16 | (uint32_t)(g * 255.0f) << 8 | (uint32_t)(b * 255.0f) << 0 | (uint32_t)(a * 255.0f) << 24;
    }
    case kRGB_101010x_SkColorType: {
        uint32_t value = *this->addr32(x, y);
        // Convert 10-bit rgb to 8-bit bgr, and mask in 0xff alpha at the top.
        return (uint32_t)(((value >> 0) & 0x3ff) * (255 / 1023.0f)) << 16 | (uint32_t)(((value >> 10) & 0x3ff) * (255 / 1023.0f)) << 8
    ...
    case kBGR_101010x_SkColorType: {
        ...
        return (uint32_t)(((value >> 0) & 0x3ff) * (255 / 1023.0f)) << 0 | (uint32_t)(((value >> 10) & 0x3ff) * (255 / 1023.0f)) << 8
    ...
    case kBGRA_1010102_SkColorType: {
        ...
        float r = ((value >> 0) & 0x3ff) * (1 / 1023.0f), g = ((value >> 10) & 0x3ff) * (1 / 1023.0f), b = ((value >> 20) & 0x3ff) * (1 / 1023.0f),
    ...
    case kRGBA_10x6_SkColorType: {
        ...
        float r = ((value >> 6) & 0x3ff) * (1 / 1023.0f), g = ((value >> 22) & 0x3ff) * (1 / 1023.0f), b = ((value >> 38) & 0x3ff) * (1 / 1023.0f),
    ...
    case kR16G16B16A16_unorm_SkColorType: {
        ...
        float r = ((value)&0xffff) * (1 / 65535.0f), g = ((value >> 16) & 0xffff) * (1 / 65535.0f), b = ((value >> 32) & 0xffff) * (1 / 65535.0f),
...
SkColor4f SkPixmap::getColor4f(int x, int y) const
{
    ...
    case kR8G8_unorm_SkColorType: {
        uint16_t value = *this->addr16(x, y);
        SkColor c = (uint32_t)(((value >> 0) & 0xff)) << 16 | (uint32_t)(((value >> 8) & 0xff)) << 8 | 0xff000000;
        return SkColor4f::FromColor(c);
    }
    case kR16G16_unorm_SkColorType: {
        uint32_t value = *this->addr32(x, y);
        SkColor c = (uint32_t)(((value >> 0) & 0xffff) * (255 / 65535.0f)) << 16 | (uint32_t)(((value >> 16) & 0xffff) * (255 / 65535.0f)) << 8 | 0xff000000;
        return SkColor4f::FromColor(c);
    }
    case kR16G16_float_SkColorType: {
        uint32_t value = *this->addr32(x, y);
...
bool SkPixmap::erase(const SkColor4f& color, const SkIRect* subset) const
{
    ...
    const auto dst = SkImageInfo::Make(1, 1, this->colorType(), this->alphaType(), sk_ref_sp(this->colorSpace()));
    ...
    if (this->colorType() == kRGBA_F32_SkColorType) {
        ...
        const MemSet procs[] = {
            ...
            [](void* addr, uint64_t c, int count) { SK_OPTS_NS::memsetT((uint64_t*)addr, c, count); },
        };
