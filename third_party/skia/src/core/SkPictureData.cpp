...
bool SkPictureData::parseStreamTag(
    SkStream* stream, uint32_t tag, uint32_t size, const SkDeserialProcs& procs, SkTypefacePlayback* topLevelTFPlayback, int recursionLimit)
{
    switch (tag) {
    ...
    case SK_PICT_PICTURE_TAG: {
        ...
        for (uint32_t i = 0; i < size; i++) {
            auto pic = SkPicture::MakeFromStreamPriv(stream, &procs, topLevelTFPlayback, recursionLimit - 1);
            if (!pic) {
                return false;
            }
...
SkPictureData* SkPictureData::CreateFromStream(
    SkStream* stream, const SkPictInfo& info, const SkDeserialProcs& procs, SkTypefacePlayback* topLevelTFPlayback, int recursionLimit)
{
...
bool SkPictureData::parseStream(SkStream* stream, const SkDeserialProcs& procs, SkTypefacePlayback* topLevelTFPlayback, int recursionLimit)
{
    for (;;) {
        uint32_t tag;
...
const SkPaint* SkPictureData::optionalPaint(SkReadBuffer* reader) const
{
    ...
    return reader->validate(index > 0 && index <= fPaints.size()) ? &fPaints[index - 1] : nullptr;
}
