...
public:
    ...
    */
    ...
    public:
        Iterate(const SkPath& path)
            : Iterate(path.fPathRef->verbsBegin(),
                ...
                (!path.isFinite()) ? path.fPathRef->verbsBegin() : path.fPathRef->verbsEnd(), path.fPathRef->points(), path.fPathRef->conicWeights())
        {
        }
        ...
        {
            return { fVerbsEnd, nullptr, nullptr };
        }
    ...
    static bool IsRectContour(
        const SkPath&, bool allowPartial, int* currVerb, const SkPoint** ptsPtr, bool* isClosed, SkPathDirection* direction, SkRect* rect);
    ...
    static SkPath MakePath(const SkPathVerbAnalysis& analysis, const SkPoint points[], const uint8_t verbs[], int verbCount, const SkScalar conics[],
