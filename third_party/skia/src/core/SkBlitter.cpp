...
void SkBlitter::blitMask(const SkMask& mask, const SkIRect& clip)
{
    ...
    if (mask.fFormat == SkMask::kBW_Format) {
        ...
        SkDEBUGCODE(const uint8_t* endOfImage = mask.fImage + (mask.fBounds.height() - 1) * maskRowBytes + ((mask.fBounds.width() + 7) >> 3));
...
{
    SkRegionPriv::VisitSpans(clip, [this](const SkIRect& r) { this->blitRect(r.left(), r.top(), r.width(), r.height()); });
}
...
void SkRectClipBlitter::blitAntiH(int left, int y, const SkAlpha const_aa[], const int16_t const_runs[])
{
...
bool SkBlitter::UseLegacyBlitter(const SkPixmap& device, const SkPaint& paint, const SkMatrix& matrix)
{
    ...
    if (device.alphaType() == kUnpremul_SkAlphaType || !paint.isSrcOver() || (mf && mf->getFormat() == SkMask::k3D_Format)) {
        return false;
    }
...
SkBlitter* SkBlitter::Choose(const SkPixmap& device, const SkMatrix& ctm, const SkPaint& origPaint, SkArenaAlloc* alloc, bool drawCoverage,
    sk_sp<SkShader> clipShader, const SkSurfaceProps& props)
{
    ...
    auto CreateSkRPBlitter = [&]() -> SkBlitter* {
        ...
        return blitter ? blitter : alloc->make<SkNullBlitter>();
    };
    ...
    if (paint->getShader()) {
        shaderContext
            = as_SB(paint->getShader())->makeContext({ paint->getAlpha(), SkShaders::MatrixRec(ctm), device.colorType(), device.colorSpace(), props }, alloc);
