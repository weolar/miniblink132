...
void SkRRect::setRectXY(const SkRect& rect, SkScalar xRad, SkScalar yRad)
{
    ...
    if (fRect.width() < xRad + xRad || fRect.height() < yRad + yRad) {
        ...
        SkScalar scale = std::min(sk_ieee_float_divide(fRect.width(), xRad + xRad), sk_ieee_float_divide(fRect.height(), yRad + yRad));
...
bool SkRRect::checkCornerContainment(SkScalar x, SkScalar y) const
{
    ...
    if (kOval_Type == this->type()) {
        ...
        if (x < fRect.fLeft + fRadii[kUpperLeft_Corner].fX && y < fRect.fTop + fRadii[kUpperLeft_Corner].fY) {
            ...
            canonicalPt.set(x - (fRect.fLeft + fRadii[kUpperLeft_Corner].fX), y - (fRect.fTop + fRadii[kUpperLeft_Corner].fY));
            ...
            canonicalPt.set(x - (fRect.fLeft + fRadii[kLowerLeft_Corner].fX), y - (fRect.fBottom - fRadii[kLowerLeft_Corner].fY));
            SkASSERT(canonicalPt.fX < 0 && canonicalPt.fY > 0);
        } else if (x > fRect.fRight - fRadii[kUpperRight_Corner].fX && y < fRect.fTop + fRadii[kUpperRight_Corner].fY) {
            // UR corner
            index = kUpperRight_Corner;
            canonicalPt.set(x - (fRect.fRight - fRadii[kUpperRight_Corner].fX), y - (fRect.fTop + fRadii[kUpperRight_Corner].fY));
            SkASSERT(canonicalPt.fX > 0 && canonicalPt.fY < 0);
        } else if (x > fRect.fRight - fRadii[kLowerRight_Corner].fX && y > fRect.fBottom - fRadii[kLowerRight_Corner].fY) {
            // LR corner
            index = kLowerRight_Corner;
            canonicalPt.set(x - (fRect.fRight - fRadii[kLowerRight_Corner].fX), y - (fRect.fBottom - fRadii[kLowerRight_Corner].fY));
            SkASSERT(canonicalPt.fX > 0 && canonicalPt.fY > 0);
        } else {
...
bool SkRRectPriv::IsNearlySimpleCircular(const SkRRect& rr, SkScalar tolerance)
{
    ...
    return SkScalarNearlyEqual(simpleRadius, rr.fRadii[0].fY, tolerance) && SkScalarNearlyEqual(simpleRadius, rr.fRadii[1].fX, tolerance)
        && SkScalarNearlyEqual(simpleRadius, rr.fRadii[1].fY, tolerance) && SkScalarNearlyEqual(simpleRadius, rr.fRadii[2].fX, tolerance)
        && SkScalarNearlyEqual(simpleRadius, rr.fRadii[2].fY, tolerance) && SkScalarNearlyEqual(simpleRadius, rr.fRadii[3].fX, tolerance)
...
{
    return SkScalarNearlyEqual(rr.fRadii[0].fX, rr.fRadii[0].fY, tolerance) && SkScalarNearlyEqual(rr.fRadii[1].fX, rr.fRadii[1].fY, tolerance)
        && SkScalarNearlyEqual(rr.fRadii[2].fX, rr.fRadii[2].fY, tolerance) && SkScalarNearlyEqual(rr.fRadii[3].fX, rr.fRadii[3].fY, tolerance);
}
...
bool SkRRect::contains(const SkRect& rect) const
{
    ...
    // all the curves
    return this->checkCornerContainment(rect.fLeft, rect.fTop) && this->checkCornerContainment(rect.fRight, rect.fTop)
        && this->checkCornerContainment(rect.fRight, rect.fBottom) && this->checkCornerContainment(rect.fLeft, rect.fBottom);
}
...
bool SkRRect::transform(const SkMatrix& matrix, SkRRect* dst) const
{
    ...
    if (!matrix.isScaleTranslate()) {
        ...
        SkASSERT(matrix.getScaleX() == 0.f && matrix.getScaleY() == 0.f && matrix.getSkewX() != 0.f && matrix.getSkewY() != 0.f);
...
bool SkRRect::isValid() const
{
    ...
    case kOval_Type:
        ...
        for (int i = 0; i < 4; ++i) {
            if (!SkScalarNearlyEqual(fRadii[i].fX, SkRectPriv::HalfWidth(fRect)) || !SkScalarNearlyEqual(fRadii[i].fY, SkRectPriv::HalfHeight(fRect))) {
                return false;
            }
        }
        break;
...
SkRRect SkRRectPriv::ConservativeIntersect(const SkRRect& a, const SkRRect& b)
{
    ...
    const SkRRect::Corner corners[] = { SkRRect::kUpperLeft_Corner, SkRRect::kUpperRight_Corner, SkRRect::kLowerRight_Corner, SkRRect::kLowerLeft_Corner };
