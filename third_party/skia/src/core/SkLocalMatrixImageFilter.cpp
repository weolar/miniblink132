...
sk_sp<SkImageFilter> SkLocalMatrixImageFilter::Make(const SkMatrix& localMatrix, sk_sp<SkImageFilter> input)
{
    if (!input) {
        return nullptr;
    }
    ...
    const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const
{
    ...
    return this->getChildInputLayerBounds(0, this->localMapping(mapping), desiredOutput, contentBounds);
}
