...
public:
    ...
    void blitRect(int x, int y, int width, int height) override
    {
        ...
        fSrcPtr.pixels = (char*)fSource.writable_addr(-fLeft + x, -fTop + y) - bpp * x - bpp * y * fSrcPtr.stride;
...
SkBlitter* SkBlitter::ChooseSprite(
    const SkPixmap& dst, const SkPaint& paint, const SkPixmap& source, int left, int top, SkArenaAlloc* alloc, sk_sp<SkShader> clipShader)
{
