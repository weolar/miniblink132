...
{
    if (rect.isEmpty() || SkPaint::kMiter_Join != paint.getStrokeJoin() || paint.getStrokeMiter() < SK_ScalarSqrt2) {
        return false;
    }
...
void SkDrawBase::drawRect(const SkRect& prePaintRect, const SkPaint& paint, const SkMatrix* paintMatrix, const SkRect* postPaintRect) const
{
    SkDEBUGCODE(this->validate();)
    ...
    if (paint.getStyle() != SkPaint::kFill_Style) {
        ...
        if (paint.getStrokeWidth() == 0) {
            ...
            const SkPoint& ssize = (kStroke_RectType == rtype) ? strokeSize : compute_stroke_size(paint, *fCTM);
...
void SkDrawBase::drawDevPath(const SkPath& devPath, const SkPaint& paint, bool drawCoverage, SkBlitter* customBlitter, bool doFill) const
{
    ...
    if (paint.getMaskFilter()) {
        SkStrokeRec::InitStyle style = doFill ? SkStrokeRec::kFill_InitStyle : SkStrokeRec::kHairline_InitStyle;
...
void SkDrawBase::drawPath(
    const SkPath& origSrcPath, const SkPaint& origPaint, const SkMatrix* prePathMatrix, bool pathIsMutable, bool drawCoverage, SkBlitter* customBlitter) const
{
    SkDEBUGCODE(this->validate();)
...
void SkDrawBase::drawBitmap(const SkBitmap&, const SkMatrix&, const SkRect*, const SkSamplingOptions&, const SkPaint&) const
{
    SkASSERT(false);
}
...
bool SkDrawBase::ComputeMaskBounds(
    const SkRect& devPathBounds, const SkIRect& clipBounds, const SkMaskFilter* filter, const SkMatrix* filterMatrix, SkIRect* bounds)
{
    ...
    *bounds = devPathBounds.makeOutset(SK_ScalarHalf, SK_ScalarHalf).roundOut();
    ...
    if (!bounds->intersect(clipBounds.makeOutset(std::min(margin.fX, kMaxMargin), std::min(margin.fY, kMaxMargin)))) {
        return false;
    }
...
bool SkDrawBase::DrawToMask(const SkPath& devPath, const SkIRect& clipBounds, const SkMaskFilter* filter, const SkMatrix* filterMatrix, SkMaskBuilder* dst,
    SkMaskBuilder::CreateMode mode, SkStrokeRec::InitStyle style)
{
    if (devPath.isEmpty()) {
        return false;
    }
    ...
    if (SkMaskBuilder::kJustRenderImage_CreateMode != mode) {
        ...
        static const SkRect kInverseBounds = { SK_ScalarNegativeInfinity, SK_ScalarNegativeInfinity, SK_ScalarInfinity, SK_ScalarInfinity };
        SkRect pathBounds = devPath.isInverseFillType() ? kInverseBounds : devPath.getBounds();
        if (!ComputeMaskBounds(pathBounds, clipBounds, filter, filterMatrix, &dst->bounds()))
            return false;
    }
...
void SkDrawBase::drawDevicePoints(SkCanvas::PointMode mode, size_t count, const SkPoint pts[], const SkPaint& paint, SkDevice* device) const
{
    ...
    case SkCanvas::kLines_PointMode:
        if (2 == count && paint.getPathEffect()) {
            ...
            if (as_PEB(paint.getPathEffect())->asPoints(&pointData, path, stroke, *fCTM, &cullRect)) {
                ...
                if (pointData.fSize.fX == pointData.fSize.fY) {
                    ...
                    if (device) {
                        device->drawPoints(SkCanvas::kPoints_PointMode, pointData.fNumPoints, pointData.fPoints, newP);
                    } else {
                        this->drawDevicePoints(SkCanvas::kPoints_PointMode, pointData.fNumPoints, pointData.fPoints, newP, device);
                    }
                    break;
                } else {
                    ...
                    for (int i = 0; i < pointData.fNumPoints; ++i) {
                        r.setLTRB(pointData.fPoints[i].fX - pointData.fSize.fX, pointData.fPoints[i].fY - pointData.fSize.fY,
                            pointData.fPoints[i].fX + pointData.fSize.fX, pointData.fPoints[i].fY + pointData.fSize.fY);
                        if (device) {
