...
size_t SkTextBlob::RunRecord::StorageSize(uint32_t glyphCount, uint32_t textSize, SkTextBlob::GlyphPositioning positioning, SkSafeMath* safe)
{
    ...
    auto glyphSize = safe->mul(glyphCount, sizeof(uint16_t)), posSize = safe->mul(PosCount(glyphCount, positioning, safe), sizeof(SkScalar));
...
void SkTextBlob::RunRecord::validate(const uint8_t* storageTop) const
{
    ...
    SkASSERT(posBuffer() + fCount * ScalarsPerGlyph(positioning()) <= (const SkScalar*)NextUnchecked(this));
    if (isExtended()) {
        SkASSERT(textSize() > 0);
    ...
    static_assert(sizeof(SkTextBlob::RunRecord) == sizeof(RunRecordStorageEquivalent), "runrecord_should_stay_packed");
}
...
const SkTextBlob::RunRecord* SkTextBlob::RunRecord::NextUnchecked(const RunRecord* run)
{
    ...
    auto res = reinterpret_cast<const RunRecord*>(
        reinterpret_cast<const uint8_t*>(run) + StorageSize(run->glyphCount(), run->textSize(), run->positioning(), &safe));
    SkASSERT(safe);
    return res;
}

size_t SkTextBlob::RunRecord::PosCount(uint32_t glyphCount, SkTextBlob::GlyphPositioning positioning, SkSafeMath* safe)
{
...
void SkTextBlobRunIterator::next()
{
    ...
    if (!this->done()) {
        SkDEBUGCODE(fCurrentRun->validate(fStorageTop);) fCurrentRun = SkTextBlob::RunRecord::Next(fCurrentRun);
    }
}
...
bool SkTextBlobBuilder::mergeRun(const SkFont& font, SkTextBlob::GlyphPositioning positioning, uint32_t count, SkPoint offset)
{
    ...
    //     y-offset
    if (SkTextBlob::kFull_Positioning != positioning && (SkTextBlob::kHorizontal_Positioning != positioning || run->offset().y() != offset.y())) {
        return false;
    }

    SkSafeMath safe;
    size_t sizeDelta = SkTextBlob::RunRecord::StorageSize(run->glyphCount() + count, 0, positioning, &safe)
    ...
    fCurrentRunBuffer.pos = run->posBuffer() + preMergeCount * SkTextBlob::ScalarsPerGlyph(positioning);
...
void SkTextBlobBuilder::allocInternal(
    const SkFont& font, SkTextBlob::GlyphPositioning positioning, int count, int textSize, SkPoint offset, const SkRect* bounds)
{
    ...
    if (textSize != 0 || !this->mergeRun(font, positioning, count, offset)) {
        ...
        SkTextBlob::RunRecord* run = new (fStorage.get() + fStorageUsed) SkTextBlob::RunRecord(count, textSize, offset, font, positioning);
...
const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunText(
    const SkFont& font, int count, SkScalar x, SkScalar y, int textByteCount, const SkRect* bounds)
{
    this->allocInternal(font, SkTextBlob::kDefault_Positioning, count, textByteCount, SkPoint::Make(x, y), bounds);
    return fCurrentRunBuffer;
}

const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunTextPosH(const SkFont& font, int count, SkScalar y, int textByteCount, const SkRect* bounds)
{
    this->allocInternal(font, SkTextBlob::kHorizontal_Positioning, count, textByteCount, SkPoint::Make(0, y), bounds);
    return fCurrentRunBuffer;
}

const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunTextPos(const SkFont& font, int count, int textByteCount, const SkRect* bounds)
{
    this->allocInternal(font, SkTextBlob::kFull_Positioning, count, textByteCount, SkPoint::Make(0, 0), bounds);
    return fCurrentRunBuffer;
}

const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunTextRSXform(const SkFont& font, int count, int textByteCount, const SkRect* bounds)
{
...
sk_sp<SkTextBlob> SkTextBlobBuilder::make()
{
    ...
    SkDEBUGCODE(const_cast<SkTextBlob*>(blob)->fStorageSize = fStorageSize;)
        ...
        SkDEBUGCODE(SkSafeMath safe; size_t validateSize = SkAlignPtr(sizeof(SkTextBlob));
                    for (const auto* run = SkTextBlob::RunRecord::First(blob); run; run = SkTextBlob::RunRecord::Next(run)) {
                        validateSize += SkTextBlob::RunRecord::StorageSize(run->fCount, run->textSize(), run->positioning(), &safe);
...
void SkTextBlobPriv::Flatten(const SkTextBlob& blob, SkWriteBuffer& buffer)
{
    ...
    while (!it.done()) {
        ...
        buffer.writeByteArray(it.pos(), it.glyphCount() * sizeof(SkScalar) * SkTextBlob::ScalarsPerGlyph(SkTo<SkTextBlob::GlyphPositioning>(it.positioning())));
        if (pe.extended) {
...
sk_sp<SkTextBlob> SkTextBlobPriv::MakeFromBuffer(SkReadBuffer& reader)
{
    ...
    for (;;) {
        ...
        const size_t glyphSize = safe.mul(glyphCount, sizeof(uint16_t)),
                     posSize = safe.mul(glyphCount, safe.mul(sizeof(SkScalar), SkTextBlob::ScalarsPerGlyph(pos))),
                     clusterSize = pe.extended ? safe.mul(glyphCount, sizeof(uint32_t)) : 0;
        const size_t totalSize = safe.add(safe.add(glyphSize, posSize), safe.add(clusterSize, textSize));
        ...
        case SkTextBlob::kDefault_Positioning:
            buf = &blobBuilder.allocRunText(font, glyphCount, offset.x(), offset.y(), textSize, &bounds);
            break;
        case SkTextBlob::kHorizontal_Positioning:
...
sk_sp<SkTextBlob> SkTextBlob::MakeFromPosTextH(
    const void* text, size_t byteLength, const SkScalar xpos[], SkScalar constY, const SkFont& font, SkTextEncoding encoding)
{
    const int count = font.countText(text, byteLength, encoding);
    if (count < 1) {
        return nullptr;
...
namespace {
int get_glyph_run_intercepts(const sktext::GlyphRun& glyphRun, const SkPaint& paint, const SkScalar bounds[2], SkScalar intervals[], int* intervalCount)
{
...
std::vector<SkScalar> SkFont::getIntercepts(
    const SkGlyphID glyphs[], int count, const SkPoint positions[], SkScalar top, SkScalar bottom, const SkPaint* paintPtr) const
{
    if (count <= 0) {
    ...
    const sktext::GlyphRun run(*this, { positions, size_t(count) }, { glyphs, size_t(count) }, { nullptr, 0 }, { nullptr, 0 }, { nullptr, 0 });
