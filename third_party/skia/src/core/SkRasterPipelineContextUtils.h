...
template <typename T> using UnpackedType = typename std::conditional<sizeof(T) <= sizeof(void*), T, const T&>::type;
 ...
 */
template <typename T> [[maybe_unused]] static void* Pack(const T& ctx, SkArenaAlloc* alloc)
{
 ...
 */
template <typename T> [[maybe_unused]] static UnpackedType<T> Unpack(const T* ctx)
{
