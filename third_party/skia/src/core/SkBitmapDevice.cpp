...
struct Bounder {
    ...
    {
        return fHasBounds ? &fBounds : nullptr;
    }
...
static bool valid_for_bitmap_device(const SkImageInfo& info, SkAlphaType* newAlphaType)
{
    ...
    if (newAlphaType) {
        *newAlphaType = SkColorTypeIsAlwaysOpaque(info.colorType()) ? kOpaque_SkAlphaType : info.alphaType();
    }
...
SkBitmapDevice::SkBitmapDevice(const SkBitmap& bitmap, const SkSurfaceProps& surfaceProps, SkRasterHandleAllocator::Handle hndl)
...
sk_sp<SkBitmapDevice> SkBitmapDevice::Create(const SkImageInfo& origInfo, const SkSurfaceProps& surfaceProps, SkRasterHandleAllocator* allocator)
{
...
void SkBitmapDevice::drawBitmap(
    const SkBitmap& bitmap, const SkMatrix& matrix, const SkRect* dstOrNull, const SkSamplingOptions& sampling, const SkPaint& paint)
{
    ...
    const SkImage* image, const SkRect* src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    if (srcIsSubset && SkCanvas::kFast_SrcRectConstraint == constraint && sampling != SkSamplingOptions()) {
...
USE_SHADER:
    ...
    auto s = SkMakeBitmapShaderForPaint(paint, *bitmapPtr, SkTileMode::kClamp, SkTileMode::kClamp, sampling, &matrix, kNever_SkCopyPixelsMode);
    if (!s) {
        return;
    }
...
void SkBitmapDevice::onDrawGlyphRunList(SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
    SkASSERT(!glyphRunList.hasRSXForm());
    LOOP_TILER(drawGlyphRunList(canvas, &fGlyphPainter, glyphRunList, paint), nullptr)
}

void SkBitmapDevice::drawVertices(const SkVertices* vertices, sk_sp<SkBlender> blender, const SkPaint& paint, bool skipColorXform)
{
...
void SkBitmapDevice::drawSpecial(
    SkSpecialImage* src, const SkMatrix& localToDevice, const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint)
{
...
{
    return SkSpecialImages::MakeFromRaster(SkIRect::MakeWH(image->width(), image->height()), image->makeNonTextureImage(), this->surfaceProps());
}
