...
{
    return sizeof(SkPathRef) + fPoints.capacity() * sizeof(fPoints[0]) + fVerbs.capacity() * sizeof(fVerbs[0])
...
void SkPathRef::CreateTransformedCopy(sk_sp<SkPathRef>* dst, const SkPathRef& src, const SkMatrix& matrix)
{
    ...
    const PathType newType = (rectStaysRect && src.fType != PathType::kArc) ? src.fType : PathType::kGeneral;
    (*dst)->fType = newType;
...
void SkPathRef::copy(const SkPathRef& ref, int additionalReserveVerbs, int additionalReservePoints, int additionalReserveConics)
{
    SkDEBUGCODE(this->validate();) this->resetToSize(
        ref.fVerbs.size(), ref.fPoints.size(), ref.fConicWeights.size(), additionalReserveVerbs, additionalReservePoints, additionalReserveConics);
    fVerbs = ref.fVerbs;
    fPoints = ref.fPoints;
...
SkRRect SkPathRef::getRRect() const
{
    ...
    while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
        if (SkPath::kConic_Verb == verb) {
            ...
            SkRRect::Corner corner = pts[1].fX == bounds.fLeft ? pts[1].fY == bounds.fTop ? SkRRect::kUpperLeft_Corner : SkRRect::kLowerLeft_Corner
                : pts[1].fY == bounds.fTop                     ? SkRRect::kUpperRight_Corner
                                                               : SkRRect::kLowerRight_Corner;
            SkASSERT(!radii[corner].fX && !radii[corner].fY);
            radii[corner] = dxdy;
        } else {
            SkASSERT((verb == SkPath::kLine_Verb && (!(pts[1].fX - pts[0].fX) || !(pts[1].fY - pts[0].fY))) || verb == SkPath::kClose_Verb);
        }
    }
    SkRRect rrect;
...
bool SkPathRef::dataMatchesVerbs() const
{
    ...
    return info.valid && info.segmentMask == fSegmentMask && info.points == fPoints.size() && info.weights == fConicWeights.size();
}
