...
SkYUVAPixmapInfo::SkYUVAPixmapInfo(const SkYUVAInfo& yuvaInfo, DataType dataType, const size_t rowBytes[kMaxPlanes])
{
...
bool SkYUVAPixmapInfo::initPixmapsFromSingleAllocation(void* memory, SkPixmap pixmaps[kMaxPlanes]) const
{
    if (!this->isValid()) {
        return false;
    }
...
SkYUVAPixmaps SkYUVAPixmaps::Allocate(const SkYUVAPixmapInfo& yuvaPixmapInfo)
{
    ...
    return SkYUVAPixmaps(yuvaPixmapInfo, SkData::MakeUninitialized(yuvaPixmapInfo.computeTotalBytes()));
}
...
SkYUVAPixmaps SkYUVAPixmaps::MakeCopy(const SkYUVAPixmaps& src)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        SkRectMemcpy(d.writable_addr(), d.rowBytes(), s.addr(), s.rowBytes(), s.info().minRowBytes(), s.height());
    }
    return result;
}

SkYUVAPixmaps SkYUVAPixmaps::FromExternalMemory(const SkYUVAPixmapInfo& yuvaPixmapInfo, void* memory)
{
    if (!yuvaPixmapInfo.isValid()) {
        return {};
    }
...
SkYUVAPixmaps SkYUVAPixmaps::FromExternalPixmaps(const SkYUVAInfo& yuvaInfo, const SkPixmap pixmaps[kMaxPlanes])
{
    SkColorType colorTypes[kMaxPlanes] = {};
    ...
    , fDataType(yuvaPixmapInfo.dataType())
{
    ...
    SkAssertResult(yuvaPixmapInfo.initPixmapsFromSingleAllocation(fData->writable_data(), fPlanes.data()));
}

SkYUVAPixmaps::SkYUVAPixmaps(const SkYUVAInfo& yuvaInfo, DataType dataType, const SkPixmap pixmaps[kMaxPlanes])
    : fYUVAInfo(yuvaInfo)
    , fDataType(dataType)
{
...
{
    uint32_t channelFlags[] = { SkColorTypeChannelFlags(fPlanes[0].colorType()), SkColorTypeChannelFlags(fPlanes[1].colorType()),
        SkColorTypeChannelFlags(fPlanes[2].colorType()), SkColorTypeChannelFlags(fPlanes[3].colorType()) };
    auto result = fYUVAInfo.toYUVALocations(channelFlags);
    SkDEBUGCODE(int numPlanes;) SkASSERT(SkYUVAInfo::YUVALocation::AreValidLocations(result, &numPlanes));
