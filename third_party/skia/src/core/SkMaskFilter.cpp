...
static SkMask extractMaskSubset(const SkMask& src, SkIRect bounds, int32_t newX, int32_t newY)
{
    ...
    return SkMask(src.fImage + dy * src.fRowBytes + dx, bounds, src.fRowBytes, src.fFormat);
}
...
static void draw_nine_clipped(const SkMask& mask, const SkIRect& outerR, const SkIPoint& center, bool fillCenter, const SkIRect& clipR, SkBlitter* blitter)
{
    ...
    bounds.fTop = cy + 1;
    if (bounds.width() > 0 && bounds.height() > 0) {
        SkMask m = extractMaskSubset(mask, bounds, outerR.right() - bounds.width(), outerR.bottom() - bounds.height());
        blitClippedMask(blitter, m, m.fBounds, clipR);
    }

    SkIRect innerR;
    innerR.setLTRB(outerR.left() + cx - mask.fBounds.left(), outerR.top() + cy - mask.fBounds.top(), outerR.right() + (cx + 1 - mask.fBounds.right()),
    ...
    if (r.intersect(clipR)) {
        SkMask leftMask(mask.getAddr8(mask.fBounds.left() + r.left() - outerR.left(), mask.fBounds.top() + cy), r,
    ...
    if (r.intersect(clipR)) {
        SkMask rightMask(mask.getAddr8(mask.fBounds.right() - outerR.right() + r.left(), mask.fBounds.top() + cy), r,
            0, // so we repeat the scanline for our height
            SkMask::kA8_Format);
...
bool SkMaskFilterBase::filterPath(
    const SkPath& devPath, const SkMatrix& matrix, const SkRasterClip& clip, SkBlitter* blitter, SkStrokeRec::InitStyle style) const
{
    SkRect rects[2];
    int rectCount = 0;
...
#endif
    if (!SkDraw::DrawToMask(devPath, clip.getBounds(), this, &matrix, &srcM, SkMaskBuilder::kComputeBoundsAndRenderImage_CreateMode, style)) {
        return false;
    }
...
SkMaskFilterBase::FilterReturn SkMaskFilterBase::filterRRectToNine(const SkRRect&, const SkMatrix&, const SkIRect& clipBounds, SkTLazy<NinePatch>*) const
{
    return kUnimplemented_FilterReturn;
}

SkMaskFilterBase::FilterReturn SkMaskFilterBase::filterRectsToNine(
    const SkRect[], int count, const SkMatrix&, const SkIRect& clipBounds, SkTLazy<NinePatch>*) const
{
    return kUnimplemented_FilterReturn;
}
...
sk_sp<SkMaskFilter> SkMaskFilter::Deserialize(const void* data, size_t size, const SkDeserialProcs* procs)
{
