...
public:
    ...
    template <typename T> std::enable_if_t<(T::kTags & kDrawWithPaint_Tag) == kDrawWithPaint_Tag, bool> operator()(T* draw)
    {
...
public:
    ...
    template <typename T> std::enable_if_t<(T::kTags & kDrawWithPaint_Tag) == kDrawWithPaint_Tag && !(T::kTags & kMultiDraw_Tag), bool> operator()(T* draw)
    {
        fPaint = AsPtr(draw->paint);
        return true;
    }

    template <typename T> std::enable_if_t<(T::kTags & kDrawWithPaint_Tag) == kDraw_Tag && !(T::kTags & kMultiDraw_Tag), bool> operator()(T* draw)
    {
        fPaint = nullptr;
        return true;
    }

    template <typename T> std::enable_if_t<!(T::kTags & kDraw_Tag) || (T::kTags & kMultiDraw_Tag), bool> operator()(T* draw)
    {
        fPaint = nullptr;
        return false;
    }
