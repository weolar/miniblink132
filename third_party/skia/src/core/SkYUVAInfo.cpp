...
static bool is_plane_config_compatible_with_subsampling(SkYUVAInfo::PlaneConfig config, SkYUVAInfo::Subsampling subsampling)
{
    ...
    return subsampling == SkYUVAInfo::Subsampling::k444
        || (config != SkYUVAInfo::PlaneConfig::kYUV && config != SkYUVAInfo::PlaneConfig::kYUVA && config != SkYUVAInfo::PlaneConfig::kUYV
...
{
    if (!is_plane_config_compatible_with_subsampling(planeConfig, subsampling) || planeIdx < 0 || planeIdx > NumPlanes(planeConfig)) {
        return { 0, 0 };
    }
...
int SkYUVAInfo::PlaneDimensions(
    SkISize imageDimensions, PlaneConfig planeConfig, Subsampling subsampling, SkEncodedOrigin origin, SkISize planeDimensions[SkYUVAInfo::kMaxPlanes])
{
...
SkYUVAInfo::SkYUVAInfo(
    SkISize dimensions, PlaneConfig planeConfig, Subsampling subsampling, SkYUVColorSpace yuvColorSpace, SkEncodedOrigin origin, Siting sitingX, Siting sitingY)
...
size_t SkYUVAInfo::computeTotalBytes(const size_t rowBytes[kMaxPlanes], size_t planeSizes[kMaxPlanes]) const
{
    if (!this->isValid()) {
        return 0;
    }
...
{
    return fPlaneConfig == that.fPlaneConfig && fSubsampling == that.fSubsampling && fYUVColorSpace == that.fYUVColorSpace && fDimensions == that.fDimensions
