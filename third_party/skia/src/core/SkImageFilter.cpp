...
bool SkImageFilter_Base::Common::unflatten(SkReadBuffer& buffer, int expectedCount)
{
    ...
    if (buffer.isVersionLT(SkPicturePriv::kRemoveDeprecatedCropRect)) {
        ...
        if (!buffer.isValid() || !buffer.validate(flags == 0x0 || flags == kHasAll_CropEdge)) {
            return false;
        }
...
skif::FilterResult SkImageFilter_Base::filterImage(const skif::Context& context) const
{
    ...
    SkImageFilterCacheKey key(fUniqueID, context.mapping().layerMatrix(), SkIRect(context.desiredOutput()), srcGenID, srcSubset);
...
sk_sp<SkImage> SkImageFilter_Base::makeImageWithFilter(
    sk_sp<skif::Backend> backend, sk_sp<SkImage> src, const SkIRect& subset, const SkIRect& clipBounds, SkIRect* outSubset, SkIPoint* offset) const
{
    ...
    skif::Stats stats;
    const skif::Context context { std::move(backend), skif::Mapping(SkMatrix::I()), skif::LayerSpace<SkIRect>(clipBounds),
        skif::FilterResult(std::move(srcSpecialImage), skif::LayerSpace<SkIPoint>(subset.topLeft())), src->imageInfo().colorSpace(), &stats };
    ...
    SkASSERT(clipBounds.contains(SkIRect::MakeXYWH(offset->fX, offset->fY, result->width(), result->height())));
    *outSubset = result->subset();
    return result->asImage();
}

skif::LayerSpace<SkIRect> SkImageFilter_Base::getInputBounds(
    const skif::Mapping& mapping, const skif::DeviceSpace<SkIRect>& desiredOutput, std::optional<skif::ParameterSpace<SkRect>> knownContentBounds) const
{
    ...
    const skif::Mapping& mapping, const skif::ParameterSpace<SkRect>& contentBounds) const
{
    ...
    std::optional<skif::LayerSpace<SkIRect>> filterOutput = this->onGetOutputLayerBounds(mapping, layerContent.roundOut());
    if (filterOutput) {
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::getChildInputLayerBounds(
    int index, const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const
{
...
std::optional<skif::LayerSpace<SkIRect>> SkImageFilter_Base::getChildOutputLayerBounds(
    int index, const skif::Mapping& mapping, std::optional<skif::LayerSpace<SkIRect>> contentBounds) const
{
    ...
    const SkImageFilter* childFilter = this->getInput(index);
    return childFilter ? as_IFB(childFilter)->onGetOutputLayerBounds(mapping, contentBounds) : contentBounds;
}
