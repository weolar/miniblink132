...
{
    return SkImages::DeferredFromGenerator(std::make_unique<EmptyImageGenerator>(SkImageInfo::MakeN32Premul(width, height)));
}
...
static sk_sp<SkImage> add_mipmaps(sk_sp<SkImage> img, sk_sp<SkData> data, SkDeserialProcs dProcs, std::optional<SkAlphaType> alphaType)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        sk_sp<SkImage> mip = deserialize_image(SkData::MakeWithoutCopy(ptr, size), dProcs, alphaType);
        if (!mip) {
            return img;
        }
