...
PERFETTO_DEFINE_CATEGORIES(perfetto::Category("GM"), perfetto::Category("skia"), perfetto::Category("skia.android"), perfetto::Category("skia.gpu"),
    perfetto::Category("skia.gpu.cache"), perfetto::Category("skia.objects"), perfetto::Category("skia.shaders"), perfetto::Category("skottie"),
    perfetto::Category("test"), perfetto::Category("test_cpu"), perfetto::Category("test_ganesh"), perfetto::Category("test_graphite"),
    // ".always" variants are currently required for any category used in TRACE_EVENTx_ALWAYS.
    perfetto::Category("GM.always").SetTags("skia.always"), perfetto::Category("skia.always").SetTags("skia.always"),
    perfetto::Category("skia.android.always").SetTags("skia.always"), perfetto::Category("skia.gpu.always").SetTags("skia.always"),
    perfetto::Category("skia.gpu.cache.always").SetTags("skia.always"), perfetto::Category("skia.objects.always").SetTags("skia.always"),
    perfetto::Category("skia.shaders.always").SetTags("skia.always"), perfetto::Category("skottie.always").SetTags("skia.always"),
    perfetto::Category("test.always").SetTags("skia.always"), perfetto::Category("test_cpu.always").SetTags("skia.always"),
    perfetto::Category("test_ganesh.always").SetTags("skia.always"), perfetto::Category("test_graphite.always").SetTags("skia.always"), );
...
template <typename... Args> static inline void sk_noop(Args...)
{
}
#define TRACE_EMPTY(...)                                                                                                                                       \
    do {                                                                                                                                                       \
...
#if defined(SK_DISABLE_TRACING) || (defined(SK_BUILD_FOR_ANDROID_FRAMEWORK) && !defined(SK_ANDROID_FRAMEWORK_USE_PERFETTO))
...
#define TRACE_EVENT_INSTANT2(cg, n, scope, a1n, a1v, a2n, a2v) TRACE_EMPTY(cg, n, scope, a1n, a1v, a2n, a2v)
...
template <typename T> inline std::string WrapTraceArgInStdString(const T numeric)
{
...
constexpr bool StrEndsWithAndLongerThan(const char* str, const char* suffix)
{
    ...
    return strView.size() > suffixView.size() && strView.compare(strView.size() - suffixView.size(), std::string_view::npos, suffixView) == 0;
}
}
...
#define SK_INTERNAL_ATRACE_ARGS_BEGIN_DANGEROUS_0(name) atrace_begin_body(::skia_private::UnboxPerfettoString(name));
...
#define SK_INTERNAL_ATRACE_ARGS_BEGIN_DANGEROUS_1(name, arg1_name, arg1_val)                                                                                   \
    ...
    snprintf(SK_PERFETTO_UID(skTraceStrBuf1), SK_ANDROID_FRAMEWORK_ATRACE_BUFFER_SIZE, "^(%s: %s)", ::skia_private::UnboxPerfettoString(arg1_name),            \
...
// WARNING: must always be guarded by an outer call to CC_UNLIKELY(ATRACE_ENABLED())
#define SK_INTERNAL_ATRACE_ARGS_BEGIN_DANGEROUS_2(name, arg1_name, arg1_val, arg2_name, arg2_val, ...)                                                         \
    char SK_PERFETTO_UID(skTraceStrBuf1)[SK_ANDROID_FRAMEWORK_ATRACE_BUFFER_SIZE];                                                                             \
    char SK_PERFETTO_UID(skTraceStrBuf2)[SK_ANDROID_FRAMEWORK_ATRACE_BUFFER_SIZE];                                                                             \
    snprintf(SK_PERFETTO_UID(skTraceStrBuf1), SK_ANDROID_FRAMEWORK_ATRACE_BUFFER_SIZE, "^(%s: %s)", ::skia_private::UnboxPerfettoString(arg1_name),            \
        ::skia_private::WrapTraceArgInStdString(arg1_val).c_str());                                                                                            \
    snprintf(SK_PERFETTO_UID(skTraceStrBuf2), SK_ANDROID_FRAMEWORK_ATRACE_BUFFER_SIZE, "^(%s: %s)", ::skia_private::UnboxPerfettoString(arg2_name),            \
...
#define SK_INTERNAL_ATRACE_ARGS_BEGIN(slice_name, ...)                                                                                                         \
    if (CC_UNLIKELY(ATRACE_ENABLED())) {                                                                                                                       \
        SK_INTERNAL_GET_ATRACE_ARGS_MACRO(0, ##__VA_ARGS__, SK_INTERNAL_ATRACE_ARGS_BEGIN_DANGEROUS_2, 0, SK_INTERNAL_ATRACE_ARGS_BEGIN_DANGEROUS_1, 0,        \
...
// WARNING: must always be guarded by an outer call to CC_UNLIKELY(ATRACE_ENABLED())
#define SK_INTERNAL_ATRACE_ARGS_END_DANGEROUS_0() atrace_end_body();
...
#define SK_INTERNAL_ATRACE_ARGS_END(...)                                                                                                                       \
    if (CC_UNLIKELY(ATRACE_ENABLED())) {                                                                                                                       \
        SK_INTERNAL_GET_ATRACE_ARGS_MACRO(                                                                                                                     \
            0, ##__VA_ARGS__, SK_INTERNAL_ATRACE_ARGS_END_DANGEROUS_2, 0, SK_INTERNAL_ATRACE_ARGS_END_DANGEROUS_1, 0, SK_INTERNAL_ATRACE_ARGS_END_DANGEROUS_0) \
        (__VA_ARGS__);                                                                                                                                         \
    }
...
#define TRACE_EVENT_ATRACE_OR_PERFETTO_FORCEABLE(force_always_trace, category, name, ...)                                                                      \
    struct SK_PERFETTO_UID(ScopedEvent) {                                                                                                                      \
        struct EventFinalizer {                                                                                                                                \
            ...
            ~EventFinalizer()                                                                                                                                  \
            {                                                                                                                                                  \
                if (force_always_trace || CC_UNLIKELY(SkAndroidFrameworkTraceUtil::getEnableTracing())) {                                                      \
    ...
    {                                                                                                                                                          \
        [&]() {                                                                                                                                                \
            static_assert(!force_always_trace || ::skia_private::StrEndsWithAndLongerThan(category, ".always"),                                                \
...
#define TRACE_EVENT_ATRACE_OR_PERFETTO(category, name, ...)                                                                                                    \
    TRACE_EVENT_ATRACE_OR_PERFETTO_FORCEABLE(/* force_always_trace = */ false, category, name, ##__VA_ARGS__)
...
#define ATRACE_ANDROID_FRAMEWORK(fmt, ...)                                                                                                                     \
    ...
    if (SkAndroidFrameworkTraceUtil::getEnableTracing()) {                                                                                                     \
        snprintf(SK_PERFETTO_UID(skTraceStrBuf), SK_ANDROID_FRAMEWORK_ATRACE_BUFFER_SIZE, fmt, ##__VA_ARGS__);                                                 \
    }                                                                                                                                                          \
...
#define TRACE_EVENT0(category_group, name) TRACE_EVENT_ATRACE_OR_PERFETTO(category_group, name)
#define TRACE_EVENT1(category_group, name, arg1_name, arg1_val) TRACE_EVENT_ATRACE_OR_PERFETTO(category_group, name, arg1_name, arg1_val)
...
#define TRACE_EVENT0_ALWAYS(category_group, name) TRACE_EVENT_ATRACE_OR_PERFETTO_FORCEABLE(/* force_always_trace = */ true, category_group ".always", name)
#define TRACE_EVENT1_ALWAYS(category_group, name, arg1_name, arg1_val)                                                                                         \
    TRACE_EVENT_ATRACE_OR_PERFETTO_FORCEABLE(/* force_always_trace = */ true, category_group ".always", name, arg1_name, arg1_val)
#define TRACE_EVENT2_ALWAYS(category_group, name, arg1_name, arg1_val, arg2_name, arg2_val)                                                                    \
    TRACE_EVENT_ATRACE_OR_PERFETTO_FORCEABLE(/* force_always_trace = */ true, category_group ".always", name, arg1_name, arg1_val, arg2_name, arg2_val)
...
#define TRACE_EVENT_OBJECT_CREATED_WITH_ID(category_group, name, id) TRACE_EMPTY(category_group, name, id)
#define TRACE_EVENT_OBJECT_SNAPSHOT_WITH_ID(category_group, name, id, snapshot) TRACE_EMPTY(category_group, name, id, snapshot)
#define TRACE_EVENT_OBJECT_DELETED_WITH_ID(category_group, name, id) TRACE_EMPTY(category_group, name, id)
...
#define TRACE_EVENT_CATEGORY_GROUP_ENABLED(category_group, ret)                                                                                                \
    if (CC_UNLIKELY(SkAndroidFrameworkTraceUtil::getEnableTracing() && SkAndroidFrameworkTraceUtil::getUsePerfettoTrackEvents)) {                              \
...
#define TRACE_EVENT_ASYNC_BEGIN0(category, name, id) INTERNAL_TRACE_EVENT_ADD_WITH_ID(TRACE_EVENT_PHASE_ASYNC_BEGIN, category, name, id, TRACE_EVENT_FLAG_NONE)
#define TRACE_EVENT_ASYNC_BEGIN1(category, name, id, arg1_name, arg1_val)                                                                                      \
    INTERNAL_TRACE_EVENT_ADD_WITH_ID(TRACE_EVENT_PHASE_ASYNC_BEGIN, category, name, id, TRACE_EVENT_FLAG_NONE, arg1_name, arg1_val)
#define TRACE_EVENT_ASYNC_BEGIN2(category, name, id, arg1_name, arg1_val, arg2_name, arg2_val)                                                                 \
    INTERNAL_TRACE_EVENT_ADD_WITH_ID(TRACE_EVENT_PHASE_ASYNC_BEGIN, category, name, id, TRACE_EVENT_FLAG_NONE, arg1_name, arg1_val, arg2_name, arg2_val)

#define TRACE_EVENT_ASYNC_END0(category, name, id) INTERNAL_TRACE_EVENT_ADD_WITH_ID(TRACE_EVENT_PHASE_ASYNC_END, category, name, id, TRACE_EVENT_FLAG_NONE)
#define TRACE_EVENT_ASYNC_END1(category, name, id, arg1_name, arg1_val)                                                                                        \
    INTERNAL_TRACE_EVENT_ADD_WITH_ID(TRACE_EVENT_PHASE_ASYNC_END, category, name, id, TRACE_EVENT_FLAG_NONE, arg1_name, arg1_val)
#define TRACE_EVENT_ASYNC_END2(category, name, id, arg1_name, arg1_val, arg2_name, arg2_val)                                                                   \
    INTERNAL_TRACE_EVENT_ADD_WITH_ID(TRACE_EVENT_PHASE_ASYNC_END, category, name, id, TRACE_EVENT_FLAG_NONE, arg1_name, arg1_val, arg2_name, arg2_val)
