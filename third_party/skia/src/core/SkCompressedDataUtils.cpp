        ...
        for (int x = 0; x < numXBlocks; ++x) {
            ...
            for (int i = 0; i < 4; ++i, ++baseShift) {
                for (int j = 0; j < 4; ++j) {
                    ...
                    *dst->getAddr32(offsetX + j, offsetY + i) = add_delta_and_clamp(colors[subBlockIndex], delta);
                }
            }
        }
    }
    ...
    for (int y = 0; y < numYBlocks; ++y) {
        for (int x = 0; x < numXBlocks; ++x) {
            ...
            if (curBlock->fColor0 <= curBlock->fColor1) { // signal for a transparent block
                colors[2] = SkPackARGB32(0xFF, (SkGetPackedR32(colors[0]) + SkGetPackedR32(colors[1])) >> 1,
                    (SkGetPackedG32(colors[0]) + SkGetPackedG32(colors[1])) >> 1, (SkGetPackedB32(colors[0]) + SkGetPackedB32(colors[1])) >> 1);
