...
template <typename Mode> void A8_row_aa(uint8_t dst[], uint8_t src, int N, uint8_t aa, Mode proc, const bool canFoldAA)
{
    if (canFoldAA) {
        src = div255(src * aa);
        for (int i = 0; i < N; ++i) {
...
#define WRAP_BLIT(proc, canFoldAA)                                                                                                                             \
    proc, [](uint8_t dst[], uint8_t src, int N) { A8_row_bw(dst, src, N, proc); },                                                                             \
        [](uint8_t dst[], uint8_t src, int N, uint8_t aa) { A8_row_aa(dst, src, N, aa, proc, canFoldAA); }
...
SkBlitter* SkA8Blitter_Choose(
    const SkPixmap& dst, const SkMatrix& ctm, const SkPaint& paint, SkArenaAlloc* alloc, bool drawCoverage, sk_sp<SkShader> clipShader, const SkSurfaceProps&)
{
