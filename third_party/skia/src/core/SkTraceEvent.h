...
#define INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()                                                                                       \
    *INTERNAL_TRACE_EVENT_UID(category_group_enabled)                                                                                                          \
        & (SkEventTracer::kEnabledForRecording_CategoryGroupEnabledFlags | SkEventTracer::kEnabledForEventCallback_CategoryGroupEnabledFlags)
...
#define INTERNAL_TRACE_EVENT_GET_CATEGORY_INFO(category_group)                                                                                                 \
    ...
    INTERNAL_TRACE_EVENT_GET_CATEGORY_INFO_CUSTOM_VARIABLES(                                                                                                   \
        TRACE_CATEGORY_PREFIX category_group, INTERNAL_TRACE_EVENT_UID(atomic), INTERNAL_TRACE_EVENT_UID(category_group_enabled));
...
#define INTERNAL_TRACE_EVENT_ADD(phase, category_group, name, flags, ...)                                                                                      \
    do {                                                                                                                                                       \
        ...
        if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {                                                                                \
            skia_private::AddTraceEvent(phase, INTERNAL_TRACE_EVENT_UID(category_group_enabled), name, skia_private::kNoEventId, flags, ##__VA_ARGS__);        \
        }                                                                                                                                                      \
    } while (0)
    ...
    do {                                                                                                                                                       \
        ...
        if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {                                                                                \
            ...
            skia_private::AddTraceEvent(                                                                                                                       \
                phase, INTERNAL_TRACE_EVENT_UID(category_group_enabled), name, trace_event_trace_id.data(), trace_event_flags, ##__VA_ARGS__);                 \
        }                                                                                                                                                      \
    } while (0)
...
#define INTERNAL_TRACE_EVENT_ADD_SCOPED(category_group, name, ...)                                                                                             \
    ...
    do {                                                                                                                                                       \
        if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {                                                                                \
            SkEventTracer::Handle h = skia_private::AddTraceEvent(TRACE_EVENT_PHASE_COMPLETE, INTERNAL_TRACE_EVENT_UID(category_group_enabled), name,          \
                skia_private::kNoEventId, TRACE_EVENT_FLAG_NONE, ##__VA_ARGS__);                                                                               \
...
static inline SkEventTracer::Handle AddTraceEvent(char phase, const uint8_t* category_group_enabled, const char* name, uint64_t id, unsigned char flags)
{
...
static inline SkEventTracer::Handle AddTraceEvent(
    char phase, const uint8_t* category_group_enabled, const char* name, uint64_t id, unsigned char flags, const char* arg1_name, const ARG1_TYPE& arg1_val)
{
...
static inline SkEventTracer::Handle AddTraceEvent(char phase, const uint8_t* category_group_enabled, const char* name, uint64_t id, unsigned char flags,
    const char* arg1_name, const ARG1_TYPE& arg1_val, const char* arg2_name, const ARG2_TYPE& arg2_val)
{
