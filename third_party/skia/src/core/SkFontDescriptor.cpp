...
[[nodiscard]] static bool read_string(SkStream* stream, SkString* string)
{
    ...
    if (length > 0) {
        ...
        if (stream->read(string->data(), length) != length) {
            return false;
        }
    }
    return true;
...
static constexpr SkScalar width_for_usWidth[0x10] = { 50, 50, 62.5, 75, 87.5, 100, 112.5, 125, 150, 200, 200, 200, 200, 200, 200, 200 };
...
bool SkFontDescriptor::Deserialize(SkStream* stream, SkFontDescriptor* result)
{
    ...
    for (size_t id; (id = read_id(stream)) != kSentinel;) {
        ...
        case kPaletteEntryOverrides:
            ...
            result->fPaletteEntryOverrideCount = SkTo<PaletteEntryOverrideCountType>(paletteEntryOverrideCount);
            ...
            for (size_t i = 0; i < paletteEntryOverrideCount; ++i) {
                ...
                result->fPaletteEntryOverrides[i].index = SkTo<PaletteEntryOverrideIndexType>(paletteEntryOverrideIndex);
            ...
            break;
        case kFactoryId:
            if (!stream->readPackedUInt(&factoryId)) {
                return false;
            }
            if (!SkTFitsIn<FactoryIdType>(factoryId)) {
                return false;
            }
