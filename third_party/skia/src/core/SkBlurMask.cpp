...
template <typename AlphaIter> static void clamp_solid_with_orig(uint8_t dst[], int dstRowBytes, AlphaIter src, int srcRowBytes, int sw, int sh)
{
    int x;
    while (--sh >= 0) {
        AlphaIter rowSrc(src);
...
template <typename AlphaIter> static void clamp_outer_with_orig(uint8_t dst[], int dstRowBytes, AlphaIter src, int srcRowBytes, int sw, int sh)
{
    int x;
    while (--sh >= 0) {
        AlphaIter rowSrc(src);
...
bool SkBlurMask::BoxBlur(SkMaskBuilder* dst, const SkMask& src, SkScalar sigma, SkBlurStyle style, SkIPoint* margin)
{
    if (src.fFormat != SkMask::kBW_Format && src.fFormat != SkMask::kA8_Format && src.fFormat != SkMask::kARGB32_Format
    ...
    case kInner_SkBlurStyle: {
        ...
        *dst = SkMaskBuilder(nullptr, src.fBounds, src.fBounds.width(), blur.format());
        ...
        switch (src.fFormat) {
        case SkMask::kBW_Format:
            merge_src_with_blur(dst->image(), dst->fRowBytes, SkMask::AlphaIter<SkMask::kBW_Format>(src.fImage, 0), src.fRowBytes, blurStart, blur.fRowBytes,
                src.fBounds.width(), src.fBounds.height());
            break;
        case SkMask::kA8_Format:
            merge_src_with_blur(dst->image(), dst->fRowBytes, SkMask::AlphaIter<SkMask::kA8_Format>(src.fImage), src.fRowBytes, blurStart, blur.fRowBytes,
                src.fBounds.width(), src.fBounds.height());
            break;
        case SkMask::kARGB32_Format: {
            const uint32_t* srcARGB = reinterpret_cast<const uint32_t*>(src.fImage);
            merge_src_with_blur(dst->image(), dst->fRowBytes, SkMask::AlphaIter<SkMask::kARGB32_Format>(srcARGB), src.fRowBytes, blurStart, blur.fRowBytes,
                src.fBounds.width(), src.fBounds.height());
        } break;
        case SkMask::kLCD16_Format: {
            const uint16_t* srcLCD = reinterpret_cast<const uint16_t*>(src.fImage);
            merge_src_with_blur(dst->image(), dst->fRowBytes, SkMask::AlphaIter<SkMask::kLCD16_Format>(srcLCD), src.fRowBytes, blurStart, blur.fRowBytes,
                src.fBounds.width(), src.fBounds.height());
        } break;
        default:
            SK_ABORT("Unhandled format.");
...
bool SkBlurMask::BlurRect(SkScalar sigma, SkMaskBuilder* dst, const SkRect& src, SkBlurStyle style, SkIPoint* margin, SkMaskBuilder::CreateMode createMode)
{
    ...
    dst->bounds().setLTRB(
        SkScalarRoundToInt(src.fLeft - pad), SkScalarRoundToInt(src.fTop - pad), SkScalarRoundToInt(src.fRight + pad), SkScalarRoundToInt(src.fBottom + pad));
...
bool SkBlurMask::BlurRRect(SkScalar sigma, SkMaskBuilder* dst, const SkRRect& src, SkBlurStyle style, SkIPoint* margin, SkMaskBuilder::CreateMode createMode)
{
...
bool SkBlurMask::BlurGroundTruth(SkScalar sigma, SkMaskBuilder* dst, const SkMask& src, SkBlurStyle style, SkIPoint* margin)
{
    ...
    if (src.fImage) {
        ...
        case kInner_SkBlurStyle: {
            ...
            dst->image() = SkMaskBuilder::AllocImage(srcSize);
            merge_src_with_blur(dst->image(), src.fRowBytes, SkMask::AlphaIter<SkMask::kA8_Format>(srcPixels), src.fRowBytes,
                dstPixels + pad * dst->fRowBytes + pad, dst->fRowBytes, srcWidth, srcHeight);
