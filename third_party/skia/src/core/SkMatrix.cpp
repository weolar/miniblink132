...
bool operator==(const SkMatrix& a, const SkMatrix& b)
{
    ...
    return ma[0] == mb[0] && ma[1] == mb[1] && ma[2] == mb[2] && ma[3] == mb[3] && ma[4] == mb[4] && ma[5] == mb[5] && ma[6] == mb[6] && ma[7] == mb[7]
        && ma[8] == mb[8];
}
...
bool SkMatrix::isSimilarity(SkScalar tol) const
{
    ...
    return (SkScalarNearlyEqual(mx, my, tol) && SkScalarNearlyEqual(sx, -sy, tol)) || (SkScalarNearlyEqual(mx, -my, tol) && SkScalarNearlyEqual(sx, sy, tol));
}
...
SkMatrix& SkMatrix::setScale(SkScalar sx, SkScalar sy)
{
    ...
    *this = SkMatrix(sx, 0, 0, 0, sy, 0, 0, 0, 1, (sx == 1 && sy == 1) ? kIdentity_Mask | rectMask : kScale_Mask | rectMask);
    return *this;
}
    ...
    if (a.isTriviallyIdentity()) {
        ...
        *this = a;
        ...
        this->setScaleTranslate(a.fMat[kMScaleX] * b.fMat[kMScaleX], a.fMat[kMScaleY] * b.fMat[kMScaleY],
            a.fMat[kMScaleX] * b.fMat[kMTransX] + a.fMat[kMTransX], a.fMat[kMScaleY] * b.fMat[kMTransY] + a.fMat[kMTransY]);
    } else {
        SkMatrix tmp;
...
static double sk_inv_determinant(const float mat[9], int isPerspective)
{
    ...
    if (SkScalarNearlyZero(sk_double_to_float(det), SK_ScalarNearlyZero * SK_ScalarNearlyZero * SK_ScalarNearlyZero)) {
        return 0;
    }
    return 1.0 / det;
}
...
const SkMatrix::MapPtsProc SkMatrix::gMapPtsProcs[] = { SkMatrix::Identity_pts, SkMatrix::Trans_pts, SkMatrix::Scale_pts, SkMatrix::Scale_pts,
    SkMatrix::Affine_vpts, SkMatrix::Affine_vpts, SkMatrix::Affine_vpts, SkMatrix::Affine_vpts,
    // repeat the persp proc 8 times
    SkMatrix::Persp_pts, SkMatrix::Persp_pts, SkMatrix::Persp_pts, SkMatrix::Persp_pts, SkMatrix::Persp_pts, SkMatrix::Persp_pts, SkMatrix::Persp_pts,
    SkMatrix::Persp_pts };
...
const SkMatrix::MapXYProc SkMatrix::gMapXYProcs[] = { SkMatrix::Identity_xy, SkMatrix::Trans_xy, SkMatrix::Scale_xy, SkMatrix::ScaleTrans_xy, SkMatrix::Rot_xy,
    SkMatrix::RotTrans_xy, SkMatrix::Rot_xy, SkMatrix::RotTrans_xy,
    // repeat the persp proc 8 times
    SkMatrix::Persp_xy, SkMatrix::Persp_xy, SkMatrix::Persp_xy, SkMatrix::Persp_xy, SkMatrix::Persp_xy, SkMatrix::Persp_xy, SkMatrix::Persp_xy,
    SkMatrix::Persp_xy };
...
bool SkMatrix::setPolyToPoly(const SkPoint src[], const SkPoint dst[], int count)
{
    ...
    const PolyMapProc gPolyMapProcs[] = { SkMatrix::Poly2Proc, SkMatrix::Poly3Proc, SkMatrix::Poly4Proc };
...
template <MinMaxOrBoth MIN_MAX_OR_BOTH> bool get_scale_factor(SkMatrix::TypeMask typeMask, const SkScalar m[9], SkScalar results[/*1 or 2*/])
{
...
{
    static constexpr SkMatrix invalid(SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, SK_ScalarMax,
        SK_ScalarMax, kTranslate_Mask | kScale_Mask | kAffine_Mask | kPerspective_Mask);
    return invalid;
}
...
bool SkMatrix::decomposeScale(SkSize* scale, SkMatrix* remaining) const
{
    ...
    if (!SkIsFinite(sx, sy) || SkScalarNearlyZero(sx) || SkScalarNearlyZero(sy)) {
        return false;
    }
...
{
    SkString str;
    str.appendf("[%8.4f %8.4f %8.4f][%8.4f %8.4f %8.4f][%8.4f %8.4f %8.4f]", fMat[0], fMat[1], fMat[2], fMat[3], fMat[4], fMat[5], fMat[6], fMat[7], fMat[8]);
    SkDebugf("%s\n", str.c_str());
}
...
bool SkTreatAsSprite(const SkMatrix& mat, const SkISize& size, const SkSamplingOptions& sampling, bool isAntiAlias)
{
    ...
    if (sampling.filter == SkFilterMode::kLinear && (mat.getTranslateX() != (int)mat.getTranslateX() || mat.getTranslateY() != (int)mat.getTranslateY())) {
        return false;
    }
...
bool SkMatrixPriv::NearlyAffine(const SkMatrix& m, const SkRect& bounds, SkScalar tolerance)
{
    if (!m.hasPerspective()) {
        return true;
    }
