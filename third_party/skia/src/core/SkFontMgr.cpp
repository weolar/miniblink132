...
sk_sp<SkTypeface> SkFontMgr::matchFamilyStyle(const char familyName[], const SkFontStyle& fs) const
{
    return this->onMatchFamilyStyle(familyName, fs);
}

sk_sp<SkTypeface> SkFontMgr::matchFamilyStyleCharacter(
...
sk_sp<SkTypeface> SkFontMgr::makeFromStream(std::unique_ptr<SkStreamAsset> stream, int ttcIndex) const
{
    if (nullptr == stream) {
        return nullptr;
    }
...
sk_sp<SkTypeface> SkFontMgr::makeFromStream(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args) const
{
    if (nullptr == stream) {
        return nullptr;
    }
...
sk_sp<SkTypeface> SkFontStyleSet::matchStyleCSS3(const SkFontStyle& pattern)
{
    ...
    struct Score {
        ...
        bool operator<(const Score& that)
        {
