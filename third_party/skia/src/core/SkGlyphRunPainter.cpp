...
std::tuple<SkZip<const SkGlyph*, SkPoint>, SkZip<SkGlyphID, SkPoint>> prepare_for_path_drawing(
    SkStrike* strike, SkZip<const SkGlyphID, const SkPoint> source, SkZip<const SkGlyph*, SkPoint> acceptedBuffer, SkZip<SkGlyphID, SkPoint> rejectedBuffer)
{
    int acceptedSize = 0;
    int rejectedSize = 0;
    strike->lock();
    for (auto [glyphID, pos] : source) {
        ...
        switch (SkGlyphDigest digest = strike->digestFor(kPath, packedID); digest.actionFor(kPath)) {
...
// TODO: collect this up into a single class when all the details are worked out.
// This is duplicate code. The original is in SubRunContainer.cpp.
std::tuple<SkZip<const SkGlyph*, SkPoint>, SkZip<SkGlyphID, SkPoint>> prepare_for_drawable_drawing(
    SkStrike* strike, SkZip<const SkGlyphID, const SkPoint> source, SkZip<const SkGlyph*, SkPoint> acceptedBuffer, SkZip<SkGlyphID, SkPoint> rejectedBuffer)
{
    int acceptedSize = 0;
    int rejectedSize = 0;
    ...
    for (auto [glyphID, pos] : source) {
        ...
        const SkPackedGlyphID packedID { glyphID };
        switch (SkGlyphDigest digest = strike->digestFor(kDrawable, packedID); digest.actionFor(kDrawable)) {
        case GlyphAction::kAccept:
            acceptedBuffer[acceptedSize++] = std::make_tuple(strike->glyph(digest), pos);
            break;
        case GlyphAction::kReject:
...
std::tuple<SkZip<const SkGlyph*, SkPoint>, SkZip<SkGlyphID, SkPoint>> prepare_for_direct_mask_drawing(SkStrike* strike, const SkMatrix& creationMatrix,
    SkZip<const SkGlyphID, const SkPoint> source, SkZip<const SkGlyph*, SkPoint> acceptedBuffer, SkZip<SkGlyphID, SkPoint> rejectedBuffer)
{
    ...
    for (auto [glyphID, pos] : source) {
        ...
        switch (SkGlyphDigest digest = strike->digestFor(kDirectMaskCPU, packedGlyphID); digest.actionFor(kDirectMaskCPU)) {
        case GlyphAction::kAccept: {
            const SkPoint roundedPos { SkScalarFloorToScalar(mappedPos.x()), SkScalarFloorToScalar(mappedPos.y()) };
            acceptedBuffer[acceptedSize++] = std::make_tuple(strike->glyph(digest), roundedPos);
            break;
        }
        case GlyphAction::kReject:
            rejectedBuffer[rejectedSize++] = std::make_tuple(glyphID, pos);
...
SkGlyphRunListPainterCPU::SkGlyphRunListPainterCPU(const SkSurfaceProps& props, SkColorType colorType, SkColorSpace* cs)
    : fDeviceProps { props }
...
void SkGlyphRunListPainterCPU::drawForBitmapDevice(
    SkCanvas* canvas, const BitmapDevicePainter* bitmapDevice, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint, const SkMatrix& drawMatrix)
{
    ...
    auto& props = (kN32_SkColorType == fColorType && paint.isSrcOver()) ? fDeviceProps : fBitmapFallbackProps;
    ...
    for (auto& glyphRun : glyphRunList) {
        ...
        if (SkStrikeSpec::ShouldDrawAsPath(paint, runFont, positionMatrix)) {
            auto [strikeSpec, strikeToSourceScale] = SkStrikeSpec::MakePath(runFont, paint, props, fScalerContextFlags);
...

            {
                ...
                const bool needsExactCTM = pathPaint.getShader() || pathPaint.getPathEffect() || pathPaint.getMaskFilter() || (stroking && !hairline);
        ...
        if (!source.empty() && !positionMatrix.hasPerspective()) {
            SkStrikeSpec strikeSpec = SkStrikeSpec::MakeMask(runFont, paint, props, fScalerContextFlags, positionMatrix);

            auto strike = strikeSpec.findOrCreateStrike();

            auto [accepted, rejected] = prepare_for_direct_mask_drawing(strike.get(), positionMatrix, source, acceptedBuffer, rejectedBuffer);
            source = rejected;
        ...
        if (!source.empty()) {
            ...
            SkStrikeSpec scaleStrikeSpec = SkStrikeSpec::MakeMask(runFont, paint, props, fScalerContextFlags, SkMatrix::I());
            ...
            SkStrikeSpec strikeSpec = SkStrikeSpec::MakeMask(runFont, paint, props, fScalerContextFlags, cacheScale);
            ...
            for (auto [glyph, srcPos] : SkMakeZip(accepted.get<0>(), sourcePositions)) {
                ...
                SkBitmap bm;
                bm.installPixels(SkImageInfo::MakeN32Premul(mask.fBounds.size()), const_cast<uint8_t*>(mask.fImage), mask.fRowBytes);
                bm.setImmutable();
                ...
                SkPoint realPos = srcPos + SkPoint::Make(mask.fBounds.left(), mask.fBounds.top()) * invMaxScale;
