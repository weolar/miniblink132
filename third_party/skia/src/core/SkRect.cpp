...
{
    SkIRect tmp = { std::max(a.fLeft, b.fLeft), std::max(a.fTop, b.fTop), std::min(a.fRight, b.fRight), std::min(a.fBottom, b.fBottom) };
    if (tmp.isEmpty()) {
        return false;
    }
    *this = tmp;
...
SkString SkRect::dumpToString(bool asHex) const
{
    ...
    if (asHex) {
        ...
        line.printf("SkRect::MakeLTRB(%s, %s, %s, %s);", strL.c_str(), strT.c_str(), strR.c_str(), strB.c_str());
    }
    return line;
}
...
template <typename R> static bool subtract(const R& a, const R& b, R* out)
{
...
bool SkRectPriv::QuadContainsRect(const SkMatrix& m, const SkIRect& a, const SkIRect& b, float tol)
{
...
{
    SkDEBUGCODE(SkM44 inverse;) SkASSERT(m.invert(&inverse));
