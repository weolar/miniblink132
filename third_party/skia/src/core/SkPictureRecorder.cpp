...
sk_sp<SkPicture> SkPictureRecorder::finishRecordingAsPicture()
{
    ...
    std::unique_ptr<SkBigPicture::SnapshotArray> pictList { drawableList ? drawableList->newDrawableSnapshot() : nullptr };
    ...
    if (fBBH) {
        ...
        SkASSERT((bbhBound.isEmpty() || fCullRect.contains(bbhBound)) || (bbhBound.isEmpty() && fCullRect.isEmpty()));
        fCullRect = bbhBound;
    }
    ...
    return sk_make_sp<SkBigPicture>(fCullRect, std::move(fRecord), std::move(pictList), std::move(fBBH), subPictureBytes);
}
...
sk_sp<SkDrawable> SkPictureRecorder::finishRecordingAsDrawable()
{
    ...
    sk_sp<SkDrawable> drawable = sk_make_sp<SkRecordedDrawable>(std::move(fRecord), std::move(fBBH), fRecorder->detachDrawableList(), fCullRect);
