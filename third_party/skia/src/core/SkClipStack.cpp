...
bool SkClipStack::Element::operator==(const Element& element) const
{
    ...
    if (fOp != element.fOp || fDeviceSpaceType != element.fDeviceSpaceType || fDoAA != element.fDoAA || fIsReplace != element.fIsReplace
...
{
    if (DeviceSpaceType::kEmpty == fDeviceSpaceType && (SkClipOp::kDifference == op || SkClipOp::kIntersect == op)) {
        return true;
    }
    // Only clips within the same save/restore frame (as captured by
    // the save count) can be merged
    return fSaveCount == saveCount && SkClipOp::kIntersect == op && (SkClipOp::kIntersect == fOp || this->isReplaceOp());
}
...
void SkClipStack::Element::updateBoundAndGenID(const Element* prior)
{
    ...
    case DeviceSpaceType::kRect:
        ...
        if (this->isReplaceOp() || (SkClipOp::kIntersect == fOp && nullptr == prior)
            || (SkClipOp::kIntersect == fOp && prior->fIsIntersectionOfRects && prior->rectRectIntersectAllowed(this->getDeviceSpaceRect(), fDoAA))) {
    ...
    SkASSERT(kInvPrev_InvCur_FillCombo == combination || kInvPrev_Cur_FillCombo == combination || kPrev_InvCur_FillCombo == combination
...
SkClipStack& SkClipStack::operator=(const SkClipStack& b)
{
    ...
    for (const Element* element = (const Element*)recIter.next(); element != nullptr; element = (const Element*)recIter.next()) {
...
void SkClipStack::getBounds(SkRect* canvFiniteBound, BoundsType* boundType, bool* isIntersectionOfRects) const
{
...
const SkClipStack::Element* SkClipStack::Iter::skipToTopmost(SkClipOp op)
{
    ...
    for (element = (const SkClipStack::Element*)fIter.prev(); element; element = (const SkClipStack::Element*)fIter.prev()) {
...
// helper method
void SkClipStack::getConservativeBounds(int offsetX, int offsetY, int maxWidth, int maxHeight, SkRect* devBounds, bool* isIntersectionOfRects) const
{
    SkASSERT(devBounds);
    ...
    if (back->getOp() == SkClipOp::kIntersect) {
        ...
        if (cnt > 1) {
            ...
            while (const Element* prior = (const Element*)iter.prev()) {
                ...
                if ((prior->getOp() != SkClipOp::kIntersect && !prior->isReplaceOp()) || !prior->contains(backBounds)) {
                    return false;
                }
...
void SkClipStack::Element::dump() const
{
    ...
    const char* opName = this->isReplaceOp() ? "replace" : (fOp == SkClipOp::kDifference ? "difference" : "intersect");
    SkDebugf("Type: %s, Op: %s, AA: %s, Save Count: %d\n", kTypeStrings[(int)fDeviceSpaceType], opName, (fDoAA ? "yes" : "no"), fSaveCount);
    switch (fDeviceSpaceType) {
    case DeviceSpaceType::kEmpty:
        SkDebugf("\n");
        break;
