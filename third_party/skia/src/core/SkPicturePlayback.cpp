...
void SkPicturePlayback::draw(SkCanvas* canvas, SkPicture::AbortCallback* callback, SkReadBuffer* buffer)
{
...
void SkPicturePlayback::handleOp(SkReadBuffer* reader, DrawType op, uint32_t size, SkCanvas* canvas, const SkM44& initialMatrix)
{
#define BREAK_ON_READ_ERROR(r)                                                                                                                                 \
    ...
    case DRAW_EDGEAA_IMAGE_SET2: {
        static const size_t kEntryReadSize = 4 * sizeof(uint32_t) + 2 * sizeof(SkRect) + sizeof(SkScalar);
        ...
        SkCanvas::SrcRectConstraint constraint = reader->checkRange(SkCanvas::kStrict_SrcRectConstraint, SkCanvas::kFast_SrcRectConstraint);
        ...
        if (!reader->validate(dstClipPointCount >= 0) || !reader->validate(expectedClipPointCount == dstClipPointCount)) {
        ...
        if (!reader->validate(matrixCount >= 0) || !reader->validate(maxMatrixIndex == (matrixCount - 1))
        ...
        canvas->experimental_DrawEdgeAAImageSet(set.get(), cnt, dstClips, matrices.begin(), sampling, paint, constraint);
    } break;
    case DRAW_IMAGE: {
        const SkPaint* paint = fPictureData->optionalPaint(reader);
    ...
    case DRAW_IMAGE_RECT: {
        ...
        if (DRAW_IMAGE_RECT == op) {
            ...
            constraint = reader->checkRange(SkCanvas::kStrict_SrcRectConstraint, SkCanvas::kFast_SrcRectConstraint);
        }
        BREAK_ON_READ_ERROR(reader);
    ...
    case DRAW_POINTS: {
        const SkPaint& paint = fPictureData->requiredPaint(reader);
        SkCanvas::PointMode mode = reader->checkRange(SkCanvas::kPoints_PointMode, SkCanvas::kPolygon_PointMode);
    ...
    case SAVE_LAYER_SAVELAYERREC: {
        ...
        if (!reader->isVersionLT(SkPicturePriv::Version::kBackdropScaleFactor) && (flatFlags & SAVELAYERREC_HAS_BACKDROP_SCALE)) {
            SkCanvasPriv::SetBackdropScaleFactor(&rec, reader->readScalar());
        }
        if (!reader->isVersionLT(SkPicturePriv::Version::kMultipleFiltersOnSaveLayer) && (flatFlags & SAVELAYERREC_HAS_MULTIPLE_FILTERS)) {
        ...
        if (!reader->isVersionLT(SkPicturePriv::Version::kSaveLayerBackdropTileMode) && (flatFlags & SAVELAYERREC_HAS_BACKDROP_TILEMODE)) {
