...
public:
    class Scan {
    public:
        Scan(uint64_t weight, int noChangeCount, uint32_t* buffer0, uint32_t* buffer0End, uint32_t* buffer1, uint32_t* buffer1End, uint32_t* buffer2,
            uint32_t* buffer2End)
            : fWeight { weight }
        ...
        template <typename AlphaIter> void blur(const AlphaIter srcBegin, const AlphaIter srcEnd, uint8_t* dst, int dstStride, uint8_t* dstEnd) const
        {
    ...
    Scan makeBlurScan(int width, uint32_t* buffer) const
    {
        ...
        return Scan(fWeight, noChangeCount, buffer0, buffer0End, buffer1, buffer1End, buffer2, buffer2End);
    }
...
static void lcd_to_a8(uint8_t* a8, const uint8_t* from, int width)
{
    ...
    for (int i = 0; i < width; ++i) {
        unsigned rgb = reinterpret_cast<const uint16_t*>(from)[i], r = SkPacked16ToR32(rgb), g = SkPacked16ToG32(rgb), b = SkPacked16ToB32(rgb);
        a8[i] = (r + g + b) / 3;
    }
}
...
static void blur_row(
    BlurX blur, const fp88& g0, const fp88& g1, const fp88& g2, const fp88& g3, const fp88& g4, const uint8_t* src, int srcW, uint8_t* dst, int dstW)
{
...
static fp88 blur_y_radius_1(
    const fp88& s0, const fp88& g0, const fp88& g1, const fp88&, const fp88&, const fp88&, fp88* d01, fp88* d12, fp88*, fp88*, fp88*, fp88*, fp88*, fp88*)
{
    auto v0 = mulhi(s0, g0);
    auto v1 = mulhi(s0, g1);
...
static fp88 blur_y_radius_2(const fp88& s0, const fp88& g0, const fp88& g1, const fp88& g2, const fp88&, const fp88&, fp88* d01, fp88* d12, fp88* d23,
    fp88* d34, fp88*, fp88*, fp88*, fp88*)
{
    auto v0 = mulhi(s0, g0);
    auto v1 = mulhi(s0, g1);
    auto v2 = mulhi(s0, g2);
...
static fp88 blur_y_radius_3(const fp88& s0, const fp88& g0, const fp88& g1, const fp88& g2, const fp88& g3, const fp88&, fp88* d01, fp88* d12, fp88* d23,
    fp88* d34, fp88* d45, fp88* d56, fp88*, fp88*)
{
    auto v0 = mulhi(s0, g0);
    auto v1 = mulhi(s0, g1);
    auto v2 = mulhi(s0, g2);
...
static fp88 blur_y_radius_4(const fp88& s0, const fp88& g0, const fp88& g1, const fp88& g2, const fp88& g3, const fp88& g4, fp88* d01, fp88* d12, fp88* d23,
    fp88* d34, fp88* d45, fp88* d56, fp88* d67, fp88* d78)
{
    auto v0 = mulhi(s0, g0);
    auto v1 = mulhi(s0, g1);
    auto v2 = mulhi(s0, g2);
...
static void blur_column(ToA8 toA8, BlurY blur, int radius, int width, const fp88& g0, const fp88& g1, const fp88& g2, const fp88& g3, const fp88& g4,
    const uint8_t* src, size_t srcRB, int srcH, uint8_t* dst, size_t dstRB)
{
    fp88 d01(kHalf), d12(kHalf), d23(kHalf), d34(kHalf), d45(kHalf), d56(kHalf), d67(kHalf), d78(kHalf);
...
static void blur_y_rect(
    ToA8 toA8, const int strideOf8, BlurY blur, int radius, uint16_t* gauss, const uint8_t* src, size_t srcRB, int srcW, int srcH, uint8_t* dst, size_t dstRB)
{
...
static void direct_blur_y(
    ToA8 toA8, const int strideOf8, int radius, uint16_t* gauss, const uint8_t* src, size_t srcRB, int srcW, int srcH, uint8_t* dst, size_t dstRB)
{
...
static SkIPoint small_blur(double sigmaX, double sigmaY, const SkMask& src, SkMaskBuilder* dst)
{
    ...
    uint16_t gaussFactorsX[SkGaussFilter::kGaussArrayMax], gaussFactorsY[SkGaussFilter::kGaussArrayMax];
    ...
    *dst = SkMaskBuilder::PrepareDestination(radiusX, radiusY, src);
    ...
    case SkMask::kBW_Format:
        direct_blur_y(bw_to_a8, 1, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH, dst->image() + radiusX, dstRB);
        break;
    case SkMask::kA8_Format:
        direct_blur_y(nullptr, 8, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH, dst->image() + radiusX, dstRB);
        break;
    case SkMask::kARGB32_Format:
        direct_blur_y(argb32_to_a8, 32, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH, dst->image() + radiusX, dstRB);
        break;
    case SkMask::kLCD16_Format:
        direct_blur_y(lcd_to_a8, 16, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH, dst->image() + radiusX, dstRB);
        break;
    default:
    ...
    direct_blur_x(radiusX, gaussFactorsX, dst->fImage + radiusX, dstRB, srcW, dst->image(), dstRB, dstW, dstH);
...
SkIPoint SkMaskBlurFilter::blur(const SkMask& src, SkMaskBuilder* dst) const
{
    ...
    *dst = SkMaskBuilder::PrepareDestination(borderW, borderH, src);
    ...
    int srcW = src.fBounds.width(), srcH = src.fBounds.height(), dstW = dst->fBounds.width(), dstH = dst->fBounds.height();
