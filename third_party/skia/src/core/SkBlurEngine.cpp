...
class Pass {
public:
    explicit Pass(int border)
        : fBorder(border)
    {
    }
    virtual ~Pass() = default;

    void blur(int srcLeft, int srcRight, int dstRight, const uint32_t* src, int srcStride, uint32_t* dst, int dstStride)
    {
        this->startBlur();

        auto srcStart = srcLeft - fBorder, srcEnd = srcRight - fBorder, dstEnd = dstRight, srcIdx = srcStart, dstIdx = 0;
...
protected:
    ...
    virtual void blurSegment(int n, const uint32_t* src, int srcStride, uint32_t* dst, int dstStride) = 0;
...
public:
    ...
    GaussPass(skvx::Vec<4, uint32_t>* buffer0, skvx::Vec<4, uint32_t>* buffer1, skvx::Vec<4, uint32_t>* buffer2, skvx::Vec<4, uint32_t>* buffersEnd, int border,
        int divisor)
        : Pass { border }
        , fBuffer0 { buffer0 }
        , fBuffer1 { buffer1 }
...
private:
    ...
    void blurSegment(int n, const uint32_t* src, int srcStride, uint32_t* dst, int dstStride) override
    {
        ...
        auto loadEdge = [&](const uint32_t* srcCursor) { return skvx::cast<uint32_t>(skvx::Vec<4, uint8_t>::Load(srcCursor)); };
...
public:
    ...
    TentPass(skvx::Vec<4, uint32_t>* buffer0, skvx::Vec<4, uint32_t>* buffer1, skvx::Vec<4, uint32_t>* buffersEnd, int border, int divisor)
        : Pass { border }
        , fBuffer0 { buffer0 }
        , fBuffer1 { buffer1 }
        , fBuffersEnd { buffersEnd }
...
public:
    ...
    sk_sp<SkSpecialImage> blur(
        SkSize sigma, sk_sp<SkSpecialImage> input, const SkIRect& originalSrcBounds, SkTileMode tileMode, const SkIRect& originalDstBounds) const override
    {
        ...
        SkASSERT(src.colorType() == kRGBA_8888_SkColorType || src.colorType() == kBGRA_8888_SkColorType);
        ...
        auto buffer = alloc.makeBytesAlignedTo(std::max(makerX->bufferSizeBytes(), makerY->bufferSizeBytes()), alignof(skvx::Vec<4, uint32_t>));
        ...
        if (makerX->window() > 1) {
            ...
            for (int y = loopStart; y < loopEnd; ++y) {
                pass->blur(srcBounds.left() - dstBounds.left(), srcBounds.right() - dstBounds.left(), dstBounds.width(), srcAddr, 1, dstAddr, 1);
        ...
        if (makerY->window() > 1) {
            ...
            for (int x = loopStart; x < loopEnd; ++x) {
                pass->blur(srcBounds.top() - dstBounds.top(), srcBounds.bottom() - dstBounds.top(), dstBounds.height(), srcAddr, src.rowBytesAsPixels(),
                    dstAddr, dst.rowBytesAsPixels());
                srcAddr += 1;
                dstAddr += 1;
            }
...
public:
    const Algorithm* findAlgorithm(SkSize sigma, SkColorType colorType) const override
    {
        ...
        const bool smallBlur = sigma.width() < kBoxBlurMinSigma && sigma.height() < kBoxBlurMinSigma;
        // The box blur doesn't actually care about channel order as long as it's 4 8-bit channels.
        const bool rgba8Blur = colorType == kRGBA_8888_SkColorType || colorType == kBGRA_8888_SkColorType;
...
void SkShaderBlurAlgorithm::Compute2DBlurKernel(SkSize sigma, SkISize radius, SkSpan<float> kernel)
{
    ...
    SkASSERT(SkBlurEngine::SigmaToRadius(sigma.width()) == radius.width() && SkBlurEngine::SigmaToRadius(sigma.height()) == radius.height());
...
void SkShaderBlurAlgorithm::Compute2DBlurKernel(SkSize sigma, SkISize radii, std::array<SkV4, kMaxSamples / 4>& kernel)
{
...
void SkShaderBlurAlgorithm::Compute1DBlurLinearKernel(float sigma, int radius, std::array<SkV4, kMaxSamples / 2>& offsetsAndKernel)
{
    ...
    if (radius & 1) {
        ...
        // \-----^---/ Lower sample
        //      \---^-----/ Upper sample
        get_new_weight(&kernel[halfRadius], &offsets[halfRadius], fullKernel[index] * 0.5f, fullKernel[index + 1]);
...
static SkKnownRuntimeEffects::StableKey to_stablekey(int kernelWidth, uint32_t baseKey)
{
    ...
    case 3:
        [[fallthrough]];
    case 4:
        return static_cast<SkKnownRuntimeEffects::StableKey>(baseKey);
    case 5:
        [[fallthrough]];
    case 6:
        [[fallthrough]];
    case 7:
        [[fallthrough]];
    case 8:
        return static_cast<SkKnownRuntimeEffects::StableKey>(baseKey + 1);
    case 9:
        [[fallthrough]];
    case 10:
        [[fallthrough]];
    case 11:
        [[fallthrough]];
    case 12:
        return static_cast<SkKnownRuntimeEffects::StableKey>(baseKey + 2);
    case 13:
        [[fallthrough]];
    case 14:
        [[fallthrough]];
    case 15:
        [[fallthrough]];
    case 16:
        return static_cast<SkKnownRuntimeEffects::StableKey>(baseKey + 3);
    case 17:
        [[fallthrough]];
    case 18:
        [[fallthrough]];
    ...
    case 20:
        return static_cast<SkKnownRuntimeEffects::StableKey>(baseKey + 4);
    case 21:
        [[fallthrough]];
    case 22:
        [[fallthrough]];
    ...
    case 27:
        [[fallthrough]];
    case 28:
        return static_cast<SkKnownRuntimeEffects::StableKey>(baseKey + 5);
    default:
        SkUNREACHABLE;
    }
}
...
{
    return GetKnownRuntimeEffect(to_stablekey(LinearKernelWidth(radius), static_cast<uint32_t>(SkKnownRuntimeEffects::StableKey::k1DBlurBase)));
}
...
const SkRuntimeEffect* SkShaderBlurAlgorithm::GetBlur2DEffect(const SkISize& radii)
{
    ...
    return GetKnownRuntimeEffect(to_stablekey(kernelArea, static_cast<uint32_t>(SkKnownRuntimeEffects::StableKey::k2DBlurBase)));
}

sk_sp<SkSpecialImage> SkShaderBlurAlgorithm::renderBlur(SkRuntimeShaderBuilder* blurEffectBuilder, SkFilterMode filter, SkISize radii,
    sk_sp<SkSpecialImage> input, const SkIRect& srcRect, SkTileMode tileMode, const SkIRect& dstRect) const
{
    SkImageInfo outII = SkImageInfo::Make({ dstRect.width(), dstRect.height() }, input->colorType(), kPremul_SkAlphaType, input->colorInfo().refColorSpace());
    ...
    if (!fastDstRect.isEmpty()) {
        ...
        SkTileMode fastTileMode = untiledSrcRect.contains(fastDstRect) ? SkTileMode::kClamp : tileMode;
        blurEffectBuilder->child("child") = input->asShader(fastTileMode, filter, SkMatrix::I(), /*strict=*/false);
    ...
    if (fastDstRect != dstRect) {
        blurEffectBuilder->child("child") = input->makeSubset(srcRect)->asShader(tileMode, filter, SkMatrix::Translate(srcRect.left(), srcRect.top()));
        paint.setShader(blurEffectBuilder->makeShader());
    }
    ...
    if (fastDstRect.isEmpty()) {
        ...
        drawBorder({ dstRect.left(), dstRect.top(), fastDstRect.left(), dstRect.bottom() }); // Left, spanning full height
        drawBorder({ fastDstRect.right(), dstRect.top(), dstRect.right(), dstRect.bottom() }); // Right, spanning full height
        drawBorder({ fastDstRect.left(), dstRect.top(), fastDstRect.right(), fastDstRect.top() }); // Top, spanning inner width
        drawBorder({ fastDstRect.left(), fastDstRect.bottom(), fastDstRect.right(), dstRect.bottom() }); // Bottom, spanning inner width
    }

    return device->snapSpecial(subset);
}

sk_sp<SkSpecialImage> SkShaderBlurAlgorithm::evalBlur2D(
    SkSize sigma, SkISize radii, sk_sp<SkSpecialImage> input, const SkIRect& srcRect, SkTileMode tileMode, const SkIRect& dstRect) const
{
    ...
    return this->renderBlur(&builder, SkFilterMode::kNearest, radii, std::move(input), srcRect, tileMode, dstRect);
}

sk_sp<SkSpecialImage> SkShaderBlurAlgorithm::evalBlur1D(
    float sigma, int radius, SkV2 dir, sk_sp<SkSpecialImage> input, SkIRect srcRect, SkTileMode tileMode, SkIRect dstRect) const
{
    ...
    return this->renderBlur(&builder, SkFilterMode::kLinear, radii, std::move(input), srcRect, tileMode, dstRect);
}

sk_sp<SkSpecialImage> SkShaderBlurAlgorithm::blur(
    SkSize sigma, sk_sp<SkSpecialImage> src, const SkIRect& srcRect, SkTileMode tileMode, const SkIRect& dstRect) const
{
    ...
    if (kernelArea <= kMaxSamples && radiusX > 0 && radiusY > 0) {
        ...
        return this->evalBlur2D(sigma, { radiusX, radiusY }, std::move(src), srcRect, tileMode, dstRect);
    } else {
        ...
        if (radiusX > 0) {
            if (radiusY > 0) {
                ...
                if (tileMode == SkTileMode::kRepeat || tileMode == SkTileMode::kMirror) {
                    ...
                    if (std::abs(dstRect.fTop - srcRect.fTop) % period != 0 || dstRect.height() != srcRect.height()) {
                    ...
                    intermediateDstRect.fBottom = std::min(intermediateDstRect.fBottom, srcRect.fBottom);
            ...
            src = this->evalBlur1D(sigma.width(), radiusX,
                /*dir=*/ { 1.f, 0.f }, std::move(src), srcRect, tileMode, intermediateDstRect);
            if (!src) {
                return nullptr;
            }
            intermediateSrcRect = SkIRect::MakeWH(src->width(), src->height());
            intermediateDstRect = dstRect.makeOffset(-intermediateDstRect.left(), -intermediateDstRect.top());
        }

        if (radiusY > 0) {
            src = this->evalBlur1D(sigma.height(), radiusY,
                /*dir=*/ { 0.f, 1.f }, std::move(src), intermediateSrcRect, tileMode, intermediateDstRect);
        }
