...
void SkScan::HairRect(const SkRect& rect, const SkRasterClip& clip, SkBlitter* blitter)
{
    ...
    SkIRect r = SkIRect::MakeLTRB(
        SkScalarFloorToInt(rect.fLeft), SkScalarFloorToInt(rect.fTop), SkScalarFloorToInt(rect.fRight + 1), SkScalarFloorToInt(rect.fBottom + 1));
...
static inline void hairquad(
    const SkPoint pts[3], const SkRegion* clip, const SkRect* insetClip, const SkRect* outsetClip, SkBlitter* blitter, int level, SkScan::HairRgnProc lineproc)
{
    if (insetClip) {
        SkASSERT(outsetClip);
...
static inline void haircubic(
    const SkPoint pts[4], const SkRegion* clip, const SkRect* insetClip, const SkRect* outsetClip, SkBlitter* blitter, int level, SkScan::HairRgnProc lineproc)
{
    if (insetClip) {
        SkASSERT(outsetClip);
...
template <SkPaint::Cap capStyle> void extend_pts(SkPath::Verb prevVerb, SkPath::Verb nextVerb, SkPoint* pts, int ptCount)
{
    ...
    if (SkPath::kMove_Verb == nextVerb || SkPath::kDone_Verb == nextVerb || SkPath::kClose_Verb == nextVerb) {
...
template <SkPaint::Cap capStyle> void hair_path(const SkPath& path, const SkRasterClip& rclip, SkBlitter* blitter, SkScan::HairRgnProc lineproc)
{
    if (path.isEmpty()) {
        return;
    }
