...
// SkMeshSpecification::~SkMeshSpecification(void)
// {
//     *(int*)1 = 1;
// }
// 
...
static bool fold_opacity_layer_color_to_paint(const SkPaint* layerPaint, bool isSaveLayer, SkPaint* paint)
{
    ...
    if (layerPaint) {
        ...
        if (layerPaint->getPathEffect() || layerPaint->getShader() || !layerPaint->isSrcOver() || layerPaint->getMaskFilter() || layerPaint->getColorFilter()
...
static bool effectively_srcover(const SkPaint* paint)
{
    ...
    return !paint->getShader() && !paint->getColorFilter() && !paint->getImageFilter() && 0xFF == paint->getAlpha()
        && paint->asBlendMode() == SkBlendMode::kSrc;
}
...
struct SvgOpacityAndFilterLayerMergePass {
    ...
    bool onMatch(SkRecord* record, Match* match, int begin, int end)
    {
        ...
        if (!match->first<SaveLayer>()->filters.empty() || !match->fourth<SaveLayer>()->filters.empty()) {
            // Our optimizations don't handle the filter list correctly - don't bother trying
            return false;
        }
