...
size_t SkImageInfo::computeByteSize(size_t rowBytes) const
{
    ...
    SkSafeMath safe;
    size_t bytes = safe.add(safe.mul(safe.addInt(this->height(), -1), rowBytes), safe.mul(this->width(), this->bytesPerPixel()));
...
SkImageInfo SkImageInfo::Make(int width, int height, SkColorType ct, SkAlphaType at, sk_sp<SkColorSpace> cs)
{
...
SkImageInfo SkImageInfo::Make(SkISize dimensions, SkColorType ct, SkAlphaType at, sk_sp<SkColorSpace> cs)
{
