...
private:
    ...
    {
        if (fPrevScanline != nullptr && fPrevScanline->fLastY + 1 == fCurrScanline->fLastY && fPrevScanline->fXCount == fCurrScanline->fXCount
...
bool SkRegion::setPath(const SkPath& path, const SkRegion& clip)
{
    ...
    if (SkScan::PathRequiresTiling(clipBounds)) {
        ...
        for (int64_t top = clipBounds.fTop; top < clipBounds.fBottom; top += kTileSize) {
            ...
            for (int64_t left = clipBounds.fLeft; left < clipBounds.fRight; left += kTileSize) {
                ...
                SkRegion tile;
                tile.setPath(path.makeTransform(SkMatrix::Translate(-left, -top)), SkRegion(tileClipBounds));
    ...
    if (!builder.init(bot - top, std::max(pathTransitions, clipTransitions), path.isInverseFillType())) {
