...
void SkTMaskGamma_build_correcting_lut(uint8_t* table, U8CPU srcI, SkScalar contrast, const SkColorSpaceLuminance& dstConvert, SkScalar dstGamma);
...
public:
        ...
        : fGammaTables(std::make_unique<uint8_t[]>(kTableNumElements))
    {
        ...
        for (U8CPU i = 0; i < kNumTables; ++i) {
            ...
            SkTMaskGamma_build_correcting_lut(&fGammaTables[i * kTableWidth], lum, contrast, deviceConvert, deviceGamma);
        }
    }
    ...
    static SkColor CanonicalColor(SkColor color)
    {
        return SkColorSetRGB(sk_t_scale255<R_LUM_BITS>(SkColorGetR(color) >> (8 - R_LUM_BITS)),
            sk_t_scale255<G_LUM_BITS>(SkColorGetG(color) >> (8 - G_LUM_BITS)), sk_t_scale255<B_LUM_BITS>(SkColorGetB(color) >> (8 - B_LUM_BITS)));
    }
...
SkTMaskPreBlend<R_LUM_BITS, G_LUM_BITS, B_LUM_BITS> SkTMaskGamma<R_LUM_BITS, G_LUM_BITS, B_LUM_BITS>::preBlend(SkColor color) const
{
    ...
    SkASSERT(r_index < kTableNumElements && g_index < kTableNumElements && b_index < kTableNumElements);
    return SkTMaskPreBlend<R_LUM_BITS, G_LUM_BITS, B_LUM_BITS>(sk_ref_sp(this), &fGammaTables[r_index], &fGammaTables[g_index], &fGammaTables[b_index]);
}
