...
void SkDraw::drawAtlas(const SkRSXform xform[], const SkRect textures[], const SkColor colors[], int count, sk_sp<SkBlender> blender, const SkPaint& paint)
{
    ...
    SkStageRec rec = { &pipeline, &alloc, fDst.colorType(), fDst.colorSpace(), p.getColor4f(), props };
    ...
    auto blitter = SkCreateRasterPipelineBlitter(fDst, p, pipeline, isOpaque, &alloc, fRC->clipShader());
    if (!blitter) {
        return;
    }
    SkPath scratchPath;
