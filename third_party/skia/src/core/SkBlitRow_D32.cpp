...
static inline __m128i SkPMLerp_SSE2(const __m128i& src, const __m128i& dst, const unsigned src_scale)
{
...
static inline __m128i SkBlendARGB32_SSE2(const __m128i& src, const __m128i& dst, const unsigned aa)
{
...
static inline __m256i SkPMLerp_LASX(const __m256i& src, const __m256i& dst, const unsigned src_scale)
{
    // Computes dst + (((src - dst)*src_scale)>>8)
...
static void blit_row_s32_blend(SkPMColor* dst, const SkPMColor* src, int count, U8CPU alpha)
{
    ...
    while (count >= 8) {
        __lasx_xvst(SkPMLerp_LASX(__lasx_xvld(src8, 0), __lasx_xvld(dst8, 0), SkAlpha255To256(alpha)), dst8, 0);
        src8++;
        dst8++;
        count -= 8;
    }
...
static inline __m256i SkBlendARGB32_LASX(const __m256i& src, const __m256i& dst, const unsigned aa)
{
    unsigned alpha = SkAlpha255To256(aa);
...
static void blit_row_s32a_blend(SkPMColor* dst, const SkPMColor* src, int count, U8CPU alpha)
{
    ...
    while (count >= 8) {
        __lasx_xvst(SkBlendARGB32_LASX(__lasx_xvld(src8, 0), __lasx_xvld(dst8, 0), alpha), dst8, 0);
        src8++;
        dst8++;
        count -= 8;
    }
...
static inline __m128i SkPMLerp_LSX(const __m128i& src, const __m128i& dst, const unsigned src_scale)
{
    // Computes dst + (((src - dst)*src_scale)>>8)
...
static void blit_row_s32_blend(SkPMColor* dst, const SkPMColor* src, int count, U8CPU alpha)
{
    ...
    while (count >= 4) {
        __lsx_vst(SkPMLerp_LSX(__lsx_vld(src4, 0), __lsx_vld(dst4, 0), SkAlpha255To256(alpha)), dst4, 0);
        src4++;
        dst4++;
        count -= 4;
    }
...
static inline __m128i SkBlendARGB32_LSX(const __m128i& src, const __m128i& dst, const unsigned aa)
{
    unsigned alpha = SkAlpha255To256(aa);
...
static void blit_row_s32a_blend(SkPMColor* dst, const SkPMColor* src, int count, U8CPU alpha)
{
    ...
    while (count >= 4) {
        __lsx_vst(SkBlendARGB32_LSX(__lsx_vld(src4, 0), __lsx_vld(dst4, 0), alpha), dst4, 0);
        src4++;
        dst4++;
        count -= 4;
    }
...
void SkBlitRow::Color32(SkPMColor dst[], int count, SkPMColor color)
{
    ...
    case 0: /* Nothing to do */
        return;
    case 255:
        SkOpts::memset32(dst, color, count);
        return;
    }
