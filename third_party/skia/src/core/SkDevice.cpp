...
void SkDevice::setDeviceCoordinateSystem(
    const SkM44& deviceToGlobal, const SkM44& globalToDevice, const SkM44& localToDevice, int bufferOriginX, int bufferOriginY)
{
...
SkIPoint SkDevice::getOrigin() const
{
    ...
    return SkIPoint::Make(SkScalarFloorToInt(fDeviceToGlobal.rc(0, 3)), SkScalarFloorToInt(fDeviceToGlobal.rc(1, 3)));
}
...
void SkDevice::drawImageLattice(const SkImage* image, const SkCanvas::Lattice& lattice, const SkRect& dst, SkFilterMode filter, const SkPaint& paint)
{
    ...
    while (iter.next(&srcR, &dstR, &isFixedColor, &c)) {
        ...
        if (isFixedColor || (srcR.width() <= 1.0f && srcR.height() <= 1.0f && image->readPixels(nullptr, info, &c, 4, srcR.fLeft, srcR.fTop))) {
            ...
            this->drawImageRect(image, &srcR, dstR, SkSamplingOptions(filter), paint, SkCanvas::kStrict_SrcRectConstraint);
        }
    }
}
...
void SkDevice::drawAtlas(const SkRSXform xform[], const SkRect tex[], const SkColor colors[], int quadCount, sk_sp<SkBlender> blender, const SkPaint& paint)
{
...
void SkDevice::drawEdgeAAQuad(const SkRect& r, const SkPoint clip[4], SkCanvas::QuadAAFlags aa, const SkColor4f& color, SkBlendMode mode)
{
    SkPaint paint;
...
void SkDevice::drawEdgeAAImageSet(const SkCanvas::ImageSetEntry images[], int count, const SkPoint dstClips[], const SkMatrix preViewMatrices[],
    const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        this->drawImageRect(images[i].fImage.get(), &images[i].fSrcRect, images[i].fDstRect, sampling, entryPaint, constraint);
        if (images[i].fHasClip) {
            this->popClipStack();
        }
        if (images[i].fMatrixIndex >= 0) {
...
void SkDevice::drawSpecial(SkSpecialImage*, const SkMatrix&, const SkSamplingOptions&, const SkPaint&, SkCanvas::SrcRectConstraint)
{
}
...
void SkDevice::drawDevice(SkDevice* device, const SkSamplingOptions& sampling, const SkPaint& paint)
{
    ...
    if (deviceImage) {
        ...
        const bool strict = sampling != SkFilterMode::kNearest || !relativeTransform.isTranslate() || !SkScalarIsInt(relativeTransform.getTranslateX())
            || !SkScalarIsInt(relativeTransform.getTranslateY());
        this->drawSpecial(
            deviceImage.get(), relativeTransform, sampling, paint, strict ? SkCanvas::kStrict_SrcRectConstraint : SkCanvas::kFast_SrcRectConstraint);
    }
}

void SkDevice::drawFilteredImage(const skif::Mapping& mapping, SkSpecialImage* src, SkColorType colorType, const SkImageFilter* filter,
    const SkSamplingOptions& sampling, const SkPaint& paint)
{
    SkASSERT(!paint.getImageFilter() && !paint.getMaskFilter());

    skif::LayerSpace<SkIRect> targetOutput = mapping.deviceToLayer(skif::DeviceSpace<SkIRect>(this->devClipBounds()));
    ...
    skif::Stats stats;
    skif::Context ctx { this->createImageFilteringBackend(src ? src->props() : this->surfaceProps(), colorType), mapping, targetOutput,
        skif::FilterResult(sk_ref_sp(src)), this->imageInfo().colorSpace(), &stats };
...
void SkDevice::drawGlyphRunList(SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
...
void SkDevice::simplifyGlyphRunRSXFormAndRedraw(SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
    for (const sktext::GlyphRun& run : glyphRunList) {
        if (run.scaledRotations().empty()) {
            ...
            sktext::GlyphRun glyphRun { run.font(), SkSpan<const SkPoint> { &sharedPos, 1 }, SkSpan<const SkGlyphID> { &sharedGlyphID, 1 },
                SkSpan<const char> {}, SkSpan<const uint32_t> {}, SkSpan<const SkVector> {} };
            ...
            for (auto [i, glyphID, pos] : SkMakeEnumerate(run.source())) {
                ...
                sktext::GlyphRunList subList = glyphRunList.builder()->makeGlyphRunList(glyphRun, paint, { 0, 0 });
...
SkNoPixelsDevice::SkNoPixelsDevice(const SkIRect& bounds, const SkSurfaceProps& props, sk_sp<SkColorSpace> colorSpace)
    : SkDevice(SkImageInfo::Make(bounds.size(), kUnknown_SkColorType, kUnknown_SkAlphaType, std::move(colorSpace)), props)
{
...
{
    this->writableClip().op(op, this->localToDevice44(), rrect.getBounds(), aa, /*fillsBounds=*/rrect.isRect());
}
...
void SkNoPixelsDevice::ClipState::op(SkClipOp op, const SkM44& transform, const SkRect& bounds, bool isAA, bool fillsBounds)
{
    const bool isRect = fillsBounds && SkMatrixPriv::IsScaleTranslateAsM33(transform);
    fIsAA |= isAA;

    SkRect devBounds = bounds.isEmpty() ? SkRect::MakeEmpty() : SkMatrixPriv::MapRect(transform, bounds);
    if (op == SkClipOp::kIntersect) {
        ...
        SkIRect difference;
        if (SkRectPriv::Subtract(fClipBounds, isAA ? devBounds.roundIn() : devBounds.round(), &difference)) {
