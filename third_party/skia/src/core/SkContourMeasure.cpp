...
void SkContourMeasure_segTo(const SkPoint pts[], unsigned segType, SkScalar startT, SkScalar stopT, SkPath* dst)
{
    ...
    case kLine_SegType:
        if (SK_Scalar1 == stopT) {
            ...
            dst->lineTo(SkScalarInterp(pts[0].fX, pts[1].fX, stopT), SkScalarInterp(pts[0].fY, pts[1].fY, stopT));
        }
        break;
    case kQuad_SegType:
        if (0 == startT) {
...
static bool cubic_too_curvy(const SkPoint pts[4], SkScalar tolerance)
{
    return cheap_dist_exceeds_limit(
               pts[1], SkScalarInterp(pts[0].fX, pts[3].fX, SK_Scalar1 / 3), SkScalarInterp(pts[0].fY, pts[3].fY, SK_Scalar1 / 3), tolerance)
        || cheap_dist_exceeds_limit(
            pts[2], SkScalarInterp(pts[0].fX, pts[3].fX, SK_Scalar1 * 2 / 3), SkScalarInterp(pts[0].fY, pts[3].fY, SK_Scalar1 * 2 / 3), tolerance);
}
...
private:
    ...
    SkDEBUGCODE(void validate() const;) SkScalar compute_line_seg(SkPoint p0, SkPoint p1, SkScalar distance, unsigned ptIndex);
    SkScalar compute_quad_segs(const SkPoint pts[3], SkScalar distance, int mint, int maxt, unsigned ptIndex, int recursionDepth = 0);
    SkScalar compute_conic_segs(
        const SkConic& conic, SkScalar distance, int mint, const SkPoint& minPt, int maxt, const SkPoint& maxPt, unsigned ptIndex, int recursionDepth = 0);
    SkScalar compute_cubic_segs(const SkPoint pts[4], SkScalar distance, int mint, int maxt, unsigned ptIndex, int recursionDepth = 0);
};

SkScalar SkContourMeasureIter::Impl::compute_quad_segs(const SkPoint pts[3], SkScalar distance, int mint, int maxt, unsigned ptIndex, int recursionDepth)
{
    if (recursionDepth < kMaxRecursionDepth && tspan_big_enough(maxt - mint) && quad_too_curvy(pts, fTolerance)) {
        SkPoint tmp[5];
...
SkScalar SkContourMeasureIter::Impl::compute_conic_segs(
    const SkConic& conic, SkScalar distance, int mint, const SkPoint& minPt, int maxt, const SkPoint& maxPt, unsigned ptIndex, int recursionDepth)
{
    int halft = (mint + maxt) >> 1;
    ...
    if (recursionDepth < kMaxRecursionDepth && tspan_big_enough(maxt - mint) && conic_too_curvy(minPt, halfPt, maxPt, fTolerance)) {
        recursionDepth += 1;
        distance = this->compute_conic_segs(conic, distance, mint, minPt, halft, halfPt, ptIndex, recursionDepth);
        distance = this->compute_conic_segs(conic, distance, halft, halfPt, maxt, maxPt, ptIndex, recursionDepth);
    } else {
...
SkScalar SkContourMeasureIter::Impl::compute_cubic_segs(const SkPoint pts[4], SkScalar distance, int mint, int maxt, unsigned ptIndex, int recursionDepth)
{
    if (recursionDepth < kMaxRecursionDepth && tspan_big_enough(maxt - mint) && cubic_too_curvy(pts, fTolerance)) {
        SkPoint tmp[7];
        int halft = (mint + maxt) >> 1;

        SkChopCubicAtHalf(pts, tmp);
        recursionDepth += 1;
        distance = this->compute_cubic_segs(tmp, distance, mint, halft, ptIndex, recursionDepth);
        distance = this->compute_cubic_segs(&tmp[3], distance, halft, maxt, ptIndex, recursionDepth);
    } else {
...
const SkContourMeasure::Segment* SkContourMeasure::distanceToSegment(SkScalar distance, SkScalar* t) const
{
