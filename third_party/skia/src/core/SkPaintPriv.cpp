...
bool SkPaintPriv::ShouldDither(const SkPaint& p, SkColorType dstCT)
{
    ...
    return p.getImageFilter() || p.getMaskFilter() || (p.getShader() && !as_SB(p.getShader())->isConstant());
}
...
void SkPaintPriv::RemoveColorFilter(SkPaint* p, SkColorSpace* dstCS)
{
    if (SkColorFilter* filter = p->getColorFilter()) {
        if (SkShader* shader = p->getShader()) {
            ...
            p->setShader(SkColorFilterShader::Make(sk_ref_sp(shader), p->getAlphaf(), sk_ref_sp(filter)));
            p->setAlphaf(1.0f);
        } else {
...
void SkPaintPriv::Flatten(const SkPaint& paint, SkWriteBuffer& buffer)
{
    ...
    if (paint.getPathEffect() || paint.getShader() || paint.getMaskFilter() || paint.getColorFilter() || paint.getImageFilter() || !paint.asBlendMode()) {
