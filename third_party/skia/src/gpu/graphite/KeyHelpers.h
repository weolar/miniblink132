...
struct GradientShaderBlocks {
    struct GradientData {
        ...
        GradientData(SkShaderBase::GradientType, SkPoint point0, SkPoint point1, float radius0, float radius1, float bias, float scale, SkTileMode,
            int numStops, const SkPMColor4f* colors, const float* offsets, const SkGradientBaseShader* shader, sk_sp<TextureProxy> colorsAndOffsetsProxy,
            bool useStorageBuffer, const SkGradientShader::Interpolation&);
...
struct MatrixColorFilterBlock {
    struct MatrixColorFilterData {
        MatrixColorFilterData(const float matrix[20], bool inHSLA, bool clamp)
            : fMatrix(matrix[0], matrix[1], matrix[2], matrix[3], matrix[5], matrix[6], matrix[7], matrix[8], matrix[10], matrix[11], matrix[12], matrix[13],
...
struct ColorSpaceTransformBlock {
    struct ColorSpaceTransformData {
        ...
        ColorSpaceTransformData(ReadSwizzle swizzle)
            : fReadSwizzle(swizzle)
        {
...
struct CircularRRectClipBlock {
    struct CircularRRectClipData {
        CircularRRectClipData(SkRect rect, SkPoint radiusPlusHalf, SkRect edgeSelect)
            : fRect(rect)
    ...
    static void AddBlock(const KeyContext&, PaintParamsKeyBuilder*, PipelineDataGatherer*, const CircularRRectClipData&);
};
 ...
 */
void AddBlendModeColorFilter(const KeyContext&, PaintParamsKeyBuilder*, PipelineDataGatherer*, SkBlendMode, const SkPMColor4f& srcColor);
...
void AddToKey(const KeyContext&, PaintParamsKeyBuilder*, PipelineDataGatherer*, const SkBlender*);
