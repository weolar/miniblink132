...
template <typename Sequence> class MonotonicValue {
public:
...
private:
    constexpr MonotonicValue(uint16_t index)
        : fIndex(index)
    {
    }
...
public:
    ...
    inline static constexpr CompressedPaintersOrder kNoIntersection = CompressedPaintersOrder::First();
    ...
    {
        return fDepth.bits() / (float)PaintersDepth::Last().bits();
    }
