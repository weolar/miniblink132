...
Resource::Resource(const SharedContext* sharedContext, Ownership ownership, skgpu::Budgeted budgeted, size_t gpuMemorySize, bool commandBufferRefsAsUsageRefs)
...
bool Resource::notifyARefIsZero(LastRemovedRef removedRef) const
{
    ...
    // SkASSERT(fReturnCache);
    if (removedRef != LastRemovedRef::kCache && fReturnCache && fReturnCache->returnResource(mutableThis, removedRef)) {
        return false;
    }
...
void Resource::dumpMemoryStatistics(SkTraceMemoryDump* traceMemoryDump) const
{
    ...
    if (this->budgeted() == skgpu::Budgeted::kNo && !traceMemoryDump->shouldDumpUnbudgetedObjects()) {
        return;
    }
    ...
    if (traceMemoryDump->shouldDumpWrappedObjects()) {
        traceMemoryDump->dumpWrappedState(resourceName.c_str(), this->ownership() == Ownership::kWrapped);
    }
    if (traceMemoryDump->shouldDumpUnbudgetedObjects()) {
        traceMemoryDump->dumpBudgetedState(resourceName.c_str(), this->budgeted() == skgpu::Budgeted::kYes);
    }
