...
bool oriented_bbox_intersection(const Rect& a, const Transform& aXform, const Rect& b, const Transform& bXform)
{
    ...
    SkASSERT(aXform.type() != Transform::Type::kPerspective && bXform.type() != Transform::Type::kPerspective);
    ...
    auto axesX = skvx::float4(-aXform.matrix().rc(1, 0), -aXform.matrix().rc(1, 1), -bXform.matrix().rc(1, 0), -bXform.matrix().rc(1, 1));
    auto axesY = skvx::float4(aXform.matrix().rc(0, 0), aXform.matrix().rc(0, 1), bXform.matrix().rc(0, 0), bXform.matrix().rc(0, 1));
...
bool ClipStack::TransformedShape::intersects(const TransformedShape& o) const
{
    ...
    if (fLocalToDevice.type() <= Transform::Type::kRectStaysRect && o.fLocalToDevice.type() <= Transform::Type::kRectStaysRect) {
    ...
    } else if (fLocalToDevice.type() != Transform::Type::kPerspective && o.fLocalToDevice.type() != Transform::Type::kPerspective) {
        ...
        // bounding boxes.
        return oriented_bbox_intersection(fShape.bounds(), fLocalToDevice, o.fShape.bounds(), o.fLocalToDevice);
    }
...
bool ClipStack::TransformedShape::contains(const TransformedShape& o) const
{
    ...
    if (fLocalToDevice == o.fLocalToDevice) {
        ...
        if (fShape.isRRect() && o.fShape.isRRect()) {
            return SkRRectPriv::ConservativeIntersect(fShape.rrect(), o.fShape.rrect()) == o.fShape.rrect();
            ...
            return (fShape.path().getGenerationID() == o.fShape.path().getGenerationID())
                || (fShape.path().countPoints() <= kMaxPathComparePoints && fShape.path() == o.fShape.path());
        } else {
            return fShape.conservativeContains(o.fShape.bounds());
        }
    } else if (fLocalToDevice.type() <= Transform::Type::kRectStaysRect && o.fLocalToDevice.type() <= Transform::Type::kRectStaysRect) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (deviceQuad[i].w < SkPathPriv::kW0PlaneDistance || localQuad[i].w < SkPathPriv::kW0PlaneDistance) {
...
ClipStack::SimplifyResult ClipStack::Simplify(const TransformedShape& a, const TransformedShape& b)
{
...
ClipStack::RawElement::RawElement(const Rect& deviceBounds, const Transform& localToDevice, const Shape& shape, SkClipOp op, PixelSnapping snapping)
    ...
    , fInvalidatedByIndex(-1)
{
    ...
    if (!fOuterBounds.isEmptyNegativeOrNaN() && fLocalToDevice.type() <= Transform::Type::kRectStaysRect) {
        if (fShape.isRect()) {
            ...
            if (fShape.rrect().transform(fLocalToDevice, &xformed)) {
                if (snapping == PixelSnapping::kYes) {
                    ...
                    xformed.setRectRadii(SkRect::Make(xformed.rect().round()), xformed.radii().data());
                }
...
void ClipStack::RawElement::drawClip(Device* device)
{
    ...
    if (!drawBounds.isEmptyNegativeOrNaN()) {
        ...
        // draw directly.
        SkASSERT((fOp == SkClipOp::kDifference && !fShape.inverted()) || (fOp == SkClipOp::kIntersect && fShape.inverted()));
        device->drawClipShape(fLocalToDevice, fShape,
...
void ClipStack::RawElement::validate() const
{
    ...
    SkASSERT((fShape.isEmpty() || !fOuterBounds.isEmptyNegativeOrNaN()) && (fInnerBounds.isEmptyNegativeOrNaN() || fOuterBounds.contains(fInnerBounds)));
    SkASSERT((fOp == SkClipOp::kDifference && !fShape.inverted()) || (fOp == SkClipOp::kIntersect && fShape.inverted()));
...
bool ClipStack::RawElement::combine(const RawElement& other, const SaveRecord& current)
{
    ...
    if (fShape.isRect() && other.fShape.isRect()) {
        ...
        if (fLocalToDevice == other.fLocalToDevice) {
            ...
            SkRRect b = other.fShape.isRect() ? SkRRect::MakeRect(other.fShape.rect().asSkRect()) : other.fShape.rrect();
...
ClipStack::RawElement::DrawInfluence ClipStack::RawElement::testForDraw(const TransformedShape& draw) const
{
    if (this->isInvalid()) {
...
CompressedPaintersOrder ClipStack::RawElement::updateForDraw(const BoundsManager* boundsManager, const Rect& drawBounds, PaintersDepth drawZ)
{
    SkASSERT(!this->isInvalid());
...
ClipStack::ClipState ClipStack::RawElement::clipType() const
{
    ...
    case Shape::Type::kRect:
        return fOp == SkClipOp::kIntersect && fLocalToDevice.type() == Transform::Type::kIdentity ? ClipState::kDeviceRect : ClipState::kComplex;

    case Shape::Type::kRRect:
        return fOp == SkClipOp::kIntersect && fLocalToDevice.type() == Transform::Type::kIdentity ? ClipState::kDeviceRRect : ClipState::kComplex;
...
bool ClipStack::SaveRecord::addElement(RawElement&& toAdd, RawElement::Stack* elements, Device* device)
{
    ...
    SkASSERT(!fOuterBounds.isEmptyNegativeOrNaN() && (fInnerBounds.isEmptyNegativeOrNaN() || fOuterBounds.contains(fInnerBounds)));

    return this->appendElement(std::move(toAdd), elements, device);
}

bool ClipStack::SaveRecord::appendElement(RawElement&& toAdd, RawElement::Stack* elements, Device* device)
{
...
void ClipStack::SaveRecord::replaceWithElement(RawElement&& toAdd, RawElement::Stack* elements, Device* device)
{
...
void ClipStack::clipShape(const Transform& localToDevice, const Shape& shape, SkClipOp op, PixelSnapping snapping)
{
    if (this->currentSaveRecord().state() == ClipState::kEmpty) {
        return;
    }
    ...
    SkDEBUGCODE(int elementCount = fElements.count();) if (!save.addElement(std::move(element), &fElements, fDevice))
    {
        if (wasDeferred) {
...
namespace {
CircularRRectClip can_apply_analytic_clip(const Shape& shape, const Transform& localToDevice)
{
...
Clip ClipStack::visitClipStackForDraw(const Transform& localToDevice, const Geometry& geometry, const SkStrokeRec& style, bool outsetBoundsForAA,
    ClipStack::ElementList* outEffectiveElements) const
{
    static const Clip kClippedOut = { Rect::InfiniteInverted(), Rect::InfiniteInverted(), SkIRect::MakeEmpty(),
        /* analyticClip= */ {}, /* shader= */ nullptr };
    ...
    float rendererOutset = outsetBoundsForAA ? localToDevice.localAARadius(styledShape->bounds()) : 0.f;
    ...
    TransformedShape draw { shapeInDeviceSpace ? kIdentity : localToDevice, *styledShape,
...
CompressedPaintersOrder ClipStack::updateClipStateForDraw(
    const Clip& clip, const ElementList& effectiveElements, const BoundsManager* boundsManager, PaintersDepth z)
{
    if (clip.isClippedOut()) {
        return DrawOrder::kNoIntersection;
    }

    SkDEBUGCODE(const SaveRecord& cs = this->currentSaveRecord();) SkASSERT(cs.state() != ClipState::kEmpty);
    ...
    for (int i = 0; i < effectiveElements.size(); ++i) {
        ...
        CompressedPaintersOrder order = const_cast<RawElement*>(e)->updateForDraw(boundsManager, clip.drawBounds(), z);
