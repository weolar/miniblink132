...
TextureProxy::TextureProxy(SkISize dimensions, const TextureInfo& info, std::string_view label, skgpu::Budgeted budgeted)
...
TextureProxy::TextureProxy(
    SkISize dimensions, const TextureInfo& textureInfo, skgpu::Budgeted budgeted, Volatile isVolatile, LazyInstantiateCallback&& callback)
    : fDimensions(dimensions)
    , fInfo(textureInfo)
    , fBudgeted(budgeted)
...
bool TextureProxy::InstantiateIfNotLazy(ResourceProvider* resourceProvider, TextureProxy* textureProxy)
{
...
bool TextureProxy::InstantiateIfNotLazy(ScratchResourceManager* scratchManager, TextureProxy* textureProxy)
{
    if (textureProxy->isLazy() || textureProxy->isInstantiated()) {
        return true;
    }

    textureProxy->fTexture = scratchManager->getScratchTexture(textureProxy->dimensions(), textureProxy->textureInfo(), textureProxy->fLabel);
...
sk_sp<TextureProxy> TextureProxy::Make(
    const Caps* caps, ResourceProvider* resourceProvider, SkISize dimensions, const TextureInfo& textureInfo, std::string_view label, skgpu::Budgeted budgeted)
{
    if (dimensions.width() < 1 || dimensions.height() < 1 || dimensions.width() > caps->maxTextureSize() || dimensions.height() > caps->maxTextureSize()
        || !textureInfo.isValid()) {
        return nullptr;
    }

    sk_sp<TextureProxy> proxy { new TextureProxy(dimensions, textureInfo, std::move(label), budgeted) };
...
sk_sp<TextureProxy> TextureProxy::MakeLazy(
    const Caps* caps, SkISize dimensions, const TextureInfo& textureInfo, skgpu::Budgeted budgeted, Volatile isVolatile, LazyInstantiateCallback&& callback)
{
    ...
    return sk_sp<TextureProxy>(new TextureProxy(dimensions, textureInfo, budgeted, isVolatile, std::move(callback)));
}

sk_sp<TextureProxy> TextureProxy::MakeFullyLazy(
    const TextureInfo& textureInfo, skgpu::Budgeted budgeted, Volatile isVolatile, LazyInstantiateCallback&& callback)
{
    SkASSERT(textureInfo.isValid());

    return sk_sp<TextureProxy>(new TextureProxy(SkISize::Make(-1, -1), textureInfo, budgeted, isVolatile, std::move(callback)));
}
...
void TextureProxy::validateTexture(const Texture* texture)
{
    ...
    SkASSERTF(fInfo.isCompatible(texture->textureInfo()), "proxy->fInfo[%s] incompatible with texture->fInfo[%s]", fInfo.toString().c_str(),
