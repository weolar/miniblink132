...
public:
    static sk_sp<MtlComputeCommandEncoder> Make(const SharedContext* sharedContext, id<MTLCommandBuffer> commandBuffer)
    {
        ...
        @autoreleasepool {
            ...
            sk_cfp<id<MTLComputeCommandEncoder>> encoder = sk_ret_cfp([commandBuffer computeCommandEncoder]);

            // TODO(armansito): Support concurrent dispatch of compute passes using
            // MTLDispatchTypeConcurrent on macOS 10.14+ and iOS 12.0+.
            return sk_sp<MtlComputeCommandEncoder>(new MtlComputeCommandEncoder(sharedContext, std::move(encoder)));
        }
    }
    ...
    void setBufferOffset(NSUInteger offset, NSUInteger index) SK_API_AVAILABLE(macos(10.11), ios(8.3), tvos(9.0))
    {
        SkASSERT(index < kMaxExpectedBuffers);
    ...
    {
        MTLSize threadgroupCount = MTLSizeMake(globalSize.fWidth, globalSize.fHeight, globalSize.fDepth);
        MTLSize threadsPerThreadgroup = MTLSizeMake(localSize.fWidth, localSize.fHeight, localSize.fDepth);
        [(*fCommandEncoder) dispatchThreadgroups:threadgroupCount threadsPerThreadgroup:threadsPerThreadgroup];
    }

    void dispatchThreadgroupsWithIndirectBuffer(id<MTLBuffer> indirectBuffer, NSUInteger offset, const WorkgroupSize& localSize)
    {
        MTLSize threadsPerThreadgroup = MTLSizeMake(localSize.fWidth, localSize.fHeight, localSize.fDepth);
        [(*fCommandEncoder) dispatchThreadgroupsWithIndirectBuffer:indirectBuffer indirectBufferOffset:offset threadsPerThreadgroup:threadsPerThreadgroup];
    }
...
private:
    ...
    MtlComputeCommandEncoder(const SharedContext* sharedContext, sk_cfp<id<MTLComputeCommandEncoder>> encoder)
