...
public:
    static sk_sp<MtlBlitCommandEncoder> Make(const SharedContext* sharedContext, id<MTLCommandBuffer> commandBuffer)
    {
        @autoreleasepool {
            ...
            sk_cfp<id<MTLBlitCommandEncoder>> encoder = sk_ret_cfp<id<MTLBlitCommandEncoder>>([commandBuffer blitCommandEncoder]);
            return sk_sp<MtlBlitCommandEncoder>(new MtlBlitCommandEncoder(sharedContext, std::move(encoder)));
        }
    }
    ...
    {
        [(*fCommandEncoder) fillBuffer:buffer range:NSMakeRange(bufferOffset, bytes) value:value];
    }
    ...
    {
        [(*fCommandEncoder) copyFromBuffer:srcBuffer sourceOffset:srcOffset toBuffer:dstBuffer destinationOffset:dstOffset size:size];
    }
...
private:
    MtlBlitCommandEncoder(const SharedContext* sharedContext, sk_cfp<id<MTLBlitCommandEncoder>> encoder)
