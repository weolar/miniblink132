...
struct MtlTextureSpec {
    ...
    {
        return fFormat == that.fFormat && fUsage == that.fUsage && fStorageMode == that.fStorageMode && fFramebufferOnly == that.fFramebufferOnly;
    }
    ...
    bool isCompatible(const MtlTextureSpec& that) const
    {
        ...
        return fFormat == that.fFormat && fStorageMode == that.fStorageMode && fFramebufferOnly == that.fFramebufferOnly && (fUsage & that.fUsage) == fUsage;
    }

    SkString toString() const
    {
        return SkStringPrintf(
            "format=%u,usage=0x%04X,storageMode=%u,framebufferOnly=%d", (uint32_t)fFormat, (uint32_t)fUsage, (uint32_t)fStorageMode, fFramebufferOnly);
    }
