...
public:
    ...
    bool isFinished()
    {
        return (*fCommandBuffer).status == MTLCommandBufferStatusCompleted || (*fCommandBuffer).status == MTLCommandBufferStatusError;
    }
    void waitUntilFinished()
    {
        // TODO: it's not clear what do to if status is Enqueued. Commit and then wait?
        if ((*fCommandBuffer).status == MTLCommandBufferStatusScheduled || (*fCommandBuffer).status == MTLCommandBufferStatusCommitted) {
...
private:
    MtlCommandBuffer(id<MTLCommandQueue>, const MtlSharedContext* sharedContext, MtlResourceProvider* resourceProvider);
    ...
    bool beginRenderPass(const RenderPassDesc&, const Texture* colorTexture, const Texture* resolveTexture, const Texture* depthStencilTexture);
    void endRenderPass();
    ...
    void dispatchThreadgroupsIndirect(const WorkgroupSize& localSize, const Buffer* indirectBuffer, size_t indirectBufferOffset);
    void endComputePass();
