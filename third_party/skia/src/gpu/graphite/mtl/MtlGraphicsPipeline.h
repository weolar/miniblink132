...
public:
    ...
    static sk_sp<MtlGraphicsPipeline> Make(const MtlSharedContext*, MtlResourceProvider*, const RuntimeEffectDictionary*, const GraphicsPipelineDesc&,
        const RenderPassDesc&, SkEnumBitMask<PipelineCreationFlags>);

    static sk_sp<MtlGraphicsPipeline> MakeLoadMSAAPipeline(const MtlSharedContext*, MtlResourceProvider*, const RenderPassDesc&);
...
private:
    MtlGraphicsPipeline(const skgpu::graphite::SharedContext* sharedContext, const PipelineInfo& pipelineInfo, sk_cfp<id<MTLRenderPipelineState>> pso,
        sk_cfp<id<MTLDepthStencilState>> dss, uint32_t refValue);

    using MSLFunction = std::pair<id<MTLLibrary>, std::string>;
    static sk_sp<MtlGraphicsPipeline> Make(const MtlSharedContext*, const std::string& label, const PipelineInfo&, MSLFunction vertexMain,
        SkSpan<const Attribute> vertexAttrs, SkSpan<const Attribute> instanceAttrs, MSLFunction fragmentMain, sk_cfp<id<MTLDepthStencilState>>,
        uint32_t stencilRefValue, const BlendInfo& blendInfo, const RenderPassDesc&);
