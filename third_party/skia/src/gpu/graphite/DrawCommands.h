...
COMMAND(BindUniformBuffer, BindBufferInfo fInfo; UniformSlot fSlot);
...
COMMAND(DrawIndexedInstanced, PrimitiveType fType; uint32_t fBaseIndex; uint32_t fIndexCount; uint32_t fBaseVertex; uint32_t fBaseInstance;
...
public:
    ...
    {
        this->add<DrawIndexedInstanced>(type, baseIndex, indexCount, baseVertex, baseInstance, instanceCount);
    }
...
private:
    ...
    template <typename T> T* copy(const T src[], size_t count)
    {
