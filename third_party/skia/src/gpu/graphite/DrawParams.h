...
public:
    StrokeStyle()
        : fHalfWidth(0.f)
        , fJoinLimit(0.f)
        , fCap(SkPaint::kButt_Cap)
    {
    }
    StrokeStyle(float width, float miterLimit, SkPaint::Join join, SkPaint::Cap cap)
        : fHalfWidth(std::max(0.f, 0.5f * width))
        , fJoinLimit(join == SkPaint::kMiter_Join ? std::max(0.f, miterLimit) : (join == SkPaint::kBevel_Join ? 0.f : -1.f))
        , fCap(cap)
    {
    }
    ...
    {
        return fJoinLimit > 0.f ? SkPaint::kMiter_Join : (fJoinLimit == 0.f ? SkPaint::kBevel_Join : SkPaint::kRound_Join);
    }
...
public:
    Clip() = default;
    Clip(const Rect& drawBounds, const Rect& shapeBounds, const SkIRect& scissor, const CircularRRectClip& analyticClip, const SkShader* shader)
    ...
    const CircularRRectClip& analyticClip() const
    {
        return fAnalyticClip;
    }
