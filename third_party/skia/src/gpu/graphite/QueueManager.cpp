...
std::vector<std::unique_ptr<CommandBuffer>>* QueueManager::getAvailableCommandBufferList(Protected isProtected)
{
    return isProtected == Protected::kNo ? &fAvailableCommandBuffers : &fAvailableProtectedCommandBuffers;
}

bool QueueManager::setupCommandBuffer(ResourceProvider* resourceProvider, Protected isProtected)
{
    if (!fCurrentCommandBuffer) {
        std::vector<std::unique_ptr<CommandBuffer>>* bufferList = this->getAvailableCommandBufferList(isProtected);
        if (!bufferList->empty()) {
...
bool QueueManager::addRecording(const InsertRecordingInfo& info, Context* context)
{
    ...
    if (fSharedContext->caps()->requireOrderedRecordings()) {
        ...
        fLastAddedRecordingIDs.set(info.fRecording->priv().recorderID(), info.fRecording->priv().uniqueID());
    }

    if (info.fTargetSurface && !static_cast<const SkSurface_Base*>(info.fTargetSurface)->isGraphiteBacked()) {
        if (callback) {
            callback->setFailureResult();
        }
    ...
    if (!info.fRecording->priv().addCommands(
            context, fCurrentCommandBuffer.get(), static_cast<Surface*>(info.fTargetSurface), info.fTargetTranslation, info.fTargetClip)) {
        if (callback) {
            callback->setFailureResult();
        }
        info.fRecording->priv().setFailureResultForFinishedProcs();
    ...
    if (info.fTargetTextureState) {
        fCurrentCommandBuffer->prepareSurfaceForStateUpdate(info.fTargetSurface, info.fTargetTextureState);
    }
...
bool QueueManager::addTask(Task* task, Context* context, Protected isProtected)
{
    SkASSERT(task);
    if (!task) {
...
bool QueueManager::addFinishInfo(const InsertFinishInfo& info, ResourceProvider* resourceProvider, SkSpan<const sk_sp<Buffer>> buffersToAsyncMap)
{
    sk_sp<RefCntedCallback> callback;
    if (info.fFinishedProc) {
        callback = RefCntedCallback::Make(info.fFinishedProc, info.fFinishedContext);
...
{
    std::vector<std::unique_ptr<CommandBuffer>>* bufferList = this->getAvailableCommandBufferList(commandBuffer->isProtected());
