...
SkISize Caps::getDepthAttachmentDimensions(const TextureInfo& textureInfo, const SkISize colorAttachmentDimensions) const
{
...
{
    do {
        auto texInfo = this->getDefaultSampledTextureInfo(ct, Mipmapped::kNo, Protected::kNo, Renderable::kYes);
...
sktext::gpu::SubRunControl Caps::getSubRunControl(bool useSDFTForSmallText) const
{
    ...
    return sktext::gpu::SubRunControl { this->shaderCaps()->supportsDistanceFieldText(), useSDFTForSmallText, true, /*ableToUsePerspectiveSDFT*/
        this->minDistanceFieldFontSize(), this->glyphsAsPathsFontSize(), true /*forcePathAA*/ };
#else
