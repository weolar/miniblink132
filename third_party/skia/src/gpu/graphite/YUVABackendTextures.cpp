...
YUVABackendTextureInfo::YUVABackendTextureInfo(const Recorder* recorder, const SkYUVAInfo& yuvaInfo, SkSpan<const TextureInfo> textureInfo, Mipmapped mipmapped)
    : fYUVAInfo(yuvaInfo)
    , fMipmapped(mipmapped)
{
    int numPlanes = yuvaInfo.numPlanes();
    if (!yuvaInfo.isValid() || numPlanes == 0 || (size_t)numPlanes > textureInfo.size()) {
        *this = {};
    ...
    for (int i = 0; i < numPlanes; ++i) {
        ...
        if (!textureInfo[i].isValid() || textureInfo[i].backend() != textureInfo[0].backend() || num_channels(fPlaneChannelMasks[i]) < numRequiredChannels) {
            *this = {};
            SkASSERT(!this->isValid());
            return;
        }
...
YUVABackendTextures::YUVABackendTextures(const Recorder* recorder, const SkYUVAInfo& yuvaInfo, SkSpan<const BackendTexture> textures)
    : fYUVAInfo(yuvaInfo)
{
    if (!yuvaInfo.isValid()) {
        SkASSERT(!this->isValid());
