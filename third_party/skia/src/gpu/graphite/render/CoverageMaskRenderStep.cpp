...
CoverageMaskRenderStep::CoverageMaskRenderStep()
    : RenderStep("CoverageMaskRenderStep", "",
        ...
        Flags::kPerformsShading | Flags::kHasTextures | Flags::kEmitsCoverage | Flags::kOutsetBoundsForAA,
        /*uniforms=*/ { { "maskToDeviceRemainder", SkSLType::kFloat3x3 } }, PrimitiveType::kTriangleStrip, kDirectDepthGreaterPass,
        /*vertexAttrs=*/ {},
        /*instanceAttrs=*/
        ...
        { { "drawBounds", VertexAttribType::kFloat4, SkSLType::kFloat4 }, // ltrb
            ...
            { "deviceOrigin", VertexAttribType::kFloat2, SkSLType::kFloat2 }, { "depth", VertexAttribType::kFloat, SkSLType::kFloat },
        ...
        { // `maskBounds` are the atlas-relative, sorted bounds of the coverage mask.
            ...
            { "maskBounds", SkSLType::kFloat4 }, { "textureCoords", SkSLType::kFloat2 },
...
std::string CoverageMaskRenderStep::texturesAndSamplersSkSL(const ResourceBindingRequirements& bindingReqs, int* nextBindingIndex) const
{
...
void CoverageMaskRenderStep::writeVertices(DrawWriter* dw, const DrawParams& params, skvx::uint2 ssboIndices) const
{
    ...
    if (coverageMask.inverted()) {
        ...
        SkASSERT(!params.clip().transformedShapeBounds().isEmptyNegativeOrNaN() ^ all(maskBounds == 0.f));
    ...
    auto atlasSizeInv = skvx::float2 { 1.f / proxy->dimensions().width(), 1.f / proxy->dimensions().height() };
    ...
    instances.append(1) << drawBounds << skvx::cast<uint16_t>(maskBounds) << deviceOrigin << params.order().depthAsFloat() << ssboIndices << m.rc(0, 0)
                        << m.rc(1, 0) << m.rc(3, 0) // mat0
                        << m.rc(0, 1) << m.rc(1, 1) << m.rc(3, 1) // mat1
                        << m.rc(0, 3) << m.rc(1, 3) << m.rc(3, 3); // mat2
}

void CoverageMaskRenderStep::writeUniformsAndTextures(const DrawParams& params, PipelineDataGatherer* gatherer) const
{
    SkDEBUGCODE(UniformExpectationsValidator uev(gatherer, this->uniforms());)

        const CoverageMaskShape& coverageMask
        = params.geometry().coverageMaskShape();
    const TextureProxy* proxy = coverageMask.textureProxy();
    SkASSERT(proxy);
    ...
    const bool pixelAligned = params.transform().type() <= Transform::Type::kSimpleRectStaysRect && params.transform().maxScaleFactor() == 1.f
        && all(deviceOrigin == floor(deviceOrigin + SK_ScalarNearlyZero));
    gatherer->add(sk_ref_sp(proxy), { pixelAligned ? SkFilterMode::kNearest : SkFilterMode::kLinear, SkTileMode::kClamp });
}
