...
template <typename FixedCountVariant> class DynamicInstancesPatchAllocator {
public:
    ...
    DynamicInstancesPatchAllocator(
        size_t stride, DrawWriter& writer, BindBufferInfo fixedVertexBuffer, BindBufferInfo fixedIndexBuffer, unsigned int reserveCount)
...
private:
    struct LinearToleranceProxy {
        ...
        void operator<<(const tess::LinearTolerances& t)
        {
            fTolerances.accumulate(t);
        }
