...
SDFTextRenderStep::SDFTextRenderStep()
    : RenderStep("SDFTextRenderStep", "", Flags::kPerformsShading | Flags::kHasTextures | Flags::kEmitsCoverage,
        /*uniforms=*/
...
std::string SDFTextRenderStep::texturesAndSamplersSkSL(const ResourceBindingRequirements& bindingReqs, int* nextBindingIndex) const
{
    std::string result;
...
void SDFTextRenderStep::writeVertices(DrawWriter* dw, const DrawParams& params, skvx::uint2 ssboIndices) const
{
...
void SDFTextRenderStep::writeUniformsAndTextures(const DrawParams& params, PipelineDataGatherer* gatherer) const
{
    ...
    U8CPU lum = SkColorSpaceLuminance::computeLuminance(SK_GAMMA_EXPONENT, subRunData.luminanceColor());
