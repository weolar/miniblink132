...
CoverBoundsRenderStep::CoverBoundsRenderStep(const char* tag, DepthStencilSettings dsSettings)
    : RenderStep("CoverBoundsRenderStep", tag, Flags::kPerformsShading,
        /*uniforms=*/ {}, PrimitiveType::kTriangleStrip, dsSettings,
        /*vertexAttrs=*/ {},
        /*instanceAttrs=*/
        { { "bounds", VertexAttribType::kFloat4, SkSLType::kFloat4 }, { "depth", VertexAttribType::kFloat, SkSLType::kFloat },
            { "ssboIndices", VertexAttribType::kUInt2, SkSLType::kUInt2 }, { "mat0", VertexAttribType::kFloat3, SkSLType::kFloat3 },
            { "mat1", VertexAttribType::kFloat3, SkSLType::kFloat3 }, { "mat2", VertexAttribType::kFloat3, SkSLType::kFloat3 } })
{
}
...
void CoverBoundsRenderStep::writeVertices(DrawWriter* writer, const DrawParams& params, skvx::uint2 ssboIndices) const
{
    ...
    if (params.geometry().isShape() && params.geometry().shape().inverted()) {
        ...
        // coordinates.
        bounds = skvx::shuffle</*R*/ 2, /*B*/ 3, /*L*/ 0, /*T*/ 1>(skvx::cast<float>(skvx::int4::Load(&params.clip().scissor())));
    } else {
    ...
    instances.append(1) << bounds << params.order().depthAsFloat() << ssboIndices << m.rc(0, 0) << m.rc(1, 0) << m.rc(3, 0) << m.rc(0, 1) << m.rc(1, 1)
                        << m.rc(3, 1) << m.rc(0, 3) << m.rc(1, 3) << m.rc(3, 3);
}

void CoverBoundsRenderStep::writeUniformsAndTextures(const DrawParams&, PipelineDataGatherer*) const
{
