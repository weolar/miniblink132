...
CircularArcRenderStep::CircularArcRenderStep(StaticBufferManager* bufferManager)
    : RenderStep("CircularArcRenderStep", "", Flags::kPerformsShading | Flags::kEmitsCoverage | Flags::kOutsetBoundsForAA,
...
void CircularArcRenderStep::writeVertices(DrawWriter* writer, const DrawParams& params, skvx::uint2 ssboIndices) const
{
    ...
    bool useCenter = (arc.isWedge() || isStroke) && !SkScalarNearlyEqual(absSweep, SK_ScalarPI);
    ...
    if (isStroke && innerRadius > -SK_ScalarHalf && params.strokeStyle().halfWidth() > 0 && params.strokeStyle().cap() == SkPaint::kRound_Cap) {
    ...
    vw << localCenter << localOuterRadius << localInnerRadius << outerRadius << innerRadius << flags << geoClipPlane << clipPlane0 << clipPlane1 << roundCapPos0
       << roundCapPos1 << params.order().depthAsFloat() << ssboIndices << m.rc(0, 0) << m.rc(1, 0) << m.rc(3, 0) // mat0
       << m.rc(0, 1) << m.rc(1, 1) << m.rc(3, 1) // mat1
       << m.rc(0, 3) << m.rc(1, 3) << m.rc(3, 3); // mat2
}

void CircularArcRenderStep::writeUniformsAndTextures(const DrawParams&, PipelineDataGatherer*) const
{
