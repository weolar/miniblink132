...
RendererProvider::RendererProvider(const Caps* caps, StaticBufferManager* bufferManager)
{
    ...
    static constexpr size_t kRendererSize = offsetof(RendererProvider, fRenderers) - offsetof(RendererProvider, fStencilTessellatedCurves);
    ...
    fConvexTessellatedWedges = makeFromStep(
        std::make_unique<TessellateWedgesRenderStep>("convex", infinitySupport, kDirectDepthGreaterPass, bufferManager), DrawTypeFlags::kNonSimpleShape);
    fTessellatedStrokes = makeFromStep(std::make_unique<TessellateStrokesRenderStep>(infinitySupport), DrawTypeFlags::kNonSimpleShape);
    fCoverageMask = makeFromStep(std::make_unique<CoverageMaskRenderStep>(),
        static_cast<DrawTypeFlags>(static_cast<int>(DrawTypeFlags::kNonSimpleShape) | static_cast<int>(InternalDrawTypeFlags::kCoverageMask)));

    static constexpr struct {
        skgpu::MaskFormat fFormat;
        DrawTypeFlags fDrawType;
    } kBitmapTextVariants[] = { // We are using 565 here to represent LCD text, regardless of texture format
        { skgpu::MaskFormat::kA8, DrawTypeFlags::kBitmapText_Mask }, { skgpu::MaskFormat::kA565, DrawTypeFlags::kBitmapText_LCD },
        { skgpu::MaskFormat::kARGB, DrawTypeFlags::kBitmapText_Color }
    };

    for (auto textVariant : kBitmapTextVariants) {
        fBitmapText[int(textVariant.fFormat)] = makeFromStep(std::make_unique<BitmapTextRenderStep>(textVariant.fFormat), textVariant.fDrawType);
    }
    for (bool lcd : { false, true }) {
        fSDFText[lcd] = lcd ? makeFromStep(std::make_unique<SDFTextLCDRenderStep>(), DrawTypeFlags::kSDFText_LCD)
                            : makeFromStep(std::make_unique<SDFTextRenderStep>(), DrawTypeFlags::kSDFText);
    }
    fAnalyticRRect = makeFromStep(std::make_unique<AnalyticRRectRenderStep>(bufferManager),
        static_cast<DrawTypeFlags>(static_cast<int>(DrawTypeFlags::kSimpleShape) | static_cast<int>(InternalDrawTypeFlags::kAnalyticRRect)));
    fPerEdgeAAQuad = makeFromStep(std::make_unique<PerEdgeAAQuadRenderStep>(bufferManager), DrawTypeFlags::kSimpleShape);
    fNonAABoundsFill = makeFromStep(std::make_unique<CoverBoundsRenderStep>("non-aa-fill", kDirectDepthGreaterPass), DrawTypeFlags::kSimpleShape);
    fCircularArc = makeFromStep(std::make_unique<CircularArcRenderStep>(bufferManager), DrawTypeFlags::kSimpleShape);
    fAnalyticBlur = makeFromStep(std::make_unique<AnalyticBlurRenderStep>(), DrawTypeFlags::kSimpleShape);
    ...
    for (PrimitiveType primType : { PrimitiveType::kTriangles, PrimitiveType::kTriangleStrip }) {
        for (bool color : { false, true }) {
            for (bool texCoords : { false, true }) {
                ...
                fVertices[index] = makeFromStep(std::make_unique<VerticesRenderStep>(primType, color, texCoords), DrawTypeFlags::kDrawVertices);
            }
        }
    }
    ...
    for (bool evenOdd : { false, true }) {
        ...
        auto stencilCurve = std::make_unique<TessellateCurvesRenderStep>(evenOdd, infinitySupport, bufferManager);
        auto stencilWedge = evenOdd ? std::make_unique<TessellateWedgesRenderStep>("evenodd", infinitySupport, kEvenOddStencilPass, bufferManager)
                                    : std::make_unique<TessellateWedgesRenderStep>("winding", infinitySupport, kWindingStencilPass, bufferManager);

        for (bool inverse : { false, true }) {
            static const char* kTessVariants[4] = { "[winding]", "[evenodd]", "[inverse-winding]", "[inverse-evenodd]" };
            ...
            fStencilTessellatedCurves[index]
                = Renderer("StencilTessellatedCurvesAndTris" + variant, DrawTypeFlags::kNonSimpleShape, stencilFan.get(), stencilCurve.get(), coverStep);

            fStencilTessellatedWedges[index] = Renderer("StencilTessellatedWedges" + variant, DrawTypeFlags::kNonSimpleShape, stencilWedge.get(), coverStep);
        }
