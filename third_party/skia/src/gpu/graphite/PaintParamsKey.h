...
public:
    ...
    {
        return PaintParamsKey(SkSpan<const uint32_t>());
    }
    bool isValid() const
    {
        return !fData.empty();
    ...
    {
        return fData.size() == that.fData.size() && !memcmp(fData.data(), that.fData.data(), fData.size());
    }
    bool operator!=(const PaintParamsKey& that) const
    {
        return !(*this == that);
...
private:
    ...
    // nodes created.
    const ShaderNode* createNode(const ShaderCodeDictionary*, int* currentIndex, SkArenaAlloc* arena) const;
...
public:
    ...
    {
        this->beginBlock(static_cast<int32_t>(id));
    }
    void beginBlock(int32_t codeSnippetID)
    {
        SkASSERT(!fLocked);
        SkDEBUGCODE(this->pushStack(codeSnippetID);) fData.push_back(codeSnippetID);
    }
...
private:
    ...
    PaintParamsKey lockAsKey()
    {
        ...
        SkDEBUGCODE(fLocked = true;) return PaintParamsKey({ fData.data(), fData.size() });
    }
    ...
    void unlock()
    {
        ...
        SkDEBUGCODE(fLocked = false;) SkDEBUGCODE(fStack.clear();) SkDEBUGCODE(this->checkReset();)
    }
