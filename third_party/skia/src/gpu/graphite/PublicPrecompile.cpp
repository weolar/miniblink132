...
void compile(const RendererProvider* rendererProvider, ResourceProvider* resourceProvider, const KeyContext& keyContext, UniquePaintParamsID uniqueID,
    DrawTypeFlags drawTypes, const RenderPassDesc& renderPassDesc, bool withPrimitiveBlender, Coverage coverage)
{
    for (const Renderer* r : rendererProvider->renderers()) {
        ...
        for (auto&& s : r->steps()) {
            ...
            sk_sp<GraphicsPipeline> pipeline = resourceProvider->findOrCreateGraphicsPipeline(
                keyContext.rtEffectDict(), pipelineDesc, renderPassDesc, PipelineCreationFlags::kForPrecompilation);
            if (!pipeline) {
...
bool Precompile(Context* context, RuntimeEffectDictionary* rteDict, const GraphicsPipelineDesc& pipelineDesc, const RenderPassDesc& renderPassDesc)
{
    ...
    sk_sp<GraphicsPipeline> pipeline
        = resourceProvider->findOrCreateGraphicsPipeline(rteDict, pipelineDesc, renderPassDesc, PipelineCreationFlags::kForPrecompilation);
    if (!pipeline) {
        SKGPU_LOG_W("Failed to create GraphicsPipeline in precompile!");
        return false;
    }
...
void Precompile(Context* context, const PaintOptions& options, DrawTypeFlags drawTypes, SkSpan<const RenderPassProperties> renderPassProperties)
{
    ...
    std::unique_ptr<ResourceProvider> tmpResourceProvider = sharedContext->makeResourceProvider(&singleOwner, SK_InvalidGenID, kDefaultBudgetInBytes,
    ...
    for (const RenderPassProperties& rpp : renderPassProperties) {
        ...
        TextureInfo info = caps->getDefaultSampledTextureInfo(rpp.fDstCT, Mipmapped::kNo, Protected::kNo, Renderable::kYes);
        ...
        int numLoadOps = 1;
        if (rpp.fRequiresMSAA && !caps->msaaRenderToSingleSampledSupport() && caps->loadOpAffectsMSAAPipelines()) {
            numLoadOps = 2;
        }

        for (int loadOpIndex = 0; loadOpIndex < numLoadOps; ++loadOpIndex) {
            const RenderPassDesc renderPassDesc = RenderPassDesc::Make(caps, info, kLoadOps[loadOpIndex], StoreOp::kStore, rpp.fDSFlags,
                /* clearColor= */ { .0f, .0f, .0f, .0f }, rpp.fRequiresMSAA, writeSwizzle);
            ...
            for (Coverage coverage : { Coverage::kNone, Coverage::kSingleChannel }) {
                PrecompileCombinations(context->priv().rendererProvider(), tmpResourceProvider.get(), options, keyContext,
                    static_cast<DrawTypeFlags>(drawTypes
                        & ~(DrawTypeFlags::kBitmapText_Color | DrawTypeFlags::kBitmapText_LCD | DrawTypeFlags::kSDFText_LCD | DrawTypeFlags::kDrawVertices)),
                    /* withPrimitiveBlender= */ false, coverage, renderPassDesc);
            }
            ...
            if (drawTypes & DrawTypeFlags::kBitmapText_Color) {
                ...
                PrecompileCombinations(context->priv().rendererProvider(), tmpResourceProvider.get(), tmp, keyContext, DrawTypeFlags::kBitmapText_Color,
                    /* withPrimitiveBlender= */ true, Coverage::kNone, renderPassDesc);
            }
            ...
            if (drawTypes & (DrawTypeFlags::kBitmapText_LCD | DrawTypeFlags::kSDFText_LCD)) {
                ...
                PrecompileCombinations(context->priv().rendererProvider(), tmpResourceProvider.get(), options, keyContext,
                    static_cast<DrawTypeFlags>(drawTypes & (DrawTypeFlags::kBitmapText_LCD | DrawTypeFlags::kSDFText_LCD)),
                    /* withPrimitiveBlender= */ false, Coverage::kLCD, renderPassDesc);
            }
            ...
            if (drawTypes & DrawTypeFlags::kDrawVertices) {
                ...
                for (bool withPrimitiveBlender : { true, false }) {
                    PrecompileCombinations(context->priv().rendererProvider(), tmpResourceProvider.get(), options, keyContext, DrawTypeFlags::kDrawVertices,
                        withPrimitiveBlender, Coverage::kNone, renderPassDesc);
                }
            }
        }
    }
...
void PrecompileCombinations(const RendererProvider* rendererProvider, ResourceProvider* resourceProvider, const PaintOptions& options,
    const KeyContext& keyContext, DrawTypeFlags drawTypes, bool withPrimitiveBlender, Coverage coverage, const RenderPassDesc& renderPassDescIn)
{
    ...
    options.priv().buildCombinations(keyContext, &gatherer, drawTypes, withPrimitiveBlender, coverage, renderPassDescIn,
        [rendererProvider, resourceProvider, &keyContext](
            UniquePaintParamsID uniqueID, DrawTypeFlags drawTypes, bool withPrimitiveBlender, Coverage coverage, const RenderPassDesc& renderPassDesc) {
            compile(rendererProvider, resourceProvider, keyContext, uniqueID, drawTypes, renderPassDesc, withPrimitiveBlender, coverage);
        });
}
