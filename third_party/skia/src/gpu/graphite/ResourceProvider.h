...
public:
    ...
    {
        return fResourceCache->proxyCache();
    }

    size_t getResourceCacheLimit() const
    {
        return fResourceCache->getMaxBudget();
    }
    ...
    {
        return fSharedContext;
    }
#endif

#ifdef SK_BUILD_FOR_ANDROID
    virtual BackendTexture createBackendTexture(AHardwareBuffer*, bool isRenderable, bool isProtectedContent, SkISize dimensions, bool fromAndroidWindow) const;
#endif
...
private:
    virtual sk_sp<GraphicsPipeline> createGraphicsPipeline(
        const RuntimeEffectDictionary*, const GraphicsPipelineDesc&, const RenderPassDesc&, SkEnumBitMask<PipelineCreationFlags>)
        = 0;
    ...
    sk_sp<Texture> findOrCreateTextureWithKey(
        SkISize dimensions, const TextureInfo& info, const GraphiteResourceKey& key, std::string_view label, skgpu::Budgeted);
...
#ifdef SK_BUILD_FOR_ANDROID
    virtual BackendTexture onCreateBackendTexture(
