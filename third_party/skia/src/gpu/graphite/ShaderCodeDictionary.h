...
struct ShaderSnippet {
    ...
    ShaderSnippet(const char* name, const char* staticFn, SkEnumBitMask<SnippetRequirementFlags> snippetRequirementFlags, SkSpan<const Uniform> uniforms,
        SkSpan<const TextureAndSampler> texturesAndSamplers = {}, GeneratePreambleForSnippetFn preambleGenerator = nullptr, int numChildren = 0)
        : fName(name)
...
public:
        ...
        , fData(data)
    {
        ...
        const bool isCompose = codeID == (int)BuiltInCodeSnippetID::kCompose || codeID == (int)BuiltInCodeSnippetID::kBlendCompose;
        for (const ShaderNode* child : children) {
            ...
            if (codeID >= kBuiltInCodeSnippetIDCount || (isCompose && child == children.back())) {
                ...
                mask = SnippetRequirementFlags::kLocalCoords | SnippetRequirementFlags::kPriorStageOutput | SnippetRequirementFlags::kBlenderDstColor;
            }
    ...
    std::string invokeAndAssign(const ShaderInfo& shaderInfo, const ShaderSnippet::Args& args, std::string* funcBody) const;
    ...
    {
        return fRequiredFlags;
    }

    int numChildren() const
    {
        return fEntry->fNumChildren;
    }
...
private:
    ...
    using PaintIDMap = skia_private::THashMap<PaintParamsKey, UniquePaintParamsID, PaintParamsKey::Hash>;
