...
{
    SkASSERT(backend == BackendApi::kDawn || backend == BackendApi::kMetal || backend == BackendApi::kVulkan);
}
...
bool TextureInfo::operator==(const TextureInfo& that) const
{
    ...
    if (fSampleCount != that.fSampleCount || fMipmapped != that.fMipmapped || fProtected != that.fProtected) {
        return false;
    }
    assert_is_supported_backend(fBackend);
...
SkString TextureInfo::toString() const
{
    ...
    ret.appendf("bytesPerPixel=%zu,sampleCount=%u,mipmapped=%d,protected=%d)", this->bytesPerPixel(), fSampleCount, static_cast<int>(fMipmapped),
