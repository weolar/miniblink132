...
vello_cpp::Point to_vello_point(const SkPoint& p)
{
    return { p.x(), p.y() };
}
    ...
    bool next_element(vello_cpp::PathElement* outElem) override
    {
        ...
        case SkPathVerb::kConic:
            ...
            SkASSERT(fConicQuadIdx >= fConicConverter.countQuads()); // No other conic->quad
                ...
                // in progress
            fConicQuads = fConicConverter.computeQuads(points, *weights, 0.25 / fTransform.maxScaleFactor());
            outElem->verb = vello_cpp::PathVerb::QuadTo;
...
VelloScene::VelloScene()
    : fEncoding(vello_cpp::new_encoding())
{
}
...
void VelloScene::solidFill(const SkPath& shape, const SkColor4f& fillColor, const SkPathFillType fillType, const Transform& t)
{
    PathIter iter(shape, t);
    fEncoding->fill(to_fill_type(fillType), to_vello_affine(t), { vello_cpp::BrushKind::Solid, { to_vello_color(fillColor) } }, iter);
}

void VelloScene::solidStroke(const SkPath& shape, const SkColor4f& fillColor, const SkStrokeRec& style, const Transform& t)
{
...
std::unique_ptr<DispatchGroup> VelloRenderer::renderScene(
    const RenderParams& params, const VelloScene& scene, sk_sp<TextureProxy> target, Recorder* recorder) const
{
    ...
    auto config = scene.fEncoding->prepare_render(std::min(params.fWidth, static_cast<uint32_t>(dims.fWidth)),
        std::min(params.fHeight, static_cast<uint32_t>(dims.fHeight)), to_vello_color(params.fBaseColor));
    ...
    ScratchBuffer tagmonoids = bufMgr->getScratchStorage(bufferSizes.path_monoids);
    {
        ...
        builder.assignSharedBuffer(new_scratch_slice(pathtagReduceOutput), kVelloSlot_PathtagReduceOutput);
        ...
        if (dispatchInfo.use_large_path_scan) {
            ...
            builder.assignSharedBuffer(new_scratch_slice(reduced2), kVelloSlot_LargePathtagReduceSecondPassOutput);
            builder.assignSharedBuffer(new_scratch_slice(reducedScan), kVelloSlot_LargePathtagScanFirstPassOutput);
