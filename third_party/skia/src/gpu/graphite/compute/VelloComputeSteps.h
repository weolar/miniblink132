...
skia_private::TArray<ComputeStep::WorkgroupBufferDesc> VelloWorkgroupBuffers(vello_cpp::ShaderStage);
ComputeStep::NativeShaderSource VelloNativeShaderSource(vello_cpp::ShaderStage, ComputeStep::NativeShaderFormat);

template <vello_cpp::ShaderStage S> class VelloStep : public ComputeStep {
public:
...
protected:
    explicit VelloStep(SkSpan<const ResourceDesc> resources)
        : ComputeStep(VelloStageName(S), VelloStageLocalSize(S), resources, AsSpan<ComputeStep::WorkgroupBufferDesc>(VelloWorkgroupBuffers(S)),
...
private:
    ...
    //
    template <typename T, typename C> static SkSpan<const T> AsSpan(C&& container)
    {
...
template <vello_cpp::ShaderStage S, SkColorType T, ::rust::Vec<uint8_t> (*MaskLutBuilder)()> class VelloFineMsaaStepBase : public VelloFineStepBase<S, T> {
public:
...
protected:
    explicit VelloFineMsaaStepBase(SkSpan<const ComputeStep::ResourceDesc> resources)
        : VelloFineStepBase<S, T>(resources)
        , fMaskLut(MaskLutBuilder())
    {
    }
...
class VelloFineAreaStep final : public VelloFineStepBase<vello_cpp::ShaderStage::FineArea, kRGBA_8888_SkColorType> {
public:
    VelloFineAreaStep();
};

class VelloFineAreaAlpha8Step final : public VelloFineStepBase<vello_cpp::ShaderStage::FineAreaR8, kAlpha_8_SkColorType> {
public:
    VelloFineAreaAlpha8Step();
};

class VelloFineMsaa16Step final : public VelloFineMsaaStepBase<vello_cpp::ShaderStage::FineMsaa16, kRGBA_8888_SkColorType, vello_cpp::build_mask_lut_16> {
public:
    VelloFineMsaa16Step();
};

class VelloFineMsaa16Alpha8Step final : public VelloFineMsaaStepBase<vello_cpp::ShaderStage::FineMsaa16R8, kAlpha_8_SkColorType, vello_cpp::build_mask_lut_16> {
public:
    VelloFineMsaa16Alpha8Step();
};

class VelloFineMsaa8Step final : public VelloFineMsaaStepBase<vello_cpp::ShaderStage::FineMsaa8, kRGBA_8888_SkColorType, vello_cpp::build_mask_lut_8> {
public:
    VelloFineMsaa8Step();
};

class VelloFineMsaa8Alpha8Step final : public VelloFineMsaaStepBase<vello_cpp::ShaderStage::FineMsaa8R8, kAlpha_8_SkColorType, vello_cpp::build_mask_lut_8> {
public:
