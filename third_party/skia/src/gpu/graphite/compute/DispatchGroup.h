...
using DispatchResourceOptional = std::variant<std::monostate, BindBufferInfo, TextureIndex, SamplerIndex>;
...
public:
    ...
    void assignSharedBuffer(BindBufferInfo buffer, unsigned int slot, ClearBuffer cleared = ClearBuffer::kNo);
...
private:
    ...
    DispatchResourceOptional allocateResource(const ComputeStep* step, const ComputeStep::ResourceDesc& resource, int resourceIdx);
