...
{
    return this->appendStepInternal(step, globalSize ? *globalSize : step->calculateGlobalDispatchSize());
}
...
bool Builder::appendStepInternal(const ComputeStep* step, const std::variant<WorkgroupSize, BindBufferInfo>& globalSizeOrIndirect)
{
    SkASSERT(fObj);
    SkASSERT(step);
        ...
        case DataFlow::kShared: {
            ...
            if (std::holds_alternative<std::monostate>(*slot)) {
                ...
                *slot = maybeResource;
                ...
                SkASSERT(((r.fType == Type::kUniformBuffer || r.fType == Type::kStorageBuffer || r.fType == Type::kReadOnlyStorageBuffer
                              || r.fType == Type::kIndirectBuffer)
                             && std::holds_alternative<BindBufferInfo>(*slot))
                    || ((r.fType == Type::kReadOnlyTexture || r.fType == Type::kSampledTexture || r.fType == Type::kWriteOnlyStorageTexture)
                ...
                if (texIdx && r.fType == Type::kWriteOnlyStorageTexture) {
                    ...
                    SkASSERT(t->textureInfo().isCompatible(fRecorder->priv().caps()->getDefaultStorageTextureInfo(colorType)));
                }
#endif // SK_DEBUG
                ...
                if (r.fType == Type::kSampledTexture) {
                    ...
                    const SamplerIndex* samplerIdx = std::get_if<SamplerIndex>(&samplerResource);
                    SkASSERT(samplerIdx);
                    int bindingIndex = distinctRanges ? texIndex : separateSampler ? bufferOrGlobalIndex++ : bufferOrGlobalIndex;
                    dispatch.fBindings.push_back({ static_cast<BindingIndex>(bindingIndex), *samplerIdx });
                }
            }
            break;
        }
    ...
    if (fObj->fPipelineDescs.empty() || fObj->fPipelineDescs.back().uniqueID() != step->uniqueID()) {
...
BindBufferInfo Builder::getSharedBufferResource(unsigned int slot) const
{
    ...
    BindBufferInfo info;
    if (const BindBufferInfo* slotValue = std::get_if<BindBufferInfo>(&fOutputTable.fSharedSlots[slot])) {
        info = *slotValue;
    }
    return info;
}
...
DispatchResourceOptional Builder::allocateResource(const ComputeStep* step, const ComputeStep::ResourceDesc& resource, int resourceIdx)
{
    SkASSERT(step);
    SkASSERT(fObj);
    using Type = ComputeStep::ResourceType;
    ...
    case Type::kStorageBuffer: {
        ...
        if (resource.fPolicy == ResourcePolicy::kMapped) {
            ...
            auto bufInfo = bufferMgr->getStorage(bufferSize, resource.fPolicy == ResourcePolicy::kClear ? ClearBuffer::kYes : ClearBuffer::kNo);
            if (bufInfo) {
                result = bufInfo;
            }
        }
    ...
    case Type::kIndirectBuffer: {
        ...
        size_t bufferSize = step->calculateBufferSize(resourceIdx, resource);
        SkASSERT(bufferSize);
        auto bufInfo = bufferMgr->getIndirectStorage(bufferSize, resource.fPolicy == ResourcePolicy::kClear ? ClearBuffer::kYes : ClearBuffer::kNo);
        if (bufInfo) {
            result = bufInfo;
        }
        break;
    ...
    case Type::kWriteOnlyStorageTexture: {
        ...
        sk_sp<TextureProxy> texture = TextureProxy::Make(
            fRecorder->priv().caps(), fRecorder->priv().resourceProvider(), size, textureInfo, "DispatchWriteOnlyStorageTexture", skgpu::Budgeted::kYes);
        if (texture) {
            fObj->fTextures.push_back(std::move(texture));
