...
ScratchResourceManager::ScratchResourceManager(ResourceProvider* resourceProvider, std::unique_ptr<ProxyReadCountMap> proxyCounts)
...
sk_sp<Texture> ScratchResourceManager::getScratchTexture(SkISize dimensions, const TextureInfo& info, std::string_view label)
{
    for (ScratchTexture& st : fScratchTextures) {
        if (st.fAvailable && st.fTexture->dimensions() == dimensions && st.fTexture->textureInfo() == info) {
    ...
    sk_sp<Texture> newScratchTexture = fResourceProvider->findOrCreateScratchTexture(dimensions, info, std::move(label), Budgeted::kYes);
