...
public:
    ...
    // contiguous data.
    template <typename VertexCountProxy> class DynamicInstances;
...
private:
    ...
    void setTemplate(BindBufferInfo vertices, BindBufferInfo indices, BindBufferInfo instances, int templateCount);
    // NOTE: bindAndFlush's templateCount is unsigned because dynamic index count instancing
    // isn't applicable.
    void bindAndFlush(BindBufferInfo vertices, BindBufferInfo indices, BindBufferInfo instances, unsigned int templateCount, unsigned int drawCount)
    {
    ...
    void reserve(unsigned int count)
    {
        ...
        if (writer) {
            ...
            if (reservedChunk.fBuffer != fTarget.fBuffer
                || reservedChunk.fOffset != (fTarget.fOffset + (fDrawer.fPendingBase + fDrawer.fPendingCount) * fStride)) {
    ...
    VertexWriter append(unsigned int count)
    {
        ...
        if (!fNextWriter)
            SK_UNLIKELY
            {
                ...
                return VertexWriter(fDrawer.fFailureStorage.reset(size, SkAutoMalloc::kReuse_OnShrink), SkTo<size_t>(size));
            }
...
template <typename VertexCountProxy> class DrawWriter::DynamicInstances : private DrawWriter::Appender {
public:
    ...
    template <typename V> VertexWriter append(const V& vertexCount, unsigned int instanceCount)
    {
