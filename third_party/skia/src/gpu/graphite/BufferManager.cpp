...
uint32_t validate_count_and_stride(size_t count, size_t stride)
{
    ...
    if (count64 > std::numeric_limits<uint32_t>::max() || stride64 > std::numeric_limits<uint32_t>::max() || bytes64 > std::numeric_limits<uint32_t>::max()) {
...
bool can_fit(uint32_t requestedSize, uint32_t allocatedSize, uint32_t currentOffset, uint32_t alignment)
{
...
uint32_t starting_alignment(BufferType type, bool useTransferBuffers, const Caps* caps)
{
    ...
    } else if (type == BufferType::kStorage || type == BufferType::kVertexStorage || type == BufferType::kIndexStorage || type == BufferType::kIndirect) {
...
ScratchBuffer::ScratchBuffer(uint32_t size, uint32_t alignment, sk_sp<Buffer> buffer, DrawBufferManager* owner)
    : fSize(size)
...
DrawBufferManager::DrawBufferManager(ResourceProvider* resourceProvider, const Caps* caps, UploadBufferManager* uploadManager)
    : fResourceProvider(resourceProvider)
    , fCaps(caps)
    , fUploadManager(uploadManager)
    , fCurrentBuffers { { { BufferType::kVertex, kVertexBufferMinSize, kVertexBufferMaxSize, caps },
          { BufferType::kIndex, kIndexBufferSize, kIndexBufferSize, caps }, { BufferType::kUniform, kUniformBufferSize, kUniformBufferSize, caps },
...
// the same in memory.
DrawBufferManager::BufferInfo::BufferInfo(BufferType type, uint32_t minBlockSize, uint32_t maxBlockSize, const Caps* caps)
    : fType(type)
...
std::pair<VertexWriter, BindBufferInfo> DrawBufferManager::getVertexWriter(size_t count, size_t stride)
{
...
std::pair<IndexWriter, BindBufferInfo> DrawBufferManager::getIndexWriter(size_t count, size_t stride)
{
    uint32_t requiredBytes = validate_count_and_stride(count, stride);
    if (!requiredBytes) {
        return {};
...
std::pair<UniformWriter, BindBufferInfo> DrawBufferManager::getUniformWriter(size_t count, size_t stride)
{
    uint32_t requiredBytes = validate_count_and_stride(count, stride);
    if (!requiredBytes) {
        return {};
...
std::pair<UniformWriter, BindBufferInfo> DrawBufferManager::getSsboWriter(size_t count, size_t stride, size_t alignment)
{
    uint32_t requiredBytes = validate_count_and_stride(count, stride);
    if (!requiredBytes) {
        return {};
    ...
    auto [ptr, bindInfo] = this->prepareMappedBindBuffer(&info, "StorageBuffer", requiredBytes, alignment);
    return { UniformWriter(ptr, requiredBytes), bindInfo };
}

std::pair<UniformWriter, BindBufferInfo> DrawBufferManager::getSsboWriter(size_t count, size_t stride)
{
...
std::pair<UniformWriter, BindBufferInfo> DrawBufferManager::getAlignedSsboWriter(size_t count, size_t stride)
{
    // Align to the provided element stride.
    return this->getSsboWriter(count, stride, stride);
}

std::pair<void* /*mappedPtr*/, BindBufferInfo> DrawBufferManager::getUniformPointer(size_t requiredBytes)
{
...
std::pair<void* /*mappedPtr*/, BindBufferInfo> DrawBufferManager::getStoragePointer(size_t requiredBytes)
{
    uint32_t requiredBytes32 = validate_size(requiredBytes);
    if (!requiredBytes32) {
        return {};
...
BindBufferInfo DrawBufferManager::getStorage(size_t requiredBytes, ClearBuffer cleared)
{
    ...
    return this->prepareBindBuffer(&info, "StorageBuffer", requiredBytes32,
        /*requiredAlignment=*/0,
        /*supportCpuUpload=*/false, cleared);
}
...
BindBufferInfo DrawBufferManager::getIndirectStorage(size_t requiredBytes, ClearBuffer cleared)
{
    ...
    return this->prepareBindBuffer(&info, "IndirectStorageBuffer", requiredBytes32,
        /*requiredAlignment=*/0,
        /*supportCpuUpload=*/false, cleared);
}
...
ScratchBuffer DrawBufferManager::getScratchStorage(size_t requiredBytes)
{
    ...
    if (!buffer) {
        buffer = fResourceProvider->findOrCreateBuffer(bufferSize, BufferType::kStorage, AccessPattern::kGpuOnly, "ScratchStorageBuffer");
...
bool DrawBufferManager::transferToRecording(Recording* recording)
{
    if (fMappingFailed) {
        ...
        SkASSERT(fUsedBuffers.empty() && fClearList.empty() && fReusableScratchStorageBuffers.empty());
    ...
    for (auto& [buffer, transferBuffer] : fUsedBuffers) {
        if (transferBuffer) {
            ...
            recording->priv().taskList()->add(CopyBufferToBufferTask::Make(transferBuffer.fBuffer, transferBuffer.fOffset, std::move(buffer),
                /*dstOffset=*/0, copySize));
        } else {
            if (buffer->isMapped()) {
                buffer->unmap();
            }
    ...
    for (auto& info : fCurrentBuffers) {
        ...
        if (info.fTransferBuffer) {
            ...
            // Since the transfer buffer is managed by the UploadManager, we don't manually unmap
            // it here or need to pass a ref into CopyBufferToBufferTask.
            recording->priv().taskList()->add(CopyBufferToBufferTask::Make(info.fTransferBuffer.fBuffer, info.fTransferBuffer.fOffset, info.fBuffer,
                /*dstOffset=*/0, info.fBuffer->size()));
        } else {
...
std::pair<void*, BindBufferInfo> DrawBufferManager::prepareMappedBindBuffer(
    BufferInfo* info, std::string_view label, uint32_t requiredBytes, uint32_t requiredAlignment)
{
    BindBufferInfo bindInfo = this->prepareBindBuffer(info, std::move(label), requiredBytes, requiredAlignment,
...
BindBufferInfo DrawBufferManager::prepareBindBuffer(
    BufferInfo* info, std::string_view label, uint32_t requiredBytes, uint32_t requiredAlignment, bool supportCpuUpload, ClearBuffer cleared)
{
    SkASSERT(info);
    ...
    const bool overflowedBuffer = info->fBuffer
        && (info->fOffset >= SkTo<uint32_t>(info->fBuffer->size())
            || !can_fit(requiredBytes, SkTo<uint32_t>(info->fBuffer->size()), info->fOffset, requiredAlignment));
    if (overflowedBuffer) {
    ...
    if (!info->fBuffer) {
        ...
        const uint32_t blockSize = overflowedBuffer ? std::max(info->fCurBlockSize / 4, info->fMinBlockSize) : info->fCurBlockSize;
        ...
        AccessPattern accessPattern = (useTransferBuffer || !supportCpuUpload) ? AccessPattern::kGpuOnly : AccessPattern::kHostVisible;

        info->fBuffer = fResourceProvider->findOrCreateBuffer(bufferSize, info->fType, accessPattern, std::move(label));
        info->fOffset = 0;
        if (!info->fBuffer) {
            this->onFailedBuffer();
            return {};
    ...
    if (useTransferBuffer && !info->fTransferBuffer) {
        std::tie(info->fTransferMapPtr, info->fTransferBuffer)
            = fUploadManager->makeBindInfo(info->fBuffer->size(), fCaps->requiredTransferBufferAlignment(), "TransferForDataBuffer");
...
StaticBufferManager::StaticBufferManager(ResourceProvider* resourceProvider, const Caps* caps)
    : fResourceProvider(resourceProvider)
...
void* StaticBufferManager::prepareStaticData(BufferInfo* info, size_t size, BindBufferInfo* target)
{
    ...
    *target = { nullptr, 0 };
    ...
    auto [transferMapPtr, transferBindInfo] = fUploadManager.makeBindInfo(size32, fRequiredTransferAlignment, "TransferForStaticBuffer");
    if (!transferMapPtr) {
...
bool StaticBufferManager::BufferInfo::createAndUpdateBindings(
    ResourceProvider* resourceProvider, Context* context, QueueManager* queueManager, GlobalCache* globalCache, std::string_view label) const
{
    if (!fTotalRequiredBytes) {
        return true; // No buffer needed
    }

    sk_sp<Buffer> staticBuffer = resourceProvider->findOrCreateBuffer(fTotalRequiredBytes, fBufferType, AccessPattern::kGpuOnly, std::move(label));
    if (!staticBuffer) {
    ...
    for (const CopyRange& data : fData) {
        ...
        auto copyTask = CopyBufferToBufferTask::Make(data.fSource.fBuffer, data.fSource.fOffset, sk_ref_sp(data.fTarget->fBuffer), data.fTarget->fOffset, size);
...
StaticBufferManager::FinishResult StaticBufferManager::finalize(Context* context, QueueManager* queueManager, GlobalCache* globalCache)
{
    if (fMappingFailed) {
        return FinishResult::kFailure;
    }

    const size_t totalRequiredBytes = fVertexBufferInfo.fTotalRequiredBytes + fIndexBufferInfo.fTotalRequiredBytes;
    SkASSERT(totalRequiredBytes <= kMaxStaticDataSize);
    if (!totalRequiredBytes) {
        return FinishResult::kNoWork;
    }

    if (!fVertexBufferInfo.createAndUpdateBindings(fResourceProvider, context, queueManager, globalCache, "StaticVertexBuffer")) {
        return FinishResult::kFailure;
    }
    if (!fIndexBufferInfo.createAndUpdateBindings(fResourceProvider, context, queueManager, globalCache, "StaticIndexBuffer")) {
        return FinishResult::kFailure;
    }
