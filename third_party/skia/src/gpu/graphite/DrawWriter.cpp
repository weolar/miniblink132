...
void DrawWriter::setTemplate(BindBufferInfo vertices, BindBufferInfo indices, BindBufferInfo instances, int templateCount)
{
    if (vertices != fVertices || instances != fInstances || fIndices != indices) {
        ...
        if (willAppendVertices != isAppendingVertices || (isAppendingVertices && fVertices != vertices) || (!isAppendingVertices && fInstances != instances)) {
...
void DrawWriter::flush()
{
    ...
    if (fTemplateCount) {
        ...
        if (fIndices) {
            ...
            fCommandList->drawIndexedInstanced(fPrimitiveType, 0, realVertexCount, 0, fPendingBase, fPendingCount);
        } else {
            SkASSERT(realVertexCount * fVertexStride <= fVertices.fSize);
            fCommandList->drawInstanced(fPrimitiveType, 0, realVertexCount, fPendingBase, fPendingCount);
        }
    } else {
        SkASSERT(!fInstances);
        if (fIndices) {
