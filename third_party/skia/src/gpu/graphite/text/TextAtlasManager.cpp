...
TextAtlasManager::TextAtlasManager(Recorder* recorder)
    ...
    , fAtlasConfig { recorder->priv().caps()->maxTextureSize(), recorder->priv().caps()->glyphCacheTextureMaximumBytes() }
{
    if (!recorder->priv().caps()->allowMultipleAtlasTextures() ||
        // multitexturing supported only if range can represent the index + texcoords fully
        !(recorder->priv().caps()->shaderCaps()->fFloatIs32Bits || recorder->priv().caps()->shaderCaps()->fIntegerSupport)) {
    ...
    if (maskFormat == expectedMaskFormat) {
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                if (kBGRAIsNative) {
                    color8888 = masks.getBlue(color565) | (masks.getGreen(color565) << 8) | (masks.getRed(color565) << 16) | (0xFF << 24);
                } else {
                    color8888 = masks.getRed(color565) | (masks.getGreen(color565) << 8) | (masks.getBlue(color565) << 16) | (0xFF << 24);
                }
...
MaskFormat TextAtlasManager::resolveMaskFormat(MaskFormat format) const
{
    if (MaskFormat::kA565 == format
        && !fRecorder->priv()
                .caps()
                ->getDefaultSampledTextureInfo(kRGB_565_SkColorType,
                    /*mipmapped=*/Mipmapped::kNo, Protected::kNo, Renderable::kNo)
                .isValid()) {
...
DrawAtlas::ErrorCode TextAtlasManager::addGlyphToAtlas(const SkGlyph& skGlyph, Glyph* glyph, int srcPadding)
{
    ...
    auto errorCode = atlas->addToAtlas(fRecorder, width, height, storage.get(), &glyph->fAtlasLocator);
...
void TextAtlasManager::addGlyphToBulkAndSetUseToken(BulkUsePlotUpdater* updater, MaskFormat format, Glyph* glyph, AtlasToken token)
{
    SkASSERT(glyph);
...
bool TextAtlasManager::initAtlas(MaskFormat format)
{
    ...
    if (fAtlases[index] == nullptr) {
        ...
        fAtlases[index] = DrawAtlas::Make(colorType, SkColorTypeBytesPerPixel(colorType), atlasDimensions.width(), atlasDimensions.height(),
            plotDimensions.width(), plotDimensions.height(),
            /*generationCounter=*/this, fAllowMultitexturing, DrawAtlas::UseStorageTextures::kNo,
            /*evictor=*/nullptr,
            /*label=*/"TextAtlas");
        if (!fAtlases[index]) {
            return false;
...
std::tuple<bool, int> GlyphVector::regenerateAtlasForGraphite(
    int begin, int end, skgpu::MaskFormat maskFormat, int srcPadding, skgpu::graphite::Recorder* recorder)
{
    ...
    const sk_sp<skgpu::graphite::TextureProxy>* proxies = atlasManager->getProxies(maskFormat, &numActiveProxies);
    if (!proxies) {
    ...
    if (fAtlasGeneration != currentAtlasGen) {
        ...
        for (const Variant& variant : glyphs) {
            ...
            atlasManager->addGlyphToBulkAndSetUseToken(&fBulkUseUpdater, maskFormat, gpuGlyph, tokenTracker->nextFlushToken());
            glyphsPlacedInAtlas++;
        }
        ...
        if (end == SkCount(fGlyphs)) {
            ...
            atlasManager->setUseTokenBulk(fBulkUseUpdater, tokenTracker->nextFlushToken(), maskFormat);
        }
