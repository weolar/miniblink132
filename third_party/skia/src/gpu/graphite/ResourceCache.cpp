...
sk_sp<ResourceCache> ResourceCache::Make(SingleOwner* singleOwner, uint32_t recorderID, size_t maxBytes)
{
...
Resource* ResourceCache::findAndRefResource(const GraphiteResourceKey& key, skgpu::Budgeted budgeted)
{
    ASSERT_SINGLE_OWNER
...
bool ResourceCache::returnResource(Resource* resource, LastRemovedRef removedRef)
{
    ...
    if (resource->shouldDeleteASAP() == Resource::DeleteASAP::kNo && resource->key().shareable() == Shareable::kNo && removedRef == LastRemovedRef::kUsage) {
...
void ResourceCache::returnResourceToCache(Resource* resource, LastRemovedRef removedRef)
{
    ...
    if (removedRef == LastRemovedRef::kUsage) {
        if (resource->key().shareable() == Shareable::kYes) {
            ...
            SkDEBUGCODE(resource->fNonShareableInCache = true;) resource->setLabel("Scratch");
            fResourceMap.insert(resource->key(), resource);
...
void ResourceCache::purgeResource(Resource* resource)
{
    ...
    TRACE_EVENT_INSTANT1("skia.gpu.cache", TRACE_FUNC, TRACE_EVENT_SCOPE_THREAD, "size", resource->gpuMemorySize());
...
void ResourceCache::purgeResources(const StdSteadyClock::time_point* purgeTime)
{
    ...
    if (fPurgeableQueue.count() && purgeTime && fPurgeableQueue.peek()->lastAccessTime() >= *purgeTime) {
        return;
    }
...
void ResourceCache::visitTextures(const std::function<void(const Texture*, bool purgeable)>& func) const
{
    for (int i = 0; i < fNonpurgeableResources.size(); ++i) {
