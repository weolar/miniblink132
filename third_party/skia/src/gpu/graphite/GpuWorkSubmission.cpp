...
GpuWorkSubmission::GpuWorkSubmission(std::unique_ptr<CommandBuffer> cmdBuffer, QueueManager* queueManager)
    ...
    , fQueueManager(queueManager)
{
    ...
    if (!buffers.empty()) {
        ...
        for (auto& buffer : fCommandBuffer->buffersToAsyncMapOnSubmit()) {
            ...
            buffer->asyncMap([](GpuFinishedContext c, CallbackResult) { static_cast<SkRefCnt*>(c)->unref(); }, fOutstandingAsyncMapCounter.get());
        }
    }
}
...
{
    return this->onIsFinished(sharedContext) && (!fOutstandingAsyncMapCounter || fOutstandingAsyncMapCounter->unique());
}
