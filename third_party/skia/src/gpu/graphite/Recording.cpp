...
Recording::Recording(uint32_t uniqueID, uint32_t recorderID, std::unordered_set<sk_sp<TextureProxy>, ProxyHash>&& nonVolatileLazyProxies,
    std::unordered_set<sk_sp<TextureProxy>, ProxyHash>&& volatileLazyProxies, std::unique_ptr<LazyProxyData> targetProxyData,
...
{
    fTargetProxy = TextureProxy::MakeFullyLazy(textureInfo, skgpu::Budgeted::kNo, Volatile::kYes, [this](ResourceProvider*) {
...
bool Recording::LazyProxyData::lazyInstantiate(ResourceProvider* resourceProvider, sk_sp<Texture> texture)
{
...
bool RecordingPriv::addCommands(Context* context, CommandBuffer* commandBuffer, Surface* targetSurface, SkIVector targetTranslation, SkIRect targetClip)
{
    AutoDeinstantiateTextureProxy autoDeinstantiateTargetProxy(fRecording->fTargetProxyData ? fRecording->fTargetProxyData->lazyProxy() : nullptr);
    ...
    if (fRecording->fTargetProxyData) {
        ...
        if (!fRecording->fTargetProxyData->lazyInstantiate(resourceProvider, surfaceTexture->refTexture())) {
    ...
    if (fRecording->fRootTaskList->addCommands(context, commandBuffer, { replayTarget, targetTranslation, targetClip }) == Task::Status::kFail) {
        return false;
    }
    for (int i = 0; i < fRecording->fFinishedProcs.size(); ++i) {
