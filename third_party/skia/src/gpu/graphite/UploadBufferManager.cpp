...
UploadBufferManager::UploadBufferManager(ResourceProvider* resourceProvider, const Caps* caps)
...
std::tuple<TextureUploadWriter, BindBufferInfo> UploadBufferManager::getTextureUploadWriter(size_t requiredBytes, size_t requiredAlignment)
{
    auto [bufferMapPtr, bindInfo] = this->makeBindInfo(requiredBytes, requiredAlignment, "TextureUploadBuffer");
    if (!bufferMapPtr) {
...
std::tuple<void* /*mappedPtr*/, BindBufferInfo> UploadBufferManager::makeBindInfo(size_t requiredBytes, size_t requiredAlignment, std::string_view label)
{
    ...
    if (requiredBytes32 > kReusedBufferSize) {
        ...
        sk_sp<Buffer> buffer = fResourceProvider->findOrCreateBuffer(requiredBytes32, BufferType::kXferCpuToGpu, AccessPattern::kHostVisible, std::move(label));
    ...
    if (!fReusedBuffer) {
        fReusedBuffer = fResourceProvider->findOrCreateBuffer(kReusedBufferSize, BufferType::kXferCpuToGpu, AccessPattern::kHostVisible, std::move(label));
