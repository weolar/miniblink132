...
public:
    ...
    {
        return sk_sp(new DefaultImageProvider);
    }
...
Recorder::Recorder(sk_sp<SharedContext> sharedContext, const RecorderOptions& options, const Context* context)
    ...
    , fTextBlobCache(std::make_unique<sktext::gpu::TextBlobRedrawCoordinator>(fUniqueID))
{
    ...
    if (context) {
        ...
        fOwnedResourceProvider = fSharedContext->makeResourceProvider(this->singleOwner(), fUniqueID, options.fGpuBudgetInBytes,
            /* avoidBufferAlloc= */ false);
        fResourceProvider = fOwnedResourceProvider.get();
    }
    fUploadBufferManager = std::make_unique<UploadBufferManager>(fResourceProvider, fSharedContext->caps());
    fDrawBufferManager = std::make_unique<DrawBufferManager>(fResourceProvider, fSharedContext->caps(), fUploadBufferManager.get());
...
std::unique_ptr<Recording> Recorder::snap()
{
    ...
    std::unique_ptr<Recording> recording(new Recording(fNextRecordingID++, fUniqueID, std::move(nonVolatileLazyProxies), std::move(volatileLazyProxies),
        std::move(fTargetProxyData), std::move(fFinishedProcs)));
    ...
    valid &= recording->priv().taskList()->prepareResources(fResourceProvider, &scratchManager, fRuntimeEffectDict.get()) != Task::Status::kFail;
    if (!valid) {
        recording = nullptr;
...
SkCanvas* Recorder::makeDeferredCanvas(const SkImageInfo& imageInfo, const TextureInfo& textureInfo)
{
    ...
    fTargetProxyDevice = Device::Make(this, fTargetProxyData->refLazyProxy(), imageInfo.dimensions(), imageInfo.colorInfo(), {}, LoadOp::kLoad);
...
BackendTexture Recorder::createBackendTexture(
    AHardwareBuffer* hardwareBuffer, bool isRenderable, bool isProtectedContent, SkISize dimensions, bool fromAndroidWindow) const
{
    ...
    return fResourceProvider->createBackendTexture(hardwareBuffer, isRenderable, isProtectedContent, dimensions, fromAndroidWindow);
}

#endif // SK_BUILD_FOR_ANDROID

bool Recorder::updateBackendTexture(
    const BackendTexture& backendTex, const SkPixmap srcData[], int numLevels, GpuFinishedProc finishedProc, GpuFinishedContext finishedContext)
{
    ASSERT_SINGLE_OWNER
    ...
    if (backendTex.info().mipmapped() == Mipmapped::kYes) {
        numExpectedLevels = SkMipmap::ComputeLevelCount(backendTex.dimensions().width(), backendTex.dimensions().height()) + 1;
    }
    ...
    UploadInstance upload = UploadInstance::Make(
        this, std::move(proxy), colorInfo, colorInfo, mipLevels, SkIRect::MakeSize(backendTex.dimensions()), std::make_unique<ImageUploadContext>());
    if (!upload.isValid()) {
...
bool Recorder::updateCompressedBackendTexture(
    const BackendTexture& backendTex, const void* data, size_t dataSize, GpuFinishedProc finishedProc, GpuFinishedContext finishedContext)
{
    ASSERT_SINGLE_OWNER
    ...
    // Add UploadTask to Recorder
    UploadInstance upload = UploadInstance::MakeCompressed(this, std::move(proxy), data, dataSize);
    if (!upload.isValid()) {
        SKGPU_LOG_E("Recorder::updateBackendTexture: Could not create UploadInstance");
        return false;
    }
...
void Recorder::addFinishInfo(const InsertFinishInfo& info)
{
    if (info.fFinishedProc) {
        sk_sp<RefCntedCallback> callback = RefCntedCallback::Make(info.fFinishedProc, info.fFinishedContext);
...
sk_sp<TextureProxy> RecorderPriv::CreateCachedProxy(Recorder* recorder, const SkBitmap& bitmap, std::string_view label)
{
    ...
    return recorder->priv().proxyCache()->findOrCreateCachedProxy(recorder, bitmap, std::move(label));
}
