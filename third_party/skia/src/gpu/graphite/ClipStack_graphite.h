...
public:
    ...
    enum class PixelSnapping : bool { kNo = false, kYes = true };
    void clipShape(const Transform& localToDevice, const Shape& shape, SkClipOp op, PixelSnapping = PixelSnapping::kNo);
...
private:
    ...
    public:
        ...
        RawElement(const Rect& deviceBounds, const Transform& localToDevice, const Shape& shape, SkClipOp op, PixelSnapping);
        ...
        {
            return fOrder != DrawOrder::kNoIntersection;
        }
        const Shape& shape() const
        {
            return fShape;
        }
        const Transform& localToDevice() const
        {
        ...
        CompressedPaintersOrder updateForDraw(const BoundsManager* boundsManager, const Rect& drawBounds, PaintersDepth drawZ);
    ...
    public:
        ...
        int firstActiveElementIndex() const
        {
            return fStartingElementIndex;
        }
        int oldestElementIndex() const
        {
...
{
    if (this->currentSaveRecord().state() == ClipState::kEmpty || this->currentSaveRecord().state() == ClipState::kWideOpen) {
