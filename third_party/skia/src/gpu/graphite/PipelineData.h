...
public:
    ...
    explicit TextureDataBlock(const SampledTexture& texture)
        : fTextures(&texture, 1)
    {
    }
    ...
    bool empty() const
    {
        return fTextures.empty();
    }

    int numTextures() const
    {
        return SkTo<int>(fTextures.size());
    }
...
private:
    ...
    constexpr TextureDataBlock(SkSpan<const SampledTexture> textures)
        : fTextures(textures)
    {
    }
...
public:
    ...
    struct Entry {
        ...
        Entry(UniformDataBlock cpuData)
            : fCpuData(cpuData)
        {
        }
    };
...
public:
    ...
    {
        return TextureDataBlock(SkSpan(fTextures));
    }

    // Mimic the type-safe API available in UniformManager
    template <typename T> void write(const T& t)
    {
        fUniformManager.write(t);
    }
    template <typename T> void writeHalf(const T& t)
    {
        fUniformManager.writeHalf(t);
    }
    template <typename T> void writeArray(SkSpan<const T> t)
    {
        fUniformManager.writeArray(t);
    }
    ...
    {
        fUniformManager.write(u, data);
    }

    void writePaintColor(const SkPMColor4f& color)
    {
        fUniformManager.writePaintColor(color);
    }

    void beginStruct(int baseAligment)
    {
        fUniformManager.beginStruct(baseAligment);
    }
    void endStruct()
    {
        fUniformManager.endStruct();
    }

    bool hasUniforms() const
    {
        return fUniformManager.size();
    }

    bool hasGradientBufferData() const
    {
        return !fGradientStorage.empty();
    }

    SkSpan<const float> gradientBufferData() const
    {
        return fGradientStorage;
    }
...
public:
    UniformExpectationsValidator(PipelineDataGatherer* gatherer, SkSpan<const Uniform> expectedUniforms, bool isSubstruct = false)
        : fGatherer(gatherer)
    {
