...
[[maybe_unused]] static std::string to_str(const SharedContext* ctx, const GraphicsPipelineDesc& gpDesc, const RenderPassDesc& rpDesc)
{
...
ResourceProvider::ResourceProvider(SharedContext* sharedContext, SingleOwner* singleOwner, uint32_t recorderID, size_t resourceBudget)
...
sk_sp<GraphicsPipeline> ResourceProvider::findOrCreateGraphicsPipeline(const RuntimeEffectDictionary* runtimeDict, const GraphicsPipelineDesc& pipelineDesc,
    const RenderPassDesc& renderPassDesc, SkEnumBitMask<PipelineCreationFlags> pipelineCreationFlags)
{
    ...
    UniqueKey pipelineKey = fSharedContext->caps()->makeGraphicsPipelineKey(pipelineDesc, renderPassDesc);
    ...
    if (!pipeline) {
        ...
        // category is enabled.
        TRACE_EVENT1_ALWAYS("skia.shaders", "createGraphicsPipeline", "desc", TRACE_STR_COPY(to_str(fSharedContext, pipelineDesc, renderPassDesc).c_str()));
        pipeline = this->createGraphicsPipeline(runtimeDict, pipelineDesc, renderPassDesc, pipelineCreationFlags);
        if (pipeline) {
...
sk_sp<ComputePipeline> ResourceProvider::findOrCreateComputePipeline(const ComputePipelineDesc& pipelineDesc)
{
    auto globalCache = fSharedContext->globalCache();
...
sk_sp<Texture> ResourceProvider::findOrCreateScratchTexture(SkISize dimensions, const TextureInfo& info, std::string_view label, skgpu::Budgeted budgeted)
{
    ...
    return this->findOrCreateTextureWithKey(dimensions, info, key, std::move(label), budgeted);
}

sk_sp<Texture> ResourceProvider::findOrCreateDepthStencilAttachment(SkISize dimensions, const TextureInfo& info)
{
    SkASSERT(info.isValid());
    ...
    return this->findOrCreateTextureWithKey(dimensions, info, key, "DepthStencilAttachment", skgpu::Budgeted::kYes);
}

sk_sp<Texture> ResourceProvider::findOrCreateDiscardableMSAAAttachment(SkISize dimensions, const TextureInfo& info)
{
    SkASSERT(info.isValid());
    ...
    return this->findOrCreateTextureWithKey(dimensions, info, key, "DiscardableMSAAAttachment", skgpu::Budgeted::kYes);
}

sk_sp<Texture> ResourceProvider::findOrCreateTextureWithKey(
    SkISize dimensions, const TextureInfo& info, const GraphiteResourceKey& key, std::string_view label, skgpu::Budgeted budgeted)
{
...
sk_sp<Texture> ResourceProvider::createWrappedTexture(const BackendTexture& backendTexture, std::string_view label)
{
...
sk_sp<Buffer> ResourceProvider::findOrCreateBuffer(size_t size, BufferType type, AccessPattern accessPattern, std::string_view label)
{
    static const ResourceType kType = GraphiteResourceKey::GenerateResourceType();
    ...
    GraphiteResourceKey key;
    {
        ...
        builder[0] = (static_cast<uint32_t>(type) << 0) | (static_cast<uint32_t>(accessPattern) << 4);
        size_t szKey = size;
...
{
    if (dimensions.isEmpty() || dimensions.width() > maxTextureSize || dimensions.height() > maxTextureSize) {
...
BackendTexture ResourceProvider::createBackendTexture(
    AHardwareBuffer* hardwareBuffer, bool isRenderable, bool isProtectedContent, SkISize dimensions, bool fromAndroidWindow) const
{
    if (!dimensions_are_valid(fSharedContext->caps()->maxTextureSize(), dimensions)) {
        return {};
    }
    return this->onCreateBackendTexture(hardwareBuffer, isRenderable, isProtectedContent, dimensions, fromAndroidWindow);
}

BackendTexture ResourceProvider::onCreateBackendTexture(
    AHardwareBuffer*, bool isRenderable, bool isProtectedContent, SkISize dimensions, bool fromAndroidWindow) const
{
    return {};
}
#endif
