...
struct SamplerDesc {
    ...
    constexpr SamplerDesc(const SkSamplingOptions& samplingOptions, const std::pair<SkTileMode, SkTileMode> tileModes, const ImmutableSamplerInfo info = {})
        : fDesc((static_cast<int>(tileModes.first) << kTileModeXShift) | (static_cast<int>(tileModes.second) << kTileModeYShift)
            | (static_cast<int>(samplingOptions.filter) << kFilterModeShift) | (static_cast<int>(samplingOptions.mipmap) << kMipmapModeShift)
        ...
        , fExternalFormatMostSignificantBits(info.fFormat >> 32)
    {
        ...
        SkASSERT(!samplingOptions.useCubic || (samplingOptions.filter == SkFilterMode::kNearest && samplingOptions.mipmap == SkMipmapMode::kNone));
    ...
    {
        return o.fDesc == fDesc && o.fFormat == fFormat && o.fExternalFormatMostSignificantBits == fExternalFormatMostSignificantBits;
    }
    ...
    {
        return static_cast<SkTileMode>((fDesc >> 0) & 0b11);
    }
    SkTileMode tileModeY() const
    {
        return static_cast<SkTileMode>((fDesc >> 2) & 0b11);
    }
    uint32_t desc() const
    {
        return fDesc;
    ...
    {
        return fExternalFormatMostSignificantBits;
    }
    bool isImmutable() const
    {
        return (fDesc >> kImmutableSamplerInfoShift) != 0;
    }
    bool usesExternalFormat() const
    {
        return (fDesc >> kImmutableSamplerInfoShift) & 0b1;
    }
    ...
    static constexpr int kMaxNumConversionInfoBits = 32 - kNumFilterModeBits - kNumMipmapModeBits - kNumTileModeBits;
