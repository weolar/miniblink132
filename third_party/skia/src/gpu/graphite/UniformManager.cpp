...
int UniformOffsetCalculator::advanceOffset(SkSLType type, int count)
{
    ...
    if ((isArray && LayoutRules::AlignArraysAsVec4(fLayout)) || (dimension == 3 && (isArray || LayoutRules::PadVec3Size(fLayout)))) {
        dimension = 4;
    }

    const int primitiveSize = LayoutRules::UseFullPrecision(fLayout) || SkSLTypeIsFullPrecisionNumericType(type) ? 4 : 2;
...
static std::pair<SkSLType, int> adjust_for_matrix_type(SkSLType type, int count)
{
    ...
    case SkSLType::kFloat2x2:
        return { SkSLType::kFloat2, 2 * std::max(1, count) };
    case SkSLType::kFloat3x3:
        return { SkSLType::kFloat3, 3 * std::max(1, count) };
    case SkSLType::kFloat4x4:
        return { SkSLType::kFloat4, 4 * std::max(1, count) };

    case SkSLType::kHalf2x2:
        return { SkSLType::kHalf2, 2 * std::max(1, count) };
    case SkSLType::kHalf3x3:
        return { SkSLType::kHalf3, 3 * std::max(1, count) };
    case SkSLType::kHalf4x4:
        return { SkSLType::kHalf4, 4 * std::max(1, count) };
...
void UniformManager::write(const Uniform& u, const void* data)
{
    ...
    if (count == Uniform::kNonArray) {
        if (fullPrecision) {
            ...
            case 1:
                this->write<1, /*Half=*/false>(data, type);
                break;
            case 2:
                this->write<2, /*Half=*/false>(data, type);
                break;
            case 3:
                this->write<3, /*Half=*/false>(data, type);
                break;
            case 4:
                this->write<4, /*Half=*/false>(data, type);
                break;
            }
        } else {
            switch (SkSLTypeVecLength(type)) {
            case 1:
                this->write<1, /*Half=*/true>(data, type);
                break;
            case 2:
                this->write<2, /*Half=*/true>(data, type);
                break;
            case 3:
                this->write<3, /*Half=*/true>(data, type);
                break;
            case 4:
                this->write<4, /*Half=*/true>(data, type);
                break;
            }
        }
    } else {
        if (fullPrecision) {
            switch (SkSLTypeVecLength(type)) {
            case 1:
                this->writeArray<1, /*Half=*/false>(data, count, type);
                break;
            case 2:
                this->writeArray<2, /*Half=*/false>(data, count, type);
                break;
            case 3:
                this->writeArray<3, /*Half=*/false>(data, count, type);
                break;
            case 4:
                this->writeArray<4, /*Half=*/false>(data, count, type);
                break;
            }
        } else {
            switch (SkSLTypeVecLength(type)) {
            case 1:
                this->writeArray<1, /*Half=*/true>(data, count, type);
                break;
            case 2:
                this->writeArray<2, /*Half=*/true>(data, count, type);
                break;
            case 3:
                this->writeArray<3, /*Half=*/true>(data, count, type);
                break;
            case 4:
                this->writeArray<4, /*Half=*/true>(data, count, type);
                break;
            }
        }
    }
...
bool UniformManager::checkEndStruct()
{
    ...
    const int structSize = SkAlignTo(fSubstructCalculator.size(), fSubstructCalculator.requiredAlignment());
    if (fStorage.size() != fSubstructStartingOffset + structSize) {
        return false; // Somehow didn't end on the correct boundary
    }
    if (fReqAlignment != fOffsetCalculator.requiredAlignment() || fReqAlignment < fSubstructCalculator.requiredAlignment()) {
...
bool UniformManager::checkExpected(const void* dst, SkSLType type, int count)
{
    ...
    if (dst) {
        ...
        int offset = static_cast<int>(reinterpret_cast<intptr_t>(dst) - reinterpret_cast<intptr_t>(fStorage.data()));
