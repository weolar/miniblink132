...
struct ShaderInfo {
    ...
    bool isValid() const
    {
        return static_cast<bool>(fModule);
    }
};

static ShaderInfo compile_shader_module(const DawnSharedContext* sharedContext, const ComputePipelineDesc& pipelineDesc)
{
    ...
    if (step->supportsNativeShader()) {
        ...
        if (!DawnCompileWGSLShaderModule(sharedContext, step->name(), std::string(nativeShader.fSource), &info.fModule, errorHandler)) {
            return {};
        }
        ...
        if (skgpu::SkSLToWGSL(caps->shaderCaps(), sksl, SkSL::ProgramKind::kCompute, settings, &wgsl, &interface, errorHandler)) {
            if (!DawnCompileWGSLShaderModule(sharedContext, step->name(), wgsl, &info.fModule, errorHandler)) {
                return {};
            }
...
sk_sp<DawnComputePipeline> DawnComputePipeline::Make(const DawnSharedContext* sharedContext, const ComputePipelineDesc& pipelineDesc)
{
    ...
    return sk_sp<DawnComputePipeline>(new DawnComputePipeline(sharedContext, std::move(pipeline), std::move(bindGroupLayout)));
}

DawnComputePipeline::DawnComputePipeline(const SharedContext* sharedContext, wgpu::ComputePipeline pso, wgpu::BindGroupLayout groupLayout)
