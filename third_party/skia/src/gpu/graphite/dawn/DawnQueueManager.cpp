...
public:
    DawnWorkSubmissionWithAsyncWait(std::unique_ptr<CommandBuffer> cmdBuffer, DawnQueueManager* queueManager, const DawnSharedContext* sharedContext);
...
DawnWorkSubmissionWithAsyncWait::DawnWorkSubmissionWithAsyncWait(
    std::unique_ptr<CommandBuffer> cmdBuffer, DawnQueueManager* queueManager, const DawnSharedContext* sharedContext)
    : GpuWorkSubmission(std::move(cmdBuffer), queueManager)
    , fAsyncWait(sharedContext)
{
...
public:
    DawnWorkSubmissionWithFuture(std::unique_ptr<CommandBuffer> cmdBuffer, DawnQueueManager* queueManager);
...
DawnWorkSubmissionWithFuture::DawnWorkSubmissionWithFuture(std::unique_ptr<CommandBuffer> cmdBuffer, DawnQueueManager* queueManager)
    : GpuWorkSubmission(std::move(cmdBuffer), queueManager)
{
    fSubmittedWorkDoneFuture = queueManager->dawnQueue().OnSubmittedWorkDone(wgpu::CallbackMode::WaitAnyOnly, [](wgpu::QueueWorkDoneStatus) {});
}
...
std::unique_ptr<CommandBuffer> DawnQueueManager::getNewCommandBuffer(ResourceProvider* resourceProvider, Protected)
{
...
#if defined(__EMSCRIPTEN__)
    return std::make_unique<DawnWorkSubmissionWithAsyncWait>(std::move(fCurrentCommandBuffer), this, dawnSharedContext());
#else
