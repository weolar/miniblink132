...
DawnSampler::DawnSampler(const DawnSharedContext* sharedContext, wgpu::Sampler sampler)
...
wgpu::YCbCrVkDescriptor get_descriptor_from_sampler_desc(const SamplerDesc& samplerDesc)
{
    ...
    if (samplerDesc.isImmutable()) {
        uint32_t nonFormatInfo = (uint32_t)(samplerDesc.desc() >> SamplerDesc::kImmutableSamplerInfoShift);
        ...
        desc.vkChromaFilter = (wgpu::FilterMode)((nonFormatInfo & kChromaFilterMask) >> kChromaFilterShift);
        desc.forceExplicitReconstruction = (nonFormatInfo & kForceExplicitReconMask) >> kForceExplicitReconShift;
        desc.externalFormat = samplerDesc.usesExternalFormat() ? (uint64_t)(samplerDesc.externalFormatMSBs()) << 32 | samplerDesc.format() : 0;
    }
    return desc;
}
} // namespace
#endif
sk_sp<DawnSampler> DawnSampler::Make(const DawnSharedContext* sharedContext, const SamplerDesc& samplerDesc)
{
    ...
    if (sharedContext->caps()->setBackendLabels()) {
        ...
        if (ycbcrUtils::DawnDescriptorIsValid(ycbcrDescriptor)) {
            ...
            static const char* chromaFilterLabels[] = { "WGPU_Undefined", "WGPU_Nearest", "WGPU_Linear" };
