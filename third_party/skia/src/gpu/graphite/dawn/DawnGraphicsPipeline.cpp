...
size_t create_vertex_attributes(SkSpan<const Attribute> attrs, int shaderLocationOffset, std::vector<wgpu::VertexAttribute>* out)
{
...
static wgpu::BlendFactor blend_coeff_to_dawn_blend_for_alpha(const DawnCaps& caps, skgpu::BlendCoeff coeff)
{
    switch (coeff) {
...
void log_pipeline_creation(const AsyncPipelineCreationBase* apcb)
{
    ...
    if (apcb->fFromPrecompile) {
        ...
        SK_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Graphite.PipelineCreationTimes.Precompile", micros_since(apcb->fStartTime),
            /* minUSec= */ 1,
            /* maxUSec= */ kOneSecInUS, kBucketCount);
    } else if (apcb->fAsynchronous) {
        SK_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Graphite.PipelineCreationTimes.Asynchronous", micros_since(apcb->fStartTime),
            /* minUSec= */ 1,
            /* maxUSec= */ kOneSecInUS, kBucketCount);
    } else {
        SK_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Graphite.PipelineCreationTimes.Synchronous", micros_since(apcb->fStartTime),
            /* minUSec= */ 1,
            /* maxUSec= */ kOneSecInUS, kBucketCount);
    }
...
// static
sk_sp<DawnGraphicsPipeline> DawnGraphicsPipeline::Make(const DawnSharedContext* sharedContext, DawnResourceProvider* resourceProvider,
    const RuntimeEffectDictionary* runtimeDict, const GraphicsPipelineDesc& pipelineDesc, const RenderPassDesc& renderPassDesc,
    SkEnumBitMask<PipelineCreationFlags> pipelineCreationFlags)
{
    ...
    std::unique_ptr<ShaderInfo> shaderInfo = ShaderInfo::Make(
        &caps, sharedContext->shaderCodeDictionary(), runtimeDict, step, paintID, useStorageBuffers, renderPassDesc.fWriteSwizzle, samplerDescArrPtr);
    ...
    if (hasFragmentSkSL) {
        if (!skgpu::SkSLToWGSL(caps.shaderCaps(), fsSkSL, SkSL::ProgramKind::kGraphiteFragment, settings, &fsCode, &fsInterface, errorHandler)) {
            return {};
        }
        if (!DawnCompileWGSLShaderModule(sharedContext, shaderInfo->fsLabel().c_str(), fsCode, &fsModule, errorHandler)) {
            return {};
        }
    }

    const std::string& vsSkSL = shaderInfo->vertexSkSL();
    if (!skgpu::SkSLToWGSL(caps.shaderCaps(), vsSkSL, SkSL::ProgramKind::kGraphiteVertex, settings, &vsCode, &vsInterface, errorHandler)) {
        return {};
    }
    if (!DawnCompileWGSLShaderModule(sharedContext, shaderInfo->vsLabel().c_str(), vsCode, &vsModule, errorHandler)) {
        return {};
    }

    std::string pipelineLabel = GetPipelineLabel(sharedContext->shaderCodeDictionary(), renderPassDesc, step, paintID);
    ...
    colorTarget.format = TextureInfos::GetDawnViewFormat(renderPassDesc.fColorAttachment.fTextureInfo);
    colorTarget.blend = blendOn ? &blend : nullptr;
    colorTarget.writeMask = blendInfo.fWritesColor && hasFragmentSkSL ? wgpu::ColorWriteMask::All : wgpu::ColorWriteMask::None;

#if !defined(__EMSCRIPTEN__)
    const bool loadMsaaFromResolve
        = renderPassDesc.fColorResolveAttachment.fTextureInfo.isValid() && renderPassDesc.fColorResolveAttachment.fLoadOp == LoadOp::kLoad;
    ...
    SkASSERT(depthStencilSettings.fDepthTestEnabled || depthStencilSettings.fDepthCompareOp == CompareOp::kAlways);
    wgpu::DepthStencilState depthStencil;
    if (renderPassDesc.fDepthStencilAttachment.fTextureInfo.isValid()) {
        wgpu::TextureFormat dsFormat = TextureInfos::GetDawnViewFormat(renderPassDesc.fDepthStencilAttachment.fTextureInfo);
        depthStencil.format = DawnFormatIsDepthOrStencil(dsFormat) ? dsFormat : wgpu::TextureFormat::Undefined;
    ...
    skia_private::TArray<sk_sp<DawnSampler>> immutableSamplers;
    {
        ...
        if (hasFragmentSamplers) {
            ...
            if (numTexturesAndSamplers == 2 && !(samplerDescArrPtr && samplerDescArrPtr->at(0).isImmutable())) {
                groupLayouts[1] = resourceProvider->getOrCreateSingleTextureSamplerBindGroupLayout();
            } else {
                ...
                for (int i = 0; i < numTexturesAndSamplers;) {
                    ...
                    if (samplerDesc.isImmutable()) {
                        sk_sp<Sampler> immutableSampler = resourceProvider->findOrCreateCompatibleSampler(samplerDesc);
                        if (!immutableSampler) {
                            SKGPU_LOG_E("Failed to find/create immutable sampler for pipeline");
                            return {};
                        }
                        sk_sp<DawnSampler> dawnImmutableSampler = sk_ref_sp<DawnSampler>(static_cast<DawnSampler*>(immutableSampler.get()));
                        SkASSERT(dawnImmutableSampler);

                        wgpu::StaticSamplerBindingLayout& immutableSamplerBinding = staticSamplerLayouts.emplace_back();
        ...
        layoutDesc.bindGroupLayoutCount = hasFragmentSamplers ? groupLayouts.size() : groupLayouts.size() - 1;
    ...
    {
        auto arrayStride = create_vertex_attributes(step->vertexAttributes(), 0, &vertexAttributes);
    ...
    {
        auto arrayStride = create_vertex_attributes(step->instanceAttributes(), step->vertexAttributes().size(), &instanceAttributes);
    ...
    const bool forPrecompilation = SkToBool(pipelineCreationFlags & PipelineCreationFlags::kForPrecompilation);
...
#else
        asyncCreation->fFuture = device.CreateRenderPipelineAsync(&descriptor, wgpu::CallbackMode::WaitAnyOnly,
            [asyncCreationPtr = asyncCreation.get()](wgpu::CreatePipelineAsyncStatus status, wgpu::RenderPipeline pipeline, char const* message) {
                if (status != wgpu::CreatePipelineAsyncStatus::Success) {
                    SKGPU_LOG_E("Failed to create render pipeline (%d): %s", static_cast<int>(status), message);
    ...
    return sk_sp<DawnGraphicsPipeline>(new DawnGraphicsPipeline(sharedContext, pipelineInfo, std::move(asyncCreation), std::move(groupLayouts),
        step->primitiveType(), depthStencilSettings.fStencilReferenceValue, std::move(immutableSamplers)));
}

DawnGraphicsPipeline::DawnGraphicsPipeline(const skgpu::graphite::SharedContext* sharedContext, const PipelineInfo& pipelineInfo,
    std::unique_ptr<AsyncPipelineCreation> asyncCreationInfo, BindGroupLayouts groupLayouts, PrimitiveType primitiveType, uint32_t refValue,
    skia_private::TArray<sk_sp<DawnSampler>> immutableSamplers)
...
const wgpu::RenderPipeline& DawnGraphicsPipeline::dawnRenderPipeline() const
{
    ...
    const auto& instance = static_cast<const DawnSharedContext*>(sharedContext())->device().GetAdapter().GetInstance();

    [[maybe_unused]] auto status = instance.WaitAny(1, &waitInfo, /*timeoutNS=*/std::numeric_limits<uint64_t>::max());
