...
constexpr DawnErrorType kErrorScopeTypes[] = { DawnErrorType::kValidation, DawnErrorType::kOutOfMemory, DawnErrorType::kInternal };
...
SkEnumBitMask<DawnErrorType> DawnErrorChecker::popErrorScopes()
{
    ...
    wgpu::ErrorCallback errorCallback = [](WGPUErrorType status, WGPUStringView msg, void* userData) {
...
#if defined(WGPU_BREAKING_CHANGE_STRING_VIEW_CALLBACKS)
            SKGPU_LOG_E("Failed in error scope (%s): %.*s", errorScopeName, static_cast<int>(msg.length), msg.data);
#else // defined(WGPU_BREAKING_CHANGE_STRING_VIEW_CALLBACKS)
