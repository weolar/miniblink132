...
wgpu::RenderPipeline create_blit_render_pipeline(const DawnSharedContext* sharedContext, const char* label, wgpu::ShaderModule vsModule,
    wgpu::ShaderModule fsModule, wgpu::TextureFormat renderPassColorFormat, wgpu::TextureFormat renderPassDepthStencilFormat, int numSamples)
{
...
template <size_t NumEntries> using BindGroupKey = typename DawnResourceProvider::BindGroupKey<NumEntries>;
...
UniformBindGroupKey make_ubo_bind_group_key(
    const std::array<std::pair<const DawnBuffer*, uint32_t>, DawnResourceProvider::kNumUniformEntries>& boundBuffersAndSizes)
{
...
BindGroupKey<1> make_texture_bind_group_key(const DawnSampler* sampler, const DawnTexture* texture)
{
...
public:
    ...
    IntrinsicBuffer(sk_sp<DawnBuffer> dawnBuffer)
        : fDawnBuffer(std::move(dawnBuffer))
    {
    }
    ~IntrinsicBuffer() = default;

    sk_sp<DawnBuffer> buffer() const
    {
        return fDawnBuffer;
    }
    ...
    int slotsUsed() const
    {
        return fCachedIntrinsicValues.count();
    }
...
private:
    skia_private::THashMap<UniformDataBlock, uint32_t, UniformDataBlock::Hash> fCachedIntrinsicValues;
...
// Find or create a bind buffer info for the given intrinsic values used in the given command
// buffer.
BindBufferInfo DawnResourceProvider::IntrinsicConstantsManager::add(DawnCommandBuffer* cb, UniformDataBlock intrinsicValues)
{
    ...
    if (!fCurrentBuffer || fCurrentBuffer->slotsUsed() == IntrinsicBuffer::kNumSlots) {
        ...
        auto dawnBuffer = resourceProvider->findOrCreateDawnBuffer(
            stride * IntrinsicBuffer::kNumSlots, BufferType::kUniform, AccessPattern::kGpuOnly, "IntrinsicConstantBuffer");
        if (!dawnBuffer) {
    ...
    fResourceProvider->dawnSharedContext()->queue().WriteBuffer(
        fCurrentBuffer->buffer()->dawnBuffer(), newOffset, intrinsicValues.data(), intrinsicValues.size());
...
DawnResourceProvider::DawnResourceProvider(SharedContext* sharedContext, SingleOwner* singleOwner, uint32_t recorderID, size_t resourceBudget)
...
wgpu::RenderPipeline DawnResourceProvider::findOrCreateBlitWithDrawPipeline(const RenderPassDesc& renderPassDesc)
{
    uint32_t renderPassKey = this->dawnSharedContext()->dawnCaps()->getRenderPassDescKeyForPipeline(renderPassDesc);
    ...
    if (!pipeline) {
        ...
        pipeline = create_blit_render_pipeline(dawnSharedContext(),
            /*label=*/"BlitWithDraw", std::move(vsModule), std::move(fsModule),
            ...
            renderPassDesc.fDepthStencilAttachment.fTextureInfo.isValid() ? TextureInfos::GetDawnViewFormat(renderPassDesc.fDepthStencilAttachment.fTextureInfo)
...
sk_sp<Texture> DawnResourceProvider::onCreateWrappedTexture(const BackendTexture& texture)
{
    ...
    if (dawnTexture) {
        return DawnTexture::MakeWrapped(this->dawnSharedContext(), texture.dimensions(), texture.info(), std::move(dawnTexture));
    } else {
        return DawnTexture::MakeWrapped(this->dawnSharedContext(), texture.dimensions(), texture.info(), std::move(dawnTextureView));
    }
}

sk_sp<DawnTexture> DawnResourceProvider::findOrCreateDiscardableMSAALoadTexture(SkISize dimensions, const TextureInfo& msaaInfo)
{
    ...
    auto texture = this->findOrCreateDiscardableMSAAAttachment(dimensions, TextureInfos::MakeDawn(dawnMsaaLoadTextureInfo));

    return sk_sp<DawnTexture>(static_cast<DawnTexture*>(texture.release()));
}

sk_sp<GraphicsPipeline> DawnResourceProvider::createGraphicsPipeline(const RuntimeEffectDictionary* runtimeDict, const GraphicsPipelineDesc& pipelineDesc,
    const RenderPassDesc& renderPassDesc, SkEnumBitMask<PipelineCreationFlags> pipelineCreationFlags)
{
    return DawnGraphicsPipeline::Make(this->dawnSharedContext(), this, runtimeDict, pipelineDesc, renderPassDesc, pipelineCreationFlags);
}

sk_sp<ComputePipeline> DawnResourceProvider::createComputePipeline(const ComputePipelineDesc& desc)
{
    return DawnComputePipeline::Make(this->dawnSharedContext(), desc);
}

sk_sp<Texture> DawnResourceProvider::createTexture(SkISize dimensions, const TextureInfo& info, skgpu::Budgeted budgeted)
{
    return DawnTexture::Make(this->dawnSharedContext(), dimensions, info, budgeted);
}

sk_sp<Buffer> DawnResourceProvider::createBuffer(size_t size, BufferType type, AccessPattern accessPattern)
{
...
BackendTexture DawnResourceProvider::onCreateBackendTexture(SkISize dimensions, const TextureInfo& info)
{
    wgpu::Texture texture = DawnTexture::MakeDawnTexture(this->dawnSharedContext(), dimensions, info);
    if (!texture) {
        return {};
    }
...
sk_sp<DawnBuffer> DawnResourceProvider::findOrCreateDawnBuffer(size_t size, BufferType type, AccessPattern accessPattern, std::string_view label)
{
...
const wgpu::BindGroupLayout& DawnResourceProvider::getOrCreateUniformBuffersBindGroupLayout()
{
    ...
    entries[1].buffer.type = fSharedContext->caps()->storageBufferSupport() ? wgpu::BufferBindingType::ReadOnlyStorage : wgpu::BufferBindingType::Uniform;
    entries[1].buffer.hasDynamicOffset = true;
    entries[1].buffer.minBindingSize = 0;

    entries[2].binding = DawnGraphicsPipeline::kPaintUniformBufferIndex;
    entries[2].visibility = wgpu::ShaderStage::Fragment;
    entries[2].buffer.type = fSharedContext->caps()->storageBufferSupport() ? wgpu::BufferBindingType::ReadOnlyStorage : wgpu::BufferBindingType::Uniform;
    ...
    entries[3].buffer.type = fSharedContext->caps()->storageBufferSupport() ? wgpu::BufferBindingType::ReadOnlyStorage : wgpu::BufferBindingType::Uniform;
    ...
    fUniformBuffersBindGroupLayout = this->dawnSharedContext()->device().CreateBindGroupLayout(&groupLayoutDesc);

    return fUniformBuffersBindGroupLayout;
}

const wgpu::BindGroupLayout& DawnResourceProvider::getOrCreateSingleTextureSamplerBindGroupLayout()
{
    ...
    groupLayoutDesc.entryCount = entries.size();
    groupLayoutDesc.entries = entries.data();
    fSingleTextureSamplerBindGroupLayout = this->dawnSharedContext()->device().CreateBindGroupLayout(&groupLayoutDesc);
...
const wgpu::Buffer& DawnResourceProvider::getOrCreateNullBuffer()
{
    if (!fNullBuffer) {
        ...
        desc.usage = wgpu::BufferUsage::CopyDst | wgpu::BufferUsage::Uniform | wgpu::BufferUsage::Storage;
...
const wgpu::BindGroup& DawnResourceProvider::findOrCreateUniformBuffersBindGroup(
    const std::array<std::pair<const DawnBuffer*, uint32_t>, kNumUniformEntries>& boundBuffersAndSizes)
{
...
const wgpu::BindGroup& DawnResourceProvider::findOrCreateSingleTextureSamplerBindGroup(const DawnSampler* sampler, const DawnTexture* texture)
{
...
BindBufferInfo DawnResourceProvider::findOrCreateIntrinsicBindBufferInfo(DawnCommandBuffer* cb, UniformDataBlock intrinsicValues)
{
