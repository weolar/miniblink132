...
// DawnCommandBuffer
// ----------------------------------------------------------------------------
std::unique_ptr<DawnCommandBuffer> DawnCommandBuffer::Make(const DawnSharedContext* sharedContext, DawnResourceProvider* resourceProvider)
{
    std::unique_ptr<DawnCommandBuffer> cmdBuffer(new DawnCommandBuffer(sharedContext, resourceProvider));
...
bool DawnCommandBuffer::onAddRenderPass(const RenderPassDesc& renderPassDesc, SkIRect renderPassBounds, const Texture* colorTexture,
    const Texture* resolveTexture, const Texture* depthStencilTexture, SkIRect viewport, const DrawPassList& drawPasses)
{
    ...
    if (!viewport.intersect(SkIRect::MakeSize(fColorAttachmentSize)))
        SK_UNLIKELY
        {
    ...
    for (const auto& drawPass : drawPasses) {
        if (!this->addDrawPass(drawPass.get()))
            SK_UNLIKELY
            {
                this->endRenderPass();
                return false;
...
bool DawnCommandBuffer::onAddComputePass(DispatchGroupSpan groups)
{
    ...
    for (const auto& group : groups) {
        ...
        for (const auto& dispatch : group->dispatches()) {
            ...
            if (const WorkgroupSize* globalSize = std::get_if<WorkgroupSize>(&dispatch.fGlobalSizeOrIndirect)) {
                this->dispatchWorkgroups(*globalSize);
            } else {
                SkASSERT(std::holds_alternative<BindBufferInfo>(dispatch.fGlobalSizeOrIndirect));
                const BindBufferInfo& indirect = *std::get_if<BindBufferInfo>(&dispatch.fGlobalSizeOrIndirect);
...
bool DawnCommandBuffer::beginRenderPass(const RenderPassDesc& renderPassDesc, SkIRect renderPassBounds, const Texture* colorTexture,
    const Texture* resolveTexture, const Texture* depthStencilTexture)
{
    ...
    constexpr static wgpu::StoreOp wgpuStoreActionMap[] { wgpu::StoreOp::Store, wgpu::StoreOp::Discard };
    ...
    if (colorTexture) {
        ...
        wgpuColorAttachment.clearValue = { clearColor[0], clearColor[1], clearColor[2], clearColor[3] };
        ...
        if (resolveTexture) {
            ...
            if (renderPassDesc.fColorResolveAttachment.fLoadOp == LoadOp::kLoad) {
                std::optional<wgpu::LoadOp> resolveLoadOp = fSharedContext->dawnCaps()->resolveTextureLoadOp();
            ...
            [[maybe_unused]] bool isMSAAToSingleSampled = renderPassDesc.fSampleCount > 1 && colorTexture->numSamples() == 1;
#if defined(__EMSCRIPTEN__)
            ...
            if (isMSAAToSingleSampled) {
                ...
                // pass.
                SkASSERT(fSharedContext->device().HasFeature(wgpu::FeatureName::MSAARenderToSingleSampled));
    ...
    if (depthStencilTexture) {
        ...
        if (DawnFormatIsDepth(format)) {
            ...
            wgpuDepthStencilAttachment.depthLoadOp = wgpuLoadActionMap[static_cast<int>(depthStencilInfo.fLoadOp)];
            wgpuDepthStencilAttachment.depthStoreOp = wgpuStoreActionMap[static_cast<int>(depthStencilInfo.fStoreOp)];
        }

        if (DawnFormatIsStencil(format)) {
            wgpuDepthStencilAttachment.stencilClearValue = renderPassDesc.fClearStencil;
            wgpuDepthStencilAttachment.stencilLoadOp = wgpuLoadActionMap[static_cast<int>(depthStencilInfo.fLoadOp)];
            wgpuDepthStencilAttachment.stencilStoreOp = wgpuStoreActionMap[static_cast<int>(depthStencilInfo.fStoreOp)];
        }
    ...
    if (loadMSAAFromResolveExplicitly) {
        ...
        if (!this->loadMSAAFromResolveAndBeginRenderPassEncoder(renderPassDesc, wgpuRenderPass, static_cast<const DawnTexture*>(colorTexture))) {
            return false;
        }
    } else {
...
bool DawnCommandBuffer::loadMSAAFromResolveAndBeginRenderPassEncoder(
    const RenderPassDesc& frontendRenderPassDesc, const wgpu::RenderPassDescriptor& wgpuRenderPassDesc, const DawnTexture* msaaTexture)
{
    SkASSERT(!fActiveRenderPassEncoder);
    ...
    auto msaaLoadTexture = fResourceProvider->findOrCreateDiscardableMSAALoadTexture(msaaTexture->dimensions(), msaaTexture->textureInfo());
    if (!msaaLoadTexture) {
    ...
    intermediateRenderPassDesc.fColorAttachment.fTextureInfo = frontendRenderPassDesc.fColorResolveAttachment.fTextureInfo;
    ...
    bool blitSucceeded = this->doBlitWithDraw(renderPassEncoder, intermediateRenderPassDesc,
        /*sourceTextureView=*/wgpuRenderPassDesc.colorAttachments[0].resolveTarget, msaaTexture->dimensions().width(), msaaTexture->dimensions().height());
    ...
    if (!this->doBlitWithDraw(renderPassEncoder, frontendRenderPassDesc,
            /*sourceTextureView=*/msaaLoadTexture->renderTextureView(), msaaTexture->dimensions().width(), msaaTexture->dimensions().height())) {
        renderPassEncoder.End();
        return false;
    }
...
bool DawnCommandBuffer::doBlitWithDraw(const wgpu::RenderPassEncoder& renderEncoder, const RenderPassDesc& frontendRenderPassDesc,
    const wgpu::TextureView& sourceTextureView, int width, int height)
{
...
bool DawnCommandBuffer::bindGraphicsPipeline(const GraphicsPipeline* graphicsPipeline)
{
    ...
    if (fActiveGraphicsPipeline->dstReadRequirement() == DstReadRequirement::kTextureCopy && fActiveGraphicsPipeline->numFragTexturesAndSamplers() == 2) {
        ...
        wgpu::BindGroup bindGroup = fResourceProvider->findOrCreateSingleTextureSamplerBindGroup(sampler, texture);
...
void DawnCommandBuffer::bindDrawBuffers(
    const BindBufferInfo& vertices, const BindBufferInfo& instances, const BindBufferInfo& indices, const BindBufferInfo& indirect)
{
    ...
    if (vertices.fBuffer) {
        ...
        fActiveRenderPassEncoder.SetVertexBuffer(DawnGraphicsPipeline::kVertexBufferIndex, dawnBuffer, vertices.fOffset);
    }
    if (instances.fBuffer) {
        auto dawnBuffer = static_cast<const DawnBuffer*>(instances.fBuffer)->dawnBuffer();
        fActiveRenderPassEncoder.SetVertexBuffer(DawnGraphicsPipeline::kInstanceBufferIndex, dawnBuffer, instances.fOffset);
    }
    if (indices.fBuffer) {
        auto dawnBuffer = static_cast<const DawnBuffer*>(indices.fBuffer)->dawnBuffer();
        fActiveRenderPassEncoder.SetIndexBuffer(dawnBuffer, wgpu::IndexFormat::Uint16, indices.fOffset);
    }
    if (indirect.fBuffer) {
...
void DawnCommandBuffer::bindTextureAndSamplers(const DrawPass& drawPass, const DrawPassCommands::BindTexturesAndSamplers& command)
{
    SkASSERT(fActiveRenderPassEncoder);
    ...
    if (command.fNumTexSamplers == 1) {
        const wgpu::YCbCrVkDescriptor& ycbcrDesc
            = TextureInfos::GetDawnTextureSpec(drawPass.getTexture(command.fTextureIndices[0])->textureInfo()).fYcbcrVkDescriptor;
    ...
    if (numTexturesAndSamplers == 1 && !usingSingleStaticSampler) {
        ...
        const auto* texture = static_cast<const DawnTexture*>(drawPass.getTexture(command.fTextureIndices[0]));
        const auto* sampler = static_cast<const DawnSampler*>(drawPass.getSampler(command.fSamplerIndices[0]));
        ...
        if (fActiveGraphicsPipeline->dstReadRequirement() == DstReadRequirement::kTextureCopy) {
            ...
            textureEntry.textureView = static_cast<const DawnTexture*>(fDstCopy.first)->sampleTextureView();
            entries.push_back(textureEntry);
        }
    ...
    if (fBoundUniformBuffersDirty) {
        ...
        for (int i = 0; i < kNumBuffers; ++i) {
            if (enabled[i] && fBoundUniforms[i]) {
                boundBuffersAndSizes[i].first = static_cast<const DawnBuffer*>(fBoundUniforms[i].fBuffer);
        ...
        auto bindGroup = fResourceProvider->findOrCreateUniformBuffersBindGroup(boundBuffersAndSizes);

        fActiveRenderPassEncoder.SetBindGroup(DawnGraphicsPipeline::kUniformBufferBindGroupIndex, bindGroup, dynamicOffsets.size(), dynamicOffsets.data());
    }
}

void DawnCommandBuffer::setScissor(unsigned int left, unsigned int top, unsigned int width, unsigned int height)
{
    SkASSERT(fActiveRenderPassEncoder);
    ...
    fActiveRenderPassEncoder.SetScissorRect(scissor.x(), scissor.y(), scissor.width(), scissor.height());
}
...
bool DawnCommandBuffer::updateIntrinsicUniforms(SkIRect viewport)
{
    ...
    BindBufferInfo binding = fResourceProvider->findOrCreateIntrinsicBindBufferInfo(this, UniformDataBlock::Wrap(&intrinsicValues));
    if (!binding) {
        return false;
...
{
    SkASSERT(fActiveRenderPassEncoder);
    fActiveRenderPassEncoder.SetViewport(viewport.x(), viewport.y(), viewport.width(), viewport.height(), 0, 1);
}

void DawnCommandBuffer::setBlendConstants(float* blendConstants)
{
    SkASSERT(fActiveRenderPassEncoder);
    wgpu::Color blendConst = { blendConstants[0], blendConstants[1], blendConstants[2], blendConstants[3] };
    fActiveRenderPassEncoder.SetBlendConstant(&blendConst);
}

void DawnCommandBuffer::draw(PrimitiveType type, unsigned int baseVertex, unsigned int vertexCount)
{
    SkASSERT(fActiveRenderPassEncoder);
...
void DawnCommandBuffer::drawIndexed(PrimitiveType type, unsigned int baseIndex, unsigned int indexCount, unsigned int baseVertex)
{
    SkASSERT(fActiveRenderPassEncoder);
    SkASSERT(fActiveGraphicsPipeline->primitiveType() == type);
...
void DawnCommandBuffer::drawInstanced(
    PrimitiveType type, unsigned int baseVertex, unsigned int vertexCount, unsigned int baseInstance, unsigned int instanceCount)
{
    SkASSERT(fActiveRenderPassEncoder);
    SkASSERT(fActiveGraphicsPipeline->primitiveType() == type);
...
void DawnCommandBuffer::drawIndexedInstanced(
    PrimitiveType type, unsigned int baseIndex, unsigned int indexCount, unsigned int baseVertex, unsigned int baseInstance, unsigned int instanceCount)
{
    SkASSERT(fActiveRenderPassEncoder);
    SkASSERT(fActiveGraphicsPipeline->primitiveType() == type);
    ...
    fActiveRenderPassEncoder.DrawIndexed(indexCount, instanceCount, baseIndex, baseVertex, baseInstance);
}
...
void DawnCommandBuffer::drawIndexedIndirect(PrimitiveType type)
{
    ...
    fActiveRenderPassEncoder.DrawIndexedIndirect(fCurrentIndirectBuffer, fCurrentIndirectBufferOffset);
}
...
void DawnCommandBuffer::bindDispatchResources(const DispatchGroup& group, const DispatchGroup::Dispatch& dispatch)
{
    SkASSERT(fActiveComputePassEncoder);
    ...
    for (const ResourceBinding& binding : dispatch.fBindings) {
        ...
        if (const BindBufferInfo* buffer = std::get_if<BindBufferInfo>(&binding.fResource)) {
            ...
            const DawnTexture* texture = static_cast<const DawnTexture*>(group.getTexture(texIdx->fValue));
            SkASSERT(texture);
            ...
            const DawnSampler* sampler = static_cast<const DawnSampler*>(group.getSampler(samplerIdx->fValue));
...
void DawnCommandBuffer::dispatchWorkgroups(const WorkgroupSize& globalSize)
{
    ...
    fActiveComputePassEncoder.DispatchWorkgroups(globalSize.fWidth, globalSize.fHeight, globalSize.fDepth);
}

void DawnCommandBuffer::dispatchWorkgroupsIndirect(const Buffer* indirectBuffer, size_t indirectBufferOffset)
{
    SkASSERT(fActiveComputePassEncoder);
    SkASSERT(fActiveComputePipeline);
...
bool DawnCommandBuffer::onCopyBufferToBuffer(const Buffer* srcBuffer, size_t srcOffset, const Buffer* dstBuffer, size_t dstOffset, size_t size)
{
    SkASSERT(!fActiveRenderPassEncoder);
    SkASSERT(!fActiveComputePassEncoder);
...
bool DawnCommandBuffer::onCopyTextureToBuffer(const Texture* texture, SkIRect srcRect, const Buffer* buffer, size_t bufferOffset, size_t bufferRowBytes)
{
    SkASSERT(!fActiveRenderPassEncoder);
    SkASSERT(!fActiveComputePassEncoder);
    ...
    wgpu::Extent3D copySize = { static_cast<uint32_t>(srcRect.width()), static_cast<uint32_t>(srcRect.height()), 1 };
    fCommandEncoder.CopyTextureToBuffer(&src, &dst, &copySize);

    return true;
}

bool DawnCommandBuffer::onCopyBufferToTexture(const Buffer* buffer, const Texture* texture, const BufferTextureCopyData* copyData, int count)
{
    SkASSERT(!fActiveRenderPassEncoder);
    SkASSERT(!fActiveComputePassEncoder);
    ...
    for (int i = 0; i < count; ++i) {
        ...
        wgpu::Extent3D copySize = { static_cast<uint32_t>(copyData[i].fRect.width()), static_cast<uint32_t>(copyData[i].fRect.height()), 1 };
...
bool DawnCommandBuffer::onCopyTextureToTexture(const Texture* src, SkIRect srcRect, const Texture* dst, SkIPoint dstPoint, int mipLevel)
{
    SkASSERT(!fActiveRenderPassEncoder);
    SkASSERT(!fActiveComputePassEncoder);
