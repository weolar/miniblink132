...
public:
    DawnTextureInfoData(DawnTextureSpec v)
        : fDawnSpec(v)
    {
    }
...
private:
    ...
    {
        return SkStringPrintf("Dawn(f=%u,s=%u)", static_cast<unsigned int>(fDawnSpec.fFormat), sampleCount);
    }
...
{
    return TextureInfoPriv::Make(skgpu::BackendApi::kDawn, dawnInfo.fSampleCount, dawnInfo.fMipmapped, Protected::kNo, DawnTextureInfoData(dawnInfo));
}
