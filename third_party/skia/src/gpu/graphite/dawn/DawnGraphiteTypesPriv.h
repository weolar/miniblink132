...
struct DawnTextureSpec {
    ...
    {
        return fUsage == that.fUsage && fFormat == that.fFormat && fViewFormat == that.fViewFormat && fAspect == that.fAspect &&
#if !defined(__EMSCRIPTEN__)
            ycbcrUtils::DawnDescriptorsAreEquivalent(fYcbcrVkDescriptor, that.fYcbcrVkDescriptor) &&
#endif
            fSlice == that.fSlice;
    }
