...
namespace {
[[maybe_unused]] bool is_map_succeeded(WGPUBufferMapAsyncStatus status)
{
    return status == WGPUBufferMapAsyncStatus_Success;
}

[[maybe_unused]] void log_map_error(WGPUBufferMapAsyncStatus status, const char*)
{
...
void log_map_error(wgpu::MapAsyncStatus status, const char* message)
{
    ...
    SKGPU_LOG(Priority::kError, "Buffer async map failed with status %s, message '%s'.", statusStr, message);
}
#endif // #if !defined(__EMSCRIPTEN__)
} // namespace

sk_sp<DawnBuffer> DawnBuffer::Make(const DawnSharedContext* sharedContext, size_t size, BufferType type, AccessPattern accessPattern)
{
    if (size <= 0) {
        return nullptr;
    }
    ...
    if (sharedContext->caps()->drawBufferCanBeMapped() && accessPattern == AccessPattern::kHostVisible && type != BufferType::kXferGpuToCpu) {
    ...
    return sk_sp<DawnBuffer>(new DawnBuffer(sharedContext, size, std::move(buffer), mappedAtCreationPtr));
}

DawnBuffer::DawnBuffer(const DawnSharedContext* sharedContext, size_t size, wgpu::Buffer buffer, void* mappedAtCreationPtr)
...
void DawnBuffer::prepareForReturnToCache(const std::function<void()>& takeRef)
{
    ...
    takeRef();
    this->asyncMap(
        [](void* ctx, skgpu::CallbackResult result) {
...
void DawnBuffer::onAsyncMap(GpuFinishedProc proc, GpuFinishedContext ctx)
{
    ...
    SkASSERT(fBuffer);
    SkASSERT((fBuffer.GetUsage() & wgpu::BufferUsage::MapRead) || (fBuffer.GetUsage() & wgpu::BufferUsage::MapWrite));
    ...
    fBuffer.MapAsync(
        isWrite ? wgpu::MapMode::Write : wgpu::MapMode::Read, 0, fBuffer.GetSize(),
...
void DawnBuffer::onMap()
{
    ...
    mapWaitInfo.future = fBuffer.MapAsync(isWrite ? wgpu::MapMode::Write : wgpu::MapMode::Read, 0, fBuffer.GetSize(), wgpu::CallbackMode::WaitAnyOnly,
        [this](wgpu::MapAsyncStatus s, const char* m) { this->mapCallback(s, m); });
    ...
    if (status != wgpu::WaitStatus::Success) {
        ...
        status = instance.WaitAny(1, &mapWaitInfo, /*timeoutNS=*/std::numeric_limits<uint64_t>::max());
    }
...
template <typename StatusT> void DawnBuffer::mapCallback(StatusT status, const char* message)
{
