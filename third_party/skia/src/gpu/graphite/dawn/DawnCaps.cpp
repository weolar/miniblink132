...
{
    static const skgpu::UniqueKey::Domain kDawnGraphicsPipelineDomain = skgpu::UniqueKey::GenerateDomain();
...
bool DawnCaps::onIsTexturable(const TextureInfo& info) const
{
    ...
    switch (spec.fFormat) {
    case wgpu::TextureFormat::R8BG8Biplanar420Unorm: {
        if (spec.fAspect == wgpu::TextureAspect::Plane0Only && spec.getViewFormat() != wgpu::TextureFormat::R8Unorm) {
            return false;
        }
        if (spec.fAspect == wgpu::TextureAspect::Plane1Only && spec.getViewFormat() != wgpu::TextureFormat::RG8Unorm) {
            return false;
        }
        break;
    }
    case wgpu::TextureFormat::R10X6BG10X6Biplanar420Unorm: {
        if (spec.fAspect == wgpu::TextureAspect::Plane0Only && spec.getViewFormat() != wgpu::TextureFormat::R16Unorm) {
            return false;
        }
        if (spec.fAspect == wgpu::TextureAspect::Plane1Only && spec.getViewFormat() != wgpu::TextureFormat::RG16Unorm) {
            return false;
        }
        break;
    }
    case wgpu::TextureFormat::R8BG8A8Triplanar420Unorm: {
        ...
        if (spec.fAspect == wgpu::TextureAspect::Plane2Only && spec.getViewFormat() != wgpu::TextureFormat::R8Unorm) {
            return false;
        }
        break;
    }
...
bool DawnCaps::isRenderable(const TextureInfo& info) const
{
    ...
    return info.isValid() && (spec.fUsage & wgpu::TextureUsage::RenderAttachment) && this->isRenderable(spec.getViewFormat(), info.numSamples());
}
...
TextureInfo DawnCaps::getDefaultSampledTextureInfo(SkColorType colorType, Mipmapped mipmapped, Protected, Renderable renderable) const
{
    wgpu::TextureUsage usage = wgpu::TextureUsage::TextureBinding | wgpu::TextureUsage::CopyDst | wgpu::TextureUsage::CopySrc;
...
TextureInfo DawnCaps::getTextureInfoForSampledCopy(const TextureInfo& textureInfo, Mipmapped mipmapped) const
{
    DawnTextureInfo info;
    ...
    info.fSampleCount = 1;
    info.fMipmapped = mipmapped;
    info.fUsage = wgpu::TextureUsage::TextureBinding | wgpu::TextureUsage::CopyDst | wgpu::TextureUsage::CopySrc;
...
TextureInfo DawnCaps::getDefaultCompressedTextureInfo(SkTextureCompressionType compression, Mipmapped mipmapped, Protected) const
{
    wgpu::TextureUsage usage = wgpu::TextureUsage::TextureBinding | wgpu::TextureUsage::CopyDst | wgpu::TextureUsage::CopySrc;
...
TextureInfo DawnCaps::getDefaultMSAATextureInfo(const TextureInfo& singleSampledInfo, Discardable discardable) const
{
    ...
    if (fSupportedTransientAttachmentUsage != wgpu::TextureUsage::None && discardable == Discardable::kYes) {
...
TextureInfo DawnCaps::getDefaultDepthStencilTextureInfo(SkEnumBitMask<DepthStencilFlags> depthStencilType, uint32_t sampleCount, Protected) const
{
    DawnTextureInfo info;
...
TextureInfo DawnCaps::getDefaultStorageTextureInfo(SkColorType colorType) const
{
    ...
    wgpu::TextureUsage usage = wgpu::TextureUsage::StorageBinding | wgpu::TextureUsage::TextureBinding | wgpu::TextureUsage::CopySrc;
    DawnTextureInfo info;
    info.fSampleCount = 1;
    info.fMipmapped = Mipmapped::kNo;
    info.fFormat = format;
...
SkISize DawnCaps::getDepthAttachmentDimensions(const TextureInfo& textureInfo, const SkISize colorAttachmentDimensions) const
{
#if !defined(__EMSCRIPTEN__)
    ...
    if (IsMultiplanarFormat(format) && dawnTextureSpec.fAspect == wgpu::TextureAspect::Plane1Only) {
        ...
        return SkISize::Make(colorAttachmentDimensions.width() * 2, colorAttachmentDimensions.height() * 2);
    }
#endif
...
const Caps::ColorTypeInfo* DawnCaps::getColorTypeInfo(SkColorType colorType, const TextureInfo& textureInfo) const
{
...
void DawnCaps::initCaps(const DawnBackendContext& backendContext, const ContextOptions& options)
{
    ...
    fResourceBindingReqs.fIntrinsicBufferBinding = DawnGraphicsPipeline::kIntrinsicUniformBufferIndex;
    fResourceBindingReqs.fRenderStepBufferBinding = DawnGraphicsPipeline::kRenderStepUniformBufferIndex;
    ...
    fStorageBufferSupport = info.backendType != wgpu::BackendType::D3D11 && info.backendType != wgpu::BackendType::OpenGL
        && info.backendType != wgpu::BackendType::OpenGLES && info.backendType != wgpu::BackendType::Vulkan;
#else
    ...
    fDrawBufferCanBeMapped = backendContext.fDevice.HasFeature(wgpu::FeatureName::BufferMapExtendedUsages);

    fMSAARenderToSingleSampledSupport = backendContext.fDevice.HasFeature(wgpu::FeatureName::MSAARenderToSingleSampled);
    ...
    fSupportsPartialLoadResolve = backendContext.fDevice.HasFeature(wgpu::FeatureName::DawnPartialLoadResolveTexture);
#endif
...
void DawnCaps::initFormatTable(const wgpu::Device& device)
{
    ...
    this->setColorType(kRGB_888x_SkColorType, { wgpu::TextureFormat::RGBA8Unorm, wgpu::TextureFormat::BGRA8Unorm });
...
{
    static_assert(std::size(kFormats) <= kFormatCount, "Size is not compatible for DawnCaps::fFormatTable and kFormats");
...
uint32_t DawnCaps::getRenderPassDescKeyForPipeline(const RenderPassDesc& renderPassDesc) const
{
    ...
    uint32_t colorFormatIndex = GetFormatIndex(TextureInfos::GetDawnTextureSpec(colorInfo).getViewFormat());
    uint32_t depthStencilFormatIndex
        = depthStencilInfo.isValid() ? GetFormatIndex(TextureInfos::GetDawnTextureSpec(depthStencilInfo).getViewFormat()) : kUnusedAttachmentIndex;
    ...
    if (shouldIncludeLoadResolveAttachmentBit && renderPassDesc.fColorResolveAttachment.fTextureInfo.isValid()
    ...
    return (colorFormatIndex << kColorFormatOffset) | (colorInfo.numSamples() << kColorNumSamplesOffset)
        | (depthStencilFormatIndex << kDepthStencilFormatOffset) | (depthStencilInfo.numSamples() << kDepthStencilNumSamplesOffset) | loadResolveAttachmentKey;
}

static constexpr int kDawnGraphicsPipelineKeyData32Count = 4;

UniqueKey DawnCaps::makeGraphicsPipelineKey(const GraphicsPipelineDesc& pipelineDesc, const RenderPassDesc& renderPassDesc) const
{
    UniqueKey pipelineKey;
    {
        // 4 uint32_t's (render step id, paint id, uint32 RenderPassDesc, uint16 write swizzle key)
        UniqueKey::Builder builder(&pipelineKey, get_pipeline_domain(), kDawnGraphicsPipelineKeyData32Count, "DawnGraphicsPipeline");
...
bool DawnCaps::extractGraphicsDescs(
    const UniqueKey& key, GraphicsPipelineDesc* pipelineDesc, RenderPassDesc* renderPassDesc, const RendererProvider* rendererProvider) const
{
    ...
    *pipelineDesc = GraphicsPipelineDesc(renderStep, UniquePaintParamsID(rawKeyData[1]));
    ...
    uint32_t depthStencilFormatIndex = (renderpassDescBits >> kDepthStencilFormatOffset) & kFormatMask;
    ...
    SkDEBUGCODE(uint32_t dsSampleCount = (renderpassDescBits >> kDepthStencilNumSamplesOffset) & kNumSamplesMask;) SkASSERT(colorSampleCount == dsSampleCount);
    ...
    *renderPassDesc = RenderPassDesc::Make(this, TextureInfos::MakeDawn(dawnInfo), loadOp, StoreOp::kStore, dsFlags,
        /* clearColor= */ { .0f, .0f, .0f, .0f }, requiresMSAA, writeSwizzle);
...
uint32_t non_format_info_as_uint32(const wgpu::YCbCrVkDescriptor& desc)
{
    ...
    SkASSERT(static_cast<uint32_t>(desc.forceExplicitReconstruction) < (1u << kForceExplicitReconBits));

    return (((uint32_t)(DawnDescriptorUsesExternalFormat(desc)) << kUsesExternalFormatShift) | ((uint32_t)(desc.vkYCbCrModel) << kYcbcrModelShift)
        | ((uint32_t)(desc.vkYCbCrRange) << kYcbcrRangeShift) | ((uint32_t)(desc.vkXChromaOffset) << kXChromaOffsetShift)
        | ((uint32_t)(desc.vkYChromaOffset) << kYChromaOffsetShift) | ((uint32_t)(desc.vkChromaFilter) << kChromaFilterShift)
        | ((uint32_t)(desc.forceExplicitReconstruction) << kForceExplicitReconShift) | ((uint32_t)(desc.vkComponentSwizzleRed) << kComponentRShift)
        | ((uint32_t)(desc.vkComponentSwizzleGreen) << kComponentGShift) | ((uint32_t)(desc.vkComponentSwizzleBlue) << kComponentBShift)
...
#if !defined(__EMSCRIPTEN__)
    if (proxy) {
        const wgpu::YCbCrVkDescriptor& ycbcrConversionInfo = TextureInfos::GetDawnTextureSpec(proxy->textureInfo()).fYcbcrVkDescriptor;
        ...
        if (ycbcrUtils::DawnDescriptorIsValid(ycbcrConversionInfo)) {
            ...
            immutableSamplerInfo.fFormat = (ycbcrConversionInfo.vkFormat == 0) ? ycbcrConversionInfo.externalFormat : ycbcrConversionInfo.vkFormat;
            immutableSamplerInfo.fNonFormatYcbcrConversionInfo = non_format_info_as_uint32(ycbcrConversionInfo);
...
void DawnCaps::buildKeyForTexture(SkISize dimensions, const TextureInfo& info, ResourceType type, Shareable shareable, GraphiteResourceKey* key) const
{
    ...
    if ((hasYcbcrInfo = ycbcrUtils::DawnDescriptorIsValid(dawnSpec.fYcbcrVkDescriptor))) {
        num32DataCnt += ycbcrUtils::DawnDescriptorUsesExternalFormat(dawnSpec.fYcbcrVkDescriptor) ? SamplerDesc::kInt32sNeededExternalFormat
    ...
    builder[2] = formatKey;
    builder[3] = (samplesKey << 0) | (static_cast<uint32_t>(isMipped) << 3) | (static_cast<uint32_t>(dawnSpec.fUsage) << 4);
