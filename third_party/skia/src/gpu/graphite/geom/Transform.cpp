...
Rect map_rect(Transform::Type type, const SkM44& m, const Rect& r)
{
    ...
    case Transform::Type::kSimpleRectStaysRect:
        ...
        return Rect::FromVals(scale_translate_rect(r.vals(), m.rc(0, 0), m.rc(1, 1), m.rc(0, 3), m.rc(1, 3)));
    case Transform::Type::kRectStaysRect: {
        ...
        if (m.rc(0, 0) == 0.f) {
            ...
            xformed = skvx::shuffle<1, 0, 3, 2>(scale_translate_rect(xformed, m.rc(1, 0), m.rc(0, 1), m.rc(1, 3), m.rc(0, 3)));
        } else {
...
std::pair<float, float> compute_svd(float m00, float m01, float m10, float m11)
{
    ...
    return { SkScalarSqrt(0.5f * (s1 - s2)), SkScalarSqrt(0.5f * (s1 + s2)) };
}
    ...
    : fM(m)
{
    ...
    if (m.row(3) != kNoPerspective) {
        ...
        if (m.invert(&fInvM)) {
            ...
            std::tie(fMinScaleFactor, fMaxScaleFactor) = compute_svd(m.rc(0, 0), m.rc(0, 1), m.rc(1, 0), m.rc(1, 1));
        } else {
            fType = Type::kInvalid;
        }
        return;
    ...
    if (kx == 0.f && ky == 0.f) {
        ...
        if (sx == 0.f || sy == 0.f) {
            ...
            fType = sx > 0.f && sy > 0.f ? Type::kSimpleRectStaysRect : Type::kRectStaysRect;
            fInvM = SkM44(ix, 0.f, 0.f, -ix * tx, 0.f, iy, 0.f, -iy * ty, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f);
        ...
        if (kx == 0.f || ky == 0.f) {
            ...
            fInvM = SkM44(0.f, iy, 0.f, -iy * ty, ix, 0.f, 0.f, -ix * tx, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f);
        ...
        if (SkInvert2x2Matrix(upper, invUpper) == 0.f) {
            ...
            fType = Type::kAffine;
            fInvM = SkM44(invUpper[0], invUpper[2], 0.f, -invUpper[0] * tx - invUpper[2] * ty, invUpper[1], invUpper[3], 0.f,
                -invUpper[1] * tx - invUpper[3] * ty, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f);
...
{
    SkASSERT(this->valid());
    SkV2 localCorners[4] = { { localRect.left(), localRect.top() }, { localRect.right(), localRect.top() }, { localRect.right(), localRect.bot() },
