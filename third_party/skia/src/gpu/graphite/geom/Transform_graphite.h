...
public:
    ...
    {
        return Transform(SkM44(SkM44::kNaN_Constructor), SkM44(SkM44::kNaN_Constructor), Type::kInvalid, 1.f, 1.f);
    }
    ...
    static inline Transform Translate(float x, float y)
    {
        if (x == 0.f && y == 0.f) {
            ...
            return Transform(SkM44::Translate(x, y), SkM44::Translate(-x, -y), Type::kSimpleRectStaysRect, 1.f, 1.f);
        } else {
            return Invalid();
        }
    }
...
private:
    ...
    constexpr Transform(const SkM44& m, const SkM44& invM, Type type, float minScale, float maxScale)
        : fM(m)
        , fInvM(invM)
        , fType(type)
