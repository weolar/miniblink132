...
class Geometry {
public:
    enum class Type : uint8_t { kEmpty, kShape, kVertices, kSubRun, kEdgeAAQuad, kCoverageMaskShape, kAnalyticBlur };
    ...
    explicit Geometry(const AnalyticBlurMask& blur)
    {
        this->setAnalyticBlur(blur);
    }
    ...
    {
        return fType == Type::kAnalyticBlur;
    }
    bool isEmpty() const
    {
        return fType == (Type::kEmpty) || (this->isShape() && this->shape().isEmpty() && !this->shape().inverted());
    }
    ...
    {
        SkASSERT(this->isAnalyticBlur());
        return fAnalyticBlurMask;
    }
    ...
    Rect bounds() const
    {
        ...
        case Type::kAnalyticBlur:
            return fAnalyticBlurMask.drawBounds();
        }
        SkUNREACHABLE;
    }
