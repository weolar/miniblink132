...
public:
    ...
    AI Rect(float l, float t, float r, float b)
        : fVals(NegateBotRight({ l, t, r, b }))
    {
    }
    AI Rect(float2 topLeft, float2 botRight)
        : fVals(topLeft, -botRight)
    {
    }
    AI Rect(const SkRect& r)
        : fVals(NegateBotRight(float4::Load(r.asScalars())))
    {
    }
    ...
    AI static Rect FromVals(float4 vals)
    { // vals.zw must already be negated.
        return Rect(vals);
    }
    ...
    {
        return all(fVals == rect.fVals);
    }
    AI bool operator!=(Rect rect) const
    {
        return any(fVals != rect.fVals);
    }
    ...
    {
        return fVals - outset.xyxy();
    }
    AI Rect makeOffset(float2 offset) const
    {
        return fVals + float4(offset, -offset);
    }
    AI Rect makeJoin(Rect rect) const
    {
        return min(fVals, rect.fVals);
    }
    AI Rect makeIntersect(Rect rect) const
    {
        return max(fVals, rect.fVals);
    }
    AI Rect makeSorted() const
    {
        return min(fVals, -fVals.zwxy());
    }

    AI Rect& roundIn()
    {
        return *this = this->makeRoundIn();
    }
    AI Rect& roundOut()
    {
        return *this = this->makeRoundOut();
    }
    AI Rect& round()
    {
        return *this = this->makeRound();
    }
    AI Rect& inset(float inset)
    {
        return *this = this->makeInset(inset);
    }
    ...
    {
        return *this = this->makeOutset(outset);
    }
    ...
    {
        return *this = this->makeOffset(offset);
    }
    AI Rect& join(Rect rect)
    {
        return *this = this->makeJoin(rect);
    }
    AI Rect& intersect(Rect rect)
    {
        return *this = this->makeIntersect(rect);
    }
    AI Rect& sort()
    {
        return *this = this->makeSorted();
    }

private:
    AI static float4 NegateBotRight(float4 vals)
    { // Returns [vals.xy, -vals.zw].
