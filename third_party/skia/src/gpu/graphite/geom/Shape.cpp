...
Shape& Shape::operator=(const Shape& shape)
{
    ...
    case Type::kEmpty:
        ...
        break;
    case Type::kLine:
        this->setLine(shape.p0(), shape.p1());
        break;
    case Type::kRect:
...
bool Shape::conservativeContains(const Rect& rect) const
{
    ...
    case Type::kRRect:
        return fRRect.contains(rect.asSkRect());
    case Type::kPath:
...
bool Shape::conservativeContains(skvx::float2 point) const
{
    ...
    case Type::kLine:
        return false;
    case Type::kRect:
        return fRect.contains(Rect::Point(point));
    case Type::kRRect:
        return SkRRectPriv::ContainsPoint(fRRect, { point.x(), point.y() });
    case Type::kPath:
...
Rect Shape::bounds() const
{
    ...
    case Type::kEmpty:
        return Rect(0, 0, 0, 0);
    case Type::kLine:
        return fRect.makeSorted(); // sorting corners computes bbox of segment
    case Type::kRect:
        return fRect; // assuming it's sorted
    case Type::kRRect:
...
SkPath Shape::asPath() const
{
    ...
    switch (fType) {
    case Type::kEmpty: /* do nothing */
        break;
    case Type::kLine:
        builder.moveTo(fRect.left(), fRect.top()).lineTo(fRect.right(), fRect.bot());
        break;
    case Type::kRect:
        builder.addRect(fRect.asSkRect());
        break;
    case Type::kRRect:
        builder.addRRect(fRRect);
        break;
    case Type::kPath:
    case Type::kArc:
        SkUNREACHABLE;
...
uint32_t Shape::stateKey(bool includeInverted) const
{
    ...
    if (includeInverted) {
        ...
        key = this->isPath() ? static_cast<uint32_t>(fPath.getFillType()) : (fInverted ? 1 : 0);
    } else {
        // Use the path's noninverted fill type.
        key = this->isPath() ? static_cast<uint32_t>(noninverted_fill_type(fPath.getFillType())) : 0;
    }
