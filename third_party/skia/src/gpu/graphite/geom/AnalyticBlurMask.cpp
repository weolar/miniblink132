...
std::optional<Rect> outset_bounds(const SkMatrix& localToDevice, float devSigma, const SkRect& srcRect)
{
...
std::optional<AnalyticBlurMask> AnalyticBlurMask::Make(Recorder* recorder, const Transform& localToDeviceTransform, float deviceSigma, const SkRRect& srcRRect)
{
    ...
    if (SkRRectPriv::IsCircle(srcRRect) && localToDevice.isSimilarity()) {
        ...
        const SkRect devRect = { devCenter.x() - devRadius, devCenter.y() - devRadius, devCenter.x() + devRadius, devCenter.y() + devRadius };
        return MakeCircle(recorder, localToDevice, deviceSigma, srcRect, devRect);
    }

    if (devRRectIsValid && SkRRectPriv::IsSimpleCircular(devRRect) && localToDevice.isScaleTranslate()) {
...
std::optional<AnalyticBlurMask> AnalyticBlurMask::MakeRect(Recorder* recorder, const SkMatrix& localToDevice, float devSigma, const SkRect& srcRect)
{
    ...
    if (localToDevice.rectStaysRect()) {
        ...
        devRect = { srcRect.left() * scale.width(), srcRect.top() * scale.height(), srcRect.right() * scale.width(), srcRect.bottom() * scale.height() };
    }
    ...
    if (!recorder->priv().caps()->shaderCaps()->fFloatIs32Bits) {
        ...
        if (std::fabs(devRect.left()) > 16000.0f || std::fabs(devRect.top()) > 16000.0f || std::fabs(devRect.right()) > 16000.0f
            || std::fabs(devRect.bottom()) > 16000.0f) {
            return std::nullopt;
        }
    }
    ...
    sk_sp<TextureProxy> integral = recorder->priv().proxyCache()->findOrCreateCachedProxy(recorder, key, &tableWidth, [](const void* context) {
    ...
    const Rect shapeData = Rect(devRect.left() + threeSigma, devRect.top() + threeSigma, devRect.right() - threeSigma, devRect.bottom() - threeSigma);
    ...
    return AnalyticBlurMask(*drawBounds, SkM44(devToScaledShape), ShapeType::kRect, shapeData, { static_cast<float>(isFast), invSixSigma }, integral);
}
...
std::optional<AnalyticBlurMask> AnalyticBlurMask::MakeCircle(
    Recorder* recorder, const SkMatrix& localToDevice, float devSigma, const SkRect& srcRect, const SkRect& devRect)
{
    ...
    sk_sp<TextureProxy> profile = recorder->priv().proxyCache()->findOrCreateCachedProxy(recorder, key, &params, [](const void* context) {
        ...
        if (params->fUseHalfPlaneApprox) {
            ...
            return CreateCircleProfile(params->fQuantizedDevSigma * scale, params->fQuantizedRadius * scale, kProfileTextureWidth);
        }
    });
    ...
    const Rect shapeData = Rect(devRect.centerX(), devRect.centerY(), 1.0f / params.fTextureRadius, (params.fSolidRadius - 0.5f) / params.fTextureRadius);

    // Determine how much to outset the draw bounds to ensure we hit pixels within 3*sigma.
    std::optional<Rect> drawBounds = outset_bounds(localToDevice, params.fQuantizedDevSigma, srcRect);
    if (!drawBounds) {
        return std::nullopt;
    }

    constexpr float kUnusedBlurData = 0.0f;
    return AnalyticBlurMask(*drawBounds, SkM44(), ShapeType::kCircle, shapeData, { kUnusedBlurData, kUnusedBlurData }, profile);
}

std::optional<AnalyticBlurMask> AnalyticBlurMask::MakeRRect(
    Recorder* recorder, const SkMatrix& localToDevice, float devSigma, const SkRRect& srcRRect, const SkRRect& devRRect)
{
    ...
    params.fDimensions = SkISize::Make(newRRWidth + 2 * devBlurRadius, newRRHeight + 2 * devBlurRadius);
    ...
    sk_sp<TextureProxy> ninePatch = recorder->priv().proxyCache()->findOrCreateCachedProxy(recorder, key, &params, [](const void* context) {
        const DerivedParams* params = static_cast<const DerivedParams*>(context);
        return CreateRRectBlurMask(params->fRRectToDraw, params->fDimensions, params->fDevSigma);
    });
    ...
    constexpr float kUnusedBlurData = 0.0f;
    return AnalyticBlurMask(*drawBounds, SkM44(), ShapeType::kRRect, shapeData, { edgeSize, kUnusedBlurData }, ninePatch);
}
