...
private:
    ...
    skvx::int4 getGridCoords(const Rect& bounds) const
    {
        ...
        return pin(skvx::cast<int32_t>(bounds.ltrb() * skvx::float2(fScaleX, fScaleY).xyxy()), skvx::int4(0), skvx::int2(fGridWidth, fGridHeight).xyxy() - 1);
    }
...
public:
    ...
    {
        SkASSERT(deviceSize.width() >= 1 && deviceSize.height() >= 1 && gridCellSize >= 1 && maxBruteForceN >= 1);
    }
...
private:
    ...
    {
        if (fCurrentManager == fGridManager.get() || fBruteForceManager.count() < fMaxBruteForceN) {
