...
template <IntersectionTree::SplitType kSplitType> class IntersectionTree::TreeNode final : public Node {
public:
    TreeNode(float splitCoord, Node* lo, Node* hi)
        : fSplitCoord(splitCoord)
        , fLo(lo)
        , fHi(hi)
    {
    }
...
private:
    ...
    IntersectionTree::Node* split(SkArenaAlloc* arena)
    {
        ...
        return (splitType == SplitType::kX) ? (Node*)arena->make<TreeNode<SplitType::kX>>(splitCoord, this, hiNode)
