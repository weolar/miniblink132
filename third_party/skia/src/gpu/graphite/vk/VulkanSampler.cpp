...
VulkanSampler::VulkanSampler(const VulkanSharedContext* sharedContext, const SamplerDesc& desc, VkSampler sampler, sk_sp<VulkanYcbcrConversion> ycbcrConversion)
...
sk_sp<VulkanSampler> VulkanSampler::Make(const VulkanSharedContext* sharedContext, const SamplerDesc& desc, sk_sp<VulkanYcbcrConversion> ycbcrConversion)
{
    ...
    samplerInfo.maxLod = (desc.samplingOptions().mipmap == SkMipmapMode::kNone) ? 0.0f : VK_LOD_CLAMP_NONE;
    samplerInfo.borderColor = VK_BORDER_COLOR_FLOAT_TRANSPARENT_BLACK;
    samplerInfo.unnormalizedCoordinates = VK_FALSE;

    VkSampler sampler;
    VkResult result;
    VULKAN_CALL_RESULT(sharedContext, result, CreateSampler(sharedContext->device(), &samplerInfo, nullptr, &sampler));
    if (result != VK_SUCCESS) {
        return nullptr;
    }

    return sk_sp<VulkanSampler>(new VulkanSampler(sharedContext, desc, sampler, std::move(ycbcrConversion)));
}
...
void VulkanSampler::freeGpuData()
{
    ...
    SkASSERT(fSampler);
    VULKAN_CALL(sharedContext->interface(), DestroySampler(sharedContext->device(), fSampler, nullptr));
