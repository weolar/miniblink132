...
std::unique_ptr<CommandBuffer> VulkanQueueManager::getNewCommandBuffer(ResourceProvider* resourceProvider, Protected isProtected)
{
    VulkanResourceProvider* vkResourceProvider = static_cast<VulkanResourceProvider*>(resourceProvider);

    auto cmdBuffer = VulkanCommandBuffer::Make(this->vkSharedContext(), vkResourceProvider, isProtected);
    return cmdBuffer;
}
...
QueueManager::OutstandingSubmission VulkanQueueManager::onSubmitToGpu()
{
    ...
    VulkanCommandBuffer* vkCmdBuffer = static_cast<VulkanCommandBuffer*>(fCurrentCommandBuffer.get());
    if (!vkCmdBuffer->submit(fQueue)) {
        fCurrentCommandBuffer->callFinishedProcs(/*success=*/false);
        return nullptr;
    }

    std::unique_ptr<GpuWorkSubmission> submission(new VulkanWorkSubmission(std::move(fCurrentCommandBuffer), this));
    return submission;
}
