...
public:
    ...
    {
        return skgpu::BackendApi::kVulkan;
    }
#endif
    ...
    VulkanAlloc memoryAllocator() const
    {
        return fMemoryAlloc;
    }
    sk_sp<skgpu::MutableTextureState> mutableState() const
    {
...
BackendTexture MakeVulkan(
    SkISize dimensions, const VulkanTextureInfo& info, VkImageLayout layout, uint32_t queueFamilyIndex, VkImage image, VulkanAlloc vulkanMemoryAllocation)
{
    return BackendTexturePriv::Make(dimensions, TextureInfos::MakeVulkan(info),
        VulkanBackendTextureData(
            vulkanMemoryAllocation, sk_make_sp<skgpu::MutableTextureState>(skgpu::MutableTextureStates::MakeVulkan(layout, queueFamilyIndex)), image));
}
