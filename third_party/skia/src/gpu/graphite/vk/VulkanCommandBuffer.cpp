...
public:
    BindBufferInfo add(VulkanCommandBuffer* cb, UniformDataBlock intrinsicValues)
    {
        ...
        const uint32_t stride = SkAlignTo(intrinsicValues.size(), caps->requiredUniformBufferAlignment());
        if (!fCurrentBuffer || fSlotsUsed == kNumSlots) {
            VulkanResourceProvider* resourceProvider = cb->fResourceProvider;
            sk_sp<Buffer> buffer
                = resourceProvider->findOrCreateBuffer(stride * kNumSlots, BufferType::kUniform, AccessPattern::kHostVisible, "IntrinsicConstantBuffer");
            if (!buffer) {
                return {};
            }
        ...
        BindBufferInfo binding { fCurrentBuffer.get(), offset, SkTo<uint32_t>(intrinsicValues.size()) };
...
private:
    ...
    skia_private::THashMap<UniformDataBlock, BindBufferInfo, UniformDataBlock::Hash> fCachedIntrinsicValues;
    SkArenaAlloc fUniformData { 0 };
};

std::unique_ptr<VulkanCommandBuffer> VulkanCommandBuffer::Make(
    const VulkanSharedContext* sharedContext, VulkanResourceProvider* resourceProvider, Protected isProtected)
{
    ...
    VkResult result;
    VkCommandPool pool;
    VULKAN_CALL_RESULT(sharedContext, result, CreateCommandPool(sharedContext->device(), &cmdPoolInfo, nullptr, &pool));
    ...
    VULKAN_CALL_RESULT(sharedContext, result, AllocateCommandBuffers(sharedContext->device(), &cmdInfo, &primaryCmdBuffer));
    if (result != VK_SUCCESS) {
        VULKAN_CALL(sharedContext->interface(), DestroyCommandPool(sharedContext->device(), pool, nullptr));
        return nullptr;
    }

    return std::unique_ptr<VulkanCommandBuffer>(new VulkanCommandBuffer(pool, primaryCmdBuffer, sharedContext, resourceProvider, isProtected));
}

VulkanCommandBuffer::VulkanCommandBuffer(VkCommandPool pool, VkCommandBuffer primaryCommandBuffer, const VulkanSharedContext* sharedContext,
    VulkanResourceProvider* resourceProvider, Protected isProtected)
...
void VulkanCommandBuffer::begin()
{
    ...
    VULKAN_CALL_ERRCHECK(fSharedContext, BeginCommandBuffer(fPrimaryCommandBuffer, &cmdBufferBeginInfo));
...
static VkResult submit_to_queue(const VulkanSharedContext* sharedContext, VkQueue queue, VkFence fence, uint32_t waitCount, const VkSemaphore* waitSemaphores,
    const VkPipelineStageFlags* waitStages, uint32_t commandBufferCount, const VkCommandBuffer* commandBuffers, uint32_t signalCount,
    const VkSemaphore* signalSemaphores, Protected protectedContext)
{
...
bool VulkanCommandBuffer::submit(VkQueue queue)
{
    ...
    if (fSubmitFence == VK_NULL_HANDLE) {
        ...
        VULKAN_CALL_RESULT(fSharedContext, err, CreateFence(device, &fenceInfo, nullptr, &fSubmitFence));
        if (err) {
    ...
    VkResult submitResult = submit_to_queue(fSharedContext, queue, fSubmitFence, waitCount, fWaitSemaphores.data(), vkWaitStages.data(),
        /*commandBufferCount*/ 1, &fPrimaryCommandBuffer, fSignalSemaphores.size(), fSignalSemaphores.data(), this->isProtected());
    ...
    if (submitResult != VK_SUCCESS) {
        ...
        if (submitResult == VK_ERROR_DEVICE_LOST) {
            ...
            SkASSERT(submitResult == VK_ERROR_OUT_OF_HOST_MEMORY || submitResult == VK_ERROR_OUT_OF_DEVICE_MEMORY);
        }
...
bool VulkanCommandBuffer::isFinished()
{
    ...
    VkResult err;
    VULKAN_CALL_RESULT_NOCHECK(fSharedContext->interface(), err, GetFenceStatus(fSharedContext->device(), fSubmitFence));
    switch (err) {
    case VK_SUCCESS:
...
void VulkanCommandBuffer::waitUntilFinished()
{
    ...
    VULKAN_CALL_ERRCHECK(fSharedContext,
        WaitForFences(fSharedContext->device(), 1, &fSubmitFence,
            /*waitAll=*/true,
            /*timeout=*/UINT64_MAX));
}
...
bool VulkanCommandBuffer::updateIntrinsicUniforms(SkIRect viewport)
{
    ...
    BindBufferInfo binding = fIntrinsicConstants->add(this, UniformDataBlock::Wrap(&intrinsicValues));
    if (!binding) {
        return false;
    } else if (binding == fUniformBuffersToBind[VulkanGraphicsPipeline::kIntrinsicUniformBufferIndex]) {
...
bool VulkanCommandBuffer::onAddRenderPass(const RenderPassDesc& renderPassDesc, SkIRect renderPassBounds, const Texture* colorTexture,
    const Texture* resolveTexture, const Texture* depthStencilTexture, SkIRect viewport, const DrawPassList& drawPasses)
{
    for (const auto& drawPass : drawPasses) {
        ...
        for (const sk_sp<TextureProxy>& textureProxy : sampledTextureProxies) {
            VulkanTexture* vulkanTexture = const_cast<VulkanTexture*>(static_cast<const VulkanTexture*>(textureProxy->texture()));
            vulkanTexture->setImageLayout(
                this, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_SHADER_READ_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, false);
        }
    }
    if (fDstCopy.first) {
        VulkanTexture* vulkanTexture = const_cast<VulkanTexture*>(static_cast<const VulkanTexture*>(fDstCopy.first));
        vulkanTexture->setImageLayout(this, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_SHADER_READ_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, false);
    }
    ...
    if (!this->beginRenderPass(renderPassDesc, renderPassBounds, colorTexture, resolveTexture, depthStencilTexture)) {
        return false;
    }
...
bool VulkanCommandBuffer::updateAndBindLoadMSAAInputAttachment(const VulkanTexture& resolveTexture)
{
    ...
    STArray<1, DescriptorData> inputDescriptors = { VulkanGraphicsPipeline::kInputAttachmentDescriptor };
    sk_sp<VulkanDescriptorSet> set
        = fResourceProvider->findOrCreateDescriptorSet(SkSpan<DescriptorData> { &inputDescriptors.front(), inputDescriptors.size() });
    if (!set) {
        return false;
    }
    ...
    textureInfo.imageView = resolveTexture.getImageView(VulkanImageView::Usage::kAttachment)->imageView();
...
bool VulkanCommandBuffer::loadMSAAFromResolve(
    const RenderPassDesc& renderPassDesc, VulkanTexture& resolveTexture, SkISize dstDimensions, const SkIRect nativeDrawBounds)
{
    sk_sp<VulkanGraphicsPipeline> loadPipeline = fResourceProvider->findOrCreateLoadMSAAPipeline(renderPassDesc);
    if (!loadPipeline) {
...
namespace {
void setup_texture_layouts(
    VulkanCommandBuffer* cmdBuf, VulkanTexture* colorTexture, VulkanTexture* resolveTexture, VulkanTexture* depthStencilTexture, bool loadMSAAFromResolve)
{
    if (colorTexture) {
        colorTexture->setImageLayout(cmdBuf, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
        ...
        if (resolveTexture) {
            if (loadMSAAFromResolve) {
                ...
                resolveTexture->setImageLayout(cmdBuf, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                    VK_ACCESS_INPUT_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_READ_BIT,
                    VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT | VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                    /*byRegion=*/false);
            } else {
                resolveTexture->setImageLayout(cmdBuf, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                    VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                    /*byRegion=*/false);
            }
        }
    ...
    if (depthStencilTexture) {
        depthStencilTexture->setImageLayout(cmdBuf, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT,
            VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT,
            /*byRegion=*/false);
    }
}

void track_attachments(VulkanCommandBuffer* cmdBuf, VulkanTexture* colorTexture, VulkanTexture* resolveTexture, VulkanTexture* depthStencilTexture)
{
    if (colorTexture) {
...
void gather_attachment_views(
    skia_private::TArray<VkImageView>& attachmentViews, VulkanTexture* colorTexture, VulkanTexture* resolveTexture, VulkanTexture* depthStencilTexture)
{
    if (colorTexture) {
        VkImageView& colorAttachmentView = attachmentViews.push_back();
        colorAttachmentView = colorTexture->getImageView(VulkanImageView::Usage::kAttachment)->imageView();

        if (resolveTexture) {
            VkImageView& resolveView = attachmentViews.push_back();
            resolveView = resolveTexture->getImageView(VulkanImageView::Usage::kAttachment)->imageView();
        }
    }

    if (depthStencilTexture) {
        VkImageView& stencilView = attachmentViews.push_back();
        stencilView = depthStencilTexture->getImageView(VulkanImageView::Usage::kAttachment)->imageView();
    }
}

void gather_clear_values(STArray<VulkanRenderPass::kMaxExpectedAttachmentCount, VkClearValue>& clearValues, const RenderPassDesc& renderPassDesc,
    VulkanTexture* colorTexture, VulkanTexture* depthStencilTexture, int depthStencilAttachmentIdx)
{
    clearValues.push_back_n(VulkanRenderPass::kMaxExpectedAttachmentCount);
    if (colorTexture) {
        VkClearValue& colorAttachmentClear = clearValues.at(VulkanRenderPass::kColorAttachmentIdx);
        memset(&colorAttachmentClear, 0, sizeof(VkClearValue));
        colorAttachmentClear.color
            = { { renderPassDesc.fClearColor[0], renderPassDesc.fClearColor[1], renderPassDesc.fClearColor[2], renderPassDesc.fClearColor[3] } };
    }
...
VkRect2D get_render_area(const SkIRect& srcBounds, const VkExtent2D& granularity, int maxWidth, int maxHeight)
{
    SkIRect dstBounds;
    // Adjust Width
...
bool VulkanCommandBuffer::beginRenderPass(const RenderPassDesc& renderPassDesc, SkIRect renderPassBounds, const Texture* colorTexture,
    const Texture* resolveTexture, const Texture* depthStencilTexture)
{
    ...
    VulkanTexture* vulkanResolveTexture = const_cast<VulkanTexture*>(static_cast<const VulkanTexture*>(resolveTexture));
    VulkanTexture* vulkanDepthStencilTexture = const_cast<VulkanTexture*>(static_cast<const VulkanTexture*>(depthStencilTexture));

    SkASSERT(resolveTexture ? renderPassDesc.fColorResolveAttachment.fStoreOp == StoreOp::kStore : true);

    // Determine if we need to load MSAA from resolve, and if so, make certain that key conditions
    // are met before proceeding.
    bool loadMSAAFromResolve = renderPassDesc.fColorResolveAttachment.fTextureInfo.isValid() && renderPassDesc.fColorResolveAttachment.fLoadOp == LoadOp::kLoad;
    if (loadMSAAFromResolve && (!vulkanResolveTexture || !vulkanColorTexture || !vulkanResolveTexture->supportsInputAttachmentUsage())) {
    ...
    setup_texture_layouts(this, vulkanColorTexture, vulkanResolveTexture, vulkanDepthStencilTexture, loadMSAAFromResolve);
    ...
    gather_attachment_views(attachmentViews, vulkanColorTexture, vulkanResolveTexture, vulkanDepthStencilTexture);
    ...
    gather_clear_values(clearValues, renderPassDesc, vulkanColorTexture, vulkanDepthStencilTexture, depthStencilAttachmentIndex);

    sk_sp<VulkanRenderPass> vulkanRenderPass = fResourceProvider->findOrCreateRenderPass(renderPassDesc, /*compatibleOnly=*/false);
    if (!vulkanRenderPass) {
    ...
    sk_sp<VulkanFramebuffer> framebuffer
        = fResourceProvider->createFramebuffer(fSharedContext, attachmentViews, *vulkanRenderPass, frameBufferWidth, frameBufferHeight);
    if (!framebuffer) {
    ...
    VULKAN_CALL(fSharedContext->interface(), GetRenderAreaGranularity(fSharedContext->device(), vulkanRenderPass->renderPass(), &granularity));

    bool useFullBounds = loadMSAAFromResolve && fSharedContext->vulkanCaps().mustLoadFullImageForMSAA();

    VkRect2D renderArea = get_render_area(
        useFullBounds ? SkIRect::MakeWH(frameBufferWidth, frameBufferHeight) : renderPassBounds, granularity, frameBufferWidth, frameBufferHeight);
    ...
    VULKAN_CALL(fSharedContext->interface(), CmdBeginRenderPass(fPrimaryCommandBuffer, &beginInfo, VK_SUBPASS_CONTENTS_INLINE));
    fActiveRenderPass = true;

    SkIRect nativeBounds = SkIRect::MakeXYWH(renderArea.offset.x, renderArea.offset.y, renderArea.extent.width, renderArea.extent.height);
    if (loadMSAAFromResolve && !this->loadMSAAFromResolve(renderPassDesc, *vulkanResolveTexture, vulkanColorTexture->dimensions(), nativeBounds)) {
...
void VulkanCommandBuffer::addDrawPass(const DrawPass* drawPass)
{
    ...
    for (auto [type, cmdPtr] : drawPass->commands()) {
        ...
        case DrawPassCommands::Type::kDrawInstanced: {
            ...
            this->drawInstanced(draw->fType, draw->fBaseVertex, draw->fVertexCount, draw->fBaseInstance, draw->fInstanceCount);
            break;
        }
        case DrawPassCommands::Type::kDrawIndexedInstanced: {
            auto draw = static_cast<DrawPassCommands::DrawIndexedInstanced*>(cmdPtr);
            this->drawIndexedInstanced(draw->fType, draw->fBaseIndex, draw->fIndexCount, draw->fBaseVertex, draw->fBaseInstance, draw->fInstanceCount);
            break;
        }
...
void VulkanCommandBuffer::bindGraphicsPipeline(const GraphicsPipeline* graphicsPipeline)
{
    ...
    VULKAN_CALL(fSharedContext->interface(), CmdBindPipeline(fPrimaryCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, fActiveGraphicsPipeline->pipeline()));
    ...
    if (graphicsPipeline->dstReadRequirement() == DstReadRequirement::kTextureCopy && graphicsPipeline->numFragTexturesAndSamplers() == 1) {
...
void VulkanCommandBuffer::bindUniformBuffers()
{
    ...
    DescriptorType uniformBufferType = fSharedContext->caps()->storageBufferSupport() ? DescriptorType::kStorageBuffer : DescriptorType::kUniformBuffer;
    if (fActiveGraphicsPipeline->hasStepUniforms() && fUniformBuffersToBind[VulkanGraphicsPipeline::kRenderStepUniformBufferIndex].fBuffer) {
        descriptors.push_back({ uniformBufferType,
            /*count=*/1, VulkanGraphicsPipeline::kRenderStepUniformBufferIndex, PipelineStageFlags::kVertexShader | PipelineStageFlags::kFragmentShader });
    }
    if (fActiveGraphicsPipeline->hasPaintUniforms() && fUniformBuffersToBind[VulkanGraphicsPipeline::kPaintUniformBufferIndex].fBuffer) {
        descriptors.push_back({ uniformBufferType,
            /*count=*/1, VulkanGraphicsPipeline::kPaintUniformBufferIndex, PipelineStageFlags::kFragmentShader });
    }
    if (fActiveGraphicsPipeline->hasGradientBuffer() && fUniformBuffersToBind[VulkanGraphicsPipeline::kGradientBufferIndex].fBuffer) {
        SkASSERT(fSharedContext->caps()->gradientBufferSupport() && fSharedContext->caps()->storageBufferSupport());
        descriptors.push_back({ DescriptorType::kStorageBuffer,
            /*count=*/1, VulkanGraphicsPipeline::kGradientBufferIndex, PipelineStageFlags::kFragmentShader });
    }

    sk_sp<VulkanDescriptorSet> descSet = fResourceProvider->findOrCreateUniformBuffersDescriptorSet(descriptors, fUniformBuffersToBind);
    if (!descSet) {
        SKGPU_LOG_E("Unable to find or create uniform descriptor set");
        return;
    }
    skia_private::AutoSTMalloc<VulkanGraphicsPipeline::kNumUniformBuffers, uint32_t> dynamicOffsets(descriptors.size());
    ...
    VULKAN_CALL(fSharedContext->interface(),
        CmdBindDescriptorSets(fPrimaryCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, fActiveGraphicsPipeline->layout(),
            ...
            /*setCount=*/1, descSet->descriptorSet(), descriptors.size(), dynamicOffsets.get()));
    this->trackResource(std::move(descSet));
}

void VulkanCommandBuffer::bindDrawBuffers(
    const BindBufferInfo& vertices, const BindBufferInfo& instances, const BindBufferInfo& indices, const BindBufferInfo& indirect)
{
...
void VulkanCommandBuffer::bindVertexBuffers(const Buffer* vertexBuffer, size_t vertexOffset, const Buffer* instanceBuffer, size_t instanceOffset)
{
...
void VulkanCommandBuffer::bindInputBuffer(const Buffer* buffer, VkDeviceSize offset, uint32_t binding)
{
    if (buffer) {
        ...
        if (vkBuffer != fBoundInputBuffers[binding] || offset != fBoundInputBufferOffsets[binding]) {
            VULKAN_CALL(fSharedContext->interface(),
                CmdBindVertexBuffers(fPrimaryCommandBuffer, binding,
...
void VulkanCommandBuffer::bindIndexBuffer(const Buffer* indexBuffer, size_t offset)
{
    if (indexBuffer) {
        ...
        if (vkBuffer != fBoundIndexBuffer || offset != fBoundIndexBufferOffset) {
            VULKAN_CALL(fSharedContext->interface(), CmdBindIndexBuffer(fPrimaryCommandBuffer, vkBuffer, offset, VK_INDEX_TYPE_UINT16));
...
void VulkanCommandBuffer::recordTextureAndSamplerDescSet(const DrawPass* drawPass, const DrawPassCommands::BindTexturesAndSamplers* command)
{
    ...
    if (command) {
        for (int i = 0; i < command->fNumTexSamplers; i++) {
            ...
            const Sampler* immutableSampler = (sampler && sampler->ycbcrConversion()) ? sampler : nullptr;
            descriptors.push_back({ DescriptorType::kCombinedTextureSampler,
                /*count=*/1,
                /*bindingIdx=*/i, PipelineStageFlags::kFragmentShader, immutableSampler });
        }
    }
    ...
    if (fActiveGraphicsPipeline->dstReadRequirement() == DstReadRequirement::kTextureCopy) {
        descriptors.push_back({ DescriptorType::kCombinedTextureSampler,
            /*count=*/1,
            /*bindingIdx=*/numTexSamplers - 1, PipelineStageFlags::kFragmentShader,
            /*immutableSampler=*/nullptr });
    }
    SkASSERT(descriptors.size() == numTexSamplers);
    sk_sp<VulkanDescriptorSet> set = fResourceProvider->findOrCreateDescriptorSet(SkSpan<DescriptorData> { &descriptors.front(), descriptors.size() });
    ...
    if (command) {
        for (int i = 0; i < command->fNumTexSamplers; ++i) {
            auto texture = static_cast<const VulkanTexture*>(drawPass->getTexture(command->fTextureIndices[i]));
            auto sampler = static_cast<const VulkanSampler*>(drawPass->getSampler(command->fSamplerIndices[i]));
    ...
    SkASSERT(writeDescriptorSets.size() == numTexSamplers && descriptorImageInfos.size() == numTexSamplers);
    VULKAN_CALL(fSharedContext->interface(),
        UpdateDescriptorSets(fSharedContext->device(), numTexSamplers, &writeDescriptorSets[0],
            /*descriptorCopyCount=*/0,
            /*pDescriptorCopies=*/nullptr));
...
{
    fBindTextureSamplers = false;
    if (fTextureSamplerDescSetToBind != VK_NULL_HANDLE && fActiveGraphicsPipeline->numFragTexturesAndSamplers() == fNumTextureSamplers) {
        VULKAN_CALL(fSharedContext->interface(),
            CmdBindDescriptorSets(fPrimaryCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, fActiveGraphicsPipeline->layout(),
...
{
    SkIRect scissor = SkIRect::MakeXYWH(left + fReplayTranslation.x(), top + fReplayTranslation.y(), width, height);
    if (!scissor.intersect(SkIRect::MakeSize(fColorAttachmentSize)) || (!fReplayClip.isEmpty() && !scissor.intersect(fReplayClip))) {
        scissor.setEmpty();
    }

    VkRect2D vkScissor = { { scissor.x(), scissor.y() }, { static_cast<unsigned int>(scissor.width()), static_cast<unsigned int>(scissor.height()) } };
    VULKAN_CALL(fSharedContext->interface(),
        CmdSetScissor(fPrimaryCommandBuffer,
            /*firstScissor=*/0,
            /*scissorCount=*/1, &vkScissor));
}
...
void VulkanCommandBuffer::drawIndexed(PrimitiveType, unsigned int baseIndex, unsigned int indexCount, unsigned int baseVertex)
{
    SkASSERT(fActiveRenderPass);
    this->syncDescriptorSets();
    // TODO: set primitive type via dynamic state if available
    VULKAN_CALL(fSharedContext->interface(),
        CmdDrawIndexed(fPrimaryCommandBuffer, indexCount,
            /*instanceCount=*/1, baseIndex, baseVertex,
            /*firstInstance=*/0));
}

void VulkanCommandBuffer::drawInstanced(PrimitiveType, unsigned int baseVertex, unsigned int vertexCount, unsigned int baseInstance, unsigned int instanceCount)
{
    SkASSERT(fActiveRenderPass);
    this->syncDescriptorSets();
    // TODO: set primitive type via dynamic state if available
    VULKAN_CALL(fSharedContext->interface(), CmdDraw(fPrimaryCommandBuffer, vertexCount, instanceCount, baseVertex, baseInstance));
}

void VulkanCommandBuffer::drawIndexedInstanced(
    PrimitiveType, unsigned int baseIndex, unsigned int indexCount, unsigned int baseVertex, unsigned int baseInstance, unsigned int instanceCount)
{
    SkASSERT(fActiveRenderPass);
    this->syncDescriptorSets();
    // TODO: set primitive type via dynamic state if available
    VULKAN_CALL(fSharedContext->interface(), CmdDrawIndexed(fPrimaryCommandBuffer, indexCount, instanceCount, baseIndex, baseVertex, baseInstance));
}
...
bool VulkanCommandBuffer::onAddComputePass(DispatchGroupSpan)
{
    return false;
}

bool VulkanCommandBuffer::onCopyBufferToBuffer(const Buffer* srcBuffer, size_t srcOffset, const Buffer* dstBuffer, size_t dstOffset, size_t size)
{
...
bool VulkanCommandBuffer::onCopyTextureToBuffer(const Texture* texture, SkIRect srcRect, const Buffer* buffer, size_t bufferOffset, size_t bufferRowBytes)
{
    ...
    const_cast<VulkanTexture*>(srcTexture)
        ->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_TRANSFER_READ_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);
    // Set current access mask for buffer
    const_cast<VulkanBuffer*>(dstBuffer)->setBufferAccess(this, VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);

    this->submitPipelineBarriers();

    VULKAN_CALL(fSharedContext->interface(),
        CmdCopyImageToBuffer(fPrimaryCommandBuffer, srcTexture->vkImage(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dstBuffer->vkBuffer(),
            /*regionCount=*/1, &region));
    return true;
}

bool VulkanCommandBuffer::onCopyBufferToTexture(const Buffer* buffer, const Texture* texture, const BufferTextureCopyData* copyData, int count)
{
    ...
    SkISize oneBlockDims = CompressedDimensions(dstTexture->textureInfo().compressionType(), { 1, 1 });
    ...
    for (int i = 0; i < count; ++i) {
        ...
        region.bufferRowLength = (uint32_t)((copyData[i].fBufferRowBytes / bytesPerBlock) * oneBlockDims.fWidth);
        region.bufferImageHeight = 0; // Tightly packed
    ...
    const_cast<VulkanTexture*>(dstTexture)
        ->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);

    this->submitPipelineBarriers();

    VULKAN_CALL(fSharedContext->interface(),
        CmdCopyBufferToImage(
            fPrimaryCommandBuffer, srcBuffer->vkBuffer(), dstTexture->vkImage(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, regions.size(), regions.begin()));
    return true;
}

bool VulkanCommandBuffer::onCopyTextureToTexture(const Texture* src, SkIRect srcRect, const Texture* dst, SkIPoint dstPoint, int mipLevel)
{
    ...
    VULKAN_CALL(fSharedContext->interface(),
        CmdCopyImage(fPrimaryCommandBuffer, srcTexture->vkImage(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dstTexture->vkImage(),
...
bool VulkanCommandBuffer::pushConstants(VkShaderStageFlags stageFlags, uint32_t offset, uint32_t size, const void* values)
{
    SkASSERT(fActiveGraphicsPipeline);
    ...
    VULKAN_CALL(fSharedContext->interface(), CmdPushConstants(fPrimaryCommandBuffer, fActiveGraphicsPipeline->layout(), stageFlags, offset, size, values));
    return true;
}

bool VulkanCommandBuffer::onSynchronizeBufferToCpu(const Buffer* buffer, bool* outDidResultInWork)
{
    static_cast<const VulkanBuffer*>(buffer)->setBufferAccess(this, VK_ACCESS_HOST_READ_BIT, VK_PIPELINE_STAGE_HOST_BIT);
...
void VulkanCommandBuffer::addBufferMemoryBarrier(
...
void VulkanCommandBuffer::addBufferMemoryBarrier(VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask, VkBufferMemoryBarrier* barrier)
{
...
void VulkanCommandBuffer::addImageMemoryBarrier(
    const Resource* resource, VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask, bool byRegion, VkImageMemoryBarrier* barrier)
{
    SkASSERT(resource);
...
void VulkanCommandBuffer::pipelineBarrier(
    const Resource* resource, VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask, bool byRegion, BarrierType barrierType, void* barrier)
{
    ...
    if (barrierType == kImageMemory_BarrierType) {
        ...
        isValidSubpassBarrier = (imgBarrier->newLayout == imgBarrier->oldLayout) && (imgBarrier->srcQueueFamilyIndex == VK_QUEUE_FAMILY_IGNORED)
    ...
    if (!fBufferBarriers.empty() || !fImageBarriers.empty()) {
        ...
        VULKAN_CALL(fSharedContext->interface(),
            CmdPipelineBarrier(fPrimaryCommandBuffer, fSrcStageMask, fDstStageMask, dependencyFlags,
                /*memoryBarrierCount=*/0, /*pMemoryBarrier=*/nullptr, fBufferBarriers.size(), fBufferBarriers.begin(), fImageBarriers.size(),
                fImageBarriers.begin()));
...
void VulkanCommandBuffer::updateBuffer(const VulkanBuffer* buffer, const void* data, size_t dataSize, size_t dstOffset)
{
    ...
    VULKAN_CALL(fSharedContext->interface(), CmdUpdateBuffer(fPrimaryCommandBuffer, buffer->vkBuffer(), dstOffset, dataSize, data));
}
...
void VulkanCommandBuffer::nextSubpass()
{
    ...
    VULKAN_CALL(fSharedContext->interface(), CmdNextSubpass(fPrimaryCommandBuffer, VK_SUBPASS_CONTENTS_INLINE));
}

void VulkanCommandBuffer::setViewport(SkIRect viewport)
{
    VkViewport vkViewport = {
        (float)viewport.fLeft, (float)viewport.fTop, (float)viewport.width(), (float)viewport.height(),
        0.0f, // minDepth
        1.0f, // maxDepth
    };
    VULKAN_CALL(fSharedContext->interface(),
