...
static void setup_vertex_input_state(const SkSpan<const Attribute>& vertexAttrs, const SkSpan<const Attribute>& instanceAttrs,
    VkPipelineVertexInputStateCreateInfo* vertexInputInfo, skia_private::STArray<2, VkVertexInputBindingDescription, true>* bindingDescs,
    skia_private::STArray<16, VkVertexInputAttributeDescription>* attributeDescs)
{
    ...
    if (bindingDescs && !vertexAttrs.empty()) {
        bindingDescs->push_back() = { VulkanGraphicsPipeline::kVertexBufferIndex, (uint32_t)vertexAttributeOffset, VK_VERTEX_INPUT_RATE_VERTEX };
    }
    if (bindingDescs && !instanceAttrs.empty()) {
        bindingDescs->push_back() = { VulkanGraphicsPipeline::kInstanceBufferIndex, (uint32_t)instanceAttributeOffset, VK_VERTEX_INPUT_RATE_INSTANCE };
    }
    ...
    vertexInputInfo->pVertexBindingDescriptions = bindingDescs && !bindingDescs->empty() ? bindingDescs->begin() : VK_NULL_HANDLE;
    vertexInputInfo->vertexAttributeDescriptionCount = attributeDescs ? attributeDescs->size() : 0;
    vertexInputInfo->pVertexAttributeDescriptions = attributeDescs && !attributeDescs->empty() ? attributeDescs->begin() : VK_NULL_HANDLE;
}
...
{
    SkASSERT(stencilSettings.fDepthTestEnabled || stencilSettings.fDepthCompareOp == CompareOp::kAlways);
    ...
    if (stencilSettings.fStencilTestEnabled) {
        setup_stencil_op_state(&stencilInfo->front, stencilSettings.fFrontStencil, stencilSettings.fStencilReferenceValue);
        setup_stencil_op_state(&stencilInfo->back, stencilSettings.fBackStencil, stencilSettings.fStencilReferenceValue);
    }
...
static void setup_multisample_state(int numSamples, VkPipelineMultisampleStateCreateInfo* multisampleInfo)
{
    ...
    SkAssertResult(skgpu::SampleCountToVkSampleCount(numSamples, &multisampleInfo->rasterizationSamples));
...
static void setup_color_blend_state(
    const skgpu::BlendInfo& blendInfo, VkPipelineColorBlendStateCreateInfo* colorBlendInfo, VkPipelineColorBlendAttachmentState* attachmentState)
{
...
static void destroy_desc_set_layouts(const VulkanSharedContext* sharedContext, skia_private::TArray<VkDescriptorSetLayout>& setLayouts)
{
    for (int i = 0; i < setLayouts.size(); i++) {
        if (setLayouts[i] != VK_NULL_HANDLE) {
            VULKAN_CALL(sharedContext->interface(), DestroyDescriptorSetLayout(sharedContext->device(), setLayouts[i], nullptr));
        }
    }
}

static VkPipelineLayout setup_pipeline_layout(const VulkanSharedContext* sharedContext, bool usesIntrinsicConstantUbo, bool useMSAALoadPushConstant,
    bool hasStepUniforms, bool hasPaintUniforms, bool hasGradientBuffer, int numTextureSamplers, int numInputAttachments,
    SkSpan<sk_sp<VulkanSampler>> immutableSamplers)
{
    SkASSERT(!useMSAALoadPushConstant || (!usesIntrinsicConstantUbo && !hasStepUniforms && !hasPaintUniforms));
    ...
    skia_private::STArray<VulkanGraphicsPipeline::kNumUniformBuffers, DescriptorData> uniformDescriptors;
    if (usesIntrinsicConstantUbo) {
        uniformDescriptors.push_back(VulkanGraphicsPipeline::kIntrinsicUniformBufferDescriptor);
    }

    DescriptorType uniformBufferType = sharedContext->caps()->storageBufferSupport() ? DescriptorType::kStorageBuffer : DescriptorType::kUniformBuffer;
    if (hasStepUniforms) {
        uniformDescriptors.push_back({ uniformBufferType,
    ...
    if (hasGradientBuffer) {
        uniformDescriptors.push_back({ DescriptorType::kStorageBuffer,
            /*count=*/1, VulkanGraphicsPipeline::kGradientBufferIndex, PipelineStageFlags::kFragmentShader });
    }

    if (!uniformDescriptors.empty()) {
        VkDescriptorSetLayout uniformSetLayout = descriptor_data_to_layout(sharedContext, { uniformDescriptors });
    ...
    if (numInputAttachments > 0) {
        ...
        VkDescriptorSetLayout inputAttachmentDescSetLayout = descriptor_data_to_layout(sharedContext, { inputAttachmentDescriptors });
    ...
    if (numTextureSamplers > 0) {
        ...
        SkASSERT(immutableSamplers.empty() || SkTo<int>(immutableSamplers.size()) == numTextureSamplers);
        ...
        VkDescriptorSetLayout textureSamplerDescSetLayout = descriptor_data_to_layout(sharedContext, { textureSamplerDescs });
    ...
    VULKAN_CALL_RESULT(sharedContext, result,
        CreatePipelineLayout(sharedContext->device(), &layoutCreateInfo,
            /*const VkAllocationCallbacks*=*/nullptr, &layout));
...
static void destroy_shader_modules(const VulkanSharedContext* sharedContext, VkShaderModule vsModule, VkShaderModule fsModule)
{
    if (vsModule != VK_NULL_HANDLE) {
        VULKAN_CALL(sharedContext->interface(), DestroyShaderModule(sharedContext->device(), vsModule, nullptr));
    }
    if (fsModule != VK_NULL_HANDLE) {
        VULKAN_CALL(sharedContext->interface(), DestroyShaderModule(sharedContext->device(), fsModule, nullptr));
    }
}
...
sk_sp<VulkanGraphicsPipeline> VulkanGraphicsPipeline::Make(VulkanResourceProvider* rsrcProvider, const RuntimeEffectDictionary* runtimeDict,
    const GraphicsPipelineDesc& pipelineDesc, const RenderPassDesc& renderPassDesc, SkEnumBitMask<PipelineCreationFlags> pipelineCreationFlags)
{
    ...
    if (step->vertexAttributes().size() + step->instanceAttributes().size() > sharedContext->vulkanCaps().maxVertexAttributes()) {
        SKGPU_LOG_W("Requested more than the supported number of vertex attributes");
        return nullptr;
    }

    skia_private::TArray<SamplerDesc> descContainer {};
    std::unique_ptr<ShaderInfo> shaderInfo = ShaderInfo::Make(sharedContext->caps(), sharedContext->shaderCodeDictionary(), runtimeDict, step,
        pipelineDesc.paintParamsID(), useStorageBuffers, renderPassDesc.fWriteSwizzle, &descContainer);
    ...
    for (int i = 0; i < descContainer.size(); i++) {
        ...
        if (descContainer.at(i) != SamplerDesc()) {
            sk_sp<Sampler> immutableSampler = rsrcProvider->findOrCreateCompatibleSampler(descContainer.at(i));
            sk_sp<VulkanSampler> vulkanSampler = sk_ref_sp<VulkanSampler>(static_cast<VulkanSampler*>(immutableSampler.get()));
    ...
    if (hasFragmentSkSL) {
        if (!skgpu::SkSLToSPIRV(
                sharedContext->caps()->shaderCaps(), fsSkSL, SkSL::ProgramKind::kGraphiteFragment, settings, &fsSPIRV, &fsInterface, errorHandler)) {
            return nullptr;
        }
    ...
    if (!skgpu::SkSLToSPIRV(sharedContext->caps()->shaderCaps(), vsSkSL, SkSL::ProgramKind::kGraphiteVertex, settings, &vsSPIRV, &vsInterface, errorHandler)) {
        return nullptr;
    }
    ...
    skia_private::STArray<16, VkVertexInputAttributeDescription> attributeDescs;
    setup_vertex_input_state(step->vertexAttributes(), step->instanceAttributes(), &vertexInputInfo, &bindingDescs, &attributeDescs);
    ...
    setup_multisample_state(renderPassDesc.fColorAttachment.fTextureInfo.numSamples(), &multisampleInfo);
    ...
    setup_shader_stage_info(VK_SHADER_STAGE_VERTEX_BIT, vsModule, &pipelineShaderStages[0]);
    if (hasFragmentSkSL) {
        setup_shader_stage_info(VK_SHADER_STAGE_FRAGMENT_BIT, fsModule, &pipelineShaderStages[1]);
    }
    ...
    VkPipelineLayout pipelineLayout = setup_pipeline_layout(sharedContext,
        ...
        /*useMSAALoadPushConstant=*/false, !step->uniforms().empty(), shaderInfo->hasPaintUniforms(), shaderInfo->hasGradientBuffer(),
        shaderInfo->numFragmentTexturesAndSamplers(),
        /*numInputAttachments=*/0, SkSpan<sk_sp<VulkanSampler>>(immutableSamplers));
    ...
    sk_sp<VulkanRenderPass> compatibleRenderPass = rsrcProvider->findOrCreateRenderPass(renderPassDesc, /*compatibleOnly=*/true);
    ...
    VkResult result;
    {
        ...
        VULKAN_CALL_RESULT(sharedContext, result,
            CreateGraphicsPipelines(sharedContext->device(), rsrcProvider->pipelineCache(),
                /*createInfoCount=*/1, &pipelineCreateInfo,
                /*pAllocator=*/nullptr, &vkPipeline));
    }
    ...
    return sk_sp<VulkanGraphicsPipeline>(new VulkanGraphicsPipeline(sharedContext, pipelineInfo, pipelineLayout, vkPipeline,
        /*ownsPipelineLayout=*/true, std::move(immutableSamplers)));
}

bool VulkanGraphicsPipeline::InitializeMSAALoadPipelineStructs(const VulkanSharedContext* sharedContext, VkShaderModule* outVertexShaderModule,
    VkShaderModule* outFragShaderModule, VkPipelineShaderStageCreateInfo* outShaderStageInfo, VkPipelineLayout* outPipelineLayout)
{
    SkSL::Program::Interface vsInterface, fsInterface;
    SkSL::ProgramSettings settings;
    ...
    vertShaderText.append("layout(vulkan,  push_constant) uniform vertexUniformBuffer {"
                          "half4 uPosXform;"
                          "};"
    ...
    fragShaderText.append("layout(vulkan, input_attachment_index=0, set=0, binding=0) subpassInput uInput;"
    ...
    if (!skgpu::SkSLToSPIRV(
            sharedContext->caps()->shaderCaps(), vertShaderText, SkSL::ProgramKind::kGraphiteVertex, settings, &vsSPIRV, &vsInterface, errorHandler)) {
        return false;
    }
    if (!skgpu::SkSLToSPIRV(
            sharedContext->caps()->shaderCaps(), fragShaderText, SkSL::ProgramKind::kGraphiteFragment, settings, &fsSPIRV, &fsInterface, errorHandler)) {
        return false;
    }
    *outFragShaderModule = createVulkanShaderModule(sharedContext, fsSPIRV, VK_SHADER_STAGE_FRAGMENT_BIT);
    if (*outFragShaderModule == VK_NULL_HANDLE) {
        return false;
    }

    *outVertexShaderModule = createVulkanShaderModule(sharedContext, vsSPIRV, VK_SHADER_STAGE_VERTEX_BIT);
    if (*outVertexShaderModule == VK_NULL_HANDLE) {
        destroy_shader_modules(sharedContext, VK_NULL_HANDLE, *outFragShaderModule);
        return false;
    }

    setup_shader_stage_info(VK_SHADER_STAGE_VERTEX_BIT, *outVertexShaderModule, &outShaderStageInfo[0]);

    setup_shader_stage_info(VK_SHADER_STAGE_FRAGMENT_BIT, *outFragShaderModule, &outShaderStageInfo[1]);
...
sk_sp<VulkanGraphicsPipeline> VulkanGraphicsPipeline::MakeLoadMSAAPipeline(const VulkanSharedContext* sharedContext, VkShaderModule vsModule,
    VkShaderModule fsModule, VkPipelineShaderStageCreateInfo* pipelineShaderStages, VkPipelineLayout pipelineLayout,
    sk_sp<VulkanRenderPass> compatibleRenderPass, VkPipelineCache pipelineCache, const TextureInfo& dstColorAttachmentTexInfo)
{
    ...
    setup_vertex_input_state(loadMSAAVertexAttribs,
        ...
        &vertexInputInfo, &bindingDescs, &attributeDescs);
    ...
    VkResult result;
    {
        ...
        VULKAN_CALL_RESULT(sharedContext, result,
            CreateGraphicsPipelines(sharedContext->device(), pipelineCache,
                /*createInfoCount=*/1, &pipelineCreateInfo,
                /*pAllocator=*/nullptr, &vkPipeline));
    }
    if (result != VK_SUCCESS) {
...
VulkanGraphicsPipeline::VulkanGraphicsPipeline(const VulkanSharedContext* sharedContext, const PipelineInfo& pipelineInfo, VkPipelineLayout pipelineLayout,
    VkPipeline pipeline, bool ownsPipelineLayout, skia_private::TArray<sk_sp<VulkanSampler>> immutableSamplers)
...
void VulkanGraphicsPipeline::freeGpuData()
{
    ...
    if (fPipeline != VK_NULL_HANDLE) {
        VULKAN_CALL(sharedCtxt->interface(), DestroyPipeline(sharedCtxt->device(), fPipeline, nullptr));
    }
    if (fOwnsPipelineLayout && fPipelineLayout != VK_NULL_HANDLE) {
        VULKAN_CALL(sharedCtxt->interface(), DestroyPipelineLayout(sharedCtxt->device(), fPipelineLayout, nullptr));
    }
}
