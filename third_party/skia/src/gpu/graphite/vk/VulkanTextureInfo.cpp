...
public:
    VulkanTextureInfoData(VulkanTextureSpec v)
        : fVkSpec(v)
    {
    }
...
private:
    ...
    {
        return VkFormatBytesPerBlock(fVkSpec.fFormat);
    }
    ...
    {
        return SkStringPrintf("Vulkan(f=%u,s=%u)", static_cast<unsigned int>(fVkSpec.fFormat), sampleCount);
    }
...
TextureInfo MakeVulkan(const VulkanTextureInfo& vkInfo)
{
    ...
    return TextureInfoPriv::Make(skgpu::BackendApi::kVulkan, vkInfo.fSampleCount, vkInfo.fMipmapped, p, VulkanTextureInfoData(vkInfo));
}
