...
std::unique_ptr<Context> MakeVulkan(const VulkanBackendContext& backendContext, const ContextOptions& options)
{
    ...
    std::unique_ptr<QueueManager> queueManager(new VulkanQueueManager(backendContext.fQueue, sharedContext.get()));
    if (!queueManager) {
        return nullptr;
    }

    return ContextCtorAccessor::MakeContext(std::move(sharedContext), std::move(queueManager), options);
}
...
VkShaderModule createVulkanShaderModule(const VulkanSharedContext* context, const std::string& spirv, VkShaderStageFlagBits stage)
{
    ...
    VkResult result;
    VULKAN_CALL_RESULT(context, result,
        CreateShaderModule(context->device(), &moduleCreateInfo,
            /*const VkAllocationCallbacks*=*/nullptr, &shaderModule));
...
void DescriptorDataToVkDescSetLayout(const VulkanSharedContext* ctxt, const SkSpan<DescriptorData>& requestedDescriptors, VkDescriptorSetLayout* outLayout)
{
    ...
    for (size_t i = 0; i < requestedDescriptors.size(); i++) {
        if (requestedDescriptors[i].fCount != 0) {
            ...
            layoutBinding.stageFlags = PipelineStageFlagsToVkShaderStageFlags(currDescriptor.fPipelineStageFlags);
            layoutBinding.pImmutableSamplers
                = currDescriptor.fImmutableSampler ? (static_cast<const VulkanSampler*>(currDescriptor.fImmutableSampler))->constVkSamplerPtr() : nullptr;
        }
    }
    ...
    VkResult result;
    VULKAN_CALL_RESULT(ctxt, result, CreateDescriptorSetLayout(ctxt->device(), &layoutCreateInfo, nullptr, outLayout));
    if (result != VK_SUCCESS) {
...
uint32_t nonFormatInfoAsUInt32(const VulkanYcbcrConversionInfo& conversionInfo)
{
    ...
    return (((uint32_t)(usesExternalFormat) << kUsesExternalFormatShift) | ((uint32_t)(conversionInfo.fYcbcrModel) << kYcbcrModelShift)
        | ((uint32_t)(conversionInfo.fYcbcrRange) << kYcbcrRangeShift) | ((uint32_t)(conversionInfo.fXChromaOffset) << kXChromaOffsetShift)
        | ((uint32_t)(conversionInfo.fYChromaOffset) << kYChromaOffsetShift) | ((uint32_t)(conversionInfo.fChromaFilter) << kChromaFilterShift)
        | ((uint32_t)(conversionInfo.fForceExplicitReconstruction) << kForceExplicitReconShift) | ((uint32_t)(conversionInfo.fComponents.r) << kComponentRShift)
        | ((uint32_t)(conversionInfo.fComponents.g) << kComponentGShift) | ((uint32_t)(conversionInfo.fComponents.b) << kComponentBShift)
...
int numInt32sNeeded(const VulkanYcbcrConversionInfo& conversionInfo)
{
    ...
    return conversionInfo.fFormat == VK_FORMAT_UNDEFINED ? SamplerDesc::kInt32sNeededExternalFormat : SamplerDesc::kInt32sNeededKnownFormat;
}
} // namespace ycbcrPackaging
