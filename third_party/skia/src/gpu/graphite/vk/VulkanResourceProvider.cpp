...
VulkanResourceProvider::VulkanResourceProvider(
    SharedContext* sharedContext, SingleOwner* singleOwner, uint32_t recorderID, size_t resourceBudget, sk_sp<Buffer> intrinsicConstantUniformBuffer)
...
VulkanResourceProvider::~VulkanResourceProvider()
{
    if (fPipelineCache != VK_NULL_HANDLE) {
        VULKAN_CALL(this->vulkanSharedContext()->interface(), DestroyPipelineCache(this->vulkanSharedContext()->device(), fPipelineCache, nullptr));
    }
    if (fMSAALoadVertShaderModule != VK_NULL_HANDLE) {
        VULKAN_CALL(this->vulkanSharedContext()->interface(), DestroyShaderModule(this->vulkanSharedContext()->device(), fMSAALoadVertShaderModule, nullptr));
    }
    if (fMSAALoadFragShaderModule != VK_NULL_HANDLE) {
        VULKAN_CALL(this->vulkanSharedContext()->interface(), DestroyShaderModule(this->vulkanSharedContext()->device(), fMSAALoadFragShaderModule, nullptr));
    }
    if (fMSAALoadPipelineLayout != VK_NULL_HANDLE) {
        VULKAN_CALL(this->vulkanSharedContext()->interface(), DestroyPipelineLayout(this->vulkanSharedContext()->device(), fMSAALoadPipelineLayout, nullptr));
    }
}
...
sk_sp<Texture> VulkanResourceProvider::onCreateWrappedTexture(const BackendTexture& texture)
{
    ...
    if (TextureInfos::GetVulkanYcbcrConversionInfo(texture.info()).isValid()) {
        ycbcrConversion = this->findOrCreateCompatibleYcbcrConversion(TextureInfos::GetVulkanYcbcrConversionInfo(texture.info()));
        if (!ycbcrConversion) {
            return nullptr;
        }
    }

    return VulkanTexture::MakeWrapped(this->vulkanSharedContext(), texture.dimensions(), texture.info(), BackendTextures::GetMutableState(texture),
        BackendTextures::GetVkImage(texture),
        /*alloc=*/ {} /*Skia does not own wrapped texture memory*/, std::move(ycbcrConversion));
}
...
sk_sp<GraphicsPipeline> VulkanResourceProvider::createGraphicsPipeline(const RuntimeEffectDictionary* runtimeDict, const GraphicsPipelineDesc& pipelineDesc,
    const RenderPassDesc& renderPassDesc, SkEnumBitMask<PipelineCreationFlags> pipelineCreationFlags)
{
    return VulkanGraphicsPipeline::Make(this, runtimeDict, pipelineDesc, renderPassDesc, pipelineCreationFlags);
}
...
sk_sp<Texture> VulkanResourceProvider::createTexture(SkISize size, const TextureInfo& info, skgpu::Budgeted budgeted)
{
    sk_sp<VulkanYcbcrConversion> ycbcrConversion;
    if (TextureInfos::GetVulkanYcbcrConversionInfo(info).isValid()) {
        ycbcrConversion = this->findOrCreateCompatibleYcbcrConversion(TextureInfos::GetVulkanYcbcrConversionInfo(info));
        if (!ycbcrConversion) {
            return nullptr;
        }
    }

    return VulkanTexture::Make(this->vulkanSharedContext(), size, info, budgeted, std::move(ycbcrConversion));
}

sk_sp<Buffer> VulkanResourceProvider::createBuffer(size_t size, BufferType type, AccessPattern accessPattern)
{
...
sk_sp<Sampler> VulkanResourceProvider::createSampler(const SamplerDesc& samplerDesc)
{
    ...
    if (usesYcbcrConversion) {
        ...
        if (Resource* resource = fResourceCache->findAndRefResource(ycbcrKey, skgpu::Budgeted::kYes)) {
            ycbcrConversion = sk_sp<VulkanYcbcrConversion>(static_cast<VulkanYcbcrConversion*>(resource));
        } else {
            ycbcrConversion
                = VulkanYcbcrConversion::Make(this->vulkanSharedContext(), static_cast<uint32_t>(samplerDesc.desc() >> SamplerDesc::kImmutableSamplerInfoShift),
    ...
    return VulkanSampler::Make(this->vulkanSharedContext(), samplerDesc, std::move(ycbcrConversion));
}

BackendTexture VulkanResourceProvider::onCreateBackendTexture(SkISize dimensions, const TextureInfo& info)
{
    ...
    if (!VulkanTexture::MakeVkImage(this->vulkanSharedContext(), dimensions, info, &createdTextureInfo)) {
        return {};
    }
    return BackendTextures::MakeVulkan(dimensions, vkTexInfo, skgpu::MutableTextureStates::GetVkImageLayout(createdTextureInfo.fMutableState.get()),
        skgpu::MutableTextureStates::GetVkQueueFamilyIndex(createdTextureInfo.fMutableState.get()), createdTextureInfo.fImage, createdTextureInfo.fMemoryAlloc);
}
...
GraphiteResourceKey build_desc_set_key(const SkSpan<DescriptorData>& requestedDescriptors)
{
    ...
    for (const DescriptorData& desc : requestedDescriptors) {
        keyData.push_back(static_cast<uint8_t>(desc.fType) << 24 | desc.fBindingIndex << 16 | static_cast<uint16_t>(desc.fCount));
        if (desc.fImmutableSampler) {
            const VulkanSampler* sampler = static_cast<const VulkanSampler*>(desc.fImmutableSampler);
            SkASSERT(sampler);
            keyData.push_back_n(sampler->samplerDesc().asSpan().size(), sampler->samplerDesc().asSpan().data());
        }
    }
...
sk_sp<VulkanDescriptorSet> add_new_desc_set_to_cache(
    const VulkanSharedContext* context, const sk_sp<VulkanDescriptorPool>& pool, const GraphiteResourceKey& descSetKey, ResourceCache* resourceCache)
{
...
sk_sp<VulkanDescriptorSet> VulkanResourceProvider::findOrCreateDescriptorSet(SkSpan<DescriptorData> requestedDescriptors)
{
    ...
    if (!pool) {
        VULKAN_CALL(context->interface(), DestroyDescriptorSetLayout(context->device(), layout, nullptr));
        return nullptr;
    }
    ...
    auto firstDescSet = add_new_desc_set_to_cache(context, pool, key, fResourceCache.get());
    if (!firstDescSet) {
        return nullptr;
    }
    ...
    for (int i = 1; i < VulkanDescriptorPool::kMaxNumSets; i++) {
        auto descSet = add_new_desc_set_to_cache(context, pool, key, fResourceCache.get());
        if (!descSet) {
            SKGPU_LOG_W("Descriptor set allocation %d of %d was unsuccessful; no more sets will be"
                        "allocated from this pool.",
                i, VulkanDescriptorPool::kMaxNumSets);
            break;
        }
    }
...
VulkanResourceProvider::UniformBindGroupKey make_ubo_bind_group_key(SkSpan<DescriptorData> requestedDescriptors, SkSpan<BindBufferInfo> bindUniformBufferInfo)
{
    VulkanResourceProvider::UniformBindGroupKey uniqueKey;
    {
        ...
        for (uint32_t i = 0; i < requestedDescriptors.size(); ++i) {
            ...
            SkASSERT(SkTo<unsigned long>(descriptorBindingIndex) < VulkanGraphicsPipeline::kNumUniformBuffers);
...
void update_uniform_descriptor_set(SkSpan<DescriptorData> requestedDescriptors, SkSpan<BindBufferInfo> bindUniformBufferInfo, VkDescriptorSet descSet,
    const VulkanSharedContext* sharedContext)
{
    for (size_t i = 0; i < requestedDescriptors.size(); i++) {
        ...
        if (bindInfo.fBuffer) {
#if defined(SK_DEBUG)
            static uint64_t maxBufferRange = sharedContext->caps()->storageBufferSupport() ? sharedContext->vulkanCaps().maxStorageBufferRange()
...
sk_sp<VulkanDescriptorSet> VulkanResourceProvider::findOrCreateUniformBuffersDescriptorSet(
    SkSpan<DescriptorData> requestedDescriptors, SkSpan<BindBufferInfo> bindUniformBufferInfo)
{
    ...
    update_uniform_descriptor_set(requestedDescriptors, bindUniformBufferInfo, *newDS->descriptorSet(), this->vulkanSharedContext());
...
sk_sp<VulkanRenderPass> VulkanResourceProvider::findOrCreateRenderPassWithKnownKey(
    const RenderPassDesc& renderPassDesc, bool compatibleOnly, const GraphiteResourceKey& rpKey)
{
    if (Resource* resource = fResourceCache->findAndRefResource(rpKey, skgpu::Budgeted::kYes)) {
        return sk_sp<VulkanRenderPass>(static_cast<VulkanRenderPass*>(resource));
    }

    sk_sp<VulkanRenderPass> renderPass = VulkanRenderPass::MakeRenderPass(this->vulkanSharedContext(), renderPassDesc, compatibleOnly);
    if (!renderPass) {
        return nullptr;
    }
...
sk_sp<VulkanRenderPass> VulkanResourceProvider::findOrCreateRenderPass(const RenderPassDesc& renderPassDesc, bool compatibleOnly)
{
...
VkPipelineCache VulkanResourceProvider::pipelineCache()
{
    if (fPipelineCache == VK_NULL_HANDLE) {
        ...
        VkResult result;
        VULKAN_CALL_RESULT(
            this->vulkanSharedContext(), result, CreatePipelineCache(this->vulkanSharedContext()->device(), &createInfo, nullptr, &fPipelineCache));
...
sk_sp<VulkanFramebuffer> VulkanResourceProvider::createFramebuffer(const VulkanSharedContext* context, const skia_private::TArray<VkImageView>& attachmentViews,
    const VulkanRenderPass& renderPass, const int width, const int height)
{
...
void VulkanResourceProvider::onDeleteBackendTexture(const BackendTexture& texture)
{
    ...
    VULKAN_CALL(this->vulkanSharedContext()->interface(),
        DestroyImage(this->vulkanSharedContext()->device(), BackendTextures::GetVkImage(texture),
    ...
    if (alloc.fBackendMemory) {
        ...
        VULKAN_CALL(this->vulkanSharedContext()->interface(), FreeMemory(this->vulkanSharedContext()->device(), alloc.fMemory, nullptr));
    }
}

sk_sp<VulkanYcbcrConversion> VulkanResourceProvider::findOrCreateCompatibleYcbcrConversion(const VulkanYcbcrConversionInfo& ycbcrInfo) const
{
    if (!ycbcrInfo.isValid()) {
        return nullptr;
    }
    GraphiteResourceKey ycbcrConversionKey = VulkanYcbcrConversion::MakeYcbcrConversionKey(this->vulkanSharedContext(), ycbcrInfo);

    if (Resource* resource = fResourceCache->findAndRefResource(ycbcrConversionKey, skgpu::Budgeted::kYes)) {
...
sk_sp<VulkanGraphicsPipeline> VulkanResourceProvider::findOrCreateLoadMSAAPipeline(const RenderPassDesc& renderPassDesc)
{

    if (!renderPassDesc.fColorResolveAttachment.fTextureInfo.isValid() || !renderPassDesc.fColorAttachment.fTextureInfo.isValid()) {
    ...
    GraphiteResourceKey renderPassKey = VulkanRenderPass::MakeRenderPassKey(renderPassDesc, /*compatibleOnly=*/true);
    ...
    if (fMSAALoadVertShaderModule == VK_NULL_HANDLE) {
        SkASSERT(fMSAALoadFragShaderModule == VK_NULL_HANDLE && fMSAALoadPipelineLayout == VK_NULL_HANDLE);
        if (!VulkanGraphicsPipeline::InitializeMSAALoadPipelineStructs(
                this->vulkanSharedContext(), &fMSAALoadVertShaderModule, &fMSAALoadFragShaderModule, &fMSAALoadShaderStageInfo[0], &fMSAALoadPipelineLayout)) {
            SKGPU_LOG_E("Failed to initialize MSAA load pipeline creation structure(s)");
            return nullptr;
        }
    }

    sk_sp<VulkanRenderPass> compatibleRenderPass = this->findOrCreateRenderPassWithKnownKey(renderPassDesc,
        /*compatibleOnly=*/true, renderPassKey);
    ...
    sk_sp<VulkanGraphicsPipeline> pipeline
        = VulkanGraphicsPipeline::MakeLoadMSAAPipeline(this->vulkanSharedContext(), fMSAALoadVertShaderModule, fMSAALoadFragShaderModule,
            &fMSAALoadShaderStageInfo[0], fMSAALoadPipelineLayout, compatibleRenderPass, this->pipelineCache(), renderPassDesc.fColorAttachment.fTextureInfo);
...
BackendTexture VulkanResourceProvider::onCreateBackendTexture(
    AHardwareBuffer* hardwareBuffer, bool isRenderable, bool isProtectedContent, SkISize dimensions, bool fromAndroidWindow) const
{
    ...
    if (!skgpu::GetAHardwareBufferProperties(&hwbFormatProps, &hwbProps, vkContext->interface(), hardwareBuffer, device)) {
        return {};
    }
    ...
    VulkanTextureInfo vkTexInfo { VK_SAMPLE_COUNT_1_BIT, Mipmapped::kNo, imgCreateflags, hwbFormatProps.format, tiling, usageFlags, VK_SHARING_MODE_EXCLUSIVE,
        VK_IMAGE_ASPECT_COLOR_BIT, VulkanYcbcrConversionInfo() };
    ...
    if (!importAsExternalFormat && (!vkCaps.isTransferSrc(vkTexInfo) || !vkCaps.isTransferDst(vkTexInfo) || !vkCaps.isTexturable(vkTexInfo))) {
        if (isRenderable) {
    ...
    SkASSERT(!(vkTexInfo.fFlags & VK_IMAGE_CREATE_PROTECTED_BIT) || fSharedContext->isProtected() == Protected::kYes);
    ...
    VkResult result;
    VkImage image;
    result = VULKAN_CALL(vkContext->interface(), CreateImage(device, &imageCreateInfo, nullptr, &image));
    if (result != VK_SUCCESS) {
        return {};
    }

    const VkPhysicalDeviceMemoryProperties2& phyDevMemProps = vkContext->vulkanCaps().physicalDeviceMemoryProperties2();
    VulkanAlloc alloc;
    if (!AllocateAndBindImageMemory(&alloc, image, phyDevMemProps, hwbProps, hardwareBuffer, vkContext->interface(), device)) {
        VULKAN_CALL(vkContext->interface(), DestroyImage(device, image, nullptr));
        return {};
    }

    return BackendTextures::MakeVulkan(dimensions, vkTexInfo, VK_IMAGE_LAYOUT_UNDEFINED, VK_QUEUE_FAMILY_FOREIGN_EXT, image, alloc);
}
