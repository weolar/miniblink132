...
public:
    ...
    {
        this->resetWithNewLayout(layout);
    }
    ...
    void reset()
    {
        this->resetWithNewLayout(fLayout);
    }

    // scalars
    void write(float f)
    {
        this->write<SkSLType::kFloat>(&f);
    }
    void write(int32_t i)
    {
        this->write<SkSLType::kInt>(&i);
    }
    void writeHalf(float f)
    {
        this->write<SkSLType::kHalf>(&f);
    }

    // [i|h]vec4 and arrays thereof (just add overloads as needed)
    void write(const SkPMColor4f& c)
    {
        this->write<SkSLType::kFloat4>(c.vec());
    }
    void write(const SkRect& r)
    {
        this->write<SkSLType::kFloat4>(r.asScalars());
    }
    void write(const SkV4& v)
    {
        this->write<SkSLType::kFloat4>(v.ptr());
    }

    void write(const SkIRect& r)
    {
        this->write<SkSLType::kInt4>(&r);
    }

    void writeHalf(const SkPMColor4f& c)
    {
        this->write<SkSLType::kHalf4>(c.vec());
    }
    void writeHalf(const SkRect& r)
    {
        this->write<SkSLType::kHalf4>(r.asScalars());
    }
    void writeHalf(const SkV4& v)
    {
        this->write<SkSLType::kHalf4>(v.ptr());
    }
    ...
    {
        this->write<SkSLType::kFloat3>(v.ptr());
    }
    void write(const SkPoint3& p)
    {
        this->write<SkSLType::kFloat3>(&p);
    }

    void writeHalf(const SkV3& v)
    {
        this->write<SkSLType::kHalf3>(v.ptr());
    }
    void writeHalf(const SkPoint3& p)
    {
        this->write<SkSLType::kHalf3>(&p);
    }
    ...
    {
        this->write<SkSLType::kFloat2>(v.ptr());
    }
    void write(const SkSize& s)
    {
        this->write<SkSLType::kFloat2>(&s);
    }
    void write(const SkPoint& p)
    {
        this->write<SkSLType::kFloat2>(&p);
    }

    void write(const SkISize& s)
    {
        this->write<SkSLType::kInt2>(&s);
    }

    void writeHalf(const SkV2& v)
    {
        this->write<SkSLType::kHalf2>(v.ptr());
    }
    void writeHalf(const SkSize& s)
    {
        this->write<SkSLType::kHalf2>(&s);
    }
    void writeHalf(const SkPoint& p)
    {
        this->write<SkSLType::kHalf2>(&p);
    }
    ...
    void write(const SkMatrix& m)
    {
        ...
        float colMajor[9] = { m[0], m[3], m[6], m[1], m[4], m[7], m[2], m[5], m[8] };
...
template <int N, bool Half> struct LayoutTraits {
    ...
    static void Validate(const void* src, SkSLType type, Layout layout)
    {
        ...
        if constexpr (Half) {
            ...
            SkASSERT(SkSLTypeIsFullPrecisionNumericType(type) || LayoutRules::UseFullPrecision(layout));
        }
    }
#endif
};
...
template <int N, bool Half> void UniformManager::write(const void* src, SkSLType type)
{
    ...
    SkDEBUGCODE(L::Validate(src, type, fLayout);)
        ...
        char* dst
        = (N == 3 && LayoutRules::PadVec3Size(fLayout)) ? this->append(L::kAlign, L::kSize + L::kElemSize) : this->append(L::kAlign, L::kSize);
...
template <int N, bool Half> void UniformManager::writeArray(const void* src, int count, SkSLType type)
{
    using L = LayoutTraits<N, Half>;
    static constexpr int kSrcStride = N * 4; // Source data is always in multiples of 4 bytes.

    SkDEBUGCODE(L::Validate(src, type, fLayout);) SkASSERT(count > 0);
