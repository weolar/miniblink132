...
#define ASSERT_SINGLE_OWNER                                                                                                                                    \
    SkASSERT(fRecorder);                                                                                                                                       \
    SKGPU_ASSERT_SINGLE_OWNER(fRecorder->singleOwner())
...
bool paint_depends_on_dst(
    SkColor4f color, const SkShader* shader, const SkColorFilter* colorFilter, const SkBlender* finalBlender, const SkBlender* primitiveBlender)
{
    const bool srcIsTransparent = !color.isOpaque() || (shader && !shader->isOpaque()) || (colorFilter && !colorFilter->isAlphaUnchanged());
...
{
    return paint_depends_on_dst(
        paintParams.color(), paintParams.shader(), paintParams.colorFilter(), paintParams.finalBlender(), paintParams.primitiveBlender());
}
...
bool paint_depends_on_dst(const SkPaint& paint)
{
    ...
    return paint_depends_on_dst(paint.getColor4f(), paint.getShader(), paint.getColorFilter(), paint.getBlender(),
        /*primitiveBlender=*/nullptr);
}

/** If the paint can be reduced to a solid flood-fill, determine the correct color to fill with. */
std::optional<SkColor4f> extract_paint_color(const SkPaint& paint, const SkColorInfo& dstColorInfo)
{
...
Rect snap_rect_to_pixels(const Transform& localToDevice, const Rect& rect, float* strokeWidth = nullptr)
{
    ...
    if (!strokeWidth) {
        ...
        if (*strokeWidth == 0.f) {
            ...
            skvx::float2 devScales = max(abs(skvx::float2(localToDevice.matrix().rc(0, 0), localToDevice.matrix().rc(1, 0))),
                abs(skvx::float2(localToDevice.matrix().rc(0, 1), localToDevice.matrix().rc(1, 1))));
...
void snap_src_and_dst_rect_to_pixels(const Transform& localToDevice, SkRect* srcRect, SkRect* dstRect)
{
    if (localToDevice.type() > Transform::Type::kRectStaysRect) {
        return;
    }
...
bool is_simple_shape(const Shape& shape, SkStrokeRec::Style type)
{
    ...
    return (shape.isEmpty() && shape.inverted())
        || (!shape.inverted() && type != SkStrokeRec::kStrokeAndFill_Style
            && (shape.isRect() || (shape.isLine() && type != SkStrokeRec::kFill_Style)
                || (shape.isRRect() && (type != SkStrokeRec::kStroke_Style || SkRRectPriv::AllCornersCircular(shape.rrect())))));
}

bool use_compute_atlas_when_available(PathRendererStrategy strategy)
{
    return strategy == PathRendererStrategy::kComputeAnalyticAA || strategy == PathRendererStrategy::kComputeMSAA16
        || strategy == PathRendererStrategy::kComputeMSAA8 || strategy == PathRendererStrategy::kDefault;
}
...
private:
    struct Hash {
        size_t operator()(const CompressedPaintersOrder& o) const noexcept
        {
            return o.bits();
        }
    };
...
sk_sp<Device> Device::Make(Recorder* recorder, const SkImageInfo& ii, skgpu::Budgeted budgeted, Mipmapped mipmapped, SkBackingFit backingFit,
    const SkSurfaceProps& props, LoadOp initialLoadOp, std::string_view label, bool registerWithRecorder)
{
    ...
    SkISize backingDimensions = backingFit == SkBackingFit::kApprox ? GetApproxSize(ii.dimensions()) : ii.dimensions();
    auto textureInfo = caps->getDefaultSampledTextureInfo(ii.colorType(), mipmapped, recorder->priv().isProtected(), Renderable::kYes);

    return Make(recorder, TextureProxy::Make(caps, recorder->priv().resourceProvider(), backingDimensions, textureInfo, std::move(label), budgeted),
        ii.dimensions(), ii.colorInfo(), props, initialLoadOp, registerWithRecorder);
}

sk_sp<Device> Device::Make(Recorder* recorder, sk_sp<TextureProxy> target, SkISize deviceSize, const SkColorInfo& colorInfo, const SkSurfaceProps& props,
    LoadOp initialLoadOp, bool registerWithRecorder)
{
    if (!recorder) {
        return nullptr;
    }

    sk_sp<DrawContext> dc = DrawContext::Make(recorder->priv().caps(), std::move(target), deviceSize, colorInfo, props);
    if (!dc) {
        return nullptr;
...
Device::Device(Recorder* recorder, sk_sp<DrawContext> dc)
    ...
    , fDC(std::move(dc))
    , fClip(this)
    , fColorDepthBoundsManager(std::make_unique<HybridBoundsManager>(fDC->imageInfo().dimensions(), kGridCellSize, kMaxBruteForceN, kMaxGridSize))
    , fDisjointStencilSet(std::make_unique<IntersectionTreeSet>())
    , fCachedLocalToDevice(SkM44())
    , fCurrentDepth(DrawOrder::kClearDepth)
    , fSubRunControl(recorder->priv().caps()->getSubRunControl(fDC->surfaceProps().isUseDeviceIndependentFonts()))
{
    ...
    if (fRecorder->priv().caps()->defaultMSAASamplesCount() > 1) {
        if (fRecorder->priv().caps()->msaaRenderToSingleSampledSupport()) {
            ...
            TextureInfo msaaTexInfo = fRecorder->priv().caps()->getDefaultMSAATextureInfo(fDC->target()->textureInfo(), Discardable::kYes);
...
sk_sp<SkDevice> Device::createDevice(const CreateInfo& info, const SkPaint*)
{
    ...
    SkSurfaceProps props = this->surfaceProps().cloneWithPixelGeometry(info.fPixelGeometry);
    ...
    return Make(fRecorder, info.fInfo, skgpu::Budgeted::kYes, Mipmapped::kNo, SkBackingFit::kApprox, props, initialLoadOp, label);
}
...
sk_sp<Image> Device::makeImageCopy(const SkIRect& subset, Budgeted budgeted, Mipmapped mipmapped, SkBackingFit backingFit)
{
    ASSERT_SINGLE_OWNER
    this->flushPendingWorkToRecorder();
    ...
    if (!srcView) {
        ...
        Swizzle readSwizzle = fRecorder->priv().caps()->getReadSwizzle(colorInfo.colorType(), this->target()->textureInfo());
    ...
    return Image::Copy(fRecorder, srcView, colorInfo, subset, budgeted, mipmapped, backingFit, label);
}
...
bool Device::onWritePixels(const SkPixmap& src, int x, int y)
{
    ...
    if ((src.alphaType() == kUnknown_SkAlphaType) != (this->imageInfo().alphaType() == kUnknown_SkAlphaType)) {
        return false;
    }
    ...
    if (!fRecorder->priv().caps()->supportsWritePixels(target->textureInfo())) {
        ...
        this->drawImageRect(image.get(),
            /*src=*/nullptr, SkRect::MakeXYWH(x, y, src.width(), src.height()), SkFilterMode::kNearest, paint, SkCanvas::kFast_SrcRectConstraint);
        return true;
    }
    ...
    return fDC->recordUpload(fRecorder, fDC->refTarget(), src.info().colorInfo(), this->imageInfo().colorInfo(), levels, dstRect, nullptr);
}
...
void Device::clipRegion(const SkRegion& globalRgn, SkClipOp op)
{
    ...
    if (globalRgn.isEmpty()) {
        ...
        fClip.clipShape(globalToDevice, Shape { SkRect::Make(globalRgn.getBounds()) }, op, ClipStack::PixelSnapping::kYes);
    } else {
...
void Device::drawPaint(const SkPaint& paint)
{
    ...
    this->drawGeometry(this->localToDeviceTransform(), Geometry(inverseFill), paint, DefaultFillStyle(), DrawFlags::kIgnorePathEffect);
}
...
void Device::drawRect(const SkRect& r, const SkPaint& paint)
{
    ...
    if (!paint.isAntiAlias()) {
        ...
        if (style.isFillStyle()) {
            ...
            rectToDraw = snap_rect_to_pixels(this->localToDeviceTransform(), rectToDraw, &strokeWidth);
...
void Device::drawVertices(const SkVertices* vertices, sk_sp<SkBlender> blender, const SkPaint& paint, bool skipColorXform)
{
    // TODO - Add GPU handling of skipColorXform once Graphite has its color system more fleshed out.
    this->drawGeometry(this->localToDeviceTransform(), Geometry(sk_ref_sp(vertices)), paint, DefaultFillStyle(), DrawFlags::kIgnorePathEffect,
        std::move(blender), skipColorXform);
}

bool Device::drawAsTiledImageRect(SkCanvas* canvas, const SkImage* image, const SkRect* src, const SkRect& dst, const SkSamplingOptions& sampling,
    const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    SkRect finalDst = dst;
    if (!paint.isAntiAlias()) {
        snap_src_and_dst_rect_to_pixels(this->localToDeviceTransform(), &finalSrc, &finalDst);
    }

    [[maybe_unused]] auto [wasTiled, numTiles]
        = skgpu::TiledTextureUtils::DrawAsTiledImageRect(canvas, image, finalSrc, finalDst, SkCanvas::kAll_QuadAAFlags, sampling, &paint, constraint,
            /* sharpenMM= */ true, cacheSize, maxTextureSize);
#if defined(GPU_TEST_UTILS)
...
void Device::drawOval(const SkRect& oval, const SkPaint& paint)
{
    if (paint.getPathEffect()) {
        ...
        this->drawGeometry(this->localToDeviceTransform(), Geometry(Shape(SkPath::Oval(oval))), paint, SkStrokeRec(paint));
    } else {
...
void Device::drawArc(const SkArc& arc, const SkPaint& paint)
{
    ...
    if (!paint.getPathEffect() && SkScalarAbs(arc.sweepAngle()) >= 360.f
        ...
        this->drawGeometry(this->localToDeviceTransform(), Geometry(Shape(arc)), paint, SkStrokeRec(paint));
    }
}
...
void Device::drawRRect(const SkRRect& rr, const SkPaint& paint)
{
    ...
    if (paint.isAntiAlias()) {
        ...
        if (style.isFillStyle()) {
            ...
            const bool strokeAndFill = style.getStyle() == SkStrokeRec::kStrokeAndFill_Style;
            float strokeWidth = style.getWidth();
            snappedBounds = snap_rect_to_pixels(this->localToDeviceTransform(), rr.rect(), &strokeWidth);
            style.setStrokeStyle(strokeWidth, strokeAndFill);
        }
...
void Device::drawPath(const SkPath& path, const SkPaint& paint, bool pathIsMutable)
{
    ...
    if (!paint.getPathEffect() && !path.isInverseFillType()) {
        ...
        bool isClosed = false;
        if (SkRect rect; path.isRect(&rect, &isClosed) && (paint.getStyle() == SkPaint::kFill_Style || isClosed)) {
            this->drawRect(rect, paint);
            return;
        }
    }
    this->drawGeometry(this->localToDeviceTransform(), Geometry(Shape(path)), paint, SkStrokeRec(paint));
}

void Device::drawPoints(SkCanvas::PointMode mode, size_t count, const SkPoint* points, const SkPaint& paint)
{
    ...
    if (mode == SkCanvas::kPoints_PointMode) {
        ...
        if (paint.getStrokeCap() == SkPaint::kButt_Cap) {
            stroke.setStrokeParams(SkPaint::kSquare_Cap, paint.getStrokeJoin(), paint.getStrokeMiter());
        }
    } else {
        next = 1;
        count--;
    ...
    for (size_t i = 0; i < count; i += inc) {
        this->drawGeometry(this->localToDeviceTransform(), Geometry(Shape(points[i], points[i + next])), paint, stroke);
    }
}

void Device::drawEdgeAAQuad(const SkRect& rect, const SkPoint clip[4], SkCanvas::QuadAAFlags aaFlags, const SkColor4f& color, SkBlendMode mode)
{
    ...
    this->drawGeometry(this->localToDeviceTransform(), Geometry(quad), solidColorPaint, DefaultFillStyle(), DrawFlags::kIgnorePathEffect);
}

void Device::drawEdgeAAImageSet(const SkCanvas::ImageSetEntry set[], int count, const SkPoint dstClips[], const SkMatrix preViewMatrices[],
    const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    SkASSERT(count > 0);
    ...
    for (int i = 0; i < count; ++i) {
        ...
        auto [imageToDraw, newSampling] = skgpu::graphite::GetGraphiteBacked(this->recorder(), set[i].fImage.get(), sampling);
        if (!imageToDraw) {
        ...
        SkRect dst = SkModifyPaintAndDstForDrawImageRect(
            imageToDraw.get(), newSampling, set[i].fSrcRect, set[i].fDstRect, constraint == SkCanvas::kStrict_SrcRectConstraint, &paintWithShader);
        if (dst.isEmpty()) {
            return;
        }

        // NOTE: See drawEdgeAAQuad for details, we do not snap non-AA quads.
        auto flags = SkEnumBitMask<EdgeAAQuad::Flags>(static_cast<EdgeAAQuad::Flags>(set[i].fAAFlags));
        EdgeAAQuad quad = set[i].fHasClip ? EdgeAAQuad(dstClips + dstClipIndex, flags) : EdgeAAQuad(dst, flags);
        ...
        if (set[i].fMatrixIndex < 0) {
            this->drawGeometry(this->localToDeviceTransform(), Geometry(quad), paintWithShader, DefaultFillStyle(), DrawFlags::kIgnorePathEffect);
        } else {
            SkM44 xtraTransform(preViewMatrices[set[i].fMatrixIndex]);
            this->drawGeometry(
                this->localToDeviceTransform().concat(xtraTransform), Geometry(quad), paintWithShader, DefaultFillStyle(), DrawFlags::kIgnorePathEffect);
        }
...
void Device::drawImageRect(
    const SkImage* image, const SkRect* src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    SkCanvas::ImageSetEntry single { sk_ref_sp(image), src ? *src : SkRect::Make(image->bounds()), dst,
        /*alpha=*/1.f, SkCanvas::kAll_QuadAAFlags };
    ...
    if (!paint.isAntiAlias()) {
        snap_src_and_dst_rect_to_pixels(this->localToDeviceTransform(), &single.fSrcRect, &single.fDstRect);
    }
...
{
    return [&](const sktext::gpu::AtlasSubRun* subRun, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt> subRunStorage,
               sktext::gpu::RendererData rendererData) { this->drawAtlasSubRun(subRun, drawOrigin, paint, std::move(subRunStorage), rendererData); };
}

void Device::onDrawGlyphRunList(SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
    ASSERT_SINGLE_OWNER
    fRecorder->priv().textBlobCache()->drawGlyphRunList(canvas, this->localToDevice(), glyphRunList, paint, this->strikeDeviceInfo(), this->atlasDelegate());
}

void Device::drawAtlasSubRun(
    const sktext::gpu::AtlasSubRun* subRun, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt> subRunStorage, sktext::gpu::RendererData rendererData)
{
    ASSERT_SINGLE_OWNER

    const int subRunEnd = subRun->glyphCount();
    auto regenerateDelegate = [&](sktext::gpu::GlyphVector* glyphs, int begin, int end, skgpu::MaskFormat maskFormat, int padding) {
    ...
    for (int subRunCursor = 0; subRunCursor < subRunEnd;) {
        ...
        auto [ok, glyphsRegenerated] = subRun->regenerateAtlas(subRunCursor, subRunEnd, regenerateDelegate);
        // There was a problem allocating the glyph in the atlas. Bail.
        if (!ok) {
            return;
        }
        if (glyphsRegenerated) {
            auto [bounds, localToDevice] = subRun->vertexFiller().boundsAndDeviceMatrix(this->localToDeviceTransform(), drawOrigin);
            ...
            bool useGammaCorrectDistanceTable = this->imageInfo().colorSpace() && this->imageInfo().colorSpace()->gammaIsLinear();
            this->drawGeometry(localToDevice,
                Geometry(SubRunData(subRun, subRunStorage, bounds, this->localToDeviceTransform().inverse(), subRunCursor, glyphsRegenerated,
                    SkPaintPriv::ComputeLuminanceColor(subRunPaint), useGammaCorrectDistanceTable, this->surfaceProps().pixelGeometry(), fRecorder,
                    rendererData)),
                subRunPaint, DefaultFillStyle(), DrawFlags::kIgnorePathEffect, SkBlender::Mode(SkBlendMode::kDstIn));
        }
...
void Device::drawGeometry(const Transform& localToDevice, const Geometry& geometry, const SkPaint& paint, const SkStrokeRec& style,
    SkEnumBitMask<DrawFlags> flags, sk_sp<SkBlender> primitiveBlender, bool skipColorXform)
{
    ASSERT_SINGLE_OWNER
    ...
    if (!(flags & DrawFlags::kIgnorePathEffect) && paint.getPathEffect()) {
        ...
        SkPath dst;
        if (paint.getPathEffect()->filterPath(&dst, geometry.shape().asPath(), &newStyle, nullptr, localToDevice)) {
            dst.setIsVolatile(true);
            // Recurse using the path and new style, while disabling downstream path effect handling
            this->drawGeometry(
                localToDevice, Geometry(Shape(dst)), paint, newStyle, flags | DrawFlags::kIgnorePathEffect, std::move(primitiveBlender), skipColorXform);
            return;
        } else {
            SKGPU_LOG_W("Path effect failed to apply, drawing original path.");
            this->drawGeometry(localToDevice, geometry, paint, style, flags | DrawFlags::kIgnorePathEffect, std::move(primitiveBlender), skipColorXform);
            return;
        }
    }
    ...
    // issues).
    if (geometry.isShape() && localToDevice.type() == Transform::Type::kPerspective && !is_simple_shape(geometry.shape(), style.getStyle())) {
        SkPath devicePath = geometry.shape().asPath();
        devicePath.transform(localToDevice.matrix().asM33());
        devicePath.setIsVolatile(true);
        this->drawGeometry(Transform::Identity(), Geometry(Shape(devicePath)), paint, style, flags, std::move(primitiveBlender), skipColorXform);
        return;
    }
    ...
    auto [renderer, pathAtlas] = this->chooseRenderer(localToDevice, geometry, style, /*requireMSAA=*/false);
    ...
    const Clip clip = fClip.visitClipStackForDraw(localToDevice, geometry, style, outsetBoundsForAA, &clipElements);
    ...
    const std::optional<SkBlendMode> blendMode = blender ? blender->asBlendMode() : SkBlendMode::kSrcOver;
    Coverage rendererCoverage = renderer ? renderer->coverage() : Coverage::kSingleChannel;
    ...
    PaintParams shading { paint, std::move(primitiveBlender), clip.analyticClip(), sk_ref_sp(clip.shader()), dstReadReq, skipColorXform };
    const bool dependsOnDst = paint_depends_on_dst(shading) || clip.shader() || !clip.analyticClip().isEmpty();
    ...
    const int numNewRenderSteps = (renderer ? renderer->numRenderSteps() : 1) + (secondaryRenderer ? secondaryRenderer->numRenderSteps() : 0);
    ...
    if (pathAtlas != nullptr) {
        std::tie(renderer, atlasMask) = pathAtlas->addShape(clip.transformedShapeBounds(), geometry.shape(), localToDevice, style);
    ...
    for (const RenderStep* step : renderer->steps()) {
        ...
        SkASSERT((!step->performsShading() || dss.fDepthTestEnabled)
            && (!dss.fDepthTestEnabled || dss.fDepthCompareOp == CompareOp::kGreater || dss.fDepthCompareOp == CompareOp::kGEqual));
    }
#endif
    ...
    CompressedPaintersOrder clipOrder = fClip.updateClipStateForDraw(clip, clipElements, fColorDepthBoundsManager.get(), order.depth());
    ...
    if (renderer->depthStencilFlags() & DepthStencilFlags::kStencil) {
        DisjointStencilIndex setIndex = fDisjointStencilSet->add(order.paintOrder(), clip.drawBounds());
    ...
    if (pathAtlas != nullptr) {
        ...
        fDC->recordDraw(renderer, Transform::Translate(origin.fX, origin.fY), Geometry(mask), clip, order, &shading, nullptr);
    } else {
        if (styleType == SkStrokeRec::kStroke_Style || styleType == SkStrokeRec::kHairline_Style || styleType == SkStrokeRec::kStrokeAndFill_Style) {
            ...
            fDC->recordDraw(styleType == SkStrokeRec::kStrokeAndFill_Style ? fRecorder->priv().rendererProvider()->tessellatedStrokes() : renderer,
        ...
        if (styleType == SkStrokeRec::kFill_Style || styleType == SkStrokeRec::kStrokeAndFill_Style) {
            ...
            if (!innerFillBounds.isEmptyNegativeOrNaN()) {
                ...
                fDC->recordDraw(fRecorder->priv().rendererProvider()->nonAABounds(), localToDevice, Geometry(Shape(innerFillBounds)), clip,
                    orderWithoutCoverage, &shading, nullptr);
...
void Device::drawClipShape(const Transform& localToDevice, const Shape& shape, const Clip& clip, DrawOrder order)
{
    ...
    auto [renderer, pathAtlas] = this->chooseRenderer(localToDevice, geometry, DefaultFillStyle(),
        /*requireMSAA=*/true);
    if (!renderer) {
    ...
    if (localToDevice.type() == Transform::Type::kPerspective) {
        SkPath devicePath = geometry.shape().asPath();
        devicePath.transform(localToDevice.matrix().asM33());
        fDC->recordDraw(renderer, Transform::Identity(), Geometry(Shape(devicePath)), clip, order, nullptr, nullptr);
    } else {
...
std::pair<const Renderer*, PathAtlas*> Device::chooseRenderer(
    const Transform& localToDevice, const Geometry& geometry, const SkStrokeRec& style, bool requireMSAA) const
{
    ...
    if (geometry.isSubRun()) {
        ...
        bool useLCD = rendererData.isLCD && geometry.subRunData().pixelGeometry() != kUnknown_SkPixelGeometry;
    ...
    if (!requireMSAA && is_simple_shape(shape, type)) {
        ...
        if (shape.isRect() && style.isFillStyle() && localToDevice.type() <= Transform::Type::kRectStaysRect) {
    ...
    if (!requireMSAA && shape.isArc() && SkScalarNearlyEqual(shape.arc().oval().width(), shape.arc().oval().height())
        && SkScalarAbs(shape.arc().sweepAngle()) < 360.f && localToDevice.type() <= Transform::Type::kAffine) {
    ...
    if (atlasProvider->isAvailable(AtlasProvider::PathAtlasFlags::kCompute) && use_compute_atlas_when_available(strategy)) {
    ...
    if (!pathAtlas && atlasProvider->isAvailable(AtlasProvider::PathAtlasFlags::kRaster)
        && (strategy == PathRendererStrategy::kRasterAA || (strategy == PathRendererStrategy::kDefault && !fMSAASupported))) {
    ...
    if (type == SkStrokeRec::kStroke_Style || type == SkStrokeRec::kHairline_Style) {
    ...
    if (shape.convex() && !shape.inverted()) {
        ...
        const bool preferWedges =
            ...
            (shape.isPath() && shape.path().countVerbs() < 50) || drawBounds->area() <= (256 * 256);
...
void Device::drawSpecial(
    SkSpecialImage* special, const SkMatrix& localToDevice, const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    SkPaint paintWithShader(paint);
    SkRect dst = SkModifyPaintAndDstForDrawImageRect(img.get(), sampling,
        /*src=*/SkRect::Make(special->subset()),
        /*dst=*/SkRect::MakeIWH(special->width(), special->height()),
        /*strictSrcSubset=*/constraint == SkCanvas::kStrict_SrcRectConstraint, &paintWithShader);
    if (dst.isEmpty()) {
        return;
    }
    ...
    EdgeAAQuad::Flags aaFlags = paint.isAntiAlias() ? EdgeAAQuad::Flags::kAll : EdgeAAQuad::Flags::kNone;
    this->drawGeometry(Transform(SkM44(localToDevice)), Geometry(EdgeAAQuad(dst, aaFlags)), paintWithShader, DefaultFillStyle(), DrawFlags::kIgnorePathEffect);
}

void Device::drawCoverageMask(const SkSpecialImage* mask, const SkMatrix& localToDevice, const SkSamplingOptions& sampling, const SkPaint& paint)
{
    CoverageMaskShape::MaskInfo maskInfo { /*fTextureOrigin=*/ { SkTo<uint16_t>(mask->subset().fLeft), SkTo<uint16_t>(mask->subset().fTop) },
        /*fMaskSize=*/ { SkTo<uint16_t>(mask->width()), SkTo<uint16_t>(mask->height()) } };
    ...
    TextureDataBlock::SampledTexture sampledMask { maskProxyView.refProxy(), { SkFilterMode::kLinear, SkTileMode::kClamp } };
    ...
    CoverageMaskShape maskShape { Shape { Rect::WH((float)mask->width(), (float)mask->height()) }, maskProxyView.proxy(),
        ...
        this->localToDeviceTransform().inverse(), maskInfo };

    this->drawGeometry(Transform(SkM44(localToDevice)), Geometry(maskShape), paint, DefaultFillStyle(), DrawFlags::kIgnorePathEffect);
}
...
sk_sp<SkSpecialImage> Device::snapSpecial(const SkIRect& subset, bool forceCopy)
{
    ...
    if (forceCopy || !this->readSurfaceView() || this->readSurfaceView().proxy()->isFullyLazy()) {
        deviceImage = this->makeImageCopy(subset, Budgeted::kYes, Mipmapped::kNo, SkBackingFit::kApprox);
    ...
    return SkSpecialImages::MakeGraphite(fRecorder, finalSubset, std::move(deviceImage), this->surfaceProps());
}

sk_sp<skif::Backend> Device::createImageFilteringBackend(const SkSurfaceProps& surfaceProps, SkColorType colorType) const
{
...
sk_sp<sktext::gpu::Slug> Device::convertGlyphRunListToSlug(const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
    return sktext::gpu::SlugImpl::Make(this->localToDevice(), glyphRunList, paint, this->strikeDeviceInfo(), SkStrikeCache::GlobalStrikeCache());
}
...
bool Device::drawBlurredRRect(const SkRRect& rrect, const SkPaint& paint, float deviceSigma)
{
    ...
    if (skgpu::BlurIsEffectivelyIdentity(deviceSigma)) {
        this->drawGeometry(this->localToDeviceTransform(), Geometry(rrect.isRect() ? Shape(rrect.rect()) : Shape(rrect)), paint, style);
        return true;
    }

    std::optional<AnalyticBlurMask> analyticBlur = AnalyticBlurMask::Make(this->recorder(), this->localToDeviceTransform(), deviceSigma, rrect);
    if (!analyticBlur) {
        return false;
    }
