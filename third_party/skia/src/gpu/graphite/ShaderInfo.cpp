...
std::string get_uniforms(UniformOffsetCalculator* offsetter, SkSpan<const Uniform> uniforms, int manglingSuffix, bool* wrotePaintColor)
{
    std::string result;
    ...
    for (const Uniform& u : uniforms) {
        ...
        SkSL::String::appendf(
            &result, "    layout(offset=%d) %s %s", offsetter->advanceOffset(u.type(), u.count()), SkSLTypeString(u.type()), uniformName.c_str());
        if (u.count()) {
            result.append("[");
...
std::string get_node_uniforms(UniformOffsetCalculator* offsetter, const ShaderNode* node, bool* wrotePaintColor)
{
    std::string result;
    ...
    if (!uniforms.empty()) {
        if (node->entry()->fUniformStructName) {
            ...
            SkSL::String::appendf(&result, "layout(offset=%d) %s node_%d;", structOffset, node->entry()->fUniformStructName, node->keyIndex());
        } else {
...
std::string get_ssbo_fields(SkSpan<const Uniform> uniforms, int manglingSuffix, bool* wrotePaintColor)
{
    std::string result;
...
std::string get_node_ssbo_fields(const ShaderNode* node, bool* wrotePaintColor)
{
    ...
    if (!uniforms.empty()) {
        if (node->entry()->fUniformStructName) {
            SkSL::String::appendf(&result, "%s node_%d;", node->entry()->fUniformStructName, node->keyIndex());
        } else {
            SkSL::String::appendf(&result, "// %d - %s uniforms\n", node->keyIndex(), node->entry()->fName);
...
std::string emit_intrinsic_uniforms(int bufferID, Layout layout)
{
    ...
    SkASSERTF(result.find('[') == std::string::npos, "Arrays are not supported in intrinsic uniforms");

    return result;
}

std::string emit_paint_params_uniforms(int bufferID, const Layout layout, SkSpan<const ShaderNode*> nodes, bool* hasUniforms, bool* wrotePaintColor)
{
    auto offsetter = UniformOffsetCalculator::ForTopLevel(layout);
...
std::string emit_render_step_uniforms(int bufferID, const Layout layout, SkSpan<const Uniform> uniforms)
{
    auto offsetter = UniformOffsetCalculator::ForTopLevel(layout);
...
std::string emit_paint_params_storage_buffer(int bufferID, SkSpan<const ShaderNode*> nodes, bool* hasUniforms, bool* wrotePaintColor)
{
    *hasUniforms = false;
...
std::string emit_render_step_storage_buffer(int bufferID, SkSpan<const Uniform> uniforms)
{
    ...
    return SkSL::String::printf("struct StepUniformData {\n"
                                "%s\n"
                                "};\n\n"
...
std::string emit_uniforms_from_storage_buffer(const char* bufferNamePrefix, const char* ssboIndex, SkSpan<const Uniform> uniforms)
{
    std::string result;
    ...
    for (const Uniform& u : uniforms) {
        ...
        SkSL::String::appendf(&result, " = %sUniformData[%s].%s;\n", bufferNamePrefix, ssboIndex, u.name());
    }

    return result;
}

void append_sampler_descs(const SkSpan<const uint32_t> samplerData, skia_private::TArray<SamplerDesc>& outDescs)
{
    ...
    for (size_t i = 0; i < samplerData.size();) {
        ...
        if (immutableSamplerInfo != 0) {
            ...
            samplerDescLength = usesExternalFormat ? SamplerDesc::kInt32sNeededExternalFormat : SamplerDesc::kInt32sNeededKnownFormat;
...
std::string get_node_texture_samplers(
    const ResourceBindingRequirements& bindingReqs, const ShaderNode* node, int* binding, skia_private::TArray<SamplerDesc>* outDescs)
{
    std::string result;
...
std::string emit_textures_and_samplers(
    const ResourceBindingRequirements& bindingReqs, SkSpan<const ShaderNode*> nodes, int* binding, skia_private::TArray<SamplerDesc>* outDescs)
{
    std::string result;
    for (const ShaderNode* n : nodes) {
...
std::string emit_varyings(const RenderStep* step, const char* direction, bool emitSsboIndicesVarying, bool emitLocalCoordsVarying)
{
    std::string result;
    int location = 0;
    ...
    auto appendVarying = [&](const Varying& v) {
        ...
        case Interpolation::kPerspective:
            interpolation = "";
            break;
        ...
        SkSL::String::appendf(&result, "layout(location=%d) %s %s%s %s;\n", location++, direction, interpolation, SkSLTypeString(v.gpuType()), v.name());
    };
...
void emit_preambles(const ShaderInfo& shaderInfo, SkSpan<const ShaderNode*> nodes, std::string treeLabel, std::string* preamble)
{
    for (int i = 0; i < SkTo<int>(nodes.size()); ++i) {
        ...
        std::string nodePreamble
            = node->entry()->fPreambleGenerator ? node->entry()->fPreambleGenerator(shaderInfo, node) : node->generateDefaultPreamble(shaderInfo);
        if (!nodePreamble.empty()) {
            SkSL::String::appendf(preamble,
                "// [%d]   %s: %s\n"
                "%s\n",
                node->keyIndex(), nextLabel.c_str(), node->entry()->fName, nodePreamble.c_str());
        }
    }
}

std::string emit_color_output(BlendFormula::OutputType outputType, const char* outColor, const char* inColor)
{
    switch (outputType) {
    ...
    case BlendFormula::kISCModulate_OutputType:
        return SkSL::String::printf("%s = (half4(1.0) - %s) * outputCoverage;", outColor, inColor);
...
constexpr skgpu::BlendInfo make_simple_blendInfo(skgpu::BlendCoeff srcCoeff, skgpu::BlendCoeff dstCoeff)
{
    return { skgpu::BlendEquation::kAdd, srcCoeff, dstCoeff, SK_PMColor4fTRANSPARENT, skgpu::BlendModifiesDst(skgpu::BlendEquation::kAdd, srcCoeff, dstCoeff) };
}
...
std::unique_ptr<ShaderInfo> ShaderInfo::Make(const Caps* caps, const ShaderCodeDictionary* dict, const RuntimeEffectDictionary* rteDict, const RenderStep* step,
    UniquePaintParamsID paintID, bool useStorageBuffers, skgpu::Swizzle writeSwizzle, skia_private::TArray<SamplerDesc>* outDescs)
{
    const char* shadingSsboIndex = useStorageBuffers && step->performsShading() ? "shadingSsboIndex" : nullptr;
    std::unique_ptr<ShaderInfo> result = std::unique_ptr<ShaderInfo>(new ShaderInfo(rteDict, shadingSsboIndex));
    ...
    if (paintID.isValid()) {
        result->generateFragmentSkSL(caps, dict, step, paintID, useStorageBuffers, writeSwizzle, outDescs);
    }

    result->generateVertexSkSL(caps, step, useStorageBuffers);
...
void ShaderInfo::generateFragmentSkSL(const Caps* caps, const ShaderCodeDictionary* dict, const RenderStep* step, UniquePaintParamsID paintID,
    bool useStorageBuffers, Swizzle writeSwizzle, skia_private::TArray<SamplerDesc>* outDescs)
{
    ...
    const ShaderSnippet* clipSnippet = fRootNodes.size() > 2 ? dict->getEntry(fRootNodes[2]->codeSnippetId()) : nullptr;
    SkASSERT(!clipSnippet || (!clipSnippet->needsPriorStageOutput() && !clipSnippet->needsBlenderDstColor()));
#endif
    ...
    SkASSERTF_RELEASE(step->performsShading(), "render step: %s, label: %s", step->name(), label.c_str());

    // Extract the root nodes for clarity
    // TODO(b/372912880): Release assert debugging for illegal instruction occurring in the wild.
    SkASSERTF_RELEASE(fRootNodes.size() == 2 || fRootNodes.size() == 3, "root node size = %zu, label = %s", fRootNodes.size(), label.c_str());
    ...
    if (finalBlendRoot->codeSnippetId() < kBuiltInCodeSnippetIDCount && finalBlendRoot->codeSnippetId() >= kFixedBlendIDOffset) {
        finalBlendMode = static_cast<SkBlendMode>(finalBlendRoot->codeSnippetId() - kFixedBlendIDOffset);
    ...
    // TODO(b/372912880): Release assert debugging for illegal instruction occurring in the wild.
    SkASSERTF_RELEASE(finalBlendMode.has_value() || fDstReadRequirement != DstReadRequirement::kNone, "blend mode: %d, dst read: %d, coverage: %d, label = %s",
        finalBlendMode.has_value() ? (int)*finalBlendMode : -1, (int)fDstReadRequirement, (int)finalCoverage, label.c_str());
    ...
    const bool useGradientStorageBuffer = caps->gradientBufferSupport() && (allReqFlags & SnippetRequirementFlags::kGradientBuffer);
    const bool useDstSampler = fDstReadRequirement == DstReadRequirement::kTextureCopy || fDstReadRequirement == DstReadRequirement::kTextureSample;

    const bool defineLocalCoordsVarying = this->needsLocalCoords();
    std::string preamble = emit_varyings(step,
        /*direction=*/"in",
        /*emitSsboIndicesVarying=*/useShadingStorageBuffer, defineLocalCoordsVarying);

    // The uniforms are mangled by having their index in 'fEntries' as a suffix (i.e., "_%d")
    const ResourceBindingRequirements& bindingReqs = caps->resourceBindingRequirements();
    preamble += emit_intrinsic_uniforms(bindingReqs.fIntrinsicBufferBinding, bindingReqs.fUniformBufferLayout);
    if (hasStepUniforms) {
        if (useStepStorageBuffer) {
            preamble += emit_render_step_storage_buffer(bindingReqs.fRenderStepBufferBinding, step->uniforms());
        } else {
            preamble += emit_render_step_uniforms(bindingReqs.fRenderStepBufferBinding, bindingReqs.fUniformBufferLayout, step->uniforms());
        }
    }

    bool wrotePaintColor = false;
    if (useShadingStorageBuffer) {
        preamble += emit_paint_params_storage_buffer(bindingReqs.fPaintParamsBufferBinding, fRootNodes, &fHasPaintUniforms, &wrotePaintColor);
        SkSL::String::appendf(&preamble, "uint %s;\n", this->ssboIndex());
    } else {
        preamble += emit_paint_params_uniforms(
            bindingReqs.fPaintParamsBufferBinding, bindingReqs.fUniformBufferLayout, fRootNodes, &fHasPaintUniforms, &wrotePaintColor);
    }
    ...
    if (useGradientStorageBuffer) {
        SkSL::String::appendf(&preamble,
            ...
            "    float %s[];\n"
            "};\n",
            bindingReqs.fGradientBufferBinding, ShaderInfo::kGradientBufferName);
...

    {
        ...
        if (step->hasTextures()) {
            ...
            if (outDescs) {
                ...
                int renderStepSamplerCount = bindingReqs.fSeparateTextureAndSamplerBinding ? (binding - paintTextureCount) / 2 : binding - paintTextureCount;
    ...
    if (useShadingStorageBuffer) {
        SkSL::String::appendf(&mainBody, "%s = %s.y;\n", this->ssboIndex(), RenderStep::ssboIndicesVarying());
    }
    ...
    if (writeSwizzle != Swizzle::RGBA()) {
        SkSL::String::appendf(&mainBody, "%s = %s.%s;", args.fPriorStageOutput.c_str(), args.fPriorStageOutput.c_str(), writeSwizzle.asString().c_str());
    }
    ...
    if (finalCoverage == Coverage::kNone) {
        ...
        // TODO(b/372912880): Release assert debugging for illegal instruction occurring in the wild
        SkASSERTF_RELEASE(finalBlendMode.has_value(), "blend mode: %d, dst read: %d, label = %s", finalBlendMode.has_value() ? (int)*finalBlendMode : -1,
            (int)fDstReadRequirement, label.c_str());
        ...
        if (useStepStorageBuffer) {
            SkSL::String::appendf(&mainBody, "uint stepSsboIndex = %s.x;\n", RenderStep::ssboIndicesVarying());
            mainBody += emit_uniforms_from_storage_buffer("step", "stepSsboIndex", step->uniforms());
        }
        ...
        if (fDstReadRequirement != DstReadRequirement::kNone) {
            ...
            if (useDstSampler) {
                ...
                // RGB anyway.
                mainBody += "if (all(lessThanEqual(outputCoverage.rgb, half3(0)))) {"
                            "discard;"
                            "}";
            }
            ...
            SkSL::String::appendf(&mainBody, "sk_FragColor = %s * outputCoverage + dstColor * (1.0 - outputCoverage);", outColor);
            ...
            SkASSERTF_RELEASE(finalBlendMode.has_value(), "blend mode: %d, dst read: %d, coverage: %d, label = %s",
                finalBlendMode.has_value() ? (int)*finalBlendMode : -1, (int)fDstReadRequirement, (int)finalCoverage, label.c_str());
            BlendFormula coverageBlendFormula = finalCoverage == Coverage::kLCD ? skgpu::GetLCDBlendFormula(*finalBlendMode)
                                                                                : skgpu::GetBlendFormula(
                                                                                    /*isOpaque=*/false, /*hasCoverage=*/true, *finalBlendMode);
            fBlendInfo = { coverageBlendFormula.equation(), coverageBlendFormula.srcCoeff(), coverageBlendFormula.dstCoeff(), SK_PMColor4fTRANSPARENT,
            ...
            mainBody += emit_color_output(coverageBlendFormula.primaryOutput(), "sk_FragColor", outColor);
            if (coverageBlendFormula.hasSecondaryOutput()) {
                SkASSERT(caps->shaderCaps()->fDualSourceBlendingSupport);
                mainBody += emit_color_output(coverageBlendFormula.secondaryOutput(), "sk_SecondaryFragColor", outColor);
            }
        }
    }
    mainBody += "}\n";
...
void ShaderInfo::generateVertexSkSL(const Caps* caps, const RenderStep* step, bool useStorageBuffers)
{
    ...
    const ResourceBindingRequirements& bindingReqs = caps->resourceBindingRequirements();
    std::string sksl = emit_intrinsic_uniforms(bindingReqs.fIntrinsicBufferBinding, bindingReqs.fUniformBufferLayout);
    ...
    if (hasStepUniforms) {
        if (useStepStorageBuffer) {
            sksl += emit_render_step_storage_buffer(bindingReqs.fRenderStepBufferBinding, step->uniforms());
        } else {
            sksl += emit_render_step_uniforms(bindingReqs.fRenderStepBufferBinding, bindingReqs.fUniformBufferLayout, step->uniforms());
        }
    }
    ...
    if (useStepStorageBuffer) {
        ...
        SkSL::String::appendf(&sksl, "uint stepSsboIndex = %s.x;\n", RenderStep::ssboIndicesAttribute());
    ...
    if (useShadingStorageBuffer) {
        ...
        SkSL::String::appendf(&sksl, "%s = %s;", RenderStep::ssboIndicesVarying(), RenderStep::ssboIndicesAttribute());
    }
...
{
    return !fRootNodes.empty() && SkToBool(fRootNodes[0]->requiredFlags() & SnippetRequirementFlags::kLocalCoords);
}
