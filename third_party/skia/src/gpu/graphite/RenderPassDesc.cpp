...
RenderPassDesc RenderPassDesc::Make(const Caps* caps, const TextureInfo& targetInfo, LoadOp loadOp, StoreOp storeOp,
    SkEnumBitMask<DepthStencilFlags> depthStencilFlags, const std::array<float, 4>& clearColor, bool requiresMSAA, Swizzle writeSwizzle)
{
    RenderPassDesc desc;
    ...
    if (depthStencilFlags != DepthStencilFlags::kNone) {
        desc.fDepthStencilAttachment.fTextureInfo = caps->getDefaultDepthStencilTextureInfo(depthStencilFlags, desc.fSampleCount, targetInfo.isProtected());
...
SkString RenderPassDesc::toString() const
{
    return SkStringPrintf("RP(color: %s, resolve: %s, ds: %s, samples: %u, swizzle: %s, "
        ...
        fColorAttachment.toString().c_str(), fColorResolveAttachment.toString().c_str(), fDepthStencilAttachment.toString().c_str(), fSampleCount,
        fWriteSwizzle.asString().c_str(), fClearColor[0], fClearColor[1], fClearColor[2], fClearColor[3], fClearDepth, fClearStencil);
}
...
SkString RenderPassDesc::toPipelineLabel() const
{
    ...
    if (fColorAttachment.fLoadOp == LoadOp::kLoad && (fColorResolveAttachment.fTextureInfo.isValid() || fSampleCount > 1)) {
    ...
    return SkStringPrintf("RP(color: %s%s, resolve: %s, ds: %s, samples: %u, swizzle: %s)", fColorAttachment.fTextureInfo.toRPAttachmentString().c_str(),
        colorLoadStr, fColorResolveAttachment.fTextureInfo.toRPAttachmentString().c_str(), fDepthStencilAttachment.fTextureInfo.toRPAttachmentString().c_str(),
        fSampleCount, fWriteSwizzle.asString().c_str());
}

SkString AttachmentDesc::toString() const
{
    if (fTextureInfo.isValid()) {
        return SkStringPrintf("info: %s loadOp: %s storeOp: %s", fTextureInfo.toString().c_str(), to_str(fLoadOp), to_str(fStoreOp));
    } else {
