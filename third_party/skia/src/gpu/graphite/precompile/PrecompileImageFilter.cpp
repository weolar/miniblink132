...
void PrecompileImageFilter::createPipelines(const KeyContext& keyContext, PipelineDataGatherer* gatherer, const RenderPassDesc& renderPassDesc,
...
void CreateBlurImageFilterPipelines(const KeyContext& keyContext, PipelineDataGatherer* gatherer, const RenderPassDesc& renderPassDesc,
    const PaintOptionsPriv::ProcessCombination& processCombination)
{
    ...
    // filtering.
    sk_sp<PrecompileShader> imageShader = PrecompileShadersPriv::Image(PrecompileImageShaderFlags::kExcludeAlpha | PrecompileImageShaderFlags::kExcludeCubic);
    ...
    blurPaintOptions.priv().buildCombinations(keyContext, gatherer, DrawTypeFlags::kSimpleShape,
        /* withPrimitiveBlender= */ false, Coverage::kSingleChannel, renderPassDesc, processCombination);
}
...
private:
    void onCreatePipelines(const KeyContext& keyContext, PipelineDataGatherer* gatherer, const RenderPassDesc& renderPassDesc,
        const PaintOptionsPriv::ProcessCombination& processCombination) const override
    {

        PaintOptions paintOptions;

        sk_sp<PrecompileShader> imageShader
            = PrecompileShadersPriv::Image(PrecompileImageShaderFlags::kExcludeAlpha | PrecompileImageShaderFlags::kExcludeCubic);

        sk_sp<PrecompileShader> blendShader = PrecompileShaders::Blend(SkSpan<const sk_sp<PrecompileBlender>>(&fBlender, 1), { imageShader }, { imageShader });

        paintOptions.setShaders({ std::move(blendShader) });

        paintOptions.priv().buildCombinations(keyContext, gatherer, DrawTypeFlags::kSimpleShape,
            /* withPrimitiveBlender= */ false, Coverage::kSingleChannel, renderPassDesc, processCombination);
    }
...
sk_sp<PrecompileImageFilter> PrecompileImageFilters::Arithmetic(sk_sp<PrecompileImageFilter> background, sk_sp<PrecompileImageFilter> foreground)
{
    return Blend(PrecompileBlenders::Arithmetic(), std::move(background), std::move(foreground));
}

sk_sp<PrecompileImageFilter> PrecompileImageFilters::Blend(SkBlendMode bm, sk_sp<PrecompileImageFilter> background, sk_sp<PrecompileImageFilter> foreground)
{
...
sk_sp<PrecompileImageFilter> PrecompileImageFilters::Blend(
    sk_sp<PrecompileBlender> blender, sk_sp<PrecompileImageFilter> background, sk_sp<PrecompileImageFilter> foreground)
{
...
private:
        ...
        const PaintOptionsPriv::ProcessCombination& processCombination) const override
    {
        ...
        PrecompileImageFiltersPriv::CreateBlurImageFilterPipelines(keyContext, gatherer, renderPassDesc, processCombination);
    }
};

sk_sp<PrecompileImageFilter> PrecompileImageFilters::Blur(sk_sp<PrecompileImageFilter> input)
{
...
sk_sp<PrecompileImageFilter> PrecompileImageFilters::ColorFilter(sk_sp<PrecompileColorFilter> colorFilter, sk_sp<PrecompileImageFilter> input)
{
    ...
    if (colorFilter) {
        filter = sk_make_sp<PrecompileColorFilterImageFilter>(std::move(colorFilter), std::move(filter));
    }
    return filter;
}
...
private:
        ...
        const PaintOptionsPriv::ProcessCombination& processCombination) const override
    {
        ...
        displacement.priv().buildCombinations(keyContext, gatherer, DrawTypeFlags::kSimpleShape,
            /* withPrimitiveBlender= */ false, Coverage::kSingleChannel, renderPassDesc, processCombination);
    }
};

sk_sp<PrecompileImageFilter> PrecompileImageFilters::DisplacementMap(sk_sp<PrecompileImageFilter> input)
{
...
private:
        ...
        const PaintOptionsPriv::ProcessCombination& processCombination) const override
    {
        ...
        lighting.priv().buildCombinations(keyContext, gatherer, DrawTypeFlags::kSimpleShape,
            /* withPrimitiveBlender= */ false, Coverage::kSingleChannel, renderPassDesc, processCombination);
    }
};

sk_sp<PrecompileImageFilter> PrecompileImageFilters::Lighting(sk_sp<PrecompileImageFilter> input)
{
...
private:
        ...
        const PaintOptionsPriv::ProcessCombination& processCombination) const override
    {
        ...
        matrixConv.priv().buildCombinations(keyContext, gatherer, DrawTypeFlags::kSimpleShape,
            /* withPrimitiveBlender= */ false, Coverage::kSingleChannel, renderPassDesc, processCombination);
    }
};

sk_sp<PrecompileImageFilter> PrecompileImageFilters::MatrixConvolution(sk_sp<PrecompileImageFilter> input)
{
...
private:
        ...
        const PaintOptionsPriv::ProcessCombination& processCombination) const override
    {
...

        {
            ...
            sparse.priv().buildCombinations(keyContext, gatherer, DrawTypeFlags::kSimpleShape,
                /* withPrimitiveBlender= */ false, Coverage::kSingleChannel, renderPassDesc, processCombination);
        }

        {
            ...
            linear.priv().buildCombinations(keyContext, gatherer, DrawTypeFlags::kSimpleShape,
                /* withPrimitiveBlender= */ false, Coverage::kSingleChannel, renderPassDesc, processCombination);
        }
    }
};

sk_sp<PrecompileImageFilter> PrecompileImageFilters::Morphology(sk_sp<PrecompileImageFilter> input)
{
