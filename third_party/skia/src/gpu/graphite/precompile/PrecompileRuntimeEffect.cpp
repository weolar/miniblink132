...
PrecompileBase::Type to_precompile_type(SkRuntimeEffect::ChildType childType)
{
    ...
    case SkRuntimeEffect::ChildType::kShader:
        return PrecompileBase::Type::kShader;
    case SkRuntimeEffect::ChildType::kColorFilter:
        return PrecompileBase::Type::kColorFilter;
    case SkRuntimeEffect::ChildType::kBlender:
        return PrecompileBase::Type::kBlender;
    }
    SkUNREACHABLE;
}

bool children_are_valid(SkRuntimeEffect* effect, SkSpan<const PrecompileChildOptions> childOptions)
{
...
template <typename T> class PrecompileRTEffect : public T {
public:
...
private:
    ...
    void addToKey(const KeyContext& keyContext, PaintParamsKeyBuilder* builder, PipelineDataGatherer* gatherer, int desiredCombination) const override
    {
        ...
        for (size_t rowIndex = 0; rowIndex < fChildOptions.size(); ++rowIndex) {
            ...
            auto [option, childOptions] = PrecompileBase::SelectOption(SkSpan<const sk_sp<PrecompileBase>>(slotOptions), slotOption);
...
sk_sp<PrecompileShader> PrecompileRuntimeEffects::MakePrecompileShader(sk_sp<SkRuntimeEffect> effect, SkSpan<const PrecompileChildOptions> childOptions)
{
...
sk_sp<PrecompileColorFilter> PrecompileRuntimeEffects::MakePrecompileColorFilter(
    sk_sp<SkRuntimeEffect> effect, SkSpan<const PrecompileChildOptions> childOptions)
{
...
sk_sp<PrecompileBlender> PrecompileRuntimeEffects::MakePrecompileBlender(sk_sp<SkRuntimeEffect> effect, SkSpan<const PrecompileChildOptions> childOptions)
{
