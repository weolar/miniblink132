...
void PaintOptions::setClipShaders(SkSpan<const sk_sp<PrecompileShader>> clipShaders)
{
    ...
    for (const sk_sp<PrecompileShader>& cs : clipShaders) {
        ...
        sk_sp<PrecompileShader> inverted = withCTM ? withCTM->makeWithColorFilter(PrecompileColorFilters::Blend()) : nullptr;
...
{
    return this->numShaderCombinations() * this->numColorFilterCombinations() * this->numBlendCombinations() * this->numClipShaderCombinations();
}
...
void PaintOptions::createKey(const KeyContext& keyContext, PaintParamsKeyBuilder* keyBuilder, PipelineDataGatherer* gatherer, int desiredCombination,
    bool addPrimitiveBlender, Coverage coverage) const
{
    SkDEBUGCODE(keyBuilder->checkReset();) SkASSERT(desiredCombination < this->numCombinations());
    ...
    auto clipShader = PrecompileBase::SelectOption(SkSpan(fClipShaderOptions), desiredClipShaderCombination);
    ...
    if (desiredBlendCombination < fBlendModeOptions.size()) {
        ...
        finalBlender = PrecompileBase::SelectOption(SkSpan(fBlenderOptions), desiredBlendCombination - fBlendModeOptions.size());
    }
    if (!finalBlender.first) {
        finalBlender = { PrecompileBlenders::Mode(SkBlendMode::kSrcOver), 0 };
    }
    DstReadRequirement dstReadReq = get_dst_read_req(keyContext.caps(), coverage, finalBlender.first.get());

    PaintOption option(kOpaquePaintColor, finalBlender, PrecompileBase::SelectOption(SkSpan(fShaderOptions), desiredShaderCombination),
        PrecompileBase::SelectOption(SkSpan(fColorFilterOptions), desiredColorFilterCombination), addPrimitiveBlender, clipShader, dstReadReq, fDither);
...
void create_image_drawing_pipelines(const KeyContext& keyContext, PipelineDataGatherer* gatherer, const PaintOptions& orig,
    const RenderPassDesc& renderPassDesc, const PaintOptionsPriv::ProcessCombination& processCombination)
{
    ...
    imagePaintOptions.priv().buildCombinations(keyContext, gatherer, DrawTypeFlags::kSimpleShape,
        /* withPrimitiveBlender= */ false, Coverage::kSingleChannel, renderPassDesc, processCombination);
}

} // anonymous namespace

void PaintOptions::buildCombinations(const KeyContext& keyContext, PipelineDataGatherer* gatherer, DrawTypeFlags drawTypes, bool withPrimitiveBlender,
    Coverage coverage, const RenderPassDesc& renderPassDesc, const ProcessCombination& processCombination) const
{
    ...
    if (!fImageFilterOptions.empty() || !fMaskFilterOptions.empty()) {
        ...
        if (!fImageFilterOptions.empty()) {
            std::vector<sk_sp<PrecompileColorFilter>> newCFs(tmp.fColorFilterOptions.begin(), tmp.fColorFilterOptions.end());
            if (newCFs.empty()) {
        ...
        tmp.buildCombinations(keyContext, gatherer, drawTypes, withPrimitiveBlender, coverage, renderPassDesc, processCombination);

        create_image_drawing_pipelines(keyContext, gatherer, *this, renderPassDesc, processCombination);
