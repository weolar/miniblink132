...
{
    const SkRuntimeEffect* arithmeticEffect = GetKnownRuntimeEffect(SkKnownRuntimeEffects::StableKey::kArithmetic);
...
std::pair<sk_sp<PrecompileBlender>, int> PrecompileBlenderList::selectOption(int desiredCombination) const
{
    ...
    if (!fFixedBlenderEffects.empty()) {
        auto [option, childCombination] = PrecompileBase::SelectOption<PrecompileBlender>(fFixedBlenderEffects, desiredCombination);

        // Double-check that we aren't returning a blend mode that should have been consolidated.
        SkDEBUGCODE(auto bm = option->priv().asBlendMode();) SkASSERT(!bm || (*bm > SkBlendMode::kXor && *bm < SkBlendMode::kHue));
