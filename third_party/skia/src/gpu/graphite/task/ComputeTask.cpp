...
Task::Status ComputeTask::prepareResources(ResourceProvider* provider, ScratchResourceManager* scratchManager, const RuntimeEffectDictionary* rtd)
{
...
Task::Status ComputeTask::addCommands(Context* ctx, CommandBuffer* commandBuffer, ReplayTargetData rtd)
{
    ...
    return (currentSpanSize == 0u || commandBuffer->addComputePass({ currentSpanPtr, currentSpanSize })) ? Status::kSuccess : Status::kFail;
}
