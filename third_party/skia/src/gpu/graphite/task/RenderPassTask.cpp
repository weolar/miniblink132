...
sk_sp<RenderPassTask> RenderPassTask::Make(
    DrawPassList passes, const RenderPassDesc& desc, sk_sp<TextureProxy> target, sk_sp<TextureProxy> dstCopy, SkIRect dstCopyBounds)
{
    ...
    SkASSERT(!dstCopy || (dstCopy->dimensions().width() >= dstCopyBounds.width() && dstCopy->dimensions().height() >= dstCopyBounds.height()));
    if (!target) {
        return nullptr;
    }
    ...
    if (desc.fColorAttachment.fTextureInfo.isValid()) {
        ...
        SkASSERT(desc.fSampleCount == desc.fColorAttachment.fTextureInfo.numSamples() || 1 == desc.fColorAttachment.fTextureInfo.numSamples());
    }
    ...
    return sk_sp<RenderPassTask>(new RenderPassTask(std::move(passes), desc, std::move(target), std::move(dstCopy), dstCopyBounds));
}

RenderPassTask::RenderPassTask(DrawPassList passes, const RenderPassDesc& desc, sk_sp<TextureProxy> target, sk_sp<TextureProxy> dstCopy, SkIRect dstCopyBounds)
...
Task::Status RenderPassTask::prepareResources(
    ResourceProvider* resourceProvider, ScratchResourceManager* scratchManager, const RuntimeEffectDictionary* runtimeDict)
{
    SkASSERT(fTarget);
    if (!TextureProxy::InstantiateIfNotLazy(scratchManager, fTarget.get())) {
        ...
        SKGPU_LOG_W("Dimensions are (%d, %d).", fTarget->dimensions().width(), fTarget->dimensions().height());
        return Status::kFail;
    }
...
Task::Status RenderPassTask::addCommands(Context* context, CommandBuffer* commandBuffer, ReplayTargetData replayData)
{
    ...
    if (fRenderPassDesc.fColorResolveAttachment.fTextureInfo.isValid()) {
        SkASSERT(fTarget->numSamples() == 1 && fRenderPassDesc.fColorAttachment.fTextureInfo.numSamples() > 1);
        colorAttachment = resourceProvider->findOrCreateDiscardableMSAAAttachment(fTarget->dimensions(), fRenderPassDesc.fColorAttachment.fTextureInfo);
        if (!colorAttachment) {
    ...
    if (fRenderPassDesc.fDepthStencilAttachment.fTextureInfo.isValid()) {
        ...
        SkISize dimensions = context->priv().caps()->getDepthAttachmentDimensions(fTarget->texture()->textureInfo(), fTarget->dimensions());
        depthStencilAttachment = resourceProvider->findOrCreateDepthStencilAttachment(dimensions, fRenderPassDesc.fDepthStencilAttachment.fTextureInfo);
    ...
    // rendering.
    if (commandBuffer->addRenderPass(fRenderPassDesc, std::move(colorAttachment), std::move(resolveAttachment), std::move(depthStencilAttachment),
            fDstCopy ? fDstCopy->texture() : nullptr, fDstCopyBounds, fTarget->dimensions(), fDrawPasses)) {
        return Status::kSuccess;
    } else {
        return Status::kFail;
    }
