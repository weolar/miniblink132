...
Image_YUVA::Image_YUVA(const YUVAProxies& proxies, const SkYUVAInfo& yuvaInfo, sk_sp<SkColorSpace> imageColorSpace)
    : Image_Base(SkImageInfo::Make(yuvaInfo.dimensions(), kAssumedColorType, yuva_alpha_type(yuvaInfo), std::move(imageColorSpace)), kNeedNewImageUniqueID)
...
sk_sp<Image_YUVA> Image_YUVA::Make(const Caps* caps, const SkYUVAInfo& yuvaInfo, SkSpan<TextureProxyView> planes, sk_sp<SkColorSpace> imageColorSpace)
{
    if (!yuvaInfo.isValid()) {
        return nullptr;
    }
    SkImageInfo info = SkImageInfo::Make(yuvaInfo.dimensions(), kAssumedColorType, yuva_alpha_type(yuvaInfo), imageColorSpace);
    ...
    int expectedPlanes;
    if (!SkYUVAInfo::YUVALocation::AreValidLocations(locations, &expectedPlanes) || expectedPlanes != numPlanes) {
        return nullptr;
    }
    ...
    if (locations[kA].fPlane >= 0 && planes[locations[kA].fPlane].dimensions() != yuvaInfo.dimensions()) {
        return nullptr;
    }

    if (yuvaInfo.planeSubsamplingFactors(locations[kU].fPlane) != yuvaInfo.planeSubsamplingFactors(locations[kV].fPlane)) {
        return nullptr;
    }
    ...
    return sk_sp<Image_YUVA>(new Image_YUVA(std::move(channelProxies), yuvaInfo, std::move(imageColorSpace)));
}

sk_sp<Image_YUVA> Image_YUVA::WrapImages(const Caps* caps, const SkYUVAInfo& yuvaInfo, SkSpan<const sk_sp<SkImage>> images, sk_sp<SkColorSpace> imageColorSpace)
{
...
{
    sk_sp<Image_YUVA> view { new Image_YUVA(fProxies, fYUVAInfo, std::move(newCS)) };
