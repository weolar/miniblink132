...
public:
    ...
    {
        return IToC(this->channelIndex(i));
    }
    ...
    template <SkAlphaType AlphaType> constexpr SkRGBA4f<AlphaType> applyTo(SkRGBA4f<AlphaType> color) const
    {
...
private:
    ...
    explicit constexpr Swizzle(uint16_t key)
        : fKey(key)
    {
    }
...
constexpr Swizzle::Swizzle(const char c[4])
    : fKey(static_cast<uint16_t>((CToI(c[0]) << 0) | (CToI(c[1]) << 4) | (CToI(c[2]) << 8) | (CToI(c[3]) << 12)))
{
}

constexpr Swizzle Swizzle::selectChannelInR(int i) const
{
    return Swizzle(static_cast<uint16_t>((this->channelIndex(i) << 0) | (CToI('0') << 4) | (CToI('0') << 8) | (CToI('0') << 12)));
}
