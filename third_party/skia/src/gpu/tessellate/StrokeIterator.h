...
public:
    ...
    constexpr static bool IsVerbGeometric(Verb verb)
    {
...
private:
    ...
    bool finishOpenContour()
    {
        if (fQueueCount) {
            SkASSERT(
                this->backVerb() == Verb::kLine || this->backVerb() == Verb::kQuad || this->backVerb() == Verb::kConic || this->backVerb() == Verb::kCubic);
            ...
            case SkPaint::kSquare_Cap: {
                ...
                if (!fStroke->isHairlineStyle()) {
                    ...
                    float a = fViewMatrix->getScaleX(), b = fViewMatrix->getSkewX(), c = fViewMatrix->getSkewY(), d = fViewMatrix->getScaleY();
                    float det = a * d - b * c;
                    if (det > 0) {
    ...
    void fillSquareCapPoints()
    {
        ...
        if (firstTangent.isZero()) {
            SkASSERT(fFirstVerbInContour == Verb::kQuad || fFirstVerbInContour == Verb::kConic || fFirstVerbInContour == Verb::kCubic);
