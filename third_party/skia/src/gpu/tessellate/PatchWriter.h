...
template <PatchAttribs A, typename T, bool Required, bool Optional> struct AttribValue {
    ...
    explicit AttribValue(PatchAttribs attribs)
        : AttribValue(attribs, {})
    {
    }
...
template <PatchAttribs A, typename T, bool Required, bool Optional> VertexWriter& operator<<(VertexWriter& w, const AttribValue<A, T, Required, Optional>& v)
{
    if constexpr (Required) {
        w << v.fV; // always write
    } else if constexpr (Optional) {
...
template <size_t Stride> struct PatchStorage {
...
// *** PatchWriter ***
template <typename PatchAllocator, typename... Traits> class PatchWriter {
    ...
    template <PatchAttribs A, typename T, bool Req /*=req_attrib<A>*/, bool Opt /*=opt_attrib<A>*/> using attrib_t = AttribValue<A, T, Req, Opt>;
...
#undef DEF_ATTRIB_TYPE
    ...
    static constexpr size_t kMaxStride = 4 * sizeof(SkPoint) + // control points
        (JoinAttrib::kEnabled ? sizeof(SkPoint) : 0) + (FanPointAttrib::kEnabled ? sizeof(SkPoint) : 0) + (StrokeAttrib::kEnabled ? sizeof(StrokeParams) : 0)
        + (ColorAttrib::kEnabled ? std::min(sizeof(Color), sizeof(SkPMColor4f)) : 0) + (DepthAttrib::kEnabled ? sizeof(float) : 0)
        + (CurveTypeAttrib::kEnabled ? sizeof(float) : 0) + (SsboIndexAttrib::kEnabled ? 2 * sizeof(uint32_t) : 0);

    // Types that vary depending on the activated features, but do not define the patch data.
    using DeferredPatch = std::conditional_t<kTrackJoinControlPoints, PatchStorage<kMaxStride>, std::monostate>;
    using InnerTriangulator = std::conditional_t<kAddTrianglesWhenChopping, MiddleOutPolygonTriangulator, NullTriangulator>;
...
public:
    ...
    ENABLE_IF(kTrackJoinControlPoints) writeDeferredStrokePatch()
    {
        if (fDeferredPatch.hasPending()) {
            ...
            memcpy(SkTAddOffset<void>(fDeferredPatch.fData, 4 * sizeof(SkPoint)), &fJoin, sizeof(SkPoint));
    ...
    {
        this->writeConic(sk_bit_cast<float2>(pts[0]), sk_bit_cast<float2>(pts[1]), sk_bit_cast<float2>(pts[2]), w);
    }
    ...
    {
        this->writeQuadratic(sk_bit_cast<float2>(pts[0]), sk_bit_cast<float2>(pts[1]), sk_bit_cast<float2>(pts[2]));
    }
    ...
    {
        this->writeTriangle(sk_bit_cast<float2>(p0), sk_bit_cast<float2>(p1), sk_bit_cast<float2>(p2));
    }
...
private:
    AI void emitPatchAttribs(VertexWriter vertexWriter, const JoinAttrib& join, float explicitCurveType)
    {
        // NOTE: operator<< overrides automatically handle optional and disabled attribs.
        vertexWriter << join << fFanPoint << fStrokeParams << fColor << fDepth << CurveTypeAttrib { fAttribs, explicitCurveType } << fSsboIndex;
    }
    ...
    int accountForCurve(float n4)
    {
        if (n4 <= kMaxParametricSegments_p4) {
            ...
            return SkScalarCeilToInt(wangs_formula::root4(std::min(n4, kMaxSegmentsPerCurve_p4) / kMaxParametricSegments_p4));
        }
    }
    ...
    void chopAndWriteConics(float2 p0, float2 p1, float2 p2, float w, int numPatches)
    {
        ...
        for (; numPatches >= 2; --numPatches) {
            ...
            this->writeConicPatch(h0.xy() / h0.w(), ab.xy() / ab.w(), midpoint, ab.w() / sqrtf(h0.w() * abc.w()));
            if constexpr (kAddTrianglesWhenChopping) {
        ...
        SkASSERT(numPatches == 1);
        this->writeConicPatch(h0.xy() / h0.w(), h1.xy() / h1.w(),
            h2.xy(), // h2.w == 1
            h1.w() / sqrtf(h0.w()));
        if constexpr (kAddTrianglesWhenChopping) {
