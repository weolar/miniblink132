...
public:
    ...
    AI explicit VectorXform(const SkMatrix& m)
    {
        *this = m;
    }
    AI explicit VectorXform(const SkM44& m)
    {
        *this = m;
    }
    ...
    {
        return join(fC0 * vectors.x() + fC1 * vectors.y(), fC0 * vectors.z() + fC1 * vectors.w());
    }
...
{
    return quadratic_p4(precision, sk_bit_cast<skvx::float2>(pts[0]), sk_bit_cast<skvx::float2>(pts[1]), sk_bit_cast<skvx::float2>(pts[2]), vectorXform);
}
...
{
    return cubic_p4(precision, sk_bit_cast<skvx::float2>(pts[0]), sk_bit_cast<skvx::float2>(pts[1]), sk_bit_cast<skvx::float2>(pts[2]),
        sk_bit_cast<skvx::float2>(pts[3]), vectorXform);
}
...
{
    return conic_p2(precision, sk_bit_cast<skvx::float2>(pts[0]), sk_bit_cast<skvx::float2>(pts[1]), sk_bit_cast<skvx::float2>(pts[2]), w, vectorXform);
}
