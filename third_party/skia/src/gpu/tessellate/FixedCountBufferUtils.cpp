...
void write_curve_index_buffer_base_index(VertexWriter vertexWriter, size_t bufferSize, uint16_t baseIndex)
{
    ...
    const auto* neighborInLastResolveLevel = &indexData.push_back({ baseIndex, (uint16_t)(baseIndex + 2), (uint16_t)(baseIndex + 1) });
    ...
    for (int resolveLevel = 2; resolveLevel <= maxResolveLevel; ++resolveLevel) {
        ...
        for (int i = 0; i < numTrianglePairsInResolveLevel; ++i) {
            ...
            indexData.push_back({ (*neighborInLastResolveLevel)[0], nextIndex++, (*neighborInLastResolveLevel)[1] });
            // Second triangle shares the right edge of "neighborInLastResolveLevel".
            indexData.push_back({ (*neighborInLastResolveLevel)[1], nextIndex++, (*neighborInLastResolveLevel)[2] });
...
void FixedCountCurves::WriteVertexBuffer(VertexWriter vertexWriter, size_t bufferSize)
{
    ...
    SkDEBUGCODE(auto end = vertexWriter.mark(vertexCount * sizeof(SkPoint));)
        ...
        vertexWriter
        << (float)0 /*resolveLevel*/ << (float)0 /*idx*/;
...
void FixedCountWedges::WriteIndexBuffer(VertexWriter vertexWriter, size_t bufferSize)
{
    ...
    write_curve_index_buffer_base_index(std::move(vertexWriter), bufferSize - sizeof(uint16_t) * 3,
        /*baseIndex=*/1);
}
