...
public:
    ...
    public:
        ...
        {
            SkASSERT(fKey);
            SkDEBUGCODE(size_t dataCount = fKey->internalSize() / sizeof(uint32_t) - kMetaDataCnt;) SkASSERT(SkToU32(dataIdx) < dataCount);
...
private:
    ...
    {
        SkASSERT(this->isValid());
        SkASSERT(fKey[kHash_MetaDataIdx] == ResourceKeyHash(&fKey[kHash_MetaDataIdx] + 1, this->internalSize() - sizeof(uint32_t)));
...
public:
    ...
    uint32_t domain() const
    {
        return this->ResourceKey::domain();
    }
    size_t dataSize() const
    {
        return this->ResourceKey::dataSize();
    }
...
static inline bool SkShouldPostMessageToBus(const UniqueKeyInvalidatedMessage& msg, uint32_t msgBusUniqueID)
{
...
static inline bool SkShouldPostMessageToBus(const UniqueKeyInvalidatedMsg_Graphite& msg, uint32_t msgBusUniqueID)
{
 ...
 */
template <size_t SizeInUInt32> class FixedSizeKey {
public:
    uint32_t hash() const
    {
        return fHash;
    ...
    {
        return fHash == that.fHash && 0 == memcmp(fPackedData, that.fPackedData, sizeof(fPackedData));
    }

    class Builder {
    public:
        Builder(FixedSizeKey* key)
            : fKey(key)
        {
        }
    ...
    struct Hash {
        uint32_t operator()(const FixedSizeKey& key) const
        {
            return key.hash();
        }
    };
