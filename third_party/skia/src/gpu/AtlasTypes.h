...
public:
    ...
    {
        return AtlasToken(fSequenceNumber + 1);
    }
...
private:
    AtlasToken() = delete;
    explicit AtlasToken(uint64_t sequenceNumber)
...
public:
    ...
    {
        return fCurrentFlushToken.next();
    }
...
public:
    ...
    bool isValid() const
    {
        return fGenID != AtlasGenerationCounter::kInvalidGeneration || fPlotIndex != 0 || fPageIndex != 0;
    }
    ...
    {
        return fGenID == other.fGenID && fPlotIndex == other.fPlotIndex && fPageIndex == other.fPageIndex;
    }
...
public:
    ...
    uint32_t pageIndex() const
    {
        return fPlotLocator.pageIndex();
    }

    uint32_t plotIndex() const
    {
        return fPlotLocator.plotIndex();
    }

    uint64_t genID() const
    {
        return fPlotLocator.genID();
    }
    ...
    {
        auto width = fUVs[2] - fUVs[0], height = fUVs[3] - fUVs[1];
...
public:
    ...
    void markFullIfUsed()
    {
        fIsFull = !fDirtyRect.isEmpty();
    }
    bool isEmpty() const
    {
        return fRectanizer.percentFull() == 0;
    }
    ...
    {
        return sk_sp<Plot>(new Plot(fPageIndex, fPlotIndex, fGenerationCounter, fX, fY, fWidth, fHeight, fColorType, fBytesPerPixel));
    }
