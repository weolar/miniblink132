...
public:
    ...
    constexpr BlendFormula(
        OutputType primaryOut, OutputType secondaryOut, skgpu::BlendEquation equation, skgpu::BlendCoeff srcCoeff, skgpu::BlendCoeff dstCoeff)
    ...
    {
        return fPrimaryOutputType == that.fPrimaryOutputType && fSecondaryOutputType == that.fSecondaryOutputType && fBlendEquation == that.fBlendEquation
            && fSrcCoeff == that.fSrcCoeff && fDstCoeff == that.fDstCoeff && fProps == that.fProps;
    }
...
private:
     ...
     */
    constexpr BlendFormula::Properties GetProperties(
        OutputType PrimaryOut, OutputType SecondaryOut, skgpu::BlendEquation BlendEquation, skgpu::BlendCoeff SrcCoeff, skgpu::BlendCoeff DstCoeff)
    {
        return
            ...
            SkASSERT((kNone_OutputType == PrimaryOut) == !skgpu::BlendCoeffsUseSrcColor(SrcCoeff, DstCoeff)), SkASSERT(!skgpu::BlendCoeffRefsSrc2(SrcCoeff)),
            ...
            static_cast<Properties>((skgpu::BlendModifiesDst(BlendEquation, SrcCoeff, DstCoeff) ? kModifiesDst_Property : 0)
                | (!skgpu::BlendCoeffsUseDstColor(SrcCoeff, DstCoeff, false /*srcColorIsOpaque*/) ? kUnaffectedByDst_Property : 0)
                | (!skgpu::BlendCoeffsUseDstColor(SrcCoeff, DstCoeff, true /*srcColorIsOpaque*/) ? kUnaffectedByDstIfOpaque_Property : 0)
                ...
                ((kModulate_OutputType == PrimaryOut || kNone_OutputType == PrimaryOut) && kNone_OutputType == SecondaryOut
