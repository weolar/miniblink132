...
GrRecordingContext::ProgramData::ProgramData(std::unique_ptr<const GrProgramDesc> desc, const GrProgramInfo* info)
    : fDesc(std::move(desc))
    , fInfo(info)
{
}
...
GrRecordingContext::Arenas::Arenas(SkArenaAlloc* recordTimeAllocator, sktext::gpu::SubRunAllocator* subRunAllocator)
...
{
    GrBackendFormat format = this->caps()->getDefaultBackendFormat(SkColorTypeToGrColorType(colorType), GrRenderable::kNo);
...
void GrRecordingContext::Stats::dumpKeyValuePairs(TArray<SkString>* keys, TArray<double>* values) const
{
...
void GrRecordingContext::DMSAAStats::dumpKeyValuePairs(TArray<SkString>* keys, TArray<double>* values) const
{
