...
public:
    ...
    {
        return SkSpan(fDependencies);
    }
    SkSpan<GrRenderTask*> dependents()
    {
        return SkSpan(fDependents);
    }
...
#if defined(GPU_TEST_UTILS)
    const GrTextureResolveRenderTask* resolveTask() const
    {
        return fTextureResolveTask;
    }
#endif
protected:
...
private:
    ...
    SkDEBUGCODE(bool isDependent(const GrRenderTask* dependent) const;) SkDEBUGCODE(void validate() const;)
    ...
    {
    } // Only OpsTask currently overrides this
    virtual void onPrepare(GrOpFlushState*)
    {
    } // OpsTask and GrDDLTask override this
