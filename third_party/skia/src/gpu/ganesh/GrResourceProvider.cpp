...
sk_sp<GrTexture> GrResourceProvider::createTexture(SkISize dimensions, const GrBackendFormat& format, GrTextureType textureType, GrColorType colorType,
    GrRenderable renderable, int renderTargetSampleCnt, skgpu::Budgeted budgeted, skgpu::Mipmapped mipmapped, GrProtected isProtected,
    const GrMipLevel texels[], std::string_view label)
{
    ...
    auto scratch = this->getExactScratch(dimensions, format, textureType, renderable, renderTargetSampleCnt, budgeted, mipmapped, isProtected, label);
    if (scratch) {
        if (!hasPixels) {
            return scratch;
        }
    ...
    return fGpu->createTexture(dimensions, format, textureType, renderable, renderTargetSampleCnt, budgeted, isProtected, colorType, tempColorType,
        tmpTexels.get(), numMipLevels, label);
}

sk_sp<GrTexture> GrResourceProvider::getExactScratch(SkISize dimensions, const GrBackendFormat& format, GrTextureType textureType, GrRenderable renderable,
    int renderTargetSampleCnt, skgpu::Budgeted budgeted, skgpu::Mipmapped mipmapped, GrProtected isProtected, std::string_view label)
{
    sk_sp<GrTexture> tex(this->findAndRefScratchTexture(dimensions, format, textureType, renderable, renderTargetSampleCnt, mipmapped, isProtected, label));
...
sk_sp<GrTexture> GrResourceProvider::createTexture(SkISize dimensions, const GrBackendFormat& format, GrTextureType textureType, GrColorType colorType,
    GrRenderable renderable, int renderTargetSampleCnt, skgpu::Budgeted budgeted, SkBackingFit fit, GrProtected isProtected, const GrMipLevel& mipLevel,
    std::string_view label)
{
    ...
    if (SkBackingFit::kApprox == fit) {
        ...
        if (!fCaps->validateSurfaceParams(dimensions, format, renderable, renderTargetSampleCnt, skgpu::Mipmapped::kNo, textureType)) {
            return nullptr;
        }

        auto tex = this->createApproxTexture(dimensions, format, textureType, renderable, renderTargetSampleCnt, isProtected, label);
        if (!tex) {
            return nullptr;
        }
        ...
        return this->createTexture(
            dimensions, format, textureType, colorType, renderable, renderTargetSampleCnt, budgeted, skgpu::Mipmapped::kNo, isProtected, &mipLevel, label);
    }
}

sk_sp<GrTexture> GrResourceProvider::createCompressedTexture(SkISize dimensions, const GrBackendFormat& format, skgpu::Budgeted budgeted,
    skgpu::Mipmapped mipmapped, GrProtected isProtected, SkData* data, std::string_view label)
{
    ASSERT_SINGLE_OWNER
    if (this->isAbandoned()) {
        return nullptr;
    }
    return fGpu->createCompressedTexture(dimensions, format, budgeted, mipmapped, isProtected, data->data(), data->size());
}

sk_sp<GrTexture> GrResourceProvider::createTexture(SkISize dimensions, const GrBackendFormat& format, GrTextureType textureType, GrRenderable renderable,
    int renderTargetSampleCnt, skgpu::Mipmapped mipmapped, skgpu::Budgeted budgeted, GrProtected isProtected, std::string_view label)
{
    ASSERT_SINGLE_OWNER
    if (this->isAbandoned()) {
        return nullptr;
    ...
    sk_sp<GrTexture> tex = this->getExactScratch(dimensions, format, textureType, renderable, renderTargetSampleCnt, budgeted, mipmapped, isProtected, label);
    if (tex) {
        return tex;
    }

    return fGpu->createTexture(dimensions, format, textureType, renderable, renderTargetSampleCnt, mipmapped, budgeted, isProtected, label);
}

sk_sp<GrTexture> GrResourceProvider::createApproxTexture(SkISize dimensions, const GrBackendFormat& format, GrTextureType textureType, GrRenderable renderable,
    int renderTargetSampleCnt, GrProtected isProtected, std::string_view label)
{
    ASSERT_SINGLE_OWNER
    ...
    if (auto tex
        = this->findAndRefScratchTexture(copyDimensions, format, textureType, renderable, renderTargetSampleCnt, skgpu::Mipmapped::kNo, isProtected, label)) {
        return tex;
    }

    return fGpu->createTexture(
        copyDimensions, format, textureType, renderable, renderTargetSampleCnt, skgpu::Mipmapped::kNo, skgpu::Budgeted::kYes, isProtected, label);
}
...
sk_sp<GrTexture> GrResourceProvider::findAndRefScratchTexture(SkISize dimensions, const GrBackendFormat& format, GrTextureType textureType,
    GrRenderable renderable, int renderTargetSampleCnt, skgpu::Mipmapped mipmapped, GrProtected isProtected, std::string_view label)
{
    ASSERT_SINGLE_OWNER
    SkASSERT(!this->isAbandoned());
    SkASSERT(!this->caps()->isFormatCompressed(format));
    SkASSERT(fCaps->validateSurfaceParams(dimensions, format, renderable, renderTargetSampleCnt, skgpu::Mipmapped::kNo, textureType));

    // We could make initial clears work with scratch textures but it is a rare case so we just opt
    // to fall back to making a new texture.
    if (fGpu->caps()->reuseScratchTextures() || renderable == GrRenderable::kYes) {
        skgpu::ScratchKey key;
        GrTexture::ComputeScratchKey(*this->caps(), format, dimensions, renderable, renderTargetSampleCnt, mipmapped, isProtected, &key);
...
sk_sp<GrTexture> GrResourceProvider::wrapBackendTexture(const GrBackendTexture& tex, GrWrapOwnership ownership, GrWrapCacheable cacheable, GrIOType ioType)
{
    ASSERT_SINGLE_OWNER
    if (this->isAbandoned()) {
        return nullptr;
...
sk_sp<GrTexture> GrResourceProvider::wrapCompressedBackendTexture(const GrBackendTexture& tex, GrWrapOwnership ownership, GrWrapCacheable cacheable)
{
    ASSERT_SINGLE_OWNER
    if (this->isAbandoned()) {
        return nullptr;
...
sk_sp<GrTexture> GrResourceProvider::wrapRenderableBackendTexture(
    const GrBackendTexture& tex, int sampleCnt, GrWrapOwnership ownership, GrWrapCacheable cacheable)
{
    ASSERT_SINGLE_OWNER
    if (this->isAbandoned()) {
        return nullptr;
...
sk_sp<GrRenderTarget> GrResourceProvider::wrapVulkanSecondaryCBAsRenderTarget(const SkImageInfo& imageInfo, const GrVkDrawableInfo& vkInfo)
{
    ASSERT_SINGLE_OWNER
...
sk_sp<const GrGpuBuffer> GrResourceProvider::findOrMakeStaticBuffer(
    GrGpuBufferType intendedType, size_t size, const void* staticData, const skgpu::UniqueKey& key)
{
...
sk_sp<const GrGpuBuffer> GrResourceProvider::findOrMakeStaticBuffer(
    GrGpuBufferType intendedType, size_t size, const skgpu::UniqueKey& uniqueKey, InitializeBufferFn initializeBufferFn)
{
...
sk_sp<const GrGpuBuffer> GrResourceProvider::createPatternedIndexBuffer(
    const uint16_t* pattern, int patternSize, int reps, int vertCount, const skgpu::UniqueKey* key)
{
    size_t bufferSize = patternSize * reps * sizeof(uint16_t);

    sk_sp<GrGpuBuffer> buffer = this->createBuffer(bufferSize, GrGpuBufferType::kIndex, kStatic_GrAccessPattern, ZeroInit::kNo);
    if (!buffer) {
        return nullptr;
    }
...
sk_sp<const GrGpuBuffer> GrResourceProvider::createNonAAQuadIndexBuffer()
{
    ...
    return this->createPatternedIndexBuffer(kNonAAQuadIndexPattern, kIndicesPerNonAAQuad, kMaxNumNonAAQuads, kVertsPerNonAAQuad, nullptr);
}
...
sk_sp<const GrGpuBuffer> GrResourceProvider::createAAQuadIndexBuffer()
{
    ...
    return this->createPatternedIndexBuffer(kAAQuadIndexPattern, kIndicesPerAAQuad, kMaxNumAAQuads, kVertsPerAAQuad, nullptr);
}
...
sk_sp<GrGpuBuffer> GrResourceProvider::createBuffer(size_t size, GrGpuBufferType intendedType, GrAccessPattern accessPattern, ZeroInit zeroInit)
{
    if (this->isAbandoned()) {
        return nullptr;
    }
    ...
    size_t allocSize = intendedType == GrGpuBufferType::kUniform ? std::max(size, MIN_UNIFORM_SIZE) : std::max(size, MIN_SIZE);
...
sk_sp<GrGpuBuffer> GrResourceProvider::createBuffer(const void* data, size_t size, GrGpuBufferType type, GrAccessPattern pattern)
{
    SkASSERT(data);
...
bool GrResourceProvider::attachStencilAttachment(GrRenderTarget* rt, bool useMSAASurface)
{
    ...
    if (!rt->wasDestroyed() && rt->canAttemptStencilAttachment(useMSAASurface)) {
        ...
        GrAttachment::ComputeSharedAttachmentUniqueKey(*this->caps(), stencilFormat, rt->dimensions(), GrAttachment::UsageFlags::kStencilAttachment,
            numStencilSamples, skgpu::Mipmapped::kNo, isProtected, GrMemoryless::kNo, &sbKey);
...
sk_sp<GrAttachment> GrResourceProvider::getDiscardableMSAAAttachment(
    SkISize dimensions, const GrBackendFormat& format, int sampleCnt, GrProtected isProtected, GrMemoryless memoryless)
{
    ASSERT_SINGLE_OWNER
    ...
    if (!fCaps->validateSurfaceParams(dimensions, format, GrRenderable::kYes, sampleCnt, skgpu::Mipmapped::kNo, GrTextureType::kNone)) {
        return nullptr;
    }

    skgpu::UniqueKey key;
    GrAttachment::ComputeSharedAttachmentUniqueKey(
        *this->caps(), format, dimensions, GrAttachment::UsageFlags::kColorAttachment, sampleCnt, skgpu::Mipmapped::kNo, isProtected, memoryless, &key);
...
sk_sp<GrAttachment> GrResourceProvider::refScratchMSAAAttachment(
    SkISize dimensions, const GrBackendFormat& format, int sampleCnt, GrProtected isProtected, GrMemoryless memoryless, std::string_view label)
{
    ASSERT_SINGLE_OWNER
    SkASSERT(!this->isAbandoned());
    SkASSERT(!this->caps()->isFormatCompressed(format));
    SkASSERT(fCaps->validateSurfaceParams(dimensions, format, GrRenderable::kYes, sampleCnt, skgpu::Mipmapped::kNo, GrTextureType::kNone));
...
static bool prepare_level(const GrMipLevel& inLevel, SkISize dimensions, bool rowBytesSupport, GrColorType origColorType, GrColorType allowedColorType,
...
GrColorType GrResourceProvider::prepareLevels(const GrBackendFormat& format, GrColorType colorType, SkISize baseSize, const GrMipLevel texels[],
    int mipLevelCount, TempLevels* tempLevels, TempLevelDatas* tempLevelDatas) const
{
    ...
    for (int i = 0; i < mipLevelCount; ++i) {
        if (!prepare_level(texels[i], size, rowBytesSupport, colorType, allowedColorType, &(*tempLevels)[i], &(*tempLevelDatas)[i])) {
            return GrColorType::kUnknown;
        }
...
sk_sp<GrTexture> GrResourceProvider::writePixels(
    sk_sp<GrTexture> texture, GrColorType colorType, SkISize baseSize, const GrMipLevel texels[], int mipLevelCount) const
{
    SkASSERT(!this->isAbandoned());
    SkASSERT(texture);
    ...
    AutoSTArray<14, GrMipLevel> tmpTexels;
    AutoSTArray<14, std::unique_ptr<char[]>> tmpDatas;
    auto tempColorType = this->prepareLevels(texture->backendFormat(), colorType, baseSize, texels, mipLevelCount, &tmpTexels, &tmpDatas);
    if (tempColorType == GrColorType::kUnknown) {
        return nullptr;
    }
    SkAssertResult(fGpu->writePixels(texture.get(), SkIRect::MakeSize(baseSize), colorType, tempColorType, tmpTexels.get(), mipLevelCount));
    return texture;
}
