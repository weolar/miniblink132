...
DECLARE_SKMESSAGEBUS_MESSAGE(GrResourceCache::UnrefResourceMessage, GrDirectContext::DirectContextID,
...
GrResourceCache::GrResourceCache(skgpu::SingleOwner* singleOwner, GrDirectContext::DirectContextID owningContextID, uint32_t familyID)
...
void GrResourceCache::insertResource(GrGpuResource* resource)
{
    ...
    if (GrBudgetedType::kBudgeted == resource->resourcePriv().budgetedType()) {
        ++fBudgetedCount;
        fBudgetedBytes += size;
        TRACE_COUNTER2("skia.gpu.cache", "skia budget", "used", fBudgetedBytes, "free", fMaxBytes - fBudgetedBytes);
#if GR_CACHE_STATS
...
void GrResourceCache::refAndMakeResourceMRU(GrGpuResource* resource)
{
    ...
    if (resource->resourcePriv().isPurgeable()) {
        ...
        fPurgeableQueue.remove(resource);
        this->addToNonpurgeableArray(resource);
    } else if (!resource->cacheAccess().hasRefOrCommandBufferUsage() && resource->resourcePriv().budgetedType() == GrBudgetedType::kBudgeted) {
...
void GrResourceCache::notifyARefCntReachedZero(GrGpuResource* resource, GrGpuResource::LastRemovedRef removedRef)
{
    ASSERT_SINGLE_OWNER
    SkASSERT(resource);
    SkASSERT(!resource->wasDestroyed());
    ...
    if (!resource->resourcePriv().isPurgeable() && resource->resourcePriv().budgetedType() == GrBudgetedType::kBudgeted) {
        ++fNumBudgetedResourcesFlushWillMakePurgeable;
    }
...
void GrResourceCache::didChangeBudgetStatus(GrGpuResource* resource)
{
    ...
    if (resource->resourcePriv().budgetedType() == GrBudgetedType::kBudgeted) {
        ...
        fBudgetedHighWaterCount = std::max(fBudgetedCount, fBudgetedHighWaterCount);
#endif
        if (!resource->resourcePriv().isPurgeable() && !resource->cacheAccess().hasRefOrCommandBufferUsage()) {
            ++fNumBudgetedResourcesFlushWillMakePurgeable;
        }
        if (resource->cacheAccess().isUsableAsScratch()) {
            fScratchMap.insert(resource->resourcePriv().getScratchKey(), resource);
        ...
        if (!resource->cacheAccess().hasRef() && !resource->getUniqueKey().isValid() && resource->resourcePriv().getScratchKey().isValid()) {
            fScratchMap.remove(resource->resourcePriv().getScratchKey(), resource);
        }
    }
    ...
    if (!invalidKeyMsgs.empty()) {
        ...
        for (int i = 0; i < invalidKeyMsgs.size(); ++i) {
            if (invalidKeyMsgs[i].inThreadSafeCache()) {
                ...
                fProxyProvider->processInvalidUniqueKey(invalidKeyMsgs[i].key(), nullptr, GrProxyProvider::InvalidateGPUResource::kYes);
...
void GrResourceCache::purgeUnlockedResources(const skgpu::StdSteadyClock::time_point* purgeTime, GrPurgeResourceOptions opts)
{
    if (opts == GrPurgeResourceOptions::kAllResources) {
        ...
        while (fPurgeableQueue.count()) {
            ...
            const skgpu::StdSteadyClock::time_point resourceTime = resource->cacheAccess().timeWhenResourceBecamePurgeable();
        ...
        if (purgeTime && fPurgeableQueue.count() && fPurgeableQueue.peek()->cacheAccess().timeWhenResourceBecamePurgeable() >= *purgeTime) {
            return;
        }
...
{
    return this->overBudget() && !fPurgeableQueue.count() && fNumBudgetedResourcesFlushWillMakePurgeable > 0;
}
...
uint32_t GrResourceCache::getNextTimestamp()
{
    ...
    if (0 == fTimestamp) {
        ...
        if (count) {
            ...
            int currP = 0;
            int currNP = 0;
            while (currP < sortedPurgeableResources.size() && currNP < fNonpurgeableResources.size()) {
...
void GrResourceCache::dumpStats(SkString* out) const
{
    ...
    out->appendf("\t\tEntry Count: current %d"
        ...
        stats.fTotal, fBudgetedCount, stats.fWrapped, stats.fNumNonPurgeable, stats.fScratch, fHighWaterCount);
    out->appendf("\t\tEntry Bytes: current %d (budgeted %d, %.2g%% full, %d unbudgeted) high %d\n", SkToInt(fBytes), SkToInt(fBudgetedBytes), byteUtilization,
        SkToInt(stats.fUnbudgetedSize), SkToInt(fHighWaterBytes));
}

void GrResourceCache::dumpStatsKeyValuePairs(TArray<SkString>* keys, TArray<double>* values) const
{
    this->validate();
...
void GrResourceCache::validate() const
{
    ...
    struct Stats {
        ...
        void update(GrGpuResource* resource)
        {
            ...
            if (resource->cacheAccess().isUsableAsScratch()) {
                ...
                SkASSERT(GrBudgetedType::kBudgeted != resource->resourcePriv().budgetedType() || uniqueKey.isValid() || resource->cacheAccess().hasRef());
                SkASSERT(!resource->resourcePriv().refsWrappedObjects());
            ...
            if (uniqueKey.isValid()) {
                ...
                SkASSERT(GrBudgetedType::kBudgeted == resource->resourcePriv().budgetedType() || resource->resourcePriv().refsWrappedObjects());
            }
    ...
    for (int i = 0; i < fNonpurgeableResources.size(); ++i) {
        SkASSERT(!fNonpurgeableResources[i]->resourcePriv().isPurgeable() || fNewlyPurgeableResourceForValidation == fNonpurgeableResources[i]);
        ...
        if (fNonpurgeableResources[i]->resourcePriv().budgetedType() == GrBudgetedType::kBudgeted
            && !fNonpurgeableResources[i]->cacheAccess().hasRefOrCommandBufferUsage() && fNewlyPurgeableResourceForValidation != fNonpurgeableResources[i]) {
    ...
    SkASSERT(fNumBudgetedResourcesFlushWillMakePurgeable == numBudgetedResourcesFlushWillMakePurgeable);
...
void GrResourceCache::visitSurfaces(const std::function<void(const GrSurface*, bool purgeable)>& func) const
{
