...
sk_sp<GrCpuBuffer> GrBufferAllocPool::CpuBufferCache::makeBuffer(size_t size, bool mustBeInitialized)
{
    SkASSERT(size > 0);
...
#define UNMAP_BUFFER(block)                                                                                                                                    \
    do {                                                                                                                                                       \
        TRACE_EVENT_INSTANT1("skia.gpu", "GrBufferAllocPool Unmapping Buffer", TRACE_EVENT_SCOPE_THREAD, "percent_unwritten",                                  \
...
void GrBufferAllocPool::validate(bool unusedBlockAllowed) const
{
    ...
    if (!wasDestroyed) {
        ...
        if (unusedBlockAllowed) {
            SkASSERT((fBytesInUse && !fBlocks.empty()) || (!fBytesInUse && (fBlocks.size() < 2)));
        } else {
...
void* GrBufferAllocPool::makeSpaceAtLeast(
    size_t minSize, size_t fallbackSize, size_t alignment, sk_sp<const GrBuffer>* buffer, size_t* offset, size_t* actualSize)
{
    VALIDATE();
    ...
    size_t usedBytes = (fBlocks.empty()) ? 0 : fBlocks.back().fBuffer->size() - fBlocks.back().fBytesFree;
    size_t pad = align_up_pad(usedBytes, alignment);
...
bool GrBufferAllocPool::createBlock(size_t requestSize)
{
    ...
    if (block.fBuffer->isCpuBuffer()) {
        ...
        if (GrCaps::kNone_MapFlags != fGpu->caps()->mapBufferFlags() && size > fGpu->caps()->bufferMapThreshold()) {
...
{
    SkASSERT(!fBlocks.empty());
    SkASSERT(fBlocks.back().fBuffer->isCpuBuffer() || !static_cast<GrGpuBuffer*>(fBlocks.back().fBuffer.get())->isMapped());
    fBlocks.pop_back();
    fBufferPtr = nullptr;
}
...
void GrBufferAllocPool::resetCpuData(size_t newSize)
{
    ...
    fCpuStagingBuffer = fCpuBufferCache ? fCpuBufferCache->makeBuffer(newSize, mustInitialize) : GrCpuBuffer::Make(newSize);
}
...
void GrBufferAllocPool::flushCpuData(const BufferBlock& block, size_t flushSize)
{
    ...
    if (GrCaps::kNone_MapFlags != fGpu->caps()->mapBufferFlags() && flushSize > fGpu->caps()->bufferMapThreshold()) {
...
sk_sp<GrBuffer> GrBufferAllocPool::getBuffer(size_t size)
{
    ...
    if (caps.preferClientSideDynamicBuffers() || (fBufferType == GrGpuBufferType::kDrawIndirect && caps.useClientSideIndirectBuffers())) {
        // Create a CPU buffer.
        bool mustInitialize = caps.mustClearUploadedBufferData();
        return fCpuBufferCache ? fCpuBufferCache->makeBuffer(size, mustInitialize) : GrCpuBuffer::Make(size);
    }
    return resourceProvider->createBuffer(size, fBufferType, kDynamic_GrAccessPattern, GrResourceProvider::ZeroInit::kNo);
}
...
void* GrVertexBufferAllocPool::makeSpace(size_t vertexSize, int vertexCount, sk_sp<const GrBuffer>* buffer, int* startVertex)
{
    ...
    void* ptr = INHERITED::makeSpace(SkSafeMath::Mul(vertexSize, vertexCount), vertexSize, buffer, &offset);
    ...
    size_t vertexSize, int minVertexCount, int fallbackVertexCount, sk_sp<const GrBuffer>* buffer, int* startVertex, int* actualVertexCount)
{
    ...
    void* ptr = INHERITED::makeSpaceAtLeast(
        SkSafeMath::Mul(vertexSize, minVertexCount), SkSafeMath::Mul(vertexSize, fallbackVertexCount), vertexSize, buffer, &offset, &actualSize);
...
void* GrIndexBufferAllocPool::makeSpace(int indexCount, sk_sp<const GrBuffer>* buffer, int* startIndex)
{
    ...
    size_t offset SK_INIT_TO_AVOID_WARNING;
    void* ptr = INHERITED::makeSpace(SkSafeMath::Mul(indexCount, sizeof(uint16_t)), sizeof(uint16_t), buffer, &offset);
...
void* GrIndexBufferAllocPool::makeSpaceAtLeast(int minIndexCount, int fallbackIndexCount, sk_sp<const GrBuffer>* buffer, int* startIndex, int* actualIndexCount)
{
    ...
    size_t offset SK_INIT_TO_AVOID_WARNING;
    size_t actualSize SK_INIT_TO_AVOID_WARNING;
    void* ptr = INHERITED::makeSpaceAtLeast(SkSafeMath::Mul(minIndexCount, sizeof(uint16_t)), SkSafeMath::Mul(fallbackIndexCount, sizeof(uint16_t)),
        sizeof(uint16_t), buffer, &offset, &actualSize);
