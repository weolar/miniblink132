...
GrAtlasManager::GrAtlasManager(
    GrProxyProvider* proxyProvider, size_t maxTextureBytes, GrDrawOpAtlas::AllowMultitexturing allowMultitexturing, bool supportBilerpAtlas)
...
template <typename INT_TYPE> static void expand_bits(INT_TYPE* dst, const uint8_t* src, int width, int height, int dstRowBytes, int srcRowBytes)
{
    ...
    if (maskFormat == expectedMaskFormat) {
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                if (kBGRAIsNative) {
                    color8888 = GrColorPackRGBA(masks.getBlue(color565), masks.getGreen(color565), masks.getRed(color565), 0xFF);
                } else {
                    color8888 = GrColorPackRGBA(masks.getRed(color565), masks.getGreen(color565), masks.getBlue(color565), 0xFF);
                }
...
GrDrawOpAtlas::ErrorCode GrAtlasManager::addGlyphToAtlas(
    const SkGlyph& skGlyph, Glyph* glyph, int srcPadding, GrResourceProvider* resourceProvider, GrDeferredUploadTarget* uploadTarget)
{
    ...
    auto errorCode = this->addToAtlas(resourceProvider, uploadTarget, expectedMaskFormat, width, height, storage.get(), &glyph->fAtlasLocator);
...
GrDrawOpAtlas::ErrorCode GrAtlasManager::addToAtlas(GrResourceProvider* resourceProvider, GrDeferredUploadTarget* target, MaskFormat format, int width,
    int height, const void* image, skgpu::AtlasLocator* atlasLocator)
{
    return this->getAtlas(format)->addToAtlas(resourceProvider, target, width, height, image, atlasLocator);
}

void GrAtlasManager::addGlyphToBulkAndSetUseToken(skgpu::BulkUsePlotUpdater* updater, MaskFormat format, Glyph* glyph, skgpu::AtlasToken token)
{
    SkASSERT(glyph);
...
bool GrAtlasManager::initAtlas(MaskFormat format)
{
    ...
    if (fAtlases[index] == nullptr) {
        ...
        const GrBackendFormat backendFormat = fCaps->getDefaultBackendFormat(grColorType, GrRenderable::kNo);

        fAtlases[index] = GrDrawOpAtlas::Make(fProxyProvider, backendFormat, GrColorTypeToSkColorType(grColorType), GrColorTypeBytesPerPixel(grColorType),
            atlasDimensions.width(), atlasDimensions.height(), plotDimensions.width(), plotDimensions.height(), this, fAllowMultitexturing, nullptr,
            /*label=*/"TextAtlas");
        if (!fAtlases[index]) {
            return false;
        }
...
std::tuple<bool, int> GlyphVector::regenerateAtlasForGanesh(int begin, int end, MaskFormat maskFormat, int srcPadding, GrMeshDrawTarget* target)
{
    ...
    if (fAtlasGeneration != currentAtlasGen) {
        ...
        for (const Variant& variant : glyphs) {
            ...
            if (!atlasManager->hasGlyph(maskFormat, gpuGlyph)) {
                ...
                auto code = atlasManager->addGlyphToAtlas(skGlyph, gpuGlyph, srcPadding, target->resourceProvider(), uploadTarget);
                if (code != GrDrawOpAtlas::ErrorCode::kSucceeded) {
                    success = code != GrDrawOpAtlas::ErrorCode::kError;
                    break;
                }
            }
            atlasManager->addGlyphToBulkAndSetUseToken(&fBulkUseUpdater, maskFormat, gpuGlyph, tokenTracker->nextDrawToken());
            glyphsPlacedInAtlas++;
        }
        ...
        if (end == SkCount(fGlyphs)) {
            ...
            atlasManager->setUseTokenBulk(fBulkUseUpdater, uploadTarget->tokenTracker()->nextDrawToken(), maskFormat);
        }
