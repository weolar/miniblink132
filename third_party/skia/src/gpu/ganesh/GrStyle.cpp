...
void GrStyle::WriteKey(uint32_t* key, const GrStyle& style, Apply apply, SkScalar scale, uint32_t flags)
{
    SkASSERT(key);
...
bool GrStyle::applyPathEffect(SkPath* dst, SkStrokeRec* strokeRec, const SkPath& src) const
{
    ...
    if (SkPathEffectBase::DashType::kDash == fDashInfo.fType) {
        ...
        SkDashPath::CalcDashParameters(phase, intervals, intervalCnt, &initialLength, &initialIndex, &intervalLength);
        if (!SkDashPath::InternalFilter(dst, src, strokeRec, nullptr, intervals, intervalCnt, initialLength, initialIndex, intervalLength, phase,
...
bool GrStyle::applyPathEffectToPath(SkPath* dst, SkStrokeRec* remainingStroke, const SkPath& src, SkScalar resScale) const
{
    SkASSERT(dst);
...
bool GrStyle::applyToPath(SkPath* dst, SkStrokeRec::InitStyle* style, const SkPath& src, SkScalar resScale) const
{
    SkASSERT(style);
    SkASSERT(dst);
    SkStrokeRec strokeRec = fStrokeRec;
    ...
    if (strokeRec.needToApply()) {
        ...
        *style = strokeRec.getStyle() == SkStrokeRec::kFill_Style ? SkStrokeRec::kFill_InitStyle : SkStrokeRec::kHairline_InitStyle;
    }
    return true;
}
