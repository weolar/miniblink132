...
void GrDirectContextPriv::createDDLTask(sk_sp<const GrDeferredDisplayList> ddl, sk_sp<GrRenderTargetProxy> newDest)
{
...
void GrDirectContextPriv::dumpCacheStatsKeyValuePairs(TArray<SkString>* keys, TArray<double>* values) const
{
#if GR_CACHE_STATS
...
void GrDirectContextPriv::dumpGpuStatsKeyValuePairs(TArray<SkString>* keys, TArray<double>* values) const
{
#if GR_GPU_STATS
...
void GrDirectContextPriv::dumpContextStatsKeyValuePairs(TArray<SkString>* keys, TArray<double>* values) const
{
#if GR_GPU_STATS
...
/////////////////////////////////////////////////
sk_sp<SkImage> GrDirectContextPriv::testingOnly_getFontAtlasImage(MaskFormat format, unsigned int index)
{
    ...
    return sk_make_sp<SkImage_Ganesh>(sk_ref_sp(this->context()), kNeedNewImageUniqueID, views[index], SkColorInfo(colorType, kPremul_SkAlphaType, nullptr));
}

void GrDirectContextPriv::testingOnly_flushAndRemoveOnFlushCallbackObject(GrOnFlushCallbackObject* cb)
{
...
static bool test_for_preserving_PM_conversions(GrDirectContext* dContext)
{
    ...
    const SkImageInfo pmII = SkImageInfo::Make(kSize, kSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    const SkImageInfo upmII = pmII.makeAlphaType(kUnpremul_SkAlphaType);

    auto readSFC = dContext->priv().makeSFC(upmII, "ReadSfcForPMUPMConversion", SkBackingFit::kExact);
    auto tempSFC = dContext->priv().makeSFC(pmII, "TempSfcForPMUPMConversion", SkBackingFit::kExact);
    if (!readSFC || !tempSFC) {
        return false;
    }
    ...
    auto fp2 = make_premul_effect(GrTextureEffect::Make(readSFC->readSurfaceView(), readSFC->colorInfo().alphaType()));
    tempSFC->fillRectWithFP(SkIRect::MakeWH(kSize, kSize), std::move(fp2));

    auto fp3 = make_unpremul_effect(GrTextureEffect::Make(tempSFC->readSurfaceView(), tempSFC->colorInfo().alphaType()));
