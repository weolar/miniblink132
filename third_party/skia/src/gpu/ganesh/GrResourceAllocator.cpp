...
void GrResourceAllocator::addInterval(
    GrSurfaceProxy* proxy, unsigned int start, unsigned int end, ActualUse actualUse, AllowRecycling allowRecycling SkDEBUGCODE(, bool isDirectDstRead))
{
    SkASSERT(start <= end);
...
GrResourceAllocator::Register::Register(GrSurfaceProxy* originatingProxy, skgpu::ScratchKey scratchKey, GrResourceProvider* provider)
    ...
    , fScratchKey(std::move(scratchKey))
{
    ...
    SkDEBUGCODE(fUniqueID = CreateUniqueID();) if (fScratchKey.isValid())
    {
        if (can_proxy_use_scratch(*provider->caps(), originatingProxy)) {
            fExistingSurface = provider->findAndRefScratchTexture(fScratchKey, /*label=*/"ResourceAllocatorRegister");
        }
    }
    else
    {
...
bool GrResourceAllocator::Register::isRecyclable(const GrCaps& caps, GrSurfaceProxy* proxy, int knownUseCount, AllowRecycling allowRecycling) const
{
    if (allowRecycling == AllowRecycling::kNo) {
        return false;
    }
...
bool GrResourceAllocator::Register::instantiateSurface(GrSurfaceProxy* proxy, GrResourceProvider* resourceProvider)
{
    ...
    if (skgpu::Budgeted::kYes == proxy->isBudgeted() && GrBudgetedType::kBudgeted != surface->resourcePriv().budgetedType()) {
...
void GrResourceAllocator::expire(unsigned int curIndex)
{
    while (!fActiveIntvls.empty() && fActiveIntvls.peekHead()->end() < curIndex) {
        ...
        if (r && r->isRecyclable(*fDContext->priv().caps(), intvl->proxy(), intvl->uses(), intvl->allowRecycling())) {
...
void GrResourceAllocator::reset()
{
    ...
    // bailing early.
    SkDEBUGCODE(fPlanned = false;) SkDEBUGCODE(fAssigned = false;) SkASSERT(fActiveIntvls.empty());
...
bool GrResourceAllocator::assign()
{
    ...
    SkASSERT(fPlanned && !fAssigned);
    SkDEBUGCODE(fAssigned = true;) auto resourceProvider = fDContext->priv().resourceProvider();
...
void GrResourceAllocator::dumpIntervals()
{
    ...
    for (const Interval* cur = fIntvlList.peekHead(); cur; cur = cur->next()) {
        SkDebugf("{ %3d,%3d }: [%2d, %2d] - refProxys:%d surfaceRefs:%d\n", cur->proxy()->uniqueID().asUInt(),
            cur->proxy()->isInstantiated() ? cur->proxy()->underlyingUniqueID().asUInt() : -1, cur->start(), cur->end(), cur->proxy()->priv().getProxyRefCnt(),
    ...
    for (const Interval* cur = fIntvlList.peekHead(); cur; cur = cur->next()) {
        SkDebugf("{ %3d,%3d }: ", cur->proxy()->uniqueID().asUInt(), cur->proxy()->isInstantiated() ? cur->proxy()->underlyingUniqueID().asUInt() : -1);
