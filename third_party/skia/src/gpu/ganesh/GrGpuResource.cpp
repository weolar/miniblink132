...
void GrGpuResource::registerWithCache(skgpu::Budgeted budgeted)
{
    ...
    fBudgetedType = budgeted == skgpu::Budgeted::kYes ? GrBudgetedType::kBudgeted : GrBudgetedType::kUnbudgetedUncacheable;
...
void GrGpuResource::registerWithCacheWrapped(GrWrapCacheable wrapType)
{
    ...
    fBudgetedType = wrapType == GrWrapCacheable::kNo ? GrBudgetedType::kUnbudgetedUncacheable : GrBudgetedType::kUnbudgetedCacheable;
...
void GrGpuResource::dumpMemoryStatistics(SkTraceMemoryDump* traceMemoryDump) const
{
    ...
    this->dumpMemoryStatisticsPriv(traceMemoryDump, this->getResourceName(), this->getResourceType(), this->gpuMemorySize());
}

void GrGpuResource::dumpMemoryStatisticsPriv(SkTraceMemoryDump* traceMemoryDump, const SkString& resourceName, const char* type, size_t size) const
{
...
bool GrGpuResource::isPurgeable() const
{
    ...
    return !this->hasRef() && this->hasNoCommandBufferUsages() && !(fBudgetedType == GrBudgetedType::kUnbudgetedCacheable && fUniqueKey.isValid());
}

bool GrGpuResource::hasRef() const
{
    return this->internalHasRef();
}
...
void GrGpuResource::setUniqueKey(const skgpu::UniqueKey& key)
{
    ...
    if (this->resourcePriv().budgetedType() != GrBudgetedType::kBudgeted && !this->fRefsWrappedObjects) {
        return;
    }
...
{
    if (!this->wasDestroyed() && fBudgetedType == GrBudgetedType::kBudgeted && !fUniqueKey.isValid()) {
