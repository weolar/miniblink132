...
template <typename Base, GrGpuBufferType Type> sk_sp<Base> GrMeshBuffer<Base, Type>::Make(GrDirectContext* dc, const void* data, size_t size)
{
    SkASSERT(dc);

    sk_sp<GrGpuBuffer> buffer = dc->priv().resourceProvider()->createBuffer(
        size, Type, kStatic_GrAccessPattern, data ? GrResourceProvider::ZeroInit::kNo : GrResourceProvider::ZeroInit::kYes);
    if (!buffer) {
        return nullptr;
    }
...
template <typename Base, GrGpuBufferType Type> bool GrMeshBuffer<Base, Type>::onUpdate(GrDirectContext* dc, const void* data, size_t offset, size_t size)
{
    ...
    if (!dc->priv().caps()->transferFromBufferToBufferSupport()) {
        ...
        dc->priv().drawingManager()->newBufferUpdateTask(std::move(ownedData), fBuffer, offset);
        return true;
    }
    ...
    if (!tempBuffer) {
        tempBuffer
            = dc->priv().resourceProvider()->createBuffer(size, GrGpuBufferType::kXferCpuToGpu, kDynamic_GrAccessPattern, GrResourceProvider::ZeroInit::kNo);
        if (!tempBuffer) {
            return false;
        }
    ...
    dc->priv().drawingManager()->newBufferTransferTask(std::move(tempBuffer), tempOffset, fBuffer, offset, size);
