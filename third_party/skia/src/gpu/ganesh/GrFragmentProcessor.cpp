...
{
    this->visitTextureEffects([&func](const GrTextureEffect& te) { func(te.view().proxy(), te.samplerState().mipmapped()); });
}
...
void GrFragmentProcessor::visitWithImpls(const std::function<void(const GrFragmentProcessor&, ProgramImpl&)>& f, ProgramImpl& impl) const
{
    f(*this, impl);
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::MakeColor(SkPMColor4f color)
{
    ...
    return GrSkSLFP::Make(
        effect, "color_fp", /*inputFP=*/nullptr, color.isOpaque() ? GrSkSLFP::OptFlags::kPreservesOpaqueInput : GrSkSLFP::OptFlags::kNone, "color", color);
}
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::ApplyPaintAlpha(std::unique_ptr<GrFragmentProcessor> child)
{
    ...
    return GrSkSLFP::Make(effect, "ApplyPaintAlpha", /*inputFP=*/nullptr,
        GrSkSLFP::OptFlags::kPreservesOpaqueInput | GrSkSLFP::OptFlags::kCompatibleWithCoverageAsAlpha, "fp", std::move(child));
}

std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::ModulateRGBA(std::unique_ptr<GrFragmentProcessor> inputFP, const SkPMColor4f& color)
{
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::SwizzleOutput(std::unique_ptr<GrFragmentProcessor> fp, const skgpu::Swizzle& swizzle)
{
    ...
    public:
        static std::unique_ptr<GrFragmentProcessor> Make(std::unique_ptr<GrFragmentProcessor> fp, const skgpu::Swizzle& swizzle)
        {
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::OverrideInput(std::unique_ptr<GrFragmentProcessor> fp, const SkPMColor4f& color)
{
    ...
    return GrSkSLFP::Make(effect, "OverrideInput", /*inputFP=*/nullptr,
        color.isOpaque() ? GrSkSLFP::OptFlags::kPreservesOpaqueInput : GrSkSLFP::OptFlags::kNone, "fp", std::move(fp), "color", color);
}
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::DisableCoverageAsAlpha(std::unique_ptr<GrFragmentProcessor> fp)
{
    ...
    static const SkRuntimeEffect* effect = SkMakeRuntimeEffect(SkRuntimeEffect::MakeForColorFilter, "half4 main(half4 inColor) { return inColor; }");
    SkASSERT(SkRuntimeEffectPriv::SupportsConstantOutputForConstantInput(effect));
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::Compose(std::unique_ptr<GrFragmentProcessor> f, std::unique_ptr<GrFragmentProcessor> g)
{
    ...
    public:
        static std::unique_ptr<GrFragmentProcessor> Make(std::unique_ptr<GrFragmentProcessor> f, std::unique_ptr<GrFragmentProcessor> g)
        {
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::ColorMatrix(
    std::unique_ptr<GrFragmentProcessor> child, const float matrix[20], bool unpremulInput, bool clampRGBOutput, bool premulOutput)
{
    static const SkRuntimeEffect* effect = SkMakeRuntimeEffect(SkRuntimeEffect::MakeForColorFilter,
        "uniform half4x4 m;"
        "uniform half4 v;"
    ...
    SkM44 m44(matrix[0], matrix[1], matrix[2], matrix[3], matrix[5], matrix[6], matrix[7], matrix[8], matrix[10], matrix[11], matrix[12], matrix[13],
        matrix[15], matrix[16], matrix[17], matrix[18]);
    SkV4 v4 = { matrix[4], matrix[9], matrix[14], matrix[19] };
    return GrSkSLFP::Make(effect, "ColorMatrix", std::move(child), GrSkSLFP::OptFlags::kNone, "m", m44, "v", v4, "unpremulInput",
        GrSkSLFP::Specialize(unpremulInput ? 1 : 0), "clampRGBOutput", GrSkSLFP::Specialize(clampRGBOutput ? 1 : 0), "premulOutput",
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::Rect(std::unique_ptr<GrFragmentProcessor> inputFP, GrClipEdgeType edgeType, SkRect rect)
{
    ...
    auto rectFP = GrSkSLFP::Make(effect, "Rect", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kCompatibleWithCoverageAsAlpha, "edgeType",
        GrSkSLFP::Specialize(static_cast<int>(edgeType)), "rectUniform", rectUniform);
...
GrFPResult GrFragmentProcessor::Circle(std::unique_ptr<GrFragmentProcessor> inputFP, GrClipEdgeType edgeType, SkPoint center, float radius)
{
    ...
    auto circleFP = GrSkSLFP::Make(effect, "Circle", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kCompatibleWithCoverageAsAlpha, "edgeType",
        GrSkSLFP::Specialize(static_cast<int>(edgeType)), "circle", circle);
...
GrFPResult GrFragmentProcessor::Ellipse(
    std::unique_ptr<GrFragmentProcessor> inputFP, GrClipEdgeType edgeType, SkPoint center, SkPoint radii, const GrShaderCaps& caps)
{
    ...
    auto ellipseFP = GrSkSLFP::Make(effect, "Ellipse", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kCompatibleWithCoverageAsAlpha, "edgeType",
        GrSkSLFP::Specialize(static_cast<int>(edgeType)), "medPrecision", GrSkSLFP::Specialize<int>(medPrecision), "ellipse", ellipse, "scale", scale);
    return GrFPSuccess(GrBlendFragmentProcessor::Make<SkBlendMode::kModulate>(std::move(ellipseFP), std::move(inputFP)));
}
...
SkString ProgramImpl::invokeChild(int childIndex, const char* inputColor, const char* destColor, EmitArgs& args, std::string_view skslCoords)
{
...
SkString ProgramImpl::invokeChildWithMatrix(int childIndex, const char* inputColor, const char* destColor, EmitArgs& args)
{
    SkASSERT(childIndex >= 0);
    ...
    GrShaderVar uniform = args.fUniformHandler->getUniformMapping(args.fFp, SkString(SkSL::SampleUsage::MatrixUniformName()));
