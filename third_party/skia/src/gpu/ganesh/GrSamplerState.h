...
public:
    ...
    constexpr /*explicit*/ GrSamplerState(Filter filter)
        : fFilter(filter)
    {
    }
    ...
    static constexpr GrSamplerState Aniso(WrapMode wrapX, WrapMode wrapY, int maxAniso, skgpu::Mipmapped viewIsMipped)
    {
        ...
        sampler.fMipmapMode = viewIsMipped == skgpu::Mipmapped::kYes ? MipmapMode::kLinear : MipmapMode::kNone;
        return sampler;
    }
    ...
    {
        return fWrapModes[0] == that.fWrapModes[0] && fWrapModes[1] == that.fWrapModes[1] && fFilter == that.fFilter && fMipmapMode == that.fMipmapMode;
    }
    ...
    uint32_t asKey(bool anisoIsOrthogonal) const
    {
        ...
        uint32_t key = (static_cast<uint32_t>(fWrapModes[0]) << kWrapXShift) | (static_cast<uint32_t>(fWrapModes[1]) << kWrapYShift)
            | (static_cast<uint32_t>(fMaxAniso) << kMaxAnisoShift);
        if (fMaxAniso == 1 || anisoIsOrthogonal) {
            key |= (static_cast<uint32_t>(fFilter) << kFilterShift) | (static_cast<uint32_t>(fMipmapMode) << kMipmapModeShift);
        }
        return key;
    }
