...
size_t GrComputeTightCombinedBufferSize(size_t bytesPerPixel, SkISize baseDimensions, TArray<size_t>* individualMipOffsets, int mipLevelCount)
{
    ...
    SkASSERT(bytesPerPixel == 1 || bytesPerPixel == 2 || bytesPerPixel == 3 || bytesPerPixel == 4 || bytesPerPixel == 8 || bytesPerPixel == 16);
...
static skgpu::Swizzle get_load_and_src_swizzle(GrColorType ct, SkRasterPipelineOp* load, bool* isNormalized, bool* isSRGB)
{
    ...
    *isSRGB = false;
    ...
    case GrColorType::kAlpha_8:
        *load = SkRasterPipelineOp::load_a8;
        break;
    case GrColorType::kAlpha_16:
        *load = SkRasterPipelineOp::load_a16;
        break;
    ...
    case GrColorType::kRGBA_8888:
        *load = SkRasterPipelineOp::load_8888;
        break;
    case GrColorType::kRG_88:
        *load = SkRasterPipelineOp::load_rg88;
        break;
    ...
    case GrColorType::kRGBA_10x6:
        *load = SkRasterPipelineOp::load_10x6;
        break;
    case GrColorType::kAlpha_F16:
        *load = SkRasterPipelineOp::load_af16;
        break;
    case GrColorType::kRGBA_F16_Clamped:
        *load = SkRasterPipelineOp::load_f16;
        break;
    case GrColorType::kRG_1616:
        *load = SkRasterPipelineOp::load_rg1616;
        break;
    case GrColorType::kRGBA_16161616:
        *load = SkRasterPipelineOp::load_16161616;
        break;
    ...
    case GrColorType::kRG_F16:
        *load = SkRasterPipelineOp::load_rgf16;
        *isNormalized = false;
        break;
    ...
    case GrColorType::kRGBA_F32:
        *load = SkRasterPipelineOp::load_f32;
        *isNormalized = false;
        break;
    ...
    *lumMode = LumMode::kNone;
    ...
    case GrColorType::kRGB_101010x:
        ...
        *store = SkRasterPipelineOp::store_1010102;
        break;
    case GrColorType::kRGBA_10x6:
        *store = SkRasterPipelineOp::store_10x6;
        break;
    case GrColorType::kRGBA_F16_Clamped:
        *store = SkRasterPipelineOp::store_f16;
        break;
    case GrColorType::kRG_1616:
        *store = SkRasterPipelineOp::store_rg1616;
        break;
    case GrColorType::kRGBA_16161616:
        *store = SkRasterPipelineOp::store_16161616;
        break;
    ...
    case GrColorType::kRGBA_8888_SRGB:
        ...
        *isSRGB = true;
        break;
    case GrColorType::kRG_F16:
        *store = SkRasterPipelineOp::store_rgf16;
        *isNormalized = false;
        break;
    case GrColorType::kAlpha_F16:
    ...
    case GrColorType::kRGB_F16F16F16x:
        ...
        *isNormalized = false;
        break;
    case GrColorType::kRGBA_F32:
        *store = SkRasterPipelineOp::store_f32;
        *isNormalized = false;
        break;
    case GrColorType::kAlpha_8xxx:
        *store = SkRasterPipelineOp::store_8888;
...
bool GrConvertPixels(const GrPixmap& dst, const GrCPixmap& src, bool flipY)
{
    ...
    bool unpremul = src.alphaType() == kPremul_SkAlphaType && dst.alphaType() == kUnpremul_SkAlphaType;
    bool alphaOrCSConversion = premul || unpremul || !SkColorSpace::Equals(src.colorSpace(), dst.colorSpace());
    ...
    bool dstIsSRGB;
    auto storeSwizzle = get_dst_swizzle_and_store(dst.colorType(), &store, &lumMode, &dstIsNormalized, &dstIsSRGB);
    ...
    SkRasterPipeline_MemoryCtx srcCtx { const_cast<void*>(src.addr()), SkToInt(src.rowBytes() / srcBpp) },
...
bool GrClearImage(const GrImageInfo& dstInfo, void* dst, size_t dstRB, std::array<float, 4> color)
{
    ...
    bool dstIsSRGB;
    SkRasterPipelineOp store;
    skgpu::Swizzle storeSwizzle = get_dst_swizzle_and_store(dstInfo.colorType(), &store, &lumMode, &isNormalized, &dstIsSRGB);
    char block[64];
