...
public:
    ...
    static sk_sp<GrSurfaceProxy> Copy(GrRecordingContext*, sk_sp<GrSurfaceProxy> src, GrSurfaceOrigin, skgpu::Mipmapped, SkIRect srcRect, SkBackingFit,
        skgpu::Budgeted, std::string_view label, RectsMustMatch = RectsMustMatch::kNo, sk_sp<GrRenderTask>* outTask = nullptr);

    // Same as above Copy but copies the entire 'src'
    static sk_sp<GrSurfaceProxy> Copy(GrRecordingContext*, sk_sp<GrSurfaceProxy> src, GrSurfaceOrigin, skgpu::Mipmapped, SkBackingFit, skgpu::Budgeted,
        std::string_view label, sk_sp<GrRenderTask>* outTask = nullptr);
    ...
    void setDebugName(SkString name)
    {
        fDebugName = std::move(name);
    }
#endif
...
protected:
    ...
    GrSurfaceProxy(LazyInstantiateCallback&&, const GrBackendFormat&, SkISize, SkBackingFit, skgpu::Budgeted, GrProtected, GrInternalSurfaceFlags, UseAllocator,
...
private:
    ...
    SkDEBUGCODE(void validateSurface(const GrSurface*);) SkDEBUGCODE(virtual void onValidateSurface(const GrSurface*) = 0;)
        ...
        static const size_t kInvalidGpuMemorySize
        = ~static_cast<size_t>(0);
    SkDEBUGCODE(size_t getRawGpuMemorySize_debugOnly() const { return fGpuMemorySize; })

        virtual size_t onUninstantiatedGpuMemorySize() const
        = 0;
