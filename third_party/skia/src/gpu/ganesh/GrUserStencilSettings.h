...
struct GrUserStencilSettings {
    ...
    template <uint16_t Ref, GrUserStencilTest Test, uint16_t TestMask, GrUserStencilOp PassOp, GrUserStencilOp FailOp, uint16_t WriteMask> struct Init { };

    template <uint16_t CWRef, uint16_t CCWRef, GrUserStencilTest CWTest, GrUserStencilTest CCWTest, uint16_t CWTestMask, uint16_t CCWTestMask,
        GrUserStencilOp CWPassOp, GrUserStencilOp CCWPassOp, GrUserStencilOp CWFailOp, GrUserStencilOp CCWFailOp, uint16_t CWWriteMask, uint16_t CCWWriteMask>
    struct InitSeparate { };
    ...
    template <uint16_t CWRef, uint16_t CCWRef, GrUserStencilTest CWTest, GrUserStencilTest CCWTest, uint16_t CWTestMask, uint16_t CCWTestMask,
        GrUserStencilOp CWPassOp, GrUserStencilOp CCWPassOp, GrUserStencilOp CWFailOp, GrUserStencilOp CCWFailOp, uint16_t CWWriteMask, uint16_t CCWWriteMask>
    constexpr static InitSeparate<CWRef, CCWRef, CWTest, CCWTest, CWTestMask, CCWTestMask, CWPassOp, CCWPassOp, CWFailOp, CCWFailOp, CWWriteMask, CCWWriteMask>
    StaticInitSeparate()
    {
        return InitSeparate<CWRef, CCWRef, CWTest, CCWTest, CWTestMask, CCWTestMask, CWPassOp, CCWPassOp, CWFailOp, CCWFailOp, CWWriteMask, CCWWriteMask>();
    }
    ...
    constexpr explicit GrUserStencilSettings(const Init<Ref, Test, TestMask, PassOp, FailOp, WriteMask>&)
        : fCWFlags { (uint16_t)(Attrs::Flags(false) | kSingleSided_StencilFlag), (uint16_t)(Attrs::Flags(true) | kSingleSided_StencilFlag) }
        , fCWFace { Ref, Test, Attrs::EffectiveTestMask(TestMask), PassOp, FailOp, Attrs::EffectiveWriteMask(WriteMask) }
        , fCCWFlags { (uint16_t)(Attrs::Flags(false) | kSingleSided_StencilFlag), (uint16_t)(Attrs::Flags(true) | kSingleSided_StencilFlag) }
        , fCCWFace { Ref, Test, Attrs::EffectiveTestMask(TestMask), PassOp, FailOp, Attrs::EffectiveWriteMask(WriteMask) }
    {
    }

    template <uint16_t CWRef, uint16_t CCWRef, GrUserStencilTest CWTest, GrUserStencilTest CCWTest, uint16_t CWTestMask, uint16_t CCWTestMask,
        GrUserStencilOp CWPassOp, GrUserStencilOp CCWPassOp, GrUserStencilOp CWFailOp, GrUserStencilOp CCWFailOp, uint16_t CWWriteMask, uint16_t CCWWriteMask,
        typename CWAttrs = Attrs<CWTest, CWPassOp, CWFailOp>, typename CCWAttrs = Attrs<CCWTest, CCWPassOp, CCWFailOp>>
    constexpr explicit GrUserStencilSettings(
        const InitSeparate<CWRef, CCWRef, CWTest, CCWTest, CWTestMask, CCWTestMask, CWPassOp, CCWPassOp, CWFailOp, CCWFailOp, CWWriteMask, CCWWriteMask>&)
        : fCWFlags { CWAttrs::Flags(false), CWAttrs::Flags(true) }
        , fCWFace { CWRef, CWTest, CWAttrs::EffectiveTestMask(CWTestMask), CWPassOp, CWFailOp, CWAttrs::EffectiveWriteMask(CWWriteMask) }
        , fCCWFlags { CCWAttrs::Flags(false), CCWAttrs::Flags(true) }
        , fCCWFace { CCWRef, CCWTest, CCWAttrs::EffectiveTestMask(CCWTestMask), CCWPassOp, CCWFailOp, CCWAttrs::EffectiveWriteMask(CCWWriteMask) }
    {
    }
...
template <GrUserStencilTest Test, GrUserStencilOp PassOp, GrUserStencilOp FailOp> struct GrUserStencilSettings::Attrs {
    // Ensure an op that only modifies user bits isn't paired with one that modifies clip bits.
    static_assert(
        GrUserStencilOp::kKeep == PassOp || GrUserStencilOp::kKeep == FailOp || (PassOp <= kLastUserOnlyStencilOp) == (FailOp <= kLastUserOnlyStencilOp));
    // Ensure an op that only modifies clip bits isn't paired with one that modifies clip and user.
    static_assert(
        GrUserStencilOp::kKeep == PassOp || GrUserStencilOp::kKeep == FailOp || (PassOp <= kLastClipOnlyStencilOp) == (FailOp <= kLastClipOnlyStencilOp));
    ...
    {
        return GrUserStencilOp::kIncWrap == PassOp || GrUserStencilOp::kDecWrap == PassOp || GrUserStencilOp::kIncWrap == FailOp
            || GrUserStencilOp::kDecWrap == FailOp;
    }
    ...
    {
        return (IsDisabled(hasStencilClip) ? kDisabled_StencilFlag : 0) | (TestAlwaysPasses(hasStencilClip) ? kTestAlwaysPasses_StencilFlag : 0)
            | (DoesNotModifyStencil(hasStencilClip) ? kNoModifyStencil_StencilFlag : 0) | (UsesWrapOps() ? 0 : kNoWrapOps_StencilFlag);
    }
