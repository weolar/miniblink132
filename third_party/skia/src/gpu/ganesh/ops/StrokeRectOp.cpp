...
{
    SkASSERT(stroke.getStyle() == SkStrokeRec::kStroke_Style || stroke.getStyle() == SkStrokeRec::kHairline_Style);
...
public:
    ...
    static GrOp::Owner Make(
        GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& rect, const SkStrokeRec& stroke, GrAAType aaType)
    {
        bool isMiter;
        ...
        return Helper::FactoryHelper<NonAAStrokeRectOp>(context, std::move(paint), inputFlags, viewMatrix, rect, stroke, aaType);
    }

    NonAAStrokeRectOp(GrProcessorSet* processorSet, const SkPMColor4f& color, Helper::InputFlags inputFlags, const SkMatrix& viewMatrix, const SkRect& rect,
        ...
        , fHelper(processorSet, aaType, inputFlags)
    {
        ...
        if (inputFlags & Helper::InputFlags::kSnapVerticesToPixelCenters) {
            ...
            bounds.setLTRB(SkScalarFloorToScalar(bounds.fLeft), SkScalarFloorToScalar(bounds.fTop), SkScalarFloorToScalar(bounds.fRight),
            ...
            this->setTransformedBounds(bounds, fViewMatrix, aaBloat, fStrokeWidth ? IsHairline::kNo : IsHairline::kYes);
        }
    }
    ...
    GrProcessorSet::Analysis finalize(const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) override
    {
        ...
        return fHelper.finalizeProcessors(caps, clip, clampType, GrProcessorAnalysisCoverage::kNone, &fColor, nullptr);
    }
...
private:
    ...
    void onCreateProgramInfo(const GrCaps* caps, SkArenaAlloc* arena, const GrSurfaceProxyView& writeView, bool usesMSAASurface, GrAppliedClip&& clip,
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        GrGeometryProcessor* gp;
        {
            ...
            Color color(fColor);
            LocalCoords::Type localCoordsType = fHelper.usesLocalCoords() ? LocalCoords::kUsePosition_Type : LocalCoords::kUnused_Type;
            gp = GrDefaultGeoProcFactory::Make(arena, color, Coverage::kSolid_Type, localCoordsType, fViewMatrix);
        }

        GrPrimitiveType primType = (fStrokeWidth > 0) ? GrPrimitiveType::kTriangleStrip : GrPrimitiveType::kLineStrip;

        fProgramInfo = fHelper.createProgramInfo(
            caps, arena, writeView, usesMSAASurface, std::move(clip), dstProxyView, gp, primType, renderPassXferBarriers, colorLoadOp);
    }
    ...
    SkString onDumpInfo() const override
    {
        return SkStringPrintf("Color: 0x%08x, Rect [L: %.2f, T: %.2f, R: %.2f, B: %.2f], "
                              "StrokeWidth: %.2f\n%s",
            fColor.toBytes_RGBA(), fRect.fLeft, fRect.fTop, fRect.fRight, fRect.fBottom, fStrokeWidth, fHelper.dumpInfo().c_str());
    }
#endif
...
bool stroke_dev_half_size_supported(SkVector devHalfStrokeSize)
{
    ...
    return SkScalarNearlyEqual(devHalfStrokeSize.fX, devHalfStrokeSize.fY) || std::min(devHalfStrokeSize.fX, devHalfStrokeSize.fY) >= .5f;
}

bool compute_aa_rects(const GrCaps& caps, SkRect* devOutside, SkRect* devOutsideAssist, SkRect* devInside, bool* isDegenerate, const SkMatrix& viewMatrix,
    const SkRect& rect, SkScalar strokeWidth, bool miterStroke, SkVector* devHalfStrokeSize)
{
...
GrGeometryProcessor* create_aa_stroke_rect_gp(
    SkArenaAlloc* arena, bool usesMSAASurface, bool tweakAlphaForCoverage, const SkMatrix& viewMatrix, bool usesLocalCoords, bool wideColor)
{
    using namespace GrDefaultGeoProcFactory;
    ...
    Coverage::Type coverageType
        = usesMSAASurface ? Coverage::kAttributeUnclamped_Type : (!tweakAlphaForCoverage ? Coverage::kAttribute_Type : Coverage::kSolid_Type);
    LocalCoords::Type localCoordsType = usesLocalCoords ? LocalCoords::kUsePosition_Type : LocalCoords::kUnused_Type;
    Color::Type colorType = wideColor ? Color::kPremulWideColorAttribute_Type : Color::kPremulGrColorAttribute_Type;
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& devOutside, const SkRect& devInside,
        const SkVector& devHalfStrokeSize)
    {
        ...
        return Helper::FactoryHelper<AAStrokeRectOp>(context, std::move(paint), viewMatrix, devOutside, devInside, devHalfStrokeSize);
    }

    AAStrokeRectOp(GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMatrix& viewMatrix, const SkRect& devOutside, const SkRect& devInside,
        const SkVector& devHalfStrokeSize)
        : INHERITED(ClassID())
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& rect, const SkStrokeRec& stroke)
    {
        if (!viewMatrix.rectStaysRect()) {
            // The AA op only supports axis-aligned rectangles
            return nullptr;
        ...
        RectInfo info;
        if (!compute_aa_rects(*context->priv().caps(), &info.fDevOutside, &info.fDevOutsideAssist, &info.fDevInside, &info.fDegenerate, viewMatrix, rect,
        ...
        return Helper::FactoryHelper<AAStrokeRectOp>(context, std::move(paint), viewMatrix, info, isMiter);
    }

    AAStrokeRectOp(GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMatrix& viewMatrix, const RectInfo& infoExceptColor, bool isMiter)
        : INHERITED(ClassID())
        , fHelper(processorSet, GrAAType::kCoverage)
        , fViewMatrix(viewMatrix)
    {
    ...
    GrProcessorSet::Analysis finalize(const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) override
    {
        return fHelper.finalizeProcessors(caps, clip, clampType, GrProcessorAnalysisCoverage::kSingleChannel, &fRects.back().fColor, &fWideColor);
    }
...
private:
    ...
    void onCreateProgramInfo(const GrCaps*, SkArenaAlloc*, const GrSurfaceProxyView& writeView, bool usesMSAASurface, GrAppliedClip&&, const GrDstProxyView&,
    ...
    SkString onDumpInfo() const override
    {
        ...
        for (const auto& info : fRects) {
            string.appendf("Color: 0x%08x, ORect [L: %.2f, T: %.2f, R: %.2f, B: %.2f], "
                ...
                info.fColor.toBytes_RGBA(), info.fDevOutside.fLeft, info.fDevOutside.fTop, info.fDevOutside.fRight, info.fDevOutside.fBottom,
                info.fDevOutsideAssist.fLeft, info.fDevOutsideAssist.fTop, info.fDevOutsideAssist.fRight, info.fDevOutsideAssist.fBottom, info.fDevInside.fLeft,
                info.fDevInside.fTop, info.fDevInside.fRight, info.fDevInside.fBottom, info.fDegenerate);
        }
    ...
    void generateAAStrokeRectGeometry(VertexWriter& vertices, const SkPMColor4f& color, bool wideColor, const SkRect& devOutside,
        const SkRect& devOutsideAssist, const SkRect& devInside, bool miterStroke, bool degenerate, const SkVector& devHalfStrokeSize,
...
void AAStrokeRectOp::onCreateProgramInfo(const GrCaps* caps, SkArenaAlloc* arena, const GrSurfaceProxyView& writeView, bool usesMSAASurface,
    GrAppliedClip&& appliedClip, const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp)
{

    GrGeometryProcessor* gp = create_aa_stroke_rect_gp(
        arena, usesMSAASurface, this->compatibleWithCoverageAsAlpha(usesMSAASurface), this->viewMatrix(), fHelper.usesLocalCoords(), fWideColor);
    if (!gp) {
    ...
    fProgramInfo = fHelper.createProgramInfo(
        caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, gp, GrPrimitiveType::kTriangles, renderPassXferBarriers, colorLoadOp);
}
...
void AAStrokeRectOp::onPrepareDraws(GrMeshDrawTarget* target)
{
    ...
    sk_sp<const GrGpuBuffer> indexBuffer = GetIndexBuffer(target->resourceProvider(), this->miterStroke());
    if (!indexBuffer) {
        SkDebugf("Could not allocate indices\n");
        return;
    }
    PatternHelper helper(target, GrPrimitiveType::kTriangles, fProgramInfo->geomProc().vertexStride(), std::move(indexBuffer), verticesPerInstance,
        indicesPerInstance, instanceCount, maxQuads);
    ...
    for (int i = 0; i < instanceCount; i++) {
        ...
        this->generateAAStrokeRectGeometry(vertices, info.fColor, fWideColor, info.fDevOutside, info.fDevOutsideAssist, info.fDevInside, fMiterStroke,
            info.fDegenerate, info.fDevHalfStrokeSize, target->usesMSAASurface());
    }
    fMesh = helper.mesh();
}
...
sk_sp<const GrGpuBuffer> AAStrokeRectOp::GetIndexBuffer(GrResourceProvider* resourceProvider, bool miterStroke)
{
    if (miterStroke) {
        // clang-format on
        ...
        return resourceProvider->findOrCreatePatternedIndexBuffer(
            gMiterIndices, kMiterIndexCnt, kNumMiterRectsInIndexBuffer, kMiterVertexCnt, gMiterIndexBufferKey);
    } else {
        /**
        ...
        return resourceProvider->findOrCreatePatternedIndexBuffer(
            gBevelIndices, kBevelIndexCnt, kNumBevelRectsInIndexBuffer, kBevelVertexCnt, gBevelIndexBufferKey);
    }
}
...
GrOp::CombineResult AAStrokeRectOp::onCombineIfPossible(GrOp* t, SkArenaAlloc*, const GrCaps& caps)
{
    ...
    if (fHelper.usesLocalCoords() && !SkMatrixPriv::CheapEqual(this->viewMatrix(), that->viewMatrix())) {
        return CombineResult::kCannotCombine;
    }
...
void AAStrokeRectOp::generateAAStrokeRectGeometry(VertexWriter& vertices, const SkPMColor4f& color, bool wideColor, const SkRect& devOutside,
    const SkRect& devOutsideAssist, const SkRect& devInside, bool miterStroke, bool degenerate, const SkVector& devHalfStrokeSize, bool usesMSAASurface) const
{
    ...
    auto inset_fan = [](const SkRect& r, SkScalar dx, SkScalar dy) { return VertexWriter::TriFanFromRect(r.makeInset(dx, dy)); };

    bool tweakAlphaForCoverage = this->compatibleWithCoverageAsAlpha(usesMSAASurface);

    auto maybe_coverage = [tweakAlphaForCoverage](float coverage) { return VertexWriter::If(!tweakAlphaForCoverage, coverage); };
    ...
    if (usesMSAASurface) {
        ...
        float insetExtraBloat = std::min(inset + msaaExtraBloat, std::min(devHalfStrokeSize.fX, devHalfStrokeSize.fY)) - inset;
        inset += insetExtraBloat;
        innerCoverage += insetExtraBloat;

        float interiorExtraBloat = std::min(interiorOutset + msaaExtraBloat, std::min(devInside.width(), devInside.height()) / 2) - interiorOutset;
    ...
    vertices.writeQuad(inset_fan(devOutside, -outset, -outset), outerColor, maybe_coverage(outerCoverage));

    if (!miterStroke) {
        // Second exterior outset.
        vertices.writeQuad(inset_fan(devOutsideAssist, -outset, -outset), outerColor, maybe_coverage(outerCoverage));
    }

    // Exterior inset rect (toward stroke).
    vertices.writeQuad(inset_fan(devOutside, inset, inset), innerColor, maybe_coverage(innerCoverage));

    if (!miterStroke) {
        // Second exterior inset.
        vertices.writeQuad(inset_fan(devOutsideAssist, inset, inset), innerColor, maybe_coverage(innerCoverage));
    }

    if (!degenerate) {
        // Interior inset rect (toward stroke).
        vertices.writeQuad(inset_fan(devInside, -inset, -inset), innerColor, maybe_coverage(innerCoverage));
        ...
        if (interiorAABoundary.fTop > interiorAABoundary.fBottom) {
            coverageBackset = std::max((interiorAABoundary.fTop - interiorAABoundary.fBottom) / (interiorOutset * 2), coverageBackset);
        ...
        if (coverageBackset > 0) {
            ...
            // case.
            interiorCoverage += interiorCoverage * (1 - coverageBackset) + innerCoverage * coverageBackset;
        }
        VertexColor interiorColor(tweakAlphaForCoverage ? color * interiorCoverage : color, wideColor);
        vertices.writeQuad(VertexWriter::TriFanFromRect(interiorAABoundary), interiorColor, maybe_coverage(interiorCoverage));
    } else {
        ...
        vertices.writeQuad(VertexWriter::TriFanFromRect(devInside), innerColor, maybe_coverage(innerCoverage));
...
GrOp::Owner MakeNested(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect rects[2])
{
    ...
    if (devInside.isEmpty() || !devInside.intersect(visibilityBounds)) {
        ...
        DrawQuad quad { GrQuad::MakeFromRect(rects[0], viewMatrix), GrQuad(rects[0]), GrQuadAAFlags::kAll };
        return ganesh::FillRectOp::Make(context, std::move(paint), GrAAType::kCoverage, &quad);
    }

    return AAStrokeRectOp::Make(context, std::move(paint), viewMatrix, devOutside, devInside, SkVector { dx, dy } * .5f);
}
...
GR_DRAW_OP_TEST_DEFINE(NonAAStrokeRectOp)
{
    ...
    return skgpu::ganesh::StrokeRectOp::NonAAStrokeRectOp::Make(context, std::move(paint), viewMatrix, rect, strokeRec, aaType);
}
...
GR_DRAW_OP_TEST_DEFINE(AAStrokeRectOp)
{
    ...
    rec.setStrokeParams(SkPaint::kButt_Cap, miterStroke ? SkPaint::kMiter_Join : SkPaint::kBevel_Join, 1.f);
    SkMatrix matrix = GrTest::TestMatrixRectStaysRect(random);
    return skgpu::ganesh::StrokeRectOp::AAStrokeRectOp::Make(context, std::move(paint), matrix, rect, rec);
}
