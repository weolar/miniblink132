...
void extract_verts(
    const GrAAConvexTessellator& tess, const SkMatrix* localCoordsMatrix, void* vertData, const VertexColor& color, uint16_t firstIndex, uint16_t* idxs)
{
...
GrGeometryProcessor* create_lines_only_gp(SkArenaAlloc* arena, bool tweakAlphaForCoverage, bool usesLocalCoords, bool wideColor)
{
    using namespace GrDefaultGeoProcFactory;

    Coverage::Type coverageType = tweakAlphaForCoverage ? Coverage::kAttributeTweakAlpha_Type : Coverage::kAttribute_Type;
    LocalCoords::Type localCoordsType = usesLocalCoords ? LocalCoords::kHasExplicit_Type : LocalCoords::kUnused_Type;
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkPath& path, SkScalar strokeWidth,
        SkStrokeRec::Style style, SkPaint::Join join, SkScalar miterLimit, const GrUserStencilSettings* stencilSettings)
    {
        return Helper::FactoryHelper<AAFlatteningConvexPathOp>(
            context, std::move(paint), viewMatrix, path, strokeWidth, style, join, miterLimit, stencilSettings);
    }

    AAFlatteningConvexPathOp(GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMatrix& viewMatrix, const SkPath& path, SkScalar strokeWidth,
        SkStrokeRec::Style style, SkPaint::Join join, SkScalar miterLimit, const GrUserStencilSettings* stencilSettings)
        : INHERITED(ClassID())
...
private:
    ...
    {
        GrGeometryProcessor* gp = create_lines_only_gp(arena, fHelper.compatibleWithCoverageAsAlpha(), fHelper.usesLocalCoords(), fWideColor);
        if (!gp) {
            SkDebugf("Couldn't create a GrGeometryProcessor\n");
            return;
        }

        fProgramInfo = fHelper.createProgramInfoWithStencil(caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, gp,
            GrPrimitiveType::kTriangles, renderPassXferBarriers, colorLoadOp);
    }

    void recordDraw(GrMeshDrawTarget* target, int vertexCount, size_t vertexStride, void* vertices, int indexCount, uint16_t* indices)
    {
        ...
        mesh->setIndexed(std::move(indexBuffer), indexCount, firstIndex, 0, vertexCount - 1, GrPrimitiveRestart::kNo, std::move(vertexBuffer), firstVertex);
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        for (int i = 0; i < instanceCount; i++) {
            ...
            extract_verts(
                tess, localCoordsMatrix, vertices + vertexStride * vertexCount, VertexColor(args.fColor, fWideColor), vertexCount, indices + indexCount);
...
PathRenderer::CanDrawPath AALinearizingConvexPathRenderer::onCanDrawPath(const CanDrawPathArgs& args) const
{
    ...
    if (stroke.getStyle() == SkStrokeRec::kStroke_Style || stroke.getStyle() == SkStrokeRec::kStrokeAndFill_Style) {
        ...
        if ((strokeWidth > kMaxStrokeWidth && !args.fShape->isRect()) || !args.fShape->knownToBeClosed() || stroke.getJoin() == SkPaint::Join::kRound_Join) {
            return CanDrawPath::kNo;
        }
...
{
    GR_AUDIT_TRAIL_AUTO_FRAME(args.fContext->priv().auditTrail(), "AALinearizingConvexPathRenderer::onDrawPath");
...
GR_DRAW_OP_TEST_DEFINE(AAFlatteningConvexPathOp)
{
    ...
    SkStrokeRec::Style styles[3] = { SkStrokeRec::kFill_Style, SkStrokeRec::kStroke_Style, SkStrokeRec::kStrokeAndFill_Style };
    ...
    return skgpu::ganesh::AAFlatteningConvexPathOp::Make(context, std::move(paint), viewMatrix, path, strokeWidth, style, join, miterLimit, stencilSettings);
}
