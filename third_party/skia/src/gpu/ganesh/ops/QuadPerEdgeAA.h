...
public:
    ...
    VertexSpec(GrQuad::Type deviceQuadType, ColorType colorType, GrQuad::Type localQuadType, bool hasLocalCoords, Subset subset, GrAAType aa,
        bool coverageAsAlpha, IndexBufferOption indexBufferOption)
        ...
        , fRequiresGeometrySubset(aa == GrAAType::kCoverage && deviceQuadType > GrQuad::Type::kRectilinear)
    {
    }
    ...
    {
        return fRequiresGeometrySubset;
    }
...
public:
    ...
    SkDEBUGCODE(skgpu::BufferWriter::Mark vertexMark() const { return fVertexWriter.mark(); })
        ...
        typedef void (*WriteQuadProc)(VertexWriter* vertices, const VertexSpec& spec, const GrQuad* deviceQuad, const GrQuad* localQuad,
            const float coverage[4], const SkPMColor4f& color, const SkRect& geomSubset, const SkRect& texSubset);
...
GrGeometryProcessor* MakeTexturedProcessor(SkArenaAlloc*, const VertexSpec&, const GrShaderCaps&, const GrBackendFormat&, GrSamplerState, const skgpu::Swizzle&,
...
void IssueDraw(const GrCaps&, GrOpsRenderPass*, const VertexSpec&, int runningQuadCount, int quadCount, int maxVerts, int absVertBufferOffset);
