...
public:
    ...
    GrProgramInfo* createProgramInfoWithStencil(const GrCaps*, SkArenaAlloc*, const GrSurfaceProxyView& writeView, bool usesMSAASurface, GrAppliedClip&&,
    ...
    template <typename Op, typename... OpArgs> static GrOp::Owner FactoryHelper(GrRecordingContext* context, GrPaint&& paint, OpArgs... opArgs)
    {
    ...
    {
        return this->INHERITED::finalizeProcessors(caps, clip, fStencilSettings, clampType, geometryCoverage, geometryColor);
    }
    ...
    bool isCompatible(
        const GrSimpleMeshDrawOpHelperWithStencil& that, const GrCaps&, const SkRect& thisBounds, const SkRect& thatBounds, bool ignoreAAType = false) const;
