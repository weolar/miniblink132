...
sk_sp<const GrBuffer> get_quads_index_buffer(GrResourceProvider* resourceProvider)
{
    ...
    return resourceProvider->findOrCreatePatternedIndexBuffer(kQuadIdxBufPattern, kIdxsPerQuad, kQuadsNumInIdxBuffer, kQuadNumVertices, gQuadsIndexBufferKey);
}
...
static const uint16_t kLineSegIdxBufPattern[] = { 0, 1, 3, 0, 3, 2, 0, 4, 5, 0, 5, 1, 0, 2, 4, 1, 5, 3 };
...
sk_sp<const GrBuffer> get_lines_index_buffer(GrResourceProvider* resourceProvider)
{
    ...
    return resourceProvider->findOrCreatePatternedIndexBuffer(
        kLineSegIdxBufPattern, kIdxsPerLineSeg, kLineSegsNumInIdxBuffer, kLineSegNumVertices, gLinesIndexBufferKey);
}
...
int is_degen_quad_or_conic(const SkPoint p[3], SkScalar* dsqd)
{
    ...
    if (SkPointPriv::DistanceToSqd(p[0], p[1]) < gDegenerateToLineTolSqd || SkPointPriv::DistanceToSqd(p[1], p[2]) < gDegenerateToLineTolSqd) {
        return 1;
    }
 ...
 */
int gather_lines_and_quads(const SkPath& path, const SkMatrix& m, const SkIRect& devClipBounds, SkScalar capLength, bool convertConicsToQuads, PtArray* lines,
...
void intersect_lines(const SkPoint& ptA, const SkVector& normA, const SkPoint& ptB, const SkVector& normB, SkPoint* result)
{
...
bool bloat_quad(const SkPoint qpts[3], const SkMatrix* toDevice, const SkMatrix* toSrc, BezierVertex verts[kQuadNumVertices])
{
    ...
    if (toSrc) {
        SkMatrixPriv::MapPointsWithStride(*toSrc, &verts[0].fPos, sizeof(BezierVertex), kQuadNumVertices);
    }
...
void set_conic_coeffs(const SkPoint p[3], BezierVertex verts[kQuadNumVertices], const SkScalar weight)
{
    SkMatrix klm;
...
void add_conics(const SkPoint p[3], const SkScalar weight, const SkMatrix* toDevice, const SkMatrix* toSrc, BezierVertex** vert)
{
...
void add_quads(const SkPoint p[3], int subdiv, const SkMatrix* toDevice, const SkMatrix* toSrc, BezierVertex** vert)
{
    SkASSERT(subdiv >= 0);
...
void add_line(const SkPoint p[2], const SkMatrix* toSrc, uint8_t coverage, LineVertex** vert)
{
    ...
    if (vec.setLength(SK_ScalarHalf)) {
        ...
        if (toSrc) {
            SkMatrixPriv::MapPointsWithStride(*toSrc, &(*vert)->fPos, sizeof(LineVertex), kLineSegNumVertices);
        }
    } else {
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkPath& path, const GrStyle& style,
        const SkIRect& devClipBounds, const GrUserStencilSettings* stencilSettings)
    {
        ...
        return Helper::FactoryHelper<AAHairlineOp>(context, std::move(paint), newCoverage, viewMatrix, path, devClipBounds, capLength, stencilSettings);
    }

    AAHairlineOp(GrProcessorSet* processorSet, const SkPMColor4f& color, uint8_t coverage, const SkMatrix& viewMatrix, const SkPath& path,
        SkIRect devClipBounds, SkScalar capLength, const GrUserStencilSettings* stencilSettings)
        : INHERITED(ClassID())
    ...
    GrProcessorSet::Analysis finalize(const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) override
    {
        ...
        return fHelper.finalizeProcessors(caps, clip, clampType, GrProcessorAnalysisCoverage::kSingleChannel, &fColor, nullptr);
    }
...
private:
    void makeLineProgramInfo(const GrCaps&, SkArenaAlloc*, const GrPipeline*, const GrSurfaceProxyView& writeView, bool usesMSAASurface,
        const SkMatrix* geometryProcessorViewM, const SkMatrix* geometryProcessorLocalM, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp);
    void makeQuadProgramInfo(const GrCaps&, SkArenaAlloc*, const GrPipeline*, const GrSurfaceProxyView& writeView, bool usesMSAASurface,
        const SkMatrix* geometryProcessorViewM, const SkMatrix* geometryProcessorLocalM, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp);
    void makeConicProgramInfo(const GrCaps&, SkArenaAlloc*, const GrPipeline*, const GrSurfaceProxyView& writeView, bool usesMSAASurface,
        const SkMatrix* geometryProcessorViewM, const SkMatrix* geometryProcessorLocalM, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp);
    ...
    void onPrePrepareDraws(GrRecordingContext*, const GrSurfaceProxyView& writeView, GrAppliedClip*, const GrDstProxyView&,
        GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override;
    ...
    CombineResult onCombineIfPossible(GrOp* t, SkArenaAlloc*, const GrCaps& caps) override
    {
        ...
        if (this->viewMatrix().hasPerspective() && !SkMatrixPriv::CheapEqual(this->viewMatrix(), that->viewMatrix())) {
            return CombineResult::kCannotCombine;
        }
    ...
    {
        return SkStringPrintf("Color: 0x%08x Coverage: 0x%02x, Count: %d\n%s", fColor.toBytes_RGBA(), fCoverage, fPaths.size(), fHelper.dumpInfo().c_str());
    }
#endif
...
void AAHairlineOp::makeLineProgramInfo(const GrCaps& caps, SkArenaAlloc* arena, const GrPipeline* pipeline, const GrSurfaceProxyView& writeView,
    bool usesMSAASurface, const SkMatrix* geometryProcessorViewM, const SkMatrix* geometryProcessorLocalM, GrXferBarrierFlags renderPassXferBarriers,
    GrLoadOp colorLoadOp)
{
    ...
    GrGeometryProcessor* lineGP;
    {
        ...
        LocalCoords localCoords(fHelper.usesLocalCoords() ? LocalCoords::kUsePosition_Type : LocalCoords::kUnused_Type);
        localCoords.fMatrix = geometryProcessorLocalM;

        lineGP = GrDefaultGeoProcFactory::Make(arena, color, Coverage::kAttribute_Type, localCoords, *geometryProcessorViewM);
        SkASSERT(sizeof(LineVertex) == lineGP->vertexStride());
    }

    fProgramInfos[0] = GrSimpleMeshDrawOpHelper::CreateProgramInfo(&caps, arena, pipeline, writeView, usesMSAASurface, lineGP, GrPrimitiveType::kTriangles,
        renderPassXferBarriers, colorLoadOp, fHelper.stencilSettings());
}

void AAHairlineOp::makeQuadProgramInfo(const GrCaps& caps, SkArenaAlloc* arena, const GrPipeline* pipeline, const GrSurfaceProxyView& writeView,
    bool usesMSAASurface, const SkMatrix* geometryProcessorViewM, const SkMatrix* geometryProcessorLocalM, GrXferBarrierFlags renderPassXferBarriers,
    GrLoadOp colorLoadOp)
{
    if (fProgramInfos[1]) {
    ...
    GrGeometryProcessor* quadGP
        = GrQuadEffect::Make(arena, this->color(), *geometryProcessorViewM, caps, *geometryProcessorLocalM, fHelper.usesLocalCoords(), this->coverage());
    SkASSERT(sizeof(BezierVertex) == quadGP->vertexStride());

    fProgramInfos[1] = GrSimpleMeshDrawOpHelper::CreateProgramInfo(&caps, arena, pipeline, writeView, usesMSAASurface, quadGP, GrPrimitiveType::kTriangles,
        renderPassXferBarriers, colorLoadOp, fHelper.stencilSettings());
}

void AAHairlineOp::makeConicProgramInfo(const GrCaps& caps, SkArenaAlloc* arena, const GrPipeline* pipeline, const GrSurfaceProxyView& writeView,
    bool usesMSAASurface, const SkMatrix* geometryProcessorViewM, const SkMatrix* geometryProcessorLocalM, GrXferBarrierFlags renderPassXferBarriers,
    GrLoadOp colorLoadOp)
{
    if (fProgramInfos[2]) {
    ...
    GrGeometryProcessor* conicGP
        = GrConicEffect::Make(arena, this->color(), *geometryProcessorViewM, caps, *geometryProcessorLocalM, fHelper.usesLocalCoords(), this->coverage());
    SkASSERT(sizeof(BezierVertex) == conicGP->vertexStride());

    fProgramInfos[2] = GrSimpleMeshDrawOpHelper::CreateProgramInfo(&caps, arena, pipeline, writeView, usesMSAASurface, conicGP, GrPrimitiveType::kTriangles,
        renderPassXferBarriers, colorLoadOp, fHelper.stencilSettings());
}
...
void AAHairlineOp::onCreateProgramInfo(const GrCaps* caps, SkArenaAlloc* arena, const GrSurfaceProxyView& writeView, bool usesMSAASurface,
    GrAppliedClip&& appliedClip, const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp)
{
    ...
    auto pipeline = fHelper.createPipeline(caps, arena, writeView.swizzle(), std::move(appliedClip), dstProxyView);

    if (fCharacterization & Program::kLine) {
        this->makeLineProgramInfo(
            *caps, arena, pipeline, writeView, usesMSAASurface, geometryProcessorViewM, geometryProcessorLocalM, renderPassXferBarriers, colorLoadOp);
    }
...
void AAHairlineOp::onPrePrepareDraws(GrRecordingContext* context, const GrSurfaceProxyView& writeView, GrAppliedClip* clip, const GrDstProxyView& dstProxyView,
    GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp)
{
    ...
    this->createProgramInfo(caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, renderPassXferBarriers, colorLoadOp);
...
void AAHairlineOp::onPrepareDraws(GrMeshDrawTarget* target)
{
    ...
    for (int i = 0; i < instanceCount && safeMath.ok(); i++) {
        ...
        quadCount = safeMath.addInt(quadCount,
            gather_lines_and_quads(
                args.fPath, args.fViewMatrix, args.fDevClipBounds, args.fCapLength, convertConicsToQuads, &lines, &quads, &conics, &qSubdivs, &cWeights));
    }
    ...
    if (lineCount) {
        ...
        GrMeshDrawOp::PatternHelper helper(target, GrPrimitiveType::kTriangles, sizeof(LineVertex), std::move(linesIndexBuffer), kLineSegNumVertices,
    ...
    if (quadCount || conicCount) {
        ...
        void* vertices = target->makeVertexSpace(sizeof(BezierVertex), vertexCount, &vertexBuffer, &firstVertex);
        ...
        if (quadCount > 0) {
            ...
            fMeshes[1]->setIndexedPatterned(quadsIndexBuffer, kIdxsPerQuad, quadCount, kQuadsNumInIdxBuffer, vertexBuffer, kQuadNumVertices, firstVertex);
        ...
        if (conicCount > 0) {
            ...
            fMeshes[2]->setIndexedPatterned(
                std::move(quadsIndexBuffer), kIdxsPerQuad, conicCount, kQuadsNumInIdxBuffer, std::move(vertexBuffer), kQuadNumVertices, firstVertex);
        }
    }
...
GR_DRAW_OP_TEST_DEFINE(AAHairlineOp)
{
    ...
    return AAHairlineOp::Make(context, std::move(paint), viewMatrix, path, GrStyle::SimpleHairline(), devClipBounds, GrGetRandomStencil(random, context));
}
...
PathRenderer::CanDrawPath AAHairLinePathRenderer::onCanDrawPath(const CanDrawPathArgs& args) const
{
    ...
    if (SkPath::kLine_SegmentMask == args.fShape->segmentMask() || args.fCaps->shaderCaps()->fShaderDerivativeSupport) {
...
{
    GR_AUDIT_TRAIL_AUTO_FRAME(args.fContext->priv().auditTrail(), "AAHairlinePathRenderer::onDrawPath");
    SkASSERT(args.fSurfaceDrawContext->numSamples() <= 1);

    SkPath path;
    args.fShape->asPath(&path);
    GrOp::Owner op = AAHairlineOp::Make(
        args.fContext, std::move(args.fPaint), *args.fViewMatrix, path, args.fShape->style(), *args.fClipConservativeBounds, args.fUserStencilSettings);
