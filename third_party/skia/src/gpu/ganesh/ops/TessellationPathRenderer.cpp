...
GrOp::Owner make_non_convex_fill_op(GrRecordingContext* rContext, SkArenaAlloc* arena, skgpu::ganesh::FillPathFlags fillPathFlags, GrAAType aaType,
    const SkRect& drawBounds, const SkIRect& clipBounds, const SkMatrix& viewMatrix, const SkPath& path, GrPaint&& paint)
{
    ...
    if (numVerbs > 0 && !path.isInverseFillType()) {
        ...
        if (clippedDrawBounds.intersect(drawBounds)) {
            ...
            if (cpuTessellationWork * kCpuWeight + kMinNumPixelsToTriangulate < gpuFragmentWork) {
                return GrOp::Make<skgpu::ganesh::PathInnerTriangulateOp>(rContext, viewMatrix, path, std::move(paint), aaType, fillPathFlags, drawBounds);
            }
        } // we should be clipped out when the GrClip is analyzed, so just return the default op
    }
#endif

    return GrOp::Make<skgpu::ganesh::PathStencilCoverOp>(rContext, arena, viewMatrix, path, std::move(paint), aaType, fillPathFlags, drawBounds);
}
...
bool ChopPathIfNecessary(
    const SkMatrix& viewMatrix, const GrStyledShape& shape, const SkIRect& clipConservativeBounds, const SkStrokeRec& stroke, SkPath* chopped_path)
{
    const SkRect pathDevBounds = viewMatrix.mapRect(shape.bounds());
    float n4 = wangs_formula::worst_case_cubic_p4(tess::kPrecision, pathDevBounds.width(), pathDevBounds.height());
    if (n4 > tess::kMaxSegmentsPerCurve_p4 && shape.segmentMask() != SkPath::kLine_SegmentMask) {
        ...
        if (!shape.style().isSimpleFill()) {
            ...
            if (stroke.isHairlineStyle()) {
                ...
                inflationRadius = SkStrokeRec::GetInflationRadius(stroke.getJoin(), stroke.getMiter(), stroke.getCap(), 1);
            } else {
                inflationRadius = stroke.getInflationRadius() * viewMatrix.getMaxScale();
            }
            viewport.outset(inflationRadius, inflationRadius);
        }
        if (wangs_formula::worst_case_cubic(tess::kPrecision, viewport.width(), viewport.height()) > kMaxSegmentsPerCurve) {
            return false;
        }
        if (chopped_path) {
            *chopped_path = PreChopPathCurves(tess::kPrecision, *chopped_path, viewMatrix, viewport);
        }
    }
    return true;
}
...
{
    return !caps.avoidStencilBuffers() && caps.drawInstancedSupport() && !caps.disableTessellationPathRenderer();
}

PathRenderer::StencilSupport TessellationPathRenderer::onGetStencilSupport(const GrStyledShape& shape) const
{
...
PathRenderer::CanDrawPath TessellationPathRenderer::onCanDrawPath(const CanDrawPathArgs& args) const
{
    const GrStyledShape& shape = *args.fShape;
    if (args.fAAType == GrAAType::kCoverage || shape.style().hasPathEffect() || args.fViewMatrix->hasPerspective()
        || shape.style().strokeRec().getStyle() == SkStrokeRec::kStrokeAndFill_Style || !args.fProxy->canUseStencil(*args.fCaps)) {
    ...
    if (!ChopPathIfNecessary(*args.fViewMatrix, shape, *args.fClipConservativeBounds, shape.style().strokeRec(), nullptr)) {
        return CanDrawPath::kNo;
    }
...
bool TessellationPathRenderer::onDrawPath(const DrawPathArgs& args)
{
    ...
    SkAssertResult(ChopPathIfNecessary(*args.fViewMatrix, *args.fShape, *args.fClipConservativeBounds, args.fShape->style().strokeRec(), &path));
    ...
    if (!args.fShape->style().isSimpleFill()) {
        ...
        auto op = GrOp::Make<StrokeTessellateOp>(args.fContext, args.fAAType, *args.fViewMatrix, path, stroke, std::move(args.fPaint));
    ...
    if (path.isConvex() && !path.isInverseFillType()) {
        auto op = GrOp::Make<PathTessellateOp>(args.fContext, args.fSurfaceDrawContext->arenaAlloc(), args.fAAType, args.fUserStencilSettings,
            *args.fViewMatrix, path, std::move(args.fPaint), pathDevBounds);
        sdc->addDrawOp(args.fClip, std::move(op));
        return true;
    }

    SkASSERT(args.fUserStencilSettings->isUnused()); // See onGetStencilSupport().
    const SkRect& drawBounds = path.isInverseFillType() ? args.fSurfaceDrawContext->asSurfaceProxy()->backingStoreBoundsRect() : pathDevBounds;
    auto op = make_non_convex_fill_op(args.fContext, args.fSurfaceDrawContext->arenaAlloc(), FillPathFlags::kNone, args.fAAType, drawBounds,
        *args.fClipConservativeBounds, *args.fViewMatrix, path, std::move(args.fPaint));
    sdc->addDrawOp(args.fClip, std::move(op));
    return true;
}
...
void TessellationPathRenderer::onStencilPath(const StencilPathArgs& args)
{
    ...
    if (path.isConvex()) {
        constexpr static GrUserStencilSettings kMarkStencil(
            GrUserStencilSettings::StaticInit<0x0001, GrUserStencilTest::kAlways, 0xffff, GrUserStencilOp::kReplace, GrUserStencilOp::kKeep, 0xffff>());

        GrPaint stencilPaint;
        stencilPaint.setXPFactory(GrDisableColorXPFactory::Get());
        auto op = GrOp::Make<PathTessellateOp>(
            args.fContext, args.fSurfaceDrawContext->arenaAlloc(), aaType, &kMarkStencil, *args.fViewMatrix, path, std::move(stencilPaint), pathDevBounds);
        sdc->addDrawOp(args.fClip, std::move(op));
        return;
    }

    auto op = make_non_convex_fill_op(args.fContext, args.fSurfaceDrawContext->arenaAlloc(), FillPathFlags::kStencilOnly, aaType, pathDevBounds,
        *args.fClipConservativeBounds, *args.fViewMatrix, path, GrPaint());
    sdc->addDrawOp(args.fClip, std::move(op));
}
