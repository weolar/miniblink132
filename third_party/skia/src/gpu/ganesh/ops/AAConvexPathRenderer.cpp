...
bool compute_vectors(SegmentArray* segments, SkPoint* fanPt, SkPathFirstDirection dir, int* vCount, int* iCount)
{
...
struct DegenerateTestData {
    ...
    {
        return Stage::kNonDegenerate != fStage;
    }
    enum class Stage {
        kInitial,
        kPoint,
...
inline bool get_direction(const SkPath& path, const SkMatrix& m, SkPathFirstDirection* dir)
{
    ...
    *dir = SkPathPriv::ComputeFirstDirection(path);
    ...
    SkScalar det2x2 = m.get(SkMatrix::kMScaleX) * m.get(SkMatrix::kMScaleY) - m.get(SkMatrix::kMSkewX) * m.get(SkMatrix::kMSkewY);
    if (det2x2 < 0) {
...
inline void add_cubic_segments(const SkPoint pts[4], SkPathFirstDirection dir, SegmentArray* segments)
{
...
bool get_segments(const SkPath& path, const SkMatrix& m, SegmentArray* segments, SkPoint* fanPt, int* vCount, int* iCount)
{
...
void create_vertices(
    const SegmentArray& segments, const SkPoint& fanPt, const VertexColor& color, DrawArray* draws, VertexWriter& verts, uint16_t* idxs, size_t vertexStride)
{
        ...
        *i += 6;
        ...
        if (Segment::kLine == segb.fType) {
            ...
            *v += 5;
            ...
            verts << posAndUVPoints[0].fPos << color << posAndUVPoints[0].fUV << (-segb.fNorms[0].dot(fanPt) + c0) << (-segb.fNorms[1].dot(fanPt) + c1);

            verts << posAndUVPoints[1].fPos << color << posAndUVPoints[1].fUV << 0.0f << (-segb.fNorms[1].dot(qpts[0]) + c1);

            verts << posAndUVPoints[2].fPos << color << posAndUVPoints[2].fUV << (-segb.fNorms[0].dot(qpts[2]) + c0) << 0.0f;
            ...
            verts << posAndUVPoints[3].fPos << color << posAndUVPoints[3].fUV << kStableLargeNegativeValue << kStableLargeNegativeValue;

            verts << posAndUVPoints[4].fPos << color << posAndUVPoints[4].fUV << kStableLargeNegativeValue << kStableLargeNegativeValue;

            verts << posAndUVPoints[5].fPos << color << posAndUVPoints[5].fUV << kStableLargeNegativeValue << kStableLargeNegativeValue;
...
public:
    static GrGeometryProcessor* Make(SkArenaAlloc* arena, const SkMatrix& localMatrix, bool usesLocalCoords, bool wideColor)
    {
        return arena->make([&](void* ptr) { return new (ptr) QuadEdgeEffect(localMatrix, usesLocalCoords, wideColor); });
    }
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> QuadEdgeEffect::makeProgramImpl(const GrShaderCaps&) const
{
    ...
    private:
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            ...
            if (qe.fUsesLocalCoords) {
                WriteLocalCoord(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, qe.fInPosition.asShaderVar(), qe.fLocalMatrix, &fLocalMatrixUniform);
            }
            ...
            fragBuilder->codeAppendf("edgeAlpha = half(min(min(%s.z, %s.w) + 0.5, 1.0));", v.fsIn(), v.fsIn());
...
GrGeometryProcessor* QuadEdgeEffect::TestCreate(GrProcessorTestData* d)
{
    ...
    return d->caps()->shaderCaps()->fShaderDerivativeSupport ? QuadEdgeEffect::Make(d->allocator(), localMatrix, usesLocalCoords, wideColor) : nullptr;
}
#endif
...
public:
    ...
    static GrOp::Owner Make(
        GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkPath& path, const GrUserStencilSettings* stencilSettings)
    {
        return Helper::FactoryHelper<AAConvexPathOp>(context, std::move(paint), viewMatrix, path, stencilSettings);
    }

    AAConvexPathOp(
        GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMatrix& viewMatrix, const SkPath& path, const GrUserStencilSettings* stencilSettings)
        : INHERITED(ClassID())
    ...
    {
        return fHelper.finalizeProcessors(caps, clip, clampType, GrProcessorAnalysisCoverage::kSingleChannel, &fPaths.back().fColor, &fWideColor);
    }
...
private:
        ...
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        ...
        GrGeometryProcessor* quadProcessor = QuadEdgeEffect::Make(arena, invert, fHelper.usesLocalCoords(), fWideColor);

        fProgramInfo = fHelper.createProgramInfoWithStencil(caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, quadProcessor,
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        for (int i = 0; i < instanceCount; i++) {
            ...
            VertexWriter verts = target->makeVertexWriter(kVertexStride, vertexCount, &vertexBuffer, &firstVertex);
            ...
            for (int j = 0; j < draws.size(); ++j) {
                ...
                meshes[j].setIndexed(indexBuffer, draw.fIndexCnt, firstIndex, 0, draw.fVertexCnt - 1, GrPrimitiveRestart::kNo, vertexBuffer, firstVertex);
    ...
    CombineResult onCombineIfPossible(GrOp* t, SkArenaAlloc*, const GrCaps& caps) override
    {
        ...
        if (fHelper.usesLocalCoords() && !SkMatrixPriv::CheapEqual(fPaths[0].fViewMatrix, that->fPaths[0].fViewMatrix)) {
            return CombineResult::kCannotCombine;
        }
...
PathRenderer::CanDrawPath AAConvexPathRenderer::onCanDrawPath(const CanDrawPathArgs& args) const
{
    ...
    if (args.fCaps->shaderCaps()->fShaderDerivativeSupport && (GrAAType::kCoverage == args.fAAType) && args.fShape->style().isSimpleFill()
        && !args.fShape->inverseFilled() && args.fShape->knownToBeConvex() && args.fShape->knownDirection()) {
...
bool AAConvexPathRenderer::onDrawPath(const DrawPathArgs& args)
{
    ...
    GrOp::Owner op = AAConvexPathOp::Make(args.fContext, std::move(args.fPaint), *args.fViewMatrix, path, args.fUserStencilSettings);
...
GR_DRAW_OP_TEST_DEFINE(AAConvexPathOp)
{
    ...
    return skgpu::ganesh::AAConvexPathOp::Make(context, std::move(paint), viewMatrix, path, stencilSettings);
}
