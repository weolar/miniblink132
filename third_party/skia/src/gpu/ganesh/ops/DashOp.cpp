...
void calc_dash_scaling(SkScalar* parallelScale, SkScalar* perpScale, const SkMatrix& viewMatrix, const SkPoint pts[2])
{
...
SkScalar calc_end_adjustment(const SkScalar intervals[2], const SkPoint pts[2], SkScalar phase, SkScalar* endingInt)
{
...
void setup_dashed_rect(const SkRect& rect, VertexWriter& vertices, const SkMatrix& matrix, SkScalar offset, SkScalar bloatX, SkScalar len,
    SkScalar startInterval, SkScalar endInterval, SkScalar strokeWidth, SkScalar perpScale, DashCap cap)
{
    ...
    if (kRound_DashCap == cap) {
        ...
        vertices.writeQuad(GrQuad::MakeFromRect(rect, matrix), VertexWriter::TriStripFromRect(dashRect), intervalLength, radius, centerX);
    } else {
        SkASSERT(kNonRound_DashCap == cap);
        SkScalar halfOffLen = SkScalarHalf(endInterval);
        SkScalar halfStroke = SkScalarHalf(strokeWidth);
        SkRect rectParam;
        rectParam.setLTRB(halfOffLen + 0.5f, -halfStroke + 0.5f, halfOffLen + startInterval - 0.5f, halfStroke - 0.5f);

        vertices.writeQuad(GrQuad::MakeFromRect(rect, matrix), VertexWriter::TriStripFromRect(dashRect), intervalLength, rectParam);
    }
}
 ...
 */
GrGeometryProcessor* make_dash_gp(SkArenaAlloc* arena, const SkPMColor4f&, AAMode aaMode, DashCap cap, const SkMatrix& localMatrix, bool usesLocalCoords);
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const LineData& geometry, SkPaint::Cap cap, AAMode aaMode, bool fullDash,
        const GrUserStencilSettings* stencilSettings)
    {
        return GrOp::Make<DashOpImpl>(context, std::move(paint), geometry, cap, aaMode, fullDash, stencilSettings);
    }
    ...
    GrProcessorSet::Analysis finalize(const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) override
    {
        ...
        auto analysis = fProcessorSet.finalize(fColor, coverage, clip, fStencilSettings, caps, clampType, &fColor);
...
private:
    ...
    DashOpImpl(GrPaint&& paint, const LineData& geometry, SkPaint::Cap cap, AAMode aaMode, bool fullDash, const GrUserStencilSettings* stencilSettings)
        : INHERITED(ClassID())
        ...
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        ...
        if (this->fullDash()) {
            gp = make_dash_gp(arena, this->color(), this->aaMode(), capType, this->viewMatrix(), fUsesLocalCoords);
        } else {
            // Set up the vertex data for the line and start/end dashes
            using namespace GrDefaultGeoProcFactory;
            Color color(this->color());
            LocalCoords::Type localCoordsType = fUsesLocalCoords ? LocalCoords::kUsePosition_Type : LocalCoords::kUnused_Type;
            gp = MakeForDeviceSpace(arena, color, Coverage::kSolid_Type, localCoordsType, this->viewMatrix());
        }
        ...
        fProgramInfo = GrSimpleMeshDrawOpHelper::CreateProgramInfo(caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, gp,
            std::move(fProcessorSet), GrPrimitiveType::kTriangles, renderPassXferBarriers, colorLoadOp, GrPipeline::InputFlags::kNone, fStencilSettings);
    }
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        for (int i = 0; i < instanceCount; i++) {
            ...
            if (useAA) {
                if (draw.fPhase > 0 && draw.fPhase < draw.fIntervals[0]) {
                    ...
                    startPts[1].fX = std::min(startPts[0].fX + draw.fIntervals[0] - draw.fPhase, draw.fPtsRot[1].fX);
            ...
            SkScalar endAdj = calc_end_adjustment(draw.fIntervals, draw.fPtsRot, draw.fPhase, &endingInterval);
            ...
            if (!lineDone) {
                ...
                bounds.setLTRB(draw.fPtsRot[0].fX, draw.fPtsRot[0].fY, draw.fPtsRot[1].fX, draw.fPtsRot[1].fY);
        ...
        for (int i = 0; i < instanceCount; i++) {
            ...
            if (!draws[i].fLineDone) {
                if (fullDash) {
                    setup_dashed_rect(rects[rectIndex], vertices, geom.fSrcRotInv, draws[i].fStartOffset, draws[i].fDevBloatX, draws[i].fLineLength,
                        draws[i].fIntervals[0], draws[i].fIntervals[1], draws[i].fStrokeWidth, draws[i].fPerpendicularScale, capType);
                } else {
            ...
            if (draws[i].fHasStartRect) {
                if (fullDash) {
                    setup_dashed_rect(rects[rectIndex], vertices, geom.fSrcRotInv, draws[i].fStartOffset, draws[i].fDevBloatX, draws[i].fIntervals[0],
                        draws[i].fIntervals[0], draws[i].fIntervals[1], draws[i].fStrokeWidth, draws[i].fPerpendicularScale, capType);
                } else {
                    vertices.writeQuad(GrQuad::MakeFromRect(rects[rectIndex], geom.fSrcRotInv));
                }
    ...
    SkString onDumpInfo() const override
    {
        ...
        for (const auto& geo : fLines) {
            string.appendf("Pt0: [%.2f, %.2f], Pt1: [%.2f, %.2f], Width: %.2f, Ival0: %.2f, "
                           "Ival1 : %.2f, Phase: %.2f\n",
                geo.fPtsRot[0].fX, geo.fPtsRot[0].fY, geo.fPtsRot[1].fX, geo.fPtsRot[1].fY, geo.fSrcStrokeWidth, geo.fIntervals[0], geo.fIntervals[1],
                geo.fPhase);
        }
...
public:
    ...
    static GrGeometryProcessor* Make(SkArenaAlloc* arena, const SkPMColor4f&, AAMode aaMode, const SkMatrix& localMatrix, bool usesLocalCoords);
...
private:
    ...
    DashingCircleEffect(const SkPMColor4f&, AAMode aaMode, const SkMatrix& localMatrix, bool usesLocalCoords);
...
void DashingCircleEffect::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs)
{
    ...
    if (dce.fUsesLocalCoords) {
        WriteLocalCoord(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, dce.fInPosition.asShaderVar(), dce.fLocalMatrix, &fLocalMatrixUniform);
    }

    // transforms all points so that we can compare them to our test circle
    fragBuilder->codeAppendf(
        "half xShifted = half(%s.x - floor(%s.x / %s.z) * %s.z);", dashParams.fsIn(), dashParams.fsIn(), dashParams.fsIn(), dashParams.fsIn());
    fragBuilder->codeAppendf("half2 fragPosShifted = half2(xShifted, half(%s.y));", dashParams.fsIn());
...
void DashingCircleEffect::Impl::setData(const GrGLSLProgramDataManager& pdman, const GrShaderCaps& shaderCaps, const GrGeometryProcessor& geomProc)
{
...
GrGeometryProcessor* DashingCircleEffect::Make(SkArenaAlloc* arena, const SkPMColor4f& color, AAMode aaMode, const SkMatrix& localMatrix, bool usesLocalCoords)
{
    return arena->make([&](void* ptr) { return new (ptr) DashingCircleEffect(color, aaMode, localMatrix, usesLocalCoords); });
}
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> DashingCircleEffect::makeProgramImpl(const GrShaderCaps&) const
{
    return std::make_unique<Impl>();
}

DashingCircleEffect::DashingCircleEffect(const SkPMColor4f& color, AAMode aaMode, const SkMatrix& localMatrix, bool usesLocalCoords)
...
GrGeometryProcessor* DashingCircleEffect::TestCreate(GrProcessorTestData* d)
{
    ...
    return DashingCircleEffect::Make(d->allocator(), SkPMColor4f::FromBytes_RGBA(color), aaMode, matrix, d->fRandom->nextBool());
}
#endif
...
private:
    ...
    DashingLineEffect(const SkPMColor4f&, AAMode aaMode, const SkMatrix& localMatrix, bool usesLocalCoords);
...
void DashingLineEffect::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs)
{
    ...
    if (de.usesLocalCoords()) {
        WriteLocalCoord(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, de.fInPosition.asShaderVar(), de.fLocalMatrix, &fLocalMatrixUniform);
    }

    // transforms all points so that we can compare them to our test rect
    fragBuilder->codeAppendf(
        "half xShifted = half(%s.x - floor(%s.x / %s.z) * %s.z);", inDashParams.fsIn(), inDashParams.fsIn(), inDashParams.fsIn(), inDashParams.fsIn());
    fragBuilder->codeAppendf("half2 fragPosShifted = half2(xShifted, half(%s.y));", inDashParams.fsIn());
    if (de.fAAMode == AAMode::kCoverage) {
        // The amount of coverage removed in x and y by the edges is computed as a pair of negative
        // numbers, xSub and ySub.
        fragBuilder->codeAppend("half xSub, ySub;");
        fragBuilder->codeAppendf("xSub = half(min(fragPosShifted.x - %s.x, 0.0));", inRectParams.fsIn());
        fragBuilder->codeAppendf("xSub += half(min(%s.z - fragPosShifted.x, 0.0));", inRectParams.fsIn());
        fragBuilder->codeAppendf("ySub = half(min(fragPosShifted.y - %s.y, 0.0));", inRectParams.fsIn());
        fragBuilder->codeAppendf("ySub += half(min(%s.w - fragPosShifted.y, 0.0));", inRectParams.fsIn());
        ...
        fragBuilder->codeAppendf("half alpha = 1.0;");
        fragBuilder->codeAppendf("alpha *= (fragPosShifted.x - %s.x) > -0.5 ? 1.0 : 0.0;", inRectParams.fsIn());
        fragBuilder->codeAppendf("alpha *= (%s.z - fragPosShifted.x) >= -0.5 ? 1.0 : 0.0;", inRectParams.fsIn());
    }
    fragBuilder->codeAppendf("half4 %s = half4(alpha);", args.fOutputCoverage);
}

void DashingLineEffect::Impl::setData(const GrGLSLProgramDataManager& pdman, const GrShaderCaps& shaderCaps, const GrGeometryProcessor& geomProc)
{
...
GrGeometryProcessor* DashingLineEffect::Make(SkArenaAlloc* arena, const SkPMColor4f& color, AAMode aaMode, const SkMatrix& localMatrix, bool usesLocalCoords)
{
    return arena->make([&](void* ptr) { return new (ptr) DashingLineEffect(color, aaMode, localMatrix, usesLocalCoords); });
}
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> DashingLineEffect::makeProgramImpl(const GrShaderCaps&) const
{
    return std::make_unique<Impl>();
}

DashingLineEffect::DashingLineEffect(const SkPMColor4f& color, AAMode aaMode, const SkMatrix& localMatrix, bool usesLocalCoords)
...
GrGeometryProcessor* DashingLineEffect::TestCreate(GrProcessorTestData* d)
{
    ...
    GrColor color = GrTest::RandomColor(d->fRandom);
    SkMatrix matrix = GrTest::TestMatrix(d->fRandom);
    return DashingLineEffect::Make(d->allocator(), SkPMColor4f::FromBytes_RGBA(color), aaMode, matrix, d->fRandom->nextBool());
}

#endif
//////////////////////////////////////////////////////////////////////////////

GrGeometryProcessor* make_dash_gp(SkArenaAlloc* arena, const SkPMColor4f& color, AAMode aaMode, DashCap cap, const SkMatrix& viewMatrix, bool usesLocalCoords)
{
    SkMatrix invert;
...
GrOp::Owner MakeDashLineOp(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkPoint pts[2], AAMode aaMode, const GrStyle& style,
    const GrUserStencilSettings* stencilSettings)
{
    ...
    if (SkScalarNearlyZero(lineData.fParallelScale) || SkScalarNearlyZero(lineData.fPerpendicularScale)) {
        return nullptr;
    }
    ...
    return DashOpImpl::Make(context, std::move(paint), lineData, cap, aaMode, fullDash, stencilSettings);
}
...
GR_DRAW_OP_TEST_DEFINE(DashOpImpl)
{
    ...
    Intervals intervalType = SkPaint::kRound_Cap == cap ? kOpenClose_Intervals : Intervals(random->nextULessThan(kCloseOpen_Intervals + 1));
    ...
    return skgpu::ganesh::DashOp::MakeDashLineOp(context, std::move(paint), viewMatrix, pts, aaMode, style, GrGetRandomStencil(random, context));
}
