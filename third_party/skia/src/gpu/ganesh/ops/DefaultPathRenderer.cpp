...
public:
    PathGeoBuilder(GrPrimitiveType primitiveType, GrMeshDrawTarget* target, SkTDArray<GrSimpleMesh*>* meshes)
    ...
    {
        if (!this->ensureSpace(GrPathUtils::kMaxPointsPerCurve, GrPathUtils::kMaxPointsPerCurve * this->indexScale(), &pts[0])) {
            return;
        }
        ...
        uint16_t numPts = (uint16_t)GrPathUtils::generateQuadraticPoints(
            pts[0], pts[1], pts[2], srcSpaceTolSqd, &fCurVert, GrPathUtils::quadraticPointCount(pts, srcSpaceTol));
        if (this->isIndexed()) {
    ...
    void addConic(SkScalar weight, const SkPoint pts[], SkScalar srcSpaceTolSqd, SkScalar srcSpaceTol)
    {
    ...
    void addCubic(const SkPoint pts[], SkScalar srcSpaceTolSqd, SkScalar srcSpaceTol)
    {
        ...
        uint16_t numPts = (uint16_t)GrPathUtils::generateCubicPoints(
            pts[0], pts[1], pts[2], pts[3], srcSpaceTolSqd, &fCurVert, GrPathUtils::cubicPointCount(pts, srcSpaceTol));
        if (this->isIndexed()) {
            for (uint16_t i = 0; i < numPts; ++i) {
...
private:
    ...
    {
        return GrPrimitiveType::kLines == fPrimitiveType || GrPrimitiveType::kTriangles == fPrimitiveType;
    }
    bool isHairline() const
    {
        return GrPrimitiveType::kLines == fPrimitiveType || GrPrimitiveType::kLineStrip == fPrimitiveType;
    }
    ...
    void allocNewBuffers()
    {
        ...
        fVertices = static_cast<SkPoint*>(
            fTarget->makeVertexSpaceAtLeast(fVertexStride, kMinVerticesPerChunk, kFallbackVerticesPerChunk, &fVertexBuffer, &fFirstVertex, &fVerticesInChunk));
        if (!fVertices) {
        ...
        if (this->isIndexed()) {
            ...
            fIndices = fTarget->makeIndexSpaceAtLeast(kMinIndicesPerChunk, kFallbackIndicesPerChunk, &fIndexBuffer, &fFirstIndex, &fIndicesInChunk);
            if (!fIndices) {
    ...
    void createMeshAndPutBackReserve()
    {
        ...
        if (this->isIndexed() ? SkToBool(indexCount) : SkToBool(vertexCount)) {
            ...
            if (!this->isIndexed()) {
                ...
                mesh->setIndexed(
                    std::move(fIndexBuffer), indexCount, fFirstIndex, 0, vertexCount - 1, GrPrimitiveRestart::kNo, std::move(fVertexBuffer), fFirstVertex);
            }
        }
    ...
    bool ensureSpace(int vertsNeeded, int indicesNeeded = 0, const SkPoint* lastPoint = nullptr)
    {
        ...
        if (fCurVert + vertsNeeded > fVertices + fVerticesInChunk || fCurIdx + indicesNeeded > fIndices + fIndicesInChunk) {
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkPath& path, SkScalar tolerance, uint8_t coverage, const SkMatrix& viewMatrix,
        bool isHairline, GrAAType aaType, const SkRect& devBounds, const GrUserStencilSettings* stencilSettings)
    {
        return Helper::FactoryHelper<DefaultPathOp>(
            context, std::move(paint), path, tolerance, coverage, viewMatrix, isHairline, aaType, devBounds, stencilSettings);
    }
    ...
    DefaultPathOp(GrProcessorSet* processorSet, const SkPMColor4f& color, const SkPath& path, SkScalar tolerance, uint8_t coverage, const SkMatrix& viewMatrix,
        bool isHairline, GrAAType aaType, const SkRect& devBounds, const GrUserStencilSettings* stencilSettings)
        : INHERITED(ClassID())
    ...
    {
        GrProcessorAnalysisCoverage gpCoverage = this->coverage() == 0xFF ? GrProcessorAnalysisCoverage::kNone : GrProcessorAnalysisCoverage::kSingleChannel;
...
private:
    GrPrimitiveType primType() const
    {
        if (this->isHairline()) {
            ...
            bool isIndexed = instanceCount > 1 || PathGeoBuilder::PathHasMultipleSubpaths(fPaths[0].fPath);
        ...
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        ...
        fProgramInfo = fHelper.createProgramInfoWithStencil(
            caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, gp, this->primType(), renderPassXferBarriers, colorLoadOp);
    }
...
GR_DRAW_OP_TEST_DEFINE(DefaultPathOp)
{
    ...
    return DefaultPathOp::Make(context, std::move(paint), path, srcSpaceTol, coverage, viewMatrix, true, aaType, bounds, GrGetRandomStencil(random, context));
}
...
bool DefaultPathRenderer::internalDrawPath(skgpu::ganesh::SurfaceDrawContext* sdc, GrPaint&& paint, GrAAType aaType,
    const GrUserStencilSettings& userStencilSettings, const GrClip* clip, const SkMatrix& viewMatrix, const GrStyledShape& shape, bool stencilOnly)
{
    ...
    SkRect devBounds;
    GetPathDevBounds(path, sdc->asRenderTargetProxy()->backingStoreDimensions(), viewMatrix, &devBounds);
    ...
    for (int p = 0; p < passCount; ++p) {
        if (lastPassIsBounds && (p == passCount - 1)) {
            ...
            assert_alive(paint);
            sdc->stencilRect(clip, passes[p], std::move(paint), GrAA(aaType == GrAAType::kMSAA), viewM, bounds, &localMatrix);
        } else {
            ...
            if (stencilPass) {
                ...
                op = DefaultPathOp::Make(
                    context, std::move(stencilPaint), path, srcSpaceTol, newCoverage, viewMatrix, isHairline, aaType, devBounds, passes[p]);
            } else {
                assert_alive(paint);
                op = DefaultPathOp::Make(context, std::move(paint), path, srcSpaceTol, newCoverage, viewMatrix, isHairline, aaType, devBounds, passes[p]);
            }
...
PathRenderer::StencilSupport DefaultPathRenderer::onGetStencilSupport(const GrStyledShape& shape) const
{
    if (single_pass_shape(shape)) {
...
{
    bool isHairline = GrIsStrokeHairlineOrEquivalent(args.fShape->style(), *args.fViewMatrix, nullptr);
    // If we aren't a single_pass_shape or hairline, we require stencil buffers.
    if (!(single_pass_shape(*args.fShape) || isHairline) && !args.fProxy->canUseStencil(*args.fCaps)) {
...
bool DefaultPathRenderer::onDrawPath(const DrawPathArgs& args)
{
    ...
    return this->internalDrawPath(
        args.fSurfaceDrawContext, std::move(args.fPaint), aaType, *args.fUserStencilSettings, args.fClip, *args.fViewMatrix, *args.fShape, false);
}

void DefaultPathRenderer::onStencilPath(const StencilPathArgs& args)
{
    GR_AUDIT_TRAIL_AUTO_FRAME(args.fContext->priv().auditTrail(), "DefaultPathRenderer::onStencilPath");
    ...
    this->internalDrawPath(
        args.fSurfaceDrawContext, std::move(paint), aaType, GrUserStencilSettings::kUnused, args.fClip, *args.fViewMatrix, *args.fShape, true);
}
