...
public:
    ...
    ShadowCircularRRectOp(
        GrColor color, const SkRect& devRect, float devRadius, bool isCircle, float blurRadius, float insetWidth, GrSurfaceProxyView falloffView)
        : INHERITED(ClassID())
        , fFalloffView(std::move(falloffView))
    {
        ...
        fGeoData.emplace_back(Geometry { color, outerRadius, umbraInset, innerRadius, blurRadius, bounds, type, isCircle });
        if (isCircle) {
...
private:
    ...
    void fillInRRectVerts(const Geometry& args, CircleVertex** verts) const
    {
        ...
        SkScalar xInner[4] = { bounds.fLeft + umbraInset, bounds.fRight - umbraInset, bounds.fLeft + umbraInset, bounds.fRight - umbraInset };
        SkScalar xMid[4] = { bounds.fLeft + outerRadius, bounds.fRight - outerRadius, bounds.fLeft + outerRadius, bounds.fRight - outerRadius };
        SkScalar xOuter[4] = { bounds.fLeft, bounds.fRight, bounds.fLeft, bounds.fRight };
        SkScalar yInner[4] = { bounds.fTop + umbraInset, bounds.fTop + umbraInset, bounds.fBottom - umbraInset, bounds.fBottom - umbraInset };
        SkScalar yMid[4] = { bounds.fTop + outerRadius, bounds.fTop + outerRadius, bounds.fBottom - outerRadius, bounds.fBottom - outerRadius };
        ...
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        ...
        fProgramInfo = GrSimpleMeshDrawOpHelper::CreateProgramInfo(caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, gp,
            GrProcessorSet::MakeEmptySet(), GrPrimitiveType::kTriangles, renderPassXferBarriers, colorLoadOp, GrPipeline::InputFlags::kNone,
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        int firstVertex;
        CircleVertex* verts = (CircleVertex*)target->makeVertexSpace(sizeof(CircleVertex), fVertCount, &vertexBuffer, &firstVertex);
        if (!verts) {
    ...
    void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override
    {
        ...
        flushState->bindTextures(fProgramInfo->geomProc(), *fFalloffView.proxy(), fProgramInfo->pipeline());
    ...
    SkString onDumpInfo() const override
    {
        ...
        for (int i = 0; i < fGeoData.size(); ++i) {
            string.appendf("Color: 0x%08x Rect [L: %.2f, T: %.2f, R: %.2f, B: %.2f],"
                ...
                fGeoData[i].fColor, fGeoData[i].fDevBounds.fLeft, fGeoData[i].fDevBounds.fTop, fGeoData[i].fDevBounds.fRight, fGeoData[i].fDevBounds.fBottom,
                fGeoData[i].fOuterRadius, fGeoData[i].fUmbraInset, fGeoData[i].fInnerRadius, fGeoData[i].fBlurRadius);
        }
        return string;
    }
#endif
...
GrOp::Owner Make(GrRecordingContext* context, GrColor color, const SkMatrix& viewMatrix, const SkRRect& rrect, SkScalar blurWidth, SkScalar insetWidth)
{
    ...
    return GrOp::Make<ShadowCircularRRectOp>(context, color, bounds, scaledRadius, rrect.isOval(), blurWidth, scaledInsetWidth, std::move(falloffView));
}
...
GR_DRAW_OP_TEST_DEFINE(ShadowRRectOp)
{
    ...
    do {
        ...
        if (isCircle) {
            ...
            if (auto op = skgpu::ganesh::ShadowRRectOp::Make(context, color, viewMatrix, rrect, blurWidth, insetWidth)) {
                return op;
            }
        } else {
            SkRRect rrect;
