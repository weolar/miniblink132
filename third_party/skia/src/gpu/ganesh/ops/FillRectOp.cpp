...
#if defined(GPU_TEST_UTILS)
SkString dump_quad_info(int index, const GrQuad* deviceQuad, const GrQuad* localQuad, const SkPMColor4f& color, GrQuadAAFlags aaFlags)
{
    ...
    str.appendf("%d: Color: [%.2f, %.2f, %.2f, %.2f], Edge AA: l%u_t%u_r%u_b%u, \n"
                ...
                "(%.2f, %.2f, %.2f)]\n",
        index, color.fR, color.fG, color.fB, color.fA, (uint32_t)(aaFlags & GrQuadAAFlags::kLeft), (uint32_t)(aaFlags & GrQuadAAFlags::kTop),
        (uint32_t)(aaFlags & GrQuadAAFlags::kRight), (uint32_t)(aaFlags & GrQuadAAFlags::kBottom), deviceQuad->x(0), deviceQuad->y(0), deviceQuad->w(0),
        deviceQuad->x(1), deviceQuad->y(1), deviceQuad->w(1), deviceQuad->x(2), deviceQuad->y(2), deviceQuad->w(2), deviceQuad->x(3), deviceQuad->y(3),
        deviceQuad->w(3), safeLocal.x(0), safeLocal.y(0), safeLocal.w(0), safeLocal.x(1), safeLocal.y(1), safeLocal.w(1), safeLocal.x(2), safeLocal.y(2),
        safeLocal.w(2), safeLocal.x(3), safeLocal.y(3), safeLocal.w(3));
    return str;
}
#endif
...
public:
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, DrawQuad* quad, const GrUserStencilSettings* stencilSettings,
        Helper::InputFlags inputFlags)
    {
        ...
        return Helper::FactoryHelper<FillRectOpImpl>(context, std::move(paint), aaType, quad, stencilSettings, inputFlags);
    }
    ...
    FillRectOpImpl(GrProcessorSet* processorSet, SkPMColor4f paintColor, GrAAType aaType, DrawQuad* quad, const GrUserStencilSettings* stencil,
        Helper::InputFlags inputFlags)
        : INHERITED(ClassID())
        ...
        , fQuads(1, !fHelper.isTrivial())
    {
        ...
        fQuads.append(quad->fDevice, { paintColor, quad->fEdgeFlags }, fHelper.isTrivial() ? nullptr : &quad->fLocal);
        if (count > 1) {
            fQuads.append(extra.fDevice, { paintColor, extra.fEdgeFlags }, fHelper.isTrivial() ? nullptr : &extra.fLocal);
        }
    }
    ...
    GrProcessorSet::Analysis finalize(const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) override
    {
        ...
        auto coverage = fHelper.aaType() == GrAAType::kCoverage ? GrProcessorAnalysisCoverage::kSingleChannel : GrProcessorAnalysisCoverage::kNone;
        ...
        if (quadColors.isConstant(&colorOverride)) {
            ...
            while (iter.next()) {
                fColorType = std::max(fColorType, skgpu::ganesh::QuadPerEdgeAA::MinColorType(iter->fColor));
            }
        }
...
private:
    ...
    {
        auto indexBufferOption = skgpu::ganesh::QuadPerEdgeAA::CalcIndexBufferOption(fHelper.aaType(), fQuads.count());

        return VertexSpec(fQuads.deviceQuadType(), fColorType, fQuads.localQuadType(), fHelper.usesLocalCoords(), Subset::kNo, fHelper.aaType(),
        ...
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        ...
        fProgramInfo = fHelper.createProgramInfoWithStencil(
            caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, gp, vertexSpec.primitiveType(), renderPassXferBarriers, colorLoadOp);
    }

    void onPrePrepareDraws(GrRecordingContext* rContext, const GrSurfaceProxyView& writeView, GrAppliedClip* clip, const GrDstProxyView& dstProxyView,
        GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        ...
        INHERITED::onPrePrepareDraws(rContext, writeView, clip, dstProxyView, renderPassXferBarriers, colorLoadOp);
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        void* vdata = target->makeVertexSpace(vertexSpec.vertexSize(), totalNumVertices, &fVertexBuffer, &fBaseVertex);
        if (!vdata) {
        ...
        if (vertexSpec.needsIndexBuffer()) {
            fIndexBuffer = skgpu::ganesh::QuadPerEdgeAA::GetIndexBuffer(target, vertexSpec.indexBufferOption());
            if (!fIndexBuffer) {
    ...
    void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override
    {
        ...
        skgpu::ganesh::QuadPerEdgeAA::IssueDraw(flushState->caps(), flushState->opsRenderPass(), vertexSpec, 0, fQuads.count(), totalNumVertices, fBaseVertex);
    }
    ...
    CombineResult onCombineIfPossible(GrOp* t, SkArenaAlloc*, const GrCaps& caps) override
    {
        ...
        if (CombinedQuadCountWillOverflow(fHelper.aaType(), upgradeToCoverageAAOnMerge, fQuads.count() + that->fQuads.count())) {
            return CombineResult::kCannotCombine;
        }
    ...
    bool canAddQuads(int numQuads, GrAAType aaType)
    {
        ...
        if (aaType != fHelper.aaType() && aaType != GrAAType::kNone) {
            auto indexBufferOption = skgpu::ganesh::QuadPerEdgeAA::CalcIndexBufferOption(aaType, quadCount);
            ...
            auto indexBufferOption = skgpu::ganesh::QuadPerEdgeAA::CalcIndexBufferOption(fHelper.aaType(), quadCount);
            if (quadCount > skgpu::ganesh::QuadPerEdgeAA::QuadLimit(indexBufferOption)) {
    ...
    bool addQuad(DrawQuad* quad, const SkPMColor4f& color, GrAAType aaType)
    {
        ...
        if (count == 0) {
            ...
            fQuads.append(quad->fDevice, { color, quad->fEdgeFlags }, fHelper.isTrivial() ? nullptr : &quad->fLocal);
            if (count > 1) {
                fQuads.append(extra.fDevice, { color, extra.fEdgeFlags }, fHelper.isTrivial() ? nullptr : &extra.fLocal);
            }
            // Update the bounds
            this->setBounds(newBounds, HasAABloat(fHelper.aaType() == GrAAType::kCoverage), IsHairline::kNo);
            return true;
        }
    }
...
GrOp::Owner FillRectOp::Make(
    GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, DrawQuad* quad, const GrUserStencilSettings* stencil, InputFlags inputFlags)
{
    return FillRectOpImpl::Make(context, std::move(paint), aaType, std::move(quad), stencil, inputFlags);
}
    ...
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& view, const SkRect& rect, const GrUserStencilSettings* stencil)
{
    ...
    return FillRectOpImpl::Make(context, std::move(paint), GrAAType::kNone, &quad, stencil, InputFlags::kNone);
}

GrOp::Owner FillRectOp::MakeOp(GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, const SkMatrix& viewMatrix, const GrQuadSetEntry quads[], int cnt,
    const GrUserStencilSettings* stencilSettings, int* numConsumed)
{
    // First make a draw op for the first quad in the set
    SkASSERT(cnt > 0);

    DrawQuad quad { GrQuad::MakeFromRect(quads[0].fRect, viewMatrix), GrQuad::MakeFromRect(quads[0].fRect, quads[0].fLocalMatrix), quads[0].fAAFlags };
    paint.setColor4f(quads[0].fColor);
    GrOp::Owner op = FillRectOp::Make(context, std::move(paint), aaType, &quad, stencilSettings, InputFlags::kNone);
    auto fillRects = op->cast<FillRectOpImpl>();

    *numConsumed = 1;
    // Accumulate remaining quads similar to onCombineIfPossible() without creating an op
    for (int i = 1; i < cnt; ++i) {
        quad = { GrQuad::MakeFromRect(quads[i].fRect, viewMatrix), GrQuad::MakeFromRect(quads[i].fRect, quads[i].fLocalMatrix), quads[i].fAAFlags };

        GrAAType resolvedAA;
        GrQuadUtils::ResolveAAType(aaType, quads[i].fAAFlags, quad.fDevice, &resolvedAA, &quad.fEdgeFlags);
...
void FillRectOp::AddFillRectOps(skgpu::ganesh::SurfaceDrawContext* sdc, const GrClip* clip, GrRecordingContext* context, GrPaint&& paint, GrAAType aaType,
    const SkMatrix& viewMatrix, const GrQuadSetEntry quads[], int cnt, const GrUserStencilSettings* stencilSettings)
{
    int offset = 0;
    int numLeft = cnt;
    while (numLeft) {
        ...
        GrOp::Owner op = MakeOp(context, GrPaint::Clone(paint), aaType, viewMatrix, &quads[offset], numLeft, stencilSettings, &numConsumed);
...
uint32_t skgpu::ganesh::FillRectOp::ClassID()
{
...
GR_DRAW_OP_TEST_DEFINE(FillRectOp)
{
    ...
    const GrUserStencilSettings* stencil = random->nextBool() ? nullptr : GrGetRandomStencil(random, context);
    ...
    if (random->nextBool()) {
        if (random->nextBool()) {
            ...
            DrawQuad quad = { GrQuad::MakeFromRect(rect, viewMatrix), GrQuad::MakeFromRect(rect, localMatrix), aaFlags };
