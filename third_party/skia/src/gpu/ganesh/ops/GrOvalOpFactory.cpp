...
public:
    static GrGeometryProcessor* Make(
        SkArenaAlloc* arena, bool stroke, bool clipPlane, bool isectPlane, bool unionPlane, bool roundCaps, bool wideColor, const SkMatrix& localMatrix)
    {
        return arena->make(
            [&](void* ptr) { return new (ptr) CircleGeometryProcessor(stroke, clipPlane, isectPlane, unionPlane, roundCaps, wideColor, localMatrix); });
    }
...
private:
    CircleGeometryProcessor(bool stroke, bool clipPlane, bool isectPlane, bool unionPlane, bool roundCaps, bool wideColor, const SkMatrix& localMatrix)
    ...
    public:
        ...
        {
            SetTransform(pdman, shaderCaps, fLocalMatrixUniform, geomProc.cast<CircleGeometryProcessor>().fLocalMatrix, &fLocalMatrix);
        }
    ...
    private:
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            ...
            if (cgp.fInRoundCapCenters.isInitialized()) {
                ...
                varyingHandler->addPassThroughAttribute(cgp.fInRoundCapCenters.asShaderVar(), "roundCapCenters");
                varyingHandler->addVarying("capRadius", &capRadius, GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
                // This is the cap radius in normalized space where the outer radius is 1 and
                // circledEdge.w is the normalized inner radius.
                vertBuilder->codeAppendf("%s = (1.0 - %s.w) / 2.0;", capRadius.vsOut(), cgp.fInCircleEdge.name());
            }
            ...
            WriteLocalCoord(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, cgp.fInPosition.asShaderVar(), cgp.fLocalMatrix, &fLocalMatrixUniform);
            ...
            if (cgp.fStroke) {
                fragBuilder->codeAppend("half distanceToInnerEdge = half(circleEdge.z * (d - circleEdge.w));");
                fragBuilder->codeAppend("half innerAlpha = saturate(distanceToInnerEdge);");
                fragBuilder->codeAppend("edgeAlpha *= innerAlpha;");
            }

            if (cgp.fInClipPlane.isInitialized()) {
                fragBuilder->codeAppend("half clip = half(saturate(circleEdge.z * dot(circleEdge.xy, "
                                        "clipPlane.xy) + clipPlane.z));");
                if (cgp.fInIsectPlane.isInitialized()) {
                    fragBuilder->codeAppend("clip *= half(saturate(circleEdge.z * dot(circleEdge.xy, "
                                            "isectPlane.xy) + isectPlane.z));");
                }
                if (cgp.fInUnionPlane.isInitialized()) {
                    fragBuilder->codeAppend("clip = saturate(clip + half(saturate(circleEdge.z * dot(circleEdge.xy,"
                ...
                if (cgp.fInRoundCapCenters.isInitialized()) {
                    ...
                    fragBuilder->codeAppendf("half dcap1 = half(circleEdge.z * (%s - length(circleEdge.xy - "
                                             "roundCapCenters.xy)));"
...
GrGeometryProcessor* CircleGeometryProcessor::TestCreate(GrProcessorTestData* d)
{
    ...
    return CircleGeometryProcessor::Make(d->allocator(), stroke, clipPlane, isectPlane, unionPlane, roundCaps, wideColor, matrix);
}
#endif
...
public:
    ...
    {
        return arena->make([&](void* ptr) { return new (ptr) ButtCapDashedCircleGeometryProcessor(wideColor, localMatrix); });
    }
...
private:
    ...
    public:
        void setData(const GrGLSLProgramDataManager& pdman, const GrShaderCaps& shaderCaps, const GrGeometryProcessor& geomProc) override
        {
            SetTransform(pdman, shaderCaps, fLocalMatrixUniform, geomProc.cast<ButtCapDashedCircleGeometryProcessor>().fLocalMatrix, &fLocalMatrix);
        }

    private:
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            const ButtCapDashedCircleGeometryProcessor& bcscgp = args.fGeomProc.cast<ButtCapDashedCircleGeometryProcessor>();
            GrGLSLVertexBuilder* vertBuilder = args.fVertBuilder;
            GrGLSLVaryingHandler* varyingHandler = args.fVaryingHandler;
            GrGLSLUniformHandler* uniformHandler = args.fUniformHandler;
            GrGLSLFPFragmentBuilder* fragBuilder = args.fFragBuilder;
            ...
            varyingHandler->addPassThroughAttribute(bcscgp.fInDashParams.asShaderVar(), "dashParams", GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
            GrGLSLVarying wrapDashes(SkSLType::kHalf4);
            varyingHandler->addVarying("wrapDashes", &wrapDashes, GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
            GrGLSLVarying lastIntervalLength(SkSLType::kHalf);
            varyingHandler->addVarying("lastIntervalLength", &lastIntervalLength, GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
            ...
            // setup pass through color
            fragBuilder->codeAppendf("half4 %s;", args.fOutputColor);
            varyingHandler->addPassThroughAttribute(bcscgp.fInColor.asShaderVar(), args.fOutputColor, GrGLSLVaryingHandler::Interpolation::kCanBeFlat);

            // Setup position
            WriteOutputPosition(vertBuilder, gpArgs, bcscgp.fInPosition.name());
            WriteLocalCoord(
                vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, bcscgp.fInPosition.asShaderVar(), bcscgp.fLocalMatrix, &fLocalMatrixUniform);
            ...
            fragBuilder->emitFunction(SkSLType::kFloat, fnName.c_str(), { fnArgs, std::size(fnArgs) },
                "float linearDist;"
                "angleToEdge = clamp(angleToEdge, -3.1415, 3.1415);"
                "linearDist = diameter * sin(angleToEdge / 2);"
                "return saturate(linearDist + 0.5);");
            fragBuilder->codeAppend("float d = length(circleEdge.xy) * circleEdge.z;"
                                    ...
                                    "half dashAlpha = 0;");
            fragBuilder->codeAppendf("if (angleFromStart - x + dashParams.y >= 6.28318530718 + kDashBoundsEpsilon) {"
            ...
            fragBuilder->codeAppendf("if (angleFromStart - x - dashParams.y < -kDashBoundsEpsilon) {"
                                     ...
                                     "}",
                fnName.c_str(), fnName.c_str());
            fragBuilder->codeAppendf("dashAlpha += half(%s(x - currDash.x, d) * %s(currDash.y - x, d));"
                                     ...
                                     "edgeAlpha *= dashAlpha;",
                fnName.c_str(), fnName.c_str(), fnName.c_str(), fnName.c_str(), fnName.c_str(), fnName.c_str());
            fragBuilder->codeAppendf("half4 %s = half4(edgeAlpha);", args.fOutputCoverage);
        }
...
public:
    ...
    {
        return arena->make([&](void* ptr) { return new (ptr) EllipseGeometryProcessor(stroke, wideColor, useScale, localMatrix); });
    }
...
private:
    ...
    private:
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            ...
            WriteLocalCoord(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, egp.fInPosition.asShaderVar(), egp.fLocalMatrix, &fLocalMatrixUniform);
            ...
            if (egp.fUseScale) {
                fragBuilder->codeAppendf("float2 grad = 2.0*offset*(%s.z*%s.xy);", ellipseOffsets.fsIn(), ellipseRadii.fsIn());
            } else {
            ...
            if (egp.fUseScale) {
                fragBuilder->codeAppendf("float invlen = %s.z*inversesqrt(grad_dot);", ellipseOffsets.fsIn());
            } else {
            ...
            // for inner curve
            if (egp.fStroke) {
                fragBuilder->codeAppendf("offset = %s.xy*%s.zw;", ellipseOffsets.fsIn(), ellipseRadii.fsIn());
                fragBuilder->codeAppend("test = dot(offset, offset) - 1.0;");
                if (egp.fUseScale) {
                    fragBuilder->codeAppendf("grad = 2.0*offset*(%s.z*%s.zw);", ellipseOffsets.fsIn(), ellipseRadii.fsIn());
                } else {
...
public:
    ...
    {
        return arena->make([&](void* ptr) { return new (ptr) DIEllipseGeometryProcessor(wideColor, useScale, viewMatrix, style); });
    }
    ...
    void addToKey(const GrShaderCaps& caps, skgpu::KeyBuilder* b) const override
    {
        ...
        b->addBits(ProgramImpl::kMatrixKeyBits, ProgramImpl::ComputeMatrixKey(caps, fViewMatrix), "viewMatrixType");
    }
...
private:
    DIEllipseGeometryProcessor(bool wideColor, bool useScale, const SkMatrix& viewMatrix, DIEllipseStyle style)
    ...
    private:
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            ...
            // Setup position
            WriteOutputPosition(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, diegp.fInPosition.name(), diegp.fViewMatrix, &fViewMatrixUniform);
            ...
            fragBuilder->codeAppendf("float2 grad = float2(%s.x*duvdx.x + %s.y*duvdx.y,"
            ...
            if (DIEllipseStyle::kStroke == diegp.fStyle) {
                ...
                fragBuilder->codeAppendf("grad = float2(%s.x*duvdx.x + %s.y*duvdx.y,"
                                         "              %s.x*duvdy.x + %s.y*duvdy.y);",
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, SkPoint center, SkScalar radius, const GrStyle& style,
        const ArcParams* arcParams = nullptr)
    {
        ...
        return Helper::FactoryHelper<CircleOp>(context, std::move(paint), viewMatrix, center, radius, style, arcParams);
    }

    CircleOp(GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMatrix& viewMatrix, SkPoint center, SkScalar radius, const GrStyle& style,
        ...
        , fHelper(processorSet, GrAAType::kCoverage)
    {
        ...
        bool isStrokeOnly = SkStrokeRec::kStroke_Style == recStyle || SkStrokeRec::kHairline_Style == recStyle;
        ...
        SkRect devBounds = SkRect::MakeLTRB(center.fX - outerRadius, center.fY - outerRadius, center.fX + outerRadius, center.fY + outerRadius);
        if (arcParams) {
            ...
            auto upperLeftDet = viewMatrix.getScaleX() * viewMatrix.getScaleY() - viewMatrix.getSkewX() * viewMatrix.getSkewY();
            if (upperLeftDet < 0) {
                std::swap(startPoint, stopPoint);
            }

            fRoundCaps = stroked && style.strokeRec().getWidth() > 0 && style.strokeRec().getCap() == SkPaint::kRound_Cap;
            SkPoint roundCaps[2];
            if (fRoundCaps) {
            ...
            bool useCenter = (arcParams->fUseCenter || isStrokeOnly) && !SkScalarNearlyEqual(absSweep, SK_ScalarPI);
            if (useCenter) {
                ...
                if (absSweep > SK_ScalarPI) {
                    fCircles.emplace_back(Circle { color, innerRadius, outerRadius, { norm0.fX, norm0.fY, 0.5f },
                        { kUnusedIsectPlane[0], kUnusedIsectPlane[1], kUnusedIsectPlane[2] }, { norm1.fX, norm1.fY, 0.5f }, { roundCaps[0], roundCaps[1] },
                        devBounds, stroked });
                    fClipPlaneIsect = false;
                    fClipPlaneUnion = true;
                } else {
                    fCircles.emplace_back(Circle { color, innerRadius, outerRadius, { norm0.fX, norm0.fY, 0.5f }, { norm1.fX, norm1.fY, 0.5f },
                        { kUnusedUnionPlane[0], kUnusedUnionPlane[1], kUnusedUnionPlane[2] }, { roundCaps[0], roundCaps[1] }, devBounds, stroked });
                ...
                fCircles.emplace_back(
                    Circle { color, innerRadius, outerRadius, { norm.fX, norm.fY, d }, { kUnusedIsectPlane[0], kUnusedIsectPlane[1], kUnusedIsectPlane[2] },
                        { kUnusedUnionPlane[0], kUnusedUnionPlane[1], kUnusedUnionPlane[2] }, { roundCaps[0], roundCaps[1] }, devBounds, stroked });
                fClipPlane = true;
                fClipPlaneIsect = false;
                fClipPlaneUnion = false;
            ...
            fCircles.emplace_back(Circle { color, innerRadius, outerRadius, { kUnusedIsectPlane[0], kUnusedIsectPlane[1], kUnusedIsectPlane[2] },
                { kUnusedIsectPlane[0], kUnusedIsectPlane[1], kUnusedIsectPlane[2] }, { kUnusedUnionPlane[0], kUnusedUnionPlane[1], kUnusedUnionPlane[2] },
                { kUnusedRoundCaps[0], kUnusedRoundCaps[1] }, devBounds, stroked });
            fClipPlane = false;
            fClipPlaneIsect = false;
            fClipPlaneUnion = false;
        }
        ...
        // AA bloat.
        radius += halfWidth;
        this->setBounds({ center.fX - radius, center.fY - radius, center.fX + radius, center.fY + radius }, HasAABloat::kYes, IsHairline::kNo);
    ...
    GrProcessorSet::Analysis finalize(const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) override
    {
        ...
        return fHelper.finalizeProcessors(caps, clip, clampType, GrProcessorAnalysisCoverage::kSingleChannel, color, &fWideColor);
    }
...
private:
    ...
    void onCreateProgramInfo(const GrCaps* caps, SkArenaAlloc* arena, const GrSurfaceProxyView& writeView, bool usesMSAASurface, GrAppliedClip&& appliedClip,
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        ...
        GrGeometryProcessor* gp
            = CircleGeometryProcessor::Make(arena, !fAllFill, fClipPlane, fClipPlaneIsect, fClipPlaneUnion, fRoundCaps, fWideColor, localMatrix);

        fProgramInfo = fHelper.createProgramInfo(caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, gp, GrPrimitiveType::kTriangles,
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        int firstVertex;
        VertexWriter vertices = target->makeVertexWriter(fProgramInfo->geomProc().vertexStride(), fVertCount, &vertexBuffer, &firstVertex);
        if (!vertices) {
        ...
        for (const auto& circle : fCircles) {
            ...
            if (!circle.fStroked && fClipPlane && fClipPlaneIsect
                && (circle.fClipPlane[0] * circle.fIsectPlane[0] + circle.fClipPlane[1] * circle.fIsectPlane[1]) < 0.0f) {
                ...
                geoClipPlane.set(circle.fClipPlane[1] - circle.fIsectPlane[1], circle.fIsectPlane[0] - circle.fClipPlane[0]);
            ...
            if (circle.fStroked) {
                ...
                for (int i = 0; i < 8; ++i) {
                    vertices << (center + kOctagonInner[i] * circle.fInnerRadius) << color << kOctagonInner[i] * innerRadius << radii;
                    if (fClipPlane) {
                        vertices << circle.fClipPlane;
                    }
                    if (fClipPlaneIsect) {
        ...
        fMesh->setIndexed(std::move(indexBuffer), fIndexCount, firstIndex, 0, fVertCount - 1, GrPrimitiveRestart::kNo, std::move(vertexBuffer), firstVertex);
    }
    ...
    CombineResult onCombineIfPossible(GrOp* t, SkArenaAlloc*, const GrCaps& caps) override
    {
        ...
        if (fHelper.usesLocalCoords() && !SkMatrixPriv::CheapEqual(fViewMatrixIfUsingLocalCoords, that->fViewMatrixIfUsingLocalCoords)) {
            return CombineResult::kCannotCombine;
        }
    ...
    SkString onDumpInfo() const override
    {
        ...
        for (int i = 0; i < fCircles.size(); ++i) {
            string.appendf("Color: 0x%08x Rect [L: %.2f, T: %.2f, R: %.2f, B: %.2f],"
                ...
                fCircles[i].fColor.toBytes_RGBA(), fCircles[i].fDevBounds.fLeft, fCircles[i].fDevBounds.fTop, fCircles[i].fDevBounds.fRight,
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, SkPoint center, SkScalar radius, SkScalar strokeWidth,
        SkScalar startAngle, SkScalar onAngle, SkScalar offAngle, SkScalar phaseAngle)
    {
        SkASSERT(circle_stays_circle(viewMatrix));
        SkASSERT(strokeWidth < 2 * radius);
        return Helper::FactoryHelper<ButtCapDashedCircleOp>(
            context, std::move(paint), viewMatrix, center, radius, strokeWidth, startAngle, onAngle, offAngle, phaseAngle);
    }

    ButtCapDashedCircleOp(GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMatrix& viewMatrix, SkPoint center, SkScalar radius,
        SkScalar strokeWidth, SkScalar startAngle, SkScalar onAngle, SkScalar offAngle, SkScalar phaseAngle)
        : GrMeshDrawOp(ClassID())
        , fHelper(processorSet, GrAAType::kCoverage)
    {
        SkASSERT(circle_stays_circle(viewMatrix));
        ...
        reflection = (viewMatrix.getScaleX() * viewMatrix.getScaleY() - viewMatrix.getSkewX() * viewMatrix.getSkewY()) < 0;
        ...
        fCircles.push_back(Circle { color, outerRadius, innerRadius, onAngle, totalAngle, startAngle, phaseAngle, devBounds });
        // Use the original radius and stroke radius for the bounds so that it does not include the
        // AA bloat.
        radius += halfWidth;
        this->setBounds({ center.fX - radius, center.fY - radius, center.fX + radius, center.fY + radius }, HasAABloat::kYes, IsHairline::kNo);
...
private:
        ...
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        ...
        GrGeometryProcessor* gp = ButtCapDashedCircleGeometryProcessor::Make(arena, fWideColor, localMatrix);
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        for (const auto& circle : fCircles) {
            ...
            for (int i = 0; i < 8; ++i) {
                vertices << (center + kOctagonOuter[i] * halfWidth) << color << reflectY(kOctagonOuter[i]) << circle.fOuterRadius << normInnerRadius
                         << dashParams;
            }

            // Compute the vertices of the inner octagon.
            for (int i = 0; i < 8; ++i) {
                vertices << (center + kOctagonInner[i] * circle.fInnerRadius) << color << (reflectY(kOctagonInner[i]) * normInnerRadius) << circle.fOuterRadius
    ...
    SkString onDumpInfo() const override
    {
        ...
        for (int i = 0; i < fCircles.size(); ++i) {
            string.appendf("Color: 0x%08x Rect [L: %.2f, T: %.2f, R: %.2f, B: %.2f],"
                           ...
                           "Phase: %.2f\n",
                fCircles[i].fColor.toBytes_RGBA(), fCircles[i].fDevBounds.fLeft, fCircles[i].fDevBounds.fTop, fCircles[i].fDevBounds.fRight,
                fCircles[i].fDevBounds.fBottom, fCircles[i].fInnerRadius, fCircles[i].fOuterRadius, fCircles[i].fOnAngle, fCircles[i].fTotalAngle,
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& ellipse, const SkStrokeRec& stroke)
    {
        ...
        params.fXRadius = SkScalarAbs(viewMatrix[SkMatrix::kMScaleX] * ellipseXRadius + viewMatrix[SkMatrix::kMSkewX] * ellipseYRadius);
        params.fYRadius = SkScalarAbs(viewMatrix[SkMatrix::kMSkewY] * ellipseXRadius + viewMatrix[SkMatrix::kMScaleY] * ellipseYRadius);
        ...
        if (hasStroke) {
            ...
            if (scaledStroke.length() > SK_ScalarHalf && (0.5f * params.fXRadius > params.fYRadius || 0.5f * params.fYRadius > params.fXRadius)) {
                return nullptr;
            }

            // we don't handle it if curvature of the stroke is less than curvature of the ellipse
            if (scaledStroke.fX * (params.fXRadius * params.fYRadius) < (scaledStroke.fY * scaledStroke.fY) * params.fXRadius
                || scaledStroke.fY * (params.fXRadius * params.fXRadius) < (scaledStroke.fX * scaledStroke.fX) * params.fYRadius) {
                return nullptr;
            }
        ...
        if (!context->priv().caps()->shaderCaps()->fFloatIs32Bits && (params.fXRadius >= kMaxOvalRadius || params.fYRadius >= kMaxOvalRadius)) {
            return nullptr;
        }
        ...
        , fUseScale(false)
    {
        ...
        fEllipses.emplace_back(Ellipse { color, params.fXRadius, params.fYRadius, params.fInnerXRadius, params.fInnerYRadius,
            SkRect::MakeLTRB(params.fCenter.fX - params.fXRadius, params.fCenter.fY - params.fYRadius, params.fCenter.fX + params.fXRadius,
...
private:
        ...
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        ...
        GrGeometryProcessor* gp = EllipseGeometryProcessor::Make(arena, fStroked, fWideColor, fUseScale, localMatrix);
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        for (const auto& ellipse : fEllipses) {
            ...
            } invRadii = { SkScalarInvert(xRadius), SkScalarInvert(yRadius), sk_ieee_float_divide(1.0f, ellipse.fInnerXRadius),
            ...
            // The inner radius in the vertex data must be specified in normalized space.
            verts.writeQuad(VertexWriter::TriStripFromRect(ellipse.fDevBounds.makeOutset(aaBloat, aaBloat)), color,
                origin_centered_tri_strip(xMaxOffset, yMaxOffset), VertexWriter::If(fUseScale, std::max(xRadius, yRadius)), invRadii);
        }
        fMesh = helper.mesh();
    }
    ...
    SkString onDumpInfo() const override
    {
        ...
        for (const auto& geo : fEllipses) {
            string.appendf("Color: 0x%08x Rect [L: %.2f, T: %.2f, R: %.2f, B: %.2f], "
                ...
                geo.fColor.toBytes_RGBA(), geo.fDevBounds.fLeft, geo.fDevBounds.fTop, geo.fDevBounds.fRight, geo.fDevBounds.fBottom, geo.fXRadius, geo.fYRadius,
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& ellipse, const SkStrokeRec& stroke)
    {
        ...
        if (SkStrokeRec::kFill_Style != style && SkStrokeRec::kHairline_Style != style) {
            ...
            // we only handle thick strokes for near-circular ellipses
            if (strokeWidth > SK_ScalarHalf && (SK_ScalarHalf * params.fXRadius > params.fYRadius || SK_ScalarHalf * params.fYRadius > params.fXRadius)) {
                return nullptr;
            }

            // we don't handle it if curvature of the stroke is less than curvature of the ellipse
            if (strokeWidth * (params.fYRadius * params.fYRadius) < (strokeWidth * strokeWidth) * params.fXRadius) {
                return nullptr;
            }
            if (strokeWidth * (params.fXRadius * params.fXRadius) < (strokeWidth * strokeWidth) * params.fYRadius) {
                return nullptr;
            }
        ...
        if (DIEllipseStyle::kStroke == params.fStyle && (params.fInnerXRadius <= 0 || params.fInnerYRadius <= 0)) {
        ...
        , fUseScale(false)
    {
        ...
        fEllipses.emplace_back(
            Ellipse { viewMatrix, color, params.fXRadius, params.fYRadius, params.fInnerXRadius, params.fInnerYRadius, geoDx, geoDy, params.fStyle,
                SkRect::MakeLTRB(params.fCenter.fX - params.fXRadius, params.fCenter.fY - params.fYRadius, params.fCenter.fX + params.fXRadius,
                    params.fCenter.fY + params.fYRadius) });
...
private:
    ...
    void onCreateProgramInfo(const GrCaps* caps, SkArenaAlloc* arena, const GrSurfaceProxyView& writeView, bool usesMSAASurface, GrAppliedClip&& appliedClip,
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        GrGeometryProcessor* gp = DIEllipseGeometryProcessor::Make(arena, fWideColor, fUseScale, this->viewMatrix(), this->style());
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        for (const auto& ellipse : fEllipses) {
            ...
            // full sample coverage.
            float aaBloat = target->usesMSAASurface() ? SK_ScalarSqrt2 : .5f;
            SkRect drawBounds = ellipse.fBounds.makeOutset(ellipse.fGeoDx * aaBloat, ellipse.fGeoDy * aaBloat);
            ...
            verts.writeQuad(VertexWriter::TriStripFromRect(drawBounds), color, origin_centered_tri_strip(outerCoordX, outerCoordY),
                VertexWriter::If(fUseScale, std::max(xRadius, yRadius)), origin_centered_tri_strip(innerCoordX, innerCoordY));
        }
        fMesh = helper.mesh();
    }
    ...
    SkString onDumpInfo() const override
    {
        ...
        for (const auto& geo : fEllipses) {
            string.appendf("Color: 0x%08x Rect [L: %.2f, T: %.2f, R: %.2f, B: %.2f], XRad: %.2f, "
                           ...
                           "GeoDY: %.2f\n",
                geo.fColor.toBytes_RGBA(), geo.fBounds.fLeft, geo.fBounds.fTop, geo.fBounds.fRight, geo.fBounds.fBottom, geo.fXRadius, geo.fYRadius,
    ...
    DIEllipseStyle style() const
    {
        return fEllipses[0].fStyle;
    }
...
public:
    ...
    static GrOp::Owner Make(
        GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& devRect, float devRadius, float devStrokeWidth, bool strokeOnly)
    {
        return Helper::FactoryHelper<CircularRRectOp>(context, std::move(paint), viewMatrix, devRect, devRadius, devStrokeWidth, strokeOnly);
    }
    CircularRRectOp(GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMatrix& viewMatrix, const SkRect& devRect, float devRadius,
...
private:
    static void FillInOverstrokeVerts(VertexWriter& verts, const SkRect& bounds, SkScalar smInset, SkScalar bigInset, SkScalar xOffset, SkScalar outerRadius,
        SkScalar innerRadius, const VertexColor& color)
    {
        SkASSERT(smInset < bigInset);

        // TL
        verts << (bounds.fLeft + smInset) << (bounds.fTop + smInset) << color << xOffset << 0.0f << outerRadius << innerRadius;

        // TR
        verts << (bounds.fRight - smInset) << (bounds.fTop + smInset) << color << xOffset << 0.0f << outerRadius << innerRadius;

        verts << (bounds.fLeft + bigInset) << (bounds.fTop + bigInset) << color << 0.0f << 0.0f << outerRadius << innerRadius;

        verts << (bounds.fRight - bigInset) << (bounds.fTop + bigInset) << color << 0.0f << 0.0f << outerRadius << innerRadius;

        verts << (bounds.fLeft + bigInset) << (bounds.fBottom - bigInset) << color << 0.0f << 0.0f << outerRadius << innerRadius;

        verts << (bounds.fRight - bigInset) << (bounds.fBottom - bigInset) << color << 0.0f << 0.0f << outerRadius << innerRadius;

        // BL
        verts << (bounds.fLeft + smInset) << (bounds.fBottom - smInset) << color << xOffset << 0.0f << outerRadius << innerRadius;

        // BR
        verts << (bounds.fRight - smInset) << (bounds.fBottom - smInset) << color << xOffset << 0.0f << outerRadius << innerRadius;
    }
        ...
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        ...
        GrGeometryProcessor* gp = CircleGeometryProcessor::Make(arena, !fAllFill, false, false, false, false, fWideColor, localMatrix);
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        VertexWriter verts = target->makeVertexWriter(fProgramInfo->geomProc().vertexStride(), fVertCount, &vertexBuffer, &firstVertex);
        if (!verts) {
            SkDebugf("Could not allocate vertices\n");
            return;
        }
        ...
        for (const auto& rrect : fRRects) {
            ...
            SkScalar yCoords[4] = { bounds.fTop, bounds.fTop + outerRadius, bounds.fBottom - outerRadius, bounds.fBottom };

            SkScalar yOuterRadii[4] = { -1, 0, 0, 1 };
            // The inner radius in the vertex data must be specified in normalized space.
            // For fills, specifying -1/outerRadius guarantees an alpha of 1.0 at the inner radius.
            SkScalar innerRadius = rrect.fType != kFill_RRectType ? rrect.fInnerRadius / rrect.fOuterRadius : -1.0f / rrect.fOuterRadius;
            for (int i = 0; i < 4; ++i) {
                verts << bounds.fLeft << yCoords[i] << color << -1.0f << yOuterRadii[i] << outerRadius << innerRadius;

                verts << (bounds.fLeft + outerRadius) << yCoords[i] << color << 0.0f << yOuterRadii[i] << outerRadius << innerRadius;

                verts << (bounds.fRight - outerRadius) << yCoords[i] << color << 0.0f << yOuterRadii[i] << outerRadius << innerRadius;

                verts << bounds.fRight << yCoords[i] << color << 1.0f << yOuterRadii[i] << outerRadius << innerRadius;
            }
            ...
            if (kOverstroke_RRectType == rrect.fType) {
                ...
                FillInOverstrokeVerts(verts, bounds, outerRadius, overstrokeOuterRadius, maxOffset, overstrokeOuterRadius, 0.0f, color);
            }
    ...
    SkString onDumpInfo() const override
    {
        ...
        for (int i = 0; i < fRRects.size(); ++i) {
            string.appendf("Color: 0x%08x Rect [L: %.2f, T: %.2f, R: %.2f, B: %.2f],"
                           "InnerRad: %.2f, OuterRad: %.2f\n",
                fRRects[i].fColor.toBytes_RGBA(), fRRects[i].fDevBounds.fLeft, fRRects[i].fDevBounds.fTop, fRRects[i].fDevBounds.fRight,
                fRRects[i].fDevBounds.fBottom, fRRects[i].fInnerRadius, fRRects[i].fOuterRadius);
        }
        string += fHelper.dumpInfo();
        return string;
    }
...
static sk_sp<const GrBuffer> get_rrect_index_buffer(RRectType type, GrResourceProvider* resourceProvider)
{
    ...
    case kFill_RRectType:
        return resourceProvider->findOrCreatePatternedIndexBuffer(
            gStandardRRectIndices, kIndicesPerFillRRect, kNumRRectsInIndexBuffer, kVertsPerStandardRRect, gRRectOnlyIndexBufferKey);
    case kStroke_RRectType:
        return resourceProvider->findOrCreatePatternedIndexBuffer(
            gStandardRRectIndices, kIndicesPerStrokeRRect, kNumRRectsInIndexBuffer, kVertsPerStandardRRect, gStrokeRRectOnlyIndexBufferKey);
    default:
        SkASSERT(false);
        return nullptr;
    }
...
public:
    ...
    // whether the rrect is only stroked or stroked and filled.
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& devRect, float devXRadius, float devYRadius,
        SkVector devStrokeWidths, bool strokeOnly)
    {
        ...
        if (devStrokeWidths.fX > 0) {
            ...
            // we only handle thick strokes for near-circular ellipses
            if (devStrokeWidths.length() > SK_ScalarHalf && (SK_ScalarHalf * devXRadius > devYRadius || SK_ScalarHalf * devYRadius > devXRadius)) {
                return nullptr;
            }

            // we don't handle it if curvature of the stroke is less than curvature of the ellipse
            if (devStrokeWidths.fX * (devYRadius * devYRadius) < (devStrokeWidths.fY * devStrokeWidths.fY) * devXRadius) {
                return nullptr;
            }
            if (devStrokeWidths.fY * (devXRadius * devXRadius) < (devStrokeWidths.fX * devStrokeWidths.fX) * devYRadius) {
                return nullptr;
            }
        }
        return Helper::FactoryHelper<EllipticalRRectOp>(context, std::move(paint), viewMatrix, devRect, devXRadius, devYRadius, devStrokeWidths, strokeOnly);
    }

    EllipticalRRectOp(GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMatrix& viewMatrix, const SkRect& devRect, float devXRadius,
...
private:
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        sk_sp<const GrBuffer> indexBuffer = get_rrect_index_buffer(fStroked ? kStroke_RRectType : kFill_RRectType, target->resourceProvider());

        if (!indexBuffer) {
            SkDebugf("Could not allocate indices\n");
            return;
        }
        PatternHelper helper(target, GrPrimitiveType::kTriangles, fProgramInfo->geomProc().vertexStride(), std::move(indexBuffer), kVertsPerStandardRRect,
            indicesPerInstance, fRRects.size(), kNumRRectsInIndexBuffer);
        VertexWriter verts { helper.vertices() };
        if (!verts) {
            SkDebugf("Could not allocate vertices\n");
            return;
        ...
        for (const auto& rrect : fRRects) {
            ...
            float reciprocalRadii[4] = { SkScalarInvert(rrect.fXRadius), SkScalarInvert(rrect.fYRadius),
                ...
                sk_ieee_float_divide(1.0f, rrect.fInnerXRadius), sk_ieee_float_divide(1.0f, rrect.fInnerYRadius) };
            ...
            SkScalar yCoords[4] = { bounds.fTop, bounds.fTop + yOuterRadius, bounds.fBottom - yOuterRadius, bounds.fBottom };
            ...
            for (int i = 0; i < 4; ++i) {
                verts << bounds.fLeft << yCoords[i] << color << xMaxOffset << yOuterOffsets[i] << maybeScale << reciprocalRadii;

                verts << (bounds.fLeft + xOuterRadius) << yCoords[i] << color << SK_ScalarNearlyZero << yOuterOffsets[i] << maybeScale << reciprocalRadii;

                verts << (bounds.fRight - xOuterRadius) << yCoords[i] << color << SK_ScalarNearlyZero << yOuterOffsets[i] << maybeScale << reciprocalRadii;

                verts << bounds.fRight << yCoords[i] << color << xMaxOffset << yOuterOffsets[i] << maybeScale << reciprocalRadii;
            }
        }
        fMesh = helper.mesh();
    }
...
GrOp::Owner GrOvalOpFactory::MakeCircularRRectOp(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRRect& rrect, const SkStrokeRec& stroke, const GrShaderCaps* shaderCaps)
{
    ...
    SkScalar scaledRadius = SkScalarAbs(radius * (viewMatrix[SkMatrix::kMScaleX] + viewMatrix[SkMatrix::kMSkewY]));
    ...
    return CircularRRectOp::Make(context, std::move(paint), viewMatrix, bounds, scaledRadius, scaledStroke, isStrokeOnly);
}
...
GrOp::Owner make_rrect_op(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRRect& rrect, const SkStrokeRec& stroke)
{
    ...
    return EllipticalRRectOp::Make(context, std::move(paint), viewMatrix, bounds, xRadius, yRadius, scaledStroke, isStrokeOnly);
}
...
{
    if (rrect.isOval()) {
        return MakeOvalOp(context, std::move(paint), viewMatrix, rrect.getBounds(), GrStyle(stroke, nullptr), shaderCaps);
    }
...
GrOp::Owner GrOvalOpFactory::MakeCircleOp(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& oval, const GrStyle& style, const GrShaderCaps* shaderCaps)
{
    SkScalar width = oval.width();
    SkASSERT(width > SK_ScalarNearlyZero && SkScalarNearlyEqual(width, oval.height()) && circle_stays_circle(viewMatrix));
    ...
    if (style.hasNonDashPathEffect()) {
        ...
        if (style.strokeRec().getCap() != SkPaint::kButt_Cap || style.dashIntervalCnt() != 2 || style.strokeRec().getWidth() >= width) {
            return nullptr;
        }
        ...
        return ButtCapDashedCircleOp::Make(
            context, std::move(paint), viewMatrix, center, r, style.strokeRec().getWidth(), kStartAngle, angularOnInterval, angularOffInterval, phaseAngle);
    }
...
GrOp::Owner GrOvalOpFactory::MakeArcOp(GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& oval, SkScalar startAngle,
    SkScalar sweepAngle, bool useCenter, const GrStyle& style, const GrShaderCaps* shaderCaps)
{
    ...
    SkPoint center = { oval.centerX(), oval.centerY() };
    CircleOp::ArcParams arcParams = { SkDegreesToRadians(startAngle), SkDegreesToRadians(sweepAngle), useCenter };
    return CircleOp::Make(context, std::move(paint), viewMatrix, center, width / 2.f, style, &arcParams);
}
...
GR_DRAW_OP_TEST_DEFINE(CircleOp)
{
    ...
    do {
        ...
        GrOp::Owner op = CircleOp::Make(context, std::move(paint), viewMatrix, center, radius, GrStyle(stroke, nullptr), arcParams);
        if (op) {
            return op;
        }
        assert_alive(paint);
...
GR_DRAW_OP_TEST_DEFINE(ButtCapDashedCircleOp)
{
    ...
    return ButtCapDashedCircleOp::Make(context, std::move(paint), viewMatrix, center, radius, strokeWidth, startAngle, onAngle, offAngle, phase);
}
...
GR_DRAW_OP_TEST_DEFINE(CircularRRectOp)
{
    do {
        ...
        GrOp::Owner op = GrOvalOpFactory::MakeCircularRRectOp(context, std::move(paint), viewMatrix, rrect, GrTest::TestStrokeRec(random), nullptr);
        if (op) {
            return op;
        }
        assert_alive(paint);
