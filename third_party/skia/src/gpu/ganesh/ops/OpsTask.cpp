...
GrOpsRenderPass* create_render_pass(GrGpu* gpu, GrRenderTarget* rt, bool useMSAASurface, GrAttachment* stencil, GrSurfaceOrigin origin, const SkIRect& bounds,
    GrLoadOp colorLoadOp, const std::array<float, 4>& loadClearColor, GrLoadOp stencilLoadOp, GrStoreOp stencilStoreOp,
    const TArray<GrSurfaceProxy*, true>& sampledProxies, GrXferBarrierFlags renderPassXferBarriers)
{
    const GrOpsRenderPass::LoadAndStoreInfo kColorLoadStoreInfo { colorLoadOp, GrStoreOp::kStore, loadClearColor };
    ...
    return gpu->getOpsRenderPass(
        rt, useMSAASurface, stencil, origin, bounds, kColorLoadStoreInfo, stencilLoadAndStoreInfo, sampledProxies, renderPassXferBarriers);
}
...
OpsTask::OpChain::OpChain(GrOp::Owner op, GrProcessorSet::Analysis processorAnalysis, GrAppliedClip* appliedClip, const GrDstProxyView* dstProxyView)
    : fList { std::move(op) }
...
OpsTask::OpChain::List OpsTask::OpChain::DoConcat(List chainA, List chainB, const GrCaps& caps, SkArenaAlloc* opsTaskArena, GrAuditTrail* auditTrail)
{
    ...
    do {
        ...
        while (a) {
            ...
            if (canForwardMerge || canBackwardMerge) {
                ...
                GrOP_INFO("\t\t: (%s opID: %u) -> Combining with (%s, opID: %u)\n", chainB.head()->name(), chainB.head()->uniqueID(), a->name(), a->uniqueID());
            }
            if (merged) {
...
bool OpsTask::OpChain::tryConcat(List* list, GrProcessorSet::Analysis processorAnalysis, const GrDstProxyView& dstProxyView, const GrAppliedClip* appliedClip,
    const SkRect& bounds, const GrCaps& caps, SkArenaAlloc* opsTaskArena, GrAuditTrail* auditTrail)
{
    ...
    if (fList.head()->classID() != list->head()->classID() || SkToBool(fAppliedClip) != SkToBool(appliedClip) || (fAppliedClip && *fAppliedClip != *appliedClip)
    ...
    SkDEBUGCODE(bool first = true;) do
    {
        ...
        case GrOp::CombineResult::kMayChain:
            fList = DoConcat(std::move(fList), std::exchange(*list, List()), caps, opsTaskArena, auditTrail);
...
bool OpsTask::OpChain::prependChain(OpChain* that, const GrCaps& caps, SkArenaAlloc* opsTaskArena, GrAuditTrail* auditTrail)
{
    if (!that->tryConcat(&fList, fProcessorAnalysis, fDstProxyView, fAppliedClip, fBounds, caps, opsTaskArena, auditTrail)) {
        this->validate();
        // append failed
        return false;
    }
...
GrOp::Owner OpsTask::OpChain::appendOp(GrOp::Owner op, GrProcessorSet::Analysis processorAnalysis, const GrDstProxyView* dstProxyView,
    const GrAppliedClip* appliedClip, const GrCaps& caps, SkArenaAlloc* opsTaskArena, GrAuditTrail* auditTrail)
{
    ...
    if (!this->tryConcat(&chain, processorAnalysis, *dstProxyView, appliedClip, opBounds, caps, opsTaskArena, auditTrail)) {
...
inline void OpsTask::OpChain::validate() const
{
    ...
    for (const auto& op : GrOp::ChainRange<>(fList.head())) {
        ...
        SkASSERT(fBounds.fLeft <= op.bounds().fLeft && fBounds.fTop <= op.bounds().fTop && fBounds.fRight >= op.bounds().fRight
            && fBounds.fBottom >= op.bounds().fBottom);
    }
#endif
}
...
OpsTask::OpsTask(GrDrawingManager* drawingMgr, GrSurfaceProxyView view, GrAuditTrail* auditTrail, sk_sp<GrArenas> arenas)
    : GrRenderTask()
...
void OpsTask::addOp(GrDrawingManager* drawingMgr, GrOp::Owner op, GrTextureResolveManager textureResolveManager, const GrCaps& caps)
{
    auto addDependency = [&](GrSurfaceProxy* p, skgpu::Mipmapped mipmapped) { this->addDependency(drawingMgr, p, mipmapped, textureResolveManager, caps); };

    op->visitProxies(addDependency);

    this->recordOp(std::move(op), false /*usesMSAA*/, GrProcessorSet::EmptySetAnalysis(), nullptr, nullptr, caps);
}

void OpsTask::addDrawOp(GrDrawingManager* drawingMgr, GrOp::Owner op, bool usesMSAA, const GrProcessorSet::Analysis& processorAnalysis, GrAppliedClip&& clip,
    const GrDstProxyView& dstProxyView, GrTextureResolveManager textureResolveManager, const GrCaps& caps)
{
    ...
    if (dstProxyView.proxy()) {
        ...
        SkASSERT(!(dstProxyView.dstSampleFlags() & GrDstSampleFlags::kAsInputAttachment) || dstProxyView.offset().isZero());
    }
    ...
    this->recordOp(std::move(op), usesMSAA, processorAnalysis, clip.doesClip() ? &clip : nullptr, &dstProxyView, caps);
}
...
void OpsTask::onPrePrepare(GrRecordingContext* context)
{
    ...
    for (const auto& chain : fOpChains) {
        if (chain.shouldExecute()) {
            chain.head()->prePrepare(context, dstView, chain.appliedClip(), chain.dstProxyView(), fRenderPassXferBarriers, fColorLoadOp);
        }
    }
}
    ...
    for (const auto& chain : fOpChains) {
        if (chain.shouldExecute()) {
            GrOpFlushState::OpArgs opArgs(
                chain.head(), dstView, fUsesMSAASurface, chain.appliedClip(), chain.dstProxyView(), fRenderPassXferBarriers, fColorLoadOp);
...
bool OpsTask::onExecute(GrOpFlushState* flushState)
{
    ...
    SkASSERT(fColorLoadOp != GrLoadOp::kClear || !flushState->gpu()->caps()->performColorClearsAsDraws());
    ...
    GrStoreOp stencilStoreOp = (caps.discardStencilValuesAfterRenderPass() && !fMustPreserveStencil) ? GrStoreOp::kDiscard : GrStoreOp::kStore;

    GrOpsRenderPass* renderPass = create_render_pass(flushState->gpu(), proxy->peekRenderTarget(), fUsesMSAASurface, stencil, fTargetOrigin,
        fClippedContentBounds, fColorLoadOp, fLoadClearColor, stencilLoadOp, stencilStoreOp, fSampledProxies, fRenderPassXferBarriers);
...
{
    return this->target(0) == opsTask->target(0) && fArenas == opsTask->fArenas && !opsTask->fCannotMergeBackward;
}
...
int OpsTask::mergeFrom(SkSpan<const sk_sp<GrRenderTask>> tasks)
{
    ...
    SkSpan<const sk_sp<OpsTask>> mergingNodes(reinterpret_cast<const sk_sp<OpsTask>*>(tasks.data()), SkToSizeT(mergedCount));
    ...
    for (const auto& toMerge : mergingNodes) {
        ...
        fDeferredProxies.move_back_n(toMerge->fDeferredProxies.size(), toMerge->fDeferredProxies.data());
        fSampledProxies.move_back_n(toMerge->fSampledProxies.size(), toMerge->fSampledProxies.data());
        fOpChains.move_back_n(toMerge->fOpChains.size(), toMerge->fOpChains.data());
...
void OpsTask::dump(const SkString& label, SkString indent, bool printDependencies, bool close) const
{
    ...
    case GrLoadOp::kClear:
        SkDebugf("kClear {%g, %g, %g, %g}\n", fLoadClearColor[0], fLoadClearColor[1], fLoadClearColor[2], fLoadClearColor[3]);
        break;
    ...
    for (int i = 0; i < fOpChains.size(); ++i) {
        ...
        if (!fOpChains[i].head()) {
            ...
            SkDebugf("%sClippedBounds: [L: %.2f, T: %.2f, R: %.2f, B: %.2f]\n", indent.c_str(), bounds.fLeft, bounds.fTop, bounds.fRight, bounds.fBottom);
...
{
    auto textureFunc = [func](GrSurfaceProxy* tex, skgpu::Mipmapped mipmapped) { func(tex, mipmapped); };
...
void OpsTask::gatherProxyIntervals(GrResourceAllocator* alloc) const
{
    ...
    for (int i = 0; i < fDeferredProxies.size(); ++i) {
        ...
        alloc->addInterval(fDeferredProxies[i], 0, 0, GrResourceAllocator::ActualUse::kNo, GrResourceAllocator::AllowRecycling::kYes);
    }
    ...
    if (!fOpChains.empty()) {
        ...
        alloc->addInterval(targetProxy, cur, cur + fOpChains.size() - 1, GrResourceAllocator::ActualUse::kYes, GrResourceAllocator::AllowRecycling::kYes);
    } else {
        // This can happen if there is a loadOp (e.g., a clear) but no other draws. In this case we
        // still need to add an interval for the destination so we create a fake op# for
        // the missing clear op.
        alloc->addInterval(targetProxy, alloc->curOp(), alloc->curOp(), GrResourceAllocator::ActualUse::kYes, GrResourceAllocator::AllowRecycling::kYes);
        alloc->incOps();
    }

    GrResourceAllocator::AllowRecycling allowRecycling
        = targetProxy->wrapsVkSecondaryCB() ? GrResourceAllocator::AllowRecycling::kNo : GrResourceAllocator::AllowRecycling::kYes;

    auto gather = [alloc, allowRecycling SkDEBUGCODE(, this)](GrSurfaceProxy* p, skgpu::Mipmapped) {
        alloc->addInterval(p, alloc->curOp(), alloc->curOp(), GrResourceAllocator::ActualUse::kYes, allowRecycling SkDEBUGCODE(, this->target(0) == p));
    };
...
void OpsTask::recordOp(
    GrOp::Owner op, bool usesMSAA, GrProcessorSet::Analysis processorAnalysis, GrAppliedClip* clip, const GrDstProxyView* dstProxyView, const GrCaps& caps)
{
    GrSurfaceProxy* proxy = this->target(0);
#ifdef SK_DEBUG
    op->validate();
    ...
    GrOP_INFO("opsTask: %d Recording (%s, opID: %u)\n"
        ...
        this->uniqueID(), op->name(), op->uniqueID(), op->bounds().fLeft, op->bounds().fTop, op->bounds().fRight, op->bounds().fBottom);
    ...
    if (maxCandidates) {
        ...
        while (true) {
            ...
            op = candidate.appendOp(std::move(op), processorAnalysis, dstProxyView, clip, caps, fArenas->arenaAlloc(), fAuditTrail);
            if (!op) {
                return;
            }
        ...
        while (true) {
            ...
            if (!can_reorder(chain.bounds(), candidate.bounds())) {
                GrOP_INFO("\t\t%d: chain (%s head opID: %u) -> "
                    ...
                    i, chain.head()->name(), chain.head()->uniqueID(), candidate.head()->name(), candidate.head()->uniqueID());
                break;
            }
...
GrRenderTask::ExpectedOutcome OpsTask::onMakeClosed(GrRecordingContext* rContext, SkIRect* targetUpdateBounds)
{
    ...
    if (!this->isColorNoOp()) {
        ...
        if (clippedContentBounds.intersect(fTotalBounds)) {
            ...
            *targetUpdateBounds = GrNativeRect::MakeIRectRelativeTo(fTargetOrigin, this->target(0)->backingStoreDimensions().height(), fClippedContentBounds);
