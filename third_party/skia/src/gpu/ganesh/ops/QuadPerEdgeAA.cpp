...
void write_quad_generic(VertexWriter* vb, const VertexSpec& spec, const GrQuad* deviceQuad, const GrQuad* localQuad, const float coverage[4],
    const SkPMColor4f& color, const SkRect& geomSubset, const SkRect& texSubset)
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        *vb << deviceQuad->x(i) << deviceQuad->y(i) << If(spec.deviceQuadType() == GrQuad::Type::kPerspective, deviceQuad->w(i))
        ...
        if (spec.hasLocalCoords()) {
            *vb << localQuad->x(i) << localQuad->y(i) << If(spec.localQuadType() == GrQuad::Type::kPerspective, localQuad->w(i));
        }
...
void write_2d_color(VertexWriter* vb, const VertexSpec& spec, const GrQuad* deviceQuad, const GrQuad* localQuad, const float coverage[4],
    const SkPMColor4f& color, const SkRect& geomSubset, const SkRect& texSubset)
{
    // Assert assumptions about VertexSpec
    SkASSERT(spec.deviceQuadType() != GrQuad::Type::kPerspective);
    SkASSERT(!spec.hasLocalCoords());
    SkASSERT(spec.coverageMode() == CoverageMode::kNone || spec.coverageMode() == CoverageMode::kWithColor);
...
void write_2d_uv(VertexWriter* vb, const VertexSpec& spec, const GrQuad* deviceQuad, const GrQuad* localQuad, const float coverage[4], const SkPMColor4f& color,
...
void write_2d_color_uv(VertexWriter* vb, const VertexSpec& spec, const GrQuad* deviceQuad, const GrQuad* localQuad, const float coverage[4],
    const SkPMColor4f& color, const SkRect& geomSubset, const SkRect& texSubset)
{
    // Assert assumptions about VertexSpec
    SkASSERT(spec.deviceQuadType() != GrQuad::Type::kPerspective);
    ...
    for (int i = 0; i < 4; ++i) {
        // If this is not coverage-with-alpha, make sure coverage == 1 so it doesn't do anything
        SkASSERT(spec.coverageMode() == CoverageMode::kWithColor || coverage[i] == 1.f);
        *vb << deviceQuad->x(i) << deviceQuad->y(i) << VertexColor(color * coverage[i], wide) << localQuad->x(i) << localQuad->y(i);
    }
}

// 2D (XY), explicit coverage, UV locals, no color, no geometry subset, no texture subset
// This represents opaque, AA, textured rects
void write_2d_cov_uv(VertexWriter* vb, const VertexSpec& spec, const GrQuad* deviceQuad, const GrQuad* localQuad, const float coverage[4],
    const SkPMColor4f& color, const SkRect& geomSubset, const SkRect& texSubset)
{
    // Assert assumptions about VertexSpec
    SkASSERT(spec.deviceQuadType() != GrQuad::Type::kPerspective);
...
void write_2d_uv_strict(VertexWriter* vb, const VertexSpec& spec, const GrQuad* deviceQuad, const GrQuad* localQuad, const float coverage[4],
    const SkPMColor4f& color, const SkRect& geomSubset, const SkRect& texSubset)
{
    // Assert assumptions about VertexSpec
    SkASSERT(spec.deviceQuadType() != GrQuad::Type::kPerspective);
...
void write_2d_color_uv_strict(VertexWriter* vb, const VertexSpec& spec, const GrQuad* deviceQuad, const GrQuad* localQuad, const float coverage[4],
    const SkPMColor4f& color, const SkRect& geomSubset, const SkRect& texSubset)
{
    // Assert assumptions about VertexSpec
    SkASSERT(spec.deviceQuadType() != GrQuad::Type::kPerspective);
    ...
    for (int i = 0; i < 4; ++i) {
        // If this is not coverage-with-alpha, make sure coverage == 1 so it doesn't do anything
        SkASSERT(spec.coverageMode() == CoverageMode::kWithColor || coverage[i] == 1.f);
        *vb << deviceQuad->x(i) << deviceQuad->y(i) << VertexColor(color * coverage[i], wide) << localQuad->x(i) << localQuad->y(i) << texSubset;
    }
}
...
void write_2d_cov_uv_strict(VertexWriter* vb, const VertexSpec& spec, const GrQuad* deviceQuad, const GrQuad* localQuad, const float coverage[4],
    const SkPMColor4f& color, const SkRect& geomSubset, const SkRect& texSubset)
{
    // Assert assumptions about VertexSpec
    SkASSERT(spec.deviceQuadType() != GrQuad::Type::kPerspective);
    ...
    for (int i = 0; i < 4; ++i) {
        *vb << deviceQuad->x(i) << deviceQuad->y(i) << coverage[i] << localQuad->x(i) << localQuad->y(i) << texSubset;
    }
}
...
void Tessellator::append(GrQuad* deviceQuad, GrQuad* localQuad, const SkPMColor4f& color, const SkRect& uvSubset, GrQuadAAFlags aaFlags)
{
    ...
    if (fVertexSpec.usesCoverageAA()) {
        SkASSERT(fVertexSpec.coverageMode() == CoverageMode::kWithColor || fVertexSpec.coverageMode() == CoverageMode::kWithPosition);
        ...
        if (aaFlags == GrQuadAAFlags::kNone) {
            ...
            fWriteProc(&fVertexWriter, fVertexSpec, deviceQuad, localQuad, kFullCoverage, color, geomSubset, uvSubset);
            ...
            fWriteProc(&fVertexWriter, fVertexSpec, deviceQuad, localQuad, kZeroCoverage, color, geomSubset, uvSubset);
        } else {
            ...
            if (aaFlags == GrQuadAAFlags::kAll) {
                ...
                edgeDistances = { (aaFlags & GrQuadAAFlags::kLeft) ? 0.5f : 0.f, (aaFlags & GrQuadAAFlags::kBottom) ? 0.5f : 0.f,
                    (aaFlags & GrQuadAAFlags::kTop) ? 0.5f : 0.f, (aaFlags & GrQuadAAFlags::kRight) ? 0.5f : 0.f };
            }

            // Write inner vertices first
            float coverage[4];
            fAAHelper.inset(edgeDistances, deviceQuad, localQuad).store(coverage);
            fWriteProc(&fVertexWriter, fVertexSpec, deviceQuad, localQuad, coverage, color, geomSubset, uvSubset);
            ...
            const bool hairline = aaFlags == GrQuadAAFlags::kAll && coverage[0] < 1.f && coverage[1] < 1.f && coverage[2] < 1.f && coverage[3] < 1.f;
            if (hairline) {
                ...
                skvx::Vec<4, float> maxWH = max(skvx::shuffle<1, 0, 3, 2>(len), skvx::shuffle<2, 3, 0, 1>(len));
        ...
        SkASSERT(fVertexSpec.coverageMode() == CoverageMode::kNone && !fVertexSpec.requiresGeometrySubset());
        fWriteProc(&fVertexWriter, fVertexSpec, deviceQuad, localQuad, kFullCoverage, color, kIgnoredSubset, uvSubset);
    }
}
...
int QuadLimit(IndexBufferOption option)
{
    switch (option) {
    case IndexBufferOption::kPictureFramed:
        return GrResourceProvider::MaxNumAAQuads();
    case IndexBufferOption::kIndexedRects:
        return GrResourceProvider::MaxNumNonAAQuads();
    case IndexBufferOption::kTriStrips:
        return SK_MaxS32; // not limited by an indexBuffer
    }
...
void IssueDraw(
    const GrCaps& caps, GrOpsRenderPass* renderPass, const VertexSpec& spec, int runningQuadCount, int quadsInDraw, int maxVerts, int absVertBufferOffset)
{
    if (spec.indexBufferOption() == IndexBufferOption::kTriStrips) {
        int offset = absVertBufferOffset + runningQuadCount * GrResourceProvider::NumVertsPerNonAAQuad();
        renderPass->draw(4, offset);
        return;
    }

    SkASSERT(spec.indexBufferOption() == IndexBufferOption::kPictureFramed || spec.indexBufferOption() == IndexBufferOption::kIndexedRects);
    ...
    if (caps.avoidLargeIndexBufferDraws()) {
        ...
        renderPass->drawIndexPattern(numIndicesPerQuad, quadsInDraw, maxNumQuads, numVertsPerQuad, offset);
    } else {
...
CoverageMode VertexSpec::coverageMode() const
{
    if (this->usesCoverageAA()) {
        if (this->compatibleWithCoverageAsAlpha() && this->hasVertexColors() && !this->requiresGeometrySubset()) {
...
size_t VertexSpec::vertexSize() const
{
    ...
    if (coverageMode == CoverageMode::kWithPosition) {
        if (needsPerspective) {
            ...
            count += GrVertexAttribTypeSize(kFloat2_GrVertexAttribType) + GrVertexAttribTypeSize(kFloat_GrVertexAttribType);
        }
    } else {
        if (needsPerspective) {
...
public:
    ...
    static GrGeometryProcessor* Make(SkArenaAlloc* arena, const VertexSpec& vertexSpec, const GrShaderCaps& caps, const GrBackendFormat& backendFormat,
        GrSamplerState samplerState, const skgpu::Swizzle& swizzle, sk_sp<GrColorSpaceXform> textureColorSpaceXform, Saturate saturate)
    {
        return arena->make([&](void* ptr) {
            return new (ptr)
                QuadPerEdgeAAGeometryProcessor(vertexSpec, caps, backendFormat, samplerState, swizzle, std::move(textureColorSpaceXform), saturate);
        });
    }
    ...
    void addToKey(const GrShaderCaps&, KeyBuilder* b) const override
    {
        ...
        if (fCoverageMode != CoverageMode::kNone) {
            coverageKey = fGeomSubset.isInitialized() ? 0x3 : (CoverageMode::kWithPosition == fCoverageMode ? 0x1 : 0x2);
        }
    ...
    std::unique_ptr<ProgramImpl> makeProgramImpl(const GrShaderCaps&) const override
    {
        ...
        public:
            void setData(const GrGLSLProgramDataManager& pdman, const GrShaderCaps&, const GrGeometryProcessor& geomProc) override
            {
        ...
        private:
            void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
            {
                ...
                fTextureColorSpaceXformHelper.emitCode(args.fUniformHandler, gp.fTextureColorSpaceXform.get());
                ...
                if (gp.fCoverageMode == CoverageMode::kWithPosition) {
                    ...
                    gpArgs->fPositionVar = { "position", gp.fNeedsPerspective ? SkSLType::kFloat3 : SkSLType::kFloat2, GrShaderVar::TypeModifier::None };
                } else {
                ...
                if (gp.fColor.isInitialized()) {
                    ...
                    args.fVaryingHandler->addPassThroughAttribute(gp.fColor.asShaderVar(), args.fOutputColor,
                        gp.fCoverageMode == CoverageMode::kWithColor ? Interpolation::kInterpolated : Interpolation::kCanBeFlat);
                ...
                if (gp.fSampler.isInitialized()) {
                    ...
                    if (gp.fTexSubset.isInitialized()) {
                        ...
                        args.fVaryingHandler->addPassThroughAttribute(gp.fTexSubset.asShaderVar(), "subset", Interpolation::kCanBeFlat);
                    ...
                    args.fFragBuilder->codeAppendf("%s = %s(", args.fOutputColor, (gp.fSaturate == Saturate::kYes) ? "saturate" : "");
                    args.fFragBuilder->appendTextureLookupAndBlend(
                        blendDst, SkBlendMode::kModulate, args.fTexSamplers[0], "texCoord", &fTextureColorSpaceXformHelper);
                ...
                if (gp.fCoverageMode == CoverageMode::kWithPosition) {
                    ...
                    if (gp.fNeedsPerspective) {
                        ...
                        args.fVertBuilder->codeAppendf("%s = %s.w * %s.z;", coverage.vsOut(), gp.fPosition.name(), gp.fPosition.name());
                        args.fFragBuilder->codeAppendf("float coverage = %s * sk_FragCoord.w;", coverage.fsIn());
                    } else {
                    ...
                    if (gp.fGeomSubset.isInitialized()) {
                        ...
                        args.fVaryingHandler->addPassThroughAttribute(gp.fGeomSubset.asShaderVar(), "geoSubset", Interpolation::kCanBeFlat);
                        args.fFragBuilder->codeAppend(
                    ...
                    args.fFragBuilder->codeAppendf("half4 %s = half4(coverage);", args.fOutputCoverage);
                } else {
...
private:
    ...
    QuadPerEdgeAAGeometryProcessor(const VertexSpec& spec, const GrShaderCaps& caps, const GrBackendFormat& backendFormat, GrSamplerState samplerState,
        const skgpu::Swizzle& swizzle, sk_sp<GrColorSpaceXform> textureColorSpaceXform, Saturate saturate)
        : INHERITED(kQuadPerEdgeAAGeometryProcessor_ClassID)
        , fSaturate(saturate)
...
GrGeometryProcessor* MakeTexturedProcessor(SkArenaAlloc* arena, const VertexSpec& spec, const GrShaderCaps& caps, const GrBackendFormat& backendFormat,
    GrSamplerState samplerState, const skgpu::Swizzle& swizzle, sk_sp<GrColorSpaceXform> textureColorSpaceXform, Saturate saturate)
{
    return QuadPerEdgeAAGeometryProcessor::Make(arena, spec, caps, backendFormat, samplerState, swizzle, std::move(textureColorSpaceXform), saturate);
}
