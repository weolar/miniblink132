...
public:
    ...
    void* lock(size_t stride, int eagerCount) override
    {
        ...
        fVertexBuffer = fResourceProvider->createBuffer(size, GrGpuBufferType::kVertex, kStatic_GrAccessPattern, GrResourceProvider::ZeroInit::kNo);
        if (!fVertexBuffer) {
            return nullptr;
        }
        if (fCanMapVB) {
    ...
    void unlock(int actualCount) override
    {
        ...
        fVertexData = GrThreadSafeCache::MakeVertexData(std::move(fVertexBuffer), actualCount, fLockStride);
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const GrStyledShape& shape, const SkMatrix& viewMatrix, SkIRect devClipBounds,
        GrAAType aaType, const GrUserStencilSettings* stencilSettings)
    {
        return Helper::FactoryHelper<TriangulatingPathOp>(context, std::move(paint), shape, viewMatrix, devClipBounds, aaType, stencilSettings);
    }
    ...
    TriangulatingPathOp(GrProcessorSet* processorSet, const SkPMColor4f& color, const GrStyledShape& shape, const SkMatrix& viewMatrix,
        const SkIRect& devClipBounds, GrAAType aaType, const GrUserStencilSettings* stencilSettings)
        : INHERITED(ClassID())
    ...
    {
        GrProcessorAnalysisCoverage coverage = fAntiAlias ? GrProcessorAnalysisCoverage::kSingleChannel : GrProcessorAnalysisCoverage::kNone;
...
private:
    ...
    static void CreateKey(skgpu::UniqueKey* key, const GrStyledShape& shape, const SkIRect& devClipBounds)
    {
    ...
    static int Triangulate(
        GrEagerVertexAllocator* allocator, const SkMatrix& viewMatrix, const GrStyledShape& shape, const SkIRect& devClipBounds, SkScalar tol, bool* isLinear)
    {
    ...
    void createNonAAMesh(GrMeshDrawTarget* target)
    {
        ...
        SkScalar tol = GrPathUtils::scaleToleranceToSrc(GrPathUtils::kDefaultTolerance, fViewMatrix, fShape.bounds());
        ...
        if (fVertexData) {
            if (!fVertexData->gpuBuffer()) {
                sk_sp<GrGpuBuffer> buffer = rp->createBuffer(fVertexData->vertices(), fVertexData->size(), GrGpuBufferType::kVertex, kStatic_GrAccessPattern);
                if (!buffer) {
                    return;
                }
        ...
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        GrGeometryProcessor* gp;
        {
            ...
            if (fAntiAlias) {
                gp = GrDefaultGeoProcFactory::MakeForDeviceSpace(arena, color, coverageType, localCoordsType, fViewMatrix);
            } else {
                gp = GrDefaultGeoProcFactory::Make(arena, color, coverageType, localCoordsType, fViewMatrix);
            }
        }
        if (!gp) {
            return;
        ...
        GrPrimitiveType primitiveType = TRIANGULATOR_WIREFRAME ? GrPrimitiveType::kLines : GrPrimitiveType::kTriangles;

        fProgramInfo = fHelper.createProgramInfoWithStencil(
            caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, gp, primitiveType, renderPassXferBarriers, colorLoadOp);
    }
        ...
        GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
        ...
        if (tmpV != fVertexData) {
            ...
            // replaced a pre-existing one. A duplicated listener is unlikely and not that
            // expensive so we just roll with it.
            fShape.addGenIDChangeListener(sk_make_sp<UniqueKeyInvalidator>(key, rContext->priv().contextID()));
        }
    }
    ...
    static GrSimpleMesh* CreateMesh(GrMeshDrawTarget* target, sk_sp<const GrBuffer> vb, int firstVertex, int count)
    {
...
GR_DRAW_OP_TEST_DEFINE(TriangulatingPathOp)
{
    ...
    SkIRect devClipBounds = SkIRect::MakeLTRB(random->nextU(), random->nextU(), random->nextU(), random->nextU());
    devClipBounds.sort();
    ...
    return TriangulatingPathOp::Make(context, std::move(paint), shape, viewMatrix, devClipBounds, aaType, GrGetRandomStencil(random, context));
}
...
PathRenderer::CanDrawPath TriangulatingPathRenderer::onCanDrawPath(const CanDrawPathArgs& args) const
{
...
{
    GR_AUDIT_TRAIL_AUTO_FRAME(args.fContext->priv().auditTrail(), "GrTriangulatingPathRenderer::onDrawPath");

    GrOp::Owner op = TriangulatingPathOp::Make(
        args.fContext, std::move(args.fPaint), *args.fShape, *args.fViewMatrix, *args.fClipConservativeBounds, args.fAAType, args.fUserStencilSettings);
