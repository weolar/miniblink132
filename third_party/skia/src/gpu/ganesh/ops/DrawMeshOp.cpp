...
{
    switch (type) {
    case MeshAttributeType::kFloat:
        return kFloat_GrVertexAttribType;
    case MeshAttributeType::kFloat2:
        return kFloat2_GrVertexAttribType;
    case MeshAttributeType::kFloat3:
        return kFloat3_GrVertexAttribType;
    case MeshAttributeType::kFloat4:
        return kFloat4_GrVertexAttribType;
    case MeshAttributeType::kUByte4_unorm:
        return kUByte4_norm_GrVertexAttribType;
    }
    SkUNREACHABLE;
}
...
public:
    static GrGeometryProcessor* Make(SkArenaAlloc* arena, sk_sp<SkMeshSpecification> spec, sk_sp<GrColorSpaceXform> colorSpaceXform, const SkMatrix& viewMatrix,
        const std::optional<SkPMColor4f>& color, bool needsLocalCoords, sk_sp<const SkData> uniforms, SkSpan<std::unique_ptr<GrFragmentProcessor>> children)
    {
        return arena->make([&](void* ptr) {
            return new (ptr) MeshGP(std::move(spec), std::move(colorSpaceXform), viewMatrix, std::move(color), needsLocalCoords, std::move(uniforms), children);
        });
    }
    ...
    void addToKey(const GrShaderCaps& caps, skgpu::KeyBuilder* b) const override
    {
        ...
        if (SkMeshSpecificationPriv::GetColorType(*fSpec) != SkMeshSpecificationPriv::ColorType::kNone) {
...
private:
    ...
    public:
        void setData(const GrGLSLProgramDataManager& pdman, const GrShaderCaps& shaderCaps, const GrGeometryProcessor& geomProc) override
        {
            ...
            if (mgp.fUniforms) {
                pdman.setRuntimeEffectUniforms(mgp.fSpec->uniforms(), SkSpan(fSpecUniformHandles), mgp.fUniforms->data());
            }
            ...
            for (size_t index = 0; index < mgp.fChildren.size(); ++index) {
                if (const GrFragmentProcessor* fp = mgp.fChildren[index].get()) {
                    ...
                    fp->visitWithImpls([&](const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl) { impl.setData(pdman, fp); }, *impl);
                }
            }
        }
    ...
    private:
        ...
        public:
            MeshCallbacks(Impl* self, const MeshGP& gp, GrGLSLShaderBuilder* builder, GrGLSLUniformHandler* uniformHandler, const char* mainName,
            ...
            std::string declareUniform(const SkSL::VarDeclaration* decl) override
            {
                ...
                auto it = std::find_if(uniforms.begin(), uniforms.end(),
                    [&name](SkMeshSpecification::Uniform uniform) { return uniform.name == std::string_view(name.c_str(), name.size()); });
                ...
                *handle = fUniformHandler->addUniformArray(&fGP, shaderFlags, gpuType, name.c_str(), isArray ? var->type().columns() : 0, &mangledName);
            ...
            std::string getMainName() override
            {
                return fMainName;
            }
            ...
            std::string sampleShader(int index, std::string coords) override
            {
                ...
                SkASSERT(impl);
                return fBuilder->getProgramBuilder()->invokeFP(*fp, *impl,
                    /*inputColor=*/"half4(0)",
                    /*destColor=*/"half4(1)", coords.c_str());
            }
            ...
            std::string sampleColorFilter(int index, std::string color) override
            {
                ...
                GrFragmentProcessor::ProgramImpl* impl = fSelf->fChildImpls[index].get();
                SkASSERT(impl);
                return fBuilder->getProgramBuilder()->invokeFP(*fp, *impl, color.c_str(),
                    /*destColor=*/"half4(1)",
                    /*coords=*/"float2(0)");
            }
            ...
            std::string sampleBlender(int index, std::string src, std::string dst) override
            {
                ...
                GrFragmentProcessor::ProgramImpl* impl = fSelf->fChildImpls[index].get();
                SkASSERT(impl);
                return fBuilder->getProgramBuilder()->invokeFP(*fp, *impl, src.c_str(), dst.c_str(),
                    /*coords=*/"float2(0)");
            }
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            ...
            SkMeshSpecificationPriv::ColorType meshColorType = SkMeshSpecificationPriv::GetColorType(*mgp.fSpec);
            int passthroughLCVaryingIndex = SkMeshSpecificationPriv::PassthroughLocalCoordsVaryingIndex(*mgp.fSpec);
            ...
            bool needUserFS = (passthroughLCVaryingIndex < 0 && mgp.fNeedsLocalCoords) || meshColorType != SkMeshSpecificationPriv::ColorType::kNone;
            ...
            SkSpan<const SkMeshSpecification::Varying> specVaryings = SkMeshSpecificationPriv::Varyings(*mgp.fSpec);
            ...
            MeshCallbacks vsCallbacks(this, mgp, vertBuilder, uniformHandler, userVertName.c_str(), *customVS->fContext);
            ...
            vertBuilder->codeAppendf("%s attributes;", vsCallbacks.getMangledName("Attributes").c_str());
            {
                ...
                for (auto attr : mgp.vertexAttributes()) {
                    vertBuilder->codeAppendf("attributes.%s = %s;", mgp.fSpec->attributes()[i++].name.c_str(), attr.name());
                }
            }

            // Call the user's vert function.
            vertBuilder->codeAppendf("%s varyings = %s(attributes);", vsCallbacks.getMangledName("Varyings").c_str(), userVertName.c_str());

            if (passthroughLCVaryingIndex >= 0 && SkMeshSpecificationPriv::VaryingIsDead(*mgp.fSpec, passthroughLCVaryingIndex)) {
            ...
            if (needUserFS) {
                for (size_t i = 0; i < specVaryings.size(); ++i) {
                    ...
                    vertBuilder->codeAppendf("%s = varyings.%s;", realVaryings.back().varying.vsOut(), v.name.c_str());
            ...
            // Setup position
            WriteOutputPosition(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, "pos", mgp.fViewMatrix, &fViewMatrixUniform);
                    ...
                    fp->visitWithImpls(
                        [&](const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl) {
                            if (fp.asTextureEffect()) {
                                static_cast<GrTextureEffect::Impl&>(impl).setSamplerHandle(args.fTexSamplers[samplerIndex++]);
                            }
                        },
                        *impl);
            ...
            MeshCallbacks fsCallbacks(this, mgp, fragBuilder, uniformHandler, userFragName.c_str(), *customFS->fContext);
            ...
            if (mgp.fColor != SK_PMColor4fILLEGAL) {
                fColorUniform = uniformHandler->addUniform(nullptr, kFragment_GrShaderFlag, SkSLType::kHalf4, "color", &uniformColorName);
            }
            ...
            if (needUserFS) {
                ...
                for (const auto& rv : realVaryings) {
                    ...
                    fragBuilder->codeAppendf("varyings.%s = %s;", v.name.c_str(), rv.varying.vsOut());
                }
                ...
                if (meshColorType == SkMeshSpecificationPriv::ColorType::kNone) {
                    fragBuilder->codeAppendf("%s%s(varyings);", local.c_str(), userFragName.c_str());
                } else {
                    fColorSpaceHelper.emitCode(uniformHandler, mgp.fColorSpaceXform.get(), kFragment_GrShaderFlag);
                    ...
                    fragBuilder->codeAppendf("%s%s(varyings, color);", local.c_str(), userFragName.c_str());
            ...
            SkASSERT(!mgp.fNeedsLocalCoords || gpArgs->fLocalCoordVar.getType() == SkSLType::kFloat2);
        }
    ...
    MeshGP(sk_sp<SkMeshSpecification> spec, sk_sp<GrColorSpaceXform> colorSpaceXform, const SkMatrix& viewMatrix, const std::optional<SkPMColor4f>& color,
        bool needsLocalCoords, sk_sp<const SkData> uniforms, SkSpan<std::unique_ptr<GrFragmentProcessor>> children)
        ...
        , fNeedsLocalCoords(needsLocalCoords)
    {
        ...
        for (const auto& srcAttr : fSpec->attributes()) {
            fAttributes.emplace_back(srcAttr.name.c_str(), attrib_type(srcAttr.type), SkMeshSpecificationPriv::AttrTypeAsSLType(srcAttr.type), srcAttr.offset);
        }
        ...
        for (const std::unique_ptr<GrFragmentProcessor>& fp : fChildren) {
            if (fp) {
                fp->visitTextureEffects([&](const GrTextureEffect& te) {
                    fTextureSamplers.push_back({ te.samplerState(), te.view().proxy()->backendFormat(), te.view().swizzle() });
                });
            }
        }
...
public:
    ...
    MeshOp(GrProcessorSet*, const SkPMColor4f&, const SkMesh&, TArray<std::unique_ptr<GrFragmentProcessor>> children, GrAAType, sk_sp<GrColorSpaceXform>,
        const SkMatrix&);

    MeshOp(GrProcessorSet*, const SkPMColor4f&, sk_sp<SkVertices>, const GrPrimitiveType*, GrAAType, sk_sp<GrColorSpaceXform>, const SkMatrix&);
    ...
    void visitProxies(const GrVisitProxyFunc& func) const override
    {
        for (const std::unique_ptr<GrFragmentProcessor>& fp : fChildren) {
            if (fp) {
                fp->visitTextureEffects([&](const GrTextureEffect& te) { func(te.view().proxy(), te.view().mipmapped()); });
            }
        }
        if (fProgramInfo) {
...
private:
    ...
    public:
        ...
        void writeVertices(skgpu::VertexWriter& writer, const SkMeshSpecification& spec, bool transform) const;
...
void MeshOp::Mesh::writeVertices(skgpu::VertexWriter& writer, const SkMeshSpecification& spec, bool transform) const
{
...
MeshOp::MeshOp(GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMesh& mesh, TArray<std::unique_ptr<GrFragmentProcessor>> children,
    GrAAType aaType, sk_sp<GrColorSpaceXform> colorSpaceXform, const SkMatrix& viewMatrix)
    : INHERITED(ClassID())
    ...
    , fViewMatrix(viewMatrix)
{
    ...
    fUniforms = SkRuntimeEffectPriv::TransformUniforms(mesh.spec()->uniforms(), mesh.refUniforms(), mesh.spec()->colorSpace());
...
static SkMeshSpecification* make_vertices_spec(bool hasColors, bool hasTex)
{
    ...
    vs += "v.position = a.pos;\nreturn v;\n}";
    fs += "}";
    auto [spec, error] = SkMeshSpecification::Make(SkSpan(attributes), size, SkSpan(varyings), vs, fs);
    SkASSERT(spec);
    return spec.release();
}

MeshOp::MeshOp(GrProcessorSet* processorSet, const SkPMColor4f& color, sk_sp<SkVertices> vertices, const GrPrimitiveType* overridePrimitiveType,
    GrAAType aaType, sk_sp<GrColorSpaceXform> colorSpaceXform, const SkMatrix& viewMatrix)
    : INHERITED(ClassID())
    , fHelper(processorSet, aaType)
    , fColorSpaceXform(std::move(colorSpaceXform))
    ...
    , fViewMatrix(viewMatrix)
{
    int attrs = (vertices->priv().hasColors() ? 0b01 : 0b00) | (vertices->priv().hasTexCoords() ? 0b10 : 0b00);
    switch (attrs) {
    case 0b00: {
...
#if defined(GPU_TEST_UTILS)
SkString MeshOp::onDumpInfo() const
{
    return {};
}
#endif
...
GrProcessorSet::Analysis MeshOp::finalize(const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType)
{
    GrProcessorAnalysisColor gpColor;
    gpColor.setToUnknown();
    auto result = fHelper.finalizeProcessors(caps, clip, clampType, GrProcessorAnalysisCoverage::kNone, &gpColor);
...
GrGeometryProcessor* MeshOp::makeGP(SkArenaAlloc* arena)
{
    ...
    return MeshGP::Make(arena, fSpecification, fColorSpaceXform, vm, color, fHelper.usesLocalCoords(), fUniforms, SkSpan(fChildren));
}

void MeshOp::onCreateProgramInfo(const GrCaps* caps, SkArenaAlloc* arena, const GrSurfaceProxyView& writeView, bool usesMSAASurface,
...
{
    fProgramInfo = fHelper.createProgramInfo(caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, this->makeGP(arena), fPrimitiveType,
...
void MeshOp::onPrepareDraws(GrMeshDrawTarget* target)
{
    ...
    if (!vertexBuffer) {
        skgpu::VertexWriter verts = target->makeVertexWriter(vertexStride, fVertexCount, &vertexBuffer, &firstVertex);
        if (!verts) {
    ...
    if (indexBuffer) {
        fMesh->setIndexed(std::move(indexBuffer), fIndexCount, firstIndex,
            /*minIndexValue=*/0, fVertexCount - 1, GrPrimitiveRestart::kNo, std::move(vertexBuffer), firstVertex);
    } else {
...
void MeshOp::onExecute(GrOpFlushState* flushState, const SkRect& chainBounds)
{
    ...
    for (const std::unique_ptr<GrFragmentProcessor>& fp : fChildren) {
        if (fp) {
            fp->visitTextureEffects([&](const GrTextureEffect& te) { geomProcTextures.push_back(te.view().proxy()); });
        }
    }

    flushState->bindPipelineAndScissorClip(*fProgramInfo, chainBounds);
    flushState->bindTextures(fProgramInfo->geomProc(), geomProcTextures.data(), fProgramInfo->pipeline());
...
GrOp::CombineResult MeshOp::onCombineIfPossible(GrOp* t, SkArenaAlloc*, const GrCaps& caps)
{
    ...
    if (!(fPrimitiveType == GrPrimitiveType::kTriangles || fPrimitiveType == GrPrimitiveType::kLines || fPrimitiveType == GrPrimitiveType::kPoints)) {
    ...
    if (SkMeshSpecificationPriv::Hash(*this->fSpecification) != SkMeshSpecificationPriv::Hash(*that->fSpecification)) {
        return CombineResult::kCannotCombine;
    }
    ...
    if (fViewMatrix != that->fViewMatrix) {
        ...
        if ((this->fViewMatrix.isFinite() && this->fViewMatrix.hasPerspective()) || (that->fViewMatrix.isFinite() && that->fViewMatrix.hasPerspective())) {
            return CombineResult::kCannotCombine;
        }
...
GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkMesh& mesh, TArray<std::unique_ptr<GrFragmentProcessor>> children,
    const SkMatrix& viewMatrix, GrAAType aaType, sk_sp<GrColorSpaceXform> colorSpaceXform)
{
    return GrSimpleMeshDrawOpHelper::FactoryHelper<MeshOp>(
        context, std::move(paint), mesh, std::move(children), aaType, std::move(colorSpaceXform), viewMatrix);
}

GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, sk_sp<SkVertices> vertices, const GrPrimitiveType* overridePrimitiveType,
    const SkMatrix& viewMatrix, GrAAType aaType, sk_sp<GrColorSpaceXform> colorSpaceXform)
{
    return GrSimpleMeshDrawOpHelper::FactoryHelper<MeshOp>(
        context, std::move(paint), std::move(vertices), overridePrimitiveType, aaType, std::move(colorSpaceXform), viewMatrix);
}
