...
public:
    ...
    template <typename Op, typename... Args> static Owner Make(GrRecordingContext* context, Args&&... args)
    {
    ...
    template <typename Op, typename... Args> static Owner MakeWithExtraMemory(GrRecordingContext* context, size_t extraSize, Args&&... args)
    {
     ...
     */
    void prePrepare(GrRecordingContext* context, const GrSurfaceProxyView& dstView, GrAppliedClip* clip, const GrDstProxyView& dstProxyView,
    ...
    virtual SkString dumpInfo() const final
    {
        return SkStringPrintf(
            "%s\nOpBounds: [L: %.2f, T: %.2f, R: %.2f, B: %.2f]", this->onDumpInfo().c_str(), fBounds.fLeft, fBounds.fTop, fBounds.fRight, fBounds.fBottom);
    }
#endif
...
private:
    ...
    virtual void onPrePrepare(GrRecordingContext*, const GrSurfaceProxyView& writeView, GrAppliedClip*, const GrDstProxyView&,
        GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp)
        = 0;
...
#if defined(GPU_TEST_UTILS)
    virtual SkString onDumpInfo() const
    {
        return SkString();
    }
#endif
    ...
    enum BoundsFlags { kAABloat_BoundsFlag = 0x1, kZeroArea_BoundsFlag = 0x2, SkDEBUGCODE(kUninitialized_BoundsFlag = 0x4) };
