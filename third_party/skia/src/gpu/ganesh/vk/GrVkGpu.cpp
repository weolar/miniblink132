...
{
    if (backendContext.fInstance == VK_NULL_HANDLE || backendContext.fPhysicalDevice == VK_NULL_HANDLE || backendContext.fDevice == VK_NULL_HANDLE
    ...
    sk_sp<const skgpu::VulkanInterface> interface = skgpu::MakeInterface(backendContext, extensions, &instanceVersion, &physDevVersion);
    if (!interface) {
        return nullptr;
    }
    ...
    if (backendContext.fDeviceFeatures2) {
        caps.reset(new GrVkCaps(options, interface.get(), backendContext.fPhysicalDevice, *backendContext.fDeviceFeatures2, instanceVersion, physDevVersion,
            *extensions, backendContext.fProtectedContext));
    } else if (backendContext.fDeviceFeatures) {
        VkPhysicalDeviceFeatures2 features2;
        features2.pNext = nullptr;
        features2.features = *backendContext.fDeviceFeatures;
        caps.reset(new GrVkCaps(options, interface.get(), backendContext.fPhysicalDevice, features2, instanceVersion, physDevVersion, *extensions,
            backendContext.fProtectedContext));
    } else {
        VkPhysicalDeviceFeatures2 features;
        memset(&features, 0, sizeof(VkPhysicalDeviceFeatures2));
        caps.reset(new GrVkCaps(options, interface.get(), backendContext.fPhysicalDevice, features, instanceVersion, physDevVersion, *extensions,
            backendContext.fProtectedContext));
    }
    ...
    if (!memoryAllocator) {
        ...
        memoryAllocator = skgpu::VulkanMemoryAllocators::Make(backendContext, skgpu::ThreadSafe::kNo, options.fVulkanVMALargeHeapBlockSize);
    }
#endif
    if (!memoryAllocator) {
    ...
    std::unique_ptr<GrVkGpu> vkGpu(
        new GrVkGpu(direct, backendContext, std::move(caps), interface, instanceVersion, physDevVersion, std::move(memoryAllocator)));
    if (backendContext.fProtectedContext == GrProtected::kYes && !vkGpu->vkCaps().supportsProtectedContent()) {
        return nullptr;
    }
    return vkGpu;
}
...
GrVkGpu::GrVkGpu(GrDirectContext* direct, const skgpu::VulkanBackendContext& backendContext, sk_sp<GrVkCaps> caps,
    sk_sp<const skgpu::VulkanInterface> interface, uint32_t instanceVersion, uint32_t physicalDeviceVersion,
...
GrOpsRenderPass* GrVkGpu::onGetOpsRenderPass(GrRenderTarget* rt, bool useMSAASurface, GrAttachment* stencil, GrSurfaceOrigin origin, const SkIRect& bounds,
    ...
    const TArray<GrSurfaceProxy*, true>& sampledProxies, GrXferBarrierFlags renderPassXferBarriers)
{
    ...
    SkASSERT(!useMSAASurface || rt->numSamples() > 1
        || (this->vkCaps().supportsDiscardableMSAAForDMSAA() && vkRT->resolveAttachment() && vkRT->resolveAttachment()->supportsInputAttachmentUsage()));
    ...
    if (!fCachedOpsRenderPass->set(
            rt, std::move(framebuffer), origin, bounds, localColorInfo, stencilInfo, resolveInfo, selfDepFlags, loadFromResolve, sampledProxies)) {
        return nullptr;
    }
...
bool GrVkGpu::submitCommandBuffer(const GrSubmitInfo& submitInfo)
{
    ...
    if (!this->currentCommandBuffer()->hasWork() && submitInfo.fSync == GrSyncCpu::kNo && fSemaphoresToSignal.empty() && fSemaphoresToWaitOn.empty()) {
    ...
    fMainCmdPool->close();
    bool didSubmit = fMainCmdBuffer->submitToQueue(this, fQueue, fSemaphoresToSignal, fSemaphoresToWaitOn, submitInfo);
...
///////////////////////////////////////////////////////////////////////////////
sk_sp<GrGpuBuffer> GrVkGpu::onCreateBuffer(size_t size, GrGpuBufferType type, GrAccessPattern accessPattern)
{
#ifdef SK_DEBUG
    switch (type) {
    ...
    case GrGpuBufferType::kDrawIndirect:
        SkASSERT(accessPattern == kDynamic_GrAccessPattern || accessPattern == kStatic_GrAccessPattern);
        break;
    case GrGpuBufferType::kXferCpuToGpu:
        SkASSERT(accessPattern == kDynamic_GrAccessPattern);
        break;
    case GrGpuBufferType::kXferGpuToCpu:
        SkASSERT(accessPattern == kDynamic_GrAccessPattern || accessPattern == kStream_GrAccessPattern);
        break;
...
bool GrVkGpu::onWritePixels(GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType srcColorType, const GrMipLevel texels[],
    int mipLevelCount, bool prepForTexSampling)
{
    ...
    if (linearTiling) {
        ...
        if (VK_IMAGE_LAYOUT_PREINITIALIZED != texImage->currentLayout()) {
            ...
            texImage->setImageLayout(this, VK_IMAGE_LAYOUT_GENERAL, VK_ACCESS_HOST_WRITE_BIT, VK_PIPELINE_STAGE_HOST_BIT, false);
        ...
        success = this->uploadTexDataLinear(texImage, rect, srcColorType, texels[0].fPixels, texels[0].fRowBytes);
    } else {
    ...
    if (prepForTexSampling) {
        texImage->setImageLayout(this, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_SHADER_READ_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, false);
    }
...
{
    if (dst->intendedType() != GrGpuBufferType::kIndex && dst->intendedType() != GrGpuBufferType::kVertex) {
        return;
    }

    VkAccessFlags srcAccessMask = dst->intendedType() == GrGpuBufferType::kIndex ? VK_ACCESS_INDEX_READ_BIT : VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT;
    ...
    gpu->addBufferMemoryBarrier(srcPipelineStageFlags, dstPipelineStageFlags,
        /*byRegion=*/false, &bufferMemoryBarrier);
}

bool GrVkGpu::onTransferFromBufferToBuffer(sk_sp<GrGpuBuffer> src, size_t srcOffset, sk_sp<GrGpuBuffer> dst, size_t dstOffset, size_t size)
{
    if (!this->currentCommandBuffer()) {
        return false;
    }
    ...
    add_transfer_dst_buffer_mem_barrier(this, static_cast<GrVkBuffer*>(dst.get()), dstOffset, size,
        /*after=*/false);
...
bool GrVkGpu::onTransferPixelsTo(GrTexture* texture, SkIRect rect, GrColorType surfaceColorType, GrColorType bufferColorType, sk_sp<GrGpuBuffer> transferBuffer,
    size_t bufferOffset, size_t rowBytes)
{
    ...
    vkImage->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);

    const GrVkBuffer* vkBuffer = static_cast<GrVkBuffer*>(transferBuffer.get());

    // Copy the buffer to the image.
    this->currentCommandBuffer()->copyBufferToImage(this, vkBuffer->vkBuffer(), vkImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
...
bool GrVkGpu::onTransferPixelsFrom(
    GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType bufferColorType, sk_sp<GrGpuBuffer> transferBuffer, size_t offset)
{
    if (!this->currentCommandBuffer()) {
        return false;
    }
    ...
    srcImage->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_TRANSFER_READ_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);

    this->currentCommandBuffer()->copyImageToBuffer(this, srcImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, transferBuffer, 1, &region);

    GrVkBuffer* vkBuffer = static_cast<GrVkBuffer*>(transferBuffer.get());
    // Make sure the copy to buffer has finished.
    vkBuffer->addMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_HOST_READ_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, false);
    return true;
}

void GrVkGpu::resolveImage(GrSurface* dst, GrVkRenderTarget* src, const SkIRect& srcRect, const SkIPoint& dstPoint)
{
    if (!this->currentCommandBuffer()) {
        return;
    }
    ...
    dstImage->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);

    src->colorAttachment()->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_TRANSFER_READ_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);
    ...
    this->currentCommandBuffer()->resolveImage(this, *src->colorAttachment(), *dstImage, 1, &resolveInfo);
}
...
bool GrVkGpu::uploadTexDataLinear(GrVkImage* texImage, SkIRect rect, GrColorType dataColorType, const void* data, size_t rowBytes)
{
    SkASSERT(data);
    ...
    SkASSERT(VK_IMAGE_LAYOUT_PREINITIALIZED == texImage->currentLayout() || VK_IMAGE_LAYOUT_GENERAL == texImage->currentLayout());
    ...
    auto checkResult = [this](VkResult result) { return this->checkVkResult(result); };
    ...
    SkRectMemcpy(mapPtr, static_cast<size_t>(layout.rowPitch), data, rowBytes, trimRowBytes, rect.height());
...
static size_t fill_in_compressed_regions(GrStagingBufferManager* stagingBufferManager, TArray<VkBufferImageCopy>* regions, TArray<size_t>* individualMipOffsets,
    GrStagingBufferManager::Slice* slice, SkTextureCompressionType compression, VkFormat vkFormat, SkISize dimensions, skgpu::Mipmapped mipmapped)
{
    ...
    size_t bufferSize = SkCompressedDataSize(compression, dimensions, individualMipOffsets, mipmapped == skgpu::Mipmapped::kYes);
    ...
    case 0:
        break; // alignment is already a multiple of 4.
    case 2:
        alignment *= 2;
        break; // alignment is a multiple of 2 but not 4.
    default:
        alignment *= 4;
        break; // alignment is not a multiple of 2.
    }
...
bool GrVkGpu::uploadTexDataOptimal(GrVkImage* texImage, SkIRect rect, GrColorType dataColorType, const GrMipLevel texels[], int mipLevelCount)
{
    if (!this->currentCommandBuffer()) {
        return false;
    }
    ...
    if (mipLevelCount > 1) {
        combinedBufferSize = GrComputeTightCombinedBufferSize(bpp, rect.size(), &individualMipOffsets, mipLevelCount);
    } else {
    ...
    GrStagingBufferManager::Slice slice = fStagingBufferManager.allocateStagingBufferSlice(combinedBufferSize, alignment);
    if (!slice.fBuffer) {
        return false;
    }
    ...
    // Change layout of our target so it can be copied to
    texImage->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);
    ...
    this->currentCommandBuffer()->copyBufferToImage(
        this, vkBuffer->vkBuffer(), texImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, regions.size(), regions.begin());
    return true;
}

// It's probably possible to roll this into uploadTexDataOptimal,
// but for now it's easier to maintain as a separate entity.
bool GrVkGpu::uploadTexDataCompressed(GrVkImage* uploadTexture, SkTextureCompressionType compression, VkFormat vkFormat, SkISize dimensions,
    skgpu::Mipmapped mipmapped, const void* data, size_t dataSize)
{
    if (!this->currentCommandBuffer()) {
        return false;
    }
    ...
    SkASSERT(dimensions.fWidth == uploadTexture->width() && dimensions.fHeight == uploadTexture->height());
    ...
    SkDEBUGCODE(size_t combinedBufferSize =)
        fill_in_compressed_regions(&fStagingBufferManager, &regions, &individualMipOffsets, &slice, compression, vkFormat, dimensions, mipmapped);
    if (!slice.fBuffer) {
        return false;
    }
    ...
    // Change layout of our target so it can be copied to
    uploadTexture->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);
    ...
    GrVkBuffer* vkBuffer = static_cast<GrVkBuffer*>(slice.fBuffer);
    this->currentCommandBuffer()->copyBufferToImage(
        this, vkBuffer->vkBuffer(), uploadTexture, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, regions.size(), regions.begin());
...
sk_sp<GrTexture> GrVkGpu::onCreateTexture(SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable, int renderTargetSampleCnt,
    skgpu::Budgeted budgeted, GrProtected isProtected, int mipLevelCount, uint32_t levelClearMask, std::string_view label)
{
    ...
    if (renderable == GrRenderable::kYes) {
        tex = GrVkTextureRenderTarget::MakeNewTextureRenderTarget(
            this, budgeted, dimensions, pixelFormat, mipLevelCount, renderTargetSampleCnt, mipmapStatus, isProtected, label);
    } else {
        tex = GrVkTexture::MakeNewTexture(this, budgeted, dimensions, pixelFormat, mipLevelCount, isProtected, mipmapStatus, label);
    }
    ...
    if (levelClearMask) {
        ...
        texImage->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);
        this->currentCommandBuffer()->clearColorImage(this, texImage, &kZeroClearColor, ranges.size(), ranges.begin());
    }
    return tex;
}

sk_sp<GrTexture> GrVkGpu::onCreateCompressedTexture(SkISize dimensions, const GrBackendFormat& format, skgpu::Budgeted budgeted, skgpu::Mipmapped mipmapped,
    GrProtected isProtected, const void* data, size_t dataSize)
{
    ...
    auto tex = GrVkTexture::MakeNewTexture(this, budgeted, dimensions, pixelFormat, numMipLevels, isProtected, mipmapStatus,
    ...
    if (!this->uploadTexDataCompressed(tex->textureImage(), compression, pixelFormat, dimensions, mipmapped, data, dataSize)) {
        return nullptr;
    }
...
bool GrVkGpu::updateBuffer(sk_sp<GrVkBuffer> buffer, const void* src, VkDeviceSize offset, VkDeviceSize size)
{
    if (!this->currentCommandBuffer()) {
        return false;
    }
    add_transfer_dst_buffer_mem_barrier(this, static_cast<GrVkBuffer*>(buffer.get()), offset, size,
        /*after=*/false);
...
bool GrVkGpu::zeroBuffer(sk_sp<GrGpuBuffer> buffer)
{
    ...
    add_transfer_dst_buffer_mem_barrier(this, static_cast<GrVkBuffer*>(buffer.get()),
        /*offset=*/0, buffer->size(),
        /*after=*/false);
    this->currentCommandBuffer()->fillBuffer(this, buffer,
        /*offset=*/0, buffer->size(),
        /*data=*/0);
    add_transfer_dst_buffer_mem_barrier(this, static_cast<GrVkBuffer*>(buffer.get()),
        /*offset=*/0, buffer->size(),
...
static bool check_image_info(const GrVkCaps& caps, const GrVkImageInfo& info, bool needsAllocation, uint32_t graphicsQueueIndex)
{
    ...
    if (info.fCurrentQueueFamily != VK_QUEUE_FAMILY_IGNORED && info.fCurrentQueueFamily != VK_QUEUE_FAMILY_EXTERNAL
    ...
    if (!SkToBool(info.fImageUsageFlags & VK_IMAGE_USAGE_TRANSFER_SRC_BIT) || !SkToBool(info.fImageUsageFlags & VK_IMAGE_USAGE_TRANSFER_DST_BIT)) {
        return false;
    }
...
sk_sp<GrTexture> GrVkGpu::onWrapBackendTexture(const GrBackendTexture& backendTex, GrWrapOwnership ownership, GrWrapCacheable cacheable, GrIOType ioType)
{
    ...
    if (!check_image_info(this->vkCaps(), imageInfo, kAdopt_GrWrapOwnership == ownership, this->queueIndex())) {
        return nullptr;
    }
    ...
    return GrVkTexture::MakeWrappedTexture(this, backendTex.dimensions(), ownership, cacheable, ioType, imageInfo, std::move(mutableState));
}

sk_sp<GrTexture> GrVkGpu::onWrapCompressedBackendTexture(const GrBackendTexture& beTex, GrWrapOwnership ownership, GrWrapCacheable cacheable)
{
    ...
    const GrBackendTexture& backendTex, int sampleCnt, GrWrapOwnership ownership, GrWrapCacheable cacheable)
{
    ...
    return GrVkTextureRenderTarget::MakeWrappedTextureRenderTarget(
        this, backendTex.dimensions(), sampleCnt, ownership, cacheable, imageInfo, std::move(mutableState));
}
...
sk_sp<GrRenderTarget> GrVkGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& backendRT)
{
    ...
    sk_sp<GrVkRenderTarget> tgt = GrVkRenderTarget::MakeWrappedRenderTarget(this, backendRT.dimensions(), backendRT.sampleCnt(), info, std::move(mutableState));
...
sk_sp<GrRenderTarget> GrVkGpu::onWrapVulkanSecondaryCBAsRenderTarget(const SkImageInfo& imageInfo, const GrVkDrawableInfo& vkInfo)
{
...
bool GrVkGpu::loadMSAAFromResolve(GrVkCommandBuffer* commandBuffer, const GrVkRenderPass& renderPass, GrAttachment* dst, GrVkImage* src, const SkIRect& srcRect)
{
...
bool GrVkGpu::onRegenerateMipMapLevels(GrTexture* tex)
{
    ...
    if (!caps.formatCanBeDstofBlit(vkTex->imageFormat(), false) || !caps.formatCanBeSrcofBlit(vkTex->imageFormat(), false) || !caps.mipmapSupport()) {
        return false;
    }
    ...
    vkTex->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);
    ...
    while (mipLevel < levelCount) {
        ...
        this->addImageMemoryBarrier(vkTex->resource(), VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false, &imageMemoryBarrier);
        ...
        this->currentCommandBuffer()->blitImage(this, vkTex->resource(), vkTex->image(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, vkTex->resource(),
            vkTex->image(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blitRegion, VK_FILTER_LINEAR);
        ++mipLevel;
    }
    if (levelCount > 1) {
...
sk_sp<GrAttachment> GrVkGpu::makeStencilAttachment(const GrBackendFormat& /*colorFormat*/, SkISize dimensions, int numStencilSamples)
{
...
sk_sp<GrAttachment> GrVkGpu::makeMSAAAttachment(
    SkISize dimensions, const GrBackendFormat& format, int numSamples, GrProtected isProtected, GrMemoryless memoryless)
{
    VkFormat pixelFormat;
    SkAssertResult(GrBackendFormats::AsVkFormat(format, &pixelFormat));
    SkASSERT(!skgpu::VkFormatIsCompressed(pixelFormat));
...
bool copy_src_data(char* mapPtr, VkFormat vkFormat, const TArray<size_t>& individualMipOffsets, const GrPixmap srcData[], int numMipLevels)
{
    ...
    for (int level = 0; level < numMipLevels; ++level) {
        ...
        SkRectMemcpy(mapPtr + individualMipOffsets[level], trimRB, srcData[level].addr(), srcData[level].rowBytes(), trimRB, srcData[level].height());
    }
    return true;
}

bool GrVkGpu::createVkImageForBackendSurface(VkFormat vkFormat, SkISize dimensions, int sampleCnt, GrTexturable texturable, GrRenderable renderable,
    skgpu::Mipmapped mipmapped, GrVkImageInfo* info, GrProtected isProtected)
{
...
bool GrVkGpu::onClearBackendTexture(const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback, std::array<float, 4> color)
{
    GrVkImageInfo info;
    ...
    sk_sp<GrVkTexture> texture = GrVkTexture::MakeWrappedTexture(
        this, backendTexture.dimensions(), kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRW_GrIOType, info, std::move(mutableState));
    if (!texture) {
        return false;
    }
    GrVkImage* texImage = texture->textureImage();
    ...
    SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable, skgpu::Mipmapped mipmapped, GrProtected isProtected, std::string_view label)
{
    ...
    GrVkImageInfo info;
    if (!this->createVkImageForBackendSurface(vkFormat, dimensions, 1, GrTexturable::kYes, renderable, mipmapped, &info, isProtected)) {
        return {};
    }
...
{
    return this->onCreateBackendTexture(dimensions, format, GrRenderable::kNo, mipmapped, isProtected,
        /*label=*/"VkGpu_CreateCompressedBackendTexture");
}

bool GrVkGpu::onUpdateCompressedBackendTexture(
    const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback, const void* data, size_t size)
{
    GrVkImageInfo info;
    SkAssertResult(GrBackendTextures::GetVkImageInfo(backendTexture, &info));
    ...
    image->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);

    SkTextureCompressionType compression = GrBackendFormatToCompressionType(backendTexture.getBackendFormat());
    ...
    fill_in_compressed_regions(
        &fStagingBufferManager, &regions, &individualMipOffsets, &slice, compression, info.fFormat, backendTexture.dimensions(), backendTexture.fMipmapped);
    ...
    cmdBuffer->copyBufferToImage(this, static_cast<GrVkBuffer*>(slice.fBuffer)->vkBuffer(), image, image->currentLayout(), regions.size(), regions.begin());

    // Change image layout to shader read since if we use this texture as a borrowed
    // texture within Ganesh we require that its layout be set to that
    image->setImageLayout(this, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_SHADER_READ_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, false);
...
void set_layout_and_queue_from_mutable_state(GrVkGpu* gpu, GrVkImage* image, VkImageLayout newLayout, uint32_t newQueueFamilyIndex)
{
    ...
    auto isSpecialQueue
        = [](uint32_t queueFamilyIndex) { return queueFamilyIndex == VK_QUEUE_FAMILY_EXTERNAL || queueFamilyIndex == VK_QUEUE_FAMILY_FOREIGN_EXT; };
    ...
    image->setImageLayoutAndQueueIndex(gpu, newLayout, dstAccess, dstStage, false, newQueueFamilyIndex);
}

bool GrVkGpu::setBackendSurfaceState(GrVkImageInfo info, sk_sp<skgpu::MutableTextureState> currentState, SkISize dimensions, VkImageLayout newLayout,
    uint32_t newQueueFamilyIndex, skgpu::MutableTextureState* previousState, sk_sp<skgpu::RefCntedCallback> finishedCallback)
{
    sk_sp<GrVkImage> texture = GrVkImage::MakeWrapped(this, dimensions, info, std::move(currentState), GrVkImage::UsageFlags::kColorAttachment,
        kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, "VkGpu_SetBackendSurfaceState",
...
bool GrVkGpu::setBackendTextureState(const GrBackendTexture& backendTeture, const skgpu::MutableTextureState& newState,
    skgpu::MutableTextureState* previousState, sk_sp<skgpu::RefCntedCallback> finishedCallback)
{
    GrVkImageInfo info;
    ...
    return this->setBackendSurfaceState(info, std::move(currentState), backendTeture.dimensions(), skgpu::MutableTextureStates::GetVkImageLayout(newState),
        skgpu::MutableTextureStates::GetVkQueueFamilyIndex(newState), previousState, std::move(finishedCallback));
}

bool GrVkGpu::setBackendRenderTargetState(const GrBackendRenderTarget& backendRenderTarget, const skgpu::MutableTextureState& newState,
    skgpu::MutableTextureState* previousState, sk_sp<skgpu::RefCntedCallback> finishedCallback)
{
    GrVkImageInfo info;
    ...
    return this->setBackendSurfaceState(info, std::move(currentState), backendRenderTarget.dimensions(),
        skgpu::MutableTextureStates::GetVkImageLayout(newState), skgpu::MutableTextureStates::GetVkQueueFamilyIndex(newState), previousState,
...
void GrVkGpu::xferBarrier(GrRenderTarget* rt, GrXferBarrierType barrierType)
{
    ...
    this->addImageMemoryBarrier(image->resource(), VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, dstStage, true, &barrier);
}
...
bool GrVkGpu::compile(const GrProgramDesc& desc, const GrProgramInfo& programInfo)
{
    ...
    GrVkRenderTarget::ReconstructAttachmentsDescriptor(this->vkCaps(), programInfo, &attachmentsDescriptor, &attachmentFlags);
    ...
    GrVkRenderPass::LoadFromResolve loadFromResolve = GrVkRenderPass::LoadFromResolve::kNo;
    if (this->vkCaps().programInfoWillUseDiscardableMSAA(programInfo) && programInfo.colorLoadOp() == GrLoadOp::kLoad) {
        loadFromResolve = GrVkRenderPass::LoadFromResolve::kLoad;
    }
    sk_sp<const GrVkRenderPass> renderPass(
        this->resourceProvider().findCompatibleRenderPass(&attachmentsDescriptor, attachmentFlags, selfDepFlags, loadFromResolve));
    if (!renderPass) {
        return false;
    }
    ...
    auto pipelineState = this->resourceProvider().findOrCreateCompatiblePipelineState(desc, programInfo, renderPass->vkRenderPass(), &stat);
    if (!pipelineState) {
        return false;
    }
...
GrBackendRenderTarget GrVkGpu::createTestingOnlyBackendRenderTarget(SkISize dimensions, GrColorType ct, int sampleCnt, GrProtected isProtected)
{
    ...
    if (!this->createVkImageForBackendSurface(
            vkFormat, dimensions, sampleCnt, GrTexturable::kNo, GrRenderable::kYes, skgpu::Mipmapped::kNo, &info, isProtected)) {
        return {};
    }
...
void GrVkGpu::addBufferMemoryBarrier(const GrManagedResource* resource, VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask, bool byRegion,
    VkBufferMemoryBarrier* barrier) const
{
    ...
    SkASSERT(resource);
    this->currentCommandBuffer()->pipelineBarrier(this, resource, srcStageMask, dstStageMask, byRegion, GrVkCommandBuffer::kBufferMemory_BarrierType, barrier);
}
void GrVkGpu::addBufferMemoryBarrier(VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask, bool byRegion, VkBufferMemoryBarrier* barrier) const
{
    if (!this->currentCommandBuffer()) {
        return;
    }
    ...
    this->currentCommandBuffer()->pipelineBarrier(this,
        /*resource=*/nullptr, srcStageMask, dstStageMask, byRegion, GrVkCommandBuffer::kBufferMemory_BarrierType, barrier);
}

void GrVkGpu::addImageMemoryBarrier(
    const GrManagedResource* resource, VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask, bool byRegion, VkImageMemoryBarrier* barrier) const
{
    ...
    SkASSERT(resource);
    this->currentCommandBuffer()->pipelineBarrier(this, resource, srcStageMask, dstStageMask, byRegion, GrVkCommandBuffer::kImageMemory_BarrierType, barrier);
}

void GrVkGpu::prepareSurfacesForBackendAccessAndStateUpdates(
    SkSpan<GrSurfaceProxy*> proxies, SkSurfaces::BackendSurfaceAccess access, const skgpu::MutableTextureState* newState)
{
    ...
    if (!proxies.empty() && (access == SkSurfaces::BackendSurfaceAccess::kPresent || newState)) {
        ...
        for (GrSurfaceProxy* proxy : proxies) {
            ...
            if (newState) {
                ...
                uint32_t newIndex = skgpu::MutableTextureStates::GetVkQueueFamilyIndex(newState);
...
void GrVkGpu::addFinishedProc(GrGpuFinishedProc finishedProc, GrGpuFinishedContext finishedContext)
{
...
void GrVkGpu::onReportSubmitHistograms()
{
    ...
    if (allocatedMemory > 0) {
        SK_HISTOGRAM_PERCENTAGE("VulkanMemoryAllocator.PercentUsed", (usedMemory * 100) / allocatedMemory);
    }
...
void GrVkGpu::copySurfaceAsCopyImage(GrSurface* dst, GrSurface* src, GrVkImage* dstImage, GrVkImage* srcImage, const SkIRect& srcRect, const SkIPoint& dstPoint)
{
    if (!this->currentCommandBuffer()) {
        return;
    }
    ...
    SkASSERT(this->vkCaps().canCopyImage(dstFormat, dstSampleCnt, dstHasYcbcr, srcFormat, srcSampleCnt, srcHasYcbcr));
#endif
    ...
    this->currentCommandBuffer()->copyImage(
        this, srcImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dstImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copyRegion);
...
void GrVkGpu::copySurfaceAsBlit(
    GrSurface* dst, GrSurface* src, GrVkImage* dstImage, GrVkImage* srcImage, const SkIRect& srcRect, const SkIRect& dstRect, GrSamplerState::Filter filter)
{
    if (!this->currentCommandBuffer()) {
        return;
    }
    ...
    SkASSERT(this->vkCaps().canCopyAsBlit(
        dstFormat, dstSampleCnt, dstImage->isLinearTiled(), dstHasYcbcr, srcFormat, srcSampleCnt, srcImage->isLinearTiled(), srcHasYcbcr));
    ...
    this->currentCommandBuffer()->blitImage(
        this, *srcImage, *dstImage, 1, &blitRegion, filter == GrSamplerState::Filter::kNearest ? VK_FILTER_NEAREST : VK_FILTER_LINEAR);

    // The rect is already in device space so we pass in kTopLeft so no flip is done.
    this->didWriteToSurface(dst, kTopLeft_GrSurfaceOrigin, &dstRect);
}

void GrVkGpu::copySurfaceAsResolve(GrSurface* dst, GrSurface* src, const SkIRect& srcRect, const SkIPoint& dstPoint)
{
    if (src->isProtected() && !dst->isProtected()) {
        SkDebugf("Can't copy from protected memory to non-protected");
        return;
...
bool GrVkGpu::onCopySurface(GrSurface* dst, const SkIRect& dstRect, GrSurface* src, const SkIRect& srcRect, GrSamplerState::Filter filter)
{
#ifdef SK_DEBUG
    ...
    if (srcRect.size() == dstRect.size()) {
        ...
        if (this->vkCaps().canCopyAsResolve(dstFormat, dstSampleCnt, dstHasYcbcr, srcFormat, srcSampleCnt, srcHasYcbcr)) {
            this->copySurfaceAsResolve(dst, src, srcRect, dstPoint);
            return true;
        }

        if (this->vkCaps().canCopyImage(dstFormat, dstSampleCnt, dstHasYcbcr, srcFormat, srcSampleCnt, srcHasYcbcr)) {
...
bool GrVkGpu::onReadPixels(GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType dstColorType, void* buffer, size_t rowBytes)
{
    ...
    if (dstColorType == GrColorType::kUnknown || dstColorType != this->vkCaps().transferColorType(image->imageFormat(), surfaceColorType)) {
        return false;
    }

    // Change layout of our target so it can be used as copy
    image->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_TRANSFER_READ_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);
    ...
    sk_sp<GrGpuBuffer> transferBuffer = resourceProvider->createBuffer(
        transBufferRowBytes * imageRows, GrGpuBufferType::kXferGpuToCpu, kDynamic_GrAccessPattern, GrResourceProvider::ZeroInit::kNo);
    ...
    this->currentCommandBuffer()->copyImageToBuffer(this, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, transferBuffer, 1, &region);
...
bool GrVkGpu::beginRenderPass(const GrVkRenderPass* renderPass, sk_sp<const GrVkFramebuffer> framebuffer, const VkClearValue* colorClear,
    const GrSurface* target, const SkIRect& renderPassBounds, bool forSecondaryCB)
{
    if (!this->currentCommandBuffer()) {
        return false;
    }
    ...
    return this->currentCommandBuffer()->beginRenderPass(this, renderPass, std::move(framebuffer), clears, target, renderPassBounds, forSecondaryCB);
}
...
bool GrVkGpu::checkVkResult(VkResult result)
{
    ...
    case VK_ERROR_DEVICE_LOST:
        if (!fDeviceIsLost) {
            ...
            fDeviceIsLost = true;
            skgpu::InvokeDeviceLostCallback(vkInterface(), device(), fDeviceLostContext, fDeviceLostProc, vkCaps().supportsDeviceFaultInfo());
        }
        return false;
...
{
    return GrVkSemaphore::MakeWrapped(this, GrBackendSemaphores::GetVkSemaphore(semaphore), wrapType, ownership);
}
...
std::unique_ptr<GrSemaphore> GrVkGpu::prepareTextureForCrossContextUsage(GrTexture* texture)
{
    ...
    vkTexture->setImageLayout(this, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_SHADER_READ_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, false);
