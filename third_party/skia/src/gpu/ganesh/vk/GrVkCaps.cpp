...
GrVkCaps::GrVkCaps(const GrContextOptions& contextOptions, const skgpu::VulkanInterface* vkInterface, VkPhysicalDevice physDev,
    const VkPhysicalDeviceFeatures2& features, uint32_t instanceVersion, uint32_t physicalDeviceVersion, const skgpu::VulkanExtensions& extensions,
    ...
    : INHERITED(contextOptions)
{
    ...
    this->init(contextOptions, vkInterface, physDev, features, physicalDeviceVersion, extensions, isProtected);
}
...
bool GrVkCaps::canCopyImage(VkFormat dstFormat, int dstSampleCnt, bool dstHasYcbcr, VkFormat srcFormat, int srcSampleCnt, bool srcHasYcbcr) const
{
...
bool GrVkCaps::canCopyAsBlit(
    VkFormat dstFormat, int dstSampleCnt, bool dstIsLinear, bool dstHasYcbcr, VkFormat srcFormat, int srcSampleCnt, bool srcIsLinear, bool srcHasYcbcr) const
{
    // We require that all vulkan GrSurfaces have been created with transfer_dst and transfer_src
    // as image usage flags.
    if (!this->formatCanBeDstofBlit(dstFormat, dstIsLinear) || !this->formatCanBeSrcofBlit(srcFormat, srcIsLinear)) {
        return false;
    }
...
bool GrVkCaps::canCopyAsResolve(VkFormat dstFormat, int dstSampleCnt, bool dstHasYcbcr, VkFormat srcFormat, int srcSampleCnt, bool srcHasYcbcr) const
{
...
bool GrVkCaps::onCanCopySurface(const GrSurfaceProxy* dst, const SkIRect& dstRect, const GrSurfaceProxy* src, const SkIRect& srcRect) const
{
    ...
    if (const GrRenderTargetProxy* rtProxy = dst->asRenderTargetProxy()) {
        ...
        if (this->preferDiscardableMSAAAttachment() && dst->asTextureProxy() && rtProxy->supportsVkInputAttachment()) {
            dstSampleCnt = 1;
        } else {
    ...
    if (const GrRenderTargetProxy* rtProxy = src->asRenderTargetProxy()) {
        ...
        if (this->preferDiscardableMSAAAttachment() && src->asTextureProxy() && rtProxy->supportsVkInputAttachment()) {
            srcSampleCnt = 1;
        } else {
    ...
    if (!copyScales
        && (this->canCopyImage(dstFormat, dstSampleCnt, dstHasYcbcr, srcFormat, srcSampleCnt, srcHasYcbcr)
            || this->canCopyAsResolve(dstFormat, dstSampleCnt, dstHasYcbcr, srcFormat, srcSampleCnt, srcHasYcbcr))) {
        return true;
    }
    return this->canCopyAsBlit(dstFormat, dstSampleCnt, dstIsLinear, dstHasYcbcr, srcFormat, srcSampleCnt, srcIsLinear, srcHasYcbcr);
}

void GrVkCaps::init(const GrContextOptions& contextOptions, const skgpu::VulkanInterface* vkInterface, VkPhysicalDevice physDev,
    const VkPhysicalDeviceFeatures2& features, uint32_t physicalDeviceVersion, const skgpu::VulkanExtensions& extensions, GrProtected isProtected)
{
    ...
    if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0) || extensions.hasExtension(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME, 1)) {
        fSupportsPhysicalDeviceProperties2 = true;
    }

    if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0) || extensions.hasExtension(VK_KHR_GET_MEMORY_REQUIREMENTS_2_EXTENSION_NAME, 1)) {
        fSupportsMemoryRequirements2 = true;
    }

    if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0) || extensions.hasExtension(VK_KHR_BIND_MEMORY_2_EXTENSION_NAME, 1)) {
        fSupportsBindMemory2 = true;
    }

    if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0) || extensions.hasExtension(VK_KHR_MAINTENANCE1_EXTENSION_NAME, 1)) {
        fSupportsMaintenance1 = true;
    }

    if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0) || extensions.hasExtension(VK_KHR_MAINTENANCE2_EXTENSION_NAME, 1)) {
        fSupportsMaintenance2 = true;
    }

    if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0) || extensions.hasExtension(VK_KHR_MAINTENANCE3_EXTENSION_NAME, 1)) {
        fSupportsMaintenance3 = true;
    }

    if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0)
        || (extensions.hasExtension(VK_KHR_DEDICATED_ALLOCATION_EXTENSION_NAME, 1) && this->supportsMemoryRequirements2())) {
        fSupportsDedicatedAllocation = true;
    }

    if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0)
        || (extensions.hasExtension(VK_KHR_EXTERNAL_MEMORY_CAPABILITIES_EXTENSION_NAME, 1) && this->supportsPhysicalDeviceProperties2()
            && extensions.hasExtension(VK_KHR_EXTERNAL_MEMORY_EXTENSION_NAME, 1) && this->supportsDedicatedAllocation())) {
    ...
    if (ycbcrFeatures && ycbcrFeatures->samplerYcbcrConversion
        && (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0)
            || (extensions.hasExtension(VK_KHR_SAMPLER_YCBCR_CONVERSION_EXTENSION_NAME, 1) && this->supportsMaintenance1() && this->supportsBindMemory2()
    ...
    if (contextOptions.fMaxCachedVulkanSecondaryCommandBuffers >= 0) {
        fMaxPerPoolCachedSecondaryCommandBuffers = contextOptions.fMaxCachedVulkanSecondaryCommandBuffers;
    }
...
void GrVkCaps::applyDriverCorrectnessWorkarounds(const VkPhysicalDeviceProperties& properties)
{
    ...
    if (kIntel_VkVendor == properties.vendorID && GetIntelGen(GetIntelGPUType(properties.deviceID)) >= 9) {
        fTextureBarrierSupport = false;
    }
#endif
...
void GrVkCaps::initGrCaps(const skgpu::VulkanInterface* vkInterface, VkPhysicalDevice physDev, const VkPhysicalDeviceProperties& properties,
    const VkPhysicalDeviceMemoryProperties& memoryProperties, const VkPhysicalDeviceFeatures2& features, const skgpu::VulkanExtensions& extensions)
{
    ...
    if (extensions.hasExtension(VK_EXT_BLEND_OPERATION_ADVANCED_EXTENSION_NAME, 2) && this->supportsPhysicalDeviceProperties2()) {
        ...
        if (blendProps.advancedBlendAllOperations == VK_TRUE) {
            ...
            auto blendFeatures = skgpu::GetExtensionFeatureStruct<VkPhysicalDeviceBlendOperationAdvancedFeaturesEXT>(
...
void GrVkCaps::initShaderCaps(const VkPhysicalDeviceProperties& properties, const VkPhysicalDeviceFeatures2& features)
{
    ...
    shaderCaps->fMaxFragmentSamplers
        = std::min(std::min(properties.limits.maxPerStageDescriptorSampledImages, properties.limits.maxPerStageDescriptorSamplers), (uint32_t)INT_MAX);
}
...
void GrVkCaps::initStencilFormat(const skgpu::VulkanInterface* interface, VkPhysicalDevice physDev)
{
...
void GrVkCaps::setColorType(GrColorType colorType, std::initializer_list<VkFormat> formats)
{
    ...
    for (size_t i = 0; i < kNumVkFormats; ++i) {
        ...
        for (int j = 0; j < formatInfo.fColorTypeInfoCount; ++j) {
            ...
            if (ctInfo.fColorType == colorType && !SkToBool(ctInfo.fFlags & ColorTypeInfo::kWrappedOnly_Flag)) {
                bool found = false;
...
{
    static_assert(std::size(kVkFormats) == GrVkCaps::kNumVkFormats, "Size of VkFormats array must match static value in header");
...
void GrVkCaps::initFormatTable(const GrContextOptions& contextOptions, const skgpu::VulkanInterface* interface, VkPhysicalDevice physDev,
    const VkPhysicalDeviceProperties& properties, const VkPhysicalDeviceFeatures2& features, const skgpu::VulkanExtensions& extensions)
{
    static_assert(std::size(kVkFormats) == GrVkCaps::kNumVkFormats, "Size of VkFormats array must match static value in header");
    ...
    if (extensions.hasExtension(VK_EXT_RGBA10X6_FORMATS_EXTENSION_NAME, 1)) {
        auto rgba10x6Feature = skgpu::GetExtensionFeatureStruct<VkPhysicalDeviceRGBA10X6FormatsFeaturesEXT>(
    ...
    this->setColorType(GrColorType::kBGR_565, { VK_FORMAT_R5G6B5_UNORM_PACK16, VK_FORMAT_B5G6R5_UNORM_PACK16 });
    this->setColorType(GrColorType::kRGB_565, { VK_FORMAT_B5G6R5_UNORM_PACK16 });
    this->setColorType(GrColorType::kABGR_4444, { VK_FORMAT_R4G4B4A4_UNORM_PACK16, VK_FORMAT_B4G4R4A4_UNORM_PACK16 });
    ...
    this->setColorType(GrColorType::kRGB_888x,
        {
            VK_FORMAT_R8G8B8_UNORM,
            VK_FORMAT_R8G8B8A8_UNORM,
            VK_FORMAT_B8G8R8A8_UNORM,
...
void GrVkCaps::FormatInfo::initSampleCounts(const GrContextOptions& contextOptions, const skgpu::VulkanInterface* interface, VkPhysicalDevice physDev,
    const VkPhysicalDeviceProperties& physProps, VkFormat format)
{
    VkImageUsageFlags usage
        = VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
    VkImageFormatProperties properties;
    GR_VK_CALL(interface,
        GetPhysicalDeviceImageFormatProperties(physDev, format, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL, usage,
            0, // createFlags
            &properties));
    ...
    if (kIntel_VkVendor == physProps.vendorID) {
        if (GetIntelGen(GetIntelGPUType(physProps.deviceID)) < 12 || !contextOptions.fAllowMSAAOnNewIntel) {
...
void GrVkCaps::FormatInfo::init(const GrContextOptions& contextOptions, const skgpu::VulkanInterface* interface, VkPhysicalDevice physDev,
...
{
    const skgpu::VulkanYcbcrConversionInfo* ycbcrInfo = GrBackendFormats::GetVkYcbcrConversionInfo(format);
    SkASSERT(ycbcrInfo);
...
bool GrVkCaps::isFormatAsColorTypeRenderable(GrColorType ct, const GrBackendFormat& format, int sampleCount) const
{
...
GrCaps::SurfaceReadPixelsSupport GrVkCaps::surfaceSupportsReadPixels(const GrSurface* surface) const
{
...
bool GrVkCaps::onAreColorTypeAndFormatCompatible(GrColorType ct, const GrBackendFormat& format) const
{
    VkFormat vkFormat;
    if (!GrBackendFormats::AsVkFormat(format, &vkFormat)) {
        return false;
...
bool GrVkCaps::onSupportsDynamicMSAA(const GrRenderTargetProxy* rtProxy) const
{
    ...
    // resolve attachment.
    return this->supportsDiscardableMSAAForDMSAA() && rtProxy->supportsVkInputAttachment() && rtProxy->numSamples() == 1;
}
...
bool GrVkCaps::renderTargetSupportsDiscardableMSAA(const GrVkRenderTarget* rt) const
{
    return rt->resolveAttachment() && rt->resolveAttachment()->supportsInputAttachmentUsage()
        && ((rt->numSamples() > 1 && this->preferDiscardableMSAAAttachment()) || (rt->numSamples() == 1 && this->supportsDiscardableMSAAForDMSAA()));
}
...
skgpu::Swizzle GrVkCaps::onGetReadSwizzle(const GrBackendFormat& format, GrColorType colorType) const
{
    VkFormat vkFormat;
    SkAssertResult(GrBackendFormats::AsVkFormat(format, &vkFormat));
    const skgpu::VulkanYcbcrConversionInfo* ycbcrInfo = GrBackendFormats::GetVkYcbcrConversionInfo(format);
    ...
    SkDEBUGFAILF("Illegal color type (%d) and format (%d) combination.", (int)colorType, (int)vkFormat);
    return {};
}

skgpu::Swizzle GrVkCaps::getWriteSwizzle(const GrBackendFormat& format, GrColorType colorType) const
{
    VkFormat vkFormat;
    SkAssertResult(GrBackendFormats::AsVkFormat(format, &vkFormat));
    const auto& info = this->getFormatInfo(vkFormat);
...
void GrVkCaps::addExtraSamplerKey(skgpu::KeyBuilder* b, GrSamplerState samplerState, const GrBackendFormat& format) const
{
    const skgpu::VulkanYcbcrConversionInfo* ycbcrInfo = GrBackendFormats::GetVkYcbcrConversionInfo(format);
    if (!ycbcrInfo) {
        return;
 ...
 * and blend constant.
 */
GrProgramDesc GrVkCaps::makeDesc(GrRenderTarget* rt, const GrProgramInfo& programInfo, ProgramDescOverrideFlags overrideFlags) const
{
    GrProgramDesc desc;
    ...
    bool forceLoadFromResolve = overrideFlags & GrCaps::ProgramDescOverrideFlags::kVulkanHasResolveLoadSubpass;
    ...
    if (rt) {
        ...
        SkASSERT(!needsResolve || (vkRT->resolveAttachment() && vkRT->resolveAttachment()->supportsInputAttachmentUsage()));
        ...
        if (!rp->isExternal()) {
            ...
            GrVkRenderTarget::ReconstructAttachmentsDescriptor(*this, programInfo, &attachmentsDescriptor, &attachmentFlags);
            SkASSERT(rp->isCompatible(attachmentsDescriptor, attachmentFlags, selfDepFlags, loadFromResolve));
        }
#endif
    } else {
        GrVkRenderPass::AttachmentsDescriptor attachmentsDescriptor;
        ...
        // parameter to GenKey).
        GrVkRenderPass::GenKey(&b, attachmentFlags, attachmentsDescriptor, selfDepFlags, loadFromResolve, 0);
    }
...
template <size_t N> static bool intel_deviceID_present(const std::array<uint32_t, N>& array, uint32_t deviceID)
{
...
GrVkCaps::IntelGPUType GrVkCaps::GetIntelGPUType(uint32_t deviceID)
{
    ...
    static constexpr std::array<uint32_t, 25> kSkyLakeIDs = { { 0x1902, 0x1906, 0x190A, 0x190B, 0x190E, 0x1912, 0x1913, 0x1915, 0x1916, 0x1917, 0x191A, 0x191B,
        0x191D, 0x191E, 0x1921, 0x1923, 0x1926, 0x1927, 0x192A, 0x192B, 0x192D, 0x1932, 0x193A, 0x193B, 0x193D } };
    static constexpr std::array<uint32_t, 14> kIceLakeIDs
        = { { 0x8A50, 0x8A51, 0x8A52, 0x8A53, 0x8A54, 0x8A56, 0x8A57, 0x8A58, 0x8A59, 0x8A5A, 0x8A5B, 0x8A5C, 0x8A5D, 0x8A71 } };
    static constexpr std::array<uint32_t, 5> kRocketLakeIDs = { { 0x4c8a, 0x4c8b, 0x4c8c, 0x4c90, 0x4c9a } };
    static constexpr std::array<uint32_t, 11> kTigerLakeIDs = { { 0x9A40, 0x9A49, 0x9A59, 0x9A60, 0x9A68, 0x9A70, 0x9A78, 0x9AC0, 0x9AC9, 0x9AD9, 0x9AF8 } };
    static constexpr std::array<uint32_t, 10> kAlderLakeIDs = { { 0x4680, 0x4681, 0x4682, 0x4683, 0x4690, 0x4691, 0x4692, 0x4693, 0x4698, 0x4699 } };
...
std::vector<GrTest::TestFormatColorTypeCombination> GrVkCaps::getTestingCombinations() const
{
    std::vector<GrTest::TestFormatColorTypeCombination> combos = {
        ...
        { GrColorType::kRGBA_10x6, GrBackendFormats::MakeVk(VK_FORMAT_R10X6G10X6B10X6A10X6_UNORM_4PACK16) },
