...
sk_sp<GrVkSecondaryCBDrawContext> GrVkSecondaryCBDrawContext::Make(
    GrRecordingContext* rContext, const SkImageInfo& imageInfo, const GrVkDrawableInfo& vkInfo, const SkSurfaceProps* props)
{
    if (!rContext) {
        return nullptr;
    }
    ...
    sk_sp<GrRenderTarget> rt = resourceProvider->wrapVulkanSecondaryCBAsRenderTarget(imageInfo, vkInfo);
    if (!rt) {
        return nullptr;
    }
    ...
    if (!gpp->caps()->isFormatAsColorTypeRenderable(colorType, GrBackendFormats::MakeVk(vkInfo.fFormat), /*sampleCount=*/1)) {
        return nullptr;
    }

    sk_sp<GrRenderTargetProxy> proxy(new GrRenderTargetProxy(std::move(rt), GrSurfaceProxy::UseAllocator::kNo, GrRenderTargetProxy::WrapsVkSecondaryCB::kYes));
    ...
    auto device = rContext->priv().createDevice(SkColorTypeToGrColorType(imageInfo.colorType()), std::move(proxy), imageInfo.refColorSpace(),
        kTopLeft_GrSurfaceOrigin, SkSurfacePropsCopyOrDefault(props), skgpu::ganesh::Device::InitContents::kUninit);
    if (!device) {
        return nullptr;
    }

    return sk_sp<GrVkSecondaryCBDrawContext>(new GrVkSecondaryCBDrawContext(std::move(device), props));
}

GrVkSecondaryCBDrawContext::GrVkSecondaryCBDrawContext(sk_sp<skgpu::ganesh::Device> device, const SkSurfaceProps* props)
...
bool GrVkSecondaryCBDrawContext::wait(int numSemaphores, const GrBackendSemaphore waitSemaphores[], bool deleteSemaphoresAfterWait)
{
...
bool GrVkSecondaryCBDrawContext::characterize(GrSurfaceCharacterization* characterization) const
{
    ...
    characterization->set(direct->threadSafeProxy(), maxResourceBytes, ii, format, readSurfaceView.origin(), numSamples,
        GrSurfaceCharacterization::Textureable(false), skgpu::Mipmapped::kNo, GrSurfaceCharacterization::UsesGLFBO0(false),
        GrSurfaceCharacterization::VkRTSupportsInputAttachment(false), GrSurfaceCharacterization::VulkanSecondaryCBCompatible(true), isProtected,
        this->props());
...
bool GrVkSecondaryCBDrawContext::isCompatible(const GrSurfaceCharacterization& characterization) const
{
    ...
    return characterization.contextInfo() && characterization.contextInfo()->priv().matches(dContext)
        && characterization.cacheMaxResourceBytes() <= maxResourceBytes && characterization.origin() == readSurfaceView.origin()
