...
sk_sp<GrVkImage> GrVkImage::MakeStencil(GrVkGpu* gpu, SkISize dimensions, int sampleCnt, VkFormat format)
{
    VkImageUsageFlags vkUsageFlags = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
    return GrVkImage::Make(gpu, dimensions, UsageFlags::kStencilAttachment, sampleCnt, format,
        /*mipLevels=*/1, vkUsageFlags, GrProtected::kNo, GrMemoryless::kNo, skgpu::Budgeted::kYes);
}

sk_sp<GrVkImage> GrVkImage::MakeMSAA(GrVkGpu* gpu, SkISize dimensions, int numSamples, VkFormat format, GrProtected isProtected, GrMemoryless memoryless)
{
    ...
    return GrVkImage::Make(gpu, dimensions, UsageFlags::kColorAttachment, numSamples, format,
        /*mipLevels=*/1, vkUsageFlags, isProtected, memoryless, skgpu::Budgeted::kYes);
}

sk_sp<GrVkImage> GrVkImage::MakeTexture(GrVkGpu* gpu, SkISize dimensions, VkFormat format, uint32_t mipLevels, GrRenderable renderable, int numSamples,
    skgpu::Budgeted budgeted, GrProtected isProtected)
{
    UsageFlags usageFlags = UsageFlags::kTexture;
    VkImageUsageFlags vkUsageFlags = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
    ...
    return GrVkImage::Make(gpu, dimensions, usageFlags, numSamples, format, mipLevels, vkUsageFlags, isProtected, GrMemoryless::kNo, budgeted);
}

static bool make_views(GrVkGpu* gpu, const GrVkImageInfo& info, GrAttachment::UsageFlags attachmentUsages, sk_sp<const GrVkImageView>* framebufferView,
    sk_sp<const GrVkImageView>* textureView)
{
    ...
    if (SkToBool(attachmentUsages & GrAttachment::UsageFlags::kStencilAttachment) || SkToBool(attachmentUsages & GrAttachment::UsageFlags::kColorAttachment)) {
        // Attachments can only have a mip level of 1
        *framebufferView = GrVkImageView::Make(gpu, info.fImage, info.fFormat, viewType, 1, info.fYcbcrConversionInfo);
        if (!*framebufferView) {
            return false;
        }
    }
    ...
    if (attachmentUsages & GrAttachment::UsageFlags::kTexture) {
        *textureView = GrVkImageView::Make(gpu, info.fImage, info.fFormat, viewType, info.fLevelCount, info.fYcbcrConversionInfo);
        if (!*textureView) {
            return false;
        }
    }
...
sk_sp<GrVkImage> GrVkImage::Make(GrVkGpu* gpu, SkISize dimensions, UsageFlags attachmentUsages, int sampleCnt, VkFormat format, uint32_t mipLevels,
    VkImageUsageFlags vkUsageFlags, GrProtected isProtected, GrMemoryless memoryless, skgpu::Budgeted budgeted)
{
    ...
    auto mutableState = sk_make_sp<skgpu::MutableTextureState>(skgpu::MutableTextureStates::MakeVulkan(info.fImageLayout, info.fCurrentQueueFamily));
    return sk_sp<GrVkImage>(
        new GrVkImage(gpu, dimensions, attachmentUsages, info, std::move(mutableState), std::move(framebufferView), std::move(textureView), budgeted,
            /*label=*/"MakeVkImage"));
}

sk_sp<GrVkImage> GrVkImage::MakeWrapped(GrVkGpu* gpu, SkISize dimensions, const GrVkImageInfo& info, sk_sp<skgpu::MutableTextureState> mutableState,
    UsageFlags attachmentUsages, GrWrapOwnership ownership, GrWrapCacheable cacheable, std::string_view label, bool forSecondaryCB)
{
    sk_sp<const GrVkImageView> framebufferView;
    sk_sp<const GrVkImageView> textureView;
    if (!forSecondaryCB) {
    ...
    GrBackendObjectOwnership backendOwnership = kBorrow_GrWrapOwnership == ownership ? GrBackendObjectOwnership::kBorrowed : GrBackendObjectOwnership::kOwned;

    return sk_sp<GrVkImage>(new GrVkImage(gpu, dimensions, attachmentUsages, info, std::move(mutableState), std::move(framebufferView), std::move(textureView),
        backendOwnership, cacheable, forSecondaryCB, label));
}

GrVkImage::GrVkImage(GrVkGpu* gpu, SkISize dimensions, UsageFlags supportedUsages, const GrVkImageInfo& info, sk_sp<skgpu::MutableTextureState> mutableState,
    sk_sp<const GrVkImageView> framebufferView, sk_sp<const GrVkImageView> textureView, skgpu::Budgeted budgeted, std::string_view label)
    : GrAttachment(gpu, dimensions, supportedUsages, info.fSampleCount, info.fLevelCount > 1 ? skgpu::Mipmapped::kYes : skgpu::Mipmapped::kNo, info.fProtected,
        label, info.fAlloc.fFlags & skgpu::VulkanAlloc::kLazilyAllocated_Flag ? GrMemoryless::kYes : GrMemoryless::kNo)
    , fInfo(info)
...
GrVkImage::GrVkImage(GrVkGpu* gpu, SkISize dimensions, UsageFlags supportedUsages, const GrVkImageInfo& info, sk_sp<skgpu::MutableTextureState> mutableState,
    sk_sp<const GrVkImageView> framebufferView, sk_sp<const GrVkImageView> textureView, GrBackendObjectOwnership ownership, GrWrapCacheable cacheable,
    bool forSecondaryCB, std::string_view label)
    : GrAttachment(
        gpu, dimensions, supportedUsages, info.fSampleCount, info.fLevelCount > 1 ? skgpu::Mipmapped::kYes : skgpu::Mipmapped::kNo, info.fProtected, label)
    , fInfo(info)
    , fInitialQueueFamily(info.fCurrentQueueFamily)
    , fMutableState(std::move(mutableState))
    , fFramebufferView(std::move(framebufferView))
...
void GrVkImage::init(GrVkGpu* gpu, bool forSecondaryCB)
{
    ...
    if (fInfo.fImageUsageFlags & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) {
        ...
        if (fInfo.fAlloc.fFlags & skgpu::VulkanAlloc::kLazilyAllocated_Flag) {
            ...
            SkASSERT(
                !SkToBool(fInfo.fImageUsageFlags & VK_IMAGE_USAGE_TRANSFER_DST_BIT) && !SkToBool(fInfo.fImageUsageFlags & VK_IMAGE_USAGE_TRANSFER_SRC_BIT));
        } else {
            SkASSERT(!SkToBool(fInfo.fImageUsageFlags & VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT));
            SkASSERT(SkToBool(fInfo.fImageUsageFlags & VK_IMAGE_USAGE_TRANSFER_DST_BIT) && SkToBool(fInfo.fImageUsageFlags & VK_IMAGE_USAGE_TRANSFER_SRC_BIT));
        }
    }
    ...
    if (fInfo.fCurrentQueueFamily != VK_QUEUE_FAMILY_IGNORED && fInfo.fCurrentQueueFamily != VK_QUEUE_FAMILY_EXTERNAL
...
VkPipelineStageFlags GrVkImage::LayoutToPipelineSrcStageFlags(const VkImageLayout layout)
{
    ...
    } else if (VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL == layout || VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL == layout) {
...
VkAccessFlags GrVkImage::LayoutToSrcAccessMask(const VkImageLayout layout)
{
    ...
    if (VK_IMAGE_LAYOUT_GENERAL == layout) {
        flags = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT | VK_ACCESS_TRANSFER_WRITE_BIT | VK_ACCESS_HOST_WRITE_BIT;
    } else if (VK_IMAGE_LAYOUT_PREINITIALIZED == layout) {
    ...
    } else if (VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL == layout || VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL == layout
...
void GrVkImage::setImageLayoutAndQueueIndex(
    const GrVkGpu* gpu, VkImageLayout newLayout, VkAccessFlags dstAccessMask, VkPipelineStageFlags dstStageMask, bool byRegion, uint32_t newQueueFamilyIndex)
{
// Enable the following block to test new devices to confirm their lazy images stay at 0 memory use.
    ...
    SkASSERT(newLayout == this->currentLayout() || (VK_IMAGE_LAYOUT_UNDEFINED != newLayout && VK_IMAGE_LAYOUT_PREINITIALIZED != newLayout));
    ...
    if (fInfo.fSharingMode == VK_SHARING_MODE_CONCURRENT) {
        if (newQueueFamilyIndex == VK_QUEUE_FAMILY_IGNORED) {
            SkASSERT(currentQueueIndex == VK_QUEUE_FAMILY_IGNORED || currentQueueIndex == VK_QUEUE_FAMILY_EXTERNAL
                || currentQueueIndex == VK_QUEUE_FAMILY_FOREIGN_EXT);
        } else {
            SkASSERT(newQueueFamilyIndex == VK_QUEUE_FAMILY_EXTERNAL || newQueueFamilyIndex == VK_QUEUE_FAMILY_FOREIGN_EXT);
        ...
        if (newQueueFamilyIndex == VK_QUEUE_FAMILY_IGNORED || currentQueueIndex == gpu->queueIndex()) {
            SkASSERT(currentQueueIndex == VK_QUEUE_FAMILY_IGNORED || currentQueueIndex == VK_QUEUE_FAMILY_EXTERNAL
                || currentQueueIndex == VK_QUEUE_FAMILY_FOREIGN_EXT || currentQueueIndex == gpu->queueIndex());
        } else if (newQueueFamilyIndex == VK_QUEUE_FAMILY_EXTERNAL || newQueueFamilyIndex == VK_QUEUE_FAMILY_FOREIGN_EXT) {
            SkASSERT(currentQueueIndex == VK_QUEUE_FAMILY_IGNORED || currentQueueIndex == gpu->queueIndex());
        }
    }
#endif
    ...
    if (newLayout == currentLayout && currentQueueIndex == newQueueFamilyIndex
        && (VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL == currentLayout || VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL == currentLayout
    ...
    gpu->addImageMemoryBarrier(this->resource(), srcStageMask, dstStageMask, byRegion, &imageMemoryBarrier);
...
bool GrVkImage::InitImageInfo(GrVkGpu* gpu, const ImageDesc& imageDesc, GrVkImageInfo* info)
{
    ...
    if ((imageDesc.fIsProtected == GrProtected::kYes) && !gpu->vkCaps().supportsProtectedContent()) {
        return false;
    }

    bool isLinear = VK_IMAGE_TILING_LINEAR == imageDesc.fImageTiling;
    VkImageLayout initialLayout = isLinear ? VK_IMAGE_LAYOUT_PREINITIALIZED : VK_IMAGE_LAYOUT_UNDEFINED;
    ...
    skgpu::Protected isProtected = gpu->protectedContext() ? skgpu::Protected::kYes : skgpu::Protected::kNo;
    bool forceDedicatedMemory = gpu->vkCaps().shouldAlwaysUseDedicatedImageMemory();
    bool useLazyAllocation = SkToBool(imageDesc.fUsageFlags & VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT);

    auto checkResult = [gpu, isProtected, forceDedicatedMemory, useLazyAllocation](VkResult result) {
        GR_VK_LOG_IF_NOT_SUCCESS(gpu, result,
            "skgpu::VulkanMemory::AllocImageMemory"
            " (isProtected:%d, forceDedicatedMemory:%d, useLazyAllocation:%d)",
            (int)isProtected, (int)forceDedicatedMemory, (int)useLazyAllocation);
        return gpu->checkVkResult(result);
    };
    auto allocator = gpu->memoryAllocator();
    skgpu::VulkanAlloc alloc;
    if (!skgpu::VulkanMemory::AllocImageMemory(allocator, image, isProtected, forceDedicatedMemory, useLazyAllocation, checkResult, &alloc)
        || (useLazyAllocation && !SkToBool(alloc.fFlags & skgpu::VulkanAlloc::kLazilyAllocated_Flag))) {
        VK_CALL(gpu, DestroyImage(gpu->device(), image, nullptr));
        return false;
    }

    // Bind buffer
    GR_VK_CALL_RESULT(gpu, result, BindImageMemory(gpu->device(), image, alloc.fMemory, alloc.fOffset));
    if (result) {
    ...
    info->fProtected = (createflags & VK_IMAGE_CREATE_PROTECTED_BIT) ? GrProtected::kYes : GrProtected::kNo;
...
void GrVkImage::prepareForPresent(GrVkGpu* gpu)
{
    ...
    if (fInitialQueueFamily != VK_QUEUE_FAMILY_EXTERNAL && fInitialQueueFamily != VK_QUEUE_FAMILY_FOREIGN_EXT) {
        if (gpu->vkCaps().supportsSwapchain()) {
            layout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
        }
    }
    this->setImageLayoutAndQueueIndex(gpu, layout, 0, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, false, fInitialQueueFamily);
}

void GrVkImage::prepareForExternal(GrVkGpu* gpu)
{
    this->setImageLayoutAndQueueIndex(gpu, this->currentLayout(), 0, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, false, fInitialQueueFamily);
}
...
gr_rp<const GrVkDescriptorSet> GrVkImage::inputDescSetForBlending(GrVkGpu* gpu)
{
    ...
    write_input_desc_set(gpu, this->framebufferView()->imageView(), VK_IMAGE_LAYOUT_GENERAL, *fCachedBlendingInputDescSet->descriptorSet());
...
gr_rp<const GrVkDescriptorSet> GrVkImage::inputDescSetForMSAALoad(GrVkGpu* gpu)
{
    ...
    write_input_desc_set(gpu, this->framebufferView()->imageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, *fCachedMSAALoadInputDescSet->descriptorSet());
