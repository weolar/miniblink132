...
bool GrVkMSAALoadManager::createMSAALoadProgram(GrVkGpu* gpu)
{
    ...
    vertShaderText.append("layout(vulkan, set=0, binding=0) uniform vertexUniformBuffer {"
                          "half4 uPosXform;"
                          "};"
    ...
    fragShaderText.append("layout(vulkan, input_attachment_index=0, set=2, binding=0) subpassInput uInput;"
    ...
    if (!GrCompileVkShaderModule(gpu, vertShaderText, VK_SHADER_STAGE_VERTEX_BIT, &fVertShaderModule, &fShaderStageInfo[0], settings, &spirv, &interface)) {
        this->destroyResources(gpu);
        return false;
    }
    SkASSERT(interface == SkSL::Program::Interface());

    if (!GrCompileVkShaderModule(gpu, fragShaderText, VK_SHADER_STAGE_FRAGMENT_BIT, &fFragShaderModule, &fShaderStageInfo[1], settings, &spirv, &interface)) {
        this->destroyResources(gpu);
        return false;
    }
    SkASSERT(interface == SkSL::Program::Interface());
    ...
    dsLayout[GrVkUniformHandler::kSamplerDescSet] = resourceProvider.getSamplerDSLayout(samplerHandle);
    ...
    VkResult err = GR_VK_CALL(gpu->vkInterface(), CreatePipelineLayout(gpu->device(), &layoutCreateInfo, nullptr, &fPipelineLayout));
    if (err) {
        this->destroyResources(gpu);
        return false;
    }
...
bool GrVkMSAALoadManager::loadMSAAFromResolve(
    GrVkGpu* gpu, GrVkCommandBuffer* commandBuffer, const GrVkRenderPass& renderPass, GrAttachment* dst, GrVkImage* src, const SkIRect& rect)
{
    if (!dst) {
        return false;
    }
    ...
    sk_sp<const GrVkPipeline> pipeline = resourceProv.findOrCreateMSAALoadPipeline(renderPass, dst->numSamples(), fShaderStageInfo, fPipelineLayout);
    if (!pipeline) {
        return false;
    }
    ...
    // data?
    sk_sp<GrGpuBuffer> uniformBuffer = resourceProvider->createBuffer(uniData, sizeof(uniData), GrGpuBufferType::kUniform, kDynamic_GrAccessPattern);
    if (!uniformBuffer) {
        return false;
    }
    ...
    commandBuffer->bindDescriptorSets(gpu, fPipelineLayout, GrVkUniformHandler::kUniformBufferDescSet,
    ...
    commandBuffer->bindDescriptorSets(gpu, fPipelineLayout, GrVkUniformHandler::kInputDescSet, /*setCount=*/1, inputDS->descriptorSet(),
        /*dynamicOffsetCount=*/0, /*dynamicOffsets=*/nullptr);
