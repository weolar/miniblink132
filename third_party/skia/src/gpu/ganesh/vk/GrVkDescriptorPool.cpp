...
GrVkDescriptorPool* GrVkDescriptorPool::Create(GrVkGpu* gpu, VkDescriptorType type, uint32_t count)
{
...
GrVkDescriptorPool::GrVkDescriptorPool(const GrVkGpu* gpu, VkDescriptorPool pool, VkDescriptorType type, uint32_t count)
    : INHERITED(gpu)
    , fType(type)
    , fCount(count)
    , fDescPool(pool)
