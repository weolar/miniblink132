...
public:
    ...
    static sk_sp<const GrVkImageView> Make(
        GrVkGpu* gpu, VkImage image, VkFormat format, Type viewType, uint32_t miplevels, const skgpu::VulkanYcbcrConversionInfo& ycbcrInfo);
...
private:
    GrVkImageView(const GrVkGpu* gpu, VkImageView imageView, GrVkSamplerYcbcrConversion* ycbcrConversion)
        : INHERITED(gpu)
