...
GrVkSampler* GrVkSampler::Create(GrVkGpu* gpu, GrSamplerState samplerState, const skgpu::VulkanYcbcrConversionInfo& ycbcrInfo)
{
    ...
    createInfo.maxAnisotropy = std::min(static_cast<float>(samplerState.maxAniso()), gpu->vkCaps().maxSamplerAnisotropy());
    ...
    if (ycbcrInfo.isValid()) {
        ...
        ycbcrConversion = gpu->resourceProvider().findOrCreateCompatibleSamplerYcbcrConversion(ycbcrInfo);
        if (!ycbcrConversion) {
            return nullptr;
        }
...
GrVkSampler::Key GrVkSampler::GenerateKey(GrSamplerState samplerState, const skgpu::VulkanYcbcrConversionInfo& ycbcrInfo)
{
    ...
    return { samplerState.asKey(/*anisoIsOrthogonal=*/true), GrVkSamplerYcbcrConversion::GenerateKey(ycbcrInfo) };
}
