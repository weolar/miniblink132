...
GrVkDescriptorSetManager* GrVkDescriptorSetManager::CreateSamplerManager(GrVkGpu* gpu, VkDescriptorType type, const GrVkUniformHandler& uniformHandler)
{
...
static bool get_layout_and_desc_count(GrVkGpu* gpu, VkDescriptorType type, const TArray<uint32_t>& visibilities,
    const TArray<const GrVkSampler*>& immutableSamplers, VkDescriptorSetLayout* descSetLayout, uint32_t* descCountPerSet)
{
    if (VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER == type || VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER == type) {
        uint32_t numBindings = visibilities.size();
        std::unique_ptr<VkDescriptorSetLayoutBinding[]> dsSamplerBindings(new VkDescriptorSetLayoutBinding[numBindings]);
        *descCountPerSet = 0;
...
#endif
        VkResult result;
        GR_VK_CALL_RESULT(gpu, result, CreateDescriptorSetLayout(gpu->device(), &dsSamplerLayoutCreateInfo, nullptr, descSetLayout));
...
#endif
        VkResult result;
        GR_VK_CALL_RESULT(gpu, result, CreateDescriptorSetLayout(gpu->device(), &uniformLayoutCreateInfo, nullptr, descSetLayout));
        if (result != VK_SUCCESS) {
            return false;
        }
...
#endif
        VkResult result;
        GR_VK_CALL_RESULT(gpu, result, CreateDescriptorSetLayout(gpu->device(), &inputLayoutCreateInfo, nullptr, descSetLayout));
        if (result != VK_SUCCESS) {
            return false;
        }
...
GrVkDescriptorSetManager* GrVkDescriptorSetManager::Create(
    GrVkGpu* gpu, VkDescriptorType type, const TArray<uint32_t>& visibilities, const TArray<const GrVkSampler*>& immutableSamplers)
{
#ifdef SK_DEBUG
    ...
    if (!get_layout_and_desc_count(gpu, type, visibilities, immutableSamplers, &descSetLayout, &descCountPerSet)) {
        return nullptr;
    }
    return new GrVkDescriptorSetManager(gpu, type, descSetLayout, descCountPerSet, visibilities, immutableSamplers);
}

GrVkDescriptorSetManager::GrVkDescriptorSetManager(GrVkGpu* gpu, VkDescriptorType type, VkDescriptorSetLayout descSetLayout, uint32_t descCountPerSet,
    const TArray<uint32_t>& visibilities, const TArray<const GrVkSampler*>& immutableSamplers)
...
const GrVkDescriptorSet* GrVkDescriptorSetManager::getDescriptorSet(GrVkGpu* gpu, const Handle& handle)
{
...
bool GrVkDescriptorSetManager::isCompatible(VkDescriptorType type, const GrVkUniformHandler* uniHandler) const
{
    SkASSERT(uniHandler);
    ...
    for (int i = 0; i < uniHandler->numSamplers(); ++i) {
        if (uniHandler->samplerVisibility(i) != fBindingVisibilities[i] || uniHandler->immutableSampler(i) != fImmutableSamplers[i]) {
            return false;
        }
    }
    return true;
...
GrVkDescriptorSetManager::DescriptorPoolManager::DescriptorPoolManager(VkDescriptorSetLayout layout, VkDescriptorType type, uint32_t descCountPerSet)
    : fDescLayout(layout)
    , fDescType(type)
...
bool GrVkDescriptorSetManager::DescriptorPoolManager::getNewDescriptorSet(GrVkGpu* gpu, VkDescriptorSet* ds)
{
    if (!fMaxDescriptors) {
        return false;
    }
