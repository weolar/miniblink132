...
GrVkBuffer::GrVkBuffer(GrVkGpu* gpu, size_t sizeInBytes, GrGpuBufferType bufferType, GrAccessPattern accessPattern, VkBuffer buffer,
    const skgpu::VulkanAlloc& alloc, const GrVkDescriptorSet* uniformDescriptorSet, std::string_view label)
...
static const GrVkDescriptorSet* make_uniform_desc_set(GrVkGpu* gpu, VkBuffer buffer, size_t size)
{
    ...
    GR_VK_CALL(gpu->vkInterface(), UpdateDescriptorSets(gpu->device(), 1, &descriptorWrite, 0, nullptr));
    return descriptorSet;
}

sk_sp<GrVkBuffer> GrVkBuffer::Make(GrVkGpu* gpu, size_t size, GrGpuBufferType bufferType, GrAccessPattern accessPattern)
{
    VkBuffer buffer;
    skgpu::VulkanAlloc alloc;

    bool isProtected = gpu->protectedContext() && accessPattern == kStatic_GrAccessPattern;
    ...
    bool requiresMappable = !isProtected
        && (accessPattern == kDynamic_GrAccessPattern || accessPattern == kStream_GrAccessPattern || !gpu->vkCaps().gpuOnlyBuffersMorePerformant());
    ...
    // Bind buffer
    GR_VK_CALL_RESULT(gpu, err, BindBufferMemory(gpu->device(), buffer, alloc.fMemory, alloc.fOffset));
    if (err) {
    ...
    return sk_sp<GrVkBuffer>(new GrVkBuffer(gpu, size, bufferType, accessPattern, buffer, alloc, uniformDescSet,
        /*label=*/"MakeVkBuffer"));
}
...
void GrVkBuffer::vkMap(size_t readOffset, size_t readSize)
{
    ...
    if (this->isVkMappable()) {
        ...
        if (fMapPtr && readSize != 0) {
            ...
            skgpu::VulkanMemory::InvalidateMappedAlloc(allocator, fAlloc, readOffset, readSize, checkResult_invalidateMapAlloc);
        }
    }
}
...
void GrVkBuffer::copyCpuDataToGpuBuffer(const void* src, size_t offset, size_t size)
{
    ...
    if ((size <= 65536) && SkIsAlign4(size) && SkIsAlign4(offset) && !gpu->vkCaps().avoidUpdateBuffers()) {
        gpu->updateBuffer(sk_ref_sp(this), src, offset, size);
    } else {
        GrResourceProvider* resourceProvider = gpu->getContext()->priv().resourceProvider();
        sk_sp<GrGpuBuffer> transferBuffer = resourceProvider->createBuffer(src, size, GrGpuBufferType::kXferCpuToGpu, kDynamic_GrAccessPattern);
        if (!transferBuffer) {
            return;
        }

        gpu->transferFromBufferToBuffer(std::move(transferBuffer),
            /*srcOffset=*/0, sk_ref_sp(this), offset, size);
    }
}

void GrVkBuffer::addMemoryBarrier(
    VkAccessFlags srcAccessMask, VkAccessFlags dstAccesMask, VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask, bool byRegion) const
{
    ...
    this->getVkGpu()->addBufferMemoryBarrier(srcStageMask, dstStageMask, byRegion, &bufferMemoryBarrier);
}
...
{
    return this->getVkGpu()->zeroBuffer(sk_ref_sp(this));
}
