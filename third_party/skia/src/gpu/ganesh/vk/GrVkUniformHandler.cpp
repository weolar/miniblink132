...
static uint32_t get_aligned_offset(uint32_t* currentOffset, SkSLType type, int arrayCount, int layout)
{
    ...
    if (layout == GrVkUniformHandler::kStd140Layout && (arrayCount || type == SkSLType::kFloat2x2 || type == SkSLType::kHalf2x2)) {
        alignmentMask = 0xF;
    }
...
GrGLSLUniformHandler::UniformHandle GrVkUniformHandler::internalAddUniformArray(
    const GrProcessor* owner, uint32_t visibility, SkSLType type, const char* name, bool mangleName, int arrayCount, const char** outName)
{
    ...
    tempInfo.fVariable = GrShaderVar { std::move(resolvedName), type, GrShaderVar::TypeModifier::None, arrayCount };
    ...
    for (int layout = 0; layout < kLayoutCount; ++layout) {
        tempInfo.fOffsets[layout] = get_aligned_offset(&fCurrentOffsets[layout], type, arrayCount, layout);
    }
...
GrGLSLUniformHandler::SamplerHandle GrVkUniformHandler::addSampler(
    const GrBackendFormat& backendFormat, GrSamplerState state, const skgpu::Swizzle& swizzle, const char* name, const GrShaderCaps* shaderCaps)
{
    SkASSERT(name && strlen(name));
    ...
    VkUniformInfo tempInfo;
    tempInfo.fVariable = GrShaderVar { std::move(mangleName), SkSLCombinedSamplerTypeForTextureType(backendFormat.textureType()),
        GrShaderVar::TypeModifier::None, GrShaderVar::kNonArray, std::move(layoutQualifier), SkString() };
...
GrGLSLUniformHandler::SamplerHandle GrVkUniformHandler::addInputSampler(const skgpu::Swizzle& swizzle, const char* name)
{
    SkASSERT(name && strlen(name));
    ...
    auto layoutQualifier = SkStringPrintf("vulkan, input_attachment_index=%d, set=%d, binding=%d", kDstInputAttachmentIndex, kInputDescSet, kInputBinding);

    fInputUniform = { GrShaderVar { std::move(mangleName), SkSLType::kInput, GrShaderVar::TypeModifier::None, GrShaderVar::kNonArray,
...
void GrVkUniformHandler::appendUniformDecls(GrShaderFlags visibility, SkString* out) const
{
    for (const VkUniformInfo& sampler : fSamplers.items()) {
        SkASSERT(sampler.fVariable.getType() == SkSLType::kTexture2DSampler || sampler.fVariable.getType() == SkSLType::kTextureExternalSampler);
    ...
    for (const VkUniformInfo& localUniform : fUniforms.items()) {
        if (!firstOffsetCheck) {
            ...
            SkASSERT(0 == localUniform.fOffsets[kStd140Layout] && 0 == localUniform.fOffsets[kStd430Layout]);
    ...
    if (!uniformsString.isEmpty()) {
        if (fUsePushConstants) {
            ...
            out->appendf("layout (vulkan, set=%d, binding=%d) ", kUniformBufferDescSet, kUniformBinding);
        }
...
void GrVkUniformHandler::determineIfUsePushConstants() const
{
    ...
    fUsePushConstants = fCurrentOffsets[kStd430Layout] > 0 && fCurrentOffsets[kStd430Layout] + kPad <= fProgramBuilder->caps()->maxPushConstantsSize();
}
