...
public:
    ...
    bool set(GrRenderTarget*, sk_sp<GrVkFramebuffer>, GrSurfaceOrigin, const SkIRect& bounds, const GrOpsRenderPass::LoadAndStoreInfo&,
        const GrOpsRenderPass::StencilLoadAndStoreInfo&, const GrOpsRenderPass::LoadAndStoreInfo& resolveInfo, GrVkRenderPass::SelfDependencyFlags selfDepFlags,
        GrVkRenderPass::LoadFromResolve loadFromResolve, const skia_private::TArray<GrSurfaceProxy*, true>& sampledProxies);
    void reset();
...
private:
    bool init(const GrOpsRenderPass::LoadAndStoreInfo& colorInfo, const GrOpsRenderPass::LoadAndStoreInfo& resolveInfo,
    ...
    void onBindBuffers(
        sk_sp<const GrBuffer> indexBuffer, sk_sp<const GrBuffer> instanceBuffer, sk_sp<const GrBuffer> vertexBuffer, GrPrimitiveRestart) override;
