...
public:
    ...
    void pipelineBarrier(const GrVkGpu* gpu, const GrManagedResource* resource, VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask,
        bool byRegion, BarrierType barrierType, void* barrier);
    ...
    void bindDescriptorSets(const GrVkGpu* gpu, VkPipelineLayout layout, uint32_t firstSet, uint32_t setCount, const VkDescriptorSet* descriptorSets,
...
protected:
    template <typename T> using TrackedResourceArray = skia_private::STArray<kInitialTrackedResourcesCount, T>;
...
public:
    ...
    // in the render pass.
    bool beginRenderPass(GrVkGpu* gpu, const GrVkRenderPass*, sk_sp<const GrVkFramebuffer>, const VkClearValue clearValues[], const GrSurface* target,
    ...
    void copyImage(const GrVkGpu* gpu, GrVkImage* srcImage, VkImageLayout srcLayout, GrVkImage* dstImage, VkImageLayout dstLayout, uint32_t copyRegionCount,
        const VkImageCopy* copyRegions);

    void blitImage(const GrVkGpu* gpu, const GrManagedResource* srcResource, VkImage srcImage, VkImageLayout srcLayout, const GrManagedResource* dstResource,
        VkImage dstImage, VkImageLayout dstLayout, uint32_t blitRegionCount, const VkImageBlit* blitRegions, VkFilter filter);
    ...
    void copyImageToBuffer(const GrVkGpu* gpu, GrVkImage* srcImage, VkImageLayout srcLayout, sk_sp<GrGpuBuffer> dstBuffer, uint32_t copyRegionCount,
    ...
    bool submitToQueue(GrVkGpu* gpu, VkQueue queue, skia_private::TArray<GrVkSemaphore::Resource*>& signalSemaphores,
        skia_private::TArray<GrVkSemaphore::Resource*>& waitSemaphores, const GrSubmitInfo&);
...
public:
    ...
    static GrVkSecondaryCommandBuffer* Create(VkCommandBuffer externalSecondaryCB, const GrVkRenderPass* externalRenderPass);
...
private:
    explicit GrVkSecondaryCommandBuffer(VkCommandBuffer cmdBuffer, const GrVkRenderPass* externalRenderPass)
