...
void GrGradientBitmapCache::fillGradient(const SkPMColor4f* colors, const SkScalar* positions, int count, bool colorsAreOpaque,
    const SkGradientShader::Interpolation& interpolation, const SkColorSpace* intermediateColorSpace, const SkColorSpace* dstColorSpace, SkBitmap* bitmap)
{
    ...
    SkGradientBaseShader::AppendInterpolatedToDstStages(&p, &alloc, colorsAreOpaque, interpolation, intermediateColorSpace, dstColorSpace);
    p.appendStore(bitmap->colorType(), &ctx);
    p.run(0, 0, bitmap->width(), 1);
}

void GrGradientBitmapCache::getGradient(const SkPMColor4f* colors, const SkScalar* positions, int count, bool colorsAreOpaque,
    const SkGradientShader::Interpolation& interpolation, const SkColorSpace* intermediateColorSpace, const SkColorSpace* dstColorSpace, SkColorType colorType,
    SkAlphaType alphaType, SkBitmap* bitmap)
{
    ...
    *buffer++ = static_cast<int32_t>(interpolation.fHueMethod);
    ...
    if (!this->find(storage.get(), size, bitmap)) {
        ...
        this->fillGradient(colors, positions, count, colorsAreOpaque, interpolation, intermediateColorSpace, dstColorSpace, bitmap);
        bitmap->setImmutable();
