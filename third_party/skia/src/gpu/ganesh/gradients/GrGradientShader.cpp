...
static std::unique_ptr<GrFragmentProcessor> make_textured_colorizer(const SkPMColor4f* colors, const SkScalar* positions, int count, bool colorsAreOpaque,
    const SkGradientShader::Interpolation& interpolation, const SkColorSpace* intermediateColorSpace, const SkColorSpace* dstColorSpace, const GrFPArgs& args)
{
    ...
    if (GrColorTypeIsWiderThan(args.fDstColorInfo->colorType(), 8)) {
        auto f16Format = args.fContext->priv().caps()->getDefaultBackendFormat(GrColorType::kRGBA_F16, GrRenderable::kNo);
        if (f16Format.isValid()) {
            colorType = kRGBA_F16_SkColorType;
        }
    }
    SkAlphaType alphaType = static_cast<bool>(interpolation.fInPremul) ? kPremul_SkAlphaType : kUnpremul_SkAlphaType;

    SkBitmap bitmap;
    gCache.getGradient(colors, positions, count, colorsAreOpaque, interpolation, intermediateColorSpace, dstColorSpace, colorType, alphaType, &bitmap);
    SkASSERT(1 == bitmap.height() && SkIsPow2(bitmap.width()));
    SkASSERT(bitmap.isImmutable());

    auto view = std::get<0>(GrMakeCachedBitmapProxyView(args.fContext, bitmap, /*label=*/"MakeTexturedColorizer", skgpu::Mipmapped::kNo));
    if (!view) {
...
static std::unique_ptr<GrFragmentProcessor> make_single_interval_colorizer(const SkPMColor4f& start, const SkPMColor4f& end)
{
    static const SkRuntimeEffect* effect = SkMakeRuntimeEffect(SkRuntimeEffect::MakeForShader,
        ...
        "}");
    return GrSkSLFP::Make(effect, "SingleIntervalColorizer", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "start", start, "end", end);
}
    ...
    const SkPMColor4f& c0, const SkPMColor4f& c1, const SkPMColor4f& c2, const SkPMColor4f& c3, float threshold)
{
    ...
    return GrSkSLFP::Make(
        effect, "DualIntervalColorizer", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "scale", SkSpan(scale), "bias", SkSpan(bias), "threshold", threshold);
}
...
static std::unique_ptr<GrFragmentProcessor> make_unrolled_colorizer(
    int intervalCount, const SkPMColor4f* scale, const SkPMColor4f* bias, SkRect thresholds1_7, SkRect thresholds9_13)
{
    ...
    once[intervalCount - 1]([intervalCount] {
        ...
        sksl.appendf("half4 main(float2 coord) {"
                     "half t = half(coord.x);"
                     "float4 s, b;"
                     ...
                     "return t * s + b;"
                     "}",
            intervalCount, intervalCount, intervalCount, (intervalCount <= 0) ? "" : "s = scale[0]; b = bias[0];",
            (intervalCount <= 1) ? "" : "s = scale[1]; b = bias[1];", intervalCount, (intervalCount <= 2) ? "" : "s = scale[2]; b = bias[2];",
            (intervalCount <= 3) ? "" : "s = scale[3]; b = bias[3];", intervalCount, intervalCount, (intervalCount <= 4) ? "" : "s = scale[4]; b = bias[4];",
            (intervalCount <= 5) ? "" : "s = scale[5]; b = bias[5];", intervalCount, (intervalCount <= 6) ? "" : "s = scale[6]; b = bias[6];",
    ...
    return GrSkSLFP::Make(effects[intervalCount - 1], "UnrolledBinaryColorizer",
        /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "thresholds1_7", thresholds1_7, "thresholds9_13", thresholds9_13, "scale", SkSpan(scale, intervalCount),
...
static std::unique_ptr<GrFragmentProcessor> make_looping_colorizer(
    int intervalCount, const SkPMColor4f* scale, const SkPMColor4f* bias, const SkScalar* thresholds)
{
    ...
    return GrSkSLFP::Make(cacheEntry->effect, "LoopingBinaryColorizer",
        /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "thresholds", SkSpan((const SkV4*)thresholds, intervalChunks), "scale", SkSpan(scale, intervalCount),
        "bias", SkSpan(bias, intervalCount));
}
...
int build_intervals(int inputLength, const SkPMColor4f* inColors, const SkScalar* inPositions, int outputLength, SkPMColor4f* outScales, SkPMColor4f* outBiases,
...
static std::unique_ptr<GrFragmentProcessor> make_unrolled_binary_colorizer(const SkPMColor4f* colors, const SkScalar* positions, int count)
{
    ...
    int intervalCount = build_intervals(count, colors, positions, kMaxUnrolledIntervalCount, scales, biases, thresholds);
    if (intervalCount <= 0) {
        return nullptr;
    }
...
static std::unique_ptr<GrFragmentProcessor> make_looping_binary_colorizer(const SkPMColor4f* colors, const SkScalar* positions, int count)
{
    ...
    int intervalCount = build_intervals(count, colors, positions, kMaxLoopingIntervalCount, scales, biases, thresholds);
    if (intervalCount <= 0) {
        return nullptr;
    }
...
// the gradient.
static std::unique_ptr<GrFragmentProcessor> make_uniform_colorizer(
    const SkPMColor4f* colors, const SkScalar* positions, int count, bool premul, const GrFPArgs& args)
{
    ...
    int binaryColorizerLimit = caps->fNonconstantArrayIndexSupport ? kMaxLoopingColorCount : kMaxUnrolledColorCount;
    if ((count <= binaryColorizerLimit) && !intervalsExceedPrecisionLimit()) {
        ...
        colorizer = caps->fNonconstantArrayIndexSupport ? make_looping_binary_colorizer(colors, positions, count)
...
static std::unique_ptr<GrFragmentProcessor> make_clamped_gradient(std::unique_ptr<GrFragmentProcessor> colorizer,
    std::unique_ptr<GrFragmentProcessor> gradLayout, SkPMColor4f leftBorderColor, SkPMColor4f rightBorderColor, bool colorsAreOpaque)
{
    static const SkRuntimeEffect* effect = SkMakeRuntimeEffect(SkRuntimeEffect::MakeForShader,
    ...
    return GrSkSLFP::Make(effect, "ClampedGradient", /*inputFP=*/nullptr, optFlags, "colorizer", GrSkSLFP::IgnoreOptFlags(std::move(colorizer)), "gradLayout",
        GrSkSLFP::IgnoreOptFlags(std::move(gradLayout)), "leftBorderColor", leftBorderColor, "rightBorderColor", rightBorderColor, "layoutPreservesOpacity",
        GrSkSLFP::Specialize<int>(layoutPreservesOpacity));
}

static std::unique_ptr<GrFragmentProcessor> make_tiled_gradient(
    const GrFPArgs& args, std::unique_ptr<GrFragmentProcessor> colorizer, std::unique_ptr<GrFragmentProcessor> gradLayout, bool mirror, bool colorsAreOpaque)
{
    static const SkRuntimeEffect* effect = SkMakeRuntimeEffect(SkRuntimeEffect::MakeForShader,
        "uniform shader colorizer;"
        "uniform shader gradLayout;"
    ...
    const bool useFloorAbsWorkaround = args.fContext->priv().caps()->shaderCaps()->fMustDoOpBetweenFloorAndAbs;

    return GrSkSLFP::Make(effect, "TiledGradient", /*inputFP=*/nullptr, optFlags, "colorizer", GrSkSLFP::IgnoreOptFlags(std::move(colorizer)), "gradLayout",
        GrSkSLFP::IgnoreOptFlags(std::move(gradLayout)), "mirror", GrSkSLFP::Specialize<int>(mirror), "layoutPreservesOpacity",
        GrSkSLFP::Specialize<int>(layoutPreservesOpacity), "useFloorAbsWorkaround", GrSkSLFP::Specialize<int>(useFloorAbsWorkaround));
}

static std::unique_ptr<GrFragmentProcessor> make_interpolated_to_dst(std::unique_ptr<GrFragmentProcessor> gradient,
    const SkGradientShader::Interpolation& interpolation, SkColorSpace* intermediateColorSpace, const GrColorInfo& dstInfo, bool allOpaque)
{
    ...
    case ColorSpace::kHWB:
        ...
        gradient = GrSkSLFP::Make(effect, "GradientCS", std::move(gradient), GrSkSLFP::OptFlags::kAll, "colorSpace",
            GrSkSLFP::Specialize<int>(static_cast<int>(interpolation.fColorSpace)), "do_unpremul", GrSkSLFP::Specialize<int>(inputPremul && !allOpaque));
    ...
    return GrColorSpaceXformEffect::Make(std::move(gradient), intermediateColorSpace, intermediateAlphaType, dstColorSpace, dstAlphaType);
}
...
std::unique_ptr<GrFragmentProcessor> MakeGradientFP(const SkGradientBaseShader& shader, const GrFPArgs& args, const SkShaders::MatrixRec& mRec,
    std::unique_ptr<GrFragmentProcessor> layout, const SkMatrix* overrideMatrix)
{
    ...
    // front if needed.
    SkColor4fXformer xformedColors(&shader, args.fDstColorInfo->colorSpace(), /*forceExplicitPositions=*/true);
    ...
    std::unique_ptr<GrFragmentProcessor> colorizer = make_uniform_colorizer(colors, positions, colorCount, shader.interpolateInPremul(), args);
    ...
    if (colorizer) {
        ...
        colorizer = make_interpolated_to_dst(
            std::move(colorizer), shader.fInterpolation, xformedColors.fIntermediateColorSpace.get(), *args.fDstColorInfo, allOpaque);
    } else {
        ...
        // hard-stops.)
        colorizer = make_textured_colorizer(colors, positions, colorCount, allOpaque, shader.fInterpolation, xformedColors.fIntermediateColorSpace.get(),
            args.fDstColorInfo->colorSpace(), args);
    }
    ...
    case SkTileMode::kClamp: {
        ...
        SkGradientBaseShader::AppendInterpolatedToDstStages(
            &p, &alloc, allOpaque, shader.fInterpolation, xformedColors.fIntermediateColorSpace.get(), args.fDstColorInfo->colorSpace());
        p.append(SkRasterPipelineOp::store_f32, &ctx);
        p.run(0, 0, 2, 1);

        gradient = make_clamped_gradient(std::move(colorizer), std::move(layout), borderColors[0], borderColors[1], allOpaque);
        break;
    }
    case SkTileMode::kDecal:
        // Even if the gradient colors are opaque, the decal borders are transparent so
        // disable that optimization
        gradient = make_clamped_gradient(std::move(colorizer), std::move(layout), SK_PMColor4fTRANSPARENT, SK_PMColor4fTRANSPARENT,
...
std::unique_ptr<GrFragmentProcessor> MakeLinear(const SkLinearGradient& shader, const GrFPArgs& args, const SkShaders::MatrixRec& mRec)
{
    ...
    auto fp = GrSkSLFP::Make(effect, "LinearLayout", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kPreservesOpaqueInput);
