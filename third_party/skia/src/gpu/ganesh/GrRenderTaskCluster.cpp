...
static void validate(SkSpan<const sk_sp<GrRenderTask>> input, const SkTInternalLList<GrRenderTask>& llist)
{
    ...
    for (GrRenderTask* t : llist) {
        ...
        for (GrRenderTask* dep : t->dependencies()) {
            SkASSERTF(seen.contains(dep), "%s came before dependency %s", describe_task(t).c_str(), describe_task(dep).c_str());
        }
    }
...
static bool task_cluster_visit(GrRenderTask* task, SkTInternalLList<GrRenderTask>* llist, THashMap<GrSurfaceProxy*, GrRenderTask*>* lastTaskMap)
{
    ...
    while (clusterHead->fPrev && 1 == clusterHead->fPrev->numTargets() && target == first_target(clusterHead->fPrev)) {
    ...
    for (GrRenderTask* moved = movedHead; moved;) {
        CLUSTER_DEBUGF("Cluster: Reorder %s behind %s.\n", describe_task(moved).c_str(), describe_task(clusterHead).c_str());
