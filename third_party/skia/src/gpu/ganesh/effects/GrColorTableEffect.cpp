...
ColorTableEffect::ColorTableEffect(std::unique_ptr<GrFragmentProcessor> inputFP, GrSurfaceProxyView view)
    // Not bothering with table-specific optimizations.
    : GrFragmentProcessor(kColorTableEffect_ClassID, kNone_OptimizationFlags)
{
    this->registerChild(GrTextureEffect::Make(std::move(view), kUnknown_SkAlphaType), SkSL::SampleUsage::Explicit());
...
std::unique_ptr<GrFragmentProcessor> ColorTableEffect::Make(std::unique_ptr<GrFragmentProcessor> inputFP, GrRecordingContext* context, const SkBitmap& bitmap)
{
    SkASSERT(kPremul_SkAlphaType == bitmap.alphaType());
    SkASSERT(bitmap.isImmutable());

    auto view = std::get<0>(GrMakeCachedBitmapProxyView(context, bitmap,
        /*label=*/"MakeColorTableEffect", skgpu::Mipmapped::kNo));
    if (!view) {
        return nullptr;
    }

    return std::unique_ptr<GrFragmentProcessor>(new ColorTableEffect(std::move(inputFP), std::move(view)));
}
...
std::unique_ptr<GrFragmentProcessor> ColorTableEffect::TestCreate(GrProcessorTestData* d)
{
    ...
    auto filter(SkColorFilters::TableARGB((flags & (1 << 0)) ? luts[0] : nullptr, (flags & (1 << 1)) ? luts[1] : nullptr,
    ...
    auto [success, fp] = GrFragmentProcessors::Make(
        d->context(), filter.get(), d->inputFP(), GrColorInfo(GrColorType::kRGBA_8888, kUnknown_SkAlphaType, std::move(colorSpace)), props);
    SkASSERT(success);
    return std::move(fp);
}
#endif
