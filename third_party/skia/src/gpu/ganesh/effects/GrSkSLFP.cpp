...
public:
    void emitCode(EmitArgs& args) override
    {
        ...
        public:
            FPCallbacks(
                Impl* self, EmitArgs& args, const char* inputColor, const SkSL::Context& context, const uint8_t* uniformData, const Specialized* specialized)
                : fSelf(self)
                , fArgs(args)
            ...
            std::string declareUniform(const SkSL::VarDeclaration* decl) override
            {
                ...
                auto handle = fArgs.fUniformHandler->addUniformArray(&fArgs.fFp.cast<GrSkSLFP>(), kFragment_GrShaderFlag, gpuType,
            ...
            std::string sampleShader(int index, std::string coords) override
            {
                ...
                return child ? std::string(fSelf->invokeChild(index, fInputColor, fArgs, coords).c_str()) : std::string("half4(0)");
            }

            std::string sampleColorFilter(int index, std::string color) override
            {
                return std::string(fSelf->invokeChild(index, color.empty() ? fInputColor : color.c_str(), fArgs).c_str());
            }
            ...
            std::string toLinearSrgb(std::string color) override
            {
                ...
                SkString xformedColor = fSelf->invokeChild(fp.fToLinearSrgbChildIndex, color.c_str(), fArgs);
            ...
            std::string fromLinearSrgb(std::string color) override
            {
                ...
                color = SkSL::String::printf("(%s).rgb1", color.c_str());
                SkString xformedColor = fSelf->invokeChild(fp.fFromLinearSrgbChildIndex, color.c_str(), fArgs);
                return SkSL::String::printf("(%s).rgb", xformedColor.c_str());
            }
        ...
        if (fp.fEffect->allowBlender()) {
            ...
            if (fp.fDestColorChildIndex >= 0) {
                args.fFragBuilder->codeAppendf("%s = %s;\n", args.fDestColor, this->invokeChild(fp.fDestColorChildIndex, args.fDestColor, args).c_str());
            }
        } else {
        ...
        if (fp.fEffect->samplesOutsideMain()) {
            GrShaderVar inputColorCopy(args.fFragBuilder->getMangledFunctionName("inColor"), SkSLType::kHalf4);
        ...
        FPCallbacks callbacks(this, args, inputColorName.c_str(), *program.fContext, fp.uniformData(), fp.specialized());
...
private:
    void onSetData(const GrGLSLProgramDataManager& pdman, const GrFragmentProcessor& _proc) override
    {
        ...
        pdman.setRuntimeEffectUniforms(
            outer.fEffect->uniforms(), SkSpan(fUniformHandles), SkSpan(outer.specialized(), outer.uniformCount()), outer.uniformData());
    }

    std::vector<UniformHandle> fUniformHandles;
};

std::unique_ptr<GrSkSLFP> GrSkSLFP::MakeWithData(sk_sp<SkRuntimeEffect> effect, const char* name, sk_sp<SkColorSpace> dstColorSpace,
    ...
    SkSpan<std::unique_ptr<GrFragmentProcessor>> childFPs)
{
    ...
    std::unique_ptr<GrSkSLFP> fp(new (uniformSize + specializedSize) GrSkSLFP(std::move(effect), name, OptFlags::kNone));
...
GrFragmentProcessor::OptimizationFlags GrSkSLFP::DetermineOptimizationFlags(OptFlags of, SkRuntimeEffect* effect)
{
...
{
    SkASSERTF(fToLinearSrgbChildIndex == -1 && fFromLinearSrgbChildIndex == -1, "addColorTransformChildren should not be called more than once");
    ...
    auto workingToLinear = GrColorSpaceXformEffect::Make(nullptr, dstColorSpace, kUnpremul_SkAlphaType, sk_srgb_linear_singleton(), kUnpremul_SkAlphaType);
    auto linearToWorking = GrColorSpaceXformEffect::Make(nullptr, sk_srgb_linear_singleton(), kUnpremul_SkAlphaType, dstColorSpace, kUnpremul_SkAlphaType);
...
bool GrSkSLFP::onIsEqual(const GrFragmentProcessor& other) const
{
    ...
    return fEffect->hash() == sk.fEffect->hash() && this->uniformCount() == sk.uniformCount() && fUniformSize == sk.fUniformSize
        && !sk_careful_memcmp(this->uniformData(), sk.uniformData(), fUniformSize + specializedSize);
}

std::unique_ptr<GrFragmentProcessor> GrSkSLFP::clone() const
{
    return std::unique_ptr<GrFragmentProcessor>(new (UniformPayloadSize(fEffect.get())) GrSkSLFP(*this));
}

SkPMColor4f GrSkSLFP::constantOutputForConstantInput(const SkPMColor4f& inputColor) const
{
    SkPMColor4f color = (fInputChildIndex >= 0) ? ConstantOutputForConstantInput(this->childProcessor(fInputChildIndex), inputColor) : inputColor;
    ...
    if (const SkSL::RP::Program* program = fEffect->getRPProgram(/*debugTrace=*/nullptr)) {
        ...
        SkSpan<const float> uniforms { reinterpret_cast<const float*>(this->uniformData()), fUniformSize / sizeof(float) };
...
std::unique_ptr<GrFragmentProcessor> GrSkSLFP::TestCreate(GrProcessorTestData* d)
{
    ...
    auto [success, fp] = GrFragmentProcessors::Make(d->context(), filter.get(), /*inputFP=*/nullptr, GrColorInfo {}, props);
    SkASSERT(success);
    return std::move(fp);
}
