...
static inline bool does_cpu_blend_impl_match_gpu(SkBlendMode mode)
{
    ...
    // GPUs.
    return mode <= SkBlendMode::kLastSeparableMode && mode != SkBlendMode::kSoftLight && mode != SkBlendMode::kColorBurn;
}
...
public:
    static std::unique_ptr<GrFragmentProcessor> Make(
        std::unique_ptr<GrFragmentProcessor> src, std::unique_ptr<GrFragmentProcessor> dst, SkBlendMode mode, bool shareBlendLogic)
    {
        return std::unique_ptr<GrFragmentProcessor>(new BlendFragmentProcessor(std::move(src), std::move(dst), mode, shareBlendLogic));
    }
...
private:
    BlendFragmentProcessor(std::unique_ptr<GrFragmentProcessor> src, std::unique_ptr<GrFragmentProcessor> dst, SkBlendMode mode, bool shareBlendLogic)
    ...
    static OptimizationFlags OptFlags(const GrFragmentProcessor* src, const GrFragmentProcessor* dst, SkBlendMode mode)
    {
        ...
        case SkBlendMode::kSrc:
            flags = ProcessorOptimizationFlags(src) & ~kConstantOutputForConstantInput_OptimizationFlag;
            break;

        // Just propagates dst, and its flags:
        case SkBlendMode::kDst:
            flags = ProcessorOptimizationFlags(dst) & ~kConstantOutputForConstantInput_OptimizationFlag;
            break;
        ...
        case SkBlendMode::kModulate:
            if (src && dst) {
                flags = ProcessorOptimizationFlags(src) & ProcessorOptimizationFlags(dst) & kPreservesOpaqueInput_OptimizationFlag;
            } else if (src) {
                flags = ProcessorOptimizationFlags(src) & ~kConstantOutputForConstantInput_OptimizationFlag;
            } else if (dst) {
                flags = ProcessorOptimizationFlags(dst) & ~kConstantOutputForConstantInput_OptimizationFlag;
        ...
        if (does_cpu_blend_impl_match_gpu(mode) && (!src || src->hasConstantOutputForConstantInput()) && (!dst || dst->hasConstantOutputForConstantInput())) {
...
std::unique_ptr<GrFragmentProcessor> BlendFragmentProcessor::TestCreate(GrProcessorTestData* d)
{
    ...
    SkBlendMode mode = static_cast<SkBlendMode>(d->fRandom->nextRangeU(0, (int)SkBlendMode::kLastMode));
    return std::unique_ptr<GrFragmentProcessor>(new BlendFragmentProcessor(std::move(src), std::move(dst), mode, shareLogic));
}
#endif
    ...
    public:
        void emitCode(EmitArgs& args) override
        {
            ...
            if (bfp.fShareBlendLogic) {
                ...
                std::string blendExpr = GrGLSLBlend::BlendExpression(&args.fFp, args.fUniformHandler, &fBlendUniform, srcColor.c_str(), dstColor.c_str(), mode);
                ...
                fragBuilder->codeAppendf("return %s(%s, %s);", skgpu::BlendFuncName(mode), srcColor.c_str(), dstColor.c_str());
            }
        }
