...
GrFPResult GrConvexPolyEffect::Make(std::unique_ptr<GrFragmentProcessor> inputFP, GrClipEdgeType type, const SkPath& path)
{
    ...
    if (dir == SkPathFirstDirection::kUnknown) {
        ...
        // the print.
        return GrFPSuccess(GrFragmentProcessor::ModulateRGBA(std::move(inputFP), SK_PMColor4fTRANSPARENT));
    }
...
std::unique_ptr<GrFragmentProcessor::ProgramImpl> GrConvexPolyEffect::onMakeProgramImpl() const
{
    ...
    public:
        void emitCode(EmitArgs& args) override
        {
            ...
            fEdgeUniform = args.fUniformHandler->addUniformArray(&cpe, kFragment_GrShaderFlag, SkSLType::kHalf3, "edgeArray", cpe.fEdgeCount, &edgeArrayName);
...
GrConvexPolyEffect::GrConvexPolyEffect(std::unique_ptr<GrFragmentProcessor> inputFP, GrClipEdgeType edgeType, int n, const float edges[])
    : INHERITED(kGrConvexPolyEffect_ClassID, ProcessorOptimizationFlags(inputFP.get()) & kCompatibleWithCoverageAsAlpha_OptimizationFlag)
    , fEdgeType(edgeType)
    , fEdgeCount(n)
{
...
bool GrConvexPolyEffect::onIsEqual(const GrFragmentProcessor& other) const
{
    ...
    return cpe.fEdgeType == fEdgeType && cpe.fEdgeCount == fEdgeCount && std::equal(cpe.fEdges.begin(), cpe.fEdges.begin() + n, fEdges.begin());
}
...
std::unique_ptr<GrFragmentProcessor> GrConvexPolyEffect::TestCreate(GrProcessorTestData* d)
{
    ...
    do {
        GrClipEdgeType edgeType = static_cast<GrClipEdgeType>(d->fRandom->nextULessThan(kGrClipEdgeTypeCnt));
