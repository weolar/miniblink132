...
private:
    bool hasHWBlendEquation() const
    {
        return skgpu::BlendEquation::kIllegal != fHWBlendEquation;
    }
...
std::unique_ptr<GrXferProcessor::ProgramImpl> CustomXP::makeProgramImpl() const
{
    ...
    private:
        void emitOutputsForBlendState(const EmitArgs& args) override
        {
            ...
            fragBuilder->codeAppendf("%s = %s * %s;", args.fOutputPrimary, args.fInputCoverage, args.fInputColor);
        }
            ...
            const char* dstColor, const char* outColor, const char* outColorSecondary, const GrXferProcessor& proc) override
        {
            ...
            std::string blendExpr = GrGLSLBlend::BlendExpression(&xp, uniformHandler, &fBlendUniform, srcColor, dstColor, xp.fMode);
...
sk_sp<const GrXferProcessor> CustomXPFactory::makeXferProcessor(
    const GrProcessorAnalysisColor&, GrProcessorAnalysisCoverage coverage, const GrCaps& caps, GrClampType clampType) const
{
...
GrXPFactory::AnalysisProperties CustomXPFactory::analysisProperties(
    const GrProcessorAnalysisColor&, const GrProcessorAnalysisCoverage& coverage, const GrCaps& caps, GrClampType clampType) const
{
    /*
    ...
    */
    if (can_use_hw_blend_equation(fHWBlendEquation, coverage, caps)) {
        if (caps.blendEquationSupport() == GrCaps::kAdvancedCoherent_BlendEquationSupport) {
            ...
            return AnalysisProperties::kCompatibleWithCoverageAsAlpha | AnalysisProperties::kRequiresNonOverlappingDraws
...
{
    int mode = d->fRandom->nextRangeU((int)SkBlendMode::kLastCoeffMode + 1, (int)SkBlendMode::kLastSeparableMode);
