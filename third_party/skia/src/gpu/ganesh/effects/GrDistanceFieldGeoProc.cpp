...
private:
    ...
    {
        const GrDistanceFieldA8TextGeoProc& dfTexEffect = args.fGeomProc.cast<GrDistanceFieldA8TextGeoProc>();
        ...
        fAtlasDimensionsInvUniform
            = uniformHandler->addUniform(nullptr, kVertex_GrShaderFlag, SkSLType::kFloat2, "AtlasDimensionsInv", &atlasDimensionsInvName);
#ifdef SK_GAMMA_APPLY_TO_A8
        // adjust based on gamma
        const char* distanceAdjustUniName = nullptr;
        // width, height, 1/(3*width)
        fDistanceAdjustUni = uniformHandler->addUniform(nullptr, kFragment_GrShaderFlag, SkSLType::kHalf, "DistanceAdjust", &distanceAdjustUniName);
#endif
        ...
        WriteLocalCoord(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, gpArgs->fPositionVar, dfTexEffect.fLocalMatrix, &fLocalMatrixUniform);

        // add varyings
        GrGLSLVarying uv, texIdx, st;
        append_index_uv_varyings(args, dfTexEffect.numTextureSamplers(), dfTexEffect.fInTextureCoords.name(), atlasDimensionsInvName, &uv, &texIdx, &st);

        bool isUniformScale = (dfTexEffect.fFlags & kUniformScale_DistanceFieldEffectMask) == kUniformScale_DistanceFieldEffectMask;
        ...
        fragBuilder->codeAppend("half distance = " SK_DistanceFieldMultiplier "*(texColor.r - " SK_DistanceFieldThreshold ");");
#ifdef SK_GAMMA_APPLY_TO_A8
        ...
        if (isUniformScale) {
            ...
            if (args.fShaderCaps->fAvoidDfDxForGradientsWhenPossible) {
                fragBuilder->codeAppendf("afwidth = abs(" SK_DistanceFieldAAFactor "*half(dFdy(%s.y)));", st.fsIn());
            } else {
                fragBuilder->codeAppendf("afwidth = abs(" SK_DistanceFieldAAFactor "*half(dFdx(%s.x)));", st.fsIn());
            }
            ...
            fragBuilder->codeAppendf("float2x2 jacobian = float2x2(dFdx(%s), dFdy(%s));", st.fsIn(), st.fsIn());
...
GrDistanceFieldA8TextGeoProc::GrDistanceFieldA8TextGeoProc(const GrShaderCaps& caps, const GrSurfaceProxyView* views, int numViews, GrSamplerState params,
#ifdef SK_GAMMA_APPLY_TO_A8
...
#endif
{
    ...
    fInTextureCoords = { "inTextureCoords", kUShort2_GrVertexAttribType, caps.fIntegerSupport ? SkSLType::kUShort2 : SkSLType::kFloat2 };
...
void GrDistanceFieldA8TextGeoProc::addNewViews(const GrSurfaceProxyView* views, int numViews, GrSamplerState params)
{
    SkASSERT(numViews <= kMaxTextures);
...
GrGeometryProcessor* GrDistanceFieldA8TextGeoProc::TestCreate(GrProcessorTestData* d)
{
    ...
    GrSamplerState samplerState(wrapModes, d->fRandom->nextBool() ? GrSamplerState::Filter::kLinear : GrSamplerState::Filter::kNearest);
...
#endif
    return GrDistanceFieldA8TextGeoProc::Make(d->allocator(), *d->caps()->shaderCaps(), &view, 1, samplerState,
#ifdef SK_GAMMA_APPLY_TO_A8
        lum,
#endif
        flags, localMatrix);
...
private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
    {
        const GrDistanceFieldPathGeoProc& dfPathEffect = args.fGeomProc.cast<GrDistanceFieldPathGeoProc>();
        ...
        GrGLSLVarying uv, texIdx, st;
        append_index_uv_varyings(args, dfPathEffect.numTextureSamplers(), dfPathEffect.fInTextureCoords.name(), atlasDimensionsInvName, &uv, &texIdx, &st);
        ...
        WriteLocalCoord(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, gpArgs->fPositionVar, dfPathEffect.fLocalMatrix, &fLocalMatrixUniform);
        ...
        fragBuilder->codeAppend("half afwidth;");
        bool isUniformScale = (dfPathEffect.fFlags & kUniformScale_DistanceFieldEffectMask) == kUniformScale_DistanceFieldEffectMask;
...
GrDistanceFieldPathGeoProc::GrDistanceFieldPathGeoProc(
    const GrShaderCaps& caps, const GrSurfaceProxyView* views, int numViews, GrSamplerState params, const SkMatrix& localMatrix, uint32_t flags)
...
void GrDistanceFieldPathGeoProc::addNewViews(const GrSurfaceProxyView* views, int numViews, GrSamplerState params)
{
    SkASSERT(numViews <= kMaxTextures);
    // Just to make sure we don't try to add too many proxies
    numViews = std::min(numViews, kMaxTextures);
...
GrGeometryProcessor* GrDistanceFieldPathGeoProc::TestCreate(GrProcessorTestData* d)
{
    ...
    SkMatrix localMatrix = GrTest::TestMatrix(d->fRandom);
    return GrDistanceFieldPathGeoProc::Make(d->allocator(), *d->caps()->shaderCaps(), &view, 1, samplerState, localMatrix, flags);
}
#endif
...
private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
    {
        const GrDistanceFieldLCDTextGeoProc& dfTexEffect = args.fGeomProc.cast<GrDistanceFieldLCDTextGeoProc>();
        ...
        gpArgs->fPositionVar = dfTexEffect.fInPosition.asShaderVar();
        WriteLocalCoord(
            vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, dfTexEffect.fInPosition.asShaderVar(), dfTexEffect.fLocalMatrix, &fLocalMatrixUniform);
        ...
        if (isUniformScale) {
            ...
            if (dfTexEffect.fFlags & kPortrait_DistanceFieldEffectFlag) {
                fragBuilder->codeAppendf("half2 offset = half2(0.0, half(st_grad_len*%s));", delta.fsIn());
            } else {
            ...
            if (args.fShaderCaps->fAvoidDfDxForGradientsWhenPossible) {
                ...
                if (dfTexEffect.fFlags & kPortrait_DistanceFieldEffectFlag) {
                    ...
                    fragBuilder->codeAppendf("half2 offset = half2(%s*float2(st_grad.y,-st_grad.x));", delta.fsIn());
                }
            } else {
            ...
            if (dfTexEffect.fFlags & kPortrait_DistanceFieldEffectFlag) {
                fragBuilder->codeAppendf("half2 offset = half2(jacobian * half2(0, %s));", delta.fsIn());
            } else {
                fragBuilder->codeAppendf("half2 offset = half2(jacobian * half2(%s, 0));", delta.fsIn());
            }
        }

        // sample the texture by index
        fragBuilder->codeAppend("half3 distance;");
        append_multitexture_lookup_lcd(args, dfTexEffect.numTextureSamplers(), texIdx, "uv", "offset", "distance");
        fragBuilder->codeAppend("distance = "
                                "half3(" SK_DistanceFieldMultiplier ")*(distance - half3(" SK_DistanceFieldThreshold "));");

        // adjust width based on gamma
        const char* distanceAdjustUniName = nullptr;
        fDistanceAdjustUni = uniformHandler->addUniform(nullptr, kFragment_GrShaderFlag, SkSLType::kHalf3, "DistanceAdjust", &distanceAdjustUniName);
        fragBuilder->codeAppendf("distance -= %s;", distanceAdjustUniName);
        ...
        if (isGammaCorrect) {
            ...
            fragBuilder->codeAppendf("half4 %s = half4(smoothstep(half3(-afwidth), half3(afwidth), distance), 1.0);", args.fOutputCoverage);
        }
    }
...
GrDistanceFieldLCDTextGeoProc::GrDistanceFieldLCDTextGeoProc(const GrShaderCaps& caps, const GrSurfaceProxyView* views, int numViews, GrSamplerState params,
...
void GrDistanceFieldLCDTextGeoProc::addNewViews(const GrSurfaceProxyView* views, int numViews, GrSamplerState params)
{
    SkASSERT(numViews <= kMaxTextures);
    // Just to make sure we don't try to add too many proxies
    numViews = std::min(numViews, kMaxTextures);
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> GrDistanceFieldLCDTextGeoProc::makeProgramImpl(const GrShaderCaps&) const
{
    return std::make_unique<Impl>();
}
...
GrGeometryProcessor* GrDistanceFieldLCDTextGeoProc::TestCreate(GrProcessorTestData* d)
{
    ...
    return GrDistanceFieldLCDTextGeoProc::Make(d->allocator(), *d->caps()->shaderCaps(), &view, 1, samplerState, wa, flags, localMatrix);
}
#endif
