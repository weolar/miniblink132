...
void GrConicEffect::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs)
{
    ...
    // Setup position
    WriteOutputPosition(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, gp.inPosition().name(), gp.fViewMatrix, &fViewMatrixUniform);
    if (gp.fUsesLocalCoords) {
        WriteLocalCoord(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, gp.inPosition().asShaderVar(), gp.fLocalMatrix, &fLocalMatrixUniform);
    }
    ...
    fragBuilder->codeAppendf(
        "%s = 2.0 * %s.x * %s.x - %s.y * %s.z - %s.z * %s.y;", dfdx.c_str(), v.fsIn(), dklmdx.c_str(), v.fsIn(), dklmdx.c_str(), v.fsIn(), dklmdx.c_str());
    fragBuilder->codeAppendf(
        "%s = 2.0 * %s.x * %s.x - %s.y * %s.z - %s.z * %s.y;", dfdy.c_str(), v.fsIn(), dklmdy.c_str(), v.fsIn(), dklmdy.c_str(), v.fsIn(), dklmdy.c_str());
    ...
    fragBuilder->codeAppendf("%s = %s.x*%s.x - %s.y*%s.z;", func.c_str(), v.fsIn(), v.fsIn(), v.fsIn(), v.fsIn());
    ...
    if (gp.fCoverageScale != 0xff) {
        ...
        fCoverageScaleUniform = uniformHandler->addUniform(nullptr, kFragment_GrShaderFlag, SkSLType::kFloat, "Coverage", &coverageScale);
        fragBuilder->codeAppendf("half4 %s = half4(half(%s) * %s);", args.fOutputCoverage, coverageScale, edgeAlpha.c_str());
    } else {
...
void GrConicEffect::addToKey(const GrShaderCaps& caps, skgpu::KeyBuilder* b) const
{
    ...
    key = ProgramImpl::AddMatrixKeys(caps, key, fViewMatrix, fUsesLocalCoords ? fLocalMatrix : SkMatrix::I());
    b->add32(key);
}
...
GrConicEffect::GrConicEffect(const SkPMColor4f& color, const SkMatrix& viewMatrix, uint8_t coverage, const SkMatrix& localMatrix, bool usesLocalCoords)
...
GrGeometryProcessor* GrConicEffect::TestCreate(GrProcessorTestData* d)
{
    ...
    return GrConicEffect::Make(d->allocator(), SkPMColor4f::FromBytes_RGBA(color), viewMatrix, *d->caps(), localMatrix, usesLocalCoords);
}
#endif
...
void GrQuadEffect::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs)
{
    ...
    if (gp.fCoverageScale != 0xFF) {
        const char* coverageScale;
        fCoverageScaleUniform = uniformHandler->addUniform(nullptr, kFragment_GrShaderFlag, SkSLType::kHalf, "Coverage", &coverageScale);
        fragBuilder->codeAppendf("half4 %s = half4(%s * edgeAlpha);", args.fOutputCoverage, coverageScale);
    } else {
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> GrQuadEffect::makeProgramImpl(const GrShaderCaps&) const
{
    return std::make_unique<Impl>();
}

GrQuadEffect::GrQuadEffect(const SkPMColor4f& color, const SkMatrix& viewMatrix, uint8_t coverage, const SkMatrix& localMatrix, bool usesLocalCoords)
...
GrGeometryProcessor* GrQuadEffect::TestCreate(GrProcessorTestData* d)
{
    ...
    SkMatrix localMatrix = GrTest::TestMatrix(d->fRandom);
    bool usesLocalCoords = d->fRandom->nextBool();
    return GrQuadEffect::Make(d->allocator(), SkPMColor4f::FromBytes_RGBA(color), viewMatrix, *d->caps(), localMatrix, usesLocalCoords);
}
#endif
