...
void GrTextureResolveRenderTask::addProxy(GrDrawingManager* drawingMgr, sk_sp<GrSurfaceProxy> proxyRef, GrSurfaceProxy::ResolveFlags flags, const GrCaps& caps)
{
    ...
    if (auto found = std::find(fTargets.begin(), fTargets.end(), proxyRef); found != fTargets.end()) {
        ...
        SkASSERT(!drawingMgr->getLastRenderTask(proxy) || drawingMgr->getLastRenderTask(proxy)->isClosed());
    ...
    if (newProxy) {
        ...
        this->addDependency(drawingMgr, proxy, skgpu::Mipmapped::kNo, GrTextureResolveManager(nullptr), caps);
...
void GrTextureResolveRenderTask::gatherProxyIntervals(GrResourceAllocator* alloc) const
{
    ...
    for (const sk_sp<GrSurfaceProxy>& target : fTargets) {
        alloc->addInterval(target.get(), fakeOp, fakeOp, GrResourceAllocator::ActualUse::kYes, GrResourceAllocator::AllowRecycling::kYes);
    }
    alloc->incOps();
}
...
GrSurfaceProxy::ResolveFlags GrTextureResolveRenderTask::flagsForProxy(sk_sp<GrSurfaceProxy> proxy) const
{
    if (auto found = std::find(fTargets.begin(), fTargets.end(), proxy); found != fTargets.end()) {
