...
void GrRenderTask::addDependency(
    GrDrawingManager* drawingMgr, GrSurfaceProxy* dependedOn, skgpu::Mipmapped mipmapped, GrTextureResolveManager textureResolveManager, const GrCaps& caps)
{
    ...
    if (dependedOnTask == this) {
        ...
        SkASSERT(!dependedOn->asTextureProxy() || !dependedOn->asTextureProxy()->texPriv().isDeferred());
        return;
    }
    ...
    if (textureProxy && textureProxy->texPriv().isDeferred()) {
        if (alreadyDependent) {
            SkASSERT(std::find(fDeferredProxies.begin(), fDeferredProxies.end(), textureProxy) != fDeferredProxies.end());
        } else {
...
void GrRenderTask::dump(const SkString& label, SkString indent, bool printDependencies, bool close) const
{
    SkDebugf("%s%s --------------------------------------------------------------\n", indent.c_str(), label.c_str());
