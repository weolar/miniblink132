...
GrYUVABackendTextureInfo::GrYUVABackendTextureInfo(
    const SkYUVAInfo& yuvaInfo, const GrBackendFormat formats[kMaxPlanes], skgpu::Mipmapped mipmapped, GrSurfaceOrigin origin)
    : fYUVAInfo(yuvaInfo)
    , fMipmapped(mipmapped)
    , fTextureOrigin(origin)
{
    if (!yuvaInfo.isValid()) {
        *this = {};
...
{
    if (fYUVAInfo != that.fYUVAInfo || fMipmapped != that.fMipmapped || fTextureOrigin != that.fTextureOrigin) {
        return false;
    }
...
SkYUVAInfo::YUVALocations GrYUVABackendTextureInfo::toYUVALocations() const
{
    uint32_t channelFlags[]
        = { fPlaneFormats[0].channelMask(), fPlaneFormats[1].channelMask(), fPlaneFormats[2].channelMask(), fPlaneFormats[3].channelMask() };
...
GrYUVABackendTextures::GrYUVABackendTextures(const SkYUVAInfo& yuvaInfo, const GrBackendTexture textures[SkYUVAInfo::kMaxPlanes], GrSurfaceOrigin textureOrigin)
    : fYUVAInfo(yuvaInfo)
    , fTextureOrigin(textureOrigin)
{
    ...
    for (int i = 0; i < numPlanes; ++i) {
        ...
        if (!textures[i].isValid() || textures[i].dimensions() != planeDimensions[i] || textures[i].backend() != textures[0].backend()
...
{
    uint32_t channelFlags[] = { fTextures[0].getBackendFormat().channelMask(), fTextures[1].getBackendFormat().channelMask(),
