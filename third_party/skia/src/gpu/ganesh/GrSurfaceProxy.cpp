...
static bool is_valid_lazy(const SkISize& dimensions, SkBackingFit fit)
{
    ...
    return ((dimensions.fWidth < 0 && dimensions.fHeight < 0 && SkBackingFit::kApprox == fit) || (dimensions.fWidth > 0 && dimensions.fHeight > 0));
}
...
GrSurfaceProxy::LazyCallbackResult::LazyCallbackResult(sk_sp<GrSurface> surf, bool releaseCallback, LazyInstantiationKeyMode mode)
...
// Deferred version
GrSurfaceProxy::GrSurfaceProxy(const GrBackendFormat& format, SkISize dimensions, SkBackingFit fit, skgpu::Budgeted budgeted, GrProtected isProtected,
...
GrSurfaceProxy::GrSurfaceProxy(LazyInstantiateCallback&& callback, const GrBackendFormat& format, SkISize dimensions, SkBackingFit fit,
    skgpu::Budgeted budgeted, GrProtected isProtected, GrInternalSurfaceFlags surfaceFlags, UseAllocator useAllocator, std::string_view label)
    : fSurfaceFlags(surfaceFlags)
    , fFormat(format)
    , fDimensions(dimensions)
    , fFit(fit)
...
GrSurfaceProxy::GrSurfaceProxy(sk_sp<GrSurface> surface, SkBackingFit fit, UseAllocator useAllocator)
    ...
    , fFit(fit)
    , fBudgeted(fTarget->resourcePriv().budgetedType() == GrBudgetedType::kBudgeted ? skgpu::Budgeted::kYes : skgpu::Budgeted::kNo)
    , fUseAllocator(useAllocator)
...
sk_sp<GrSurface> GrSurfaceProxy::createSurfaceImpl(
    GrResourceProvider* resourceProvider, int sampleCnt, GrRenderable renderable, skgpu::Mipmapped mipmapped) const
{
    ...
    if (SkBackingFit::kApprox == fFit) {
        surface = resourceProvider->createApproxTexture(fDimensions, fFormat, fFormat.textureType(), renderable, sampleCnt, fIsProtected, fLabel);
    } else {
        surface
            = resourceProvider->createTexture(fDimensions, fFormat, fFormat.textureType(), renderable, sampleCnt, mipmapped, fBudgeted, fIsProtected, fLabel);
    }
    if (!surface) {
        return nullptr;
    }
...
bool GrSurfaceProxy::instantiateImpl(
    GrResourceProvider* resourceProvider, int sampleCnt, GrRenderable renderable, skgpu::Mipmapped mipmapped, const skgpu::UniqueKey* uniqueKey)
{
    SkASSERT(!this->isLazy());
    if (fTarget) {
...
void GrSurfaceProxy::computeScratchKey(const GrCaps& caps, skgpu::ScratchKey* key) const
{
    ...
    GrTexture::ComputeScratchKey(caps, this->backendFormat(), this->backingStoreDimensions(), renderable, sampleCount, mipmapped, fIsProtected, key);
}
...
sk_sp<GrSurfaceProxy> GrSurfaceProxy::Copy(GrRecordingContext* rContext, sk_sp<GrSurfaceProxy> src, GrSurfaceOrigin origin, skgpu::Mipmapped mipmapped,
    SkIRect srcRect, SkBackingFit fit, skgpu::Budgeted budgeted, std::string_view label, RectsMustMatch rectsMustMatch, sk_sp<GrRenderTask>* outTask)
{
    ...
    if (src->backendFormat().textureType() != GrTextureType::kExternal) {
        ...
        auto dstContext = rContext->priv().makeSC(info, format, label, fit, origin, GrRenderable::kNo, 1, mipmapped, src->isProtected(), budgeted);
    ...
    if (src->asTextureProxy()) {
        auto dstContext = rContext->priv().makeSFC(kUnknown_SkAlphaType, nullptr, { width, height }, fit, format, 1, mipmapped, src->isProtected(),
            skgpu::Swizzle::RGBA(), skgpu::Swizzle::RGBA(), origin, budgeted, label);
...
sk_sp<GrSurfaceProxy> GrSurfaceProxy::Copy(GrRecordingContext* context, sk_sp<GrSurfaceProxy> src, GrSurfaceOrigin origin, skgpu::Mipmapped mipmapped,
    SkBackingFit fit, skgpu::Budgeted budgeted, std::string_view label, sk_sp<GrRenderTask>* outTask)
{
    SkASSERT(!src->isFullyLazy());
...
SkString GrSurfaceProxy::dump() const
{
    ...
    tmp.appendf("proxyID: %u - surfaceID: %u", this->uniqueID().asUInt(), this->peekSurface() ? this->peekSurface()->uniqueID().asUInt() : -1);
    return tmp;
}
...
void GrSurfaceProxyPriv::exactify()
{
    ...
    fProxy->fDimensions = fProxy->fTarget ? fProxy->fTarget->dimensions() : fProxy->backingStoreDimensions();
...
{
    SkASSERTF(surface->backendFormat() == fFormat, "%s != %s", surface->backendFormat().toStr().c_str(), fFormat.toStr().c_str());
