...
public:
    ...
    {
        SkDEBUGCODE(if (fIsFlushed) SK_ABORT("Using a flushed arena");) return &fArenaAlloc;
    }
    void flush()
    {
...
protected:
    ...
    // Deferred version
    GrRenderTargetProxy(const GrCaps&, const GrBackendFormat&, SkISize, int sampleCount, SkBackingFit, skgpu::Budgeted, skgpu::Protected,
    ...
    GrRenderTargetProxy(LazyInstantiateCallback&&, const GrBackendFormat&, SkISize, int sampleCount, SkBackingFit, skgpu::Budgeted, skgpu::Protected,
        GrInternalSurfaceFlags, UseAllocator, WrapsVkSecondaryCB, std::string_view label);
