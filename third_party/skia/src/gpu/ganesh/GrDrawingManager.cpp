...
///////////////////////////////////////////////////////////////////////////////////////////////////
GrDrawingManager::GrDrawingManager(GrRecordingContext* rContext, const PathRendererChain::Options& optionsForPathRendererChain, bool reduceOpsTaskSplitting)
    : fContext(rContext)
    ...
    SkSpan<GrSurfaceProxy*> proxies, SkSurfaces::BackendSurfaceAccess access, const GrFlushInfo& info, const skgpu::MutableTextureState* newState)
{
    ...
    if (!proxies.empty() && !info.fNumSemaphores && !info.fFinishedProc && access == SkSurfaces::BackendSurfaceAccess::kNoAccess && !newState) {
        bool allUnused = std::all_of(proxies.begin(), proxies.end(), [&](GrSurfaceProxy* proxy) {
            bool used = std::any_of(fDAG.begin(), fDAG.end(), [&](auto& task) { return task && task->isUsed(proxy); });
            return !used;
        });
        if (allUnused) {
            if (info.fSubmittedProc) {
    ...
    if (preFlushSuccessful) {
        ...
        cachePurgeNeeded = !resourceAllocator.failedInstantiation() && this->executeRenderTasks(&flushState);
    }
    this->removeRenderTasks();
...
void GrDrawingManager::sortTasks()
{
    ...
    for (size_t i = 0, start = 0, end; start < SkToSizeT(fDAG.size()); ++i, start = end + 1) {
        ...
        SkASSERT(std::none_of(span.begin(), span.end(), [](const auto& t) { return t->blocksReordering(); }));
        ...
        auto check = [&](const GrRenderTask* task, auto&& check) -> void {
            SkASSERT(
                GrRenderTask::TopoSortTraits::WasOutput(task) || std::find_if(span.begin(), span.end(), [task](const auto& n) { return n.get() == task; }));
...
template <typename T> static void reorder_array_by_llist(const SkTInternalLList<T>& llist, TArray<sk_sp<T>>* array)
{
    int i = 0;
    for (T* t : llist) {
...
{
    int n = std::find(fOnFlushCBObjects.begin(), fOnFlushCBObjects.end(), cb) - fOnFlushCBObjects.begin();
...
void GrDrawingManager::createDDLTask(sk_sp<const GrDeferredDisplayList> ddl, sk_sp<GrRenderTargetProxy> newDest)
{
    SkDEBUGCODE(this->validate());
    ...
    if (ddl->priv().targetProxy()->isMSAADirty()) {
        auto nativeRect = GrNativeRect::MakeIRectRelativeTo(
            ddl->characterization().origin(), ddl->priv().targetProxy()->backingStoreDimensions().height(), ddl->priv().targetProxy()->msaaDirtyRect());
    ...
    SkDEBUGCODE(auto ddlTask =) this->appendTask(sk_make_sp<GrDDLTask>(this, std::move(newDest), std::move(ddl)));
...
void GrDrawingManager::validate() const
{
    ...
    for (int i = 0; i < fDAG.size(); ++i) {
        if (fActiveOpsTask != fDAG[i].get()) {
            ...
            // activeTask does.
            bool isActiveResolveTask = fActiveOpsTask && fActiveOpsTask->fTextureResolveTask == fDAG[i].get();
...
sk_sp<skgpu::ganesh::OpsTask> GrDrawingManager::newOpsTask(GrSurfaceProxyView surfaceView, sk_sp<GrArenas> arenas)
{
    SkDEBUGCODE(this->validate());
    SkASSERT(fContext);
    ...
    sk_sp<skgpu::ganesh::OpsTask> opsTask(new skgpu::ganesh::OpsTask(this, std::move(surfaceView), fContext->priv().auditTrail(), std::move(arenas)));
...
void GrDrawingManager::addAtlasTask(sk_sp<GrRenderTask> atlasTask, GrRenderTask* previousAtlasTask)
{
    SkDEBUGCODE(this->validate());
    SkASSERT(fContext);
...
void GrDrawingManager::newTextureResolveRenderTask(sk_sp<GrSurfaceProxy> proxy, GrSurfaceProxy::ResolveFlags flags, const GrCaps& caps)
{
    SkDEBUGCODE(this->validate());
    SkASSERT(fContext);
...
void GrDrawingManager::newWaitRenderTask(const sk_sp<GrSurfaceProxy>& proxy, std::unique_ptr<std::unique_ptr<GrSemaphore>[]> semaphores, int numSemaphores)
{
    SkDEBUGCODE(this->validate());
    SkASSERT(fContext);

    sk_sp<GrWaitRenderTask> waitTask = sk_make_sp<GrWaitRenderTask>(GrSurfaceProxyView(proxy), std::move(semaphores), numSemaphores);
...
void GrDrawingManager::newTransferFromRenderTask(const sk_sp<GrSurfaceProxy>& srcProxy, const SkIRect& srcRect, GrColorType surfaceColorType,
    GrColorType dstColorType, sk_sp<GrGpuBuffer> dstBuffer, size_t dstOffset)
{
    ...
    GrRenderTask* task
        = this->appendTask(sk_make_sp<GrTransferFromRenderTask>(srcProxy, srcRect, surfaceColorType, dstColorType, std::move(dstBuffer), dstOffset));
    ...
    task->addDependency(this, srcProxy.get(), skgpu::Mipmapped::kNo, GrTextureResolveManager(this), caps);
    task->makeClosed(fContext);
...
void GrDrawingManager::newBufferTransferTask(sk_sp<GrGpuBuffer> src, size_t srcOffset, sk_sp<GrGpuBuffer> dst, size_t dstOffset, size_t size)
{
    SkASSERT(src);
    SkASSERT(dst);
    ...
    sk_sp<GrRenderTask> task = GrBufferTransferRenderTask::Make(std::move(src), srcOffset, std::move(dst), dstOffset, size);
    SkASSERT(task);
...
void GrDrawingManager::newBufferUpdateTask(sk_sp<SkData> src, sk_sp<GrGpuBuffer> dst, size_t dstOffset)
{
    SkASSERT(src);
    SkASSERT(dst);
    ...
    sk_sp<GrRenderTask> task = GrBufferUpdateRenderTask::Make(std::move(src), std::move(dst), dstOffset);
    SkASSERT(task);
...
sk_sp<GrRenderTask> GrDrawingManager::newCopyRenderTask(
    sk_sp<GrSurfaceProxy> dst, SkIRect dstRect, const sk_sp<GrSurfaceProxy>& src, SkIRect srcRect, GrSamplerState::Filter filter, GrSurfaceOrigin origin)
{
    SkDEBUGCODE(this->validate());
    SkASSERT(fContext);
    ...
    sk_sp<GrRenderTask> task = GrCopyRenderTask::Make(this, std::move(dst), dstRect, src, srcRect, filter, origin);
    if (!task) {
        return nullptr;
    }
    ...
    task->addDependency(this, src.get(), skgpu::Mipmapped::kNo, GrTextureResolveManager(this), caps);
    task->makeClosed(fContext);
...
bool GrDrawingManager::newWritePixelsTask(
    sk_sp<GrSurfaceProxy> dst, SkIRect rect, GrColorType srcColorType, GrColorType dstColorType, const GrMipLevel levels[], int levelCount)
{
    SkDEBUGCODE(this->validate());
    SkASSERT(fContext);
    ...
    if (!caps.preferVRAMUseOverFlushes()) {
        this->flushSurfaces(SkSpan<GrSurfaceProxy*> {}, SkSurfaces::BackendSurfaceAccess::kNoAccess, GrFlushInfo {}, nullptr);
    }

    GrRenderTask* task = this->appendTask(GrWritePixelsTask::Make(this, std::move(dst), rect, srcColorType, dstColorType, levels, levelCount));
    if (!task) {
        return false;
    }
...
skgpu::ganesh::PathRenderer* GrDrawingManager::getPathRenderer(
    const PathRenderer::CanDrawPathArgs& args, bool allowSW, PathRendererChain::DrawType drawType, PathRenderer::StencilSupport* stencilSupport)
{
    if (!fPathRendererChain) {
        fPathRendererChain = std::make_unique<PathRendererChain>(fContext, fOptionsForPathRendererChain);
    }
...
skgpu::ganesh::PathRenderer* GrDrawingManager::getSoftwarePathRenderer()
{
    if (!fSoftwarePathRenderer) {
        fSoftwarePathRenderer.reset(
            new skgpu::ganesh::SoftwarePathRenderer(fContext->priv().proxyProvider(), fOptionsForPathRendererChain.fAllowPathMaskCaching));
    }
