...
static const char* kRobustNormalizeDiffFn = "float2 robust_normalize_diff(float2 a, float2 b) {"
                                            "float2 diff = a - b;"
                                            "if (diff == float2(0.0)) {"
                                            "return float2(0.0);"
                                            "} else {"
...
static const char* kCosineBetweenUnitVectorsFn = "float cosine_between_unit_vectors(float2 a, float2 b) {"
...
static const char* kMiterExtentFn = "float miter_extent(float cosTheta, float miterLimit) {"
...
static const char* kNumRadialSegmentsPerRadianFn = "float num_radial_segments_per_radian(float approxDevStrokeRadius) {"
...
static const char* kUncheckedMixFn = "float unchecked_mix(float a, float b, float T) {"
                                     "return fma(b - a, T, a);"
                                     "}"
...
GrStrokeTessellationShader::GrStrokeTessellationShader(
    const GrShaderCaps& shaderCaps, PatchAttribs attribs, const SkMatrix& viewMatrix, const SkStrokeRec& stroke, SkPMColor4f color)
    : GrTessellationShader(kTessellate_GrStrokeTessellationShader_ClassID, GrPrimitiveType::kTriangleStrip, viewMatrix, color)
    ...
    , fStroke(stroke)
{
    ...
    if (fPatchAttribs & PatchAttribs::kColor) {
        fAttribs.emplace_back("dynamicColorAttr",
            (fPatchAttribs & PatchAttribs::kWideColorIfEnabled) ? kFloat4_GrVertexAttribType : kUByte4_norm_GrVertexAttribType, SkSLType::kHalf4);
    }
...
class GrStrokeTessellationShader::Impl : public ProgramImpl {
    ...
    //     float strokeOutset;
    //
    void emitTessellationCode(const GrStrokeTessellationShader& shader, SkString* code, GrGPArgs* gpArgs, const GrShaderCaps& shaderCaps) const;
...
void GrStrokeTessellationShader::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs)
{
    ...
    float maxEdges = args.fShaderCaps->fVertexIDSupport ? FixedCountStrokes::kMaxEdges : FixedCountStrokes::kMaxEdgesNoVertexIDs;
    ...
    if (!shader.hasDynamicStroke()) {
        ...
        fTessControlArgsUniform = args.fUniformHandler->addUniform(nullptr, kVertex_GrShaderFlag, SkSLType::kFloat3, "tessControlArgs", &tessArgsName);
        args.fVertBuilder->codeAppendf("float NUM_RADIAL_SEGMENTS_PER_RADIAN = %s.x;"
                                       "float JOIN_TYPE = %s.y;"
        ...
        fTessControlArgsUniform = args.fUniformHandler->addUniform(nullptr, kVertex_GrShaderFlag, SkSLType::kFloat, "maxScale", &maxScaleName);
        args.fVertBuilder->codeAppendf("float STROKE_RADIUS = dynamicStrokeAttr.x;"
    ...
    fAffineMatrixUniform = args.fUniformHandler->addUniform(nullptr, kVertex_GrShaderFlag, SkSLType::kFloat4, "affineMatrix", &affineMatrixName);
    fTranslateUniform = args.fUniformHandler->addUniform(nullptr, kVertex_GrShaderFlag, SkSLType::kFloat2, "translate", &translateName);
    ...
    if (shader.hasExplicitCurveType()) {
        args.fVertBuilder->insertFunction(SkStringPrintf("bool is_conic_curve() { return curveTypeAttr != %g; }", skgpu::tess::kCubicCurveType).c_str());
    } else {
        args.fVertBuilder->insertFunction("bool is_conic_curve() { return isinf(pts23Attr.w); }");
    }

    // Tessellation code.
    args.fVertBuilder->codeAppend("float2 p0=pts01Attr.xy, p1=pts01Attr.zw, p2=pts23Attr.xy, p3=pts23Attr.zw;"
    ...
    if (shader.stroke().isHairlineStyle()) {
        ...
        args.fVertBuilder->codeAppend("p0 = AFFINE_MATRIX * p0;"
                                      "p1 = AFFINE_MATRIX * p1;"
                                      "p2 = AFFINE_MATRIX * p2;"
                                      "p3 = AFFINE_MATRIX * p3;"
    ...
    if (args.fShaderCaps->fVertexIDSupport) {
        ...
        args.fVertBuilder->codeAppend("float edgeID = float(sk_VertexID >> 1);"
                                      "if ((sk_VertexID & 1) != 0) {"
                                      "edgeID = -edgeID;"
                                      "}");
    }
    ...
    if (shader.stroke().getJoin() == SkPaint::kRound_Join || shader.hasDynamicStroke()) {
        ...
        if (shader.hasDynamicStroke()) {
            args.fVertBuilder->codeAppend("if (JOIN_TYPE >= 0) {" // Is the join not a round type?
        ...
        args.fVertBuilder->codeAppendf("float numEdgesInJoin = %i;", skgpu::tess::NumFixedEdgesInJoin(joinType));
    }
...
void GrStrokeTessellationShader::Impl::emitTessellationCode(
    const GrStrokeTessellationShader& shader, SkString* code, GrGPArgs* gpArgs, const GrShaderCaps& shaderCaps) const
{
    ...
    //     float strokeOutset;
    //
    code->appendf("float2 tangent, strokeCoord;"
    ...
    if (!shader.stroke().isHairlineStyle()) {
        ...
        code->append("float2 devCoord = strokeCoord + TRANSLATE;"
...
void GrStrokeTessellationShader::Impl::emitFragmentCode(const GrStrokeTessellationShader& shader, const EmitArgs& args)
{
    if (!shader.hasDynamicColor()) {
        // The fragment shader just outputs a uniform color.
        const char* colorUniformName;
        fColorUniform = args.fUniformHandler->addUniform(nullptr, kFragment_GrShaderFlag, SkSLType::kHalf4, "color", &colorUniformName);
...
void GrStrokeTessellationShader::Impl::setData(const GrGLSLProgramDataManager& pdman, const GrShaderCaps&, const GrGeometryProcessor& geomProc)
{
    ...
    if (!shader.hasDynamicStroke()) {
        ...
        float numRadialSegmentsPerRadian = skgpu::tess::CalcNumRadialSegmentsPerRadian((stroke.isHairlineStyle() ? 1.f : maxScale) * strokeRadius);
