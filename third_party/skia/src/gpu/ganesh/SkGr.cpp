...
sk_sp<SkIDChangeListener> GrMakeUniqueKeyInvalidationListener(skgpu::UniqueKey* key, uint32_t contextID)
{
    ...
    auto data = SkData::MakeWithProc(new sk_sp<Listener>(listener), sizeof(sk_sp<Listener>), invalidateListener, nullptr);
...
sk_sp<GrSurfaceProxy> GrCopyBaseMipMapToTextureProxy(
    GrRecordingContext* ctx, sk_sp<GrSurfaceProxy> baseProxy, GrSurfaceOrigin origin, std::string_view label, skgpu::Budgeted budgeted)
{
    SkASSERT(baseProxy);
    ...
    auto copy = GrSurfaceProxy::Copy(ctx, std::move(baseProxy), origin, skgpu::Mipmapped::kYes, SkBackingFit::kExact, budgeted, label);
    if (!copy) {
        return nullptr;
    }
...
GrSurfaceProxyView GrCopyBaseMipMapToView(GrRecordingContext* context, GrSurfaceProxyView src, skgpu::Budgeted budgeted)
{
    ...
    return { GrCopyBaseMipMapToTextureProxy(context, proxy, origin, /*label=*/"CopyBaseMipMapToView", budgeted), origin, swizzle };
}
...
static sk_sp<GrTextureProxy> make_bmp_proxy(
    GrProxyProvider* proxyProvider, const SkBitmap& bitmap, GrColorType ct, skgpu::Mipmapped mipmapped, SkBackingFit fit, skgpu::Budgeted budgeted)
{
    SkBitmap bmpToUpload;
    if (ct != SkColorTypeToGrColorType(bitmap.info().colorType())) {
        SkColorType skCT = GrColorTypeToSkColorType(ct);
        if (!bmpToUpload.tryAllocPixels(bitmap.info().makeColorType(skCT)) || !bitmap.readPixels(bmpToUpload.pixmap())) {
            return {};
        }
    ...
    SkASSERT(!proxy || mipmapped == skgpu::Mipmapped::kNo || proxy->mipmapped() == skgpu::Mipmapped::kYes);
    return proxy;
}

std::tuple<GrSurfaceProxyView, GrColorType> GrMakeCachedBitmapProxyView(
    GrRecordingContext* rContext, const SkBitmap& bitmap, std::string_view label, skgpu::Mipmapped mipmapped)
{
    ...
    if (!proxy) {
        proxy = make_bmp_proxy(proxyProvider, bitmap, ct, mipmapped, SkBackingFit::kExact, skgpu::Budgeted::kYes);
        if (!proxy) {
            return {};
        }
    ...
    auto mippedProxy = GrCopyBaseMipMapToTextureProxy(rContext, proxy, kTopLeft_GrSurfaceOrigin, /*label=*/"MakeCachedBitmapProxyView");
    if (!mippedProxy) {
...
std::tuple<GrSurfaceProxyView, GrColorType> GrMakeUncachedBitmapProxyView(
    GrRecordingContext* rContext, const SkBitmap& bitmap, skgpu::Mipmapped mipmapped, SkBackingFit fit, skgpu::Budgeted budgeted)
{
    GrProxyProvider* proxyProvider = rContext->priv().proxyProvider();
    const GrCaps* caps = rContext->priv().caps();
...
static std::unique_ptr<GrFragmentProcessor> make_dither_effect(
    GrRecordingContext* rContext, std::unique_ptr<GrFragmentProcessor> inputFP, float range, const GrCaps* caps)
{
    ...
    auto [tex, ct] = GrMakeCachedBitmapProxyView(rContext, gLUT, /*label=*/"MakeDitherEffect", skgpu::Mipmapped::kNo);
    if (!tex) {
        return inputFP;
    }
    SkASSERT(ct == GrColorType::kAlpha_8);
    GrSamplerState sampler(GrSamplerState::WrapMode::kRepeat, SkFilterMode::kNearest);
    auto te = GrTextureEffect::Make(std::move(tex), kPremul_SkAlphaType, SkMatrix::I(), sampler, *caps);
    static const SkRuntimeEffect* effect = SkMakeRuntimeEffect(SkRuntimeEffect::MakeForShader,
        ...
        "}");
    return GrSkSLFP::Make(effect, "Dither", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kPreservesOpaqueInput, "range", range, "inputFP", std::move(inputFP),
        "table", GrSkSLFP::IgnoreOptFlags(std::move(te)));
}
#endif

static inline bool skpaint_to_grpaint_impl(GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkPaint& skPaint, const SkMatrix& ctm,
    std::optional<std::unique_ptr<GrFragmentProcessor>> shaderFP, SkBlender* primColorBlender, const SkSurfaceProps& surfaceProps, GrPaint* grPaint)
{
    ...
    if (paintFP) {
        if (primColorBlender) {
            ...
            if (1.0f != paintAlpha) {
                ...
                paintFP = GrFragmentProcessor::ModulateRGBA(std::move(paintFP), { paintAlpha, paintAlpha, paintAlpha, paintAlpha });
            }
        } else {
            float paintAlpha = skPaint.getColor4f().fA;
            if (paintAlpha != 1.0f) {
    ...
    if (colorFilter) {
        if (applyColorFilterToPaintColor) {
            ...
            auto [success, fp] = GrFragmentProcessors::Make(context, colorFilter, std::move(paintFP), dstColorInfo, surfaceProps);
            if (!success) {
                return false;
            }
            paintFP = std::move(fp);
    ...
    if (paintFP != nullptr && (surfaceProps.isAlwaysDither() || SkPaintPriv::ShouldDither(skPaint, ct))) {
    ...
    if (auto bm = skPaint.asBlendMode()) {
        ...
        paintFP = GrFragmentProcessors::Make(as_BB(skPaint.getBlender()), std::move(paintFP), GrFragmentProcessor::SurfaceColor(), fpArgs);
        if (!paintFP) {
            return false;
        }
    ...
    if (GrColorTypeClampType(dstColorInfo.colorType()) == GrClampType::kManual) {
        if (paintFP != nullptr) {
            ...
            grPaint->setColor4f({ SkTPin(color.fR, 0.f, 1.f), SkTPin(color.fG, 0.f, 1.f), SkTPin(color.fB, 0.f, 1.f), SkTPin(color.fA, 0.f, 1.f) });
        }
    }
...
bool SkPaintToGrPaint(GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkPaint& skPaint, const SkMatrix& ctm,
    const SkSurfaceProps& surfaceProps, GrPaint* grPaint)
{
    return skpaint_to_grpaint_impl(context, dstColorInfo, skPaint, ctm,
        /*shaderFP=*/std::nullopt,
        /*primColorBlender=*/nullptr, surfaceProps, grPaint);
}

/** Replaces the SkShader (if any) on skPaint with the passed in GrFragmentProcessor. */
bool SkPaintToGrPaintReplaceShader(GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkPaint& skPaint, const SkMatrix& ctm,
    std::unique_ptr<GrFragmentProcessor> shaderFP, const SkSurfaceProps& surfaceProps, GrPaint* grPaint)
{
    return skpaint_to_grpaint_impl(context, dstColorInfo, skPaint, ctm, std::move(shaderFP),
        /*primColorBlender=*/nullptr, surfaceProps, grPaint);
}

/** Blends the SkPaint's shader (or color if no shader) with a per-primitive color which must
    be setup as a vertex attribute using the specified SkBlender. */
bool SkPaintToGrPaintWithBlend(GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkPaint& skPaint, const SkMatrix& ctm,
    SkBlender* primColorBlender, const SkSurfaceProps& surfaceProps, GrPaint* grPaint)
{
    return skpaint_to_grpaint_impl(context, dstColorInfo, skPaint, ctm,
        /*shaderFP=*/std::nullopt, primColorBlender, surfaceProps, grPaint);
}
