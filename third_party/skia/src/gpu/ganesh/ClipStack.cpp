...
template <typename A, typename B> ClipGeometry get_clip_geometry(const A& a, const B& b)
{
...
bool shape_contains_rect(const GrShape& a, const SkMatrix& aToDevice, const SkMatrix& deviceToA, const SkRect& b, const SkMatrix& bToDevice, bool mixedAAMode)
{
    if (!a.convex()) {
        return false;
    }
...
GrFPResult analytic_clip_fp(const skgpu::ganesh::ClipStack::Element& e, const GrShaderCaps& caps, std::unique_ptr<GrFragmentProcessor> fp)
{
...
// into a shared atlas.
GrFPResult clip_atlas_fp(const skgpu::ganesh::SurfaceDrawContext* sdc, const GrOp* opBeingClipped, skgpu::ganesh::AtlasPathRenderer* atlasPathRenderer,
    const SkIRect& scissorBounds, const skgpu::ganesh::ClipStack::Element& e, std::unique_ptr<GrFragmentProcessor> inputFP)
{
    if (e.fAA != GrAA::kYes) {
    ...
    return atlasPathRenderer->makeAtlasClipEffect(sdc, opBeingClipped, std::move(inputFP), scissorBounds, e.fLocalToDevice, path);
}

void draw_to_sw_mask(GrSWMaskHelper* helper, const skgpu::ganesh::ClipStack::Element& e, bool clearMask)
{
...
GrSurfaceProxyView render_sw_mask(GrRecordingContext* context, const SkIRect& bounds, const skgpu::ganesh::ClipStack::Element** elements, int count)
{
    SkASSERT(count > 0);
    ...
    if (taskGroup) {
        ...
        auto proxy = proxyProvider->createProxy(format, bounds.size(), GrRenderable::kNo, 1, skgpu::Mipmapped::kNo, SkBackingFit::kApprox,
            skgpu::Budgeted::kYes, GrProtected::kNo,
...
void render_stencil_mask(GrRecordingContext* rContext, skgpu::ganesh::SurfaceDrawContext* sdc, uint32_t genID, const SkIRect& bounds,
    const skgpu::ganesh::ClipStack::Element** elements, int count, GrAppliedClip* out)
{
...
ClipStack::RawElement::RawElement(const SkMatrix& localToDevice, const GrShape& shape, GrAA aa, SkClipOp op)
...
bool ClipStack::RawElement::contains(const Draw& d) const
{
    if (fInnerBounds.contains(d.outerBounds())) {
        ...
        return shape_contains_rect(fShape, fLocalToDevice, fDeviceToLocal, queryBounds, SkMatrix::I(), /* mixed-aa */ false);
    }
}
...
bool ClipStack::RawElement::contains(const RawElement& e) const
{
    ...
    if (!mixedAA && fLocalToDevice == e.fLocalToDevice) {
        ...
        if (fShape.isRRect() && e.fShape.isRRect()) {
            return SkRRectPriv::ConservativeIntersect(fShape.rrect(), e.fShape.rrect()) == e.fShape.rrect();
        } else if (fShape.isPath() && e.fShape.isPath()) {
            return fShape.path().getGenerationID() == e.fShape.path().getGenerationID()
                || (fShape.path().getPoints(nullptr, 0) <= kMaxPathComparePoints && fShape.path() == e.fShape.path());
        } // else fall through to shape_contains_rect
    }

    return shape_contains_rect(fShape, fLocalToDevice, fDeviceToLocal, e.fShape.bounds(), e.fLocalToDevice, mixedAA);
}
...
bool ClipStack::RawElement::combine(const RawElement& other, const SaveRecord& current)
{
    ...
    if (fShape.isRect() && other.fShape.isRect()) {
    ...
    } else if ((fShape.isRect() || fShape.isRRect()) && (other.fShape.isRect() || other.fShape.isRRect())) {
        ...
        if (fAA == other.fAA && fLocalToDevice == other.fLocalToDevice) {
            ...
            SkRRect b = other.fShape.isRect() ? SkRRect::MakeRect(other.fShape.rect()) : other.fShape.rrect();
...
ClipStack::ClipState ClipStack::RawElement::clipType() const
{
    ...
    case GrShape::Type::kRect:
        return fOp == SkClipOp::kIntersect && fLocalToDevice.isIdentity() ? ClipState::kDeviceRect : ClipState::kComplex;

    case GrShape::Type::kRRect:
        return fOp == SkClipOp::kIntersect && fLocalToDevice.isIdentity() ? ClipState::kDeviceRRect : ClipState::kComplex;
...
void ClipStack::Mask::invalidate(GrProxyProvider* proxyProvider)
{
    ...
    proxyProvider->processInvalidUniqueKey(fKey, nullptr, GrProxyProvider::InvalidateGPUResource::kYes);
    fKey.reset();
}
...
ClipStack::SaveRecord::SaveRecord(const SaveRecord& prior, int startingMaskIndex, int startingElementIndex)
...
bool ClipStack::SaveRecord::addElement(RawElement&& toAdd, RawElement::Stack* elements)
{
    ...
    SkASSERT(
        (toAdd.shape().isEmpty() || !toAdd.outerBounds().isEmpty()) && (toAdd.innerBounds().isEmpty() || toAdd.outerBounds().contains(toAdd.innerBounds())));
    ...
    if (fStackOp == SkClipOp::kIntersect) {
        ...
        if (toAdd.op() == SkClipOp::kIntersect) {
            ...
            if (toAdd.innerBounds().width() * toAdd.innerBounds().height() > fInnerBounds.width() * fInnerBounds.height()) {
    ...
    SkASSERT(!fOuterBounds.isEmpty() && (fInnerBounds.isEmpty() || fOuterBounds.contains(fInnerBounds)));
...
bool ClipStack::SaveRecord::appendElement(RawElement&& toAdd, RawElement::Stack* elements)
{
    ...
    SkASSERT(oldestValid == elements->count() || (oldestValid >= fOldestValidIndex && oldestValid < elements->count()));
    SkASSERT(youngestValid == fStartingElementIndex - 1 || (youngestValid >= fStartingElementIndex && youngestValid < elements->count()));
    SkASSERT(
        (oldestActiveInvalid && oldestActiveInvalidIndex >= fStartingElementIndex && oldestActiveInvalidIndex < elements->count()) || !oldestActiveInvalid);
...
GrClip::Effect ClipStack::apply(GrRecordingContext* rContext, SurfaceDrawContext* sdc, GrDrawOp* op, GrAAType aa, GrAppliedClip* out, SkRect* bounds) const
{
    ...
    if (cs.shader()) {
        ...
        GrFPArgs args(rContext, &kCoverageColorInfo, sdc->surfaceProps(), GrFPArgs::Scope::kDefault);
    ...
    case ClipGeometry::kBoth:
        ...
        SkASSERT(cs.state() == ClipState::kDeviceRect || cs.state() == ClipState::kDeviceRRect || cs.state() == ClipState::kComplex);
        break;
    }
    ...
    for (const RawElement& e : fElements.ritems()) {
        ...
        case ClipGeometry::kBoth: {
            ...
            auto result = op->clipToShape(sdc, e.op(), e.localToDevice(), e.shape(), GrAA(e.aa() == GrAA::kYes || fForceAA));
            ...
            if (!fullyApplied && remainingAnalyticFPs > 0) {
                std::tie(fullyApplied, clipFP) = analytic_clip_fp(e.asElement(), *caps->shaderCaps(), std::move(clipFP));
                if (!fullyApplied && atlasPathRenderer) {
                    std::tie(fullyApplied, clipFP) = clip_atlas_fp(sdc, op, atlasPathRenderer, scissorBounds, e.asElement(), std::move(clipFP));
                }
                if (fullyApplied) {
    ...
    if (!elementsForMask.empty()) {
        ...
        if ((sdc->numSamples() <= 1 && !sdc->canUseDynamicMSAA() && maskRequiresAA) || stencilUnavailable) {
            ...
            std::tie(hasSWMask, clipFP) = GetSWMaskFP(rContext, &fMasks, cs, scissorBounds, elementsForMask.begin(), elementsForMask.size(), std::move(clipFP));
        }
        ...
        if (!hasSWMask) {
            if (stencilUnavailable) {
                ...
                render_stencil_mask(rContext, sdc, cs.genID(), scissorBounds, elementsForMask.begin(), elementsForMask.size(), out);
            }
        }
    }
...
void ClipStack::clip(RawElement&& element)
{
    ...
    bool wasDeferred;
    SaveRecord& save = this->writableSaveRecord(&wasDeferred);
    SkDEBUGCODE(uint32_t oldGenID = save.genID();) SkDEBUGCODE(int elementCount = fElements.count();) if (!save.addElement(std::move(element), &fElements))
    {
        if (wasDeferred) {
...
GrFPResult ClipStack::GetSWMaskFP(GrRecordingContext* context, Mask::Stack* masks, const SaveRecord& current, const SkIRect& bounds, const Element** elements,
    int count, std::unique_ptr<GrFragmentProcessor> clipFP)
{
    GrProxyProvider* proxyProvider = context->priv().proxyProvider();
    ...
    for (const Mask& m : masks->ritems()) {
        ...
        if (m.appliesToDraw(current, bounds)) {
            maskProxy = proxyProvider->findCachedProxyWithColorTypeFallback(m.key(), kMaskOrigin, GrColorType::kAlpha_8, 1);
            if (maskProxy) {
                maskBounds = m.bounds();
                break;
            }
    ...
    GrSamplerState samplerState(GrSamplerState::WrapMode::kClampToBorder, GrSamplerState::Filter::kNearest);
    ...
    auto fp = GrTextureEffect::MakeSubset(std::move(maskProxy), kPremul_SkAlphaType, m, samplerState, subset, domain, *context->priv().caps());
