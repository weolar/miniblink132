...
{
    switch (t) {
    case GrShaderVar::TypeModifier::None:
        return "";
    case GrShaderVar::TypeModifier::In:
        return "in";
    case GrShaderVar::TypeModifier::InOut:
        return "inout";
    case GrShaderVar::TypeModifier::Out:
        return "out";
    case GrShaderVar::TypeModifier::Uniform:
        return "uniform";
    }
...
void GrShaderVar::appendDecl(const GrShaderCaps* shaderCaps, SkString* out) const
{
    ...
    if (this->isArray()) {
        ...
        out->appendf("%s %s[%d]", SkSLTypeString(effectiveType), this->getName().c_str(), this->getArrayCount());
    } else {
