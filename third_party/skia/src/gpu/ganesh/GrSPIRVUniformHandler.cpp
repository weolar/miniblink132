    ...
    const GrProcessor* owner, uint32_t visibility, SkSLType type, const char* name, bool mangleName, int arrayCount, const char** outName)
{
    ...
    tempInfo.fVariable = GrShaderVar { std::move(resolvedName), type, GrShaderVar::TypeModifier::None, arrayCount, std::move(layoutQualifier), SkString() };
...
GrGLSLUniformHandler::SamplerHandle GrSPIRVUniformHandler::addSampler(
    const GrBackendFormat& backendFormat, GrSamplerState, const skgpu::Swizzle& swizzle, const char* name, const GrShaderCaps* caps)
{
    ...
    SkString layoutQualifier = SkStringPrintf("direct3d, set = %d, sampler = %d, texture = %d", kSamplerTextureDescriptorSet, binding, binding + 1);

    SPIRVUniformInfo& uniformInfo = fSamplers.emplace_back();
    uniformInfo.fVariable = GrShaderVar { std::move(mangleName), SkSLCombinedSamplerTypeForTextureType(backendFormat.textureType()),
...
const char* GrSPIRVUniformHandler::samplerVariable(GrGLSLUniformHandler::SamplerHandle handle) const
{
    return fSamplers.item(handle.toIndex()).fVariable.getName().c_str();
}

skgpu::Swizzle GrSPIRVUniformHandler::samplerSwizzle(GrGLSLUniformHandler::SamplerHandle handle) const
{
