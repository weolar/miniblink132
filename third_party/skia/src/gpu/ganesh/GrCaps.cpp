...
void GrCaps::dumpJSON(SkJSONWriter* writer) const
{
    ...
    writer->appendBool("Two-sided Stencil Refs And Masks Must Match", fTwoSidedStencilRefsAndMasksMustMatch);
    ...
    writer->appendBool("Specify GeometryProcessor textures as a dynamic state array", fDynamicStateArrayGeometryProcessorTextureSupport);
    ...
    writer->appendBool("Supports transfers from buffers to textures", fTransferFromBufferToTextureSupport);
    writer->appendBool("Supports transfers from textures to buffers", fTransferFromSurfaceToBufferSupport);
    ...
    writer->appendBool("Disable TessellationPathRenderer current driver [workaround]", fDisableTessellationPathRenderer);
    ...
    writer->appendBool("Native draw indexed indirect is broken [workaround]", fNativeDrawIndexedIndirectIsBroken);
    ...
    writer->appendCString("Blend Equation Support", kBlendEquationSupportNames[fBlendEquationSupport]);
...
bool GrCaps::canCopySurface(const GrSurfaceProxy* dst, const SkIRect& dstRect, const GrSurfaceProxy* src, const SkIRect& srcRect) const
{
    if (dst->readOnly()) {
        return false;
    }
    ...
    if (!SkIRect::MakeSize(dst->dimensions()).contains(dstRect) || !SkIRect::MakeSize(src->dimensions()).contains(srcRect)) {
        return false;
    }
    return this->onCanCopySurface(dst, dstRect, src, srcRect);
}

bool GrCaps::validateSurfaceParams(const SkISize& dimensions, const GrBackendFormat& format, GrRenderable renderable, int renderTargetSampleCnt,
...
GrCaps::SupportedRead GrCaps::supportedReadPixelsColorType(GrColorType srcColorType, const GrBackendFormat& srcFormat, GrColorType dstColorType) const
{
    ...
    if ((channelFlags == kRGBA_SkColorChannelFlags || channelFlags == kRGB_SkColorChannelFlags || channelFlags == kAlpha_SkColorChannelFlag
            || channelFlags == kGray_SkColorChannelFlag)
...
GrBackendFormat GrCaps::getDefaultBackendFormat(GrColorType colorType, GrRenderable renderable) const
{
    ...
    if (this->supportedWritePixelsColorType(colorType, format, colorType).fColorType == GrColorType::kUnknown) {
        return {};
    }
...
bool GrCaps::areColorTypeAndFormatCompatible(GrColorType grCT, const GrBackendFormat& format) const
{
    ...
    if (compression != SkTextureCompressionType::kNone) {
        return grCT == (SkTextureCompressionTypeIsOpaque(compression) ? GrColorType::kRGB_888x : GrColorType::kRGBA_8888);
    }
...
skgpu::Swizzle GrCaps::getReadSwizzle(const GrBackendFormat& format, GrColorType colorType) const
{
    ...
    if (compression != SkTextureCompressionType::kNone) {
        ...
        SkDEBUGFAILF("Illegal color type (%d) and compressed format (%d) combination.", (int)colorType, (int)compression);
        return {};
    }
...
GrDstSampleFlags GrCaps::getDstSampleFlagsForProxy(const GrRenderTargetProxy* rt, bool drawUsesMSAA) const
{
    SkASSERT(rt);
...
{
    return rtProxy->numSamples() == 1 && this->internalMultisampleCount(rtProxy->backendFormat()) > 1 && this->onSupportsDynamicMSAA(rtProxy);
}
...
std::tuple<GrColorType, GrBackendFormat> GrCaps::getFallbackColorTypeAndFormat(GrColorType ct, int sampleCnt) const
{
    do {
