...
GrPipeline::GrPipeline(const InitArgs& args, sk_sp<const GrXferProcessor> xferProcessor, const GrAppliedHardClip& hardClip)
    ...
    : GrPipeline(args, processors.refXferProcessor(), appliedClip.hardClip())
{
    ...
    int numTotalProcessors = fNumColorProcessors + (processors.hasCoverageFragmentProcessor() ? 1 : 0) + (appliedClip.hasCoverageFragmentProcessor() ? 1 : 0);
...
GrPipeline::GrPipeline(GrScissorTest scissorTest, sk_sp<const GrXferProcessor> xp, const skgpu::Swizzle& writeSwizzle, InputFlags inputFlags)
    : fWindowRectsState()
...
void GrPipeline::setDstTextureUniforms(const GrGLSLProgramDataManager& pdm, GrGLSLBuiltinUniformHandles* fBuiltinUniformHandles) const
{
    ...
    if (dstTexture) {
        if (fBuiltinUniformHandles->fDstTextureCoordsUni.isValid()) {
            ...
            pdm.set4f(fBuiltinUniformHandles->fDstTextureCoordsUni, static_cast<float>(this->dstTextureOffset().fX),
                static_cast<float>(this->dstTextureOffset().fY), scaleX, scaleY);
        }
    } else {
