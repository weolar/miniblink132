...
sk_sp<SkData> PackCachedShaders(
    SkFourByteTag shaderType, const std::string shaders[], const SkSL::Program::Interface[], int numInterfaces, const ShaderMetadata* meta = nullptr);

SkFourByteTag GetType(SkReadBuffer* reader);

bool UnpackCachedShaders(SkReadBuffer* reader, std::string shaders[], SkSL::Program::Interface[], int numInterfaces, ShaderMetadata* meta = nullptr);
