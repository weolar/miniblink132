...
const char* GrGLSLFragmentShaderBuilder::dstColor()
{
    ...
    if (shaderCaps->fFBFetchSupport) {
        this->addFeature(1 << kFramebufferFetch_GLSLPrivateFeature, shaderCaps->fFBFetchExtensionString);
...
void GrGLSLFragmentShaderBuilder::enableAdvancedBlendEquationIfNeeded(skgpu::BlendEquation equation)
{
    SkASSERT(skgpu::BlendEquationIsAdvanced(equation));

    if (fProgramBuilder->shaderCaps()->mustEnableAdvBlendEqs()) {
        this->addFeature(1 << kBlendEquationAdvanced_GLSLPrivateFeature, "GL_KHR_blend_equation_advanced");
...
void GrGLSLFragmentShaderBuilder::enableSecondaryOutput()
{
    ...
    if (caps.mustDeclareFragmentShaderOutput()) {
        fOutputs.emplace_back(DeclaredSecondaryColorOutputName(), SkSLType::kHalf4, GrShaderVar::TypeModifier::Out);
...
const char* GrGLSLFragmentShaderBuilder::getSecondaryColorOutputName() const
{
    if (this->hasSecondaryOutput()) {
        return (fProgramBuilder->shaderCaps()->mustDeclareFragmentShaderOutput()) ? DeclaredSecondaryColorOutputName() : "sk_SecondaryFragColor";
    }
    return nullptr;
}
