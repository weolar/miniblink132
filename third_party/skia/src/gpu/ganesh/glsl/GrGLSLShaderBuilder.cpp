...
void GrGLSLShaderBuilder::appendFunctionDecl(SkSLType returnType, const char* mangledName, SkSpan<const GrShaderVar> args)
{
...
void GrGLSLShaderBuilder::emitFunction(SkSLType returnType, const char* mangledName, SkSpan<const GrShaderVar> args, const char* body)
{
...
void GrGLSLShaderBuilder::emitFunctionPrototype(SkSLType returnType, const char* mangledName, SkSpan<const GrShaderVar> args)
{
    this->appendFunctionDecl(returnType, mangledName, args);
    this->functions().append(";\n");
}
...
void GrGLSLShaderBuilder::appendTextureLookup(SkString* out, SamplerHandle samplerHandle, const char* coordName) const
{
...
void GrGLSLShaderBuilder::appendTextureLookup(SamplerHandle samplerHandle, const char* coordName, GrGLSLColorSpaceXformHelper* colorXformHelper)
{
    SkString lookup;
...
void GrGLSLShaderBuilder::appendTextureLookupAndBlend(
    const char* dst, SkBlendMode mode, SamplerHandle samplerHandle, const char* coordName, GrGLSLColorSpaceXformHelper* colorXformHelper)
{
    if (!dst) {
        dst = "half4(1)";
    }
...
void GrGLSLShaderBuilder::appendColorGamutXform(SkString* out, const char* srcColor, GrGLSLColorSpaceXformHelper* colorXformHelper)
{
    ...
    auto emitTFFunc = [this, &uniformHandler](const char* name, GrGLSLProgramDataManager::UniformHandle uniform, skcms_TFType tfType) {
        ...
        this->emitFunction(SkSLType::kFloat, funcName.c_str(), { gTFArgs, std::size(gTFArgs) }, body.c_str());
        return funcName;
    };

    SkString srcTFFuncName;
    if (colorXformHelper->applySrcTF()) {
        srcTFFuncName = emitTFFunc("src_tf", colorXformHelper->srcTFUniform(), colorXformHelper->srcTFType());
    }

    SkString dstTFFuncName;
    if (colorXformHelper->applyDstTF()) {
        dstTFFuncName = emitTFFunc("dst_tf", colorXformHelper->dstTFUniform(), colorXformHelper->dstTFType());
    }
    ...
    if (colorXformHelper->applyGamutXform()) {
        ...
        this->emitFunction(SkSLType::kFloat4, gamutXformFuncName.c_str(), { gGamutXformArgs, std::size(gGamutXformArgs) }, body.c_str());
    }
    ...
    // Now define a wrapper function that applies all the intermediate steps
    {
        ...
        this->emitFunction(SkSLType::kHalf4, colorXformFuncName.c_str(), { gColorXformArgs, std::size(gColorXformArgs) }, body.c_str());
        out->appendf("%s(%s)", colorXformFuncName.c_str(), srcColor);
    }
}

void GrGLSLShaderBuilder::appendColorGamutXform(const char* srcColor, GrGLSLColorSpaceXformHelper* colorXformHelper)
{
    SkString xform;
...
{
    SkASSERT(fProgramBuilder->shaderCaps()->fGLSLGeneration >= SkSL::GLSLGeneration::k330 || fProgramBuilder->shaderCaps()->mustEnableAdvBlendEqs());
