...
std::string BlendExpression(const GrProcessor* processor, GrGLSLUniformHandler* uniformHandler, GrGLSLProgramDataManager::UniformHandle* blendUniform,
    const char* srcColor, const char* dstColor, SkBlendMode mode)
{
    ...
    *blendUniform = uniformHandler->addUniform(processor, kFragment_GrShaderFlag, skslType, "blend", &blendUniName);
...
void SetBlendModeUniformData(const GrGLSLProgramDataManager& pdman, GrGLSLProgramDataManager::UniformHandle blendUniform, SkBlendMode mode)
{
    ...
    case 3:
        ...
        break;
    case 4:
        pdman.set4f(blendUniform, info.fUniformData[0], info.fUniformData[1], info.fUniformData[2], info.fUniformData[3]);
        break;
    }
}
