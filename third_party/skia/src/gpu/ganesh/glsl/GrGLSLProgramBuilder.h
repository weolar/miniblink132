...
public:
    ...
    {
        return fProgramInfo.origin();
    }
    const GrPipeline& pipeline() const
    {
        return fProgramInfo.pipeline();
    }
    const GrGeometryProcessor& geometryProcessor() const
    {
        return fProgramInfo.geomProc();
    }
    ...
    {
        fStageIndex++;
        SkDEBUGCODE(fFS.debugOnly_resetPerStageVerification();) fFS.nextStage();
    }
...
private:
    ...
    SkString emitRootFragProc(const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl, const SkString& input, SkString output);
    ...
    SamplerHandle emitSampler(const GrBackendFormat&, GrSamplerState, const skgpu::Swizzle&, const char* name);
