...
void GrGLSLVaryingHandler::addPassThroughAttribute(const GrShaderVar& vsVar, const char* output, Interpolation interpolation)
{
...
void GrGLSLVaryingHandler::addVarying(const char* name, GrGLSLVarying* varying, Interpolation interpolation)
{
...
void GrGLSLVaryingHandler::finalize()
{
    for (const VaryingInfo& v : fVaryings.items()) {
        ...
        if (v.fVisibility & kVertex_GrShaderFlag) {
            fVertexOutputs.emplace_back(v.fVsOut, v.fType, GrShaderVar::TypeModifier::Out, GrShaderVar::kNonArray, SkString(), SkString(modifier));
        }
        if (v.fVisibility & kFragment_GrShaderFlag) {
            const char* fsIn = v.fVsOut.c_str();
            fFragInputs.emplace_back(SkString(fsIn), v.fType, GrShaderVar::TypeModifier::In, GrShaderVar::kNonArray, SkString(), SkString(modifier));
        }
    }
    this->onFinalize();
}
...
void GrGLSLVaryingHandler::getFragDecls(SkString* inputDecls, SkString* outputDecls) const
{
    ...
    SkASSERT(SkSL::GLSLGeneration::k110 != fProgramBuilder->shaderCaps()->fGLSLGeneration || fFragOutputs.empty());
