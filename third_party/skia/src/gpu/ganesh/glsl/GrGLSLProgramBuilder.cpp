...
GrGLSLProgramBuilder::GrGLSLProgramBuilder(const GrProgramDesc& desc, const GrProgramInfo& programInfo)
    : fVS(this)
    , fFS(this)
    , fDesc(desc)
...
void GrGLSLProgramBuilder::addFeature(GrShaderFlags shaders, uint32_t featureBit, const char* extensionName)
{
...
bool GrGLSLProgramBuilder::emitAndInstallPrimProc(SkString* outputColor, SkString* outputCoverage)
{
    ...
    fUniformHandles.fRTAdjustmentUni = this->uniformHandler()->addUniform(nullptr, kVertex_GrShaderFlag, SkSLType::kFloat4, SkSL::Compiler::RTADJUST_NAME);
    ...
    for (int i = 0; i < geomProc.numTextureSamplers(); ++i) {
        ...
        texSamplers[i] = this->emitSampler(sampler.backendFormat(), sampler.samplerState(), sampler.swizzle(), name.c_str());
        if (!texSamplers[i].isValid()) {
            return false;
        }
    }

    GrGeometryProcessor::ProgramImpl::EmitArgs args(&fVS, &fFS, this->varyingHandler(), this->uniformHandler(), this->shaderCaps(), geomProc,
...
bool GrGLSLProgramBuilder::emitTextureSamplersForFPs(const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl, int* samplerIndex)
{
    bool ok = true;
    fp.visitWithImpls(
...
std::string GrGLSLProgramBuilder::invokeFP(
    const GrFragmentProcessor& fp, const GrFragmentProcessor::ProgramImpl& impl, const char* inputColor, const char* destColor, const char* coords) const
{
    if (fp.isBlendFunction()) {
        if (this->fragmentProcessorHasCoordsParam(&fp)) {
            return SkSL::String::printf("%s(%s, %s, %s)", impl.functionName(), inputColor, destColor, coords);
        } else {
...
SkString GrGLSLProgramBuilder::emitRootFragProc(const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl, const SkString& input, SkString output)
{
    SkASSERT(input.size());
    ...
    fFS.codeAppendf("%s = %s;", output.c_str(), this->invokeFP(fp, impl, input.c_str(), "half4(1)", fLocalCoordsVar.c_str()).c_str());
...
void GrGLSLProgramBuilder::writeChildFPFunctions(const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl)
{
...
void GrGLSLProgramBuilder::writeFPFunction(const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl)
{
    ...
    if (fpCoordsIter == fFPCoordsMap.end()) {
        ...
        case SkSLType::kFloat3:
            ...
            fFS.codeAppendf("float2 %s = %s.xy / %s.z;\n", sampleCoords, varying.getName().c_str(), varying.getName().c_str());
            break;
        default:
    ...
    GrFragmentProcessor::ProgramImpl::EmitArgs args(&fFS, this->uniformHandler(), this->shaderCaps(), fp, inputColor, kDstColor, sampleCoords);

    impl.emitCode(args);
    impl.setFunctionName(fFS.getMangledFunctionName(args.fFp.name()));

    fFS.emitFunction(SkSLType::kHalf4, impl.functionName(), SkSpan(params, numParams), fFS.code().c_str());
    fFS.deleteStage();
}
...
bool GrGLSLProgramBuilder::emitAndInstallDstTexture()
{
    ...
    if (this->pipeline().usesDstTexture()) {
        ...
        fDstTextureSamplerHandle = this->emitSampler(dstTextureProxy->backendFormat(), GrSamplerState(), swizzle, "DstTextureSampler");
        ...
        fUniformHandles.fDstTextureCoordsUni = this->uniformHandler()->addUniform(
            /*owner=*/nullptr, kFragment_GrShaderFlag, SkSLType::kHalf4, "DstTextureCoords", &dstTextureCoordsName);
...
bool GrGLSLProgramBuilder::emitAndInstallXferProc(const SkString& colorIn, const SkString& coverageIn)
{
    // Program builders have a bit of state we need to clear with each effect
    this->advanceStage();
    ...
    GrXferProcessor::ProgramImpl::EmitArgs args(&fFS, this->uniformHandler(), this->shaderCaps(), xp, finalInColor.c_str(),
        coverageIn.size() ? coverageIn.c_str() : "float4(1)", fFS.getPrimaryColorOutputName(), fFS.getSecondaryColorOutputName(), fDstTextureSamplerHandle,
        fDstTextureOrigin, this->pipeline().writeSwizzle());
...
GrGLSLProgramBuilder::SamplerHandle GrGLSLProgramBuilder::emitSampler(
    const GrBackendFormat& backendFormat, GrSamplerState state, const skgpu::Swizzle& swizzle, const char* name)
{
    ++fNumFragmentSamplers;
    return this->uniformHandler()->addSampler(backendFormat, state, swizzle, name, this->shaderCaps());
}

GrGLSLProgramBuilder::SamplerHandle GrGLSLProgramBuilder::emitInputSampler(const skgpu::Swizzle& swizzle, const char* name)
{
...
void GrGLSLProgramBuilder::addRTFlipUniform(const char* name)
{
    ...
    fUniformHandles.fRTFlipUni = uniformHandler->internalAddUniformArray(nullptr, kFragment_GrShaderFlag, SkSLType::kFloat2, name, false, 0, nullptr);
}
...
bool GrGLSLProgramBuilder::fragmentProcessorHasCoordsParam(const GrFragmentProcessor* fp) const
{
    ...
    return (iter != fFPCoordsMap.end()) ? iter->second.hasCoordsParam : fp->usesSampleCoords();
}
