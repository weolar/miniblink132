...
private:
    ...
    typedef skia_private::THashMap<skgpu::UniqueKey, Register*, UniqueKeyHash> UniqueKeyRegisterHash;
    ...
    public:
        ...
        const skgpu::ScratchKey& scratchKey() const
        {
            return fScratchKey;
        }
        const skgpu::UniqueKey& uniqueKey() const
        {
            return fOriginatingProxy->getUniqueKey();
        }

        bool accountedForInBudget() const
        {
            return fAccountedForInBudget;
        }
        void setAccountedForInBudget()
        {
            fAccountedForInBudget = true;
        }

        GrSurface* existingSurface() const
        {
            return fExistingSurface.get();
        }

        // Can this register be used by other proxies after this one?
        bool isRecyclable(const GrCaps&, GrSurfaceProxy* proxy, int knownUseCount, AllowRecycling) const;
        ...
        SkDEBUGCODE(uint32_t uniqueID() const { return fUniqueID; })
            ...
            private : GrSurfaceProxy* fOriginatingProxy;
    ...
    SkDEBUGCODE(bool fPlanned = false;) SkDEBUGCODE(bool fAssigned = false;)
