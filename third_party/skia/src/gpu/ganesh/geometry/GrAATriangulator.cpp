...
void GrAATriangulator::makeEvent(SSEdge* edge, Vertex* v, SSEdge* other, Vertex* dest, EventList* events, const Comparator& c) const
{
    if (!v->fPartner) {
        return;
    }
    ...
    if (line.intersect(bisector.fLine, &p) && !c.sweep_lt(p, top->fPoint) && c.sweep_lt(p, bottom->fPoint)) {
...
void GrAATriangulator::simplifyBoundary(EdgeList* boundary, const Comparator& c)
{
    ...
    for (Edge* e = boundary->fHead; e != nullptr;) {
        ...
        } else if (prevNormal.dot(normal) < 0.0 && (distPrev * distPrev <= kQuarterPixelSq || distNext * distNext <= kQuarterPixelSq)) {
...
void GrAATriangulator::Event::apply(VertexList* mesh, const Comparator& c, EventList* events, GrAATriangulator* triangulator)
{
    if (!fEdge) {
        return;
    }
...
bool GrAATriangulator::collapseOverlapRegions(VertexList* mesh, const Comparator& c, EventComparator comp)
{
    ...
    for (Vertex* v = mesh->fHead; v != nullptr; v = v->fNext) {
        ...
        for (Edge* e = v->fLastEdgeAbove; e && e != leftEnclosingEdge;) {
            ...
            bool isOuterBoundary = e->fType == EdgeType::kOuter && (!prev || prev->fWinding == 0 || e->fWinding == 0);
            if (prev) {
                e->fWinding -= prev->fWinding;
            }
            if (leftOverlap && rightOverlap) {
                ...
                TESS_LOG("found overlap edge %g -> %g%s\n", e->fTop->fID, e->fBottom->fID, isOuterBoundary ? ", is outer boundary" : "");
...
void GrAATriangulator::strokeBoundary(EdgeList* boundary, VertexList* innerMesh, const Comparator& c)
{
    TESS_LOG("\nstroking boundary\n");
    ...
    for (Edge* e = boundary->fHead; e != nullptr; e = e->fRight) {
        ...
        if (!prevEdge->fLine.nearParallel(e->fLine) && prevInner.intersect(inner, &innerPoint) && prevOuter.intersect(outer, &outerPoint)) {
            ...
            if (cosAngle < -kCosMiterAngle) {
                ...
                if (prevNormal.cross(normal) > 0) {
                    // Miter inner points
                    if (!innerTangent.intersect(prevInner, &innerPoint1) || !innerTangent.intersect(inner, &innerPoint2)
                        || !outerTangent.intersect(bisector, &outerPoint)) {
                        continue;
                    }
                    Line prevTangent(prevV->fPoint, prevV->fPoint + SkVector::Make(prevOuter.fA, prevOuter.fB));
                    ...
                    // Miter outer points
                    if (!outerTangent.intersect(prevOuter, &outerPoint1) || !outerTangent.intersect(outer, &outerPoint2)) {
                        continue;
                    }
                    Line prevTangent(prevV->fPoint, prevV->fPoint + SkVector::Make(prevInner.fA, prevInner.fB));
                ...
                if (!innerPoint1.isFinite() || !innerPoint2.isFinite() || !outerPoint1.isFinite() || !outerPoint2.isFinite()) {
                    continue;
                }
                TESS_LOG("inner (%g, %g), (%g, %g), ", innerPoint1.fX, innerPoint1.fY, innerPoint2.fX, innerPoint2.fY);
    ...
    this->makeConnectingEdge(innerVertices.fTail, innerVertices.fHead, EdgeType::kInner, c, innerWinding);
    for (Vertex* v = outerVertices.fHead; v && v->fNext; v = v->fNext) {
        this->makeConnectingEdge(v, v->fNext, EdgeType::kOuter, c, outerWinding);
    }
    this->makeConnectingEdge(outerVertices.fTail, outerVertices.fHead, EdgeType::kOuter, c, outerWinding);
...
void GrAATriangulator::extractBoundaries(const VertexList& inMesh, VertexList* innerVertices, const Comparator& c)
{
...
int GrAATriangulator::polysToAATriangles(Poly* polys, GrEagerVertexAllocator* vertexAllocator) const
{
