...
template <typename T> class GrQuadBuffer {
public:
    GrQuadBuffer()
        : fCount(0)
...
template <typename T> float* GrQuadBuffer<T>::packQuad(const GrQuad& quad, float* coords)
{
...
template <typename T> const float* GrQuadBuffer<T>::unpackQuad(GrQuad::Type type, const float* coords, GrQuad* quad) const
{
    SkASSERT(quad->xs() + 4 == quad->ys() && quad->xs() + 8 == quad->ws());
...
template <typename T> void GrQuadBuffer<T>::append(const GrQuad& deviceQuad, T&& metadata, const GrQuad* localQuad)
{
...
template <typename T> void GrQuadBuffer<T>::concat(const GrQuadBuffer<T>& that)
{
...
#ifdef SK_DEBUG
template <typename T> void GrQuadBuffer<T>::validate(const char* entry, int expectedCount) const
{
...
template <typename T> bool GrQuadBuffer<T>::Iter::next()
{
    SkASSERT(fNextEntry);
...
template <typename T> bool GrQuadBuffer<T>::MetadataIter::next()
{
    if (fCurrentEntry) {
