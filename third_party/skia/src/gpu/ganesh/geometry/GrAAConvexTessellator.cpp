...
int GrAAConvexTessellator::addPt(const SkPoint& pt, SkScalar depth, SkScalar coverage, bool movable, CurveState curve)
{
...
void GrAAConvexTessellator::updatePt(int index, const SkPoint& pt, SkScalar depth, SkScalar coverage)
{
    this->validate();
...
void GrAAConvexTessellator::computeBisectors()
{
    ...
    for (int cur = 0; cur < fBisectors.size(); prev = cur, ++cur) {
        ...
        if (!fBisectors[cur].normalize()) {
            fBisectors[cur] = SkPointPriv::MakeOrthog(fNorms[cur], (SkPointPriv::Side)-fSide) + SkPointPriv::MakeOrthog(fNorms[prev], fSide);
...
bool GrAAConvexTessellator::createInsetRings(
    Ring& previousRing, SkScalar initialDepth, SkScalar initialCoverage, SkScalar targetDepth, SkScalar targetCoverage, Ring** finalRing)
{
    ...
    for (i = 0; i < kMaxNumRings; ++i) {
        ...
        bool done = this->createInsetRing(*currentRing, nextRing, initialDepth, initialCoverage, targetDepth, targetCoverage, i == 0);
...
bool GrAAConvexTessellator::tessellate(const SkMatrix& m, const SkPath& path)
{
    ...
    if (SkStrokeRec::kStrokeAndFill_Style == fStyle) {
        ...
        this->createOuterRing(fInitialRing, effectiveStrokeWidth / 2 + kAntialiasingRadius, 0.0, &outerStrokeAndAARing);
        ...
        Ring* insetAARing;
        this->createInsetRings(outerStrokeAndAARing, 0.0f, 0.0f, 2 * kAntialiasingRadius, 1.0f, &insetAARing);
    ...
    if (SkStrokeRec::kStroke_Style == fStyle) {
        ...
        this->createOuterRing(fInitialRing, effectiveStrokeWidth / 2 - kAntialiasingRadius, coverage, &outerStrokeRing);
    ...
    if (SkStrokeRec::kStroke_Style == fStyle && fInitialRing.numPts() > 2) {
        ...
        if (this->createInsetRings(fInitialRing, 0.0f, coverage, strokeDepth, coverage, &insetStrokeRing)) {
            Ring* insetAARing;
...
bool GrAAConvexTessellator::computePtAlongBisector(int startIdx, const SkVector& bisector, int edgeIdx, SkScalar desiredDepth, SkPoint* result) const
{
...
bool GrAAConvexTessellator::extractFromPath(const SkMatrix& m, const SkPath& path)
{
    ...
    while (!noRemovalsToDo && this->numPts() >= 3) {
        if (points_are_colinear_and_b_is_middle(fPts[fPts.size() - 2], fPts.back(), fPts[0], &fAccumLinearError)) {
            this->popLastPt();
        } else if (points_are_colinear_and_b_is_middle(fPts.back(), fPts[0], fPts[1], &fAccumLinearError)) {
...
void GrAAConvexTessellator::createOuterRing(const Ring& previousRing, SkScalar outset, SkScalar coverage, Ring* nextRing)
{
...
bool GrAAConvexTessellator::createInsetRing(
    const Ring& lastRing, Ring* nextRing, SkScalar initialDepth, SkScalar initialCoverage, SkScalar targetDepth, SkScalar targetCoverage, bool forceNew)
{
    bool done = false;
    ...
    for (int cur = 0; cur < lastRing.numPts(); ++cur) {
        ...
        SkScalar t;
        bool result = intersect(this->point(lastRing.index(cur)), lastRing.bisector(cur), this->point(lastRing.index(next)), lastRing.bisector(next), &t);
    ...
    if (!this->computePtAlongBisector(lastRing.index(0), lastRing.bisector(0), lastRing.origEdgeID(0), depth, &newPt)) {
        this->terminate(lastRing);
        return true;
    }
    dst[0] = fCandidateVerts.addNewPt(newPt, lastRing.index(0), lastRing.origEdgeID(0), !this->movable(lastRing.index(0)));

    // Handle the middle points (who only compare with the prior point)
    for (int cur = 1; cur < lastRing.numPts() - 1; ++cur) {
        if (!this->computePtAlongBisector(lastRing.index(cur), lastRing.bisector(cur), lastRing.origEdgeID(cur), depth, &newPt)) {
            this->terminate(lastRing);
            return true;
        }
        if (!duplicate_pt(newPt, fCandidateVerts.lastPoint())) {
            dst[cur] = fCandidateVerts.addNewPt(newPt, lastRing.index(cur), lastRing.origEdgeID(cur), !this->movable(lastRing.index(cur)));
        } else {
    ...
    for (int i = 0; i < fCandidateVerts.numPts(); ++i) {
        ...
        if (fCandidateVerts.needsToBeNew(i) || forceNew) {
            ...
            SkScalar coverage = compute_coverage(depth, initialDepth, initialCoverage, targetDepth, targetCoverage);
            newIdx = this->addPt(fCandidateVerts.point(i), depth, coverage, fCandidateVerts.originatingIdx(i) != -1, kSharp_CurveState);
        } else {
            SkASSERT(fCandidateVerts.originatingIdx(i) != -1);
            this->updatePt(fCandidateVerts.originatingIdx(i), fCandidateVerts.point(i), depth, targetCoverage);
...
void GrAAConvexTessellator::Ring::init(const SkTDArray<SkVector>& norms, const SkTDArray<SkVector>& bisectors)
{
...
void GrAAConvexTessellator::Ring::computeBisectors(const GrAAConvexTessellator& tess)
{
    ...
    for (int cur = 0; cur < fPts.size(); prev = cur, ++cur) {
        ...
        if (!fPts[cur].fBisector.normalize()) {
            fPts[cur].fBisector
                = SkPointPriv::MakeOrthog(fPts[cur].fNorm, (SkPointPriv::Side)-tess.side()) + SkPointPriv::MakeOrthog(fPts[prev].fNorm, tess.side());
...
void GrAAConvexTessellator::lineTo(const SkPoint& p, CurveState curve)
{
    ...
    if (this->numPts() >= 2 && points_are_colinear_and_b_is_middle(fPts[fPts.size() - 2], fPts.back(), p, &fAccumLinearError)) {
...
void GrAAConvexTessellator::quadTo(const SkPoint pts[3])
{
    ...
    int count = GrPathUtils::generateQuadraticPoints(pts[0], pts[1], pts[2], kQuadToleranceSqd, &target, maxCount);
    fPointBuffer.resize(count);
    for (int i = 0; i < count - 1; i++) {
        this->lineTo(fPointBuffer[i], kCurve_CurveState);
    }
    this->lineTo(fPointBuffer[count - 1], count == 1 ? kSharp_CurveState : kIndeterminate_CurveState);
}
...
void GrAAConvexTessellator::cubicTo(const SkMatrix& m, const SkPoint srcPts[4])
{
    ...
    fPointBuffer.resize(maxCount);
    SkPoint* target = fPointBuffer.begin();
    int count = GrPathUtils::generateCubicPoints(pts[0], pts[1], pts[2], pts[3], kCubicToleranceSqd, &target, maxCount);
    fPointBuffer.resize(count);
    for (int i = 0; i < count - 1; i++) {
        this->lineTo(fPointBuffer[i], kCurve_CurveState);
    }
...
static void draw_arrow(SkCanvas* canvas, const SkPoint& p, const SkPoint& n, SkScalar len, SkColor color)
{
    SkPaint paint;
...
void GrAAConvexTessellator::Ring::draw(SkCanvas* canvas, const GrAAConvexTessellator& tess) const
{
    ...
    for (int cur = 0; cur < fPts.count(); ++cur) {
        ...
        if (fPts.count()) {
            draw_arrow(canvas, tess.point(fPts[cur].fIndex), fPts[cur].fBisector, kArrowLength, SK_ColorBLUE);
        }
    }
}
...
void GrAAConvexTessellator::draw(SkCanvas* canvas) const
{
    for (int i = 0; i < fIndices.count(); i += 3) {
        ...
        draw_line(canvas, this->point(this->fIndices[i]), this->point(this->fIndices[i + 1]), SK_ColorBLACK);
        draw_line(canvas, this->point(this->fIndices[i + 1]), this->point(this->fIndices[i + 2]), SK_ColorBLACK);
        draw_line(canvas, this->point(this->fIndices[i + 2]), this->point(this->fIndices[i]), SK_ColorBLACK);
    }
    ...
    for (int i = 0; i < this->numPts(); ++i) {
        draw_point(canvas, this->point(i), 0.5f + (this->depth(i) / (2 * kAntialiasingRadius)), !this->movable(i));
