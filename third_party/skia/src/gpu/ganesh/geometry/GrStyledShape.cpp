...
GrStyledShape& GrStyledShape::operator=(const GrStyledShape& that)
{
    ...
    sk_careful_memcpy(fInheritedKey.get(), that.fInheritedKey.get(), sizeof(uint32_t) * fInheritedKey.count());
...
void GrStyledShape::writeUnstyledKey(uint32_t* key) const
{
    ...
    else
    {
        ...
        SkASSERT((fShape.isRect() || fShape.isRRect()) || (fShape.dir() == GrShape::kDefaultDir && fShape.startIndex() == GrShape::kDefaultStart));
...
void GrStyledShape::setInheritedKey(const GrStyledShape& parent, GrStyle::Apply apply, SkScalar scale)
{
...
GrStyledShape GrStyledShape::MakeArc(const SkArc& arc, const GrStyle& style, DoSimplify doSimplify)
{
    GrStyledShape result;
    result.fShape.setArc(SkArc::Make(arc.fOval.makeSorted(), arc.fStartAngle, arc.fSweepAngle, arc.fType));
    result.fStyle = style;
...
GrStyledShape::GrStyledShape(const GrStyledShape& parent, GrStyle::Apply apply, SkScalar scale)
{
    ...
    if (pe) {
        ...
        if (!parent.fStyle.applyPathEffectToPath(&fShape.path(), &strokeRec, *srcForPathEffect, scale)) {
        ...
        if (GrStyle::Apply::kPathEffectAndStrokeRec == apply && strokeRec.needToApply()) {
            ...
            if (tmpParent->style().applies()) {
                SkAssertResult(tmpParent.get()->style().applyToPath(&fShape.path(), &fillOrHairline, *tmpPath.get(), scale));
        ...
        SkAssertResult(parent.fStyle.applyToPath(&fShape.path(), &fillOrHairline, *srcForParentStyle, scale));
        fStyle.resetToInitStyle(fillOrHairline);
    }
...
public:
    AutoRestoreInverseness(GrShape* shape, const GrStyle& style)
        ...
        : fShape(shape)
        , fInverted(!style.isDashed() && fShape->inverted())
    {
    }
...
void GrStyledShape::simplify()
{
    ...
    if (fShape.isPath()) {
        ...
        if (!fStyle.hasNonDashPathEffect()
            && (fStyle.strokeRec().getStyle() == SkStrokeRec::kStroke_Style || fStyle.strokeRec().getStyle() == SkStrokeRec::kHairline_Style
...
void GrStyledShape::simplifyStroke()
{
    ...
    if (!fStyle.hasPathEffect() && fShape.isRect() && fStyle.strokeRec().getStyle() == SkStrokeRec::kStrokeAndFill_Style) {
        if (fStyle.strokeRec().getJoin() == SkPaint::kBevel_Join
            || (fStyle.strokeRec().getJoin() == SkPaint::kMiter_Join && fStyle.strokeRec().getMiter() < SK_ScalarSqrt2)) {
    ...
    if ((!fShape.isPoint() && !fShape.isLine()) || fStyle.hasNonDashPathEffect() || fStyle.strokeRec().isHairlineStyle()) {
        return;
    }
    ...
    if (fClosed) {
        ...
        if (cap != fStyle.strokeRec().getCap() || SkPaint::kDefault_Join != fStyle.strokeRec().getJoin()) {
            SkStrokeRec rec = fStyle.strokeRec();
