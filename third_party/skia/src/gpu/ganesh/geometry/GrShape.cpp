...
bool GrShape::simplifyArc(unsigned flags)
{
    ...
    if (fArc.fOval.isEmpty() || !fArc.fSweepAngle) {
        if (flags & kSimpleFill_Flag) {
            ...
            SkPoint start = { center.fX + 0.5f * fArc.fOval.width() * SkScalarCos(startRad), center.fY + 0.5f * fArc.fOval.height() * SkScalarSin(startRad) };
        ...
        if ((flags & kSimpleFill_Flag) || ((flags & kIgnoreWinding_Flag) && !fArc.isWedge())) {
...
void GrShape::simplifyRRect(const SkRRect& rrect, SkPathDirection dir, unsigned start, unsigned flags)
{
...
void GrShape::simplifyRect(const SkRect& rect, SkPathDirection dir, unsigned start, unsigned flags)
{
...
void GrShape::simplifyLine(const SkPoint& p1, const SkPoint& p2, unsigned flags)
{
    if (flags & kSimpleFill_Flag) {
        ...
        if (flags & kMakeCanonical_Flag) {
            // Sort the end points
            if (fLine.fP2.fY < fLine.fP1.fY || (fLine.fP2.fY == fLine.fP1.fY && fLine.fP2.fX < fLine.fP1.fX)) {
                using std::swap;
...
bool GrShape::simplify(unsigned flags)
{
    ...
    SkASSERT((fType == Type::kRect || fType == Type::kRRect) || (this->dir() == kDefaultDir && this->startIndex() == kDefaultStart));
