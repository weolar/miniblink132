...
template <class T, T* T::*Prev, T* T::*Next> static void list_insert(T* t, T* prev, T* next, T** head, T** tail)
{
    t->*Prev = prev;
    t->*Next = next;
    if (prev) {
...
static inline skgpu::VertexWriter emit_vertex(Vertex* v, bool emitCoverage, skgpu::VertexWriter data)
{
    data << v->fPoint;
...
static bool edge_line_needs_recursion(const SkPoint& p0, const SkPoint& p1)
{
    ...
    int expDiffX = std::abs((std::abs(p0.fX) < 1.f ? 0 : std::ilogbf(p0.fX)) - (std::abs(p1.fX) < 1.f ? 0 : std::ilogbf(p1.fX)));
    int expDiffY = std::abs((std::abs(p0.fY) < 1.f ? 0 : std::ilogbf(p0.fY)) - (std::abs(p1.fY) < 1.f ? 0 : std::ilogbf(p1.fY)));
...
static bool recursive_edge_intersect(const Line& u, SkPoint u0, SkPoint u1, const Line& v, SkPoint v0, SkPoint v1, SkPoint* p, double* s, double* t)
{
    ...
    if (std::min(u0.fX, u1.fX) > std::max(v0.fX, v1.fX) || std::max(u0.fX, u1.fX) < std::min(v0.fX, v1.fX) || std::min(u0.fY, u1.fY) > std::max(v0.fY, v1.fY)
    ...
    if (denom > 0.0 ? (sNumer < 0.0 || sNumer > denom || tNumer < 0.0 || tNumer > denom) : (sNumer > 0.0 || sNumer < denom || tNumer > 0.0 || tNumer < denom)) {
        return false;
    }
...
bool GrTriangulator::Edge::intersect(const Edge& other, SkPoint* p, uint8_t* alpha) const
{
    ...
    if (fTop == other.fTop || fBottom == other.fBottom || fTop == other.fBottom || fBottom == other.fTop) {
    ...
    const bool intersects = recursive_edge_intersect(fLine, fTop->fPoint, fBottom->fPoint, other.fLine, other.fTop->fPoint, other.fBottom->fPoint, p, &s, &t);
    if (!intersects) {
        return false;
    }
    ...
    if (alpha) {
        if (fType == EdgeType::kInner || other.fType == EdgeType::kInner) {
            ...
            *alpha = std::max((1.0 - s) * fTop->fAlpha + s * fBottom->fAlpha, (1.0 - t) * other.fTop->fAlpha + t * other.fBottom->fAlpha);
        }
    }
    return true;
}
...
void GrTriangulator::MonotonePoly::addEdge(Edge* edge)
{
    if (fSide == Side::kRight) {
        ...
        list_insert<Edge, &Edge::fRightPolyPrev, &Edge::fRightPolyNext>(edge, fLastEdge, nullptr, &fFirstEdge, &fLastEdge);
        edge->fUsedInRightPoly = true;
    } else {
        SkASSERT(!edge->fUsedInLeftPoly);
        list_insert<Edge, &Edge::fLeftPolyPrev, &Edge::fLeftPolyNext>(edge, fLastEdge, nullptr, &fFirstEdge, &fLastEdge);
...
skgpu::VertexWriter GrTriangulator::emitTriangle(Vertex* prev, Vertex* curr, Vertex* next, int winding, skgpu::VertexWriter data) const
{
    ...
    if (fCollectBreadcrumbTriangles && abs(winding) > 1 && fPath.getFillType() == SkPathFillType::kWinding) {
...
{
    TESS_LOG("addEdge (%g -> %g) to poly %d, %s side\n", e->fTop->fID, e->fBottom->fID, fID, side == Side::kLeft ? "left" : "right");
...
void GrTriangulator::generateCubicPoints(
    const SkPoint& p0, const SkPoint& p1, const SkPoint& p2, const SkPoint& p3, SkScalar tolSqd, VertexList* contour, int pointsLeft) const
{
    ...
    const SkPoint q[] = { { SkScalarAve(p0.fX, p1.fX), SkScalarAve(p0.fY, p1.fY) }, { SkScalarAve(p1.fX, p2.fX), SkScalarAve(p1.fY, p2.fY) },
        { SkScalarAve(p2.fX, p3.fX), SkScalarAve(p2.fY, p3.fY) } };
    const SkPoint r[] = { { SkScalarAve(q[0].fX, q[1].fX), SkScalarAve(q[0].fY, q[1].fY) }, { SkScalarAve(q[1].fX, q[2].fX), SkScalarAve(q[1].fY, q[2].fY) } };
    ...
    while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
        ...
        case SkPath::kCubic_Verb: {
            *isLinear = false;
            ...
            this->generateCubicPoints(pts[0], pts[1], pts[2], pts[3], toleranceSqd, contour, pointsLeft);
            break;
        }
...
void GrTriangulator::Edge::insertAbove(Vertex* v, const Comparator& c)
{
    ...
    list_insert<Edge, &Edge::fPrevEdgeAbove, &Edge::fNextEdgeAbove>(this, prev, next, &v->fFirstEdgeAbove, &v->fLastEdgeAbove);
}
...
void GrTriangulator::Edge::insertBelow(Vertex* v, const Comparator& c)
{
    ...
    list_insert<Edge, &Edge::fPrevEdgeBelow, &Edge::fNextEdgeBelow>(this, prev, next, &v->fFirstEdgeBelow, &v->fLastEdgeBelow);
}
...
static void remove_edge_above(Edge* edge)
{
    ...
    list_remove<Edge, &Edge::fPrevEdgeAbove, &Edge::fNextEdgeAbove>(edge, &edge->fBottom->fFirstEdgeAbove, &edge->fBottom->fLastEdgeAbove);
}
...
static void remove_edge_below(Edge* edge)
{
    ...
    list_remove<Edge, &Edge::fPrevEdgeBelow, &Edge::fNextEdgeBelow>(edge, &edge->fTop->fFirstEdgeBelow, &edge->fTop->fLastEdgeBelow);
}
...
bool GrTriangulator::setTop(Edge* edge, Vertex* v, EdgeList* activeEdges, Vertex** current, const Comparator& c) const
{
    ...
    if (fCollectBreadcrumbTriangles) {
        fBreadcrumbList.append(fAlloc, edge->fTop->fPoint, edge->fBottom->fPoint, v->fPoint, edge->fWinding);
    }
    edge->fTop = v;
    edge->recompute();
...
bool GrTriangulator::mergeEdgesAbove(Edge* edge, Edge* other, EdgeList* activeEdges, Vertex** current, const Comparator& c) const
{
    ...
    if (coincident(edge->fTop->fPoint, other->fTop->fPoint)) {
        TESS_LOG("merging coincident above edges (%g, %g) -> (%g, %g)\n", edge->fTop->fPoint.fX, edge->fTop->fPoint.fY, edge->fBottom->fPoint.fX,
...
static bool top_collinear(Edge* left, Edge* right)
{
    ...
    return left->fTop->fPoint == right->fTop->fPoint || !left->isLeftOf(*right->fTop) || !right->isRightOf(*left->fTop);
}
...
static bool bottom_collinear(Edge* left, Edge* right)
{
    ...
    return left->fBottom->fPoint == right->fBottom->fPoint || !left->isLeftOf(*right->fBottom) || !right->isRightOf(*left->fBottom);
}
...
Edge* GrTriangulator::makeConnectingEdge(Vertex* prev, Vertex* next, EdgeType type, const Comparator& c, int windingScale)
{
...
static SkPoint clamp(SkPoint p, SkPoint min, SkPoint max, const Comparator& c)
{
    if (c.fDirection == Comparator::Direction::kHorizontal) {
        ...
        return { SkTPin(p.fX, min.fX, max.fX), min.fY < max.fY ? SkTPin(p.fY, min.fY, max.fY) : SkTPin(p.fY, max.fY, min.fY) };
    } else {
        // And with vertical sorting, we know Y's relation but not necessarily X's.
        return { min.fX < max.fX ? SkTPin(p.fX, min.fX, max.fX) : SkTPin(p.fX, max.fX, min.fX), SkTPin(p.fY, min.fY, max.fY) };
    }
}
...
GrTriangulator::BoolFail GrTriangulator::checkForIntersection(
    Edge* left, Edge* right, EdgeList* activeEdges, Vertex** current, VertexList* mesh, const Comparator& c)
{
    if (!left || !right) {
        return BoolFail::kFalse;
    }
    ...
    for (VertexList* contour = contours; contourCnt > 0; --contourCnt, ++contour) {
        ...
        for (Vertex* v = contour->fHead; v;) {
            ...
            if (coincident(prev->fPoint, v->fPoint)) {
                ...
                contour->remove(v);
            } else if (!fPreserveCollinearVertices && Line(prev->fPoint, nextWrap->fPoint).dist(v->fPoint) == 0.0) {
...
template <CompareFunc sweep_lt> static void sorted_merge(VertexList* front, VertexList* back, VertexList* result)
{
        ...
        do {
            ...
            if (v->fFirstEdgeBelow) {
                for (Edge* edge = v->fFirstEdgeBelow; edge; edge = edge->fNextEdgeBelow) {
                    BoolFail l = this->checkForIntersection(leftEnclosingEdge, edge, &activeEdges, &v, mesh, c);
                    if (l == BoolFail::kFail) {
                        return SimplifyResult::kFailed;
                    }
                    if (l == BoolFail::kFalse) {
                        BoolFail r = this->checkForIntersection(edge, rightEnclosingEdge, &activeEdges, &v, mesh, c);
                        if (r == BoolFail::kFail) {
                            return SimplifyResult::kFailed;
                        }
                ...
                BoolFail bf = this->checkForIntersection(leftEnclosingEdge, rightEnclosingEdge, &activeEdges, &v, mesh, c);
...
std::tuple<Poly*, bool> GrTriangulator::tessellate(const VertexList& vertices, const Comparator&)
{
    ...
    for (Vertex* v = vertices.fHead; v != nullptr; v = v->fNext) {
        ...
        for (Edge* e = v->fFirstEdgeAbove; e; e = e->fNextEdgeAbove) {
            TESS_LOG("%g -> %g, lpoly %d, rpoly %d\n", e->fTop->fID, e->fBottom->fID, e->fLeftPoly ? e->fLeftPoly->fID : -1,
...
std::tuple<Poly*, bool> GrTriangulator::contoursToPolys(VertexList* contours, int contourCnt)
{
    ...
    Comparator c(pathBounds.width() > pathBounds.height() ? Comparator::Direction::kHorizontal : Comparator::Direction::kVertical);
    VertexList mesh;
