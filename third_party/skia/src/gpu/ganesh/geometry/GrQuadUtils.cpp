...
// order.
template <typename T> static AI skvx::Vec<4, T> next_cw(const skvx::Vec<4, T>& v)
{
    return skvx::shuffle<2, 0, 3, 1>(v);
}

template <typename T> static AI skvx::Vec<4, T> next_ccw(const skvx::Vec<4, T>& v)
{
...
static bool is_simple_rect(const GrQuad& quad)
{
    ...
    return ((quad.x(0) + SK_ScalarNearlyZero) < quad.x(2)) && ((quad.y(0) + SK_ScalarNearlyZero) < quad.y(1));
}
...
static bool barycentric_coords(
    float x0, float y0, float x1, float y1, float x2, float y2, const float4& testX, const float4& testY, float4* u, float4* v, float4* w)
{
...
SkRect GrQuad::projectedBounds() const
{
    ...
    if (any(clipW)) {
        ...
        SkRect frontBounds = { min(if_then_else(clipW, float4(SK_ScalarInfinity), x2d)), min(if_then_else(clipW, float4(SK_ScalarInfinity), y2d)),
            max(if_then_else(clipW, float4(SK_ScalarNegativeInfinity), x2d)), max(if_then_else(clipW, float4(SK_ScalarNegativeInfinity), y2d)) };
        ...
        return { min(if_then_else(clipW, x2d, float4(frontBounds.fLeft))), min(if_then_else(clipW, y2d, float4(frontBounds.fTop))),
            max(if_then_else(clipW, x2d, float4(frontBounds.fRight))), max(if_then_else(clipW, y2d, float4(frontBounds.fBottom))) };
    } else {
...
void ResolveAAType(GrAAType requestedAAType, GrQuadAAFlags requestedEdgeFlags, const GrQuad& quad, GrAAType* outAAType, GrQuadAAFlags* outEdgeFlags)
{
    ...
    *outEdgeFlags = requestedEdgeFlags;
    ...
    case GrAAType::kCoverage:
        ...
        } else if (quad.quadType() == GrQuad::Type::kAxisAligned && !quad.aaHasEffectOnRect(requestedEdgeFlags)) {
...
int ClipToW0(DrawQuad* quad, DrawQuad* extraVertices)
{
    ...
    int clipCount = (validW[0] ? 0 : 1) + (validW[1] ? 0 : 1) + (validW[2] ? 0 : 1) + (validW[3] ? 0 : 1);
    ...
    if (clipCount != 1) {
        ...
        v.fX = if_then_else(
            validW, v.fX, if_then_else((!ccwValid) & (!cwValid), next_ccw(clip.fX), if_then_else(ccwValid, clip.fX, /* cwValid */ next_cw(clip.fX))));
        v.fY = if_then_else(
            validW, v.fY, if_then_else((!ccwValid) & (!cwValid), next_ccw(clip.fY), if_then_else(ccwValid, clip.fY, /* cwValid */ next_cw(clip.fY))));
        v.fW = if_then_else(validW, v.fW, clip.fW);

        v.fU = if_then_else(
            validW, v.fU, if_then_else((!ccwValid) & (!cwValid), next_ccw(clip.fU), if_then_else(ccwValid, clip.fU, /* cwValid */ next_cw(clip.fU))));
        v.fV = if_then_else(
            validW, v.fV, if_then_else((!ccwValid) & (!cwValid), next_ccw(clip.fV), if_then_else(ccwValid, clip.fV, /* cwValid */ next_cw(clip.fV))));
        v.fR = if_then_else(
            validW, v.fR, if_then_else((!ccwValid) & (!cwValid), next_ccw(clip.fR), if_then_else(ccwValid, clip.fR, /* cwValid */ next_cw(clip.fR))));
        ...
        v2.fX = if_then_else((!validW) | (!ccwValid), clip.fX, if_then_else(cwValid, next_cw(mid.fX), v.fX));
        v2.fY = if_then_else((!validW) | (!ccwValid), clip.fY, if_then_else(cwValid, next_cw(mid.fY), v.fY));
        v2.fW = if_then_else((!validW) | (!ccwValid), clip.fW, if_then_else(cwValid, next_cw(mid.fW), v.fW));

        v2.fU = if_then_else((!validW) | (!ccwValid), clip.fU, if_then_else(cwValid, next_cw(mid.fU), v.fU));
        v2.fV = if_then_else((!validW) | (!ccwValid), clip.fV, if_then_else(cwValid, next_cw(mid.fV), v.fV));
        v2.fR = if_then_else((!validW) | (!ccwValid), clip.fR, if_then_else(cwValid, next_cw(mid.fR), v.fR));
        ...
        GrQuadAAFlags v1EdgeFlag
            = (!validW[0] ? GrQuadAAFlags::kLeft : (!validW[1] ? GrQuadAAFlags::kBottom : (!validW[2] ? GrQuadAAFlags::kTop : GrQuadAAFlags::kRight)));

        v.asGrQuads(&quad->fDevice, GrQuad::Type::kPerspective, &quad->fLocal, localType);
        quad->fEdgeFlags &= ~v1EdgeFlag;

        v2.asGrQuads(&extraVertices->fDevice, GrQuad::Type::kPerspective, &extraVertices->fLocal, localType);
...
bool CropToRect(const SkRect& cropRect, GrAA cropAA, DrawQuad* quad, bool computeLocal)
{
    ...
    if (quad->fDevice.quadType() == GrQuad::Type::kAxisAligned) {
        ...
        if (computeLocal) {
            if (is_simple_rect(quad->fDevice) && is_simple_rect(quad->fLocal)) {
                clippedEdges = crop_simple_rect(cropRect, quad->fDevice.xs(), quad->fDevice.ys(), quad->fLocal.xs(), quad->fLocal.ys());
            } else {
                clippedEdges = crop_rect(cropRect, quad->fDevice.xs(), quad->fDevice.ys(), quad->fLocal.xs(), quad->fLocal.ys(), quad->fLocal.ws());
            }
        } else {
            if (is_simple_rect(quad->fDevice)) {
                clippedEdges = crop_simple_rect(cropRect, quad->fDevice.xs(), quad->fDevice.ys(), nullptr, nullptr);
            } else {
                clippedEdges = crop_rect(cropRect, quad->fDevice.xs(), quad->fDevice.ys(), nullptr, nullptr, nullptr);
            }
        }
    ...
    float4 u1, v1, w1;
    float4 u2, v2, w2;
    if (!barycentric_coords(devX[0], devY[0], devX[1], devY[1], devX[2], devY[2], clipX, clipY, &u1, &v1, &w1)
        || !barycentric_coords(devX[1], devY[1], devX[3], devY[3], devX[2], devY[2], clipX, clipY, &u2, &v2, &w2)) {
...
void TessellationHelper::EdgeVectors::reset(const skvx::Vec<4, float>& xs, const skvx::Vec<4, float>& ys, const skvx::Vec<4, float>& ws, GrQuad::Type quadType)
{
...
float4 TessellationHelper::EdgeEquations::estimateCoverage(const float4& x2d, const float4& y2d) const
{
...
bool TessellationHelper::EdgeEquations::isSubpixel(const float4& x2d, const float4& y2d) const
{
    ...
    float4 d = min(x2d * skvx::shuffle<1, 2, 1, 2>(fA) + y2d * skvx::shuffle<1, 2, 1, 2>(fB) + skvx::shuffle<1, 2, 1, 2>(fC),
        x2d * skvx::shuffle<3, 3, 0, 0>(fA) + y2d * skvx::shuffle<3, 3, 0, 0>(fB) + skvx::shuffle<3, 3, 0, 0>(fC));
    return all(d < 1.f);
}

int TessellationHelper::EdgeEquations::computeDegenerateQuad(const float4& signedEdgeDistances, float4* x2d, float4* y2d, mask4* aaMask) const
{
    ...
    *aaMask = signedEdgeDistances != 0.f;
    ...
    // p(3) to e0,e2
    float4 dists1 = px * skvx::shuffle<3, 3, 0, 0>(fA) + py * skvx::shuffle<3, 3, 0, 0>(fB) + skvx::shuffle<3, 3, 0, 0>(oc);
    float4 dists2 = px * skvx::shuffle<1, 2, 1, 2>(fA) + py * skvx::shuffle<1, 2, 1, 2>(fB) + skvx::shuffle<1, 2, 1, 2>(oc);
    ...
    if (!any(d1Or2)) {
        ...
        *y2d = py;
        ...
        SkPoint center = { 0.25f * ((*x2d)[0] + (*x2d)[1] + (*x2d)[2] + (*x2d)[3]), 0.25f * ((*y2d)[0] + (*y2d)[1] + (*y2d)[2] + (*y2d)[3]) };
        *x2d = center.fX;
        *y2d = center.fY;
        *aaMask = any(*aaMask);
        return 1;
        ...
        V2f eDenom = skvx::shuffle<0, 1>(fA) * skvx::shuffle<3, 2>(fB) - skvx::shuffle<0, 1>(fB) * skvx::shuffle<3, 2>(fA);
        V2f ex = (skvx::shuffle<0, 1>(fB) * skvx::shuffle<3, 2>(oc) - skvx::shuffle<0, 1>(oc) * skvx::shuffle<3, 2>(fB)) / eDenom;
        V2f ey = (skvx::shuffle<0, 1>(oc) * skvx::shuffle<3, 2>(fA) - skvx::shuffle<0, 1>(fA) * skvx::shuffle<3, 2>(oc)) / eDenom;
...
void TessellationHelper::OutsetRequest::reset(const EdgeVectors& edgeVectors, GrQuad::Type quadType, const skvx::Vec<4, float>& edgeDistances)
{
    ...
    if (quadType <= GrQuad::Type::kRectilinear) {
        ...
        fInsetDegenerate
            = (widthChange > 0.f && edgeVectors.fInvLengths[1] > 1.f / widthChange) || (heightChange > 0.f && edgeVectors.fInvLengths[0] > 1.f / heightChange);
        ...
        if (any(abs(edgeVectors.fCosTheta) >= 0.9f)) {
            ...
            float4 edgeAdjust = edgeDistances * (halfTanTheta + next_ccw(halfTanTheta)) + next_ccw(edgeDistances) * next_ccw(edgeVectors.fInvSinTheta)
...
void TessellationHelper::Vertices::asGrQuads(GrQuad* deviceOut, GrQuad::Type deviceType, GrQuad* localOut, GrQuad::Type localType) const
{
    SkASSERT(deviceOut);
...
void TessellationHelper::Vertices::moveAlong(const EdgeVectors& edgeVectors, const float4& signedEdgeDistances)
{
    ...
    SkASSERT(all(abs(edgeVectors.fCosTheta) < 0.9f) || any(edgeVectors.fCosTheta != edgeVectors.fCosTheta));
...
void TessellationHelper::Vertices::moveTo(const float4& x2d, const float4& y2d, const mask4& mask)
{
    ...
    if (all(mask)) {
        ...
        denom = if_then_else(aMask,
            if_then_else(bMask, c1x * c2y - c2x * c1y, /* A & B   */
                if_then_else(useC1x, c1x, c1y)), /* A & !B  */
            if_then_else(bMask, if_then_else(useC2x, c2x, c2y), /* !A & B  */
                float4(1.f))); /* !A & !B */

        a = if_then_else(aMask,
                if_then_else(bMask, c2x * c3y - c3x * c2y, /* A & B   */
                    if_then_else(useC1x, -c3x, -c3y)), /* A & !B  */
                float4(0.f))
            / denom; /* !A      */
        b = if_then_else(bMask,
                if_then_else(aMask, c3x * c1y - c1x * c3y, /* A & B   */
...
float4 TessellationHelper::inset(const skvx::Vec<4, float>& edgeDistances, GrQuad* deviceInset, GrQuad* localInset)
{
...
void TessellationHelper::outset(const skvx::Vec<4, float>& edgeDistances, GrQuad* deviceOutset, GrQuad* localOutset)
{
    SkASSERT(fVerticesValid);
...
void TessellationHelper::getEdgeEquations(skvx::Vec<4, float>* a, skvx::Vec<4, float>* b, skvx::Vec<4, float>* c)
{
    SkASSERT(a && b && c);
    SkASSERT(fVerticesValid);
...
const TessellationHelper::OutsetRequest& TessellationHelper::getOutsetRequest(const skvx::Vec<4, float>& edgeDistances)
{
...
void TessellationHelper::adjustVertices(const skvx::Vec<4, float>& signedEdgeDistances, Vertices* vertices)
{
    SkASSERT(vertices);
...
int TessellationHelper::adjustDegenerateVertices(const skvx::Vec<4, float>& signedEdgeDistances, Vertices* vertices)
{
    SkASSERT(vertices);
    SkASSERT(vertices->fUVRCount == 0 || vertices->fUVRCount == 2 || vertices->fUVRCount == 3);
    ...
    if (fDeviceType <= GrQuad::Type::kRectilinear) {
        ...
        mask4 aaMask;
        int vertexCount = this->getEdgeEquations().computeDegenerateQuad(signedEdgeDistances, &x2d, &y2d, &aaMask);
