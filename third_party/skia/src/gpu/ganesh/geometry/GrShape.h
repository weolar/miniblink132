...
public:
    ...
    explicit GrShape(const SkPoint& point)
    {
        this->setPoint(point);
    }
    ...
    explicit GrShape(const SkArc& arc)
    {
        this->setArc(arc);
    }
    explicit GrShape(const GrLineSegment& line)
    {
        this->setLine(line);
    }
    ...
    {
        SkASSERT((this->isRect() && start < 4) || (this->isRRect() && start < 8) || (dir == kDefaultDir && start == kDefaultStart));
