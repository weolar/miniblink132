...
static bool aa_affects_rect(GrQuadAAFlags edgeFlags, float ql, float qt, float qr, float qb)
{
    ...
    return ((edgeFlags & GrQuadAAFlags::kLeft) && !SkScalarIsInt(ql)) || ((edgeFlags & GrQuadAAFlags::kRight) && !SkScalarIsInt(qr))
        || ((edgeFlags & GrQuadAAFlags::kTop) && !SkScalarIsInt(qt)) || ((edgeFlags & GrQuadAAFlags::kBottom) && !SkScalarIsInt(qb));
}
...
static GrQuad::Type quad_type_for_points(const SkPoint pts[4], const SkMatrix& matrix)
{
    ...
    if ((pts[0].fX == pts[3].fX && pts[1].fX == pts[2].fX) && (pts[0].fY == pts[1].fY && pts[2].fY == pts[3].fY)) {
