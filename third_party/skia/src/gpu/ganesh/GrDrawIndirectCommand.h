...
static_assert(sizeof(GrDrawIndexedIndirectCommand) == 20, "GrDrawIndexedIndirectCommand must be tightly packed");
...
public:
    ...
    GrDrawIndirectWriter(void* data)
        : fData(static_cast<GrDrawIndirectCommand*>(data))
    {
    }
    ...
    {
        return { fData + drawCount };
    }

    inline void write(uint32_t instanceCount, uint32_t baseInstance, uint32_t vertexCount, int32_t baseVertex)
    {
...
public:
    ...
    inline void writeIndexed(uint32_t indexCount, uint32_t baseIndex, uint32_t instanceCount, uint32_t baseInstance, int32_t baseVertex)
    {
