...
GrGLRenderTarget::GrGLRenderTarget(GrGLGpu* gpu, const SkISize& dimensions, GrGLFormat format, int sampleCount, const IDs& ids, sk_sp<GrGLAttachment> stencil,
    skgpu::Protected isProtected, std::string_view label)
...
GrGLRenderTarget::GrGLRenderTarget(
    GrGLGpu* gpu, const SkISize& dimensions, GrGLFormat format, int sampleCount, const IDs& ids, skgpu::Protected isProtected, std::string_view label)
    : GrSurface(gpu, dimensions, isProtected, label)
...
sk_sp<GrGLRenderTarget> GrGLRenderTarget::MakeWrapped(GrGLGpu* gpu, const SkISize& dimensions, GrGLFormat format, int sampleCount, const IDs& idDesc,
    int stencilBits, skgpu::Protected isProtected, std::string_view label)
{
    ...
    if (stencilBits) {
        ...
        sb = GrGLAttachment::MakeWrappedRenderBuffer(gpu,
            /*renderbufferID=*/0, dimensions, GrAttachment::UsageFlags::kStencilAttachment, sampleCount, sFmt);
    }
    return sk_sp<GrGLRenderTarget>(new GrGLRenderTarget(gpu, dimensions, format, sampleCount, idDesc, std::move(sb), isProtected, label));
}
...
GrBackendRenderTarget GrGLRenderTarget::getBackendRenderTarget() const
{
    ...
    return GrBackendRenderTargets::MakeGL(this->width(), this->height(), this->numSamples(), numStencilBits, fbi);
}
...
{
    return GrSurface::ComputeSize(this->backendFormat(), this->dimensions(), fTotalMemorySamplesPerPixel, skgpu::Mipmapped::kNo);
}
...
bool GrGLRenderTarget::ensureDynamicMSAAAttachment()
{
    ...
    fDynamicMSAAAttachment.reset(static_cast<GrGLAttachment*>(
        resourceProvider
            ->getDiscardableMSAAAttachment(this->dimensions(), this->backendFormat(), internalSampleCount, GrProtected(this->isProtected()), GrMemoryless::kNo)
            .release()));
    if (!fDynamicMSAAAttachment) {
        return false;
    }

    GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_RENDERBUFFER, fDynamicMSAAAttachment->renderbufferID()));
    return true;
}
...
void GrGLRenderTarget::bindInternal(GrGLenum fboTarget, bool useMultisampleFBO)
{
    ...
    if (fSingleSampleFBOID != 0 && fSingleSampleFBOID == fMultisampleFBOID && useMultisampleFBO != fDMSAARenderToTextureFBOIsMultisample) {
        ...
        if (this->getGLGpu()->glCaps().bindTexture0WhenChangingTextureFBOMultisampleCount()) {
            GL_CALL(FramebufferTexture2D(fboTarget, GR_GL_COLOR_ATTACHMENT0, GR_GL_TEXTURE_2D, 0 /*texture*/, 0 /*mipMapLevel*/));
        }
        if (useMultisampleFBO) {
            int sampleCount = this->numSamples() > 1 ? this->numSamples() : this->getGpu()->caps()->internalMultisampleCount(this->backendFormat());
            GL_CALL(FramebufferTexture2DMultisample(fboTarget, GR_GL_COLOR_ATTACHMENT0, glTex->target(), glTex->textureID(), 0 /*mipMapLevel*/, sampleCount));
        } else {
            GL_CALL(FramebufferTexture2D(fboTarget, GR_GL_COLOR_ATTACHMENT0, glTex->target(), glTex->textureID(), 0 /*mipMapLevel*/));
        }
    ...
    if (fNeedsStencilAttachmentBind[useMultisampleFBO]) {
        if (auto stencil = this->getStencilAttachment(useMultisampleFBO)) {
            ...
            GL_CALL(FramebufferRenderbuffer(fboTarget, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER, glStencil->renderbufferID()));
            if (GrGLFormatIsPackedDepthStencil(glStencil->format())) {
                GL_CALL(FramebufferRenderbuffer(fboTarget, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, glStencil->renderbufferID()));
            } else {
...
#ifdef SK_DEBUG
        if (!this->getGLGpu()->glCaps().skipErrorChecks() && !this->getGLGpu()->glCaps().rebindColorAttachmentAfterCheckFramebufferStatus()) {
            GrGLenum status;
...
void GrGLRenderTarget::dumpMemoryStatistics(SkTraceMemoryDump* traceMemoryDump) const
{
    ...
    bool refsWrappedRenderTargetObjects = this->fRTFBOOwnership == GrBackendObjectOwnership::kBorrowed;
    ...
    if (numSamplesNotInTexture >= 1) {
        size_t size = GrSurface::ComputeSize(this->backendFormat(), this->dimensions(), numSamplesNotInTexture, skgpu::Mipmapped::kNo);
        ...
        traceMemoryDump->setMemoryBacking(resourceName.c_str(), "gl_renderbuffer", renderbuffer_id.c_str());
    }
}
