...
static GrGLenum filter_to_gl_min_filter(GrSamplerState::Filter filter, GrSamplerState::MipmapMode mm)
{
    ...
    case GrSamplerState::MipmapMode::kNearest:
        switch (filter) {
        case GrSamplerState::Filter::kNearest:
            return GR_GL_NEAREST_MIPMAP_NEAREST;
        case GrSamplerState::Filter::kLinear:
            return GR_GL_LINEAR_MIPMAP_NEAREST;
        }
        SkUNREACHABLE;
    case GrSamplerState::MipmapMode::kLinear:
        switch (filter) {
        case GrSamplerState::Filter::kNearest:
            return GR_GL_NEAREST_MIPMAP_LINEAR;
        case GrSamplerState::Filter::kLinear:
    ...
    void bindSampler(int unitIdx, GrSamplerState state)
    {
        ...
        if (!sampler) {
            ...
            if (fGpu->glCaps().anisoSupport()) {
                float maxAniso = std::min(static_cast<GrGLfloat>(state.maxAniso()), fGpu->glCaps().maxTextureMaxAnisotropy());
                GR_GL_CALL(fGpu->glInterface(), SamplerParameterf(s, GR_GL_TEXTURE_MAX_ANISOTROPY, maxAniso));
            }
        }
        SkASSERT(sampler && sampler->id());
        if (!fTextureUnitStates[unitIdx].fKnown || fTextureUnitStates[unitIdx].fSamplerIDIfKnown != sampler->id()) {
...
private:
    ...
    public:
        ...
        Sampler(GrGLuint id, const GrGLInterface* interface)
            : fID(id)
    ...
    , fFinishCallbacks(this)
{
    ...
    if (GrGLCaps::TransferBufferType::kChromium == this->glCaps().transferBufferType()) {
        this->hwBufferState(GrGpuBufferType::kXferCpuToGpu)->fGLTarget = GR_GL_PIXEL_UNPACK_TRANSFER_BUFFER_CHROMIUM;
        this->hwBufferState(GrGpuBufferType::kXferGpuToCpu)->fGLTarget = GR_GL_PIXEL_PACK_TRANSFER_BUFFER_CHROMIUM;
    } else {
...
void GrGLGpu::onResetContext(uint32_t resetBits)
{
    ...
    if (resetBits & kPixelStore_GrGLBackendState) {
        if (this->caps()->writePixelsRowBytesSupport() || this->caps()->transferPixelsToRowBytesSupport()) {
...
static bool check_backend_texture(const GrBackendTexture& backendTex, const GrGLCaps& caps, GrGLTexture::Desc* desc, bool skipRectTexSupportCheck = false)
{
    ...
    desc->fIsProtected = skgpu::Protected(info.fProtected == skgpu::Protected::kYes || caps.strictProtectedness());
...
sk_sp<GrTexture> GrGLGpu::onWrapBackendTexture(const GrBackendTexture& backendTex, GrWrapOwnership ownership, GrWrapCacheable cacheable, GrIOType ioType)
{
    ...
    GrMipmapStatus mipmapStatus = backendTex.hasMipmaps() ? GrMipmapStatus::kValid : GrMipmapStatus::kNotAllocated;

    auto texture = GrGLTexture::MakeWrapped(this, mipmapStatus, desc, get_gl_texture_params(backendTex), cacheable, ioType, backendTex.getLabel());
...
static bool check_compressed_backend_texture(
    const GrBackendTexture& backendTex, const GrGLCaps& caps, GrGLTexture::Desc* desc, bool skipRectTexSupportCheck = false)
{
    GrGLTextureInfo info;
    if (!GrBackendTextures::GetGLTextureInfo(backendTex, &info) || !info.fID || !info.fFormat) {
        return false;
...
sk_sp<GrTexture> GrGLGpu::onWrapCompressedBackendTexture(const GrBackendTexture& backendTex, GrWrapOwnership ownership, GrWrapCacheable cacheable)
{
    ...
    return GrGLTexture::MakeWrapped(this, mipmapStatus, desc, get_gl_texture_params(backendTex), cacheable, kRead_GrIOType, backendTex.getLabel());
}

sk_sp<GrTexture> GrGLGpu::onWrapRenderableBackendTexture(
    const GrBackendTexture& backendTex, int sampleCnt, GrWrapOwnership ownership, GrWrapCacheable cacheable)
{
    ...
    GrMipmapStatus mipmapStatus = backendTex.hasMipmaps() ? GrMipmapStatus::kDirty : GrMipmapStatus::kNotAllocated;

    sk_sp<GrGLTextureRenderTarget> texRT(
        GrGLTextureRenderTarget::MakeWrapped(this, sampleCnt, desc, get_gl_texture_params(backendTex), rtIDs, cacheable, mipmapStatus, backendTex.getLabel()));
...
sk_sp<GrRenderTarget> GrGLGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& backendRT)
{
    ...
    return GrGLRenderTarget::MakeWrapped(this, backendRT.dimensions(), format, sampleCount, rtIDs, backendRT.stencilBits(),
...
bool GrGLGpu::onWritePixels(GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType srcColorType, const GrMipLevel texels[],
    int mipLevelCount, bool prepForTexSampling)
{
...
bool GrGLGpu::onTransferFromBufferToBuffer(sk_sp<GrGpuBuffer> src, size_t srcOffset, sk_sp<GrGpuBuffer> dst, size_t dstOffset, size_t size)
{
    ...
    GL_CALL(CopyBufferSubData(GR_GL_PIXEL_UNPACK_BUFFER, GR_GL_PIXEL_PACK_BUFFER, srcOffset, dstOffset, size));
    return true;
}

bool GrGLGpu::onTransferPixelsTo(GrTexture* texture, SkIRect rect, GrColorType textureColorType, GrColorType bufferColorType, sk_sp<GrGpuBuffer> transferBuffer,
    size_t offset, size_t rowBytes)
{
    ...
    this->glCaps().getTexSubImageExternalFormatAndType(textureFormat, textureColorType, bufferColorType, &externalFormat, &externalType);
    if (!externalFormat || !externalType) {
        return false;
    }

    GL_CALL(PixelStorei(GR_GL_UNPACK_ALIGNMENT, 1));
    GL_CALL(TexSubImage2D(glTex->target(), 0, rect.left(), rect.top(), rect.width(), rect.height(), externalFormat, externalType, pixels));
...
bool GrGLGpu::onTransferPixelsFrom(
    GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType dstColorType, sk_sp<GrGpuBuffer> transferBuffer, size_t offset)
{
    auto* glBuffer = static_cast<GrGLBuffer*>(transferBuffer.get());
    SkASSERT(glBuffer->size() >= offset + (rect.width() * rect.height() * GrColorTypeBytesPerPixel(dstColorType)));

    this->bindBuffer(GrGpuBufferType::kXferGpuToCpu, glBuffer);

    auto offsetAsPtr = reinterpret_cast<void*>(offset);
    return this->readOrTransferPixelsFrom(surface, rect, surfaceColorType, dstColorType, offsetAsPtr, rect.width());
}
...
bool GrGLGpu::uploadColorTypeTexData(GrGLFormat textureFormat, GrColorType textureColorType, SkISize texDims, GrGLenum target, SkIRect dstRect,
    GrColorType srcColorType, const GrMipLevel texels[], int mipLevelCount)
{
    ...
    this->glCaps().getTexSubImageExternalFormatAndType(textureFormat, textureColorType, srcColorType, &externalFormat, &externalType);
    if (!externalFormat || !externalType) {
        return false;
    }
    this->uploadTexData(texDims, target, dstRect, externalFormat, externalType, bpp, texels, mipLevelCount);
    return true;
}

bool GrGLGpu::uploadColorToTex(GrGLFormat textureFormat, SkISize texDims, GrGLenum target, std::array<float, 4> color, uint32_t levelMask)
{
    GrColorType colorType;
    GrGLenum externalFormat, externalType;
    this->glCaps().getTexSubImageDefaultFormatTypeAndColorType(textureFormat, &externalFormat, &externalType, &colorType);
    ...
    for (int i = 0; i < numLevels; ++i, levelDims = { std::max(levelDims.width() >> 1, 1), std::max(levelDims.height() >> 1, 1) }) {
        if (levelMask & (1 << i)) {
            if (!pixelStorage) {
    ...
    this->uploadTexData(texDims, target, SkIRect::MakeSize(texDims), externalFormat, externalType, bpp, levels.begin(), levels.size());
    return true;
}

void GrGLGpu::uploadTexData(
    SkISize texDims, GrGLenum target, SkIRect dstRect, GrGLenum externalFormat, GrGLenum externalType, size_t bpp, const GrMipLevel texels[], int mipLevelCount)
{
    ...
    for (int level = 0; level < mipLevelCount; ++level, dims = { std::max(dims.width() >> 1, 1), std::max(dims.height() >> 1, 1) }) {
        ...
        GL_CALL(TexSubImage2D(target, level, dstRect.x(), dstRect.y(), dims.width(), dims.height(), externalFormat, externalType, texels[level].fPixels));
    }
    if (restoreGLRowLength) {
...
bool GrGLGpu::uploadCompressedTexData(SkTextureCompressionType compressionType, GrGLFormat format, SkISize dimensions, skgpu::Mipmapped mipmapped,
    GrGLenum target, const void* data, size_t dataSize)
{
    ...
    if (useTexStorage) {
        ...
        GrGLenum error = GL_ALLOC_CALL(TexStorage2D(target, numMipLevels, internalFormat, dimensions.width(), dimensions.height()));
        ...
        for (int level = 0; level < numMipLevels; ++level) {
            ...
            error = GL_ALLOC_CALL(CompressedTexSubImage2D(target, level,
                0, // left
                0, // top
                dimensions.width(), dimensions.height(), internalFormat, SkToInt(levelDataSize), &((const char*)data)[offset]));
        ...
        for (int level = 0; level < numMipLevels; ++level) {
            ...
            GrGLenum error = GL_ALLOC_CALL(CompressedTexImage2D(target, level, internalFormat, dimensions.width(), dimensions.height(),
                0, // border
...
bool GrGLGpu::renderbufferStorageMSAA(const GrGLContext& ctx, int sampleCount, GrGLenum format, int width, int height)
{
    SkASSERT(GrGLCaps::kNone_MSFBOType != ctx.caps()->msFBOType());
    GrGLenum error;
    switch (ctx.caps()->msFBOType()) {
    case GrGLCaps::kStandard_MSFBOType:
        error = GL_ALLOC_CALL(RenderbufferStorageMultisample(GR_GL_RENDERBUFFER, sampleCount, format, width, height));
        break;
    case GrGLCaps::kES_Apple_MSFBOType:
        error = GL_ALLOC_CALL(RenderbufferStorageMultisampleES2APPLE(GR_GL_RENDERBUFFER, sampleCount, format, width, height));
        break;
    case GrGLCaps::kES_EXT_MsToTexture_MSFBOType:
    case GrGLCaps::kES_IMG_MsToTexture_MSFBOType:
        error = GL_ALLOC_CALL(RenderbufferStorageMultisampleES2EXT(GR_GL_RENDERBUFFER, sampleCount, format, width, height));
        break;
...
bool GrGLGpu::createRenderTargetObjects(const GrGLTexture::Desc& desc, int sampleCount, GrGLRenderTarget::IDs* rtIDs)
{
    rtIDs->fMSColorRenderbufferID = 0;
...
#define IS_TSAN 0
    ...
    if (rtIDs->fMSColorRenderbufferID) {
        ...
        if (!this->renderbufferStorageMSAA(*fGLContext, sampleCount, colorRenderbufferFormat, desc.fSize.width(), desc.fSize.height())) {
            return false;
        }
        this->bindFramebuffer(GR_GL_FRAMEBUFFER, rtIDs->fMultisampleFBOID);
        GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_RENDERBUFFER, rtIDs->fMSColorRenderbufferID));
// See skbug.com/12644
#if !IS_TSAN
        if (!this->glCaps().skipErrorChecks()) {
            GrGLenum status;
            ...
            if (this->glCaps().rebindColorAttachmentAfterCheckFramebufferStatus()) {
                GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_RENDERBUFFER, 0));
                GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_RENDERBUFFER, rtIDs->fMSColorRenderbufferID));
            }
        }
#endif
    ...
    GL_CALL(FramebufferTexture2D(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, desc.fTarget, desc.fID, 0));
// See skbug.com/12644
#if !IS_TSAN
    if (!this->glCaps().skipErrorChecks()) {
        GrGLenum status;
...
sk_sp<GrTexture> GrGLGpu::onCreateTexture(SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable, int renderTargetSampleCnt,
    skgpu::Budgeted budgeted, GrProtected isProtected, int mipLevelCount, uint32_t levelClearMask, std::string_view label)
{
    ...
    GrMipmapStatus mipmapStatus = mipLevelCount > 1 ? GrMipmapStatus::kDirty : GrMipmapStatus::kNotAllocated;
    ...
    texDesc.fIsProtected = skgpu::Protected(isProtected == skgpu::Protected::kYes || this->glCaps().strictProtectedness());

    texDesc.fID = this->createTexture(dimensions, texDesc.fFormat, texDesc.fTarget, renderable, &initialState, mipLevelCount, texDesc.fIsProtected, label);
    if (!texDesc.fID) {
    ...
    if (renderable == GrRenderable::kYes) {
        ...
        tex = sk_make_sp<GrGLTextureRenderTarget>(this, budgeted, renderTargetSampleCnt, texDesc, rtIDDesc, mipmapStatus, label);
        tex->baseLevelWasBoundToFBO();
    } else {
        tex = sk_make_sp<GrGLTexture>(this, budgeted, texDesc, mipmapStatus, label);
    }
    // The non-sampler params are still at their default values.
    tex->parameters()->set(&initialState, GrGLTextureParameters::NonsamplerState(), fResetTimestampForTextureParameters);
    if (levelClearMask) {
        if (this->glCaps().clearTextureSupport()) {
            GrGLenum externalFormat, externalType;
            GrColorType colorType;
            this->glCaps().getTexSubImageDefaultFormatTypeAndColorType(texDesc.fFormat, &externalFormat, &externalType, &colorType);
            for (int i = 0; i < mipLevelCount; ++i) {
                if (levelClearMask & (1U << i)) {
                    GL_CALL(ClearTexImage(tex->textureID(), i, externalFormat, externalType, nullptr));
                }
            }
        } else if (this->glCaps().canFormatBeFBOColorAttachment(GrBackendFormats::AsGLFormat(format)) && !this->glCaps().performColorClearsAsDraws()) {
            ...
            this->uploadColorToTex(texDesc.fFormat, texDesc.fSize, texDesc.fTarget, zeros, levelClearMask);
        }
    }
    return tex;
}

sk_sp<GrTexture> GrGLGpu::onCreateCompressedTexture(SkISize dimensions, const GrBackendFormat& format, skgpu::Budgeted budgeted, skgpu::Mipmapped mipmapped,
    GrProtected isProtected, const void* data, size_t dataSize)
{
    ...
    desc.fIsProtected = skgpu::Protected(isProtected == skgpu::Protected::kYes || this->glCaps().strictProtectedness());
    desc.fID = this->createCompressedTexture2D(desc.fSize, compression, desc.fFormat, mipmapped, desc.fIsProtected, &initialState);
    if (!desc.fID) {
        return nullptr;
    }

    if (data) {
        if (!this->uploadCompressedTexData(compression, desc.fFormat, dimensions, mipmapped, GR_GL_TEXTURE_2D, data, dataSize)) {
    ...
    GrMipmapStatus mipmapStatus = mipmapped == skgpu::Mipmapped::kYes ? GrMipmapStatus::kValid : GrMipmapStatus::kNotAllocated;
    ...
    SkISize dimensions, const GrBackendFormat& format, skgpu::Mipmapped mipmapped, GrProtected isProtected)
{
    ...
    info.fProtected = skgpu::Protected(isProtected == skgpu::Protected::kYes || this->glCaps().strictProtectedness());
    info.fID = this->createCompressedTexture2D(dimensions, compression, glFormat, mipmapped, info.fProtected, &initialState);
    if (!info.fID) {
        return {};
    }
    ...
    return GrBackendTextures::MakeGL(dimensions.width(), dimensions.height(), mipmapped, info, std::move(parameters));
}

bool GrGLGpu::onUpdateCompressedBackendTexture(
    const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback, const void* data, size_t length)
{
    GrGLTextureInfo info;
    ...
    skgpu::Mipmapped mipmapped = backendTexture.hasMipmaps() ? skgpu::Mipmapped::kYes : skgpu::Mipmapped::kNo;
    ...
    if (backendTexture.hasMipmaps() && this->glCaps().mipmapLevelControlSupport()) {
        ...
        int numMipLevels = SkMipmap::ComputeLevelCount(backendTexture.width(), backendTexture.height()) + 1;
    ...
    bool result = this->uploadCompressedTexData(compression, glFormat, backendTexture.dimensions(), mipmapped, GR_GL_TEXTURE_2D, data, length);
...
int GrGLGpu::getCompatibleStencilIndex(GrGLFormat format)
{
    ...
    if (!this->glCaps().hasStencilFormatBeenDeterminedForFormat(format)) {
        ...
        GrGLuint colorID = this->createTexture({ kSize, kSize }, format, GR_GL_TEXTURE_2D, GrRenderable::kYes, nullptr, 1, kNotProtected,
            /*label=*/"Skia");
        if (!colorID) {
            return -1;
        }
        ...
        fHWBoundRenderTargetUniqueID.makeInvalid();
        GL_CALL(FramebufferTexture2D(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_TEXTURE_2D, colorID, 0));
        GrGLuint sbRBID = 0;
        ...
        if (sbRBID) {
            ...
            for (int i = 0; i < stencilFmtCnt && sbRBID; ++i) {
                ...
                GrGLenum error = GL_ALLOC_CALL(RenderbufferStorage(GR_GL_RENDERBUFFER, GrGLFormatToEnum(sFmt), kSize, kSize));
                if (error == GR_GL_NO_ERROR) {
                    GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER, sbRBID));
                    if (GrGLFormatIsPackedDepthStencil(sFmt)) {
                        GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, sbRBID));
                    } else {
                        GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, 0));
                    }
                    GrGLenum status;
                    GL_CALL_RET(status, CheckFramebufferStatus(GR_GL_FRAMEBUFFER));
                    if (status == GR_GL_FRAMEBUFFER_COMPLETE) {
                    ...
                    GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER, 0));
                    if (GrGLFormatIsPackedDepthStencil(sFmt)) {
                        GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, 0));
                    }
                }
...
GrGLuint GrGLGpu::createCompressedTexture2D(SkISize dimensions, SkTextureCompressionType compression, GrGLFormat format, skgpu::Mipmapped mipmapped,
    GrProtected isProtected, GrGLTextureParameters::SamplerOverriddenState* initialState)
{
...
GrGLuint GrGLGpu::createTexture(SkISize dimensions, GrGLFormat format, GrGLenum target, GrRenderable renderable,
    GrGLTextureParameters::SamplerOverriddenState* initialState, int mipLevelCount, GrProtected isProtected, std::string_view label)
{
    SkASSERT(format != GrGLFormat::kUnknown);
    ...
    if (internalFormat) {
        if (this->glCaps().formatSupportsTexStorage(format)) {
            ...
            GrGLenum error = GL_ALLOC_CALL(TexStorage2D(target, levelCount, internalFormat, dimensions.width(), dimensions.height()));
            ...
            if (externalFormat && externalType) {
                ...
                for (int level = 0; level < mipLevelCount && error == GR_GL_NO_ERROR; level++) {
                    ...
                    error = GL_ALLOC_CALL(TexImage2D(target, level, internalFormat, currentWidth, currentHeight, 0, externalFormat, externalType, nullptr));
                }
                success = (error == GR_GL_NO_ERROR);
            }
        }
...
sk_sp<GrAttachment> GrGLGpu::makeMSAAAttachment(
    SkISize dimensions, const GrBackendFormat& format, int numSamples, GrProtected isProtected, GrMemoryless isMemoryless)
{
    SkASSERT(isMemoryless == GrMemoryless::kNo);
    return GrGLAttachment::MakeMSAA(this, dimensions, numSamples, GrBackendFormats::AsGLFormat(format));
}
...
void GrGLGpu::flushScissorRect(const SkIRect& scissor, int rtHeight, GrSurfaceOrigin rtOrigin)
{
    ...
    if (fHWScissorSettings.fRect != nativeScissor) {
        GL_CALL(Scissor(nativeScissor.fX, nativeScissor.fY, nativeScissor.fWidth, nativeScissor.fHeight));
...
void GrGLGpu::flushViewport(const SkIRect& viewport, int rtHeight, GrSurfaceOrigin rtOrigin)
{
    ...
    if (fHWViewport != nativeViewport) {
        GL_CALL(Viewport(nativeViewport.fX, nativeViewport.fY, nativeViewport.fWidth, nativeViewport.fHeight));
        fHWViewport = nativeViewport;
    }
}

void GrGLGpu::flushWindowRectangles(const GrWindowRectsState& windowState, const GrGLRenderTarget* rt, GrSurfaceOrigin origin)
{
#ifndef USE_NSIGHT
    ...
    if (!this->caps()->maxWindowRectangles() || fHWWindowRectsState.knownEqualTo(origin, rt->width(), rt->height(), windowState)) {
        return;
    }
...
bool GrGLGpu::flushGLState(GrRenderTarget* renderTarget, bool useMultisampleFBO, const GrProgramInfo& programInfo)
{
    this->handleDirtyContext();
    ...
    this->flushBlendAndColorWrite(programInfo.pipeline().getXferProcessor().getBlendInfo(), programInfo.pipeline().writeSwizzle());
    ...
    this->flushWindowRectangles(programInfo.pipeline().getWindowRectsState(), glRT, programInfo.origin());
...
GrGLenum GrGLGpu::bindBuffer(GrGpuBufferType type, const GrBuffer* buffer)
{
    ...
    } else if (static_cast<const GrGpuBuffer*>(buffer)->uniqueID() != bufferState->fBoundBufferUniqueID) {
...
void GrGLGpu::clear(const GrScissorState& scissor, std::array<float, 4> color, GrRenderTarget* target, bool useMultisampleFBO, GrSurfaceOrigin origin)
{
...
static bool use_tiled_rendering(const GrGLCaps& glCaps, const GrOpsRenderPass::StencilLoadAndStoreInfo& stencilLoadStore)
{
    ...
    return glCaps.tiledRenderingSupport() && GrLoadOp::kClear == stencilLoadStore.fLoadOp && GrStoreOp::kDiscard == stencilLoadStore.fStoreOp;
}

void GrGLGpu::beginCommandBuffer(GrGLRenderTarget* rt, bool useMultisampleFBO, const SkIRect& bounds, GrSurfaceOrigin origin,
    const GrOpsRenderPass::LoadAndStoreInfo& colorLoadStore, const GrOpsRenderPass::StencilLoadAndStoreInfo& stencilLoadStore)
{
    ...
    if (use_tiled_rendering(this->glCaps(), stencilLoadStore)) {
        ...
        GrGLbitfield preserveMask = (GrLoadOp::kLoad == colorLoadStore.fLoadOp) ? GR_GL_COLOR_BUFFER_BIT0 : GR_GL_NONE;
        SkASSERT(GrLoadOp::kLoad != stencilLoadStore.fLoadOp); // Handled by use_tiled_rendering().
        GL_CALL(StartTiling(nativeBounds.fX, nativeBounds.fY, nativeBounds.fWidth, nativeBounds.fHeight, preserveMask));
    }
...
void GrGLGpu::endCommandBuffer(GrGLRenderTarget* rt, bool useMultisampleFBO, const GrOpsRenderPass::LoadAndStoreInfo& colorLoadStore,
    const GrOpsRenderPass::StencilLoadAndStoreInfo& stencilLoadStore)
{
    ...
    if (rt->uniqueID() != fHWBoundRenderTargetUniqueID || useMultisampleFBO != fHWBoundFramebufferIsMSAA) {
    ...
    if (GrGLCaps::kNone_InvalidateFBType != this->glCaps().invalidateFBType()) {
        ...
        if (GrStoreOp::kDiscard == colorLoadStore.fStoreOp) {
            discardAttachments.push_back(rt->isFBO0(useMultisampleFBO) ? GR_GL_COLOR : GR_GL_COLOR_ATTACHMENT0);
        }
        if (GrStoreOp::kDiscard == stencilLoadStore.fStoreOp) {
            discardAttachments.push_back(rt->isFBO0(useMultisampleFBO) ? GR_GL_STENCIL : GR_GL_STENCIL_ATTACHMENT);
        }

        if (!discardAttachments.empty()) {
            if (GrGLCaps::kInvalidate_InvalidateFBType == this->glCaps().invalidateFBType()) {
                GL_CALL(InvalidateFramebuffer(GR_GL_FRAMEBUFFER, discardAttachments.size(), discardAttachments.begin()));
            } else {
                SkASSERT(GrGLCaps::kDiscard_InvalidateFBType == this->glCaps().invalidateFBType());
                GL_CALL(DiscardFramebuffer(GR_GL_FRAMEBUFFER, discardAttachments.size(), discardAttachments.begin()));
            }
        }
    }

    if (use_tiled_rendering(this->glCaps(), stencilLoadStore)) {
        GrGLbitfield preserveMask = (GrStoreOp::kStore == colorLoadStore.fStoreOp) ? GR_GL_COLOR_BUFFER_BIT0 : GR_GL_NONE;
...
void GrGLGpu::clearStencilClip(const GrScissorState& scissor, bool insideStencilMask, GrRenderTarget* target, bool useMultisampleFBO, GrSurfaceOrigin origin)
{
    SkASSERT(target);
    SkASSERT(!this->caps()->performStencilClearsAsDraws());
    SkASSERT(!scissor.enabled() || !this->caps()->performPartialClearsAsDraws());
...
bool GrGLGpu::readOrTransferPixelsFrom(
    GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType dstColorType, void* offsetOrPtr, int rowWidthInPixels)
{
    SkASSERT(surface);
    ...
    GrGLenum externalFormat = 0;
    GrGLenum externalType = 0;
    this->glCaps().getReadPixelsFormat(format, surfaceColorType, dstColorType, &externalFormat, &externalType);
    if (!externalFormat || !externalType) {
        return false;
    }
    ...
    GL_CALL(ReadPixels(rect.left(), rect.top(), rect.width(), rect.height(), externalFormat, externalType, offsetOrPtr));
...
bool GrGLGpu::onReadPixels(GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType dstColorType, void* buffer, size_t rowBytes)
{
    ...
    return this->readOrTransferPixelsFrom(surface, rect, surfaceColorType, dstColorType, buffer, rowPixelWidth);
}

GrOpsRenderPass* GrGLGpu::onGetOpsRenderPass(GrRenderTarget* rt, bool useMultisampleFBO, GrAttachment*, GrSurfaceOrigin origin, const SkIRect& bounds,
    const GrOpsRenderPass::LoadAndStoreInfo& colorInfo, const GrOpsRenderPass::StencilLoadAndStoreInfo& stencilInfo,
    const TArray<GrSurfaceProxy*, true>& sampledProxies, GrXferBarrierFlags renderPassXferBarriers)
{
...
void GrGLGpu::flushRenderTarget(GrGLRenderTarget* target, bool useMultisampleFBO)
{
    ...
    if (fHWBoundRenderTargetUniqueID != rtID || fHWBoundFramebufferIsMSAA != useMultisampleFBO || target->mustRebind(useMultisampleFBO)) {
        ...
        if (!this->glCaps().skipErrorChecks() && !this->glCaps().rebindColorAttachmentAfterCheckFramebufferStatus()) {
            GrGLenum status;
            GL_CALL_RET(status, CheckFramebufferStatus(GR_GL_FRAMEBUFFER));
            if (status != GR_GL_FRAMEBUFFER_COMPLETE) {
        ...
        this->flushViewport(SkIRect::MakeSize(target->dimensions()), target->height(),
...
GrGLenum GrGLGpu::prepareToDraw(GrPrimitiveType primitiveType)
{
    ...
    if (this->glCaps().requiresCullFaceEnableDisableWhenDrawingLinesAfterNonLines() && GrIsPrimTypeLines(primitiveType)
...
void GrGLGpu::onResolveRenderTarget(GrRenderTarget* target, const SkIRect& resolveRect)
{
    ...
    if (this->glCaps().framebufferResolvesMustBeFullSize()) {
        this->resolveRenderFBOs(glRT, SkIRect::MakeSize(glRT->dimensions()), ResolveDirection::kMSAAToSingle);
    } else {
        this->resolveRenderFBOs(glRT, resolveRect, ResolveDirection::kMSAAToSingle);
    }
}

void GrGLGpu::resolveRenderFBOs(GrGLRenderTarget* rt, const SkIRect& resolveRect, ResolveDirection resolveDirection, bool invalidateReadBufferAfterBlit)
{
    this->handleDirtyContext();
    ...
    if (GrGLCaps::kES_Apple_MSFBOType == caps.msFBOType()) {
        ...
        SkASSERT(!caps.framebufferResolvesMustBeFullSize() || resolveRect == SkIRect::MakeSize(rt->dimensions()));
        int l = resolveRect.x();
        int b = resolveRect.y();
    ...
    if (caps.invalidateFBType() != GrGLCaps::kNone_InvalidateFBType && invalidateReadBufferAfterBlit) {
        ...
        GrGLenum colorDiscardAttachment = rt->isFBO0(readBufferIsMSAA) ? GR_GL_COLOR : GR_GL_COLOR_ATTACHMENT0;
...
void GrGLGpu::flushStencil(const GrStencilSettings& stencilSettings, GrSurfaceOrigin origin)
{
    if (stencilSettings.isDisabled()) {
        this->disableStencil();
    } else if (fHWStencilSettings != stencilSettings || (stencilSettings.isTwoSided() && fHWStencilOrigin != origin)) {
...
void GrGLGpu::flushBlendAndColorWrite(const skgpu::BlendInfo& blendInfo, const skgpu::Swizzle& swizzle)
{
    ...
    bool blendOff = skgpu::BlendShouldDisable(equation, srcCoeff, dstCoeff) || !blendInfo.fWritesColor;
    ...
    if (blendOff) {
        if (kNo_TriState != fHWBlendState.fEnabled) {
            ...
            if (this->ctxInfo().vendor() == GrGLVendor::kARM && skgpu::BlendEquationIsAdvanced(fHWBlendState.fEquation)) {
            ...
            if (this->glCaps().mustResetBlendFuncBetweenDualSourceAndDisable()
                && (skgpu::BlendCoeffRefsSrc2(fHWBlendState.fSrcCoeff) || skgpu::BlendCoeffRefsSrc2(fHWBlendState.fDstCoeff)
                    || fHWBlendState.fSrcCoeff == skgpu::BlendCoeff::kIllegal || fHWBlendState.fDstCoeff == skgpu::BlendCoeff::kIllegal)) {
...
void GrGLGpu::bindTexture(int unitIdx, GrSamplerState samplerState, const skgpu::Swizzle& swizzle, GrGLTexture* texture)
{
    SkASSERT(texture);
    ...
    if (samplerState.mipmapped() == skgpu::Mipmapped::kYes) {
        if (!this->caps()->mipmapSupport() || texture->mipmapped() == skgpu::Mipmapped::kNo) {
            ...
            samplerState = GrSamplerState(samplerState.wrapModeX(), samplerState.wrapModeY(), samplerState.filter(), GrSamplerState::MipmapMode::kNone);
        } else {
    ...
    if (this->glCaps().useSamplerObjects()) {
        ...
        if (this->glCaps().mustSetAnyTexParameterToEnableMipmapping()) {
            if (samplerState.mipmapped() == skgpu::Mipmapped::kYes) {
                GrGLenum minFilter = filter_to_gl_min_filter(samplerState.filter(), samplerState.mipmapMode());
                const GrGLTextureParameters::SamplerOverriddenState& oldSamplerState = texture->parameters()->samplerOverriddenState();
                this->setTextureUnit(unitIdx);
        ...
        newSamplerState.fMinFilter = filter_to_gl_min_filter(samplerState.filter(), samplerState.mipmapMode());
        ...
        newSamplerState.fMaxAniso = std::min(static_cast<GrGLfloat>(samplerState.maxAniso()), this->glCaps().maxTextureMaxAnisotropy());
    ...
    const GrGLTextureParameters::NonsamplerState& oldNonsamplerState = texture->parameters()->nonsamplerState();
    if (this->glCaps().textureSwizzleSupport()) {
        if (setAll || !oldNonsamplerState.fSwizzleIsRGBA) {
            ...
            if (GR_IS_GR_GL(this->glStandard())) {
                ...
                GL_CALL(TexParameteriv(target, GR_GL_TEXTURE_SWIZZLE_RGBA, reinterpret_cast<const GrGLint*>(kRGBA)));
    ...
    if (this->glCaps().mipmapLevelControlSupport()
        && (texture->textureType() != GrTextureType::kExternal || !this->glCaps().dontSetBaseOrMaxLevelForExternalTextures())) {
    ...
    texture->parameters()->set(samplerStateToRecord, newNonsamplerState, fResetTimestampForTextureParameters);
}

void GrGLGpu::onResetTextureBindings()
{
    static constexpr GrGLenum kTargets[] = { GR_GL_TEXTURE_2D, GR_GL_TEXTURE_RECTANGLE, GR_GL_TEXTURE_EXTERNAL };
...
void GrGLGpu::flushClearColor(std::array<float, 4> color)
{
    ...
    if (this->glCaps().clearToBoundaryValuesIsBroken() && (1 == r || 0 == r) && (1 == g || 0 == g) && (1 == b || 0 == b) && (1 == a || 0 == a)) {
    ...
    if (r != fHWClearColor[0] || g != fHWClearColor[1] || b != fHWClearColor[2] || a != fHWClearColor[3]) {
...
static inline bool can_blit_framebuffer_for_copy_surface(
    const GrSurface* dst, const GrSurface* src, const SkIRect& srcRect, const SkIRect& dstRect, const GrGLCaps& caps)
{
    int dstSampleCnt = 0;
    int srcSampleCnt = 0;
    ...
    return caps.canCopyAsBlit(dstFormat, dstSampleCnt, dstTexTypePtr, srcFormat, srcSampleCnt, srcTexTypePtr, src->getBoundsRect(), true, srcRect, dstRect);
}
...
static inline bool can_copy_texsubimage(const GrSurface* dst, const GrSurface* src, const GrGLCaps& caps)
{
    ...
    return caps.canCopyTexSubImage(dstFormat, dstHasMSAARenderBuffer, dstTexTypePtr, srcFormat, srcHasMSAARenderBuffer, srcTexTypePtr);
}

void GrGLGpu::bindSurfaceFBOForPixelOps(GrSurface* surface, int mipLevel, GrGLenum fboTarget, TempFBOTarget tempFBOTarget)
{
    ...
    if (!rt || mipLevel > 0) {
        ...
        GR_GL_CALL(this->glInterface(), FramebufferTexture2D(fboTarget, GR_GL_COLOR_ATTACHMENT0, target, texID, mipLevel));
        if (mipLevel == 0) {
            texture->baseLevelWasBoundToFBO();
        }
    } else {
...
void GrGLGpu::deleteFramebuffer(GrGLuint fboid)
{
    ...
    this->handleDirtyContext();
    if (fboid == fBoundDrawFramebuffer && this->caps()->workarounds().unbind_attachments_on_bound_render_fbo_delete) {
...
bool GrGLGpu::onCopySurface(GrSurface* dst, const SkIRect& dstRect, GrSurface* src, const SkIRect& srcRect, GrSamplerState::Filter filter)
{
    ...
    if (preferCopy && this->glCaps().canCopyAsDraw(dstFormat, SkToBool(src->asTexture()), scalingCopy)) {
...
bool GrGLGpu::createCopyProgram(GrTexture* srcTex)
{
    ...
    if (!fCopyProgramArrayBuffer) {
        ...
        fCopyProgramArrayBuffer = GrGLBuffer::Make(this, sizeof(vdata), GrGpuBufferType::kVertex, kStatic_GrAccessPattern);
        if (fCopyProgramArrayBuffer) {
    ...
    GrShaderVar uTexCoordXform("u_texCoordXform", SkSLType::kHalf4, GrShaderVar::TypeModifier::Uniform);
    ...
    skgpu::SkSLToGLSL(shaderCaps, vertexSkSL, SkSL::ProgramKind::kVertex, settings, &glsl[kVertex_GrShaderType], &interface, errorHandler);
    GrGLuint vshader = GrGLCompileAndAttachShader(*fGLContext, fCopyPrograms[progIdx].fProgram, GR_GL_VERTEX_SHADER, glsl[kVertex_GrShaderType],
        /*shaderWasCached=*/false, fProgramCache->stats(), errorHandler);
    ...
    skgpu::SkSLToGLSL(shaderCaps, fragmentSkSL, SkSL::ProgramKind::kFragment, settings, &glsl[kFragment_GrShaderType], &interface, errorHandler);
    GrGLuint fshader = GrGLCompileAndAttachShader(*fGLContext, fCopyPrograms[progIdx].fProgram, GR_GL_FRAGMENT_SHADER, glsl[kFragment_GrShaderType],
        /*shaderWasCached=*/false, fProgramCache->stats(), errorHandler);
    SkASSERT(interface == SkSL::Program::Interface());
    if (!fshader) {
    ...
    if (!GrGLCheckLinkStatus(this, fCopyPrograms[progIdx].fProgram,
            /*shaderWasCached=*/false, errorHandler, sksl, glsl)) {
        // Failed to link, delete everything
        cleanup_program(this, &fCopyPrograms[progIdx].fProgram, &vshader, &fshader);
        return false;
    }

    GL_CALL_RET(fCopyPrograms[progIdx].fTextureUniform, GetUniformLocation(fCopyPrograms[progIdx].fProgram, "u_texture"));
    GL_CALL_RET(fCopyPrograms[progIdx].fPosXformUniform, GetUniformLocation(fCopyPrograms[progIdx].fProgram, "u_posXform"));
    GL_CALL_RET(fCopyPrograms[progIdx].fTexCoordXformUniform, GetUniformLocation(fCopyPrograms[progIdx].fProgram, "u_texCoordXform"));
...
bool GrGLGpu::createMipmapProgram(int progIdx)
{
    ...
    if (oddWidth && oddHeight) {
        vshaderTxt.append("v_texCoord0 = a_vertex.xy * u_texCoordXform.yw;"
        ...
        vshaderTxt.append("v_texCoord0 = a_vertex.xy * half2(u_texCoordXform.y, 1);"
                          "v_texCoord1 = a_vertex.xy * half2(u_texCoordXform.y, 1) + half2(u_texCoordXform.x, 0);");
    } else if (oddHeight) {
        vshaderTxt.append("v_texCoord0 = a_vertex.xy * half2(1, u_texCoordXform.w);"
    ...
    skgpu::SkSLToGLSL(shaderCaps, vertexSkSL, SkSL::ProgramKind::kVertex, settings, &glsl[kVertex_GrShaderType], &interface, errorHandler);
    GrGLuint vshader = GrGLCompileAndAttachShader(*fGLContext, fMipmapPrograms[progIdx].fProgram, GR_GL_VERTEX_SHADER, glsl[kVertex_GrShaderType],
        /*shaderWasCached=*/false, fProgramCache->stats(), errorHandler);
    SkASSERT(interface == SkSL::Program::Interface());
    if (!vshader) {
    ...
    skgpu::SkSLToGLSL(shaderCaps, fragmentSkSL, SkSL::ProgramKind::kFragment, settings, &glsl[kFragment_GrShaderType], &interface, errorHandler);
    GrGLuint fshader = GrGLCompileAndAttachShader(*fGLContext, fMipmapPrograms[progIdx].fProgram, GR_GL_FRAGMENT_SHADER, glsl[kFragment_GrShaderType],
        /*shaderWasCached=*/false, fProgramCache->stats(), errorHandler);
    SkASSERT(interface == SkSL::Program::Interface());
    if (!fshader) {
    ...
    if (!GrGLCheckLinkStatus(this, fMipmapPrograms[progIdx].fProgram,
            /*shaderWasCached=*/false, errorHandler, sksl, glsl)) {
    ...
    GL_CALL_RET(fMipmapPrograms[progIdx].fTextureUniform, GetUniformLocation(fMipmapPrograms[progIdx].fProgram, "u_texture"));
    GL_CALL_RET(fMipmapPrograms[progIdx].fTexCoordXformUniform, GetUniformLocation(fMipmapPrograms[progIdx].fProgram, "u_texCoordXform"));
...
bool GrGLGpu::copySurfaceAsDraw(
    GrSurface* dst, bool drawToMultisampleFBO, GrSurface* src, const SkIRect& srcRect, const SkIRect& dstRect, GrSamplerState::Filter filter)
{
    ...
    attribs->set(this, 0, fCopyProgramArrayBuffer.get(), kFloat2_GrVertexAttribType, SkSLType::kFloat2, 2 * sizeof(GrGLfloat), 0);
...
void GrGLGpu::copySurfaceAsCopyTexSubImage(GrSurface* dst, GrSurface* src, const SkIRect& srcRect, const SkIPoint& dstPoint)
{
    ...
    GL_CALL(CopyTexSubImage2D(dstTex->target(), 0, dstPoint.fX, dstPoint.fY, srcRect.fLeft, srcRect.fTop, srcRect.width(), srcRect.height()));
...
bool GrGLGpu::copySurfaceAsBlitFramebuffer(GrSurface* dst, GrSurface* src, const SkIRect& srcRect, const SkIRect& dstRect, GrSamplerState::Filter filter)
{
    ...
    GL_CALL(BlitFramebuffer(srcRect.fLeft, srcRect.fTop, srcRect.fRight, srcRect.fBottom, dstRect.fLeft, dstRect.fTop, dstRect.fRight, dstRect.fBottom,
        GR_GL_COLOR_BUFFER_BIT, filter_to_gl_mag_filter(filter)));
...
bool GrGLGpu::onRegenerateMipMapLevels(GrTexture* texture)
{
    ...
    if (!fMipmapProgramArrayBuffer) {
        static const GrGLfloat vdata[] = { 0, 0, 0, 1, 1, 0, 1, 1 };
        fMipmapProgramArrayBuffer = GrGLBuffer::Make(this, sizeof(vdata), GrGpuBufferType::kVertex, kStatic_GrAccessPattern);
    ...
    GrGLAttribArrayState* attribs = fHWVertexArrayState.bindInternalVertexArray(this);
    attribs->enableVertexArrays(this, 1);
    attribs->set(this, 0, fMipmapProgramArrayBuffer.get(), kFloat2_GrVertexAttribType, SkSLType::kFloat2, 2 * sizeof(GrGLfloat), 0);
    ...
    for (GrGLint level = 1; level < levelCount; ++level) {
        ...
        if (this->glCaps().regenerateMipmapType() == RegenerateMipmapType::kBasePlusSync && level > 1) {
        ...
        GL_CALL(Uniform4f(fMipmapPrograms[progIdx].fTexCoordXformUniform, invWidth, (width - 1) * invWidth, invHeight, (height - 1) * invHeight));
        ...
        GL_CALL(FramebufferTexture2D(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_TEXTURE_2D, glTex->textureID(), level));
        ...
        if (this->glCaps().regenerateMipmapType() == RegenerateMipmapType::kBasePlusSync && level < levelCount - 1) {
...
GrBackendTexture GrGLGpu::onCreateBackendTexture(
    SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable, skgpu::Mipmapped mipmapped, GrProtected isProtected, std::string_view label)
{
    this->handleDirtyContext();
    ...
    info.fProtected = skgpu::Protected(isProtected == skgpu::Protected::kYes || this->glCaps().strictProtectedness());
    info.fFormat = GrGLFormatToEnum(glFormat);
    info.fID = this->createTexture(dimensions, glFormat, info.fTarget, renderable, &initialState, numMipLevels, info.fProtected, label);
    if (!info.fID) {
        return {};
    }
    ...
    return GrBackendTextures::MakeGL(dimensions.width(), dimensions.height(), mipmapped, info, std::move(parameters), label);
}
...
bool GrGLGpu::onClearBackendTexture(const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback, std::array<float, 4> color)
{
    ...
    bool result = this->uploadColorToTex(glFormat, backendTexture.dimensions(), info.fTarget, color, levelMask);
...
bool GrGLGpu::compile(const GrProgramDesc& desc, const GrProgramInfo& programInfo)
{
    ...
    sk_sp<GrGLProgram> tmp = fProgramCache->findOrCreateProgram(this->getContext(), desc, programInfo, &stat);
    if (!tmp) {
        return false;
    }
...
GrBackendRenderTarget GrGLGpu::createTestingOnlyBackendRenderTarget(SkISize dimensions, GrColorType colorType, int sampleCnt, GrProtected isProtected)
{
    if (dimensions.width() > this->caps()->maxRenderTargetSize() || dimensions.height() > this->caps()->maxRenderTargetSize()) {
        return {};
    }
    if (isProtected == GrProtected::kYes && !this->glCaps().supportsProtectedContent()) {
        return {};
    ...
    if (sampleCnt > 1 && !this->glCaps().usesMSAARenderBuffers()) {
        useTexture = true;
    } else if (format == GrGLFormat::kBGRA8 && this->glCaps().getRenderbufferInternalFormat(GrGLFormat::kBGRA8) != GR_GL_BGRA8) {
    ...
    if (useTexture) {
        GrGLTextureParameters::SamplerOverriddenState initialState;
        colorID = this->createTexture(dimensions, format, GR_GL_TEXTURE_2D, GrRenderable::kYes, &initialState, 1, info.fProtected,
            /*label=*/"Skia");
        if (!colorID) {
            deleteIDs();
            return {};
        ...
        if (sampleCnt == 1) {
            ...
            GL_CALL(FramebufferTexture2DMultisample(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_TEXTURE_2D, colorID, 0, sampleCnt));
        }
    } else {
        ...
        if (sampleCnt == 1) {
            GL_CALL(RenderbufferStorage(GR_GL_RENDERBUFFER, renderBufferFormat, dimensions.width(), dimensions.height()));
        } else {
            if (!this->renderbufferStorageMSAA(this->glContext(), sampleCnt, renderBufferFormat, dimensions.width(), dimensions.height())) {
                deleteIDs();
                return {};
            }
        }
        GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_RENDERBUFFER, colorID));
    }
    if (!avoidStencil) {
        GL_CALL(BindRenderbuffer(GR_GL_RENDERBUFFER, stencilID));
        auto stencilBufferFormat = this->glCaps().stencilFormats()[sFormatIdx];
        if (sampleCnt == 1) {
            GL_CALL(RenderbufferStorage(GR_GL_RENDERBUFFER, GrGLFormatToEnum(stencilBufferFormat), dimensions.width(), dimensions.height()));
        } else {
            if (!this->renderbufferStorageMSAA(this->glContext(), sampleCnt, GrGLFormatToEnum(stencilBufferFormat), dimensions.width(), dimensions.height())) {
                deleteIDs();
                return {};
            }
        }
        GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER, stencilID));
        if (GrGLFormatIsPackedDepthStencil(this->glCaps().stencilFormats()[sFormatIdx])) {
            GL_CALL(FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, stencilID));
        }
    }
    ...
    GrBackendRenderTarget beRT = GrBackendRenderTargets::MakeGL(dimensions.width(), dimensions.height(), sampleCnt, stencilBits, info);
...
GrGLAttribArrayState* GrGLGpu::HWVertexArrayState::bindInternalVertexArray(GrGLGpu* gpu, const GrBuffer* ibuf)
{
...
{
    if (info.fSync == GrSyncCpu::kYes || (!fFinishCallbacks.empty() && !this->glCaps().fenceSyncSupport())) {
