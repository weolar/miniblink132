...
#define ASSERT_ARRAY_UPLOAD_IN_BOUNDS(UNI, COUNT) SkASSERT((COUNT) <= (UNI).fArrayCount || (1 == (COUNT) && GrShaderVar::kNonArray == (UNI).fArrayCount))
    ...
    : fGpu(gpu)
{
    ...
    for (const GLUniformInfo& builderUniform : uniforms.items()) {
        ...
        SkASSERT(GrShaderVar::kNonArray == builderUniform.fVariable.getArrayCount() || builderUniform.fVariable.getArrayCount() > 0);
        SkDEBUGCODE(uniform.fArrayCount = builderUniform.fVariable.getArrayCount(); uniform.fType = builderUniform.fVariable.getType();) uniform.fLocation
            = builderUniform.fLocation;
    }
}

void GrGLProgramDataManager::setSamplerUniforms(const UniformInfoArray& samplers, int startUnit) const
{
    int i = 0;
...
void GrGLProgramDataManager::set4i(UniformHandle u, int32_t i0, int32_t i1, int32_t i2, int32_t i3) const
{
    const Uniform& uni = fUniforms[u.toIndex()];
...
template <int N> inline void GrGLProgramDataManager::setMatrices(UniformHandle u, int arrayCount, const float matrices[]) const
{
    const Uniform& uni = fUniforms[u.toIndex()];
