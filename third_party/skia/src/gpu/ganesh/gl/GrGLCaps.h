...
public:
    ...
    {
        return this->getRenderTargetSampleCount(requestedCount, GrBackendFormats::AsGLFormat(format));
    }
    ...
    {
        return fMaxTextureMaxAnisotropy;
    }
    ...
    */
    ...
    bool framebufferResolvesMustBeFullSize() const
    {
        ...
        return fMSFBOType == kES_Apple_MSFBOType || (fBlitFramebufferFlags & kResolveMustBeFull_BlitFrambufferFlag);
    }
    ...
    {
        SkASSERT(fMSFBOType != kNone_MSFBOType);
        return fMSFBOType != kES_Apple_MSFBOType && !(fBlitFramebufferFlags & GrGLCaps::kNoMSAADst_BlitFramebufferFlag);
    }
    ...
    bool strictProtectedness() const
    {
        return fStrictProtectedness;
    }
    ...
    {
        return fBaseVertexBaseInstanceSupport;
    }
    ...
    {
        return fMipmapLevelControlSupport;
    }

    /// Can set the MIN/MAX LOD value.
    bool mipmapLodControlSupport() const
    {
        return fMipmapLodControlSupport;
    }
    ...
    bool canCopyTexSubImage(GrGLFormat dstFormat, bool dstHasMSAARenderBuffer, const GrTextureType* dstTypeIfTexture, GrGLFormat srcFormat,
        bool srcHasMSAARenderBuffer, const GrTextureType* srcTypeIfTexture) const;
    bool canCopyAsBlit(GrGLFormat dstFormat, int dstSampleCnt, const GrTextureType* dstTypeIfTexture, GrGLFormat srcFormat, int srcSampleCnt,
    ...
    {
        return fUseSamplerObjects;
    }

    bool textureSwizzleSupport() const
    {
        return fTextureSwizzleSupport;
    }

    bool tiledRenderingSupport() const
    {
        return fTiledRenderingSupport;
    }
    ...
    {
        return fClientCanDisableMultisample;
    }
...
private:
    ...
    void getExternalFormat(GrGLFormat surfaceFormat, GrColorType surfaceColorType, GrColorType memoryColorType, ExternalFormatUsage usage,
    ...
    void applyDriverCorrectnessWorkarounds(const GrGLContextInfo&, const GrContextOptions&, const GrGLInterface*, GrShaderCaps*, FormatWorkarounds*);
    ...
    struct ColorTypeInfo {
        ...
        GrGLenum externalFormat(GrColorType externalColorType, ExternalFormatUsage usage, bool haveQueriedImplementationReadFormat) const
        {
            for (int i = 0; i < fExternalIOFormatCount; ++i) {
                if (fExternalIOFormats[i].fColorType == externalColorType) {
                    if (usage == kTexImage_ExternalFormatUsage) {
                        ...
                        if (!haveQueriedImplementationReadFormat && fExternalIOFormats[i].fRequiresImplementationReadQuery) {
                            return 0;
                        }
        ...
        GrGLenum externalFormat(GrColorType surfaceColorType, GrColorType externalColorType, ExternalFormatUsage usage) const
        {
            for (int i = 0; i < fColorTypeInfoCount; ++i) {
                if (fColorTypeInfos[i].fColorType == surfaceColorType) {
                    return fColorTypeInfos[i].externalFormat(externalColorType, usage, fHaveQueriedImplementationReadSupport);
                }
            }
            return 0;
        }
