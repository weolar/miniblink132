...
sk_sp<GrGLProgram> GrGLGpu::ProgramCache::findOrCreateProgram(GrDirectContext* dContext, const GrProgramInfo& programInfo)
{
...
sk_sp<GrGLProgram> GrGLGpu::ProgramCache::findOrCreateProgram(
    GrDirectContext* dContext, const GrProgramDesc& desc, const GrProgramInfo& programInfo, Stats::ProgramCacheResult* stat)
{
    sk_sp<GrGLProgram> tmp = this->findOrCreateProgramImpl(dContext, desc, programInfo, stat);
    if (!tmp) {
    ...
    GrDirectContext* dContext, const GrProgramDesc& desc, const GrProgramInfo& programInfo, Stats::ProgramCacheResult* stat)
{
    *stat = Stats::ProgramCacheResult::kHit;
    ...
    if (entry && !(*entry)->fProgram) {
        ...
        (*entry)->fProgram = GrGLProgramBuilder::CreateProgram(dContext, desc, programInfo, precompiledProgram);
        if (!(*entry)->fProgram) {
...
bool GrGLGpu::ProgramCache::precompileShader(GrDirectContext* dContext, const SkData& key, const SkData& data)
{
    GrProgramDesc desc;
