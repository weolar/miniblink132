...
static int find_string(const TArray<SkString>& strings, const char ext[])
{
    ...
    int idx = SkTSearch<SkString, extension_compare>(&strings.front(), strings.size(), extensionStr, sizeof(SkString));
    return idx;
}
...
bool GrGLExtensions::init(GrGLStandard standard, GrGLFunction<GrGLGetStringFn> getString, GrGLFunction<GrGLGetStringiFn> getStringi,
    GrGLFunction<GrGLGetIntegervFn> getIntegerv, GrGLFunction<GrEGLQueryStringFn> queryString, GrEGLDisplay eglDisplay)
{
    fInitialized = false;
    fStrings.clear();
