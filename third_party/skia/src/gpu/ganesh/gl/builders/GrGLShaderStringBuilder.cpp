    ...
    GrThreadSafePipelineBuilder::Stats* stats, GrContextOptions::ShaderErrorHandler* errorHandler)
{
...

    {
        ...
        if (!compiled) {
            ...
            errorHandler->compileError(glsl.c_str(), infoLen > 0 ? (const char*)log.get() : "", shaderWasCached);
    ...
    const std::string* sksl[kGrShaderTypeCount], const std::string glsl[kGrShaderTypeCount])
{
    ...
    if (!linked && errorHandler) {
        ...
        const char* errorMsg = (infoLen > 0) ? (const char*)log.get() : "link failed but did not provide an info log";
