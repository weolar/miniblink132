...
GrGLTexture::GrGLTexture(GrGLGpu* gpu, skgpu::Budgeted budgeted, const Desc& desc, GrMipmapStatus mipmapStatus, std::string_view label)
    : GrSurface(gpu, desc.fSize, desc.fIsProtected, label)
    , GrTexture(gpu, desc.fSize, desc.fIsProtected, TextureTypeFromTarget(desc.fTarget), mipmapStatus, label)
...
GrGLTexture::GrGLTexture(GrGLGpu* gpu, const Desc& desc, GrMipmapStatus mipmapStatus, sk_sp<GrGLTextureParameters> parameters, GrWrapCacheable cacheable,
...
GrGLTexture::GrGLTexture(GrGLGpu* gpu, const Desc& desc, sk_sp<GrGLTextureParameters> parameters, GrMipmapStatus mipmapStatus, std::string_view label)
    : GrSurface(gpu, desc.fSize, desc.fIsProtected, label)
    , GrTexture(gpu, desc.fSize, desc.fIsProtected, TextureTypeFromTarget(desc.fTarget), mipmapStatus, label)
{
...
{
    return target_from_texture_type(this->textureType());
}
...
GrBackendTexture GrGLTexture::getBackendTexture() const
{
    ...
    return GrBackendTextures::MakeGL(this->width(), this->height(), this->mipmapped(), info, fParameters);
}

GrBackendFormat GrGLTexture::backendFormat() const
{
    return GrBackendFormats::MakeGL(GrGLFormatToEnum(fFormat), target_from_texture_type(this->textureType()));
}

sk_sp<GrGLTexture> GrGLTexture::MakeWrapped(GrGLGpu* gpu, GrMipmapStatus mipmapStatus, const Desc& desc, sk_sp<GrGLTextureParameters> parameters,
    GrWrapCacheable cacheable, GrIOType ioType, std::string_view label)
{
    return sk_sp<GrGLTexture>(new GrGLTexture(gpu, desc, mipmapStatus, std::move(parameters), cacheable, ioType, label));
}

bool GrGLTexture::onStealBackendTexture(GrBackendTexture* backendTexture, SkImages::BackendTextureReleaseProc* releaseProc)
{
...
void GrGLTexture::dumpMemoryStatistics(SkTraceMemoryDump* traceMemoryDump) const
{
    ...
    size_t size = GrSurface::ComputeSize(this->backendFormat(), this->dimensions(), 1, this->mipmapped());
