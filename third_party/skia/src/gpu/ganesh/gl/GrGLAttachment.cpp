...
sk_sp<GrGLAttachment> GrGLAttachment::MakeStencil(GrGLGpu* gpu, SkISize dimensions, int sampleCnt, GrGLFormat format)
{
    GrGLuint rbID = 0;
    ...
    if (sampleCnt > 1) {
        ...
        GrGLenum error = GL_ALLOC_CALL(RenderbufferStorage(GR_GL_RENDERBUFFER, glFmt, dimensions.width(), dimensions.height()));
    ...
    return sk_sp<GrGLAttachment>(new GrGLAttachment(gpu, rbID, dimensions, GrAttachment::UsageFlags::kStencilAttachment, sampleCnt, format,
        /*label=*/"GLAttachmentMakeStencil"));
}

sk_sp<GrGLAttachment> GrGLAttachment::MakeMSAA(GrGLGpu* gpu, SkISize dimensions, int sampleCnt, GrGLFormat format)
{
    GrGLuint rbID = 0;
    ...
    return sk_sp<GrGLAttachment>(new GrGLAttachment(gpu, rbID, dimensions, GrAttachment::UsageFlags::kColorAttachment, sampleCnt, format,
...
void GrGLAttachment::setMemoryBacking(SkTraceMemoryDump* traceMemoryDump, const SkString& dumpName) const
{
...
void GrGLAttachment::onSetLabel()
{
    ...
    if (!this->getLabel().empty()) {
        ...
        if (glGpu->glCaps().debugSupport()) {
            GR_GL_CALL(glGpu->glInterface(), ObjectLabel(GR_GL_TEXTURE, fRenderbufferID, -1, label.c_str()));
        }
    }
}
