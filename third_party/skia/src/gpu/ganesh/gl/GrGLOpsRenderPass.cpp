...
void GrGLOpsRenderPass::set(GrRenderTarget* rt, bool useMSAASurface, const SkIRect& contentBounds, GrSurfaceOrigin origin, const LoadAndStoreInfo& colorInfo,
...
GrNativeRect GrGLOpsRenderPass::dmsaaLoadStoreBounds() const
{
    if (fGpu->glCaps().framebufferResolvesMustBeFullSize()) {
        ...
        return GrNativeRect::MakeRelativeTo(fOrigin, fRenderTarget->height(), SkIRect::MakeSize(fRenderTarget->dimensions()));
    } else {
...
void GrGLOpsRenderPass::onBegin()
{
    ...
    if (fUseMultisampleFBO && fColorLoadAndStoreInfo.fLoadOp == GrLoadOp::kLoad && glRT->hasDynamicMSAAAttachment()) {
        ...
        if (fGpu->glCaps().canResolveSingleToMSAA()) {
            fGpu->resolveRenderFBOs(glRT, this->dmsaaLoadStoreBounds().asSkIRect(), GrGLGpu::ResolveDirection::kSingleToMSAA);
        } else {
            fGpu->drawSingleIntoMSAAFBO(glRT, this->dmsaaLoadStoreBounds().asSkIRect());
        }
    }

    fGpu->beginCommandBuffer(glRT, fUseMultisampleFBO, fContentBounds, fOrigin, fColorLoadAndStoreInfo, fStencilLoadAndStoreInfo);
}
...
void GrGLOpsRenderPass::onEnd()
{
    ...
    if (fUseMultisampleFBO && fColorLoadAndStoreInfo.fStoreOp == GrStoreOp::kStore && glRT->hasDynamicMSAAAttachment()) {
        // Blit the msaa attachment into the single sample fbo.
        fGpu->resolveRenderFBOs(
            glRT, this->dmsaaLoadStoreBounds().asSkIRect(), GrGLGpu::ResolveDirection::kMSAAToSingle, true /*invalidateReadBufferAfterBlit*/);
    }
}
...
bool GrGLOpsRenderPass::onBindTextures(const GrGeometryProcessor& geomProc, const GrSurfaceProxy* const geomProcTextures[], const GrPipeline& pipeline)
{
...
void GrGLOpsRenderPass::onBindBuffers(
    sk_sp<const GrBuffer> indexBuffer, sk_sp<const GrBuffer> instanceBuffer, sk_sp<const GrBuffer> vertexBuffer, GrPrimitiveRestart primitiveRestart)
{
    ...
    if ((indexBuffer && fGpu->glCaps().baseVertexBaseInstanceSupport()) || (!indexBuffer && !fGpu->glCaps().drawArraysBaseVertexIsBroken())) {
    ...
    if (int instanceStride = program->instanceStride()) {
        ...
        for (int i = 0; i < program->numInstanceAttributes(); ++i, ++attribIdx) {
            ...
            fAttribArrayState->set(
                fGpu, attrib.fLocation, instanceBuffer, attrib.fCPUType, attrib.fGPUType, instanceStride, bufferOffset + attrib.fOffset, kDivisor);
        }
    }
}
...
void GrGLOpsRenderPass::bindVertexBuffer(const GrBuffer* vertexBuffer, int baseVertex)
{
    ...
    if (int vertexStride = program->vertexStride()) {
        ...
        for (int i = 0; i < program->numVertexAttributes(); ++i) {
            ...
            fAttribArrayState->set(
                fGpu, attrib.fLocation, vertexBuffer, attrib.fCPUType, attrib.fGPUType, vertexStride, bufferOffset + attrib.fOffset, kDivisor);
        }
    }
}
...
void GrGLOpsRenderPass::onDrawIndexed(int indexCount, int baseIndex, uint16_t minIndexValue, uint16_t maxIndexValue, int baseVertex)
{
    GrGLenum glPrimType = fGpu->prepareToDraw(fPrimitiveType);
    if (fGpu->glCaps().baseVertexBaseInstanceSupport()) {
        ...
        if (baseVertex != 0) {
            GL_CALL(DrawElementsInstancedBaseVertexBaseInstance(
                glPrimType, indexCount, GR_GL_UNSIGNED_SHORT, this->offsetForBaseIndex(baseIndex), 1, baseVertex, 0));
            return;
        }
    } else {
        this->bindVertexBuffer(fActiveVertexBuffer.get(), baseVertex);
    ...
    if (fGpu->glCaps().drawRangeElementsSupport()) {
        GL_CALL(DrawRangeElements(glPrimType, minIndexValue, maxIndexValue, indexCount, GR_GL_UNSIGNED_SHORT, this->offsetForBaseIndex(baseIndex)));
    } else {
...
void GrGLOpsRenderPass::onDrawInstanced(int instanceCount, int baseInstance, int vertexCount, int baseVertex)
{
    ...
    for (int i = 0; i < instanceCount; i += maxInstances) {
        ...
        if (fGpu->glCaps().baseVertexBaseInstanceSupport()) {
            ...
            GL_CALL(DrawArraysInstancedBaseInstance(glPrimType, baseVertex, vertexCount, instanceCountForDraw, baseInstanceForDraw));
        } else {
...
void GrGLOpsRenderPass::onDrawIndexedInstanced(int indexCount, int baseIndex, int instanceCount, int baseInstance, int baseVertex)
{
    int maxInstances = fGpu->glCaps().maxInstancesPerDrawWithoutCrashing(instanceCount);
    for (int i = 0; i < instanceCount; i += maxInstances) {
        GrGLenum glPrimType = fGpu->prepareToDraw(fPrimitiveType);
        ...
        if (fGpu->glCaps().baseVertexBaseInstanceSupport()) {
            ...
            SkASSERT(fDidBindVertexBuffer);
            GL_CALL(DrawElementsInstancedBaseVertexBaseInstance(
                glPrimType, indexCount, GR_GL_UNSIGNED_SHORT, this->offsetForBaseIndex(baseIndex), instanceCountForDraw, baseVertex, baseInstanceForDraw));
        } else {
            this->bindInstanceBuffer(fActiveInstanceBuffer.get(), baseInstanceForDraw);
            this->bindVertexBuffer(fActiveVertexBuffer.get(), baseVertex);
            GL_CALL(DrawElementsInstanced(glPrimType, indexCount, GR_GL_UNSIGNED_SHORT, this->offsetForBaseIndex(baseIndex), instanceCountForDraw));
        }
    }
    fGpu->didDrawTo(fRenderTarget);
}
...
void GrGLOpsRenderPass::onDrawIndirect(const GrBuffer* drawIndirectBuffer, size_t offset, int drawCount)
{
    ...
    if (drawCount > 1 && fGpu->glCaps().multiDrawType() == MultiDrawType::kMultiDrawIndirect) {
        GrGLenum glPrimType = fGpu->prepareToDraw(fPrimitiveType);
        GL_CALL(MultiDrawArraysIndirect(glPrimType, buffer_offset_to_gl_address(drawIndirectBuffer, offset), drawCount, sizeof(GrDrawIndirectCommand)));
        return;
    }
...
void GrGLOpsRenderPass::multiDrawArraysANGLEOrWebGL(const GrBuffer* drawIndirectBuffer, size_t offset, int drawCount)
{
    ...
    while (drawCount) {
        ...
        if (countInBatch == 1) {
            GL_CALL(DrawArraysInstancedBaseInstance(glPrimType, fFirsts[0], fCounts[0], fInstanceCounts[0], fBaseInstances[0]));
        } else {
            GL_CALL(MultiDrawArraysInstancedBaseInstance(glPrimType, fFirsts, fCounts, fInstanceCounts, fBaseInstances, countInBatch));
        }
...
void GrGLOpsRenderPass::onDrawIndexedIndirect(const GrBuffer* drawIndirectBuffer, size_t offset, int drawCount)
{
    ...
    if (drawCount > 1 && fGpu->glCaps().multiDrawType() == MultiDrawType::kMultiDrawIndirect) {
        ...
        GL_CALL(MultiDrawElementsIndirect(
            glPrimType, GR_GL_UNSIGNED_SHORT, buffer_offset_to_gl_address(drawIndirectBuffer, offset), drawCount, sizeof(GrDrawIndexedIndirectCommand)));
        return;
    }

    for (int i = 0; i < drawCount; ++i) {
        GrGLenum glPrimType = fGpu->prepareToDraw(fPrimitiveType);
        GL_CALL(DrawElementsIndirect(glPrimType, GR_GL_UNSIGNED_SHORT, buffer_offset_to_gl_address(drawIndirectBuffer, offset)));
...
void GrGLOpsRenderPass::multiDrawElementsANGLEOrWebGL(const GrBuffer* drawIndirectBuffer, size_t offset, int drawCount)
{
    ...
    while (drawCount) {
        ...
        if (countInBatch == 1) {
            GL_CALL(DrawElementsInstancedBaseVertexBaseInstance(
                glPrimType, fCounts[0], GR_GL_UNSIGNED_SHORT, fIndices[0], fInstanceCounts[0], fBaseVertices[0], fBaseInstances[0]));
        } else {
            GL_CALL(MultiDrawElementsInstancedBaseVertexBaseInstance(
                glPrimType, fCounts, GR_GL_UNSIGNED_SHORT, fIndices, fInstanceCounts, fBaseVertices, fBaseInstances, countInBatch));
        }
        drawCount -= countInBatch;
        cmds += countInBatch;
    }
