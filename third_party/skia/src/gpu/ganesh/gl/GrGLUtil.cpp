...
static GrGLRenderer get_renderer(const char* rendererString, const GrGLExtensions& extensions)
{
    ...
    if (0 == strncmp(rendererString, kTegraStr, std::size(kTegraStr) - 1)) {
        ...
        return extensions.has("GL_NV_path_rendering") ? GrGLRenderer::kTegra : GrGLRenderer::kTegra_PreK1;
    }
    int lastDigit;
    ...
    if (0 == strncmp(rendererString, kAppleA4Str, std::size(kAppleA4Str) - 1) || 0 == strncmp(rendererString, kAppleA5Str, std::size(kAppleA5Str) - 1)
    ...
    if (0 == strncmp(rendererString, kPowerVRRogueStr, std::size(kPowerVRRogueStr) - 1) || 0 == strncmp(rendererString, kAppleA7Str, std::size(kAppleA7Str) - 1)
    ...
    if (const char* intelString = strstr(rendererString, "Intel")) {
        ...
        if (0 == strcmp("Intel Iris OpenGL Engine", intelString) || 0 == strcmp("Intel Iris Pro OpenGL Engine", intelString)) {
        ...
        if (const char* intelGfxString = strstr(intelString, "Graphics")) {
            int intelNumber;
            if (sscanf(intelGfxString, "Graphics %d", &intelNumber) || sscanf(intelGfxString, "Graphics P%d", &intelNumber)) {
                ...
                if (intelNumber == 610 || intelNumber == 630) {
                    return strstr(intelString, "UHD") ? GrGLRenderer::kIntelCoffeeLake : GrGLRenderer::kIntelKabyLake;
                }
    ...
    int mali400Num;
    if (1 == sscanf(rendererString, "Mali-%d", &mali400Num) && mali400Num >= 400 && mali400Num < 500) {
...
static bool is_commamd_buffer(const char* rendererString, const char* versionString)
{
    ...
    return (0 == strcmp(rendererString, kChromium)
        || (3 == sscanf(versionString, "OpenGL ES %d.%d %8s", &major, &minor, suffix) && 0 == strcmp(kChromium, suffix)));
}
...
static std::tuple<GrGLDriver, GrGLDriverVersion> get_driver_and_version(
    GrGLStandard standard, GrGLVendor vendor, const char* vendorString, const char* rendererString, const char* versionString)
{
    SkASSERT(rendererString);
    SkASSERT(versionString);
    ...
    if (!strcmp(vendorString, "freedreno")) {
        ...
        if (vendor == GrGLVendor::kNVIDIA) {
            ...
            int n = sscanf(versionString, "%d.%d.%d NVIDIA %d.%d", &major, &minor, &rev, &driverMajor, &driverMinor);
            ...
            if (n != 4) {
                n = sscanf(versionString, "%d.%d (Core Profile) Mesa %d.%d", &major, &minor, &driverMajor, &driverMinor);
            }
            if (n == 4) {
        ...
        if (vendor == GrGLVendor::kNVIDIA) {
            driver = GrGLDriver::kNVIDIA;
            int n = sscanf(versionString, "OpenGL ES %d.%d NVIDIA %d.%d", &major, &minor, &driverMajor, &driverMinor);
            // Some older NVIDIA drivers don't report the driver version.
            if (n == 4) {
                driverVersion = GR_GL_DRIVER_VER(driverMajor, driverMinor, 0);
            }
            ...
            int n = sscanf(versionString, "OpenGL ES %d.%d Mesa %d.%d", &major, &minor, &driverMajor, &driverMinor);
            if (n == 4) {
                driver = GrGLDriver::kMesa;
                driverVersion = GR_GL_DRIVER_VER(driverMajor, driverMinor, 0);
            }
    ...
    if (driver == GrGLDriver::kUnknown) {
        if (vendor == GrGLVendor::kIntel) {
            ...
            // different
            //  OSes.
            int n = sscanf(versionString, "%d.%d INTEL-%d.%d.%d", &major, &minor, &driverMajor, &driverMinor, &driverPoint);
            if (n == 5) {
            ...
            int n = sscanf(versionString, "OpenGL ES %d.%d V@%d.%d", &major, &minor, &driverMajor, &driverMinor);
            if (n == 4) {
                driverVersion = GR_GL_DRIVER_VER(driverMajor, driverMinor, 0);
            }
        } else if (vendor == GrGLVendor::kImagination) {
            ...
            int ignored0;
            int ignored1;
            int n = sscanf(versionString, "OpenGL ES %d.%d v%d.r%dp%d-%drel", &major, &minor, &driverMajor, &driverMinor, &ignored0, &ignored1);
            if (n == 6) {
            ...
            int n = sscanf(versionString, "%d.%d Metal - %d", &major, &minor, &driverMajor);
            if (n == 3) {
...
static std::tuple<GrGLANGLEBackend, SkString> get_angle_backend(const char* rendererString)
{
    ...
    if (!strncmp(rendererString, kHeader, kHeaderLength) && rendererString[rendererLength - 1] == ')') {
...
static std::tuple<GrGLVendor, GrGLRenderer, GrGLDriver, GrGLDriverVersion> get_angle_gl_vendor_and_renderer(
    const char* innerString, const GrGLExtensions& extensions)
{
    TArray<SkString> parts;
    SkStrSplit(innerString, ",", &parts);
    // This would need some fixing if we have substrings that contain commas.
    if (parts.size() != 3) {
        return { GrGLVendor::kOther, GrGLRenderer::kOther, GrGLDriver::kUnknown, GR_GL_DRIVER_UNKNOWN_VER };
    }
    ...
    auto [angleDriver, angleDriverVersion]
        = get_driver_and_version(kGLES_GrGLStandard, angleVendor, angleVendorString, angleRendererString, angleVersionString);
...
static std::tuple<GrGLVendor, GrGLRenderer, GrGLDriver, GrGLDriverVersion> get_angle_d3d_vendor_and_renderer(const char* innerString)
{
    ...
    if (strstr(innerString, "Intel")) {
        ...
        if ((modelStr = strstr(innerString, "HD Graphics"))
            && (1 == sscanf(modelStr, "HD Graphics %i", &modelNumber) || 1 == sscanf(modelStr, "HD Graphics P%i", &modelNumber))) {
            switch (modelNumber) {
            case 2000:
            case 3000:
            ...
            && (1 == sscanf(modelStr, "Iris(TM) Graphics %i", &modelNumber) || 1 == sscanf(modelStr, "Iris(TM) Pro Graphics %i", &modelNumber)
...
static std::tuple<GrGLVendor, GrGLRenderer> get_webgl_vendor_and_renderer(const GrGLInterface* interface)
{
    if (!interface->fExtensions.has("WEBGL_debug_renderer_info")) {
        return { GrGLVendor::kOther, GrGLRenderer::kOther };
    }
...
GrGLDriverInfo GrGLGetDriverInfo(const GrGLInterface* interface)
{
    ...
    std::tie(info.fDriver, info.fDriverVersion) = get_driver_and_version(interface->fStandard, info.fVendor, vendor, renderer, version);

    SkString innerAngleRendererString;
    std::tie(info.fANGLEBackend, innerAngleRendererString) = get_angle_backend(renderer);

    if (info.fANGLEBackend == GrGLANGLEBackend::kD3D9 || info.fANGLEBackend == GrGLANGLEBackend::kD3D11) {
    ...
    if (info.fRenderer == GrGLRenderer::kWebGL) {
        std::tie(info.fWebGLVendor, info.fWebGLRenderer) = get_webgl_vendor_and_renderer(interface);
    }
