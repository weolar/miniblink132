...
std::unique_ptr<GrFragmentProcessor> make_inverse_rrect_fp(const SkMatrix& viewMatrix, const SkRRect& rrect, GrAA aa, const GrShaderCaps& shaderCaps)
{
    SkTCopyOnFirstWrite<SkRRect> devRRect(rrect);
    if (viewMatrix.isIdentity() || rrect.transform(viewMatrix, devRRect.writable())) {
        auto edgeType = (aa == GrAA::kYes) ? GrClipEdgeType::kInverseFillAA : GrClipEdgeType::kInverseFillBW;
...
bool init_vertices_paint(GrRecordingContext* rContext, const GrColorInfo& colorInfo, const SkPaint& skPaint, const SkMatrix& ctm, SkBlender* blender,
    bool hasColors, const SkSurfaceProps& props, GrPaint* grPaint)
{
    if (hasColors) {
        return SkPaintToGrPaintWithBlend(rContext, colorInfo, skPaint, ctm, blender, props, grPaint);
    } else {
        return SkPaintToGrPaint(rContext, colorInfo, skPaint, ctm, props, grPaint);
    }
}

bool init_mesh_child_effects(GrRecordingContext* rContext, const GrColorInfo& colorInfo, const SkSurfaceProps& surfaceProps, const SkMesh& mesh,
...
sk_sp<Device> Device::Make(GrRecordingContext* rContext, GrColorType colorType, sk_sp<GrSurfaceProxy> proxy, sk_sp<SkColorSpace> colorSpace,
    GrSurfaceOrigin origin, const SkSurfaceProps& surfaceProps, InitContents init)
{
    auto sdc = SurfaceDrawContext::Make(rContext, colorType, std::move(proxy), std::move(colorSpace), origin, surfaceProps);
...
SkImageInfo Device::MakeInfo(SurfaceContext* sc, DeviceFlags flags)
{
    ...
    return SkImageInfo::Make(
        sc->width(), sc->height(), colorType, flags & DeviceFlags::kIsOpaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType, sc->colorInfo().refColorSpace());
}

/** Checks that the alpha type is legal and gets constructor flags. Returns false if device creation
    should fail. */
bool Device::CheckAlphaTypeAndGetFlags(SkAlphaType alphaType, InitContents init, DeviceFlags* flags)
{
...
sk_sp<Device> Device::Make(std::unique_ptr<SurfaceDrawContext> sdc, SkAlphaType alphaType, InitContents init)
{
    if (!sdc) {
        return nullptr;
    }
    ...
    DeviceFlags flags;
    if (!rContext->colorTypeSupportedAsSurface(ct) || !CheckAlphaTypeAndGetFlags(alphaType, init, &flags)) {
        return nullptr;
    }
    return sk_sp<Device>(new Device(std::move(sdc), flags));
}

sk_sp<Device> Device::Make(GrRecordingContext* rContext, skgpu::Budgeted budgeted, const SkImageInfo& ii, SkBackingFit fit, int sampleCount,
    skgpu::Mipmapped mipmapped, GrProtected isProtected, GrSurfaceOrigin origin, const SkSurfaceProps& props, InitContents init)
{
    if (!rContext) {
        return nullptr;
    }

    auto sdc = SurfaceDrawContext::Make(rContext, SkColorTypeToGrColorType(ii.colorType()), ii.refColorSpace(), fit, ii.dimensions(), props,
        /*label=*/"MakeDevice", sampleCount, mipmapped, isProtected, origin, budgeted);
...
Device::Device(std::unique_ptr<SurfaceDrawContext> sdc, DeviceFlags flags)
    ...
    , fSubRunControl(sdc->recordingContext()->priv().getSubRunControl(sdc->surfaceProps().isUseDeviceIndependentFonts()))
    , fSurfaceDrawContext(std::move(sdc))
    , fClip(SkIRect::MakeSize(fSurfaceDrawContext->dimensions()), &this->localToDevice(), force_aa_clip(fSurfaceDrawContext.get()))
{
...
void Device::drawPaint(const SkPaint& paint)
{
    ...
    GrPaint grPaint;
    if (!SkPaintToGrPaint(
            this->recordingContext(), fSurfaceDrawContext->colorInfo(), paint, this->localToDevice(), fSurfaceDrawContext->surfaceProps(), &grPaint)) {
        return;
    }
...
void Device::drawPoints(SkCanvas::PointMode mode, size_t count, const SkPoint pts[], const SkPaint& paint)
{
    ASSERT_SINGLE_OWNER
    ...
    if (count == 2 && mode == SkCanvas::kLines_PointMode) {
        if (paint.getPathEffect()) {
            ...
            if (SkPaintToGrPaint(
                ...
                path.moveTo(pts[0]);
                path.lineTo(pts[1]);
                fSurfaceDrawContext->drawPath(this->clip(), std::move(grPaint), aa, this->localToDevice(), path, GrStyle(paint, SkPaint::kStroke_Style));
            }
            return;
        }
        if (!paint.getMaskFilter() && paint.getStrokeWidth() > 0 && // drawStrokedLine doesn't support hairlines.
            ...
            if (SkPaintToGrPaint(
                ...
                fSurfaceDrawContext->drawStrokedLine(
                    this->clip(), std::move(grPaint), aa, this->localToDevice(), pts, SkStrokeRec(paint, SkPaint::kStroke_Style));
            }
            return;
        }
    }
    ...
    bool isHairline
        = ((0 == width)
              || (1 == width && this->localToDevice().getMinMaxScales(scales) && SkScalarNearlyEqual(scales[0], 1.f) && SkScalarNearlyEqual(scales[1], 1.f)))
        &&

        // Don't do this as a hairline draw, which will emit line primitives, if
        // lines are not permitted by caps.
        !((mode == SkCanvas::kLines_PointMode || mode == SkCanvas::kPolygon_PointMode) && caps->avoidLineDraws());

    // we only handle non-coverage-aa hairlines and paints without path effects or mask filters,
    // else we let the SkDraw call our drawPath()
    if (!isHairline || paint.getPathEffect() || paint.getMaskFilter() || fSurfaceDrawContext->chooseAAType(aa) == GrAAType::kCoverage) {
    ...
    sk_sp<SkVertices> vertices = SkVertices::MakeCopy(kIgnoredMode, SkToS32(count), pts, nullptr, nullptr);

    GrPrimitiveType primitiveType = point_mode_to_primitive_type(mode);
    fSurfaceDrawContext->drawVertices(this->clip(), std::move(grPaint), this->localToDevice(), std::move(vertices), &primitiveType);
}
...
void Device::drawRect(const SkRect& rect, const SkPaint& paint)
{
    ...
    if (paint.getMaskFilter() || paint.getPathEffect()) {
        ...
        GrBlurUtils::DrawShapeWithMaskFilter(fContext.get(), fSurfaceDrawContext.get(), this->clip(), paint, this->localToDevice(), shape);
        return;
    }
    ...
    fSurfaceDrawContext->drawRect(this->clip(), std::move(grPaint), fSurfaceDrawContext->chooseAA(paint), this->localToDevice(), rect, &style);
}
...
void Device::drawEdgeAAQuad(const SkRect& rect, const SkPoint clip[4], SkCanvas::QuadAAFlags aaFlags, const SkColor4f& color, SkBlendMode mode)
{
    ...
    if (clip) {
        ...
        fSurfaceDrawContext->fillQuadWithEdgeAA(this->clip(), std::move(grPaint), SkToGrQuadAAFlags(aaFlags), this->localToDevice(), clip, nullptr);
    } else {
        // Use fillRectWithEdgeAA to preserve mathematical properties of dst being rectangular
        fSurfaceDrawContext->fillRectWithEdgeAA(this->clip(), std::move(grPaint), SkToGrQuadAAFlags(aaFlags), this->localToDevice(), rect);
    }
}
...
void Device::drawRRect(const SkRRect& rrect, const SkPaint& paint)
{
    ...
    fSurfaceDrawContext->drawRRect(this->clip(), std::move(grPaint), fSurfaceDrawContext->chooseAA(paint), this->localToDevice(), rrect, style);
}
...
void Device::drawDRRect(const SkRRect& outer, const SkRRect& inner, const SkPaint& paint)
{
    ...
    if (stroke.isFillStyle() && !paint.getMaskFilter() && !paint.getPathEffect()) {
        ...
        if (auto fp = make_inverse_rrect_fp(this->localToDevice(), inner, fSurfaceDrawContext->chooseAA(paint), *fSurfaceDrawContext->caps()->shaderCaps())) {
            GrPaint grPaint;
            if (!SkPaintToGrPaint(
                    this->recordingContext(), fSurfaceDrawContext->colorInfo(), paint, this->localToDevice(), fSurfaceDrawContext->surfaceProps(), &grPaint)) {
                return;
            ...
            fSurfaceDrawContext->drawRRect(this->clip(), std::move(grPaint), fSurfaceDrawContext->chooseAA(paint), this->localToDevice(), outer, GrStyle());
            return;
        }
    }
...
void Device::drawRegion(const SkRegion& region, const SkPaint& paint)
{
    ...
    fSurfaceDrawContext->drawRegion(this->clip(), std::move(grPaint), fSurfaceDrawContext->chooseAA(paint), this->localToDevice(), region, GrStyle(paint));
}
...
void Device::drawOval(const SkRect& oval, const SkPaint& paint)
{
    ...
    fSurfaceDrawContext->drawOval(this->clip(), std::move(grPaint), fSurfaceDrawContext->chooseAA(paint), this->localToDevice(), oval, GrStyle(paint));
}
...
void Device::drawArc(const SkArc& arc, const SkPaint& paint)
{
    ...
    fSurfaceDrawContext->drawArc(this->clip(), std::move(grPaint), fSurfaceDrawContext->chooseAA(paint), this->localToDevice(), arc, GrStyle(paint));
}
...
void Device::drawPath(const SkPath& origSrcPath, const SkPaint& paint, bool pathIsMutable)
{
    ...
    if (!paint.getMaskFilter()) {
        ...
        fSurfaceDrawContext->drawPath(
            this->clip(), std::move(grPaint), fSurfaceDrawContext->chooseAA(paint), this->localToDevice(), origSrcPath, GrStyle(paint));
        return;
    }
...
{
    return skif::MakeGaneshBackend(fContext, fSurfaceDrawContext->origin(), surfaceProps, colorType);
}
...
sk_sp<SkSpecialImage> Device::makeSpecial(const SkBitmap& bitmap)
{
    ...
    auto view = std::get<0>(GrMakeCachedBitmapProxyView(fContext.get(), bitmap, /*label=*/"Device_MakeSpecial"));
    if (!view) {
        return nullptr;
    }
    ...
    // the special image
    return SkSpecialImages::MakeDeferredFromGpu(fContext.get(), rect, bitmap.getGenerationID(), std::move(view),
        { SkColorTypeToGrColorType(bitmap.colorType()), kPremul_SkAlphaType, bitmap.refColorSpace() }, this->surfaceProps());
}
...
sk_sp<SkSpecialImage> Device::makeSpecial(const SkImage* image)
{
    ...
    if (image->isTextureBacked()) {
        ...
        return SkSpecialImages::MakeDeferredFromGpu(fContext.get(), SkIRect::MakeWH(image->width(), image->height()), image->uniqueID(), std::move(view),
            { ct, kPremul_SkAlphaType, image->refColorSpace() }, this->surfaceProps());
...
sk_sp<SkSpecialImage> Device::snapSpecial(const SkIRect& subset, bool forceCopy)
{
    ...
    if (forceCopy || !view.asTextureProxy()) {
        ...
        view = GrSurfaceProxyView::Copy(fContext.get(), std::move(view),
            skgpu::Mipmapped::kNo, // Don't auto generate mips
            subset, SkBackingFit::kApprox, skgpu::Budgeted::kYes,
    ...
    return SkSpecialImages::MakeDeferredFromGpu(
        fContext.get(), finalSubset, kNeedNewImageUniqueID_SpecialImage, std::move(view), GrColorInfo(this->imageInfo().colorInfo()), this->surfaceProps());
}
...
sk_sp<SkSpecialImage> Device::snapSpecialScaled(const SkIRect& subset, const SkISize& dstDims)
{
    ...
    auto scaledContext = sdc->rescale(sdc->imageInfo().makeDimensions(dstDims), sdc->origin(), subset, RescaleGamma::kSrc, RescaleMode::kLinear);
    if (!scaledContext) {
        return nullptr;
    }

    return SkSpecialImages::MakeDeferredFromGpu(fContext.get(), SkIRect::MakeSize(dstDims), kNeedNewImageUniqueID_SpecialImage,
        scaledContext->readSurfaceView(), GrColorInfo(this->imageInfo().colorInfo()), this->surfaceProps());
}

void Device::drawDevice(SkDevice* device, const SkSamplingOptions& sampling, const SkPaint& paint)
{
    ASSERT_SINGLE_OWNER
    ...
    const SkImage* image, const SkRect* src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    GrAA aa = fSurfaceDrawContext->chooseAA(paint);
    SkCanvas::QuadAAFlags aaFlags = (aa == GrAA::kYes) ? SkCanvas::kAll_QuadAAFlags : SkCanvas::kNone_QuadAAFlags;

    this->drawImageQuadDirect(image, src ? *src : SkRect::MakeIWH(image->width(), image->height()), dst,
        /* dstClip= */ nullptr, aaFlags,
    ...
    const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    [[maybe_unused]] auto [wasTiled, numTiles]
        = TiledTextureUtils::DrawAsTiledImageRect(canvas, image, src ? *src : SkRect::MakeIWH(image->width(), image->height()), dst, aaFlags, sampling, &paint,
            constraint, rCtx->priv().options().fSharpenMipmappedTextures, cacheSize, maxTextureSize);
#if defined(GPU_TEST_UTILS)
...
void Device::drawViewLattice(
    GrSurfaceProxyView view, const GrColorInfo& info, std::unique_ptr<SkLatticeIter> iter, const SkRect& dst, SkFilterMode filter, const SkPaint& origPaint)
{
    ...
    if (!SkPaintToGrPaintReplaceShader(this->recordingContext(), fSurfaceDrawContext->colorInfo(), *paint, this->localToDevice(),
            /*shaderFP=*/nullptr, fSurfaceDrawContext->surfaceProps(), &grPaint)) {
        return;
    }
    ...
    fSurfaceDrawContext->drawImageLattice(
        this->clip(), std::move(grPaint), this->localToDevice(), std::move(view), info.alphaType(), std::move(csxf), filter, std::move(iter), dst);
}

void Device::drawImageLattice(const SkImage* image, const SkCanvas::Lattice& lattice, const SkRect& dst, SkFilterMode filter, const SkPaint& paint)
{
    ASSERT_SINGLE_OWNER
    ...
    if (view) {
        ...
        this->drawViewLattice(std::move(view), std::move(colorInfo), std::move(iter), dst, filter, paint);
    }
}
...
void Device::drawVertices(const SkVertices* vertices, sk_sp<SkBlender> blender, const SkPaint& paint, bool skipColorXform)
{
    ...
    GrPaint grPaint;
    if (!init_vertices_paint(fContext.get(), fSurfaceDrawContext->colorInfo(), paint, this->localToDevice(), blender.get(), info.hasColors(),
            fSurfaceDrawContext->surfaceProps(), &grPaint)) {
        return;
    }
    fSurfaceDrawContext->drawVertices(
        this->clip(), std::move(grPaint), this->localToDevice(), sk_ref_sp(const_cast<SkVertices*>(vertices)), nullptr, skipColorXform);
}
...
void Device::drawMesh(const SkMesh& mesh, sk_sp<SkBlender> blender, const SkPaint& paint)
{
    ...
    GrPaint grPaint;
    if (!init_vertices_paint(fContext.get(), fSurfaceDrawContext->colorInfo(), paint, this->localToDevice(), blender.get(),
            SkMeshSpecificationPriv::HasColors(*mesh.spec()), fSurfaceDrawContext->surfaceProps(), &grPaint)) {
        return;
    }

    TArray<std::unique_ptr<GrFragmentProcessor>> meshChildFPs;
    if (!init_mesh_child_effects(fContext.get(), fSurfaceDrawContext->colorInfo(), fSurfaceDrawContext->surfaceProps(), mesh, &meshChildFPs)) {
        return;
    }
    fSurfaceDrawContext->drawMesh(this->clip(), std::move(grPaint), this->localToDevice(), mesh, std::move(meshChildFPs));
}
...
void Device::drawAtlas(const SkRSXform xform[], const SkRect texRect[], const SkColor colors[], int count, sk_sp<SkBlender> blender, const SkPaint& paint)
{
    ASSERT_SINGLE_OWNER
    ...
    GrPaint grPaint;
    if (colors) {
        if (!SkPaintToGrPaintWithBlend(this->recordingContext(), fSurfaceDrawContext->colorInfo(), paint, this->localToDevice(), blender.get(),
                fSurfaceDrawContext->surfaceProps(), &grPaint)) {
            return;
        }
    } else {
    ...
    fSurfaceDrawContext->drawAtlas(this->clip(), std::move(grPaint), this->localToDevice(), count, xform, texRect, colors);
}
...
void Device::onDrawGlyphRunList(SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
    ...
    if (glyphRunList.blob() == nullptr) {
        ...
        fSurfaceDrawContext->drawGlyphRunList(canvas, this->clip(), this->localToDevice(), glyphRunList, this->strikeDeviceInfo(), paint);
    }
}
...
void Device::drawDrawable(SkCanvas* canvas, SkDrawable* drawable, const SkMatrix* matrix)
{
    ...
    if (GrBackendApi::kVulkan == api) {
        ...
        std::unique_ptr<SkDrawable::GpuDrawHandler> gpuDraw = drawable->snapGpuDrawHandler(api, combinedMatrix, this->devClipBounds(), this->imageInfo());
        if (gpuDraw) {
            fSurfaceDrawContext->drawDrawable(std::move(gpuDraw), combinedMatrix.mapRect(drawable->getBounds()));
            return;
        }
    }
...
bool Device::wait(int numSemaphores, const GrBackendSemaphore* waitSemaphores, bool deleteSemaphoresAfterWait)
{
    ASSERT_SINGLE_OWNER

    return fSurfaceDrawContext->waitOnSemaphores(numSemaphores, waitSemaphores, deleteSemaphoresAfterWait);
}
...
bool Device::replaceBackingProxy(SkSurface::ContentChangeMode mode, sk_sp<GrRenderTargetProxy> newRTP, GrColorType grColorType, sk_sp<SkColorSpace> colorSpace,
    GrSurfaceOrigin origin, const SkSurfaceProps& props)
{
    auto sdc = SurfaceDrawContext::Make(fContext.get(), grColorType, std::move(newRTP), std::move(colorSpace), origin, props);
    if (!sdc) {
        return false;
    }
    ...
    if (mode == SkSurface::kRetain_ContentChangeMode) {
        ...
        SkAssertResult(sdc->blitTexture(fSurfaceDrawContext->readSurfaceView(), SkIRect::MakeWH(this->width(), this->height()), SkIPoint::Make(0, 0)));
    }
...
bool Device::replaceBackingProxy(SkSurface::ContentChangeMode mode)
{
    ...
    sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(format, ii.dimensions(), GrRenderable::kYes, oldRTP->numSamples(), oldView.mipmapped(),
        SkBackingFit::kExact, oldRTP->isBudgeted(), oldRTP->isProtected(),
        /*label=*/"BaseDevice_ReplaceBackingProxy");
    if (!proxy) {
        return false;
    }

    return this->replaceBackingProxy(mode, sk_ref_sp(proxy->asRenderTargetProxy()), grColorType, ii.refColorSpace(), oldView.origin(), this->surfaceProps());
}

void Device::asyncRescaleAndReadPixels(
    const SkImageInfo& info, const SkIRect& srcRect, RescaleGamma rescaleGamma, RescaleMode rescaleMode, ReadPixelsCallback callback, ReadPixelsContext context)
{
    auto* sdc = fSurfaceDrawContext.get();
    // Context TODO: Elevate direct context requirement to public API.
    ...
    sdc->asyncRescaleAndReadPixels(dContext, info, srcRect, rescaleGamma, rescaleMode, callback, context);
}

void Device::asyncRescaleAndReadPixelsYUV420(SkYUVColorSpace yuvColorSpace, bool readAlpha, sk_sp<SkColorSpace> dstColorSpace, const SkIRect& srcRect,
    SkISize dstSize, RescaleGamma rescaleGamma, RescaleMode rescaleMode, ReadPixelsCallback callback, ReadPixelsContext context)
{
    auto* sdc = fSurfaceDrawContext.get();
    // Context TODO: Elevate direct context requirement to public API.
    auto dContext = sdc->recordingContext()->asDirectContext();
    ...
    sdc->asyncRescaleAndReadPixelsYUV420(
        dContext, yuvColorSpace, readAlpha, std::move(dstColorSpace), srcRect, dstSize, rescaleGamma, rescaleMode, callback, context);
}
...
sk_sp<SkDevice> Device::createDevice(const CreateInfo& cinfo, const SkPaint*)
{
    ...
    SkSurfaceProps props = this->surfaceProps().cloneWithPixelGeometry(cinfo.fPixelGeometry);

    SkASSERT(cinfo.fInfo.colorType() != kRGBA_1010102_SkColorType);

    auto sdc = SurfaceDrawContext::MakeWithFallback(fContext.get(), SkColorTypeToGrColorType(cinfo.fInfo.colorType()), cinfo.fInfo.refColorSpace(),
        SkBackingFit::kApprox, cinfo.fInfo.dimensions(), props, fSurfaceDrawContext->numSamples(), skgpu::Mipmapped::kNo,
        fSurfaceDrawContext->asSurfaceProxy()->isProtected(), fSurfaceDrawContext->origin(), skgpu::Budgeted::kYes);
    if (!sdc) {
        return nullptr;
    }
...
sk_sp<SkSurface> Device::makeSurface(const SkImageInfo& info, const SkSurfaceProps& props)
{
    ...
    return SkSurfaces::RenderTarget(fContext.get(), kBudgeted, info, fSurfaceDrawContext->numSamples(), fSurfaceDrawContext->origin(), &props,
...
bool Device::android_utils_clipWithStencil()
{
    ...
    static constexpr GrUserStencilSettings kDrawToStencil(
        GrUserStencilSettings::StaticInit<0x1, GrUserStencilTest::kAlways, 0x1, GrUserStencilOp::kReplace, GrUserStencilOp::kReplace, 0x1>());
    // Regions don't actually need AA, but in DMSAA mode everything is antialiased.
    GrAA aa = GrAA(fSurfaceDrawContext->alwaysAntialias());
    sdc->drawRegion(nullptr, std::move(grPaint), aa, SkMatrix::I(), clipRegion, GrStyle::SimpleFill(), &kDrawToStencil);
    return true;
}
...
static bool valid_slug_matrices(const SkMatrix& creationMatrix, const SkMatrix& positionMatrix)
{
    ...
    return creationMatrix.getScaleX() == positionMatrix.getScaleX() && creationMatrix.getScaleY() == positionMatrix.getScaleY()
        && creationMatrix.getSkewX() == positionMatrix.getSkewX() && creationMatrix.getSkewY() == positionMatrix.getSkewY() && !positionMatrix.hasPerspective()
        && !creationMatrix.hasPerspective() && SkScalarIsInt(translation.x()) && SkScalarIsInt(translation.y());
}
#endif
...
void Device::drawSlug(SkCanvas* canvas, const sktext::gpu::Slug* slug, const SkPaint& paint)
{
    ...
    auto atlasDelegate
        = [&](const sktext::gpu::AtlasSubRun* subRun, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt> subRunStorage, sktext::gpu::RendererData) {
              auto [drawingClip, op]
                  = subRun->makeAtlasTextOp(this->clip(), this->localToDevice(), drawOrigin, paint, std::move(subRunStorage), fSurfaceDrawContext.get());
              if (op != nullptr) {
