...
std::unique_ptr<GrD3DDescriptorHeap> GrD3DDescriptorHeap::Make(
    GrD3DGpu* gpu, D3D12_DESCRIPTOR_HEAP_TYPE type, unsigned int numDescriptors, D3D12_DESCRIPTOR_HEAP_FLAGS flags)
{
    ...
    return std::unique_ptr<GrD3DDescriptorHeap>(
        new GrD3DDescriptorHeap(std::move(gr_cp<ID3D12DescriptorHeap>(heap)), gpu->device()->GetDescriptorHandleIncrementSize(type)));
}

GrD3DDescriptorHeap::GrD3DDescriptorHeap(const gr_cp<ID3D12DescriptorHeap>& heap, unsigned int handleIncrementSize)
    : fHeap(heap)
