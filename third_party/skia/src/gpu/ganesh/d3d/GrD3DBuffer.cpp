...
static gr_cp<ID3D12Resource> make_d3d_buffer(
    GrD3DGpu* gpu, size_t size, GrGpuBufferType intendedType, GrAccessPattern accessPattern, D3D12_RESOURCE_STATES* resourceState, sk_sp<GrD3DAlloc>* alloc)
{
    D3D12_HEAP_TYPE heapType;
    if (accessPattern == kStatic_GrAccessPattern) {
        SkASSERT(intendedType != GrGpuBufferType::kXferCpuToGpu && intendedType != GrGpuBufferType::kXferGpuToCpu);
    ...
    gr_cp<ID3D12Resource> resource = gpu->memoryAllocator()->createResource(heapType, &bufferDesc, *resourceState, alloc, nullptr);

    return resource;
}

sk_sp<GrD3DBuffer> GrD3DBuffer::Make(GrD3DGpu* gpu, size_t size, GrGpuBufferType intendedType, GrAccessPattern accessPattern)
{
    SkASSERT(!gpu->protectedContext() || (accessPattern != kStatic_GrAccessPattern));
    D3D12_RESOURCE_STATES resourceState;

    sk_sp<GrD3DAlloc> alloc;
    gr_cp<ID3D12Resource> resource = make_d3d_buffer(gpu, size, intendedType, accessPattern, &resourceState, &alloc);
    if (!resource) {
        return nullptr;
    }

    return sk_sp<GrD3DBuffer>(new GrD3DBuffer(gpu, size, intendedType, accessPattern, std::move(resource), std::move(alloc), resourceState,
        /*label=*/"MakeD3DBuffer"));
}

GrD3DBuffer::GrD3DBuffer(GrD3DGpu* gpu, size_t size, GrGpuBufferType intendedType, GrAccessPattern accessPattern, gr_cp<ID3D12Resource> bufferResource,
    sk_sp<GrD3DAlloc> alloc, D3D12_RESOURCE_STATES resourceState, std::string_view label)
...
void GrD3DBuffer::setResourceState(const GrD3DGpu* gpu, D3D12_RESOURCE_STATES newResourceState)
{
    if (newResourceState == fResourceState ||
        ...
        (fResourceState == D3D12_RESOURCE_STATE_GENERIC_READ && SkToBool(newResourceState | fResourceState))) {
        return;
    }
...
bool GrD3DBuffer::onClearToZero()
{
    ...
    if (this->accessPattern() == kStatic_GrAccessPattern) {
        GrStagingBufferManager::Slice slice = this->getD3DGpu()->stagingBufferManager()->allocateStagingBufferSlice(this->size());
        if (!slice.fBuffer) {
            return false;
        }
        ...
        this->getD3DGpu()->currentCommandList()->copyBufferToBuffer(
            sk_ref_sp<GrD3DBuffer>(this), 0, static_cast<const GrD3DBuffer*>(slice.fBuffer)->d3dResource(), slice.fOffset, this->size());
        return true;
    }
...
void* GrD3DBuffer::internalMap(MapType type, size_t offset, size_t size)
{
    ...
    if (this->accessPattern() == kStatic_GrAccessPattern) {
        ...
        GrStagingBufferManager::Slice slice = this->getD3DGpu()->stagingBufferManager()->allocateStagingBufferSlice(size);
        if (!slice.fBuffer) {
            return nullptr;
        }
...
void GrD3DBuffer::internalUnmap(MapType type, size_t offset, size_t size)
{
    ...
    if (this->accessPattern() == kStatic_GrAccessPattern) {
        ...
        SkASSERT(fStagingBuffer);
        this->setResourceState(this->getD3DGpu(), D3D12_RESOURCE_STATE_COPY_DEST);
        this->getD3DGpu()->currentCommandList()->copyBufferToBuffer(sk_ref_sp<GrD3DBuffer>(this), offset, fStagingBuffer, fStagingOffset, size);
...
{
    SkASSERT(this->intendedType() == GrGpuBufferType::kVertex || this->intendedType() == GrGpuBufferType::kIndex
        || this->intendedType() == GrGpuBufferType::kDrawIndirect || this->intendedType() == GrGpuBufferType::kXferCpuToGpu
