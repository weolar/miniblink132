...
GrD3DCaps::GrD3DCaps(const GrContextOptions& contextOptions, IDXGIAdapter1* adapter, ID3D12Device* device)
...
bool GrD3DCaps::canCopyTexture(DXGI_FORMAT dstFormat, int dstSampleCnt, DXGI_FORMAT srcFormat, int srcSampleCnt) const
{
...
bool GrD3DCaps::canCopyAsResolve(DXGI_FORMAT dstFormat, int dstSampleCnt, DXGI_FORMAT srcFormat, int srcSampleCnt) const
{
...
bool GrD3DCaps::onCanCopySurface(const GrSurfaceProxy* dst, const SkIRect& dstRect, const GrSurfaceProxy* src, const SkIRect& srcRect) const
{
    ...
    return this->canCopyTexture(dstFormat, dstSampleCnt, srcFormat, srcSampleCnt) || this->canCopyAsResolve(dstFormat, dstSampleCnt, srcFormat, srcSampleCnt);
}

void GrD3DCaps::init(const GrContextOptions& contextOptions, IDXGIAdapter1* adapter, ID3D12Device* device)
{
    ...
    GR_D3D_CALL_ERRCHECK(device->CheckFeatureSupport(D3D12_FEATURE_FEATURE_LEVELS, &flDesc, sizeof(flDesc)));
    ...
    GR_D3D_CALL_ERRCHECK(device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS, &optionsDesc, sizeof(optionsDesc)));
    ...
    GR_D3D_CALL_ERRCHECK(device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS2, &optionsDesc2, sizeof(optionsDesc2)));
    fResolveSubresourceRegionSupport = (optionsDesc2.ProgrammableSamplePositionsTier != D3D12_PROGRAMMABLE_SAMPLE_POSITIONS_TIER_NOT_SUPPORTED);
...
void GrD3DCaps::initGrCaps(const D3D12_FEATURE_DATA_D3D12_OPTIONS& optionsDesc, ID3D12Device* device)
{
    ...
    if (D3D12_CONSERVATIVE_RASTERIZATION_TIER_NOT_SUPPORTED != optionsDesc.ConservativeRasterizationTier) {
...
void GrD3DCaps::initShaderCaps(int vendorID, const D3D12_FEATURE_DATA_D3D12_OPTIONS& optionsDesc)
{
    ...
    shaderCaps->fHalfIs32Bits = D3D12_SHADER_MIN_PRECISION_SUPPORT_NONE == optionsDesc.MinPrecisionSupport;

    // See https://docs.microsoft.com/en-us/windows/win32/direct3d12/hardware-support
    // The maximum number of samplers in a shader-visible descriptor heap is 2048, but
    // 16 of those are reserved for the driver.
    shaderCaps->fMaxFragmentSamplers = (D3D12_RESOURCE_BINDING_TIER_1 == optionsDesc.ResourceBindingTier) ? 16 : 2032;
}
...
bool stencil_format_supported(ID3D12Device* device, DXGI_FORMAT format)
{
    ...
    GR_D3D_CALL_ERRCHECK(device->CheckFeatureSupport(D3D12_FEATURE_FORMAT_SUPPORT, &formatSupportDesc, sizeof(formatSupportDesc)));
...
static constexpr DXGI_FORMAT kDxgiFormats[]
    = { DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_FORMAT_R8_UNORM, DXGI_FORMAT_B8G8R8A8_UNORM, DXGI_FORMAT_B5G6R5_UNORM, DXGI_FORMAT_R16G16B16A16_FLOAT,
          DXGI_FORMAT_R16_FLOAT, DXGI_FORMAT_R8G8_UNORM, DXGI_FORMAT_R10G10B10A2_UNORM, DXGI_FORMAT_B4G4R4A4_UNORM, DXGI_FORMAT_R8G8B8A8_UNORM_SRGB,
          DXGI_FORMAT_BC1_UNORM, DXGI_FORMAT_R16_UNORM, DXGI_FORMAT_R16G16_UNORM, DXGI_FORMAT_R16G16B16A16_UNORM, DXGI_FORMAT_R16G16_FLOAT };
...
{
    static_assert(std::size(kDxgiFormats) == GrD3DCaps::kNumDxgiFormats, "Size of DXGI_FORMATs array must match static value in header");
...
void GrD3DCaps::FormatInfo::InitFormatFlags(const D3D12_FEATURE_DATA_FORMAT_SUPPORT& formatSupport, uint16_t* flags)
{
    if (SkToBool(D3D12_FORMAT_SUPPORT1_SHADER_SAMPLE & formatSupport.Support1)) {
        *flags = *flags | kTexturable_Flag;
        ...
        if (SkToBool(D3D12_FORMAT_SUPPORT1_RENDER_TARGET & formatSupport.Support1) && SkToBool(D3D12_FORMAT_SUPPORT1_BLENDABLE & formatSupport.Support1)) {
...
static bool multisample_count_supported(ID3D12Device* device, DXGI_FORMAT format, int sampleCount)
{
    ...
    GR_D3D_CALL_ERRCHECK(device->CheckFeatureSupport(D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS, &msqLevels, sizeof(msqLevels)));

    return msqLevels.NumQualityLevels > 0;
}

void GrD3DCaps::FormatInfo::initSampleCounts(const DXGI_ADAPTER_DESC& adapterDesc, ID3D12Device* device, DXGI_FORMAT format)
{
...
void GrD3DCaps::FormatInfo::init(const DXGI_ADAPTER_DESC& adapterDesc, ID3D12Device* device, DXGI_FORMAT format)
{
    D3D12_FEATURE_DATA_FORMAT_SUPPORT formatSupportDesc;
    formatSupportDesc.Format = format;
    GR_D3D_CALL_ERRCHECK(device->CheckFeatureSupport(D3D12_FEATURE_FORMAT_SUPPORT, &formatSupportDesc, sizeof(formatSupportDesc)));
...
bool GrD3DCaps::isFormatAsColorTypeRenderable(GrColorType ct, const GrBackendFormat& format, int sampleCount) const
{
    DXGI_FORMAT dxgiFormat;
    if (!format.asDxgiFormat(&dxgiFormat)) {
        return false;
    ...
    GrColorType surfaceColorType, const GrBackendFormat& surfaceFormat, GrColorType srcColorType) const
{
    ...
    size_t offsetAlignment = SkAlignTo(GrDxgiFormatBytesPerBlock(dxgiFormat), D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT);
...
GrCaps::SurfaceReadPixelsSupport GrD3DCaps::surfaceSupportsReadPixels(const GrSurface* surface) const
{
...
bool GrD3DCaps::onAreColorTypeAndFormatCompatible(GrColorType ct, const GrBackendFormat& format) const
{
    DXGI_FORMAT dxgiFormat;
    if (!format.asDxgiFormat(&dxgiFormat)) {
        return false;
...
skgpu::Swizzle GrD3DCaps::onGetReadSwizzle(const GrBackendFormat& format, GrColorType colorType) const
{
    DXGI_FORMAT dxgiFormat;
    ...
    SkDEBUGFAILF("Illegal color type (%d) and format (%d) combination.", (int)colorType, (int)dxgiFormat);
    return {};
}

skgpu::Swizzle GrD3DCaps::getWriteSwizzle(const GrBackendFormat& format, GrColorType colorType) const
{
    DXGI_FORMAT dxgiFormat;
    SkAssertResult(format.asDxgiFormat(&dxgiFormat));
    const auto& info = this->getFormatInfo(dxgiFormat);
...
void GrD3DCaps::addExtraSamplerKey(skgpu::KeyBuilder* b, GrSamplerState samplerState, const GrBackendFormat& format) const
{
    // TODO
}
 ...
 */
GrProgramDesc GrD3DCaps::makeDesc(GrRenderTarget* rt, const GrProgramInfo& programInfo, ProgramDescOverrideFlags overrideFlags) const
{
