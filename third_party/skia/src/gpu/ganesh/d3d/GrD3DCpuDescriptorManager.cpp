...
GrD3DDescriptorHeap::CPUHandle GrD3DCpuDescriptorManager::createRenderTargetView(GrD3DGpu* gpu, ID3D12Resource* textureResource)
{
...
GrD3DDescriptorHeap::CPUHandle GrD3DCpuDescriptorManager::createDepthStencilView(GrD3DGpu* gpu, ID3D12Resource* textureResource)
{
...
GrD3DDescriptorHeap::CPUHandle GrD3DCpuDescriptorManager::createConstantBufferView(GrD3DGpu* gpu, ID3D12Resource* bufferResource, size_t offset, size_t size)
{
...
GrD3DDescriptorHeap::CPUHandle GrD3DCpuDescriptorManager::createUnorderedAccessView(GrD3DGpu* gpu, ID3D12Resource* resource, unsigned int mipSlice)
{
    const GrD3DDescriptorHeap::CPUHandle& descriptor = fShaderViewDescriptorPool.allocateHandle(gpu);
...
GrD3DDescriptorHeap::CPUHandle GrD3DCpuDescriptorManager::createSampler(GrD3DGpu* gpu, D3D12_FILTER filter, float maxLOD, unsigned int maxAnisotropy,
...
{
    std::unique_ptr<GrD3DDescriptorHeap> heap = GrD3DDescriptorHeap::Make(gpu, type, numDescriptors, D3D12_DESCRIPTOR_HEAP_FLAG_NONE);
    if (!heap) {
        return nullptr;
    }
...
GrD3DCpuDescriptorManager::HeapPool::HeapPool(GrD3DGpu* gpu, D3D12_DESCRIPTOR_HEAP_TYPE heapType)
    ...
    , fHeapType(heapType)
{
    std::unique_ptr<GrD3DCpuDescriptorManager::Heap> heap = GrD3DCpuDescriptorManager::Heap::Make(gpu, fHeapType, fMaxAvailableDescriptors);
...
GrD3DDescriptorHeap::CPUHandle GrD3DCpuDescriptorManager::HeapPool::allocateHandle(GrD3DGpu* gpu)
{
    ...
    GrD3DDescriptorHeap::CPUHandle handle = fDescriptorHeaps[fDescriptorHeaps.size() - 1]->allocateCPUHandle();
    return handle;
}

void GrD3DCpuDescriptorManager::HeapPool::releaseHandle(const GrD3DDescriptorHeap::CPUHandle& dsvDescriptor)
{
    for (unsigned int i = 0; i < fDescriptorHeaps.size(); ++i) {
