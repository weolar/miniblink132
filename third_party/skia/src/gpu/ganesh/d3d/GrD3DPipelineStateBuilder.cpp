...
std::unique_ptr<GrD3DPipelineState> GrD3DPipelineStateBuilder::MakePipelineState(
    GrD3DGpu* gpu, GrD3DRenderTarget* renderTarget, const GrProgramDesc& desc, const GrProgramInfo& programInfo)
{
...
static gr_cp<ID3DBlob> GrCompileHLSLShader(GrD3DGpu* gpu, const std::string& hlsl, SkSL::ProgramKind kind)
{
    ...
    HRESULT hr = D3DCompile(hlsl.c_str(), hlsl.length(), nullptr, nullptr, nullptr, "main", compileTarget, compileFlags, 0, &shader, &errors);
    if (!SUCCEEDED(hr)) {
        gpu->getContext()->priv().getShaderErrorHandler()->compileError(hlsl.c_str(), reinterpret_cast<char*>(errors->GetBufferPointer()));
    }
    return shader;
}
...
bool GrD3DPipelineStateBuilder::loadHLSLFromCache(SkReadBuffer* reader, gr_cp<ID3DBlob> shaders[])
{
    ...
    return compile(SkSL::ProgramKind::kVertex, kVertex_GrShaderType) && compile(SkSL::ProgramKind::kFragment, kFragment_GrShaderType);
}

gr_cp<ID3DBlob> GrD3DPipelineStateBuilder::compileD3DProgram(
    SkSL::ProgramKind kind, const std::string& sksl, const SkSL::ProgramSettings& settings, SkSL::Program::Interface* outInterface, std::string* outHLSL)
{
    if (!skgpu::SkSLToHLSL(this->caps()->shaderCaps(), sksl, kind, settings, outHLSL, outInterface, fGpu->getContext()->priv().getShaderErrorHandler())) {
...
static void setup_vertex_input_layout(const GrGeometryProcessor& geomProc, D3D12_INPUT_ELEMENT_DESC* inputElements)
{
    ...
    for (auto attrib : geomProc.vertexAttributes()) {
        ...
        inputElements[currentAttrib] = { "TEXCOORD", currentAttrib, attrib_type_to_format(attrib.cpuType()), vertexSlot, SkToU32(*attrib.offset()),
    ...
    for (auto attrib : geomProc.instanceAttributes()) {
        // When using SPIRV-Cross it converts the location modifier in SPIRV to be
        // TEXCOORD<N> where N is the location value for eveery vertext attribute
        inputElements[currentAttrib] = { "TEXCOORD", currentAttrib, attrib_type_to_format(attrib.cpuType()), instanceSlot, SkToU32(*attrib.offset()),
...
{
    rasterizer->FillMode = (caps->wireframeMode() || pipeline.isWireframe()) ? D3D12_FILL_MODE_WIREFRAME : D3D12_FILL_MODE_SOLID;
...
gr_cp<ID3D12PipelineState> create_pipeline_state(GrD3DGpu* gpu, const GrProgramInfo& programInfo, const sk_sp<GrD3DRootSignature>& rootSig,
    gr_cp<ID3DBlob> vertexShader, gr_cp<ID3DBlob> pixelShader, DXGI_FORMAT renderTargetFormat, DXGI_FORMAT depthStencilFormat,
    unsigned int sampleQualityPattern)
{
    ...
    fill_in_rasterizer_state(programInfo.pipeline(), programInfo.numSamples() > 1, gpu->caps(), &psoDesc.RasterizerState);

    fill_in_depth_stencil_state(programInfo, &psoDesc.DepthStencilState);

    unsigned int totalAttributeCnt = programInfo.geomProc().numVertexAttributes() + programInfo.geomProc().numInstanceAttributes();
    ...
    gr_cp<ID3D12PipelineState> pipelineState;
    {
        ...
        GR_D3D_CALL_ERRCHECK(gpu->device()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&pipelineState)));
    }
...
std::unique_ptr<GrD3DPipelineState> GrD3DPipelineStateBuilder::finalize()
{
    ...
    if (kHLSL_Tag == shaderType && this->loadHLSLFromCache(&reader, shaders)) {
        ...
        if (kSKSL_Tag == shaderType) {
            if (GrPersistentCacheUtils::UnpackCachedShaders(&reader, cached_sksl, intfs, kGrShaderTypeCount)) {
        ...
        auto compile = [&](SkSL::ProgramKind kind, GrShaderType shaderType) {
            shaders[shaderType] = this->compileD3DProgram(kind, *sksl[shaderType], settings, &intfs[shaderType], &hlsl[shaderType]);
            return shaders[shaderType].get();
        };

        if (!compile(SkSL::ProgramKind::kVertex, kVertex_GrShaderType) || !compile(SkSL::ProgramKind::kFragment, kFragment_GrShaderType)) {
            return nullptr;
        }

        if (persistentCache && !cached) {
            const bool cacheSkSL = fGpu->getContext()->priv().options().fShaderCacheStrategy == GrContextOptions::ShaderCacheStrategy::kSkSL;
            if (cacheSkSL) {
            ...
            sk_sp<SkData> data = GrPersistentCacheUtils::PackCachedShaders(cacheSkSL ? kSKSL_Tag : kHLSL_Tag, hlsl, intfs, kGrShaderTypeCount);
            persistentCache->store(*key, *data, description);
        }
    }

    sk_sp<GrD3DRootSignature> rootSig = fGpu->resourceProvider().findOrCreateRootSignature(fUniformHandler.fSamplers.count());
    if (!rootSig) {
        return nullptr;
    }

    const GrD3DRenderTarget* rt = static_cast<const GrD3DRenderTarget*>(fRenderTarget);
    gr_cp<ID3D12PipelineState> pipelineState = create_pipeline_state(fGpu, fProgramInfo, rootSig, std::move(shaders[kVertex_GrShaderType]),
        std::move(shaders[kFragment_GrShaderType]), rt->dxgiFormat(), rt->stencilDxgiFormat(), rt->sampleQualityPattern());
    sk_sp<GrD3DPipeline> pipeline = GrD3DPipeline::Make(std::move(pipelineState));
    if (!pipeline) {
        return nullptr;
    }

    return std::unique_ptr<GrD3DPipelineState>(new GrD3DPipelineState(std::move(pipeline), std::move(rootSig), fUniformHandles, fUniformHandler.fUniforms,
        fUniformHandler.fCurrentUBOOffset, fUniformHandler.fSamplers.count(), std::move(fGPImpl), std::move(fXPImpl), std::move(fFPImpls),
        geomProc.vertexStride(), geomProc.instanceStride()));
}

sk_sp<GrD3DPipeline> GrD3DPipelineStateBuilder::MakeComputePipeline(GrD3DGpu* gpu, GrD3DRootSignature* rootSig, const char* shader)
{
    ...
    gr_cp<ID3DBlob> shaderBlob;
    {
        ...
        gr_cp<ID3DBlob> errors;
        HRESULT hr = D3DCompile(shader, strlen(shader), nullptr, nullptr, nullptr, "main", "cs_5_1", compileFlags, 0, &shaderBlob, &errors);
        if (!SUCCEEDED(hr)) {
            gpu->getContext()->priv().getShaderErrorHandler()->compileError(shader, reinterpret_cast<char*>(errors->GetBufferPointer()));
            return nullptr;
        }
    ...
    gr_cp<ID3D12PipelineState> pipelineState;
    {
        ...
        GR_D3D_CALL_ERRCHECK(gpu->device()->CreateComputePipelineState(&psoDesc, IID_PPV_ARGS(&pipelineState)));
    }
