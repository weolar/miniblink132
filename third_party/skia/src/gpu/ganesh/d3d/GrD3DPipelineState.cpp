...
GrD3DPipelineState::GrD3DPipelineState(sk_sp<GrD3DPipeline> pipeline, sk_sp<GrD3DRootSignature> rootSignature,
    const GrGLSLBuiltinUniformHandles& builtinUniformHandles, const UniformInfoArray& uniforms, uint32_t uniformSize, uint32_t numSamplers,
    std::unique_ptr<GrGeometryProcessor::ProgramImpl> gpImpl, std::unique_ptr<GrXferProcessor::ProgramImpl> xpImpl,
    std::vector<std::unique_ptr<GrFragmentProcessor::ProgramImpl>> fpImpls, size_t vertexStride, size_t instanceStride)
...
void GrD3DPipelineState::setAndBindConstants(GrD3DGpu* gpu, const GrRenderTarget* renderTarget, const GrProgramInfo& programInfo)
{
    ...
    for (int i = 0; i < programInfo.pipeline().numFragmentProcessors(); ++i) {
        ...
        fp.visitWithImpls([&](const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl) { impl.setData(fDataManager, fp); }, *fFPImpls[i]);
    }
    ...
    gpu->currentCommandList()->setGraphicsRootConstantBufferView((unsigned int)(GrD3DRootSignature::ParamIndex::kConstantBufferView), constantsAddress);
}
...
void GrD3DPipelineState::setRenderTargetState(const GrRenderTarget* rt, GrSurfaceOrigin origin)
{
    ...
    if (fRenderTargetState.fRenderTargetOrigin != origin || fRenderTargetState.fRenderTargetSize != dimensions) {
...
void GrD3DPipelineState::setAndBindTextures(
    GrD3DGpu* gpu, const GrGeometryProcessor& geomProc, const GrSurfaceProxy* const geomProcTextures[], const GrPipeline& pipeline)
{
    ...
    for (int i = 0; i < geomProc.numTextureSamplers(); ++i) {
        ...
        samplers[currTextureBinding++] = gpu->resourceProvider().findOrCreateCompatibleSampler(sampler.samplerState());
    ...
    if (GrTexture* dstTexture = pipeline.peekDstTexture()) {
        ...
        shaderResourceViews[currTextureBinding] = texture->shaderResourceView();
        samplers[currTextureBinding++] = gpu->resourceProvider().findOrCreateCompatibleSampler(GrSamplerState::Filter::kNearest);
        gpu->currentCommandList()->addSampledTextureRef(texture);
    }
    ...
    pipeline.visitTextureEffects([&](const GrTextureEffect& te) {
        ...
        shaderResourceViews[currTextureBinding] = texture->shaderResourceView();
        samplers[currTextureBinding++] = gpu->resourceProvider().findOrCreateCompatibleSampler(samplerState);
        gpu->currentCommandList()->addSampledTextureRef(texture);
    });
    ...
    if (fNumSamplers > 0) {
        ...
        sk_sp<GrD3DDescriptorTable> srvTable = gpu->resourceProvider().findOrCreateShaderViewTable(shaderResourceViews);
        sk_sp<GrD3DDescriptorTable> samplerTable = gpu->resourceProvider().findOrCreateSamplerTable(samplers);
        ...
        // bind sampler table
        gpu->currentCommandList()->setGraphicsRootDescriptorTable(
            (unsigned int)GrD3DRootSignature::ParamIndex::kSamplerDescriptorTable, samplerTable->baseGpuDescriptor());
    }
}

void GrD3DPipelineState::bindBuffers(GrD3DGpu* gpu, sk_sp<const GrBuffer> indexBuffer, sk_sp<const GrBuffer> instanceBuffer, sk_sp<const GrBuffer> vertexBuffer,
    GrD3DDirectCommandList* commandList)
{
    ...
    if (vertexBuffer) {
        ...
        const_cast<GrD3DBuffer*>(d3dVertexBuffer)->setResourceState(gpu, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
    }
    if (instanceBuffer) {
        ...
        const_cast<GrD3DBuffer*>(d3dInstanceBuffer)->setResourceState(gpu, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
    }
    commandList->setVertexBuffers(0, std::move(vertexBuffer), fVertexStride, std::move(instanceBuffer), fInstanceStride);
    ...
    if (auto* d3dIndexBuffer = static_cast<const GrD3DBuffer*>(indexBuffer.get())) {
        ...
        const_cast<GrD3DBuffer*>(d3dIndexBuffer)->setResourceState(gpu, D3D12_RESOURCE_STATE_INDEX_BUFFER);
