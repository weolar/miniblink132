...
void GrD3DTextureResource::setResourceState(const GrD3DGpu* gpu, D3D12_RESOURCE_STATES newResourceState, unsigned int subresource)
{
...
bool GrD3DTextureResource::InitTextureResourceInfo(GrD3DGpu* gpu, const D3D12_RESOURCE_DESC& desc, D3D12_RESOURCE_STATES initialState, GrProtected isProtected,
    D3D12_CLEAR_VALUE* clearValue, GrD3DTextureResourceInfo* info)
{
    ...
    info->fResource = gpu->memoryAllocator()->createResource(D3D12_HEAP_TYPE_DEFAULT, &desc, initialState, &info->fAlloc, clearValue);
    if (!info->fResource) {
        return false;
    }
...
std::pair<GrD3DTextureResourceInfo, sk_sp<GrD3DResourceState>> GrD3DTextureResource::CreateMSAA(
    GrD3DGpu* gpu, SkISize dimensions, int sampleCnt, const GrD3DTextureResourceInfo& info, SkColor4f clearColor)
{
    ...
    if (!InitTextureResourceInfo(gpu, msTextureDesc, D3D12_RESOURCE_STATE_RENDER_TARGET, info.fProtected, &clearValue, &msInfo)) {
        return {};
    }
