...
std::unique_ptr<GrGpu> GrD3DGpu::Make(const GrD3DBackendContext& backendContext, const GrContextOptions& contextOptions, GrDirectContext* direct)
{
    ...
    if (!memoryAllocator) {
        ...
        memoryAllocator = GrD3DAMDMemoryAllocator::Make(backendContext.fAdapter.get(), backendContext.fDevice.get());
    }
    if (!memoryAllocator) {
    ...
    return std::unique_ptr<GrGpu>(new GrD3DGpu(direct, contextOptions, backendContext, memoryAllocator));
}
...
GrD3DGpu::GrD3DGpu(
    GrDirectContext* direct, const GrContextOptions& contextOptions, const GrD3DBackendContext& backendContext, sk_sp<GrD3DMemoryAllocator> allocator)
    : INHERITED(direct)
...
{
    this->initCaps(sk_make_sp<GrD3DCaps>(contextOptions, backendContext.fAdapter.get(), backendContext.fDevice.get()));
    ...
    GR_D3D_CALL_ERRCHECK(fDevice->CreateFence(fCurrentFenceValue, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&fFence)));
...
GrOpsRenderPass* GrD3DGpu::onGetOpsRenderPass(GrRenderTarget* rt, bool /*useMSAASurface*/, GrAttachment*, GrSurfaceOrigin origin, const SkIRect& bounds,
...
bool GrD3DGpu::submitDirectCommandList(SyncQueue sync)
{
    ...
    new (fOutstandingCommandLists.push_back()) OutstandingCommandList(std::move(fCurrentDirectCommandList), fCurrentFenceValue);
...
void GrD3DGpu::endRenderPass(GrRenderTarget* target, GrSurfaceOrigin origin, const SkIRect& bounds)
{
...
sk_sp<GrD3DTexture> GrD3DGpu::createD3DTexture(SkISize dimensions, DXGI_FORMAT dxgiFormat, GrRenderable renderable, int renderTargetSampleCnt,
    skgpu::Budgeted budgeted, GrProtected isProtected, int mipLevelCount, GrMipmapStatus mipmapStatus, std::string_view label)
{
    ...
    if (renderable == GrRenderable::kYes) {
        return GrD3DTextureRenderTarget::MakeNewTextureRenderTarget(
            this, budgeted, dimensions, renderTargetSampleCnt, resourceDesc, isProtected, mipmapStatus, label);
    } else {
        return GrD3DTexture::MakeNewTexture(this, budgeted, dimensions, resourceDesc, isProtected, mipmapStatus, label);
    }
}

sk_sp<GrTexture> GrD3DGpu::onCreateTexture(SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable, int renderTargetSampleCnt,
    skgpu::Budgeted budgeted, GrProtected isProtected, int mipLevelCount, uint32_t levelClearMask, std::string_view label)
{
    ...
    sk_sp<GrD3DTexture> tex
        = this->createD3DTexture(dimensions, dxgiFormat, renderable, renderTargetSampleCnt, budgeted, isProtected, mipLevelCount, mipmapStatus, label);
    if (!tex) {
        return nullptr;
    }
...
static void copy_compressed_data(char* mapPtr, DXGI_FORMAT dxgiFormat, D3D12_PLACED_SUBRESOURCE_FOOTPRINT* placedFootprints, UINT* numRows,
    UINT64* rowSizeInBytes, const void* compressedData, int numMipLevels)
{
    ...
    for (int currentMipLevel = 0; currentMipLevel < numMipLevels; currentMipLevel++) {
        ...
        SkRectMemcpy(dst, placedFootprints[currentMipLevel].Footprint.RowPitch, src, rowSizeInBytes[currentMipLevel], rowSizeInBytes[currentMipLevel],
            numRows[currentMipLevel]);
        src += numRows[currentMipLevel] * rowSizeInBytes[currentMipLevel];
    }
}

sk_sp<GrTexture> GrD3DGpu::onCreateCompressedTexture(SkISize dimensions, const GrBackendFormat& format, skgpu::Budgeted budgeted, skgpu::Mipmapped mipmapped,
    GrProtected isProtected, const void* data, size_t dataSize)
{
    ...
    SkASSERT(dataSize == SkCompressedFormatDataSize(compression, dimensions, mipmapped == skgpu::Mipmapped::kYes));

    int mipLevelCount = 1;
    if (mipmapped == skgpu::Mipmapped::kYes) {
        mipLevelCount = SkMipmap::ComputeLevelCount(dimensions.width(), dimensions.height()) + 1;
    }
    GrMipmapStatus mipmapStatus = mipLevelCount > 1 ? GrMipmapStatus::kValid : GrMipmapStatus::kNotAllocated;

    sk_sp<GrD3DTexture> d3dTex = this->createD3DTexture(dimensions, dxgiFormat, GrRenderable::kNo, 1, budgeted, isProtected, mipLevelCount, mipmapStatus,
    ...
    fDevice->GetCopyableFootprints(&desc, 0, mipLevelCount, 0, placedFootprints.get(), numRows.get(), rowSizeInBytes.get(), &combinedBufferSize);
    SkASSERT(combinedBufferSize);

    GrStagingBufferManager::Slice slice = fStagingBufferManager.allocateStagingBufferSlice(combinedBufferSize, D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT);
    if (!slice.fBuffer) {
        return nullptr;
    }
    ...
    copy_compressed_data(bufferData, desc.Format, placedFootprints.get(), numRows.get(), rowSizeInBytes.get(), data, mipLevelCount);
    ...
    fCurrentDirectCommandList->copyBufferToTexture(d3dBuffer, d3dTex.get(), mipLevelCount, placedFootprints.get(), 0, 0);
...
void GrD3DGpu::copySurfaceAsCopyTexture(
    GrSurface* dst, GrSurface* src, GrD3DTextureResource* dstResource, GrD3DTextureResource* srcResource, const SkIRect& srcRect, const SkIPoint& dstPoint)
{
#ifdef SK_DEBUG
    int dstSampleCnt = get_surface_sample_cnt(dst);
    int srcSampleCnt = get_surface_sample_cnt(src);
    ...
    fCurrentDirectCommandList->copyTextureRegionToTexture(
        dstResource->resource(), &dstLocation, dstPoint.fX, dstPoint.fY, srcResource->resource(), &srcLocation, &srcBox);
...
void GrD3DGpu::copySurfaceAsResolve(GrSurface* dst, GrSurface* src, const SkIRect& srcRect, const SkIPoint& dstPoint)
{
...
void GrD3DGpu::resolveTexture(GrSurface* dst, int32_t dstX, int32_t dstY, GrD3DRenderTarget* src, const SkIRect& srcIRect)
{
    SkASSERT(dst);
    ...
    fCurrentDirectCommandList->resolveSubresourceRegion(dstTextureResource, dstX, dstY, src->msaaTextureResource(), &srcRect);
}
...
bool GrD3DGpu::onReadPixels(GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType dstColorType, void* buffer, size_t rowBytes)
{
    ...
    fDevice->GetCopyableFootprints(&desc, 0, 1, 0, &placedFootprint, nullptr, nullptr, &transferTotalBytes);
    SkASSERT(transferTotalBytes);
    GrResourceProvider* resourceProvider = this->getContext()->priv().resourceProvider();
    sk_sp<GrGpuBuffer> transferBuffer
        = resourceProvider->createBuffer(transferTotalBytes, GrGpuBufferType::kXferGpuToCpu, kDynamic_GrAccessPattern, GrResourceProvider::ZeroInit::kNo);
    if (!transferBuffer) {
        return false;
    }
    ...
    SkRectMemcpy(buffer, rowBytes, mappedMemory, placedFootprint.Footprint.RowPitch, tightRowBytes, rect.height());
...
void GrD3DGpu::readOrTransferPixels(
    GrD3DTextureResource* texResource, SkIRect rect, sk_sp<GrGpuBuffer> transferBuffer, const D3D12_PLACED_SUBRESOURCE_FOOTPRINT& placedFootprint)
{
    ...
    fCurrentDirectCommandList->copyTextureRegionToBuffer(transferBuffer, &dstLocation, 0, 0, texResource->resource(), &srcLocation, &srcBox);
}

bool GrD3DGpu::onWritePixels(GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType srcColorType, const GrMipLevel texels[],
...
bool GrD3DGpu::uploadToTexture(GrD3DTexture* tex, SkIRect rect, GrColorType colorType, const GrMipLevel* texels, int mipLevelCount)
{
    ...
    fDevice->GetCopyableFootprints(&desc, 0, mipLevelCount, 0, placedFootprints.get(), nullptr, nullptr, &combinedBufferSize);
    ...
    for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
        if (texels[currentMipLevel].fPixels) {
            ...
            SkRectMemcpy(dst, placedFootprints[currentMipLevel].Footprint.RowPitch, src, srcRowBytes, trimRowBytes, currentHeight);
        }
    ...
    ID3D12Resource* d3dBuffer = static_cast<GrD3DBuffer*>(slice.fBuffer)->d3dResource();
    fCurrentDirectCommandList->copyBufferToTexture(d3dBuffer, tex, mipLevelCount, placedFootprints.get(), rect.left(), rect.top());
...
bool GrD3DGpu::onTransferFromBufferToBuffer(sk_sp<GrGpuBuffer> src, size_t srcOffset, sk_sp<GrGpuBuffer> dst, size_t dstOffset, size_t size)
{
    ...
    fCurrentDirectCommandList->copyBufferToBuffer(std::move(d3dDst), dstOffset, d3dSrc->d3dResource(), srcOffset, size);
...
bool GrD3DGpu::onTransferPixelsTo(GrTexture* texture, SkIRect rect, GrColorType surfaceColorType, GrColorType bufferColorType,
    sk_sp<GrGpuBuffer> transferBuffer, size_t bufferOffset, size_t rowBytes)
{
    ...
    fCurrentDirectCommandList->copyBufferToTexture(d3dBuffer, d3dTex, 1, &placedFootprint, rect.left(), rect.top());
    ...
    GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType bufferColorType, sk_sp<GrGpuBuffer> transferBuffer, size_t offset)
{
    ...
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT placedFootprint;
    UINT64 transferTotalBytes;
    fDevice->GetCopyableFootprints(&desc, 0, 1, offset, &placedFootprint, nullptr, nullptr, &transferTotalBytes);
    SkASSERT(transferTotalBytes);
...
sk_sp<GrTexture> GrD3DGpu::onWrapBackendTexture(const GrBackendTexture& tex, GrWrapOwnership, GrWrapCacheable wrapType, GrIOType ioType)
{
    ...
    SkASSERT(state);
    return GrD3DTexture::MakeWrappedTexture(this, tex.dimensions(), wrapType, ioType, textureInfo, std::move(state));
}
...
sk_sp<GrTexture> GrD3DGpu::onWrapRenderableBackendTexture(const GrBackendTexture& tex, int sampleCnt, GrWrapOwnership ownership, GrWrapCacheable cacheable)
{
    ...
    return GrD3DTextureRenderTarget::MakeWrappedTextureRenderTarget(this, tex.dimensions(), sampleCnt, cacheable, textureInfo, std::move(state));
}
...
sk_sp<GrRenderTarget> GrD3DGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& rt)
{
    ...
    sk_sp<GrD3DRenderTarget> tgt = GrD3DRenderTarget::MakeWrappedRenderTarget(this, rt.dimensions(), rt.sampleCnt(), info, std::move(state));
...
bool GrD3DGpu::onRegenerateMipMapLevels(GrTexture* tex)
{
    ...
    if (caps.isFormatUnorderedAccessible(originalFormat) && (originalDesc.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS)) {
        ...
        uavTexture = GrD3DTexture::MakeNewTexture(this, skgpu::Budgeted::kNo, tex->dimensions(), uavDesc, grProtected, GrMipmapStatus::kDirty,
        ...
        if (!caps.isFormatUnorderedAccessible(originalFormat) && is_bgra(originalFormat)) {
            ...
            bgraAliasTexture = GrD3DTexture::MakeAliasingTexture(this, uavTexture, originalDesc, D3D12_RESOURCE_STATE_COPY_DEST);
            // make the BGRA version the active alias
            this->currentCommandList()->aliasingBarrier(nullptr, nullptr, bgraAliasTexture->resource(), bgraAliasTexture->d3dResource());
            ...
            this->currentCommandList()->aliasingBarrier(
                bgraAliasTexture->resource(), bgraAliasTexture->d3dResource(), uavTexture->resource(), uavTexture->d3dResource());
        } else {
    ...
    for (unsigned int dstMip = 1; dstMip < levelCount; ++dstMip) {
        ...
        D3D12_GPU_VIRTUAL_ADDRESS constantsAddress = fResourceProvider.uploadConstantData(&constantData, sizeof(constantData));
        this->currentCommandList()->setComputeRootConstantBufferView((unsigned int)GrD3DRootSignature::ParamIndex::kConstantBufferView, constantsAddress);

        std::vector<D3D12_CPU_DESCRIPTOR_HANDLE> shaderViews;
        // create SRV
        GrD3DDescriptorHeap::CPUHandle srvHandle = fResourceProvider.createShaderResourceView(uavTexture->d3dResource(), srcMip, 1);
        shaderViews.push_back(srvHandle.fHandle);
        fMipmapCPUDescriptors.push_back(srvHandle);
        // create UAV
        GrD3DDescriptorHeap::CPUHandle uavHandle = fResourceProvider.createUnorderedAccessView(uavTexture->d3dResource(), dstMip);
        shaderViews.push_back(uavHandle.fHandle);
        fMipmapCPUDescriptors.push_back(uavHandle);

        // set up shaderView descriptor table
        sk_sp<GrD3DDescriptorTable> srvTable = fResourceProvider.findOrCreateShaderViewTable(shaderViews);
        ...
        this->currentCommandList()->setComputeRootDescriptorTable(
            (unsigned int)GrD3DRootSignature::ParamIndex::kShaderViewDescriptorTable, srvTable->baseGpuDescriptor());
        this->currentCommandList()->setComputeRootDescriptorTable(
            static_cast<unsigned int>(GrD3DRootSignature::ParamIndex::kSamplerDescriptorTable), samplerTable->baseGpuDescriptor());
    ...
    if (uavTexture.get() != d3dTex) {
        ...
        if (bgraAliasTexture) {
            // make the BGRA version the active alias
            this->currentCommandList()->aliasingBarrier(
                uavTexture->resource(), uavTexture->d3dResource(), bgraAliasTexture->resource(), bgraAliasTexture->d3dResource());
...
sk_sp<GrAttachment> GrD3DGpu::makeStencilAttachment(const GrBackendFormat& /*colorFormat*/, SkISize dimensions, int numStencilSamples)
{
...
bool GrD3DGpu::createTextureResourceForBackendSurface(DXGI_FORMAT dxgiFormat, SkISize dimensions, GrTexturable texturable, GrRenderable renderable,
    skgpu::Mipmapped mipmapped, int sampleCnt, GrD3DTextureResourceInfo* info, GrProtected isProtected)
{
    ...
    D3D12_RESOURCE_STATES initialState = (renderable == GrRenderable::kYes) ? D3D12_RESOURCE_STATE_RENDER_TARGET : D3D12_RESOURCE_STATE_COPY_DEST;
    if (!GrD3DTextureResource::InitTextureResourceInfo(this, resourceDesc, initialState, isProtected, clearValuePtr, info)) {
    ...
    SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable, skgpu::Mipmapped mipmapped, GrProtected isProtected, std::string_view label)
{
    ...
    GrD3DTextureResourceInfo info;
    if (!this->createTextureResourceForBackendSurface(dxgiFormat, dimensions, GrTexturable::kYes, renderable, mipmapped, 1, &info, isProtected)) {
        return {};
    }
...
static bool copy_color_data(const GrD3DCaps& caps, char* mapPtr, DXGI_FORMAT dxgiFormat, SkISize dimensions,
...
bool GrD3DGpu::onClearBackendTexture(const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback, std::array<float, 4> color)
{
    ...
    sk_sp<GrD3DTexture> texture
        = GrD3DTexture::MakeWrappedTexture(this, backendTexture.dimensions(), GrWrapCacheable::kNo, kRW_GrIOType, info, std::move(state));
    if (!texture) {
        return false;
    }
    ...
    fDevice->GetCopyableFootprints(&desc,
        /* first resource  */ 0,
        /* mip level count */ 1,
        /* base offset     */ 0, placedFootprints.get(), &numRows, &rowSizeInBytes, &combinedBufferSize);
    SkASSERT(combinedBufferSize);
    ...
    char* bufferData = (char*)slice.fOffsetMapPtr;
    SkASSERT(bufferData);
    if (!copy_color_data(this->d3dCaps(), bufferData, info.fFormat, backendTexture.dimensions(), placedFootprints, color)) {
        return false;
    }
    ...
    const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback, const void* data, size_t size)
{
    ...
    if (backendTexture.hasMipmaps()) {
        mipLevelCount = SkMipmap::ComputeLevelCount(backendTexture.dimensions().width(), backendTexture.dimensions().height()) + 1;
    }
    SkASSERT(mipLevelCount == info.fLevelCount);
    AutoTMalloc<D3D12_PLACED_SUBRESOURCE_FOOTPRINT> placedFootprints(mipLevelCount);
    UINT64 combinedBufferSize;
    ...
    char* bufferData = (char*)slice.fOffsetMapPtr;
    SkASSERT(bufferData);
    copy_compressed_data(bufferData, info.fFormat, placedFootprints.get(), numRows.get(), rowSizeInBytes.get(), data, info.fLevelCount);
    ...
    ID3D12Resource* d3dBuffer = static_cast<GrD3DBuffer*>(slice.fBuffer)->d3dResource();
    cmdList->copyBufferToTexture(d3dBuffer, texture.get(), mipLevelCount, placedFootprints.get(), 0, 0);
...
GrBackendRenderTarget GrD3DGpu::createTestingOnlyBackendRenderTarget(SkISize dimensions, GrColorType colorType, int sampleCnt, GrProtected isProtected)
{
...
void GrD3DGpu::addResourceBarriers(sk_sp<GrManagedResource> resource, int numBarriers, D3D12_RESOURCE_TRANSITION_BARRIER* barriers) const
{
    SkASSERT(fCurrentDirectCommandList);
    SkASSERT(resource);
...
void GrD3DGpu::addBufferResourceBarriers(GrD3DBuffer* buffer, int numBarriers, D3D12_RESOURCE_TRANSITION_BARRIER* barriers) const
{
    SkASSERT(fCurrentDirectCommandList);
    SkASSERT(buffer);
