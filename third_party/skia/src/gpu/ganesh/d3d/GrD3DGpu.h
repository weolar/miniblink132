...
private:
    ...
    GrD3DGpu(GrDirectContext*, const GrContextOptions&, const GrD3DBackendContext&, sk_sp<GrD3DMemoryAllocator>);
    ...
    bool onTransferPixelsTo(
        GrTexture*, SkIRect, GrColorType surfaceColorType, GrColorType bufferColorType, sk_sp<GrGpuBuffer>, size_t offset, size_t rowBytes) override;
    ...
    GrOpsRenderPass* onGetOpsRenderPass(GrRenderTarget*, bool useMSAASurface, GrAttachment*, GrSurfaceOrigin, const SkIRect&,
    ...
    void resolveTexture(GrSurface* dst, int32_t dstX, int32_t dstY, GrD3DRenderTarget* src, const SkIRect& srcRect);

    sk_sp<GrD3DTexture> createD3DTexture(
        SkISize, DXGI_FORMAT, GrRenderable, int renderTargetSampleCnt, skgpu::Budgeted, GrProtected, int mipLevelCount, GrMipmapStatus, std::string_view label);

    bool uploadToTexture(GrD3DTexture* tex, SkIRect rect, GrColorType colorType, const GrMipLevel texels[], int mipLevelCount);
    ...
    bool createTextureResourceForBackendSurface(DXGI_FORMAT dxgiFormat, SkISize dimensions, GrTexturable texturable, GrRenderable renderable,
