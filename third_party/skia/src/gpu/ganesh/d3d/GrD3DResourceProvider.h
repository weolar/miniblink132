...
public:
    ...
    {
        fPipelineStateCache->markPipelineStateUniformsDirty();
    }
...
#define GR_PIPELINE_STATE_CACHE_STATS
    ...
    public:
        DescriptorTableCache(GrD3DGpu* gpu)
            : fGpu(gpu)
            , fMap(64)
        {
        ...
        sk_sp<GrD3DDescriptorTable> findOrCreateDescTable(const std::vector<D3D12_CPU_DESCRIPTOR_HANDLE>&, CreateFunc);
    ...
    private:
        ...
        struct DescTableHash {
            ...
            {
                return SkChecksum::Hash32(key.data(), key.size() * sizeof(D3D12_CPU_DESCRIPTOR_HANDLE));
            }
        };
