...
bool GrD3DOpsRenderPass::set(GrRenderTarget* rt, GrSurfaceOrigin origin, const SkIRect& bounds, const GrOpsRenderPass::LoadAndStoreInfo& colorInfo,
    const GrOpsRenderPass::StencilLoadAndStoreInfo& stencilInfo, const TArray<GrSurfaceProxy*, true>& sampledProxies)
{
...
void set_stencil_ref(GrD3DGpu* gpu, const GrProgramInfo& info)
{
    ...
    if (!stencilSettings.isDisabled()) {
        ...
        if (stencilSettings.isTwoSided()) {
            SkASSERT(stencilSettings.postOriginCCWFace(info.origin()).fRef == stencilSettings.postOriginCWFace(info.origin()).fRef);
...
{
    SkASSERT(scissorRect.isEmpty() || SkIRect::MakeWH(renderTarget->width(), renderTarget->height()).contains(scissorRect));
...
bool GrD3DOpsRenderPass::onBindTextures(const GrGeometryProcessor& geomProc, const GrSurfaceProxy* const geomProcTextures[], const GrPipeline& pipeline)
{
    ...
    pipeline.visitTextureEffects([&](const GrTextureEffect& te) { update_resource_state(te.texture(), fRenderTarget, fGpu); });
    ...
    sk_sp<const GrBuffer> indexBuffer, sk_sp<const GrBuffer> instanceBuffer, sk_sp<const GrBuffer> vertexBuffer, GrPrimitiveRestart primRestart)
{
    ...
    fCurrentPipelineState->bindBuffers(fGpu, std::move(indexBuffer), std::move(instanceBuffer), std::move(vertexBuffer), currCmdList);
}

void GrD3DOpsRenderPass::onDrawInstanced(int instanceCount, int baseInstance, int vertexCount, int baseVertex)
{
    SkASSERT(fCurrentPipelineState);
...
void GrD3DOpsRenderPass::onDrawIndexedInstanced(int indexCount, int baseIndex, int instanceCount, int baseInstance, int baseVertex)
{
    SkASSERT(fCurrentPipelineState);
    fGpu->currentCommandList()->drawIndexedInstanced(indexCount, instanceCount, baseIndex, baseVertex, baseInstance);
    fGpu->stats()->incNumDraws();
}
...
void GrD3DOpsRenderPass::onDrawIndirect(const GrBuffer* buffer, size_t offset, int drawCount)
{
    ...
    sk_sp<GrD3DCommandSignature> cmdSig = fGpu->resourceProvider().findOrCreateCommandSignature(GrD3DCommandSignature::ForIndexed::kNo, kSlot);
    fGpu->currentCommandList()->executeIndirect(cmdSig, drawCount, static_cast<const GrD3DBuffer*>(buffer), offset);
    fGpu->stats()->incNumDraws();
}

void GrD3DOpsRenderPass::onDrawIndexedIndirect(const GrBuffer* buffer, size_t offset, int drawCount)
{
    constexpr unsigned int kSlot = 0;
    sk_sp<GrD3DCommandSignature> cmdSig = fGpu->resourceProvider().findOrCreateCommandSignature(GrD3DCommandSignature::ForIndexed::kYes, kSlot);
    fGpu->currentCommandList()->executeIndirect(cmdSig, drawCount, static_cast<const GrD3DBuffer*>(buffer), offset);
    fGpu->stats()->incNumDraws();
}
...
static D3D12_RECT scissor_to_d3d_clear_rect(const GrScissorState& scissor, const GrSurface* surface, GrSurfaceOrigin origin)
{
    ...
    if (!scissor.enabled()) {
        ...
        d3dRect.setLTRB(scissor.rect().fLeft, surface->height() - scissor.rect().fBottom, scissor.rect().fRight, surface->height() - scissor.rect().fTop);
    }
