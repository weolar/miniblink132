...
public:
    ...
    struct OpArgs {
        ...
        explicit OpArgs(GrOp* op, const GrSurfaceProxyView& surfaceView, bool usesMSAASurface, GrAppliedClip* appliedClip, const GrDstProxyView& dstProxyView,
        ...
        {
            return fRenderPassXferBarriers;
        }
    ...
    void* makeVertexSpaceAtLeast(
        size_t vertexSize, int minVertexCount, int fallbackVertexCount, sk_sp<const GrBuffer>*, int* startVertex, int* actualVertexCount) final;
    ...
    {
        return (fOpArgs->appliedClip()) ? fOpArgs->appliedClip()->hardClip() : GrAppliedHardClip::Disabled();
    }
    ...
    {
        SkASSERT((programInfo.pipeline().isScissorTestEnabled()) == (this->appliedClip() && this->appliedClip()->scissorState().enabled()));
    ...
    void bindBuffers(sk_sp<const GrBuffer> indexBuffer, sk_sp<const GrBuffer> instanceBuffer, sk_sp<const GrBuffer> vertexBuffer,
        GrPrimitiveRestart primitiveRestart = GrPrimitiveRestart::kNo)
    {
        fOpsRenderPass->bindBuffers(std::move(indexBuffer), std::move(instanceBuffer), std::move(vertexBuffer), primitiveRestart);
    }
    ...
    {
        fOpsRenderPass->drawIndexedInstanced(indexCount, baseIndex, instanceCount, baseInstance, baseVertex);
    }
    ...
    {
        fOpsRenderPass->drawIndexPattern(patternIndexCount, patternRepeatCount, maxPatternRepetitionsInIndexBuffer, patternVertexCount, baseVertex);
    }
