...
static void gen_xp_key(const GrXferProcessor& xp, const GrCaps& caps, const GrPipeline& pipeline, skgpu::KeyBuilder* b)
{
    ...
    if (dstView.proxy()) {
        ...
        uint32_t samplerKey = sampler_key(dstView.proxy()->backendFormat().textureType(), dstView.swizzle(), caps);
        b->add32(samplerKey);
    }

    xp.addToKey(*caps.shaderCaps(), b, originIfDstTexture, pipeline.dstSampleFlags() & GrDstSampleFlags::kAsInputAttachment);
}
...
static void gen_fp_key(const GrFragmentProcessor& fp, const GrCaps& caps, skgpu::KeyBuilder* b)
{
    ...
    b->addBits(GrGeometryProcessor::kCoordTransformKeyBits, GrGeometryProcessor::ComputeCoordTransformsKey(fp), "fpTransforms");
...
void GrProgramDesc::Build(GrProgramDesc* desc, const GrProgramInfo& programInfo, const GrCaps& caps)
{
    desc->reset();
