...
sk_sp<GrTextureProxy> GrDynamicAtlas::MakeLazyAtlasProxy(LazyInstantiateAtlasCallback&& callback, GrColorType colorType,
    InternalMultisample internalMultisample, const GrCaps& caps, GrSurfaceProxy::UseAllocator useAllocator)
{
    ...
    sk_sp<GrTextureProxy> proxy
        = GrProxyProvider::MakeFullyLazyProxy(std::move(callback), format, GrRenderable::kYes, sampleCount, GrProtected::kNo, caps, useAllocator);

    return proxy;
}

GrDynamicAtlas::GrDynamicAtlas(
    GrColorType colorType, InternalMultisample internalMultisample, SkISize initialSize, int maxAtlasSize, const GrCaps& caps, RectanizerAlgorithm algorithm)
...
void GrDynamicAtlas::reset(SkISize initialSize, const GrCaps& caps)
{
    ...
    fTextureProxy = MakeLazyAtlasProxy(
        [this](GrResourceProvider* resourceProvider, const LazyAtlasDesc& desc) {
            if (!fBackingTexture) {
                fBackingTexture = resourceProvider->createTexture(fTextureProxy->backingStoreDimensions(), desc.fFormat, desc.fTextureType, desc.fRenderable,
                    desc.fSampleCnt, desc.fMipmapped, desc.fBudgeted, desc.fProtected, desc.fLabel);
            }
...
GrDynamicAtlas::Node* GrDynamicAtlas::makeNode(Node* previous, int l, int t, int r, int b)
{
    int width = r - l;
    int height = b - t;
    Rectanizer* rectanizer = (fRectanizerAlgorithm == RectanizerAlgorithm::kSkyline) ? (Rectanizer*)fNodeAllocator.make<RectanizerSkyline>(width, height)
...
{
    return { fTextureProxy, kTextureOrigin, caps.getReadSwizzle(fTextureProxy->backendFormat(), fColorType) };
}

GrSurfaceProxyView GrDynamicAtlas::writeView(const GrCaps& caps) const
{
    return { fTextureProxy, kTextureOrigin, caps.getWriteSwizzle(fTextureProxy->backendFormat(), fColorType) };
}
...
bool GrDynamicAtlas::instantiate(GrOnFlushResourceProvider* onFlushRP, sk_sp<GrTexture> backingTexture)
{
