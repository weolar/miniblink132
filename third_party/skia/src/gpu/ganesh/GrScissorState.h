...
public:
    ...
    {
        SkASSERT(logicalDimensions.fWidth <= fRTSize.fWidth && logicalDimensions.fHeight <= fRTSize.fHeight);
        if (fRect.fLeft == 0 && fRect.fTop == 0 && fRect.fRight >= logicalDimensions.fWidth && fRect.fBottom >= logicalDimensions.fHeight) {
            this->setDisabled();
            return true;
        } else {
            return false;
    ...
    bool enabled() const
    {
        ...
        return fRect.fLeft > 0 || fRect.fTop > 0 || fRect.fRight < fRTSize.fWidth || fRect.fBottom < fRTSize.fHeight;
    }
