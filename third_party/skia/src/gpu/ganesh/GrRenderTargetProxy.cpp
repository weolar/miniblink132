...
GrRenderTargetProxy::GrRenderTargetProxy(const GrCaps& caps, const GrBackendFormat& format, SkISize dimensions, int sampleCount, SkBackingFit fit,
...
GrRenderTargetProxy::GrRenderTargetProxy(LazyInstantiateCallback&& callback, const GrBackendFormat& format, SkISize dimensions, int sampleCount,
    SkBackingFit fit, skgpu::Budgeted budgeted, GrProtected isProtected, GrInternalSurfaceFlags surfaceFlags, UseAllocator useAllocator,
...
// Wrapped version
GrRenderTargetProxy::GrRenderTargetProxy(sk_sp<GrSurface> surf, UseAllocator useAllocator, WrapsVkSecondaryCB wrapsVkSecondaryCB)
...
bool GrRenderTargetProxy::instantiate(GrResourceProvider* resourceProvider)
{
    ...
    if (!this->instantiateImpl(resourceProvider, fSampleCnt, GrRenderable::kYes, skgpu::Mipmapped::kNo, nullptr)) {
        return false;
    }
...
bool GrRenderTargetProxy::canUseStencil(const GrCaps& caps) const
{
    ...
    return rt->getStencilAttachment(useMSAASurface) || rt->canAttemptStencilAttachment(useMSAASurface);
}

sk_sp<GrSurface> GrRenderTargetProxy::createSurface(GrResourceProvider* resourceProvider) const
{
    sk_sp<GrSurface> surface = this->createSurfaceImpl(resourceProvider, fSampleCnt, GrRenderable::kYes, skgpu::Mipmapped::kNo);
    if (!surface) {
        return nullptr;
    }
...
size_t GrRenderTargetProxy::onUninstantiatedGpuMemorySize() const
{
    ...
    return GrSurface::ComputeSize(this->backendFormat(), this->dimensions(), colorSamplesPerPixel, skgpu::Mipmapped::kNo, !this->priv().isExact());
}
...
void GrRenderTargetProxy::onValidateSurface(const GrSurface* surface)
{
    ...
    SkASSERT(((int)proxyFlags & kGrInternalRenderTargetFlagsMask) == ((int)surfaceFlags & kGrInternalRenderTargetFlagsMask));
