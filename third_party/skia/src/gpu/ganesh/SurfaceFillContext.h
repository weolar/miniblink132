...
public:
    ...
    OpsTask* testingOnly_PeekLastOpsTask()
    {
        return fOpsTask.get();
    }
#endif
     ...
     */
    template <SkAlphaType AlphaType> void clear(const SkIRect& rect, const SkRGBA4f<AlphaType>& color)
    {
     ...
     */
    template <SkAlphaType AlphaType> void clearAtLeast(const SkIRect& scissor, const SkRGBA4f<AlphaType>& color)
    {
        this->internalClear(&scissor, this->adjustColorAlphaType(color),
    ...
    {
        return this->arenas()->arenaAlloc();
    }
    sktext::gpu::SubRunAllocator* subRunAlloc()
    {
        return this->arenas()->subRunAlloc();
    }
...
protected:
    ...
    template <SkAlphaType AlphaType> static std::array<float, 4> ConvertColor(SkRGBA4f<AlphaType> color);

    template <SkAlphaType AlphaType> std::array<float, 4> adjustColorAlphaType(SkRGBA4f<AlphaType> color) const;
...
private:
    ...
    {
        return fWriteView.proxy()->asRenderTargetProxy()->arenas();
    }
    ...
    void internalClear(const SkIRect* scissor, std::array<float, 4> color, bool upgradePartialToFull = false);
...
template <> inline std::array<float, 4> SurfaceFillContext::ConvertColor<kPremul_SkAlphaType>(SkPMColor4f color)
{
    return color.unpremul().array();
}

template <> inline std::array<float, 4> SurfaceFillContext::ConvertColor<kUnpremul_SkAlphaType>(SkColor4f color)
{
    return color.premul().array();
}

template <SkAlphaType AlphaType> std::array<float, 4> SurfaceFillContext::adjustColorAlphaType(SkRGBA4f<AlphaType> color) const
{
