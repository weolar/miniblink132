...
std::tuple<GrSurfaceProxyView, GrColorType> SkImage_RasterPinnable::asView(
    GrRecordingContext* rContext, skgpu::Mipmapped mipmapped, GrImageTexGenPolicy policy) const
{
    if (fPinnedData) {
        ...
        if (policy != GrImageTexGenPolicy::kDraw) {
            return { skgpu::ganesh::CopyView(rContext, fPinnedData->fPinnedView, mipmapped, policy,
...
bool PinAsTexture(GrRecordingContext* rContext, SkImage* img)
{
    ...
    if (!raster->fPinnedData) {
        ...
        std::tie(data->fPinnedView, data->fPinnedColorType) = GrMakeCachedBitmapProxyView(rContext, raster->bitmap(),
            /*label=*/"ganesh_PinAsTexture", skgpu::Mipmapped::kNo);
        if (!data->fPinnedView) {
            return false;
        }
