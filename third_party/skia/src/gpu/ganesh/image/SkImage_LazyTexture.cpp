...
sk_sp<SkImage> SkImage_LazyTexture::onMakeSubset(GrDirectContext* direct, const SkIRect& subset) const
{
    auto pixels = direct ? SkImages::TextureFromImage(direct, this) : this->makeRasterImage(nullptr);
...
bool SkImage_LazyTexture::readPixelsProxy(GrDirectContext* ctx, const SkPixmap& pixmap) const
{
    ...
    GrSurfaceProxyView view = skgpu::ganesh::LockTextureProxyView(ctx, this, GrImageTexGenPolicy::kDraw, skgpu::Mipmapped::kNo);

    if (!view) {
        return false;
    }

    GrColorType ct = skgpu::ganesh::ColorTypeOfLockTextureProxy(ctx->priv().caps(), this->colorType());
