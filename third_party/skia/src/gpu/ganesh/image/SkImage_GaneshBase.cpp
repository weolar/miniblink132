...
SkImage_GaneshBase::SkImage_GaneshBase(sk_sp<GrImageContext> context, SkImageInfo info, uint32_t uniqueID)
...
bool SkImage_GaneshBase::ValidateBackendTexture(
    const GrCaps* caps, const GrBackendTexture& tex, GrColorType grCT, SkColorType ct, SkAlphaType at, sk_sp<SkColorSpace> cs)
{
    if (!tex.isValid()) {
        return false;
    }
...
bool SkImage_GaneshBase::ValidateCompressedBackendTexture(const GrCaps* caps, const GrBackendTexture& tex, SkAlphaType at)
{
...
bool SkImage_GaneshBase::getROPixels(GrDirectContext* dContext, SkBitmap* dst, CachingHint chint) const
{
...
sk_sp<SkImage> SkImage_GaneshBase::makeSubset(GrDirectContext* direct, const SkIRect& subset) const
{
...
sk_sp<SkImage> SkImage_GaneshBase::onMakeSubset(GrDirectContext* direct, const SkIRect& subset) const
{
    if (!fContext->priv().matches(direct)) {
        return nullptr;
    }
    ...
    auto copyView = GrSurfaceProxyView::Copy(direct, std::move(view), skgpu::Mipmapped::kNo, subset, SkBackingFit::kExact, isBudgeted,
    ...
    return sk_make_sp<SkImage_Ganesh>(sk_ref_sp(direct), kNeedNewImageUniqueID, std::move(copyView), this->imageInfo().colorInfo());
}

sk_sp<SkImage> SkImage_GaneshBase::onMakeSubset(skgpu::graphite::Recorder*, const SkIRect&, RequiredProperties) const
{
    SkDEBUGFAIL("Cannot convert Ganesh-backed image to Graphite");
    return nullptr;
}

sk_sp<SkImage> SkImage_GaneshBase::makeColorTypeAndColorSpace(skgpu::graphite::Recorder*, SkColorType, sk_sp<SkColorSpace>, RequiredProperties) const
{
    SkDEBUGFAIL("Cannot convert Ganesh-backed image to Graphite");
    return nullptr;
}

sk_sp<SkSurface> SkImage_GaneshBase::onMakeSurface(skgpu::graphite::Recorder*, const SkImageInfo& info) const
{
...
bool SkImage_GaneshBase::onReadPixels(
    GrDirectContext* dContext, const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB, int srcX, int srcY, CachingHint) const
{
    if (!fContext->priv().matches(dContext) || !SkImageInfoValidConversion(dstInfo, this->imageInfo())) {
        return false;
    }
...
sk_sp<SkImage> SkImage_GaneshBase::makeColorTypeAndColorSpace(GrDirectContext* dContext, SkColorType targetColorType, sk_sp<SkColorSpace> targetCS) const
{
    ...
    if (colorType == targetColorType && (SkColorSpace::Equals(colorSpace, targetCS.get()) || this->isAlphaOnly())) {
        return sk_ref_sp(const_cast<SkImage_GaneshBase*>(this));
    }
...
sk_sp<GrTextureProxy> SkImage_GaneshBase::MakePromiseImageLazyProxy(GrContextThreadSafeProxy* tsp, SkISize dimensions, const GrBackendFormat& backendFormat,
    skgpu::Mipmapped mipmapped, SkImages::PromiseImageTextureFulfillProc fulfillProc, sk_sp<skgpu::RefCntedCallback> releaseHelper)
{
    SkASSERT(tsp);
    ...
    if (mipmapped == skgpu::Mipmapped::kYes && GrTextureTypeHasRestrictedSampling(backendFormat.textureType())) {
    ...
    public:
        PromiseLazyInstantiateCallback(SkImages::PromiseImageTextureFulfillProc fulfillProc, sk_sp<skgpu::RefCntedCallback> releaseHelper)
        ...
        GrSurfaceProxy::LazyCallbackResult operator()(GrResourceProvider* resourceProvider, const GrSurfaceProxy::LazySurfaceDesc&)
        {
            ...
            fTexture = resourceProvider->wrapBackendTexture(backendTexture, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType);
            if (!fTexture) {
                return {};
            }
    ...
    return GrProxyProvider::CreatePromiseProxy(tsp, std::move(callback), backendFormat, dimensions, mipmapped);
}

namespace SkImages {
sk_sp<SkImage> SubsetTextureFrom(GrDirectContext* context, const SkImage* img, const SkIRect& subset)
{
...
sk_sp<SkImage> MakeWithFilter(GrRecordingContext* rContext, sk_sp<SkImage> src, const SkImageFilter* filter, const SkIRect& subset, const SkIRect& clipBounds,
    SkIRect* outSubset, SkIPoint* offset)
{
    ...
    sk_sp<skif::Backend> backend = skif::MakeGaneshBackend(sk_ref_sp(rContext), origin, {}, src->colorType());
