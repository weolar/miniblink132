...
GrSurfaceProxyView GrTextureGenerator::generateTexture(
    GrRecordingContext* ctx, const SkImageInfo& info, skgpu::Mipmapped mipmapped, GrImageTexGenPolicy texGenPolicy)
{
...
GrSurfaceProxyView GrExternalTextureGenerator::onGenerateTexture(
    GrRecordingContext* ctx, const SkImageInfo& info, skgpu::Mipmapped mipmapped, GrImageTexGenPolicy texGenPolicy)
{
    ...
    sk_sp<GrSurfaceProxy> proxy = ctx->priv().proxyProvider()->wrapBackendTexture(
        backendTexture, kBorrow_GrWrapOwnership, GrWrapCacheable::kYes, kRead_GrIOType, std::move(cleanupCallback));
    if (!proxy) {
        return {};
    }
