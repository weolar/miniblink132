...
{
    skgpu::Budgeted budgeted = policy == GrImageTexGenPolicy::kNew_Uncached_Budgeted ? skgpu::Budgeted::kYes : skgpu::Budgeted::kNo;
    return GrSurfaceProxyView::Copy(context, std::move(src), mipmapped, SkBackingFit::kExact, budgeted,
        /*label=*/label);
}

std::tuple<GrSurfaceProxyView, GrColorType> RasterAsView(
    GrRecordingContext* rContext, const SkImage_Raster* raster, skgpu::Mipmapped mipmapped, GrImageTexGenPolicy policy)
{
    if (policy == GrImageTexGenPolicy::kDraw) {
        ...
        return GrMakeCachedBitmapProxyView(rContext, raster->bitmap(),
            /*label=*/"TextureForImageRasterWithPolicyEqualKDraw", mipmapped);
    }
    auto budgeted = (policy == GrImageTexGenPolicy::kNew_Uncached_Unbudgeted) ? skgpu::Budgeted::kNo : skgpu::Budgeted::kYes;
    return GrMakeUncachedBitmapProxyView(rContext, raster->bitmap(), mipmapped, SkBackingFit::kExact, budgeted);
}
...
static GrSurfaceProxyView texture_proxy_view_from_planes(GrRecordingContext* ctx, const SkImage_Lazy* img, skgpu::Budgeted budgeted)
{
    ...
    for (int i = 0; i < yuvaPixmaps.numPlanes(); ++i) {
        ...
        SkBackingFit fit = yuvaPixmaps.plane(i).dimensions() == img->dimensions() ? SkBackingFit::kApprox : SkBackingFit::kExact;
        ...
        bitmap.installPixels(
            yuvaPixmaps.plane(i).info(), yuvaPixmaps.plane(i).writable_addr(), yuvaPixmaps.plane(i).rowBytes(), releaseProc, SkRef(dataStorage.get()));
        bitmap.setImmutable();

        std::tie(views[i], std::ignore) = GrMakeUncachedBitmapProxyView(ctx, bitmap, skgpu::Mipmapped::kNo, fit);
        if (!views[i]) {
            return {};
        }
    ...
    GrImageInfo info(SkColorTypeToGrColorType(img->colorType()), kPremul_SkAlphaType,
        /*color space*/ nullptr, img->dimensions());

    auto sfc = ctx->priv().makeSFC(
        info, "ImageLazy_TextureProxyViewFromPlanes", SkBackingFit::kExact, 1, skgpu::Mipmapped::kNo, GrProtected::kNo, kTopLeft_GrSurfaceOrigin, budgeted);
    if (!sfc) {
        return {};
    }
    ...
    std::unique_ptr<GrFragmentProcessor> fp = GrYUVtoRGBEffect::Make(yuvaProxies, GrSamplerState::Filter::kNearest, *ctx->priv().caps());
    ...
    fp = GrColorSpaceXformEffect::Make(std::move(fp), srcColorSpace, kOpaque_SkAlphaType, dstColorSpace, kOpaque_SkAlphaType);
    sfc->fillWithFP(std::move(fp));

    return sfc->readSurfaceView();
}

static GrSurfaceProxyView generate_picture_texture(
    GrRecordingContext* ctx, const SkImage_Picture* img, skgpu::Mipmapped mipmapped, GrImageTexGenPolicy texGenPolicy)
{
    SkASSERT(ctx);
    SkASSERT(img);

    skgpu::Budgeted budgeted = texGenPolicy == GrImageTexGenPolicy::kNew_Uncached_Unbudgeted ? skgpu::Budgeted::kNo : skgpu::Budgeted::kYes;
    auto surface = SkSurfaces::RenderTarget(ctx, budgeted, img->imageInfo(), 0, kTopLeft_GrSurfaceOrigin, img->props(), mipmapped == skgpu::Mipmapped::kYes);
    if (!surface) {
        return {};
    }
...
GrSurfaceProxyView LockTextureProxyView(GrRecordingContext* rContext, const SkImage_Lazy* img, GrImageTexGenPolicy texGenPolicy, skgpu::Mipmapped mipmapped)
{
    ...
    if (key.isValid()) {
        ...
        if (proxy) {
            ...
            if (mipmapped == skgpu::Mipmapped::kNo || view.asTextureProxy()->mipmapped() == skgpu::Mipmapped::kYes) {
                return view;
            } else {
    ...
    // 2. Ask the generator to natively create one (if it knows how)
    {
        if (img->type() == SkImage_Base::Type::kLazyPicture) {
            if (auto view = generate_picture_texture(rContext, static_cast<const SkImage_Picture*>(img), mipmapped, texGenPolicy)) {
                installKey(view);
                return view;
            }
            ...
            if (auto view = textureGen->generateTexture(rContext, img->imageInfo(), mipmapped, texGenPolicy)) {
                installKey(view);
                return view;
            }
        }
    ...
    if (mipmapped == skgpu::Mipmapped::kNo && !rContext->priv().options().fDisableGpuYUVConversion) {
    ...
    auto hint = texGenPolicy == GrImageTexGenPolicy::kDraw ? SkImage::CachingHint::kAllow_CachingHint : SkImage::CachingHint::kDisallow_CachingHint;
    if (SkBitmap bitmap; img->getROPixels(nullptr, &bitmap, hint)) {
        ...
        auto budgeted = texGenPolicy == GrImageTexGenPolicy::kNew_Uncached_Unbudgeted ? skgpu::Budgeted::kNo : skgpu::Budgeted::kYes;
        auto view = std::get<0>(GrMakeUncachedBitmapProxyView(rContext, bitmap, mipmapped, SkBackingFit::kExact, budgeted));
        if (view) {
            installKey(view);
            return view;
        }
...
static std::tuple<GrSurfaceProxyView, GrColorType> lazy_as_view(
    GrRecordingContext* context, const SkImage_Lazy* img, skgpu::Mipmapped mipmapped, GrImageTexGenPolicy policy)
{
...
std::tuple<GrSurfaceProxyView, GrColorType> AsView(GrRecordingContext* rContext, const SkImage* img, skgpu::Mipmapped mipmapped, GrImageTexGenPolicy policy)
{
    ...
    if (ib->type() == SkImage_Base::Type::kRaster) {
        return skgpu::ganesh::RasterAsView(rContext, static_cast<const SkImage_Raster*>(ib), mipmapped, policy);
...
static std::unique_ptr<GrFragmentProcessor> make_fp_from_view(GrRecordingContext* rContext, GrSurfaceProxyView view, SkAlphaType at, SkSamplingOptions sampling,
    const SkTileMode tileModes[2], const SkMatrix& m, const SkRect* subset, const SkRect* domain)
{
    if (!view) {
        return nullptr;
    }
        ...
        if (subset) {
            if (domain) {
                return GrBicubicEffect::MakeSubset(
                    std::move(view), at, m, wmx, wmy, *subset, *domain, sampling.cubic, GrBicubicEffect::Direction::kXY, *rContext->priv().caps());
            }
            return GrBicubicEffect::MakeSubset(
                std::move(view), at, m, wmx, wmy, *subset, sampling.cubic, GrBicubicEffect::Direction::kXY, *rContext->priv().caps());
        }
        return GrBicubicEffect::Make(std::move(view), at, m, wmx, wmy, sampling.cubic, GrBicubicEffect::Direction::kXY, *rContext->priv().caps());
    }
    if (sampling.isAniso()) {
...
std::unique_ptr<GrFragmentProcessor> raster_as_fp(GrRecordingContext* rContext, const SkImage_Raster* img, SkSamplingOptions sampling,
    const SkTileMode tileModes[2], const SkMatrix& m, const SkRect* subset, const SkRect* domain)
{
    auto mm = sampling.mipmap == SkMipmapMode::kNone ? skgpu::Mipmapped::kNo : skgpu::Mipmapped::kYes;
    return make_fp_from_view(rContext, std::get<0>(AsView(rContext, img, mm)), img->alphaType(), sampling, tileModes, m, subset, domain);
}

std::unique_ptr<GrFragmentProcessor> AsFragmentProcessor(GrRecordingContext* rContext, const SkImage* img, SkSamplingOptions sampling,
    const SkTileMode tileModes[2], const SkMatrix& m, const SkRect* subset, const SkRect* domain)
{
    if (!rContext) {
        return {};
    ...
    if (sampling.mipmap != SkMipmapMode::kNone && (!rContext->priv().caps()->mipmapSupport() || img->dimensions().area() <= 1)) {
        sampling = SkSamplingOptions(sampling.filter);
    }

    auto ib = static_cast<const SkImage_Base*>(img);
    if (ib->isRasterBacked()) {
        return raster_as_fp(rContext, static_cast<const SkImage_Raster*>(ib), sampling, tileModes, m, subset, domain);
    } else if (ib->isGaneshBacked()) {
        auto gb = static_cast<const SkImage_GaneshBase*>(img);
        ...
        // the planes.
        auto mm = sampling.mipmap == SkMipmapMode::kNone ? skgpu::Mipmapped::kNo : skgpu::Mipmapped::kYes;
        return MakeFragmentProcessorFromView(rContext, std::get<0>(AsView(rContext, img, mm)), img->alphaType(), sampling, tileModes, m, subset, domain);
    }
...
std::unique_ptr<GrFragmentProcessor> MakeFragmentProcessorFromView(GrRecordingContext* rContext, GrSurfaceProxyView view, SkAlphaType at,
    SkSamplingOptions sampling, const SkTileMode tileModes[2], const SkMatrix& m, const SkRect* subset, const SkRect* domain)
{
    if (!view) {
        return nullptr;
    }
...
GrSurfaceProxyView FindOrMakeCachedMipmappedView(GrRecordingContext* rContext, GrSurfaceProxyView view, uint32_t imageUniqueID)
{
    ...
    if (sk_sp<GrTextureProxy> cachedMippedView = proxyProvider->findOrCreateProxyByUniqueKey(mipmappedKey)) {
...
SkYUVAPixmapInfo::SupportedDataTypes SupportedTextureFormats(const GrImageContext& context)
{
    ...
    const auto isValid = [&context](DataType dt, int n) {
        return context.defaultBackendFormat(SkYUVAPixmapInfo::DefaultColorTypeForDataType(dt, n), GrRenderable::kNo).isValid();
    };
    for (int n = 1; n <= 4; ++n) {
...
public:
    GaneshBackend(sk_sp<GrRecordingContext> context, GrSurfaceOrigin origin, const SkSurfaceProps& surfaceProps, SkColorType colorType)
    ...
    sk_sp<SkDevice> makeDevice(SkISize size, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* props) const override
    {
        ...
        return fContext->priv().createDevice(skgpu::Budgeted::kYes, imageInfo, SkBackingFit::kApprox, 1, skgpu::Mipmapped::kNo, GrProtected::kNo, fOrigin,
            props ? *props : this->surfaceProps(), skgpu::ganesh::Device::InitContents::kUninit);
    }

    sk_sp<SkSpecialImage> makeImage(const SkIRect& subset, sk_sp<SkImage> image) const override
    {
        return SkSpecialImages::MakeFromTextureImage(fContext.get(), subset, image, this->surfaceProps());
    }
    ...
    sk_sp<SkImage> getCachedBitmap(const SkBitmap& data) const override
    {
        ...
        return sk_make_sp<SkImage_Ganesh>(fContext, data.getGenerationID(), std::move(view), data.info().colorInfo());
    }
    ...
    sk_sp<SkSpecialImage> blur(SkSize sigma, sk_sp<SkSpecialImage> input, const SkIRect& srcRect, SkTileMode tileMode, const SkIRect& dstRect) const override
    {
        ...
        auto sdc = GrBlurUtils::GaussianBlur(fContext.get(), std::move(inputView), SkColorTypeToGrColorType(input->colorType()), input->alphaType(),
            sk_ref_sp(input->getColorSpace()), dstRect.makeOffset(proxyOffset), srcRect.makeOffset(proxyOffset), sigma.width(), sigma.height(), tileMode);
        if (!sdc) {
            return nullptr;
        }

        return SkSpecialImages::MakeDeferredFromGpu(fContext.get(), SkIRect::MakeSize(dstRect.size()), kNeedNewImageUniqueID_SpecialImage,
            sdc->readSurfaceView(), sdc->colorInfo(), this->surfaceProps());
    }
#else
    ...
    {
        return fContext->priv().createDevice(skgpu::Budgeted::kYes, imageInfo, SkBackingFit::kApprox, 1, skgpu::Mipmapped::kNo, GrProtected::kNo, fOrigin,
            this->surfaceProps(), skgpu::ganesh::Device::InitContents::kUninit);
    }
