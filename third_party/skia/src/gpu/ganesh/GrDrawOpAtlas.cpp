...
std::unique_ptr<GrDrawOpAtlas> GrDrawOpAtlas::Make(GrProxyProvider* proxyProvider, const GrBackendFormat& format, SkColorType colorType, size_t bpp, int width,
    int height, int plotWidth, int plotHeight, GenerationCounter* generationCounter, AllowMultitexturing allowMultitexturing, EvictionCallback* evictor,
    std::string_view label)
{
    ...
    std::unique_ptr<GrDrawOpAtlas> atlas(
        new GrDrawOpAtlas(proxyProvider, format, colorType, bpp, width, height, plotWidth, plotHeight, generationCounter, allowMultitexturing, label));
...
GrDrawOpAtlas::GrDrawOpAtlas(GrProxyProvider* proxyProvider, const GrBackendFormat& format, SkColorType colorType, size_t bpp, int width, int height,
    int plotWidth, int plotHeight, GenerationCounter* generationCounter, AllowMultitexturing allowMultitexturing, std::string_view label)
    : fFormat(format)
    ...
    , fMaxPages(AllowMultitexturing::kYes == allowMultitexturing ? PlotLocator::kMaxMultitexturePages : 1)
    , fNumActivePages(0)
{
...
void GrDrawOpAtlas::uploadPlotToTexture(GrDeferredTextureUploadWritePixelsFn& writePixels, GrTextureProxy* proxy, Plot* plot)
{
    ...
    writePixels(proxy, rect, SkColorTypeToGrColorType(fColorType), dataPtr, fBytesPerPixel * fPlotWidth);
}

inline bool GrDrawOpAtlas::updatePlot(GrDeferredUploadTarget* target, AtlasLocator* atlasLocator, Plot* plot)
{
    ...
    if (plot->lastUploadToken() < target->tokenTracker()->nextFlushToken()) {
        ...
        AtlasToken lastUploadToken = target->addASAPUpload(
            [this, plotsp, proxy](GrDeferredTextureUploadWritePixelsFn& writePixels) { this->uploadPlotToTexture(writePixels, proxy, plotsp.get()); });
        plot->setLastUploadToken(lastUploadToken);
    }
    atlasLocator->updatePlotLocator(plot->plotLocator());
    SkDEBUGCODE(this->validate(*atlasLocator);) return true;
}

bool GrDrawOpAtlas::uploadToPage(unsigned int pageIdx, GrDeferredUploadTarget* target, int width, int height, const void* image, AtlasLocator* atlasLocator)
{
...
GrDrawOpAtlas::ErrorCode GrDrawOpAtlas::addToAtlas(
    GrResourceProvider* resourceProvider, GrDeferredUploadTarget* target, int width, int height, const void* image, AtlasLocator* atlasLocator)
{
...
bool GrDrawOpAtlas::createPages(GrProxyProvider* proxyProvider, GenerationCounter* generationCounter)
{
    ...
    for (uint32_t i = 0; i < this->maxPages(); ++i) {
        ...
        sk_sp<GrSurfaceProxy> proxy = proxyProvider->createProxy(fFormat, dims, GrRenderable::kNo, 1, skgpu::Mipmapped::kNo, SkBackingFit::kExact,
            skgpu::Budgeted::kYes, GrProtected::kNo, fLabel, GrInternalSurfaceFlags::kNone, GrSurfaceProxy::UseAllocator::kNo);
        if (!proxy) {
            return false;
        }
        ...
        for (int y = numPlotsY - 1, r = 0; y >= 0; --y, ++r) {
            for (int x = numPlotsX - 1, c = 0; x >= 0; --x, ++c) {
                ...
                currPlot->reset(new Plot(i, plotIndex, generationCounter, x, y, fPlotWidth, fPlotHeight, fColorType, fBytesPerPixel));
...
GrDrawOpAtlasConfig::GrDrawOpAtlasConfig(int maxTextureSize, size_t maxBytes)
{
    ...
    int index = maxBytes > 0 ? SkTPin<int>(SkPrevLog2(maxBytes), 0, std::size(kARGBDimensions) - 1) : 0;

    SkASSERT(kARGBDimensions[index].width() <= kMaxAtlasDim);
    SkASSERT(kARGBDimensions[index].height() <= kMaxAtlasDim);
    fARGBDimensions.set(std::min<int>(kARGBDimensions[index].width(), maxTextureSize), std::min<int>(kARGBDimensions[index].height(), maxTextureSize));
...
SkISize GrDrawOpAtlasConfig::atlasDimensions(MaskFormat type) const
{
    if (MaskFormat::kA8 == type) {
        ...
        return { std::min<int>(2 * fARGBDimensions.width(), fMaxTextureSize), std::min<int>(2 * fARGBDimensions.height(), fMaxTextureSize) };
    } else {
