...
SurfaceFillContext::SurfaceFillContext(GrRecordingContext* rContext, GrSurfaceProxyView readView, GrSurfaceProxyView writeView, const GrColorInfo& colorInfo)
...
void SurfaceFillContext::discard()
{
    ASSERT_SINGLE_OWNER
    RETURN_IF_ABANDONED
    SkDEBUGCODE(this->validate();) GR_CREATE_TRACE_MARKER_CONTEXT("SurfaceFillContext", "discard", fContext);
...
void SurfaceFillContext::resolveMSAA()
{
    ASSERT_SINGLE_OWNER
    RETURN_IF_ABANDONED
    SkDEBUGCODE(this->validate();) GR_CREATE_TRACE_MARKER_CONTEXT("SurfaceFillContext", "resolveMSAA", fContext);

    AutoCheckFlush acf(this->drawingManager());

    this->drawingManager()->newTextureResolveRenderTask(this->asSurfaceProxyRef(), GrSurfaceProxy::ResolveFlags::kMSAA, *this->caps());
}

void SurfaceFillContext::fillRectWithFP(const SkIRect& dstRect, std::unique_ptr<GrFragmentProcessor> fp)
{
    ASSERT_SINGLE_OWNER
    RETURN_IF_ABANDONED
    SkDEBUGCODE(this->validate();) GR_CREATE_TRACE_MARKER_CONTEXT("SurfaceFillContext", "fillRectWithFP", fContext);
    ...
    auto op = FillRectOp::MakeNonAARect(fContext, std::move(paint), SkMatrix::I(), SkRect::Make(dstRect));
    this->addDrawOp(std::move(op));
}

void SurfaceFillContext::fillRectWithFP(const SkIRect& dstRect, const SkMatrix& localMatrix, std::unique_ptr<GrFragmentProcessor> fp)
{
    fp = GrMatrixEffect::Make(localMatrix, std::move(fp));
    this->fillRectWithFP(dstRect, std::move(fp));
}

bool SurfaceFillContext::blitTexture(GrSurfaceProxyView view, const SkIRect& srcRect, const SkIPoint& dstPoint)
{
    SkASSERT(view.asTextureProxy());

    SkIPoint clippedDstPoint = dstPoint;
    SkIRect clippedSrcRect = srcRect;
    if (!GrClipSrcRectAndDstPoint(this->dimensions(), &clippedDstPoint, view.dimensions(), &clippedSrcRect)) {
        return false;
    }
...
{
    sk_sp<OpsTask> newOpsTask = this->drawingManager()->newOpsTask(this->writeSurfaceView(), this->arenas());
...
void SurfaceFillContext::addOp(GrOp::Owner op)
{
    ...
    this->getOpsTask()->addOp(drawingMgr, std::move(op), GrTextureResolveManager(drawingMgr), *this->caps());
}
...
void SurfaceFillContext::addDrawOp(GrOp::Owner owner)
{
    ...
    this->getOpsTask()->addDrawOp(fContext->priv().drawingManager(), std::move(owner), op->usesMSAA(), analysis, std::move(clip), dstProxyView,
        GrTextureResolveManager(this->drawingManager()), caps);
}

void SurfaceFillContext::internalClear(const SkIRect* scissor, std::array<float, 4> color, bool upgradePartialToFull)
{
    ASSERT_SINGLE_OWNER
    RETURN_IF_ABANDONED
    SkDEBUGCODE(this->validate();) GR_CREATE_TRACE_MARKER_CONTEXT("SurfaceFillContext", "clear", fContext);
    ...
    if (scissorState.enabled() && !this->caps()->performColorClearsAsDraws()) {
        if (upgradePartialToFull && (this->caps()->preferFullscreenClears() || this->caps()->shouldInitializeTextures())) {
    ...
    if (!scissorState.enabled()) {
        ...
        if (opsTask->resetForFullscreenClear(this->canDiscardPreviousOpsOnFullClear()) && !this->caps()->performColorClearsAsDraws()) {
    ...
    // as a load op.
    bool clearAsDraw = this->caps()->performColorClearsAsDraws() || (scissorState.enabled() && this->caps()->performPartialClearsAsDraws());
    if (clearAsDraw) {
        GrPaint paint;
        ClearToGrPaint(color, &paint);
        auto op = FillRectOp::MakeNonAARect(fContext, std::move(paint), SkMatrix::I(), SkRect::Make(scissorState.rect()));
        this->addDrawOp(std::move(op));
    } else {
        color = this->writeSurfaceView().swizzle().applyTo(color);
