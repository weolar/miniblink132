...
public:
    GrMockOpTarget(sk_sp<GrDirectContext> mockContext)
        : fMockContext(std::move(mockContext))
    {
        fStaticVertexBuffer = fMockContext->priv().getGpu()->createBuffer(sizeof(fStaticVertexData), GrGpuBufferType::kVertex, kDynamic_GrAccessPattern);
        fStaticIndirectBuffer
            = fMockContext->priv().getGpu()->createBuffer(sizeof(fStaticIndirectData), GrGpuBufferType::kDrawIndirect, kDynamic_GrAccessPattern);
    }
    const GrDirectContext* mockContext() const
    {
        return fMockContext.get();
    }
    ...
    {
        return GrXferBarrierFlags::kNone;
    }
    GrLoadOp colorLoadOp() const override
    {
        return GrLoadOp::kLoad;
    }
    ...
    void* makeVertexSpaceAtLeast(
        size_t vertexSize, int minVertexCount, int fallbackVertexCount, sk_sp<const GrBuffer>* buffer, int* startVertex, int* actualVertexCount) override
    {
    ...
    GrDrawIndexedIndirectWriter makeDrawIndexedIndirectSpace(int drawCount, sk_sp<const GrBuffer>* buffer, size_t* offsetInBytes) override
    {
        if (sizeof(GrDrawIndexedIndirectCommand) * drawCount > sizeof(fStaticIndirectData)) {
            SK_ABORT("FATAL: wanted %zu bytes of static indirect data; only have %zu.\n", sizeof(GrDrawIndexedIndirectCommand) * drawCount,
                sizeof(fStaticIndirectData));
        }
        *buffer = fStaticIndirectBuffer;
        *offsetInBytes = 0;
        return fStaticIndirectData;
...
#define UNIMPL(...)                                                                                                                                            \
    ...
    UNIMPL(void recordDraw(const GrGeometryProcessor*, const GrSimpleMesh[], int, const GrSurfaceProxy* const[], GrPrimitiveType))
