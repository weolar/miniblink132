...
public:
    ...
    bool isFormatAsColorTypeRenderable(GrColorType ct, const GrBackendFormat& format, int sampleCount = 1) const override
    {
        ...
        if (ct == GrColorType::kRGB_888x || ct == GrColorType::kRGB_F16F16F16x || ct == GrColorType::kRGB_101010x) {
            return false;
        }
    ...
    {
        return surface->isProtected() ? SurfaceReadPixelsSupport::kUnsupported : SurfaceReadPixelsSupport::kSupported;
    }
...
private:
    ...
    bool onAreColorTypeAndFormatCompatible(GrColorType ct, const GrBackendFormat& format) const override
    {
        ...
        SkTextureCompressionType compression = format.asMockCompressionType();
        if (compression == SkTextureCompressionType::kETC2_RGB8_UNORM || compression == SkTextureCompressionType::kBC1_RGB8_UNORM) {
