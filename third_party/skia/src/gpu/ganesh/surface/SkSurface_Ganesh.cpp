...
sk_sp<SkSurface> SkSurface_Ganesh::onNewSurface(const SkImageInfo& info)
{
    ...
    return SkSurfaces::RenderTarget(fDevice->recordingContext(), kBudgeted, info, sampleCount, origin, &this->props(),
...
sk_sp<SkImage> SkSurface_Ganesh::onNewImageSnapshot(const SkIRect* subset)
{
    ...
    if (subset || !srcView.asTextureProxy() || rtp->refsWrappedObjects()) {
        ...
        if (!subset && srcView.asTextureProxy()) {
            return SkImage_Ganesh::MakeWithVolatileSrc(sk_ref_sp(rContext), srcView, fDevice->imageInfo().colorInfo());
        }
        auto rect = subset ? *subset : SkIRect::MakeSize(srcView.dimensions());
        skgpu::Mipmapped mipmapped = srcView.mipmapped();
        srcView = GrSurfaceProxyView::Copy(rContext, std::move(srcView), mipmapped, rect, SkBackingFit::kExact, budgeted,
    ...
    return sk_make_sp<SkImage_Ganesh>(sk_ref_sp(rContext), kNeedNewImageUniqueID, std::move(srcView), info.colorInfo());
}
...
void SkSurface_Ganesh::onAsyncRescaleAndReadPixels(
    const SkImageInfo& info, SkIRect srcRect, RescaleGamma rescaleGamma, RescaleMode rescaleMode, ReadPixelsCallback callback, ReadPixelsContext context)
{
    fDevice->asyncRescaleAndReadPixels(info, srcRect, rescaleGamma, rescaleMode, callback, context);
}

void SkSurface_Ganesh::onAsyncRescaleAndReadPixelsYUV420(SkYUVColorSpace yuvColorSpace, bool readAlpha, sk_sp<SkColorSpace> dstColorSpace, SkIRect srcRect,
    SkISize dstSize, RescaleGamma rescaleGamma, RescaleMode rescaleMode, ReadPixelsCallback callback, ReadPixelsContext context)
{
    fDevice->asyncRescaleAndReadPixelsYUV420(
        yuvColorSpace, readAlpha, std::move(dstColorSpace), srcRect, dstSize, rescaleGamma, rescaleMode, callback, context);
}
...
bool SkSurface_Ganesh::onCopyOnWrite(ContentChangeMode mode)
{
    ...
    if (static_cast<SkImage_Ganesh*>(image.get())->surfaceMustCopyOnWrite(readSurfaceView.proxy())) {
...
bool SkSurface_Ganesh::onCharacterize(GrSurfaceCharacterization* characterization) const
{
    ...
    skgpu::Mipmapped mipmapped = readSurfaceView.asTextureProxy() ? readSurfaceView.asTextureProxy()->mipmapped() : skgpu::Mipmapped::kNo;
    ...
    characterization->set(direct->threadSafeProxy(), maxResourceBytes, ii, format, readSurfaceView.origin(), numSamples,
        GrSurfaceCharacterization::Textureable(SkToBool(readSurfaceView.asTextureProxy())), mipmapped, GrSurfaceCharacterization::UsesGLFBO0(usesGLFBO0),
        GrSurfaceCharacterization::VkRTSupportsInputAttachment(vkRTSupportsInputAttachment), GrSurfaceCharacterization::VulkanSecondaryCBCompatible(false),
        isProtected, this->props());
    return true;
}
...
void SkSurface_Ganesh::onDraw(SkCanvas* canvas, SkScalar x, SkScalar y, const SkSamplingOptions& sampling, const SkPaint* paint)
{
    ...
    auto tryDraw = [&] {
        ...
        const SkImageInfo info = fDevice->imageInfo();
        sk_sp<SkImage> image = sk_make_sp<SkImage_Ganesh>(sk_ref_sp(canvasContext), kNeedNewImageUniqueID, std::move(srcView), info.colorInfo());
...
bool SkSurface_Ganesh::onIsCompatible(const GrSurfaceCharacterization& characterization) const
{
    ...
    if (characterization.isTextureable()) {
        ...
        if (characterization.isMipMapped() && skgpu::Mipmapped::kNo == targetView.asTextureProxy()->mipmapped()) {
    ...
    return characterization.contextInfo() && characterization.contextInfo()->priv().matches(direct)
        && characterization.cacheMaxResourceBytes() <= maxResourceBytes && characterization.origin() == targetView.origin()
        && characterization.backendFormat() == format && characterization.width() == ii.width() && characterization.height() == ii.height()
        && characterization.colorType() == ii.colorType() && characterization.sampleCount() == numSamples
        && SkColorSpace::Equals(characterization.colorSpace(), ii.colorInfo().colorSpace()) && characterization.isProtected() == isProtected
...
static bool validate_backend_texture(const GrCaps* caps, const GrBackendTexture& tex, int sampleCnt, GrColorType grCT, bool texturable)
{
    ...
    if (!caps->areColorTypeAndFormatCompatible(grCT, backendFormat)) {
        RENDERENGINE_ABORTF("%s failed due to an invalid format and colorType combination", __func__);
        return false;
    }

    if (!caps->isFormatAsColorTypeRenderable(grCT, backendFormat, sampleCnt)) {
        RENDERENGINE_ABORTF("%s failed due to no supported rendering path for the selected "
    ...
    if (texturable && !caps->isFormatTexturable(backendFormat, tex.textureType())) {
        RENDERENGINE_ABORTF("%s failed due to no texturing support for the selected format and "
                            "colorType",
            __func__);
        return false;
    }
...
bool SkSurface_Ganesh::replaceBackendTexture(
    const GrBackendTexture& backendTexture, GrSurfaceOrigin origin, ContentChangeMode mode, TextureReleaseProc releaseProc, ReleaseContext releaseContext)
{
    ...
    if (!validate_backend_texture(rContext->priv().caps(), backendTexture, sampleCnt, grColorType,
            /* texturable= */ true)) {
        return false;
    }
    ...
    sk_sp<GrTextureProxy> proxy(rContext->priv().proxyProvider()->wrapRenderableBackendTexture(
        backendTexture, sampleCnt, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, std::move(releaseHelper)));
    if (!proxy) {
        return false;
    }

    return fDevice->replaceBackingProxy(mode, sk_ref_sp(proxy->asRenderTargetProxy()), grColorType, std::move(colorSpace), origin, this->props());
}
...
sk_sp<SkSurface> RenderTarget(GrRecordingContext* rContext, const GrSurfaceCharacterization& c, skgpu::Budgeted budgeted)
{
    ...
    auto device = rContext->priv().createDevice(budgeted, c.imageInfo(), SkBackingFit::kExact, c.sampleCount(), skgpu::Mipmapped(c.isMipMapped()),
        c.isProtected(), c.origin(), c.surfaceProps(), skgpu::ganesh::Device::InitContents::kClear);
    if (!device) {
        return nullptr;
    }
...
sk_sp<SkSurface> RenderTarget(GrRecordingContext* rContext, skgpu::Budgeted budgeted, const SkImageInfo& info, int sampleCount, GrSurfaceOrigin origin,
    const SkSurfaceProps* props, bool shouldCreateWithMips, bool isProtected)
{
    if (!rContext) {
        return nullptr;
    }
    sampleCount = std::max(1, sampleCount);
    skgpu::Mipmapped mipmapped = shouldCreateWithMips ? skgpu::Mipmapped::kYes : skgpu::Mipmapped::kNo;

    if (!rContext->priv().caps()->mipmapSupport()) {
        mipmapped = skgpu::Mipmapped::kNo;
    }

    auto device = rContext->priv().createDevice(budgeted, info, SkBackingFit::kExact, sampleCount, mipmapped, GrProtected(isProtected), origin,
        SkSurfacePropsCopyOrDefault(props), skgpu::ganesh::Device::InitContents::kClear);
    if (!device) {
        return nullptr;
    }
...
sk_sp<SkSurface> WrapBackendTexture(GrRecordingContext* rContext, const GrBackendTexture& tex, GrSurfaceOrigin origin, int sampleCnt, SkColorType colorType,
    sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* props, TextureReleaseProc textureReleaseProc, ReleaseContext releaseContext)
{
    ...
    if (!proxy) {
        ...
        RENDERENGINE_ABORTF("%s failed to wrap the texture into a renderable target "
            ...
            __func__, tex.width(), tex.height(), tex.hasMipmaps(), tex.isProtected(), static_cast<int>(tex.textureType()), retrievedTextureInfo,
            textureInfo.fTarget, textureInfo.fFormat, rContext->priv().caps()->maxRenderTargetSize());
#endif
        return nullptr;
    }

    auto device = rContext->priv().createDevice(
        grColorType, std::move(proxy), std::move(colorSpace), origin, SkSurfacePropsCopyOrDefault(props), skgpu::ganesh::Device::InitContents::kUninit);
    if (!device) {
...
sk_sp<SkSurface> WrapBackendRenderTarget(GrRecordingContext* rContext, const GrBackendRenderTarget& rt, GrSurfaceOrigin origin, SkColorType colorType,
    sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* props, RenderTargetReleaseProc relProc, ReleaseContext releaseContext)
{
