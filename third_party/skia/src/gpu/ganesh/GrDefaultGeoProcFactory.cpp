...
public:
    static GrGeometryProcessor* Make(SkArenaAlloc* arena, uint32_t gpTypeFlags, const SkPMColor4f& color, const SkMatrix& viewMatrix,
        const SkMatrix& localMatrix, bool localCoordsWillBeRead, uint8_t coverage)
    {
        return arena->make([&](void* ptr) { return new (ptr) DefaultGeoProc(gpTypeFlags, color, viewMatrix, localMatrix, coverage, localCoordsWillBeRead); });
    }
    ...
    void addToKey(const GrShaderCaps& caps, skgpu::KeyBuilder* b) const override
    {
        ...
        key = ProgramImpl::AddMatrixKeys(caps, key, fViewMatrix, usesLocalMatrix ? fLocalMatrix : SkMatrix::I());
        b->add32(key);
    }
...
private:
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            ...
            if (gp.hasVertexColor() || tweakAlpha) {
                ...
                if (gp.hasVertexColor()) {
                    ...
                    fColorUniform = uniformHandler->addUniform(nullptr, kVertex_GrShaderFlag, SkSLType::kHalf4, "Color", &colorUniformName);
            ...
            // Setup position
            WriteOutputPosition(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, gp.fInPosition.name(), gp.fViewMatrix, &fViewMatrixUniform);
            ...
            if (gp.fInLocalCoords.isInitialized()) {
                ...
                WriteLocalCoord(vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, gp.fInPosition.asShaderVar(), gp.fLocalMatrix, &fLocalMatrixUniform);
            }
            ...
            if (gp.hasVertexCoverage() && !tweakAlpha) {
                ...
                fCoverageUniform = uniformHandler->addUniform(nullptr, kFragment_GrShaderFlag, SkSLType::kHalf, "Coverage", &fragCoverage);
    ...
    bool hasVertexCoverage() const
    {
        return fInCoverage.isInitialized();
    }

    DefaultGeoProc(
        uint32_t gpTypeFlags, const SkPMColor4f& color, const SkMatrix& viewMatrix, const SkMatrix& localMatrix, uint8_t coverage, bool localCoordsWillBeRead)
...
GrGeometryProcessor* DefaultGeoProc::TestCreate(GrProcessorTestData* d)
{
    ...
    if (d->fRandom->nextBool()) {
        ...
        if (d->fRandom->nextBool()) {
            flags |= (d->fRandom->nextBool()) ? kCoverageAttributeTweak_GPFlag : kCoverageAttributeUnclamped_GPFlag;
        }
    }
    if (d->fRandom->nextBool()) {
    ...
    return DefaultGeoProc::Make(d->allocator(), flags, SkPMColor4f::FromBytes_RGBA(color), viewMtx, localMtx, readsLocalCoords, coverage);
}
#endif

GrGeometryProcessor* GrDefaultGeoProcFactory::Make(
    SkArenaAlloc* arena, const Color& color, const Coverage& coverage, const LocalCoords& localCoords, const SkMatrix& viewMatrix)
{
    uint32_t flags = 0;
    ...
    return DefaultGeoProc::Make(
        arena, flags, color.fColor, viewMatrix, localCoords.fMatrix ? *localCoords.fMatrix : SkMatrix::I(), localCoordsWillBeRead, inCoverage);
}
