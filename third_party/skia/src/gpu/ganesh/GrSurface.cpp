...
size_t GrSurface::ComputeSize(const GrBackendFormat& format, SkISize dimensions, int colorSamplesPerPixel, skgpu::Mipmapped mipmapped, bool binSize)
{
    ...
    if (compressionType != SkTextureCompressionType::kNone) {
        colorSize = SkCompressedFormatDataSize(compressionType, dimensions, mipmapped == skgpu::Mipmapped::kYes);
    } else {
        colorSize = (size_t)dimensions.width() * dimensions.height() * GrBackendFormatBytesPerPixel(format);
    }
...
void GrSurface::setRelease(sk_sp<skgpu::RefCntedCallback> releaseHelper)
{
    ...
    fReleaseHelper.reset(new RefCntedReleaseProc(std::move(releaseHelper), sk_ref_sp(this->getContext())));
    this->onSetRelease(fReleaseHelper);
}

GrSurface::RefCntedReleaseProc::RefCntedReleaseProc(sk_sp<skgpu::RefCntedCallback> callback, sk_sp<GrDirectContext> directContext)
