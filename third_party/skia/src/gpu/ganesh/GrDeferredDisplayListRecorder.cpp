...
bool GrDeferredDisplayListRecorder::init()
{
    ...
    fLazyProxyData = sk_sp<GrDeferredDisplayList::LazyProxyData>(new GrDeferredDisplayList::LazyProxyData);
    ...
    static constexpr GrProxyProvider::TextureInfo kTextureInfo { skgpu::Mipmapped::kNo, GrTextureType::k2D };
    ...
    fTargetProxy = proxyProvider->createLazyRenderTargetProxy(
        [lazyProxyData = fLazyProxyData](GrResourceProvider* resourceProvider, const GrSurfaceProxy::LazySurfaceDesc&) {
        ...
        fCharacterization.backendFormat(), fCharacterization.dimensions(), fCharacterization.sampleCount(), surfaceFlags, optionalTextureInfo,
        GrMipmapStatus::kNotAllocated, SkBackingFit::kExact, skgpu::Budgeted::kYes, fCharacterization.isProtected(),
    ...
    auto device = fContext->priv().createDevice(grColorType, fTargetProxy, fCharacterization.refColorSpace(), fCharacterization.origin(),
        fCharacterization.surfaceProps(), skgpu::ganesh::Device::InitContents::kUninit);
    if (!device) {
        return false;
    }
...
sk_sp<GrDeferredDisplayList> GrDeferredDisplayListRecorder::detach()
{
    ...
    auto ddl = sk_sp<GrDeferredDisplayList>(new GrDeferredDisplayList(fCharacterization, std::move(fTargetProxy), std::move(fLazyProxyData)));
