...
std::unique_ptr<GrMemoryPool> GrMemoryPool::Make(size_t preallocSize, size_t minAllocSize)
{
    ...
    preallocSize = SkTPin(preallocSize, kMinAllocationSize, (size_t)SkBlockAllocator::kMaxAllocationSize);
    minAllocSize = SkTPin(minAllocSize, kMinAllocationSize, (size_t)SkBlockAllocator::kMaxAllocationSize);
...
GrMemoryPool::GrMemoryPool(size_t preallocSize, size_t minAllocSize)
    : fAllocator(SkBlockAllocator::GrowthPolicy::kFixed, minAllocSize, preallocSize - offsetof(GrMemoryPool, fAllocator) - sizeof(SkBlockAllocator))
{
...
void* GrMemoryPool::allocate(size_t size)
{
    ...
    SkDEBUGCODE(this->validate();)
        ...
        SkBlockAllocator::ByteRange alloc
        = fAllocator.allocate<kAlignment, sizeof(Header)>(size);
