...
{
    return test_matrix(random, true, true);
}
...
const SkMatrix& TestMatrixInvertible(SkRandom* random)
{
    return test_matrix(random, true, false);
}
const SkMatrix& TestMatrixPerspective(SkRandom* random)
{
    return test_matrix(random, false, true);
}
...
const SkPath& TestPath(SkRandom* random)
{
    ...
    if (!gOnce) {
        ...
        // cubic
        gPath[3] = SkPathBuilder().moveTo(0.f, 0.f).cubicTo(10.f, 10.f, 20.f, 20.f, 30.f, 30.f).detach();
        // all three
        gPath[4] = SkPathBuilder()
                       ...
                       .detach();
        // convex
        gPath[5] = SkPathBuilder().moveTo(0.0f, 0.0f).lineTo(10.0f, 0.0f).lineTo(10.0f, 10.0f).lineTo(0.0f, 10.0f).close().detach();
        // concave
        gPath[6] = SkPathBuilder().moveTo(0.0f, 0.0f).lineTo(5.0f, 5.0f).lineTo(10.0f, 0.0f).lineTo(10.0f, 10.0f).lineTo(0.0f, 10.0f).close().detach();
    }
...
const SkPath& TestPathConvex(SkRandom* random)
{
    ...
    if (!gOnce) {
        gOnce = true;
        // narrow rect
        gPath[0] = SkPath::Polygon({ { -1.5f, -50.0f }, { -1.5f, -50.0f }, { 1.5f, -50.0f }, { 1.5f, 50.0f }, { -1.5f, 50.0f } }, false);
        // degenerate
        gPath[1] = SkPath::Polygon({ { -0.025f, -0.025f }, { -0.025f, -0.025f }, { 0.025f, -0.025f }, { 0.025f, 0.025f }, { -0.025f, 0.025f } }, false);
        // clipped triangle
        gPath[2] = SkPath::Polygon(
            { { -10.0f, -50.0f }, { -10.0f, -50.0f }, { 10.0f, -50.0f }, { 50.0f, 31.0f }, { 40.0f, 50.0f }, { -40.0f, 50.0f }, { -50.0f, 31.0f } }, false);
...
TestDashPathEffect::TestDashPathEffect(const SkScalar* intervals, int count, SkScalar phase)
{
    ...
    SkDashPath::CalcDashParameters(phase, intervals, count, &fInitialDashLength, &fInitialDashIndex, &fIntervalLength, &fPhase);
}

bool TestDashPathEffect::onFilterPath(SkPath* dst, const SkPath& src, SkStrokeRec* rec, const SkRect* cullRect, const SkMatrix&) const
{
    return SkDashPath::InternalFilter(dst, src, rec, cullRect, fIntervals.get(), fCount, fInitialDashLength, fInitialDashIndex, fIntervalLength, fPhase);
}
...
sk_sp<GrColorSpaceXform> TestColorXform(SkRandom* random)
{
    ...
    if (!gOnce) {
        ...
        // No gamut change
        gXforms[0] = nullptr;
        gXforms[1] = GrColorSpaceXform::Make(srgb.get(), kPremul_SkAlphaType, spin.get(), kPremul_SkAlphaType);
        gXforms[2] = GrColorSpaceXform::Make(spin.get(), kPremul_SkAlphaType, srgb.get(), kPremul_SkAlphaType);
    }
    return gXforms[random->nextULessThan(static_cast<uint32_t>(std::size(gXforms)))];
}

TestAsFPArgs::TestAsFPArgs(GrProcessorTestData* d)
    : fColorInfoStorage(std::make_unique<GrColorInfo>(GrColorType::kRGBA_8888, kPremul_SkAlphaType, TestColorSpace(d->fRandom)))
...
GrColor RandomColor(SkRandom* random)
{
    ...
    enum ColorMode { kAllOnes_ColorMode, kAllZeros_ColorMode, kAlphaOne_ColorMode, kRandom_ColorMode, kLast_ColorMode = kRandom_ColorMode };
...
{
    enum CoverageMode { kZero_CoverageMode, kAllOnes_CoverageMode, kRandom_CoverageMode, kLast_CoverageMode = kRandom_CoverageMode };
