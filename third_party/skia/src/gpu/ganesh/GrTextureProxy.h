...
public:
    ...
    {
        SkASSERT((skgpu::Mipmapped::kNo == fMipmapped) == (GrMipmapStatus::kNotAllocated == fMipmapStatus));
    ...
    {
#ifdef SK_DEBUG
        if (this->isInstantiated() && fUniqueKey.isValid() && fSyncTargetKey && fCreatingProvider == GrDDLProvider::kNo) {
    ...
    GrTextureProxy(const GrBackendFormat&, SkISize, skgpu::Mipmapped, GrMipmapStatus, SkBackingFit, skgpu::Budgeted, skgpu::Protected, GrInternalSurfaceFlags,
    ...
    GrTextureProxy(LazyInstantiateCallback&&, const GrBackendFormat&, SkISize, skgpu::Mipmapped, GrMipmapStatus, SkBackingFit, skgpu::Budgeted,
        skgpu::Protected, GrInternalSurfaceFlags, UseAllocator, GrDDLProvider creatingProvider, std::string_view label);
...
private:
    ...
    SkDEBUGCODE(const GrMipmapStatus fInitialMipmapStatus;)
        ...
        bool fSyncTargetKey
        = true; // Should target's unique key be sync'ed with ours.
