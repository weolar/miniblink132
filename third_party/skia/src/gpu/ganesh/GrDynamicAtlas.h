...
public:
    ...
    static sk_sp<GrTextureProxy> MakeLazyAtlasProxy(
        LazyInstantiateAtlasCallback&&, GrColorType colorType, InternalMultisample, const GrCaps&, GrSurfaceProxy::UseAllocator);

    enum class RectanizerAlgorithm { kSkyline, kPow2 };

    GrDynamicAtlas(
        GrColorType colorType, InternalMultisample, SkISize initialSize, int maxAtlasSize, const GrCaps&, RectanizerAlgorithm = RectanizerAlgorithm::kSkyline);
    ...
    {
        return fTextureProxy->isInstantiated();
    }
    ...
    [[nodiscard]] bool instantiate(GrOnFlushResourceProvider*, sk_sp<GrTexture> backingTexture = nullptr);
