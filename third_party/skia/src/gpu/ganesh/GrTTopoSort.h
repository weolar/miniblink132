...
#ifdef SK_DEBUG
template <typename T, typename Traits = T> void GrTTopoSort_CheckAllUnmarked(SkSpan<const sk_sp<T>> graph)
{
...
template <typename T, typename Traits = T> void GrTTopoSort_CleanExit(SkSpan<const sk_sp<T>> graph, uint32_t offset)
{
...
template <typename T, typename Traits = T> bool GrTTopoSort_Visit(T* node, uint32_t* counter)
{
...
template <typename T, typename Traits = T> bool GrTTopoSort(SkSpan<sk_sp<T>> graph, uint32_t offset = 0)
{
    ...
    for (uint32_t i = 0; i < (uint32_t)graph.size(); ++i) {
        for (uint32_t correctIndex = Traits::GetIndex(graph[i].get()) - offset; correctIndex != i; correctIndex = Traits::GetIndex(graph[i].get()) - offset) {
