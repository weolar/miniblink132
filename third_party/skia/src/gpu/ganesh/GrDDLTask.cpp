...
GrDDLTask::GrDDLTask(GrDrawingManager* drawingMgr, sk_sp<GrRenderTargetProxy> ddlTarget, sk_sp<const GrDeferredDisplayList> ddl)
    : fDDL(std::move(ddl))
...
GrRenderTask::ExpectedOutcome GrDDLTask::onMakeClosed(GrRecordingContext*, SkIRect* targetUpdateBounds)
{
    SkASSERT(0);
...
void GrDDLTask::dump(const SkString& label, SkString indent, bool printDependencies, bool close) const
{
