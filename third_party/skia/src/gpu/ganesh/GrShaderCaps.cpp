...
void GrShaderCaps::dumpJSON(SkJSONWriter* writer) const
{
    ...
    writer->appendBool("Can use void-typed expressions in a sequence expression", fCanUseVoidInSequenceExpressions);
    ...
    writer->appendBool("Must force negated ldexp param to multiply", fMustForceNegatedLdexpParamToMultiply);
    ...
    writer->appendBool("Must guard division even after explicit zero check", fMustGuardDivisionEvenAfterExplicitZeroCheck);
    ...
    writer->appendBool("Don't add default precision statement for samplerExternalOES", fNoDefaultPrecisionForExternalSamplers);
