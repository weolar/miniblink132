...
struct DPoint {
    ...
    double distance(DPoint p) const
    {
...
public:
    ...
    */
    ...
    {
        return { fMat[0] * src.fX + fMat[1] * src.fY + fMat[2], fMat[3] * src.fX + fMat[4] * src.fY + fMat[5] };
    }
...
static inline bool between_closed_open(double a, double b, double c, double tolerance = 0.0, bool xformToleranceToX = false)
{
...
// returns true if a >= min(b,c) && a <= max(b,c)
static inline bool between_closed(double a, double b, double c, double tolerance = 0.0, bool xformToleranceToX = false)
{
    SkASSERT(tolerance >= 0.0);
    double tolB = tolerance;
    double tolC = tolerance;
...
static inline bool nearly_equal(double x, double y, double tolerance = kNearlyZero, bool xformToleranceToX = false)
{
    SkASSERT(tolerance >= 0.0);
    if (xformToleranceToX) {
...
{
    return nearly_zero((pts[1].fY - pts[0].fY) * (pts[1].fX - pts[2].fX) - (pts[1].fY - pts[2].fY) * (pts[1].fX - pts[0].fX), kCloseSqd);
}
...
void PathSegment::init()
{
    ...
    if (fType == PathSegment::kLine) {
        ...
        if (SkTAbs(hypotenuse) < 1.0e-100) {
            ...
            fXformMatrix.setAffine(cosTheta, sinTheta, -(cosTheta * p0x) - (sinTheta * p0y), -sinTheta, cosTheta, (sinTheta * p0x) - (cosTheta * p0y));
        }
    } else {
        ...
        const double c = (p0xSqd * p2ySqd) - (4.0 * p01xProd * b12yProd) - (2.0 * p02xProd * p02yProd) + (4.0 * p02xProd * p1ySqd) + (4.0 * p1xSqd * p02yProd)
            - (4.0 * b12xProd * p01yProd) + (p2xSqd * p0ySqd);
        const double g = (p0x * p02yProd) - (2.0 * p0x * p1ySqd) + (2.0 * p0x * b12yProd) - (p0x * p2ySqd) + (2.0 * p1x * p01yProd) - (4.0 * p1x * p02yProd)
            + (2.0 * p1x * b12yProd) - (p2x * p0ySqd) + (2.0 * p2x * p01yProd) + (p2x * p02yProd) - (2.0 * p2x * p1ySqd);
        const double f = -((p0xSqd * p2y) - (2.0 * p01xProd * p1y) - (2.0 * p01xProd * p2y) - (p02xProd * p0y) + (4.0 * p02xProd * p1y) - (p02xProd * p2y)
            + (2.0 * p1xSqd * p0y) + (2.0 * p1xSqd * p2y) - (2.0 * b12xProd * p0y) - (2.0 * b12xProd * p1y) + (p2xSqd * p0y));
        ...
        fXformMatrix.setAffine(lambda_cosTheta, -lambda_sinTheta, lambda * x0, lambda_sinTheta, lambda_cosTheta, lambda * y0);
    }
...
{
    if (SkPointPriv::DistanceToSqd(pts[0], pts[1]) < kCloseSqd || SkPointPriv::DistanceToSqd(pts[1], pts[2]) < kCloseSqd || is_colinear(pts)) {
        if (pts[0] != pts[2]) {
            SkPoint line_pts[2];
            line_pts[0] = pts[0];
            line_pts[1] = pts[2];
...
struct RowData {
    ...
    enum IntersectionType { kNoIntersection, kVerticalLine, kTangentLine, kTwoPointsIntersect } fIntersectionType;
...
void precomputation_for_row(RowData* rowData, const PathSegment& segment, const SkPoint& pointLeft, const SkPoint& pointRight)
{
    ...
    if ((rowData->fScanlineXDirection == 1 && (segment.fPts[0].fY == pointLeft.fY || segment.fPts[2].fY == pointLeft.fY)) && nearly_zero(c, tol)) {
...
SegSide calculate_side_of_quad(const PathSegment& segment, const SkPoint& point, const DPoint& xFormPt, const RowData& rowData)
{
    ...
    if (RowData::kVerticalLine == rowData.fIntersectionType) {
        ...
        if (rowData.fScanlineXDirection == 1) {
            if ((rowData.fQuadXDirection == -1 && segment.fPts[0].fY <= point.fY && nearly_equal(segment.fP0T.fX, p1, segment.fNearlyZeroScaled, true))
                || (rowData.fQuadXDirection == 1 && segment.fPts[2].fY <= point.fY && nearly_equal(segment.fP2T.fX, p1, segment.fNearlyZeroScaled, true))) {
                includeP1 = false;
            }
            if ((rowData.fQuadXDirection == -1 && segment.fPts[2].fY <= point.fY && nearly_equal(segment.fP2T.fX, p2, segment.fNearlyZeroScaled, true))
                || (rowData.fQuadXDirection == 1 && segment.fPts[0].fY <= point.fY && nearly_equal(segment.fP0T.fX, p2, segment.fNearlyZeroScaled, true))) {
                includeP2 = false;
            }
        }

        if (includeP1 && between_closed(p1, segment.fP0T.fX, segment.fP2T.fX, segment.fNearlyZeroScaled, true)) {
            side = (SegSide)(signP1 * rowData.fQuadXDirection);
        }
        if (includeP2 && between_closed(p2, segment.fP0T.fX, segment.fP2T.fX, segment.fNearlyZeroScaled, true)) {
...
static float distance_to_segment(const SkPoint& point, const PathSegment& segment, const RowData& rowData, SegSide* side)
{
    ...
    if (segment.fType == PathSegment::kLine) {
        ...
        if (between_closed(xformPt.fX, segment.fP0T.fX, segment.fP2T.fX)) {
            ...
            result = (float)((xformPt.fX - segment.fP2T.fX) * (xformPt.fX - segment.fP2T.fX) + xformPt.fY * xformPt.fY);
        }
    ...
    for (int a = 0; a < count; ++a) {
        ...
        for (int row = startRow; row < endRow; ++row) {
            ...
            for (int col = startColumn; col < endColumn; ++col) {
                ...
                int dilation = distSq < 1.5f * 1.5f ? 1 : distSq < 2.5f * 2.5f ? 2 : distSq < 3.5f * 3.5f ? 3 : SK_DistanceFieldPad;
                if (dilation < SK_DistanceFieldPad && !segBB.roundOut().makeOutset(dilation, dilation).contains(col, row)) {
                    continue;
                }
...
bool GrGenerateDistanceFieldFromPath(unsigned char* distanceField, const SkPath& path, const SkMatrix& drawMatrix, int width, int height, size_t rowBytes)
{
    ...
    SkASSERT(expectPathBounds.isEmpty() || expectPathBounds.contains(pathBounds.fLeft, pathBounds.fTop));
    SkASSERT(expectPathBounds.isEmpty() || pathBounds.isEmpty() || expectPathBounds.contains(pathBounds));
