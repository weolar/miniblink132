...
public:
    ...
    void newTransferFromRenderTask(const sk_sp<GrSurfaceProxy>& srcProxy, const SkIRect& srcRect, GrColorType surfaceColorType, GrColorType dstColorType,
    ...
    sk_sp<GrRenderTask> newCopyRenderTask(
        sk_sp<GrSurfaceProxy> dst, SkIRect dstRect, const sk_sp<GrSurfaceProxy>& src, SkIRect srcRect, GrSamplerState::Filter filter, GrSurfaceOrigin);
    ...
    GrSemaphoresSubmitted flushSurfaces(
        SkSpan<GrSurfaceProxy*>, SkSurfaces::BackendSurfaceAccess, const GrFlushInfo&, const skgpu::MutableTextureState* newState);
...
private:
    GrDrawingManager(GrRecordingContext*, const PathRendererChain::Options&, bool reduceOpsTaskSplitting);
