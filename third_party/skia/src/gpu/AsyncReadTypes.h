 ...
 */
template <typename T, typename IDType> class TClientMappedBufferManager {
public:
    /**
...
template <typename T, typename IDType, typename TransferResultType> class TAsyncReadResult : public SkImage::AsyncReadResult {
public:
    ...
    {
        return fPlanes[i].data();
    }
    size_t rowBytes(int i) const override
    {
        return fPlanes[i].rowBytes();
    }
...
private:
    ...
    public:
        Plane(sk_sp<T> buffer, size_t rowBytes)
            : fMappedBuffer(std::move(buffer))
            , fRowBytes(rowBytes)
        {
        }
        Plane(sk_sp<SkData> data, size_t rowBytes)
            : fData(std::move(data))
            , fRowBytes(rowBytes)
        {
        }
        ...
        {
            if (fMappedBuffer) {
                TClientMappedBufferManager<T, IDType>::BufferFinishedMessageBus::Post({ std::move(fMappedBuffer), intendedRecipient });
            }
        }
