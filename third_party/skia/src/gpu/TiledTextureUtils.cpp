...
SkIRect determine_clipped_src_rect(
    SkIRect clippedSrcIRect, const SkMatrix& viewMatrix, const SkMatrix& srcToDstRect, const SkISize& imageDimensions, const SkRect* srcRectPtr)
{
...
int draw_tiled_bitmap(SkCanvas* canvas, const SkBitmap& bitmap, int tileSize, const SkMatrix& srcToDst, const SkRect& srcRect, const SkIRect& clippedSrcIRect,
    const SkPaint* paint, SkCanvas::QuadAAFlags origAAFlags, SkCanvas::SrcRectConstraint constraint, SkSamplingOptions sampling)
{
    if (sampling.isAniso()) {
    ...
    for (int x = 0; x <= nx; x++) {
        for (int y = 0; y <= ny; y++) {
            SkRect tileR;
            tileR.setLTRB(SkIntToScalar(x * tileSize), SkIntToScalar(y * tileSize), SkIntToScalar((x + 1) * tileSize), SkIntToScalar((y + 1) * tileSize));
            ...
            if (sampling.filter != SkFilterMode::kNearest || sampling.useCubic) {
                ...
                skgpu::TiledTextureUtils::ClampedOutsetWithOffset(&iTileR, outset, &offset, iClampRect);
            }
            ...
            if (SkBitmap subsetBmp; bitmap.extractSubset(&subsetBmp, iTileR)) {
                sk_sp<SkImage> image = SkMakeImageFromRasterBitmap(subsetBmp, kNever_SkCopyPixelsMode);
                if (!image) {
                    continue;
                }
                ...
                if (tileR.fBottom >= srcRect.fBottom && (origAAFlags & SkCanvas::kBottom_QuadAAFlag)) {
                ...
                imgSet.push_back(SkCanvas::ImageSetEntry(std::move(image), tileR, rectToDraw,
                    /* matrixIndex= */ -1,
                    /* alpha= */ 1.0f, aaFlags,
                    /* hasClip= */ false));
    ...
    canvas->experimental_DrawEdgeAAImageSet(imgSet.data(), imgSet.size(),
        /* dstClips= */ nullptr,
        /* preViewMatrices= */ nullptr, sampling, paint, constraint);
...
bool TiledTextureUtils::ShouldTileImage(SkIRect conservativeClipBounds, const SkISize& imageSize, const SkMatrix& ctm, const SkMatrix& srcToDst,
    const SkRect* src, int maxTileSize, size_t cacheSize, int* tileSize, SkIRect* clippedSubset)
{
    // if it's larger than the max tile size, then we have no choice but tiling.
    if (imageSize.width() > maxTileSize || imageSize.height() > maxTileSize) {
        *clippedSubset = determine_clipped_src_rect(conservativeClipBounds, ctm, srcToDst, imageSize, src);
    ...
    *tileSize = kBmpSmallTileSize; // already know whole bitmap fits in one max sized tile.
    size_t usedTileBytes
        = get_tile_count(*clippedSubset, kBmpSmallTileSize) * kBmpSmallTileSize * kBmpSmallTileSize * sizeof(SkPMColor); // assume 32bit pixels;
 ...
 */
TiledTextureUtils::ImageDrawMode TiledTextureUtils::OptimizeSampleArea(const SkISize& imageSize, const SkRect& origSrcRect, const SkRect& origDstRect,
    const SkPoint dstClip[4], SkRect* outSrcRect, SkRect* outDstRect, SkMatrix* outSrcToDst)
{
...
bool TiledTextureUtils::CanDisableMipmap(const SkMatrix& viewM, const SkMatrix& localM, bool sharpenMipmappedTextures)
{
    SkMatrix matrix;
...
void TiledTextureUtils::ClampedOutsetWithOffset(SkIRect* iRect, int outset, SkPoint* offset, const SkIRect& clamp)
{
...
std::tuple<bool, size_t> TiledTextureUtils::DrawAsTiledImageRect(SkCanvas* canvas, const SkImage* image, const SkRect& srcRect, const SkRect& dstRect,
    SkCanvas::QuadAAFlags aaFlags, const SkSamplingOptions& origSampling, const SkPaint* paint, SkCanvas::SrcRectConstraint constraint, bool sharpenMM,
    size_t cacheSize, size_t maxTextureSize)
{
    ...
    if (!image->isTextureBacked()) {
        ...
        SkMatrix srcToDst;
        ImageDrawMode mode
            = OptimizeSampleArea(SkISize::Make(image->width(), image->height()), srcRect, dstRect, /* dstClip= */ nullptr, &src, &dst, &srcToDst);
        ...
        if (sampling.mipmap != SkMipmapMode::kNone && CanDisableMipmap(localToDevice, srcToDst, sharpenMM)) {
        ...
        if (ShouldTileImage(clipRect, image->dimensions(), localToDevice, srcToDst, &src, maxTileSize, cacheSize, &tileSize, &clippedSubset)) {
            ...
            if (SkBitmap bm; as_IB(image)->getROPixels(nullptr, &bm)) {
                size_t tiles = draw_tiled_bitmap(canvas, bm, tileSize, srcToDst, src, clippedSubset, paint, aaFlags, constraint, sampling);
                return { true, tiles };
            }
        }
    }
