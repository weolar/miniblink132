...
static void apply_kernel_in_y(float* results, int numSteps, float firstX, float circleR, int halfKernelSize, const float* summedHalfKernelTable)
{
    ...
    for (int i = 0; i < numSteps; ++i, x += 1.0f) {
        ...
        if (y < 0) {
            ...
            float yFrac = y - yInt;
            results[i] = (1.0f - yFrac) * summedHalfKernelTable[yInt] + yFrac * summedHalfKernelTable[yInt + 1];
        }
    }
}
...
static uint8_t eval_H(
    int x, int y, const std::vector<float>& topVec, const float* kernel, int kernelSize, const uint8_t* integral, int integralSize, float sixSigma)
{
...
SkBitmap CreateRRectBlurMask(const SkRRect& rrectToDraw, const SkISize& dimensions, float sigma)
{
    ...
    for (int y = 0; y < halfHeightPlus1; ++y) {
        ...
        for (int x = 0; x < halfWidthPlus1; ++x) {
            scanline[x] = eval_H(x, y, topVec, kernel.get(), kernelSize, integral.getAddr8(0, 0), integral.width(), 6.0f * sigma);
