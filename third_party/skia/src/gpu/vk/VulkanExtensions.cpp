...
static int find_info(const TArray<VulkanExtensions::Info>& infos, const char ext[])
{
    ...
    int idx = SkTSearch<VulkanExtensions::Info, SkString, VulkanExtensions::Info::Less>(
        &infos.front(), infos.size(), extensionStr, sizeof(VulkanExtensions::Info), less);
    return idx;
}
...
void VulkanExtensions::init(VulkanGetProc getProc, VkInstance instance, VkPhysicalDevice physDev, uint32_t instanceExtensionCount,
    const char* const* instanceExtensions, uint32_t deviceExtensionCount, const char* const* deviceExtensions)
{
...
void VulkanExtensions::getSpecVersions(const VulkanGetProc& getProc, VkInstance instance, VkPhysicalDevice physDevice)
{
