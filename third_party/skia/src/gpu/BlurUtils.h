...
{
    return SkShaderBlurAlgorithm::KernelWidth(radius);
}
...
{
    return SkBlurEngine::SigmaToRadius(sigma);
}
...
inline void Compute2DBlurKernel(SkSize sigma, SkISize radius, std::array<SkV4, kMaxBlurSamples / 4>& kernel)
{
    SkShaderBlurAlgorithm::Compute2DBlurKernel(sigma, radius, kernel);
}
...
inline void Compute1DBlurLinearKernel(float sigma, int radius, std::array<SkV4, kMaxBlurSamples / 2>& offsetsAndKernel)
{
