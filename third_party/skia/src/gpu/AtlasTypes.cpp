...
Plot::Plot(
    int pageIndex, int plotIndex, AtlasGenerationCounter* generationCounter, int offX, int offY, int width, int height, SkColorType colorType, size_t bpp)
...
void* Plot::dataAt(const AtlasLocator& atlasLocator)
{
    ...
    SkASSERT(topLeft.fX >= fOffset.fX && topLeft.fX < fOffset.fX + fWidth && topLeft.fY >= fOffset.fY && topLeft.fY < fOffset.fY + fHeight);
...
SkIPoint Plot::prepForRender(const AtlasLocator& al, SkAutoPixmapStorage* pixmap)
{
    ...
    pixmap->reset(SkImageInfo::Make(fWidth, fHeight, fColorType, kOpaque_SkAlphaType), fData, fBytesPerPixel * fWidth);
