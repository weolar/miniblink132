...
sk_sp<SkAnimatedImage> SkAnimatedImage::Make(
    std::unique_ptr<SkAndroidCodec> codec, const SkImageInfo& requestedInfo, SkIRect cropRect, sk_sp<SkPicture> postProcess)
{
    if (!codec) {
        return nullptr;
    }
    ...
    auto image = sk_sp<SkAnimatedImage>(new SkAnimatedImage(std::move(codec), requestedInfo, cropRect, std::move(postProcess)));
...
SkAnimatedImage::SkAnimatedImage(std::unique_ptr<SkAndroidCodec> codec, const SkImageInfo& requestedInfo, SkIRect cropRect, sk_sp<SkPicture> postProcess)
...
int SkAnimatedImage::computeNextFrame(int current, bool* animationEnded)
{
    ...
    *animationEnded = false;
    ...
    if (frameToDecode == fFrameCount - 1) {
        ...
        if (fRepetitionCount != SkCodec::kRepetitionCountInfinite && fRepetitionsCompleted > fRepetitionCount) {
            *animationEnded = true;
        }
...
int SkAnimatedImage::decodeNextFrame()
{
    ...
    if (frameInfo.fRequiredFrame == SkCodec::kNoFrame) {
        if (is_restore_previous(frameInfo.fDisposalMethod)) {
            ...
            // future.
            if (fDecodingFrame.fIndex != SkCodec::kNoFrame && !is_restore_previous(fDecodingFrame.fDisposalMethod)) {
                using std::swap;
    ...
    auto alphaType = kOpaque_SkAlphaType == frameInfo.fAlphaType ? kOpaque_SkAlphaType : kPremul_SkAlphaType;
