...
public:
    MergeSampleUsageVisitor(const Context& context, const Variable& child, bool writesToSampleCoords)
        : fContext(context)
        , fChild(child)
        , fWritesToSampleCoords(writesToSampleCoords)
    {
    }
    ...
    int elidedSampleCoordCount() const
    {
...
protected:
    ...
    {
        fMainCoordsParam = pe.is<FunctionDefinition>() ? pe.as<FunctionDefinition>().declaration().getMainCoordsParameter() : nullptr;
    ...
    bool visitExpression(const Expression& e) override
    {
        ...
        case ExpressionKind::kChildCall: {
            ...
            if (&cc.child() == &fChild) {
                ...
                if (maybeCoords->type().matches(*fContext.fTypes.fFloat2)) {
                    ...
                    if (!fWritesToSampleCoords && maybeCoords->is<VariableReference>() && maybeCoords->as<VariableReference>().variable() == fMainCoordsParam) {
        ...
        case ExpressionKind::kFunctionCall: {
            ...
            for (const std::unique_ptr<Expression>& arg : call.arguments()) {
                if (arg->is<VariableReference>() && arg->as<VariableReference>().variable() == &fChild) {
...
public:
    ...
    {
        return p.is<FunctionDefinition>() && !p.as<FunctionDefinition>().declaration().isMain() && INHERITED::visitProgramElement(p);
    }
...
public:
    ...
    bool visitStatement(const Statement& s) override
    {
        if (s.is<ReturnStatement>()) {
            ...
            bool knownOpaque = e && e->type().slotCount() == 4 && ConstantFolder::GetConstantValueForVariable(*e)->getConstantValue(/*n=*/3).value_or(0) == 1;
            return !knownOpaque;
        }
    ...
    bool visitExpression(const Expression& e) override
    {
        if (e.is<VariableReference>()) {
            ...
            if (ref.variable() == fVar
                && (ref.refKind() == VariableReference::RefKind::kWrite || ref.refKind() == VariableReference::RefKind::kReadWrite
...
public:
    IsAssignableVisitor(ErrorReporter* errors)
        : fErrors(errors)
    {
    }
    ...
    void visitExpression(Expression& expr, const FieldAccess* fieldAccess = nullptr)
    {
        ...
        case Expression::Kind::kVariableReference: {
            ...
            auto fieldName = [&] { return fieldAccess ? fieldAccess->description(OperatorPrecedence::kExpression) : std::string(var->name()); };
            if (var->modifierFlags().isConst() || var->modifierFlags().isUniform()) {
                fErrors->error(expr.fPosition, "cannot modify immutable variable '" + fieldName() + "'");
            } else if (var->storage() == Variable::Storage::kGlobal && (var->modifierFlags() & ModifierFlag::kIn)) {
                fErrors->error(expr.fPosition, "cannot modify pipeline input variable '" + fieldName() + "'");
            } else {
...
SampleUsage Analysis::GetSampleUsage(const Program& program, const Variable& child, bool writesToSampleCoords, int* elidedSampleCoordCount)
{
...
bool Analysis::CallsColorTransformIntrinsics(const Program& program)
{
    for (auto [symbol, count] : program.usage()->fCallCounts) {
        ...
        if (count != 0 && (fn.intrinsicKind() == k_toLinearSrgb_IntrinsicKind || fn.intrinsicKind() == k_fromLinearSrgb_IntrinsicKind)) {
            return true;
        }
    }
    return false;
...
bool Analysis::ContainsRTAdjust(const Expression& expr)
{
    ...
    public:
        ...
        {
            if (expr.is<VariableReference>() && expr.as<VariableReference>().variable()->name() == Compiler::RTADJUST_NAME) {
                return true;
            }
...
bool Analysis::ContainsVariable(const Expression& expr, const Variable& var)
{
    ...
    public:
        ContainsVariableVisitor(const Variable* v)
            : fVariable(v)
        {
        }
        ...
        bool visitExpression(const Expression& expr) override
        {
            if (expr.is<VariableReference>() && expr.as<VariableReference>().variable() == fVariable) {
                return true;
            }
            return INHERITED::visitExpression(expr);
        }
...
bool Analysis::UpdateVariableRefKind(Expression* expr, VariableReference::RefKind kind, ErrorReporter* errors)
{
...
template <typename T> bool TProgramVisitor<T>::visitExpression(typename T::Expression& e)
{
    ...
    case Expression::Kind::kBinary: {
        ...
        return (b.left() && this->visitExpressionPtr(b.left())) || (b.right() && this->visitExpressionPtr(b.right()));
    }
    ...
    case Expression::Kind::kTernary: {
        ...
        return this->visitExpressionPtr(t.test()) || (t.ifTrue() && this->visitExpressionPtr(t.ifTrue()))
...
template <typename T> bool TProgramVisitor<T>::visitStatement(typename T::Statement& s)
{
    ...
    case Statement::Kind::kFor: {
        ...
        return (f.initializer() && this->visitStatementPtr(f.initializer())) || (f.test() && this->visitExpressionPtr(f.test()))
            || (f.next() && this->visitExpressionPtr(f.next())) || this->visitStatementPtr(f.statement());
    }
    case Statement::Kind::kIf: {
        auto& i = s.template as<IfStatement>();
        return (i.test() && this->visitExpressionPtr(i.test())) || (i.ifTrue() && this->visitStatementPtr(i.ifTrue()))
