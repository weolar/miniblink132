...
static std::unique_ptr<Module> compile_and_shrink(
    SkSL::Compiler* compiler, ProgramKind kind, ModuleType moduleType, std::string moduleSource, const Module* parent)
{
    std::unique_ptr<Module> m = compiler->compileModule(kind, moduleType, std::move(moduleSource), parent,
        /*shouldInline=*/true);
    if (!m) {
...
const Module* ModuleLoader::loadPublicModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fPublicModule) {
        ...
        fModuleLoader.fPublicModule = compile_and_shrink(compiler, ProgramKind::kFragment, MODULE_DATA(sksl_public), sharedModule);
...
const Module* ModuleLoader::loadPrivateRTShaderModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fRuntimeShaderModule) {
        ...
        fModuleLoader.fRuntimeShaderModule = compile_and_shrink(compiler, ProgramKind::kFragment, MODULE_DATA(sksl_rt_shader), publicModule);
    }
...
const Module* ModuleLoader::loadSharedModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fSharedModule) {
        ...
        fModuleLoader.fSharedModule = compile_and_shrink(compiler, ProgramKind::kFragment, MODULE_DATA(sksl_shared), rootModule);
    }
...
const Module* ModuleLoader::loadGPUModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fGPUModule) {
        const Module* sharedModule = this->loadSharedModule(compiler);
        fModuleLoader.fGPUModule = compile_and_shrink(compiler, ProgramKind::kFragment, MODULE_DATA(sksl_gpu), sharedModule);
    }
...
const Module* ModuleLoader::loadFragmentModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fFragmentModule) {
        ...
        fModuleLoader.fFragmentModule = compile_and_shrink(compiler, ProgramKind::kFragment, MODULE_DATA(sksl_frag), gpuModule);
    }
...
const Module* ModuleLoader::loadVertexModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fVertexModule) {
        const Module* gpuModule = this->loadGPUModule(compiler);
        fModuleLoader.fVertexModule = compile_and_shrink(compiler, ProgramKind::kVertex, MODULE_DATA(sksl_vert), gpuModule);
    }
...
const Module* ModuleLoader::loadComputeModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fComputeModule) {
        const Module* gpuModule = this->loadGPUModule(compiler);
        fModuleLoader.fComputeModule = compile_and_shrink(compiler, ProgramKind::kCompute, MODULE_DATA(sksl_compute), gpuModule);
    }
...
const Module* ModuleLoader::loadGraphiteFragmentModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fGraphiteFragmentModule) {
        ...
        fModuleLoader.fGraphiteFragmentModule = compile_and_shrink(compiler, ProgramKind::kGraphiteFragment, MODULE_DATA(sksl_graphite_frag), fragmentModule);
    }
...
const Module* ModuleLoader::loadGraphiteFragmentES2Module(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fGraphiteFragmentES2Module) {
        ...
        fModuleLoader.fGraphiteFragmentES2Module
            = compile_and_shrink(compiler, ProgramKind::kGraphiteFragmentES2, MODULE_DATA(sksl_graphite_frag_es2), fragmentModule);
    }
...
const Module* ModuleLoader::loadGraphiteVertexModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fGraphiteVertexModule) {
        ...
        fModuleLoader.fGraphiteVertexModule = compile_and_shrink(compiler, ProgramKind::kGraphiteVertex, MODULE_DATA(sksl_graphite_vert), vertexModule);
    }
...
const Module* ModuleLoader::loadGraphiteVertexES2Module(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fGraphiteVertexES2Module) {
        ...
        fModuleLoader.fGraphiteVertexES2Module
            = compile_and_shrink(compiler, ProgramKind::kGraphiteVertexES2, MODULE_DATA(sksl_graphite_vert_es2), vertexModule);
    }
...
void ModuleLoader::Impl::makeRootSymbolTable()
{
    ...
    rootModule->fSymbols->addOrDie(Variable::Make(/*pos=*/Position(),
        /*modifiersPosition=*/Position(), Layout {}, ModifierFlag::kNone, fBuiltinTypes.fSkCaps.get(), "sk_Caps",
        /*mangledName=*/"",
