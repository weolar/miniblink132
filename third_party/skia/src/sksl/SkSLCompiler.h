...
public:
    ...
    std::unique_ptr<Module> compileModule(ProgramKind kind, ModuleType moduleType, std::string moduleSource, const Module* parentModule, bool shouldInline);
...
private:
    ...
    void initializeContext(const SkSL::Module* module, ProgramKind kind, ProgramSettings settings, std::string_view source, ModuleType moduleType);
     ...
     */
    std::unique_ptr<SkSL::Program> releaseProgram(std::unique_ptr<std::string> source, std::vector<std::unique_ptr<SkSL::ProgramElement>> programElements);
    ...
    bool runInliner(Inliner* inliner, const std::vector<std::unique_ptr<ProgramElement>>& elements, SymbolTable* symbols, ProgramUsage* usage);
