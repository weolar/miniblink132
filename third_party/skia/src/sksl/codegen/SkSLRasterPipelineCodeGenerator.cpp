...
public:
    ...
    [[nodiscard]] bool pushBinaryExpression(const Expression& left, Operator op, const Expression& right);
    ...
    [[nodiscard]] bool pushIntrinsic(IntrinsicKind intrinsic, const Expression& arg0, const Expression& arg1);
    [[nodiscard]] bool pushIntrinsic(IntrinsicKind intrinsic, const Expression& arg0, const Expression& arg1, const Expression& arg2);
    ...
    [[nodiscard]] bool pushTernaryExpression(const Expression& test, const Expression& ifTrue, const Expression& ifFalse);
    [[nodiscard]] bool pushDynamicallyUniformTernaryExpression(const Expression& test, const Expression& ifTrue, const Expression& ifFalse);
    ...
    [[nodiscard]] std::optional<SlotRange> findPreexistingImmutableData(const TArray<ImmutableBits>& immutableValues);
    [[nodiscard]] std::optional<ImmutableBits> getImmutableBitsForSlot(const Expression& expr, size_t slot);
    [[nodiscard]] bool getImmutableValueForExpression(const Expression& expr, TArray<ImmutableBits>* immutableValues);
    ...
    [[nodiscard]] bool pushIntrinsic(const TypedOps& ops, const Expression& arg0, const Expression& arg1);
    [[nodiscard]] bool pushIntrinsic(BuilderOp builderOp, const Expression& arg0);
    [[nodiscard]] bool pushIntrinsic(BuilderOp builderOp, const Expression& arg0, const Expression& arg1);
    ...
    [[nodiscard]] bool pushMatrixMultiply(
        LValue* lvalue, const Expression& left, const Expression& right, int leftColumns, int leftRows, int rightColumns, int rightRows);
    [[nodiscard]] bool pushStructuredComparison(LValue* left, Operator op, LValue* right, const Type& type);
    ...
    Analysis::ReturnComplexity returnComplexity(const FunctionDefinition* func)
    {
        ...
        if (!complexity) {
            complexity = fReturnComplexityMap.set(fCurrentFunction, Analysis::GetReturnComplexity(*func));
        }
        return *complexity;
    }
    ...
    {
        return this->shouldWriteTraceOps() || (this->returnComplexity(func) > Analysis::ReturnComplexity::kSingleSafeReturn);
    }
    ...
    {
        return (var.modifierFlags() & (ModifierFlag::kIn | ModifierFlag::kOut)) == ModifierFlag::kOut;
    }

    static bool IsInoutParameter(const Variable& var)
    {
        return (var.modifierFlags() & (ModifierFlag::kIn | ModifierFlag::kOut)) == (ModifierFlag::kIn | ModifierFlag::kOut);
    }
...
private:
    ...
    static constexpr auto kAddOps = TypedOps { BuilderOp::add_n_floats, BuilderOp::add_n_ints, BuilderOp::add_n_ints, BuilderOp::unsupported };
    static constexpr auto kSubtractOps = TypedOps { BuilderOp::sub_n_floats, BuilderOp::sub_n_ints, BuilderOp::sub_n_ints, BuilderOp::unsupported };
    static constexpr auto kMultiplyOps = TypedOps { BuilderOp::mul_n_floats, BuilderOp::mul_n_ints, BuilderOp::mul_n_ints, BuilderOp::unsupported };
    static constexpr auto kDivideOps = TypedOps { BuilderOp::div_n_floats, BuilderOp::div_n_ints, BuilderOp::div_n_uints, BuilderOp::unsupported };
    static constexpr auto kLessThanOps = TypedOps { BuilderOp::cmplt_n_floats, BuilderOp::cmplt_n_ints, BuilderOp::cmplt_n_uints, BuilderOp::unsupported };
    static constexpr auto kLessThanEqualOps = TypedOps { BuilderOp::cmple_n_floats, BuilderOp::cmple_n_ints, BuilderOp::cmple_n_uints, BuilderOp::unsupported };
    static constexpr auto kEqualOps = TypedOps { BuilderOp::cmpeq_n_floats, BuilderOp::cmpeq_n_ints, BuilderOp::cmpeq_n_ints, BuilderOp::cmpeq_n_ints };
    static constexpr auto kNotEqualOps = TypedOps { BuilderOp::cmpne_n_floats, BuilderOp::cmpne_n_ints, BuilderOp::cmpne_n_ints, BuilderOp::cmpne_n_ints };
    static constexpr auto kModOps = TypedOps { BuilderOp::mod_n_floats, BuilderOp::unsupported, BuilderOp::unsupported, BuilderOp::unsupported };
    static constexpr auto kMinOps = TypedOps { BuilderOp::min_n_floats, BuilderOp::min_n_ints, BuilderOp::min_n_uints, BuilderOp::min_n_uints };
    static constexpr auto kMaxOps = TypedOps { BuilderOp::max_n_floats, BuilderOp::max_n_ints, BuilderOp::max_n_uints, BuilderOp::max_n_uints };
    static constexpr auto kMixOps = TypedOps { BuilderOp::mix_n_floats, BuilderOp::unsupported, BuilderOp::unsupported, BuilderOp::unsupported };
    static constexpr auto kInverseSqrtOps = TypedOps { BuilderOp::invsqrt_float, BuilderOp::unsupported, BuilderOp::unsupported, BuilderOp::unsupported };
...
{
    fGenerator->builder()->push_clone_indirect_from_stack(range, dynamicStackID, /*otherStackID=*/fStackID, offsetFromStackTop);
}
...
public:
    ...
    [[nodiscard]] virtual bool push(Generator* gen, SlotRange fixedOffset, AutoStack* dynamicOffset, SkSpan<const int8_t> swizzle) = 0;

    /** Stores topmost values from the stack directly into the lvalue. */
    [[nodiscard]] virtual bool store(Generator* gen, SlotRange fixedOffset, AutoStack* dynamicOffset, SkSpan<const int8_t> swizzle) = 0;
    /**
...
public:
    ...
    [[nodiscard]] bool push(Generator* gen, SlotRange fixedOffset, AutoStack* dynamicOffset, SkSpan<const int8_t> swizzle) override
    {
...
public:
    explicit VariableLValue(const Variable* v)
        : fVariable(v)
    {
    }
    ...
    SlotRange fixedSlotRange(Generator* gen) override
    {
        return Generator::IsUniform(*fVariable) ? gen->getUniformSlots(*fVariable) : gen->getVariableSlots(*fVariable);
    }
    ...
    [[nodiscard]] bool push(Generator* gen, SlotRange fixedOffset, AutoStack* dynamicOffset, SkSpan<const int8_t> swizzle) override
    {
        if (Generator::IsUniform(*fVariable)) {
            if (dynamicOffset) {
                gen->builder()->push_uniform_indirect(fixedOffset, dynamicOffset->stackID(), this->fixedSlotRange(gen));
            } else {
                gen->builder()->push_uniform(fixedOffset);
            }
        } else {
            if (dynamicOffset) {
                gen->builder()->push_slots_indirect(fixedOffset, dynamicOffset->stackID(), this->fixedSlotRange(gen));
            } else {
    ...
    [[nodiscard]] bool store(Generator* gen, SlotRange fixedOffset, AutoStack* dynamicOffset, SkSpan<const int8_t> swizzle) override
    {
        SkASSERT(!Generator::IsUniform(*fVariable));

        if (swizzle.empty()) {
            if (dynamicOffset) {
                gen->builder()->copy_stack_to_slots_indirect(fixedOffset, dynamicOffset->stackID(), this->fixedSlotRange(gen));
            } else {
                gen->builder()->copy_stack_to_slots(fixedOffset);
            }
        } else {
            if (dynamicOffset) {
                gen->builder()->swizzle_copy_stack_to_slots_indirect(fixedOffset, dynamicOffset->stackID(), this->fixedSlotRange(gen), swizzle, swizzle.size());
            } else {
        ...
        if (gen->shouldWriteTraceOps()) {
            if (dynamicOffset) {
                gen->builder()->trace_var_indirect(gen->traceMaskStackID(), fixedOffset, dynamicOffset->stackID(), this->fixedSlotRange(gen));
            } else {
...
public:
    explicit ImmutableLValue(const Variable* v)
        : fVariable(v)
    {
    }
    ...
    {
        if (dynamicOffset) {
            gen->builder()->push_immutable_indirect(fixedOffset, dynamicOffset->stackID(), this->fixedSlotRange(gen));
        } else {
...
void SlotManager::addSlotDebugInfoForGroup(const std::string& varName, const Type& type, Position pos, int* groupIndex, bool isFunctionReturnValue)
{
    ...
    case Type::TypeKind::kArray: {
        ...
        for (int slot = 0; slot < nslots; ++slot) {
            this->addSlotDebugInfoForGroup(varName + "[" + std::to_string(slot) + "]", elemType, pos, groupIndex, isFunctionReturnValue);
        }
        break;
    }
    case Type::TypeKind::kStruct: {
        for (const Field& field : type.fields()) {
            this->addSlotDebugInfoForGroup(varName + "." + std::string(field.fName), *field.fType, pos, groupIndex, isFunctionReturnValue);
        }
        break;
    }
    default:
...
void SlotManager::addSlotDebugInfo(const std::string& varName, const Type& type, Position pos, bool isFunctionReturnValue)
{
    int groupIndex = 0;
...
SlotRange SlotManager::createSlots(std::string name, const Type& type, Position pos, bool isFunctionReturnValue)
{
...
std::optional<SlotRange> SlotManager::mapVariableToSlots(const Variable& v, SlotRange range)
{
    ...
    std::optional<SlotRange> originalRange = existingEntry ? std::optional(*existingEntry) : std::nullopt;
...
SlotRange SlotManager::getVariableSlots(const Variable& v)
{
    ...
    SlotRange range = this->createSlots(std::string(v.name()), v.type(), v.fPosition,
...
SlotRange SlotManager::getFunctionSlots(const IRNode& callSite, const FunctionDeclaration& f)
{
    ...
    SlotRange range = this->createSlots("[" + std::string(f.name()) + "].result", f.returnType(), f.fPosition,
...
std::unique_ptr<LValue> Generator::makeLValue(const Expression& e, bool allowScratch)
{
    ...
    if (e.is<Swizzle>()) {
        ...
        if (std::unique_ptr<LValue> base = this->makeLValue(*swizzleExpr.base(), allowScratch)) {
            ...
            if (is_sliceable_swizzle(components)) {
                ...
                return std::make_unique<LValueSlice>(std::move(base), components[0], components.size());
            }
    ...
    if (e.is<FieldAccess>()) {
        ...
        if (std::unique_ptr<LValue> base = this->makeLValue(*fieldExpr.base(), allowScratch)) {
            // Represent field access with a slice.
            return std::make_unique<LValueSlice>(std::move(base), fieldExpr.initialSlot(), fieldExpr.type().slotCount());
        }
        return nullptr;
    }
    if (e.is<IndexExpression>()) {
        ...
        if (std::unique_ptr<Expression> rewritten = Transform::RewriteIndexedSwizzle(fContext, indexExpr)) {
        ...
        if (std::unique_ptr<LValue> base = this->makeLValue(*indexExpr.base(), allowScratch)) {
            ...
            if (ConstantFolder::GetConstantInt(*indexExpr.index(), &indexValue)) {
                ...
                return std::make_unique<LValueSlice>(std::move(base), numSlots * indexValue, numSlots);
            }
            ...
            return dynLValue->evaluateDynamicIndices(this) ? std::move(dynLValue) : nullptr;
        }
        return nullptr;
    }
    if (allowScratch) {
...
{
    return lvalue.push(this, lvalue.fixedSlotRange(this), lvalue.dynamicSlotRange(),
        /*swizzle=*/ {});
}
...
bool Generator::store(LValue& lvalue)
{
    ...
    return lvalue.store(this, lvalue.fixedSlotRange(this), lvalue.dynamicSlotRange(),
        /*swizzle=*/ {});
}
...
std::optional<SlotRange> Generator::writeFunction(
    const IRNode& callSite, const FunctionDefinition& function, SkSpan<std::unique_ptr<Expression> const> arguments)
{
        ...
        for (size_t index = 0; index < arguments.size(); ++index) {
            ...
            if (paramCounts.fWrite == 0 && arg.is<VariableReference>()) {
                ...
                if (this->hasVariableSlots(var)) {
                    std::optional<SlotRange> originalRange = fProgramSlots.mapVariableToSlots(param, this->getVariableSlots(var));
...
void Generator::emitTraceLine(Position pos)
{
    if (fDebugTrace && fWriteTraceOps && pos.valid() && fInsideCompoundStatement == 0) {
        ...
        int lineNumber = std::distance(fLineOffsets.begin(), std::upper_bound(fLineOffsets.begin(), fLineOffsets.end(), pos.startOffset()));
...
bool Generator::writeSwitchStatement(const SwitchStatement& s)
{
    ...
    SkASSERT(std::all_of(cases.begin(), cases.end(), [](const std::unique_ptr<Statement>& stmt) { return stmt->is<SwitchCase>(); }));
...
BuilderOp Generator::GetTypedOp(const SkSL::Type& type, const TypedOps& ops)
{
    ...
    case Type::NumberKind::kSigned:
        return ops.fSignedOp;
    case Type::NumberKind::kUnsigned:
        return ops.fUnsignedOp;
    case Type::NumberKind::kBoolean:
        return ops.fBooleanOp;
    default:
        return BuilderOp::unsupported;
    }
}
...
{
    return lvalue ? this->push(*lvalue) : this->pushExpression(expr);
}

bool Generator::pushMatrixMultiply(
    LValue* lvalue, const Expression& left, const Expression& right, int leftColumns, int leftRows, int rightColumns, int rightRows)
{
    ...
    return lvalue ? this->store(*lvalue) : true;
}
...
bool Generator::pushStructuredComparison(LValue* left, Operator op, LValue* right, const Type& type)
{
    if (type.isStruct()) {
...
bool Generator::pushBinaryExpression(const Expression& left, Operator op, const Expression& right)
{
    ...
    case OperatorKind::LOGICALXOR: {
        double unused;
        if (ConstantFolder::GetConstantValue(left, &unused) && !ConstantFolder::GetConstantValue(right, &unused)) {
    ...
    if (op.isAssignment()) {
        ...
        if (op.kind() == OperatorKind::EQ) {
            return this->pushExpression(right) && this->store(*lvalue);
        }
    ...
    if (op.kind() == OperatorKind::STAR) {
        ...
        if (left.type().isMatrix() && right.type().isMatrix()) {
            return this->pushMatrixMultiply(lvalue.get(), left, right, left.type().columns(), left.type().rows(), right.type().columns(), right.type().rows());
        }

        // Vector * matrix:
        if (left.type().isVector() && right.type().isMatrix()) {
            return this->pushMatrixMultiply(lvalue.get(), left, right, left.type().columns(), 1, right.type().columns(), right.type().rows());
        }

        // Matrix * vector:
        if (left.type().isMatrix() && right.type().isVector()) {
            return this->pushMatrixMultiply(lvalue.get(), left, right, left.type().columns(), left.type().rows(), 1, right.type().columns());
        }
    }
...
std::optional<Generator::ImmutableBits> Generator::getImmutableBitsForSlot(const Expression& expr, size_t slot)
{
...
bool Generator::getImmutableValueForExpression(const Expression& expr, TArray<ImmutableBits>* immutableValues)
{
...
void Generator::storeImmutableValueToSlots(const TArray<ImmutableBits>& immutableValues, SlotRange slots)
{
...
std::optional<SlotRange> Generator::findPreexistingImmutableData(const TArray<ImmutableBits>& immutableValues)
{
...
bool Generator::pushImmutableData(const Expression& e)
{
    ...
    SlotRange range = fImmutableSlots.createSlots(e.description(), e.type(), e.fPosition,
        /*isFunctionReturnValue=*/false);
...
bool Generator::pushChildCall(const ChildCall& c)
{
    ...
    case Type::TypeKind::kColorFilter: {
        ...
        SkASSERT(c.arguments().size() == 1);
        SkASSERT(arg->type().matches(*fContext.fTypes.fHalf4) || arg->type().matches(*fContext.fTypes.fFloat4));
    ...
    case Type::TypeKind::kBlender: {
        ...
        SkASSERT(c.arguments()[0]->type().matches(*fContext.fTypes.fHalf4) || c.arguments()[0]->type().matches(*fContext.fTypes.fFloat4));
        SkASSERT(c.arguments()[1]->type().matches(*fContext.fTypes.fHalf4) || c.arguments()[1]->type().matches(*fContext.fTypes.fFloat4));
...
bool Generator::pushConstructorCast(const AnyConstructor& c)
{
    ...
    case Type::NumberKind::kBoolean:
        ...
        } else if (outerKind == Type::NumberKind::kSigned || outerKind == Type::NumberKind::kUnsigned) {
    ...
    SkDEBUGFAILF("unexpected cast from %s to %s", c.type().description().c_str(), inner.type().description().c_str());
    return unsupported();
}
...
bool Generator::pushConstructorMatrixResize(const ConstructorMatrixResize& c)
{
    ...
    fBuilder.matrix_resize(c.argument()->type().columns(), c.argument()->type().rows(), c.type().columns(), c.type().rows());
    return true;
}
...
bool Generator::pushIntrinsic(IntrinsicKind intrinsic, const Expression& arg0)
{
    ...
    case IntrinsicKind::k_length_IntrinsicKind:
        return this->pushExpression(arg0) && this->pushLengthIntrinsic(arg0.type().slotCount());
    ...
    case IntrinsicKind::k_normalize_IntrinsicKind: {
        ...
        if (slotCount > 1) {
            ...
            fBuilder.push_clone(slotCount);
            return this->pushAbsFloatIntrinsic(/*slots=*/1) && this->binaryOp(arg0.type(), kDivideOps);
        }
    }
...
bool Generator::pushIntrinsic(IntrinsicKind intrinsic, const Expression& arg0, const Expression& arg1)
{
    switch (intrinsic) {
    case IntrinsicKind::k_atan_IntrinsicKind:
    ...
    case IntrinsicKind::k_distance_IntrinsicKind:
        ...
        return this->pushBinaryExpression(arg0, OperatorKind::MINUS, arg1) && this->pushLengthIntrinsic(arg0.type().slotCount());
...
bool Generator::pushIntrinsic(IntrinsicKind intrinsic, const Expression& arg0, const Expression& arg1, const Expression& arg2)
{
    switch (intrinsic) {
    ...
    case IntrinsicKind::k_smoothstep_IntrinsicKind:
        ...
        if (!this->pushVectorizedExpression(arg0, arg2.type()) || !this->pushVectorizedExpression(arg1, arg2.type()) || !this->pushExpression(arg2)) {
            return unsupported();
        }
...
bool Generator::pushSwizzle(const Swizzle& s)
{
    ...
    if (isSimpleSubset && s.base()->is<VariableReference>()) {
        ...
        return this->pushVariableReferencePartial(
            s.base()->as<VariableReference>(), SlotRange { /*index=*/s.components()[0], /*count=*/s.components().size() });
    }
...
bool Generator::pushDynamicallyUniformTernaryExpression(const Expression& test, const Expression& ifTrue, const Expression& ifFalse)
{
...
bool Generator::pushTernaryExpression(const Expression& test, const Expression& ifTrue, const Expression& ifFalse)
{
...
{
    return fBuilder.finish(fProgramSlots.slotCount(), fUniformSlots.slotCount(), fImmutableSlots.slotCount(), fDebugTrace);
}

} // namespace RP

std::unique_ptr<RP::Program> MakeRasterPipelineProgram(
    const SkSL::Program& program, const FunctionDefinition& function, DebugTracePriv* debugTrace, bool writeTraceOps)
{
