...
private:
    ...
    std::string functionName(const FunctionDeclaration& decl, Analysis::SpecializationIndex specIndex);
    ...
    struct AutoOutputBuffer {
        AutoOutputBuffer(PipelineStageCodeGenerator* generator)
    ...
    THashMap<Analysis::SpecializedFunctionKey, std::string, Analysis::SpecializedFunctionKey::Hash> fFunctionNames;
...
void PipelineStageCodeGenerator::writeChildCall(const ChildCall& c)
{
    ...
    std::string sampleOutput;
    {
        ...
        case Type::TypeKind::kColorFilter: {
            SkASSERT(arguments.size() == 1);
            SkASSERT(arguments[0]->type().matches(*fProgram.fContext->fTypes.fHalf4) || arguments[0]->type().matches(*fProgram.fContext->fTypes.fFloat4));
        ...
        case Type::TypeKind::kBlender: {
            ...
            SkASSERT(arguments[0]->type().matches(*fProgram.fContext->fTypes.fHalf4) || arguments[0]->type().matches(*fProgram.fContext->fTypes.fFloat4));
            SkASSERT(arguments[1]->type().matches(*fProgram.fContext->fTypes.fHalf4) || arguments[1]->type().matches(*fProgram.fContext->fTypes.fFloat4));

            AutoOutputBuffer exprBuffer2(this);
            this->writeExpression(*arguments[1], Precedence::kSequence);

            sampleOutput = fCallbacks->sampleBlender(index, exprBuffer.fBuffer.str(), exprBuffer2.fBuffer.str());
            break;
        }
        default: {
...
void PipelineStageCodeGenerator::writeFunctionCall(const FunctionCall& c)
{
    ...
    if (function.intrinsicKind() == IntrinsicKind::k_toLinearSrgb_IntrinsicKind || function.intrinsicKind() == IntrinsicKind::k_fromLinearSrgb_IntrinsicKind) {
    ...
    Analysis::SpecializationIndex callIndex = Analysis::FindSpecializationIndexForCall(c, fSpecializationInfo, fActiveSpecializationIndex);
    SkBitSet specializedParams = Analysis::FindSpecializedParametersForFunction(function, fSpecializationInfo);
...
std::string PipelineStageCodeGenerator::functionName(const FunctionDeclaration& decl, Analysis::SpecializationIndex specIndex)
{
    if (decl.isMain()) {
    ...
    Analysis::GetParameterMappingsForFunction(decl, fSpecializationInfo, specIndex, [&](int, const Variable*, const Expression* expr) {
        specializedName += '_';
...
void PipelineStageCodeGenerator::writeFunction(const FunctionDefinition& f)
{
    ...
    this->forEachSpecialization(*fCurrentFunction, [&] {
        ...
        // Emit the function.
        fCallbacks->defineFunction(this->functionDeclaration(decl).c_str(), body.fBuffer.str().c_str(), decl.isMain());
    });
...
std::string PipelineStageCodeGenerator::functionDeclaration(const FunctionDeclaration& decl)
{
    ...
    std::string declString
        = String::printf("%s%s%s %s(", decl.modifierFlags().isInline() ? "inline " : "", decl.modifierFlags().isNoInline() ? "noinline " : "",
            this->typeName(decl.returnType()).c_str(), this->functionName(decl, fActiveSpecializationIndex).c_str());
...
void PipelineStageCodeGenerator::forEachSpecialization(const FunctionDeclaration& decl, const std::function<void()>& fn)
{
    ...
    if (const Analysis::Specializations* specializations = fSpecializationInfo.fSpecializationMap.find(&decl)) {
        // Invoke the callback for each specialization.
        for (fActiveSpecializationIndex = 0; fActiveSpecializationIndex < specializations->size(); ++fActiveSpecializationIndex) {
...
void PipelineStageCodeGenerator::writeGlobalVarDeclaration(const GlobalVarDeclaration& g)
{
    ...
    if (var.isBuiltin() || var.type().isOpaque()) {
        ...
        std::string declaration = this->modifierString(var.modifierFlags()) + this->typedVariable(var.type(), mangledName);
        if (decl.value()) {
...
void PipelineStageCodeGenerator::writeExpression(const Expression& expr, Precedence parentPrecedence)
{
    switch (expr.kind()) {
...
void PipelineStageCodeGenerator::writeAnyConstructor(const AnyConstructor& c, Precedence parentPrecedence)
{
...
void PipelineStageCodeGenerator::writeBinaryExpression(const BinaryExpression& b, Precedence parentPrecedence)
{
...
void PipelineStageCodeGenerator::writeTernaryExpression(const TernaryExpression& t, Precedence parentPrecedence)
{
...
void PipelineStageCodeGenerator::writePrefixExpression(const PrefixExpression& p, Precedence parentPrecedence)
{
...
void PipelineStageCodeGenerator::writePostfixExpression(const PostfixExpression& p, Precedence parentPrecedence)
{
...
void PipelineStageCodeGenerator::writeStatement(const Statement& s)
{
    ...
    case Statement::Kind::kExpression:
        this->writeExpression(*s.as<ExpressionStatement>().expression(), Precedence::kStatement);
        this->write(";");
        break;
...
void PipelineStageCodeGenerator::generateCode()
{
    ...
    Analysis::FindFunctionsToSpecialize(fProgram, &fSpecializationInfo, [](const Variable& param) { return param.type().isEffectChild(); });
