...
public:
    MetalCodeGenerator(const Context* context, const ShaderCaps* caps, const Program* program, OutputStream* out, PrettyPrint pp)
...
std::string MetalCodeGenerator::typeName(const Type& raw)
{
    ...
    case Type::TypeKind::kMatrix:
        return this->typeName(type.componentType()) + std::to_string(type.columns()) + "x" + std::to_string(type.rows());
    ...
    case Type::TypeKind::kTexture:
        ...
        case Type::TextureAccess::kRead:
            return "texture2d<half, access::read>";
        case Type::TextureAccess::kWrite:
            return "texture2d<half, access::write>";
        case Type::TextureAccess::kReadWrite:
            return "texture2d<half, access::read_write>";
        default:
            break;
        }
        SkUNREACHABLE;
...
void MetalCodeGenerator::assembleMatrixFromExpressions(const AnyConstructor& ctor, int columns, int rows)
{
    SkASSERT(rows <= 4);
    SkASSERT(columns <= 4);
...
std::string MetalCodeGenerator::getMatrixConstructHelper(const AnyConstructor& c)
{
    ...
    if (!fHelpers.contains(name)) {
        ...
        for (const std::unique_ptr<Expression>& expr : args) {
            fExtraFunctions.printf("%s%s x%zu", argSeparator, this->typeName(expr->type()).c_str(), argIndex++);
            argSeparator = ", ";
        }
...
void MetalCodeGenerator::writeConstructorMatrixResize(const ConstructorMatrixResize& c, Precedence parentPrecedence)
{
...
void MetalCodeGenerator::writeConstructorCompound(const ConstructorCompound& c, Precedence parentPrecedence)
{
    if (c.type().isVector()) {
...
void MetalCodeGenerator::writeConstructorArrayCast(const ConstructorArrayCast& c, Precedence parentPrecedence)
{
...
array<%s, N> %s(thread const array<%s, N>& x) {
    ...
    return result;
            ...
            outTypeName.c_str(), name.c_str(), inTypeName.c_str(), outTypeName.c_str(), outTypeName.c_str());
    }
...
void MetalCodeGenerator::writeConstructorCompoundVector(const ConstructorCompound& c, Precedence parentPrecedence)
{
...
void MetalCodeGenerator::writeConstructorCompoundMatrix(const ConstructorCompound& c, Precedence parentPrecedence)
{
    SkASSERT(c.type().isMatrix());
...
void MetalCodeGenerator::writeAnyConstructor(const AnyConstructor& c, const char* leftBracket, const char* rightBracket, Precedence parentPrecedence)
{
...
void MetalCodeGenerator::writeCastConstructor(const AnyConstructor& c, const char* leftBracket, const char* rightBracket, Precedence parentPrecedence)
{
...
static bool is_input(const Variable& var)
{
    ...
    return var.modifierFlags() & ModifierFlag::kIn && (var.layout().fBuiltin == -1 || is_compute_builtin(var))
...
static bool is_output(const Variable& var)
{
    ...
    return (var.modifierFlags() & ModifierFlag::kOut) && !(var.modifierFlags() & ModifierFlag::kIn) && var.layout().fBuiltin == -1
        && var.type().typeKind() != Type::TypeKind::kTexture;
}
...
{
    SkASSERT(var.storage() == VariableStorage::kGlobal);
    return var.modifierFlags().isUniform() && var.type().typeKind() != Type::TypeKind::kSampler;
}
...
void MetalCodeGenerator::writeMatrixTimesEqualHelper(const Type& left, const Type& right, const Type& result)
{
    ...
    if (!fHelpers.contains(key)) {
        ...
        fExtraFunctions.printf("thread %s& operator*=(thread %s& left, thread const %s& right) {\n"
                               ...
                               "    return left;\n"
                               "}\n",
            this->typeName(result).c_str(), this->typeName(left).c_str(), this->typeName(right).c_str());
    }
}
...
thread bool operator!=(const %s left, const %s right);
            ...
            this->typeName(left).c_str(), this->typeName(right).c_str(), this->typeName(left).c_str(), this->typeName(right).c_str());
...
void MetalCodeGenerator::writeMatrixDivisionHelpers(const Type& type)
{
    ...
    if (!fHelpers.contains(key)) {
        ...
        fExtraFunctions.printf("thread %s operator/(const %s left, const %s right) {\n"
                               "    return %s(",
...
thread bool operator!=(thread const %s& left, thread const %s& right);
            ...
            this->typeName(type).c_str(), this->typeName(type).c_str(), this->typeName(type).c_str(), this->typeName(type).c_str());

        fExtraFunctions.printf("thread bool operator==(thread const %s& left, thread const %s& right) {\n"
                               "    return ",
        ...
        for (const Field& field : type.fields()) {
            if (field.fType->isArray()) {
                fExtraFunctions.printf("%s(make_array_ref(left.%.*s) == make_array_ref(right.%.*s))", separator, (int)field.fName.size(), field.fName.data(),
                    (int)field.fName.size(), field.fName.data());
            } else {
                fExtraFunctions.printf(
                    "%sall(left.%.*s == right.%.*s)", separator, (int)field.fName.size(), field.fName.data(), (int)field.fName.size(), field.fName.data());
            }
            separator = " &&\n           ";
        }
        fExtraFunctions.printf(";\n"
...
void MetalCodeGenerator::writeBinaryExpressionElement(const Expression& expr, Operator op, const Expression& other, Precedence precedence)
{
    bool needMatrixSplatOnScalar
        = other.type().isMatrix() && expr.type().isNumber() && op.isValidForMatrixOrVector() && op.removeAssignment().kind() != Operator::Kind::STAR;
...
void MetalCodeGenerator::writeBinaryExpression(const BinaryExpression& b, Precedence parentPrecedence)
{
    ...
    if (op.removeAssignment().kind() == Operator::Kind::SLASH
        && ((leftType.isMatrix() && rightType.isMatrix()) || (leftType.isScalar() && rightType.isMatrix()) || (leftType.isMatrix() && rightType.isScalar()))) {
    ...
    if (op.isCompoundAssignment() && left.kind() == Expression::Kind::kSwizzle) {
        ...
        this->writeWithIndexSubstitution([&] {
            ...
            this->writeBinaryExpressionElement(right, op, left, op.removeAssignment().getBinaryPrecedence());
        });
    } else {
...
void MetalCodeGenerator::writeTernaryExpression(const TernaryExpression& t, Precedence parentPrecedence)
{
...
void MetalCodeGenerator::writePrefixExpression(const PrefixExpression& p, Precedence parentPrecedence)
{
    ...
    case Operator::Kind::MINUS:
        ...
        if (p.operand()->type().isMatrix()) {
            this->write(p.type().componentType().highPrecision() ? "(-1.0 * " : "(-1.0h * ");
...
void MetalCodeGenerator::writePostfixExpression(const PostfixExpression& p, Precedence parentPrecedence)
{
    const Operator op = p.getOperator();
    switch (op.kind()) {
    case Operator::Kind::PLUSPLUS:
...
void MetalCodeGenerator::writeFunctionRequirementArgs(const FunctionDeclaration& f, const char*& separator)
{
...
void MetalCodeGenerator::writeFunctionRequirementParams(const FunctionDeclaration& f, const char*& separator)
{
    Requirements requirements = this->requirements(f);
    if (requirements & kInputs_Requirement) {
        this->write(separator);
...
{
    return (layout.fBinding >= 0) ? layout.fBinding : fProgram.fConfig->fSettings.fDefaultUniformBinding;
}

int MetalCodeGenerator::getUniformSet(const Layout& layout)
{
    return (layout.fSet >= 0) ? layout.fSet : fProgram.fConfig->fSettings.fDefaultUniformSet;
}

bool MetalCodeGenerator::writeFunctionDeclaration(const FunctionDeclaration& f)
{
    fRTFlipName = (fProgram.fInterface.fRTFlipUniform != Program::Interface::kRTFlip_None) ? "_globals._anonInterface0->" SKSL_RTFLIP_NAME : "";
    const char* separator = "";
    if (f.isMain()) {
        ...
        if (-1 != fUniformBuffer) {
            this->write(separator);
            this->write("constant Uniforms& _uniforms [[buffer(" + std::to_string(fUniformBuffer) + ")]]");
            separator = ", ";
        }
        ...
        if (ProgramConfig::IsFragment(fProgram.fConfig->fKind)) {
            if (fProgram.fInterface.fRTFlipUniform != Program::Interface::kRTFlip_None && fInterfaceBlockNameMap.empty()) {
                this->write(separator);
...
{
    if (ProgramConfig::IsCompute(fProgram.fConfig->fKind) && (flags & (ModifierFlag::kIn | ModifierFlag::kOut))) {
        this->write("device ");
...
void MetalCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf)
{
    ...
    if (!intf.instanceName().empty()) {
        ...
        fInterfaceBlockNameMap.set(&intf.var()->type(), "_anonInterface" + std::to_string(fAnonInterfaceCount++));
    }
    this->writeLine(";");
}
...
void MetalCodeGenerator::writeFields(SkSpan<const Field> fields, Position parentPos)
{
    ...
    for (const Field& field : fields) {
        ...
        if (!memoryLayout.isSupported(*fieldType)) {
            fContext.fErrors->error(parentPos, "type '" + std::string(fieldType->name()) + "' is not permitted here");
            return;
        }
        if (fieldOffset != -1) {
            if (currentOffset > fieldOffset) {
                fContext.fErrors->error(
                    field.fPosition, "offset of field '" + std::string(field.fName) + "' must be at least " + std::to_string(currentOffset));
                return;
...
void MetalCodeGenerator::writeReturnStatementFromMain()
{
    ...
    if (ProgramConfig::IsVertex(fProgram.fConfig->fKind) || ProgramConfig::IsFragment(fProgram.fConfig->fKind)) {
    ...
    if (fCurrentFunction && fCurrentFunction->isMain()) {
        if (r.expression()) {
            if (r.expression()->type().matches(*fContext.fTypes.fHalf4)) {
                ...
                fContext.fErrors->error(r.fPosition, "Metal does not support returning '" + r.expression()->type().description() + "' from main()");
            }
        }
        this->writeReturnStatementFromMain();
        return;
...
half4 sampleGrad(sampler2D i, float2 p, float2 dPdx, float2 dPdy) {
    ...
    visitor.fTextureBias = fProgram.fConfig->fSettings.fSharpenTextures ? kSharpenTexturesBias : 0.0f;
    ...
    for (const ProgramElement* e : fProgram.elements()) {
        if (e->is<GlobalVarDeclaration>()) {
            ...
            if (var.modifierFlags().isUniform()) {
                SkASSERT(var.type().typeKind() != Type::TypeKind::kSampler && var.type().typeKind() != Type::TypeKind::kTexture);
...
{
    SkASSERT((is_output(var) && ProgramConfig::IsVertex(fProgram.fConfig->fKind)) || (is_input(var) && ProgramConfig::IsFragment(fProgram.fConfig->fKind)));
...
void MetalCodeGenerator::writeInputStruct()
{
    ...
    for (const ProgramElement* e : fProgram.elements()) {
        if (e->is<GlobalVarDeclaration>()) {
            ...
            if (is_input(var)) {
                this->write("    ");
                if (ProgramConfig::IsCompute(fProgram.fConfig->fKind) && needs_address_space(var.type(), var.modifierFlags())) {
    ...
    for (const ProgramElement* e : fProgram.elements()) {
        if (e->is<GlobalVarDeclaration>()) {
            ...
            if (is_output(var)) {
                ...
                this->writeType(var.type());
                if (ProgramConfig::IsCompute(fProgram.fConfig->fKind) && pass_by_reference(var.type(), var.modifierFlags())) {
                    this->write("&");
                }
                this->write(" ");
                this->writeName(var.mangledName());
                ...
                if (!ProgramConfig::IsCompute(fProgram.fConfig->fKind) && location < 0 && var.type().typeKind() != Type::TypeKind::kTexture) {
                    fContext.fErrors->error(var.fPosition, "Metal out variables must have 'layout(location=...)'");
                } else if (ProgramConfig::IsVertex(fProgram.fConfig->fKind)) {
...
void MetalCodeGenerator::writeInterfaceBlocks()
{
    ...
    if (!wroteInterfaceBlock && fProgram.fInterface.fRTFlipUniform != Program::Interface::kRTFlip_None) {
