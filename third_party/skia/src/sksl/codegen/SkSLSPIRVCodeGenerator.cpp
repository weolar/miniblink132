...
private:
    ...
    SpvId getPointerType(const Type& type, const Layout& typeLayout, const MemoryLayout& memoryLayout, StorageClass storageClass);
    ...
    void writeFunctionInstantiation(const FunctionDefinition& f, Analysis::SpecializationIndex specializationIndex,
        const Analysis::SpecializedParameters* specializedParams, OutputStream& out);
    ...
    void writeFunctionCallArgument(TArray<SpvId>& argumentList, const FunctionCall& call, int argIndex, std::vector<TempVar>* tempVars,
        const SkBitSet* specializedParams, OutputStream& out);
    ...
    void writeGLSLExtendedInstruction(
        const Type& type, SpvId id, SpvId floatInst, SpvId signedInst, SpvId unsignedInst, const TArray<SpvId>& args, OutputStream& out);
    ...
    SpvId writeAtomicIntrinsic(const FunctionCall& c, SpecialIntrinsic kind, SpvId resultId, OutputStream& out);

    SpvId castScalarToFloat(SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out);

    SpvId castScalarToSignedInt(SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out);

    SpvId castScalarToUnsignedInt(SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out);

    SpvId castScalarToBoolean(SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out);

    SpvId castScalarToType(SpvId inputExprId, const Type& inputType, const Type& outputType, OutputStream& out);
    ...
    void addColumnEntry(const Type& columnType, TArray<SpvId>* currentColumn, TArray<SpvId>* columnIds, int rows, SpvId entry, OutputStream& out);
    ...
    SpvId writeSwizzle(const Expression& baseExpr, const ComponentArray& components, OutputStream& out);
    ...
    SpvId writeMatrixComparison(const Type& operandType, SpvId lhs, SpvId rhs, SpvOp_ floatOperator, SpvOp_ intOperator, SpvOp_ vectorMergeOperator,
        SpvOp_ mergeOperator, OutputStream& out);

    SpvId writeStructComparison(const Type& structType, SpvId lhs, Operator op, SpvId rhs, OutputStream& out);

    SpvId writeArrayComparison(const Type& structType, SpvId lhs, Operator op, SpvId rhs, OutputStream& out);
    ...
    SpvId writeDecomposedMatrixVectorMultiply(const Type& leftType, SpvId lhs, const Type& rightType, SpvId rhs, const Type& resultType, OutputStream& out);

    SpvId writeComponentwiseMatrixUnary(const Type& operandType, SpvId operand, SpvOp_ op, OutputStream& out);

    SpvId writeComponentwiseMatrixBinary(const Type& operandType, SpvId lhs, SpvId rhs, SpvOp_ op, OutputStream& out);

    SpvId writeBinaryOperationComponentwiseIfMatrix(
        const Type& resultType, const Type& operandType, SpvId lhs, SpvId rhs, SpvOp_ ifFloat, SpvOp_ ifInt, SpvOp_ ifUInt, SpvOp_ ifBool, OutputStream& out);

    SpvId writeBinaryOperation(
        const Type& resultType, const Type& operandType, SpvId lhs, SpvId rhs, SpvOp_ ifFloat, SpvOp_ ifInt, SpvOp_ ifUInt, SpvOp_ ifBool, OutputStream& out);

    SpvId writeBinaryOperation(const Type& resultType, const Type& operandType, SpvId lhs, SpvId rhs, bool writeComponentwiseIfMatrix, SpvOp_ ifFloat,
    ...
    SpvId writeBinaryExpression(const Type& leftType, SpvId lhs, Operator op, const Type& rightType, SpvId rhs, const Type& resultType, OutputStream& out);
    ...
    void writeInstruction(SpvOp_ opCode, int32_t word1, int32_t word2, std::string_view string, OutputStream& out);
    ...
    void writeInstruction(
        SpvOp_ opCode, int32_t word1, int32_t word2, int32_t word3, int32_t word4, int32_t word5, int32_t word6, int32_t word7, OutputStream& out);

    void writeInstruction(SpvOp_ opCode, int32_t word1, int32_t word2, int32_t word3, int32_t word4, int32_t word5, int32_t word6, int32_t word7, int32_t word8,
        OutputStream& out);
    ...
    SpvId writeOpCompositeExtract(const Type& type, SpvId base, int componentA, int componentB, OutputStream& out);
    ...
    std::tuple<const Variable*, const Variable*> synthesizeTextureAndSampler(const Variable& combinedSampler);
    ...
    THashMap<Analysis::SpecializedFunctionKey, SpvId, Analysis::SpecializedFunctionKey::Hash> fFunctionMap;
    ...
    THashMap<const Variable*, std::unique_ptr<SynthesizedTextureSamplerPair>> fSynthesizedSamplerMap;
...
struct SPIRVCodeGenerator::Word {
    ...
    {
        return Word { key, Kind::kKeyedResult };
    }

    bool isResult() const
    {
        return fKind >= Kind::kDefaultPrecisionResult;
    }
...
#define BY_TYPE_GLSL(ifFloat, ifInt, ifUInt)                                                                                                                   \
    Intrinsic                                                                                                                                                  \
    {                                                                                                                                                          \
        kGLSL_STD_450_IntrinsicOpcodeKind, GLSLstd450##ifFloat, GLSLstd450##ifInt, GLSLstd450##ifUInt, SpvOpUndef                                              \
    }
#define ALL_SPIRV(x)                                                                                                                                           \
    Intrinsic                                                                                                                                                  \
    {                                                                                                                                                          \
        kSPIRV_IntrinsicOpcodeKind, SpvOp##x, SpvOp##x, SpvOp##x, SpvOp##x                                                                                     \
    }
#define BOOL_SPIRV(x)                                                                                                                                          \
    Intrinsic                                                                                                                                                  \
    {                                                                                                                                                          \
        kSPIRV_IntrinsicOpcodeKind, SpvOpUndef, SpvOpUndef, SpvOpUndef, SpvOp##x                                                                               \
    }
#define FLOAT_SPIRV(x)                                                                                                                                         \
    Intrinsic                                                                                                                                                  \
    {                                                                                                                                                          \
        kSPIRV_IntrinsicOpcodeKind, SpvOp##x, SpvOpUndef, SpvOpUndef, SpvOpUndef                                                                               \
    }
#define SPECIAL(x)                                                                                                                                             \
    Intrinsic                                                                                                                                                  \
    {                                                                                                                                                          \
        kSpecial_IntrinsicOpcodeKind, k##x##_SpecialIntrinsic, k##x##_SpecialIntrinsic, k##x##_SpecialIntrinsic, k##x##_SpecialIntrinsic                       \
    }
    ...
    case k_isinf_IntrinsicKind:
        return ALL_SPIRV(IsInf);
    case k_isnan_IntrinsicKind:
    ...
    case k_matrixInverse_IntrinsicKind:
        return ALL_GLSL(MatrixInverse);
...
#define PACK(type)                                                                                                                                             \
    case k_pack##type##_IntrinsicKind:                                                                                                                         \
        return ALL_GLSL(Pack##type);                                                                                                                           \
    case k_unpack##type##_IntrinsicKind:                                                                                                                       \
        return ALL_GLSL(Unpack##type)
        PACK(Snorm4x8);
        PACK(Unorm4x8);
        PACK(Snorm2x16);
        PACK(Unorm2x16);
...
#undef PACK
    ...
    case k_subpassLoad_IntrinsicKind:
    ...
    case k_equal_IntrinsicKind:
        return Intrinsic { kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdEqual, SpvOpIEqual, SpvOpIEqual, SpvOpLogicalEqual };
    case k_notEqual_IntrinsicKind:
        return Intrinsic { kSPIRV_IntrinsicOpcodeKind, SpvOpFUnordNotEqual, SpvOpINotEqual, SpvOpINotEqual, SpvOpLogicalNotEqual };
    case k_lessThan_IntrinsicKind:
        return Intrinsic { kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdLessThan, SpvOpSLessThan, SpvOpULessThan, SpvOpUndef };
    case k_lessThanEqual_IntrinsicKind:
        return Intrinsic { kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdLessThanEqual, SpvOpSLessThanEqual, SpvOpULessThanEqual, SpvOpUndef };
    case k_greaterThan_IntrinsicKind:
        return Intrinsic { kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdGreaterThan, SpvOpSGreaterThan, SpvOpUGreaterThan, SpvOpUndef };
    case k_greaterThanEqual_IntrinsicKind:
        return Intrinsic { kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdGreaterThanEqual, SpvOpSGreaterThanEqual, SpvOpUGreaterThanEqual, SpvOpUndef };
...
template <typename T> static T pick_by_type(const Type& type, T ifFloat, T ifInt, T ifUInt, T ifBool)
{
    if (is_float(type)) {
        return ifFloat;
    }
...
void SPIRVCodeGenerator::writeLabel(SpvId label, BranchingLabelType type, ConditionalOpCounts ops, OutputStream& out)
{
    switch (type) {
    case kBranchIsBelow:
...
void SPIRVCodeGenerator::writeInstruction(SpvOp_ opCode, std::string_view string, OutputStream& out)
{
    this->writeOpCode(opCode, 1 + (string.length() + 4) / 4, out);
    this->writeString(string, out);
}

void SPIRVCodeGenerator::writeInstruction(SpvOp_ opCode, int32_t word1, std::string_view string, OutputStream& out)
{
...
void SPIRVCodeGenerator::writeInstruction(SpvOp_ opCode, int32_t word1, int32_t word2, std::string_view string, OutputStream& out)
{
...
void SPIRVCodeGenerator::writeInstruction(SpvOp_ opCode, int32_t word1, int32_t word2, OutputStream& out)
{
...
void SPIRVCodeGenerator::writeInstruction(SpvOp_ opCode, int32_t word1, int32_t word2, int32_t word3, OutputStream& out)
{
...
void SPIRVCodeGenerator::writeInstruction(SpvOp_ opCode, int32_t word1, int32_t word2, int32_t word3, int32_t word4, OutputStream& out)
{
...
void SPIRVCodeGenerator::writeInstruction(SpvOp_ opCode, int32_t word1, int32_t word2, int32_t word3, int32_t word4, int32_t word5, OutputStream& out)
{
...
void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, int32_t word2, int32_t word3, int32_t word4, int32_t word5, int32_t word6, OutputStream& out)
{
...
void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, int32_t word2, int32_t word3, int32_t word4, int32_t word5, int32_t word6, int32_t word7, int32_t word8, OutputStream& out)
{
...
SPIRVCodeGenerator::Instruction SPIRVCodeGenerator::BuildInstructionKey(SpvOp_ opCode, const TArray<Word>& words)
{
...
SpvId SPIRVCodeGenerator::writeInstruction(SpvOp_ opCode, const TArray<Word>& words, OutputStream& out)
{
...
SpvId SPIRVCodeGenerator::writeOpLoad(SpvId type, Precision precision, SpvId pointer, OutputStream& out)
{
...
void SPIRVCodeGenerator::writeOpStore(StorageClass storageClass, SpvId pointer, SpvId value, OutputStream& out)
{
...
{
    return this->writeInstruction(SpvOpConstantTrue, Words { this->getType(type), Word::Result() }, fConstantBuffer);
}

SpvId SPIRVCodeGenerator::writeOpConstantFalse(const Type& type)
{
    return this->writeInstruction(SpvOpConstantFalse, Words { this->getType(type), Word::Result() }, fConstantBuffer);
}

SpvId SPIRVCodeGenerator::writeOpConstant(const Type& type, int32_t valueBits)
{
    return this->writeInstruction(SpvOpConstant, Words { this->getType(type), Word::Result(), Word::Number(valueBits) }, fConstantBuffer);
}

SpvId SPIRVCodeGenerator::writeOpConstantComposite(const Type& type, const TArray<SpvId>& values)
{
...
SpvId SPIRVCodeGenerator::writeOpCompositeConstruct(const Type& type, const TArray<SpvId>& values, OutputStream& out)
{
    ...
    if (type.isMatrix()) {
        ...
        if (this->toConstants(SkSpan(values), &constants)) {
            ...
            for (int index = 0; index < type.columns(); ++index) {
                STArray<4, SpvId> columnConstants(&constants[index * type.rows()], type.rows());
...
SPIRVCodeGenerator::Instruction* SPIRVCodeGenerator::resultTypeForInstruction(const Instruction& instr)
{
...
int SPIRVCodeGenerator::numComponentsForVecInstruction(const Instruction& instr)
{
    ...
    SkASSERT(typeInstr);
    SkASSERT(typeInstr->fOp == SpvOpTypeVector || typeInstr->fOp == SpvOpTypeFloat || typeInstr->fOp == SpvOpTypeInt || typeInstr->fOp == SpvOpTypeBool);
...
SpvId SPIRVCodeGenerator::writeOpCompositeExtract(const Type& type, SpvId base, int component, OutputStream& out)
{
    ...
    return this->writeInstruction(SpvOpCompositeExtract, { this->getType(type), Word::Result(type), base, Word::Number(component) }, out);
}

SpvId SPIRVCodeGenerator::writeOpCompositeExtract(const Type& type, SpvId base, int componentA, int componentB, OutputStream& out)
{
    // If the base op is a composite, we can extract from it directly.
    ...
    return this->writeInstruction(
        SpvOpCompositeExtract, { this->getType(type), Word::Result(type), base, Word::Number(componentA), Word::Number(componentB) }, out);
}
...
{
    return this->nextId(type && type->hasPrecision() && !type->highPrecision() ? Precision::kRelaxed : Precision::kDefault);
}
...
SpvId SPIRVCodeGenerator::nextId(Precision precision)
{
    if (precision == Precision::kRelaxed && !fProgram.fConfig->fSettings.fForceHighPrecision) {
        this->writeInstruction(SpvOpDecorate, fIdCount, SpvDecorationRelaxedPrecision, fDecorationBuffer);
    }
    return fIdCount++;
}
...
SpvId SPIRVCodeGenerator::writeStruct(const Type& type, const MemoryLayout& memoryLayout)
{
    ...
    for (int32_t i = 0; i < (int32_t)type.fields().size(); i++) {
        ...
        if (!memoryLayout.isSupported(*field.fType)) {
            fContext.fErrors->error(type.fPosition, "type '" + field.fType->displayName() + "' is not permitted here");
            return resultId;
        }
        ...
        if (fieldLayout.fOffset >= 0) {
            if (fieldLayout.fOffset < (int)offset) {
                fContext.fErrors->error(field.fPosition, "offset of field '" + std::string(field.fName) + "' must be at least " + std::to_string(offset));
            }
            if (fieldLayout.fOffset % alignment) {
                fContext.fErrors->error(
                    field.fPosition, "offset of field '" + std::string(field.fName) + "' must be a multiple of " + std::to_string(alignment));
            }
        ...
        if (field.fLayout.fBuiltin < 0) {
            this->writeInstruction(SpvOpMemberDecorate, resultId, (SpvId)i, SpvDecorationOffset, (SpvId)offset, fDecorationBuffer);
        }
        if (field.fType->isMatrix()) {
            this->writeInstruction(SpvOpMemberDecorate, resultId, i, SpvDecorationColMajor, fDecorationBuffer);
            this->writeInstruction(SpvOpMemberDecorate, resultId, i, SpvDecorationMatrixStride, (SpvId)memoryLayout.stride(*field.fType), fDecorationBuffer);
        }
        if (!field.fType->highPrecision()) {
            this->writeInstruction(SpvOpMemberDecorate, resultId, (SpvId)i, SpvDecorationRelaxedPrecision, fDecorationBuffer);
        }
        offset += size;
...
SpvId SPIRVCodeGenerator::getType(const Type& rawType, const Layout& typeLayout, const MemoryLayout& memoryLayout)
{
    ...
    case Type::TypeKind::kLiteral: {
        ...
        if (type->isSigned()) {
            return this->writeInstruction(SpvOpTypeInt, Words { Word::Result(), Word::Number(32), Word::Number(1) }, fConstantBuffer);
        }
        if (type->isUnsigned()) {
            return this->writeInstruction(SpvOpTypeInt, Words { Word::Result(), Word::Number(32), Word::Number(0) }, fConstantBuffer);
        }
        if (type->isFloat()) {
            return this->writeInstruction(SpvOpTypeFloat, Words { Word::Result(), Word::Number(32) }, fConstantBuffer);
        }
    ...
    case Type::TypeKind::kVector: {
        ...
        return this->writeInstruction(SpvOpTypeVector, Words { Word::Result(), scalarTypeId, Word::Number(type->columns()) }, fConstantBuffer);
    }
    case Type::TypeKind::kMatrix: {
        SpvId vectorTypeId = this->getType(IndexExpression::IndexType(fContext, *type), typeLayout, memoryLayout);
        return this->writeInstruction(SpvOpTypeMatrix, Words { Word::Result(), vectorTypeId, Word::Number(type->columns()) }, fConstantBuffer);
    }
    case Type::TypeKind::kArray: {
        const MemoryLayout arrayMemoryLayout = fCaps.fForceStd430ArrayLayout ? MemoryLayout(MemoryLayout::Standard::k430) : memoryLayout;

        if (!arrayMemoryLayout.isSupported(*type)) {
            fContext.fErrors->error(type->fPosition, "type '" + type->displayName() + "' is not permitted here");
            return NA;
        }
        ...
        if (type->isUnsizedArray()) {
            result = this->writeInstruction(SpvOpTypeRuntimeArray, Words { Word::KeyedResult(stride), typeId }, fConstantBuffer);
        } else {
            SpvId countId = this->writeLiteral(type->columns(), *fContext.fTypes.fInt);
            result = this->writeInstruction(SpvOpTypeArray, Words { Word::KeyedResult(stride), typeId, countId }, fConstantBuffer);
        }
        this->writeInstruction(SpvOpDecorate, { result, SpvDecorationArrayStride, Word::Number(stride) }, fDecorationBuffer);
        return result;
    }
    ...
    case Type::TypeKind::kSampler: {
        ...
        return this->writeInstruction(SpvOpTypeSampledImage, Words { Word::Result(), imageTypeId }, fConstantBuffer);
    }
    case Type::TypeKind::kTexture: {
        SpvId floatTypeId = this->getType(*fContext.fTypes.fFloat, kDefaultTypeLayout, memoryLayout);

        bool sampled = (type->textureAccess() == Type::TextureAccess::kSample);
        SpvImageFormat format = (!sampled && type->dimensions() != SpvDimSubpassData) ? layout_flags_to_image_format(typeLayout.fFlags) : SpvImageFormatUnknown;

        return this->writeInstruction(SpvOpTypeImage,
            Words { Word::Result(), floatTypeId, Word::Number(type->dimensions()), Word::Number(type->isDepth()), Word::Number(type->isArrayedTexture()),
                Word::Number(type->isMultisampled()), Word::Number(sampled ? 1 : 2), format },
            fConstantBuffer);
    }
...
SpvId SPIRVCodeGenerator::getFunctionType(const FunctionDeclaration& function)
{
    ...
    for (const Variable* parameter : function.parameters()) {
        ...
        if (fUseTextureSamplerPairs && parameter->type().isSampler()) {
            words.push_back(this->getFunctionParameterType(parameter->type().textureType(), parameter->layout()));
            if (!paramIsSpecialized) {
                words.push_back(this->getFunctionParameterType(*fContext.fTypes.fSampler, kDefaultTypeLayout));
            }
...
SpvId SPIRVCodeGenerator::getFunctionParameterType(const Type& parameterType, const Layout& parameterLayout)
{
    ...
    if (parameterType.typeKind() == Type::TypeKind::kSampler || parameterType.typeKind() == Type::TypeKind::kSeparateSampler
    ...
    return this->getPointerType(parameterType, parameterLayout, this->memoryLayoutForStorageClass(storageClass), storageClass);
}

SpvId SPIRVCodeGenerator::getPointerType(const Type& type, StorageClass storageClass)
{
    return this->getPointerType(type, kDefaultTypeLayout, this->memoryLayoutForStorageClass(storageClass), storageClass);
}

SpvId SPIRVCodeGenerator::getPointerType(const Type& type, const Layout& typeLayout, const MemoryLayout& memoryLayout, StorageClass storageClass)
{
    return this->writeInstruction(SpvOpTypePointer,
        Words { Word::Result(), Word::Number(get_storage_class_spv_id(storageClass)), this->getType(type, typeLayout, memoryLayout) }, fConstantBuffer);
}
...
SpvId SPIRVCodeGenerator::writeIntrinsicCall(const FunctionCall& c, OutputStream& out)
{
    ...
    if (!arguments.empty()) {
        ...
        if (intrinsic.opKind == kSpecial_IntrinsicOpcodeKind) {
            ...
            intrinsicId = pick_by_type(type, intrinsic.floatOp, intrinsic.signedOp, intrinsic.unsignedOp, intrinsic.boolOp);
        }
    }
...
SpvId SPIRVCodeGenerator::writeSpecialIntrinsic(const FunctionCall& c, SpecialIntrinsic kind, OutputStream& out)
{
    const ExpressionArray& arguments = c.arguments();
    ...
    case kSubpassLoad_SpecialIntrinsic: {
        ...
        if (arguments.size() == 1) {
            ...
            this->writeInstruction(SpvOpImageRead, this->getType(callType), result, img, coords, SpvImageOperandsSampleMask, sample, out);
        }
        break;
    }
    case kTexture_SpecialIntrinsic: {
        ...
        if (arguments.size() == 3) {
            this->writeInstruction(op, type, result, sampler, uv, SpvImageOperandsBiasMask, this->writeExpression(*arguments[2], out), out);
        } else {
            SkASSERT(arguments.size() == 2);
            if (fProgram.fConfig->fSettings.fSharpenTextures) {
                ...
                this->writeInstruction(op, type, result, sampler, uv, SpvImageOperandsBiasMask, lodBias, out);
            } else {
    ...
    case kTextureGrad_SpecialIntrinsic: {
        ...
        this->writeInstruction(op, type, result, sampler, uv, SpvImageOperandsGradMask, dPdx, dPdy, out);
        break;
    }
    case kTextureLod_SpecialIntrinsic: {
        ...
        SpvId sampler = this->writeExpression(*arguments[0], out);
        SpvId uv = this->writeExpression(*arguments[1], out);
        this->writeInstruction(op, type, result, sampler, uv, SpvImageOperandsLodMask, this->writeExpression(*arguments[2], out), out);
        break;
    }
    case kTextureRead_SpecialIntrinsic: {
        ...
        case Type::TextureAccess::kSample:
            this->writeInstruction(SpvOpImageFetch, type, result, image, coord, SpvImageOperandsLodMask, this->writeOpConstant(*fContext.fTypes.fInt, 0), out);
            break;
    ...
    case kDFdy_SpecialIntrinsic: {
        ...
        if (!fProgram.fConfig->fSettings.fForceNoRTFlip) {
            ...
            SpvId rtFlipY = this->writeSwizzle(*this->identifier(SKSL_RTFLIP_NAME), componentArray, out);
    ...
    case kClamp_SpecialIntrinsic: {
        ...
        this->writeGLSLExtendedInstruction(callType, result, GLSLstd450FClamp, GLSLstd450SClamp, GLSLstd450UClamp, args, out);
        break;
    }
    case kMax_SpecialIntrinsic: {
        TArray<SpvId> args = this->vectorize(arguments, out);
        SkASSERT(args.size() == 2);
        this->writeGLSLExtendedInstruction(callType, result, GLSLstd450FMax, GLSLstd450SMax, GLSLstd450UMax, args, out);
        break;
    }
    case kMin_SpecialIntrinsic: {
        TArray<SpvId> args = this->vectorize(arguments, out);
        SkASSERT(args.size() == 2);
        this->writeGLSLExtendedInstruction(callType, result, GLSLstd450FMin, GLSLstd450SMin, GLSLstd450UMin, args, out);
        break;
    }
    case kMix_SpecialIntrinsic: {
        ...
        if (arguments[2]->type().componentType().isBoolean()) {
            ...
            this->writeInstruction(SpvOpSelect, this->getType(arguments[0]->type()), result, conditionId, trueId, falseId, out);
        } else {
            this->writeGLSLExtendedInstruction(callType, result, GLSLstd450FMix, SpvOpUndef, SpvOpUndef, args, out);
        }
        break;
    }
    case kSaturate_SpecialIntrinsic: {
        ...
        this->writeGLSLExtendedInstruction(callType, result, GLSLstd450FClamp, GLSLstd450SClamp, GLSLstd450UClamp, spvArgs, out);
        break;
    }
    case kSmoothStep_SpecialIntrinsic: {
        TArray<SpvId> args = this->vectorize(arguments, out);
        SkASSERT(args.size() == 3);
        this->writeGLSLExtendedInstruction(callType, result, GLSLstd450SmoothStep, SpvOpUndef, SpvOpUndef, args, out);
        break;
    }
    case kStep_SpecialIntrinsic: {
        TArray<SpvId> args = this->vectorize(arguments, out);
        SkASSERT(args.size() == 2);
        this->writeGLSLExtendedInstruction(callType, result, GLSLstd450Step, SpvOpUndef, SpvOpUndef, args, out);
        break;
    }
    ...
    case kWorkgroupBarrier_SpecialIntrinsic: {
        ...
        SpvId scopeId = this->writeOpConstant(*fContext.fTypes.fUInt, (int32_t)SpvScopeWorkgroup);
        int32_t memSemMask = (kind == kStorageBarrier_SpecialIntrinsic) ? SpvMemorySemanticsAcquireReleaseMask | SpvMemorySemanticsUniformMemoryMask
...
SpvId SPIRVCodeGenerator::writeAtomicIntrinsic(const FunctionCall& c, SpecialIntrinsic kind, SpvId resultId, OutputStream& out)
{
    const ExpressionArray& arguments = c.arguments();
    ...
    if (atomicPtrId == NA) {
        SkDEBUGFAILF("atomic intrinsic expected a pointer argument: %s", arguments[0]->description().c_str());
        return NA;
    }
    ...
    SpvId memoryScopeId = NA;
    {
        ...
        case StorageClass::kWorkgroup:
            ...
            break;
        default:
            SkDEBUGFAILF("atomic argument has invalid storage class: %d", get_storage_class_spv_id(atomicPtr->storageClass()));
            return NA;
        }
        memoryScopeId = this->writeOpConstant(*fContext.fTypes.fUInt, (int32_t)memoryScope);
    }

    SpvId relaxedMemoryOrderId = this->writeOpConstant(*fContext.fTypes.fUInt, (int32_t)SpvMemorySemanticsMaskNone);

    switch (kind) {
    case kAtomicAdd_SpecialIntrinsic:
        SkASSERT(arguments.size() == 2);
        this->writeInstruction(SpvOpAtomicIAdd, this->getType(c.type()), resultId, atomicPtrId, memoryScopeId, relaxedMemoryOrderId,
            this->writeExpression(*arguments[1], out), out);
        break;
    case kAtomicLoad_SpecialIntrinsic:
        SkASSERT(arguments.size() == 1);
        this->writeInstruction(SpvOpAtomicLoad, this->getType(c.type()), resultId, atomicPtrId, memoryScopeId, relaxedMemoryOrderId, out);
        break;
    case kAtomicStore_SpecialIntrinsic:
        SkASSERT(arguments.size() == 2);
        this->writeInstruction(SpvOpAtomicStore, atomicPtrId, memoryScopeId, relaxedMemoryOrderId, this->writeExpression(*arguments[1], out), out);
        break;
    default:
        SkUNREACHABLE;
    }
...
void SPIRVCodeGenerator::writeFunctionCallArgument(
    TArray<SpvId>& argumentList, const FunctionCall& call, int argIndex, std::vector<TempVar>* tempVars, const SkBitSet* specializedParams, OutputStream& out)
{
    ...
    if (arg.is<VariableReference>()
        && (arg.type().typeKind() == Type::TypeKind::kSampler || arg.type().typeKind() == Type::TypeKind::kSeparateSampler
    ...
    this->writeInstruction(SpvOpVariable, this->getPointerType(arg.type(), StorageClass::kFunction), tmpVar, SpvStorageClassFunction, fVariableBuffer);
    if (tmpValueId != NA) {
...
SpvId SPIRVCodeGenerator::writeFunctionCall(const FunctionCall& c, OutputStream& out)
{
    ...
    Analysis::SpecializationIndex specializationIndex = Analysis::FindSpecializationIndexForCall(c, fSpecializationInfo, fActiveSpecializationIndex);
    SpvId* entry = fFunctionMap.find({ &function, specializationIndex });
    if (!entry) {
        fContext.fErrors->error(c.fPosition, "function '" + function.description() + "' is not defined");
        return NA;
    }
    ...
    SkBitSet specializedParams = Analysis::FindSpecializedParametersForFunction(c.function(), fSpecializationInfo);
...
SpvId SPIRVCodeGenerator::castScalarToType(SpvId inputExprId, const Type& inputType, const Type& outputType, OutputStream& out)
{
    ...
    fContext.fErrors->error(Position(), "unsupported cast: " + inputType.description() + " to " + outputType.description());
    return inputExprId;
}

SpvId SPIRVCodeGenerator::castScalarToFloat(SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out)
{
    ...
    if (inputType.isBoolean()) {
        ...
        this->writeInstruction(SpvOpSelect, this->getType(outputType), result, inputId, oneID, zeroID, out);
...
SpvId SPIRVCodeGenerator::castScalarToSignedInt(SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out)
{
...
SpvId SPIRVCodeGenerator::castScalarToUnsignedInt(SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out)
{
...
SpvId SPIRVCodeGenerator::castScalarToBoolean(SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out)
{
    ...
    if (inputType.isSigned()) {
        ...
        const SpvId zeroID = this->writeLiteral(0.0, *fContext.fTypes.fFloat);
        this->writeInstruction(SpvOpFUnordNotEqual, this->getType(outputType), result, inputId, zeroID, out);
    } else {
...
SpvId SPIRVCodeGenerator::writeMatrixCopy(SpvId src, const Type& srcType, const Type& dstType, OutputStream& out)
{
...
void SPIRVCodeGenerator::addColumnEntry(
    const Type& columnType, TArray<SpvId>* currentColumn, TArray<SpvId>* columnIds, int rows, SpvId entry, OutputStream& out)
{
    ...
    for (int i = 0; i < arguments.size(); i++) {
        ...
        if (currentColumn.empty() && argType.isVector() && argType.columns() == rows) {
            ...
            for (int j = 0; j < argType.columns(); ++j) {
                SpvId swizzle = this->writeOpCompositeExtract(argType.componentType(), arguments[i], j, out);
...
SpvId SPIRVCodeGenerator::writeConstructorCompound(const ConstructorCompound& c, OutputStream& out)
{
    return c.type().isMatrix() ? this->writeMatrixConstructor(c, out) : this->writeVectorConstructor(c, out);
}
...
SpvId SPIRVCodeGenerator::writeConstructorScalarCast(const ConstructorScalarCast& c, OutputStream& out)
{
    const Type& type = c.type();
...
SpvId SPIRVCodeGenerator::writeConstructorCompoundCast(const ConstructorCompoundCast& c, OutputStream& out)
{
...
SpvId SPIRVCodeGenerator::writeConstructorDiagonalMatrix(const ConstructorDiagonalMatrix& c, OutputStream& out)
{
    const Type& type = c.type();
    SkASSERT(type.isMatrix());
...
SpvId SPIRVCodeGenerator::writeConstructorMatrixResize(const ConstructorMatrixResize& c, OutputStream& out)
{
...
static StorageClass get_storage_class_for_global_variable(const Variable& var, StorageClass fallbackStorageClass)
{
    SkASSERT(var.storage() == Variable::Storage::kGlobal);

    if (var.type().typeKind() == Type::TypeKind::kSampler || var.type().typeKind() == Type::TypeKind::kSeparateSampler
...
TArray<SpvId> SPIRVCodeGenerator::getAccessChain(const Expression& expr, OutputStream& out)
{
    ...
    case Expression::Kind::kIndex: {
        ...
        if (indexExpr.base()->is<Swizzle>()) {
            ...
            return this->getAccessChain(*Transform::RewriteIndexedSwizzle(fContext, indexExpr), out);
        }
    ...
    case Expression::Kind::kVariableReference: {
        if (fActiveSpecialization) {
            const Expression** specializedFieldIndex = fActiveSpecialization->find(expr.as<VariableReference>().variable());
...
public:
    PointerLValue(SPIRVCodeGenerator& gen, SpvId pointer, bool isMemoryObject, SpvId type, SPIRVCodeGenerator::Precision precision, StorageClass storageClass)
        : fGen(gen)
        , fPointer(pointer)
...
public:
    SwizzleLValue(
        SPIRVCodeGenerator& gen, SpvId vecPointer, const ComponentArray& components, const Type& baseType, const Type& swizzleType, StorageClass storageClass)
        : fGen(gen)
...
std::unique_ptr<SPIRVCodeGenerator::LValue> SPIRVCodeGenerator::getLValue(const Expression& expr, OutputStream& out)
{
    ...
    case Expression::Kind::kVariableReference: {
        ...
        if (uniformIdx >= 0) {
            ...
            this->writeInstruction(SpvOpAccessChain, typeId, memberId, fUniformBufferId, uniformIdxId, out);
            return std::make_unique<PointerLValue>(*this, memberId,
                /*isMemoryObjectPointer=*/true, this->getType(type, kDefaultTypeLayout, this->memoryLayoutForVariable(var)), precision, StorageClass::kUniform);
        }

        SpvId* entry = fVariableMap.find(&var);
        SkASSERTF(entry, "%s", expr.description().c_str());

        if (var.layout().fBuiltin == SK_SAMPLEMASKIN_BUILTIN || var.layout().fBuiltin == SK_SAMPLEMASK_BUILTIN) {
            // Access sk_SampleMask and sk_SampleMaskIn via an array access, since Vulkan
            // represents sample masks as an array of uints.
            StorageClass storageClass = get_storage_class_for_global_variable(var, StorageClass::kPrivate);
            ...
            return std::make_unique<PointerLValue>(*this, accessId,
                /*isMemoryObjectPointer=*/true, this->getType(type), precision, storageClass);
        }
        ...
        return std::make_unique<PointerLValue>(*this, *entry,
            /*isMemoryObjectPointer=*/true, typeId, precision, this->getStorageClass(expr));
    }
    ...
    case Expression::Kind::kFieldAccess: {
        ...
        return std::make_unique<PointerLValue>(*this, member,
            /*isMemoryObjectPointer=*/false, this->getType(type, kDefaultTypeLayout, this->memoryLayoutForStorageClass(storageClass)), precision, storageClass);
    }
    case Expression::Kind::kSwizzle: {
        ...
        if (swizzle.components().size() == 1) {
            ...
            return std::make_unique<SwizzleLValue>(*this, base, swizzle.components(), swizzle.base()->type(), type, storageClass);
        }
    }
    default: {
        ...
        this->writeInstruction(SpvOpVariable, pointerType, result, SpvStorageClassFunction, fVariableBuffer);
        this->writeOpStore(StorageClass::kFunction, result, this->writeExpression(expr, out), out);
        return std::make_unique<PointerLValue>(*this, result, /*isMemoryObjectPointer=*/true, this->getType(type), precision, StorageClass::kFunction);
    }
    }
}
...
{
    std::unique_ptr<Expression> expr = fProgram.fSymbols->instantiateSymbolRef(fContext, name, Position());
    return expr ? std::move(expr) : Poison::Make(Position(), fContext);
}
    ...
    case SK_FRAGCOORD_BUILTIN: {
        ...
        if (!fProgram.fSymbols->find(DEVICE_COORDS_NAME)) {
            ...
            auto coordsVar = Variable::Make(/*pos=*/Position(),
                /*modifiersPosition=*/Position(), layout, ModifierFlag::kNone, fContext.fTypes.fFloat4.get(), DEVICE_COORDS_NAME,
                /*mangledName=*/"",
        ...
        SpvId deviceCoordZW = this->writeSwizzle(*deviceCoord, { SwizzleComponent::Z, SwizzleComponent::W }, out);
        // Compute `flippedY = u_RTFlip.y * $device_FragCoords.y`.
        SpvId flippedY = this->writeBinaryExpression(
            *fContext.fTypes.fFloat, rtFlipY, OperatorKind::STAR, *fContext.fTypes.fFloat, deviceCoordY, *fContext.fTypes.fFloat, out);

        // Compute `flippedY = u_RTFlip.x + flippedY`.
        flippedY = this->writeBinaryExpression(
            *fContext.fTypes.fFloat, rtFlipX, OperatorKind::PLUS, *fContext.fTypes.fFloat, flippedY, *fContext.fTypes.fFloat, out);

        // Return `float4(deviceCoord.x, flippedY, deviceCoord.zw)`.
        return this->writeOpCompositeConstruct(*fContext.fTypes.fFloat4, { deviceCoordX, flippedY, deviceCoordZW }, out);
    }
    case SK_CLOCKWISE_BUILTIN: {
        ...
        if (!fProgram.fSymbols->find(DEVICE_CLOCKWISE_NAME)) {
            ...
            auto clockwiseVar = Variable::Make(/*pos=*/Position(),
                /*modifiersPosition=*/Position(), layout, ModifierFlag::kNone, fContext.fTypes.fBool.get(), DEVICE_CLOCKWISE_NAME,
                /*mangledName=*/"",
                /*builtin=*/true, Variable::Storage::kGlobal);
        ...
        SpvId positiveRTFlip
            = this->writeBinaryExpression(*fContext.fTypes.fFloat, rtFlipY, OperatorKind::GT, *fContext.fTypes.fFloat, zero, *fContext.fTypes.fBool, out);
        ...
        return this->writeBinaryExpression(
            *fContext.fTypes.fBool, positiveRTFlip, OperatorKind::LOGICALXOR, *fContext.fTypes.fBool, deviceClockwiseID, *fContext.fTypes.fBool, out);
    }
    default: {
        ...
        if (fUseTextureSamplerPairs && variable->type().isSampler()) {
            if (const auto* p = fSynthesizedSamplerMap.find(variable)) {
                ...
                SpvId img = this->writeOpLoad(this->getType((*p)->fTexture->type()), Precision::kDefault, *imgPtr, out);
                SpvId sampler = this->writeOpLoad(this->getType((*p)->fSampler->type()), Precision::kDefault, *samplerPtr, out);
                SpvId result = this->nextId(nullptr);
                this->writeInstruction(SpvOpSampledImage, this->getType(variable->type()), result, img, sampler, out);
                return result;
            }
            SkDEBUGFAIL("sampler missing from fSynthesizedSamplerMap");
        }
...
SpvId SPIRVCodeGenerator::writeIndexExpression(const IndexExpression& expr, OutputStream& out)
{
    if (expr.base()->type().isVector()) {
        ...
        SpvId result = this->nextId(nullptr);
        this->writeInstruction(SpvOpVectorExtractDynamic, this->getType(expr.type()), result, base, index, out);
        return result;
    }
...
SpvId SPIRVCodeGenerator::writeSwizzle(const Expression& baseExpr, const ComponentArray& components, OutputStream& out)
{
...
SpvId SPIRVCodeGenerator::writeBinaryOperation(const Type& resultType, const Type& operandType, SpvId lhs, SpvId rhs, bool writeComponentwiseIfMatrix,
    SpvOp_ ifFloat, SpvOp_ ifInt, SpvOp_ ifUInt, SpvOp_ ifBool, OutputStream& out)
{
    SpvOp_ op = pick_by_type(operandType, ifFloat, ifInt, ifUInt, ifBool);
    if (op == SpvOpUndef) {
        fContext.fErrors->error(operandType.fPosition, "unsupported operand for binary expression: " + operandType.description());
        return NA;
    }
...
SpvId SPIRVCodeGenerator::writeBinaryOperationComponentwiseIfMatrix(
    const Type& resultType, const Type& operandType, SpvId lhs, SpvId rhs, SpvOp_ ifFloat, SpvOp_ ifInt, SpvOp_ ifUInt, SpvOp_ ifBool, OutputStream& out)
{
    return this->writeBinaryOperation(resultType, operandType, lhs, rhs,
        /*writeComponentwiseIfMatrix=*/true, ifFloat, ifInt, ifUInt, ifBool, out);
}
...
{
    return this->writeBinaryOperation(resultType, operandType, lhs, rhs,
        /*writeComponentwiseIfMatrix=*/false, ifFloat, ifInt, ifUInt, ifBool, out);
}

SpvId SPIRVCodeGenerator::foldToBool(SpvId id, const Type& operandType, SpvOp op, OutputStream& out)
{
...
SpvId SPIRVCodeGenerator::writeMatrixComparison(const Type& operandType, SpvId lhs, SpvId rhs, SpvOp_ floatOperator, SpvOp_ intOperator,
    SpvOp_ vectorMergeOperator, SpvOp_ mergeOperator, OutputStream& out)
{
    ...
    SpvId bvecType = this->getType(fContext.fTypes.fBool->toCompound(fContext, operandType.rows(), 1));
...
SpvId SPIRVCodeGenerator::writeComponentwiseMatrixUnary(const Type& operandType, SpvId operand, SpvOp_ op, OutputStream& out)
{
    SkASSERT(operandType.isMatrix());
...
SpvId SPIRVCodeGenerator::writeComponentwiseMatrixBinary(const Type& operandType, SpvId lhs, SpvId rhs, SpvOp_ op, OutputStream& out)
{
    SkASSERT(operandType.isMatrix());
    const Type& columnType = operandType.columnType(fContext);
    SpvId columnTypeId = this->getType(columnType);
...
static bool types_match(const Type& a, const Type& b)
{
    ...
    return (a.typeKind() == b.typeKind()) && (a.isScalar() || a.isVector() || a.isMatrix()) && (a.columns() == b.columns() && a.rows() == b.rows())
        && a.componentType().numberKind() == b.componentType().numberKind();
}

SpvId SPIRVCodeGenerator::writeDecomposedMatrixVectorMultiply(
    const Type& leftType, SpvId lhs, const Type& rightType, SpvId rhs, const Type& resultType, OutputStream& out)
{
    SpvId sum = NA;
    ...
    for (int n = 0; n < leftType.rows(); ++n) {
        ...
        SpvId product = this->writeBinaryExpression(columnType, matN, OperatorKind::STAR, scalarType, vecN, columnType, out);

        // Sum all the components together.
        if (sum == NA) {
            sum = product;
        } else {
            sum = this->writeBinaryExpression(columnType, sum, OperatorKind::PLUS, columnType, product, columnType, out);
        }
    }
...
SpvId SPIRVCodeGenerator::writeBinaryExpression(
    const Type& leftType, SpvId lhs, Operator op, const Type& rightType, SpvId rhs, const Type& resultType, OutputStream& out)
{
        ...
        if (leftType.isVector() && rightType.isNumber()) {
            if (resultType.componentType().isFloat()) {
                ...
                case Operator::Kind::STAR: {
                    SpvId result = this->nextId(&resultType);
                    this->writeInstruction(SpvOpVectorTimesScalar, this->getType(resultType), result, lhs, rhs, out);
                    return result;
                }
                default:
                    break;
            ...
            if (resultType.componentType().isFloat()) {
                if (op.kind() == Operator::Kind::STAR) {
                    SpvId result = this->nextId(&resultType);
                    this->writeInstruction(SpvOpVectorTimesScalar, this->getType(resultType), result, rhs, lhs, out);
                    return result;
                }
            }
            ...
            if (op.kind() == Operator::Kind::STAR) {
                ...
                if (fCaps.fRewriteMatrixVectorMultiply && rightType.isVector() && !resultType.highPrecision()) {
                    return this->writeDecomposedMatrixVectorMultiply(leftType, lhs, rightType, rhs, resultType, out);
                }
            ...
            if (op.kind() == Operator::Kind::STAR) {
                ...
                if (leftType.isVector()) {
                    this->writeInstruction(SpvOpVectorTimesMatrix, this->getType(resultType), result, lhs, rhs, out);
                } else {
                    SkASSERT(leftType.isScalar());
                    this->writeInstruction(SpvOpMatrixTimesScalar, this->getType(resultType), result, rhs, lhs, out);
                }
                return result;
            } else {
                ...
                // Perform this operation as matrix-op-matrix.
                return this->writeBinaryExpression(rightType, lhsMatrix, op, rightType, rhs, resultType, out);
            }
        } else {
    ...
    case Operator::Kind::EQEQ: {
        if (operandType->isMatrix()) {
            return this->writeMatrixComparison(*operandType, lhs, rhs, SpvOpFOrdEqual, SpvOpIEqual, SpvOpAll, SpvOpLogicalAnd, out);
        }
        ...
        const Type* tmpType;
        if (operandType->isVector()) {
            tmpType = &fContext.fTypes.fBool->toCompound(fContext, operandType->columns(), operandType->rows());
        } else {
            tmpType = &resultType;
        }
        if (lhs == rhs) {
        ...
        return this->foldToBool(this->writeBinaryOperation(*tmpType, *operandType, lhs, rhs, SpvOpFOrdEqual, SpvOpIEqual, SpvOpIEqual, SpvOpLogicalEqual, out),
            *operandType, SpvOpAll, out);
    }
    case Operator::Kind::NEQ:
        if (operandType->isMatrix()) {
            return this->writeMatrixComparison(*operandType, lhs, rhs, SpvOpFUnordNotEqual, SpvOpINotEqual, SpvOpAny, SpvOpLogicalOr, out);
        }
        if (operandType->isStruct()) {
            return this->writeStructComparison(*operandType, lhs, op, rhs, out);
        }
    ...
    case Operator::Kind::LOGICALXOR:
        ...
        return this->foldToBool(
            this->writeBinaryOperation(*tmpType, *operandType, lhs, rhs, SpvOpFUnordNotEqual, SpvOpINotEqual, SpvOpINotEqual, SpvOpLogicalNotEqual, out),
            *operandType, SpvOpAny, out);
    case Operator::Kind::GT:
        SkASSERT(resultType.isBoolean());
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpFOrdGreaterThan, SpvOpSGreaterThan, SpvOpUGreaterThan, SpvOpUndef, out);
    case Operator::Kind::LT:
        SkASSERT(resultType.isBoolean());
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpFOrdLessThan, SpvOpSLessThan, SpvOpULessThan, SpvOpUndef, out);
    case Operator::Kind::GTEQ:
        SkASSERT(resultType.isBoolean());
        return this->writeBinaryOperation(
            resultType, *operandType, lhs, rhs, SpvOpFOrdGreaterThanEqual, SpvOpSGreaterThanEqual, SpvOpUGreaterThanEqual, SpvOpUndef, out);
    case Operator::Kind::LTEQ:
        SkASSERT(resultType.isBoolean());
        return this->writeBinaryOperation(
            resultType, *operandType, lhs, rhs, SpvOpFOrdLessThanEqual, SpvOpSLessThanEqual, SpvOpULessThanEqual, SpvOpUndef, out);
    case Operator::Kind::PLUS:
        return this->writeBinaryOperationComponentwiseIfMatrix(resultType, *operandType, lhs, rhs, SpvOpFAdd, SpvOpIAdd, SpvOpIAdd, SpvOpUndef, out);
    case Operator::Kind::MINUS:
        return this->writeBinaryOperationComponentwiseIfMatrix(resultType, *operandType, lhs, rhs, SpvOpFSub, SpvOpISub, SpvOpISub, SpvOpUndef, out);
    case Operator::Kind::STAR:
        if (leftType.isMatrix() && rightType.isMatrix()) {
            // matrix multiply
            SpvId result = this->nextId(&resultType);
            this->writeInstruction(SpvOpMatrixTimesMatrix, this->getType(resultType), result, lhs, rhs, out);
            return result;
        }
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpFMul, SpvOpIMul, SpvOpIMul, SpvOpUndef, out);
    case Operator::Kind::SLASH:
        return this->writeBinaryOperationComponentwiseIfMatrix(resultType, *operandType, lhs, rhs, SpvOpFDiv, SpvOpSDiv, SpvOpUDiv, SpvOpUndef, out);
    case Operator::Kind::PERCENT:
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpFMod, SpvOpSMod, SpvOpUMod, SpvOpUndef, out);
    case Operator::Kind::SHL:
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpUndef, SpvOpShiftLeftLogical, SpvOpShiftLeftLogical, SpvOpUndef, out);
    case Operator::Kind::SHR:
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpUndef, SpvOpShiftRightArithmetic, SpvOpShiftRightLogical, SpvOpUndef, out);
    case Operator::Kind::BITWISEAND:
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpUndef, SpvOpBitwiseAnd, SpvOpBitwiseAnd, SpvOpUndef, out);
    case Operator::Kind::BITWISEOR:
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpUndef, SpvOpBitwiseOr, SpvOpBitwiseOr, SpvOpUndef, out);
    case Operator::Kind::BITWISEXOR:
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpUndef, SpvOpBitwiseXor, SpvOpBitwiseXor, SpvOpUndef, out);
    default:
...
SpvId SPIRVCodeGenerator::writeArrayComparison(const Type& arrayType, SpvId lhs, Operator op, SpvId rhs, OutputStream& out)
{
    ...
    for (int index = 0; index < arraySize; ++index) {
        ...
        SpvId comparison = this->writeBinaryExpression(componentType, itemL, op, componentType, itemR, boolType, out);
...
SpvId SPIRVCodeGenerator::writeStructComparison(const Type& structType, SpvId lhs, Operator op, SpvId rhs, OutputStream& out)
{
    ...
    for (int index = 0; index < (int)fields.size(); ++index) {
        ...
        SpvId comparison = this->writeBinaryExpression(fieldType, fieldL, op, fieldType, fieldR, boolType, out);
        // Merge this comparison result with all the other comparisons we've done.
        allComparisons = this->mergeComparisons(comparison, allComparisons, op, out);
    }
    return allComparisons;
...
SpvId SPIRVCodeGenerator::mergeComparisons(SpvId comparison, SpvId allComparisons, Operator op, OutputStream& out)
{
    ...
    switch (op.kind()) {
    case Operator::Kind::EQEQ:
        this->writeInstruction(SpvOpLogicalAnd, boolTypeId, logicalOp, comparison, allComparisons, out);
        break;
    case Operator::Kind::NEQ:
        this->writeInstruction(SpvOpLogicalOr, boolTypeId, logicalOp, comparison, allComparisons, out);
        break;
    default:
...
SpvId SPIRVCodeGenerator::writeBinaryExpression(const BinaryExpression& b, OutputStream& out)
{
    ...
    SpvId rhs = this->writeExpression(*right, out);
    SpvId result = this->writeBinaryExpression(left->type(), lhs, op.removeAssignment(), right->type(), rhs, b.type(), out);
    if (lvalue) {
...
SpvId SPIRVCodeGenerator::writeLogicalAnd(const Expression& left, const Expression& right, OutputStream& out)
{
    ...
    SpvId result = this->nextId(nullptr);
    this->writeInstruction(SpvOpPhi, this->getType(*fContext.fTypes.fBool), result, falseConstant, lhsBlock, rhs, rhsBlock, out);

    return result;
}

SpvId SPIRVCodeGenerator::writeLogicalOr(const Expression& left, const Expression& right, OutputStream& out)
{
    ...
    this->writeLabel(end, kBranchIsAbove, conditionalOps, out);
    SpvId result = this->nextId(nullptr);
    this->writeInstruction(SpvOpPhi, this->getType(*fContext.fTypes.fBool), result, trueConstant, lhsBlock, rhs, rhsBlock, out);
...
SpvId SPIRVCodeGenerator::writeTernaryExpression(const TernaryExpression& t, OutputStream& out)
{
    ...
    if (t.ifTrue()->type().columns() == 1 && Analysis::IsCompileTimeConstant(*t.ifTrue()) && Analysis::IsCompileTimeConstant(*t.ifFalse())) {
    ...
    this->writeInstruction(SpvOpVariable, this->getPointerType(type, StorageClass::kFunction), var, SpvStorageClassFunction, fVariableBuffer);
...
SpvId SPIRVCodeGenerator::writePrefixExpression(const PrefixExpression& p, OutputStream& out)
{
    ...
    case Operator::Kind::PLUSPLUS: {
        ...
        SpvId result = this->writeBinaryOperationComponentwiseIfMatrix(type, type, lv->load(out), one, SpvOpFAdd, SpvOpIAdd, SpvOpIAdd, SpvOpUndef, out);
        lv->store(result, out);
        return result;
    }
    case Operator::Kind::MINUSMINUS: {
        ...
        SpvId one = this->writeLiteral(1.0, type.componentType());
        one = this->splat(type, one, out);
        SpvId result = this->writeBinaryOperationComponentwiseIfMatrix(type, type, lv->load(out), one, SpvOpFSub, SpvOpISub, SpvOpISub, SpvOpUndef, out);
        lv->store(result, out);
        return result;
    }
    case Operator::Kind::LOGICALNOT: {
        ...
        SpvId result = this->nextId(nullptr);
        this->writeInstruction(SpvOpLogicalNot, this->getType(type), result, this->writeExpression(*p.operand(), out), out);
        return result;
    }
    case Operator::Kind::BITWISENOT: {
        SpvId result = this->nextId(nullptr);
        this->writeInstruction(SpvOpNot, this->getType(type), result, this->writeExpression(*p.operand(), out), out);
        return result;
    }
    default:
        SkDEBUGFAILF("unsupported prefix expression: %s", p.description(OperatorPrecedence::kExpression).c_str());
        return NA;
    }
}
...
SpvId SPIRVCodeGenerator::writePostfixExpression(const PostfixExpression& p, OutputStream& out)
{
    ...
    switch (p.getOperator().kind()) {
    case Operator::Kind::PLUSPLUS: {
        SpvId temp = this->writeBinaryOperationComponentwiseIfMatrix(type, type, result, one, SpvOpFAdd, SpvOpIAdd, SpvOpIAdd, SpvOpUndef, out);
        lv->store(temp, out);
        return result;
    }
    case Operator::Kind::MINUSMINUS: {
        SpvId temp = this->writeBinaryOperationComponentwiseIfMatrix(type, type, result, one, SpvOpFSub, SpvOpISub, SpvOpISub, SpvOpUndef, out);
        lv->store(temp, out);
        return result;
    }
    default:
        SkDEBUGFAILF("unsupported postfix expression %s", p.description(OperatorPrecedence::kExpression).c_str());
        return NA;
    }
}
...
void SPIRVCodeGenerator::writeFunctionStart(const FunctionDeclaration& f, OutputStream& out)
{
    ...
    this->writeInstruction(SpvOpFunction, returnTypeId, result, SpvFunctionControlMaskNone, functionTypeId, out);
    std::string mangledName = f.mangledName();

    // For specialized functions, tack on `_param1_param2` to the function name.
    Analysis::GetParameterMappingsForFunction(f, fSpecializationInfo, fActiveSpecializationIndex, [&](int, const Variable*, const Expression* expr) {
    ...
    this->writeInstruction(SpvOpName, result, std::string_view(mangledName.c_str(), mangledName.size()), fNameBuffer);
...
{
    if (const Analysis::Specializations* specializations = fSpecializationInfo.fSpecializationMap.find(&f.declaration())) {
        for (int i = 0; i < specializations->size(); i++) {
            this->writeFunctionInstantiation(f, i, &specializations->at(i), out);
        }
    } else {
        this->writeFunctionInstantiation(f, Analysis::kUnspecialized,
            /*specializedParams=*/nullptr, out);
    }
}

void SPIRVCodeGenerator::writeFunctionInstantiation(
    const FunctionDefinition& f, Analysis::SpecializationIndex specializationIndex, const Analysis::SpecializedParameters* specializedParams, OutputStream& out)
{
    ConditionalOpCounts conditionalOps = this->getConditionalOpCounts();
...
void SPIRVCodeGenerator::writeLayout(const Layout& layout, SpvId target, Position pos)
{
    ...
    if (layout.fLocation >= 0) {
        this->writeInstruction(SpvOpDecorate, target, SpvDecorationLocation, layout.fLocation, fDecorationBuffer);
    }
    if (layout.fBinding >= 0) {
        if (isPushConstant) {
            fContext.fErrors->error(pos, "Can't apply 'binding' to push constants");
        } else {
            this->writeInstruction(SpvOpDecorate, target, SpvDecorationBinding, layout.fBinding, fDecorationBuffer);
        }
    }
    if (layout.fIndex >= 0) {
        this->writeInstruction(SpvOpDecorate, target, SpvDecorationIndex, layout.fIndex, fDecorationBuffer);
    }
    if (layout.fSet >= 0) {
        if (isPushConstant) {
            fContext.fErrors->error(pos, "Can't apply 'set' to push constants");
        } else {
            this->writeInstruction(SpvOpDecorate, target, SpvDecorationDescriptorSet, layout.fSet, fDecorationBuffer);
        }
    }
    if (layout.fInputAttachmentIndex >= 0) {
        this->writeInstruction(SpvOpDecorate, target, SpvDecorationInputAttachmentIndex, layout.fInputAttachmentIndex, fDecorationBuffer);
        fCapabilities |= (((uint64_t)1) << SpvCapabilityInputAttachment);
    }
    if (layout.fBuiltin >= 0 && (layout.fBuiltin != SK_FRAGCOLOR_BUILTIN && layout.fBuiltin != SK_SECONDARYFRAGCOLOR_BUILTIN)) {
        this->writeInstruction(SpvOpDecorate, target, SpvDecorationBuiltIn, layout.fBuiltin, fDecorationBuffer);
    }
}
...
void SPIRVCodeGenerator::writeFieldLayout(const Layout& layout, SpvId target, int member)
{
    ...
    if (layout.fLocation >= 0) {
        this->writeInstruction(SpvOpMemberDecorate, target, member, SpvDecorationLocation, layout.fLocation, fDecorationBuffer);
    }
    if (layout.fIndex >= 0) {
        this->writeInstruction(SpvOpMemberDecorate, target, member, SpvDecorationIndex, layout.fIndex, fDecorationBuffer);
    }
    if (layout.fInputAttachmentIndex >= 0) {
        this->writeInstruction(SpvOpDecorate, target, member, SpvDecorationInputAttachmentIndex, layout.fInputAttachmentIndex, fDecorationBuffer);
    }
    if (layout.fBuiltin >= 0) {
        this->writeInstruction(SpvOpMemberDecorate, target, member, SpvDecorationBuiltIn, layout.fBuiltin, fDecorationBuffer);
    }
}

MemoryLayout SPIRVCodeGenerator::memoryLayoutForStorageClass(StorageClass storageClass)
{
    return storageClass == StorageClass::kPushConstant || storageClass == StorageClass::kStorageBuffer ? MemoryLayout(MemoryLayout::Standard::k430)
...
MemoryLayout SPIRVCodeGenerator::memoryLayoutForVariable(const Variable& v) const
{
    ...
    return pushConstant || buffer ? MemoryLayout(MemoryLayout::Standard::k430) : fDefaultMemoryLayout;
}
...
SpvId SPIRVCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf, bool appendRTFlip)
{
    ...
    StorageClass storageClass = get_storage_class_for_global_variable(intfVar, StorageClass::kFunction);
    if (fProgram.fInterface.fRTFlipUniform != Program::Interface::kRTFlip_None && appendRTFlip && !fWroteRTFlip && type.isStruct()) {
        ...
        fields.emplace_back(Position(),
            ...
            ModifierFlag::kNone, SKSL_RTFLIP_NAME, fContext.fTypes.fFloat2.get());
        {
            AutoAttachPoolToThread attach(fProgram.fPool.get());
            const Type* rtFlipStructType
                = fProgram.fSymbols->takeOwnershipOfSymbol(Type::MakeStructType(fContext, type.fPosition, type.name(), std::move(fields),
                    /*interfaceBlock=*/true));
            Variable* modifiedVar = fProgram.fSymbols->takeOwnershipOfSymbol(
                Variable::Make(intfVar.fPosition, intfVar.modifiersPosition(), intfVar.layout(), intfVar.modifierFlags(), rtFlipStructType, intfVar.name(),
                    /*mangledName=*/"", intfVar.isBuiltin(), intfVar.storage()));
            InterfaceBlock modifiedCopy(intf.fPosition, modifiedVar);
            result = this->writeInterfaceBlock(modifiedCopy, /*appendRTFlip=*/false);
            fProgram.fSymbols->add(fContext, std::make_unique<FieldSymbol>(Position(), modifiedVar, rtFlipStructType->fields().size() - 1));
        }
    ...
    if (intfVar.layout().fBuiltin == -1) {
        ...
        this->writeInstruction(SpvOpDecorate, typeId, isStorageBuffer ? SpvDecorationBufferBlock : SpvDecorationBlock, fDecorationBuffer);
    }
    SpvId ptrType = this->nextId(nullptr);
    this->writeInstruction(SpvOpTypePointer, ptrType, get_storage_class_spv_id(storageClass), typeId, fConstantBuffer);
    this->writeInstruction(SpvOpVariable, ptrType, result, get_storage_class_spv_id(storageClass), fConstantBuffer);
    Layout layout = intfVar.layout();
    if ((storageClass == StorageClass::kUniform || storageClass == StorageClass::kStorageBuffer) && layout.fSet < 0) {
...
{
    return varDecl.var()->modifierFlags().isConst() && (varDecl.var()->type().isScalar() || varDecl.var()->type().isVector())
        && (ConstantFolder::GetConstantValueOrNull(*varDecl.value()) || Analysis::IsCompileTimeConstant(*varDecl.value()));
}

bool SPIRVCodeGenerator::writeGlobalVarDeclaration(ProgramKind kind, const VarDeclaration& varDecl)
{
    const Variable* var = varDecl.var();
    const LayoutFlags backendFlags = var->layout().fFlags & LayoutFlag::kAllBackends;
    const LayoutFlags kPermittedBackendFlags = LayoutFlag::kVulkan | LayoutFlag::kWebGPU | LayoutFlag::kDirect3D;
    ...
    if (fUseTextureSamplerPairs && var->type().isSampler()) {
        if (var->layout().fTexture == -1 || var->layout().fSampler == -1) {
            fContext.fErrors->error(var->fPosition,
                "selected backend requires separate texture "
                "and sampler indices");
            return false;
        }
...
SpvId SPIRVCodeGenerator::writeGlobalVar(ProgramKind kind, StorageClass storageClass, const Variable& var)
{
    ...
    SpvId typeId = this->getPointerType(*type, layout, this->memoryLayoutForStorageClass(storageClass), storageClass);
    this->writeInstruction(SpvOpVariable, typeId, id, get_storage_class_spv_id(storageClass), fConstantBuffer);
...
SPIRVCodeGenerator::EntrypointAdapter SPIRVCodeGenerator::writeEntrypointAdapter(const FunctionDeclaration& main)
{
    ...
    auto skFragColorRef = std::make_unique<VariableReference>(Position(), &skFragColorVar, VariableReference::RefKind::kWrite);
    ...
    if (!main.returnType().matches(skFragColorRef->type())) {
        fContext.fErrors->error(main.fPosition, "SPIR-V does not support returning '" + main.returnType().description() + "' from main()");
        return {};
    }
    ExpressionArray args;
    if (main.parameters().size() == 1) {
        if (!main.parameters()[0]->type().matches(*fContext.fTypes.fFloat2)) {
            fContext.fErrors->error(main.fPosition, "SPIR-V does not support parameter of type '" + main.parameters()[0]->type().description() + "' to main()");
            return {};
        }
        double kZero[2] = { 0.0, 0.0 };
        args.push_back(ConstructorCompound::MakeFromConstants(fContext, Position {}, *fContext.fTypes.fFloat2, kZero));
    }
    auto callMainFn = FunctionCall::Make(fContext, Position(), &main.returnType(), main, std::move(args));

    // Synthesize `skFragColor = main()` as a BinaryExpression.
    auto assignmentStmt = std::make_unique<ExpressionStatement>(
        std::make_unique<BinaryExpression>(Position(), std::move(skFragColorRef), Operator::Kind::EQ, std::move(callMainFn), &main.returnType()));
    ...
    auto entrypointBlock = Block::Make(Position(), std::move(entrypointStmts), Block::Kind::kBracedScope, /*symbols=*/nullptr);
    // Declare an entrypoint function.
    EntrypointAdapter adapter;
    adapter.entrypointDecl = std::make_unique<FunctionDeclaration>(fContext, Position(), ModifierFlag::kNone, "_entrypoint",
        /*parameters=*/TArray<Variable*> {},
        /*returnType=*/fContext.fTypes.fVoid.get(), kNotIntrinsic);
    // Define it.
    adapter.entrypointDef = FunctionDefinition::Convert(fContext, Position(), *adapter.entrypointDecl, std::move(entrypointBlock));
...
void SPIRVCodeGenerator::writeUniformBuffer(SymbolTable* topLevelSymbolTable)
{
    ...
    fUniformBuffer.fStruct = Type::MakeStructType(fContext, Position(), kUniformBufferName, std::move(fields),
        /*interfaceBlock=*/true);
    ...
    fUniformBuffer.fInnerVariable = Variable::Make(/*pos=*/Position(),
        /*modifiersPosition=*/Position(), layout, ModifierFlag::kUniform, fUniformBuffer.fStruct.get(), kUniformBufferName,
        /*mangledName=*/"",
        /*builtin=*/false, Variable::Storage::kGlobal);

    // Create an interface block object for this global variable.
    fUniformBuffer.fInterfaceBlock = std::make_unique<InterfaceBlock>(Position(), fUniformBuffer.fInnerVariable.get());
...
void SPIRVCodeGenerator::addRTFlipUniform(Position pos)
{
    ...
    const Type* intfStruct = fSynthetics.takeOwnershipOfSymbol(Type::MakeStructType(fContext, Position(), name, std::move(fields), /*interfaceBlock=*/true));
    ...
    Layout layout(/*flags=*/usePushConstants ? LayoutFlag::kPushConstant : LayoutFlag::kNone,
        ...
        /*builtin=*/-1,
        /*inputAttachmentIndex=*/-1);
    Variable* intfVar = fSynthetics.takeOwnershipOfSymbol(Variable::Make(/*pos=*/Position(),
        /*modifiersPosition=*/Position(), layout, ModifierFlag::kUniform, intfStruct, name,
        /*mangledName=*/"",
        /*builtin=*/false, Variable::Storage::kGlobal));
    {
        AutoAttachPoolToThread attach(fProgram.fPool.get());
        fProgram.fSymbols->add(fContext, std::make_unique<FieldSymbol>(Position(), intfVar, /*field=*/0));
    }
    InterfaceBlock intf(Position(), intfVar);
    this->writeInterfaceBlock(intf, false);
}

std::tuple<const Variable*, const Variable*> SPIRVCodeGenerator::synthesizeTextureAndSampler(const Variable& combinedSampler)
{
    SkASSERT(fUseTextureSamplerPairs);
    SkASSERT(combinedSampler.type().typeKind() == Type::TypeKind::kSampler);

    if (std::unique_ptr<SynthesizedTextureSamplerPair>* existingData = fSynthesizedSamplerMap.find(&combinedSampler)) {
    ...
    auto texture = Variable::Make(/*pos=*/Position(),
        /*modifiersPosition=*/Position(), texLayout, combinedSampler.modifierFlags(), &combinedSampler.type().textureType(), data->fTextureName,
        /*mangledName=*/"",
        /*builtin=*/false, Variable::Storage::kGlobal);
    ...
    auto sampler = Variable::Make(/*pos=*/Position(),
        /*modifiersPosition=*/Position(), samplerLayout, combinedSampler.modifierFlags(), fContext.fTypes.fSampler.get(), data->fSamplerName,
        /*mangledName=*/"",
        /*builtin=*/false, Variable::Storage::kGlobal);
...
void SPIRVCodeGenerator::writeInstructions(const Program& program, OutputStream& out)
{
    Analysis::FindFunctionsToSpecialize(
        program, &fSpecializationInfo, [](const Variable& param) { return param.type().isSampler() || param.type().isUnsizedArray(); });
    ...
    for (const ProgramElement* e : program.elements()) {
        ...
        case ProgramElement::Kind::kFunction: {
            ...
            if (const Analysis::Specializations* specializations = fSpecializationInfo.fSpecializationMap.find(&funcDecl)) {
                for (int i = 0; i < specializations->size(); i++) {
    ...
    for (const ProgramElement* e : program.elements()) {
        if (e->is<InterfaceBlock>()) {
            ...
            if ((intf.var()->modifierFlags() & (ModifierFlag::kIn | ModifierFlag::kOut)) && intf.var()->layout().fBuiltin == -1) {
    ...
    if (main->returnType().matches(*fContext.fTypes.fHalf4)) {
        ...
        if (adapter.entrypointDecl) {
            fFunctionMap.set({ adapter.entrypointDecl.get(), Analysis::kUnspecialized }, this->nextId(nullptr));
    ...
    for (const auto& [var, spvId] : fVariableMap) {
        if (var->storage() == Variable::Storage::kGlobal && (var->modifierFlags() & (ModifierFlag::kIn | ModifierFlag::kOut))) {
    ...
    this->writeOpCode(SpvOpEntryPoint, (SpvId)(3 + (main->name().length() + 4) / 4) + (int32_t)interfaceVars.size(), out);
    ...
    if (ProgramConfig::IsFragment(program.fConfig->fKind)) {
        this->writeInstruction(SpvOpExecutionMode, fFunctionMap[mainKey], SpvExecutionModeOriginUpperLeft, out);
    } else if (ProgramConfig::IsCompute(program.fConfig->fKind)) {
        this->writeInstruction(SpvOpExecutionMode, fFunctionMap[mainKey], SpvExecutionModeLocalSize, localSizeX, localSizeY, localSizeZ, out);
    }
    for (const ProgramElement* e : program.elements()) {
        if (e->is<Extension>()) {
...
bool ToSPIRV(Program& program, const ShaderCaps* caps, OutputStream& out, ValidateSPIRVProc validateSPIRV)
{
...
bool ToSPIRV(Program& program, const ShaderCaps* caps, std::string* out, ValidateSPIRVProc validateSPIRV)
{
    StringStream buffer;
