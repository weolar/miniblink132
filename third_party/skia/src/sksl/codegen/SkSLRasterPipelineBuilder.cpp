...
#define ALL_SINGLE_SLOT_UNARY_OP_CASES                                                                                                                         \
    BuilderOp::acos_float : case BuilderOp::asin_float:                                                                                                        \
...
#define ALL_MULTI_SLOT_UNARY_OP_CASES                                                                                                                          \
    BuilderOp::abs_int : case BuilderOp::cast_to_float_from_int:                                                                                               \
...
#define ALL_N_WAY_BINARY_OP_CASES BuilderOp::atan2_n_floats : case BuilderOp::pow_n_floats

#define ALL_MULTI_SLOT_BINARY_OP_CASES                                                                                                                         \
    BuilderOp::add_n_floats : case BuilderOp::add_n_ints:                                                                                                      \
...
#define ALL_IMMEDIATE_BINARY_OP_CASES                                                                                                                          \
    BuilderOp::add_imm_float : case BuilderOp::add_imm_int:                                                                                                    \
...
#define ALL_MULTI_SLOT_TERNARY_OP_CASES BuilderOp::mix_n_floats : case BuilderOp::mix_n_ints
...
{
    switch (op) {
    case ALL_IMMEDIATE_MULTI_SLOT_BINARY_OP_CASES:
        return true;
    default:
        return false;
    }
...
void Builder::appendInstruction(BuilderOp op, SlotList slots, int immA, int immB, int immC, int immD)
{
    fInstructions.push_back({ op, slots.fSlotA, slots.fSlotB, immA, immB, immC, immD, fCurrentStackID });
}
...
void Builder::dot_floats(int32_t slots)
{
    switch (slots) {
    case 1:
        this->appendInstruction(BuilderOp::mul_n_floats, {}, slots);
        break;
    case 2:
        this->appendInstruction(BuilderOp::dot_2_floats, {}, slots);
        break;
    case 3:
        this->appendInstruction(BuilderOp::dot_3_floats, {}, slots);
        break;
    case 4:
        this->appendInstruction(BuilderOp::dot_4_floats, {}, slots);
        break;
...
void Builder::inverse_matrix(int32_t n)
{
    switch (n) {
    case 2:
        this->appendInstruction(BuilderOp::inverse_mat2, {}, 4);
        break;
    case 3:
        this->appendInstruction(BuilderOp::inverse_mat3, {}, 9);
        break;
    case 4:
        this->appendInstruction(BuilderOp::inverse_mat4, {}, 16);
        break;
    default:
        SkUNREACHABLE;
    }
...
bool Builder::simplifyImmediateUnmaskedOp()
{
    ...
    if (popInstruction && immInstruction && pushInstruction && popInstruction->fOp == BuilderOp::copy_stack_to_slots_unmasked) {
        ...
        if (is_immediate_op(immInstruction->fOp) && immInstruction->fImmA == popInstruction->fImmA) {
            ...
            if (immInstruction->fImmA == 1 || is_multi_slot_immediate_op(immInstruction->fOp)) {
                ...
                // that many slots...
                if ((pushInstruction->fOp == BuilderOp::push_slots || pushInstruction->fOp == BuilderOp::push_immutable)
...
void Builder::branch_if_any_lanes_active(int labelID)
{
    ...
    SkASSERT(labelID >= 0 && labelID < fNumLabels);
    if (const Instruction* lastInstruction = this->lastInstructionOnAnyStack()) {
        if (lastInstruction->fOp == BuilderOp::branch_if_any_lanes_active || lastInstruction->fOp == BuilderOp::jump) {
...
void Builder::branch_if_all_lanes_active(int labelID)
{
    ...
    SkASSERT(labelID >= 0 && labelID < fNumLabels);
    if (const Instruction* lastInstruction = this->lastInstructionOnAnyStack()) {
        if (lastInstruction->fOp == BuilderOp::branch_if_all_lanes_active || lastInstruction->fOp == BuilderOp::jump) {
...
void Builder::branch_if_no_lanes_active(int labelID)
{
    ...
    SkASSERT(labelID >= 0 && labelID < fNumLabels);
    if (const Instruction* lastInstruction = this->lastInstructionOnAnyStack()) {
        if (lastInstruction->fOp == BuilderOp::branch_if_no_lanes_active || lastInstruction->fOp == BuilderOp::jump) {
...
void Builder::branch_if_no_active_lanes_on_stack_top_equal(int value, int labelID)
{
    ...
    if (const Instruction* lastInstruction = this->lastInstructionOnAnyStack()) {
        if (lastInstruction->fOp == BuilderOp::jump
            || (lastInstruction->fOp == BuilderOp::branch_if_no_active_lanes_on_stack_top_equal && lastInstruction->fImmB == value)) {
    ...
    this->appendInstruction(BuilderOp::branch_if_no_active_lanes_on_stack_top_equal, {}, labelID, value);
}
...
void Builder::push_slots_or_immutable(SlotRange src, BuilderOp op)
{
    ...
    if (Instruction* lastInstruction = this->lastInstruction()) {
        ...
        if (lastInstruction->fOp == op && lastInstruction->fSlotA + lastInstruction->fImmA == src.index) {
    ...
    if (fInstructions.size() >= 3) {
        ...
        if (pushInst && discardInst && copyToSlotsInst && pushInst->fOp == BuilderOp::push_slots) {
            ...
            if (discardInst->fOp == BuilderOp::discard_stack && discardInst->fImmA == pushCount) {
                ...
                if ((copyToSlotsInst->fOp == BuilderOp::copy_stack_to_slots || copyToSlotsInst->fOp == BuilderOp::copy_stack_to_slots_unmasked)
...
void Builder::push_slots_or_immutable_indirect(SlotRange fixedRange, int dynamicStackID, SlotRange limitRange, BuilderOp op)
{
    // SlotA: fixed-range start
    // SlotB: limit-range end
    // immA: number of slots
    // immB: dynamic stack ID
    this->appendInstruction(op, { fixedRange.index, limitRange.index + limitRange.count }, fixedRange.count, dynamicStackID);
}
...
void Builder::push_uniform(SlotRange src)
{
    ...
    if (Instruction* lastInstruction = this->lastInstruction()) {
        ...
        if (lastInstruction->fOp == BuilderOp::push_uniform && lastInstruction->fSlotA + lastInstruction->fImmA == src.index) {
            lastInstruction->fImmA += src.count;
            return;
        }
    }
...
void Builder::push_uniform_indirect(SlotRange fixedRange, int dynamicStackID, SlotRange limitRange)
{
    // SlotA: fixed-range start
    // SlotB: limit-range end
    // immA: number of slots
    // immB: dynamic stack ID
    this->appendInstruction(BuilderOp::push_uniform_indirect, { fixedRange.index, limitRange.index + limitRange.count }, fixedRange.count, dynamicStackID);
}

void Builder::trace_var_indirect(int traceMaskStackID, SlotRange fixedRange, int dynamicStackID, SlotRange limitRange)
{
    // SlotA: fixed-range start
    // SlotB: limit-range end
    ...
    this->appendInstruction(
        BuilderOp::trace_var_indirect, { fixedRange.index, limitRange.index + limitRange.count }, traceMaskStackID, fixedRange.count, dynamicStackID);
}
...
void Builder::push_duplicates(int count)
{
    ...
    case 3:
        ...
        break;
    case 2:
        this->swizzle(/*consumedSlots=*/1, { 0, 0, 0 });
        break;
    case 1:
        this->push_clone(/*numSlots=*/1);
        break;
    default:
        break;
    }
...
void Builder::push_clone_from_stack(SlotRange range, int otherStackID, int offsetFromStackTop)
{
    ...
    this->appendInstruction(BuilderOp::push_clone_from_stack, {}, range.count, otherStackID, offsetFromStackTop);
}

void Builder::push_clone_indirect_from_stack(SlotRange fixedOffset, int dynamicStackID, int otherStackID, int offsetFromStackTop)
{
    // immA: number of slots
    // immB: other stack ID
    // immC: offset from stack top
    ...
    this->appendInstruction(BuilderOp::push_clone_indirect_from_stack, {}, fixedOffset.count, otherStackID, offsetFromStackTop, dynamicStackID);
}
...
void Builder::copy_stack_to_slots(SlotRange dst, int offsetFromStackTop)
{
    ...
    this->appendInstruction(BuilderOp::copy_stack_to_slots, { dst.index }, dst.count, offsetFromStackTop);
}

void Builder::copy_stack_to_slots_indirect(SlotRange fixedRange, int dynamicStackID, SlotRange limitRange)
{
    // SlotA: fixed-range start
    // SlotB: limit-range end
    // immA: number of slots
    // immB: dynamic stack ID
    this->appendInstruction(
        BuilderOp::copy_stack_to_slots_indirect, { fixedRange.index, limitRange.index + limitRange.count }, fixedRange.count, dynamicStackID);
}

static bool slot_ranges_overlap(SlotRange x, SlotRange y)
{
    return x.index < y.index + y.count && y.index < x.index + x.count;
}
...
void Builder::copy_slots_unmasked(SlotRange dst, SlotRange src)
{
    ...
    if (Instruction* lastInstr = this->lastInstruction()) {
        ...
        if (lastInstr->fOp == BuilderOp::copy_slot_unmasked &&
            ...
            !slot_ranges_overlap({ lastInstr->fSlotB, lastInstr->fImmA + dst.count }, { lastInstr->fSlotA, lastInstr->fImmA + dst.count })) {
            // then we can just extend the copy!
...
void Builder::copy_uniform_to_slots_unmasked(SlotRange dst, SlotRange src)
{
    ...
    SkASSERT(dst.count == src.count);
    this->appendInstruction(BuilderOp::copy_uniform_to_slots_unmasked, { src.index, dst.index }, dst.count);
}
...
void Builder::copy_stack_to_slots_unmasked(SlotRange dst, int offsetFromStackTop)
{
    ...
    this->appendInstruction(BuilderOp::copy_stack_to_slots_unmasked, { dst.index }, dst.count, offsetFromStackTop);
}
...
template <typename T> static void unpack_nybbles_to_offsets(uint32_t components, SkSpan<T> offsets)
{
...
void Builder::swizzle_copy_stack_to_slots(SlotRange dst, SkSpan<const int8_t> components, int offsetFromStackTop)
{
    ...
    // immC: offset from stack top
    this->appendInstruction(BuilderOp::swizzle_copy_stack_to_slots, { dst.index }, (int)components.size(), pack_nybbles(components), offsetFromStackTop);
}

void Builder::swizzle_copy_stack_to_slots_indirect(
    SlotRange fixedRange, int dynamicStackID, SlotRange limitRange, SkSpan<const int8_t> components, int offsetFromStackTop)
{
    // When the execution-mask writes-enabled flag is off, we could squeeze out a little bit of
    // extra speed here by implementing and using an unmasked version of this op.
    ...
    // immC: offset from stack top
    // immD: dynamic stack ID
    this->appendInstruction(BuilderOp::swizzle_copy_stack_to_slots_indirect, { fixedRange.index, limitRange.index + limitRange.count }, (int)components.size(),
        pack_nybbles(components), offsetFromStackTop, dynamicStackID);
}
...
void Builder::swizzle(int consumedSlots, SkSpan<const int8_t> components)
{
    ...
    if (consumedSlots <= 4 && numElements <= 4) {
        ...
        this->appendInstruction((BuilderOp)op, {}, consumedSlots, pack_nybbles(SkSpan(elements, numElements)));
        return;
    }
    ...
    this->appendInstruction(BuilderOp::shuffle, {}, consumedSlots, numElements, pack_nybbles(SkSpan(&elements[0], 8)), pack_nybbles(SkSpan(&elements[8], 8)));
}
...
void Builder::matrix_multiply(int leftColumns, int leftRows, int rightColumns, int rightRows)
{
    ...
    switch (leftColumns) {
    case 2:
        op = BuilderOp::matrix_multiply_2;
        break;
    case 3:
        op = BuilderOp::matrix_multiply_3;
        break;
    case 4:
        op = BuilderOp::matrix_multiply_4;
        break;
    default:
        SkDEBUGFAIL("unsupported matrix dimensions");
        return;
    }
...
std::unique_ptr<Program> Builder::finish(int numValueSlots, int numUniformSlots, int numImmutableSlots, DebugTracePriv* debugTrace)
{
    ...
    return std::make_unique<Program>(std::move(fInstructions), numValueSlots, numUniformSlots, numImmutableSlots, fNumLabels, debugTrace);
}
...
Program::Program(TArray<Instruction> instrs, int numValueSlots, int numUniformSlots, int numImmutableSlots, int numLabels, DebugTracePriv* debugTrace)
...
void Program::appendCopy(TArray<Stage>* pipeline, SkArenaAlloc* alloc,
    std::byte* basePtr, // only used for immutable-value copies
    ProgramOp baseStage, SkRPOffset dst, int dstStride, SkRPOffset src, int srcStride, int numSlots) const
{
    SkASSERT(numSlots >= 0);
    while (numSlots > 4) {
        // If we are appending a large copy, split it up into groups of four at a time.
        this->appendCopy(pipeline, alloc, basePtr, baseStage, dst, dstStride, src, srcStride,
            /*numSlots=*/4);
...
void Program::appendCopySlotsUnmasked(TArray<Stage>* pipeline, SkArenaAlloc* alloc, SkRPOffset dst, SkRPOffset src, int numSlots) const
{
    this->appendCopy(pipeline, alloc, /*basePtr=*/nullptr, ProgramOp::copy_slot_unmasked, dst, SkOpts::raster_pipeline_highp_stride, src,
        SkOpts::raster_pipeline_highp_stride, numSlots);
}

void Program::appendCopyImmutableUnmasked(TArray<Stage>* pipeline, SkArenaAlloc* alloc, std::byte* basePtr, SkRPOffset dst, SkRPOffset src, int numSlots) const
{
    this->appendCopy(pipeline, alloc, basePtr, ProgramOp::copy_immutable_unmasked, dst, SkOpts::raster_pipeline_highp_stride, src, 1, numSlots);
}

void Program::appendCopySlotsMasked(TArray<Stage>* pipeline, SkArenaAlloc* alloc, SkRPOffset dst, SkRPOffset src, int numSlots) const
{
    this->appendCopy(pipeline, alloc, /*basePtr=*/nullptr, ProgramOp::copy_slot_masked, dst, SkOpts::raster_pipeline_highp_stride, src,
        SkOpts::raster_pipeline_highp_stride, numSlots);
}

void Program::appendSingleSlotUnaryOp(TArray<Stage>* pipeline, ProgramOp stage, float* dst, int numSlots) const
{
    SkASSERT(numSlots >= 0);
    while (numSlots--) {
...
void Program::appendMultiSlotUnaryOp(TArray<Stage>* pipeline, ProgramOp baseStage, float* dst, int numSlots) const
{
    SkASSERT(numSlots >= 0);
    while (numSlots > 0) {
...
void Program::appendImmediateBinaryOp(TArray<Stage>* pipeline, SkArenaAlloc* alloc, ProgramOp baseStage, SkRPOffset dst, int32_t value, int numSlots) const
{
...
void Program::appendAdjacentNWayBinaryOp(TArray<Stage>* pipeline, SkArenaAlloc* alloc, ProgramOp stage, SkRPOffset dst, SkRPOffset src, int numSlots) const
{
...
void Program::appendAdjacentMultiSlotBinaryOp(
    TArray<Stage>* pipeline, SkArenaAlloc* alloc, ProgramOp baseStage, std::byte* basePtr, SkRPOffset dst, SkRPOffset src, int numSlots) const
{
    // The source and destination must be directly next to one another.
    SkASSERT(numSlots >= 0);
    SkASSERT((dst + SkOpts::raster_pipeline_highp_stride * numSlots * sizeof(float)) == src);
...
void Program::appendAdjacentNWayTernaryOp(
    TArray<Stage>* pipeline, SkArenaAlloc* alloc, ProgramOp stage, std::byte* basePtr, SkRPOffset dst, SkRPOffset src0, SkRPOffset src1, int numSlots) const
{
...
void Program::appendAdjacentMultiSlotTernaryOp(
    TArray<Stage>* pipeline, SkArenaAlloc* alloc, ProgramOp baseStage, std::byte* basePtr, SkRPOffset dst, SkRPOffset src0, SkRPOffset src1, int numSlots) const
{
    // The float pointers must all be immediately adjacent to each other.
    SkASSERT(numSlots >= 0);
    SkASSERT((dst + SkOpts::raster_pipeline_highp_stride * numSlots * sizeof(float)) == src0);
    ...
    if (numSlots > 4) {
        this->appendAdjacentNWayTernaryOp(pipeline, alloc, baseStage, basePtr, dst, src0, src1, numSlots);
        return;
    }
    if (numSlots > 0) {
        auto specializedStage = (ProgramOp)((int)baseStage + numSlots);
...
std::optional<Program::SlotData> Program::allocateSlotData(SkArenaAlloc* alloc) const
{
    ...
    SkSafeMath safe;
    size_t allocSize = safe.add(safe.mul(vectorWidth, safe.add(fNumValueSlots, fNumTempStackSlots)), safe.mul(scalarWidth, fNumImmutableSlots));
...
bool Program::appendStages(SkRasterPipeline* pipeline, SkArenaAlloc* alloc, RP::Callbacks* callbacks, SkSpan<const float> uniforms) const
{
#if defined(SKSL_STANDALONE)
    return false;
#else
...
void Program::makeStages(TArray<Stage>* pipeline, SkArenaAlloc* alloc, SkSpan<const float> uniforms, const SlotData& slots) const
{
    ...
    auto OffsetFromBase = [&](const void* ptr) -> SkRPOffset { return (SkRPOffset)((const std::byte*)ptr - basePtr); };
    ...
    for (int instructionIdx = 0; instructionIdx < fInstructions.size(); ++instructionIdx) {
        ...
        case ALL_IMMEDIATE_BINARY_OP_CASES: {
            float* dst = (inst.fSlotA == NA) ? tempStackPtr - (inst.fImmA * N) : SlotA();

            this->appendImmediateBinaryOp(pipeline, alloc, (ProgramOp)inst.fOp, OffsetFromBase(dst), inst.fImmB, inst.fImmA);
            break;
        }
        case ALL_N_WAY_BINARY_OP_CASES: {
            float* src = tempStackPtr - (inst.fImmA * N);
            float* dst = tempStackPtr - (inst.fImmA * 2 * N);
            this->appendAdjacentNWayBinaryOp(pipeline, alloc, (ProgramOp)inst.fOp, OffsetFromBase(dst), OffsetFromBase(src), inst.fImmA);
            break;
        }
        case ALL_MULTI_SLOT_BINARY_OP_CASES: {
            float* src = tempStackPtr - (inst.fImmA * N);
            float* dst = tempStackPtr - (inst.fImmA * 2 * N);
            this->appendAdjacentMultiSlotBinaryOp(pipeline, alloc, (ProgramOp)inst.fOp, basePtr, OffsetFromBase(dst), OffsetFromBase(src), inst.fImmA);
            break;
        }
        case ALL_N_WAY_TERNARY_OP_CASES: {
            ...
            this->appendAdjacentNWayTernaryOp(
                pipeline, alloc, (ProgramOp)inst.fOp, basePtr, OffsetFromBase(dst), OffsetFromBase(src0), OffsetFromBase(src1), inst.fImmA);
            break;
        }
        case ALL_MULTI_SLOT_TERNARY_OP_CASES: {
            float* src1 = tempStackPtr - (inst.fImmA * N);
            float* src0 = tempStackPtr - (inst.fImmA * 2 * N);
            float* dst = tempStackPtr - (inst.fImmA * 3 * N);
            this->appendAdjacentMultiSlotTernaryOp(
                pipeline, alloc, (ProgramOp)inst.fOp, basePtr, OffsetFromBase(dst), OffsetFromBase(src0), OffsetFromBase(src1), inst.fImmA);
            break;
        }
        case BuilderOp::select: {
            float* src = tempStackPtr - (inst.fImmA * N);
            float* dst = tempStackPtr - (inst.fImmA * 2 * N);
            this->appendCopySlotsMasked(pipeline, alloc, OffsetFromBase(dst), OffsetFromBase(src), inst.fImmA);
            break;
        }
        case BuilderOp::copy_slot_masked:
            this->appendCopySlotsMasked(pipeline, alloc, OffsetFromBase(SlotA()), OffsetFromBase(SlotB()), inst.fImmA);
            break;

        case BuilderOp::copy_slot_unmasked:
            this->appendCopySlotsUnmasked(pipeline, alloc, OffsetFromBase(SlotA()), OffsetFromBase(SlotB()), inst.fImmA);
            break;

        case BuilderOp::copy_immutable_unmasked:
            this->appendCopyImmutableUnmasked(pipeline, alloc, basePtr, OffsetFromBase(SlotA()), OffsetFromBase(ImmutableB()), inst.fImmA);
            break;
        ...
        case BuilderOp::swizzle_1: {
            ...
            if (src != dst) {
                this->appendCopySlotsUnmasked(pipeline, alloc, OffsetFromBase(dst), OffsetFromBase(src),
                    /*numSlots=*/1);
            }
            break;
        }
        ...
        case BuilderOp::push_slots: {
            float* dst = tempStackPtr;
            this->appendCopySlotsUnmasked(pipeline, alloc, OffsetFromBase(dst), OffsetFromBase(SlotA()), inst.fImmA);
            break;
        }
        case BuilderOp::push_immutable: {
            float* dst = tempStackPtr;
            this->appendCopyImmutableUnmasked(pipeline, alloc, basePtr, OffsetFromBase(dst), OffsetFromBase(ImmutableA()), inst.fImmA);
            break;
        }
        ...
        case BuilderOp::push_uniform_indirect: {
            ...
            ctx->indirectOffset = reinterpret_cast<const uint32_t*>(tempStackMap[inst.fImmB]) - (1 * N);
        ...
        case BuilderOp::copy_uniform_to_slots_unmasked: {
            ...
            for (int remaining = inst.fImmA; remaining > 0; remaining -= 4) {
                ...
                switch (remaining) {
                case 1:
                    pipeline->push_back({ ProgramOp::copy_uniform, ctx });
                    break;
                case 2:
                    pipeline->push_back({ ProgramOp::copy_2_uniforms, ctx });
                    break;
                case 3:
                    pipeline->push_back({ ProgramOp::copy_3_uniforms, ctx });
                    break;
                default:
                    pipeline->push_back({ ProgramOp::copy_4_uniforms, ctx });
                    break;
                }
                dst += 4 * N;
                src += 4;
        ...
        case BuilderOp::push_constant: {
            ...
            for (int remaining = inst.fImmA; remaining > 0; remaining -= 4) {
                ...
                switch (remaining) {
                case 1:
                    pipeline->push_back({ ProgramOp::copy_constant, ptr });
                    break;
                case 2:
                    pipeline->push_back({ ProgramOp::splat_2_constants, ptr });
                    break;
                case 3:
                    pipeline->push_back({ ProgramOp::splat_3_constants, ptr });
                    break;
                default:
                    pipeline->push_back({ ProgramOp::splat_4_constants, ptr });
                    break;
                }
                dst += 4 * N;
            }
        ...
        case BuilderOp::copy_stack_to_slots: {
            ...
            this->appendCopySlotsMasked(pipeline, alloc, OffsetFromBase(SlotA()), OffsetFromBase(src), inst.fImmA);
            break;
        }
        case BuilderOp::copy_stack_to_slots_unmasked: {
            float* src = tempStackPtr - (inst.fImmB * N);
            this->appendCopySlotsUnmasked(pipeline, alloc, OffsetFromBase(SlotA()), OffsetFromBase(src), inst.fImmA);
            break;
        }
        ...
        case BuilderOp::push_clone: {
            float* src = tempStackPtr - (inst.fImmB * N);
            float* dst = tempStackPtr;
            this->appendCopySlotsUnmasked(pipeline, alloc, OffsetFromBase(dst), OffsetFromBase(src), inst.fImmA);
            break;
        }
        case BuilderOp::push_clone_from_stack: {
            // immA: number of slots
        ...
        case BuilderOp::push_clone_indirect_from_stack: {
            ...
            ctx->indirectOffset = reinterpret_cast<const uint32_t*>(tempStackMap[inst.fImmD]) - (1 * N);
        ...
        case BuilderOp::swizzle_copy_stack_to_slots_indirect: {
            ...
            ctx->dst = reinterpret_cast<int32_t*>(SlotA());
            ctx->indirectOffset = reinterpret_cast<const uint32_t*>(tempStackMap[inst.fImmD]) - (1 * N);
            ctx->indirectLimit = inst.fSlotB - inst.fSlotA - (max_packed_nybble(inst.fImmB, inst.fImmA) + 1);
            ctx->slots = inst.fImmA;
            unpack_nybbles_to_offsets(inst.fImmB, SkSpan(ctx->offsets));
        ...
        case BuilderOp::trace_var_indirect: {
            ...
            if (inst.fOp == BuilderOp::trace_var_indirect) {
                ctx->indirectOffset = reinterpret_cast<const uint32_t*>(tempStackMap[inst.fImmC]) - (1 * N);
...
public:
    ...
    std::string branchOffset(const SkRasterPipeline_BranchCtx* ctx, int index) const
    {
        ...
        return SkSL::String::printf("%+d (label %d at #%d)", *targetIndex - index, labelID, *targetIndex + 1);
    }
    ...
    std::string immutablePtrCtx(const float* ptr, int numSlots) const
    {
        ...
        if (ptr >= fSlots.immutable.begin() && end <= fSlots.immutable.end()) {
            ...
            return 'i' + this->asRange(index, numSlots) + ' ' + this->multiImmCtx(ptr, numSlots);
        }
        return {};
    }
    ...
    std::tuple<std::string, std::string> constantCtx(const void* v, int slots, bool showAsFloat = true) const
    {
        ...
        return { this->offsetCtx(ctx.dst, slots), this->imm(sk_bit_cast<float>(ctx.value), showAsFloat) };
    }
    ...
    std::tuple<std::string, std::string> binaryOpCtx(const void* v, int numSlots) const
    {
        ...
        return { this->offsetCtx(ctx.dst, numSlots), this->offsetCtx(ctx.src, numSlots) };
    }
    ...
    std::tuple<std::string, std::string> copyUniformCtx(const void* v, int numSlots) const
    {
        ...
        return { this->ptrCtx(ctx->dst, numSlots), this->multiImmCtx(reinterpret_cast<const float*>(ctx->src), numSlots) };
    }
    ...
    {
        const float* ctxAsSlot = static_cast<const float*>(ctx);
        return std::make_tuple(this->ptrCtx(ctxAsSlot, numSlots), this->ptrCtx(ctxAsSlot + (N * numSlots), numSlots));
    }
    ...
    {
        const float* ctxAsSlot = static_cast<const float*>(ctx);
        return { this->ptrCtx(ctxAsSlot, numSlots), this->ptrCtx(ctxAsSlot + (N * numSlots), numSlots),
    ...
    template <typename T> std::string swizzleOffsetSpan(SkSpan<T> offsets) const
    {
        std::string src;
    ...
    template <typename T> size_t swizzleWidth(SkSpan<T> offsets) const
    {
        size_t highestComponent = *std::max_element(offsets.begin(), offsets.end()) / (N * sizeof(float));
        size_t swizzleWidth = offsets.size();
        return std::max(swizzleWidth, highestComponent + 1);
    }

    // Stringizes a swizzled pointer.
    template <typename T> std::string swizzlePtr(const void* ptr, SkSpan<T> offsets) const
    {
        return "(" + this->ptrCtx(ptr, this->swizzleWidth(SkSpan(offsets))) + ")." + this->swizzleOffsetSpan(SkSpan(offsets));
    }
    ...
    std::tuple<std::string, std::string> swizzleCtx(ProgramOp op, const void* v) const
    {
        ...
        return { this->offsetCtx(ctx.dst, destSlots), this->swizzlePtr(this->offsetToPtr(ctx.dst), SkSpan(ctx.offsets, destSlots)) };
    }
    ...
    std::tuple<std::string, std::string> swizzleCopyCtx(ProgramOp op, const void* v) const
    {
        ...
        return { this->swizzlePtr(ctx->dst, SkSpan(ctx->offsets, destSlots)), this->ptrCtx(ctx->src, destSlots) };
    }
    ...
    std::tuple<std::string, std::string, std::string> matrixMultiply(const void* v) const
    {
        ...
        return { SkSL::String::printf("mat%dx%d(%s)", ctx.rightColumns, ctx.leftRows, this->offsetCtx(ctx.dst, resultMatrix).c_str()),
            SkSL::String::printf("mat%dx%d(%s)", ctx.leftColumns, ctx.leftRows, this->offsetCtx(leftOffset, leftMatrix).c_str()),
            SkSL::String::printf("mat%dx%d(%s)", ctx.rightColumns, ctx.rightRows, this->offsetCtx(rightOffset, rightMatrix).c_str()) };
    }
...
void Program::Dumper::dump(SkWStream* out, bool writeInstructionCount)
{
    ...
    for (int index = 0; index < fStages.size(); ++index) {
        ...
        case POp::swizzle_copy_to_indirect_masked: {
            ...
            opArg1 = this->ptrCtx(ctx->dst, this->swizzleWidth(SkSpan(ctx->offsets, ctx->slots)));
            opArg2 = this->ptrCtx(ctx->src, ctx->slots);
            opArg3 = this->ptrCtx(ctx->indirectOffset, 1);
        ...
        case POp::max_int:
        case POp::max_uint:
        case POp::cmplt_float:
        case POp::cmplt_int:
        case POp::cmplt_uint:
        case POp::cmple_float:
        case POp::cmple_int:
        case POp::cmple_uint:
        case POp::cmpeq_float:
        case POp::cmpeq_int:
        case POp::cmpne_float:
        case POp::cmpne_int:
        ...
        case POp::add_2_ints:
        case POp::sub_2_floats:
        case POp::sub_2_ints:
        case POp::mul_2_floats:
        case POp::mul_2_ints:
        case POp::div_2_floats:
        case POp::div_2_ints:
        ...
        case POp::add_3_floats:
        case POp::add_3_ints:
        case POp::sub_3_floats:
        case POp::sub_3_ints:
        case POp::mul_3_floats:
        case POp::mul_3_ints:
        case POp::div_3_floats:
        case POp::div_3_ints:
        ...
        case POp::add_4_floats:
        case POp::add_4_ints:
        case POp::sub_4_floats:
        case POp::sub_4_ints:
        case POp::mul_4_floats:
        case POp::mul_4_ints:
        case POp::div_4_floats:
        case POp::div_4_ints:
        ...
        case POp::add_n_floats:
        case POp::add_n_ints:
        case POp::sub_n_floats:
        case POp::sub_n_ints:
        case POp::mul_n_floats:
        case POp::mul_n_ints:
        case POp::div_n_floats:
        case POp::div_n_ints:
        ...
        case POp::branch_if_no_lanes_active:
            opArg1 = this->branchOffset(static_cast<SkRasterPipeline_BranchCtx*>(stage.ctx), index);
            break;
        ...
        case POp::trace_exit: {
            ...
            opArg1 = this->ptrCtx(ctx->traceMask, 1);
            opArg2 = (fProgram.fDebugTrace && ctx->funcIdx >= 0 && ctx->funcIdx < (int)fProgram.fDebugTrace->fFuncInfo.size())
...
#undef M
        ...
        case POp::swizzle_copy_to_indirect_masked:
            opText = "Indirect(" + opArg1 + " + " + opArg3 + ")." + opSwizzle + " = Mask(" + opArg2 + ")";
            break;
        ...
        case POp::case_op:
            opText = "if (" + opArg1 + " == " + opArg3 + ") { LoopMask = true; " + opArg2 + " = false; }";
            break;

        case POp::continue_op:
            opText = opArg1 + " |= Mask(0xFFFFFFFF); LoopMask &= ~(CondMask & LoopMask & RetMask)";
            break;
        ...
        if (!opText.empty()) {
            out->writeText(SkSL::String::printf("%-30.*s %s\n", (int)opName.size(), opName.data(), opText.c_str()).c_str());
        } else {
