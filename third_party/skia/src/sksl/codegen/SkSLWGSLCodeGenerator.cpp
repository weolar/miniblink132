...
public:
    ...
    WGSLCodeGenerator(const Context* context, const ShaderCaps* caps, const Program* program, OutputStream* out, PrettyPrint pp, IncludeSyntheticCode isc)
...
private:
    ...
    void writePipelineIODeclaration(const Layout& layout, const Type& type, ModifierFlags modifiers, std::string_view name, Delimiter delimiter);
    void writeUserDefinedIODecl(const Layout& layout, const Type& type, ModifierFlags modifiers, std::string_view name, Delimiter delimiter);
    ...
    void writeFunctionDeclaration(const FunctionDeclaration& f, SkSpan<const bool> paramNeedsDedicatedStorage);
    ...
    void writeEmulatedSwitchFallthroughCases(SkSpan<const SwitchCase* const> cases, std::string_view switchValue);
    ...
    std::string assembleBinaryExpression(const Expression& left, Operator op, const Expression& right, const Type& resultType, Precedence parentPrecedence);
    ...
    std::string assembleIntrinsicCall(const FunctionCall& call, IntrinsicKind kind, Precedence parentPrecedence);
    std::string assembleSimpleIntrinsic(std::string_view intrinsicName, const FunctionCall& call);
    std::string assembleUnaryOpIntrinsic(Operator op, const FunctionCall& call, Precedence parentPrecedence);
    std::string assembleBinaryOpIntrinsic(Operator op, const FunctionCall& call, Precedence parentPrecedence);
    std::string assembleVectorizedIntrinsic(std::string_view intrinsicName, const FunctionCall& call);
    std::string assembleOutAssignedIntrinsic(
        std::string_view intrinsicName, std::string_view returnFieldName, std::string_view outFieldName, const FunctionCall& call);
    std::string assemblePartialSampleCall(std::string_view intrinsicName, const Expression& sampler, const Expression& coords);
    std::string assembleInversePolyfill(const FunctionCall& call);
    std::string assembleComponentwiseMatrixBinary(const Type& leftType, const Type& rightType, const std::string& left, const std::string& right, Operator op);
    ...
    std::string assembleEqualityExpression(
        const Type& left, const std::string& leftName, const Type& right, const std::string& rightName, Operator op, Precedence parentPrecedence);
    std::string assembleEqualityExpression(const Expression& left, const Expression& right, Operator op, Precedence parentPrecedence);
    ...
    void prepareUniformPolyfillsForInterfaceBlock(const InterfaceBlock* interfaceBlock, std::string_view instanceName, MemoryLayout::Standard nativeLayout);
    void writeEnables();
    void writeUniformPolyfills();

    void writeTextureOrSampler(const Variable& var, int bindingLocation, std::string_view suffix, std::string_view wgslType);
...
std::string to_wgsl_type(const Context& context, const Type& raw, const Layout* layout = nullptr)
{
    ...
    case Type::TypeKind::kMatrix: {
        std::string_view ct = to_scalar_type(type.componentType());
        return String::printf("mat%dx%d<%.*s>", type.columns(), type.rows(), (int)ct.length(), ct.data());
    }
...
std::string to_ptr_type(const Context& context, const Type& type, const Layout* layout, PtrAddressSpace addressSpace = PtrAddressSpace::kFunction)
{
    return "ptr<" + std::string(address_space_to_str(addressSpace)) + ", " + to_wgsl_type(context, type, layout) + '>';
}
...
public:
    ...
    FunctionDependencyResolver(const Program* p, const FunctionDeclaration* f, DepsMap* programDependencyMap)
        : fProgram(p)
        , fFunction(f)
        , fDependencyMap(programDependencyMap)
    {
    }
...
void collect_pipeline_io_vars(const Program* program, TArray<const Variable*>* ioVars, ModifierFlag ioType)
{
    for (const ProgramElement* e : program->elements()) {
...
bool is_in_global_uniforms(const Variable& var)
{
    ...
    return var.modifierFlags().isUniform() && !var.type().isOpaque() && !var.interfaceBlock();
}
...
void WGSLCodeGenerator::writeUniformPolyfills()
{
    ...
    fFieldPolyfillMap.foreach ([&](const FieldPolyfillMap::Pair& pair) { orderedFields.push_back(&pair); });

    std::sort(orderedFields.begin(), orderedFields.end(),
        [](const FieldPolyfillMap::Pair* a, const FieldPolyfillMap::Pair* b) { return a->second.fReplacementName < b->second.fReplacementName; });
...
std::string WGSLCodeGenerator::assembleName(std::string_view name)
{
    ...
    return (skstd::starts_with(name, "_sk") || skstd::starts_with(name, "R_") || is_reserved_word(name)) ? std::string("R_") + std::string(name)
                                                                                                         : std::string(name);
}

void WGSLCodeGenerator::writeVariableDecl(const Layout& layout, const Type& type, std::string_view name, Delimiter delimiter)
{
...
void WGSLCodeGenerator::writePipelineIODeclaration(const Layout& layout, const Type& type, ModifierFlags modifiers, std::string_view name, Delimiter delimiter)
{
...
void WGSLCodeGenerator::writeUserDefinedIODecl(const Layout& layout, const Type& type, ModifierFlags flags, std::string_view name, Delimiter delimiter)
{
    ...
    if (flags.isFlat() || type.isInteger() || (type.isVector() && type.componentType().isInteger())) {
...
void WGSLCodeGenerator::writeBuiltinIODecl(const Type& type, std::string_view name, Builtin builtin, Delimiter delimiter)
{
...
void WGSLCodeGenerator::writeFunctionDeclaration(const FunctionDeclaration& decl, SkSpan<const bool> paramNeedsDedicatedStorage)
{
    this->write("fn ");
    if (decl.isMain()) {
...
void WGSLCodeGenerator::writeEntryPoint(const FunctionDefinition& main)
{
    ...
    if (fGenSyntheticCode == IncludeSyntheticCode::kYes && ProgramConfig::IsRuntimeShader(programKind)) {
    ...
    if (fGenSyntheticCode == IncludeSyntheticCode::kYes) {
        if (const Variable* v = main.declaration().getMainCoordsParameter()) {
            ...
            if (!type.matches(*fContext.fTypes.fFloat2)) {
                fContext.fErrors->error(main.fPosition, "main function has unsupported parameter: " + type.description());
                return;
            }
            this->write(separator());
...
void WGSLCodeGenerator::writeDoStatement(const DoStatement& s)
{
    ...
    std::unique_ptr<Expression> invertedTestExpr = PrefixExpression::Make(fContext, s.test()->fPosition, OperatorKind::LOGICALNOT, s.test()->clone());
...
void WGSLCodeGenerator::writeForStatement(const ForStatement& s)
{
    ...
    if (s.unrollInfo()) {
        if (s.unrollInfo()->fCount <= 0) {
            ...
            if (s.test()) {
                std::unique_ptr<Expression> invertedTestExpr
                    = PrefixExpression::Make(fContext, s.test()->fPosition, OperatorKind::LOGICALNOT, s.test()->clone());
...
void WGSLCodeGenerator::writeReturnStatement(const ReturnStatement& s)
{
    ...
    std::string expr = s.expression() ? this->assembleExpression(*s.expression(), Precedence::kExpression) : std::string();
    this->write("return ");
    this->write(expr);
    this->write(";");
}
...
void WGSLCodeGenerator::writeSwitchCases(SkSpan<const SwitchCase* const> cases)
{
    if (!cases.empty()) {
        ...
        SkASSERT(std::all_of(cases.begin(), std::prev(cases.end()), [](const SwitchCase* sc) { return sc->statement()->isEmpty(); }));
...
void WGSLCodeGenerator::writeEmulatedSwitchFallthroughCases(SkSpan<const SwitchCase* const> cases, std::string_view switchValue)
{
...
{
    std::string initialValue = varDecl.value() ? this->assembleExpression(*varDecl.value(), Precedence::kAssignment) : std::string();

    if (varDecl.var()->modifierFlags().isConst() || (fProgram.fUsage->get(*varDecl.var()).fWrite == 1 && varDecl.value())) {
        ...
        const bool useConst = !fAtFunctionScope || Analysis::IsCompileTimeConstant(*varDecl.value());
...
{
    if (e.is<VariableReference>()) {
        return std::make_unique<PointerLValue>(this->variableReferenceNameForLValue(e.as<VariableReference>()));
    }
    if (e.is<FieldAccess>()) {
    ...
    if (e.is<IndexExpression>()) {
        ...
        if (idx.base()->type().isVector()) {
            ...
            if (std::unique_ptr<Expression> rewrite = Transform::RewriteIndexedSwizzle(fContext, idx)) {
                return std::make_unique<VectorComponentLValue>(this->assembleExpression(*rewrite, Precedence::kAssignment));
            } else {
    ...
    if (e.is<Swizzle>()) {
        ...
        if (swizzle.components().size() == 1) {
            ...
            return std::make_unique<SwizzleLValue>(
                fContext, this->assembleExpression(*swizzle.base(), Precedence::kAssignment), swizzle.base()->type(), swizzle.components());
        }
    }
...
std::string WGSLCodeGenerator::assembleExpression(const Expression& e, Precedence parentPrecedence)
{
    switch (e.kind()) {
    ...
    case Expression::Kind::kConstructorArrayCast:
        ...
        return this->assembleExpression(*e.as<ConstructorArrayCast>().argument(), parentPrecedence);
...
bool WGSLCodeGenerator::binaryOpNeedsComponentwiseMatrixPolyfill(const Type& left, const Type& right, Operator op)
{
    switch (op.kind()) {
    ...
    case OperatorKind::MINUS:
        ...
        return (left.isMatrix() && right.isScalar()) || (left.isScalar() && right.isMatrix());
...
std::string WGSLCodeGenerator::assembleBinaryExpression(const BinaryExpression& b, Precedence parentPrecedence)
{
    return this->assembleBinaryExpression(*b.left(), b.getOperator(), *b.right(), b.type(), parentPrecedence);
}

std::string WGSLCodeGenerator::assembleBinaryExpression(
    const Expression& left, Operator op, const Expression& right, const Type& resultType, Precedence parentPrecedence)
{
    ...
    bool bothSidesConstant = ConstantFolder::GetConstantValueOrNull(left) && ConstantFolder::GetConstantValueOrNull(right);
...
std::string WGSLCodeGenerator::assembleFieldAccess(const FieldAccess& f)
{
    ...
    case FieldAccess::OwnerKind::kAnonymousInterfaceBlock:
        if (f.base()->is<VariableReference>() && field->fLayout.fBuiltin != SK_POINTSIZE_BUILTIN) {
...
std::string WGSLCodeGenerator::assembleSimpleIntrinsic(std::string_view intrinsicName, const FunctionCall& call)
{
...
std::string WGSLCodeGenerator::assembleVectorizedIntrinsic(std::string_view intrinsicName, const FunctionCall& call)
{
    ...
    for (int index = 0; index < args.size(); ++index) {
        ...
        // We can use a scratch-let for argument 0 to dodge WGSL overflow errors. (skia:14385)
        std::string argument = this->assembleExpression(*args[index], Precedence::kSequence);
        expr += (allConstant && index == 0) ? this->writeScratchLet(argument) : argument;
        if (vectorize) {
            expr.push_back(')');
        }
    }
...
std::string WGSLCodeGenerator::assembleUnaryOpIntrinsic(Operator op, const FunctionCall& call, Precedence parentPrecedence)
{
    SkASSERT(!call.type().isVoid());
...
std::string WGSLCodeGenerator::assembleBinaryOpIntrinsic(Operator op, const FunctionCall& call, Precedence parentPrecedence)
{
    SkASSERT(!call.type().isVoid());

    Precedence precedence = op.getBinaryPrecedence();
    bool needParens = precedence >= parentPrecedence || binary_op_is_ambiguous_in_wgsl(op);
    std::string expr;
    if (needParens) {
        expr.push_back('(');
    }
    ...
    expr += all_arguments_constant(call.arguments()) ? this->writeScratchLet(argument) : argument;
    expr += operator_name(op);
...
std::string WGSLCodeGenerator::assembleOutAssignedIntrinsic(
    std::string_view intrinsicName, std::string_view returnField, std::string_view outField, const FunctionCall& call)
{
    ...
    expr += ConstantFolder::GetConstantValueOrNull(*call.arguments()[0]) ? this->writeScratchLet(argument) : argument;
    expr += ")";
...
std::string WGSLCodeGenerator::assemblePartialSampleCall(std::string_view functionName, const Expression& sampler, const Expression& coords)
{
...
std::string WGSLCodeGenerator::assembleComponentwiseMatrixBinary(
    const Type& leftType, const Type& rightType, const std::string& left, const std::string& right, Operator op)
{
...
std::string WGSLCodeGenerator::assembleIntrinsicCall(const FunctionCall& call, IntrinsicKind kind, Precedence parentPrecedence)
{
    ...
    case k_faceforward_IntrinsicKind: {
        if (arguments[0]->type().isScalar()) {
            // select(-N, N, (I * Nref) < 0)
            std::string N = this->writeNontrivialScratchLet(*arguments[0], Precedence::kAssignment);
            return this->writeScratchLet("select(-" + N + ", " + N + ", "
                + this->assembleBinaryExpression(*arguments[1], OperatorKind::STAR, *arguments[2], arguments[1]->type(), Precedence::kRelational) + " < 0)");
        }
    ...
    case k_matrixCompMult_IntrinsicKind: {
        ...
        std::string arg0 = all_arguments_constant(arguments) ? this->writeScratchLet(*arguments[0], Precedence::kPostfix)
        ...
        return this->writeScratchLet(this->assembleComponentwiseMatrixBinary(arguments[0]->type(), arguments[1]->type(), arg0, arg1, OperatorKind::STAR));
    }
    ...
    case k_mod_IntrinsicKind: {
        ...
        std::string arg0 = all_arguments_constant(arguments) ? this->writeScratchLet(*arguments[0], Precedence::kAdditive)
                                                             : this->writeNontrivialScratchLet(*arguments[0], Precedence::kAdditive);
        std::string arg1 = this->writeNontrivialScratchLet(*arguments[1], Precedence::kAdditive);
        return this->writeScratchLet(arg0 + " - " + arg1 + " * floor(" + arg0 + " / " + arg1 + ")");
    }
    ...
    case k_reflect_IntrinsicKind:
        if (arguments[0]->type().isScalar()) {
            ...
            std::string I = this->writeNontrivialScratchLet(*arguments[0], Precedence::kAdditive);
            ...
            return this->writeScratchLet(String::printf("%s - 2 * %s * %s * %s", I.c_str(), N.c_str(), I.c_str(), N.c_str()));
        }
    ...
    case k_refract_IntrinsicKind:
        if (arguments[0]->type().isScalar()) {
            ...
            std::string I = this->writeNontrivialScratchLet(*arguments[0], Precedence::kSequence);
            std::string N = this->writeNontrivialScratchLet(*arguments[1], Precedence::kSequence);
            // We can use a scratch-let for Eta to avoid WGSL overflow errors. (skia:14385)
            std::string Eta = all_arguments_constant(arguments) ? this->writeScratchLet(*arguments[2], Precedence::kSequence)
                                                                : this->writeNontrivialScratchLet(*arguments[2], Precedence::kSequence);
            return this->writeScratchLet(String::printf("refract(vec2<%s>(%s, 0), vec2<%s>(%s, 0), %s).x", to_wgsl_type(fContext, arguments[0]->type()).c_str(),
                I.c_str(), to_wgsl_type(fContext, arguments[1]->type()).c_str(), N.c_str(), Eta.c_str()));
        }
    ...
    case k_sample_IntrinsicKind: {
        ...
        if (fProgram.fConfig->fSettings.fSharpenTextures || callIncludesBias) {
            ...
            std::string expr = this->assemblePartialSampleCall("textureSampleBias", *arguments[0], *arguments[1]);
            expr += ", ";
            if (callIncludesBias) {
                expr += this->assembleExpression(*arguments[2], Precedence::kAdditive) + " + ";
            }
            expr += skstd::to_string(fProgram.fConfig->fSettings.fSharpenTextures ? kSharpenTexturesBias : 0.0f);
            return expr + ')';
        }

        // No bias is necessary, so we can call `textureSample` directly.
        return this->assemblePartialSampleCall("textureSample", *arguments[0], *arguments[1]) + ')';
    }
    case k_sampleLod_IntrinsicKind: {
        std::string expr = this->assemblePartialSampleCall("textureSampleLevel", *arguments[0], *arguments[1]);
        expr += ", " + this->assembleExpression(*arguments[2], Precedence::kSequence);
        return expr + ')';
    }
    case k_sampleGrad_IntrinsicKind: {
        std::string expr = this->assemblePartialSampleCall("textureSampleGrad", *arguments[0], *arguments[1]);
        expr += ", " + this->assembleExpression(*arguments[2], Precedence::kSequence);
...
static constexpr char kInverse2x2[] = "fn mat2_inverse(m: mat2x2<f32>) -> mat2x2<f32> {"
                                      "\n"
...
static constexpr char kInverse3x3[] = "fn mat3_inverse(m: mat3x3<f32>) -> mat3x3<f32> {"
                                      "\n"
...
static constexpr char kInverse4x4[] = "fn mat4_inverse(m: mat4x4<f32>) -> mat4x4<f32>{"
                                      "\n"
...
std::string WGSLCodeGenerator::assembleFunctionCall(const FunctionCall& call, Precedence parentPrecedence)
{
    ...
    for (int index = 0; index < args.size(); ++index) {
        if (params[index]->modifierFlags() & ModifierFlag::kOut) {
            ...
            if (params[index]->modifierFlags() & ModifierFlag::kIn) {
                ...
                substituteArgument.push_back(this->writeScratchVar(args[index]->type(), lvalue->load()));
            } else {
...
std::string WGSLCodeGenerator::assemblePrefixExpression(const PrefixExpression& p, Precedence parentPrecedence)
{
    ...
    if (op.kind() == Operator::Kind::PLUSPLUS || op.kind() == Operator::Kind::MINUSMINUS) {
        ...
        std::string newValue
            = lvalue->load() + (p.getOperator().kind() == Operator::Kind::PLUSPLUS ? " + " : " - ") + this->assembleIncrementExpr(p.operand()->type());
        this->writeLine(lvalue->store(newValue));
        return lvalue->load();
    }

    // WGSL natively supports unary negation/not expressions (!,~,-).
    SkASSERT(op.kind() == OperatorKind::LOGICALNOT || op.kind() == OperatorKind::BITWISENOT || op.kind() == OperatorKind::MINUS);
    ...
    std::string expr;
    const bool needsNegation = op.kind() == Operator::Kind::MINUS && !p.operand()->type().isScalar() && !p.operand()->type().isVector();
...
std::string WGSLCodeGenerator::assemblePostfixExpression(const PostfixExpression& p, Precedence parentPrecedence)
{
    SkASSERT(p.getOperator().kind() == Operator::Kind::PLUSPLUS || p.getOperator().kind() == Operator::Kind::MINUSMINUS);
...
{
    return this->assembleExpression(*swizzle.base(), Precedence::kPostfix) + "." + Swizzle::MaskString(swizzle.components());
}
...
std::string WGSLCodeGenerator::writeScratchLet(const std::string& expr, bool isCompileTimeConstant)
{
    std::string scratchVarName = "_skTemp" + std::to_string(fScratchCount++);
...
std::string WGSLCodeGenerator::writeScratchLet(const Expression& expr, Precedence parentPrecedence)
{
    return this->writeScratchLet(this->assembleExpression(expr, parentPrecedence));
}

std::string WGSLCodeGenerator::writeNontrivialScratchLet(const Expression& expr, Precedence parentPrecedence)
{
    std::string result = this->assembleExpression(expr, parentPrecedence);
    return is_nontrivial_expression(expr) ? this->writeScratchLet(result, Analysis::IsCompileTimeConstant(expr)) : result;
}

std::string WGSLCodeGenerator::assembleTernaryExpression(const TernaryExpression& t, Precedence parentPrecedence)
{
    std::string expr;
    ...
    if ((t.type().isScalar() || t.type().isVector()) && !Analysis::HasSideEffects(*t.test()) && Analysis::IsTrivialExpression(*t.ifTrue())
...
std::string WGSLCodeGenerator::variableReferenceNameForLValue(const VariableReference& r)
{
    ...
    if (v.storage() == Variable::Storage::kParameter && (v.modifierFlags() & ModifierFlag::kOut || v.type().isUnsizedArray())) {
...
std::string WGSLCodeGenerator::assembleConstructorCompoundVector(const ConstructorCompound& c)
{
    ...
    if (c.type().columns() == 4 && c.argumentSpan().size() == 1) {
        ...
        if (arg.type().isMatrix()) {
            ...
            return String::printf("%s(%s[0], %s[1])", to_wgsl_type(fContext, c.type()).c_str(), matrix.c_str(), matrix.c_str());
        }
    }
...
std::string WGSLCodeGenerator::assembleEqualityExpression(
    const Type& left, const std::string& leftName, const Type& right, const std::string& rightName, Operator op, Precedence parentPrecedence)
{
    ...
    if (left.isMatrix()) {
        ...
        for (int index = 0; index < columns; ++index) {
            ...
            expr += this->assembleEqualityExpression(vecType, leftName + suffix, vecType, rightName + suffix, op, Precedence::kParentheses);
            separator = combiner;
        }
        return expr + ')';
    }
    ...
    if (left.isArray()) {
        ...
        for (int index = 0; index < left.columns(); ++index) {
            expr += separator;
            std::string suffix = '[' + std::to_string(index) + ']';
            expr += this->assembleEqualityExpression(indexedType, leftName + suffix, indexedType, rightName + suffix, op, Precedence::kParentheses);
            separator = combiner;
        }
        return expr + ')';
    }
    ...
    if (left.isStruct()) {
        ...
        for (const Field& field : fields) {
            expr += separator;
            expr += this->assembleEqualityExpression(*field.fType, leftName + '.' + this->assembleName(field.fName), *field.fType,
                rightName + '.' + this->assembleName(field.fName), op, Precedence::kParentheses);
            separator = combiner;
        }
        return expr + ')';
    }
...
std::string WGSLCodeGenerator::assembleEqualityExpression(const Expression& left, const Expression& right, Operator op, Precedence parentPrecedence)
{
    ...
    return this->assembleEqualityExpression(left.type(), leftName, right.type(), rightName, op, parentPrecedence);
}
...
void WGSLCodeGenerator::writeTextureOrSampler(const Variable& var, int bindingLocation, std::string_view suffix, std::string_view wgslType)
{
...
void WGSLCodeGenerator::writeGlobalVarDeclaration(const GlobalVarDeclaration& d)
{
    ...
    if ((var.modifierFlags() & (ModifierFlag::kIn | ModifierFlag::kOut)) || is_in_global_uniforms(var)) {
    ...
    if (varKind == Type::TypeKind::kSampler) {
        ...
        int samplerLocation = var.layout().fSampler >= 0 ? var.layout().fSampler : 10000 + fScratchCount++;
        ...
        int textureLocation = var.layout().fTexture >= 0 ? var.layout().fTexture : 10000 + fScratchCount++;
    ...
    if (varKind == Type::TypeKind::kTexture) {
        ...
        int textureLocation = var.layout().fBinding >= 0 ? var.layout().fBinding : 10000 + fScratchCount++;
        // For a texture without an associated sampler, we don't apply a suffix.
        this->writeTextureOrSampler(var, textureLocation, /*suffix=*/"", to_wgsl_type(fContext, var.type(), &var.layout()));
        return;
    }
...
void WGSLCodeGenerator::writeFields(SkSpan<const Field> fields, const MemoryLayout* memoryLayout)
{
    ...
    for (size_t index = 0; index < fields.size(); ++index) {
        ...
        if (memoryLayout && !memoryLayout->isSupported(*field.fType)) {
            ...
            fContext.fErrors->error(field.fPosition, "type '" + std::string(field.fType->name()) + "' is not permitted here");
            return;
        }
    ...
    for (const Variable* v : fPipelineInputs) {
        if (v->type().isInterfaceBlock()) {
            for (const Field& f : v->type().fields()) {
                this->writePipelineIODeclaration(f.fLayout, *f.fType, f.fModifierFlags, f.fName, Delimiter::kComma);
            }
        } else {
            this->writePipelineIODeclaration(v->layout(), v->type(), v->modifierFlags(), v->mangledName(), Delimiter::kComma);
        }
    }
...
void WGSLCodeGenerator::prepareUniformPolyfillsForInterfaceBlock(
    const InterfaceBlock* interfaceBlock, std::string_view instanceName, MemoryLayout::Standard nativeLayout)
{
    ...
    for (const Field& field : structType.fields()) {
        ...
        auto isPolyfillableMatrixType = [&](const Type* type) { return type->isMatrix() && std140.stride(*type) != native.stride(*type); };
        ...
        } else if (type->isArray() && !type->isUnsizedArray() && !type->componentType().isOpaque()) {
        ...
        if (needsArrayPolyfill || needsMatrixPolyfill) {
            ...
            info.fReplacementName = "_skUnpacked_" + std::string(instanceName) + '_' + this->assembleName(field.fName);
    ...
    for (const ProgramElement* e : fProgram.elements()) {
        if (e->is<GlobalVarDeclaration>()) {
            ...
            if (is_in_global_uniforms(var)) {
                ...
                this->write("  ");
                this->writeVariableDecl(var.layout(), var.type(), var.mangledName(), Delimiter::kComma);
            }
        }
    }
    if (declaredUniformsStruct) {
...
bool ToWGSL(Program& program, const ShaderCaps* caps, OutputStream& out, PrettyPrint pp, IncludeSyntheticCode isc, ValidateWGSLProc validateWGSL)
{
