...
static std::unique_ptr<Expression> simplify_constant_equality(
    const Context& context, Position pos, const Expression& left, Operator op, const Expression& right)
{
...
static std::unique_ptr<Expression> simplify_matrix_multiplication(
    const Context& context, Position pos, const Expression& left, const Expression& right, int leftColumns, int leftRows, int rightColumns, int rightRows)
{
...
static std::unique_ptr<Expression> simplify_matrix_times_matrix(const Context& context, Position pos, const Expression& left, const Expression& right)
{
    ...
    return simplify_matrix_multiplication(context, pos, left, right, leftType.columns(), leftType.rows(), rightType.columns(), rightType.rows());
}
...
static std::unique_ptr<Expression> simplify_matrix_times_vector(const Context& context, Position pos, const Expression& left, const Expression& right)
{
    ...
    return simplify_matrix_multiplication(context, pos, left, right, leftType.columns(), leftType.rows(),
...
static std::unique_ptr<Expression> simplify_componentwise(const Context& context, Position pos, const Expression& left, Operator op, const Expression& right)
{
    ...
    if (std::unique_ptr<Expression> result = simplify_constant_equality(context, pos, left, op, right)) {
        return result;
    }
    ...
    case Operator::Kind::PLUS:
        foldFn = +[](double a, double b) { return a + b; };
        break;
...
static std::unique_ptr<Expression> negate_expression(const Context& context, Position pos, const Expression& expr, const Type& type)
{
    ...
    return ctor ? PrefixExpression::Make(context, pos, Operator::Kind::MINUS, std::move(ctor)) : nullptr;
}
...
{
    return expr.type().isMatrix() ? is_constant_diagonal(expr, value) : ConstantFolder::IsConstantSplat(expr, value);
}
...
std::unique_ptr<Expression> ConstantFolder::MakeConstantValueForVariable(Position pos, std::unique_ptr<Expression> inExpr)
{
    const Expression* expr = GetConstantValueOrNull(*inExpr);
...
static std::unique_ptr<Expression> simplify_arithmetic(
    const Context& context, Position pos, const Expression& left, Operator op, const Expression& right, const Type& resultType)
{
    switch (op.kind()) {
    case Operator::Kind::PLUS:
        if (!is_scalar_op_matrix(left, right) && ConstantFolder::IsConstantSplat(right, 0.0)) { // x + 0
            if (std::unique_ptr<Expression> expr = cast_expression(context, pos, left, resultType)) {
                return expr;
            }
        }
        if (!is_matrix_op_scalar(left, right) && ConstantFolder::IsConstantSplat(left, 0.0)) { // 0 + x
            if (std::unique_ptr<Expression> expr = cast_expression(context, pos, right, resultType)) {
                return expr;
            }
        }
        break;
    ...
    case Operator::Kind::STAR:
        ...
        if (is_constant_value(right, -1.0)) { // x * -1 (to `-x`)
            if (std::unique_ptr<Expression> expr = negate_expression(context, pos, left, resultType)) {
                return expr;
            }
        }
        if (is_constant_value(left, -1.0)) { // -1 * x (to `-x`)
            if (std::unique_ptr<Expression> expr = negate_expression(context, pos, right, resultType)) {
                return expr;
            }
        }
        break;
    ...
    case Operator::Kind::SLASH:
        if (!is_scalar_op_matrix(left, right) && ConstantFolder::IsConstantSplat(right, 1.0)) { // x / 1
            if (std::unique_ptr<Expression> expr = cast_expression(context, pos, left, resultType)) {
                return expr;
            }
        }
        if (!left.type().isMatrix()) { // convert `x / 2` into `x * 0.5`
            if (std::unique_ptr<Expression> expr = make_reciprocal_expression(context, right)) {
                return BinaryExpression::Make(context, pos, left.clone(), Operator::Kind::STAR, std::move(expr));
            }
        }
        break;
    ...
    case Operator::Kind::SLASHEQ:
        ...
        if (std::unique_ptr<Expression> expr = make_reciprocal_expression(context, right)) {
            return BinaryExpression::Make(context, pos, left.clone(), Operator::Kind::STAREQ, std::move(expr));
        }
        break;
...
static std::unique_ptr<Expression> one_over_scalar(const Context& context, const Expression& right)
{
    ...
    return BinaryExpression::Make(context, pos, Literal::Make(pos, 1.0, &right.type()), Operator::Kind::SLASH, right.clone());
}
    ...
    const Context& context, Position pos, const Expression& left, Operator op, const Expression& right, const Type& resultType)
{
    ...
    case OperatorKind::SLASHEQ:
        if (left.type().isMatrix() && right.type().isScalar()) {
            Operator multiplyOp = op.isAssignment() ? OperatorKind::STAREQ : OperatorKind::STAR;
            return BinaryExpression::Make(context, pos, left.clone(), multiplyOp, one_over_scalar(context, right));
        }
        break;
...
static std::unique_ptr<Expression> fold_two_constants(
    const Context& context, Position pos, const Expression* left, Operator op, const Expression* right, const Type& resultType)
{
...
#define RESULT(Op) fold_expression(pos, (SKSL_INT)(leftVal)Op(SKSL_INT)(rightVal), &resultType)
#define URESULT(Op) fold_expression(pos, (SKSL_INT)((SKSL_UINT)(leftVal)Op(SKSL_UINT)(rightVal)), &resultType)
        switch (op.kind()) {
        case Operator::Kind::PLUS:
            return URESULT(+);
        case Operator::Kind::MINUS:
...
#undef RESULT
    ...
    if (rightType.isScalar() && is_vec_or_mat(leftType) && leftType.componentType().matches(rightType)) {
        return simplify_componentwise(context, pos, *left, op, *splat_scalar(context, *right, left->type()));
    }

    // Perform constant folding on scalars against vectors/matrices, e.g.: 2 + half4(2)
    if (leftType.isScalar() && is_vec_or_mat(rightType) && rightType.componentType().matches(leftType)) {
        return simplify_componentwise(context, pos, *splat_scalar(context, *left, right->type()), op, *right);
    }

    // Perform constant folding on pairs of matrices, arrays or structs.
    if ((leftType.isMatrix() && rightType.isMatrix()) || (leftType.isArray() && rightType.isArray()) || (leftType.isStruct() && rightType.isStruct())) {
...
std::unique_ptr<Expression> ConstantFolder::Simplify(
    const Context& context, Position pos, const Expression& leftExpr, Operator op, const Expression& rightExpr, const Type& resultType)
{
    ...
    if (context.fConfig->fSettings.fOptimize) {
        ...
        if (leftSideIsConstant || rightSideIsConstant) {
            if (std::unique_ptr<Expression> expr = simplify_arithmetic(context, pos, *left, op, *right, resultType)) {
                return expr;
            }
        }

        // We can simplify some forms of matrix division even when neither side is constant.
        if (std::unique_ptr<Expression> expr = simplify_matrix_division(context, pos, *left, op, *right, resultType)) {
            return expr;
        }
    }
