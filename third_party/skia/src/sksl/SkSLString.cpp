...
template <typename RoundtripType, int kFullPrecision> static std::string to_string_impl(RoundtripType value)
{
    ...
    if (value != (RoundtripType)roundtripped && std::isfinite(value)) {
        ...
        buffer << value;
        text = buffer.str();
        SkASSERTF((buffer >> roundtripped, value == (RoundtripType)roundtripped), "%.17g -> %s -> %.17g", value, text.c_str(), roundtripped);
    }
