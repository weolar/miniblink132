...
const Module* Compiler::moduleForProgramKind(ProgramKind kind)
{
    ...
    case ProgramKind::kFragment:
        return m.loadFragmentModule(this);
    case ProgramKind::kVertex:
        return m.loadVertexModule(this);
    case ProgramKind::kCompute:
        return m.loadComputeModule(this);
    case ProgramKind::kGraphiteFragment:
        return m.loadGraphiteFragmentModule(this);
    case ProgramKind::kGraphiteVertex:
        return m.loadGraphiteVertexModule(this);
    case ProgramKind::kGraphiteFragmentES2:
        return m.loadGraphiteFragmentES2Module(this);
    case ProgramKind::kGraphiteVertexES2:
        return m.loadGraphiteVertexES2Module(this);
    case ProgramKind::kPrivateRuntimeBlender:
    case ProgramKind::kPrivateRuntimeColorFilter:
    case ProgramKind::kPrivateRuntimeShader:
        return m.loadPrivateRTShaderModule(this);
    ...
    case ProgramKind::kMeshFragment:
        return m.loadPublicModule(this);
    }
    SkUNREACHABLE;
}
...
void Compiler::initializeContext(const SkSL::Module* module, ProgramKind kind, ProgramSettings settings, std::string_view source, ModuleType moduleType)
{
    SkASSERT(!fPool);
    SkASSERT(!fConfig);
    ...
    fGlobalSymbols = std::make_unique<SymbolTable>(module->fSymbols.get(), moduleType != ModuleType::program);
...
std::unique_ptr<Module> Compiler::compileModule(
    ProgramKind kind, ModuleType moduleType, std::string moduleSource, const Module* parentModule, bool shouldInline)
{
    ...
    std::unique_ptr<Module> module = SkSL::Parser(this, settings, kind, std::move(sourcePtr)).moduleInheritingFrom(parentModule);

    this->cleanupContext();

    if (this->errorCount() != 0) {
        SkDebugf("Unexpected errors compiling %s:\n\n%s\n", ModuleTypeToString(moduleType), this->errorText().c_str());
        return nullptr;
    }
    if (shouldInline) {
...
std::unique_ptr<Program> Compiler::convertProgram(ProgramKind kind, std::string programSource, const ProgramSettings& settings)
{
    ...
    std::unique_ptr<Program> program = SkSL::Parser(this, settings, kind, std::move(sourcePtr)).programInheritingFrom(module);

    this->cleanupContext();
    return program;
}

std::unique_ptr<SkSL::Program> Compiler::releaseProgram(std::unique_ptr<std::string> source, std::vector<std::unique_ptr<SkSL::ProgramElement>> programElements)
{
    Pool* pool = fPool.get();
    auto result = std::make_unique<SkSL::Program>(
        std::move(source), std::move(fConfig), fContext, std::move(programElements), std::move(fGlobalSymbols), std::move(fPool));
    fContext->fSymbolTable = nullptr;

    bool success = this->finalize(*result) && this->optimize(*result);
    if (pool) {
...
bool Compiler::optimizeModuleBeforeMinifying(ProgramKind kind, Module& module, bool shrinkSymbols)
{
    ...
    while (Transform::EliminateDeadGlobalVariables(this->context(), module, usage.get(), onlyPrivateGlobals)) {
...
bool Compiler::optimize(Program& program)
{
    ...
    if (this->errorCount() == 0) {
        ...
        Inliner inliner(fContext.get());
        this->runInliner(&inliner, program.fOwnedElements, program.fSymbols.get(), program.fUsage.get());
#endif
...
bool Compiler::runInliner(Inliner* inliner, const std::vector<std::unique_ptr<ProgramElement>>& elements, SymbolTable* symbols, ProgramUsage* usage)
{
...
void Compiler::writeErrorCount()
{
    ...
    if (count) {
        fErrorText += std::to_string(count) + ((count == 1) ? " error\n" : " errors\n");
    }
}
