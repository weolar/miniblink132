...
static std::unique_ptr<Statement>* find_parent_statement(const std::vector<std::unique_ptr<Statement>*>& stmtStack)
{
...
std::unique_ptr<Expression> clone_with_ref_kind(const Expression& expr, VariableReference::RefKind refKind, Position pos)
{
...
const Variable* Inliner::RemapVariable(const Variable* variable, const VariableRewriteMap* varMap)
{
    ...
    if (!remap) {
        SkDEBUGFAILF("rewrite map does not contain variable '%.*s'", (int)variable->name().size(), variable->name().data());
        return variable;
    }
    Expression* expr = remap->get();
    SkASSERT(expr);
    if (!expr->is<VariableReference>()) {
        SkDEBUGFAILF("rewrite map contains non-variable replacement for '%.*s'", (int)variable->name().size(), variable->name().data());
        return variable;
    }
...
void Inliner::ensureScopedBlocks(Statement* inlinedBody, Statement* parentStmt)
{
    ...
    if (!parentStmt || !(parentStmt->is<IfStatement>() || parentStmt->is<ForStatement>() || parentStmt->is<DoStatement>() || is_scopeless_block(parentStmt))) {
        return;
    }
...
std::unique_ptr<Expression> Inliner::inlineExpression(
    Position pos, VariableRewriteMap* varMap, SymbolTable* symbolTableForExpression, const Expression& expression)
{
    ...
    auto childRemap = [&](const Variable& var) -> const Variable& {
        ...
        if (std::unique_ptr<Expression>* remap = varMap->find(&var)) {
            ...
            if ((*remap)->is<VariableReference>()) {
                ...
                SkDEBUGFAILF(
                    "Child effect '%.*s' remaps to unexpected expression '%s'", (int)var.name().size(), var.name().data(), (*remap)->description().c_str());
            }
        }
    ...
    case Expression::Kind::kBinary: {
        ...
        return BinaryExpression::Make(*fContext, pos, expr(binaryExpr.left()), binaryExpr.getOperator(), expr(binaryExpr.right()));
    }
    ...
    case Expression::Kind::kChildCall: {
        ...
        return ChildCall::Make(
            *fContext, pos, childCall.type().clone(*fContext, symbolTableForExpression), childRemap(childCall.child()), argList(childCall.arguments()));
    }
    case Expression::Kind::kConstructorArray: {
        const ConstructorArray& ctor = expression.as<ConstructorArray>();
        return ConstructorArray::Make(*fContext, pos, *ctor.type().clone(*fContext, symbolTableForExpression), argList(ctor.arguments()));
    }
    case Expression::Kind::kConstructorArrayCast: {
        const ConstructorArrayCast& ctor = expression.as<ConstructorArrayCast>();
        return ConstructorArrayCast::Make(*fContext, pos, *ctor.type().clone(*fContext, symbolTableForExpression), expr(ctor.argument()));
    }
    case Expression::Kind::kConstructorCompound: {
        const ConstructorCompound& ctor = expression.as<ConstructorCompound>();
        return ConstructorCompound::Make(*fContext, pos, *ctor.type().clone(*fContext, symbolTableForExpression), argList(ctor.arguments()));
    }
    case Expression::Kind::kConstructorCompoundCast: {
        const ConstructorCompoundCast& ctor = expression.as<ConstructorCompoundCast>();
        return ConstructorCompoundCast::Make(*fContext, pos, *ctor.type().clone(*fContext, symbolTableForExpression), expr(ctor.argument()));
    }
    case Expression::Kind::kConstructorDiagonalMatrix: {
        const ConstructorDiagonalMatrix& ctor = expression.as<ConstructorDiagonalMatrix>();
        return ConstructorDiagonalMatrix::Make(*fContext, pos, *ctor.type().clone(*fContext, symbolTableForExpression), expr(ctor.argument()));
    }
    case Expression::Kind::kConstructorMatrixResize: {
        const ConstructorMatrixResize& ctor = expression.as<ConstructorMatrixResize>();
        return ConstructorMatrixResize::Make(*fContext, pos, *ctor.type().clone(*fContext, symbolTableForExpression), expr(ctor.argument()));
    }
    case Expression::Kind::kConstructorScalarCast: {
        const ConstructorScalarCast& ctor = expression.as<ConstructorScalarCast>();
        return ConstructorScalarCast::Make(*fContext, pos, *ctor.type().clone(*fContext, symbolTableForExpression), expr(ctor.argument()));
    }
    case Expression::Kind::kConstructorSplat: {
        const ConstructorSplat& ctor = expression.as<ConstructorSplat>();
        return ConstructorSplat::Make(*fContext, pos, *ctor.type().clone(*fContext, symbolTableForExpression), expr(ctor.argument()));
    }
    case Expression::Kind::kConstructorStruct: {
        const ConstructorStruct& ctor = expression.as<ConstructorStruct>();
        return ConstructorStruct::Make(*fContext, pos, *ctor.type().clone(*fContext, symbolTableForExpression), argList(ctor.arguments()));
    }
    ...
    case Expression::Kind::kFunctionCall: {
        ...
        return FunctionCall::Make(
            *fContext, pos, funcCall.type().clone(*fContext, symbolTableForExpression), funcCall.function(), argList(funcCall.arguments()));
    }
    ...
    case Expression::Kind::kTernary: {
        ...
        return TernaryExpression::Make(*fContext, pos, expr(t.test()), expr(t.ifTrue()), expr(t.ifFalse()));
    }
...
std::unique_ptr<Statement> Inliner::inlineStatement(Position pos, VariableRewriteMap* varMap, SymbolTable* symbolTableForStatement,
    std::unique_ptr<Expression>* resultExpr, Analysis::ReturnComplexity returnComplexity, const Statement& statement, const ProgramUsage& usage,
    bool isBuiltinCode)
{
    auto stmt = [&](const std::unique_ptr<Statement>& s) -> std::unique_ptr<Statement> {
        if (s) {
            return this->inlineStatement(pos, varMap, symbolTableForStatement, resultExpr, returnComplexity, *s, usage, isBuiltinCode);
        }
        return nullptr;
    };
    auto expr = [&](const std::unique_ptr<Expression>& e) -> std::unique_ptr<Expression> {
    ...
    auto variableModifiers = [&](const Variable& variable, const Expression* initialValue) -> ModifierFlags {
    ...
    case Statement::Kind::kBlock:
        return makeWithChildSymbolTable([&](std::unique_ptr<SymbolTable> symbolTable) {
            ...
            return Block::Make(pos, std::move(statements), block.blockKind(), std::move(symbolTable));
        });
    ...
    case Statement::Kind::kFor:
        return makeWithChildSymbolTable([&](std::unique_ptr<SymbolTable> symbolTable) {
            ...
            return ForStatement::Make(*fContext, pos, ForLoopPositions {}, std::move(initializerStmt), std::move(testExpr), std::move(nextExpr),
                std::move(bodyStmt), std::move(unrollInfo), std::move(symbolTable));
        });

    case Statement::Kind::kIf: {
        const IfStatement& i = statement.as<IfStatement>();
        return IfStatement::Make(*fContext, pos, expr(i.test()), stmt(i.ifTrue()), stmt(i.ifFalse()));
    }
    ...
    case Statement::Kind::kReturn: {
        ...
        return ExpressionStatement::Make(*fContext,
            BinaryExpression::Make(*fContext, pos, clone_with_ref_kind(**resultExpr, VariableRefKind::kWrite, pos), Operator::Kind::EQ, expr(r.expression())));
    }
    ...
    case Statement::Kind::kSwitchCase: {
        ...
        return sc.isDefault() ? SwitchCase::MakeDefault(pos, stmt(sc.statement())) : SwitchCase::Make(pos, sc.value(), stmt(sc.statement()));
    }
    case Statement::Kind::kVarDeclaration: {
        ...
        // names are important.
        const std::string* name = symbolTableForStatement->takeOwnershipOfString(fMangler.uniqueName(variable->name(), symbolTableForStatement));
        auto clonedVar = Variable::Make(pos, variable->modifiersPosition(), variable->layout(), variableModifiers(*variable, initialValue.get()),
            variable->type().clone(*fContext, symbolTableForStatement), name->c_str(),
            /*mangledName=*/"", isBuiltinCode, variable->storage());
        varMap->set(variable, VariableReference::Make(pos, clonedVar.get()));
        std::unique_ptr<Statement> result = VarDeclaration::Make(
            *fContext, clonedVar.get(), decl.baseType().clone(*fContext, symbolTableForStatement), decl.arraySize(), std::move(initialValue));
...
static bool argument_needs_scratch_variable(const Expression* arg, const Variable* param, const ProgramUsage& usage)
{
    ...
    if (!paramUsage.fWrite) {
        ...
        if ((paramUsage.fRead > 1) ? Analysis::IsTrivialExpression(*arg) : !Analysis::HasSideEffects(*arg)) {
...
Inliner::InlinedCall Inliner::inlineCall(const FunctionCall& call, SymbolTable* symbolTable, const ProgramUsage& usage, const FunctionDeclaration* caller)
{
    ...
    if (returnComplexity > Analysis::ReturnComplexity::kSingleSafeReturn && !function.declaration().returnType().isVoid()) {
        ...
        ScratchVariable var
            = Variable::MakeScratchVariable(*fContext, fMangler, function.declaration().name(), &function.declaration().returnType(), symbolTable,
    ...
    for (int i = 0; i < arguments.size(); ++i) {
        ...
        ScratchVariable var = Variable::MakeScratchVariable(*fContext, fMangler, param->name(), &arg->type(), symbolTable, arg->clone());
        inlineStatements.push_back(std::move(var.fVarDecl));
        varMap.set(param, VariableReference::Make(Position(), var.fVarSymbol));
    }

    for (const std::unique_ptr<Statement>& stmt : body.children()) {
        inlineStatements.push_back(this->inlineStatement(pos, &varMap, symbolTable, &resultExpr, returnComplexity, *stmt, usage, caller->isBuiltin()));
    }
    ...
    inlinedCall.fInlinedBody = Block::MakeBlock(pos, std::move(inlineStatements), Block::Kind::kUnbracedBlock);
    if (resultExpr) {
        ...
        fContext->fErrors->error(function.fPosition,
            "inliner found non-void function '" + std::string(function.declaration().name()) + "' that fails to return a value on any path");
        inlinedCall = {};
    }
...
bool Inliner::isSafeToInline(const FunctionDefinition* functionDef, const ProgramUsage& usage)
{
    ...
    for (const Variable* param : functionDef->declaration().parameters()) {
        ...
        if ((param->modifierFlags() & ModifierFlag::kOut) || param->type().isArray() || param->type().isStruct()) {
...
public:
    ...
    void visitStatement(std::unique_ptr<Statement>* stmt, bool isViableAsEnclosingStatement = true)
    {
        ...
        if (scopedStackBuilder.foundSymbolTable() && fSymbolTableStack.back()->wouldShadowSymbolsFrom(fSymbolTableStack.front())) {
            return;
        }
    ...
    void visitExpression(std::unique_ptr<Expression>* expr)
    {
        ...
        case Expression::Kind::kBinary: {
            ...
            bool shortCircuitable = (op.kind() == Operator::Kind::LOGICALAND || op.kind() == Operator::Kind::LOGICALOR);
            if (!shortCircuitable) {
    ...
    void addInlineCandidate(std::unique_ptr<Expression>* candidate)
    {
        fCandidateList->fCandidates.push_back(InlineCandidate {
            fSymbolTableStack.back(), find_parent_statement(fEnclosingStmtStack), fEnclosingStmtStack.back(), candidate, fEnclosingFunction });
    }
};
...
bool Inliner::functionCanBeInlined(const FunctionDeclaration& funcDecl, const ProgramUsage& usage, InlinabilityCache* cache)
{
...
bool Inliner::candidateCanBeInlined(const InlineCandidate& candidate, const ProgramUsage& usage, InlinabilityCache* cache)
{
...
int Inliner::getFunctionSize(const FunctionDeclaration& funcDecl, FunctionSizeCache* cache)
{
    ...
    int size = Analysis::NodeCountUpToLimit(*funcDecl.definition(), this->settings().fInlineThreshold);
    cache->set(&funcDecl, size);
    return size;
}

void Inliner::buildCandidateList(
    const std::vector<std::unique_ptr<ProgramElement>>& elements, SymbolTable* symbols, ProgramUsage* usage, InlineCandidateList* candidateList)
{
    ...
    candidates.erase(std::remove_if(candidates.begin(), candidates.end(),
                         [&](const InlineCandidate& candidate) { return !this->candidateCanBeInlined(candidate, *usage, &cache); }),
        candidates.end());
...
bool Inliner::analyze(const std::vector<std::unique_ptr<ProgramElement>>& elements, SymbolTable* symbols, ProgramUsage* usage)
{
    // A threshold of zero indicates that the inliner is completely disabled, so we can just return.
    if (this->settings().fInlineThreshold <= 0) {
        return false;
    ...
    using StatementRemappingTable = THashMap<std::unique_ptr<Statement>*, std::unique_ptr<Statement>*>;
    ...
    for (const InlineCandidate& candidate : candidateList.fCandidates) {
        ...
        InlinedCall inlinedCall = this->inlineCall(funcCall, candidate.fSymbols, *usage, &candidate.fEnclosingFunction->declaration());
