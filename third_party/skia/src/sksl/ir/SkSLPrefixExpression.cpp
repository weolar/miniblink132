...
static std::unique_ptr<Expression> simplify_negation(const Context& context, Position pos, const Expression& originalExpr)
{
    ...
    case Expression::Kind::kConstructorCompound: {
        ...
        if (std::unique_ptr<Expression> expr = apply_to_elements(context, pos, *value, negate_value)) {
            return expr;
        }
        break;
    }
    ...
    case Expression::Kind::kConstructorArray:
        ...
        if (Analysis::IsCompileTimeConstant(*value)) {
            ...
            return ConstructorArray::Make(context, pos, ctor.type(), negate_operands(context, pos, ctor.arguments()));
        }
        break;
    ...
    case Expression::Kind::kConstructorDiagonalMatrix:
        ...
        if (Analysis::IsCompileTimeConstant(*value)) {
            ...
            if (std::unique_ptr<Expression> simplified = simplify_negation(context, pos, *ctor.argument())) {
...
static ExpressionArray negate_operands(const Context& context, Position pos, const ExpressionArray& array)
{
    ...
    for (const std::unique_ptr<Expression>& expr : array) {
        ...
        if (std::unique_ptr<Expression> simplified = simplify_negation(context, pos, *expr)) {
            ...
            replacement.push_back(std::make_unique<PrefixExpression>(pos, Operator::Kind::MINUS, expr->clone()));
        }
    }
    return replacement;
}
...
static std::unique_ptr<Expression> logical_not_operand(const Context& context, Position pos, std::unique_ptr<Expression> operand)
{
    ...
    case Expression::Kind::kBinary: {
        ...
        case OperatorKind::EQEQ:
            replacement = OperatorKind::NEQ;
            break;
        case OperatorKind::NEQ:
            replacement = OperatorKind::EQEQ;
            break;
        case OperatorKind::LT:
            replacement = OperatorKind::GTEQ;
            break;
        case OperatorKind::LTEQ:
            replacement = OperatorKind::GT;
            break;
        case OperatorKind::GT:
            replacement = OperatorKind::LTEQ;
            break;
        case OperatorKind::GTEQ:
            replacement = OperatorKind::LT;
            break;
        default:
            break;
        }
        if (replacement.has_value()) {
            return BinaryExpression::Make(context, pos, std::move(binary.left()), *replacement, std::move(binary.right()), &binary.type());
        }
        break;
    }
    default:
...
static std::unique_ptr<Expression> bitwise_not_operand(const Context& context, Position pos, std::unique_ptr<Expression> operand)
{
    ...
    case Expression::Kind::kConstructorCompound: {
        ...
        if (std::unique_ptr<Expression> expr = apply_to_elements(context, pos, *value, bitwise_not_value)) {
            return expr;
        }
        break;
    }
...
std::unique_ptr<Expression> PrefixExpression::Convert(const Context& context, Position pos, Operator op, std::unique_ptr<Expression> base)
{
    ...
    case Operator::Kind::MINUSMINUS:
        if (baseType.isArray() || !baseType.componentType().isNumber()) {
            context.fErrors->error(pos, "'" + std::string(op.tightOperatorName()) + "' cannot operate on '" + baseType.displayName() + "'");
            return nullptr;
        }
        if (!Analysis::UpdateVariableRefKind(base.get(), VariableReference::RefKind::kReadWrite, context.fErrors)) {
            return nullptr;
        }
        break;
...
std::unique_ptr<Expression> PrefixExpression::Make(const Context& context, Position pos, Operator op, std::unique_ptr<Expression> base)
{
    const Type& baseType = base->type();
    switch (op.kind()) {
    case Operator::Kind::PLUS:
...
std::string PrefixExpression::description(OperatorPrecedence parentPrecedence) const
{
    ...
    return std::string(needsParens ? "(" : "") + std::string(this->getOperator().tightOperatorName())
        + this->operand()->description(OperatorPrecedence::kPrefix) + std::string(needsParens ? ")" : "");
}
