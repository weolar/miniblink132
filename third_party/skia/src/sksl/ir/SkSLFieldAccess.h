...
public:
    ...
    static std::unique_ptr<Expression> Make(
        const Context& context, Position pos, std::unique_ptr<Expression> base, int fieldIndex, OwnerKind ownerKind = OwnerKind::kDefault);
    ...
    {
        return std::make_unique<FieldAccess>(pos, this->base()->clone(), this->fieldIndex(), this->ownerKind());
    }
