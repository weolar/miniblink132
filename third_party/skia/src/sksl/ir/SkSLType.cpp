...
public:
    ...
    bool matches(const Type& otherType) const override
    {
        ...
        return that.isArray() && this->columns() == that.columns() && this->componentType().matches(that.componentType());
    }
...
public:
    ...
    double minimumValue() const override
    {
        ...
        case NumberKind::kSigned:
            return this->highPrecision() ? int_limits::lowest() : short_limits::lowest();
    ...
    double maximumValue() const override
    {
        switch (this->numberKind()) {
        case NumberKind::kSigned:
            return this->highPrecision() ? int_limits::max() : short_limits::max();

        case NumberKind::kUnsigned:
            return this->highPrecision() ? uint_limits::max() : ushort_limits::max();
...
std::unique_ptr<Type> Type::MakeArrayType(const Context& context, std::string_view name, const Type& componentType, int columns)
{
    return std::make_unique<ArrayType>(std::move(name), componentType.abbreviatedName(), componentType, columns, context.fConfig->isBuiltinCode());
}

std::unique_ptr<Type> Type::MakeGenericType(const char* name, SkSpan<const Type* const> types, const Type* slotType)
{
    return std::make_unique<GenericType>(name, types, slotType);
}

std::unique_ptr<Type> Type::MakeLiteralType(const char* name, const Type& scalarType, int8_t priority)
{
    return std::make_unique<LiteralType>(name, scalarType, priority);
}

std::unique_ptr<Type> Type::MakeMatrixType(std::string_view name, const char* abbrev, const Type& componentType, int columns, int8_t rows)
{
...
std::unique_ptr<Type> Type::MakeSpecialType(const char* name, const char* abbrev, Type::TypeKind typeKind)
{
    return std::unique_ptr<Type>(new Type(name, abbrev, typeKind));
}

std::unique_ptr<Type> Type::MakeScalarType(std::string_view name, const char* abbrev, Type::NumberKind numberKind, int8_t priority, int8_t bitWidth)
{
...
std::unique_ptr<Type> Type::MakeStructType(const Context& context, Position pos, std::string_view name, TArray<Field> fields, bool interfaceBlock)
{
    ...
    if (fields.empty()) {
        context.fErrors->error(pos, std::string(structOrIB) + " '" + std::string(name) + "' must contain at least one field");
    }
    size_t slots = 0;
    ...
    for (const Field& field : fields) {
        ...
        if (fieldNames.count() == numFieldNames) {
            context.fErrors->error(field.fPosition,
                "field '" + std::string(field.fName) + "' was already defined in the same " + std::string(structOrIB) + " ('" + std::string(name) + "')");
        }
        if (field.fModifierFlags != ModifierFlag::kNone) {
            std::string desc = field.fModifierFlags.description();
            context.fErrors->error(field.fPosition, "modifier '" + desc + "' is not permitted on " + std::string(aStructOrIB) + " field");
        }
        ...
        if (field.fLayout.fFlags & LayoutFlag::kSet) {
            context.fErrors->error(field.fPosition, "layout qualifier 'set' is not permitted on " + std::string(aStructOrIB) + " field");
        }

        if (field.fType->isVoid()) {
            context.fErrors->error(field.fPosition, "type 'void' is not permitted in " + std::string(aStructOrIB));
        }
        if (field.fType->isOpaque() && !field.fType->isAtomic()) {
            context.fErrors->error(field.fPosition, "opaque type '" + field.fType->displayName() + "' is not permitted in " + std::string(aStructOrIB));
        }
        if (interfaceBlock && field.fType->isOrContainsBool()) {
            // Reject booleans anywhere in an interface block.
            context.fErrors->error(field.fPosition,
                "type 'bool' is not permitted in an "
                "interface block");
        }
    ...
    if (nestingDepth >= kMaxStructDepth) {
        context.fErrors->error(pos, std::string(structOrIB) + " '" + std::string(name) + "' is too deeply nested");
    }
    return std::make_unique<StructType>(pos, name, std::move(fields), nestingDepth + 1, interfaceBlock, context.fConfig->isBuiltinCode());
}

std::unique_ptr<Type> Type::MakeTextureType(
    const char* name, SpvDim_ dimensions, bool isDepth, bool isArrayedTexture, bool isMultisampled, TextureAccess textureAccess)
{
    return std::make_unique<TextureType>(name, dimensions, isDepth, isArrayedTexture, isMultisampled, textureAccess);
}

std::unique_ptr<Type> Type::MakeVectorType(std::string_view name, const char* abbrev, const Type& componentType, int columns)
{
...
CoercionCost Type::coercionCost(const Type& other) const
{
    ...
    if (this->typeKind() == other.typeKind() && (this->isVector() || this->isMatrix() || this->isArray())) {
...
const Type* Type::applyQualifiers(const Context& context, ModifierFlags* modifierFlags, Position pos) const
{
    const Type* type;
...
const Type* Type::applyPrecisionQualifiers(const Context& context, ModifierFlags* modifierFlags, Position pos) const
{
    ModifierFlags precisionQualifiers = *modifierFlags & (ModifierFlag::kHighp | ModifierFlag::kMediump | ModifierFlag::kLowp);
    ...
    *modifierFlags &= ~(ModifierFlag::kHighp | ModifierFlag::kMediump | ModifierFlag::kLowp);
    ...
    if (component.highPrecision()) {
        ...
        if (mediumpType) {
            ...
            return this->isArray() ? context.fSymbolTable->addArrayDimension(context, mediumpType, this->columns())
                                   : &mediumpType->toCompound(context, this->columns(), this->rows());
        }
    }

    context.fErrors->error(pos, "type '" + this->displayName() + "' does not support precision qualifiers");
    return context.fTypes.fPoison.get();
}

const Type* Type::applyAccessQualifiers(const Context& context, ModifierFlags* modifierFlags, Position pos) const
{
    ModifierFlags accessQualifiers = *modifierFlags & (ModifierFlag::kReadOnly | ModifierFlag::kWriteOnly);

    // We're going to return a whole new type, so the modifier bits must be cleared out.
    *modifierFlags &= ~(ModifierFlag::kReadOnly | ModifierFlag::kWriteOnly);
    ...
    if (this->matches(*context.fTypes.fTexture2D)) {
        ...
        context.fErrors->error(pos,
            accessQualifiers ? "'readonly' and 'writeonly' qualifiers cannot be combined"
...
const Type& Type::toCompound(const Context& context, int columns, int rows) const
{
    ...
    if (this->matches(*context.fTypes.fFloat) || this->matches(*context.fTypes.fFloatLiteral)) {
        ...
        case 1:
            ...
            default:
                SK_ABORT("unsupported vector column count (%d)", columns);
            }
        case 2:
            switch (columns) {
            case 2:
            ...
            default:
                SK_ABORT("unsupported matrix column count (%d)", columns);
            }
        case 3:
            switch (columns) {
            case 2:
        ...
        default:
            SK_ABORT("unsupported row count (%d)", rows);
        }
...
const Type* Type::clone(const Context& context, SymbolTable* symbolTable) const
{
    ...
    case TypeKind::kStruct: {
        ...
        return symbolTable->add(context,
            std::make_unique<StructType>(this->fPosition, *name, TArray<Field>(fieldSpan.data(), fieldSpan.size()), this->structNestingDepth(),
...
std::unique_ptr<Expression> Type::coerceExpression(std::unique_ptr<Expression> expr, const Context& context) const
{
    ...
    if (!expr->coercionCost(*this).isPossible(settings.fAllowNarrowingConversions)) {
        context.fErrors->error(pos, "expected '" + this->displayName() + "', but found '" + expr->type().displayName() + "'");
        return nullptr;
    }
    ...
    if (baseType.isNumber()) {
        ...
        if (valueExpr->supportsConstantValues()) {
            ...
            for (int slot = 0; slot < numSlots; ++slot) {
                ...
                if (slotVal.has_value() && baseType.checkForOutOfRangeLiteral(context, *slotVal, valueExpr->fPosition)) {
                    foundError = true;
                }
            }
        }
...
bool Type::checkForOutOfRangeLiteral(const Context& context, double value, Position pos) const
{
    ...
    context.fErrors->error(pos, SkSL::String::printf("value is out of range for type '%s': %.0f", this->displayName().c_str(), value));
    return true;
}
...
bool Type::checkIfUsableInArray(const Context& context, Position arrayPos) const
{
    ...
    if (this->isOpaque() && !this->isAtomic()) {
        context.fErrors->error(arrayPos, "opaque type '" + std::string(this->name()) + "' may not be used in an array");
        return false;
    }
    return true;
}

SKSL_INT Type::convertArraySize(const Context& context, Position arrayPos, std::unique_ptr<Expression> size) const
{
...
SKSL_INT Type::convertArraySize(const Context& context, Position arrayPos, Position sizePos, SKSL_INT size) const
{
...
{
    return fLayout.paddedDescription() + fModifierFlags.paddedDescription() + fType->displayName() + ' ' + std::string(fName) + ';';
}
