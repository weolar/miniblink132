...
struct IRHelpers {
    ...
    {
        return FieldAccess::Make(fContext, Position(), Ref(var), idx, FieldAccess::OwnerKind::kAnonymousInterfaceBlock);
    }
    ...
    std::unique_ptr<Expression> CtorXYZW(std::unique_ptr<Expression> xy, std::unique_ptr<Expression> z, std::unique_ptr<Expression> w) const
    {
        ...
        return ConstructorCompound::Make(fContext, Position(), *fContext.fTypes.fFloat4, std::move(args));
    }
    ...
    std::unique_ptr<Statement> Assign(std::unique_ptr<Expression> l, std::unique_ptr<Expression> r) const
    {
        ...
        return ExpressionStatement::Make(fContext, Binary(std::move(l), OperatorKind::EQ, std::move(r)));
    }
