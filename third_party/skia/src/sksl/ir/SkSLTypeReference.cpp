...
std::unique_ptr<TypeReference> TypeReference::Convert(const Context& context, Position pos, const Type* type)
{
    return VerifyType(context, type, pos) ? TypeReference::Make(context, pos, type) : nullptr;
}

std::unique_ptr<TypeReference> TypeReference::Make(const Context& context, Position pos, const Type* type)
{
