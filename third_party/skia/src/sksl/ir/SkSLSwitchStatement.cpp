...
std::unique_ptr<Statement> SwitchStatement::Convert(const Context& context, Position pos, std::unique_ptr<Expression> value, ExpressionArray caseValues,
    StatementArray caseStatements, std::unique_ptr<SymbolTable> symbolTable)
{
    ...
    for (int i = 0; i < caseValues.size(); ++i) {
        if (caseValues[i]) {
            ...
            std::unique_ptr<Expression> caseValue = value->type().coerceExpression(std::move(caseValues[i]), context);
            if (!caseValue) {
                return nullptr;
            }
            SKSL_INT intValue;
    ...
    if (!duplicateCases.empty()) {
        for (const SwitchCase* sc : duplicateCases) {
            if (sc->isDefault()) {
                ...
                context.fErrors->error(sc->fPosition, "duplicate case value '" + std::to_string(sc->value()) + "'");
            }
        }
        return nullptr;
    }
    ...
    std::unique_ptr<Block> block = Transform::HoistSwitchVarDeclarationsAtTopLevel(context, cases, *symbolTable, pos);

    std::unique_ptr<Statement> switchStmt
        = SwitchStatement::Make(context, pos, std::move(value), Block::MakeBlock(pos, std::move(cases), Block::Kind::kBracedScope, std::move(symbolTable)));
    if (block) {
...
std::unique_ptr<Statement> SwitchStatement::Make(const Context& context, Position pos, std::unique_ptr<Expression> value, std::unique_ptr<Statement> caseBlock)
{
