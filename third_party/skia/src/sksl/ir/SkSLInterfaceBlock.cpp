...
std::unique_ptr<InterfaceBlock> InterfaceBlock::Convert(
    const Context& context, Position pos, const Modifiers& modifiers, std::string_view typeName, TArray<Field> fields, std::string_view varName, int arraySize)
{
    if (SkSL::ProgramKind kind = context.fConfig->fKind;
        !ProgramConfig::IsFragment(kind) && !ProgramConfig::IsVertex(kind) && !ProgramConfig::IsCompute(kind)) {
    ...
    const Type* baseType = context.fSymbolTable->add(context,
        Type::MakeStructType(context, pos, typeName, std::move(fields),
            /*interfaceBlock=*/true));
    ...
    VarDeclaration::ErrorCheck(context, pos, modifiers.fPosition, modifiers.fLayout, modifiers.fFlags, type, baseType, VariableStorage::kGlobal);

    // Create a global variable for the Interface Block.
    std::unique_ptr<SkSL::Variable> var
        = SkSL::Variable::Convert(context, pos, modifiers.fPosition, modifiers.fLayout, modifiers.fFlags, type, pos, varName, VariableStorage::kGlobal);
    return InterfaceBlock::Make(context, pos, context.fSymbolTable->takeOwnershipOfSymbol(std::move(var)));
}

std::unique_ptr<InterfaceBlock> InterfaceBlock::Make(const Context& context, Position pos, Variable* variable)
{
    SkASSERT(ProgramConfig::IsFragment(context.fConfig->fKind) || ProgramConfig::IsVertex(context.fConfig->fKind)
    ...
    if (variable->name().empty()) {
        ...
        for (size_t i = 0; i < fields.size(); ++i) {
            context.fSymbolTable->add(context, std::make_unique<SkSL::FieldSymbol>(fields[i].fPosition, variable, i));
        }
    } else {
...
{
    std::string result = this->var()->layout().description() + this->var()->modifierFlags().description() + ' ' + std::string(this->typeName()) + " {\n";
