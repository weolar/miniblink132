...
static bool index_out_of_range(const Context& context, Position pos, SKSL_INT index, const Expression& base)
{
    ...
    context.fErrors->error(pos, "index " + std::to_string(index) + " out of range for '" + base.type().displayName() + "'");
    return true;
}
...
std::unique_ptr<Expression> IndexExpression::Convert(const Context& context, Position pos, std::unique_ptr<Expression> base, std::unique_ptr<Expression> index)
{
    ...
    if (base->is<TypeReference>()) {
        ...
        return TypeReference::Convert(context, pos, context.fSymbolTable->addArrayDimension(context, &baseType, arraySize));
    }
    ...
    if (!baseType.isArray() && !baseType.isMatrix() && !baseType.isVector()) {
        context.fErrors->error(base->fPosition, "expected array, but found '" + baseType.displayName() + "'");
        return nullptr;
    }
...
std::unique_ptr<Expression> IndexExpression::Make(const Context& context, Position pos, std::unique_ptr<Expression> base, std::unique_ptr<Expression> index)
{
    const Type& baseType = base->type();
...
{
    return this->base()->description(OperatorPrecedence::kPostfix) + "[" + this->index()->description(OperatorPrecedence::kExpression) + "]";
}
