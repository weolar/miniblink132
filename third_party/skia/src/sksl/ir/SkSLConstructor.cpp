...
static std::unique_ptr<Expression> convert_compound_constructor(const Context& context, Position pos, const Type& type, ExpressionArray args)
{
    ...
    if (args.size() == 1) {
        ...
        if (type.isVector() && argument->type().isVector() && argument->type().componentType().matches(type.componentType())
            ...
            case 2:
                swizzleHint = "; use '.xy' instead";
                break;
            case 3:
                swizzleHint = "; use '.xyz' instead";
                break;
            default:
                swizzleHint = "";
            ...
            context.fErrors->error(
                pos, "'" + argument->type().displayName() + "' is not a valid parameter to '" + type.displayName() + "' constructor" + swizzleHint);
            return nullptr;
        }
        ...
        if (argument->type().isScalar()) {
            ...
            std::unique_ptr<Expression> typecast = ConstructorScalarCast::Convert(context, pos, type.componentType(), std::move(args));
            if (!typecast) {
                return nullptr;
            }

            // Matrix-from-scalar creates a diagonal matrix; vector-from-scalar creates a splat.
            return type.isMatrix() ? ConstructorDiagonalMatrix::Make(context, pos, type, std::move(typecast))
            ...
            if (type.isMatrix()) {
                ...
                const Type& typecastType = type.componentType().toCompound(context, argument->type().columns(), argument->type().rows());
            ...
            if (type.isVector() && type.columns() == 4 && argument->type().slotCount() == 4) {
                ...
                const Type& vectorType = argument->type().componentType().toCompound(context,
                    /*columns=*/4,
                    /*rows=*/1);
                std::unique_ptr<Expression> vecCtor = ConstructorCompound::Make(context, pos, vectorType, std::move(args));
    ...
    for (std::unique_ptr<Expression>& arg : args) {
        if (!arg->type().isScalar() && !arg->type().isVector()) {
            context.fErrors->error(pos, "'" + arg->type().displayName() + "' is not a valid parameter to '" + type.displayName() + "' constructor");
            return nullptr;
        }
    ...
    if (actual != expected) {
        context.fErrors->error(pos,
            "invalid arguments to '" + type.displayName() + "' constructor (expected " + std::to_string(expected) + " scalars, but found "
...
std::unique_ptr<Expression> Constructor::Convert(const Context& context, Position pos, const Type& type, ExpressionArray args)
{
