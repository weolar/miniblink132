...
std::unique_ptr<Statement> IfStatement::Convert(
    const Context& context, Position pos, std::unique_ptr<Expression> test, std::unique_ptr<Statement> ifTrue, std::unique_ptr<Statement> ifFalse)
{
    ...
    const Context& context, Position pos, std::unique_ptr<Expression> test, std::unique_ptr<Statement> ifTrue, std::unique_ptr<Statement> ifFalse)
{
    ...
    return std::make_unique<IfStatement>(pos, std::move(test), std::move(ifTrue), std::move(ifFalse));
}
