...
std::unique_ptr<Expression> ConstructorStruct::Convert(const Context& context, Position pos, const Type& type, ExpressionArray args)
{
    ...
    if (type.isOrContainsAtomic()) {
        context.fErrors->error(pos, String::printf("construction of struct type '%s' with atomic member is not allowed", type.displayName().c_str()));
        return nullptr;
    }
...
[[maybe_unused]] static bool arguments_match_field_types(const ExpressionArray& args, const Type& type)
{
...
std::unique_ptr<Expression> ConstructorStruct::Make(const Context& context, Position pos, const Type& type, ExpressionArray args)
{
