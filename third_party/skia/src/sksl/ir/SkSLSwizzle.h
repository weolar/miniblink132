...
enum Type : int8_t { X = 0, Y = 1, Z = 2, W = 3, R = 4, G = 5, B = 6, A = 7, S = 8, T = 9, P = 10, Q = 11, UL = 12, UT = 13, UR = 14, UB = 15, ZERO, ONE };
...
public:
    ...
    Swizzle(const Context& context, Position pos, std::unique_ptr<Expression> base, const ComponentArray& components)
        : INHERITED(pos, kIRNodeKind, &base->type().componentType().toCompound(context, components.size(), 1))
        , fBase(std::move(base))
    ...
    // reported via ASSERT.
    static std::unique_ptr<Expression> Make(const Context& context, Position pos, std::unique_ptr<Expression> expr, ComponentArray components);

    // Swizzle::MakeExact creates the swizzle as requested, without simplification.
    static std::unique_ptr<Expression> MakeExact(const Context& context, Position pos, std::unique_ptr<Expression> expr, ComponentArray components);
    ...
    {
        return std::unique_ptr<Expression>(new Swizzle(pos, &this->type(), this->base()->clone(), this->components()));
    }
...
private:
    Swizzle(Position pos, const Type* type, std::unique_ptr<Expression> base, const ComponentArray& components)
