...
void SymbolTable::renameSymbol(const Context& context, Symbol* symbol, std::string_view newName)
{
    if (symbol->is<FunctionDeclaration>()) {
        ...
        for (FunctionDeclaration* fn = &symbol->as<FunctionDeclaration>(); fn != nullptr; fn = fn->mutableNextOverload()) {
            fn->setName(newName);
        }
    } else {
...
void SymbolTable::addWithoutOwnership(const Context& context, Symbol* symbol)
{
    if (!this->addWithoutOwnership(symbol)) {
        context.fErrors->error(symbol->position(), "symbol '" + std::string(symbol->name()) + "' was already defined");
    }
}
...
{
    if (!this->addWithoutOwnership(symbol)) {
        SK_ABORT("symbol '%.*s' was already defined", (int)symbol->name().size(), symbol->name().data());
    }
}
...
const Type* SymbolTable::addArrayDimension(const Context& context, const Type* type, int arraySize)
{
    if (arraySize == 0) {
        return type;
    }
...
std::unique_ptr<Expression> SymbolTable::instantiateSymbolRef(const Context& context, std::string_view name, Position pos)
{
