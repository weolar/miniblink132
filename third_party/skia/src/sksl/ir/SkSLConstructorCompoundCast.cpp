...
static std::unique_ptr<Expression> cast_constant_composite(const Context& context, Position pos, const Type& destType, std::unique_ptr<Expression> constCtor)
{
    ...
    if (constCtor->is<ConstructorSplat>()) {
        ...
        return ConstructorSplat::Make(context, pos, destType, ConstructorScalarCast::Make(context, pos, scalarType, std::move(splat.argument())));
    }
    ...
    if (constCtor->is<ConstructorDiagonalMatrix>() && destType.isMatrix()) {
        ...
        return ConstructorDiagonalMatrix::Make(context, pos, destType, ConstructorScalarCast::Make(context, pos, scalarType, std::move(matrixCtor.argument())));
    }
...
std::unique_ptr<Expression> ConstructorCompoundCast::Make(const Context& context, Position pos, const Type& type, std::unique_ptr<Expression> arg)
{
