...
std::unique_ptr<Expression> ConstructorScalarCast::Convert(const Context& context, Position pos, const Type& rawType, ExpressionArray args)
{
    ...
    if (args.size() != 1) {
        context.fErrors->error(
            pos, "invalid arguments to '" + type.displayName() + "' constructor, (expected exactly 1 argument, but found " + std::to_string(args.size()) + ")");
        return nullptr;
    }
    ...
    if (!argType.isScalar()) {
        ...
        context.fErrors->error(pos, "'" + argType.displayName() + "' is not a valid parameter to '" + type.displayName() + "' constructor" + swizzleHint);
        return nullptr;
    }
...
std::unique_ptr<Expression> ConstructorScalarCast::Make(const Context& context, Position pos, const Type& type, std::unique_ptr<Expression> arg)
{
    SkASSERT(type.isScalar());
