...
Program::Program(std::unique_ptr<std::string> source, std::unique_ptr<ProgramConfig> config, std::shared_ptr<Context> context,
...
const FunctionDeclaration* Program::getFunction(const char* functionName) const
{
    ...
    bool valid = symbol && symbol->is<FunctionDeclaration>() && symbol->as<FunctionDeclaration>().definition();
