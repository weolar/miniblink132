...
public:
    ...
    FieldSymbol(Position pos, const Variable* owner, int fieldIndex)
        : INHERITED(pos, kIRNodeKind, owner->type().fields()[fieldIndex].fName, owner->type().fields()[fieldIndex].fType)
        , fOwner(owner)
    ...
    {
        return this->owner().name().empty() ? std::string(this->name()) : (this->owner().description() + "." + std::string(this->name()));
    }
