    ...
    if (const SkSL::Symbol* rtAdjust = context.fSymbolTable->find(Compiler::RTADJUST_NAME)) {
        ...
        struct AppendRTAdjustFixupHelper : public IRHelpers {
            AppendRTAdjustFixupHelper(const Context& ctx, const SkSL::Symbol* rtAdjust)
                ...
                , fRTAdjust(rtAdjust)
            {
                fSkPositionField = &fContext.fSymbolTable->find(Compiler::POSITION_NAME)->as<FieldSymbol>();
            }
            ...
            std::unique_ptr<Statement> makeFixupStmt() const
            {
                ...
                return Assign(Pos(),
                    CtorXYZW(
                        Add(Mul(Swizzle(Pos(), { SwizzleComponent::X, SwizzleComponent::Y }), Swizzle(Adjust(), { SwizzleComponent::X, SwizzleComponent::Z })),
                            Mul(Swizzle(Pos(), { SwizzleComponent::W, SwizzleComponent::W }), Swizzle(Adjust(), { SwizzleComponent::Y, SwizzleComponent::W }))),
                        Float(0.0), Swizzle(Pos(), { SwizzleComponent::W })));
            }
...
std::unique_ptr<FunctionDefinition> FunctionDefinition::Convert(
    const Context& context, Position pos, const FunctionDeclaration& function, std::unique_ptr<Statement> body)
{
        ...
        void fuseVariableDeclarationsWithInitialization(std::unique_ptr<Statement>& stmt)
        {
            ...
            case Statement::Kind::kExpression: {
                ...
                if (fUninitializedVarDecl) {
                    ...
                    std::unique_ptr<Expression>& nextExpr = stmt->as<ExpressionStatement>().expression();
        ...
        bool visitStatementPtr(std::unique_ptr<Statement>& stmt) override
        {
            ...
            case Statement::Kind::kReturn: {
                ...
                if (ProgramConfig::IsVertex(fContext.fConfig->fKind) && fFunction.isMain()) {
                    fContext.fErrors->error(stmt->fPosition, "early returns from vertex programs are not supported");
                }
                ...
                if (returnStmt.expression()) {
                    if (this->functionReturnsValue()) {
                        ...
                        returnStmt.setExpression(fFunction.returnType().coerceExpression(std::move(returnStmt.expression()), fContext));
                    } else {
                    ...
                    if (this->functionReturnsValue()) {
                        ...
                        fContext.fErrors->error(returnStmt.fPosition, "expected function to return '" + fFunction.returnType().displayName() + "'");
                    }
                }
                break;
            }
            ...
            case Statement::Kind::kContinue:
                if (fContinuableLevel.front() == 0) {
                    if (std::any_of(fContinuableLevel.begin(), fContinuableLevel.end(), [](int level) { return level > 0; })) {
    ...
    if (function.isIntrinsic()) {
        context.fErrors->error(pos, "intrinsic function '" + std::string(function.name()) + "' should not have a definition");
        return nullptr;
    }
    ...
    if (!body || !body->is<Block>() || !body->as<Block>().isScope()) {
        context.fErrors->error(pos, "function body '" + function.description() + "' must be a braced block");
        return nullptr;
    }

    // A function can't have more than one definition.
    if (function.definition()) {
        context.fErrors->error(pos, "function '" + function.description() + "' was already defined");
        return nullptr;
    }
...
std::unique_ptr<FunctionDefinition> FunctionDefinition::Make(
    const Context& context, Position pos, const FunctionDeclaration& function, std::unique_ptr<Statement> body)
{
