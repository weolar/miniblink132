...
template <typename T> static void type_check_expression(const Expression& expr);

template <> void type_check_expression<float>(const Expression& expr)
{
    SkASSERT(expr.type().componentType().isFloat());
}

template <> void type_check_expression<SKSL_INT>(const Expression& expr)
{
    SkASSERT(expr.type().componentType().isInteger());
}

template <> void type_check_expression<bool>(const Expression& expr)
{
...
static std::unique_ptr<Expression> coalesce_n_way_vector(
    const Expression* arg0, const Expression* arg1, double startingState, const Type& returnType, CoalesceFn coalesce, FinalizeFn finalize)
{
    ...
    const Type& vecType = arg0->type().isVector() ? arg0->type() : (arg1 && arg1->type().isVector()) ? arg1->type() : arg0->type();
...
static std::unique_ptr<Expression> coalesce_vector(
    const IntrinsicArguments& arguments, double startingState, const Type& returnType, CoalesceFn coalesce, FinalizeFn finalize)
{
    SkASSERT(arguments[0]);
    SkASSERT(!arguments[1]);
    type_check_expression<T>(*arguments[0]);

    return coalesce_n_way_vector(arguments[0], /*arg1=*/nullptr, startingState, returnType, coalesce, finalize);
}
    ...
    const IntrinsicArguments& arguments, double startingState, const Type& returnType, CoalesceFn coalesce, FinalizeFn finalize)
{
    ...
    return coalesce_n_way_vector(arguments[0], arguments[1], startingState, returnType, coalesce, finalize);
}
...
static std::unique_ptr<Expression> evaluate_n_way_intrinsic(
    const Context& context, const Expression* arg0, const Expression* arg1, const Expression* arg2, const Type& returnType, EvaluateFn eval)
{
...
static std::unique_ptr<Expression> evaluate_intrinsic(const Context& context, const IntrinsicArguments& arguments, const Type& returnType, EvaluateFn eval)
{
    ...
    return evaluate_n_way_intrinsic(context, arguments[0], /*arg1=*/nullptr, /*arg2=*/nullptr, returnType, eval);
}

static std::unique_ptr<Expression> evaluate_intrinsic_numeric(
    const Context& context, const IntrinsicArguments& arguments, const Type& returnType, EvaluateFn eval)
{
    SkASSERT(arguments[0]);
    SkASSERT(!arguments[1]);
    ...
    const Context& context, const IntrinsicArguments& arguments, const Type& returnType, EvaluateFn eval)
{
    ...
    return evaluate_n_way_intrinsic(context, arguments[0], arguments[1], /*arg2=*/nullptr, returnType, eval);
}
    ...
    const Context& context, const IntrinsicArguments& arguments, const Type& returnType, EvaluateFn eval)
{
    ...
    return evaluate_n_way_intrinsic(context, arguments[0], arguments[1], arguments[2], returnType, eval);
}

template <typename T1, typename T2> static double pun_value(double val)
{
...
{
    return sk_ieee_double_divide(a, b);
}
...
{
    return pun_value<float, int32_t>(a);
}
double evaluate_floatBitsToUint(double a, double, double)
{
    return pun_value<float, uint32_t>(a);
}
double evaluate_intBitsToFloat(double a, double, double)
{
    return pun_value<int32_t, float>(a);
}
double evaluate_uintBitsToFloat(double a, double, double)
{
    return pun_value<uint32_t, float>(a);
}

std::unique_ptr<Expression> evaluate_length(const IntrinsicArguments& arguments)
{
    return coalesce_vector<float>(arguments, /*startingState=*/0, arguments[0]->type().componentType(), coalesce_length, finalize_length);
}

std::unique_ptr<Expression> evaluate_distance(const IntrinsicArguments& arguments)
{
    return coalesce_pairwise_vectors<float>(arguments, /*startingState=*/0, arguments[0]->type().componentType(), coalesce_distance, finalize_distance);
}
std::unique_ptr<Expression> evaluate_dot(const IntrinsicArguments& arguments)
{
    return coalesce_pairwise_vectors<float>(arguments, /*startingState=*/0, arguments[0]->type().componentType(), coalesce_dot,
        /*finalize=*/nullptr);
}

std::unique_ptr<Expression> evaluate_sign(const Context& context, const IntrinsicArguments& arguments)
{
    return evaluate_intrinsic_numeric(context, arguments, arguments[0]->type(), evaluate_sign);
}

std::unique_ptr<Expression> evaluate_opposite_sign(const Context& context, const IntrinsicArguments& arguments)
{
    return evaluate_intrinsic_numeric(context, arguments, arguments[0]->type(), evaluate_opposite_sign);
}

std::unique_ptr<Expression> evaluate_add(const Context& context, const IntrinsicArguments& arguments)
{
    return evaluate_pairwise_intrinsic(context, arguments, arguments[0]->type(), evaluate_add);
}

std::unique_ptr<Expression> evaluate_sub(const Context& context, const IntrinsicArguments& arguments)
{
    return evaluate_pairwise_intrinsic(context, arguments, arguments[0]->type(), evaluate_sub);
}

std::unique_ptr<Expression> evaluate_mul(const Context& context, const IntrinsicArguments& arguments)
{
    return evaluate_pairwise_intrinsic(context, arguments, arguments[0]->type(), evaluate_mul);
}

std::unique_ptr<Expression> evaluate_div(const Context& context, const IntrinsicArguments& arguments)
{
    return evaluate_pairwise_intrinsic(context, arguments, arguments[0]->type(), evaluate_div);
}

std::unique_ptr<Expression> evaluate_normalize(const Context& context, const IntrinsicArguments& arguments)
{
...
std::unique_ptr<Expression> evaluate_faceforward(const Context& context, const IntrinsicArguments& arguments)
{
...
std::unique_ptr<Expression> evaluate_reflect(const Context& context, const IntrinsicArguments& arguments)
{
...
std::unique_ptr<Expression> evaluate_refract(const Context& context, const IntrinsicArguments& arguments)
{
    const Expression* I = arguments[0]; // vector
    const Expression* N = arguments[1]; // vector
...
static std::unique_ptr<Expression> optimize_intrinsic_call(
    const Context& context, Position pos, IntrinsicKind intrinsic, const ExpressionArray& argArray, const Type& returnType)
{
    ...
    case k_radians_IntrinsicKind:
        return evaluate_intrinsic<float>(context, arguments, returnType, Intrinsics::evaluate_radians);
    case k_degrees_IntrinsicKind:
        return evaluate_intrinsic<float>(context, arguments, returnType, Intrinsics::evaluate_degrees);
    ...
    case k_atan_IntrinsicKind:
        if (argArray.size() == 1) {
            ...
            return evaluate_pairwise_intrinsic(context, arguments, returnType, Intrinsics::evaluate_atan2);
        }
    ...
    case k_inversesqrt_IntrinsicKind:
        return evaluate_intrinsic<float>(context, arguments, returnType, Intrinsics::evaluate_inversesqrt);
    ...
    case k_fma_IntrinsicKind:
        return evaluate_3_way_intrinsic(context, arguments, returnType, Intrinsics::evaluate_fma);
    case k_saturate_IntrinsicKind:
        return evaluate_intrinsic<float>(context, arguments, returnType, Intrinsics::evaluate_saturate);
    case k_mix_IntrinsicKind:
        if (arguments[2]->type().componentType().isBoolean()) {
            ...
            return evaluate_n_way_intrinsic(context, arguments[0], arguments[1], arguments[2], returnType, Intrinsics::evaluate_mix);
        } else {
    ...
    case k_smoothstep_IntrinsicKind:
        return evaluate_3_way_intrinsic(context, arguments, returnType, Intrinsics::evaluate_smoothstep);
    ...
    case k_floatBitsToInt_IntrinsicKind:
        return evaluate_intrinsic<float>(context, arguments, returnType, Intrinsics::evaluate_floatBitsToInt);
    case k_floatBitsToUint_IntrinsicKind:
        return evaluate_intrinsic<float>(context, arguments, returnType, Intrinsics::evaluate_floatBitsToUint);
    ...
    case k_uintBitsToFloat_IntrinsicKind:
        return evaluate_intrinsic<SKSL_INT>(context, arguments, returnType, Intrinsics::evaluate_uintBitsToFloat);
    ...
    case k_packUnorm2x16_IntrinsicKind: {
        ...
        const double packed = ((Pack(0) << 0) & 0x0000FFFF) | ((Pack(1) << 16) & 0xFFFF0000);
        return ConstructorCompound::MakeFromConstants(context, arguments[0]->fPosition, *context.fTypes.fUInt, &packed);
    }
    ...
    case k_packHalf2x16_IntrinsicKind: {
        auto Pack = [&](int n) -> unsigned int { return SkFloatToHalf(Get(0, n)); };
        const double packed = ((Pack(0) << 0) & 0x0000FFFF) | ((Pack(1) << 16) & 0xFFFF0000);
        return ConstructorCompound::MakeFromConstants(context, arguments[0]->fPosition, *context.fTypes.fUInt, &packed);
    }
    case k_unpackUnorm2x16_IntrinsicKind: {
        ...
        const double unpacked[2] = { double(a) / 65535.0, double(b) / 65535.0 };
        return ConstructorCompound::MakeFromConstants(context, arguments[0]->fPosition, *context.fTypes.fFloat2, unpacked);
    }
    case k_unpackSnorm2x16_IntrinsicKind: {
        SKSL_INT x = *arguments[0]->getConstantValue(0);
        int16_t a = ((x >> 0) & 0x0000FFFF);
        int16_t b = ((x >> 16) & 0x0000FFFF);
        const double unpacked[2] = { Intrinsics::evaluate_clamp(double(a) / 32767.0, -1.0, 1.0), Intrinsics::evaluate_clamp(double(b) / 32767.0, -1.0, 1.0) };
        return ConstructorCompound::MakeFromConstants(context, arguments[0]->fPosition, *context.fTypes.fFloat2, unpacked);
    }
    case k_unpackHalf2x16_IntrinsicKind: {
        ...
        uint16_t a = ((x >> 0) & 0x0000FFFF);
        uint16_t b = ((x >> 16) & 0x0000FFFF);
        const double unpacked[2] = { SkHalfToFloat(a), SkHalfToFloat(b) };
        return ConstructorCompound::MakeFromConstants(context, arguments[0]->fPosition, *context.fTypes.fFloat2, unpacked);
    }
    ...
    case k_cross_IntrinsicKind: {
        ...
        double vec[3] = { X(1) * Y(2) - Y(1) * X(2), X(2) * Y(0) - Y(2) * X(0), X(0) * Y(1) - Y(0) * X(1) };
        return ConstructorCompound::MakeFromConstants(context, arguments[0]->fPosition, returnType, vec);
    }
    ...
    case k_matrixCompMult_IntrinsicKind:
        return evaluate_pairwise_intrinsic(context, arguments, returnType, Intrinsics::evaluate_matrixCompMult);
    case k_transpose_IntrinsicKind: {
        ...
        return ConstructorCompound::MakeFromConstants(context, arguments[0]->fPosition, returnType, mat);
    }
    ...
    case k_inverse_IntrinsicKind: {
        ...
        return ConstructorCompound::MakeFromConstants(context, arguments[0]->fPosition, returnType, dmat);
    }
    ...
    case k_any_IntrinsicKind:
        return coalesce_vector<bool>(arguments, /*startingState=*/false, returnType, Intrinsics::coalesce_any,
            /*finalize=*/nullptr);
    case k_all_IntrinsicKind:
        return coalesce_vector<bool>(arguments, /*startingState=*/true, returnType, Intrinsics::coalesce_all,
            /*finalize=*/nullptr);
...
{
    return std::make_unique<FunctionCall>(pos, &this->type(), &this->function(), this->arguments().clone(), this->stablePointer());
}
...
const FunctionDeclaration* FunctionCall::FindBestFunctionForCall(
    const Context& context, const FunctionDeclaration* overloadChain, const ExpressionArray& arguments)
{
...
std::unique_ptr<Expression> FunctionCall::Convert(const Context& context, Position pos, std::unique_ptr<Expression> functionValue, ExpressionArray arguments)
{
    switch (functionValue->kind()) {
    case Expression::Kind::kTypeReference:
        return Constructor::Convert(context, pos, functionValue->as<TypeReference>().value(), std::move(arguments));
    case Expression::Kind::kFunctionReference: {
        const FunctionReference& ref = functionValue->as<FunctionReference>();
        const FunctionDeclaration* best = FindBestFunctionForCall(context, ref.overloadChain(), arguments);
        if (best) {
            return FunctionCall::Convert(context, pos, *best, std::move(arguments));
        }
        std::string msg = "no match for " + std::string(ref.overloadChain()->name()) + build_argument_type_list(arguments);
...
std::unique_ptr<Expression> FunctionCall::Convert(const Context& context, Position pos, const FunctionDeclaration& function, ExpressionArray arguments)
{
    ...
    if (function.parameters().size() != SkToSizeT(arguments.size())) {
        std::string msg = "call to '" + std::string(function.name()) + "' expected " + std::to_string(function.parameters().size()) + " argument";
    ...
    for (int i = 0; i < arguments.size(); i++) {
        ...
        const Variable& param = *function.parameters()[i];
        if (!argument_and_parameter_flags_match(arg, param)) {
            context.fErrors->error(arg.position(),
                "expected argument of type '" + param.layout().paddedDescription() + param.modifierFlags().paddedDescription() + param.type().description()
                    + "'");
            return nullptr;
        }
    }
    ...
    for (int i = 0; i < arguments.size(); i++) {
        ...
        if (paramFlags & ModifierFlag::kOut) {
            const VariableRefKind refKind = (paramFlags & ModifierFlag::kIn) ? VariableReference::RefKind::kReadWrite : VariableReference::RefKind::kPointer;
...
std::unique_ptr<Expression> FunctionCall::Make(
    const Context& context, Position pos, const Type* returnType, const FunctionDeclaration& function, ExpressionArray arguments)
{
    ...
    if (function.isIntrinsic() && has_compile_time_constant_arguments(arguments)) {
        ...
        if (std::unique_ptr<Expression> expr = optimize_intrinsic_call(context, pos, function.intrinsicKind(), arguments, *returnType)) {
            expr->fPosition = pos;
            return expr;
        }
    }
