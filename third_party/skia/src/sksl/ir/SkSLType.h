...
struct CoercionCost {
    ...
    {
        return std::tie(fImpossible, fNarrowingCost, fNormalCost) < std::tie(rhs.fImpossible, rhs.fNarrowingCost, rhs.fNormalCost);
    }
...
public:
    ...
    static std::unique_ptr<Type> MakeStructType(
        const Context& context, Position pos, std::string_view name, skia_private::TArray<Field> fields, bool interfaceBlock = false);
    ...
    {
        return fTypeKind == TypeKind::kColorFilter || fTypeKind == TypeKind::kShader || fTypeKind == TypeKind::kBlender;
    }
