...
static std::unique_ptr<Expression> cast_constant_array(const Context& context, Position pos, const Type& destType, std::unique_ptr<Expression> constCtor)
{
    ...
    for (std::unique_ptr<Expression>& arg : inputArgs) {
        ...
        if (arg->type().isScalar()) {
            typecastArgs.push_back(ConstructorScalarCast::Make(context, argPos, scalarType, std::move(arg)));
        } else {
            typecastArgs.push_back(ConstructorCompoundCast::Make(context, argPos, scalarType, std::move(arg)));
        }
    }
...
std::unique_ptr<Expression> ConstructorArrayCast::Make(const Context& context, Position pos, const Type& type, std::unique_ptr<Expression> arg)
{
