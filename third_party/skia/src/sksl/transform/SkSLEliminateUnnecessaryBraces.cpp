...
void Transform::EliminateUnnecessaryBraces(const Context& context, Module& module)
{
    ...
    public:
        RequiredBraceWriter(const Context& ctx)
            : fContext(ctx)
        {
        }
            ...
            if (stmt->is<IfStatement>()) {
                ...
                if (outer.ifFalse() && outer.ifTrue()->is<IfStatement>()) {
                    ...
                    if (!inner.ifFalse()) {
                        ...
                        std::unique_ptr<Statement> bracedIfTrue = Block::MakeBlock(stmtPosition, std::move(blockStmts));
                        stmt = IfStatement::Make(fContext, outer.position(), std::move(outer.test()), std::move(bracedIfTrue), std::move(outer.ifFalse()));
                    }
                }
            }
