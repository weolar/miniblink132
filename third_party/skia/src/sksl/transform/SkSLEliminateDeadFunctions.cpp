...
bool Transform::EliminateDeadFunctions(Program& program)
{
    ...
    if (program.fConfig->fSettings.fRemoveDeadFunctions) {
        program.fOwnedElements.erase(std::remove_if(program.fOwnedElements.begin(), program.fOwnedElements.end(),
                                         [&](const std::unique_ptr<ProgramElement>& pe) { return dead_function_predicate(pe.get(), usage); }),
            program.fOwnedElements.end());
        program.fSharedElements.erase(std::remove_if(program.fSharedElements.begin(), program.fSharedElements.end(),
                                          [&](const ProgramElement* pe) { return dead_function_predicate(pe, usage); }),
...
bool Transform::EliminateDeadFunctions(const Context& context, Module& module, ProgramUsage* usage)
{
    size_t numElements = module.fElements.size();

    if (context.fConfig->fSettings.fRemoveDeadFunctions) {
        module.fElements.erase(std::remove_if(module.fElements.begin(), module.fElements.end(),
                                   [&](const std::unique_ptr<ProgramElement>& pe) { return dead_function_predicate(pe.get(), usage); }),
