...
public:
    ...
    void addDeclaringElement(const Symbol* symbol)
    {
        ...
        if (const GlobalVarDeclaration* decl = var.globalVarDeclaration()) {
            ...
            SkASSERTF(var.storage() != VariableStorage::kGlobal && var.storage() != VariableStorage::kInterfaceBlock, "%.*s", (int)var.name().size(),
                var.name().data());
        }
    }
    ...
    {
        std::sort(fNewElements.begin(), fNewElements.end(), [](const ProgramElement* a, const ProgramElement* b) {
            if (a->kind() != b->kind()) {
...
void FindAndDeclareBuiltinVariables(Program& program)
{
    ...
    program.fSharedElements.insert(program.fSharedElements.begin(), scanner.fNewElements.begin(), scanner.fNewElements.end());
