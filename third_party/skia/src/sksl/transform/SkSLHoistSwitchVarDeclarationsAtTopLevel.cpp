...
std::unique_ptr<Block> Transform::HoistSwitchVarDeclarationsAtTopLevel(const Context& context, StatementArray& cases, SymbolTable& switchSymbols, Position pos)
{
    struct HoistSwitchVarDeclsVisitor : public ProgramWriter {
        HoistSwitchVarDeclsVisitor(const Context& c)
            : fContext(c)
        {
        }
    ...
    return Block::MakeBlock(pos, std::move(blockStmts), Block::Kind::kBracedScope, std::move(blockSymbols));
}
