...
void Transform::FindAndDeclareBuiltinFunctions(Program& program)
{
    ...
    for (;;) {
        ...
        for (const auto& [symbol, count] : usage->fCallCounts) {
            ...
            if (const FunctionDefinition* builtinDef = fn.definition()) {
                ...
                if (std::find(addedBuiltins.begin(), addedBuiltins.end(), builtinDef) == addedBuiltins.end()) {
        ...
        std::sort(
            addedBuiltins.begin() + numBuiltinsAtStart, addedBuiltins.end(), [](const FunctionDefinition* aDefinition, const FunctionDefinition* bDefinition) {
    ...
    program.fSharedElements.insert(program.fSharedElements.begin(), addedBuiltins.rbegin(), addedBuiltins.rend());
}
