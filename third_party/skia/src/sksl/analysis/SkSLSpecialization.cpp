...
static bool parameter_mappings_are_equal(const SpecializedParameters& left, const SpecializedParameters& right)
{
...
void FindFunctionsToSpecialize(const Program& program, SpecializationInfo* info, const ParameterMatchesFn& parameterMatchesFn)
{
            ...
            if (expr.is<FunctionCall>()) {
                ...
                if (!decl.isIntrinsic()) {
                    ...
                    for (int i = 0; i < numParams; i++) {
                        ...
                        } else if (arg.is<FieldAccess>() && arg.as<FieldAccess>().base()->is<VariableReference>()) {
                            argBase = arg.as<FieldAccess>().base()->as<VariableReference>().variable();
                        } else {
                            continue;
                        }
                        SkASSERT(argBase);
                        ...
                        if (argBase->storage() == Variable::Storage::kGlobal) {
                            ...
                            const Expression** uniformExpr = fInheritedSpecializations.find(argBase);
                    ...
                    if (specialization.count() > 0) {
                        ...
                        SpecializedCallKey callKey { call.stablePointer(), fInheritedSpecializationIndex };
...
SpecializationIndex FindSpecializationIndexForCall(const FunctionCall& call, const SpecializationInfo& info, SpecializationIndex parentSpecializationIndex)
{
    ...
    const FunctionDeclaration& func, const SpecializationInfo& info, SpecializationIndex specializationIndex, const ParameterMappingCallback& callback)
{
    if (specializationIndex != Analysis::kUnspecialized) {
        if (const Specializations* specializations = info.fSpecializationMap.find(&func)) {
            const Analysis::SpecializedParameters& specializedParams = specializations->at(specializationIndex);
            const SkSpan<Variable* const> funcParams = func.parameters();
