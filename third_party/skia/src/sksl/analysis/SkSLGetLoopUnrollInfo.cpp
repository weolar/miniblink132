...
std::unique_ptr<LoopUnrollInfo> Analysis::GetLoopUnrollInfo(const Context& context, Position loopPos, const ForLoopPositions& positions,
    const Statement* loopInitializer, std::unique_ptr<Expression>* loopTest, const Expression* loopNext, const Statement* loopStatement,
    ErrorReporter* errorPtr)
{
    ...
    if (!ConstantFolder::GetConstantValue(*initDecl.value(), &loopInfo->fStart)) {
        errors.error(loopInitializer->fPosition, "loop index initializer must be a constant expression");
        return nullptr;
    }
    ...
    auto is_loop_index = [&](const std::unique_ptr<Expression>& expr) {
        return expr->is<VariableReference>() && expr->as<VariableReference>().variable() == loopInfo->fIndex;
    };
    ...
    case Expression::Kind::kBinary: {
        ...
        case Operator::Kind::MINUSEQ:
            loopInfo->fDelta = -loopInfo->fDelta;
            break;
        default:
    ...
    if (Analysis::StatementWritesToVariable(*loopStatement, *initDecl.var())) {
        errors.error(loopStatement->fPosition, "loop index must not be modified within body of the loop");
        return nullptr;
    }
    ...
    case Operator::Kind::NEQ: {
        ...
        if (loopInfo->fIndex->type().componentType().isFloat()) {
            ...
            *loopTest = BinaryExpression::Make(context, cond->fPosition, cond->left()->clone(), op, cond->right()->clone());
