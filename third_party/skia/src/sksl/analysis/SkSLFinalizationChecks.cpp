...
public:
    FinalizationVisitor(const Context& c, const ProgramUsage& u)
        : fContext(c)
        , fUsage(u)
    {
    }
    ...
    void checkGlobalVariableSizeLimit(const GlobalVarDeclaration& globalDecl)
    {
        ...
        if (prevSlotsUsed < kVariableSlotLimit && fGlobalSlotsUsed >= kVariableSlotLimit) {
            fContext.fErrors->error(decl.fPosition, "global variable '" + std::string(decl.var()->name()) + "' exceeds the size limit");
        }
    }
            ...
            if (!fBindings.contains(key)) {
                ...
                if (set != -1) {
                    fContext.fErrors->error(
                        block.fPosition, "layout(set=" + std::to_string(set) + ", binding=" + std::to_string(binding) + ") has already been defined");
                } else {
                    fContext.fErrors->error(block.fPosition, "layout(binding=" + std::to_string(binding) + ") has already been defined");
                }
            }
        }
    }
    ...
    void checkOutParamsAreAssigned(const FunctionDefinition& funcDef)
    {
        ...
        for (const Variable* param : funcDecl.parameters()) {
            const ModifierFlags paramInout = param->modifierFlags() & (ModifierFlag::kIn | ModifierFlag::kOut);
            if (paramInout == ModifierFlag::kOut) {
                ProgramUsage::VariableCounts counts = fUsage.get(*param);
                if (counts.fWrite <= 0) {
                    fContext.fErrors->error(param->fPosition,
                        "function '" + std::string(funcDecl.name()) + "' never assigns a value to out parameter '" + std::string(param->name()) + "'");
                }
            }
        }
    }
    ...
    bool visitExpression(const Expression& expr) override
    {
        ...
        case Expression::Kind::kFunctionCall: {
            ...
            if (!decl.isBuiltin() && !decl.definition()) {
                fContext.fErrors->error(expr.fPosition, "function '" + decl.description() + "' is not defined");
            }
            break;
        }
...
void Analysis::DoFinalizationChecks(const Program& program)
{
    ...
    if (ProgramConfig::IsCompute(program.fConfig->fKind) && !visitor.definesLocalSize()) {
        program.fContext->fErrors->error(Position(), "compute programs must specify a workgroup size");
    }
}
