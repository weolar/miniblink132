...
struct SpecializedFunctionKey {
    ...
    {
        return fDeclaration == other.fDeclaration && fSpecializationIndex == other.fSpecializationIndex;
    }
...
struct SpecializedCallKey {
    struct Hash {
        ...
        {
            return SkGoodHash()(entry.fStablePointer) ^ SkGoodHash()(entry.fParentSpecializationIndex);
        }
    };

    bool operator==(const SpecializedCallKey& other) const
    {
        return fStablePointer == other.fStablePointer && fParentSpecializationIndex == other.fParentSpecializationIndex;
    }
...
using SpecializedCallMap = skia_private::THashMap<SpecializedCallKey, SpecializationIndex, SpecializedCallKey::Hash>;
...
void FindFunctionsToSpecialize(const Program& program, SpecializationInfo* info, const ParameterMatchesFn& specializationFn);
...
SpecializationIndex FindSpecializationIndexForCall(const FunctionCall& call, const SpecializationInfo& info, SpecializationIndex activeSpecializationIndex);
...
SkBitSet FindSpecializedParametersForFunction(const FunctionDeclaration& func, const SpecializationInfo& info);
...
using ParameterMappingCallback = std::function<void(int paramIndex, const Variable* param, const Expression* value)>;

void GetParameterMappingsForFunction(
    const FunctionDeclaration& func, const SpecializationInfo& info, SpecializationIndex specializationIndex, const ParameterMappingCallback& callback);
