...
public:
    ReturnsInputAlphaVisitor(const ProgramUsage& u)
        : fUsage(u)
    {
    }
    ...
    bool visitProgramElement(const ProgramElement& pe) override
    {
        ...
        if (parameters.size() != 1 || parameters[0]->type().columns() != 4 || !parameters[0]->type().componentType().isFloat()) {
    ...
    bool returnsInputAlpha(const Expression& expr)
    {
        ...
        if (expr.is<TernaryExpression>()) {
            ...
            return this->returnsInputAlpha(*ternary.ifTrue()) && this->returnsInputAlpha(*ternary.ifFalse());
        }
