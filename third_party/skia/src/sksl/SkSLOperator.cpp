...
OperatorPrecedence Operator::getBinaryPrecedence() const
{
    ...
    case Kind::LOGICALXOR:
        return OperatorPrecedence::kLogicalXor;
    case Kind::LOGICALOR:
    ...
    case Kind::COMMA:
        return OperatorPrecedence::kSequence;
    default:
        SK_ABORT("unsupported binary operator");
    }
}
...
bool Operator::determineBinaryType(
    const Context& context, const Type& left, const Type& right, const Type** outLeftType, const Type** outRightType, const Type** outResultType) const
{
    ...
    case Kind::LOGICALXOR: // left ^^ right
        ...
        *outResultType = context.fTypes.fBool.get();
        return left.canCoerceTo(*context.fTypes.fBool, allowNarrowing) && right.canCoerceTo(*context.fTypes.fBool, allowNarrowing);
    ...
    if (this->isMatrixMultiply(left, right)) { // left * right
        ...
        if (!this->determineBinaryType(context, left.componentType(), right.componentType(), outLeftType, outRightType, outResultType)) {
            return false;
        }
        ...
        *outRightType = &(*outResultType)->toCompound(context, right.columns(), right.rows());
        ...
        if (isAssignment && ((*outResultType)->columns() != leftColumns || (*outResultType)->rows() != leftRows)) {
            return false;
        }
    ...
    if (leftIsVectorOrMatrix && validMatrixOrVectorOp && right.isScalar()) {
        // Determine final component type.
        if (!this->determineBinaryType(context, left.componentType(), right, outLeftType, outRightType, outResultType)) {
            return false;
        }
        // Convert component type to compound.
    ...
    if (!isAssignment && rightIsVectorOrMatrix && validMatrixOrVectorOp && left.isScalar()) {
        // Determine final component type.
        if (!this->determineBinaryType(context, left, right.componentType(), outLeftType, outRightType, outResultType)) {
            return false;
        }
        // Convert component type to compound.
    ...
    CoercionCost leftToRightCost = isAssignment ? CoercionCost::Impossible() : left.coercionCost(right);
