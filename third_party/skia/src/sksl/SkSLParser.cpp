...
static ModifierFlags parse_modifier_token(Token::Kind token)
{
    ...
    case Token::Kind::TK_UNIFORM:
        return ModifierFlag::kUniform;
    case Token::Kind::TK_CONST:
        return ModifierFlag::kConst;
    case Token::Kind::TK_IN:
        return ModifierFlag::kIn;
    case Token::Kind::TK_OUT:
        return ModifierFlag::kOut;
    case Token::Kind::TK_INOUT:
        return ModifierFlag::kIn | ModifierFlag::kOut;
    case Token::Kind::TK_FLAT:
        return ModifierFlag::kFlat;
    case Token::Kind::TK_NOPERSPECTIVE:
        return ModifierFlag::kNoPerspective;
    case Token::Kind::TK_PURE:
        return ModifierFlag::kPure;
    case Token::Kind::TK_INLINE:
        return ModifierFlag::kInline;
    case Token::Kind::TK_NOINLINE:
        return ModifierFlag::kNoInline;
    case Token::Kind::TK_HIGHP:
        return ModifierFlag::kHighp;
    case Token::Kind::TK_MEDIUMP:
        return ModifierFlag::kMediump;
    case Token::Kind::TK_LOWP:
        return ModifierFlag::kLowp;
    case Token::Kind::TK_EXPORT:
        return ModifierFlag::kExport;
    case Token::Kind::TK_ES3:
        return ModifierFlag::kES3;
    case Token::Kind::TK_WORKGROUP:
        return ModifierFlag::kWorkgroup;
    case Token::Kind::TK_READONLY:
        return ModifierFlag::kReadOnly;
    case Token::Kind::TK_WRITEONLY:
        return ModifierFlag::kWriteOnly;
    case Token::Kind::TK_BUFFER:
        return ModifierFlag::kBuffer;
    case Token::Kind::TK_PIXELLOCAL:
        return ModifierFlag::kPixelLocal;
    default:
        return ModifierFlag::kNone;
    }
}
...
private:
    ...
    {
        return fParser->fCompiler.context().fSymbolTable;
    }
...
Parser::Parser(Compiler* compiler, const ProgramSettings& settings, ProgramKind kind, std::unique_ptr<std::string> text)
    : fCompiler(*compiler)
    , fSettings(settings)
    , fKind(kind)
    , fText(std::move(text))
...
bool Parser::expectNewline()
{
    ...
    if (token.fKind == Token::Kind::TK_WHITESPACE) {
        ...
        if (tokenText.find_first_of('\r') != std::string_view::npos || tokenText.find_first_of('\n') != std::string_view::npos) {
            return true;
        }
    }
...
bool Parser::expect(Token::Kind kind, const char* expected, Token* result)
{
    ...
    if (next.fKind == kind) {
        ...
        this->error(next, "expected " + std::string(expected) + ", but found '" + std::string(this->text(next)) + "'");
...
bool Parser::expectIdentifier(Token* result)
{
    ...
    if (this->symbolTable()->isBuiltinType(this->text(*result))) {
        this->error(*result, "expected an identifier, but found type '" + std::string(this->text(*result)) + "'");
        this->fEncounteredFatalError = true;
        return false;
    }
    return true;
...
{
    int offset = fPushback.fKind != Token::Kind::TK_NONE ? fPushback.fOffset : fLexer.getCheckpoint().fOffset;
...
void Parser::extensionDirective(Position start)
{
    ...
    if (this->expectNewline()) {
        std::unique_ptr<SkSL::Extension> ext = Extension::Convert(fCompiler.context(), this->rangeFrom(start), this->text(name), this->text(behavior));
        if (ext) {
...
{
    std::unique_ptr<ModifiersDeclaration> decl = ModifiersDeclaration::Convert(fCompiler.context(), mods);
    if (!decl) {
        return false;
    }
...
bool Parser::declaration()
{
    ...
    if (lookahead.fKind == Token::Kind::TK_IDENTIFIER && !this->symbolTable()->isType(this->text(lookahead))) {
...
bool Parser::functionDeclarationEnd(Position start, Modifiers& modifiers, const Type* returnType, const Token& name)
{
    Token lookahead = this->peek();
    ...
    if (validParams) {
        decl = SkSL::FunctionDeclaration::Convert(
            fCompiler.context(), this->rangeFrom(start), modifiers, this->text(name), std::move(parameters), start, returnType);
    }
...
bool Parser::defineFunction(SkSL::FunctionDeclaration* decl)
{
    ...
    std::unique_ptr<FunctionDefinition> function = FunctionDefinition::Convert(context, pos, *decl, std::move(block));
    if (!function) {
        return false;
    }
...
const Type* Parser::unsizedArrayType(const Type* base, Position pos)
{
    ...
    return this->symbolTable()->addArrayDimension(fCompiler.context(), base, SkSL::Type::kUnsizedArray);
}
...
void Parser::globalVarDeclarationEnd(Position pos, const Modifiers& mods, const Type* baseType, Token name)
{
    ...
    this->addGlobalVarDeclaration(VarDeclaration::Convert(
        fCompiler.context(), this->rangeFrom(pos), mods, *type, this->position(name), this->text(name), VariableStorage::kGlobal, std::move(initializer)));
    while (this->checkNext(Token::Kind::TK_COMMA)) {
        ...
        this->addGlobalVarDeclaration(VarDeclaration::Convert(fCompiler.context(), this->rangeFrom(identifierName), mods, *type, this->position(identifierName),
            this->text(identifierName), VariableStorage::kGlobal, std::move(anotherInitializer)));
    }
...
/* (LBRACKET expression? RBRACKET)* (EQ assignmentExpression)? (COMMA IDENTIFER
   (LBRACKET expression? RBRACKET)* (EQ assignmentExpression)?)* SEMICOLON */
std::unique_ptr<Statement> Parser::localVarDeclarationEnd(Position pos, const Modifiers& mods, const Type* baseType, Token name)
{
    const Type* type = baseType;
    std::unique_ptr<Expression> initializer;
    if (!this->parseArrayDimensions(pos, &type)) {
    ...
    std::unique_ptr<Statement> result = VarDeclaration::Convert(
        fCompiler.context(), this->rangeFrom(pos), mods, *type, this->position(name), this->text(name), VariableStorage::kLocal, std::move(initializer));
    for (;;) {
        if (!this->checkNext(Token::Kind::TK_COMMA)) {
            this->expect(Token::Kind::TK_SEMICOLON, "';'");
            break;
        ...
        std::unique_ptr<Statement> next = VarDeclaration::Convert(fCompiler.context(), this->rangeFrom(identifierName), mods, *type,
            this->position(identifierName), this->text(identifierName), VariableStorage::kLocal, std::move(anotherInitializer));
...
std::unique_ptr<Statement> Parser::varDeclarationsOrExpressionStatement()
{
    ...
    if (nextToken.fKind == Token::Kind::TK_HIGHP || nextToken.fKind == Token::Kind::TK_MEDIUMP || nextToken.fKind == Token::Kind::TK_LOWP
        ...
        if (this->varDeclarationsPrefix(&prefix)) {
            checkpoint.accept();
            return this->localVarDeclarationEnd(prefix.fPosition, prefix.fModifiers, prefix.fType, prefix.fName);
        }
...
const Type* Parser::structDeclaration()
{
    ...
    while (!this->checkNext(Token::Kind::TK_RBRACE)) {
        ...
        do {
            ...
            while (this->checkNext(Token::Kind::TK_LBRACKET)) {
                ...
                actualType = this->arrayType(actualType, size, this->rangeFrom(this->position(fieldStart)));
            }

            fields.push_back(SkSL::Field(this->rangeFrom(fieldStart), modifiers.fLayout, modifiers.fFlags, this->text(memberName), actualType));
        } while (this->checkNext(Token::Kind::TK_COMMA));
    ...
    std::unique_ptr<SkSL::StructDefinition> def = StructDefinition::Convert(fCompiler.context(), this->rangeFrom(start), this->text(name), std::move(fields));
    if (!def) {
        return nullptr;
    }
...
bool Parser::parameter(std::unique_ptr<SkSL::Variable>* outParam)
{
    ...
    *outParam = SkSL::Variable::Convert(fCompiler.context(), this->rangeFrom(pos), modifiers.fPosition, modifiers.fLayout, modifiers.fFlags, type, namePos,
        nameText, VariableStorage::kParameter);
    return true;
}
...
SkSL::Layout Parser::layout()
{
    ...
    Layout result;
    if (this->checkNext(Token::Kind::TK_LAYOUT) && this->expect(Token::Kind::TK_LPAREN, "'('")) {
        ...
        for (;;) {
            ...
            if (!found) {
                ...
                if (result.fFlags & *found) {
                    this->error(t, "layout qualifier '" + std::string(text) + "' appears more than once");
                }
...
const Type* Parser::findType(Position pos, Modifiers* modifiers, std::string_view name)
{
    const Context& context = fCompiler.context();
    ...
    return modifiers ? type->applyQualifiers(context, &modifiers->fFlags, qualifierRange) : type;
}
...
bool Parser::interfaceBlock(const Modifiers& modifiers)
{
    ...
    while (!this->checkNext(Token::Kind::TK_RBRACE)) {
        ...
        do {
            ...
            fields.push_back(SkSL::Field(this->rangeFrom(fieldPos), fieldModifiers.fLayout, fieldModifiers.fFlags, this->text(fieldName), actualType));
        } while (this->checkNext(Token::Kind::TK_COMMA));
    }
    ...
    if (std::unique_ptr<SkSL::InterfaceBlock> ib
        = InterfaceBlock::Convert(fCompiler.context(), this->position(typeName), modifiers, this->text(typeName), std::move(fields), instanceName, size)) {
...
std::unique_ptr<Statement> Parser::ifStatement()
{
    ...
    return this->statementOrNop(pos, IfStatement::Convert(fCompiler.context(), pos, std::move(test), std::move(ifTrue), std::move(ifFalse)));
}
...
std::unique_ptr<Statement> Parser::doStatement()
{
    ...
    Position pos = this->rangeFrom(start);
    return this->statementOrNop(pos, DoStatement::Convert(fCompiler.context(), pos, std::move(statement), std::move(test)));
}
...
std::unique_ptr<Statement> Parser::whileStatement()
{
    ...
    Position pos = this->rangeFrom(start);
    return this->statementOrNop(pos, ForStatement::ConvertWhile(fCompiler.context(), pos, std::move(test), std::move(statement)));
}

/* COLON statement* */
bool Parser::switchCaseBody(ExpressionArray* values, StatementArray* caseBlocks, std::unique_ptr<Expression> caseValue)
{
    if (!this->expect(Token::Kind::TK_COLON, "':'")) {
        return false;
    }
    StatementArray statements;
    while (this->peek().fKind != Token::Kind::TK_RBRACE && this->peek().fKind != Token::Kind::TK_CASE && this->peek().fKind != Token::Kind::TK_DEFAULT) {
    ...
    caseBlocks->push_back(SkSL::Block::Make(Position(), std::move(statements), Block::Kind::kUnbracedBlock));
    return true;
}
...
std::unique_ptr<Statement> Parser::switchStatement()
{
    ...
    Position pos = this->rangeFrom(start);
    return this->statementOrNop(
        pos, SwitchStatement::Convert(fCompiler.context(), pos, std::move(value), std::move(values), std::move(caseBlocks), std::move(symbolTable)));
}
...
std::unique_ptr<Statement> Parser::forStatement()
{
    ...
    return this->statementOrNop(pos,
        ForStatement::Convert(
            fCompiler.context(), pos, loopPositions, std::move(initializer), std::move(test), std::move(next), std::move(statement), std::move(symbolTable)));
}
...
std::unique_ptr<Statement> Parser::block(bool introduceNewScope, std::unique_ptr<SymbolTable>* adoptExistingSymbolTable)
{
    ...
    std::unique_ptr<SymbolTable>* symbolTableToUse = adoptExistingSymbolTable ? adoptExistingSymbolTable : &newSymbolTable;
    ...
    return SkSL::Block::MakeBlock(this->rangeFrom(start), std::move(statements), Block::Kind::kBracedScope, std::move(*symbolTableToUse));
}
...
std::unique_ptr<Statement> Parser::expressionStatement()
{
    ...
    return this->statementOrNop(pos, SkSL::ExpressionStatement::Convert(fCompiler.context(), std::move(expr)));
}
...
std::unique_ptr<Expression> Parser::expressionOrPoison(Position pos, std::unique_ptr<Expression> expr)
{
    if (!expr) {
    ...
    SkASSERTF(!pos.valid() || expr->position() == pos, "expected expression position (%d-%d), but received (%d-%d)", pos.startOffset(), pos.endOffset(),
        expr->position().startOffset(), expr->position().endOffset());
    return expr;
}

bool Parser::operatorRight(Parser::AutoDepth& depth, Operator::Kind op, BinaryParseFn rightFn, std::unique_ptr<Expression>& expr)
{
    this->nextToken();
    if (!depth.increase()) {
        return false;
    ...
    expr = this->expressionOrPoison(pos, BinaryExpression::Convert(fCompiler.context(), pos, std::move(expr), op, std::move(right)));
    return true;
}
...
std::unique_ptr<Expression> Parser::expression()
{
    ...
    SkASSERTF(result->position().valid(), "Expression %s has invalid position", result->description().c_str());
    SkASSERTF(result->position().startOffset() == this->position(start).startOffset(),
        ...
        (int)this->text(start).length(), this->text(start).data(), result->position().startOffset(), result->position().endOffset());
    return result;
}
...
std::unique_ptr<Expression> Parser::assignmentExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_STAREQ:
            op = Operator::Kind::STAREQ;
            break;
        case Token::Kind::TK_SLASHEQ:
            op = Operator::Kind::SLASHEQ;
            break;
        case Token::Kind::TK_PERCENTEQ:
            op = Operator::Kind::PERCENTEQ;
            break;
        case Token::Kind::TK_PLUSEQ:
            op = Operator::Kind::PLUSEQ;
            break;
        case Token::Kind::TK_MINUSEQ:
            op = Operator::Kind::MINUSEQ;
            break;
        case Token::Kind::TK_SHLEQ:
            op = Operator::Kind::SHLEQ;
            break;
        case Token::Kind::TK_SHREQ:
            op = Operator::Kind::SHREQ;
            break;
        case Token::Kind::TK_BITWISEANDEQ:
            op = Operator::Kind::BITWISEANDEQ;
            break;
        case Token::Kind::TK_BITWISEXOREQ:
            op = Operator::Kind::BITWISEXOREQ;
            break;
        case Token::Kind::TK_BITWISEOREQ:
            op = Operator::Kind::BITWISEOREQ;
            break;
        default:
            return result;
        }
...
std::unique_ptr<Expression> Parser::ternaryExpression()
{
    ...
    return this->expressionOrPoison(pos, TernaryExpression::Convert(fCompiler.context(), pos, std::move(base), std::move(trueExpr), std::move(falseExpr)));
}
...
std::unique_ptr<Expression> Parser::logicalOrExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_LOGICALOR) {
        if (!this->operatorRight(depth, Operator::Kind::LOGICALOR, &Parser::logicalXorExpression, result)) {
            return nullptr;
        }
    }
    return result;
...
std::unique_ptr<Expression> Parser::logicalXorExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_LOGICALXOR) {
        if (!this->operatorRight(depth, Operator::Kind::LOGICALXOR, &Parser::logicalAndExpression, result)) {
            return nullptr;
        }
    }
    return result;
...
std::unique_ptr<Expression> Parser::logicalAndExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_LOGICALAND) {
        if (!this->operatorRight(depth, Operator::Kind::LOGICALAND, &Parser::bitwiseOrExpression, result)) {
            return nullptr;
        }
    }
    return result;
...
std::unique_ptr<Expression> Parser::bitwiseOrExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_BITWISEOR) {
        if (!this->operatorRight(depth, Operator::Kind::BITWISEOR, &Parser::bitwiseXorExpression, result)) {
            return nullptr;
        }
    }
    return result;
...
std::unique_ptr<Expression> Parser::bitwiseXorExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_BITWISEXOR) {
        if (!this->operatorRight(depth, Operator::Kind::BITWISEXOR, &Parser::bitwiseAndExpression, result)) {
            return nullptr;
        }
    }
    return result;
...
std::unique_ptr<Expression> Parser::bitwiseAndExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_BITWISEAND) {
        if (!this->operatorRight(depth, Operator::Kind::BITWISEAND, &Parser::equalityExpression, result)) {
            return nullptr;
        }
    }
    return result;
...
std::unique_ptr<Expression> Parser::equalityExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_EQEQ:
            op = Operator::Kind::EQEQ;
            break;
        case Token::Kind::TK_NEQ:
            op = Operator::Kind::NEQ;
            break;
        default:
            return result;
        }
...
std::unique_ptr<Expression> Parser::relationalExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_LTEQ:
            op = Operator::Kind::LTEQ;
            break;
        case Token::Kind::TK_GTEQ:
            op = Operator::Kind::GTEQ;
            break;
        default:
            return result;
        }
...
std::unique_ptr<Expression> Parser::shiftExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_SHL:
            op = Operator::Kind::SHL;
            break;
        case Token::Kind::TK_SHR:
            op = Operator::Kind::SHR;
            break;
        default:
            return result;
        }
...
std::unique_ptr<Expression> Parser::additiveExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_PLUS:
            op = Operator::Kind::PLUS;
            break;
        case Token::Kind::TK_MINUS:
            op = Operator::Kind::MINUS;
            break;
        default:
            return result;
        }
...
std::unique_ptr<Expression> Parser::multiplicativeExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_STAR:
            op = Operator::Kind::STAR;
            break;
        case Token::Kind::TK_SLASH:
            op = Operator::Kind::SLASH;
            break;
        case Token::Kind::TK_PERCENT:
            op = Operator::Kind::PERCENT;
            break;
        default:
            return result;
        }
...
std::unique_ptr<Expression> Parser::unaryExpression()
{
    ...
    case Token::Kind::TK_LOGICALNOT:
        op = Operator::Kind::LOGICALNOT;
        break;
    case Token::Kind::TK_BITWISENOT:
        op = Operator::Kind::BITWISENOT;
        break;
    case Token::Kind::TK_PLUSPLUS:
        op = Operator::Kind::PLUSPLUS;
        break;
    case Token::Kind::TK_MINUSMINUS:
        op = Operator::Kind::MINUSMINUS;
        break;
    default:
    ...
    return this->expressionOrPoison(pos, PrefixExpression::Convert(fCompiler.context(), pos, op, std::move(expr)));
}
...
std::unique_ptr<Expression> Parser::swizzle(Position pos, std::unique_ptr<Expression> base, std::string_view swizzleMask, Position maskPos)
{
    SkASSERT(!swizzleMask.empty());
    if (!base->type().isVector() && !base->type().isScalar()) {
        return this->expressionOrPoison(pos, FieldAccess::Convert(fCompiler.context(), pos, std::move(base), swizzleMask));
    }
    return this->expressionOrPoison(pos, Swizzle::Convert(fCompiler.context(), pos, maskPos, std::move(base), swizzleMask));
}

std::unique_ptr<Expression> Parser::call(Position pos, std::unique_ptr<Expression> base, ExpressionArray args)
{
    return this->expressionOrPoison(pos, SkSL::FunctionCall::Convert(fCompiler.context(), pos, std::move(base), std::move(args)));
}
...
std::unique_ptr<Expression> Parser::suffix(std::unique_ptr<Expression> base)
{
    ...
    case Token::Kind::TK_LBRACKET: {
        ...
        return this->expressionOrPoison(pos, IndexExpression::Convert(fCompiler.context(), pos, std::move(base), std::move(index)));
    }
    case Token::Kind::TK_DOT: {
        std::string_view text;
        if (this->identifier(&text)) {
            Position pos = this->rangeFrom(base->position());
            return this->swizzle(pos, std::move(base), text, this->rangeFrom(this->position(next).after()));
        }
        [[fallthrough]];
    }
    case Token::Kind::TK_FLOAT_LITERAL: {
        ...
        if (id.fKind == Token::Kind::TK_IDENTIFIER) {
            ...
            return this->swizzle(pos, std::move(base), std::string(field) + std::string(this->text(id)), maskPos);
        }
        if (field.empty()) {
    ...
    case Token::Kind::TK_PLUSPLUS:
    case Token::Kind::TK_MINUSMINUS: {
        Operator::Kind op = (next.fKind == Token::Kind::TK_PLUSPLUS) ? Operator::Kind::PLUSPLUS : Operator::Kind::MINUSMINUS;
        Position pos = this->rangeFrom(base->position());
        return this->expressionOrPoison(pos, PostfixExpression::Convert(fCompiler.context(), pos, std::move(base), op));
    }
    default: {
        this->error(next, "expected expression suffix, but found '" + std::string(this->text(next)) + "'");
        return nullptr;
    }
    }
}
...
std::unique_ptr<Expression> Parser::term()
{
    ...
    case Token::Kind::TK_IDENTIFIER: {
        ...
        if (this->identifier(&text)) {
            ...
            return this->expressionOrPoison(pos, this->symbolTable()->instantiateSymbolRef(fCompiler.context(), text, pos));
        }
        break;
    }
    case Token::Kind::TK_INT_LITERAL: {
        ...
        Position pos = this->position(t);
        return this->expressionOrPoison(pos, SkSL::Literal::MakeInt(fCompiler.context(), pos, i));
    }
    case Token::Kind::TK_FLOAT_LITERAL: {
        SKSL_FLOAT f;
        ...
        Position pos = this->position(t);
        return this->expressionOrPoison(pos, SkSL::Literal::MakeFloat(fCompiler.context(), pos, f));
    }
    ...
    case Token::Kind::TK_FALSE_LITERAL: {
        ...
        Position pos = this->position(t);
        return this->expressionOrPoison(pos, SkSL::Literal::MakeBool(fCompiler.context(), pos, b));
    }
    case Token::Kind::TK_LPAREN: {
        this->nextToken();
        if (!depth.increase()) {
