...
private:
    ...
    bool functionDeclarationEnd(Position start, Modifiers& modifiers, const Type* returnType, const Token& name);
    ...
    {
        return ProgramConfig::IsCompute(fKind) || ProgramConfig::IsFragment(fKind) || ProgramConfig::IsVertex(fKind);
    }
    ...
    void globalVarDeclarationEnd(Position position, const Modifiers& mods, const Type* baseType, Token name);

    std::unique_ptr<Statement> localVarDeclarationEnd(Position position, const Modifiers& mods, const Type* baseType, Token name);
    ...
    bool switchCaseBody(ExpressionArray* values, StatementArray* caseBlocks, std::unique_ptr<Expression> value);
    ...
    std::unique_ptr<Statement> block(bool introduceNewScope, std::unique_ptr<SymbolTable>* adoptExistingSymbolTable);

    std::unique_ptr<Statement> expressionStatement();

    using BinaryParseFn = std::unique_ptr<Expression> (Parser::*)();
    [[nodiscard]] bool operatorRight(AutoDepth& depth, Operator::Kind op, BinaryParseFn rightFn, std::unique_ptr<Expression>& expr);
    ...
    std::unique_ptr<Expression> swizzle(Position pos, std::unique_ptr<Expression> base, std::string_view swizzleMask, Position maskPos);

    std::unique_ptr<Expression> call(Position pos, std::unique_ptr<Expression> base, ExpressionArray args);
