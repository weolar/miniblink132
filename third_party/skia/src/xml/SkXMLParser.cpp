...
static char const* const gErrorStrings[]
    = { "empty or missing file ", "unknown element ", "unknown attribute name ", "error in attribute value ", "duplicate ID ", "unknown error " };
...
void XMLCALL entity_decl_handler(void* data, const XML_Char* entityName, int is_parameter_entity, const XML_Char* value, int value_length, const XML_Char* base,
...
bool SkXMLParser::parse(SkStream& docStream)
{
    ...
    unsigned long seed = static_cast<unsigned long>(reinterpret_cast<size_t>(kHashSeed) & 0xFFFFFFFF);
    ...
    if (docStream.getMemoryBase() && docStream.hasLength()) {
        ...
        status = XML_Parse(ctx.fXMLParser, base + docStream.getPosition(), docStream.getLength() - docStream.getPosition(), true);
    } else {
