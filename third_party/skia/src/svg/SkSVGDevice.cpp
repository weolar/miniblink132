...
static SkString svg_transform(const SkMatrix& t)
{
    ...
    default:
        ...
        //    | b d f |
        //    | 0 0 1 |
        tstr.printf("matrix(%g %g %g %g %g %g)", t.getScaleX(), t.getSkewY(), t.getSkewX(), t.getScaleY(), t.getTranslateX(), t.getTranslateY());
        break;
    }
...
public:
    ...
    {
        return SkStringPrintf("cfilter_%u", fColorFilterCount++);
    }
...
private:
    ...
    void addGradientShaderResources(const SkShader* shader, const SkPaint& paint, Resources* resources);
    ...
    SkString addLinearGradientDef(const SkShaderBase::GradientInfo& info, const SkShader* shader, const SkMatrix& localMatrix);
...
void SkSVGDevice::AutoElement::addGradientShaderResources(const SkShader* shader, const SkPaint& paint, Resources* resources)
{
    SkASSERT(shader);
    ...
    resources->fPaintServer = SkStringPrintf("url(#%s)", addLinearGradientDef(grInfo, shader, localMatrix).c_str());
}

void SkSVGDevice::AutoElement::addColorFilterResources(const SkColorFilter& cf, Resources* resources)
{
...
void SkSVGDevice::AutoElement::addImageShaderResources(const SkShader* shader, const SkPaint& paint, Resources* resources)
{
    SkMatrix outMatrix;
...
void SkSVGDevice::AutoElement::addShaderResources(const SkPaint& paint, Resources* resources)
{
    ...
    if (shaderType == SkShaderBase::ShaderType::kColor || shaderType == SkShaderBase::ShaderType::kGradientBase) {
...
SkString SkSVGDevice::AutoElement::addLinearGradientDef(const SkShaderBase::GradientInfo& info, const SkShader* shader, const SkMatrix& localMatrix)
{
...
void SkSVGDevice::AutoElement::addPathAttributes(const SkPath& path, SkParsePath::PathEncoding encoding)
{
...
void SkSVGDevice::AutoElement::addTextAttributes(const SkFont& font)
{
    ...
    if (weightIndex != 3) {
        static constexpr const char* weights[] = { "100", "200", "300", "normal", "400", "500", "600", "bold", "800", "900" };
        this->addAttribute("font-weight", weights[weightIndex]);
    }
    int stretchIndex = style.width() - 1;
    if (stretchIndex != 4) {
        static constexpr const char* stretches[] = { "ultra-condensed", "extra-condensed", "condensed", "semi-condensed", "normal", "semi-expanded", "expanded",
...
sk_sp<SkDevice> SkSVGDevice::Make(const SkISize& size, std::unique_ptr<SkXMLWriter> writer, uint32_t flags)
{
    return writer ? sk_sp<SkDevice>(new SkSVGDevice(size, std::move(writer), flags)) : nullptr;
}

SkSVGDevice::SkSVGDevice(const SkISize& size, std::unique_ptr<SkXMLWriter> writer, uint32_t flags)
    : SkClipStackDevice(SkImageInfo::MakeUnknown(size.fWidth, size.fHeight), SkSurfaceProps())
...
{
    return (fFlags & SkSVGCanvas::kRelativePathEncoding_Flag) ? SkParsePath::PathEncoding::Relative : SkParsePath::PathEncoding::Absolute;
}
...
void SkSVGDevice::drawPaint(const SkPaint& paint)
{
    ...
    rect.addRectAttributes(SkRect::MakeWH(SkIntToScalar(this->width()), SkIntToScalar(this->height())));
}
...
void SkSVGDevice::drawAnnotation(const SkRect& rect, const char key[], SkData* value)
{
    ...
    if (!strcmp(SkAnnotationKeys::URL_Key(), key) || !strcmp(SkAnnotationKeys::Link_Named_Dest_Key(), key)) {
        this->cs().save();
...
void SkSVGDevice::drawPoints(SkCanvas::PointMode mode, size_t count, const SkPoint pts[], const SkPaint& paint)
{
    SkPathBuilder path;
    ...
    const SkImage* image, const SkRect* src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    SkMatrix adjustedMatrix = this->localToDevice() * SkMatrix::RectToRect(src ? *src : SkRect::Make(bm.bounds()), dst);
...
public:
        ...
        : fOrigin(origin)
    {
        ...
        SkFontPriv::GlyphsToUnichars(glyphRun.font(), glyphRun.glyphsIDs().data(), runSize, unichars.get());
...
void SkSVGDevice::onDrawGlyphRunList(SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
    SkASSERT(!glyphRunList.hasRSXForm());
    const auto draw_as_path = (fFlags & SkSVGCanvas::kConvertTextToPaths_Flag) || paint.getPathEffect();
