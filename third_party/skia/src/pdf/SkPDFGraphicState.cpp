...
SkPDFIndirectReference SkPDFGraphicState::GetGraphicStateForPaint(SkPDFDocument* doc, const SkPaint& p)
{
    SkASSERT(doc);
    ...
    if (SkPaint::kFill_Style == p.getStyle()) {
        ...
        SkPDFStrokeGraphicState strokeKey
            = { p.getStrokeWidth(), p.getStrokeMiter(), p.getColor4f().fA, SkToU8(p.getStrokeCap()), SkToU8(p.getStrokeJoin()), pdf_blend_mode(mode) };
