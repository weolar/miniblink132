...
template <typename T>
void emit_image_stream(SkPDFDocument* doc, SkPDFIndirectReference ref, T writeStream, SkISize size, SkPDFUnion&& colorSpace, SkPDFIndirectReference sMask,
    int length, SkPDFStreamFormat format)
{
    ...
    switch (format) {
    case SkPDFStreamFormat::DCT:
        filters->appendName("DCTDecode");
        break;
    case SkPDFStreamFormat::Flate:
        filters->appendName("FlateDecode");
        break;
    case SkPDFStreamFormat::Uncompressed:
        break;
    }
...
void do_deflated_alpha(const SkPixmap& pm, SkPDFDocument* doc, SkPDFIndirectReference ref)
{
    ...
    SkPDFStreamFormat format = compressionLevel == SkPDF::Metadata::CompressionLevel::None ? SkPDFStreamFormat::Uncompressed : SkPDFStreamFormat::Flate;
    ...
    emit_image_stream(
        doc, ref, [&buffer](SkWStream* stream) { buffer.writeToAndReset(stream); }, pm.info().dimensions(), SkPDFUnion::Name("DeviceGray"),
...
void do_deflated_image(const SkPixmap& pm, SkPDFDocument* doc, bool isOpaque, SkPDFIndirectReference ref)
{
    ...
    int length = SkToInt(buffer.bytesWritten());
    emit_image_stream(
        doc, ref, [&buffer](SkWStream* stream) { buffer.writeToAndReset(stream); }, pm.info().dimensions(), std::move(colorSpace), sMask, length, format);
    if (!isOpaque) {
        do_deflated_alpha(pm, doc, sMask);
    }
}

bool do_jpeg(sk_sp<SkData> data, SkColorSpace* imageColorSpace, SkPDFDocument* doc, SkISize size, SkPDFIndirectReference ref)
{
    ...
    if (sk_sp<SkData> encodedIccProfileData = encodedInfo.profileData(); encodedIccProfileData && !icc_channel_mismatch(encodedInfo.profile(), channels)) {
        colorSpace = write_icc_profile(doc, std::move(encodedIccProfileData), channels);
    } else if (const skcms_ICCProfile* codecIccProfile = codec->getICCProfile(); codecIccProfile && !icc_channel_mismatch(codecIccProfile, channels)) {
    ...
    emit_image_stream(
        doc, ref, [&data](SkWStream* dst) { dst->write(data->data(), data->size()); }, jpegSize, std::move(colorSpace), SkPDFIndirectReference(),
        SkToInt(data->size()), SkPDFStreamFormat::DCT);
    return true;
}
...
SkBitmap to_pixels(const SkImage* image)
{
    ...
    default: {
        ...
        bm.allocPixels(SkImageInfo::Make(w, h, kBGRA_8888_SkColorType, at, image->refColorSpace()));
    }
    }
...
void serialize_image(const SkImage* img, int encodingQuality, SkPDFDocument* doc, SkPDFIndirectReference ref)
{
    SkASSERT(img);
    SkASSERT(doc);
