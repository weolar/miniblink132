...
struct SkPDFStructElem {
    ...
    SkPDFIndirectReference emitStructElem(SkPDFIndirectReference parent, std::vector<SkPDFStructTree::IDTreeEntry>* idTree, SkPDFDocument* doc);
};
...
void SkPDF::AttributeList::appendFloatArray(const char* owner, const char* name, const std::vector<float>& value)
{
    if (!fAttrs) {
        fAttrs = SkPDFMakeArray();
    }
...
void SkPDF::AttributeList::appendNodeIdArray(const char* owner, const char* name, const std::vector<int>& elemIds)
{
    if (!fAttrs) {
        fAttrs = SkPDFMakeArray();
    }
...
SkPDFStructTree::SkPDFStructTree(SkPDF::StructureElementNode* node, SkPDF::Metadata::Outline outline)
...
void SkPDFStructTree::move(SkPDF::StructureElementNode& node, SkPDFStructElem* structElem, bool wantTitle)
{
    ...
    wantTitle |= fOutline == SkPDF::Metadata::Outline::StructureElementHeaders && type.size() == 2 && type[0] == 'H' && '1' <= type[1] && type[1] <= '6';
...
auto SkPDFStructTree::createMarkForElemId(int elemId, unsigned pageIndex) -> Mark
{
    ...
    SkASSERT(structElem->fMarkedContent.empty() || structElem->fMarkedContent.back().fLocation.fPageIndex <= pageIndex);
...
int SkPDFStructTree::createStructParentKeyForElemId(int elemId, SkPDFIndirectReference contentItemRef, unsigned pageIndex)
{
    if (!fRoot) {
        return -1;
    }
...
SkPDFIndirectReference SkPDFStructElem::emitStructElem(SkPDFIndirectReference parent, std::vector<SkPDFStructTree::IDTreeEntry>* idTree, SkPDFDocument* doc)
{
    fRef = doc->reserveRef();
...
SkPDFIndirectReference SkPDFStructTree::emitStructTreeRoot(SkPDFDocument* doc) const
{
    ...
    if (!idTree.empty()) {
        std::sort(idTree.begin(), idTree.end(), [](const IDTreeEntry& a, const IDTreeEntry& b) { return a.elemId < b.elemId; });
...
void create_outline_from_headers(SkPDFDocument* const doc, SkPDFStructElem* const structElem, STArray<7, OutlineEntry*>& stack)
{
...
{
    if (!fRoot || !fRoot->fUsed || fOutline != SkPDF::Metadata::Outline::StructureElementHeaders) {
        return SkPDFIndirectReference();
    }
