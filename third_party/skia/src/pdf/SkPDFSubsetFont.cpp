...
HBBlob stream_to_blob(std::unique_ptr<SkStreamAsset> asset)
{
    ...
    if (const void* base = asset->getMemoryBase()) {
        blob.reset(hb_blob_create(const_cast<char*>(static_cast<const char*>(base)), SkToUInt(size), HB_MEMORY_MODE_READONLY, asset.release(),
...
sk_sp<SkData> to_data(HBBlob blob)
{
    ...
    return SkData::MakeWithProc(
        data, SkToSizeT(length), [](const void*, void* ctx) { hb_blob_destroy((hb_blob_t*)ctx); }, blob.release());
}
