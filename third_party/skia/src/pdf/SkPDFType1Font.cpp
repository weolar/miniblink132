...
static bool parsePFB(const uint8_t* src, size_t size, size_t* headerLen, size_t* dataLen, size_t* trailerLen)
{
    ...
    return parsePFBSection(&srcPtr, &remaining, 1, headerLen) && parsePFBSection(&srcPtr, &remaining, 2, dataLen)
        && parsePFBSection(&srcPtr, &remaining, 1, trailerLen) && parsePFBSection(&srcPtr, &remaining, 3, nullptr);
}
...
static SkPDFIndirectReference make_type1_font_descriptor(SkPDFDocument* doc, const SkPDFStrikeSpec& pdfStrikeSpec, const SkAdvancedTypefaceMetrics* info)
{
    ...
    if (info) {
        ...
        if (can_embed(*info)) {
            ...
            sk_sp<SkData> fontData = convert_type1_font_stream(std::move(rawFontData), &header, &data, &trailer);
            if (fontData) {
                ...
                descriptor.insertRef("FontFile", SkPDFStreamOut(std::move(dict), std::move(fontStream), doc, SkPDFSteamCompressionEnabled::Yes));
            }
        }
    }
...
static SkPDFIndirectReference type1_font_descriptor(SkPDFDocument* doc, const SkPDFStrikeSpec& pdfStrikeSpec)
{
    const SkTypeface& typeface = pdfStrikeSpec.fStrikeSpec.typeface();
    SkTypefaceID typefaceID = typeface.uniqueID();
