...
static bool scale_paint(SkPaint& paint, SkScalar fontToEMScale)
{
    ...
    if (SkPathEffectBase* peb = as_PEB(paint.getPathEffect())) {
        ...
        if (peb->asADash(&dashInfo) == SkPathEffectBase::DashType::kDash) {
            ...
            paint.setPathEffect(SkDashPathEffect::Make(dashInfo.fIntervals, dashInfo.fCount, dashInfo.fPhase));
        } else {
            return false;
        }
    }
...
sk_sp<SkPDFStrike> SkPDFStrike::Make(SkPDFDocument* doc, const SkFont& font, const SkPaint& paint)
{
    ...
    if (kBitmapFontSize <= 0) {
        ...
        sk_sp<SkPDFStrike> strike(
            new SkPDFStrike(SkPDFStrikeSpec(pathStrikeSpec, pathStrikeEM), SkPDFStrikeSpec(pathStrikeSpec, pathStrikeEM), pathPaint.getMaskFilter(), doc));
    ...
    sk_sp<SkPDFStrike> strike(
        new SkPDFStrike(SkPDFStrikeSpec(pathStrikeSpec, pathStrikeEM), SkPDFStrikeSpec(imageStrikeSpec, imageStrikeEM), pathPaint.getMaskFilter(), doc));
    doc->fStrikes.set(strike);
    return strike;
}

SkPDFStrike::SkPDFStrike(SkPDFStrikeSpec path, SkPDFStrikeSpec image, bool hasMaskFilter, SkPDFDocument* doc)
    : fPath(std::move(path))
...
const SkAdvancedTypefaceMetrics* SkPDFFont::GetMetrics(const SkTypeface& typeface, SkPDFDocument* canon)
{
...
const std::vector<SkUnichar>& SkPDFFont::GetUnicodeMap(const SkTypeface& typeface, SkPDFDocument* canon)
{
    SkASSERT(canon);
    SkTypefaceID id = typeface.uniqueID();
...
THashMap<SkGlyphID, SkString>& SkPDFFont::GetUnicodeMapEx(const SkTypeface& typeface, SkPDFDocument* canon)
{
    SkASSERT(canon);
    SkTypefaceID id = typeface.uniqueID();
...
SkAdvancedTypefaceMetrics::FontType SkPDFFont::FontType(const SkPDFStrike& pdfStrike, const SkAdvancedTypefaceMetrics& metrics)
{
    if (SkToBool(metrics.fFlags & SkAdvancedTypefaceMetrics::kVariable_FontFlag) ||
        ...
        metrics.fType == SkAdvancedTypefaceMetrics::kCFF_Font || pdfStrike.fHasMaskFilter) {
...
SkPDFFont* SkPDFStrike::getFontResource(const SkGlyph* glyph)
{
    ...
    SkGlyphID subsetCode = multibyte ? 0 : first_nonzero_glyph_for_single_byte_encoding(glyph->getGlyphID());
...
SkPDFFont::SkPDFFont(const SkPDFStrike* strike, SkGlyphID firstGlyphID, SkGlyphID lastGlyphID, SkAdvancedTypefaceMetrics::FontType fontType,
...
void SkPDFFont::PopulateCommonFontDescriptor(SkPDFDict* descriptor, const SkAdvancedTypefaceMetrics& metrics, uint16_t emSize, int16_t defaultWidth)
{
    ...
    descriptor->insertObject("FontBBox",
        SkPDFMakeArray(scaleFromFontUnits(metrics.fBBox.left(), emSize), scaleFromFontUnits(metrics.fBBox.bottom(), emSize),
            scaleFromFontUnits(metrics.fBBox.right(), emSize), scaleFromFontUnits(metrics.fBBox.top(), emSize)));
    if (defaultWidth > 0) {
...
static void emit_subset_type0(const SkPDFFont& font, SkPDFDocument* doc)
{
    ...
    if (0 == fontSize) {
        ...
        descriptor->insertRef("FontFile2", SkPDFStreamOut(std::move(streamDict), std::move(subsetFontAsset), doc, SkPDFSteamCompressionEnabled::Yes));
    } else if (type == SkAdvancedTypefaceMetrics::kType1CID_Font) {
        std::unique_ptr<SkPDFDict> streamDict = SkPDFMakeDict();
        streamDict->insertName("Subtype", "CIDFontType0C");
        descriptor->insertRef("FontFile3", SkPDFStreamOut(std::move(streamDict), std::move(fontAsset), doc, SkPDFSteamCompressionEnabled::Yes));
    } else {
        SkASSERT(false);
    }
    ...
    {
        std::unique_ptr<SkPDFArray> widths = SkPDFMakeCIDGlyphWidthsArray(font.strike().fPath, font.glyphUsage(), &defaultWidth);
    ...
    std::unique_ptr<SkStreamAsset> toUnicode = SkPDFMakeToUnicodeCmap(
        glyphToUnicode.data(), SkPDFFont::GetUnicodeMapEx(typeface, doc), &font.glyphUsage(), font.multiByteGlyphs(), font.firstGlyphID(), font.lastGlyphID());
...
struct SingleByteGlyphIdIterator {
    ...
    struct Iter {
        ...
        SkGlyphID operator*() const
        {
            return (SkGlyphID)fCurrent;
        }
        bool operator!=(const Iter& rhs) const
        {
            return fCurrent != rhs.fCurrent;
        }
        Iter(SkGlyphID f, int c)
            : fFirst(f)
            , fCurrent(c)
        {
        }
    ...
    Iter end() const
    {
        return Iter(fFirst, (int)fLast + 1);
    }
...
static ImageAndOffset to_image(SkGlyphID gid, SkBulkGlyphMetricsAndImages* smallGlyphs)
{
    ...
    case SkMask::k3D_Format: // just do the A8 part
        // Make a gray image, used to smask a rectangle.
        return { SkImages::RasterFromData(SkImageInfo::Make(bounds.size(), kGray_8_SkColorType, kUnknown_SkAlphaType),
    ...
    case SkMask::kARGB32_Format:
        ...
        return { SkImages::RasterFromData(
                     SkImageInfo::MakeN32Premul(bounds.size()), SkData::MakeWithCopy(mask.fImage, mask.computeTotalImageSize()), mask.fRowBytes),
            { bounds.x(), bounds.y() } };
...
static void emit_subset_type3(const SkPDFFont& pdfFont, SkPDFDocument* doc)
{
    ...
    for (SkGlyphID gID : SingleByteGlyphIdIterator(firstGlyphID, lastGlyphID)) {
        ...
        if (drawable && !drawable->getBounds().isEmpty()) {
            ...
            SkPDFIndirectReference xobject = SkPDFMakeFormXObject(doc, glyphDevice->content(), SkPDFMakeArray(0, 0, glyphBBox.width(), glyphBBox.height()),
                glyphDevice->makeResourceDict(), SkMatrix::Translate(glyphBBox.fLeft, glyphBBox.fTop), nullptr);
            ...
            SkPaint::Style style = pathGlyph->pathIsHairline() ? SkPaint::kStroke_Style : SkPaint::kFill_Style;
            ...
            if (pimg.fImage->colorType() != kGray_8_SkColorType) {
                ...
                SkPDFIndirectReference sMask = SkPDFMakeFormXObject(doc, glyphDevice->content(),
                    SkPDFMakeArray(0, 0, pimg.fImage->width(), pimg.fImage->height()), glyphDevice->makeResourceDict(), SkMatrix(), "DeviceGray");

                // Use Form XObject as SMask (luminosity) on the graphics state
                SkPDFIndirectReference smaskGraphicState = SkPDFGraphicState::GetSMaskGraphicState(sMask, false, SkPDFGraphicState::kLuminosity_SMaskMode, doc);
                ...
                graphicStates->insertRef(SkStringPrintf("G%d", smaskGraphicState.fValue), smaskGraphicState);
            }
        } else {
            setGlyphWidthAndBoundingBox(pathGlyph->advanceX(), glyphBBox, &content);
        }
        charProcs->insertRef(std::move(characterName), SkPDFStreamOut(nullptr, content.detachAsStream(), doc));
    }
    ...
    /* FontBBox: "A rectangle expressed in the glyph coordinate
      ...
      then filled." */
    font.insertObject("FontBBox", SkPDFMakeArray(bbox.left(), bbox.bottom(), bbox.right(), bbox.top()));
    ...
    auto toUnicodeCmap
        = SkPDFMakeToUnicodeCmap(glyphToUnicode.data(), SkPDFFont::GetUnicodeMapEx(pathTypeface, doc), &subset, false, firstGlyphID, lastGlyphID);
