...
static void append_quad(const SkPoint quad[], SkWStream* content)
{
    ...
    append_cubic(cubic[1].fX, cubic[1].fY, cubic[2].fX, cubic[2].fY, cubic[3].fX, cubic[3].fY, content);
}
...
void SkPDFUtils::EmitPath(const SkPath& path, SkPaint::Style paintStyle, bool doConsumeDegerates, SkWStream* content, SkScalar tolerance)
{
    ...
    if (path.isRect(&rect, &isClosed, &direction) && isClosed && (SkPathDirection::kCW == direction || SkPathFillType::kEvenOdd == path.getFillType())) {
    ...
    for (SkPath::Verb verb = iter.next(args); verb != SkPath::kDone_Verb; verb = iter.next(args)) {
        ...
        case SkPath::kCubic_Verb:
            if (!doConsumeDegerates || !SkPathPriv::AllPointsEq(args, 4)) {
                append_cubic(args[1].fX, args[1].fY, args[2].fX, args[2].fY, args[3].fX, args[3].fY, &currentSegment);
                fillState = kNonSingleLine_SkipFillState;
            }
            break;
