...
static void append_bfchar_section_ex(
    const THashMap<SkGlyphID, SkString>& glyphToUnicodeEx, bool multiByte, SkGlyphID firstGlyphID, SkGlyphID lastGlyphID, SkDynamicMemoryWStream* cmap)
{
    size_t glyphCount = 0;
...
void SkPDFAppendCmapSections(const SkUnichar* glyphToUnicode, const THashMap<SkGlyphID, SkString>& glyphToUnicodeEx, const SkPDFGlyphUse* subset,
    SkDynamicMemoryWStream* cmap, bool multiByteGlyphs, SkGlyphID firstGlyphID, SkGlyphID lastGlyphID)
{
    int glyphOffset = 0;
    if (!multiByteGlyphs) {
        glyphOffset = firstGlyphID - 1;
    ...
    for (int i = firstGlyphID - glyphOffset; i < limit + 1; ++i) {
        ...
        if (!rangeEmpty) {
            ...
            bool inRange = i == currentRangeEntry.fEnd + 1 && i >> 8 == currentRangeEntry.fStart >> 8 && i < limit
...
std::unique_ptr<SkStreamAsset> SkPDFMakeToUnicodeCmap(const SkUnichar* glyphToUnicode, const THashMap<SkGlyphID, SkString>& glyphToUnicodeEx,
    const SkPDFGlyphUse* subset, bool multiByteGlyphs, SkGlyphID firstGlyphID, SkGlyphID lastGlyphID)
{
    SkDynamicMemoryWStream cmap;
    append_tounicode_header(&cmap, multiByteGlyphs);
    SkPDFAppendCmapSections(glyphToUnicode, glyphToUnicodeEx, subset, &cmap, multiByteGlyphs, firstGlyphID, lastGlyphID);
