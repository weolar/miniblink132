...
static void write_name_escaped(SkWStream* o, const char* name)
{
    ...
    for (const uint8_t* n = reinterpret_cast<const uint8_t*>(name); *n; ++n) {
        ...
        if (v < '!' || v > '~' || strchr(kToEscape, v)) {
            char buffer[3] = { '#', SkHexadecimalDigits::gUpper[v >> 4], SkHexadecimalDigits::gUpper[v & 0xF] };
...
static void write_literal_byte_string(SkWStream* wStream, const char* cin, size_t len)
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (c < ' ' || '~' < c) {
            uint8_t octal[4] = { '\\', (uint8_t)('0' | (c >> 6)), (uint8_t)('0' | ((c >> 3) & 0x07)), (uint8_t)('0' | (c & 0x07)) };
...
static void serialize_stream(SkPDFDict* origDict, SkStreamAsset* stream, SkPDFSteamCompressionEnabled compress, SkPDFDocument* doc, SkPDFIndirectReference ref)
{
    ...
    if (doc->metadata().fCompressionLevel != SkPDF::Metadata::CompressionLevel::None && compress == SkPDFSteamCompressionEnabled::Yes
...
SkPDFIndirectReference SkPDFStreamOut(
    std::unique_ptr<SkPDFDict> dict, std::unique_ptr<SkStreamAsset> content, SkPDFDocument* doc, SkPDFSteamCompressionEnabled compress)
{
