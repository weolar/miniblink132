...
SkPDFDevice::MarkedContentManager::MarkedContentManager(SkPDFDocument* document, SkDynamicMemoryWStream* out)
    : fDoc(document)
    , fOut(out)
...
int SkPDFDevice::MarkedContentManager::elemId() const
{
    return fNextMarksElemId;
}
...
bool SkPDFDevice::MarkedContentManager::hasActiveMark() const
{
    return bool(fCurrentlyActiveMark);
}
...
{
    sk_sp<SkImage> img;
    SkPixmap pm(SkImageInfo::Make(mask->fBounds.width(), mask->fBounds.height(), kGray_8_SkColorType, kOpaque_SkAlphaType), mask->fImage, mask->fRowBytes);
...
sk_sp<SkImage> alpha_image_to_greyscale_image(const SkImage* mask)
{
    ...
    if (!mask->readPixels(nullptr, SkImageInfo::MakeA8(w, h), greyBitmap.getPixels(), greyBitmap.rowBytes(), 0, 0)) {
        return nullptr;
    }
...
static SkTCopyOnFirstWrite<SkPaint> clean_paint(const SkPaint& srcPaint)
{
    ...
    if (!paint->isSrcOver() && SkBlendFastPath::kSrcOver == CheckFastPath(*paint, false)) {
...
sk_sp<SkDevice> SkPDFDevice::createDevice(const CreateInfo& cinfo, const SkPaint* layerPaint)
{
    ...
    if ((layerPaint && (layerPaint->getImageFilter() || layerPaint->getColorFilter())) || (cinfo.fInfo.colorSpace() && !cinfo.fInfo.colorSpace()->isSRGB())) {
        // need to return a raster device, which we will detect in drawDevice()
        return SkBitmapDevice::Create(cinfo.fInfo, SkSurfaceProps());
    }
...
public:
        ...
        , fClipStack(clipStack)
    {
        ...
        fContentStream = fDevice->setUpContentEntry(clipStack, matrix, paint, textScale, &fDstFormXObject);
    }
...
SkPDFDevice::SkPDFDevice(SkISize pageSize, SkPDFDocument* doc, const SkMatrix& transform)
    : SkClipStackDevice(SkImageInfo::MakeUnknown(pageSize.width(), pageSize.height()), SkSurfaceProps())
...
void SkPDFDevice::drawAnnotation(const SkRect& rect, const char key[], SkData* value)
{
    ...
    if (linkType != SkPDFLink::Type::kNone) {
        std::unique_ptr<SkPDFLink> link = std::make_unique<SkPDFLink>(linkType, value, transformedRect, fMarkManager.elemId());
...
void SkPDFDevice::internalDrawPathWithFilter(const SkClipStack& clipStack, const SkMatrix& ctm, const SkPath& origPath, const SkPaint& origPaint)
{
    SkASSERT(origPaint.getMaskFilter());
    SkPath path(origPath);
    SkTCopyOnFirstWrite<SkPaint> paint(origPaint);

    SkStrokeRec::InitStyle initStyle = skpathutils::FillPathWithPaint(path, *paint, &path) ? SkStrokeRec::kFill_InitStyle : SkStrokeRec::kHairline_InitStyle;
    path.transform(ctm, &path);

    SkIRect bounds = clipStack.bounds(this->bounds()).roundOut();
    SkMaskBuilder sourceMask;
    if (!SkDraw::DrawToMask(
            path, bounds, paint->getMaskFilter(), &SkMatrix::I(), &sourceMask, SkMaskBuilder::kComputeBoundsAndRenderImage_CreateMode, initStyle)) {
        return;
    }
    ...
    this->setGraphicState(SkPDFGraphicState::GetSMaskGraphicState(
                              maskDevice->makeFormXObjectFromDevice(dstMaskBounds, true), false, SkPDFGraphicState::kLuminosity_SMaskMode, fDocument),
        content.stream());
...
void SkPDFDevice::internalDrawPath(const SkClipStack& clipStack, const SkMatrix& ctm, const SkPath& origPath, const SkPaint& srcPaint, bool pathIsMutable)
{
    ...
    bool consumeDegeratePathSegments
        = paint->getStyle() == SkPaint::kFill_Style || (paint->getStrokeCap() != SkPaint::kRound_Cap && paint->getStrokeCap() != SkPaint::kSquare_Cap);
    SkPDFUtils::EmitPath(*pathPtr, paint->getStyle(), consumeDegeratePathSegments, content.stream(), tolerance);
...
void SkPDFDevice::drawImageRect(
    const SkImage* image, const SkRect* src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint)
{
    SkASSERT(image);
    this->internalDrawImageRect(SkKeyedImage(sk_ref_sp(const_cast<SkImage*>(image))), src, dst, sampling, paint, this->localToDevice());
}
...
void SkPDFDevice::drawSprite(const SkBitmap& bm, int x, int y, const SkPaint& paint)
{
    ...
    this->internalDrawImageRect(SkKeyedImage(bm), nullptr, r, SkSamplingOptions(), paint, SkMatrix::I());
}
...
void SkPDFDevice::drawGlyphRunAsPath(const sktext::GlyphRun& glyphRun, SkPoint offset, const SkPaint& runPaint)
{
...
static bool needs_new_font(SkPDFFont* font, const SkGlyph* glyph, SkAdvancedTypefaceMetrics::FontType initialFontType)
{
...
void SkPDFDevice::internalDrawGlyphRun(const sktext::GlyphRun& glyphRun, SkPoint offset, const SkPaint& runPaint)
{
    ...
    while (SkClusterator::Cluster c = clusterator.next()) {
        ...
        for (; glyphIndex < glyphLimit; ++glyphIndex) {
            ...
            SkRect glyphBounds = get_glyph_bounds_device_space(glyphs[glyphIndex], textScaleX, textScaleY, xy + offset, this->localToDevice());
            ...
            if (needs_new_font(font, glyphs[glyphIndex], initialFontType)) {
                ...
                SkPDFWriteResourceName(out, SkPDFResourceType::kFont, add_resource(fFontResources, font->indirectReference()));
                out->writeText(" ");
...
void SkPDFDevice::onDrawGlyphRunList(SkCanvas*, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
...
void SkPDFDevice::drawFormXObject(SkPDFIndirectReference xObject, SkDynamicMemoryWStream* content, SkPath* shape)
{
    fMarkManager.beginMark();
    ...
    SkASSERT(xObject);
    SkPDFWriteResourceName(content, SkPDFResourceType::kXObject, add_resource(fXObjectResources, xObject));
...
static std::vector<SkPDFIndirectReference> sort(const THashSet<SkPDFIndirectReference>& src)
{
    ...
    std::sort(dst.begin(), dst.end(), [](SkPDFIndirectReference a, SkPDFIndirectReference b) { return a.fValue < b.fValue; });
    return dst;
}

std::unique_ptr<SkPDFDict> SkPDFDevice::makeResourceDict()
{
    return SkPDFMakeResourceDict(sort(fGraphicStateResources), sort(fShaderResources), sort(fXObjectResources), sort(fFontResources));
}
 ...
 * in the first place.
 */
bool SkPDFDevice::handleInversePath(const SkPath& origPath, const SkPaint& paint, bool pathIsMutable)
{
...
SkPDFIndirectReference SkPDFDevice::makeFormXObjectFromDevice(SkIRect bounds, bool alpha)
{
    ...
    SkPDFIndirectReference xobject = SkPDFMakeFormXObject(fDocument, this->content(),
        SkPDFMakeArray(bounds.left(), bounds.top(), bounds.right(), bounds.bottom()), this->makeResourceDict(), inverseTransform, colorSpace);
...
void SkPDFDevice::drawFormXObjectWithMask(SkPDFIndirectReference xObject, SkPDFIndirectReference sMask, SkBlendMode mode, bool invertClip)
{
    SkASSERT(sMask);
    SkPaint paint;
    ...
    this->setGraphicState(SkPDFGraphicState::GetSMaskGraphicState(sMask, invertClip, SkPDFGraphicState::kAlpha_SMaskMode, fDocument), content.stream());
...
static void populate_graphic_state_entry_from_paint(SkPDFDocument* doc, const SkMatrix& matrix, const SkClipStack* clipStack, SkIRect deviceBounds,
    const SkPaint& paint, const SkMatrix& initialTransform, SkScalar textScale, SkPDFGraphicStackState::Entry* entry,
    THashSet<SkPDFIndirectReference>* shaderResources, THashSet<SkPDFIndirectReference>* graphicStateResources)
{
    ...
    if (shader) {
        ...
        if (as_SB(shader)->type() == SkShaderBase::ShaderType::kColor) {
            ...
            SkRect clipStackBounds = clipStack ? clipStack->bounds(deviceBounds) : SkRect::Make(deviceBounds);
            ...
            SkPDFIndirectReference pdfShader = SkPDFMakeShader(doc, shader, transform, bounds, { c.fR, c.fG, c.fB, 1.0f });
...
SkDynamicMemoryWStream* SkPDFDevice::setUpContentEntry(
    const SkClipStack* clipStack, const SkMatrix& matrix, const SkPaint& paint, SkScalar textScale, SkPDFIndirectReference* dst)
{
    SkASSERT(!*dst);
    ...
    populate_graphic_state_entry_from_paint(
        fDocument, matrix, clipStack, this->bounds(), paint, fInitialTransform, textScale, &entry, &fShaderResources, &fGraphicStateResources);
...
void SkPDFDevice::finishContentEntry(const SkClipStack* clipStack, SkBlendMode blendMode, SkPDFIndirectReference dst, SkPath* shape)
{
    ...
    if (this->isContentEmpty()) {
        ...
        // reduces to Dst.
        if (shape == nullptr || blendMode == SkBlendMode::kDstOut || blendMode == SkBlendMode::kSrcATop) {
    ...
    if (blendMode == SkBlendMode::kSrcATop) {
        ...
        if (shape != nullptr) {
            ...
            shapeDev.internalDrawPath(clipStack ? *clipStack : empty, SkMatrix::I(), *shape, filledPaint, true);
            this->drawFormXObjectWithMask(dst, shapeDev.makeFormXObjectFromDevice(), SkBlendMode::kSrcOver, true);
        } else {
    ...
    SkASSERT(blendMode == SkBlendMode::kSrcIn || blendMode == SkBlendMode::kDstIn || blendMode == SkBlendMode::kSrcOut || blendMode == SkBlendMode::kDstOut
        || blendMode == SkBlendMode::kSrcATop || blendMode == SkBlendMode::kDstATop || blendMode == SkBlendMode::kModulate);

    if (blendMode == SkBlendMode::kSrcIn || blendMode == SkBlendMode::kSrcOut || blendMode == SkBlendMode::kSrcATop) {
        this->drawFormXObjectWithMask(srcFormXObject, dst, SkBlendMode::kSrcOver, blendMode == SkBlendMode::kSrcOut);
        return;
    } else {
...
{
    return is_integer(r.left()) && is_integer(r.top()) && is_integer(r.right()) && is_integer(r.bottom());
}

void SkPDFDevice::internalDrawImageRect(
    SkKeyedImage imageSubset, const SkRect* src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint& srcPaint, const SkMatrix& ctm)
{
    if (this->hasEmptyClip()) {
        return;
    }
    ...
    SkTCopyOnFirstWrite<SkPaint> paint(srcPaint);
    if (!paint->isSrcOver() && imageSubset.image()->isOpaque() && SkBlendFastPath::kSrcOver == CheckFastPath(*paint, false)) {
        paint.writable()->setBlendMode(SkBlendMode::kSrcOver);
    }
    ...
    if (imageSubset.image()->isAlphaOnly() && paint->getColorFilter()) {
        ...
        auto surface = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(imageSubset.image()->dimensions()));
        SkCanvas* canvas = surface->getCanvas();
        SkPaint tmpPaint;
    ...
    if (imageSubset.image()->isAlphaOnly()) {
        ...
        this->setGraphicState(SkPDFGraphicState::GetSMaskGraphicState(
                                  maskDevice->makeFormXObjectFromDevice(maskDeviceBounds, true), false, SkPDFGraphicState::kLuminosity_SMaskMode, fDocument),
            content.stream());
    ...
    if (!pdfimagePtr) {
        ...
        pdfimage = SkPDFSerializeImage(imageSubset.image().get(), fDocument, fDocument->metadata().fEncodingQuality);
...
void SkPDFDevice::drawDevice(SkDevice* device, const SkSamplingOptions& sampling, const SkPaint& paint)
{
...
void SkPDFDevice::drawSpecial(
    SkSpecialImage* srcImg, const SkMatrix& localToDevice, const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint)
{
    if (this->hasEmptyClip()) {
        return;
    }
...
{
    return SkSpecialImages::MakeFromRaster(image->bounds(), image->makeNonTextureImage(), this->surfaceProps());
}
