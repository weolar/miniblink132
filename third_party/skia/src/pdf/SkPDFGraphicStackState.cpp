...
template <typename F> static void apply_clip(const SkClipStack& stack, const SkRect& outerBounds, F fn)
{
    ...
    while (const SkClipStack::Element* element = iter.next()) {
        ...
        if (op == kDifference_SkPathOp || operand.isInverseFillType() || !kHuge.contains(operand.getBounds())) {
...
static void append_clip(const SkClipStack& clipStack, const SkIRect& bounds, SkWStream* wStream)
{
    ...
    if (is_complex_clip(clipStack)) {
        ...
        apply_clip(clipStack, outsetBounds, [wStream](const SkPath& path) { append_clip_path(path, wStream); });
    }
}
