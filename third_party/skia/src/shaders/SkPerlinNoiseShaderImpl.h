...
public:
    ...
    struct PaintingData {
        PaintingData(const SkISize& tileSize, SkScalar seed, SkScalar baseFrequencyX, SkScalar baseFrequencyY)
        {
            ...
            fTileSize.set(SkScalarRoundToInt(tileSize.fWidth), SkScalarRoundToInt(tileSize.fHeight));
            this->init(seed);
    ...
    private:
        ...
        void init(SkScalar seed)
        {
            ...
            for (int channel = 0; channel < 4; ++channel) {
                for (int i = 0; i < kBlockSize; ++i) {
                    SkPoint gradient
                        = SkPoint::Make((fNoise[channel][i][0] - kBlockSize) * kInvBlockSizef, (fNoise[channel][i][1] - kBlockSize) * kInvBlockSizef);
                    gradient.normalize();
    ...
    {
        return ShaderType::kPerlinNoise;
    }
