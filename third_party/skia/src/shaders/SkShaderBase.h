...
public:
    ...
    {
        return SkMatrix::Concat(fCTM, fTotalLocalMatrix);
    }
...
private:
    MatrixRec(const SkMatrix& ctm, const SkMatrix& totalLocalMatrix, const SkMatrix& pendingLocalMatrix, bool totalIsValid, bool ctmApplied)
        : fCTM(ctm)
...
#undef M
    ...
    struct ContextRec {
        ContextRec(
            SkAlpha paintAlpha, const SkShaders::MatrixRec& matrixRec, SkColorType dstColorType, SkColorSpace* dstColorSpace, const SkSurfaceProps& props)
        ...
        {
            return { parentRec.fPaintAlpha, parentRec.fMatrixRec.concat(localM), parentRec.fDstColorType, parentRec.fDstColorSpace, parentRec.fProps };
        }
