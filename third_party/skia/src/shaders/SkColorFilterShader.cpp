...
SkColorFilterShader::SkColorFilterShader(sk_sp<SkShader> shader, float alpha, sk_sp<SkColorFilter> filter)
...
sk_sp<SkShader> SkColorFilterShader::Make(sk_sp<SkShader> shader, float alpha, sk_sp<SkColorFilter> filter)
{
    if (!shader) {
        return nullptr;
    } else if (!filter) {
...
bool SkColorFilterShader::appendStages(const SkStageRec& rec, const SkShaders::MatrixRec& mRec) const
{
