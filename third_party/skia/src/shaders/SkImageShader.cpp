...
#endif
    return SkM44((1.f / 6) * B, -(3.f / 6) * B - C, (3.f / 6) * B + 2 * C, -(1.f / 6) * B - C, 1 - (2.f / 6) * B, 0, -3 + (12.f / 6) * B + C,
        2 - (9.f / 6) * B - C, (1.f / 6) * B, (3.f / 6) * B + C, 3 - (15.f / 6) * B - 2 * C, -2 + (9.f / 6) * B + C, 0, 0, -C, (1.f / 6) * B + C);
}
...
SkImageShader::SkImageShader(
    sk_sp<SkImage> img, const SkRect& subset, SkTileMode tmx, SkTileMode tmy, const SkSamplingOptions& sampling, bool raw, bool clampAsIfUnpremul)
    : fImage(std::move(img))
    , fSampling(sampling)
...
sk_sp<SkFlattenable> SkImageShader::CreateProc(SkReadBuffer& buffer)
{
    ...
    if (buffer.isVersionLT(SkPicturePriv::kNoFilterQualityShaders_Version) && !buffer.readBool() /* legacy has_sampling */) {
        readSampling = false;
    ...
    bool raw = buffer.isVersionLT(SkPicturePriv::Version::kRawImageShaders) ? false : buffer.readBool();
...
static bool legacy_shader_can_handle(const SkMatrix& inv)
{
    ...
    if (!SkRect::MakeLTRB(-max_fixed32dot32, -max_fixed32dot32, +max_fixed32dot32, +max_fixed32dot32).contains(src)) {
        return false;
    }
...
SkShaderBase::Context* SkImageShader::onMakeContext(const ContextRec& rec, SkArenaAlloc* alloc) const
{
    ...
    return SkBitmapProcLegacyShader::MakeContext(*this, fTileModeX, fTileModeY, sampling, as_IB(fImage.get()), rec, alloc);
}
#endif
...
sk_sp<SkShader> SkImageShader::Make(
    sk_sp<SkImage> image, SkTileMode tmx, SkTileMode tmy, const SkSamplingOptions& options, const SkMatrix* localMatrix, bool clampAsIfUnpremul)
{
...
sk_sp<SkShader> SkImageShader::MakeRaw(sk_sp<SkImage> image, SkTileMode tmx, SkTileMode tmy, const SkSamplingOptions& options, const SkMatrix* localMatrix)
{
    ...
    sk_sp<SkShader> s = sk_make_sp<SkImageShader>(image, subset, tmx, tmy, options,
        /*raw=*/true,
        /*clampAsIfUnpremul=*/false);
    return s->makeWithLocalMatrix(localMatrix ? *localMatrix : SkMatrix::I());
}

sk_sp<SkShader> SkImageShader::MakeSubset(sk_sp<SkImage> image, const SkRect& subset, SkTileMode tmx, SkTileMode tmy, const SkSamplingOptions& options,
    const SkMatrix* localMatrix, bool clampAsIfUnpremul)
{
    ...
    sk_sp<SkShader> s = sk_make_sp<SkImageShader>(std::move(image), subset, tmx, tmy, options,
        /*raw=*/false, clampAsIfUnpremul);
    return s->makeWithLocalMatrix(localMatrix ? *localMatrix : SkMatrix::I());
}

///////////////////////////////////////////////////////////////////////////////////////////////////

sk_sp<SkShader> SkMakeBitmapShaderForPaint(const SkPaint& paint, const SkBitmap& src, SkTileMode tmx, SkTileMode tmy, const SkSamplingOptions& sampling,
    const SkMatrix* localMatrix, SkCopyPixelsMode mode)
{
    auto s = SkImageShader::Make(SkMakeImageFromRasterBitmap(src, mode), tmx, tmy, sampling, localMatrix);
    if (!s) {
        return nullptr;
    }
...
SkRect SkModifyPaintAndDstForDrawImageRect(
    const SkImage* image, const SkSamplingOptions& sampling, SkRect src, SkRect dst, bool strictSrcSubset, SkPaint* paint)
{
    ...
    if (strictSrcSubset) {
        imgShader = SkImageShader::MakeSubset(sk_ref_sp(image), src, SkTileMode::kClamp, SkTileMode::kClamp, sampling, &localMatrix);
    } else {
        imgShader = image->makeShader(SkTileMode::kClamp, SkTileMode::kClamp, sampling, &localMatrix);
    }
    if (!imgShader) {
...
struct MipLevelHelper {
    ...
    void allocAndInit(SkArenaAlloc* alloc, const SkSamplingOptions& sampling, SkTileMode tileModeX, SkTileMode tileModeY)
    {
        ...
        if (sampling.useCubic) {
            SkImageShader::CubicResamplerMatrix(sampling.cubic.B, sampling.cubic.C).getColMajor(gather->weights);
        }
...
static SkSamplingOptions tweak_sampling(SkSamplingOptions sampling, const SkMatrix& matrix)
{
    ...
    if (filter == SkFilterMode::kLinear && matrix.getType() <= SkMatrix::kTranslate_Mask && matrix.getTranslateX() == (int)matrix.getTranslateX()
...
bool SkImageShader::appendStages(const SkStageRec& rec, const SkShaders::MatrixRec& mRec) const
{
    ...
    auto append_tiling_and_gather = [&](const MipLevelHelper* level) {
        ...
        case kAlpha_8_SkColorType:
            p->append(SkRasterPipelineOp::gather_a8, ctx);
            break;
        case kA16_unorm_SkColorType:
            p->append(SkRasterPipelineOp::gather_a16, ctx);
            break;
        case kA16_float_SkColorType:
            p->append(SkRasterPipelineOp::gather_af16, ctx);
            break;
        case kRGB_565_SkColorType:
            p->append(SkRasterPipelineOp::gather_565, ctx);
            break;
        case kARGB_4444_SkColorType:
            p->append(SkRasterPipelineOp::gather_4444, ctx);
            break;
        case kR8G8_unorm_SkColorType:
            p->append(SkRasterPipelineOp::gather_rg88, ctx);
            break;
        case kR16G16_unorm_SkColorType:
            p->append(SkRasterPipelineOp::gather_rg1616, ctx);
            break;
        case kR16G16_float_SkColorType:
            p->append(SkRasterPipelineOp::gather_rgf16, ctx);
            break;
        ...
        case kRGBA_F32_SkColorType:
            p->append(SkRasterPipelineOp::gather_f32, ctx);
            break;
        ...
        case kRGBA_10x6_SkColorType:
            p->append(SkRasterPipelineOp::gather_10x6, ctx);
            break;

        case kGray_8_SkColorType:
            p->append(SkRasterPipelineOp::gather_a8, ctx);
            p->append(SkRasterPipelineOp::alpha_to_gray);
            break;

        case kR8_unorm_SkColorType:
            p->append(SkRasterPipelineOp::gather_a8, ctx);
            p->append(SkRasterPipelineOp::alpha_to_red);
            break;
    ...
    auto append_misc = [&] {
        ...
        if (sampling.useCubic) {
            p->append(at == kUnpremul_SkAlphaType || fClampAsIfUnpremul ? SkRasterPipelineOp::clamp_01 : SkRasterPipelineOp::clamp_gamut);
        }
    ...
    if (true && (ct == kRGBA_8888_SkColorType || ct == kBGRA_8888_SkColorType) && !sampling.useCubic && sampling.filter == SkFilterMode::kLinear
        && sampling.mipmap != SkMipmapMode::kLinear && fTileModeX == SkTileMode::kClamp && fTileModeY == SkTileMode::kClamp) {
    ...
    if (true && (ct == kRGBA_8888_SkColorType || ct == kBGRA_8888_SkColorType) && sampling.useCubic && fTileModeX == SkTileMode::kClamp
    ...
    auto sample = [&](SkRasterPipelineOp setup_x, SkRasterPipelineOp setup_y, const MipLevelHelper* level) {
...
sk_sp<SkShader> Image(sk_sp<SkImage> image, SkTileMode tmx, SkTileMode tmy, const SkSamplingOptions& options, const SkMatrix* localMatrix)
{
    return SkImageShader::Make(std::move(image), tmx, tmy, options, localMatrix);
}

sk_sp<SkShader> RawImage(sk_sp<SkImage> image, SkTileMode tmx, SkTileMode tmy, const SkSamplingOptions& options, const SkMatrix* localMatrix)
{
