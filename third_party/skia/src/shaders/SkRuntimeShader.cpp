...
SkRuntimeShader::SkRuntimeShader(
    sk_sp<SkRuntimeEffect> effect, sk_sp<SkSL::DebugTracePriv> debugTrace, sk_sp<const SkData> uniforms, SkSpan<const SkRuntimeEffect::ChildPtr> children)
...
SkRuntimeShader::SkRuntimeShader(
    sk_sp<SkRuntimeEffect> effect, sk_sp<SkSL::DebugTracePriv> debugTrace, UniformsCallback uniformsCallback, SkSpan<const SkRuntimeEffect::ChildPtr> children)
    : fEffect(std::move(effect))
    , fDebugTrace(std::move(debugTrace))
...
SkRuntimeEffect::TracedShader SkRuntimeShader::makeTracedClone(const SkIPoint& coord)
{
    ...
    auto debugShader = sk_make_sp<SkRuntimeShader>(unoptimized, debugTrace, this->uniformData(nullptr), SkSpan(fChildren));
...
bool SkRuntimeShader::appendStages(const SkStageRec& rec, const SkShaders::MatrixRec& mRec) const
{
    ...
    if (const SkSL::RP::Program* program = fEffect->getRPProgram(fDebugTrace.get())) {
        ...
        SkSpan<const float> uniforms = SkRuntimeEffectPriv::UniformsAsSpan(fEffect->uniforms(), this->uniformData(rec.fDstCS),
            /*alwaysCopyIntoAlloc=*/fUniformData == nullptr, rec.fDstCS, rec.fAlloc);
