...
bool SkColor4Shader::appendStages(const SkStageRec& rec, const SkShaders::MatrixRec&) const
{
    ...
    SkColorSpaceXformSteps(fColorSpace.get(), kUnpremul_SkAlphaType, rec.fDstCS, kUnpremul_SkAlphaType).apply(color.vec());
    rec.fPipeline->appendConstantColor(rec.fAlloc, color.premul().vec());
    return true;
}
...
{
    SkColor4f color = fColor;
    SkColorSpaceXformSteps(fColorSpace.get(), kUnpremul_SkAlphaType, sk_srgb_singleton(), kUnpremul_SkAlphaType).apply(color.vec());
    *lum = color;
    return true;
}
...
{
    return sk_make_sp<SkColorShader>(color);
}
