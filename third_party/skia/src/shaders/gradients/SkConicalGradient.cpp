...
bool SkConicalGradient::MapToUnitX(const SkPoint& startCenter, const SkPoint& endCenter, SkMatrix* dstMatrix)
{
...
sk_sp<SkShader> SkConicalGradient::Create(const SkPoint& c0, SkScalar r0, const SkPoint& c1, SkScalar r1, const Descriptor& desc, const SkMatrix* localMatrix)
{
    ...
    sk_sp<SkShader> s = sk_make_sp<SkConicalGradient>(c0, r0, c1, r1, desc, gradientType, gradientMatrix, focalData);
    return s->makeWithLocalMatrix(localMatrix ? *localMatrix : SkMatrix::I());
}

SkConicalGradient::SkConicalGradient(const SkPoint& start, SkScalar startRadius, const SkPoint& end, SkScalar endRadius, const Descriptor& desc, Type type,
    const SkMatrix& gradientMatrix, const FocalData& data)
...
SkShaderBase::GradientType SkConicalGradient::asGradient(GradientInfo* info, SkMatrix* localMatrix) const
{
    if (info) {
...
sk_sp<SkFlattenable> SkConicalGradient::CreateProc(SkReadBuffer& buffer)
{
    ...
    return SkGradientShader::MakeTwoPointConical(
        c1, r1, c2, r2, desc.fColors, std::move(desc.fColorSpace), desc.fPositions, desc.fColorCount, desc.fTileMode, desc.fInterpolation, lmPtr);
}
...
void SkConicalGradient::appendGradientStages(SkArenaAlloc* alloc, SkRasterPipeline* p, SkRasterPipeline* postPipeline) const
{
...
sk_sp<SkShader> SkGradientShader::MakeTwoPointConical(const SkPoint& start, SkScalar startRadius, const SkPoint& end, SkScalar endRadius,
    const SkColor4f colors[], sk_sp<SkColorSpace> colorSpace, const SkScalar pos[], int colorCount, SkTileMode mode, const Interpolation& interpolation,
    const SkMatrix* localMatrix)
{
    ...
    if (SkScalarNearlyZero((start - end).length(), SkGradientBaseShader::kDegenerateThreshold)) {
        ...
        if (SkScalarNearlyEqual(startRadius, endRadius, SkGradientBaseShader::kDegenerateThreshold)) {
            ...
            if (mode == SkTileMode::kClamp && endRadius > SkGradientBaseShader::kDegenerateThreshold) {
                ...
                return MakeRadial(start, endRadius, reColors, std::move(colorSpace), circlePos, 3, mode, interpolation, localMatrix);
            } else {
                // Otherwise use the default degenerate case
                return SkGradientBaseShader::MakeDegenerateGradient(colors, pos, colorCount, std::move(colorSpace), mode);
            }
            ...
            return MakeRadial(start, endRadius, colors, std::move(colorSpace), pos, colorCount, mode, interpolation, localMatrix);
        }
    ...
    SkGradientBaseShader::Descriptor desc(colors, std::move(colorSpace), pos, colorCount, mode, interpolation);
...
sk_sp<SkShader> SkGradientShader::MakeTwoPointConical(const SkPoint& start, SkScalar startRadius, const SkPoint& end, SkScalar endRadius,
    const SkColor colors[], const SkScalar pos[], int colorCount, SkTileMode mode, uint32_t flags, const SkMatrix* localMatrix)
{
    SkColorConverter converter(colors, colorCount);
    return MakeTwoPointConical(start, startRadius, end, endRadius, converter.fColors4f.begin(), nullptr, pos, colorCount, mode, flags, localMatrix);
}
