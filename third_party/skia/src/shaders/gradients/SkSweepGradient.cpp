...
SkSweepGradient::SkSweepGradient(const SkPoint& center, SkScalar t0, SkScalar t1, const Descriptor& desc)
...
SkShaderBase::GradientType SkSweepGradient::asGradient(GradientInfo* info, SkMatrix* localMatrix) const
{
    if (info) {
...
sk_sp<SkFlattenable> SkSweepGradient::CreateProc(SkReadBuffer& buffer)
{
    ...
    return SkGradientShader::MakeSweep(center.x(), center.y(), desc.fColors, std::move(desc.fColorSpace), desc.fPositions, desc.fColorCount, desc.fTileMode,
        startAngle, endAngle, desc.fInterpolation, lmPtr);
}
...
void SkSweepGradient::appendGradientStages(SkArenaAlloc* alloc, SkRasterPipeline* p, SkRasterPipeline*) const
{
    p->append(SkRasterPipelineOp::xy_to_unit_angle);
    p->appendMatrix(alloc, SkMatrix::Scale(fTScale, 1) * SkMatrix::Translate(fTBias, 0));
}

sk_sp<SkShader> SkGradientShader::MakeSweep(SkScalar cx, SkScalar cy, const SkColor4f colors[], sk_sp<SkColorSpace> colorSpace, const SkScalar pos[],
    int colorCount, SkTileMode mode, SkScalar startAngle, SkScalar endAngle, const Interpolation& interpolation, const SkMatrix* localMatrix)
{
    ...
    if (SkScalarNearlyEqual(startAngle, endAngle, SkGradientBaseShader::kDegenerateThreshold)) {
        ...
        if (mode == SkTileMode::kClamp && endAngle > SkGradientBaseShader::kDegenerateThreshold) {
            ...
            return MakeSweep(cx, cy, reColors, std::move(colorSpace), clampPos, 3, mode, 0, endAngle, interpolation, localMatrix);
        } else {
...
sk_sp<SkShader> SkGradientShader::MakeSweep(SkScalar cx, SkScalar cy, const SkColor colors[], const SkScalar pos[], int colorCount, SkTileMode mode,
    SkScalar startAngle, SkScalar endAngle, uint32_t flags, const SkMatrix* localMatrix)
{
    SkColorConverter converter(colors, colorCount);
    return MakeSweep(cx, cy, converter.fColors4f.begin(), nullptr, pos, colorCount, mode, startAngle, endAngle, flags, localMatrix);
}
