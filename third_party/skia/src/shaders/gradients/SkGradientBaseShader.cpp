...
template <int N, typename T, bool MEM_MOVE> static bool validate_array(SkReadBuffer& buffer, size_t count, STArray<N, T, MEM_MOVE>* array)
{
...
bool SkGradientBaseShader::DescriptorScope::unflatten(SkReadBuffer& buffer, SkMatrix* legacyLocalMatrix)
{
    ...
    fInterpolation.fColorSpace = (Interpolation::ColorSpace)((flags >> kInterpolationColorSpaceShift_GSF) & kInterpolationColorSpaceMask_GSF);
    fInterpolation.fHueMethod = (Interpolation::HueMethod)((flags >> kInterpolationHueMethodShift_GSF) & kInterpolationHueMethodMask_GSF);
    fInterpolation.fInPremul = (flags & kInterpolationInPremul_GSF) ? Interpolation::InPremul::kYes : Interpolation::InPremul::kNo;

    fColorCount = buffer.getArrayCount();

    if (!(validate_array(buffer, fColorCount, &fColorStorage) && buffer.readColor4fArray(fColorStorage.begin(), fColorCount))) {
        return false;
    }
    ...
    if (SkToBool(flags & kHasPosition_GSF)) {
        if (!(validate_array(buffer, fColorCount, &fPositionStorage) && buffer.readScalarArray(fPositionStorage.begin(), fColorCount))) {
            return false;
        }
    ...
    , fColorsAreOpaque(true)
{
    ...
    size_t storageSize = fColorCount * (sizeof(SkColor4f) + (desc.fPositions ? sizeof(SkScalar) : 0));
        ...
        if (uniformStops) {
            ...
            for (int j = 1; j <= fColorCount; j++) {
                ...
                if (j == fColorCount || fPositions[i] != fPositions[j]) {
                    ...
                    bool ignoreLeftmost = dupStop && fTileMode != SkTileMode::kClamp && fPositions[i] == 0;
                    if (!ignoreLeftmost) {
                    ...
                    bool ignoreRightmost = fTileMode != SkTileMode::kClamp && fPositions[j - 1] == 1;
...
void SkGradientBaseShader::AppendGradientFillStages(SkRasterPipeline* p, SkArenaAlloc* alloc, const SkPMColor4f* pmColors, const SkScalar* positions, int count)
{
...
void SkGradientBaseShader::AppendInterpolatedToDstStages(SkRasterPipeline* p, SkArenaAlloc* alloc, bool colorsAreOpaque, const Interpolation& interpolation,
    const SkColorSpace* intermediateColorSpace, const SkColorSpace* dstColorSpace)
{
    ...
    alloc->make<SkColorSpaceXformSteps>(intermediateColorSpace, intermediateAlphaType, dstColorSpace, dstAlphaType)->apply(p);
}

bool SkGradientBaseShader::appendStages(const SkStageRec& rec, const SkShaders::MatrixRec& mRec) const
{
    ...
    AppendGradientFillStages(p, alloc, xformedColors.fColors.begin(), xformedColors.fPositions, xformedColors.fColors.size());
    AppendInterpolatedToDstStages(p, alloc, fColorsAreOpaque, fInterpolation, xformedColors.fIntermediateColorSpace.get(), rec.fDstCS);
...
static SkPMColor4f lin_srgb_to_oklab(SkPMColor4f rgb, bool* /*hueIsPowerless*/)
{
    ...
    return { 0.2104542553f * l + 0.7936177850f * m - 0.0040720468f * s, 1.9779984951f * l - 2.4285922050f * m + 0.4505937099f * s,
        0.0259040371f * l + 0.7827717662f * m - 0.8086757660f * s, rgb.fA };
}
...
SkColor4fXformer::SkColor4fXformer(const SkGradientBaseShader* shader, SkColorSpace* dst, bool forceExplicitPositions)
{
    using ColorSpace = SkGradientShader::Interpolation::ColorSpace;
    ...
    SkAssertResult(SkConvertPixels(dstInfo, fColors.begin(), info.minRowBytes(), srcInfo, shader->fColors, info.minRowBytes()));

    // 3) Transform to the interpolation color space (if it's special)
    ConvertColorProc convertFn = nullptr;
    switch (interpolation.fColorSpace) {
    case ColorSpace::kHSL:
        convertFn = srgb_to_hsl;
        break;
    case ColorSpace::kHWB:
        convertFn = srgb_to_hwb;
        break;
    case ColorSpace::kLab:
        convertFn = xyzd50_to_lab;
        break;
    case ColorSpace::kLCH:
        convertFn = xyzd50_to_hcl;
        break;
    case ColorSpace::kOKLab:
        convertFn = lin_srgb_to_oklab;
        break;
    case ColorSpace::kOKLabGamutMap:
        convertFn = lin_srgb_to_oklab;
        break;
    case ColorSpace::kOKLCH:
        convertFn = lin_srgb_to_okhcl;
        break;
    case ColorSpace::kOKLCHGamutMap:
        convertFn = lin_srgb_to_okhcl;
        break;
    ...
    if (color_space_is_polar(interpolation.fColorSpace)) {
        ...
        for (int i = 0; i < colorCount - 1; ++i) {
            ...
            case HueMethod::kLonger:
                if ((i == 0 && shader->fFirstStopIsImplicit) || (i == colorCount - 2 && shader->fLastStopIsImplicit)) {
...
SkColorConverter::SkColorConverter(const SkColor* colors, int count)
{
    ...
    for (int i = 0; i < count; ++i) {
        fColors4f.push_back({ SkColorGetR(colors[i]) * ONE_OVER_255, SkColorGetG(colors[i]) * ONE_OVER_255, SkColorGetB(colors[i]) * ONE_OVER_255,
...
void SkGradientBaseShader::commonAsAGradient(GradientInfo* info) const
{
    if (info) {
        ...
        info->fGradientFlags = this->interpolateInPremul() ? SkGradientShader::kInterpolateColorsInPremul_Flag : 0;
    }
}
...
{
    return nullptr != colors && count >= 1 && (unsigned)tileMode < kSkTileModeCount && (unsigned)interpolation.fColorSpace < Interpolation::kColorSpaceCount
        && (unsigned)interpolation.fHueMethod < Interpolation::kHueMethodCount;
}

SkGradientBaseShader::Descriptor::Descriptor(
    const SkColor4f colors[], sk_sp<SkColorSpace> colorSpace, const SkScalar positions[], int colorCount, SkTileMode mode, const Interpolation& interpolation)
    : fColors(colors)
...
sk_sp<SkShader> SkGradientBaseShader::MakeDegenerateGradient(
    const SkColor4f colors[], const SkScalar pos[], int colorCount, sk_sp<SkColorSpace> colorSpace, SkTileMode mode)
{
    switch (mode) {
    case SkTileMode::kDecal:
    ...
    case SkTileMode::kMirror:
        ...
        return SkShaders::Color(average_gradient_color(colors, pos, colorCount), std::move(colorSpace));
    case SkTileMode::kClamp:
