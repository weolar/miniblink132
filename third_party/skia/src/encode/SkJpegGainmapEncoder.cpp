...
sk_sp<SkData> get_gainmap_image_xmp_metadata(const SkGainmapInfo& gainmapInfo)
{
    ...
    const SkColor4f gainMapMin = { std::log(gainmapInfo.fGainmapRatioMin.fR) / kLog2, std::log(gainmapInfo.fGainmapRatioMin.fG) / kLog2,
        std::log(gainmapInfo.fGainmapRatioMin.fB) / kLog2, 1.f };
    const SkColor4f gainMapMax = { std::log(gainmapInfo.fGainmapRatioMax.fR) / kLog2, std::log(gainmapInfo.fGainmapRatioMax.fG) / kLog2,
        std::log(gainmapInfo.fGainmapRatioMax.fB) / kLog2, 1.f };
    const SkColor4f gamma = { 1.f / gainmapInfo.fGainmapGamma.fR, 1.f / gainmapInfo.fGainmapGamma.fG, 1.f / gainmapInfo.fGainmapGamma.fB, 1.f };
    ...
    s.writeText("<x:xmpmeta xmlns:x=\"adobe:ns:meta/\" x:xmptk=\"XMP Core 5.5.0\">\n"
...
{
    SkDynamicMemoryWStream s;
    s.writeText("<x:xmpmeta xmlns:x=\"adobe:ns:meta/\" x:xmptk=\"Adobe XMP Core 5.1.2\">\n"
                "  <rdf:RDF xmlns:rdf=\"http://www.w3.org/1999/02/22-rdf-syntax-ns#\">\n"
                "    <rdf:Description\n"
...
bool SkJpegGainmapEncoder::EncodeHDRGM(SkWStream* dst, const SkPixmap& base, const SkJpegEncoder::Options& baseOptions, const SkPixmap& gainmap,
    const SkJpegEncoder::Options& gainmapOptions, const SkGainmapInfo& gainmapInfo)
{
    ...
    sk_sp<SkData> gainmapData;
    {
        ...
        if (includeUltraHDRv1) {
            SkJpegMetadataEncoder::AppendXMPStandard(metadataSegments, get_gainmap_image_xmp_metadata(gainmapInfo).get());
        }

        // Include the ICC profile of the alternate color space, if it is used.
        if (gainmapInfo.fGainmapMathColorSpace) {
            SkJpegMetadataEncoder::AppendICC(metadataSegments, gainmapOptions, gainmapInfo.fGainmapMathColorSpace.get());
        }

        // Add the ISO 21946-1 metadata.
        metadataSegments.emplace_back(kISOGainmapMarker, get_iso_gainmap_segment_params(gainmapInfo.serialize()));
    ...
    sk_sp<SkData> baseData;
    {
        ...
        if (includeUltraHDRv1) {
            ...
            SkJpegMetadataEncoder::AppendXMPStandard(metadataSegments, get_base_image_xmp_metadata(static_cast<int32_t>(gainmapImageSize)).get());
        }
        ...
        metadataSegments.emplace_back(kISOGainmapMarker, get_iso_gainmap_segment_params(SkGainmapInfo::SerializeVersion()));
...
static size_t mp_segment_offset(const SkData* image)
{
    ...
    for (size_t segmentIndex = 0; segmentIndex < segments.size() - 1; ++segmentIndex) {
        ...
        if (params->size() < sizeof(kExifSig) || memcmp(params->data(), kExifSig, sizeof(kExifSig)) != 0) {
            continue;
        }
...
bool SkJpegGainmapEncoder::MakeMPF(SkWStream* dst, const SkData** images, size_t imageCount)
{
    ...
    for (size_t i = 0; i < imageCount; ++i) {
        ...
        mpParams.images[i].dataOffset = SkJpegMultiPictureParameters::GetImageDataOffset(cumulativeSize, mpSegmentOffsets[0]);
    ...
    for (size_t i = 0; i < imageCount; ++i) {
        ...
        if (!dst->write(images[i]->bytes() + mpSegmentOffsets[i], images[i]->size() - mpSegmentOffsets[i])) {
