...
bool SkJpegEncoderMgr::initializeRGB(
    const SkImageInfo& srcInfo, const SkJpegEncoder::Options& options, const SkJpegMetadataEncoder::SegmentList& metadataSegments)
{
    auto chooseProc8888 = [&]() {
        if (kUnpremul_SkAlphaType == srcInfo.alphaType() && options.fAlphaOption == SkJpegEncoder::AlphaOption::kBlendOnBlack) {
...
static void yuva_copy_row(const SkYUVAPixmaps& src, int row, uint8_t* dst)
{
    ...
    case SkYUVAInfo::PlaneConfig::kY_U_V: {
        ...
        const uint8_t* srcU = reinterpret_cast<const uint8_t*>(src.plane(1).addr(0, row / ssHeightU));
        const uint8_t* srcV = reinterpret_cast<const uint8_t*>(src.plane(2).addr(0, row / ssHeightV));
    ...
    case SkYUVAInfo::PlaneConfig::kY_UV: {
        ...
        const uint8_t* srcY = reinterpret_cast<const uint8_t*>(src.plane(0).addr(0, row));
        const uint8_t* srcUV = reinterpret_cast<const uint8_t*>(src.plane(1).addr(0, row / ssHeightUV));
        for (int col = 0; col < width; ++col) {
            dst[3 * col + 0] = srcY[col];
...
bool SkJpegEncoderMgr::initializeYUV(
    const SkYUVAPixmapInfo& srcInfo, const SkJpegEncoder::Options& options, const SkJpegMetadataEncoder::SegmentList& metadataSegments)
{
...
void SkJpegEncoderMgr::initializeCommon(const SkJpegEncoder::Options& options, const SkJpegMetadataEncoder::SegmentList& metadataSegments)
{
    ...
    for (const auto& segment : metadataSegments) {
        jpeg_write_marker(&fCInfo, segment.fMarker, segment.fParameters->bytes(), segment.fParameters->size());
    }
}

std::unique_ptr<SkEncoder> SkJpegEncoderImpl::MakeYUV(SkWStream* dst, const SkYUVAPixmaps& srcYUVA, const SkColorSpace* srcYUVAColorSpace,
    const SkJpegEncoder::Options& options, const SkJpegMetadataEncoder::SegmentList& metadataSegments)
{
    if (!srcYUVA.isValid()) {
        return nullptr;
    }
    ...
    return std::unique_ptr<SkJpegEncoderImpl>(new SkJpegEncoderImpl(std::move(encoderMgr), srcYUVA));
}

std::unique_ptr<SkEncoder> SkJpegEncoderImpl::MakeRGB(
    SkWStream* dst, const SkPixmap& src, const SkJpegEncoder::Options& options, const SkJpegMetadataEncoder::SegmentList& metadataSegments)
{
...
SkJpegEncoderImpl::SkJpegEncoderImpl(std::unique_ptr<SkJpegEncoderMgr> encoderMgr, const SkPixmap& src)
    : SkEncoder(src, encoderMgr->proc() ? encoderMgr->cinfo()->input_components * src.width() : 0)
    , fEncoderMgr(std::move(encoderMgr))
{
}

SkJpegEncoderImpl::SkJpegEncoderImpl(std::unique_ptr<SkJpegEncoderMgr> encoderMgr, const SkYUVAPixmaps& src)
    ...
    if (fSrcYUVA) {
        ...
        for (int i = 0; i < numRows; i++) {
            ...
            if (fEncoderMgr->proc()) {
                ...
                fEncoderMgr->proc()((char*)fStorage.get(), (const char*)srcRow, fSrc.width(), fEncoderMgr->cinfo()->input_components);
...
bool Encode(SkWStream* dst, const SkYUVAPixmaps& src, const SkColorSpace* srcColorSpace, const Options& options)
{
...
std::unique_ptr<SkEncoder> Make(SkWStream* dst, const SkYUVAPixmaps& src, const SkColorSpace* srcColorSpace, const Options& options)
{
    SkJpegMetadataEncoder::SegmentList metadataSegments;
    SkJpegMetadataEncoder::AppendXMPStandard(metadataSegments, options.xmpMetadata);
...
{
    sk_sp<SkData> icc = icc_from_color_space(colorSpace, options.fICCProfile, options.fICCProfileDescription);
    if (!icc) {
        return;
    }
