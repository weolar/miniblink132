...
static bool preprocess_webp_picture(WebPPicture* pic, WebPConfig* webp_config, const SkPixmap& pixmap, const SkWebpEncoder::Options& opts)
{
...

    {
        ...
        if (ct == kRGB_888x_SkColorType) {
            ...
            importProc = WebPPictureImportRGBA;
            auto info = pixmap.info().makeColorType(kRGBA_8888_SkColorType).makeAlphaType(kUnpremul_SkAlphaType);
            if (!tmpBm.tryAllocPixels(info) || !pixmap.readPixels(tmpBm.info(), tmpBm.getPixels(), tmpBm.rowBytes())) {
                return false;
            }
            src = &tmpBm.pixmap();
        }
...
bool Encode(SkWStream* stream, const SkPixmap& pixmap, const Options& opts)
{
    ...
    sk_sp<SkData> icc = icc_from_color_space(pixmap.info(), opts.fICCProfile, opts.fICCProfileDescription);
...
bool EncodeAnimated(SkWStream* stream, SkSpan<const SkEncoder::Frame> frames, const Options& opts)
{
    ...
    std::unique_ptr<WebPAnimEncoder, void (*)(WebPAnimEncoder*)> enc(WebPAnimEncoderNew(canvasWidth, canvasHeight, nullptr), WebPAnimEncoderDelete);
    if (!enc) {
        return false;
    }
