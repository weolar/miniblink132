...
{
    return nearly_equal(u.g, v.g) && nearly_equal(u.a, v.a) && nearly_equal(u.b, v.b) && nearly_equal(u.c, v.c) && nearly_equal(u.d, v.d)
        && nearly_equal(u.e, v.e) && nearly_equal(u.f, v.f);
}
...
std::string get_desc_string(const skcms_TransferFunction& fn, const skcms_Matrix3x3& toXYZD50)
{
...
sk_sp<SkData> write_mAB_or_mBA_tag(uint32_t type, const skcms_Curve* b_curves, const skcms_Curve* a_curves, const uint8_t* grid_points, const uint8_t* grid_16,
    const skcms_Curve* m_curves, const skcms_Matrix3x4* matrix)
{
    size_t offset = 32;
...
sk_sp<SkData> SkWriteICCProfile(const skcms_ICCProfile* profile, const char* desc)
{
    ...
    if (profile->has_A2B) {
        ...
        auto a2b_data = write_mAB_or_mBA_tag(kTAG_mABType, a2b.output_curves, a2b.input_channels ? a2b.input_curves : nullptr,
            a2b.input_channels ? a2b.grid_points : nullptr, a2b.input_channels ? a2b.grid_16 : nullptr, a2b.matrix_channels ? a2b.matrix_curves : nullptr,
    ...
    if (profile->has_B2A) {
        ...
        auto b2a_data = write_mAB_or_mBA_tag(kTAG_mBAType, b2a.input_curves, b2a.output_channels ? b2a.input_curves : nullptr,
            b2a.output_channels ? b2a.grid_points : nullptr, b2a.output_channels ? b2a.grid_16 : nullptr, b2a.matrix_channels ? b2a.matrix_curves : nullptr,
            ...
            for (uint32_t g_index = 0; g_index < kGridSize; ++g_index) {
                for (uint32_t b_index = 0; b_index < kGridSize; ++b_index) {
                    ...
                    for (const auto& c : rgb) {
                        a2b_grid[a2b_grid_index++] = SkEndian_SwapBE16(float_to_uInt16Number(c, kOne16XYZ));
                    }
                }
            }
        }
