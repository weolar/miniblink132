...
std::unique_ptr<SkCodec> SkWebpCodec::MakeFromStream(std::unique_ptr<SkStream> stream, Result* result)
{
    SkASSERT(result);
    if (!stream) {
    ...
    *result = kSuccess;
    ...
    return std::unique_ptr<SkCodec>(new SkWebpCodec(std::move(info), std::move(stream), demux.release(), std::move(data), origin));
}
...
int SkWebpCodec::onGetFrameCount()
{
    ...
    for (uint32_t i = oldFrameCount; i < frameCount; i++) {
        ...
        frame->setDisposalMethod(
            iter.dispose_method == WEBP_MUX_DISPOSE_BACKGROUND ? SkCodecAnimation::DisposalMethod::kRestoreBGColor : SkCodecAnimation::DisposalMethod::kKeep);
...
{
    SkRasterPipeline_MemoryCtx dst_ctx = { dst, 0 }, src_ctx = { const_cast<void*>(src), 0 };
...
SkCodec::Result SkWebpCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst, size_t rowBytes, const Options& options, int* rowsDecodedPtr)
{
    ...
    const bool independent = index == 0 ? true : (fFrameHolder.frame(index)->getRequiredFrame() == kNoFrame);
    ...
    const bool blendWithPrevFrame = !independent && frame.blend_method == WEBP_MUX_BLEND && frame.has_alpha;
    ...
    config.output.colorspace = webp_decode_mode(webpInfo.colorType(), webpInfo.alphaType() == kPremul_SkAlphaType);
    ...
    if (this->colorXform()) {
        ...
        for (int y = 0; y < rowsDecoded; y++) {
            blend_line(dstCT, dst, webpDst.colorType(), src, dstInfo.alphaType(), frame.has_alpha, scaledWidth);
...
SkWebpCodec::SkWebpCodec(SkEncodedInfo&& info, std::unique_ptr<SkStream> stream, WebPDemuxer* demux, sk_sp<SkData> data, SkEncodedOrigin origin)
