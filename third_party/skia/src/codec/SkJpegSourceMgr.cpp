...
public:
    ...
    sk_sp<SkData> getSubsetData(size_t offset, size_t size, bool* wasCopied) override
    {
        ...
        return SkData::MakeWithoutCopy(reinterpret_cast<const uint8_t*>(fStream->getMemoryBase()) + offset, size);
    }
    sk_sp<SkData> getSegmentParameters(const SkJpegSegment& segment) override
    {
        const uint8_t* base = reinterpret_cast<const uint8_t*>(fStream->getMemoryBase()) + segment.offset;
        SkASSERT(segment.offset < fStream->getLength());
        SkASSERT(kJpegMarkerCodeSize + segment.parameterLength <= fStream->getLength() - segment.offset);
        ...
        return SkData::MakeWithoutCopy(base + kJpegMarkerCodeSize + kJpegSegmentParameterLengthSize, segment.parameterLength - kJpegSegmentParameterLengthSize);
    }
#endif // SK_CODEC_DECODES_JPEG_GAINMAPS
};
...
public:
    ...
    sk_sp<SkData> getSegmentParameters(const SkJpegSegment& segment) override
    {
        ...
        if (fStream->read(parameterLength, kJpegSegmentParameterLengthSize) != kJpegSegmentParameterLengthSize) {
...
public:
    SkJpegUnseekableSourceMgr(SkStream* stream, size_t bufferSize)
        : SkJpegSourceMgr(stream)
    {
        fBuffer = SkData::MakeUninitialized(bufferSize);
        fScanner = std::make_unique<SkJpegSegmentScanner>(kJpegMarkerEndOfImage);
