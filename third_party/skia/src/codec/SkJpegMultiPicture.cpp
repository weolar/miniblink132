...
std::unique_ptr<SkJpegMultiPictureParameters> SkJpegMultiPictureParameters::Make(const sk_sp<const SkData>& segmentParameters)
{
    ...
    auto ifdData = SkData::MakeSubset(segmentParameters.get(), sizeof(kMpfSig), segmentParameters->size() - sizeof(kMpfSig));
    SkASSERT(ifdData);
    ...
    for (uint16_t idfEntryIndex = 0; idfEntryIndex < tagCount; ++idfEntryIndex) {
        ...
        case kMPEntryTag: {
            ...
            if (mpEntriesData->size() != kMPEntrySize * numberOfImages) {
                SkCodecPrintf("MP entries data should be %ux%u bytes, was %u.\n", kMPEntrySize, numberOfImages, static_cast<uint32_t>(mpEntriesData->size()));
                return nullptr;
            }
            break;
        }
    ...
    for (uint32_t i = 0; i < numberOfImages; ++i) {
        ...
        const bool isPrimary = (attribute & kMPEntryAttributeTypeMask) == kMPEntryAttributeTypePrimary;
        const bool isJpeg = (attribute & kMPEntryAttributeFormatMask) == kMPEntryAttributeFormatJpeg;
...
sk_sp<SkData> SkJpegMultiPictureParameters::serialize(uint32_t individualImageNumber) const
{
    ...
    if (individualImageNumber == 0) {
        ...
        const uint32_t mpEntryOffset = static_cast<uint32_t>(s.bytesWritten() - // The bytes written so far
...
size_t SkJpegMultiPictureParameters::GetImageAbsoluteOffset(uint32_t dataOffset, size_t mpSegmentOffset)
{
...
uint32_t SkJpegMultiPictureParameters::GetImageDataOffset(size_t imageAbsoluteOffset, size_t mpSegmentOffset)
{
    // The value of zero is used by the primary image.
