...
bool SkHeifCodec::IsSupported(const void* buffer, size_t bytesRead, SkEncodedImageFormat* format)
{
    ...
    for (size_t i = 0; i < numCompatibleBrands + 2; ++i) {
        ...
        if (brand == FOURCC('m', 'i', 'f', '1') || brand == FOURCC('h', 'e', 'i', 'c') || brand == FOURCC('m', 's', 'f', '1')
            || brand == FOURCC('h', 'e', 'v', 'c') || brand == FOURCC('a', 'v', 'i', 'f') || brand == FOURCC('a', 'v', 'i', 's')) {
...
std::unique_ptr<SkCodec> SkHeifCodec::MakeFromStream(std::unique_ptr<SkStream> stream, SkCodec::SelectionPolicy selectionPolicy, Result* result)
{
    SkASSERT(result);
    if (!stream) {
    ...
    SkEncodedInfo info = SkEncodedInfo::Make(heifInfo.mWidth, heifInfo.mHeight, SkEncodedInfo::kYUV_Color, SkEncodedInfo::kOpaque_Alpha,
    ...
    *result = SkCodec::kSuccess;
    return std::unique_ptr<SkCodec>(new SkHeifCodec(std::move(info), heifDecoder.release(), orientation, frameCount > 1, format));
}

SkHeifCodec::SkHeifCodec(SkEncodedInfo&& info, HeifDecoder* heifDecoder, SkEncodedOrigin origin, bool useAnimation, SkEncodedImageFormat format)
...
bool SkHeifCodec::conversionSupported(const SkImageInfo& dstInfo, bool srcIsOpaque, bool needsColorXform)
{
...
int SkHeifCodec::readRows(const SkImageInfo& dstInfo, void* dst, size_t rowBytes, int count, const Options& opts)
{
 ...
 */
SkCodec::Result SkHeifCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst, size_t dstRowBytes, const Options& options, int* rowsDecoded)
{
    if (options.fSubset) {
...
void SkHeifCodec::allocateStorage(const SkImageInfo& dstInfo)
{
    ...
    size_t xformBytes = 0;
    if (this->colorXform() && (kRGBA_F16_SkColorType == dstInfo.colorType() || kRGB_565_SkColorType == dstInfo.colorType())) {
...
SkCodec::Result SkHeifCodec::onStartScanlineDecode(const SkImageInfo& dstInfo, const Options& options)
{
