...
dng_point num_tiles_in_area(const dng_point& areaSize, const dng_point_real64& tileSize)
{
    ...
    return dng_point(static_cast<int32>((areaSize.v + tileSize.v - 1) / tileSize.v), static_cast<int32>((areaSize.h + tileSize.h - 1) / tileSize.h));
}
...
std::vector<dng_rect> compute_task_areas(const int maxTasks, const dng_rect& area, const dng_point& tileSize)
{
...
public:
    ...
    void PerformAreaTask(dng_area_task& task, const dng_rect& area) override
    {
        ...
        const std::vector<dng_rect> taskAreas = compute_task_areas(this->PerformAreaTaskThreads(), area, tileSize);
...
template <class T> bool safe_add_to_size_t(T arg1, T arg2, size_t* result)
{
    SkASSERT(arg1 >= 0);
    SkASSERT(arg2 >= 0);
...
private:
    ...
    bool initFromPiex()
    {
        ...
        if (::piex::IsRaw(&piexStream) && ::piex::GetPreviewImageData(&piexStream, &imageData) == ::piex::Error::kOk) {
            dng_point cfaPatternSize(imageData.cfa_pattern_dim[1], imageData.cfa_pattern_dim[0]);
            return this->init(static_cast<int>(imageData.full_width), static_cast<int>(imageData.full_height), cfaPatternSize);
        }
        return false;
    }
    ...
    bool readDng()
    {
        try {
            ...
            return this->init(
                static_cast<int>(fNegative->DefaultCropSizeH().As_real64()), static_cast<int>(fNegative->DefaultCropSizeV().As_real64()), cfaPatternSize);
        } catch (...) {
            return false;
        }
    }
 ...
 */
std::unique_ptr<SkCodec> SkRawCodec::MakeFromStream(std::unique_ptr<SkStream> stream, Result* result)
{
    SkASSERT(result);
    if (!stream) {
    ...
    if (::piex::IsRaw(&piexStream)) {
        ...
        if (error == ::piex::Error::kOk && imageData.preview.length > 0 && imageData.preview.format == ::piex::Image::kJpegCompressed) {
            ...
            auto memoryStream = rawStream->transferBuffer(imageData.preview.offset, imageData.preview.length);
            if (!memoryStream) {
                *result = kInvalidInput;
                return nullptr;
            }
...
SkCodec::Result SkRawCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst, size_t dstRowBytes, const Options& options, int* rowsDecoded)
{
    ...
    if (imageSize.h / (float)width > maxDiffRatio || imageSize.h < width || imageSize.v / (float)height > maxDiffRatio || imageSize.v < height) {
    ...
    for (int i = 0; i < height; ++i) {
        ...
        if (!skcms_Transform(
                &srcRow[0], srcFormat, skcms_AlphaFormat_Unpremul, srcProfile, dstRow, dstFormat, skcms_AlphaFormat_Unpremul, dstProfile, dstInfo.width())) {
...
SkISize SkRawCodec::onGetScaledDimensions(float desiredScale) const
{
    ...
    return SkISize::Make(static_cast<int32_t>(std::floor(dim.fWidth / finalScale)), static_cast<int32_t>(std::floor(dim.fHeight / finalScale)));
}
...
SkRawCodec::SkRawCodec(SkDngImage* dngImage)
    : INHERITED(SkEncodedInfo::Make(dngImage->width(), dngImage->height(), SkEncodedInfo::kRGB_Color, SkEncodedInfo::kOpaque_Alpha, 8),
