...
static void swizzle_bit_to_grayscale(
    void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src, int dstWidth, int bpp, int deltaSrc, int offset, const SkPMColor* /*ctable*/)
{
...
static void swizzle_bit_to_f16(
    void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src, int dstWidth, int bpp, int deltaSrc, int offset, const SkPMColor* /*ctable*/)
{
    constexpr uint64_t kWhite = (((uint64_t)SK_Half1) << 0) | (((uint64_t)SK_Half1) << 16) | (((uint64_t)SK_Half1) << 32) | (((uint64_t)SK_Half1) << 48);
    constexpr uint64_t kBlack = (((uint64_t)0) << 0) | (((uint64_t)0) << 16) | (((uint64_t)0) << 32) | (((uint64_t)SK_Half1) << 48);
...
static void swizzle_index_to_565(
    void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src, int dstWidth, int bytesPerPixel, int deltaSrc, int offset, const SkPMColor ctable[])
{
    src += offset;
    uint16_t* SK_RESTRICT dst = (uint16_t*)dstRow;
    for (int x = 0; x < dstWidth; x++) {
...
{
    auto strip16to8 = [](const uint8_t* ptr) { return 0xFF000000 | (ptr[4] << 16) | (ptr[2] << 8) | ptr[0]; };
...
{
    auto strip16to8 = [](const uint8_t* ptr) { return 0xFF000000 | (ptr[0] << 16) | (ptr[2] << 8) | ptr[4]; };
...
{
    auto strip16to8 = [](const uint8_t* ptr) { return (ptr[6] << 24) | (ptr[4] << 16) | (ptr[2] << 8) | ptr[0]; };
...
{
    auto stripAndPremul16to8 = [](const uint8_t* ptr) { return premultiply_argb_as_rgba(ptr[6], ptr[0], ptr[2], ptr[4]); };
...
{
    auto strip16to8 = [](const uint8_t* ptr) { return (ptr[6] << 24) | (ptr[0] << 16) | (ptr[2] << 8) | ptr[4]; };
...
{
    auto stripAndPremul16to8 = [](const uint8_t* ptr) { return premultiply_argb_as_bgra(ptr[6], ptr[0], ptr[2], ptr[4]); };
...
void SkSwizzler::SkipLeadingGrayAlphaZerosThen(void* dst, const uint8_t* src, int width, int bpp, int deltaSrc, int offset, const SkPMColor ctable[])
{
    SkASSERT(!ctable);
...
std::unique_ptr<SkSwizzler> SkSwizzler::MakeSimple(int srcBPP, const SkImageInfo& dstInfo, const SkCodec::Options& options, const SkIRect* frame)
{
    ...
    return Make(dstInfo, &copy, proc, nullptr /*ctable*/, srcBPP, dstInfo.bytesPerPixel(), options, frame);
}

std::unique_ptr<SkSwizzler> SkSwizzler::Make(
    const SkEncodedInfo& encodedInfo, const SkPMColor* ctable, const SkImageInfo& dstInfo, const SkCodec::Options& options, const SkIRect* frame)
{
...
std::unique_ptr<SkSwizzler> SkSwizzler::Make(const SkImageInfo& dstInfo, RowProc fastProc, RowProc proc, const SkPMColor* ctable, int srcBPP, int dstBPP,
    const SkCodec::Options& options, const SkIRect* frame)
{
    ...
    return std::unique_ptr<SkSwizzler>(new SkSwizzler(fastProc, proc, ctable, srcOffset, srcWidth, dstOffset, dstWidth, srcBPP, dstBPP));
}

SkSwizzler::SkSwizzler(
    RowProc fastProc, RowProc proc, const SkPMColor* ctable, int srcOffset, int srcWidth, int dstOffset, int dstWidth, int srcBPP, int dstBPP)
    : fFastProc(fastProc)
    , fSlowProc(proc)
...
void SkSwizzler::swizzle(void* dst, const uint8_t* SK_RESTRICT src)
{
    ...
    fActualProc(SkTAddOffset<void>(dst, fDstOffsetBytes), src, fSwizzleWidth, fSrcBPP, fSampleX * fSrcBPP, fSrcOffsetUnits, fColorTable);
}
