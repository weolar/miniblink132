...
// #endif
#include "../../../../third_party/wuffs/src/release/c/wuffs-v0.3.c" // NO_G3_REWRITE
...
static SkCodec::Result reset_and_decode_image_config(wuffs_gif__decoder* decoder, wuffs_base__image_config* imgcfg, wuffs_base__io_buffer* b, SkStream* s)
{
    ...
    wuffs_base__status status = decoder->initialize(sizeof__wuffs_gif__decoder(), WUFFS_VERSION, SK_WUFFS_INITIALIZE_FLAGS);
    ...
    if (imgcfg) {
        imgcfg->pixcfg.set(pixfmt, WUFFS_BASE__PIXEL_SUBSAMPLING__NONE, imgcfg->pixcfg.width(), imgcfg->pixcfg.height());
    }
...
public:
    SkWuffsCodec(SkEncodedInfo&& encodedInfo, std::unique_ptr<SkStream> stream, bool canSeek, std::unique_ptr<wuffs_gif__decoder, decltype(&sk_free)> dec,
        std::unique_ptr<uint8_t, decltype(&sk_free)> workbuf_ptr, size_t workbuf_len, wuffs_base__image_config imgcfg, wuffs_base__io_buffer iobuf);
...
private:
    ...
    Result onStartIncrementalDecodeOnePass(
        const SkImageInfo& dstInfo, uint8_t* dst, size_t rowBytes, const SkCodec::Options& options, uint32_t pixelFormat, size_t bytesPerPixel);
...
SkWuffsFrame::SkWuffsFrame(wuffs_base__frame_config* fc)
    ...
    , fReportedAlpha(fc->opaque_within_bounds() ? SkEncodedInfo::kOpaque_Alpha : SkEncodedInfo::kUnpremul_Alpha)
{
    ...
    this->setBlend(fc->overwrite_instead_of_blend() ? SkCodecAnimation::Blend::kSrc : SkCodecAnimation::Blend::kSrcOver);
}
...
SkWuffsCodec::SkWuffsCodec(SkEncodedInfo&& encodedInfo, std::unique_ptr<SkStream> stream, bool canSeek,
    std::unique_ptr<wuffs_gif__decoder, decltype(&sk_free)> dec, std::unique_ptr<uint8_t, decltype(&sk_free)> workbuf_ptr, size_t workbuf_len,
    wuffs_base__image_config imgcfg, wuffs_base__io_buffer iobuf)
    : INHERITED(std::move(encodedInfo), skcms_PixelFormat_RGBA_8888,
...
SkCodec::Result SkWuffsCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst, size_t rowBytes, const Options& options, int* rowsDecoded)
{
...
SkCodec::Result SkWuffsCodec::onStartIncrementalDecode(const SkImageInfo& dstInfo, void* dst, size_t rowBytes, const SkCodec::Options& options)
{
    if (!dst) {
    ...
    result = fIncrDecOnePass ? this->onStartIncrementalDecodeOnePass(dstInfo, static_cast<uint8_t*>(dst), rowBytes, options, pixelFormat, bytesPerPixel)
    ...
    const SkImageInfo& dstInfo, uint8_t* dst, size_t rowBytes, const SkCodec::Options& options, uint32_t pixelFormat, size_t bytesPerPixel)
{
    ...
    pixelConfig.set(pixelFormat, WUFFS_BASE__PIXEL_SUBSAMPLING__NONE, dstInfo.width(), dstInfo.height());
    ...
    if ((options.fFrameIndex != 0) && (this->frame(options.fFrameIndex)->getRequiredFrame() != SkCodec::kNoFrame)) {
...
SkCodec::Result SkWuffsCodec::onStartIncrementalDecodeTwoPass()
{
    ...
    if (!fTwoPassPixbufPtr) {
        ...
        void* pixbuf_ptr_raw = (pixbuf_len <= SIZE_MAX) ? sk_malloc_flags(pixbuf_len, SK_MALLOC_ZERO_INITIALIZE) : nullptr;
        if (!pixbuf_ptr_raw) {
            return SkCodec::kInternalError;
        }
    ...
    wuffs_base__status status = fPixelBuffer.set_from_slice(&fPixelConfig, wuffs_base__make_slice_u8(fTwoPassPixbufPtr.get(), fTwoPassPixbufLen));
    if (status.repr != nullptr) {
...
SkCodec::Result SkWuffsCodec::onIncrementalDecode(int* rowsDecoded)
{
    ...
    SkCodec::Result result = fIncrDecOnePass ? this->onIncrementalDecodeOnePass() : this->onIncrementalDecodeTwoPass();
    if (result == SkCodec::kSuccess) {
        fIncrDecDst = nullptr;
        fIncrDecRowBytes = 0;
        fIncrDecPixelBlend = WUFFS_BASE__PIXEL_BLEND__SRC;
...
SkCodec::Result SkWuffsCodec::onIncrementalDecodeTwoPass()
{
    ...
    if (fFirstCallToIncrementalDecode) {
        ...
        auto bounds = SkIRect::MakeLTRB(frame_rect.min_incl_x, frame_rect.min_incl_y, frame_rect.max_excl_x, frame_rect.max_excl_y);
    ...
    if (!dirty_rect.is_empty()) {
        ...
        uint8_t* s = pixels.ptr + (dirty_rect.min_incl_y * pixels.stride) + (dirty_rect.min_incl_x * src_bytes_per_pixel);
        ...
        auto srcInfo = getInfo().makeWH(dirty_rect.width(), dirty_rect.height()).makeAlphaType(alphaType);
        SkBitmap src;
        ...
        SkMatrix matrix = SkMatrix::RectToRect(SkRect::Make(this->dimensions()), SkRect::Make(this->dstInfo().dimensions()));
        draw.fCTM = &matrix;
...
SkCodec::Result SkWuffsCodec::resetDecoder()
{
    ...
    SkCodec::Result result = reset_and_decode_image_config(fDecoder.get(), nullptr, &fIOBuffer, fPrivStream.get());
...
const char* SkWuffsCodec::decodeFrameConfig()
{
    while (true) {
        ...
        if ((status.repr == wuffs_base__suspension__short_read) && fill_buffer(&fIOBuffer, fPrivStream.get())) {
            continue;
        }
...
const char* SkWuffsCodec::decodeFrame()
{
    while (true) {
        wuffs_base__status status
            = fDecoder->decode_frame(&fPixelBuffer, &fIOBuffer, fIncrDecPixelBlend, wuffs_base__make_slice_u8(fWorkbufPtr.get(), fWorkbufLen), nullptr);
        if ((status.repr == wuffs_base__suspension__short_read) && fill_buffer(&fIOBuffer, fPrivStream.get())) {
            continue;
        }
        fDecoderIsSuspended = !status.is_complete();
...
std::unique_ptr<SkCodec> MakeFromStream(std::unique_ptr<SkStream> stream, SkCodec::SelectionPolicy selectionPolicy, SkCodec::Result* result)
{
    ...
    wuffs_base__io_buffer iobuf = wuffs_base__make_io_buffer(wuffs_base__make_slice_u8(buffer, SK_WUFFS_CODEC_BUFFER_SIZE), wuffs_base__empty_io_buffer_meta());
    ...
    std::unique_ptr<wuffs_gif__decoder, decltype(&sk_free)> decoder(reinterpret_cast<wuffs_gif__decoder*>(decoder_raw), &sk_free);

    SkCodec::Result reset_result = reset_and_decode_image_config(decoder.get(), &imgcfg, &iobuf, stream.get());
    ...
    std::unique_ptr<uint8_t, decltype(&sk_free)> workbuf_ptr(reinterpret_cast<uint8_t*>(workbuf_ptr_raw), &sk_free);

    SkEncodedInfo::Color color
        = (imgcfg.pixcfg.pixel_format().repr == WUFFS_BASE__PIXEL_FORMAT__BGRA_NONPREMUL) ? SkEncodedInfo::kBGRA_Color : SkEncodedInfo::kRGBA_Color;

    // In Skia's API, the alpha we calculate here and return is only for the
    // first frame.
    SkEncodedInfo::Alpha alpha = imgcfg.first_frame_is_opaque() ? SkEncodedInfo::kOpaque_Alpha : SkEncodedInfo::kBinary_Alpha;
    ...
    *result = SkCodec::kSuccess;
    return std::unique_ptr<SkCodec>(
        new SkWuffsCodec(std::move(encodedInfo), std::move(stream), canSeek, std::move(decoder), std::move(workbuf_ptr), workbuf_len, imgcfg, iobuf));
}

std::unique_ptr<SkCodec> Decode(std::unique_ptr<SkStream> stream, SkCodec::Result* outResult, SkCodecs::DecodeContext ctx)
{
...
std::unique_ptr<SkCodec> Decode(sk_sp<SkData> data, SkCodec::Result* outResult, SkCodecs::DecodeContext ctx)
{
    if (!data) {
        if (outResult) {
