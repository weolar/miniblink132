...
// static
bool SkPngCodecBase::isCompatibleColorProfileAndType(const SkEncodedInfo::ICCProfile* profile, SkEncodedInfo::Color color)
{
    if (profile) {
...
SkCodec::Result SkPngCodecBase::initializeXforms(const SkImageInfo& dstInfo, const Options& options, int frameWidth)
{
    ...
    if (skipFormatConversion && !options.fSubset) {
        ...
        Result result = this->initializeSwizzler(dstInfo, options, skipFormatConversion, frameWidth);
        if (result != kSuccess) {
            return result;
        }
    }
...
SkCodec::Result SkPngCodecBase::initializeSwizzler(const SkImageInfo& dstInfo, const Options& options, bool skipFormatConversion, int frameWidth)
{
    ...
    if (skipFormatConversion) {
        ...
        fSwizzler = SkSwizzler::Make(this->getEncodedInfo(), colors, swizzlerInfo, swizzlerOptions, frameRectPtr);
    }
...
SkSampler* SkPngCodecBase::getSampler(bool createIfNecessary)
{
    ...
    // failure.
    std::ignore = this->initializeSwizzler(this->dstInfo(), this->options(), true, this->dstInfo().width());
...
bool SkPngCodecBase::createColorTable(const SkImageInfo& dstInfo)
{
    ...
    if (numColorsWithAlpha < numColors) {
        ...
        if (is_rgba(tableColorType)) {
            SkOpts::RGB_to_RGB1(colorTable + numColorsWithAlpha, (const uint8_t*)palette, numColors - numColorsWithAlpha);
        } else {
            SkOpts::RGB_to_BGR1(colorTable + numColorsWithAlpha, (const uint8_t*)palette, numColors - numColorsWithAlpha);
        }
    }
