...
static bool get_attr_float3_as_list(const SkDOM* dom, const SkDOM::Node* node, const std::string& prefix, const std::string& key, SkColor4f* outValue)
{
    ...
    for (const auto* liNode = dom->getFirstChild(seq, "rdf:li"); liNode; liNode = dom->getNextSibling(liNode, "rdf:li")) {
        if (count > 2) {
...
static const SkDOM::Node* find_uri_namespaces(const SkDOM& dom, size_t count, const char* uris[], const char* outNamespaces[])
{
    ...
    for (const auto* rdf = dom.getFirstChild(root, kRdf); rdf; rdf = dom.getNextSibling(rdf, kRdf)) {
        ...
        for (const auto* desc = dom.getFirstChild(rdf, kDesc); desc; desc = dom.getNextSibling(desc, kDesc)) {
...
private:
    bool findUriNamespaces(size_t count, const char* uris[], const char* outNamespaces[], const SkDOM** outDom, const SkDOM::Node** outNode) const;
...
bool SkXmpImpl::findUriNamespaces(size_t count, const char* uris[], const char* outNamespaces[], const SkDOM** outDom, const SkDOM::Node** outNode) const
{
...
bool SkXmpImpl::getContainerGainmapLocation(size_t* outOffset, size_t* outSize) const
{
    ...
    const char* uris[2] = { "http://ns.google.com/photos/1.0/container/", "http://ns.google.com/photos/1.0/container/item/" };
    ...
    size_t offset = 0;
    for (const auto* li = dom->getFirstChild(seq, "rdf:li"); li; li = dom->getNextSibling(li, "rdf:li")) {
...
bool SkXmpImpl::getGainmapInfoApple(float exifHdrHeadroom, SkGainmapInfo* info) const
{
    // Find a node that matches the requested namespaces and URIs.
    const char* namespaces[2] = { nullptr, nullptr };
    const char* uris[2] = { "http://ns.apple.com/pixeldatainfo/1.0/", "http://ns.apple.com/HDRGainMap/1.0/" };
    const SkDOM* dom = nullptr;
    const SkDOM::Node* node = nullptr;
    if (!findUriNamespaces(2, uris, namespaces, &dom, &node)) {
        return false;
...
bool SkXmpImpl::getGainmapInfoAdobe(SkGainmapInfo* outGainmapInfo) const
{
    ...
    outGainmapInfo->fGainmapRatioMin = { std::exp(gainMapMin.fR * kLog2), std::exp(gainMapMin.fG * kLog2), std::exp(gainMapMin.fB * kLog2), 1.f };
    outGainmapInfo->fGainmapRatioMax = { std::exp(gainMapMax.fR * kLog2), std::exp(gainMapMax.fG * kLog2), std::exp(gainMapMax.fB * kLog2), 1.f };
