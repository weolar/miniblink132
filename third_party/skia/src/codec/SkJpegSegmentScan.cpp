...
sk_sp<SkData> SkJpegSegmentScanner::GetParameters(const SkData* scannedData, const SkJpegSegment& segment)
{
    return SkData::MakeSubset(
        scannedData, segment.offset + kJpegMarkerCodeSize + kJpegSegmentParameterLengthSize, segment.parameterLength - kJpegSegmentParameterLengthSize);
}
...
void SkJpegSegmentScanner::onBytes(const void* data, size_t size)
{
    ...
    while (bytesRemaining > 0) {
        ...
        case State::kEntropyCodedData: {
            ...
            const uint8_t* sentinel = reinterpret_cast<const uint8_t*>(memchr(bytes, 0xFF, bytesRemaining));
            if (sentinel) {
...
{
    SkASSERT(fState == State::kStartOfImageByte1 || fState == State::kSecondMarkerByte1 || fState == State::kEntropyCodedDataSentinel
