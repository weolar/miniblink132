...
static inline bool process_data(png_structp png_ptr, png_infop info_ptr, SkStream* stream, void* buffer, size_t bufferSize, size_t length)
{
    while (length > 0) {
...
bool SkPngCodec::processData()
{
    ...
    while (true) {
        ...
        // Process the full chunk + CRC.
        if (!process_data(fPng_ptr, fInfo_ptr, this->stream(), buffer, kBufferSize, length + 4) || iend) {
            break;
        }
    }
...
std::unique_ptr<SkEncodedInfo::ICCProfile> read_color_profile(png_structp png_ptr, png_infop info_ptr)
{
    ...
    if (png_get_cHRM_fixed(png_ptr, info_ptr, &chrm[0], &chrm[1], &chrm[2], &chrm[3], &chrm[4], &chrm[5], &chrm[6], &chrm[7])) {
...
public:
    SkPngNormalDecoder(SkEncodedInfo&& info, std::unique_ptr<SkStream> stream, SkPngChunkReader* reader, png_structp png_ptr, png_infop info_ptr)
...
public:
    SkPngInterlacedDecoder(
        SkEncodedInfo&& info, std::unique_ptr<SkStream> stream, SkPngChunkReader* reader, png_structp png_ptr, png_infop info_ptr, int numberPasses)
        : SkPngCodec(std::move(info), std::move(stream), reader, png_ptr, info_ptr)
    ...
    void interlacedRowCallback(png_bytep row, int rowNum, int pass)
    {
        ...
        if (0 == pass) {
            ...
            if (fNumberPasses - 1 == pass && rowNum == fLastRow) {
                ...
                if (fLastRow != this->dimensions().height() - 1 || (this->swizzler() && this->swizzler()->sampleY() != 1)) {
...
void AutoCleanPng::infoCallback(size_t idatLength)
{
    ...
    png_get_IHDR(fPng_ptr, fInfo_ptr, &origWidth, &origHeight, &bitDepth, &encodedColorType, nullptr, nullptr, nullptr);

    // TODO(https://crbug.com/359245096): Should we support 16-bits of precision
    // for gray images?
    if (bitDepth == 16 && (PNG_COLOR_TYPE_GRAY == encodedColorType || PNG_COLOR_TYPE_GRAY_ALPHA == encodedColorType)) {
        bitDepth = 8;
    ...
    case PNG_COLOR_TYPE_PALETTE:
        ...
        alpha = png_get_valid(fPng_ptr, fInfo_ptr, PNG_INFO_tRNS) ? SkEncodedInfo::kUnpremul_Alpha : SkEncodedInfo::kOpaque_Alpha;
        break;
    ...
    if (fOutCodec) {
        ...
        SkEncodedInfo encodedInfo = SkEncodedInfo::Make(origWidth, origHeight, color, alpha, bitDepth, std::move(profile));
        if (1 == numberPasses) {
            *fOutCodec = new SkPngNormalDecoder(std::move(encodedInfo), std::unique_ptr<SkStream>(fStream), fChunkReader, fPng_ptr, fInfo_ptr);
        } else {
            *fOutCodec
                = new SkPngInterlacedDecoder(std::move(encodedInfo), std::unique_ptr<SkStream>(fStream), fChunkReader, fPng_ptr, fInfo_ptr, numberPasses);
        }
...
SkPngCodec::SkPngCodec(SkEncodedInfo&& encodedInfo, std::unique_ptr<SkStream> stream, SkPngChunkReader* chunkReader, void* png_ptr, void* info_ptr)
...
bool SkPngCodec::onRewind()
{
    ...
    png_structp png_ptr;
    png_infop info_ptr;
    if (kSuccess != read_header(this->stream(), fPngChunkReader.get(), nullptr, &png_ptr, &info_ptr)) {
        return false;
    }
...
SkCodec::Result SkPngCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst, size_t rowBytes, const Options& options, int* rowsDecoded)
{
...
SkCodec::Result SkPngCodec::onStartIncrementalDecode(const SkImageInfo& dstInfo, void* dst, size_t rowBytes, const SkCodec::Options& options)
{
    Result result = this->initializeXforms(dstInfo, options);
    if (kSuccess != result) {
        return result;
...
std::unique_ptr<SkCodec> SkPngCodec::MakeFromStream(std::unique_ptr<SkStream> stream, Result* result, SkPngChunkReader* chunkReader)
{
    SkASSERT(result);
    if (!stream) {
