...
SkJpegxlCodec::SkJpegxlCodec(std::unique_ptr<SkJpegxlCodecPriv> codec, SkEncodedInfo&& info, std::unique_ptr<SkStream> stream, sk_sp<SkData> data)
...
std::unique_ptr<SkCodec> SkJpegxlCodec::MakeFromStream(std::unique_ptr<SkStream> stream, Result* result)
{
    SkASSERT(result);
    if (!stream) {
    ...
    *result = kInternalError;
    ...
    SkEncodedInfo::Alpha alpha = hasAlpha ? SkEncodedInfo::kUnpremul_Alpha : SkEncodedInfo::kOpaque_Alpha;
    ...
    if (iccSize) {
        ...
        status = JxlDecoderGetColorAsICCProfile(dec,
            /* format = */ nullptr, JXL_COLOR_PROFILE_TARGET_DATA, reinterpret_cast<uint8_t*>(icc->writable_data()), iccSize);
        if (status != JXL_DEC_SUCCESS) {
    ...
    *result = kSuccess;
    SkEncodedInfo encodedInfo = SkEncodedInfo::Make(width, height, color, alpha, bitsPerChannel, std::move(profile));

    return std::unique_ptr<SkCodec>(new SkJpegxlCodec(std::move(priv), std::move(encodedInfo), std::move(stream), std::move(data)));
}

SkCodec::Result SkJpegxlCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst, size_t rowBytes, const Options& options, int* rowsDecodedPtr)
{
    ...
    if (fCodec->fDstColorType == kRGBA_F16_SkColorType)
        halfFloatOutput = true;
    if (colorXform())
        halfFloatOutput = true;
    auto dataType = halfFloatOutput ? JXL_TYPE_FLOAT16 : JXL_TYPE_UINT8;

    JxlPixelFormat format = { numColorChannels + numAlphaChannels, dataType, endianness, /* align = */ 0 };
...
bool SkJpegxlCodec::conversionSupported(const SkImageInfo& dstInfo, bool srcIsOpaque, bool needsColorXform)
{
...
void SkJpegxlCodec::imageOutCallback(void* opaque, size_t x, size_t y, size_t num_pixels, const void* pixels)
{
...
bool SkJpegxlCodec::scanFrames()
{
    ...
    auto alpha = (info.alpha_bits != 0) ? SkEncodedInfo::Alpha::kUnpremul_Alpha : SkEncodedInfo::Alpha::kOpaque_Alpha;
    ...
    while (true) {
        ...
        case JXL_DEC_FRAME: {
            ...
            int duration = (1000 * frameHeader.duration * info.animation.tps_denominator) / info.animation.tps_numerator;
