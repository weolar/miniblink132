...
std::unique_ptr<SkCodec> SkCodec::MakeFromStream(
    std::unique_ptr<SkStream> stream, Result* outResult, SkPngChunkReader* chunkReader, SelectionPolicy selectionPolicy)
{
    return MakeFromStream(std::move(stream), SkCodecs::get_decoders(), outResult, chunkReader, selectionPolicy);
}
std::unique_ptr<SkCodec> SkCodec::MakeFromStream(std::unique_ptr<SkStream> stream, SkSpan<const SkCodecs::Decoder> decoders, Result* outResult,
    SkPngChunkReader* chunkReader, SelectionPolicy selectionPolicy)
{
    ...
    if (selectionPolicy != SelectionPolicy::kPreferStillImage && selectionPolicy != SelectionPolicy::kPreferAnimation) {
...
std::unique_ptr<SkCodec> SkCodec::MakeFromData(sk_sp<SkData> data, SkSpan<const SkCodecs::Decoder> decoders, SkPngChunkReader* reader)
{
    if (!data) {
        return nullptr;
    }
...
SkCodec::SkCodec(SkEncodedInfo&& info, XformFormat srcFormat, std::unique_ptr<SkStream> stream, SkEncodedOrigin origin)
...
bool SkCodec::queryYUVAInfo(const SkYUVAPixmapInfo::SupportedDataTypes& supportedDataTypes, SkYUVAPixmapInfo* yuvaPixmapInfo) const
{
    if (!yuvaPixmapInfo) {
        return false;
    }
...
SkCodec::Result SkCodec::handleFrameIndex(const SkImageInfo& info, void* pixels, size_t rowBytes, const Options& options, GetPixelsCallback getPixelsFn)
{
    if (getPixelsFn) {
    ...
    if (0 == index) {
        return this->initializeColorXform(info, fEncodedInfo.alpha(), fEncodedInfo.opaque()) ? kSuccess : kInvalidConversion;
    }
    ...
    return this->initializeColorXform(info, frame->reportedAlpha(), !frame->hasAlpha()) ? kSuccess : kInvalidConversion;
}

SkCodec::Result SkCodec::getPixels(const SkImageInfo& info, void* pixels, size_t rowBytes, const Options* options)
{
    ...
    if ((kIncompleteInput == result || kErrorInInput == result) && rowsDecoded != info.height()) {
        ...
        this->fillIncompleteImage(info, pixels, rowBytes, options->fZeroInitialized, info.height(), rowsDecoded);
    }

    return result;
}

std::tuple<sk_sp<SkImage>, SkCodec::Result> SkCodec::getImage(const SkImageInfo& info, const Options* options)
{
    SkBitmap bm;
...
SkCodec::Result SkCodec::startIncrementalDecode(const SkImageInfo& info, void* pixels, size_t rowBytes, const SkCodec::Options* options)
{
    fStartedIncrementalDecode = false;
...
SkCodec::Result SkCodec::startScanlineDecode(const SkImageInfo& info, const SkCodec::Options* options)
{
...
int SkCodec::getScanlines(void* dst, int countLines, size_t rowBytes)
{
    ...
    if (linesDecoded < countLines) {
        this->fillIncompleteImage(this->dstInfo(), dst, rowBytes, this->options().fZeroInitialized, countLines, linesDecoded);
    }
...
void SkCodec::fillIncompleteImage(const SkImageInfo& info, void* dst, size_t rowBytes, ZeroInitialized zeroInit, int linesRequested, int linesDecoded)
{
    ...
    const int fillWidth = sampler ? sampler->fillWidth() : fOptions.fSubset ? fOptions.fSubset->width() : info.width();
    void* fillDst = this->getScanlineOrder() == kBottomUp_SkScanlineOrder ? dst : SkTAddOffset<void>(dst, linesDecoded * rowBytes);
...
bool SkCodec::initializeColorXform(const SkImageInfo& dstInfo, SkEncodedInfo::Alpha encodedAlpha, bool srcIsOpaque)
{
    fXformTime = kNo_XformTime;
    bool needsColorXform = false;
    if (this->usesColorXform()) {
        if (kRGBA_F16_SkColorType == dstInfo.colorType() || kBGR_101010x_XR_SkColorType == dstInfo.colorType()) {
    ...
    if (needsColorXform) {
        fXformTime
            = SkEncodedInfo::kPalette_Color != fEncodedInfo.color() || kRGBA_F16_SkColorType == dstInfo.colorType() ? kDecodeRow_XformTime : kPalette_XformTime;
        if (!sk_select_xform_format(dstInfo.colorType(), fXformTime == kPalette_XformTime, &fDstXformFormat)) {
            return false;
        }
        if (encodedAlpha == SkEncodedInfo::kUnpremul_Alpha && dstInfo.alphaType() == kPremul_SkAlphaType) {
...
void SkCodec::applyColorXform(void* dst, const void* src, int count) const
{
    ...
    const auto* srcProfile = fEncodedInfo.profile();
    SkAssertResult(
        skcms_Transform(src, fSrcXformFormat, skcms_AlphaFormat_Unpremul, srcProfile, dst, fDstXformFormat, fDstXformAlphaFormat, &fDstProfile, count));
}
