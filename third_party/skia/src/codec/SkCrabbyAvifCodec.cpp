...
template <typename NumeratorType> float FractionToFloat(NumeratorType numerator, uint32_t denominator)
{
...
bool PopulateGainmapInfo(const crabbyavif::avifGainMap& gain_map, SkGainmapInfo* info)
{
    ...
    info->fBaseImageType = base_is_hdr ? SkGainmapInfo::BaseImageType::kHDR : SkGainmapInfo::BaseImageType::kSDR;
    for (int i = 0; i < 3; ++i) {
        ...
        info->fGainmapGamma[i] = FractionToFloat(gain_map.gainMapGamma[i].d, gain_map.gainMapGamma[i].n);
...
std::unique_ptr<SkCodec> SkCrabbyAvifCodec::MakeFromStream(std::unique_ptr<SkStream> stream, Result* result, bool gainmapOnly /*=false*/)
{
    SkASSERT(result);
    if (!stream) {
...
std::unique_ptr<SkCodec> SkCrabbyAvifCodec::MakeFromData(std::unique_ptr<SkStream> stream, sk_sp<SkData> data, Result* result, bool gainmapOnly /*=false*/)
{
    SkASSERT(result);
    ...
    crabbyavif::avifResult res = crabbyavif::avifDecoderSetIOMemory(avifDecoder.get(), data->bytes(), data->size());
    ...
    crabbyavif::avifImage* image = gainmapOnly ? avifDecoder->image->gainMap->image : avifDecoder->image;
    auto width = image->width;
    auto height = image->height;
    if (image->transformFlags & crabbyavif::AVIF_TRANSFORM_CLAP) {
        crabbyavif::avifCropRect rect;
        if (crabbyavif::crabby_avifCropRectConvertCleanApertureBox(&rect, &image->clap, width, height, image->yuvFormat, nullptr)) {
            width = rect.width;
            height = rect.height;
        }
    }
    SkEncodedInfo info = SkEncodedInfo::Make(width, height, color, alpha, bitsPerComponent, std::move(profile), image->depth);
    bool animation = avifDecoder->imageCount > 1;
    *result = kSuccess;
    return std::unique_ptr<SkCodec>(
        new SkCrabbyAvifCodec(std::move(info), std::move(stream), std::move(data), std::move(avifDecoder), kDefault_SkEncodedOrigin, animation, gainmapOnly));
}

SkCrabbyAvifCodec::SkCrabbyAvifCodec(SkEncodedInfo&& info, std::unique_ptr<SkStream> stream, sk_sp<SkData> data, AvifDecoder avifDecoder,
    SkEncodedOrigin origin, bool useAnimation, bool gainmapOnly)
...
int SkCrabbyAvifCodec::onGetFrameCount()
{
    ...
    if (fFrameHolder.size() == 0) {
        ...
        for (int i = 0; i < fAvifDecoder->imageCount; i++) {
            Frame* frame = fFrameHolder.appendNewFrame(fAvifDecoder->alphaPresent == crabbyavif::CRABBY_AVIF_TRUE);
...
bool SkCrabbyAvifCodec::conversionSupported(const SkImageInfo& dstInfo, bool srcIsOpaque, bool needsColorXform)
{
    return dstInfo.colorType() == kRGBA_8888_SkColorType || dstInfo.colorType() == kRGBA_1010102_SkColorType || dstInfo.colorType() == kRGBA_F16_SkColorType;
}

SkCodec::Result SkCrabbyAvifCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst, size_t dstRowBytes, const Options& options, int* rowsDecoded)
{
    if (options.fSubset) {
        return kUnimplemented;
    }

    crabbyavif::avifResult result = crabbyavif::avifDecoderNthImage(fAvifDecoder.get(), options.fFrameIndex);
    ...
    crabbyavif::avifImage* image = fGainmapOnly ? fAvifDecoder->image->gainMap->image : fAvifDecoder->image;
    if (this->dimensions() != dstInfo.dimensions()) {
        result = crabbyavif::avifImageScale(image, dstInfo.width(), dstInfo.height(), &fAvifDecoder->diag);
        if (result != crabbyavif::AVIF_RESULT_OK) {
            return kInvalidInput;
        }
    }

    using AvifImagePtr = std::unique_ptr<crabbyavif::avifImage, decltype(&crabbyavif::crabby_avifImageDestroy)>;
    ...
    if (image->transformFlags & crabbyavif::AVIF_TRANSFORM_CLAP) {
        crabbyavif::avifCropRect rect;
        if (crabbyavif::crabby_avifCropRectConvertCleanApertureBox(&rect, &image->clap, image->width, image->height, image->yuvFormat, nullptr)) {
...
bool SkCrabbyAvifCodec::onGetGainmapCodec(SkGainmapInfo* info, std::unique_ptr<SkCodec>* gainmapCodec)
{
    if (!gainmapCodec || !info || !fAvifDecoder->image || !fAvifDecoder->image->gainMap || !PopulateGainmapInfo(*fAvifDecoder->image->gainMap, info)) {
        return false;
    }
    Result result;
...
{
    return SkCrabbyAvifCodec::IsAvif(data, len);
}

std::unique_ptr<SkCodec> Decode(std::unique_ptr<SkStream> stream, SkCodec::Result* outResult, SkCodecs::DecodeContext)
{
...
std::unique_ptr<SkCodec> Decode(sk_sp<SkData> data, SkCodec::Result* outResult, SkCodecs::DecodeContext)
{
    if (!data) {
        if (outResult) {
