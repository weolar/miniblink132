...
#define FAIL_IF_COIN(cond, coin)                                                                                                                               \
    do {                                                                                                                                                       \
        if (cond)                                                                                                                                              \
            log->record(SkPathOpsDebug::kFail_Glitch, coin);                                                                                                   \
    } while (false)
...
#define FAIL_WITH_NULL_IF(cond, span)                                                                                                                          \
    do {                                                                                                                                                       \
        if (cond)                                                                                                                                              \
            log->record(SkPathOpsDebug::kFail_Glitch, span);                                                                                                   \
    } while (false)
...
bool SkPathOpsDebug::ChaseContains(const SkTDArray<SkOpSpanBase*>& chaseArray, const SkOpSpanBase* span)
{
...
struct SkPathOpsDebug::GlitchLog {
    ...
    void record(GlitchType type, const SkCoincidentSpans* coin, const SkCoincidentSpans* opp = nullptr)
    {
        SpanGlitch* glitch = recordCommon(type);
    ...
    void record(GlitchType type, const SkOpSpanBase* base, const SkOpSegment* seg, double t, SkPoint pt)
    {
        SpanGlitch* glitch = recordCommon(type);
        glitch->fBase = base;
        glitch->fSegment = seg;
    ...
    void record(GlitchType type, const SkCoincidentSpans* coin, const SkOpPtT* coinSpan, const SkOpPtT* endSpan)
    {
        SpanGlitch* glitch = recordCommon(type);
        glitch->fCoinSpan = coin->coinPtTStart();
        glitch->fEndSpan = coin->coinPtTEnd();
    ...
    void record(GlitchType type, const SkOpPtT* ptTS, const SkOpPtT* ptTE, const SkOpPtT* oPtTS, const SkOpPtT* oPtTE)
    {
        SpanGlitch* glitch = recordCommon(type);
    ...
    void record(GlitchType type, const SkOpSegment* seg, double startT, double endT, const SkOpSegment* oppSeg, double oppStartT, double oppEndT)
    {
        SpanGlitch* glitch = recordCommon(type);
        glitch->fSegment = seg;
...
void SkPathOpsDebug::CheckHealth(SkOpContourHead* contourList)
{
    ...
    for (int index = 0; index < glitches.fGlitches.size(); ++index) {
        ...
        if (glitch.fSuspect) {
            SkDebugf(" seg/base=%d/%d", glitch.fSuspect->segment()->debugID(), glitch.fSuspect->debugID());
        }
        if (glitch.fSegment) {
            SkDebugf(" segment=%d", glitch.fSegment->debugID());
        }
        if (glitch.fCoinSpan) {
            SkDebugf(" coinSeg/Span/PtT=%d/%d/%d", glitch.fCoinSpan->segment()->debugID(), glitch.fCoinSpan->span()->debugID(), glitch.fCoinSpan->debugID());
        }
        if (glitch.fEndSpan) {
            SkDebugf(" endSpan=%d", glitch.fEndSpan->debugID());
        }
        if (glitch.fOppSpan) {
            SkDebugf(" oppSeg/Span/PtT=%d/%d/%d", glitch.fOppSpan->segment()->debugID(), glitch.fOppSpan->span()->debugID(), glitch.fOppSpan->debugID());
        }
...
void SkPathOpsDebug::ShowPath(const SkPath& a, const SkPath& b, SkPathOp shapeOp, const char* testName)
{
...
void SkOpGlobalState::debugAddLoopCount(SkIntersections* i, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn)
{
    for (int index = 0; index < (int)std::size(fDebugLoopCount); ++index) {
        ...
        memcpy(&fDebugWorstPts[index * 2 * 4], wt.pts(), (SkPathOpsVerbToPoints(wt.segment()->verb()) + 1) * sizeof(SkPoint));
        memcpy(&fDebugWorstPts[(index * 2 + 1) * 4], wn.pts(), (SkPathOpsVerbToPoints(wn.segment()->verb()) + 1) * sizeof(SkPoint));
...
void SkOpGlobalState::debugDoYourWorst(SkOpGlobalState* local)
{
    for (int index = 0; index < (int)std::size(fDebugLoopCount); ++index) {
        ...
        memcpy(&fDebugWorstPts[index * 2 * 4], &local->fDebugWorstPts[index * 2 * 4], sizeof(SkPoint) * 8);
...
void SkOpGlobalState::debugLoopReport()
{
    ...
    for (int index = 0; index < (int)std::size(fDebugLoopCount); ++index) {
        ...
        dump_curve(fDebugWorstVerb[index * 2], fDebugWorstPts[index * 2 * 4], fDebugWorstWeight[index * 2]);
        dump_curve(fDebugWorstVerb[index * 2 + 1], fDebugWorstPts[(index * 2 + 1) * 4], fDebugWorstWeight[index * 2 + 1]);
    }
}
...
{
    if (span->windValue())
        glitches->record(SkPathOpsDebug::kCollapsedWindValue_Glitch, span);
    if (span->oppValue())
        glitches->record(SkPathOpsDebug::kCollapsedOppValue_Glitch, span);
    if (!span->done())
        glitches->record(SkPathOpsDebug::kCollapsedDone_Glitch, span);
}
#endif
    ...
    do {
        ...
        while ((ptT = ptT->next()) != spanStopPtT) {
            ...
            if (span && span->segment() != opp
                && span->containsCoincidence(opp)) { // debug has additional condition since it may be called before inner duplicate points have been deleted
                continue;
            }
            if (spanBase->segment() != opp
                && spanBase->containsCoinEnd(opp)) { // debug has additional condition since it may be called before inner duplicate points have been deleted
                continue;
            }
...
void SkOpSegment::debugShowNewWinding(const char* fun, const SkOpSpan* span, int winding)
{
    ...
    SkDebugf(") t=%1.9g [%d] (%1.9g,%1.9g) tEnd=%1.9g newWindSum=", span->t(), span->debugID(), pt.fX, pt.fY, span->next()->t());
...
void SkOpSegment::debugShowNewWinding(const char* fun, const SkOpSpan* span, int winding, int oppWinding)
{
    const SkPoint& pt = span->ptT()->fPt;
    SkDebugf("%s id=%d", fun, this->debugID());
    SkDebugf(" (%1.9g,%1.9g", fPts[0].fX, fPts[0].fY);
...
void SkOpAngle::debugCheckNearCoincidence() const
{
    ...
    do {
        ...
        while ((next = next->fNext) != this) {
            ...
            SkDebugf("%s distSq=%1.9g testId=%d nextId=%d\n", __FUNCTION__, distSq, testSegment->debugID(), nextSegment->debugID());
...
SkString SkOpAngle::debugPart() const
{
    ...
    case SkPath::kLine_Verb:
        result.printf(LINE_DEBUG_STR " id=%d", LINE_DEBUG_DATA(fPart.fCurve), this->segment()->debugID());
        break;
    case SkPath::kQuad_Verb:
        result.printf(QUAD_DEBUG_STR " id=%d", QUAD_DEBUG_DATA(fPart.fCurve), this->segment()->debugID());
        break;
    case SkPath::kConic_Verb:
        result.printf(CONIC_DEBUG_STR " id=%d", CONIC_DEBUG_DATA(fPart.fCurve, fPart.fCurve.fConic.fWeight), this->segment()->debugID());
        break;
    case SkPath::kCubic_Verb:
        result.printf(CUBIC_DEBUG_STR " id=%d", CUBIC_DEBUG_DATA(fPart.fCurve), this->segment()->debugID());
        break;
    default:
        SkASSERT(0);
    }
...
void SkOpAngle::debugValidate() const
{
    ...
    do {
        ...
        SkASSERT(!DEBUG_LIMIT_WIND_SUM || between(-DEBUG_LIMIT_WIND_SUM, minSpan->oppValue(), DEBUG_LIMIT_WIND_SUM));
...
#ifdef SK_DEBUG
void SkCoincidentSpans::debugStartCheck(const SkOpSpanBase* outer, const SkOpSpanBase* over, const SkOpGlobalState* debugState) const
{
...
void SkCoincidentSpans::debugCorrectOneEnd(
    SkPathOpsDebug::GlitchLog* log, const SkOpPtT* (SkCoincidentSpans::*getEnd)() const, void (SkCoincidentSpans::*setEnd)(const SkOpPtT* ptT) const) const
{
...
bool SkCoincidentSpans::debugExpand(SkPathOpsDebug::GlitchLog* log) const
{
    ...
    do {
        ...
        if (log)
            log->record(SkPathOpsDebug::kExpandCoin_Glitch, this, prev->ptT(), oppPtT);
        expanded = true;
    ...
    do {
        ...
        if (log)
            log->record(SkPathOpsDebug::kExpandCoin_Glitch, this, next->ptT(), oppPtT);
        expanded = true;
    } while (false); // actual continues while expansion is possible
    return expanded;
}
    ...
    do {
        ...
        while (test != end || oTest != oEnd) {
            ...
            if (!containedOpp || !containedThis) {
                ...
                addToOpp ? log->record(SkPathOpsDebug::kAddExpandedCoin_Glitch, oPriorT + oStartRange * startPart, test)
                         : log->record(SkPathOpsDebug::kAddExpandedCoin_Glitch, priorT + startRange * oStartPart, oTest);
...
void SkOpCoincidence::debugAddIfMissing(SkPathOpsDebug::GlitchLog* log, const SkOpPtT* over1s, const SkOpPtT* over2s, double tStart, double tEnd,
    const SkOpSegment* coinSeg, const SkOpSegment* oppSeg, bool* added, const SkOpPtT* over1e, const SkOpPtT* over2e) const
{
...
void SkOpCoincidence::debugAddOrOverlap(SkPathOpsDebug::GlitchLog* log, const SkOpSegment* coinSeg, const SkOpSegment* oppSeg, double coinTs, double coinTe,
    double oppTs, double oppTe, bool* added) const
{
    ...
    if (fHead && !this->checkOverlap(fHead, coinSeg, oppSeg, coinTs, coinTe, oppTs, oppTe, &overlaps)) {
        return;
    }
    ...
    for (int index = 1; index < overlaps.size(); ++index) { // combine overlaps before continuing
        ...
        if (overlap->flipped() ? overlap->oppPtTStart()->fT < test->oppPtTStart()->fT : overlap->oppPtTStart()->fT > test->oppPtTStart()->fT) {
            log->record(SkPathOpsDebug::kAddOrOverlap_Glitch, overlap, test->oppPtTStart());
        }
        if (overlap->flipped() ? overlap->oppPtTEnd()->fT > test->oppPtTEnd()->fT : overlap->oppPtTEnd()->fT < test->oppPtTEnd()->fT) {
    ...
    RETURN_FALSE_IF(csExisting && (csExisting == ce || csExisting->contains(ceExisting ? ceExisting : ce)), coinSeg);
    RETURN_FALSE_IF(ceExisting && (ceExisting == cs || ceExisting->contains(csExisting ? csExisting : cs)), coinSeg);
    ...
    RETURN_FALSE_IF(osExisting && (osExisting == oe || osExisting->contains(oeExisting ? oeExisting : oe)), oppSeg);
    RETURN_FALSE_IF(oeExisting && (oeExisting == os || oeExisting->contains(osExisting ? osExisting : os)), oppSeg);
    ...
    if (!cs || !os) {
        ...
        if (cs && os)
            cs->span()->debugAddOpp(log, os->span());
        //         cs = csWritable;
    ...
    if (!ce || !oe) {
        ...
        if (ce && oe)
            ce->span()->debugAddOpp(log, oe->span());
        //         ce = ceWritable;
        //         oe = oeWritable;
    }
    this->debugValidate();
            ...
            if (outerCoin == innerCoin) {
                ...
                if (outerOpp != innerOpp && this->overlap(ocs, oce, ics, ice, &overS, &overE)) {
                    this->debugAddIfMissing(
                        log, ocs->starter(oce), ics->starter(ice), overS, overE, outerOpp, innerOpp, added, ocs->debugEnder(oce), ics->debugEnder(ice));
                }
                ...
                if (outerOpp != innerCoin && this->overlap(ocs, oce, ios, ioe, &overS, &overE)) {
                    this->debugAddIfMissing(
                        log, ocs->starter(oce), ios->starter(ioe), overS, overE, outerOpp, innerCoin, added, ocs->debugEnder(oce), ios->debugEnder(ioe));
                }
                ...
                if (this->overlap(oos, ooe, ics, ice, &overS, &overE)) {
                    this->debugAddIfMissing(
                        log, oos->starter(ooe), ics->starter(ice), overS, overE, outerCoin, innerOpp, added, oos->debugEnder(ooe), ics->debugEnder(ice));
                }
                ...
                if (this->overlap(oos, ooe, ios, ioe, &overS, &overE)) {
                    this->debugAddIfMissing(
                        log, oos->starter(ooe), ios->starter(ioe), overS, overE, outerCoin, innerCoin, added, oos->debugEnder(ooe), ios->debugEnder(ioe));
                }
            }
            this->debugValidate();
        }
...
void SkOpCoincidence::debugRelease(SkPathOpsDebug::GlitchLog* log, const SkOpSegment* deleted) const
{
    ...
    do {
        if (coin->coinPtTStart()->segment() == deleted || coin->coinPtTEnd()->segment() == deleted || coin->oppPtTStart()->segment() == deleted
        ...
        if (coin->debugExpand(log)) {
            ...
            do {
                ...
                if (coin->coinPtTStart() == test->coinPtTStart() && coin->oppPtTStart() == test->oppPtTStart()) {
                    if (log)
                        log->record(SkPathOpsDebug::kExpandCoin_Glitch, fHead, test->coinPtTStart());
                    break;
                }
...
{
    SkDebugf("coinSpan - id=%d t=%1.9g tEnd=%1.9g\n", coinPtTStart()->segment()->debugID(), coinPtTStart()->fT, coinPtTEnd()->fT);
    SkDebugf("coinSpan + id=%d t=%1.9g tEnd=%1.9g\n", oppPtTStart()->segment()->debugID(), oppPtTStart()->fT, oppPtTEnd()->fT);
}
...
static void DebugCheckBetween(
    const SkOpSpanBase* next, const SkOpSpanBase* end, double oStart, double oEnd, const SkOpSegment* oSegment, SkPathOpsDebug::GlitchLog* log)
{
    SkASSERT(next != end);
...
static void DebugCheckOverlap(const SkCoincidentSpans* test, const SkCoincidentSpans* list, SkPathOpsDebug::GlitchLog* log)
{
    if (!list) {
        return;
    }
...
static void DebugCheckOverlapTop(const SkCoincidentSpans* head, const SkCoincidentSpans* opt, SkPathOpsDebug::GlitchLog* log)
{
...
static void DebugValidate(const SkCoincidentSpans* head, const SkCoincidentSpans* opt, SkPathOpsDebug::GlitchLog* log)
{
    // look for pts inside coincident spans that are not inside the opposite spans
    const SkCoincidentSpans* coin = head;
    while (coin) {
        SkASSERT(SkOpCoincidence::Ordered(coin->coinPtTStart()->segment(), coin->oppPtTStart()->segment()));
...
#if DEBUG_COIN
static void DebugCheckBetween(const SkCoincidentSpans* head, const SkCoincidentSpans* opt, SkPathOpsDebug::GlitchLog* log)
{
    // look for pts inside coincident spans that are not inside the opposite spans
    const SkCoincidentSpans* coin = head;
    while (coin) {
        DebugCheckBetween(
            coin->coinPtTStart()->span(), coin->coinPtTEnd()->span(), coin->oppPtTStart()->fT, coin->oppPtTEnd()->fT, coin->oppPtTStart()->segment(), log);
        DebugCheckBetween(
            coin->oppPtTStart()->span(), coin->oppPtTEnd()->span(), coin->coinPtTStart()->fT, coin->coinPtTEnd()->fT, coin->coinPtTStart()->segment(), log);
        coin = coin->next();
    }
    DebugCheckOverlapTop(head, opt, log);
}
    ...
    if (rectCount > 0) {
        ...
        for (int contour = 0; contour < rectCount; ++contour) {
            ...
            SkDebugf("path.addRect(%1.9g, %1.9g, %1.9g, %1.9g, %s);\n", rect.fLeft, rect.fTop, rect.fRight, rect.fBottom,
                directions[contour] == SkPathDirection::kCCW ? "SkPathDirection::kCCW" : "SkPathDirection::kCW");
        }
        return;
    }
#endif
...
static void debug_scale_matrix(const SkPath& one, const SkPath* two, SkMatrix& scale)
{
    ...
    SkScalar dx = -16000 > larger.fLeft ? -16000 - larger.fLeft : 16000 < larger.fRight ? 16000 - larger.fRight : 0;
    SkScalar dy = -16000 > larger.fTop ? -16000 - larger.fTop : 16000 < larger.fBottom ? 16000 - larger.fBottom : 0;
...
static int debug_paths_draw_the_same(const SkPath& one, const SkPath& two, SkBitmap& bits)
{
    ...
    for (int y = 0; y < bitHeight - 1; ++y) {
        ...
        for (int x = 0; x < bitWidth - 1; ++x) {
            ...
            if (err) {
                errors += addr1[x + 1] != addr3[x + 1] && addr2[x] != addr4[x] && addr2[x + 1] != addr4[x + 1];
            }
        }
    }
    return errors;
