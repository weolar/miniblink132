...
{
    return dy > 0 ? Contour::Direction::kCCW : dy < 0 ? Contour::Direction::kCW : Contour::Direction::kNone;
}
...
bool AsWinding(const SkPath& path, SkPath* result)
{
    ...
    if (std::all_of(sorted.fChildren.begin(), sorted.fChildren.end(), [](const Contour* contour) -> bool { return contour->fChildren.empty(); })) {
        return set_result_path(result, path, fillType);
    }
