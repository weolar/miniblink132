    ...
    while (!chase.empty()) {
        ...
        *endPtr = nullptr;
        ...
        while ((angle = angle->next()) != firstAngle) {
            ...
            if (sortable) {
                segment->setUpWindings(start, end, &sumMiWinding, &sumSuWinding, &maxWinding, &sumWinding, &oppMaxWinding, &oppSumWinding);
            }
            if (!segment->done(angle)) {
                ...
                if (sortable) {
                    if (!segment->markAngle(maxWinding, sumWinding, oppMaxWinding, oppSumWinding, angle, nullptr)) {
                        return false;
                    }
                }
            }
        ...
        do {
            if (current->activeOp(start, end, xorMask, xorOpMask, op)) {
                do {
                    ...
                    lastSimple = simple;
                    SkOpSegment* next = current->findNextOp(&chase, &nextStart, &nextEnd, &unsortable, &simple, op, xorMask, xorOpMask);
                    if (!next) {
                        if (!unsortable && writer->hasMove() && current->verb() != SkPath::kLine_Verb && !writer->isClosed()) {
...
#if DEBUG_FLOW
                    SkDebugf("%s current id=%d from=(%1.9g,%1.9g) to=(%1.9g,%1.9g)\n", __FUNCTION__, current->debugID(), start->pt().fX, start->pt().fY,
                        end->pt().fX, end->pt().fY);
#endif
                    if (!current->addCurveTo(start, end, writer)) {
                        return false;
...
bool OpDebug(const SkPath& one, const SkPath& two, SkPathOp op, SkPath* result SkDEBUGPARAMS(bool skipAssert) SkDEBUGPARAMS(const char* testName))
{
    ...
    SkPathFillType fillType = inverseFill ? SkPathFillType::kInverseEvenOdd : SkPathFillType::kEvenOdd;
    SkRect rect1, rect2;
    ...
    SkOpGlobalState globalState(contourList, &allocator SkDEBUGPARAMS(skipAssert) SkDEBUGPARAMS(testName));
    ...
    if (!SortContourList(&contourList, xorMask == kEvenOdd_PathOpsMask, xorOpMask == kEvenOdd_PathOpsMask)) {
        result->reset();
        result->setFillType(fillType);
        return true;
    }
