...
public:
    ...
    LineConicIntersections(const SkDConic& c)
        : fConic(c) SkDEBUGPARAMS(fLine(nullptr)) SkDEBUGPARAMS(fIntersections(nullptr)) SkDEBUGPARAMS(fAllowNear(false))
    {
    }
...
#endif
            SkDPoint pt;
            if (this->pinTs(&conicT, &lineT, &pt, kPointUninitialized) && this->uniqueAnswer(conicT, pt)) {
                fIntersections->insert(conicT, lineT, pt);
            }
        }
        this->checkCoincident();
...
protected:
    ...
    void addLineNearEndPoints()
    {
        for (int lIndex = 0; lIndex < 2; ++lIndex) {
            ...
            double conicT = ((const SkDCurve*)&fConic)->nearPoint(SkPath::kConic_Verb, (*fLine)[lIndex], (*fLine)[!lIndex]);
            if (conicT < 0) {
                continue;
            }
