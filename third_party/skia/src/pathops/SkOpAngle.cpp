...
#if DEBUG_ANGLE
static bool CompareResult(const char* func, SkString* bugOut, SkString* bugPart, int append, bool compare)
{
...
#define COMPARE_RESULT(append, compare) CompareResult(__FUNCTION__, &bugOut, bugPart, append, compare)
#else
...
bool SkOpAngle::after(SkOpAngle* test)
{
    ...
    bugOut.printf("%s [%d/%d] %d/%d tStart=%1.9g tEnd=%1.9g"
                  ...
                  " < [%d/%d] %d/%d tStart=%1.9g tEnd=%1.9g ",
        __FUNCTION__, lh->segment()->debugID(), lh->debugID(), lh->fSectorStart, lh->fSectorEnd, lh->fStart->t(), lh->fEnd->t(), segment()->debugID(),
        debugID(), fSectorStart, fSectorEnd, fStart->t(), fEnd->t(), rh->segment()->debugID(), rh->debugID(), rh->fSectorStart, rh->fSectorEnd, rh->fStart->t(),
        rh->fEnd->t());
...
int SkOpAngle::lineOnOneSide(const SkDPoint& origin, const SkDVector& line, const SkOpAngle* test, bool useOriginal) const
{
    double crosses[3];
...
bool SkOpAngle::computeSector()
{
    ...
    do {
        ...
        checkEnd = stepUp ? !checkEnd->final() ? checkEnd->upCast()->next() : nullptr : checkEnd->prev();
    } while (checkEnd);
recomputeSector:
...
bool SkOpAngle::endsIntersect(SkOpAngle* rh)
{
    ...
    SkDLine rays[] = { { { this->fPart.fCurve[0], rh->fPart.fCurve[rPts] } }, { { this->fPart.fCurve[0], this->fPart.fCurve[lPts] } } };
    ...
    for (int index = 0; index < 2; ++index) {
        ...
        // in the bug.
        if (delta < 4e-3 && delta > 1e-3 && !useIntersect && fPart.isCurve() && rh->fPart.isCurve() && fOriginalCurvePart[0] != fPart.fCurve.fLine[0]) {
...
bool SkOpAngle::endToSide(const SkOpAngle* rh, bool* inside) const
{
    ...
    rayEnd[1] = rayEnd[0];
    SkDVector slopeAtEnd = (*CurveDSlopeAtT[verb])(segment->pts(), segment->weight(), this->fEnd->t());
...
void SkOpAngle::setSpans()
{
    ...
    SkDEBUGCODE(fPart.fCurve[2].fX = fPart.fCurve[2].fY = fPart.fCurve[3].fX = fPart.fCurve[3].fY = SK_ScalarNaN); //  make the non-line part uninitialized
...
void SkOpAngle::setSector()
{
    ...
    if (fSectorEnd == fSectorStart && (fSectorStart & 3) != 3) { // if the sector has no span, it can't be an exact angle
        fSectorMask = 1 << fSectorStart;
        return;
    }
