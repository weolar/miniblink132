...
{
    SkDVector result = { conic_eval_tan(&fPts[0].fX, fWeight, t), conic_eval_tan(&fPts[0].fY, fWeight, t) };
...
SkDPoint SkDConic::ptAtT(double t) const
{
    ...
    SkDPoint result = { sk_ieee_double_divide(conic_eval_numerator(&fPts[0].fX, fWeight, t), denominator),
...
SkDConic SkDConic::subDivide(double t1, double t2) const
{
    ...
    SkDConic dst = { { { { ax / az, ay / az }, { bx / bz, by / bz }, { cx / cz, cy / cz } } SkDEBUGPARAMS(fPts.fDebugGlobalState) },
        SkDoubleToScalar(bz / sqrt(az * cz)) };
    return dst;
}

SkDPoint SkDConic::subDivide(const SkDPoint& a, const SkDPoint& c, double t1, double t2, SkScalar* weight) const
{
