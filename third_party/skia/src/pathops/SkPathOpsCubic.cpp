...
double SkDCubic::binarySearch(double min, double max, double axisIntercept, SearchAxis xAxis) const
{
    ...
    do {
        ...
        if (approximately_equal_half(lessPt.fX, cubicAtT.fX) && approximately_equal_half(lessPt.fY, cubicAtT.fY)) {
        ...
        if (calcDist > 0 ? calcDist > lessDist : calcDist < lessDist) {
            ...
            if (approximately_equal_half(morePt.fX, cubicAtT.fX) && approximately_equal_half(morePt.fY, cubicAtT.fY)) {
                return -1; // binary search found no point at this axis intercept
            }
...
{
    return ((fPts[1] - fPts[0]).length() + (fPts[2] - fPts[1]).length() + (fPts[3] - fPts[2]).length()) / gPrecisionUnit;
}
...
{
    return (between(fPts[0].fX, fPts[1].fX, fPts[3].fX) && between(fPts[0].fX, fPts[2].fX, fPts[3].fX))
        || (between(fPts[0].fY, fPts[1].fY, fPts[3].fY) && between(fPts[0].fY, fPts[2].fY, fPts[3].fY));
}
...
bool SkDCubic::isLinear(int startIndex, int endIndex) const
{
    ...
    double tiniest = std::min(
        std::min(std::min(std::min(std::min(std::min(std::min(fPts[0].fX, fPts[0].fY), fPts[1].fX), fPts[1].fY), fPts[2].fX), fPts[2].fY), fPts[3].fX),
        fPts[3].fY);
    double largest = std::max(
        std::max(std::max(std::max(std::max(std::max(std::max(fPts[0].fX, fPts[0].fY), fPts[1].fX), fPts[1].fY), fPts[2].fX), fPts[2].fY), fPts[3].fX),
        fPts[3].fY);
    ...
    case SkCubicType::kCuspAtInfinity: {
        ...
        if (infTCount == 2) {
            ...
            for (int index = 0; index < roots; ++index) {
                ...
                SkDVector dPt = { derivative_at_t(&cubic.fPts[0].fX, testT), derivative_at_t(&cubic.fPts[0].fY, testT) };
...
{
    return precisely_between(fPts[0].fX, fPts[1].fX, fPts[3].fX) && precisely_between(fPts[0].fX, fPts[2].fX, fPts[3].fX);
}

bool SkDCubic::monotonicInY() const
{
    return precisely_between(fPts[0].fY, fPts[1].fY, fPts[3].fY) && precisely_between(fPts[0].fY, fPts[2].fY, fPts[3].fY);
}
...
int SkDCubic::searchRoots(double extremeTs[6], int extrema, double axisIntercept, SearchAxis xAxis, double* validRoots) const
{
...
int SkDCubic::RootsReal(double A, double B, double C, double D, double s[3])
{
    ...
    snprintf(str, sizeof(str), "Solve[%1.19g x^3 + %1.19g x^2 + %1.19g x + %1.19g == 0, x]", A, B, C, D);
    SkPathOpsDebug::MathematicaIze(str, sizeof(str));
    SkDebugf("%s\n", str);
#endif
#endif
    if (approximately_zero(A) && approximately_zero_when_compared_to(A, B) && approximately_zero_when_compared_to(A, C)
        && approximately_zero_when_compared_to(A, D)) { // we're just a quadratic
        return SkDQuad::RootsReal(B, C, D, s);
    }
    if (approximately_zero_when_compared_to(D, A) && approximately_zero_when_compared_to(D, B) && approximately_zero_when_compared_to(D, C)) { // 0 is one root
...
SkDPoint SkDCubic::ptAtT(double t) const
{
    ...
    double c = 3 * one_t * t2;
    double d = t2 * t;
    SkDPoint result = { a * fPts[0].fX + b * fPts[1].fX + c * fPts[2].fX + d * fPts[3].fX, a * fPts[0].fY + b * fPts[1].fY + c * fPts[2].fY + d * fPts[3].fY };
    return result;
}
...
void SkDCubic::subDivide(const SkDPoint& a, const SkDPoint& d, double t1, double t2, SkDPoint dst[2]) const
{
    SkASSERT(t1 != t2);
