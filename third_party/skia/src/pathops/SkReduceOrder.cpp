...
static int check_quadratic(const SkDCubic& cubic, SkDCubic& reduction)
{
    ...
    if (approximately_zero(sideAx) ? !approximately_equal(sideAx, sideBx) : !AlmostEqualUlps_Pin(sideAx, sideBx)) {
        return 0;
    }
    ...
    if (approximately_zero(sideAy) ? !approximately_equal(sideAy, sideBy) : !AlmostEqualUlps_Pin(sideAy, sideBy)) {
        return 0;
    }
    reduction[0] = cubic[0];
    reduction[1].fX = midX;
...
int SkReduceOrder::reduce(const SkDCubic& cubic, Quadratics allowQuadratics)
{
    ...
    for (index = 0; index < 4; ++index) {
        ...
        double denom = std::max(fabs(cx), std::max(fabs(cy), std::max(fabs(cubic[minX].fX), fabs(cubic[minY].fY))));
        if (denom == 0) {
            minXSet |= 1 << index;
            minYSet |= 1 << index;
            continue;
    ...
    if (allowQuadratics == SkReduceOrder::kAllow_Quadratics && (result = check_quadratic(cubic, fCubic))) {
        return result;
    }
    fCubic = cubic;
    return 4;
...
{
    if (SkDPoint::ApproximatelyEqual(a[0], a[1]) && SkDPoint::ApproximatelyEqual(a[0], a[2]) && SkDPoint::ApproximatelyEqual(a[0], a[3])) {
        reducePts[0] = a[0];
