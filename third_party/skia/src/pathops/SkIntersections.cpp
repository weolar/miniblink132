...
int SkIntersections::insert(double one, double two, const SkDPoint& pt)
{
    ...
    for (index = 0; index < fUsed; ++index) {
        ...
        if (more_roughly_equal(oldOne, one) && more_roughly_equal(oldTwo, two)) {
            if ((!precisely_zero(one) || precisely_zero(oldOne)) && (!precisely_equal(one, 1) || precisely_equal(oldOne, 1))
                && (!precisely_zero(two) || precisely_zero(oldTwo)) && (!precisely_equal(two, 1) || precisely_equal(oldTwo, 1))) {
                return -1;
            }
