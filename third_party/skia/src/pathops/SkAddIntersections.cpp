...
static void debugShowLineIntersection(int pts, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn, const SkIntersections& i)
{
    SkASSERT(i.used() == pts);
    if (!pts) {
    ...
    SkDebugf("%s " T_DEBUG_STR(wtTs, 0) " " LINE_DEBUG_STR " " PT_DEBUG_STR, __FUNCTION__, i[0][0], LINE_DEBUG_DATA(wt.pts()), PT_DEBUG_DATA(i, 0));
    if (pts == 2) {
...
static void debugShowQuadLineIntersection(int pts, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn, const SkIntersections& i)
{
    SkASSERT(i.used() == pts);
    if (!pts) {
    ...
    SkDebugf("%s " T_DEBUG_STR(wtTs, 0) " " QUAD_DEBUG_STR " " PT_DEBUG_STR, __FUNCTION__, i[0][0], QUAD_DEBUG_DATA(wt.pts()), PT_DEBUG_DATA(i, 0));
    for (int n = 1; n < pts; ++n) {
...
static void debugShowQuadIntersection(int pts, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn, const SkIntersections& i)
{
    SkASSERT(i.used() == pts);
    if (!pts) {
...
static void debugShowConicLineIntersection(int pts, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn, const SkIntersections& i)
{
    SkASSERT(i.used() == pts);
    if (!pts) {
    ...
    SkDebugf(
        "%s " T_DEBUG_STR(wtTs, 0) " " CONIC_DEBUG_STR " " PT_DEBUG_STR, __FUNCTION__, i[0][0], CONIC_DEBUG_DATA(wt.pts(), wt.weight()), PT_DEBUG_DATA(i, 0));
    for (int n = 1; n < pts; ++n) {
        SkDebugf(" " TX_DEBUG_STR(wtTs) " " PT_DEBUG_STR, n, i[0][n], PT_DEBUG_DATA(i, n));
    }
    SkDebugf(" wnTs[0]=%g " LINE_DEBUG_STR, i[1][0], LINE_DEBUG_DATA(wn.pts()));
...
static void debugShowConicQuadIntersection(int pts, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn, const SkIntersections& i)
{
    SkASSERT(i.used() == pts);
    if (!pts) {
...
static void debugShowConicIntersection(int pts, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn, const SkIntersections& i)
{
    SkASSERT(i.used() == pts);
    if (!pts) {
...
static void debugShowCubicLineIntersection(int pts, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn, const SkIntersections& i)
{
    SkASSERT(i.used() == pts);
    if (!pts) {
    ...
    SkDebugf("%s " T_DEBUG_STR(wtTs, 0) " " CUBIC_DEBUG_STR " " PT_DEBUG_STR, __FUNCTION__, i[0][0], CUBIC_DEBUG_DATA(wt.pts()), PT_DEBUG_DATA(i, 0));
    for (int n = 1; n < pts; ++n) {
        SkDebugf(" " TX_DEBUG_STR(wtTs) " " PT_DEBUG_STR, n, i[0][n], PT_DEBUG_DATA(i, n));
    }
    SkDebugf(" wnTs[0]=%g " LINE_DEBUG_STR, i[1][0], LINE_DEBUG_DATA(wn.pts()));
...
static void debugShowCubicQuadIntersection(int pts, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn, const SkIntersections& i)
{
    SkASSERT(i.used() == pts);
    if (!pts) {
...
static void debugShowCubicConicIntersection(int pts, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn, const SkIntersections& i)
{
    SkASSERT(i.used() == pts);
    if (!pts) {
        SkDebugf(
...
static void debugShowCubicIntersection(int pts, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn, const SkIntersections& i)
{
    SkASSERT(i.used() == pts);
    if (!pts) {
        ...
        do {
            ...
            case SkIntersectionHelper::kHorizontalLine_Segment:
                ...
                case SkIntersectionHelper::kConic_Segment:
                    pts = ts.conicHorizontal(wn.pts(), wn.weight(), wt.left(), wt.right(), wt.y(), wt.xFlipped());
            ...
            case SkIntersectionHelper::kVerticalLine_Segment:
                ...
                case SkIntersectionHelper::kConic_Segment: {
                    pts = ts.conicVertical(wn.pts(), wn.weight(), wt.top(), wt.bottom(), wt.x(), wt.yFlipped());
                    debugShowConicLineIntersection(pts, wn, wt, ts);
                    break;
                }
                case SkIntersectionHelper::kCubic_Segment: {
            ...
            case SkIntersectionHelper::kConic_Segment:
                switch (wn.segmentType()) {
                case SkIntersectionHelper::kHorizontalLine_Segment:
                    pts = ts.conicHorizontal(wt.pts(), wt.weight(), wn.left(), wn.right(), wn.y(), wn.xFlipped());
                    debugShowConicLineIntersection(pts, wt, wn, ts);
                    break;
                case SkIntersectionHelper::kVerticalLine_Segment:
                    pts = ts.conicVertical(wt.pts(), wt.weight(), wn.top(), wn.bottom(), wn.x(), wn.yFlipped());
                    debugShowConicLineIntersection(pts, wt, wn, ts);
                    break;
                case SkIntersectionHelper::kLine_Segment:
                ...
                case SkIntersectionHelper::kConic_Segment: {
                    pts = ts.intersect(conic1.set(wt.pts(), wt.weight()), conic2.set(wn.pts(), wn.weight()));
                    debugShowConicIntersection(pts, wt, wn, ts);
                    break;
                }
                case SkIntersectionHelper::kCubic_Segment: {
                    swap = true;
                    pts = ts.intersect(cubic2.set(wn.pts() SkDEBUGPARAMS(ts.globalState())), conic1.set(wt.pts(), wt.weight() SkDEBUGPARAMS(ts.globalState())));
            ...
            case SkIntersectionHelper::kCubic_Segment:
                ...
                case SkIntersectionHelper::kConic_Segment: {
                    pts = ts.intersect(cubic1.set(wt.pts() SkDEBUGPARAMS(ts.globalState())), conic2.set(wn.pts(), wn.weight() SkDEBUGPARAMS(ts.globalState())));
            ...
            for (int pt = 0; pt < pts; ++pt) {
                ...
                SkOpPtT* testTAt = iPtIsIntegral ? wt.segment()->addT(ts[swap][pt], iPt) : wt.segment()->addT(ts[swap][pt]);
                wn.segment()->debugValidate();
                SkOpPtT* nextTAt = iPtIsIntegral ? wn.segment()->addT(ts[!swap][pt], iPt) : wn.segment()->addT(ts[!swap][pt]);
                ...
                SkASSERT(coincidence->globalState()->debugSkipAssert() || coinPtT[0]->span()->t() < testTAt->span()->t());
