...
public:
    ...
    LineQuadraticIntersections(const SkDQuad& q)
        : fQuad(q) SkDEBUGPARAMS(fLine(nullptr)) SkDEBUGPARAMS(fIntersections(nullptr)) SkDEBUGPARAMS(fAllowNear(false))
    {
    }
...
protected:
    ...
    void addLineNearEndPoints()
    {
        for (int lIndex = 0; lIndex < 2; ++lIndex) {
            ...
            double quadT = ((const SkDCurve*)&fQuad)->nearPoint(SkPath::kQuad_Verb, (*fLine)[lIndex], (*fLine)[!lIndex]);
            if (quadT < 0) {
                continue;
            }
...
int SkIntersections::horizontal(const SkDQuad& quad, double left, double right, double y, bool flipped)
{
...
int SkIntersections::vertical(const SkDQuad& quad, double top, double bottom, double x, bool flipped)
{
