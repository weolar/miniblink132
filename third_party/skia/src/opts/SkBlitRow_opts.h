...
static inline __m256i SkPMSrcOver_AVX2(const __m256i& src, const __m256i& dst)
{
    ...
    __m256i srcA_x2
        = _mm256_shuffle_epi8(src, _mm256_setr_epi8(3, _, 3, _, 7, _, 7, _, 11, _, 11, _, 15, _, 15, _, 3, _, 3, _, 7, _, 7, _, 11, _, 11, _, 15, _, 15, _));
...
inline void blit_row_s32a_opaque(SkPMColor* dst, const SkPMColor* src, int len, U8CPU alpha)
{
...
#if SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_AVX2
    while (len >= 8) {
        _mm256_storeu_si256((__m256i*)dst, SkPMSrcOver_AVX2(_mm256_loadu_si256((const __m256i*)src), _mm256_loadu_si256((const __m256i*)dst)));
        src += 8;
        dst += 8;
        len -= 8;
    }
...
#if SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_SSE2
    while (len >= 4) {
        _mm_storeu_si128((__m128i*)dst, SkPMSrcOver_SSE2(_mm_loadu_si128((const __m128i*)src), _mm_loadu_si128((const __m128i*)dst)));
        src += 4;
        dst += 4;
        len -= 4;
    }
...
#if defined(SK_ARM_HAS_NEON)
    while (len >= 8) {
        vst4_u8((uint8_t*)dst, SkPMSrcOver_neon8(vld4_u8((const uint8_t*)dst), vld4_u8((const uint8_t*)src)));
        src += 8;
        dst += 8;
        len -= 8;
    }
    ...
    while (len >= 2) {
        vst1_u8((uint8_t*)dst, SkPMSrcOver_neon2(vld1_u8((const uint8_t*)dst), vld1_u8((const uint8_t*)src)));
        src += 2;
        dst += 2;
        len -= 2;
    }
...
#if SK_CPU_LSX_LEVEL >= SK_CPU_LSX_LEVEL_LASX
    while (len >= 8) {
        __lasx_xvst(SkPMSrcOver_LASX(__lasx_xvld(src, 0), __lasx_xvld(dst, 0)), (__m256i*)dst, 0);
        src += 8;
        dst += 8;
        len -= 8;
    }
...
#if SK_CPU_LSX_LEVEL >= SK_CPU_LSX_LEVEL_LSX
    while (len >= 4) {
        __lsx_vst(SkPMSrcOver_LSX(__lsx_vld(src, 0), __lsx_vld(dst, 0)), (__m128i*)dst, 0);
        src += 4;
        dst += 4;
        len -= 4;
    }
...
inline void blit_row_color32(SkPMColor* dst, int count, SkPMColor color)
{
    ...
    auto kernel = [color](U32 src) {
        ...
        U16 c = skvx::cast<uint16_t>(sk_bit_cast<U8>(U32(color))), d = (mull(s, a) + (c << 8) + 128) >> 8;
