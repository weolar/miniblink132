...
public:
    ...
    // used by makeScaled()
    virtual sk_sp<SkSurface> onMakeSurface(skgpu::graphite::Recorder*, const SkImageInfo&) const = 0;

    virtual bool readPixelsGraphite(skgpu::graphite::Recorder*, const SkPixmap& dst, int srcX, int srcY) const
    {
        return false;
    }
     ...
     */
    virtual void onAsyncRescaleAndReadPixelsYUV420(SkYUVColorSpace, bool readAlpha, sk_sp<SkColorSpace> dstColorSpace, SkIRect srcRect, SkISize dstSize,
        RescaleGamma, RescaleMode, ReadPixelsCallback, ReadPixelsContext) const;
