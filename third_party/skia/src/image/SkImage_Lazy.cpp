...
{
    return fGenerator->getInfo();
}

bool SharedGenerator::isTextureGenerator()
{
    return fGenerator->isTextureGenerator();
}

///////////////////////////////////////////////////////////////////////////////

SkImage_Lazy::Validator::Validator(sk_sp<SharedGenerator> gen, const SkColorType* colorType, sk_sp<SkColorSpace> colorSpace)
...
bool SkImage_Lazy::getROPixels(GrDirectContext* ctx, SkBitmap* bitmap, SkImage::CachingHint chint) const
{
...
bool SkImage_Lazy::onReadPixels(
    GrDirectContext* dContext, const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB, int srcX, int srcY, CachingHint chint) const
{
    SkBitmap bm;
...
sk_sp<SkImage> SkImage_Lazy::onMakeSubset(skgpu::graphite::Recorder*, const SkIRect& subset, RequiredProperties props) const
{
...
sk_sp<SkSurface> SkImage_Lazy::onMakeSurface(skgpu::graphite::Recorder*, const SkImageInfo& info) const
{
...
sk_sp<SkImage> SkImage_Lazy::onMakeColorTypeAndColorSpace(SkColorType targetCT, sk_sp<SkColorSpace> targetCS, GrDirectContext*) const
{
...
sk_sp<SkCachedData> SkImage_Lazy::getPlanes(const SkYUVAPixmapInfo::SupportedDataTypes& supportedDataTypes, SkYUVAPixmaps* yuvaPixmaps) const
{
    ScopedGenerator generator(fSharedGenerator);
    ...
    if (!generator->queryYUVAInfo(supportedDataTypes, &yuvaPixmapInfo) || yuvaPixmapInfo.yuvaInfo().dimensions() != this->dimensions()) {
        return nullptr;
    }
    data.reset(SkResourceCache::NewCachedData(yuvaPixmapInfo.computeTotalBytes()));
    SkYUVAPixmaps tempPixmaps = SkYUVAPixmaps::FromExternalMemory(yuvaPixmapInfo, data->writable_data());
