...
SkImage_Raster::SkImage_Raster(const SkImageInfo& info, sk_sp<SkData> data, size_t rowBytes, uint32_t id)
...
SkImage_Raster::SkImage_Raster(const SkBitmap& bm, bool bitmapMayBeMutable)
    : SkImage_Base(bm.info(), is_not_subset(bm) ? bm.getGenerationID() : (uint32_t)kNeedNewImageUniqueID)
    , fBitmap(bm)
{
...
bool SkImage_Raster::onReadPixels(GrDirectContext*, const SkImageInfo& dstInfo, void* dstPixels, size_t dstRowBytes, int srcX, int srcY, CachingHint) const
{
...
sk_sp<SkSurface> SkImage_Raster::onMakeSurface(skgpu::graphite::Recorder*, const SkImageInfo& info) const
{
...
static SkBitmap copy_bitmap_subset(const SkBitmap& orig, const SkIRect& subset)
{
    ...
    SkRectMemcpy(dst, bitmap.rowBytes(), src, orig.rowBytes(), bitmap.rowBytes(), subset.height());
...
sk_sp<SkImage> SkImage_Raster::onMakeSubset(skgpu::graphite::Recorder*, const SkIRect& subset, RequiredProperties requiredProperties) const
{
    sk_sp<SkImage> img;
...
sk_sp<SkImage> SkImage_Raster::onMakeColorTypeAndColorSpace(SkColorType targetCT, sk_sp<SkColorSpace> targetCS, GrDirectContext*) const
{
    SkPixmap src;
