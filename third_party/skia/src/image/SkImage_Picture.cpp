...
sk_sp<SkImage> SkImage_Picture::Make(sk_sp<SkPicture> picture, const SkISize& dimensions, const SkMatrix* matrix, const SkPaint* paint,
    SkImages::BitDepth bitDepth, sk_sp<SkColorSpace> colorSpace, SkSurfaceProps props)
{
    auto gen = SkImageGenerators::MakeFromPicture(dimensions, std::move(picture), matrix, paint, bitDepth, std::move(colorSpace), props);
...
void SkImage_Picture::replay(SkCanvas* canvas) const
{
    ...
    canvas->drawPicture(pictureIG->fPicture, &pictureIG->fMatrix, pictureIG->fPaint.getMaybeNull());
}
