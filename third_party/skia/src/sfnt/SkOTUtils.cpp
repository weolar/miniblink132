...
SkData* SkOTUtils::RenameFont(SkStreamAsset* fontData, const char* fontName, int fontNameLen)
{
    ...
    size_t nameTableLogicalSize
        = sizeof(SkOTTableName) + (encodings.size() * names.size() * sizeof(SkOTTableName::Record)) + (fontNameLen * sizeof(SK_OT_USHORT));
...
sk_sp<SkOTUtils::LocalizedStrings_NameTable> SkOTUtils::LocalizedStrings_NameTable::Make(const SkTypeface& typeface, SK_OT_USHORT types[], int typesCount)
{
    ...
    return sk_sp<SkOTUtils::LocalizedStrings_NameTable>(new SkOTUtils::LocalizedStrings_NameTable(std::move(nameTableData), nameTableSize, types, typesCount));
}
...
{
    return Make(typeface, SkOTUtils::LocalizedStrings_NameTable::familyNameTypes, std::size(SkOTUtils::LocalizedStrings_NameTable::familyNameTypes));
}
...
void SkOTUtils::SetAdvancedTypefaceFlags(SkOTTableOS2_V4::Type fsType, SkAdvancedTypefaceMetrics* info)
{
    SkASSERT(info);
