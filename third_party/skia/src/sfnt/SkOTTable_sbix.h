...
struct SkOTTableStandardBitmapGraphics {
    ...
    {
        return sk_unaligned_load<SK_OT_ULONG>(SkTAddOffset<void*>(&numStrikes, sizeof(numStrikes) + sizeof(SK_OT_ULONG) * strikeIndex));
    }
    ...
    struct Strike {
        ...
        {
            return sk_unaligned_load<SK_OT_ULONG>(SkTAddOffset<void*>(&ppi, sizeof(ppi) + sizeof(SK_OT_ULONG) * glyphId));
        }
    };
    ...
    struct GlyphData {
        ...
        SK_OT_BYTE* data()
        {
            return SkTAfter<SK_OT_BYTE>(&graphicType);
        }
        const SK_OT_BYTE* data() const
        {
            return SkTAfter<const SK_OT_BYTE>(&graphicType);
        }
    };
};
