...
struct SkOTTableGlyph {
    ...
    public:
        ...
        {
            SkASSERT(locaFormat.value == SkOTTableHead::IndexToLocFormat::ShortOffsets || locaFormat.value == SkOTTableHead::IndexToLocFormat::LongOffsets);
        }
        ...
        SkOTTableGlyphData* next()
        {
            ...
            if (previousGlyphOffset == fCurrentGlyphOffset) {
                ...
                return reinterpret_cast<SkOTTableGlyphData*>(reinterpret_cast<SK_OT_BYTE*>(&fGlyf) + previousGlyphOffset);
            }
        }
...
struct SkOTTableGlyphData {
    ...
    struct Simple {
        ...
        union Flags {
            struct Field {
                SK_OT_BYTE_BITFIELD(
                    OnCurve, xShortVector, yShortVector, Repeat, xIsSame_xShortVectorPositive, yIsSame_yShortVectorPositive, Reserved6, Reserved7)
            } field;
            struct Raw {
    ...
    struct Composite {
        struct Component {
            union Flags {
                struct Field {
                    //8-15
                    SK_OT_BYTE_BITFIELD(WE_HAVE_INSTRUCTIONS, USE_MY_METRICS, OVERLAP_COMPOUND, SCALED_COMPONENT_OFFSET, UNSCALED_COMPONENT_OFFSET, Reserved13,
                        Reserved14, Reserved15)
                    //0-7
                    SK_OT_BYTE_BITFIELD(ARG_1_AND_2_ARE_WORDS, ARGS_ARE_XY_VALUES, ROUND_XY_TO_GRID, WE_HAVE_A_SCALE, RESERVED, MORE_COMPONENTS,
                        WE_HAVE_AN_X_AND_Y_SCALE, WE_HAVE_A_TWO_BY_TWO)
                } field;
                struct Raw {
