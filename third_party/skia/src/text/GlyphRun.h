...
class GlyphRun {
public:
    GlyphRun(const SkFont& font, SkSpan<const SkPoint> positions, SkSpan<const SkGlyphID> glyphIDs, SkSpan<const char> text, SkSpan<const uint32_t> clusters,
    ...
    {
        return fSource.get<1>();
    }
    SkSpan<const SkGlyphID> glyphsIDs() const
    {
        return fSource.get<0>();
    }
...
public:
    ...
    {
        return fSourceBounds;
    }
    SkRect sourceBoundsWithOrigin() const
    {
        return fSourceBounds.makeOffset(fOrigin);
    }
...
public:
    ...
    const GlyphRunList& textToGlyphRunList(
        const SkFont& font, const SkPaint& paint, const void* bytes, size_t byteLength, SkPoint origin, SkTextEncoding encoding = SkTextEncoding::kUTF8);
...
private:
    ...
    void makeGlyphRun(const SkFont& font, SkSpan<const SkGlyphID> glyphIDs, SkSpan<const SkPoint> positions, SkSpan<const char> text,
        SkSpan<const uint32_t> clusters, SkSpan<const SkVector> scaledRotations);

    const GlyphRunList& setGlyphRunList(const SkTextBlob* blob, const SkRect& bounds, SkPoint origin);
