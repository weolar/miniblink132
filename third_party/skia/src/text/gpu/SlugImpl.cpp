...
SlugImpl::SlugImpl(SubRunAllocator&& alloc, gpu::SubRunContainerOwner subRuns, SkRect sourceBounds, SkPoint origin)
...
sk_sp<Slug> SlugImpl::MakeFromBuffer(SkReadBuffer& buffer, const SkStrikeClient* client)
{
    ...
    auto [initializer, _, alloc] = SubRunAllocator::AllocateClassMemoryAndArena<SlugImpl>(allocSizeHint);
    ...
    return sk_sp<SlugImpl>(initializer.initialize(std::move(alloc), std::move(container), sourceBounds, origin));
}
...
sk_sp<SlugImpl> SlugImpl::Make(const SkMatrix& viewMatrix, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint, SkStrikeDeviceInfo strikeDeviceInfo,
    sktext::StrikeForGPUCacheInterface* strikeCache)
{
    size_t subRunSizeHint = gpu::SubRunContainer::EstimateAllocSize(glyphRunList);
    auto [initializer, _, alloc] = SubRunAllocator::AllocateClassMemoryAndArena<SlugImpl>(subRunSizeHint);

    const SkMatrix positionMatrix = position_matrix(viewMatrix, glyphRunList.origin());

    auto subRuns = gpu::SubRunContainer::MakeInAlloc(
        glyphRunList, positionMatrix, paint, strikeDeviceInfo, strikeCache, &alloc, gpu::SubRunContainer::kAddSubRuns, "Make Slug");

    sk_sp<SlugImpl> slug = sk_sp<SlugImpl>(initializer.initialize(std::move(alloc), std::move(subRuns), glyphRunList.sourceBounds(), glyphRunList.origin()));
