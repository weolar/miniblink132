...
std::tuple<bool, SkVector> can_use_direct(const SkMatrix& initialPositionMatrix, const SkMatrix& positionMatrix)
{
    ...
    return { initialPositionMatrix.getScaleX() == positionMatrix.getScaleX() && initialPositionMatrix.getScaleY() == positionMatrix.getScaleY()
            && initialPositionMatrix.getSkewX() == positionMatrix.getSkewX() && initialPositionMatrix.getSkewY() == positionMatrix.getSkewY()
...
static SkColor compute_canonical_color(const SkPaint& paint, bool lcd)
{
    ...
    if (lcd) {
        ...
        U8CPU lum = SkComputeLuminance(SkColorGetR(canonicalColor), SkColorGetG(canonicalColor), SkColorGetB(canonicalColor));
...
// -- TextBlob::Key ------------------------------------------------------------------------------
auto TextBlob::Key::Make(const GlyphRunList& glyphRunList, const SkPaint& paint, const SkMatrix& drawMatrix, const SkStrikeDeviceInfo& strikeDevice)
    -> std::tuple<bool, Key>
{
    ...
    bool canCache = glyphRunList.canCache() && !(paint.getPathEffect() || (maskFilter && !as_MFB(maskFilter)->asABlur(&blurRec)));
    ...
    if (canCache) {
        ...
        SkPixelGeometry pixelGeometry = hasLCD ? strikeDevice.fSurfaceProps.pixelGeometry() : kUnknown_SkPixelGeometry;
        ...
        for (auto& run : glyphRunList) {
            SkScalar approximateDeviceTextSize = SkFontPriv::ApproximateTransformedTextSize(run.font(), drawMatrix, glyphRunListLocation);
            key.fHasSomeDirectSubRuns |= strikeDevice.fSubRunControl->isDirect(approximateDeviceTextSize, paint, drawMatrix);
        }
        ...
        if (key.fHasSomeDirectSubRuns) {
            ...
            key.fPositionMatrix.setTranslateX(mappedOrigin.x() - SkScalarFloorToScalar(mappedOrigin.x()));
            key.fPositionMatrix.setTranslateY(mappedOrigin.y() - SkScalarFloorToScalar(mappedOrigin.y()));
        } else {
...
bool TextBlob::Key::operator==(const TextBlob::Key& that) const
{
    ...
    if (fPositionMatrix.hasPerspective() && fHasSomeDirectSubRuns) {
        return false;
    }
...
sk_sp<TextBlob> TextBlob::Make(const GlyphRunList& glyphRunList, const SkPaint& paint, const SkMatrix& positionMatrix, SkStrikeDeviceInfo strikeDeviceInfo,
    StrikeForGPUCacheInterface* strikeCache)
{
    size_t subRunSizeHint = SubRunContainer::EstimateAllocSize(glyphRunList);
    auto [initializer, totalMemoryAllocated, alloc] = SubRunAllocator::AllocateClassMemoryAndArena<TextBlob>(subRunSizeHint);

    auto container
        = SubRunContainer::MakeInAlloc(glyphRunList, positionMatrix, paint, strikeDeviceInfo, strikeCache, &alloc, SubRunContainer::kAddSubRuns, "TextBlob");

    SkColor initialLuminance = SkPaintPriv::ComputeLuminanceColor(paint);
    sk_sp<TextBlob> blob = sk_sp<TextBlob>(initializer.initialize(std::move(alloc), std::move(container), totalMemoryAllocated, initialLuminance));
    return blob;
}
...
bool TextBlob::canReuse(const SkPaint& paint, const SkMatrix& positionMatrix) const
{
    ...
    if (fKey.fCanonicalColor == SK_ColorTRANSPARENT && fInitialLuminance != SkPaintPriv::ComputeLuminanceColor(paint)) {
        return false;
    }
...
void TextBlob::draw(SkCanvas* canvas, SkPoint drawOrigin, const SkPaint& paint, const AtlasDrawDelegate& atlasDelegate)
{
    fSubRuns->draw(canvas, drawOrigin, paint, this, atlasDelegate);
}

TextBlob::TextBlob(SubRunAllocator&& alloc, SubRunContainerOwner subRuns, int totalMemorySize, SkColor initialLuminance)
...
sk_sp<Slug> MakeSlug(const SkMatrix& drawMatrix, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint, SkStrikeDeviceInfo strikeDeviceInfo,
