...
public:
    ...
    {
        return SkMaskFilterBase::Type::kSDF;
    }
...
bool SDFMaskFilterImpl::filterMask(SkMaskBuilder* dst, const SkMask& src, const SkMatrix& matrix, SkIPoint* margin) const
{
    if (src.fFormat != SkMask::kA8_Format && src.fFormat != SkMask::kBW_Format && src.fFormat != SkMask::kLCD16_Format) {
        return false;
    }
    ...
    *dst = SkMaskBuilder::PrepareDestination(SK_DistanceFieldPad, SK_DistanceFieldPad, src);
    ...
    if (src.fFormat == SkMask::kA8_Format) {
        return SkGenerateDistanceFieldFromA8Image(dst->image(), src.fImage, src.fBounds.width(), src.fBounds.height(), src.fRowBytes);
    } else if (src.fFormat == SkMask::kLCD16_Format) {
        return SkGenerateDistanceFieldFromLCD16Mask(dst->image(), src.fImage, src.fBounds.width(), src.fBounds.height(), src.fRowBytes);
    } else {
        return SkGenerateDistanceFieldFromBWImage(dst->image(), src.fImage, src.fBounds.width(), src.fBounds.height(), src.fRowBytes);
    }
}

void SDFMaskFilterImpl::computeFastBounds(const SkRect& src, SkRect* dst) const
{
    dst->setLTRB(src.fLeft - SK_DistanceFieldPad, src.fTop - SK_DistanceFieldPad, src.fRight + SK_DistanceFieldPad, src.fBottom + SK_DistanceFieldPad);
}
