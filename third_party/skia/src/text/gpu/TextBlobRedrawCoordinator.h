...
public:
    ...
    void drawGlyphRunList(SkCanvas* canvas, const SkMatrix& viewMatrix, const GlyphRunList& glyphRunList, const SkPaint& paint,
        SkStrikeDeviceInfo strikeDeviceInfo, const AtlasDrawDelegate&);
...
private:
    ...
    sk_sp<TextBlob> findOrCreateBlob(
        const SkMatrix& positionMatrix, const GlyphRunList& glyphRunList, const SkPaint& paint, SkStrikeDeviceInfo strikeDeviceInfo);

    // If not already in the cache, then add it else, return the text blob from the cache.
    sk_sp<TextBlob> addOrReturnExisting(const GlyphRunList& glyphRunList, sk_sp<TextBlob> blob) SK_EXCLUDES(fSpinLock);
    ...
    sk_sp<TextBlob> internalAdd(sk_sp<TextBlob> blob) SK_REQUIRES(fSpinLock);
