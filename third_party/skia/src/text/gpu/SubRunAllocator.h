...
public:
    ...
    static constexpr int MinimumSizeWithOverhead(int requestedSize, int assumedAlignment, int blockSize, int maxAlignment)
    {
        ...
        int minimumSize = SkToInt(AlignUp(requestedSize, minAlignment)) + blockSize + maxAlignment - minAlignment;
    ...
    template <int size> using Storage = std::array<char, PlatformMinimumSizeWithOverhead(size, 1)>;

    // Returns true if n * sizeof(T) will fit in an allocation block.
    template <typename T> static bool WillCountFit(int n)
    {
...
template <typename T> class SubRunInitializer {
public:
    ...
    template <typename... Args> T* initialize(Args&&... args)
    {
...
public:
    ...
    template <class T> inline static constexpr bool HasNoDestructor = std::is_trivially_destructible<T>::value;
    ...
    template <typename T> static std::tuple<SubRunInitializer<T>, int, SubRunAllocator> AllocateClassMemoryAndArena(int allocSizeHint)
    {
    ...
    template <typename T, typename... Args> std::unique_ptr<T, Destroyer> makeUnique(Args&&... args)
    {
    ...
    template <typename T> SkSpan<T> makePODSpan(SkSpan<const T> s)
    {
        static_assert(HasNoDestructor<T>, "This is not POD. Use makeUniqueArray.");
        if (s.empty()) {
            return SkSpan<T> {};
    ...
    template <typename T, typename Src, typename Map> SkSpan<T> makePODArray(const Src& src, Map map)
    {
        static_assert(HasNoDestructor<T>, "This is not POD. Use makeUniqueArray.");
    ...
    template <typename T> std::unique_ptr<T[], ArrayDestroyer> makeUniqueArray(int n)
    {
    ...
    template <typename T, typename I> std::unique_ptr<T[], ArrayDestroyer> makeUniqueArray(int n, I initializer)
    {
        static_assert(!HasNoDestructor<T>, "This is POD. Use makePODArray.");
        T* array = reinterpret_cast<T*>(fAlloc.template allocateBytesFor<T>(n));
        for (int i = 0; i < n; i++) {
    ...
    template <typename T, typename U, typename Map> std::unique_ptr<T[], ArrayDestroyer> makeUniqueArray(SkSpan<const U> src, Map map)
    {
        static_assert(!HasNoDestructor<T>, "This is POD. Use makePODArray.");
...
class STSubRunAllocator : private std::array<char, BagOfBytes::PlatformMinimumSizeWithOverhead(InlineStorageSize, InlineStorageAlignment)>,
