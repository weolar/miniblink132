...
public:
    ...
    RemoteStrike(const SkStrikeSpec& strikeSpec, std::unique_ptr<SkScalerContext> context, SkDiscardableHandleId discardableHandleId);
    ...
    SkDiscardableHandleId discardableHandleId() const
    {
        return fDiscardableHandleId;
    }
...
RemoteStrike::RemoteStrike(const SkStrikeSpec& strikeSpec, std::unique_ptr<SkScalerContext> context, uint32_t discardableHandleId)
...
private:
    ...
    using DescToRemoteStrike = std::unordered_map<const SkDescriptor*, sk_sp<RemoteStrike>, MapOps, MapOps>;
...
sk_sp<RemoteStrike> SkStrikeServerImpl::getOrCreateCache(const SkStrikeSpec& strikeSpec)
{
    ...
    if (auto it = fDescToRemoteStrike.find(&strikeSpec.descriptor()); it != fDescToRemoteStrike.end()) {
...
public:
    GlyphTrackingDevice(const SkISize& dimensions, const SkSurfaceProps& props, SkStrikeServerImpl* server, sk_sp<SkColorSpace> colorSpace,
        sktext::gpu::SubRunControl SubRunControl)
    ...
    {
        const SkSurfaceProps surfaceProps = this->surfaceProps().cloneWithPixelGeometry(cinfo.fPixelGeometry);

        return sk_make_sp<GlyphTrackingDevice>(cinfo.fInfo.dimensions(), surfaceProps, fStrikeServerImpl, cinfo.fInfo.refColorSpace(), fSubRunControl);
    }
...
protected:
    void onDrawGlyphRunList(SkCanvas*, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint) override
    {
        ...
        auto container = SubRunContainer::MakeInAlloc(
            glyphRunList, drawMatrix, paint, this->strikeDeviceInfo(), fStrikeServerImpl, &tempAlloc, SubRunContainer::kStrikeCalculationsOnly, "Cache Diff");
    ...
    sk_sp<sktext::gpu::Slug> convertGlyphRunListToSlug(const sktext::GlyphRunList& glyphRunList, const SkPaint& paint) override
    {
        ...
        return sktext::gpu::MakeSlug(this->localToDevice(), glyphRunList, paint, this->strikeDeviceInfo(), fStrikeServerImpl);
    }
...
std::unique_ptr<SkCanvas> SkStrikeServer::makeAnalysisCanvas(
    int width, int height, const SkSurfaceProps& props, sk_sp<SkColorSpace> colorSpace, bool DFTSupport, bool DFTPerspSupport)
{
    ...
    auto control
        = sktext::gpu::SubRunControl { DFTSupport, props.isUseDeviceIndependentFonts(), DFTPerspSupport, kMinDistanceFieldFontSize, kGlyphsAsPathsFontSize };
#else
    auto control = sktext::gpu::SubRunControl {};
#endif

    sk_sp<SkDevice> trackingDevice = sk_make_sp<GlyphTrackingDevice>(SkISize::Make(width, height), props, this->impl(), std::move(colorSpace), control);
...
public:
    ...
    {
        return fManager->deleteHandle(fDiscardableHandleId);
    }
    void assertValid() override
    {
        fManager->assertHandleValid(fDiscardableHandleId);
    }
...
public:
    explicit SkStrikeClientImpl(sk_sp<SkStrikeClient::DiscardableHandleManager>, bool isLogging = true, SkStrikeCache* strikeCache = nullptr);
...
private:
    ...
    public:
        PictureBackedGlyphDrawable(sk_sp<SkPicture> self)
            : fSelf(std::move(self))
        {
        }
...
SkStrikeClientImpl::SkStrikeClientImpl(sk_sp<SkStrikeClient::DiscardableHandleManager> discardableManager, bool isLogging, SkStrikeCache* strikeCache)
...
#define READ_FAILURE                                                                                                                                           \
    {                                                                                                                                                          \
        ...
        SkStrikeClient::DiscardableHandleManager::ReadFailureData data                                                                                         \
            = { memorySize, deserializer.bytesRead(), typefaceSize, strikeCount, glyphImagesCount, __LINE__ };                                                 \
...
bool SkStrikeClientImpl::readStrikeData(const volatile void* memory, size_t memorySize)
{
    ...
    int curTypeface = 0, curStrike = 0;

    auto postError = [&](int line) {
        SkDebugf("Read Error Posted %s : %d", __FILE__, line);
        SkStrikeClient::DiscardableHandleManager::ReadFailureData data { memorySize, buffer.offset(), SkTo<uint64_t>(curTypeface), SkTo<uint64_t>(curStrike),
            SkTo<uint64_t>(0), SkTo<uint64_t>(0) };
        fDiscardableHandleManager->notifyReadFailure(data);
    };
    ...
    for (curStrike = 0; curStrike < stirkeCount; ++curStrike) {
        ...
        if (strike == nullptr) {
            ...
            strike = fStrikeCache->createStrike(
                strikeSpec, &fontMetrics.value(), std::make_unique<DiscardableStrikePinner>(discardableHandleID, fDiscardableHandleManager));
        }
...
{
    sk_sp<SkTypeface>* typeface = fServerTypefaceIdToTypeface.find(typefaceProto.serverTypefaceID());
    ...
    auto newTypeface = sk_make_sp<SkTypefaceProxy>(typefaceProto, fDiscardableHandleManager, fIsLogging);
    fServerTypefaceIdToTypeface.set(typefaceProto.serverTypefaceID(), newTypeface);
    return newTypeface;
}

// SkStrikeClient ----------------------------------------------------------------------------------
SkStrikeClient::SkStrikeClient(sk_sp<DiscardableHandleManager> discardableManager, bool isLogging, SkStrikeCache* strikeCache)
