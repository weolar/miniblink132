...
namespace sktext {
// -- GlyphRun -------------------------------------------------------------------------------------
GlyphRun::GlyphRun(const SkFont& font, SkSpan<const SkPoint> positions, SkSpan<const SkGlyphID> glyphIDs, SkSpan<const char> text,
    SkSpan<const uint32_t> clusters, SkSpan<const SkVector> scaledRotations)
...
// -- GlyphRunList ---------------------------------------------------------------------------------
GlyphRunList::GlyphRunList(const SkTextBlob* blob, SkRect bounds, SkPoint origin, SkSpan<const GlyphRun> glyphRunList, GlyphRunBuilder* builder)
...
static SkRect glyphrun_source_bounds(
    const SkFont& font, const SkPaint& paint, SkZip<const SkGlyphID, const SkPoint> source, SkSpan<const SkVector> scaledRotations)
{
    ...
    if (fontBounds.isEmpty()) {
        ...
        if (scaledRotations.empty()) {
            ...
            auto scaleAndTranslateRect = [scale = strikeToSourceScale](const SkRect& in, const SkPoint& pos) {
                return SkRect::MakeLTRB(in.left() * scale + pos.x(), in.top() * scale + pos.y(), in.right() * scale + pos.x(), in.bottom() * scale + pos.y());
            };
            ...
            for (auto [pos, scaleRotate, glyph] : SkMakeZip(positions, scaledRotations, glyphs)) {
                if (!glyph->rect().isEmpty()) {
                    SkMatrix xform = SkMatrix().setRSXform(SkRSXform { pos.x(), pos.y(), scaleRotate.x(), scaleRotate.y() });
...
const GlyphRunList& GlyphRunBuilder::textToGlyphRunList(
    const SkFont& font, const SkPaint& paint, const void* bytes, size_t byteLength, SkPoint origin, SkTextEncoding encoding)
{
    ...
    if (!glyphIDs.empty()) {
        ...
        this->makeGlyphRun(font, glyphIDs, positions, SkSpan<const char> {}, SkSpan<const uint32_t> {}, SkSpan<const SkVector> {});
...
const GlyphRunList& sktext::GlyphRunBuilder::blobToGlyphRunList(const SkTextBlob& blob, SkPoint origin)
{
    ...
    for (SkTextBlobRunIterator it(&blob); !it.done(); it.next()) {
        ...
        this->makeGlyphRun(
            font, glyphIDs, positions, SkSpan<const char>(it.text(), it.textSize()), SkSpan<const uint32_t>(clusters, clusters ? runSize : 0), scaledRotations);
    }

    return this->setGlyphRunList(&blob, blob.bounds(), origin);
}

std::tuple<SkSpan<const SkPoint>, SkSpan<const SkVector>> GlyphRunBuilder::convertRSXForm(SkSpan<const SkRSXform> xforms)
{
...
SkSpan<const SkGlyphID> GlyphRunBuilder::textToGlyphIDs(const SkFont& font, const void* bytes, size_t byteLength, SkTextEncoding encoding)
{
...
void GlyphRunBuilder::makeGlyphRun(const SkFont& font, SkSpan<const SkGlyphID> glyphIDs, SkSpan<const SkPoint> positions, SkSpan<const char> text,
    SkSpan<const uint32_t> clusters, SkSpan<const SkVector> scaledRotations)
{
...
const GlyphRunList& sktext::GlyphRunBuilder::setGlyphRunList(const SkTextBlob* blob, const SkRect& bounds, SkPoint origin)
{
