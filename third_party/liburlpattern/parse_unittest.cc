...
{
    std::string output;
    std::transform(input.begin(), input.end(), std::back_inserter(output), [](unsigned char c) { return std::toupper(c); });
    return output;
}

void RunParseTest(std::string_view pattern, absl::StatusOr<std::vector<Part>> expected, EncodeCallback callback = PassThrough)
{
    auto result = Parse(pattern, std::move(callback));
    ASSERT_EQ(result.ok(), expected.ok()) << "parse status '" << result.status() << "' for: " << pattern;
    if (!expected.ok()) {
        ASSERT_EQ(result.status().code(), expected.status().code()) << "parse status code for: " << pattern;
        EXPECT_NE(result.status().message().find(expected.status().message()), std::string::npos)
            << "parse message '" << result.status().message() << "' does not contain '" << expected.status().message() << "' for: " << pattern;
        return;
    }
    const auto& expected_part_list = expected.value();
    const auto& part_list = result.value().PartList();
    EXPECT_EQ(part_list.size(), expected_part_list.size()) << "parser should produce expected number of parts for: " << pattern;
    for (size_t i = 0; i < part_list.size() && i < expected_part_list.size(); ++i) {
        EXPECT_EQ(part_list[i], expected_part_list[i]) << "token at index " << i << " wrong for: " << pattern;
    }
}
...
TEST(ParseTest, Regex)
{
    std::vector<Part> expected_parts = {
        ...
        Part(PartType::kRegex, /*name=*/"0", /*prefix=*/"", "oo", /*suffix=*/"", Modifier::kNone),
    };
    RunParseTest("/f(oo)", expected_parts);
}
...
TEST(ParseTest, RegexWithPrefixAndSuffixInGroup)
{
    std::vector<Part> expected_parts = {
        ...
        Part(PartType::kRegex, /*name=*/"0", /*prefix=*/"f", "o", /*suffix=*/"o", Modifier::kNone),
    };
...
{
    std::vector<Part> expected_parts = {
        Part(PartType::kRegex, /*name=*/"0", /*prefix=*/"/", "foo", /*suffix=*/"", Modifier::kNone),
    };
    RunParseTest("/(foo)", expected_parts);
}
...
{
    RunParseTest("/:foo*?", absl::InvalidArgumentError("expected end of pattern"));
}
