...
void EscapeStringAndAppendInternal(std::string_view input, std::string& append_target, std::string_view special_chars)
{
    for (auto& c : input) {
...
void EscapeRegexpStringAndAppend(std::string_view input, std::string& append_target)
{
    return EscapeStringAndAppendInternal(input, append_target, kRegexpSpecialCharacters);
}

void EscapePatternStringAndAppend(std::string_view input, std::string& append_target)
{
    return EscapeStringAndAppendInternal(input, append_target, kPatternSpecialCharacters);
}
...
bool IsNameCodepoint(UChar32 c, bool first_codepoint)
{
    ...
    return u_hasBinaryProperty(c, UCHAR_ID_CONTINUE) || c == '$' || c == '_' || c == 0x200c || c == 0x200d;
}
