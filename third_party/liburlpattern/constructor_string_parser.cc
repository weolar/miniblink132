...
ConstructorStringParser::ConstructorStringParser(std::string_view constructor_string)
    : input_(constructor_string)
{
}

absl::Status ConstructorStringParser::Parse(ProtocolCheckCallback protocol_matches_special_scheme)
{
    ...
    for (; token_index_ < token_list_.size(); token_index_ += token_increment_) {
        ...
        case StringParseState::kProtocol:
            ...
            if (IsProtocolSuffix()) {
                absl::StatusOr<bool> protocol_check_result = protocol_matches_special_scheme(MakeComponentString());
...
void ConstructorStringParser::ChangeState(StringParseState new_state, Skip skip)
{
    ...
    if (state_ != StringParseState::kInit && new_state != StringParseState::kDone) {
        ...
        if (state_ < StringParseState::kHostname && new_state > StringParseState::kHostname && !result_.hostname) {
            result_.hostname = "";
        }
        if (state_ < StringParseState::kPort && new_state > StringParseState::kPort && !result_.port) {
            result_.port = "";
        }
        if (state_ < StringParseState::kPathname && new_state > StringParseState::kPathname && !result_.pathname) {
            result_.pathname = should_treat_as_standard_url_ ? "/" : "";
        }
        if (state_ < StringParseState::kSearch && new_state > StringParseState::kSearch && !result_.search) {
            result_.search = "";
        }
    }
...
void ConstructorStringParser::ChangeStateWithoutSettingComponent(StringParseState new_state, Skip skip)
{
    state_ = new_state;
...
bool ConstructorStringParser::IsNonSpecialPatternChar(size_t index, const char* value) const
{
    const Token& token = SafeToken(index);
    return token.value == value && (token.type == TokenType::kChar || token.type == TokenType::kEscapedChar || token.type == TokenType::kInvalidChar);
}
...
{
    return IsNonSpecialPatternChar(token_index_ + 1, "/") && IsNonSpecialPatternChar(token_index_ + 2, "/");
}
...
bool ConstructorStringParser::IsSearchPrefix() const
{
    ...
    return previous_token.type != TokenType::kName && previous_token.type != TokenType::kRegex && previous_token.type != TokenType::kClose
...
std::string_view ConstructorStringParser::MakeComponentString() const
{
    ...
    ABSL_ASSERT(token.index < input_.size() || (token.index == input_.size() && token.type == TokenType::kEnd));
    return input_.substr(component_char_start, token.index - component_char_start);
}
