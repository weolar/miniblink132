...
void RunRegexTest(std::string_view input, std::string_view expected_regex, std::vector<std::string> expected_name_list, Options options = Options())
{
...
{
    RunRegexTest("/foo/([a-z]+)+", R"(^\/foo(?:\/((?:[a-z]+)(?:\/(?:[a-z]+))*))[\/#\?]?$)", { "0" });
}
...
{
    RunRegexTest("/foo/:bar+", R"(^\/foo(?:\/((?:[^\/#\?]+?)(?:\/(?:[^\/#\?]+?))*))$)", { "bar" }, { .strict = true });
}
...
{
    RunRegexTest("/foo/bar/", R"(^\/foo\/bar\/(?:[\/#\?](?=[]|$))?)", {}, { .end = false });
}

TEST(PatternRegexTest, StrictNoEndFixedWithTrailingDelimiter)
{
    RunRegexTest("/foo/bar/", R"(^\/foo\/bar\/)", {}, { .strict = true, .end = false });
}
...
{
    RunRegexTest("/foo/:bar+", R"(\/foo(?:\/((?:[^\/#\?]+?)(?:\/(?:[^\/#\?]+?))*))[\/#\?]?$)", { "bar" }, { .start = false });
}
...
void RunPatternStringTest(std::string_view input, std::string_view expected_pattern_string)
{
...
TEST(PatternStringTest, NamedGroupInGroupingWithOptionalModifierFollowedByWildcard)
{
    RunPatternStringTest("{:foo}?(.*)", ":foo?*");
}
...
TEST(PatternStringTest, NamedGroupFollowedByEmptyGroupWithOptionalModifierAndValidNameText)
{
    RunPatternStringTest(":foo{}?bar", "{:foo}bar");
}
...
TEST(PatternStringTest, WildcardFollowedByEmptyGroupWithZeroOrMoreModifierAndWildcardWithOptionalModifier)
{
    RunPatternStringTest("*{}**?", "*(.*)?");
}
...
struct DirectMatchCase {
    ...
    std::vector<std::pair<std::string_view, std::optional<std::string_view>>> expected_groups;
};

void RunDirectMatchTest(std::string_view input, std::vector<DirectMatchCase> case_list)
{
    auto result = Parse(input, PassThrough, { .sensitive = true, .strict = true, .end = true, .start = true });
    ASSERT_TRUE(result.ok());
    auto& pattern = result.value();
    EXPECT_TRUE(pattern.CanDirectMatch());
    for (const auto& c : case_list) {
        std::vector<std::pair<std::string_view, std::optional<std::string_view>>> matched_groups;
...
void RunDirectMatchUnsupportedTest(std::string_view input, Options options = { .sensitive = true, .strict = true, .end = true, .start = true })
{
    auto result = Parse(input, PassThrough, options);
    ASSERT_TRUE(result.ok());
    auto& pattern = result.value();
...
{
    RunDirectMatchUnsupportedTest("*", { .sensitive = true, .strict = true, .end = true, .start = false });
}

TEST(PatternDirectMatch, OptionsFalseEndUnsupported)
{
    RunDirectMatchUnsupportedTest("*", { .sensitive = true, .strict = true, .end = false, .start = true });
}

TEST(PatternDirectMatch, OptionsFalseStrictUnsupported)
{
    RunDirectMatchUnsupportedTest("*", { .sensitive = true, .strict = false, .end = true, .start = true });
}

TEST(PatternDirectMatch, OptionsFalseSensitiveUnsupported)
{
    RunDirectMatchUnsupportedTest("*", { .sensitive = false, .strict = true, .end = true, .start = true });
}
