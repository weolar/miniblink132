        ...
        while (index_ < pattern_.size()) {
            ...
            if (codepoint_ == '\\') {
                if (index_ == (pattern_.size() - 1)) {
                    Error(absl::StrFormat("Trailing escape character at index %d.", index_));
                    continue;
                }
                size_t escaped_i = next_index_;
                if (!Next()) {
                    Error(absl::StrFormat("Invalid UTF-8 codepoint at index %d.", next_index_));
                    continue;
                }
            ...
            if (codepoint_ == ':') {
                ...
                if (pos <= name_start) {
                    Error(absl::StrFormat("Missing parameter name at index %d.", index_), name_start, index_);
                    continue;
                }
            ...
            if (codepoint_ == '(') {
                ...
                while (j < pattern_.size()) {
                    ...
                    if (!IsASCII(codepoint_)) {
                        Error(absl::StrFormat("Invalid non-ASCII character 0x%02x at index %d.", codepoint_, j), regex_start, index_);
                        error = true;
                        break;
                    }
                    if (j == regex_start && codepoint_ == '?') {
                        Error(absl::StrFormat("Regex cannot start with '?' at index %d", j), regex_start, index_);
                        error = true;
                        break;
                    }
                    ...
                    if (codepoint_ == '\\') {
                        if (j == (pattern_.size() - 1)) {
                            Error(absl::StrFormat("Trailing escape character at index %d.", j), regex_start, index_);
                            error = true;
                            break;
                        }
                        ...
                        if (!IsASCII(codepoint_)) {
                            Error(absl::StrFormat("Invalid non-ASCII character 0x%02x at index %d.", codepoint_, escaped_j), regex_start, index_);
                            error = true;
                            break;
                        }
                        j = next_index_;
                    ...
                    if (codepoint_ == ')') {
                        ...
                        paren_nesting += 1;
                        if (j == (pattern_.size() - 1)) {
                            Error(absl::StrFormat("Unbalanced regex at index %d.", j), regex_start, index_);
                            error = true;
                            break;
                        }
                        ...
                        if (codepoint_ != '?') {
                            Error(absl::StrFormat("Unnamed capturing groups are not allowed at index %d.", tmp_j), regex_start, index_);
                            error = true;
                            break;
                        }
                        next_index_ = tmp_j;
                ...
                if (paren_nesting) {
                    Error(absl::StrFormat("Unbalanced regex at index %d.", index_), regex_start, index_);
                    continue;
                }

                const size_t regex_length = j - regex_start - 1;
                if (regex_length == 0) {
                    Error(absl::StrFormat("Missing regex at index %d.", index_), regex_start, index_);
                    continue;
                }
...
private:
    ...
    void AddToken(TokenType type, size_t next_pos, size_t value_pos, size_t value_length)
    {
        token_list_.emplace_back(type, index_, pattern_.substr(value_pos, value_length));
        index_ = next_pos;
    }
    ...
    {
        AddToken(type, next_index_, index_);
    }
    ...
    {
        Error(message, next_index_, index_);
    }
...
{
    o << "{ type:" << static_cast<int>(token.type) << ", index:" << token.index << ", value:" << token.value << " }";
    return o;
}

// Split the input pattern into a list of tokens.
absl::StatusOr<std::vector<Token>> Tokenize(absl::string_view pattern, TokenizePolicy policy)
{
