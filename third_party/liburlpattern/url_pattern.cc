...
bool Pattern::HasRegexGroups(void) const
{
    *(int*)1 = 1;
    return false;
}

absl::Status ConstructorStringParser::Parse(ConstructorStringParser::ProtocolCheckCallback protocol_matches_special_scheme)
{
    *(int*)1 = 1;
    return absl::OkStatus();
}

ConstructorStringParser::ConstructorStringParser(std::string_view)
{
    *(int*)1 = 1;
}
...
{
    o << "{ type:" << static_cast<int>(part.type) << ", name:" << part.name << ", prefix:" << part.prefix << ", value:" << part.value
      << ", suffix:" << part.suffix << ", modifier:" << static_cast<int>(part.modifier) << " }";
    return o;
}
...
Part::Part(PartType t, std::string n, std::string p, std::string v, std::string s, Modifier m)
    : type(t)
    , name(std::move(n))
    , prefix(std::move(p))
    , value(std::move(v))
...
Pattern::Pattern(std::vector<Part> part_list, Options options, std::string segment_wildcard_regex)
...
std::string Pattern::GeneratePatternString() const
{
    ...
    for (const Part& part : part_list_) {
        ...
        estimated_length += part.prefix.size() + part.value.size() + part.suffix.size() + 3;
    }
    ...
    for (size_t i = 0; i < part_list_.size(); ++i) {
        ...
        bool needs_grouping
            = !part.suffix.empty() || (!part.prefix.empty() && (part.prefix.size() != 1 || options_.prefix_list.find(part.prefix[0]) == std::string::npos));
        ...
        const Part* next_part = (i + 1) < part_list_.size() ? &part_list_[i + 1] : nullptr;
        if (!needs_grouping && custom_name && part.type == PartType::kSegmentWildcard && part.modifier == Modifier::kNone && next_part
            && next_part->prefix.empty() && next_part->suffix.empty()) {
            if (next_part->type == PartType::kFixed) {
                UChar32 codepoint = -1;
                U8_GET(reinterpret_cast<const uint8_t*>(next_part->value.data()), 0, 0, static_cast<int>(next_part->value.size()), codepoint);
        ...
        if (!needs_grouping && part.prefix.empty() && last_part && last_part->type == PartType::kFixed) {
            needs_grouping = options_.prefix_list.find(last_part->value.back()) != std::string::npos;
        }
        ...
        if (part.type == PartType::kRegex) {
            ...
            if (!custom_name
                && (!last_part || last_part->type == PartType::kFixed || last_part->modifier != Modifier::kNone || needs_grouping || !part.prefix.empty())) {
                result += "*";
            } else {
                result += "(";
        ...
        if (part.type == PartType::kSegmentWildcard && custom_name && !part.suffix.empty()) {
            UChar32 codepoint = -1;
            U8_GET(reinterpret_cast<const uint8_t*>(part.suffix.data()), 0, 0, static_cast<int>(part.suffix.size()), codepoint);
...
std::string Pattern::GenerateRegexString(std::vector<std::string>* name_list_out) const
{
    std::string result;
    ...
    if (!part_list_.empty()) {
        ...
        if (last_part.type == PartType::kFixed && last_part.modifier == Modifier::kNone) {
            ABSL_ASSERT(!last_part.value.empty());
            end_delimited = options_.delimiter_list.find(last_part.value.back()) != std::string::npos;
        }
    }
    if (!end_delimited) {
        result += "(?=";
...
bool Pattern::CanDirectMatch() const
{
    ...
    // URLPattern.
    if (!options_.start || !options_.end || !options_.strict || !options_.sensitive) {
        return false;
    }
...
bool Pattern::DirectMatch(absl::string_view input, std::vector<std::pair<absl::string_view, std::optional<absl::string_view>>>* group_list_out) const
{
    ...
    for (const Part& part : part_list_) {
        if (part.type == PartType::kFixed) {
            if (part.modifier == Modifier::kNone) {
                ...
                // (?:<escaped-fixed-value>)<modifier>
                result += EscapedRegexpStringLength(part.value) + 4 + ModifierLength(part.modifier);
            }
            continue;
        }
        ...
        if (part.modifier == Modifier::kNone || part.modifier == Modifier::kOptional) {
            // (?:<prefix>(<regex-value>)<suffix>)<modifier>
            result += prefix_length + regex_value.size() + suffix_length + ModifierLength(part.modifier) + 6;
            continue;
        }

        // (?:<prefix>((?:<regex-value>)(?:<suffix><prefix>(?:<regex-value>))*)<suffix>)?
        result += prefix_length + regex_value.size() + suffix_length + prefix_length + regex_value.size() + suffix_length + 19;
        if (part.modifier == Modifier::kZeroOrMore)
            result += 1;
    }
...
bool Pattern::IsOnlyFullWildcard() const
{
    ...
    return part.type == PartType::kFullWildcard && part.prefix.empty() && part.suffix.empty();
}
...
bool Pattern::IsOnlyFixedText() const
{
    if (part_list_.size() != 1)
        return false;
    auto& part = part_list_[0];
    bool result = part.type == PartType::kFixed && part.modifier == Modifier::kNone;
    if (result) {
