...
void RunTokenizeTest(std::string_view pattern, absl::StatusOr<std::vector<Token>> expected, TokenizePolicy policy = TokenizePolicy::kStrict)
{
    auto result = Tokenize(pattern, policy);
    ASSERT_EQ(result.ok(), expected.ok()) << "lexer status for: " << pattern;
    if (!expected.ok()) {
        ASSERT_EQ(result.status().code(), expected.status().code()) << "lexer status code for: " << pattern;
        EXPECT_NE(result.status().message().find(expected.status().message()), std::string::npos)
            << "lexer message '" << result.status().message() << "' does not contain '" << expected.status().message() << "' for: " << pattern;
        return;
    }
    const auto& expected_token_list = expected.value();
    const auto& token_list = result.value();
    EXPECT_EQ(token_list.size(), expected_token_list.size()) << "lexer should produce expected number of tokens for: " << pattern;
    for (size_t i = 0; i < token_list.size() && i < expected_token_list.size(); ++i) {
        EXPECT_EQ(token_list[i], expected_token_list[i]) << "token at index " << i << " wrong for: " << pattern;
    }
}
...
{
    RunTokenizeTest("/foo\\", absl::InvalidArgumentError("Trailing escape character"));
}
...
{
    RunTokenizeTest("/:/foo", absl::InvalidArgumentError("Missing parameter name"));
}
...
{
    RunTokenizeTest("/: bad", absl::InvalidArgumentError("Missing parameter name"));
}
...
{
    RunTokenizeTest("(ßar)", absl::InvalidArgumentError("Invalid non-ASCII character"));
}
...
{
    RunTokenizeTest("(f(oo))", absl::InvalidArgumentError("Unnamed capturing groups are not allowed"));
}
...
TEST(TokenizeTest, RegexWithEscapedInvalidChar)
{
    ...
    // character.
    RunTokenizeTest("(\\\xc2\xa2)", absl::InvalidArgumentError("Invalid non-ASCII character"));
}

TEST(TokenizeTest, RegexWithLeadingQuestion)
{
    RunTokenizeTest("(?foo)", absl::InvalidArgumentError("Regex cannot start with '?'"));
}
...
TEST(TokenizeTest, LenientPolicy)
{
    ...
    RunTokenizeTest("http://a.com:8080/foo?bar#baz", expected_tokens, TokenizePolicy::kLenient);
}
...
{
    RunTokenizeTest("hello\xcdworld", absl::InvalidArgumentError("Invalid UTF-8 codepoint at index 5."));
}

TEST(TokenizeTest, InvalidUtf8Escaped)
{
    RunTokenizeTest("hello\\\xcdworld", absl::InvalidArgumentError("Invalid UTF-8 codepoint at index 7."));
}

TEST(TokenizeTest, InvalidUtf8InName)
{
    RunTokenizeTest("/:foo:hello\xcdworld", absl::InvalidArgumentError("Invalid UTF-8 codepoint at index 11."));
}

TEST(TokenizeTest, InvalidUtf8InRegexGroup)
{
    RunTokenizeTest("(foo\xcd)", absl::InvalidArgumentError("Invalid UTF-8 codepoint at index 4."));
}

TEST(TokenizeTest, InvalidUtf8EscapedInRegexGroup)
{
    RunTokenizeTest("(foo\\\xcd)", absl::InvalidArgumentError("Invalid UTF-8 codepoint at index 6."));
}
