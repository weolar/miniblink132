...
NSPR_API(void) PR_ExplodeTime(PRTime usecs, PRTimeParamFn params, PRExplodedTime* exploded);
...
NSPR_API(PRStatus) PR_ParseTimeString(const char* string, PRBool default_to_gmt, PRTime* result);
...
NSPR_API(PRUint32) PR_FormatTime(char* buf, int buflen, const char* fmt, const PRExplodedTime* tm);
