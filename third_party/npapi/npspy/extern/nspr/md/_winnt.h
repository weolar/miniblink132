...
extern PRStatus _PR_NT_MakeSecurityDescriptorACL(PRIntn mode, DWORD accessTable[], PSECURITY_DESCRIPTOR* resultSD, PACL* resultACL);
extern void _PR_NT_FreeSecurityDescriptorACL(PSECURITY_DESCRIPTOR pSD, PACL pACL);
...
extern int _PR_NTFiberSafeSelect(int, fd_set*, fd_set*, fd_set*, const struct timeval*);
...
extern struct PRProcess* _PR_CreateWindowsProcess(const char* path, char* const* argv, char* const* envp, const struct PRProcessAttr* attr);
...
extern PRStatus _PR_WaitWindowsProcess(struct PRProcess* process, PRInt32* exitCode);
...
#define _MD_GET_ATTACHED_THREAD() (_pr_use_static_tls ? _pr_current_fiber : (PRThread*)TlsGetValue(_pr_currentFiberIndex))
...
#define _MD_LAST_THREAD() (_pr_use_static_tls ? _pr_fiber_last_run : (PRThread*)TlsGetValue(_pr_lastFiberIndex))
...
#define _MD_CURRENT_CPU() (_pr_use_static_tls ? _pr_current_cpu : (struct _PRCPU*)TlsGetValue(_pr_currentCPUIndex))
...
#define _MD_GET_INTSOFF() (_pr_use_static_tls ? _pr_ints_off : (PRUintn)TlsGetValue(_pr_intsOffIndex))
