...
#define COND_TIMEDWAIT(condp, mutexp, tspec) cond_timedwait(condp, mutexp, tspec)
...
#define _MD_SWITCH_CONTEXT(_thread)                                                                                                                            \
    ...
    sigprocmask(SIG_BLOCK, &((_thread)->md.blockMask), &((_thread)->md.oldMask));                                                                              \
...
extern void _MD_SET_PRIORITY(struct _MDThread* thread, PRThreadPriority newPri);
extern PRStatus _MD_CREATE_THREAD(
    PRThread* thread, void (*start)(void*), PRThreadPriority priority, PRThreadScope scope, PRThreadState state, PRUint32 stackSize);
