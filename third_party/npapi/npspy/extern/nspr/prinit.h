...
*/
NSPR_API(void) PR_Init(PRThreadType type, PRThreadPriority priority, PRUintn maxPTDs);
...
NSPR_API(PRIntn) PR_Initialize(PRPrimordialFn prmain, PRIntn argc, char** argv, PRUintn maxPTDs);
...
NSPR_API(PRStatus) PR_CallOnce(PRCallOnceType* once, PRCallOnceFN func);

NSPR_API(PRStatus) PR_CallOnceWithArg(PRCallOnceType* once, PRCallOnceWithArgFN func, void* arg);
