...
*/
NSPR_API(PRThread*) PR_AttachThread(PRThreadType type, PRThreadPriority priority, PRThreadStack* stack);
...
NSPR_API(void) PR_SetThreadDumpProc(PRThread* thread, PRThreadDumpProc dump, void* arg);
...
PR_CreateThreadGCAble(
    PRThreadType type, void (*start)(void* arg), void* arg, PRThreadPriority priority, PRThreadScope scope, PRThreadState state, PRUint32 stackSize);

/*
** Same as PR_AttachThread except that the thread being attached is marked as 
** garbage collectible.
*/
NSPR_API(PRThread*) PR_AttachThreadGCAble(PRThreadType type, PRThreadPriority priority, PRThreadStack* stack);
...
** number of pointers. 
*/
typedef PRStatus(PR_CALLBACK* PRScanStackFun)(PRThread* t, void** baseAddr, PRUword count, void* closure);
