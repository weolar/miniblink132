...
static bool cookie_tailmatch(const char* cookie_domain, size_t cookie_domain_len, const char* hostname)
{
    ...
    if (!strncasecompare(cookie_domain, hostname + hostname_len - cookie_domain_len, cookie_domain_len))
        return FALSE;
...
static int invalid_octets(const char* p)
{
    ...
    static const char badoctets[] = { "\x01\x02\x03\x04\x05\x06\x07\x08\x0a"
 ...
 */
struct Cookie* Curl_cookie_add(struct Curl_easy* data, struct CookieInfo* c, bool httpheader, /* TRUE if HTTP header-style line */
    ...
    bool secure) /* TRUE if connection is over secure origin */
{
    ...
    if (httpheader) {
         ...
         * 4096 bytes combo
         */
                if (nlen >= (MAX_NAME - 1) || vlen >= (MAX_NAME - 1) || ((nlen + vlen) > MAX_NAME)) {
                    freecookie(co);
                    infof(data, "oversized cookie dropped, name/val %zu + %zu bytes", nlen, vlen);
                    return NULL;
                }
         ...
         */
           ...
           */
                    ...
                    if (!domain || (is_ip && !strncmp(valuep, domain, vlen) && (vlen == strlen(domain)))
           ...
           */
                    ...
                    offt = curlx_strtoofft((*maxage == '\"') ? &maxage[1] : &maxage[0], NULL, 10, &co->expires);
                    switch (offt) {
                    case CURL_OFFT_FLOW:
     ...
     * allocated
     */
        for (ptr = firstptr, fields = 0; ptr && !badcookie; ptr = strtok_r(NULL, "\t", &tok_buf), fields++) {
            switch (fields) {
            case 0:
    ...
    while (clist) {
        ...
        if (!replace_co && strcasecompare(clist->name, co->name)) {
            ...
            if (replace_old) {
                ...
                if (clist->spath && co->spath && !strcasecompare(clist->spath, co->spath))
                    replace_old = FALSE;
...
struct Cookie* Curl_cookie_getlist(struct Curl_easy* data, struct CookieInfo* c, const char* host, const char* path, bool secure)
{
    ...
    while (co) {
        ...
        if (co->secure ? secure : TRUE) {
            ...
            if (!co->domain || (co->tailmatch && !is_ip && cookie_tailmatch(co->domain, strlen(co->domain), host))
...
{
    return aprintf("%s" /* httponly preamble */
                   "%s%s\t" /* domain */
                   "%s\t" /* tailmatch */
                   "%s\t" /* path */
                   "%s\t" /* secure */
     ...
     */
        (co->tailmatch && co->domain && co->domain[0] != '.') ? "." : "", co->domain ? co->domain : "unknown", co->tailmatch ? "TRUE" : "FALSE",
