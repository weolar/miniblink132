    ...
    while (m) {
        if (m->socket.fd == s) {
           ...
           mask. Convert from libcurl bitmask to the poll one. */
                ...
                infof(easy, "socket cb: socket %d UPDATED as %s%s", s, (what & CURL_POLL_IN) ? "IN" : "", (what & CURL_POLL_OUT) ? "OUT" : "");
            }
            break;
        }
        prev = m;
        ...
        if (what == CURL_POLL_REMOVE) {
         ...
         "%s: socket %d asked to be REMOVED but not present!\n",
            ...
            if (m) {
                ...
                m->socket.revents = 0;
                ev->list = m;
                infof(easy, "socket cb: socket %d ADDED as %s%s", s, (what & CURL_POLL_IN) ? "IN" : "", (what & CURL_POLL_OUT) ? "OUT" : "");
            } else
        ...
        if (!pollrc) {
            ...
            for (i = 0; i < numfds; i++) {
                if (fds[i].revents) {
                    ...
                    infof(multi->easyp, "call curl_multi_socket_action(socket %d)", fds[i].fd);
   ...
   */
    ...
    if (data->cookies) {
       ...
       in the clone as well! */
        outcurl->cookies = Curl_cookie_init(data, NULL, outcurl->cookies, data->set.cookiesession);
        if (!outcurl->cookies)
            goto fail;
    }
...
CURLcode curl_easy_pause(struct Curl_easy* data, int action)
{
    ...
    newstate = (k->keepon & ~(KEEP_RECV_PAUSE | KEEP_SEND_PAUSE)) | ((action & CURLPAUSE_RECV) ? KEEP_RECV_PAUSE : 0)
    ...
    if ((k->keepon & ~newstate & KEEP_SEND_PAUSE) && (data->mstate == MSTATE_PERFORMING || data->mstate == MSTATE_RATELIMITING)
 ...
 * Returns always 0.
 */
static int conn_upkeep(struct Curl_easy* data, struct connectdata* conn, void* param)
{
