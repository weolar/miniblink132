 ...
 */
const struct Curl_handler Curl_handler_http = { "HTTP", /* scheme */
...
#ifdef USE_WEBSOCKETS
const struct Curl_handler Curl_handler_ws = { "WS", /* scheme */
 ...
 */
const struct Curl_handler Curl_handler_https = { "HTTPS", /* scheme */
    http_setup_conn, /* setup_connection */
    Curl_http, /* do_it */
    Curl_http_done, /* done */
    ZERO_NULL, /* do_more */
...
#ifdef USE_WEBSOCKETS
const struct Curl_handler Curl_handler_wss = { "WSS", /* scheme */
    ws_setup_conn, /* setup_connection */
    Curl_http, /* do_it */
    Curl_http_done, /* done */
    ZERO_NULL, /* do_more */
...
CURLcode Curl_http_auth_act(struct Curl_easy* data)
{
    ...
    if (pickhost || pickproxy) {
        if ((data->state.httpreq != HTTPREQ_GET) && (data->state.httpreq != HTTPREQ_HEAD) && !data->state.rewindbeforesend) {
...
static CURLcode output_auth_headers(
    struct Curl_easy* data, struct connectdata* conn, struct auth* authstatus, const char* request, const char* path, bool proxy)
{
    ...
    if (authstatus->picked == CURLAUTH_AWS_SIGV4) {
        ...
        if (
#ifndef CURL_DISABLE_PROXY
            (proxy && conn->bits.proxy_user_passwd && !Curl_checkProxyheaders(data, conn, STRCONST("Proxy-authorization"))) ||
#endif
            (!proxy && data->state.aptr.user && !Curl_checkheaders(data, STRCONST("Authorization")))) {
            auth = "Basic";
    ...
    if (auth) {
#ifndef CURL_DISABLE_PROXY
        infof(data, "%s auth using %s with user '%s'", proxy ? "Proxy" : "Server", auth,
            proxy ? (data->state.aptr.proxyuser ? data->state.aptr.proxyuser : "") : (data->state.aptr.user ? data->state.aptr.user : ""));
#else
 ...
 * @returns CURLcode
 */
CURLcode Curl_http_output_auth(struct Curl_easy* data, struct connectdata* conn, const char* request, Curl_HttpReq httpreq, const char* path,
...
#ifdef USE_SPNEGO
        authhost->want & CURLAUTH_NEGOTIATE || authproxy->want & CURLAUTH_NEGOTIATE ||
#endif
        data->set.str[STRING_BEARER])
        /* continue please */;
    else {
...
CURLcode Curl_http_input_auth(struct Curl_easy* data, bool proxy, const char* auth) /* the first non-space */
{
    /*
 ...
 * Returns CURLcode
 */
CURLcode Curl_buffer_send(struct dynbuf* in, struct Curl_easy* data, struct HTTP* http,
...
CURLcode Curl_http_done(struct Curl_easy* data, CURLcode status, bool premature)
{
   ...
   * a chance to be set back to true when we output the next auth header */
    ...
    if (!premature && /* this check is pointless when DONE is called before the
        ...
        !conn->bits.retry && !data->set.connect_only && (data->req.bytecount + data->req.headerbytecount - data->req.deductheadercount) <= 0) {
       ...
       */
           ...
           * ignore this for now */
            ...
            header as that will produce *two* in the same request! */
                ...
                ;
            else if ((conn->httpversion >= 20) && hd_name_eq(name, namelen, STRCONST("Transfer-Encoding:")))
                /* HTTP/2 doesn't support chunked requests */
                ;
            else if ((hd_name_eq(name, namelen, STRCONST("Authorization:")) || hd_name_eq(name, namelen, STRCONST("Cookie:"))) &&
...
void Curl_http_method(struct Curl_easy* data, struct connectdata* conn, const char** method, Curl_HttpReq* reqp)
{
    ...
    const char* request;
    if ((conn->handler->protocol & (PROTO_FAMILY_HTTP | CURLPROTO_FTP)) && data->state.upload)
        httpreq = HTTPREQ_PUT;
...
CURLcode Curl_http_host(struct Curl_easy* data, struct connectdata* conn)
{
    ...
    if (ptr && (!data->state.this_is_a_follow || strcasecompare(data->state.first_host, conn->host.name))) {
       ...
       [brackets] if the host name is a plain IPv6-address. RFC2732-style. */
        ...
        if (((conn->given->protocol & (CURLPROTO_HTTPS | CURLPROTO_WSS)) && (conn->remote_port == PORT_HTTPS))
            || ((conn->given->protocol & (CURLPROTO_HTTP | CURLPROTO_WS)) && (conn->remote_port == PORT_HTTP)))
...
CURLcode Curl_http_body(struct Curl_easy* data, struct connectdata* conn, Curl_HttpReq httpreq, const char** tep)
{
    ...
    case HTTPREQ_POST_FORM:
       ...
       the conversion */
        if (!data->state.formp) {
            ...
            result = Curl_getformdata(data, data->state.formp, data->set.httppost, data->state.fread_func);
            if (result)
                return result;
    ...
    if (data->state.mimepost) {
        ...
        result = Curl_mime_prepare_headers(data, data->state.mimepost, cthdr, NULL, MIMESTRATEGY_FORM);
    ...
    if (ptr) {
        ...
        if ((conn->handler->protocol & PROTO_FAMILY_HTTP)
            && (((httpreq == HTTPREQ_POST_MIME || httpreq == HTTPREQ_POST_FORM) && http->postsize < 0)
                || ((data->state.upload || httpreq == HTTPREQ_POST) && data->state.infilesize == -1))) {
            if (conn->bits.authneg)
...
static CURLcode addexpect(struct Curl_easy* data, struct connectdata* conn, struct dynbuf* r)
{
...
#define Curl_buffer_send(a, b, c, d, e, f) CURLE_OK
    ...
    case HTTPREQ_PUT: /* Let's PUT the data to the server! */
        ...
        result = Curl_buffer_send(r, data, data->req.p.http, &data->info.request_size, 0, FIRSTSOCKET);
        if (result)
            failf(data, "Failed sending PUT request");
        else
            /* prepare for transfer */
            Curl_setup_transfer(data, FIRSTSOCKET, -1, TRUE, http->postsize ? FIRSTSOCKET : -1);
        if (result)
            return result;
        break;
    ...
    case HTTPREQ_POST_MIME:
       ...
       kinds of headers (Transfer-Encoding: chunked and Content-Length) */
        if (http->postsize != -1 && !data->req.upload_chunky && (!Curl_checkheaders(data, STRCONST("Content-Length")))) {
    ...
    case HTTPREQ_POST:
       ...
       kinds of headers (Transfer-Encoding: chunked and Content-Length) */
        ...
        if (!Curl_checkheaders(data, STRCONST("Content-Type"))) {
            result = Curl_dyn_addn(r,
                STRCONST("Content-Type: application/"
        ...
        if (data->set.postfields) {
            if (!data->state.expect100header && (http->postsize < MAX_INITIAL_POST_SIZE)) {
        ...
        /* issue the request */
        result = Curl_buffer_send(r, data, data->req.p.http, &data->info.request_size, included_body, FIRSTSOCKET);
...
#ifdef USE_WEBSOCKETS
        else if ((conn->handler->protocol & (CURLPROTO_WS | CURLPROTO_WSS)) && !(data->set.connect_only))
...
CURLcode Curl_http_cookies(struct Curl_easy* data, struct connectdata* conn, struct dynbuf* r)
{
    ...
    if (data->cookies || addcookies) {
        ...
        if (data->cookies && data->state.cookie_engine) {
            ...
            const bool secure_context = conn->handler->protocol & (CURLPROTO_HTTPS | CURLPROTO_WSS) || strcasecompare("localhost", host)
                    || !strcmp(host, "127.0.0.1") || !strcmp(host, "::1")
                ? TRUE
                : FALSE;
            Curl_share_lock(data, CURL_LOCK_DATA_COOKIE, CURL_LOCK_ACCESS_SINGLE);
            co = Curl_cookie_getlist(data, data->cookies, host, data->state.up.path, secure_context);
        ...
        if (((httpreq == HTTPREQ_GET) || (httpreq == HTTPREQ_HEAD)) && !Curl_checkheaders(data, STRCONST("Range"))) {
           ...
           upload the whole file (again) */
                ...
                data->state.aptr.rangeline = aprintf("Content-Range: bytes %s%" CURL_FORMAT_CURL_OFF_T "/%" CURL_FORMAT_CURL_OFF_T "\r\n", data->state.range,
                    total_expected_size - 1, total_expected_size);
            } else {
...
CURLcode Curl_http(struct Curl_easy* data, bool* done)
{
    ...
    *done = TRUE;
...
#ifndef CURL_DISABLE_PROXY
        if (!Curl_conn_is_http2(data, conn, FIRSTSOCKET) && conn->bits.proxy && !conn->bits.tunnel_proxy) {
    ...
    result = Curl_dyn_addf(&req,
        ...
        httpstring, (data->state.aptr.host ? data->state.aptr.host : ""), data->state.aptr.proxyuserpwd ? data->state.aptr.proxyuserpwd : "",
        data->state.aptr.userpwd ? data->state.aptr.userpwd : "", (data->state.use_range && data->state.aptr.rangeline) ? data->state.aptr.rangeline : "",
        ...
        (data->set.str[STRING_ENCODING] && *data->set.str[STRING_ENCODING] && data->state.aptr.accept_encoding) ? data->state.aptr.accept_encoding : "",
        (data->state.referer && data->state.aptr.ref) ? data->state.aptr.ref : "" /* Referer: <data> */,
#ifndef CURL_DISABLE_PROXY
        (conn->bits.httpproxy && !conn->bits.tunnel_proxy && !Curl_checkheaders(data, STRCONST("Proxy-Connection"))
    ...
    if (data->req.writebytecount) {
       ...
       properly */
        ...
        if (!http->postsize) {
         ...
         complete */
            infof(data, "upload completely sent off: %" CURL_FORMAT_CURL_OFF_T " out of %" CURL_FORMAT_CURL_OFF_T " bytes", data->req.writebytecount,
                http->postsize);
            data->req.upload_done = TRUE;
...
CURLcode Curl_http_header(struct Curl_easy* data, struct connectdata* conn, char* headp)
{
    ...
    */
    ...
    /* If enabled, the header is incoming and this is over HTTPS */
        ...
        CURLcode check = Curl_hsts_parse(data->hsts, conn->host.name, headp + strlen("Strict-Transport-Security:"));
        if (check)
            infof(data, "Illegal STS header skipped");
#ifdef DEBUGBUILD
        else
            infof(data, "Parsed STS header fine (%zu entries)", data->hsts->list.size);
#endif
    }
#endif
#ifndef CURL_DISABLE_ALTSVC
    /* If enabled, the header is incoming and this is over HTTPS */
        ...
        enum alpnid id = (conn->httpversion == 30) ? ALPN_h3 : (conn->httpversion == 20) ? ALPN_h2 : ALPN_h1;
        result = Curl_altsvc_parse(data, data->asi, headp + strlen("Alt-Svc:"), id, conn->host.name, curlx_uitous((unsigned int)conn->remote_port));
        if (result)
            return result;
    }
#endif
...
CURLcode Curl_http_statusline(struct Curl_easy* data, struct connectdata* conn)
{
   ...
   */
    ...
    } else if (conn->httpversion == 20 || (k->upgr101 == UPGR101_H2 && k->httpcode == 101)) {
...
CURLcode Curl_bump_headersize(struct Curl_easy* data, size_t delta, bool connect_only)
{
    size_t bad = 0;
...
CURLcode Curl_http_readwrite_headers(struct Curl_easy* data, struct connectdata* conn, ssize_t* nread, bool* stop_reading)
{
    ...
    *stop_reading = FALSE;
     ...
     *****/
        ...
        if ((0x0a == *headp) || (0x0d == *headp)) {
           ...
           */
               ...
               are also processed here, otherwise they are lost. */
                        result = Curl_http2_upgrade(data, conn, FIRSTSOCKET, k->str, *nread);
                        if (result)
                            return result;
                        *nread = 0;
                    }
       ...
       */
            writetype = CLIENTWRITE_HEADER | ((k->httpcode / 100 == 1) ? CLIENTWRITE_1XX : 0);
       ...
       * use. It will set 'newurl' if an auth method was picked. */
            ...
            if (k->httpcode >= 300) {
                if ((!conn->bits.authneg) && !conn->bits.close && !data->state.rewindbeforesend) {
                    /*
           ...
           */
             ...
             */
                   ...
                   header */
                                if (!k->writebytecount && k->exp100 == EXP100_AWAITING_CONTINUE) {
                ...
                if (data->state.rewindbeforesend && (conn->writesockfd != CURL_SOCKET_BAD)) {
         ...
         */
           ...
           close the stream. */
                if (0 == k->maxdownload && !Curl_conn_is_http2(data, conn, FIRSTSOCKET) && !Curl_conn_is_http3(data, conn, FIRSTSOCKET))
                    *stop_reading = TRUE;
     ...
     */
        ...
        result = Curl_bump_headersize(data, Curl_dyn_len(&data->state.headerb), FALSE);
        if (result)
            return result;
...
CURLcode Curl_http_req_make(struct httpreq** preq, const char* method, size_t m_len, const char* scheme, size_t s_len, const char* authority, size_t a_len,
...
static bool h2_non_field(const char* name, size_t namelen)
{
    ...
    for (i = 0; i < sizeof(H2_NON_FIELD) / sizeof(H2_NON_FIELD[0]); ++i) {
        if (namelen < H2_NON_FIELD[i].namelen)
            return FALSE;
        if (namelen == H2_NON_FIELD[i].namelen && strcasecompare(H2_NON_FIELD[i].name, name))
            return TRUE;
    }
    return FALSE;
}

CURLcode Curl_http_req_to_h2(struct dynhds* h2_headers, struct httpreq* req, struct Curl_easy* data)
{
    ...
    if (req->scheme) {
        ...
        if (scheme) {
            ...
            scheme = (data->conn && data->conn->handler->flags & PROTOPT_SSL) ? "https" : "http";
        }
    }
