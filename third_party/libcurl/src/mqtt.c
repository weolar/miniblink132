...
static int mqtt_getsock(struct Curl_easy* data, struct connectdata* conn, curl_socket_t* sock);
static CURLcode mqtt_setup_conn(struct Curl_easy* data, struct connectdata* conn);
...
static int add_passwd(const char* passwd, const size_t plen, char* pkt, const size_t start, int remain_pos)
{
...
static int add_user(const char* username, const size_t ulen, unsigned char* pkt, const size_t start, int remain_pos)
{
    /* magic number that need to be set properly */
    const size_t conn_flags_pos = remain_pos + 8;
    if (ulen > 0xffff)
...
static int add_client_id(const char* client_id, const size_t client_id_len, char* pkt, const size_t start)
{
...
static CURLcode mqtt_connect(struct Curl_easy* data)
{
   ...
   * be added on the payload. Refer to spec 1.5.2 and 1.5.4 */
    ...
    result = Curl_rand_alnum(data, (unsigned char*)&client_id[clen], MQTT_CLIENTID_LEN - clen + 1);
    /* add client id */
    ...
    if (ulen) {
        ...
        rc = add_user(username, ulen, (unsigned char*)packet, start_user, remain_pos);
        if (rc) {
...
static CURLcode mqtt_recv_atleast(struct Curl_easy* data, size_t nbytes)
{
    ...
    if (rlen < nbytes) {
        ...
        result = Curl_read(data, data->conn->sock[FIRSTSOCKET], (char*)readbuf, nbytes - rlen, &nread);
        if (result)
            return result;
        DEBUGASSERT(nread >= 0);
...
static CURLcode mqtt_verify_connack(struct Curl_easy* data)
{
    ...
    if (ptr[0] != 0x00 || ptr[1] != 0x00) {
        failf(data, "Expected %02x%02x but got %02x%02x", 0x00, 0x00, ptr[0], ptr[1]);
        Curl_dyn_reset(&mq->recvbuf);
        result = CURLE_WEIRD_SERVER_REPLY;
        goto fail;
    }
...
static CURLcode mqtt_get_topic(struct Curl_easy* data, char** topic, size_t* topiclen)
{
...
static CURLcode mqtt_verify_suback(struct Curl_easy* data)
{
    ...
    if (((unsigned char)ptr[0]) != ((mqtt->packetid >> 8) & 0xff) || ((unsigned char)ptr[1]) != (mqtt->packetid & 0xff) || ptr[2] != 0x00) {
        Curl_dyn_reset(&mq->recvbuf);
        result = CURLE_WEIRD_SERVER_REPLY;
        goto fail;
    }
...
static const char* statenames[]
    = { "MQTT_FIRST", "MQTT_REMAINING_LENGTH", "MQTT_CONNACK", "MQTT_SUBACK", "MQTT_SUBACK_COMING", "MQTT_PUBWAIT", "MQTT_PUB_REMAIN",

          "NOT A STATE" };
#endif

/* The only way to change state */
static void mqstate(struct Curl_easy* data, enum mqttstate state, enum mqttstate nextstate) /* used if state == FIRST */
{
    struct connectdata* conn = data->conn;
    struct mqtt_conn* mqtt = &conn->proto.mqtt;
#ifdef CURLDEBUG
    infof(data, "%s (from %s) (next is %s)", statenames[state], statenames[mqtt->state], (state == MQTT_FIRST) ? statenames[nextstate] : "");
#endif
    mqtt->state = state;
    if (state == MQTT_FIRST)
