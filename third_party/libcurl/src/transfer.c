...
static int data_pending(struct Curl_easy* data)
{
    ...
    return conn->handler->protocol & (CURLPROTO_SCP | CURLPROTO_SFTP) || Curl_conn_data_pending(data, FIRSTSOCKET);
}
...
static CURLcode readwrite_data(struct Curl_easy* data, struct connectdata* conn, struct SingleRequest* k, int* didwhat, bool* done, bool* comeback)
{
    ...
    int maxloops = 100;
    curl_off_t max_recv = data->set.max_recv_speed ? data->set.max_recv_speed : CURL_OFF_T_MAX;
       ...
       is non-headers. */
        if (!k->header && (nread > 0 || is_empty_data)) {
            ...
            if (data->req.no_body && nread > 0) {
                ...
                DEBUGF(infof(data, "did not want a BODY, but seeing %zd bytes", nread));
                *done = TRUE;
...
static CURLcode readwrite_upload(struct Curl_easy* data, struct connectdata* conn, int* didwhat)
{
    ...
    *didwhat |= KEEP_SEND;
    ...
    do {
        ...
        if (0 != k->upload_present && k->upload_present < curl_upload_refill_watermark(data) && !k->upload_chunky
            && /*(variable sized chunked header; append not safe)*/
            !k->upload_done && /*!(k->upload_done once k->upload_present sent)*/
            !(k->writebytecount + k->upload_present - k->pendingheader == data->state.infilesize)) {
           ...
           protocol agnostic. */
             ...
             go into the Expect: 100 state and await such a header */
               ...
               Remember that so we don't change the line endings. */
                ...
                result = Curl_fillreadbuffer(data, data->set.upload_buffer_size - offset, &fillcount);
                k->upload_fromhere -= offset;
                if (result)
                    return result;
 ...
 */
CURLcode Curl_follow(struct Curl_easy* data, char* newurl, /* the Location: string */
    followtype type) /* see transfer.h */
{
    ...
    if ((type != FOLLOW_RETRY) && (data->req.httpcode != 401) && (data->req.httpcode != 407) && Curl_is_absolute_url(newurl, NULL, 0, FALSE)) {
    ...
    uc = curl_url_set(data->state.uh, CURLUPART_URL, newurl,
        (type == FOLLOW_FAKE) ? CURLU_NON_SUPPORT_SCHEME
                              : ((type == FOLLOW_REDIR) ? CURLU_URLENCODE : 0) | CURLU_ALLOW_SPACE | (data->set.path_as_is ? CURLU_PATH_AS_IS : 0));
    if (uc) {
        if (type != FOLLOW_FAKE) {
            failf(data, "The redirect target URL could not be parsed: %s", curl_url_strerror(uc));
   ...
   */
     ...
     */
        if (data->state.httpreq != HTTPREQ_GET
            && ((data->state.httpreq != HTTPREQ_POST && data->state.httpreq != HTTPREQ_POST_FORM && data->state.httpreq != HTTPREQ_POST_MIME)
...
CURLcode Curl_retry_request(struct Curl_easy* data, char** url)
{
    ...
    *url = NULL;
    ...
    if (data->state.upload && !(conn->handler->protocol & (PROTO_FAMILY_HTTP | CURLPROTO_RTSP)))
        return CURLE_OK;

    if ((data->req.bytecount + data->req.headerbytecount == 0) && conn->bits.reuse && (!data->req.no_body || (conn->handler->protocol & PROTO_FAMILY_HTTP))
...
#define CONN_MAX_RETRIES 5
        ...
        *url = strdup(data->state.url);
        ...
        if ((conn->handler->protocol & PROTO_FAMILY_HTTP) && data->req.writebytecount) {
...
)
{
    ...
    httpsending = ((conn->handler->protocol & PROTO_FAMILY_HTTP) && (http->sending == HTTPSEND_REQUEST));

    if (conn->bits.multiplex || conn->httpversion >= 20 || httpsending) {
        /* when multiplexing, the read/write sockets need to be the same! */
        conn->sockfd = sockindex == -1 ? ((writesockindex == -1 ? CURL_SOCKET_BAD : conn->sock[writesockindex])) : conn->sock[sockindex];
