...
static CURLcode imap_perform_authenticate(struct Curl_easy* data, const char* mech, const struct bufref* initresp);
static CURLcode imap_continue_authenticate(struct Curl_easy* data, const char* mech, const struct bufref* resp);
static CURLcode imap_cancel_authenticate(struct Curl_easy* data, const char* mech);
...
const struct Curl_handler Curl_handler_imap = { "IMAP", /* scheme */
...
const struct Curl_handler Curl_handler_imaps = { "IMAPS", /* scheme */
    imap_setup_connection, /* setup_connection */
    imap_do, /* do_it */
    imap_done, /* done */
    ZERO_NULL, /* do_more */
    ...
    if (len >= 2 && !memcmp("* ", line, 2)) {
        ...
        case IMAP_LIST:
            if ((!imap->custom && !imap_matchresp(line, len, "LIST"))
                || (imap->custom && !imap_matchresp(line, len, imap->custom) && (!strcasecompare(imap->custom, "STORE") || !imap_matchresp(line, len, "FETCH"))
                    && !strcasecompare(imap->custom, "SELECT") && !strcasecompare(imap->custom, "EXAMINE") && !strcasecompare(imap->custom, "SEARCH")
                    && !strcasecompare(imap->custom, "EXPUNGE") && !strcasecompare(imap->custom, "LSUB") && !strcasecompare(imap->custom, "UID")
                    && !strcasecompare(imap->custom, "GETQUOTAROOT") && !strcasecompare(imap->custom, "NOOP")))
                return FALSE;
            break;
...
static void imap_state(struct Curl_easy* data, imapstate newstate)
{
    ...
    static const char* const names[] = {
        "STOP", "SERVERGREET", "CAPABILITY", "STARTTLS", "UPGRADETLS", "AUTHENTICATE", "LOGIN", "LIST", "SELECT", "FETCH", "FETCH_FINAL", "APPEND",
        "APPEND_FINAL", "SEARCH", "LOGOUT",
        /* LAST */
    };
...
static CURLcode imap_perform_login(struct Curl_easy* data, struct connectdata* conn)
{
    ...
    result = imap_sendf(data, "LOGIN %s %s", user ? user : "", passwd ? passwd : "");
...
static CURLcode imap_continue_authenticate(struct Curl_easy* data, const char* mech, const struct bufref* resp)
{
    ...
    return Curl_pp_sendf(data, &imapc->pp, "%s", (const char*)Curl_bufref_ptr(resp));
}
...
static CURLcode imap_perform_list(struct Curl_easy* data)
{
    ...
    if (imap->custom)
        ...
        result = imap_sendf(data, "%s%s", imap->custom, imap->custom_params ? imap->custom_params : "");
    else {
...
static CURLcode imap_perform_fetch(struct Curl_easy* data)
{
    ...
    if (imap->uid) {
        ...
        if (imap->partial)
            ...
            result = imap_sendf(data, "UID FETCH %s BODY[%s]", imap->uid, imap->section ? imap->section : "");
    } else if (imap->mindex) {
        /* Send the FETCH command */
        if (imap->partial)
            result = imap_sendf(data, "FETCH %s BODY[%s]<%s>", imap->mindex, imap->section ? imap->section : "", imap->partial);
        else
            result = imap_sendf(data, "FETCH %s BODY[%s]", imap->mindex, imap->section ? imap->section : "");
    } else {
...
static CURLcode imap_perform_append(struct Curl_easy* data)
{
    ...
    if (data->set.mimepost.kind != MIMEKIND_NONE) {
        ...
        result = Curl_mime_prepare_headers(data, &data->set.mimepost, NULL, NULL, MIMESTRATEGY_MAIL);
    ...
    result = imap_sendf(data, "APPEND %s (\\Seen) {%" CURL_FORMAT_CURL_OFF_T "}", mailbox, data->state.infilesize);
...
static CURLcode imap_done(struct Curl_easy* data, CURLcode status, bool premature)
{
    ...
    } else if (!data->set.connect_only && !imap->custom && (imap->uid || imap->mindex || data->state.upload || data->set.mimepost.kind != MIMEKIND_NONE)) {
...
static CURLcode imap_perform(struct Curl_easy* data, bool* connected, bool* dophase_done)
{
    ...
    *dophase_done = FALSE; /* not done yet */
    ...
    if (imap->mailbox && imapc->mailbox && strcasecompare(imap->mailbox, imapc->mailbox)
        && (!imap->uidvalidity || !imapc->mailbox_uidvalidity || strcasecompare(imap->uidvalidity, imapc->mailbox_uidvalidity)))
        selected = TRUE;
...
static CURLcode imap_sendf(struct Curl_easy* data, const char* fmt, ...)
{
    ...
    msnprintf(imapc->resptag, sizeof(imapc->resptag), "%c%03d", 'A' + curlx_sltosi((long)(data->conn->connection_id % 26)), ++imapc->cmdid);
...
static CURLcode imap_parse_url_path(struct Curl_easy* data)
{
    ...
    if (ptr != begin) {
        ...
        result = Curl_urldecode(begin, end - begin, &imap->mailbox, NULL, REJECT_CTRL);
        if (result)
            return result;
    } else
        imap->mailbox = NULL;
