...
CURLcode Curl_input_ntlm(struct Curl_easy* data, bool proxy, /* if proxy or not */
...
CURLcode Curl_output_ntlm(struct Curl_easy* data, bool proxy)
{
   ...
   * requests so go directly to NTLMSTATE_LAST */
    ...
    default: /* for the weird cases we (re)start here */
        ...
        result = Curl_auth_create_ntlm_type1_message(data, userp, passwdp, service, hostname, ntlm, &ntlmmsg);
        if (!result) {
            DEBUGASSERT(Curl_bufref_len(&ntlmmsg) != 0);
            result = Curl_base64_encode((const char*)Curl_bufref_ptr(&ntlmmsg), Curl_bufref_len(&ntlmmsg), &base64, &len);
            if (!result) {
                free(*allocuserpwd);
    ...
    case NTLMSTATE_TYPE2:
        ...
        result = Curl_auth_create_ntlm_type3_message(data, userp, passwdp, ntlm, &ntlmmsg);
