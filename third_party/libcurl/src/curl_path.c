...
CURLcode Curl_getworkingpath(struct Curl_easy* data, char* homedir, /* when SFTP is used */
                                             ...
                                             real path to work with */
{
    ...
    if ((data->conn->handler->protocol & CURLPROTO_SCP) && (working_path_len > 3) && (!memcmp(working_path, "/~/", 3))) {
        ...
        && (!strcmp("/~", working_path) || ((working_path_len > 2) && !memcmp(working_path, "/~/", 3)))) {
        ...
        if (working_path_len > 2) {
            ...
            if (Curl_dyn_addn(&npath, &working_path[copyfrom], working_path_len - copyfrom)) {
                free(working_path);
                return CURLE_OUT_OF_MEMORY;
            }
        }
