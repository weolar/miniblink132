...
{
#if !(defined(HAVE_GETADDRINFO) && defined(HAVE_GETADDRINFO_THREADSAFE)) && defined(HAVE_GETHOSTBYNAME_R_3)
    int res;
#endif
    struct Curl_addrinfo* ai = NULL;
   ...
   */
    ...
    (void)gethostbyname_r(hostname, (struct hostent*)buf, (char*)buf + sizeof(struct hostent), CURL_HOSTENT_SIZE - sizeof(struct hostent), &h, /* DIFFERENCE */
        &h_errnop);
