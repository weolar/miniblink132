...
static void zstd_version(char* buf, size_t bufsz)
{
    ...
    unsigned int minor = (unsigned int)((zstd_version - (major * 100 * 100)) / 100);
    unsigned int patch = (unsigned int)(zstd_version - (major * 100 * 100) - (minor * 100));
    (void)msnprintf(buf, bufsz, "%u.%u.%u", major, minor, patch);
}
#endif
...
char* curl_version(void)
{
...
#ifdef USE_LIBRTMP
    {
        ...
        msnprintf(rtmp_version, sizeof(rtmp_version), "librtmp/%d.%d%s", RTMP_LIB_VERSION >> 16, (RTMP_LIB_VERSION >> 8) & 0xff, suff);
        src[i++] = rtmp_version;
    }
#endif
#ifdef USE_HYPER
...
#endif
#ifdef USE_GSASL
    msnprintf(gsasl_buf, sizeof(gsasl_buf), "libgsasl/%s", gsasl_check_version(NULL));
    src[i++] = gsasl_buf;
#endif
#ifdef USE_OPENLDAP
    {
        ...
        if (ldap_get_option(NULL, LDAP_OPT_API_INFO, &api) == LDAP_OPT_SUCCESS) {
            ...
            unsigned int minor = ((api.ldapai_vendor_version - major * 10000) - patch) / 100;
            msnprintf(ldap_buf, sizeof(ldap_buf), "%s/%u.%u.%u", api.ldapai_vendor_name, major, minor, patch);
            src[i++] = ldap_buf;
...
#define FEATURE(name, present, bitmask)                                                                                                                        \
    {                                                                                                                                                          \
        (name), (present), (bitmask)                                                                                                                           \
    }
...
#endif
#if (SIZEOF_CURL_OFF_T > 4) && ((SIZEOF_OFF_T > 4) || defined(USE_WIN32_LARGE_FILES))
...
static const char* feature_names[sizeof(features_table) / sizeof(features_table[0])] = { NULL };

static curl_version_info_data version_info
    = { CURLVERSION_NOW, LIBCURL_VERSION, LIBCURL_VERSION_NUM, OS, /* as found by configure or set by hand at build-time */
          0, /* features bitmask is built at run-time */
          NULL, /* ssl_version */
          0, /* ssl_version_num, this is kept at zero */
          NULL, /* zlib_version */
          supported_protocols, NULL, /* c-ares version */
