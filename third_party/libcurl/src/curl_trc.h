...
#if defined(__GNUC__) && !defined(printf) && defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901L) && !defined(__MINGW32__)
...
#define Curl_trc_cf_is_verbose(cf, data) ((data) && (data)->set.verbose && (cf) && (cf)->cft->log_level >= CURL_LOG_LVL_INFO)

/**
 * Output an informational message when transfer's verbose logging is enabled.
 */
void Curl_infof(struct Curl_easy* data,
#if defined(__GNUC__) && !defined(printf) && defined(CURL_HAVE_C99) && !defined(__MINGW32__)
    const char* fmt, ...) __attribute__((format(printf, 2, 3)));
#else
    const char* fmt, ...);
#endif
