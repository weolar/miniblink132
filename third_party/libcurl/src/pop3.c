...
static CURLcode pop3_perform_auth(struct Curl_easy* data, const char* mech, const struct bufref* initresp);
static CURLcode pop3_continue_auth(struct Curl_easy* data, const char* mech, const struct bufref* resp);
...
const struct Curl_handler Curl_handler_pop3 = { "POP3", /* scheme */
...
static CURLcode pop3_continue_auth(struct Curl_easy* data, const char* mech, const struct bufref* resp)
{
    ...
    return Curl_pp_sendf(data, &pop3c->pp, "%s", (const char*)Curl_bufref_ptr(resp));
}
...
static CURLcode pop3_perform_command(struct Curl_easy* data)
{
    ...
    if (pop3->id[0] != '\0')
        ...
        result = Curl_pp_sendf(data, &conn->proto.pop3c.pp, "%s", (pop3->custom && pop3->custom[0] != '\0' ? pop3->custom : command));
...
static CURLcode pop3_state_user_resp(struct Curl_easy* data, int pop3code, pop3state instate)
{
    ...
    if (pop3code != '+') {
        ...
        result = Curl_pp_sendf(data, &conn->proto.pop3c.pp, "PASS %s", conn->passwd ? conn->passwd : "");
    if (!result)
