...
static CURLcode hsts_create(struct hsts* h, const char* hostname, bool subdomains, curl_off_t expires)
{
...
CURLcode Curl_hsts_parse(struct hsts* h, const char* hostname, const char* header)
{
    const char* p = header;
        ...
        for (e = h->list.head; e; e = n) {
            ...
            if (subdomain && sts->includeSubDomains) {
                ...
                if (ntail < hlen) {
                    ...
                    if ((hostname[offs - 1] == '.') && strncasecompare(&hostname[offs], sts->host, ntail))
                        return sts;
                }
            }
 ...
 */
static CURLcode hsts_push(struct Curl_easy* data, struct curl_index* i, struct stsentry* sts, bool* stop)
{
    ...
    if (sts->expires != TIME_T_MAX) {
        ...
        msnprintf(e.expire, sizeof(e.expire), "%d%02d%02d %02d:%02d:%02d", stamp.tm_year + 1900, stamp.tm_mon + 1, stamp.tm_mday, stamp.tm_hour, stamp.tm_min,
            stamp.tm_sec);
    } else
...
static CURLcode hsts_out(struct stsentry* sts, FILE* fp)
{
    ...
    if (sts->expires != TIME_T_MAX) {
        ...
        if (result)
            return result;
        fprintf(fp, "%s%s \"%d%02d%02d %02d:%02d:%02d\"\n", sts->includeSubDomains ? "." : "", sts->host, stamp.tm_year + 1900, stamp.tm_mon + 1, stamp.tm_mday,
 ...
 */
CURLcode Curl_hsts_save(struct Curl_easy* data, struct hsts* h, const char* file)
{
    struct Curl_llist_element* e;
    struct Curl_llist_element* n;
...
static CURLcode hsts_add(struct hsts* h, char* line)
{
   ...
   */
    ...
    rc = sscanf(line, "%" MAX_HSTS_HOSTLENSTR "s \"%" MAX_HSTS_DATELENSTR "[^\"]\"", host, date);
    if (2 == rc) {
        time_t expires = strcmp(date, UNLIMITED) ? Curl_getdate_capped(date) : TIME_T_MAX;
        CURLcode result = CURLE_OK;
        char* p = host;
 ...
 */
CURLcode Curl_hsts_loadfile(struct Curl_easy* data, struct hsts* h, const char* file)
{
    DEBUGASSERT(h);
    (void)data;
