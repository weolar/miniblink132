    ...
    if (sshc->kh && !data->set.str[STRING_SSH_HOST_PUBLIC_KEY_MD5]) {
        ...
        if (found) {
           ...
           */
            ...
            rc = libssh2_session_method_pref(sshc->ssh_session, LIBSSH2_METHOD_HOSTKEY, hostkey_method);
            if (rc) {
                char* errmsg = NULL;
                int errlen;
    ...
    do {
       ...
       */
         ...
         */
                rc = libssh2_sftp_stat_ex(
                    sshc->sftp_session, sshc->quote_path2, curlx_uztoui(strlen(sshc->quote_path2)), LIBSSH2_SFTP_STAT, &sshp->quote_attrs);
                if (rc == LIBSSH2_ERROR_EAGAIN) {
                    break;
                }
       ...
       */
            ...
            sshc->sftp_handle = libssh2_sftp_open_ex(
                sshc->sftp_session, sshp->path, curlx_uztoui(strlen(sshp->path)), flags, data->set.new_file_perms, LIBSSH2_SFTP_OPENFILE);
       ...
       */
             ...
             newline in this counter */
                    ...
                    if (!result) {
                        if ((sshp->readdir_attrs.flags & LIBSSH2_SFTP_ATTR_PERMISSIONS)
                            && ((sshp->readdir_attrs.permissions & LIBSSH2_SFTP_S_IFMT) == LIBSSH2_SFTP_S_IFLNK)) {
        ...
        case SSH_SFTP_READDIR_LINK:
            rc = libssh2_sftp_symlink_ex(sshc->sftp_session, Curl_dyn_ptr(&sshp->readdir_link), (int)Curl_dyn_len(&sshp->readdir_link), sshp->readdir_filename,
       ...
       */
            sshc->sftp_handle = libssh2_sftp_open_ex(
                sshc->sftp_session, sshp->path, curlx_uztoui(strlen(sshp->path)), LIBSSH2_FXF_READ, data->set.new_file_perms, LIBSSH2_SFTP_OPENFILE);
            if (!sshc->sftp_handle) {
                if (libssh2_session_last_errno(sshc->ssh_session) == LIBSSH2_ERROR_EAGAIN) {
                    rc = LIBSSH2_ERROR_EAGAIN;
                    break;
         ...
         */
                ...
                if (data->state.use_range) {
                    ...
                    if (from > size) {
                        failf(
                            data, "Offset (%" CURL_FORMAT_CURL_OFF_T ") was beyond file size (%" CURL_FORMAT_CURL_OFF_T ")", from, (curl_off_t)attrs.filesize);
                        return CURLE_BAD_DOWNLOAD_RESUME;
                    }
                    if (from > to) {
                        from = to;
...
static CURLcode ssh_connect(struct Curl_easy* data, bool* done)
{
    ...
    if (data->set.server_response_timeout > 0) {
        libssh2_session_set_read_timeout(sshc->ssh_session, data->set.server_response_timeout / 1000);
    }
#endif
#endif
