...
static int myssh_getsock(struct Curl_easy* data, struct connectdata* conn, curl_socket_t* sock);

static CURLcode myssh_setup_connection(struct Curl_easy* data, struct connectdata* conn);
...
)
{
    ...
    if (sshc->state != nowstate) {
        infof(data, "SSH %p state change from %s to %s (line %d)", (void*)sshc, names[sshc->state], names[nowstate], lineno);
    }
#endif
    ...
    do {
      ...
      ssh_set_log_level(SSH_LOG_PROTOCOL);
        ...
        case SSH_AUTHLIST: {
            ...
            if (sshc->auth_methods)
                infof(data, "SSH authentication methods available: %s%s%s%s", sshc->auth_methods & SSH_AUTH_METHOD_PUBLICKEY ? "public key, " : "",
       ...
       * (2) use the "default" keys. */
            if (data->set.str[STRING_SSH_PRIVATE_KEY]) {
                ...
                rc = ssh_pki_import_privkey_file(data->set.str[STRING_SSH_PRIVATE_KEY], data->set.ssl.key_passwd, NULL, NULL, &sshc->privkey);
                if (rc != SSH_OK) {
                    failf(data, "Could not load private key file %s", data->set.str[STRING_SSH_PRIVATE_KEY]);
                    MOVE_TO_ERROR_STATE(CURLE_LOGIN_DENIED);
                    break;
                }
                ...
                rc = ssh_userauth_publickey_auto(sshc->ssh_session, NULL, data->set.ssl.key_passwd);
                if (rc == SSH_AUTH_AGAIN) {
                    rc = SSH_AGAIN;
                    break;
                }
       ...
       */
        ...
        case SSH_SFTP_INIT:
            ...
            if (!sshc->sftp_session) {
                failf(data, "Failure initializing sftp session: %s", ssh_get_error(sshc->ssh_session));
       ...
       */
        ...
        case SSH_SFTP_QUOTE_SETSTAT:
            ...
            if (rc && !sshc->acceptfail) {
                Curl_safefree(sshc->quote_path1);
                Curl_safefree(sshc->quote_path2);
                failf(data, "Attempt to set SFTP stats failed: %s", ssh_get_error(sshc->ssh_session));
                state(data, SSH_SFTP_CLOSE);
                sshc->nextstate = SSH_NO_STATE;
        ...
        case SSH_SFTP_QUOTE_MKDIR:
            rc = sftp_mkdir(sshc->sftp_session, sshc->quote_path1, (mode_t)data->set.new_directory_perms);
            if (rc && !sshc->acceptfail) {
                Curl_safefree(sshc->quote_path1);
        ...
        case SSH_SFTP_QUOTE_STATVFS: {
            ...
            if (!statvfs && !sshc->acceptfail) {
                ...
                char* tmp = aprintf("statvfs:\n"
                                    ...
                                    "f_flag: %llu\n"
                                    "f_namemax: %llu\n",
                    statvfs->f_bsize, statvfs->f_frsize, statvfs->f_blocks, statvfs->f_bfree, statvfs->f_bavail, statvfs->f_files, statvfs->f_ffree,
                    statvfs->f_favail, statvfs->f_fsid, statvfs->f_flag, statvfs->f_namemax);
        ...
        case SSH_SFTP_UPLOAD_INIT: {
            ...
            sshc->sftp_file = sftp_open(sshc->sftp_session, protop->path, flags, (mode_t)data->set.new_file_perms);
            if (!sshc->sftp_file) {
        ...
        case SSH_SFTP_CREATE_DIRS_MKDIR:
            ...
            rc = sftp_mkdir(sshc->sftp_session, protop->path, (mode_t)data->set.new_directory_perms);
            *sshc->slash_pos = '/';
            ++sshc->slash_pos;
            if (rc < 0) {
                /*
         ...
         */
                err = sftp_get_error(sshc->sftp_session);
                if ((err != SSH_FX_FILE_ALREADY_EXISTS) && (err != SSH_FX_FAILURE) && (err != SSH_FX_PERMISSION_DENIED)) {
                    MOVE_TO_SFTP_CLOSE_STATE();
                    break;
                }
       ...
       */
            ...
            if (!sshc->sftp_dir) {
                failf(data, "Could not open directory for reading: %s", ssh_get_error(sshc->ssh_session));
                MOVE_TO_SFTP_CLOSE_STATE();
                break;
            }
        ...
        case SSH_SFTP_READDIR:
            ...
            if (sshc->readdir_attrs) {
                ...
                if (data->set.list_only) {
                    ...
                    result = Curl_client_write(data, CLIENTWRITE_BODY, tmpLine, sshc->readdir_len + 1);
                    free(tmpLine);
             ...
             newline in this counter */
                    ...
                    Curl_debug(data, CURLINFO_DATA_OUT, (char*)sshc->readdir_filename, sshc->readdir_len);
                } else {
                    ...
                    if ((sshc->readdir_attrs->flags & SSH_FILEXFER_ATTR_PERMISSIONS) && ((sshc->readdir_attrs->permissions & SSH_S_IFMT) == SSH_S_IFLNK)) {
                        sshc->readdir_linkPath = aprintf("%s%s", protop->path, sshc->readdir_filename);
        ...
        case SSH_SFTP_READDIR_BOTTOM:
            if (Curl_dyn_addn(&sshc->readdir_buf, "\n", 1))
                ...
                result = Curl_client_write(data, CLIENTWRITE_BODY, Curl_dyn_ptr(&sshc->readdir_buf), Curl_dyn_len(&sshc->readdir_buf));

            if (!result) {
                /* output debug output if that is requested */
                Curl_debug(data, CURLINFO_DATA_OUT, Curl_dyn_ptr(&sshc->readdir_buf), Curl_dyn_len(&sshc->readdir_buf));
       ...
       */
            ...
            sshc->sftp_file = sftp_open(sshc->sftp_session, protop->path, O_RDONLY, (mode_t)data->set.new_file_perms);
            if (!sshc->sftp_file) {
                failf(data, "Could not open remote file for reading: %s", ssh_get_error(sshc->ssh_session));
         ...
         */
                ...
                if (data->state.use_range) {
                    ...
                    if (from > size) {
                        failf(data, "Offset (%" CURL_FORMAT_CURL_OFF_T ") was beyond file size (%" CURL_FORMAT_CURL_OFF_T ")", from, size);
                        return CURLE_BAD_DOWNLOAD_RESUME;
                    }
                    if (from > to) {
                        from = to;
            ...
            if (data->state.resume_from) {
                if (data->state.resume_from < 0) {
                    ...
                    if ((curl_off_t)size < -data->state.resume_from) {
                        failf(data, "Offset (%" CURL_FORMAT_CURL_OFF_T ") was beyond file size (%" CURL_FORMAT_CURL_OFF_T ")", data->state.resume_from, size);
                        return CURLE_BAD_DOWNLOAD_RESUME;
                    }
                    /* download from where? */
       ...
       with both accordingly */
        ...
        case SSH_SCP_UPLOAD_INIT:
            ...
            rc = ssh_scp_push_file(sshc->scp_session, protop->path, data->state.infilesize, (int)data->set.new_file_perms);
            if (rc != SSH_OK) {
                err_msg = ssh_get_error(sshc->ssh_session);
                failf(data, "%s", err_msg);
                MOVE_TO_ERROR_STATE(CURLE_UPLOAD_FAILED);
        ...
        case SSH_SCP_SEND_EOF:
           ...
           */
                ...
                if (rc != SSH_OK) {
                    infof(data, "Failed to close libssh scp channel: %s", ssh_get_error(sshc->ssh_session));
                }
            }
...
static CURLcode myssh_block_statemach(struct Curl_easy* data, bool disconnect)
{
    ...
    while ((sshc->state != SSH_STOP) && !result) {
        ...
        if (block) {
            ...
            (void)Curl_socket_check(fd_read, CURL_SOCKET_BAD, CURL_SOCKET_BAD, left > 1000 ? 1000 : left);
        }
    }
...
static CURLcode myssh_connect(struct Curl_easy* data, bool* done)
{
    ...
    if (data->set.str[STRING_SSH_KNOWNHOSTS]) {
        ...
        rc = ssh_options_set(ssh->ssh_session, SSH_OPTIONS_KNOWNHOSTS, data->set.str[STRING_SSH_KNOWNHOSTS]);
        if (rc != SSH_OK) {
    ...
    if (data->set.ssh_compression) {
        rc = ssh_options_set(ssh->ssh_session, SSH_OPTIONS_COMPRESSION, "zlib,zlib@openssh.com,none");
        if (rc != SSH_OK) {
...
static ssize_t sftp_recv(struct Curl_easy* data, int sockindex, char* mem, size_t len, CURLcode* err)
{
    ...
    case 0:
        conn->proto.sshc.sftp_file_index = sftp_async_read_begin(conn->proto.sshc.sftp_file, (uint32_t)len);
    ...
    case 1:
        ...
        nread = sftp_async_read(conn->proto.sshc.sftp_file, mem, (uint32_t)len, conn->proto.sshc.sftp_file_index);
...
static void sftp_quote(struct Curl_easy* data)
{
   ...
   * functions.
   */
    if (strncasecompare(cmd, "chgrp ", 6) || strncasecompare(cmd, "chmod ", 6) || strncasecompare(cmd, "chown ", 6) || strncasecompare(cmd, "atime ", 6)
        ...
        if (result) {
            if (result == CURLE_OUT_OF_MEMORY)
                ...
                failf(data,
                    "Syntax error in chgrp/chmod/chown/atime/mtime: "
