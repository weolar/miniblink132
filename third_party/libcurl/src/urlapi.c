 ...
 * will fail.
 */
static CURLUcode urlencode_str(struct dynbuf* o, const char* url, size_t len, bool relative, bool query)
{
...
static CURLUcode junkscan(const char* url, size_t* urllen, unsigned int flags)
{
    static const char badbytes[] = {
        ...
        0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x7f, 0x00 /* null-terminate */
    };
    size_t n = strlen(url);
    size_t nfine;
 ...
 *
 */
static CURLUcode parse_hostname_login(struct Curl_URL* u, const char* login, size_t len, unsigned int flags, size_t* offset) /* to the host name */
{
   ...
   * ftp://user:password@ftp.my.site:8021/README */
    ...
    ccode = Curl_parse_login_details(login, ptr - login - 1, &userp, &passwdp, (h && (h->flags & PROTOPT_URLOPTIONS)) ? &optionsp : NULL);
    if (ccode) {
...
UNITTEST CURLUcode Curl_parse_port(struct Curl_URL* u, struct dynbuf* host, bool has_scheme)
{
    char* portptr;
...
static CURLUcode parse_authority(struct Curl_URL* u, const char* auth, size_t authlen, unsigned int flags, struct dynbuf* host, bool has_scheme)
{
    size_t offset;
    CURLUcode result;
...
CURLUcode Curl_url_set_authority(CURLU* u, const char* authority, unsigned int flags)
{
    CURLUcode result;
    struct dynbuf host;
    ...
    do {
        ...
        if (*input == '.') {
          ...
          that from the input buffer; otherwise, */
            ...
            else if (!strcmp(".", input) || !strcmp("..", input) || !strncmp(".?", input, 2) || !strncmp("..?", input, 3)) {
                *out = 0;
                break;
            } else
                dotdot = FALSE;
...
static CURLUcode parseurl(const char* url, CURLU* u, unsigned int flags)
{
    ...
    schemelen = Curl_is_absolute_url(url, schemebuf, sizeof(schemebuf), flags & (CURLU_GUESS_SCHEME | CURLU_DEFAULT_SCHEME));
     ...
     * This catches both "file:/c:" and "file:c:" */
        ...
        if (schemelen) {
            ...
            schemep = schemebuf;
            if (!Curl_builtin_scheme(schemep, CURL_ZERO_TERMINATED) && !(flags & CURLU_NON_SUPPORT_SCHEME)) {
...
CURLUcode curl_url_get(const CURLU* u, CURLUPart what, char** part, unsigned int flags)
{
    const char* ptr;
    ...
    case CURLUPART_URL: {
        ...
        if (u->scheme && strcasecompare("file", u->scheme)) {
           ...
           the default one for the scheme */
            ...
            url = aprintf("%s://%s%s%s%s%s%s%s%s%s%s%s%s%s%s", scheme, u->user ? u->user : "", u->password ? ":" : "", u->password ? u->password : "",
                options ? ";" : "", options ? options : "", (u->user || u->password || options) ? "@" : "", allochost ? allochost : u->host, port ? ":" : "",
                port ? port : "", u->path ? u->path : "/", (u->query && u->query[0]) ? "?" : "", (u->query && u->query[0]) ? u->query : "",
...
CURLUcode curl_url_set(CURLU* u, CURLUPart what, const char* part, unsigned int flags)
{
     ...
     * then replace the existing with the new. */
        if (Curl_is_absolute_url(part, NULL, 0, flags & (CURLU_GUESS_SCHEME | CURLU_DEFAULT_SCHEME)) || curl_url_get(u, CURLUPART_URL, &oldurl, flags)) {
        ...
        if (urlencode) {
            ...
            for (i = (const unsigned char*)part; *i; i++) {
                ...
                if ((*i == ' ') && plusencode) {
                    ...
                    if (result)
                        return CURLUE_OUT_OF_MEMORY;
                } else if (ISUNRESERVED(*i) || ((*i == '/') && urlskipslash) || ((*i == '=') && equalsencode)) {
                    if ((*i == '=') && equalsencode)
            ...
            while (*p) {
                ...
                if ((*p == '%') && ISXDIGIT(p[1]) && ISXDIGIT(p[2]) && (ISUPPER(p[1]) || ISUPPER(p[2]))) {
