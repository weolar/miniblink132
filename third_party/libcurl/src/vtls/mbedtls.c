...
static int mbedtls_bio_cf_write(void* bio, const unsigned char* buf, size_t blen)
{
    ...
    CURL_TRC_CF(data, cf, "mbedtls_bio_cf_out_write(len=%zu) -> %zd, err=%d", blen, nwritten, result);
...
static int mbedtls_bio_cf_read(void* bio, unsigned char* buf, size_t blen)
{
    ...
    CURL_TRC_CF(data, cf, "mbedtls_bio_cf_in_read(len=%zu) -> %zd, err=%d", blen, nread, result);
...
static const mbedtls_x509_crt_profile mbedtls_x509_crt_profile_fr = {
    ...
    MBEDTLS_X509_ID_FLAG(MBEDTLS_MD_SHA1) | MBEDTLS_X509_ID_FLAG(MBEDTLS_MD_RIPEMD160) | MBEDTLS_X509_ID_FLAG(MBEDTLS_MD_SHA224)
...
static CURLcode mbed_connect_step1(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct mbed_ssl_backend_data* backend = (struct mbed_ssl_backend_data*)connssl->backend;
    struct ssl_primary_config* conn_config = Curl_ssl_cf_get_primary_config(cf);
    ...
    if ((conn_config->version == CURL_SSLVERSION_SSLv2) || (conn_config->version == CURL_SSLVERSION_SSLv3)) {
    ...
    if (ca_info_blob && verifypeer) {
       ...
       extra memory here. */
        ...
        ret = mbedtls_x509_crt_parse(&backend->cacert, newblob, ca_info_blob->len + 1);
        free(newblob);
        if (ret < 0) {
            mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
            failf(data, "Error importing ca cert blob - mbedTLS: (-0x%04X) %s", -ret, errorbuf);
    ...
    if (ssl_cert_blob) {
       ...
       extra memory here. */
        ...
        ret = mbedtls_x509_crt_parse(&backend->clicert, newblob, ssl_cert_blob->len + 1);
        free(newblob);

        if (ret) {
            mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
            failf(data, "Error reading private key %s - mbedTLS: (-0x%04X) %s", ssl_config->key, -ret, errorbuf);
            return CURLE_SSL_CERTPROBLEM;
        }
    }
...
#ifdef MBEDTLS_FS_IO
#if MBEDTLS_VERSION_NUMBER >= 0x03000000
            ret = mbedtls_pk_parse_keyfile(&backend->pk, ssl_config->key, ssl_config->key_passwd, mbedtls_ctr_drbg_random, &backend->ctr_drbg);
#else
            ret = mbedtls_pk_parse_keyfile(&backend->pk, ssl_config->key, ssl_config->key_passwd);
#endif
...
#if MBEDTLS_VERSION_NUMBER >= 0x03000000
            ret = mbedtls_pk_parse_key(&backend->pk, key_data, ssl_key_blob->len, (const unsigned char*)passwd, passwd ? strlen(passwd) : 0,
    ...
    mbedtls_ssl_set_bio(&backend->ssl, cf, mbedtls_bio_cf_write, mbedtls_bio_cf_read, NULL /*  rev_timeout() */);
...
static CURLcode mbed_connect_step2(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    int ret;
    struct ssl_connect_data* connssl = cf->ctx;
    struct mbed_ssl_backend_data* backend = (struct mbed_ssl_backend_data*)connssl->backend;
    ...
    infof(data, "mbedTLS: Handshake complete, cipher is %s", mbedtls_ssl_get_ciphersuite(&backend->ssl));
...
#if MBEDTLS_VERSION_NUMBER == 0x03000000
        if (!peercert || !peercert->MBEDTLS_PRIVATE(raw).MBEDTLS_PRIVATE(p) || !peercert->MBEDTLS_PRIVATE(raw).MBEDTLS_PRIVATE(len)) {
#else
...
#if MBEDTLS_VERSION_NUMBER == 0x03000000
        if (mbedtls_x509_crt_parse_der(p, peercert->MBEDTLS_PRIVATE(raw).MBEDTLS_PRIVATE(p), peercert->MBEDTLS_PRIVATE(raw).MBEDTLS_PRIVATE(len))) {
#else
...
#if MBEDTLS_VERSION_NUMBER == 0x03000000
        size = mbedtls_pk_write_pubkey_der(&p->MBEDTLS_PRIVATE(pk), pubkey, PUB_DER_MAX_BYTES);
#else
...
static CURLcode mbed_connect_step3(struct Curl_cfilter* cf, struct Curl_easy* data)
{
...
static ssize_t mbed_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* mem, size_t len, CURLcode* curlcode)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct mbed_ssl_backend_data* backend = (struct mbed_ssl_backend_data*)connssl->backend;
    int ret = -1;
...
static ssize_t mbed_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t buffersize, CURLcode* curlcode)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct mbed_ssl_backend_data* backend = (struct mbed_ssl_backend_data*)connssl->backend;
    int ret = -1;
...
static CURLcode mbedtls_random(struct Curl_easy* data, unsigned char* entropy, size_t length)
{
    ...
    if (ret) {
        ...
        if (ret) {
            mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
            failf(data, "mbedtls_ctr_drbg_random returned (-0x%04X) %s", -ret, errorbuf);
        }
    }
...
static CURLcode mbed_connect_common(struct Curl_cfilter* cf, struct Curl_easy* data, bool nonblocking, bool* done)
{
    CURLcode retcode;
    struct ssl_connect_data* connssl = cf->ctx;
...
static CURLcode mbedtls_connect_nonblocking(struct Curl_cfilter* cf, struct Curl_easy* data, bool* done)
{
    return mbed_connect_common(cf, data, TRUE, done);
}

static CURLcode mbedtls_connect(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    CURLcode retcode;
    bool done = FALSE;
...
static bool mbedtls_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data)
{
...
const struct Curl_ssl Curl_ssl_mbedtls = {
    ...
    SSLSUPP_CA_PATH | SSLSUPP_CAINFO_BLOB | SSLSUPP_PINNEDPUBKEY | SSLSUPP_SSL_CTX | SSLSUPP_HTTPS_PROXY,
