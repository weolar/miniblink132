...
static CURLcode schannel_pkp_pin_peer_pubkey(struct Curl_cfilter* cf, struct Curl_easy* data, const char* pinnedpubkey);
...
static CURLcode schannel_set_ssl_version_min_max(DWORD* enabled_protocols, struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    */
        if (curlx_verify_windows_version(10, 0, 20348, PLATFORM_WINNT, VERSION_GREATER_THAN_EQUAL)) {
...
static CURLcode set_ssl_ciphers(SCHANNEL_CRED* schannel_cred, char* ciphers, ALG_ID* algIds)
{
    ...
    while (startCur && (0 != *startCur) && (algCount < NUM_CIPHERS)) {
        ...
        if (alg)
            algIds[algCount++] = alg;
        else if (!strncmp(startCur, "USE_STRONG_CRYPTO", sizeof("USE_STRONG_CRYPTO") - 1)
            || !strncmp(startCur, "SCH_USE_STRONG_CRYPTO", sizeof("SCH_USE_STRONG_CRYPTO") - 1))
...
#endif
static CURLcode schannel_acquire_credential_handle(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    struct schannel_ssl_backend_data* backend = (struct schannel_ssl_backend_data*)(connssl->backend);
    ...
    if (conn_config->verifypeer) {
        ...
        if (ssl_config->no_revoke) {
            ...
            flags |= SCH_CRED_IGNORE_NO_REVOCATION_CHECK | SCH_CRED_IGNORE_REVOCATION_OFFLINE | SCH_CRED_REVOCATION_CHECK_CHAIN;
        ...
        flags = SCH_CRED_MANUAL_CRED_VALIDATION | SCH_CRED_IGNORE_NO_REVOCATION_CHECK | SCH_CRED_IGNORE_REVOCATION_OFFLINE;
    ...
    if (data->set.ssl.primary.clientcert || data->set.ssl.primary.cert_blob) {
      ...
      */
            ...
            if (pszPassword) {
                ...
                if (curlx_verify_windows_version(6, 0, 0, PLATFORM_WINNT, VERSION_GREATER_THAN_EQUAL))
                    cert_store = PFXImportCertStore(&datablob, pszPassword, PKCS12_NO_PERSIST_KEY);
                else
            ...
            if (!CryptStringToBinary(
                    cert_thumbprint_str, CERT_THUMBPRINT_STR_LEN, CRYPT_STRING_HEX, cert_thumbprint_data, &cert_thumbprint.cbData, NULL, NULL)) {
                curlx_unicodefree(cert_path);
                CertCloseStore(cert_store, 0);
                return CURLE_SSL_CERTPROBLEM;
            }
   ...
   */
    ...
    if (!conn_config->cipher_list && curlx_verify_windows_version(10, 0, 17763, PLATFORM_WINNT, VERSION_GREATER_THAN_EQUAL)) {
     ...
     */
      ...
      */
            ...
            while (startCur && (0 != *startCur) && (algCount < remaining_ciphers)) {
                ...
                } else if (disable_aes_gcm_sha256 && !strcmp("TLS_AES_128_GCM_SHA256", tmp)) {
                    disable_aes_gcm_sha256 = FALSE;
                } else if (disable_chacha_poly && !strcmp("TLS_CHACHA20_POLY1305_SHA256", tmp)) {
                    disable_chacha_poly = FALSE;
                } else if (disable_aes_ccm_8_sha256 && !strcmp("TLS_AES_128_CCM_8_SHA256", tmp)) {
                    disable_aes_ccm_8_sha256 = FALSE;
                } else if (disable_aes_ccm_sha256 && !strcmp("TLS_AES_128_CCM_SHA256", tmp)) {
      ...
      */
            ...
            crypto_settings[crypto_settings_idx].cChainingModes = ARRAYSIZE(blocked_ccm_modes);
      ...
      */
         ...
         digest cipher suite (sha384 or sha256) */
            if (disable_aes_gcm_sha384 != disable_aes_gcm_sha256) {
                crypto_settings[crypto_settings_idx].eAlgorithmUsage = TlsParametersCngAlgUsageDigest;
                crypto_settings[crypto_settings_idx].strCngAlgId.Length = sizeof(disable_aes_gcm_sha384 ? BCRYPT_SHA384_ALGORITHM : BCRYPT_SHA256_ALGORITHM);
                crypto_settings[crypto_settings_idx].strCngAlgId.MaximumLength
                    = sizeof(disable_aes_gcm_sha384 ? BCRYPT_SHA384_ALGORITHM : BCRYPT_SHA256_ALGORITHM);
                crypto_settings[crypto_settings_idx].strCngAlgId.Buffer = (PWSTR)(disable_aes_gcm_sha384 ? BCRYPT_SHA384_ALGORITHM : BCRYPT_SHA256_ALGORITHM);
    ...
    */
        if (disable_chacha_poly) {
            crypto_settings[crypto_settings_idx].eAlgorithmUsage = TlsParametersCngAlgUsageCipher;
            crypto_settings[crypto_settings_idx].strCngAlgId.Length = sizeof(BCRYPT_CHACHA20_POLY1305_ALGORITHM);
            crypto_settings[crypto_settings_idx].strCngAlgId.MaximumLength = sizeof(BCRYPT_CHACHA20_POLY1305_ALGORITHM);
            crypto_settings[crypto_settings_idx].strCngAlgId.Buffer = (PWSTR)BCRYPT_CHACHA20_POLY1305_ALGORITHM;
            crypto_settings_idx++;
        }
        ...
        sspi_status = s_pSecFn->AcquireCredentialsHandle(
            NULL, (TCHAR*)UNISP_NAME, SECPKG_CRED_OUTBOUND, NULL, &credentials, NULL, NULL, &backend->cred->cred_handle, &backend->cred->time_stamp);
    } else {
       ...
       SCHANNEL_CRED is used. */
        ...
        sspi_status = s_pSecFn->AcquireCredentialsHandle(
            NULL, (TCHAR*)UNISP_NAME, SECPKG_CRED_OUTBOUND, NULL, &schannel_cred, NULL, NULL, &backend->cred->cred_handle, &backend->cred->time_stamp);
    }
...
static CURLcode schannel_connect_step1(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    DEBUGASSERT(backend);
    DEBUGF(infof(data, "schannel: SSL/TLS connection with %s port %d (step 1/3)", hostname, connssl->port));
    ...
    backend->use_alpn = connssl->alpn && !GetProcAddress(GetModuleHandle(TEXT("ntdll")), "wine_get_version")
        && curlx_verify_windows_version(6, 3, 0, PLATFORM_WINNT, VERSION_GREATER_THAN_EQUAL);
#else
...
#error "compiler too old to support requisite manual cert verify for Win CE"
    ...
    if (conn_config->CAfile || conn_config->ca_info_blob) {
        if (curlx_verify_windows_version(6, 1, 0, PLATFORM_WINNT, VERSION_GREATER_THAN_EQUAL)) {
            backend->use_manual_cred_validation = true;
        } else {
            failf(data,
    ...
    if (ssl_config->primary.sessionid) {
        ...
        if (!Curl_ssl_getsessionid(cf, data, (void**)&old_cred, NULL)) {
            ...
            DEBUGF(infof(data, "schannel: incremented credential handle refcount = %d", backend->cred->refcount));
        }
    ...
    if (backend->use_alpn) {
        ...
        *(unsigned int*)(void*)&alpn_buffer[cur] = SecApplicationProtocolNegotiationExt_ALPN;
        cur += (int)sizeof(unsigned int);
        ...
        *extension_len = *list_len + (unsigned short)sizeof(unsigned int) + (unsigned short)sizeof(unsigned short);
  ...
  */
    sspi_status = s_pSecFn->InitializeSecurityContext(&backend->cred->cred_handle, NULL, backend->cred->sni_hostname, backend->req_flags, 0, 0,
        (backend->use_alpn ? &inbuf_desc : NULL), 0, &backend->ctxt->ctxt_handle, &outbuf_desc, &backend->ret_flags, &backend->ctxt->time_stamp);
    ...
    written = Curl_conn_cf_send(cf->next, data, outbuf.pvBuffer, outbuf.cbBuffer, &result);
...
static CURLcode schannel_connect_step2(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    DEBUGF(infof(data, "schannel: SSL/TLS connection with %s port %d (step 2/3)", connssl->hostname, connssl->port));
    ...
    for (;;) {
        if (doread) {
            ...
            nread = Curl_conn_cf_recv(
                cf->next, data, (char*)(backend->encdata_buffer + backend->encdata_offset), backend->encdata_length - backend->encdata_offset, &result);
        ...
        DEBUGF(infof(data, "schannel: encrypted data buffer: offset %zu length %zu", backend->encdata_offset, backend->encdata_length));
        ...
        sspi_status = s_pSecFn->InitializeSecurityContext(&backend->cred->cred_handle, &backend->ctxt->ctxt_handle, backend->cred->sni_hostname,
            backend->req_flags, 0, 0, &inbuf_desc, 0, NULL, &outbuf_desc, &backend->ret_flags, &backend->ctxt->time_stamp);
        ...
        if (sspi_status == SEC_I_CONTINUE_NEEDED || sspi_status == SEC_E_OK) {
            for (i = 0; i < 3; i++) {
                ...
                if (outbuf[i].BufferType == SECBUFFER_TOKEN && outbuf[i].cbBuffer > 0) {
                    ...
                    written = Curl_conn_cf_send(cf->next, data, outbuf[i].pvBuffer, outbuf[i].cbBuffer, &result);
        ...
        */
        ...
        if (inbuf[1].BufferType == SECBUFFER_EXTRA && inbuf[1].cbBuffer > 0) {
            DEBUGF(infof(data, "schannel: encrypted data length: %lu", inbuf[1].cbBuffer));
            /*
...
{
    return (cert_context != NULL) && ((cert_context->dwCertEncodingType & X509_ASN_ENCODING) != 0) && (cert_context->pbCertEncoded != NULL)
        && (cert_context->cbCertEncoded > 0);
}

typedef bool (*Read_crt_func)(const CERT_CONTEXT* ccert_context, bool reverse_order, void* arg);
...
static bool cert_counter_callback(const CERT_CONTEXT* ccert_context, bool reverse_order, void* certs_count)
{
...
static bool add_cert_to_certinfo(const CERT_CONTEXT* ccert_context, bool reverse_order, void* raw_arg)
{
...
static CURLcode schannel_connect_step3(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    DEBUGF(infof(data, "schannel: SSL/TLS connection with %s port %d (step 3/3)", connssl->hostname, connssl->port));
    ...
    if (backend->use_alpn) {
        ...
        if (alpn_result.ProtoNegoStatus == SecApplicationProtocolNegotiationStatus_Success) {
            ...
            Curl_alpn_set_negotiated(cf, data, alpn_result.ProtocolId, alpn_result.ProtocolIdSize);
            if (backend->recv_renegotiating) {
                if (prev_alpn != cf->conn->alpn && prev_alpn != CURL_HTTP_VERSION_NONE) {
    ...
    if (ssl_config->primary.sessionid) {
        ...
        if (!incache) {
            result = Curl_ssl_addsessionid(cf, data, backend->cred, sizeof(struct Curl_schannel_cred), &added);
            if (result) {
                Curl_ssl_sessionid_unlock(data);
...
static CURLcode schannel_connect_common(struct Curl_cfilter* cf, struct Curl_easy* data, bool nonblocking, bool* done)
{
    CURLcode result;
    struct ssl_connect_data* connssl = cf->ctx;
...
static ssize_t schannel_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len, CURLcode* err)
{
    ssize_t written = -1;
    size_t data_len = 0;
    ...
    */
        ...
        while (len > (size_t)written) {
            ...
            this_write = Curl_conn_cf_send(cf->next, data, ptr + written, len - written, &result);
            if (result == CURLE_AGAIN)
                continue;
...
static ssize_t schannel_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t len, CURLcode* err)
{
    size_t size = 0;
    ssize_t nread = -1;
    struct ssl_connect_data* connssl = cf->ctx;
  ...
  */
        ...
        if (size < CURL_SCHANNEL_BUFFER_FREE_SIZE || backend->encdata_length < min_encdata_length) {
            ...
            backend->encdata_length = reallocated_length;
            size = backend->encdata_length - backend->encdata_offset;
            DEBUGF(infof(data, "schannel: encdata_buffer resized %zu", backend->encdata_length));
        }

        DEBUGF(infof(data, "schannel: encrypted data buffer: offset %zu length %zu", backend->encdata_offset, backend->encdata_length));

        /* read encrypted data from socket */
        nread = Curl_conn_cf_recv(cf->next, data, (char*)(backend->encdata_buffer + backend->encdata_offset), size, err);
        if (*err) {
            nread = -1;
            if (*err == CURLE_AGAIN)
       ...
       or shutdown the connection context) */
        if (sspi_status == SEC_E_OK || sspi_status == SEC_I_RENEGOTIATE || sspi_status == SEC_I_CONTEXT_EXPIRED) {
         ...
         renegotiation or shutdown of the connection context */
            if (inbuf[1].BufferType == SECBUFFER_DATA) {
                DEBUGF(infof(data, "schannel: decrypted data length: %lu", inbuf[1].cbBuffer));
                ...
                DEBUGF(infof(data, "schannel: decrypted cached: offset %zu length %zu", backend->decdata_offset, backend->decdata_length));
            }

            /* check for remaining encrypted data */
            if (inbuf[3].BufferType == SECBUFFER_EXTRA && inbuf[3].cbBuffer > 0) {
                DEBUGF(infof(data, "schannel: encrypted data length: %lu", inbuf[3].cbBuffer));
         ...
         */
             ...
             buffer */
                ...
                DEBUGF(infof(data, "schannel: encrypted cached: offset %zu length %zu", backend->encdata_offset, backend->encdata_length));
            } else {
            ...
            *err = CURLE_RECV_ERROR;
            infof(data, "schannel: failed to read data from server: %s", Curl_sspi_strerror(sspi_status, buffer, sizeof(buffer)));
            goto cleanup;
        }
    }
    ...
    DEBUGF(infof(data, "schannel: decrypted data buffer: offset %zu length %zu", backend->decdata_offset, backend->decdata_length));
...
cleanup:
  ...
  */
    if (len && !backend->decdata_offset && backend->recv_connection_closed && !backend->recv_sspi_close_notify) {
        bool isWin2k = curlx_verify_windows_version(5, 0, 0, PLATFORM_WINNT, VERSION_EQUAL);
...
static CURLcode schannel_connect_nonblocking(struct Curl_cfilter* cf, struct Curl_easy* data, bool* done)
{
    return schannel_connect_common(cf, data, TRUE, done);
}

static CURLcode schannel_connect(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    CURLcode result;
    bool done = FALSE;
...
static bool schannel_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data)
{
...
static int schannel_shutdown(struct Curl_cfilter* cf, struct Curl_easy* data)
{
   ...
   */
    ...
    if (backend->ctxt) {
        infof(data, "schannel: shutting down SSL/TLS connection with %s port %d", connssl->hostname, connssl->port);
    }
    ...
    if (backend->cred && backend->ctxt) {
        ...
        sspi_status = s_pSecFn->InitializeSecurityContext(&backend->cred->cred_handle, &backend->ctxt->ctxt_handle, backend->cred->sni_hostname,
            backend->req_flags, 0, 0, NULL, 0, &backend->ctxt->ctxt_handle, &outbuf_desc, &backend->ret_flags, &backend->ctxt->time_stamp);

        if ((sspi_status == SEC_E_OK) || (sspi_status == SEC_I_CONTEXT_EXPIRED)) {
            /* send close message which is in output buffer */
            ssize_t written = Curl_conn_cf_send(cf->next, data, outbuf.pvBuffer, outbuf.cbBuffer, &result);
            s_pSecFn->FreeContextBuffer(outbuf.pvBuffer);
            if ((result != CURLE_OK) || (outbuf.cbBuffer != (size_t)written)) {
                infof(data,
...
static void schannel_checksum(
    const unsigned char* input, size_t inputlen, unsigned char* checksum, size_t checksumlen, DWORD provType, const unsigned int algId)
{
    HCRYPTPROV hProv = 0;
    HCRYPTHASH hHash = 0;
    DWORD cbHashSize = 0;
