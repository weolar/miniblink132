...
static void wolfssl_log_tls12_secret(SSL* ssl)
{
   ...
   * key log line when TLS 1.3 is actually negotiated. */
    ...
    if (wolfSSL_get_keys(ssl, &ms, &msLen, &sr, &srLen, &cr, &crLen) != SSL_SUCCESS) {
        return;
    }
...
static const struct group_name_map gnm[] = { { WOLFSSL_KYBER_LEVEL1, "KYBER_LEVEL1" }, { WOLFSSL_KYBER_LEVEL3, "KYBER_LEVEL3" },
    { WOLFSSL_KYBER_LEVEL5, "KYBER_LEVEL5" }, { WOLFSSL_P256_KYBER_LEVEL1, "P256_KYBER_LEVEL1" }, { WOLFSSL_P384_KYBER_LEVEL3, "P384_KYBER_LEVEL3" },
    { WOLFSSL_P521_KYBER_LEVEL5, "P521_KYBER_LEVEL5" }, { 0, NULL } };
#endif
...
static int wolfssl_bio_cf_out_write(WOLFSSL_BIO* bio, const char* buf, int blen)
{
    struct Curl_cfilter* cf = wolfSSL_BIO_get_data(bio);
    struct ssl_connect_data* connssl = cf->ctx;
    struct wolfssl_ssl_backend_data* backend = (struct wolfssl_ssl_backend_data*)connssl->backend;
...
#define use_sni(x) Curl_nop_stmt
    ...
    */
        if ((wolfSSL_CTX_SetMinVersion(backend->ctx, WOLFSSL_TLSV1) != 1) && (wolfSSL_CTX_SetMinVersion(backend->ctx, WOLFSSL_TLSV1_1) != 1)
    ...
    if (ca_info_blob) {
        if (wolfSSL_CTX_load_verify_buffer(backend->ctx, ca_info_blob->data, ca_info_blob->len, SSL_FILETYPE_PEM) != SSL_SUCCESS) {
    ...
    if (ssl_cafile || ssl_capath) {
        int rc = wolfSSL_CTX_load_verify_locations_ex(backend->ctx, ssl_cafile, ssl_capath, WOLFSSL_LOAD_FLAG_IGNORE_ERR);
        if (SSL_SUCCESS != rc) {
            if (conn_config->verifypeer && !imported_ca_info_blob && !imported_native_ca) {
    ...
    if (ssl_config->primary.clientcert && ssl_config->key) {
        ...
        if (wolfSSL_CTX_use_certificate_file(backend->ctx, ssl_config->primary.clientcert, file_type) != 1) {
            failf(data,
        ...
        if (wolfSSL_CTX_use_PrivateKey_file(backend->ctx, ssl_config->key, file_type) != 1) {
   ...
   * SSL_get_verify_result() below. */
    wolfSSL_CTX_set_verify(backend->ctx, conn_config->verifypeer ? SSL_VERIFY_PEER : SSL_VERIFY_NONE, NULL);
    ...
    if (sni) {
        ...
        if ((hostname_len < USHRT_MAX) && !Curl_inet_pton(AF_INET, connssl->hostname, &addr4)
#ifdef ENABLE_IPV6
            ...
            if (!snihost || wolfSSL_CTX_UseSNI(backend->ctx, WOLFSSL_SNI_HOST_NAME, snihost, (unsigned short)snilen) != 1) {
    ...
    if (connssl->alpn) {
        ...
        if (result || wolfSSL_UseALPN(backend->handle, (char*)proto.data, proto.len, WOLFSSL_ALPN_CONTINUE_ON_MISMATCH) != SSL_SUCCESS) {
    ...
    if (!wolfSSL_set_fd(backend->handle, (int)Curl_conn_cf_get_socket(cf, data))) {
...
static CURLcode wolfssl_connect_step2(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    if (conn_config->verifyhost) {
        ...
        if (!snihost || (wolfSSL_check_domain_name(backend->handle, snihost) == SSL_FAILURE))
            return CURLE_SSL_CONNECT_ERROR;
    }
     ...
     * as also mismatching CN fields */
...
#if 1
            failf(data, " subject alt name(s) or common name do not match \"%s\"", connssl->dispname);
       ...
       * 'ssl_config.verifyhost' value. */
            if (conn_config->verifyhost) {
                ...
                infof(data, " subject alt name(s) and/or common name do not match \"%s\"", connssl->dispname);
                return CURLE_OK;
            }
#endif
        }
           ...
           verification is required. */
            ...
            failf(data, "SSL_connect failed with error %d: %s", detail, wolfSSL_ERR_error_string(detail, error_buffer));
            return CURLE_SSL_CONNECT_ERROR;
        }
    }
    ...
    if (connssl->alpn) {
        ...
        if (rc == SSL_SUCCESS) {
            Curl_alpn_set_negotiated(cf, data, (const unsigned char*)protocol, protocol_len);
    ...
    infof(data, "SSL connection using %s / %s", wolfSSL_get_version(backend->handle), wolfSSL_get_cipher_name(backend->handle));
#else
...
static ssize_t wolfssl_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* mem, size_t len, CURLcode* curlcode)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct wolfssl_ssl_backend_data* backend = (struct wolfssl_ssl_backend_data*)connssl->backend;
    char error_buffer[WOLFSSL_MAX_ERROR_SZ];
    ...
    if (rc <= 0) {
        ...
        default:
            ...
            failf(data, "SSL write: %s, errno %d", wolfSSL_ERR_error_string(err, error_buffer), SOCKERRNO);
...
static ssize_t wolfssl_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t blen, CURLcode* curlcode)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct wolfssl_ssl_backend_data* backend = (struct wolfssl_ssl_backend_data*)connssl->backend;
    char error_buffer[WOLFSSL_MAX_ERROR_SZ];
    ...
    *curlcode = CURLE_OK;
    ...
    if (nread <= 0) {
        ...
        default:
            ...
            failf(data, "SSL read: %s, errno %d", wolfSSL_ERR_error_string(err, error_buffer), SOCKERRNO);
...
static bool wolfssl_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data)
{
...
static CURLcode wolfssl_connect_common(struct Curl_cfilter* cf, struct Curl_easy* data, bool nonblocking, bool* done)
{
    CURLcode result;
    struct ssl_connect_data* connssl = cf->ctx;
...
static CURLcode wolfssl_connect_nonblocking(struct Curl_cfilter* cf, struct Curl_easy* data, bool* done)
{
    return wolfssl_connect_common(cf, data, TRUE, done);
}

static CURLcode wolfssl_connect(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    CURLcode result;
    bool done = FALSE;
