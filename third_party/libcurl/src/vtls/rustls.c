...
static bool cr_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data)
{
...
static CURLcode cr_connect(struct Curl_cfilter* cf UNUSED_PARAM, struct Curl_easy* data UNUSED_PARAM)
{
...
static int read_cb(void* userdata, uint8_t* buf, uintptr_t len, uintptr_t* out_n)
{
    struct io_ctx* io_ctx = userdata;
    CURLcode result;
    int ret = 0;
    ssize_t nread = Curl_conn_cf_recv(io_ctx->cf->next, io_ctx->data, (char*)buf, len, &result);
    if (nread < 0) {
        nread = 0;
...
static int write_cb(void* userdata, const uint8_t* buf, uintptr_t len, uintptr_t* out_n)
{
    struct io_ctx* io_ctx = userdata;
    CURLcode result;
    int ret = 0;
    ssize_t nwritten = Curl_conn_cf_send(io_ctx->cf->next, io_ctx->data, (const char*)buf, len, &result);
    if (nwritten < 0) {
        nwritten = 0;
        if (CURLE_AGAIN == result)
            ret = EAGAIN;
...
static ssize_t tls_recv_more(struct Curl_cfilter* cf, struct Curl_easy* data, CURLcode* err)
{
    struct ssl_connect_data* const connssl = cf->ctx;
    struct rustls_ssl_backend_data* const backend = (struct rustls_ssl_backend_data*)connssl->backend;
    ...
    io_ctx.cf = cf;
    io_ctx.data = data;
    io_error = rustls_connection_read_tls(backend->conn, read_cb, &io_ctx, &tls_bytes_read);
    if (io_error == EAGAIN || io_error == EWOULDBLOCK) {
        *err = CURLE_AGAIN;
        return -1;
    } else if (io_error) {
        char buffer[STRERROR_LEN];
        failf(data, "reading from socket: %s", Curl_strerror(io_error, buffer, sizeof(buffer)));
    ...
    if (rresult != RUSTLS_RESULT_OK) {
        ...
        failf(data, "rustls_connection_process_new_packets: %.*s", errorlen, errorbuf);
 ...
 * output buffer.
 */
static ssize_t cr_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* plainbuf, size_t plainlen, CURLcode* err)
{
    struct ssl_connect_data* const connssl = cf->ctx;
    struct rustls_ssl_backend_data* const backend = (struct rustls_ssl_backend_data*)connssl->backend;
    ...
    while (plain_bytes_copied < plainlen) {
        ...
        rresult = rustls_connection_read(rconn, (uint8_t*)plainbuf + plain_bytes_copied, plainlen - plain_bytes_copied, &n);
        if (rresult == RUSTLS_RESULT_PLAINTEXT_EMPTY) {
            backend->data_pending = FALSE;
        } else if (rresult == RUSTLS_RESULT_UNEXPECTED_EOF) {
            failf(data,
                "rustls: peer closed TCP connection "
 ...
 */
static ssize_t cr_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* plainbuf, size_t plainlen, CURLcode* err)
{
    struct ssl_connect_data* const connssl = cf->ctx;
    struct rustls_ssl_backend_data* const backend = (struct rustls_ssl_backend_data*)connssl->backend;
    struct rustls_connection* rconn = NULL;
    ...
    while (rustls_connection_wants_write(rconn)) {
        ...
        if (io_error == EAGAIN || io_error == EWOULDBLOCK) {
            CURL_TRC_CF(data, cf, "cf_send: EAGAIN after %zu bytes", tlswritten_total);
            *err = CURLE_AGAIN;
            return -1;
        } else if (io_error) {
            char buffer[STRERROR_LEN];
            failf(data, "writing to socket: %s", Curl_strerror(io_error, buffer, sizeof(buffer)));
            *err = CURLE_WRITE_ERROR;
            return -1;
        }
        if (tlswritten == 0) {
...
static CURLcode cr_init_backend(struct Curl_cfilter* cf, struct Curl_easy* data, struct rustls_ssl_backend_data* const backend)
{
    ...
    if (connssl->alpn) {
        ...
        rustls_client_config_builder_set_alpn_protocols(config_builder, alpn, connssl->alpn->count);
        Curl_alpn_to_proto_str(&proto, connssl->alpn);
        infof(data, VTLS_INFOF_ALPN_OFFER_1STR, proto.data);
    }
    if (!verifypeer) {
        rustls_client_config_builder_dangerous_set_certificate_verifier(config_builder, cr_verify_none);
     ...
     * SNI. */
        ...
        result = rustls_root_cert_store_add_pem(roots, ca_info_blob->data, ca_info_blob->len, verifypeer);
...
static void cr_set_negotiated_alpn(struct Curl_cfilter* cf, struct Curl_easy* data, const struct rustls_connection* rconn)
{
...
static CURLcode cr_connect_nonblocking(struct Curl_cfilter* cf, struct Curl_easy* data, bool* done)
{
    struct ssl_connect_data* const connssl = cf->ctx;
    ...
    if (ssl_connection_none == connssl->state) {
        result = cr_init_backend(cf, data, (struct rustls_ssl_backend_data*)connssl->backend);
        if (result != CURLE_OK) {
            return result;
        }
    ...
    */
        ...
        if (0 == what) {
            infof(data, "Curl_socket_check: %s would block", wants_read && wants_write ? "writing and reading" : wants_write ? "writing" : "reading");
            *done = FALSE;
            return CURLE_OK;
        }
...
static void cr_adjust_pollset(struct Curl_cfilter* cf, struct Curl_easy* data, struct easy_pollset* ps)
{
    if (!cf->connected) {
...
{
    struct rustls_ssl_backend_data* backend = (struct rustls_ssl_backend_data*)connssl->backend;
    DEBUGASSERT(backend);
    return &backend->conn;
}
