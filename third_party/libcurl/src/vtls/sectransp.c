...
static const unsigned char ecDsaSecp256r1SpkiHeader[] = { 0x30, 0x59, 0x30, 0x13, 0x06, 0x07, 0x2a, 0x86, 0x48, 0xce, 0x3d, 0x02, 0x01, 0x06, 0x08, 0x2a, 0x86,
...
static OSStatus sectransp_bio_cf_in_read(SSLConnectionRef connection, void* buf, size_t* dataLength) /* IN/OUT */
{
    ...
    CURL_TRC_CF(data, cf, "bio_read(len=%zu) -> %zd, result=%d", *dataLength, nread, result);
    if (nread < 0) {
        switch (result) {
        case CURLE_OK:
        case CURLE_AGAIN:
...
static OSStatus sectransp_bio_cf_out_write(SSLConnectionRef connection, const void* buf, size_t* dataLength) /* IN/OUT */
{
    struct Curl_cfilter* cf = (struct Curl_cfilter*)connection;
    struct ssl_connect_data* connssl = cf->ctx;
    struct st_ssl_backend_data* backend = (struct st_ssl_backend_data*)connssl->backend;
    ...
    CURL_TRC_CF(data, cf, "bio_send(len=%zu) -> %zd, result=%d", *dataLength, nwritten, result);
    if (nwritten <= 0) {
...
CF_INLINE const char* TLSCipherNameForNumber(SSLCipherSuite cipher)
{
  ...
  */
    ...
    for (i = SSL_FORTEZZA_DMS_WITH_FORTEZZA_CBC_SHA + 1; i < NUM_OF_CIPHERS; ++i) {
...
static OSStatus CopyIdentityWithLabel(char* label, SecIdentityRef* out_cert_and_key)
{
    ...
    if (SecItemCopyMatching && kSecClassIdentity) {
        ...
        query_dict
            = CFDictionaryCreate(NULL, (const void**)keys, (const void**)values, 5L, &kCFCopyStringDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
        CFRelease(values[3]);
...
static CURLcode set_ssl_version_min_max(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct st_ssl_backend_data* backend = (struct st_ssl_backend_data*)connssl->backend;
...
static CURLcode sectransp_connect_step1(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct st_ssl_backend_data* backend = (struct st_ssl_backend_data*)connssl->backend;
    struct ssl_primary_config* conn_config = Curl_ssl_cf_get_primary_config(cf);
    ...
    if (connssl->alpn) {
        if (__builtin_available(macOS 10.13.4, iOS 11, tvOS 11, *)) {
            ...
            for (i = 0; i < connssl->alpn->count; ++i) {
                cstr = CFStringCreateWithCString(NULL, connssl->alpn->entries[i], kCFStringEncodingUTF8);
                if (!cstr)
                    return CURLE_OUT_OF_MEMORY;
       ...
       it as a file on disk */
        ...
        if ((err != noErr) && (is_cert_file || is_cert_data)) {
            if (!ssl_config->cert_type)
                infof(data,
                    "SSL: Certificate type not set, assuming "
                    "PKCS#12 format.");
            else if (!strcasecompare(ssl_config->cert_type, "P12")) {
                failf(data,
                    "SSL: The Security framework only supports "
                    "loading identities that are in PKCS#12 format.");
                return CURLE_SSL_CERTPROBLEM;
            }

            err = CopyIdentityFromPKCS12File(ssl_cert, ssl_cert_blob, ssl_config->key_passwd, &cert_and_key);
        }
  ...
  */
...
#if CURL_SUPPORT_MAC_10_8
        err = SSLSetEnableCertVerify(backend->ssl_ctx, conn_config->verifypeer ? true : false);
        if (err != noErr) {
   ...
   * Also: the verifyhost setting influences SNI usage */
    ...
    if (SSLSetSessionOption) {
        SSLSetSessionOption(backend->ssl_ctx, kSSLSessionOptionSendOneByteRecord, !ssl_config->enable_beast);
        SSLSetSessionOption(backend->ssl_ctx, kSSLSessionOptionFalseStart, ssl_config->falsestart); /* false start support */
    }
#endif /* CURL_BUILD_MAC_10_9 || CURL_BUILD_IOS_7 */
    ...
    if (ssl_config->primary.sessionid) {
        ...
        if (!Curl_ssl_getsessionid(cf, data, (void**)&ssl_sessionid, &ssl_sessionid_len)) {
       ...
       to starting the handshake. */
            ...
            CURLcode result;
            ssl_sessionid
                = aprintf("%s:%d:%d:%s:%d", ssl_cafile ? ssl_cafile : "(blob memory)", verifypeer, conn_config->verifyhost, connssl->hostname, connssl->port);
            ...
            result = Curl_ssl_addsessionid(cf, data, ssl_sessionid, ssl_sessionid_len, NULL);
            Curl_ssl_sessionid_unlock(data);
            if (result) {
    ...
    err = SSLSetIOFuncs(backend->ssl_ctx, sectransp_bio_cf_in_read, sectransp_bio_cf_out_write);
    if (err != noErr) {
...
static CURLcode verify_cert_buf(struct Curl_cfilter* cf, struct Curl_easy* data, const unsigned char* certbuf, size_t buflen, SSLContextRef ctx)
{
    int n = 0, rc;
    long res;
    unsigned char* der;
...
static CURLcode verify_cert(struct Curl_cfilter* cf, struct Curl_easy* data, const char* cafile, const struct curl_blob* ca_info_blob, SSLContextRef ctx)
{
    int result;
...
{ /* Scratch */
    ...
    do {
        ...
        {
            OSStatus success;
            success = SecItemExport(keyRef, kSecFormatOpenSSL, 0, NULL, &publicKeyBits);
            CFRelease(keyRef);
...
static CURLcode sectransp_connect_step2(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct st_ssl_backend_data* backend = (struct st_ssl_backend_data*)connssl->backend;
    struct ssl_primary_config* conn_config = Curl_ssl_cf_get_primary_config(cf);
...
check_handshake:
    ...
    if (err != noErr) {
        ...
        Leopard's headers */
        case -9841:
            if ((conn_config->CAfile || conn_config->ca_info_blob) && conn_config->verifypeer) {
                CURLcode result = verify_cert(cf, data, conn_config->CAfile, conn_config->ca_info_blob, backend->ssl_ctx);
                if (result)
                    return result;
            }
        ...
        case errSSLClientCertRequested:
            failf(data,
                "Server requested a client certificate during the "
                "handshake");
        ...
        default:
            ...
            failf(data, "Unknown SSL protocol error in connection to %s:%d", connssl->hostname, err);
            break;
        }
        return CURLE_SSL_CONNECT_ERROR;
    } else {
        ...
        switch (protocol) {
        case kSSLProtocol2:
            infof(data, "SSL 2.0 connection using %s", TLSCipherNameForNumber(cipher));
            break;
        case kSSLProtocol3:
            infof(data, "SSL 3.0 connection using %s", TLSCipherNameForNumber(cipher));
            break;
        case kTLSProtocol1:
            infof(data, "TLS 1.0 connection using %s", TLSCipherNameForNumber(cipher));
            break;
#if CURL_BUILD_MAC_10_8 || CURL_BUILD_IOS
        case kTLSProtocol11:
            infof(data, "TLS 1.1 connection using %s", TLSCipherNameForNumber(cipher));
            break;
        case kTLSProtocol12:
            infof(data, "TLS 1.2 connection using %s", TLSCipherNameForNumber(cipher));
            break;
#endif /* CURL_BUILD_MAC_10_8 || CURL_BUILD_IOS */
#if CURL_BUILD_MAC_10_13 || CURL_BUILD_IOS_11
        case kTLSProtocol13:
            infof(data, "TLS 1.3 connection using %s", TLSCipherNameForNumber(cipher));
            break;
#endif /* CURL_BUILD_MAC_10_13 || CURL_BUILD_IOS_11 */
        default:
        ...
        if (connssl->alpn) {
            if (__builtin_available(macOS 10.13.4, iOS 11, tvOS 11, *)) {
                ...
                Curl_multiuse_state(data, cf->conn->alpn == CURL_HTTP_VERSION_2 ? BUNDLE_MULTIPLEX : BUNDLE_NO_MULTIUSE);
...
static CURLcode add_cert_to_certinfo(struct Curl_easy* data, SecCertificateRef server_cert, int idx)
{
    CURLcode result = CURLE_OK;
    const char* beg;
    const char* end;
...
static CURLcode collect_server_cert_single(struct Curl_cfilter* cf, struct Curl_easy* data, SecCertificateRef server_cert, CFIndex idx)
{
    CURLcode result = CURLE_OK;
    struct ssl_config_data* ssl_config = Curl_ssl_cf_get_config(cf, data);
#ifndef CURL_DISABLE_VERBOSE_STRINGS
...
static CURLcode collect_server_cert(struct Curl_cfilter* cf, struct Curl_easy* data)
{
#ifndef CURL_DISABLE_VERBOSE_STRINGS
...
#endif
    struct ssl_config_data* ssl_config = Curl_ssl_cf_get_config(cf, data);
    CURLcode result = ssl_config->certinfo ? CURLE_PEER_FAILED_VERIFICATION : CURLE_OK;
    struct ssl_connect_data* connssl = cf->ctx;
    struct st_ssl_backend_data* backend = (struct st_ssl_backend_data*)connssl->backend;
...
static CURLcode sectransp_connect_step3(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct ssl_connect_data* connssl = cf->ctx;
    CURLcode result;
...
static CURLcode sectransp_connect_common(struct Curl_cfilter* cf, struct Curl_easy* data, bool nonblocking, bool* done)
{
    CURLcode result;
    struct ssl_connect_data* connssl = cf->ctx;
...
static CURLcode sectransp_connect_nonblocking(struct Curl_cfilter* cf, struct Curl_easy* data, bool* done)
{
    return sectransp_connect_common(cf, data, TRUE, done);
}

static CURLcode sectransp_connect(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    CURLcode result;
    bool done = FALSE;
...
static ssize_t sectransp_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* mem, size_t len, CURLcode* curlcode)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct st_ssl_backend_data* backend = (struct st_ssl_backend_data*)connssl->backend;
...
static ssize_t sectransp_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t buffersize, CURLcode* curlcode)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct st_ssl_backend_data* backend = (struct st_ssl_backend_data*)connssl->backend;
    struct ssl_primary_config* conn_config = Curl_ssl_cf_get_primary_config(cf);
