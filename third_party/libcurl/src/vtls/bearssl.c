...
static CURLcode load_cafile(struct cafile_source* source, br_x509_trust_anchor** anchors, size_t* anchors_len)
{
    ...
    DEBUGASSERT(source->type == CAFILE_SOURCE_PATH || source->type == CAFILE_SOURCE_BLOB);
...
static CURLcode bearssl_set_selected_ciphers(struct Curl_easy* data, br_ssl_engine_context* ssl_eng, const char* ciphers)
{
    ...
    while (true) {
       ...
       to match cipher name by an (OpenSSL) alias. */
        if (strncasecompare(cipher_name, "TLS_", 4)) {
            for (i = 0; i < NUM_OF_CIPHERS && !strcasecompare(cipher_name, ciphertable[i].name); ++i)
                ;
        } else {
            for (i = 0; i < NUM_OF_CIPHERS && !strcasecompare(cipher_name, ciphertable[i].alias_name); ++i)
                ;
        }
        if (i == NUM_OF_CIPHERS) {
        ...
        for (j = 0; j < selected_count && selected_ciphers[j] != ciphertable[i].num; j++)
            ;
        if (j < selected_count) {
...
static CURLcode bearssl_connect_step1(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct bearssl_ssl_backend_data* backend = (struct bearssl_ssl_backend_data*)connssl->backend;
    ...
    if (conn_config->cipher_list) {
       ...
       BearSSL source code of br_ssl_client_init_full() */
        ...
        ret = bearssl_set_selected_ciphers(data, &backend->ctx.eng, conn_config->cipher_list);
        if (ret)
            return ret;
    }
    ...
    if (connssl->alpn) {
        ...
        br_ssl_engine_set_protocol_names(&backend->ctx.eng, backend->protocols, connssl->alpn->count);
...
static void bearssl_adjust_pollset(struct Curl_cfilter* cf, struct Curl_easy* data, struct easy_pollset* ps)
{
    if (!cf->connected) {
...
static CURLcode bearssl_run_until(struct Curl_cfilter* cf, struct Curl_easy* data, unsigned target)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct bearssl_ssl_backend_data* backend = (struct bearssl_ssl_backend_data*)connssl->backend;
    unsigned state;
...
static CURLcode bearssl_connect_step2(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct bearssl_ssl_backend_data* backend = (struct bearssl_ssl_backend_data*)connssl->backend;
    CURLcode ret;
...
static CURLcode bearssl_connect_step3(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct bearssl_ssl_backend_data* backend = (struct bearssl_ssl_backend_data*)connssl->backend;
    struct ssl_config_data* ssl_config = Curl_ssl_cf_get_config(cf, data);
...
static ssize_t bearssl_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len, CURLcode* err)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct bearssl_ssl_backend_data* backend = (struct bearssl_ssl_backend_data*)connssl->backend;
    unsigned char* app;
...
static ssize_t bearssl_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t len, CURLcode* err)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct bearssl_ssl_backend_data* backend = (struct bearssl_ssl_backend_data*)connssl->backend;
    unsigned char* app;
...
static CURLcode bearssl_connect_common(struct Curl_cfilter* cf, struct Curl_easy* data, bool nonblocking, bool* done)
{
    CURLcode ret;
    struct ssl_connect_data* connssl = cf->ctx;
...
static bool bearssl_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data)
{
...
static CURLcode bearssl_connect(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    CURLcode ret;
    bool done = FALSE;
...
static CURLcode bearssl_connect_nonblocking(struct Curl_cfilter* cf, struct Curl_easy* data, bool* done)
{
...
const struct Curl_ssl Curl_ssl_bearssl = {
    ...
    SSLSUPP_CAINFO_BLOB | SSLSUPP_SSL_CTX | SSLSUPP_HTTPS_PROXY, sizeof(struct bearssl_ssl_backend_data),
