...
static ssize_t gtls_push(void* s, const void* buf, size_t blen)
{
    ...
    if (nwritten < 0) {
        struct gtls_ssl_backend_data* backend = (struct gtls_ssl_backend_data*)connssl->backend;
        gnutls_transport_set_errno(backend->gtls.session, (CURLE_AGAIN == result) ? EAGAIN : EINVAL);
        nwritten = -1;
    }
    return nwritten;
}
...
static void showtime(struct Curl_easy* data, const char* text, time_t stamp)
{
    ...
    msnprintf(str, sizeof(str), "  %s: %s, %02d %s %4d %02d:%02d:%02d GMT", text, Curl_wkday[tm->tm_wday ? tm->tm_wday - 1 : 6], tm->tm_mday,
...
static CURLcode handshake(struct Curl_cfilter* cf, struct Curl_easy* data, bool duringconnect, bool nonblocking)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct gtls_ssl_backend_data* backend = (struct gtls_ssl_backend_data*)connssl->backend;
...
static CURLcode set_ssl_version_min_max(struct Curl_easy* data, struct ssl_primary_config* conn_config, const char** prioritylist, const char* tls13support)
{
...
CURLcode gtls_client_init(struct Curl_easy* data, struct ssl_primary_config* config, struct ssl_config_data* ssl_config, const char* hostname,
    struct gtls_instance* gtls, long* pverifyresult)
{
    ...
    *pverifyresult = 0;
    ...
    if (config->verifypeer) {
        ...
        if (ssl_config->native_ca_store) {
            ...
            if (rc < 0)
                infof(data, "error reading native ca store (%s), continuing anyway", gnutls_strerror(rc));
            else {
        ...
        if (config->CAfile) {
            ...
            gnutls_certificate_set_verify_flags(gtls->cred, GNUTLS_VERIFY_ALLOW_X509_V1_CA_CRT);

            rc = gnutls_certificate_set_x509_trust_file(gtls->cred, config->CAfile, GNUTLS_X509_FMT_PEM);
            if (rc < 0) {
                infof(data, "error reading ca cert file %s (%s)%s", config->CAfile, gnutls_strerror(rc), (imported_native_ca ? ", continuing anyway" : ""));
        ...
        if (config->CApath) {
            ...
            rc = gnutls_certificate_set_x509_trust_dir(gtls->cred, config->CApath, GNUTLS_X509_FMT_PEM);
            if (rc < 0) {
                infof(data, "error reading ca cert file %s (%s)%s", config->CApath, gnutls_strerror(rc), (imported_native_ca ? ", continuing anyway" : ""));
                if (!imported_native_ca) {
                    *pverifyresult = rc;
                    return CURLE_SSL_CACERT_BADFILE;
                }
    ...
    if (config->CRLfile) {
        ...
        rc = gnutls_certificate_set_x509_crl_file(gtls->cred, config->CRLfile, GNUTLS_X509_FMT_PEM);
        if (rc < 0) {
            failf(data, "error reading crl file %s (%s)", config->CRLfile, gnutls_strerror(rc));
    ...
    if ((0 == Curl_inet_pton(AF_INET, hostname, &addr)) &&
        ...
        if (!snihost || gnutls_server_name_set(gtls->session, GNUTLS_NAME_DNS, snihost, snilen) < 0) {
   ...
   * GnuTLS version */
    ...
    if (config->version == CURL_SSLVERSION_SSLv2 || config->version == CURL_SSLVERSION_SSLv3) {
   ...
   * GnuTLS will disable TLS 1.3 support. */
    ...
    if (config->clientcert) {
        if (ssl_config->key_passwd) {
            const unsigned int supported_key_encryption_algorithms = GNUTLS_PKCS_USE_PKCS12_3DES | GNUTLS_PKCS_USE_PKCS12_ARCFOUR
                | GNUTLS_PKCS_USE_PKCS12_RC2_40 | GNUTLS_PKCS_USE_PBES2_3DES | GNUTLS_PKCS_USE_PBES2_AES_128 | GNUTLS_PKCS_USE_PBES2_AES_192
                | GNUTLS_PKCS_USE_PBES2_AES_256;
            rc = gnutls_certificate_set_x509_key_file2(gtls->cred, config->clientcert, ssl_config->key ? ssl_config->key : config->clientcert,
                do_file_type(ssl_config->cert_type), ssl_config->key_passwd, supported_key_encryption_algorithms);
            if (rc != GNUTLS_E_SUCCESS) {
            ...
            if (gnutls_certificate_set_x509_key_file(
                    gtls->cred, config->clientcert, ssl_config->key ? ssl_config->key : config->clientcert, do_file_type(ssl_config->cert_type))
                != GNUTLS_E_SUCCESS) {
    ...
    if (config->username) {
        rc = gnutls_credentials_set(gtls->session, GNUTLS_CRD_SRP, gtls->srp_client_cred);
        if (rc != GNUTLS_E_SUCCESS) {
...
#endif
    {
        rc = gnutls_credentials_set(gtls->session, GNUTLS_CRD_CERTIFICATE, gtls->cred);
        if (rc != GNUTLS_E_SUCCESS) {
            failf(data, "gnutls_credentials_set() failed: %s", gnutls_strerror(rc));
            return CURLE_SSL_CONNECT_ERROR;
        }
...
static CURLcode gtls_connect_step1(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct gtls_ssl_backend_data* backend = (struct gtls_ssl_backend_data*)connssl->backend;
    ...
    result = gtls_client_init(data, conn_config, ssl_config, connssl->hostname, &backend->gtls, pverifyresult);
    if (result)
        return result;
    ...
    if (connssl->alpn) {
        ...
        if (gnutls_alpn_set_protocols(backend->gtls.session, alpn, (unsigned)connssl->alpn->count, 0)) {
...
CURLcode Curl_gtls_verifyserver(struct Curl_easy* data, gnutls_session_t session, struct ssl_primary_config* config, struct ssl_config_data* ssl_config,
    const char* hostname, const char* dispname, const char* pinned_key)
{
...
#ifdef USE_GNUTLS_SRP
            if (ssl_config->primary.username && !config->verifypeer && gnutls_cipher_get(session)) {
        ...
        if (verify_status & GNUTLS_CERT_INVALID) {
            if (config->verifypeer) {
                failf(data,
                    ...
                    "CRLfile: %s",
                    config->CAfile ? config->CAfile : "none", ssl_config->primary.CRLfile ? ssl_config->primary.CRLfile : "none");
                return CURLE_PEER_FAILED_VERIFICATION;
            } else
    ...
    if (config->issuercert) {
        ...
        unload_file(issuerp);
        if (rc <= 0) {
            failf(data, "server certificate issuer check failed (IssuerCert: %s)", config->issuercert ? config->issuercert : "none");
            gnutls_x509_crt_deinit(x509_cert);
            return CURLE_SSL_ISSUER_ERROR;
        }
        infof(data, "  server certificate issuer check OK (Issuer Cert: %s)", config->issuercert ? config->issuercert : "none");
    }

    size = sizeof(certname);
    rc = gnutls_x509_crt_get_dn_by_oid(x509_cert, GNUTLS_OID_X520_COMMON_NAME, 0, /* the first and only one */
        FALSE, certname, &size);
    if (rc) {
...
#define use_addr in_addr
    ...
    if (!rc) {
        if (config->verifyhost) {
            ...
            infof(data, "  common name: %s (does not match '%s')", certname, dispname);
    } else
...
static CURLcode gtls_verifyserver(struct Curl_cfilter* cf, struct Curl_easy* data, gnutls_session_t session)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct ssl_primary_config* conn_config = Curl_ssl_cf_get_primary_config(cf);
    struct ssl_config_data* ssl_config = Curl_ssl_cf_get_config(cf, data);
    const char* pinned_key = Curl_ssl_cf_is_proxy(cf) ? data->set.str[STRING_SSL_PINNEDPUBLICKEY_PROXY] : data->set.str[STRING_SSL_PINNEDPUBLICKEY];
    CURLcode result;

    result = Curl_gtls_verifyserver(data, session, conn_config, ssl_config, connssl->hostname, connssl->dispname, pinned_key);
    if (result)
        goto out;
       ...
       detect that. */
        ...
        if (connect_sessionid) {
           ...
           previous one was rejected, we just kill that and store the new */
            ...
            result = Curl_ssl_addsessionid(cf, data, connect_sessionid, connect_idsize, &added);
            Curl_ssl_sessionid_unlock(data);
            if (!added)
 ...
 */
static CURLcode gtls_connect_common(struct Curl_cfilter* cf, struct Curl_easy* data, bool nonblocking, bool* done)
{
    struct ssl_connect_data* connssl = cf->ctx;
    int rc;
    CURLcode result = CURLE_OK;
...
static CURLcode gtls_connect_nonblocking(struct Curl_cfilter* cf, struct Curl_easy* data, bool* done)
{
...
static bool gtls_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data)
{
    ...
    if (backend->gtls.session && 0 != gnutls_record_check_pending(backend->gtls.session))
        return TRUE;
    return FALSE;
}

static ssize_t gtls_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* mem, size_t len, CURLcode* curlcode)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct gtls_ssl_backend_data* backend = (struct gtls_ssl_backend_data*)connssl->backend;
    ssize_t rc;
...
static ssize_t gtls_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t buffersize, CURLcode* curlcode)
{
    struct ssl_connect_data* connssl = cf->ctx;
    struct gtls_ssl_backend_data* backend = (struct gtls_ssl_backend_data*)connssl->backend;
    ssize_t ret;
...
static CURLcode gtls_sha256sum(const unsigned char* tmp, /* input */
    size_t tmplen, unsigned char* sha256sum, /* output */
    size_t sha256len)
{
