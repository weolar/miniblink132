...
static CURLcode pausewrite(struct Curl_easy* data, int type, /* what type of data */
    bool paused_body, const char* ptr, size_t len)
{
    ...
    if (s->tempcount) {
        for (i = 0; i < s->tempcount; i++) {
            if (s->tempwrite[i].type == type && !!s->tempwrite[i].paused_body == !!paused_body) {
 ...
 * callbacks.
 */
static CURLcode chop_write(struct Curl_easy* data, int type, bool skip_body_write, char* optr, size_t olen)
{
    ...
    if (!skip_body_write && ((type & CLIENTWRITE_BODY) || ((type & CLIENTWRITE_HEADER) && data->set.include_header))) {
    ...
    if ((type & (CLIENTWRITE_HEADER | CLIENTWRITE_INFO)) && (data->set.fwrite_header || data->set.writeheader)) {
        /*
    ...
    if ((conn->handler->protocol & PROTO_FAMILY_HTTP) && (type & CLIENTWRITE_HEADER) && !(type & CLIENTWRITE_STATUS)) {
        unsigned char htype
            = (unsigned char)(type & CLIENTWRITE_CONNECT ? CURLH_CONNECT
                                                         : (type & CLIENTWRITE_1XX ? CURLH_1XX : (type & CLIENTWRITE_TRAILER ? CURLH_TRAILER : CURLH_HEADER)));
    ...
    if (data->state.tempcount) {
       ...
       pausing is lifted! */
        ...
        for (i = 0; i < count; i++) {
         ...
         all buffers */
            if (!result)
                result = chop_write(data, writebuf[i].type, !writebuf[i].paused_body, Curl_dyn_ptr(&writebuf[i].b), Curl_dyn_len(&writebuf[i].b));
...
static CURLcode client_cew_init(struct Curl_easy* data, struct contenc_writer* writer)
{
    (void)data;
    (void)writer;
    return CURLE_OK;
...
static CURLcode client_cew_write(struct Curl_easy* data, struct contenc_writer* writer, const char* buf, size_t nbytes)
{
    (void)writer;
...
static void client_cew_close(struct Curl_easy* data, struct contenc_writer* writer)
{
    (void)data;
    (void)writer;
}

static const struct content_encoding client_cew = { NULL, NULL, client_cew_init, client_cew_write, client_cew_close, sizeof(struct contenc_writer) };

/* Create an unencoding writer stage using the given handler. */
CURLcode Curl_client_create_writer(struct contenc_writer** pwriter, struct Curl_easy* data, const struct content_encoding* ce_handler, int order)
{
...
void Curl_client_free_writer(struct Curl_easy* data, struct contenc_writer* writer)
{
    if (writer) {
        writer->handler->close_writer(data, writer);
        free(writer);
...
static CURLcode init_writer_stack(struct Curl_easy* data)
{
    ...
    return Curl_client_create_writer(&data->req.writer_stack, data, &client_cew, 0);
}

CURLcode Curl_client_add_writer(struct Curl_easy* data, struct contenc_writer* writer)
{
    CURLcode result;
