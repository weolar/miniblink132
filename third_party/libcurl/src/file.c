...
FILE* fopen_wrap(/*_In_z_*/ const char* _Filename, /*_In_*/ const char* _OpenFlag)
{
    ...
    WCHAR* filename_w = 0;
    WCHAR* openFlag_w = 0;
    filename_w = utf8_to_wide_char(_Filename);
    openFlag_w = utf8_to_wide_char(_OpenFlag);

    fp = _wfopen(filename_w, openFlag_w);
...
int open_readonly(const char* filename, int flag)
{
    ...
    filename_w = utf8_to_wide_char(filename);
    fp = _wopen(filename_w, flag);
  ...
  */
    ...
    *done = TRUE; /* unconditionally */
    ...
    if (fstated) {
        ...
        headerlen = msnprintf(header, sizeof(header), "Last-Modified: %s, %02d %s %4d %02d:%02d:%02d GMT\r\n%s", Curl_wkday[tm->tm_wday ? tm->tm_wday - 1 : 6],
            tm->tm_mday, Curl_month[tm->tm_mon], tm->tm_year + 1900, tm->tm_hour, tm->tm_min, tm->tm_sec, data->req.no_body ? "" : "\r\n");
        result = Curl_client_write(data, CLIENTWRITE_HEADER, header, headerlen);
        if (result)
            return result;
