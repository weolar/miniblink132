...
static void ftp_pasv_verbose(struct Curl_easy* data, struct Curl_addrinfo* ai, char* newhost, /* ascii version */
    int port);
#endif
...
const struct Curl_handler Curl_handler_ftp = {
    ...
    PROTOPT_DUAL | PROTOPT_CLOSEACTION | PROTOPT_NEEDSPWD | PROTOPT_NOURLQUERY | PROTOPT_PROXY_AS_HTTP | PROTOPT_WILDCARD /* flags */
};
...
const struct Curl_handler Curl_handler_ftps = {
    ...
    CURLPROTO_FTP, /* family */
    PROTOPT_SSL | PROTOPT_DUAL | PROTOPT_CLOSEACTION | PROTOPT_NEEDSPWD | PROTOPT_NOURLQUERY | PROTOPT_WILDCARD /* flags */
};
#endif
...
static CURLcode InitiateTransfer(struct Curl_easy* data)
{
    ...
    if (conn->bits.ftp_use_data_ssl && data->set.ftp_use_port && !Curl_conn_is_ssl(conn, SECONDARYSOCKET)) {
...
static CURLcode ftp_readresp(struct Curl_easy* data, curl_socket_t sockfd, struct pingpong* pp, int* ftpcode, /* return the ftp-code if done */
...
CURLcode Curl_GetFTPResponse(struct Curl_easy* data, ssize_t* nreadp, /* return number of bytes read */
...
/* for debug purposes */
static const char* const ftp_state_names[] = { "STOP", "WAIT220", "AUTH", "USER", "PASS", "ACCT", "PBSZ", "PROT", "CCC", "PWD", "SYST", "NAMEFMT", "QUOTE",
    "RETR_PREQUOTE", "STOR_PREQUOTE", "POSTQUOTE", "CWD", "MKD", "MDTM", "TYPE", "LIST_TYPE", "RETR_TYPE", "STOR_TYPE", "SIZE", "RETR_SIZE", "STOR_SIZE",
    "REST", "RETR_REST", "PORT", "PRET", "PASV", "LIST", "RETR", "STOR", "QUIT" };
#endif
...
static int ftp_domore_getsock(struct Curl_easy* data, struct connectdata* conn, curl_socket_t* socks)
{
   ...
   */
    ...
    if (conn->cfilter[SECONDARYSOCKET] && !Curl_conn_is_connected(conn, SECONDARYSOCKET))
        return 0;
...
static CURLcode ftp_state_use_port(struct Curl_easy* data, ftpport fcmd) /* start with this */
{
...
#ifdef ENABLE_IPV6
                Curl_ipv6_scope(&conn->remote_addr->sa_addr), conn->scope_id,
#endif
         ...
         to prepare the server for the upcoming PASV */
            ...
            if (!conn->proto.ftpc.file)
                result = Curl_pp_sendf(data, &ftpc->pp, "PRET %s",
                    data->set.str[STRING_CUSTOMREQUEST] ? data->set.str[STRING_CUSTOMREQUEST] : (data->state.list_only ? "NLST" : "LIST"));
            else if (data->state.upload)
                result = Curl_pp_sendf(data, &ftpc->pp, "PRET STOR %s", conn->proto.ftpc.file);
            else
                result = Curl_pp_sendf(data, &ftpc->pp, "PRET RETR %s", conn->proto.ftpc.file);
            if (!result)
...
static CURLcode ftp_state_type(struct Curl_easy* data)
{
    ...
    /* If we have selected NOBODY and HEADER, it means that we only want file
     ...
     date. */
    if (data->req.no_body && ftpc->file && ftp_need_type(conn, data->state.prefer_ascii)) {
...
static bool ftp_213_date(const char* p, int* year, int* month, int* day, int* hour, int* minute, int* second)
{
    size_t len = strlen(p);
    if (len < 14)
        return FALSE;
...
static CURLcode client_write_header(struct Curl_easy* data, char* buf, size_t blen)
{
...
static CURLcode ftp_state_mdtm_resp(struct Curl_easy* data, int ftpcode)
{
    ...
    case 213: {
        ...
        if (ftp_213_date(&data->state.buffer[4], &year, &month, &day, &hour, &minute, &second)) {
        ...
        if (data->req.no_body && ftpc->file && data->set.get_filetime && (data->info.filetime >= 0)) {
            char headerbuf[128];
            int headerbuflen;
...
static CURLcode ftp_state_get_resp(struct Curl_easy* data, int ftpcode, ftpstate instate)
{
     ...
     */
        ...
        if ((instate != FTP_LIST) && !data->state.prefer_ascii && !data->set.ignorecl && (ftp->downloadsize < 1)) {
            /*
...
static CURLcode ftp_state_user_resp(struct Curl_easy* data, int ftpcode)
{
    ...
    (the server denies to log the specified user) */
        ...
        if (data->set.str[STRING_FTP_ALTERNATIVE_TO_USER] && !ftpc->ftp_trying_alternative) {
 ...
 *
 */
static CURLcode ftp_connect(struct Curl_easy* data, bool* done) /* see description above */
{
    CURLcode result;
    struct connectdata* conn = data->conn;
    struct ftp_conn* ftpc = &conn->proto.ftpc;
    ...
    if (result || premature)
       ...
       use checking further */
        ...
        if ((-1 != data->state.infilesize) && (data->state.infilesize != data->req.writebytecount) && !data->set.crlf && (ftp->transfer == PPTRANSFER_BODY)) {
            failf(data, "Uploaded unaligned file size (%" CURL_FORMAT_CURL_OFF_T " out of %" CURL_FORMAT_CURL_OFF_T " bytes)", data->req.writebytecount,
...
static CURLcode ftp_perform(struct Curl_easy* data, bool* connected, /* connect status after PASV / PORT */
    bool* dophase_done)
{
    ...
    for (;;) {
        ...
        case CURLWC_DOWNLOADING: {
            ...
            if (data->set.chunk_bgn) {
                long userresponse;
                Curl_set_in_callback(data, true);
                userresponse = data->set.chunk_bgn(finfo, data->set.wildcardptr, (int)wildcard->filelist.size);
                Curl_set_in_callback(data, false);
                switch (userresponse) {
...
static CURLcode ftp_setup_connection(struct Curl_easy* data, struct connectdata* conn)
{
    ...
    if (data->set.str[STRING_FTP_ALTERNATIVE_TO_USER]) {
        ftpc->alternative_to_user = strdup(data->set.str[STRING_FTP_ALTERNATIVE_TO_USER]);
