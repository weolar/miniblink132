...
#elif (defined(__MAC_OS_X_VERSION_MAX_ALLOWED) && (__MAC_OS_X_VERSION_MAX_ALLOWED >= 1040) && defined(__MAC_OS_X_VERSION_MIN_REQUIRED)                         \
    && (__MAC_OS_X_VERSION_MIN_REQUIRED < 101500))                                                                                                             \
    || (defined(__IPHONE_OS_VERSION_MAX_ALLOWED) && (__IPHONE_OS_VERSION_MAX_ALLOWED >= 20000) && defined(__IPHONE_OS_VERSION_MIN_REQUIRED)                    \
...
static void my_md5_update(my_md5_ctx* ctx, const unsigned char* input, unsigned int inputLen)
{
...
static void my_md5_update(my_md5_ctx* ctx, const unsigned char* input, unsigned int len)
{
...
static void my_md5_update(my_md5_ctx* ctx, const unsigned char* data, unsigned int length)
{
...
static CURLcode my_md5_init(my_md5_ctx* ctx);
static void my_md5_update(my_md5_ctx* ctx, const void* data, unsigned long size);
...
const struct HMAC_params Curl_HMAC_MD5[] = { { /* Hash initialization function. */
...
const struct MD5_params Curl_DIGEST_MD5[] = { { /* Digest initialization function */
 ...
 */
CURLcode Curl_md5it(unsigned char* outbuffer, const unsigned char* input, const size_t len)
{
    CURLcode result;
    my_md5_ctx ctx;
