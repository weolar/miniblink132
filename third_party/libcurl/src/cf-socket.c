...
static CURLcode socket_open(struct Curl_easy* data, struct Curl_sockaddr_ex* addr, curl_socket_t* sockfd)
{
    DEBUGASSERT(data);
 ...
 *
 */
CURLcode Curl_socket_open(struct Curl_easy* data, const struct Curl_addrinfo* ai, struct Curl_sockaddr_ex* addr, int transport, curl_socket_t* sockfd)
{
...
static int socket_close(struct Curl_easy* data, struct connectdata* conn, int use_callback, curl_socket_t sock)
{
...
static CURLcode bindlocal(struct Curl_easy* data, struct connectdata* conn, curl_socket_t sockfd, int af, unsigned int scope)
{
    ...
    if (dev && (strlen(dev) < 255)) {
       ...
       */
         ...
         */
           ...
           */
                infof(data, "Local Interface %s is ip %s using address family %i", dev, myhost, af);
                done = 1;
                break;
            }
        }
       ...
       */
            ...
            if (h) {
                ...
                infof(data, "Name '%s' family %i resolved to '%s' family %i", dev, af, myhost, h->addr->ai_family);
 ...
 */
static CURLcode socket_connect_result(struct Curl_easy* data, const char* ipaddress, int error)
{
    switch (error) {
    case EINPROGRESS:
    case EWOULDBLOCK:
...
static ssize_t nw_in_read(void* reader_ctx, unsigned char* buf, size_t len, CURLcode* err)
{
    ...
    *err = CURLE_OK;
    ...
    CURL_TRC_CF(rctx->data, rctx->cf, "nw_in_read(len=%zu) -> %d, err=%d", len, (int)nread, *err);
    return nread;
}
    ...
    if (ctx && CURL_SOCKET_BAD != ctx->sock) {
       ...
       */
            if (ctx->sock == cf->conn->sock[cf->sockindex]) {
                CURL_TRC_CF(data, cf, "cf_socket_close(%" CURL_FORMAT_SOCKET_T ", active)", ctx->sock);
                socket_close(data, cf->conn, !ctx->accepted, ctx->sock);
                cf->conn->sock[cf->sockindex] = CURL_SOCKET_BAD;
            } else {
                CURL_TRC_CF(data, cf, "cf_socket_close(%" CURL_FORMAT_SOCKET_T ") no longer at conn->sock[], discarding", ctx->sock);
         ...
         * code is messing with conn->sock[cf->sockindex] */
            ...
            CURL_TRC_CF(data, cf, "cf_socket_close(%" CURL_FORMAT_SOCKET_T ", not active)", ctx->sock);
            socket_close(data, cf->conn, !ctx->accepted, ctx->sock);
            ctx->sock = CURL_SOCKET_BAD;
        }
...
static CURLcode set_local_ip(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    if (!(data->conn->handler->protocol & CURLPROTO_TFTP)) {
        ...
        if (!Curl_addr2string((struct sockaddr*)&ssloc, slen, ctx->l_ip, &ctx->l_port)) {
...
static CURLcode set_remote_ip(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    if (!Curl_addr2string(&ctx->addr.sa_addr, ctx->addr.addrlen, ctx->r_ip, &ctx->r_port)) {
        char buffer[STRERROR_LEN];
...
static CURLcode cf_socket_open(struct Curl_cfilter* cf, struct Curl_easy* data)
{
...
#ifdef ENABLE_IPV6
    is_tcp = (ctx->addr.family == AF_INET || ctx->addr.family == AF_INET6) && ctx->addr.socktype == SOCK_STREAM;
#else
    ...
    if (data->set.fsockopt) {
        ...
        Curl_set_in_callback(data, true);
        error = data->set.fsockopt(data->set.sockopt_client, ctx->sock, CURLSOCKTYPE_IPCXN);
        Curl_set_in_callback(data, false);
    ...
    if (ctx->addr.family == AF_INET
        ...
        result = bindlocal(data, cf->conn, ctx->sock, ctx->addr.family, Curl_ipv6_scope(&ctx->addr.sa_addr));
        if (result) {
...
out:
    ...
    CURL_TRC_CF(data, cf, "cf_socket_open() -> %d, fd=%" CURL_FORMAT_SOCKET_T, result, ctx->sock);
    return result;
}
    ...
    if (is_tcp_fastopen) {
       ...
       Xcode 9 / macOS SDK 10.13 */
        if (__builtin_available(macOS 10.11, iOS 9.0, tvOS 9.0, watchOS 2.0, *)) {
            ...
            rc = connectx(ctx->sock, &endpoints, SAE_ASSOCID_ANY, CONNECT_RESUME_ON_READ_WRITE | CONNECT_DATA_IDEMPOTENT, NULL, 0, NULL, NULL);
        } else {
        ...
        if (setsockopt(ctx->sock, IPPROTO_TCP, TCP_FASTOPEN_CONNECT, (void*)&optval, sizeof(optval)) < 0)
            infof(data, "Failed to enable TCP Fast Open on fd %" CURL_FORMAT_SOCKET_T, ctx->sock);
...
static CURLcode cf_tcp_connect(struct Curl_cfilter* cf, struct Curl_easy* data, bool blocking, bool* done)
{
    struct cf_socket_ctx* ctx = cf->ctx;
    CURLcode result = CURLE_COULDNT_CONNECT;
    int rc = 0;
...
out:
    if (result) {
        if (ctx->error) {
            ...
            {
                char buffer[STRERROR_LEN];
                infof(data, "connect to %s port %u failed: %s", ctx->r_ip, ctx->r_port, Curl_strerror(ctx->error, buffer, sizeof(buffer)));
            }
#endif
        }
        if (ctx->sock != CURL_SOCKET_BAD) {
...
static void cf_socket_get_host(struct Curl_cfilter* cf, struct Curl_easy* data, const char** phost, const char** pdisplay_host, int* pport)
{
    (void)data;
...
static void cf_socket_adjust_pollset(struct Curl_cfilter* cf, struct Curl_easy* data, struct easy_pollset* ps)
{
    struct cf_socket_ctx* ctx = cf->ctx;
...
static bool cf_socket_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data)
{
    struct cf_socket_ctx* ctx = cf->ctx;
    int readable;
...
static ssize_t cf_socket_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len, CURLcode* err)
{
    struct cf_socket_ctx* ctx = cf->ctx;
    ...
    *err = CURLE_OK;
    ...
    if (cf->cft != &Curl_cft_udp && ctx->wpartial_percent > 0 && len > 8) {
        ...
        if (!len)
            len = 1;
        CURL_TRC_CF(data, cf, "send(len=%zu) SIMULATE partial write of %zu bytes", orig_len, len);
    }
#endif

#if defined(MSG_FASTOPEN) && !defined(TCP_FASTOPEN_CONNECT) /* Linux */
    if (cf->conn->bits.tcp_fastopen) {
        nwritten = sendto(ctx->sock, buf, len, MSG_FASTOPEN, &cf->conn->remote_addr->sa_addr, cf->conn->remote_addr->addrlen);
    ...
    if (-1 == nwritten) {
        ...
        if (
         ...
         due to its inability to send off data without blocking. We therefore
         treat both error codes the same here */
            (EWOULDBLOCK == sockerr) || (EAGAIN == sockerr) || (EINTR == sockerr) || (EINPROGRESS == sockerr)
#endif
        ) {
            /* this is just a case of EWOULDBLOCK */
            *err = CURLE_AGAIN;
...
static ssize_t cf_socket_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t len, CURLcode* err)
{
    ...
    *err = CURLE_OK;
    ...
    if (cf->cft != &Curl_cft_udp && ctx->recv_max && ctx->recv_max < len) {
        size_t orig_len = len;
        len = ctx->recv_max;
        CURL_TRC_CF(data, cf, "recv(len=%zu) SIMULATE max read of %zu bytes", orig_len, len);
    }
#endif
...
static CURLcode cf_udp_setup_quic(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    set_local_ip(cf, data);
    CURL_TRC_CF(data, cf, "%s socket %" CURL_FORMAT_SOCKET_T " connected: [%s:%d] -> [%s:%d]", (ctx->transport == TRNSPRT_QUIC) ? "QUIC" : "UDP", ctx->sock,
    ...
    *done = FALSE;
    if (ctx->sock == CURL_SOCKET_BAD) {
        ...
        if (ctx->transport == TRNSPRT_QUIC) {
            ...
            if (result)
                goto out;
            CURL_TRC_CF(data, cf, "cf_udp_connect(), opened socket=%" CURL_FORMAT_SOCKET_T " (%s:%d)", ctx->sock, ctx->l_ip, ctx->l_port);
        } else {
            CURL_TRC_CF(data, cf, "cf_udp_connect(), opened socket=%" CURL_FORMAT_SOCKET_T " (unconnected)", ctx->sock);
        }
        *done = TRUE;
        cf->connected = TRUE;
    }
...
CURLcode Curl_cf_udp_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn, const struct Curl_addrinfo* ai, int transport)
{
    struct cf_socket_ctx* ctx = NULL;
    struct Curl_cfilter* cf = NULL;
    CURLcode result;
...
CURLcode Curl_cf_unix_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn, const struct Curl_addrinfo* ai, int transport)
{
    struct cf_socket_ctx* ctx = NULL;
    struct Curl_cfilter* cf = NULL;
    CURLcode result;
...
static CURLcode cf_tcp_accept_connect(struct Curl_cfilter* cf, struct Curl_easy* data, bool blocking, bool* done)
{
...
CURLcode Curl_conn_tcp_listen_set(struct Curl_easy* data, struct connectdata* conn, int sockindex, curl_socket_t* s)
{
    ...
    ctx->connected_at = Curl_now();
    cf->connected = TRUE;
    CURL_TRC_CF(data, cf, "Curl_conn_tcp_listen_set(%" CURL_FORMAT_SOCKET_T ")", ctx->sock);
...
static void set_accepted_remote_ip(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    if (!Curl_addr2string((struct sockaddr*)&ssrem, plen, ctx->r_ip, &ctx->r_port)) {
        failf(data, "ssrem inet_ntop() failed with errno %d: %s", errno, Curl_strerror(errno, buffer, sizeof(buffer)));
        return;
    }
#else
...
CURLcode Curl_conn_tcp_accepted_set(struct Curl_easy* data, struct connectdata* conn, int sockindex, curl_socket_t* s)
{
    ...
    ctx->connected_at = Curl_now();
    cf->connected = TRUE;
    CURL_TRC_CF(data, cf, "accepted_set(sock=%" CURL_FORMAT_SOCKET_T ", remote=%s port=%d)", ctx->sock, ctx->r_ip, ctx->r_port);
...
{
    return cf && (cf->cft == &Curl_cft_tcp || cf->cft == &Curl_cft_udp || cf->cft == &Curl_cft_unix || cf->cft == &Curl_cft_tcp_accept);
}

CURLcode Curl_cf_socket_peek(struct Curl_cfilter* cf, struct Curl_easy* data, curl_socket_t* psock, const struct Curl_sockaddr_ex** paddr,
