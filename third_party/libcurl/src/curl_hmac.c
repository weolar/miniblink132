...
#if (defined(USE_CURL_NTLM_CORE) && !defined(USE_WINDOWS_SSPI)) || !defined(CURL_DISABLE_AWS) || !defined(CURL_DISABLE_DIGEST_AUTH)
...
CURLcode Curl_hmacit(
    const struct HMAC_params* hashparams, const unsigned char* key, const size_t keylen, const unsigned char* data, const size_t datalen, unsigned char* output)
{
