...
#define HMAC_SHA256(k, kl, d, dl, o)                                                                                                                           \
    do {                                                                                                                                                       \
        result = Curl_hmacit(Curl_HMAC_SHA256, (unsigned char*)k, kl, (unsigned char*)d, dl, o);                                                               \
        if (result) {                                                                                                                                          \
            goto fail;                                                                                                                                         \
        }                                                                                                                                                      \
    } while (0)
...
{
    Curl_hexencode(sha, SHA256_DIGEST_LENGTH, (unsigned char*)dst, SHA256_HEX_LENGTH);
}
...
static CURLcode make_headers(struct Curl_easy* data, const char* hostname, char* timestamp, char* provider1, char** date_header, char* content_sha256_header,
    struct dynbuf* canonical_headers, struct dynbuf* signed_headers)
{
    ...
    msnprintf(date_full_hdr, DATE_FULL_HDR_LEN, "x-%s-date:%s", provider1, timestamp);
...
static char* parse_content_sha_hdr(struct Curl_easy* data, const char* provider1, size_t* value_len)
{
...
static CURLcode calc_payload_hash(struct Curl_easy* data, unsigned char* sha_hash, char* sha_hex)
{
    ...
    result = Curl_sha256it(sha_hash, (const unsigned char*)post_data, post_data_len);
    if (!result)
...
static CURLcode calc_s3_payload_hash(struct Curl_easy* data, Curl_HttpReq httpreq, char* provider1, unsigned char* sha_hash, char* sha_hex, char* header)
{
    ...
    msnprintf(header, CONTENT_SHA256_HDR_LEN, "x-%s-content-sha256: %s", provider1, sha_hex);
...
CURLcode Curl_output_aws_sigv4(struct Curl_easy* data, bool proxy)
{
   ...
   */
    arg = data->set.str[STRING_AWS_SIGV4] ? data->set.str[STRING_AWS_SIGV4] : "aws:amz";
  ...
  */
   ...
   * values like UNSIGNED-PAYLOAD */
    sign_as_s3 = (strcasecompare(provider0, "aws") && strcasecompare(service, "s3"));
    ...
    result = make_headers(data, hostname, timestamp, provider1, &date_header, content_sha256_hdr, &canonical_headers, &signed_headers);
    if (result)
        goto fail;
    ...
    canonical_request = curl_maprintf("%s\n" /* HTTPRequestMethod */
                                      "%s\n" /* CanonicalURI */
                                      "%s\n" /* CanonicalQueryString */
                                      "%s\n" /* CanonicalHeaders */
                                      "%s\n" /* SignedHeaders */
                                      "%.*s", /* HashedRequestPayload in hex */
        method, data->state.up.path, Curl_dyn_ptr(&canonical_query) ? Curl_dyn_ptr(&canonical_query) : "", Curl_dyn_ptr(&canonical_headers),
        Curl_dyn_ptr(&signed_headers), (int)payload_hash_len, payload_hash);
    ...
    if (Curl_sha256it(sha_hash, (unsigned char*)canonical_request, strlen(canonical_request)))
        goto fail;
   ...
   */
    ...
    /* provider 0 uppercase */
    secret = curl_maprintf("%s4%s", provider0, data->state.aptr.passwd ? data->state.aptr.passwd : "");
    if (!secret)
        goto fail;
    ...
    auth_headers = curl_maprintf("Authorization: %s4-HMAC-SHA256 "
        ...
        provider0, user, credential_scope, Curl_dyn_ptr(&signed_headers), sha_hex, date_header ? date_header : "", content_sha256_hdr);
    if (!auth_headers) {
        goto fail;
    }
