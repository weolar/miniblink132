 ...
 */
CHUNKcode Curl_httpchunk_read(struct Curl_easy* data, char* datap, ssize_t datalen, ssize_t* wrote, CURLcode* extrap)
{
           ...
           there was no trailer and we move on */
                ...
                if (tr) {
                    ...
                    if (!data->set.http_te_skip) {
                        result = Curl_client_write(data, CLIENTWRITE_HEADER | CLIENTWRITE_TRAILER, tr, trlen);
                        if (result) {
                            *extrap = result;
                            return CHUNKE_PASSTHRU_ERROR;
                        }
