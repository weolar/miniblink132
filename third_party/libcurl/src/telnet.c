...
static CURLcode telrcv(struct Curl_easy* data, const unsigned char* inbuf, /* Data received from socket */
...
static CURLcode send_telnet_data(struct Curl_easy* data, char* buffer, ssize_t nread);
...
static void suboption(struct Curl_easy* data)
{
    ...
    case CURL_TELOPT_TTYPE:
        ...
        msnprintf((char*)temp, sizeof(temp), "%c%c%c%c%s%c%c", CURL_IAC, CURL_SB, CURL_TELOPT_TTYPE, CURL_TELQUAL_IS, tn->subopt_ttype, CURL_IAC, CURL_SE);
    ...
    case CURL_TELOPT_XDISPLOC:
        ...
        msnprintf(
            (char*)temp, sizeof(temp), "%c%c%c%c%s%c%c", CURL_IAC, CURL_SB, CURL_TELOPT_XDISPLOC, CURL_TELQUAL_IS, tn->subopt_xdisploc, CURL_IAC, CURL_SE);
        bytes_written = swrite(conn->sock[FIRSTSOCKET], temp, len);
        if (bytes_written < 0) {
            err = SOCKERRNO;
            failf(data, "Sending data failed (%d)", err);
    ...
    case CURL_TELOPT_NEW_ENVIRON:
        ...
        for (v = tn->telnet_vars; v; v = v->next) {
            ...
            if (len + tmplen < (int)sizeof(temp) - 6) {
                ...
                if (!s)
                    len += msnprintf((char*)&temp[len], sizeof(temp) - len, "%c%s", CURL_NEW_ENV_VAR, v->data);
                else {
                    size_t vlen = s - v->data;
                    len += msnprintf((char*)&temp[len], sizeof(temp) - len, "%c%.*s%c%s", CURL_NEW_ENV_VAR, (int)vlen, v->data, CURL_NEW_ENV_VALUE, ++s);
                }
            }
        }
...
#define startskipping()                                                                                                                                        \
    if (startwrite >= 0) {                                                                                                                                     \
        result = Curl_client_write(data, CLIENTWRITE_BODY, (char*)&inbuf[startwrite], in - startwrite);                                                        \
        if (result)                                                                                                                                            \
            return result;                                                                                                                                     \
    }                                                                                                                                                          \
    startwrite = -1
...
static CURLcode send_telnet_data(struct Curl_easy* data, char* buffer, ssize_t nread)
{
    ...
    while (!result && total_written < outlen) {
        ...
        default: /* write! */
            bytes_written = 0;
            result = Curl_nwrite(data, FIRSTSOCKET, outbuf + total_written, outlen - total_written, &bytes_written);
