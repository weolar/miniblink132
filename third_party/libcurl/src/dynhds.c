...
static struct dynhds_entry* entry_new(const char* name, size_t namelen, const char* value, size_t valuelen, int opts)
{
...
static struct dynhds_entry* entry_append(struct dynhds_entry* e, const char* value, size_t valuelen)
{
...
void Curl_dynhds_init(struct dynhds* dynhds, size_t max_entries, size_t max_strs_size)
{
    DEBUGASSERT(dynhds);
...
struct dynhds_entry* Curl_dynhds_get(struct dynhds* dynhds, const char* name, size_t namelen)
{
    size_t i;
    for (i = 0; i < dynhds->hds_len; ++i) {
        if (dynhds->hds[i]->namelen == namelen && strncasecompare(dynhds->hds[i]->name, name, namelen)) {
            return dynhds->hds[i];
        }
    }
    return NULL;
...
CURLcode Curl_dynhds_add(struct dynhds* dynhds, const char* name, size_t namelen, const char* value, size_t valuelen)
{
    ...
    if (dynhds->hds_len + 1 >= dynhds->hds_allc) {
        ...
        if (!nhds)
            goto out;
        if (dynhds->hds) {
            memcpy(nhds, dynhds->hds, dynhds->hds_len * sizeof(struct dynhds_entry*));
            Curl_safefree(dynhds->hds);
        }
        dynhds->hds = nhds;
...
CURLcode Curl_dynhds_cadd(struct dynhds* dynhds, const char* name, const char* value)
{
    return Curl_dynhds_add(dynhds, name, strlen(name), value, strlen(value));
}

CURLcode Curl_dynhds_h1_add_line(struct dynhds* dynhds, const char* line, size_t line_len)
{
    const char* p;
    const char* name;
    size_t namelen;
...
bool Curl_dynhds_contains(struct dynhds* dynhds, const char* name, size_t namelen)
{
...
size_t Curl_dynhds_count_name(struct dynhds* dynhds, const char* name, size_t namelen)
{
    size_t n = 0;
    if (dynhds->hds_len) {
        size_t i;
        for (i = 0; i < dynhds->hds_len; ++i) {
            if ((namelen == dynhds->hds[i]->namelen) && strncasecompare(name, dynhds->hds[i]->name, namelen))
                ++n;
        }
    }
    return n;
...
CURLcode Curl_dynhds_set(struct dynhds* dynhds, const char* name, size_t namelen, const char* value, size_t valuelen)
{
    Curl_dynhds_remove(dynhds, name, namelen);
    return Curl_dynhds_add(dynhds, name, namelen, value, valuelen);
}

size_t Curl_dynhds_remove(struct dynhds* dynhds, const char* name, size_t namelen)
{
    size_t n = 0;
    if (dynhds->hds_len) {
        size_t i, len;
        for (i = 0; i < dynhds->hds_len; ++i) {
            if ((namelen == dynhds->hds[i]->namelen) && strncasecompare(name, dynhds->hds[i]->name, namelen)) {
                ++n;
                --dynhds->hds_len;
                dynhds->strs_len -= (dynhds->hds[i]->namelen + dynhds->hds[i]->valuelen);
                entry_free(dynhds->hds[i]);
                len = dynhds->hds_len - i; /* remaining entries */
                if (len) {
                    memmove(&dynhds->hds[i], &dynhds->hds[i + 1], len * sizeof(dynhds->hds[i]));
                }
...
CURLcode Curl_dynhds_h1_dprint(struct dynhds* dynhds, struct dynbuf* dbuf)
{
    ...
    for (i = 0; i < dynhds->hds_len; ++i) {
        result
            = Curl_dyn_addf(dbuf, "%.*s: %.*s\r\n", (int)dynhds->hds[i]->namelen, dynhds->hds[i]->name, (int)dynhds->hds[i]->valuelen, dynhds->hds[i]->value);
        if (result)
            break;
    }
