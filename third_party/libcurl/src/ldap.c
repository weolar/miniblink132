...
static int _ldap_url_parse(struct Curl_easy* data, const struct connectdata* conn, LDAPURLDesc** ludp);
...
static int ldap_win_bind_auth(LDAP* server, const char* user, const char* passwd, unsigned long authflags)
{
    ULONG method = 0;
...
static int ldap_win_bind(struct Curl_easy* data, LDAP* server, const char* user, const char* passwd)
{
...
static CURLcode ldap_do(struct Curl_easy* data, bool* done)
{
    ...
    *done = TRUE; /* unconditionally */
    infof(data, "LDAP local: LDAP Vendor = %s ; LDAP Version = %d", LDAP_VENDOR_NAME, LDAP_VENDOR_VERSION);
    ...
    if (conn->given->flags & PROTOPT_SSL)
        ldap_ssl = 1;
    infof(data, "LDAP local: trying to establish %s connection", ldap_ssl ? "encrypted" : "cleartext");
    ...
    if (ldap_ssl) {
        ...
        if (conn->ssl_config.verifypeer) {
            ...
            if (!ldap_ca) {
                failf(data, "LDAP local: ERROR %s CA cert not set", (cert_type == LDAPSSL_CERT_FILETYPE_DER ? "DER" : "PEM"));
                result = CURLE_SSL_CERTPROBLEM;
                goto quit;
            }
            infof(data, "LDAP local: using %s CA cert '%s'", (cert_type == LDAPSSL_CERT_FILETYPE_DER ? "DER" : "PEM"), ldap_ca);
            rc = ldapssl_add_trusted_cert(ldap_ca, cert_type);
            if (rc != LDAP_SUCCESS) {
                failf(data, "LDAP local: ERROR setting %s CA cert: %s", (cert_type == LDAPSSL_CERT_FILETYPE_DER ? "DER" : "PEM"), ldap_err2string(rc));
                result = CURLE_SSL_CERTPROBLEM;
                goto quit;
            }
        ...
        if (!server) {
            failf(data, "LDAP local: Cannot connect to %s:%u", conn->host.dispname, conn->port);
            ...
            if (vals) {
                for (i = 0; (vals[i] != NULL); i++) {
                    ...
                    if ((attr_len > 7) && (strcmp(";binary", attr + (attr_len - 7)) == 0)) {
                        ...
                        result = Curl_client_write(data, CLIENTWRITE_BODY, vals[i]->bv_val, vals[i]->bv_len);
                        if (result) {
                            ldap_value_free_len(vals);
                            FREE_ON_WINLDAP(attr);
                            ldap_memfree(attribute);
 ...
 */
static int _ldap_url_parse2(struct Curl_easy* data, const struct connectdata* conn, LDAPURLDesc* ludp)
{
    int rc = LDAP_SUCCESS;
    char* p;
    char* path;
    ...
    if (!data || !data->state.up.path || data->state.up.path[0] != '/' || !strncasecompare("LDAP", data->state.up.scheme, 4))
...
static int _ldap_url_parse(struct Curl_easy* data, const struct connectdata* conn, LDAPURLDesc** ludpp)
{
