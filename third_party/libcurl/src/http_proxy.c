...
CURLcode Curl_http_proxy_get_destination(struct Curl_cfilter* cf, const char** phostname, int* pport, bool* pipv6_ip)
{
    DEBUGASSERT(cf);
    DEBUGASSERT(cf->conn);
...
CURLcode Curl_http_proxy_create_CONNECT(struct httpreq** preq, struct Curl_cfilter* cf, struct Curl_easy* data, int http_version_major)
{
    ...
    result = Curl_http_req_make(&req, "CONNECT", sizeof("CONNECT") - 1, NULL, 0, authority, strlen(authority), NULL, 0);
    if (result)
        goto out;
    ...
    if (http_version_major == 1 && !Curl_checkProxyheaders(data, cf->conn, STRCONST("Host"))) {
    ...
    if (http_version_major == 1 && !Curl_checkProxyheaders(data, cf->conn, STRCONST("Proxy-Connection"))) {
...
connect_sub:
    ...
    *done = FALSE;
    if (!ctx->cf_protocol) {
        ...
        int alpn = Curl_conn_cf_is_ssl(cf->next) ? cf->conn->proxy_alpn : CURL_HTTP_VERSION_1_1;
        ...
        case CURL_HTTP_VERSION_1_1:
            ...
            infof(data, "CONNECT tunnel: HTTP/1.%d negotiated", (alpn == CURL_HTTP_VERSION_1_0) ? 0 : 1);
...
void Curl_cf_http_proxy_get_host(struct Curl_cfilter* cf, struct Curl_easy* data, const char** phost, const char** pdisplay_host, int* pport)
{
    (void)data;
    if (!cf->connected) {
