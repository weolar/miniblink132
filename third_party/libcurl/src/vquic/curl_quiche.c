...
static CURLcode quic_x509_store_setup(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    if (!ctx->x509_store_setup) {
        if (cf->conn->ssl_config.verifypeer) {
           ...
           the server's certificate. */
                if (!SSL_CTX_load_verify_locations(ctx->sslctx, ssl_cafile, ssl_capath)) {
...
static CURLcode quic_ssl_setup(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    SSL_CTX_set_alpn_protos(ctx->sslctx, (const uint8_t*)QUICHE_H3_APPLICATION_PROTOCOL, sizeof(QUICHE_H3_APPLICATION_PROTOCOL) - 1);
...
#define H3_STREAM_CTX(d) ((struct stream_ctx*)(((d) && (d)->req.p.http) ? ((struct HTTP*)(d)->req.p.http)->h3_ctx : NULL))
...
static struct Curl_easy* get_stream_easy(struct Curl_cfilter* cf, struct Curl_easy* data, int64_t stream3_id)
{
    struct Curl_easy* sdata;
 ...
 */
static CURLcode write_resp_raw(struct Curl_cfilter* cf, struct Curl_easy* data, const void* mem, size_t memlen)
{
    struct stream_ctx* stream = H3_STREAM_CTX(data);
...
static int cb_each_header(uint8_t* name, size_t name_len, uint8_t* value, size_t value_len, void* argp)
{
    ...
    if ((name_len == 7) && !strncmp(HTTP_PSEUDO_STATUS, (char*)name, 7)) {
        CURL_TRC_CF(x->data, x->cf, "[%" PRId64 "] status: %.*s", stream->id, (int)value_len, value);
        ...
        CURL_TRC_CF(x->data, x->cf, "[%" PRId64 "] header: %.*s: %.*s", stream->id, (int)name_len, name, (int)value_len, value);
    ...
    if (result) {
        CURL_TRC_CF(x->data, x->cf, "[%" PRId64 "] on header error %d", stream->id, result);
    }
    return result;
}

static ssize_t stream_resp_read(void* reader_ctx, unsigned char* buf, size_t len, CURLcode* err)
{
...
static CURLcode cf_recv_body(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    cb_ctx.cf = cf;
    cb_ctx.data = data;
    nwritten = Curl_bufq_slurp(&stream->recvbuf, stream_resp_read, &cb_ctx, &result);

    if (nwritten < 0 && result != CURLE_AGAIN) {
        CURL_TRC_CF(data, cf, "[%" PRId64 "] recv_body error %zd", stream->id, nwritten);
        failf(data, "Error %d in HTTP/3 response body for stream[%" PRId64 "]", result, stream->id);
        stream->closed = TRUE;
        stream->reset = TRUE;
...
static CURLcode h3_process_event(struct Curl_cfilter* cf, struct Curl_easy* data, int64_t stream3_id, quiche_h3_event* ev)
{
    struct stream_ctx* stream = H3_STREAM_CTX(data);
    struct cb_ctx cb_ctx;
    CURLcode result = CURLE_OK;
    ...
    case QUICHE_H3_EVENT_HEADERS:
        ...
        if (rc) {
            failf(data, "Error %d in HTTP/3 response header for stream[%" PRId64 "]", rc, stream3_id);
            return CURLE_RECV_ERROR;
        }
    ...
    default:
        CURL_TRC_CF(data, cf, "[%" PRId64 "] recv, unhandled event %d", stream3_id, quiche_h3_event_type(ev));
        break;
    }
    return result;
}

static CURLcode cf_poll_events(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
    struct stream_ctx* stream = H3_STREAM_CTX(data);
    struct Curl_easy* sdata;
    ...
    while (ctx->h3c) {
        ...
        if (stream3_id == QUICHE_H3_ERR_DONE) {
            ...
            CURL_TRC_CF(data, cf, "[%" PRId64 "] error poll: %" PRId64, stream ? stream->id : -1, stream3_id);
            return CURLE_HTTP3;
        }
        ...
        if (!sdata) {
            ...
            if (result) {
                CURL_TRC_CF(data, cf,
                    ...
                    "for [%" PRId64 "] -> %d",
                    stream ? stream->id : -1, cf_ev_name(ev), stream3_id, result);
                if (data == sdata) {
...
static CURLcode recv_pkt(const unsigned char* pkt, size_t pktlen, struct sockaddr_storage* remote_addr, socklen_t remote_addrlen, int ecn, void* userp)
{
    ...
    nread = quiche_conn_recv(ctx->qconn, (unsigned char*)pkt, pktlen, &recv_info);
    if (nread < 0) {
        if (QUICHE_ERR_DONE == nread) {
            ...
            if (verify_ok != X509_V_OK) {
                failf(r->data, "SSL certificate problem: %s", X509_verify_cert_error_string(verify_ok));
        ...
        CURL_TRC_CF(r->data, r->cf, "ingress, quiche only read %zd/%zu bytes", nread, pktlen);
    }

    return CURLE_OK;
}

static CURLcode cf_process_ingress(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
 ...
 */
static CURLcode cf_flush_egress(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
    ssize_t nread;
    CURLcode result;
    ...
    for (;;) {
        ...
        nread = Curl_bufq_sipn(&ctx->q.sendbuf, 0, read_pkt_to_send, &readx, &result);
        if (nread < 0) {
            if (result != CURLE_AGAIN)
                return result;
...
static ssize_t recv_closed_stream(struct Curl_cfilter* cf, struct Curl_easy* data, CURLcode* err)
{
    struct stream_ctx* stream = H3_STREAM_CTX(data);
    ssize_t nread = -1;
    ...
    if (stream->reset) {
        ...
        *err = stream->resp_got_header ? CURLE_PARTIAL_FILE : CURLE_RECV_ERROR;
        CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_recv, was reset -> %d", stream->id, *err);
...
static ssize_t cf_quiche_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t len, CURLcode* err)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
    struct stream_ctx* stream = H3_STREAM_CTX(data);
    ssize_t nread = -1;
...
out:
    ...
    CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_recv(total=%" CURL_FORMAT_CURL_OFF_T ") -> %zd, %d", stream->id, ctx->data_recvd, nread, *err);
    return nread;
}
...
static ssize_t h3_open_stream(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len, CURLcode* err)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
    struct stream_ctx* stream = H3_STREAM_CTX(data);
    size_t nheader, i;
    ...
    *err = Curl_http_req_to_h2(&h2_headers, stream->h1.req, data);
    ...
    stream3_id = quiche_h3_send_request(ctx->h3c, ctx->qconn, nva, nheader, stream->send_closed);
    if (stream3_id < 0) {
       ...
       * exhausted. Which happens frequently and intermittent. */
            ...
            *err = CURLE_AGAIN;
            ...
            CURL_TRC_CF(data, cf, "send_request(%s) -> %" PRId64, data->state.url, stream3_id);
        }
        *err = CURLE_SEND_ERROR;
        nwritten = -1;
        goto out;
    ...
    *err = CURLE_OK;
    ...
    if (Curl_trc_is_verbose(data)) {
        ...
        for (i = 0; i < nheader; ++i) {
            infof(data, "[HTTP/3] [%" PRId64 "] [%.*s: %.*s]", stream->id, (int)nva[i].name_len, nva[i].name, (int)nva[i].value_len, nva[i].value);
        }
    }
...
static ssize_t cf_quiche_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len, CURLcode* err)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
    struct stream_ctx* stream = H3_STREAM_CTX(data);
    CURLcode result;
    ...
    *err = cf_process_ingress(cf, data);
    ...
    if (!stream || stream->id < 0) {
        ...
        bool eof = (stream->upload_left >= 0 && (curl_off_t)len >= stream->upload_left);
        nwritten = quiche_h3_send_body(ctx->h3c, ctx->qconn, stream->id, (uint8_t*)buf, len, eof);
       ...
       * sending. But when do we open again? */
            ...
            *err = CURLE_AGAIN;
            nwritten = -1;
            goto out;
        } else if (nwritten == QUICHE_H3_TRANSPORT_ERR_INVALID_STREAM_STATE && stream->closed && stream->resp_hds_complete) {
       ...
       * it would see the response and stop/discard sending on its own- */
            ...
            *err = CURLE_SEND_ERROR;
            ...
            if (stream->upload_left > 0) {
                stream->upload_left = (nwritten < stream->upload_left) ? (stream->upload_left - nwritten) : 0;
            }
            if (stream->upload_left == 0)
                stream->send_closed = TRUE;
...
out:
    ...
    CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_send(len=%zu) -> %zd, %d", stream ? stream->id : -1, len, nwritten, *err);
    return nwritten;
}

static bool stream_is_writeable(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
    struct stream_ctx* stream = H3_STREAM_CTX(data);

    return stream && (quiche_conn_stream_writable(ctx->qconn, (uint64_t)stream->id, 1) > 0);
}

static void cf_quiche_adjust_pollset(struct Curl_cfilter* cf, struct Curl_easy* data, struct easy_pollset* ps)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
    ...
    if (ctx->qconn && (want_recv || want_send)) {
        ...
        s_exhaust = stream && stream->id >= 0 && (stream->quic_flow_blocked || !stream_is_writeable(cf, data));
 ...
 */
static bool cf_quiche_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data)
{
...
static CURLcode cf_quiche_data_event(struct Curl_cfilter* cf, struct Curl_easy* data, int event, int arg1, void* arg2)
{
    CURLcode result = CURLE_OK;
    ...
    case CF_CTRL_DATA_DONE_SEND: {
        ...
        if (stream && !stream->send_closed) {
            ...
            CURL_TRC_CF(data, cf, "[%" PRId64 "] DONE_SEND -> %zd, %d", stream->id, sent, result);
        }
        break;
    }
...
static CURLcode cf_verify_peer(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
    CURLcode result = CURLE_OK;
...
static CURLcode cf_connect_start(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
    int rv;
    CURLcode result;
    ...
    quiche_config_set_initial_max_stream_data_bidi_local(ctx->cfg, H3_STREAM_WINDOW_SIZE);
    quiche_config_set_initial_max_stream_data_bidi_remote(ctx->cfg, H3_STREAM_WINDOW_SIZE);
    quiche_config_set_initial_max_stream_data_uni(ctx->cfg, H3_STREAM_WINDOW_SIZE);
    quiche_config_set_disable_active_migration(ctx->cfg, TRUE);

    quiche_config_set_max_connection_window(ctx->cfg, 10 * QUIC_MAX_STREAMS * H3_STREAM_WINDOW_SIZE);
    quiche_config_set_max_stream_window(ctx->cfg, 10 * H3_STREAM_WINDOW_SIZE);
    quiche_config_set_application_protos(ctx->cfg, (uint8_t*)QUICHE_H3_APPLICATION_PROTOCOL, sizeof(QUICHE_H3_APPLICATION_PROTOCOL) - 1);
    ...
    ctx->qconn = quiche_conn_new_with_tls((const uint8_t*)ctx->scid, sizeof(ctx->scid), NULL, 0, (struct sockaddr*)&ctx->q.local_addr, ctx->q.local_addrlen,
...

    {
        ...
        CURL_TRC_CF(data, cf, "Sent QUIC client Initial, ALPN: %s", alpn_protocols + 1);
    }

    return CURLE_OK;
}

static CURLcode cf_quiche_connect(struct Curl_cfilter* cf, struct Curl_easy* data, bool blocking, bool* done)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
    CURLcode result = CURLE_OK;
    ...
    *done = FALSE;
    ...
    if (ctx->reconnect_at.tv_sec && Curl_timediff(ctx->q.last_op, ctx->reconnect_at) < 0) {
    ...
    if (quiche_conn_is_established(ctx->qconn)) {
        ...
        CURL_TRC_CF(data, cf, "handshake complete after %dms", (int)Curl_timediff(ctx->handshake_at, ctx->started_at));
...
out:
    ...
    if (result && result != CURLE_AGAIN) {
        ...
        infof(data, "connect to %s port %u failed: %s", r_ip, r_port, curl_easy_strerror(result));
    }
#endif
    return result;
}
...
static CURLcode cf_quiche_query(struct Curl_cfilter* cf, struct Curl_easy* data, int query, int* pres1, void* pres2)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
...
static bool cf_quiche_conn_is_alive(struct Curl_cfilter* cf, struct Curl_easy* data, bool* input_pending)
{
    struct cf_quiche_ctx* ctx = cf->ctx;
    bool alive = TRUE;
...
CURLcode Curl_cf_quiche_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn, const struct Curl_addrinfo* ai)
{
...
bool Curl_conn_is_quiche(const struct Curl_easy* data, const struct connectdata* conn, int sockindex)
{
