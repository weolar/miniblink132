...
void Curl_ngtcp2_ver(char* p, size_t len)
{
    ...
    (void)msnprintf(p, len, "ngtcp2/%s nghttp3/%s", ng2->version_str, ht3->version_str);
}
...
#define H3_STREAM_CTX(d) ((struct h3_stream_ctx*)(((d) && (d)->req.p.http) ? ((struct HTTP*)(d)->req.p.http)->h3_ctx : NULL))
...
static CURLcode h3_data_setup(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    Curl_bufq_initp(&stream->sendbuf, &ctx->stream_bufcp, H3_STREAM_SEND_CHUNKS, BUFQ_OPT_NONE);
   ...
   * headers to it that are not counted against the nghttp3 flow limits. */
    Curl_bufq_initp(&stream->recvbuf, &ctx->stream_bufcp, H3_STREAM_RECV_CHUNKS, BUFQ_OPT_SOFT_LIMIT);
...
static struct Curl_easy* get_stream_easy(struct Curl_cfilter* cf, struct Curl_easy* data, int64_t stream_id)
{
...
static void pktx_update_time(struct pkt_io_ctx* pktx, struct Curl_cfilter* cf)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;

    vquic_ctx_update_time(&ctx->q);
    pktx->ts = ctx->q.last_op.tv_sec * NGTCP2_SECONDS + ctx->q.last_op.tv_usec * NGTCP2_MICROSECONDS;
}

static void pktx_init(struct pkt_io_ctx* pktx, struct Curl_cfilter* cf, struct Curl_easy* data)
{
    pktx->cf = cf;
    pktx->data = data;
    pktx->pkt_count = 0;
...
static CURLcode cf_progress_ingress(struct Curl_cfilter* cf, struct Curl_easy* data, struct pkt_io_ctx* pktx);
static CURLcode cf_progress_egress(struct Curl_cfilter* cf, struct Curl_easy* data, struct pkt_io_ctx* pktx);
...
static void quic_settings(struct cf_ngtcp2_ctx* ctx, struct Curl_easy* data, struct pkt_io_ctx* pktx)
{
...
#elif defined(USE_GNUTLS)
static int keylog_callback(gnutls_session_t session, const char* label, const gnutls_datum_t* secret)
{
...
#ifdef USE_OPENSSL
static CURLcode quic_ssl_ctx(SSL_CTX** pssl_ctx, struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    ...
    {
        const char* curves = conn->ssl_config.curves ? conn->ssl_config.curves : QUIC_GROUPS;
    ...
    {
        const char* ciphers13 = conn->ssl_config.cipher_list13 ? conn->ssl_config.cipher_list13 : QUIC_CIPHERS;
   ...
   * SSL_get_verify_result() below. */
    SSL_CTX_set_verify(ssl_ctx, conn->ssl_config.verifypeer ? SSL_VERIFY_PEER : SSL_VERIFY_NONE, NULL);
...
static CURLcode quic_set_client_cert(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    ...
    if (ssl_config->primary.clientcert || ssl_config->primary.cert_blob || ssl_config->cert_type) {
        return Curl_ossl_set_client_cert(data, ssl_ctx, ssl_config->primary.clientcert, ssl_config->primary.cert_blob, ssl_config->cert_type, ssl_config->key,
...
static CURLcode quic_init_ssl(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    result = gtls_client_init(data, &cf->conn->ssl_config, &data->set.ssl, hostname, ctx->gtls, pverifyresult);
    if (result)
        return result;
...
static CURLcode quic_ssl_ctx(WOLFSSL_CTX** pssl_ctx, struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct connectdata* conn = cf->conn;
    CURLcode result = CURLE_FAILED_INIT;
    ...
    if (wolfSSL_CTX_set_cipher_list(ssl_ctx, conn->ssl_config.cipher_list13 ? conn->ssl_config.cipher_list13 : QUIC_CIPHERS) != 1) {
    ...
    if (wolfSSL_CTX_set1_groups_list(ssl_ctx, conn->ssl_config.curves ? conn->ssl_config.curves : (char*)QUIC_GROUPS) != 1) {
    ...
    if (conn->ssl_config.verifypeer) {
        ...
        if (ssl_cafile || ssl_capath) {
         ...
         the server's certificate. */
            int rc = wolfSSL_CTX_load_verify_locations_ex(ssl_ctx, ssl_cafile, ssl_capath, WOLFSSL_LOAD_FLAG_IGNORE_ERR);
            if (SSL_SUCCESS != rc) {
...
static CURLcode quic_init_ssl(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    /* set SNI */
    wolfSSL_UseSNI(ctx->ssl, WOLFSSL_SNI_HOST_NAME, hostname, (unsigned short)strlen(hostname));
...
static void report_consumed_data(struct Curl_cfilter* cf, struct Curl_easy* data, size_t consumed)
{
    struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
   ...
   * consuming those does not count against flow control. */
    ...
    if (consumed > 0) {
        CURL_TRC_CF(data, cf, "[%" PRId64 "] ACK %zu bytes of DATA", stream->id, consumed);
...
static int cb_recv_stream_data(
    ngtcp2_conn* tconn, uint32_t flags, int64_t stream_id, uint64_t offset, const uint8_t* buf, size_t buflen, void* user_data, void* stream_user_data)
{
    struct Curl_cfilter* cf = user_data;
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    ...
    nconsumed = nghttp3_conn_read_stream(ctx->h3conn, stream_id, buf, buflen, fin);
    CURL_TRC_CF(data, cf, "[%" PRId64 "] read_stream(len=%zu) -> %zd", stream_id, buflen, nconsumed);
    if (nconsumed < 0) {
        ngtcp2_ccerr_set_application_error(&ctx->last_error, nghttp3_err_infer_quic_app_error_code((int)nconsumed), NULL, 0);
...
static int cb_acked_stream_data_offset(ngtcp2_conn* tconn, int64_t stream_id, uint64_t offset, uint64_t datalen, void* user_data, void* stream_user_data)
{
    struct Curl_cfilter* cf = user_data;
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    int rv;
...
static int cb_stream_close(ngtcp2_conn* tconn, uint32_t flags, int64_t stream3_id, uint64_t app_error_code, void* user_data, void* stream_user_data)
{
    struct Curl_cfilter* cf = user_data;
    struct Curl_easy* data = stream_user_data;
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    ...
    CURL_TRC_CF(data, cf, "[%" PRId64 "] quic close(err=%" PRIu64 ") -> %d", stream3_id, app_error_code, rv);
    if (rv) {
        ngtcp2_ccerr_set_application_error(&ctx->last_error, nghttp3_err_infer_quic_app_error_code(rv), NULL, 0);
        return NGTCP2_ERR_CALLBACK_FAILURE;
    }
...
static int cb_stream_reset(ngtcp2_conn* tconn, int64_t stream_id, uint64_t final_size, uint64_t app_error_code, void* user_data, void* stream_user_data)
{
    struct Curl_cfilter* cf = user_data;
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    struct Curl_easy* data = stream_user_data;
...
static int cb_stream_stop_sending(ngtcp2_conn* tconn, int64_t stream_id, uint64_t app_error_code, void* user_data, void* stream_user_data)
{
    struct Curl_cfilter* cf = user_data;
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    int rv;
...
static int cb_extend_max_local_streams_bidi(ngtcp2_conn* tconn, uint64_t max_streams, void* user_data)
{
    (void)tconn;
    (void)max_streams;
    (void)user_data;
...
static int cb_extend_max_stream_data(ngtcp2_conn* tconn, int64_t stream_id, uint64_t max_data, void* user_data, void* stream_user_data)
{
    struct Curl_cfilter* cf = user_data;
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
...
static int cb_get_new_connection_id(ngtcp2_conn* tconn, ngtcp2_cid* cid, uint8_t* token, size_t cidlen, void* user_data)
{
    CURLcode result;
    (void)tconn;
    (void)user_data;
...
static int cb_recv_rx_key(ngtcp2_conn* tconn, ngtcp2_encryption_level level, void* user_data)
{
    struct Curl_cfilter* cf = user_data;
    (void)tconn;
...
static ngtcp2_callbacks ng_callbacks = {
    ngtcp2_crypto_client_initial_cb, NULL, /* recv_client_initial */
    ngtcp2_crypto_recv_crypto_data_cb, cb_handshake_completed, NULL, /* recv_version_negotiation */
    ngtcp2_crypto_encrypt_cb, ngtcp2_crypto_decrypt_cb, ngtcp2_crypto_hp_mask_cb, cb_recv_stream_data, cb_acked_stream_data_offset, NULL, /* stream_open */
    cb_stream_close, NULL, /* recv_stateless_reset */
    ngtcp2_crypto_recv_retry_cb, cb_extend_max_local_streams_bidi, NULL, /* extend_max_local_streams_uni */
    cb_rand, cb_get_new_connection_id, NULL, /* remove_connection_id */
    ngtcp2_crypto_update_key_cb, /* update_key */
    NULL, /* path_validation */
    NULL, /* select_preferred_addr */
    cb_stream_reset, NULL, /* extend_max_remote_streams_bidi */
    NULL, /* extend_max_remote_streams_uni */
    cb_extend_max_stream_data, NULL, /* dcid_status */
    NULL, /* handshake_confirmed */
    NULL, /* recv_new_token */
    ngtcp2_crypto_delete_crypto_aead_ctx_cb, ngtcp2_crypto_delete_crypto_cipher_ctx_cb, NULL, /* recv_datagram */
    NULL, /* ack_datagram */
    NULL, /* lost_datagram */
    ngtcp2_crypto_get_path_challenge_data_cb, cb_stream_stop_sending, NULL, /* version_negotiation */
    cb_recv_rx_key, NULL, /* recv_tx_key */
 ...
 */
static CURLcode check_and_set_expiry(struct Curl_cfilter* cf, struct Curl_easy* data, struct pkt_io_ctx* pktx)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    ...
    if (expiry != UINT64_MAX) {
        if (expiry <= pktx->ts) {
            ...
            if (rv) {
                failf(data, "ngtcp2_conn_handle_expiry returned error: %s", ngtcp2_strerror(rv));
...
static void cf_ngtcp2_adjust_pollset(struct Curl_cfilter* cf, struct Curl_easy* data, struct easy_pollset* ps)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    ...
    if (ctx->qconn && (want_recv || want_send)) {
        ...
        c_exhaust = !ngtcp2_conn_get_cwnd_left(ctx->qconn) || !ngtcp2_conn_get_max_data_left(ctx->qconn);
        s_exhaust = stream && stream->id >= 0 && stream->quic_flow_blocked;
        want_recv = (want_recv || c_exhaust || s_exhaust);
        want_send = (!s_exhaust && want_send) || !Curl_bufq_is_empty(&ctx->q.sendbuf);
...
static int cb_h3_stream_close(nghttp3_conn* conn, int64_t stream_id, uint64_t app_error_code, void* user_data, void* stream_user_data)
{
    ...
    if (stream->error3 != NGHTTP3_H3_NO_ERROR) {
        ...
        CURL_TRC_CF(data, cf, "[%" PRId64 "] RESET: error %" PRId64, stream->id, stream->error3);
    } else {
 ...
 */
static CURLcode write_resp_raw(struct Curl_cfilter* cf, struct Curl_easy* data, const void* mem, size_t memlen, bool flow)
{
    struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
...
static int cb_h3_recv_data(nghttp3_conn* conn, int64_t stream3_id, const uint8_t* buf, size_t buflen, void* user_data, void* stream_user_data)
{
    struct Curl_cfilter* cf = user_data;
    struct Curl_easy* data = stream_user_data;
    struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
    ...
    if (result) {
        CURL_TRC_CF(data, cf, "[%" PRId64 "] DATA len=%zu, ERROR receiving %d", stream->id, buflen, result);
        return NGHTTP3_ERR_CALLBACK_FAILURE;
    }
...
static int cb_h3_recv_header(
    nghttp3_conn* conn, int64_t stream_id, int32_t token, nghttp3_rcbuf* name, nghttp3_rcbuf* value, uint8_t flags, void* user_data, void* stream_user_data)
{
    struct Curl_cfilter* cf = user_data;
    ...
    if (token == NGHTTP3_QPACK_TOKEN__STATUS) {
        ...
        result = Curl_http_decode_status(&stream->status_code, (const char*)h3val.base, h3val.len);
        if (result)
            return -1;
...
static int cb_h3_reset_stream(nghttp3_conn* conn, int64_t stream_id, uint64_t app_error_code, void* user_data, void* stream_user_data)
{
    ...
    rv = ngtcp2_conn_shutdown_stream_write(ctx->qconn, 0, stream_id, app_error_code);
    CURL_TRC_CF(data, cf, "[%" PRId64 "] reset -> %d", stream_id, rv);
    if (rv && rv != NGTCP2_ERR_STREAM_NOT_FOUND) {
        return NGTCP2_ERR_CALLBACK_FAILURE;
    }
...
static nghttp3_callbacks ngh3_callbacks = {
    ...
    cb_h3_stream_close, cb_h3_recv_data, cb_h3_deferred_consume, NULL, /* begin_headers */
    cb_h3_recv_header, cb_h3_end_headers, NULL, /* begin_trailers */
    cb_h3_recv_header, NULL, /* end_trailers */
    cb_h3_stop_sending, NULL, /* end_stream */
    cb_h3_reset_stream, NULL, /* shutdown */
    NULL /* recv_settings */
};
...
static int init_ngh3_conn(struct Curl_cfilter* cf)
{
    ...
    rc = nghttp3_conn_client_new(&ctx->h3conn, &ngh3_callbacks, &ctx->h3settings, nghttp3_mem_default(), cf);
    if (rc) {
        result = CURLE_OUT_OF_MEMORY;
        goto fail;
    }
    ...
    rc = nghttp3_conn_bind_qpack_streams(ctx->h3conn, qpack_enc_stream_id, qpack_dec_stream_id);
    if (rc) {
        result = CURLE_QUIC_CONNECT_ERROR;
        goto fail;
    }
...
static ssize_t recv_closed_stream(struct Curl_cfilter* cf, struct Curl_easy* data, struct h3_stream_ctx* stream, CURLcode* err)
{
    ssize_t nread = -1;
...
static ssize_t cf_ngtcp2_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t len, CURLcode* err)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
    ssize_t nread = -1;
...
out:
    ...
    CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_recv(len=%zu) -> %zd, %d", stream ? stream->id : -1, len, nread, *err);
    CF_DATA_RESTORE(cf, save);
    return nread;
}
...
static nghttp3_ssize cb_h3_read_req_body(
    nghttp3_conn* conn, int64_t stream_id, nghttp3_vec* vec, size_t veccnt, uint32_t* pflags, void* user_data, void* stream_user_data)
{
    struct Curl_cfilter* cf = user_data;
    struct Curl_easy* data = stream_user_data;
    struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
   ...
   * to nghttp3. Do that now, if we can. */
    if (stream->sendbuf_len_in_flight < Curl_bufq_len(&stream->sendbuf)) {
        nvecs = 0;
        while (nvecs < veccnt && Curl_bufq_peek_at(&stream->sendbuf, stream->sendbuf_len_in_flight, (const unsigned char**)&vec[nvecs].base, &vec[nvecs].len)) {
   ...
   * we are at the end of the request body. */
    ...
    CURL_TRC_CF(data, cf,
        ...
        stream->id, (int)nvecs, *pflags == NGHTTP3_DATA_FLAG_EOF ? " EOF" : "", nwritten, Curl_bufq_len(&stream->sendbuf), stream->upload_left);
...
static ssize_t h3_stream_open(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len, CURLcode* err)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    ...
    *err = Curl_http_req_to_h2(&h2_headers, stream->h1.req, data);
    ...
    rc = nghttp3_conn_submit_request(ctx->h3conn, stream->id, nva, nheader, preader, data);
    if (rc) {
        switch (rc) {
        case NGHTTP3_ERR_CONN_CLOSING:
            ...
            break;
        default:
            CURL_TRC_CF(data, cf, "h3sid[%" PRId64 "] failed to send -> %d (%s)", stream->id, rc, ngtcp2_strerror(rc));
            break;
        }
        *err = CURLE_SEND_ERROR;
        nwritten = -1;
    ...
    if (Curl_trc_is_verbose(data)) {
        infof(data, "[HTTP/3] [%" PRId64 "] OPENED stream for %s", stream->id, data->state.url);
        for (i = 0; i < nheader; ++i) {
            infof(data, "[HTTP/3] [%" PRId64 "] [%.*s: %.*s]", stream->id, (int)nva[i].namelen, nva[i].name, (int)nva[i].valuelen, nva[i].value);
        }
    }
...
static ssize_t cf_ngtcp2_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len, CURLcode* err)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
    ssize_t sent = 0;
...
out:
    ...
    CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_send(len=%zu) -> %zd, %d", stream ? stream->id : -1, len, sent, *err);
    CF_DATA_RESTORE(cf, save);
    return sent;
}

static CURLcode qng_verify_peer(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    CURLcode result = CURLE_OK;
    ...
    if (cf->conn->ssl_config.verifyhost) {
        ...
        result = Curl_gtls_verifyserver(
            data, ctx->gtls->session, &cf->conn->ssl_config, &data->set.ssl, hostname, disp_hostname, data->set.str[STRING_SSL_PINNEDPUBLICKEY]);
        if (result)
            return result;
#elif defined(USE_WOLFSSL)
...
static CURLcode recv_pkt(const unsigned char* pkt, size_t pktlen, struct sockaddr_storage* remote_addr, socklen_t remote_addrlen, int ecn, void* userp)
{
    ...
    ++pktx->pkt_count;
    ngtcp2_addr_init(&path.local, (struct sockaddr*)&ctx->q.local_addr, ctx->q.local_addrlen);
    ...
    if (rv) {
        CURL_TRC_CF(pktx->data, pktx->cf, "ingress, read_pkt -> %s", ngtcp2_strerror(rv));
        if (!ctx->last_error.error_code) {
            if (rv == NGTCP2_ERR_CRYPTO) {
                ngtcp2_ccerr_set_tls_alert(&ctx->last_error, ngtcp2_conn_get_tls_alert(ctx->qconn), NULL, 0);
            } else {
                ngtcp2_ccerr_set_liberr(&ctx->last_error, rv, NULL, 0);
            }
        }
 ...
 */
static ssize_t read_pkt_to_send(void* userp, unsigned char* buf, size_t buflen, CURLcode* err)
{
    ...
    *err = CURLE_OK;
    for (;;) {
        ...
        if (ctx->h3conn && ngtcp2_conn_get_max_data_left(ctx->qconn)) {
            veccnt = nghttp3_conn_writev_stream(ctx->h3conn, &stream_id, &fin, vec, sizeof(vec) / sizeof(vec[0]));
            if (veccnt < 0) {
                failf(x->data, "nghttp3_conn_writev_stream returned error: %s", nghttp3_strerror((int)veccnt));
                ngtcp2_ccerr_set_application_error(&ctx->last_error, nghttp3_err_infer_quic_app_error_code((int)veccnt), NULL, 0);
                *err = CURLE_SEND_ERROR;
                return -1;
            }
        }
        ...
        n = ngtcp2_conn_writev_stream(ctx->qconn, &x->ps.path, NULL, buf, buflen, &ndatalen, flags, stream_id, (const ngtcp2_vec*)vec, veccnt, x->ts);
        if (n == 0) {
            /* nothing to send */
            *err = CURLE_AGAIN;
            nwritten = -1;
            ...
            case NGTCP2_ERR_STREAM_DATA_BLOCKED: {
                ...
                CURL_TRC_CF(x->data, x->cf, "[%" PRId64 "] block quic flow", stream_id);
                DEBUGASSERT(stream);
                if (stream)
         ...
         * is in the buffer and continue */
            ...
            default:
                DEBUGASSERT(ndatalen == -1);
                failf(x->data, "ngtcp2_conn_writev_stream returned error: %s", ngtcp2_strerror((int)n));
...
static CURLcode cf_progress_egress(struct Curl_cfilter* cf, struct Curl_easy* data, struct pkt_io_ctx* pktx)
{
   ...
   */
    ...
    path_max_payload_size = ngtcp2_conn_get_path_max_tx_udp_payload_size(ctx->qconn);
    /* maximum number of packets buffered before we flush to the socket */
    max_pktcnt = CURLMIN(MAX_PKT_BURST, ctx->q.sendbuf.chunk_size / max_payload_size);

    for (;;) {
        /* add the next packet to send, if any, to our buffer */
        nread = Curl_bufq_sipn(&ctx->q.sendbuf, max_payload_size, read_pkt_to_send, pktx, &curlcode);
        if (nread < 0) {
            if (curlcode != CURLE_AGAIN)
                return curlcode;
       ...
       * payload size or it is a PMTUD. We'll see. */
            gsolen = (size_t)nread;
        } else if ((size_t)nread > gsolen || (gsolen > path_max_payload_size && (size_t)nread != gsolen)) {
 ...
 */
static bool cf_ngtcp2_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data)
{
...
static CURLcode h3_data_pause(struct Curl_cfilter* cf, struct Curl_easy* data, bool pause)
{
...
static CURLcode cf_ngtcp2_data_event(struct Curl_cfilter* cf, struct Curl_easy* data, int event, int arg1, void* arg2)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    CURLcode result = CURLE_OK;
    struct cf_call_data save;
...
static void cf_ngtcp2_close(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    if (ctx && ctx->qconn) {
        ...
        rc = ngtcp2_conn_write_connection_close(ctx->qconn, NULL, /* path */
            NULL, /* pkt_info */
            (uint8_t*)buffer, sizeof(buffer), &ctx->last_error, pktx.ts);
        if (rc > 0) {
            while ((send(ctx->q.sockfd, buffer, (SEND_TYPE_ARG3)rc, 0) == -1) && SOCKERRNO == EINTR)
                ;
        }
 ...
 */
static CURLcode cf_connect_start(struct Curl_cfilter* cf, struct Curl_easy* data, struct pkt_io_ctx* pktx)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    int rc;
    int rv;
    ...
    Curl_bufcp_init(&ctx->stream_bufcp, H3_STREAM_CHUNK_SIZE, H3_STREAM_POOL_SPARES);
    ...
    Curl_cf_socket_peek(cf->next, data, &ctx->q.sockfd, &sockaddr, NULL, NULL, NULL, NULL);
    if (!sockaddr)
        return CURLE_QUIC_CONNECT_ERROR;
    ctx->q.local_addrlen = sizeof(ctx->q.local_addr);
    rv = getsockname(ctx->q.sockfd, (struct sockaddr*)&ctx->q.local_addr, &ctx->q.local_addrlen);
    if (rv == -1)
        return CURLE_QUIC_CONNECT_ERROR;

    ngtcp2_addr_init(&ctx->connected_path.local, (struct sockaddr*)&ctx->q.local_addr, ctx->q.local_addrlen);
    ngtcp2_addr_init(&ctx->connected_path.remote, &sockaddr->sa_addr, sockaddr->addrlen);

    rc = ngtcp2_conn_client_new(
        &ctx->qconn, &ctx->dcid, &ctx->scid, &ctx->connected_path, NGTCP2_PROTO_VER_V1, &ng_callbacks, &ctx->settings, &ctx->transport_params, NULL, cf);
    if (rc)
        return CURLE_QUIC_CONNECT_ERROR;
...
static CURLcode cf_ngtcp2_connect(struct Curl_cfilter* cf, struct Curl_easy* data, bool blocking, bool* done)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    CURLcode result = CURLE_OK;
    struct cf_call_data save;
    ...
    *done = FALSE;
    ...
    if (ngtcp2_conn_get_handshake_completed(ctx->qconn)) {
        ...
        CURL_TRC_CF(data, cf, "handshake complete after %dms", (int)Curl_timediff(now, ctx->started_at));
...
out:
    if (result == CURLE_RECV_ERROR && ctx->qconn && ngtcp2_conn_in_draining_period(ctx->qconn)) {
     ...
     */
        ...
        CURL_TRC_CF(data, cf, "connect, remote closed, reconnect after %dms", reconn_delay_ms);
    ...
    if (result) {
        ...
        infof(data, "QUIC connect to %s port %u failed: %s", r_ip, r_port, curl_easy_strerror(result));
    }
#endif
    if (!result && ctx->qconn) {
        result = check_and_set_expiry(cf, data, &pktx);
...
static CURLcode cf_ngtcp2_query(struct Curl_cfilter* cf, struct Curl_easy* data, int query, int* pres1, void* pres2)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    struct cf_call_data save;
    ...
    case CF_QUERY_MAX_CONCURRENT: {
        ...
        if (rp)
            *pres1 = (rp->initial_max_streams_bidi > INT_MAX) ? INT_MAX : (int)rp->initial_max_streams_bidi;
...
static bool cf_ngtcp2_conn_is_alive(struct Curl_cfilter* cf, struct Curl_easy* data, bool* input_pending)
{
    struct cf_ngtcp2_ctx* ctx = cf->ctx;
    bool alive = TRUE;
    const ngtcp2_transport_params* rp;
   ...
   * to any packets from us. */
    ...
    if (rp) {
        ...
        if (rp->max_idle_timeout && (rp->max_idle_timeout / NGTCP2_MILLISECONDS) < idle_ms)
...
CURLcode Curl_cf_ngtcp2_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn, const struct Curl_addrinfo* ai)
{
...
bool Curl_conn_is_ngtcp2(const struct Curl_easy* data, const struct connectdata* conn, int sockindex)
{
