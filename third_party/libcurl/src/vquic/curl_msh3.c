...
static void MSH3_CALL msh3_conn_connected(MSH3_CONNECTION* Connection, void* IfContext);
static void MSH3_CALL msh3_conn_shutdown_complete(MSH3_CONNECTION* Connection, void* IfContext);
static void MSH3_CALL msh3_conn_new_request(MSH3_CONNECTION* Connection, void* IfContext, MSH3_REQUEST* Request);
static void MSH3_CALL msh3_header_received(MSH3_REQUEST* Request, void* IfContext, const MSH3_HEADER* Header);
static bool MSH3_CALL msh3_data_received(MSH3_REQUEST* Request, void* IfContext, uint32_t* Length, const uint8_t* Data);
static void MSH3_CALL msh3_complete(MSH3_REQUEST* Request, void* IfContext, bool Aborted, uint64_t AbortError);
static void MSH3_CALL msh3_shutdown_complete(MSH3_REQUEST* Request, void* IfContext);
static void MSH3_CALL msh3_data_sent(MSH3_REQUEST* Request, void* IfContext, void* SendContext);
...
static CURLcode h3_data_setup(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    Curl_bufq_init2(&stream->recvbuf, H3_STREAM_CHUNK_SIZE, H3_STREAM_RECV_CHUNKS, BUFQ_OPT_SOFT_LIMIT);
...
static void drain_stream_from_other_thread(struct Curl_easy* data, struct stream_ctx* stream)
{
    unsigned char bits;
...
static const MSH3_CONNECTION_IF msh3_conn_if = { msh3_conn_connected, msh3_conn_shutdown_complete, msh3_conn_new_request };
...
static const MSH3_REQUEST_IF msh3_request_if = { msh3_header_received, msh3_data_received, msh3_complete, msh3_shutdown_complete, msh3_data_sent };
 ...
 */
static CURLcode write_resp_raw(struct Curl_easy* data, const void* mem, size_t memlen)
{
    struct stream_ctx* stream = H3_STREAM_CTX(data);
...
static void MSH3_CALL msh3_header_received(MSH3_REQUEST* Request, void* userp, const MSH3_HEADER* hd)
{
    ...
    if ((hd->NameLength == 7) && !strncmp(HTTP_PSEUDO_STATUS, (char*)hd->Name, 7)) {
...
static bool MSH3_CALL msh3_data_received(MSH3_REQUEST* Request, void* IfContext, uint32_t* buflen, const uint8_t* buf)
{
...
static void MSH3_CALL msh3_complete(MSH3_REQUEST* Request, void* IfContext, bool aborted, uint64_t error)
{
    struct Curl_easy* data = IfContext;
    struct stream_ctx* stream = H3_STREAM_CTX(data);
...
static ssize_t recv_closed_stream(struct Curl_cfilter* cf, struct Curl_easy* data, CURLcode* err)
{
    ...
    if (stream->reset) {
        ...
        *err = CURLE_PARTIAL_FILE;
        ...
        failf(data, "HTTP/3 stream was not closed cleanly: (error %zd)", (ssize_t)stream->error3);
        *err = CURLE_HTTP3;
...
static ssize_t cf_msh3_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t len, CURLcode* err)
{
    struct stream_ctx* stream = H3_STREAM_CTX(data);
    ssize_t nread = -1;
...
static ssize_t cf_msh3_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len, CURLcode* err)
{
    struct cf_msh3_ctx* ctx = cf->ctx;
    struct stream_ctx* stream = H3_STREAM_CTX(data);
    ...
    if (!stream->req) {
        ...
        *err = Curl_http_req_to_h2(&h2_headers, h1.req, data);
        ...
        stream->req = MsH3RequestOpen(ctx->qconn, &msh3_request_if, data, nva, nheader, eos ? MSH3_REQUEST_FLAG_FIN : MSH3_REQUEST_FLAG_NONE);
        if (!stream->req) {
        ...
        *err = CURLE_OK;
        ...
        if (!MsH3RequestSend(stream->req, MSH3_REQUEST_FLAG_NONE, buf, (uint32_t)len, stream)) {
            *err = CURLE_SEND_ERROR;
            goto out;
        }
...
static void cf_msh3_adjust_pollset(struct Curl_cfilter* cf, struct Curl_easy* data, struct easy_pollset* ps)
{
    struct cf_msh3_ctx* ctx = cf->ctx;
    struct stream_ctx* stream = H3_STREAM_CTX(data);
    struct cf_call_data save;
...
static bool cf_msh3_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data)
{
    struct stream_ctx* stream = H3_STREAM_CTX(data);
    struct cf_call_data save;
    bool pending = FALSE;
...
static CURLcode cf_msh3_data_event(struct Curl_cfilter* cf, struct Curl_easy* data, int event, int arg1, void* arg2)
{
    struct stream_ctx* stream = H3_STREAM_CTX(data);
    struct cf_call_data save;
    CURLcode result = CURLE_OK;
...
static CURLcode cf_connect_start(struct Curl_cfilter* cf, struct Curl_easy* data)
{
    ...
    CURL_TRC_CF(data, cf, "connecting to %s:%d (verify=%d)", cf->conn->host.name, (int)cf->conn->remote_port, verify);
    ...
    ctx->qconn = MsH3ConnectionOpen(ctx->api, &msh3_conn_if, cf, cf->conn->host.name, &addr, !verify);
    if (!ctx->qconn) {
...
static CURLcode cf_msh3_connect(struct Curl_cfilter* cf, struct Curl_easy* data, bool blocking, bool* done)
{
    struct cf_msh3_ctx* ctx = cf->ctx;
    struct cf_call_data save;
    CURLcode result = CURLE_OK;
    ...
    if (ctx) {
       ...
       */
            ...
            if (ctx->sock[SP_LOCAL] == cf->conn->sock[cf->sockindex]) {
                CURL_TRC_CF(data, cf, "cf_msh3_close(%d) active", (int)ctx->sock[SP_LOCAL]);
...
static CURLcode cf_msh3_query(struct Curl_cfilter* cf, struct Curl_easy* data, int query, int* pres1, void* pres2)
{
    struct cf_msh3_ctx* ctx = cf->ctx;
...
static bool cf_msh3_conn_is_alive(struct Curl_cfilter* cf, struct Curl_easy* data, bool* input_pending)
{
    struct cf_msh3_ctx* ctx = cf->ctx;

    (void)data;
    *input_pending = FALSE;
    return ctx && ctx->sock[SP_LOCAL] != CURL_SOCKET_BAD && ctx->qconn && ctx->connected;
}
...
CURLcode Curl_cf_msh3_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn, const struct Curl_addrinfo* ai)
{
...
bool Curl_conn_is_msh3(const struct Curl_easy* data, const struct connectdata* conn, int sockindex)
{
