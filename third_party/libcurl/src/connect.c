...
static const struct Curl_addrinfo* addr_next_match(const struct Curl_addrinfo* addr, int family)
{
...
static int conn_is_conn(struct Curl_easy* data, struct connectdata* conn, void* param)
{
...
)
{
    ...
    closeit = (ctrl == CONNCTRL_CONNECTION) || ((ctrl == CONNCTRL_STREAM) && !is_multiplex);
...
static CURLcode eyeballer_new(struct eyeballer** pballer, cf_ip_connect_create* cf_create, const struct Curl_addrinfo* addr, int ai_family,
    struct eyeballer* primary, timediff_t delay_ms, timediff_t timeout_ms, expire_id timeout_id)
{
 ...
 */
static void baller_initiate(struct Curl_cfilter* cf, struct Curl_easy* data, struct eyeballer* baller)
{
    ...
    result = baller->cf_create(&baller->cf, data, cf->conn, baller->addr, ctx->transport);
    if (result)
        goto out;
 ...
 */
static CURLcode baller_start(struct Curl_cfilter* cf, struct Curl_easy* data, struct eyeballer* baller, timediff_t timeoutms)
{
    baller->error = 0;
...
static CURLcode baller_start_next(struct Curl_cfilter* cf, struct Curl_easy* data, struct eyeballer* baller, timediff_t timeoutms)
{
...
static CURLcode baller_connect(struct Curl_cfilter* cf, struct Curl_easy* data, struct eyeballer* baller, struct curltime* now, bool* connected)
{
    (void)cf;
    ...
    if (!baller->result && !*connected) {
        ...
        if (!baller->result) {
            if (*connected) {
                ...
                infof(data, "%s connect timeout after %" CURL_FORMAT_TIMEDIFF_T "ms, move on!", baller->name, baller->timeoutms);
 ...
 */
static CURLcode is_connected(struct Curl_cfilter* cf, struct Curl_easy* data, bool* connected)
{
    struct cf_he_ctx* ctx = cf->ctx;
...
evaluate:
    *connected = FALSE; /* a very negative world view is best */
    ...
    for (i = 0; i < ARRAYSIZE(ctx->baller); i++) {
        ...
        CURL_TRC_CF(data, cf, "%s connect -> %d, connected=%d", baller->name, baller->result, *connected);
   ...
   * start new ballers or return ok. */
    if ((ongoing || not_started) && Curl_timeleft(data, &now, TRUE) < 0) {
        failf(data, "Connection timeout after %" CURL_FORMAT_CURL_OFF_T " ms", Curl_timediff(now, data->progress.t_startsingle));
    ...
    if (not_started > 0) {
       ...
       * its start delay_ms have expired */
            if ((baller->primary && baller->primary->is_done) || Curl_timediff(now, ctx->started) >= baller->delay_ms) {
                baller_start(cf, data, baller, Curl_timeleft(data, &now, TRUE));
                if (baller->is_done) {
                    CURL_TRC_CF(data, cf, "%s done", baller->name);
                } else {
                    CURL_TRC_CF(data, cf, "%s starting (timeout=%" CURL_FORMAT_TIMEDIFF_T "ms)", baller->name, baller->timeoutms);
                    ++ongoing;
                    ++added;
                }
            }
    ...
    for (i = 0; i < ARRAYSIZE(ctx->baller); i++) {
        ...
        if (!baller)
            continue;
        CURL_TRC_CF(data, cf, "%s assess started=%d, result=%d", baller->name, baller->has_started, baller->result);
    ...
    failf(data,
        ...
        hostname, conn->port, Curl_timediff(now, data->progress.t_startsingle), curl_easy_strerror(result));
 ...
 */
static CURLcode start_connect(struct Curl_cfilter* cf, struct Curl_easy* data, const struct Curl_dns_entry* remotehost)
{
    struct cf_he_ctx* ctx = cf->ctx;
    struct connectdata* conn = cf->conn;
   ...
   * this might give NULL, if we do not have any matches. */
    ...
    result = eyeballer_new(&ctx->baller[0], ctx->cf_create, addr0, ai_family0, NULL, 0, /* no primary/delay, start now */
    ...
    if (addr1) {
        ...
        result = eyeballer_new(&ctx->baller[1], ctx->cf_create, addr1, ai_family1, ctx->baller[0], /* wait on that to fail */
            /* or start this delayed */
            data->set.happy_eyeballs_timeout, timeout_ms, EXPIRE_DNS_PER_NAME2);
        if (result)
            return result;
        CURL_TRC_CF(data, cf, "created %s (timeout %" CURL_FORMAT_TIMEDIFF_T "ms)", ctx->baller[1]->name, ctx->baller[1]->timeoutms);
...
static void cf_he_adjust_pollset(struct Curl_cfilter* cf, struct Curl_easy* data, struct easy_pollset* ps)
{
    struct cf_he_ctx* ctx = cf->ctx;
    size_t i;
...
static CURLcode cf_he_connect(struct Curl_cfilter* cf, struct Curl_easy* data, bool blocking, bool* done)
{
    struct cf_he_ctx* ctx = cf->ctx;
...
static bool cf_he_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data)
{
    struct cf_he_ctx* ctx = cf->ctx;
    size_t i;
...
static struct curltime get_max_baller_time(struct Curl_cfilter* cf, struct Curl_easy* data, int query)
{
    struct cf_he_ctx* ctx = cf->ctx;
...
static CURLcode cf_he_query(struct Curl_cfilter* cf, struct Curl_easy* data, int query, int* pres1, void* pres2)
{
    struct cf_he_ctx* ctx = cf->ctx;
    ...
    if (!cf->connected) {
        ...
        case CF_QUERY_CONNECT_REPLY_MS: {
            ...
            for (i = 0; i < ARRAYSIZE(ctx->baller); i++) {
                ...
                if (baller && baller->cf && !baller->cf->cft->query(baller->cf, data, query, &breply_ms, NULL)) {
 ...
 *                   actual connects.
 */
static CURLcode cf_happy_eyeballs_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn, cf_ip_connect_create* cf_create,
    const struct Curl_dns_entry* remotehost, int transport)
{
...
static CURLcode cf_setup_connect(struct Curl_cfilter* cf, struct Curl_easy* data, bool blocking, bool* done)
{
...
#ifdef USE_SSL
        if ((ctx->ssl_mode == CURL_CF_SSL_ENABLE || (ctx->ssl_mode != CURL_CF_SSL_DISABLE && cf->conn->handler->flags & PROTOPT_SSL)) /* we want SSL */
...
static CURLcode cf_setup_create(struct Curl_cfilter** pcf, struct Curl_easy* data, const struct Curl_dns_entry* remotehost, int transport, int ssl_mode)
{
...
static CURLcode cf_setup_add(
    struct Curl_easy* data, struct connectdata* conn, int sockindex, const struct Curl_dns_entry* remotehost, int transport, int ssl_mode)
{
    struct Curl_cfilter* cf;
    CURLcode result = CURLE_OK;
...
CURLcode Curl_conn_setup(struct Curl_easy* data, struct connectdata* conn, int sockindex, const struct Curl_dns_entry* remotehost, int ssl_mode)
{
    ...
    if (!conn->cfilter[sockindex]) {
        result = cf_setup_add(data, conn, sockindex, remotehost, conn->transport, ssl_mode);
        if (result)
            goto out;
    }
