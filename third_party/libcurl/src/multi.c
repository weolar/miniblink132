...
#define GOOD_MULTI_HANDLE(x) (((x) && (x)->magic == CURL_MULTI_HANDLE) ? TRUE : (DEBUGASSERT(!(x)), FALSE))
#else
...
)
{
    ...
    if (data->mstate >= MSTATE_PENDING && data->mstate < MSTATE_COMPLETED) {
        infof(data, "STATE: %s => %s handle %p; line %d", multi_statename[oldstate], multi_statename[data->mstate], (void*)data, lineno);
    }
#endif
...
{
    return Curl_multi_handle(CURL_SOCKET_HASH_TABLE_SIZE, CURL_CONNECTION_HASH_SIZE, CURL_DNS_HASH_SIZE);
}
...
static void multi_warn_debug(struct Curl_multi* multi, struct Curl_easy* data)
{
    if (!multi->warned) {
        infof(data, "!!! WARNING !!!");
        infof(data,
            "This is a debug build of libcurl, "
    ...
    bool premature)
{
  ...
  */
    ...
    if ((data->set.reuse_forbid
        ...
        || conn->bits.close || (premature && !Curl_conn_is_multiplex(conn, FIRSTSOCKET))) {
        DEBUGF(infof(data,
            "multi_done, not reusing connection=%" CURL_FORMAT_CURL_OFF_T ", forbid=%d"
            ", close=%d, premature=%d, conn_multiplex=%d",
            conn->connection_id, data->set.reuse_forbid, conn->bits.close, premature, Curl_conn_is_multiplex(conn, FIRSTSOCKET)));
        ...
        msnprintf(buffer, sizeof(buffer), "Connection #%" CURL_FORMAT_CURL_OFF_T " to host %s left intact", connection_id, host);
 ...
 * for transfer `data`. */
static void multi_getsock(struct Curl_easy* data, struct easy_pollset* ps)
{
...
static CURLMcode multi_wait(struct Curl_multi* multi, struct curl_waitfd extra_fds[], unsigned int extra_nfds, int timeout_ms, int* ret,
         ...
         and remove them from the event. */
            if (curlfds) {
                ...
                for (data = multi->easyp; data; data = data->next) {
                    ...
                    for (i = 0; i < ps.num; i++) {
                        wsa_events.lNetworkEvents = 0;
                        if (WSAEnumNetworkEvents(ps.sockets[i], NULL, &wsa_events) == 0) {
                            if (ret && !pollrc && wsa_events.lNetworkEvents)
                                retcode++;
                        }
 ...
 */
static bool multi_handle_timeout(struct Curl_easy* data, struct curltime* now, bool* stream_error, CURLcode* result, bool connect_timeout)
{
    ...
    if (timeout_ms < 0) {
        ...
        if (data->mstate == MSTATE_RESOLVING)
            ...
            if (k->size != -1) {
                failf(data,
                    "Operation timed out after %" CURL_FORMAT_TIMEDIFF_T " milliseconds with %" CURL_FORMAT_CURL_OFF_T " out of %" CURL_FORMAT_CURL_OFF_T
                    " bytes received",
                    Curl_timediff(*now, data->progress.t_startsingle), k->bytecount, k->size);
            } else {
                failf(data, "Operation timed out after %" CURL_FORMAT_TIMEDIFF_T " milliseconds with %" CURL_FORMAT_CURL_OFF_T " bytes received",
...
#endif
    if (data->set.postfields || (data->state.httpreq == HTTPREQ_GET) || (data->state.httpreq == HTTPREQ_HEAD))
        ; /* no need to rewind */
...
static CURLMcode multi_runsingle(struct Curl_multi* multi, struct curltime* nowp, struct Curl_easy* data)
{
    ...
    do {
        ...
        case MSTATE_DO:
            if (data->set.fprereq) {
                ...
                prereq_rc = data->set.fprereq(
                    data->set.prereq_userp, data->info.conn_primary_ip, data->info.conn_local_ip, data->info.conn_primary_port, data->info.conn_local_port);
                Curl_set_in_callback(data, false);
       ...
       */
            ...
            if (result) {
                ...
                if (data->set.max_send_speed)
                    send_timeout_ms = Curl_pgrsLimitWaitTime(
                        data->progress.uploaded, data->progress.ul_limit_size, data->set.max_send_speed, data->progress.ul_limit_start, *nowp);

                recv_timeout_ms = 0;
                if (data->set.max_recv_speed)
                    recv_timeout_ms = Curl_pgrsLimitWaitTime(
                        data->progress.downloaded, data->progress.dl_limit_size, data->set.max_recv_speed, data->progress.dl_limit_start, *nowp);
        ...
        if (data->conn && data->mstate >= MSTATE_CONNECT && data->mstate < MSTATE_DO && rc != CURLM_CALL_MULTI_PERFORM && !multi_ischanged(multi, false)) {
    ...
    statemachine_end:
        ...
        if (MSTATE_COMPLETED == data->mstate) {
            ...
            Curl_llist_insert_next(&multi->msgsent, multi->msgsent.tail, data, &data->connect_queue);
            /* unlink from the main list */
            unlink_easy(multi, data);
            return CURLM_OK;
        }
...
static CURLMcode singlesocket(struct Curl_multi* multi, struct Curl_easy* data)
{
    ...
    for (i = 0; i < cur_poll.num; i++) {
        ...
        if (multi->socket_cb) {
            ...
            rc = multi->socket_cb(data, s, comboaction, multi->socket_userp, entry->socketp);
       ...
       already by Curl_multi_closed() */
        if (entry) {
            ...
            if (!entry->users) {
                if (multi->socket_cb) {
                    set_in_callback(multi, TRUE);
                    rc = multi->socket_cb(data, s, CURL_POLL_REMOVE, multi->socket_userp, entry->socketp);
                    set_in_callback(multi, FALSE);
                    if (rc == -1) {
                        multi->dead = TRUE;
                        return CURLM_ABORTED_BY_CALLBACK;
...
{
    struct Curl_easy** a = malloc(sizeof(struct Curl_easy*) * (multi->num_easy + 1));
    if (a) {
        int i = 0;
