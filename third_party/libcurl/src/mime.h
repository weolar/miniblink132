...
struct mime_encoder {
    ...
    size_t (*encodefunc)(char* buffer, size_t size, bool ateof, curl_mimepart* part); /* Encoded read. */
...
CURLcode Curl_mime_duppart(struct Curl_easy* data, struct curl_mimepart* dst, const curl_mimepart* src);
CURLcode Curl_mime_set_subparts(struct curl_mimepart* part, struct curl_mime* subparts, int take_ownership);
CURLcode Curl_mime_prepare_headers(
    struct Curl_easy* data, struct curl_mimepart* part, const char* contenttype, const char* disposition, enum mimestrategy strategy);
