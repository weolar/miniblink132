...
static CURLcode smtp_parse_address(const char* fqma, char** address, struct hostname* host);
static CURLcode smtp_perform_auth(struct Curl_easy* data, const char* mech, const struct bufref* initresp);
static CURLcode smtp_continue_auth(struct Curl_easy* data, const char* mech, const struct bufref* resp);
...
const struct Curl_handler Curl_handler_smtp = { "SMTP", /* scheme */
...
static CURLcode smtp_continue_auth(struct Curl_easy* data, const char* mech, const struct bufref* resp)
{
    ...
    return Curl_pp_sendf(data, &smtpc->pp, "%s", (const char*)Curl_bufref_ptr(resp));
}
...
static CURLcode smtp_perform_authentication(struct Curl_easy* data)
{
    ...
    if (!smtpc->auth_supported || !Curl_sasl_can_authenticate(&smtpc->sasl, data)) {
        ...
        if ((!smtp->custom) || (!smtp->custom[0])) {
         ...
         host is a local system) */
            result = Curl_pp_sendf(
                data, &conn->proto.smtpc.pp, "VRFY %s%s%s%s", address, host.name ? "@" : "", host.name ? host.name : "", utf8 ? " SMTPUTF8" : "");
...
static CURLcode smtp_perform_mail(struct Curl_easy* data)
{
    ...
    if (data->set.str[STRING_MAIL_FROM]) {
       ...
       converting the host name to an IDN A-label if necessary */
        result = smtp_parse_address(data->set.str[STRING_MAIL_FROM], &address, &host);
        if (result)
            return result;
    ...
    if (data->set.str[STRING_MAIL_AUTH] && conn->proto.smtpc.sasl.authused) {
        if (data->set.str[STRING_MAIL_AUTH][0] != '\0') {
         ...
         converting the host name to an IDN A-label if necessary */
            result = smtp_parse_address(data->set.str[STRING_MAIL_AUTH], &address, &host);
            if (result) {
                free(from);
                return result;
            }
    ...
    result = Curl_pp_sendf(data, &conn->proto.smtpc.pp, "MAIL FROM:%s%s%s%s%s%s", from, /* Mandatory                 */
...
static CURLcode smtp_perform_rcpt_to(struct Curl_easy* data)
{
    ...
    if (host.name)
        ...
        /* An invalid mailbox was provided but we'll simply let the server worry
       about that and reply with a 501 error */
        result = Curl_pp_sendf(data, &conn->proto.smtpc.pp, "RCPT TO:<%s>", address);
...
static CURLcode smtp_state_ehlo_resp(struct Curl_easy* data, struct connectdata* conn, int smtpcode, smtpstate instate)
{
    ...
    if (smtpcode / 100 != 2 && smtpcode != 1) {
        if (data->set.use_ssl <= CURLUSESSL_TRY || Curl_conn_is_ssl(conn, FIRSTSOCKET))
...
static CURLcode smtp_done(struct Curl_easy* data, CURLcode status, bool premature)
{
    ...
    } else if (!data->set.connect_only && data->set.mail_rcpt && (data->state.upload || data->set.mimepost.kind)) {
