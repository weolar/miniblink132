...
#if defined(CURLRES_SYNCH) && defined(HAVE_ALARM) && defined(SIGALRM) && defined(HAVE_SIGSETJMP) && defined(GLOBAL_INIT_IS_THREADSAFE)
...
static void show_resolve_info(struct Curl_easy* data, struct Curl_dns_entry* dns);
#else
 ...
 */
static size_t create_hostcache_id(const char* name, size_t nlen, /* 0 or actual name length */
 ...
 */
struct Curl_dns_entry* Curl_cache_addr(struct Curl_easy* data, struct Curl_addrinfo* addr, const char* hostname, size_t hostlen, /* length or zero */
    int port)
{
    char entry_id[MAX_HOSTCACHE_LEN];
    size_t entry_len;
...
CURLcode Curl_loadhostpairs(struct Curl_easy* data)
{
    ...
    for (hostp = data->state.resolve; hostp; hostp = hostp->next) {
        ...
        err:
            ...
            if (dns) {
                infof(data, "RESOLVE %.*s:%d - old addresses discarded", (int)hlen, host_begin, port);
...
static void show_resolve_info(struct Curl_easy* data, struct Curl_dns_entry* dns)
{
    ...
    infof(data, "Host %s:%d was resolved.", (dns->hostname[0] ? dns->hostname : "(none)"), dns->hostport);
...
#ifdef CURLRES_IPV6
    infof(data, "IPv6: %s", (Curl_dyn_len(&out[1]) ? Curl_dyn_ptr(&out[1]) : "(none)"));
#endif
    infof(data, "IPv4: %s", (Curl_dyn_len(&out[0]) ? Curl_dyn_ptr(&out[0]) : "(none)"));
...
CURLcode Curl_resolver_error(struct Curl_easy* data)
{
    ...
    failf(data, "Could not resolve %s: %s", host_or_proxy, conn->resolve_async.hostname);
