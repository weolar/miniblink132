...
static CURLcode auth_decode_digest_md5_message(
    const struct bufref* chlgref, char* nonce, size_t nlen, char* realm, size_t rlen, char* alg, size_t alen, char* qop, size_t qlen)
{
    ...
    struct Curl_easy* data, const struct bufref* chlg, const char* userp, const char* passwdp, const char* service, struct bufref* out)
{
    ...
    CURLcode result
        = auth_decode_digest_md5_message(chlg, nonce, sizeof(nonce), realm, sizeof(realm), algorithm, sizeof(algorithm), qop_options, sizeof(qop_options));
    if (result)
        return result;
 ...
 * Returns CURLE_OK on success.
 */
static CURLcode auth_create_digest_http_message(struct Curl_easy* data, const char* userp, const char* passwdp, const unsigned char* request,
    const unsigned char* uripath, struct digestdata* digest, char** outptr, size_t* outlen, void (*convert_to_ascii)(unsigned char*, unsigned char*),
    CURLcode (*hash)(unsigned char*, const unsigned char*, const size_t))
{
    ...
    if (!digest->cnonce) {
        ...
        result = Curl_base64_encode(cnoncebuf, strlen(cnoncebuf), &cnonce, &cnonce_sz);
        if (result)
            return result;
  ...
  */
    ...
    hashthis = aprintf("%s:%s:%s", userp, digest->realm ? digest->realm : "", passwdp);
    if (!hashthis)
        return CURLE_OUT_OF_MEMORY;
  ...
  */
    ...
    if (digest->qop) {
        hashthis = aprintf("%s:%s:%08x:%s:%s:%s", ha1, digest->nonce, digest->nc, digest->cnonce, digest->qop, ha2);
    } else {
  ...
  */
    ...
    if (digest->qop) {
        response = aprintf("username=\"%s\", "
                           ...
                           "qop=%s, "
                           "response=\"%s\"",
            userp_quoted, realm_quoted, nonce_quoted, uripath, digest->cnonce, digest->nc, digest->qop, request_digest);
        ...
        response = aprintf("username=\"%s\", "
                           ...
                           "uri=\"%s\", "
                           "response=\"%s\"",
            userp_quoted, realm_quoted, nonce_quoted, uripath, request_digest);
    }
    free(nonce_quoted);
    free(realm_quoted);
    free(userp_quoted);
...
CURLcode Curl_auth_create_digest_http_message(struct Curl_easy* data, const char* userp, const char* passwdp, const unsigned char* request,
    const unsigned char* uripath, struct digestdata* digest, char** outptr, size_t* outlen)
{
    if (digest->algo <= ALGO_MD5SESS)
        return auth_create_digest_http_message(data, userp, passwdp, request, uripath, digest, outptr, outlen, auth_digest_md5_to_ascii, Curl_md5it);
    DEBUGASSERT(digest->algo <= ALGO_SHA512_256SESS);
    return auth_create_digest_http_message(data, userp, passwdp, request, uripath, digest, outptr, outlen, auth_digest_sha256_to_ascii, Curl_sha256it);
}
