    ...
    struct Curl_easy* data, const char* user, const char* password, const char* service, const char* host, const char* chlg64, struct negotiatedata* nego)
{
    ...
    major_status = Curl_gss_init_sec_context(
        data, &minor_status, &nego->context, nego->spn, &Curl_spnego_mech_oid, GSS_C_NO_CHANNEL_BINDINGS, &input_token, &output_token, TRUE, NULL);
...
CURLcode Curl_auth_create_spnego_message(struct negotiatedata* nego, char** outptr, size_t* outlen)
{
    ...
    result = Curl_base64_encode(nego->output_token.value, nego->output_token.length, outptr, outlen);
