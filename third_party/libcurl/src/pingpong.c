...
/* Returns timeout in ms. 0 or negative number means the timeout has already
   triggered */
timediff_t Curl_pp_state_timeout(struct Curl_easy* data, struct pingpong* pp, bool disconnecting)
{
    ...
    timeout_ms = response_time - Curl_timediff(Curl_now(), pp->response); /* spent time */
 ...
 * Curl_pp_statemach()
 */
CURLcode Curl_pp_statemach(struct Curl_easy* data, struct pingpong* pp, bool block, bool disconnecting)
{
    struct connectdata* conn = data->conn;
    ...
    if (Curl_conn_data_pending(data, FIRSTSOCKET))
        ...
        rc = Curl_socket_check(pp->sendleft ? CURL_SOCKET_BAD : sock, /* reading */
            CURL_SOCKET_BAD, pp->sendleft ? sock : CURL_SOCKET_BAD, /* writing */
            interval_ms);
 ...
 * made to never block
 */
CURLcode Curl_pp_vsendf(struct Curl_easy* data, struct pingpong* pp, const char* fmt, va_list args)
{
 ...
 * made to never block
 */
CURLcode Curl_pp_sendf(struct Curl_easy* data, struct pingpong* pp, const char* fmt, ...)
{
    CURLcode result;
    va_list ap;
    va_start(ap, fmt);
 ...
 */
CURLcode Curl_pp_readresp(struct Curl_easy* data, curl_socket_t sockfd, struct pingpong* pp, int* code, /* return the server code if done */
    ...
    *size = 0;
    ...
    while ((pp->nread_resp < (size_t)data->set.buffer_size) && (keepon && !result)) {
...
#endif
            DEBUGASSERT((ptr + data->set.buffer_size - pp->nread_resp) <= (buf + data->set.buffer_size + 1));
            result = Curl_read(data, sockfd, ptr, data->set.buffer_size - pp->nread_resp, &gotbytes);
#ifdef HAVE_GSSAPI
       ...
       * line */
           ...
           * headers.
           */
                    result = Curl_client_write(data, CLIENTWRITE_INFO, pp->linestart_resp, perline);
                    if (result)
                        return result;
           ...
           it may actually contain another end of response already! */
                ...
                if ((perline == gotbytes) && (gotbytes > (ssize_t)data->set.buffer_size / 2)) {
...
int Curl_pp_getsock(struct Curl_easy* data, struct pingpong* pp, curl_socket_t* socks)
{
    struct connectdata* conn = data->conn;
...
CURLcode Curl_pp_flushsend(struct Curl_easy* data, struct pingpong* pp)
{
    ...
    ssize_t written;
    CURLcode result = Curl_nwrite(data, FIRSTSOCKET, pp->sendthis + pp->sendsize - pp->sendleft, pp->sendleft, &written);
    if (result)
        return result;
