...
#define RTP_PKT_LENGTH(p) ((((unsigned int)((unsigned char)((p)[2]))) << 8) | ((unsigned int)((unsigned char)((p)[3]))))

/* protocol-specific functions set up to be called by the main engine */
static CURLcode rtsp_do(struct Curl_easy* data, bool* done);
static CURLcode rtsp_done(struct Curl_easy* data, CURLcode, bool premature);
static CURLcode rtsp_connect(struct Curl_easy* data, bool* done);
static CURLcode rtsp_disconnect(struct Curl_easy* data, struct connectdata* conn, bool dead);
static int rtsp_getsock_do(struct Curl_easy* data, struct connectdata* conn, curl_socket_t* socks);
...
static unsigned int rtsp_conncheck(struct Curl_easy* data, struct connectdata* check, unsigned int checks_to_perform);
...
static CURLcode rtp_client_write(struct Curl_easy* data, const char* ptr, size_t len);
...
static CURLcode rtsp_done(struct Curl_easy* data, CURLcode status, bool premature)
{
    ...
    if (rtsp && !status && !httpStatus) {
        ...
        if (data->set.rtspreq == RTSPREQ_RECEIVE && (data->conn->proto.rtspc.rtp_channel == -1)) {
...
static CURLcode rtsp_do(struct Curl_easy* data, bool* done)
{
   ...
   **/
    ...
    if (rtspreq == RTSPREQ_SETUP && !p_transport) {
        ...
        if (data->set.str[STRING_RTSP_TRANSPORT]) {
            ...
            data->state.aptr.rtsp_transport = aprintf("Transport: %s\r\n", data->set.str[STRING_RTSP_TRANSPORT]);
    ...
    /* The User-Agent string might have been allocated in url.c already, because
     ...
     here. */
    if (Curl_checkheaders(data, STRCONST("User-Agent")) && data->state.aptr.uagent) {
        Curl_safefree(data->state.aptr.uagent);
    } else if (!Curl_checkheaders(data, STRCONST("User-Agent")) && data->set.str[STRING_USERAGENT]) {
        p_uagent = data->state.aptr.uagent;
    }

    /* setup the authentication headers */
    result = Curl_http_output_auth(data, conn, p_request, HTTPREQ_GET, p_stream_uri, FALSE);
    if (result)
        return result;
   ...
   */
    result = Curl_dyn_addf(&req_buffer,
        ...
        "%s" /* userpwd */
        ,
        p_transport ? p_transport : "", p_accept ? p_accept : "", p_accept_encoding ? p_accept_encoding : "", p_range ? p_range : "",
        p_referrer ? p_referrer : "", p_uagent ? p_uagent : "", p_proxyuserpwd ? p_proxyuserpwd : "", p_userpwd ? p_userpwd : "");
   ...
   */
    ...
    if (rtspreq == RTSPREQ_ANNOUNCE || rtspreq == RTSPREQ_SET_PARAMETER || rtspreq == RTSPREQ_GET_PARAMETER) {
        ...
        if (data->state.upload) {
            ...
            postsize = (data->state.infilesize != -1) ? data->state.infilesize : (data->set.postfields ? (curl_off_t)strlen(data->set.postfields) : 0);
    ...
    /* issue the request */
    result = Curl_buffer_send(&req_buffer, data, data->req.p.http, &data->info.request_size, 0, FIRSTSOCKET);
    if (result) {
...
static CURLcode rtsp_filter_rtp(struct Curl_easy* data, struct connectdata* conn, const char* buf, size_t blen, bool in_body, size_t* pconsumed)
{
    ...
    *pconsumed = 0;
    while (blen) {
        ...
        case RTP_PARSE_SKIP: {
            ...
            while (blen && buf[0] != '$') {
                if (!in_body && buf[0] == 'R' && data->set.rtspreq != RTSPREQ_RECEIVE) {
                    if (strncmp(buf, "RTSP/", (blen < 5) ? blen : 5) == 0) {
                        /* This could be the next response, no consume and return */
                        if (*pconsumed) {
                            DEBUGF(infof(data,
                                "RTP rtsp_filter_rtp[SKIP] RTSP/ prefix, "
        ...
        case RTP_PARSE_CHANNEL: {
            ...
            if (!(data->state.rtp_channel_mask[idx] & (1 << off))) {
                ...
                if (in_body) {
                    ...
                    DEBUGF(infof(data,
                        "RTSP: invalid RTP channel %d in BODY, "
                        "treating as BODY data",
                        idx));
                    if (*pconsumed == 0) {
                        /* We did not consume the initial '$' in our buffer, but had
             * it from an earlier call. We cannot un-consume it and have
             * to write it directly as BODY data */
                        result = Curl_client_write(data, CLIENTWRITE_BODY, Curl_dyn_ptr(&rtspc->buf), 1);
                        ++data->req.bytecount;
        ...
        case RTP_PARSE_DATA: {
            ...
            if (needed <= blen) {
                ...
                *pconsumed += needed;
                ...
                DEBUGF(infof(data, "RTP write channel %d rtp_len %zu", rtspc->rtp_channel, rtspc->rtp_len));
                result = rtp_client_write(data, Curl_dyn_ptr(&rtspc->buf), rtspc->rtp_len);
                Curl_dyn_free(&rtspc->buf);
                rtspc->state = RTP_PARSE_SKIP;
                if (result)
                    goto out;
...
static CURLcode rtsp_rtp_readwrite(struct Curl_easy* data, struct connectdata* conn, ssize_t* nread, bool* readmore)
{
    ...
    in_body = (data->req.headerline && !rtspc->in_header) && (data->req.size >= 0) && (data->req.bytecount < data->req.size);
...
CURLcode Curl_rtsp_parseheader(struct Curl_easy* data, char* header)
{
     ...
     */
        ...
        if (data->set.str[STRING_RTSP_SESSION_ID]) {
            ...
            if (strlen(data->set.str[STRING_RTSP_SESSION_ID]) != idlen || strncmp(start, data->set.str[STRING_RTSP_SESSION_ID], idlen) != 0) {
