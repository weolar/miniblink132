...
static void ws_dec_info(struct ws_decoder* dec, struct Curl_easy* data, const char* msg)
{
    switch (dec->head_len) {
    case 0:
        break;
    case 1:
        infof(data, "WS-DEC: %s [%s%s]", msg, ws_frame_name_of_op(dec->head[0]), (dec->head[0] & WSBIT_FIN) ? "" : " NON-FINAL");
        break;
    default:
        if (dec->head_len < dec->head_total) {
            infof(data, "WS-DEC: %s [%s%s](%d/%d)", msg, ws_frame_name_of_op(dec->head[0]), (dec->head[0] & WSBIT_FIN) ? "" : " NON-FINAL", dec->head_len,
                dec->head_total);
        } else {
            infof(data, "WS-DEC: %s [%s%s payload=%" CURL_FORMAT_CURL_OFF_T "/%" CURL_FORMAT_CURL_OFF_T "]", msg, ws_frame_name_of_op(dec->head[0]),
                (dec->head[0] & WSBIT_FIN) ? "" : " NON-FINAL", dec->payload_offset, dec->payload_len);
        }
        break;
    }
}

typedef ssize_t ws_write_payload(
    const unsigned char* buf, size_t buflen, int frame_age, int frame_flags, curl_off_t payload_offset, curl_off_t payload_len, void* userp, CURLcode* err);
...
static CURLcode ws_dec_read_head(struct ws_decoder* dec, struct Curl_easy* data, struct bufq* inraw)
{
    ...
    while (Curl_bufq_peek(inraw, &inbuf, &inlen)) {
        ...
        case 4:
            ...
            break;
        case 10:
            dec->payload_len = ((curl_off_t)dec->head[2] << 56) | (curl_off_t)dec->head[3] << 48 | (curl_off_t)dec->head[4] << 40
                | (curl_off_t)dec->head[5] << 32 | (curl_off_t)dec->head[6] << 24 | (curl_off_t)dec->head[7] << 16 | (curl_off_t)dec->head[8] << 8
                | dec->head[9];
            break;
        default:
...
static CURLcode ws_dec_pass_payload(struct ws_decoder* dec, struct Curl_easy* data, struct bufq* inraw, ws_write_payload* write_payload, void* write_ctx)
{
    const unsigned char* inbuf;
    size_t inlen;
    ssize_t nwritten;
    ...
    while (remain && Curl_bufq_peek(inraw, &inbuf, &inlen)) {
        if ((curl_off_t)inlen > remain)
            inlen = (size_t)remain;
        nwritten = write_payload(inbuf, inlen, dec->frame_age, dec->frame_flags, dec->payload_offset, dec->payload_len, write_ctx, &result);
        if (nwritten < 0)
            return result;
...
static CURLcode ws_dec_pass(struct ws_decoder* dec, struct Curl_easy* data, struct bufq* inraw, ws_write_payload* write_payload, void* write_ctx)
{
    CURLcode result;
    ...
    case WS_DEC_HEAD:
        ...
        if (dec->payload_len == 0) {
            ...
            nwritten = write_payload(&tmp, 0, dec->frame_age, dec->frame_flags, 0, 0, write_ctx, &result);
            if (nwritten < 0)
                return result;
            dec->state = WS_DEC_INIT;
            break;
...
static void update_meta(struct websocket* ws, int frame_age, int frame_flags, curl_off_t payload_offset, curl_off_t payload_len, size_t cur_len)
{
...
static void ws_enc_info(struct ws_encoder* enc, struct Curl_easy* data, const char* msg)
{
    infof(data, "WS-ENC: %s [%s%s%s payload=%" CURL_FORMAT_CURL_OFF_T "/%" CURL_FORMAT_CURL_OFF_T "]", msg, ws_frame_name_of_op(enc->firstbyte),
        (enc->firstbyte & WSBIT_OPCODE_MASK) == WSBIT_OPCODE_CONT ? " CONT" : "", (enc->firstbyte & WSBIT_FIN) ? "" : " NON-FIN",
...
static ssize_t ws_enc_write_head(struct Curl_easy* data, struct ws_encoder* enc, unsigned int flags, curl_off_t payload_len, struct bufq* out, CURLcode* err)
{
...
static ssize_t ws_enc_write_payload(struct ws_encoder* enc, struct Curl_easy* data, const unsigned char* buf, size_t buflen, struct bufq* out, CURLcode* err)
{
    ssize_t n;
    size_t i, len;
...
CURLcode Curl_ws_request(struct Curl_easy* data, REQTYPE* req)
{
    ...
    struct wsfield heads[] = { { /* The request MUST contain an |Upgrade| header field whose value
...
CURLcode Curl_ws_accept(struct Curl_easy* data, const char* mem, size_t nread)
{
    ...
    if (!ws) {
        ...
        Curl_bufq_init2(&ws->sendbuf, WS_CHUNK_SIZE, WS_CHUNK_COUNT, BUFQ_OPT_SOFT_LIMIT);
        ws_dec_init(&ws->dec);
        ws_enc_init(&ws->enc);
    } else {
  ...
  */
    ...
    result = Curl_rand(data, (unsigned char*)&ws->enc.mask, sizeof(ws->enc.mask));
    if (result)
        return result;
    infof(data, "Received 101, switch to WebSocket; mask %02x%02x%02x%02x", ws->enc.mask[0], ws->enc.mask[1], ws->enc.mask[2], ws->enc.mask[3]);
     ...
     * marked as DONE. */
        nwritten = Curl_bufq_write(&ws->recvbuf, (const unsigned char*)mem, nread, &result);
        if (nwritten < 0)
            return result;
...
static ssize_t ws_client_write(
    const unsigned char* buf, size_t buflen, int frame_age, int frame_flags, curl_off_t payload_offset, curl_off_t payload_len, void* userp, CURLcode* err)
{
     ...
     * may invoke curl_ws_meta() to access frame information. */
        update_meta(ws, frame_age, frame_flags, payload_offset, payload_len, buflen);
...
*/
size_t Curl_ws_writecb(char* buffer, size_t size /* 1 */, size_t nitems, void* userp)
{
    struct Curl_easy* data = userp;
    ...
    if (data->set.ws_raw_mode)
        ...
        if (buffer) {
            ...
            nwritten = Curl_bufq_write(&ws->recvbuf, (const unsigned char*)buffer, nitems, &result);
            if (nwritten < 0) {
...
static ssize_t ws_client_collect(
    const unsigned char* buf, size_t buflen, int frame_age, int frame_flags, curl_off_t payload_offset, curl_off_t payload_len, void* userp, CURLcode* err)
{
...
static ssize_t nw_in_recv(void* reader_ctx, unsigned char* buf, size_t buflen, CURLcode* err)
{
...
CURL_EXTERN CURLcode curl_ws_recv(struct Curl_easy* data, void* buffer, size_t buflen, size_t* nread, const struct curl_ws_frame** metap)
{
    ...
    *metap = NULL;
    ...
    while (!done) {
        ...
        if (Curl_bufq_is_empty(&ws->recvbuf)) {
            ...
            DEBUGF(infof(data, "curl_ws_recv, added %zu bytes from network", Curl_bufq_len(&ws->recvbuf)));
        }
    ...
    update_meta(ws, ctx.frame_age, ctx.frame_flags, ctx.payload_offset, ctx.payload_len, ctx.bufidx);
    *metap = &ws->frame;
    *nread = ws->frame.len;
...
static CURLcode ws_flush(struct Curl_easy* data, struct websocket* ws, bool complete)
{
    if (!Curl_bufq_is_empty(&ws->sendbuf)) {
        ...
        while (Curl_bufq_peek(&ws->sendbuf, &out, &outlen)) {
            ...
            if (result) {
                if (result == CURLE_AGAIN) {
                    if (!complete) {
                        infof(data, "WS: flush EAGAIN, %zu bytes remain in buffer", Curl_bufq_len(&ws->sendbuf));
                        return result;
                    }
...
CURL_EXTERN CURLcode curl_ws_send(CURL* data, const void* buffer, size_t buflen, size_t* sent, curl_off_t fragsize, unsigned int flags)
{
    struct websocket* ws;
    ssize_t nwritten, n;
    size_t space;
    ...
    if (data->set.ws_raw_mode) {
       ...
       the write callback */
        if (Curl_is_in_callback(data)) {
            result = Curl_write(data, data->conn->writesockfd, buffer, buflen, &nwritten);
        } else
            result = Curl_senddata(data, buffer, buflen, &nwritten);

        infof(data, "WS: wanted to send %zu bytes, sent %zu bytes", buflen, nwritten);
   ...
   * It is not clear who the application is supposed to react. */
    ...
    DEBUGF(infof(data, "curl_ws_send(len=%zu), sendbuf len=%zu space %zu", buflen, Curl_bufq_len(&ws->sendbuf), space));
    if (space < 14)
        return CURLE_AGAIN;
    ...
    if (flags & CURLWS_OFFSET) {
        if (fragsize) {
            ...
            n = ws_enc_write_head(data, &ws->enc, flags, fragsize, &ws->sendbuf, &result);
            if (n < 0)
                return result;
        } else {
            if ((curl_off_t)buflen > ws->enc.payload_remain) {
                infof(data, "WS: unaligned frame size (sending %zu instead of %" CURL_FORMAT_CURL_OFF_T ")", buflen, ws->enc.payload_remain);
            }
        }
    } else if (!ws->enc.payload_remain) {
        n = ws_enc_write_head(data, &ws->enc, flags, (curl_off_t)buflen, &ws->sendbuf, &result);
        if (n < 0)
            return result;
    }

    n = ws_enc_write_payload(&ws->enc, data, buffer, buflen, &ws->sendbuf, &result);
    if (n < 0)
        return result;
