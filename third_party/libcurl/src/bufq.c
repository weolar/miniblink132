...
static size_t chunk_append(struct buf_chunk* chunk, const unsigned char* buf, size_t len)
{
...
static size_t chunk_read(struct buf_chunk* chunk, unsigned char* buf, size_t len)
{
...
static ssize_t chunk_slurpn(struct buf_chunk* chunk, size_t max_len, Curl_bufq_reader* reader, void* reader_ctx, CURLcode* err)
{
    unsigned char* p = &chunk->x.data[chunk->w_offset];
...
static void chunk_peek(const struct buf_chunk* chunk, const unsigned char** pbuf, size_t* plen)
{
    DEBUGASSERT(chunk->w_offset >= chunk->r_offset);
...
static void chunk_peek_at(const struct buf_chunk* chunk, size_t offset, const unsigned char** pbuf, size_t* plen)
{
...
static void prune_head(struct bufq* q)
{
    ...
    while (q->head && chunk_is_empty(q->head)) {
        ...
        if (q->pool) {
            ...
            --q->chunk_count;
        } else if ((q->chunk_count > q->max_chunks) || (q->opts & BUFQ_OPT_NO_SPARES)) {
...
bool Curl_bufq_peek_at(struct bufq* q, size_t offset, const unsigned char** pbuf, size_t* plen)
{
...
ssize_t Curl_bufq_pass(struct bufq* q, Curl_bufq_writer* writer, void* writer_ctx, CURLcode* err)
{
...
ssize_t Curl_bufq_write_pass(struct bufq* q, const unsigned char* buf, size_t len, Curl_bufq_writer* writer, void* writer_ctx, CURLcode* err)
{
...
ssize_t Curl_bufq_sipn(struct bufq* q, size_t max_len, Curl_bufq_reader* reader, void* reader_ctx, CURLcode* err)
{
 ...
 */
static ssize_t bufq_slurpn(struct bufq* q, size_t max_len, Curl_bufq_reader* reader, void* reader_ctx, CURLcode* err)
{
    ssize_t nread = 0, n;
...
ssize_t Curl_bufq_slurp(struct bufq* q, Curl_bufq_reader* reader, void* reader_ctx, CURLcode* err)
{
