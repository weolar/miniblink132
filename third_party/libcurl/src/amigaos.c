...
CURLcode Curl_amiga_init(void)
{
    ...
    if (base) {
        ...
        if (ISocket) {
            ...
            SocketBaseTags(SBTM_SETVAL(SBTC_CAN_SHARE_LIBRARY_BASES), TRUE, SBTM_GETREF(SBTC_HAVE_GETHOSTADDR_R_API), (ULONG)&enabled, TAG_DONE);
...
struct Curl_addrinfo* Curl_ipv4_resolve_r(const char* hostname, int port)
{
    ...
    if (SocketFeatures & HAVE_BSDSOCKET_GETHOSTBYNAME_R) {
        ...
        if (buf) {
            h = gethostbyname_r((STRPTR)hostname, buf, (char*)buf + sizeof(struct hostent), CURL_HOSTENT_SIZE - sizeof(struct hostent), &h_errnop);
            if (h) {
