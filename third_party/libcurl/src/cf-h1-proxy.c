...
static CURLcode tunnel_reinit(struct Curl_cfilter* cf, struct Curl_easy* data, struct h1_tunnel_state* ts)
{
    (void)data;
    (void)cf;
    DEBUGASSERT(ts);
...
static CURLcode tunnel_init(struct Curl_cfilter* cf, struct Curl_easy* data, struct h1_tunnel_state** pts)
{
...
static void h1_tunnel_go_state(struct Curl_cfilter* cf, struct h1_tunnel_state* ts, h1_tunnel_state new_state, struct Curl_easy* data)
{
...
static CURLcode send_CONNECT(struct Curl_cfilter* cf, struct Curl_easy* data, struct h1_tunnel_state* ts, bool* done)
{
...
static CURLcode on_resp_header(struct Curl_cfilter* cf, struct Curl_easy* data, struct h1_tunnel_state* ts, const char* header)
{
    CURLcode result = CURLE_OK;
    struct SingleRequest* k = &data->req;
    (void)cf;

    if ((checkprefix("WWW-Authenticate:", header) && (401 == k->httpcode)) || (checkprefix("Proxy-authenticate:", header) && (407 == k->httpcode))) {
    ...
    } else if (Curl_compareheader(header, STRCONST("Connection:"), STRCONST("close")))
        ...
        if (k->httpcode / 100 == 2) {
         ...
         "Successful" described as: 2xx (Successful). RFC 7231 4.3.6 */
            infof(data,
                ...
                k->httpcode);
        } else if (Curl_compareheader(header, STRCONST("Transfer-Encoding:"), STRCONST("chunked"))) {
        ...
        ts->close_connection = TRUE;
    else if (!strncmp(header, "HTTP/1.", 7) && ((header[7] == '0') || (header[7] == '1')) && (header[8] == ' ') && ISDIGIT(header[9]) && ISDIGIT(header[10])
        && ISDIGIT(header[11]) && !ISDIGIT(header[12])) {
       ...
       as end-of-headers as well.*/
        ...
        if (('\r' == linep[0]) || ('\n' == linep[0])) {
           ...
           whole response-body */
                ...
                if (ts->cl) {
                    infof(data, "Ignore %" CURL_FORMAT_CURL_OFF_T " bytes of response-body", ts->cl);
...
static CURLcode CONNECT_host(struct Curl_cfilter* cf, struct Curl_easy* data, char** pauthority, char** phost_header)
{
    const char* hostname;
    int port;
    bool ipv6_ip;
    ...
    authority = aprintf("%s%s%s:%d", ipv6_ip ? "[" : "", hostname, ipv6_ip ? "]" : "", port);
    if (!authority)
        return CURLE_OUT_OF_MEMORY;
...
static CURLcode start_CONNECT(struct Curl_cfilter* cf, struct Curl_easy* data, struct h1_tunnel_state* ts)
{
    ...
    result = Curl_http_output_auth(data, conn, "CONNECT", HTTPREQ_GET, authority, TRUE);
    if (result)
        goto error;
    Curl_safefree(authority);

    /* default is 1.1 */
    if ((conn->http_proxy.proxytype == CURLPROXY_HTTP_1_0) && (HYPERE_OK != hyper_request_set_version(req, HYPER_HTTP_VERSION_1_0))) {
    ...
    if (!Curl_checkProxyheaders(data, conn, STRCONST("User-Agent")) && data->set.str[STRING_USERAGENT]) {
        struct dynbuf ua;
...
static CURLcode H1_CONNECT(struct Curl_cfilter* cf, struct Curl_easy* data, struct h1_tunnel_state* ts)
{
    ...
    infof(data, "CONNECT tunnel established, response %d", data->info.httpproxycode);
    result = CURLE_OK;
...
static CURLcode cf_h1_proxy_connect(struct Curl_cfilter* cf, struct Curl_easy* data, bool blocking, bool* done)
{
    CURLcode result;
    struct h1_tunnel_state* ts = cf->ctx;
...
static void cf_h1_proxy_adjust_pollset(struct Curl_cfilter* cf, struct Curl_easy* data, struct easy_pollset* ps)
{
    struct h1_tunnel_state* ts = cf->ctx;
...
static void cf_h1_proxy_destroy(struct Curl_cfilter* cf, struct Curl_easy* data)
{
