...
/* SMB request state */
enum smb_req_state { SMB_REQUESTING, SMB_TREE_CONNECT, SMB_OPEN, SMB_DOWNLOAD, SMB_UPLOAD, SMB_CLOSE, SMB_TREE_DISCONNECT, SMB_DONE };
...
static void conn_state(struct Curl_easy* data, enum smb_conn_state newstate)
{
    ...
    if (smbc->state != newstate)
        infof(data, "SMB conn %p state change from %s to %s", (void*)smbc, names[smbc->state], names[newstate]);
#endif
...
static void request_state(struct Curl_easy* data, enum smb_req_state newstate)
{
    ...
    /* For debug purposes */
    static const char* const names[] = {
        "SMB_REQUESTING", "SMB_TREE_CONNECT", "SMB_OPEN", "SMB_DOWNLOAD", "SMB_UPLOAD", "SMB_CLOSE", "SMB_TREE_DISCONNECT", "SMB_DONE",
        /* LAST */
    };

    if (req->state != newstate)
        infof(data, "SMB request %p state change from %s to %s", (void*)req, names[req->state], names[newstate]);
#endif
...
static CURLcode smb_recv_message(struct Curl_easy* data, void** msg)
{
    ...
    nbt_size = Curl_read16_be((const unsigned char*)(buf + sizeof(unsigned short))) + sizeof(unsigned int);
    ...
    if (nbt_size >= msg_size + 1) {
        ...
        if (nbt_size >= msg_size + sizeof(unsigned short)) {
            /* Add the byte count */
            msg_size += sizeof(unsigned short) + Curl_read16_le((const unsigned char*)&buf[msg_size]);
...
static void smb_format_message(struct Curl_easy* data, struct smb_header* h, unsigned char cmd, size_t len)
{
    struct connectdata* conn = data->conn;
    struct smb_conn* smbc = &conn->proto.smbc;
    struct smb_request* req = data->req.p.smb;
...
static CURLcode smb_send(struct Curl_easy* data, ssize_t len, size_t upload_size)
{
    struct connectdata* conn = data->conn;
    struct smb_conn* smbc = &conn->proto.smbc;
    ...
    result = Curl_nwrite(data, FIRSTSOCKET, data->state.ulbuf, len, &bytes_written);
    if (result)
        return result;
...
static CURLcode smb_flush(struct Curl_easy* data)
{
    ...
    result = Curl_nwrite(data, FIRSTSOCKET, data->state.ulbuf + smbc->sent, len, &bytes_written);
    if (result)
        return result;
...
static CURLcode smb_send_message(struct Curl_easy* data, unsigned char cmd, const void* msg, size_t msg_len)
{
    CURLcode result = Curl_get_upload_buffer(data);
    if (result)
        return result;
    smb_format_message(data, (struct smb_header*)data->state.ulbuf, cmd, msg_len);
...
static CURLcode smb_send_setup(struct Curl_easy* data)
{
    ...
    return smb_send_message(data, SMB_COM_SETUP_ANDX, &msg, sizeof(msg) - sizeof(msg.bytes) + byte_count);
}
...
static CURLcode smb_send_tree_connect(struct Curl_easy* data)
{
    ...
    return smb_send_message(data, SMB_COM_TREE_CONNECT_ANDX, &msg, sizeof(msg) - sizeof(msg.bytes) + byte_count);
}
...
static CURLcode smb_send_open(struct Curl_easy* data)
{
    ...
    return smb_send_message(data, SMB_COM_NT_CREATE_ANDX, &msg, sizeof(msg) - sizeof(msg.bytes) + byte_count);
}
...
static CURLcode smb_send_write(struct Curl_easy* data)
{
    ...
    smb_format_message(data, &msg->h, SMB_COM_WRITE_ANDX, sizeof(*msg) - sizeof(msg->h) + (size_t)upload_size);
...
static CURLcode smb_send_and_recv(struct Curl_easy* data, void** msg)
{
    ...
    *msg = NULL; /* if it returns early */
    ...
    if (!smbc->send_size && smbc->upload_size) {
        size_t nread = smbc->upload_size > (size_t)data->set.upload_buffer_size ? (size_t)data->set.upload_buffer_size : smbc->upload_size;
    ...
    case SMB_DOWNLOAD:
        ...
        if (len > 0) {
            if (off + sizeof(unsigned int) + len > smbc->got) {
                ...
                result = Curl_client_write(data, CLIENTWRITE_BODY, (char*)msg + off + sizeof(unsigned int), len);
            if (result) {
                req->result = result;
                next_state = SMB_CLOSE;
                break;
...
static CURLcode smb_parse_url_path(struct Curl_easy* data, struct connectdata* conn)
{
    ...
    /* URL decode the path */
    CURLcode result = Curl_urldecode(data->state.up.path, 0, &path, NULL, REJECT_CTRL);
    if (result)
        return result;
