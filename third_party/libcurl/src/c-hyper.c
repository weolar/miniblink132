...
size_t Curl_hyper_recv(void* userp, hyper_context* ctx, uint8_t* buf, size_t buflen)
{
...
size_t Curl_hyper_send(void* userp, hyper_context* ctx, const uint8_t* buf, size_t buflen)
{
    struct Curl_easy* data = userp;
    struct connectdata* conn = data->conn;
    CURLcode result;
...
static int hyper_each_header(void* userdata, const uint8_t* name, size_t name_len, const uint8_t* value, size_t value_len)
{
...
static int hyper_body_chunk(void* userdata, const hyper_buf* chunk)
{
    ...
    if (0 == k->bodywrites++) {
        ...
        if (data->state.hconnect && (data->req.httpcode / 100 != 2) && data->state.authproxy.done) {
            done = TRUE;
            result = CURLE_OK;
        } else
...
static CURLcode status_line(struct Curl_easy* data, struct connectdata* conn, uint16_t http_status, int http_version, const uint8_t* reason, size_t rlen)
{
    CURLcode result;
    size_t len;
    const char* vstr;
    int writetype;
    vstr = http_version == HYPER_HTTP_VERSION_1_1 ? "1.1" : (http_version == HYPER_HTTP_VERSION_2 ? "2" : "1.0");
    ...
    if (data->state.hconnect)
        ...
        conn->httpversion = http_version == HYPER_HTTP_VERSION_1_1 ? 11 : (http_version == HYPER_HTTP_VERSION_2 ? 20 : 10);
    ...
    if (!data->state.hconnect || !data->set.suppress_connect_headers) {
        ...
        result = Curl_client_write(data, writetype, Curl_dyn_ptr(&data->state.headerb), len);
        if (result)
            return result;
    }
    result = Curl_bump_headersize(data, len, FALSE);
...
static CURLcode empty_header(struct Curl_easy* data)
{
    ...
    if (!result) {
        result = hyper_each_header(data, NULL, 0, NULL, 0) ? CURLE_WRITE_ERROR : CURLE_OK;
        if (result)
...
CURLcode Curl_hyper_stream(struct Curl_easy* data, struct connectdata* conn, int* didwhat, bool* done, int select_res)
{
    ...
    *done = FALSE;
    do {
        ...
        *didwhat = KEEP_RECV;
        ...
        result = status_line(data, conn, http_status, http_version, reasonp, reason_len);
        if (result)
            break;
        ...
        k->deductheadercount = (100 <= http_status && 199 >= http_status) ? k->headerbytecount : 0;
...
static CURLcode debug_request(struct Curl_easy* data, const char* method, const char* path, bool h2)
{
...
CURLcode Curl_hyper_header(struct Curl_easy* data, hyper_headers* headers, const char* line)
{
    const char* p;
    const char* n;
    size_t nlen;
    ...
    do {
        ...
        if (HYPERE_OK != hyper_headers_add(headers, (uint8_t*)n, nlen, (uint8_t*)v, vlen)) {
...
static CURLcode request_target(struct Curl_easy* data, struct connectdata* conn, const char* method, bool h2, hyper_request* req)
{
    CURLcode result;
    struct dynbuf r;
    ...
    if (h2
        && hyper_request_set_uri_parts(req,
            /* scheme */
        ...
        result = CURLE_OUT_OF_MEMORY;
    } else if (!h2 && hyper_request_set_uri(req, (uint8_t*)Curl_dyn_uptr(&r), Curl_dyn_len(&r))) {
...
static int uploadpostfields(void* userdata, hyper_context* ctx, hyper_buf** chunk)
{
    struct Curl_easy* data = (struct Curl_easy*)userdata;
    (void)ctx;
    ...
    if (data->req.upload_done)
        *chunk = NULL; /* nothing more to deliver */
        ...
        hyper_buf* copy = hyper_buf_copy(data->set.postfields, (size_t)data->req.p.http->postsize);
        if (copy)
            *chunk = copy;
        else {
            data->state.hresult = CURLE_OUT_OF_MEMORY;
...
static int uploadstreamed(void* userdata, hyper_context* ctx, hyper_buf** chunk)
{
    size_t fillcount;
    struct Curl_easy* data = (struct Curl_easy*)userdata;
...
static CURLcode bodysend(struct Curl_easy* data, struct connectdata* conn, hyper_headers* headers, hyper_request* hyperreq, Curl_HttpReq httpreq)
{
...
static CURLcode cookies(struct Curl_easy* data, struct connectdata* conn, hyper_headers* headers)
{
    struct dynbuf req;
    CURLcode result;
    Curl_dyn_init(&req, DYN_HTTP_REQUEST);
...
static void http1xx_cb(void* arg, struct hyper_response* resp)
{
    ...
    result = status_line(data, data->conn, http_status, http_version, reasonp, reason_len);
    if (!result) {
        headers = hyper_response_headers(resp);
        if (!headers) {
...
CURLcode Curl_http(struct Curl_easy* data, bool* done)
{
    ...
    *done = TRUE;
    ...
    /* setup the authentication headers */
    {
        ...
        result = Curl_http_output_auth(data, conn, method, httpreq, (pq ? pq : data->state.up.path), FALSE);
        free(pq);
        if (result)
            return result;
    }
    ...
    if (data->set.str[STRING_USERAGENT] && *data->set.str[STRING_USERAGENT] && data->state.aptr.uagent) {
    ...
    if (conn->bits.httpproxy && !conn->bits.tunnel_proxy && !Curl_checkheaders(data, STRCONST("Proxy-Connection"))
    ...
    if (!Curl_checkheaders(data, STRCONST("Accept-Encoding")) && data->set.str[STRING_ENCODING]) {
        ...
        if (!data->state.aptr.accept_encoding)
            ...
            result = Curl_hyper_header(data, headers, data->state.aptr.accept_encoding);
        if (result)
            goto error;
    } else
        Curl_safefree(data->state.aptr.accept_encoding);
