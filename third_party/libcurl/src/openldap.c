...
static CURLcode oldap_setup_connection(struct Curl_easy* data, struct connectdata* conn);
...
static CURLcode oldap_disconnect(struct Curl_easy* data, struct connectdata* conn, bool dead);

static CURLcode oldap_perform_auth(struct Curl_easy* data, const char* mech, const struct bufref* initresp);
static CURLcode oldap_continue_auth(struct Curl_easy* data, const char* mech, const struct bufref* resp);
...
static void oldap_state(struct Curl_easy* data, ldapstate newstate)
{
    ...
    if (ldapc->state != newstate)
        infof(data, "LDAP %p state change from %s to %s", (void*)ldapc, names[ldapc->state], names[newstate]);
#endif
...
static CURLcode oldap_url_parse(struct Curl_easy* data, LDAPURLDesc** ludp)
{
    ...
    static const char* const url_errs[] = { "success", "out of memory", "bad parameter", "unrecognized scheme", "unbalanced delimiter", "bad URL",
        "bad host or port", "bad or missing attributes", "bad or missing scope", "bad or missing filter", "bad or missing extensions" };

    *ludp = NULL;
    if (!data->state.up.user && !data->state.up.password && !data->state.up.options)
...
static CURLcode oldap_cancel_auth(struct Curl_easy* data, const char* mech)
{
    ...
    CURLcode result = CURLE_OK;
    int rc = ldap_sasl_bind(li->ld, NULL, LDAP_SASL_NULL, NULL, NULL, NULL, &li->msgid);
...
static CURLcode oldap_perform_bind(struct Curl_easy* data, ldapstate newstate)
{
    ...
    if (rc == LDAP_SUCCESS)
        ...
        result = oldap_map_error(rc, data->state.aptr.user ? CURLE_LOGIN_DENIED : CURLE_LDAP_CANNOT_BIND);
    return result;
}
...
static CURLcode oldap_perform_mechs(struct Curl_easy* data)
{
    ...
    struct ldapconninfo* li = data->conn->proto.ldapc;
    int rc;
    static const char* const supportedSASLMechanisms[] = { "supportedSASLMechanisms", NULL };

    rc = ldap_search_ext(li->ld, "", LDAP_SCOPE_BASE, "(objectclass=*)", (char**)supportedSASLMechanisms, 0, NULL, NULL, NULL, 0, &li->msgid);
    if (rc == LDAP_SUCCESS)
...
static CURLcode oldap_connect(struct Curl_easy* data, bool* done)
{
    ...
    hosturl = aprintf("ldap%s://%s:%d", conn->handler->flags & PROTOPT_SSL ? "s" : "", conn->host.name, conn->remote_port);
    if (!hosturl)
        return CURLE_OUT_OF_MEMORY;
...
static CURLcode oldap_state_mechs_resp(struct Curl_easy* data, LDAPMessage* msg, int code)
{
    struct connectdata* conn = data->conn;
    struct ldapconninfo* li = conn->proto.ldapc;
    int rc;
    ...
    case LDAP_RES_SEARCH_ENTRY:
        ...
        for (rc = ldap_get_attribute_ber(li->ld, msg, ber, &bv, &bvals); rc == LDAP_SUCCESS; rc = ldap_get_attribute_ber(li->ld, msg, ber, &bv, &bvals)) {
            int i;
            ...
            if (bvals) {
                for (i = 0; bvals[i].bv_val; i++) {
                    size_t llen;
                    unsigned short mech = Curl_sasl_decode_mech((char*)bvals[i].bv_val, bvals[i].bv_len, &llen);
...
static CURLcode oldap_state_bind_resp(struct Curl_easy* data, LDAPMessage* msg, int code)
{
    struct connectdata* conn = data->conn;
    struct ldapconninfo* li = conn->proto.ldapc;
    CURLcode result = CURLE_OK;
...
static CURLcode oldap_connecting(struct Curl_easy* data, bool* done)
{
    ...
    if (li->state != OLDAP_SSL && li->state != OLDAP_TLS) {
        ...
        default:
            ...
            if (rc < 0) {
                failf(data, "LDAP local: connecting ldap_result %s", ldap_err2string(rc));
...
static CURLcode oldap_do(struct Curl_easy* data, bool* done)
{
    ...
    if (!result) {
        ...
        rc = ldap_search_ext(li->ld, lud->lud_dn, lud->lud_scope, lud->lud_filter, lud->lud_attrs, 0, NULL, NULL, NULL, 0, &msgid);
        ldap_free_urldesc(lud);
        if (rc != LDAP_SUCCESS) {
...
static CURLcode client_write(struct Curl_easy* data, const char* prefix, size_t plen, const char* value, size_t len, const char* suffix, size_t slen)
{
    CURLcode result = CURLE_OK;
...
static ssize_t oldap_recv(struct Curl_easy* data, int sockindex, char* buf, size_t len, CURLcode* err)
{
    struct connectdata* conn = data->conn;
    struct ldapconninfo* li = conn->proto.ldapc;
    struct ldapreqinfo* lr = data->req.p.ldap;
    ...
    *err = result;
    ...
    case LDAP_RES_SEARCH_RESULT:
        ...
        case LDAP_SUCCESS:
            ...
            break;
        default:
            failf(data, "LDAP remote: search failed %s %s", ldap_err2string(code), info ? info : "");
            result = CURLE_LDAP_SEARCH_FAILED;
            break;
        }
        if (info)
    ...
    case LDAP_RES_SEARCH_ENTRY:
        ...
        for (rc = ldap_get_attribute_ber(li->ld, msg, ber, &bv, &bvals); rc == LDAP_SUCCESS; rc = ldap_get_attribute_ber(li->ld, msg, ber, &bv, &bvals)) {
            ...
            binary = bv.bv_len > 7 && !strncmp(bv.bv_val + bv.bv_len - 7, ";binary", 7);
            ...
            for (i = 0; bvals[i].bv_val != NULL; i++) {
                ...
                if (!binary) {
                    ...
                    if (ISBLANK(bvals[i].bv_val[0]) || ISBLANK(bvals[i].bv_val[bvals[i].bv_len - 1]))
                        binval = 1;
                    else {
                ...
                if (binary || binval) {
                    ...
                    if (bvals[i].bv_len)
                        result = Curl_base64_encode(bvals[i].bv_val, bvals[i].bv_len, &val_b64, &val_b64_sz);
                    if (!result)
...
static Sockbuf_IO ldapsb_tls = { ldapsb_tls_setup, ldapsb_tls_remove, ldapsb_tls_ctrl, ldapsb_tls_read, ldapsb_tls_write, ldapsb_tls_close };
#endif /* USE_SSL */
