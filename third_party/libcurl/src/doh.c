...
static const char* const errors[] = { "", "Bad label", "Out of range", "Label loop", "Too small", "Out of memory", "RDATA length", "Malformat", "Bad RCODE",
    "Unexpected TYPE", "Unexpected CLASS", "No content", "Bad ID", "Name too long" };
...
/* @unittest 1655
 */
UNITTEST DOHcode doh_encode(const char* host, DNStype dnstype, unsigned char* dnsp, /* buffer */
...
#define ERROR_CHECK_SETOPT(x, y)                                                                                                                               \
    do {                                                                                                                                                       \
        ...
        if (result && result != CURLE_NOT_BUILT_IN && result != CURLE_UNKNOWN_OPTION)                                                                          \
            goto error;                                                                                                                                        \
    } while (0)

static CURLcode dohprobe(
    struct Curl_easy* data, struct dnsprobe* p, DNStype dnstype, const char* host, const char* url, CURLM* multi, struct curl_slist* headers)
{
    ...
    if (!result) {
       ...
       */
        ...
        {
            long mask = (data->set.ssl.enable_beast ? CURLSSLOPT_ALLOW_BEAST : 0) | (data->set.ssl.no_revoke ? CURLSSLOPT_NO_REVOKE : 0)
                | (data->set.ssl.no_partialchain ? CURLSSLOPT_NO_PARTIALCHAIN : 0) | (data->set.ssl.revoke_best_effort ? CURLSSLOPT_REVOKE_BEST_EFFORT : 0)
                | (data->set.ssl.native_ca_store ? CURLSSLOPT_NATIVE_CA : 0) | (data->set.ssl.auto_client_cert ? CURLSSLOPT_AUTO_CLIENT_CERT : 0);
...
struct Curl_addrinfo* Curl_doh(struct Curl_easy* data, const char* hostname, int port, int* waitp)
{
    ...
    *waitp = FALSE;
    ...
    result = dohprobe(data, &dohp->probe[DOH_PROBE_SLOT_IPADDR_V4], DNS_TYPE_A, hostname, data->set.str[STRING_DOH], data->multi, dohp->headers);
    if (result)
        goto error;
    dohp->pending++;
    ...
    if ((conn->ip_version != CURL_IPRESOLVE_V4) && Curl_ipv6works(data)) {
        ...
        result = dohprobe(data, &dohp->probe[DOH_PROBE_SLOT_IPADDR_V6], DNS_TYPE_AAAA, hostname, data->set.str[STRING_DOH], data->multi, dohp->headers);
        if (result)
            goto error;
        dohp->pending++;
    }
...
static DOHcode store_cname(const unsigned char* doh, size_t dohlen, unsigned int index, struct dohentry* d)
{
    struct dynbuf* c;
...
static DOHcode rdata(const unsigned char* doh, size_t dohlen, unsigned short rdlength, unsigned short type, int index, struct dohentry* d)
{
    /* RDATA
     - A (TYPE 1):  4 bytes
...
UNITTEST DOHcode doh_decode(const unsigned char* doh, size_t dohlen, DNStype dnstype, struct dohentry* d)
{
    unsigned char rcode;
...
static void showdoh(struct Curl_easy* data, const struct dohentry* d)
{
    ...
    for (i = 0; i < d->numaddr; i++) {
        ...
        if (a->type == DNS_TYPE_A) {
            infof(data, "DoH A: %u.%u.%u.%u", a->ip.v4[0], a->ip.v4[1], a->ip.v4[2], a->ip.v4[3]);
            ...
            for (j = 0; j < 16; j += 2) {
                size_t l;
                msnprintf(ptr, len, "%s%02x%02x", j ? ":" : "", d->addr[i].ip.v6[j], d->addr[i].ip.v6[j + 1]);
                l = strlen(ptr);
                len -= l;
                ptr += l;
            }
...
CURLcode Curl_doh_is_resolved(struct Curl_easy* data, struct Curl_dns_entry** dnsp)
{
    struct connectdata* conn = data->conn;
    CURLcode result;
    struct dohdata* dohp = data->req.doh;
    *dnsp = NULL; /* defaults to no response */
    ...
    if (!dohp->probe[DOH_PROBE_SLOT_IPADDR_V4].easy && !dohp->probe[DOH_PROBE_SLOT_IPADDR_V6].easy) {
        ...
        for (slot = 0; slot < DOH_PROBE_SLOTS; slot++) {
            ...
            if (!p->dnstype)
                continue;
            rc[slot] = doh_decode(Curl_dyn_uptr(&p->serverdoh), Curl_dyn_len(&p->serverdoh), p->dnstype, &de);
            Curl_dyn_free(&p->serverdoh);
#ifndef CURL_DISABLE_VERBOSE_STRINGS
            if (rc[slot]) {
                infof(data, "DoH: %s type %s for %s", doh_strerror(rc[slot]), type2name(p->dnstype), dohp->host);
            }
#endif
        } /* next slot */
