...
#if !defined(CURL_DISABLE_SMB) && defined(USE_CURL_NTLM_CORE) && (SIZEOF_CURL_OFF_T > 4)
    &Curl_handler_smb,
#ifdef USE_SSL
    &Curl_handler_smbs,
#endif
...
#ifdef USE_LIBRTMP
    &Curl_handler_rtmp, &Curl_handler_rtmpt, &Curl_handler_rtmpe, &Curl_handler_rtmpte, &Curl_handler_rtmps, &Curl_handler_rtmpts,
#endif
...
void Curl_disconnect(struct Curl_easy* data, struct connectdata* conn, bool dead_connection)
{
    ...
    DEBUGF(infof(data, "Curl_disconnect(conn #%" CURL_FORMAT_CURL_OFF_T ", dead=%d)", conn->connection_id, dead_connection));
    /*
...
{
    if ((data->proxytype == needle->proxytype) && (data->port == needle->port) && strcasecompare(data->host.name, needle->host.name))
        return TRUE;
...
static bool socks_proxy_info_matches(const struct proxy_info* data, const struct proxy_info* needle)
{
    ...
    if (Curl_timestrcmp(data->user, needle->user) || Curl_timestrcmp(data->passwd, needle->passwd))
        return FALSE;
    return TRUE;
}
#else
...
static bool conn_maxage(struct Curl_easy* data, struct connectdata* conn, struct curltime now)
{
    ...
    if (idletime > data->set.maxage_conn) {
        infof(data, "Too old connection (%" CURL_FORMAT_TIMEDIFF_T " seconds idle), disconnect it", idletime);
        return TRUE;
    }
    ...
    if (data->set.maxlifetime_conn && lifetime > data->set.maxlifetime_conn) {
        infof(data, "Too old connection (%" CURL_FORMAT_TIMEDIFF_T " seconds since creation), disconnect it", lifetime);
        return TRUE;
    }
...
static bool extract_if_dead(struct connectdata* conn, struct Curl_easy* data)
{
    if (!CONN_INUSE(conn)) {
       ...
       use */
        ...
        if (dead) {
            infof(data, "Connection %" CURL_FORMAT_CURL_OFF_T " seems to be dead", conn->connection_id);
...
{
    return (Curl_safecmp(one->proto.sshc.rsa, two->proto.sshc.rsa) && Curl_safecmp(one->proto.sshc.rsa_pub, two->proto.sshc.rsa_pub));
}
#else
...
static bool ConnectionExists(struct Curl_easy* data, struct connectdata* needle, struct connectdata** usethis, bool* force_reuse, bool* waitpipe)
{
    ...
    bool wantProxyNTLMhttp = (needle->bits.proxy_user_passwd
        && ((data->state.authproxy.want & (CURLAUTH_NTLM | CURLAUTH_NTLM_WB)) && (needle->handler->protocol & PROTO_FAMILY_HTTP)));
#else
    bool wantProxyNTLMhttp = FALSE;
#endif
#endif
    /* plain HTTP with upgrade */
    bool h2upgrade = (data->state.httpwant == CURL_HTTP_VERSION_2_0) && (needle->handler->protocol & CURLPROTO_HTTP);
    ...
    if (bundle) {
       ...
       */
            ...
            if (!Curl_conn_is_connected(check, FIRSTSOCKET)) {
                ...
                infof(data, "Connection #%" CURL_FORMAT_CURL_OFF_T " isn't open enough, can't reuse", check->connection_id);
                continue;
            }
        ...
        */
                if ((check->localport != needle->localport) || (check->localportrange != needle->localportrange)
                    || (needle->localdev && (!check->localdev || strcmp(check->localdev, needle->localdev))))
                    continue;
            }
           ...
           so verify that we're using the same name and password as well */
                if (Curl_timestrcmp(needle->user, check->user) || Curl_timestrcmp(needle->passwd, check->passwd)
                    || Curl_timestrcmp(needle->sasl_authzid, check->sasl_authzid) || Curl_timestrcmp(needle->oauth_bearer, check->oauth_bearer)) {
         ...
         explicitly requested, handle it: */
            if ((needle->handler->protocol & PROTO_FAMILY_HTTP)
                && (((check->httpversion >= 20) && (data->state.httpwant < CURL_HTTP_VERSION_2_0))
                    || ((check->httpversion >= 30) && (data->state.httpwant < CURL_HTTP_VERSION_3))))
                continue;
#ifdef USE_SSH
                ...
                if (Curl_timestrcmp(needle->proto.ftpc.account, check->proto.ftpc.account)
                    ...
                    || (needle->proto.ftpc.use_ssl != check->proto.ftpc.use_ssl) || (needle->proto.ftpc.ccc != check->proto.ftpc.ccc))
                    continue;
            }
#endif
           ...
           which is allowed to be upgraded via TLS */
                ...
                if ((strcasecompare(needle->handler->scheme, check->handler->scheme)
                        || (get_protocol_family(check->handler) == needle->handler->protocol && check->bits.tls_upgraded))
                    && (!needle->bits.conn_to_host || strcasecompare(needle->conn_to_host.name, check->conn_to_host.name))
                    && (!needle->bits.conn_to_port || needle->conn_to_port == check->conn_to_port) && strcasecompare(needle->host.name, check->host.name)
           ...
           * NULL */
                    ...
                    if (Curl_timestrcmp(needle->http_proxy.user, check->http_proxy.user)
             ...
             the optimal connection to use. */
                    ...
                    if (check->bits.multiplex) {
                        if (multiplexed >= Curl_conn_get_max_concurrent(data, check, FIRSTSOCKET)) {
...
{
    if (data->set.verbose)
        infof(data, "Connected to %s (%s) port %u", CURL_CONN_HOST_DISPNAME(conn), conn->primary_ip, conn->port);
}
#endif
...
static struct connectdata* allocate_conn(struct Curl_easy* data)
{
    ...
    conn->bits.httpproxy = (conn->bits.proxy
                               && (conn->http_proxy.proxytype == CURLPROXY_HTTP || conn->http_proxy.proxytype == CURLPROXY_HTTP_1_0
...
const struct Curl_handler* Curl_builtin_scheme(const char* scheme, size_t schemelen)
{
...
{
    const struct Curl_handler* p = Curl_builtin_scheme(protostr, CURL_ZERO_TERMINATED);
...
static CURLcode parseurlandfillconn(struct Curl_easy* data, struct connectdata* conn)
{
    ...
    if (!use_set_uh) {
        ...
        uc = curl_url_set(uh, CURLUPART_URL, data->state.url,
            CURLU_GUESS_SCHEME | CURLU_NON_SUPPORT_SCHEME | (data->set.disallow_username_in_url ? CURLU_DISALLOW_USER : 0)
...
static CURLcode create_conn_helper_init_proxy(struct Curl_easy* data, struct connectdata* conn)
{
   ...
   *************************************************************/
    ...
    if (Curl_check_noproxy(conn->host.name, data->set.str[STRING_NOPROXY] ? data->set.str[STRING_NOPROXY] : no_proxy, &spacesep)) {
        Curl_safefree(proxy);
...
static CURLcode override_login(struct Curl_easy* data, struct connectdata* conn)
{
    ...
    if (data->set.use_netrc && !data->set.str[STRING_USERNAME]) {
        ...
        ret = Curl_parsenetrc(conn->host.name, userp, passwdp, data->set.str[STRING_NETRC_FILE]);
        if (ret > 0) {
            infof(data, "Couldn't find host %s in the %s file; using defaults", conn->host.name,
...
static CURLcode parse_connect_to_slist(struct Curl_easy* data, struct connectdata* conn, struct curl_slist* conn_to_host)
{
    ...
    if (data->asi && !host && (port == -1)
        ...
        srcalpnid = ALPN_h2;
        hit = Curl_altsvc_lookup(data->asi, srcalpnid, host, conn->remote_port, /* from */
        ...
        if (hit) {
            ...
            infof(data, "Alt-svc connecting from [%s]%s:%d to [%s]%s:%d", Curl_alpnid2str(srcalpnid), host, conn->remote_port, Curl_alpnid2str(as->dst.alpnid),
                hostd, as->dst.port);
...
#ifdef USE_UNIX_SOCKETS
static CURLcode resolve_unix(struct Curl_easy* data, struct connectdata* conn, char* unix_path)
{
   ...
   * no DNS involved and we already have the filesystem path available. */
    ...
    hostaddr->addr = Curl_unix2addr(unix_path, &longpath, conn->bits.abstract_unix_socket);
    if (!hostaddr->addr) {
        if (longpath)
...
#ifndef CURL_DISABLE_PROXY
static CURLcode resolve_proxy(struct Curl_easy* data, struct connectdata* conn, bool* async)
{
    struct Curl_dns_entry* hostaddr = NULL;
    ...
    host = conn->bits.socksproxy ? &conn->socks_proxy.host : &conn->http_proxy.host;
...
static CURLcode resolve_host(struct Curl_easy* data, struct connectdata* conn, bool* async)
{
    struct Curl_dns_entry* hostaddr = NULL;
   ...
   * there, thus overriding any defaults that might have been set above. */
    ...
    if (rc == CURLRESOLV_PENDING)
        *async = TRUE;
        ...
        failf(data, "Failed to resolve host '%s' with timeout after %" CURL_FORMAT_TIMEDIFF_T " ms", connhost->dispname,
...
static CURLcode resolve_fresh(struct Curl_easy* data, struct connectdata* conn, bool* async)
{
#ifdef USE_UNIX_SOCKETS
    char* unix_path = conn->unix_domain_socket;

#ifndef CURL_DISABLE_PROXY
    if (!unix_path && conn->socks_proxy.host.name && !strncmp(UNIX_SOCKET_PREFIX "/", conn->socks_proxy.host.name, sizeof(UNIX_SOCKET_PREFIX)))
 ...
 * and `temp` is freed.
 */
static void reuse_conn(struct Curl_easy* data, struct connectdata* temp, struct connectdata* existing)
{
...
static CURLcode create_conn(struct Curl_easy* data, struct connectdata** in_connect, bool* async)
{
  ...
  */
   ...
   *************************************************************/
    ...
    if ((data->set.reuse_fresh && !data->state.followlocation) || data->set.connect_only)
        reuse = FALSE;
    else
...
#ifndef CURL_DISABLE_PROXY
        infof(data, "Re-using existing connection with %s %s", conn->bits.proxy ? "proxy" : "host",
            conn->socks_proxy.host.name ? conn->socks_proxy.host.dispname : conn->http_proxy.host.name ? conn->http_proxy.host.dispname : conn->host.dispname);
#else
        infof(data, "Re-using existing connection with host %s", conn->host.dispname);
#endif
    } else {
...
CURLcode Curl_setup_conn(struct Curl_easy* data, bool* protocol_done)
{
    ...
    conn->now = Curl_now();
    if (!conn->bits.reuse)
        result = Curl_conn_setup(data, conn, FIRSTSOCKET, conn->dns_entry, CURL_CF_SSL_DEFAULT);
...
static void priority_remove_child(struct Curl_easy* parent, struct Curl_easy* child)
{
