...
static CURLcode tftp_setup_connection(struct Curl_easy* data, struct connectdata* conn);
...
static int tftp_getsock(struct Curl_easy* data, struct connectdata* conn, curl_socket_t* socks);
...
static CURLcode tftp_set_timeouts(struct tftp_state_data* state)
{
    ...
    infof(state->data, "set timeouts for state %d; Total % " CURL_FORMAT_CURL_OFF_T ", retry %d maxtry %d", (int)state->state, timeout_ms, state->retry_time,
        state->retry_max);
...
static CURLcode tftp_parse_option_ack(struct tftp_state_data* state, const char* ptr, int len)
{
    ...
    while (tmp < ptr + len) {
         ...
         * requests */
            ...
            infof(data, "%s (%d) %s (%d)", "blksize parsed from OACK", state->blksize, "requested", state->requested_blksize);
...
static CURLcode tftp_option_add(struct tftp_state_data* state, size_t* csize, char* buf, const char* option)
{
...
static CURLcode tftp_connect_for_tx(struct tftp_state_data* state, tftp_event_t event)
{
    CURLcode result;
...
static CURLcode tftp_connect_for_rx(struct tftp_state_data* state, tftp_event_t event)
{
    CURLcode result;
#ifndef CURL_DISABLE_VERBOSE_STRINGS
    struct Curl_easy* data = state->data;
...
static CURLcode tftp_send_first(struct tftp_state_data* state, tftp_event_t event)
{
    size_t sbytes;
    ssize_t senddata;
    ...
    case TFTP_EVENT_TIMEOUT: /* Resend the first packet out */
       ...
       string. */
        result = Curl_urldecode(&state->data->state.up.path[1], 0, &filename, NULL, REJECT_ZERO);
        if (result)
            return result;
        ...
        msnprintf((char*)state->spacket.data + 2, state->blksize, "%s%c%s%c", filename, '\0', mode, '\0');
        ...
        if (!data->set.tftp_no_options) {
            ...
            result = tftp_option_add(state, &sbytes, (char*)state->spacket.data + sbytes, TFTP_OPTION_TSIZE);
            if (result == CURLE_OK)
                result = tftp_option_add(state, &sbytes, (char*)state->spacket.data + sbytes, buf);

            /* add blksize option */
            msnprintf(buf, sizeof(buf), "%d", state->requested_blksize);
            if (result == CURLE_OK)
                result = tftp_option_add(state, &sbytes, (char*)state->spacket.data + sbytes, TFTP_OPTION_BLKSIZE);
            if (result == CURLE_OK)
                result = tftp_option_add(state, &sbytes, (char*)state->spacket.data + sbytes, buf);
            ...
            if (result == CURLE_OK)
                result = tftp_option_add(state, &sbytes, (char*)state->spacket.data + sbytes, TFTP_OPTION_INTERVAL);
            if (result == CURLE_OK)
                result = tftp_option_add(state, &sbytes, (char*)state->spacket.data + sbytes, buf);
       ...
       not have a size_t argument, like older unixes that want an 'int' */
        senddata
            = sendto(state->sockfd, (void*)state->spacket.data, (SEND_TYPE_ARG3)sbytes, 0, &data->conn->remote_addr->sa_addr, data->conn->remote_addr->addrlen);
...
static CURLcode tftp_rx(struct tftp_state_data* state, tftp_event_t event)
{
    ...
    case TFTP_EVENT_DATA:
        ...
        if (NEXT_BLOCKNUM(state->block) == rblock) {
         ...
         again. */
            ...
            infof(data, "Received unexpected DATA packet block %d, expecting block %d", rblock, NEXT_BLOCKNUM(state->block));
            break;
        }
        ...
        sbytes = sendto(state->sockfd, (void*)state->spacket.data, 4, SEND_4TH_ARG, (struct sockaddr*)&state->remote_addr, state->remote_addrlen);
        if (sbytes < 0) {
            failf(data, "%s", Curl_strerror(SOCKERRNO, buffer, sizeof(buffer)));
    ...
    case TFTP_EVENT_TIMEOUT:
        ...
        state->retries++;
        infof(data, "Timeout waiting for block %d ACK.  Retries = %d", NEXT_BLOCKNUM(state->block), state->retries);
        if (state->retries > state->retry_max) {
    ...
    case TFTP_EVENT_ERROR:
        ...
        setpacketblock(&state->spacket, state->block);
        (void)sendto(state->sockfd, (void*)state->spacket.data, 4, SEND_4TH_ARG, (struct sockaddr*)&state->remote_addr, state->remote_addrlen);
...
static CURLcode tftp_tx(struct tftp_state_data* state, tftp_event_t event)
{
    ...
    case TFTP_EVENT_OACK:
        if (event == TFTP_EVENT_ACK) {
          ...
          * */
                ...
                infof(data, "Received ACK for block %d, expecting %d", rblock, state->block);
                state->retries++;
                ...
                if (state->retries > state->retry_max) {
                    ...
                    sbytes = sendto(state->sockfd, (void*)state->spacket.data, 4 + state->sbytes, SEND_4TH_ARG, (struct sockaddr*)&state->remote_addr,
     ...
     * */
        ...
        sbytes
            = sendto(state->sockfd, (void*)state->spacket.data, 4 + state->sbytes, SEND_4TH_ARG, (struct sockaddr*)&state->remote_addr, state->remote_addrlen);
        /* Check all sbytes were sent */
        if (sbytes < 0) {
            failf(data, "%s", Curl_strerror(SOCKERRNO, buffer, sizeof(buffer)));
            return CURLE_SEND_ERROR;
    ...
    case TFTP_EVENT_TIMEOUT:
        ...
        if (state->retries > state->retry_max) {
            ...
            /* Re-send the data packet */
            sbytes = sendto(
                state->sockfd, (void*)state->spacket.data, 4 + state->sbytes, SEND_4TH_ARG, (struct sockaddr*)&state->remote_addr, state->remote_addrlen);
            /* Check all sbytes were sent */
            if (sbytes < 0) {
                failf(data, "%s", Curl_strerror(SOCKERRNO, buffer, sizeof(buffer)));
                return CURLE_SEND_ERROR;
 ...
 *
 **********************************************************/
static CURLcode tftp_state_machine(struct tftp_state_data* state, tftp_event_t event)
{
    CURLcode result = CURLE_OK;
    struct Curl_easy* data = state->data;
...
static CURLcode tftp_connect(struct Curl_easy* data, bool* done)
{
   ...
   * little gain for UDP */
    ...
    ((struct sockaddr*)&state->local_addr)->sa_family = (CURL_SA_FAMILY_T)(conn->remote_addr->family);
     ...
     * IPv4 and IPv6...
     */
        int rc = bind(state->sockfd, (struct sockaddr*)&state->local_addr, conn->remote_addr->addrlen);
        if (rc) {
            char buffer[STRERROR_LEN];
 ...
 *
 **********************************************************/
static CURLcode tftp_done(struct Curl_easy* data, CURLcode status, bool premature)
{
    CURLcode result = CURLE_OK;
    struct connectdata* conn = data->conn;
    struct tftp_state_data* state = conn->proto.tftpc;
...
static CURLcode tftp_receive_packet(struct Curl_easy* data)
{
    ...
    state->rbytes = (int)recvfrom(state->sockfd, (void*)state->rpacket.data, state->blksize + 4, 0, (struct sockaddr*)&fromaddr, &fromlen);
    ...
    if (state->rbytes < 4) {
        ...
        case TFTP_EVENT_DATA:
            ...
            if (state->rbytes > 4 && (NEXT_BLOCKNUM(state->block) == getrpacketblock(&state->rpacket))) {
                result = Curl_client_write(data, CLIENTWRITE_BODY, (char*)state->rpacket.data + 4, state->rbytes - 4);
                if (!result) {
 ...
 *
 **********************************************************/
static timediff_t tftp_state_timeout(struct Curl_easy* data, tftp_event_t* event)
{
    time_t current;
    struct connectdata* conn = data->conn;
    struct tftp_state_data* state = conn->proto.tftpc;
    ...
    timeout_ms = Curl_timeleft(state->data, NULL, (state->state == TFTP_STATE_START));
    if (timeout_ms < 0) {
        state->error = TFTP_ERR_TIMEOUT;
        state->state = TFTP_STATE_FIN;
        return 0;
