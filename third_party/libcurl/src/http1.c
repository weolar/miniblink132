...
static ssize_t next_line(struct h1_req_parser* parser, const char* buf, const size_t buflen, int options, CURLcode* err)
{
    ssize_t nread = 0;
   ...
   */
    ...
    if (target_len == 1 && target[0] == '*') {
        ...
        if (Curl_is_absolute_url(tmp, NULL, 0, FALSE)) {
            ...
            url_options = (CURLU_NON_SUPPORT_SCHEME | CURLU_PATH_AS_IS | CURLU_NO_DEFAULT_PORT);
    ...
    if (url) {
        ...
        result = Curl_http_req_make(&parser->req, m, m_len, scheme, scheme_len, authority, authority_len, path, path_len);
    }
...
ssize_t Curl_h1_req_parse_read(struct h1_req_parser* parser, const char* buf, size_t buflen, const char* scheme_default, int options, CURLcode* err)
{
    ssize_t nread = 0, n;
    ...
    *err = CURLE_OK;
    while (!parser->done) {
        ...
        if (!parser->line) {
            ...
            *err = Curl_dynhds_h1_add_line(&parser->req->headers, parser->line, parser->line_len);
            if (*err) {
                nread = -1;
                goto out;
            }
...
CURLcode Curl_h1_req_write_head(struct httpreq* req, int http_minor, struct dynbuf* dbuf)
{
    CURLcode result;

    result = Curl_dyn_addf(dbuf, "%s %s%s%s%s HTTP/1.%d\r\n", req->method, req->scheme ? req->scheme : "", req->scheme ? "://" : "",
        req->authority ? req->authority : "", req->path ? req->path : "", http_minor);
    if (result)
        goto out;
