...
static void copy_header_external(struct Curl_header_store* hs, size_t index, size_t amount, struct Curl_llist_element* e, struct curl_header* hout)
{
...
/* public API */
CURLHcode curl_easy_header(CURL* easy, const char* name, size_t nameindex, unsigned int type, int request, struct curl_header** hout)
{
    ...
    if (!name || !hout || !data || (type > (CURLH_HEADER | CURLH_TRAILER | CURLH_CONNECT | CURLH_1XX | CURLH_PSEUDO)) || !type || (request < -1))
    ...
    copy_header_external(hs, nameindex, amount, e_pick, &data->state.headerout[0]);
    *hout = &data->state.headerout[0];
    return CURLHE_OK;
}

/* public API */
struct curl_header* curl_easy_nextheader(CURL* easy, unsigned int type, int request, struct curl_header* prev)
{
    struct Curl_easy* data = easy;
    ...
    for (e = data->state.httphdrs.head; e; e = e->next) {
        ...
        if (strcasecompare(hs->name, check->name) && (check->request == request) && (check->type & type))
            amount++;
        if (e == pick)
            index = amount - 1;
    }
...
static CURLcode namevalue(char* header, size_t hlen, unsigned int type, char** name, char** value)
{
...
static CURLcode unfold_value(struct Curl_easy* data, const char* value, size_t vlen) /* length of the incoming header */
{
    struct Curl_header_store* hs;
    ...
    Curl_llist_insert_next(&data->state.httphdrs, data->state.httphdrs.tail, newhs, &newhs->node);
...
CURLcode Curl_headers_push(struct Curl_easy* data, const char* header, unsigned char type)
{
    ...
    /* insert this node into the list of headers */
    Curl_llist_insert_next(&data->state.httphdrs, data->state.httphdrs.tail, hs, &hs->node);
