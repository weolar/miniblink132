...
// static
VibrationController::VibrationPattern VibrationController::SanitizeVibrationPattern(const V8UnionUnsignedLongOrUnsignedLongSequence* input)
{
    switch (input->GetContentType()) {
    case V8UnionUnsignedLongOrUnsignedLongSequence::ContentType::kUnsignedLong: {
        VibrationPattern pattern;
        pattern.push_back(input->GetAsUnsignedLong());
        return sanitizeVibrationPatternInternal(pattern);
    }
    case V8UnionUnsignedLongOrUnsignedLongSequence::ContentType::kUnsignedLongSequence:
        return sanitizeVibrationPatternInternal(input->GetAsUnsignedLongSequence());
    }
    NOTREACHED();
}
...
{
    VibrationController* vibration_controller = Supplement<Navigator>::From<VibrationController>(navigator);
...
// static
bool VibrationController::vibrate(Navigator& navigator, const VibrationPattern& pattern)
{
...
VibrationController::VibrationController(Navigator& navigator)
    ...
    , timer_do_vibrate_(DomWindow()->GetTaskRunner(TaskType::kMiscPlatformAPI), this, &VibrationController::DoVibrate)
    , is_running_(false)
    , is_calling_cancel_(false)
    , is_calling_vibrate_(false)
{
    DomWindow()->GetBrowserInterfaceBroker().GetInterface(
        vibration_manager_.BindNewPipeAndPassReceiver(DomWindow()->GetTaskRunner(TaskType::kMiscPlatformAPI)));
}
...
bool VibrationController::Vibrate(const VibrationPattern& pattern)
{
    ...
    if (frame->IsInFencedFrameTree()) {
        Intervention::GenerateReport(frame, "NavigatorVibrate", "Blocked call to navigator.vibrate inside a fenced frame.");
        return false;
    }
    ...
    if (!frame->HasStickyUserActivation()) {
        ...
        if (frame->IsCrossOriginToNearestMainFrame()) {
            message = "Blocked call to navigator.vibrate inside a cross-origin "
                      "iframe because the frame has never been activated by the user: "
                      "https://www.chromestatus.com/feature/5682658461876224.";
        } else {
            message = "Blocked call to navigator.vibrate because user hasn't tapped "
...
void VibrationController::DoVibrate(TimerBase* timer)
{
    ...
    if (!is_running_ || is_calling_cancel_ || is_calling_vibrate_ || !GetExecutionContext() || !GetPage()->IsPageVisible())
        return;

    if (vibration_manager_.is_bound()) {
        is_calling_vibrate_ = true;
        vibration_manager_->Vibrate(pattern_[0], WTF::BindOnce(&VibrationController::DidVibrate, WrapPersistent(this)));
    }
}
...
void VibrationController::Cancel()
{
    ...
    if (is_running_ && !is_calling_cancel_ && vibration_manager_.is_bound()) {
        ...
        vibration_manager_->Cancel(WTF::BindOnce(&VibrationController::DidCancel, WrapPersistent(this)));
    }
