...
bool IsSupportedType(AtomicString type)
{
    DEFINE_STATIC_LOCAL(HashSet<AtomicString>, elements,
        ({ // Common types that include addresses.
            AtomicString("AutoDealer"), AtomicString("Hotel"), AtomicString("LocalBusiness"), AtomicString("Organization"), AtomicString("Person"),
            AtomicString("Place"), AtomicString("PostalAddress"), AtomicString("Product"), AtomicString("Residence"), AtomicString("Restaurant"),
            AtomicString("SingleFamilyResidence"),
            // Common types including phone numbers
            AtomicString("Store"), AtomicString("ContactPoint"), AtomicString("LodgingBusiness") }));
    return type && elements.Contains(type);
}

void ExtractEntity(const JSONObject&, int recursion_level, Entity&);

bool ParseRepeatedValue(const JSONArray& arr, int recursion_level, ValuesPtr& values)
{
    if (arr.size() < 1) {
        return false;
    }
    ...
    for (wtf_size_t i = 0; i < arr_size; ++i) {
        ...
        case JSONValue::ValueType::kTypeObject: {
            ...
            ExtractEntity(*(JSONObject::Cast(element)), recursion_level + 1, *entity);
...
void ExtractEntity(const JSONObject& val, int recursion_level, Entity& entity)
{
    ...
    for (wtf_size_t i = 0; i < std::min(val.size(), kMaxNumFields); ++i) {
        ...
        case JSONValue::ValueType::kTypeObject: {
            ...
            ExtractEntity(*(val.GetJSONObject(entry.first)), recursion_level + 1, *entities[0]);
            property->values = Values::NewEntityValues(std::move(entities));
        } break;
        case JSONValue::ValueType::kTypeArray:
            add_property = ParseRepeatedValue(*(val.GetArray(entry.first)), recursion_level, property->values);
            break;
        case JSONValue::ValueType::kTypeNull:
            add_property = false;
            break;
...
void ExtractEntitiesFromArray(const JSONArray& arr, Vector<EntityPtr>& entities)
{
...
void ExtractEntityFromTopLevelObject(const JSONObject& val, Vector<EntityPtr>& entities)
{
...
ExtractionStatus ExtractMetadata(const Element& root, Vector<EntityPtr>& entities)
{
    for (Element& element : ElementTraversal::DescendantsOf(root)) {
        if (element.HasTagName(html_names::kScriptTag) && element.FastGetAttribute(html_names::kTypeAttr) == "application/ld+json") {
            // TODO(crbug.com/1264024): Deprecate JSON comments here, if possible.
            std::unique_ptr<JSONValue> json = ParseJSONWithCommentsDeprecated(element.textContent());
            if (!json) {
            ...
            case JSONValue::ValueType::kTypeArray:
                ...
                break;
            case JSONValue::ValueType::kTypeObject:
                ExtractEntityFromTopLevelObject(*(JSONObject::Cast(json.get())), entities);
                break;
            default:
