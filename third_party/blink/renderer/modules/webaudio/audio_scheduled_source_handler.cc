...
AudioScheduledSourceHandler::AudioScheduledSourceHandler(NodeType node_type, AudioNode& node, float sample_rate)
...
std::tuple<size_t, size_t, double> AudioScheduledSourceHandler::UpdateSchedulingInfo(size_t quantum_frame_size, AudioBus* output_bus)
{
    ...
    DCHECK(output_bus);
    DCHECK_EQ(quantum_frame_size, static_cast<size_t>(GetDeferredTaskHandler().RenderQuantumFrames()));
    ...
    // early.
    size_t start_frame = audio_utilities::TimeToSampleFrame(start_time_, sample_rate, audio_utilities::kRoundUp);
    size_t end_frame = 0;
    ...
    if (end_time_ == kUnknownTime) {
        ...
        end_frame = audio_utilities::TimeToSampleFrame(end_time_, sample_rate, audio_utilities::kRoundUp);
    }
    ...
    if (state == UNSCHEDULED_STATE || state == FINISHED_STATE || start_frame >= quantum_end_frame) {
        // Output silence.
        output_bus->Zero();
        non_silent_frames_to_process = 0;
        return std::make_tuple(quantum_frame_offset, non_silent_frames_to_process, start_frame_offset);
    }
    ...
    quantum_frame_offset = start_frame > quantum_start_frame ? start_frame - quantum_start_frame : 0;
    ...
    if (quantum_frame_offset) {
        for (unsigned i = 0; i < output_bus->NumberOfChannels(); ++i) {
            memset(output_bus->Channel(i)->MutableData(), 0, sizeof(float) * quantum_frame_offset);
        }
    }
    ...
    if (end_time_ != kUnknownTime && end_frame >= quantum_start_frame && end_frame < quantum_end_frame) {
        ...
        bool is_safe = zero_start_frame < quantum_frame_size && frames_to_zero <= quantum_frame_size && zero_start_frame + frames_to_zero <= quantum_frame_size;
        if (is_safe) {
            ...
            for (unsigned i = 0; i < output_bus->NumberOfChannels(); ++i) {
                memset(output_bus->Channel(i)->MutableData() + zero_start_frame, 0, sizeof(float) * frames_to_zero);
            }
        }
...
void AudioScheduledSourceHandler::Start(double when, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
...
void AudioScheduledSourceHandler::Stop(double when, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());

    if (GetPlaybackState() == UNSCHEDULED_STATE) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "cannot call stop without calling start first.");
        return;
    }

    if (when < 0) {
        exception_state.ThrowRangeError(ExceptionMessages::IndexExceedsMinimumBound("stop time", when, 0.0));
        return;
    }
...
void AudioScheduledSourceHandler::Finish()
{
    ...
    PostCrossThreadTask(*task_runner_, FROM_HERE, CrossThreadBindOnce(&AudioScheduledSourceHandler::NotifyEnded, AsWeakPtr()));
}
...
void AudioScheduledSourceHandler::NotifyEnded()
{
    ...
    if (GetNode()) {
        DispatchEventResult result = GetNode()->DispatchEvent(*Event::Create(event_type_names::kEnded));
