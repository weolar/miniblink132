...
class MODULES_EXPORT AudioContext final : public BaseAudioContext, public mojom::blink::PermissionObserver, public mojom::blink::MediaDevicesListener {
    DEFINE_WRAPPERTYPEINFO();

public:
    static AudioContext* Create(ExecutionContext*, const AudioContextOptions*, ExceptionState&);

    AudioContext(LocalDOMWindow&, const WebAudioLatencyHint&, std::optional<float> sample_rate, WebAudioSinkDescriptor sink_descriptor,
    ...
    bool HasRealtimeConstraint() override
    {
        return true;
    }
    ...
    bool HandlePreRenderTasks(uint32_t frames_to_process, const AudioIOPosition* output_position, const AudioCallbackMetric* metric,
        base::TimeDelta playout_delay, const media::AudioGlitchInfo& glitch_info) override;
    ...
    V8UnionAudioSinkInfoOrString* sinkId() const
    {
        return v8_sink_id_.Get();
    }
    ...
    ScriptPromise<IDLUndefined> setSinkId(ScriptState*, const V8UnionAudioSinkOptionsOrString*, ExceptionState&);
    ...
    WebAudioSinkDescriptor GetSinkDescriptor() const
    {
        return sink_descriptor_;
    }
    ...
    void set_was_audible_for_testing(bool value)
    {
        was_audible_ = value;
    }
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(AudioContextTest, OnRenderErrorFromPlatformDestination);
    ...
    AudioIOPosition OutputPosition() const VALID_CONTEXT_REQUIRED(main_thread_sequence_checker_);

    // Send notification to browser that an AudioContext has started or stopped
    // playing audible audio.
    void NotifyAudibleAudioStarted() VALID_CONTEXT_REQUIRED(main_thread_sequence_checker_);
    void NotifyAudibleAudioStopped() VALID_CONTEXT_REQUIRED(main_thread_sequence_checker_);

    void EnsureAudioContextManagerService();
    void OnAudioContextManagerServiceConnectionError();

    void DidInitialPermissionCheck(mojom::blink::PermissionDescriptorPtr, mojom::blink::PermissionStatus);
    ...
    void DevicesEnumerated(const Vector<Vector<WebMediaDeviceInfo>>& enumeration,
        ...
        Vector<mojom::blink::AudioInputDeviceCapabilitiesPtr> audio_input_capabilities) VALID_CONTEXT_REQUIRED(main_thread_sequence_checker_);
    ...
    void HandleRenderError() VALID_CONTEXT_REQUIRED(main_thread_sequence_checker_);
    ...
    mojom::blink::PermissionStatus microphone_permission_status_ = mojom::blink::PermissionStatus::DENIED;

    HeapMojoRemote<mojom::blink::PermissionService> permission_service_;
    HeapMojoReceiver<mojom::blink::PermissionObserver, AudioContext> permission_receiver_;
    ...
    HeapMojoRemote<mojom::blink::MediaDevicesDispatcherHost> media_device_service_;
    HeapMojoReceiver<mojom::blink::MediaDevicesListener, AudioContext> media_device_service_receiver_;
    ...
    // device list is refrehsed before using it.  A value of 0 means no updates
    // are pending.
    int pending_device_list_updates_ GUARDED_BY_CONTEXT(main_thread_sequence_checker_) = 0;
