...
public:
    explicit MockWebAudioDeviceForAutoplayTest(double sample_rate, int frames_per_buffer)
...
public:
    ...
    {
        return std::make_unique<MockWebAudioDeviceForAutoplayTest>(AudioHardwareSampleRate(), AudioHardwareBufferSize());
    }
...
class AudioContextAutoplayTest : public testing::TestWithParam<AutoplayPolicy::Type> {
protected:
    ...
    {
        helper_.Initialize();
        frame_test_helpers::LoadHTMLString(helper_.LocalMainFrame(), "<iframe></iframe>", WebURL(KURL("https://example.com")));
        frame_test_helpers::LoadHTMLString(To<WebLocalFrameImpl>(helper_.LocalMainFrame()->FirstChild()), "", WebURL(KURL("https://cross-origin.com")));
    ...
    {
        return *To<WebLocalFrameImpl>(helper_.LocalMainFrame()->FirstChild())->GetFrame()->DomWindow();
    }
...
{
    AudioContext* audio_context = AudioContext::Create(&ChildWindow(), AudioContextOptions::Create(), ASSERT_NO_EXCEPTION);
    ...
    case AutoplayPolicy::Type::kDocumentUserActivationRequired:
        GetHistogramTester()->ExpectBucketCount(kAutoplayMetric, static_cast<int>(AutoplayStatus::kFailed), 1);
        GetHistogramTester()->ExpectTotalCount(kAutoplayMetric, 1);
        GetHistogramTester()->ExpectBucketCount(kAutoplayCrossOriginMetric, static_cast<int>(AutoplayStatus::kFailed), 1);
...
{
    AudioContext* audio_context = AudioContext::Create(&GetWindow(), AudioContextOptions::Create(), ASSERT_NO_EXCEPTION);
    RecordAutoplayStatus(audio_context);
...
// child frame.
TEST_P(AudioContextAutoplayTest, AutoplayMetrics_CallResumeNoGesture_Child)
{
    ScriptState::Scope scope(GetScriptStateFrom(ChildWindow()));

    AudioContext* audio_context = AudioContext::Create(&ChildWindow(), AudioContextOptions::Create(), ASSERT_NO_EXCEPTION);
    audio_context->resumeContext(GetScriptStateFrom(ChildWindow()), ASSERT_NO_EXCEPTION);
...
{
    LocalFrame::NotifyUserActivation(ChildWindow().GetFrame(), mojom::UserActivationNotificationType::kTest);
    ...
    case AutoplayPolicy::Type::kUserGestureRequired:
    case AutoplayPolicy::Type::kDocumentUserActivationRequired:
        GetHistogramTester()->ExpectBucketCount(kAutoplayMetric, static_cast<int>(AutoplayStatus::kSucceeded), 1);
        GetHistogramTester()->ExpectTotalCount(kAutoplayMetric, 1);
        GetHistogramTester()->ExpectBucketCount(kAutoplayCrossOriginMetric, static_cast<int>(AutoplayStatus::kSucceeded), 1);
        GetHistogramTester()->ExpectTotalCount(kAutoplayCrossOriginMetric, 1);
        break;
    }
}
...
{
    LocalFrame::NotifyUserActivation(GetWindow().GetFrame(), mojom::UserActivationNotificationType::kTest);
...
TEST_P(AudioContextAutoplayTest, AutoplayMetrics_CallResumeGesture_Main)
{
    ...
    audio_context->resumeContext(GetScriptStateFrom(GetWindow()), ASSERT_NO_EXCEPTION);
    RejectPendingResolvers(audio_context);
    RecordAutoplayStatus(audio_context);
...
TEST_P(AudioContextAutoplayTest, AutoplayMetrics_NodeStartNoGestureThenSuccess_Child)
{
    ScriptState::Scope scope(GetScriptStateFrom(ChildWindow()));
...
TEST_P(AudioContextAutoplayTest, AutoplayMetrics_NodeStartNoGestureThenSuccess_Main)
{
    ScriptState::Scope scope(GetScriptStateFrom(GetWindow()));
...
// finally allows the AudioContext to produce sound inside x-origin child frame.
TEST_P(AudioContextAutoplayTest, AutoplayMetrics_NodeStartGestureThenSucces_Child)
{
    ScriptState::Scope scope(GetScriptStateFrom(ChildWindow()));
...
// Creates an AudioContext then calls start on a node with a gesture and
// finally allows the AudioContext to produce sound inside a main frame.
TEST_P(AudioContextAutoplayTest, AutoplayMetrics_NodeStartGestureThenSucces_Main)
{
    ScriptState::Scope scope(GetScriptStateFrom(GetWindow()));
...
TEST_P(AudioContextAutoplayTest, AutoplayMetrics_DocumentReceivedGesture_Child)
{
    LocalFrame::NotifyUserActivation(ChildWindow().GetFrame(), mojom::UserActivationNotificationType::kTest);
...
// document previous received a user gesture.
TEST_P(AudioContextAutoplayTest, AutoplayMetrics_DocumentReceivedGesture_Main)
{
    LocalFrame::NotifyUserActivation(ChildWindow().GetFrame(), mojom::UserActivationNotificationType::kTest);
...
TEST_P(AudioContextAutoplayTest, AutoplayMetrics_DocumentReceivedGesture_BeforeNavigation)
{
...
INSTANTIATE_TEST_SUITE_P(AudioContextAutoplayTest, AudioContextAutoplayTest,
    testing::Values(
        AutoplayPolicy::Type::kNoUserGestureRequired, AutoplayPolicy::Type::kUserGestureRequired, AutoplayPolicy::Type::kDocumentUserActivationRequired));
