...
GainHandler::GainHandler(AudioNode& node, float sample_rate, AudioParamHandler& gain)
    : AudioHandler(kNodeTypeGain, node, sample_rate)
    , gain_(&gain)
    , sample_accurate_gain_values_(GetDeferredTaskHandler().RenderQuantumFrames())
{
    AddInput();
...
scoped_refptr<GainHandler> GainHandler::Create(AudioNode& node, float sample_rate, AudioParamHandler& gain)
{
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("webaudio.audionode"), "GainHandler::Process");
    ...
    if (!IsInitialized() || !Input(0).IsConnected()) {
        ...
        if (is_sample_accurate && gain_->IsAudioRate()) {
            ...
            output_bus->CopyWithSampleAccurateGainValuesFrom(*input_bus, gain_values, frames_to_process);
