...
public:
    ...
    AudioWorkletProcessor(AudioWorkletGlobalScope*, const String& name, MessagePort*);
    ~AudioWorkletProcessor() override;

    // `AudioWorkletHandler` invokes this method to process audio.
    bool Process(const Vector<scoped_refptr<AudioBus>>& inputs, Vector<scoped_refptr<AudioBus>>& outputs,
...
private:
    using BackingArrayBuffers = HeapVector<HeapVector<TraceWrapperV8Reference<v8::ArrayBuffer>>>;
    ...
    static bool PortTopologyMatches(
        v8::Isolate*, v8::Local<v8::Context>, const Vector<scoped_refptr<AudioBus>>& audio_port_1, const TraceWrapperV8Reference<v8::Array>& audio_port_2);
    ...
    // exception.
    static bool FreezeAudioPort(v8::Isolate*, v8::Local<v8::Context>, v8::Local<v8::Array>& audio_port_array);
    ...
    // exception.
    static bool ClonePortTopology(v8::Isolate*, v8::Local<v8::Context>, const Vector<scoped_refptr<AudioBus>>& audio_port_1,
    ...
    // be identical.
    static void CopyPortToArrayBuffers(v8::Isolate*, const Vector<scoped_refptr<AudioBus>>& audio_port, BackingArrayBuffers& array_buffers);

    // Copies a BackingArrayBuffers to an AudioPort. The size of two args must
    // be identical.
    static void CopyArrayBuffersToPort(v8::Isolate*, const BackingArrayBuffers& array_buffers, Vector<scoped_refptr<AudioBus>>& audio_port);

    // Fills a given BackingArrayBuffers with zeros.
    static void ZeroArrayBuffers(v8::Isolate*, const BackingArrayBuffers& array_buffers);
    ...
    static bool ParamValueMapMatchesToParamsObject(v8::Isolate*, v8::Local<v8::Context>,
    ...
    static bool CloneParamValueMapToObject(v8::Isolate*, v8::Local<v8::Context>, const HashMap<String, std::unique_ptr<AudioFloatArray>>& param_value_map,
        TraceWrapperV8Reference<v8::Object>& params);

    // Copies the content of float arrays from `param_value_map` to `params`
    // v8::Object.
    static bool CopyParamValueMapToObject(v8::Isolate*, v8::Local<v8::Context>, const HashMap<String, std::unique_ptr<AudioFloatArray>>& param_value_map,
        TraceWrapperV8Reference<v8::Object>& params);
    ...
    AudioWorkletProcessorErrorState error_state_ = AudioWorkletProcessorErrorState::kNoError;
};
