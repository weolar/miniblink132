...
AudioBufferSourceHandler::AudioBufferSourceHandler(AudioNode& node, float sample_rate, AudioParamHandler& playback_rate, AudioParamHandler& detune)
    : AudioScheduledSourceHandler(kNodeTypeAudioBufferSource, node, sample_rate)
...
scoped_refptr<AudioBufferSourceHandler> AudioBufferSourceHandler::Create(
    AudioNode& node, float sample_rate, AudioParamHandler& playback_rate, AudioParamHandler& detune)
{
    return base::AdoptRef(new AudioBufferSourceHandler(node, sample_rate, playback_rate, detune));
}
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("webaudio.audionode"), "AudioBufferSourceHandler::Process");
    ...
    if (try_locker.is_acquired()) {
        ...
        std::tie(quantum_frame_offset, buffer_frames_to_process, start_time_offset) = UpdateSchedulingInfo(frames_to_process, output_bus);
        ...
        if (!RenderFromBuffer(output_bus, quantum_frame_offset, buffer_frames_to_process, start_time_offset)) {
            output_bus->Zero();
            return;
        }
...
bool AudioBufferSourceHandler::RenderSilenceAndFinishIfNotLooping(AudioBus*, unsigned index, uint32_t frames_to_process)
{
    if (!Loop()) {
        ...
        if (frames_to_process > 0) {
            ...
            for (unsigned i = 0; i < NumberOfChannels(); ++i) {
                memset(destination_channels_[i] + index, 0, sizeof(float) * frames_to_process);
            }
        }
...
bool AudioBufferSourceHandler::RenderFromBuffer(AudioBus* bus, unsigned destination_frame_offset, uint32_t number_of_frames, double start_time_offset)
{
    ...
    bool channel_count_good = number_of_channels && number_of_channels == bus_number_of_channels;
    ...
    if (destination_frame_offset) {
        for (unsigned i = 0; i < number_of_channels; ++i) {
            memset(destination_channels_[i], 0, sizeof(float) * destination_frame_offset);
        }
    }
    ...
    unsigned end_frame
        = is_grain_ ? base::saturated_cast<uint32_t>(audio_utilities::TimeToSampleFrame(grain_offset_ + grain_duration_, buffer_sample_rate)) : buffer_length;
    ...
    if (Loop() && (loop_start_ || loop_end_) && loop_start_ >= 0 && loop_end_ > 0 && loop_start_ < loop_end_) {
    ...
    if (Loop() && virtual_read_index_ >= virtual_end_frame) {
        virtual_read_index_ = (loop_start_ < 0) ? 0 : (loop_start_ * shared_buffer_->sampleRate());
        virtual_read_index_ = std::min(virtual_read_index_, static_cast<double>(buffer_length - 1));
    }
    ...
    if (start_time_offset < 0) {
        if (computed_playback_rate != 0) {
            virtual_read_index += std::abs(start_time_offset * computed_playback_rate);
        }
    }
    ...
    if (computed_playback_rate == 1 && virtual_read_index == floor(virtual_read_index) && virtual_delta_frames == floor(virtual_delta_frames)
        ...
        while (frames_to_process > 0) {
            ...
            for (unsigned i = 0; i < number_of_channels; ++i) {
                ...
                if (source_channels[i]) {
                    memcpy(destination_channels[i] + write_index, source_channels[i] + read_index, sizeof(float) * frames_this_time);
                } else {
                    // Recall that a floating-point zero is represented by 4 bytes of 0.
                    memset(destination_channels[i] + write_index, 0, sizeof(float) * frames_this_time);
                }
            }
            ...
            if (read_index >= end_frame) {
                ...
                if (RenderSilenceAndFinishIfNotLooping(bus, write_index, frames_to_process)) {
                    break;
                }
            }
        }
        ...
        while (frames_to_process--) {
            ...
            if (read_index2 >= buffer_length) {
                if (Loop()) {
                    ...
                    read_index2 = static_cast<unsigned>(virtual_read_index + 1 - virtual_delta_frames);
                } else {
            ...
            for (unsigned i = 0; i < number_of_channels; ++i) {
                ...
                if (source) {
                    if (read_index == read_index2 && read_index >= 1) {
                        ...
                        sample = (1.0 - interpolation_factor) * sample1 + interpolation_factor * sample2;
                    }
...
void AudioBufferSourceHandler::SetBuffer(AudioBuffer* buffer, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    ...
    if (!buffer) {
        ...
        if (number_of_channels > BaseAudioContext::MaxNumberOfChannels()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError,
                ExceptionMessages::IndexOutsideRange("number of input channels", number_of_channels, 1u, ExceptionMessages::kInclusiveBound,
        ...
        for (unsigned i = 0; i < number_of_channels; ++i) {
            source_channels_[i] = static_cast<float*>(shared_buffer_->channels()[i].Data());
        }
...
void AudioBufferSourceHandler::ClampGrainParameters(const SharedAudioBuffer* buffer)
{
    DCHECK(buffer);
    ...
    if (is_duration_given_ && Loop()) {
        ...
        grain_duration_ = ClampTo(grain_duration_, 0.0, std::numeric_limits<double>::infinity());
    ...
    virtual_read_index_ = audio_utilities::TimeToSampleFrame(grain_offset_, shared_buffer_->sampleRate());
}

base::WeakPtr<AudioScheduledSourceHandler> AudioBufferSourceHandler::AsWeakPtr()
{
    return weak_ptr_factory_.GetWeakPtr();
}

void AudioBufferSourceHandler::Start(double when, ExceptionState& exception_state)
{
    AudioScheduledSourceHandler::Start(when, exception_state);
}

void AudioBufferSourceHandler::Start(double when, double grain_offset, ExceptionState& exception_state)
{
    StartSource(when, grain_offset, Buffer() ? shared_buffer_->duration() : 0, false, exception_state);
}

void AudioBufferSourceHandler::Start(double when, double grain_offset, double grain_duration, ExceptionState& exception_state)
{
    StartSource(when, grain_offset, grain_duration, true, exception_state);
}

void AudioBufferSourceHandler::StartSource(double when, double grain_offset, double grain_duration, bool is_duration_given, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    ...
    if (GetPlaybackState() != UNSCHEDULED_STATE) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "cannot call start more than once.");
        return;
    }

    if (when < 0) {
        exception_state.ThrowRangeError(ExceptionMessages::IndexExceedsMinimumBound("start time", when, 0.0));
        return;
    }

    if (grain_offset < 0) {
        exception_state.ThrowRangeError(ExceptionMessages::IndexExceedsMinimumBound("offset", grain_offset, 0.0));
        return;
    }

    if (grain_duration < 0) {
        exception_state.ThrowRangeError(ExceptionMessages::IndexExceedsMinimumBound("duration", grain_duration, 0.0));
        return;
    }
...
double AudioBufferSourceHandler::ComputePlaybackRate()
{
    ...
    if (Buffer()) {
        ...
        sample_rate_factor = shared_buffer_->sampleRate() / static_cast<double>(Context()->sampleRate());
    }
...
void AudioBufferSourceHandler::HandleStoppableSourceNode()
{
    ...
    if (end_time_ != AudioScheduledSourceHandler::kUnknownTime && Context()->currentTime() > end_time_) {
        Finish();
        return;
    }
    ...
    if (!DidSetLooping() && Buffer() && IsPlayingOrScheduled() && min_playback_rate > 0) {
        ...
        double extra_stop_time = kExtraStopFrames / static_cast<double>(Context()->sampleRate());
