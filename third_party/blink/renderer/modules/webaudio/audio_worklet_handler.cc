...
AudioWorkletHandler::AudioWorkletHandler(AudioNode& node, float sample_rate, String name, HashMap<String, scoped_refptr<AudioParamHandler>> param_handler_map,
    ...
    , param_handler_map_(param_handler_map)
{
    ...
    for (const auto& param_name : param_handler_map_.Keys()) {
        param_value_map_.Set(param_name, std::make_unique<AudioFloatArray>(GetDeferredTaskHandler().RenderQuantumFrames()));
    }
    ...
    for (unsigned i = 0; i < options->numberOfOutputs(); ++i) {
        ...
        AddOutput(is_output_channel_count_given_ ? options->outputChannelCount()[i] : kDefaultNumberOfOutputChannels);
    }
    ...
    if (Context()->GetExecutionContext()) {
        ...
        main_thread_task_runner_ = Context()->GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI);
...
scoped_refptr<AudioWorkletHandler> AudioWorkletHandler::Create(AudioNode& node, float sample_rate, String name,
    HashMap<String, scoped_refptr<AudioParamHandler>> param_handler_map, const AudioWorkletNodeOptions* options)
{
    return base::AdoptRef(new AudioWorkletHandler(node, sample_rate, name, param_handler_map, options));
}
...
void AudioWorkletHandler::Process(uint32_t frames_to_process)
{
    ...
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("webaudio.audionode"), "AudioWorkletHandler::Process");
    ...
    for (unsigned i = 0; i < NumberOfOutputs(); ++i) {
        if (!Output(i).IsConnectedDuringRendering()) {
            ...
            if (!unconnected_outputs_[i] || !unconnected_outputs_[i]->TopologyMatches(*Output(i).Bus())) {
                unconnected_outputs_[i] = AudioBus::Create(Output(i).Bus()->NumberOfChannels(), GetDeferredTaskHandler().RenderQuantumFrames());
            }
    ...
    for (const auto& param_name : param_value_map_.Keys()) {
        ...
        if (param_handler->HasSampleAccurateValues() && param_handler->IsAudioRate()) {
            param_handler->CalculateSampleAccurateValues(param_values->Data(), static_cast<uint32_t>(frames_to_process));
        } else {
            std::fill(param_values->Data(), param_values->Data() + frames_to_process, param_handler->FinalValue());
        }
    }
    ...
    if (!processor_->Process(inputs_, outputs_, param_value_map_) || processor_->hasErrorOccurred()) {
...
void AudioWorkletHandler::CheckNumberOfChannelsForInput(AudioNodeInput* input)
{
    ...
    if (NumberOfInputs() == 1 && NumberOfOutputs() == 1 && !is_output_channel_count_given_) {
...
void AudioWorkletHandler::SetProcessorOnRenderThread(AudioWorkletProcessor* processor)
{
    ...
    if (processor) {
        ...
        PostCrossThreadTask(*main_thread_task_runner_, FROM_HERE,
            CrossThreadBindOnce(
                &AudioWorkletHandler::NotifyProcessorError, weak_ptr_factory_.GetWeakPtr(), AudioWorkletProcessorErrorState::kConstructionError));
    }
}
...
void AudioWorkletHandler::FinishProcessorOnRenderThread()
{
    ...
    if (error_state == AudioWorkletProcessorErrorState::kProcessError) {
        PostCrossThreadTask(
            *main_thread_task_runner_, FROM_HERE, CrossThreadBindOnce(&AudioWorkletHandler::NotifyProcessorError, weak_ptr_factory_.GetWeakPtr(), error_state));
    }
    ...
    PostCrossThreadTask(
        *main_thread_task_runner_, FROM_HERE, CrossThreadBindOnce(&AudioWorkletHandler::MarkProcessorInactiveOnMainThread, weak_ptr_factory_.GetWeakPtr()));
}

void AudioWorkletHandler::NotifyProcessorError(AudioWorkletProcessorErrorState error_state)
{
    DCHECK(IsMainThread());
