...
scoped_refptr<DelayHandler> DelayHandler::Create(AudioNode& node, float sample_rate, AudioParamHandler& delay_time, double max_delay_time)
{
    return base::AdoptRef(new DelayHandler(node, sample_rate, delay_time, max_delay_time));
}
...
DelayHandler::DelayHandler(AudioNode& node, float sample_rate, AudioParamHandler& delay_time, double max_delay_time)
    : AudioHandler(kNodeTypeDelay, node, sample_rate)
    , number_of_channels_(kDefaultNumberOfChannels)
    , sample_rate_(sample_rate)
    , render_quantum_frames_(node.context()->GetDeferredTaskHandler().RenderQuantumFrames())
...
void DelayHandler::Process(uint32_t frames_to_process)
{
    ...
    if (!IsInitialized() || number_of_channels_ != Output(0).NumberOfChannels()) {
        ...
        if (process_try_locker.is_acquired() && rate_try_locker.is_acquired()) {
            DCHECK_EQ(source_bus->NumberOfChannels(), destination_bus->NumberOfChannels());
            ...
            if (delay_time_->IsAudioRate()) {
                for (unsigned i = 0; i < kernels_.size(); ++i) {
                    ...
                    delay_time_->CalculateSampleAccurateValues(kernels_[i]->DelayTimes(), frames_to_process);
                    kernels_[i]->ProcessARate(source_bus->Channel(i)->Data(), destination_bus->Channel(i)->MutableData(), frames_to_process);
                }
            } else {
                for (unsigned i = 0; i < kernels_.size(); ++i) {
                    CHECK(!delay_time_->IsAudioRate());
                    kernels_[i]->SetDelayTime(delay_time_->FinalValue());
                    kernels_[i]->ProcessKRate(source_bus->Channel(i)->Data(), destination_bus->Channel(i)->MutableData(), frames_to_process);
                }
            }
        } else {
            destination_bus->Zero();
...
void DelayHandler::Initialize()
{
...

    {
        ...
        for (unsigned i = 0; i < number_of_channels_; ++i) {
            kernels_.push_back(std::make_unique<Delay>(max_delay_time_, sample_rate_, render_quantum_frames_));
        }
    }
