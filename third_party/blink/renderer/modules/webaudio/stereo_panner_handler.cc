...
StereoPannerHandler::StereoPannerHandler(AudioNode& node, float sample_rate, AudioParamHandler& pan)
    : AudioHandler(kNodeTypeStereoPanner, node, sample_rate)
    , pan_(&pan)
    , sample_accurate_pan_values_(GetDeferredTaskHandler().RenderQuantumFrames())
{
    AddInput();
...
scoped_refptr<StereoPannerHandler> StereoPannerHandler::Create(AudioNode& node, float sample_rate, AudioParamHandler& pan)
{
...
void StereoPannerHandler::Process(uint32_t frames_to_process)
{
    ...
    if (is_sample_accurate && pan_->IsAudioRate()) {
        ...
        stereo_panner_->PanWithSampleAccurateValues(input_bus.get(), output_bus, pan_values, frames_to_process);
        return;
    }
    ...
    stereo_panner_->PanToTargetValue(input_bus.get(), output_bus, pan_value, frames_to_process);
}
...
void StereoPannerHandler::SetChannelCount(unsigned channel_count, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
...
void StereoPannerHandler::SetChannelCountMode(V8ChannelCountMode::Enum mode, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    DeferredTaskHandler::GraphAutoLocker locker(Context());
    ...
    if (mode == V8ChannelCountMode::Enum::kClampedMax || mode == V8ChannelCountMode::Enum::kExplicit) {
        ...
        // 1 or 2 channels.
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "StereoPanner: 'max' is not allowed");
