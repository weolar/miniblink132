...
TEST(HasConstantValuesTest, RegressionTest)
{
    ...
    EXPECT_TRUE(blink::BiquadDSPKernel::HasConstantValuesForTesting(values, frames_to_process));

    // Test with a single different element at each position
    for (float& value : values) {
        value = 1.0;
        EXPECT_FALSE(blink::BiquadDSPKernel::HasConstantValuesForTesting(values, frames_to_process));
        value = 0;
    }
}
