...
class MODULES_EXPORT AudioWorkletMessagingProxy final : public ThreadedWorkletMessagingProxy {
public:
    ...
    void CreateProcessor(scoped_refptr<AudioWorkletHandler>, MessagePortChannel, scoped_refptr<SerializedScriptValue> node_options);

    // Invokes AudioWorkletGlobalScope to create an instance of
    // AudioWorkletProcessor.
    void CreateProcessorOnRenderingThread(
        WorkerThread*, scoped_refptr<AudioWorkletHandler>, const String& name, MessagePortChannel, scoped_refptr<SerializedScriptValue> node_options);
    ...
    void SynchronizeWorkletProcessorInfoList(std::unique_ptr<Vector<CrossThreadAudioWorkletProcessorInfo>>);

    // Returns true if the processor with given name is registered in
    // AudioWorkletGlobalScope.
    bool IsProcessorRegistered(const String& name) const;

    Vector<CrossThreadAudioParamInfo> GetParamInfoListForProcessor(const String& name) const;
    ...
    static std::unique_ptr<WorkerThread> CreateWorkletThreadWithConstraints(
        WorkerReportingProxy&, std::optional<base::TimeDelta> realtime_buffer_duration, const bool is_outermost_main_frame);

    void Trace(Visitor*) const override;

private:
    // Implements ThreadedWorkletMessagingProxy.
    std::unique_ptr<ThreadedWorkletObjectProxy> CreateObjectProxy(ThreadedWorkletMessagingProxy*, ParentExecutionContextTaskRunners*,
        scoped_refptr<base::SingleThreadTaskRunner> parent_agent_group_task_runner) override;
