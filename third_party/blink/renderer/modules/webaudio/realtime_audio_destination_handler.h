...
class MODULES_EXPORT RealtimeAudioDestinationHandler final : public AudioDestinationHandler, public AudioIOCallback {
public:
    static scoped_refptr<RealtimeAudioDestinationHandler> Create(
        AudioNode&, const WebAudioSinkDescriptor&, const WebAudioLatencyHint&, std::optional<float> sample_rate, bool update_echo_cancellation_on_first_start);
    ...
    // `output_position`.
    void Render(AudioBus* destination_bus, uint32_t number_of_frames, const AudioIOPosition& output_position, const AudioCallbackMetric& metric,
    ...
    void SetSinkDescriptor(const WebAudioSinkDescriptor& sink_descriptor, media::OutputDeviceStatusCB callback);
