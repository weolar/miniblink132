...
public:
    ...
    std::unique_ptr<WorkerThread> CreateAudioWorkletThread(
        bool has_realtime_constraint, bool is_top_level_frame, base::TimeDelta realtime_buffer_duration = base::Milliseconds(3))
    {
        std::unique_ptr<WorkerThread> thread = AudioWorkletMessagingProxy::CreateWorkletThreadWithConstraints(
            *reporting_proxy_, has_realtime_constraint ? std::optional<base::TimeDelta>(realtime_buffer_duration) : std::nullopt, is_top_level_frame);
    ...
    void CheckWorkletCanExecuteScript(WorkerThread* thread)
    {
        ...
        PostCrossThreadTask(*thread->GetWorkerBackingThread().BackingThread().GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(&AudioWorkletThreadTest::ExecuteScriptInWorklet, CrossThreadUnretained(this), CrossThreadUnretained(thread),
...
private:
    void StartBackingThreadAndWaitUntilInit(WorkerThread* thread)
    {
        ...
        thread->Start(
            std::make_unique<GlobalScopeCreationParams>(window->Url(), mojom::blink::ScriptType::kModule, "AudioWorklet", window->UserAgent(),
                window->GetFrame()->Loader().UserAgentMetadata(), nullptr /* web_worker_fetch_context */,
                Vector<network::mojom::blink::ContentSecurityPolicyPtr>(), Vector<network::mojom::blink::ContentSecurityPolicyPtr>(),
                window->GetReferrerPolicy(), window->GetSecurityOrigin(), window->IsSecureContext(), window->GetHttpsState(), nullptr /* worker_clients */,
                nullptr /* content_settings_client */, OriginTrialContext::GetInheritedTrialFeatures(window).get(), base::UnguessableToken::Create(),
                nullptr /* worker_settings */, mojom::blink::V8CacheOptions::kDefault, MakeGarbageCollected<WorkletModuleResponsesMap>(),
                mojo::NullRemote() /* browser_interface_broker */, window->GetFrame()->Loader().CreateWorkerCodeCacheHost(),
                window->GetFrame()->GetBlobUrlStorePendingRemote(), BeginFrameProviderParams(), nullptr /* parent_permissions_policy */,
                window->GetAgentClusterID(), ukm::kInvalidSourceId, window->GetExecutionContextToken()),
            std::optional(WorkerBackingThreadStartupData::CreateDefault()), std::make_unique<WorkerDevToolsParams>());

        // Wait until the cross-thread initialization is completed.
        base::WaitableEvent completion_event;
        PostCrossThreadTask(*thread->GetWorkerBackingThread().BackingThread().GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(&base::WaitableEvent::Signal, CrossThreadUnretained(&completion_event)));
        completion_event.Wait();
    }

    void ExecuteScriptInWorklet(WorkerThread* thread, base::WaitableEvent* wait_event)
    {
        ScriptState* script_state = thread->GlobalScope()->ScriptController()->GetScriptState();
        EXPECT_TRUE(script_state);
        ScriptState::Scope scope(script_state);
        KURL js_url("https://example.com/worklet.js");
        v8::Local<v8::Module> module = ModuleTestBase::CompileModule(script_state, "var counter = 0; ++counter;", js_url);
        EXPECT_FALSE(module.IsEmpty());
        ScriptValue exception = ModuleRecord::Instantiate(script_state, module, js_url);
        EXPECT_TRUE(exception.IsEmpty());
        ScriptEvaluationResult result
            = JSModuleScript::CreateForTest(Modulator::From(script_state), module, js_url)->RunScriptOnScriptStateAndReturnValue(script_state);
...
{
    std::unique_ptr<WorkerThread> audio_worklet_thread = CreateAudioWorkletThread(true, true);
...
TEST_F(AudioWorkletThreadTest, CreateDifferentWorkletThreadsAndTerminate_1)
{
    ...
    std::unique_ptr<WorkerThread> first_worklet_thread = CreateAudioWorkletThread(true, true);
    Thread* first_backing_thread = &first_worklet_thread->GetWorkerBackingThread().BackingThread();
    v8::Isolate* first_isolate = first_worklet_thread->GetIsolate();

    // Create OfflineAudioWorkletThread.
    std::unique_ptr<WorkerThread> second_worklet_thread = CreateAudioWorkletThread(false, true);
    Thread* second_backing_thread = &second_worklet_thread->GetWorkerBackingThread().BackingThread();
...
TEST_F(AudioWorkletThreadTest, CreateDifferentWorkletThreadsAndTerminate_2)
{
    ...
    std::unique_ptr<WorkerThread> second_worklet_thread = CreateAudioWorkletThread(true, false);
    Thread* second_backing_thread = &second_worklet_thread->GetWorkerBackingThread().BackingThread();
    v8::Isolate* second_isolate = second_worklet_thread->GetIsolate();
...
class AudioWorkletThreadInteractionTest : public AudioWorkletThreadTest, public testing::WithParamInterface<std::tuple<bool, bool>> {
public:
...
TEST_P(AudioWorkletThreadInteractionTest, CreateSecondAndTerminateFirst)
{
    ...
    std::unique_ptr<WorkerThread> first_worklet_thread = CreateAudioWorkletThread(has_realtime_constraint_, is_top_level_frame_);
    Thread* first_backing_thread = &first_worklet_thread->GetWorkerBackingThread().BackingThread();
    ...
    std::unique_ptr<WorkerThread> second_worklet_thread = CreateAudioWorkletThread(has_realtime_constraint_, is_top_level_frame_);
    Thread* second_backing_thread = &second_worklet_thread->GetWorkerBackingThread().BackingThread();
...
TEST_P(AudioWorkletThreadInteractionTest, TerminateFirstAndCreateSecond)
{
    ...
    std::unique_ptr<WorkerThread> worklet_thread = CreateAudioWorkletThread(has_realtime_constraint_, is_top_level_frame_);
    Thread* first_backing_thread = &worklet_thread->GetWorkerBackingThread().BackingThread();
    ...
    worklet_thread = CreateAudioWorkletThread(has_realtime_constraint_, is_top_level_frame_);
    Thread* second_backing_thread = &worklet_thread->GetWorkerBackingThread().BackingThread();
    CheckWorkletCanExecuteScript(worklet_thread.get());
...
TEST_P(AudioWorkletThreadInteractionTest, ThreadManagementSystemForRealtimeAndTopLevelFrame)
{
    ...
    for (int i = 0; i < number_of_threads; i++) {
        worklet_threads[i] = CreateAudioWorkletThread(has_realtime_constraint_, is_top_level_frame_);
        worklet_backing_threads[i] = &worklet_threads[i]->GetWorkerBackingThread().BackingThread();
    }
    ...
    if (has_realtime_constraint_ && is_top_level_frame_) {
        ...
        ASSERT_EQ(worklet_threads[0]->GetExitCodeForTesting(), WorkerThread::ExitCode::kNotTerminated);
        ASSERT_EQ(worklet_threads[1]->GetExitCodeForTesting(), WorkerThread::ExitCode::kNotTerminated);

        // Create a new thread and verify if 3 dedicated threads are running.
        std::unique_ptr<WorkerThread> new_worklet_thread = CreateAudioWorkletThread(has_realtime_constraint_, is_top_level_frame_);
        Thread* new_worklet_backing_thread = &new_worklet_thread->GetWorkerBackingThread().BackingThread();
    ...
    ASSERT_EQ(worklet_threads[3]->GetExitCodeForTesting(), WorkerThread::ExitCode::kNotTerminated);
...
INSTANTIATE_TEST_SUITE_P(AudioWorkletThreadInteractionTestGroup, AudioWorkletThreadInteractionTest, testing::Combine(testing::Bool(), testing::Bool()));
...
class AudioWorkletThreadPriorityTest : public AudioWorkletThreadTest, public testing::WithParamInterface<ThreadPriorityTestParam> {
public:
    ...
    void CreateCheckThreadPriority(bool has_realtime_constraint, bool is_top_level_frame, base::ThreadPriorityForTest expected_priority)
    {
        std::unique_ptr<WorkerThread> audio_worklet_thread = CreateAudioWorkletThread(has_realtime_constraint, is_top_level_frame);
        WorkerThread* thread = audio_worklet_thread.get();
        base::WaitableEvent wait_event;
        PostCrossThreadTask(*thread->GetWorkerBackingThread().BackingThread().GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(&AudioWorkletThreadPriorityTest::CheckThreadPriorityOnWorkerThread, CrossThreadUnretained(this), CrossThreadUnretained(thread),
                expected_priority, CrossThreadUnretained(&wait_event)));
        wait_event.Wait();
        audio_worklet_thread->Terminate();
        audio_worklet_thread->WaitForShutdownForTesting();
    }
...
private:
    void CheckThreadPriorityOnWorkerThread(WorkerThread* thread, base::ThreadPriorityForTest expected_priority, base::WaitableEvent* wait_event)
    {
        ASSERT_TRUE(thread->IsCurrentThread());
        base::ThreadPriorityForTest actual_priority = base::PlatformThread::GetCurrentThreadPriorityForTest();

        // TODO(crbug.com/1022888): The worklet thread priority is always NORMAL
        // on OS_LINUX and OS_CHROMEOS regardless of the thread priority setting.
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
        if (expected_priority == base::ThreadPriorityForTest::kRealtimeAudio || expected_priority == base::ThreadPriorityForTest::kDisplay) {
...
TEST_P(AudioWorkletThreadPriorityTest, CheckThreadPriority)
{
    ...
    CreateCheckThreadPriority(test_param.has_realtime_constraint, test_param.is_top_level_frame, test_param.expected_priority);
}

INSTANTIATE_TEST_SUITE_P(AudioWorkletThreadPriorityTestGroup, AudioWorkletThreadPriorityTest, testing::ValuesIn(kThreadPriorityTestParams));
...
namespace WTF {
template <> struct CrossThreadCopier<base::TimeDelta> : public CrossThreadCopierPassThrough<base::TimeDelta> {
...
public:
    std::unique_ptr<WorkerThread> CreateThreadAndCheckRealtimePeriod(base::TimeDelta realtime_buffer_duration, base::TimeDelta expected_realtime_period)
    {
        std::unique_ptr<WorkerThread> audio_worklet_thread = CreateAudioWorkletThread(/*has_realtime_constraint=*/true,
            /*is_top_level_frame=*/true, realtime_buffer_duration);
        WorkerThread* thread = audio_worklet_thread.get();
        base::WaitableEvent wait_event;
        PostCrossThreadTask(*thread->GetWorkerBackingThread().BackingThread().GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(&AudioWorkletRealtimePeriodTestMac::CheckThreadRealtimePeriodOnWorkerThread, CrossThreadUnretained(this),
                CrossThreadUnretained(thread), expected_realtime_period, CrossThreadUnretained(&wait_event)));
        wait_event.Wait();
        return audio_worklet_thread;
    }

private:
    void CheckThreadRealtimePeriodOnWorkerThread(WorkerThread* thread, base::TimeDelta expected_realtime_period, base::WaitableEvent* wait_event)
    {
        ASSERT_TRUE(thread->IsCurrentThread());

        base::ThreadPriorityForTest actual_priority = base::PlatformThread::GetCurrentThreadPriorityForTest();

        base::TimeDelta actual_realtime_period = base::PlatformThread::GetCurrentThreadRealtimePeriodForTest();
...
TEST_F(AudioWorkletRealtimePeriodTestMac, CheckRealtimePeriod)
{
    ...
    base::TimeDelta realtime_buffer_durations[]
        = { base::Milliseconds(10), base::Milliseconds(20), base::Milliseconds(30), base::Milliseconds(40), base::Milliseconds(50) };

    std::vector<std::unique_ptr<WorkerThread>> worklet_threads;
    worklet_threads.push_back(CreateThreadAndCheckRealtimePeriod(realtime_buffer_durations[0], realtime_buffer_durations[0]));
    worklet_threads.push_back(CreateThreadAndCheckRealtimePeriod(realtime_buffer_durations[1], realtime_buffer_durations[1]));
    worklet_threads.push_back(CreateThreadAndCheckRealtimePeriod(realtime_buffer_durations[2], realtime_buffer_durations[2]));
    worklet_threads.push_back(CreateThreadAndCheckRealtimePeriod(realtime_buffer_durations[3], realtime_buffer_durations[3]));
    // Note: we expect that the last two worklets share the same backng thread, so
    // the should have the same realtime period.
    worklet_threads.push_back(CreateThreadAndCheckRealtimePeriod(realtime_buffer_durations[4], realtime_buffer_durations[3]));
