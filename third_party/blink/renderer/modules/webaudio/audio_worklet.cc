    ...
    scoped_refptr<AudioWorkletHandler> handler, MessagePortChannel message_port_channel, scoped_refptr<SerializedScriptValue> node_options)
{
    ...
    GetMessagingProxy()->CreateProcessor(std::move(handler), std::move(message_port_channel), std::move(node_options));
}
...
Vector<CrossThreadAudioParamInfo> AudioWorklet::GetParamInfoListForProcessor(const String& name)
{
    DCHECK(IsMainThread());
    DCHECK(GetMessagingProxy());
...
WorkletGlobalScopeProxy* AudioWorklet::CreateGlobalScope()
{
    ...
    AudioWorkletMessagingProxy* proxy = MakeGarbageCollected<AudioWorkletMessagingProxy>(GetExecutionContext(), this);
    proxy->Initialize(MakeGarbageCollected<WorkerClients>(), ModuleResponsesMap(), WorkerBackingThreadStartupData::CreateDefault());
    return proxy;
}

AudioWorkletMessagingProxy* AudioWorklet::GetMessagingProxy()
{
    return GetNumberOfGlobalScopes() == 0 ? nullptr : static_cast<AudioWorkletMessagingProxy*>(FindAvailableGlobalScope());
}
