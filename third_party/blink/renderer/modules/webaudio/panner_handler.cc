...
PannerHandler::PannerHandler(AudioNode& node, float sample_rate, AudioParamHandler& position_x, AudioParamHandler& position_y, AudioParamHandler& position_z,
    AudioParamHandler& orientation_x, AudioParamHandler& orientation_y, AudioParamHandler& orientation_z)
...
scoped_refptr<PannerHandler> PannerHandler::Create(AudioNode& node, float sample_rate, AudioParamHandler& position_x, AudioParamHandler& position_y,
    AudioParamHandler& position_z, AudioParamHandler& orientation_x, AudioParamHandler& orientation_y, AudioParamHandler& orientation_z)
{
    return base::AdoptRef(new PannerHandler(node, sample_rate, position_x, position_y, position_z, orientation_x, orientation_y, orientation_z));
}
...
void PannerHandler::ProcessIfNecessary(uint32_t frames_to_process)
{
    ...
    if (last_processing_time_ != current_time) {
        ...
        if (!silent_inputs) {
            ...
            last_non_silent_time_ = (Context()->CurrentSampleFrame() + frames_to_process) / static_cast<double>(Context()->sampleRate());
        }
    }
}
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("webaudio.audionode"), "PannerHandler::Process");
    ...
    if (try_listener_locker.is_acquired()) {
        if (!Context()->HasRealtimeConstraint() && panning_model_ == Panner::PanningModel::kHRTF) {
        ...
        if ((HasSampleAccurateValues() || listener_handler_->HasSampleAccurateValues()) && (IsAudioRate() || listener_handler_->IsAudioRate())) {
            ...
            panner_->Pan(azimuth, elevation, source.get(), destination, frames_to_process, InternalChannelInterpretation());
...
void PannerHandler::ProcessSampleAccurateValues(AudioBus* destination, const AudioBus* source, uint32_t frames_to_process)
{
    ...
    const unsigned render_quantum_frames = GetDeferredTaskHandler().RenderQuantumFrames();
    ...
    orientation_x_->CalculateSampleAccurateValues(orientation_x, frames_to_process);
    orientation_y_->CalculateSampleAccurateValues(orientation_y, frames_to_process);
    orientation_z_->CalculateSampleAccurateValues(orientation_z, frames_to_process);

    const float* listener_x = listener_handler_->GetPositionXValues(render_quantum_frames);
    const float* listener_y = listener_handler_->GetPositionYValues(render_quantum_frames);
    const float* listener_z = listener_handler_->GetPositionZValues(render_quantum_frames);
    const float* forward_x = listener_handler_->GetForwardXValues(render_quantum_frames);
    const float* forward_y = listener_handler_->GetForwardYValues(render_quantum_frames);
    const float* forward_z = listener_handler_->GetForwardZValues(render_quantum_frames);
    ...
    for (unsigned k = 0; k < frames_to_process; ++k) {
        ...
        gfx::Vector3dF orientation(orientation_x[k], orientation_y[k], orientation_z[k]);
        ...
        CalculateAzimuthElevation(&azimuth[k], &elevation[k], panner_position, listener_position, listener_forward, listener_up);

        total_gain[k] = CalculateDistanceConeGain(panner_position, orientation, listener_position);
    }
    ...
    panner_->PanWithSampleAccurateValues(azimuth, elevation, source, destination, frames_to_process, InternalChannelInterpretation());
    destination->CopyWithSampleAccurateGainValuesFrom(*destination, total_gain, frames_to_process);
}
...
void PannerHandler::ProcessOnlyAudioParams(uint32_t frames_to_process)
{
    ...
    // stack allocation to heap allocation.
    constexpr unsigned render_quantum_frames_expected = 128;
    CHECK_EQ(GetDeferredTaskHandler().RenderQuantumFrames(), render_quantum_frames_expected);
...
void PannerHandler::Initialize()
{
    ...
    panner_ = Panner::Create(panning_model_, Context()->sampleRate(), GetDeferredTaskHandler().RenderQuantumFrames(), listener_handler_->HrtfDatabaseLoader());
...
bool PannerHandler::SetPanningModel(Panner::PanningModel model)
{
    ...
    if (!panner_.get() || model != panning_model_) {
        ...
        panner_ = Panner::Create(model, Context()->sampleRate(), GetDeferredTaskHandler().RenderQuantumFrames(), listener_handler_->HrtfDatabaseLoader());
...
bool PannerHandler::SetDistanceModel(unsigned model)
{
    ...
    case DistanceEffect::kModelExponential:
        if (model != distance_model_) {
            // This synchronizes with process().
            base::AutoLock process_locker(process_lock_);
            distance_effect_.SetModel(static_cast<DistanceEffect::ModelType>(model));
...
void PannerHandler::SetPosition(float x, float y, float z, ExceptionState& exceptionState)
{
    // This synchronizes with process().
    base::AutoLock process_locker(process_lock_);
    ...
    MarkPannerAsDirty(PannerHandler::kAzimuthElevationDirty | PannerHandler::kDistanceConeGainDirty);
}

void PannerHandler::SetOrientation(float x, float y, float z, ExceptionState& exceptionState)
{
    // This synchronizes with process().
    base::AutoLock process_locker(process_lock_);
...
void PannerHandler::CalculateAzimuthElevation(double* out_azimuth, double* out_elevation, const gfx::Point3F& position, const gfx::Point3F& listener_position,
    const gfx::Vector3dF& listener_forward, const gfx::Vector3dF& listener_up)
{
    ...
    // Align axes
    gfx::Vector3dF listener_right = gfx::CrossProduct(listener_forward, listener_up);
    ...
    gfx::Vector3dF projected_source = source_listener - gfx::ScaleVector3d(up, up_projection);
    ...
    double azimuth = Rad2deg(acos(ClampTo(gfx::DotProduct(projected_source, listener_right), -1.0f, 1.0f)));
    ...
    // Elevation
    double elevation = 90 - gfx::AngleBetweenVectorsInDegrees(source_listener, up);
...
float PannerHandler::CalculateDistanceConeGain(const gfx::Point3F& position, const gfx::Vector3dF& orientation, const gfx::Point3F& listener_position)
{
    double listener_distance = (position - listener_position).Length();
    double distance_gain = distance_effect_.Gain(listener_distance);
    double cone_gain = cone_effect_.Gain(position, orientation, listener_position);

    return static_cast<float>(distance_gain * cone_gain);
}

void PannerHandler::AzimuthElevation(double* out_azimuth, double* out_elevation)
{
    DCHECK(Context()->IsAudioThread());
    ...
    if (IsAzimuthElevationDirty() || listener_handler_->IsListenerDirty()) {
        CalculateAzimuthElevation(&cached_azimuth_, &cached_elevation_, GetPosition(), listener_handler_->GetPosition(), listener_handler_->GetOrientation(),
...
float PannerHandler::DistanceConeGain()
{
    ...
    if (IsDistanceConeGainDirty() || listener_handler_->IsListenerDirty()) {
        cached_distance_cone_gain_ = CalculateDistanceConeGain(GetPosition(), Orientation(), listener_handler_->GetPosition());
...
void PannerHandler::SetChannelCount(unsigned channel_count, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    DeferredTaskHandler::GraphAutoLocker locker(Context());

    if (channel_count >= kMinimumOutputChannels && channel_count <= kMaximumOutputChannels) {
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError,
            ExceptionMessages::IndexOutsideRange<uint32_t>("channelCount", channel_count, kMinimumOutputChannels, ExceptionMessages::kInclusiveBound,
                kMaximumOutputChannels, ExceptionMessages::kInclusiveBound));
    }
}

void PannerHandler::SetChannelCountMode(V8ChannelCountMode::Enum mode, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    DeferredTaskHandler::GraphAutoLocker locker(Context());
    ...
    if (mode == V8ChannelCountMode::Enum::kClampedMax || mode == V8ChannelCountMode::Enum::kExplicit) {
        ...
        // channels.
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Panner: 'max' is not allowed");
...
{
    auto x = position_x_->IsAudioRate() ? position_x_->FinalValue() : position_x_->Value();
    auto y = position_y_->IsAudioRate() ? position_y_->FinalValue() : position_y_->Value();
    auto z = position_z_->IsAudioRate() ? position_z_->FinalValue() : position_z_->Value();
...
{
    auto x = orientation_x_->IsAudioRate() ? orientation_x_->FinalValue() : orientation_x_->Value();
    auto y = orientation_y_->IsAudioRate() ? orientation_y_->FinalValue() : orientation_y_->Value();
    auto z = orientation_z_->IsAudioRate() ? orientation_z_->FinalValue() : orientation_z_->Value();
...
{
    return position_x_->HasSampleAccurateValues() || position_y_->HasSampleAccurateValues() || position_z_->HasSampleAccurateValues()
        || orientation_x_->HasSampleAccurateValues() || orientation_y_->HasSampleAccurateValues() || orientation_z_->HasSampleAccurateValues();
}

bool PannerHandler::IsAudioRate() const
{
    return position_x_->IsAudioRate() || position_y_->IsAudioRate() || position_z_->IsAudioRate() || orientation_x_->IsAudioRate()
...
void PannerHandler::UpdateDirtyState()
{
    ...
    bool has_moved = current_position != last_position_ || current_orientation != last_orientation_;
