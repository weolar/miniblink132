...
void HandleNaNValues(float* values, unsigned number_of_values, float default_value)
{
    unsigned k = 0;
#if defined(ARCH_CPU_X86_FAMILY)
...
AudioParamHandler::AudioParamHandler(BaseAudioContext& context, AudioParamType param_type, double default_value, AutomationRate rate,
    AutomationRateMode rate_mode, float min_value, float max_value)
    ...
    , summing_bus_(AudioBus::Create(1, GetDeferredTaskHandler().RenderQuantumFrames(), false))
{
...
float AudioParamHandler::Value()
{
    ...
    if (GetDeferredTaskHandler().IsAudioThread()) {
        auto [has_value, timeline_value]
            = timeline_.ValueForContextTime(DestinationHandler(), v, MinValue(), MaxValue(), GetDeferredTaskHandler().RenderQuantumFrames());
...
void AudioParamHandler::CalculateSampleAccurateValues(float* values, unsigned number_of_values)
{
...
void AudioParamHandler::CalculateFinalValues(float* values, unsigned number_of_values, bool sample_accurate)
{
    DCHECK(GetDeferredTaskHandler().IsAudioThread());
    DCHECK(values);
    DCHECK_GT(number_of_values, 0u);
    ...
    if (sample_accurate) {
        ...
        float value = IntrinsicValue();
        auto [has_value, timeline_value]
            = timeline_.ValueForContextTime(DestinationHandler(), value, MinValue(), MaxValue(), GetDeferredTaskHandler().RenderQuantumFrames());
    ...
    if (NumberOfRenderingConnections() > 0) {
        ...
        summing_bus_->SetChannelMemory(0, values, sample_accurate ? number_of_values : 1);
        ...
        for (unsigned i = 0; i < NumberOfRenderingConnections(); ++i) {
            ...
            AudioBus* connection_bus = output->Pull(nullptr, GetDeferredTaskHandler().RenderQuantumFrames());
...
void AudioParamHandler::CalculateTimelineValues(float* values, unsigned number_of_values)
{
    ...
    SetIntrinsicValue(timeline_.ValuesForFrameRange(start_frame, end_frame, IntrinsicValue(), values, number_of_values, sample_rate, sample_rate, MinValue(),
        MaxValue(), GetDeferredTaskHandler().RenderQuantumFrames()));
}
