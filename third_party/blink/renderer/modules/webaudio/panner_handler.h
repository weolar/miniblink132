...
public:
    ...
    static scoped_refptr<PannerHandler> Create(AudioNode&, float sample_rate, AudioParamHandler& position_x, AudioParamHandler& position_y,
    ...
    void ProcessSampleAccurateValues(AudioBus* destination, const AudioBus* source, uint32_t frames_to_process);
    ...
    {
        return panner_ ? panner_->TailTime() : 0;
    }
    void Uninitialize() override;

    double ConeInnerAngle() const
    {
        return cone_effect_.InnerAngle();
    }
    double ConeOuterAngle() const
    {
        return cone_effect_.OuterAngle();
    }
    double ConeOuterGain() const
    {
        return cone_effect_.OuterGain();
    }
    V8DistanceModelType::Enum DistanceModel() const;
    double MaxDistance()
    {
        return distance_effect_.MaxDistance();
    }
    V8PanningModelType::Enum PanningModel() const;
    double RefDistance()
    {
        return distance_effect_.RefDistance();
    }
    double RolloffFactor()
    {
        return distance_effect_.RolloffFactor();
    }
...
private:
    PannerHandler(AudioNode&, float sample_rate, AudioParamHandler& position_x, AudioParamHandler& position_y, AudioParamHandler& position_z,
    ...
    void CalculateAzimuthElevation(double* out_azimuth, double* out_elevation, const gfx::Point3F& position, const gfx::Point3F& listener_position,
    ...
    float CalculateDistanceConeGain(const gfx::Point3F& position, const gfx::Vector3dF& orientation, const gfx::Point3F& listener_position);
    ...
    bool IsAzimuthElevationDirty() const
    {
        return is_azimuth_elevation_dirty_;
    }
    bool IsDistanceConeGainDirty() const
    {
        return is_distance_cone_gain_dirty_;
    }
