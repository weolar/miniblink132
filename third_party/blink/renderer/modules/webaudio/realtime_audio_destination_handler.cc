...
scoped_refptr<RealtimeAudioDestinationHandler> RealtimeAudioDestinationHandler::Create(AudioNode& node, const WebAudioSinkDescriptor& sink_descriptor,
    const WebAudioLatencyHint& latency_hint, std::optional<float> sample_rate, bool update_echo_cancellation_on_first_start)
{
    return base::AdoptRef(new RealtimeAudioDestinationHandler(node, sink_descriptor, latency_hint, sample_rate, update_echo_cancellation_on_first_start));
}

RealtimeAudioDestinationHandler::RealtimeAudioDestinationHandler(AudioNode& node, const WebAudioSinkDescriptor& sink_descriptor,
    const WebAudioLatencyHint& latency_hint, std::optional<float> sample_rate, bool update_echo_cancellation_on_first_start)
    ...
    , task_runner_(Context()->GetExecutionContext()->GetTaskRunner(TaskType::kInternalMediaRealTime))
    , update_echo_cancellation_on_next_start_(update_echo_cancellation_on_first_start)
{
...
void RealtimeAudioDestinationHandler::SetChannelCount(unsigned channel_count, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());

    SendLogMessage(__func__, String::Format("({channel_count=%u})", channel_count));
    ...
    if (!blink::WebLocalFrame::FrameForCurrentContext()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot change channel count on a detached document.");
        return;
    }
    ...
    if (channel_count > MaxChannelCount()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError,
            ExceptionMessages::IndexOutsideRange<unsigned>(
                "channel count", channel_count, 1, ExceptionMessages::kInclusiveBound, MaxChannelCount(), ExceptionMessages::kInclusiveBound));
        return;
    }
    ...
    CHECK(context);
    if (context->ContextState() == V8AudioContextState::Enum::kClosed || ChannelCount() == old_channel_count || exception_state.HadException()) {
        return;
    }
...
void RealtimeAudioDestinationHandler::Render(AudioBus* destination_bus, uint32_t number_of_frames, const AudioIOPosition& output_position,
    const AudioCallbackMetric& metric, base::TimeDelta playout_delay, const media::AudioGlitchInfo& glitch_info)
{
    TRACE_EVENT("webaudio", "RealtimeAudioDestinationHandler::Render", "frames", number_of_frames, "playout_delay (ms)", playout_delay.InMillisecondsF());
    ...
    context->HandlePreRenderTasks(number_of_frames, &output_position, &metric, playout_delay, glitch_info);
    ...
    if (IsPullingAudioGraphAllowed()) {
        ...
        scoped_refptr<AudioBus> rendered_bus = Input(0).Pull(destination_bus, number_of_frames);
    ...
    SetDetectSilenceIfNecessary(context->GetDeferredTaskHandler().HasAutomaticPullNodes());
}
...
BASE_FEATURE(kUseFakeAudioWorkerForPlaybackLatency, "UseFakeAudioWorkerForPlaybackLatency", base::FEATURE_ENABLED_BY_DEFAULT);

void RealtimeAudioDestinationHandler::SetDetectSilenceIfNecessary(bool has_automatic_pull_nodes)
{
    ...
    bool needs_silence_detection = !has_automatic_pull_nodes || Input(0).IsConnected();
    ...
    if (is_detecting_silence_ != needs_silence_detection) {
        PostCrossThreadTask(*task_runner_, FROM_HERE,
            CrossThreadBindOnce(&RealtimeAudioDestinationHandler::SetDetectSilence, weak_ptr_factory_.GetWeakPtr(), needs_silence_detection));
...
base::TimeDelta RealtimeAudioDestinationHandler::GetPlatformBufferDuration() const
{
    DCHECK(IsMainThread());
    DCHECK(IsInitialized());
...
void RealtimeAudioDestinationHandler::CreatePlatformDestination()
{
    ...
    platform_destination_ = AudioDestination::Create(
        *this, sink_descriptor_, ChannelCount(), latency_hint_, sample_rate_, Context()->GetDeferredTaskHandler().RenderQuantumFrames());
    ...
    TRACE_EVENT0("webaudio", "RealtimeAudioDestinationHandler::CreatePlatformDestination");
}

void RealtimeAudioDestinationHandler::StartPlatformDestination()
{
    TRACE_EVENT1("webaudio", "RealtimeAudioDestinationHandler::StartPlatformDestination", "sink information (when starting a new destination)",
        audio_utilities::GetSinkInfoForTracing(
            sink_descriptor_, latency_hint_, MaxChannelCount(), sample_rate_.has_value() ? sample_rate_.value() : -1, GetCallbackBufferSize()));
    DCHECK(IsMainThread());
    ...
    if (update_echo_cancellation_on_next_start_) {
        ...
        if (sink_descriptor_.Type() == WebAudioSinkDescriptor::AudioSinkType::kAudible) {
            const media::OutputDeviceStatus output_device_status = platform_destination_->MaybeCreateSinkAndGetStatus();
            if (output_device_status == media::OutputDeviceStatus::OUTPUT_DEVICE_STATUS_OK) {
                if (auto* execution_context = Context()->GetExecutionContext()) {
                    PeerConnectionDependencyFactory::From(*execution_context).GetWebRtcAudioDevice()->SetOutputDeviceForAec(sink_descriptor_.SinkId());
                    SendLogMessage(__func__, "=> sink is OK and echo cancellation reference was updated.");
                } else {
                ...
                SendLogMessage(__func__, String::Format("=> sink is not OK. (output_device_status=%i)", output_device_status));
            }
        }
    }
    ...
    if (audio_worklet && audio_worklet->IsReady()) {
        ...
        platform_destination_->StartWithWorkletTaskRunner(
            audio_worklet->GetMessagingProxy()->GetBackingWorkerThread()->GetTaskRunner(TaskType::kInternalMediaRealTime));
    } else {
...
void RealtimeAudioDestinationHandler::PrepareTaskRunnerForWorklet()
{
    ...
    platform_destination_->SetWorkletTaskRunner(
        Context()->audioWorklet()->GetMessagingProxy()->GetBackingWorkerThread()->GetTaskRunner(TaskType::kInternalMediaRealTime));
}

void RealtimeAudioDestinationHandler::SetSinkDescriptor(const WebAudioSinkDescriptor& sink_descriptor, media::OutputDeviceStatusCB callback)
{
    TRACE_EVENT1("webaudio", "RealtimeAudioDestinationHandler::SetSinkDescriptor", "sink information (when descriptor change requested)",
        audio_utilities::GetSinkInfoForTracing(
            sink_descriptor, latency_hint_, MaxChannelCount(), sample_rate_.has_value() ? sample_rate_.value() : -1, GetCallbackBufferSize()));
    DCHECK(IsMainThread());
    ...
    if (context->ContextState() == V8AudioContextState::Enum::kClosed) {
        std::move(callback).Run(media::OutputDeviceStatus::OUTPUT_DEVICE_STATUS_ERROR_INTERNAL);
        return;
    }

    // Create a pending AudioDestination to replace the current one.
    scoped_refptr<AudioDestination> pending_platform_destination = AudioDestination::Create(
        *this, sink_descriptor, ChannelCount(), latency_hint_, sample_rate_, Context()->GetDeferredTaskHandler().RenderQuantumFrames());
    ...
    media::OutputDeviceStatus status = pending_platform_destination->MaybeCreateSinkAndGetStatus();
    if (status == media::OutputDeviceStatus::OUTPUT_DEVICE_STATUS_OK) {
        ...
        update_echo_cancellation_on_next_start_ = update_echo_cancellation_on_next_start_ || (sink_descriptor_ != sink_descriptor);
        sink_descriptor_ = sink_descriptor;
        ...
        SendLogMessage(__func__, String::Format("=> sink is not OK. (status=%i)", status));
    }

    std::move(callback).Run(status);
}

void RealtimeAudioDestinationHandler::invoke_onrendererror_from_platform_for_testing()
{
    platform_destination_->OnRenderError();
}

bool RealtimeAudioDestinationHandler::get_platform_destination_is_playing_for_testing()
{
    return platform_destination_->IsPlaying();
}

void RealtimeAudioDestinationHandler::SendLogMessage(const char* const function_name, const String& message) const
{
    WebRtcLogMessage(String::Format("[WA]RADH::%s %s (sink_descriptor_=%s)", function_name, message.Utf8().c_str(), sink_descriptor_.SinkId().Utf8().c_str())
                         .Utf8()
                         .c_str());
}
