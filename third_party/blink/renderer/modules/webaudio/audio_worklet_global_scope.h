...
public:
    ProcessorCreationParams(const String& name, MessagePortChannel message_port_channel)
        : name_(name)
        , message_port_channel_(message_port_channel)
    {
    }
    ...
    MessagePortChannel PortChannel()
    {
        return message_port_channel_;
    }
...
public:
    AudioWorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams>, WorkerThread*);
    ~AudioWorkletGlobalScope() override;

    bool IsAudioWorkletGlobalScope() const final
    {
        return true;
    }
    void Dispose() final;
    ...
    void registerProcessor(const String& name, V8BlinkAudioWorkletProcessorConstructor* processor_ctor, ExceptionState&);
    ...
    // for some reason.
    AudioWorkletProcessor* CreateProcessor(const String& name, MessagePortChannel, scoped_refptr<SerializedScriptValue> node_options);

    AudioWorkletProcessorDefinition* FindDefinition(const String& name);

    unsigned NumberOfRegisteredDefinitions();

    std::unique_ptr<Vector<CrossThreadAudioWorkletProcessorInfo>> WorkletProcessorInfoListForSynchronization();
    ...
    const AudioWorkletToken& GetAudioWorkletToken() const
    {
        return token_;
    }
    WorkletToken GetWorkletToken() const final
    {
        return token_;
    }
...
private:
    typedef HeapHashMap<String, Member<AudioWorkletProcessorDefinition>> ProcessorDefinitionMap;
...
template <> struct DowncastTraits<AudioWorkletGlobalScope> {
