...
public:
    ...
    AudioParamType GetParamType() const
    {
        return param_type_;
    }
    ...
    static scoped_refptr<AudioParamHandler> Create(BaseAudioContext& context, AudioParamType param_type, double default_value, AutomationRate rate,
        AutomationRateMode rate_mode, float min_value, float max_value)
    {
        return base::AdoptRef(new AudioParamHandler(context, param_type, default_value, rate, rate_mode, min_value, max_value));
    }
    ...
    {
        return static_cast<float>(default_value_);
    }
    ...
    {
        bool has_values = timeline_.HasValues(
            destination_handler_->CurrentSampleFrame(), destination_handler_->SampleRate(), GetDeferredTaskHandler().RenderQuantumFrames());
    ...
    {
        return automation_rate_ == kAudio;
    }
    ...
    base::Lock& RateLock() const
    {
        return rate_lock_;
    }

private:
    AudioParamHandler(BaseAudioContext&, AudioParamType, double default_value, AutomationRate rate, AutomationRateMode rate_mode, float min, float max);

    // sampleAccurate corresponds to a-rate (audio rate) vs. k-rate in the Web
    // Audio specification.
    void CalculateFinalValues(float* values, unsigned number_of_values, bool sample_accurate);
