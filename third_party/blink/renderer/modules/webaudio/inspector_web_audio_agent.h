...
class MODULES_EXPORT InspectorWebAudioAgent final : public InspectorBaseAgent<protocol::WebAudio::Metainfo> {
public:
    ...
    protocol::Response getRealtimeData(const protocol::WebAudio::GraphObjectId&, std::unique_ptr<ContextRealtimeData>*) override;
    ...
    void DidConnectNodes(AudioNode* source_node, AudioNode* destination_node, int32_t source_output_index = 0, int32_t destination_input_index = 0);
    void DidDisconnectNodes(
        AudioNode* source_node, AudioNode* destination_node = nullptr, int32_t source_output_index = 0, int32_t destination_input_index = 0);
    void DidConnectNodeParam(AudioNode* source_node, AudioParam* destination_param, int32_t source_output_index = 0);
    void DidDisconnectNodeParam(AudioNode* source_node, AudioParam* destination_param, int32_t source_output_index = 0);

    void Trace(Visitor*) const override;

private:
    std::unique_ptr<protocol::WebAudio::BaseAudioContext> BuildProtocolContext(BaseAudioContext*);
