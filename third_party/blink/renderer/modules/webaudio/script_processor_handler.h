...
public:
    static scoped_refptr<ScriptProcessorHandler> Create(AudioNode&, float sample_rate, uint32_t buffer_size, uint32_t number_of_input_channels,
...
private:
    ScriptProcessorHandler(AudioNode&, float sample_rate, uint32_t buffer_size, uint32_t number_of_input_channels, uint32_t number_of_output_channels,
        const HeapVector<Member<AudioBuffer>>& input_buffers, const HeapVector<Member<AudioBuffer>>& output_buffers);
    ...
    // Double buffering
    uint32_t DoubleBufferIndex() const
    {
        return double_buffer_index_;
    }
    void SwapBuffers()
    {
        double_buffer_index_ = 1 - double_buffer_index_;
    }
    uint32_t double_buffer_index_ = 0;

    mutable base::Lock buffer_lock_;
    WTF::Vector<std::unique_ptr<SharedAudioBuffer>> shared_input_buffers_ GUARDED_BY(buffer_lock_);
    WTF::Vector<std::unique_ptr<SharedAudioBuffer>> shared_output_buffers_ GUARDED_BY(buffer_lock_);
