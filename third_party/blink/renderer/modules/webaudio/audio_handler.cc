...
#if DEBUG_AUDIONODE_REFERENCES
    fprintf(stderr, "[%16p]: %16p: %2d: AudioHandler::AudioHandler() %d [%d] total: %u\n", Context(), this, GetNodeType(), connection_ref_count_,
        node_count_[GetNodeType()], InstanceCounters::CounterValue(InstanceCounters::kAudioHandlerCounter));
#endif
...
AudioHandler::~AudioHandler()
{
    ...
    fprintf(stderr,
        ...
        "%u\n",
        Context(), this, GetNodeType(), connection_ref_count_, node_count_[GetNodeType()],
...
void AudioHandler::AddOutput(unsigned number_of_channels)
{
    ...
    outputs_.push_back(std::make_unique<AudioNodeOutput>(this, number_of_channels));
...
void AudioHandler::SetChannelCount(unsigned channel_count, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    DeferredTaskHandler::GraphAutoLocker locker(Context());

    if (channel_count > 0 && channel_count <= BaseAudioContext::MaxNumberOfChannels()) {
...
void AudioHandler::SetChannelCountMode(V8ChannelCountMode::Enum mode, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    DeferredTaskHandler::GraphAutoLocker locker(Context());
...
void AudioHandler::SetChannelInterpretation(V8ChannelInterpretation::Enum interpretation, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    DeferredTaskHandler::GraphAutoLocker locker(Context());
...
void AudioHandler::ProcessIfNecessary(uint32_t frames_to_process)
{
    ...
    TRACE_EVENT2(TRACE_DISABLED_BY_DEFAULT("webaudio.audionode"), "AudioHandler::ProcessIfNecessary", "this", reinterpret_cast<void*>(this), "node type",
        NodeTypeName().Ascii());
    ...
    if (last_processing_time_ != current_time) {
        ...
        if (!is_processing_) {
            SendLogMessage(__func__, String::Format("=> (processing is alive [frames=%u])", frames_to_process));
            is_processing_ = true;
        }
    }
}
...
{
    return last_non_silent_time_ + LatencyTime() + TailTime() < Context()->currentTime();
}
...
void AudioHandler::EnableOutputsIfNecessary()
{
    ...
    fprintf(stderr,
        ...
        "%d output size %u\n",
        Context(), this, GetNodeType(), is_disabled_, connection_ref_count_, outputs_.size());
#endif
...
void AudioHandler::DisableOutputsIfNecessary()
{
    ...
    fprintf(stderr,
        ...
        " tail %d\n",
        Context(), this, GetNodeType(), is_disabled_, connection_ref_count_, RequiresTailProcessing());
#endif
...
#if DEBUG_AUDIONODE_REFERENCES
    fprintf(stderr, "[%16p]: %16p: %2d: AudioHandler::MakeConnection   %3d [%3d] @%.15g\n", Context(), this, GetNodeType(), connection_ref_count_,
...
void AudioHandler::BreakConnectionWithLock()
{
    ...
    fprintf(stderr,
        ...
        "@%.15g\n",
        Context(), this, GetNodeType(), connection_ref_count_, node_count_[GetNodeType()], Context()->currentTime());
#endif
...
{
    fprintf(stderr, "[%16p]: %16p: %2d: %s %d @%.15g flag=%d", Context(), this, GetNodeType(), note, connection_ref_count_, Context()->currentTime(), flag);
    ...
    if (Context()->IsAudioThread()) {
        fprintf(stderr, ", tail=%.15g + %.15g, last=%.15g\n", TailTime(), LatencyTime(), last_non_silent_time_);
    }
...
void AudioHandler::SendLogMessage(const char* const function_name, const String& message)
{
    WebRtcLogMessage(String::Format(
        "[WA]AH::%s %s [type=%s, this=0x%" PRIXPTR "]", function_name, message.Utf8().c_str(), NodeTypeName().Utf8().c_str(), reinterpret_cast<uintptr_t>(this))
                         .Utf8());
}
