...
float DoInterpolation(double virtual_read_index, float incr, unsigned read_index_mask, float table_interpolation_factor, const float* lower_wave_data,
    const float* higher_wave_data)
{
    ...
    if (incr >= OscillatorHandler::kInterpolate2Point) {
        ...
        double interpolation_factor = static_cast<float>(virtual_read_index) - read_index_0;
        sample_higher = (1 - interpolation_factor) * sample1_higher + interpolation_factor * sample2_higher;
        sample_lower = (1 - interpolation_factor) * sample1_lower + interpolation_factor * sample2_lower;
    ...
    float sample = (1 - table_interpolation_factor) * sample_higher + table_interpolation_factor * sample_lower;
    return sample;
}
...
OscillatorHandler::OscillatorHandler(
    AudioNode& node, float sample_rate, const String& oscillator_type, PeriodicWaveImpl* wave_table, AudioParamHandler& frequency, AudioParamHandler& detune)
...
scoped_refptr<OscillatorHandler> OscillatorHandler::Create(
    AudioNode& node, float sample_rate, const String& oscillator_type, PeriodicWaveImpl* wave_table, AudioParamHandler& frequency, AudioParamHandler& detune)
{
    return base::AdoptRef(new OscillatorHandler(node, sample_rate, oscillator_type, wave_table, frequency, detune));
}
...
void OscillatorHandler::SetType(V8OscillatorType::Enum type, ExceptionState& exception_state)
{
    switch (type) {
...
bool OscillatorHandler::CalculateSampleAccuratePhaseIncrements(uint32_t frames_to_process)
{
    ...
    if (frequency_->HasSampleAccurateValues() && frequency_->IsAudioRate()) {
        ...
        frequency_->CalculateSampleAccurateValues(phase_increments, frames_to_process);
    } else {
    ...
    if (detune_->HasSampleAccurateValues() && detune_->IsAudioRate()) {
        ...
        float* detune_values = has_frequency_changes ? detune_values_.Data() : phase_increments;
        detune_->CalculateSampleAccurateValues(detune_values, frames_to_process);

        // Convert from cents to rate scalar.
        float k = 1.0 / 1200;
        vector_math::Vsmul(detune_values, 1, &k, detune_values, 1, frames_to_process);
        ...
        if (has_frequency_changes) {
            ...
            vector_math::Vmul(detune_values, 1, phase_increments, 1, phase_increments, 1, frames_to_process);
        }
    } else {
    ...
    if (has_sample_accurate_values) {
        ClampFrequency(phase_increments, frames_to_process, Context()->sampleRate() / 2);
        // Convert from frequency to wavetable increment.
        vector_math::Vsmul(phase_increments, 1, &final_scale, phase_increments, 1, frames_to_process);
    }
...
std::tuple<int, double> OscillatorHandler::ProcessKRateVector(int n, float* dest_p, double virtual_read_index, float frequency, float rate_scale) const
{
...
#if !(defined(ARCH_CPU_X86_FAMILY) || defined(CPU_ARM_NEON))
double OscillatorHandler::ProcessARateVectorKernel(float* dest_p, double virtual_read_index, const float* phase_increments, unsigned periodic_wave_size,
    const float* const lower_wave_data[4], const float* const higher_wave_data[4], const float table_interpolation_factor[4]) const
{
    ...
    for (int m = 0; m < 4; ++m) {
        ...
        float sample_higher = sample1_higher + interpolation_factor * (sample2_higher - sample1_higher);
        float sample_lower = sample1_lower + interpolation_factor * (sample2_lower - sample1_lower);

        // Then interpolate between the two tables.
        float sample = sample_higher + table_interpolation_factor[m] * (sample_lower - sample_higher);
        ...
        virtual_read_index -= floor(virtual_read_index * inv_periodic_wave_size) * periodic_wave_size;
    }
...
double OscillatorHandler::ProcessKRateScalar(int start, int n, float* dest_p, double virtual_read_index, float frequency, float rate_scale) const
{
    ...
    periodic_wave_->WaveDataForFundamentalFrequency(frequency, lower_wave_data, higher_wave_data, table_interpolation_factor);
    ...
    for (int k = start; k < n; ++k) {
        ...
        // valid range
        const unsigned read_index_0 = static_cast<unsigned>(virtual_read_index) & read_index_mask;
        ...
        // Linearly interpolate within each table (lower and higher).
        const float interpolation_factor = static_cast<float>(virtual_read_index) - read_index_0;
        const float sample_higher = sample1_higher + interpolation_factor * (sample2_higher - sample1_higher);
        const float sample_lower = sample1_lower + interpolation_factor * (sample2_lower - sample1_lower);

        // Then interpolate between the two tables.
        const float sample = sample_higher + table_interpolation_factor * (sample_lower - sample_higher);
...
double OscillatorHandler::ProcessKRate(int n, float* dest_p, double virtual_read_index) const
{
    const unsigned periodic_wave_size = periodic_wave_->PeriodicWaveSize();
    const double inv_periodic_wave_size = 1.0 / periodic_wave_size;
    const unsigned read_index_mask = periodic_wave_size - 1;
    ...
    if (incr >= kInterpolate2Point) {
        ...
        std::tie(k, v_index) = ProcessKRateVector(n, dest_p, v_index, frequency, rate_scale);

        if (k < n) {
            // In typical cases, this won't be run because the number of frames is 128
            // so the vector version will process all the samples.
            v_index = ProcessKRateScalar(k, n, dest_p, v_index, frequency, rate_scale);
        }
        ...
        for (int k = 0; k < n; ++k) {
            float sample = DoInterpolation(virtual_read_index, fabs(incr), read_index_mask, table_interpolation_factor, lower_wave_data, higher_wave_data);
...
std::tuple<int, double> OscillatorHandler::ProcessARateVector(int n, float* destination, double virtual_read_index, const float* phase_increments) const
{
    ...
    for (int loop = 0; loop < n_loops; ++loop, k += 4) {
        ...
        for (int m = 0; m < 4; ++m) {
            ...
            is_big_increment = is_big_increment && (fabs(phase_incr) >= kInterpolate2Point);
        ...
        if (is_big_increment) {
            virtual_read_index = ProcessARateVectorKernel(
                destination + k, virtual_read_index, phase_increments + k, periodic_wave_size, lower_wave_data, higher_wave_data, table_interpolation_factor);
        } else {
            for (int m = 0; m < 4; ++m) {
                float sample = DoInterpolation(
                    virtual_read_index, fabs(phase_increments[k + m]), read_index_mask, table_interpolation_factor[m], lower_wave_data[m], higher_wave_data[m]);
...
double OscillatorHandler::ProcessARateScalar(int k, int n, float* destination, double virtual_read_index, const float* phase_increments) const
{
    float rate_scale = periodic_wave_->RateScale();
    float inv_rate_scale = 1 / rate_scale;
    unsigned periodic_wave_size = periodic_wave_->PeriodicWaveSize();
...
double OscillatorHandler::ProcessARate(int n, float* destination, double virtual_read_index, float* phase_increments) const
{
    int frames_processed = 0;

    std::tie(frames_processed, virtual_read_index) = ProcessARateVector(n, destination, virtual_read_index, phase_increments);

    virtual_read_index = ProcessARateScalar(frames_processed, n, destination, virtual_read_index, phase_increments);
...
{
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("webaudio.audionode"), "OscillatorHandler::Process", "this", reinterpret_cast<void*>(this), "type", GetType());
    ...
    std::tie(quantum_frame_offset, non_silent_frames_to_process, start_frame_offset) = UpdateSchedulingInfo(frames_to_process, output_bus);
    ...
    float rate_scale = periodic_wave_->RateScale();
    bool has_sample_accurate_values = CalculateSampleAccuratePhaseIncrements(frames_to_process);
    ...
    if (has_sample_accurate_values) {
        virtual_read_index = ProcessARate(n, dest_p, virtual_read_index, phase_increments);
    } else {
...
void OscillatorHandler::HandleStoppableSourceNode()
{
    ...
    // to stop it.)
    if (end_time_ != kUnknownTime && IsPlayingOrScheduled() && now >= end_time_ + kExtraStopFrames / Context()->sampleRate()) {
        Finish();
    }
}
