...
AnalyserHandler::AnalyserHandler(AudioNode& node, float sample_rate)
    ...
    , analyser_(node.context()->GetDeferredTaskHandler().RenderQuantumFrames())
{
    AddInput();
...
scoped_refptr<AnalyserHandler> AnalyserHandler::Create(AudioNode& node, float sample_rate)
{
...
void AnalyserHandler::SetFftSize(unsigned size, ExceptionState& exception_state)
{
    if (!analyser_.SetFftSize(size)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError,
            (size < RealtimeAnalyser::kMinFFTSize || size > RealtimeAnalyser::kMaxFFTSize)
                ? ExceptionMessages::IndexOutsideRange("FFT size", size, RealtimeAnalyser::kMinFFTSize, ExceptionMessages::kInclusiveBound,
                    RealtimeAnalyser::kMaxFFTSize, ExceptionMessages::kInclusiveBound)
...
void AnalyserHandler::SetMinDecibels(double k, ExceptionState& exception_state)
{
    if (k < MaxDecibels()) {
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError, ExceptionMessages::IndexExceedsMaximumBound("minDecibels", k, MaxDecibels()));
    }
}
...
void AnalyserHandler::SetMaxDecibels(double k, ExceptionState& exception_state)
{
    if (k > MinDecibels()) {
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError, ExceptionMessages::IndexExceedsMinimumBound("maxDecibels", k, MinDecibels()));
    }
}

void AnalyserHandler::SetMinMaxDecibels(double min_decibels, double max_decibels, ExceptionState& exception_state)
{
    if (min_decibels >= max_decibels) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError,
            "maxDecibels (" + String::Number(max_decibels) + ") must be greater than or equal to minDecibels " + "( " + String::Number(min_decibels) + ").");
        return;
    }
...
void AnalyserHandler::SetSmoothingTimeConstant(double k, ExceptionState& exception_state)
{
    if (k >= 0 && k <= 1) {
        analyser_.SetSmoothingTimeConstant(k);
    } else {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError,
            ExceptionMessages::IndexOutsideRange("smoothing value", k, 0.0, ExceptionMessages::kInclusiveBound, 1.0, ExceptionMessages::kInclusiveBound));
    }
}
...
{
    return RealtimeAnalyser::kMaxFFTSize / static_cast<double>(Context()->sampleRate());
}
