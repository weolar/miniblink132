...
OfflineAudioDestinationHandler::OfflineAudioDestinationHandler(AudioNode& node, unsigned number_of_channels, uint32_t frames_to_process, float sample_rate)
    : AudioDestinationHandler(node)
    , frames_to_process_(frames_to_process)
    , number_of_channels_(number_of_channels)
    , sample_rate_(sample_rate)
    , main_thread_task_runner_(Context()->GetExecutionContext()->GetTaskRunner(TaskType::kInternalMedia))
{
...
scoped_refptr<OfflineAudioDestinationHandler> OfflineAudioDestinationHandler::Create(
    AudioNode& node, unsigned number_of_channels, uint32_t frames_to_process, float sample_rate)
{
    return base::AdoptRef(new OfflineAudioDestinationHandler(node, number_of_channels, frames_to_process, sample_rate));
}
...
void OfflineAudioDestinationHandler::StartRendering()
{
    ...
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("webaudio.audionode"), "OfflineAudioDestinationHandler::StartRendering", "this", reinterpret_cast<void*>(this));

    // Rendering was not started. Starting now.
    if (!is_rendering_started_) {
        is_rendering_started_ = true;
        PostCrossThreadTask(
            *render_thread_task_runner_, FROM_HERE, CrossThreadBindOnce(&OfflineAudioDestinationHandler::StartOfflineRendering, WrapRefCounted(this)));
        return;
    }
    ...
    PostCrossThreadTask(*render_thread_task_runner_, FROM_HERE, CrossThreadBindOnce(&OfflineAudioDestinationHandler::DoOfflineRendering, WrapRefCounted(this)));
}
...
void OfflineAudioDestinationHandler::InitializeOfflineRenderThread(AudioBuffer* render_target)
{
    DCHECK(IsMainThread());

    shared_render_target_ = render_target->CreateSharedAudioBuffer();
    render_bus_ = AudioBus::Create(render_target->numberOfChannels(), GetDeferredTaskHandler().RenderQuantumFrames());
    DCHECK(render_bus_);
...
void OfflineAudioDestinationHandler::StartOfflineRendering()
{
    ...
    DCHECK_EQ(render_bus_->NumberOfChannels(), shared_render_target_->numberOfChannels());
    DCHECK_GE(render_bus_->length(), GetDeferredTaskHandler().RenderQuantumFrames());
...
{
    DCHECK(!IsMainThread());
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("webaudio.audionode"), "OfflineAudioDestinationHandler::DoOfflineRendering", "this", reinterpret_cast<void*>(this));
    ...
    for (unsigned i = 0; i < number_of_channels; ++i) {
        destinations.push_back(static_cast<float*>(shared_render_target_->channels()[i].Data()));
    }
    ...
    while (frames_to_process_ > 0) {
        ...
        if (RenderIfNotSuspended(nullptr, render_bus_.get(), GetDeferredTaskHandler().RenderQuantumFrames())) {
            return;
        }

        uint32_t frames_available_to_copy = std::min(frames_to_process_, GetDeferredTaskHandler().RenderQuantumFrames());
        ...
        for (unsigned channel_index = 0; channel_index < number_of_channels; ++channel_index) {
            ...
            memcpy(destinations[channel_index] + frames_processed_, source, sizeof(float) * frames_available_to_copy);
        }
...
void OfflineAudioDestinationHandler::SuspendOfflineRendering()
{
    ...
    PostCrossThreadTask(*main_thread_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&OfflineAudioDestinationHandler::NotifySuspend, GetWeakPtr(), Context()->CurrentSampleFrame()));
}
...
void OfflineAudioDestinationHandler::FinishOfflineRendering()
{
    ...
    PostCrossThreadTask(*main_thread_task_runner_, FROM_HERE, CrossThreadBindOnce(&OfflineAudioDestinationHandler::NotifyComplete, GetWeakPtr()));
}
...
bool OfflineAudioDestinationHandler::RenderIfNotSuspended(AudioBus* source_bus, AudioBus* destination_bus, uint32_t number_of_frames)
{
    ...
    if (Context()->HandlePreRenderTasks(number_of_frames, nullptr, nullptr, base::TimeDelta(), media::AudioGlitchInfo())) {
    ...
    if (audio_worklet && audio_worklet->IsReady()) {
        if (!render_thread_) {
            ...
            if (!render_thread_task_runner_) {
                render_thread_task_runner_ = audio_worklet->GetMessagingProxy()->GetBackingWorkerThread()->GetTaskRunner(TaskType::kMiscPlatformAPI);
            }
        } else {
        ...
        if (!render_thread_) {
            ...
            render_thread_ = NonMainThread::CreateThread(ThreadCreationParams(ThreadType::kOfflineAudioRenderThread));
