...
class MockMediaDevicesDispatcherHost final : public mojom::blink::MediaDevicesDispatcherHost {
public:
    ...
    {
        receivers_.Add(this, mojo::PendingReceiver<mojom::blink::MediaDevicesDispatcherHost>(std::move(handle)));
    }
    ...
    {
        Vector<Vector<WebMediaDeviceInfo>> enumeration(static_cast<size_t>(blink::mojom::blink::MediaDeviceType::kNumMediaDeviceTypes));
        ...
        if (request_audio_output) {
            wtf_size_t index = static_cast<wtf_size_t>(blink::mojom::blink::MediaDeviceType::kMediaAudioOutput);
            enumeration[index] = enumeration_[index];
        }
        std::move(callback).Run(std::move(enumeration), std::move(video_input_capabilities), std::move(audio_input_capabilities));
    }
    ...
    void GetAllVideoInputDeviceFormats(const String&, GetAllVideoInputDeviceFormatsCallback) override
    {
    }

    void GetAvailableVideoInputDeviceFormats(const String&, GetAvailableVideoInputDeviceFormatsCallback) override
    {
    }
    ...
    void AddMediaDevicesListener(bool subscribe_audio_input, bool subscribe_video_input, bool subscribe_audio_output,
        mojo::PendingRemote<mojom::blink::MediaDevicesListener> listener) override
    {
        listeners_.Add(std::move(listener));
    }

    void SetCaptureHandleConfig(mojom::blink::CaptureHandleConfigPtr config) override
    {
    }
    ...
    void ProduceSubCaptureTargetId(SubCaptureTarget::Type type, ProduceSubCaptureTargetIdCallback callback) override
    {
    }
#endif
...
private:
    ...
    Vector<Vector<WebMediaDeviceInfo>> enumeration_ { static_cast<size_t>(blink::mojom::blink::MediaDeviceType::kNumMediaDeviceTypes) };
};

class MockWebAudioDeviceForAudioContext : public WebAudioDevice {
public:
    explicit MockWebAudioDeviceForAudioContext(double sample_rate, int frames_per_buffer)
    ...
    void Pause() override
    {
        web_audio_device_paused_ = true;
    }
    void Resume() override
    {
        web_audio_device_paused_ = false;
    }
...
public:
    std::unique_ptr<WebAudioDevice> CreateAudioDevice(const WebAudioSinkDescriptor& sink_descriptor, unsigned number_of_output_channels,
        const WebAudioLatencyHint& latency_hint, media::AudioRendererSink::RenderCallback*) override
    {
        ...
        case WebAudioLatencyHint::kCategoryExact:
            buffer_size = ClampTo(
                latency_hint.Seconds() * AudioHardwareSampleRate(), static_cast<double>(AudioHardwareBufferSize()), static_cast<double>(playback_size));
            break;
        default:
            NOTREACHED();
        }

        return std::make_unique<MockWebAudioDeviceForAudioContext>(AudioHardwareSampleRate(), buffer_size);
    }
...
{
    return PeerConnectionDependencyFactory::From(*execution_context).GetWebRtcAudioDevice()->GetOutputDeviceForAecForTesting();
}
...
protected:
    AudioContextTest()
    {
        mock_media_devices_dispatcher_host_ = std::make_unique<MockMediaDevicesDispatcherHost>();
    }
    ...
    void SetUp() override
    {
        ...
        GetFrame().DomWindow()->GetBrowserInterfaceBroker().SetBinderForTesting(mojom::blink::MediaDevicesDispatcherHost::Name_,
            WTF::BindRepeating(&MockMediaDevicesDispatcherHost::BindRequest, WTF::Unretained(mock_media_devices_dispatcher_host_.get())));
    }
    ...
    void SetContextState(AudioContext* audio_context, V8AudioContextState::Enum state)
    {
    ...
    void VerifyPlayoutStats(AudioPlayoutStats* playout_stats, ScriptState* script_state, int total_processed_frames,
        const media::AudioGlitchInfo& total_glitches, base::TimeDelta average_delay, base::TimeDelta min_delay, base::TimeDelta max_delay, int source_line)
    {
        EXPECT_EQ(playout_stats->fallbackFramesEvents(script_state), total_glitches.count) << " LINE " << source_line;
        EXPECT_FLOAT_EQ(playout_stats->fallbackFramesDuration(script_state), total_glitches.duration.InMillisecondsF()) << " LINE " << source_line;
        EXPECT_EQ(playout_stats->averageLatency(script_state), average_delay.InMillisecondsF()) << " LINE " << source_line;
        EXPECT_EQ(playout_stats->minimumLatency(script_state), min_delay.InMillisecondsF()) << " LINE " << source_line;
        EXPECT_EQ(playout_stats->maximumLatency(script_state), max_delay.InMillisecondsF()) << " LINE " << source_line;
        EXPECT_NEAR(playout_stats->totalFramesDuration(script_state),
            (media::AudioTimestampHelper::FramesToTime(total_processed_frames, platform()->AudioHardwareSampleRate()) + total_glitches.duration)
                .InMillisecondsF(),
            0.01)
            << " LINE " << source_line;
    }
...
private:
    ...
    std::unique_ptr<MockMediaDevicesDispatcherHost> mock_media_devices_dispatcher_host_;
};
...
TEST_F(AudioContextTest, AudioContextOptions_WebAudioLatencyHint)
{
    ...
    AudioContext* interactive_context = AudioContext::Create(GetFrame().DomWindow(), interactive_options, ASSERT_NO_EXCEPTION);

    AudioContextOptions* balanced_options = AudioContextOptions::Create();
    balanced_options->setLatencyHint(
        MakeGarbageCollected<V8UnionAudioContextLatencyCategoryOrDouble>(V8AudioContextLatencyCategory(V8AudioContextLatencyCategory::Enum::kBalanced)));
    AudioContext* balanced_context = AudioContext::Create(GetFrame().DomWindow(), balanced_options, ASSERT_NO_EXCEPTION);
    EXPECT_GT(balanced_context->baseLatency(), interactive_context->baseLatency());

    AudioContextOptions* playback_options = AudioContextOptions::Create();
    playback_options->setLatencyHint(
        MakeGarbageCollected<V8UnionAudioContextLatencyCategoryOrDouble>(V8AudioContextLatencyCategory(V8AudioContextLatencyCategory::Enum::kPlayback)));
    AudioContext* playback_context = AudioContext::Create(GetFrame().DomWindow(), playback_options, ASSERT_NO_EXCEPTION);
    ...
    exact_too_small_options->setLatencyHint(MakeGarbageCollected<V8UnionAudioContextLatencyCategoryOrDouble>(interactive_context->baseLatency() / 2));
    AudioContext* exact_too_small_context = AudioContext::Create(GetFrame().DomWindow(), exact_too_small_options, ASSERT_NO_EXCEPTION);
    EXPECT_EQ(exact_too_small_context->baseLatency(), interactive_context->baseLatency());

    const double exact_latency_sec = (interactive_context->baseLatency() + playback_context->baseLatency()) / 2;
    AudioContextOptions* exact_ok_options = AudioContextOptions::Create();
    exact_ok_options->setLatencyHint(MakeGarbageCollected<V8UnionAudioContextLatencyCategoryOrDouble>(exact_latency_sec));
    AudioContext* exact_ok_context = AudioContext::Create(GetFrame().DomWindow(), exact_ok_options, ASSERT_NO_EXCEPTION);
    ...
    exact_too_big_options->setLatencyHint(MakeGarbageCollected<V8UnionAudioContextLatencyCategoryOrDouble>(playback_context->baseLatency() * 2));
    AudioContext* exact_too_big_context = AudioContext::Create(GetFrame().DomWindow(), exact_too_big_options, ASSERT_NO_EXCEPTION);
    EXPECT_EQ(exact_too_big_context->baseLatency(), playback_context->baseLatency());
}

TEST_F(AudioContextTest, AudioContextAudibility_ServiceUnbind)
{
    AudioContextOptions* options = AudioContextOptions::Create();
    AudioContext* audio_context = AudioContext::Create(GetFrame().DomWindow(), options, ASSERT_NO_EXCEPTION);
...
TEST_F(AudioContextTest, ExecutionContextPaused)
{
    ...
    GetFrame().DomWindow()->SetLifecycleState(mojom::FrameLifecycleState::kFrozen);
    EXPECT_TRUE(web_audio_device_paused_);
    GetFrame().DomWindow()->SetLifecycleState(mojom::FrameLifecycleState::kRunning);
    EXPECT_FALSE(web_audio_device_paused_);
}
...
public:
    explicit ContextRenderer(AudioContext* context)
        : context_(context)
        , audio_thread_(NonMainThread::CreateThread(ThreadCreationParams(ThreadType::kRealtimeAudioWorkletThread)))
    {
    }
    ...
    {
        PostCrossThreadTask(
            *audio_thread_->GetTaskRunner(), FROM_HERE, CrossThreadBindOnce(&ContextRenderer::SetContextAudioThread, WrapCrossThreadWeakPersistent(this)));
        event_.Wait();
    }

    void Render(uint32_t frames_to_process, base::TimeDelta playout_delay, const media::AudioGlitchInfo& glitch_info)
    {
        PostCrossThreadTask(*audio_thread_->GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(&ContextRenderer::RenderOnAudioThread, WrapCrossThreadWeakPersistent(this), frames_to_process, playout_delay, glitch_info));
        event_.Wait();
    }
...
private:
    ...
    {
        static_cast<AudioContext*>(context_)->GetDeferredTaskHandler().SetAudioThreadToCurrentThread();
        event_.Signal();
    }

    void RenderOnAudioThread(uint32_t frames_to_process, base::TimeDelta playout_delay, const media::AudioGlitchInfo& glitch_info)
    {
        const AudioIOPosition output_position { 0, 0, 0 };
        const AudioCallbackMetric audio_callback_metric;
        static_cast<AudioContext*>(context_)->HandlePreRenderTasks(frames_to_process, &output_position, &audio_callback_metric, playout_delay, glitch_info);
        event_.Signal();
    }
...
{
    blink::WebRuntimeFeatures::EnableFeatureFromString("AudioContextPlayoutStats", true);
    AudioContextOptions* options = AudioContextOptions::Create();
    AudioContext* audio_context = AudioContext::Create(GetFrame().DomWindow(), options, ASSERT_NO_EXCEPTION);

    const int kNumberOfRenderEvents = 9;
    uint32_t frames_to_process[kNumberOfRenderEvents] { 100, 200, 300, 10, 500, 120, 120, 30, 100 };
    base::TimeDelta playout_delay[kNumberOfRenderEvents] { base::Milliseconds(10), base::Milliseconds(20), base::Milliseconds(300), base::Milliseconds(107),
        base::Milliseconds(17), base::Milliseconds(3), base::Milliseconds(500), base::Milliseconds(10), base::Milliseconds(112) };
    const media::AudioGlitchInfo glitch_info[kNumberOfRenderEvents] { { .duration = base::Milliseconds(5), .count = 1 }, {},
        { .duration = base::Milliseconds(60), .count = 3 }, {}, { .duration = base::Milliseconds(600), .count = 20 },
        { .duration = base::Milliseconds(200), .count = 5 }, {}, { .duration = base::Milliseconds(2), .count = 1 },
    ...
    ContextRenderer* renderer = MakeGarbageCollected<ContextRenderer>(audio_context);
    renderer->Init();

    // Empty stats in be beginning, all latencies are zero.
    VerifyPlayoutStats(playout_stats, script_state, total_processed_frames, total_glitches, last_delay, last_delay, last_delay, __LINE__);
    ...
    for (; i < 3; ++i) {
        ...
        // Stats updated.
        VerifyPlayoutStats(playout_stats, script_state, total_processed_frames, total_glitches, interval_delay_sum / interval_processed_frames, min_delay,
            max_delay, __LINE__);
    }

    // Same stats, since we are within the same execution cycle.
    VerifyPlayoutStats(
        playout_stats, script_state, total_processed_frames, total_glitches, interval_delay_sum / interval_processed_frames, min_delay, max_delay, __LINE__);
    ...
    const base::TimeDelta observed_average_delay = interval_delay_sum / interval_processed_frames;
    const base::TimeDelta observed_max_delay = max_delay;
    const base::TimeDelta observed_min_delay = min_delay;

    VerifyPlayoutStats(playout_stats, script_state, observed_total_processed_frames, observed_total_glitches, observed_average_delay, observed_min_delay,
    ...
    // Still same stats: we are in the same execution cycle.
    VerifyPlayoutStats(
        playout_stats, script_state, reset_total_processed_frames, reset_total_glitches, reset_average_delay, reset_min_delay, reset_max_delay, __LINE__);
...
TEST_F(AudioContextTest, ChannelCountRunning)
{
    ...
    test::ScopedMockedURLLoad scoped_mocked_url_load(KURL(kSecurityOrigin), test::CoreTestDataPath(kTestData));
    frame_test_helpers::WebViewHelper web_view_helper;
    WebViewImpl* web_view_impl = web_view_helper.InitializeAndLoad(kSecurityOrigin);
    ...
    security_context.SetSecurityOrigin(SecurityOrigin::CreateFromString(kSecurityOrigin));

    // Creating an AudioContext should result in the context running and the
    // destination playing.
    AudioContext* context = AudioContext::Create(execution_context, AudioContextOptions::Create(), ASSERT_NO_EXCEPTION);
    EXPECT_EQ(context->ContextState(), V8AudioContextState::Enum::kRunning);
    EXPECT_TRUE(context->GetRealtimeAudioDestinationNode()->GetOwnHandler().get_platform_destination_is_playing_for_testing());

    // Changing the channel count should should result in the same running and
    // playing state.
    context->destination()->setChannelCount(context->destination()->maxChannelCount(), ASSERT_NO_EXCEPTION);
    EXPECT_EQ(context->ContextState(), V8AudioContextState::Enum::kRunning);
    EXPECT_TRUE(context->GetRealtimeAudioDestinationNode()->GetOwnHandler().get_platform_destination_is_playing_for_testing());
}
...
TEST_F(AudioContextTest, ChannelCountSuspended)
{
    ...
    EXPECT_FALSE(context->GetRealtimeAudioDestinationNode()->GetOwnHandler().get_platform_destination_is_playing_for_testing());
...
TEST_F(AudioContextTest, SetSinkIdRunning)
{
    ...
    // and playing state.
    context->setSinkId(script_state, MakeGarbageCollected<V8UnionAudioSinkOptionsOrString>(kFakeAudioOutput1), ASSERT_NO_EXCEPTION);
    FlushMediaDevicesDispatcherHost();
    EXPECT_EQ(context->ContextState(), V8AudioContextState::Enum::kRunning);
    EXPECT_TRUE(context->GetRealtimeAudioDestinationNode()->GetOwnHandler().get_platform_destination_is_playing_for_testing());
}
...
TEST_F(AudioContextTest, SetSinkIdSuspended)
{
    ...
    context->setSinkId(script_state, MakeGarbageCollected<V8UnionAudioSinkOptionsOrString>(kInvalidAudioOutput), ASSERT_NO_EXCEPTION);
    FlushMediaDevicesDispatcherHost();
    EXPECT_EQ(context->ContextState(), V8AudioContextState::Enum::kSuspended);
    EXPECT_FALSE(context->GetRealtimeAudioDestinationNode()->GetOwnHandler().get_platform_destination_is_playing_for_testing());
...
TEST_F(AudioContextTest, AecConstructor)
{
    ...
    options_null->setSinkId(MakeGarbageCollected<V8UnionAudioSinkOptionsOrString>(MakeGarbageCollected<AudioSinkOptions>()));
    ...
    options_a->setSinkId(MakeGarbageCollected<V8UnionAudioSinkOptionsOrString>(kFakeAudioOutput1));
    ...
    options_b->setSinkId(MakeGarbageCollected<V8UnionAudioSinkOptionsOrString>(kFakeAudioOutput2));
    ...
    options_explicit_default->setSinkId(MakeGarbageCollected<V8UnionAudioSinkOptionsOrString>(kDefaultDeviceId));
    AudioContext::Create(execution_context, options_explicit_default, ASSERT_NO_EXCEPTION);
...
TEST_F(AudioContextTest, AecSetSinkIdSuspended)
{
    ...
    const String initial_aec_device = GetAecDevice(execution_context);
    AudioContextOptions* options_empty = AudioContextOptions::Create();
    AudioContext* context_a = AudioContext::Create(execution_context, options_empty, ASSERT_NO_EXCEPTION);
    context_a->suspendContext(script_state, ASSERT_NO_EXCEPTION);
    AudioContext* context_b = AudioContext::Create(execution_context, options_empty, ASSERT_NO_EXCEPTION);
    ...
    context_a->setSinkId(script_state, MakeGarbageCollected<V8UnionAudioSinkOptionsOrString>(kFakeAudioOutput1), ASSERT_NO_EXCEPTION);
    FlushMediaDevicesDispatcherHost();
    EXPECT_EQ(GetAecDevice(execution_context), initial_aec_device);

    // Calling setSinkId on a different suspended context with a valid device ID
    // should not change the AEC device.
    context_b->setSinkId(script_state, MakeGarbageCollected<V8UnionAudioSinkOptionsOrString>(kFakeAudioOutput2), ASSERT_NO_EXCEPTION);
    FlushMediaDevicesDispatcherHost();
    EXPECT_EQ(GetAecDevice(execution_context), initial_aec_device);
...
TEST_F(AudioContextTest, AecSetSinkIdMultiple)
{
    ...
    const String initial_aec_device = GetAecDevice(execution_context);
    AudioContextOptions* options_empty = AudioContextOptions::Create();
    AudioContext* context = AudioContext::Create(execution_context, options_empty, ASSERT_NO_EXCEPTION);
    FlushMediaDevicesDispatcherHost();
    EXPECT_EQ(GetAecDevice(execution_context), initial_aec_device);
    ...
    context->setSinkId(script_state, MakeGarbageCollected<V8UnionAudioSinkOptionsOrString>(kFakeAudioOutput2), ASSERT_NO_EXCEPTION);
    FlushMediaDevicesDispatcherHost();
    EXPECT_EQ(GetAecDevice(execution_context), kFakeAudioOutput2);
}
...
TEST_F(AudioContextTest, AecSetSinkIdAfterConstructor)
{
    ...
    AudioContextOptions* options = AudioContextOptions::Create();
    options->setSinkId(MakeGarbageCollected<V8UnionAudioSinkOptionsOrString>(kFakeAudioOutput1));
    AudioContext* context = AudioContext::Create(execution_context, options, ASSERT_NO_EXCEPTION);
    FlushMediaDevicesDispatcherHost();
    EXPECT_EQ(GetAecDevice(execution_context), kFakeAudioOutput1);
...
class AudioContextInterruptedStateTest : public testing::WithParamInterface<bool>, public AudioContextTest {
public:
    AudioContextInterruptedStateTest()
    {
        if (GetParam()) {
            blink::WebRuntimeFeatures::EnableFeatureFromString("AudioContextInterruptedState", true);
        } else {
            blink::WebRuntimeFeatures::EnableFeatureFromString("AudioContextInterruptedState", false);
        }
    }
    ...
    {
        EXPECT_EQ(audio_context->ContextState(), V8AudioContextState::Enum::kRunning);
        EXPECT_TRUE(audio_context->GetRealtimeAudioDestinationNode()->GetOwnHandler().get_platform_destination_is_playing_for_testing());
    }

    void ExpectAudioContextSuspended(AudioContext* audio_context)
    {
        EXPECT_EQ(audio_context->ContextState(), V8AudioContextState::Enum::kSuspended);
        EXPECT_FALSE(audio_context->GetRealtimeAudioDestinationNode()->GetOwnHandler().get_platform_destination_is_playing_for_testing());
    }

    void ExpectAudioContextInterrupted(AudioContext* audio_context)
    {
        EXPECT_EQ(audio_context->ContextState(), V8AudioContextState::Enum::kInterrupted);
        EXPECT_FALSE(audio_context->GetRealtimeAudioDestinationNode()->GetOwnHandler().get_platform_destination_is_playing_for_testing());
    }
};
...
TEST_P(AudioContextInterruptedStateTest, ResumingSuspendedContextWhileInterrupted)
{
    // If an interruption occurs while the AudioContext is suspended, the context
    // should remain in the suspended state and the platform destination should
    // not start playing.
...
TEST_P(AudioContextInterruptedStateTest, SuspendingRunningContextWhileInterrupted)
{
...
INSTANTIATE_TEST_SUITE_P(AudioContextInterruptedStateTests, AudioContextInterruptedStateTest, testing::Bool());
