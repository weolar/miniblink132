...
public:
    MockAudioProcessor()
        : AudioProcessor(48000, 2, kRenderQuantumFrames)
    {
    }
...
public:
    static scoped_refptr<MockProcessorHandler> Create(AudioNode& node, float sample_rate)
    {
...
private:
    MockProcessorHandler(AudioNode& node, float sample_rate)
        : AudioBasicProcessorHandler(AudioHandler::kNodeTypeWaveShaper, node, sample_rate, std::make_unique<MockAudioProcessor>())
    {
        Initialize();
    }
};
...
public:
    explicit MockProcessorNode(BaseAudioContext& context)
        : AudioNode(context)
    {
...
TEST(AudioBasicProcessorHandlerTest, ProcessorFinalization)
{
    ...
    OfflineAudioContext* context = OfflineAudioContext::Create(page->GetFrame().DomWindow(), 2, 1, 48000, ASSERT_NO_EXCEPTION);
