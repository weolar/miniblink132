...
scoped_refptr<ConvolverHandler> ConvolverHandler::Create(AudioNode& node, float sample_rate)
{
...
void ConvolverHandler::SetBuffer(AudioBuffer* buffer, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    ...
    if (buffer->sampleRate() != Context()->sampleRate()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError,
            "The buffer sample rate of " + String::Number(buffer->sampleRate()) + " does not match the context rate of "
    ...
    // (see Reverb class).
    bool is_channel_count_good = number_of_channels == 1 || number_of_channels == 2 || number_of_channels == 4;

    if (!is_channel_count_good) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kNotSupportedError, "The buffer must have 1, 2, or 4 channels, not " + String::Number(number_of_channels));
        return;
    }

    {
        // Get some statistics on the size of the impulse response.
        UMA_HISTOGRAM_LONG_TIMES("WebAudio.ConvolverNode.ImpulseResponseLength", base::Seconds(buffer->duration()));
    }
    ...
    scoped_refptr<AudioBus> buffer_bus = AudioBus::Create(number_of_channels, buffer_length, false);
    ...
    for (unsigned i = 0; i < number_of_channels; ++i) {
        buffer_bus->SetChannelMemory(i, buffer->getChannelData(i)->Data(), buffer_length);
    }
    ...
    std::unique_ptr<Reverb> reverb = std::make_unique<Reverb>(
        buffer_bus.get(), GetDeferredTaskHandler().RenderQuantumFrames(), kMaxFftSize, Context() && Context()->HasRealtimeConstraint(), normalize_);

    {
        ...
        if (buffer) {
            ...
            Output(0).SetNumberOfChannels(ComputeNumberOfOutputChannels(Input(0).NumberOfChannels(), shared_buffer_->numberOfChannels()));
        }
    }
}
...
double ConvolverHandler::TailTime() const
{
    base::AutoTryLock try_locker(process_lock_);
    if (try_locker.is_acquired()) {
        return reverb_ ? reverb_->ImpulseResponseLength() / static_cast<double>(Context()->sampleRate()) : 0;
    }
...
double ConvolverHandler::LatencyTime() const
{
    base::AutoTryLock try_locker(process_lock_);
    if (try_locker.is_acquired()) {
        return reverb_ ? reverb_->LatencyFrames() / static_cast<double>(Context()->sampleRate()) : 0;
    }
    // Since we don't want to block the Audio Device thread, we return a large
    // value instead of trying to acquire the lock.
    return std::numeric_limits<double>::infinity();
...
unsigned ConvolverHandler::ComputeNumberOfOutputChannels(unsigned input_channels, unsigned response_channels) const
{
...
void ConvolverHandler::SetChannelCount(unsigned channel_count, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    ...
    if (channel_count == 1 || channel_count == 2) {
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError,
            ExceptionMessages::IndexOutsideRange<uint32_t>(
                "channelCount", channel_count, 1, ExceptionMessages::kInclusiveBound, 2, ExceptionMessages::kInclusiveBound));
    }
}

void ConvolverHandler::SetChannelCountMode(V8ChannelCountMode::Enum mode, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    DeferredTaskHandler::GraphAutoLocker locker(Context());
    ...
    if (mode == V8ChannelCountMode::Enum::kMax) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "ConvolverNode: channelCountMode cannot be changed to 'max'");
        new_channel_count_mode_ = old_mode;
    } else if (mode == V8ChannelCountMode::Enum::kExplicit || mode == V8ChannelCountMode::Enum::kClampedMax) {
...
void ConvolverHandler::CheckNumberOfChannelsForInput(AudioNodeInput* input)
{
    ...
    if (has_shared_buffer || !lock_successfully_acquired) {
        unsigned number_of_output_channels = ComputeNumberOfOutputChannels(input->NumberOfChannels(), number_of_channels);

        if (IsInitialized() && number_of_output_channels != Output(0).NumberOfChannels()) {
