...
MediaStreamAudioDestinationHandler::MediaStreamAudioDestinationHandler(AudioNode& node, uint32_t number_of_channels)
    : AudioHandler(kNodeTypeMediaStreamAudioDestination, node, node.context()->sampleRate())
    , source_(static_cast<MediaStreamAudioDestinationNode&>(node).source())
    , mix_bus_(AudioBus::Create(number_of_channels, GetDeferredTaskHandler().RenderQuantumFrames()))
{
    AddInput();
    SendLogMessage(__func__, "");
    source_.Lock()->SetAudioFormat(static_cast<int>(number_of_channels), node.context()->sampleRate());
    SetInternalChannelCountMode(V8ChannelCountMode::Enum::kExplicit);
    Initialize();
}

scoped_refptr<MediaStreamAudioDestinationHandler> MediaStreamAudioDestinationHandler::Create(AudioNode& node, uint32_t number_of_channels)
{
    return base::AdoptRef(new MediaStreamAudioDestinationHandler(node, number_of_channels));
}
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("webaudio.audionode"), "MediaStreamAudioDestinationHandler::Process");
    ...
    if (try_locker.is_acquired()) {
        ...
        if (count != mix_bus_->NumberOfChannels()) {
            mix_bus_ = AudioBus::Create(count, GetDeferredTaskHandler().RenderQuantumFrames());
...
void MediaStreamAudioDestinationHandler::SetChannelCount(unsigned channel_count, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
...
void MediaStreamAudioDestinationHandler::PullInputs(uint32_t frames_to_process)
{
...
void MediaStreamAudioDestinationHandler::CheckNumberOfChannelsForInput(AudioNodeInput* input)
{
...
void MediaStreamAudioDestinationHandler::SendLogMessage(const char* const function_name, const String& message)
{
    WebRtcLogMessage(String::Format("[WA]MSADH::%s %s [this=0x%" PRIXPTR "]", function_name, message.Utf8().c_str(), reinterpret_cast<uintptr_t>(this)).Utf8());
}
