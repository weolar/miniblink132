...
// active nor disabled).
FindOutputResult FindOutput(AudioNodeOutput& output, AudioNodeOutputSet& outputs, AudioNodeOutputSet& disabled_outputs)
{
...
void AudioNodeWiring::Connect(AudioNodeOutput& output, AudioNodeInput& input)
{
    ...
    const bool input_connected_to_output = input.outputs_.Contains(&output) || input.disabled_outputs_.Contains(&output);
    ...
    (output.is_enabled_ ? input.outputs_ : input.disabled_outputs_).insert(&output);
...
void AudioNodeWiring::Connect(AudioNodeOutput& output, AudioParamHandler& param)
{
...
void AudioNodeWiring::Disconnect(AudioNodeOutput& output, AudioNodeInput& input)
{
    input.GetDeferredTaskHandler().AssertGraphOwner();

    // These must be connected.
    DCHECK(output.inputs_.Contains(&input));
    DCHECK(input.outputs_.Contains(&output) || input.disabled_outputs_.Contains(&output));
...
void AudioNodeWiring::Disconnect(AudioNodeOutput& output, AudioParamHandler& param)
{
    param.GetDeferredTaskHandler().AssertGraphOwner();
...
bool AudioNodeWiring::IsConnected(AudioNodeOutput& output, AudioNodeInput& input)
{
    input.GetDeferredTaskHandler().AssertGraphOwner();

    bool is_connected = output.inputs_.Contains(&input);
    DCHECK_EQ(is_connected, input.outputs_.Contains(&output) || input.disabled_outputs_.Contains(&output));
    return is_connected;
}

bool AudioNodeWiring::IsConnected(AudioNodeOutput& output, AudioParamHandler& param)
{
    param.GetDeferredTaskHandler().AssertGraphOwner();
