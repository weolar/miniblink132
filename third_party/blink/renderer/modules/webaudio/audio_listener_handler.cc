...
scoped_refptr<AudioListenerHandler> AudioListenerHandler::Create(AudioParamHandler& position_x_handler, AudioParamHandler& position_y_handler,
    AudioParamHandler& position_z_handler, AudioParamHandler& forward_x_handler, AudioParamHandler& forward_y_handler, AudioParamHandler& forward_z_handler,
    AudioParamHandler& up_x_handler, AudioParamHandler& up_y_handler, AudioParamHandler& up_z_handler, unsigned int render_quantum_frames)
{
    return base::AdoptRef(new AudioListenerHandler(position_x_handler, position_y_handler, position_z_handler, forward_x_handler, forward_y_handler,
        forward_z_handler, up_x_handler, up_y_handler, up_z_handler, render_quantum_frames));
}

AudioListenerHandler::AudioListenerHandler(AudioParamHandler& position_x_handler, AudioParamHandler& position_y_handler, AudioParamHandler& position_z_handler,
    AudioParamHandler& forward_x_handler, AudioParamHandler& forward_y_handler, AudioParamHandler& forward_z_handler, AudioParamHandler& up_x_handler,
    AudioParamHandler& up_y_handler, AudioParamHandler& up_z_handler, unsigned int render_quantum_frames)
...
const float* AudioListenerHandler::GetPositionXValues(uint32_t frames_to_process)
{
    UpdateValuesIfNeeded(frames_to_process);
    return position_x_values_.Data();
}

const float* AudioListenerHandler::GetPositionYValues(uint32_t frames_to_process)
{
    UpdateValuesIfNeeded(frames_to_process);
    return position_y_values_.Data();
}

const float* AudioListenerHandler::GetPositionZValues(uint32_t frames_to_process)
{
    UpdateValuesIfNeeded(frames_to_process);
    return position_z_values_.Data();
}

const float* AudioListenerHandler::GetForwardXValues(uint32_t frames_to_process)
{
    UpdateValuesIfNeeded(frames_to_process);
    return forward_x_values_.Data();
}

const float* AudioListenerHandler::GetForwardYValues(uint32_t frames_to_process)
{
    UpdateValuesIfNeeded(frames_to_process);
    return forward_y_values_.Data();
}

const float* AudioListenerHandler::GetForwardZValues(uint32_t frames_to_process)
{
    UpdateValuesIfNeeded(frames_to_process);
...
{
    return position_x_handler_->HasSampleAccurateValues() || position_y_handler_->HasSampleAccurateValues() || position_z_handler_->HasSampleAccurateValues()
        || forward_x_handler_->HasSampleAccurateValues() || forward_y_handler_->HasSampleAccurateValues() || forward_z_handler_->HasSampleAccurateValues()
        || up_x_handler_->HasSampleAccurateValues() || up_y_handler_->HasSampleAccurateValues() || up_z_handler_->HasSampleAccurateValues();
}

bool AudioListenerHandler::IsAudioRate() const
{
    return position_x_handler_->IsAudioRate() || position_y_handler_->IsAudioRate() || position_z_handler_->IsAudioRate() || forward_x_handler_->IsAudioRate()
        || forward_y_handler_->IsAudioRate() || forward_z_handler_->IsAudioRate() || up_x_handler_->IsAudioRate() || up_y_handler_->IsAudioRate()
...
void AudioListenerHandler::UpdateState()
{
    ...
    if (try_locker.is_acquired()) {
        ...
        is_listener_dirty_ = current_position != last_position_ || current_forward != last_forward_ || current_up != last_up_;
...
void AudioListenerHandler::CreateAndLoadHRTFDatabaseLoader(float sample_rate)
{
    ...
    hrtf_database_loader_ = HRTFDatabaseLoader::CreateAndLoadAsynchronouslyIfNecessary(sample_rate);
}
...
void AudioListenerHandler::UpdateValuesIfNeeded(uint32_t frames_to_process)
{
    ...
    if (last_update_time_ != current_time) {
        ...
        position_x_handler_->CalculateSampleAccurateValues(position_x_values_.Data(), frames_to_process);
        position_y_handler_->CalculateSampleAccurateValues(position_y_values_.Data(), frames_to_process);
        position_z_handler_->CalculateSampleAccurateValues(position_z_values_.Data(), frames_to_process);
        forward_x_handler_->CalculateSampleAccurateValues(forward_x_values_.Data(), frames_to_process);
        forward_y_handler_->CalculateSampleAccurateValues(forward_y_values_.Data(), frames_to_process);
        forward_z_handler_->CalculateSampleAccurateValues(forward_z_values_.Data(), frames_to_process);
        up_x_handler_->CalculateSampleAccurateValues(up_x_values_.Data(), frames_to_process);
        up_y_handler_->CalculateSampleAccurateValues(up_y_values_.Data(), frames_to_process);
        up_z_handler_->CalculateSampleAccurateValues(up_z_values_.Data(), frames_to_process);
    }
}
