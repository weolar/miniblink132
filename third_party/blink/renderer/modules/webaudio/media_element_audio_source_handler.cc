...
public:
    explicit MediaElementAudioSourceHandlerLocker(MediaElementAudioSourceHandler& lockable)
        : lockable_(lockable)
    {
        lockable_.lock();
    }

    MediaElementAudioSourceHandlerLocker(const MediaElementAudioSourceHandlerLocker&) = delete;
    MediaElementAudioSourceHandlerLocker& operator=(const MediaElementAudioSourceHandlerLocker&) = delete;
...
MediaElementAudioSourceHandler::MediaElementAudioSourceHandler(AudioNode& node, HTMLMediaElement& media_element)
    : AudioHandler(kNodeTypeMediaElementAudioSource, node, node.context()->sampleRate())
...
scoped_refptr<MediaElementAudioSourceHandler> MediaElementAudioSourceHandler::Create(AudioNode& node, HTMLMediaElement& media_element)
{
    return base::AdoptRef(new MediaElementAudioSourceHandler(node, media_element));
}
...
CrossThreadPersistent<HTMLMediaElement> MediaElementAudioSourceHandler::MediaElement() const
{
...
void MediaElementAudioSourceHandler::SetFormat(uint32_t number_of_channels, float source_sample_rate)
{
    DCHECK(MediaElement());
    ...
    if (number_of_channels != source_number_of_channels_ || source_sample_rate != source_sample_rate_) {
        if (!number_of_channels || number_of_channels > BaseAudioContext::MaxNumberOfChannels()
            ...
            DLOG(ERROR) << "setFormat(" << number_of_channels << ", " << source_sample_rate << ") - unhandled format change";
        ...
        if (source_sample_rate != Context()->sampleRate()) {
            ...
            multi_channel_resampler_
                = std::make_unique<MediaMultiChannelResampler>(number_of_channels, scale_factor, GetDeferredTaskHandler().RenderQuantumFrames(),
                    CrossThreadBindRepeating(&MediaElementAudioSourceHandler::ProvideResamplerInput, CrossThreadUnretained(this)));
        } else {
            // Bypass resampling.
...
void MediaElementAudioSourceHandler::PrintCorsMessage(const String& message)
{
    if (Context()->GetExecutionContext()) {
        Context()->GetExecutionContext()->AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kSecurity, mojom::ConsoleMessageLevel::kInfo,
...
void MediaElementAudioSourceHandler::ProvideResamplerInput(int resampler_frame_delay, AudioBus* dest)
{
    DCHECK(Context()->IsAudioThread());
    DCHECK(MediaElement());
    DCHECK(dest);
    MediaElement()->GetAudioSourceProvider().ProvideInput(dest, base::checked_cast<int>(dest->length()));
}
