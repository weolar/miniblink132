...
class MultiResolutionImageResourceFetcher::ClientImpl : public WebAssociatedURLLoaderClient {
...
private:
    ...
    void OnLoadCompleteInternal(LoadStatus status)
    {
        ...
        if (callback_.is_null())
            return;
        std::move(callback_).Run(status_ == kLoadFailed ? WebURLResponse() : response_, status_ == kLoadFailed ? std::string() : data_);
    }
...
MultiResolutionImageResourceFetcher::MultiResolutionImageResourceFetcher(
    const KURL& image_url, LocalFrame* frame, bool is_favicon, mojom::blink::FetchCacheMode cache_mode, Callback callback)
    : callback_(std::move(callback))
    , http_status_code_(0)
    , request_(image_url)
{
    ...
    if (is_favicon) {
        ...
        if (!frame->DomWindow()->GetSecurityOrigin()->CanAccess(SecurityOrigin::Create(image_url).get())) {
    ...
    Start(frame, is_favicon, network::mojom::RequestMode::kNoCors, network::mojom::CredentialsMode::kInclude,
        WTF::BindOnce(&MultiResolutionImageResourceFetcher::OnURLFetchComplete, WTF::Unretained(this)));
}
...
void MultiResolutionImageResourceFetcher::OnURLFetchComplete(const WebURLResponse& response, const std::string& data)
{
    if (!response.IsNull()) {
...
void MultiResolutionImageResourceFetcher::SetSkipServiceWorker(bool skip_service_worker)
{
...
void MultiResolutionImageResourceFetcher::SetCacheMode(mojom::FetchCacheMode mode)
{
    DCHECK(!request_.IsNull());
    DCHECK(!loader_);
...
void MultiResolutionImageResourceFetcher::SetLoaderOptions(const WebAssociatedURLLoaderOptions& options)
{
    DCHECK(!request_.IsNull());
    DCHECK(!loader_);
...
void MultiResolutionImageResourceFetcher::Start(
    LocalFrame* frame, bool is_favicon, network::mojom::RequestMode request_mode, network::mojom::CredentialsMode credentials_mode, StartCallback callback)
{
    DCHECK(!loader_);
    DCHECK(!client_);
    DCHECK(!request_.IsNull());
    ...
    mojom::blink::RequestContextType request_context = is_favicon ? mojom::blink::RequestContextType::FAVICON : mojom::blink::RequestContextType::IMAGE;
    ...
    loader_ = std::make_unique<WebAssociatedURLLoaderImpl>(frame->DomWindow(), options_);
