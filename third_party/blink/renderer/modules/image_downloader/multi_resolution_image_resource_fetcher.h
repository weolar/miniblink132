...
public:
    ...
    using Callback = base::OnceCallback<void(MultiResolutionImageResourceFetcher*, const std::string& data, const WebString& mime_type)>;
    ...
    // is destroyed.
    using StartCallback = base::OnceCallback<void(const WebURLResponse& response, const std::string& data)>;

    MultiResolutionImageResourceFetcher(const KURL& image_url, LocalFrame* frame, bool is_favicon, mojom::blink::FetchCacheMode cache_mode, Callback callback);

    MultiResolutionImageResourceFetcher(const MultiResolutionImageResourceFetcher&) = delete;
    MultiResolutionImageResourceFetcher& operator=(const MultiResolutionImageResourceFetcher&) = delete;
...
private:
    ...
    // |callback_|.
    void OnURLFetchComplete(const WebURLResponse& response, const std::string& data);
