...
public:
    ...
    using DownloadCallback = base::OnceCallback<void(int32_t, const WTF::Vector<SkBitmap>&)>;
...
private:
    ...
    void DownloadImage(const KURL& url, bool is_favicon, const gfx::Size& preferred_size, uint32_t max_bitmap_size, bool bypass_cache,
        DownloadImageCallback callback) override;

    // ImageDownloader implementation. Request to asynchronously download an
    // image. When done, |callback| will be called.
    void DownloadImageFromAxNode(
        int ax_node_id, const gfx::Size& preferred_size, uint32_t max_bitmap_size, bool bypass_cache, DownloadImageCallback callback) override;
    ...
    void DidDownloadImage(uint32_t max_bitmap_size, DownloadImageCallback callback, int32_t http_status_code, const WTF::Vector<SkBitmap>& images);

    void CreateMojoService(mojo::PendingReceiver<mojom::blink::ImageDownloader> receiver);
    ...
    // are returned.
    void FetchImage(const KURL& image_url, bool is_favicon, const gfx::Size& preferred_size, bool bypass_cache, DownloadCallback callback);

    // This callback is triggered when FetchImage completes, either
    // successfully or with a failure. See FetchImage for more
    // details.
    void DidFetchImage(DownloadCallback callback, const gfx::Size& preferred_size, MultiResolutionImageResourceFetcher* fetcher, const std::string& image_data,
        const WebString& mime_type);

    typedef WTF::Vector<std::unique_ptr<MultiResolutionImageResourceFetcher>> ImageResourceFetcherList;

    // ImageResourceFetchers schedule via FetchImage.
    ImageResourceFetcherList image_fetchers_;

    HeapMojoReceiver<mojom::blink::ImageDownloader, ImageDownloaderImpl, HeapMojoWrapperMode::kForceWithoutContextObserver> receiver_;
};
