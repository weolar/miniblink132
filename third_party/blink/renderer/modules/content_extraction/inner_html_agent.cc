...
// static
void InnerHtmlAgent::BindReceiver(LocalFrame* frame, mojo::PendingReceiver<mojom::blink::InnerHtmlAgent> receiver)
{
    DCHECK(frame && frame->GetDocument());
    auto& document = *frame->GetDocument();
    auto* agent = InnerHtmlAgent::From(document);
    if (!agent) {
        agent = MakeGarbageCollected<InnerHtmlAgent>(base::PassKey<InnerHtmlAgent>(), *frame);
...
void InnerHtmlAgent::Bind(mojo::PendingReceiver<mojom::blink::InnerHtmlAgent> receiver)
{
