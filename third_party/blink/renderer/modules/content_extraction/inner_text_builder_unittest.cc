...
TEST(InnerTextBuilderTest, Basic)
{
    ...
    frame_test_helpers::LoadHTMLString(
        helper.LocalMainFrame(), "<body>container<iframe id='x'>iframe</iframe>after</body>", url_test_helpers::ToKURL("http://foobar.com"));
    ...
    auto frame = InnerTextBuilder::Build(*helper.LocalMainFrame()->GetFrame(), params);
    ASSERT_TRUE(frame);
    ...
    const mojom::blink::InnerTextFramePtr& child_frame = frame->segments[2]->get_frame();
    HTMLIFrameElement* html_iframe = DynamicTo<HTMLIFrameElement>(iframe.Unwrap<Node>());
    EXPECT_EQ(html_iframe->contentDocument()->GetFrame()->GetLocalFrameToken(), child_frame->token);
...
TEST(InnerTextBuilderTest, MultiFrames)
{
    ...
    url_test_helpers::RegisterMockedURLLoadFromBase(WebString::FromUTF8(base_url), test::CoreTestDataPath(), "inner_text_test1.html");
    url_test_helpers::RegisterMockedURLLoadFromBase(WebString::FromUTF8(base_url), test::CoreTestDataPath(), "subframe-a.html");
    url_test_helpers::RegisterMockedURLLoadFromBase(WebString::FromUTF8(base_url), test::CoreTestDataPath(), "subframe-b.html");
    url_test_helpers::RegisterMockedURLLoadFromBase(WebString::FromUTF8(base_url), test::CoreTestDataPath(), "subframe-d.html");
    helper.InitializeAndLoad(base_url + "inner_text_test1.html");

    auto web_iframe_d = helper.LocalMainFrame()->GetDocument().GetElementById("d");
    ASSERT_FALSE(web_iframe_d.IsNull());
    HTMLIFrameElement* iframe_d = DynamicTo<HTMLIFrameElement>(web_iframe_d.Unwrap<Node>());
    ASSERT_TRUE(iframe_d);
    mojom::blink::InnerTextParams params;
    params.node_id = iframe_d->contentDocument()->getElementById(AtomicString("bold"))->GetDomNodeId();
    ...
    {
        const mojom::blink::InnerTextFramePtr& frame_1 = frame->segments[1]->get_frame();
    ...
    {
        const mojom::blink::InnerTextFramePtr& frame_2 = frame->segments[3]->get_frame();
        ASSERT_EQ(2u, frame_2->segments.size());
        EXPECT_EQ("b ", frame_2->segments[0]->get_text());
        ASSERT_TRUE(frame_2->segments[1]->is_frame());
        {
            const mojom::blink::InnerTextFramePtr& frame_2_1 = frame_2->segments[1]->get_frame();
    ...
    {
        const mojom::blink::InnerTextFramePtr& frame_3 = frame->segments[5]->get_frame();
...
TEST(InnerTextBuilderTest, DifferentOrigin)
{
    ...
    frame_test_helpers::WebViewHelper helper;
    std::string base_url("http://internal.test/");
    url_test_helpers::RegisterMockedURLLoadFromBase(WebString::FromUTF8(base_url), test::CoreTestDataPath(), "inner_text_test2.html");
    url_test_helpers::RegisterMockedURLLoadFromBase(WebString::FromUTF8("http://different-host.com/"), test::CoreTestDataPath(), "subframe-a.html");
    helper.InitializeAndLoad(base_url + "inner_text_test2.html");

    auto frame = InnerTextBuilder::Build(*helper.LocalMainFrame()->GetFrame(), {});
...
void ExpectChunkerResult(int max_words_per_aggregate_passage, bool greedily_aggregate_sibling_nodes, int max_passages, int min_words_per_passage,
    const std::string& html, const std::vector<String>& expected_passages)
{
    test::TaskEnvironment task_environment;
    frame_test_helpers::WebViewHelper helper;
    helper.Initialize();
    ASSERT_TRUE(helper.LocalMainFrame());
    frame_test_helpers::LoadHTMLString(helper.LocalMainFrame(), html, url_test_helpers::ToKURL("http://foobar.com"));
    ASSERT_TRUE(helper.LocalMainFrame());
    ...
    mojom::blink::InnerTextFramePtr frame = InnerTextPassagesBuilder::Build(*helper.LocalMainFrame()->GetFrame(), params);
    ASSERT_TRUE(frame);
...
TEST(InnerTextBuilderTest, InnerTextPassagesStripsWhitespaceAroundNodeText)
{
    ...
    ExpectChunkerResult(8, false, 0, 0, html,
        {
...
TEST(InnerTextBuilderTest, InnerTextPassagesIncludesNodesOverMaxAggregateChunkSize)
{
    std::string html = R"(
      <div>
        <div>First level one.
...
TEST(InnerTextBuilderTest, InnerTextPassagesDoesNotJoinSplitTextNodesWithinPTagWhenOverMax)
{
    std::string html = R"(
      <p>Paragraph one with
          <a>link</a>
...
TEST(InnerTextBuilderTest, InnerTextPassagesGreedilyAggregatesSiblingNodes)
{
    ...
    ExpectChunkerResult(10, true, 0, 0, html,
        {
            "First level one.",
            "Second level one.",
...
TEST(InnerTextBuilderTest, InnerTextPassagesDoesNotGreedilyAggregateAcrossSectionBreaks)
{
