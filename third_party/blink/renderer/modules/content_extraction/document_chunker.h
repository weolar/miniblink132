...
public:
    ...
    DocumentChunker(size_t max_words_per_aggregate_passage, bool greedily_aggregate_sibling_nodes, uint32_t max_passages, uint32_t min_words_per_passage);
...
private:
    ...
    struct PassageList {
        ...
        void AddPassageForNode(const AggregateNode& node, size_t min_words_per_passage);
    ...
    AggregateNode ProcessNode(const Node& node, int depth, uint32_t passage_count);
