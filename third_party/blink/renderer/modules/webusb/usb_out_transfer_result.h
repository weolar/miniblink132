...
public:
    ...
    static USBOutTransferResult* Create(const V8USBTransferStatus& status, uint32_t bytes_written)
    {
        return MakeGarbageCollected<USBOutTransferResult>(status, bytes_written);
    }

    USBOutTransferResult(const V8USBTransferStatus& status, uint32_t bytes_written)
        : status_(status)
