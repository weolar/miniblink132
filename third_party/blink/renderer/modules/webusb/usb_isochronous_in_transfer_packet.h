...
public:
    static USBIsochronousInTransferPacket* Create(const V8USBTransferStatus& status)
    {
        return MakeGarbageCollected<USBIsochronousInTransferPacket>(status, nullptr);
    }

    static USBIsochronousInTransferPacket* Create(const V8USBTransferStatus& status, NotShared<DOMDataView> data)
    {
        return MakeGarbageCollected<USBIsochronousInTransferPacket>(status, data.Get());
    }

    USBIsochronousInTransferPacket(const V8USBTransferStatus& status, DOMDataView* data)
        : status_(status)
        , data_(data)
    {
    }
    ...
    V8USBTransferStatus status() const
    {
        return status_;
    }
