...
USBConfiguration* USBConfiguration::Create(const USBDevice* device, wtf_size_t configuration_index)
{
    return MakeGarbageCollected<USBConfiguration>(device, configuration_index);
}

USBConfiguration* USBConfiguration::Create(const USBDevice* device, uint8_t configuration_value, ExceptionState& exception_state)
{
...
USBConfiguration::USBConfiguration(const USBDevice* device, wtf_size_t configuration_index)
    : device_(device)
...
const device::mojom::blink::UsbConfigurationInfo& USBConfiguration::Info() const
{
