...
V8USBDirection::Enum ConvertDirectionToEnum(const UsbTransferDirection& direction)
{
    switch (direction) {
...
USBEndpoint* USBEndpoint::Create(const USBAlternateInterface* alternate, wtf_size_t endpoint_index)
{
    return MakeGarbageCollected<USBEndpoint>(alternate, endpoint_index);
}

USBEndpoint* USBEndpoint::Create(
    const USBAlternateInterface* alternate, uint8_t endpoint_number, const V8USBDirection& direction, ExceptionState& exception_state)
{
    UsbTransferDirection mojo_direction = direction == V8USBDirection::Enum::kIn ? UsbTransferDirection::INBOUND : UsbTransferDirection::OUTBOUND;
    const auto& endpoints = alternate->Info().endpoints;
    for (wtf_size_t i = 0; i < endpoints.size(); ++i) {
        const auto& endpoint = endpoints[i];
        if (endpoint->endpoint_number == endpoint_number && endpoint->direction == mojo_direction)
            return USBEndpoint::Create(alternate, i);
    }
    exception_state.ThrowRangeError("No such endpoint exists in the given alternate interface.");
    return nullptr;
}

USBEndpoint::USBEndpoint(const USBAlternateInterface* alternate, wtf_size_t endpoint_index)
...
{
    const device::mojom::blink::UsbAlternateInterfaceInfo& alternate_info = alternate_->Info();
