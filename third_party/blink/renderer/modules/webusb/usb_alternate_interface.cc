...
USBAlternateInterface* USBAlternateInterface::Create(const USBInterface* interface, wtf_size_t alternate_index)
{
    return MakeGarbageCollected<USBAlternateInterface>(interface, alternate_index);
}

USBAlternateInterface* USBAlternateInterface::Create(const USBInterface* interface, uint8_t alternate_setting, ExceptionState& exception_state)
{
...
USBAlternateInterface::USBAlternateInterface(const USBInterface* interface, wtf_size_t alternate_index)
    : interface_(interface)
    , alternate_index_(alternate_index)
{
    DCHECK(interface_);
    ...
    for (wtf_size_t i = 0; i < Info().endpoints.size(); ++i) {
        ...
        // defined in WebUSB.
        if (Info().endpoints[i]->type != device::mojom::blink::UsbTransferType::CONTROL) {
            endpoints_.push_back(USBEndpoint::Create(this, i));
        }
    }
}

const device::mojom::blink::UsbAlternateInterfaceInfo& USBAlternateInterface::Info() const
{
    const device::mojom::blink::UsbInterfaceInfo& interface_info = interface_->Info();
