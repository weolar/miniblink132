...
public:
    static USBIsochronousOutTransferPacket* Create(const V8USBTransferStatus& status)
    {
        return MakeGarbageCollected<USBIsochronousOutTransferPacket>(status, 0);
    }

    static USBIsochronousOutTransferPacket* Create(const V8USBTransferStatus& status, unsigned bytes_written)
    {
        return MakeGarbageCollected<USBIsochronousOutTransferPacket>(status, bytes_written);
    }

    USBIsochronousOutTransferPacket(const V8USBTransferStatus& status, unsigned bytes_written)
        : status_(status)
