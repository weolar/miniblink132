...
public:
    static ScriptPromise<PictureInPictureWindow> requestPictureInPicture(ScriptState*, HTMLVideoElement&, ExceptionState&);

    static bool FastHasAttribute(const HTMLVideoElement&, const QualifiedName&);

    static void SetBooleanAttribute(HTMLVideoElement&, const QualifiedName&, bool);

    static void CheckIfPictureInPictureIsAllowed(HTMLVideoElement&, ExceptionState&);

    DEFINE_STATIC_ATTRIBUTE_EVENT_LISTENER(enterpictureinpicture, kEnterpictureinpicture)
    DEFINE_STATIC_ATTRIBUTE_EVENT_LISTENER(leavepictureinpicture, kLeavepictureinpicture)
};
