...
ScriptPromise<Blob> FontMetadata::blob(ScriptState* script_state)
{
    ...
    ExecutionContext::From(script_state)
        ...
        ->PostTask(FROM_HERE, WTF::BindOnce(&FontMetadata::BlobImpl, WrapPersistent(resolver), postscriptName_));
...
// static
void FontMetadata::BlobImpl(ScriptPromiseResolver<Blob>* resolver, const String& postscriptName)
{
    ...
    const SimpleFontData* font_data = FontCache::Get().GetFontData(description, AtomicString(postscriptName), AlternateFontName::kLocalUniqueFace);
    if (!font_data) {
        auto message = String::Format("The font %s could not be accessed.", postscriptName.Latin1().c_str());
        ScriptState::Scope scope(resolver->GetScriptState());
        resolver->Reject(V8ThrowException::CreateTypeError(resolver->GetScriptState()->GetIsolate(), message));
        return;
    }
    ...
    if (!(stream && stream->getMemoryBase())) {
        ...
        auto message = String::Format("Font data for %s could not be accessed.", postscriptName.Latin1().c_str());
        ScriptState::Scope scope(resolver->GetScriptState());
        resolver->Reject(V8ThrowException::CreateTypeError(resolver->GetScriptState()->GetIsolate(), message));
        return;
    }

    wtf_size_t font_byte_size = base::checked_cast<wtf_size_t>(stream->getLength());
    ...
    auto* blob = MakeGarbageCollected<Blob>(BlobDataHandle::Create(std::move(blob_data), font_byte_size));
