...
Vector<uint8_t> BufferSourceToVector(const V8UnionBufferSourceOrString* application_server_key, ExceptionState& exception_state)
{
    ...
    case V8UnionBufferSourceOrString::ContentType::kArrayBuffer:
        input = base::as_chars(application_server_key->GetAsArrayBuffer()->ByteSpan());
        break;
    case V8UnionBufferSourceOrString::ContentType::kArrayBufferView:
        input = base::as_chars(application_server_key->GetAsArrayBufferView()->ByteSpan());
        break;
    case V8UnionBufferSourceOrString::ContentType::kString:
        if (!Base64UnpaddedURLDecode(application_server_key->GetAsString(), decoded_application_server_key)) {
    ...
    const bool is_sender_id
        = input.size() > 0 && input.size() < kMaxApplicationServerKeyLength && (base::ranges::find_if_not(input, WTF::IsASCIIDigit<char>) == input.end());

    if (is_vapid || is_sender_id) {
        result.AppendSpan(base::as_bytes(input));
    } else {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "The provided applicationServerKey is not valid.");
    }
...
// static
PushSubscriptionOptions* PushSubscriptionOptions::FromOptionsInit(const PushSubscriptionOptionsInit* options_init, ExceptionState& exception_state)
{
    ...
    if (options_init->hasApplicationServerKey() && options_init->applicationServerKey()) {
        application_server_key.AppendVector(BufferSourceToVector(options_init->applicationServerKey(), exception_state));
    }

    return MakeGarbageCollected<PushSubscriptionOptions>(options_init->userVisibleOnly(), application_server_key);
}

PushSubscriptionOptions::PushSubscriptionOptions(bool user_visible_only, const Vector<uint8_t>& application_server_key)
