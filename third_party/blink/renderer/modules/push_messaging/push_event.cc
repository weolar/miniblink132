...
PushEvent::PushEvent(const AtomicString& type, const PushEventInit* initializer, ExceptionState& exception_state)
    ...
    if (initializer->hasData()) {
        ...
        if (message_data->IsArrayBuffer() || message_data->IsArrayBufferView()) {
            DOMArrayPiece array_piece
                = message_data->IsArrayBuffer() ? DOMArrayPiece(message_data->GetAsArrayBuffer()) : DOMArrayPiece(message_data->GetAsArrayBufferView().Get());
