...
const std::array<uint8_t, kApplicationServerKeyLength> kApplicationServerKey
    = { 0x04, 0x55, 0x52, 0x6A, 0xA5, 0x6E, 0x8E, 0xAA, 0x47, 0x97, 0x36, 0x10, 0xC1, 0x66, 0x3C, 0x1E, 0x65, 0xBF, 0xA1, 0x7B, 0xEE, 0x48, 0xC9, 0xC6, 0xBB,
          0xBF, 0x02, 0x18, 0x53, 0x72, 0x1D, 0x0C, 0x7B, 0xA9, 0xE3, 0x11, 0xB7, 0x03, 0x52, 0x21, 0xD3, 0x71, 0x90, 0x13, 0xA8, 0xC1, 0xCF, 0xED, 0x20, 0xF7,
          0x1F, 0xD1, 0x7F, 0xF2, 0x76, 0xB6, 0x01, 0x20, 0xD8, 0x35, 0xA5, 0xD9, 0x3C, 0x43, 0xDF };
...
void IsApplicationServerKeyValid(PushSubscriptionOptions* output)
{
    ...
    ASSERT_EQ(output->applicationServerKey()->ByteLength(), kApplicationServerKeyLength);

    String application_server_key(output->applicationServerKey()->ByteSpan());
    ASSERT_EQ(reinterpret_cast<const char*>(sender_key.data()), application_server_key.Latin1());
}
...
TEST(PushManagerTest, ValidSenderKey)
{
    ...
    options->setApplicationServerKey(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferViewOrString>(DOMArrayBuffer::Create(kApplicationServerKey)));

    DummyExceptionStateForTesting exception_state;
    PushSubscriptionOptions* output = PushSubscriptionOptions::FromOptionsInit(options, exception_state);
    ASSERT_TRUE(output);
...
{
    test::TaskEnvironment task_environment;
    PushSubscriptionOptionsInit* options = MakeGarbageCollected<PushSubscriptionOptionsInit>();
    String base64_url = WTF::Base64URLEncode(kApplicationServerKey);
    base64_url = base64_url.RemoveCharacters(RemovePad);
    options->setApplicationServerKey(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferViewOrString>(base64_url));
...
TEST(PushManagerTest, InvalidSenderKeyLength)
{
    ...
    PushSubscriptionOptionsInit* options = PushSubscriptionOptionsInit::Create();
    options->setApplicationServerKey(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferViewOrString>(DOMArrayBuffer::Create(sender_key)));

    DummyExceptionStateForTesting exception_state;
    PushSubscriptionOptions* output = PushSubscriptionOptions::FromOptionsInit(options, exception_state);
    ASSERT_TRUE(output);
    ASSERT_TRUE(exception_state.HadException());
    ASSERT_EQ(exception_state.Message(), "The provided applicationServerKey is not valid.");
}
...
TEST(PushManagerTest, InvalidBase64SenderKey)
{
    test::TaskEnvironment task_environment;
    PushSubscriptionOptionsInit* options = MakeGarbageCollected<PushSubscriptionOptionsInit>();
    options->setApplicationServerKey(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferViewOrString>(Base64Encode(kApplicationServerKey)));
...
TEST(PushManagerTest, InvalidBase64URLWithPaddingSenderKey)
{
    test::TaskEnvironment task_environment;
    PushSubscriptionOptionsInit* options = MakeGarbageCollected<PushSubscriptionOptionsInit>();
    options->setApplicationServerKey(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferViewOrString>(WTF::Base64URLEncode(kApplicationServerKey)));
