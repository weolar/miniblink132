...
PushMessageData* PushMessageData::Create(const String& message_string)
{
    ...
    if (message_string.IsNull())
        return nullptr;
    return PushMessageData::Create(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferViewOrUSVString>(message_string));
}

PushMessageData* PushMessageData::Create(const V8UnionArrayBufferOrArrayBufferViewOrUSVString* message_data)
{
    if (!message_data)
        return nullptr;
    ...
    case V8UnionArrayBufferOrArrayBufferViewOrUSVString::ContentType::kArrayBufferView: {
        const DOMArrayBufferView* buffer_view = message_data->GetAsArrayBufferView().Get();
    ...
    case V8UnionArrayBufferOrArrayBufferViewOrUSVString::ContentType::kUSVString: {
        std::string encoded_string = UTF8Encoding().Encode(message_data->GetAsUSVString(), WTF::kNoUnencodables);
        return MakeGarbageCollected<PushMessageData>(base::as_byte_span(encoded_string));
    }
    }
    NOTREACHED();
}
...
Blob* PushMessageData::blob() const
{
    ...
    return MakeGarbageCollected<Blob>(BlobDataHandle::Create(std::move(blob_data), byte_length));
}
...
{
    return ScriptValue(script_state->GetIsolate(), FromJSONString(script_state, text()));
}
