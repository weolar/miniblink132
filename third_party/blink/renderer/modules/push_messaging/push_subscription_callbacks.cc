...
PushSubscriptionCallbacks::PushSubscriptionCallbacks(ScriptPromiseResolverBase* resolver, bool null_allowed)
    : resolver_(resolver)
...
{
    if (null_allowed_) {
        resolver_->DowncastTo<IDLNullable<PushSubscription>>()->Resolve(push_subscription);
    } else {
