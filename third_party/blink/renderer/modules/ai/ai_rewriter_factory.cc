...
const char kExceptionMessageUnableToCreateRewriter[] = "The rewriter cannot be created.";
...
public:
    CreateRewriterClient(ScriptState* script_state, AI* ai, ScriptPromiseResolver<AIRewriter>* resolver, AbortSignal* signal, V8AIRewriterTone tone,
        V8AIRewriterLength length, String shared_context_string)
        ...
        , length_(length)
    {
        mojo::PendingRemote<mojom::blink::AIManagerCreateRewriterClient> client_remote;
        ...
        ai_->GetAIRemote()->CreateRewriter(std::move(client_remote),
            mojom::blink::AIRewriterCreateOptions::New(shared_context_string_, ToMojoAIRewriterTone(tone), ToMojoAIRewriterLength(length)));
    }
    ...
    void OnResult(mojo::PendingRemote<mojom::blink::AIRewriter> rewriter) override
    {
        if (!GetResolver()) {
            return;
        }
        if (rewriter) {
            GetResolver()->Resolve(MakeGarbageCollected<AIRewriter>(
                ai_->GetExecutionContext(), ai_->GetTaskRunner(), std::move(rewriter), shared_context_string_, tone_, length_));
        } else {
            GetResolver()->Reject(
                DOMException::Create(kExceptionMessageUnableToCreateRewriter, DOMException::GetErrorName(DOMExceptionCode::kInvalidStateError)));
        }
        Cleanup();
    }
...
private:
    Member<AI> ai_;
    HeapMojoReceiver<mojom::blink::AIManagerCreateRewriterClient, CreateRewriterClient> receiver_;
...
ScriptPromise<AIRewriter> AIRewriterFactory::create(ScriptState* script_state, const AIRewriterCreateOptions* options, ExceptionState& exception_state)
{
    ...
    CHECK(options);
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<AIRewriter>>(script_state);
    ...
    MakeGarbageCollected<CreateRewriterClient>(script_state, ai_, resolver, signal, options->tone(), options->length(), options->getSharedContextOr(String()));
    return promise;
}
