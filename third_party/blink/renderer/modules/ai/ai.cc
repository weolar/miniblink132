...
HeapMojoRemote<mojom::blink::AIManager>& AI::GetAIRemote()
{
    if (!ai_remote_.is_bound()) {
        if (GetExecutionContext()) {
            GetExecutionContext()->GetBrowserInterfaceBroker().GetInterface(ai_remote_.BindNewPipeAndPassReceiver(task_runner_));
        }
    }
    return ai_remote_;
}
...
AILanguageModelFactory* AI::languageModel()
{
    if (!ai_language_model_factory_) {
        ai_language_model_factory_ = MakeGarbageCollected<AILanguageModelFactory>(this);
    }
...
AISummarizerFactory* AI::summarizer()
{
    if (!ai_summarizer_factory_) {
        ai_summarizer_factory_ = MakeGarbageCollected<AISummarizerFactory>(this, GetExecutionContext(), task_runner_);
    }
...
AILanguageDetectorFactory* AI::languageDetector()
{
    if (!ai_language_detector_factory_) {
        ai_language_detector_factory_ = MakeGarbageCollected<AILanguageDetectorFactory>();
    }
