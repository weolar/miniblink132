...
template <typename V8SessionObjectType> class AIMojoClient : public ContextLifecycleObserver {
public:
    AIMojoClient(
        ScriptState* script_state, ExecutionContextClient* context_client, ScriptPromiseResolver<V8SessionObjectType>* resolver, AbortSignal* abort_signal)
        ...
        , abort_signal_(abort_signal)
    {
        ...
        if (abort_signal_) {
            ...
            abort_handle_ = abort_signal_->AddAlgorithm(WTF::BindOnce(&AIMojoClient::OnAborted, WrapWeakPersistent(this)));
        }
    }
