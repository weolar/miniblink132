...
ScriptPromise<IDLSequence<LanguageDetectionResult>> AILanguageDetector::detect(
    ScriptState* script_state, const WTF::String& input, AILanguageDetectorDetectOptions* options, ExceptionState& exception_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<LanguageDetectionResult>>>(script_state);

    DetectLanguage(input, WTF::BindOnce(AILanguageDetector::OnDetectComplete, WrapPersistent(resolver)));
...
HeapVector<Member<LanguageDetectionResult>> AILanguageDetector::ConvertResult(WTF::Vector<LanguagePrediction> predictions)
{
...
void AILanguageDetector::OnDetectComplete(
    ScriptPromiseResolver<IDLSequence<LanguageDetectionResult>>* resolver, base::expected<WTF::Vector<LanguagePrediction>, DetectLanguageError> result)
{
    if (result.has_value()) {
