...
AICapabilityAvailability HandleModelAvailabilityCheckResult(
    ExecutionContext* execution_context, AIMetrics::AISessionType session_type, mojom::blink::ModelAvailabilityCheckResult result)
{
    ...
    if (result == mojom::blink::ModelAvailabilityCheckResult::kReadily) {
    ...
    } else if (result == mojom::blink::ModelAvailabilityCheckResult::kAfterDownload) {
        ...
        execution_context->AddConsoleMessage(mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kWarning,
            ConvertModelAvailabilityCheckResultToDebugString(result));
    }
    base::UmaHistogramEnumeration(AIMetrics::GetAICapabilityAvailabilityMetricName(session_type), availability);
    return availability;
}

V8AICapabilityAvailability AICapabilityAvailabilityToV8(AICapabilityAvailability availability)
{
    switch (availability) {
    ...
    case AICapabilityAvailability::kAfterDownload:
        return V8AICapabilityAvailability(V8AICapabilityAvailability::Enum::kAfterDownload);
