...
{
    return element.GetLoadType() != WebMediaPlayer::kLoadTypeMediaStream && element.duration() != std::numeric_limits<double>::infinity();
}

} // namespace

// static
PictureInPictureControllerImpl& PictureInPictureControllerImpl::From(Document& document)
{
    return static_cast<PictureInPictureControllerImpl&>(PictureInPictureController::From(document));
}
...
PictureInPictureController::Status PictureInPictureControllerImpl::IsDocumentAllowed(bool report_failure) const
{
    ...
    if (RuntimeEnabledFeatures::DocumentPictureInPictureAPIEnabled(GetSupplementable()->GetExecutionContext()) && DomWindow()
        && DomWindow()->IsPictureInPictureWindow()) {
    ...
    if (!GetSupplementable()->GetExecutionContext()->IsFeatureEnabled(
            blink::mojom::blink::PermissionsPolicyFeature::kPictureInPicture, report_failure ? ReportOptions::kReportOnFailure : ReportOptions::kDoNotReport)) {
...
PictureInPictureController::Status PictureInPictureControllerImpl::IsElementAllowed(const HTMLVideoElement& video_element, bool report_failure) const
{
...
void PictureInPictureControllerImpl::EnterPictureInPicture(HTMLVideoElement* video_element, ScriptPromiseResolver<PictureInPictureWindow>* resolver)
{
    ...
    mojo::PendingRemote<mojom::blink::PictureInPictureSessionObserver> session_observer;
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = video_element->GetDocument().GetTaskRunner(TaskType::kMediaElementEvent);
    session_observer_receiver_.Bind(session_observer.InitWithNewPipeAndPassReceiver(), task_runner);

    mojo::PendingAssociatedRemote<media::mojom::blink::MediaPlayer> media_player_remote;
    video_element->BindMediaPlayerReceiver(media_player_remote.InitWithNewEndpointAndPassReceiver());

    gfx::Rect video_bounds;
    if (auto* layout_video = DynamicTo<LayoutVideo>(video_element->GetLayoutObject())) {
        PhysicalRect content_rect = layout_video->ReplacedContentRect();
        video_bounds = video_element->GetDocument().View()->FrameToViewport(ToEnclosingRect(layout_video->LocalToAbsoluteRect(content_rect)));
    } else {
        video_bounds = video_element->BoundsInWidget();
    }

    picture_in_picture_service_->StartSession(video_element->GetWebMediaPlayer()->GetDelegateId(), std::move(media_player_remote),
        video_element->GetWebMediaPlayer()->GetSurfaceId().value(), video_element->GetWebMediaPlayer()->NaturalSize(),
        ShouldShowPlayPauseButton(*video_element), std::move(session_observer), video_bounds,
        WTF::BindOnce(
            &PictureInPictureControllerImpl::OnEnteredPictureInPicture, WrapPersistent(this), WrapPersistent(video_element), WrapPersistent(resolver)));
}

void PictureInPictureControllerImpl::OnEnteredPictureInPicture(HTMLVideoElement* element, ScriptPromiseResolver<PictureInPictureWindow>* resolver,
    mojo::PendingRemote<mojom::blink::PictureInPictureSession> session_remote, const gfx::Size& picture_in_picture_window_size)
{
    ...
    if (!session_remote) {
        if (resolver && IsInParallelAlgorithmRunnable(resolver->GetExecutionContext(), resolver->GetScriptState())) {
            ScriptState::Scope script_state_scope(resolver->GetScriptState());
            resolver->RejectWithDOMException(DOMExceptionCode::kNotSupportedError, "Picture-in-Picture is not available.");
        }
    ...
    picture_in_picture_session_.Bind(std::move(session_remote), element->GetDocument().GetTaskRunner(TaskType::kMediaElementEvent));
    ...
    picture_in_picture_window_ = MakeGarbageCollected<PictureInPictureWindow>(GetExecutionContext(), picture_in_picture_window_size);

    picture_in_picture_element_->DispatchEvent(
        *PictureInPictureEvent::Create(event_type_names::kEnterpictureinpicture, WrapPersistent(picture_in_picture_window_.Get())));
    ...
    if (picture_in_picture_element_->GetWebMediaPlayer()) {
        picture_in_picture_element_->GetWebMediaPlayer()->UnregisterFrameSinkHierarchy();
    }
}

void PictureInPictureControllerImpl::ExitPictureInPicture(HTMLVideoElement* element, ScriptPromiseResolver<IDLUndefined>* resolver)
{
    if (!EnsureService())
        return;
    ...
    picture_in_picture_session_->Stop(WTF::BindOnce(&PictureInPictureControllerImpl::OnExitedPictureInPicture, WrapPersistent(this), WrapPersistent(resolver)));
    session_observer_receiver_.reset();
}

void PictureInPictureControllerImpl::OnExitedPictureInPicture(ScriptPromiseResolver<IDLUndefined>* resolver)
{
    DCHECK(GetSupplementable());
    ...
    if (picture_in_picture_element_) {
        ...
        element->DispatchEvent(*PictureInPictureEvent::Create(event_type_names::kLeavepictureinpicture, WrapPersistent(picture_in_picture_window_.Get())));
...
PictureInPictureWindow* PictureInPictureControllerImpl::pictureInPictureWindow() const
{
...
Element* PictureInPictureControllerImpl::PictureInPictureElement(TreeScope& scope) const
{
    if (!picture_in_picture_element_)
        return nullptr;
...
bool PictureInPictureControllerImpl::IsPictureInPictureElement(const Element* element) const
{
    DCHECK(element);
...
#if !BUILDFLAG(TARGET_OS_IS_ANDROID)
LocalDOMWindow* PictureInPictureControllerImpl::documentPictureInPictureWindow() const
{
    return document_picture_in_picture_window_.Get();
}

LocalDOMWindow* PictureInPictureControllerImpl::GetDocumentPictureInPictureWindow() const
{
    return document_picture_in_picture_window_;
}

LocalDOMWindow* PictureInPictureControllerImpl::GetDocumentPictureInPictureOwner() const
{
    return document_picture_in_picture_owner_;
}

void PictureInPictureControllerImpl::SetDocumentPictureInPictureOwner(LocalDOMWindow* owner)
{
    CHECK(owner);

    document_picture_in_picture_owner_ = owner;
    document_pip_context_observer_ = MakeGarbageCollected<DocumentPictureInPictureObserver>(this);
...
void PictureInPictureControllerImpl::CreateDocumentPictureInPictureWindow(
    ScriptState* script_state, LocalDOMWindow& opener, DocumentPictureInPictureOptions* options, ScriptPromiseResolver<DOMWindow>* resolver)
{
    if (!LocalFrame::ConsumeTransientUserActivation(opener.GetFrame())) {
        resolver->RejectWithDOMException(DOMExceptionCode::kNotAllowedError, "Document PiP requires user activation");
        return;
    }
    ...
    web_options.prefer_initial_window_placement = options->preferInitialWindowPlacement();

    // If either width or height is specified, then both must be specified.
    if (web_options.width > 0 && web_options.height == 0) {
        resolver->RejectWithRangeError("Height must be specified if width is specified");
        return;
    } else if (web_options.width == 0 && web_options.height > 0) {
        resolver->RejectWithRangeError("Width must be specified if height is specified");
        return;
    }

    auto* dom_window = opener.openPictureInPictureWindow(script_state->GetIsolate(), web_options);

    if (!dom_window) {
        resolver->RejectWithDOMException(DOMExceptionCode::kInvalidStateError, "Internal error: no window");
        return;
    }
    ...
    document_pip_context_observer_->SetContextLifecycleNotifier(pip_document->GetExecutionContext());
    ...
    From(*pip_document).SetDocumentPictureInPictureOwner(GetSupplementable()->domWindow());
    ...
    open_document_pip_task_ = PostCancellableTask(*opener.GetTaskRunner(TaskType::kInternalDefault), FROM_HERE,
        WTF::BindOnce(&PictureInPictureControllerImpl::ResolveOpenDocumentPictureInPicture, WrapPersistent(this)));
}
...
void PictureInPictureControllerImpl::ResolveOpenDocumentPictureInPicture()
{
    ...
    if (DomWindow()) {
        DocumentPictureInPicture::From(*DomWindow())
            ->DispatchEvent(*DocumentPictureInPictureEvent::Create(event_type_names::kEnter, WrapPersistent(document_picture_in_picture_window_.Get())));
    }
...
PictureInPictureControllerImpl::DocumentPictureInPictureObserver::DocumentPictureInPictureObserver(PictureInPictureControllerImpl* controller)
    : controller_(controller)
{
}
PictureInPictureControllerImpl::DocumentPictureInPictureObserver::~DocumentPictureInPictureObserver() = default;

void PictureInPictureControllerImpl::DocumentPictureInPictureObserver::ContextDestroyed()
{
    controller_->OnDocumentPictureInPictureContextDestroyed();
}

void PictureInPictureControllerImpl::DocumentPictureInPictureObserver::Trace(Visitor* visitor) const
{
    visitor->Trace(controller_);
    ContextLifecycleObserver::Trace(visitor);
}

void PictureInPictureControllerImpl::OnDocumentPictureInPictureContextDestroyed()
{
...
void PictureInPictureControllerImpl::OnOwnedDocumentPictureInPictureWindowContextDestroyed()
{
...
void PictureInPictureControllerImpl::OnDocumentPictureInPictureOwnerWindowContextDestroyed()
{
...
void PictureInPictureControllerImpl::OnPictureInPictureStateChange()
{
    ...
    DCHECK(picture_in_picture_element_->GetWebMediaPlayer()->GetSurfaceId().has_value());
    ...
    mojo::PendingAssociatedRemote<media::mojom::blink::MediaPlayer> media_player_remote;
    picture_in_picture_element_->BindMediaPlayerReceiver(media_player_remote.InitWithNewEndpointAndPassReceiver());

    picture_in_picture_session_->Update(picture_in_picture_element_->GetWebMediaPlayer()->GetDelegateId(), std::move(media_player_remote),
        picture_in_picture_element_->GetWebMediaPlayer()->GetSurfaceId().value(), picture_in_picture_element_->GetWebMediaPlayer()->NaturalSize(),
        ShouldShowPlayPauseButton(*picture_in_picture_element_));
}

void PictureInPictureControllerImpl::OnWindowSizeChanged(const gfx::Size& size)
{
...
void PictureInPictureControllerImpl::SetMayThrottleIfUndrawnFrames(bool may_throttle)
{
    if (!GetSupplementable()->GetFrame() || !GetSupplementable()->GetFrame()->GetWidgetForLocalRoot()) {
        // Tests do not always have a frame or widget.
        return;
    }
    GetSupplementable()->GetFrame()->GetWidgetForLocalRoot()->SetMayThrottleIfUndrawnFrames(may_throttle);
}
...
PictureInPictureControllerImpl::PictureInPictureControllerImpl(Document& document)
...
bool PictureInPictureControllerImpl::EnsureService()
{
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = GetSupplementable()->GetFrame()->GetTaskRunner(TaskType::kMediaElementEvent);
    GetSupplementable()->GetFrame()->GetBrowserInterfaceBroker().GetInterface(picture_in_picture_service_.BindNewPipeAndPassReceiver(task_runner));
    return true;
}
