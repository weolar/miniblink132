...
LocalDOMWindow* OpenDocumentPictureInPictureWindow(V8TestingScope& v8_scope, Document& document, KURL opener_url = GetOpenerURL())
{
    ...
    document.domWindow()->GetSecurityContext().SetSecurityOriginForTesting(nullptr);
    document.domWindow()->GetSecurityContext().SetSecurityOrigin(SecurityOrigin::Create(opener_url));
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<DOMWindow>>(script_state);
    ExceptionState exception_state(script_state->GetIsolate(), v8::ExceptionContext::kOperation, "DocumentPictureInPicture", "requestWindow");

    v8::Local<v8::Object> v8_object = v8::Object::New(v8_scope.GetIsolate());
    v8_object->Set(v8_scope.GetContext(), V8String(v8_scope.GetIsolate(), "width"), v8::Number::New(v8_scope.GetIsolate(), 640)).Check();
    v8_object->Set(v8_scope.GetContext(), V8String(v8_scope.GetIsolate(), "height"), v8::Number::New(v8_scope.GetIsolate(), 320)).Check();
    DocumentPictureInPictureOptions* options = DocumentPictureInPictureOptions::Create(script_state->GetIsolate(), v8_object, exception_state);
    ...
    controller.CreateDocumentPictureInPictureWindow(script_state, *document.domWindow(), options, resolver);
...
viz::SurfaceId TestSurfaceId()
{
    ...
    return { viz::FrameSinkId(1, 1), viz::LocalSurfaceId(11, base::UnguessableToken::CreateForTesting(0x111111, 0)) };
}
...
class MockPictureInPictureSession : public mojom::blink::PictureInPictureSession {
public:
    MockPictureInPictureSession(mojo::PendingReceiver<mojom::blink::PictureInPictureSession> receiver)
        : receiver_(this, std::move(receiver))
    {
        ON_CALL(*this, Stop(_)).WillByDefault([](StopCallback callback) { std::move(callback).Run(); });
    }
    ~MockPictureInPictureSession() override = default;

    MOCK_METHOD(void, Stop, (StopCallback));
    MOCK_METHOD(void, Update, (uint32_t, mojo::PendingAssociatedRemote<media::mojom::blink::MediaPlayer>, const viz::SurfaceId&, const gfx::Size&, bool));
...
// order for the events to be fired.
class MockPictureInPictureService : public mojom::blink::PictureInPictureService {
public:
    MockPictureInPictureService()
    {
        // Setup default implementations.
        ON_CALL(*this, StartSession(_, _, _, _, _, _, _, _)).WillByDefault(testing::Invoke(this, &MockPictureInPictureService::StartSessionInternal));
    }

    MockPictureInPictureService(const MockPictureInPictureService&) = delete;
    MockPictureInPictureService& operator=(const MockPictureInPictureService&) = delete;

    ~MockPictureInPictureService() override = default;

    void Bind(mojo::ScopedMessagePipeHandle handle)
    {
        receiver_.Bind(mojo::PendingReceiver<mojom::blink::PictureInPictureService>(std::move(handle)));

        session_ = std::make_unique<MockPictureInPictureSession>(session_remote_.InitWithNewPipeAndPassReceiver());
    }

    MOCK_METHOD(void, StartSession,
        (uint32_t, mojo::PendingAssociatedRemote<media::mojom::blink::MediaPlayer>, const viz::SurfaceId&, const gfx::Size&, bool,
            mojo::PendingRemote<mojom::blink::PictureInPictureSessionObserver>, const gfx::Rect&, StartSessionCallback));

    MockPictureInPictureSession& Session()
    {
        return *session_.get();
    }

    void StartSessionInternal(uint32_t, mojo::PendingAssociatedRemote<media::mojom::blink::MediaPlayer>, const viz::SurfaceId&, const gfx::Size&, bool,
        mojo::PendingRemote<mojom::blink::PictureInPictureSessionObserver>, const gfx::Rect& source_bounds, StartSessionCallback callback)
    {
        source_bounds_ = source_bounds;
        std::move(callback).Run(std::move(session_remote_), gfx::Size());
    }

    const gfx::Rect& source_bounds() const
    {
...
class PictureInPictureControllerFrameClient : public test::MediaStubLocalFrameClient {
public:
    static PictureInPictureControllerFrameClient* Create(std::unique_ptr<WebMediaPlayer> player)
    {
        return MakeGarbageCollected<PictureInPictureControllerFrameClient>(std::move(player));
    }

    explicit PictureInPictureControllerFrameClient(std::unique_ptr<WebMediaPlayer> player)
        : test::MediaStubLocalFrameClient(std::move(player))
    {
    }

    PictureInPictureControllerFrameClient(const PictureInPictureControllerFrameClient&) = delete;
    PictureInPictureControllerFrameClient& operator=(const PictureInPictureControllerFrameClient&) = delete;
};
...
public:
    ...
    PictureInPictureControllerPlayer(const PictureInPictureControllerPlayer&) = delete;
    ...
    {
        return kReadyStateHaveMetadata;
    }
    ...
    void set_infinity_duration(bool value)
    {
        infinity_duration_ = value;
    }
...
class PictureInPictureTestWebFrameClient : public frame_test_helpers::TestWebFrameClient {
public:
    explicit PictureInPictureTestWebFrameClient(std::unique_ptr<WebMediaPlayer> web_media_player)
...
public:
    void SetUp() override
    {
        client_ = std::make_unique<PictureInPictureTestWebFrameClient>(std::make_unique<PictureInPictureControllerPlayer>());

        helper_.Initialize(client_.get());

        GetFrame().GetBrowserInterfaceBroker().SetBinderForTesting(
            mojom::blink::PictureInPictureService::Name_, WTF::BindRepeating(&MockPictureInPictureService::Bind, WTF::Unretained(&mock_service_)));
        ...
        if (base::Contains(test_name, "MediaSource")) {
            ...
            auto* descriptor = MakeGarbageCollected<MediaStreamDescriptor>(dummy_tracks, dummy_tracks);
    ...
    {
        GetFrame().GetBrowserInterfaceBroker().SetBinderForTesting(mojom::blink::PictureInPictureService::Name_, {});
    ...
    MockPictureInPictureService& Service()
    {
    ...
    {
        return static_cast<WebFrameWidgetImpl*>(GetDocument().GetFrame()->GetWidgetForLocalRoot());
    }
...
TEST_F(PictureInPictureControllerTestWithWidget, EnterPictureInPictureFiresEvent)
{
    EXPECT_EQ(nullptr, PictureInPictureControllerImpl::From(GetDocument()).PictureInPictureElement());

    WebMediaPlayer* player = Video()->GetWebMediaPlayer();
    EXPECT_CALL(Service(), StartSession(player->GetDelegateId(), _, TestSurfaceId(), player->NaturalSize(), true, _, _, _));

    PictureInPictureControllerImpl::From(GetDocument()).EnterPictureInPicture(Video(), /*promise=*/nullptr);

    MakeGarbageCollected<WaitForEvent>(Video(), event_type_names::kEnterpictureinpicture);

    EXPECT_NE(nullptr, PictureInPictureControllerImpl::From(GetDocument()).PictureInPictureElement());
}

TEST_F(PictureInPictureControllerTestWithWidget, FrameThrottlingIsSetProperlyWithoutSetup)
{
    ...
    PictureInPictureControllerImpl::From(GetDocument()).ExitPictureInPicture(Video(), nullptr /* resolver */);
    MakeGarbageCollected<WaitForEvent>(Video(), event_type_names::kLeavepictureinpicture);
    EXPECT_TRUE(GetWidget()->GetMayThrottleIfUndrawnFramesForTesting());
}

TEST_F(PictureInPictureControllerTestWithWidget, ExitPictureInPictureFiresEvent)
{
    EXPECT_EQ(nullptr, PictureInPictureControllerImpl::From(GetDocument()).PictureInPictureElement());
    ...
    EXPECT_NE(nullptr, PictureInPictureControllerImpl::From(GetDocument()).PictureInPictureElement());
    EXPECT_NE(nullptr, PictureInPictureControllerImpl::From(GetDocument()).pictureInPictureWindow());

    PictureInPictureControllerImpl::From(GetDocument()).ExitPictureInPicture(Video(), nullptr);
    ...
    EXPECT_EQ(nullptr, PictureInPictureControllerImpl::From(GetDocument()).PictureInPictureElement());
    EXPECT_EQ(nullptr, PictureInPictureControllerImpl::From(GetDocument()).pictureInPictureWindow());
}

TEST_F(PictureInPictureControllerTestWithWidget, StartObserving)
{
    EXPECT_FALSE(PictureInPictureControllerImpl::From(GetDocument()).IsSessionObserverReceiverBoundForTesting());
    ...
    EXPECT_TRUE(PictureInPictureControllerImpl::From(GetDocument()).IsSessionObserverReceiverBoundForTesting());
}
...
TEST_F(PictureInPictureControllerTestWithWidget, PlayPauseButton_InfiniteDuration)
{
    EXPECT_EQ(nullptr, PictureInPictureControllerImpl::From(GetDocument()).PictureInPictureElement());
    ...
    WebMediaPlayer* player = Video()->GetWebMediaPlayer();
    EXPECT_CALL(Service(), StartSession(player->GetDelegateId(), _, TestSurfaceId(), player->NaturalSize(), false, _, _, _));
...
TEST_F(PictureInPictureControllerTestWithWidget, PerformMediaPlayerAction)
{
    ...
    Persistent<HTMLVideoElement> video = MakeGarbageCollected<HTMLVideoElement>(*document);
    ...
    // given location.
    frame->GetFrame()->MediaPlayerActionAtViewportPoint(bounds, blink::mojom::MediaPlayerActionType::kPictureInPicture, true);
}

TEST_F(PictureInPictureControllerTestWithWidget, EnterPictureInPictureAfterResettingWMP)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<PictureInPictureWindow>>(scope.GetScriptState());
    auto promise = resolver->Promise();
    PictureInPictureControllerImpl::From(GetDocument()).EnterPictureInPicture(Video(), resolver);

    // Verify rejected with DOMExceptionCode::kInvalidStateError.
    EXPECT_EQ(v8::Promise::kRejected, promise.V8Promise()->State());
    DOMException* dom_exception = V8DOMException::ToWrappable(scope.GetIsolate(), promise.V8Promise()->Result());
    ASSERT_NE(dom_exception, nullptr);
    EXPECT_EQ(static_cast<int>(DOMExceptionCode::kInvalidStateError), dom_exception->code());
}

TEST_F(PictureInPictureControllerTestWithWidget, EnterPictureInPictureProvideSourceBoundsSetToBoundsInWidget)
{
    EXPECT_EQ(nullptr, PictureInPictureControllerImpl::From(GetDocument()).PictureInPictureElement());
...
TEST_F(PictureInPictureControllerTestWithWidget, EnterPictureInPictureProvideSourceBoundsSetToReplacedContentRect)
{
    // Create one image with a size of 10x10px
    SkImageInfo raster_image_info = SkImageInfo::MakeN32Premul(10, 10, SkColorSpace::MakeSRGB());
    sk_sp<SkSurface> surface(SkSurfaces::Raster(raster_image_info));
    ImageResourceContent* image_content = ImageResourceContent::CreateLoaded(UnacceleratedStaticBitmapImage::Create(surface->makeImageSnapshot()).get());
    ...
    url_test_helpers::RegisterMockedErrorURLLoad(url_test_helpers::ToKURL(kPosterUrl));
...
TEST_F(PictureInPictureControllerTestWithWidget, VideoIsNotAllowedIfAutoPip)
{
    EXPECT_EQ(PictureInPictureControllerImpl::Status::kEnabled,
        PictureInPictureControllerImpl::From(GetDocument()).IsElementAllowed(*Video(), /*report_failure=*/false));
...
#if !BUILDFLAG(TARGET_OS_IS_ANDROID)
TEST_F(PictureInPictureControllerTestWithWidget, DocumentPiPDoesNotAllowVizThrottling)
{
    EXPECT_TRUE(GetWidget()->GetMayThrottleIfUndrawnFramesForTesting());
    ...
    ScriptState::Scope entered_context_scope(script_state);
    LocalFrame::NotifyUserActivation(&GetFrame(), mojom::UserActivationNotificationType::kTest);
...
TEST_F(PictureInPictureControllerTestWithWidget, DocumentPiPDoesOpenWithFileUrl)
{
    V8TestingScope v8_scope;
    ScriptState* script_state = ToScriptStateForMainWorld(GetDocument().GetFrame());
    ScriptState::Scope entered_context_scope(script_state);
    LocalFrame::NotifyUserActivation(&GetFrame(), mojom::UserActivationNotificationType::kTest);
    auto* pip = OpenDocumentPictureInPictureWindow(v8_scope, GetDocument(), KURL("file://my/file.html"));
    EXPECT_TRUE(pip);
}

class PictureInPictureControllerChromeClient : public RenderingTestChromeClient {
public:
    ...
    Page* CreateWindowDelegate(LocalFrame*, const FrameLoadRequest&, const AtomicString&, const WebWindowFeatures&, network::mojom::blink::WebSandboxFlags,
        const SessionStorageNamespaceId&, bool& consumed_user_gesture) override
    {
...
public:
    void SetUp() override
    {
        chrome_client_ = MakeGarbageCollected<PictureInPictureControllerChromeClient>();
        dummy_page_holder_ = std::make_unique<DummyPageHolder>(gfx::Size(), chrome_client_);
...
TEST_F(PictureInPictureControllerTestWithChromeClient, CreateDocumentPictureInPictureWindow)
{
    EXPECT_EQ(nullptr, PictureInPictureControllerImpl::From(GetDocument()).pictureInPictureWindow());
    V8TestingScope v8_scope;
    LocalFrame::NotifyUserActivation(&GetFrame(), mojom::UserActivationNotificationType::kTest);
    auto* pictureInPictureWindow = OpenDocumentPictureInPictureWindow(v8_scope, GetDocument());
...

    {
        ...
        ExceptionState exception_state(ToScriptStateForMainWorld(document->GetFrame())->GetIsolate(), v8::ExceptionContext::kOperation, "Window", "resizeTo");
...

    {
        ...
        EXPECT_CALL(GetPipChromeClient(), SetWindowRect(_, _));
        LocalFrame::NotifyUserActivation(document->GetFrame(), mojom::UserActivationNotificationType::kTest);
        ExceptionState exception_state(ToScriptStateForMainWorld(document->GetFrame())->GetIsolate(), v8::ExceptionContext::kOperation, "Window", "resizeBy");
    ...
    // Make sure that the `window` attribute returns the window.
    {
        ScriptState* script_state = ToScriptStateForMainWorld(GetDocument().GetFrame());
        ScriptState::Scope entered_context_scope(script_state);
        EXPECT_EQ(pictureInPictureWindow, DocumentPictureInPicture::From(*GetDocument().domWindow())->window(script_state));
    }
}

TEST_F(PictureInPictureControllerTestWithChromeClient, CreateDocumentPictureInPictureWindowThrowsExceptionWhenDomWindowNull)
{
    ...
    document.domWindow()->GetSecurityContext().SetSandboxFlags(network::mojom::blink::WebSandboxFlags::kPopups);
    ...
    DOMException* dom_exception = V8DOMException::ToWrappable(script_state->GetIsolate(), promise.V8Promise()->Result());
    ASSERT_NE(dom_exception, nullptr);
    EXPECT_EQ(dom_exception->code(), static_cast<int>(DOMExceptionCode::kInvalidStateError));
}
...
TEST_F(PictureInPictureControllerTestWithChromeClient, OpenDocumentPiPTwiceSynchronouslyDoesNotCrash)
{
    V8TestingScope v8_scope;
    LocalFrame::NotifyUserActivation(&GetFrame(), mojom::UserActivationNotificationType::kTest);
    auto* pictureInPictureWindow1 = OpenDocumentPictureInPictureWindow(v8_scope, GetDocument());
    LocalFrame::NotifyUserActivation(&GetFrame(), mojom::UserActivationNotificationType::kTest);
    auto* pictureInPictureWindow2 = OpenDocumentPictureInPictureWindow(v8_scope, GetDocument());
...
TEST_F(PictureInPictureControllerTestWithChromeClient, CopiesAutoplayFlags)
{
    ...
    auto* pictureInPictureWindow = OpenDocumentPictureInPictureWindow(v8_scope, GetDocument());
    EXPECT_EQ(pictureInPictureWindow->document()->GetPage()->AutoplayFlags(), flags);
}

TEST_F(PictureInPictureControllerTestWithChromeClient, CopiesCompatibilityMode_Quirks)
{
    V8TestingScope v8_scope;
    LocalFrame::NotifyUserActivation(&GetFrame(), mojom::UserActivationNotificationType::kTest);
    ...
    auto* pictureInPictureWindow = OpenDocumentPictureInPictureWindow(v8_scope, GetDocument());
    EXPECT_EQ(pictureInPictureWindow->document()->GetCompatibilityMode(), Document::kQuirksMode);
}

TEST_F(PictureInPictureControllerTestWithChromeClient, CopiesCompatibilityMode_NoQuirks)
{
    V8TestingScope v8_scope;
    LocalFrame::NotifyUserActivation(&GetFrame(), mojom::UserActivationNotificationType::kTest);
    ...
    auto* pictureInPictureWindow = OpenDocumentPictureInPictureWindow(v8_scope, GetDocument());
    EXPECT_EQ(pictureInPictureWindow->document()->GetCompatibilityMode(), Document::kNoQuirksMode);
}
#endif // !BUILDFLAG(TARGET_OS_IS_ANDROID)
