...
class MODULES_EXPORT PictureInPictureControllerImpl : public PictureInPictureController,
                                                      public ExecutionContextClient,
                                                      public blink::mojom::blink::PictureInPictureSessionObserver {
public:
    explicit PictureInPictureControllerImpl(Document&);

    PictureInPictureControllerImpl(const PictureInPictureControllerImpl&) = delete;
    PictureInPictureControllerImpl& operator=(const PictureInPictureControllerImpl&) = delete;
    ...
    void CreateDocumentPictureInPictureWindow(ScriptState*, LocalDOMWindow&, DocumentPictureInPictureOptions*, ScriptPromiseResolver<DOMWindow>*);
#endif // !BUILDFLAG(TARGET_OS_IS_ANDROID)

    // Implementation of PictureInPictureController.
    void EnterPictureInPicture(HTMLVideoElement*, ScriptPromiseResolver<PictureInPictureWindow>*) override;
    void ExitPictureInPicture(HTMLVideoElement*, ScriptPromiseResolver<IDLUndefined>*) override;
    ...
    Status IsElementAllowed(const HTMLVideoElement&, bool report_failure) const override;
#if !BUILDFLAG(TARGET_OS_IS_ANDROID)
...
private:
    void OnEnteredPictureInPicture(
        HTMLVideoElement*, ScriptPromiseResolver<PictureInPictureWindow>*, mojo::PendingRemote<mojom::blink::PictureInPictureSession>, const gfx::Size&);
    ...
    class DocumentPictureInPictureObserver final : public GarbageCollected<DocumentPictureInPictureObserver>, public ContextLifecycleObserver {
    public:
        // Notify `controller` when our context is destroyed.
        explicit DocumentPictureInPictureObserver(PictureInPictureControllerImpl* controller);
    ...
    HeapMojoReceiver<mojom::blink::PictureInPictureSessionObserver, PictureInPictureControllerImpl> session_observer_receiver_;

    // Picture-in-Picture service living in the browser process.
    HeapMojoRemote<mojom::blink::PictureInPictureService> picture_in_picture_service_;

    // Instance of the Picture-in-Picture session sent back by the service. This
    // is for video-only PiP.
    HeapMojoRemote<mojom::blink::PictureInPictureSession> picture_in_picture_session_;
};
