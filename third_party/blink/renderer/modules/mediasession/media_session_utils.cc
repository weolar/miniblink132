...
HeapVector<Member<MediaImage>> ProcessArtworkVector(ScriptState* script_state, const HeapVector<Member<MediaImage>>& artwork, ExceptionState& exception_state)
{
    ...
    for (MediaImage* image : processed_artwork) {
        ...
        if (!url.IsValid()) {
            exception_state.ThrowTypeError("'" + image->src() + "' can't be resolved to a valid URL.");
            return {};
        }
        image->setSrc(url);
    }
