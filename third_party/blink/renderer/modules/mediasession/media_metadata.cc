...
// static
MediaMetadata* MediaMetadata::Create(ScriptState* script_state, const MediaMetadataInit* metadata, ExceptionState& exception_state)
{
    return MakeGarbageCollected<MediaMetadata>(script_state, metadata, exception_state);
}

MediaMetadata::MediaMetadata(ScriptState* script_state, const MediaMetadataInit* metadata, ExceptionState& exception_state)
    : notify_session_timer_(ExecutionContext::From(script_state)->GetTaskRunner(TaskType::kMiscPlatformAPI), this, &MediaMetadata::NotifySessionTimerFired)
{
    ...
    SetChapterInfoFromInit(script_state, metadata->chapterInfo(), exception_state);
}
...
const HeapVector<Member<ChapterInformation>>& MediaMetadata::chapterInfo() const
{
    return chapterInfo_;
}

v8::LocalVector<v8::Value> MediaMetadata::artwork(ScriptState* script_state) const
{
    v8::LocalVector<v8::Value> result(script_state->GetIsolate(), artwork_.size());

    for (wtf_size_t i = 0; i < artwork_.size(); ++i) {
        result[i] = FreezeV8Object(ToV8Traits<MediaImage>::ToV8(script_state, artwork_[i]), script_state->GetIsolate());
    }

    return result;
}

v8::LocalVector<v8::Value> MediaMetadata::chapterInfo(ScriptState* script_state) const
{
    v8::LocalVector<v8::Value> result(script_state->GetIsolate(), chapterInfo_.size());

    for (wtf_size_t i = 0; i < chapterInfo_.size(); ++i) {
        result[i] = FreezeV8Object(ToV8Traits<blink::ChapterInformation>::ToV8(script_state, chapterInfo_[i]), script_state->GetIsolate());
    }
...
void MediaMetadata::setArtwork(ScriptState* script_state, const HeapVector<Member<MediaImage>>& artwork, ExceptionState& exception_state)
{
...
void MediaMetadata::SetArtworkInternal(ScriptState* script_state, const HeapVector<Member<MediaImage>>& artwork, ExceptionState& exception_state)
{
    HeapVector<Member<MediaImage>> processed_artwork = media_session_utils::ProcessArtworkVector(script_state, artwork, exception_state);
...
void MediaMetadata::SetChapterInfoFromInit(
    ScriptState* script_state, const HeapVector<Member<ChapterInformationInit>>& chapter_info, ExceptionState& exception_state)
{
    HeapVector<Member<ChapterInformation>> processed_chapters;
    for (ChapterInformationInit* init_chapter : chapter_info) {
        auto* chapter = ChapterInformation::From(script_state, init_chapter, exception_state);
