...
public:
    ...
    HeapMojoRemote<mojom::blink::MediaSessionService> CreateRemoteAndBind(
        ContextLifecycleNotifier* notifier, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
    ...
    void SetClient(mojo::PendingRemote<mojom::blink::MediaSessionClient> client) override
    {
    }
    void SetPlaybackState(mojom::blink::MediaSessionPlaybackState state) override
    {
    }
    MOCK_METHOD1(SetPositionState, void(media_session::mojom::blink::MediaPositionPtr));
...
public:
    ...
    void SetUp() override
    {
        ...
        media_session_ = MediaSession::mediaSession(*GetFrame().DomWindow()->navigator());
        media_session_->service_
            = mock_service_->CreateRemoteAndBind(GetFrame().DomWindow(), GetFrame().DomWindow()->GetTaskRunner(TaskType::kMiscPlatformAPI));
        media_session_->clock_ = &test_clock_;
    }

    void SetPositionState(double duration, double position, double playback_rate)
    {
    ...
    void SetPositionStateThrowsException(double duration, double position, double playback_rate)
    {
        auto* position_state = MediaPositionState::Create();
        position_state->setDuration(duration);
        position_state->setPosition(position);
    ...
    {
        NonThrowableExceptionState exception_state;
        media_session_->setPositionState(MediaPositionState::Create(), exception_state);
    }
...
{
    base::RunLoop loop;
    EXPECT_CALL(service(), SetPositionState(_)).WillOnce(testing::Invoke([&](auto position_state) {
