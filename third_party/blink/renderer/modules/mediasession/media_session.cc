...
V8MediaSessionPlaybackState::Enum MediaSessionPlaybackStateToEnum(mojom::blink::MediaSessionPlaybackState state)
{
    switch (state) {
...
mojom::blink::MediaSessionPlaybackState EnumToMediaSessionPlaybackState(const V8MediaSessionPlaybackState::Enum& state)
{
    switch (state) {
...
{
    MediaSession* supplement = Supplement<Navigator>::From<MediaSession>(navigator);
    if (!supplement) {
...
void MediaSession::setPlaybackState(const V8MediaSessionPlaybackState& playback_state)
{
...
{
    return V8MediaSessionPlaybackState(MediaSessionPlaybackStateToEnum(playback_state_));
}
...
void MediaSession::OnMetadataChanged()
{
    ...
    service->SetMetadata(MediaMetadataSanitizer::SanitizeAndConvertToMojo(metadata_, context));
}

void MediaSession::setActionHandler(const V8MediaSessionAction& action, V8MediaSessionActionHandler* handler, ExceptionState& exception_state)
{
    ...
    if (action_value == V8MediaSessionAction::Enum::kSkipad) {
        ...
        if (!RuntimeEnabledFeatures::SkipAdEnabled(window)) {
            exception_state.ThrowTypeError("The provided value 'skipad' is not a valid enum "
    ...
    if (!RuntimeEnabledFeatures::MediaSessionEnterPictureInPictureEnabled()) {
        if (action_value == V8MediaSessionAction::Enum::kEnterpictureinpicture) {
            exception_state.ThrowTypeError("The provided value 'enterpictureinpicture'"
                                           " is not a valid enum "
                                           "value of type MediaSessionAction.");
            return;
        }
...
void MediaSession::setPositionState(MediaPositionState* position_state, ExceptionState& exception_state)
{
    // If the dictionary is empty / null then we should reset the position state.
    if (!position_state->hasDuration() && !position_state->hasPlaybackRate() && !position_state->hasPosition()) {
    ...
    if (position_state->duration() < 0) {
        exception_state.ThrowTypeError("The provided duration cannot be less than zero.");
        return;
    }

    // The position cannot be negative.
    if (position_state->hasPosition() && position_state->position() < 0) {
        exception_state.ThrowTypeError("The provided position cannot be less than zero.");
        return;
    }

    // The position cannot be greater than the duration.
    if (position_state->hasPosition() && position_state->position() > position_state->duration()) {
        exception_state.ThrowTypeError("The provided position cannot be greater than the duration.");
        return;
    }

    // The playback rate cannot be equal to zero.
    if (position_state->hasPlaybackRate() && position_state->playbackRate() == 0) {
        exception_state.ThrowTypeError("The provided playbackRate cannot be equal to zero.");
        return;
    }

    position_state_ = mojo::ConvertTo<media_session::mojom::blink::MediaPositionPtr>(position_state);
...
void MediaSession::setMicrophoneActive(bool active)
{
    ...
    if (active) {
        service->SetMicrophoneState(media_session::mojom::MicrophoneState::kUnmuted);
    } else {
...
void MediaSession::NotifyActionChange(V8MediaSessionAction::Enum action, ActionChangeType type)
{
    mojom::blink::MediaSessionService* service = GetService();
    if (!service)
        return;
...
base::TimeDelta MediaSession::GetPositionNow() const
{
    ...
    const base::TimeDelta elapsed_time = position_state_->playback_rate * (now - position_state_->last_updated_time);
    const base::TimeDelta updated_position = position_state_->position + elapsed_time;
...
void MediaSession::RecalculatePositionState(bool was_set)
{
    ...
    double new_playback_rate = playback_state_ == mojom::blink::MediaSessionPlaybackState::PAUSED ? 0.0 : declared_playback_rate_;
...
mojom::blink::MediaSessionService* MediaSession::GetService()
{
    ...
    window->GetBrowserInterfaceBroker().GetInterface(service_.BindNewPipeAndPassReceiver(task_runner));
    if (service_.get())
        service_->SetClient(client_receiver_.BindNewPipeAndPassRemote(task_runner));
    return service_.get();
}

void MediaSession::DidReceiveAction(media_session::mojom::blink::MediaSessionAction action, mojom::blink::MediaSessionActionDetailsPtr details)
{
    LocalDOMWindow* window = GetSupplementable()->DomWindow();
    if (!window)
        return;
    ...
    const auto* blink_details
        = mojo::TypeConverter<const blink::MediaSessionActionDetails*, blink::mojom::blink::MediaSessionActionDetailsPtr>::ConvertWithV8Action(
            details, v8_action);
