...
public:
    ...
    // returns nullptr.
    static ClipboardItem* Create(const HeapVector<std::pair<String, ScriptPromise<V8UnionBlobOrString>>>& representations, ExceptionState& exception_state);
    ...
    explicit ClipboardItem(const HeapVector<std::pair<String, ScriptPromise<V8UnionBlobOrString>>>& representations);
    ...
    ScriptPromise<Blob> getType(ScriptState* script_state, const String& type, ExceptionState& exception_state) const;
    ...
    const HeapVector<std::pair<String, ScriptPromise<V8UnionBlobOrString>>>& GetRepresentations() const
    {
        return representations_;
    }

    // Returns the custom formats that have a "web " prefix.
    const Vector<String>& CustomFormats() const
    {
        return custom_format_types_;
    }
...
private:
    ...
    HeapVector<std::pair<String, ScriptPromise<V8UnionBlobOrString>>> representations_;
