...
public:
    ClipboardImageWriter(SystemClipboard* system_clipboard, ClipboardPromise* promise)
...
private:
    void StartWrite(DOMArrayBuffer* raw_data, scoped_refptr<base::SingleThreadTaskRunner> task_runner) override
    {
        ...
        worker_pool::PostTask(
            FROM_HERE, CrossThreadBindOnce(&ClipboardImageWriter::DecodeOnBackgroundThread, std::move(contents), MakeCrossThreadHandle(this), task_runner));
    }
    static void DecodeOnBackgroundThread(
        ArrayBufferContents png_data, CrossThreadHandle<ClipboardImageWriter> writer, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    {
        DCHECK(!IsMainThread());
        std::unique_ptr<ImageDecoder> decoder
            = ImageDecoder::Create(SegmentReader::CreateFromSkData(SkData::MakeWithoutCopy(png_data.Data(), png_data.DataLength())),
                /*data_complete=*/true, ImageDecoder::kAlphaPremultiplied, ImageDecoder::kDefaultBitDepth, ColorBehavior::kTag, cc::AuxImage::kDefault,
        ...
        PostCrossThreadTask(
            *task_runner, FROM_HERE, CrossThreadBindOnce(&ClipboardImageWriter::Write, MakeUnwrappingCrossThreadHandle(std::move(writer)), std::move(image)));
    }
...
public:
    ClipboardTextWriter(SystemClipboard* system_clipboard, ClipboardPromise* promise)
        : ClipboardWriter(system_clipboard, promise)
    {
    }
...
private:
    void StartWrite(DOMArrayBuffer* raw_data, scoped_refptr<base::SingleThreadTaskRunner> task_runner) override
    {
        ...
        ArrayBufferContents contents = *raw_data->Content();
        worker_pool::PostTask(
            FROM_HERE, CrossThreadBindOnce(&ClipboardTextWriter::DecodeOnBackgroundThread, std::move(contents), MakeCrossThreadHandle(this), task_runner));
    }
    static void DecodeOnBackgroundThread(
        ArrayBufferContents raw_data, CrossThreadHandle<ClipboardTextWriter> writer, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    {
        DCHECK(!IsMainThread());

        String wtf_string = String::FromUTF8(raw_data.ByteSpan());
        PostCrossThreadTask(*task_runner, FROM_HERE,
            CrossThreadBindOnce(&ClipboardTextWriter::Write, MakeUnwrappingCrossThreadHandle(std::move(writer)), std::move(wtf_string)));
    }
...
public:
    ClipboardHtmlWriter(SystemClipboard* system_clipboard, ClipboardPromise* promise)
        : ClipboardWriter(system_clipboard, promise)
    {
    }
...
private:
    void StartWrite(DOMArrayBuffer* html_data, scoped_refptr<base::SingleThreadTaskRunner> task_runner) override
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        ...
        const Document* doc = dom_parser->parseFromString(html_string, V8SupportedType(V8SupportedType::Enum::kTextHtml));
        DCHECK(doc);
...
public:
    ClipboardSvgWriter(SystemClipboard* system_clipboard, ClipboardPromise* promise)
        : ClipboardWriter(system_clipboard, promise)
    {
    }
...
private:
    void StartWrite(DOMArrayBuffer* svg_data, scoped_refptr<base::SingleThreadTaskRunner> task_runner) override
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        ...
        const Document* doc = dom_parser->parseFromString(svg_string, V8SupportedType(V8SupportedType::Enum::kImageSvgXml));
...
public:
    ClipboardCustomFormatWriter(SystemClipboard* system_clipboard, ClipboardPromise* promise, const String& mime_type)
        : ClipboardWriter(system_clipboard, promise)
...
private:
    void StartWrite(DOMArrayBuffer* custom_format_data, scoped_refptr<base::SingleThreadTaskRunner> task_runner) override
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    void Write(DOMArrayBuffer* custom_format_data)
    {
        ...
        if (custom_format_data->ByteLength() >= mojom::blink::ClipboardHost::kMaxDataSize) {
            promise_->RejectFromReadOrDecodeFailure();
            return;
        }
        mojo_base::BigBuffer buffer(base::make_span(static_cast<uint8_t*>(custom_format_data->Data()), custom_format_data->ByteLength()));
        system_clipboard()->WriteUnsanitizedCustomFormat(mime_type_, std::move(buffer));
        promise_->CompleteWriteRepresentation();
    }
...
// static
ClipboardWriter* ClipboardWriter::Create(SystemClipboard* system_clipboard, const String& mime_type, ClipboardPromise* promise)
{
    ...
    if (!web_custom_format.empty()) {
        ...
        return MakeGarbageCollected<ClipboardCustomFormatWriter>(system_clipboard, promise, web_custom_format);
    }

    if (mime_type == kMimeTypeImagePng) {
        return MakeGarbageCollected<ClipboardImageWriter>(system_clipboard, promise);
    }
...
ClipboardWriter::ClipboardWriter(SystemClipboard* system_clipboard, ClipboardPromise* promise)
    : promise_(promise)
    , clipboard_task_runner_(promise->GetExecutionContext()->GetTaskRunner(TaskType::kUserInteraction))
    , file_reading_task_runner_(promise->GetExecutionContext()->GetTaskRunner(TaskType::kFileReading))
...
void ClipboardWriter::WriteToSystem(V8UnionBlobOrString* clipboard_item_data)
{
    ...
    if (clipboard_item_data->IsBlob()) {
        DCHECK(!file_reader_);
        file_reader_ = MakeGarbageCollected<FileReaderLoader>(this, std::move(file_reading_task_runner_));
        ...
        StartWrite(DOMArrayBuffer::Create(clipboard_item_data->GetAsString().Span8()), clipboard_task_runner_);
    } else {
        NOTREACHED();
    }
}
