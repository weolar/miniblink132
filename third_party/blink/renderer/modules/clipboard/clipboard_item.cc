...
class UnionToBlobResolverFunction final : public ThenCallable<V8UnionBlobOrString, UnionToBlobResolverFunction, Blob> {
public:
...
// static
ClipboardItem* ClipboardItem::Create(const HeapVector<std::pair<String, ScriptPromise<V8UnionBlobOrString>>>& representations, ExceptionState& exception_state)
{
...
ClipboardItem::ClipboardItem(const HeapVector<std::pair<String, ScriptPromise<V8UnionBlobOrString>>>& representations)
{
    for (const auto& representation : representations) {
        String web_custom_format = Clipboard::ParseWebCustomFormat(representation.first);
        if (web_custom_format.empty()) {
            ...
            representations_.emplace_back(representation.first, representation.second);
        } else {
            ...
            // TODO(caseq,japhet): we can't pass typed promises from bindings yet, but
            // when we can, the type cast below should go away.
            String web_custom_format_string = String::Format("%s%s", ui::kWebClipboardFormatPrefix, web_custom_format.Utf8().c_str());
            representations_.emplace_back(web_custom_format_string, representation.second);
...
ScriptPromise<Blob> ClipboardItem::getType(ScriptState* script_state, const String& type, ExceptionState& exception_state) const
{
    for (const auto& item : representations_) {
        if (type == item.first) {
            return item.second.Then(script_state, MakeGarbageCollected<UnionToBlobResolverFunction>(type));
        }
    }

    exception_state.ThrowDOMException(DOMExceptionCode::kNotFoundError, "The type was not found");
...
bool ClipboardItem::supports(const String& type)
{
    ...
    return type == kMimeTypeImagePng || type == kMimeTypeTextPlain || type == kMimeTypeTextHTML || type == kMimeTypeImageSvg;
}
