...
class ClipboardPromise::ClipboardItemDataPromiseFulfill final : public ThenCallable<IDLSequence<V8UnionBlobOrString>, ClipboardItemDataPromiseFulfill> {
public:
    ...
    {
        ThenCallable<IDLSequence<V8UnionBlobOrString>, ClipboardItemDataPromiseFulfill>::Trace(visitor);
        visitor->Trace(clipboard_promise_);
    }

    void React(ScriptState* script_state, HeapVector<Member<V8UnionBlobOrString>> clipboard_item_list)
    {
        auto* list_copy = MakeGarbageCollected<HeapVector<Member<V8UnionBlobOrString>>>(std::move(clipboard_item_list));
...
class ClipboardPromise::ClipboardItemDataPromiseReject final : public ThenCallable<IDLAny, ClipboardItemDataPromiseReject> {
public:
...
ScriptPromise<IDLSequence<ClipboardItem>> ClipboardPromise::CreateForRead(
    ExecutionContext* context, ScriptState* script_state, ClipboardUnsanitizedFormats* formats, ExceptionState& exception_state)
{
    if (!script_state->ContextIsValid()) {
        return ScriptPromise<IDLSequence<ClipboardItem>>();
    }
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<ClipboardItem>>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();
    ClipboardPromise* clipboard_promise = MakeGarbageCollected<ClipboardPromise>(context, resolver, exception_state);
    clipboard_promise->HandleRead(formats);
    return promise;
}

// static
ScriptPromise<IDLString> ClipboardPromise::CreateForReadText(ExecutionContext* context, ScriptState* script_state, ExceptionState& exception_state)
{
    if (!script_state->ContextIsValid()) {
        return EmptyPromise();
    }
...
ScriptPromise<IDLUndefined> ClipboardPromise::CreateForWrite(
    ExecutionContext* context, ScriptState* script_state, const HeapVector<Member<ClipboardItem>>& items, ExceptionState& exception_state)
{
    if (!script_state->ContextIsValid()) {
        return EmptyPromise();
    }
...
ScriptPromise<IDLUndefined> ClipboardPromise::CreateForWriteText(
    ExecutionContext* context, ScriptState* script_state, const String& data, ExceptionState& exception_state)
{
    if (!script_state->ContextIsValid()) {
        return EmptyPromise();
    }
...
ClipboardPromise::ClipboardPromise(ExecutionContext* context, ScriptPromiseResolverBase* resolver, ExceptionState& exception_state)
...
void ClipboardPromise::WriteNextRepresentation()
{
    ...
    const String& type = clipboard_item_data_[clipboard_representation_index_].first;
    const Member<V8UnionBlobOrString>& clipboard_item_data = clipboard_item_data_[clipboard_representation_index_].second;

    DCHECK(!clipboard_writer_);
    clipboard_writer_ = ClipboardWriter::Create(local_frame->GetSystemClipboard(), type, this);
    if (!clipboard_writer_) {
        script_promise_resolver_->RejectWithDOMException(DOMExceptionCode::kNotAllowedError, "Type " + type + " is not supported");
        return;
    }
...
void ClipboardPromise::RejectFromReadOrDecodeFailure()
{
    ...
    ScriptState::Scope scope(GetScriptState());
    String exception_text = RuntimeEnabledFeatures::ClipboardItemWithDOMStringSupportEnabled() ? "Failed to read or decode ClipboardItemData for type "
                                                                                               : "Failed to read or decode Blob for clipboard item type ";
    script_promise_resolver_->RejectWithDOMException(
        DOMExceptionCode::kDataError, exception_text + clipboard_item_data_[clipboard_representation_index_].first + ".");
}
...
void ClipboardPromise::HandleRead(ClipboardUnsanitizedFormats* formats)
{
    ...
    if (formats && formats->hasUnsanitized() && !formats->unsanitized().empty()) {
        ...
        if (unsanitized_formats.size() > 1) {
            script_promise_resolver_->RejectWithDOMException(DOMExceptionCode::kNotAllowedError, "Reading multiple unsanitized formats is not supported.");
            return;
        }
        if (unsanitized_formats[0] != kMimeTypeTextHTML) {
            script_promise_resolver_->RejectWithDOMException(
                DOMExceptionCode::kNotAllowedError, "The unsanitized type " + unsanitized_formats[0] + " is not supported.");
            return;
        }
    ...
    ValidatePreconditions(mojom::blink::PermissionName::CLIPBOARD_READ,
        /*will_be_sanitized=*/false, WTF::BindOnce(&ClipboardPromise::HandleReadWithPermission, WrapPersistent(this)));
}
...
void ClipboardPromise::HandleReadText()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ValidatePreconditions(mojom::blink::PermissionName::CLIPBOARD_READ,
        /*will_be_sanitized=*/true, WTF::BindOnce(&ClipboardPromise::HandleReadTextWithPermission, WrapPersistent(this)));
}

void ClipboardPromise::HandleWrite(const HeapVector<Member<ClipboardItem>>& clipboard_items)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (static_cast<int>(write_custom_format_types_.size()) > ui::kMaxRegisteredClipboardFormats) {
        script_promise_resolver_->RejectWithDOMException(
            DOMExceptionCode::kNotAllowedError, "Number of custom formats exceeds the max limit which is set to 100.");
        return;
    }

    // Input in standard formats is sanitized, so the write will be sanitized
    // unless there are custom formats.
    ValidatePreconditions(mojom::blink::PermissionName::CLIPBOARD_WRITE,
        /*will_be_sanitized=*/write_custom_format_types_.empty(), WTF::BindOnce(&ClipboardPromise::HandleWriteWithPermission, WrapPersistent(this)));
}
...
void ClipboardPromise::HandleWriteText(const String& data)
{
    ...
    plain_text_ = data;
    ValidatePreconditions(mojom::blink::PermissionName::CLIPBOARD_WRITE,
        /*will_be_sanitized=*/true, WTF::BindOnce(&ClipboardPromise::HandleWriteTextWithPermission, WrapPersistent(this)));
}

void ClipboardPromise::HandleReadWithPermission(mojom::blink::PermissionStatus status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (status != mojom::blink::PermissionStatus::GRANTED) {
        script_promise_resolver_->RejectWithDOMException(DOMExceptionCode::kNotAllowedError, "Read permission denied.");
        return;
    }

    SystemClipboard* system_clipboard = GetLocalFrame()->GetSystemClipboard();
    system_clipboard->ReadAvailableCustomAndStandardFormats(WTF::BindOnce(&ClipboardPromise::OnReadAvailableFormatNames, WrapPersistent(this)));
}
...
void ClipboardPromise::ResolveRead()
{
    ...
    base::UmaHistogramCounts100("Blink.Clipboard.Read.NumberOfFormats", clipboard_item_data_.size());
    ...
    HeapVector<Member<ClipboardItem>> clipboard_items = { MakeGarbageCollected<ClipboardItem>(items) };
    script_promise_resolver_->DowncastTo<IDLSequence<ClipboardItem>>()->Resolve(clipboard_items);
}

void ClipboardPromise::OnReadAvailableFormatNames(const Vector<String>& format_names)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!GetExecutionContext()) {
        return;
...
void ClipboardPromise::ReadNextRepresentation()
{
    ...
    ClipboardReader* clipboard_reader
        = ClipboardReader::Create(GetLocalFrame()->GetSystemClipboard(), clipboard_item_data_[clipboard_representation_index_].first, this,
...
void ClipboardPromise::OnRead(Blob* blob)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (blob) {
        clipboard_item_data_[clipboard_representation_index_].second = MakeGarbageCollected<V8UnionBlobOrString>(blob);
    }
    ++clipboard_representation_index_;
    ReadNextRepresentation();
}

void ClipboardPromise::HandleReadTextWithPermission(mojom::blink::PermissionStatus status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!GetExecutionContext()) {
        return;
    ...
    String text = GetLocalFrame()->GetSystemClipboard()->ReadPlainText(mojom::blink::ClipboardBuffer::kStandard);
    script_promise_resolver_->DowncastTo<IDLString>()->Resolve(text);
}

void ClipboardPromise::HandlePromiseWrite(HeapVector<Member<V8UnionBlobOrString>>* clipboard_item_list)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    GetClipboardTaskRunner()->PostTask(
        FROM_HERE, WTF::BindOnce(&ClipboardPromise::WriteClipboardItemData, WrapPersistent(this), WrapPersistent(clipboard_item_list)));
}

void ClipboardPromise::WriteClipboardItemData(HeapVector<Member<V8UnionBlobOrString>>* clipboard_item_list)
{
    ...
    for (const auto& clipboard_item_data : *clipboard_item_list) {
        if (!RuntimeEnabledFeatures::ClipboardItemWithDOMStringSupportEnabled() && !clipboard_item_data->IsBlob()) {
            script_promise_resolver_->RejectWithDOMException(DOMExceptionCode::kNotAllowedError, "DOMString is not supported in ClipboardItem");
            return;
        }
        ...
        if (clipboard_item_data->IsBlob()) {
            ...
            if ((!type_with_args.Contains(type.LowerASCII()) && web_custom_format.empty())
                || (!web_custom_format.empty() && !type_with_args.Contains(web_custom_format))) {
                script_promise_resolver_->RejectWithDOMException(
                    DOMExceptionCode::kNotAllowedError, "Type " + type + " does not match the blob's type " + type_with_args);
                return;
            }
        }
...
void ClipboardPromise::HandleWriteWithPermission(mojom::blink::PermissionStatus status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!GetExecutionContext()) {
        return;
    ...
    if (status != mojom::blink::PermissionStatus::GRANTED) {
        script_promise_resolver_->RejectWithDOMException(DOMExceptionCode::kNotAllowedError, "Write permission denied.");
        return;
    }

    HeapVector<ScriptPromise<V8UnionBlobOrString>> promise_list;
    promise_list.ReserveInitialCapacity(clipboard_item_data_with_promises_.size());
    write_clipboard_item_types_.ReserveInitialCapacity(clipboard_item_data_with_promises_.size());
    ...
    for (const auto& type_and_promise : clipboard_item_data_with_promises_) {
        ...
        if (!ClipboardItem::supports(type)) {
            script_promise_resolver_->RejectWithDOMException(DOMExceptionCode::kNotAllowedError, "Type " + type + " not supported on write.");
            return;
        }
    }
    ScriptState* script_state = GetScriptState();
    ...
    PromiseAll<V8UnionBlobOrString>::Create(script_state, promise_list)
        .React(script_state, MakeGarbageCollected<ClipboardItemDataPromiseFulfill>(this), MakeGarbageCollected<ClipboardItemDataPromiseReject>(this));
}

void ClipboardPromise::HandleWriteTextWithPermission(mojom::blink::PermissionStatus status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!GetExecutionContext()) {
        return;
...
scoped_refptr<base::SingleThreadTaskRunner> ClipboardPromise::GetClipboardTaskRunner()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void ClipboardPromise::ContextDestroyed()
{
    ...
    script_promise_resolver_->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotAllowedError, "Document detached."));
