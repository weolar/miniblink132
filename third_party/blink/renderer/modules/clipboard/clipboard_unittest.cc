...
public:
    ...
    void BindMockPermissionService(ExecutionContext* executionContext)
    {
        executionContext->GetBrowserInterfaceBroker().SetBinderForTesting(
            mojom::blink::PermissionService::Name_, WTF::BindRepeating(&MockClipboardPermissionService::BindRequest, WTF::Unretained(&permission_service_)));
    }
...
TEST_F(ClipboardTest, ClipboardPromiseReadText)
{
    ...
    EXPECT_CALL(permission_service_, RequestPermission).WillOnce(WithArg<2>(Invoke([](mojom::blink::PermissionService::RequestPermissionCallback callback) {
    ...
    ScriptPromise<IDLString> promise = ClipboardPromise::CreateForReadText(executionContext, scope.GetScriptState(), scope.GetExceptionState());
