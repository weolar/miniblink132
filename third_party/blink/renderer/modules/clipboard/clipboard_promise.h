...
class MODULES_EXPORT ClipboardPromise final : public GarbageCollected<ClipboardPromise>, public ExecutionContextLifecycleObserver {
public:
    ...
    static ScriptPromise<IDLSequence<ClipboardItem>> CreateForRead(
        ExecutionContext* execution_context, ScriptState* script_state, ClipboardUnsanitizedFormats* formats, ExceptionState& exception_state);
    ...
    static ScriptPromise<IDLString> CreateForReadText(ExecutionContext* execution_context, ScriptState* script_state, ExceptionState& exception_state);
    ...
    static ScriptPromise<IDLUndefined> CreateForWrite(
        ExecutionContext* execution_context, ScriptState* script_state, const HeapVector<Member<ClipboardItem>>& items, ExceptionState& exception_state);
    ...
    static ScriptPromise<IDLUndefined> CreateForWriteText(
        ExecutionContext* execution_context, ScriptState* script_state, const String& text, ExceptionState& exception_state);

    // Use one of the above factories to construct. This ctor is public for
    // `MakeGarbageCollected<>`.
    ClipboardPromise(ExecutionContext* execution_context, ScriptPromiseResolverBase*, ExceptionState& exception_state);
...
private:
    ...
    void HandlePromiseWrite(HeapVector<Member<V8UnionBlobOrString>>* clipboard_item_list);
    void WriteClipboardItemData(HeapVector<Member<V8UnionBlobOrString>>* clipboard_item_list);
    ...
    HeapVector<std::pair<String, Member<V8UnionBlobOrString>>> clipboard_item_data_;
    // The list of formats with their corresponding promises to the Blob data to
    // be written to the clipboard.
    HeapVector<std::pair<String, ScriptPromise<V8UnionBlobOrString>>> clipboard_item_data_with_promises_;
