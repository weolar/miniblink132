...
{
    return next_state == kDeferTreeUpdates && state_ != kDisposing && state_ != kDisposed;
}
#endif
...
static WTF::String StateAsDebugString(const AXObjectCacheLifecycle::LifecycleState& state)
{
    switch (state) {
...
{
#if DCHECK_IS_ON()
    DCHECK(CanAdvanceTo(next_state)) << "Cannot advance a11y lifecycle from " << StateAsDebugString(state_) << " to " << StateAsDebugString(next_state) << ".";
#endif
    state_ = next_state;
}
...
{
#if DCHECK_IS_ON()
    DCHECK(CanRewindTo(state)) << "Cannot rewind a11y lifecycle from " << StateAsDebugString(state_) << " to " << StateAsDebugString(state) << ".";
#endif
    state_ = state;
}
