...
bool IncludeTrailingWhitespace(const WTF::String& text, wtf_size_t offset, const FragmentItem& item, const AXBlockFlowData::Line& line)
{
    ...
    const base::span<const OffsetMappingUnit> mapping_units = mapping->GetMappingUnitsForTextContentOffsetRange(offset, offset + 1);
...
WTF::String AXBlockFlowData::GetText(wtf_size_t item_index) const
{
    ...
    const PhysicalBoxFragment* box_fragment = BoxFragment(position.fragmentainer_index);
    ...
    const FragmentProperties& fragment_properties = fragment_properties_[item_index];
    ...
    if (fragment_properties.line_index) {
        const AXBlockFlowData::Line& line = lines_[fragment_properties.line_index.value()];
...
const AXBlockFlowData::FragmentProperties& AXBlockFlowData::GetProperties(wtf_size_t index) const
{
...
void AXBlockFlowData::ProcessLayoutBlock(LayoutBlockFlow* container)
{
    ...
    if (container_fragment_count) {
        for (int fragment_index = 0; fragment_index < container_fragment_count; fragment_index++) {
            const PhysicalBoxFragment* fragment = container->GetPhysicalFragment(fragment_index);
            if (fragment->Items()) {
                wtf_size_t next_starting_index = starting_fragment_index + fragment->Items()->Size();
...
void AXBlockFlowData::ProcessBoxFragment(const PhysicalBoxFragment* fragment, wtf_size_t starting_fragment_index)
{
    ...
    for (auto it = items->Items().begin(); it != items->Items().end(); it++, fragment_index++) {
        ...
        if (it->Type() == FragmentItem::kLine) {
            ...
            lines_.push_back<Line>({ .start_index = fragment_index, .length = length, .forced_break = forced_break, .break_index = break_index });
            if (!on_current_line) {
        ...
        if (it->Type() == FragmentItem::kText || it->Type() == FragmentItem::kGeneratedText) {
            if (previous_on_line) {
                properties.previous_on_line = previous_on_line;
                fragment_properties_[previous_on_line.value()].next_on_line = fragment_index;
            }
...
bool AXBlockFlowData::OnCurrentLine(wtf_size_t index) const
{
    ...
    return candidate.start_index < index && candidate.start_index + candidate.length > index;
}

const std::optional<wtf_size_t> AXBlockFlowData::FindFirstFragment(const LayoutObject* layout_object) const
{
...
const AXBlockFlowData::Position AXBlockFlowData::GetPosition(wtf_size_t index) const
{
    wtf_size_t container_fragment_count = block_flow_container_->PhysicalFragmentCount();

    if (container_fragment_count) {
        for (wtf_size_t fragment_index = 0; fragment_index < container_fragment_count; fragment_index++) {
            const PhysicalBoxFragment* fragment = block_flow_container_->GetPhysicalFragment(fragment_index);
...
const FragmentItem* AXBlockFlowData::ItemAt(wtf_size_t index) const
{
    ...
    const PhysicalBoxFragment* box_fragment = block_flow_container_->GetPhysicalFragment(position.fragmentainer_index);
    return &box_fragment->Items()->Items()[position.item_index];
}

const PhysicalBoxFragment* AXBlockFlowData::BoxFragment(wtf_size_t index) const
{
...
bool AXBlockFlowIterator::Next()
{
    ...
    wtf_size_t delta = block_flow_data_->ItemAt(current_index_.value())->DeltaToNextForSameLayoutObject();
    if (delta) {
...
// static
WTF::String AXBlockFlowIterator::GetTextForTesting(AXBlockFlowIterator::MapKey map_key)
{
...
const std::optional<AXBlockFlowIterator::MapKey> AXBlockFlowIterator::NextOnLine()
{
    DCHECK(current_index_) << "Must call Next to set initial iterator position before calling "
                           << "NextOnLine";

    const AXBlockFlowData::FragmentProperties& properties = block_flow_data_->GetProperties(current_index_.value());
    if (properties.next_on_line) {
        const AXBlockFlowData::Position position = block_flow_data_->GetPosition(properties.next_on_line.value());
        const PhysicalBoxFragment* box_fragment = block_flow_data_->BoxFragment(position.fragmentainer_index);
...
const std::optional<AXBlockFlowIterator::MapKey> AXBlockFlowIterator::PreviousOnLine()
{
    DCHECK(current_index_) << "Must call Next to set initial iterator position before calling "
                           << "PreviousOnLine";
    ...
    if (properties.previous_on_line) {
        const AXBlockFlowData::Position position = block_flow_data_->GetPosition(properties.previous_on_line.value());
        const PhysicalBoxFragment* box_fragment = block_flow_data_->BoxFragment(position.fragmentainer_index);
        return MapKey(box_fragment->Items(), position.item_index);
    }
...
const AXBlockFlowIterator::MapKey AXBlockFlowIterator::GetMapKey() const
{
    ...
    const AXBlockFlowData::Position position = block_flow_data_->GetPosition(current_index_.value());
    const PhysicalBoxFragment* box_fragment = block_flow_data_->BoxFragment(position.fragmentainer_index);
    return MapKey(box_fragment->Items(), position.item_index);
}
