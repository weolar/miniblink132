...
TEST_F(AccessibilityTest, PositionInText)
{
    ...
    const AXObject* ax_static_text = GetAXObjectByElementId("paragraph")->FirstChildIncludingIgnored();
...
TEST_F(AccessibilityTest, PositionAfterLineBreak)
{
    ...
    ASSERT_NE(nullptr, ax_br);
    ASSERT_EQ(ax::mojom::Role::kLineBreak, ax_br->RoleValue());
    const AXObject* ax_static_text = GetAXRootObject()->DeepestLastChildIncludingIgnored()->ParentObject();
    ASSERT_NE(nullptr, ax_static_text);
    ASSERT_EQ(ax::mojom::Role::kStaticText, ax_static_text->RoleValue());
...
TEST_F(AccessibilityTest, FirstPositionInTextContainer)
{
    ...
    ASSERT_NE(nullptr, text);
    ASSERT_TRUE(text->IsTextNode());
    const AXObject* ax_static_text = GetAXObjectByElementId("div")->FirstChildIncludingIgnored();
    ASSERT_NE(nullptr, ax_static_text);
    ASSERT_EQ(ax::mojom::Role::kStaticText, ax_static_text->RoleValue());

    const auto ax_position = AXPosition::CreateFirstPositionInObject(*ax_static_text);
    const auto position = ax_position.ToPositionWithAffinity();
    EXPECT_EQ(text, position.AnchorNode());
    EXPECT_EQ(0, position.GetPosition().OffsetInContainerNode());
...
TEST_F(AccessibilityTest, LastPositionInTextContainer)
{
    ...
    ASSERT_NE(nullptr, text);
    ASSERT_TRUE(text->IsTextNode());
    const AXObject* ax_static_text = GetAXObjectByElementId("div")->LastChildIncludingIgnored();
    ASSERT_NE(nullptr, ax_static_text);
    ASSERT_EQ(ax::mojom::Role::kStaticText, ax_static_text->RoleValue());

    const auto ax_position = AXPosition::CreateLastPositionInObject(*ax_static_text);
    const auto position = ax_position.ToPositionWithAffinity();
    EXPECT_EQ(text, position.AnchorNode());
    EXPECT_EQ(5, position.GetPosition().OffsetInContainerNode());
...
TEST_F(AccessibilityTest, AXPositionComparisonOperators)
{
    ...
    const auto paragraph_before = AXPosition::CreatePositionBeforeObject(*paragraph->FirstChildIncludingIgnored());
    const auto paragraph_after = AXPosition::CreatePositionAfterObject(*paragraph->LastChildIncludingIgnored());
    const auto paragraph_start = AXPosition::CreatePositionInTextObject(*paragraph->FirstChildIncludingIgnored(), 0);
    const auto paragraph_end = AXPosition::CreatePositionInTextObject(*paragraph->LastChildIncludingIgnored(), 5);
...
TEST_F(AccessibilityTest, PositionAfterTextWithWhiteSpace)
{
    ...
    ASSERT_NE(nullptr, text);
    ASSERT_TRUE(text->IsTextNode());
    const AXObject* ax_static_text = GetAXObjectByElementId("paragraph")->LastChildIncludingIgnored();
    ASSERT_NE(nullptr, ax_static_text);
    ASSERT_EQ(ax::mojom::Role::kStaticText, ax_static_text->RoleValue());
...
TEST_F(AccessibilityTest, AXPositionFromDOMPositionWithWhiteSpace)
{
    ...
    const auto ax_position_after_white_space = AXPosition::FromPosition(position_after_white_space);
    ...
    const auto ax_position_before_white_space = AXPosition::FromPosition(position_before_white_space);
...
TEST_F(AccessibilityTest, AXPositionsWithPreservedLeadingWhitespaceAndBreak)
{
    ...
    const auto ax_position_at_start_1 = AXPosition::FromPosition(position_at_start_1);
    ...
    const auto ax_position_after_white_space_1 = AXPosition::FromPosition(position_after_white_space_1);
    ...
    const auto ax_position_at_start_2 = AXPosition::FromPosition(position_at_start_2);
    ...
    const auto ax_position_after_white_space_2 = AXPosition::FromPosition(position_after_white_space_2);
...
TEST_F(AccessibilityTest, PositionInTextWithAffinity)
{
    ...
    const auto ax_position = AXPosition::CreatePositionInTextObject(*ax_static_text, 3, TextAffinity::kUpstream);
    const auto position = ax_position.ToPositionWithAffinity();
...
TEST_F(AccessibilityTest, PositionInHTMLLabel)
{
    ...
    const auto ax_position_before_label = AXPosition::FromPosition(position_before_label, TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveLeft);
    ...
    for (const auto& position : { position_before_text, position_in_text, position_after_label }) {
        const auto ax_position = AXPosition::FromPosition(position, TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveLeft);
        EXPECT_TRUE(ax_position.IsTextPosition());
    ...
    const auto ax_position_before_paragraph
        = AXPosition::FromPosition(position_before_paragraph, TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveLeft);
    ...
    EXPECT_EQ(ax_paragraph, ax_position_before_paragraph.ChildAfterTreePosition());
}
...
TEST_F(AccessibilityTest, PositionInHTMLLabelIgnored)
{
    ...
    const auto ax_position_before = AXPosition::FromPosition(position_before, TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveLeft);
    ...
    const auto position_from_ax_before = ax_position_before.ToPositionWithAffinity();
    ...
    EXPECT_EQ(label, position_from_ax_before.GetPosition().ComputeNodeAfterPosition());
    ...
    for (const auto& position : { position_before_text, position_in_text, position_after }) {
        const auto ax_position = AXPosition::FromPosition(position, TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveLeft);
        EXPECT_TRUE(ax_position.IsTextPosition());
        EXPECT_EQ(ax_label_text, ax_position.ContainerObject());
        EXPECT_EQ(nullptr, ax_position.ChildAfterTreePosition());
        ...
        EXPECT_EQ(nullptr, position_from_ax.GetPosition().ComputeNodeAfterPosition());
...
TEST_F(AccessibilityTest, PositionInIgnoredObject)
{
    ...
    const auto ax_position_before_visible = AXPosition::CreatePositionBeforeObject(*ax_visible);
    const auto position_before_visible = ax_position_before_visible.ToPositionWithAffinity();
    ...
    EXPECT_EQ(visible, position_before_visible.GetPosition().ComputeNodeAfterPosition());

    const auto ax_position_before_visible_from_dom = AXPosition::FromPosition(position_before_visible);
    EXPECT_EQ(ax_position_before_visible, ax_position_before_visible_from_dom);
    EXPECT_EQ(ax_visible, ax_position_before_visible_from_dom.ChildAfterTreePosition());

    // A position at the beginning of the body will appear to be before the hidden
    // element in the DOM.
    const auto ax_position_first = AXPosition::CreateFirstPositionInObject(*ax_root);
    ...
    EXPECT_EQ(GetDocument().documentElement(), position_first.GetPosition().ComputeNodeAfterPosition());

    const auto ax_position_first_from_dom = AXPosition::FromPosition(position_first);
    ...
    const auto ax_position_before_from_dom = AXPosition::FromPosition(position_before);
    ...
    const auto ax_position_after_from_dom = AXPosition::FromPosition(position_after);
...
TEST_F(AccessibilityTest, PreviousPositionAfterARIAHiddenShouldNotSkipARIAHidden)
{
    // Note: aria-describedby adds hidden target subtrees to the a11y tree as
    // "ignored but included in tree".
    SetBodyInnerHTML(R"HTML(
    ...
    EXPECT_EQ(nullptr, position_previous.GetPosition().ComputeNodeAfterPosition());

    const auto ax_position_previous_from_dom = AXPosition::FromPosition(position_previous);
...
TEST_F(AccessibilityTest, FromPositionInARIAHidden)
{
    ...
    auto ax_position_left = AXPosition::FromPosition(position_first, TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveLeft);
    EXPECT_TRUE(ax_position_left.IsValid());
    EXPECT_TRUE(ax_position_left.IsTextPosition());
    EXPECT_EQ(ax_hidden->FirstChildIncludingIgnored(), ax_position_left.ContainerObject());
    ...
    auto ax_position_right = AXPosition::FromPosition(position_first, TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveRight);
    EXPECT_TRUE(ax_position_right.IsValid());
    EXPECT_TRUE(ax_position_right.IsTextPosition());
    EXPECT_EQ(ax_hidden->FirstChildIncludingIgnored(), ax_position_right.ContainerObject());
    EXPECT_EQ(0, ax_position_right.TextOffset());

    const auto position_before = Position::BeforeNode(*hidden);
    ax_position_left = AXPosition::FromPosition(position_before, TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveLeft);
    EXPECT_TRUE(ax_position_left.IsValid());
    ...
    // outcome.
    ax_position_right = AXPosition::FromPosition(position_before, TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveRight);
    EXPECT_TRUE(ax_position_right.IsValid());
    ...
    const auto position_after = Position::AfterNode(*hidden);
    ax_position_left = AXPosition::FromPosition(position_after, TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveLeft);
    EXPECT_TRUE(ax_position_left.IsValid());
    EXPECT_TRUE(ax_position_left.IsTextPosition());
    EXPECT_EQ(ax_hidden->FirstChildIncludingIgnored(), ax_position_left.ContainerObject());
    EXPECT_EQ(12, ax_position_left.TextOffset());

    ax_position_right = AXPosition::FromPosition(position_after, TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveRight);
    EXPECT_TRUE(ax_position_right.IsValid());
    EXPECT_FALSE(ax_position_right.IsTextPosition());
    EXPECT_EQ(ax_container, ax_position_right.ContainerObject());
...
TEST_F(AccessibilityTest, PositionInCanvas)
{
    ...
    const auto ax_position_1 = AXPosition::CreateFirstPositionInObject(*ax_canvas_1);
    ...
    const auto ax_position_3 = AXPosition::CreateLastPositionInObject(*ax_canvas_2);
...
TEST_F(AccessibilityTest, PositionBeforeListMarker)
{
    ...
    const auto position_1 = ax_position_1.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveLeft);
    ...
    const auto ax_position_from_dom_1 = AXPosition::FromPosition(position_1);
    EXPECT_EQ(ax_position_1.AsValidDOMPosition(AXPositionAdjustmentBehavior::kMoveLeft), ax_position_from_dom_1);
    ...
    const auto position_2 = ax_position_2.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveLeft);
    ...
    const auto ax_position_from_dom_2 = AXPosition::FromPosition(position_2);
    EXPECT_EQ(ax_position_2.AsValidDOMPosition(AXPositionAdjustmentBehavior::kMoveLeft), ax_position_from_dom_2);
    ...
    const auto position_3 = ax_position_1.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveRight);
    ...
    const auto position_4 = ax_position_2.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveRight);
...
TEST_F(AccessibilityTest, DISABLED_PositionInCSSContent)
{
    ...
    const auto position_before = ax_position_before.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveRight);
    EXPECT_EQ(text, position_before.AnchorNode());
    EXPECT_EQ(0, position_before.GetPosition().OffsetInContainerNode());

    const auto ax_position_after = AXPosition::CreateLastPositionInObject(*ax_css_after);
    ...
    const auto position_after = ax_position_after.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveLeft);
...
TEST_F(AccessibilityTest, DISABLED_PositionInCSSImageContent)
{
   ...
   <h1 id="heading" class="heading">Heading</h1>)HTML";
    ...
    const auto ax_position_before = AXPosition::CreateFirstPositionInObject(*ax_css_before);
    const auto position = ax_position_before.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveLeft);
    EXPECT_EQ(GetDocument().body(), position.AnchorNode());
    EXPECT_EQ(3, position.GetPosition().OffsetInContainerNode());
}
...
TEST_F(AccessibilityTest, DISABLED_PositionInTableWithCSSContent)
{
    ...
    // table header cell.
    Element* const style_element = GetDocument().CreateRawElement(html_names::kStyleTag);
    ...
    GetDocument().body()->insertBefore(style_element, GetDocument().body()->firstChild());
    ...
    const AXObject* ax_first_header_cell = GetAXObjectByElementId("firstHeaderCell");
    ASSERT_NE(nullptr, ax_first_header_cell);
    ASSERT_EQ(ax::mojom::Role::kColumnHeader, ax_first_header_cell->RoleValue());
    const AXObject* ax_last_header_cell = GetAXObjectByElementId("lastHeaderCell");
    ...
    AXObject* const ax_first_cell_css_before = ax_first_header_cell->FirstChildIncludingIgnored()->FirstChildIncludingIgnored();
    ASSERT_NE(nullptr, ax_first_cell_css_before);
    ASSERT_EQ(ax::mojom::Role::kStaticText, ax_first_cell_css_before->RoleValue());

    ASSERT_EQ(3, ax_last_header_cell->ChildCountIncludingIgnored());
    // Get grandchild text, not the child ignored generic container.
    AXObject* const ax_last_cell_css_after = ax_last_header_cell->FirstChildIncludingIgnored()->LastChildIncludingIgnored();
    ...
    auto ax_position_before = AXPosition::CreateFirstPositionInObject(*ax_first_header_cell);
    EXPECT_TRUE(ax_position_before.IsTextPosition());
    EXPECT_EQ(0, ax_position_before.TextOffset());
    auto position_before = ax_position_before.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveRight);
    ...
    ax_position_before = AXPosition::CreateFirstPositionInObject(*ax_first_cell_css_before);
    EXPECT_TRUE(ax_position_before.IsTextPosition());
    EXPECT_EQ(0, ax_position_before.TextOffset());
    position_before = ax_position_before.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveRight);
    EXPECT_EQ(first_header_cell_text, position_before.AnchorNode());
    EXPECT_EQ(0, position_before.GetPosition().OffsetInContainerNode());
    ...
    ax_position_before = AXPosition::CreateFirstPositionInObject(*ax_first_cell_css_before->FirstChildIncludingIgnored());
    EXPECT_TRUE(ax_position_before.IsTextPosition());
    EXPECT_EQ(0, ax_position_before.TextOffset());
    position_before = ax_position_before.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveRight);
    EXPECT_EQ(first_header_cell_text, position_before.AnchorNode());
    ...
    auto ax_position_after = AXPosition::CreateLastPositionInObject(*ax_last_header_cell);
    EXPECT_FALSE(ax_position_after.IsTextPosition());
    EXPECT_EQ(3, ax_position_after.ChildIndex());
    auto position_after = ax_position_after.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveLeft);
    ...
    ax_position_after = AXPosition::CreateLastPositionInObject(*ax_last_cell_css_after);
    EXPECT_TRUE(ax_position_after.IsTextPosition());
    EXPECT_EQ(1, ax_position_after.TextOffset());
    position_after = ax_position_after.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveLeft);
    EXPECT_EQ(last_header_cell_text, position_after.AnchorNode());
    EXPECT_EQ(8, position_after.GetPosition().OffsetInContainerNode());
    ...
    ax_position_after = AXPosition::CreateLastPositionInObject(*ax_last_cell_css_after->FirstChildIncludingIgnored());
    EXPECT_TRUE(ax_position_after.IsTextPosition());
    EXPECT_EQ(1, ax_position_after.TextOffset());
    position_after = ax_position_after.ToPositionWithAffinity(AXPositionAdjustmentBehavior::kMoveLeft);
    EXPECT_EQ(last_header_cell_text, position_after.AnchorNode());
...
TEST_F(AccessibilityTest, PositionBeforeAndAfterTable)
{
    ...
    const auto ax_position_before = AXPosition::CreatePositionBeforeObject(*ax_table);
    ...
    const auto ax_position_after = AXPosition::CreatePositionAfterObject(*ax_table);
    ...
    const Node* node_after = position_after.GetPosition().ComputeNodeAfterPosition();
...
TEST_F(AccessibilityTest, PositionAtStartAndEndOfTable)
{
    ...
    const auto ax_position_at_start = AXPosition::CreateFirstPositionInObject(*ax_table);
    ...
    const auto ax_position_at_start_from_dom = AXPosition::FromPosition(position_at_start);
    ...
    const auto ax_position_at_end = AXPosition::CreateLastPositionInObject(*ax_table);
    ...
    const auto ax_position_at_end_from_dom = AXPosition::FromPosition(position_at_end);
...
TEST_F(AccessibilityTest, PositionInTableHeader)
{
    ...
    const auto ax_position_before = AXPosition::CreatePositionBeforeObject(*ax_first_header_cell);
    const auto position_before = ax_position_before.ToPositionWithAffinity();
    EXPECT_EQ(header_row, position_before.AnchorNode());
    EXPECT_EQ(1, position_before.GetPosition().OffsetInContainerNode());
    EXPECT_EQ(first_header_cell, position_before.GetPosition().ComputeNodeAfterPosition());

    const auto ax_position_before_from_dom = AXPosition::FromPosition(position_before);
    EXPECT_EQ(ax_position_before, ax_position_before_from_dom);
    EXPECT_EQ(ax_first_header_cell, ax_position_before_from_dom.ChildAfterTreePosition());

    const auto ax_position_after = AXPosition::CreatePositionAfterObject(*ax_last_header_cell);
    const auto position_after = ax_position_after.ToPositionWithAffinity();
...
TEST_F(AccessibilityTest, PositionInTableRow)
{
    ...
    const auto ax_position_before = AXPosition::CreatePositionBeforeObject(*ax_first_cell);
    const auto position_before = ax_position_before.ToPositionWithAffinity();
    EXPECT_EQ(first_row, position_before.AnchorNode());
    EXPECT_EQ(1, position_before.GetPosition().OffsetInContainerNode());
    EXPECT_EQ(first_cell, position_before.GetPosition().ComputeNodeAfterPosition());

    const auto ax_position_before_from_dom = AXPosition::FromPosition(position_before);
    EXPECT_EQ(ax_position_before, ax_position_before_from_dom);
    EXPECT_EQ(ax_first_cell, ax_position_before_from_dom.ChildAfterTreePosition());

    const auto ax_position_after = AXPosition::CreatePositionAfterObject(*ax_last_cell);
    const auto position_after = ax_position_after.ToPositionWithAffinity();
...
TEST_F(AccessibilityTest, PositionInInvalidMapLayout)
{
    ...
    const auto ax_position_before = AXPosition::CreateFirstPositionInObject(*ax_br);
    const auto position_before = ax_position_before.ToPositionWithAffinity();
...
TEST_F(AccessibilityTest, ToPositionWithAffinityWithMultipleInlineTextBoxes)
{
    ...
    ASSERT_NE(nullptr, text);
    ASSERT_TRUE(text->IsTextNode());
    AXObject* ax_static_text = GetAXObjectByElementId("paragraph")->FirstChildIncludingIgnored();
    ...
    const AXObject* ax_last_inline_box = ax_static_text->LastChildIncludingIgnored();
    const auto ax_position = AXPosition::CreatePositionBeforeObject(*ax_last_inline_box);
    const auto position = ax_position.ToPositionWithAffinity();
