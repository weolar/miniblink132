...
{
    SetBodyInnerHTML("<select id='t' style='text-transform:uppercase'>"
                     "<option>abc</select>");
...
{
    SetBodyInnerHTML("<select id='t' style='-webkit-text-security:disc'>"
                     "<option>abc</select>");
    const AXObject* ax_select = GetAXObjectByElementId("t");
    ASSERT_NE(nullptr, ax_select);
    EXPECT_TRUE(ax_select->GetLayoutObject() != nullptr);
    // U+2022 -> \xE2\x80\xA2 in UTF-8
    EXPECT_EQ("\xE2\x80\xA2\xE2\x80\xA2\xE2\x80\xA2", ax_select->GetValueForControl().Utf8());
}
...
TEST_F(AXLayoutObjectTest, AccessibilityHitTest)
{
    ...
    EXPECT_EQ(hit_test_result->RoleValue(), ax::mojom::Role::kTextFieldWithComboBox);
}
...
TEST_F(AXLayoutObjectTest, GetListStyleDecimalLeadingZeroAsCustomCounterStyle)
{
  ...
  <ul>
    ...
    EXPECT_EQ(ListStyle::kNumeric, GetAXObjectByElementId("target")->GetListStyle());
}
// https://crbug.com/1167596
TEST_F(AXLayoutObjectTest, GetListStyleOverriddenDecimalLeadingZero)
{
  ...
  <ul>
    ...
    ListStyle expected = RuntimeEnabledFeatures::CSSAtRuleCounterStyleSpeakAsDescriptorEnabled() ? ListStyle::kNumeric : ListStyle::kOther;
...
TEST_F(AXLayoutObjectTest, GetPredefinedListStyleWithSpeakAs)
{
  ...
  <ul>
    ...
    EXPECT_EQ(ListStyle::kOther, GetAXObjectByElementId("string")->GetListStyle());
    EXPECT_EQ(ListStyle::kDisc, GetAXObjectByElementId("disc")->GetListStyle());
    EXPECT_EQ(ListStyle::kCircle, GetAXObjectByElementId("circle")->GetListStyle());
    EXPECT_EQ(ListStyle::kSquare, GetAXObjectByElementId("square")->GetListStyle());
    EXPECT_EQ(ListStyle::kOther, GetAXObjectByElementId("disclosure-open")->GetListStyle());
    EXPECT_EQ(ListStyle::kOther, GetAXObjectByElementId("disclosure-closed")->GetListStyle());
    EXPECT_EQ(ListStyle::kNumeric, GetAXObjectByElementId("decimal")->GetListStyle());
    EXPECT_EQ(ListStyle::kNumeric, GetAXObjectByElementId("decimal-zero")->GetListStyle());
    EXPECT_EQ(ListStyle::kNumeric, GetAXObjectByElementId("roman")->GetListStyle());
    EXPECT_EQ(ListStyle::kNumeric, GetAXObjectByElementId("armenian")->GetListStyle());
    EXPECT_EQ(ListStyle::kNumeric, GetAXObjectByElementId("persian")->GetListStyle());
    EXPECT_EQ(ListStyle::kNumeric, GetAXObjectByElementId("chinese")->GetListStyle());
...
TEST_F(AXLayoutObjectTest, GetCustomListStyleWithSpeakAs)
{
  ...
  <ul>
    ...
    EXPECT_EQ(ListStyle::kDisc, GetAXObjectByElementId("bullets")->GetListStyle());
    EXPECT_EQ(ListStyle::kNumeric, GetAXObjectByElementId("numbers")->GetListStyle());
