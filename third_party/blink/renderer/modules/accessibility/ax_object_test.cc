...
TEST_F(AccessibilityTest, GetClosestElementSearchesAmongAncestors)
{
    ...
    const AXObject* nodeWithNoElement = button->DeepestFirstChildIncludingIgnored()->ParentObject();
    ASSERT_EQ(nullptr, nodeWithNoElement->GetElement());

    EXPECT_EQ(nodeWithNoElement->GetClosestElement(), button->GetElement()->GetPseudoElement(kPseudoIdBefore));
}
...
TEST_F(AccessibilityTest, IsEditableInTextField)
{
    ...
    const AXObject* input_text = input->FirstChildIncludingIgnored()->UnignoredChildAt(0);
    ...
    const AXObject* textarea_text = textarea->FirstChildIncludingIgnored()->UnignoredChildAt(0);
...
TEST_F(AccessibilityTest, IsEditableInContentEditable)
{
    ...
    const AXObject* root = GetAXRootObject();
    ASSERT_NE(nullptr, root);
    const AXObject* outer_contenteditable = GetAXObjectByElementId("outerContenteditable");
    ASSERT_NE(nullptr, outer_contenteditable);
    const AXObject* outer_contenteditable_text = outer_contenteditable->UnignoredChildAt(0);
    ASSERT_NE(nullptr, outer_contenteditable_text);
    ASSERT_EQ(ax::mojom::blink::Role::kStaticText, outer_contenteditable_text->RoleValue());
    const AXObject* inner_contenteditable = GetAXObjectByElementId("innerContenteditable");
    ASSERT_NE(nullptr, inner_contenteditable);
    const AXObject* inner_contenteditable_text = inner_contenteditable->UnignoredChildAt(0);
    ASSERT_NE(nullptr, inner_contenteditable_text);
    ASSERT_EQ(ax::mojom::blink::Role::kStaticText, inner_contenteditable_text->RoleValue());
...
TEST_F(AccessibilityTest, UnignoredChildren)
{
    ...
    EXPECT_EQ(ax::mojom::blink::Role::kStaticText, ax_body->UnignoredChildAt(0)->RoleValue());
    EXPECT_EQ("This is a test with", ax_body->UnignoredChildAt(0)->ComputedName());
    EXPECT_EQ(ax::mojom::blink::Role::kStaticText, ax_body->UnignoredChildAt(1)->RoleValue());
    EXPECT_EQ("ignored objects", ax_body->UnignoredChildAt(1)->ComputedName());
    EXPECT_EQ(ax::mojom::blink::Role::kParagraph, ax_body->UnignoredChildAt(2)->RoleValue());
    EXPECT_EQ(ax::mojom::blink::Role::kStaticText, ax_body->UnignoredChildAt(3)->RoleValue());
    EXPECT_EQ("depth levels", ax_body->UnignoredChildAt(3)->ComputedName());
    EXPECT_EQ(ax::mojom::blink::Role::kStaticText, ax_body->UnignoredChildAt(4)->RoleValue());
    EXPECT_EQ("in the accessibility tree.", ax_body->UnignoredChildAt(4)->ComputedName());
}
...
TEST_F(AccessibilityTest, SimpleTreeNavigation)
{
    ...
    EXPECT_EQ(ax::mojom::Role::kStaticText, paragraph->FirstChildIncludingIgnored()->RoleValue());
    ASSERT_NE(nullptr, paragraph->LastChildIncludingIgnored());
    EXPECT_EQ(ax::mojom::Role::kStaticText, paragraph->LastChildIncludingIgnored()->RoleValue());
    ASSERT_NE(nullptr, paragraph->FirstChildIncludingIgnored()->ParentObject());
    EXPECT_EQ(ax::mojom::Role::kStaticText, paragraph->DeepestFirstChildIncludingIgnored()->ParentObject()->RoleValue());
    ASSERT_NE(nullptr, paragraph->DeepestLastChildIncludingIgnored()->ParentObject());
    EXPECT_EQ(ax::mojom::Role::kStaticText, paragraph->DeepestLastChildIncludingIgnored()->ParentObject()->RoleValue());

    EXPECT_EQ(paragraph->PreviousSiblingIncludingIgnored(), GetAXObjectByElementId("ignored_a"));
    EXPECT_EQ(GetAXObjectByElementId("ignored_a"), input->NextSiblingIncludingIgnored());
    ASSERT_NE(nullptr, br->NextSiblingIncludingIgnored());
    EXPECT_EQ(ax::mojom::Role::kStaticText, br->NextSiblingIncludingIgnored()->RoleValue());
    ASSERT_NE(nullptr, br->PreviousSiblingIncludingIgnored());
    EXPECT_EQ(ax::mojom::Role::kStaticText, br->PreviousSiblingIncludingIgnored()->RoleValue());
    ...
    EXPECT_EQ(ax::mojom::Role::kStaticText, br->UnignoredNextSibling()->RoleValue());
    ASSERT_NE(nullptr, br->UnignoredPreviousSibling());
    EXPECT_EQ(ax::mojom::Role::kStaticText, br->UnignoredPreviousSibling()->RoleValue());

    ASSERT_NE(nullptr, button->FirstChildIncludingIgnored());
    EXPECT_EQ(ax::mojom::Role::kStaticText, button->FirstChildIncludingIgnored()->RoleValue());
    ASSERT_NE(nullptr, button->LastChildIncludingIgnored());
    EXPECT_EQ(ax::mojom::Role::kStaticText, button->LastChildIncludingIgnored()->RoleValue());
    ASSERT_NE(nullptr, button->DeepestFirstChildIncludingIgnored()->ParentObject());
    EXPECT_EQ(ax::mojom::Role::kStaticText, paragraph->DeepestFirstChildIncludingIgnored()->ParentObject()->RoleValue());
}
...
TEST_F(AccessibilityTest, TreeNavigationWithIgnoredContainer)
{
    ...
    EXPECT_EQ(obj_b_text, obj_c->PreviousInPreOrderIncludingIgnored()->ParentObjectUnignored());
    EXPECT_EQ(obj_b_text, obj_c->UnignoredPreviousInPreOrder()->ParentObjectUnignored());
...
TEST_F(AccessibilityTest, TreeNavigationWithInlineTextBoxes)
{
    ...
    const AXObject* ax_inline_before = ax_text_before->FirstChildIncludingIgnored();
    EXPECT_EQ(ax::mojom::blink::Role::kInlineTextBox, ax_inline_before->RoleValue());
    ...
    const AXObject* ax_inline_inside = ax_text_inside->FirstChildIncludingIgnored();
    EXPECT_EQ(ax::mojom::blink::Role::kInlineTextBox, ax_inline_inside->RoleValue());
    ...
    EXPECT_EQ(ax::mojom::blink::Role::kInlineTextBox, ax_inline_after->RoleValue());
...
{
    SetBodyInnerHTML(R"HTML(<p id="paragraph"><b role="none" id="bold"><br id="br"></b></p>)HTML");
...
TEST_F(AccessibilityTest, TestSetRangeValueVideoControlSlider)
{
    ...
    ASSERT_NE(nullptr, video_node);
    Node* timeline_node = video_element->GetMediaControls()->TimelineLayoutObject()->GetNode();
...
TEST_F(AccessibilityTest, PreservedWhitespaceWithInitialLineBreakIsLineBreakingObject)
{
    SetBodyInnerHTML(R"HTML(
    ...
    EXPECT_TRUE(preserved_text->IsLineBreakingObject()) << "This text node starts with a line break character, so it should be a "
                                                           "paragraph boundary.";
    ...
    ASSERT_THAT(preserved_text->UnignoredChildren(),
        Each(SafeMatcherCast<AXObject*>(Property("AXObject::RoleValue()", &AXObject::RoleValue, ax::mojom::Role::kInlineTextBox))));
    ...
    ASSERT_EQ(preserved_text->UnignoredChildAt(1)->ComputedName(), "First Paragraph");
    ...
    ASSERT_EQ(preserved_text->UnignoredChildAt(3)->ComputedName(), "Second Paragraph");
    ...
    ASSERT_EQ(preserved_text->UnignoredChildAt(5)->ComputedName(), "Third Paragraph");
...
TEST_F(AccessibilityTest, DivWithFirstLetterIsLineBreakingObject)
{
    ...
    ASSERT_EQ(ax::mojom::Role::kInlineTextBox, div_text->UnignoredChildAt(0)->RoleValue());
...
TEST_F(AccessibilityTest, SlotIsLineBreakingObject)
{
    ...
    ShadowRoot& shadow_root = GetElementById("host")->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    shadow_root.setInnerHTML(String::FromUTF8(shadow_content), ASSERT_NO_EXCEPTION);
...
TEST_F(AccessibilityTest, LineBreakInDisplayLockedIsLineBreakingObject)
{
    ...
    ASSERT_EQ(paragraph->GetNode(), DisplayLockUtilities::LockedInclusiveAncestorPreventingPaint(*paragraph->GetNode()))
        << "The <p> element should be display locked.";
    EXPECT_TRUE(paragraph->IsLineBreakingObject());

    const AXObject* br = GetAXObjectByElementId("br");
    ASSERT_NE(nullptr, br);
    ASSERT_EQ(ax::mojom::Role::kGenericContainer, br->RoleValue()) << "The <br> child should be display locked and thus have a generic "
                                                                      "role.";
    ASSERT_EQ(paragraph->GetNode(), DisplayLockUtilities::LockedInclusiveAncestorPreventingPaint(*br->GetNode())) << "The <br> child should be display locked.";
...
TEST_F(AccessibilityTest, ListMarkerIsNotLineBreakingObject)
{
    ...
    const AXObject* unordered_list_item = GetAXObjectByElementId("unorderedListItem");
    ...
    const AXObject* unordered_list_marker = unordered_list_item->UnignoredChildAt(0);
...
TEST_F(AccessibilityTest, CheckNoDuplicateChildren)
{
    ...
    ASSERT_EQ(ax_select->FirstChildIncludingIgnored()->ChildCountIncludingIgnored(), 1);
}
...
TEST_F(AccessibilityTest, CanSetFocusInCanvasFallbackContent)
{
    ...
    ASSERT_FALSE(GetAXObjectByElementId("div-hidden-inert")->CanSetFocusAttribute());
    ASSERT_FALSE(GetAXObjectByElementId("span-hidden-inert")->CanSetFocusAttribute());
    ASSERT_FALSE(GetAXObjectByElementId("a-hidden-inert")->CanSetFocusAttribute());
}
...
TEST_F(AccessibilityTest, StitchChildTree)
{
    ...
    ASSERT_EQ(paragraph_text->RoleValue(), ax::mojom::blink::Role::kInlineTextBox);
    AXObject* button = GetAXObjectByElementId("button");
    ASSERT_NE(nullptr, button);
    ...
    EXPECT_EQ(div_child_tree_id.ToString(), div_node_data.GetStringAttribute(ax::mojom::blink::StringAttribute::kChildTreeId));
    EXPECT_EQ(button_child_tree_id.ToString(), button_node_data.GetStringAttribute(ax::mojom::blink::StringAttribute::kChildTreeId));
    EXPECT_EQ(canvas_child_tree_id.ToString(), canvas_node_data.GetStringAttribute(ax::mojom::blink::StringAttribute::kChildTreeId));
    ...
    EXPECT_TRUE(button->IsIncludedInTree()) << "`button` should switch from ignored due to `display:none`, to "
                                               "included in the tree.";
    EXPECT_FALSE(button->IsVisible()) << "The visibility state should not change, only the inclusion in the "
                                         "tree.";
    EXPECT_EQ(0, button->ChildCountIncludingIgnored());
    EXPECT_TRUE(canvas->IsIgnoredButIncludedInTree());
    EXPECT_FALSE(canvas->IsVisible()) << "The visibility state should not change, only the inclusion in the "
                                         "tree.";
...
TEST_F(AccessibilityTest, UpdateTreeUpdatesInheritedLiveProperty)
{
    ...
    main->GetElement()->setAttribute(html_names::kAriaLiveAttr, AtomicString("polite"));
    GetAXObjectCache().UpdateAXForAllDocuments();
...
TEST_F(AccessibilityTest, UpdateTreeUpdatesInheritedAriaHiddenProperty)
{
    ...
    main->GetElement()->setAttribute(html_names::kAriaHiddenAttr, keywords::kTrue);
    GetAXObjectCache().UpdateAXForAllDocuments();
...
TEST_F(AccessibilityTest, UpdateTreeUpdatesInheritedDisabledProperty)
{
    ...
    fieldset->GetElement()->setAttribute(html_names::kAriaDisabledAttr, keywords::kTrue);
    GetAXObjectCache().UpdateAXForAllDocuments();
