...
BlinkAXTreeSource::BlinkAXTreeSource(AXObjectCacheImpl& ax_object_cache, bool is_snapshot)
...
void BlinkAXTreeSource::Selection(const AXObject* obj, bool& is_selection_backward, const AXObject** anchor_object, int& anchor_offset,
    ax::mojom::blink::TextAffinity& anchor_affinity, const AXObject** focus_object, int& focus_offset, ax::mojom::blink::TextAffinity& focus_affinity) const
{
    ...
    *focus_object = nullptr;
    ...
    const auto ax_selection = focus->IsAtomicTextField() ? AXSelection::FromCurrentSelection(ToTextControl(*focus->GetNode()))
...
{
    const std::optional<base::UnguessableToken>& embedding_token = local_frame->GetEmbeddingToken();
...
bool BlinkAXTreeSource::GetTreeData(ui::AXTreeData* tree_data) const
{
    ...
    Selection(root, is_selection_backward, &anchor_object, anchor_offset, anchor_affinity, &focus_object, focus_offset, focus_affinity);
    if (anchor_object && focus_object && anchor_offset >= 0 && focus_offset >= 0) {
        ...
        if (HTMLHeadElement* head = ax_object_cache_->GetDocument().head()) {
            for (Node* child = head->firstChild(); child; child = child->nextSibling()) {
                ...
                if (IsA<HTMLScriptElement>(*elem)) {
                    if (elem->getAttribute(html_names::kTypeAttr) != "application/ld+json") {
                        continue;
                    }
                } else if (!IsA<HTMLLinkElement>(*elem) && !IsA<HTMLTitleElement>(*elem) && !IsA<HTMLMetaElement>(*elem)) {
                    continue;
                }
                ...
                for (unsigned i = 0; i < elem->Attributes().size(); i++) {
                    html = html + String(" ") + elem->Attributes().at(i).LocalName() + String("=\"") + elem->Attributes().at(i).Value() + "\"";
                }
                html = html + String(">") + elem->innerHTML() + String("</") + tag + String(">");
...
{
    if (ShouldTruncateInlineTextBoxes() && ui::CanHaveInlineTextBoxChildren(node->RoleValue())) {
        return 0;
    }
...
AXObject* BlinkAXTreeSource::ChildAt(const AXObject* node, size_t index) const
{
    ...
    CHECK(child);
    if (child->IsDetached()) {
        NOTREACHED(base::NotFatalUntil::M127) << "Should not try to serialize an invalid child:"
                                              << "\nParent: " << node->ToString().Utf8() << "\nChild: " << child->ToString().Utf8();
        return nullptr;
    }

    if (!child->IsIncludedInTree()) {
        NOTREACHED(base::NotFatalUntil::M127) << "Should not receive unincluded child."
                                              << "\nChild: " << child->ToString().Utf8() << "\nParent: " << node->ToString().Utf8();
        return nullptr;
    }
...
void BlinkAXTreeSource::SerializeNode(const AXObject* src, ui::AXNodeData* dst) const
{
#if DCHECK_IS_ON()
    ...
    DocumentLifecycle::DisallowTransitionScope disallow(ax_object_cache_->GetDocument().Lifecycle());
#endif
