...
// static
AXObject* AccessibilityMediaControl::Create(LayoutObject* layout_object, AXObjectCacheImpl& ax_object_cache)
{
    DCHECK(layout_object->GetNode());
    return MakeGarbageCollected<AccessibilityMediaControl>(layout_object, ax_object_cache);
}

AccessibilityMediaControl::AccessibilityMediaControl(LayoutObject* layout_object, AXObjectCacheImpl& ax_object_cache)
...
bool AccessibilityMediaControl::OnNativeSetValueAction(const String& value)
{
    ...
    if (!input || input->FormControlType() != mojom::FormControlType::kInputRange) {
