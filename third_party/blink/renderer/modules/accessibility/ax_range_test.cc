...
TEST_F(AccessibilityTest, CommonAncestorContainerOfRange)
{
    ...
    EXPECT_EQ(body, AXRange(AXPosition::CreateFirstPositionInObject(*input), AXPosition::CreateLastPositionInObject(*button)).CommonAncestorContainer());
    EXPECT_EQ(body, AXRange(AXPosition::CreateFirstPositionInObject(*br), AXPosition::CreateFirstPositionInObject(*button)).CommonAncestorContainer());
    EXPECT_EQ(paragraph, AXRange(AXPosition::CreatePositionBeforeObject(*text1), AXPosition::CreatePositionBeforeObject(*br)).CommonAncestorContainer());
    EXPECT_EQ(paragraph, AXRange(AXPosition::CreatePositionBeforeObject(*text1), AXPosition::CreatePositionAfterObject(*text2)).CommonAncestorContainer());
}
...
TEST_F(AccessibilityTest, IsCollapsedRange)
{
    ...
    const AXRange paragraph_range(AXPosition::CreateLastPositionInObject(*paragraph), AXPosition::CreateLastPositionInObject(*paragraph));
    const AXRange text_range(AXPosition::CreateLastPositionInObject(*text), AXPosition::CreateLastPositionInObject(*text));
...
TEST_F(AccessibilityTest, RangeOfContents)
{
    ...
    EXPECT_EQ(AXPosition::CreateFirstPositionInObject(*paragraph), paragraph_range.Start());
    EXPECT_EQ(AXPosition::CreateLastPositionInObject(*paragraph), paragraph_range.End());
}
