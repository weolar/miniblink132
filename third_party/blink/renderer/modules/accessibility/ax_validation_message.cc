...
bool AXValidationMessage::ComputeIsIgnored(IgnoredReasons* ignored_reasons) const
{
    return false;
}
    ...
    AXObject** out_container, gfx::RectF& out_bounds_in_container, gfx::Transform& out_container_transform, bool* clips_children) const
{
    ...
    *out_container = ParentObject();
    ...
    if (form_control.GetLayoutObject()) {
        out_bounds_in_container = gfx::RectF(form_control.GetLayoutObject()->AbsoluteBoundingBoxRect());
    }
}
...
bool AXValidationMessage::IsVisible() const
{
    ...
    DCHECK(!is_visible || ParentObject() == AXObjectCache().Root()) << "A visible validation message's parent must be the root object'.";
    return is_visible;
}
...
{
    DEFINE_STATIC_LOCAL(const AtomicString, live_region_relevant_additions, ("additions"));
...
String AXValidationMessage::TextAlternative(bool recursive, const AXObject* aria_label_or_description_root, AXObjectSet& visited,
    ax::mojom::NameFrom& name_from, AXRelatedObjectVector* related_objects, NameSources* name_sources) const
{
