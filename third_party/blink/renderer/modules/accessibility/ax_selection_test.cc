...
TEST_F(AccessibilitySelectionTest, FromCurrentSelection)
{
    ...
    Element* const script_element = GetDocument().CreateRawElement(html_names::kScriptTag);
    ...
    const AXObject* ax_static_text_1 = GetAXObjectByElementId("paragraph1")->FirstChildIncludingIgnored();
...
TEST_F(AccessibilitySelectionTest, FromCurrentSelectionSelectAll)
{
    ...
    EXPECT_EQ(html_object->ChildCountIncludingIgnored(), ax_selection.Focus().ChildIndex());
...
TEST_F(AccessibilitySelectionTest, CancelSelect)
{
    ...
    AXSelection ax_selection = builder.SetAnchor(AXPosition::CreatePositionInTextObject(*ax_static_text_1, 3))
...
TEST_F(AccessibilitySelectionTest, DocumentRangeMatchesSelection)
{
    ...
    EXPECT_EQ(String("lo.\n      How are you?"), Selection().DocumentCachedRange()->toString());
}
...
TEST_F(AccessibilitySelectionTest, SetSelectionInText)
{
    ...
    const Node* text = GetDocument().QuerySelector(AtomicString("p"))->firstChild();
    ...
    const auto ax_base = AXPosition::CreatePositionInTextObject(*ax_static_text, 3);
    const auto ax_extent = AXPosition::CreatePositionAfterObject(*ax_static_text);

    AXSelection::Builder builder;
    const AXSelection ax_selection = builder.SetAnchor(ax_base).SetFocus(ax_extent).Build();
...
// On Android we use an ifdef to disable inline text boxes.
    ...
    ASSERT_EQ(ax_inline_text_box->RoleValue(), ax::mojom::Role::kGenericContainer);
    ...
    ax_inline_text_box = ax_inline_text_box->NextSiblingIncludingIgnored()->NextSiblingIncludingIgnored();
    ASSERT_NE(nullptr, ax_inline_text_box);
    ASSERT_EQ(ax_inline_text_box->RoleValue(), ax::mojom::Role::kInlineTextBox);
    ...
    const auto ax_base = AXPosition::CreatePositionInTextObject(*ax_inline_text_box, 0);
    const auto ax_extent = AXPosition::CreatePositionInTextObject(*ax_inline_text_box, 2);

    AXSelection::Builder builder;
    AXSelection ax_selection = builder.SetAnchor(ax_base).SetFocus(ax_extent).Build();
...
TEST_F(AccessibilitySelectionTest, SetSelectionAcrossLineBreak)
{
    ...
    const Node* line2 = GetDocument().QuerySelector(AtomicString("p"))->lastChild();
    ...
    const AXObject* ax_line2 = GetAXObjectByElementId("paragraph")->LastChildIncludingIgnored();
...
TEST_F(AccessibilitySelectionTest, SetSelectionInDisplayNone)
{
    ...
    const auto selection = SelectionInDOMTree::Builder().SetBaseAndExtent(hidden_1_first, hidden_2_first).Build();

    const auto ax_selection_shrink = AXSelection::FromSelection(selection, AXSelectionBehavior::kShrinkToValidRange);
    const auto ax_selection_extend = AXSelection::FromSelection(selection, AXSelectionBehavior::kExtendToValidRange);
...
TEST_F(AccessibilitySelectionTest, SetSelectionAroundListBullet)
{
    ...
    AXSelection ax_selection
        = builder.SetAnchor(AXPosition::CreateFirstPositionInObject(*ax_bullet_1)).SetFocus(AXPosition::CreateLastPositionInObject(*ax_text_2)).Build();
    ...
    const SelectionInDOMTree shrunk_selection = Selection().GetSelectionInDOMTree();
    ...
    const SelectionInDOMTree extended_selection = Selection().GetSelectionInDOMTree();

    ASSERT_TRUE(extended_selection.Anchor().IsOffsetInAnchor());
    EXPECT_EQ(item_1->parentNode(), extended_selection.Anchor().AnchorNode());
    EXPECT_EQ(static_cast<int>(item_1->NodeIndex()), extended_selection.Anchor().OffsetInContainerNode());
...
TEST_F(AccessibilitySelectionTest, FromCurrentSelectionInTextField)
{
    ...
    const auto ax_selection = AXSelection::FromCurrentSelection(ToTextControl(*input));
    ASSERT_TRUE(ax_selection.IsValid());
...
TEST_F(AccessibilitySelectionTest, FromCurrentSelectionInTextarea)
{
    ...
    const Element* textarea = GetDocument().QuerySelector(AtomicString("textarea"));
    ASSERT_NE(nullptr, textarea);
    ASSERT_TRUE(IsTextControl(textarea));
    ...
    const auto ax_selection = AXSelection::FromCurrentSelection(ToTextControl(*textarea));
    ASSERT_TRUE(ax_selection.IsValid());
...
TEST_F(AccessibilitySelectionTest, FromCurrentSelectionInTextareaWithAffinity)
{
    ...
    // functions instead.
    textarea->Focus();
    Selection().Modify(SelectionModifyAlteration::kMove, SelectionModifyDirection::kBackward, TextGranularity::kDocumentBoundary, SetSelectionBy::kUser);
    Selection().Modify(SelectionModifyAlteration::kExtend, SelectionModifyDirection::kForward, TextGranularity::kLineBoundary, SetSelectionBy::kUser);
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(AccessibilitySelectionTest, FromCurrentSelectionInTextareaWithCollapsedSelectionAndAffinity)
{
    SetBodyInnerHTML(R"HTML(
      <textarea id="textarea"
          rows="2" cols="15"
    ...
    textarea->Focus();
    Selection().Modify(SelectionModifyAlteration::kMove, SelectionModifyDirection::kBackward, TextGranularity::kDocumentBoundary, SetSelectionBy::kUser);
    Selection().Modify(SelectionModifyAlteration::kMove, SelectionModifyDirection::kForward, TextGranularity::kLineBoundary, SetSelectionBy::kUser);
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(AccessibilitySelectionTest, FromCurrentSelectionInContentEditableWithSoftLineBreaks)
{
    GetPage().GetSettings().SetScriptEnabled(true);
    SetBodyInnerHTML(R"HTML(
    ...
    const AXObject* ax_contenteditable = GetAXObjectByElementId("contenteditable");
    ASSERT_NE(nullptr, ax_contenteditable);
    ASSERT_EQ(ax::mojom::Role::kTextField, ax_contenteditable->RoleValue());
    const AXObject* ax_static_text = ax_contenteditable->FirstChildIncludingIgnored();
    ASSERT_NE(nullptr, ax_static_text);
    // Guard against the structure of the accessibility tree unexpectedly
    // changing, causing a hard to debug test failure.
    ASSERT_EQ(ax::mojom::Role::kStaticText, ax_static_text->RoleValue()) << "A content editable with only text inside it should have static text "
                                                                            "children.";
    ...
    EXPECT_EQ(ax_static_text->ComputedName().length(), static_cast<unsigned>(ax_selection.Focus().TextOffset()));
}

TEST_F(AccessibilitySelectionTest, FromCurrentSelectionInContentEditableSelectFirstSoftLineBreak)
{
    GetPage().GetSettings().SetScriptEnabled(true);
...
TEST_F(AccessibilitySelectionTest, FromCurrentSelectionInContentEditableSelectFirstHardLineBreak)
{
    ...
    ASSERT_NE(nullptr, ax_contenteditable);
    ASSERT_EQ(ax::mojom::Role::kTextField, ax_contenteditable->RoleValue());
    ASSERT_EQ(3, ax_contenteditable->UnignoredChildCount()) << "The content editable should have two lines with a line break between "
                                                               "them.";
...
TEST_F(AccessibilitySelectionTest, ForwardSelectionInTextField)
{
    ...
    AXSelection::Builder builder;
    AXSelection ax_selection
        = builder.SetAnchor(AXPosition::CreateFirstPositionInObject(*ax_input)).SetFocus(AXPosition::CreateLastPositionInObject(*ax_input)).Build();
    ...
    // retrieved.
    GetDocument().ExistingAXObjectCache()->UpdateAXForAllDocuments();
    const auto ax_current_selection = AXSelection::FromCurrentSelection(ToTextControl(*input));
...
TEST_F(AccessibilitySelectionTest, BackwardSelectionInTextField)
{
    ...
    AXSelection::Builder builder;
    AXSelection ax_selection
        = builder.SetAnchor(AXPosition::CreatePositionInTextObject(*ax_input, 10)).SetFocus(AXPosition::CreatePositionInTextObject(*ax_input, 3)).Build();
...
TEST_F(AccessibilitySelectionTest, SelectingTheWholeOfTheTextField)
{
    ...
    ASSERT_NE(nullptr, input);
    ASSERT_TRUE(IsTextControl(input));
    ASSERT_TRUE(ToTextControl(*input).SetSelectionRange(3u, 10u, kSelectionHasBackwardDirection));
    ...
    AXSelection::Builder builder;
    AXSelection ax_selection
        = builder.SetAnchor(AXPosition::CreatePositionBeforeObject(*ax_before)).SetFocus(AXPosition::CreatePositionAfterObject(*ax_input)).Build();
    ...
    EXPECT_EQ(1, dom_selection.Anchor().OffsetInContainerNode());
    EXPECT_EQ(GetElementById("before"), dom_selection.Anchor().ComputeNodeAfterPosition());
    EXPECT_EQ(GetDocument().body(), dom_selection.Focus().AnchorNode());
    EXPECT_EQ(5, dom_selection.Focus().OffsetInContainerNode());
    EXPECT_EQ(GetElementById("after"), dom_selection.Focus().ComputeNodeAfterPosition());
...
TEST_F(AccessibilitySelectionTest, SelectEachConsecutiveCharacterInTextField)
{
    ...
    for (unsigned int i = 0; i < text_control.InnerEditorValue().length() - 1; ++i) {
        GetDocument().ExistingAXObjectCache()->UpdateAXForAllDocuments();
        AXSelection::Builder builder;
        AXSelection ax_selection = builder.SetAnchor(AXPosition::CreatePositionInTextObject(*ax_input, i))
                                       .SetFocus(AXPosition::CreatePositionInTextObject(*ax_input, i + 1))
                                       .Build();

        testing::Message message;
        message << "While selecting forward character " << static_cast<char>(text_control.InnerEditorValue()[i]) << " at position " << i << " in text field.";
    ...
    for (unsigned int i = text_control.InnerEditorValue().length(); i > 0; --i) {
        ...
        testing::Message message;
        message << "While selecting backward character " << static_cast<char>(text_control.InnerEditorValue()[i]) << " at position " << i << " in text field.";
        SCOPED_TRACE(message);
        EXPECT_TRUE(ax_selection.Select());
...
TEST_F(AccessibilitySelectionTest, SelectEachConsecutiveCharacterInEmailFieldWithInvalidAddress)
{
    GetPage().GetSettings().SetScriptEnabled(true);
...
TEST_F(AccessibilitySelectionTest, InvalidSelectionInTextField)
{
    ...
    AXSelection::Builder builder;
    AXSelection ax_selection
        = builder.SetAnchor(AXPosition::CreatePositionInTextObject(*ax_input, 0)).SetFocus(AXPosition::CreatePositionBeforeObject(*ax_after)).Build();
...
TEST_F(AccessibilitySelectionTest, ForwardSelectionInTextarea)
{
    ...
    AXSelection::Builder builder;
    AXSelection ax_selection
        = builder.SetAnchor(AXPosition::CreateFirstPositionInObject(*ax_textarea)).SetFocus(AXPosition::CreateLastPositionInObject(*ax_textarea)).Build();
    ...
    // retrieved.
    GetDocument().ExistingAXObjectCache()->UpdateAXForAllDocuments();
    const auto ax_current_selection = AXSelection::FromCurrentSelection(ToTextControl(*textarea));
    EXPECT_EQ(ax_selection, ax_current_selection);
}
...
TEST_F(AccessibilitySelectionTest, BackwardSelectionInTextarea)
{
    ...
    GetDocument().ExistingAXObjectCache()->UpdateAXForAllDocuments();
    AXSelection ax_selection
        = builder.SetAnchor(AXPosition::CreatePositionInTextObject(*ax_textarea, 10)).SetFocus(AXPosition::CreatePositionInTextObject(*ax_textarea, 3)).Build();
...
TEST_F(AccessibilitySelectionTest, SelectTheWholeOfTheTextarea)
{
    ...
    ASSERT_NE(nullptr, textarea);
    ASSERT_TRUE(IsTextControl(textarea));
    ASSERT_TRUE(ToTextControl(*textarea).SetSelectionRange(3u, 10u, kSelectionHasBackwardDirection));
    ...
    AXSelection::Builder builder;
    AXSelection ax_selection
        = builder.SetAnchor(AXPosition::CreatePositionBeforeObject(*ax_before)).SetFocus(AXPosition::CreatePositionAfterObject(*ax_textarea)).Build();
...
TEST_F(AccessibilitySelectionTest, SelectEachConsecutiveCharacterInTextarea)
{
    ...
    for (unsigned int i = 0; i < text_control.Value().length() - 1; ++i) {
        GetDocument().ExistingAXObjectCache()->UpdateAXForAllDocuments();
        AXSelection::Builder builder;
        AXSelection ax_selection = builder.SetAnchor(AXPosition::CreatePositionInTextObject(*ax_textarea, i))
                                       .SetFocus(AXPosition::CreatePositionInTextObject(*ax_textarea, i + 1))
                                       .Build();

        testing::Message message;
        message << "While selecting forward character " << static_cast<char>(text_control.Value()[i]) << " at position " << i << " in textarea.";
        SCOPED_TRACE(message);
        EXPECT_TRUE(ax_selection.Select());
    ...
    for (unsigned int i = text_control.Value().length(); i > 0; --i) {
        ...
        testing::Message message;
        message << "While selecting backward character " << static_cast<char>(text_control.Value()[i]) << " at position " << i << " in textarea.";
        SCOPED_TRACE(message);
        EXPECT_TRUE(ax_selection.Select());
...
TEST_F(AccessibilitySelectionTest, InvalidSelectionInTextarea)
{
    ...
    AXSelection::Builder builder;
    AXSelection ax_selection
        = builder.SetAnchor(AXPosition::CreatePositionInTextObject(*ax_textarea, 0)).SetFocus(AXPosition::CreatePositionBeforeObject(*ax_after)).Build();
...
TEST_F(AccessibilitySelectionTest, FromCurrentSelectionInContenteditableWithAffinity)
{
    SetBodyInnerHTML(R"HTML(
    ...
    Element* const contenteditable = GetDocument().QuerySelector(AtomicString("div[role=textbox]"));
    ...
    Selection().Modify(SelectionModifyAlteration::kMove, SelectionModifyDirection::kForward, TextGranularity::kLineBoundary, SetSelectionBy::kUser);
    UpdateAllLifecyclePhasesForTest();
    ASSERT_EQ(TextAffinity::kUpstream, Selection().GetSelectionInDOMTree().Affinity());
    ...
    EXPECT_GT(static_cast<int>(ax_text->ComputedName().length()), ax_selection.Anchor().TextOffset());
    EXPECT_EQ(TextAffinity::kUpstream, ax_selection.Anchor().Affinity());
    EXPECT_TRUE(ax_selection.Focus().IsTextPosition());
    EXPECT_EQ(ax_text, ax_selection.Focus().ContainerObject());
    EXPECT_LE(15, ax_selection.Focus().TextOffset());
    EXPECT_GT(static_cast<int>(ax_text->ComputedName().length()), ax_selection.Focus().TextOffset());
    EXPECT_EQ(TextAffinity::kUpstream, ax_selection.Focus().Affinity());
}

TEST_F(AccessibilitySelectionTest, SelectEachConsecutiveCharacterInContenteditable)
{
    ...
    const Element* contenteditable = GetDocument().QuerySelector(AtomicString("div[contenteditable]"));
    ASSERT_NE(nullptr, contenteditable);
    ...
    for (unsigned int i = 0; i < computed_name.length() - 1; ++i) {
        AXSelection::Builder builder;
        AXSelection ax_selection = builder.SetAnchor(AXPosition::CreatePositionInTextObject(*ax_static_text, i))
                                       .SetFocus(AXPosition::CreatePositionInTextObject(*ax_static_text, i + 1))
                                       .Build();

        testing::Message message;
        message << "While selecting forward character " << std::u16string(1, computed_name[i]) << " at position " << i << " in contenteditable.";
        SCOPED_TRACE(message);
        EXPECT_TRUE(ax_selection.Select());
        ...
        // accessibility tree.
        EXPECT_EQ(static_cast<int>(i + 9), dom_selection.Anchor().OffsetInContainerNode());
        EXPECT_EQ(static_cast<int>(i + 10), dom_selection.Focus().OffsetInContainerNode());
    }
    ...
    for (unsigned int i = computed_name.length(); i > 0; --i) {
        ...
        testing::Message message;
        message << "While selecting backward character " << std::u16string(1, computed_name[i]) << " at position " << i << " in contenteditable.";
        SCOPED_TRACE(message);
        EXPECT_TRUE(ax_selection.Select());
        ...
        // accessibility tree.
        EXPECT_EQ(static_cast<int>(i + 9), dom_selection.Anchor().OffsetInContainerNode());
        EXPECT_EQ(static_cast<int>(i + 8), dom_selection.Focus().OffsetInContainerNode());
    }
}
...
TEST_F(AccessibilitySelectionTest, SelectionWithEqualBaseAndExtent)
{
    ...
    AXObject* ax_sel = GetAXObjectByElementId("sel")->FirstChildIncludingIgnored();
    AXPosition ax_position = AXPosition::CreatePositionBeforeObject(*ax_sel);
    AXSelection::Builder builder;
    AXSelection ax_selection = builder.SetAnchor(ax_position).SetFocus(ax_position).Build();
}
...
TEST_F(AccessibilitySelectionTest, InvalidSelectionOnAShadowRoot)
{
    ...
    const auto selection = SelectionInDOMTree::Builder().SetBaseAndExtent(base, extent).Build();
