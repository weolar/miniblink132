...
namespace {
void IdsFromAttribute(Element& element, Vector<AtomicString>& ids, const QualifiedName& attr_name)
{
...
void AXRelationCache::DoInitialDocumentScan(Document& document)
{
    ...
    for (Node& node : ShadowIncludingTreeOrderTraversal::DescendantsOf(document)) {
...
void AXRelationCache::CheckRelationsCached(Element& element)
{
    ...
    for (const auto& owns_id : owns_ids) {
        DCHECK(id_attr_to_owns_relation_mapping_.Contains(owns_id))
            << element << " with aria-owns=" << owns_id << " and DOMNodeId=" << DOMNodeIds::ExistingIdForNode(&element) << " should already be in cache.";
    }
    ...
    if (IsA<HTMLLabelElement>(element)) {
        ...
        if (!for_id.empty()) {
            DCHECK(all_previously_seen_label_target_ids_.Contains(for_id))
                << element << " <label for=" << for_id << " with DOMNodeId=" << DOMNodeIds::ExistingIdForNode(&element) << " should already be in cache.";
        }
    }
    ...
    for (const auto& target_id : target_ids) {
        DCHECK(id_attr_to_text_relation_mapping_.Contains(target_id))
            << element << " with aria-labelledby/describedby=" << target_id << " and DOMNodeId=" << DOMNodeIds::ExistingIdForNode(&element)
            << " should already be in cache.";
    }

    // Check aria-activedescendant.
    if (auto& activedescendant_id = AXObject::AriaAttribute(element, html_names::kAriaActivedescendantAttr)) {
        DCHECK(id_attr_to_active_descendant_mapping_.Contains(activedescendant_id))
            << element << " with aria-activedescendant=" << activedescendant_id << " and DOMNodeId=" << DOMNodeIds::ExistingIdForNode(&element)
            << " should already be in cache.";
    }
}
...
void AXRelationCache::CheckElementWasProcessed(Element& element)
{
    ...
    NOTREACHED() << "The following element was attached to the document, but "
                 ...
                 << (obj && obj->ParentObjectIncludedInTree() ? obj->ParentObjectIncludedInTree()->GetAXTreeForThis() : "");
}
#endif
...
bool AXRelationCache::IsAriaOwned(const AXObject* child, bool check) const
{
    ...
    bool is_owned = aria_owned_child_to_owner_mapping_.Contains(child->AXObjectID());
    if (is_owned) {
        return true;
    }
    ...
    if (parent && parent->GetElement() && child->GetElement() && !child->GetElement()->IsPseudoElement()) {
        Node* natural_parent = AXObject::GetParentNodeForComputeParent(*object_cache_, child->GetElement());
...
AXObject* AXRelationCache::GetAriaOwnedParent(const AXObject* child) const
{
    ...
    HashMap<AXID, AXID>::const_iterator iter = aria_owned_child_to_owner_mapping_.find(child->AXObjectID());
...
Vector<AtomicString> AXRelationCache::GetTextRelationIds(Element& relation_source)
{
    SpaceSplitString ids_1(relation_source.FastGetAttribute(html_names::kAriaLabelledbyAttr));
    SpaceSplitString ids_2(relation_source.FastGetAttribute(html_names::kAriaLabeledbyAttr));
    SpaceSplitString ids_3(relation_source.FastGetAttribute(html_names::kAriaDescribedbyAttr));
...
void AXRelationCache::UpdateReverseRelations(
    HashMap<AtomicString, HashSet<DOMNodeId>>& id_attr_to_node_map, Node* relation_source, const Vector<AtomicString>& target_ids)
{
...
void AXRelationCache::UpdateReverseTextRelations(Element& relation_source)
{
    ...
    UpdateReverseTextRelations(relation_source, GetTextRelationIds(relation_source));
}

// TODO(crbug.com/41469336): Track reverse relations set via explicitly set
// attr-elements on element or element internals.
void AXRelationCache::UpdateReverseTextRelations(Element& relation_source, const QualifiedName& attr_name)
{
    Vector<AtomicString> ids;
    ...
    ExplicitlySetAttrElementsMap* element_attribute_map = relation_source.GetDocument().GetExplicitlySetAttrElementsMap(&relation_source);
    auto it = element_attribute_map->find(attr_name);
    if (it != element_attribute_map->end()) {
        HeapLinkedHashSet<WeakMember<Element>>* explicitly_set_target_elements = it->value;
    ...
    if (const ElementInternals* element_internals = relation_source.GetElementInternals()) {
        const FrozenArray<Element>* element_internals_target_elements = element_internals->GetElementArrayAttribute(attr_name);
...
// TODO(crbug.com/41469336): Track reverse relations set via explicitly set
// attr-elements on element or element internals.
void AXRelationCache::UpdateReverseTextRelations(Element& relation_source, const Vector<AtomicString>& target_ids)
{
    ...
    UpdateReverseRelations(id_attr_to_text_relation_mapping_, &relation_source, target_ids);
...
void AXRelationCache::UpdateReverseActiveDescendantRelations(Element& relation_source)
{
    const AtomicString& id = AXObject::AriaAttribute(relation_source, html_names::kAriaActivedescendantAttr);
    if (!id) {
        return;
    }
    UpdateReverseRelations(id_attr_to_active_descendant_mapping_, &relation_source, { id });
}
...
void AXRelationCache::UpdateReverseOwnsRelations(Element& relation_source)
{
    ...
    UpdateReverseRelations(id_attr_to_owns_relation_mapping_, &relation_source, owned_id_vector);
}

Vector<AtomicString> AXRelationCache::GetOtherRelationIds(Element& relation_source)
{
    SpaceSplitString ids_1(relation_source.FastGetAttribute(html_names::kAriaControlsAttr));
    SpaceSplitString ids_2(relation_source.FastGetAttribute(html_names::kAriaDetailsAttr));
    SpaceSplitString ids_3(relation_source.FastGetAttribute(html_names::kAriaErrormessageAttr));
    SpaceSplitString ids_4(relation_source.FastGetAttribute(html_names::kAriaFlowtoAttr));
    SpaceSplitString ids_5(relation_source.FastGetAttribute(html_names::kAriaActionsAttr));
    Vector<AtomicString> ids;
    ids.AppendRange(ids_1.begin(), ids_1.end());
    ids.AppendRange(ids_2.begin(), ids_2.end());
    ids.AppendRange(ids_3.begin(), ids_3.end());
...
{
    UpdateReverseRelations(id_attr_to_other_relation_mapping_, &relation_source, GetOtherRelationIds(relation_source));
}
...
static bool ContainsCycle(AXObject* owner, Node& child_node)
{
    ...
    for (AXObject* ancestor = owner; ancestor; ancestor = ancestor->ParentObject()) {
        if (ancestor->GetNode() == &child_node) {
            return true;
        }
        CHECK(visited.insert(ancestor->AXObjectID()).is_new_entry) << "Cycle in unexpected place:\n"
                                                                   << "* Owner = " << owner << "* Child = " << child_node;
    }
    return false;
}

bool AXRelationCache::IsValidOwnsRelation(AXObject* owner, Node& child_node) const
{
...
bool AXRelationCache::IsValidOwnedChild(Node& child_node)
{
    ...
    if (IsA<HTMLOptionElement>(child_node) || IsA<HTMLOptGroupElement>(child_node)) {
        return false;
    }

    // aria-hidden is problematic for cycles, and does not solve a known use case.
    // Easiest to omit the possibility.
    if (AXObject::IsAriaAttributeTrue(*child_element, html_names::kAriaHiddenAttr)) {
        return false;
    }
...
void AXRelationCache::UnmapOwnedChildrenWithCleanLayout(const AXObject* owner, const Vector<AXID>& removed_child_ids, Vector<AXID>& unparented_child_ids)
{
    DCHECK(owner);
...
void AXRelationCache::MapOwnedChildrenWithCleanLayout(const AXObject* owner, const Vector<AXID>& child_ids)
{
    DCHECK(owner);
    DCHECK(!owner->IsDetached());
    for (AXID added_child_id : child_ids) {
        ...
        if (original_parent != owner) {
            ...
            if (original_parent) {
                ...
                object_cache_->MarkAXObjectDirtyWithCleanLayout(original_parent->ParentObject());
            }
        }
...
void AXRelationCache::UpdateAriaOwnsFromAttrAssociatedElementsWithCleanLayout(
    AXObject* owner, const HeapVector<Member<Element>>& attr_associated_elements, HeapVector<Member<AXObject>>& validated_owned_children_result, bool force)
{
    ...
    // Track reverse relations for future tree updates.
    UpdateReverseRelations(id_attr_to_owns_relation_mapping_, owner->GetNode(), owned_id_vector);

    // Update the internal mappings of owned children.
    UpdateAriaOwnerToChildrenMappingWithCleanLayout(owner, validated_owned_children_result, force);
}

void AXRelationCache::ValidatedAriaOwnedChildren(const AXObject* owner, HeapVector<Member<AXObject>>& validated_owned_children_result)
{
    if (!aria_owner_to_children_mapping_.Contains(owner->AXObjectID()))
        return;
    Vector<AXID> current_child_axids = aria_owner_to_children_mapping_.at(owner->AXObjectID());
...
void AXRelationCache::UpdateAriaOwnsWithCleanLayout(AXObject* owner, bool force)
{
    CHECK(!object_cache_->IsFrozen());
    DCHECK(owner);
    ...
    if (!is_valid_owner) {
        DCHECK(force) << "Should not reach here except when an AXObject was "
                      ...
                      << owner;
    } else if (element && element->HasExplicitlySetAttrAssociatedElements(html_names::kAriaOwnsAttr)) {
        // TODO (crbug.com/41469336): Also check ElementInternals here.
        UpdateAriaOwnsFromAttrAssociatedElementsWithCleanLayout(owner,
        ...
        SpaceSplitString owned_id_vector(AXObject::AriaAttribute(*element, html_names::kAriaOwnsAttr));
        ...
        for (AtomicString id_name : owned_id_vector) {
            ...
            if (!child_element || !IsValidOwnsRelation(const_cast<AXObject*>(owner), *child_element)) {
                continue;
            }
            AXID future_child_axid = child_element->GetDomNodeId();
            HashMap<AXID, AXID>::const_iterator iter = aria_owned_child_to_owner_mapping_.find(future_child_axid);
            bool has_previous_owner = iter != aria_owned_child_to_owner_mapping_.end();
            ...
            aria_owned_child_to_owner_mapping_.Set(future_child_axid, owner->AXObjectID());
...
void AXRelationCache::UpdateAriaOwnerToChildrenMappingWithCleanLayout(
    AXObject* owner, HeapVector<Member<AXObject>>& validated_owned_children_result, bool force)
{
    DCHECK(owner);
    ...
    if (previously_owned_child_ids == validated_owned_child_axids && (!force || previously_owned_child_ids.empty())) {
        return;
    }
    ...
    UnmapOwnedChildrenWithCleanLayout(owner, previously_owned_child_ids, unparented_child_ids);
    ...
    if (validated_owned_child_axids.empty()) {
        ...
        aria_owner_to_children_mapping_.Set(owner->AXObjectID(), validated_owned_child_axids);
    }
...
bool AXRelationCache::MayHaveHTMLLabelViaForAttribute(const HTMLElement& labelable)
{
...
bool AXRelationCache::IsARIALabelOrDescription(Element& element)
{
    ...
    if (explicitly_set_text_relations_.find(element.GetDomNodeId()) != explicitly_set_text_relations_.end()) {
        return true;
    }
    ...
    return id_attr_to_text_relation_mapping_.find(id_value) != id_attr_to_text_relation_mapping_.end();
}

// Fill source_objects with AXObjects for relations pointing to target.
void AXRelationCache::GetReverseRelated(
    const AtomicString& target_id_attr, HashMap<AtomicString, HashSet<DOMNodeId>>& id_attr_to_node_map, HeapVector<Member<AXObject>>& source_objects)
{
...
AXObject* AXRelationCache::GetOrCreateAriaOwnerFor(Node* node, AXObject* obj)
{
    ...
    DCHECK(!obj || obj_for_node == obj) << "Object and node did not match:"
    ...
    GetReverseRelated(element->GetIdAttribute(), id_attr_to_owns_relation_mapping_, related_sources);
...
void AXRelationCache::UpdateRelatedTreeForIdChange(Element& element)
{
    ...
    MarkOldAndNewRelationSourcesDirty(element, id_attr_to_active_descendant_mapping_);
    Element* focused_element = element.GetDocument().FocusedElement();
    if (AXObject* ax_focus = Get(focused_element)) {
        if (AXObject::ElementFromAttributeOrInternals(focused_element, html_names::kAriaActivedescendantAttr) == &element) {
    ...
    // aria-actions.
    MarkOldAndNewRelationSourcesDirty(element, id_attr_to_other_relation_mapping_);
    UpdateReverseOtherRelations(element);
...
void AXRelationCache::UpdateRegisteredIdAttribute(Element& element, DOMNodeId node_id)
{
...
void AXRelationCache::UpdateRelatedText(Node* node)
{
    ...
    for (Node* current_node = node; ++count < kMaxAncestorsForNameChangeCheck && current_node && !IsA<HTMLBodyElement>(current_node);
         current_node = current_node->parentNode()) {
        if (Element* element = DynamicTo<Element>(current_node)) {
            // Reverse relations via aria-labelledby, aria-describedby, aria-owns.
            HeapVector<Member<AXObject>> related_sources;
            GetReverseRelated(element->GetIdAttribute(), id_attr_to_text_relation_mapping_, related_sources);
            for (AXObject* related : related_sources) {
                if (related && related->IsIncludedInTree() && !related->NeedsToUpdateChildren()) {
                    object_cache_->MarkAXObjectDirtyWithCleanLayout(related);
                }
            }
        }
        ...
        if (current_node != node) {
            ...
            if (obj && (!obj->IsIgnored() || obj->CanSetFocusAttribute()) && obj->SupportsNameFromContents(/*recursive=*/false)
...
void AXRelationCache::MarkOldAndNewRelationSourcesDirty(Element& element, HashMap<AtomicString, HashSet<DOMNodeId>>& id_attr_to_node_id_map)
{
    HeapVector<Member<AXObject>> related_sources;
...
void AXRelationCache::UpdateCSSAnchorFor(Node* positioned_node)
{
    ...
    if (positioned_obj_to_anchor_mapping_.Contains(positioned_id)) {
        ...
        object_cache_->MarkAXObjectDirtyWithCleanLayout(ObjectFromAXID(prev_anchor));
    }

    LayoutBox* layout_box = DynamicTo<LayoutBox>(positioned_node->GetLayoutObject());
    if (!layout_box) {
        return;
    }
...
AXObject* AXRelationCache::GetPositionedObjectForAnchor(const AXObject* anchor)
{
    HashMap<AXID, AXID>::const_iterator iter = anchor_to_positioned_obj_mapping_.find(anchor->AXObjectID());
...
AXObject* AXRelationCache::GetAnchorForPositionedObject(const AXObject* positioned_obj)
{
    HashMap<AXID, AXID>::const_iterator iter = positioned_obj_to_anchor_mapping_.find(positioned_obj->AXObjectID());
        ...
        if (!object_cache_->IsDisposing()) {
            for (const auto& child_axid : child_axids) {
                if (AXObject* owned_child = ObjectFromAXID(child_axid)) {
                    ...
                    CHECK(object_cache_->lifecycle().StateAllowsReparentingAXObjects()) << "Removing owned child at a bad time, which leads to "
...
void AXRelationCache::RemoveOwnedRelation(AXID obj_id)
{
    ...
    if (aria_owned_child_to_owner_mapping_.Contains(obj_id)) {
        ...
        const Vector<AXID>& owners_owned_children = aria_owner_to_children_mapping_.at(owner_id);
...
void AXRelationCache::MaybeRestoreParentOfOwnedChild(AXID removed_child_axid)
{
    ...
    if (Node* child_node = DOMNodeIds::NodeForId(removed_child_axid)) {
        ...
        if (Element* child_element = DynamicTo<Element>(child_node)) {
            GetReverseRelated(child_element->GetIdAttribute(), id_attr_to_owns_relation_mapping_, other_potential_owners);
