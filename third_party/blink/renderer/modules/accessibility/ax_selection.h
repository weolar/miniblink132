...
public:
    ...
    static AXSelection FromCurrentSelection(const Document&, const AXSelectionBehavior = AXSelectionBehavior::kExtendToValidRange);

    static AXSelection FromCurrentSelection(const TextControlElement&);

    static AXSelection FromSelection(const SelectionInDOMTree&, const AXSelectionBehavior = AXSelectionBehavior::kExtendToValidRange);

    AXSelection(const AXSelection&) = default;
    AXSelection& operator=(const AXSelection&) = default;
    ~AXSelection() = default;

    const AXPosition Anchor() const
    {
        return anchor_;
    }
    const AXPosition Focus() const
    {
        return focus_;
    }
    ...
    const SelectionInDOMTree AsSelection(const AXSelectionBehavior = AXSelectionBehavior::kExtendToValidRange) const;
    ...
    bool Select(const AXSelectionBehavior = AXSelectionBehavior::kExtendToValidRange);
...
private:
    ...
    struct TextControlSelection final {
        TextControlSelection(int start, int end, TextFieldSelectionDirection direction)
            : start(start)
            , end(end)
            , direction(direction)
        {
        ...
        TextControlSelection()
            : start(-1)
            , end(-1)
            , direction(kSelectionHasNoDirection)
        {
        }
