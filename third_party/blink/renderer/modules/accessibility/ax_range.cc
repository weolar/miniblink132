...
AXObject* AXRange::CommonAncestorContainer() const
{
    ...
    return const_cast<AXObject*>(AXObject::LowestCommonAncestor(*start_.ContainerObject(), *end_.ContainerObject(), &start_index, &end_index));
}
...
bool AXRange::IsValid() const
{
    ...
    // We don't support ranges that span across documents.
    if (start_.ContainerObject()->GetDocument() != end_.ContainerObject()->GetDocument()) {
        return false;
    }

    DCHECK(!start_.ContainerObject()->GetDocument()->NeedsLayoutTreeUpdate());
#if DCHECK_IS_ON()
    DCHECK_EQ(start_.ContainerObject()->GetDocument()->DomTreeVersion(), dom_tree_version_);
    DCHECK_EQ(start_.ContainerObject()->GetDocument()->StyleVersion(), style_version_);
#endif // DCHECK_IS_ON()
    return true;
}
...
{
    return AXRange(AXPosition::CreateFirstPositionInObject(container), AXPosition::CreateLastPositionInObject(container));
}
