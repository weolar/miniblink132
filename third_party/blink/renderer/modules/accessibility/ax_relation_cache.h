...
public:
    ...
    void MarkOldAndNewRelationSourcesDirty(Element& element, HashMap<AtomicString, HashSet<DOMNodeId>>& id_attr_to_node_id_map);
    ...
    // UpdateRelatedTree.
    void UpdateReverseRelations(HashMap<AtomicString, HashSet<DOMNodeId>>& id_attr_to_node_map, Node* relation_source, const Vector<AtomicString>& target_ids);

    void UpdateReverseTextRelations(Element& relation_source, const QualifiedName& attr_name);
    ...
    void UpdateReverseTextRelations(Element& relation_source, const Vector<AtomicString>& target_ids);
...
private:
    ...
    void UpdateAriaOwnsFromAttrAssociatedElementsWithCleanLayout(
        AXObject* owner, const HeapVector<Member<Element>>& attr_associated_elements, HeapVector<Member<AXObject>>& owned_children, bool force);
    ...
    void UnmapOwnedChildrenWithCleanLayout(const AXObject* owner, const Vector<AXID>& removed_child_ids, Vector<AXID>& unparented_child_ids);

    void MapOwnedChildrenWithCleanLayout(const AXObject* owner, const Vector<AXID>&);
    void GetReverseRelated(
        const AtomicString& target_id_attr, HashMap<AtomicString, HashSet<DOMNodeId>>& id_attr_to_node_map, HeapVector<Member<AXObject>>& sources);
    ...
    void UpdateAriaOwnerToChildrenMappingWithCleanLayout(AXObject* owner, HeapVector<Member<AXObject>>& validated_owned_children_result, bool force);
    ...
    HashMap<AtomicString, HashSet<DOMNodeId>> id_attr_to_active_descendant_mapping_;
