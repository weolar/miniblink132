...
// static
const AXPosition AXPosition::CreatePositionBeforeObject(const AXObject& child, const AXPositionAdjustmentBehavior adjustment_behavior)
{
...
// static
const AXPosition AXPosition::CreatePositionAfterObject(const AXObject& child, const AXPositionAdjustmentBehavior adjustment_behavior)
{
    if (child.IsDetached() || !child.IsIncludedInTree())
        return {};
...
// static
const AXPosition AXPosition::CreateFirstPositionInObject(const AXObject& container, const AXPositionAdjustmentBehavior adjustment_behavior)
{
    ...
    // objects.
    const AXObject* unignored_container = !container.IsIncludedInTree() ? container.ParentObjectIncludedInTree() : &container;
...
// static
const AXPosition AXPosition::CreateLastPositionInObject(const AXObject& container, const AXPositionAdjustmentBehavior adjustment_behavior)
{
    if (container.IsDetached())
        return {};
    ...
    DCHECK(unignored_container);
    AXPosition position(*unignored_container);
    position.text_offset_or_child_index_ = unignored_container->ChildCountIncludingIgnored();
#if DCHECK_IS_ON()
    String failure_reason;
    DCHECK(position.IsValid(&failure_reason)) << failure_reason;
#endif
...
const AXPosition AXPosition::CreatePositionInTextObject(
    const AXObject& container, const int offset, const TextAffinity affinity, const AXPositionAdjustmentBehavior adjustment_behavior)
{
    if (container.IsDetached() || !(container.IsTextObject() || container.IsTextField())) {
        return {};
    }
...
// static
const AXPosition AXPosition::FromPosition(const Position& position, const TextAffinity affinity, const AXPositionAdjustmentBehavior adjustment_behavior)
{
    ...
    if (container_node->IsTextNode()) {
        if (!container->IsIncludedInTree()) {
            ...
            case AXPositionAdjustmentBehavior::kMoveRight: {
                const AXObject* next_container = FindNeighboringUnignoredObject(*document, *container_node, container_node->parentNode(), adjustment_behavior);
                if (next_container) {
                    return CreatePositionBeforeObject(*next_container, adjustment_behavior);
                }
                ...
                if (!container || !container->ParentObjectIncludedInTree())
                    return {};
                return CreateLastPositionInObject(*container->ParentObjectIncludedInTree(), adjustment_behavior);
            }

            case AXPositionAdjustmentBehavior::kMoveLeft: {
                const AXObject* previous_container
                    = FindNeighboringUnignoredObject(*document, *container_node, container_node->parentNode(), adjustment_behavior);
                if (previous_container) {
                    return CreatePositionAfterObject(*previous_container, adjustment_behavior);
                }
                ...
                if (!container || !container->ParentObjectIncludedInTree())
                    return {};
                return CreateFirstPositionInObject(*container->ParentObjectIncludedInTree(), adjustment_behavior);
            }
            }
        }
        ...
        LayoutBlockFlow* formatting_context = OffsetMapping::GetInlineFormattingContextOf(parent_anchored_position);
        const OffsetMapping* container_offset_mapping = formatting_context ? InlineNode::GetOffsetMapping(formatting_context) : nullptr;
        ...
        std::optional<unsigned> content_offset = container_offset_mapping->GetTextContentOffset(parent_anchored_position);
        int text_offset = 0;
        if (content_offset.has_value()) {
            ...
            int adjustment
                = ax_position.GetLeadingIgnoredCharacterCount(container_offset_mapping, container->GetClosestNode(), container_offset, content_offset.value());
    ...
    if (!node_after_position) {
        ax_position.text_offset_or_child_index_ = container->ChildCountIncludingIgnored();
        ...
        if (!ax_child || !ax_child->IsIncludedInTree()) {
            ...
            switch (adjustment_behavior) {
            case AXPositionAdjustmentBehavior::kMoveRight: {
                const AXObject* next_child
                    = FindNeighboringUnignoredObject(*document, *node_after_position, DynamicTo<ContainerNode>(container_node), adjustment_behavior);
                if (next_child) {
                    return CreatePositionBeforeObject(*next_child, adjustment_behavior);
                }
            ...
            case AXPositionAdjustmentBehavior::kMoveLeft: {
                ...
                if (previous_child) {
                    ...
                    // itself.
                    return CreateLastPositionInObject(*previous_child, adjustment_behavior);
                }
...
// static
const AXPosition AXPosition::FromPosition(const PositionWithAffinity& position_with_affinity, const AXPositionAdjustmentBehavior adjustment_behavior)
{
    return FromPosition(position_with_affinity.GetPosition(), position_with_affinity.Affinity(), adjustment_behavior);
}
...
int AXPosition::MaxTextOffset() const
{
    ...
    // AXObject::TextLength().
    const bool is_atomic_inline_level = layout_object->IsInline() && layout_object->IsAtomicInlineLevel();
    ...
    LayoutBlockFlow* formatting_context = OffsetMapping::GetInlineFormattingContextOf(*layout_object);
    const OffsetMapping* container_offset_mapping = formatting_context ? InlineNode::GetOffsetMapping(formatting_context) : nullptr;
    if (!container_offset_mapping)
        return container_object_->ComputedName().length();
    const base::span<const OffsetMappingUnit> mapping_units = container_offset_mapping->GetMappingUnitsForNode(*container_object_->GetClosestNode());
...
bool AXPosition::IsValid(String* failure_reason) const
{
    ...
    if (container_object_->GetClosestNode() && !container_object_->GetClosestNode()->isConnected()) {
        if (failure_reason) {
    ...
    if (IsTextPosition()) {
        if (text_offset_or_child_index_ > MaxTextOffset()) {
            if (failure_reason) {
                *failure_reason = String::Format("\nPosition invalid: text offset too large.\n%d vs. %d.", text_offset_or_child_index_, MaxTextOffset());
            }
            return false;
        }
    } else {
        if (text_offset_or_child_index_ > container_object_->ChildCountIncludingIgnored()) {
            if (failure_reason) {
                *failure_reason = String::Format(
                    "\nPosition invalid: child index too large.\n%d vs. %d.", text_offset_or_child_index_, container_object_->ChildCountIncludingIgnored());
            }
            return false;
        }
    }
...
#if DCHECK_IS_ON()
    DCHECK_EQ(container_object_->GetDocument()->DomTreeVersion(), dom_tree_version_);
...
bool AXPosition::IsTextPosition() const
{
    ...
    if (!container_object_)
        return false;
    return container_object_->IsTextObject() || container_object_->IsAtomicTextField();
}
...
const AXPosition AXPosition::CreateNextPosition() const
{
    ...
    if (IsTextPosition() && TextOffset() < MaxTextOffset()) {
        return CreatePositionInTextObject(*container_object_, (TextOffset() + 1), TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveRight);
    }
    ...
    if (!child) {
        ...
        const AXObject* next_in_order = container_object_->ChildCountIncludingIgnored()
            ? container_object_->DeepestLastChildIncludingIgnored()->NextInPreOrderIncludingIgnored()
            : container_object_->NextInPreOrderIncludingIgnored();
        if (!next_in_order || !next_in_order->ParentObjectIncludedInTree())
            return {};

        return CreatePositionBeforeObject(*next_in_order, AXPositionAdjustmentBehavior::kMoveRight);
    }

    if (!child->ParentObjectIncludedInTree())
        return {};

    return CreatePositionAfterObject(*child, AXPositionAdjustmentBehavior::kMoveRight);
}
...
const AXPosition AXPosition::CreatePreviousPosition() const
{
    ...
    if (IsTextPosition() && TextOffset() > 0) {
        return CreatePositionInTextObject(*container_object_, (TextOffset() - 1), TextAffinity::kDownstream, AXPositionAdjustmentBehavior::kMoveLeft);
    }
    ...
    if (!child) {
        ...
        // text boxes when present, because we'll just be creating a text position
        // in the same piece of text.
        if (!container_object_->IsTextObject() && container_object_->ChildCountIncludingIgnored()) {
            ...
            if (last_child->IsTextObject() || last_child->IsAtomicTextField()) {
                return CreatePositionAfterObject(*last_child, AXPositionAdjustmentBehavior::kMoveLeft);
            }

            return CreatePositionBeforeObject(*last_child, AXPositionAdjustmentBehavior::kMoveLeft);
        }

        object_before_position = container_object_->PreviousInPreOrderIncludingIgnored();
    } else {
        object_before_position = child->PreviousInPreOrderIncludingIgnored();
    }

    if (!object_before_position || !object_before_position->ParentObjectIncludedInTree()) {
        return {};
    }

    // Dont skip over any intervening text.
    if (object_before_position->IsTextObject() || object_before_position->IsAtomicTextField()) {
        return CreatePositionAfterObject(*object_before_position, AXPositionAdjustmentBehavior::kMoveLeft);
    }

    return CreatePositionBeforeObject(*object_before_position, AXPositionAdjustmentBehavior::kMoveLeft);
}

const AXPosition AXPosition::AsUnignoredPosition(const AXPositionAdjustmentBehavior adjustment_behavior) const
{
    if (!IsValid())
        return {};
    ...
    if (!container->IsIncludedInTree()) {
        ...
        case AXPositionAdjustmentBehavior::kMoveLeft:
            return CreatePreviousPosition().AsUnignoredPosition(adjustment_behavior);
        }
    }
...
const AXPosition AXPosition::AsValidDOMPosition(const AXPositionAdjustmentBehavior adjustment_behavior) const
{
    if (!IsValid())
        return {};
    ...
    if ((IsTextPosition() && (!container->GetClosestNode() || container->GetClosestNode()->IsMarkerPseudoElement()))
        || (!child && last_child && (!last_child->GetClosestNode() || last_child->GetClosestNode()->IsMarkerPseudoElement()))
        || (child && (!child->GetClosestNode() || child->GetClosestNode()->IsMarkerPseudoElement()))) {
        AXPosition result;
    ...
    if (new_container == container->ParentObjectIncludedInTree()) {
        ...
        switch (adjustment_behavior) {
        case AXPositionAdjustmentBehavior::kMoveRight:
            position.text_offset_or_child_index_ = new_container->ChildCountIncludingIgnored();
            break;
        case AXPositionAdjustmentBehavior::kMoveLeft:
            position.text_offset_or_child_index_ = 0;
            break;
...
const PositionWithAffinity AXPosition::ToPositionWithAffinity(const AXPositionAdjustmentBehavior adjustment_behavior) const
{
    const AXPosition adjusted_position = AsValidDOMPosition(adjustment_behavior);
    if (!adjusted_position.IsValid())
        return {};

    const Node* container_node = adjusted_position.container_object_->GetClosestNode();
    ...
    if (!adjusted_position.IsTextPosition()) {
        ...
        if (child) {
            ...
            if (!child_node->previousSibling()) {
                ...
                return PositionWithAffinity(Position::FirstPositionInNode(*container_node), affinity_);
            }

            // Creates a |PositionAnchorType::kOffsetInAnchor| position.
            return PositionWithAffinity(Position::InParentBeforeNode(*child_node), affinity_);
        }
        ...
        if (last_child) {
            ...
            if (!last_child_node->nextSibling()) {
                ...
                return PositionWithAffinity(Position::LastPositionInNode(*container_node), affinity_);
            }

            // Do the next best thing by creating a
            // |PositionAnchorType::kOffsetInAnchor| position after the last unignored
            // child.
            return PositionWithAffinity(Position::InParentAfterNode(*last_child_node), affinity_);
        }
    ...
    const bool supports_ng_offset_mapping = layout_object && ((layout_object->IsInline() && layout_object->IsAtomicInlineLevel()) || layout_object->IsText());
    const OffsetMapping* container_offset_mapping = nullptr;
    if (supports_ng_offset_mapping) {
        LayoutBlockFlow* formatting_context = OffsetMapping::GetInlineFormattingContextOf(*layout_object);
        container_offset_mapping = formatting_context ? InlineNode::GetOffsetMapping(formatting_context) : nullptr;
    }
    ...
    int text_offset_in_formatting_context = adjusted_position.container_object_->TextOffsetInFormattingContext(adjusted_position.TextOffset());
    ...
    Position dom_position = adjusted_position.TextOffset() < adjusted_position.MaxTextOffset()
        ? container_offset_mapping->GetLastPosition(static_cast<unsigned int>(text_offset_in_formatting_context))
        : container_offset_mapping->GetFirstPosition(static_cast<unsigned int>(text_offset_in_formatting_context));
...
const Position AXPosition::ToPosition(const AXPositionAdjustmentBehavior adjustment_behavior) const
{
...
int AXPosition::GetLeadingIgnoredCharacterCount(const OffsetMapping* mapping, const Node* node, int container_offset, int content_offset) const
{
    if (!mapping) {
    ...
    for (auto unit : mapping->GetMappingUnitsForNode(*node)) {
        ...
        if (unit.TextContentStart() != previous_content_end) {
            String substring = text.Substring(previous_content_end, unit.TextContentStart() - previous_content_end);
...
const AXObject* AXPosition::FindNeighboringUnignoredObject(
    const Document& document, const Node& child_node, const ContainerNode* container_node, const AXPositionAdjustmentBehavior adjustment_behavior)
{
    AXObjectCache* ax_object_cache = document.ExistingAXObjectCache();
    if (!ax_object_cache)
        return nullptr;
    ...
    case AXPositionAdjustmentBehavior::kMoveRight: {
        ...
        while ((next_node = NodeTraversal::NextIncludingPseudo(*next_node, container_node))) {
    ...
    case AXPositionAdjustmentBehavior::kMoveLeft: {
        ...
        while ((previous_node = NodeTraversal::PreviousIncludingPseudo(*previous_node, container_node)) && previous_node != container_node) {
            const AXObject* previous_object = ax_object_cache_impl->Get(previous_node);
...
#if DCHECK_IS_ON()
    String failure_reason;
    DCHECK(a.IsValid(&failure_reason) && b.IsValid(&failure_reason)) << failure_reason;
#endif
...
bool operator<(const AXPosition& a, const AXPosition& b)
{
    ...
    const AXObject* ancestor = AXObject::LowestCommonAncestor(*a.ContainerObject(), *b.ContainerObject(), &index_in_ancestor1, &index_in_ancestor2);
