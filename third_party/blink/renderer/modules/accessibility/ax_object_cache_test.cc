...
TEST_F(AccessibilityTest, IsARIAWidget)
{
    ...
    EXPECT_FALSE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById(AtomicString("plain"))));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById(AtomicString("button"))));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById(AtomicString("button-parent"))));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById(AtomicString("button-caps"))));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById(AtomicString("button-second"))));
    EXPECT_FALSE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById(AtomicString("aria-bogus"))));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById(AtomicString("aria-selected"))));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById(AtomicString("haspopup"))));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById(AtomicString("focusable"))));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById(AtomicString("focusable-parent"))));
}
...
TEST_F(AccessibilityTest, HistogramTest)
{
    ...
    histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Snapshot", 0);
    histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental", 0);
    histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental.Float", 0);
    histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental.Int", 0);
    histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental.HTML", 0);
    histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental.String", 0);

    {
        ui::AXTreeUpdate response;
        ScopedFreezeAXCache freeze(cache);
        cache.SerializeEntireTree(/* max_node_count */ 1000, base::TimeDelta::FiniteMax(), &response);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Snapshot", 1);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental", 0);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental.Float", 0);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental.Int", 0);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental.HTML", 0);
...

    {
        ...
        ScopedFreezeAXCache freeze(cache);
        cache.GetUpdatesAndEventsForSerialization(updates, events, had_end_of_test_event, had_load_complete_messages);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Snapshot", 1);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental", 1);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental.Float", 1);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental.Int", 1);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental.HTML", 1);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.AXObjectCacheImpl.Incremental.String", 1);
    }
}
...
TEST_F(AccessibilityTest, PauseUpdatesAfterMaxNumberQueued)
{
    ...
    for (unsigned i = 0; i < max_updates + 1; i++) {
        ax_object_cache->DeferTreeUpdate(AXObjectCacheImpl::TreeUpdateReason::kChildrenChanged, ax_obj);
    }
...
TEST_F(AccessibilityTest, UpdateAXForAllDocumentsAfterPausedUpdates)
{
    ...
    ax_object_cache->DeferTreeUpdate(AXObjectCacheImpl::TreeUpdateReason::kChildrenChanged, root);
    ax_object_cache->DeferTreeUpdate(AXObjectCacheImpl::TreeUpdateReason::kChildrenChanged, root);
...
TEST_F(AXViewTransitionTest, TransitionPseudoNotRelevant)
{
    ...
    auto* transition = ViewTransitionSupplement::startViewTransition(script_state, GetDocument(), view_transition_callback, ASSERT_NO_EXCEPTION);

    ScriptPromiseTester finish_tester(script_state, transition->finished(script_state));
    ...
    auto* image_wrapper_pseudo = container_pseudo->GetPseudoElement(kPseudoIdViewTransitionImagePair, AtomicString("shared"));
    ASSERT_TRUE(image_wrapper_pseudo);
    auto* incoming_image_pseudo = image_wrapper_pseudo->GetPseudoElement(kPseudoIdViewTransitionNew, AtomicString("shared"));
    ASSERT_TRUE(incoming_image_pseudo);
    auto* outgoing_image_pseudo = image_wrapper_pseudo->GetPseudoElement(kPseudoIdViewTransitionOld, AtomicString("shared"));
    ...
    EXPECT_FALSE(AXObjectCacheImpl::IsRelevantPseudoElement(*image_wrapper_pseudo));
    EXPECT_FALSE(AXObjectCacheImpl::IsRelevantPseudoElement(*incoming_image_pseudo));
    EXPECT_FALSE(AXObjectCacheImpl::IsRelevantPseudoElement(*outgoing_image_pseudo));
}
...
TEST_F(AccessibilityEnabledLaterTest, CSSAnchorPositioning)
{
    ...
    DCHECK(GetElementById("1")->GetComputedStyle()->AnchorName()->GetNames()[0]->GetName() == "--anchor-el");
    DCHECK(GetElementById("2")->GetComputedStyle()->PositionAnchor()->GetName() == "--anchor-el");

    EnableAccessibility();
    AXObject* anchor = GetAXObjectByElementId("1");
    AXObject* positioned_object = GetAXObjectByElementId("2");
    EXPECT_EQ(GetAXObjectCache().GetPositionedObjectForAnchor(anchor), positioned_object);
    EXPECT_EQ(GetAXObjectCache().GetAnchorForPositionedObject(positioned_object), anchor);
}
