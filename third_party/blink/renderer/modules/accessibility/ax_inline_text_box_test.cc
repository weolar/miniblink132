...
TEST_F(AccessibilityTest, GetWordBoundaries)
{
    ...
    const AXObject* ax_inline_text_box = ax_paragraph->DeepestFirstChildIncludingIgnored();
    ASSERT_NE(nullptr, ax_inline_text_box);
    ASSERT_EQ(ax::mojom::Role::kInlineTextBox, ax_inline_text_box->RoleValue());

    VectorOf<int> expected_word_starts { 0, 1, 5, 7, 9, 11, 14, 18, 19, 22, 23, 24, 25, 29, 31, 32, 33, 34, 38, 40, 41 };
    VectorOf<int> expected_word_ends { 1, 5, 6, 8, 10, 13, 17, 19, 22, 23, 24, 25, 29, 30, 32, 33, 34, 38, 40, 41, 43 };
...
TEST_F(AccessibilityTest, GetDocumentMarkers)
{
    ...
    ASSERT_NE(nullptr, ax_inline_text_box);
    {
        ...
        EXPECT_EQ(std::vector<int32_t> { int32_t(AXMarkerType::kSpelling) }, node_data.GetIntListAttribute(AXIntListAttribute::kMarkerTypes));
        EXPECT_EQ(std::vector<int32_t> { 0 }, node_data.GetIntListAttribute(AXIntListAttribute::kMarkerStarts));
        EXPECT_EQ(std::vector<int32_t> { 11 }, node_data.GetIntListAttribute(AXIntListAttribute::kMarkerEnds));
    }
    ...
    ASSERT_NE(nullptr, ax_inline_text_box);
    {
        ...
        ui::AXNodeData node_data;
        ax_inline_text_box->Serialize(&node_data, ui::kAXModeComplete);
        EXPECT_EQ((std::vector<int32_t> { int32_t(AXMarkerType::kSpelling), int32_t(AXMarkerType::kGrammar) }),
            node_data.GetIntListAttribute(AXIntListAttribute::kMarkerTypes));
        EXPECT_EQ((std::vector<int32_t> { 0, 10 }), node_data.GetIntListAttribute(AXIntListAttribute::kMarkerStarts));
        EXPECT_EQ((std::vector<int32_t> { 4, 12 }), node_data.GetIntListAttribute(AXIntListAttribute::kMarkerEnds));
    }
    ...
    ASSERT_NE(nullptr, ax_inline_text_box);
    {
        ...
        ui::AXNodeData node_data;
        ax_inline_text_box->Serialize(&node_data, ui::kAXModeComplete);
        EXPECT_EQ(std::vector<int32_t> { int32_t(AXMarkerType::kGrammar) }, node_data.GetIntListAttribute(AXIntListAttribute::kMarkerTypes));
        EXPECT_EQ(std::vector<int32_t> { 0 }, node_data.GetIntListAttribute(AXIntListAttribute::kMarkerStarts));
        EXPECT_EQ(std::vector<int32_t> { 8 }, node_data.GetIntListAttribute(AXIntListAttribute::kMarkerEnds));
    }
    ...
    ASSERT_NE(nullptr, ax_inline_text_box);
    {
        ...
        EXPECT_EQ(std::vector<int32_t> { int32_t(AXMarkerType::kGrammar) }, node_data.GetIntListAttribute(AXIntListAttribute::kMarkerTypes));
        EXPECT_EQ(std::vector<int32_t> { 0 }, node_data.GetIntListAttribute(AXIntListAttribute::kMarkerStarts));
        EXPECT_EQ(std::vector<int32_t> { 5 }, node_data.GetIntListAttribute(AXIntListAttribute::kMarkerEnds));
    }
}
...
TEST_F(AccessibilityTest, TextOffsetInContainerWithASpan)
{
    ...
    ax_inline_text_box = ax_inline_text_box->NextInPreOrderIncludingIgnored()->DeepestFirstChildIncludingIgnored();
    ASSERT_NE(nullptr, ax_inline_text_box);
    ASSERT_EQ(ax::mojom::Role::kInlineTextBox, ax_inline_text_box->RoleValue());
    EXPECT_EQ(0, ax_inline_text_box->TextOffsetInContainer(0));
...
TEST_F(AccessibilityTest, TextOffsetInContainerWithBreakWord)
{
    ...
    EXPECT_EQ(text_end_offset, ax_inline_text_box->TextOffsetInContainer(ax_inline_text_box->TextLength()));
...
TEST_F(AccessibilityTest, GetTextDirection)
{
    ...
    const AXObject* ax_text_box = ax_paragraph->DeepestFirstChildIncludingIgnored();
...
TEST_F(AccessibilityTest, AXBlockFlowIteratorAPI_Simple)
{
    ...
    std::optional<AXBlockFlowIterator::MapKey> previous_on_line = it.PreviousOnLine();
...
TEST_F(AccessibilityTest, AXBlockFlowIteratorAPI_SoftLinebreak)
{
    ...
    const AXObject* ax_textarea_container = ax_textarea->FirstChildIncludingIgnored();
    ASSERT_NE(nullptr, ax_textarea_container);
    const AXObject* ax_text_box_static_text = ax_textarea_container->FirstChildIncludingIgnored();
...
TEST_F(AccessibilityTest, AXBlockFlowIteratorAPI_Ruby)
{
    ...
    std::optional<AXBlockFlowIterator::MapKey> next_on_line = it.NextOnLine();
    ASSERT_TRUE(next_on_line.has_value());
    ASSERT_EQ("ruby text", AXBlockFlowIterator::GetTextForTesting(next_on_line.value()));
...
TEST_F(AccessibilityTest, AXBlockFlowIteratorAPI_Ruby2)
{
    ...
    std::optional<AXBlockFlowIterator::MapKey> next_on_line = it.NextOnLine();
    ASSERT_TRUE(next_on_line.has_value());
    ASSERT_EQ("ruby", AXBlockFlowIterator::GetTextForTesting(next_on_line.value()));
    ASSERT_TRUE(it.Next());
    ASSERT_EQ("base", it.GetText());
    next_on_line = it.NextOnLine();
    ASSERT_TRUE(next_on_line.has_value());
    ASSERT_EQ("text", AXBlockFlowIterator::GetTextForTesting(next_on_line.value()));
    ASSERT_FALSE(it.Next());
}
