...
// static
SmartCardError* SmartCardError::Create(String message, const SmartCardErrorOptions* options)
{
    return MakeGarbageCollected<SmartCardError>(std::move(message), options->responseCode());
}

// static
void SmartCardError::MaybeReject(ScriptPromiseResolverBase* resolver, device::mojom::blink::SmartCardError mojom_error)
{
    ...
    case device::mojom::blink::SmartCardError::kNoService:
        resolver->Reject(MakeGarbageCollected<SmartCardError>("No smart card service available in the system.", V8SmartCardResponseCode::Enum::kNoService));
        break;
    case device::mojom::blink::SmartCardError::kNoSmartcard:
        resolver->Reject(MakeGarbageCollected<SmartCardError>("The operation requires a smart card, but no smart card is "
                                                              "currently in the device.",
            V8SmartCardResponseCode::Enum::kNoSmartcard));
        break;
    case device::mojom::blink::SmartCardError::kNotReady:
        resolver->Reject(
            MakeGarbageCollected<SmartCardError>("The reader or smart card is not ready to accept commands.", V8SmartCardResponseCode::Enum::kNotReady));
        break;
    case device::mojom::blink::SmartCardError::kNotTransacted:
        resolver->Reject(
            MakeGarbageCollected<SmartCardError>("An attempt was made to end a non-existent transaction.", V8SmartCardResponseCode::Enum::kNotTransacted));
        break;
    case device::mojom::blink::SmartCardError::kProtoMismatch:
        resolver->Reject(MakeGarbageCollected<SmartCardError>("The requested protocols are incompatible with the protocol "
    ...
    case device::mojom::blink::SmartCardError::kReaderUnavailable:
        resolver->Reject(MakeGarbageCollected<SmartCardError>(
            "The specified reader is not currently available for use.", V8SmartCardResponseCode::Enum::kReaderUnavailable));
        break;
    case device::mojom::blink::SmartCardError::kRemovedCard:
        resolver->Reject(MakeGarbageCollected<SmartCardError>("The smart card has been removed, so further communication is not "
                                                              "possible.",
            V8SmartCardResponseCode::Enum::kRemovedCard));
        break;
    case device::mojom::blink::SmartCardError::kResetCard:
        resolver->Reject(MakeGarbageCollected<SmartCardError>("The smart card has been reset, so any shared state information "
                                                              "is invalid.",
            V8SmartCardResponseCode::Enum::kResetCard));
        break;
    case device::mojom::blink::SmartCardError::kServerTooBusy:
        resolver->Reject(MakeGarbageCollected<SmartCardError>("The smart card resource manager is too busy to complete this "
                                                              "operation.",
            V8SmartCardResponseCode::Enum::kServerTooBusy));
        break;
    case device::mojom::blink::SmartCardError::kSharingViolation:
        resolver->Reject(MakeGarbageCollected<SmartCardError>("The smart card cannot be accessed because of other connections "
                                                              "outstanding.",
            V8SmartCardResponseCode::Enum::kSharingViolation));
        break;
    case device::mojom::blink::SmartCardError::kSystemCancelled:
        resolver->Reject(MakeGarbageCollected<SmartCardError>("The action was cancelled by the system, presumably to log off or "
                                                              "shut down.",
            V8SmartCardResponseCode::Enum::kSystemCancelled));
        break;
    case device::mojom::blink::SmartCardError::kUnknownReader:
        resolver->Reject(MakeGarbageCollected<SmartCardError>("The specified reader name is not recognized.", V8SmartCardResponseCode::Enum::kUnknownReader));
        break;
    case device::mojom::blink::SmartCardError::kUnpoweredCard:
        resolver->Reject(MakeGarbageCollected<SmartCardError>("Power has been removed from the smart card, so that further "
                                                              "communication is not possible.",
            V8SmartCardResponseCode::Enum::kUnpoweredCard));
        break;
    case device::mojom::blink::SmartCardError::kUnresponsiveCard:
        resolver->Reject(
            MakeGarbageCollected<SmartCardError>("The smart card is not responding to a reset.", V8SmartCardResponseCode::Enum::kUnresponsiveCard));
        break;
    case device::mojom::blink::SmartCardError::kUnsupportedCard:
        resolver->Reject(MakeGarbageCollected<SmartCardError>("The reader cannot communicate with the card, due to ATR string "
                                                              "configuration conflicts.",
            V8SmartCardResponseCode::Enum::kUnsupportedCard));
        break;
    case device::mojom::blink::SmartCardError::kUnsupportedFeature:
        resolver->Reject(MakeGarbageCollected<SmartCardError>(
            "This smart card does not support the requested feature.", V8SmartCardResponseCode::Enum::kUnsupportedFeature));
        break;
    ...
    case device::mojom::blink::SmartCardError::kInvalidParameter:
        resolver->RejectWithTypeError("One or more of the supplied parameters could not be properly "
                                      "interpreted.");
        break;
    ...
    case device::mojom::blink::SmartCardError::kInvalidHandle:
        resolver->RejectWithDOMException(DOMExceptionCode::kInvalidStateError, "Connection is invalid.");
        break;
    case device::mojom::blink::SmartCardError::kServiceStopped:
        resolver->RejectWithDOMException(DOMExceptionCode::kInvalidStateError, "The smart card resource manager has shut down.");
        break;
    // "AbortError"
    ...
    case device::mojom::blink::SmartCardError::kPermissionDenied:
        resolver->RejectWithDOMException(DOMExceptionCode::kNotAllowedError, "The user has denied permission.");
        break;
    // "UnknownError"
    case device::mojom::blink::SmartCardError::kCommError:
        resolver->RejectWithDOMException(DOMExceptionCode::kUnknownError, "An internal communications error has been detected.");
        break;
    case device::mojom::blink::SmartCardError::kInternalError:
        resolver->RejectWithDOMException(DOMExceptionCode::kUnknownError, "An internal consistency check failed.");
        break;
    case device::mojom::blink::SmartCardError::kNoMemory:
        resolver->RejectWithDOMException(DOMExceptionCode::kUnknownError, "Not enough memory available to complete this command.");
        break;
    case device::mojom::blink::SmartCardError::kUnexpected:
        resolver->RejectWithDOMException(DOMExceptionCode::kUnknownError, "An unexpected card error has occurred.");
        break;
    case device::mojom::blink::SmartCardError::kUnknownError:
        resolver->RejectWithDOMException(DOMExceptionCode::kUnknownError, "An internal error has been detected, but the source is unknown.");
        break;
    case device::mojom::blink::SmartCardError::kUnknown:
        ...
        resolver->RejectWithDOMException(DOMExceptionCode::kUnknownError, "An unknown error has occurred.");
        break;
...
SmartCardError::SmartCardError(String message, V8SmartCardResponseCode::Enum response_code_enum)
    : SmartCardError(std::move(message), V8SmartCardResponseCode(response_code_enum))
{
}

SmartCardError::SmartCardError(String message, V8SmartCardResponseCode response_code)
