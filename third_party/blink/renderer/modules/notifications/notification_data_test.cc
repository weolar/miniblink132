...
const mojom::blink::NotificationActionType kBlinkNotificationActionType = mojom::blink::NotificationActionType::TEXT;
...
const std::array<unsigned, 4> kNotificationVibrationUnnormalized = { 10, 1000000, 50, 42 };
...
TEST(NotificationDataTest, ReflectProperties)
{
    ...
    auto* vibration_sequence = MakeGarbageCollected<V8UnionUnsignedLongOrUnsignedLongSequence>(vibration_pattern);
    ...
    const DOMTimeStamp show_timestamp = base::Time::Now().InMillisecondsSinceUnixEpoch();
    ...
    mojom::blink::NotificationDataPtr notification_data = CreateNotificationData(scope.GetExecutionContext(), kNotificationTitle, options, exception_state);
    ASSERT_THAT(exception_state, HadNoException());

    EXPECT_EQ(kNotificationTitle, notification_data->title);

    EXPECT_EQ(mojom::blink::NotificationDirection::RIGHT_TO_LEFT, notification_data->direction);
    ...
    EXPECT_EQ(base::Time::FromMillisecondsSinceUnixEpoch(static_cast<int64_t>(show_timestamp)), notification_data->show_trigger_timestamp);
    ...
    ASSERT_EQ(vibration_pattern.size(), notification_data->vibration_pattern->size());
    for (wtf_size_t i = 0; i < vibration_pattern.size(); ++i) {
        EXPECT_EQ(vibration_pattern[i], static_cast<unsigned>(notification_data->vibration_pattern.value()[i]));
    }
    ...
    EXPECT_EQ(kNotificationRequireInteraction, notification_data->require_interaction);
...
TEST(NotificationDataTest, SilentNotificationWithVibration)
{
    ...
    auto* vibration_sequence = MakeGarbageCollected<V8UnionUnsignedLongOrUnsignedLongSequence>(std::move(vibration_pattern));
    ...
    auto& exception_state = scope.GetExceptionState();
    mojom::blink::NotificationDataPtr notification_data = CreateNotificationData(scope.GetExecutionContext(), kNotificationTitle, options, exception_state);
    ASSERT_THAT(exception_state, HadException(ESErrorType::kTypeError, "Silent notifications must not specify vibration patterns."));
}
...
TEST(NotificationDataTest, ActionTypeButtonWithPlaceholder)
{
    ...
    auto& exception_state = scope.GetExceptionState();
    mojom::blink::NotificationDataPtr notification_data = CreateNotificationData(scope.GetExecutionContext(), kNotificationTitle, options, exception_state);
    ASSERT_THAT(exception_state, HadException(ESErrorType::kTypeError, "Notifications of type \"button\" cannot specify a placeholder."));
}
...
TEST(NotificationDataTest, VibrationNormalization)
{
    ...
    auto* vibration_sequence = MakeGarbageCollected<V8UnionUnsignedLongOrUnsignedLongSequence>(unnormalized_pattern);
    ...
    ASSERT_EQ(normalized_pattern.size(), notification_data->vibration_pattern->size());
    for (wtf_size_t i = 0; i < normalized_pattern.size(); ++i) {
        EXPECT_EQ(normalized_pattern[i], notification_data->vibration_pattern.value()[i]);
    }
}
...
TEST(NotificationDataTest, DefaultTimestampValue)
{
    ...
    EXPECT_NEAR(notification_data->timestamp, base::Time::Now().InMillisecondsFSinceUnixEpoch(), 32);
}
...
TEST(NotificationDataTest, RejectsTriggerTimestampOverAYear)
{
    ...
    base::Time show_timestamp = base::Time::Now() + kMaxNotificationShowTriggerDelay + base::Days(1);
    TimestampTrigger* show_trigger = TimestampTrigger::Create(show_timestamp.InMillisecondsFSinceUnixEpoch());
    ...
    auto& exception_state = scope.GetExceptionState();
    mojom::blink::NotificationDataPtr notification_data = CreateNotificationData(scope.GetExecutionContext(), kNotificationTitle, options, exception_state);
    ASSERT_THAT(exception_state, HadException(ESErrorType::kTypeError, "Notification trigger timestamp too far ahead in the future."));
}
