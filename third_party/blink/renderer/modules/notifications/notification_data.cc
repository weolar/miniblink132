...
mojom::blink::NotificationDirection ToDirectionEnumValue(const V8NotificationDirection& direction)
{
...
mojom::blink::NotificationScenario ToScenarioEnumValue(const V8NotificationScenario& scenario)
{
...
mojom::blink::NotificationDataPtr CreateNotificationData(
    ExecutionContext* context, const String& title, const NotificationOptions* options, ExceptionState& exception_state)
{
    // If silent is true, the notification must not have a vibration pattern.
    if (options->hasVibrate() && options->silent()) {
        RecordPersistentNotificationDisplayResult(PersistentNotificationDisplayResult::kSilentWithVibrate);
        exception_state.ThrowTypeError("Silent notifications must not specify vibration patterns.");
        return nullptr;
    }

    // If renotify is true, the notification must have a tag.
    if (options->renotify() && options->tag().empty()) {
        RecordPersistentNotificationDisplayResult(PersistentNotificationDisplayResult::kRenotifyWithoutTag);
        exception_state.ThrowTypeError("Notifications which set the renotify flag must specify a non-empty "
                                       "tag.");
        return nullptr;
    }
    ...
    if (options->hasVibrate()) {
        vibration_pattern = VibrationController::SanitizeVibrationPattern(options->vibrate());
    }
    notification_data->vibration_pattern = Vector<int32_t>();
    notification_data->vibration_pattern->AppendSpan(base::span(vibration_pattern));

    notification_data->timestamp = options->hasTimestamp() ? static_cast<double>(options->timestamp()) : base::Time::Now().InMillisecondsFSinceUnixEpoch();
    ...
    if (options->hasData() && !options->data().IsNull()) {
        ...
        scoped_refptr<SerializedScriptValue> serialized_script_value
            = SerializedScriptValue::Serialize(isolate, data.V8Value(), serialize_options, exception_state);
        if (exception_state.HadException()) {
            RecordPersistentNotificationDisplayResult(PersistentNotificationDisplayResult::kFailedToSerializeData);
            return nullptr;
        }
    ...
    for (const NotificationAction* action : options->actions()) {
        ...
        if (action->type() == "button") {
            ...
            NOTREACHED() << "Unknown action type: " << IDLEnumAsString(action->type());
        }

        if (!action->placeholder().IsNull() && notification_action->type == mojom::blink::NotificationActionType::BUTTON) {
            RecordPersistentNotificationDisplayResult(PersistentNotificationDisplayResult::kButtonActionWithPlaceholder);
            exception_state.ThrowTypeError("Notifications of type \"button\" cannot specify a placeholder.");
            return nullptr;
        }
    ...
    if (options->hasShowTrigger()) {
        ...
        auto timestamp = base::Time::FromMillisecondsSinceUnixEpoch(base::checked_cast<int64_t>(timestamp_trigger->timestamp()));

        if (timestamp - base::Time::Now() > kMaxNotificationShowTriggerDelay) {
            RecordPersistentNotificationDisplayResult(PersistentNotificationDisplayResult::kShowTriggerDelayTooFarAhead);
            exception_state.ThrowTypeError("Notification trigger timestamp too far ahead in the future.");
            return nullptr;
        }
