...
class MODULES_EXPORT NotificationResourcesLoader final : public GarbageCollected<NotificationResourcesLoader> {
...
public:
    ...
    using CompletionCallback = base::OnceCallback<void(NotificationResourcesLoader*)>;
    ...
    // more than once.
    void Start(ExecutionContext* context, const mojom::blink::NotificationData& notification_data);
...
private:
    void LoadIcon(ExecutionContext* context, const KURL& url, const gfx::Size& resize_dimensions, ThreadedIconLoader::IconCallback icon_callback);
