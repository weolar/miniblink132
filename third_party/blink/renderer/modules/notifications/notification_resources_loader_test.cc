...
public:
    NotificationResourcesLoaderTest()
        : loader_(MakeGarbageCollected<NotificationResourcesLoader>(WTF::BindOnce(&NotificationResourcesLoaderTest::DidFetchResources, WTF::Unretained(this))))
    {
    }
...
protected:
    ...
    NotificationResourcesLoader* Loader() const
    {
        return loader_.Get();
    }
    ...
    void StartAndWaitForResources(const mojom::blink::NotificationData& notification_data)
    {
    ...
    {
        KURL registered_url
            = url_test_helpers::RegisterMockedURLLoadFromBase(kResourcesLoaderBaseUrl, test::CoreTestDataPath(kResourcesLoaderBaseDir), file_name, "image/png");
...
TEST_F(NotificationResourcesLoaderTest, LoadMultipleResources)
{
    ...
    notification_data->actions.value()[0]->icon = RegisterMockedURL(kResourcesLoaderIcon110x110);
    notification_data->actions->push_back(mojom::blink::NotificationAction::New());
    notification_data->actions.value()[1]->icon = RegisterMockedURL(kResourcesLoaderIcon120x120);
