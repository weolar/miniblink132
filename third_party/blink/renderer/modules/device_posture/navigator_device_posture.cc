...
{
    DCHECK(RuntimeEnabledFeatures::DevicePostureEnabled(navigator.GetExecutionContext()));

    UseCounter::Count(navigator.GetExecutionContext(), WebFeature::kFoldableAPIs);
    NavigatorDevicePosture* supplement = Supplement<Navigator>::From<NavigatorDevicePosture>(navigator);
    if (!supplement) {
...
NavigatorDevicePosture::NavigatorDevicePosture(Navigator& navigator)
    ...
    , posture_(MakeGarbageCollected<DevicePosture>(GetSupplementable()->DomWindow()))
{
}
