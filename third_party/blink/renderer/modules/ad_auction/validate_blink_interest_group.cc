...
bool IsUrlAllowedForTrustedBiddingSignals(const KURL& url, const mojom::blink::InterestGroup& group, String& error_out)
{
    if (!IsUrlAllowedForRenderUrls(url) || url.HasFragmentIdentifier() || !group.trusted_bidding_signals_url->Query().IsNull()) {
        error_out = "trustedBiddingSignalsURL must have https schema and have no query "
...
size_t EstimateBlinkInterestGroupSize(const mojom::blink::InterestGroup& group)
{
    size_t size = 0u;
...
bool ValidateBlinkInterestGroup(const mojom::blink::InterestGroup& group, String& error_field_name, String& error_field_value, String& error)
{
    ...
    // valid.
    if (group.execution_mode != mojom::blink::InterestGroup::ExecutionMode::kCompatibilityMode
        && group.execution_mode != mojom::blink::InterestGroup::ExecutionMode::kGroupedByOriginMode
        && group.execution_mode != mojom::blink::InterestGroup::ExecutionMode::kFrozenContext) {
    ...
    if (group.bidding_url) {
        if (!IsUrlAllowed(*group.bidding_url, group)) {
            ...
            error = "biddingLogicURL must have the same origin as the InterestGroup "
    ...
    if (group.bidding_wasm_helper_url) {
        if (!IsUrlAllowed(*group.bidding_wasm_helper_url, group)) {
            ...
            error = "biddingWasmHelperURL must have the same origin as the InterestGroup "
                    "owner and have no fragment identifier or embedded credentials.";
            return false;
        }
    }
    ...
    if (group.update_url) {
        if (!IsUrlAllowed(*group.update_url, group)) {
            ...
            error = "updateURL must have the same origin as the InterestGroup owner "
    ...
    if (group.trusted_bidding_signals_url) {
        ...
        if (!IsUrlAllowedForTrustedBiddingSignals(*group.trusted_bidding_signals_url, group, trusted_url_error)
    ...
    // checks in navigator_auction.cc should ensure the execution mode is always
    // valid.
    if (group.trusted_bidding_signals_slot_size_mode != mojom::blink::InterestGroup::TrustedBiddingSignalsSlotSizeMode::kNone
        && group.trusted_bidding_signals_slot_size_mode != mojom::blink::InterestGroup::TrustedBiddingSignalsSlotSizeMode::kSlotSize
        && group.trusted_bidding_signals_slot_size_mode != mojom::blink::InterestGroup::TrustedBiddingSignalsSlotSizeMode::kAllSlotsRequestedSizes) {
        error_field_name = "trustedBiddingSignalsSlotSizeMode";
        error_field_value = String::Number(static_cast<int>(group.trusted_bidding_signals_slot_size_mode));
    ...
    if (group.max_trusted_bidding_signals_url_length < 0) {
        ...
        error_field_value = String::Number(group.max_trusted_bidding_signals_url_length);
        error = "maxTrustedBiddingSignalsURLLength is negative.";
        return false;
    }

    if (group.trusted_bidding_signals_coordinator) {
        if (group.trusted_bidding_signals_coordinator->Protocol() != url::kHttpsScheme) {
    ...
    if (group.ads) {
        for (WTF::wtf_size_t i = 0; i < group.ads.value().size(); ++i) {
            ...
            auto& allowed_reporting_origins = group.ads.value()[i]->allowed_reporting_origins;
            if (allowed_reporting_origins) {
                if (allowed_reporting_origins->size() > mojom::blink::kMaxAllowedReportingOrigins) {
                    error_field_name = String::Format("ads[%u].allowedReportingOrigins", i);
                    error_field_value = "";
                    error = String::Format("allowedReportingOrigins cannot have more than %hu elements.", mojom::blink::kMaxAllowedReportingOrigins);
                    return false;
                }
                for (WTF::wtf_size_t j = 0; j < allowed_reporting_origins->size(); ++j) {
                    if (allowed_reporting_origins.value()[j]->Protocol() != url::kHttpsScheme) {
                        error_field_name = String::Format("ads[%u].allowedReportingOrigins", i);
                        error_field_value = allowed_reporting_origins.value()[j]->ToString();
    ...
    if (group.ad_components) {
        for (WTF::wtf_size_t i = 0; i < group.ad_components.value().size(); ++i) {
            ...
            const WTF::String& ad_component_size_group = group.ad_components.value()[i]->size_group;
            if (!ad_component_size_group.IsNull()) {
                ...
                if (!group.size_groups || !group.size_groups->Contains(ad_component_size_group)) {
                    error_field_name = String::Format("adComponents[%u].sizeGroup", i);
                    error_field_value = ad_component_size_group;
                    error = "The assigned size group does not exist in sizeGroups map.";
                    return false;
            ...
            if (group.ad_components.value()[i]->ad_render_id.length() > kMaxAdRenderIdSize) {
            ...
            DCHECK(group.ad_components.value()[i]->buyer_and_seller_reporting_id.IsNull());
            DCHECK(!group.ad_components.value()[i]->selectable_buyer_and_seller_reporting_ids.has_value());
            DCHECK(!group.ad_components.value()[i]->allowed_reporting_origins.has_value());
        }
    }
    ...
    if (group.ad_sizes) {
        for (auto const& it : group.ad_sizes.value()) {
            ...
            if (it.value->width_units == mojom::blink::AdSize::LengthUnit::kInvalid || it.value->height_units == mojom::blink::AdSize::LengthUnit::kInvalid) {
                error_field_name = "adSizes";
                error_field_value = "";
                error = "Ad size dimensions must be a valid number either in pixels (px) "
                        "or screen width (sw).";
                return false;
            }
            if (it.value->width <= 0 || it.value->height <= 0 || !std::isfinite(it.value->width) || !std::isfinite(it.value->height)) {
                error_field_name = "adSizes";
                error_field_value = String::Format("%f x %f", it.value->width, it.value->height);
                error = "Ad sizes must have a valid (non-zero/non-infinite) width and "
                        "height.";
                return false;
            }
        }
    ...
    if (group.additional_bid_key) {
        if (group.additional_bid_key->size() != X25519_PUBLIC_VALUE_LEN) {
            ...
            error = String::Format("additionalBidKey must be exactly %u bytes.", X25519_PUBLIC_VALUE_LEN);
            return false;
        }
    }

    if (group.additional_bid_key && group.ads) {
        error = "Interest groups that provide a value of additionalBidKey "
    ...
    if (group.aggregation_coordinator_origin && group.aggregation_coordinator_origin->Protocol() != url::kHttpsScheme) {
    ...
    if (size >= mojom::blink::kMaxInterestGroupSize) {
        ...
        error = String::Format("interest groups must be less than %u bytes", mojom::blink::kMaxInterestGroupSize);
        return false;
    }
