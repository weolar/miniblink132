...
DOMMimeType::DOMMimeType(LocalDOMWindow* window, const MimeClassInfo& mime_class_info)
    : ExecutionContextClient(window)
    , mime_class_info_(&mime_class_info)
{
}
...
DOMPlugin* DOMMimeType::enabledPlugin() const
{
    ...
    return NavigatorPlugins::plugins(*DomWindow()->navigator())->namedItem(AtomicString(mime_class_info_->Plugin()->Name()));
}
