...
DOMPluginArray::DOMPluginArray(LocalDOMWindow* window, bool should_return_fixed_plugin_data)
...
DOMPlugin* DOMPluginArray::item(unsigned index)
{
    ...
    if (!dom_plugins_[index]) {
        if (should_return_fixed_plugin_data_)
            return nullptr;
        dom_plugins_[index] = MakeGarbageCollected<DOMPlugin>(DomWindow(), *GetPluginData()->Plugins()[index]);
    }
...
DOMPlugin* DOMPluginArray::namedItem(const AtomicString& property_name)
{
    ...
    for (const Member<PluginInfo>& plugin_info : data->Plugins()) {
        if (plugin_info->Name() == property_name) {
            unsigned index = static_cast<unsigned>(&plugin_info - &data->Plugins()[0]);
            return item(index);
        }
    }
    return nullptr;
...
void DOMPluginArray::NamedPropertyEnumerator(Vector<String>& property_names, ExceptionState&) const
{
    if (should_return_fixed_plugin_data_) {
...
bool DOMPluginArray::NamedPropertyQuery(const AtomicString& property_name, ExceptionState& exception_state) const
{
...
void DOMPluginArray::refresh(bool reload)
{
    ...
    for (Frame* frame = DomWindow()->GetFrame()->GetPage()->MainFrame(); frame; frame = frame->Tree().TraverseNext()) {
...
DOMPlugin* MakeFakePlugin(String plugin_name, LocalDOMWindow* window)
{
    ...
    auto* plugin_info = MakeGarbageCollected<PluginInfo>(plugin_name, filename, description,
    ...
    for (const char* mime_type : { "application/pdf", "text/pdf" }) {
        auto* mime_info = MakeGarbageCollected<MimeClassInfo>(mime_type, description, *plugin_info, extensions);
...
void DOMPluginArray::UpdatePluginData()
{
    if (should_return_fixed_plugin_data_) {
        ...
        if (IsPdfViewerAvailable()) {
            ...
            Vector<String> plugins { "PDF Viewer", "Chrome PDF Viewer", "Chromium PDF Viewer", "Microsoft Edge PDF Viewer", "WebKit built-in PDF" };
