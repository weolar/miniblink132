...
DOMMimeTypeArray::DOMMimeTypeArray(LocalDOMWindow* window, bool should_return_fixed_plugin_data)
...
DOMMimeType* DOMMimeTypeArray::item(unsigned index)
{
    ...
    if (!dom_mime_types_[index]) {
        dom_mime_types_[index] = MakeGarbageCollected<DOMMimeType>(DomWindow(), *GetPluginData()->Mimes()[index]);
    }
...
void DOMMimeTypeArray::NamedPropertyEnumerator(Vector<String>& property_names, ExceptionState&) const
{
    if (should_return_fixed_plugin_data_) {
...
bool DOMMimeTypeArray::NamedPropertyQuery(const AtomicString& property_name, ExceptionState&) const
{
    if (should_return_fixed_plugin_data_) {
...
void DOMMimeTypeArray::UpdatePluginData()
{
    ...
    if (should_return_fixed_plugin_data_) {
        if (DomWindow()) {
            dom_mime_types_ = NavigatorPlugins::plugins(*DomWindow()->navigator())->GetFixedMimeTypeArray();
        }
        return;
    }
    PluginData* data = GetPluginData();
    ...
    HeapVector<Member<DOMMimeType>> old_dom_mime_types(std::move(dom_mime_types_));
        ...
        if (mime) {
            for (const Member<MimeClassInfo>& mime_info : data->Mimes()) {
                if (mime->type() == mime_info->Type()) {
                    unsigned index = static_cast<unsigned>(&mime_info - &data->Mimes()[0]);
