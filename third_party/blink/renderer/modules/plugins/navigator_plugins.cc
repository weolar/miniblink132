...
{
    return NavigatorPlugins::From(navigator).pdfViewerEnabled(navigator.DomWindow());
}
...
{
    if (!IdentifiabilityStudySettings::Get()->ShouldSampleWebFeature(WebFeature::kNavigatorPlugins) || !window) {
        return;
    }
    ...
    for (unsigned i = 0; i < plugins->length(); i++) {
        ...
        for (unsigned j = 0; j < plugin->length(); j++) {
            ...
            builder.AddToken(IdentifiabilityBenignStringToken(mimeType->description()));
            builder.AddToken(IdentifiabilityBenignStringToken(mimeType->suffixes()));
        }
    }
    IdentifiabilityMetricBuilder(window->UkmSourceID()).AddWebFeature(WebFeature::kNavigatorPlugins, builder.GetToken()).Record(window->UkmRecorder());
}

void RecordMimeTypes(LocalDOMWindow* window, DOMMimeTypeArray* mime_types)
{
    constexpr IdentifiableSurface surface = IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kWebFeature, WebFeature::kNavigatorMimeTypes);
    if (!IdentifiabilityStudySettings::Get()->ShouldSampleSurface(surface) || !window) {
        return;
    }
    IdentifiableTokenBuilder builder;
    for (unsigned i = 0; i < mime_types->length(); i++) {
        ...
        builder.AddToken(IdentifiabilityBenignStringToken(mime_type->description()));
    ...
    IdentifiabilityMetricBuilder(window->UkmSourceID()).Add(surface, builder.GetToken()).Record(window->UkmRecorder());
}
...
{
    if (!plugins_) {
        plugins_ = MakeGarbageCollected<DOMPluginArray>(window, should_return_fixed_plugin_data_);
    }
...
{
    if (!mime_types_) {
        mime_types_ = MakeGarbageCollected<DOMMimeTypeArray>(window, should_return_fixed_plugin_data_);
