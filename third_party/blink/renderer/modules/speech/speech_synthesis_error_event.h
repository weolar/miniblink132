...
public:
    static SpeechSynthesisErrorEvent* Create(const AtomicString& type, const SpeechSynthesisErrorEventInit* init);

    SpeechSynthesisErrorEvent(const AtomicString& type, const SpeechSynthesisErrorEventInit* init);

    const V8SpeechSynthesisErrorCode& error() const
    {
        return error_;
    }
