...
class MODULES_EXPORT ThreadSafeScriptContainer : public WTF::ThreadSafeRefCounted<ThreadSafeScriptContainer> {
public:
    ...
    public:
        RawScriptData(const String& encoding, Vector<uint8_t> script_text, Vector<uint8_t> meta_data);
        ~RawScriptData();
        ...
        Vector<uint8_t> TakeScriptText()
        {
            return std::move(script_text_);
        }

        // An array of raw bytes representing the cached metadata.
        Vector<uint8_t> TakeMetaData()
        {
...
private:
    ...
    HashMap<KURL, std::pair<ScriptStatus, std::unique_ptr<RawScriptData>>> script_data_ GUARDED_BY(lock_);
