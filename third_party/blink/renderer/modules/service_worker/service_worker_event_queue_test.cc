...
public:
    ...
    void EnqueueTo(ServiceWorkerEventQueue* event_queue)
    {
        ...
        event_queue->EnqueueNormal(*event_id_, WTF::BindOnce(&MockEvent::Start, weak_factory_.GetWeakPtr()),
            WTF::BindOnce(&MockEvent::Abort, weak_factory_.GetWeakPtr()), std::nullopt);
    }

    void EnqueuePendingTo(ServiceWorkerEventQueue* event_queue)
    {
        event_id_ = event_queue->NextEventId();
        event_queue->EnqueuePending(*event_id_, WTF::BindOnce(&MockEvent::Start, weak_factory_.GetWeakPtr()),
            WTF::BindOnce(&MockEvent::Abort, weak_factory_.GetWeakPtr()), std::nullopt);
    }

    void EnqueueWithCustomTimeoutTo(ServiceWorkerEventQueue* event_queue, base::TimeDelta custom_timeout)
    {
        event_id_ = event_queue->NextEventId();
        event_queue->EnqueueNormal(*event_id_, WTF::BindOnce(&MockEvent::Start, weak_factory_.GetWeakPtr()),
            WTF::BindOnce(&MockEvent::Abort, weak_factory_.GetWeakPtr()), custom_timeout);
    }

    void EnqueueOfflineTo(ServiceWorkerEventQueue* event_queue)
    {
        event_id_ = event_queue->NextEventId();
        event_queue->EnqueueOffline(*event_id_, WTF::BindOnce(&MockEvent::Start, weak_factory_.GetWeakPtr()),
            WTF::BindOnce(&MockEvent::Abort, weak_factory_.GetWeakPtr()), std::nullopt);
    }

    void EnqueueOfflineWithCustomTimeoutTo(ServiceWorkerEventQueue* event_queue, base::TimeDelta custom_timeout)
    {
        event_id_ = event_queue->NextEventId();
        event_queue->EnqueueOffline(*event_id_, WTF::BindOnce(&MockEvent::Start, weak_factory_.GetWeakPtr()),
            WTF::BindOnce(&MockEvent::Abort, weak_factory_.GetWeakPtr()), custom_timeout);
    ...
    void EnqueuePendingDispatchingEventTo(ServiceWorkerEventQueue* event_queue, String tag, Vector<String>* out_tags)
    {
        event_id_ = event_queue->NextEventId();
        event_queue->EnqueuePending(*event_id_,
            WTF::BindOnce(
                [](ServiceWorkerEventQueue* event_queue, MockEvent* event, String tag, Vector<String>* out_tags, int /* event id */) {
                ...
                WTF::Unretained(event_queue), WTF::Unretained(this), std::move(tag), WTF::Unretained(out_tags)),
...
{
    return WTF::BindRepeating([](bool* out_is_called) { *out_is_called = true; }, WTF::Unretained(out_is_called));
}
...
protected:
    void SetUp() override
    {
        task_runner_ = base::MakeRefCounted<base::TestMockTimeTaskRunner>(base::Time::Now(), base::TimeTicks::Now());
        // Ensure all things run on |task_runner_| instead of the default task
        // runner initialized by blink_unittests.
        task_runner_context_ = std::make_unique<base::TestMockTimeTaskRunner::ScopedContext>(task_runner_);
    }
...
private:
    ...
    std::unique_ptr<base::TestMockTimeTaskRunner::ScopedContext> task_runner_context_;
};

TEST_F(ServiceWorkerEventQueueTest, IdleTimer)
{
    const base::TimeDelta kIdleInterval = base::Seconds(mojom::blink::kServiceWorkerDefaultIdleDelayInSeconds);

    bool is_idle = false;
    ServiceWorkerEventQueue event_queue(base::NullCallback(), CreateReceiverWithCalledFlag(&is_idle), task_runner(), task_runner()->GetMockTickClock());
...
TEST_F(ServiceWorkerEventQueueTest, InflightEventBeforeStart)
{
    ...
    bool is_idle = false;
    ServiceWorkerEventQueue event_queue(base::DoNothing(), CreateReceiverWithCalledFlag(&is_idle), task_runner(), task_runner()->GetMockTickClock());
    MockEvent event;
    event.EnqueueTo(&event_queue);
    event_queue.Start();
    task_runner()->FastForwardBy(kIdleInterval);
...
{
    ServiceWorkerEventQueue event_queue(base::DoNothing(), base::DoNothing(), task_runner(), task_runner()->GetMockTickClock());
    event_queue.Start();

    MockEvent event1, event2;
    event1.EnqueueTo(&event_queue);
    event2.EnqueueTo(&event_queue);
    task_runner()->FastForwardBy(ServiceWorkerEventQueue::kUpdateInterval + base::Seconds(1));

    EXPECT_FALSE(event1.status().has_value());
    EXPECT_FALSE(event2.status().has_value());
    event_queue.EndEvent(event1.event_id());
    task_runner()->FastForwardBy(ServiceWorkerEventQueue::kEventTimeout + base::Seconds(1));

    EXPECT_FALSE(event1.status().has_value());
    EXPECT_TRUE(event2.status().has_value());
    EXPECT_EQ(mojom::blink::ServiceWorkerEventStatus::TIMEOUT, event2.status().value());
}
...
TEST_F(ServiceWorkerEventQueueTest, CustomTimeouts)
{
    ...
    event_queue.Start();
    MockEvent event1, event2;
    event1.EnqueueWithCustomTimeoutTo(&event_queue, ServiceWorkerEventQueue::kUpdateInterval - base::Seconds(1));
    event2.EnqueueWithCustomTimeoutTo(&event_queue, ServiceWorkerEventQueue::kUpdateInterval * 2 - base::Seconds(1));
    task_runner()->FastForwardBy(ServiceWorkerEventQueue::kUpdateInterval + base::Seconds(1));

    EXPECT_TRUE(event1.status().has_value());
    EXPECT_FALSE(event2.status().has_value());
    EXPECT_EQ(mojom::blink::ServiceWorkerEventStatus::TIMEOUT, event1.status().value());
    task_runner()->FastForwardBy(ServiceWorkerEventQueue::kUpdateInterval + base::Seconds(1));
...
TEST_F(ServiceWorkerEventQueueTest, BecomeIdleAfterAbort)
{
    ...
    MockEvent event;
    event.EnqueueTo(&event_queue);
    task_runner()->FastForwardBy(ServiceWorkerEventQueue::kEventTimeout + ServiceWorkerEventQueue::kUpdateInterval + base::Seconds(1));
...
TEST_F(ServiceWorkerEventQueueTest, AbortAllOnDestruction)
{
    ...
    EXPECT_TRUE(event1.status().has_value());
    EXPECT_EQ(mojom::blink::ServiceWorkerEventStatus::ABORTED, event1.status().value());
    EXPECT_TRUE(event2.status().has_value());
    EXPECT_EQ(mojom::blink::ServiceWorkerEventStatus::ABORTED, event2.status().value());
}
...
TEST_F(ServiceWorkerEventQueueTest, PushPendingTask)
{
    ServiceWorkerEventQueue event_queue(base::DoNothing(), base::DoNothing(), task_runner(), task_runner()->GetMockTickClock());
    event_queue.Start();
    task_runner()->FastForwardBy(base::Seconds(mojom::blink::kServiceWorkerDefaultIdleDelayInSeconds));
...
TEST_F(ServiceWorkerEventQueueTest, SetIdleTimerDelayToZero)
{
...

    {
        ...
        ServiceWorkerEventQueue event_queue(base::DoNothing(), CreateReceiverWithCalledFlag(&is_idle), task_runner(), task_runner()->GetMockTickClock());
        event_queue.Start();
        std::unique_ptr<StayAwakeToken> token_1 = event_queue.CreateStayAwakeToken();
        std::unique_ptr<StayAwakeToken> token_2 = event_queue.CreateStayAwakeToken();
        event_queue.SetIdleDelay(base::Seconds(0));
        task_runner()->RunUntilIdle();
...
TEST_F(ServiceWorkerEventQueueTest, TimeoutNotStartedEvent)
{
    ...
    MockEvent event1, event2;
    event1.EnqueueWithCustomTimeoutTo(&event_queue, ServiceWorkerEventQueue::kUpdateInterval - base::Seconds(1));
    event2.EnqueueOfflineWithCustomTimeoutTo(&event_queue, ServiceWorkerEventQueue::kUpdateInterval - base::Seconds(1));
