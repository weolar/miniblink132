...
std::optional<ServiceWorkerRouterCondition> RouterConditionToBlink(
    v8::Isolate* isolate, RouterCondition* v8_condition, const KURL& url_pattern_base_url, ExceptionState& exception_state);

[[nodiscard]] bool ExceedsMaxConditionDepth(const RouterCondition* v8_condition, ExceptionState& exception_state, int depth = 0)
{
    CHECK(v8_condition);
    ...
    if (v8_condition->hasNotCondition()) {
        if (ExceedsMaxConditionDepth(v8_condition->notCondition(), exception_state, depth + 1)) {
            CHECK(exception_state.HadException());
            return true;
        }
    }
...
std::optional<SafeUrlPattern> RouterUrlPatternConditionToBlink(
    v8::Isolate* isolate, const V8URLPatternCompatible* url_pattern_compatible, const KURL& url_pattern_base_url, ExceptionState& exception_state)
{
    ...
    URLPattern* url_pattern = URLPattern::From(isolate, url_pattern_compatible, url_pattern_base_url, exception_state);
    if (!url_pattern) {
        CHECK(exception_state.HadException());
        return std::nullopt;
    }

    std::optional<SafeUrlPattern> safe_url_pattern = url_pattern->ToSafeUrlPattern(exception_state);
    if (!safe_url_pattern) {
        CHECK(exception_state.HadException());
        return std::nullopt;
    }
...
std::optional<ServiceWorkerRouterRequestCondition> RouterRequestConditionToBlink(RouterCondition* v8_condition, ExceptionState& exception_state)
{
    CHECK(v8_condition);
    ...
    if (v8_condition->hasRequestMethod()) {
        ...
        request.method = FetchUtils::NormalizeMethod(AtomicString(v8_condition->requestMethod())).Latin1();
    }
    ...
    if (v8_condition->hasRequestDestination()) {
        request_condition_exist = true;
        request.destination = V8RequestDestinationToMojom(v8_condition->requestDestination());
    }
...
std::optional<ServiceWorkerRouterRunningStatusCondition> RouterRunningStatusConditionToBlink(RouterCondition* v8_condition, ExceptionState& exception_state)
{
    CHECK(v8_condition);
    if (!v8_condition->hasRunningStatus()) {
        exception_state.ThrowTypeError("RunningState condition should not be empty.");
        return std::nullopt;
    }
    ...
    case V8RunningStatusEnum::Enum::kRunning:
        running_status.status = ServiceWorkerRouterRunningStatusCondition::RunningStatusEnum::kRunning;
        break;
    case V8RunningStatusEnum::Enum::kNotRunning:
        running_status.status = ServiceWorkerRouterRunningStatusCondition::RunningStatusEnum::kNotRunning;
        break;
    }
    ...
    v8::Isolate* isolate, RouterCondition* v8_condition, const KURL& url_pattern_base_url, ExceptionState& exception_state)
{
    ...
    for (auto&& v8_ob : v8_objects) {
        std::optional<ServiceWorkerRouterCondition> c = RouterConditionToBlink(isolate, v8_ob, url_pattern_base_url, exception_state);
        if (!c) {
            CHECK(exception_state.HadException());
            return std::nullopt;
        }
...
std::optional<ServiceWorkerRouterNotCondition> RouterNotConditionToBlink(
    v8::Isolate* isolate, RouterCondition* v8_condition, const KURL& url_pattern_base_url, ExceptionState& exception_state)
{
    std::optional<ServiceWorkerRouterCondition> c = RouterConditionToBlink(isolate, v8_condition->notCondition(), url_pattern_base_url, exception_state);
    if (!c) {
        CHECK(exception_state.HadException());
        return std::nullopt;
    }
    ServiceWorkerRouterNotCondition not_condition;
    not_condition.condition = std::make_unique<blink::ServiceWorkerRouterCondition>(*c);
    ...
    v8::Isolate* isolate, RouterCondition* v8_condition, const KURL& url_pattern_base_url, ExceptionState& exception_state)
{
    ...
    if (v8_condition->hasUrlPattern()) {
        url_pattern = RouterUrlPatternConditionToBlink(isolate, v8_condition->urlPattern(), url_pattern_base_url, exception_state);
    ...
    if (v8_condition->hasRequestMethod() || v8_condition->hasRequestMode() || v8_condition->hasRequestDestination()) {
    ...
    if (v8_condition->hasRunningStatus()) {
        running_status = RouterRunningStatusConditionToBlink(v8_condition, exception_state);
    ...
    if (v8_condition->hasOrConditions()) {
        ...
        or_condition = RouterOrConditionToBlink(isolate, v8_condition, url_pattern_base_url, exception_state);
    ...
    if (v8_condition->hasNotCondition()) {
        ...
        not_condition = RouterNotConditionToBlink(isolate, v8_condition, url_pattern_base_url, exception_state);
        if (!not_condition.has_value()) {
            CHECK(exception_state.HadException());
            return std::nullopt;
        }
    }
    blink::ServiceWorkerRouterCondition ret(url_pattern, request, running_status, or_condition, not_condition);
    if (ret.IsEmpty()) {
        // At least one condition should exist per rule.
        exception_state.ThrowTypeError("At least one condition must be set, but no condition has been set "
                                       "to the rule.");
        return std::nullopt;
    }
    if (!ret.IsOrConditionExclusive()) {
        // `or` condition must be exclusive.
        exception_state.ThrowTypeError("Cannot set other conditions when the `or` condition is specified");
        return std::nullopt;
    }
    if (!ret.IsNotConditionExclusive()) {
        // `not` condition must be exclusive.
        exception_state.ThrowTypeError("Cannot set other conditions when the `not` condition is specified");
        return std::nullopt;
    }
    return ret;
}
...
std::optional<ServiceWorkerRouterSource> RouterSourceEnumToBlink(
    V8RouterSourceEnum v8_source_enum, mojom::blink::ServiceWorkerFetchHandlerType fetch_handler_type, ExceptionState& exception_state)
{
    ...
    case V8RouterSourceEnum::Enum::kFetchEvent: {
        if (fetch_handler_type == mojom::blink::ServiceWorkerFetchHandlerType::kNoHandler) {
            exception_state.ThrowTypeError("fetch-event source is specified without a fetch handler");
            return std::nullopt;
        }
        ServiceWorkerRouterSource source;
...
std::optional<ServiceWorkerRouterSource> RouterSourceToBlink(const RouterSource* v8_source, ExceptionState& exception_state)
{
    if (!v8_source) {
    ...
    exception_state.ThrowTypeError("Got a dictionary for source but no field is set");
    return std::nullopt;
}

std::optional<ServiceWorkerRouterSource> RouterSourceInputToBlink(
    const V8RouterSourceInput* router_source_input, mojom::blink::ServiceWorkerFetchHandlerType fetch_handler_type, ExceptionState& exception_state)
{
    switch (router_source_input->GetContentType()) {
    case blink::V8RouterSourceInput::ContentType::kRouterSourceEnum:
        return RouterSourceEnumToBlink(router_source_input->GetAsRouterSourceEnum(), fetch_handler_type, exception_state);
    case blink::V8RouterSourceInput::ContentType::kRouterSource:
        return RouterSourceToBlink(router_source_input->GetAsRouterSource(), exception_state);
    }
}

} // namespace

std::optional<ServiceWorkerRouterRule> ConvertV8RouterRuleToBlink(v8::Isolate* isolate, const RouterRule* input, const KURL& url_pattern_base_url,
    mojom::blink::ServiceWorkerFetchHandlerType fetch_handler_type, ExceptionState& exception_state)
{
    if (!input) {
    ...
    std::optional<ServiceWorkerRouterCondition> condition = RouterConditionToBlink(isolate, input->condition(), url_pattern_base_url, exception_state);
    ...
    const std::optional<ServiceWorkerRouterSource> source = RouterSourceInputToBlink(input->source(), fetch_handler_type, exception_state);
    if (!source.has_value()) {
        CHECK(exception_state.HadException());
        return std::nullopt;
    }
