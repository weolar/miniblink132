...
ServiceWorkerModuleTreeClient::ServiceWorkerModuleTreeClient(ScriptState* script_state)
...
// script resource.
void ServiceWorkerModuleTreeClient::NotifyModuleTreeLoadFinished(ModuleScript* module_script)
{
    auto* worker_global_scope = To<WorkerGlobalScope>(ExecutionContext::From(script_state_));
    blink::WorkerReportingProxy& worker_reporting_proxy = worker_global_scope->ReportingProxy();
    ...
    if (!module_script->HasEmptyRecord()) {
        ...
        if (record->GetStatus() >= v8::Module::kInstantiated && record->IsGraphAsync()) {
            worker_reporting_proxy.DidFailToFetchModuleScript();
            worker_global_scope->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript,
                mojom::blink::ConsoleMessageLevel::kError, "Top-level await is disallowed in service workers."));
            worker_global_scope->close();
            return;
        }
    }
    ...
    // completion value."
    worker_global_scope->WorkerScriptFetchFinished(*module_script, std::nullopt /* v8_inspector::V8StackTraceId */);
}
