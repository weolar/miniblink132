...
public:
    void React(ScriptState*, ScriptValue result)
    {
        result_ = result;
    }
...
void ExpectRejected(ScriptState* script_state, ScriptPromise<ServiceWorkerRegistration>& promise, const ScriptValueTest& value_test)
{
    StubScriptFunction* rejected = MakeGarbageCollected<StubScriptFunction>();
    promise.Catch(script_state, rejected);
    script_state->GetContext()->GetMicrotaskQueue()->PerformCheckpoint(script_state->GetIsolate());
...
public:
    ...
    {
        DOMException* exception = V8DOMException::ToWrappable(script_state->GetIsolate(), value.V8Value());
...
public:
    ...
    void operator()(ScriptState* script_state, ScriptValue value) const override
    {
        ...
        EXPECT_EQ("TypeError", ToCoreString(isolate, name->ToString(context).ToLocalChecked()));
        EXPECT_EQ(expected_message_, ToCoreString(isolate, message->ToString(context).ToLocalChecked()));
    }
...
public:
    ...
    void RegisterServiceWorker(const WebURL& scope, const WebURL& script_url, blink::mojom::blink::ScriptType script_type,
        mojom::ServiceWorkerUpdateViaCache update_via_cache, const WebFetchClientSettingsObject& fetch_client_settings_object,
        std::unique_ptr<WebServiceWorkerRegistrationCallbacks> callbacks) override
    {
        ADD_FAILURE() << "the provider should not be called to register a Service Worker";
    }

    bool ValidateScopeAndScriptURL(const WebURL& scope, const WebURL& script_url, WebString* error_message) override
    {
        return true;
    }
};
...
protected:
    ...
    void TestRegisterRejected(const String& script_url, const String& scope, const ScriptValueTest& value_test)
    {
        // When the registration is rejected, a register call must not reach
        // the provider.
        ServiceWorkerContainer* container
            = ServiceWorkerContainer::CreateForTesting(*GetFrame().DomWindow(), std::make_unique<NotReachedWebServiceWorkerProvider>());
        ScriptState::Scope script_scope(GetScriptState());
        RegistrationOptions* options = RegistrationOptions::Create();
        options->setScope(scope);
        ScriptPromise<ServiceWorkerRegistration> promise = container->registerServiceWorker(GetScriptState(), script_url, options);
        ExpectRejected(GetScriptState(), promise, value_test);
    }

    void TestGetRegistrationRejected(const String& document_url, const ScriptValueTest& value_test)
    {
        ServiceWorkerContainer* container
            = ServiceWorkerContainer::CreateForTesting(*GetFrame().DomWindow(), std::make_unique<NotReachedWebServiceWorkerProvider>());
        ScriptState::Scope script_scope(GetScriptState());
        ScriptPromise<ServiceWorkerRegistration> promise = container->getRegistration(GetScriptState(), document_url);
        ExpectRejected(GetScriptState(), promise, value_test);
    }
};
...
TEST_F(ServiceWorkerContainerTest, Register_CrossOriginScriptIsRejected)
{
    ...
    TestRegisterRejected("https://www.example.com:8080/", // Differs by port
...
{
    SetPageURL("https://www.example.com");
    TestRegisterRejected("https://www.example.com",
        "wss://www.example.com/", // Only support http and https
        ExpectTypeError("Failed to register a ServiceWorker: The URL protocol "
...
{
    SetPageURL("https://www.example.com/");
    TestGetRegistrationRejected("https://foo.example.com/", // Differs by host
        ExpectDOMException("SecurityError",
...
public:
    ...
    size_t RegisterCallCount()
    {
        return register_call_count_;
    }
    const WebURL& RegisterScope()
    {
        return register_scope_;
    }
    const WebURL& RegisterScriptURL()
    {
        return register_script_url_;
    }
    size_t GetRegistrationCallCount()
    {
        return get_registration_call_count_;
    }
    const WebURL& GetRegistrationURL()
    {
        return get_registration_url_;
    }
    mojom::blink::ScriptType ScriptType() const
    {
        return script_type_;
    }
...
private:
    ...
    public:
        ...
        void GetRegistration(const WebURL& document_url, std::unique_ptr<WebServiceWorkerGetRegistrationCallbacks> callbacks) override
        {
    ...
    private:
        ...
        Vector<std::unique_ptr<WebServiceWorkerRegistrationCallbacks>> registration_callbacks_to_delete_;
        Vector<std::unique_ptr<WebServiceWorkerGetRegistrationCallbacks>> get_registration_callbacks_to_delete_;
    };
...
TEST_F(ServiceWorkerContainerTest, RegisterUnregister_NonHttpsSecureOriginDelegatesToProvider)
{
    ...
    ServiceWorkerContainer* container = ServiceWorkerContainer::CreateForTesting(*GetFrame().DomWindow(), stub_provider.Provider());
    ...
    // register
    {
        ...
        EXPECT_EQ(WebURL(KURL("http://localhost/x/y/")), stub_provider.RegisterScope());
        EXPECT_EQ(WebURL(KURL("http://localhost/x/y/worker.js")), stub_provider.RegisterScriptURL());
        EXPECT_EQ(mojom::blink::ScriptType::kClassic, stub_provider.ScriptType());
        EXPECT_EQ(mojom::ServiceWorkerUpdateViaCache::kImports, stub_provider.UpdateViaCache());
    }
}
...
TEST_F(ServiceWorkerContainerTest, GetRegistration_OmittedDocumentURLDefaultsToPageURL)
{
...

    {
        ...
        EXPECT_EQ(WebURL(KURL("http://localhost/x/index.html")), stub_provider.GetRegistrationURL());
        EXPECT_EQ(mojom::blink::ScriptType::kClassic, stub_provider.ScriptType());
        EXPECT_EQ(mojom::ServiceWorkerUpdateViaCache::kImports, stub_provider.UpdateViaCache());
    }
}

TEST_F(ServiceWorkerContainerTest, RegisterUnregister_UpdateViaCacheOptionDelegatesToProvider)
{
    SetPageURL("http://localhost/x/index.html");
    ...
    // register
    {
        ...
        EXPECT_EQ(1ul, stub_provider.RegisterCallCount());
        EXPECT_EQ(WebURL(KURL(KURL(), "http://localhost/x/y/")), stub_provider.RegisterScope());
        EXPECT_EQ(WebURL(KURL(KURL(), "http://localhost/x/y/worker.js")), stub_provider.RegisterScriptURL());
        EXPECT_EQ(mojom::blink::ScriptType::kClassic, stub_provider.ScriptType());
        EXPECT_EQ(mojom::ServiceWorkerUpdateViaCache::kNone, stub_provider.UpdateViaCache());
    }
}
...
{
    return { 1, mojom::blink::ServiceWorkerState::kActivated, WebURL(KURL(KURL(), "http://localhost/x/y/worker.js")), {}, {} };
}
...
TEST_F(ServiceWorkerContainerTest, ReceiveMessage)
{
    ...
    container->ReceiveMessage(MakeServiceWorkerObjectInfo(), MakeTransferableMessage());
    run_loop.Run();
...
TEST_F(ServiceWorkerContainerTest, ReceiveMessageWhichCannotDeserialize)
{
    ...
    ServiceWorkerContainer* container = ServiceWorkerContainer::CreateForTesting(*window, stub_provider.Provider());
