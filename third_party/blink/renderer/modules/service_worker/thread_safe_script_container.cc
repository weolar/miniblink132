...
ThreadSafeScriptContainer::RawScriptData::RawScriptData(const String& encoding, Vector<uint8_t> script_text, Vector<uint8_t> meta_data)
    : encoding_(encoding)
...
void ThreadSafeScriptContainer::RawScriptData::AddHeader(const String& key, const String& value)
{
...
void ThreadSafeScriptContainer::AddOnIOThread(const KURL& url, std::unique_ptr<RawScriptData> data)
{
...
ThreadSafeScriptContainer::ScriptStatus ThreadSafeScriptContainer::GetStatusOnWorkerThread(const KURL& url)
{
    base::AutoLock locker(lock_);
...
{
    base::AutoLock locker(lock_);
    DCHECK(!waiting_url_.IsValid()) << "The script container is unexpectedly shared among worker threads.";
    waiting_url_ = url;
...
std::unique_ptr<ThreadSafeScriptContainer::RawScriptData> ThreadSafeScriptContainer::TakeOnWorkerThread(const KURL& url)
{
    base::AutoLock locker(lock_);
    auto it = script_data_.find(url);
    CHECK(it != script_data_.end(), base::NotFatalUntil::M130) << "Script should have been received before calling Take";
