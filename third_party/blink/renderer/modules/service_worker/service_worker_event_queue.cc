...
BASE_FEATURE(kServiceWorkerEventQueueWaitForScriptEvaluation, "ServiceWorkerEventQueueWaitForScriptEvaluation", base::FEATURE_ENABLED_BY_DEFAULT);
...
ServiceWorkerEventQueue::StayAwakeToken::StayAwakeToken(base::WeakPtr<ServiceWorkerEventQueue> event_queue)
...
ServiceWorkerEventQueue::ServiceWorkerEventQueue(
    BeforeStartEventCallback before_start_event_callback, base::RepeatingClosure idle_callback, scoped_refptr<base::SequencedTaskRunner> task_runner)
    : ServiceWorkerEventQueue(std::move(before_start_event_callback), std::move(idle_callback), std::move(task_runner), base::DefaultTickClock::GetInstance())
{
}

ServiceWorkerEventQueue::ServiceWorkerEventQueue(BeforeStartEventCallback before_start_event_callback, base::RepeatingClosure idle_callback,
...
{
    if (!base::FeatureList::IsEnabled(kServiceWorkerEventQueueWaitForScriptEvaluation)) {
...
ServiceWorkerEventQueue::~ServiceWorkerEventQueue()
{
    ...
    for (auto& event : all_events_) {
        std::move(event.value->abort_callback).Run(blink::mojom::ServiceWorkerEventStatus::ABORTED);
    }
}
...
void ServiceWorkerEventQueue::Start()
{
    ...
    timer_.Start(FROM_HERE, kUpdateInterval, WTF::BindRepeating(&ServiceWorkerEventQueue::UpdateStatus, WTF::Unretained(this)));
    if (base::FeatureList::IsEnabled(kServiceWorkerEventQueueWaitForScriptEvaluation)) {
        is_ready_for_processing_events_ = true;
        ResetIdleTimeout();
        ProcessEvents();
...
void ServiceWorkerEventQueue::EnqueueNormal(
    int event_id, StartCallback start_callback, AbortCallback abort_callback, std::optional<base::TimeDelta> custom_timeout)
{
    EnqueueEvent(std::make_unique<Event>(event_id, Event::Type::Normal, std::move(start_callback), std::move(abort_callback), std::move(custom_timeout)));
}

void ServiceWorkerEventQueue::EnqueuePending(
    int event_id, StartCallback start_callback, AbortCallback abort_callback, std::optional<base::TimeDelta> custom_timeout)
{
    EnqueueEvent(std::make_unique<Event>(event_id, Event::Type::Pending, std::move(start_callback), std::move(abort_callback), std::move(custom_timeout)));
}
    ...
    int event_id, StartCallback start_callback, AbortCallback abort_callback, std::optional<base::TimeDelta> custom_timeout)
{
    EnqueueEvent(std::make_unique<ServiceWorkerEventQueue::Event>(
        event_id, ServiceWorkerEventQueue::Event::Type::Offline, std::move(start_callback), std::move(abort_callback), std::move(custom_timeout)));
}
...
std::map<int, std::unique_ptr<ServiceWorkerEventQueue::Event>>& ServiceWorkerEventQueue::GetActiveEventQueue()
{
    if (running_event_type_ == RunningEventType::kNone) {
        ...
        // online events.
        return queued_online_events_.empty() ? queued_offline_events_ : queued_online_events_;
    }
...
void ServiceWorkerEventQueue::EnqueueEvent(std::unique_ptr<Event> event)
{
    ...
    bool can_start_processing_events = is_ready_for_processing_events_ && !processing_events_ && event->type != Event::Type::Pending;

    // Start counting the timer when an event is enqueued.
    all_events_.insert(event->event_id,
        std::make_unique<EventInfo>(
            tick_clock_->NowTicks() + event->custom_timeout.value_or(kEventTimeout), WTF::BindOnce(std::move(event->abort_callback), event->event_id)));

    auto& queue = event->type == Event::Type::Offline ? queued_offline_events_ : queued_online_events_;
...
void ServiceWorkerEventQueue::StartEvent(int event_id, std::unique_ptr<Event> event)
{
    DCHECK(HasEvent(event_id));
    running_event_type_ = event->type == Event::Type::Offline ? RunningEventType::kOffline : RunningEventType::kOnline;
...
{
    return (base::Contains(queued_online_events_, event_id) || base::Contains(queued_offline_events_, event_id));
}

std::unique_ptr<ServiceWorkerEventQueue::StayAwakeToken> ServiceWorkerEventQueue::CreateStayAwakeToken()
{
    return std::make_unique<ServiceWorkerEventQueue::StayAwakeToken>(weak_factory_.GetWeakPtr());
}
...
void ServiceWorkerEventQueue::SetIdleDelay(base::TimeDelta idle_delay)
{
    ...
    base::TimeDelta delta_until_idle = new_idle_callback_time - tick_clock_->NowTicks();
...
void ServiceWorkerEventQueue::UpdateStatus()
{
    ...
    for (auto& it : all_events_) {
        ...
        std::move(event_info->abort_callback).Run(blink::mojom::ServiceWorkerEventStatus::TIMEOUT);
...
void ServiceWorkerEventQueue::ScheduleIdleCallback(base::TimeDelta delay)
{
    ...
    idle_callback_handle_
        = PostDelayedCancellableTask(*task_runner_, FROM_HERE, WTF::BindOnce(&ServiceWorkerEventQueue::TriggerIdleCallback, WTF::Unretained(this)), delay);
}
...
{
    size_t num_queued_events = queued_online_events_.size() + queued_offline_events_.size();
    DCHECK_LE(num_queued_events, all_events_.size());
    return all_events_.size() - num_queued_events > 0 || num_of_stay_awake_tokens_ > 0;
}
...
ServiceWorkerEventQueue::Event::Event(int event_id, ServiceWorkerEventQueue::Event::Type type, StartCallback start_callback, AbortCallback abort_callback,
...
ServiceWorkerEventQueue::EventInfo::EventInfo(base::TimeTicks expiration_time, base::OnceCallback<void(blink::mojom::ServiceWorkerEventStatus)> abort_callback)
