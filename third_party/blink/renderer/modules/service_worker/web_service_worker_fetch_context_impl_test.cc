...
public:
    ...
    class FakeURLLoaderThrottleProvider : public URLLoaderThrottleProvider {
        ...
        WebVector<std::unique_ptr<URLLoaderThrottle>> CreateThrottles(
            base::optional_ref<const blink::LocalFrameToken> local_frame_token, const network::ResourceRequest& request) override
        {
...
TEST_F(WebServiceWorkerFetchContextImplTest, SkipThrottling)
{
    ...
    auto context = WebServiceWorkerFetchContext::Create(RendererPreferences(), kScriptUrl,
        /*pending_url_loader_factory=*/nullptr,
        /*pending_script_loader_factory=*/nullptr, kScriptUrlToSkipThrottling, std::make_unique<FakeURLLoaderThrottleProvider>(),
        /*websocket_handshake_throttle_provider=*/nullptr, mojo::NullReceiver(), mojo::NullReceiver(),
...

    {
        ...
        WebVector<std::unique_ptr<URLLoaderThrottle>> throttles = context->CreateThrottles(request);
