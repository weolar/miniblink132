...
class MODULES_EXPORT ServiceWorkerInstalledScriptsManager final : public InstalledScriptsManager {
public:
    ServiceWorkerInstalledScriptsManager(
        std::unique_ptr<WebServiceWorkerInstalledScriptsManagerParams>, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner);
...
private:
    ...
    std::unique_ptr<ThreadSafeScriptContainer::RawScriptData> GetRawScriptData(const KURL& script_url);
    ...
    mojo::SharedRemote<mojom::blink::ServiceWorkerInstalledScriptsManagerHost> manager_host_;
};
