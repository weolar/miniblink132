...
public:
    ThreadSafeScriptContainerTest()
        : writer_thread_(NonMainThread::CreateThread(ThreadCreationParams(ThreadType::kTestThread).SetThreadNameForTest("writer_thread")))
        , reader_thread_(NonMainThread::CreateThread(ThreadCreationParams(ThreadType::kTestThread).SetThreadNameForTest("reader_thread")))
        , writer_waiter_(std::make_unique<base::WaitableEvent>(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED))
        , reader_waiter_(std::make_unique<base::WaitableEvent>(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED))
        , container_(base::MakeRefCounted<ThreadSafeScriptContainer>())
    {
    }

protected:
    base::WaitableEvent* AddOnWriterThread(ThreadSafeScriptContainer::RawScriptData** out_data)
    {
        PostCrossThreadTask(*writer_thread_->GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(
                [](scoped_refptr<ThreadSafeScriptContainer> container, ThreadSafeScriptContainer::RawScriptData** out_data, base::WaitableEvent* waiter) {
                    auto data = std::make_unique<ThreadSafeScriptContainer::RawScriptData>(
                        String::FromUTF8("utf-8") /* encoding */, Vector<uint8_t>() /* script_text */, Vector<uint8_t>() /* meta_data */);
                    *out_data = data.get();
                    container->AddOnIOThread(KURL(kKeyUrl), std::move(data));
                    waiter->Signal();
                },
                container_, CrossThreadUnretained(out_data), CrossThreadUnretained(writer_waiter_.get())));
    ...
    base::WaitableEvent* OnAllDataAddedOnWriterThread()
    {
        PostCrossThreadTask(*writer_thread_->GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(
                [](scoped_refptr<ThreadSafeScriptContainer> container, base::WaitableEvent* waiter) {
    ...
    {
        PostCrossThreadTask(*reader_thread_->GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(
                [](scoped_refptr<ThreadSafeScriptContainer> container, ScriptStatus* out_status, base::WaitableEvent* waiter) {
                    *out_status = container->GetStatusOnWorkerThread(KURL(kKeyUrl));
                    waiter->Signal();
                },
                container_, CrossThreadUnretained(out_status), CrossThreadUnretained(reader_waiter_.get())));
    ...
    base::WaitableEvent* WaitOnReaderThread(bool* out_exists)
    {
        PostCrossThreadTask(*reader_thread_->GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(
                [](scoped_refptr<ThreadSafeScriptContainer> container, bool* out_exists, base::WaitableEvent* waiter) {
                    *out_exists = container->WaitOnWorkerThread(KURL(kKeyUrl));
                    waiter->Signal();
                },
                container_, CrossThreadUnretained(out_exists), CrossThreadUnretained(reader_waiter_.get())));
        return reader_waiter_.get();
    }

    base::WaitableEvent* TakeOnReaderThread(ThreadSafeScriptContainer::RawScriptData** out_data)
    {
        PostCrossThreadTask(*reader_thread_->GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(
                [](scoped_refptr<ThreadSafeScriptContainer> container, ThreadSafeScriptContainer::RawScriptData** out_data, base::WaitableEvent* waiter) {
                ...
                container_, CrossThreadUnretained(out_data), CrossThreadUnretained(reader_waiter_.get())));
        return reader_waiter_.get();
    }
...
TEST_F(ThreadSafeScriptContainerTest, WaitNonExistingKey)
{
...

    {
        ...
        EXPECT_FALSE(pending_wait->IsSignaled());
        base::WaitableEvent* pending_on_all_data_added = OnAllDataAddedOnWriterThread();
        pending_wait->Wait();
