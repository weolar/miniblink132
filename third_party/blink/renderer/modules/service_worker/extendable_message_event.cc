...
ExtendableMessageEvent* ExtendableMessageEvent::Create(
    scoped_refptr<SerializedScriptValue> data, const String& origin, MessagePortArray* ports, ServiceWorkerClient* source, WaitUntilObserver* observer)
{
    ExtendableMessageEvent* event = MakeGarbageCollected<ExtendableMessageEvent>(std::move(data), origin, ports, observer);
    event->source_as_client_ = source;
    return event;
}

ExtendableMessageEvent* ExtendableMessageEvent::Create(
    scoped_refptr<SerializedScriptValue> data, const String& origin, MessagePortArray* ports, ServiceWorker* source, WaitUntilObserver* observer)
{
    ExtendableMessageEvent* event = MakeGarbageCollected<ExtendableMessageEvent>(std::move(data), origin, ports, observer);
...
ExtendableMessageEvent* ExtendableMessageEvent::CreateError(
    const String& origin, MessagePortArray* ports, ServiceWorkerClient* source, WaitUntilObserver* observer)
{
    ExtendableMessageEvent* event = MakeGarbageCollected<ExtendableMessageEvent>(origin, ports, observer);
    event->source_as_client_ = source;
    return event;
}

ExtendableMessageEvent* ExtendableMessageEvent::CreateError(const String& origin, MessagePortArray* ports, ServiceWorker* source, WaitUntilObserver* observer)
{
    ExtendableMessageEvent* event = MakeGarbageCollected<ExtendableMessageEvent>(origin, ports, observer);
    event->source_as_service_worker_ = source;
    return event;
...
V8UnionClientOrMessagePortOrServiceWorker* ExtendableMessageEvent::source() const
{
    if (source_as_client_) {
        return MakeGarbageCollected<V8UnionClientOrMessagePortOrServiceWorker>(source_as_client_);
    } else if (source_as_service_worker_) {
        return MakeGarbageCollected<V8UnionClientOrMessagePortOrServiceWorker>(source_as_service_worker_);
    } else if (source_as_message_port_) {
        return MakeGarbageCollected<V8UnionClientOrMessagePortOrServiceWorker>(source_as_message_port_);
    }
    return nullptr;
}
    ...
    : ExtendableEvent(type, initializer, observer)
{
    ...
    if (initializer->hasSource() and initializer->source()) {
        ...
        case V8UnionClientOrMessagePortOrServiceWorker::ContentType::kMessagePort:
            ...
            break;
        case V8UnionClientOrMessagePortOrServiceWorker::ContentType::kServiceWorker:
...
ExtendableMessageEvent::ExtendableMessageEvent(
    scoped_refptr<SerializedScriptValue> data, const String& origin, MessagePortArray* ports, WaitUntilObserver* observer)
    : ExtendableEvent(event_type_names::kMessage, ExtendableMessageEventInit::Create(), observer)
...
ExtendableMessageEvent::ExtendableMessageEvent(const String& origin, MessagePortArray* ports, WaitUntilObserver* observer)
    : ExtendableEvent(event_type_names::kMessageerror, ExtendableMessageEventInit::Create(), observer)
    , origin_(origin)
    , last_event_id_(String())
    , ports_(ports)
{
