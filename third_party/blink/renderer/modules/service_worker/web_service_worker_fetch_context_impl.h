...
class BLINK_EXPORT WebServiceWorkerFetchContextImpl final : public WebServiceWorkerFetchContext,
...
public:
    ...
    // again.
    WebServiceWorkerFetchContextImpl(const RendererPreferences& renderer_preferences, const KURL& worker_script_url,
...
private:
    ...
    std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_url_loader_factory_;
    // Consumed on the worker thread to create |web_script_loader_factory_|.
    std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_script_loader_factory_;
    ...
    std::unique_ptr<InternetDisconnectedURLLoaderFactory> internet_disconnected_url_loader_factory_;
