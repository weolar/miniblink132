...
class MODULES_EXPORT RespondWithObserver : public GarbageCollected<RespondWithObserver>, public ExecutionContextClient {
public:
    virtual ~RespondWithObserver() = default;

    template <typename IDLType, typename Derived>
    void RespondWith(
        ScriptState* script_state, const ScriptPromise<IDLType>& script_promise, ThenCallable<IDLType, Derived>* on_fulfill, ExceptionState& exception_state)
    {
        if (!StartRespondWith(exception_state)) {
            return;
        }
        has_started_ = true;
        auto next_promise = script_promise.Then(script_state, on_fulfill, MakeGarbageCollected<RespondWithReject>(this));
        // 3. `Add r to the extend lifetime promises.`
        // 4. `Increment the pending promises count by one.`
        // This is accomplised by WaitUntil().
        bool will_wait = observer_->WaitUntil(script_state, next_promise, exception_state);
...
protected:
    ...
    class RespondWithReject final : public ThenCallable<IDLAny, RespondWithReject, IDLAny> {
    public:
    ...
    bool WaitUntil(ScriptState*, const ScriptPromise<IDLUndefined>&, ExceptionState&);
