...
public:
    ...
    using AbortCallback = base::OnceCallback<void(int /* event_id */, mojom::blink::ServiceWorkerEventStatus)>;

    using BeforeStartEventCallback = base::RepeatingCallback<void(/*is_offline_event=*/bool)>;
    ...
    // For testing.
    ServiceWorkerEventQueue(BeforeStartEventCallback before_start_event_callback, base::RepeatingClosure idle_callback,
    ...
    void EnqueueNormal(int event_id, StartCallback start_callback, AbortCallback abort_callback, std::optional<base::TimeDelta> custom_timeout);

    // Similar to EnqueueNormal(), but enqueues a Pending event.
    void EnqueuePending(int event_id, StartCallback start_callback, AbortCallback abort_callback, std::optional<base::TimeDelta> custom_timeout);

    // Similar to EnqueueNormal(), but enqueues an Offline event.
    void EnqueueOffline(int event_id, StartCallback start_callback, AbortCallback abort_callback, std::optional<base::TimeDelta> custom_timeout);
    ...
    bool did_idle_timeout() const
    {
        return did_idle_timeout_;
    }
...
private:
    ...
    struct Event {
        ...
        Event(int event_id, Type type, StartCallback start_callback, AbortCallback abort_callback, std::optional<base::TimeDelta> custom_timeout);
        ~Event();
        const int event_id;
        Type type;
    ...
    struct EventInfo {
        EventInfo(base::TimeTicks expiration_time, base::OnceCallback<void(mojom::blink::ServiceWorkerEventStatus)> abort_callback);
        ~EventInfo();
        const base::TimeTicks expiration_time;
        base::OnceCallback<void(mojom::blink::ServiceWorkerEventStatus)> abort_callback;
    };
    ...
    base::TimeDelta idle_delay_ = base::Seconds(mojom::blink::kServiceWorkerDefaultIdleDelayInSeconds);
    ...
    std::map<int /* event_id */, std::unique_ptr<Event>> queued_online_events_ ALLOW_DISCOURAGED_TYPE("Needs to be ordered by event_id");
    ...
    std::map<int /* event_id */, std::unique_ptr<Event>> queued_offline_events_ ALLOW_DISCOURAGED_TYPE("Needs to be ordered by event_id");
