...
public:
    ...
    void LoadSynchronously(std::unique_ptr<network::ResourceRequest> request, scoped_refptr<const SecurityOrigin> top_frame_origin, bool download_to_blob,
        bool no_mime_sniffing, base::TimeDelta timeout_interval, URLLoaderClient*, WebURLResponse&, std::optional<WebURLError>&, scoped_refptr<SharedBuffer>&,
        int64_t& encoded_data_length, uint64_t& encoded_body_length, scoped_refptr<BlobDataHandle>& downloaded_blob,
...
public:
    std::unique_ptr<URLLoader> CreateURLLoader(const network::ResourceRequest&, scoped_refptr<base::SingleThreadTaskRunner>,
        scoped_refptr<base::SingleThreadTaskRunner>, mojo::PendingRemote<mojom::blink::KeepAliveHandle>, BackForwardCacheLoaderHelper*,
...
// fetch tests.
class FakeWebServiceWorkerFetchContext final : public WebServiceWorkerFetchContext {
public:
    ...
    std::unique_ptr<URLLoaderFactory> WrapURLLoaderFactory(CrossVariantMojoRemote<network::mojom::URLLoaderFactoryInterfaceBase> url_loader_factory) override
    {
        return nullptr;
    }
    ...
    WebVector<std::unique_ptr<URLLoaderThrottle>> CreateThrottles(const network::ResourceRequest& request) override
    {
        return {};
    }

    mojom::ControllerServiceWorkerMode GetControllerServiceWorkerMode() const override
    {
...
class FakeBrowserInterfaceBroker final : public mojom::blink::BrowserInterfaceBroker {
public:
    ...
    mojo::PendingRemote<mojom::blink::BrowserInterfaceBroker> BindNewPipeAndPassRemote()
    {
...
class OnFallbackReceiver : public mojom::blink::ServiceWorkerFetchResponseCallback {
public:
    mojo::PendingRemote<mojom::blink::ServiceWorkerFetchResponseCallback> BindNewPipeAndPassRemote()
    {
...
private:
    ...
    void OnResponse(mojom::blink::FetchAPIResponsePtr response, mojom::blink::ServiceWorkerFetchEventTimingPtr timing) override
    {
        NOTREACHED();
    }
    void OnResponseStream(mojom::blink::FetchAPIResponsePtr response, mojom::blink::ServiceWorkerStreamHandlePtr body_as_stream,
        mojom::blink::ServiceWorkerFetchEventTimingPtr timing) override
    {
        NOTREACHED();
    }
    void OnFallback(std::optional<network::DataElementChunkedDataPipe> request_body, mojom::blink::ServiceWorkerFetchEventTimingPtr timing) override
    {
    ...
    mojo::Receiver<mojom::blink::ServiceWorkerFetchResponseCallback> response_callback_receiver_ { this };
    base::RunLoop run_loop_;
    std::optional<std::optional<network::DataElementChunkedDataPipe>> fallback_request_body_;
};

class MojoHandleWatcher {
public:
    explicit MojoHandleWatcher(mojo::Handle handle)
        : handle_watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, base::SequencedTaskRunner::GetCurrentDefault())
    {
        handle_watcher_.Watch(handle, MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_WRITABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED,
            base::BindRepeating(&MojoHandleWatcher::OnReady, base::Unretained(this)));
    }
...
public:
    ...
    mojo::PendingRemote<network::mojom::blink::ChunkedDataPipeGetter> BindNewPipeAndPassRemote()
    {
        auto pending_remote = receiver_.BindNewPipeAndPassRemote();
        receiver_.set_disconnect_with_reason_handler(base::BindLambdaForTesting([&](uint32_t reason, const std::string& description) {
            LOG(INFO) << "TestDataUploader Mojo closed reason" << reason << ", desc=" << description;
        }));
...
private:
    ...
    void StartReading(mojo::ScopedDataPipeProducerHandle producer) override
    {
        ...
        handle_watcher_->WaitAsync(base::BindOnce(&TestDataUploader::OnMojoReady, base::Unretained(this)));
    }
    ...
    void OnMojoReady()
    {
        ...
        CHECK_EQ(MOJO_RESULT_OK,
            producer_->WriteData(base::as_byte_span(upload_contents_).subspan(0u, upload_contents_.size()), MOJO_WRITE_DATA_FLAG_NONE, bytes_written));
...
public:
    explicit TestDataPipeReader(mojo::PendingRemote<network::mojom::ChunkedDataPipeGetter> chunked_data_pipe_getter, uint32_t capacity_read_pipe_size)
        : chunked_data_pipe_getter_(std::move(chunked_data_pipe_getter))
    {
        CHECK_EQ(MOJO_RESULT_OK, mojo::CreateDataPipe(capacity_read_pipe_size, producer_, consumer_));
        handle_watcher_ = std::make_unique<MojoHandleWatcher>(consumer_.get());

        chunked_data_pipe_getter_.set_disconnect_with_reason_handler(base::BindLambdaForTesting([&](uint32_t reason, const std::string& description) {
            LOG(INFO) << "TestDataPipeReader Mojo closed reason" << reason << ", desc=" << description;
        }));

        chunked_data_pipe_getter_->GetSize(base::BindLambdaForTesting([](int32_t status, uint64_t size) {}));
    ...
    std::string Read()
    {
        ...
        CHECK_EQ(MOJO_RESULT_OK, consumer_->ReadData(MOJO_READ_DATA_FLAG_NONE, base::as_writable_byte_span(buffer), actually_read_bytes));
        return buffer.substr(0, actually_read_bytes);
    }

    bool IsConnected() const
    {
        return chunked_data_pipe_getter_.is_connected();
    }
...
class MockServiceWorkerContextClient final : public WebServiceWorkerContextClient {
public:
    ...
    MOCK_METHOD2(WorkerReadyForInspectionOnInitiatorThread,
        void(CrossVariantMojoRemote<mojom::DevToolsAgentInterfaceBase> devtools_agent_remote, CrossVariantMojoReceiver<mojom::DevToolsAgentHostInterfaceBase>));
    ...
    void WorkerContextStarted(WebServiceWorkerContextProxy* proxy, scoped_refptr<base::SequencedTaskRunner> worker_task_runner) override
    {
        ...
        mojo::PendingAssociatedRemote<mojom::blink::ServiceWorkerRegistrationObjectHost> registration_object_host;
        auto registration_object_host_receiver = registration_object_host.InitWithNewEndpointAndPassReceiver();
        mojo::PendingAssociatedRemote<mojom::blink::ServiceWorkerRegistrationObject> registration_object;

        mojo::PendingAssociatedRemote<mojom::blink::ServiceWorkerObjectHost> service_worker_object_host;
        auto service_worker_object_host_receiver = service_worker_object_host.InitWithNewEndpointAndPassReceiver();
        mojo::PendingAssociatedRemote<mojom::blink::ServiceWorkerObject> service_worker_object;

        mojo::PendingAssociatedRemote<mojom::blink::AssociatedInterfaceProvider> associated_interfaces_remote_from_browser;
        auto associated_interfaces_recevier_from_browser = associated_interfaces_remote_from_browser.InitWithNewEndpointAndPassReceiver();

        mojo::PendingAssociatedRemote<mojom::blink::AssociatedInterfaceProvider> associated_interfaces_remote_to_browser;
        auto associated_interfaces_recevier_to_browser = associated_interfaces_remote_to_browser.InitWithNewEndpointAndPassReceiver();
        ...
        service_worker->InitializeGlobalScope(std::move(host_remote), std::move(associated_interfaces_remote_from_browser),
            std::move(associated_interfaces_recevier_to_browser),
            mojom::blink::ServiceWorkerRegistrationObjectInfo::New(2 /* registration_id */, KURL("https://example.com"),
                mojom::blink::ServiceWorkerUpdateViaCache::kImports, std::move(registration_object_host),
                registration_object.InitWithNewEndpointAndPassReceiver(), nullptr, nullptr, nullptr),
            mojom::blink::ServiceWorkerObjectInfo::New(1 /* service_worker_version_id */, mojom::blink::ServiceWorkerState::kParsed,
                KURL("https://example.com"), std::move(service_worker_object_host), service_worker_object.InitWithNewEndpointAndPassReceiver()),
            mojom::blink::FetchHandlerExistence::EXISTS,
            /*reporting_observer_receiver=*/mojo::NullReceiver(),
            /*ancestor_frame_type=*/mojom::blink::AncestorFrameType::kNormalFrame, blink::BlinkStorageKey());

        MockPolicyContainerHost mock_policy_container_host;
        web_policy_container_->remote = mock_policy_container_host.BindNewEndpointAndPassDedicatedRemote();
        ...
        mojo::Remote<mojom::blink::ControllerServiceWorker> stub_controller_service_worker;
        proxy->BindControllerServiceWorker(stub_controller_service_worker.BindNewPipeAndPassReceiver());
        stub_controller_service_worker->Clone(
            controller_service_worker_.BindNewPipeAndPassReceiver(), network::CrossOriginEmbedderPolicy(), mojo::NullRemote());
    ...
    scoped_refptr<WebServiceWorkerFetchContext> CreateWorkerFetchContextOnInitiatorThread() override
    {
        return base::MakeRefCounted<FakeWebServiceWorkerFetchContext>();
    }

    void OnNavigationPreloadResponse(int fetch_event_id, std::unique_ptr<WebURLResponse> response, mojo::ScopedDataPipeConsumerHandle data_pipe) override
    {
    }

    void OnNavigationPreloadComplete(
        int fetch_event_id, base::TimeTicks completion_time, int64_t encoded_data_length, int64_t encoded_body_length, int64_t decoded_body_length) override
    {
    }

    void OnNavigationPreloadError(int fetch_event_id, std::unique_ptr<WebServiceWorkerError> error) override
    {
    }

    TestDataPipeReader DispatchFetchEventForSubresourceAndCreateReader(const std::string& upload_contents, uint32_t capacity_read_pipe_size)
    {
        OnFallbackReceiver on_fallback_receiver;
        worker_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&MockServiceWorkerContextClient::DispatchFetchEventForSubresourceonWorkerThread, base::Unretained(this),
                base::Unretained(&on_fallback_receiver), upload_contents));
        auto fallback_request_body = on_fallback_receiver.WaitFallbackRequestBody();
        return TestDataPipeReader(fallback_request_body->ReleaseChunkedDataPipeGetter(), capacity_read_pipe_size);
    }

    void DispatchFetchEventForSubresourceonWorkerThread(OnFallbackReceiver* on_fallback_receiver, const std::string& upload_contents)
    {
        ...
        controller_service_worker_->DispatchFetchEventForSubresource(std::move(params), on_fallback_receiver->BindNewPipeAndPassRemote(), base::DoNothing());
    }
    ...
    void WaitUntilScriptEvaluated()
    {
        script_evaluated_event_.Wait();
    }
    void WaitUntilThreadTermination()
    {
        termination_event_.Wait();
    }
...
private:
    ...
    mojo::Remote<mojom::blink::ControllerServiceWorker> controller_service_worker_;
...
protected:
    ...
    std::unique_ptr<WebEmbeddedWorkerStartData> CreateStartData()
    {
        ...
        WebFetchClientSettingsObject outside_settings_object(network::mojom::ReferrerPolicy::kDefault,
            /*outgoing_referrer=*/script_url, blink::mojom::InsecureRequestsPolicy::kDoNotUpgrade);
        auto start_data = std::make_unique<WebEmbeddedWorkerStartData>(std::move(outside_settings_object));
        ...
        start_data->wait_for_debugger_mode = WebEmbeddedWorkerStartData::kDontWaitForDebugger;
...
TEST_F(WebEmbeddedWorkerImplTest, TerminateSoonAfterStart)
{
    ...
    worker_->StartWorkerContext(CreateStartData(),
        ...
        /*cache_storage_remote=*/mojo::NullRemote(), browser_interface_broker.BindNewPipeAndPassRemote(), InterfaceRegistry::GetEmptyInterfaceRegistry(),
...
TEST_F(WebEmbeddedWorkerImplTest, TerminateWhileWaitingForDebugger)
{
    ...
    start_data->wait_for_debugger_mode = WebEmbeddedWorkerStartData::kWaitForDebugger;
    FakeBrowserInterfaceBroker browser_interface_broker;
...
TEST_F(WebEmbeddedWorkerImplTest, GCOnWorkerThreadShouldNotCauseUploadFail)
{
    ...
    TestDataPipeReader reader = mock_client_->DispatchFetchEventForSubresourceAndCreateReader(
