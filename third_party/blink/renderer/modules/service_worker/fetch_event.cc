...
class FetchRespondWithFulfill final : public ThenCallable<Response, FetchRespondWithFulfill> {
public:
...
FetchEvent* FetchEvent::Create(ScriptState* script_state, const AtomicString& type, const FetchEventInit* initializer)
{
    return MakeGarbageCollected<FetchEvent>(script_state, type, initializer, nullptr, nullptr, false);
}
...
void FetchEvent::respondWith(ScriptState* script_state, ScriptPromise<Response> script_promise, ExceptionState& exception_state)
{
    stopImmediatePropagation();
    if (observer_) {
        observer_->RespondWith(script_state, script_promise, MakeGarbageCollected<FetchRespondWithFulfill>(observer_), exception_state);
    }
}
...
{
    handled_property_->Reject(ServiceWorkerError::GetException(nullptr, mojom::blink::ServiceWorkerErrorType::kNetwork, error_message));
}
...
bool FetchEvent::HasPendingActivity() const
{
    ...
    return !preload_response_property_ || preload_response_property_->GetState() == PreloadResponseProperty::kPending;
}

FetchEvent::FetchEvent(ScriptState* script_state, const AtomicString& type, const FetchEventInit* initializer, FetchRespondWithObserver* respond_with_observer,
    WaitUntilObserver* wait_until_observer, bool navigation_preload_sent)
    : ExtendableEvent(type, initializer, wait_until_observer)
    , ActiveScriptWrappable<FetchEvent>({})
    , ExecutionContextClient(ExecutionContext::From(script_state))
    , observer_(respond_with_observer)
    , preload_response_property_(MakeGarbageCollected<PreloadResponseProperty>(ExecutionContext::From(script_state)))
    , handled_property_(MakeGarbageCollected<ScriptPromiseProperty<IDLUndefined, DOMException>>(ExecutionContext::From(script_state)))
{
    if (!navigation_preload_sent) {
        preload_response_property_->Resolve(ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())));
    }
...
void FetchEvent::OnNavigationPreloadResponse(ScriptState* script_state, std::unique_ptr<WebURLResponse> response, mojo::ScopedDataPipeConsumerHandle data_pipe)
{
    ...
    if (data_pipe.is_valid()) {
        ...
        bytes_consumer = MakeGarbageCollected<DataPipeBytesConsumer>(
            ExecutionContext::From(script_state)->GetTaskRunner(TaskType::kNetworking), std::move(data_pipe), &completion_notifier);
        body_completion_notifier_ = completion_notifier;
    }
    // TODO(ricea): Verify that this response can't be aborted from JS.
    FetchResponseData* response_data = bytes_consumer ? FetchResponseData::CreateWithBuffer(BodyStreamBuffer::Create(script_state, bytes_consumer,
    ...
    auto response_type = network_utils::IsRedirectResponseCode(preload_response_->HttpStatusCode()) ? network::mojom::FetchResponseType::kOpaqueRedirect
                                                                                                    : network::mojom::FetchResponseType::kBasic;

    response_data->InitFromResourceResponse(ExecutionContext::From(script_state), response_type, url_list, http_names::kGET,
        network::mojom::CredentialsMode::kInclude, preload_response_->ToResourceResponse());

    FetchResponseData* tainted_response = response_type == network::mojom::FetchResponseType::kOpaqueRedirect
        ? response_data->CreateOpaqueRedirectFilteredResponse()
        : response_data->CreateBasicFilteredResponse();
    preload_response_property_->Resolve(ScriptValue::From(script_state, Response::Create(ExecutionContext::From(script_state), tainted_response)));
}

void FetchEvent::OnNavigationPreloadError(ScriptState* script_state, std::unique_ptr<WebServiceWorkerError> error)
{
    if (!script_state->ContextIsValid())
        return;
    ...
    DCHECK(preload_response_property_);
    if (preload_response_property_->GetState() != PreloadResponseProperty::kPending) {
        return;
    }
    preload_response_property_->Reject(ServiceWorkerError::Take(nullptr, *error.get()));
}

void FetchEvent::OnNavigationPreloadComplete(WorkerGlobalScope* worker_global_scope, base::TimeTicks completion_time, int64_t encoded_data_length,
    int64_t encoded_body_length, int64_t decoded_body_length)
{
    DCHECK(preload_response_);
    if (body_completion_notifier_) {
    ...
    base::TimeTicks request_time = timing ? timing->RequestTime() : base::TimeTicks();
    // According to the Resource Timing spec, the initiator type of
    // navigation preload request is "navigation".
    mojom::blink::ResourceTimingInfoPtr info = CreateResourceTimingInfo(request_time, request_->url(), &resource_response);
    info->response_end = completion_time;
    info->allow_negative_values = true;
    WorkerGlobalScopePerformance::performance(*worker_global_scope)->AddResourceTiming(std::move(info), AtomicString("navigation"));
}
