...
// unusual failures.
const String GetMessageForResponseError(ServiceWorkerResponseError error, const KURL& request_url)
{
    String error_message = "The FetchEvent for \"" + request_url.GetString() + "\" resulted in a network error response: ";
    switch (error) {
    ...
    case ServiceWorkerResponseError::kDefaultPrevented:
        error_message = error_message + "preventDefault() was called without calling respondWith().";
        break;
    case ServiceWorkerResponseError::kNoV8Instance:
        error_message = error_message + "an object that was not a Response was passed to respondWith().";
        break;
    case ServiceWorkerResponseError::kResponseTypeError:
        error_message = error_message + "the promise was resolved with an error response object.";
        break;
    ...
    case ServiceWorkerResponseError::kResponseTypeOpaqueForClientRequest:
        error_message = error_message + "an \"opaque\" response was used for a client request.";
        break;
    ...
    case ServiceWorkerResponseError::kResponseBodyBroken:
        error_message = error_message + "a response body's status could not be checked.";
        break;
...
bool IsClientRequest(mojom::RequestContextFrameType frame_type, network::mojom::RequestDestination destination)
{
    return IsNavigationRequest(frame_type) || destination == network::mojom::RequestDestination::kSharedWorker
...
class FetchLoaderClient final : public GarbageCollected<FetchLoaderClient>, public FetchDataLoader::Client {
public:
    FetchLoaderClient(std::unique_ptr<ServiceWorkerEventQueue::StayAwakeToken> token, ServiceWorkerGlobalScope* service_worker,
        scoped_refptr<base::SingleThreadTaskRunner> task_runner)
        : callback_(service_worker)
        , token_(std::move(token))
    {
        ...
        // StartLoading().
        callback_receiver_ = callback_.BindNewPipeAndPassReceiver(std::move(task_runner));
    }

    FetchLoaderClient(const FetchLoaderClient&) = delete;
    FetchLoaderClient& operator=(const FetchLoaderClient&) = delete;

    void DidFetchDataStartedDataPipe(mojo::ScopedDataPipeConsumerHandle pipe) override
    {
    ...
    mojom::blink::ServiceWorkerStreamHandlePtr CreateStreamHandle()
    {
        if (!body_stream_.is_valid())
            return nullptr;
        return mojom::blink::ServiceWorkerStreamHandle::New(std::move(body_stream_), std::move(callback_receiver_));
    }
...
private:
    ...
    mojo::PendingReceiver<mojom::blink::ServiceWorkerStreamCallback> callback_receiver_;
...
class UploadingCompletionObserver : public GarbageCollected<UploadingCompletionObserver>, public BytesUploader::Client {
public:
    explicit UploadingCompletionObserver(
        int fetch_event_id, ScriptPromiseResolver<IDLUndefined>* resolver, ServiceWorkerGlobalScope* service_worker_global_scope)
...
// JavaScript.
void FetchRespondWithObserver::OnResponseRejected(ServiceWorkerResponseError error)
{
    DCHECK(GetExecutionContext());
    const String error_message = GetMessageForResponseError(error, request_url_);
    GetExecutionContext()->AddConsoleMessage(
        MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kWarning, error_message));
    ...
    ServiceWorkerGlobalScope* service_worker_global_scope = To<ServiceWorkerGlobalScope>(GetExecutionContext());
    service_worker_global_scope->RespondToFetchEvent(
        event_id_, request_url_, range_request_, std::move(response), event_dispatch_time_, base::TimeTicks::Now());
    event_->RejectHandledPromise(error_message);
}

void FetchRespondWithObserver::OnResponseFulfilled(ScriptState* script_state, Response* response)
{
    DCHECK(GetExecutionContext());
    ...
    //     |basic| nor |default|."
    const network::mojom::FetchResponseType response_type = response->GetResponse()->GetType();
    if (response_type == network::mojom::FetchResponseType::kError) {
        OnResponseRejected(ServiceWorkerResponseError::kResponseTypeError);
        return;
    }
    if (response_type == network::mojom::FetchResponseType::kCors && request_mode_ == network::mojom::RequestMode::kSameOrigin) {
        OnResponseRejected(ServiceWorkerResponseError::kResponseTypeCorsForRequestModeSameOrigin);
        return;
    }
    if (response_type == network::mojom::FetchResponseType::kOpaque) {
        ...
        if (IsClientRequest(frame_type_, request_destination_)) {
            OnResponseRejected(ServiceWorkerResponseError::kResponseTypeOpaqueForClientRequest);
            return;
        }
    }
    if (redirect_mode_ != network::mojom::RedirectMode::kManual && response_type == network::mojom::FetchResponseType::kOpaqueRedirect) {
        OnResponseRejected(ServiceWorkerResponseError::kResponseTypeOpaqueRedirect);
        return;
    }
    if (redirect_mode_ != network::mojom::RedirectMode::kFollow && response->redirected()) {
        OnResponseRejected(ServiceWorkerResponseError::kRedirectedResponseForNotFollowRequest);
        return;
    }
    ...
    mojom::blink::FetchAPIResponsePtr fetch_api_response = response->PopulateFetchAPIResponse(request_url_);
    ServiceWorkerGlobalScope* service_worker_global_scope = To<ServiceWorkerGlobalScope>(GetExecutionContext());
    ...
    auto initiator_origin = url::Origin::Create(GURL(service_worker_global_scope->Url()));
    ...
    if (corp_checker_
        && corp_checker_->IsBlocked(url::Origin::Create(GURL(service_worker_global_scope->Url())), request_mode_, request_destination_, *response)) {
    ...
    if (buffer) {
        ...
        scoped_refptr<BlobDataHandle> blob_data_handle
            = buffer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize, exception_state);

        if (blob_data_handle) {
            // Handle the blob response body.
            fetch_api_response->blob = blob_data_handle;
            service_worker_global_scope->RespondToFetchEvent(
                event_id_, request_url_, range_request_, std::move(fetch_api_response), event_dispatch_time_, base::TimeTicks::Now());
        ...
        FetchLoaderClient* fetch_loader_client
            = MakeGarbageCollected<FetchLoaderClient>(service_worker_global_scope->CreateStayAwakeToken(), service_worker_global_scope, task_runner_);
        buffer->StartLoading(FetchDataLoader::CreateLoaderAsDataPipe(task_runner_), fetch_loader_client, exception_state);
        if (exception_state.HadException()) {
            OnResponseRejected(ServiceWorkerResponseError::kResponseBodyBroken);
            return;
        }

        mojom::blink::ServiceWorkerStreamHandlePtr stream_handle = fetch_loader_client->CreateStreamHandle();
        ...
        service_worker_global_scope->RespondToFetchEventWithResponseStream(
            event_id_, request_url_, range_request_, std::move(fetch_api_response), std::move(stream_handle), event_dispatch_time_, base::TimeTicks::Now());
        event_->ResolveHandledPromise();
        return;
    }
    service_worker_global_scope->RespondToFetchEvent(
...
{
    DCHECK(GetExecutionContext());
    if (original_request_body_stream_ && (original_request_body_stream_->IsLocked() || original_request_body_stream_->IsDisturbed())) {
        GetExecutionContext()->CountUse(WebFeature::kFetchRespondWithNoResponseWithUsedRequestBody);
    }
    ...
    if (body_buffer && !request_body_has_source_) {
        ...
        if (body_stream->IsLocked() || body_stream->IsDisturbed()) {
            OnResponseRejected(mojom::blink::ServiceWorkerResponseError::kRequestBodyUnusable);
            return;
        }
        ...
        auto* observer = MakeGarbageCollected<UploadingCompletionObserver>(event_id_, resolver, service_worker_global_scope);
        mojo::PendingRemote<network::mojom::blink::ChunkedDataPipeGetter> remote;
        body_buffer->DrainAsChunkedDataPipeGetter(script_state, remote.InitWithNewPipeAndPassReceiver(), observer);
        request_body_to_pass.emplace(ToCrossVariantMojoType(std::move(remote)), network::DataElementChunkedDataPipe::ReadOnlyOnce(true));
    }

    service_worker_global_scope->RespondToFetchEventWithNoResponse(
        event_id_, event_.Get(), request_url_, range_request_, std::move(request_body_to_pass), event_dispatch_time_, base::TimeTicks::Now());
    event_->ResolveHandledPromise();
}
...
FetchRespondWithObserver::FetchRespondWithObserver(ExecutionContext* context, int fetch_event_id, base::WeakPtr<CrossOriginResourcePolicyChecker> corp_checker,
    const mojom::blink::FetchAPIRequest& request, WaitUntilObserver* observer)
