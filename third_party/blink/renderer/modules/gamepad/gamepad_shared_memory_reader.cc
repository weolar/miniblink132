...
GamepadSharedMemoryReader::GamepadSharedMemoryReader(LocalDOMWindow& window)
    ...
    , gamepad_monitor_remote_(&window)
{
    // See https://bit.ly/2S0zRAS for task types
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = window.GetTaskRunner(TaskType::kMiscPlatformAPI);
    window.GetBrowserInterfaceBroker().GetInterface(gamepad_monitor_remote_.BindNewPipeAndPassReceiver(task_runner));
    gamepad_monitor_remote_->SetObserver(receiver_.BindNewPipeAndPassRemote(task_runner));
}
...
void GamepadSharedMemoryReader::SendStartMessage()
{
    if (gamepad_monitor_remote_.is_bound()) {
        gamepad_monitor_remote_->GamepadStartPolling(&renderer_shared_buffer_region_);
    }
}
...
void GamepadSharedMemoryReader::Start(blink::GamepadListener* listener)
{
    ...
    gamepad_hardware_buffer_ = renderer_shared_buffer_mapping_.GetMemoryAs<device::GamepadHardwareBuffer>();
...
void GamepadSharedMemoryReader::GamepadConnected(uint32_t index, const device::Gamepad& gamepad)
{
...
void GamepadSharedMemoryReader::GamepadDisconnected(uint32_t index, const device::Gamepad& gamepad)
{
    if (listener_)
