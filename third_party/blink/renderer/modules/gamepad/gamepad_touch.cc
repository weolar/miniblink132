...
{
    const std::array<float, 2> values = { static_cast<float>(x), static_cast<float>(y) };
...
{
    return device_touch.touch_id == touch_id_ && device_touch.surface_id == surface_id_ && device_touch.has_surface_dimensions == has_surface_dimensions_
        && device_touch.x == position_->Item(0) && device_touch.y == position_->Item(1) && device_touch.surface_width == surface_dimensions_->Item(0)
        && device_touch.surface_height == surface_dimensions_->Item(1);
}

void GamepadTouch::UpdateValuesFrom(const device::GamepadTouch& device_touch, uint32_t id_offset)
{
    touch_id_ = id_offset;
    surface_id_ = device_touch.surface_id;
    position_ = ToFloat32Array(device_touch.x, device_touch.y);
    surface_dimensions_ = ToUint32Array(device_touch.surface_width, device_touch.surface_height);
