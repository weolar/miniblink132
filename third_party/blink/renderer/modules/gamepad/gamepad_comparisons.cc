...
template <typename T> auto AsSpan(const T& collection)
{
    return collection.AsSpan();
}

base::span<const GamepadTouchVector::ValueType> AsSpan(const GamepadTouchVector& collection)
{
    return base::span(collection);
}

template <typename Collection, typename Pred = std::equal_to<typename Collection::ValueType>>
bool Compare(const Collection* old_array, const Collection* new_array, Pred pred = Pred {})
{
...
// static
bool GamepadComparisons::HasUserActivation(const HeapVector<Member<Gamepad>> gamepads)
{
...
// static
void GamepadComparisons::HasGamepadConnectionChanged(bool old_connected, bool new_connected, bool id_changed, bool* gamepad_found, bool* gamepad_lost)
{
    if (gamepad_found)
...
GamepadStateCompareResult::GamepadStateCompareResult(
    const HeapVector<Member<Gamepad>> old_gamepads, const HeapVector<Member<Gamepad>> new_gamepads, bool compare_all_axes, bool compare_all_buttons)
{
    any_change_ = CompareGamepads(old_gamepads, new_gamepads, compare_all_axes, compare_all_buttons);
}
...
bool GamepadStateCompareResult::IsAxisChanged(size_t pad_index, size_t axis_index) const
{
    DCHECK_LT(pad_index, device::Gamepads::kItemsLengthCap);
...
bool GamepadStateCompareResult::IsButtonChanged(size_t pad_index, size_t button_index) const
{
    DCHECK_LT(pad_index, device::Gamepads::kItemsLengthCap);
...
bool GamepadStateCompareResult::IsButtonDown(size_t pad_index, size_t button_index) const
{
    DCHECK_LT(pad_index, device::Gamepads::kItemsLengthCap);
    DCHECK_LT(button_index, device::Gamepad::kButtonsLengthCap);
...
bool GamepadStateCompareResult::IsButtonUp(size_t pad_index, size_t button_index) const
{
    DCHECK_LT(pad_index, device::Gamepads::kItemsLengthCap);
    DCHECK_LT(button_index, device::Gamepad::kButtonsLengthCap);
    ...
    for (uint32_t i = 0; i < new_gamepads.size(); ++i) {
        ...
        if (old_gamepad && new_gamepad) {
            GamepadComparisons::HasGamepadConnectionChanged(
                old_connected, new_connected, old_gamepad->id() != new_gamepad->id(), &newly_connected, &newly_disconnected);
        } else {
            newly_connected = new_connected;
            newly_disconnected = old_connected;
        }

        bool any_axis_updated = CompareAxes(old_gamepad, new_gamepad, i, compare_all_axes);
        bool any_button_updated = CompareButtons(old_gamepad, new_gamepad, i, compare_all_buttons);
        ...
        if (newly_connected || newly_disconnected || any_axis_updated || any_button_updated || any_touch_updated) {
            any_change = true;
        }
    }
    return any_change;
...
bool GamepadStateCompareResult::CompareAxes(Gamepad* old_gamepad, Gamepad* new_gamepad, size_t index, bool compare_all)
{
...
bool GamepadStateCompareResult::CompareButtons(Gamepad* old_gamepad, Gamepad* new_gamepad, size_t index, bool compare_all)
{
    DCHECK_LT(index, device::Gamepads::kItemsLengthCap);
    if (!new_gamepad)
        return false;
...
bool GamepadStateCompareResult::CompareTouches(Gamepad* old_gamepad, Gamepad* new_gamepad)
{
    if (!new_gamepad) {
        return false;
    }
    ...
    return Compare(old_touches, new_touches, [](const Member<GamepadTouch>& new_touch, const Member<GamepadTouch>& old_touch) {
        return new_touch->touchId() == old_touch->touchId() && new_touch->surfaceId() == old_touch->surfaceId()
            && new_touch->HasSurfaceDimensions() == old_touch->HasSurfaceDimensions()
            && !Compare(new_touch->surfaceDimensions(), old_touch->surfaceDimensions()) && !Compare(new_touch->position(), old_touch->position());
    });
}
...
GamepadStateCompareResult GamepadComparisons::Compare(
    const HeapVector<Member<Gamepad>> old_gamepads, const HeapVector<Member<Gamepad>> new_gamepads, bool compare_all_axes, bool compare_all_buttons)
{
    return GamepadStateCompareResult(old_gamepads, new_gamepads, compare_all_axes, compare_all_buttons);
}
