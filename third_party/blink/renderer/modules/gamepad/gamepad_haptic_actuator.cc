...
GamepadHapticEffectType EffectTypeFromEnum(V8GamepadHapticEffectType::Enum type)
{
    switch (type) {
...
V8GamepadHapticsResult ResultToV8(GamepadHapticsResult result)
{
    ...
    case GamepadHapticsResult::GamepadHapticsResultInvalidParameter:
        return V8GamepadHapticsResult(V8GamepadHapticsResult::Enum::kInvalidParameter);
...
GamepadHapticActuator::GamepadHapticActuator(ExecutionContext& context, int pad_index, device::GamepadHapticActuatorType type)
...
void GamepadHapticActuator::SetType(device::GamepadHapticActuatorType type)
{
    ...
    case device::GamepadHapticActuatorType::kTriggerRumble:
        supported_effects_.push_back(V8GamepadHapticEffectType(V8GamepadHapticEffectType::Enum::kTriggerRumble));
        [[fallthrough]];
    case device::GamepadHapticActuatorType::kDualRumble:
        supported_effects_.push_back(V8GamepadHapticEffectType(V8GamepadHapticEffectType::Enum::kDualRumble));
...
ScriptPromise<V8GamepadHapticsResult> GamepadHapticActuator::playEffect(
    ScriptState* script_state, const V8GamepadHapticEffectType& type, const GamepadEffectParameters* params)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<V8GamepadHapticsResult>>(script_state);
    auto promise = resolver->Promise();

    if (params->duration() < 0.0 || params->startDelay() < 0.0 || params->strongMagnitude() < 0.0 || params->strongMagnitude() > 1.0
        || params->weakMagnitude() < 0.0 || params->weakMagnitude() > 1.0 || params->leftTrigger() < 0.0 || params->leftTrigger() > 1.0
        || params->rightTrigger() < 0.0 || params->rightTrigger() > 1.0) {
        resolver->Resolve(ResultToV8(GamepadHapticsResult::GamepadHapticsResultInvalidParameter));
        return promise;
    }

    // Limit the total effect duration.
    double effect_duration = params->duration() + params->startDelay();
    if (effect_duration > device::GamepadHapticActuator::kMaxEffectDurationMillis) {
        resolver->Resolve(ResultToV8(GamepadHapticsResult::GamepadHapticsResultInvalidParameter));
        return promise;
    }
    ...
    auto callback = WTF::BindOnce(&GamepadHapticActuator::OnPlayEffectCompleted, WrapPersistent(this), WrapPersistent(resolver));

    gamepad_dispatcher_->PlayVibrationEffectOnce(pad_index_, EffectTypeFromEnum(type.AsEnum()),
        device::mojom::blink::GamepadEffectParameters::New(
            params->duration(), params->startDelay(), params->strongMagnitude(), params->weakMagnitude(), params->leftTrigger(), params->rightTrigger()),
        std::move(callback));

    return promise;
}

void GamepadHapticActuator::OnPlayEffectCompleted(ScriptPromiseResolver<V8GamepadHapticsResult>* resolver, device::mojom::GamepadHapticsResult result)
{
    if (result == GamepadHapticsResult::GamepadHapticsResultError) {
        ...
        if (context) {
            ...
            context->GetTaskRunner(TaskType::kMiscPlatformAPI)
                ->PostTask(FROM_HERE, WTF::BindOnce(&GamepadHapticActuator::ResetVibrationIfNotPreempted, WrapPersistent(this)));
        } else {
...
ScriptPromise<V8GamepadHapticsResult> GamepadHapticActuator::reset(ScriptState* script_state)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<V8GamepadHapticsResult>>(script_state);

    auto callback = WTF::BindOnce(&GamepadHapticActuator::OnResetCompleted, WrapPersistent(this), WrapPersistent(resolver));
...
void GamepadHapticActuator::OnResetCompleted(ScriptPromiseResolver<V8GamepadHapticsResult>* resolver, device::mojom::GamepadHapticsResult result)
{
    if (result == GamepadHapticsResult::GamepadHapticsResultError) {
        resolver->Reject();
        return;
