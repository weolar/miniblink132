...
WebPrintJob::WebPrintJob(ExecutionContext* execution_context, mojom::blink::WebPrintJobInfoPtr print_job_info)
    ...
    , controller_(execution_context)
{
    ...
    observer_.Bind(std::move(print_job_info->observer), execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI));
    controller_.Bind(std::move(print_job_info->controller), execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI));
}
...
void WebPrintJob::cancel()
{
    ...
    if (cancel_called_ || !AreFurtherStateUpdatesPossible(attributes_->jobState().AsEnum())) {
        return;
    }
    cancel_called_ = true;
    controller_->Cancel();
...
void WebPrintJob::OnWebPrintJobUpdate(mojom::blink::WebPrintJobUpdatePtr update)
{
    auto state = mojo::ConvertTo<V8JobStateEnum>(update->state);
    // Discard the update if nothing has actually changed.
    if (state == attributes_->jobState().AsEnum() && update->pages_printed == attributes_->jobPagesCompleted()) {
        return;
    }
...
bool WebPrintJob::HasPendingActivity() const
{
    ...
    return AreFurtherStateUpdatesPossible(attributes_->jobState().AsEnum()) && HasEventListeners();
}
