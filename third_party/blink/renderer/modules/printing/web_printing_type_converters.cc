...
using BlinkMediaCollectionRequested = blink::WebPrintingMediaCollectionRequested;
...
using MojomMediaCollectionRequested = blink::mojom::blink::WebPrintingMediaCollectionRequested;
using MojomMediaCollectionRequestedPtr = blink::mojom::blink::WebPrintingMediaCollectionRequestedPtr;
using MojomMediaSizeDimensionPtr = blink::mojom::blink::WebPrintingMediaSizeDimensionPtr;

// multiple-document-handling:
using V8MultipleDocumentHandling = blink::V8WebPrintingMultipleDocumentHandling;
using MojomMultipleDocumentHandling = blink::mojom::blink::WebPrintingMultipleDocumentHandling;

// orientation-requested:
using V8OrientationRequested = blink::V8WebPrintingOrientationRequested;
using MojomOrientationRequested = blink::mojom::blink::WebPrintingOrientationRequested;
...
template <> struct TypeConverter<V8Sides, MojomSides> {
...
template <> struct TypeConverter<MojomSides, V8Sides> {
...
template <> struct TypeConverter<V8MultipleDocumentHandling, MojomMultipleDocumentHandling> {
    static V8MultipleDocumentHandling Convert(const MojomMultipleDocumentHandling& mdh)
    {
        switch (mdh) {
        case MojomMultipleDocumentHandling::kSeparateDocumentsCollatedCopies:
            return V8MultipleDocumentHandling(V8MultipleDocumentHandling::Enum::kSeparateDocumentsCollatedCopies);
        case MojomMultipleDocumentHandling::kSeparateDocumentsUncollatedCopies:
            return V8MultipleDocumentHandling(V8MultipleDocumentHandling::Enum::kSeparateDocumentsUncollatedCopies);
        }
    }
};

template <> struct TypeConverter<MojomMultipleDocumentHandling, V8MultipleDocumentHandling> {
    static MojomMultipleDocumentHandling Convert(const V8MultipleDocumentHandling& mdh)
    {
        switch (mdh.AsEnum()) {
        ...
        case V8MultipleDocumentHandling::Enum::kSeparateDocumentsUncollatedCopies:
            return MojomMultipleDocumentHandling::kSeparateDocumentsUncollatedCopies;
        }
    }
};

template <> struct TypeConverter<V8OrientationRequested, MojomOrientationRequested> {
    static V8OrientationRequested Convert(const MojomOrientationRequested& orientation)
    {
        switch (orientation) {
...
template <> struct TypeConverter<MojomOrientationRequested, V8OrientationRequested> {
    static MojomOrientationRequested Convert(const V8OrientationRequested& orientation)
    {
...
template <> struct TypeConverter<BlinkRange*, MojomRangePtr> {
...
template <> struct TypeConverter<V8MediaSizeDimension*, MojomMediaSizeDimensionPtr> {
    static V8MediaSizeDimension* Convert(const MojomMediaSizeDimensionPtr& dimension)
    {
        return dimension->is_range() ? blink::MakeGarbageCollected<V8MediaSizeDimension>(mojo::ConvertTo<BlinkRange*>(dimension->get_range()))
                                     : blink::MakeGarbageCollected<V8MediaSizeDimension>(dimension->get_value());
    }
};

template <> struct TypeConverter<BlinkMediaCollection*, MojomMediaCollection*> {
    static BlinkMediaCollection* Convert(const MojomMediaCollection* media_col_in)
    {
        auto* media_col = blink::MakeGarbageCollected<BlinkMediaCollection>();
        media_col->setMediaSizeName(media_col_in->media_size_name);
        auto* media_size = blink::MakeGarbageCollected<blink::WebPrintingMediaSize>();
        media_size->setYDimension(mojo::ConvertTo<V8MediaSizeDimension*>(media_col_in->media_size->y_dimension));
        media_size->setXDimension(mojo::ConvertTo<V8MediaSizeDimension*>(media_col_in->media_size->x_dimension));
        media_col->setMediaSize(media_size);
        return media_col;
    }
};

template <> struct TypeConverter<MojomMediaCollectionRequestedPtr, BlinkMediaCollectionRequested*> {
    static MojomMediaCollectionRequestedPtr Convert(const BlinkMediaCollectionRequested* media_col_in)
    {
        auto media_col = MojomMediaCollectionRequested::New();
        media_col->media_size
            = { base::checked_cast<int32_t>(media_col_in->mediaSize()->xDimension()), base::checked_cast<int32_t>(media_col_in->mediaSize()->yDimension()) };
        return media_col;
    }
};

template <> struct TypeConverter<gfx::Size, blink::WebPrintingResolution*> {
    static gfx::Size Convert(const blink::WebPrintingResolution* printer_resolution)
    {
        CHECK(printer_resolution->hasCrossFeedDirectionResolution());
        CHECK(printer_resolution->hasFeedDirectionResolution());
        if (printer_resolution->hasUnits() && printer_resolution->units() == blink::V8WebPrintingResolutionUnits::Enum::kDotsPerCentimeter) {
            return gfx::Size(printer_resolution->crossFeedDirectionResolution() * blink::kCentimetersPerInch,
                printer_resolution->feedDirectionResolution() * blink::kCentimetersPerInch);
        }
        return gfx::Size(printer_resolution->crossFeedDirectionResolution(), printer_resolution->feedDirectionResolution());
    }
};

template <> struct TypeConverter<blink::WebPrintingResolution*, gfx::Size> {
    static blink::WebPrintingResolution* Convert(const gfx::Size& printer_resolution)
    {
        auto* output_resolution = blink::MakeGarbageCollected<blink::WebPrintingResolution>();
        output_resolution->setCrossFeedDirectionResolution(printer_resolution.width());
        output_resolution->setFeedDirectionResolution(printer_resolution.height());
        output_resolution->setUnits(blink::V8WebPrintingResolutionUnits::Enum::kDotsPerInch);
        return output_resolution;
    }
};

template <> struct TypeConverter<V8ColorMode, MojomColorMode> {
...
template <> struct TypeConverter<MojomColorMode, V8ColorMode> {
...
template <> struct TypeConverter<V8PrinterState::Enum, MojomPrinterState> {
...
template <> struct TypeConverter<V8PrinterStateReason, MojomPrinterStateReason> {
    static V8PrinterStateReason Convert(const MojomPrinterStateReason& printer_state_reason)
    {
        ...
        case MojomPrinterStateReason::kConnectingToDevice:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kConnectingToDevice);
        ...
        case MojomPrinterStateReason::kDeveloperEmpty:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kDeveloperEmpty);
        ...
        case MojomPrinterStateReason::kFuserUnderTemp:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kFuserUnderTemp);
        case MojomPrinterStateReason::kInputTrayMissing:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kInputTrayMissing);
        ...
        case MojomPrinterStateReason::kInterpreterResourceUnavailable:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kInterpreterResourceUnavailable);
        case MojomPrinterStateReason::kMarkerSupplyEmpty:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kMarkerSupplyEmpty);
        case MojomPrinterStateReason::kMarkerSupplyLow:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kMarkerSupplyLow);
        case MojomPrinterStateReason::kMarkerWasteAlmostFull:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kMarkerWasteAlmostFull);
        case MojomPrinterStateReason::kMarkerWasteFull:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kMarkerWasteFull);
        ...
        case MojomPrinterStateReason::kMovingToPaused:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kMovingToPaused);
        ...
        case MojomPrinterStateReason::kOutputAreaAlmostFull:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kOutputAreaAlmostFull);
        case MojomPrinterStateReason::kOutputAreaFull:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kOutputAreaFull);
        case MojomPrinterStateReason::kOutputTrayMissing:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kOutputTrayMissing);
        ...
        case MojomPrinterStateReason::kCupsPkiExpired:
            return V8PrinterStateReason(V8PrinterStateReason::Enum::kCupsPkiExpired);
        }
    }
};
...
void ProcessCopies(const mojom::blink::WebPrinterAttributes& new_attributes, WebPrinterAttributes* current_attributes)
{
    current_attributes->setCopiesDefault(new_attributes.copies_default);
    current_attributes->setCopiesSupported(mojo::ConvertTo<BlinkRange*>(new_attributes.copies_supported));
}

void ProcessDocumentFormat(const mojom::blink::WebPrinterAttributes& new_attributes, WebPrinterAttributes* current_attributes)
{
    current_attributes->setDocumentFormatDefault(V8WebPrintingMimeMediaType::Enum::kApplicationPdf);
    current_attributes->setDocumentFormatSupported({ V8WebPrintingMimeMediaType(V8WebPrintingMimeMediaType::Enum::kApplicationPdf) });
}

void ProcessMediaCollection(const mojom::blink::WebPrinterAttributes& new_attributes, WebPrinterAttributes* current_attributes)
{
    current_attributes->setMediaColDefault(mojo::ConvertTo<BlinkMediaCollection*>(new_attributes.media_col_default));
    current_attributes->setMediaColDatabase(mojo::ConvertTo<HeapVector<Member<BlinkMediaCollection>>>(new_attributes.media_col_database));
}

void ProcessMediaSource(const mojom::blink::WebPrinterAttributes& new_attributes, WebPrinterAttributes* current_attributes)
{
    if (new_attributes.media_source_default) {
        current_attributes->setMediaSourceDefault(new_attributes.media_source_default);
    }
    if (!new_attributes.media_source_supported.empty()) {
        current_attributes->setMediaSourceSupported(new_attributes.media_source_supported);
    }
}

void ProcessMultipleDocumentHandling(const mojom::blink::WebPrinterAttributes& new_attributes, WebPrinterAttributes* current_attributes)
{
    current_attributes->setMultipleDocumentHandlingDefault(mojo::ConvertTo<V8MultipleDocumentHandling>(new_attributes.multiple_document_handling_default));
    current_attributes->setMultipleDocumentHandlingSupported(
        mojo::ConvertTo<Vector<V8MultipleDocumentHandling>>(new_attributes.multiple_document_handling_supported));
}

void ProcessOrientationRequested(const mojom::blink::WebPrinterAttributes& new_attributes, WebPrinterAttributes* current_attributes)
{
    current_attributes->setOrientationRequestedDefault(mojo::ConvertTo<V8OrientationRequested>(new_attributes.orientation_requested_default));
    current_attributes->setOrientationRequestedSupported(mojo::ConvertTo<Vector<V8OrientationRequested>>(new_attributes.orientation_requested_supported));
}

void ProcessPrinterResolution(const mojom::blink::WebPrinterAttributes& new_attributes, WebPrinterAttributes* current_attributes)
{
    current_attributes->setPrinterResolutionDefault(mojo::ConvertTo<blink::WebPrintingResolution*>(new_attributes.printer_resolution_default));
    current_attributes->setPrinterResolutionSupported(
        mojo::ConvertTo<HeapVector<Member<blink::WebPrintingResolution>>>(new_attributes.printer_resolution_supported));
}

void ProcessPrintColorMode(const mojom::blink::WebPrinterAttributes& new_attributes, WebPrinterAttributes* current_attributes)
{
    current_attributes->setPrintColorModeDefault(mojo::ConvertTo<V8ColorMode>(new_attributes.print_color_mode_default));
    current_attributes->setPrintColorModeSupported(mojo::ConvertTo<Vector<V8ColorMode>>(new_attributes.print_color_mode_supported));
}

void ProcessSides(const mojom::blink::WebPrinterAttributes& new_attributes, WebPrinterAttributes* current_attributes)
{
    if (new_attributes.sides_default) {
        current_attributes->setSidesDefault(mojo::ConvertTo<V8Sides>(*new_attributes.sides_default));
    }
    if (!new_attributes.sides_supported.empty()) {
        current_attributes->setSidesSupported(mojo::ConvertTo<Vector<V8Sides>>(new_attributes.sides_supported));
    }
}
...
blink::WebPrinterAttributes* TypeConverter<blink::WebPrinterAttributes*, blink::mojom::blink::WebPrinterAttributesPtr>::Convert(
    const blink::mojom::blink::WebPrinterAttributesPtr& printer_attributes)
{
    ...
    attributes->setPrinterState(mojo::ConvertTo<V8PrinterState::Enum>(printer_attributes->printer_state));
    attributes->setPrinterStateReasons(mojo::ConvertTo<Vector<V8PrinterStateReason>>(printer_attributes->printer_state_reasons));
...
TypeConverter<blink::mojom::blink::WebPrintJobTemplateAttributesPtr, blink::WebPrintJobTemplateAttributes*>::Convert(
    const blink::WebPrintJobTemplateAttributes* pjt_attributes)
{
    ...
    if (pjt_attributes->hasMediaCol()) {
        attributes->media_col = mojo::ConvertTo<MojomMediaCollectionRequestedPtr>(pjt_attributes->mediaCol());
    }
    ...
    if (pjt_attributes->hasMultipleDocumentHandling()) {
        attributes->multiple_document_handling = mojo::ConvertTo<MojomMultipleDocumentHandling>(pjt_attributes->multipleDocumentHandling());
    }
    if (pjt_attributes->hasOrientationRequested()) {
        attributes->orientation_requested = mojo::ConvertTo<MojomOrientationRequested>(pjt_attributes->orientationRequested());
    }
    if (pjt_attributes->hasPrinterResolution()) {
        attributes->printer_resolution = mojo::ConvertTo<gfx::Size>(pjt_attributes->printerResolution());
    }
    if (pjt_attributes->hasPrintColorMode()) {
        attributes->print_color_mode = mojo::ConvertTo<MojomColorMode>(pjt_attributes->printColorMode());
    }
...
V8JobState::Enum TypeConverter<V8JobState::Enum, MojomJobState>::Convert(const MojomJobState& state)
{
    switch (state) {
