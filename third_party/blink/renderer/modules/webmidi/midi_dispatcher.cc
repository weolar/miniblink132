    ...
    , midi_session_provider_(execution_context)
{
    ...
    execution_context->GetBrowserInterfaceBroker().GetInterface(
        midi_session_provider_.BindNewPipeAndPassReceiver(execution_context->GetTaskRunner(blink::TaskType::kMiscPlatformAPI)));
    midi_session_provider_->StartSession(midi_session_.BindNewPipeAndPassReceiver(execution_context->GetTaskRunner(blink::TaskType::kMiscPlatformAPI)),
        receiver_.BindNewPipeAndPassRemote(execution_context->GetTaskRunner(blink::TaskType::kMiscPlatformAPI)));
}

MIDIDispatcher::~MIDIDispatcher() = default;

void MIDIDispatcher::SendMIDIData(uint32_t port, base::span<const uint8_t> data, base::TimeTicks timestamp)
{
    if ((kMaxUnacknowledgedBytesSent - unacknowledged_bytes_sent_) < data.size()) {
...
void MIDIDispatcher::AddInputPort(midi::mojom::blink::PortInfoPtr info)
{
    ...
    if (initialized_) {
        client_->DidAddInputPort(info->id, info->manufacturer, info->name, info->version, info->state);
    }
}
...
void MIDIDispatcher::AddOutputPort(midi::mojom::blink::PortInfoPtr info)
{
    ...
    if (initialized_) {
        client_->DidAddOutputPort(info->id, info->manufacturer, info->name, info->version, info->state);
    }
}

void MIDIDispatcher::SetInputPortState(uint32_t port, midi::mojom::blink::PortState state)
{
    DCHECK(client_);
...
void MIDIDispatcher::SetOutputPortState(uint32_t port, midi::mojom::blink::PortState state)
{
    DCHECK(client_);
...
void MIDIDispatcher::DataReceived(uint32_t port, const Vector<uint8_t>& data, base::TimeTicks timestamp)
{
    DCHECK(client_);
