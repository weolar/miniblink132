...
template <typename InterfaceType, typename ValueType> class MIDIPortMap : public ScriptWrappable, public Maplike<InterfaceType> {
public:
...
private:
    ...
    using IteratorType = typename base::CheckedContiguousIterator<const typename Entries::ValueType>;

    typename PairSyncIterable<InterfaceType>::IterationSource* CreateIterationSource(ScriptState*, ExceptionState&) override
    {
        return MakeGarbageCollected<MapIterationSource>(this, entries_.CheckedBegin(), entries_.CheckedEnd());
    }

    bool GetMapEntry(ScriptState*, const String& key, ValueType*& value, ExceptionState&) override
    {
    ...
    class MapIterationSource final : public PairSyncIterable<InterfaceType>::IterationSource {
    public:
        MapIterationSource(MIDIPortMap<InterfaceType, ValueType>* map, IteratorType iterator, IteratorType end)
            : map_(map)
            , iterator_(iterator)
            , end_(end)
        {
        ...
        bool FetchNextItem(ScriptState* script_state, String& key, ValueType*& value, ExceptionState&) override
        {
            if (iterator_ == end_)
                return false;
            key = (*iterator_)->id();
