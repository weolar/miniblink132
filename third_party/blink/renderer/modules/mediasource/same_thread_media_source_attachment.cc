...
// semantic.
blink::SameThreadMediaSourceTracer* GetTracerImpl(blink::MediaSourceTracer* tracer)
{
...
SameThreadMediaSourceAttachment::SameThreadMediaSourceAttachment(MediaSource* media_source, AttachmentCreationPassKeyProvider::PassKey /* passkey */)
...
void SameThreadMediaSourceAttachment::NotifyDurationChanged(MediaSourceTracer* tracer, double duration)
{
    DVLOG(1) << __func__ << " this=" << this;
...
base::TimeDelta SameThreadMediaSourceAttachment::GetRecentMediaTime(MediaSourceTracer* tracer)
{
    DVLOG(1) << __func__ << " this=" << this;
    ...
    DVLOG(2) << __func__ << " this=" << this << " -> recent time=" << recent_element_time_ << ", actual currentTime=" << result;
    return result;
}

bool SameThreadMediaSourceAttachment::GetElementError(MediaSourceTracer* tracer)
{
    DVLOG(1) << __func__ << " this=" << this;
...
AudioTrackList* SameThreadMediaSourceAttachment::CreateAudioTrackList(MediaSourceTracer* tracer)
{
    DVLOG(1) << __func__ << " this=" << this;
...
VideoTrackList* SameThreadMediaSourceAttachment::CreateVideoTrackList(MediaSourceTracer* tracer)
{
    DVLOG(1) << __func__ << " this=" << this;
...
void SameThreadMediaSourceAttachment::AddAudioTrackToMediaElement(MediaSourceTracer* tracer, AudioTrack* track)
{
...
void SameThreadMediaSourceAttachment::AddVideoTrackToMediaElement(MediaSourceTracer* tracer, VideoTrack* track)
{
    DVLOG(3) << __func__ << " this=" << this;
...
void SameThreadMediaSourceAttachment::RemoveAudioTracksFromMediaElement(MediaSourceTracer* tracer, Vector<String> audio_ids, bool enqueue_change_event)
{
    DVLOG(3) << __func__ << " this=" << this << ", ids size=" << audio_ids.size() << ", enqueue_change_event=" << enqueue_change_event;

    if (element_context_destroyed_ || media_source_context_destroyed_) {
        DVLOG(3) << __func__ << " this=" << this << " -> skipping due to context(s) destroyed";
        return;
    }
    ...
    for (auto& audio_id : audio_ids) {
        if (element->audioTracks().getTrackById(audio_id)) {
            ...
            DVLOG(3) << __func__ << " this=" << this << ", skipping removal of missing audio track id " << audio_id;
        }
    }
...
void SameThreadMediaSourceAttachment::RemoveVideoTracksFromMediaElement(MediaSourceTracer* tracer, Vector<String> video_ids, bool enqueue_change_event)
{
    DVLOG(3) << __func__ << " this=" << this << ", ids size=" << video_ids.size() << ", enqueue_change_event=" << enqueue_change_event;
    ...
    for (auto& video_id : video_ids) {
        if (element->videoTracks().getTrackById(video_id)) {
            ...
            // This case can happen on element noneSupported() after MSE had added
            // some track(s). See https://crbug.com/1204656.
            DVLOG(3) << __func__ << " this=" << this << ", skipping removal of missing video track id " << video_id;
        }
    }
...
MediaSourceTracer* SameThreadMediaSourceAttachment::StartAttachingToMediaElement(HTMLMediaElement* element, bool* success)
{
    VerifyCalledWhileContextsAliveForDebugging();
    DCHECK(success);
    ...
    MediaSourceTracer* tracer = registered_media_source_->StartAttachingToMediaElement(WrapRefCounted(this), element);
...
void SameThreadMediaSourceAttachment::CompleteAttachingToMediaElement(MediaSourceTracer* tracer, std::unique_ptr<WebMediaSource> web_media_source)
{
    VerifyCalledWhileContextsAliveForDebugging();

    GetMediaSource(tracer)->CompleteAttachingToMediaElement(std::move(web_media_source));
}
...
WebTimeRanges SameThreadMediaSourceAttachment::BufferedInternal(MediaSourceTracer* tracer) const
{
...
WebTimeRanges SameThreadMediaSourceAttachment::SeekableInternal(MediaSourceTracer* tracer) const
{
    VerifyCalledWhileContextsAliveForDebugging();
...
void SameThreadMediaSourceAttachment::OnTrackChanged(MediaSourceTracer* tracer, TrackBase* track)
{
...
void SameThreadMediaSourceAttachment::VerifyCalledWhileContextsAliveForDebugging() const
{
    DCHECK(!element_context_destroyed_);
    DCHECK(!media_source_context_destroyed_);
}
