...
class HandleAttachmentProvider final : public WTF::ThreadSafeRefCounted<HandleAttachmentProvider> {
public:
    explicit HandleAttachmentProvider(scoped_refptr<MediaSourceAttachment> attachment);
    ...
    scoped_refptr<MediaSourceAttachment> TakeAttachment() LOCKS_EXCLUDED(attachment_lock_);
