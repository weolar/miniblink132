...
TrackDefaultList* TrackDefaultList::Create(const HeapVector<Member<TrackDefault>>& track_defaults, ExceptionState& exception_state)
{
    ...
    for (const auto& track_default : track_defaults) {
        TypeAndID key = TypeAndID(track_default->type().AsEnum(), track_default->byteStreamTrackID());
        if (!type_and_id_to_track_default_map.insert(key, track_default).is_new_entry) {
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError,
                "Duplicate TrackDefault type (" + V8TrackDefaultType(key.first).AsString() + ") and byteStreamTrackID (" + key.second + ")");
            return nullptr;
        }
    }
...
TrackDefaultList::TrackDefaultList(const HeapVector<Member<TrackDefault>>& track_defaults)
