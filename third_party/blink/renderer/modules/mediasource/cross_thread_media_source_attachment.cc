...
std::ostream& operator<<(std::ostream& stream, CrossThreadMediaSourceAttachment::TrackAddRemovalType track_type)
{
    return stream << (track_type == CrossThreadMediaSourceAttachment::TrackAddRemovalType::kAudio ? "audio" : "video");
}

} // anonymous namespace

CrossThreadMediaSourceAttachment::CrossThreadMediaSourceAttachment(MediaSource* media_source, AttachmentCreationPassKeyProvider::PassKey /* passkey */)
    ...
    worker_runner_(media_source->GetExecutionContext()->GetTaskRunner(TaskType::kPostedMessage))
...
void CrossThreadMediaSourceAttachment::NotifyDurationChanged(MediaSourceTracer* /* tracer */, double new_duration)
{
    ...
    SendUpdatedInfoToMainThreadCacheInternal(/* has_duration */ true, new_duration);
}

base::TimeDelta CrossThreadMediaSourceAttachment::GetRecentMediaTime(MediaSourceTracer* /* tracer */)
{
    attachment_state_lock_.AssertAcquired();
...
bool CrossThreadMediaSourceAttachment::GetElementError(MediaSourceTracer* /* tracer */)
{
    attachment_state_lock_.AssertAcquired();
...
AudioTrackList* CrossThreadMediaSourceAttachment::CreateAudioTrackList(MediaSourceTracer* /* tracer */)
{
...
VideoTrackList* CrossThreadMediaSourceAttachment::CreateVideoTrackList(MediaSourceTracer* /* tracer */)
{
    // TODO(https://crbug.com/878133): Implement this once worker thread can
    // create track lists.
    NOTIMPLEMENTED();
...
void CrossThreadMediaSourceAttachment::AddAudioTrackToMediaElement(MediaSourceTracer* /* tracer */, AudioTrack* /* track */)
{
    // TODO(https://crbug.com/878133): Implement this once worker thread can
    // create tracks.
    NOTIMPLEMENTED();
...
void CrossThreadMediaSourceAttachment::AddVideoTrackToMediaElement(MediaSourceTracer* /* tracer */, VideoTrack* /* track */)
{
    // TODO(https://crbug.com/878133): Implement this once worker thread can
    // create tracks.
    NOTIMPLEMENTED();
...
void CrossThreadMediaSourceAttachment::RemoveAudioTracksFromMediaElement(MediaSourceTracer* /* tracer */, Vector<String> audio_ids, bool enqueue_change_event)
{
    DVLOG(1) << __func__ << " this=" << this << " audio_ids size()=" << audio_ids.size() << ", enqueue_change_event=" << enqueue_change_event;
    attachment_state_lock_.AssertAcquired();
    RemoveTracksFromMediaElementInternal(TrackAddRemovalType::kAudio, std::move(audio_ids), std::move(enqueue_change_event));
}

void CrossThreadMediaSourceAttachment::RemoveVideoTracksFromMediaElement(MediaSourceTracer* /* tracer */, Vector<String> video_ids, bool enqueue_change_event)
{
    DVLOG(1) << __func__ << " this=" << this << " video_ids size()=" << video_ids.size() << ", enqueue_change_event=" << enqueue_change_event;
    attachment_state_lock_.AssertAcquired();
    RemoveTracksFromMediaElementInternal(TrackAddRemovalType::kVideo, std::move(video_ids), std::move(enqueue_change_event));
}

void CrossThreadMediaSourceAttachment::RemoveTracksFromMediaElementInternal(TrackAddRemovalType track_type, Vector<String> track_ids, bool enqueue_change_event)
{
    attachment_state_lock_.AssertAcquired();
    ...
    if (have_ever_started_closing_) {
        DVLOG(1) << __func__ << " this=" << this << ", track_type=" << track_type << ": media element has begun detachment (::Close). no-op";
        return;
    }

    if (media_element_context_destroyed_) {
        DVLOG(1) << __func__ << " this=" << this << ", track_type=" << track_type << ": media element context is destroyed. no-op";
        return;
    }
    ...
    PostCrossThreadTask(*main_runner_, FROM_HERE,
        CrossThreadBindOnce(
            &CrossThreadMediaSourceAttachment::RemoveTracksFromMediaElementOnMainThread, WTF::RetainedRef(this), track_type, track_ids, enqueue_change_event));
}

void CrossThreadMediaSourceAttachment::RemoveTracksFromMediaElementOnMainThread(
    TrackAddRemovalType track_type, Vector<String> track_ids, bool enqueue_change_event)
{
    DCHECK(!track_ids.empty());

    {
        ...
        DVLOG(1) << __func__ << " this=" << this << ", track_type=" << track_type << ", track_ids size()=" << track_ids.size()
        ...
        DVLOG(1) << __func__ << " this=" << this << ", track_type=" << track_type << ": done";
    }
}

void CrossThreadMediaSourceAttachment::AddMainThreadAudioTrackToMediaElement(String id, String kind, String label, String language, bool enabled)
{
    DVLOG(1) << __func__ << " this=" << this << ", id=" << id << ", kind=" << kind << ", label=" << label << ", language=" << language
             << ", enabled=" << enabled;
    attachment_state_lock_.AssertAcquired();
    AddTrackToMediaElementInternal(TrackAddRemovalType::kAudio, std::move(id), std::move(kind), std::move(label), std::move(language), enabled);
}

void CrossThreadMediaSourceAttachment::AddMainThreadVideoTrackToMediaElement(String id, String kind, String label, String language, bool selected)
{
    DVLOG(1) << __func__ << " this=" << this << ", id=" << id << ", kind=" << kind << ", label=" << label << ", language=" << language
             << ", selected=" << selected;
    attachment_state_lock_.AssertAcquired();
    AddTrackToMediaElementInternal(TrackAddRemovalType::kVideo, std::move(id), std::move(kind), std::move(label), std::move(language), selected);
}

void CrossThreadMediaSourceAttachment::AddTrackToMediaElementInternal(
    TrackAddRemovalType track_type, String id, String kind, String label, String language, bool enable_or_select)
{
    attachment_state_lock_.AssertAcquired();
    ...
    PostCrossThreadTask(*main_runner_, FROM_HERE,
        CrossThreadBindOnce(&CrossThreadMediaSourceAttachment::AddTrackToMediaElementOnMainThread, WTF::RetainedRef(this), track_type, id, kind, label,
            language, enable_or_select));
}
    ...
    TrackAddRemovalType track_type, String id, String kind, String label, String language, bool enable_or_select)
{
    ...
    DVLOG(1) << __func__ << " this=" << this << ", track_type=" << track_type << ", id=" << id << ", kind=" << kind << ", label=" << label
             << ", language=" << language << ", enable_or_select=" << enable_or_select;
    ...
    switch (track_type) {
    case TrackAddRemovalType::kAudio: {
        auto* audio_track = MakeGarbageCollected<AudioTrack>(atomic_id, atomic_kind, atomic_label, atomic_language, enable_or_select,
            /*exclusive=*/false);
        attached_element_->audioTracks().Add(audio_track);
        break;
    }
    case TrackAddRemovalType::kVideo: {
        auto* video_track = MakeGarbageCollected<VideoTrack>(atomic_id, atomic_kind, atomic_label, atomic_language, enable_or_select);
...
bool CrossThreadMediaSourceAttachment::FullyAttachedOrSameThread(SourceBufferPassKey) const
{
    attachment_state_lock_.AssertAcquired();
    ...
    return !media_element_context_destroyed_ && attached_element_ && !have_ever_started_closing_;
}

bool CrossThreadMediaSourceAttachment::RunExclusively(bool abort_if_not_fully_attached, RunExclusivelyCB cb)
{
    base::AutoLock lock(attachment_state_lock_);
    ...
    if (abort_if_not_fully_attached && (media_element_context_destroyed_ || !attached_element_ || have_ever_started_closing_)) {
        DVLOG(1) << __func__ << " this=" << this << ", aborting due to not currently being fully attached";
        return false;
    }

    DVLOG(1) << __func__ << " this=" << this << ", running the callback with attachment state lock held";
...
MediaSourceTracer* CrossThreadMediaSourceAttachment::StartAttachingToMediaElement(HTMLMediaElement* element, bool* success)
{
...

    {
        ...
        if (!registered_media_source_) {
            DVLOG(1) << __func__ << " this=" << this << ", element=" << element << ": failed: unregistered already";
            *success = false;
            return nullptr;
        }
        ...
        *success = registered_media_source_->StartWorkerAttachingToMainThreadMediaElement(WrapRefCounted(this));
        if (!*success) {
            DVLOG(1) << __func__ << " this=" << this << ", element=" << element << ": failed: MediaSource possibly in use via another attachment";
            return nullptr;
        }

        attached_element_ = element;
        attached_media_source_ = registered_media_source_;
        main_runner_ = element->GetExecutionContext()->GetTaskRunner(TaskType::kPostedMessage);
        DCHECK(main_runner_->BelongsToCurrentThread());
...
void CrossThreadMediaSourceAttachment::CompleteAttachingToMediaElement(MediaSourceTracer* /* tracer */, std::unique_ptr<WebMediaSource> web_media_source)
{
    DVLOG(1) << __func__ << " this=" << this << ", web_media_source=" << web_media_source.get();
...

    {
        ...
        PostCrossThreadTask(*worker_runner_, FROM_HERE,
            CrossThreadBindOnce(
                &CrossThreadMediaSourceAttachment::CompleteAttachingToMediaElementOnWorkerThread, WTF::RetainedRef(this), std::move(web_media_source)));
    }
}

void CrossThreadMediaSourceAttachment::CompleteAttachingToMediaElementOnWorkerThread(std::unique_ptr<WebMediaSource> web_media_source)
{
    DCHECK(web_media_source);

    {
        ...
        // these cases.
        if (have_ever_started_closing_) {
            DVLOG(1) << __func__ << " this=" << this << ": media element has begun detachment. no-op";
            return;
        }
        if (media_element_context_destroyed_) {
            DVLOG(1) << __func__ << " this=" << this << ": media element context is destroyed. no-op";
            return;
        }
        ...
        attached_media_source_->CompleteAttachingToMediaElement(std::move(web_media_source));
    }
}
...
void CrossThreadMediaSourceAttachment::Close(MediaSourceTracer* /* tracer */)
{
    ...
    // thread. Using WTF::RetainedRef(this) here to ensure we are still alive
    // if/when |worker_runner_| executes the task.
    PostCrossThreadTask(*worker_runner_, FROM_HERE, CrossThreadBindOnce(&CrossThreadMediaSourceAttachment::CloseOnWorkerThread, WTF::RetainedRef(this)));
}
...
void CrossThreadMediaSourceAttachment::CloseOnWorkerThread()
{
    ...
    if (media_source_context_destroyed_) {
        ...
        DVLOG(1) << __func__ << " this=" << this << ": worker context is destroyed. no-op";
        return;
    }
    ...
    if (media_element_context_destroyed_) {
        ...
        DVLOG(1) << __func__ << " this=" << this << ": main context is destroyed. no-op";
        return;
    }
...
WebTimeRanges CrossThreadMediaSourceAttachment::BufferedInternal(MediaSourceTracer* /* tracer */) const
{
    base::AutoLock lock(attachment_state_lock_);
    ...
    if (media_source_context_destroyed_) {
        DVLOG(1) << __func__ << " this=" << this << ": worker context destroyed. Returning 'nothing buffered'";
        return {};
    }
...
WebTimeRanges CrossThreadMediaSourceAttachment::SeekableInternal(MediaSourceTracer* /* tracer */) const
{
    base::AutoLock lock(attachment_state_lock_);
    ...
    // and return an empty range here if the worker's context has been destroyed.
    if (media_source_context_destroyed_) {
        DVLOG(1) << __func__ << " this=" << this << ": worker context destroyed. Returning 'nothing seekable'";
        return {};
    }
...
void CrossThreadMediaSourceAttachment::OnTrackChanged(MediaSourceTracer* /* tracer */, TrackBase* track)
{
    base::AutoLock lock(attachment_state_lock_);
...
void CrossThreadMediaSourceAttachment::OnElementTimeUpdate(double time)
{
...

    {
        ...
        PostCrossThreadTask(*worker_runner_, FROM_HERE,
            CrossThreadBindOnce(&CrossThreadMediaSourceAttachment::UpdateWorkerThreadTimeCache, WTF::RetainedRef(this), base::Seconds(time)));
    }
}

void CrossThreadMediaSourceAttachment::UpdateWorkerThreadTimeCache(base::TimeDelta time)
{
    {
        base::AutoLock lock(attachment_state_lock_);
        DCHECK(!IsMainThread());
        ...
        DVLOG(1) << __func__ << " this=" << this << ": updated recent_element_time_=" << recent_element_time_;
    }
}
...
void CrossThreadMediaSourceAttachment::OnElementError()
{
...

    {
        ...
        PostCrossThreadTask(
            *worker_runner_, FROM_HERE, CrossThreadBindOnce(&CrossThreadMediaSourceAttachment::HandleElementErrorOnWorkerThread, WTF::RetainedRef(this)));
    }
}
...
void CrossThreadMediaSourceAttachment::HandleElementErrorOnWorkerThread()
{
    ...
    DCHECK(!element_has_error_) << "At most one transition to element error per attachment is expected";
...
void CrossThreadMediaSourceAttachment::AssertCrossThreadMutexIsAcquiredForDebugging()
{
    attachment_state_lock_.AssertAcquired();
}
...
void CrossThreadMediaSourceAttachment::SendUpdatedInfoToMainThreadCacheInternal(bool has_new_duration, double new_duration)
{
    attachment_state_lock_.AssertAcquired();
    VerifyCalledWhileContextsAliveForDebugging();
    DCHECK(main_runner_);
    ...
    WebTimeRanges new_buffered = attached_media_source_->BufferedInternal(GetExclusiveKey());
    WebTimeRanges new_seekable = attached_media_source_->SeekableInternal(GetExclusiveKey());

    PostCrossThreadTask(*main_runner_, FROM_HERE,
        CrossThreadBindOnce(&CrossThreadMediaSourceAttachment::UpdateMainThreadInfoCache, WTF::RetainedRef(this), std::move(new_buffered),
            std::move(new_seekable), has_new_duration, new_duration));
}

void CrossThreadMediaSourceAttachment::UpdateMainThreadInfoCache(
    WebTimeRanges new_buffered, WebTimeRanges new_seekable, bool has_new_duration, double new_duration)
{
    base::AutoLock lock(attachment_state_lock_);
    ...
    if (have_ever_started_closing_) {
        DVLOG(1) << __func__ << " this=" << this << ": media element has begun detachment (::Close). no-op";
        return;
    }
    ...
    if (has_new_duration) {
        ...
        if (attached_element_->getReadyState() == HTMLMediaElement::kHaveNothing) {
            DVLOG(1) << __func__ << " this=" << this << ": new_duration=" << new_duration << " and media element readyState is HAVE_NOTHING";
        ...
        DVLOG(1) << __func__ << " this=" << this << ": new_duration=" << new_duration << " and media element readyState is beyond HAVE_NOTHING (no-op)";
    }
}

void CrossThreadMediaSourceAttachment::VerifyCalledWhileContextsAliveForDebugging() const
{
    attachment_state_lock_.AssertAcquired();
    DCHECK(!media_source_context_destroyed_);
    DCHECK(!media_element_context_destroyed_);
