...
{
    return ScriptValue(script_state->GetIsolate(), ToV8Traits<IDLSequence<IDLString>>::ToV8(script_state, kinds_));
}

TrackDefault* TrackDefault::Create(const V8TrackDefaultType& type, const String& language, const String& label, const Vector<String>& kinds,
    const String& byte_stream_track_id, ExceptionState& exception_state)
{
    ...
    if (type.AsEnum() == V8TrackDefaultType::Enum::kAudio) {
        ...
        for (const String& kind : kinds) {
            if (!AudioTrack::IsValidKindKeyword(kind)) {
                exception_state.ThrowTypeError("Invalid audio track default kind '" + kind + "'");
                return nullptr;
            }
        }
        ...
        for (const String& kind : kinds) {
            if (!VideoTrack::IsValidKindKeyword(kind)) {
                exception_state.ThrowTypeError("Invalid video track default kind '" + kind + "'");
                return nullptr;
            }
        }
        ...
        for (const String& kind : kinds) {
            if (!TextTrack::IsValidKindKeyword(kind)) {
                exception_state.ThrowTypeError("Invalid text track default kind '" + kind + "'");
                return nullptr;
            }
        }
    } else {
    ...
    return MakeGarbageCollected<TrackDefault>(type, language, label, kinds, byte_stream_track_id);
}

TrackDefault::~TrackDefault() = default;

TrackDefault::TrackDefault(
    const V8TrackDefaultType& type, const String& language, const String& label, const Vector<String>& kinds, const String& byte_stream_track_id)
    : type_(type)
