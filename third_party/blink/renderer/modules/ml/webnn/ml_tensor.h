...
public:
    ...
    // tensor.
    MLTensor(ExecutionContext* execution_context, MLContext* context, webnn::OperandDescriptor descriptor, webnn::MLTensorUsage usage,
    ...
    bool IsValid() const
    {
        return remote_tensor_.is_bound();
    }
    ...
    ScriptPromise<DOMArrayBuffer> ReadTensorImpl(ScopedMLTrace scoped_trace, ScriptState* script_state, ExceptionState& exception_state);
    ...
    void WriteTensorImpl(base::span<const uint8_t> src_data, ExceptionState& exception_state);

private:
    // The callback of reading from `WebNNTensor` by calling hardware accelerated
    // OS machine learning APIs.
    void OnDidReadTensor(ScopedMLTrace scoped_trace, ScriptPromiseResolver<DOMArrayBuffer>* resolver, base::ElapsedTimer read_tensor_timer,
        webnn::mojom::blink::ReadTensorResultPtr result);
    void OnDidReadTensorByob(ScopedMLTrace scoped_trace, ScriptPromiseResolver<IDLUndefined>* resolver, DOMArrayBufferBase* dst_data,
        base::ElapsedTimer read_tensor_timer, webnn::mojom::blink::ReadTensorResultPtr result);
    void OnDidReadTensorByobView(ScopedMLTrace scoped_trace, ScriptPromiseResolver<IDLUndefined>* resolver, DOMArrayBufferView* dst_data,
        base::ElapsedTimer read_tensor_timer, webnn::mojom::blink::ReadTensorResultPtr result);
    ...
    HeapHashSet<Member<ScriptPromiseResolver<IDLUndefined>>> pending_byob_resolvers_;
};
