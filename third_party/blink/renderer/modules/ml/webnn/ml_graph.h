...
typedef HeapVector<std::pair<String, NotShared<DOMArrayBufferView>>> MLNamedArrayBufferViews;
...
public:
    using NamedOperandDescriptors = HashMap<String, std::optional<webnn::OperandDescriptor>>;
    ...
    MLGraph(ExecutionContext* execution_context, MLContext* context, mojo::PendingAssociatedRemote<webnn::mojom::blink::WebNNGraph> pending_graph_remote,
    ...
    ScriptPromise<MLComputeResult> Compute(ScopedMLTrace scoped_trace, const MLNamedArrayBufferViews& inputs, const MLNamedArrayBufferViews& outputs,
    ...
    // platform graph.
    void Dispatch(ScopedMLTrace scoped_trace, const MLNamedTensors& inputs, const MLNamedTensors& outputs, ExceptionState& exception_state);

    const MLContext* Context() const;

private:
    void DidCompute(ScopedMLTrace scoped_trace, ScriptPromiseResolver<MLComputeResult>* resolver,
        std::unique_ptr<Vector<std::pair<String, ArrayBufferViewInfo>>> inputs_info,
        std::unique_ptr<Vector<std::pair<String, ArrayBufferViewInfo>>> outputs_info, webnn::mojom::blink::ComputeResultPtr mojo_result);
    ...
    HeapHashSet<Member<ScriptPromiseResolver<MLComputeResult>>> pending_resolvers_;
};
