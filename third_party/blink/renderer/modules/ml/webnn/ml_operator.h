...
public:
    using OperationSubKind
        = absl::variant<webnn::mojom::blink::ArgMinMax::Kind, webnn::mojom::blink::Conv2d::Kind, webnn::mojom::blink::ElementWiseBinary::Kind,
            webnn::mojom::blink::ElementWiseUnary::Kind, webnn::mojom::blink::Pool2d::Kind, webnn::mojom::blink::Reduce::Kind, absl::monostate>;

    static String OperatorKindToString(webnn::mojom::blink::Operation::Tag kind, OperationSubKind sub_kind = absl::monostate {});
    ...
    MLOperator(
        MLGraphBuilder* builder, webnn::mojom::blink::Operation::Tag kind, const MLOperatorOptions* options, OperationSubKind sub_kind = absl::monostate {});
    ...
    template <typename MojomKind> MojomKind SubKind() const
    {
    ...
    MLGraphBuilder const* Builder() const
    {
        return builder_.Get();
    }
    ...
    void Connect(HeapVector<Member<MLOperand>> inputs, HeapVector<Member<const MLOperand>> outputs);
...
public:
    MLArgMinMaxOperator(MLGraphBuilder* builder, OperationSubKind sub_kind, const uint32_t axis, const MLArgMinMaxOptions* options);
...
public:
    MLConcatOperator(MLGraphBuilder* builder, const uint32_t axis, const MLOperatorOptions* options);
...
public:
    MLCumulativeSumOperator(MLGraphBuilder* builder, const uint32_t axis, const MLCumulativeSumOptions* options);
...
public:
    MLLstmOperator(MLGraphBuilder* builder, uint32_t steps, uint32_t hidden_size, const MLLstmOptions* options);
...
public:
    MLLstmCellOperator(MLGraphBuilder* builder, uint32_t hidden_size, const MLLstmCellOptions* options);
...
public:
    MLGruOperator(MLGraphBuilder* builder, uint32_t steps, uint32_t hidden_size, const MLOperatorOptions* options);
...
public:
    MLGruCellOperator(MLGraphBuilder* builder, uint32_t hidden_size, const MLGruCellOptions* options);
...
public:
    MLPadOperator(MLGraphBuilder* builder, const Vector<uint32_t>& beginning_padding, const Vector<uint32_t>& ending_padding, const MLPadOptions* options);
...
public:
    MLSoftmaxOperator(MLGraphBuilder* builder, const uint32_t axis, const MLOperatorOptions* options);
...
public:
    MLSplitOperator(MLGraphBuilder* builder, const uint32_t splits, const MLSplitOptions* options);
    MLSplitOperator(MLGraphBuilder* builder, const Vector<uint32_t>& splits, const MLSplitOptions* options);
...
public:
    MLTileOperator(MLGraphBuilder* builder, const Vector<uint32_t>& repetitons, const MLOperatorOptions* options);
