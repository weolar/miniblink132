...
{
    base::UmaHistogramMediumTimes("WebNN.MLTensor.TimingMs.Read", read_tensor_timer.Elapsed());
}

} // namespace

MLTensor::MLTensor(ExecutionContext* execution_context, MLContext* context, webnn::OperandDescriptor descriptor, webnn::MLTensorUsage usage,
    webnn::mojom::blink::CreateTensorSuccessPtr create_tensor_success, base::PassKey<MLContext> /*pass_key*/)
    : ml_context_(context)
...
{
    remote_tensor_.Bind(std::move(create_tensor_success->tensor_remote), execution_context->GetTaskRunner(TaskType::kMachineLearning));
    remote_tensor_.set_disconnect_handler(WTF::BindOnce(&MLTensor::OnConnectionError, WrapWeakPersistent(this)));
}
...
ScriptPromise<DOMArrayBuffer> MLTensor::ReadTensorImpl(ScopedMLTrace scoped_trace, ScriptState* script_state, ExceptionState& exception_state)
{
    // Remote context gets automatically unbound when the execution context
    // destructs.
    if (!remote_tensor_.is_bound()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Tensor has been destroyed or context is lost.");
        return EmptyPromise();
    }
    ...
    remote_tensor_->ReadTensor(
        WTF::BindOnce(&MLTensor::OnDidReadTensor, WrapPersistent(this), std::move(scoped_trace), WrapPersistent(resolver), std::move(read_tensor_timer)));

    return resolver->Promise();
}

ScriptPromise<IDLUndefined> MLTensor::ReadTensorImpl(
    ScopedMLTrace scoped_trace, ScriptState* script_state, DOMArrayBufferBase* dst_data, ExceptionState& exception_state)
{
    // Remote context gets automatically unbound when the execution context
    // destructs.
    if (!remote_tensor_.is_bound()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Invalid tensor state");
        return EmptyPromise();
    }
    ...
    base::ElapsedTimer read_tensor_timer;
    remote_tensor_->ReadTensor(WTF::BindOnce(&MLTensor::OnDidReadTensorByob, WrapPersistent(this), std::move(scoped_trace), WrapPersistent(resolver),
        WrapPersistent(dst_data), std::move(read_tensor_timer)));
    return resolver->Promise();
}

ScriptPromise<IDLUndefined> MLTensor::ReadTensorImpl(
    ScopedMLTrace scoped_trace, ScriptState* script_state, DOMArrayBufferView* dst_data, ExceptionState& exception_state)
{
    // Remote context gets automatically unbound when the execution context
    // destructs.
    if (!remote_tensor_.is_bound()) {
    ...
    base::ElapsedTimer read_tensor_timer;
    remote_tensor_->ReadTensor(WTF::BindOnce(&MLTensor::OnDidReadTensorByobView, WrapPersistent(this), std::move(scoped_trace), WrapPersistent(resolver),
        WrapPersistent(dst_data), std::move(read_tensor_timer)));
    return resolver->Promise();
}

void MLTensor::OnDidReadTensor(ScopedMLTrace scoped_trace, ScriptPromiseResolver<DOMArrayBuffer>* resolver, base::ElapsedTimer read_tensor_timer,
    webnn::mojom::blink::ReadTensorResultPtr result)
{
    ...
    if (result->is_error()) {
        ...
        resolver->RejectWithDOMException(WebNNErrorCodeToDOMExceptionCode(read_tensor_error.code), read_tensor_error.message);
        return;
    }
...
void MLTensor::OnDidReadTensorByob(ScopedMLTrace scoped_trace, ScriptPromiseResolver<IDLUndefined>* resolver, DOMArrayBufferBase* dst_data,
    base::ElapsedTimer read_tensor_timer, webnn::mojom::blink::ReadTensorResultPtr result)
{
...
void MLTensor::OnDidReadTensorByobView(ScopedMLTrace scoped_trace, ScriptPromiseResolver<IDLUndefined>* resolver, DOMArrayBufferView* dst_data,
    base::ElapsedTimer read_tensor_timer, webnn::mojom::blink::ReadTensorResultPtr result)
{
    pending_byob_resolvers_.erase(resolver);
...
void MLTensor::WriteTensorImpl(base::span<const uint8_t> src_data, ExceptionState& exception_state)
{
    // Remote context gets automatically unbound when the execution context
    // destructs.
    if (!remote_tensor_.is_bound()) {
...
void MLTensor::OnConnectionError()
{
    ...
    for (const auto& resolver : pending_resolvers_) {
        resolver->RejectWithDOMException(DOMExceptionCode::kInvalidStateError, "Tensor has been destroyed or context is lost.");
    }
