...
protected:
    ...
    static void TraceDataCb(base::OnceClosure quit_closure, base::trace_event::TraceResultBuffer::SimpleOutput* json_output,
        const scoped_refptr<base::RefCountedString>& json_events_str, bool has_more_events)
    {
    ...
    std::map<std::string, std::pair<int, int>> EndTracing()
    {
        ...
        base::trace_event::TraceLog::GetInstance()->Flush(base::BindRepeating(&ScopedMLTraceTest::TraceDataCb, run_loop.QuitClosure(), &json_data));
        run_loop.Run();

        auto parsed_json = base::JSONReader::ReadAndReturnValueWithError(json_data.json_output);
        CHECK(parsed_json.has_value()) << "JSON parsing failed (" << parsed_json.error().message << ") JSON data:" << std::endl << json_data.json_output;
...
TEST_F(ScopedMLTraceTest, PassScopedTraceToFunc)
{
...

    {
        ...
        ([](ScopedMLTrace trace) { trace.AddStep("Step1"); })(std::move(scoped_trace2));
        auto event_counts = EndTracing();
...
{
    {
        ...
        StartTracing("webnn");
        ScopedMLTrace scoped_trace1("Method1");
        PostCrossThreadTask(*test_task_runner_, FROM_HERE, CrossThreadBindOnce([](ScopedMLTrace trace) {}, std::move(scoped_trace1)));
...

    {
        ...
        StartTracing("webnn");
        ScopedMLTrace scoped_trace2("Method1");
        PostCrossThreadTask(*test_task_runner_, FROM_HERE, CrossThreadBindOnce([](ScopedMLTrace trace) { trace.AddStep("Step1"); }, std::move(scoped_trace2)));
        test_task_runner_->RunUntilIdle();
        auto event_counts = EndTracing();
...

    {
        ...
        PostCrossThreadTask(*test_task_runner_, FROM_HERE, CrossThreadBindOnce([](ScopedMLTrace trace) {}, std::move(scoped_trace3)));
        test_task_runner_->RunUntilIdle();
        auto event_counts = EndTracing();
...

    {
        ...
        PostCrossThreadTask(*test_task_runner_, FROM_HERE, CrossThreadBindOnce([](ScopedMLTrace trace) { trace.AddStep("Step2"); }, std::move(scoped_trace4)));
        test_task_runner_->RunUntilIdle();
        auto event_counts = EndTracing();
...
{
    {
        ...
        StartTracing("webnn");
        ScopedMLTrace scoped_trace1("Method1");
        test_task_runner_->PostTask(FROM_HERE, base::BindOnce([](ScopedMLTrace trace) {}, std::move(scoped_trace1)));
        test_task_runner_->RunUntilIdle();
        auto event_counts = EndTracing();
...

    {
        ...
        StartTracing("webnn");
        ScopedMLTrace scoped_trace2("Method1");
        test_task_runner_->PostTask(FROM_HERE, base::BindOnce([](ScopedMLTrace trace) { trace.AddStep("Step1"); }, std::move(scoped_trace2)));
        test_task_runner_->RunUntilIdle();
        auto event_counts = EndTracing();
