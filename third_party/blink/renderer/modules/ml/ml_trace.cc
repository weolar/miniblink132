...
{
    if (id_.has_value()) {
        TRACE_EVENT_NESTABLE_ASYNC_END0(kWebNNTraceCategory, name_, TRACE_ID_LOCAL(id_.value()));
    }
}
...
ScopedMLTrace::ScopedMLTrace(const char* name, uint64_t id)
    ...
    , id_(id)
{
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0(kWebNNTraceCategory, name_, TRACE_ID_LOCAL(id_.value()));
}
