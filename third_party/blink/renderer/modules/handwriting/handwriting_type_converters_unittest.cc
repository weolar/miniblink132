...
TEST(HandwritingTypeConvertersTest, IdlHandwritingModelConstraintToMojo)
{
    ...
    auto mojo_constraint = mojo::ConvertTo<HandwritingModelConstraintPtr>(idl_constraint);
...
{
    test::TaskEnvironment task_environment;
    auto mojo_drawing_segment = handwriting::mojom::blink::HandwritingDrawingSegment::New();
    mojo_drawing_segment->stroke_index = 123u;
    mojo_drawing_segment->begin_point_index = 10u;
    mojo_drawing_segment->end_point_index = 20u;

    auto* idl_drawing_segment = mojo_drawing_segment.To<blink::HandwritingDrawingSegment*>();
...
TEST(HandwritingTypeConvertersTest, MojoHandwritingPredictionIdl)
{
    ...
    mojo_segment->drawing_segments.push_back(std::move(mojo_drawing_segment));
    auto mojo_prediction = handwriting::mojom::blink::HandwritingPrediction::New();
    ...
    EXPECT_EQ(idl_prediction->segmentationResult()[0]->grapheme(), "The grapheme");
    EXPECT_EQ(idl_prediction->segmentationResult()[0]->beginIndex(), 100u);
    EXPECT_EQ(idl_prediction->segmentationResult()[0]->endIndex(), 200u);
    ASSERT_EQ(idl_prediction->segmentationResult()[0]->drawingSegments().size(), 1u);
    EXPECT_EQ(idl_prediction->segmentationResult()[0]->drawingSegments()[0]->strokeIndex(), 456u);
    EXPECT_EQ(idl_prediction->segmentationResult()[0]->drawingSegments()[0]->beginPointIndex(), 7u);
    EXPECT_EQ(idl_prediction->segmentationResult()[0]->drawingSegments()[0]->endPointIndex(), 8u);
}

TEST(HandwritingTypeConvertersTest, MojoHandwritingRecognizerQueryResultIdl)
{
    test::TaskEnvironment task_environment;
    auto mojo_query_result = handwriting::mojom::blink::QueryHandwritingRecognizerResult::New();
    mojo_query_result->text_alternatives = true;
    mojo_query_result->text_segmentation = true;
    mojo_query_result->hints = handwriting::mojom::blink::HandwritingHintsQueryResult::New();
    mojo_query_result->hints->recognition_type
        = Vector<handwriting::mojom::blink::HandwritingRecognitionType> { handwriting::mojom::blink::HandwritingRecognitionType::kText };
    mojo_query_result->hints->input_type = Vector<handwriting::mojom::blink::HandwritingInputType> { handwriting::mojom::blink::HandwritingInputType::kMouse,
        handwriting::mojom::blink::HandwritingInputType::kStylus, handwriting::mojom::blink::HandwritingInputType::kTouch };
    mojo_query_result->hints->alternatives = true;
    mojo_query_result->hints->text_context = true;

    auto* idl_query_result = mojo::ConvertTo<blink::HandwritingRecognizerQueryResult*>(mojo_query_result);
...
TEST(HandwritingTypeConvertersTest, MojoHandwritingRecognizerQueryResultIdl_FalseValues)
{
    auto mojo_query_result = handwriting::mojom::blink::QueryHandwritingRecognizerResult::New();
    mojo_query_result->text_alternatives = false;
    mojo_query_result->text_segmentation = false;
    mojo_query_result->hints = handwriting::mojom::blink::HandwritingHintsQueryResult::New();
    mojo_query_result->hints->recognition_type = Vector<handwriting::mojom::blink::HandwritingRecognitionType> {};
    mojo_query_result->hints->input_type = Vector<handwriting::mojom::blink::HandwritingInputType> {};
