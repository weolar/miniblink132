...
// static
WindowControlsOverlay* WindowControlsOverlay::FromIfExists(Navigator& navigator)
{
    return Supplement<Navigator>::From<WindowControlsOverlay>(navigator);
}

// static
WindowControlsOverlay* WindowControlsOverlay::windowControlsOverlay(Navigator& navigator)
{
...
WindowControlsOverlay::WindowControlsOverlay(Navigator& navigator)
    ...
    , WindowControlsOverlayChangedDelegate(navigator.DomWindow() ? navigator.DomWindow()->GetFrame() : nullptr)
{
}
...
bool WindowControlsOverlay::visible() const
{
    ...
    return GetSupplementable()->DomWindow()->GetFrame()->IsWindowControlsOverlayVisible();
}
...
DOMRect* WindowControlsOverlay::getTitlebarAreaRect() const
{
    ...
    const auto& rect = GetSupplementable()->DomWindow()->GetFrame()->GetWindowControlsOverlayRect();
    return DOMRect::Create(rect.x(), rect.y(), rect.width(), rect.height());
}

void WindowControlsOverlay::WindowControlsOverlayChanged(const gfx::Rect& rect)
{
    DispatchEvent(*(MakeGarbageCollected<WindowControlsOverlayGeometryChangeEvent>(
        event_type_names::kGeometrychange, DOMRect::Create(rect.x(), rect.y(), rect.width(), rect.height()), !rect.IsEmpty())));
}
