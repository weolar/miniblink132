...
const char kFeaturePolicyBlocked[] = "Access to the feature \"idle-detection\" is disallowed by permissions "
                                     "policy.";

constexpr base::TimeDelta kMinimumThreshold = base::Seconds(60);
constexpr base::TimeDelta kUserInputThreshold = base::Milliseconds(mojom::blink::IdleManager::kUserInputThresholdMs);

static_assert(kMinimumThreshold >= kUserInputThreshold, "Browser threshold can't be less than the minimum allowed by the API");
...
{
    return MakeGarbageCollected<IdleDetector>(ExecutionContext::From(script_state));
}
...
std::optional<V8UserIdleState> IdleDetector::userState() const
{
    ...
    return user_idle_ ? V8UserIdleState(V8UserIdleState::Enum::kIdle) : V8UserIdleState(V8UserIdleState::Enum::kActive);
}
...
std::optional<V8ScreenIdleState> IdleDetector::screenState() const
{
    ...
    return screen_locked_ ? V8ScreenIdleState(V8ScreenIdleState::Enum::kLocked) : V8ScreenIdleState(V8ScreenIdleState::Enum::kUnlocked);
}

// static
ScriptPromise<V8PermissionState> IdleDetector::requestPermission(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!script_state->ContextIsValid()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Execution context is detached.");
        return EmptyPromise();
    }

    auto* context = ExecutionContext::From(script_state);
    return IdleManager::From(context)->RequestPermission(script_state, exception_state);
}

ScriptPromise<IDLUndefined> IdleDetector::start(ScriptState* script_state, const IdleOptions* options, ExceptionState& exception_state)
{
    ...
    if (!context->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kIdleDetection, ReportOptions::kReportOnFailure)) {
        exception_state.ThrowSecurityError(kFeaturePolicyBlocked);
        return EmptyPromise();
    }

    if (receiver_.is_bound()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Idle detector is already started.");
        return EmptyPromise();
    }
    ...
    if (signal_) {
        if (signal_->aborted()) {
            return ScriptPromise<IDLUndefined>::Reject(script_state, signal_->reason(script_state));
        }
        // If there was a previous algorithm, it should have been removed when we
        // reached the "stopped" state.
        DCHECK(!abort_handle_);
        abort_handle_ = signal_->AddAlgorithm(MakeGarbageCollected<StartAbortAlgorithm>(this));
    }

    mojo::PendingRemote<mojom::blink::IdleMonitor> remote;
    receiver_.Bind(remote.InitWithNewPipeAndPassReceiver(), task_runner_);
    receiver_.set_disconnect_handler(WTF::BindOnce(&IdleDetector::OnMonitorDisconnected, WrapWeakPersistent(this)));

    resolver_ = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state, exception_state.GetContext());
    auto promise = resolver_->Promise();
    IdleManager::From(context)->AddMonitor(
        std::move(remote), WTF::BindOnce(&IdleDetector::OnAddMonitor, WrapWeakPersistent(this), WrapPersistent(resolver_.Get())));
    return promise;
}

void IdleDetector::SetTaskRunnerForTesting(scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TickClock* tick_clock)
{
...
void IdleDetector::OnMonitorDisconnected()
{
    ...
    if (resolver_ && (resolver_script_state = resolver_->GetScriptState())
        && IsInParallelAlgorithmRunnable(resolver_->GetExecutionContext(), resolver_script_state)) {
        ScriptState::Scope script_state_scope(resolver_->GetScriptState());
        resolver_->Reject(
            V8ThrowDOMException::CreateOrDie(resolver_->GetScriptState()->GetIsolate(), DOMExceptionCode::kNotSupportedError, "Idle detection not available."));
    }
    Clear();
}

void IdleDetector::OnAddMonitor(ScriptPromiseResolver<IDLUndefined>* resolver, IdleManagerError error, mojom::blink::IdleStatePtr state)
{
    ...
    case IdleManagerError::kPermissionDisabled:
        resolver_->Reject(
            V8ThrowDOMException::CreateOrDie(resolver_script_state->GetIsolate(), DOMExceptionCode::kNotAllowedError, "Idle detection permission denied"));
        resolver_ = nullptr;
        break;
...
void IdleDetector::Update(mojom::blink::IdleStatePtr state, bool is_overridden_by_devtools)
{
    ...
    if (state->idle_time.has_value()) {
        ...
        if (!is_overridden_by_devtools && threshold_ > kUserInputThreshold + *state->idle_time) {
            base::TimeDelta delay = threshold_ - kUserInputThreshold - *state->idle_time;
