...
public:
    ...
    void SetState(std::optional<base::TimeDelta> idle_time, bool screen_locked, bool override = false)
    {
    ...
    // mojom::IdleManager
    void AddMonitor(mojo::PendingRemote<mojom::blink::IdleMonitor> monitor, AddMonitorCallback callback) override
    {
        monitor_.Bind(std::move(monitor));
        std::move(callback).Run(mojom::blink::IdleManagerError::kSuccess, state_.Clone());
    }
...
TEST(IdleDetectorTest, Start)
{
    ...
    auto start_promise = detector->start(scope.GetScriptState(), options, scope.GetExceptionState());
...
TEST(IdleDetectorTest, StartIdleWithLongThreshold)
{
    ...
    auto* detector = IdleDetector::Create(scope.GetScriptState());
    detector->SetTaskRunnerForTesting(task_runner, task_runner->GetMockTickClock());
...
TEST(IdleDetectorTest, LockScreen)
{
    ...
    auto* listener = MakeGarbageCollected<MockEventListener>();
    detector->addEventListener(event_type_names::kChange, listener);
    EXPECT_CALL(*listener, Invoke).WillOnce(WithoutArgs(Invoke([detector, &loop]() {
        EXPECT_EQ(V8UserIdleState::Enum::kActive, detector->userState());
