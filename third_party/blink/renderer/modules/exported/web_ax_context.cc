    ...
    size_t max_node_count, base::TimeDelta timeout, ui::AXTreeUpdate* response, std::set<ui::AXSerializationErrorFlag>* out_error)
{
    ...
    return private_->GetAXObjectCache().SerializeEntireTree(max_node_count, timeout, response, out_error);
}

void WebAXContext::GetImagesToAnnotate(ui::AXTreeUpdate& updates, std::vector<ui::AXNodeData*>& nodes)
{
...
void WebAXContext::AddEventToSerializationQueue(const ui::AXEvent& event, bool immediate_serialization)
{
    if (!HasActiveDocument()) {
        return;
    }
