...
public:
    ...
    ScriptPromise<IDLUndefined> Transform(v8::Local<v8::Value> chunk, TransformStreamDefaultController* controller, ExceptionState& exception_state) override
    {
        ...
        if (input.Is8Bit()) {
            ...
            bool have_output = Encode16BitString(input, high_surrogate, &prefix, &result);
            if (!have_output)
                return ToResolvedUndefinedPromise(script_state_.Get());
        }

        DOMUint8Array* array = CreateDOMUint8ArrayFromTwoStdStringsConcatenated(prefix, result);
        controller->enqueue(script_state_, ScriptValue::From(script_state_, array), exception_state);
    ...
    ScriptPromise<IDLUndefined> Flush(TransformStreamDefaultController* controller, ExceptionState& exception_state) override
    {
        ...
        controller->enqueue(script_state_, ScriptValue::From(script_state_, DOMUint8Array::Create(base::as_byte_span(replacement_character))), exception_state);
...
private:
    ...
    static DOMUint8Array* CreateDOMUint8ArrayFromTwoStdStringsConcatenated(const std::string& string1, const std::string& string2)
    {
    ...
    // value.
    bool Encode16BitString(const String& input, std::optional<UChar> high_surrogate, std::string* prefix, std::string* result)
    {
        ...
        if (high_surrogate.has_value()) {
            ...
            if (code_unit >= 0xDC00 && code_unit <= 0xDFFF) {
                ...
                *prefix = encoder_->Encode(base::span(astral_character), WTF::kNoUnencodables);
...
TextEncoderStream* TextEncoderStream::Create(ScriptState* script_state, ExceptionState& exception_state)
{
...
TextEncoderStream::TextEncoderStream(ScriptState* script_state, ExceptionState& exception_state)
    : transform_(TransformStream::Create(script_state, MakeGarbageCollected<Transformer>(script_state), exception_state))
{
}
