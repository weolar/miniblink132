...
void FetchIcon(ExecutionContext* execution_context, const KURL& icon_url, const gfx::Size& icon_size, ThreadedIconLoader* threaded_icon_loader,
    ThreadedIconLoader::IconCallback callback)
{
    ...
    threaded_icon_loader->Start(execution_context, resource_request, icon_size, std::move(callback));
}

WebVector<Manifest::ImageResource> ToImageResource(ExecutionContext* execution_context, const Vector<mojom::blink::ContentIconDefinitionPtr>& icon_definitions)
{
    ...
    for (const auto& icon_definition : icon_definitions) {
        ...
        image_resource.src = GURL(execution_context->CompleteURL(icon_definition->src));
        image_resource.type = WebString(icon_definition->type).Utf16();
        for (const auto& size : WebIconSizesParser::ParseIconSizes(icon_definition->sizes)) {
...
KURL FindBestIcon(WebVector<Manifest::ImageResource> image_resources, const gfx::Size& icon_size)
{
    return KURL(ManifestIconSelector::FindBestMatchingIcon(image_resources.ReleaseVector(),
        /* ideal_icon_height_in_px= */ icon_size.height(),
        /* minimum_icon_size_in_px= */ 0,
        /* max_width_to_height_ratio= */ icon_size.width() * 1.0f / icon_size.height(), mojom::ManifestImageResource_Purpose::ANY));
}
...
void ContentIndexIconLoader::Start(
    ExecutionContext* execution_context, mojom::blink::ContentDescriptionPtr description, const Vector<gfx::Size>& icon_sizes, IconsCallback callback)
{
    ...
    auto barrier_closure = base::BarrierClosure(icon_sizes.size(),
        WTF::BindOnce(&ContentIndexIconLoader::DidGetIcons, WrapPersistent(this), std::move(description), std::move(icons), std::move(callback)));
    ...
    for (const auto& icon_size : icon_sizes) {
        ...
        FetchIcon(execution_context, icon_url, icon_size, threaded_icon_loader,
            WTF::BindOnce(
                [](base::OnceClosure done_closure, Vector<SkBitmap>* icons_ptr, ThreadedIconLoader* icon_loader, SkBitmap icon, double resize_scale) {
...
void ContentIndexIconLoader::DidGetIcons(mojom::blink::ContentDescriptionPtr description, std::unique_ptr<Vector<SkBitmap>> icons, IconsCallback callback)
{
    DCHECK(icons);
