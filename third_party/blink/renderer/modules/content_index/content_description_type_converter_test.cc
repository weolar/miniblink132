...
const blink::ContentDescription* CreateDescription(const WTF::String& category, const WTF::String& url)
{
...
bool operator==(const Member<ContentIconDefinition>& cid1, const Member<ContentIconDefinition>& cid2)
{
    return cid1->src() == cid2->src() && GetSizesOrNone(cid1) == GetSizesOrNone(cid2) && GetTypeOrNone(cid1) == GetTypeOrNone(cid2);
}

bool operator==(const ContentDescription& cd1, const ContentDescription& cd2)
{
    return cd1.id() == cd2.id() && cd1.title() == cd2.title() && cd1.description() == cd2.description() && cd1.category() == cd2.category()
        && cd1.icons() == cd2.icons() && cd1.url() == cd2.url();
}
...
TEST(ContentDescriptionConversionTest, RoundTrip)
{
    ...
    auto* round_trip_description = mojo_description.To<blink::ContentDescription*>();
...
{
    test::TaskEnvironment task_environment;
    WTF::Vector<WTF::String> categories = { "homepage", "article", "video", "audio" };
    for (const auto& category : categories) {
        auto* description = CreateDescription(category, "https://example.com/");
        auto* round_trip_description = mojom::blink::ContentDescription::From(description).To<blink::ContentDescription*>();
        EXPECT_EQ(*description, *round_trip_description);
    }
}
