...
public:
    static ContentIndexEvent* Create(const AtomicString& type, ContentIndexEventInit* init)
    {
        return MakeGarbageCollected<ContentIndexEvent>(type, init,
            /* observer= */ nullptr);
    }

    ContentIndexEvent(const AtomicString& type, ContentIndexEventInit* init, WaitUntilObserver* observer);
