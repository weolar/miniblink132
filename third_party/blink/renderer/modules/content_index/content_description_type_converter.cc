...
blink::mojom::blink::ContentCategory GetContentCategory(blink::V8ContentCategory::Enum category)
{
    switch (category) {
...
blink::V8ContentCategory::Enum GetContentCategory(blink::mojom::blink::ContentCategory category)
{
    switch (category) {
...
blink::mojom::blink::ContentDescriptionPtr TypeConverter<blink::mojom::blink::ContentDescriptionPtr, const blink::ContentDescription*>::Convert(
    const blink::ContentDescription* description)
{
    ...
    for (const auto& icon : description->icons()) {
        result->icons.push_back(blink::mojom::blink::ContentIconDefinition::New(
            icon->src(), icon->hasSizes() ? icon->sizes() : String(), icon->hasType() ? icon->type() : String()));
    }
...
blink::ContentDescription* TypeConverter<blink::ContentDescription*, blink::mojom::blink::ContentDescriptionPtr>::Convert(
