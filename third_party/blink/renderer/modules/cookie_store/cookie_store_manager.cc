...
mojom::blink::CookieChangeSubscriptionPtr ToBackendSubscription(
    const KURL& default_cookie_url, const CookieStoreGetOptions* subscription, ExceptionState& exception_state)
{
    ...
    if (subscription->hasUrl()) {
        ...
        if (!subscription_url.GetString().StartsWith(default_cookie_url.GetString())) {
    ...
    if (subscription->hasName()) {
        ...
        backend_subscription->match_type = network::mojom::blink::CookieMatchType::STARTS_WITH;
...
CookieStoreGetOptions* ToCookieChangeSubscription(const mojom::blink::CookieChangeSubscription& backend_subscription)
{
...
// static
CookieStoreManager* CookieStoreManager::cookies(ServiceWorkerRegistration& registration)
{
    auto* supplement = Supplement<ServiceWorkerRegistration>::From<CookieStoreManager>(registration);
    if (!supplement) {
    ...
    , default_cookie_url_(DefaultCookieURL(&registration))
{
    ...
    execution_context->GetBrowserInterfaceBroker().GetInterface(
        backend_.BindNewPipeAndPassReceiver(execution_context->GetTaskRunner(TaskType::kDOMManipulation)));
}

ScriptPromise<IDLUndefined> CookieStoreManager::subscribe(
    ScriptState* script_state, const HeapVector<Member<CookieStoreGetOptions>>& subscriptions, ExceptionState& exception_state)
{
    ...
    for (const CookieStoreGetOptions* subscription : subscriptions) {
        mojom::blink::CookieChangeSubscriptionPtr backend_subscription = ToBackendSubscription(default_cookie_url_, subscription, exception_state);
    ...
    backend_->AddSubscriptions(registration_->RegistrationId(), std::move(backend_subscriptions),
        WTF::BindOnce(&CookieStoreManager::OnSubscribeResult, WrapPersistent(this), WrapPersistent(resolver)));
    ...
    ScriptState* script_state, const HeapVector<Member<CookieStoreGetOptions>>& subscriptions, ExceptionState& exception_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state, exception_state.GetContext());
    backend_->RemoveSubscriptions(registration_->RegistrationId(), std::move(backend_subscriptions),
        WTF::BindOnce(&CookieStoreManager::OnSubscribeResult, WrapPersistent(this), WrapPersistent(resolver)));
    return resolver->Promise();
}

ScriptPromise<IDLSequence<CookieStoreGetOptions>> CookieStoreManager::getSubscriptions(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<CookieStoreGetOptions>>>(script_state, exception_state.GetContext());
    backend_->GetSubscriptions(
        registration_->RegistrationId(), WTF::BindOnce(&CookieStoreManager::OnGetSubscriptionsResult, WrapPersistent(this), WrapPersistent(resolver)));
    return resolver->Promise();
}
...
void CookieStoreManager::OnSubscribeResult(ScriptPromiseResolver<IDLUndefined>* resolver, bool backend_success)
{
    if (!backend_success) {
        resolver->RejectWithDOMException(DOMExceptionCode::kUnknownError, "An unknown error occurred while subscribing to cookie changes.");
        return;
    }
    resolver->Resolve();
}
...
void CookieStoreManager::OnGetSubscriptionsResult(
    ScriptPromiseResolver<IDLSequence<CookieStoreGetOptions>>* resolver, Vector<mojom::blink::CookieChangeSubscriptionPtr> backend_result, bool backend_success)
{
    if (!backend_success) {
        resolver->RejectWithDOMException(DOMExceptionCode::kUnknownError, "An unknown error occurred while subscribing to cookie changes.");
        return;
    ...
    for (const auto& backend_subscription : backend_result) {
        CookieStoreGetOptions* subscription = ToCookieChangeSubscription(*backend_subscription);
