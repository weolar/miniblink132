...
class CookieStore final : public EventTarget, public ExecutionContextClient, public network::mojom::blink::CookieChangeListener {
    DEFINE_WRAPPERTYPEINFO();

public:
    CookieStore(ExecutionContext*, HeapMojoRemote<network::mojom::blink::RestrictedCookieManager> backend);
    // Needed because of the
    // mojo::Remote<network::mojom::blink::RestrictedCookieManager>
    ~CookieStore() override;

    ScriptPromise<IDLSequence<CookieListItem>> getAll(ScriptState*, const String& name, ExceptionState&);
    ScriptPromise<IDLSequence<CookieListItem>> getAll(ScriptState*, const CookieStoreGetOptions*, ExceptionState&);
    ScriptPromise<IDLNullable<CookieListItem>> get(ScriptState*, const String& name, ExceptionState&);
    ScriptPromise<IDLNullable<CookieListItem>> get(ScriptState*, const CookieStoreGetOptions*, ExceptionState&);

    ScriptPromise<IDLUndefined> set(ScriptState*, const String& name, const String& value, ExceptionState&);
    ScriptPromise<IDLUndefined> set(ScriptState*, const CookieInit*, ExceptionState&);
    ScriptPromise<IDLUndefined> Delete(ScriptState*, const String& name, ExceptionState&);
    ScriptPromise<IDLUndefined> Delete(ScriptState*, const CookieStoreDeleteOptions*, ExceptionState&);
    ...
    void OnCookieChange(network::mojom::blink::CookieChangeInfoPtr change) override;

protected:
    // EventTarget overrides.
    void AddedEventListener(const AtomicString& event_type, RegisteredEventListener&) final;
...
private:
    ...
    using GetAllForUrlCallback = network::mojom::blink::RestrictedCookieManager::GetAllForUrlCallback;
    void DoRead(ScriptState*, const CookieStoreGetOptions*, GetAllForUrlCallback backend_result_converter, ExceptionState&);
    ...
    static void GetAllForUrlToGetAllResult(
        ScriptPromiseResolver<IDLSequence<CookieListItem>>*, const Vector<network::mojom::blink::CookieWithAccessResultPtr> backend_result);

    // Converts the result of a RestrictedCookieManager::GetAllForUrl mojo call to
    // the promise result expected by CookieStore.get.
    static void GetAllForUrlToGetResult(
        ScriptPromiseResolver<IDLNullable<CookieListItem>>*, const Vector<network::mojom::blink::CookieWithAccessResultPtr> backend_result);

    // Common code in CookieStore::delete and CookieStore::set.
    ScriptPromise<IDLUndefined> DoWrite(ScriptState*, const CookieInit*, ExceptionState&);

    static void OnSetCanonicalCookieResult(ScriptPromiseResolver<IDLUndefined>*, bool backend_result);
    ...
    // is torn down.
    HeapMojoReceiver<network::mojom::blink::CookieChangeListener, CookieStore> change_listener_receiver_;
