...
public:
    ...
    // method.
    static CookieChangeEvent* Create(const AtomicString& type, HeapVector<Member<CookieListItem>> changed, HeapVector<Member<CookieListItem>> deleted)
    {
        return MakeGarbageCollected<CookieChangeEvent>(type, std::move(changed), std::move(deleted));
    }

    // Used by JavaScript, via the V8 bindings.
    static CookieChangeEvent* Create(const AtomicString& type, const CookieChangeEventInit* initializer)
    {
        return MakeGarbageCollected<CookieChangeEvent>(type, initializer);
    }

    CookieChangeEvent();
    CookieChangeEvent(const AtomicString& type, HeapVector<Member<CookieListItem>> changed, HeapVector<Member<CookieListItem>> deleted);
    CookieChangeEvent(const AtomicString& type, const CookieChangeEventInit* initializer);
    ...
    static CookieListItem* ToCookieListItem(const net::CanonicalCookie& canonical_cookie, const network::mojom::blink::CookieEffectiveSameSite&,
    ...
    static void ToEventInfo(const network::mojom::blink::CookieChangeInfoPtr& change_info, HeapVector<Member<CookieListItem>>& changed,
