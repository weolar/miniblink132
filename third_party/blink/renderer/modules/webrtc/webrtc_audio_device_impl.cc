...
void WebRtcAudioDeviceImpl::RenderData(
    media::AudioBus* audio_bus, int sample_rate, base::TimeDelta audio_delay, base::TimeDelta* current_time, const media::AudioGlitchInfo& glitch_info)
{
    TRACE_EVENT("audio", "WebRtcAudioDeviceImpl::RenderData", "sample_rate", sample_rate, "playout_delay (ms)", audio_delay.InMillisecondsF());
    {
        ...
        total_samples_duration_ += media::AudioTimestampHelper::FramesToTime(audio_bus->frames(), sample_rate);
#if DCHECK_IS_ON()
    ...
    static_assert(sizeof(render_buffer_[0]) == kBytesPerSample, "kBytesPerSample and FromInterleaved expect 2 bytes.");
    int64_t elapsed_time_ms = -1;
    int64_t ntp_time_ms = -1;
    int16_t* audio_data = render_buffer_.data();

    TRACE_EVENT_BEGIN1("audio", "VoE::PullRenderData", "frames", frames_per_10_ms);
    audio_transport_callback_->PullRenderData(
        kBytesPerSample * 8, sample_rate, audio_bus->channels(), frames_per_10_ms, audio_data, &elapsed_time_ms, &ntp_time_ms);
    TRACE_EVENT_END2("audio", "VoE::PullRenderData", "elapsed_time_ms", elapsed_time_ms, "ntp_time_ms", ntp_time_ms);
    ...
    audio_bus->FromInterleaved<media::SignedInt16SampleTypeTraits>(audio_data, audio_bus->frames());
...
void WebRtcAudioDeviceImpl::RemoveAudioRenderer(blink::WebRtcAudioRenderer* renderer)
{
...
void WebRtcAudioDeviceImpl::AudioRendererThreadStopped()
{
    ...
    for (WebRtcPlayoutDataSource::Sink* sink : TS_UNCHECKED_READ(playout_sinks_)) {
        sink->OnPlayoutDataSourceChanged();
    }
}

void WebRtcAudioDeviceImpl::SetOutputDeviceForAec(const String& output_device_id)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
    SendLogMessage(base::StringPrintf("%s({output_device_id=%s})", __func__, output_device_id.Utf8().c_str()));
    DVLOG(1) << __func__ << " current id=[" << output_device_id_for_aec_ << "], new id [" << output_device_id << "]";
...
int32_t WebRtcAudioDeviceImpl::RegisterAudioCallback(webrtc::AudioTransport* audio_callback)
{
...
int32_t WebRtcAudioDeviceImpl::Terminate()
{
    ...
    {
        base::AutoLock auto_lock(lock_);
        DCHECK(!renderer_ || !renderer_->IsStarted()) << "The shared audio renderer shouldn't be running";
        capturers_.clear();
    }
...
bool WebRtcAudioDeviceImpl::SetAudioRenderer(blink::WebRtcAudioRenderer* renderer)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
    DCHECK(renderer);
    SendLogMessage(base::StringPrintf("%s()", __func__));
...
void WebRtcAudioDeviceImpl::AddAudioCapturer(ProcessedLocalAudioSource* capturer)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
    SendLogMessage(base::StringPrintf("%s()", __func__));
    DCHECK(capturer);
...
void WebRtcAudioDeviceImpl::RemoveAudioCapturer(ProcessedLocalAudioSource* capturer)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
    SendLogMessage(base::StringPrintf("%s()", __func__));
    DCHECK(capturer);
...
void WebRtcAudioDeviceImpl::AddPlayoutSink(blink::WebRtcPlayoutDataSource::Sink* sink)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
...
void WebRtcAudioDeviceImpl::RemovePlayoutSink(blink::WebRtcPlayoutDataSource::Sink* sink)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
...
std::optional<webrtc::AudioDeviceModule::Stats> WebRtcAudioDeviceImpl::GetStats() const
{
    base::AutoLock auto_lock(lock_);
    return std::optional<webrtc::AudioDeviceModule::Stats>(
        webrtc::AudioDeviceModule::Stats { .synthesized_samples_duration_s = cumulative_glitch_info_.duration.InSecondsF(),
...
base::UnguessableToken WebRtcAudioDeviceImpl::GetAuthorizedDeviceSessionIdForAudioRenderer()
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
    base::AutoLock lock(lock_);
