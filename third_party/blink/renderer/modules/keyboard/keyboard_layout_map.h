...
class KeyboardLayoutMap final : public ScriptWrappable, public Maplike<KeyboardLayoutMap> {
    DEFINE_WRAPPERTYPEINFO();

public:
    explicit KeyboardLayoutMap(const HashMap<String, String>& map);

    const HashMap<String, String>& Map() const
    {
        return layout_map_;
    }
...
private:
    ...
    PairSyncIterable<KeyboardLayoutMap>::IterationSource* CreateIterationSource(ScriptState*, ExceptionState&) override;
    bool GetMapEntry(ScriptState*, const String& key, String& value, ExceptionState&) override;
