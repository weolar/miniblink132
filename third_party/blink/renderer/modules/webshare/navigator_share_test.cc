...
public:
    ...
    {
        receiver_.Bind(mojo::PendingReceiver<mojom::blink::ShareService>(std::move(handle)));
    }

    void set_error(mojom::ShareError value)
    {
        error_ = value;
    }
    ...
    const WTF::Vector<SharedFilePtr>& files() const
    {
        return files_;
    }
    mojom::ShareError error() const
    {
        return error_;
    }

private:
    void Share(const WTF::String& title, const WTF::String& text, const KURL& url, WTF::Vector<SharedFilePtr> files, ShareCallback callback) override
    {
        title_ = title;
        text_ = text;
        url_ = url;
...
public:
    ...
    void Share(const ShareData& share_data)
    {
        ...
        auto promise = NavigatorShare::share(GetScriptState(), *navigator, &share_data, exception_state);
        test::RunPendingTasks();
        EXPECT_EQ(mock_share_service_.error() == mojom::ShareError::OK ? v8::Promise::kFulfilled : v8::Promise::kRejected, promise.V8Promise()->State());
    }

    MockShareService& mock_share_service()
    {
...
protected:
    void SetUp() override
    {
        GetFrame().Loader().CommitNavigation(WebNavigationParams::CreateWithEmptyHTMLForTesting(KURL("https://example.com")), nullptr /* extra_data */);
        test::RunPendingTasks();

        GetFrame().GetBrowserInterfaceBroker().SetBinderForTesting(
            ShareService::Name_, WTF::BindRepeating(&MockShareService::Bind, WTF::Unretained(&mock_share_service_)));
    }
    ...
    void TearDown() override
    {
        ...
        GetFrame().GetBrowserInterfaceBroker().SetBinderForTesting(ShareService::Name_, {});
...
TEST_F(NavigatorShareTest, ShareText)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kWebShareSuccessfulWithoutFiles));
}

File* CreateSampleFile(ExecutionContext* context, const String& file_name, const String& content_type, const String& file_contents)
{
...
TEST_F(NavigatorShareTest, ShareFile)
{
    ...
    files.push_back(CreateSampleFile(ExecutionContext::From(GetScriptState()), file_name, content_type, file_contents));
    ...
    EXPECT_EQ(mock_share_service().files()[0]->name.path(), StringToFilePath(file_name));
    EXPECT_EQ(mock_share_service().files()[0]->blob->GetType(), content_type);
    EXPECT_EQ(mock_share_service().files()[0]->blob->size(), file_contents.length());
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kWebShareContainingFiles));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kWebShareSuccessfulContainingFiles));
}
...
TEST_F(NavigatorShareTest, CancelShare)
{
    ...
    Share(*share_data);
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kWebShareContainingTitle));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kWebShareUnsuccessfulWithoutFiles));
}
...
TEST_F(NavigatorShareTest, CancelShareWithFile)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kWebShareContainingFiles));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kWebShareContainingUrl));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kWebShareUnsuccessfulContainingFiles));
}
