...
TimedCanvasDrawListener::TimedCanvasDrawListener(std::unique_ptr<CanvasCaptureHandler> handler, double frame_rate, ExecutionContext* context)
    : OnRequestCanvasDrawListener(std::move(handler))
    , frame_interval_(base::Seconds(1 / frame_rate))
    , request_frame_timer_(context->GetTaskRunner(TaskType::kInternalMedia), this, &TimedCanvasDrawListener::RequestFrameTimerFired)
{
}
...
// static
TimedCanvasDrawListener* TimedCanvasDrawListener::Create(std::unique_ptr<CanvasCaptureHandler> handler, double frame_rate, ExecutionContext* context)
{
    TimedCanvasDrawListener* listener = MakeGarbageCollected<TimedCanvasDrawListener>(std::move(handler), frame_rate, context);
    listener->request_frame_timer_.StartRepeating(listener->frame_interval_, FROM_HERE);
    return listener;
}
