...
class MODULES_EXPORT HtmlAudioElementCapturerSource final : public blink::MediaStreamAudioSource {
public:
    static HtmlAudioElementCapturerSource* CreateFromWebMediaPlayerImpl(blink::WebMediaPlayer* player, scoped_refptr<base::SingleThreadTaskRunner> task_runner);

    HtmlAudioElementCapturerSource(scoped_refptr<blink::WebAudioSourceProviderImpl> audio_source, scoped_refptr<base::SingleThreadTaskRunner> task_runner);

    HtmlAudioElementCapturerSource(const HtmlAudioElementCapturerSource&) = delete;
    HtmlAudioElementCapturerSource& operator=(const HtmlAudioElementCapturerSource&) = delete;
...
private:
    ...
    void OnAudioBus(std::unique_ptr<media::AudioBus> audio_bus, uint32_t frames_delayed, int sample_rate);
