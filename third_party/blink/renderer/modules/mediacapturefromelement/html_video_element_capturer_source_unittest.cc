...
public:
    ...
    LoadTiming Load(LoadType, const WebMediaPlayerSource&, CorsMode, bool is_cache_disabled) override
    {
    ...
    bool SetSinkId(const WebString& sinkId, WebSetSinkIdCompleteCallback) override
    {
        return false;
    }
    ...
    void SetWouldTaintOrigin(bool taint)
    {
        would_taint_origin_ = taint;
    }
    ...
    void Paint(cc::PaintCanvas* canvas, const gfx::Rect& rect, cc::PaintFlags&) override
    {
        return;
    }
    ...
    scoped_refptr<media::VideoFrame> GetCurrentFrameThenUpdate() override
    {
        ...
        return is_video_opaque_ ? media::VideoFrame::CreateBlackFrame(size_) : media::VideoFrame::CreateTransparentFrame(size_);
    }
...
public:
    HTMLVideoElementCapturerSourceTest()
        ...
        , html_video_capturer_(new HtmlVideoElementCapturerSource(
              web_media_player_->AsWeakPtr(), scheduler::GetSingleThreadTaskRunnerForTesting(), scheduler::GetSingleThreadTaskRunnerForTesting()))
    {
    }
...
TEST_F(HTMLVideoElementCapturerSourceTest, EmptyWebMediaPlayerFailsCapture)
{
    ...
    html_video_capturer_->StartCapture(media::VideoCaptureParams(),
        WTF::BindRepeating(&HTMLVideoElementCapturerSourceTest::OnDeliverFrame, base::Unretained(this)), base::DoNothing(), base::DoNothing(),
        WTF::BindRepeating(&HTMLVideoElementCapturerSourceTest::OnRunning, base::Unretained(this)));
}
...
{
    InSequence s;
    media::VideoCaptureFormats formats = html_video_capturer_->GetPreferredFormats();
    ...
    EXPECT_CALL(*this, DoOnDeliverFrame(_, _)).Times(1).WillOnce(DoAll(SaveArg<0>(&second_frame), RunOnceClosure(std::move(quit_closure))));

    html_video_capturer_->StartCapture(params, WTF::BindRepeating(&HTMLVideoElementCapturerSourceTest::OnDeliverFrame, base::Unretained(this)),
        base::DoNothing(), base::DoNothing(), WTF::BindRepeating(&HTMLVideoElementCapturerSourceTest::OnRunning, base::Unretained(this)));
...
INSTANTIATE_TEST_SUITE_P(All, HTMLVideoElementCapturerSourceTest, ::testing::Bool());
...
TEST_F(HTMLVideoElementCapturerSourceTest, StartAndStopInSameTaskCaptureZeroFrames)
{
    InSequence s;
    media::VideoCaptureFormats formats = html_video_capturer_->GetPreferredFormats();
    ASSERT_EQ(1u, formats.size());
...
TEST_F(HTMLVideoElementCapturerSourceTest, AlphaAndNot)
{
...

    {
        ...
        EXPECT_CALL(*this, DoOnRunning(true)).Times(1);
        EXPECT_CALL(*this, DoOnDeliverFrame(_, _)).WillOnce(DoAll(SaveArg<0>(&frame), RunOnceClosure(std::move(quit_closure))));
        html_video_capturer_->StartCapture(params, WTF::BindRepeating(&HTMLVideoElementCapturerSourceTest::OnDeliverFrame, base::Unretained(this)),
            base::DoNothing(), base::DoNothing(), WTF::BindRepeating(&HTMLVideoElementCapturerSourceTest::OnRunning, base::Unretained(this)));
        run_loop.Run();
