...
public:
    ...
    static std::unique_ptr<CanvasCaptureHandler> CreateCanvasCaptureHandler(LocalFrame* frame, const gfx::Size& size, double frame_rate,
        scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner,
        MediaStreamComponent** component);

    // Return a callback to provide a new frame. See the method
    // CanvasCaptureHandler::GetNewFrameCallback for more details.
    using NewFrameCallback = base::OnceCallback<void(scoped_refptr<media::VideoFrame>)>;
    NewFrameCallback GetNewFrameCallback();
    bool CanDiscardAlpha() const
    {
    ...
    void StartVideoCapture(const media::VideoCaptureParams& params, const VideoCaptureDeliverFrameCB& new_frame_callback,
...
private:
    ...
    CanvasCaptureHandler(LocalFrame* frame, const gfx::Size& size, double frame_rate, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner,
        scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, MediaStreamComponent** component);

    void OnNewFrameCallback(base::ScopedClosureRunner pending_call_tracker, base::TimeTicks this_frame_ticks, const gfx::ColorSpace& color_space,
        scoped_refptr<media::VideoFrame> video_frame);

    void SendFrame(base::TimeTicks this_frame_ticks, const gfx::ColorSpace& color_space, scoped_refptr<media::VideoFrame> video_frame);

    void AddVideoCapturerSourceToVideoTrack(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, LocalFrame* frame,
