...
CanvasCaptureMediaStreamTrack* CanvasCaptureMediaStreamTrack::clone(ExecutionContext* script_state)
{
    MediaStreamComponent* cloned_component = Component()->Clone();
    CanvasCaptureMediaStreamTrack* cloned_track = MakeGarbageCollected<CanvasCaptureMediaStreamTrack>(*this, cloned_component);
...
CanvasCaptureMediaStreamTrack::CanvasCaptureMediaStreamTrack(const CanvasCaptureMediaStreamTrack& track, MediaStreamComponent* component)
    : MediaStreamTrackImpl(track.canvas_element_->GetExecutionContext(), component)
...
CanvasCaptureMediaStreamTrack::CanvasCaptureMediaStreamTrack(
    MediaStreamComponent* component, HTMLCanvasElement* element, ExecutionContext* context, std::unique_ptr<CanvasCaptureHandler> handler)
    : MediaStreamTrackImpl(context, component)
    , canvas_element_(element)
{
    draw_listener_ = MakeGarbageCollected<AutoCanvasDrawListener>(std::move(handler));
    canvas_element_->AddListener(draw_listener_.Get());
}

CanvasCaptureMediaStreamTrack::CanvasCaptureMediaStreamTrack(
    MediaStreamComponent* component, HTMLCanvasElement* element, ExecutionContext* context, std::unique_ptr<CanvasCaptureHandler> handler, double frame_rate)
    : MediaStreamTrackImpl(context, component)
    , canvas_element_(element)
{
    if (frame_rate == 0) {
        draw_listener_ = MakeGarbageCollected<OnRequestCanvasDrawListener>(std::move(handler));
    } else {
        draw_listener_ = TimedCanvasDrawListener::Create(std::move(handler), frame_rate, context);
    }
    canvas_element_->AddListener(draw_listener_.Get());
}
