...
class CanvasCaptureHandlerTest : public TestWithParam<testing::tuple<bool, int, int>> {
public:
    ...
    void SetUp() override
    {
        ...
        canvas_capture_handler_ = CanvasCaptureHandler::CreateCanvasCaptureHandler(
            /*LocalFrame =*/nullptr, gfx::Size(kTestCanvasCaptureWidth, kTestCanvasCaptureHeight), kTestCanvasCaptureFramesPerSecond,
            scheduler::GetSingleThreadTaskRunnerForTesting(), scheduler::GetSingleThreadTaskRunnerForTesting(), &component);
    ...
    MOCK_METHOD2(DoOnDeliverFrame, void(scoped_refptr<media::VideoFrame>, base::TimeTicks));
    void OnDeliverFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks estimated_capture_time)
    {
    ...
    static scoped_refptr<StaticBitmapImage> GenerateTestImage(bool opaque, int width, int height)
    {
        SkImageInfo info = SkImageInfo::MakeN32(width, height, opaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType, SkColorSpace::MakeSRGB());
        SkBitmap testBitmap;
        testBitmap.allocPixels(info);
        testBitmap.eraseARGB(opaque ? 255 : kTestAlphaValue, 30, 60, 200);
        return UnacceleratedStaticBitmapImage::Create(SkImages::RasterFromBitmap(testBitmap));
    }

    void OnVerifyDeliveredFrame(
        bool opaque, int expected_width, int expected_height, scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks estimated_capture_time)
    {
        if (video_frame->format() != media::PIXEL_FORMAT_I420 && video_frame->format() != media::PIXEL_FORMAT_I420A) {
            auto size = video_frame->visible_rect().size();
            auto converted_format = opaque ? media::PIXEL_FORMAT_I420 : media::PIXEL_FORMAT_I420A;
            auto i420_frame = media::VideoFrame::CreateFrame(converted_format, size, gfx::Rect(size), size, video_frame->timestamp());
        ...
        const uint8_t* y_plane = video_frame->visible_data(media::VideoFrame::Plane::kY);
        EXPECT_NEAR(74, y_plane[0], kTestCanvasCaptureFrameColorErrorTolerance);
        const uint8_t* u_plane = video_frame->visible_data(media::VideoFrame::Plane::kU);
        EXPECT_NEAR(193, u_plane[0], kTestCanvasCaptureFrameColorErrorTolerance);
        const uint8_t* v_plane = video_frame->visible_data(media::VideoFrame::Plane::kV);
        EXPECT_NEAR(105, v_plane[0], kTestCanvasCaptureFrameColorErrorTolerance);
        if (!opaque) {
            const uint8_t* a_plane = video_frame->visible_data(media::VideoFrame::Plane::kA);
...
protected:
    VideoCapturerSource* GetVideoCapturerSource(blink::MediaStreamVideoCapturerSource* ms_source)
    {
...
TEST_P(CanvasCaptureHandlerTest, GetFormatsStartAndStop)
{
    ...
    blink::MediaStreamVideoCapturerSource* const ms_source = static_cast<blink::MediaStreamVideoCapturerSource*>(media_stream_source->GetPlatformSource());
    ...
    EXPECT_CALL(*this, DoOnDeliverFrame(_, _)).Times(1).WillOnce(RunOnceClosure(std::move(quit_closure)));
    source->StartCapture(params, base::BindRepeating(&CanvasCaptureHandlerTest::OnDeliverFrame, base::Unretained(this)),
        /*sub_capture_target_version_callback=*/base::DoNothing(),
        /*frame_dropped_callback=*/base::DoNothing(), base::BindRepeating(&CanvasCaptureHandlerTest::OnRunning, base::Unretained(this)));
    copier_->Convert(GenerateTestImage(testing::get<0>(GetParam()), testing::get<1>(GetParam()), testing::get<2>(GetParam())),
        canvas_capture_handler_->CanDiscardAlpha(),
        /*context_provider=*/nullptr, canvas_capture_handler_->GetNewFrameCallback());
    run_loop.Run();
...
TEST_P(CanvasCaptureHandlerTest, VerifyFrame)
{
    ...
    InSequence s;
    VideoCapturerSource* const source = GetVideoCapturerSource(static_cast<blink::MediaStreamVideoCapturerSource*>(component_->Source()->GetPlatformSource()));
    EXPECT_TRUE(source);

    base::RunLoop run_loop;
    EXPECT_CALL(*this, DoOnRunning(true)).Times(1);
    media::VideoCaptureParams params;
    source->StartCapture(params, base::BindRepeating(&CanvasCaptureHandlerTest::OnVerifyDeliveredFrame, base::Unretained(this), opaque_frame, width, height),
        /*sub_capture_target_version_callback=*/base::DoNothing(),
        /*frame_dropped_callback=*/base::DoNothing(), base::BindRepeating(&CanvasCaptureHandlerTest::OnRunning, base::Unretained(this)));
    copier_->Convert(GenerateTestImage(opaque_frame, width, height), canvas_capture_handler_->CanDiscardAlpha(),
        /*context_provider=*/nullptr, canvas_capture_handler_->GetNewFrameCallback());
...
TEST_F(CanvasCaptureHandlerTest, DropAlphaDeliversOpaqueFrame)
{
    ...
    source->StartCapture(params,
        base::BindRepeating(&CanvasCaptureHandlerTest::OnVerifyDeliveredFrame, base::Unretained(this), /*opaque_frame=*/true, width, height),
        /*sub_capture_target_version_callback=*/base::DoNothing(),
        /*frame_dropped_callback=*/base::DoNothing(), base::BindRepeating(&CanvasCaptureHandlerTest::OnRunning, base::Unretained(this)));
    copier_->Convert(GenerateTestImage(/*opaque=*/false, width, height), canvas_capture_handler_->CanDiscardAlpha(),
        /*context_provider=*/nullptr, canvas_capture_handler_->GetNewFrameCallback());
    run_loop.RunUntilIdle();
}
...
{
    InSequence s;
    VideoCapturerSource* source = GetVideoCapturerSource(static_cast<blink::MediaStreamVideoCapturerSource*>(component_->Source()->GetPlatformSource()));
    EXPECT_TRUE(source);
    EXPECT_TRUE(canvas_capture_handler_->NeedsNewFrame());
    source->StopCapture();
...
INSTANTIATE_TEST_SUITE_P(All, CanvasCaptureHandlerTest,
    ::testing::Combine(::testing::Bool(), ::testing::Values(kTestCanvasCaptureFrameEvenSize, kTestCanvasCaptureFrameOddSize),
        ::testing::Values(kTestCanvasCaptureFrameEvenSize, kTestCanvasCaptureFrameOddSize)));
