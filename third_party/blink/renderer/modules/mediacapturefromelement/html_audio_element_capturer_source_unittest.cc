...
public:
    HTMLAudioElementCapturerSourceTest()
        ...
        , audio_source_(new blink::WebAudioSourceProviderImpl(new media::NullAudioSink(blink::scheduler::GetSingleThreadTaskRunnerForTesting()), &media_log_))
    {
    }
    ...
    {
        return static_cast<HtmlAudioElementCapturerSource*>(MediaStreamAudioSource::From(media_stream_source_.Get()));
    }
...
protected:
    ...
    {
        const media::AudioParameters params(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Guess(kNumChannelsForTest),
            kAudioTrackSampleRate /* sample_rate */, kAudioTrackSamplesPerBuffer /* frames_per_buffer */);
        audio_source_->Initialize(params, &fake_callback_);

        auto capture_source = std::make_unique<HtmlAudioElementCapturerSource>(audio_source_, blink::scheduler::GetSingleThreadTaskRunnerForTesting());
        media_stream_source_ = MakeGarbageCollected<MediaStreamSource>(
            String::FromUTF8("audio_id"), MediaStreamSource::kTypeAudio, String::FromUTF8("audio_track"), false /* remote */, std::move(capture_source));
        media_stream_component_ = MakeGarbageCollected<MediaStreamComponentImpl>(
            media_stream_source_->Id(), media_stream_source_, std::make_unique<MediaStreamAudioTrack>(/*is_local=*/true));

        ASSERT_TRUE(source()->ConnectToInitializedTrack(media_stream_component_.Get()));
    }
...
TEST_F(HTMLAudioElementCapturerSourceTest, CaptureAudio)
{
    ...
    EXPECT_CALL(sink,
        OnData(testing::AllOf(testing::Property(&media::AudioBus::channels, kNumChannelsForTest),
                   testing::Property(&media::AudioBus::frames, kAudioTrackSamplesPerBuffer)),
            testing::_))
        .Times(1)
        .WillOnce([&](const auto&, auto) { std::move(quit_closure).Run(); });

    std::unique_ptr<media::AudioBus> bus = media::AudioBus::Create(kNumChannelsForTest, kAudioTrackSamplesPerBuffer);
    InjectAudio(bus.get());
    run_loop.Run();
...
TEST_F(HTMLAudioElementCapturerSourceTest, StartAndStopInSameTaskCapturesZeroFrames)
{
    testing::InSequence s;
...
TEST_F(HTMLAudioElementCapturerSourceTest, TaintedPlayerDeliversMutedAudio)
{
    ...
    EXPECT_CALL(sink,
        OnData(testing::AllOf(testing::Property(&media::AudioBus::channels, kNumChannelsForTest),
                   testing::Property(&media::AudioBus::frames, kAudioTrackSamplesPerBuffer), testing::Property(&media::AudioBus::AreFramesZero, true)),
            testing::_))
        .Times(1)
        .WillOnce([&](const auto&, auto) { std::move(quit_closure).Run(); });
