...
void InstalledAppController::GetInstalledRelatedApps(std::unique_ptr<AppInstalledCallbacks> callbacks)
{
    ...
    ManifestManager::From(*GetSupplementable())
        ->RequestManifest(WTF::BindOnce(&InstalledAppController::OnGetManifestForRelatedApps, WrapPersistent(this), std::move(callbacks)));
}

InstalledAppController* InstalledAppController::From(LocalDOMWindow& window)
{
    InstalledAppController* controller = Supplement<LocalDOMWindow>::From<InstalledAppController>(window);
    if (!controller) {
...
void InstalledAppController::OnGetManifestForRelatedApps(
    std::unique_ptr<AppInstalledCallbacks> callbacks, mojom::blink::ManifestRequestResult result, const KURL& url, mojom::blink::ManifestPtr manifest)
{
    if (!GetSupplementable()->GetFrame()) {
        callbacks->OnError();
        return;
    ...
    if (!provider_.is_bound()) {
        ...
        GetSupplementable()->GetBrowserInterfaceBroker().GetInterface(
            provider_.BindNewPipeAndPassReceiver(GetSupplementable()->GetTaskRunner(TaskType::kMiscPlatformAPI)));
    ...
    provider_->FilterInstalledApps(
        std::move(mojo_related_apps), url, WTF::BindOnce(&InstalledAppController::OnFilterInstalledApps, WrapPersistent(this), std::move(callbacks)));
}

void InstalledAppController::OnFilterInstalledApps(std::unique_ptr<AppInstalledCallbacks> callbacks, Vector<mojom::blink::RelatedApplicationPtr> result)
{
    ...
    ukm::builders::InstalledRelatedApps(window->UkmSourceID()).SetCalled(true).Record(window->UkmRecorder());
