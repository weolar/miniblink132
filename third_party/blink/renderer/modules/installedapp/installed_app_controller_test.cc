...
protected:
    void SetUp() override
    {
        url_test_helpers::RegisterMockedURLLoad(KURL("https://example.com/manifest.json"), "", "");
        ...
        link_manifest->setAttribute(html_names::kHrefAttr, AtomicString("https://example.com/manifest.json"));
...
TEST_F(InstalledAppControllerTest, DestroyContextBeforeCallback)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<RelatedApplication>>>(GetScriptState());
    controller->GetInstalledRelatedApps(std::make_unique<CallbackPromiseAdapter<IDLSequence<RelatedApplication>, void>>(resolver));
