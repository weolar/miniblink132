...
{
    MojoFileSystemAccess* supplement = Supplement<Mojo>::From<MojoFileSystemAccess>(mojo);
    if (!supplement) {
...
// static
MojoHandle* MojoFileSystemAccess::getFileSystemAccessTransferToken(FileSystemFileHandle* fs_handle)
{
    return MakeGarbageCollected<MojoHandle>(mojo::ScopedHandleBase<mojo::Handle>::From(fs_handle->Transfer().PassPipe()));
}
