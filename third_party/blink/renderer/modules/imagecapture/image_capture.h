...
class MODULES_EXPORT ImageCapture final : public ScriptWrappable, public ExecutionContextLifecycleObserver, public mojom::blink::PermissionObserver {
...
public:
    ...
    // retrieved.
    ImageCapture(ExecutionContext*, MediaStreamTrack*, bool pan_tilt_zoom_allowed, base::OnceClosure initialized_callback,
    ...
    MediaStreamTrack* videoStreamTrack() const
    {
        return stream_track_.Get();
    }
    ...
    bool CheckAndApplyMediaTrackConstraintsToSettings(media::mojom::blink::PhotoSettings*, const MediaTrackConstraints*, ScriptPromiseResolverBase*) const;
    void GetMediaTrackCapabilities(MediaTrackCapabilities*) const;
    void SetMediaTrackConstraints(ScriptPromiseResolverBase*, const MediaTrackConstraints* constraints);
    ...
    void UpdateAndCheckMediaTrackSettingsAndCapabilities(base::OnceCallback<void(bool)>);
...
private:
    using PromiseResolverFunction = base::OnceCallback<void(ScriptPromiseResolverBase*)>;
    ...
    void ApplyMediaTrackConstraintSetToSettings(media::mojom::blink::PhotoSettings*, MediaTrackCapabilities* effective_capabilities,
        MediaTrackSettings* effective_settings, const MediaTrackConstraintSet*, MediaTrackConstraintSetType) const;
    // Called by `CheckAndApplyMediaTrackConstraintsToSettings()` check if
    // effective capabilities satisfy a single constraint set.
    bool CheckMediaTrackConstraintSet(const MediaTrackCapabilities* effective_capabilities, const MediaTrackSettings* effective_settings,
        const MediaTrackConstraintSet*, MediaTrackConstraintSetType, ScriptPromiseResolverBase*) const;
    ...
    void OnMojoGetPhotoState(ScriptPromiseResolverBase*, PromiseResolverFunction, bool trigger_take_photo, media::mojom::blink::PhotoStatePtr);
    void OnMojoSetPhotoOptions(ScriptPromiseResolverBase*, bool trigger_take_photo, bool result);
    void OnMojoTakePhoto(ScriptPromiseResolverBase*, media::mojom::blink::BlobPtr);

    // If getUserMedia contains Image Capture constraints, the
    // corresponding settings will be set when image capture is created.
    void SetVideoTrackDeviceSettingsFromTrack(base::OnceClosure callback, media::mojom::blink::PhotoStatePtr photo_state);
    ...
    void OnSetVideoTrackDeviceSettingsFromTrack(base::OnceClosure done_callback, bool result);
    ...
    // retrieved.
    void UpdateMediaTrackSettingsAndCapabilities(base::OnceClosure initialized_callback, media::mojom::blink::PhotoStatePtr photo_state);

    void OnServiceConnectionError();

    void MaybeRejectWithOverconstrainedError(ScriptPromiseResolverBase*, const char* constraint, const char* message) const;
    ...
    void GotPhotoState(base::OnceCallback<void(bool)> callback, media::mojom::blink::PhotoStatePtr photo_state);
    ...
    const std::optional<const char*> GetConstraintWithCapabilityExistenceMismatch(
        const MediaTrackConstraintSet* constraint_set, MediaTrackConstraintSetType) const;
    ...
    HeapMojoReceiver<mojom::blink::PermissionObserver, ImageCapture> permission_observer_receiver_;
