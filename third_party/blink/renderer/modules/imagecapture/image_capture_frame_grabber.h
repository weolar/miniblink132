...
template <typename CallbacksType> class ScopedWebCallbacks {
public:
    using DestructionCallback = base::OnceCallback<void(std::unique_ptr<CallbacksType> callbacks)>;

    ScopedWebCallbacks(std::unique_ptr<CallbacksType> callbacks, DestructionCallback destruction_callback)
...
ScopedWebCallbacks<CallbacksType> MakeScopedWebCallbacks(
    std::unique_ptr<CallbacksType> callbacks, typename ScopedWebCallbacks<CallbacksType>::DestructionCallback destruction_callback)
{
    return ScopedWebCallbacks<CallbacksType>(std::move(callbacks), std::move(destruction_callback));
}

using ImageCaptureGrabFrameCallbacks = CallbackPromiseAdapter<ImageBitmap, void>;
...
public:
    ...
    void GrabFrame(MediaStreamComponent* component, std::unique_ptr<ImageCaptureGrabFrameCallbacks> callbacks,
...
private:
    ...
    void OnSkImage(ScopedWebCallbacks<ImageCaptureGrabFrameCallbacks> callbacks, sk_sp<SkImage> image);
    void OnTimeout();
