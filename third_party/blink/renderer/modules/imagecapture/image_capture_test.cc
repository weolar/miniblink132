...
using ExpectHasExposureModeAndFocusMode = base::StrongAlias<class ExpectHasExposureModeAndFocusModeTag, bool>;
using ExpectHasPanTiltZoom = base::StrongAlias<class ExpectHasPanTiltZoomTag, bool>;
using PopulatePanTiltZoom = base::StrongAlias<class PopulatePanTiltZoomZoomTag, bool>;
...
class CaptureErrorFunction final : public ThenCallable<IDLAny, CaptureErrorFunction> {
public:
    ...
    void React(ScriptState* script_state, ScriptValue value)
    {
        ...
        v8::Local<v8::Object> error_object = value.V8Value()->ToObject(context).ToLocalChecked();

        v8::Local<v8::Value> name = error_object->Get(context, V8String(isolate, "name")).ToLocalChecked();
        name_ = ToCoreString(isolate, name->ToString(context).ToLocalChecked());
        v8::Local<v8::Value> message = error_object->Get(context, V8String(isolate, "message")).ToLocalChecked();
        message_ = ToCoreString(isolate, message->ToString(context).ToLocalChecked());
        v8::Local<v8::Value> constraint = error_object->Get(context, V8String(isolate, "constraint")).ToLocalChecked();
        constraint_ = ToCoreString(isolate, constraint->ToString(context).ToLocalChecked());
    }
...
template <typename T> struct ConstrainWithDictionaryTraits;

template <> struct ConstrainWithDictionaryTraits<bool> {
    using DictionaryType = ConstrainBooleanParameters;
};

template <> struct ConstrainWithDictionaryTraits<double> {
    using DictionaryType = ConstrainDoubleRange;
};

template <> struct ConstrainWithDictionaryTraits<HeapVector<Member<Point2D>>> {
    using DictionaryType = ConstrainPoint2DParameters;
};

template <> struct ConstrainWithDictionaryTraits<String> {
    using DictionaryType = ConstrainDOMStringParameters;
};

template <> struct ConstrainWithDictionaryTraits<Vector<String>> {
    using DictionaryType = ConstrainDOMStringParameters;
};

template <typename T> using ConstrainWithDictionaryType = ConstrainWithDictionaryTraits<T>::DictionaryType;
...
V8UnionStringOrStringSequence* CreateDictionaryMemberValue(const String& value)
{
    return MakeGarbageCollected<V8UnionStringOrStringSequence>(value);
}

V8UnionStringOrStringSequence* CreateDictionaryMemberValue(const Vector<String>& value)
{
    return MakeGarbageCollected<V8UnionStringOrStringSequence>(value);
}
...
template <typename T> const T& CreateDictionaryMemberValue(const T& value)
{
    return value;
}
...
struct ConstrainWithBareValueCreator {
    template <typename T> static const T& Create(const T& bare_value)
    {
        return bare_value;
    }
};
...
struct ConstrainWithEmptyDictionaryCreator {
    template <typename T> static auto* Create(const T&)
    {
...
struct ConstrainWithExactDictionaryCreator {
    template <typename T> static auto* Create(const T& exact)
    {
...
struct ConstrainWithIdealDictionaryCreator {
    template <typename T> static auto* Create(const T& ideal)
    {
        auto* constrain_value = ConstrainWithDictionaryType<T>::Create();
...
struct ConstrainWithMaxDictionaryCreator {
    template <typename T> static auto* Create(const T& max)
    {
        auto* constrain_value = ConstrainWithDictionaryType<T>::Create();
...
struct ConstrainWithMinDictionaryCreator {
    template <typename T> static auto* Create(const T& min)
    {
        auto* constrain_value = ConstrainWithDictionaryType<T>::Create();
...
MediaSettingsRange* CreateMediaSettingsRange(double min, double max, double step)
{
...
void CheckExactValues(const media::mojom::blink::PhotoSettingsPtr& settings, const MediaTrackCapabilities* all_capabilities,
    ExpectHasPanTiltZoom expect_has_pan_tilt_zoom = ExpectHasPanTiltZoom(true),
    ExpectHasExposureModeAndFocusMode expect_has_exposure_mode_and_focus_mode = ExpectHasExposureModeAndFocusMode(true))
{
    EXPECT_TRUE(settings->has_white_balance_mode);
    EXPECT_EQ(settings->white_balance_mode, media::mojom::blink::MeteringMode::CONTINUOUS);
    if (expect_has_exposure_mode_and_focus_mode) {
        EXPECT_TRUE(settings->has_exposure_mode);
        EXPECT_EQ(settings->exposure_mode, media::mojom::blink::MeteringMode::MANUAL);
    ...
    EXPECT_EQ(settings->exposure_compensation, all_capabilities->exposureCompensation()->min() + kExposureCompensationDelta);
    EXPECT_TRUE(settings->has_exposure_time);
    EXPECT_EQ(settings->exposure_time, all_capabilities->exposureTime()->min() + kExposureTimeDelta);
    EXPECT_TRUE(settings->has_color_temperature);
    EXPECT_EQ(settings->color_temperature, all_capabilities->colorTemperature()->min() + kColorTemperatureDelta);
    EXPECT_TRUE(settings->has_iso);
    EXPECT_EQ(settings->iso, all_capabilities->iso()->min() + kIsoDelta);
    EXPECT_TRUE(settings->has_brightness);
    EXPECT_EQ(settings->brightness, all_capabilities->brightness()->min() + kBrightnessDelta);
    EXPECT_TRUE(settings->has_contrast);
    EXPECT_EQ(settings->contrast, all_capabilities->contrast()->min() + kContrastDelta);
    EXPECT_TRUE(settings->has_saturation);
    EXPECT_EQ(settings->saturation, all_capabilities->saturation()->min() + kSaturationDelta);
    EXPECT_TRUE(settings->has_sharpness);
    EXPECT_EQ(settings->sharpness, all_capabilities->sharpness()->min() + kSharpnessDelta);
    EXPECT_TRUE(settings->has_focus_distance);
    EXPECT_EQ(settings->focus_distance, all_capabilities->focusDistance()->min() + kFocusDistanceDelta);
    ...
    EXPECT_EQ(settings->background_blur_mode, media::mojom::blink::BackgroundBlurMode::BLUR);
    EXPECT_TRUE(settings->eye_gaze_correction_mode.has_value());
    EXPECT_EQ(settings->eye_gaze_correction_mode.value(), media::mojom::blink::EyeGazeCorrectionMode::OFF);
    EXPECT_TRUE(settings->has_face_framing_mode);
    EXPECT_EQ(settings->face_framing_mode, media::mojom::blink::MeteringMode::CONTINUOUS);
...
void CheckMaxValues(const media::mojom::blink::PhotoSettingsPtr& settings, const MediaTrackCapabilities* all_capabilities,
    const MediaTrackSettings* default_settings, ExpectHasPanTiltZoom expect_has_pan_tilt_zoom = ExpectHasPanTiltZoom(true))
{
    ...
    EXPECT_EQ(settings->exposure_compensation,
        std::min(all_capabilities->exposureCompensation()->min() + kExposureCompensationDelta, default_settings->exposureCompensation()));
    EXPECT_TRUE(settings->has_exposure_time);
    EXPECT_EQ(settings->exposure_time, std::min(all_capabilities->exposureTime()->min() + kExposureTimeDelta, default_settings->exposureTime()));
    EXPECT_TRUE(settings->has_color_temperature);
    EXPECT_EQ(
        settings->color_temperature, std::min(all_capabilities->colorTemperature()->min() + kColorTemperatureDelta, default_settings->colorTemperature()));
    EXPECT_TRUE(settings->has_iso);
    EXPECT_EQ(settings->iso, std::min(all_capabilities->iso()->min() + kIsoDelta, default_settings->iso()));
    EXPECT_TRUE(settings->has_brightness);
    EXPECT_EQ(settings->brightness, std::min(all_capabilities->brightness()->min() + kBrightnessDelta, default_settings->brightness()));
    EXPECT_TRUE(settings->has_contrast);
    EXPECT_EQ(settings->contrast, std::min(all_capabilities->contrast()->min() + kContrastDelta, default_settings->contrast()));
    EXPECT_TRUE(settings->has_saturation);
    EXPECT_EQ(settings->saturation, std::min(all_capabilities->saturation()->min() + kSaturationDelta, default_settings->saturation()));
    EXPECT_TRUE(settings->has_sharpness);
    EXPECT_EQ(settings->sharpness, std::min(all_capabilities->sharpness()->min() + kSharpnessDelta, default_settings->sharpness()));
    EXPECT_TRUE(settings->has_focus_distance);
    EXPECT_EQ(settings->focus_distance, std::min(all_capabilities->focusDistance()->min() + kFocusDistanceDelta, default_settings->focusDistance()));
    if (expect_has_pan_tilt_zoom) {
        EXPECT_TRUE(settings->has_pan);
        EXPECT_EQ(settings->pan, std::min(all_capabilities->pan()->min() + kPanDelta, default_settings->pan()));
        EXPECT_TRUE(settings->has_tilt);
        EXPECT_EQ(settings->tilt, std::min(all_capabilities->tilt()->min() + kTiltDelta, default_settings->tilt()));
        EXPECT_TRUE(settings->has_zoom);
        EXPECT_EQ(settings->zoom, std::min(all_capabilities->zoom()->min() + kZoomDelta, default_settings->zoom()));
    } else {
        EXPECT_FALSE(settings->has_pan);
        EXPECT_FALSE(settings->has_tilt);
        EXPECT_FALSE(settings->has_zoom);
...
void CheckMinValues(const media::mojom::blink::PhotoSettingsPtr& settings, const MediaTrackCapabilities* all_capabilities,
    const MediaTrackSettings* default_settings, ExpectHasPanTiltZoom expect_has_pan_tilt_zoom = ExpectHasPanTiltZoom(true))
{
    EXPECT_FALSE(settings->has_white_balance_mode);
    EXPECT_FALSE(settings->has_exposure_mode);
    ...
    EXPECT_TRUE(settings->has_exposure_compensation);
    EXPECT_EQ(settings->exposure_compensation,
        std::max(all_capabilities->exposureCompensation()->min() + kExposureCompensationDelta, default_settings->exposureCompensation()));
    EXPECT_TRUE(settings->has_exposure_time);
    EXPECT_EQ(settings->exposure_time, std::max(all_capabilities->exposureTime()->min() + kExposureTimeDelta, default_settings->exposureTime()));
    EXPECT_TRUE(settings->has_color_temperature);
    EXPECT_EQ(
        settings->color_temperature, std::max(all_capabilities->colorTemperature()->min() + kColorTemperatureDelta, default_settings->colorTemperature()));
    EXPECT_TRUE(settings->has_iso);
    EXPECT_EQ(settings->iso, std::max(all_capabilities->iso()->min() + kIsoDelta, default_settings->iso()));
    EXPECT_TRUE(settings->has_brightness);
    EXPECT_EQ(settings->brightness, std::max(all_capabilities->brightness()->min() + kBrightnessDelta, default_settings->brightness()));
    EXPECT_TRUE(settings->has_contrast);
    EXPECT_EQ(settings->contrast, std::max(all_capabilities->contrast()->min() + kContrastDelta, default_settings->contrast()));
    EXPECT_TRUE(settings->has_saturation);
    EXPECT_EQ(settings->saturation, std::max(all_capabilities->saturation()->min() + kSaturationDelta, default_settings->saturation()));
    EXPECT_TRUE(settings->has_sharpness);
    EXPECT_EQ(settings->sharpness, std::max(all_capabilities->sharpness()->min() + kSharpnessDelta, default_settings->sharpness()));
    EXPECT_TRUE(settings->has_focus_distance);
    EXPECT_EQ(settings->focus_distance, std::max(all_capabilities->focusDistance()->min() + kFocusDistanceDelta, default_settings->focusDistance()));
    if (expect_has_pan_tilt_zoom) {
        EXPECT_TRUE(settings->has_pan);
        EXPECT_EQ(settings->pan, std::max(all_capabilities->pan()->min() + kPanDelta, default_settings->pan()));
        EXPECT_TRUE(settings->has_tilt);
        EXPECT_EQ(settings->tilt, std::max(all_capabilities->tilt()->min() + kTiltDelta, default_settings->tilt()));
        EXPECT_TRUE(settings->has_zoom);
        EXPECT_EQ(settings->zoom, std::max(all_capabilities->zoom()->min() + kZoomDelta, default_settings->zoom()));
    } else {
        EXPECT_FALSE(settings->has_pan);
        EXPECT_FALSE(settings->has_tilt);
        EXPECT_FALSE(settings->has_zoom);
...
void CheckNoValues(const media::mojom::blink::PhotoSettingsPtr& settings, size_t expected_points_of_interest_size = 0u)
{
    EXPECT_FALSE(settings->has_white_balance_mode);
    EXPECT_FALSE(settings->has_exposure_mode);
    EXPECT_FALSE(settings->has_focus_mode);
    EXPECT_EQ(settings->points_of_interest.size(), expected_points_of_interest_size);
...
void PopulateConstraintSet(MediaTrackConstraintSet* constraint_set, const MediaTrackCapabilities* all_capabilities,
    PopulatePanTiltZoom populate_pan_tilt_zoom = PopulatePanTiltZoom(true))
{
    constraint_set->setWhiteBalanceMode(
        MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(ConstraintCreator::Create(all_capabilities->whiteBalanceMode()[0])));
    constraint_set->setExposureMode(
        MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(ConstraintCreator::Create(all_capabilities->exposureMode())));
    constraint_set->setFocusMode(
        MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(ConstraintCreator::Create(all_capabilities->focusMode())));
    HeapVector<Member<Point2D>> points_of_interest = { CreatePoint2D(-0.75, -0.25), CreatePoint2D(0.25, 0.75), CreatePoint2D(1.25, 1.75) };
    constraint_set->setPointsOfInterest(
        MakeGarbageCollected<V8UnionConstrainPoint2DParametersOrPoint2DSequence>(ConstraintCreator::Create(points_of_interest)));
    constraint_set->setExposureCompensation(MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(
        ConstraintCreator::Create(all_capabilities->exposureCompensation()->min() + kExposureCompensationDelta)));
    constraint_set->setExposureTime(
        MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(ConstraintCreator::Create(all_capabilities->exposureTime()->min() + kExposureTimeDelta)));
    constraint_set->setColorTemperature(MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(
        ConstraintCreator::Create(all_capabilities->colorTemperature()->min() + kColorTemperatureDelta)));
    constraint_set->setIso(MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(ConstraintCreator::Create(all_capabilities->iso()->min() + kIsoDelta)));
    constraint_set->setBrightness(
        MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(ConstraintCreator::Create(all_capabilities->brightness()->min() + kBrightnessDelta)));
    constraint_set->setContrast(
        MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(ConstraintCreator::Create(all_capabilities->contrast()->min() + kContrastDelta)));
    constraint_set->setSaturation(
        MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(ConstraintCreator::Create(all_capabilities->saturation()->min() + kSaturationDelta)));
    constraint_set->setSharpness(
        MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(ConstraintCreator::Create(all_capabilities->sharpness()->min() + kSharpnessDelta)));
    constraint_set->setFocusDistance(
        MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(ConstraintCreator::Create(all_capabilities->focusDistance()->min() + kFocusDistanceDelta)));
    if (populate_pan_tilt_zoom) {
        constraint_set->setPan(
            MakeGarbageCollected<V8UnionBooleanOrConstrainDoubleRangeOrDouble>(ConstraintCreator::Create(all_capabilities->pan()->min() + kPanDelta)));
        constraint_set->setTilt(
            MakeGarbageCollected<V8UnionBooleanOrConstrainDoubleRangeOrDouble>(ConstraintCreator::Create(all_capabilities->tilt()->min() + kTiltDelta)));
        constraint_set->setZoom(
            MakeGarbageCollected<V8UnionBooleanOrConstrainDoubleRangeOrDouble>(ConstraintCreator::Create(all_capabilities->zoom()->min() + kZoomDelta)));
    }
    constraint_set->setTorch(MakeGarbageCollected<V8UnionBooleanOrConstrainBooleanParameters>(ConstraintCreator::Create(true)));
    constraint_set->setBackgroundBlur(
        MakeGarbageCollected<V8UnionBooleanOrConstrainBooleanParameters>(ConstraintCreator::Create(all_capabilities->backgroundBlur()[0])));
    constraint_set->setEyeGazeCorrection(
        MakeGarbageCollected<V8UnionBooleanOrConstrainBooleanParameters>(ConstraintCreator::Create(all_capabilities->eyeGazeCorrection()[0])));
    constraint_set->setFaceFraming(
        MakeGarbageCollected<V8UnionBooleanOrConstrainBooleanParameters>(ConstraintCreator::Create(all_capabilities->faceFraming()[0])));
    constraint_set->setBackgroundSegmentationMask(
        MakeGarbageCollected<V8UnionBooleanOrConstrainBooleanParameters>(ConstraintCreator::Create(all_capabilities->backgroundSegmentationMask()[0])));
}

class MockMediaStreamComponent : public GarbageCollected<MockMediaStreamComponent>, public MediaStreamComponent {
public:
    ...
    MOCK_METHOD1(SetCreationFrameGetter, void(base::RepeatingCallback<WebLocalFrame*()>));
    MOCK_METHOD1(AddSourceObserver, void(MediaStreamSource::Observer*));
    MOCK_METHOD1(AddSink, void(WebMediaStreamAudioSink*));
    MOCK_METHOD4(AddSink, void(WebMediaStreamSink*, const VideoCaptureDeliverFrameCB&, MediaStreamVideoSink::IsSecure, MediaStreamVideoSink::UsesAlpha));
...
public:
    ImageCaptureTest()
        ...
        , image_capture_(MakeGarbageCollected<ImageCapture>(
              /*execution_context=*/nullptr, track_,
              /*pan_tilt_zoom_allowed=*/true, base::DoNothing(), base::Milliseconds(1)))
    {
    ...
    void SetupTrackMocks(V8TestingScope& scope, bool produce_frame_on_add_sink = true)
    {
        produce_frame_on_add_sink_ = produce_frame_on_add_sink;
        source_ = std::make_unique<MediaStreamVideoCapturerSource>(scope.GetFrame().GetTaskRunner(TaskType::kInternalMediaRealTime), &scope.GetFrame(),
            MediaStreamVideoCapturerSource::SourceStoppedCallback(), std::make_unique<NiceMock<MockVideoCapturerSource>>());
        platform_track_ = std::make_unique<MediaStreamVideoTrack>(source_.get(), WebPlatformMediaStreamSource::ConstraintsOnceCallback(),
            /*enabled=*/true);
        EXPECT_CALL(*component_, GetPlatformTrack).WillRepeatedly(Return(platform_track_.get()));
        EXPECT_CALL(*component_, GetSourceType).WillRepeatedly(Return(MediaStreamSource::kTypeVideo));

        ON_CALL(*component_, AddSink(_, _, _, _))
            .WillByDefault(Invoke([&](WebMediaStreamSink* sink, const VideoCaptureDeliverFrameCB& callback, MediaStreamVideoSink::IsSecure is_secure,
...
public:
    ImageCaptureConstraintTest()
    {
        ...
        default_settings_->setWhiteBalanceMode(all_capabilities_->whiteBalanceMode()[0]);
        ...
        default_settings_->setExposureCompensation(RangeMean(all_capabilities_->exposureCompensation()));
        default_settings_->setExposureTime(RangeMean(all_capabilities_->exposureTime()));
        default_settings_->setColorTemperature(RangeMean(all_capabilities_->colorTemperature()));
        default_settings_->setIso(RangeMean(all_capabilities_->iso()));
        default_settings_->setBrightness(RangeMean(all_capabilities_->brightness()));
        default_settings_->setContrast(RangeMean(all_capabilities_->contrast()));
        default_settings_->setSaturation(RangeMean(all_capabilities_->saturation()));
        default_settings_->setSharpness(RangeMean(all_capabilities_->sharpness()));
        default_settings_->setFocusDistance(RangeMean(all_capabilities_->focusDistance()));
        ...
        DCHECK_LT(all_capabilities_->exposureCompensation()->min() + kExposureCompensationDelta, default_settings_->exposureCompensation());
        // Capabilities and default settings must be chosen so that at least
        ...
        DCHECK_GT(all_capabilities_->focusDistance()->min() + kFocusDistanceDelta, default_settings_->focusDistance());
    }
...
TEST_F(ImageCaptureConstraintTest, ApplyBasicBareValueConstraints)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
    ...
    PopulateConstraintSet<ConstrainWithBareValueCreator>(constraints, all_capabilities_);
    auto settings = media::mojom::blink::PhotoSettings::New();
    // Should apply the constraints to the settings as is and succeed.
    EXPECT_TRUE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, constraints, resolver));
    ...
    constraints->setExposureCompensation(MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(all_capabilities_->exposureCompensation()->max() + 1));
    settings = media::mojom::blink::PhotoSettings::New();
    // Should apply the closest setting within the capability range and succeed.
    EXPECT_TRUE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, constraints, resolver));
    EXPECT_TRUE(settings->has_exposure_compensation);
    EXPECT_EQ(settings->exposure_compensation, all_capabilities_->exposureCompensation()->max());
}
...
TEST_F(ImageCaptureConstraintTest, ApplyBasicExactConstraints)
{
    ...
    //   }
    auto* constraints = MediaTrackConstraints::Create();
    PopulateConstraintSet<ConstrainWithExactDictionaryCreator>(constraints, all_capabilities_);
    auto settings = media::mojom::blink::PhotoSettings::New();
    // Should apply the constraints to the settings as is and succeed.
    EXPECT_TRUE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, constraints, resolver));
    CheckExactValues(settings, all_capabilities_);
...
TEST_F(ImageCaptureConstraintTest, ApplyBasicIdealConstraints)
{
    ...
    PopulateConstraintSet<ConstrainWithIdealDictionaryCreator>(full_constraints, all_capabilities_);
    auto settings = media::mojom::blink::PhotoSettings::New();
    // Should apply the constraints to the settings as is and succeed.
    EXPECT_TRUE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, full_constraints, resolver));
    CheckExactValues(settings, all_capabilities_);

    // Create constraints: {exposureCompensation: {ideal: ...}}
    auto* constraints = MediaTrackConstraints::Create();
    constraints->setExposureCompensation(MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(
        ConstrainWithIdealDictionaryCreator::Create(all_capabilities_->exposureCompensation()->max() + 1)));
    settings = media::mojom::blink::PhotoSettings::New();
    // Should apply the closest setting within the capability range and succeed.
    EXPECT_TRUE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, constraints, resolver));
    EXPECT_TRUE(settings->has_exposure_compensation);
    ...
    image_capture_->SetCapabilitiesForTesting(MakeGarbageCollected<MediaTrackCapabilities>());
    settings = media::mojom::blink::PhotoSettings::New();
    // Shuold ignore ideal constraints without capabilities and succeed.
    EXPECT_TRUE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, full_constraints, resolver));
    CheckNoValues(settings, full_constraints->pointsOfInterest()->GetAsConstrainPoint2DParameters()->ideal().size());
}
...
TEST_F(ImageCaptureConstraintTest, ApplyBasicMaxConstraints)
{
    ...
    //   }
    auto* constraints = MediaTrackConstraints::Create();
    PopulateConstraintSet<ConstrainWithMaxOrEmptyDictionaryCreator>(constraints, all_capabilities_);
    auto settings = media::mojom::blink::PhotoSettings::New();
...
TEST_F(ImageCaptureConstraintTest, ApplyBasicMinConstraints)
{
    ...
    //   }
    auto* constraints = MediaTrackConstraints::Create();
    PopulateConstraintSet<ConstrainWithMinOrEmptyDictionaryCreator>(constraints, all_capabilities_);
    auto settings = media::mojom::blink::PhotoSettings::New();
...
TEST_F(ImageCaptureConstraintTest, ApplyBasicNoConstraints)
{
    ...
    //   }
    auto* constraints = MediaTrackConstraints::Create();
    constraints->setWhiteBalanceMode(MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(Vector<String>()));
    constraints->setExposureMode(
        MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(ConstrainWithExactDictionaryCreator::Create(Vector<String>())));
    constraints->setFocusMode(
        MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(ConstrainWithIdealDictionaryCreator::Create(Vector<String>())));
    constraints->setPointsOfInterest(
        MakeGarbageCollected<V8UnionConstrainPoint2DParametersOrPoint2DSequence>(ConstrainWithExactDictionaryCreator::Create(HeapVector<Member<Point2D>>())));
    auto settings = media::mojom::blink::PhotoSettings::New();
...
TEST_F(ImageCaptureConstraintTest, ApplyBasicOverconstrainedConstraints)
{
    ...
    auto* constraints = MediaTrackConstraints::Create();
    constraints->setWhiteBalanceMode(MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(
        ConstrainWithExactDictionaryCreator::Create(all_non_capabilities_->whiteBalanceMode()[0])));
    auto* capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
    resolver->Promise().Catch(scope.GetScriptState(), capture_error);
    EXPECT_FALSE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, constraints, resolver));
    ...
    constraints = MediaTrackConstraints::Create();
    constraints->setWhiteBalanceMode(MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(
        ConstrainWithExactDictionaryCreator::Create(all_non_capabilities_->whiteBalanceMode())));
    capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
    resolver->Promise().Catch(scope.GetScriptState(), capture_error);
    EXPECT_FALSE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, constraints, resolver));
    scope.PerformMicrotaskCheckpoint(); // Resolve/reject promises.
    EXPECT_TRUE(capture_error->WasCalled());
    ...
    constraints = MediaTrackConstraints::Create();
    constraints->setExposureCompensation(MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(
        ConstrainWithExactDictionaryCreator::Create(all_capabilities_->exposureCompensation()->min() - 1)));
    capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
    resolver->Promise().Catch(scope.GetScriptState(), capture_error);
    EXPECT_FALSE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, constraints, resolver));
    ...
    constraints = MediaTrackConstraints::Create();
    constraints->setExposureCompensation(MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(
        ConstrainWithMaxDictionaryCreator::Create(all_capabilities_->exposureCompensation()->min() - 1)));
    capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
    resolver->Promise().Catch(scope.GetScriptState(), capture_error);
    EXPECT_FALSE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, constraints, resolver));
    ...
    constraints = MediaTrackConstraints::Create();
    constraints->setExposureCompensation(MakeGarbageCollected<V8UnionConstrainDoubleRangeOrDouble>(
        ConstrainWithMinDictionaryCreator::Create(all_capabilities_->exposureCompensation()->max() + 1)));
    capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
    resolver->Promise().Catch(scope.GetScriptState(), capture_error);
    EXPECT_FALSE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, constraints, resolver));
    ...
    constraints = MediaTrackConstraints::Create();
    constraints->setBackgroundBlur(MakeGarbageCollected<V8UnionBooleanOrConstrainBooleanParameters>(
        ConstrainWithExactDictionaryCreator::Create(all_non_capabilities_->backgroundBlur()[0])));
    capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
    resolver->Promise().Catch(scope.GetScriptState(), capture_error);
    EXPECT_FALSE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, constraints, resolver));
...
TEST_F(ImageCaptureConstraintTest, ApplyFirstAdvancedBareValueConstraints)
{
    ...
    PopulateConstraintSet<ConstrainWithBareValueCreator>(constraint_set, all_capabilities_);
    auto* constraints = MediaTrackConstraints::Create();
    ...
    CheckExactValues(settings, all_capabilities_, ExpectHasPanTiltZoom(true), ExpectHasExposureModeAndFocusMode(false));
}
...
TEST_F(ImageCaptureConstraintTest, ApplyFirstAdvancedExactConstraints)
{
    ...
    //   ]}
    auto* constraint_set = MediaTrackConstraintSet::Create();
    PopulateConstraintSet<ConstrainWithExactDictionaryCreator>(constraint_set, all_capabilities_);
    auto* constraints = MediaTrackConstraints::Create();
    constraints->setAdvanced({ constraint_set });
    auto settings = media::mojom::blink::PhotoSettings::New();
    // Should apply the constraints to the settings as is and succeed.
...
TEST_F(ImageCaptureConstraintTest, ApplyFirstAdvancedIdealConstraints)
{
    ...
    //   ]}
    auto* constraint_set = MediaTrackConstraintSet::Create();
    PopulateConstraintSet<ConstrainWithIdealDictionaryCreator>(constraint_set, all_capabilities_);
    auto* constraints = MediaTrackConstraints::Create();
    constraints->setAdvanced({ constraint_set });
    auto settings = media::mojom::blink::PhotoSettings::New();
...
TEST_F(ImageCaptureConstraintTest, ApplyFirstAdvancedOverconstrainedConstraints)
{
    V8TestingScope scope;
    image_capture_->SetExecutionContext(scope.GetExecutionContext());
    const HeapVector<Member<Point2D>> points_of_interest = { CreatePoint2D(0.25, 0.75) };
    auto settings = media::mojom::blink::PhotoSettings::New();

    // Create constraints: {advanced: [{whiteBalanceMode: "..."}]}
    auto* constraint_set = MediaTrackConstraintSet::Create();
    constraint_set->setWhiteBalanceMode(
        MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(all_non_capabilities_->whiteBalanceMode()[0]));
    auto* constraints = MediaTrackConstraints::Create();
    constraints->setAdvanced({ constraint_set });
    auto* capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
    ...
    constraint_set->setPointsOfInterest(MakeGarbageCollected<V8UnionConstrainPoint2DParametersOrPoint2DSequence>(points_of_interest));
    constraint_set->setPan(MakeGarbageCollected<V8UnionBooleanOrConstrainDoubleRangeOrDouble>(false));
    constraints = MediaTrackConstraints::Create();
    constraints->setAdvanced({ constraint_set });
    capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
    ...
    constraint_set = MediaTrackConstraintSet::Create();
    constraint_set->setPointsOfInterest(MakeGarbageCollected<V8UnionConstrainPoint2DParametersOrPoint2DSequence>(points_of_interest));
    constraint_set->setPan(MakeGarbageCollected<V8UnionBooleanOrConstrainDoubleRangeOrDouble>(true));
    constraints = MediaTrackConstraints::Create();
    constraints->setAdvanced({ constraint_set });
    capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
...
TEST_F(ImageCaptureConstraintTest, ApplyAdvancedNoConstraints)
{
    ...
    //   ]}
    auto* constraint_set = MediaTrackConstraintSet::Create();
    constraint_set->setWhiteBalanceMode(MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(Vector<String>()));
    constraint_set->setExposureMode(
        MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(ConstrainWithExactDictionaryCreator::Create(Vector<String>())));
    constraint_set->setFocusMode(
        MakeGarbageCollected<V8UnionConstrainDOMStringParametersOrStringOrStringSequence>(ConstrainWithIdealDictionaryCreator::Create(Vector<String>())));
...
TEST_F(ImageCaptureConstraintTest, ApplySecurityErrorConstraints)
{
    ...
    auto* constraints = MediaTrackConstraints::Create();
    constraints->setPan(MakeGarbageCollected<V8UnionBooleanOrConstrainDoubleRangeOrDouble>(default_settings_->pan()));
    auto* capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
    resolver->Promise().Catch(scope.GetScriptState(), capture_error);
    EXPECT_FALSE(image_capture_->CheckAndApplyMediaTrackConstraintsToSettings(&*settings, constraints, resolver));
    ...
    auto* constraint_set = MediaTrackConstraintSet::Create();
    constraint_set->setTilt(MakeGarbageCollected<V8UnionBooleanOrConstrainDoubleRangeOrDouble>(default_settings_->tilt()));
    constraints = MediaTrackConstraints::Create();
    constraints->setAdvanced({ constraint_set });
    capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
    ...
    constraint_set = MediaTrackConstraintSet::Create();
    constraint_set->setZoom(MakeGarbageCollected<V8UnionBooleanOrConstrainDoubleRangeOrDouble>(default_settings_->zoom()));
    constraints = MediaTrackConstraints::Create();
    constraints->setAdvanced({ MediaTrackConstraintSet::Create(), constraint_set });
    capture_error = MakeGarbageCollected<CaptureErrorFunction>();
    resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(scope.GetScriptState());
