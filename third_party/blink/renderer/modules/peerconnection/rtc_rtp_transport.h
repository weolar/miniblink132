...
public:
    explicit RTCRtpTransportProcessorEvent(RTCRtpTransportProcessor* processor)
        : Event(event_type_names::kRtcrtptransportprocessor, Bubbles::kNo, Cancelable::kNo)
...
public:
    ...
    virtual void SetProcessor(
        CrossThreadWeakHandle<RTCRtpTransportProcessor> rtp_transport_processor_handle, scoped_refptr<base::SequencedTaskRunner> task_runner)
        = 0;
    virtual void SetCustomMaxBitrateBps(uint64_t custom_max_bitrate_bps) = 0;
};

class MODULES_EXPORT RTCRtpTransport : public ScriptWrappable, public ExecutionContextClient {
    DEFINE_WRAPPERTYPEINFO();
...
public:
    ...
    void createProcessor(ScriptState*, DedicatedWorker* worker, const ScriptValue& options, ExceptionState&);
    void createProcessor(ScriptState*, DedicatedWorker* worker, const ScriptValue& options, HeapVector<ScriptValue> transfer, ExceptionState&);
    ...
    void SetProcessorHandle(CrossThreadWeakHandle<RTCRtpTransportProcessor> processor, scoped_refptr<base::SequencedTaskRunner> processor_task_runner);
