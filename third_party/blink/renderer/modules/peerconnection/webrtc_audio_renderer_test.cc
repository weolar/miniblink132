...
const media::AudioParameters kAudioParameters(
    media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), kHardwareSampleRate, kHardwareBufferSize);
...
public:
    ...
    MOCK_METHOD5(RenderData,
        void(media::AudioBus* audio_bus, int sample_rate, base::TimeDelta audio_delay, base::TimeDelta* current_time,
...
public:
    scoped_refptr<media::AudioRendererSink> NewAudioRendererSink(
        blink::WebAudioDeviceSourceType source_type, blink::WebLocalFrame* web_frame, const media::AudioSinkParameters& params) override
    {
        MockNewAudioRendererSink(source_type, web_frame, params);

        mock_sink_ = new media::MockAudioRendererSink(params.device_id,
            params.device_id == kInvalidOutputDeviceId ? media::OUTPUT_DEVICE_STATUS_ERROR_INTERNAL : media::OUTPUT_DEVICE_STATUS_OK, kAudioParameters);
    ...
    MOCK_METHOD3(MockNewAudioRendererSink, void(blink::WebAudioDeviceSourceType, blink::WebLocalFrame*, const media::AudioSinkParameters&));
...
public:
    ...
    void SwitchDeviceCallback(base::RunLoop* loop, media::OutputDeviceStatus result)
    {
...
protected:
    WebRtcAudioRendererTest()
        ...
        , web_view_(blink::WebView::Create(
              ...
              /*page_base_background_color=*/std::nullopt, blink::BrowsingContextGroupInfo::CreateUnique(),
              /*color_provider_colors=*/nullptr,
              /*partitioned_popin_oarams=*/nullptr))
        , web_local_frame_(
              blink::WebLocalFrame::CreateMainFrame(web_view_, &web_local_frame_client_, nullptr, mojo::NullRemote(), LocalFrameToken(), DocumentToken(),
                  /*policy_container=*/nullptr))
    {
        MediaStreamComponentVector dummy_components;
        stream_descriptor_ = MakeGarbageCollected<MediaStreamDescriptor>(String::FromUTF8("new stream"), dummy_components, dummy_components);
    }

    void SetupRenderer(const String& device_id)
    {
        renderer_ = base::MakeRefCounted<WebRtcAudioRenderer>(scheduler::GetSingleThreadTaskRunnerForTesting(), stream_descriptor_, *web_local_frame_,
            base::UnguessableToken::Create(), device_id, base::RepeatingCallback<void()>());

        media::AudioSinkParameters params;
        EXPECT_CALL(*audio_device_factory_platform_, MockNewAudioRendererSink(blink::WebAudioDeviceSourceType::kWebRtc, web_local_frame_.get(), _))
            .Times(testing::AtLeast(1))
            .WillRepeatedly(DoAll(SaveArg<2>(&params), InvokeWithoutArgs([&]() { EXPECT_EQ(params.device_id, device_id.Utf8()); })));

        EXPECT_CALL(*source_.get(), SetOutputDeviceForAec(device_id));
        EXPECT_TRUE(renderer_->Initialize(source_.get()));

        renderer_proxy_ = renderer_->CreateSharedAudioRendererProxy(stream_descriptor_);
    }
    MOCK_METHOD2(CreateAudioCapturerSource, scoped_refptr<media::AudioCapturerSource>(int, const media::AudioSourceParameters&));
    MOCK_METHOD3(CreateFinalAudioRendererSink, scoped_refptr<media::AudioRendererSink>(int, const media::AudioSinkParameters&, base::TimeDelta));
    MOCK_METHOD3(CreateSwitchableAudioRendererSink,
        scoped_refptr<media::SwitchableAudioRendererSink>(blink::WebAudioDeviceSourceType, int, const media::AudioSinkParameters&));
    MOCK_METHOD5(MockCreateAudioRendererSink,
        void(blink::WebAudioDeviceSourceType, int, const base::UnguessableToken&, const std::string&, const std::optional<base::UnguessableToken>&));
    ...
    blink::ScopedTestingPlatformSupport<AudioDeviceFactoryTestingPlatformSupport> audio_device_factory_platform_;
...
TEST_F(WebRtcAudioRendererTest, DISABLED_MultipleRenderers)
{
    ...
    for (int i = 0; i < kNumberOfRendererProxy; ++i) {
        scoped_refptr<MediaStreamAudioRenderer> renderer_proxy = renderer_->CreateSharedAudioRendererProxy(stream_descriptor_);
...
{
    SetupRenderer(kDefaultOutputDeviceId);
    EXPECT_EQ(kDefaultOutputDeviceId, mock_sink()->GetOutputDeviceInfo().device_id());
    renderer_proxy_->Start();
    ...
    EXPECT_CALL(*mock_sink(), CurrentThreadIsRenderingThread()).WillRepeatedly(Return(true));
    ...
    EXPECT_CALL(*source_, RenderData(_, kAudioParameters.sample_rate(), _, _, _)).Times(AnyNumber());
    render_callback()->Render(audio_delay, base::TimeTicks(), glitch_info, dest.get());
...
TEST_F(WebRtcAudioRendererTest, NonDefaultDevice)
{
    ...
    EXPECT_EQ(kOtherOutputDeviceId, mock_sink()->GetOutputDeviceInfo().device_id());
    renderer_proxy_->Start();
...
TEST_F(WebRtcAudioRendererTest, SwitchOutputDevice)
{
    ...
    media::AudioSinkParameters params;
    EXPECT_CALL(*audio_device_factory_platform_, MockNewAudioRendererSink(blink::WebAudioDeviceSourceType::kWebRtc, _, _)).WillOnce(SaveArg<2>(&params));
    EXPECT_CALL(*source_.get(), AudioRendererThreadStopped());
    EXPECT_CALL(*source_.get(), SetOutputDeviceForAec(String::FromUTF8(kOtherOutputDeviceId)));
    EXPECT_CALL(*this, MockSwitchDeviceCallback(media::OUTPUT_DEVICE_STATUS_OK));
    base::RunLoop loop;
    renderer_proxy_->SwitchOutputDevice(kOtherOutputDeviceId, base::BindOnce(&WebRtcAudioRendererTest::SwitchDeviceCallback, base::Unretained(this), &loop));
    loop.Run();
    EXPECT_EQ(kOtherOutputDeviceId, mock_sink()->GetOutputDeviceInfo().device_id());
...
TEST_F(WebRtcAudioRendererTest, SwitchOutputDeviceInvalidDevice)
{
    ...
    base::RunLoop loop;
    renderer_proxy_->SwitchOutputDevice(kInvalidOutputDeviceId, base::BindOnce(&WebRtcAudioRendererTest::SwitchDeviceCallback, base::Unretained(this), &loop));
    loop.Run();
    EXPECT_EQ(kDefaultOutputDeviceId, original_sink->GetOutputDeviceInfo().device_id());
...
{
    renderer_ = base::MakeRefCounted<WebRtcAudioRenderer>(scheduler::GetSingleThreadTaskRunnerForTesting(), stream_descriptor_, *web_local_frame_,
        base::UnguessableToken::Create(), kInvalidOutputDeviceId, base::RepeatingCallback<void()>());
    ...
    EXPECT_EQ(kInvalidOutputDeviceId, mock_sink()->GetOutputDeviceInfo().device_id());
}
