...
// static
const char RTCPeerConnectionController::kSupplementName[] = "RTCPeerConnectionController";

// static
RTCPeerConnectionController& RTCPeerConnectionController::From(Document& document)
{
    RTCPeerConnectionController* supplement = Supplement<Document>::From<RTCPeerConnectionController>(document);
    if (!supplement) {
...
void RTCPeerConnectionController::MaybeReportComplexSdp(ComplexSdpCategory complex_sdp_category)
{
    ...
    ukm::builders::WebRTC_ComplexSdp(source_id).SetCategory(static_cast<int64_t>(complex_sdp_category)).Record(GetSupplementable()->UkmRecorder());
}
