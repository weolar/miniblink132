...
class MODULES_EXPORT PeerConnectionDependencyFactory : public GarbageCollected<PeerConnectionDependencyFactory>,
                                                       public Supplement<ExecutionContext>,
                                                       public ExecutionContextLifecycleObserver {
    USING_PRE_FINALIZER(PeerConnectionDependencyFactory, CleanupPeerConnectionFactory);

public:
    static const char kSupplementName[];

    static PeerConnectionDependencyFactory& From(ExecutionContext& context);
    PeerConnectionDependencyFactory(ExecutionContext& context, base::PassKey<PeerConnectionDependencyFactory>);

    PeerConnectionDependencyFactory(const PeerConnectionDependencyFactory&) = delete;
    PeerConnectionDependencyFactory& operator=(const PeerConnectionDependencyFactory&) = delete;
    ...
    // the correct threads.
    virtual scoped_refptr<webrtc::VideoTrackSourceInterface> CreateVideoTrackSourceProxy(webrtc::VideoTrackSourceInterface* source);
    ...
    virtual rtc::scoped_refptr<webrtc::PeerConnectionInterface> CreatePeerConnection(const webrtc::PeerConnectionInterface::RTCConfiguration& config,
        blink::WebLocalFrame* web_frame, webrtc::PeerConnectionObserver* observer, ExceptionState& exception_state, RTCRtpTransport* rtp_transport);
    ...
    virtual std::unique_ptr<webrtc::AsyncDnsResolverFactoryInterface> CreateAsyncDnsResolverFactory();

    // Creates a libjingle representation of an ice candidate.
    virtual webrtc::IceCandidateInterface* CreateIceCandidate(const String& sdp_mid, int sdp_mline_index, const String& sdp);
    ...
    virtual std::unique_ptr<webrtc::RtpCapabilities> GetSenderCapabilities(const String& kind);
    virtual std::unique_ptr<webrtc::RtpCapabilities> GetReceiverCapabilities(const String& kind);
    ...
    virtual scoped_refptr<base::SingleThreadTaskRunner> GetWebRtcNetworkTaskRunner();

    virtual scoped_refptr<base::SingleThreadTaskRunner> GetWebRtcSignalingTaskRunner();
...
protected:
    ...
    virtual const rtc::scoped_refptr<webrtc::PeerConnectionFactoryInterface>& GetPcFactory();
...
private:
    ...
    void InitializeSignalingThread(const gfx::ColorSpace& render_color_space, media::GpuVideoAcceleratorFactories* gpu_factories,
    ...
    void CreateIpcNetworkManagerOnNetworkThread(base::WaitableEvent* event, std::unique_ptr<MdnsResponderAdapter> mdns_responder);
    static void DeleteIpcNetworkManager(std::unique_ptr<IpcNetworkManager> network_manager, base::WaitableEvent* event);
    void CleanupPeerConnectionFactory();

    void DoGetDevtoolsToken(base::OnceCallback<void(std::optional<base::UnguessableToken>)> then);
