...
V8RTCRtpTransceiverDirection::Enum TransceiverDirectionToEnum(const webrtc::RtpTransceiverDirection& direction)
{
    switch (direction) {
...
std::optional<V8RTCRtpTransceiverDirection::Enum> OptionalTransceiverDirectionToEnum(const std::optional<webrtc::RtpTransceiverDirection>& direction)
{
    if (!direction) {
        return std::nullopt;
    }
...
bool TransceiverDirectionFromEnum(V8RTCRtpTransceiverDirection::Enum direction, std::optional<webrtc::RtpTransceiverDirection>* direction_out)
{
    switch (direction) {
...
bool OptionalTransceiverDirectionFromEnumWithStopped(
    V8RTCRtpTransceiverDirection::Enum direction, std::optional<webrtc::RtpTransceiverDirection>* direction_out)
{
...
webrtc::RtpTransceiverInit ToRtpTransceiverInit(ExecutionContext* context, const RTCRtpTransceiverInit* init, const String& kind)
{
    webrtc::RtpTransceiverInit webrtc_init;
    std::optional<webrtc::RtpTransceiverDirection> direction;
    if (init->hasDirection() && TransceiverDirectionFromEnum(init->direction().AsEnum(), &direction) && direction) {
    ...
    for (const auto& encoding : init->sendEncodings()) {
        webrtc_init.send_encodings.push_back(ToRtpEncodingParameters(context, encoding, kind));
    }
    return webrtc_init;
}

RTCRtpTransceiver::RTCRtpTransceiver(
    RTCPeerConnection* pc, std::unique_ptr<RTCRtpTransceiverPlatform> platform_transceiver, RTCRtpSender* sender, RTCRtpReceiver* receiver)
    : pc_(pc)
...
void RTCRtpTransceiver::setDirection(const V8RTCRtpTransceiverDirection& direction, ExceptionState& exception_state)
{
    std::optional<webrtc::RtpTransceiverDirection> webrtc_direction;
    if (!TransceiverDirectionFromEnum(direction.AsEnum(), &webrtc_direction) || !webrtc_direction) {
        exception_state.ThrowTypeError("Invalid RTCRtpTransceiverDirection.");
        return;
    }
    if (pc_->IsClosed()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The peer connection is closed.");
        return;
    }
    if (current_direction_ == V8RTCRtpTransceiverDirection::Enum::kStopped) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The transceiver is stopped.");
        return;
    }
    if (direction_ == V8RTCRtpTransceiverDirection::Enum::kStopped) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The transceiver is stopping.");
        return;
    }
...
std::optional<V8RTCRtpTransceiverDirection> RTCRtpTransceiver::currentDirection() const
{
...
void RTCRtpTransceiver::UpdateMembers()
{
    ...
    current_direction_ = OptionalTransceiverDirectionToEnum(platform_transceiver_->CurrentDirection());
...
void RTCRtpTransceiver::OnTransceiverStopped()
{
    ...
    mid_ = String();
    direction_ = TransceiverDirectionToEnum(webrtc::RtpTransceiverDirection::kStopped);
    current_direction_ = TransceiverDirectionToEnum(webrtc::RtpTransceiverDirection::kStopped);
...
std::optional<webrtc::RtpTransceiverDirection> RTCRtpTransceiver::fired_direction() const
{
...
bool RTCRtpTransceiver::DirectionHasSend() const
{
    ...
    return direction == webrtc::RtpTransceiverDirection::kSendRecv || direction == webrtc::RtpTransceiverDirection::kSendOnly;
}

bool RTCRtpTransceiver::DirectionHasRecv() const
{
    auto direction = platform_transceiver_->Direction();
    return direction == webrtc::RtpTransceiverDirection::kSendRecv || direction == webrtc::RtpTransceiverDirection::kRecvOnly;
}

bool RTCRtpTransceiver::FiredDirectionHasRecv() const
{
    return fired_direction_
        && (*fired_direction_ == webrtc::RtpTransceiverDirection::kSendRecv || *fired_direction_ == webrtc::RtpTransceiverDirection::kRecvOnly);
}
...
void RTCRtpTransceiver::setCodecPreferences(const HeapVector<Member<RTCRtpCodecCapability>>& codecs, ExceptionState& exception_state)
{
    ...
    for (const auto& codec : codecs) {
        ...
        if (slash_position == WTF::kNotFound) {
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidModificationError, "Invalid codec");
            return;
        }
        ...
        if (codec->hasSdpFmtpLine()) {
            ...
            if (sdpFmtpLine.find('=') == WTF::kNotFound) {
                ...
                for (const auto& parameter : parameters) {
                    ...
                    webrtc_codec.parameters.emplace(parameter_name.Ascii(), parameter_value.Ascii());
                }
            }
        }
    }
    ...
    if (!result.ok()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidModificationError, result.message());
    }
}

void RTCRtpTransceiver::setHeaderExtensionsToNegotiate(const HeapVector<Member<RTCRtpHeaderExtensionCapability>>& extensions, ExceptionState& exception_state)
{
    Vector<webrtc::RtpHeaderExtensionCapability> webrtc_hdr_exts;
    auto webrtc_offered_exts = platform_transceiver_->GetHeaderExtensionsToNegotiate();
    int id = 1;
    for (const auto& hdr_ext : extensions) {
        ...
        std::optional<webrtc::RtpTransceiverDirection> direction;
        if (!OptionalTransceiverDirectionFromEnumWithStopped(hdr_ext->direction().AsEnum(), &direction) || !direction) {
            exception_state.ThrowTypeError("Invalid RTCRtpTransceiverDirection.");
            return;
        }
        const int id_to_store = direction ? id++ : 0;
        webrtc_hdr_exts.emplace_back(hdr_ext->uri().Ascii(), id_to_store, *direction);
    }
    webrtc::RTCError status = platform_transceiver_->SetHeaderExtensionsToNegotiate(std::move(webrtc_hdr_exts));
    if (status.type() == webrtc::RTCErrorType::UNSUPPORTED_PARAMETER) {
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, status.message());
        return;
...
HeapVector<Member<RTCRtpHeaderExtensionCapability>> RTCRtpTransceiver::getHeaderExtensionsToNegotiate() const
{
...
HeapVector<Member<RTCRtpHeaderExtensionCapability>> RTCRtpTransceiver::getNegotiatedHeaderExtensions() const
{
