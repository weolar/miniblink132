...
namespace {
V8RTCSctpTransportState::Enum TransportStateToEnum(webrtc::SctpTransportState state)
{
    switch (state) {
...
std::unique_ptr<SctpTransportProxy> CreateProxy(ExecutionContext* context, webrtc::SctpTransportInterface* native_transport,
    SctpTransportProxy::Delegate* delegate, scoped_refptr<base::SingleThreadTaskRunner> main_thread, scoped_refptr<base::SingleThreadTaskRunner> worker_thread)
{
    DCHECK(main_thread);
    DCHECK(worker_thread);
    LocalFrame* frame = To<LocalDOMWindow>(context)->GetFrame();
    DCHECK(frame);
    return SctpTransportProxy::Create(*frame, main_thread, worker_thread, rtc::scoped_refptr<webrtc::SctpTransportInterface>(native_transport), delegate);
}

} // namespace

RTCSctpTransport::RTCSctpTransport(ExecutionContext* context, rtc::scoped_refptr<webrtc::SctpTransportInterface> native_transport)
    : RTCSctpTransport(
        context, native_transport, context->GetTaskRunner(TaskType::kNetworking), PeerConnectionDependencyFactory::From(*context).GetWebRtcNetworkTaskRunner())
{
}

RTCSctpTransport::RTCSctpTransport(ExecutionContext* context, rtc::scoped_refptr<webrtc::SctpTransportInterface> native_transport,
    scoped_refptr<base::SingleThreadTaskRunner> main_thread, scoped_refptr<base::SingleThreadTaskRunner> worker_thread)
    : ExecutionContextClient(context)
    , current_state_(webrtc::SctpTransportState::kNew)
    , native_transport_(native_transport)
    , proxy_(CreateProxy(context, native_transport.get(), this, main_thread, worker_thread))
{
}
...
rtc::scoped_refptr<webrtc::SctpTransportInterface> RTCSctpTransport::native_transport()
{
