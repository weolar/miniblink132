...
TEST(RTCIceCandidateTest, Url)
{
    RTCIceCandidate* candidate(
        RTCIceCandidate::Create(MakeGarbageCollected<RTCIceCandidatePlatform>(kUdpRelayCandidateStr, kMid, kSdpMLineIndex, kUsernameFragment, kUrl)));
...
TEST(RTCIceCandidateTest, RelayProtocol)
{
    RTCIceCandidate* candidate(
        RTCIceCandidate::Create(MakeGarbageCollected<RTCIceCandidatePlatform>(kUdpRelayCandidateStr, kMid, kSdpMLineIndex, kUsernameFragment, kUrl)));
    EXPECT_EQ(candidate->relayProtocol(), V8RTCIceServerTransportProtocol::Enum::kUdp);
}
