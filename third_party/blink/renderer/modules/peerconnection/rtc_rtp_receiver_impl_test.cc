...
public:
    ...
    std::unique_ptr<RTCRtpReceiverImpl> CreateReceiver(
        scoped_refptr<webrtc::MediaStreamTrackInterface> webrtc_track, bool require_encoded_insertable_streams = false)
    {
        ...
        dependency_factory_->GetWebRtcSignalingTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&RTCRtpReceiverImplTest::CreateReceiverOnSignalingThread, base::Unretained(this), std::move(webrtc_track),
                base::Unretained(&track_ref), base::Unretained(&run_loop)));
        run_loop.Run();
        DCHECK(mock_webrtc_receiver_);
        DCHECK(track_ref);
        blink::RtpReceiverState state(main_thread_, dependency_factory_->GetWebRtcSignalingTaskRunner(), mock_webrtc_receiver_.get(), std::move(track_ref), {});
        state.Initialize();
        return std::make_unique<RTCRtpReceiverImpl>(peer_connection_, std::move(state), require_encoded_insertable_streams,
...
protected:
    void CreateReceiverOnSignalingThread(scoped_refptr<webrtc::MediaStreamTrackInterface> webrtc_track,
...
{
    scoped_refptr<blink::MockWebRtcAudioTrack> webrtc_track = blink::MockWebRtcAudioTrack::Create("webrtc_track");
    receiver_ = CreateReceiver(webrtc_track);
    EXPECT_FALSE(!receiver_->Track());
    EXPECT_EQ(receiver_->Track()->Id().Utf8(), webrtc_track->id());
    EXPECT_EQ(receiver_->state().track_ref()->webrtc_track().get(), webrtc_track.get());
...
TEST_F(RTCRtpReceiverImplTest, ShallowCopy)
{
    ...
    EXPECT_EQ(copy->state().track_ref()->webrtc_track().get(), webrtc_track.get());
...
TEST_F(RTCRtpReceiverImplTest, GetStats)
{
    ...
    webrtc_report->AddStats(std::make_unique<webrtc::RTCInboundRtpStreamStats>("stats-id", webrtc::Timestamp::Micros(1234)));
