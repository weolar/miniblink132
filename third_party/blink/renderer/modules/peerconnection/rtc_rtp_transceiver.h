...
webrtc::RtpTransceiverInit ToRtpTransceiverInit(ExecutionContext* context, const RTCRtpTransceiverInit*, const String& kind);

class RTCRtpTransceiver final : public ScriptWrappable {
    DEFINE_WRAPPERTYPEINFO();

public:
    RTCRtpTransceiver(RTCPeerConnection* pc, std::unique_ptr<RTCRtpTransceiverPlatform>, RTCRtpSender*, RTCRtpReceiver*);
    ...
    void setDirection(const V8RTCRtpTransceiverDirection& direction, ExceptionState&);
    std::optional<V8RTCRtpTransceiverDirection> currentDirection() const;
    void stop(ExceptionState&);
    void setCodecPreferences(const HeapVector<Member<RTCRtpCodecCapability>>& codecs, ExceptionState& exception_state);
    ...
    void setHeaderExtensionsToNegotiate(const HeapVector<Member<RTCRtpHeaderExtensionCapability>>& extensions, ExceptionState& exception_state);
    HeapVector<Member<RTCRtpHeaderExtensionCapability>> getHeaderExtensionsToNegotiate() const;
    HeapVector<Member<RTCRtpHeaderExtensionCapability>> getNegotiatedHeaderExtensions() const;
