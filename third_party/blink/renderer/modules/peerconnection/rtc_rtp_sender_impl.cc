...
template <> struct CrossThreadCopier<webrtc::RtpParameters> : public CrossThreadCopierPassThrough<webrtc::RtpParameters> {
    STATIC_ONLY(CrossThreadCopier);
};

template <> struct CrossThreadCopier<webrtc::RTCError> : public CrossThreadCopierPassThrough<webrtc::RTCError> {
    STATIC_ONLY(CrossThreadCopier);
};
...
void OnReplaceTrackCompleted(blink::RTCVoidRequest* request, bool result)
{
    if (result) {
        ...
        request->RequestFailed(webrtc::RTCError(webrtc::RTCErrorType::INVALID_MODIFICATION));
    }
}

void OnSetParametersCompleted(blink::RTCVoidRequest* request, webrtc::RTCError result)
{
    if (result.ok())
        request->RequestSucceeded();
    else
...
RtpSenderState::RtpSenderState(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, scoped_refptr<base::SingleThreadTaskRunner> signaling_task_runner,
    rtc::scoped_refptr<webrtc::RtpSenderInterface> webrtc_sender, std::unique_ptr<blink::WebRtcMediaStreamTrackAdapterMap::AdapterRef> track_ref,
...
scoped_refptr<base::SingleThreadTaskRunner> RtpSenderState::main_task_runner() const
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    return main_task_runner_;
}

scoped_refptr<base::SingleThreadTaskRunner> RtpSenderState::signaling_task_runner() const
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    return signaling_task_runner_;
}

rtc::scoped_refptr<webrtc::RtpSenderInterface> RtpSenderState::webrtc_sender() const
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    return webrtc_sender_;
}

rtc::scoped_refptr<webrtc::DtlsTransportInterface> RtpSenderState::webrtc_dtls_transport() const
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    return webrtc_dtls_transport_;
}

webrtc::DtlsTransportInformation RtpSenderState::webrtc_dtls_transport_information() const
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    return webrtc_dtls_transport_information_;
}

const std::unique_ptr<blink::WebRtcMediaStreamTrackAdapterMap::AdapterRef>& RtpSenderState::track_ref() const
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    return track_ref_;
}

void RtpSenderState::set_track_ref(std::unique_ptr<blink::WebRtcMediaStreamTrackAdapterMap::AdapterRef> track_ref)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
...
class RTCRtpSenderImpl::RTCRtpSenderInternal
    : public WTF::ThreadSafeRefCounted<RTCRtpSenderImpl::RTCRtpSenderInternal, RTCRtpSenderImpl::RTCRtpSenderInternalTraits> {
public:
    RTCRtpSenderInternal(rtc::scoped_refptr<webrtc::PeerConnectionInterface> native_peer_connection,
        scoped_refptr<blink::WebRtcMediaStreamTrackAdapterMap> track_map, RtpSenderState state, bool require_encoded_insertable_streams)
        ...
        , state_(std::move(state))
    {
        ...
        if (webrtc_sender_->media_type() == cricket::MEDIA_TYPE_AUDIO) {
            encoded_audio_transformer_ = std::make_unique<RTCEncodedAudioStreamTransformer>(main_task_runner_);
            webrtc_sender_->SetEncoderToPacketizerFrameTransformer(encoded_audio_transformer_->Delegate());
        } else {
            CHECK(webrtc_sender_->media_type() == cricket::MEDIA_TYPE_VIDEO);
            encoded_video_transformer_ = std::make_unique<RTCEncodedVideoStreamTransformer>(main_task_runner_, /*metronome=*/nullptr);
            webrtc_sender_->SetEncoderToPacketizerFrameTransformer(encoded_video_transformer_->Delegate());
        }
    }
    ...
    void ReplaceTrack(MediaStreamComponent* with_track, base::OnceCallback<void(bool)> callback)
    {
        DCHECK(main_task_runner_->BelongsToCurrentThread());
        std::unique_ptr<blink::WebRtcMediaStreamTrackAdapterMap::AdapterRef> track_ref;
        ...
        PostCrossThreadTask(*signaling_task_runner_.get(), FROM_HERE,
            CrossThreadBindOnce(&RTCRtpSenderImpl::RTCRtpSenderInternal::ReplaceTrackOnSignalingThread, WrapRefCounted(this), std::move(track_ref),
                CrossThreadUnretained(webrtc_track), CrossThreadBindOnce(std::move(callback))));
    }
    ...
    std::unique_ptr<blink::RtcDtmfSenderHandler> GetDtmfSender() const
    {
        ...
        return std::make_unique<RtcDtmfSenderHandler>(main_task_runner_, dtmf_sender.get());
    }
    ...
    void SetParameters(Vector<webrtc::RtpEncodingParameters> encodings, std::optional<webrtc::DegradationPreference> degradation_preference,
        base::OnceCallback<void(webrtc::RTCError)> callback)
    {
        ...
        for (WTF::wtf_size_t i = 0; i < new_parameters.encodings.size(); ++i) {
            ...
            new_parameters.encodings[i].scale_resolution_down_by = encoding.scale_resolution_down_by;
            new_parameters.encodings[i].scale_resolution_down_to = encoding.scale_resolution_down_to;
            ...
            new_parameters.encodings[i].request_key_frame = encoding.request_key_frame;
        }

        PostCrossThreadTask(*signaling_task_runner_.get(), FROM_HERE,
            CrossThreadBindOnce(&RTCRtpSenderImpl::RTCRtpSenderInternal::SetParametersOnSignalingThread, WrapRefCounted(this), std::move(new_parameters),
    ...
    void GetStats(RTCStatsReportCallback callback)
    {
        PostCrossThreadTask(*signaling_task_runner_.get(), FROM_HERE,
            CrossThreadBindOnce(
                &RTCRtpSenderImpl::RTCRtpSenderInternal::GetStatsOnSignalingThread, WrapRefCounted(this), CrossThreadBindOnce(std::move(callback))));
    }
    ...
    void SetStreams(const Vector<String>& stream_ids)
    {
        DCHECK(main_task_runner_->BelongsToCurrentThread());
        PostCrossThreadTask(*signaling_task_runner_.get(), FROM_HERE,
            CrossThreadBindOnce(&RTCRtpSenderImpl::RTCRtpSenderInternal::SetStreamsOnSignalingThread, WrapRefCounted(this), stream_ids));
    }
...
private:
    friend class WTF::ThreadSafeRefCounted<RTCRtpSenderInternal, RTCRtpSenderInternalTraits>;
    ...
    void ReplaceTrackOnSignalingThread(std::unique_ptr<WebRtcMediaStreamTrackAdapterMap::AdapterRef> track_ref, webrtc::MediaStreamTrackInterface* webrtc_track,
        CrossThreadOnceFunction<void(bool)> callback)
    {
        ...
        PostCrossThreadTask(*main_task_runner_.get(), FROM_HERE,
            CrossThreadBindOnce(
                &RTCRtpSenderImpl::RTCRtpSenderInternal::ReplaceTrackCallback, WrapRefCounted(this), result, std::move(track_ref), std::move(callback)));
    }

    void ReplaceTrackCallback(
        bool result, std::unique_ptr<blink::WebRtcMediaStreamTrackAdapterMap::AdapterRef> track_ref, CrossThreadOnceFunction<void(bool)> callback)
    {
        DCHECK(main_task_runner_->BelongsToCurrentThread());
        if (result)
    ...
    {
        native_peer_connection_->GetStats(rtc::scoped_refptr<webrtc::RtpSenderInterface>(webrtc_sender_.get()),
            CreateRTCStatsCollectorCallback(main_task_runner_, ConvertToBaseOnceCallback(std::move(callback))));
    }

    void SetParametersOnSignalingThread(webrtc::RtpParameters parameters, CrossThreadOnceFunction<void(webrtc::RTCError)> callback)
    {
        DCHECK(signaling_task_runner_->BelongsToCurrentThread());

        webrtc_sender_->SetParametersAsync(parameters, [callback = std::move(callback), task_runner = main_task_runner_](webrtc::RTCError error) mutable {
            PostCrossThreadTask(*task_runner.get(), FROM_HERE, CrossThreadBindOnce(std::move(callback), std::move(error)));
        });
    }
    ...
    const rtc::scoped_refptr<webrtc::PeerConnectionInterface> native_peer_connection_;
...
struct RTCRtpSenderImpl::RTCRtpSenderInternalTraits {
    static void Destruct(const RTCRtpSenderInternal* sender)
    {
        ...
        if (!sender->main_task_runner_->BelongsToCurrentThread()) {
            PostCrossThreadTask(*sender->main_task_runner_.get(), FROM_HERE,
                CrossThreadBindOnce(&RTCRtpSenderImpl::RTCRtpSenderInternalTraits::Destruct, CrossThreadUnretained(sender)));
            return;
        }
        delete sender;
    }
...
uintptr_t RTCRtpSenderImpl::getId(const webrtc::RtpSenderInterface* webrtc_sender)
{
    return reinterpret_cast<uintptr_t>(webrtc_sender);
}

RTCRtpSenderImpl::RTCRtpSenderImpl(rtc::scoped_refptr<webrtc::PeerConnectionInterface> native_peer_connection,
    scoped_refptr<blink::WebRtcMediaStreamTrackAdapterMap> track_map, RtpSenderState state, bool require_encoded_insertable_streams)
    : internal_(base::MakeRefCounted<RTCRtpSenderInternal>(
        std::move(native_peer_connection), std::move(track_map), std::move(state), require_encoded_insertable_streams))
{
}
...
std::unique_ptr<blink::RTCRtpSenderPlatform> RTCRtpSenderImpl::ShallowCopy() const
{
...
rtc::scoped_refptr<webrtc::DtlsTransportInterface> RTCRtpSenderImpl::DtlsTransport()
{
...
Vector<String> RTCRtpSenderImpl::StreamIds() const
{
    ...
    Vector<String> wtf_stream_ids(static_cast<WTF::wtf_size_t>(stream_ids.size()));
    for (WTF::wtf_size_t i = 0; i < stream_ids.size(); ++i)
        wtf_stream_ids[i] = String::FromUTF8(stream_ids[i]);
    return wtf_stream_ids;
}

void RTCRtpSenderImpl::ReplaceTrack(MediaStreamComponent* with_track, RTCVoidRequest* request)
{
    internal_->ReplaceTrack(with_track, WTF::BindOnce(&OnReplaceTrackCompleted, WrapPersistent(request)));
}

std::unique_ptr<blink::RtcDtmfSenderHandler> RTCRtpSenderImpl::GetDtmfSender() const
{
...
void RTCRtpSenderImpl::SetParameters(
    Vector<webrtc::RtpEncodingParameters> encodings, std::optional<webrtc::DegradationPreference> degradation_preference, blink::RTCVoidRequest* request)
{
    internal_->SetParameters(std::move(encodings), degradation_preference, WTF::BindOnce(&OnSetParametersCompleted, WrapPersistent(request)));
}
...
void RTCRtpSenderImpl::ReplaceTrack(MediaStreamComponent* with_track, base::OnceCallback<void(bool)> callback)
{
    internal_->ReplaceTrack(with_track, std::move(callback));
}

bool RTCRtpSenderImpl::RemoveFromPeerConnection(webrtc::PeerConnectionInterface* pc)
{
    return internal_->RemoveFromPeerConnection(pc);
}

RTCEncodedAudioStreamTransformer* RTCRtpSenderImpl::GetEncodedAudioStreamTransformer() const
{
    return internal_->GetEncodedAudioStreamTransformer();
}

RTCEncodedVideoStreamTransformer* RTCRtpSenderImpl::GetEncodedVideoStreamTransformer() const
{
