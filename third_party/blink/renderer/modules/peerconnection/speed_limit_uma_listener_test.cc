...
{
    EXPECT_THAT(histogram_.GetTotalCountsForPrefix("WebRTC.PeerConnectio"), IsEmpty());
    task_runner_->FastForwardBy(SpeedLimitUmaListener::kStatsReportingPeriod);
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.SpeedLimit"), IsEmpty());
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.ThermalThrottling"), ElementsAre(Bucket(false, 1)));
}
...
TEST_F(SpeedLimitUmaListenerTest, HistogramAfterThrottledSignal)
{
    ...
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.SpeedLimit"), ElementsAre(Bucket(55, 1)));
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.ThermalThrottling"), ElementsAre(Bucket(true, 1)));
}
...
TEST_F(SpeedLimitUmaListenerTest, DeletionCancelsListener)
{
    ...
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.SpeedLimit"), ElementsAre(Bucket(33, 2)));

    listener_ = nullptr;
    task_runner_->FastForwardBy(SpeedLimitUmaListener::kStatsReportingPeriod);
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.SpeedLimit"), ElementsAre(Bucket(33, 2)));
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.ThermalThrottling"), ElementsAre(Bucket(true, 2)));
}
...
TEST_F(SpeedLimitUmaListenerTest, RecordsMostRecentState)
{
    ...
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.SpeedLimit"), ElementsAre(Bucket(44, 1)));
}
...
TEST_F(SpeedLimitUmaListenerTest, HistogramBucketsIncludesPreviousPeriod)
{
    ...
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.SpeedLimit"), ElementsAre(Bucket(1, 1), Bucket(2, 1), Bucket(3, 1)));
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.ThermalThrottling"), ElementsAre(Bucket(false, 1), Bucket(true, 3)));
}

TEST_F(SpeedLimitUmaListenerTest, NoThrottlingEpisodesIfNothingReported)
{
    listener_ = nullptr;
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.ThermalThrottlingEpisodes"), ElementsAre(Bucket(0, 1)));
}
...
TEST_F(SpeedLimitUmaListenerTest, CountsOneEpisode)
{
    listener_->OnSpeedLimitChange(55);
    listener_ = nullptr;
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.ThermalThrottlingEpisodes"), ElementsAre(Bucket(1, 1)));
}
...
TEST_F(SpeedLimitUmaListenerTest, CountsTwoEpisodes)
{
    ...
    listener_->OnSpeedLimitChange(100);
    listener_ = nullptr;
    EXPECT_THAT(histogram_.GetAllSamples("WebRTC.PeerConnection.ThermalThrottlingEpisodes"), ElementsAre(Bucket(2, 1)));
}
