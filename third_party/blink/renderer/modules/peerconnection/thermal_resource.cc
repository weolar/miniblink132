...
BASE_FEATURE(kWebRtcThermalResource, "WebRtcThermalResource",
...
// static
scoped_refptr<ThermalResource> ThermalResource::Create(scoped_refptr<base::SequencedTaskRunner> task_runner)
{
    return new rtc::RefCountedObject<ThermalResource>(std::move(task_runner));
}

ThermalResource::ThermalResource(scoped_refptr<base::SequencedTaskRunner> task_runner)
    : task_runner_(std::move(task_runner))
{
}

void ThermalResource::OnThermalMeasurement(mojom::blink::DeviceThermalState measurement)
{
...
void ThermalResource::ReportMeasurementWhileHoldingLock(size_t measurement_id)
{
    ...
    case mojom::blink::DeviceThermalState::kFair:
        listener_->OnResourceUsageStateMeasured(rtc::scoped_refptr<Resource>(this), webrtc::ResourceUsageState::kUnderuse);
        break;
    case mojom::blink::DeviceThermalState::kSerious:
    case mojom::blink::DeviceThermalState::kCritical:
        listener_->OnResourceUsageStateMeasured(rtc::scoped_refptr<Resource>(this), webrtc::ResourceUsageState::kOveruse);
        break;
    }
    // Repeat the reporting every 10 seconds until a new measurement is made or
    // the listener is unregistered.
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&ThermalResource::ReportMeasurement, rtc::scoped_refptr<ThermalResource>(this), measurement_id),
