...
DOMExceptionCode RTCErrorToDOMExceptionCode(const webrtc::RTCError& error)
{
    ...
    default:
        LOG(ERROR) << "Got unhandled RTC error " << static_cast<int>(error.type());
        // No DOM equivalent.
...
DOMException* CreateDOMExceptionFromRTCError(const webrtc::RTCError& error)
{
    if (error.error_detail() != webrtc::RTCErrorDetailType::NONE
        && (error.type() == webrtc::RTCErrorType::UNSUPPORTED_PARAMETER || error.type() == webrtc::RTCErrorType::UNSUPPORTED_OPERATION
            || error.type() == webrtc::RTCErrorType::RESOURCE_EXHAUSTED || error.type() == webrtc::RTCErrorType::INTERNAL_ERROR)) {
        return MakeGarbageCollected<RTCError>(error);
    }
    return MakeGarbageCollected<DOMException>(RTCErrorToDOMExceptionCode(error), error.message());
}

void RejectPromiseFromRTCError(const webrtc::RTCError& error, ScriptPromiseResolverBase* resolver)
{
    if (error.type() == webrtc::RTCErrorType::INVALID_RANGE) {
        resolver->RejectWithRangeError(error.message());
        return;
    }
    resolver->RejectWithDOMException(RTCErrorToDOMExceptionCode(error), error.message());
}

void ThrowExceptionFromRTCError(const webrtc::RTCError& error, ExceptionState& exception_state)
{
    if (error.type() == webrtc::RTCErrorType::INVALID_RANGE) {
        exception_state.ThrowRangeError(error.message());
        return;
    }
    exception_state.ThrowDOMException(RTCErrorToDOMExceptionCode(error), error.message());
}
} // namespace blink
