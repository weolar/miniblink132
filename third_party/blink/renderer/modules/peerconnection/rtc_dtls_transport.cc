...
namespace {
V8RTCDtlsTransportState::Enum TransportStateToEnum(webrtc::DtlsTransportState state)
{
    switch (state) {
...
std::unique_ptr<DtlsTransportProxy> CreateProxy(
    ExecutionContext* context, webrtc::DtlsTransportInterface* native_transport, DtlsTransportProxy::Delegate* delegate)
{
    LocalFrame* frame = To<LocalDOMWindow>(context)->GetFrame();
    scoped_refptr<base::SingleThreadTaskRunner> proxy_thread = frame->GetTaskRunner(TaskType::kNetworking);
    ...
    return DtlsTransportProxy::Create(*frame, proxy_thread, host_thread, native_transport, delegate);
}

} // namespace

RTCDtlsTransport::RTCDtlsTransport(
    ExecutionContext* context, rtc::scoped_refptr<webrtc::DtlsTransportInterface> native_transport, RTCIceTransport* ice_transport)
...
const HeapVector<Member<DOMArrayBuffer>>& RTCDtlsTransport::getRemoteCertificates() const
{
...
{
    DCHECK(info.state() == webrtc::DtlsTransportState::kClosed || current_state_.state() != webrtc::DtlsTransportState::kClosed);
    current_state_ = info;
}
...
void RTCDtlsTransport::OnStateChange(webrtc::DtlsTransportInformation info)
{
    ...
    if (current_state_.state() == webrtc::DtlsTransportState::kConnected) {
        if (current_state_.tls_version()) {
            if (*current_state_.tls_version() == DTLS1_VERSION || *current_state_.tls_version() == SSL3_VERSION || *current_state_.tls_version() == TLS1_VERSION
                || *current_state_.tls_version() == TLS1_1_VERSION) {
                Deprecation::CountDeprecation(GetExecutionContext(), WebFeature::kObsoleteWebrtcTlsVersion);
            }
        }
    }
        ...
        if (certs->GetSize() != remote_certificates_.size()) {
            ...
            for (WTF::wtf_size_t i = 0; i < certs->GetSize(); i++) {
                ...
                if (dab_cert->ByteLength() != remote_certificates_[i]->ByteLength()
                    || memcmp(dab_cert->Data(), remote_certificates_[i]->Data(), dab_cert->ByteLength()) != 0) {
