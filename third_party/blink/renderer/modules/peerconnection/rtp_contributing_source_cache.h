...
public:
    ...
    RtpContributingSourceCache(RTCPeerConnection* pc, scoped_refptr<base::SingleThreadTaskRunner> worker_thread_runner);
    ...
    HeapVector<Member<RTCRtpContributingSource>> getContributingSources(ScriptState* script_state, ExceptionState& exception_state, RTCRtpReceiver* receiver);
...
private:
    ...
    // it is at it.
    void MaybeUpdateRtpSources(ScriptState* script_state, RTCRtpReceiver* requesting_receiver);
    ...
    HashMap<RTCRtpReceiverPlatform*, RTCRtpSources> cached_sources_by_audio_receiver_;
    HashMap<RTCRtpReceiverPlatform*, RTCRtpSources> cached_sources_by_video_receiver_;
