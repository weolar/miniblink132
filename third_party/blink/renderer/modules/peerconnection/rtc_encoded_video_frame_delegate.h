...
class RTCEncodedVideoFrameDelegate : public WTF::ThreadSafeRefCounted<RTCEncodedVideoFrameDelegate> {
public:
    explicit RTCEncodedVideoFrameDelegate(std::unique_ptr<webrtc::TransformableVideoFrameInterface> webrtc_frame);
    ...
    base::expected<void, String> SetMetadata(const webrtc::VideoFrameMetadata& metadata, uint32_t rtpTimestamp);
...
private:
    ...
    std::unique_ptr<webrtc::TransformableVideoFrameInterface> webrtc_frame_ GUARDED_BY(lock_);
};

class MODULES_EXPORT RTCEncodedVideoFramesAttachment : public SerializedScriptValue::Attachment {
public:
    ...
    const Vector<scoped_refptr<RTCEncodedVideoFrameDelegate>>& EncodedVideoFrames() const
    {
        return encoded_video_frames_;
    }
