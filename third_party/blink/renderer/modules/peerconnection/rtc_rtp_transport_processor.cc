...
void RTCRtpTransportProcessor::SetFeedbackProviders(Vector<scoped_refptr<FeedbackProvider>> feedback_providers)
{
    feedback_providers_ = feedback_providers;
}

webrtc::NetworkControlUpdate RTCRtpTransportProcessor::OnFeedback(webrtc::TransportPacketsFeedback feedback)
{
    ...
    for (const webrtc::PacketResult& result : feedback.packet_feedbacks) {
        ...
        ack->setRemoteReceiveTimestamp(result.receive_time.IsFinite() ? result.receive_time.ms() : 0);
    ...
    acks_messages_.push_back(MakeGarbageCollected<RTCRtpAcks>(acks, feedback.feedback_time.IsFinite() ? feedback.feedback_time.ms() : 0,
        /*received_time=*/0, /*explicit_congestion_notification=*/
        V8ExplicitCongestionNotification(V8ExplicitCongestionNotification::Enum::kUnset)));

    return webrtc::NetworkControlUpdate();
}

HeapVector<Member<RTCRtpAcks>> RTCRtpTransportProcessor::readReceivedAcks(uint32_t maxCount)
{
    CHECK(!GetExecutionContext() || GetExecutionContext()->IsContextThread());
...
{
    CHECK(!GetExecutionContext() || GetExecutionContext()->IsContextThread());
    sents_.push_back(MakeGarbageCollected<RTCRtpSent>(sp.send_time.ms<double>(), sp.sequence_number, sp.size.bytes()));
}

HeapVector<Member<RTCRtpSent>> RTCRtpTransportProcessor::readSentRtp(uint32_t maxCount)
{
    CHECK(!GetExecutionContext() || GetExecutionContext()->IsContextThread());
...
void RTCRtpTransportProcessor::setCustomMaxBandwidth(uint64_t custom_max_bitrate_bps)
{
