...
void RunSynchronous(base::TestSimpleTaskRunner* thread, CrossThreadOnceClosure closure)
{
    ...
    PostCrossThreadTask(*thread, FROM_HERE,
        CrossThreadBindOnce(
...
public:
    MockPeerConnectionHandler(scoped_refptr<base::TestSimpleTaskRunner> signaling_thread)
        : signaling_thread_(signaling_thread)
    {
    }

    MockPeerConnectionHandler(const MockPeerConnectionHandler&) = delete;
    MockPeerConnectionHandler& operator=(const MockPeerConnectionHandler&) = delete;

    scoped_refptr<base::SingleThreadTaskRunner> signaling_thread() const override
    {
...
public:
    explicit MockDataChannel(scoped_refptr<base::TestSimpleTaskRunner> signaling_thread)
        : signaling_thread_(signaling_thread)
        , buffered_amount_(0)
        , observer_(nullptr)
    ...
    void RegisterObserver(webrtc::DataChannelObserver* observer) override
    {
        RunSynchronous(signaling_thread_.get(),
            CrossThreadBindOnce(&MockDataChannel::RegisterObserverOnSignalingThread, CrossThreadUnretained(this), CrossThreadUnretained(observer)));
    }

    void UnregisterObserver() override
    {
        RunSynchronous(signaling_thread_.get(), CrossThreadBindOnce(&MockDataChannel::UnregisterObserverOnSignalingThread, CrossThreadUnretained(this)));
    }
    ...
    uint64_t buffered_amount() const override
    {
        ...
        RunSynchronous(signaling_thread_.get(),
            CrossThreadBindOnce(&MockDataChannel::GetBufferedAmountOnSignalingThread, CrossThreadUnretained(this), CrossThreadUnretained(&buffered_amount)));
    ...
    DataState state() const override
    {
        DataState state;
        RunSynchronous(signaling_thread_.get(),
            CrossThreadBindOnce(&MockDataChannel::GetStateOnSignalingThread, CrossThreadUnretained(this), CrossThreadUnretained(&state)));
        return state;
    }

    bool Send(const webrtc::DataBuffer& buffer) override
    {
        RunSynchronous(signaling_thread_.get(), CrossThreadBindOnce(&MockDataChannel::SendOnSignalingThread, CrossThreadUnretained(this), buffer.size()));
        return true;
    }

    void SendAsync(webrtc::DataBuffer buffer, absl::AnyInvocable<void(webrtc::RTCError) &&> on_complete) override
    {
        base::WaitableEvent waitable_event(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
        auto* adapter = new absl::AnyInvocable<void(webrtc::RTCError) &&>(std::move(on_complete));

        PostCrossThreadTask(*signaling_thread_.get(), FROM_HERE,
            CrossThreadBindOnce(
                [](MockDataChannel* channel, uint64_t buffer_size, absl::AnyInvocable<void(webrtc::RTCError) &&>* adapter) {
                ...
                CrossThreadUnretained(this), buffer.size(), CrossThreadUnretained(adapter)));
    }

    // For testing.
    void ChangeState(DataState state)
    {
        RunSynchronous(signaling_thread_.get(), CrossThreadBindOnce(&MockDataChannel::ChangeStateOnSignalingThread, CrossThreadUnretained(this), state));
...
private:
    void RegisterObserverOnSignalingThread(webrtc::DataChannelObserver* observer)
    {
        DCHECK(signaling_thread_->BelongsToCurrentThread());
        observer_ = observer;
    }
...
public:
    ...
    void VerifyNoTransfersAfterSend(base::OnceCallback<void(RTCDataChannel*)> send_data_callback)
    {
        V8TestingScope scope;
        ScopedTransferableRTCDataChannelForTest scoped_feature(/*enabled=*/true);

        rtc::scoped_refptr<MockDataChannel> webrtc_channel(new rtc::RefCountedObject<MockDataChannel>(signaling_thread()));
        auto* channel = MakeGarbageCollected<RTCDataChannel>(scope.GetExecutionContext(), webrtc_channel);
...
protected:
    ...
    Persistent<NullExecutionContext> execution_context_ = MakeGarbageCollected<NullExecutionContext>();
...
TEST_F(RTCDataChannelTest, ChangeStateEarly)
{
    ...
    auto* channel = MakeGarbageCollected<RTCDataChannel>(execution_context_, webrtc_channel);
...
TEST_F(RTCDataChannelTest, BufferedAmountLow)
{
    ...
    auto* channel = MakeGarbageCollected<RTCDataChannel>(execution_context_, webrtc_channel);
    channel->addEventListener(event_type_names::kBufferedamountlow, onbufferedamountlow_handler);
...
TEST_F(RTCDataChannelTest, SendPreventsTransfers)
{
...

    {
        ...
        VerifyNoTransfersAfterSend(WTF::BindOnce([](RTCDataChannel* channel) {
            DOMArrayBuffer* buffer = DOMArrayBuffer::Create(10, 4);
            channel->send(NotShared<DOMArrayBufferView>(DOMDataView::Create(buffer, 0, 10)), IGNORE_EXCEPTION_FOR_TESTING);
        }));
    }
...
TEST_F(RTCDataChannelTest, NoSendAfterClose)
{
...

    {
        ...
        DOMArrayBuffer* buffer = DOMArrayBuffer::Create(10, 4);
        DummyExceptionStateForTesting exception_state;
        channel->send(NotShared<DOMArrayBufferView>(DOMDataView::Create(buffer, 0, 10)), exception_state);
        EXPECT_TRUE(exception_state.HadException());
    }
