...
class MODULES_EXPORT RTCIceTransport final : public EventTarget,
...
public:
    ...
    static RTCIceTransport* Create(
        ExecutionContext* context, rtc::scoped_refptr<webrtc::IceTransportInterface> ice_transport_channel, RTCPeerConnection* peer_connection);

    RTCIceTransport(ExecutionContext* context, scoped_refptr<base::SingleThreadTaskRunner> proxy_thread,
    ...
    {
        return role_ != cricket::ICEROLE_UNKNOWN;
    }

    // Returns the role specified in start().
    cricket::IceRole GetRole() const
    {
        return role_;
    }

    webrtc::IceTransportState GetState() const
    {
        return state_;
    }
    ...
    bool IsClosed() const
    {
        return state_ == webrtc::IceTransportState::kClosed;
    }
    void Stop()
    {
    ...
    DEFINE_ATTRIBUTE_EVENT_LISTENER(selectedcandidatepairchange, kSelectedcandidatepairchange)
...
private:
    ...
    void OnSelectedCandidatePairChanged(const std::pair<cricket::Candidate, cricket::Candidate>& selected_candidate_pair) override;
