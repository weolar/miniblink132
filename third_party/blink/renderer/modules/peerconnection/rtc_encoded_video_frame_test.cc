...
webrtc::VideoFrameMetadata MockVP9Metadata(MockTransformableVideoFrame* frame)
{
    ...
    decode_target_indications.push_back(webrtc::DecodeTargetIndication::kRequired);
...
TEST_F(RTCEncodedVideoFrameTest, GetMetadataReturnsMetadata)
{
    ...
    std::unique_ptr<MockTransformableVideoFrame> frame = std::make_unique<MockTransformableVideoFrame>();
    ...
    RTCEncodedVideoFrame* encoded_frame = MakeGarbageCollected<RTCEncodedVideoFrame>(std::move(frame));

    RTCEncodedVideoFrameMetadata* retrieved_metadata = encoded_frame->getMetadata();
...
TEST_F(RTCEncodedVideoFrameTest, SetMetadataPreservesVP9CodecSpecifics)
{
    ...
    std::unique_ptr<MockTransformableVideoFrame> frame = std::make_unique<NiceMock<MockTransformableVideoFrame>>();
    ...
    EXPECT_EQ(actual_metadata.GetRTPVideoHeaderCodecSpecifics(), webrtc_metadata.GetRTPVideoHeaderCodecSpecifics());
}
...
TEST_F(RTCEncodedVideoFrameTest, SetMetadataMissingFieldsFails)
{
    ...
    RTCEncodedVideoFrameMetadata* empty_metadata = RTCEncodedVideoFrameMetadata::Create();

    DummyExceptionStateForTesting exception_state;
    encoded_frame->setMetadata(empty_metadata, exception_state);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(exception_state.Message(), "Cannot setMetadata: new metadata has member(s) missing.");
}

RTCEncodedVideoFrameMetadata* CreateMetadata(bool change_all_fields = false)
{
    RTCEncodedVideoFrameMetadata* new_metadata = RTCEncodedVideoFrameMetadata::Create();
...
TEST_F(RTCEncodedVideoFrameTest, SetMetadataWithoutFeatureFailsModifications)
{
    ...
    RTCEncodedVideoFrameMetadata* new_metadata = CreateMetadata(/*change_all_fields=*/true);
...
TEST_F(RTCEncodedVideoFrameTest, SetMetadataOnEmptyFrameFails)
{
    ...
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(exception_state.Message(), "Cannot setMetadata: underlying webrtc frame is an empty frame.");
}
...
TEST_F(RTCEncodedVideoFrameTest, SetMetadataRejectsTooManyDependencies)
{
    ...
    encoded_frame->setMetadata(new_metadata, exception_state);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(exception_state.Message(), "Cannot setMetadata: new metadata has too many dependencies.");
}
...
TEST_F(RTCEncodedVideoFrameTest, ConstructorPreservesVP9CodecSpecifics)
{
    ...
    RTCEncodedVideoFrame* new_frame = RTCEncodedVideoFrame::Create(encoded_frame, exception_state);
    EXPECT_FALSE(exception_state.HadException()) << exception_state.Message();
    ...
    EXPECT_EQ(new_frame->getMetadata()->spatialIndex(), webrtc_metadata.GetSpatialIndex());
    EXPECT_EQ(new_frame->getMetadata()->temporalIndex(), webrtc_metadata.GetTemporalIndex());
    EXPECT_EQ(new_frame->getMetadata()->synchronizationSource(), webrtc_metadata.GetSsrc());
    std::vector<uint32_t> actual_csrcs;
    for (const auto& dependency : new_frame->getMetadata()->contributingSources()) {
        actual_csrcs.push_back(dependency);
    }
...
TEST_F(RTCEncodedVideoFrameTest, ConstructorMissingFieldsFails)
{
    ...
    MockVP8Metadata(frame.get());
    RTCEncodedVideoFrame* encoded_frame = MakeGarbageCollected<RTCEncodedVideoFrame>(std::move(frame));
    RTCEncodedVideoFrameOptions* empty_frame_options = RTCEncodedVideoFrameOptions::Create();
    empty_frame_options->setMetadata(RTCEncodedVideoFrameMetadata::Create());

    DummyExceptionStateForTesting exception_state;
    RTCEncodedVideoFrame* new_frame = RTCEncodedVideoFrame::Create(encoded_frame, empty_frame_options, exception_state);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(exception_state.Message(),
...
TEST_F(RTCEncodedVideoFrameTest, ConstructorWithoutFeatureFailsModifications)
{
    ...
    RTCEncodedVideoFrame* encoded_frame = MakeGarbageCollected<RTCEncodedVideoFrame>(std::move(frame));
    RTCEncodedVideoFrameOptions* frame_options = RTCEncodedVideoFrameOptions::Create();
    frame_options->setMetadata(CreateMetadata(/*change_all_fields=*/true));

    DummyExceptionStateForTesting exception_state;
    RTCEncodedVideoFrame* new_frame = RTCEncodedVideoFrame::Create(encoded_frame, frame_options, exception_state);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(exception_state.Message(),
...
TEST_F(RTCEncodedVideoFrameTest, ConstructorWithFeatureAllowsModifications)
{
    ...
    EXPECT_EQ(new_frame->getMetadata()->spatialIndex(), new_metadata->spatialIndex());
    EXPECT_EQ(new_frame->getMetadata()->temporalIndex(), new_metadata->temporalIndex());
    EXPECT_EQ(new_frame->getMetadata()->synchronizationSource(), new_metadata->synchronizationSource());
    Vector<uint32_t> actual_csrcs;
    for (const auto& dependency : new_frame->getMetadata()->contributingSources()) {
        actual_csrcs.push_back(dependency);
    }
...
TEST_F(RTCEncodedVideoFrameTest, ConstructorFromNull)
{
    V8TestingScope v8_scope;
    DummyExceptionStateForTesting exception_state;
    RTCEncodedVideoFrame* new_frame = RTCEncodedVideoFrame::Create(nullptr, exception_state);

    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(exception_state.Message(), "Cannot create a new VideoFrame from an empty VideoFrame");
    EXPECT_EQ(new_frame, nullptr);
}
...
TEST_F(RTCEncodedVideoFrameTest, ConstructorCopiesMetadata)
{
    ...
    EXPECT_EQ(new_frame->getMetadata()->frameId(), encoded_frame->getMetadata()->frameId());
    EXPECT_EQ(new_frame->getMetadata()->dependencies(), encoded_frame->getMetadata()->dependencies());
    EXPECT_EQ(new_frame->getMetadata()->width(), encoded_frame->getMetadata()->width());
    EXPECT_EQ(new_frame->getMetadata()->height(), encoded_frame->getMetadata()->height());
    EXPECT_EQ(new_frame->getMetadata()->spatialIndex(), encoded_frame->getMetadata()->spatialIndex());
    EXPECT_EQ(new_frame->getMetadata()->temporalIndex(), encoded_frame->getMetadata()->temporalIndex());
    EXPECT_EQ(new_frame->getMetadata()->synchronizationSource(), encoded_frame->getMetadata()->synchronizationSource());
    EXPECT_EQ(new_frame->getMetadata()->contributingSources(), encoded_frame->getMetadata()->contributingSources());
    EXPECT_EQ(new_frame->getMetadata()->rtpTimestamp(), encoded_frame->getMetadata()->rtpTimestamp());
}
...
TEST_F(RTCEncodedVideoFrameTest, ConstructorWithMetadataGetsNewMetadata)
{
    ...
    // metadata.
    EXPECT_NE(new_frame->getMetadata()->frameId(), encoded_frame->getMetadata()->frameId());
    EXPECT_NE(new_frame->getMetadata()->dependencies(), encoded_frame->getMetadata()->dependencies());
    EXPECT_NE(new_frame->getMetadata()->rtpTimestamp(), encoded_frame->getMetadata()->rtpTimestamp());
}

TEST_F(RTCEncodedVideoFrameTest, ConstructorWithMetadataDoesNotAllowChangingPayloadType)
{
    V8TestingScope v8_scope;
