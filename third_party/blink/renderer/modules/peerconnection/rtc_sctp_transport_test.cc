...
public:
    MockSctpTransport()
    {
        ...
        ON_CALL(*this, RegisterObserver(_)).WillByDefault(Invoke(this, &MockSctpTransport::SetObserver));
    }
    ...
    {
        if (observer_) {
            observer_->OnStateChange(webrtc::SctpTransportInformation(webrtc::SctpTransportState::kClosed));
        }
    }

private:
    webrtc::SctpTransportInformation info_ = webrtc::SctpTransportInformation(webrtc::SctpTransportState::kNew);
    raw_ptr<webrtc::SctpTransportObserverInterface, DanglingUntriaged> observer_ = nullptr;
};
...
TEST_F(RTCSctpTransportTest, CreateFromMocks)
{
    ...
    rtc::scoped_refptr<webrtc::SctpTransportInterface> mock_native_transport(new rtc::RefCountedObject<NiceMock<MockSctpTransport>>());
    RTCSctpTransport* transport = MakeGarbageCollected<RTCSctpTransport>(context, mock_native_transport, main_thread_, worker_thread_);
