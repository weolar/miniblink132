...
class MockWebrtcVideoPerfRecorder : public media::mojom::blink::WebrtcVideoPerfRecorder {
public:
    MockWebrtcVideoPerfRecorder() = default;
    MOCK_METHOD2(UpdateRecord, void(media::mojom::blink::WebrtcPredictionFeaturesPtr, media::mojom::blink::WebrtcVideoStatsPtr));

    mojo::PendingRemote<media::mojom::blink::WebrtcVideoPerfRecorder> CreatePendingRemote()
    {
        receiver_.reset();
...
public:
    WebrtcVideoPerfReporterTest()
    {
        ...
        reporter_ = MakeGarbageCollected<WebrtcVideoPerfReporter>(blink::scheduler::GetSingleThreadTaskRunnerForTesting(),
...
{
    const StatsCollector::StatsKey kStatsKeyA = { /*is_decode=*/true, kCodecProfile, 1920 * 1080,
        /*hw_accelerated=*/false };
    const auto kExpectedFeaturesA = media::mojom::blink::WebrtcPredictionFeatures(
        /*is_decode_stats=*/true, static_cast<media::mojom::blink::VideoCodecProfile>(kCodecProfile),
    ...
    EXPECT_CALL(*mock_recorder_, UpdateRecord)
        .WillOnce([&kExpectedFeaturesA, &kExpectedVideoStats](
                      media::mojom::blink::WebrtcPredictionFeaturesPtr features, media::mojom::blink::WebrtcVideoStatsPtr video_stats) {
    ...
    // Toggle the booleans.
    const StatsCollector::StatsKey kStatsKeyB = { /*is_decode=*/false, kCodecProfile, 1920 * 1080,
        /*hw_accelerated=*/true };
    const auto kExpectedFeaturesB = media::mojom::blink::WebrtcPredictionFeatures(
        /*is_decode_stats=*/false, static_cast<media::mojom::blink::VideoCodecProfile>(kCodecProfile),
