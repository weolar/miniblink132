...
RTCRtpReceiver::RTCRtpReceiver(RTCPeerConnection* pc, std::unique_ptr<RTCRtpReceiverPlatform> receiver, MediaStreamTrack* track, MediaStreamVector streams,
    ...
    , encoded_audio_transformer_(track_->kind() == "audio" ? receiver_->GetEncodedAudioStreamTransformer()->GetBroker() : nullptr)
    , encoded_video_transformer_(track_->kind() == "video" ? receiver_->GetEncodedVideoStreamTransformer()->GetBroker() : nullptr)
{
    DCHECK(pc_);
    DCHECK(receiver_);
    DCHECK(track_);
    ...
    if (!require_encoded_insertable_streams) {
        ...
        encoded_transform_shortcircuit_runner->PostTask(FROM_HERE, WTF::BindOnce(&RTCRtpReceiver::MaybeShortCircuitEncodedStreams, WrapPersistent(this)));
    }
}
...
void RTCRtpReceiver::setPlayoutDelayHint(std::optional<double> hint, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void RTCRtpReceiver::setJitterBufferTarget(std::optional<double> target, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (target.has_value() && (target.value() < 0.0 || target.value() > 4000.0)) {
        exception_state.ThrowRangeError("jitterBufferTarget is out of expected range 0 to 4000 ms");
        return;
    }

    jitter_buffer_target_ = target;
    if (jitter_buffer_target_.has_value()) {
        receiver_->SetJitterBufferMinimumDelay(jitter_buffer_target_.value() / 1000.0);
    } else {
        receiver_->SetJitterBufferMinimumDelay(std::nullopt);
    }
}

HeapVector<Member<RTCRtpSynchronizationSource>> RTCRtpReceiver::getSynchronizationSources(ScriptState* script_state, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    return pc_->GetRtpContributingSourceCache().getSynchronizationSources(script_state, exception_state, this);
}

HeapVector<Member<RTCRtpContributingSource>> RTCRtpReceiver::getContributingSources(ScriptState* script_state, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    return pc_->GetRtpContributingSourceCache().getContributingSources(script_state, exception_state, this);
}

ScriptPromise<RTCStatsReport> RTCRtpReceiver::getStats(ScriptState* script_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<RTCStatsReport>>(script_state);
    auto promise = resolver->Promise();
    receiver_->GetStats(WTF::BindOnce(WebRTCStatsReportCallbackResolver, WrapPersistent(resolver)));
    return promise;
}

RTCInsertableStreams* RTCRtpReceiver::createEncodedStreams(ScriptState* script_state, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
std::optional<V8RTCRtpTransceiverDirection> RTCRtpReceiver::TransceiverCurrentDirection()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    // `transceiver_` is always initialized to a valid value.
...
RTCRtpCapabilities* RTCRtpReceiver::getCapabilities(ScriptState* state, const String& kind)
{
    ...
    std::unique_ptr<webrtc::RtpCapabilities> rtc_capabilities
        = PeerConnectionDependencyFactory::From(*ExecutionContext::From(state)).GetReceiverCapabilities(kind);
    ...
    if (IdentifiabilityStudySettings::Get()->ShouldSampleType(IdentifiableSurface::Type::kRtcRtpReceiverGetCapabilities)) {
        ...
        IdentifiabilityMetricBuilder(ExecutionContext::From(state)->UkmSourceID())
            .Add(IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kRtcRtpReceiverGetCapabilities, IdentifiabilityBenignStringToken(kind)),
                builder.GetToken())
...
RTCRtpReceiveParameters* RTCRtpReceiver::getParameters()
{
    ...
    std::unique_ptr<webrtc::RtpParameters> webrtc_parameters = receiver_->GetParameters();
...
void RTCRtpReceiver::RegisterEncodedAudioStreamCallback()
{
    ...
    encoded_audio_transformer_->SetTransformerCallback(
        WTF::CrossThreadBindRepeating(&RTCRtpReceiver::OnAudioFrameFromDepacketizer, WrapCrossThreadWeakPersistent(this)));
}
    ...
    RTCEncodedAudioUnderlyingSource* new_underlying_source, scoped_refptr<base::SingleThreadTaskRunner> new_source_task_runner)
{
    ...
    }
    {
        ...
        if (base::FeatureList::IsEnabled(kWebRtcEncodedTransformDirectCallback)) {
            encoded_audio_transformer_->SetTransformerCallback(
                WTF::CrossThreadBindRepeating(&RTCEncodedAudioUnderlyingSource::OnFrameFromSource, audio_from_depacketizer_underlying_source_));
        }
    }
...
void RTCRtpReceiver::SetAudioUnderlyingSink(RTCEncodedAudioUnderlyingSink* new_underlying_sink)
{
    if (!GetExecutionContext()) {
...
RTCInsertableStreams* RTCRtpReceiver::CreateEncodedAudioStreams(ScriptState* script_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...

    {
        ...
        // Set up readable.
        audio_from_depacketizer_underlying_source_ = MakeGarbageCollected<RTCEncodedAudioUnderlyingSource>(
            script_state, WTF::CrossThreadBindOnce(&RTCRtpReceiver::UnregisterEncodedAudioStreamCallback, WrapCrossThreadWeakPersistent(this)));

        auto set_underlying_source = WTF::CrossThreadBindRepeating(&RTCRtpReceiver::SetAudioUnderlyingSource, WrapCrossThreadWeakPersistent(this));
        auto disconnect_callback = WTF::CrossThreadBindOnce(&RTCRtpReceiver::UnregisterEncodedAudioStreamCallback, WrapCrossThreadWeakPersistent(this));
        ...
        ReadableStream* readable_stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, audio_from_depacketizer_underlying_source_,
            /*high_water_mark=*/0, AllowPerChunkTransferring(false),
            std::make_unique<RtcEncodedAudioReceiverSourceOptimizer>(std::move(set_underlying_source), std::move(disconnect_callback)));
    ...
    WritableStream* writable_stream;
    {
        ...
        // Set up writable.
        audio_to_decoder_underlying_sink_ = MakeGarbageCollected<RTCEncodedAudioUnderlyingSink>(script_state, encoded_audio_transformer_,
            /*detach_frame_data_on_write=*/false);

        auto set_underlying_sink = WTF::CrossThreadBindOnce(&RTCRtpReceiver::SetAudioUnderlyingSink, WrapCrossThreadWeakPersistent(this));

        // The high water mark for the stream is set to 1 so that the stream seems
        // ready to write, but without queuing frames.
        writable_stream = WritableStream::CreateWithCountQueueingStrategy(script_state, audio_to_decoder_underlying_sink_,
            /*high_water_mark=*/1, std::make_unique<RtcEncodedAudioReceiverSinkOptimizer>(std::move(set_underlying_sink), encoded_audio_transformer_));
    }
...
void RTCRtpReceiver::setTransform(RTCRtpScriptTransform* transform, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (kind() == MediaKind::kAudio) {
        transform_->CreateAudioUnderlyingSourceAndSink(
            WTF::CrossThreadBindOnce(&RTCRtpReceiver::UnregisterEncodedAudioStreamCallback, WrapCrossThreadWeakPersistent(this)), encoded_audio_transformer_);
        return;
    }
    CHECK(kind() == MediaKind::kVideo);
    transform_->CreateVideoUnderlyingSourceAndSink(
        WTF::CrossThreadBindOnce(&RTCRtpReceiver::UnregisterEncodedVideoStreamCallback, WrapCrossThreadWeakPersistent(this)), encoded_video_transformer_);
}

void RTCRtpReceiver::OnAudioFrameFromDepacketizer(std::unique_ptr<webrtc::TransformableAudioFrameInterface> encoded_audio_frame)
{
    // TODO(crbug.com/347915599): Delete this method once
    // kWebRtcEncodedTransformDirectCallback is fully launched.
    CHECK(!base::FeatureList::IsEnabled(kWebRtcEncodedTransformDirectCallback));
    ...
    if (audio_from_depacketizer_underlying_source_) {
        audio_from_depacketizer_underlying_source_->OnFrameFromSource(std::move(encoded_audio_frame));
    }
}
...
void RTCRtpReceiver::RegisterEncodedVideoStreamCallback()
{
    ...
    encoded_video_transformer_->SetTransformerCallback(
        WTF::CrossThreadBindRepeating(&RTCRtpReceiver::OnVideoFrameFromDepacketizer, WrapCrossThreadWeakPersistent(this)));
}
    ...
    RTCEncodedVideoUnderlyingSource* new_underlying_source, scoped_refptr<base::SingleThreadTaskRunner> new_source_task_runner)
{
    ...
    }
    {
        ...
        if (base::FeatureList::IsEnabled(kWebRtcEncodedTransformDirectCallback)) {
            encoded_video_transformer_->SetTransformerCallback(
                WTF::CrossThreadBindRepeating(&RTCEncodedVideoUnderlyingSource::OnFrameFromSource, video_from_depacketizer_underlying_source_));
        }
    }
...
void RTCRtpReceiver::SetVideoUnderlyingSink(RTCEncodedVideoUnderlyingSink* new_underlying_sink)
{
    if (!GetExecutionContext()) {
        // If our context is destroyed, then the RTCRtpReceiver and underlying
        // sink(s) are about to be garbage collected, so there's no reason to
...
RTCInsertableStreams* RTCRtpReceiver::CreateEncodedVideoStreams(ScriptState* script_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    CHECK(!encoded_streams_);
...

    {
        ...
        // Set up readable.
        video_from_depacketizer_underlying_source_ = MakeGarbageCollected<RTCEncodedVideoUnderlyingSource>(
            script_state, WTF::CrossThreadBindOnce(&RTCRtpReceiver::UnregisterEncodedVideoStreamCallback, WrapCrossThreadWeakPersistent(this)));

        auto set_underlying_source = WTF::CrossThreadBindRepeating(&RTCRtpReceiver::SetVideoUnderlyingSource, WrapCrossThreadWeakPersistent(this));
        auto disconnect_callback = WTF::CrossThreadBindOnce(&RTCRtpReceiver::UnregisterEncodedVideoStreamCallback, WrapCrossThreadWeakPersistent(this));
        // The high water mark for the readable stream is set to 0 so that frames
        // are removed from the queue right away, without introducing a new buffer.
        ReadableStream* readable_stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, video_from_depacketizer_underlying_source_,
            /*high_water_mark=*/0, AllowPerChunkTransferring(false),
            std::make_unique<RtcEncodedVideoReceiverSourceOptimizer>(std::move(set_underlying_source), std::move(disconnect_callback)));
        encoded_streams_->setReadable(readable_stream);
    ...
    WritableStream* writable_stream;
    {
        ...
        // Set up writable.
        video_to_decoder_underlying_sink_ = MakeGarbageCollected<RTCEncodedVideoUnderlyingSink>(script_state, encoded_video_transformer_,
            /*detach_frame_data_on_write=*/false);

        auto set_underlying_sink = WTF::CrossThreadBindOnce(&RTCRtpReceiver::SetVideoUnderlyingSink, WrapCrossThreadWeakPersistent(this));

        // The high water mark for the stream is set to 1 so that the stream seems
        // ready to write, but without queuing frames.
        writable_stream = WritableStream::CreateWithCountQueueingStrategy(script_state, video_to_decoder_underlying_sink_,
            /*high_water_mark=*/1, std::make_unique<RtcEncodedVideoReceiverSinkOptimizer>(std::move(set_underlying_sink), encoded_video_transformer_));
    }
...
void RTCRtpReceiver::OnVideoFrameFromDepacketizer(std::unique_ptr<webrtc::TransformableVideoFrameInterface> encoded_video_frame)
{
    ...
    if (video_from_depacketizer_underlying_source_) {
        video_from_depacketizer_underlying_source_->OnFrameFromSource(std::move(encoded_video_frame));
    }
}

void RTCRtpReceiver::LogMessage(const std::string& message)
{
    blink::WebRtcLogMessage(base::StringPrintf("RtpRcvr::%s [this=0x%" PRIXPTR "]", message.c_str(), reinterpret_cast<uintptr_t>(this)));
}
