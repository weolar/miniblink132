...
public:
    PeerConnectionDependencyFactoryTest()
        : mock_client_(MakeGarbageCollected<MockRTCPeerConnectionHandlerClient>())
    {
    }
    ...
    std::unique_ptr<RTCPeerConnectionHandler> CreateRTCPeerConnectionHandler()
    {
        std::unique_ptr<RTCPeerConnectionHandler> handler
            = dependency_factory_->CreateRTCPeerConnectionHandler(mock_client_.Get(), blink::scheduler::GetSingleThreadTaskRunnerForTesting(),
...
TEST_F(PeerConnectionDependencyFactoryTest, CreateRTCPeerConnectionHandler)
{
    ...
    std::unique_ptr<RTCPeerConnectionHandler> pc_handler = CreateRTCPeerConnectionHandler();
