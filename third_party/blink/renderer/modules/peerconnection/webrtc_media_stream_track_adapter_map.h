...
public:
    ...
    public:
        ...
        {
            return adapter_->is_initialized();
        }
        void InitializeOnMainThread();
        MediaStreamComponent* track() const
        {
            return adapter_->track();
        }
    ...
    private:
        ...
        AdapterRef(scoped_refptr<WebRtcMediaStreamTrackAdapterMap> map, Type type, scoped_refptr<blink::WebRtcMediaStreamTrackAdapter> adapter);
    ...
    WebRtcMediaStreamTrackAdapterMap(blink::PeerConnectionDependencyFactory* const factory, scoped_refptr<base::SingleThreadTaskRunner> main_thread);
    ...
    // either track.
    std::unique_ptr<AdapterRef> GetLocalTrackAdapter(MediaStreamComponent* component);
    std::unique_ptr<AdapterRef> GetLocalTrackAdapter(webrtc::MediaStreamTrackInterface* webrtc_track);
    ...
    std::unique_ptr<AdapterRef> GetOrCreateLocalTrackAdapter(MediaStreamComponent* component);
    ...
    std::unique_ptr<AdapterRef> GetRemoteTrackAdapter(MediaStreamComponent* component);
    ...
    // returned.
    std::unique_ptr<AdapterRef> GetRemoteTrackAdapter(webrtc::MediaStreamTrackInterface* webrtc_track);
    ...
    std::unique_ptr<AdapterRef> GetOrCreateRemoteTrackAdapter(scoped_refptr<webrtc::MediaStreamTrackInterface> webrtc_track);
...
private:
    ...
    using LocalTrackAdapterMap = blink::TwoKeysAdapterMap<int, // MediaStreamComponent::UniqueId()
        webrtc::MediaStreamTrackInterface*, scoped_refptr<blink::WebRtcMediaStreamTrackAdapter>>;
