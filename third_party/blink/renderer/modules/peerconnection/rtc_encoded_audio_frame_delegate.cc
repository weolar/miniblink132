...
RTCEncodedAudioFrameDelegate::RTCEncodedAudioFrameDelegate(std::unique_ptr<webrtc::TransformableAudioFrameInterface> webrtc_frame,
    rtc::ArrayView<const unsigned int> contributing_sources, std::optional<uint16_t> sequence_number)
...
DOMArrayBuffer* RTCEncodedAudioFrameDelegate::CreateDataBuffer(v8::Isolate* isolate) const
{
    ArrayBufferContents contents;
    {
        ...
        if (!webrtc_frame_) {
            ...
            buffer->Transfer(isolate, V8AtomicString(isolate, kRTCEncodedAudioFrameDetachKey), contents_to_drop, exception_state);
            return buffer;
        }
...
base::expected<void, String> RTCEncodedAudioFrameDelegate::SetRtpTimestamp(uint32_t timestamp)
{
    base::AutoLock lock(lock_);
    if (!webrtc_frame_) {
...
{
    base::AutoLock lock(lock_);
    return webrtc_frame_ ? std::make_optional(webrtc_frame_->GetSsrc()) : std::nullopt;
}
...
{
    base::AutoLock lock(lock_);
    return webrtc_frame_ ? webrtc_frame_->AbsoluteCaptureTimestamp() : std::nullopt;
}

std::unique_ptr<webrtc::TransformableAudioFrameInterface> RTCEncodedAudioFrameDelegate::PassWebRtcFrame()
{
    base::AutoLock lock(lock_);
    return std::move(webrtc_frame_);
}

std::unique_ptr<webrtc::TransformableAudioFrameInterface> RTCEncodedAudioFrameDelegate::CloneWebRtcFrame()
{
    base::AutoLock lock(lock_);
    if (!webrtc_frame_) {
        return nullptr;
