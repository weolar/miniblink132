...
class MockWebRtcVideoTrackSource : public rtc::RefCountedObject<webrtc::VideoTrackSourceInterface> {
public:
    static scoped_refptr<MockWebRtcVideoTrackSource> Create(bool supports_encoded_output);
...
class MockWebRtcVideoTrack : public rtc::RefCountedObject<webrtc::VideoTrackInterface> {
public:
    static scoped_refptr<MockWebRtcVideoTrack> Create(const std::string& id, scoped_refptr<webrtc::VideoTrackSourceInterface> source = nullptr);
    MockWebRtcVideoTrack(const std::string& id, webrtc::VideoTrackSourceInterface* source);
    void AddOrUpdateSink(rtc::VideoSinkInterface<webrtc::VideoFrame>* sink, const rtc::VideoSinkWants& wants) override;
    void RemoveSink(rtc::VideoSinkInterface<webrtc::VideoFrame>* sink) override;
...
public:
    ...
    bool RemoveTrack(rtc::scoped_refptr<webrtc::AudioTrackInterface> track) override;
    bool RemoveTrack(rtc::scoped_refptr<webrtc::VideoTrackInterface> track) override;
    std::string id() const override;
    webrtc::AudioTrackVector GetAudioTracks() override;
    webrtc::VideoTrackVector GetVideoTracks() override;
    rtc::scoped_refptr<webrtc::AudioTrackInterface> FindAudioTrack(const std::string& track_id) override;
    rtc::scoped_refptr<webrtc::VideoTrackInterface> FindVideoTrack(const std::string& track_id) override;
    void RegisterObserver(webrtc::ObserverInterface* observer) override;
    void UnregisterObserver(webrtc::ObserverInterface* observer) override;
...
// RTC PeerConnections.
class MockPeerConnectionDependencyFactory : public blink::PeerConnectionDependencyFactory {
public:
    MockPeerConnectionDependencyFactory();

    MockPeerConnectionDependencyFactory(const MockPeerConnectionDependencyFactory&) = delete;
    MockPeerConnectionDependencyFactory& operator=(const MockPeerConnectionDependencyFactory&) = delete;
    ...
    rtc::scoped_refptr<webrtc::PeerConnectionInterface> CreatePeerConnection(const webrtc::PeerConnectionInterface::RTCConfiguration& config,
        blink::WebLocalFrame* frame, webrtc::PeerConnectionObserver* observer, ExceptionState& exception_state, RTCRtpTransport* rtp_transport) override;
    scoped_refptr<webrtc::VideoTrackSourceInterface> CreateVideoTrackSourceProxy(webrtc::VideoTrackSourceInterface* source) override;
    scoped_refptr<webrtc::MediaStreamInterface> CreateLocalMediaStream(const String& label) override;
    scoped_refptr<webrtc::VideoTrackInterface> CreateLocalVideoTrack(const String& id, webrtc::VideoTrackSourceInterface* source) override;
    webrtc::IceCandidateInterface* CreateIceCandidate(const String& sdp_mid, int sdp_mline_index, const String& sdp) override;

    scoped_refptr<base::SingleThreadTaskRunner> GetWebRtcNetworkTaskRunner() override;
    scoped_refptr<base::SingleThreadTaskRunner> GetWebRtcSignalingTaskRunner() override;
