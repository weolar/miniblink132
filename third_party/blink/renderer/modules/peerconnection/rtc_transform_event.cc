...
RTCTransformEvent::RTCTransformEvent(ScriptState* script_state, CustomEventMessage data, scoped_refptr<base::SequencedTaskRunner> transform_task_runner,
    CrossThreadWeakHandle<RTCRtpScriptTransform> transform)
    : Event(event_type_names::kRtctransform, Bubbles::kNo, Cancelable::kNo)
    , transformer_(MakeGarbageCollected<RTCRtpScriptTransformer>(script_state, std::move(data), transform_task_runner, std::move(transform)))
{
}
