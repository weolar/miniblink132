...
webrtc::TransportPacketsFeedback CreateFeedback(size_t packet_count, uint32_t feedback_time_ms)
{
    ...
    for (size_t i = 0; i < packet_count; i++) {
        ...
        packet_result.receive_time = webrtc::Timestamp::Millis(feedback_time_ms - packet_count + i);
...
TEST_F(RTCRtpTransportProcessorTest, EmptyReadReceivedAcks)
{
    ...
    RTCRtpTransportProcessor* processor = MakeGarbageCollected<RTCRtpTransportProcessor>(scope_.GetExecutionContext());
...
TEST_F(RTCRtpTransportProcessorTest, ReadReceivedAcksReturnsFewerThanMax)
{
    ...
    HeapVector<Member<RTCRtpAcks>> acks_vector = processor->readReceivedAcks(kMaxCount);
