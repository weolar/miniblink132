...
V8RTCErrorDetailType::Enum RTCErrorDetailToEnum(webrtc::RTCErrorDetailType detail)
{
    switch (detail) {
...
RTCError::RTCError(const RTCErrorInit* init, String message)
    ...
    , sdp_line_number_(init->hasSdpLineNumber() ? std::optional<int32_t>(init->sdpLineNumber()) : std::nullopt)
    , http_request_status_code_(init->hasHttpRequestStatusCode() ? std::optional<int32_t>(init->httpRequestStatusCode()) : std::nullopt)
    , sctp_cause_code_(init->hasSctpCauseCode() ? std::optional<int32_t>(init->sctpCauseCode()) : std::nullopt)
    , received_alert_(init->hasReceivedAlert() ? std::optional<uint32_t>(init->receivedAlert()) : std::nullopt)
    , sent_alert_(init->hasSentAlert() ? std::optional<uint32_t>(init->sentAlert()) : std::nullopt)
{
}
...
RTCError::RTCError(webrtc::RTCError err)
    ...
    , sctp_cause_code_(err.sctp_cause_code() ? std::optional<int32_t>(*err.sctp_cause_code()) : std::nullopt)
{
}
