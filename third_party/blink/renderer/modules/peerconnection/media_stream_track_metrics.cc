...
public:
    MediaStreamTrackMetricsObserver(
        MediaStreamTrackMetrics::Direction direction, MediaStreamTrackMetrics::Kind kind, std::string track_id, MediaStreamTrackMetrics* owner);
    ~MediaStreamTrackMetricsObserver();

    // Sends begin/end messages for the track if not already reported.
    void SendLifetimeMessageForTrack(MediaStreamTrackMetrics::LifetimeEvent event);
...
{
    DCHECK(thread_checker_.CalledOnValidThread());
    SendLifetimeMessageForTrack(MediaStreamTrackMetrics::LifetimeEvent::kDisconnected);
}

void MediaStreamTrackMetricsObserver::SendLifetimeMessageForTrack(MediaStreamTrackMetrics::LifetimeEvent event)
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
void MediaStreamTrackMetrics::AddTrack(Direction direction, Kind kind, const std::string& track_id)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    observers_.push_back(std::make_unique<MediaStreamTrackMetricsObserver>(direction, kind, std::move(track_id), this));
    SendLifeTimeMessageDependingOnIceState(observers_.back().get());
}

void MediaStreamTrackMetrics::RemoveTrack(Direction direction, Kind kind, const std::string& track_id)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    auto it = base::ranges::find_if(observers_, [&](const std::unique_ptr<MediaStreamTrackMetricsObserver>& observer) {
        return direction == observer->direction() && kind == observer->kind() && track_id == observer->track_id();
    });
...
void MediaStreamTrackMetrics::IceConnectionChange(webrtc::PeerConnectionInterface::IceConnectionState new_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void MediaStreamTrackMetrics::SendLifeTimeMessageDependingOnIceState(MediaStreamTrackMetricsObserver* observer)
{
...
void MediaStreamTrackMetrics::SendLifetimeMessage(const std::string& track_id, Kind kind, LifetimeEvent event, Direction direction)
{
    if (event == LifetimeEvent::kConnected) {
        GetMediaStreamTrackMetricsHost()->AddTrack(MakeUniqueId(track_id, direction), kind == Kind::kAudio, direction == Direction::kReceive);
    } else {
        DCHECK_EQ(LifetimeEvent::kDisconnected, event);
        GetMediaStreamTrackMetricsHost()->RemoveTrack(MakeUniqueId(track_id, direction));
    }
}

uint64_t MediaStreamTrackMetrics::MakeUniqueIdImpl(uint64_t pc_id, const std::string& track_id, Direction direction)
{
    ...
    std::string unique_id_string = base::StringPrintf("%" PRIu64 " %s %d", pc_id, track_id.c_str(), direction == Direction::kReceive ? 1 : 0);
...
uint64_t MediaStreamTrackMetrics::MakeUniqueId(const std::string& track_id, Direction direction)
{
    return MakeUniqueIdImpl(reinterpret_cast<uint64_t>(reinterpret_cast<void*>(this)), track_id, direction);
}

mojo::Remote<blink::mojom::blink::MediaStreamTrackMetricsHost>& MediaStreamTrackMetrics::GetMediaStreamTrackMetricsHost()
{
    if (!track_metrics_host_) {
        blink::Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(track_metrics_host_.BindNewPipeAndPassReceiver());
    }
