...
Vector<webrtc::RtpHeaderExtensionCapability> GetNegotiatedHeaderExtensions(const webrtc::RtpTransceiverInterface* webrtc_transceiver)
{
...
TransceiverStateSurfacer::TransceiverStateSurfacer(TransceiverStateSurfacer&& other)
...
void TransceiverStateSurfacer::Initialize(rtc::scoped_refptr<webrtc::PeerConnectionInterface> native_peer_connection,
    ...
    std::vector<rtc::scoped_refptr<webrtc::RtpTransceiverInterface>> webrtc_transceivers)
{
    ...
    sctp_transport_snapshot_.transport = native_peer_connection->GetSctpTransport();
    if (sctp_transport_snapshot_.transport) {
        sctp_transport_snapshot_.sctp_transport_state = sctp_transport_snapshot_.transport->Information();
        if (sctp_transport_snapshot_.sctp_transport_state.dtls_transport()) {
            sctp_transport_snapshot_.dtls_transport_state = sctp_transport_snapshot_.sctp_transport_state.dtls_transport()->Information();
        }
    }
    ...
    for (auto& webrtc_transceiver : webrtc_transceivers) {
        ...
        if (webrtc_sender) {
            ...
            if (webrtc_sender->track()) {
                ...
                sender_track_ref = track_adapter_map->GetLocalTrackAdapter(webrtc_sender->track().get());
                CHECK(sender_track_ref);
            }
            sender_state
                = blink::RtpSenderState(main_task_runner_, signaling_task_runner_, webrtc_sender, std::move(sender_track_ref), webrtc_sender->stream_ids());
        }
        ...
        if (webrtc_receiver) {
            ...
            receiver_state = blink::RtpReceiverState(
                main_task_runner_, signaling_task_runner_, webrtc_receiver.get(), std::move(receiver_track_ref), std::move(receiver_stream_ids));
        }

        // Create the transceiver state.
        transceiver_states_.emplace_back(main_task_runner_, signaling_task_runner_, webrtc_transceiver.get(), std::move(sender_state),
            std::move(receiver_state), webrtc_transceiver->mid(), webrtc_transceiver->direction(), webrtc_transceiver->current_direction(),
            webrtc_transceiver->fired_direction(), GetNegotiatedHeaderExtensions(webrtc_transceiver.get()));
    }
    is_initialized_ = true;
}

blink::WebRTCSctpTransportSnapshot TransceiverStateSurfacer::SctpTransportSnapshot()
{
...
std::vector<blink::RtpTransceiverState> TransceiverStateSurfacer::ObtainStates()
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    DCHECK(is_initialized_);
