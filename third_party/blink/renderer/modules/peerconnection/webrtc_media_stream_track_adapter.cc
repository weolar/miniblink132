...
scoped_refptr<WebRtcMediaStreamTrackAdapter> WebRtcMediaStreamTrackAdapter::CreateLocalTrackAdapter(
    blink::PeerConnectionDependencyFactory* factory, const scoped_refptr<base::SingleThreadTaskRunner>& main_thread, MediaStreamComponent* component)
{
    DCHECK(factory);
...
// static
scoped_refptr<WebRtcMediaStreamTrackAdapter> WebRtcMediaStreamTrackAdapter::CreateRemoteTrackAdapter(blink::PeerConnectionDependencyFactory* factory,
    const scoped_refptr<base::SingleThreadTaskRunner>& main_thread, const scoped_refptr<webrtc::MediaStreamTrackInterface>& webrtc_track)
{
    DCHECK(factory);
    DCHECK(!main_thread->BelongsToCurrentThread());
    DCHECK(webrtc_track);
    ...
    if (webrtc_track->kind() == webrtc::MediaStreamTrackInterface::kAudioKind) {
        remote_track_adapter->InitializeRemoteAudioTrack(
            base::WrapRefCounted(static_cast<webrtc::AudioTrackInterface*>(webrtc_track.get())), factory->GetSupplementable());
    } else {
        DCHECK_EQ(webrtc_track->kind(), webrtc::MediaStreamTrackInterface::kVideoKind);
        remote_track_adapter->InitializeRemoteVideoTrack(
            base::WrapRefCounted(static_cast<webrtc::VideoTrackInterface*>(webrtc_track.get())), factory->GetSupplementable());
    }
    return remote_track_adapter;
}

WebRtcMediaStreamTrackAdapter::WebRtcMediaStreamTrackAdapter(
    blink::PeerConnectionDependencyFactory* factory, const scoped_refptr<base::SingleThreadTaskRunner>& main_thread)
    : factory_(factory)
    , webrtc_signaling_task_runner_(nullptr)
    , main_thread_(main_thread)
    , remote_track_can_complete_initialization_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
...
// static
void WebRtcMediaStreamTrackAdapterTraits::Destruct(const WebRtcMediaStreamTrackAdapter* adapter)
{
    if (!adapter->main_thread_->BelongsToCurrentThread()) {
        PostCrossThreadTask(
            *adapter->main_thread_.get(), FROM_HERE, CrossThreadBindOnce(&WebRtcMediaStreamTrackAdapterTraits::Destruct, CrossThreadUnretained(adapter)));
        return;
    }
    delete adapter;
}
...
rtc::scoped_refptr<webrtc::MediaStreamTrackInterface> WebRtcMediaStreamTrackAdapter::webrtc_track()
{
    DCHECK(main_thread_->BelongsToCurrentThread());
    DCHECK(webrtc_track_);
    EnsureTrackIsInitialized();
    return rtc::scoped_refptr<webrtc::MediaStreamTrackInterface>(webrtc_track_.get());
}
...
void WebRtcMediaStreamTrackAdapter::InitializeLocalAudioTrack(MediaStreamComponent* component)
{
    DCHECK(main_thread_->BelongsToCurrentThread());
    DCHECK(!is_initialized_);
    DCHECK(component);
    DCHECK_EQ(component->GetSourceType(), MediaStreamSource::kTypeAudio);
    SendLogMessage(base::StringPrintf("InitializeLocalAudioTrack({id=%s})", component->Id().Utf8().c_str()));
    ...
    local_track_audio_sink_ = std::make_unique<blink::WebRtcAudioSink>(component_->Id().Utf8(), source_interface, webrtc_signaling_task_runner_, main_thread_);

    if (auto* media_stream_source = blink::ProcessedLocalAudioSource::From(blink::MediaStreamAudioSource::From(component_->Source()))) {
        local_track_audio_sink_->SetLevel(media_stream_source->audio_level());
        local_track_audio_sink_->SetAudioProcessor(media_stream_source->GetAudioProcessor());
    }
...
void WebRtcMediaStreamTrackAdapter::InitializeLocalVideoTrack(MediaStreamComponent* component)
{
    DCHECK(main_thread_->BelongsToCurrentThread());
    DCHECK(!is_initialized_);
    DCHECK(component);
    ...
    auto factory = factory_.Lock();
    DCHECK(factory);
    local_track_video_sink_ = std::make_unique<blink::MediaStreamVideoWebRtcSink>(component_, factory, main_thread_);
...
void WebRtcMediaStreamTrackAdapter::InitializeRemoteAudioTrack(
    const scoped_refptr<webrtc::AudioTrackInterface>& webrtc_audio_track, ExecutionContext* track_execution_context)
{
    DCHECK(!main_thread_->BelongsToCurrentThread());
    DCHECK(!is_initialized_);
    DCHECK(!remote_track_can_complete_initialization_.IsSignaled());
    DCHECK(webrtc_audio_track);
    DCHECK_EQ(webrtc_audio_track->kind(), webrtc::MediaStreamTrackInterface::kAudioKind);
    SendLogMessage(base::StringPrintf("InitializeRemoteAudioTrack([this=%p])", this));
    remote_audio_track_adapter_ = base::MakeRefCounted<blink::RemoteAudioTrackAdapter>(main_thread_, webrtc_audio_track.get(), track_execution_context);
    ...
    PostCrossThreadTask(*main_thread_.get(), FROM_HERE,
        CrossThreadBindOnce(&WebRtcMediaStreamTrackAdapter::FinalizeRemoteTrackInitializationOnMainThread, WrapRefCounted(this)));
}

void WebRtcMediaStreamTrackAdapter::InitializeRemoteVideoTrack(
    const scoped_refptr<webrtc::VideoTrackInterface>& webrtc_video_track, ExecutionContext* track_execution_context)
{
    DCHECK(!main_thread_->BelongsToCurrentThread());
    DCHECK(webrtc_video_track);
    DCHECK_EQ(webrtc_video_track->kind(), webrtc::MediaStreamTrackInterface::kVideoKind);
    remote_video_track_adapter_ = base::MakeRefCounted<blink::RemoteVideoTrackAdapter>(main_thread_, webrtc_video_track.get(), track_execution_context);
...
void WebRtcMediaStreamTrackAdapter::DisposeRemoteAudioTrack()
{
    ...
    PostCrossThreadTask(*webrtc_signaling_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&WebRtcMediaStreamTrackAdapter::UnregisterRemoteAudioTrackAdapterOnSignalingThread, WrapRefCounted(this)));
}
...
void WebRtcMediaStreamTrackAdapter::UnregisterRemoteAudioTrackAdapterOnSignalingThread()
{
    DCHECK(!main_thread_->BelongsToCurrentThread());
    DCHECK(remote_audio_track_adapter_);
    remote_audio_track_adapter_->Unregister();
    PostCrossThreadTask(
        *main_thread_.get(), FROM_HERE, CrossThreadBindOnce(&WebRtcMediaStreamTrackAdapter::FinalizeRemoteTrackDisposingOnMainThread, WrapRefCounted(this)));
}
