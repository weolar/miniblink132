...
RTCEncodedAudioUnderlyingSource::RTCEncodedAudioUnderlyingSource(ScriptState* script_state, WTF::CrossThreadOnceClosure disconnect_callback)
    : blink::RTCEncodedAudioUnderlyingSource(script_state, std::move(disconnect_callback),
...
RTCEncodedAudioUnderlyingSource::RTCEncodedAudioUnderlyingSource(ScriptState* script_state, WTF::CrossThreadOnceClosure disconnect_callback,
    bool enable_frame_restrictions, base::UnguessableToken owner_id, ReadableStreamDefaultControllerWithScriptScope* override_controller)
...
ReadableStreamDefaultControllerWithScriptScope* RTCEncodedAudioUnderlyingSource::GetController()
{
...
ScriptPromise<IDLUndefined> RTCEncodedAudioUnderlyingSource::Pull(ScriptState* script_state, ExceptionState&)
{
...
ScriptPromise<IDLUndefined> RTCEncodedAudioUnderlyingSource::Cancel(ScriptState* script_state, ScriptValue reason, ExceptionState&)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...
void RTCEncodedAudioUnderlyingSource::OnFrameFromSource(std::unique_ptr<webrtc::TransformableAudioFrameInterface> webrtc_frame)
{
    ...
    if (GetController()->DesiredSize() <= kMinQueueDesiredSize) {
        dropped_frames_++;
        VLOG_IF(2, (dropped_frames_ % 20 == 0)) << "Dropped total of " << dropped_frames_ << " encoded audio frames due to too many already being queued.";
        return;
    }
    RTCEncodedAudioFrame* encoded_frame;
    if (enable_frame_restrictions_) {
        encoded_frame = MakeGarbageCollected<RTCEncodedAudioFrame>(std::move(webrtc_frame), owner_id_, ++last_enqueued_frame_counter_);
    } else {
        encoded_frame = MakeGarbageCollected<RTCEncodedAudioFrame>(std::move(webrtc_frame));
    }
...
{
    PostCrossThreadTask(
        *task_runner_, FROM_HERE, CrossThreadBindOnce(&RTCEncodedAudioUnderlyingSource::OnSourceTransferStartedOnTaskRunner, WrapCrossThreadPersistent(this)));
}
