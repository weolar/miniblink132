...
RTCEncodedVideoUnderlyingSource::RTCEncodedVideoUnderlyingSource(ScriptState* script_state, WTF::CrossThreadOnceClosure disconnect_callback)
    : blink::RTCEncodedVideoUnderlyingSource(script_state, std::move(disconnect_callback),
...
ScriptPromise<IDLUndefined> RTCEncodedVideoUnderlyingSource::Pull(ScriptState* script_state, ExceptionState&)
{
...
ScriptPromise<IDLUndefined> RTCEncodedVideoUnderlyingSource::Cancel(ScriptState* script_state, ScriptValue reason, ExceptionState&)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...
ReadableStreamDefaultControllerWithScriptScope* RTCEncodedVideoUnderlyingSource::GetController()
{
...
void RTCEncodedVideoUnderlyingSource::OnFrameFromSource(std::unique_ptr<webrtc::TransformableVideoFrameInterface> webrtc_frame)
{
    ...
    if (GetController()->DesiredSize() <= kMinQueueDesiredSize) {
        dropped_frames_++;
        VLOG_IF(2, (dropped_frames_ % 20 == 0)) << "Dropped total of " << dropped_frames_ << " encoded video frames due to too many already being queued.";
        return;
    }
    RTCEncodedVideoFrame* encoded_frame;
    if (enable_frame_restrictions_) {
        encoded_frame = MakeGarbageCollected<RTCEncodedVideoFrame>(std::move(webrtc_frame), owner_id_, ++last_enqueued_frame_counter_);
    } else {
        encoded_frame = MakeGarbageCollected<RTCEncodedVideoFrame>(std::move(webrtc_frame));
    }
...
{
    PostCrossThreadTask(
        *task_runner_, FROM_HERE, CrossThreadBindOnce(&RTCEncodedVideoUnderlyingSource::OnSourceTransferStartedOnTaskRunner, WrapCrossThreadPersistent(this)));
}
