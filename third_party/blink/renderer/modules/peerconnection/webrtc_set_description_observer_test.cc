...
class WebRtcSetDescriptionObserverForTest : public WebRtcSetDescriptionObserver {
public:
    bool called() const
    {
        return called_;
...
public:
    ObserverHandlerWrapper(ObserverHandlerType handler_type, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner,
        scoped_refptr<base::SingleThreadTaskRunner> signaling_task_runner, rtc::scoped_refptr<webrtc::PeerConnectionInterface> pc,
        scoped_refptr<blink::WebRtcMediaStreamTrackAdapterMap> track_adapter_map, scoped_refptr<WebRtcSetDescriptionObserver> observer)
        ...
        , remote_handler_(nullptr)
    {
        ...
        case ObserverHandlerType::kLocal:
            local_handler_ = WebRtcSetLocalDescriptionObserverHandler::Create(
                std::move(main_task_runner), signaling_task_runner_, std::move(pc), std::move(track_adapter_map), std::move(observer));
            break;
...
private:
    void InvokeLocalHandlerOnSuccess()
    {
        ...
        signaling_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&ObserverHandlerWrapper::InvokeLocalHandlerOnSuccessOnSignalingThread, base::Unretained(this), base::Unretained(&run_loop)));
        run_loop.Run();
    }
    ...
    void InvokeLocalHandlerOnFailure(webrtc::RTCError error)
    {
        ...
        signaling_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(
                &ObserverHandlerWrapper::InvokeLocalHandlerOnFailureOnSignalingThread, base::Unretained(this), std::move(error), base::Unretained(&run_loop)));
        run_loop.Run();
    }
    void InvokeLocalHandlerOnFailureOnSignalingThread(webrtc::RTCError error, base::RunLoop* run_loop)
    {
    ...
    void InvokeRemoteHandlerOnComplete(webrtc::RTCError error)
    {
        base::RunLoop run_loop;
        signaling_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&ObserverHandlerWrapper::InvokeRemoteHandlerOnCompleteOnSignalingThread, base::Unretained(this), std::move(error),
                base::Unretained(&run_loop)));
        run_loop.Run();
    }
    void InvokeRemoteHandlerOnCompleteOnSignalingThread(webrtc::RTCError error, base::RunLoop* run_loop)
    {
...
struct PrintToStringObserverHandlerType {
    std::string operator()(const testing::TestParamInfo<ObserverHandlerType>& info) const
    {
...
class WebRtcSetDescriptionObserverHandlerTest : public ::testing::TestWithParam<ObserverHandlerType> {
public:
    WebRtcSetDescriptionObserverHandlerTest()
        : handler_type_(GetParam())
    {
    }
    ...
    void SetUp() override
    {
        ...
        track_adapter_map_ = base::MakeRefCounted<blink::WebRtcMediaStreamTrackAdapterMap>(dependency_factory_.Get(), main_thread_);
        ...
        observer_handler_ = std::make_unique<ObserverHandlerWrapper>(
            handler_type_, main_thread_, dependency_factory_->GetWebRtcSignalingTaskRunner(), pc_, track_adapter_map_, observer_);
    }
    ...
    {
        auto audio_source = std::make_unique<MediaStreamAudioSource>(blink::scheduler::GetSingleThreadTaskRunnerForTesting(), true);
    ...
    void CreateTransceivers()
    {
        ...
        auto local_track_adapter = track_adapter_map_->GetOrCreateLocalTrackAdapter(component);
        rtc::scoped_refptr<webrtc::MediaStreamTrackInterface> local_track = local_track_adapter->webrtc_track();
        rtc::scoped_refptr<blink::FakeRtpSender> sender(
            new rtc::RefCountedObject<blink::FakeRtpSender>(local_track, std::vector<std::string>({ "local_stream" })));
        ...
        scoped_refptr<blink::MockWebRtcAudioTrack> remote_track = blink::MockWebRtcAudioTrack::Create("remote_track");
        rtc::scoped_refptr<webrtc::MediaStreamInterface> remote_stream(new rtc::RefCountedObject<blink::MockMediaStream>("remote_stream"));
        rtc::scoped_refptr<blink::FakeRtpReceiver> receiver(
            new rtc::RefCountedObject<blink::FakeRtpReceiver>(rtc::scoped_refptr<blink::MockWebRtcAudioTrack>(remote_track.get()),
                std::vector<rtc::scoped_refptr<webrtc::MediaStreamInterface>>({ remote_stream })));
        rtc::scoped_refptr<webrtc::RtpTransceiverInterface> transceiver(new rtc::RefCountedObject<blink::FakeRtpTransceiver>(
            cricket::MEDIA_TYPE_AUDIO, sender, receiver, std::nullopt, false, webrtc::RtpTransceiverDirection::kSendRecv, std::nullopt));
    ...
    void ExpectMatchingTransceivers()
    {
        ...
        const blink::RtpTransceiverState& transceiver_state = observer_->states().transceiver_states[0];
        ...
        EXPECT_EQ(transceiver_state.current_direction(), transceiver->current_direction());
        EXPECT_EQ(transceiver_state.fired_direction(), transceiver->fired_direction());
        // Inspect sender states.
        EXPECT_TRUE(transceiver_state.sender_state());
        const blink::RtpSenderState& sender_state = *transceiver_state.sender_state();
        ...
        const blink::RtpReceiverState& receiver_state = *transceiver_state.receiver_state();
...
protected:
    ...
    std::vector<rtc::scoped_refptr<webrtc::RtpTransceiverInterface>> transceivers_;
    std::vector<std::unique_ptr<blink::WebRtcMediaStreamTrackAdapterMap::AdapterRef>> local_track_adapters_;
};
...
TEST_P(WebRtcSetDescriptionObserverHandlerTest, OnSuccess)
{
    ...
    EXPECT_CALL(*pc_, signaling_state()).WillRepeatedly(Return(webrtc::PeerConnectionInterface::kStable));

    observer_handler_->InvokeOnComplete(webrtc::RTCError::OK());
    EXPECT_TRUE(observer_->called());
    EXPECT_TRUE(observer_->error().ok());

    EXPECT_EQ(webrtc::PeerConnectionInterface::kStable, observer_->states().signaling_state);
...
TEST_P(WebRtcSetDescriptionObserverHandlerTest, OnFailure)
{
    ...
    observer_handler_->InvokeOnComplete(webrtc::RTCError(webrtc::RTCErrorType::INVALID_PARAMETER, "Oh noes!"));
    EXPECT_TRUE(observer_->called());
...
TEST_P(WebRtcSetDescriptionObserverHandlerTest, ClosePeerConnectionBeforeCallback)
{
    CreateTransceivers();

    // Simulate the peer connection having been closed and local track adapters
    // destroyed before the observer was invoked.
    EXPECT_CALL(*pc_, signaling_state()).WillRepeatedly(Return(webrtc::PeerConnectionInterface::kClosed));
    ...
    EXPECT_EQ(webrtc::PeerConnectionInterface::kClosed, observer_->states().signaling_state);

    EXPECT_EQ(0u, observer_->states().transceiver_states.size());
}

INSTANTIATE_TEST_SUITE_P(All, WebRtcSetDescriptionObserverHandlerTest, ::testing::Values(ObserverHandlerType::kLocal, ObserverHandlerType::kRemote),
