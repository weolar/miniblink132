...
public:
    MOCK_METHOD(void, SetProcessor,
        (CrossThreadWeakHandle<RTCRtpTransportProcessor> rtp_transport_processor_handle, scoped_refptr<base::SequencedTaskRunner> task_runner), (override));

    MOCK_METHOD(void, SetCustomMaxBitrateBps, (uint64_t custom_max_bitrate_bps), (override));
};
...
TEST_F(RTCRtpTransportTest, RegisterFeedbackProviderAfterCreateProcessor)
{
    ...
    RTCRtpTransport* transport = MakeGarbageCollected<RTCRtpTransport>(scope_.GetExecutionContext());
    ...
    transport->createProcessor(scope_.GetScriptState(), WorkerObject(), scope_.GetExceptionState());

    auto mock_feedback_provider = base::MakeRefCounted<MockFeedbackProvider>();

    EXPECT_CALL(*mock_feedback_provider, SetProcessor(_, _)).WillOnce(Invoke([&]() { loop.Quit(); }));
