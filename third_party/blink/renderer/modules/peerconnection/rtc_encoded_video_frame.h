...
MODULES_EXPORT BASE_DECLARE_FEATURE(kAllowRTCEncodedVideoFrameSetMetadataAllFields);

class MODULES_EXPORT RTCEncodedVideoFrame final : public ScriptWrappable {
    DEFINE_WRAPPERTYPEINFO();

public:
    static RTCEncodedVideoFrame* Create(RTCEncodedVideoFrame* original_frame, ExceptionState& exception_state);
    static RTCEncodedVideoFrame* Create(RTCEncodedVideoFrame* original_frame, const RTCEncodedVideoFrameOptions* options_dict, ExceptionState& exception_state);
    explicit RTCEncodedVideoFrame(std::unique_ptr<webrtc::TransformableVideoFrameInterface> webrtc_frame);
    explicit RTCEncodedVideoFrame(std::unique_ptr<webrtc::TransformableVideoFrameInterface> webrtc_frame, base::UnguessableToken owner_id, int64_t counter);
    explicit RTCEncodedVideoFrame(scoped_refptr<RTCEncodedVideoFrameDelegate> delegate);
    ...
    base::expected<void, String> SetMetadata(const RTCEncodedVideoFrameMetadata* metadata);
    void setMetadata(RTCEncodedVideoFrameMetadata* metadata, ExceptionState& exception_state);
    ...
    std::unique_ptr<webrtc::TransformableVideoFrameInterface> PassWebRtcFrame(v8::Isolate* isolate, bool detach_frame_data);
