...
webrtc::VideoTrackInterface::ContentHint ContentHintTypeToWebRtcContentHint(WebMediaStreamTrack::ContentHintType content_hint)
{
    switch (content_hint) {
...
void RequestRefreshFrameOnRenderTaskRunner(MediaStreamComponent* component)
{
    if (!component)
        return;
    if (MediaStreamVideoTrack* video_track = MediaStreamVideoTrack::From(component)) {
...
void RequestRefreshFrame(scoped_refptr<base::SingleThreadTaskRunner> task_runner, CrossThreadWeakPersistent<MediaStreamComponent> component)
{
    PostCrossThreadTask(*task_runner, FROM_HERE, CrossThreadBindOnce(RequestRefreshFrameOnRenderTaskRunner, std::move(component)));
}
...
public:
    WebRtcVideoSourceAdapter(const scoped_refptr<base::SingleThreadTaskRunner>& libjingle_network_task_runner,
        const scoped_refptr<WebRtcVideoTrackSource>& source, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    void OnVideoFrameOnIO(scoped_refptr<media::VideoFrame> frame, base::TimeTicks estimated_capture_time);
...
MediaStreamVideoWebRtcSink::WebRtcVideoSourceAdapter::WebRtcVideoSourceAdapter(const scoped_refptr<base::SingleThreadTaskRunner>& libjingle_network_task_runner,
    const scoped_refptr<WebRtcVideoTrackSource>& source, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
void MediaStreamVideoWebRtcSink::WebRtcVideoSourceAdapter::ReleaseSourceOnMainThread()
{
...
void MediaStreamVideoWebRtcSink::WebRtcVideoSourceAdapter::OnVideoFrameOnIO(scoped_refptr<media::VideoFrame> frame, base::TimeTicks estimated_capture_time)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(io_sequence_checker_);
    PostCrossThreadTask(*libjingle_network_task_runner_.get(), FROM_HERE,
        CrossThreadBindOnce(&WebRtcVideoSourceAdapter::OnVideoFrameOnNetworkThread, WrapRefCounted(this), std::move(frame)));
}

void MediaStreamVideoWebRtcSink::WebRtcVideoSourceAdapter::OnNotifyVideoFrameDroppedOnIO(media::VideoCaptureFrameDropReason)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(io_sequence_checker_);
    DVLOG(1) << __func__;
    PostCrossThreadTask(*libjingle_network_task_runner_.get(), FROM_HERE,
        CrossThreadBindOnce(&WebRtcVideoSourceAdapter::OnNotifyVideoFrameDroppedOnNetworkThread, WrapRefCounted(this)));
}

void MediaStreamVideoWebRtcSink::WebRtcVideoSourceAdapter::OnVideoFrameOnNetworkThread(scoped_refptr<media::VideoFrame> frame)
{
...
void MediaStreamVideoWebRtcSink::WebRtcVideoSourceAdapter::OnNotifyVideoFrameDroppedOnNetworkThread()
{
    DCHECK(libjingle_network_task_runner_->BelongsToCurrentThread());
    base::AutoLock auto_lock(video_source_stop_lock_);
    if (video_source_)
...
MediaStreamVideoWebRtcSink::MediaStreamVideoWebRtcSink(
    MediaStreamComponent* component, PeerConnectionDependencyFactory* factory, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    MediaStreamVideoTrack* video_track = MediaStreamVideoTrack::From(component);
    DCHECK(video_track);

    std::optional<bool> needs_denoising = ToAbslOptionalBool(video_track->noise_reduction());

    bool is_screencast = video_track->is_screencast();

    MediaStreamVideoSource* source = video_track->source();
    VideoCaptureFeedbackCB feedback_cb = source ? source->GetFeedbackCallback() : base::DoNothing();
    base::RepeatingClosure request_refresh_frame_closure = source
        ? ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(RequestRefreshFrame, task_runner, WrapCrossThreadWeakPersistent(component)))
        : base::DoNothing();
    ...
    video_source_ = scoped_refptr<WebRtcVideoTrackSource>(new rtc::RefCountedObject<WebRtcVideoTrackSource>(
        is_screencast, needs_denoising, feedback_cb, request_refresh_frame_closure, factory->GetGpuFactories()));
    ...
    video_track_ = factory->CreateLocalVideoTrack(component->Id(), video_source_proxy_.get());

    video_track_->set_content_hint(ContentHintTypeToWebRtcContentHint(component->ContentHint()));
    video_track_->set_enabled(component->Enabled());

    source_adapter_ = base::MakeRefCounted<WebRtcVideoSourceAdapter>(factory->GetWebRtcNetworkTaskRunner(), video_source_.get(), std::move(task_runner));
    ...
    MediaStreamVideoSink::ConnectToTrack(WebMediaStreamTrack(component),
        ...
        MediaStreamVideoSink::IsSecure::kNo, MediaStreamVideoSink::UsesAlpha::kNo);
    video_track->SetSinkNotifyFrameDroppedCallback(
        this, ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&WebRtcVideoSourceAdapter::OnNotifyVideoFrameDroppedOnIO, source_adapter_)));

    DVLOG(3) << "MediaStreamVideoWebRtcSink ctor() : is_screencast " << is_screencast;
}
...
void MediaStreamVideoWebRtcSink::OnContentHintChanged(WebMediaStreamTrack::ContentHintType content_hint)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    video_track_->set_content_hint(ContentHintTypeToWebRtcContentHint(content_hint));
}

void MediaStreamVideoWebRtcSink::OnVideoConstraintsChanged(std::optional<double> min_fps, std::optional<double> max_fps)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOG(3) << __func__ << " min " << min_fps.value_or(-1) << " max " << max_fps.value_or(-1);
    video_source_proxy_->ProcessConstraints(webrtc::VideoTrackSourceConstraints { min_fps, max_fps });
}

std::optional<bool> MediaStreamVideoWebRtcSink::SourceNeedsDenoisingForTesting() const
{
