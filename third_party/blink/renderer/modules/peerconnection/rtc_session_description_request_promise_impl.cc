...
RTCSessionDescriptionRequestPromiseImpl* RTCSessionDescriptionRequestPromiseImpl::Create(
    RTCPeerConnection* requester, ScriptPromiseResolver<RTCSessionDescriptionInit>* resolver)
{
    return MakeGarbageCollected<RTCSessionDescriptionRequestPromiseImpl>(requester, resolver);
}
...
RTCSessionDescriptionRequestPromiseImpl::~RTCSessionDescriptionRequestPromiseImpl() = default;

void RTCSessionDescriptionRequestPromiseImpl::RequestSucceeded(RTCSessionDescriptionPlatform* platform_session_description)
{
...
void RTCSessionDescriptionRequestPromiseImpl::RequestFailed(const webrtc::RTCError& error)
{
    if (requester_ && requester_->ShouldFireDefaultCallbacks()) {
