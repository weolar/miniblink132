...
class MockPeerConnectionDependencyFactory2 : public MockPeerConnectionDependencyFactory {
public:
    MOCK_METHOD(scoped_refptr<webrtc::VideoTrackSourceInterface>, CreateVideoTrackSourceProxy, (webrtc::VideoTrackSourceInterface * source), (override));
};
...
public:
    ...
    MOCK_METHOD(void, ProcessConstraints, (const webrtc::VideoTrackSourceConstraints& constraints), (override));
};
...
public:
    ...
    {
        registry_.Init();
        MockMediaStreamVideoSource* source = registry_.AddVideoTrack("test video track");
    ...
    {
        registry_.Init();
        registry_.AddVideoTrack("test video track", blink::VideoTrackAdapterSettings(), noise_reduction, false, 0.0);
        CompleteSetVideoTrack();
    }

    MockMediaStreamVideoSource* SetVideoTrackWithMaxFramerate(int max_frame_rate)
    {
        registry_.Init();
        MockMediaStreamVideoSource* source
            = registry_.AddVideoTrack("test video track", blink::VideoTrackAdapterSettings(gfx::Size(100, 100), max_frame_rate), std::nullopt, false, 0.0);
        CompleteSetVideoTrack();
        return source;
    }
...
protected:
    ...
    Persistent<MockPeerConnectionDependencyFactory> dependency_factory_ = MakeGarbageCollected<MockPeerConnectionDependencyFactory>();
...
{
    SetVideoTrack();
    blink::MediaStreamVideoWebRtcSink my_sink(component_, dependency_factory_.Get(), blink::scheduler::GetSingleThreadTaskRunnerForTesting());
...
TEST_F(MediaStreamVideoWebRtcSinkTest, NotifiesFrameDropped)
{
    ...
    EXPECT_CALL(mock_sink, OnDiscardedFrame).WillOnce([&] { std::move(quit_closure).Run(); });
    scoped_refptr<media::VideoFrame> frame1 = media::VideoFrame::CreateBlackFrame(gfx::Size(100, 100));
    frame1->set_timestamp(base::Milliseconds(1));
    mock_source->DeliverVideoFrame(frame1);
    scoped_refptr<media::VideoFrame> frame2 = media::VideoFrame::CreateBlackFrame(gfx::Size(100, 100));
...
TEST_F(MediaStreamVideoWebRtcSinkTest, ForwardsConstraintsChangeToWebRtcVideoTrackSourceProxy)
{
    Persistent<MockPeerConnectionDependencyFactory2> dependency_factory2 = MakeGarbageCollected<MockPeerConnectionDependencyFactory2>();
    dependency_factory_ = dependency_factory2;
    MockVideoTrackSourceProxy* source_proxy = nullptr;
    EXPECT_CALL(*dependency_factory2, CreateVideoTrackSourceProxy).WillOnce(Invoke([&source_proxy](webrtc::VideoTrackSourceInterface*) {
        source_proxy = new MockVideoTrackSourceProxy();
        return source_proxy;
    }));
    SetVideoTrack();
    blink::MediaStreamVideoWebRtcSink sink(component_, dependency_factory_.Get(), blink::scheduler::GetSingleThreadTaskRunnerForTesting());
    ASSERT_TRUE(source_proxy != nullptr);
    Mock::VerifyAndClearExpectations(dependency_factory_);

    EXPECT_CALL(*source_proxy,
        ProcessConstraints(
            AllOf(Field(&webrtc::VideoTrackSourceConstraints::min_fps, Optional(12.0)), Field(&webrtc::VideoTrackSourceConstraints::max_fps, Optional(34.0)))));
...
TEST_F(MediaStreamVideoWebRtcSinkTest, RequestsRefreshFrameFromSource)
{
    ...
    MediaStreamVideoWebRtcSink sink(component_, dependency_factory_.Get(), blink::scheduler::GetSingleThreadTaskRunnerForTesting());
