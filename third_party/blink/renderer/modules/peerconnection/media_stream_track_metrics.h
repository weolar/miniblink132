...
public:
    ...
    void IceConnectionChange(webrtc::PeerConnectionInterface::IceConnectionState new_state);
    ...
    // PeerConnection).
    virtual void SendLifetimeMessage(const std::string& track_id, Kind kind, LifetimeEvent lifetime_event, Direction direction);

protected:
    // Calls SendLifetimeMessage for |observer| depending on |ice_state_|.
    void SendLifeTimeMessageDependingOnIceState(MediaStreamTrackMetricsObserver* observer);
    ...
    uint64_t MakeUniqueIdImpl(uint64_t pc_id, const std::string& track, Direction direction);
...
private:
    ...
    mojo::Remote<blink::mojom::blink::MediaStreamTrackMetricsHost>& GetMediaStreamTrackMetricsHost();

    mojo::Remote<blink::mojom::blink::MediaStreamTrackMetricsHost> track_metrics_host_;
