...
ScriptPromise<IDLAny> InternalsRTCPeerConnection::waitForPeerConnectionDispatchEventsTaskCreated(
    ScriptState* script_state, Internals& internals, RTCPeerConnection* connection)
{
    ...
    connection->dispatch_events_task_created_callback_for_testing_
        = WTF::BindOnce([](ScriptPromiseResolver<IDLAny>* resolver) { resolver->Resolve(); }, WrapPersistent(resolver));
    return promise;
}
