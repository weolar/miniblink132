...
MockDataChannel::MockDataChannel(const std::string& label, const webrtc::DataChannelInit* config)
    : label_(label)
...
void MockDataChannel::SendAsync(webrtc::DataBuffer buffer, absl::AnyInvocable<void(webrtc::RTCError) &&> on_complete)
{
    if (!on_complete) {
        return;
    }
    std::move(on_complete)(state_ == webrtc::DataChannelInterface::kOpen ? webrtc::RTCError::OK() : webrtc::RTCError(webrtc::RTCErrorType::INVALID_STATE));
}
