...
public:
    static PeriodicSyncEvent* Create(const AtomicString& type, const String& tag, WaitUntilObserver* observer)
    {
        return MakeGarbageCollected<PeriodicSyncEvent>(type, tag, observer);
    }
    static PeriodicSyncEvent* Create(const AtomicString& type, const PeriodicSyncEventInit* init)
    {
        return MakeGarbageCollected<PeriodicSyncEvent>(type, init);
    }

    PeriodicSyncEvent(const AtomicString& type, const String& tag, WaitUntilObserver* observer);
    PeriodicSyncEvent(const AtomicString& type, const PeriodicSyncEventInit* init);
