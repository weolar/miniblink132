...
const char ServiceWorkerRegistrationSync::kSupplementName[] = "ServiceWorkerRegistrationSync";

ServiceWorkerRegistrationSync& ServiceWorkerRegistrationSync::From(ServiceWorkerRegistration& registration)
{
    ServiceWorkerRegistrationSync* supplement = Supplement<ServiceWorkerRegistration>::From<ServiceWorkerRegistrationSync>(registration);
    if (!supplement) {
        supplement = MakeGarbageCollected<ServiceWorkerRegistrationSync>(&registration);
...
SyncManager* ServiceWorkerRegistrationSync::sync()
{
    if (!sync_manager_) {
        ...
        sync_manager_ = MakeGarbageCollected<SyncManager>(GetSupplementable(), execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI));
    }
...
PeriodicSyncManager* ServiceWorkerRegistrationSync::periodicSync()
{
    if (!periodic_sync_manager_) {
        ExecutionContext* execution_context = GetSupplementable()->GetExecutionContext();
        // TODO(falken): Consider defining a task source in the spec for this event.
        periodic_sync_manager_ = MakeGarbageCollected<PeriodicSyncManager>(GetSupplementable(), execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI));
    }
