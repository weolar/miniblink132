...
Lock::Lock(ScriptState* script_state, const String& name, mojom::blink::LockMode mode, mojo::PendingAssociatedRemote<mojom::blink::LockHandle> handle,
    mojo::PendingRemote<mojom::blink::ObservedFeature> lock_lifetime, LockManager* manager)
    ...
    , manager_(manager)
{
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = ExecutionContext::From(script_state)->GetTaskRunner(TaskType::kWebLocks);
    handle_.Bind(std::move(handle), task_runner);
    lock_lifetime_.Bind(std::move(lock_lifetime), task_runner);
    handle_.set_disconnect_handler(WTF::BindOnce(&Lock::OnConnectionError, WrapWeakPersistent(this)));
}
...
void Lock::HoldUntil(ScriptPromise<IDLAny> promise, ScriptPromiseResolver<IDLAny>* resolver)
{
    DCHECK(!resolver_);
    ...
    resolver_ = resolver;
    promise.React(
        script_state, MakeGarbageCollected<ThenFunction>(this, ThenFunction::kFulfilled), MakeGarbageCollected<ThenFunction>(this, ThenFunction::kRejected));
}
...
void Lock::OnConnectionError()
{
    ...
    resolver_->Reject(
        V8ThrowDOMException::CreateOrDie(script_state->GetIsolate(), DOMExceptionCode::kAbortError, "Lock broken by another request with the 'steal' option."));
}
