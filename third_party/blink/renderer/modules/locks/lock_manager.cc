...
HeapVector<Member<LockInfo>> ToLockInfos(const Vector<mojom::blink::LockInfoPtr>& records)
{
...
class LockManager::LockRequestImpl final : public GarbageCollected<LockRequestImpl>, public NameClient, public mojom::blink::LockRequest {
public:
    LockRequestImpl(V8LockGrantedCallback* callback, ScriptPromiseResolver<IDLAny>* resolver, const String& name, mojom::blink::LockMode mode,
        mojo::PendingAssociatedReceiver<mojom::blink::LockRequest> receiver, mojo::PendingRemote<mojom::blink::ObservedFeature> lock_lifetime,
        LockManager* manager)
        : callback_(callback)
        , resolver_(resolver)
        , name_(name)
        ...
        , manager_(manager)
    {
        receiver_.Bind(std::move(receiver), manager->GetExecutionContext()->GetTaskRunner(TaskType::kWebLocks));
    }
    ...
    void Abort(AbortSignal* signal)
    {
        ...
        if (!IsInParallelAlgorithmRunnable(resolver_->GetExecutionContext(), script_state)) {
            return;
        }
    ...
    void Granted(mojo::PendingAssociatedRemote<mojom::blink::LockHandle> handle_remote) override
    {
        ...
        Lock* lock = MakeGarbageCollected<Lock>(script_state, name_, mode_, std::move(handle_remote), std::move(lock_lifetime_), manager_);
        ...
        if (try_catch.HasCaught()) {
            lock->HoldUntil(ScriptPromise<IDLAny>::Reject(script_state, try_catch.Exception()), resolver_);
        } else if (!result.IsNothing()) {
            lock->HoldUntil(ToResolvedPromise<IDLAny>(script_state, result.FromJust()), resolver_);
        }
    }
...
private:
    ...
    HeapMojoAssociatedReceiver<mojom::blink::LockRequest, LockRequestImpl> receiver_;
...
void LockManager::SetManager(mojo::PendingRemote<mojom::blink::LockManager> manager, ExecutionContext* execution_context)
{
    service_.Bind(std::move(manager), execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI));
}

ScriptPromise<IDLAny> LockManager::request(ScriptState* script_state, const String& name, V8LockGrantedCallback* callback, ExceptionState& exception_state)
{
    return request(script_state, name, LockOptions::Create(), callback, exception_state);
}

ScriptPromise<IDLAny> LockManager::request(
    ScriptState* script_state, const String& name, const LockOptions* options, V8LockGrantedCallback* callback, ExceptionState& exception_state)
{
    // Observed context may be gone if frame is detached.
    if (!GetExecutionContext()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, kInvalidStateErrorMessage);
        return EmptyPromise();
    }
    ...
    context->GetScheduler()->RegisterStickyFeature(blink::SchedulingPolicy::Feature::kWebLocks, { blink::SchedulingPolicy::DisableBackForwardCache() });
    ...
    if (!context->GetSecurityOrigin()->CanAccessLocks() && !context->IsSharedStorageWorkletGlobalScope()) {
        exception_state.ThrowSecurityError("Access to the Locks API is denied in this context.");
        return EmptyPromise();
    }
    ...
    if (name.StartsWith("-")) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Names cannot start with '-'.");
        return EmptyPromise();
    }
    ...
    if (options->steal() && options->ifAvailable()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "The 'steal' and 'ifAvailable' options cannot be used together.");
        return EmptyPromise();
    }
    ...
    if (options->steal() && mode != mojom::blink::LockMode::EXCLUSIVE) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "The 'steal' option may only be used with 'exclusive' locks.");
        return EmptyPromise();
    }
    ...
    if (options->hasSignal() && options->ifAvailable()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "The 'signal' and 'ifAvailable' options cannot be used together.");
        return EmptyPromise();
    }
    if (options->hasSignal() && options->steal()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "The 'signal' and 'steal' options cannot be used together.");
        return EmptyPromise();
    }
    ...
    if (options->hasSignal() && options->signal()->aborted()) {
        return ScriptPromise<IDLAny>::Reject(script_state, options->signal()->reason(script_state));
    }

    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLAny>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();

    CheckStorageAccessAllowed(context, resolver,
        resolver->WrapCallbackInScriptScope(
            WTF::BindOnce(&LockManager::RequestImpl, WrapWeakPersistent(this), WrapPersistent(options), name, WrapPersistent(callback), mode)));

    // 12. Return promise.
    return promise;
}

void LockManager::RequestImpl(
    const LockOptions* options, const String& name, V8LockGrantedCallback* callback, mojom::blink::LockMode mode, ScriptPromiseResolver<IDLAny>* resolver)
{
    ExecutionContext* context = resolver->GetExecutionContext();
    if (!service_.is_bound()) {
        context->GetBrowserInterfaceBroker().GetInterface(service_.BindNewPipeAndPassReceiver(context->GetTaskRunner(TaskType::kMiscPlatformAPI)));
    ...
    if (!observer_.is_bound()) {
        context->GetBrowserInterfaceBroker().GetInterface(observer_.BindNewPipeAndPassReceiver(context->GetTaskRunner(TaskType::kMiscPlatformAPI)));
    ...
    mojom::blink::LockManager::WaitMode wait = options->steal() ? mojom::blink::LockManager::WaitMode::PREEMPT
    ...
    observer_->Register(lock_lifetime.InitWithNewPipeAndPassReceiver(), mojom::blink::ObservedFeatureType::kWebLock);
    ...
    LockRequestImpl* request = MakeGarbageCollected<LockRequestImpl>(
        callback, resolver, name, mode, request_remote.InitWithNewEndpointAndPassReceiver(), std::move(lock_lifetime), this);
    ...
    if (options->hasSignal()) {
        ...
        AbortSignal::AlgorithmHandle* handle
            = options->signal()->AddAlgorithm(WTF::BindOnce(&LockRequestImpl::Abort, WrapWeakPersistent(request), WrapPersistent(options->signal())));
        request->InitializeAbortAlgorithm(*handle);
    }
    service_->RequestLock(name, mode, wait, std::move(request_remote));
}

ScriptPromise<LockManagerSnapshot> LockManager::query(ScriptState* script_state, ExceptionState& exception_state)
{
    // Observed context may be gone if frame is detached.
    if (!GetExecutionContext()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, kInvalidStateErrorMessage);
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<LockManagerSnapshot>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();

    CheckStorageAccessAllowed(context, resolver, resolver->WrapCallbackInScriptScope(WTF::BindOnce(&LockManager::QueryImpl, WrapWeakPersistent(this))));
    return promise;
}

void LockManager::QueryImpl(ScriptPromiseResolver<LockManagerSnapshot>* resolver)
{
    ExecutionContext* context = resolver->GetExecutionContext();
    if (!service_.is_bound()) {
        context->GetBrowserInterfaceBroker().GetInterface(service_.BindNewPipeAndPassReceiver(context->GetTaskRunner(TaskType::kMiscPlatformAPI)));
    ...
    service_->QueryState(WTF::BindOnce(
        [](ScriptPromiseResolver<LockManagerSnapshot>* resolver, Vector<mojom::blink::LockInfoPtr> pending, Vector<mojom::blink::LockInfoPtr> held) {
...
void LockManager::CheckStorageAccessAllowed(ExecutionContext* context, ScriptPromiseResolverBase* resolver, base::OnceCallback<void()> callback)
{
    DCHECK(context->IsWindow() || context->IsWorkerGlobalScope() || context->IsSharedStorageWorkletGlobalScope());

    auto wrapped_callback = WTF::BindOnce(&LockManager::DidCheckStorageAccessAllowed, WrapWeakPersistent(this), WrapPersistent(resolver), std::move(callback));
    ...
    if (auto* window = DynamicTo<LocalDOMWindow>(context)) {
        ...
        frame->AllowStorageAccessAndNotify(WebContentSettingsClient::StorageType::kWebLocks, std::move(wrapped_callback));
    } else if (auto* worker_global_scope = DynamicTo<WorkerGlobalScope>(context)) {
        WebContentSettingsClient* content_settings_client = worker_global_scope->ContentSettingsClient();
        if (!content_settings_client) {
            std::move(wrapped_callback).Run(true);
            return;
        }
        content_settings_client->AllowStorageAccess(WebContentSettingsClient::StorageType::kWebLocks, std::move(wrapped_callback));
    } else {
...
void LockManager::DidCheckStorageAccessAllowed(ScriptPromiseResolverBase* resolver, base::OnceCallback<void()> callback, bool allow_access)
{
    if (cached_allowed_.has_value()) {
    ...
    resolver->Reject(V8ThrowDOMException::CreateOrDie(script_state->GetIsolate(), DOMExceptionCode::kSecurityError, kSecurityErrorMessage));
}
