...
{
    ukm::builders::BrowsingTopics_DocumentBrowsingTopicsApiResult2 builder(document.UkmSourceID());

    builder.SetFailureReason(static_cast<int64_t>(browsing_topics::ApiAccessResult::kInvalidRequestingContext));
...
// static
const char BrowsingTopicsDocumentSupplement::kSupplementName[] = "BrowsingTopicsDocumentSupplement";

// static
BrowsingTopicsDocumentSupplement* BrowsingTopicsDocumentSupplement::From(Document& document)
{
    auto* supplement = Supplement<Document>::From<BrowsingTopicsDocumentSupplement>(document);
    if (!supplement) {
        supplement = MakeGarbageCollected<BrowsingTopicsDocumentSupplement>(document);
...
// static
ScriptPromise<IDLSequence<BrowsingTopic>> BrowsingTopicsDocumentSupplement::browsingTopics(
    ScriptState* script_state, Document& document, ExceptionState& exception_state)
{
    auto* supplement = From(document);
    return supplement->GetBrowsingTopics(script_state, document, BrowsingTopicsOptions::Create(), exception_state);
}

// static
ScriptPromise<IDLSequence<BrowsingTopic>> BrowsingTopicsDocumentSupplement::browsingTopics(
    ScriptState* script_state, Document& document, const BrowsingTopicsOptions* options, ExceptionState& exception_state)
{
    auto* supplement = From(document);
    return supplement->GetBrowsingTopics(script_state, document, options, exception_state);
}

BrowsingTopicsDocumentSupplement::BrowsingTopicsDocumentSupplement(Document& document)
    : Supplement<Document>(document)
    , document_host_(document.GetExecutionContext())
{
}

ScriptPromise<IDLSequence<BrowsingTopic>> BrowsingTopicsDocumentSupplement::GetBrowsingTopics(
    ScriptState* script_state, Document& document, const BrowsingTopicsOptions* options, ExceptionState& exception_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<BrowsingTopic>>>(script_state, exception_state.GetContext());
    ...
    if (document.GetFrame()->IsInFencedFrameTree()) {
        resolver->Reject(V8ThrowDOMException::CreateOrEmpty(
            script_state->GetIsolate(), DOMExceptionCode::kInvalidAccessError, "document.browsingTopics() is not allowed in a fenced frame."));
        RecordInvalidRequestingContextUkmMetrics(document);
        return promise;
    }
    ...
    if (!document.GetExecutionContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kBrowsingTopics)) {
        resolver->Reject(V8ThrowDOMException::CreateOrEmpty(script_state->GetIsolate(), DOMExceptionCode::kInvalidAccessError,
    ...
    if (!document.GetExecutionContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kBrowsingTopicsBackwardCompatible)) {
        resolver->Reject(V8ThrowDOMException::CreateOrEmpty(script_state->GetIsolate(), DOMExceptionCode::kInvalidAccessError,
    ...
    if (!document_host_.is_bound()) {
        execution_context->GetBrowserInterfaceBroker().GetInterface(
            document_host_.BindNewPipeAndPassReceiver(execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI)));
    }

    document_host_->GetBrowsingTopics(
        /*observe=*/!options->skipObservation(),
        WTF::BindOnce(
            [](ScriptPromiseResolver<IDLSequence<BrowsingTopic>>* resolver, BrowsingTopicsDocumentSupplement* supplement, base::TimeTicks start_time,
                ...
                if (result->is_error_message()) {
                    ...
                    resolver->Reject(
                        V8ThrowDOMException::CreateOrEmpty(script_state->GetIsolate(), DOMExceptionCode::kInvalidAccessError, result->get_error_message()));
                    return;
                }
                ...
                base::UmaHistogramTimes("BrowsingTopics.JavaScriptAPI.TimeToResolve", time_to_resolve);
