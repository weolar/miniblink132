...
BluetoothLEScan::BluetoothLEScan(mojo::ReceiverId id, Bluetooth* bluetooth, mojom::blink::WebBluetoothRequestLEScanOptionsPtr options)
    : id_(id)
    , bluetooth_(bluetooth)
    , keep_repeated_devices_(options ? options->keep_repeated_devices : false)
    , accept_all_advertisements_(options ? options->accept_all_advertisements : false)
{
        ...
        for (const auto& filter : options->filters.value()) {
            ...
            if (filter->services && filter->services.has_value()) {
                ...
                for (const auto& uuid : filter->services.value()) {
                    services.push_back(MakeGarbageCollected<V8UnionStringOrUnsignedLong>(uuid));
                }
...
const HeapVector<Member<BluetoothLEScanFilterInit>>& BluetoothLEScan::filters() const
{
    return filters_;
}
