...
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(NameToAssignedNumberMap, characteristics_map,
        ({
...
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(NameToAssignedNumberMap, descriptors_map,
        ({
...
String GetUUIDForGATTAttribute(GATTAttribute attribute, const V8UnionStringOrUnsignedLong* name, ExceptionState& exception_state)
{
    DCHECK(name);
    ...
    error_message.Append("'. It must be a valid UUID alias (e.g. 0x1234), "
                         "UUID (lowercase hex characters e.g. "
                         "'00001234-0000-1000-8000-00805f9b34fb'), "
                         "or recognized standard name from ");
    switch (attribute) {
    case GATTAttribute::kService:
        error_message.Append("https://www.bluetooth.com/specifications/gatt/services"
                             " e.g. 'alert_notification'.");
        break;
    case GATTAttribute::kCharacteristic:
        error_message.Append("https://www.bluetooth.com/specifications/gatt/characteristics"
                             " e.g. 'aerobic_heart_rate_lower_limit'.");
        break;
    case GATTAttribute::kDescriptor:
        error_message.Append("https://www.bluetooth.com/specifications/gatt/descriptors"
...
// static
String BluetoothUUID::getService(const V8BluetoothServiceUUID* name, ExceptionState& exception_state)
{
    return GetUUIDForGATTAttribute(GATTAttribute::kService, name, exception_state);
}

// static
String BluetoothUUID::getCharacteristic(const V8BluetoothCharacteristicUUID* name, ExceptionState& exception_state)
{
    return GetUUIDForGATTAttribute(GATTAttribute::kCharacteristic, name, exception_state);
}

// static
String BluetoothUUID::getDescriptor(const V8BluetoothDescriptorUUID* name, ExceptionState& exception_state)
{
    return GetUUIDForGATTAttribute(GATTAttribute::kDescriptor, name, exception_state);
}
