...
const char kInvalidStateErrorMessage[] = "Pending watch advertisements operation.";

BluetoothDevice::BluetoothDevice(ExecutionContext* context, mojom::blink::WebBluetoothDevicePtr device, Bluetooth* bluetooth)
    : ExecutionContextClient(context)
    , ActiveScriptWrappable<BluetoothDevice>({})
    , attribute_instance_map_(MakeGarbageCollected<BluetoothAttributeInstanceMap>(this))
...
BluetoothRemoteGATTService* BluetoothDevice::GetOrCreateRemoteGATTService(
    mojom::blink::WebBluetoothRemoteGATTServicePtr service, bool is_primary, const String& device_instance_id)
{
    return attribute_instance_map_->GetOrCreateRemoteGATTService(std::move(service), is_primary, device_instance_id);
}
...
BluetoothRemoteGATTCharacteristic* BluetoothDevice::GetOrCreateRemoteGATTCharacteristic(
    ExecutionContext* context, mojom::blink::WebBluetoothRemoteGATTCharacteristicPtr characteristic, BluetoothRemoteGATTService* service)
{
    return attribute_instance_map_->GetOrCreateRemoteGATTCharacteristic(context, std::move(characteristic), service);
}

bool BluetoothDevice::IsValidCharacteristic(const String& characteristic_instance_id)
{
    return attribute_instance_map_->ContainsCharacteristic(characteristic_instance_id);
}

BluetoothRemoteGATTDescriptor* BluetoothDevice::GetOrCreateBluetoothRemoteGATTDescriptor(
    mojom::blink::WebBluetoothRemoteGATTDescriptorPtr descriptor, BluetoothRemoteGATTCharacteristic* characteristic)
{
    return attribute_instance_map_->GetOrCreateBluetoothRemoteGATTDescriptor(std::move(descriptor), characteristic);
}
...
void BluetoothDevice::ClearAttributeInstanceMapAndFireEvent()
{
    ...
    DispatchEvent(*Event::CreateBubble(event_type_names::kGattserverdisconnected));
}
...
ScriptPromise<IDLUndefined> BluetoothDevice::watchAdvertisements(
    ScriptState* script_state, const WatchAdvertisementsOptions* options, ExceptionState& exception_state)
{
    ...
    if (options->hasSignal()) {
        ...
        if (options->signal()->aborted()) {
            ...
            exception_state.ThrowDOMException(DOMExceptionCode::kAbortError, kAbortErrorMessage);
            return EmptyPromise();
        }
        ...
        if (!abort_handle_map_.Contains(options->signal())) {
            auto* handle = options->signal()->AddAlgorithm(
                WTF::BindOnce(&BluetoothDevice::AbortWatchAdvertisements, WrapWeakPersistent(this), WrapWeakPersistent(options->signal())));
    ...
    // 'pending-watch'.
    watch_advertisements_resolver_ = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state, exception_state.GetContext());
    mojo::PendingAssociatedRemote<mojom::blink::WebBluetoothAdvertisementClient> client;
    client_receiver_.Bind(client.InitWithNewEndpointAndPassReceiver(), context->GetTaskRunner(TaskType::kMiscPlatformAPI));
    ...
    bluetooth_->Service()->WatchAdvertisementsForDevice(
        device_->id, std::move(client), WTF::BindOnce(&BluetoothDevice::WatchAdvertisementsCallback, WrapPersistent(this)));
...
void BluetoothDevice::AbortWatchAdvertisements(AbortSignal* signal)
{
    ...
    if (watch_advertisements_resolver_) {
        ...
        watch_advertisements_resolver_->Reject(
            V8ThrowDOMException::CreateOrEmpty(script_state->GetIsolate(), DOMExceptionCode::kAbortError, kAbortErrorMessage));
...
ScriptPromise<IDLUndefined> BluetoothDevice::forget(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    bluetooth_->Service()->ForgetDevice(
        device_->id, WTF::BindOnce([](ScriptPromiseResolver<IDLUndefined>* resolver) { resolver->Resolve(); }, WrapPersistent(resolver)));

    return promise;
}

void BluetoothDevice::AdvertisingEvent(mojom::blink::WebBluetoothAdvertisingEventPtr advertising_event)
{
    auto* event = MakeGarbageCollected<BluetoothAdvertisingEvent>(event_type_names::kAdvertisementreceived, this, std::move(advertising_event));
    DispatchEvent(*event);
}
...
void BluetoothDevice::AddedEventListener(const AtomicString& event_type, RegisteredEventListener& registered_listener)
{
    EventTarget::AddedEventListener(event_type, registered_listener);
    if (event_type == event_type_names::kGattserverdisconnected) {
        UseCounter::Count(GetExecutionContext(), WebFeature::kGATTServerDisconnectedEvent);
    }
}

void BluetoothDevice::WatchAdvertisementsCallback(mojom::blink::WebBluetoothResult result)
{
    // https://webbluetoothcg.github.io/web-bluetooth/#dom-bluetoothdevice-watchadvertisements
    ...
    if (result != mojom::blink::WebBluetoothResult::SUCCESS) {
        ...
        watch_advertisements_resolver_->Reject(BluetoothError::CreateDOMException(result));
        watch_advertisements_resolver_.Clear();
        return;
    }
