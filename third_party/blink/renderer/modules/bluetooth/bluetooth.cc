...
const char kHandleGestureForPermissionRequest[] = "Must be handling a user gesture to show a permission request.";
const char kFencedFrameError[] = "Web Bluetooth is not allowed in a fenced frame tree.";
const char kPermissionsPolicyBlocked[] = "Access to the feature \"bluetooth\" is disallowed by permissions policy.";
...
bool IsRequestDenied(LocalDOMWindow* window, ExceptionState& exception_state)
{
    if (!window) {
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, kFencedFrameError);
    } else if (window->GetFrame()->Top()->GetSecurityContext()->GetSecurityOrigin()->IsOpaque()) {
        exception_state.ThrowSecurityError("Access to the Web Bluetooth API is denied from contexts where the "
...
{
    return window->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kBluetooth, ReportOptions::kReportOnFailure);
}
...
{
#if !BUILDFLAG(IS_CHROMEOS_ASH) && !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_MAC) && !BUILDFLAG(IS_WIN)
    context->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kInfo,
...
void CanonicalizeFilter(
    const BluetoothLEScanFilterInit* filter, mojom::blink::WebBluetoothLeScanFilterPtr& canonicalized_filter, ExceptionState& exception_state)
{
    if (!(filter->hasServices() || filter->hasName() || filter->hasNamePrefix() || filter->hasManufacturerData())) {
        exception_state.ThrowTypeError("A filter must restrict the devices in some way.");
        return;
    }

    if (filter->hasServices()) {
        if (filter->services().size() == 0) {
            exception_state.ThrowTypeError("'services', if present, must contain at least one service.");
            return;
        }
        canonicalized_filter->services.emplace();
        for (const V8UnionStringOrUnsignedLong* service : filter->services()) {
            const String& validated_service = BluetoothUUID::getService(service, exception_state);
    ...
    if (filter->hasNamePrefix()) {
        ...
        if (filter->namePrefix().length() == 0) {
            exception_state.ThrowTypeError("'namePrefix', if present, must be non-empty.");
            return;
        }
    ...
    if (filter->hasManufacturerData()) {
        if (filter->manufacturerData().size() == 0) {
            exception_state.ThrowTypeError("'manufacturerData', if present, must be non-empty.");
            return;
        }
        ...
        for (const auto& manufacturer_data : filter->manufacturerData()) {
            ...
            if (manufacturer_data->hasDataPrefix()) {
                data_prefix_buffer = DOMArrayPiece(manufacturer_data->dataPrefix()).ByteSpan();
            }
            ...
            if (mask_buffer.has_value()) {
                if (!data_prefix_buffer.has_value()) {
                    exception_state.ThrowTypeError("'dataPrefix' must be non-empty when 'mask' is present.");
                    return;
                }

                if (data_prefix_buffer->size() != mask_buffer->size()) {
                    exception_state.ThrowTypeError("'mask' size must be equal to 'dataPrefix' size.");
                    return;
                }
            }
            ...
            if (data_prefix_buffer.has_value()) {
                if (data_prefix_buffer->size() == 0) {
                    exception_state.ThrowTypeError("'dataPrefix', if present, must be non-empty.");
                    return;
                }
                ...
                for (size_t i = 0; i < data_prefix_buffer->size(); ++i) {
                    ...
                    const uint8_t mask = mask_buffer.has_value() ? (*mask_buffer)[i] : 0xff;
                    data_filters_vector.push_back(mojom::blink::WebBluetoothDataFilter::New(data, mask));
                }
            }

            auto company = mojom::blink::WebBluetoothCompany::New();
            company->id = manufacturer_data->companyIdentifier();
            auto result = canonicalized_filter->manufacturer_data->insert(std::move(company), std::move(data_filters_vector));
...
void ConvertRequestDeviceOptions(const RequestDeviceOptions* options, mojom::blink::WebBluetoothRequestDeviceOptionsPtr& result,
    ExecutionContext* execution_context, ExceptionState& exception_state)
{
    if (options->hasExclusionFilters() && !options->hasFilters()) {
        exception_state.ThrowTypeError("'filters' member must be present if 'exclusionFilters' is present.");
        return;
    }

    if (!(options->hasFilters() ^ options->acceptAllDevices())) {
        exception_state.ThrowTypeError("Either 'filters' should be present or 'acceptAllDevices' should be "
                                       "true, but not both.");
        return;
    }
    ...
    if (options->hasFilters()) {
        if (options->filters().empty()) {
            exception_state.ThrowTypeError("'filters' member must be non-empty to find any devices.");
            return;
        }
        ...
        for (const BluetoothLEScanFilterInit* filter : options->filters()) {
            ...
            if (canonicalized_filter->manufacturer_data) {
                UseCounter::Count(execution_context, WebFeature::kWebBluetoothManufacturerDataFilter);
            }
    ...
    if (options->hasExclusionFilters()) {
        if (options->exclusionFilters().empty()) {
            exception_state.ThrowTypeError("'exclusionFilters' member must be non-empty to exclude any device.");
            return;
        }

        result->exclusion_filters.emplace();

        for (const BluetoothLEScanFilterInit* filter : options->exclusionFilters()) {
            auto canonicalized_filter = mojom::blink::WebBluetoothLeScanFilter::New();
    ...
    if (options->hasOptionalServices()) {
        for (const V8UnionStringOrUnsignedLong* optional_service : options->optionalServices()) {
            const String& validated_optional_service = BluetoothUUID::getService(optional_service, exception_state);
            if (exception_state.HadException())
                return;
    ...
    if (options->hasOptionalManufacturerData()) {
        for (const uint16_t manufacturer_code : options->optionalManufacturerData()) {
...
ScriptPromise<IDLBoolean> Bluetooth::getAvailability(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    service_->GetAvailability(
        WTF::BindOnce([](ScriptPromiseResolver<IDLBoolean>* resolver, bool result) { resolver->Resolve(result); }, WrapPersistent(resolver)));
    return promise;
}

void Bluetooth::GetDevicesCallback(ScriptPromiseResolver<IDLSequence<BluetoothDevice>>* resolver, Vector<mojom::blink::WebBluetoothDevicePtr> devices)
{
    ...
    for (auto& device : devices) {
        BluetoothDevice* bluetooth_device = GetBluetoothDeviceRepresentingDevice(std::move(device), resolver->GetExecutionContext());
...
void Bluetooth::RequestDeviceCallback(
    ScriptPromiseResolver<BluetoothDevice>* resolver, mojom::blink::WebBluetoothResult result, mojom::blink::WebBluetoothDevicePtr device)
{
    if (!resolver->GetExecutionContext() || resolver->GetExecutionContext()->IsContextDestroyed()) {
        return;
    }
...
ScriptPromise<IDLSequence<BluetoothDevice>> Bluetooth::getDevices(ScriptState* script_state, ExceptionState& exception_state)
{
    LocalDOMWindow* window = GetSupplementable()->DomWindow();
    ...
    EnsureServiceConnection(window);
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<BluetoothDevice>>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();

    service_->GetDevices(WTF::BindOnce(&Bluetooth::GetDevicesCallback, WrapPersistent(this), WrapPersistent(resolver)));
    return promise;
}

// https://webbluetoothcg.github.io/web-bluetooth/#dom-bluetooth-requestdevice
ScriptPromise<BluetoothDevice> Bluetooth::requestDevice(ScriptState* script_state, const RequestDeviceOptions* options, ExceptionState& exception_state)
{
    LocalDOMWindow* window = GetSupplementable()->DomWindow();
    ...
    ConvertRequestDeviceOptions(options, device_options, GetExecutionContext(), exception_state);

    if (exception_state.HadException())
        return EmptyPromise();

    // Subsequent steps are handled in the browser process.
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<BluetoothDevice>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();

    service_->RequestDevice(std::move(device_options), WTF::BindOnce(&Bluetooth::RequestDeviceCallback, WrapPersistent(this), WrapPersistent(resolver)));
    return promise;
}

static void ConvertRequestLEScanOptions(
    const BluetoothLEScanOptions* options, mojom::blink::WebBluetoothRequestLEScanOptionsPtr& result, ExceptionState& exception_state)
{
    if (!(options->hasFilters() ^ options->acceptAllAdvertisements())) {
        exception_state.ThrowTypeError("Either 'filters' should be present or 'acceptAllAdvertisements' "
...
void Bluetooth::RequestScanningCallback(ScriptPromiseResolver<BluetoothLEScan>* resolver, mojo::ReceiverId id,
    mojom::blink::WebBluetoothRequestLEScanOptionsPtr options, mojom::blink::WebBluetoothResult result)
{
    if (!resolver->GetExecutionContext() || resolver->GetExecutionContext()->IsContextDestroyed()) {
        return;
    }
    ...
    auto* scan = MakeGarbageCollected<BluetoothLEScan>(id, this, std::move(options));
    resolver->Resolve(scan);
}

// https://webbluetoothcg.github.io/web-bluetooth/scanning.html#dom-bluetooth-requestlescan
ScriptPromise<BluetoothLEScan> Bluetooth::requestLEScan(ScriptState* script_state, const BluetoothLEScanOptions* options, ExceptionState& exception_state)
{
    LocalDOMWindow* window = GetSupplementable()->DomWindow();
    ...
    // platforms.
    window->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kInfo,
    ...
    // Subsequent steps are handled in the browser process.
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<BluetoothLEScan>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();

    mojo::PendingAssociatedRemote<mojom::blink::WebBluetoothAdvertisementClient> client;
    // See https://bit.ly/2S0zRAS for task types.
    mojo::ReceiverId id = client_receivers_.Add(client.InitWithNewEndpointAndPassReceiver(), window->GetTaskRunner(TaskType::kMiscPlatformAPI));

    auto scan_options_copy = scan_options->Clone();
    service_->RequestScanningStart(std::move(client), std::move(scan_options),
        WTF::BindOnce(&Bluetooth::RequestScanningCallback, WrapPersistent(this), WrapPersistent(resolver), id, std::move(scan_options_copy)));

    return promise;
}

void Bluetooth::AdvertisingEvent(mojom::blink::WebBluetoothAdvertisingEventPtr advertising_event)
{
    auto* event = MakeGarbageCollected<BluetoothAdvertisingEvent>(event_type_names::kAdvertisementreceived,
        GetBluetoothDeviceRepresentingDevice(std::move(advertising_event->device), GetExecutionContext()), std::move(advertising_event));
    DispatchEvent(*event);
}
...
BluetoothDevice* Bluetooth::GetBluetoothDeviceRepresentingDevice(mojom::blink::WebBluetoothDevicePtr device_ptr, ExecutionContext* context)
{
    // TODO(crbug.com/1275634): convert device_instance_map_ to use
    // WebBluetoothDeviceId as key
    auto it = device_instance_map_.find(device_ptr->id.DeviceIdInBase64().c_str());
    if (it != device_instance_map_.end()) {
        return it->value.Get();
    }

    BluetoothDevice* device = MakeGarbageCollected<BluetoothDevice>(context, std::move(device_ptr), this);
    auto result = device_instance_map_.insert(device->GetDevice()->id.DeviceIdInBase64().c_str(), device);
...
void Bluetooth::EnsureServiceConnection(ExecutionContext* context)
{
    if (!service_.is_bound()) {
        ...
        context->GetBrowserInterfaceBroker().GetInterface(service_.BindNewPipeAndPassReceiver(task_runner));
    }
}
