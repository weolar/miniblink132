...
class BluetoothDevice final : public EventTarget,
...
public:
    BluetoothDevice(ExecutionContext*, mojom::blink::WebBluetoothDevicePtr, Bluetooth*);

    BluetoothRemoteGATTService* GetOrCreateRemoteGATTService(mojom::blink::WebBluetoothRemoteGATTServicePtr, bool is_primary, const String& device_instance_id);
    ...
    BluetoothRemoteGATTCharacteristic* GetOrCreateRemoteGATTCharacteristic(
        ExecutionContext*, mojom::blink::WebBluetoothRemoteGATTCharacteristicPtr, BluetoothRemoteGATTService*);
    ...
    BluetoothRemoteGATTDescriptor* GetOrCreateBluetoothRemoteGATTDescriptor(
        mojom::blink::WebBluetoothRemoteGATTDescriptorPtr, BluetoothRemoteGATTCharacteristic*);
    ...
    ScriptPromise<IDLUndefined> watchAdvertisements(ScriptState*, const WatchAdvertisementsOptions*, ExceptionState&);
    ScriptPromise<IDLUndefined> forget(ScriptState*, ExceptionState&);
    String id()
    {
        return device_->id.DeviceIdInBase64().c_str();
    }
    String name()
    {
        return device_->name;
    ...
    DEFINE_ATTRIBUTE_EVENT_LISTENER(gattserverdisconnected, kGattserverdisconnected)
...
private:
    ...
    HeapMojoAssociatedReceiver<mojom::blink::WebBluetoothAdvertisementClient, BluetoothDevice> client_receiver_;
    ...
    HeapHashMap<WeakMember<AbortSignal>, Member<AbortSignal::AlgorithmHandle>> abort_handle_map_;
};
