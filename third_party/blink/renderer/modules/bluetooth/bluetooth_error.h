...
enum class BluetoothErrorCode { kInvalidService, kInvalidCharacteristic, kInvalidDescriptor, kServiceNotFound, kCharacteristicNotFound, kDescriptorNotFound };
...
public:
    static String CreateNotConnectedExceptionMessage(BluetoothOperation operation);
    static DOMException* CreateNotConnectedException(BluetoothOperation);
    static DOMException* CreateDOMException(BluetoothErrorCode, const String& detailed_message);

    static DOMException* CreateDOMException(mojom::blink::WebBluetoothResult error);
};
