...
public:
    static WorkletAnimation* Create(ScriptState* script_state, const String& animator_name, const V8UnionAnimationEffectOrAnimationEffectSequence* effects,
        ExceptionState& exception_state);
    static WorkletAnimation* Create(ScriptState* script_state, const String& animator_name, const V8UnionAnimationEffectOrAnimationEffectSequence* effects,
        const V8UnionDocumentTimelineOrScrollTimeline* timeline, ExceptionState& exception_state);
    static WorkletAnimation* Create(ScriptState* script_state, const String& animator_name, const V8UnionAnimationEffectOrAnimationEffectSequence* effects,
        const V8UnionDocumentTimelineOrScrollTimeline* timeline, const ScriptValue& options, ExceptionState& exception_state);

    WorkletAnimation(WorkletAnimationId id, const String& animator_name, Document&, const HeapVector<Member<KeyframeEffect>>&, AnimationTimeline*,
        scoped_refptr<SerializedScriptValue> options);
    ~WorkletAnimation() override = default;

    String animatorName()
    {
        return animator_name_;
    }
    ...
    AnimationTimeline* GetTimeline() const override
    {
        return timeline_.Get();
    }
    const String& Name()
    ...
    void SetOutputState(const AnimationWorkletOutput::AnimationState& state) override;
...
private:
    ...
    V8AnimationPlayState::Enum PlayState() const
    {
        return play_state_;
    }
    void SetPlayState(V8AnimationPlayState::Enum state)
    {
        play_state_ = state;
    }
    ...
    V8AnimationPlayState::Enum last_play_state_ = V8AnimationPlayState::Enum::kIdle;
