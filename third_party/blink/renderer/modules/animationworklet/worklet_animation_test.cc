...
#define EXPECT_TIME_NEAR(expected, value) EXPECT_NEAR(expected, value, time_error_ms)
...
KeyframeEffectModelBase* CreateEffectModel()
{
    ...
    frames_mixed_properties.push_back(keyframe);
    return MakeGarbageCollected<StringKeyframeEffectModel>(frames_mixed_properties);
}
...
KeyframeEffect* CreateKeyframeEffect(Element* element)
{
    ...
    return MakeGarbageCollected<KeyframeEffect>(element, CreateEffectModel(), timing);
}

WorkletAnimation* CreateWorkletAnimation(ScriptState* script_state, Element* element, const String& animator_name, ScrollTimeline* scroll_timeline = nullptr)
{
    auto* effects = MakeGarbageCollected<V8UnionAnimationEffectOrAnimationEffectSequence>(CreateKeyframeEffect(element));
    V8UnionDocumentTimelineOrScrollTimeline* timeline = nullptr;
    if (scroll_timeline) {
        timeline = MakeGarbageCollected<V8UnionDocumentTimelineOrScrollTimeline>(scroll_timeline);
    }
    ScriptValue options;

    ScriptState::Scope scope(script_state);
    return WorkletAnimation::Create(script_state, animator_name, effects, timeline, options, ASSERT_NO_EXCEPTION);
}
...
public:
    ...
    void SetUp() override
    {
        ...
        GetDocument().GetWorkletAnimationController().SynchronizeAnimatorName(animator_name_);
        worklet_animation_ = CreateWorkletAnimation(GetScriptState(), element_, animator_name_);
    ...
    {
        base::TimeTicks tick = base::TimeTicks() + GetDocument().Timeline().CalculateZeroTime().since_origin() + ToTimeDelta(milliseconds);
        GetDocument().GetAnimationClock().UpdateTime(tick);
        GetDocument().GetWorkletAnimationController().UpdateAnimationStates();
        GetDocument().GetWorkletAnimationController().UpdateAnimationTimings(kTimingUpdateForAnimationFrame);
    }
...
TEST_F(WorkletAnimationTest, WorkletAnimationInElementAnimations)
{
    ...
    EXPECT_EQ(1u, element_->EnsureElementAnimations().GetWorkletAnimations().size());
    worklet_animation_->cancel();
    EXPECT_EQ(0u, element_->EnsureElementAnimations().GetWorkletAnimations().size());
}
...
{
    const ComputedStyle* style1 = GetDocument().GetStyleResolver().ResolveStyle(element_, StyleRecalcContext());
    EXPECT_FALSE(style1->HasCurrentOpacityAnimation());
    worklet_animation_->play(ASSERT_NO_EXCEPTION);
    const ComputedStyle* style2 = GetDocument().GetStyleResolver().ResolveStyle(element_, StyleRecalcContext());
    EXPECT_TRUE(style2->HasCurrentOpacityAnimation());
}

TEST_F(WorkletAnimationTest, CurrentTimeFromDocumentTimelineIsOffsetByStartTime)
{
    ...
    std::unique_ptr<AnimationWorkletDispatcherInput> state = std::make_unique<AnimationWorkletDispatcherInput>();
    worklet_animation_->UpdateInputState(state.get());
    // First state request sets the start time and thus current time should be 0.
    std::unique_ptr<AnimationWorkletInput> input = state->TakeWorkletState(id.worklet_id);
...
TEST_F(WorkletAnimationTest, DISABLED_CurrentTimeFromScrollTimelineNotOffsetByStartTime)
{
    ...
    WorkletAnimation* worklet_animation = CreateWorkletAnimation(GetScriptState(), element_, animator_name_, scroll_timeline);
    ...
    scrollable_area->SetScrollOffset(ScrollOffset(0, 70), mojom::blink::ScrollType::kProgrammatic);
    GetPage().Animator().ServiceScriptedAnimations(base::TimeTicks::Now());
    ASSERT_TRUE(worklet_animation->currentTime().has_value());
...
TEST_F(WorkletAnimationTest, DocumentTimelineSetPlaybackRate)
{
    ...
    // timeline time.
    ASSERT_TRUE(worklet_animation_->currentTime().has_value());
    EXPECT_TIME_NEAR(123.4 * playback_rate, worklet_animation_->currentTime().value());
}
...
TEST_F(WorkletAnimationTest, DocumentTimelineSetPlaybackRateWhilePlaying)
{
    ...
    ASSERT_TRUE(worklet_animation_->currentTime().has_value());
    EXPECT_TIME_NEAR(123.4 + 200.0 * playback_rate, worklet_animation_->currentTime().value());
}
...
TEST_F(WorkletAnimationTest, PausePlay)
{
    SimulateFrame(0);
    worklet_animation_->play(ASSERT_NO_EXCEPTION);
    EXPECT_EQ(V8AnimationPlayState::Enum::kPending, worklet_animation_->PlayState());
    SimulateFrame(0);
    EXPECT_EQ(V8AnimationPlayState::Enum::kRunning, worklet_animation_->PlayState());
    EXPECT_TRUE(worklet_animation_->Playing());
    EXPECT_TIME_NEAR(0, worklet_animation_->currentTime().value());
    SimulateFrame(10);
    worklet_animation_->pause(ASSERT_NO_EXCEPTION);
    EXPECT_EQ(V8AnimationPlayState::Enum::kPaused, worklet_animation_->PlayState());
...
TEST_F(WorkletAnimationTest, DISABLED_ScrollTimelineSetPlaybackRate)
{
    ...
    // timeline time.
    ASSERT_TRUE(worklet_animation->currentTime().has_value());
    EXPECT_TIME_NEAR(40 + 20 * playback_rate, worklet_animation->currentTime().value());
}
...
TEST_F(WorkletAnimationTest, DISABLED_ScrollTimelineSetPlaybackRateWhilePlaying)
{
    ...
    GetPage().Animator().ServiceScriptedAnimations(base::TimeTicks::Now());
    ASSERT_TRUE(worklet_animation->currentTime().has_value());
    EXPECT_TIME_NEAR(40 + 40 * playback_rate, worklet_animation->currentTime().value());
}
...
TEST_F(WorkletAnimationTest, DISABLED_ScrollTimelineNewlyActive)
{
    ...
    scroller_element->setAttribute(html_names::kStyleAttr, AtomicString("overflow:scroll;width:100px;height:100px;"));
...
TEST_F(WorkletAnimationTest, DISABLED_ScrollTimelineNewlyInactive)
{
    ...
    scroller_element->setAttribute(html_names::kStyleAttr, AtomicString("overflow:visible;width:100px;height:100px;"));
    UpdateAllLifecyclePhasesForTest();
    GetPage().Animator().ServiceScriptedAnimations(base::TimeTicks::Now());
    ASSERT_FALSE(scroll_timeline->IsActive());
