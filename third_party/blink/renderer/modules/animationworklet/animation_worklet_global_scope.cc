...
void UpdateAnimation(v8::Isolate* isolate, Animator* animator, WorkletAnimationId id, double current_time, AnimationWorkletDispatcherOutput* result)
{
...
AnimationWorkletGlobalScope::AnimationWorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams> creation_params, WorkerThread* thread)
...
void AnimationWorkletGlobalScope::Dispose()
{
    ...
    if (AnimationWorkletProxyClient* proxy_client = AnimationWorkletProxyClient::From(Clients()))
        proxy_client->Dispose();
    WorkletGlobalScope::Dispose();
}

Animator* AnimationWorkletGlobalScope::CreateAnimatorFor(int animation_id, const String& name, WorkletAnimationOptions options,
    scoped_refptr<SerializedScriptValue> serialized_state, const Vector<std::optional<base::TimeDelta>>& local_times, const Vector<Timing>& timings,
    const Vector<Timing::NormalizedTiming>& normalized_timings)
{
    DCHECK(!animators_.Contains(animation_id));
    Animator* animator = CreateInstance(name, options, serialized_state, local_times, timings, normalized_timings);
    if (!animator)
        return nullptr;
...
void AnimationWorkletGlobalScope::UpdateAnimatorsList(const AnimationWorkletInput& input)
{
    DCHECK(IsContextThread());
    ...
    for (const auto& animation : input.added_and_updated_animations) {
        ...
        if (animation.options) {
            options = *(static_cast<WorkletAnimationOptions*>(animation.options.get()));
        }

        // Down casting to blink type
        WorkletAnimationEffectTimings* effect_timings = (static_cast<WorkletAnimationEffectTimings*>(animation.effect_timings.get()));
        Vector<Timing> timings = effect_timings->GetTimings()->data;
        DCHECK_GE(timings.size(), 1u);
        Vector<Timing::NormalizedTiming> normalized_timings = effect_timings->GetNormalizedTimings()->data;
        DCHECK_GE(normalized_timings.size(), 1u);

        Vector<std::optional<base::TimeDelta>> local_times(static_cast<int>(timings.size()), std::nullopt);

        CreateAnimatorFor(id, name, options, nullptr /* serialized_state */, local_times, timings, normalized_timings);
    }
}

void AnimationWorkletGlobalScope::UpdateAnimators(const AnimationWorkletInput& input, AnimationWorkletOutput* output, bool (*predicate)(Animator*))
{
    DCHECK(IsContextThread());
    ...
    for (const auto& animation : input.added_and_updated_animations) {
        ...
        UpdateAnimation(isolate, animator, animation.worklet_animation_id, animation.current_time, output);
    }
...
void AnimationWorkletGlobalScope::registerAnimator(const String& name, V8AnimatorConstructor* animator_ctor, ExceptionState& exception_state)
{
    RegisterWithProxyClientIfNeeded();

    DCHECK(IsContextThread());
    if (animator_definitions_.Contains(name)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "A class with name:'" + name + "' is already registered.");
        return;
    }
    ...
    if (!animator_ctor->IsConstructor()) {
        exception_state.ThrowTypeError("The provided callback is not a constructor.");
        return;
    }
    ...
    if (exception_state.HadException())
        return;
    v8::Local<v8::Function> v8_animate = retriever.GetMethodOrThrow("animate", exception_state);
    if (exception_state.HadException())
        return;
    V8AnimateCallback* animate = V8AnimateCallback::Create(v8_animate);

    v8::Local<v8::Value> v8_state = retriever.GetMethodOrUndefined("state", exception_state);
    if (exception_state.HadException())
        return;

    V8StateCallback* state = v8_state->IsFunction() ? V8StateCallback::Create(v8_state.As<v8::Function>()) : nullptr;

    AnimatorDefinition* definition = MakeGarbageCollected<AnimatorDefinition>(animator_ctor, animate, state);
...
Animator* AnimationWorkletGlobalScope::CreateInstance(const String& name, WorkletAnimationOptions options,
    scoped_refptr<SerializedScriptValue> serialized_state, const Vector<std::optional<base::TimeDelta>>& local_times, const Vector<Timing>& timings,
    const Vector<Timing::NormalizedTiming>& normalized_timings)
{
    DCHECK(IsContextThread());
    ...
    v8::Local<v8::Value> v8_options = options.GetData() ? options.GetData()->Deserialize(isolate) : v8::Undefined(isolate).As<v8::Value>();
    v8::Local<v8::Value> v8_state = serialized_state ? serialized_state->Deserialize(isolate) : v8::Undefined(isolate).As<v8::Value>();
    ScriptValue options_value(isolate, v8_options);
    ScriptValue state_value(isolate, v8_state);

    ScriptValue instance;
    if (!definition->ConstructorFunction()->Construct(options_value, state_value).To(&instance)) {
        return nullptr;
    }

    return MakeGarbageCollected<Animator>(isolate, definition, instance.V8Value(), name, std::move(options), local_times, timings, normalized_timings);
}
...
// the state function.
void AnimationWorkletGlobalScope::MigrateAnimatorsTo(AnimationWorkletGlobalScope* target_global_scope)
{
    ...
    for (const auto& animator_map : animators_) {
        ...
        if (animator->IsStateful()) {
            ...
            // case of exceptions.
            v8::Local<v8::Value> state = animator->State(isolate, PassThroughException(isolate));
            ...
            if (!state->IsNullOrUndefined()) {
                serialized_state = SerializedScriptValue::Serialize(isolate, state, SerializedScriptValue::SerializeOptions(), PassThroughException(isolate));
                if (try_catch.HasCaught()) {
                    continue;
                }
            }
        ...
        target_global_scope->CreateAnimatorFor(animation_id, animator->name(), animator->options(), serialized_state, std::move(local_times),
            animator->GetTimings(), animator->GetNormalizedTimings());
    }
    animators_.clear();
}

AnimatorDefinition* AnimationWorkletGlobalScope::FindDefinitionForTest(const String& name)
{
