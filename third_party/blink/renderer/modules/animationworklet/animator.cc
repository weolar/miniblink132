...
Animator::Animator(v8::Isolate* isolate, AnimatorDefinition* definition, v8::Local<v8::Value> instance, const String& name, WorkletAnimationOptions options,
    const Vector<std::optional<base::TimeDelta>>& local_times, const Vector<Timing>& timings, const Vector<Timing::NormalizedTiming>& normalized_timings)
    : definition_(definition)
    , instance_(isolate, instance)
    , name_(name)
    , options_(options)
    , group_effect_(MakeGarbageCollected<WorkletGroupEffect>(local_times, timings, normalized_timings))
{
...
bool Animator::Animate(v8::Isolate* isolate, double current_time, AnimationWorkletDispatcherOutput::AnimationState* output)
{
    ...
    if (group_effect_->getChildren().size() == 1) {
        effect = MakeGarbageCollected<V8UnionWorkletAnimationEffectOrWorkletGroupEffect>(group_effect_->getChildren()[0]);
    } else {
        effect = MakeGarbageCollected<V8UnionWorkletAnimationEffectOrWorkletGroupEffect>(group_effect_);
    }

    v8::TryCatch try_catch(isolate);
    try_catch.SetVerbose(true);
    if (definition_->AnimateFunction()->Invoke(instance, current_time, effect).IsNothing()) {
        return false;
    }
...
Vector<Timing::NormalizedTiming> Animator::GetNormalizedTimings() const
{
    ...
    normalized_timings.ReserveInitialCapacity(group_effect_->getChildren().size());
    for (const auto& effect : group_effect_->getChildren()) {
...
v8::Local<v8::Value> Animator::State(v8::Isolate* isolate, ExceptionState& exception_state)
{
    if (!IsStateful())
