...
bool ConvertAnimationEffects(
    const V8UnionAnimationEffectOrAnimationEffectSequence* effects, HeapVector<Member<KeyframeEffect>>& keyframe_effects, String& error_string)
{
    DCHECK(effects);
    ...
    case V8UnionAnimationEffectOrAnimationEffectSequence::ContentType::kAnimationEffect: {
    ...
    case V8UnionAnimationEffectOrAnimationEffectSequence::ContentType::kAnimationEffectSequence: {
        const HeapVector<Member<AnimationEffect>>& effect_sequence = effects->GetAsAnimationEffectSequence();
        keyframe_effects.ReserveInitialCapacity(effect_sequence.size());
        for (const auto& effect : effect_sequence) {
            KeyframeEffect* keyframe_effect = DynamicTo<KeyframeEffect>(effect.Get());
            if (!keyframe_effect) {
    ...
    if (keyframe_effects.size() > 1 && !RuntimeEnabledFeatures::GroupEffectEnabled()) {
...
bool ValidateTimeline(const V8UnionDocumentTimelineOrScrollTimeline* timeline, String& error_string)
{
    if (!timeline)
        return true;
...
AnimationTimeline* ConvertAnimationTimeline(const Document& document, const V8UnionDocumentTimelineOrScrollTimeline* timeline)
{
    if (!timeline)
        return &document.Timeline();
    switch (timeline->GetContentType()) {
    case V8UnionDocumentTimelineOrScrollTimeline::ContentType::kDocumentTimeline:
...
void StartEffectOnCompositor(CompositorAnimation* animation, KeyframeEffect* effect)
{
    DCHECK(effect);
    DCHECK(effect->EffectTarget());
    Element& target = *effect->EffectTarget();
    effect->Model()->SnapshotAllCompositorKeyframesIfNecessary(target, target.ComputedStyleRef(), target.ParentComputedStyle());
    ...
    effect->StartAnimationOnCompositor(group, start_time, base::TimeDelta(), playback_rate, animation);
}
...
{
    return time ? time->InMillisecondsF() : std::numeric_limits<double>::quiet_NaN();
}

// Calculates start time backwards from the current time and
// timeline.currentTime.
std::optional<base::TimeDelta> CalculateStartTime(base::TimeDelta current_time, double playback_rate, AnimationTimeline& timeline)
{
    ...
    std::optional<double> timeline_current_time_ms = timeline.CurrentTimeMilliseconds();
    return base::Milliseconds(timeline_current_time_ms.value()) - (current_time / playback_rate);
}

} // namespace

WorkletAnimation* WorkletAnimation::Create(
    ScriptState* script_state, const String& animator_name, const V8UnionAnimationEffectOrAnimationEffectSequence* effects, ExceptionState& exception_state)
{
    return Create(script_state, animator_name, effects, nullptr, ScriptValue(), exception_state);
}

WorkletAnimation* WorkletAnimation::Create(ScriptState* script_state, const String& animator_name,
    const V8UnionAnimationEffectOrAnimationEffectSequence* effects, const V8UnionDocumentTimelineOrScrollTimeline* timeline, ExceptionState& exception_state)
{
    return Create(script_state, animator_name, effects, timeline, ScriptValue(), exception_state);
}

WorkletAnimation* WorkletAnimation::Create(ScriptState* script_state, const String& animator_name,
    const V8UnionAnimationEffectOrAnimationEffectSequence* effects, const V8UnionDocumentTimelineOrScrollTimeline* timeline, const ScriptValue& options,
    ExceptionState& exception_state)
{
    ...
    if (!ConvertAnimationEffects(effects, keyframe_effects, error_string)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, error_string);
        return nullptr;
    }
    ...
    if (!document.GetWorkletAnimationController().IsAnimatorRegistered(animator_name)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The animator '" + animator_name + "' has not yet been registered.");
        return nullptr;
    }

    AnimationWorklet* worklet = CSSAnimationWorklet::animationWorklet(script_state);

    WorkletAnimationId id = worklet->NextWorkletAnimationId();

    AnimationTimeline* animation_timeline = ConvertAnimationTimeline(document, timeline);

    scoped_refptr<SerializedScriptValue> animation_options;
    if (!options.IsEmpty()) {
        animation_options = SerializedScriptValue::Serialize(
            script_state->GetIsolate(), options.V8Value(), SerializedScriptValue::SerializeOptions(SerializedScriptValue::kNotForStorage), exception_state);
        if (exception_state.HadException())
            return nullptr;
    }

    WorkletAnimation* animation
        = MakeGarbageCollected<WorkletAnimation>(id, animator_name, document, keyframe_effects, animation_timeline, std::move(animation_options));

    return animation;
}

WorkletAnimation::WorkletAnimation(WorkletAnimationId id, const String& animator_name, Document& document, const HeapVector<Member<KeyframeEffect>>& effects,
    AnimationTimeline* timeline, scoped_refptr<SerializedScriptValue> options)
    ...
    , effect_needs_restart_(false)
{
    ...
    auto normalized_timings = base::MakeRefCounted<base::RefCountedData<Vector<Timing::NormalizedTiming>>>();
    ...
    effect_timings_ = std::make_unique<WorkletAnimationEffectTimings>(timings, normalized_timings);
}
...
{
    DCHECK(IsMainThread());
    if (play_state_ == V8AnimationPlayState::Enum::kPending || play_state_ == V8AnimationPlayState::Enum::kRunning) {
        return;
    }
    ...
    if (!CheckCanStart(&failure_message)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, failure_message);
        return;
    }
...
void WorkletAnimation::pause(ExceptionState& exception_state)
{
    ...
    std::optional<base::TimeDelta> new_current_time = IsCurrentTimeInitialized() ? CurrentTime() : InitialCurrentTime();
    ...
    if (IsActive(play_state_)) {
        for (auto& effect : effects_) {
            effect->UpdateInheritedTime(std::nullopt,
                /* is_idle */ false, playback_rate_, kTimingUpdateOnDemand);
        }
    }
...
void WorkletAnimation::setPlaybackRate(ScriptState* script_state, double playback_rate)
{
...
{
    if (play_state_ != V8AnimationPlayState::Enum::kRunning && play_state_ != V8AnimationPlayState::Enum::kPaused) {
        return;
    }

    DCHECK_EQ(effects_.size(), local_times_.size());
    for (wtf_size_t i = 0; i < effects_.size(); ++i) {
        effects_[i]->UpdateInheritedTime(local_times_[i] ? std::make_optional(AnimationTimeDelta(local_times_[i].value())) : std::nullopt,
...
void WorkletAnimation::SetCurrentTime(std::optional<base::TimeDelta> current_time)
{
    DCHECK(timeline_);
    ...
    bool should_hold = play_state_ == V8AnimationPlayState::Enum::kPaused || !current_time || !IsTimelineActive();
    if (should_hold) {
        start_time_ = std::nullopt;
        hold_time_ = current_time;
    } else {
        start_time_ = CalculateStartTime(current_time.value(), playback_rate_, *timeline_);
...
void WorkletAnimation::UpdateCompositingState()
{
    ...
    DCHECK(running_on_main_thread_ != !!compositor_animation_) << "Active worklet animation should either run on main or compositor";
}
...
void WorkletAnimation::StartOnMain()
{
    ...
    std::optional<base::TimeDelta> current_time = IsCurrentTimeInitialized() ? CurrentTime() : InitialCurrentTime();
    DCHECK(current_time);
...
bool WorkletAnimation::CanStartOnCompositor()
{
    ...
    GetEffect()->Model()->SnapshotAllCompositorKeyframesIfNecessary(target, target.ComputedStyleRef(), target.ParentComputedStyle());

    CompositorAnimations::FailureReasons failure_reasons = GetEffect()->CheckCanStartAnimationOnCompositor(nullptr, playback_rate_);

    if (failure_reasons != CompositorAnimations::kNoFailure)
        return false;

    // If the scroll source is not composited, fall back to main thread.
    if (timeline_->IsScrollTimeline() && !CompositorAnimations::CanStartScrollTimelineOnCompositor(To<ScrollTimeline>(*timeline_).ResolvedSource())) {
        return false;
    }
...
bool WorkletAnimation::StartOnCompositor()
{
    ...
    if (!compositor_animation_) {
        ...
        compositor_animation_
            = CompositorAnimation::CreateWorkletAnimation(id_, animator_name_, playback_rate_, std::move(options_), std::move(effect_timings_));
    ...
    if (compositor_timeline) {
        if (GetCompositorAnimation()) {
            compositor_timeline->AttachAnimation(GetCompositorAnimation()->CcAnimation());
        }
    ...
    CompositorAnimations::AttachCompositedLayers(*GetEffect()->EffectTarget(), compositor_animation_.get());
...
void WorkletAnimation::DestroyCompositorAnimation()
{
    ...
    if (compositor_timeline && GetCompositorAnimation()) {
        compositor_timeline->DetachAnimation(GetCompositorAnimation()->CcAnimation());
    }
...
std::optional<base::TimeDelta> WorkletAnimation::InitialCurrentTime() const
{
    ...
    std::optional<base::TimeDelta> starting_time = timeline_->InitialStartTimeForAnimations();
    ...
    return (base::Milliseconds(current_time.value()) - starting_time.value()) * playback_rate_;
}
...
void WorkletAnimation::UpdateInputState(AnimationWorkletDispatcherInput* input_state)
{
    std::optional<base::TimeDelta> current_time = CurrentTime();
    if (!running_on_main_thread_) {
        return;
    ...
    if (!was_active && is_active) {
        input_state->Add({ id_, animator_name_.Utf8(), current_time_ms, CloneOptions(), CloneEffectTimings() });
...
void WorkletAnimation::SetOutputState(const AnimationWorkletOutput::AnimationState& state)
{
    ...
    DCHECK(local_times_.size() == state.local_times.size() && running_on_main_thread_);
    for (wtf_size_t i = 0; i < state.local_times.size(); ++i)
        local_times_[i] = state.local_times[i];
}

void WorkletAnimation::NotifyLocalTimeUpdated(std::optional<base::TimeDelta> local_time)
{
