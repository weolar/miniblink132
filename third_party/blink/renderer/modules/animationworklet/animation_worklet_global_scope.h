...
public:
    AnimationWorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams>, WorkerThread*);
    ~AnimationWorkletGlobalScope() override;

    void Trace(Visitor*) const override;
    void Dispose() override;
    bool IsAnimationWorkletGlobalScope() const final
    {
        return true;
    }
    ...
    void UpdateAnimators(const AnimationWorkletInput&, AnimationWorkletOutput*, bool (*predicate)(Animator*));

    // Registers a animator definition with the given name and constructor.
    void registerAnimator(const String& name, V8AnimatorConstructor* animator_ctor, ExceptionState&);
    ...
    unsigned GetAnimatorsSizeForTest()
    {
...
private:
    ...
    Animator* CreateInstance(const String& name, WorkletAnimationOptions options, scoped_refptr<SerializedScriptValue> serialized_state,
    ...
    Animator* CreateAnimatorFor(int animation_id, const String& name, WorkletAnimationOptions options, scoped_refptr<SerializedScriptValue> serialized_state,
        const Vector<std::optional<base::TimeDelta>>& local_times, const Vector<Timing>& timings, const Vector<Timing::NormalizedTiming>& normalized_timings);
...
template <> struct DowncastTraits<AnimationWorkletGlobalScope> {
