...
class MODULES_EXPORT OutgoingStream final : public GarbageCollected<OutgoingStream> {
...
private:
    ...
    ScriptPromise<IDLUndefined> SinkWrite(ScriptState*, ScriptValue chunk, ExceptionState&);

    // Writes |data| to |data_pipe_|, possible saving unwritten data to
    // |cached_data_|.
    ScriptPromise<IDLUndefined> WriteOrCacheData(ScriptState*, base::span<const uint8_t> data);
