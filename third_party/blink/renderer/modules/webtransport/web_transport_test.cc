...
public:
    struct ConnectArgs {
        ConnectArgs(const KURL& url, Vector<network::mojom::blink::WebTransportCertificateFingerprintPtr> fingerprints,
    ...
    void Connect(const KURL& url, Vector<network::mojom::blink::WebTransportCertificateFingerprintPtr> fingerprints,
        mojo::PendingRemote<network::mojom::blink::WebTransportHandshakeClient> handshake_client) override
    {
        connect_args_.push_back(ConnectArgs(url, std::move(fingerprints), std::move(handshake_client)));
    }
    ...
    void Bind(mojo::PendingReceiver<mojom::blink::WebTransportConnector> receiver)
    {
...
public:
    explicit MockWebTransport(mojo::PendingReceiver<network::mojom::blink::WebTransport> pending_receiver)
        : receiver_(this, std::move(pending_receiver))
    {
    }

    MOCK_METHOD2(SendDatagram, void(base::span<const uint8_t> data, base::OnceCallback<void(bool)> callback));

    MOCK_METHOD3(CreateStream,
        void(mojo::ScopedDataPipeConsumerHandle readable, mojo::ScopedDataPipeProducerHandle writable, base::OnceCallback<void(bool, uint32_t)> callback));

    MOCK_METHOD1(AcceptBidirectionalStream, void(base::OnceCallback<void(uint32_t, mojo::ScopedDataPipeConsumerHandle, mojo::ScopedDataPipeProducerHandle)>));

    MOCK_METHOD1(AcceptUnidirectionalStream, void(base::OnceCallback<void(uint32_t, mojo::ScopedDataPipeConsumerHandle)>));
    ...
    void Close(network::mojom::blink::WebTransportCloseInfoPtr close_info) override
    {
        if (!close_info) {
            Close();
            return;
...
public:
    ...
    void AddBinder(const V8TestingScope& scope)
    {
        ...
        interface_broker_->SetBinderForTesting(
            mojom::blink::WebTransportConnector::Name_, WTF::BindRepeating(&WebTransportTest::BindConnector, weak_ptr_factory_.GetWeakPtr()));
    }
    ...
    WebTransport* Create(const V8TestingScope& scope, const String& url, WebTransportOptions* options)
    {
        AddBinder(scope);
        return WebTransport::Create(scope.GetScriptState(), url, options, ASSERT_NO_EXCEPTION);
    }

    // Connects a WebTransport object. Runs the event loop.
    void ConnectSuccessfully(WebTransport* web_transport, base::TimeDelta expected_outgoing_datagram_expiration_duration = base::TimeDelta())
    {
        ConnectSuccessfullyWithoutRunningPendingTasks(web_transport, expected_outgoing_datagram_expiration_duration);
        test::RunPendingTasks();
    }

    void ConnectSuccessfullyWithoutRunningPendingTasks(
        WebTransport* web_transport, base::TimeDelta expected_outgoing_datagram_expiration_duration = base::TimeDelta())
    {
        ...
        mojo::Remote<network::mojom::blink::WebTransportHandshakeClient> handshake_client(std::move(args[0].handshake_client));

        mojo::PendingRemote<network::mojom::blink::WebTransport> web_transport_to_pass;
        mojo::PendingRemote<network::mojom::blink::WebTransportClient> client_remote;

        mock_web_transport_ = std::make_unique<StrictMock<MockWebTransport>>(web_transport_to_pass.InitWithNewPipeAndPassReceiver());

        // These are called on every connection, so expect them in every test.
        EXPECT_CALL(*mock_web_transport_, AcceptUnidirectionalStream(_)).WillRepeatedly([this](AcceptUnidirectionalStreamCallback callback) {
            pending_unidirectional_accept_callbacks_.push_back(std::move(callback));
        });

        EXPECT_CALL(*mock_web_transport_, AcceptBidirectionalStream(_)).WillRepeatedly([this](AcceptBidirectionalStreamCallback callback) {
            pending_bidirectional_accept_callbacks_.push_back(std::move(callback));
        });

        if (expected_outgoing_datagram_expiration_duration != base::TimeDelta()) {
            EXPECT_CALL(*mock_web_transport_, SetOutgoingDatagramExpirationDuration(expected_outgoing_datagram_expiration_duration));
        }

        handshake_client->OnConnectionEstablished(std::move(web_transport_to_pass), client_remote.InitWithNewPipeAndPassReceiver(),
            network::mojom::blink::HttpResponseHeaders::New(), network::mojom::blink::WebTransportStats::New());
        client_remote_.Bind(std::move(client_remote));
    }

    // Creates, connects and returns a WebTransport object with the given |url|.
    // Runs the event loop.
    WebTransport* CreateAndConnectSuccessfully(const V8TestingScope& scope, const String& url, WebTransportOptions* options = EmptyOptions())
    {
    ...
    SendStream* CreateSendStreamSuccessfully(const V8TestingScope& scope, WebTransport* web_transport)
    {
        EXPECT_CALL(*mock_web_transport_, CreateStream(_, _, _))
            .WillOnce([this](mojo::ScopedDataPipeConsumerHandle handle, Unused, base::OnceCallback<void(bool, uint32_t)> callback) {
                send_stream_consumer_handle_ = std::move(handle);
                std::move(callback).Run(true, next_stream_id_++);
            });

        auto* script_state = scope.GetScriptState();
        auto send_stream_promise = web_transport->createUnidirectionalStream(script_state, ASSERT_NO_EXCEPTION);
        ScriptPromiseTester tester(script_state, send_stream_promise);

        tester.WaitUntilSettled();

        EXPECT_TRUE(tester.IsFulfilled());
        auto* writable = V8WritableStream::ToWrappable(scope.GetIsolate(), tester.Value().V8Value());
    ...
    mojo::ScopedDataPipeProducerHandle DoAcceptUnidirectionalStream()
    {
        ...
        std::move(pending_unidirectional_accept_callbacks_.front()).Run(next_stream_id_++, std::move(consumer));
        pending_unidirectional_accept_callbacks_.pop_front();

        return producer;
    }

    ReceiveStream* ReadReceiveStream(const V8TestingScope& scope, WebTransport* web_transport)
    {
        ...
        ReadableStream* readable = V8ReadableStream::ToWrappable(scope.GetIsolate(), v8value);
    ...
    {
        connector_.Bind(mojo::PendingReceiver<mojom::blink::WebTransportConnector>(std::move(handle)));
    }
    ...
    void TearDown() override
    {
        if (!interface_broker_)
            return;
        interface_broker_->SetBinderForTesting(mojom::blink::WebTransportConnector::Name_, {});
    }

    raw_ptr<const BrowserInterfaceBrokerProxy, DanglingUntriaged> interface_broker_ = nullptr;
    WTF::Deque<AcceptUnidirectionalStreamCallback> pending_unidirectional_accept_callbacks_;
    WTF::Deque<AcceptBidirectionalStreamCallback> pending_bidirectional_accept_callbacks_;
...
TEST_F(WebTransportTest, FailWithNullURL)
{
    ...
    WebTransport::Create(scope.GetScriptState(), String(), EmptyOptions(), exception_state);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(static_cast<int>(DOMExceptionCode::kSyntaxError), exception_state.Code());
}
...
TEST_F(WebTransportTest, FailWithNoScheme)
{
    V8TestingScope scope;
    auto& exception_state = scope.GetExceptionState();
    WebTransport::Create(scope.GetScriptState(), String("no-scheme"), EmptyOptions(), exception_state);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(static_cast<int>(DOMExceptionCode::kSyntaxError), exception_state.Code());
...
TEST_F(WebTransportTest, FailWithHttpsURL)
{
    V8TestingScope scope;
    auto& exception_state = scope.GetExceptionState();
    WebTransport::Create(scope.GetScriptState(), String("http://example.com/"), EmptyOptions(), exception_state);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(static_cast<int>(DOMExceptionCode::kSyntaxError), exception_state.Code());
    EXPECT_EQ("The URL's scheme must be 'https'. 'http' is not allowed.", exception_state.Message());
}
...
TEST_F(WebTransportTest, FailWithNoHost)
{
    V8TestingScope scope;
    auto& exception_state = scope.GetExceptionState();
    WebTransport::Create(scope.GetScriptState(), String("https:///"), EmptyOptions(), exception_state);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(static_cast<int>(DOMExceptionCode::kSyntaxError), exception_state.Code());
...
TEST_F(WebTransportTest, FailWithURLFragment)
{
    V8TestingScope scope;
    auto& exception_state = scope.GetExceptionState();
    WebTransport::Create(scope.GetScriptState(), String("https://example.com/#failing"), EmptyOptions(), exception_state);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(static_cast<int>(DOMExceptionCode::kSyntaxError), exception_state.Code());
    EXPECT_EQ("The URL contains a fragment identifier ('#failing'). Fragment "
...
{
    V8TestingScope scope;
    scope.GetExecutionContext()->GetContentSecurityPolicyForCurrentWorld()->AddPolicies(
        ParseContentSecurityPolicies("connect-src 'none'", network::mojom::ContentSecurityPolicyType::kEnforce,
            network::mojom::ContentSecurityPolicySource::kHTTP, *(scope.GetExecutionContext()->GetSecurityOrigin())));
    auto* web_transport = WebTransport::Create(scope.GetScriptState(), String("https://example.com/"), EmptyOptions(), ASSERT_NO_EXCEPTION);
    ScriptPromiseTester ready_tester(scope.GetScriptState(), web_transport->ready(scope.GetScriptState()));
    ScriptPromiseTester closed_tester(scope.GetScriptState(), web_transport->closed(scope.GetScriptState()));
...
TEST_F(WebTransportTest, PassCSP)
{
    ...
    scope.GetExecutionContext()->GetContentSecurityPolicyForCurrentWorld()->AddPolicies(
        ParseContentSecurityPolicies("connect-src https://example.com", network::mojom::ContentSecurityPolicyType::kEnforce,
            network::mojom::ContentSecurityPolicySource::kHTTP, *(scope.GetExecutionContext()->GetSecurityOrigin())));
    auto* web_transport = CreateAndConnectSuccessfully(scope, "https://example.com/");
    ScriptPromiseTester ready_tester(scope.GetScriptState(), web_transport->ready(scope.GetScriptState()));
...
TEST_F(WebTransportTest, SendConnectWithFingerprint)
{
    ...
    hash->setValue(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(NotShared<DOMUint8Array>(hashValue)));
    auto* options = MakeGarbageCollected<WebTransportOptions>();
    options->setServerCertificateHashes({ hash });
    WebTransport::Create(scope.GetScriptState(), String("https://example.com/"), options, ASSERT_NO_EXCEPTION);
...
TEST_F(WebTransportTest, SendConnectWithArrayBufferHash)
{
    ...
    hash->setValue(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(hashValue));
    auto* options = MakeGarbageCollected<WebTransportOptions>();
    options->setServerCertificateHashes({ hash });
    WebTransport::Create(scope.GetScriptState(), String("https://example.com/"), options, ASSERT_NO_EXCEPTION);
...
TEST_F(WebTransportTest, SendConnectWithOffsetArrayBufferViewHash)
{
    ...
    hash->setValue(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(NotShared<DOMUint8Array>(view)));
    auto* options = MakeGarbageCollected<WebTransportOptions>();
    options->setServerCertificateHashes({ hash });
    WebTransport::Create(scope.GetScriptState(), String("https://example.com/"), options, ASSERT_NO_EXCEPTION);
...
TEST_F(WebTransportTest, CloseAfterConnection)
{
    ...
    WebTransportCloseInfo* close_info = MakeGarbageCollected<WebTransportCloseInfo>();
...
TEST_F(WebTransportTest, SendDatagram)
{
    ...
    EXPECT_CALL(*mock_web_transport_, SendDatagram(ElementsAre('A'), _))
        .WillOnce(Invoke([](base::span<const uint8_t>, MockWebTransport::SendDatagramCallback callback) { std::move(callback).Run(true); }));
    ...
    *chunk->Data() = 'A';
    auto result = writer->write(script_state, ScriptValue::From(script_state, chunk), ASSERT_NO_EXCEPTION);
...
TEST_F(WebTransportTest, BackpressureForOutgoingDatagrams)
{
    ...
    EXPECT_CALL(*mock_web_transport_, SendDatagram(_, _))
        .Times(4)
        .WillRepeatedly(Invoke([](base::span<const uint8_t>, MockWebTransport::SendDatagramCallback callback) { std::move(callback).Run(true); }));
...

    {
        auto* chunk = DOMUint8Array::Create(1);
        *chunk->Data() = 'A';
        promise1 = writer->write(script_state, ScriptValue::From(script_state, chunk), ASSERT_NO_EXCEPTION);
    }
    {
        auto* chunk = DOMUint8Array::Create(1);
        *chunk->Data() = 'B';
        promise2 = writer->write(script_state, ScriptValue::From(script_state, chunk), ASSERT_NO_EXCEPTION);
    }
    {
        auto* chunk = DOMUint8Array::Create(1);
        *chunk->Data() = 'C';
        promise3 = writer->write(script_state, ScriptValue::From(script_state, chunk), ASSERT_NO_EXCEPTION);
    }
    {
        auto* chunk = DOMUint8Array::Create(1);
        *chunk->Data() = 'D';
        promise4 = writer->write(script_state, ScriptValue::From(script_state, chunk), ASSERT_NO_EXCEPTION);
    }
...
TEST_F(WebTransportTest, SendDatagramBeforeConnect)
{
    ...
    *chunk->Data() = 'A';
    ...
    test::RunPendingTasks();
    *chunk->Data() = 'N';
    result = writer->write(script_state, ScriptValue::From(script_state, chunk), ASSERT_NO_EXCEPTION);
...
Vector<uint8_t> GetValueAsVector(ScriptState* script_state, ScriptValue iterator_result)
{
    bool done = false;
    v8::Local<v8::Value> value;
    if (!V8UnpackIterationResult(script_state, iterator_result.V8Value().As<v8::Object>(), &value, &done)) {
    ...
    auto array = NativeValueTraits<NotShared<DOMUint8Array>>::NativeValue(script_state->GetIsolate(), value, exception_state);
    if (!array) {
...
TEST_F(WebTransportTest, ReceiveDatagramWithBYOBReader)
{
    ...
    auto* readable = web_transport->datagrams()->readable();
    auto* script_state = scope.GetScriptState();
    auto* reader = readable->GetBYOBReaderForTesting(script_state, ASSERT_NO_EXCEPTION);

    NotShared<DOMArrayBufferView> view = NotShared<DOMUint8Array>(DOMUint8Array::Create(1));
...
bool IsRangeError(ScriptState* script_state, ScriptValue value, const String& message)
{
    ...
    const auto& Has = [script_state, object](const String& key, const String& value) -> bool {
        v8::Local<v8::Value> actual;
        return object->Get(script_state->GetContext(), V8AtomicString(script_state->GetIsolate(), key)).ToLocal(&actual)
            && ToCoreStringWithUndefinedOrNullCheck(script_state->GetIsolate(), actual) == value;
    };
...
TEST_F(WebTransportTest, ReceiveDatagramWithoutEnoughBuffer)
{
    ...
    tester.WaitUntilSettled();
    EXPECT_TRUE(tester.IsRejected());
    EXPECT_TRUE(IsRangeError(script_state, tester.Value(), "supplied view is not large enough."));
}
...
TEST_F(WebTransportTest, DatagramsAreDropped)
{
    ...
    EXPECT_THAT(GetValueAsVector(script_state, tester1.Value()), ElementsAre('B'));
    ...
    EXPECT_THAT(GetValueAsVector(script_state, tester2.Value()), ElementsAre('C'));
}
...
TEST_F(WebTransportTest, IncomingHighWaterMarkIsObeyed)
{
    ...
    for (int i = 0; i < kHighWaterMark; ++i) {
        ...
        EXPECT_TRUE(tester.IsFulfilled());
        EXPECT_THAT(GetValueAsVector(script_state, tester.Value()), ElementsAre('0' + i + 1));
    }
}
...
TEST_F(WebTransportTest, TwoSimultaneousReadsWork)
{
    ...
    EXPECT_THAT(GetValueAsVector(script_state, tester1.Value()), ElementsAre('A'));

    ScriptPromiseTester tester2(script_state, result2);
    tester2.WaitUntilSettled();
    EXPECT_TRUE(tester2.IsFulfilled());

    EXPECT_THAT(GetValueAsVector(script_state, tester2.Value()), ElementsAre('B'));
}
...
TEST_F(WebTransportTest, CreateSendStream)
{
    ...
    EXPECT_CALL(*mock_web_transport_, CreateStream(Truly(ValidConsumerHandle), Not(Truly(ValidProducerHandle)), _))
        .WillOnce([](Unused, Unused, base::OnceCallback<void(bool, uint32_t)> callback) { std::move(callback).Run(true, 0); });
...
TEST_F(WebTransportTest, CreateSendStreamBeforeConnect)
{
    ...
    auto* script_state = scope.GetScriptState();
    auto* web_transport = WebTransport::Create(script_state, "https://example.com", EmptyOptions(), ASSERT_NO_EXCEPTION);
    auto& exception_state = scope.GetExceptionState();
    auto send_stream_promise = web_transport->createUnidirectionalStream(script_state, exception_state);
    EXPECT_TRUE(send_stream_promise.IsEmpty());
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(static_cast<int>(DOMExceptionCode::kNetworkError), exception_state.Code());
}
...
TEST_F(WebTransportTest, CreateSendStreamFailure)
{
    ...
    EXPECT_CALL(*mock_web_transport_, CreateStream(_, _, _)).WillOnce([](Unused, Unused, base::OnceCallback<void(bool, uint32_t)> callback) {
    ...
    EXPECT_TRUE(tester.IsRejected());
    DOMException* exception = V8DOMException::ToWrappable(scope.GetIsolate(), tester.Value().V8Value());
...
TEST_F(WebTransportTest, SendStreamGarbageCollectionLocalClose)
{
    ...
    {
        v8::HandleScope handle_scope(isolate);
        ScriptPromiseTester tester(script_state, ScriptPromise<IDLUndefined>::FromV8Promise(isolate, close_promise_persistent.Get(isolate)));
...
TEST_F(WebTransportTest, ReceiveStreamGarbageCollectionCancel)
{
    ...
    std::optional<ScriptPromise<IDLUndefined>> cancel_promise;
    {
        ...
        // above.
        v8::HandleScope handle_scope(scope.GetIsolate());
        cancel_promise.emplace(receive_stream->cancel(script_state, ASSERT_NO_EXCEPTION));
    }
...
TEST_F(WebTransportTest, CreateReceiveStream)
{
    ...
    auto* reader = receive_stream->GetDefaultReaderForTesting(script_state, ASSERT_NO_EXCEPTION);
    ...
    v8::Local<v8::Value> value;
    bool done = false;
    ASSERT_TRUE(V8UnpackIterationResult(script_state, read_result.As<v8::Object>(), &value, &done));
    NotShared<DOMUint8Array> u8array = NativeValueTraits<NotShared<DOMUint8Array>>::NativeValue(scope.GetIsolate(), value, ASSERT_NO_EXCEPTION);
    ASSERT_TRUE(u8array);
...
TEST_F(WebTransportTest, CreateReceiveStreamThenClose)
{
    ...
    WebTransportError* exception = V8WebTransportError::ToWrappable(scope.GetIsolate(), read_tester.Value().V8Value());
...
TEST_F(WebTransportTest, CreateBidirectionalStream)
{
    ...
    EXPECT_CALL(*mock_web_transport_, CreateStream(Truly(ValidConsumerHandle), Truly(ValidProducerHandle), _))
        .WillOnce([](Unused, Unused, base::OnceCallback<void(bool, uint32_t)> callback) { std::move(callback).Run(true, 0); });

    auto* script_state = scope.GetScriptState();
    auto bidirectional_stream_promise = web_transport->createBidirectionalStream(script_state, ASSERT_NO_EXCEPTION);
    ScriptPromiseTester tester(script_state, bidirectional_stream_promise);

    tester.WaitUntilSettled();

    EXPECT_TRUE(tester.IsFulfilled());
    auto* bidirectional_stream = V8WebTransportBidirectionalStream::ToWrappable(scope.GetIsolate(), tester.Value().V8Value());
...
TEST_F(WebTransportTest, ReceiveBidirectionalStream)
{
    ...
    ASSERT_TRUE(CreateDataPipeForWebTransportTests(&outgoing_producer, &outgoing_consumer));

    mojo::ScopedDataPipeProducerHandle incoming_producer;
    mojo::ScopedDataPipeConsumerHandle incoming_consumer;
    ASSERT_TRUE(CreateDataPipeForWebTransportTests(&incoming_producer, &incoming_consumer));

    std::move(pending_bidirectional_accept_callbacks_.front()).Run(next_stream_id_++, std::move(incoming_consumer), std::move(outgoing_producer));

    ReadableStream* streams = web_transport->incomingBidirectionalStreams();

    v8::Local<v8::Value> v8value = ReadValueFromStream(scope, streams);

    BidirectionalStream* bidirectional_stream = V8WebTransportBidirectionalStream::ToWrappable(scope.GetIsolate(), v8value);
    EXPECT_TRUE(bidirectional_stream);
}
...
TEST_F(WebTransportTest, SetDatagramWritableQueueExpirationDuration)
{
    ...
    EXPECT_CALL(*mock_web_transport_, SetOutgoingDatagramExpirationDuration(kDurationDelta));
...
TEST_F(WebTransportTest, OnClosed)
{
    ...
    auto* script_state = scope.GetScriptState();
    ScriptPromiseTester tester(script_state, web_transport->closed(scope.GetScriptState()));

    web_transport->OnClosed(network::mojom::blink::WebTransportCloseInfo::New(99, "reason"), network::mojom::blink::WebTransportStats::New());
    ...
    WebTransportCloseInfo* close_info = WebTransportCloseInfo::Create(isolate, value.V8Value(), ASSERT_NO_EXCEPTION);
...
TEST_F(WebTransportTest, OnClosedWithNull)
{
    ...
    web_transport->OnClosed(nullptr, network::mojom::blink::WebTransportStats::New());
...
TEST_F(WebTransportTest, ReceivedResetStream)
{
    ...
    EXPECT_CALL(*mock_web_transport_, CreateStream(Truly(ValidConsumerHandle), Truly(ValidProducerHandle), _))
        .WillOnce([&](mojo::ScopedDataPipeConsumerHandle readable_handle, mojo::ScopedDataPipeProducerHandle writable_handle,
    ...
    v8::Local<v8::Value> error_value = bidirectional_stream->readable()->GetStoredError(isolate);
    WebTransportError* error = V8WebTransportError::ToWrappable(scope.GetIsolate(), error_value);
    ASSERT_TRUE(error);
...
TEST_F(WebTransportTest, ReceivedStopSending)
{
    ...
    v8::Local<v8::Value> error_value = bidirectional_stream->writable()->GetStoredError(isolate);
    WebTransportError* error = V8WebTransportError::ToWrappable(scope.GetIsolate(), error_value);
    ASSERT_TRUE(error);
