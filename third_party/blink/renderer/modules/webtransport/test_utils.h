...
class TestWebTransportCreator final : public mojom::blink::WebTransportConnector {
    DISALLOW_NEW();

public:
    using CreateStubCallback = base::RepeatingCallback<void(mojo::PendingRemote<network::mojom::blink::WebTransport>&)>;
    ...
    {
        return web_transport_;
    }

    // Implementation of mojom::blink::WebTransportConnector.
    void Connect(const KURL&, Vector<network::mojom::blink::WebTransportCertificateFingerprintPtr>,
        mojo::PendingRemote<network::mojom::blink::WebTransportHandshakeClient>) override;
...
private:
    ...
    raw_ptr<const BrowserInterfaceBrokerProxy> browser_interface_broker_ = nullptr;
