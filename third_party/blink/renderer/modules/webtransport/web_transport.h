...
class MODULES_EXPORT WebTransport final : public ScriptWrappable,
...
public:
    ...
    static WebTransport* Create(ScriptState*, const String& url, WebTransportOptions*, ExceptionState&);

    WebTransport(PassKey, ScriptState*, const String& url);
    ~WebTransport() override;

    // WebTransport IDL implementation.
    ScriptPromise<WritableStream> createUnidirectionalStream(ScriptState*, ExceptionState&);
    ReadableStream* incomingUnidirectionalStreams();

    ScriptPromise<BidirectionalStream> createBidirectionalStream(ScriptState*, ExceptionState&);
    ...
    void OnConnectionEstablished(mojo::PendingRemote<network::mojom::blink::WebTransport>, mojo::PendingReceiver<network::mojom::blink::WebTransportClient>,
        network::mojom::blink::HttpResponseHeadersPtr response_headers, network::mojom::blink::WebTransportStatsPtr initial_stats) override;
    ...
    void OnClosed(network::mojom::blink::WebTransportCloseInfoPtr close_info, network::mojom::blink::WebTransportStatsPtr final_stats) override;
...
private:
    ...
    void Init(const String& url_for_diagnostics, const WebTransportOptions&, ExceptionState&);

    void Dispose();
    void Cleanup(WebTransportCloseInfo*, v8::Local<v8::Value> error, bool abruptly);
    void OnConnectionError();
    void RejectPendingStreamResolvers(v8::Local<v8::Value> error);
    void HandlePendingGetStatsResolvers(v8::Local<v8::Value> error);
    void OnCreateSendStreamResponse(ScriptPromiseResolver<WritableStream>*, mojo::ScopedDataPipeProducerHandle, bool succeeded, uint32_t stream_id);
    void OnCreateBidirectionalStreamResponse(ScriptPromiseResolver<BidirectionalStream>*, mojo::ScopedDataPipeProducerHandle,
        mojo::ScopedDataPipeConsumerHandle, bool succeeded, uint32_t stream_id);
    void OnGetStatsResponse(network::mojom::blink::WebTransportStatsPtr);

    bool DoesSubresourceFilterBlockConnection(const KURL& url);

    WebTransportConnectionStats* ConvertStatsFromMojom(network::mojom::blink::WebTransportStatsPtr in);
    ...
    HeapHashMap<uint32_t, Member<IncomingStream>, IntWithZeroKeyHashTraits<uint32_t>> incoming_stream_map_;

    // Map from stream_id to OutgoingStream.
    // Intentionally keeps streams reachable by GC as long as they are open.
    // This doesn't support stream ids of 0xfffffffe or larger.
    // TODO(ricea): Find out if such large stream ids are possible.
    HeapHashMap<uint32_t, Member<OutgoingStream>, IntWithZeroKeyHashTraits<uint32_t>> outgoing_stream_map_;
    ...
    HashMap<uint32_t, bool, IntWithZeroKeyHashTraits<uint32_t>> closed_potentially_pending_streams_;
    ...
    HeapMojoReceiver<network::mojom::blink::WebTransportHandshakeClient, WebTransport> handshake_client_receiver_;
    HeapMojoReceiver<network::mojom::blink::WebTransportClient, WebTransport> client_receiver_;
    ...
    HeapVector<Member<ScriptPromiseResolver<WebTransportConnectionStats>>> pending_get_stats_resolvers_;
    ...
    Member<StreamVendingUnderlyingSource> received_bidirectional_streams_underlying_source_;
