...
class MODULES_EXPORT IncomingStream final : public GarbageCollected<IncomingStream> {
...
public:
    ...
    IncomingStream(ScriptState*, base::OnceCallback<void(std::optional<uint8_t>)> on_abort, mojo::ScopedDataPipeConsumerHandle);
    ~IncomingStream();
...
private:
    ...
    size_t RespondBYOBRequestOrEnqueueBytes(base::span<const uint8_t> source, ExceptionState&);
