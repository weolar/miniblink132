...
class MockClient : public GarbageCollected<MockClient>, public OutgoingStream::Client {
public:
...
public:
    ...
    OutgoingStream* Create(const V8TestingScope& scope, uint32_t capacity = 0)
    {
        ...
        MojoResult result = mojo::CreateDataPipe(&options, data_pipe_producer, data_pipe_consumer_);
        ...
        auto* outgoing_stream = MakeGarbageCollected<OutgoingStream>(script_state, mock_client_, std::move(data_pipe_producer));
    ...
    Vector<uint8_t> ReadAllPendingData()
    {
        ...
        MojoResult result = data_pipe_consumer_->BeginReadData(MOJO_BEGIN_READ_DATA_FLAG_NONE, buffer);
...
TEST(OutgoingStreamTest, WriteArrayBuffer)
{
    ...
    auto* outgoing_stream = stream_creator.Create(scope);
    auto* script_state = scope.GetScriptState();
    auto* writer = outgoing_stream->Writable()->getWriter(script_state, ASSERT_NO_EXCEPTION);
...
TEST(OutgoingStreamTest, WriteThenClose)
{
    ...
    auto write_promise = writer->write(script_state, ScriptValue::From(script_state, chunk), ASSERT_NO_EXCEPTION);

    EXPECT_CALL(stream_creator.GetMockClient(), SendFin()).WillOnce([&]() {
        // This needs to happen asynchronously.
        scope.GetExecutionContext()
            ->GetTaskRunner(TaskType::kNetworking)
            ->PostTask(FROM_HERE, WTF::BindOnce(&OutgoingStream::OnOutgoingStreamClosed, WrapWeakPersistent(outgoing_stream)));
    });
...
TEST(OutgoingStreamTest, DataPipeClosed)
{
    ...
    DOMException* closed_exception = V8DOMException::ToWrappable(scope.GetIsolate(), closed_tester.Value().V8Value());
    ASSERT_TRUE(closed_exception);
    EXPECT_EQ(closed_exception->name(), "NetworkError");
    EXPECT_EQ(closed_exception->message(), "The stream was aborted by the remote server");
    ...
    DOMException* write_exception = V8DOMException::ToWrappable(scope.GetIsolate(), write_tester.Value().V8Value());
    ASSERT_TRUE(write_exception);
    EXPECT_EQ(write_exception->name(), "NetworkError");
    EXPECT_EQ(write_exception->message(), "The stream was aborted by the remote server");
}
...
TEST(OutgoingStreamTest, DataPipeClosedDuringAsyncWrite)
{
    ...
    DOMException* closed_exception = V8DOMException::ToWrappable(scope.GetIsolate(), write_tester.Value().V8Value());
    ASSERT_TRUE(closed_exception);
    EXPECT_EQ(closed_exception->name(), "NetworkError");
    EXPECT_EQ(closed_exception->message(), "The stream was aborted by the remote server");
}
...
TEST(OutgoingStreamTest, Abort)
{
    ...
    auto* writer = outgoing_stream->Writable()->getWriter(script_state, ASSERT_NO_EXCEPTION);
    writer->abort(script_state, ScriptValue(isolate, v8::Undefined(isolate)), ASSERT_NO_EXCEPTION);
}
...
TEST(OutgoingStreamTest, AbortWithWebTransportErrorWithCode)
{
    ...
    v8::Local<v8::Value> error = WebTransportError::Create(isolate,
        /*stream_error_code=*/8, "foobar", V8WebTransportErrorSource::Enum::kStream);
