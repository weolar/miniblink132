...
public:
    explicit StubWebTransport(mojo::PendingReceiver<network::mojom::blink::WebTransport> pending_receiver)
    ...
    bool WasSendFinCalled() const
    {
        return was_send_fin_called_;
    }
    bool WasAbortStreamCalled() const
    {
        return was_abort_stream_called_;
    }
    ...
    void CreateRemote()
    {
        ...
        ASSERT_TRUE(CreateDataPipeForWebTransportTests(&output_producer, &output_consumer));
        ...
        ASSERT_TRUE(CreateDataPipeForWebTransportTests(&input_producer, &input_consumer));
        input_producer_ = std::move(input_producer);

        std::move(accept_callback_).Run(kDefaultStreamId, std::move(input_consumer), std::move(output_producer));
    ...
    void SendDatagram(base::span<const uint8_t> data, base::OnceCallback<void(bool)>) override
    {
        NOTREACHED();
    }

    void CreateStream(mojo::ScopedDataPipeConsumerHandle output_consumer, mojo::ScopedDataPipeProducerHandle input_producer,
    ...
    void AcceptBidirectionalStream(base::OnceCallback<void(uint32_t, mojo::ScopedDataPipeConsumerHandle, mojo::ScopedDataPipeProducerHandle)> callback) override
    {
        DCHECK(!accept_callback_);
        accept_callback_ = std::move(callback);
    }

    void AcceptUnidirectionalStream(base::OnceCallback<void(uint32_t, mojo::ScopedDataPipeConsumerHandle)> callback) override
    {
...
private:
    base::OnceCallback<void(uint32_t, mojo::ScopedDataPipeConsumerHandle, mojo::ScopedDataPipeProducerHandle)> accept_callback_;
    base::OnceCallback<void(uint32_t, mojo::ScopedDataPipeConsumerHandle)> ignored_unidirectional_stream_callback_;
...
public:
    ...
    {
        creator_.Init(scope.GetScriptState(), WTF::BindRepeating(&ScopedWebTransport::CreateStub, weak_ptr_factory_.GetWeakPtr()));
    }

    WebTransport* GetWebTransport() const
    {
        return creator_.GetWebTransport();
    }
    StubWebTransport* Stub() const
    {
        return stub_.get();
    }
    ...
    BidirectionalStream* CreateBidirectionalStream(const V8TestingScope& scope)
    {
        ...
        auto bidirectional_stream_promise = GetWebTransport()->createBidirectionalStream(script_state, ASSERT_NO_EXCEPTION);
    ...
    BidirectionalStream* RemoteCreateBidirectionalStream(const V8TestingScope& scope)
    {
        stub_->CreateRemote();
...
private:
    void CreateStub(mojo::PendingRemote<network::mojom::blink::WebTransport>& web_transport_to_pass)
    {
        stub_ = std::make_unique<StubWebTransport>(web_transport_to_pass.InitWithNewPipeAndPassReceiver());
    }
...
void TestWrite(const V8TestingScope& scope, ScopedWebTransport* scoped_web_transport, BidirectionalStream* bidirectional_stream)
{
    auto* script_state = scope.GetScriptState();
    auto* writer = bidirectional_stream->writable()->getWriter(script_state, ASSERT_NO_EXCEPTION);
    ...
    *chunk->Data() = 'A';
    ...
    mojo::ScopedDataPipeConsumerHandle& output_consumer = scoped_web_transport->Stub()->OutputConsumer();
    base::span<const uint8_t> buffer;
    MojoResult mojo_result = output_consumer->BeginReadData(MOJO_BEGIN_READ_DATA_FLAG_NONE, buffer);
...
TEST(BidirectionalStreamTest, CreateLocallyAndWrite)
{
    ...
    auto* bidirectional_stream = scoped_web_transport.CreateBidirectionalStream(scope);
...
TEST(BidirectionalStreamTest, CreateRemotelyAndWrite)
{
    ...
    V8TestingScope scope;
    ScopedWebTransport scoped_web_transport(scope);
    auto* bidirectional_stream = scoped_web_transport.RemoteCreateBidirectionalStream(scope);
    ASSERT_TRUE(bidirectional_stream);
...
void TestRead(V8TestingScope& scope, ScopedWebTransport* scoped_web_transport, BidirectionalStream* bidirectional_stream)
{
    mojo::ScopedDataPipeProducerHandle& input_producer = scoped_web_transport->Stub()->InputProducer();
    MojoResult mojo_result = input_producer->WriteAllData(base::as_byte_span(std::string_view("B")));

    ASSERT_EQ(mojo_result, MOJO_RESULT_OK);

    v8::Local<v8::Value> v8array = ReadValueFromStream(scope, bidirectional_stream->readable());
    NotShared<DOMUint8Array> u8array = NativeValueTraits<NotShared<DOMUint8Array>>::NativeValue(scope.GetIsolate(), v8array, scope.GetExceptionState());
    ASSERT_TRUE(u8array);
...
TEST(BidirectionalStreamTest, IncomingStreamCleanClose)
{
    ...
    scoped_web_transport.GetWebTransport()->OnIncomingStreamClosed(kDefaultStreamId, true);
    scoped_web_transport.Stub()->InputProducer().reset();

    auto* script_state = scope.GetScriptState();
    auto* reader = bidirectional_stream->readable()->GetDefaultReaderForTesting(script_state, ASSERT_NO_EXCEPTION);
    ...
    bool done = false;
    EXPECT_TRUE(V8UnpackIterationResult(script_state, result.As<v8::Object>(), &v8value, &done));
    EXPECT_TRUE(done);
}
...
TEST(BidirectionalStreamTest, OutgoingStreamCleanClose)
{
    ...
    auto* script_state = scope.GetScriptState();
    auto close_promise = bidirectional_stream->writable()->close(script_state, ASSERT_NO_EXCEPTION);

    scoped_web_transport.GetWebTransport()->OnOutgoingStreamClosed(kDefaultStreamId);
    ...
    scoped_web_transport.GetWebTransport()->OnIncomingStreamClosed(kDefaultStreamId, false);
    scoped_web_transport.Stub()->InputProducer().reset();
...
TEST(BidirectionalStreamTest, WriteAfterCancellingIncoming)
{
    ...
    auto* script_state = scope.GetScriptState();
    auto cancel_promise = bidirectional_stream->readable()->cancel(script_state, ASSERT_NO_EXCEPTION);
...
TEST(BidirectionalStreamTest, ReadAfterAbortingOutgoing)
{
    ...
    auto* script_state = scope.GetScriptState();
    auto abort_promise = bidirectional_stream->writable()->abort(script_state, ASSERT_NO_EXCEPTION);
