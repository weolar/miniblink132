...
private:
    base::OnceCallback<void(uint32_t, mojo::ScopedDataPipeConsumerHandle, mojo::ScopedDataPipeProducerHandle)> ignored_accept_callback_;
...
public:
    ScopedDatagramDuplexStream()
        ...
        , duplex_(MakeGarbageCollected<DatagramDuplexStream>(scoped_web_transport_.GetWebTransport(), kInitialOutgoingHighWaterMark))
    {
    }
    ScopedDatagramDuplexStream(const ScopedDatagramDuplexStream&) = delete;
    ScopedDatagramDuplexStream& operator=(const ScopedDatagramDuplexStream&) = delete;

    DatagramDuplexStream* Duplex()
    {
        return duplex_;
    }

    StubWebTransport* Stub()
    {
        return scoped_web_transport_.Stub();
    }
