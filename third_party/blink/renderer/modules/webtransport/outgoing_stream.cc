...
public:
    explicit SendStreamAbortAlgorithm(OutgoingStream* stream)
        : stream_(stream)
    {
    }
    ...
    void Run() override
    {
        stream_->AbortAlgorithm(stream_);
    }
...
public:
    ...
    ScriptPromise<IDLUndefined> start(ScriptState* script_state, WritableStreamDefaultController* controller, ExceptionState&) override
    {
        DVLOG(1) << "OutgoingStream::UnderlyinkSink::start() outgoing_stream_=" << outgoing_stream_;
    ...
    {
        DVLOG(1) << "OutgoingStream::UnderlyingSink::write() outgoing_stream_=" << outgoing_stream_;
    ...
    {
        DVLOG(1) << "OutgoingStream::UnderlingSink::close() outgoing_stream_=" << outgoing_stream_;
        ...
        outgoing_stream_->close_promise_resolver_ = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state);
        outgoing_stream_->pending_operation_ = outgoing_stream_->close_promise_resolver_;
    ...
    ScriptPromise<IDLUndefined> abort(ScriptState* script_state, ScriptValue reason, ExceptionState& exception_state) override
    {
        DVLOG(1) << "OutgoingStream::UnderlyingSink::abort() outgoing_stream_=" << outgoing_stream_;
        DCHECK(!reason.IsEmpty());

        uint8_t code = 0;
        WebTransportError* exception = V8WebTransportError::ToWrappable(script_state->GetIsolate(), reason.V8Value());
        if (exception) {
...
public:
        ...
        : isolate_(isolate)
    {
        ...
        void* memory_buffer = WTF::Partitions::BufferPartition()->Alloc(data.size(), "OutgoingStream");
        // SAFETY: WTF::Partitions::BufferPartition()->Alloc() returns a valid
        // pointer to at least data.size() bytes.
        buffer_ = UNSAFE_BUFFERS(HeapBuffer::FromOwningPointer(reinterpret_cast<uint8_t*>(memory_buffer), data.size()));
...
OutgoingStream::OutgoingStream(ScriptState* script_state, Client* client, mojo::ScopedDataPipeProducerHandle handle)
...
void OutgoingStream::InitWithExistingWritableStream(WritableStream* stream, ExceptionState& exception_state)
{
    write_watcher_.Watch(data_pipe_.get(), MOJO_HANDLE_SIGNAL_WRITABLE, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
        WTF::BindRepeating(&OutgoingStream::OnHandleReady, WrapWeakPersistent(this)));
    close_watcher_.Watch(data_pipe_.get(), MOJO_HANDLE_SIGNAL_PEER_CLOSED, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
        WTF::BindRepeating(&OutgoingStream::OnPeerClosed, WrapWeakPersistent(this)));

    writable_ = stream;
    stream->InitWithCountQueueingStrategy(script_state_, MakeGarbageCollected<UnderlyingSink>(this), 1,
        /*optimizer=*/nullptr, exception_state);
    send_stream_abort_handle_ = controller_->signal()->AddAlgorithm(MakeGarbageCollected<SendStreamAbortAlgorithm>(this));
}
...
void OutgoingStream::AbortAlgorithm(OutgoingStream* stream)
{
    ...
    ScriptPromise<IDLUndefined> abort_promise = underlying_sink->abort(script_state_, reason, ASSERT_NO_EXCEPTION);

    // 6. Upon fulfillment of promise, reject pendingOperation with reason.
    class ResolveFunction final : public ThenCallable<IDLUndefined, ResolveFunction> {
    public:
        ResolveFunction(ScriptValue reason, ScriptPromiseResolver<IDLUndefined>* resolver)
            : reason_(reason)
            , resolver_(resolver)
        {
        }
    ...
    abort_promise.React(script_state_, MakeGarbageCollected<ResolveFunction>(reason, pending_operation));
}
...
void OutgoingStream::OnHandleReady(MojoResult result, const mojo::HandleSignalsState&)
{
    DVLOG(1) << "OutgoingStream::OnHandleReady() this=" << this << " result=" << result;
...
void OutgoingStream::OnPeerClosed(MojoResult result, const mojo::HandleSignalsState&)
{
    DVLOG(1) << "OutgoingStream::OnPeerClosed() this=" << this << " result=" << result;
...
ScriptPromise<IDLUndefined> OutgoingStream::SinkWrite(ScriptState* script_state, ScriptValue chunk, ExceptionState& exception_state)
{
    ...
    auto* buffer_source = V8BufferSource::Create(script_state_->GetIsolate(), chunk.V8Value(), exception_state);
    if (exception_state.HadException())
        return EmptyPromise();
    DCHECK(buffer_source);

    if (!data_pipe_) {
        return ScriptPromise<IDLUndefined>::Reject(script_state, CreateAbortException(IsLocalAbort(false)));
    }
...
ScriptPromise<IDLUndefined> OutgoingStream::WriteOrCacheData(ScriptState* script_state, base::span<const uint8_t> data)
{
    DVLOG(1) << "OutgoingStream::WriteOrCacheData() this=" << this << " data=(" << static_cast<const void*>(data.data()) << ", " << data.size() << ")";
    ...
    DCHECK(!cached_data_);
    cached_data_ = std::make_unique<CachedDataBuffer>(script_state->GetIsolate(), data.subspan(written));
    DCHECK_EQ(offset_, 0u);
    write_watcher_.ArmOrNotify();
    write_promise_resolver_ = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state);
...
{
    DVLOG(1) << "OutgoingStream::WriteDataSynchronously() this=" << this << " data=(" << static_cast<const void*>(data.data()) << ", " << data.size() << ")";
    DCHECK(data_pipe_);

    size_t actually_written_bytes = 0;
    MojoResult result = data_pipe_->WriteData(data, MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
    switch (result) {
    case MOJO_RESULT_OK:
...
{
    DVLOG(1) << "OutgoingStream::CreateAbortException() this=" << this << " is_local_abort=" << static_cast<bool>(is_local_abort);

    DOMExceptionCode code = is_local_abort ? DOMExceptionCode::kAbortError : DOMExceptionCode::kNetworkError;
    String message = String::Format("The stream was aborted %s", is_local_abort ? "locally" : "by the remote server");

    return ScriptValue(script_state_->GetIsolate(), V8ThrowDOMException::CreateOrEmpty(script_state_->GetIsolate(), code, message));
}
