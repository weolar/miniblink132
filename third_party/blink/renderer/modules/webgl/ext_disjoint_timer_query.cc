...
void EXTDisjointTimerQuery::deleteQueryEXT(WebGLTimerQueryEXT* query)
{
    ...
    if (!query->Validate(nullptr, scoped.Context())) {
        scoped.Context()->SynthesizeGLError(GL_INVALID_OPERATION, "delete", "object does not belong to this context");
        return;
    }
...
{
    WebGLExtensionScopedContext scoped(this);
    if (!query || scoped.IsLost() || query->MarkedForDeletion() || !query->Validate(nullptr, scoped.Context())) {
        return false;
    }
...
void EXTDisjointTimerQuery::beginQueryEXT(GLenum target, WebGLTimerQueryEXT* query)
{
    ...
    if (current_elapsed_query_) {
        scoped.Context()->SynthesizeGLError(GL_INVALID_OPERATION, "beginQueryEXT", "a query is already active for target");
        return;
    }
...
ScriptValue EXTDisjointTimerQuery::getQueryEXT(ScriptState* script_state, GLenum target, GLenum pname)
{
    WebGLExtensionScopedContext scoped(this);
    if (scoped.IsLost())
    ...
    if (pname == GL_QUERY_COUNTER_BITS_EXT) {
        ...
        scoped.Context()->SynthesizeGLError(GL_INVALID_ENUM, "getQuery", "invalid target/pname combination");
        return ScriptValue::CreateNull(script_state->GetIsolate());
    }
...
ScriptValue EXTDisjointTimerQuery::getQueryObjectEXT(ScriptState* script_state, WebGLTimerQueryEXT* query, GLenum pname)
{
    WebGLExtensionScopedContext scoped(this);
    if (scoped.IsLost())
        return ScriptValue::CreateNull(script_state->GetIsolate());
    ...
    if (current_elapsed_query_ == query) {
        scoped.Context()->SynthesizeGLError(GL_INVALID_OPERATION, "getQueryObjectEXT", "query is currently active");
        return ScriptValue::CreateNull(script_state->GetIsolate());
    }
