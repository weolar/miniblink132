...
WebGLTransformFeedback::WebGLTransformFeedback(WebGL2RenderingContextBase* ctx, TFType type)
...
{
    return program && program_ == program && program->LinkCount() == program_link_count_;
}

bool WebGLTransformFeedback::SetBoundIndexedTransformFeedbackBuffer(GLuint index, WebGLBuffer* buffer)
{
    ...
    if (bound_indexed_transform_feedback_buffers_[index]) {
        bound_indexed_transform_feedback_buffers_[index]->OnDetached(Context()->ContextGL());
    }
    bound_indexed_transform_feedback_buffers_[index] = buffer;
    return true;
}

bool WebGLTransformFeedback::GetBoundIndexedTransformFeedbackBuffer(GLuint index, WebGLBuffer** outBuffer) const
{
    if (index >= bound_indexed_transform_feedback_buffers_.size())
        return false;
...
{
    for (WebGLBuffer* feedback_buffer : bound_indexed_transform_feedback_buffers_) {
...
{
    for (wtf_size_t i = 0; i < bound_indexed_transform_feedback_buffers_.size(); ++i) {
        if (bound_indexed_transform_feedback_buffers_[i] == buffer) {
            bound_indexed_transform_feedback_buffers_[i]->OnDetached(Context()->ContextGL());
