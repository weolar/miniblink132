...
WebGLSync::WebGLSync(WebGL2RenderingContextBase* ctx, GLuint object, GLenum object_type)
...
void WebGLSync::ScheduleAllowCacheUpdate()
{
    if (task_handle_.IsActive())
        return;
    task_handle_ = PostCancellableTask(*task_runner_, FROM_HERE, WTF::BindOnce(&WebGLSync::AllowCacheUpdate, WrapWeakPersistent(this)));
}
