...
{
    context->ExtensionsUtil()->EnsureExtensionEnabled("GL_EXT_blend_func_extended");
}
...
bool WebGLBlendFuncExtended::Supported(WebGLRenderingContextBase* context)
{
    ...
    if (!context->GetDrawingBuffer()->GetGraphicsInfo().using_passthrough_command_decoder) {
        return false;
    }
    return context->ExtensionsUtil()->SupportsExtension("GL_EXT_blend_func_extended");
}
