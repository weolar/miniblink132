...
public:
    struct ProgramInfoLog {
        ...
        static void LogFunction(gpu::gles2::GLES2Interface* gl, GLuint id, GLint length, GLint* returned_length, LChar* ptr)
        {
            return gl->GetProgramInfoLog(id, length, returned_length, reinterpret_cast<GLchar*>(ptr));
        }
    };
    ...
    struct ShaderInfoLog {
        ...
        static void LogFunction(gpu::gles2::GLES2Interface* gl, GLuint id, GLint length, GLint* returned_length, LChar* ptr)
        {
            gl->GetShaderInfoLog(id, length, returned_length, reinterpret_cast<GLchar*>(ptr));
        }
    };
    ...
    struct TranslatedShaderSourceANGLE {
        ...
        static void LogFunction(gpu::gles2::GLES2Interface* gl, GLuint id, GLint length, GLint* returned_length, LChar* ptr)
        {
            gl->GetTranslatedShaderSourceANGLE(id, length, returned_length, reinterpret_cast<GLchar*>(ptr));
        }
    };
    ...
    template <class Traits> WTF::String Run(GLuint id)
    {
        GLint length = 0;
        ...
        LChar* log_ptr;
        scoped_refptr<WTF::StringImpl> name_impl = WTF::StringImpl::CreateUninitialized(length, log_ptr);
