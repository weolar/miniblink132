...
class MODULES_EXPORT WebLaunchServiceImpl final : public GarbageCollected<WebLaunchServiceImpl>,
                                                  public mojom::blink::WebLaunchService,
                                                  public Supplement<LocalDOMWindow> {
public:
    static const char kSupplementName[];
    static WebLaunchServiceImpl* From(LocalDOMWindow&);
    static void BindReceiver(LocalFrame* frame, mojo::PendingAssociatedReceiver<mojom::blink::WebLaunchService>);
    explicit WebLaunchServiceImpl(base::PassKey<WebLaunchServiceImpl>, LocalDOMWindow& window);
    ...
    void SetLaunchFiles(WTF::Vector<mojom::blink::FileSystemAccessEntryPtr>) override;
    void EnqueueLaunchParams(const KURL& launch_url) override;

private:
    HeapMojoAssociatedReceiver<mojom::blink::WebLaunchService, WebLaunchServiceImpl, HeapMojoWrapperMode::kForceWithoutContextObserver> receiver_;
};
