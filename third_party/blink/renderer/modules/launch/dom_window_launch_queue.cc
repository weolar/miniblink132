...
void DOMWindowLaunchQueue::UpdateLaunchFiles(LocalDOMWindow* window, HeapVector<Member<FileSystemHandle>> files)
{
    FromState(window)->launch_queue_->Enqueue(MakeGarbageCollected<LaunchParams>(std::move(files)));
}

void DOMWindowLaunchQueue::EnqueueLaunchParams(LocalDOMWindow* window, const KURL& launch_url)
{
    FromState(window)->launch_queue_->Enqueue(MakeGarbageCollected<LaunchParams>(launch_url));
}
...
{
    DOMWindowLaunchQueue* supplement = Supplement<LocalDOMWindow>::From<DOMWindowLaunchQueue>(window);
    if (!supplement) {
