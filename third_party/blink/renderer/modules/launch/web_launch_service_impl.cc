...
// static
void WebLaunchServiceImpl::BindReceiver(LocalFrame* frame, mojo::PendingAssociatedReceiver<mojom::blink::WebLaunchService> receiver)
{
    DCHECK(frame);
    auto* service = WebLaunchServiceImpl::From(*frame->DomWindow());
    if (!service) {
        service = MakeGarbageCollected<WebLaunchServiceImpl>(base::PassKey<WebLaunchServiceImpl>(), *frame->DomWindow());
        Supplement<LocalDOMWindow>::ProvideTo(*frame->DomWindow(), service);
    }
    service->Bind(std::move(receiver));
}

WebLaunchServiceImpl::WebLaunchServiceImpl(base::PassKey<WebLaunchServiceImpl>, LocalDOMWindow& window)
...
void WebLaunchServiceImpl::Bind(mojo::PendingAssociatedReceiver<mojom::blink::WebLaunchService> receiver)
{
...
void WebLaunchServiceImpl::SetLaunchFiles(WTF::Vector<mojom::blink::FileSystemAccessEntryPtr> entries)
{
    HeapVector<Member<FileSystemHandle>> files;
    for (auto& entry : entries) {
        files.push_back(FileSystemHandle::CreateFromMojoEntry(std::move(entry), GetSupplementable()->GetExecutionContext()));
    }

    UseCounter::Count(GetSupplementable()->GetExecutionContext(), WebFeature::kFileHandlingLaunch);
    DOMWindowLaunchQueue::UpdateLaunchFiles(GetSupplementable(), std::move(files));
}
