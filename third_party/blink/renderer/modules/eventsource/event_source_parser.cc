...
EventSourceParser::EventSourceParser(const AtomicString& last_event_id, Client* client)
    : id_(last_event_id)
...
bool CmpVector(const Vector<char>& a, const unsigned char* b, size_t size)
{
    if (a.size() != size)
        return false;
...
void EventSourceParser::AddBytes(base::span<const char> bytes)
{
    ...
    for (size_t i = 0; i < bytes.size() && !is_stopped_; ++i) {
        ...
        if (is_recognizing_bom_ && line_.size() + (i - start) == std::size(kBOM)) {
            ...
            if (CmpVector(line, kBOM, sizeof(kBOM))) {
                start = i;
                line_.clear();
                continue;
            }
...
void EventSourceParser::ParseLine()
{
    if (line_.size() == 0) {
        ...
        if (!data_.empty()) {
            ...
            client_->OnMessageEvent(event_type_.empty() ? event_type_names::kMessage : event_type_, data, last_event_id_);
            data_.clear();
        }
    ...
    if (field_name == "retry") {
        const bool has_only_digits = base::ranges::all_of(field_value, IsASCIIDigit<char>);
