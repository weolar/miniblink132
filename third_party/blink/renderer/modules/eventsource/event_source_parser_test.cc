...
class Client : public GarbageCollected<Client>, public EventSourceParser::Client {
public:
...
class StoppingClient : public GarbageCollected<StoppingClient>, public EventSourceParser::Client {
public:
    ...
    void SetParser(EventSourceParser* parser)
    {
        parser_ = parser;
    }
    void OnMessageEvent(const AtomicString& event, const String& data, const AtomicString& id) override
...
protected:
    ...
    EventSourceParserTest()
        ...
        , parser_(MakeGarbageCollected<EventSourceParser>(AtomicString(), client_))
    {
    }
    ~EventSourceParserTest() override = default;

    void Enqueue(std::string_view chars)
    {
        parser_->AddBytes(chars);
    }
...
{
    parser_ = MakeGarbageCollected<EventSourceParser>(AtomicString("hoge"), client_);
...
TEST_F(EventSourceParserTest, InvalidUTF8Sequence)
{
    ...
    ASSERT_EQ(Type::kEvent, Events()[0].type);
    EXPECT_EQ("message", Events()[0].event);
    String expected = String() + kReplacementCharacter + "hello" + kReplacementCharacter + "\nbye";
...
TEST(EventSourceParserStoppingTest, StopWhileParsing)
{
    ...
    EventSourceParser* parser = MakeGarbageCollected<EventSourceParser>(AtomicString(), client);
