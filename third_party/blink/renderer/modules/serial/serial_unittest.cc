...
TEST(SerialTest, CreateMojoFilter_EmptyFilter)
{
    ...
    mojom::blink::SerialPortFilterPtr mojo_filter = Serial::CreateMojoFilter(js_filter, scope.GetExceptionState());
    EXPECT_FALSE(mojo_filter);
    EXPECT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ(ToExceptionCode(ESErrorType::kTypeError), scope.GetExceptionState().Code());
}
...
TEST(SerialTest, CreateMojoFilter_BluetoothServiceClassAndVendorId)
{
    ...
    V8UnionStringOrUnsignedLong* uuid = MakeGarbageCollected<V8UnionStringOrUnsignedLong>(kTestServiceClassId);
    js_filter->setUsbVendorId(kTestVendorId);
...
TEST(SerialTest, CreateMojoFilter_BluetoothServiceClassAndProductId)
{
    ...
    // Can't have both Bluetooth and USB filter parameters.
    js_filter->setUsbProductId(kTestProductId);
    js_filter->setBluetoothServiceClassId(MakeGarbageCollected<V8UnionStringOrUnsignedLong>(kTestServiceClassId));
...
TEST(SerialTest, CreateMojoFilter_InvalidBluetoothServiceClass)
{
    ...
    SerialPortFilter* js_filter = SerialPortFilter::Create(scope.GetIsolate());
    js_filter->setBluetoothServiceClassId(MakeGarbageCollected<V8UnionStringOrUnsignedLong>("invalid-uuid"));
