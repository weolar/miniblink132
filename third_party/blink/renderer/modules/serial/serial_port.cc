...
const char kResourcesExhaustedReadBuffer[] = "Resources exhausted allocating read buffer.";
const char kResourcesExhaustedWriteBuffer[] = "Resources exhausted allocation write buffer.";
const char kNoSignals[] = "Signals dictionary must contain at least one member.";
...
SerialPortInfo* SerialPort::getInfo()
{
    ...
    if (info_->bluetooth_service_class_id) {
        info->setBluetoothServiceClassId(MakeGarbageCollected<V8UnionStringOrUnsignedLong>(info_->bluetooth_service_class_id->uuid));
    }
    return info;
}

ScriptPromise<IDLUndefined> SerialPort::open(ScriptState* script_state, const SerialOptions* options, ExceptionState& exception_state)
{
    ...
    if (open_resolver_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "A call to open() is already in progress.");
        return EmptyPromise();
    }

    if (port_.is_bound()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The port is already open.");
        return EmptyPromise();
    }
    ...
    if (options->baudRate() == 0) {
        exception_state.ThrowTypeError("Requested baud rate must be greater than zero.");
        return EmptyPromise();
    }
    ...
    case 8:
        ...
        break;
    default:
        exception_state.ThrowTypeError("Requested number of data bits must be 7 or 8.");
        return EmptyPromise();
    }
    ...
    case 2:
        ...
        break;
    default:
        exception_state.ThrowTypeError("Requested number of stop bits must be 1 or 2.");
        return EmptyPromise();
    }
    ...
    if (options->bufferSize() > kMaxBufferSize) {
        exception_state.ThrowTypeError(String::Format("Requested buffer size (%d bytes) is greater than "
    ...
    open_resolver_ = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state, exception_state.GetContext());
    auto callback = WTF::BindOnce(&SerialPort::OnOpen, WrapPersistent(this), client.InitWithNewPipeAndPassReceiver());

    parent_->OpenPort(info_->token, std::move(mojo_options), std::move(client), std::move(callback));

    return open_resolver_->Promise();
}

ReadableStream* SerialPort::readable(ScriptState* script_state, ExceptionState& exception_state)
{
    if (readable_)
    ...
    if (!CreateDataPipe(&producer, &consumer)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kQuotaExceededError, kResourcesExhaustedReadBuffer);
        return nullptr;
    }
    ...
    underlying_source_ = MakeGarbageCollected<SerialPortUnderlyingSource>(script_state, this, std::move(consumer));
    readable_ = ReadableStream::CreateByteStream(script_state, underlying_source_);
    return readable_.Get();
}

WritableStream* SerialPort::writable(ScriptState* script_state, ExceptionState& exception_state)
{
    if (writable_)
    ...
    mojo::ScopedDataPipeConsumerHandle consumer;
    if (!CreateDataPipe(&producer, &consumer)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kQuotaExceededError, kResourcesExhaustedWriteBuffer);
        return nullptr;
    }
    ...
    underlying_sink_ = MakeGarbageCollected<SerialPortUnderlyingSink>(this, std::move(producer));
    ...
    writable_ = WritableStream::CreateWithCountQueueingStrategy(script_state, underlying_sink_, /*high_water_mark=*/1);
    return writable_.Get();
}

ScriptPromise<SerialInputSignals> SerialPort::getSignals(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!port_.is_bound()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, kPortClosed);
        return EmptyPromise();
    }

    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<SerialInputSignals>>(script_state, exception_state.GetContext());
    signal_resolvers_.insert(resolver);
    port_->GetControlSignals(resolver->WrapCallbackInScriptScope(WTF::BindOnce(&SerialPort::OnGetSignals, WrapPersistent(this))));
    return resolver->Promise();
}

ScriptPromise<IDLUndefined> SerialPort::setSignals(ScriptState* script_state, const SerialOutputSignals* signals, ExceptionState& exception_state)
{
    ...
    if (signals->hasRequestToSend()) {
        ...
        if (hardware_flow_control_) {
            ...
            context->AddConsoleMessage(
                MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRecommendation, mojom::blink::ConsoleMessageLevel::kInfo,
    ...
    signal_resolvers_.insert(resolver);
    port_->SetControlSignals(std::move(mojo_signals), resolver->WrapCallbackInScriptScope(WTF::BindOnce(&SerialPort::OnSetSignals, WrapPersistent(this))));
    return resolver->Promise();
}

ScriptPromise<IDLUndefined> SerialPort::close(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!port_.is_bound()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The port is already closed.");
        return EmptyPromise();
    }

    if (IsClosing()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "A call to close() is already in progress.");
        return EmptyPromise();
    }
    ...
    if (writable_) {
        ScriptValue reason(
            script_state->GetIsolate(), V8ThrowDOMException::CreateOrDie(script_state->GetIsolate(), DOMExceptionCode::kInvalidStateError, kPortClosed));
...
ScriptPromise<IDLUndefined> SerialPort::forget(ScriptState* script_state, ExceptionState& exception_state)
{
    ExecutionContext* context = GetExecutionContext();
    if (!context) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Script context has shut down.");
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state, exception_state.GetContext());
    parent_->ForgetPort(info_->token, WTF::BindOnce([](ScriptPromiseResolver<IDLUndefined>* resolver) { resolver->Resolve(); }, WrapPersistent(resolver)));
...
void SerialPort::StreamsClosed()
{
    ...
    port_->Close(/*flush=*/true, WTF::BindOnce(&SerialPort::OnClose, WrapPersistent(this)));
}

void SerialPort::Flush(device::mojom::blink::SerialPortFlushMode mode, device::mojom::blink::SerialPort::FlushCallback callback)
{
    DCHECK(port_.is_bound());
    port_->Flush(mode, std::move(callback));
}

void SerialPort::Drain(device::mojom::blink::SerialPort::DrainCallback callback)
{
    DCHECK(port_.is_bound());
...
bool SerialPort::CreateDataPipe(mojo::ScopedDataPipeProducerHandle* producer, mojo::ScopedDataPipeConsumerHandle* consumer)
{
...
void SerialPort::OnConnectionError()
{
    ...
    if (open_resolver_) {
        ...
        if (IsInParallelAlgorithmRunnable(open_resolver_->GetExecutionContext(), script_state)) {
            ScriptState::Scope script_state_scope(script_state);
            open_resolver_->RejectWithDOMException(DOMExceptionCode::kNetworkError, kOpenError);
    ...
    for (auto& resolver : signal_resolvers_) {
        ...
        if (IsInParallelAlgorithmRunnable(resolver->GetExecutionContext(), script_state)) {
            ScriptState::Scope script_state_scope(script_state);
            resolver->RejectWithDOMException(DOMExceptionCode::kNetworkError, kDeviceLostError);
        }
    }
...
void SerialPort::OnOpen(
    mojo::PendingReceiver<device::mojom::blink::SerialPortClient> client_receiver, mojo::PendingRemote<device::mojom::blink::SerialPort> port)
{
    if (!port) {
        open_resolver_->RejectWithDOMException(DOMExceptionCode::kNetworkError, kOpenError);
        open_resolver_ = nullptr;
    ...
    feature_handle_for_scheduler_ = execution_context->GetScheduler()->RegisterFeature(
        SchedulingPolicy::Feature::kWebSerial, SchedulingPolicy { SchedulingPolicy::DisableAggressiveThrottling() });

    port_.Bind(std::move(port), execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI));
    port_.set_disconnect_handler(WTF::BindOnce(&SerialPort::OnConnectionError, WrapWeakPersistent(this)));
    client_receiver_.Bind(std::move(client_receiver), execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI));

    open_resolver_->Resolve();
    open_resolver_ = nullptr;
}

void SerialPort::OnGetSignals(ScriptPromiseResolver<SerialInputSignals>* resolver, device::mojom::blink::SerialPortControlSignalsPtr mojo_signals)
{
    DCHECK(signal_resolvers_.Contains(resolver));
    signal_resolvers_.erase(resolver);

    if (!mojo_signals) {
        resolver->RejectWithDOMException(DOMExceptionCode::kNetworkError, "Failed to get control signals.");
        return;
    }
...
void SerialPort::OnSetSignals(ScriptPromiseResolver<IDLUndefined>* resolver, bool success)
{
    DCHECK(signal_resolvers_.Contains(resolver));
    signal_resolvers_.erase(resolver);

    if (!success) {
        resolver->RejectWithDOMException(DOMExceptionCode::kNetworkError, "Failed to set control signals.");
        return;
    }
