...
class BroadcastChannelTester : public GarbageCollected<BroadcastChannelTester>, public mojom::blink::BroadcastChannelClient {
public:
        ...
        , remote_(execution_context)
    {
        ...
        mojo::PendingAssociatedReceiver<mojom::blink::BroadcastChannelClient> receiver0;
        mojo::PendingAssociatedRemote<mojom::blink::BroadcastChannelClient> remote0 = receiver0.InitWithNewEndpointAndPassRemote();
        receiver0.EnableUnassociatedUsage();
        mojo::PendingAssociatedReceiver<mojom::blink::BroadcastChannelClient> receiver1;
        mojo::PendingAssociatedRemote<mojom::blink::BroadcastChannelClient> remote1 = receiver1.InitWithNewEndpointAndPassRemote();
        receiver1.EnableUnassociatedUsage();

        scoped_refptr<base::SequencedTaskRunner> task_runner = execution_context->GetTaskRunner(TaskType::kInternalTest);
        ...
        channel_ = MakeGarbageCollected<BroadcastChannel>(
            base::PassKey<BroadcastChannelTester>(), execution_context, "BroadcastChannelTester", std::move(receiver1), std::move(remote0));
    ...
    BroadcastChannel* channel() const
    {
        return channel_.Get();
    }
...
private:
    ...
    public:
        explicit EventListener(BroadcastChannelTester* tester)
            : tester_(tester)
        {
        }
    ...
    HeapMojoAssociatedReceiver<mojom::blink::BroadcastChannelClient, BroadcastChannelTester> receiver_;
...
TEST(BroadcastChannelTest, DispatchMessageEvent)
{
    ...
    auto* tester = MakeGarbageCollected<BroadcastChannelTester>(execution_context);
...
TEST(BroadcastChannelTest, AgentClusterLockedMismatch)
{
    ...
    ASSERT_EQ(tester->received_events().size(), 1u);
    EXPECT_EQ(tester->received_events()[0]->type(), event_type_names::kMessageerror);
}
...
TEST(BroadcastChannelTest, OutgoingMessagesMarkedWithAgentClusterId)
{
    ...
    tester->AwaitNextUpdate(run_loop.QuitClosure());
    {
        ...
        tester->channel()->postMessage(ScriptValue::CreateNull(script_state->GetIsolate()), ASSERT_NO_EXCEPTION);
    }
    run_loop.Run();

    ASSERT_EQ(tester->sent_messages().size(), 1u);
    EXPECT_EQ(tester->sent_messages()[0].sender_agent_cluster_id, execution_context->GetAgentClusterID());
...
#if BUILDFLAG(IS_IOS)
#define MAYBE_OutgoingAgentClusterLockedMessage DISABLED_OutgoingAgentClusterLockedMessage
#else
#define MAYBE_OutgoingAgentClusterLockedMessage OutgoingAgentClusterLockedMessage
#endif
...
TEST(BroadcastChannelTest, MAYBE_OutgoingAgentClusterLockedMessage)
{
    ...
    tester->AwaitNextUpdate(run_loop.QuitClosure());
    {
        ...
        static constexpr uint8_t kTrivialModuleBytes[] = { 0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00 };
        ScriptState::Scope scope(script_state);
        v8::Local<v8::WasmModuleObject> trivial_module
            = v8::WasmModuleObject::Compile(isolate, { std::data(kTrivialModuleBytes), std::size(kTrivialModuleBytes) }).ToLocalChecked();
        tester->channel()->postMessage(ScriptValue(isolate, trivial_module), ASSERT_NO_EXCEPTION);
    }
    run_loop.Run();
