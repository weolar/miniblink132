...
class GeoNotifier final : public GarbageCollected<GeoNotifier>, public NameClient {
public:
    GeoNotifier(Geolocation*, V8PositionCallback*, V8PositionErrorCallback*, const PositionOptions*);
    ...
    const PositionOptions* Options() const
    {
        return options_.Get();
    }
    ...
    bool UseCachedPosition() const
    {
        return use_cached_position_;
    }
...
private:
    ...
    public:
        explicit Timer(scoped_refptr<base::SingleThreadTaskRunner> web_task_runner, GeoNotifier* notifier, void (GeoNotifier::*member_func)(TimerBase*))
            : timer_(web_task_runner, notifier, member_func)
            , notifier_(notifier)
        {
        }
