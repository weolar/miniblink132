...
GeoNotifier::GeoNotifier(
    Geolocation* geolocation, V8PositionCallback* success_callback, V8PositionErrorCallback* error_callback, const PositionOptions* options)
    : geolocation_(geolocation)
    , success_callback_(success_callback)
    , error_callback_(error_callback)
    , options_(options)
    , timer_(MakeGarbageCollected<Timer>(geolocation->DomWindow()->GetTaskRunner(TaskType::kMiscPlatformAPI), this, &GeoNotifier::TimerFired))
...
void GeoNotifier::Timer::StartOneShot(base::TimeDelta interval, const base::Location& caller)
{
...
void GeoNotifier::TimerFired(TimerBase*)
{
    ...
    if (error_callback_) {
        error_callback_->InvokeAndReportException(
            nullptr, MakeGarbageCollected<GeolocationPositionError>(GeolocationPositionError::kTimeout, "Timeout expired"));
    }
