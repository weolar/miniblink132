...
public:
    ...
    void populateMatrix(const V8RotationMatrixType* target_buffer, ExceptionState& exception_state);
...
protected:
    OrientationSensor(ExecutionContext*, const SpatialSensorOptions*, ExceptionState&, device::mojom::blink::SensorType,
        const Vector<mojom::blink::PermissionsPolicyFeature>& features);

private:
    // SensorProxy override.
    void OnSensorReadingChanged() override;
    template <typename Matrix> void PopulateMatrixInternal(Matrix*, ExceptionState&);
