...
template <typename T> void DoPopulateMatrix(T* target_matrix, double x, double y, double z, double w)
{
...
template <> void DoPopulateMatrix(DOMMatrix* target_matrix, double x, double y, double z, double w)
{
...
template <typename T> bool CheckBufferLength(T* buffer)
{
    return buffer->length() >= 16;
}

template <> bool CheckBufferLength(DOMMatrix*)
{
    return true;
}

template <typename Matrix> void OrientationSensor::PopulateMatrixInternal(Matrix* target_matrix, ExceptionState& exception_state)
{
    if (!CheckBufferLength(target_matrix)) {
        exception_state.ThrowTypeError("Target buffer must have at least 16 elements.");
        return;
    }
    if (!hasReading()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotReadableError, "Sensor data is not available.");
        return;
    }
...
void OrientationSensor::populateMatrix(const V8RotationMatrixType* target_buffer, ExceptionState& exception_state)
{
    ...
    case V8RotationMatrixType::ContentType::kFloat32Array:
        PopulateMatrixInternal(target_buffer->GetAsFloat32Array().Get(), exception_state);
        break;
    case V8RotationMatrixType::ContentType::kFloat64Array:
        PopulateMatrixInternal(target_buffer->GetAsFloat64Array().Get(), exception_state);
        break;
    }
}
...
OrientationSensor::OrientationSensor(ExecutionContext* execution_context, const SpatialSensorOptions* options, ExceptionState& exception_state,
    device::mojom::blink::SensorType type, const Vector<mojom::blink::PermissionsPolicyFeature>& features)
