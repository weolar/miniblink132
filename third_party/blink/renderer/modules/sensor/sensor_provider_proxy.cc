...
void SensorProviderProxy::InitializeIfNeeded()
{
    ...
    GetSupplementable()->GetBrowserInterfaceBroker().GetInterface(
        sensor_provider_.BindNewPipeAndPassReceiver(GetSupplementable()->GetTaskRunner(TaskType::kSensor)));
    sensor_provider_.set_disconnect_handler(WTF::BindOnce(&SensorProviderProxy::OnSensorProviderConnectionError, WrapWeakPersistent(this)));
}
...
{
    DCHECK(window);
    SensorProviderProxy* provider_proxy = Supplement<LocalDOMWindow>::From<SensorProviderProxy>(*window);
    if (!provider_proxy) {
...
SensorProxy* SensorProviderProxy::CreateSensorProxy(device::mojom::blink::SensorType type, Page* page)
{
    DCHECK(!GetSensorProxy(type));

    SensorProxy* sensor = static_cast<SensorProxy*>(MakeGarbageCollected<SensorProxyImpl>(type, this, page));
    sensor_proxies_.insert(sensor);

    return sensor;
}

SensorProxy* SensorProviderProxy::GetSensorProxy(device::mojom::blink::SensorType type)
{
...
void SensorProviderProxy::OnSensorProviderConnectionError()
{
    ...
    for (SensorProxy* sensor : sensor_proxies_) {
        sensor->ReportError(DOMExceptionCode::kNotReadableError, SensorProxy::kDefaultErrorDescription);
    }
}
...
void SensorProviderProxy::GetSensor(device::mojom::blink::SensorType type, mojom::blink::WebSensorProviderProxy::GetSensorCallback callback)
{
    InitializeIfNeeded();
