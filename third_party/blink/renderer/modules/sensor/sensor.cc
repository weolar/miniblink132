...
bool AreFeaturesEnabled(ExecutionContext* context, const Vector<mojom::blink::PermissionsPolicyFeature>& features)
{
    return base::ranges::all_of(
        features, [context](mojom::blink::PermissionsPolicyFeature feature) { return context->IsFeatureEnabled(feature, ReportOptions::kReportOnFailure); });
}

} // namespace

Sensor::Sensor(ExecutionContext* execution_context, const SensorOptions* sensor_options, ExceptionState& exception_state, device::mojom::blink::SensorType type,
    ...
    , last_reported_timestamp_(0.0)
{
    ...
    if (!AreFeaturesEnabled(execution_context, features)) {
        exception_state.ThrowSecurityError("Access to sensor features is disallowed by permissions policy");
        return;
    }
    ...
    if (sensor_options->hasFrequency()) {
        ...
        const double max_allowed_frequency = device::GetSensorMaxAllowedFrequency(type_);
        if (frequency_ > max_allowed_frequency) {
            frequency_ = max_allowed_frequency;
            String message = String::Format("Maximum allowed frequency value for this sensor type is %.0f Hz.", max_allowed_frequency);
            auto* console_message
                = MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kInfo, std::move(message));
            execution_context->AddConsoleMessage(console_message);
        }
    }
}

Sensor::Sensor(ExecutionContext* execution_context, const SpatialSensorOptions* options, ExceptionState& exception_state,
    device::mojom::blink::SensorType sensor_type, const Vector<mojom::blink::PermissionsPolicyFeature>& features)
    : Sensor(execution_context, static_cast<const SensorOptions*>(options), exception_state, sensor_type, features)
{
...
std::optional<DOMHighResTimeStamp> Sensor::timestamp(ScriptState* script_state) const
{
    if (!hasReading()) {
        return std::nullopt;
    }
    ...
    return performance->MonotonicTimeToDOMHighResTimeStamp(base::TimeTicks() + base::Seconds(sensor_proxy_->GetReading().timestamp()));
}
...
void Sensor::InitSensorProxyIfNeeded()
{
    ...
    if (!sensor_proxy_) {
        sensor_proxy_ = provider->CreateSensorProxy(type_, window->GetFrame()->GetPage());
    }
}
...
void Sensor::OnSensorReadingChanged()
{
    ...
    if (pending_reading_notification_.IsActive())
        return;
    double elapsedTime = sensor_proxy_->GetReading().timestamp() - last_reported_timestamp_;
    ...
    // polling period.
    auto sensor_reading_changed = WTF::BindOnce(&Sensor::NotifyReading, WrapWeakPersistent(this));
    if (waitingTime < kWaitingIntervalThreshold) {
        ...
        pending_reading_notification_
            = PostCancellableTask(*GetExecutionContext()->GetTaskRunner(TaskType::kSensor), FROM_HERE, std::move(sensor_reading_changed));
    } else {
        pending_reading_notification_ = PostDelayedCancellableTask(
            *GetExecutionContext()->GetTaskRunner(TaskType::kSensor), FROM_HERE, std::move(sensor_reading_changed), base::Seconds(waitingTime));
    }
}

void Sensor::OnSensorError(DOMExceptionCode code, const String& sanitized_message, const String& unsanitized_message)
{
...
void Sensor::OnAddConfigurationRequestCompleted(bool result)
{
    ...
    if (!result) {
        HandleError(DOMExceptionCode::kNotReadableError, "start() call has failed.");
        return;
    }
    ...
    pending_activated_notification_ = PostCancellableTask(
        *GetExecutionContext()->GetTaskRunner(TaskType::kSensor), FROM_HERE, WTF::BindOnce(&Sensor::NotifyActivated, WrapWeakPersistent(this)));
}
...
void Sensor::RequestAddConfiguration()
{
    if (!configuration_) {
        ...
        DCHECK(configuration_);
        DCHECK_GE(configuration_->frequency, sensor_proxy_->GetFrequencyLimits().first);
        DCHECK_LE(configuration_->frequency, sensor_proxy_->GetFrequencyLimits().second);
    }

    DCHECK(sensor_proxy_);
    sensor_proxy_->AddConfiguration(configuration_->Clone(), WTF::BindOnce(&Sensor::OnAddConfigurationRequestCompleted, WrapWeakPersistent(this)));
}

void Sensor::HandleError(DOMExceptionCode code, const String& sanitized_message, const String& unsanitized_message)
{
    if (!GetExecutionContext()) {
    ...
    auto* error = MakeGarbageCollected<DOMException>(code, sanitized_message, unsanitized_message);
    pending_error_notification_ = PostCancellableTask(*GetExecutionContext()->GetTaskRunner(TaskType::kSensor), FROM_HERE,
        WTF::BindOnce(&Sensor::NotifyError, WrapWeakPersistent(this), WrapPersistent(error)));
}
...
void Sensor::NotifyActivated()
{
    ...
    if (hasReading()) {
        ...
        pending_reading_notification_ = PostCancellableTask(
            *GetExecutionContext()->GetTaskRunner(TaskType::kSensor), FROM_HERE, WTF::BindOnce(&Sensor::OnSensorReadingChanged, WrapWeakPersistent(this)));
    }
...
{
    return (state_ == SensorState::kIdle) || pending_error_notification_.IsActive();
}
