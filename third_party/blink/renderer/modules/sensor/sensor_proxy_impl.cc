...
SensorProxyImpl::SensorProxyImpl(device::mojom::blink::SensorType sensor_type, SensorProviderProxy* provider, Page* page)
    ...
    , task_runner_(provider->GetSupplementable()->GetTaskRunner(TaskType::kSensor))
    , polling_timer_(provider->GetSupplementable()->GetTaskRunner(TaskType::kSensor), this, &SensorProxyImpl::OnPollingTimer)
{
}
...
void SensorProxyImpl::Initialize()
{
    ...
    sensor_provider_proxy()->GetSensor(type_, WTF::BindOnce(&SensorProxyImpl::OnSensorCreated, WrapWeakPersistent(this)));
}

void SensorProxyImpl::AddConfiguration(device::mojom::blink::SensorConfigurationPtr configuration, base::OnceCallback<void(bool)> callback)
{
    DCHECK(IsInitialized());
    AddActiveFrequency(configuration->frequency);
    sensor_remote_->AddConfiguration(std::move(configuration), std::move(callback));
}

void SensorProxyImpl::RemoveConfiguration(device::mojom::blink::SensorConfigurationPtr configuration)
{
    DCHECK(IsInitialized());
...
void SensorProxyImpl::UpdateSensorReading()
{
    ...
    if (reading_.timestamp() != latest_timestamp && latest_timestamp != 0.0) // The shared buffer is zeroed when
    // sensor is stopped, we skip this
    // reading.
    {
        DCHECK_GT(latest_timestamp, reading_.timestamp()) << "Timestamps must increase monotonically";
...
void SensorProxyImpl::ReportError(DOMExceptionCode code, const String& message)
{
...
void SensorProxyImpl::OnSensorCreated(SensorCreationResult result, device::mojom::blink::SensorInitParamsPtr params)
{
    ...
    shared_buffer_reader_ = device::SensorReadingSharedBufferReader::Create(std::move(params->memory), params->buffer_offset);
    ...
    DCHECK_GE(device::GetSensorMaxAllowedFrequency(type_), frequency_limits_.second);

    auto error_callback = WTF::BindOnce(&SensorProxyImpl::HandleSensorError, WrapWeakPersistent(this), SensorCreationResult::ERROR_NOT_AVAILABLE);
...
void SensorProxyImpl::UpdatePollingStatus()
{
    ...
    if (ShouldProcessReadings()) {
        ...
        // polling frequency.
        polling_timer_.StartRepeating(base::Seconds(1 / active_frequencies_.back()), FROM_HERE);
    } else {
        polling_timer_.Stop();
    }
}
...
void SensorProxyImpl::RemoveActiveFrequency(double frequency)
{
    ...
    Vector<double>::iterator it = std::lower_bound(active_frequencies_.begin(), active_frequencies_.end(), frequency);
...
void SensorProxyImpl::AddActiveFrequency(double frequency)
{
    ...
    if (it == active_frequencies_.end()) {
        ...
        active_frequencies_.insert(static_cast<wtf_size_t>(std::distance(active_frequencies_.begin(), it)), frequency);
    }
    UpdatePollingStatus();
}
