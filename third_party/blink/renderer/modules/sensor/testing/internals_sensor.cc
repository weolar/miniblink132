...
device::mojom::blink::SensorType ToMojoSensorType(V8VirtualSensorType::Enum type)
{
    switch (type) {
...
device::mojom::blink::VirtualSensorMetadataPtr ToMojoSensorMetadata(CreateVirtualSensorOptions* options)
{
    if (!options) {
...
base::expected<device::mojom::blink::SensorReadingRawPtr, String> ToMojoRawReading(V8VirtualSensorType::Enum type, VirtualSensorReading* reading)
{
    if (!reading) {
    ...
    raw_reading->timestamp = (base::TimeTicks::Now() - base::TimeTicks()).InSecondsF();
    ...
    case V8VirtualSensorType::Enum::kRelativeOrientation: {
        if (reading->hasAlpha() && reading->hasBeta() && reading->hasGamma()) {
            ...
            if (!device::ComputeQuaternionFromEulerAngles(alpha, beta, gamma, &quaternion_readings)) {
            ...
            return base::unexpected("'alpha'/'beta'/'gamma' expected in the readings");
        }
        break;
    }
    case V8VirtualSensorType::Enum::kAmbientLight:
...
ScriptPromise<IDLUndefined> InternalsSensor::createVirtualSensor(
    ScriptState* script_state, Internals&, V8VirtualSensorType type, CreateVirtualSensorOptions* options)
{
    LocalDOMWindow* window = LocalDOMWindow::From(script_state);
    CHECK(window);
    mojo::Remote<test::mojom::blink::WebSensorProviderAutomation> virtual_sensor_provider;
    window->GetBrowserInterfaceBroker().GetInterface(virtual_sensor_provider.BindNewPipeAndPassReceiver());
    ...
    raw_virtual_sensor_provider->CreateVirtualSensor(ToMojoSensorType(type.AsEnum()), ToMojoSensorMetadata(options),
        WTF::BindOnce(
            // While we only really need |resolver|, we also take the
            // mojo::Remote<> so that it remains alive after this function exits.
            [](ScriptPromiseResolver<IDLUndefined>* resolver, mojo::Remote<test::mojom::blink::WebSensorProviderAutomation>,
                device::mojom::blink::CreateVirtualSensorResult result) {
                switch (result) {
                case device::mojom::blink::CreateVirtualSensorResult::kSuccess:
                    resolver->Resolve();
                    break;
                case device::mojom::blink::CreateVirtualSensorResult::kSensorTypeAlreadyOverridden:
...
// static
ScriptPromise<IDLUndefined> InternalsSensor::updateVirtualSensor(ScriptState* script_state, Internals&, V8VirtualSensorType type, VirtualSensorReading* reading)
{
    auto mojo_reading = ToMojoRawReading(type.AsEnum(), reading);
    if (!mojo_reading.has_value()) {
        return ScriptPromise<IDLUndefined>::Reject(
            script_state, V8ThrowDOMException::CreateOrEmpty(script_state->GetIsolate(), DOMExceptionCode::kInvalidStateError, mojo_reading.error()));
    }
    ...
    auto promise = resolver->Promise();
    auto* raw_virtual_sensor_provider = virtual_sensor_provider.get();
    raw_virtual_sensor_provider->UpdateVirtualSensor(ToMojoSensorType(type.AsEnum()), std::move(mojo_reading.value()),
        WTF::BindOnce(
            // While we only really need |resolver|, we also take the
            // mojo::Remote<> so that it remains alive after this function exits.
            [](ScriptPromiseResolver<IDLUndefined>* resolver, mojo::Remote<test::mojom::blink::WebSensorProviderAutomation>,
                ...
                case device::mojom::blink::UpdateVirtualSensorResult::kSensorTypeNotOverridden:
                    resolver->Reject("A virtual sensor with this type has not been created");
                    break;
                }
            },
            WrapPersistent(resolver), std::move(virtual_sensor_provider)));
...
// static
ScriptPromise<IDLUndefined> InternalsSensor::removeVirtualSensor(ScriptState* script_state, Internals&, V8VirtualSensorType type)
{
    LocalDOMWindow* window = LocalDOMWindow::From(script_state);
    CHECK(window);
    mojo::Remote<test::mojom::blink::WebSensorProviderAutomation> virtual_sensor_provider;
    ...
    auto promise = resolver->Promise();
    auto* raw_virtual_sensor_provider = virtual_sensor_provider.get();
    raw_virtual_sensor_provider->RemoveVirtualSensor(ToMojoSensorType(type.AsEnum()),
        WTF::BindOnce(
            // While we only really need |resolver|, we also take the
            // mojo::Remote<> so that it remains alive after this function exits.
            [](ScriptPromiseResolver<IDLUndefined>* resolver, mojo::Remote<test::mojom::blink::WebSensorProviderAutomation>) { resolver->Resolve(); },
            WrapPersistent(resolver), std::move(virtual_sensor_provider)));
    return promise;
}

// static
ScriptPromise<VirtualSensorInformation> InternalsSensor::getVirtualSensorInformation(ScriptState* script_state, Internals&, V8VirtualSensorType type)
{
    LocalDOMWindow* window = LocalDOMWindow::From(script_state);
    CHECK(window);
    mojo::Remote<test::mojom::blink::WebSensorProviderAutomation> virtual_sensor_provider;
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<VirtualSensorInformation>>(script_state);
    auto promise = resolver->Promise();
    auto* raw_virtual_sensor_provider = virtual_sensor_provider.get();
    raw_virtual_sensor_provider->GetVirtualSensorInformation(ToMojoSensorType(type.AsEnum()),
        WTF::BindOnce(
            // While we only really need |resolver|, we also take the
            // mojo::Remote<> so that it remains alive after this function exits.
            [](ScriptPromiseResolver<VirtualSensorInformation>* resolver, mojo::Remote<test::mojom::blink::WebSensorProviderAutomation>,
                device::mojom::blink::GetVirtualSensorInformationResultPtr result) {
                if (result->is_error()) {
                    switch (result->get_error()) {
                    case device::mojom::blink::GetVirtualSensorInformationError::kSensorTypeNotOverridden:
                        resolver->Reject("A virtual sensor with this type has not been created");
                        return;
                    }
                }
                ...
                sensor_info->setRequestedSamplingFrequency(result->get_info()->sampling_frequency);
