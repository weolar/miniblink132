    ...
    : testing_scope_(KURL("https://example.com"))
{
    ...
    testing_scope_.GetFrame().GetBrowserInterfaceBroker().SetBinderForTesting(
        mojom::blink::WebSensorProvider::Name_, WTF::BindRepeating(&SensorTestContext::BindSensorProviderRequest, WTF::Unretained(this)));
}

SensorTestContext::~SensorTestContext()
{
    testing_scope_.GetFrame().GetBrowserInterfaceBroker().SetBinderForTesting(mojom::blink::WebSensorProvider::Name_, {});
}
...
void SensorTestContext::BindSensorProviderRequest(mojo::ScopedMessagePipeHandle handle)
{
    sensor_provider_.Bind(mojo::PendingReceiver<device::mojom::SensorProvider>(std::move(handle)));
}

// SensorTestUtils

// static
void SensorTestUtils::WaitForEvent(EventTarget* event_target, const WTF::AtomicString& event_type)
{
