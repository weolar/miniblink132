...
const char SensorProxy::kDefaultErrorDescription[] = "Could not connect to a sensor";

SensorProxy::SensorProxy(device::mojom::blink::SensorType sensor_type, SensorProviderProxy* provider, Page* page)
...
const device::SensorReading& SensorProxy::GetReading(bool remapped) const
{
    ...
    if (remapped) {
        if (remapped_reading_.timestamp() != reading_.timestamp()) {
            ...
            SensorReadingRemapper::RemapToScreenCoords(type_, frame.GetChromeClient().GetScreenInfo(frame).orientation_angle, &remapped_reading_);
        }
...
bool SensorProxy::ShouldSuspendUpdates() const
{
    ...
    const SecurityOrigin* focused_frame_origin = focused_frame->GetSecurityContext()->GetSecurityOrigin();
    const SecurityOrigin* this_origin = this_frame->GetSecurityContext()->GetSecurityOrigin();
