...
PermissionStatusListener* PermissionStatusListener::Create(
    Permissions& associated_permissions_object, ExecutionContext* execution_context, MojoPermissionStatus status, MojoPermissionDescriptor descriptor)
{
    PermissionStatusListener* permission_status
        = MakeGarbageCollected<PermissionStatusListener>(associated_permissions_object, execution_context, status, std::move(descriptor));
...
void PermissionStatusListener::StartListening()
{
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = GetExecutionContext()->GetTaskRunner(TaskType::kPermission);
    receiver_.Bind(observer.InitWithNewPipeAndPassReceiver(), task_runner);

    mojo::Remote<mojom::blink::PermissionService> service;
    ConnectToPermissionService(GetExecutionContext(), service.BindNewPipeAndPassReceiver(task_runner));
    service->AddPermissionObserver(descriptor_->Clone(), status_, std::move(observer));
}
...
void PermissionStatusListener::NotifyEventListener(const AtomicString& event_type, bool is_added)
{
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = GetExecutionContext()->GetTaskRunner(TaskType::kPermission);
...
void PermissionStatusListener::OnPermissionStatusChange(MojoPermissionStatus status)
{
    if (status_ == status)
        return;
...
void PermissionStatusListener::AddedEventListener(const AtomicString& event_type)
{
    if (observers_.empty())
        StartListening();
...
void PermissionStatusListener::RemovedEventListener(const AtomicString& event_type)
{
    if (observers_.empty())
        StartListening();
