...
class MODULES_EXPORT NavigatorManagedData final : public EventTarget,
...
public:
    ...
    void AddedEventListener(const AtomicString& event_type, RegisteredEventListener& registered_listener) override;
    void RemovedEventListener(const AtomicString& event_type, const RegisteredEventListener& registered_listener) override;

    // ScriptWrappable implementation.
    bool HasPendingActivity() const final;

    // Managed Configuration API:
    ScriptPromise<IDLRecord<IDLString, IDLAny>> getManagedConfiguration(ScriptState* script_state, Vector<String> keys);
    DEFINE_ATTRIBUTE_EVENT_LISTENER(managedconfigurationchange, kManagedconfigurationchange)
...
private:
    ...
    void OnConfigurationReceived(
        ScriptPromiseResolver<IDLRecord<IDLString, IDLAny>>* scoped_resolver, const std::optional<HashMap<String, String>>& configurations);

    void OnAttributeReceived(ScriptState* script_state, ScriptPromiseResolver<IDLNullable<IDLString>>* resolver, mojom::blink::DeviceAttributeResultPtr result);
    ...
    HeapMojoRemote<mojom::blink::ManagedConfigurationService> managed_configuration_service_;

    HeapMojoReceiver<mojom::blink::ManagedConfigurationObserver, NavigatorManagedData> configuration_observer_;
