...
public:
    ...
    // record.
    static NDEFMessage* Create(const ScriptState*, const NDEFMessageInit*, ExceptionState&, uint8_t records_depth = 0U, bool is_embedded = false);
    static NDEFMessage* Create(const ScriptState*, const V8NDEFMessageSource* source, ExceptionState& exception_state);
    static NDEFMessage* CreateAsPayloadOfSmartPoster(const ScriptState*, const NDEFMessageInit*, ExceptionState&, uint8_t records_depth);
