...
public:
    ...
    static NDEFRecord* CreateForBindings(const ScriptState*, const NDEFRecordInit*, ExceptionState&);

    // |is_embedded| indicates if this record is within the context of a parent
    // record.
    static NDEFRecord* Create(const ScriptState*, const NDEFRecordInit*, ExceptionState&, uint8_t records_depth, bool is_embedded);

    explicit NDEFRecord(device::mojom::NDEFRecordTypeCategory, const String& record_type, const String& id, WTF::Vector<uint8_t>);

    // For constructing a "smart-poster", an external type or a local type record
    // whose payload is an NDEF message.
    explicit NDEFRecord(device::mojom::NDEFRecordTypeCategory, const String& record_type, const String& id, NDEFMessage*);

    // Only for constructing "text" type record. The type category will be
    // device::mojom::NDEFRecordTypeCategory::kStandardized.
    explicit NDEFRecord(const String& id, const String& encoding, const String& lang, WTF::Vector<uint8_t>);
    ...
    // device::mojom::NDEFRecordTypeCategory::kStandardized.
    explicit NDEFRecord(const String& id, const String& media_type, WTF::Vector<uint8_t>);
    ...
    std::optional<HeapVector<Member<NDEFRecord>>> toRecords(ExceptionState& exception_state) const;

    device::mojom::NDEFRecordTypeCategory category() const
    {
        return category_;
    }
