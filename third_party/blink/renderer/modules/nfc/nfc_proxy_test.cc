...
static const char kFakeRecordId[] = "https://w3c.github.io/web-nfc/dummy-record-id";
...
public:
    ...
    MOCK_METHOD2(OnReading, void(const String& serial_number, const device::mojom::blink::NDEFMessage& message));
};
...
public:
    ...
    void BindRequest(mojo::ScopedMessagePipeHandle handle)
    {
        ...
        receiver_.Bind(mojo::PendingReceiver<device::mojom::blink::NFC>(std::move(handle)));
        receiver_.set_disconnect_handler(WTF::BindOnce(&FakeNfcService::OnConnectionError, WTF::Unretained(this)));
    }
    ...
    void TriggerWatchEvent()
    {
        ...
        client_->OnWatch(std::move(watchIDs_), kFakeNfcTagSerialNumber, tag_message_.Clone());
    }
...
private:
    ...
    void SetClient(mojo::PendingRemote<device::mojom::blink::NFCClient> client) override
    {
        client_.Bind(std::move(client));
    }
    void Push(device::mojom::blink::NDEFMessagePtr message, device::mojom::blink::NDEFWriteOptionsPtr options, PushCallback callback) override
    {
...
public:
    NFCProxyTest()
    {
        nfc_service_ = std::make_unique<FakeNfcService>();
    }
    ...
    void SetUp() override
    {
        ...
        GetFrame().DomWindow()->GetBrowserInterfaceBroker().SetBinderForTesting(
            device::mojom::blink::NFC::Name_, WTF::BindRepeating(&FakeNfcService::BindRequest, WTF::Unretained(nfc_service())));
    }

    void TearDown() override
    {
        GetFrame().DomWindow()->GetBrowserInterfaceBroker().SetBinderForTesting(device::mojom::blink::NFC::Name_, {});
    }

    FakeNfcService* nfc_service()
    {
        return nfc_service_.get();
    }
...
TEST_F(NFCProxyTest, SuccessfulPath)
{
    ...
    {
        base::RunLoop loop;
        nfc_proxy->StartReading(reader, base::BindLambdaForTesting([&](device::mojom::blink::NDEFErrorPtr error) {
    ...
    WTF::Vector<uint8_t> record_data({ 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10 });
    ...
    {
        base::RunLoop loop;
        EXPECT_CALL(*reader, OnReading(String(kFakeNfcTagSerialNumber), MessageEquals(record_data)))
            .WillOnce(Invoke([&](const String& serial_number, const device::mojom::blink::NDEFMessage& message) { loop.Quit(); }));

        nfc_proxy->Push(std::move(message), /*options=*/nullptr,
            base::BindLambdaForTesting([&](device::mojom::blink::NDEFErrorPtr error) { nfc_service()->TriggerWatchEvent(); }));
        loop.Run();
    }
...
TEST_F(NFCProxyTest, ErrorPath)
{
    ...
    nfc_service()->set_watch_error(device::mojom::blink::NDEFError::New(device::mojom::blink::NDEFErrorType::NOT_READABLE, ""));
    base::RunLoop loop;
    nfc_proxy->StartReading(reader, base::BindLambdaForTesting([&](device::mojom::blink::NDEFErrorPtr error) {
        // We got the error prepared before.
        EXPECT_FALSE(error.is_null());
        EXPECT_EQ(error->error_type, device::mojom::blink::NDEFErrorType::NOT_READABLE);
        loop.Quit();
    }));
    EXPECT_TRUE(nfc_proxy->IsReading(reader));
    loop.Run();
