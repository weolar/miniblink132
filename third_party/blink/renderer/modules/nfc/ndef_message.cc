...
constexpr char kRecursionLimitExceededErrorMessage[] = "NDEFMessage recursion limit exceeded.";

} // namespace

// static
NDEFMessage* NDEFMessage::Create(
    const ScriptState* script_state, const NDEFMessageInit* init, ExceptionState& exception_state, uint8_t records_depth, bool is_embedded)
{
    // https://w3c.github.io/web-nfc/#creating-ndef-message
    ...
    if (init->records().empty()) {
        exception_state.ThrowTypeError("NDEFMessageInit#records being empty makes no sense.");
        return nullptr;
    }
    ...
    for (const NDEFRecordInit* record_init : init->records()) {
        NDEFRecord* record = NDEFRecord::Create(script_state, record_init, exception_state, records_depth, is_embedded);
...
// static
NDEFMessage* NDEFMessage::Create(const ScriptState* script_state, const V8NDEFMessageSource* source, ExceptionState& exception_state)
{
    DCHECK(source);
    ...
    case V8NDEFMessageSource::ContentType::kArrayBuffer: {
        ...
        if (buffer->ByteLength() > std::numeric_limits<wtf_size_t>::max()) {
            exception_state.ThrowRangeError("Buffer size exceeds maximum heap object size.");
            return nullptr;
        }
        Vector<uint8_t> payload_data;
        payload_data.AppendSpan(buffer->ByteSpan());
        NDEFMessage* message = MakeGarbageCollected<NDEFMessage>();
        message->records_.push_back(MakeGarbageCollected<NDEFRecord>(String() /* id */, "application/octet-stream", std::move(payload_data)));
        return message;
    }
    case V8NDEFMessageSource::ContentType::kArrayBufferView: {
        const DOMArrayBufferView* buffer_view = source->GetAsArrayBufferView().Get();
    ...
    case V8NDEFMessageSource::ContentType::kNDEFMessageInit: {
        return Create(script_state, source->GetAsNDEFMessageInit(), exception_state,
            /*records_depth=*/0U);
    }
    case V8NDEFMessageSource::ContentType::kString: {
        NDEFMessage* message = MakeGarbageCollected<NDEFMessage>();
        message->records_.push_back(MakeGarbageCollected<NDEFRecord>(script_state, source->GetAsString()));
        return message;
    }
    }
...
NDEFMessage* NDEFMessage::CreateAsPayloadOfSmartPoster(
    const ScriptState* script_state, const NDEFMessageInit* init, ExceptionState& exception_state, uint8_t records_depth)
{
    // NDEFMessageInit#records is a required field.
    DCHECK(init->hasRecords());
    ...
    for (const NDEFRecordInit* record_init : init->records()) {
        ...
        if (record_type == "url") {
            ...
            if (has_url_record) {
                exception_state.ThrowTypeError("'smart-poster' NDEFRecord contains more than one url record.");
                return nullptr;
            }
            has_url_record = true;
            ...
            if (has_size_record) {
                exception_state.ThrowTypeError("'smart-poster' NDEFRecord contains more than one size record.");
                return nullptr;
            }
            ...
            if (has_type_record) {
                exception_state.ThrowTypeError("'smart-poster' NDEFRecord contains more than one type record.");
                return nullptr;
            }
            ...
            if (has_action_record) {
                exception_state.ThrowTypeError("'smart-poster' NDEFRecord contains more than one action record.");
                return nullptr;
            }
        ...
        NDEFRecord* record = NDEFRecord::Create(script_state, record_init, exception_state, records_depth, /*is_embedded=*/true);
        if (exception_state.HadException())
            return nullptr;
        DCHECK(record);

        if (record->recordType() == ":s" && record->payloadData().size() != 4) {
            exception_state.ThrowTypeError("Size record of smart-poster must contain a 4-byte 32 bit unsigned "
                                           "integer.");
            return nullptr;
        }
        if (record->recordType() == ":act" && record->payloadData().size() != 1) {
            exception_state.ThrowTypeError("Action record of smart-poster must contain only a single byte.");
            return nullptr;
        }
    ...
    if (!has_url_record) {
        exception_state.ThrowTypeError("'smart-poster' NDEFRecord is missing the single mandatory url "
                                       "record.");
        return nullptr;
    }
