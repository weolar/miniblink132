...
// to crash.
v8::Local<v8::Value> GetPayloadDataOrUndefined(v8::Isolate* isolate, const NDEFRecordInit& record_init)
{
    return record_init.hasData() ? record_init.data().V8Value() : v8::Undefined(isolate).As<v8::Value>();
}
...
bool MaybeIsBufferSource(const ScriptValue& script_value)
{
    ...
    return v8_value->IsArrayBuffer() || v8_value->IsSharedArrayBuffer() || v8_value->IsArrayBufferView();
}

bool GetBytesOfBufferSource(const V8BufferSource* buffer_source, WTF::Vector<uint8_t>* target, ExceptionState& exception_state)
{
    ...
    if (!base::CheckedNumeric<wtf_size_t>(array_piece.ByteLength()).IsValid()) {
        exception_state.ThrowRangeError("The provided buffer source exceeds the maximum supported length");
        return false;
    }
...
bool IsValidExternalType(const String& input)
{
    ...
    for (wtf_size_t i = 0; i < type.length(); i++) {
        if (!IsASCIIAlphanumeric(type[i]) && !base::Contains(kOtherCharsForCustomType, type[i])) {
            return false;
        }
    }
...
String getDocumentLanguage(const ExecutionContext* execution_context)
{
    ...
    if (execution_context) {
        Element* document_element = To<LocalDOMWindow>(execution_context)->document()->documentElement();
        if (document_element) {
...
static NDEFRecord* CreateTextRecord(const ScriptState* script_state, const String& id, const NDEFRecordInit& record, ExceptionState& exception_state)
{
    // https://w3c.github.io/web-nfc/#mapping-string-to-ndef
    if (!record.hasData() || !(record.data().V8Value()->IsString() || MaybeIsBufferSource(record.data()))) {
        exception_state.ThrowTypeError("The data for 'text' NDEFRecords must be a String or a BufferSource.");
        return nullptr;
    }
    ...
    if (language.length() > 63) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Lang length cannot be stored in 6 bit.");
        return nullptr;
    }
    ...
    if (MaybeIsBufferSource(record.data())) {
        if (encoding_label != "utf-8" && encoding_label != "utf-16" && encoding_label != "utf-16be" && encoding_label != "utf-16le") {
            exception_state.ThrowTypeError("Encoding must be either \"utf-8\", \"utf-16\", \"utf-16be\", or "
                                           "\"utf-16le\".");
            return nullptr;
        }
        auto* buffer_source = NativeValueTraits<V8BufferSource>::NativeValue(script_state->GetIsolate(), record.data().V8Value(), exception_state);
        ...
        if (encoding_label != "utf-8") {
            exception_state.ThrowTypeError("A DOMString data source is always encoded as \"utf-8\" so other "
                                           "encodings are not allowed.");
            return nullptr;
        }
        const String data = NativeValueTraits<IDLString>::NativeValue(script_state->GetIsolate(), record.data().V8Value(), exception_state);
        if (exception_state.HadException())
            return nullptr;
    ...
    return MakeGarbageCollected<NDEFRecord>(id, encoding_label, language, std::move(bytes));
}

// Create a 'url' record or an 'absolute-url' record.
NDEFRecord* CreateUrlRecord(const ScriptState* script_state, const String& id, const NDEFRecordInit& record, ExceptionState& exception_state)
{
    ...
    if (!record.hasData() || !record.data().V8Value()->IsString()) {
        ...
        // SyntaxError below.
        exception_state.ThrowTypeError("The data for url NDEFRecord must be a String.");
        return nullptr;
    }
    auto* isolate = script_state->GetIsolate();
    const String& url = NativeValueTraits<IDLString>::NativeValue(isolate, GetPayloadDataOrUndefined(isolate, record), exception_state);
    if (exception_state.HadException())
        return nullptr;
    ...
    return MakeGarbageCollected<NDEFRecord>(device::mojom::blink::NDEFRecordTypeCategory::kStandardized, record.recordType(), id, GetUTF8DataFromString(url));
}

NDEFRecord* CreateMimeRecord(const ScriptState* script_state, const String& id, const NDEFRecordInit& record, ExceptionState& exception_state)
{
    // https://w3c.github.io/web-nfc/#mapping-binary-data-to-ndef
    auto* isolate = script_state->GetIsolate();
    auto* buffer_source = NativeValueTraits<V8BufferSource>::NativeValue(isolate, GetPayloadDataOrUndefined(isolate, record), exception_state);
    if (exception_state.HadException())
        return nullptr;
...
NDEFRecord* CreateUnknownRecord(const ScriptState* script_state, const String& id, const NDEFRecordInit& record, ExceptionState& exception_state)
{
    auto* isolate = script_state->GetIsolate();
    auto* buffer_source = NativeValueTraits<V8BufferSource>::NativeValue(isolate, GetPayloadDataOrUndefined(isolate, record), exception_state);
    if (exception_state.HadException())
    ...
    return MakeGarbageCollected<NDEFRecord>(device::mojom::blink::NDEFRecordTypeCategory::kStandardized, "unknown", id, bytes);
}

NDEFRecord* CreateSmartPosterRecord(
    const ScriptState* script_state, const String& id, const NDEFRecordInit& record, uint8_t records_depth, ExceptionState& exception_state)
{
    // https://w3c.github.io/web-nfc/#dfn-map-smart-poster-to-ndef
    auto* isolate = script_state->GetIsolate();
    auto* ndef_message_init = NativeValueTraits<NDEFMessageInit>::NativeValue(isolate, GetPayloadDataOrUndefined(isolate, record), exception_state);
    if (exception_state.HadException())
        return nullptr;

    NDEFMessage* payload_message = NDEFMessage::CreateAsPayloadOfSmartPoster(script_state, ndef_message_init, exception_state, records_depth);
    if (exception_state.HadException())
        return nullptr;
    DCHECK(payload_message);

    return MakeGarbageCollected<NDEFRecord>(device::mojom::blink::NDEFRecordTypeCategory::kStandardized, "smart-poster", id, payload_message);
}

NDEFRecord* CreateExternalRecord(
    const ScriptState* script_state, const String& id, const NDEFRecordInit& record, uint8_t records_depth, ExceptionState& exception_state)
{
    ...
    if (record.hasData()) {
        ...
        if (MaybeIsBufferSource(record.data())) {
            ...
            return MakeGarbageCollected<NDEFRecord>(device::mojom::blink::NDEFRecordTypeCategory::kExternal, record_type, id, bytes);
        } else if (record.data().IsObject()) {
            auto* ndef_message_init = NativeValueTraits<NDEFMessageInit>::NativeValue(script_state->GetIsolate(), record.data().V8Value(), exception_state);
            if (exception_state.HadException())
                return nullptr;

            NDEFMessage* payload_message = NDEFMessage::Create(script_state, ndef_message_init, exception_state,
                /*records_depth=*/records_depth, /*is_embedded=*/true);
            if (exception_state.HadException())
                return nullptr;
            DCHECK(payload_message);

            return MakeGarbageCollected<NDEFRecord>(device::mojom::blink::NDEFRecordTypeCategory::kExternal, record_type, id, payload_message);
        }
    }

    exception_state.ThrowTypeError("The data for external type NDEFRecord must be a BufferSource or an "
                                   "NDEFMessageInit.");
    return nullptr;
}

NDEFRecord* CreateLocalRecord(
    const ScriptState* script_state, const String& id, const NDEFRecordInit& record, uint8_t records_depth, ExceptionState& exception_state)
{
    ...
    if (record.hasData()) {
        ...
        if (MaybeIsBufferSource(record.data())) {
            ...
            return MakeGarbageCollected<NDEFRecord>(device::mojom::blink::NDEFRecordTypeCategory::kLocal, record_type, id, bytes);
        } else if (record.data().IsObject()) {
            auto* ndef_message_init = NativeValueTraits<NDEFMessageInit>::NativeValue(script_state->GetIsolate(), record.data().V8Value(), exception_state);
            if (exception_state.HadException())
                return nullptr;
            ...
            return MakeGarbageCollected<NDEFRecord>(device::mojom::blink::NDEFRecordTypeCategory::kLocal, record_type, id, payload_message);
        }
    }

    exception_state.ThrowTypeError("The data for local type NDEFRecord must be a BufferSource or an "
                                   "NDEFMessageInit.");
    return nullptr;
}
...
// static
NDEFRecord* NDEFRecord::CreateForBindings(const ScriptState* script_state, const NDEFRecordInit* record, ExceptionState& exception_state)
{
...
// static
NDEFRecord* NDEFRecord::Create(
    const ScriptState* script_state, const NDEFRecordInit* record, ExceptionState& exception_state, uint8_t records_depth, bool is_embedded)
{
    ...
    if (record->hasMediaType() && record_type != "mime") {
        exception_state.ThrowTypeError("NDEFRecordInit#mediaType is only applicable for 'mime' records.");
        return nullptr;
    }

    // https://w3c.github.io/web-nfc/#dfn-map-empty-record-to-ndef
    if (record->hasId() && record_type == "empty") {
        exception_state.ThrowTypeError("NDEFRecordInit#id is not applicable for 'empty' records.");
        return nullptr;
    }
    ...
    if (record_type == "empty") {
        ...
        return MakeGarbageCollected<NDEFRecord>(
            device::mojom::blink::NDEFRecordTypeCategory::kStandardized, record_type, /*id=*/String(), WTF::Vector<uint8_t>());
        ...
        return CreateSmartPosterRecord(script_state, id, *record, records_depth, exception_state);
    } else if (IsValidExternalType(record_type)) {
        return CreateExternalRecord(script_state, id, *record, records_depth, exception_state);
    } else if (IsValidLocalType(record_type)) {
        if (!is_embedded) {
            exception_state.ThrowTypeError("Local type records are only supposed to be embedded in the payload "
                                           "of another record (smart-poster, external, or local).");
            return nullptr;
        }
        return CreateLocalRecord(script_state, id, *record, records_depth, exception_state);
    }
...
NDEFRecord::NDEFRecord(device::mojom::blink::NDEFRecordTypeCategory category, const String& record_type, const String& id, WTF::Vector<uint8_t> data)
    : category_(category)
    , record_type_(record_type)
    , id_(id)
    , payload_data_(std::move(data))
{
    DCHECK_EQ(category_ == device::mojom::blink::NDEFRecordTypeCategory::kExternal, IsValidExternalType(record_type_));
    DCHECK_EQ(category_ == device::mojom::blink::NDEFRecordTypeCategory::kLocal, IsValidLocalType(record_type_));
}

NDEFRecord::NDEFRecord(device::mojom::blink::NDEFRecordTypeCategory category, const String& record_type, const String& id, NDEFMessage* payload_message)
    : category_(category)
    , record_type_(record_type)
    , id_(id)
    , payload_message_(payload_message)
{
    DCHECK(record_type_ == "smart-poster" || category_ == device::mojom::blink::NDEFRecordTypeCategory::kExternal
        || category_ == device::mojom::blink::NDEFRecordTypeCategory::kLocal);
    DCHECK_EQ(category_ == device::mojom::blink::NDEFRecordTypeCategory::kExternal, IsValidExternalType(record_type_));
    DCHECK_EQ(category_ == device::mojom::blink::NDEFRecordTypeCategory::kLocal, IsValidLocalType(record_type_));
}

NDEFRecord::NDEFRecord(const String& id, const String& encoding, const String& lang, WTF::Vector<uint8_t> data)
...
NDEFRecord::NDEFRecord(const String& id, const String& media_type, WTF::Vector<uint8_t> data)
    : category_(device::mojom::blink::NDEFRecordTypeCategory::kStandardized)
    , record_type_("mime")
    , id_(id)
...
NDEFRecord::NDEFRecord(const device::mojom::blink::NDEFRecord& record)
    ...
    , payload_message_(record.payload_message ? MakeGarbageCollected<NDEFMessage>(*record.payload_message) : nullptr)
{
...
std::optional<HeapVector<Member<NDEFRecord>>> NDEFRecord::toRecords(ExceptionState& exception_state) const
{
    if (record_type_ != "smart-poster" && category_ != device::mojom::blink::NDEFRecordTypeCategory::kExternal
        && category_ != device::mojom::blink::NDEFRecordTypeCategory::kLocal) {
