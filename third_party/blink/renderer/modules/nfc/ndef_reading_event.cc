...
// static
NDEFReadingEvent* NDEFReadingEvent::Create(
    const ScriptState* script_state, const AtomicString& event_type, const NDEFReadingEventInit* init, ExceptionState& exception_state)
{
    NDEFMessage* message = NDEFMessage::Create(script_state, init->message(), exception_state, /*records_depth=*/0U);
...
NDEFReadingEvent::NDEFReadingEvent(const AtomicString& event_type, const NDEFReadingEventInit* init, NDEFMessage* message)
...
NDEFReadingEvent::NDEFReadingEvent(const AtomicString& event_type, const String& serial_number, NDEFMessage* message)
