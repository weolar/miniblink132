...
class MODULES_EXPORT NDEFReader : public EventTarget, public ActiveScriptWrappable<NDEFReader>, public ExecutionContextLifecycleObserver {
...
public:
    ...
    ScriptPromise<IDLUndefined> scan(ScriptState* script_state, const NDEFScanOptions* options, ExceptionState& exception_state);
    ...
    ScriptPromise<IDLUndefined> makeReadOnly(ScriptState* script_state, const NDEFMakeReadOnlyOptions* options, ExceptionState& exception_state);

    void Trace(Visitor*) const override;

    // Called by NFCProxy for dispatching events.
    virtual void OnReading(const String& serial_number, const device::mojom::blink::NDEFMessage&);
...
private:
    ...
    void ReadOnRequestPermission(const NDEFScanOptions* options, mojom::blink::PermissionStatus status);

    // Write Permission handling
    void WriteOnRequestPermission(ScriptPromiseResolver<IDLUndefined>* resolver, std::unique_ptr<ScopedAbortState> scoped_abort_state,
        const NDEFWriteOptions* options, device::mojom::blink::NDEFMessagePtr ndef_message, mojom::blink::PermissionStatus status);

    // Make read-only permission handling
    void MakeReadOnlyOnRequestPermission(ScriptPromiseResolver<IDLUndefined>* resolver, std::unique_ptr<ScopedAbortState> scoped_abort_state,
    ...
    HeapHashSet<Member<ScriptPromiseResolver<IDLUndefined>>> make_read_only_requests_;
};
