...
public:
    ...
    MOCK_METHOD1(AddAvailabilityObserver, void(PresentationAvailabilityObserver*));
    MOCK_METHOD1(RemoveAvailabilityObserver, void(PresentationAvailabilityObserver*));
};
} // namespace

class RemotePlaybackTest : public testing::Test, private ScopedRemotePlaybackBackendForTest {
public:
    RemotePlaybackTest()
        : ScopedRemotePlaybackBackendForTest(true)
    {
    }
    ...
    void SetUp() override
    {
        ...
        element_ = MakeGarbageCollected<HTMLVideoElement>(page_holder_->GetDocument());
...
protected:
    ...
    void SetState(RemotePlayback& remote_playback, mojom::blink::PresentationConnectionState state)
    {
    ...
    {
        LocalFrame::NotifyUserActivation(&page_holder_->GetFrame(), mojom::UserActivationNotificationType::kTest);
    }

    void DisableRemotePlaybackAttr()
    {
        HTMLMediaElementRemotePlayback::SetBooleanAttribute(*element_, html_names::kDisableremoteplaybackAttr, true);
    }
...
TEST_F(RemotePlaybackTest, PromptCancelledRejectsWithNotAllowedError)
{
    ...
    ScriptPromiseTester promise_tester(scope.GetScriptState(), remote_playback.prompt(scope.GetScriptState(), scope.GetExceptionState()));
...
TEST_F(RemotePlaybackTest, PromptConnectedRejectsWhenCancelled)
{
    ...
    SetState(remote_playback, mojom::blink::PresentationConnectionState::CONNECTED);
...
TEST_F(RemotePlaybackTest, StateChangeEvents)
{
    ...
    auto* connecting_handler = MakeGarbageCollected<testing::StrictMock<MockEventListenerForRemotePlayback>>();
    auto* connect_handler = MakeGarbageCollected<testing::StrictMock<MockEventListenerForRemotePlayback>>();
    auto* disconnect_handler = MakeGarbageCollected<testing::StrictMock<MockEventListenerForRemotePlayback>>();

    remote_playback.addEventListener(event_type_names::kConnecting, connecting_handler);
    remote_playback.addEventListener(event_type_names::kConnect, connect_handler);
    remote_playback.addEventListener(event_type_names::kDisconnect, disconnect_handler);
    ...
    SetState(remote_playback, mojom::blink::PresentationConnectionState::CONNECTING);
    SetState(remote_playback, mojom::blink::PresentationConnectionState::CONNECTING);
    SetState(remote_playback, mojom::blink::PresentationConnectionState::CONNECTED);
    SetState(remote_playback, mojom::blink::PresentationConnectionState::CONNECTED);
    SetState(remote_playback, mojom::blink::PresentationConnectionState::CLOSED);
    ...
    SetState(remote_playback, mojom::blink::PresentationConnectionState::CONNECTING);
    SetState(remote_playback, mojom::blink::PresentationConnectionState::CONNECTED);
    SetState(remote_playback, mojom::blink::PresentationConnectionState::TERMINATED);
...
TEST_F(RemotePlaybackTest, DisableRemotePlaybackRejectsPromptWithInvalidStateError)
{
    V8TestingScope scope;
    RemotePlayback& remote_playback = get_remote_playback();
...
TEST_F(RemotePlaybackTest, DisableRemotePlaybackCancelsAvailabilityCallbacks)
{
    ...
    V8RemotePlaybackAvailabilityCallback* availability_callback
        = V8RemotePlaybackAvailabilityCallback::Create(funcs.ExpectNoCall()->ToV8Function(scope.GetScriptState()));

    ScriptPromiseTester promise_tester(
        scope.GetScriptState(), remote_playback.watchAvailability(scope.GetScriptState(), availability_callback, scope.GetExceptionState()));
...
TEST_F(RemotePlaybackTest, CallingWatchAvailabilityFromAvailabilityCallback)
{
    ...
    V8RemotePlaybackAvailabilityCallback* availability_callback
        = V8RemotePlaybackAvailabilityCallback::Create(callback_function->ToV8Function(scope.GetScriptState()));

    const int kNumberCallbacks = 10;
    for (int i = 0; i < kNumberCallbacks; ++i) {
        remote_playback.watchAvailability(scope.GetScriptState(), availability_callback, scope.GetExceptionState());
    }
    ...
    EXPECT_CALL(*callback_function, Call(testing::_, testing::_)).Times(kNumberCallbacks).WillRepeatedly(testing::InvokeWithoutArgs(add_callback_lambda));
    ...
    EXPECT_CALL(*callback_function, Call(testing::_, testing::_)).Times(kNumberCallbacks * 2).WillRepeatedly(testing::InvokeWithoutArgs(add_callback_lambda));
...
TEST_F(RemotePlaybackTest, IsListening)
{
    ...
    MockPresentationController* mock_controller = MakeGarbageCollected<MockPresentationController>(window);
    Supplement<LocalDOMWindow>::ProvideTo(window, static_cast<PresentationController*>(mock_controller));

    EXPECT_CALL(*mock_controller, AddAvailabilityObserver(testing::Eq(&remote_playback))).Times(3);
    EXPECT_CALL(*mock_controller, RemoveAvailabilityObserver(testing::Eq(&remote_playback))).Times(3);
    ...
    remote_playback.cancelWatchAvailability(scope.GetScriptState(), scope.GetExceptionState());
    ASSERT_EQ((size_t)1, remote_playback.Urls().size());
    ASSERT_FALSE(IsListening(remote_playback));
...
TEST_F(RemotePlaybackTest, GetAvailabilityUrl)
{
    ...
    EXPECT_EQ("remote-playback:media-element?source=aHR0cDovL3d3dy5leGFtcGxlLmNvbS8=", remote_playback.Urls()[0]);

    remote_playback.MediaMetadataChanged(media::VideoCodec::kVP9, media::AudioCodec::kMP3);
    EXPECT_EQ("remote-playback:media-element?source=aHR0cDovL3d3dy5leGFtcGxlLmNvbS8=&"
