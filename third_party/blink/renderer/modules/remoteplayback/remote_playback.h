...
class MODULES_EXPORT RemotePlayback final : public EventTarget,
...
public:
    ...
    ScriptPromise<IDLLong> watchAvailability(ScriptState*, V8RemotePlaybackAvailabilityCallback*, ExceptionState&);

    // Cancels updating the page via the callback specified by its id.
    ScriptPromise<IDLUndefined> cancelWatchAvailability(ScriptState*, int id, ExceptionState&);

    // Cancels all the callbacks watching remote playback availability changes
    // registered with this element.
    ScriptPromise<IDLUndefined> cancelWatchAvailability(ScriptState*, ExceptionState&);
    ...
    void MediaMetadataChanged(std::optional<media::VideoCodec> video_codec, std::optional<media::AudioCodec> audio_codec) override;
...
private:
    ...
    HeapMojoReceiver<mojom::blink::PresentationConnection, RemotePlayback> presentation_connection_receiver_;
    HeapMojoRemote<mojom::blink::PresentationConnection> target_presentation_connection_;
