...
std::ostream& blink::mojom::operator<<(std::ostream& a, ::blink::mojom::ScreenAvailability b)
{
    *(int*)1 = 1;
    return a << (int)(b);
}
...
V8RemotePlaybackState::Enum RemotePlaybackStateToEnum(mojom::blink::PresentationConnectionState state)
{
    switch (state) {
...
void RunRemotePlaybackTask(ExecutionContext* context, base::OnceClosure task, std::unique_ptr<probe::AsyncTaskContext> task_context)
{
    probe::AsyncTask async_task(context, task_context.get());
    std::move(task).Run();
}

KURL GetAvailabilityUrl(
    const KURL& source, bool is_source_supported, std::optional<media::VideoCodec> video_codec, std::optional<media::AudioCodec> audio_codec)
{
    ...
    String encoded_source = WTF::Base64URLEncode(base::as_byte_span(source_string));

    std::string video_codec_str = video_codec.has_value() ? ("&video_codec=" + media::GetCodecName(video_codec.value())) : "";
    std::string audio_codec_str = audio_codec.has_value() ? ("&audio_codec=" + media::GetCodecName(audio_codec.value())) : "";
    return KURL(StringView(kRemotePlaybackPresentationUrlPath) + "?source=" + encoded_source + video_codec_str.c_str() + audio_codec_str.c_str());
}
...
{
    RemotePlayback* self = static_cast<RemotePlayback*>(RemotePlaybackController::From(element));
    if (!self) {
...
ScriptPromise<IDLLong> RemotePlayback::watchAvailability(
    ScriptState* script_state, V8RemotePlaybackAvailabilityCallback* callback, ExceptionState& exception_state)
{
    if (media_element_->FastHasAttribute(html_names::kDisableremoteplaybackAttr)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "disableRemotePlayback attribute is present.");
        return EmptyPromise();
    }

    int id = WatchAvailabilityInternal(MakeGarbageCollected<AvailabilityCallbackWrapper>(callback));
    if (id == kWatchAvailabilityNotSupported) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Availability monitoring is not supported on this device.");
        return EmptyPromise();
    }
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLLong>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();
    resolver->Resolve(id);
    return promise;
}

ScriptPromise<IDLUndefined> RemotePlayback::cancelWatchAvailability(ScriptState* script_state, int id, ExceptionState& exception_state)
{
    if (media_element_->FastHasAttribute(html_names::kDisableremoteplaybackAttr)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "disableRemotePlayback attribute is present.");
        return EmptyPromise();
    ...
    if (!CancelWatchAvailabilityInternal(id)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotFoundError, "A callback with the given id is not found.");
        return EmptyPromise();
    }
...
ScriptPromise<IDLUndefined> RemotePlayback::cancelWatchAvailability(ScriptState* script_state, ExceptionState& exception_state)
{
    if (media_element_->FastHasAttribute(html_names::kDisableremoteplaybackAttr)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "disableRemotePlayback attribute is present.");
        return EmptyPromise();
...
ScriptPromise<IDLUndefined> RemotePlayback::prompt(ScriptState* script_state, ExceptionState& exception_state)
{
    if (media_element_->FastHasAttribute(html_names::kDisableremoteplaybackAttr)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "disableRemotePlayback attribute is present.");
        return EmptyPromise();
    ...
    if (prompt_promise_resolver_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "A prompt is already being shown for this media element.");
        return EmptyPromise();
    }

    if (!media_element_->DomWindow()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "RemotePlayback::prompt() does not work in a detached window.");
        return EmptyPromise();
    }

    if (!LocalFrame::HasTransientUserActivation(media_element_->DomWindow()->GetFrame())) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "RemotePlayback::prompt() requires user gesture.");
        return EmptyPromise();
    }

    if (!RuntimeEnabledFeatures::RemotePlaybackBackendEnabled()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "The RemotePlayback API is disabled on this platform.");
        return EmptyPromise();
    }

    if (availability_ == mojom::ScreenAvailability::UNAVAILABLE) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotFoundError, "No remote playback devices found.");
        return EmptyPromise();
    }

    if (availability_ == mojom::ScreenAvailability::SOURCE_NOT_SUPPORTED) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "The currentSrc is not compatible with remote playback");
        return EmptyPromise();
    }
    ...
    PromptInternal();
    RemotePlaybackMetrics::RecordRemotePlaybackLocation(RemotePlaybackInitiationLocation::kRemovePlaybackAPI);
    return promise;
}
...
{
    return HasEventListeners() || !availability_callbacks_.empty() || prompt_promise_resolver_;
}
...
void RemotePlayback::PromptInternal()
{
    ...
    PresentationController* controller = PresentationController::FromContext(GetExecutionContext());
    if (controller && !availability_urls_.empty()) {
        controller->GetPresentationService()->StartPresentation(
            availability_urls_, WTF::BindOnce(&RemotePlayback::HandlePresentationResponse, WrapPersistent(this)));
    } else {
        ...
        // task id.
        base::OnceClosure task = WTF::BindOnce(&RemotePlayback::PromptCancelled, WrapPersistent(this));

        std::unique_ptr<probe::AsyncTaskContext> task_context = std::make_unique<probe::AsyncTaskContext>();
        task_context->Schedule(GetExecutionContext(), "promptCancelled");
        GetExecutionContext()
            ->GetTaskRunner(TaskType::kMediaElementEvent)
            ->PostTask(FROM_HERE, WTF::BindOnce(RunRemotePlaybackTask, WrapPersistent(GetExecutionContext()), std::move(task), std::move(task_context)));
    }
}

int RemotePlayback::WatchAvailabilityInternal(AvailabilityCallbackWrapper* callback)
{
    if (RuntimeEnabledFeatures::RemotePlaybackBackendEnabled() && IsBackgroundAvailabilityMonitoringDisabled()) {
    ...
    base::OnceClosure task = WTF::BindOnce(&RemotePlayback::NotifyInitialAvailability, WrapPersistent(this), id);
    std::unique_ptr<probe::AsyncTaskContext> task_context = std::make_unique<probe::AsyncTaskContext>();
...
void RemotePlayback::StateChanged(mojom::blink::PresentationConnectionState state)
{
    if (prompt_promise_resolver_
        && IsInParallelAlgorithmRunnable(prompt_promise_resolver_->GetExecutionContext(), prompt_promise_resolver_->GetScriptState())) {
        ...
        // prompt() succeeded.
        ScriptState::Scope script_state_scope(prompt_promise_resolver_->GetScriptState());

        if (state_ != mojom::blink::PresentationConnectionState::CONNECTED && state == mojom::blink::PresentationConnectionState::CLOSED) {
            prompt_promise_resolver_->Reject(V8ThrowDOMException::CreateOrDie(
                prompt_promise_resolver_->GetScriptState()->GetIsolate(), DOMExceptionCode::kAbortError, "Failed to connect to the remote device."));
        } else {
    ...
    } else if (state_ == mojom::blink::PresentationConnectionState::CLOSED || state_ == mojom::blink::PresentationConnectionState::TERMINATED) {
        DispatchEvent(*Event::Create(event_type_names::kDisconnect));
        if (auto* video_element = DynamicTo<HTMLVideoElement>(media_element_.Get())) {
...
void RemotePlayback::PromptCancelled()
{
    ...
    prompt_promise_resolver_->Reject(V8ThrowDOMException::CreateOrDie(
        prompt_promise_resolver_->GetScriptState()->GetIsolate(), DOMExceptionCode::kNotAllowedError, "The prompt was dismissed."));
    prompt_promise_resolver_ = nullptr;
}

void RemotePlayback::SourceChanged(const KURL& source, bool is_source_supported)
{
    source_ = source;
...
{
    if (is_background_availability_monitoring_disabled_for_testing_ || IsBackgroundAvailabilityMonitoringDisabled()
    ...
    if (!media_element_ || std::isnan(media_element_->duration()) || media_element_->duration() <= media::remoting::kMinRemotingMediaDurationInSec) {
        StopListeningForAvailability();
        availability_urls_.clear();
        return;
    }

    KURL current_url = availability_urls_.empty() ? KURL() : availability_urls_[0];
    KURL new_url = GetAvailabilityUrl(source_, is_source_supported_, video_codec_, audio_codec_);
...
void RemotePlayback::MediaMetadataChanged(std::optional<media::VideoCodec> video_codec, std::optional<media::AudioCodec> audio_codec)
{
...
void RemotePlayback::AvailabilityChangedForTesting(bool screen_is_available)
{
    ...
    AvailabilityChanged(screen_is_available ? mojom::blink::ScreenAvailability::AVAILABLE : mojom::blink::ScreenAvailability::UNAVAILABLE);
}

void RemotePlayback::StateChangedForTesting(bool is_connected)
{
    StateChanged(is_connected ? mojom::blink::PresentationConnectionState::CONNECTED : mojom::blink::PresentationConnectionState::CLOSED);
}

bool RemotePlayback::RemotePlaybackAvailable() const
{
    if (IsBackgroundAvailabilityMonitoringDisabled() && RuntimeEnabledFeatures::RemotePlaybackBackendEnabled() && !media_element_->currentSrc().IsEmpty()) {
        return true;
    }
...
void RemotePlayback::RemotePlaybackDisabled()
{
    if (prompt_promise_resolver_) {
        prompt_promise_resolver_->Reject(V8ThrowDOMException::CreateOrDie(
            prompt_promise_resolver_->GetScriptState()->GetIsolate(), DOMExceptionCode::kInvalidStateError, "disableRemotePlayback attribute is present."));
        prompt_promise_resolver_ = nullptr;
    }
    ...
    if (state_ == mojom::blink::PresentationConnectionState::CLOSED || state_ == mojom::blink::PresentationConnectionState::TERMINATED) {
        return;
    }

    auto* controller = PresentationController::FromContext(GetExecutionContext());
    if (controller) {
        controller->GetPresentationService()->CloseConnection(presentation_url_, presentation_id_);
    }
}
...
{
    DCHECK(is_listening_ || is_background_availability_monitoring_disabled_for_testing_);
...
void RemotePlayback::OnConnectionSuccess(mojom::blink::PresentationConnectionResultPtr result)
{
    ...
    auto* presentation_controller = PresentationController::FromContext(GetExecutionContext());
    ...
    target_presentation_connection_.Bind(std::move(result->connection_remote), GetExecutionContext()->GetTaskRunner(TaskType::kMediaElementEvent));
    presentation_connection_receiver_.Bind(std::move(result->connection_receiver), GetExecutionContext()->GetTaskRunner(TaskType::kMediaElementEvent));
    RemotePlaybackMetrics::RecordRemotePlaybackStartSessionResult(GetExecutionContext(), true);
}

void RemotePlayback::OnConnectionError(const mojom::blink::PresentationError& error)
{
    // This is called when:
    ...
    if (error.error_type == mojom::blink::PresentationErrorType::PRESENTATION_REQUEST_CANCELLED) {
        PromptCancelled();
        return;
    }
    ...
    RemotePlaybackMetrics::RecordRemotePlaybackStartSessionResult(GetExecutionContext(), false);
}

void RemotePlayback::HandlePresentationResponse(mojom::blink::PresentationConnectionResultPtr result, mojom::blink::PresentationErrorPtr error)
{
    if (result) {
...
{
    if (IsBackgroundAvailabilityMonitoringDisabled() || is_background_availability_monitoring_disabled_for_testing_) {
        return;
    }
