...
void XRRigidTransform::DecomposeMatrix()
{
    ...
    CHECK(decomp, base::NotFatalUntil::M129) << "Matrix decompose failed for " << matrix_->ToString();

    position_ = DOMPointReadOnly::Create(decomp->translate[0], decomp->translate[1], decomp->translate[2], 1.0);

    orientation_ = makeNormalizedQuaternion(decomp->quaternion.x(), decomp->quaternion.y(), decomp->quaternion.z(), decomp->quaternion.w());
}

XRRigidTransform::XRRigidTransform(DOMPointInit* position, DOMPointInit* orientation)
{
    if (position) {
        position_ = DOMPointReadOnly::Create(position->x(), position->y(), position->z(), 1.0);
    } else {
        position_ = DOMPointReadOnly::Create(0.0, 0.0, 0.0, 1.0);
    }

    if (orientation) {
        orientation_ = makeNormalizedQuaternion(orientation->x(), orientation->y(), orientation->z(), orientation->w());
    } else {
...
XRRigidTransform* XRRigidTransform::Create(DOMPointInit* position, DOMPointInit* orientation, ExceptionState& exception_state)
{
    ...
    if ((position && !IsComponentValid(position)) || (orientation && !IsComponentValid(orientation))) {
        exception_state.ThrowTypeError("Position and Orientation must consist of only finite values");
        return nullptr;
    }
    ...
    if (orientation) {
        ...
        if (sq_len == 0.0) {
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Orientation's length cannot be 0");
            return nullptr;
            ...
            // quaternion.
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Orientation is too large to normalize");
            return nullptr;
        }
    }
...
void XRRigidTransform::EnsureMatrix()
{
    if (!matrix_) {
        ...
        decomp.quaternion = gfx::Quaternion(orientation_->x(), orientation_->y(), orientation_->z(), orientation_->w());
