...
HeapVector<Member<XRHitTestResult>> XRHitTestSource::Results()
{
    ...
    for (const auto& result : last_frame_results_) {
        results.emplace_back(MakeGarbageCollected<XRHitTestResult>(xr_session_, *result));
    }

    return results;
}

void XRHitTestSource::Update(const Vector<device::mojom::blink::XRHitResultPtr>& hit_test_results)
{
    last_frame_results_.clear();

    for (auto& result : hit_test_results) {
        DVLOG(3) << __func__ << ": processing hit test result, position=" << result->mojo_from_result.position().ToString()
                 << ", orientation=" << result->mojo_from_result.orientation().ToString() << ", plane_id=" << result->plane_id;
