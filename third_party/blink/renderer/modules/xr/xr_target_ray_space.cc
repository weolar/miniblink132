...
{
    auto mojo_from_viewer = session()->GetMojoFrom(device::mojom::blink::XRReferenceSpaceType::kViewer);
    ...
    case device::mojom::XRTargetRayMode::POINTING: {
        ...
        if (!(input_source_->MojoFromInput() && input_source_->InputFromPointer()))
            return std::nullopt;

        return *(input_source_->MojoFromInput()) * *(input_source_->InputFromPointer());
    }
    }
}
...
device::mojom::blink::XRNativeOriginInformationPtr XRTargetRaySpace::NativeOrigin() const
{
    return device::mojom::blink::XRNativeOriginInformation::NewInputSourceSpaceInfo(
        device::mojom::blink::XRInputSourceSpaceInfo::New(input_source_->source_id(), device::mojom::blink::XRInputSourceSpaceType::kTargetRay));
}
