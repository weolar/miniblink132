...
XRGPUTextureArraySwapChain::XRGPUTextureArraySwapChain(GPUDevice* device, XRGPUSwapChain* wrapped_swap_chain, uint32_t layers)
    ...
    , wrapped_swap_chain_(wrapped_swap_chain)
{
    ...
    descriptor_.size = { descriptor_.size.width / layers, descriptor_.size.height, layers };

    texture_internal_usage_ = { {
        .internalUsage = wgpu::TextureUsage::RenderAttachment | wgpu::TextureUsage::CopySrc,
    } };
