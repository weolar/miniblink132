...
XRFrameProvider::XRFrameProvider(XRSystem* xr)
    : xr_(xr)
    , frame_transport_(MakeGarbageCollected<XRFrameTransport>(xr->GetExecutionContext(), xr->GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI)))
...
void XRFrameProvider::OnSessionStarted(XRSession* session, device::mojom::blink::XRSessionPtr session_ptr)
{
    DCHECK(session);
    ...
    if (session->immersive()) {
        ...
        immersive_data_provider_.Bind(std::move(session_ptr->data_provider), xr_->GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI));
        immersive_data_provider_.set_disconnect_handler(
            WTF::BindOnce(&XRFrameProvider::OnProviderConnectionError, WrapWeakPersistent(this), WrapWeakPersistent(session)));

        immersive_presentation_provider_.Bind(
            std::move(session_ptr->submit_frame_sink->provider), xr_->GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI));
        immersive_presentation_provider_.set_disconnect_handler(
            WTF::BindOnce(&XRFrameProvider::OnProviderConnectionError, WrapWeakPersistent(this), WrapWeakPersistent(session)));

        frame_transport_->RegisterFrameRenderedCallback(WTF::BindRepeating(&XRFrameProvider::OnRenderComplete, WrapWeakPersistent(this)));

        frame_transport_->BindSubmitFrameClient(std::move(session_ptr->submit_frame_sink->client_receiver));
        frame_transport_->SetTransportOptions(std::move(session_ptr->submit_frame_sink->transport_options));
        frame_transport_->PresentChange();

        last_frame_statistics_sent_time_ = base::TimeTicks::Now();

        repeating_timer_.Start(FROM_HERE, base::Seconds(1), WTF::BindRepeating(&XRFrameProvider::SendFrameData, WrapWeakPersistent(this)));
    } else {
        ...
        HeapMojoRemote<device::mojom::blink::XRFrameDataProvider> data_provider(xr_->GetExecutionContext());
        data_provider.Bind(std::move(session_ptr->data_provider), xr_->GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI));
        data_provider.set_disconnect_handler(WTF::BindOnce(&XRFrameProvider::OnProviderConnectionError, WrapWeakPersistent(this), WrapWeakPersistent(session)));

        non_immersive_data_providers_.insert(session, WrapDisallowNew(std::move(data_provider)));
    }
}
...
void XRFrameProvider::OnFocusChanged()
{
    ...
    if (focus && !last_has_focus_ && requesting_sessions_.size() > 0 && !immersive_session_) {
...
void XRFrameProvider::OnSessionEnded(XRSession* session)
{
    ...
    if (session->immersive()) {
        ...
        frame_transport_
            = MakeGarbageCollected<XRFrameTransport>(session->GetExecutionContext(), session->GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI));
...
void XRFrameProvider::ScheduleImmersiveFrame(device::mojom::blink::XRFrameDataRequestOptionsPtr options)
{
    TRACE_EVENT0("gpu", __FUNCTION__);
    ...
    immersive_data_provider_->GetFrameData(std::move(options), WTF::BindOnce(&XRFrameProvider::OnImmersiveFrameData, WrapWeakPersistent(this)));
}

void XRFrameProvider::ScheduleNonImmersiveFrame(device::mojom::blink::XRFrameDataRequestOptionsPtr options)
{
    DVLOG(3) << __FUNCTION__;
    TRACE_EVENT0("gpu", __FUNCTION__);

    DCHECK(!immersive_session_) << "Scheduling should be done via the exclusive session if present.";
    ...
    window->document()->RequestAnimationFrame(MakeGarbageCollected<XRFrameProviderRequestCallback>(this));
}

void XRFrameProvider::OnImmersiveFrameData(device::mojom::blink::XRFrameDataPtr data)
{
    ...
    if (data->buffer_shared_image.has_value()) {
        buffer_shared_image_ = gpu::ClientSharedImage::ImportUnowned(data->buffer_shared_image.value());
        buffer_sync_token_ = data->buffer_sync_token.value();
    }

    if (data->camera_image_buffer_shared_image.has_value()) {
        camera_image_shared_image_ = gpu::ClientSharedImage::ImportUnowned(data->camera_image_buffer_shared_image.value());
    ...
    window->GetTaskRunner(blink::TaskType::kInternalMedia)
        ->PostTask(FROM_HERE,
            WTF::BindOnce(&XRFrameProvider::ProcessScheduledFrame, WrapWeakPersistent(this), std::move(data), high_res_now_ms, ScheduledFrameType::kImmersive));
}
...
void XRFrameProvider::OnNonImmersiveVSync(double high_res_now_ms)
{
    ...
    window->GetTaskRunner(blink::TaskType::kInternalMedia)
        ->PostTask(
            FROM_HERE, WTF::BindOnce(&XRFrameProvider::ProcessScheduledFrame, WrapWeakPersistent(this), nullptr, high_res_now_ms, ScheduledFrameType::kInline));
}

void XRFrameProvider::OnNonImmersiveFrameData(XRSession* session, device::mojom::blink::XRFrameDataPtr frame_data)
{
    TRACE_EVENT0("gpu", __FUNCTION__);
    DVLOG(2) << __FUNCTION__;
    ...
    if (request == requesting_sessions_.end()) {
        DVLOG(3) << __FUNCTION__ << ": request corresponding to received frame data not found";
        if (!session->ended()) {
            DVLOG(2) << __FUNCTION__ << ": the session's frame data provider missed the vsync";
        }
...
void XRFrameProvider::RequestNonImmersiveFrameData(XRSession* session)
{
    ...
    if (provider == non_immersive_data_providers_.end()) {
        ...
        data_provider->GetFrameData(
            std::move(options), WTF::BindOnce(&XRFrameProvider::OnNonImmersiveFrameData, WrapWeakPersistent(this), WrapWeakPersistent(session)));
    }
}

void XRFrameProvider::ProcessScheduledFrame(device::mojom::blink::XRFrameDataPtr frame_data, double high_res_now_ms, ScheduledFrameType frame_type)
{
    DVLOG(2) << __FUNCTION__ << ": frame_id_=" << frame_id_ << ", high_res_now_ms=" << high_res_now_ms;

    TRACE_EVENT2("gpu", "XRFrameProvider::ProcessScheduledFrame", "frame", frame_id_, "timestamp", high_res_now_ms);
    ...
    if (immersive_session_) {
        if (frame_type != ScheduledFrameType::kImmersive) {
            DVLOG(1) << __func__ << " Attempted to process non-immersive scheduled frame as immersive";
            return;
        }
        ...
        if (frame_data && frame_data->mojo_from_viewer) {
            DVLOG(3) << __func__ << ": pose available, emulated_position=" << frame_data->mojo_from_viewer->emulated_position;
        ...
        immersive_session_->UpdatePresentationFrameState(high_res_now_ms, std::move(frame_data), frame_id_, emulated_position);
        ...
        window->GetTaskRunner(blink::TaskType::kInternalMedia)
            ->PostTask(FROM_HERE,
                WTF::BindOnce(&XRSession::OnFrame, WrapWeakPersistent(immersive_session_.Get()), high_res_now_ms, buffer_shared_image_, buffer_sync_token_,
                    camera_image_shared_image_, camera_image_sync_token_));
    } else {
        ...
        for (auto& request : processing_sessions) {
            ...
            session->UpdatePresentationFrameState(high_res_now_ms, std::move(request.value), frame_id_, true /* Non-immersive positions are always emulated */);
            ...
            window->GetTaskRunner(blink::TaskType::kInternalMedia)
                ->PostTask(FROM_HERE,
                    WTF::BindOnce(&XRFrameProvider::OnPreDispatchInlineFrame, WrapWeakPersistent(this), WrapWeakPersistent(session), high_res_now_ms));
        }
    }
}

void XRFrameProvider::OnPreDispatchInlineFrame(XRSession* session, double timestamp)
{
    ...
    session->OnFrame(timestamp, nullptr, gpu::SyncToken(), nullptr, gpu::SyncToken());
}

double XRFrameProvider::UpdateImmersiveFrameTime(LocalDOMWindow* window, const device::mojom::blink::XRFrameData& data)
{
    DVLOG(3) << __func__;
    ...
    base::TimeDelta current_frame_time_from_first_frame = data.time_delta - *first_immersive_frame_time_delta_;
    // renderer_t_N is then:
    base::TimeTicks current_frame_time = *first_immersive_frame_time_ + current_frame_time_from_first_frame;

    double high_res_now_ms = window->document()->Loader()->GetTiming().MonotonicTimeToZeroBasedDocumentTime(current_frame_time).InMillisecondsF();
...
void XRFrameProvider::SubmitWebGLLayer(XRWebGLLayer* layer, bool was_changed)
{
    ...
    if (!was_changed) {
        ...
        frame_transport_->FrameSubmitMissing(immersive_presentation_provider_.get(), webgl_context->ContextGL(), frame_id_);
        dropped_frames_++;
    ...
    if (frame_transport_->DrawingIntoSharedBuffer()) {
        ...
        bool succeeded = frame_transport_->FrameSubmit(immersive_presentation_provider_.get(), webgl_context->ContextGL(),
            webgl_context->SharedImageInterface(), webgl_context, std::move(image_ref), frame_id_);
    ...
    scoped_refptr<StaticBitmapImage> image_ref = layer->TransferToStaticBitmapImage();
    ...
    bool succeeded = frame_transport_->FrameSubmit(immersive_presentation_provider_.get(), webgl_context->ContextGL(), webgl_context->SharedImageInterface(),
...
void XRFrameProvider::UpdateWebGLLayerViewports(XRWebGLLayer* layer)
{
    ...
    XRViewport* left = layer->GetViewportForEye(device::mojom::blink::XREye::kLeft);
    XRViewport* right = layer->GetViewportForEye(device::mojom::blink::XREye::kRight);
    ...
    gfx::RectF left_coords = left ? gfx::RectF(static_cast<float>(left->x()) / width, static_cast<float>(height - (left->y() + left->height())) / height,
                                 static_cast<float>(left->width()) / width, static_cast<float>(left->height()) / height)
                                  : gfx::RectF();
    gfx::RectF right_coords = right ? gfx::RectF(static_cast<float>(right->x()) / width, static_cast<float>(height - (right->y() + right->height())) / height,
                                  static_cast<float>(right->width()) / width, static_cast<float>(right->height()) / height)
                                    : gfx::RectF();

    immersive_presentation_provider_->UpdateLayerBounds(frame_id_, left_coords, right_coords, gfx::Size(width, height));
}

void XRFrameProvider::SubmitWebGPULayer(XRGPUProjectionLayer* layer, bool was_queried)
{
    CHECK(layer);
    CHECK(immersive_session_);
    CHECK_EQ(layer->session(), immersive_session_);
    ...
    if (!was_queried) {
        // Just tell the device side that there was no submitted frame instead of
        // executing the implicit end-of-frame submit.
        frame_transport_->FrameSubmitMissingWebGPU(immersive_presentation_provider_.get(), device->GetDawnControlClient(), frame_id_);
        dropped_frames_++;
        return;
    }
    ...
    if (!frame_transport_->DrawingIntoSharedBuffer()) {
        NOTREACHED() << "WebXR/WebGPU bindings only supports the DRAW_INTO_TEXTURE_MAILBOX "
                     << "XRPresentationTransportMethod at this time.";
    }

    DVLOG(3) << __func__ << ": FrameSubmitWebGPU for SharedBuffer mode";
    bool succeeded
        = frame_transport_->FrameSubmitWebGPU(immersive_presentation_provider_.get(), device->GetDawnControlClient(), device->GetHandle(), frame_id_);
    succeeded ? num_frames_++ : dropped_frames_++;
    if (succeeded) {
        submit_frame_time_.StartTimer();
    }
...
void XRFrameProvider::UpdateWebGPULayerViewports(XRGPUProjectionLayer* layer)
{
    ...
    gfx::Rect left = webgpu_binding->GetViewportForEye(layer, device::mojom::blink::XREye::kLeft);
    gfx::Rect right = webgpu_binding->GetViewportForEye(layer, device::mojom::blink::XREye::kRight);
    ...
    // We may only have one eye view, i.e. in smartphone immersive AR mode.
    // Use all-zero bounds for unused views.
    gfx::RectF left_coords = gfx::RectF(static_cast<float>(left.x()) / width, static_cast<float>(height - (left.y() + left.height())) / height,
        static_cast<float>(left.width()) / width, static_cast<float>(left.height()) / height);
    gfx::RectF right_coords = layer->textureArrayLength() > 1
        ? gfx::RectF((static_cast<float>(right.x()) / width), static_cast<float>(height - (right.y() + right.height())) / height,
            (static_cast<float>(right.width()) / width), static_cast<float>(right.height()) / height)
        : gfx::RectF();
...
void XRFrameProvider::SendFrameData()
{
    ...
    device::mojom::blink::XrFrameStatisticsPtr xr_frame_stat = device::mojom::blink::XrFrameStatistics::New();
    ...
    xr_frame_stat->page_animation_frame_time = immersive_session()->TakeAnimationFrameTimerAverage();

    xr_frame_stat->submit_frame_time = submit_frame_time_.TakeAverageMicroseconds();

    if (xr_->GetWebXrInternalsRendererListener()) {
        xr_->GetWebXrInternalsRendererListener()->OnFrameData(std::move(xr_frame_stat));
    }
}
