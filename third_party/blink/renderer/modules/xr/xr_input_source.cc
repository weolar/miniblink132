...
namespace {
std::unique_ptr<gfx::Transform> TryGetTransform(const std::optional<gfx::Transform>& transform)
{
    if (transform) {
...
XRInputSource::InternalState::InternalState(uint32_t source_id, device::mojom::XRTargetRayMode target_ray_mode, base::TimeTicks base_timestamp)
    : source_id(source_id)
...
XRInputSource::InternalState::InternalState(const InternalState& other) = default;

XRInputSource::InternalState::~InternalState() = default;

// static
XRInputSource* XRInputSource::CreateOrUpdateFrom(XRInputSource* other, XRSession* session, const device::mojom::blink::XRInputSourceStatePtr& state)
{
    if (!state)
        return other;
    ...
    if (state->description) {
        ...
        if (updated_source->state_.is_visible) {
            updated_source->input_from_pointer_ = TryGetTransform(desc->input_from_pointer);
        }

        updated_source->profiles_ = MakeGarbageCollected<FrozenArray<IDLString>>(state->description->profiles);
    }
...
XRInputSource::XRInputSource(XRSession* session, uint32_t source_id, device::mojom::XRTargetRayMode target_ray_mode)
...
XRInputSource::XRInputSource(const XRInputSource& other)
    ...
    , target_ray_space_(MakeGarbageCollected<XRTargetRaySpace>(other.session_, this))
    , grip_space_(MakeGarbageCollected<XRGripSpace>(other.session_, this))
    , gamepad_(other.gamepad_)
    , hand_(other.hand_)
    , profiles_(MakeGarbageCollected<FrozenArray<IDLString>>(other.profiles_->AsVector()))
...
bool XRInputSource::InvalidatesSameObject(const device::mojom::blink::XRInputSourceStatePtr& state)
{
    ...
    if ((state->hand_tracking_data.get() && !hand_) || (!state->hand_tracking_data.get() && hand_)) {
        return true;
    }
...
void XRInputSource::SetInputFromPointer(const gfx::Transform* input_from_pointer)
{
    if (state_.is_visible) {
...
void XRInputSource::UpdateGamepad(const std::optional<device::Gamepad>& gamepad)
{
    if (gamepad) {
        if (!gamepad_) {
            gamepad_ = MakeGarbageCollected<Gamepad>(this, -1, state_.base_timestamp, base::TimeTicks::Now());
        }

        LocalDOMWindow* window = session_->xr()->DomWindow();
        bool cross_origin_isolated_capability = window ? window->CrossOriginIsolatedCapability() : false;
...
void XRInputSource::UpdateHand(const device::mojom::blink::XRHandTrackingData* hand_tracking_data)
{
...
void XRInputSource::OnSelectStart()
{
    ...
    XRInputSourceEvent* event = CreateInputSourceEvent(event_type_names::kSelectstart);
...
void XRInputSource::OnSelectEnd()
{
    ...
    XRInputSourceEvent* event = CreateInputSourceEvent(event_type_names::kSelectend);
    session_->DispatchEvent(*event);
...
void XRInputSource::OnSelect()
{
    ...
    if (!state_.selection_cancelled && !session_->ended()) {
        ...
        XRInputSourceEvent* event = CreateInputSourceEvent(event_type_names::kSelect);
        session_->DispatchEvent(*event);
...
void XRInputSource::OnSqueezeStart()
{
    ...
    XRInputSourceEvent* event = CreateInputSourceEvent(event_type_names::kSqueezestart);
    session_->DispatchEvent(*event);
...
void XRInputSource::OnSqueezeEnd()
{
    ...
    XRInputSourceEvent* event = CreateInputSourceEvent(event_type_names::kSqueezeend);
    session_->DispatchEvent(*event);
...
void XRInputSource::OnSqueeze()
{
    ...
    if (!state_.squeezing_cancelled && !session_->ended()) {
        ...
        XRInputSourceEvent* event = CreateInputSourceEvent(event_type_names::kSqueeze);
        session_->DispatchEvent(*event);
...
void XRInputSource::UpdateButtonStates(const device::mojom::blink::XRInputSourceStatePtr& new_state)
{
    if (!new_state)
        return;

    DVLOG(3) << __func__ << ": state_.is_visible=" << state_.is_visible << ", state_.xr_select_events_suppressed=" << state_.xr_select_events_suppressed
             << ", new_state->primary_input_clicked=" << new_state->primary_input_clicked;
...
void XRInputSource::ProcessOverlayHitTest(Element* overlay_element, const device::mojom::blink::XRInputSourceStatePtr& new_state)
{
    DVLOG(3) << __func__ << ": state_.xr_select_events_suppressed=" << state_.xr_select_events_suppressed;
    ...
    gfx::PointF point(new_state->overlay_pointer_position->x(), new_state->overlay_pointer_position->y());
    ...
    HitTestResult result = event_handling_util::HitTestResultInFrame(overlay_element->GetDocument().GetFrame(), HitTestLocation(point), hit_type);
    ...
    if (frame) {
        ...
        if (hit_document) {
            ...
            if (hit_frame->IsCrossOriginToOutermostMainFrame()) {
                ...
                DVLOG(3) << __func__ << ": input source overlaps with cross origin content, is_visible=" << state_.is_visible
                         << ", xr_select_events_suppressed=" << state_.xr_select_events_suppressed;
                return;
            }
        }
    }
    ...
    bool is_primary_press = new_state->primary_input_pressed || new_state->primary_input_clicked;
    if (!is_primary_press) {
    ...
    Event* event = MakeGarbageCollected<XRSessionEvent>(
        event_type_names::kBeforexrselect, session_, Event::Bubbles::kYes, Event::Cancelable::kYes, Event::ComposedMode::kComposed);
...
XRInputSourceEvent* XRInputSource::CreateInputSourceEvent(const AtomicString& type)
{
