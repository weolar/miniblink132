...
public:
    explicit XRHitTestResult(XRSession* session, const device::mojom::blink::XRHitResult& hit_result);

    XRPose* getPose(XRSpace* relative_to, ExceptionState& exception_state);

    ScriptPromise<XRAnchor> createAnchor(ScriptState* script_state, ExceptionState& exception_state);
