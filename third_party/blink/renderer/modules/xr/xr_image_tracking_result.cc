...
XRImageTrackingResult::XRImageTrackingResult(XRSession* session, const device::mojom::blink::XRTrackedImageData& result)
    : session_(session)
    , index_(result.index)
    ...
    , width_in_meters_(result.width_in_meters)
{
    ...
    if (result.actively_tracked) {
        tracking_state_ = V8XRImageTrackingState(V8XRImageTrackingState::Enum::kTracked);
    } else {
        tracking_state_ = V8XRImageTrackingState(V8XRImageTrackingState::Enum::kEmulated);
    }
}
...
{
    if (!image_space_) {
        image_space_ = MakeGarbageCollected<XRObjectSpace<XRImageTrackingResult>>(session_, this);
    }

    return image_space_.Get();
}

device::mojom::blink::XRNativeOriginInformationPtr XRImageTrackingResult::NativeOrigin() const
{
