...
{
    if (!probe_space_) {
        probe_space_ = MakeGarbageCollected<XRObjectSpace<XRLightProbe>>(session_, this);
    }

    return probe_space_.Get();
}

device::mojom::blink::XRNativeOriginInformationPtr XRLightProbe::NativeOrigin() const
{
    return device::mojom::blink::XRNativeOriginInformation::NewReferenceSpaceType(device::mojom::XRReferenceSpaceType::kLocal);
}
...
void XRLightProbe::ProcessLightEstimationData(const device::mojom::blink::XRLightEstimationData* data, double timestamp)
{
    ...
    if (data) {
        ...
        if (data->reflection_probe) {
            ...
            const device::mojom::blink::XRReflectionProbe& reflection_probe = *data->reflection_probe;
    ...
    if (reflection_changed || (cube_map_ && timestamp > last_reflection_change_ + kReflectionChangeDelta)) {
