...
XRHitTestResult::XRHitTestResult(XRSession* session, const device::mojom::blink::XRHitResult& hit_result)
    : session_(session)
    , mojo_from_this_(hit_result.mojo_from_result)
    , plane_id_(hit_result.plane_id != 0 ? std::optional<uint64_t>(hit_result.plane_id) : std::nullopt)
{
}

XRPose* XRHitTestResult::getPose(XRSpace* other, ExceptionState& exception_state)
{
    ...
    auto other_offset_from_mojo = other_offset_from_other_native * other_native_from_mojo;
...
ScriptPromise<XRAnchor> XRHitTestResult::createAnchor(ScriptState* script_state, ExceptionState& exception_state)
{
    DVLOG(2) << __func__;
    ...
    auto space_from_mojo = reference_space_information->mojo_from_space.GetCheckedInverse();
    auto space_from_anchor = space_from_mojo * mojo_from_this_.ToTransform();

    return session_->CreateAnchorHelper(script_state, space_from_anchor, reference_space_information->native_origin, plane_id_, exception_state);
}
