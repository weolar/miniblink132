...
class XRHand : public ScriptWrappable, public PairSyncIterable<XRHand> {
    ...
    static constexpr unsigned kNumJoints = static_cast<unsigned>(device::mojom::blink::XRHandJoint::kMaxValue) + 1u;

public:
    explicit XRHand(const device::mojom::blink::XRHandTrackingData* state, XRInputSource* input_source);
    ...
    void updateFromHandTrackingData(const device::mojom::blink::XRHandTrackingData* state, XRInputSource* input_source);

    bool hasMissingPoses() const
    {
        return has_missing_poses_;
    }
