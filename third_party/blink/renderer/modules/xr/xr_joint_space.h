...
public:
    XRJointSpace(XRHand* hand, XRSession* session, std::unique_ptr<gfx::Transform> mojo_from_joint, device::mojom::blink::XRHandJoint joint, float radius,
    ...
    device::mojom::blink::XRHandJoint joint() const
    {
        return joint_;
    }
    V8XRHandJoint jointName() const;
    device::mojom::XRHandedness handedness() const
    {
        return handedness_;
    }

    std::optional<gfx::Transform> MojoFromNative() const override;
    device::mojom::blink::XRNativeOriginInformationPtr NativeOrigin() const override;
    bool EmulatedPosition() const override;
    XRPose* getPose(const XRSpace* other_space) const override;

    void UpdateTracking(std::unique_ptr<gfx::Transform> mojo_from_joint, float radius);
