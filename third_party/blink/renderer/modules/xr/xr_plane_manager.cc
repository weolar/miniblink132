...
XRPlaneManager::XRPlaneManager(base::PassKey<XRSession> pass_key, XRSession* session)
    : session_(session)
{
}

void XRPlaneManager::ProcessPlaneInformation(const device::mojom::blink::XRPlaneDetectionData* detected_planes_data, double timestamp)
{
    ...
    TRACE_COUNTER2("xr", "Plane statistics", "All planes", detected_planes_data->all_planes_ids.size(), "Updated planes",
        detected_planes_data->updated_planes_data.size());

    DVLOG(3) << __func__ << ": updated planes size=" << detected_planes_data->updated_planes_data.size()
             << ", all planes size=" << detected_planes_data->all_planes_ids.size();
    ...
    for (const auto& plane : detected_planes_data->updated_planes_data) {
        ...
        if (it != plane_ids_to_planes_.end()) {
            ...
            updated_planes.insert(plane->id, MakeGarbageCollected<XRPlane>(plane->id, session_, *plane, timestamp));
        }
    }
...
{
    if (!session_->IsFeatureEnabled(device::mojom::XRSessionFeature::PLANE_DETECTION)) {
