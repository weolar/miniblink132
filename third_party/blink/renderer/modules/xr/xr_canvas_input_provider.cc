...
XRCanvasInputProvider::XRCanvasInputProvider(XRSession* session, HTMLCanvasElement* canvas)
    : session_(session)
    , canvas_(canvas)
{
...
void XRCanvasInputProvider::UpdateInputSource(PointerEvent* event)
{
    ...
    if (!input_source_) {
        ...
        input_source_ = MakeGarbageCollected<XRInputSource>(session_, 1, device::mojom::XRTargetRayMode::TAPPING);
    ...
    gfx::Transform viewer_from_pointer = view->UnprojectPointer(element_x, element_y, canvas_->OffsetWidth(), canvas_->OffsetHeight());
