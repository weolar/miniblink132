...
{
    float cs = std::clamp(cl < 0.0031308f ? 12.92f * cl : 1.055f * std::pow(cl, 0.41666f) - 0.055f, 0.0f, 1.0f);
    return static_cast<uint8_t>(255.0f * cs + 0.5f);
}

void Rgba16fToSrgba8(base::span<const device::RgbaTupleF16> input, base::span<uint8_t> output)
{
...
{
    constexpr auto kNumComponentsPerPixel = device::mojom::blink::XRCubeMap::kNumComponentsPerPixel;
    static_assert(kNumComponentsPerPixel == 4, "XRCubeMaps are expected to be in the RGBA16F format");

    // Cube map sides must all be a power-of-two image
    bool valid = std::has_single_bit(cube_map.width_and_height);
    const size_t expected_size = cube_map.width_and_height * cube_map.width_and_height;
...
WebGLTexture* XRCubeMap::updateWebGLEnvironmentCube(
    WebGLRenderingContextBase* context, WebGLTexture* texture, GLenum internal_format, GLenum format, GLenum type) const
{
    // Ensure a texture was supplied from the passed context and with an
    // appropriate bound target.
    DCHECK(texture);
    DCHECK(!texture->HasEverBeenBound() || texture->GetTarget() == GL_TEXTURE_CUBE_MAP);
    ...
    if (type == GL_UNSIGNED_BYTE) {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            gl->TexImage2D(target, 0, internal_format, width_and_height_, width_and_height_, 0, format, type, sRGB.data());
        }
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            gl->TexImage2D(target, 0, internal_format, width_and_height_, width_and_height_, 0, format, type, image.data());
        }
    } else {
