...
void XrEnterFullscreenObserver::Invoke(ExecutionContext* execution_context, Event* event)
{
...
void XrEnterFullscreenObserver::RequestFullscreen(
    Element* fullscreen_element, bool setup_for_dom_overlay, bool may_have_camera_access, base::OnceCallback<void(bool)> on_completed)
{
    DCHECK(!on_completed_);
    ...
    DCHECK(!Fullscreen::FullscreenElementFrom(fullscreen_element_->GetDocument()));

    // Set up event listeners for success and failure.
    fullscreen_element_->GetDocument().addEventListener(event_type_names::kFullscreenchange, this, true);
    fullscreen_element_->GetDocument().addEventListener(event_type_names::kFullscreenerror, this, true);
    ...
    // prompt.
    ScopedAllowFullscreen scope(setup_for_dom_overlay ? ScopedAllowFullscreen::kXrOverlay : ScopedAllowFullscreen::kXrSession);

    FullscreenRequestType request_type = may_have_camera_access ? FullscreenRequestType::kForXrArWithCamera : FullscreenRequestType::kNull;
