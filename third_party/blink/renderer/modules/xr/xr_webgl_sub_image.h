...
public:
    explicit XRWebGLSubImage(const gfx::Rect& viewport)
        : XRSubImage(viewport)
    {
    }
    ...
    std::optional<uint16_t> imageIndex() const
    {
        return image_index_;
    }
    uint16_t colorTextureWidth() const
    {
        return color_texture_width_;
    }
    uint16_t colorTextureHeight() const
    {
        return color_texture_height_;
    }
