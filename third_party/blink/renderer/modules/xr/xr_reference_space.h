...
public:
    static device::mojom::blink::XRReferenceSpaceType V8EnumToReferenceSpaceType(V8XRReferenceSpaceType::Enum reference_space_type);

    XRReferenceSpace(XRSession* session, device::mojom::blink::XRReferenceSpaceType type);
    XRReferenceSpace(XRSession* session, XRRigidTransform* origin_offset, device::mojom::blink::XRReferenceSpaceType type);
    ~XRReferenceSpace() override;

    std::optional<gfx::Transform> NativeFromViewer(const std::optional<gfx::Transform>& mojo_from_viewer) const override;
...
private:
    virtual XRReferenceSpace* cloneWithOriginOffset(XRRigidTransform* origin_offset) const;
