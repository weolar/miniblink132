...
public:
    explicit XRCPUDepthInformation(const XRFrame* xr_frame, const gfx::Size& size, const gfx::Transform& norm_texture_from_norm_view, float raw_value_to_meters,
        device::mojom::XRDepthDataFormat data_format, DOMArrayBuffer* data);

    DOMArrayBuffer* data(ExceptionState& exception_state) const;

    float getDepthInMeters(float x, float y, ExceptionState& exception_state) const;
