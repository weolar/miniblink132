...
public:
    ...
    ScriptPromise<IDLUndefined> supportsSession(ScriptState*, const V8XRSessionMode&, ExceptionState& exception_state);
    ScriptPromise<IDLBoolean> isSessionSupported(ScriptState*, const V8XRSessionMode&, ExceptionState& exception_state);
    ScriptPromise<XRSession> requestSession(ScriptState*, const V8XRSessionMode&, XRSessionInit*, ExceptionState& exception_state);

    XRFrameProvider* frameProvider();

    device::mojom::blink::XREnvironmentIntegrationProvider* xrEnvironmentProviderRemote();
    ...
    void AddEnvironmentProviderErrorHandler(EnvironmentProviderErrorCallback callback);
    ...
    {
        return is_context_destroyed_;
    }

    void MakeXrCompatibleAsync(device::mojom::blink::VRService::MakeXrCompatibleCallback callback);
    void MakeXrCompatibleSync(device::mojom::XrCompatibleResult* xr_compatible_result);

    void OnSessionEnded(XRSession* session);

    device::mojom::blink::WebXrInternalsRendererListener* GetWebXrInternalsRendererListener();
...
private:
    ...
    class PendingRequestSessionQuery final : public GarbageCollected<PendingRequestSessionQuery> {
    public:
        PendingRequestSessionQuery(int64_t ukm_source_id, ScriptPromiseResolver<XRSession>* resolver, device::mojom::blink::XRSessionMode mode,
            RequestedXRSessionFeatureSet required_features, RequestedXRSessionFeatureSet optional_features);

        PendingRequestSessionQuery(const PendingRequestSessionQuery&) = delete;
        PendingRequestSessionQuery& operator=(const PendingRequestSessionQuery&) = delete;
        ...
        void Resolve(XRSession* session, mojo::PendingRemote<device::mojom::blink::XRSessionMetricsRecorder> metrics_recorder = mojo::NullRemote());
        ...
        void RejectWithDOMException(DOMExceptionCode exception_code, const String& message, ExceptionState* exception_state);
        ...
        void RejectWithSecurityError(const String& message, ExceptionState* exception_state);
        ...
        // |message| - it will be accessible to the application and should not
        // contain any sensitive data.
        void RejectWithTypeError(const String& message, ExceptionState* exception_state);
        ...
        Element* DOMOverlayElement()
        {
            return dom_overlay_element_.Get();
        }

        void SetTrackedImages(const Vector<device::mojom::blink::XRTrackedImage>& images)
        {
        ...
        void SetDepthSensingConfiguration(
            const Vector<device::mojom::XRDepthUsage>& preferred_usage, const Vector<device::mojom::XRDepthDataFormat>& preferred_format)
        {
    ...
    private:
        ...
        device::mojom::XRSessionFeatureRequestStatus GetFeatureRequestStatus(device::mojom::XRSessionFeature feature, const XRSession* session) const;
        void ReportRequestSessionResult(SessionRequestStatus status, XRSession* session = nullptr,
            mojo::PendingRemote<device::mojom::blink::XRSessionMetricsRecorder> metrics_recorder = mojo::NullRemote());
    ...
    static device::mojom::blink::XRSessionOptionsPtr XRSessionOptionsFromQuery(const PendingRequestSessionQuery& query);
    ...
    public:
        PendingSupportsSessionQuery(ScriptPromiseResolverBase*, device::mojom::blink::XRSessionMode, bool throw_on_unsupported);

        PendingSupportsSessionQuery(const PendingSupportsSessionQuery&) = delete;
        PendingSupportsSessionQuery& operator=(const PendingSupportsSessionQuery&) = delete;
        ...
        bool ThrowOnUnsupported() const
        {
            return throw_on_unsupported_;
        }
    ...
    void AddConsoleMessage(mojom::blink::ConsoleMessageLevel error_level, const String& message);

    void InternalIsSessionSupported(ScriptPromiseResolverBase*, const V8XRSessionMode&, ExceptionState& exception_state, bool throw_on_unsupported);

    const char* CheckInlineSessionRequestAllowed(LocalFrame* frame, const PendingRequestSessionQuery& query);

    RequestedXRSessionFeatureSet ParseRequestedFeatures(const Vector<String>& features, const device::mojom::blink::XRSessionMode& session_mode,
        XRSessionInit* session_init, mojom::ConsoleMessageLevel error_level);

    void RequestSessionInternal(device::mojom::blink::XRSessionMode session_mode, PendingRequestSessionQuery* query, ExceptionState* exception_state);

    void RequestImmersiveSession(PendingRequestSessionQuery* query, ExceptionState* exception_state);

    void RequestInlineSession(PendingRequestSessionQuery* query, ExceptionState* exception_state);

    void DoRequestSession(PendingRequestSessionQuery* query, device::mojom::blink::XRSessionOptionsPtr session_options);
    void OnRequestSessionReturned(PendingRequestSessionQuery*, device::mojom::blink::RequestSessionResultPtr result);
    void OnFullscreenConfigured(PendingRequestSessionQuery* query, device::mojom::blink::RequestSessionResultPtr result, bool fullscreen_succeeded);
    void FinishSessionCreation(PendingRequestSessionQuery*, device::mojom::blink::RequestSessionResultPtr result);
    void OnSupportsSessionReturned(PendingSupportsSessionQuery*, bool supports_session);
    void ResolveSessionRequest(PendingRequestSessionQuery*, device::mojom::blink::RequestSessionResultPtr result);
    ...
    XRSession* CreateSession(device::mojom::blink::XRSessionMode mode, device::mojom::blink::XREnvironmentBlendMode blend_mode,
        ...
        device::mojom::blink::XRSessionDeviceConfigPtr device_config, XRSessionFeatureSet enabled_features, uint64_t trace_id, bool sensorless_session = false);
    ...
    Vector<EnvironmentProviderErrorCallback> environment_provider_error_callbacks_;

    Member<XRFrameProvider> frame_provider_;
    HeapHashSet<WeakMember<XRSession>> sessions_;
    HeapMojoRemote<device::mojom::blink::VRService> service_;
    HeapMojoAssociatedRemote<device::mojom::blink::XREnvironmentIntegrationProvider> environment_provider_;
    ...
    FrameOrWorkerScheduler::SchedulingAffectingFeatureHandle feature_handle_for_scheduler_;
    ...
    HeapMojoRemote<device::mojom::blink::WebXrInternalsRendererListener> webxr_internals_renderer_listener_;
};
