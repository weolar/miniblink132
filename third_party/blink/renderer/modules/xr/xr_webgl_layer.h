...
public:
    XRWebGLLayer(
        XRSession*, WebGLRenderingContextBase*, scoped_refptr<XRWebGLDrawingBuffer>, WebGLFramebuffer*, double framebuffer_scale, bool ignore_depth_values);
    ~XRWebGLLayer() override;

    static XRWebGLLayer* Create(XRSession*, const V8XRWebGLRenderingContext*, const XRWebGLLayerInit*, ExceptionState&);
...
private:
    uint32_t GetBufferTextureId(const scoped_refptr<gpu::ClientSharedImage>& buffer_shared_image, const gpu::SyncToken& buffer_sync_token);

    void BindCameraBufferTexture(const scoped_refptr<gpu::ClientSharedImage>& buffer_shared_image);
