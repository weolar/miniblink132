...
XRTransientInputHitTestSource::XRTransientInputHitTestSource(uint64_t id, XRSession* xr_session)
    : id_(id)
...
void XRTransientInputHitTestSource::cancel(ExceptionState& exception_state)
{
    ...
    if (!xr_session_->RemoveHitTestSource(this)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, XRSession::kCannotCancelHitTestSource);
    }
}

void XRTransientInputHitTestSource::Update(
    const HashMap<uint32_t, Vector<device::mojom::blink::XRHitResultPtr>>& hit_test_results, XRInputSourceArray* input_source_array)
{
    ...
    for (const auto& source_id_and_results : hit_test_results) {
        XRInputSource* input_source = input_source_array->GetWithSourceId(source_id_and_results.key);
        ...
        if (input_source->IsVisible()) {
            current_frame_results_.push_back(MakeGarbageCollected<XRTransientInputHitTestResult>(input_source, source_id_and_results.value));
        }
    }
}

HeapVector<Member<XRTransientInputHitTestResult>> XRTransientInputHitTestSource::Results()
{
