...
public:
    ...
    XRPlane(uint64_t id, XRSession* session, const device::mojom::blink::XRPlaneData& plane_data, double timestamp);
    ...
    // XRPlane(plane_data).
    void Update(const device::mojom::blink::XRPlaneData& plane_data, double timestamp);
...
private:
    XRPlane(uint64_t id, XRSession* session, const std::optional<Orientation>& orientation, HeapVector<Member<DOMPointReadOnly>> polygon,
