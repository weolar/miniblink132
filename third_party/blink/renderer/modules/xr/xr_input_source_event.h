...
public:
    ...
    static XRInputSourceEvent* Create(const AtomicString& type, XRFrame* frame, XRInputSource* input_source)
    {
        return MakeGarbageCollected<XRInputSourceEvent>(type, frame, input_source);
    }

    static XRInputSourceEvent* Create(const AtomicString& type, const XRInputSourceEventInit* initializer)
    {
