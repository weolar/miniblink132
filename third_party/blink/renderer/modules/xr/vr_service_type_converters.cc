...
std::optional<blink::XRPlane::Orientation> TypeConverter<std::optional<blink::XRPlane::Orientation>, device::mojom::blink::XRPlaneOrientation>::Convert(
...
TypeConverter<blink::HeapVector<blink::Member<blink::DOMPointReadOnly>>, Vector<device::mojom::blink::XRPlanePointDataPtr>>::Convert(
    const Vector<device::mojom::blink::XRPlanePointDataPtr>& vertices)
{
    ...
    for (const auto& vertex_data : vertices) {
        result.push_back(blink::DOMPointReadOnly::Create(vertex_data->x, 0.0, vertex_data->z, 1.0));
    }
