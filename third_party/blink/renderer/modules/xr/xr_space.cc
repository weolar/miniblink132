...
std::optional<gfx::Transform> XRSpace::NativeFromViewer(const std::optional<gfx::Transform>& mojo_from_viewer) const
{
    if (!mojo_from_viewer)
        return std::nullopt;
...
{
    DVLOG(2) << __func__ << ": ToString()=" << ToString() << ", other_space->ToString()=" << other_space->ToString();
    ...
    gfx::Transform other_offset_from_mojo = other_space->OffsetFromNativeMatrix() * other_from_mojo.value();
    ...
    // resolved.
    gfx::Transform other_offset_from_offset = other_offset_from_mojo * mojo_from_offset.value();
    ...
    bool contains_nan = base::ranges::any_of(transform_data, [](const float f) { return std::isnan(f); });
    ...
    return MakeGarbageCollected<XRPose>(other_offset_from_offset, EmulatedPosition() || other_space->EmulatedPosition());
}

std::optional<gfx::Transform> XRSpace::OffsetFromViewer() const
{
    std::optional<gfx::Transform> native_from_viewer = NativeFromViewer(session()->GetMojoFrom(device::mojom::blink::XRReferenceSpaceType::kViewer));
