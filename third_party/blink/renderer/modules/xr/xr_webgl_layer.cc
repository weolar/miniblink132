...
const char kCleanFrameWarning[] = "Note: The XRSession has completed multiple animation frames without "
...
XRWebGLLayer* XRWebGLLayer::Create(
    XRSession* session, const V8XRWebGLRenderingContext* context, const XRWebGLLayerInit* initializer, ExceptionState& exception_state)
{
    if (session->ended()) {
    ...
    WebGLRenderingContextBase* webgl_context = webglRenderingContextBaseFromUnion(context);
    ...
    if (want_ignore_depth_values) {
        UseCounter::Count(session->GetExecutionContext(), WebFeature::kWebXrIgnoreDepthValues);
    }
    ...
    bool ignore_depth_values = !compositor_supports_depth_values || want_ignore_depth_values;
    ...
    if (!session->immersive()) {
        return MakeGarbageCollected<XRWebGLLayer>(session, webgl_context, nullptr, nullptr, framebuffer_scale, ignore_depth_values);
    }

    const bool want_antialiasing = initializer->antialias() && session->CanEnableAntiAliasing();
    ...
    if (initializer->hasFramebufferScaleFactor()) {
        UseCounter::Count(session->GetExecutionContext(), WebFeature::kWebXrFramebufferScale);
        ...
        framebuffer_scale = std::clamp(initializer->framebufferScaleFactor(), kFramebufferMinScale, max_scale);
    }

    gfx::SizeF framebuffers_size = session->RecommendedFramebufferSize();

    gfx::Size desired_size = gfx::ToFlooredSize(gfx::ScaleSize(framebuffers_size, framebuffer_scale));

    // Create an opaque WebGL Framebuffer
    WebGLFramebuffer* framebuffer = WebGLFramebuffer::CreateOpaque(webgl_context, want_depth_buffer, want_stencil_buffer);

    scoped_refptr<XRWebGLDrawingBuffer> drawing_buffer = XRWebGLDrawingBuffer::Create(
        webgl_context->GetDrawingBuffer(), framebuffer->Object(), desired_size, want_alpha_channel, want_depth_buffer, want_stencil_buffer, want_antialiasing);

    if (!drawing_buffer) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Unable to create a framebuffer.");
        return nullptr;
    }

    return MakeGarbageCollected<XRWebGLLayer>(session, webgl_context, std::move(drawing_buffer), framebuffer, framebuffer_scale, ignore_depth_values);
}

XRWebGLLayer::XRWebGLLayer(XRSession* session, WebGLRenderingContextBase* webgl_context, scoped_refptr<XRWebGLDrawingBuffer> drawing_buffer,
    WebGLFramebuffer* framebuffer, double framebuffer_scale, bool ignore_depth_values)
    : XRLayer(session)
...
XRViewport* XRWebGLLayer::getViewport(XRView* view)
{
    ...
    if (view_data->ViewportModifiable() && view_data->CurrentViewportScale() != view_data->RequestedViewportScale()) {
        DVLOG(2) << __func__ << ": apply ViewportScale=" << view_data->RequestedViewportScale();
        view_data->SetCurrentViewportScale(view_data->RequestedViewportScale());
        viewports_dirty_ = true;
    }
    TRACE_COUNTER1("xr", "XR viewport scale (%)", view_data->CurrentViewportScale() * 100);
    ...
    return view->Viewport(framebuffer_scale_ * session()->RecommendedFramebufferScale());
}
...
void XRWebGLLayer::UpdateViewports()
{
    ...
    if (framebuffer_width == 0U || framebuffer_height == 0U) {
        LOG_IF(ERROR, !webgl_context_->isContextLost())
            << __func__ << " Received width=" << framebuffer_width << " height=" << framebuffer_height << " without having lost context";
        return;
    }
    ...
    auto rounded = [](double v) { return std::max(1, base::saturated_cast<int>(v)); };
    ...
    if (session()->immersive()) {
        ...
        if (session()->StereoscopicViews()) {
            ...
            left_viewport_ = MakeGarbageCollected<XRViewport>(0, 0, rounded(framebuffer_width * 0.5 * left_scale), rounded(framebuffer_height * left_scale));
            double right_scale = session()->views()[1]->CurrentViewportScale();
            right_viewport_ = MakeGarbageCollected<XRViewport>(
                framebuffer_width * 0.5, 0, rounded(framebuffer_width * 0.5 * right_scale), rounded(framebuffer_height * right_scale));
        } else {
            ...
            left_viewport_ = MakeGarbageCollected<XRViewport>(0, 0, rounded(framebuffer_width * mono_scale), rounded(framebuffer_height * mono_scale));
        ...
        left_viewport_ = MakeGarbageCollected<XRViewport>(0, 0, framebuffer_width, framebuffer_height);
    }
}
...
WebGLTexture* XRWebGLLayer::GetCameraTexture()
{
    ...
    // return it:
    camera_image_texture_ = MakeGarbageCollected<WebGLUnownedTexture>(webgl_context_, camera_image_texture_id_, GL_TEXTURE_2D);
...
void XRWebGLLayer::OnFrameStart()
{
    if (framebuffer_) {
        ...
        const XRSharedImageData& content_image_data = layer_shared_images.content_image_data;
        const XRSharedImageData& camera_image_data = layer_shared_images.camera_image_data;

        if (content_image_data.shared_image) {
            drawing_buffer_->UseSharedBuffer(content_image_data.shared_image, content_image_data.sync_token);
            DVLOG(3) << __func__ << ": content_image_data.shared_image->mailbox()=" << content_image_data.shared_image->mailbox().ToDebugString();
        ...
        if (camera_image_data.shared_image) {
            DVLOG(3) << __func__ << ": camera_image_data.shared_image->mailbox()" << camera_image_data.shared_image->mailbox().ToDebugString();
            camera_image_texture_id_ = GetBufferTextureId(camera_image_data.shared_image, camera_image_data.sync_token);
            DVLOG(3) << __func__ << ": camera_image_texture_id_=" << camera_image_texture_id_;
            BindCameraBufferTexture(camera_image_data.shared_image);
        }
    }
}

uint32_t XRWebGLLayer::GetBufferTextureId(const scoped_refptr<gpu::ClientSharedImage>& buffer_shared_image, const gpu::SyncToken& buffer_sync_token)
{
    gpu::gles2::GLES2Interface* gl = drawing_buffer_->ContextGL();
    gl->WaitSyncTokenCHROMIUM(buffer_sync_token.GetConstData());
    DVLOG(3) << __func__ << ": buffer_sync_token=" << buffer_sync_token.ToDebugString();
    GLuint texture_id = gl->CreateAndTexStorage2DSharedImageCHROMIUM(buffer_shared_image->mailbox().name);
    DVLOG(3) << __func__ << ": texture_id=" << texture_id;
    return texture_id;
}

void XRWebGLLayer::BindCameraBufferTexture(const scoped_refptr<gpu::ClientSharedImage>& buffer_shared_image)
{
    gpu::gles2::GLES2Interface* gl = drawing_buffer_->ContextGL();
    ...
    if (buffer_shared_image) {
        ...
        gl->BeginSharedImageAccessDirectCHROMIUM(camera_image_texture_id_, GL_SHARED_IMAGE_ACCESS_MODE_READ_CHROMIUM);
    }
}
    ...
    if (framebuffer_) {
        ...
        if (session()->immersive()) {
            ...
            if (!framebuffer_dirty) {
                ...
                bool frame_had_pose = !!session()->GetMojoFrom(device::mojom::blink::XRReferenceSpaceType::kViewer);
                if (frame_had_pose) {
                    clean_frame_count++;
                    if (clean_frame_count == kCleanFrameWarningLimit) {
                        session()->xr()->GetExecutionContext()->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
                            mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning, kCleanFrameWarning));
                    }
                }
            }
            ...
            if (camera_image_texture_id_) {
                ...
                DVLOG(3) << __func__ << ": deleting camera image texture, camera_image_texture_id_=" << camera_image_texture_id_;
                gpu::gles2::GLES2Interface* gl = drawing_buffer_->ContextGL();
            ...
            session()->xr()->frameProvider()->SubmitWebGLLayer(this, framebuffer_dirty);
        }
    }
}
...
void XRWebGLLayer::OnResize()
{
    if (drawing_buffer_) {
        ...
        gfx::Size desired_size = gfx::ToFlooredSize(gfx::ScaleSize(framebuffers_size, framebuffer_scale_));
