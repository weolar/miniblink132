...
void AssertMatrixEquals(const Vector<double>& actual, const Vector<double>& expected)
{
    for (int i = 0; i < 16; ++i) {
...
TEST(XRViewTest, ViewMatrices)
{
    ...
    xr_view->field_of_view = device::mojom::blink::VRFieldOfView::New(kFov, kFov, kFov, kFov);
    ...
    HashSet<device::mojom::XRSessionFeature> features = { device::mojom::XRSessionFeature::REF_SPACE_VIEWER };
    XRViewData* view_data = MakeGarbageCollected<XRViewData>(
        /*index=*/0, std::move(xr_view), kDepthNear, kDepthFar, *device_config, features, XRGraphicsBinding::Api::kWebGL);
    XRView* view = MakeGarbageCollected<XRView>(nullptr, view_data, ref_space_from_mojo);

    AssertMatrixEquals(GetMatrixDataForTest(view_data->MojoFromView()), GetMatrixDataForTest(mojo_from_view));
    AssertMatrixEquals(GetMatrixDataForTest(view->refSpaceFromView()->TransformMatrix()), GetMatrixDataForTest(ref_space_from_view));
    AssertMatrixEquals(GetMatrixDataForTest(view_data->ProjectionMatrix()),
        GetMatrixDataForTest(gfx::Transform::ColMajor(0.78128596636, 0, 0, 0, 0, 0.78128596636, 0, 0, 0, 0, -1.00020002, -1, 0, 0, -0.200020002, 0)));
}
