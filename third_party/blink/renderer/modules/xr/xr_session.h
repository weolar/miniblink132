...
class XRSession final : public EventTarget, public device::mojom::blink::XRSessionClient, public ActiveScriptWrappable<XRSession> {
    DEFINE_WRAPPERTYPEINFO();

public:
    // Error strings used outside of XRSession:
    static constexpr char kNoRigidTransformSpecified[] = "No XRRigidTransform specified.";
    static constexpr char kUnableToRetrieveMatrix[] = "The operation was unable to retrieve a matrix from passed in space and "
                                                      "could not be completed.";
    static constexpr char kNoSpaceSpecified[] = "No XRSpace specified.";
    static constexpr char kAnchorsFeatureNotSupported[] = "Anchors feature is not supported by the session.";
    static constexpr char kPlanesFeatureNotSupported[] = "Plane detection feature is not supported by the session.";
    static constexpr char kDepthSensingFeatureNotSupported[] = "Depth sensing feature is not supported by the session.";
    static constexpr char kRawCameraAccessFeatureNotSupported[] = "Raw camera access feature is not supported by the session.";
    static constexpr char kCannotCancelHitTestSource[] = "Hit test source could not be canceled! Ensure that it was not already "
                                                         "canceled.";
    static constexpr char kCannotReportPoses[] = "Poses cannot be given out for the current state.";
    ...
    struct MetricsReporter {
        explicit MetricsReporter(mojo::Remote<device::mojom::blink::XRSessionMetricsRecorder> recorder);
    ...
    XRSession(XRSystem* xr, mojo::PendingReceiver<device::mojom::blink::XRSessionClient> client_receiver, device::mojom::blink::XRSessionMode mode,
    ...
    DOMFloat32Array* supportedFrameRates() const
    {
        return nullptr;
    }
    ...
    void updateRenderState(XRRenderStateInit* render_state_init, ExceptionState& exception_state);

    std::optional<V8XRDepthUsage> depthUsage(ExceptionState& exception_state);
    std::optional<V8XRDepthDataFormat> depthDataFormat(ExceptionState& exception_state);

    ScriptPromise<IDLUndefined> updateTargetFrameRate(float rate, ExceptionState&);

    ScriptPromise<XRReferenceSpace> requestReferenceSpace(ScriptState* script_state, const V8XRReferenceSpaceType& type, ExceptionState&);
    ...
    // plane.
    ScriptPromise<XRAnchor> CreateAnchorHelper(ScriptState* script_state, const gfx::Transform& native_origin_from_anchor,
    ...
    ScriptPromise<XRHitTestSource> requestHitTestSource(ScriptState* script_state, XRHitTestOptionsInit* options, ExceptionState& exception_state);
    ScriptPromise<XRTransientInputHitTestSource> requestHitTestSourceForTransientInput(
        ScriptState* script_state, XRTransientInputHitTestOptionsInit* options_init, ExceptionState& exception_state);

    ScriptPromise<XRLightProbe> requestLightProbe(ScriptState* script_state, XRLightProbeInit*, ExceptionState&);

    ScriptPromise<IDLArray<V8XRImageTrackingScore>> getTrackedImageScores(ScriptState* script_state, ExceptionState&);
    ...
    size_t array_texture_layers() const
    {
        return views_.size();
    }
    ...
    void OnFocusChanged();
    void OnFrame(double timestamp, scoped_refptr<gpu::ClientSharedImage> output_shared_image, const gpu::SyncToken& output_sync_token,
    ...
    bool StereoscopicViews()
    {
        return views_.size() >= 2;
    }
    ...
    uint32_t StageParametersId() const
    {
        return stage_parameters_id_;
    }
    ...
    bool ValidateHitTestSourceExists(XRTransientInputHitTestSource* hit_test_source) const;
    ...
    // types.
    std::optional<gfx::Transform> GetMojoFrom(device::mojom::blink::XRReferenceSpaceType space_type) const;
    ...
    // presentation frames.
    void UpdatePresentationFrameState(double timestamp, device::mojom::blink::XRFrameDataPtr frame_data, int16_t frame_id, bool emulated_position);
    ...
    const FrozenArray<XRImageTrackingResult>& ImageTrackingResults(ExceptionState&);
...
private:
    ...
    void OnInputStateChangeInternal(int16_t frame_id, base::span<const device::mojom::blink::XRInputSourceStatePtr> input_states);
    void ProcessInputSourceEvents(base::span<const device::mojom::blink::XRInputSourceStatePtr> input_states);

    void UpdateViews(Vector<device::mojom::blink::XRViewPtr> views);
    void UpdateStageParameters(uint32_t stage_parameters_id, const device::mojom::blink::VRStageParametersPtr& stage_parameters);
    ...
    void UpdateWorldUnderstandingStateForFrame(double timestamp, const device::mojom::blink::XRFrameDataPtr& frame_data);
    ...
    void OnSubscribeToHitTestResult(ScriptPromiseResolver<XRHitTestSource>* resolver, device::mojom::SubscribeToHitTestResult result, uint64_t subscription_id);
    ...
    void OnCreateAnchorResult(ScriptPromiseResolver<XRAnchor>* resolver, device::mojom::CreateAnchorResult result, uint64_t id);

    void EnsureEnvironmentErrorHandler();
    void OnEnvironmentProviderError();

    void ProcessAnchorsData(const device::mojom::blink::XRAnchorsData* tracked_anchors_data, double timestamp);

    void CleanUpUnusedHitTestSources();

    void ProcessHitTestData(const device::mojom::blink::XRHitTestSubscriptionResultsData* hit_test_data);

    void ProcessTrackedImagesData(const device::mojom::blink::XRTrackedImagesData*);
    ...
    using ImageScoreResolverType = ScriptPromiseResolver<IDLArray<V8XRImageTrackingScore>>;
    ...
    // resolved then.
    HeapHashMap<uint64_t, Member<ScriptPromiseResolverBase>> anchor_ids_to_pending_anchor_promises_;
    ...
    HeapHashMap<uint64_t, WeakMember<XRHitTestSource>> hit_test_source_ids_to_hit_test_sources_;
    HeapHashMap<uint64_t, WeakMember<XRTransientInputHitTestSource>> hit_test_source_ids_to_transient_input_hit_test_sources_;
    ...
    HeapHashSet<Member<ScriptPromiseResolverBase>> request_hit_test_source_promises_;
    ...
    HeapMojoReceiver<device::mojom::blink::XRSessionClient, XRSession> client_receiver_;
