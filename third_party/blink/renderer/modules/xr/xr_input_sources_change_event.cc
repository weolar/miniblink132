...
XRInputSourcesChangeEvent::XRInputSourcesChangeEvent(
    ...
    , session_(session)
    , added_(MakeGarbageCollected<FrozenArray<XRInputSource>>(std::move(added)))
    , removed_(MakeGarbageCollected<FrozenArray<XRInputSource>>(std::move(removed)))
{
}

XRInputSourcesChangeEvent::XRInputSourcesChangeEvent(const AtomicString& type, const XRInputSourcesChangeEventInit* initializer)
    : Event(type, initializer)
{
    ...
    if (initializer->hasAdded()) {
        added_ = MakeGarbageCollected<FrozenArray<XRInputSource>>(initializer->added());
    } else {
        added_ = MakeGarbageCollected<FrozenArray<XRInputSource>>();
    }
    if (initializer->hasRemoved()) {
        removed_ = MakeGarbageCollected<FrozenArray<XRInputSource>>(initializer->removed());
    } else {
