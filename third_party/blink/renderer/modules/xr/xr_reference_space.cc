...
ReferenceSpaceType XRReferenceSpace::V8EnumToReferenceSpaceType(V8XRReferenceSpaceType::Enum reference_space_type)
{
...
XRReferenceSpace::XRReferenceSpace(XRSession* session, ReferenceSpaceType type)
    : XRReferenceSpace(session, MakeGarbageCollected<XRRigidTransform>(nullptr, nullptr), type)
{
}

XRReferenceSpace::XRReferenceSpace(XRSession* session, XRRigidTransform* origin_offset, ReferenceSpaceType type)
    : XRSpace(session)
...
XRPose* XRReferenceSpace::getPose(const XRSpace* other_space) const
{
    if (type_ == ReferenceSpaceType::kViewer) {
        std::optional<gfx::Transform> other_offset_from_viewer = other_space->OffsetFromViewer();
        ...
        auto other_offset_from_offset = *other_offset_from_viewer * viewer_from_offset;

        return MakeGarbageCollected<XRPose>(other_offset_from_offset, session()->EmulatedPosition());
    } else {
...
{
    const device::mojom::blink::VRStageParametersPtr& stage_parameters = session()->GetStageParameters();

    if (stage_parameters) {
        // Use the transform given by stage_parameters if available.
        mojo_from_floor_ = std::make_unique<gfx::Transform>(stage_parameters->mojo_from_floor);
    } else {
...
std::optional<gfx::Transform> XRReferenceSpace::MojoFromNative() const
{
    ...
    case ReferenceSpaceType::kUnbounded: {
        ...
        if (!mojo_from_native) {
            ...
            // return nullopt.
            return type_ == ReferenceSpaceType::kViewer ? std::optional<gfx::Transform>(gfx::Transform {}) : std::nullopt;
        }
    ...
    case ReferenceSpaceType::kLocalFloor: {
        ...
        auto local_from_floor = gfx::Transform::MakeTranslation(0, kDefaultEmulationHeightMeters);
...
std::optional<gfx::Transform> XRReferenceSpace::NativeFromViewer(const std::optional<gfx::Transform>& mojo_from_viewer) const
{
    if (type_ == ReferenceSpaceType::kViewer) {
...
XRReferenceSpace* XRReferenceSpace::getOffsetReferenceSpace(XRRigidTransform* additional_offset) const
{
...
XRReferenceSpace* XRReferenceSpace::cloneWithOriginOffset(XRRigidTransform* origin_offset) const
{
    return MakeGarbageCollected<XRReferenceSpace>(this->session(), origin_offset, type_);
}

device::mojom::blink::XRNativeOriginInformationPtr XRReferenceSpace::NativeOrigin() const
{
    return device::mojom::blink::XRNativeOriginInformation::NewReferenceSpaceType(this->GetType());
}
...
void XRReferenceSpace::OnReset()
{
    if (type_ != ReferenceSpaceType::kViewer) {
        ...
        mutable_this->DispatchEvent(*XRReferenceSpaceEvent::Create(event_type_names::kReset, mutable_this));
    }
}
