...
XRFrameRequestCallbackCollection::XRFrameRequestCallbackCollection(ExecutionContext* context)
    : context_(context)
{
}

XRFrameRequestCallbackCollection::CallbackId XRFrameRequestCallbackCollection::RegisterCallback(V8XRFrameRequestCallback* callback)
{
    CallbackId id = ++next_callback_id_;
    auto add_result_frame_request = callback_frame_requests_.Set(id, callback);
    auto add_result_async_task = callback_async_tasks_.Set(id, std::make_unique<probe::AsyncTaskContext>());
    DCHECK_EQ(add_result_frame_request.is_new_entry, add_result_async_task.is_new_entry);
    pending_callbacks_.push_back(id);

    add_result_async_task.stored_value->value->Schedule(context_, "XRRequestFrame");
...
void XRFrameRequestCallbackCollection::ExecuteCallbacks(XRSession* session, double timestamp, XRFrame* frame)
{
    ...
    for (const auto& id : current_callback_ids) {
        ...
        CHECK_NE(current_callback_async_tasks_.end(), it_async_task, base::NotFatalUntil::M130);

        probe::AsyncTask async_task(context_, it_async_task->value.get());
        it_frame_request->value->InvokeAndReportException(session, timestamp, frame);
    }
