...
namespace device {
template <class T> class GamepadImpl;
...
public:
    static XRInputSource* CreateOrUpdateFrom(
        XRInputSource* other /* may be null, input */, XRSession* session, const device::mojom::blink::XRInputSourceStatePtr& state);

    XRInputSource(XRSession*, uint32_t source_id, device::mojom::XRTargetRayMode target_ray_mode = device::mojom::XRTargetRayMode::GAZING);
    ...
    {
        return state_.emulated_position;
    }
    ...
    const FrozenArray<IDLString>& profiles() const
    {
    ...
    // Gamepad::Client
    GamepadHapticActuator* GetVibrationActuatorForGamepad(const Gamepad&) override
    {
    ...
    void UpdateButtonStates(const device::mojom::blink::XRInputSourceStatePtr& state);
    void OnRemoved();
    ...
    void ProcessOverlayHitTest(Element* overlay_element, const device::mojom::blink::XRInputSourceStatePtr& state);
...
private:
    ...
    struct InternalState {
        ...
        InternalState(uint32_t source_id, device::mojom::XRTargetRayMode, base::TimeTicks base_timestamp);
    ...
    bool InvalidatesSameObject(const device::mojom::blink::XRInputSourceStatePtr& state);
    ...
    void UpdateHand(const device::mojom::blink::XRHandTrackingData* hand_joint_data);
