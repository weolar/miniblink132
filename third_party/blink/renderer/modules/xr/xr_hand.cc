...
class XRHandIterationSource final : public PairSyncIterable<XRHand>::IterationSource {
public:
    explicit XRHandIterationSource(const Member<XRJointVector>& joints, XRHand* xr_hand)
        : joints_(joints)
        , xr_hand_(xr_hand)
    {
    }

    bool FetchNextItem(ScriptState*, V8XRHandJoint& key, XRJointSpace*& value, ExceptionState&) override
    {
...
XRHand::XRHand(const device::mojom::blink::XRHandTrackingData* state, XRInputSource* input_source)
    : joints_(MakeGarbageCollected<XRJointVector>())
{
    ...
    for (unsigned i = 0; i < kNumJoints; ++i) {
        device::mojom::blink::XRHandJoint joint = static_cast<device::mojom::blink::XRHandJoint>(i);
        DCHECK_EQ(MojomHandJointToV8Enum(joint), static_cast<V8XRHandJoint::Enum>(i));
        joints_->push_back(MakeGarbageCollected<XRJointSpace>(this, input_source->session(), nullptr, joint, 0.0f, input_source->xr_handedness()));
    }
...
void XRHand::updateFromHandTrackingData(const device::mojom::blink::XRHandTrackingData* state, XRInputSource* input_source)
{
    ...
    for (const auto& hand_joint : state->hand_joint_data) {
        ...
        if (hand_joint->mojo_from_joint) {
            new_poses = true;
            mojo_from_joint = std::make_unique<gfx::Transform>(*hand_joint->mojo_from_joint);
        } else {
            new_missing_poses = true;
        }

        joints_->at(joint_index)->UpdateTracking(std::move(mojo_from_joint), hand_joint->radius);
    }
    ...
    if (new_missing_poses) {
        ...
        has_missing_poses_ = !base::ranges::all_of(*joints_, &XRJointSpace::MojoFromNative);
    }
}

XRHand::IterationSource* XRHand::CreateIterationSource(ScriptState* script_state, ExceptionState& exception_state)
{
