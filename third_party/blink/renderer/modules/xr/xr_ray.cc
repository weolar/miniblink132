...
constexpr char kInvalidWComponentInOrigin[] = "Origin's `w` component must be set to 1.0f!";
constexpr char kInvalidWComponentInDirection[] = "Direction's `w` component must be set to 0.0f!";
...
XRRay::XRRay(DOMPointInit* origin, XRRayDirectionInit* direction, ExceptionState& exception_state)
{
    DCHECK(origin);
    DCHECK(direction);
...
void XRRay::Set(gfx::Point3F origin, gfx::Vector3dF direction, ExceptionState& exception_state)
{
    DVLOG(3) << __FUNCTION__ << ": origin=" << origin.ToString() << ", direction=" << direction.ToString();
    ...
    direction_ = DOMPointReadOnly::Create(normalized_direction.x(), normalized_direction.y(), normalized_direction.z(), 0.0);
}

XRRay* XRRay::Create(XRRigidTransform* transform, ExceptionState& exception_state)
{
...
XRRay* XRRay::Create(DOMPointInit* origin, XRRayDirectionInit* direction, ExceptionState& exception_state)
{
    auto* result = MakeGarbageCollected<XRRay>(origin, direction, exception_state);
...
DOMFloat32Array* XRRay::matrix()
{
    ...
    if (!matrix_ || !matrix_->Data()) {
        ...
        const gfx::Vector3dF desired_ray_direction(
            static_cast<float>(direction_->x()), static_cast<float>(direction_->y()), static_cast<float>(direction_->z()));
        ...
        // z × direction
        gfx::Vector3dF axis = gfx::CrossProduct(initial_ray_direction, desired_ray_direction);

        // Step 4: Let cos_angle be the scalar dot product of z and ray’s direction,
        // z · direction
        float cos_angle = gfx::DotProduct(initial_ray_direction, desired_ray_direction);
