...
public:
    XRView(XRFrame* frame, XRViewData* view_data, const gfx::Transform& ref_space_from_mojo);

    V8XREye eye() const;
    device::mojom::blink::XREye EyeValue() const
    {
        return eye_;
    }
    XRViewData* ViewData() const
    {
        return view_data_.Get();
    }
    ...
    XRCPUDepthInformation* GetCpuDepthInformation(ExceptionState& exception_state) const;

    XRWebGLDepthInformation* GetWebGLDepthInformation(ExceptionState& exception_state) const;
...
public:
    explicit XRViewData(wtf_size_t index, device::mojom::blink::XREye eye, gfx::Rect viewport, XRGraphicsBinding::Api graphics_api)
        : index_(index)
        , eye_(eye)
    ...
    XRViewData(wtf_size_t index, device::mojom::blink::XRViewPtr view, double depth_near, double depth_far,
    ...
    void UpdateView(device::mojom::blink::XRViewPtr view, double depth_near, double depth_far);

    void UpdateProjectionMatrixFromFoV(float up_rad, float down_rad, float left_rad, float right_rad, float near_depth, float far_depth);
    void UpdateProjectionMatrixFromAspect(float fovy, float aspect, float near_depth, float far_depth);

    gfx::Transform UnprojectPointer(double x, double y, double canvas_width, double canvas_height);
    ...
    device::mojom::blink::XREye Eye() const
    {
        return eye_;
    }
    const gfx::Transform& MojoFromView() const
    {
        return mojo_from_view_;
    }
    const gfx::Transform& ProjectionMatrix() const
    {
    ...
    bool IsFirstPersonObserver() const
    {
        return is_first_person_observer_;
    }

    XRCPUDepthInformation* GetCpuDepthInformation(const XRFrame* xr_frame, ExceptionState& exception_state) const;

    XRWebGLDepthInformation* GetWebGLDepthInformation(const XRFrame* xr_frame, ExceptionState& exception_state) const;
    ...
    bool ViewportModifiable() const
    {
        return viewport_modifiable_;
    }
    void SetViewportModifiable(bool modifiable)
    {
        viewport_modifiable_ = modifiable;
    }
    double CurrentViewportScale() const
    {
        return current_viewport_scale_;
    }
    void SetCurrentViewportScale(double scale)
    {
        current_viewport_scale_ = scale;
    }
    double RequestedViewportScale() const
    {
        return requested_viewport_scale_;
    }
