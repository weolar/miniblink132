...
XRJointSpace::XRJointSpace(XRHand* hand, XRSession* session, std::unique_ptr<gfx::Transform> mojo_from_joint, device::mojom::blink::XRHandJoint joint,
    float radius, device::mojom::blink::XRHandedness handedness)
    : XRSpace(session)
    , hand_(hand)
...
device::mojom::blink::XRNativeOriginInformationPtr XRJointSpace::NativeOrigin() const
{
    device::mojom::blink::XRHandJointSpaceInfoPtr joint_space_info = device::mojom::blink::XRHandJointSpaceInfo::New();
    joint_space_info->handedness = this->handedness();
    joint_space_info->joint = this->joint();
    return device::mojom::blink::XRNativeOriginInformation::NewHandJointSpaceInfo(std::move(joint_space_info));
}
...
void XRJointSpace::UpdateTracking(std::unique_ptr<gfx::Transform> mojo_from_joint, float radius)
{
    mojo_from_joint_space_ = std::move(mojo_from_joint);
    radius_ = radius;
}
