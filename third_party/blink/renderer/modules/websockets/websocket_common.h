...
public:
    ...
    // are.
    ConnectResult Connect(ExecutionContext*, const String& url, const Vector<String>& protocols, WebSocketChannel*, ExceptionState&);

    // Closes the connection if |code| and |reason| are valid.
    void CloseInternal(std::optional<uint16_t> code, const String& reason, WebSocketChannel*, ExceptionState&);
    ...
    static std::optional<uint16_t> ValidateCloseCodeAndReason(std::optional<uint16_t> code, const String& reason, ExceptionState&);
