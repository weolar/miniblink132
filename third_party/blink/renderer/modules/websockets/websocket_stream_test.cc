...
public:
    ...
    MockWebSocketChannel& Channel() const
    {
        return *channel_;
    }

    WebSocketStream* Create(ScriptState* script_state, const String& url, ExceptionState& exception_state)
    {
        return Create(script_state, url, WebSocketStreamOptions::Create(), exception_state);
    }

    WebSocketStream* Create(ScriptState* script_state, const String& url, WebSocketStreamOptions* options, ExceptionState& exception_state)
    {
        return WebSocketStream::CreateForTesting(script_state, url, options, channel_, exception_state);
    }
    ...
    {
        return V8WebSocketError::HasInstance(script_state->GetIsolate(), value.V8Value());
    }
    ...
    // found".
    std::string PropertyAsString(ScriptState* script_state, v8::Local<v8::Value> object, String key)
    {
        v8::Local<v8::Value> value;
        auto* isolate = script_state->GetIsolate();
        if (!object.As<v8::Object>()->GetRealNamedProperty(script_state->GetContext(), V8String(isolate, key)).ToLocal(&value)) {
...
TEST_F(WebSocketStreamTest, ConstructWithBadURL)
{
    ...
    EXPECT_EQ(DOMExceptionCode::kSyntaxError, exception_state.CodeAs<DOMExceptionCode>());
...
TEST_F(WebSocketStreamTest, Connect)
{
...

    {
        InSequence s;
        EXPECT_CALL(Channel(), ApplyBackpressure());
        EXPECT_CALL(Channel(), Connect(KURL("ws://example.com/hoge"), String())).WillOnce(Return(true));
    }

    auto* stream = Create(scope.GetScriptState(), "ws://example.com/hoge", ASSERT_NO_EXCEPTION);
...
TEST_F(WebSocketStreamTest, ConnectWithProtocols)
{
...

    {
        InSequence s;
        EXPECT_CALL(Channel(), ApplyBackpressure());
        EXPECT_CALL(Channel(), Connect(KURL("ws://example.com/chat"), String("chat0, chat1"))).WillOnce(Return(true));
    }

    auto* options = WebSocketStreamOptions::Create();
    options->setProtocols({ "chat0", "chat1" });
    auto* stream = Create(scope.GetScriptState(), "ws://example.com/chat", options, ASSERT_NO_EXCEPTION);
...
TEST_F(WebSocketStreamTest, ConnectWithFailedHandshake)
{
...

    {
        InSequence s;
        EXPECT_CALL(Channel(), ApplyBackpressure());
        EXPECT_CALL(Channel(), Connect(KURL("ws://example.com/chat"), String())).WillOnce(Return(true));
        EXPECT_CALL(Channel(), Disconnect());
    }

    auto* script_state = scope.GetScriptState();
    auto* stream = Create(script_state, "ws://example.com/chat", ASSERT_NO_EXCEPTION);
    ...
    stream->DidError();
    stream->DidClose(WebSocketChannelClient::kClosingHandshakeIncomplete, WebSocketChannel::kCloseEventCodeAbnormalClosure, String());
...
TEST_F(WebSocketStreamTest, ConnectWithSuccessfulHandshake)
{
...

    {
        InSequence s;
        EXPECT_CALL(Channel(), ApplyBackpressure());
        EXPECT_CALL(Channel(), Connect(KURL("ws://example.com/chat"), String("chat"))).WillOnce(Return(true));
    }

    auto* options = WebSocketStreamOptions::Create();
    options->setProtocols({ "chat" });
    auto* script_state = scope.GetScriptState();
    auto* stream = Create(script_state, "ws://example.com/chat", options, ASSERT_NO_EXCEPTION);
    ...
    EXPECT_EQ(PropertyAsString(script_state, value, "readable"), "[object ReadableStream]");
    EXPECT_EQ(PropertyAsString(script_state, value, "writable"), "[object WritableStream]");
    EXPECT_EQ(PropertyAsString(script_state, value, "protocol"), "chat");
    EXPECT_EQ(PropertyAsString(script_state, value, "extensions"), "permessage-deflate");
}
...
TEST_F(WebSocketStreamTest, ConnectThenCloseCleanly)
{
...

    {
        InSequence s;
        EXPECT_CALL(Channel(), ApplyBackpressure());
        EXPECT_CALL(Channel(), Connect(KURL("ws://example.com/echo"), String())).WillOnce(Return(true));
        EXPECT_CALL(Channel(), Close(-1, String("")));
        EXPECT_CALL(Channel(), Disconnect());
    }

    auto* script_state = scope.GetScriptState();
    auto* stream = Create(script_state, "ws://example.com/echo", ASSERT_NO_EXCEPTION);
    ...
    stream->close(MakeGarbageCollected<WebSocketCloseInfo>(), scope.GetExceptionState());
    ...
    EXPECT_EQ(PropertyAsString(script_state, closed_tester.Value().V8Value(), "closeCode"), "1005");
    EXPECT_EQ(PropertyAsString(script_state, closed_tester.Value().V8Value(), "reason"), "");
}
...
TEST_F(WebSocketStreamTest, CloseDuringHandshake)
{
...

    {
        ...
        EXPECT_CALL(Channel(), ApplyBackpressure());
        EXPECT_CALL(Channel(), Connect(KURL("ws://example.com/echo"), String())).WillOnce(Return(true));
        EXPECT_CALL(Channel(), FailMock(String("WebSocket is closed before the connection is established."), mojom::ConsoleMessageLevel::kWarning, _));
        EXPECT_CALL(Channel(), Disconnect());
    }
...
TEST_F(WebSocketStreamTest, AbortBeforeHandshake)
{
    ...
    auto* stream = Create(script_state, "ws://example.com/echo", options, ASSERT_NO_EXCEPTION);
    ...
    EXPECT_TRUE(opened_tester.IsRejected());
    EXPECT_TRUE(IsDOMException(script_state, opened_tester.Value(), DOMExceptionCode::kAbortError));
    EXPECT_TRUE(closed_tester.IsRejected());
    EXPECT_TRUE(IsDOMException(script_state, closed_tester.Value(), DOMExceptionCode::kAbortError));
}
