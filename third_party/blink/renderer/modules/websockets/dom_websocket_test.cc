...
public:
    ...
    {
        DOMWebSocketWithMockChannel* websocket = MakeGarbageCollected<DOMWebSocketWithMockChannel>(context);
...
public:
    ...
    MockWebSocketChannel& Channel()
    {
        return *websocket_->Channel();
    }
    DOMWebSocketWithMockChannel& Socket()
    {
...
TEST(DOMWebSocketTest, connectToBadURL)
{
    ...
    websocket_scope.Socket().Connect("xxx", Vector<String>(), scope.GetExceptionState());

    EXPECT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ(DOMExceptionCode::kSyntaxError, scope.GetExceptionState().CodeAs<DOMExceptionCode>());
...
TEST(DOMWebSocketTest, connectToNonWsURL)
{
    ...
    V8TestingScope scope;
    DOMWebSocketTestScope websocket_scope(scope.GetExecutionContext());
    websocket_scope.Socket().Connect("bad-scheme://example.com/", Vector<String>(), scope.GetExceptionState());

    EXPECT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ(DOMExceptionCode::kSyntaxError, scope.GetExceptionState().CodeAs<DOMExceptionCode>());
    EXPECT_EQ("The URL's scheme must be either 'http', 'https', 'ws', or 'wss'. "
...
TEST(DOMWebSocketTest, connectToURLHavingFragmentIdentifier)
{
    ...
    V8TestingScope scope;
    DOMWebSocketTestScope websocket_scope(scope.GetExecutionContext());
    websocket_scope.Socket().Connect("ws://example.com/#fragment", Vector<String>(), scope.GetExceptionState());

    EXPECT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ(DOMExceptionCode::kSyntaxError, scope.GetExceptionState().CodeAs<DOMExceptionCode>());
    EXPECT_EQ("The URL contains a fragment identifier ('fragment'). Fragment "
...
TEST(DOMWebSocketTest, invalidSubprotocols)
{
    ...
    websocket_scope.Socket().Connect("ws://example.com/", subprotocols, scope.GetExceptionState());
...
TEST(DOMWebSocketTest, insecureRequestsUpgrade)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(websocket_scope.Channel(), Connect(KURL("wss://example.com/endpoint"), String())).WillOnce(Return(true));
    }

    scope.GetWindow().GetSecurityContext().SetInsecureRequestPolicy(mojom::blink::InsecureRequestPolicy::kUpgradeInsecureRequests);
    websocket_scope.Socket().Connect("ws://example.com/endpoint", Vector<String>(), scope.GetExceptionState());
...
TEST(DOMWebSocketTest, insecureRequestsUpgradePotentiallyTrustworthy)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(websocket_scope.Channel(), Connect(KURL("ws://127.0.0.1/endpoint"), String())).WillOnce(Return(true));
    }

    scope.GetWindow().GetSecurityContext().SetInsecureRequestPolicy(mojom::blink::InsecureRequestPolicy::kUpgradeInsecureRequests);
    websocket_scope.Socket().Connect("ws://127.0.0.1/endpoint", Vector<String>(), scope.GetExceptionState());
...
TEST(DOMWebSocketTest, insecureRequestsDoNotUpgrade)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(websocket_scope.Channel(), Connect(KURL("ws://example.com/endpoint"), String())).WillOnce(Return(true));
    }

    scope.GetWindow().GetSecurityContext().SetInsecureRequestPolicy(mojom::blink::InsecureRequestPolicy::kLeaveInsecureRequestsAlone);
    websocket_scope.Socket().Connect("ws://example.com/endpoint", Vector<String>(), scope.GetExceptionState());
...
TEST(DOMWebSocketTest, channelConnectSuccess)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(websocket_scope.Channel(), Connect(KURL("ws://example.com/hoge"), String("aa, bb"))).WillOnce(Return(true));
    }

    websocket_scope.Socket().Connect("ws://example.com/hoge", Vector<String>(subprotocols), scope.GetExceptionState());
...
TEST(DOMWebSocketTest, channelConnectFail)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(websocket_scope.Channel(), Connect(KURL("ws://example.com/"), String("aa, bb"))).WillOnce(Return(false));
        EXPECT_CALL(websocket_scope.Channel(), Disconnect());
    }

    websocket_scope.Socket().Connect("ws://example.com/", Vector<String>(subprotocols), scope.GetExceptionState());

    EXPECT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ(DOMExceptionCode::kSecurityError, scope.GetExceptionState().CodeAs<DOMExceptionCode>());
    EXPECT_EQ("An insecure WebSocket connection may not be initiated from a page "
              "loaded over HTTPS.",
        scope.GetExceptionState().Message());
    EXPECT_EQ(DOMWebSocket::kClosed, websocket_scope.Socket().readyState());
}
...
TEST(DOMWebSocketTest, connectSuccess)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(websocket_scope.Channel(), Connect(KURL("ws://example.com/"), String("aa, bb"))).WillOnce(Return(true));
    }
    websocket_scope.Socket().Connect("ws://example.com/", subprotocols, scope.GetExceptionState());
...
TEST(DOMWebSocketTest, didClose)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(websocket_scope.Channel(), Connect(KURL("ws://example.com/"), String())).WillOnce(Return(true));
        EXPECT_CALL(websocket_scope.Channel(), Disconnect());
    }
    websocket_scope.Socket().Connect("ws://example.com/", Vector<String>(), scope.GetExceptionState());

    EXPECT_FALSE(scope.GetExceptionState().HadException());
    EXPECT_EQ(DOMWebSocket::kConnecting, websocket_scope.Socket().readyState());

    websocket_scope.Socket().DidClose(WebSocketChannelClient::kClosingHandshakeIncomplete, 1006, "");
...
TEST(DOMWebSocketTest, maximumReasonSize)
{
    ...
    websocket_scope.Socket().close(1000, reason.ToString(), scope.GetExceptionState());
...
TEST(DOMWebSocketTest, reasonSizeExceeding)
{
    ...
    EXPECT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ(DOMExceptionCode::kSyntaxError, scope.GetExceptionState().CodeAs<DOMExceptionCode>());
    EXPECT_EQ("The close reason must not be greater than 123 UTF-8 bytes.", scope.GetExceptionState().Message());
    EXPECT_EQ(DOMWebSocket::kConnecting, websocket_scope.Socket().readyState());
}
...
TEST(DOMWebSocketTest, closeWhenConnecting)
{
    ...
    DOMWebSocketTestScope websocket_scope(scope.GetExecutionContext());
    {
        ...
        EXPECT_CALL(websocket_scope.Channel(), Connect(KURL("ws://example.com/"), String())).WillOnce(Return(true));
        EXPECT_CALL(
            websocket_scope.Channel(), FailMock(String("WebSocket is closed before the connection is established."), mojom::ConsoleMessageLevel::kWarning, _));
    }
    websocket_scope.Socket().Connect("ws://example.com/", Vector<String>(), scope.GetExceptionState());
...
TEST(DOMWebSocketTest, closeWhenClosed)
{
    ...
    websocket_scope.Socket().DidClose(WebSocketChannelClient::kClosingHandshakeComplete, 1000, String());
    EXPECT_EQ(DOMWebSocket::kClosed, websocket_scope.Socket().readyState());
    websocket_scope.Socket().close(scope.GetExceptionState());
...
TEST(DOMWebSocketTest, sendStringWhenConnecting)
{
    ...
    EXPECT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ(DOMExceptionCode::kInvalidStateError, scope.GetExceptionState().CodeAs<DOMExceptionCode>());
...
TEST(DOMWebSocketTest, sendNonLatin1String)
{
    ...
    DOMWebSocketTestScope websocket_scope(scope.GetExecutionContext());
    {
        InSequence s;
        EXPECT_CALL(websocket_scope.Channel(), Connect(KURL("ws://example.com/"), String())).WillOnce(Return(true));
        EXPECT_CALL(websocket_scope.Channel(), Send(std::string("\xe7\x8b\x90\xe0\xa4\x94"), _));
    }
    websocket_scope.Socket().Connect("ws://example.com/", Vector<String>(), scope.GetExceptionState());
...
TEST(DOMWebSocketTest, binaryType)
{
    ...
    websocket_scope.Socket().setBinaryType(V8BinaryType(V8BinaryType::Enum::kArraybuffer));

    EXPECT_EQ("arraybuffer", websocket_scope.Socket().binaryType().AsString());

    websocket_scope.Socket().setBinaryType(V8BinaryType(V8BinaryType::Enum::kBlob));
...
INSTANTIATE_TEST_SUITE_P(DOMWebSocketValidClosing, DOMWebSocketValidClosingTest, testing::Values(1000, 3000, 3001, 4998, 4999));

class DOMWebSocketInvalidClosingCodeTest : public testing::TestWithParam<uint16_t> {
    test::TaskEnvironment task_environment_;
};
...
TEST_P(DOMWebSocketInvalidClosingCodeTest, test)
{
    ...
    EXPECT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ(DOMExceptionCode::kInvalidAccessError, scope.GetExceptionState().CodeAs<DOMExceptionCode>());
...
INSTANTIATE_TEST_SUITE_P(DOMWebSocketInvalidClosingCode, DOMWebSocketInvalidClosingCodeTest, testing::Values(0, 1, 998, 999, 1001, 2999, 5000, 9999, 65535));
