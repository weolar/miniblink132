...
class MockWebSocketChannelClient : public GarbageCollected<MockWebSocketChannelClient>, public WebSocketChannelClient {
public:
    ...
    void DidReceiveBinaryMessage(const Vector<base::span<const char>>& data) override
    {
        Vector<char> flatten;
    ...
    MOCK_METHOD3(DidClose, void(ClosingHandshakeCompletionStatus, uint16_t, const String&));
...
public:
    ...
    ~MockWebSocketHandshakeThrottle() override
    {
        Destructor();
    }

    MOCK_METHOD4(ThrottleHandshake, void(const WebURL&, const WebSecurityOrigin&, const WebSecurityOrigin&, WebSocketHandshakeThrottle::OnCompletion));
...
public:
    void SetUp() override
    {
        ...
        GetFrame().GetBrowserInterfaceBroker().SetBinderForTesting(
            mojom::blink::WebSocketConnector::Name_, WTF::BindRepeating(&WebSocketChannelImplTestBase::BindWebSocketConnector, GetWeakPtr()));
    ...
    {
        GetFrame().GetBrowserInterfaceBroker().SetBinderForTesting(mojom::blink::WebSocketConnector::Name_, {});
...
public:
    ...
    public:
        struct DataFrame final {
            ...
            {
                return std::tie(type, data_length) == std::tie(that.type, that.data_length);
            }
        };

        explicit TestWebSocket(mojo::PendingReceiver<network::mojom::blink::WebSocket> pending_receiver)
        ...
        {
            pending_send_data_frames_.clear();
        }
        bool IsStartReceivingCalled() const
        {
            return is_start_receiving_called_;
        }
        bool IsStartClosingHandshakeCalled() const
        {
            return is_start_closing_handshake_called_;
        }
        uint16_t GetClosingCode() const
        {
            return closing_code_;
        }
        const String& GetClosingReason() const
        {
    ...
    public:
        struct ConnectArgs {
            ConnectArgs(const KURL& url, const Vector<String>& protocols, const net::SiteForCookies& site_for_cookies, const String& user_agent,
        ...
        void Connect(const KURL& url, const Vector<String>& requested_protocols, const net::SiteForCookies& site_for_cookies, const String& user_agent,
            net::StorageAccessApiStatus storage_access_api_status, mojo::PendingRemote<network::mojom::blink::WebSocketHandshakeClient> handshake_client,
            const std::optional<base::UnguessableToken>& throttling_profile_id) override
        {
            connect_args_.push_back(ConnectArgs(url, requested_protocols, site_for_cookies, user_agent, std::move(handshake_client)));
        }

        const Vector<ConnectArgs>& GetConnectArgs() const
        {
            return connect_args_;
        }
        Vector<ConnectArgs> TakeConnectArgs()
        {
            return std::move(connect_args_);
        }

        void Bind(mojo::PendingReceiver<mojom::blink::WebSocketConnector> receiver)
        {
    ...
    explicit WebSocketChannelImplTest(std::unique_ptr<MockWebSocketHandshakeThrottle> handshake_throttle = nullptr)
    ...
    {
        ON_CALL(*ChannelClient(), DidConsumeBufferedAmount(_)).WillByDefault(Invoke(this, &WebSocketChannelImplTest::DidConsumeBufferedAmount));
    }

    ~WebSocketChannelImplTest() override
    {
        Channel()->Disconnect();
    }
    ...
    {
        connector_.Bind(mojo::PendingReceiver<mojom::blink::WebSocketConnector>(std::move(handle)));
    }

    MojoResult CreateDataPipe(uint32_t capacity, mojo::ScopedDataPipeProducerHandle* writable, mojo::ScopedDataPipeConsumerHandle* readable)
    {
        const MojoCreateDataPipeOptions data_pipe_options { sizeof(MojoCreateDataPipeOptions), MOJO_CREATE_DATA_PIPE_FLAG_NONE, 1, capacity };
        return mojo::CreateDataPipe(&data_pipe_options, *writable, *readable);
    }

    std::unique_ptr<TestWebSocket> EstablishConnection(network::mojom::blink::WebSocketHandshakeClient* handshake_client, const String& selected_protocol,
        const String& extensions, mojo::ScopedDataPipeConsumerHandle readable, mojo::ScopedDataPipeProducerHandle writable,
        mojo::Remote<network::mojom::blink::WebSocketClient>* client)
    {
        ...
        auto websocket = std::make_unique<TestWebSocket>(websocket_to_pass.InitWithNewPipeAndPassReceiver());
        ...
        handshake_client->OnConnectionEstablished(
            std::move(websocket_to_pass), client_remote.InitWithNewPipeAndPassReceiver(), std::move(response), std::move(readable), std::move(writable));
    ...
    void SetUp() override
    {
        ...
        channel_
            = WebSocketChannelImpl::CreateForTesting(GetFrame().DomWindow(), channel_client_.Get(), CaptureSourceLocation(), std::move(handshake_throttle_));
    }

    MockWebSocketChannelClient* ChannelClient()
    {
        return channel_client_.Get();
    }
    ...
    {
        Vector<uint8_t> v;
        v.Append(reinterpret_cast<const uint8_t*>(data), static_cast<wtf_size_t>(size));
        return v;
    }
    ...
    Vector<uint8_t> ReadDataFromDataPipe(mojo::ScopedDataPipeConsumerHandle& readable, size_t bytes_to_read)
    {
        base::span<const uint8_t> buffer;
        const MojoResult begin_result = readable->BeginReadData(MOJO_READ_DATA_FLAG_NONE, buffer);

        DCHECK_EQ(begin_result, MOJO_RESULT_OK);
        if (buffer.size() < bytes_to_read) {
            ADD_FAILURE() << "ReadDataFromDataPipe expected " << bytes_to_read << " bytes but only received " << buffer.size() << " bytes";
    ...
    std::unique_ptr<TestWebSocket> Connect(uint32_t capacity, mojo::ScopedDataPipeProducerHandle* writable, mojo::ScopedDataPipeConsumerHandle* readable,
        mojo::Remote<network::mojom::blink::WebSocketClient>* client)
    {
        ...
        mojo::Remote<network::mojom::blink::WebSocketHandshakeClient> handshake_client(std::move(connect_args[0].handshake_client));

        mojo::ScopedDataPipeConsumerHandle remote_readable;
        if (CreateDataPipe(capacity, writable, &remote_readable) != MOJO_RESULT_OK) {
            ADD_FAILURE() << "Failed to create a datapipe.";
            return nullptr;
        }

        mojo::ScopedDataPipeProducerHandle remote_writable;
        if (CreateDataPipe(capacity, &remote_writable, readable) != MOJO_RESULT_OK) {
            ADD_FAILURE() << "Failed to create a datapipe.";
            return nullptr;
        }
        auto websocket = EstablishConnection(handshake_client.get(), "", "", std::move(remote_readable), std::move(remote_writable), client);
        test::RunPendingTasks();
        return websocket;
    }
    ...
    const raw_ptr<MockWebSocketHandshakeThrottle, DanglingUntriaged> raw_handshake_throttle_;
...
std::ostream& operator<<(std::ostream& o, const WebSocketChannelImplTest::TestWebSocket::DataFrame& f)
{
...
TEST_F(WebSocketChannelImplTest, ConnectSuccess)
{
    ...
    EXPECT_TRUE(net::SiteForCookies::FromUrl(GURL("http://example.com/")).IsEquivalent(GetDocument().SiteForCookies()));
    ...
    EXPECT_TRUE(connect_args[0].site_for_cookies.IsEquivalent(net::SiteForCookies::FromUrl(GURL("http://example.com/"))));
    ...
    ASSERT_EQ(CreateDataPipe(32, &incoming_writable, &incoming_readable), MOJO_RESULT_OK);

    mojo::ScopedDataPipeProducerHandle outgoing_writable;
    mojo::ScopedDataPipeConsumerHandle outgoing_readable;
    ASSERT_EQ(CreateDataPipe(32, &outgoing_writable, &outgoing_readable), MOJO_RESULT_OK);

    mojo::Remote<network::mojom::blink::WebSocketClient> client;
    auto websocket = EstablishConnection(handshake_client.get(), "a", "b", std::move(incoming_readable), std::move(outgoing_writable), &client);
...
TEST_F(WebSocketChannelImplTest, MojoConnectionErrorDuringHandshake)
{
...

    {
        ...
        EXPECT_CALL(
            *ChannelClient(), DidClose(WebSocketChannelClient::kClosingHandshakeIncomplete, WebSocketChannel::kCloseEventCodeAbnormalClosure, String()));
    }
...
TEST_F(WebSocketChannelImplTest, SendText)
{
    ...
    EXPECT_EQ(websocket->GetDataFrames(),
        (DataFrames {
            { WebSocketMessageType::TEXT, strlen("foo") }, { WebSocketMessageType::TEXT, strlen("bar") }, { WebSocketMessageType::TEXT, strlen("baz") } }));
}
...
TEST_F(WebSocketChannelImplTest, SendBinaryInVector)
{
    ...
    DOMArrayBuffer* foo_buffer = DOMArrayBuffer::Create(base::byte_span_from_cstring("foo"));
    Channel()->Send(*foo_buffer, 0, 3, base::OnceClosure());
    test::RunPendingTasks();

    EXPECT_EQ(websocket->GetDataFrames(), (DataFrames { { WebSocketMessageType::BINARY, strlen("foo") } }));
...
TEST_F(WebSocketChannelImplTest, SendBinaryInArrayBufferPartial)
{
    ...
    DOMArrayBuffer* foobar_buffer = DOMArrayBuffer::Create(base::byte_span_from_cstring("foobar"));
    DOMArrayBuffer* qbazux_buffer = DOMArrayBuffer::Create(base::byte_span_from_cstring("qbazux"));
...
TEST_F(WebSocketChannelImplTest, SendBinaryInArrayBufferWithNullBytes)
{
    ...
    ASSERT_EQ(AsVector("\0ar", kLengthOfEachMessage), ReadDataFromDataPipe(readable, 3u));
    ASSERT_EQ(AsVector("b\0z", kLengthOfEachMessage), ReadDataFromDataPipe(readable, 3u));
    ASSERT_EQ(AsVector("qu\0", kLengthOfEachMessage), ReadDataFromDataPipe(readable, 3u));
    ASSERT_EQ(AsVector("\0\0\0", kLengthOfEachMessage), ReadDataFromDataPipe(readable, 3u));
}
...
TEST_F(WebSocketChannelImplTest, SendBinaryInArrayBufferNonLatin1UTF8)
{
    ...
    DOMArrayBuffer* b = DOMArrayBuffer::Create(base::byte_span_from_cstring("\xe7\x8b\x90"));
    Channel()->Send(*b, 0, 3, base::OnceClosure());

    test::RunPendingTasks();

    EXPECT_EQ(websocket->GetDataFrames(), (DataFrames { { WebSocketMessageType::BINARY, strlen("\xe7\x8b\x90") } }));
...
TEST_F(WebSocketChannelImplTest, SendBinaryInArrayBufferNonUTF8)
{
    ...
    DOMArrayBuffer* b = DOMArrayBuffer::Create(base::byte_span_from_cstring("\x80\xff\xe7"));
    Channel()->Send(*b, 0, 3, base::OnceClosure());

    test::RunPendingTasks();

    EXPECT_EQ(websocket->GetDataFrames(), (DataFrames { { WebSocketMessageType::BINARY, strlen("\x80\xff\xe7") } }));
...
TEST_F(WebSocketChannelImplTest, SendTextSync)
{
    ...
    EXPECT_EQ(WebSocketChannel::SendResult::kSentSynchronously, Channel()->Send("hello", closure.Closure()));
...
TEST_F(WebSocketChannelImplTest, SendTextAsyncDueToQueueing)
{
    ...
    CallTrackingClosure closure;
    EXPECT_EQ(WebSocketChannel::SendResult::kCallbackWillBeCalled, Channel()->Send(long_message, closure.Closure()));
...
TEST_F(WebSocketChannelImplTest, SendBinaryInArrayBufferSync)
{
    ...
    EXPECT_EQ(WebSocketChannel::SendResult::kSentSynchronously, Channel()->Send(*b, 0, 5, closure.Closure()));
...
TEST_F(WebSocketChannelImplTest, SendBinaryInArrayBufferAsyncDueToQueueing)
{
    ...
    EXPECT_EQ(WebSocketChannel::SendResult::kCallbackWillBeCalled, Channel()->Send(*b, 0, kMessageSize, closure.Closure()));
...
TEST_F(WebSocketChannelImplTest, ReceiveText)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData(base::byte_span_from_cstring("FOOBAR"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
...
TEST_F(WebSocketChannelImplTest, ReceiveTextContinuation)
{
    ...
    size_t actually_written_bytes = 0;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData(base::byte_span_from_cstring("BAZ"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
...
{
    {
        ...
        EXPECT_CALL(*ChannelClient(), DidReceiveTextMessage(String(non_latin1_string)));
    }
    ...
    size_t actually_written_bytes = 0;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData(base::byte_span_from_cstring("\xe7\x8b\x90\xe0\xa4\x94"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
    EXPECT_EQ(actually_written_bytes, 6u);
...
{
    {
        InSequence s;
        EXPECT_CALL(*ChannelClient(), DidConnect(_, _));
        EXPECT_CALL(*ChannelClient(), DidReceiveBinaryMessageMock((Vector<char> { 'F', 'O', 'O' })));
    }
    ...
    size_t actually_written_bytes = 0;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData(base::byte_span_from_cstring("FOO"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
    EXPECT_EQ(actually_written_bytes, 3u);
...
{
    {
        InSequence s;
        EXPECT_CALL(*ChannelClient(), DidConnect(_, _));
        EXPECT_CALL(*ChannelClient(), DidReceiveBinaryMessageMock((Vector<char> { 'B', 'A', 'Z' })));
    }
...
{
    {
        InSequence s;
        EXPECT_CALL(*ChannelClient(), DidConnect(_, _));
        EXPECT_CALL(*ChannelClient(), DidReceiveBinaryMessageMock((Vector<char> { '\0', 'A', '3' })));
        EXPECT_CALL(*ChannelClient(), DidReceiveBinaryMessageMock((Vector<char> { 'B', '\0', 'Z' })));
        EXPECT_CALL(*ChannelClient(), DidReceiveBinaryMessageMock((Vector<char> { 'Q', 'U', '\0' })));
        EXPECT_CALL(*ChannelClient(), DidReceiveBinaryMessageMock((Vector<char> { '\0', '\0', '\0' })));
    }
    ...
    size_t actually_written_bytes = 0;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData(base::as_byte_span("\0A3B\0ZQU\0\0\0\0"sv), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
...
{
    {
        InSequence s;
        EXPECT_CALL(*ChannelClient(), DidConnect(_, _));
        EXPECT_CALL(*ChannelClient(), DidReceiveBinaryMessageMock((Vector<char> { '\xe7', '\x8b', '\x90', '\xe0', '\xa4', '\x94' })));
    }
    mojo::ScopedDataPipeProducerHandle writable;
    mojo::ScopedDataPipeConsumerHandle readable;
    mojo::Remote<network::mojom::blink::WebSocketClient> client;
...
{
    {
        InSequence s;
        EXPECT_CALL(*ChannelClient(), DidConnect(_, _));
        EXPECT_CALL(*ChannelClient(), DidReceiveBinaryMessageMock((Vector<char> { '\x80', '\xff' })));
    }
    ...
    size_t actually_written_bytes = 0;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData(base::byte_span_from_cstring("\x80\xff"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
...
TEST_F(WebSocketChannelImplTest, ReceiveWithExplicitBackpressure)
{
    ...
    size_t actually_written_bytes = 0;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData(base::byte_span_from_cstring("abc"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
    EXPECT_EQ(actually_written_bytes, 3u);
...
TEST_F(WebSocketChannelImplTest, ReceiveMultipleMessagesWithSmallDataPipeWrites)
{
    Checkpoint checkpoint;
    {
        InSequence s;
    ...
    checkpoint.Call(1);
    size_t actually_written_bytes = 0;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData(base::byte_span_from_cstring("ab"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
    EXPECT_EQ(actually_written_bytes, 2u);
    test::RunPendingTasks();

    checkpoint.Call(2);
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData(base::byte_span_from_cstring("cd"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
    EXPECT_EQ(actually_written_bytes, 2u);
    test::RunPendingTasks();

    checkpoint.Call(3);
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData(base::byte_span_from_cstring("efgh"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
    ...
    checkpoint.Call(6);
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData(base::byte_span_from_cstring("ijkl"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
    EXPECT_EQ(actually_written_bytes, 4u);
    test::RunPendingTasks();
}
...
TEST_F(WebSocketChannelImplTest, ConnectionCloseInitiatedByServer)
{
    Checkpoint checkpoint;
    {
        ...
        EXPECT_CALL(*ChannelClient(),
            DidClose(WebSocketChannelClient::kClosingHandshakeComplete, WebSocketChannel::kCloseEventCodeNormalClosure, String("close reason")));
    }
    ...
    checkpoint.Call(1);
    Channel()->Close(WebSocketChannel::kCloseEventCodeNormalClosure, "close reason");
    test::RunPendingTasks();

    EXPECT_TRUE(websocket->IsStartClosingHandshakeCalled());
    EXPECT_EQ(websocket->GetClosingCode(), WebSocketChannel::kCloseEventCodeNormalClosure);
    EXPECT_EQ(websocket->GetClosingReason(), "close reason");

    checkpoint.Call(2);
    client->OnDropChannel(true, WebSocketChannel::kCloseEventCodeNormalClosure, "close reason");
    test::RunPendingTasks();
}
...
TEST_F(WebSocketChannelImplTest, FailFromClient)
{
    ...
    Channel()->Fail("fail message from WebSocket", mojom::ConsoleMessageLevel::kError, std::make_unique<SourceLocation>(String(), String(), 0, 0, nullptr));
    checkpoint.Call(1);
...
class WebSocketChannelImplHandshakeThrottleTest : public WebSocketChannelImplTest {
public:
    WebSocketChannelImplHandshakeThrottleTest()
        : WebSocketChannelImplTest(std::make_unique<StrictMock<MockWebSocketHandshakeThrottle>>())
    {
    }

    static KURL url()
    {
        return KURL("ws://localhost/");
    }
};
...
TEST_F(WebSocketChannelImplHandshakeThrottleTest, ThrottleSucceedsFirst)
{
    ...
    auto websocket = EstablishConnection(handshake_client.get(), "", "", std::move(readable), std::move(outgoing_writable), &client);
    test::RunPendingTasks();
}
...
TEST_F(WebSocketChannelImplHandshakeThrottleTest, FailDuringThrottle)
{
    ...
    Channel()->Connect(url(), "");
    Channel()->Fail("close during handshake", mojom::ConsoleMessageLevel::kWarning, std::make_unique<SourceLocation>(String(), String(), 0, 0, nullptr));
    checkpoint.Call(1);
    test::RunPendingTasks();
    checkpoint.Call(2);
}
...
// connected.
TEST_F(WebSocketChannelImplHandshakeThrottleTest, FailDuringThrottleAfterConnect)
{
    Checkpoint checkpoint;
    {
        InSequence s;
...
TEST_F(WebSocketChannelImplHandshakeThrottleTest, DisconnectDuringThrottleAfterConnect)
{
    Checkpoint checkpoint;
    {
        InSequence s;
...
TEST_F(WebSocketChannelImplHandshakeThrottleTest, ThrottleReportsErrorBeforeConnect)
{
    Checkpoint checkpoint;
    {
        InSequence s;
...
TEST_F(WebSocketChannelImplHandshakeThrottleTest, ThrottleReportsErrorAfterConnect)
{
    Checkpoint checkpoint;
    {
        InSequence s;
...
public:
    MOCK_METHOD(void, Connect,
        (const KURL&, const Vector<String>&, const net::SiteForCookies&, const String&, net::StorageAccessApiStatus,
            mojo::PendingRemote<network::mojom::blink::WebSocketHandshakeClient>, const std::optional<base::UnguessableToken>&));
};
...
public:
    ...
    void BindWebSocketConnector(mojo::ScopedMessagePipeHandle handle) override
    {
        connector_receiver_set_.Add(&connector_, mojo::PendingReceiver<mojom::blink::WebSocketConnector>(std::move(handle)));
    }
...
protected:
    ...
    base::WeakPtrFactory<WebSocketChannelImplMultipleTest> weak_ptr_factory_ { this };
};
...
TEST_F(WebSocketChannelImplMultipleTest, ConnectionLimit)
{
    ...
    // connection failures.
    mojo::RemoteSet<network::mojom::blink::WebSocketHandshakeClient> handshake_clients;
    auto handshake_client_add_action
        = [&handshake_clients](Unused, Unused, Unused, Unused, Unused, mojo::PendingRemote<network::mojom::blink::WebSocketHandshakeClient> handshake_client,
              Unused) { handshake_clients.Add(std::move(handshake_client)); };

    auto failure_handshake_throttle = std::make_unique<StrictMock<MockWebSocketHandshakeThrottle>>();
    auto* failure_channel_client = MockWebSocketChannelClient::Create();

    auto successful_handshake_throttle = std::make_unique<StrictMock<MockWebSocketHandshakeThrottle>>();
...

    {
        ...
        EXPECT_CALL(
            *failure_channel_client, DidClose(WebSocketChannelClient::kClosingHandshakeIncomplete, WebSocketChannel::kCloseEventCodeAbnormalClosure, String()));
        EXPECT_CALL(*failure_handshake_throttle, Destructor());

        EXPECT_CALL(checkpoint, Call(2));

        EXPECT_CALL(*successful_handshake_throttle, ThrottleHandshake(_, _, _, _));
        EXPECT_CALL(connector_, Connect(_, _, _, _, _, _, _)).WillOnce(handshake_client_add_action);
        EXPECT_CALL(*successful_handshake_throttle, Destructor());
    }

    WebSocketChannelImpl* channels[WebSocketChannelImpl::kMaxWebSocketsPerRenderProcess] = {};
    for (WebSocketChannelImpl*& channel : channels) {
        auto handshake_throttle = std::make_unique<StrictMock<MockWebSocketHandshakeThrottle>>();
        ...
        channel = WebSocketChannelImpl::CreateForTesting(GetFrame().DomWindow(), channel_client, CaptureSourceLocation(), std::move(handshake_throttle));
        channel->Connect(url, "");
    }
    ...
    auto* failing_channel = WebSocketChannelImpl::CreateForTesting(
        GetFrame().DomWindow(), failure_channel_client, CaptureSourceLocation(), std::move(failure_handshake_throttle));
    ...
    auto* successful_channel = WebSocketChannelImpl::CreateForTesting(
        GetFrame().DomWindow(), successful_channel_client, CaptureSourceLocation(), std::move(successful_handshake_throttle));
