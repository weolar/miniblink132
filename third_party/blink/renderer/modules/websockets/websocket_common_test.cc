...
TEST(WebSocketCommonTest, IsValidSubprotocolString)
{
    ...
    const String valid_characters("!#$%&'*+-.0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ^_`"
                                  "abcdefghijklmnopqrstuvwxyz|~");
    for (wtf_size_t i = 0; i < valid_characters.length(); ++i) {
        EXPECT_TRUE(WebSocketCommon::IsValidSubprotocolString(valid_characters.Substring(i, 1u)));
    }
...
TEST(WebSocketCommonTest, JoinStrings)
{
    ...
    EXPECT_EQ("a\r\nbcd\r\nef", WebSocketCommon::JoinStrings({ "a", "bcd", "ef" }, "\r\n"));
    EXPECT_EQ("|||", WebSocketCommon::JoinStrings({ "|", "|" }, "|"));
