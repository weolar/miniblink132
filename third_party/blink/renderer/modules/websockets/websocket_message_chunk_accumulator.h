...
// manner.
class MODULES_EXPORT WebSocketMessageChunkAccumulator final : public GarbageCollected<WebSocketMessageChunkAccumulator> {
public:
    explicit WebSocketMessageChunkAccumulator(scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    wtf_size_t GetPoolSizeForTesting() const
    {
        return pool_.size();
    }
    bool IsTimerActiveForTesting() const
    {
...
private:
    ...
    {
        return SegmentPtr(static_cast<char*>(WTF::Partitions::FastMalloc(kSegmentSize, "blink::WebSocketMessageChunkAccumulator::Segment")));
    }
