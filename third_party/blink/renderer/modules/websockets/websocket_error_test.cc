...
public:
    ...
    static WebSocketError* CreateError(
        std::optional<uint16_t> close_code = std::nullopt, String reason = String(), ExceptionState& exception_state = ASSERT_NO_EXCEPTION)
    {
...
{
    auto* error = WebSocketError::Create("", WebSocketCloseInfo::Create(), ASSERT_NO_EXCEPTION);
...
{
    auto* error = WebSocketError::Create("hello", WebSocketCloseInfo::Create(), ASSERT_NO_EXCEPTION);
...
TEST_F(WebSocketErrorTest, ConstructWithInvalidCloseCode)
{
    ...
    EXPECT_EQ("The close code must be either 1000, or between 3000 and 4999. 1005 is "
              "neither.",
        exception_state.Message());
    EXPECT_EQ(DOMExceptionCode::kInvalidAccessError, DOMExceptionCode { exception_state.Code() });
}
...
TEST_F(WebSocketErrorTest, ConstructWithOverlongReason)
{
    ...
    auto* error = CreateError(std::nullopt, builder.ReleaseString(), exception_state);
    EXPECT_FALSE(error);
    ASSERT_TRUE(exception_state.HadException());
    EXPECT_EQ("The close reason must not be greater than 123 UTF-8 bytes.", exception_state.Message());
    EXPECT_EQ(DOMExceptionCode::kSyntaxError, DOMExceptionCode { exception_state.Code() });
}
...
TEST_F(WebSocketErrorTest, InternalCreate)
{
    ...
    ASSERT_TRUE(v8value.As<v8::Object>()->Get(context, V8String(isolate, "stack")).ToLocal(&stack));
