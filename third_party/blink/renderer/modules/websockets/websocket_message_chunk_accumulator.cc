...
WebSocketMessageChunkAccumulator::WebSocketMessageChunkAccumulator(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : timer_(std::move(task_runner), this, &WebSocketMessageChunkAccumulator::OnTimerFired)
{
}

WebSocketMessageChunkAccumulator::~WebSocketMessageChunkAccumulator() = default;

void WebSocketMessageChunkAccumulator::SetTaskRunnerForTesting(scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TickClock* tick_clock)
{
...
{
    if (!segments_.empty()) {
        const size_t to_be_written = std::min(data.size(), kSegmentSize - GetLastSegmentSize());
        base::ranges::copy(data.first(to_be_written), segments_.back().get() + GetLastSegmentSize());
...
Vector<base::span<const char>> WebSocketMessageChunkAccumulator::GetView() const
{
...
{
    num_pooled_segments_to_be_removed_ = std::min(num_pooled_segments_to_be_removed_, pool_.size());
    size_ = 0;
...
void WebSocketMessageChunkAccumulator::OnTimerFired(TimerBase*)
{
    ...
    const auto to_be_removed = std::min(num_pooled_segments_to_be_removed_, pool_.size());
