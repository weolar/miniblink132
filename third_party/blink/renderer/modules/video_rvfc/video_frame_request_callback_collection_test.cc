...
class MockVideoFrameCallback : public VideoFrameRequestCallbackCollection::VideoFrameCallback {
public:
...
public:
    ...
    VideoFrameRequestCallbackCollectionTest()
        ...
        , collection_(MakeGarbageCollected<VideoFrameRequestCallbackCollection>(execution_context_.Get()))
    {
    }
...
TEST_F(VideoFrameRequestCallbackCollectionTest, CancelSingleCallback)
{
    ...
    collection()->ExecuteFrameCallbacks(kDefaultTimestamp, VideoFrameCallbackMetadata::Create());
    EXPECT_TRUE(collection()->IsEmpty());
}
...
TEST_F(VideoFrameRequestCallbackCollectionTest, CreateCallbackDuringExecution)
{
    ...
    auto callback = CreateCallback();
    EXPECT_CALL(*callback, Invoke(_, _)).WillOnce(testing::WithoutArgs(testing::Invoke([&]() {
...
TEST_F(VideoFrameRequestCallbackCollectionTest, CancelCallbackDuringExecution)
{
    ...
    CallbackId dummy_callback_id = collection()->RegisterFrameCallback(dummy_callback.Get());
    ...
    EXPECT_CALL(*cancelling_callback, Invoke(_, _)).WillOnce(testing::WithoutArgs(testing::Invoke([&]() {
    ...
    CallbackId target_callback_id = collection()->RegisterFrameCallback(target_callback.Get());
