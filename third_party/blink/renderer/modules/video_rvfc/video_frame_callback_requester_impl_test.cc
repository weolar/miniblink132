...
using VideoFramePresentationMetadata = WebMediaPlayer::VideoFramePresentationMetadata;
...
public:
    ...
    MOCK_METHOD0(GetVideoFramePresentationMetadata, std::unique_ptr<VideoFramePresentationMetadata>());
};
...
class VfcRequesterParameterVerifierCallback : public VideoFrameRequestCallbackCollection::VideoFrameCallback {
public:
    ...
    void Invoke(double now, const VideoFrameCallbackMetadata* metadata) override
    {
        ...
        VerifyTicksClamping(expected.presentation_time, metadata->presentationTime(), "presentation_time");
        VerifyTicksClamping(expected.expected_display_time, metadata->expectedDisplayTime(), "expected_display_time");

        VerifyTicksClamping(*expected.metadata.capture_begin_time, metadata->captureTime(), "capture_time");

        VerifyTicksClamping(*expected.metadata.receive_time, metadata->receiveTime(), "receive_time");

        base::TimeDelta processing_time = *expected.metadata.processing_time;
        EXPECT_EQ(ClampElapsedProcessingTime(processing_time), metadata->processingDuration());
        EXPECT_NE(processing_time.InSecondsF(), metadata->processingDuration());
    }

    double last_now() const
    {
        return now_;
    }
...
private:
    void VerifyTicksClamping(base::TimeTicks reference, double actual, std::string name)
    {
        EXPECT_EQ(TicksToClampedMillisecondsF(reference), actual) << name << " was not clamped properly.";
        EXPECT_NE(TicksToMillisecondsF(reference), actual) << "Did not successfully test clamping for " << name;
    }

    double TicksToClampedMillisecondsF(base::TimeTicks ticks)
    {
        return Performance::ClampTimeResolution(loader_->GetTiming().MonotonicTimeToZeroBasedDocumentTime(ticks),
            /*cross_origin_isolated_capability_=*/false);
    }

    double TicksToMillisecondsF(base::TimeTicks ticks)
    {
        return loader_->GetTiming().MonotonicTimeToZeroBasedDocumentTime(ticks).InMillisecondsF();
    }
...
public:
    virtual void SetUpWebMediaPlayer()
    {
        ...
        SetupPageWithClients(nullptr, MakeGarbageCollected<test::MediaStubLocalFrameClient>(std::move(mock_media_player)), nullptr);
    }
    ...
    MockWebMediaPlayer* media_player()
    {
    ...
    void SimulateFramePresented()
    {
        video_->OnRequestVideoFrameCallback();
    }

    void SimulateVideoFrameCallback(base::TimeTicks now)
    {
        PageAnimator::ServiceScriptedAnimations(now, { { GetDocument().GetScriptedAnimationController(), false } });
    }

    V8VideoFrameRequestCallback* GetCallback(ScriptState* script_state, MockFunction* function)
    {
        return V8VideoFrameRequestCallback::Create(function->ToV8Function(script_state));
    }

    void RegisterCallbackDirectly(VfcRequesterParameterVerifierCallback* callback)
    {
...
class VideoFrameCallbackRequesterImplNullMediaPlayerTest : public VideoFrameCallbackRequesterImplTest {
public:
    void SetUpWebMediaPlayer() override
    {
        SetupPageWithClients(nullptr,
            MakeGarbageCollected<test::MediaStubLocalFrameClient>(std::unique_ptr<MockWebMediaPlayer>(),
...
TEST_F(VideoFrameCallbackRequesterImplTest, VerifyRequestVideoFrameCallback)
{
    ...
    vfc_requester().requestVideoFrameCallback(GetCallback(scope.GetScriptState(), function));
    ...
    EXPECT_CALL(*media_player(), GetVideoFramePresentationMetadata()).WillOnce(Return(ByMove(std::move(metadata))));
    SimulateVideoFrameCallback(base::TimeTicks::Now());

    testing::Mock::VerifyAndClear(function);
}

TEST_F(VideoFrameCallbackRequesterImplTest, VerifyCancelVideoFrameCallback_BeforePresentedFrame)
{
    V8TestingScope scope;
    ...
    int callback_id = vfc_requester().requestVideoFrameCallback(GetCallback(scope.GetScriptState(), function));
...
TEST_F(VideoFrameCallbackRequesterImplTest, VerifyCancelVideoFrameCallback_AfterPresentedFrame)
{
    V8TestingScope scope;
...
TEST_F(VideoFrameCallbackRequesterImplTest, VerifyClearedMediaPlayerCancelsPendingExecution)
{
    V8TestingScope scope;
...
TEST_F(VideoFrameCallbackRequesterImplTest, VerifyParameters_WindowRaf)
{
    ...
    auto* callback = MakeGarbageCollected<VfcRequesterParameterVerifierCallback>(loader);

    // Register the non-V8 callback.
    RegisterCallbackDirectly(callback);

    EXPECT_CALL(*media_player(), GetVideoFramePresentationMetadata()).WillOnce(Return(ByMove(MetadataHelper::CopyDefaultMedatada())));

    const double now_ms = timing.MonotonicTimeToZeroBasedDocumentTime(base::TimeTicks::Now()).InMillisecondsF();
