...
class MODULES_EXPORT VideoFrameCallbackRequesterImpl final : public VideoFrameCallbackRequester, public XRFrameProvider::ImmersiveSessionObserver {
public:
    static VideoFrameCallbackRequesterImpl& From(HTMLVideoElement&);

    // Web API entry points for requestAnimationFrame().
    static int requestVideoFrameCallback(HTMLVideoElement&, V8VideoFrameRequestCallback*);
    static void cancelVideoFrameCallback(HTMLVideoElement&, int);

    explicit VideoFrameCallbackRequesterImpl(HTMLVideoElement&);

    VideoFrameCallbackRequesterImpl(const VideoFrameCallbackRequesterImpl&) = delete;
    VideoFrameCallbackRequesterImpl& operator=(const VideoFrameCallbackRequesterImpl&) = delete;
...
private:
    ...
    // reasons.
    static double GetClampedTimeInMillis(base::TimeDelta time, bool cross_origin_isolated_capability);
    static double GetCoarseClampedTimeInSeconds(base::TimeDelta time);

    void ExecuteVideoFrameCallbacks(double high_res_now_ms, std::unique_ptr<WebMediaPlayer::VideoFramePresentationMetadata>);
    ...
    void RegisterCallbackForTest(VideoFrameRequestCallbackCollection::VideoFrameCallback*);
