...
TEST(WakeLockSentinelTest, SentinelType)
{
    ...
    auto* sentinel = MakeGarbageCollected<WakeLockSentinel>(context.GetScriptState(), V8WakeLockType::Enum::kScreen,
        /*manager=*/nullptr);
    EXPECT_EQ("screen", sentinel->type().AsString());

    sentinel = MakeGarbageCollected<WakeLockSentinel>(context.GetScriptState(), V8WakeLockType::Enum::kSystem,
        /*manager=*/nullptr);
...
TEST(WakeLockSentinelTest, SentinelReleased)
{
    ...
    auto* manager = MakeGarbageCollected<WakeLockManager>(context.DomWindow(), V8WakeLockType::Enum::kScreen);
    auto* sentinel = MakeGarbageCollected<WakeLockSentinel>(context.GetScriptState(), V8WakeLockType::Enum::kScreen, manager);
    EXPECT_FALSE(sentinel->released());

    manager = MakeGarbageCollected<WakeLockManager>(context.DomWindow(), V8WakeLockType::Enum::kSystem);
    sentinel = MakeGarbageCollected<WakeLockSentinel>(context.GetScriptState(), V8WakeLockType::Enum::kSystem, manager);
    EXPECT_FALSE(sentinel->released());
}
...
TEST(WakeLockSentinelTest, MultipleReleaseCalls)
{
    ...
    auto* manager = MakeGarbageCollected<WakeLockManager>(context.DomWindow(), V8WakeLockType::Enum::kScreen);
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<WakeLockSentinel>>(context.GetScriptState());
    auto promise = resolver->Promise();
    manager->AcquireWakeLock(resolver);
    context.WaitForPromiseFulfillment(promise);
    auto* sentinel = ScriptPromiseUtils::GetPromiseResolutionAsWakeLockSentinel(context.GetScriptState()->GetIsolate(), promise);
    ASSERT_NE(nullptr, sentinel);
    EXPECT_FALSE(sentinel->released());

    base::RunLoop run_loop;
    auto* event_listener = MakeGarbageCollected<SyncEventListener>(run_loop.QuitClosure());
    ...
    event_listener = MakeGarbageCollected<SyncEventListener>(WTF::BindOnce([]() { EXPECT_TRUE(false) << "This event handler should not be reached."; }));
    sentinel->addEventListener(event_type_names::kRelease, event_listener);
    sentinel->release(context.GetScriptState());
    EXPECT_TRUE(sentinel->released());
}
...
TEST(WakeLockSentinelTest, ContextDestruction)
{
    ...
    WakeLockManager* manager = wake_lock->managers_[static_cast<size_t>(V8WakeLockType::Enum::kScreen)];
    ASSERT_TRUE(manager);

    context.WaitForPromiseFulfillment(screen_promise);
    auto* sentinel = ScriptPromiseUtils::GetPromiseResolutionAsWakeLockSentinel(context.GetScriptState()->GetIsolate(), screen_promise);
    ASSERT_TRUE(sentinel);

    auto* event_listener = MakeGarbageCollected<SyncEventListener>(WTF::BindOnce([]() { EXPECT_TRUE(false) << "This event handler should not be reached."; }));
    sentinel->addEventListener(event_type_names::kRelease, event_listener);
