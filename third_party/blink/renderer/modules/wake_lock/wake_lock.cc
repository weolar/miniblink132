...
WakeLock::WakeLock(NavigatorBase& navigator)
    ...
    , PageVisibilityObserver(navigator.DomWindow() ? navigator.DomWindow()->GetFrame()->GetPage() : nullptr)
    , permission_service_(navigator.GetExecutionContext())
    , managers_ { MakeGarbageCollected<WakeLockManager>(navigator.GetExecutionContext(), V8WakeLockType::Enum::kScreen),
        MakeGarbageCollected<WakeLockManager>(navigator.GetExecutionContext(), V8WakeLockType::Enum::kSystem) }
{
}

ScriptPromise<WakeLockSentinel> WakeLock::request(ScriptState* script_state, V8WakeLockType type, ExceptionState& exception_state)
{
    ...
    if (!script_state->ContextIsValid()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "The document has no associated browsing context");
        return EmptyPromise();
    }
    ...
    if (type == V8WakeLockType::Enum::kSystem && !RuntimeEnabledFeatures::SystemWakeLockEnabled()) {
        exception_state.ThrowTypeError("The provided value 'system' is not a valid enum value of type "
                                       "WakeLockType.");
        return EmptyPromise();
    }
    ...
    if (type == V8WakeLockType::Enum::kScreen
        && !context->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kScreenWakeLock, ReportOptions::kReportOnFailure)) {
    ...
    if (context->IsDedicatedWorkerGlobalScope()) {
        ...
        if (type == V8WakeLockType::Enum::kScreen) {
            exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "Screen locks cannot be requested from workers");
            return EmptyPromise();
        }
        ...
        if (!window->document()->IsActive()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "The document is not active");
            return EmptyPromise();
        }
        ...
        if (type == V8WakeLockType::Enum::kScreen && !window->GetFrame()->GetPage()->IsPageVisible()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "The requesting page is not visible");
            return EmptyPromise();
        }

        // Measure calls without sticky activation as proposed in
        // https://github.com/w3c/screen-wake-lock/pull/351.
        if (type == V8WakeLockType::Enum::kScreen && !window->GetFrame()->HasStickyUserActivation()) {
            UseCounter::Count(context, WebFeature::kWakeLockAcquireScreenLockWithoutStickyActivation);
        }
    }

    // 7. Let promise be a new promise.
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<WakeLockSentinel>>(script_state, exception_state.GetContext());
...
void WakeLock::DoRequest(V8WakeLockType::Enum type, ScriptPromiseResolver<WakeLockSentinel>* resolver)
{
    // https://w3c.github.io/screen-wake-lock/#the-request-method
    ...
    GetPermissionService()->RequestPermission(CreatePermissionDescriptor(permission_name), LocalFrame::HasTransientUserActivation(local_frame),
        resolver->WrapCallbackInScriptScope(WTF::BindOnce(&WakeLock::DidReceivePermissionResponse, WrapPersistent(this), type)));
}

void WakeLock::DidReceivePermissionResponse(V8WakeLockType::Enum type, ScriptPromiseResolver<WakeLockSentinel>* resolver, mojom::blink::PermissionStatus status)
{
    // https://w3c.github.io/screen-wake-lock/#the-request-method
    ...
    if (status != mojom::blink::PermissionStatus::GRANTED) {
        resolver->Reject(V8ThrowDOMException::CreateOrDie(
            resolver->GetScriptState()->GetIsolate(), DOMExceptionCode::kNotAllowedError, "Wake Lock permission request denied"));
        return;
    }
    // 8.3. Queue a global task on the screen wake lock task source given
    //      document's relevant global object to run these steps:
    if (type == V8WakeLockType::Enum::kScreen && !(GetPage() && GetPage()->IsPageVisible())) {
        ...
        resolver->Reject(V8ThrowDOMException::CreateOrDie(
            resolver->GetScriptState()->GetIsolate(), DOMExceptionCode::kNotAllowedError, "The requesting page is not visible"));
        return;
    }
...
void WakeLock::PageVisibilityChanged()
{
    ...
    // 1. For each lock in document.[[ActiveLocks]]["screen"]:
    // 1.1. Run release a wake lock with document, lock, and "screen".
    WakeLockManager* manager = managers_[static_cast<size_t>(V8WakeLockType::Enum::kScreen)];
    if (manager)
        manager->ClearWakeLocks();
}
...
PermissionService* WakeLock::GetPermissionService()
{
    if (!permission_service_.is_bound()) {
        ConnectToPermissionService(
            GetExecutionContext(), permission_service_.BindNewPipeAndPassReceiver(GetExecutionContext()->GetTaskRunner(TaskType::kWakeLock)));
    }
