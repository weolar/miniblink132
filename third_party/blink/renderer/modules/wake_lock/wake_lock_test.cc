...
TEST(WakeLockTest, RequestWakeLockGranted)
{
    ...
    context.GetPermissionService().SetPermissionResponse(V8WakeLockType::Enum::kScreen, mojom::blink::PermissionStatus::GRANTED);

    auto* screen_resolver = MakeGarbageCollected<ScriptPromiseResolver<WakeLockSentinel>>(context.GetScriptState());
    ...
    MockWakeLock& screen_lock = wake_lock_service.get_wake_lock(V8WakeLockType::Enum::kScreen);
    ...
    EXPECT_NE(nullptr, ScriptPromiseUtils::GetPromiseResolutionAsWakeLockSentinel(context.GetScriptState()->GetIsolate(), screen_promise));
...
TEST(WakeLockTest, RequestWakeLockDenied)
{
    ...
    context.GetPermissionService().SetPermissionResponse(V8WakeLockType::Enum::kSystem, mojom::blink::PermissionStatus::DENIED);

    auto* system_resolver = MakeGarbageCollected<ScriptPromiseResolver<WakeLockSentinel>>(context.GetScriptState());
    auto system_promise = system_resolver->Promise();

    auto* wake_lock = WakeLock::wakeLock(*context.DomWindow()->navigator());
    wake_lock->DoRequest(V8WakeLockType::Enum::kSystem, system_resolver);

    MockWakeLock& system_lock = wake_lock_service.get_wake_lock(V8WakeLockType::Enum::kSystem);
    MockPermissionService& permission_service = context.GetPermissionService();

    permission_service.WaitForPermissionRequest(V8WakeLockType::Enum::kSystem);
    context.WaitForPromiseRejection(system_promise);

    EXPECT_EQ(v8::Promise::kRejected, ScriptPromiseUtils::GetPromiseState(system_promise));
    ...
    DOMException* dom_exception = ScriptPromiseUtils::GetPromiseResolutionAsDOMException(context.GetScriptState()->GetIsolate(), system_promise);
...
TEST(WakeLockTest, LossOfDocumentActivity)
{
    ...
    MockWakeLock& system_lock = wake_lock_service.get_wake_lock(V8WakeLockType::Enum::kSystem);
    context.GetPermissionService().SetPermissionResponse(V8WakeLockType::Enum::kScreen, mojom::blink::PermissionStatus::GRANTED);
    context.GetPermissionService().SetPermissionResponse(V8WakeLockType::Enum::kSystem, mojom::blink::PermissionStatus::GRANTED);

    // First, acquire a handful of locks of different types.
    auto* screen_resolver1 = MakeGarbageCollected<ScriptPromiseResolver<WakeLockSentinel>>(context.GetScriptState());
    auto* screen_resolver2 = MakeGarbageCollected<ScriptPromiseResolver<WakeLockSentinel>>(context.GetScriptState());
    auto* system_resolver1 = MakeGarbageCollected<ScriptPromiseResolver<WakeLockSentinel>>(context.GetScriptState());
...
TEST(WakeLockTest, PageVisibilityHidden)
{
    ...
    context.Frame()->GetPage()->SetVisibilityState(mojom::blink::PageVisibilityState::kVisible, false);

    auto* other_resolver = MakeGarbageCollected<ScriptPromiseResolver<WakeLockSentinel>>(context.GetScriptState());
...
TEST(WakeLockTest, PageVisibilityHiddenBeforeLockAcquisition)
{
    ...
    EXPECT_EQ(v8::Promise::kRejected, ScriptPromiseUtils::GetPromiseState(screen_promise));
    DOMException* dom_exception = ScriptPromiseUtils::GetPromiseResolutionAsDOMException(context.GetScriptState()->GetIsolate(), screen_promise);
    ASSERT_NE(dom_exception, nullptr);
    EXPECT_EQ("NotAllowedError", dom_exception->name());
