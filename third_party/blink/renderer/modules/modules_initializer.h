...
private:
    ...
    PictureInPictureController* CreatePictureInPictureController(Document&) const override;
    void InitInspectorAgentSession(DevToolsSession*, bool, InspectorDOMAgent*, InspectedFrames*, Page*) const override;
    std::unique_ptr<WebMediaPlayer> CreateWebMediaPlayer(
        WebLocalFrameClient*, HTMLMediaElement&, const WebMediaPlayerSource&, WebMediaPlayerClient*) const override;
    RemotePlaybackClient* CreateRemotePlaybackClient(HTMLMediaElement&) const override;

    void ProvideModulesToPage(Page&, const SessionStorageNamespaceId&) const override;
    ...
    void CloneSessionStorage(Page* clone_from_page, const SessionStorageNamespaceId& clone_to_namespace) override;
    ...
    void SetLocalStorageArea(LocalFrame& frame, mojo::PendingRemote<mojom::blink::StorageArea> local_storage_area) override;
    void SetSessionStorageArea(LocalFrame& frame, mojo::PendingRemote<mojom::blink::StorageArea> session_storage_area) override;
    mojom::blink::FileSystemManager& GetFileSystemManager(ExecutionContext* context) override;
};
