...
public:
    static ImageDecoderExternal* Create(ScriptState*, const ImageDecoderInit*, ExceptionState&);
    ...
    ScriptPromise<ImageDecodeResult> decode(const ImageDecodeOptions* options = nullptr);
...
private:
    ...
    void OnDecodeReady(std::unique_ptr<ImageDecoderCore::ImageDecodeResult> result);
    ...
    ImageDecoder::AnimationOption animation_option_ = ImageDecoder::AnimationOption::kUnspecified;
    ...
    struct DecodeRequest final : public GarbageCollected<DecodeRequest> {
        DecodeRequest(ScriptPromiseResolver<ImageDecodeResult>* resolver, uint32_t frame_index, bool complete_frames_only);
        ~DecodeRequest();
