...
class MODULES_EXPORT VideoFrame final : public ScriptWrappable, public CanvasImageSource, public ImageBitmapSource {
    DEFINE_WRAPPERTYPEINFO();

public:
    // Creates a VideoFrame with a new VideoFrameHandle wrapping |frame|, and
    // monitored using |monitoring_source_id|.
    VideoFrame(scoped_refptr<media::VideoFrame> frame, ExecutionContext*, std::string monitoring_source_id = std::string(), sk_sp<SkImage> sk_image = nullptr,
    ...
    static VideoFrame* Create(ScriptState* script_state, const V8CanvasImageSource* source, const VideoFrameInit* init, ExceptionState& exception_state);
    static VideoFrame* Create(ScriptState*, const AllowSharedBufferSource*, const VideoFrameBufferInit*, ExceptionState&);
    ...
    scoped_refptr<VideoFrameHandle> handle() const
    {
        return handle_;
    }
...
private:
    ...
    void ConvertAndCopyToRGB(scoped_refptr<media::VideoFrame> frame, const gfx::Rect& src_rect, const VideoFrameLayout& dest_layout, base::span<uint8_t> buffer,
        PredefinedColorSpace target_color_space);

    bool CopyToAsync(ScriptPromiseResolver<IDLSequence<PlaneLayout>>*, scoped_refptr<media::VideoFrame> frame, gfx::Rect src_rect,
