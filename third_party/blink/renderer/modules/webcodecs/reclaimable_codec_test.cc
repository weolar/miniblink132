...
constexpr base::TimeDelta kTimerPeriod = ReclaimableCodec::kInactivityReclamationThreshold / 2;

class FakeReclaimableCodec final : public GarbageCollected<FakeReclaimableCodec>, public ReclaimableCodec {
public:
    FakeReclaimableCodec(ReclaimableCodec::CodecType type, ExecutionContext* context)
...
class ReclaimableCodecTest : public testing::TestWithParam<ReclaimableCodec::CodecType> {
public:
    FakeReclaimableCodec* CreateCodec(ExecutionContext* context)
    {
        if (!is_gauge_threshold_set_) {
            CodecPressureGauge::GetInstance(GetParam()).set_pressure_threshold_for_testing(kTestPressureThreshold);
...
TEST_P(ReclaimableCodecTest, BackgroundInactivityTimerStartStops)
{
    ...
    TestBackgroundInactivityTimerStartStops(CreateCodec(v8_scope.GetExecutionContext()));
}
...
TEST_P(ReclaimableCodecTest, BackgroundInactivityTimerWorks)
{
    ...
    // Only background reclamation permitted, so simulate backgrouding.
    TestBackgroundInactivityTimerWorks(CreateCodec(v8_scope.GetExecutionContext()));
}
...
TEST_P(ReclaimableCodecTest, ForegroundCodecReclaimedOnceBackgrounded)
{
    ...
    codec->SimulateLifecycleStateForTesting(scheduler::SchedulingLifecycleState::kNotThrottled);
    EXPECT_FALSE(codec->IsReclamationTimerActiveForTesting());
    EXPECT_FALSE(codec->is_backgrounded_for_testing());
    EXPECT_FALSE(codec->reclaimed());
...
TEST_P(ReclaimableCodecTest, RepeatLifecycleEventsDontBreakState)
{
    ...
    codec->SimulateLifecycleStateForTesting(scheduler::SchedulingLifecycleState::kThrottled);
    EXPECT_TRUE(codec->IsReclamationTimerActiveForTesting());
    EXPECT_TRUE(codec->is_backgrounded_for_testing());
    EXPECT_FALSE(codec->reclaimed());

    // Further background lifecycle progression shouldn't affect codec state.
    codec->SimulateLifecycleStateForTesting(scheduler::SchedulingLifecycleState::kStopped);
    EXPECT_TRUE(codec->IsReclamationTimerActiveForTesting());
    EXPECT_TRUE(codec->is_backgrounded_for_testing());
    EXPECT_FALSE(codec->reclaimed());
...
INSTANTIATE_TEST_SUITE_P(, ReclaimableCodecTest, testing::Values(ReclaimableCodec::CodecType::kDecoder, ReclaimableCodec::CodecType::kEncoder));
