...
public:
    ...
    void Initialize(const media::AudioDecoderConfig& config, media::CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb,
    ...
    void Decode(scoped_refptr<media::DecoderBuffer> buffer, DecodeCB done_cb) override
    {
        DCHECK(output_cb_);
        ...
        if (!buffer->end_of_stream()) {
            output_cb_.Run(MakeAudioBuffer(kSampleFormat, kChannelLayout, kChannels, kSamplesPerSecond, 1.0f, 0.0f, kInputFramesChunk, buffer->timestamp()));
        }
    }
...
public:
    ...
    std::unique_ptr<media::AudioDecoder> CreateAudioDecoder(
        scoped_refptr<base::SequencedTaskRunner> task_runner, std::unique_ptr<media::MediaLog> media_log) override
    {
...
public:
    ...
    void CreateAudioDecoder(mojo::PendingReceiver<media::mojom::AudioDecoder> receiver) override
    {
        audio_decoder_receivers_.Add(
            std::make_unique<media::MojoAudioDecoderService>(&mojo_media_client_, &cdm_service_context_, base::SingleThreadTaskRunner::GetCurrentDefault()),
            std::move(receiver));
    }
...
public:
    ...
    {
        decoder_broker_ = std::make_unique<AudioDecoderBroker>(&null_media_log_, execution_context);
    }
    ...
    void InitializeDecoder(media::AudioDecoderConfig config)
    {
        ...
        decoder_broker_->Initialize(config, nullptr /* cdm_context */,
            WTF::BindOnce(&AudioDecoderBrokerTest::OnInitWithClosure, WTF::Unretained(this), run_loop.QuitClosure()),
            WTF::BindRepeating(&AudioDecoderBrokerTest::OnOutput, WTF::Unretained(this)), media::WaitingCB());
        run_loop.Run();
    ...
    void DecodeBuffer(scoped_refptr<media::DecoderBuffer> buffer, media::DecoderStatus::Codes expected_status = media::DecoderStatus::Codes::kOk)
    {
        ...
        decoder_broker_->Decode(buffer, WTF::BindOnce(&AudioDecoderBrokerTest::OnDecodeDoneWithClosure, WTF::Unretained(this), run_loop.QuitClosure()));
        run_loop.Run();
        testing::Mock::VerifyAndClearExpectations(this);
    }
    ...
    void ResetDecoder()
    {
        ...
        decoder_broker_->Reset(WTF::BindOnce(&AudioDecoderBrokerTest::OnResetDoneWithClosure, WTF::Unretained(this), run_loop.QuitClosure()));
        run_loop.Run();
        testing::Mock::VerifyAndClearExpectations(this);
    }
    ...
    {
        return decoder_broker_->SupportsDecryption();
    }
...
TEST_F(AudioDecoderBrokerTest, Decode_Uninitialized)
{
    ...
    DecodeBuffer(media::ReadTestDataFile("vorbis-packet-0"), media::DecoderStatus::Codes::kNotInitialized);
...
media::AudioDecoderConfig MakeVorbisConfig()
{
    ...
    CHECK_EQ(file_size, base::ReadFile(extradata_path, reinterpret_cast<char*>(&extradata[0]), file_size)) << "Failed to read '" << extradata_name << "'";

    return media::AudioDecoderConfig(kCodec, kSampleFormat, kChannelLayout, kSamplesPerSecond, std::move(extradata), media::EncryptionScheme::kUnencrypted);
}
...
TEST_F(AudioDecoderBrokerTest, Decode_NoMojoDecoder)
{
    ...
    DecodeBuffer(media::ReadTestDataFile("vorbis-packet-0", base::Milliseconds(0)));
    DecodeBuffer(media::ReadTestDataFile("vorbis-packet-1", base::Milliseconds(1)));
    DecodeBuffer(media::ReadTestDataFile("vorbis-packet-2", base::Milliseconds(2)));
...
TEST_F(AudioDecoderBrokerTest, Decode_WithMojoDecoder)
{
    ...
    InitializeDecoder(media::AudioDecoderConfig(
        media::AudioCodec::kMpegHAudio, kSampleFormat, kChannelLayout, kSamplesPerSecond, media::EmptyExtraData(), media::EncryptionScheme::kUnencrypted));
