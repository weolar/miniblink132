...
std::unique_ptr<media::DecryptConfig> CreateMediaDecryptConfig(const DecryptConfig& js_config)
{
    ...
    if (scheme == "cenc") {
        return media::DecryptConfig::CreateCencConfig(std::move(key_id_str), std::move(iv_str), subsamples);
    }

    DCHECK_EQ(scheme, "cbcs");
    std::optional<media::EncryptionPattern> encryption_pattern;
    if (js_config.hasEncryptionPattern()) {
        encryption_pattern.emplace(js_config.encryptionPattern()->cryptByteBlock(), js_config.encryptionPattern()->skipByteBlock());
    }
    return media::DecryptConfig::CreateCbcsConfig(std::move(key_id_str), std::move(iv_str), subsamples, encryption_pattern);
}

std::optional<media::EncryptionScheme> ToMediaEncryptionScheme(const String& scheme)
{
    if (scheme == "cenc") {
