...
public:
    FakeGpuVideoDecoder()
        : FakeVideoDecoder(0 /* decoder_id */, 0 /* decoding_delay */, 13 /* max_parallel_decoding_requests */, media::BytesDecodedCB())
    {
    }
    ~FakeGpuVideoDecoder() override = default;

    scoped_refptr<media::VideoFrame> MakeVideoFrame(const media::DecoderBuffer& buffer) override
    {
        scoped_refptr<gpu::ClientSharedImage> shared_image = gpu::ClientSharedImage::CreateForTesting();
        scoped_refptr<media::VideoFrame> frame
            = media::VideoFrame::WrapSharedImage(media::PIXEL_FORMAT_ARGB, shared_image, gpu::SyncToken(), media::VideoFrame::ReleaseMailboxCB(),
                current_config_.coded_size(), current_config_.visible_rect(), current_config_.natural_size(), buffer.timestamp());
...
public:
    ...
    std::unique_ptr<media::VideoDecoder> CreateVideoDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner, media::MediaLog* media_log,
        media::mojom::CommandBufferIdPtr command_buffer_id, media::RequestOverlayInfoCB request_overlay_info_cb, const gfx::ColorSpace& target_color_space,
        mojo::PendingRemote<media::stable::mojom::StableVideoDecoder> oop_video_decoder) override
    {
...
public:
    ...
    void BindRequest(mojo::ScopedMessagePipeHandle handle)
    {
        ...
        receiver_.set_disconnect_handler(WTF::BindOnce(&FakeInterfaceFactory::OnConnectionError, base::Unretained(this)));
    }
    ...
    {
        video_decoder_receivers_.Add(std::make_unique<media::MojoVideoDecoderService>(
                                         &mojo_media_client_, &cdm_service_context_, mojo::PendingRemote<media::stable::mojom::StableVideoDecoder>()),
            std::move(receiver));
    }

#if BUILDFLAG(ALLOW_OOP_VIDEO_DECODER)
    void CreateStableVideoDecoder(mojo::PendingReceiver<media::stable::mojom::StableVideoDecoder> video_decoder) override
    {
...
public:
    ...
    ~VideoDecoderBrokerTest() override
    {
        ...
        Platform::Current()->GetBrowserInterfaceBroker()->SetBinderForTesting(
            media::mojom::InterfaceFactory::Name_, base::RepeatingCallback<void(mojo::ScopedMessagePipeHandle)>());
    ...
    void OnInitWithClosure(base::RepeatingClosure done_cb, media::DecoderStatus status)
    {
        OnInit(status);
        done_cb.Run();
    }
    void OnDecodeDoneWithClosure(base::RepeatingClosure done_cb, media::DecoderStatus status)
    {
    ...
    void SetupMojo(ExecutionContext& execution_context)
    {
        ...
        EXPECT_TRUE(Platform::Current()->GetBrowserInterfaceBroker()->SetBinderForTesting(
            media::mojom::InterfaceFactory::Name_, WTF::BindRepeating(&FakeInterfaceFactory::BindRequest, base::Unretained(interface_factory_.get()))));
        ...
        gpu_factories_ = std::make_unique<media::MockGpuVideoAcceleratorFactories>(nullptr);
        EXPECT_CALL(*gpu_factories_, GetTaskRunner()).WillRepeatedly(Return(media_thread_->task_runner()));
        EXPECT_CALL(*gpu_factories_, IsDecoderConfigSupported(_)).WillRepeatedly(Return(media::GpuVideoAcceleratorFactories::Supported::kTrue));
        EXPECT_CALL(*gpu_factories_, GetChannelToken(_)).WillRepeatedly(Invoke([](base::OnceCallback<void(const base::UnguessableToken&)> callback) {
    ...
    {
        decoder_broker_ = std::make_unique<VideoDecoderBroker>(execution_context, gpu_factories_.get(), &null_media_log_);
    }

    void InitializeDecoder(media::VideoDecoderConfig config, bool expect_success = true)
    {
        base::RunLoop run_loop;
        if (expect_success) {
            EXPECT_CALL(*this, OnInit(media::SameStatusCode(media::DecoderStatus(media::DecoderStatus::Codes::kOk))));
        } else {
            EXPECT_CALL(*this, OnInit(media::SameStatusCode(media::DecoderStatus(media::DecoderStatus::Codes::kUnsupportedConfig))));
        }
        decoder_broker_->Initialize(config, false /*low_delay*/, nullptr /* cdm_context */,
            WTF::BindOnce(&VideoDecoderBrokerTest::OnInitWithClosure, WTF::Unretained(this), run_loop.QuitClosure()),
            WTF::BindRepeating(&VideoDecoderBrokerTest::OnOutput, WTF::Unretained(this)), media::WaitingCB());
        run_loop.Run();
        testing::Mock::VerifyAndClearExpectations(this);
    }

    void DecodeBuffer(scoped_refptr<media::DecoderBuffer> buffer, media::DecoderStatus::Codes expected_status = media::DecoderStatus::Codes::kOk)
    {
        base::RunLoop run_loop;
        EXPECT_CALL(*this, OnDecodeDone(HasStatusCode(expected_status)));
        decoder_broker_->Decode(buffer, WTF::BindOnce(&VideoDecoderBrokerTest::OnDecodeDoneWithClosure, WTF::Unretained(this), run_loop.QuitClosure()));
        run_loop.Run();
        testing::Mock::VerifyAndClearExpectations(this);
    }
    ...
    void ResetDecoder()
    {
        ...
        decoder_broker_->Reset(WTF::BindOnce(&VideoDecoderBrokerTest::OnResetDoneWithClosure, WTF::Unretained(this), run_loop.QuitClosure()));
        run_loop.Run();
        testing::Mock::VerifyAndClearExpectations(this);
    }
    ...
    bool IsPlatformDecoder()
    {
        return decoder_broker_->IsPlatformDecoder();
    }
    ...
    {
        return decoder_broker_->GetMaxDecodeRequests();
    }
...
TEST_F(VideoDecoderBrokerTest, Decode_Uninitialized)
{
    ...
    DecodeBuffer(media::ReadTestDataFile("vp8-I-frame-320x120"), media::DecoderStatus::Codes::kNotInitialized);
    DecodeBuffer(media::DecoderBuffer::CreateEOSBuffer(), media::DecoderStatus::Codes::kNotInitialized);
...
TEST_F(VideoDecoderBrokerTest, Decode_MultipleAccelerationPreferences)
{
    ...
    DecodeBuffer(media::CreateFakeVideoBufferForTest(large_config, base::TimeDelta(), base::Milliseconds(33)));
    DecodeBuffer(media::DecoderBuffer::CreateEOSBuffer());
    ASSERT_EQ(2U, output_frames_.size());
    ...
    DecodeBuffer(media::CreateFakeVideoBufferForTest(normal_config, base::TimeDelta(), base::Milliseconds(33)));
    DecodeBuffer(media::DecoderBuffer::CreateEOSBuffer());
...
TEST_F(VideoDecoderBrokerTest, Decode_WithMojoDecoder)
{
    ...
    DecodeBuffer(media::CreateFakeVideoBufferForTest(config, base::TimeDelta(), base::Milliseconds(33)));
    DecodeBuffer(media::DecoderBuffer::CreateEOSBuffer());
    ASSERT_EQ(1U, output_frames_.size());
