...
public:
    ...
    struct AudioEncoderEncodeOptions : public GarbageCollected<AudioEncoderEncodeOptions> {
        void Trace(Visitor*) const
        {
        }
    };
...
public:
    static AudioEncoder* Create(ScriptState*, const AudioEncoderInit*, ExceptionState&);
    ...
    static ScriptPromise<AudioEncoderSupport> isConfigSupported(ScriptState*, const AudioEncoderConfig*, ExceptionState&);
...
private:
    ...
    ParsedConfig* ParseConfig(const AudioEncoderConfig* opts, ExceptionState&) override;
    ...
    std::unique_ptr<media::AudioEncoder> CreateMediaAudioEncoder(const ParsedConfig& config);
    void CallOutputCallback(ParsedConfig* active_config, uint32_t reset_count, media::EncodedAudioBuffer encoded_buffer,
        std::optional<media::AudioEncoder::CodecDescription> codec_desc);
    DOMException* MakeOperationError(std::string error_msg, media::EncoderStatus status);
    DOMException* MakeEncodingError(std::string error_msg, media::EncoderStatus status);
