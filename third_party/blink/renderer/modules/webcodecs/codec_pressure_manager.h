...
class MODULES_EXPORT CodecPressureManager : public GarbageCollected<CodecPressureManager> {
    USING_PRE_FINALIZER(CodecPressureManager, UnregisterManager);

public:
    CodecPressureManager(ReclaimableCodec::CodecType, scoped_refptr<base::SequencedTaskRunner> task_runner);
    ...
    size_t pressure_for_testing()
    {
        return local_codec_pressure_;
    }
