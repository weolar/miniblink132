...
TEST(EncodedAudioChunkTest, ConstructorAndAttributes)
{
    ...
    auto* encoded = EncodedAudioChunk::Create(v8_scope.GetScriptState(), init, v8_scope.GetExceptionState());
...
TEST(EncodedAudioChunkTest, DecryptConfig)
{
    ...
    auto expected_media_decrypt_config = CreateTestDecryptConfig(media::EncryptionScheme::kCenc);

    auto* decrypt_config = MakeGarbageCollected<DecryptConfig>();
    decrypt_config->setEncryptionScheme("cenc");
    decrypt_config->setKeyId(StringToBuffer(expected_media_decrypt_config->key_id()));
    decrypt_config->setInitializationVector(StringToBuffer(expected_media_decrypt_config->iv()));
    ...
    EXPECT_TRUE(expected_media_decrypt_config->Matches(*encoded->buffer()->decrypt_config()));
}
