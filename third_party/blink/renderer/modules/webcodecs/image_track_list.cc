...
ImageTrackList::ImageTrackList(ImageDecoderExternal* image_decoder)
    ...
    , ready_property_(MakeGarbageCollected<ReadyProperty>(image_decoder->GetExecutionContext()))
{
}
...
void ImageTrackList::AddTrack(uint32_t frame_count, int repetition_count, bool selected)
{
    if (selected) {
    ...
    tracks_.push_back(MakeGarbageCollected<ImageTrack>(this, tracks_.size(), frame_count, repetition_count, selected));
}
