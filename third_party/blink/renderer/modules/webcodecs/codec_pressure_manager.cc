...
CodecPressureManager::CodecPressureManager(ReclaimableCodec::CodecType codec_type, scoped_refptr<base::SequencedTaskRunner> task_runner)
    : codec_type_(codec_type)
{
    auto pressure_threshold_changed_cb
        = [](CrossThreadWeakPersistent<CodecPressureManager> self, scoped_refptr<base::SequencedTaskRunner> task_runner, bool global_pressure_exceeded) {
              ...
              DCHECK(task_runner);
              PostCrossThreadTask(
                  *task_runner, FROM_HERE, CrossThreadBindOnce(&CodecPressureManager::OnGlobalPressureThresholdChanged, self, global_pressure_exceeded));
          };

    CodecPressureGauge::RegistrationResult result = GetCodecPressureGauge().RegisterPressureCallback(
        ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(pressure_threshold_changed_cb, WrapCrossThreadWeakPersistent(this), task_runner)));
...
void CodecPressureManager::OnGlobalPressureThresholdChanged(bool pressure_threshold_exceeded)
{
...
void CodecPressureManager::UnregisterManager()
{
    ...
    GetCodecPressureGauge().UnregisterPressureCallback(pressure_callback_id_, local_codec_pressure_);
