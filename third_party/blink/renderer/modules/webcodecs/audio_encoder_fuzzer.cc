...
DEFINE_TEXT_PROTO_FUZZER(const wc_fuzzer::AudioEncoderApiInvocationSequence& proto)
{
    ...
    static const bool kSetTestBinder = []() {
        ...
        return Platform::Current()->GetBrowserInterfaceBroker()->SetBinderForTesting(
    ...
    Persistent<ScriptState> script_state = ToScriptStateForMainWorld(&page_holder->GetFrame());
    ScriptState::Scope scope(script_state);

    Persistent<V8WebCodecsErrorCallback> error_callback
        = V8WebCodecsErrorCallback::Create(MakeGarbageCollected<FakeFunction>("error")->ToV8Function(script_state));
    Persistent<V8EncodedAudioChunkOutputCallback> output_callback
        = V8EncodedAudioChunkOutputCallback::Create(MakeGarbageCollected<FakeFunction>("output")->ToV8Function(script_state));

    Persistent<AudioEncoderInit> audio_encoder_init = MakeGarbageCollected<AudioEncoderInit>();
    ...
    if (audio_encoder) {
        for (auto& invocation : proto.invocations()) {
            ...
            case wc_fuzzer::AudioEncoderApiInvocation::kConfigure: {
                AudioEncoderConfig* config = MakeAudioEncoderConfig(invocation.configure());
            ...
            case wc_fuzzer::AudioEncoderApiInvocation::kEncode: {
                AudioData* data = MakeAudioData(script_state, invocation.encode().data());
                if (!data) {
                    return;
                }
