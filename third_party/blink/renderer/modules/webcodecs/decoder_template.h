...
class MODULES_EXPORT DecoderTemplate : public EventTarget, public ActiveScriptWrappable<DecoderTemplate<Traits>>, public ReclaimableCodec {
public:
...
protected:
    virtual bool IsValidConfig(const ConfigType& config, String* js_error_message) = 0;

    // Convert a configuration to a DecoderConfig. Returns std::nullopt if the
    // configuration is not supported.
    virtual std::optional<MediaConfigType> MakeMediaConfig(const ConfigType& config, String* js_error_message) = 0;
    ...
    // Virtual for UTs.
    virtual MediaDecoderType* decoder()
    {
        return decoder_.get();
    }
    ...
    virtual media::DecoderStatus::Or<scoped_refptr<media::DecoderBuffer>> MakeInput(const InputType& chunk, bool verify_key_frame) = 0;

    // Convert an output to the WebCodecs type.
    virtual media::DecoderStatus::Or<OutputType*> MakeOutput(scoped_refptr<MediaOutputType> output, ExecutionContext* context) = 0;
...
private:
    ...
    void ContinueConfigureWithGpuFactories(Request* request, media::GpuVideoAcceleratorFactories* factories);
    ...
    DOMException* MakeOperationError(std::string error_msg, media::DecoderStatus status);
    DOMException* MakeEncodingError(std::string error_msg, media::DecoderStatus status);
