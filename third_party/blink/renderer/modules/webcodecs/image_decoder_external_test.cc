...
protected:
    ImageDecoderExternal* CreateDecoder(V8TestingScope* v8_scope, const char* file_name, const char* mime_type)
    {
        auto* init = MakeGarbageCollected<ImageDecoderInit>();
        init->setType(mime_type);

        init->setData(MakeGarbageCollected<V8ImageBufferSource>(DOMArrayBuffer::Create(base::as_byte_span(ReadFile(file_name)))));
        return ImageDecoderExternal::Create(v8_scope->GetScriptState(), init, v8_scope->GetExceptionState());
    }

    ImageDecodeResult* ToImageDecodeResult(V8TestingScope* v8_scope, ScriptValue value)
    {
        return NativeValueTraits<ImageDecodeResult>::NativeValue(v8_scope->GetIsolate(), value.V8Value(), v8_scope->GetExceptionState());
    }

    ImageDecodeOptions* MakeOptions(uint32_t frame_index = 0, bool complete_frames_only = true)
    {
    ...
    {
        auto promise = ImageDecoderExternal::isTypeSupported(v8_scope->GetScriptState(), type);
...
TEST_F(ImageDecoderTest, DecodeEmpty)
{
    ...
    init->setData(MakeGarbageCollected<V8ImageBufferSource>(DOMArrayBuffer::Create(SharedBuffer::Create())));
    auto* decoder = ImageDecoderExternal::Create(v8_scope.GetScriptState(), init, v8_scope.GetExceptionState());
...
TEST_F(ImageDecoderTest, DecodeNeuteredAtConstruction)
{
    ...
    ASSERT_TRUE(buffer->Transfer(v8_scope.GetIsolate(), contents, v8_scope.GetExceptionState()));
...
TEST_F(ImageDecoderTest, DecodeUnsupported)
{
    ...
    auto* decoder = CreateDecoder(&v8_scope, "images/resources/test.svg", kImageType);
    EXPECT_TRUE(decoder);
...
TEST_F(ImageDecoderTest, DecoderCreationMixedCaseMimeType)
{
    ...
    constexpr char kImageType[] = "image/GiF";
    EXPECT_TRUE(IsTypeSupported(&v8_scope, kImageType));
    auto* decoder = CreateDecoder(&v8_scope, "images/resources/animated.gif", kImageType);
    ASSERT_TRUE(decoder);
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
...
TEST_F(ImageDecoderTest, DecodeGif)
{
    ...
    constexpr char kImageType[] = "image/gif";
    EXPECT_TRUE(IsTypeSupported(&v8_scope, kImageType));
    auto* decoder = CreateDecoder(&v8_scope, "images/resources/animated-10color.gif", kImageType);
    ASSERT_TRUE(decoder);
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
...
TEST_F(ImageDecoderTest, DecodeAborted)
{
    ...
    auto* decoder = CreateDecoder(&v8_scope, "images/resources/avif/red-at-12-oclock-with-color-profile-12bpc.avif", kImageType);
...
TEST_F(ImageDecoderTest, DecoderReadableStream)
{
    ...
    Persistent<TestUnderlyingSource> underlying_source = MakeGarbageCollected<TestUnderlyingSource>(v8_scope.GetScriptState());
    Persistent<ReadableStream> stream = ReadableStream::CreateWithCountQueueingStrategy(v8_scope.GetScriptState(), underlying_source, 0);

    auto* init = MakeGarbageCollected<ImageDecoderInit>();
    init->setType(kImageType);
    init->setData(MakeGarbageCollected<V8ImageBufferSource>(stream));

    Persistent<ImageDecoderExternal> decoder = ImageDecoderExternal::Create(v8_scope.GetScriptState(), init, IGNORE_EXCEPTION_FOR_TESTING);
    ASSERT_TRUE(decoder);
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
    EXPECT_EQ(decoder->type(), kImageType);
    ...
    v8::Local<v8::Value> v8_data_array = ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(data_span.subspan(0, chunk_size)));
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
    ...
    v8_data_array = ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(data_span.subspan(chunk_size, data.size() - chunk_size)));
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
    ...
    ScriptPromiseTester completed_tester(v8_scope.GetScriptState(), completed_promise);
...
TEST_F(ImageDecoderTest, DecoderReadableStreamAvif)
{
    ...
    base::span<const uint8_t> data_span = base::as_byte_span(data);
    v8::Local<v8::Value> v8_data_array = ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(data_span.subspan(0, 1)));
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
    ...
    // be resolved.
    ScriptPromiseTester metadata_tester(v8_scope.GetScriptState(), metadata_promise);
    ...
    v8_data_array = ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(data_span.subspan(1, data.size() - 1)));
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
...
TEST_F(ImageDecoderTest, ReadableStreamAvifStillYuvDecoding)
{
    ...
    v8::Local<v8::Value> v8_data_array = ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(base::as_byte_span(data)));
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
...
TEST_F(ImageDecoderTest, DecodePartialImage)
{
    ...
    array_buffer->ByteSpan().copy_from(base::as_byte_span(data).subspan(0, array_buffer->ByteLength()));
...
TEST_F(ImageDecoderTest, DecodeClosedDuringReadableStream)
{
    ...
    v8::Local<v8::Value> v8_data_array
        = ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(data_span.subspan(0, data.size() / 2)));
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
...
TEST_F(ImageDecoderTest, DecodeInvalidFileViaReadableStream)
{
    ...
    {
        auto promise = decoder->decode(MakeOptions(decoder->tracks().selectedTrack()->frameCount() - 1, true));
        ScriptPromiseTester tester(v8_scope.GetScriptState(), promise);
        tester.WaitUntilSettled();
        EXPECT_TRUE(tester.IsRejected());
    }
...
TEST_F(ImageDecoderTest, DecodeYuv)
{
    ...
    EXPECT_TRUE(IsTypeSupported(&v8_scope, kImageType));
    auto* decoder = CreateDecoder(&v8_scope, "images/resources/ycbcr-420.jpg", kImageType);
    ASSERT_TRUE(decoder);
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
