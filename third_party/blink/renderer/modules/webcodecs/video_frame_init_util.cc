...
template <typename T> gfx::Size ParseAndValidateDisplaySizeImpl(T* init, ExceptionState& exception_state)
{
    DCHECK(init->hasDisplayWidth() || init->hasDisplayHeight());

    if (!init->hasDisplayWidth()) {
        exception_state.ThrowTypeError("displayHeight specified without displayWidth.");
        return gfx::Size();
    }
    if (!init->hasDisplayHeight()) {
        exception_state.ThrowTypeError("displayWidth specified without displayHeight.");
        return gfx::Size();
    }
    ...
    if (display_width > media::limits::kMaxDimension || display_height > media::limits::kMaxDimension) {
        exception_state.ThrowTypeError(String::Format("Invalid display size (%u, %u); exceeds "
                                                      "implementation limit.",
            display_width, display_height));
        return gfx::Size();
    }

    return gfx::Size(static_cast<int>(display_width), static_cast<int>(display_height));
}

gfx::Size ParseAndValidateDisplaySize(const VideoFrameInit* init, ExceptionState& exception_state)
{
    return ParseAndValidateDisplaySizeImpl(init, exception_state);
}

gfx::Size ParseAndValidateDisplaySize(const VideoFrameBufferInit* init, ExceptionState& exception_state)
{
    return ParseAndValidateDisplaySizeImpl(init, exception_state);
}
...
ParsedVideoFrameInit::ParsedVideoFrameInit(const VideoFrameInit* init, media::VideoPixelFormat format, const gfx::Size& coded_size,
    const gfx::Rect& default_visible_rect, const gfx::Size& default_display_size, ExceptionState& exception_state)
{
    ...
    if (init->hasVisibleRect()) {
        visible_rect = ToGfxRect(init->visibleRect(), "visibleRect", coded_size, exception_state);
        ...
        ValidateOffsetAlignment(format, visible_rect, "visibleRect", exception_state);
        if (exception_state.HadException())
            return;
    }
    ...
    if (init->hasDisplayWidth() || init->hasDisplayHeight()) {
        ...
        double widthScale = default_display_size.width() / default_visible_rect.width();
        double heightScale = default_display_size.height() / default_visible_rect.height();
        display_size = gfx::Size(std::round(visible_rect.width() * widthScale), std::round(visible_rect.height() * heightScale));
