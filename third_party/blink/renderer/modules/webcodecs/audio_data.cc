...
std::optional<V8AudioSampleFormat> MediaFormatToBlinkFormat(media::SampleFormat media_format)
{
...
void CopyToInterleaved(uint8_t* dest_data, const std::vector<uint8_t*>& src_channels_data, const int frame_offset, const int frames_to_copy)
{
    const int channels = static_cast<int>(src_channels_data.size());

    SampleType* dest = reinterpret_cast<SampleType*>(dest_data);
    for (int ch = 0; ch < channels; ++ch) {
        const SampleType* src_start = reinterpret_cast<SampleType*>(src_channels_data[ch]) + frame_offset;
...
template <typename SampleType>
void CopyToPlanar(uint8_t* dest_data, const uint8_t* src_data, const int src_channel_count, const int dest_channel_index, const int frame_offset,
    const int frames_to_copy, ExceptionState& exception_state)
{
    SampleType* dest = reinterpret_cast<SampleType*>(dest_data);

    uint32_t offset_in_samples = 0;
    bool overflow = false;
    if (!base::CheckMul(frame_offset, src_channel_count).AssignIfValid(&offset_in_samples)) {
        overflow = true;
        return;
    }
    ...
    // below.
    if (overflow || !base::CheckMul(frames_to_copy, src_channel_count).IsValid()) {
        exception_state.ThrowTypeError(String::Format("Provided options cause overflow when calculating offset."));
        return;
    }

    const SampleType* src_start = reinterpret_cast<const SampleType*>(src_data) + offset_in_samples;
    for (int i = 0; i < frames_to_copy; ++i) {
...
class ArrayBufferContentsAsAudioExternalMemory : public media::AudioBuffer::ExternalMemory {
public:
    explicit ArrayBufferContentsAsAudioExternalMemory(ArrayBufferContents contents, base::span<uint8_t> span)
...
// static
AudioData* AudioData::Create(ScriptState* script_state, AudioDataInit* init, ExceptionState& exception_state)
{
    return MakeGarbageCollected<AudioData>(script_state, init, exception_state);
}

AudioData::AudioData(ScriptState* script_state, AudioDataInit* init, ExceptionState& exception_state)
    : format_(std::nullopt)
    , timestamp_(init->timestamp())
{
    ...
    uint32_t bytes_per_sample = media::SampleFormatToBytesPerChannel(media_format);

    uint32_t total_bytes;
    if (!base::CheckMul(bytes_per_sample, base::CheckMul(init->numberOfChannels(), init->numberOfFrames())).AssignIfValid(&total_bytes)) {
        exception_state.ThrowTypeError("AudioData allocation size exceeds implementation limits.");
        return;
    }
    ...
    if (total_bytes > array_span.size()) {
        exception_state.ThrowTypeError(String::Format("data is too small: needs %u bytes, received %zu.", total_bytes, array_span.size()));
        return;
    }
    ...
    auto buffer_contents = TransferArrayBufferForSpan(init->transfer(), array_span, exception_state, isolate);
    if (exception_state.HadException()) {
        return;
    }

    int sample_rate = base::saturated_cast<int>(init->sampleRate());
    if (sample_rate < media::limits::kMinSampleRate || sample_rate > media::limits::kMaxSampleRate) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError,
            String::Format("sampleRate is outside of supported implementation "
                           "limits: need between %u and %u, received %d.",
                media::limits::kMinSampleRate, media::limits::kMaxSampleRate, sample_rate));
        return;
    }

    format_ = init->format();
    auto channel_layout = init->numberOfChannels() > 8
    ...
    if (buffer_contents.IsValid() && sample_aligned) {
        ...
        auto external_memory = std::make_unique<ArrayBufferContentsAsAudioExternalMemory>(std::move(buffer_contents), array_span);
        data_ = media::AudioBuffer::CreateFromExternalMemory(media_format, channel_layout, init->numberOfChannels(), sample_rate, init->numberOfFrames(),
            base::Microseconds(timestamp_), std::move(external_memory));
        CHECK(data_);
        return;
    }
    ...
    if (media::IsInterleaved(media_format)) {
        ...
        uint32_t plane_size_in_bytes = init->numberOfFrames() * media::SampleFormatToBytesPerChannel(media_format);

        const uint8_t* plane_start = reinterpret_cast<const uint8_t*>(array_span.data());
    ...
    data_ = media::AudioBuffer::CopyFrom(
        media_format, channel_layout, init->numberOfChannels(), sample_rate, init->numberOfFrames(), channel_ptrs.data(), base::Microseconds(timestamp_));
    CHECK(data_);
}
...
{
    if (!data_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot clone closed AudioData.");
        return nullptr;
    }
...
uint32_t AudioData::allocationSize(AudioDataCopyToOptions* copy_to_options, ExceptionState& exception_state)
{
    if (!data_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "AudioData is closed.");
        return 0;
    }

    auto format = copy_to_options->hasFormat() ? BlinkFormatToMediaFormat(copy_to_options->format()) : data_->sample_format();

    // Interleaved formats only have one plane, despite many channels.
    uint32_t max_plane_index = media::IsInterleaved(format) ? 0 : data_->channel_count() - 1;
    ...
    if (offset >= total_frames) {
        exception_state.ThrowRangeError(String::Format("Frame offset exceeds total frames (%u >= %u).", offset, total_frames));
        return 0;
    }

    const uint32_t available_frames = total_frames - offset;
    const uint32_t frame_count = copy_to_options->hasFrameCount() ? copy_to_options->frameCount() : available_frames;

    if (frame_count > available_frames) {
        exception_state.ThrowRangeError(String::Format("Frame count exceeds available_frames frames (%u > %u).", frame_count, available_frames));
        return 0;
    }
    ...
    bool overflow = false;
    if (media::IsInterleaved(format) && !base::CheckMul(frame_count, data_->channel_count()).AssignIfValid(&sample_count)) {
        overflow = true;
    }

    uint32_t allocation_size;
    if (overflow || !base::CheckMul(sample_count, media::SampleFormatToBytesPerChannel(format)).AssignIfValid(&allocation_size)) {
        exception_state.ThrowTypeError(String::Format("Provided options cause overflow when calculating allocation size."));
        return 0;
    }
...
void AudioData::copyTo(const AllowSharedBufferSource* destination, AudioDataCopyToOptions* copy_to_options, ExceptionState& exception_state)
{
    if (!data_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot copy closed AudioData.");
        return;
    }

    uint32_t copy_size_in_bytes = allocationSize(copy_to_options, exception_state);
    ...
    const auto dest_format = copy_to_options->hasFormat() ? BlinkFormatToMediaFormat(copy_to_options->format()) : data_->sample_format();

    const auto src_format = data_->sample_format();

    // Ignore (de)interleaving, and verify whether we need to convert between
    // sample types.
    const bool needs_conversion = RemovePlanar(src_format) != RemovePlanar(dest_format);
    ...
    CHECK_LE(copy_to_options->planeIndex(), static_cast<uint32_t>(data_->channel_count()));

    size_t src_data_size = 0;
    if (media::IsInterleaved(src_format)) {
        src_data_size = data_->data_size();
    } else {
        src_data_size = media::SampleFormatToBytesPerChannel(src_format) * data_->frame_count();
    }
    ...
    if (media::IsInterleaved(dest_format) && !base::CheckMul(copy_to_options->frameOffset(), data_->channel_count()).AssignIfValid(&offset_in_samples)) {
        overflow = true;
    }

    uint32_t offset_in_bytes = 0;
    if (overflow || !base::CheckMul(offset_in_samples, media::SampleFormatToBytesPerChannel(dest_format)).AssignIfValid(&offset_in_bytes)) {
        exception_state.ThrowTypeError(String::Format("Provided options cause overflow when calculating offset."));
        return;
    }

    const uint8_t* src_data = data_->channel_data()[copy_to_options->planeIndex()];
...
void AudioData::CopyConvert(base::span<uint8_t> dest, AudioDataCopyToOptions* copy_to_options)
{
    const media::SampleFormat dest_format = BlinkFormatToMediaFormat(copy_to_options->format());
    ...
    const uint32_t offset = copy_to_options->frameOffset();
    const uint32_t total_frames = static_cast<uint32_t>(data_as_f32_bus_->frames());
    ...
    if (media::IsInterleaved(dest_format)) {
        ...
        switch (dest_format) {
        case media::kSampleFormatU8: {
            data_as_f32_bus_->ToInterleavedPartial<media::UnsignedInt8SampleTypeTraits>(offset, frame_count, dest.data());
            return;
        }
        ...
        case media::kSampleFormatS16: {
            ...
            data_as_f32_bus_->ToInterleavedPartial<media::SignedInt16SampleTypeTraits>(offset, frame_count, dest_data);
            return;
        }
        ...
        case media::kSampleFormatS32: {
            ...
            data_as_f32_bus_->ToInterleavedPartial<media::SignedInt32SampleTypeTraits>(offset, frame_count, dest_data);
            return;
        }
        ...
        case media::kSampleFormatF32: {
            ...
            data_as_f32_bus_->ToInterleavedPartial<media::Float32SampleTypeTraits>(offset, frame_count, dest_data);
            return;
        }
    ...
    CHECK_LE(offset_src_data + frame_count, src_data + data_as_f32_bus_->frames());
    switch (dest_format) {
    case media::kSampleFormatPlanarU8: {
        uint8_t* dest_data = dest.data();
        for (uint32_t i = 0; i < frame_count; ++i) {
            dest_data[i] = media::UnsignedInt8SampleTypeTraits::FromFloat(offset_src_data[i]);
        }
        return;
    }
    case media::kSampleFormatPlanarS16: {
        int16_t* dest_data = reinterpret_cast<int16_t*>(dest.data());
        for (uint32_t i = 0; i < frame_count; ++i) {
            dest_data[i] = media::SignedInt16SampleTypeTraits::FromFloat(offset_src_data[i]);
        }
        return;
    }
    case media::kSampleFormatPlanarS32: {
        int32_t* dest_data = reinterpret_cast<int32_t*>(dest.data());
        for (uint32_t i = 0; i < frame_count; ++i) {
            dest_data[i] = media::SignedInt32SampleTypeTraits::FromFloat(offset_src_data[i]);
        }
        return;
    }
    case media::kSampleFormatPlanarF32: {
...
void AudioData::CopyToInterleaved(base::span<uint8_t> dest, AudioDataCopyToOptions* copy_to_options)
{
    ...
    const int frames_to_copy = copy_to_options->hasFrameCount() ? copy_to_options->frameCount() : available_frames;
    ...
    switch (dest_format) {
    case media::kSampleFormatU8:
        ::blink::CopyToInterleaved<uint8_t>(dest.data(), channel_data, frame_offset, frames_to_copy);
        return;
    case media::kSampleFormatS16:
        ::blink::CopyToInterleaved<int16_t>(dest.data(), channel_data, frame_offset, frames_to_copy);
        return;
    case media::kSampleFormatS32:
        ::blink::CopyToInterleaved<int32_t>(dest.data(), channel_data, frame_offset, frames_to_copy);
        return;
    case media::kSampleFormatF32:
        ::blink::CopyToInterleaved<float>(dest.data(), channel_data, frame_offset, frames_to_copy);
        return;
    default:
        NOTREACHED();
    }
...
void AudioData::CopyToPlanar(base::span<uint8_t> dest, AudioDataCopyToOptions* copy_to_options, ExceptionState& exception_state)
{
    const media::SampleFormat src_format = data_->sample_format();
    const media::SampleFormat dest_format = BlinkFormatToMediaFormat(copy_to_options->format());
    CHECK_EQ(RemovePlanar(src_format), RemovePlanar(dest_format));
    ...
    switch (dest_format) {
    case media::kSampleFormatPlanarU8:
        ::blink::CopyToPlanar<uint8_t>(dest.data(), src_data, channels, channel_index, frame_offset, frames_to_copy, exception_state);
        return;
    case media::kSampleFormatPlanarS16:
        ::blink::CopyToPlanar<int16_t>(dest.data(), src_data, channels, channel_index, frame_offset, frames_to_copy, exception_state);
        return;
    case media::kSampleFormatPlanarS32:
        ::blink::CopyToPlanar<int32_t>(dest.data(), src_data, channels, channel_index, frame_offset, frames_to_copy, exception_state);
        return;
    case media::kSampleFormatPlanarF32:
        ::blink::CopyToPlanar<float>(dest.data(), src_data, channels, channel_index, frame_offset, frames_to_copy, exception_state);
        return;
    default:
        NOTREACHED();
    }
