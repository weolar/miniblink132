...
ReclaimableCodec::ReclaimableCodec(CodecType type, ExecutionContext* context)
    ...
    , activity_timer_(context->GetTaskRunner(TaskType::kInternalMedia), this, &ReclaimableCodec::OnActivityTimerFired)
{
    DCHECK(context);
    ...
    observer_handle_ = context->GetScheduler()->AddLifecycleObserver(
        FrameOrWorkerScheduler::ObserverType::kWorkerScheduler, WTF::BindRepeating(&ReclaimableCodec::OnLifecycleStateChanged, WrapWeakPersistent(this)));
}
...
void ReclaimableCodec::SetGlobalPressureExceededFlag(bool global_pressure_exceeded)
{
    if (!is_applying_pressure_) {
...
void ReclaimableCodec::OnLifecycleStateChanged(scheduler::SchedulingLifecycleState lifecycle_state)
{
    DVLOG(5) << __func__ << " lifecycle_state=" << static_cast<int>(lifecycle_state);
    bool is_backgrounded = lifecycle_state != scheduler::SchedulingLifecycleState::kNotThrottled;
...
void ReclaimableCodec::SimulateLifecycleStateForTesting(scheduler::SchedulingLifecycleState state)
{
...
{
    OnCodecReclaimed(MakeGarbageCollected<DOMException>(DOMExceptionCode::kQuotaExceededError, "Codec reclaimed for testing."));
}
...
bool ReclaimableCodec::AreReclamationPreconditionsMet()
{
    ...
    DCHECK_EQ(global_pressure_exceeded_, global_pressure_exceeded_ && is_applying_pressure_);
...
void ReclaimableCodec::OnActivityTimerFired(TimerBase*)
{
    ...
    if (is_inactive && last_tick_was_inactive_) {
        activity_timer_.Stop();
        OnCodecReclaimed(MakeGarbageCollected<DOMException>(DOMExceptionCode::kQuotaExceededError, "Codec reclaimed due to inactivity."));
    }
...
CodecPressureManager* ReclaimableCodec::PressureManager()
{
    ...
    auto& manager_provider = CodecPressureManagerProvider::From(*execution_context);
