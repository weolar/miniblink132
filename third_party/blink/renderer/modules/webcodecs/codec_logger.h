...
void SendPlayerNameInformationInternal(media::MediaLog* media_log, const ExecutionContext& context, std::string loadedAs);
...
template <typename StatusImpl> class MODULES_EXPORT CodecLogger final {
public:
    // Attempts to create CodecLogger backed by a BatchingMediaLog. Falls back to
    // a NullMediaLog on failure.
    CodecLogger(ExecutionContext* context, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    {
        DCHECK(context);
        ...
        if (!context->IsContextDestroyed()) {
            parent_media_log_ = Platform::Current()->GetMediaLog(MediaInspectorContextImpl::From(*context), task_runner,
    ...
    void SendPlayerNameInformation(const ExecutionContext& context, std::string loadedAs)
    {
        internal::SendPlayerNameInformationInternal(media_log_.get(), context, loadedAs);
    }
    ...
    {
        return MakeAndLogException(DOMExceptionCode::kOperationError, std::move(error_msg), status,
    ...
    DOMException* MakeSoftwareCodecOperationError(std::string error_msg, StatusImpl status)
    {
        return MakeAndLogException(DOMExceptionCode::kOperationError, std::move(error_msg), status,
    ...
    DOMException* MakeOperationError(std::string error_msg, typename StatusImpl::Codes code, const base::Location& location = base::Location::Current())
    {
    ...
    {
        return MakeAndLogException(DOMExceptionCode::kEncodingError, std::move(error_msg), status,
            /*can_log_status_message=*/false);
    }

    // Convenience wrapper for MakeEncodingError(), where |error_msg| is shared
    // for both the exception message and the status message.
    DOMException* MakeEncodingError(std::string error_msg, typename StatusImpl::Codes code, const base::Location& location = base::Location::Current())
    {
    ...
    // software codec the error message can include the status message.
    DOMException* MakeSoftwareCodecEncodingError(std::string error_msg, StatusImpl status)
    {
        return MakeAndLogException(DOMExceptionCode::kEncodingError, std::move(error_msg), status,
            /*can_log_status_message=*/true);
    }
...
private:
    DOMException* MakeAndLogException(DOMExceptionCode code, std::string error_msg, StatusImpl status, bool can_log_status_message)
    {
        ...
        auto detailed_message = String::Format("%s (%s)", error_msg.c_str(), status.message().c_str());
        ...
        return MakeGarbageCollected<DOMException>(code, error_msg.c_str(), detailed_message);
    }
