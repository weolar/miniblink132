...
EncodedAudioChunk* EncodedAudioChunk::Create(ScriptState* script_state, const EncodedAudioChunkInit* init, ExceptionState& exception_state)
{
    ...
    if (array_span.empty()) {
        ...
        buffer = media::DecoderBuffer::FromExternalMemory(std::make_unique<ArrayBufferContentsExternalMemory>(std::move(buffer_contents), array_span));
    } else {
    ...
    // handled differently.
    buffer->set_duration(
        init->hasDuration() ? base::Microseconds(std::min(uint64_t { base::TimeDelta::FiniteMax().InMicroseconds() }, init->duration())) : media::kNoTimestamp);
    ...
    if (init->hasDecryptConfig()) {
        ...
        if (!decrypt_config) {
            exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Unsupported decryptConfig");
            return nullptr;
        }
...
{
    return V8EncodedAudioChunkType(buffer_->is_key_frame() ? V8EncodedAudioChunkType::Enum::kKey : V8EncodedAudioChunkType::Enum::kDelta);
}
...
void EncodedAudioChunk::copyTo(const AllowSharedBufferSource* destination, ExceptionState& exception_state)
{
