...
public:
    FakeVideoDecoder(ScriptState* script_state, const VideoDecoderInit* init, ExceptionState& exception_state)
...
private:
    ...
    {
        EXPECT_CALL(*mock_decoder_, GetMaxDecodeRequests()).WillRepeatedly(testing::Return(4));

        EXPECT_CALL(*mock_decoder_, Decode_(_, _)).WillOnce([](Unused, media::VideoDecoder::DecodeCB& decode_cb) {
            scheduler::GetSequencedTaskRunnerForTesting()->PostTask(FROM_HERE, WTF::BindOnce(std::move(decode_cb), media::OkStatus()));
        });

        EXPECT_CALL(*mock_decoder_, Initialize_(_, _, _, _, _, _))
            .WillOnce([quit_closure](Unused, Unused, Unused, media::VideoDecoder::InitCB& init_cb, Unused, Unused) {
                scheduler::GetSequencedTaskRunnerForTesting()->PostTask(FROM_HERE, WTF::BindOnce(std::move(init_cb), media::OkStatus()));
                scheduler::GetSequencedTaskRunnerForTesting()->PostTask(FROM_HERE, std::move(quit_closure));
            });
    }

    MediaDecoderType* decoder() override
    {
...
public:
    ...
    FakeVideoDecoder* CreateFakeDecoder(ScriptState* script_state, VideoDecoderInit* init, ExceptionState& exception_state)
    {
        return MakeGarbageCollected<FakeVideoDecoder>(script_state, init, exception_state);
    }

    VideoDecoderInit* CreateVideoDecoderInit(ScriptState* script_state, MockFunctionScope& mock_functions)
    {
        auto* init = MakeGarbageCollected<VideoDecoderInit>();
        init->setOutput(V8VideoFrameOutputCallback::Create(mock_functions.ExpectNoCall()->ToV8Function(script_state)));
        init->setError(V8WebCodecsErrorCallback::Create(mock_functions.ExpectNoCall()->ToV8Function(script_state)));
        return init;
    }
    ...
    VideoDecoderSupport* ToVideoDecoderSupport(V8TestingScope* v8_scope, ScriptValue value)
    {
        return NativeValueTraits<VideoDecoderSupport>::NativeValue(v8_scope->GetIsolate(), value.V8Value(), v8_scope->GetExceptionState());
    }
...
TEST_F(VideoDecoderTest, HardwareDecodersApplyPressure)
{
    ...
    auto* fake_decoder
        = CreateFakeDecoder(v8_scope.GetScriptState(), CreateVideoDecoderInit(v8_scope.GetScriptState(), mock_functions), v8_scope.GetExceptionState());
...
TEST_F(VideoDecoderTest, ResetReleasesPressure)
{
    ...
    fake_decoder->SimulateLifecycleStateForTesting(scheduler::SchedulingLifecycleState::kHidden);
...
TEST_F(VideoDecoderTest, isConfigureSupportedWithInvalidSWConfig)
{
    ...
    auto promise = VideoDecoder::isConfigSupported(v8_scope.GetScriptState(), config, v8_scope.GetExceptionState());
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
