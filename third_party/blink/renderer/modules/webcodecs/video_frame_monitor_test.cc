...
protected:
    static void SequenceOfOperations(const std::string& source_id)
    {
        ...
        monitor.OnOpenFrame(source_id, media::VideoFrame::ID::FromUnsafeValue(20));
        EXPECT_EQ(monitor.NumFrames(source_id), 2u);
        EXPECT_EQ(monitor.NumRefs(source_id, media::VideoFrame::ID::FromUnsafeValue(20)), 2);

        // JS closes one of its VideoFrames with ID 20
        monitor.OnCloseFrame(source_id, media::VideoFrame::ID::FromUnsafeValue(20));
        EXPECT_EQ(monitor.NumFrames(source_id), 2u);
        EXPECT_EQ(monitor.NumRefs(source_id, media::VideoFrame::ID::FromUnsafeValue(20)), 1);

        {
            base::AutoLock locker(monitor.GetLock());
            monitor.OnOpenFrameLocked(source_id, media::VideoFrame::ID::FromUnsafeValue(30));
            EXPECT_EQ(monitor.NumFramesLocked(source_id), 2u);
            EXPECT_EQ(monitor.NumRefsLocked(source_id, media::VideoFrame::ID::FromUnsafeValue(30)), 2);

            monitor.OnCloseFrameLocked(source_id, media::VideoFrame::ID::FromUnsafeValue(20));
            ...
            monitor.OnCloseFrameLocked(source_id, media::VideoFrame::ID::FromUnsafeValue(30));
            EXPECT_EQ(monitor.NumFramesLocked(source_id), 1u);
            EXPECT_EQ(monitor.NumRefsLocked(source_id, media::VideoFrame::ID::FromUnsafeValue(30)), 1);
        }

        monitor.OnCloseFrame(source_id, media::VideoFrame::ID::FromUnsafeValue(30));
        EXPECT_EQ(monitor.NumRefs(source_id, media::VideoFrame::ID::FromUnsafeValue(30)), 0);
