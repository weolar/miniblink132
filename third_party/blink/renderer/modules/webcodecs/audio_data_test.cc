...
constexpr uint64_t kExpectedDuration = static_cast<int64_t>(kFrames * kMicrosecondPerSecond / kSampleRate);
...
protected:
    ...
    AllowSharedBufferSource* CreateCustomData(int channels, int frames)
    {
        ...
        for (int ch = 0; ch < channels; ++ch) {
            float* plane_start = reinterpret_cast<float*>(buffer->Data()) + ch * frames;
    ...
    AudioData* CreateDefaultAudioData(ScriptState* script_state, ExceptionState& exception_state)
    {
        auto* data = CreateDefaultData();
        auto* audio_data_init = CreateDefaultAudioDataInit(data);
        return MakeGarbageCollected<AudioData>(script_state, audio_data_init, exception_state);
    }

    AudioDataCopyToOptions* CreateCopyToOptions(int index, std::optional<uint32_t> offset, std::optional<uint32_t> count)
    {
    ...
    void VerifyAllocationSize(int plane_index, std::optional<uint32_t> frame_offset, std::optional<uint32_t> frame_count, bool should_throw, int expected_size,
        std::string description)
    {
        V8TestingScope scope;
        auto* frame = CreateDefaultAudioData(scope.GetScriptState(), scope.GetExceptionState());

        auto* options = CreateCopyToOptions(plane_index, frame_offset, frame_count);
        {
            SCOPED_TRACE(description);
            int allocations_size = frame->allocationSize(options, scope.GetExceptionState());
...
{
    const media::ChannelLayout channel_layout = media::ChannelLayout::CHANNEL_LAYOUT_STEREO;
    const int channels = ChannelLayoutToChannelCount(channel_layout);
    constexpr base::TimeDelta timestamp = base::Microseconds(kTimestampInMicroSeconds);
    constexpr int kValueStart = 1;
    constexpr int kValueIncrement = 1;
    scoped_refptr<media::AudioBuffer> media_buffer = media::MakeAudioBuffer<int16_t>(
        media::SampleFormat::kSampleFormatS16, channel_layout, channels, kSampleRate, kValueStart, kValueIncrement, kFrames, timestamp);
...
TEST_F(AudioDataTest, ConstructFromAudioDataInit)
{
    ...
    auto* frame = MakeGarbageCollected<AudioData>(scope.GetScriptState(), audio_data_init, scope.GetExceptionState());
...
TEST_F(AudioDataTest, ConstructFromAudioDataInit_HighChannelCount)
{
    ...
    EXPECT_EQ(frame->sampleRate(), static_cast<uint32_t>(kSampleRate));
    EXPECT_EQ(frame->numberOfFrames(), static_cast<uint32_t>(kFrames));
    EXPECT_EQ(frame->numberOfChannels(), static_cast<uint32_t>(kHighChannelCount));
    EXPECT_EQ(frame->duration(), kExpectedDuration);
    EXPECT_EQ(frame->timestamp(), kTimestampInMicroSeconds);
}
...
TEST_F(AudioDataTest, AllocationSize)
{
    ...
    // Basic cases.
    VerifyAllocationSize(0, std::nullopt, std::nullopt, false, kTotalSizeInBytes, "Default");
    VerifyAllocationSize(1, std::nullopt, std::nullopt, false, kTotalSizeInBytes, "Valid index.");
    VerifyAllocationSize(0, 0, kFrames, false, kTotalSizeInBytes, "Specifying defaults");

    // Cases where we cover a subset of samples.
    VerifyAllocationSize(0, kFrames / 2, std::nullopt, false, kTotalSizeInBytes / 2, "Valid offset, no count");
    VerifyAllocationSize(0, kFrames / 2, kFrames / 4, false, kTotalSizeInBytes / 4, "Valid offset and count");
    VerifyAllocationSize(0, std::nullopt, kFrames / 2, false, kTotalSizeInBytes / 2, "No offset, valid count");

    // Copying 0 frames is technically valid.
    VerifyAllocationSize(0, std::nullopt, 0, false, 0, "Frame count is 0");

    // Failures
    VerifyAllocationSize(2, std::nullopt, std::nullopt, true, 0, "Invalid index.");
...
TEST_F(AudioDataTest, CopyTo_DestinationTooSmall)
{
    ...
    AllowSharedBufferSource* small_dest = MakeGarbageCollected<AllowSharedBufferSource>(DOMArrayBuffer::Create(kFrames - 1, sizeof(float)));
...
TEST_F(AudioDataTest, CopyTo_FullFrames)
{
    ...
    AllowSharedBufferSource* dest = MakeGarbageCollected<AllowSharedBufferSource>(data_copy);
...
TEST_F(AudioDataTest, TransferBuffer)
{
    ...
    auto* audio_data = MakeGarbageCollected<AudioData>(scope.GetScriptState(), audio_data_init, scope.GetExceptionState());
    ...
    uint32_t allocations_size = audio_data->allocationSize(options, scope.GetExceptionState());
...
TEST_F(AudioDataTest, FailToTransferUnAlignedBuffer)
{
    ...
    auto* buffer = DOMArrayBuffer::Create(base::as_byte_span(data));
    auto* view = DOMDataView::Create(buffer, 1 /* offset one byte from the float ptr, that how we are sure that
                   the view is not aligned to sizeof(float) */
        ,
        frames * sizeof(float));
    auto* buffer_source = MakeGarbageCollected<AllowSharedBufferSource>(MaybeShared<DOMArrayBufferView>(view));
...
TEST_F(AudioDataTest, CopyTo_Offset)
{
    ...
    auto* frame = CreateDefaultAudioData(scope.GetScriptState(), scope.GetExceptionState());
    auto* options = CreateCopyToOptions(/*index=*/0, kOffset, /*count=*/std::nullopt);
...
TEST_F(AudioDataTest, CopyTo_PartialFrames)
{
    ...
    auto* frame = CreateDefaultAudioData(scope.GetScriptState(), scope.GetExceptionState());
    auto* options = CreateCopyToOptions(/*index=*/0, /*offset=*/std::nullopt, kPartialFrameCount);

    DOMArrayBuffer* data_copy = DOMArrayBuffer::Create(kPartialFrameCount, sizeof(float));
    AllowSharedBufferSource* dest = MakeGarbageCollected<AllowSharedBufferSource>(data_copy);
...
TEST_F(AudioDataTest, Interleaved)
{
    ...
    auto media_buffer = media::AudioBuffer::CopyFrom(media::SampleFormat::kSampleFormatS16, media::GuessChannelLayout(kInterleavedChannels),
        kInterleavedChannels, kSampleRate, kFrames, data, base::TimeDelta());
    ...
    EXPECT_EQ(static_cast<unsigned int>(allocations_size), kPartialFrameCount * kInterleavedChannels * sizeof(uint16_t));

    DOMArrayBuffer* data_copy = DOMArrayBuffer::Create(kPartialFrameCount * kInterleavedChannels, sizeof(uint16_t));
    AllowSharedBufferSource* dest = MakeGarbageCollected<AllowSharedBufferSource>(data_copy);
...
template <typename SourceTraits, typename TargetTraits> struct ConversionConfig {
...
template <typename TestConfig> class AudioDataConversionTest : public testing::Test {
protected:
    AudioDataInit* CreateAudioDataInit(AllowSharedBufferSource* data, std::string format)
    {
        auto* audio_data_init = AudioDataInit::Create();
        audio_data_init->setData(data);
        audio_data_init->setTimestamp(kTimestampInMicroSeconds);
    ...
    AudioData* CreateAudioData(
        std::string format, bool planar, bool use_offset, bool use_frame_count, ScriptState* script_state, ExceptionState& exception_state)
    {
        auto* data = planar ? CreatePlanarData(use_offset, use_frame_count) : CreateInterleavedData(use_offset, use_frame_count);
    ...
    AudioDataCopyToOptions* CreateCopyToOptions(int plane_index, std::string format, bool use_offset, bool use_frame_count)
    {
        auto* copy_to_options = AudioDataCopyToOptions::Create();
    ...
    AllowSharedBufferSource* CreatePlanarData(bool use_offset, bool use_frame_count)
    {
        ...
        auto* buffer = DOMArrayBuffer::Create(kChannels * kFrames, sizeof(ValueType));
    ...
    // If `use_offset` is true, the first sample of every channel will be 0.
    // If `use_frame_count` is true, the last sample of every channel will be 0.
    AllowSharedBufferSource* CreateInterleavedData(bool use_offset, bool use_frame_count)
    {
        static_assert(kChannels == 2, "CreateInterleavedData() assumes 2 channels.");
        using SourceTraits = TestConfig::From::Traits;
        using ValueType = SourceTraits::ValueType;
    ...
    void TestConversionToPlanar(bool source_is_planar, bool use_offset, bool use_frame_count)
    {
        using Config = TestConfig;
        using TargetType = Config::To::Traits::ValueType;
        constexpr int kChannelToCopy = 1;

        std::string source_format = source_is_planar ? Config::From::PlanarFormat : Config::From::Format;

        // Create original data. The first and last frame will be zero'ed, if
        // `use_offset` or `use_frame_count` are set, respectively.
        V8TestingScope scope;
        auto* audio_data = CreateAudioData(source_format, source_is_planar, use_offset, use_frame_count, scope.GetScriptState(), scope.GetExceptionState());
        ASSERT_FALSE(scope.GetExceptionState().HadException()) << scope.GetExceptionState().Message();

        // Prepare to copy.
        auto* copy_to_options = CreateCopyToOptions(kChannelToCopy, Config::To::PlanarFormat, use_offset, use_frame_count);

        const int frames_to_copy = GetFramesToCopy(use_offset, use_frame_count);
        DOMArrayBuffer* data_copy = DOMArrayBuffer::Create(frames_to_copy, sizeof(TargetType));
        AllowSharedBufferSource* dest = MakeGarbageCollected<AllowSharedBufferSource>(data_copy);
        ...
        EXPECT_FALSE(scope.GetExceptionState().HadException()) << scope.GetExceptionState().Message();
    ...
    void TestConversionToInterleaved(bool source_is_planar, bool use_offset, bool use_frame_count)
    {
        using Config = TestConfig;
        using TargetType = Config::To::Traits::ValueType;
        ...
        // Prepare to copy.
        auto* copy_to_options = CreateCopyToOptions(0, Config::To::Format, use_offset, use_frame_count);

        const int total_frames = GetFramesToCopy(use_offset, use_frame_count) * kChannels;
        DOMArrayBuffer* data_copy = DOMArrayBuffer::Create(total_frames, sizeof(TargetType));
        AllowSharedBufferSource* dest = MakeGarbageCollected<AllowSharedBufferSource>(data_copy);
...
REGISTER_TYPED_TEST_SUITE_P(AudioDataConversionTest, PlanarToPlanar, InterleavedToPlanar, PlanarToInterleaved, InterleavedToInterleaved);

typedef ::testing::Types<ConversionConfig<U8Traits, U8Traits>, ConversionConfig<U8Traits, S16Traits>, ConversionConfig<U8Traits, S32Traits>,
    ConversionConfig<U8Traits, F32Traits>, ConversionConfig<S16Traits, U8Traits>, ConversionConfig<S16Traits, S16Traits>,
    ConversionConfig<S16Traits, S32Traits>, ConversionConfig<S16Traits, F32Traits>, ConversionConfig<S32Traits, U8Traits>,
    ConversionConfig<S32Traits, S16Traits>, ConversionConfig<S32Traits, S32Traits>, ConversionConfig<S32Traits, F32Traits>,
    ConversionConfig<F32Traits, U8Traits>, ConversionConfig<F32Traits, S16Traits>, ConversionConfig<F32Traits, S32Traits>,
    ConversionConfig<F32Traits, F32Traits>>
    TestConfigs;

INSTANTIATE_TYPED_TEST_SUITE_P(CommonTypes, AudioDataConversionTest, TestConfigs);
