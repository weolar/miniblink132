...
public:
    ...
    void copyTo(const AllowSharedBufferSource* destination, AudioDataCopyToOptions*, ExceptionState&);

    scoped_refptr<media::AudioBuffer> data() const
    {
        return data_;
    }
...
private:
    void CopyConvert(base::span<uint8_t> dest, AudioDataCopyToOptions* copy_to_options);

    void CopyToInterleaved(base::span<uint8_t> dest, AudioDataCopyToOptions* copy_to_options);

    void CopyToPlanar(base::span<uint8_t> dest, AudioDataCopyToOptions* copy_to_options, ExceptionState& exception_state);
