...
using MockPressureChangeCallback = base::MockCallback<CodecPressureGauge::PressureThresholdChangedCallback>;

} // namespace

class CodecPressureGaugeTest : public testing::TestWithParam<ReclaimableCodec::CodecType> {
public:
...
TEST_P(CodecPressureGaugeTest, GaugeIsSharedForDecodersEncoders)
{
    ...
    bool gauge_is_shared
        = &CodecPressureGauge::GetInstance(ReclaimableCodec::CodecType::kDecoder) == &CodecPressureGauge::GetInstance(ReclaimableCodec::CodecType::kEncoder);
...
TEST_P(CodecPressureGaugeTest, RegisterUnregisterCallbacks)
{
    ...
    RegistrationResult result_a = PressureGauge().RegisterPressureCallback(callback.Get());
    RegistrationResult result_b = PressureGauge().RegisterPressureCallback(callback.Get());
...
TEST_P(CodecPressureGaugeTest, IncrementDecrement)
{
    ...
    RegistrationResult result = PressureGauge().RegisterPressureCallback(callback.Get());
...
TEST_P(CodecPressureGaugeTest, UnregisterAllLeftoverPressure)
{
    ...
    EXPECT_EQ(kTestPressureThreshold, PressureGauge().global_pressure_for_testing());

    // Releasing all pressure should reset global pressure.
    PressureGauge().UnregisterPressureCallback(result.first, kTestPressureThreshold);
...
TEST_P(CodecPressureGaugeTest, UnregisterPartialLeftoverPressure)
{
    ...
    RegistrationResult other_result = PressureGauge().RegisterPressureCallback(callback.Get());
    ...
    EXPECT_EQ(kTestPressureThreshold - kPartialPressure, PressureGauge().global_pressure_for_testing());

    // Test cleanup
    PressureGauge().UnregisterPressureCallback(other_result.first, PressureGauge().global_pressure_for_testing());
}
...
TEST_P(CodecPressureGaugeTest, ExceedingThresholdRunsCallbacks)
{
    ...
    RegistrationResult other_result = PressureGauge().RegisterPressureCallback(other_callback.Get());
    ...
    // Test cleanup
    PressureGauge().UnregisterPressureCallback(result.first, PressureGauge().global_pressure_for_testing());
...
TEST_P(CodecPressureGaugeTest, RepeatedlyCrossingThresholds)
{
    ...
    RegistrationResult result = PressureGauge().RegisterPressureCallback(base::BindLambdaForTesting(pressure_cb));
...
INSTANTIATE_TEST_SUITE_P(, CodecPressureGaugeTest, testing::Values(ReclaimableCodec::CodecType::kDecoder, ReclaimableCodec::CodecType::kEncoder));
