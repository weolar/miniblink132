...
DEFINE_TEXT_PROTO_FUZZER(const wc_fuzzer::AudioDecoderApiInvocationSequence& proto)
{
    ...
    Persistent<V8AudioDataOutputCallback> output_callback
        = V8AudioDataOutputCallback::Create(MakeGarbageCollected<FakeFunction>("output")->ToV8Function(script_state));

    Persistent<AudioDecoderInit> audio_decoder_init = MakeGarbageCollected<AudioDecoderInit>();
    audio_decoder_init->setError(error_callback);
    audio_decoder_init->setOutput(output_callback);

    Persistent<AudioDecoder> audio_decoder = AudioDecoder::Create(script_state, audio_decoder_init, IGNORE_EXCEPTION_FOR_TESTING);

    if (audio_decoder) {
        for (auto& invocation : proto.invocations()) {
            switch (invocation.Api_case()) {
            case wc_fuzzer::AudioDecoderApiInvocation::kConfigure: {
                AudioDecoderConfig* config = MakeAudioDecoderConfig(invocation.configure());

                // Use the same config to fuzz isConfigSupported().
                AudioDecoder::isConfigSupported(script_state, config, IGNORE_EXCEPTION_FOR_TESTING);

                audio_decoder->configure(config, IGNORE_EXCEPTION_FOR_TESTING);
                break;
            }
            case wc_fuzzer::AudioDecoderApiInvocation::kDecode:
                audio_decoder->decode(MakeEncodedAudioChunk(script_state, invocation.decode().chunk()), IGNORE_EXCEPTION_FOR_TESTING);
                break;
