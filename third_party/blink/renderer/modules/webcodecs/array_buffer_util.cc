    ...
    case AllowSharedBufferSource::ContentType::kArrayBufferAllowShared: {
        ...
        if (buffer && !buffer->IsDetached()) {
            if (buffer->IsShared()) {
                ...
                static_cast<blink::DOMArrayBuffer*>(buffer)->ShareNonSharedForInternalUse(result);
            }
        }
        return result;
    }
    ...
    const HeapVector<Member<DOMArrayBuffer>>& transfer_list, base::span<const uint8_t> data_range, ExceptionState& exception_state, v8::Isolate* isolate)
{
    ...
    for (const Member<DOMArrayBuffer>& array_buffer : transfer_list) {
        ...
        if (!array_buffer->IsDetachable(isolate) || array_buffer->IsDetached()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "Cannot detach ArrayBuffer");
            return {};
        }

        if (!seen_buffers.insert(array_buffer).is_new_entry) {
            // While inserting we found that the buffer has already been seen.
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "Duplicate ArrayBuffers in the transfer list");
            return {};
        }
    }
    ...
    for (const Member<DOMArrayBuffer>& array_buffer : transfer_list) {
        ...
        if (!array_buffer->Transfer(isolate, contents, exception_state) || !contents.IsValid()) {
        ...
        if (data_range.data() < contents_data || data_range.data() + data_range.size() > contents_data + contents.DataLength()) {
