...
template <media::DemuxerStream::Type StreamType> class NullDemuxerStream;

template <media::DemuxerStream::Type StreamType> class DecoderSelector {
public:
    ...
    using DecoderOrError = typename media::DecoderSelector<StreamType>::DecoderOrError;
    ...
    // be Post()'ed.
    DecoderSelector(scoped_refptr<base::SequencedTaskRunner> task_runner, CreateDecodersCB create_decoders_cb, typename Decoder::OutputCB output_cb);
    ...
    void SelectDecoder(const DecoderConfig& config, bool low_delay, SelectDecoderCB select_decoder_cb);
...
private:
    ...
    void OnDecoderSelected(SelectDecoderCB select_decoder_cb, DecoderOrError decoder_or_error, std::unique_ptr<media::DecryptingDemuxerStream>);
...
typedef DecoderSelector<media::DemuxerStream::VIDEO> WebCodecsVideoDecoderSelector;
typedef DecoderSelector<media::DemuxerStream::AUDIO> WebCodecsAudioDecoderSelector;
