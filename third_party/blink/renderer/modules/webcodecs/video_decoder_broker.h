...
public:
    ...
    virtual void OnInitialize(media::DecoderStatus status, std::optional<DecoderDetails> details) = 0;

    virtual void OnDecodeDone(int cb_id, media::DecoderStatus status) = 0;

    virtual void OnDecodeOutput(scoped_refptr<media::VideoFrame> frame, bool can_read_without_stalling) = 0;
...
class MODULES_EXPORT VideoDecoderBroker : public media::VideoDecoder, public CrossThreadVideoDecoderClient {
public:
    // |gpu_factories| may be null when GPU accelerated decoding is not available.
    explicit VideoDecoderBroker(ExecutionContext& execution_context, media::GpuVideoAcceleratorFactories* gpu_factories, media::MediaLog* media_log);
    ...
    void Decode(scoped_refptr<media::DecoderBuffer> buffer, DecodeCB decode_cb) override;
...
private:
    ...
    void OnInitialize(media::DecoderStatus status, std::optional<DecoderDetails> details) override;
    void OnDecodeDone(int cb_id, media::DecoderStatus status) override;
    void OnDecodeOutput(scoped_refptr<media::VideoFrame> frame, bool can_read_without_stalling) override;
