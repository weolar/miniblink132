...
class MODULES_EXPORT ReclaimableCodec : public ExecutionContextLifecycleObserver {
    USING_PRE_FINALIZER(ReclaimableCodec, Dispose);

public:
    // Use 1.5 minutes since some RDP clients are only ticking at 1 FPM.
    static constexpr base::TimeDelta kInactivityReclamationThreshold = base::Seconds(90);
    ...
    bool is_applying_codec_pressure() const
    {
    ...
    bool is_backgrounded_for_testing()
    {
...
protected:
    ...
    CodecPressureManager* get_manager_for_testing()
    {
        return PressureManager();
    }

    base::TimeTicks last_activity_for_testing() const
    {
...
private:
    ...
    std::unique_ptr<FrameOrWorkerScheduler::LifecycleObserverHandle> observer_handle_;
};
