...
WebCodecsLogger::WebCodecsLogger(ExecutionContext& context)
    ...
    , timer_(context.GetTaskRunner(TaskType::kInternalMedia), this, &WebCodecsLogger::LogCloseErrors)
{
}
...
{
    WebCodecsLogger* supplement = Supplement<ExecutionContext>::From<WebCodecsLogger>(context);
    if (!supplement) {
...
scoped_refptr<WebCodecsLogger::VideoFrameCloseAuditor> WebCodecsLogger::GetCloseAuditor()
{
...
void WebCodecsLogger::LogCloseErrors(TimerBase*)
{
    ...
    if (base::TimeTicks::Now() - last_auditor_access_ > kTimerShutdownDelay && close_auditor_->HasOneRef()) {
        timer_.Stop();
    }
