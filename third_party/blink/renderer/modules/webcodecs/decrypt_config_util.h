...
MODULES_EXPORT std::unique_ptr<media::DecryptConfig> CreateMediaDecryptConfig(const DecryptConfig& js_config);

// Attempts to create a media::EncryptionScheme value given the JS version.
// Returns std::nullopt if the scheme is unrecognized.
MODULES_EXPORT std::optional<media::EncryptionScheme> ToMediaEncryptionScheme(const String& scheme);
