...
void VideoFrameMonitor::OnOpenFrame(const std::string& source_id, media::VideoFrame::ID frame_id)
{
    base::AutoLock locker(GetLock());
    OnOpenFrameLocked(source_id, frame_id);
}

void VideoFrameMonitor::OnCloseFrame(const std::string& source_id, media::VideoFrame::ID frame_id)
{
    base::AutoLock locker(GetLock());
...
int VideoFrameMonitor::NumRefs(const std::string& source_id, media::VideoFrame::ID frame_id)
{
    base::AutoLock locker(GetLock());
...
void VideoFrameMonitor::OnOpenFrameLocked(const std::string& source_id, media::VideoFrame::ID frame_id)
{
...
void VideoFrameMonitor::OnCloseFrameLocked(const std::string& source_id, media::VideoFrame::ID frame_id)
{
    DCHECK(!source_id.empty());
    lock_.AssertAcquired();
...
int VideoFrameMonitor::NumRefsLocked(const std::string& source_id, media::VideoFrame::ID frame_id)
{
    DCHECK(!source_id.empty());
    lock_.AssertAcquired();
    auto it = map_.find(source_id);
