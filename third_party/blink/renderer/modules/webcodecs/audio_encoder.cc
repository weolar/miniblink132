...
template <typename T> bool VerifyParameterValues(const T& value, String error_message_base_base, WTF::Vector<T> supported_values, String* js_error_message)
{
...
AudioEncoderTraits::ParsedConfig* ParseAacConfigStatic(
    const AacEncoderConfig* aac_config, AudioEncoderTraits::ParsedConfig* result, ExceptionState& exception_state)
{
...
AudioEncoderTraits::ParsedConfig* ParseOpusConfigStatic(
    const OpusEncoderConfig* opus_config, AudioEncoderTraits::ParsedConfig* result, ExceptionState& exception_state)
{
    ...
    if (complexity > kComplexityUpperBound) {
        exception_state.ThrowTypeError(ExceptionMessages::IndexExceedsMaximumBound<uint32_t>("Opus complexity", complexity, kComplexityUpperBound));
        return nullptr;
    }
    ...
    if (packet_loss_perc > kPacketLossPercUpperBound) {
        exception_state.ThrowTypeError(
            ExceptionMessages::IndexExceedsMaximumBound<uint32_t>("Opus packetlossperc", packet_loss_perc, kPacketLossPercUpperBound));
        return nullptr;
    }
    ...
    if (frame_duration < kFrameDurationLowerBound.InMicroseconds() || frame_duration > kFrameDurationUpperBound.InMicroseconds()) {
        exception_state.ThrowTypeError(
            ExceptionMessages::IndexOutsideRange<uint64_t>("Opus frameDuration", frame_duration, kFrameDurationLowerBound.InMicroseconds(),
                ExceptionMessages::BoundType::kInclusiveBound, kFrameDurationUpperBound.InMicroseconds(), ExceptionMessages::BoundType::kInclusiveBound));
        return nullptr;
    }
    ...
    if (frame_duration % kFrameDurationLowerBound.InMicroseconds() != 0) {
        exception_state.ThrowTypeError(String::Format("Invalid Opus frameDuration; expected a multiple of %" PRIu64 ", received %" PRIu64 ".",
            kFrameDurationLowerBound.InMicroseconds(), frame_duration));
        return nullptr;
    }

    if (opus_config->format().AsEnum() == V8OpusBitstreamFormat::Enum::kOgg) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Opus Ogg format is unsupported");
        return nullptr;
    }
...
AudioEncoderTraits::ParsedConfig* ParseConfigStatic(const AudioEncoderConfig* config, ExceptionState& exception_state)
{
    if (!config) {
    ...
    bool parse_succeeded = ParseAudioCodecString("", config->codec().Utf8(), &is_codec_ambiguous, &result->options.codec);
    ...
    if (result->options.channels == 0) {
        exception_state.ThrowTypeError(String::Format("Invalid channel count; channel count must be non-zero, received %d.", result->options.channels));
        return nullptr;
    }

    result->options.sample_rate = config->sampleRate();
    if (result->options.sample_rate == 0) {
        exception_state.ThrowTypeError(String::Format("Invalid sample rate; sample rate must be non-zero, received %d.", result->options.sample_rate));
        return nullptr;
    }
    ...
    if (config->hasBitrate()) {
        if (config->bitrate() > std::numeric_limits<int>::max()) {
            exception_state.ThrowTypeError(
                String::Format("Bitrate is too large; expected at most %d, received %" PRIu64, std::numeric_limits<int>::max(), config->bitrate()));
            return nullptr;
        }
    ...
    if (config->hasBitrateMode()) {
        result->options.bitrate_mode = config->bitrateMode().AsEnum() == V8BitrateMode::Enum::kConstant ? media::AudioEncoder::BitrateMode::kConstant
                                                                                                        : media::AudioEncoder::BitrateMode::kVariable;
    }

    switch (result->options.codec) {
    case media::AudioCodec::kOpus:
        return ParseOpusConfigStatic(config->hasOpus() ? config->opus() : OpusEncoderConfig::Create(), result, exception_state);
    case media::AudioCodec::kAAC: {
        auto* aac_config = config->hasAac() ? config->aac() : AacEncoderConfig::Create();
...
bool VerifyCodecSupportStatic(AudioEncoderTraits::ParsedConfig* config, String* js_error_message)
{
    if (config->options.channels < 1 || config->options.channels > media::limits::kMaxChannels) {
        *js_error_message = String::Format("Unsupported channel count; expected range from %d to "
                                           "%d, received %d.",
            1, media::limits::kMaxChannels, config->options.channels);
        return false;
    }

    if (config->options.sample_rate < media::limits::kMinSampleRate || config->options.sample_rate > media::limits::kMaxSampleRate) {
        *js_error_message = String::Format("Unsupported sample rate; expected range from %d to %d, "
                                           "received %d.",
            media::limits::kMinSampleRate, media::limits::kMaxSampleRate, config->options.sample_rate);
        return false;
    }
    ...
    case media::AudioCodec::kOpus: {
        ...
        // durations.
        if (!VerifyParameterValues(config->options.opus->frame_duration.InMicroseconds(), "Unsupported Opus frameDuration.",
        ...
        if (config->options.channels > 2) {
            ...
            *js_error_message = String::Format("Too many channels for Opus encoder; "
                                               "expected at most 2, received %d.",
                config->options.channels);
            return false;
        }
        if (config->options.bitrate.has_value() && config->options.bitrate.value() < media::AudioOpusEncoder::kMinBitrate) {
            *js_error_message = String::Format(
                "Opus bitrate is too low; expected at least %d, received %d.", media::AudioOpusEncoder::kMinBitrate, config->options.bitrate.value());
            return false;
        }
        return true;
    }
    case media::AudioCodec::kAAC: {
        if (media::MojoAudioEncoder::IsSupported(media::AudioCodec::kAAC)) {
            if (!VerifyParameterValues(config->options.channels, "Unsupported number of channels.", { 1, 2, 6 }, js_error_message)) {
                return false;
            }
            if (config->options.bitrate.has_value()) {
                if (!VerifyParameterValues(config->options.bitrate.value(), "Unsupported bitrate.", { 96000, 128000, 160000, 192000 }, js_error_message)) {
                    return false;
                }
            }
            if (!VerifyParameterValues(config->options.sample_rate, "Unsupported sample rate.", { 44100, 48000 }, js_error_message)) {
                return false;
            }
            return true;
        }
...
std::unique_ptr<media::AudioEncoder> CreateSoftwareAudioEncoder(media::AudioCodec codec)
{
    if (codec != media::AudioCodec::kOpus)
        return nullptr;
    auto software_encoder = std::make_unique<media::AudioOpusEncoder>();
    return std::make_unique<media::OffloadingAudioEncoder>(std::move(software_encoder));
}

std::unique_ptr<media::AudioEncoder> CreatePlatformAudioEncoder(media::AudioCodec codec)
{
    ...
    Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(pending_interface_factory.InitWithNewPipeAndPassReceiver());
    interface_factory.Bind(std::move(pending_interface_factory));

    mojo::PendingRemote<media::mojom::AudioEncoder> encoder_remote;
    interface_factory->CreateAudioEncoder(encoder_remote.InitWithNewPipeAndPassReceiver());
...
AudioEncoder* AudioEncoder::Create(ScriptState* script_state, const AudioEncoderInit* init, ExceptionState& exception_state)
{
    auto* result = MakeGarbageCollected<AudioEncoder>(script_state, init, exception_state);
    return exception_state.HadException() ? nullptr : result;
}

AudioEncoder::AudioEncoder(ScriptState* script_state, const AudioEncoderInit* init, ExceptionState& exception_state)
...
std::unique_ptr<media::AudioEncoder> AudioEncoder::CreateMediaAudioEncoder(const ParsedConfig& config)
{
...
void AudioEncoder::ProcessConfigure(Request* request)
{
    ...
    if (!media_encoder_) {
        blocking_request_in_progress_ = request;
        QueueHandleError(MakeOperationError("Encoder creation error.",
            media::EncoderStatus(media::EncoderStatus::Codes::kEncoderInitializationError,
    ...
    auto output_cb = ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&AudioEncoder::CallOutputCallback, MakeUnwrappingCrossThreadWeakHandle(this),
    ...
    auto done_callback = [](AudioEncoder* self, media::AudioCodec codec, Request* req, media::EncoderStatus status) {
        ...
        if (!status.is_ok()) {
            self->HandleError(self->MakeOperationError("Encoding error.", std::move(status)));
        } else {
            base::UmaHistogramEnumeration("Blink.WebCodecs.AudioEncoder.Codec", codec);
        }
    ...
    media_encoder_->Initialize(active_config_->options, std::move(output_cb),
        ConvertToBaseOnceCallback(CrossThreadBindOnce(
            done_callback, MakeUnwrappingCrossThreadWeakHandle(this), active_config_->options.codec, MakeUnwrappingCrossThreadHandle(request))));
}
...
void AudioEncoder::ProcessEncode(Request* request)
{
    ...
    auto done_callback = [](AudioEncoder* self, Request* req, media::EncoderStatus status) {
        if (!self || self->reset_count_ != req->reset_count) {
            req->EndTracing(/*aborted=*/true);
            return;
        }
        DCHECK_CALLED_ON_VALID_SEQUENCE(self->sequence_checker_);
        if (!status.is_ok()) {
            self->HandleError(self->MakeEncodingError("Encoding error.", std::move(status)));
        }
    ...
    if (data->channel_count() != active_config_->options.channels || data->sample_rate() != active_config_->options.sample_rate) {
        // Per spec we must queue a task for error handling.
        QueueHandleError(MakeEncodingError("Input audio buffer is incompatible with codec parameters",
    ...
    media_encoder_->Encode(std::move(audio_bus), timestamp,
        ConvertToBaseOnceCallback(CrossThreadBindOnce(done_callback, MakeUnwrappingCrossThreadWeakHandle(this), MakeUnwrappingCrossThreadHandle(request))));
...
AudioEncoder::ParsedConfig* AudioEncoder::ParseConfig(const AudioEncoderConfig* opts, ExceptionState& exception_state)
{
    return ParseConfigStatic(opts, exception_state);
}

bool AudioEncoder::CanReconfigure(ParsedConfig& original_config, ParsedConfig& new_config)
{
    return original_config.options.codec == new_config.options.codec && original_config.options.channels == new_config.options.channels
        && original_config.options.bitrate == new_config.options.bitrate && original_config.options.sample_rate == new_config.options.sample_rate;
}

bool AudioEncoder::VerifyCodecSupport(ParsedConfig* config, String* js_error_message)
{
    return VerifyCodecSupportStatic(config, js_error_message);
}

void AudioEncoder::CallOutputCallback(ParsedConfig* active_config, uint32_t reset_count, media::EncodedAudioBuffer encoded_buffer,
    std::optional<media::AudioEncoder::CodecDescription> codec_desc)
{
    ...
    auto buffer = media::DecoderBuffer::FromArray(std::move(encoded_buffer.encoded_data));
...
// static
ScriptPromise<AudioEncoderSupport> AudioEncoder::isConfigSupported(ScriptState* script_state, const AudioEncoderConfig* config, ExceptionState& exception_state)
{
    ...
    support->setSupported(VerifyCodecSupportStatic(parsed_config, &unused_js_error_message));
...
DOMException* AudioEncoder::MakeOperationError(std::string error_msg, media::EncoderStatus status)
{
...
DOMException* AudioEncoder::MakeEncodingError(std::string error_msg, media::EncoderStatus status)
{
    if (is_platform_encoder_) {
