...
using AllowSharedBufferSource = V8UnionArrayBufferAllowSharedOrArrayBufferViewAllowShared;

// Helper function for turning various DOMArray-like things into a pointer+size.
template <typename T> base::span<T> AsSpan(const AllowSharedBufferSource* buffer_union)
{
    ...
    case AllowSharedBufferSource::ContentType::kArrayBufferAllowShared: {
        ...
        return (buffer && !buffer->IsDetached()) ? base::span<T>(reinterpret_cast<T*>(buffer->DataMaybeShared()), buffer->ByteLength()) : base::span<T>();
    }
    case AllowSharedBufferSource::ContentType::kArrayBufferViewAllowShared: {
        ...
        return (buffer && !buffer->IsDetached()) ? base::span<T>(reinterpret_cast<T*>(buffer->BaseAddressMaybeShared()), buffer->byteLength())
                                                 : base::span<T>();
    }
    }
}
...
ArrayBufferContents PinArrayBufferContent(const AllowSharedBufferSource* buffer_union);
...
ArrayBufferContents TransferArrayBufferForSpan(
    const HeapVector<Member<DOMArrayBuffer>>& transfer_list, base::span<const uint8_t> data_range, ExceptionState& exception_state, v8::Isolate* isolate);

// A wrapper around `ArrayBuffer` contents that allows to transfer ownership
// to `DecoderBuffer`.
class ArrayBufferContentsExternalMemory : public media::DecoderBuffer::ExternalMemory {
public:
    explicit ArrayBufferContentsExternalMemory(ArrayBufferContents contents, base::span<const uint8_t> span)
