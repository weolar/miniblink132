...
DEFINE_TEXT_PROTO_FUZZER(const wc_fuzzer::VideoDecoderApiInvocationSequence& proto)
{
    ...
    Persistent<V8VideoFrameOutputCallback> output_callback
        = V8VideoFrameOutputCallback::Create(MakeGarbageCollected<FakeFunction>("output")->ToV8Function(script_state));

    Persistent<VideoDecoderInit> video_decoder_init = MakeGarbageCollected<VideoDecoderInit>();
    video_decoder_init->setError(error_callback);
    video_decoder_init->setOutput(output_callback);

    Persistent<VideoDecoder> video_decoder = VideoDecoder::Create(script_state, video_decoder_init, IGNORE_EXCEPTION_FOR_TESTING);

    if (video_decoder) {
        for (auto& invocation : proto.invocations()) {
            switch (invocation.Api_case()) {
            case wc_fuzzer::VideoDecoderApiInvocation::kConfigure: {
                VideoDecoderConfig* config = MakeVideoDecoderConfig(invocation.configure());
            ...
            case wc_fuzzer::VideoDecoderApiInvocation::kDecode:
                video_decoder->decode(MakeEncodedVideoChunk(script_state, invocation.decode().chunk()), IGNORE_EXCEPTION_FOR_TESTING);
                break;
