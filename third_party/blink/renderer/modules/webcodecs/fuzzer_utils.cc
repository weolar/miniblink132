...
base::ScopedClosureRunner MakeScopedGarbageCollectionRequest(v8::Isolate* isolate)
{
...
FakeFunction::FakeFunction(std::string name)
    : name_(std::move(name))
{
}
...
VideoDecoderConfig* MakeVideoDecoderConfig(const wc_fuzzer::ConfigureVideoDecoder& proto)
{
    auto* config = VideoDecoderConfig::Create();
    config->setCodec(proto.codec().c_str());
    DOMArrayBuffer* data_copy = DOMArrayBuffer::Create(proto.description().data(), proto.description().size());
    config->setDescription(MakeGarbageCollected<AllowSharedBufferSource>(data_copy));
    return config;
}

AudioDecoderConfig* MakeAudioDecoderConfig(const wc_fuzzer::ConfigureAudioDecoder& proto)
{
...
VideoEncoderConfig* MakeVideoEncoderConfig(const wc_fuzzer::ConfigureVideoEncoder& proto)
{
...
AudioEncoderConfig* MakeAudioEncoderConfig(const wc_fuzzer::ConfigureAudioEncoder& proto)
{
...
String ToAccelerationType(wc_fuzzer::ConfigureVideoEncoder_EncoderAccelerationPreference type)
{
    switch (type) {
...
String ToBitrateMode(wc_fuzzer::ConfigureVideoEncoder_VideoEncoderBitrateMode mode)
{
    switch (mode) {
...
EncodedVideoChunk* MakeEncodedVideoChunk(ScriptState* script_state, const wc_fuzzer::EncodedVideoChunk& proto)
{
    auto* data = MakeGarbageCollected<AllowSharedBufferSource>(DOMArrayBuffer::Create(proto.data().data(), proto.data().size()));
    ...
    return EncodedVideoChunk::Create(script_state, init, IGNORE_EXCEPTION_FOR_TESTING);
}

EncodedAudioChunk* MakeEncodedAudioChunk(ScriptState* script_state, const wc_fuzzer::EncodedAudioChunk& proto)
{
    auto* data = MakeGarbageCollected<AllowSharedBufferSource>(DOMArrayBuffer::Create(proto.data().data(), proto.data().size()));
    ...
    return EncodedAudioChunk::Create(script_state, init, IGNORE_EXCEPTION_FOR_TESTING);
}

VideoEncoderEncodeOptions* MakeEncodeOptions(const wc_fuzzer::EncodeVideo_EncodeOptions& proto)
{
...
BufferAndSource MakeAllowSharedBufferSource(const wc_fuzzer::AllowSharedBufferSource& proto)
{
    BufferAndSource result = {};
    size_t length = std::min(static_cast<size_t>(proto.length()), kMaxBufferLength);
    ...
    size_t view_offset = std::min(static_cast<size_t>(proto.view_offset()), length);
    size_t view_length = std::min(static_cast<size_t>(proto.view_length()), length - view_offset);
    ...
    case wc_fuzzer::AllowSharedBufferSource_ViewType_INT8:
        result.source = MakeGarbageCollected<AllowSharedBufferSource>(MaybeShared<DOMInt8Array>(DOMInt8Array::Create(buffer, view_offset, view_length)));
        break;
    case wc_fuzzer::AllowSharedBufferSource_ViewType_UINT32:
        ...
        result.source = MakeGarbageCollected<AllowSharedBufferSource>(MaybeShared<DOMUint32Array>(DOMUint32Array::Create(buffer, view_offset, view_length)));
        break;
    case wc_fuzzer::AllowSharedBufferSource_ViewType_DATA:
        result.source = MakeGarbageCollected<AllowSharedBufferSource>(MaybeShared<DOMDataView>(DOMDataView::Create(buffer, view_offset, view_length)));
    }
...
VideoColorSpaceInit* MakeVideoColorSpaceInit(const wc_fuzzer::VideoColorSpaceInit& proto)
{
    ...
    if (proto.has_transfer()) {
        ...
        case wc_fuzzer::VideoColorSpaceInit_VideoTransferCharacteristics_VTC_BT709:
            init->setTransfer("bt709");
            break;
        case wc_fuzzer::VideoColorSpaceInit_VideoTransferCharacteristics_VTC_SMPTE170M:
            init->setTransfer("smpte170m");
            break;
        case wc_fuzzer::VideoColorSpaceInit_VideoTransferCharacteristics_VTC_IEC61966_2_1:
            init->setTransfer("iec61966-2-1");
            break;
        case wc_fuzzer::VideoColorSpaceInit_VideoTransferCharacteristics_VTC_LINEAR:
    ...
    if (proto.has_matrix()) {
        ...
        case wc_fuzzer::VideoColorSpaceInit_VideoMatrixCoefficients_VMC_SMPTE170M:
            ...
            break;
        case wc_fuzzer::VideoColorSpaceInit_VideoMatrixCoefficients_VMC_BT2020_NCL:
...
VideoFrame* MakeVideoFrame(ScriptState* script_state, const wc_fuzzer::VideoFrameBufferInitInvocation& proto)
{
    ...
    init->setCodedWidth(std::min(proto.init().coded_width(), kMaxVideoFrameDimension));
    init->setCodedHeight(std::min(proto.init().coded_height(), kMaxVideoFrameDimension));
    ...
    return VideoFrame::Create(script_state, data.source, init, IGNORE_EXCEPTION_FOR_TESTING);
}

VideoFrame* MakeVideoFrame(ScriptState* script_state, const wc_fuzzer::VideoFrameBitmapInit& proto)
{
    ...
    NotShared<DOMUint8ClampedArray> data_u8(DOMUint8ClampedArray::Create(base::as_byte_span(proto.rgb_bitmap()).first(bitmap_size)));

    ImageData* image_data = ImageData::Create(data_u8, proto.bitmap_width(), IGNORE_EXCEPTION_FOR_TESTING);
...
AudioData* MakeAudioData(ScriptState* script_state, const wc_fuzzer::AudioDataInit& proto)
{
    if (!proto.channels().size() || proto.channels().size() > media::limits::kMaxChannels)
        return nullptr;

    if (!proto.length() || proto.length() > media::limits::kMaxSamplesPerPacket / 4) {
        return nullptr;
    }

    V8AudioSampleFormat format = V8AudioSampleFormat::Create(ToAudioSampleFormat(proto.format())).value();
    ...
    for (int i = 0; i < proto.channels().size(); i++) {
        ...
        void* plane_start = reinterpret_cast<uint8_t*>(buffer->Data()) + i * max_plane_size;
...
AudioDataCopyToOptions* MakeAudioDataCopyToOptions(const wc_fuzzer::AudioDataCopyToOptions& options_proto)
{
