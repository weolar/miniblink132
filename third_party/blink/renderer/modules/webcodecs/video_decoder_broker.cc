...
template <> struct CrossThreadCopier<media::DecoderStatus> : public CrossThreadCopierPassThrough<media::DecoderStatus> {
    STATIC_ONLY(CrossThreadCopier);
};

template <> struct CrossThreadCopier<std::optional<DecoderDetails>> : public CrossThreadCopierPassThrough<std::optional<DecoderDetails>> {
    STATIC_ONLY(CrossThreadCopier);
};
...
public:
    using CrossThreadOnceInitCB = WTF::CrossThreadOnceFunction<void(media::DecoderStatus status, std::optional<DecoderDetails>)>;
    using CrossThreadOnceDecodeCB = WTF::CrossThreadOnceFunction<void(const media::DecoderStatus&)>;
    using CrossThreadOnceResetCB = WTF::CrossThreadOnceClosure;

    MediaVideoTaskWrapper(base::WeakPtr<CrossThreadVideoDecoderClient> weak_client, ExecutionContext& execution_context,
        media::GpuVideoAcceleratorFactories* gpu_factories, std::unique_ptr<media::MediaLog> media_log,
        scoped_refptr<base::SequencedTaskRunner> media_task_runner, scoped_refptr<base::SequencedTaskRunner> main_task_runner)
        ...
        , media_log_(std::move(media_log))
    {
        ...
        Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(media_interface_factory.InitWithNewPipeAndPassReceiver());
        ...
        PostCrossThreadTask(*media_task_runner_, FROM_HERE,
            WTF::CrossThreadBindOnce(&MediaVideoTaskWrapper::BindOnTaskRunner, WTF::CrossThreadUnretained(this), std::move(media_interface_factory)));

#if BUILDFLAG(IS_FUCHSIA)
        execution_context.GetBrowserInterfaceBroker().GetInterface(fuchsia_media_codec_provider_.InitWithNewPipeAndPassReceiver());
#endif
    ...
    void Initialize(const media::VideoDecoderConfig& config, bool low_delay)
    {
        ...
        selector_ = std::make_unique<WebCodecsVideoDecoderSelector>(media_task_runner_,
            ...
            WTF::BindRepeating(&MediaVideoTaskWrapper::OnCreateDecoders, WTF::Unretained(this)),
            WTF::BindRepeating(&MediaVideoTaskWrapper::OnDecodeOutput, weak_factory_.GetWeakPtr()));

        selector_->SelectDecoder(config, low_delay, WTF::BindOnce(&MediaVideoTaskWrapper::OnDecoderSelected, weak_factory_.GetWeakPtr()));
    }
    ...
    void Decode(scoped_refptr<media::DecoderBuffer> buffer, int cb_id)
    {
        ...
        decoder_->Decode(std::move(buffer), WTF::BindOnce(&MediaVideoTaskWrapper::OnDecodeDone, weak_factory_.GetWeakPtr(), cb_id));
    }
    ...
    void Reset(int cb_id)
    {
        ...
        decoder_->Reset(WTF::BindOnce(&MediaVideoTaskWrapper::OnReset, weak_factory_.GetWeakPtr(), cb_id));
    }
...
private:
    void BindOnTaskRunner(mojo::PendingRemote<media::mojom::InterfaceFactory> interface_factory)
    {
        DVLOG(2) << __func__;
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    void UpdateDecoderFactory()
    {
        ...
        if (hardware_preference_ != HardwarePreference::kPreferSoftware) {
            ...
            external_decoder_factory = std::make_unique<media::MojoDecoderFactory>(media_interface_factory_.get());
#elif BUILDFLAG(IS_FUCHSIA)
            DCHECK(fuchsia_media_codec_provider_);
            external_decoder_factory = std::make_unique<media::FuchsiaDecoderFactory>(std::move(fuchsia_media_codec_provider_),
        ...
        decoder_factory_ = std::make_unique<media::DefaultDecoderFactory>(std::move(external_decoder_factory));
    }
    ...
    std::vector<std::unique_ptr<media::VideoDecoder>> OnCreateDecoders()
    {
        ...
        if (decoder_factory_) {
            decoder_factory_->CreateVideoDecoders(media_task_runner_, gpu_factories_, media_log_.get(),
                WTF::BindRepeating(&MediaVideoTaskWrapper::OnRequestOverlayInfo, weak_factory_.GetWeakPtr()), target_color_space_, &video_decoders);
        }
    ...
    void OnDecoderSelected(std::unique_ptr<media::VideoDecoder> decoder)
    {
        ...
        if (decoder_) {
            decoder_details = DecoderDetails(
                { decoder_->GetDecoderType(), decoder_->IsPlatformDecoder(), decoder_->NeedsBitstreamConversion(), decoder_->GetMaxDecodeRequests() });
        } else {
        ...
        // Fire |init_cb|.
        PostCrossThreadTask(
            *main_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&CrossThreadVideoDecoderClient::OnInitialize, weak_client_, status, decoder_details));
    }
    ...
    void OnDecodeOutput(scoped_refptr<media::VideoFrame> frame)
    {
        ...
        PostCrossThreadTask(*main_task_runner_, FROM_HERE,
            WTF::CrossThreadBindOnce(&CrossThreadVideoDecoderClient::OnDecodeOutput, weak_client_, std::move(frame), decoder_->CanReadWithoutStalling()));
    }
    ...
    void OnDecodeDone(int cb_id, media::DecoderStatus status)
    {
        ...
        PostCrossThreadTask(
            *main_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&CrossThreadVideoDecoderClient::OnDecodeDone, weak_client_, cb_id, std::move(status)));
    }
    ...
    void OnReset(int cb_id)
    {
        DVLOG(2) << __func__;
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        PostCrossThreadTask(*main_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&CrossThreadVideoDecoderClient::OnReset, weak_client_, cb_id));
    }

    base::WeakPtr<CrossThreadVideoDecoderClient> weak_client_;
    scoped_refptr<base::SequencedTaskRunner> media_task_runner_;
    scoped_refptr<base::SequencedTaskRunner> main_task_runner_;
    raw_ptr<media::GpuVideoAcceleratorFactories, DanglingUntriaged> gpu_factories_;
...
#if BUILDFLAG(IS_FUCHSIA)
    mojo::PendingRemote<media::mojom::FuchsiaMediaCodecProvider> fuchsia_media_codec_provider_;
#endif
...
VideoDecoderBroker::VideoDecoderBroker(ExecutionContext& execution_context, media::GpuVideoAcceleratorFactories* gpu_factories, media::MediaLog* media_log)
    : media_task_runner_(gpu_factories
...
{
    DVLOG(2) << __func__;
    media_tasks_ = std::make_unique<MediaVideoTaskWrapper>(weak_factory_.GetWeakPtr(), execution_context, gpu_factories, media_log->Clone(), media_task_runner_,
...
{
    return decoder_details_ ? decoder_details_->decoder_id : media::VideoDecoderType::kBroker;
}
...
void VideoDecoderBroker::SetHardwarePreference(HardwarePreference hardware_preference)
{
    PostCrossThreadTask(*media_task_runner_, FROM_HERE,
        WTF::CrossThreadBindOnce(&MediaVideoTaskWrapper::UpdateHardwarePreference, WTF::CrossThreadUnretained(media_tasks_.get()), hardware_preference));
}

void VideoDecoderBroker::Initialize(const media::VideoDecoderConfig& config, bool low_delay, media::CdmContext* cdm_context, InitCB init_cb,
    const OutputCB& output_cb, const media::WaitingCB& waiting_cb)
{
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    PostCrossThreadTask(*media_task_runner_, FROM_HERE,
        WTF::CrossThreadBindOnce(&MediaVideoTaskWrapper::Initialize, WTF::CrossThreadUnretained(media_tasks_.get()), config, low_delay));
}
...
int VideoDecoderBroker::CreateCallbackId()
{
    ...
    while (++last_callback_id_ == 0 || last_callback_id_ == std::numeric_limits<uint32_t>::max() || pending_decode_cb_map_.Contains(last_callback_id_)
...
void VideoDecoderBroker::OnInitialize(media::DecoderStatus status, std::optional<DecoderDetails> details)
{
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(init_cb_);
...
void VideoDecoderBroker::Decode(scoped_refptr<media::DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    PostCrossThreadTask(*media_task_runner_, FROM_HERE,
        WTF::CrossThreadBindOnce(&MediaVideoTaskWrapper::Decode, WTF::CrossThreadUnretained(media_tasks_.get()), buffer, callback_id));
}
...
void VideoDecoderBroker::Reset(base::OnceClosure reset_cb)
{
    ...
    PostCrossThreadTask(
        *media_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&MediaVideoTaskWrapper::Reset, WTF::CrossThreadUnretained(media_tasks_.get()), callback_id));
}

bool VideoDecoderBroker::NeedsBitstreamConversion() const
{
    return decoder_details_ ? decoder_details_->needs_bitstream_conversion : false;
}
...
void VideoDecoderBroker::OnDecodeOutput(scoped_refptr<media::VideoFrame> frame, bool can_read_without_stalling)
{
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(output_cb_);
