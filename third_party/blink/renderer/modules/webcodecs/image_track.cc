...
ImageTrack::ImageTrack(ImageTrackList* image_track_list, wtf_size_t id, uint32_t frame_count, int repetition_count, bool selected)
    : id_(id)
...
{
    if (repetition_count_ == kAnimationNone || repetition_count_ == kAnimationLoopOnce) {
        return 0;
    }
