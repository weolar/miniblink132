...
{
    return is_animated ? ImageDecoder::AnimationOption::kPreferAnimation : ImageDecoder::AnimationOption::kPreferStillImage;
}

DOMException* CreateUnsupportedImageTypeException(String type)
{
    return MakeGarbageCollected<DOMException>(
        DOMExceptionCode::kNotSupportedError, String::Format("The provided image type (%s) is not supported", type.Ascii().c_str()));
}
...
public:
    explicit ArrayBufferContentsSegmentReader(ArrayBufferContents contents)
        ...
        , segment_reader_(SegmentReader::CreateFromSkData(SkData::MakeWithoutCopy(contents_.Data(), contents_.DataLength())))
    {
    }

    size_t size() const override
    {
        return segment_reader_->size();
    }
...
// static
ImageDecoderExternal* ImageDecoderExternal::Create(ScriptState* script_state, const ImageDecoderInit* init, ExceptionState& exception_state)
{
    auto* result = MakeGarbageCollected<ImageDecoderExternal>(script_state, init, exception_state);
    return exception_state.HadException() ? nullptr : result;
}

ImageDecoderExternal::DecodeRequest::DecodeRequest(ScriptPromiseResolver<ImageDecodeResult>* resolver, uint32_t frame_index, bool complete_frames_only)
    : resolver(resolver)
...
// static
ScriptPromise<IDLBoolean> ImageDecoderExternal::isTypeSupported(ScriptState* script_state, String type)
{
...
ImageDecoderExternal::ImageDecoderExternal(ScriptState* script_state, const ImageDecoderInit* init, ExceptionState& exception_state)
    : ActiveScriptWrappable<ImageDecoderExternal>({})
    , ExecutionContextLifecycleObserver(ExecutionContext::From(script_state))
    , script_state_(script_state)
    , tracks_(MakeGarbageCollected<ImageTrackList>(this))
    , completed_property_(MakeGarbageCollected<CompletedProperty>(GetExecutionContext()))
{
    // ImageDecoder requires an active context to operate correctly.
    if (GetExecutionContext()->IsContextDestroyed()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Invalid context.");
        return;
    }
    ...
    decode_task_runner_ = base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_VISIBLE, base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN });

    if (init->data()->IsReadableStream()) {
        if (init->data()->GetAsReadableStream()->IsLocked() || init->data()->GetAsReadableStream()->IsDisturbed()) {
            exception_state.ThrowTypeError("ImageDecoder can only accept readable streams that are not yet "
                                           "locked to a reader");
            return;
        }

        decoder_ = std::make_unique<WTF::SequenceBound<ImageDecoderCore>>(decode_task_runner_, mime_type_, /*data=*/nullptr,
            /*data_complete=*/false, color_behavior, desired_size, animation_option_);

        consumer_ = MakeGarbageCollected<ReadableStreamBytesConsumer>(script_state, init->data()->GetAsReadableStream());
    ...
    auto buffer_contents = TransferArrayBufferForSpan(init->transfer(), array_span, exception_state, script_state_->GetIsolate());
    ...
    if (buffer_contents.IsValid()) {
        segment_reader = base::MakeRefCounted<ArrayBufferContentsSegmentReader>(std::move(buffer_contents));
    } else {
        segment_reader = SegmentReader::CreateFromSkData(SkData::MakeWithCopy(array_span.data(), array_span.size()));
    }

    if (!segment_reader) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Failed to read image data");
        return;
    }
    ...
    decoder_ = std::make_unique<WTF::SequenceBound<ImageDecoderCore>>(
        decode_task_runner_, mime_type_, std::move(segment_reader), data_complete_, color_behavior, desired_size, animation_option_);
...
ScriptPromise<ImageDecodeResult> ImageDecoderExternal::decode(const ImageDecodeOptions* options)
{
    DVLOG(1) << __func__;
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<ImageDecodeResult>>(script_state_);
    auto promise = resolver->Promise();

    if (closed_) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "The decoder has been closed."));
        return promise;
    }
    ...
    if (!tracks_->IsEmpty() && !tracks_->selectedTrack()) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "No selected track."));
        return promise;
    }

    pending_decodes_.push_back(
        MakeGarbageCollected<DecodeRequest>(resolver, options ? options->frameIndex() : 0, options ? options->completeFramesOnly() : true));
...
void ImageDecoderExternal::UpdateSelectedTrack()
{
    ...
    reset(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Aborted by track change"));
    ...
    animation_option_ = AnimationOptionFromIsAnimated(tracks_->selectedTrack()->animated());

    decoder_->AsyncCall(&ImageDecoderCore::Reinitialize).WithArgs(animation_option_);
...
ScriptPromise<IDLUndefined> ImageDecoderExternal::completed(ScriptState* script_state)
{
...
{
    if (!exception) {
        exception = MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Aborted by reset.");
    }
...
void ImageDecoderExternal::close()
{
    ...
    auto* exception = MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, failed_ ? "Aborted by close." : "Aborted by failure.");
...
void ImageDecoderExternal::OnStateChange()
{
    ...
    while (!internal_data_complete_) {
        ...
        const bool data_complete = result == BytesConsumer::Result::kDone || result == BytesConsumer::Result::kError;
        if (!buffer.empty() || data_complete != internal_data_complete_) {
            decoder_->AsyncCall(&ImageDecoderCore::AppendData).WithArgs(std::move(data), data_complete);
...
{
    auto* exception = MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Aborted by close.");
    CloseInternal(exception);
}

bool ImageDecoderExternal::HasPendingActivity() const
{
    const bool has_pending_activity = !pending_decodes_.empty() || pending_metadata_requests_ > 0;
...
void ImageDecoderExternal::MaybeSatisfyPendingDecodes()
{
    ...
    for (auto& request : pending_decodes_) {
        if (failed_) {
            request->exception = MakeGarbageCollected<DOMException>(
                DOMExceptionCode::kEncodingError, String::Format("Failed to decode frame at index %d", request->frame_index));
            continue;
        }
        ...
        decoder_->AsyncCall(&ImageDecoderCore::Decode)
            .WithArgs(request->frame_index, request->complete_frames_only, WTF::CrossThreadUnretained(request->abort_flag.get()))
            .Then(CrossThreadBindOnce(&ImageDecoderExternal::OnDecodeReady, MakeUnwrappingCrossThreadHandle(decode_weak_factory_.GetWeakCell())));
    }

    auto new_end = std::stable_partition(pending_decodes_.begin(), pending_decodes_.end(), [](const auto& request) { return !request->IsFinal(); });
    ...
    pending_decodes_.Shrink(static_cast<wtf_size_t>(new_end - pending_decodes_.begin()));
    ...
    for (auto& request : completed_decodes) {
        ...
        if (request->exception) {
            ...
            request->resolver->Reject(V8ThrowException::CreateRangeError(script_state_->GetIsolate(), *request->range_error_message));
        } else {
...
void ImageDecoderExternal::OnDecodeReady(std::unique_ptr<ImageDecoderCore::ImageDecodeResult> result)
{
    DCHECK(decoder_);
    DCHECK(!closed_);
    DCHECK(result);
    ...
    if (result->status == ImageDecoderCore::Status::kIndexError) {
        request->range_error_message = ExceptionMessages::IndexOutsideRange<uint32_t>("frame index", request->frame_index, 0,
            ExceptionMessages::kInclusiveBound, tracks_->selectedTrack()->frameCount(), ExceptionMessages::kExclusiveBound);
        MaybeSatisfyPendingDecodes();
        return;
    }
    ...
    if (result->status == ImageDecoderCore::Status::kNoImage) {
        ...
        if (data_complete_) {
            request->range_error_message = String::Format("Unexpected end of image. Request for frame index %d "
                                                          "can't be satisfied.",
                request->frame_index);
        }
    ...
    request->result->setImage(MakeGarbageCollected<VideoFrame>(base::MakeRefCounted<VideoFrameHandle>(std::move(result->frame), std::move(result->sk_image))));
...
void ImageDecoderExternal::DecodeMetadata()
{
    ...
    decoder_->AsyncCall(&ImageDecoderCore::DecodeMetadata)
        .Then(CrossThreadBindOnce(&ImageDecoderExternal::OnMetadata, MakeUnwrappingCrossThreadHandle(weak_factory_.GetWeakCell())));
}

void ImageDecoderExternal::OnMetadata(ImageDecoderCore::ImageMetadata metadata)
{
    DCHECK(decoder_);
    DCHECK(!closed_);
    ...
    if (!tracks_->IsEmpty()) {
        tracks_->selectedTrack()->UpdateTrack(metadata.frame_count, metadata.repetition_count);
        if (did_complete)
            MaybeSatisfyPendingDecodes();
        return;
    }
    ...
    if (metadata.image_has_both_still_and_animated_sub_images) {
        ...
        tracks_->AddTrack(metadata.frame_count, metadata.repetition_count, selected_track_id == 1);
    } else {
...
void ImageDecoderExternal::SetFailed()
{
    ...
    decode_weak_factory_.Invalidate();
    if (tracks_->IsEmpty()) {
        tracks_->OnTracksReady(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "Failed to retrieve track metadata."));
    }
    MaybeSatisfyPendingDecodes();
    close();
}
