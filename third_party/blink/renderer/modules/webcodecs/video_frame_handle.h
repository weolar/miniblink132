...
class MODULES_EXPORT VideoFrameHandle : public WTF::ThreadSafeRefCounted<VideoFrameHandle> {
public:
    VideoFrameHandle(scoped_refptr<media::VideoFrame>, ExecutionContext*, std::string monitoring_source_id = std::string(), bool use_capture_timestamp = false);
    VideoFrameHandle(scoped_refptr<media::VideoFrame>, sk_sp<SkImage> sk_image, ExecutionContext*, std::string monitoring_source_id = std::string(),
        bool use_capture_timestamp = false);
    VideoFrameHandle(scoped_refptr<media::VideoFrame>, sk_sp<SkImage> sk_image, base::TimeDelta timestamp,
        scoped_refptr<WebCodecsLogger::VideoFrameCloseAuditor>, std::string monitoring_source_id = std::string());
    VideoFrameHandle(scoped_refptr<media::VideoFrame>, sk_sp<SkImage> sk_image, base::TimeDelta timestamp, std::string monitoring_source_id = std::string());
    VideoFrameHandle(scoped_refptr<media::VideoFrame>, sk_sp<SkImage> sk_image, std::string monitoring_source_id = std::string());
    VideoFrameHandle(scoped_refptr<media::VideoFrame>, sk_sp<SkImage> sk_image, scoped_refptr<WebCodecsLogger::VideoFrameCloseAuditor>,
    ...
    bool WebGPURegisterExternalTextureExpireCallback(WebGPUExternalTextureExpireCallback webgpu_external_texture_expire_callback);

    base::TimeDelta timestamp() const
    {
        return timestamp_;
    }
    std::optional<base::TimeDelta> duration() const
    {
        return duration_;
    }
...
private:
    ...
    Vector<WebGPUExternalTextureExpireCallback> webgpu_external_texture_expire_callbacks_;
