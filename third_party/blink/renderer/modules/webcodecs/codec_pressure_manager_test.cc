...
class FakeReclaimableCodec final : public GarbageCollected<FakeReclaimableCodec>, public ReclaimableCodec {
public:
    explicit FakeReclaimableCodec(ReclaimableCodec::CodecType type, ExecutionContext* context)
    ...
    bool IsTimerActive()
    {
        return IsReclamationTimerActiveForTesting();
    }
...
class CodecPressureManagerTest : public testing::TestWithParam<ReclaimableCodec::CodecType> {
public:
    using TestCodecSet = HeapHashSet<Member<FakeReclaimableCodec>>;

    CodecPressureManagerTest()
    {
        GetCodecPressureGauge().set_pressure_threshold_for_testing(kTestPressureThreshold);
    }
...
protected:
    void SyncPressureFlags()
    {
        ...
        scheduler::GetSequencedTaskRunnerForTesting()->PostTask(FROM_HERE, run_loop.QuitClosure());
        run_loop.Run();
    }
    ...
    FakeReclaimableCodec* CreateBackgroundedCodec(ExecutionContext* context)
    {
        ...
        codec->SimulateLifecycleStateForTesting(scheduler::SchedulingLifecycleState::kHidden);
...
TEST_P(CodecPressureManagerTest, OneManagerPerContext)
{
...

    {
        ...
        ASSERT_NE(other_v8_scope.GetExecutionContext(), v8_scope.GetExecutionContext());

        EXPECT_NE(GetManagerFromContext(v8_scope.GetExecutionContext()), GetManagerFromContext(other_v8_scope.GetExecutionContext()));
    }
}
...
TEST_P(CodecPressureManagerTest, AllManagersIncrementGlobalPressureGauge)
{
    ...
    EXPECT_EQ(1u, GetManagerFromContext(v8_scope.GetExecutionContext())->pressure_for_testing());

    {
        V8TestingScope other_v8_scope;
        ASSERT_NE(other_v8_scope.GetExecutionContext(), v8_scope.GetExecutionContext());

        auto* other_codec = CreatePressuringCodec(other_v8_scope.GetExecutionContext());

        EXPECT_TRUE(other_codec->is_applying_codec_pressure());

        EXPECT_EQ(2u, GetCodecPressureGauge().global_pressure_for_testing());
        EXPECT_EQ(1u, GetManagerFromContext(other_v8_scope.GetExecutionContext())->pressure_for_testing());
...
TEST_P(CodecPressureManagerTest, ManagersAreInitializedWithGlobalPressureValue)
{
    V8TestingScope v8_scope;
    ...
    for (size_t i = 0; i < kTestPressureThreshold + 1; ++i) {
        codecs_with_pressure.insert(CreatePressuringCodec(v8_scope.GetExecutionContext()));
    }
...

    {
        ...
        EXPECT_TRUE(GetManagerFromContext(other_v8_scope.GetExecutionContext())->is_global_pressure_exceeded_for_testing());
    }
}
...
TEST_P(CodecPressureManagerTest, DifferentManagersForEncodersAndDecoders)
{
    ...
    auto& provider = CodecPressureManagerProvider::From(*v8_scope.GetExecutionContext());

    EXPECT_NE(provider.GetDecoderPressureManager(), provider.GetEncoderPressureManager());
}
...
TEST_P(CodecPressureManagerTest, PressureStartsTimers)
{
    ...
    for (size_t i = 0; i < kTestPressureThreshold; ++i) {
        pressuring_codecs.insert(CreatePressuringCodec(v8_scope.GetExecutionContext()));
    }
...
INSTANTIATE_TEST_SUITE_P(, CodecPressureManagerTest, testing::Values(ReclaimableCodec::CodecType::kDecoder, ReclaimableCodec::CodecType::kEncoder));
