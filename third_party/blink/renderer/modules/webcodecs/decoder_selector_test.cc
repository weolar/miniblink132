...
public:
    static constexpr media::DemuxerStream::Type kStreamType = media::DemuxerStream::AUDIO;
    ...
    static void ExpectInitialize(MockDecoder* decoder, DecoderCapability capability, media::AudioDecoderConfig expected_config, bool /*low_delay */)
    {
        EXPECT_CALL(*decoder, Initialize_(_, _, _, _, _))
            .WillRepeatedly([capability, expected_config](const media::AudioDecoderConfig& config, media::CdmContext*, media::AudioDecoder::InitCB& init_cb,
                                const media::AudioDecoder::OutputCB&, const media::WaitingCB&) {
                EXPECT_TRUE(config.Matches(expected_config));
                std::move(init_cb).Run(capability == kSucceed ? media::DecoderStatus::Codes::kOk : media::DecoderStatus::Codes::kFailed);
            });
    }
};
...
public:
    static constexpr media::DemuxerStream::Type kStreamType = media::DemuxerStream::VIDEO;
    ...
    static void ExpectInitialize(MockDecoder* decoder, DecoderCapability capability, media::VideoDecoderConfig expected_config, bool low_delay)
    {
        EXPECT_CALL(*decoder, Initialize_(_, low_delay, _, _, _, _))
            .WillRepeatedly([capability, expected_config](const media::VideoDecoderConfig& config, bool low_delay, media::CdmContext*,
                                media::VideoDecoder::InitCB& init_cb, const media::VideoDecoder::OutputCB&, const media::WaitingCB&) {
                EXPECT_TRUE(config.Matches(expected_config));
                std::move(init_cb).Run(capability == kSucceed ? media::DecoderStatus::Codes::kOk : media::DecoderStatus::Codes::kFailed);
            });
    }
...
template <typename TypeParam> class WebCodecsDecoderSelectorTest : public ::testing::Test {
public:
    // Convenience aliases.
    ...
    WebCodecsDecoderSelectorTest()
    {
        CreateDecoderSelector();
    }

    WebCodecsDecoderSelectorTest(const WebCodecsDecoderSelectorTest&) = delete;
    WebCodecsDecoderSelectorTest& operator=(const WebCodecsDecoderSelectorTest&) = delete;
    ...
    void OnDecoderSelectedThunk(std::unique_ptr<Decoder> decoder)
    {
        ...
        OnDecoderSelected(decoder ? static_cast<MockDecoder*>(decoder.get())->GetDecoderId() : kNoDecoder);
    }
    ...
    std::vector<std::unique_ptr<Decoder>> CreateDecoders()
    {
        ...
        for (const auto& info : mock_decoders_to_create_) {
            std::unique_ptr<StrictMock<MockDecoder>> decoder = std::make_unique<StrictMock<MockDecoder>>(
                /*is_platform_decoder=*/false, /*supports_decryption=*/true, info.first);
            TypeParam::ExpectInitialize(decoder.get(), info.second, last_set_decoder_config_, low_delay_);
    ...
    {
        decoder_selector_ = std::make_unique<DecoderSelector<TypeParam::kStreamType>>(scheduler::GetSingleThreadTaskRunnerForTesting(),
            WTF::BindRepeating(&Self::CreateDecoders, base::Unretained(this)), WTF::BindRepeating(&Self::OnOutput, base::Unretained(this)));
    }

    void SelectDecoder(DecoderConfig config = TypeParam::CreateConfig())
    {
        last_set_decoder_config_ = config;
        decoder_selector_->SelectDecoder(config, low_delay_, WTF::BindOnce(&Self::OnDecoderSelectedThunk, base::Unretained(this)));
        RunUntilIdle();
    }
...
using WebCodecsDecoderSelectorTestParams = ::testing::Types<AudioDecoderSelectorTestParam, VideoDecoderSelectorTestParam>;
TYPED_TEST_SUITE(WebCodecsDecoderSelectorTest, WebCodecsDecoderSelectorTestParams);
