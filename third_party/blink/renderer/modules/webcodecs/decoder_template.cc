...
// static
template <typename Traits> const CodecTraceNames* DecoderTemplate<Traits>::GetTraceNames()
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(CodecTraceNames, trace_names, (Traits::GetName()));
    return &trace_names;
}

template <typename Traits>
DecoderTemplate<Traits>::DecoderTemplate(ScriptState* script_state, const InitType* init, ExceptionState& exception_state)
    : ActiveScriptWrappable<DecoderTemplate<Traits>>({})
    , ReclaimableCodec(ReclaimableCodec::CodecType::kDecoder, ExecutionContext::From(script_state))
    ...
    , trace_counter_id_(g_sequence_num_for_counters.GetNext())
{
    ...
    main_thread_task_runner_ = context->GetTaskRunner(TaskType::kInternalMediaRealTime);

    logger_ = std::make_unique<CodecLogger<media::DecoderStatus>>(context, main_thread_task_runner_);

    logger_->log()->SetProperty<media::MediaLogProperty::kFrameUrl>(context->Url().GetString().Ascii());

    output_cb_ = init->output();
    error_cb_ = init->error();
}

template <typename Traits> DecoderTemplate<Traits>::~DecoderTemplate()
{
    DVLOG(1) << __func__;
    base::UmaHistogramSparse(String::Format("Blink.WebCodecs.%s.FinalStatus", Traits::GetName()).Ascii().c_str(), static_cast<int>(logger_->status_code()));
}

template <typename Traits> uint32_t DecoderTemplate<Traits>::decodeQueueSize()
{
    return num_pending_decodes_;
}

template <typename Traits> bool DecoderTemplate<Traits>::IsClosed()
{
    return state_ == V8CodecState::Enum::kClosed;
}

template <typename Traits> HardwarePreference DecoderTemplate<Traits>::GetHardwarePreference(const ConfigType&)
{
    return HardwarePreference::kNoPreference;
}

template <typename Traits> bool DecoderTemplate<Traits>::GetLowDelayPreference(const ConfigType&)
{
    return false;
}

template <typename Traits> void DecoderTemplate<Traits>::SetHardwarePreference(HardwarePreference)
{
}

template <typename Traits> void DecoderTemplate<Traits>::configure(const ConfigType* config, ExceptionState& exception_state)
{
    DVLOG(1) << __func__;
    ...
    std::optional<MediaConfigType> media_config = MakeMediaConfig(*config, &js_error_message);
...
template <typename Traits> void DecoderTemplate<Traits>::decode(const InputType* chunk, ExceptionState& exception_state)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (ThrowIfCodecStateClosed(state_, "decode", exception_state))
    ...
    if (status_or_buffer.has_value()) {
        ...
        if (request->status == media::DecoderStatus::Codes::kKeyFrameRequired) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataError, request->status.message().c_str());
            return;
        }
    }
    MarkCodecActive();
...
template <typename Traits> ScriptPromise<IDLUndefined> DecoderTemplate<Traits>::flush(ExceptionState& exception_state)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state_);
...
template <typename Traits> void DecoderTemplate<Traits>::reset(ExceptionState& exception_state)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
template <typename Traits> void DecoderTemplate<Traits>::close(ExceptionState& exception_state)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
template <typename Traits> void DecoderTemplate<Traits>::ProcessRequests()
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(!IsClosed());
...
template <typename Traits> bool DecoderTemplate<Traits>::ProcessConfigureRequest(Request* request)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(!IsClosed());
    ...
    if (decoder() && pending_decodes_.size() + 1 > static_cast<size_t>(Traits::GetMaxDecodeRequests(*decoder()))) {
    ...
    if (!request->media_config) {
        main_thread_task_runner_->PostTask(FROM_HERE,
            WTF::BindOnce(&DecoderTemplate<Traits>::Shutdown, WrapWeakPersistent(this),
                WrapPersistent(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotSupportedError, request->js_error_message))));
        return false;
    }
    ...
    if (gpu_factories_.has_value()) {
        ...
        RetrieveGpuFactoriesWithKnownDecoderSupport(CrossThreadBindOnce(
            &DecoderTemplate<Traits>::ContinueConfigureWithGpuFactories, MakeUnwrappingCrossThreadHandle(this), MakeUnwrappingCrossThreadHandle(request)));
    } else {
...
template <typename Traits> void DecoderTemplate<Traits>::ContinueConfigureWithGpuFactories(Request* request, media::GpuVideoAcceleratorFactories* gpu_factories)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(request);
    DCHECK_EQ(request->type, Request::Type::kConfigure);
    ...
    if (!decoder()) {
        decoder_ = Traits::CreateDecoder(*ExecutionContext::From(script_state_), gpu_factories_.value(), logger_->log());
        if (!decoder()) {
            Shutdown(MakeOperationError("Internal error: Could not create decoder.", media::DecoderStatus::Codes::kFailedToCreateDecoder));
            return;
        }
        ...
        Traits::InitializeDecoder(*decoder(), request->low_delay.value(), *request->media_config,
            WTF::BindOnce(&DecoderTemplate::OnInitializeDone, WrapWeakPersistent(this)),
            WTF::BindRepeating(&DecoderTemplate::OnOutput, WrapWeakPersistent(this), reset_generation_));
        initializing_sync_ = false;
        return;
    }

    // Processing continues in OnFlushDone().
    decoder()->Decode(media::DecoderBuffer::CreateEOSBuffer(), WTF::BindOnce(&DecoderTemplate::OnFlushDone, WrapWeakPersistent(this)));
}

template <typename Traits> bool DecoderTemplate<Traits>::ProcessDecodeRequest(Request* request)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (!decoder()) {
        Shutdown(MakeEncodingError("Decoding error: no decoder found.", media::DecoderStatus::Codes::kNotInitialized));
        return false;
    }

    if (pending_decodes_.size() + 1 > static_cast<size_t>(Traits::GetMaxDecodeRequests(*decoder()))) {
        // Try again after OnDecodeDone().
        return false;
    }
    ...
    if (!request->decoder_buffer || request->decoder_buffer->empty()) {
        if (request->status.is_ok()) {
            Shutdown(MakeEncodingError("Null or empty decoder buffer.", media::DecoderStatus::Codes::kFailed));
        } else {
    ...
    while (++pending_decode_id_ == 0 || pending_decode_id_ == std::numeric_limits<uint32_t>::max() || pending_decodes_.Contains(pending_decode_id_))
        ;
    ...
    if (media::MediaTraceIsEnabled()) {
        request->decode_trace = std::make_unique<media::ScopedDecodeTrace>(GetTraceNames()->decode.c_str(), *request->decoder_buffer);
    }

    decoder()->Decode(std::move(request->decoder_buffer), WTF::BindOnce(&DecoderTemplate::OnDecodeDone, WrapWeakPersistent(this), pending_decode_id_));
    return true;
}

template <typename Traits> bool DecoderTemplate<Traits>::ProcessFlushRequest(Request* request)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(!IsClosed());
...
template <typename Traits> bool DecoderTemplate<Traits>::ProcessResetRequest(Request* request)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(!IsClosed());
    ...
    if (decoder()) {
        ...
        decoder()->Reset(WTF::BindOnce(&DecoderTemplate::OnResetDone, WrapWeakPersistent(this)));
    }

    return true;
}

template <typename Traits> void DecoderTemplate<Traits>::Shutdown(DOMException* exception)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (IsClosed())
    ...
    TRACE_EVENT1(kCategory, GetTraceNames()->shutdown.c_str(), "has_exception", !!exception);
    ...
    if (pending_request_) {
        if (pending_request_->resolver) {
            pending_request_->resolver.Release()->Reject(
                exception ? exception : MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Aborted due to close()"));
        }
...
template <typename Traits> void DecoderTemplate<Traits>::ResetAlgorithm()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
template <typename Traits> void DecoderTemplate<Traits>::OnFlushDone(media::DecoderStatus status)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (IsClosed())
    ...
    DCHECK(pending_request_);
    DCHECK(pending_request_->type == Request::Type::kConfigure || pending_request_->type == Request::Type::kFlush);
    ...
    // Processing continues in OnInitializeDone().
    Traits::InitializeDecoder(*decoder(), is_flush ? low_delay_ : pending_request_->low_delay.value(),
        is_flush ? *active_config_ : *pending_request_->media_config, WTF::BindOnce(&DecoderTemplate::OnInitializeDone, WrapWeakPersistent(this)),
        WTF::BindRepeating(&DecoderTemplate::OnOutput, WrapWeakPersistent(this), reset_generation_));
}

template <typename Traits> void DecoderTemplate<Traits>::OnInitializeDone(media::DecoderStatus status)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (IsClosed())
    ...
    if (!status.is_ok()) {
        ...
        } else if (status.code() == media::DecoderStatus::Codes::kUnsupportedConfig) {
            error_message = "Unsupported configuration. Check isConfigSupported() prior to "
                            "calling configure().";
        } else {
    ...
    if (is_flush) {
        ...
        Traits::UpdateDecoderLog(*decoder(), *pending_request_->media_config, logger_->log());
...
template <typename Traits> void DecoderTemplate<Traits>::OnDecodeDone(uint32_t id, media::DecoderStatus status)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (IsClosed())
...
template <typename Traits> void DecoderTemplate<Traits>::OnResetDone()
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (IsClosed())
...
template <typename Traits> void DecoderTemplate<Traits>::OnOutput(uint32_t reset_generation, scoped_refptr<MediaOutputType> output)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (!output_or_error.has_value()) {
        Shutdown(MakeEncodingError("Error creating output from decoded data", std::move(output_or_error).error()));
        return;
    }

    OutputType* blink_output = std::move(output_or_error).value();

    TRACE_EVENT_BEGIN1(kCategory, GetTraceNames()->output.c_str(), "timestamp", blink_output->timestamp());
...
template <typename Traits> void DecoderTemplate<Traits>::TraceQueueSizes() const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    TRACE_COUNTER_ID2(kCategory, GetTraceNames()->requests_counter.c_str(), trace_counter_id_, "decodes", num_pending_decodes_, "other",
        requests_.size() - num_pending_decodes_);
}

template <typename Traits> void DecoderTemplate<Traits>::DispatchDequeueEvent(Event* event)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    probe::AsyncTask async_task(GetExecutionContext(), event->async_task_context());
    dequeue_event_pending_ = false;
    DispatchEvent(*event);
}

template <typename Traits> void DecoderTemplate<Traits>::ScheduleDequeueEvent()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    main_thread_task_runner_->PostTask(
        FROM_HERE, WTF::BindOnce(&DecoderTemplate<Traits>::DispatchDequeueEvent, WrapWeakPersistent(this), WrapPersistent(event)));
}

template <typename Traits> ExecutionContext* DecoderTemplate<Traits>::GetExecutionContext() const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    return ExecutionContextLifecycleObserver::GetExecutionContext();
}

template <typename Traits> void DecoderTemplate<Traits>::ContextDestroyed()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
template <typename Traits> void DecoderTemplate<Traits>::Trace(Visitor* visitor) const
{
...
template <typename Traits> void DecoderTemplate<Traits>::OnCodecReclaimed(DOMException* exception)
{
...
template <typename Traits> bool DecoderTemplate<Traits>::HasPendingActivity() const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    return pending_request_ || !requests_.empty();
}

template <typename Traits> bool DecoderTemplate<Traits>::MaybeAbortRequest(Request* request) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (request->resolver) {
        request->resolver.Release()->Reject(shutting_down_due_to_error_
                ? shutting_down_due_to_error_.Get()
                : MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, shutting_down_ ? "Aborted due to close()" : "Aborted due to reset()"));
    }
    return true;
}

template <typename Traits> void DecoderTemplate<Traits>::Request::Trace(Visitor* visitor) const
{
    visitor->Trace(resolver);
}

template <typename Traits> const char* DecoderTemplate<Traits>::Request::TraceNameFromType()
{
...
template <typename Traits> void DecoderTemplate<Traits>::Request::StartTracing()
{
#if DCHECK_IS_ON()
    DCHECK(!is_tracing);
    is_tracing = true;
...
template <typename Traits> void DecoderTemplate<Traits>::Request::EndTracing(bool shutting_down)
{
#if DCHECK_IS_ON()
    DCHECK(is_tracing);
    is_tracing = false;
#endif
    TRACE_EVENT_NESTABLE_ASYNC_END1(kCategory, TraceNameFromType(), this, "completed", !shutting_down);
}

template <typename Traits> DOMException* DecoderTemplate<Traits>::MakeOperationError(std::string error_msg, media::DecoderStatus status)
{
    if (!decoder_ || decoder_->IsPlatformDecoder()) {
        return logger_->MakeOperationError(std::move(error_msg), std::move(status));
    }
    return logger_->MakeSoftwareCodecOperationError(std::move(error_msg), std::move(status));
}

template <typename Traits> DOMException* DecoderTemplate<Traits>::MakeEncodingError(std::string error_msg, media::DecoderStatus status)
{
    if (!decoder_ || decoder_->IsPlatformDecoder()) {
        return logger_->MakeEncodingError(std::move(error_msg), std::move(status));
    }
    return logger_->MakeSoftwareCodecEncodingError(std::move(error_msg), std::move(status));
}
