...
{
    test::TaskEnvironment task_environment;
    auto expected_media_config = CreateTestDecryptConfig(media::EncryptionScheme::kCbcs);

    auto* js_config = MakeGarbageCollected<DecryptConfig>();
    js_config->setEncryptionScheme("cbcs");
    js_config->setKeyId(StringToBuffer(expected_media_config->key_id()));
    js_config->setInitializationVector(StringToBuffer(expected_media_config->iv()));
...
TEST(DecryptConfigUtilTest, CreateCbcsWithPattern)
{
    ...
    auto expected_media_config = CreateTestDecryptConfig(media::EncryptionScheme::kCbcs, kPattern);
    ...
    pattern->setCryptByteBlock(expected_media_config->encryption_pattern()->crypt_byte_block());
    pattern->setSkipByteBlock(expected_media_config->encryption_pattern()->skip_byte_block());
...
{
    test::TaskEnvironment task_environment;
    auto expected_media_config = CreateTestDecryptConfig(media::EncryptionScheme::kCenc);
