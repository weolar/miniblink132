...
template <> struct CrossThreadCopier<media::AudioDecoderConfig> : public CrossThreadCopierPassThrough<media::AudioDecoderConfig> {
...
public:
    ...
    using CrossThreadOnceDecodeCB = WTF::CrossThreadOnceFunction<void(media::DecoderStatus)>;
    using CrossThreadOnceResetCB = WTF::CrossThreadOnceClosure;

    MediaAudioTaskWrapper(base::WeakPtr<CrossThreadAudioDecoderClient> weak_client, ExecutionContext& execution_context,
        std::unique_ptr<media::MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> media_task_runner,
        ...
        , media_log_(std::move(media_log))
    {
        ...
        PostCrossThreadTask(*media_task_runner_, FROM_HERE,
            WTF::CrossThreadBindOnce(&MediaAudioTaskWrapper::BindOnTaskRunner, WTF::CrossThreadUnretained(this), std::move(media_interface_factory)));
    }
    ...
    void Initialize(const media::AudioDecoderConfig& config)
    {
        ...
        selector_ = std::make_unique<WebCodecsAudioDecoderSelector>(media_task_runner_,
            ...
            WTF::BindRepeating(&MediaAudioTaskWrapper::OnCreateDecoders, WTF::Unretained(this)),
            WTF::BindRepeating(&MediaAudioTaskWrapper::OnDecodeOutput, weak_factory_.GetWeakPtr()));

        selector_->SelectDecoder(config, /*low_delay=*/false, WTF::BindOnce(&MediaAudioTaskWrapper::OnDecoderSelected, weak_factory_.GetWeakPtr()));
    }
    ...
    void Decode(scoped_refptr<media::DecoderBuffer> buffer, int cb_id)
    {
        ...
        decoder_->Decode(std::move(buffer), WTF::BindOnce(&MediaAudioTaskWrapper::OnDecodeDone, weak_factory_.GetWeakPtr(), cb_id));
    }
    ...
    void Reset(int cb_id)
    {
        ...
        decoder_->Reset(WTF::BindOnce(&MediaAudioTaskWrapper::OnReset, weak_factory_.GetWeakPtr(), cb_id));
    }
...
private:
    ...
    std::vector<std::unique_ptr<media::AudioDecoder>> OnCreateDecoders()
    {
        ...
        decoder_factory_->CreateAudioDecoders(media_task_runner_, media_log_.get(), &audio_decoders);
    ...
    void OnDecoderSelected(std::unique_ptr<media::AudioDecoder> decoder)
    {
        ...
        if (decoder_) {
            decoder_details = DecoderDetails({ decoder_->GetDecoderType(), decoder_->IsPlatformDecoder(), decoder_->NeedsBitstreamConversion() });
        } else {
        ...
        // Fire |init_cb|.
        PostCrossThreadTask(
            *main_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&CrossThreadAudioDecoderClient::OnInitialize, weak_client_, status, decoder_details));
    }
    ...
    void OnDecodeOutput(scoped_refptr<media::AudioBuffer> buffer)
    {
        ...
        PostCrossThreadTask(
            *main_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&CrossThreadAudioDecoderClient::OnDecodeOutput, weak_client_, std::move(buffer)));
    }
    ...
    void OnDecodeDone(int cb_id, media::DecoderStatus status)
    {
        ...
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        PostCrossThreadTask(
            *main_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&CrossThreadAudioDecoderClient::OnDecodeDone, weak_client_, cb_id, std::move(status)));
    }
    ...
    void OnReset(int cb_id)
    {
        DVLOG(2) << __func__;
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        PostCrossThreadTask(*main_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&CrossThreadAudioDecoderClient::OnReset, weak_client_, cb_id));
    }
...
AudioDecoderBroker::AudioDecoderBroker(media::MediaLog* media_log, ExecutionContext& execution_context)
    ...
    : media_task_runner_(worker_pool::CreateSequencedTaskRunner({}))
{
    ...
    media_tasks_ = std::make_unique<MediaAudioTaskWrapper>(
        weak_factory_.GetWeakPtr(), execution_context, media_log->Clone(), media_task_runner_, execution_context.GetTaskRunner(TaskType::kInternalMedia));
}
...
{
    return decoder_details_ ? decoder_details_->decoder_type : media::AudioDecoderType::kBroker;
}
...
void AudioDecoderBroker::Initialize(
    const media::AudioDecoderConfig& config, media::CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const media::WaitingCB& waiting_cb)
{
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    PostCrossThreadTask(
        *media_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&MediaAudioTaskWrapper::Initialize, WTF::CrossThreadUnretained(media_tasks_.get()), config));
}
...
void AudioDecoderBroker::OnInitialize(media::DecoderStatus status, std::optional<DecoderDetails> details)
{
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void AudioDecoderBroker::Decode(scoped_refptr<media::DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    PostCrossThreadTask(*media_task_runner_, FROM_HERE,
        WTF::CrossThreadBindOnce(&MediaAudioTaskWrapper::Decode, WTF::CrossThreadUnretained(media_tasks_.get()), buffer, callback_id));
}
...
void AudioDecoderBroker::Reset(base::OnceClosure reset_cb)
{
    ...
    PostCrossThreadTask(
        *media_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&MediaAudioTaskWrapper::Reset, WTF::CrossThreadUnretained(media_tasks_.get()), callback_id));
}
...
void AudioDecoderBroker::OnDecodeOutput(scoped_refptr<media::AudioBuffer> buffer)
{
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(output_cb_);
