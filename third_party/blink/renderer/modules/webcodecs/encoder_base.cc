...
// static
template <typename Traits> const CodecTraceNames* EncoderBase<Traits>::GetTraceNames()
{
...
EncoderBase<Traits>::EncoderBase(ScriptState* script_state, const InitType* init, ExceptionState& exception_state)
    : ActiveScriptWrappable<EncoderBase<Traits>>({})
    , ReclaimableCodec(ReclaimableCodec::CodecType::kEncoder, ExecutionContext::From(script_state))
    ...
    , trace_counter_id_(g_sequence_num_for_counters.GetNext())
{
    ...
    log->SetProperty<media::MediaLogProperty::kFrameUrl>(GetExecutionContext()->Url().GetString().Ascii());
...
template <typename Traits> EncoderBase<Traits>::~EncoderBase()
{
...
template <typename Traits> void EncoderBase<Traits>::configure(const ConfigType* config, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (active_config_ && state_.AsEnum() == V8CodecState::Enum::kConfigured && CanReconfigure(*active_config_, *parsed_config)) {
...
template <typename Traits> void EncoderBase<Traits>::encode(InputType* input, const EncodeOptionsType* opts, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
template <typename Traits> void EncoderBase<Traits>::close(ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    ResetInternal(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Aborted due to close()"));
    output_callback_.Clear();
    error_callback_.Clear();
}

template <typename Traits> ScriptPromise<IDLUndefined> EncoderBase<Traits>::flush(ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    Request* request = MakeGarbageCollected<Request>();
    request->resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state_);
    request->reset_count = reset_count_;
...
template <typename Traits> void EncoderBase<Traits>::reset(ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    ResetInternal(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Aborted due to reset()"));
}

template <typename Traits> void EncoderBase<Traits>::ResetInternal(DOMException* ex)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    reset_count_++;

    if (blocking_request_in_progress_ && blocking_request_in_progress_->resolver) {
...
template <typename Traits> void EncoderBase<Traits>::QueueHandleError(DOMException* ex)
{
    callback_runner_->PostTask(FROM_HERE, WTF::BindOnce(&EncoderBase<Traits>::HandleError, WrapWeakPersistent(this), WrapPersistent(ex)));
}

template <typename Traits> void EncoderBase<Traits>::HandleError(DOMException* ex)
{
...
template <typename Traits> void EncoderBase<Traits>::EnqueueRequest(Request* request)
{
    requests_.push_back(request);
    ProcessRequests();
}

template <typename Traits> void EncoderBase<Traits>::ProcessRequests()
{
...
template <typename Traits> bool EncoderBase<Traits>::ReadyToProcessNextRequest()
{
    return !blocking_request_in_progress_;
}

template <typename Traits> void EncoderBase<Traits>::ProcessFlush(Request* request)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    auto done_callback = [](EncoderBase<Traits>* self, Request* req, media::EncoderStatus status) {
        DCHECK(req);
        ...
        if (!self) {
            req->resolver.Release()->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Aborted due to close()"));
        ...
        if (self->reset_count_ != req->reset_count) {
            req->resolver.Release()->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Aborted due to reset()"));
            req->EndTracing(/*aborted=*/true);
            return;
        }
        if (status.is_ok()) {
            ...
            self->HandleError(self->logger_->MakeEncodingError("Flushing error.", std::move(status)));
            DCHECK(!req->resolver);
        }
        req->EndTracing();
...
template <typename Traits> void EncoderBase<Traits>::OnCodecReclaimed(DOMException* exception)
{
...
template <typename Traits> void EncoderBase<Traits>::ContextDestroyed()
{
    state_ = V8CodecState(V8CodecState::Enum::kClosed);
    ResetInternal(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Aborted due to close()"));
    logger_->Neuter();
...
template <typename Traits> bool EncoderBase<Traits>::HasPendingActivity() const
{
    return blocking_request_in_progress_ || !requests_.empty();
}

template <typename Traits> void EncoderBase<Traits>::TraceQueueSizes() const
{
    TRACE_COUNTER_ID2(
        kCategory, GetTraceNames()->requests_counter.c_str(), trace_counter_id_, "encodes", requested_encodes_, "other", requests_.size() - requested_encodes_);
}

template <typename Traits> void EncoderBase<Traits>::DispatchDequeueEvent(Event* event)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
template <typename Traits> void EncoderBase<Traits>::ScheduleDequeueEvent()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    callback_runner_->PostTask(FROM_HERE, WTF::BindOnce(&EncoderBase<Traits>::DispatchDequeueEvent, WrapWeakPersistent(this), WrapPersistent(event)));
}

template <typename Traits> ExecutionContext* EncoderBase<Traits>::GetExecutionContext() const
{
    return ExecutionContextLifecycleObserver::GetExecutionContext();
}

template <typename Traits> void EncoderBase<Traits>::Trace(Visitor* visitor) const
{
...
template <typename Traits> void EncoderBase<Traits>::Request::Trace(Visitor* visitor) const
{
    visitor->Trace(input);
...
template <typename Traits> const char* EncoderBase<Traits>::Request::TraceNameFromType()
{
...
template <typename Traits> void EncoderBase<Traits>::Request::StartTracingVideoEncode(bool is_keyframe, base::TimeDelta timestamp)
{
#if DCHECK_IS_ON()
    DCHECK(!is_tracing);
    is_tracing = true;
#endif
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN2(kCategory, TraceNameFromType(), this, "key_frame", is_keyframe, "timestamp", timestamp);
}

template <typename Traits> void EncoderBase<Traits>::Request::StartTracing()
{
#if DCHECK_IS_ON()
    DCHECK(!is_tracing);
    is_tracing = true;
...
template <typename Traits> void EncoderBase<Traits>::Request::EndTracing(bool aborted)
{
#if DCHECK_IS_ON()
    DCHECK(is_tracing);
    is_tracing = false;
#endif
    TRACE_EVENT_NESTABLE_ASYNC_END1(kCategory, TraceNameFromType(), this, "aborted", aborted);
}
