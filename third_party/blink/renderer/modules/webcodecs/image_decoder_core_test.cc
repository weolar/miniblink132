...
protected:
    std::unique_ptr<ImageDecoderCore> CreateDecoder(const char* file_name, const char* mime_type)
    {
        auto data = ReadFile(file_name);
        DCHECK(data->size()) << "Missing file: " << file_name;
        return std::make_unique<ImageDecoderCore>(mime_type, std::move(data),
            /*data_complete=*/true, ColorBehavior::kTag, SkISize::MakeEmpty(), ImageDecoder::AnimationOption::kPreferAnimation);
    }
    ...
    scoped_refptr<SegmentReader> ReadFile(StringView file_name)
    {
        ...
        CHECK(data);
        return SegmentReader::CreateFromSharedBuffer(SharedBuffer::Create(std::move(*data)));
    }
...
TEST_F(ImageDecoderCoreTest, InOrderDecodePreservesMemory)
{
    ...
    auto decoder = CreateDecoder("images/resources/webp-animated-large.webp", kImageType);
    ASSERT_TRUE(decoder);
    ...
    for (int j = 0; j < 2; ++j) {
        for (uint32_t i = 0; i < metadata.frame_count; ++i) {
            auto result = decoder->Decode(i, /*complete_frames_only=*/true, &abort_flag);
    ...
    decoder->Decode(metadata.frame_count / 2, /*complete_frames_only=*/true, &abort_flag);
