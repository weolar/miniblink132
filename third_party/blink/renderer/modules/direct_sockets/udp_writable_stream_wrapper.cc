...
UDPWritableStreamWrapper::UDPWritableStreamWrapper(
    ScriptState* script_state, CloseOnceCallback on_close, const Member<UDPSocketMojoRemote> udp_socket, network::mojom::blink::RestrictedUDPSocketMode mode)
...
ScriptPromise<IDLUndefined> UDPWritableStreamWrapper::Write(ScriptValue chunk, ExceptionState& exception_state)
{
    DCHECK(udp_socket_->get().is_bound());

    UDPMessage* message = UDPMessage::Create(GetScriptState()->GetIsolate(), chunk.V8Value(), exception_state);
    ...
    if (message->hasRemoteAddress() && message->hasRemotePort()) {
        if (mode_ == network::mojom::RestrictedUDPSocketMode::CONNECTED) {
            exception_state.ThrowTypeError("UDPMessage: 'remoteAddress' and 'remotePort' must not be specified "
                                           "in 'connected' mode.");
            return EmptyPromise();
        }
        dest_addr = net::HostPortPair(message->remoteAddress().Utf8(), message->remotePort());
    } else if (message->hasRemoteAddress() || message->hasRemotePort()) {
        exception_state.ThrowTypeError("UDPMessage: either none or both 'remoteAddress' and 'remotePort' "
                                       "fields must be specified.");
        return EmptyPromise();
    } else if (mode_ == network::mojom::RestrictedUDPSocketMode::BOUND) {
        exception_state.ThrowTypeError("UDPMessage: 'remoteAddress' and 'remotePort' must be specified "
                                       "in 'bound' mode.");
        return EmptyPromise();
    }
    ...
    if (message->hasDnsQueryType()) {
        if (mode_ == network::mojom::RestrictedUDPSocketMode::CONNECTED) {
            exception_state.ThrowTypeError("UDPMessage: 'dnsQueryType' must not be specified "
                                           "in 'connected' mode.");
            return EmptyPromise();
        }
    ...
    if (data.empty()) {
        exception_state.ThrowTypeError("UDPMessage: 'data' field must not be empty.");
        return EmptyPromise();
    }
    ...
    auto callback = WTF::BindOnce(&UDPWritableStreamWrapper::OnSend, WrapWeakPersistent(this));
    if (dest_addr) {
        udp_socket_->get()->SendTo(data, *dest_addr, dns_query_type, std::move(callback));
    } else {
...
void UDPWritableStreamWrapper::ErrorStream(int32_t error_code)
{
    ...
    auto exception = ScriptValue(script_state->GetIsolate(),
        V8ThrowDOMException::CreateOrDie(
            script_state->GetIsolate(), DOMExceptionCode::kNetworkError, String { "Stream aborted by the remote: " + net::ErrorToString(error_code) }));
