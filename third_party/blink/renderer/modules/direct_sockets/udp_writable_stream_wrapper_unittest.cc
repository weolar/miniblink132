...
class FakeRestrictedUDPSocket : public GarbageCollected<FakeRestrictedUDPSocket>, public network::mojom::blink::RestrictedUDPSocket {
public:
    ...
    const Vector<uint8_t>& GetReceivedData() const
    {
        return data_;
    }
...
public:
    ...
    UDPWritableStreamWrapper* Create(const V8TestingScope& scope)
    {
        ...
        stream_wrapper_ = MakeGarbageCollected<UDPWritableStreamWrapper>(
            script_state, WTF::BindOnce(&StreamCreator::Close, WrapWeakPersistent(this)), udp_socket, network::mojom::RestrictedUDPSocketMode::CONNECTED);
    ...
    {
        return fake_udp_socket_.Get();
    }

    bool CloseCalledWith(bool error)
    {
        return close_called_with_ == error;
    }
...
TEST(UDPWritableStreamWrapperTest, WriteUdpMessage)
{
    ...
    auto* writer = udp_writable_stream_wrapper->Writable()->getWriter(script_state, ASSERT_NO_EXCEPTION);

    auto* chunk = DOMArrayBuffer::Create(base::byte_span_from_cstring("A"));
    auto* message = UDPMessage::Create();
    message->setData(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(chunk));

    auto result = writer->write(script_state, ScriptValue::From(script_state, message), ASSERT_NO_EXCEPTION);
...
TEST(UDPWritableStreamWrapperTest, WriteUdpMessageFromTypedArray)
{
    ...
    auto* message = UDPMessage::Create();
    message->setData(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(NotShared<DOMUint8Array>(chunk)));
    ...
    auto* fake_udp_socket = stream_creator->fake_udp_socket();
    EXPECT_THAT(fake_udp_socket->GetReceivedData(), ::testing::ElementsAre('A', 'B', 'C'));
}
...
TEST(UDPWritableStreamWrapperTest, WriteAfterFinishedWrite)
{
    ...
    auto* fake_udp_socket = stream_creator->fake_udp_socket();
    EXPECT_THAT(fake_udp_socket->GetReceivedData(), ::testing::ElementsAre('A', 'B'));
}
...
TEST(UDPWritableStreamWrapperTest, WriteAfterClose)
{
    ...
    auto write_result = writer->write(script_state, ScriptValue::From(script_state, message), ASSERT_NO_EXCEPTION);
    ...
    ASSERT_EQ(udp_writable_stream_wrapper->GetState(), StreamWrapper::State::kClosed);

    auto write_after_close_result = writer->write(script_state, ScriptValue::From(script_state, message), ASSERT_NO_EXCEPTION);
    ScriptPromiseTester write_after_close_tester(script_state, write_after_close_result);
...
TEST(UDPWritableStreamWrapperTest, WriteFailed)
{
    ...
    ScopedStreamCreator stream_creator(MakeGarbageCollected<StreamCreator>(scope, MakeGarbageCollected<FailingFakeRestrictedUDPSocket>()));
    auto* udp_writable_stream_wrapper = stream_creator->Create(scope);
    ...
    ASSERT_EQ(udp_writable_stream_wrapper->GetState(), StreamWrapper::State::kAborted);
