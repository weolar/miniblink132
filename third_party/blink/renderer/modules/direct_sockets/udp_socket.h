...
class MODULES_EXPORT UDPSocket final : public ScriptWrappable, public Socket, public ActiveScriptWrappable<UDPSocket> {
    DEFINE_WRAPPERTYPEINFO();

public:
    // IDL definitions
    static UDPSocket* Create(ScriptState*, const UDPSocketOptions*, ExceptionState&);
...
public:
    ...
    void OnConnectedUDPSocketOpened(mojo::PendingReceiver<network::mojom::blink::UDPSocketListener>, int32_t result,
        const std::optional<net::IPEndPoint>& local_addr, const std::optional<net::IPEndPoint>& peer_addr);

    // On net::OK initializes readable/writable streams and resolves opened
    // promise. Otherwise rejects the opened promise.
    void OnBoundUDPSocketOpened(
        mojo::PendingReceiver<network::mojom::blink::UDPSocketListener>, int32_t result, const std::optional<net::IPEndPoint>& local_addr);
...
private:
    void FinishOpen(network::mojom::RestrictedUDPSocketMode, mojo::PendingReceiver<network::mojom::blink::UDPSocketListener>, int32_t result,
        const std::optional<net::IPEndPoint>& local_addr, const std::optional<net::IPEndPoint>& peer_addr);

    void FailOpenWith(int32_t error);

    mojo::PendingReceiver<network::mojom::blink::RestrictedUDPSocket> GetUDPSocketReceiver();
