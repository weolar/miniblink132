...
class MODULES_EXPORT TCPServerSocket final : public ScriptWrappable, public Socket {
    DEFINE_WRAPPERTYPEINFO();

public:
    // IDL definitions
    static TCPServerSocket* Create(ScriptState*, const String& local_address, const TCPServerSocketOptions*, ExceptionState&);
    ...
    bool Open(const String& local_address, const TCPServerSocketOptions*, ExceptionState&);

    // On net::OK initializes readable stream and resolves opened promise.
    // Otherwise rejects the opened promise.
    void OnTCPServerSocketOpened(mojo::PendingRemote<network::mojom::blink::TCPServerSocket>, int32_t result, const std::optional<net::IPEndPoint>& local_addr);
