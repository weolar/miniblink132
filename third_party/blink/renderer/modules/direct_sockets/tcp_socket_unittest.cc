...
std::pair<mojo::ScopedDataPipeProducerHandle, mojo::ScopedDataPipeConsumerHandle> CreateDataPipe(int32_t capacity = 1)
{
...
TEST(TCPSocketTest, CloseAfterInitWithResultOK)
{
    ...
    tcp_socket->OnTCPSocketOpened(socket_receiver.InitWithNewPipeAndPassRemote(), observer_remote.InitWithNewPipeAndPassReceiver(), net::OK,
        net::IPEndPoint { net::IPAddress::IPv4Localhost(), 0 }, net::IPEndPoint { net::IPAddress::IPv4Localhost(), 0 }, std::move(consumer),
        std::move(producer));
...
TEST(TCPSocketTest, OnSocketObserverConnectionError)
{
    ...
    ScriptPromiseTester closed_tester(script_state, tcp_socket->closed(script_state));
...
class TCPSocketCloseTest : public testing::TestWithParam<std::tuple<bool, bool>> { };
...
TEST_P(TCPSocketCloseTest, OnErrorOrClose)
{
    ...
    if (read_error) {
        ...
        auto cancel = ScriptPromiseTester(script_state, readable->cancel(script_state, ASSERT_NO_EXCEPTION));
        cancel.WaitUntilSettled();
        ASSERT_TRUE(cancel.IsFulfilled());
    }

    ASSERT_EQ(tcp_socket->readable_stream_wrapper_->GetState(), read_error ? StreamWrapper::State::kAborted : StreamWrapper::State::kClosed);
    ...
    if (write_error) {
        ...
        auto abort = ScriptPromiseTester(script_state, writable->abort(script_state, ASSERT_NO_EXCEPTION));
        abort.WaitUntilSettled();
        ASSERT_TRUE(abort.IsFulfilled());
    }

    ASSERT_EQ(tcp_socket->writable_stream_wrapper_->GetState(), write_error ? StreamWrapper::State::kAborted : StreamWrapper::State::kClosed);
...
INSTANTIATE_TEST_SUITE_P(/**/, TCPSocketCloseTest, testing::Combine(testing::Bool(), testing::Bool()));
