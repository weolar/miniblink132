...
UDPReadableStreamWrapper::UDPReadableStreamWrapper(ScriptState* script_state, CloseOnceCallback on_close, const Member<UDPSocketMojoRemote> udp_socket,
...
{
    socket_listener_.Bind(std::move(socket_listener), ExecutionContext::From(script_state)->GetTaskRunner(TaskType::kNetworking));
    socket_listener_.set_disconnect_handler(WTF::BindOnce(&UDPReadableStreamWrapper::ErrorStream, WrapWeakPersistent(this), net::ERR_CONNECTION_ABORTED));
    ...
    auto* readable = ReadableStream::CreateWithCountQueueingStrategy(script_state, source, /*high_water_mark=*/kReadableStreamBufferSize);
...
void UDPReadableStreamWrapper::OnReceived(int32_t result, const std::optional<::net::IPEndPoint>& src_addr, std::optional<::base::span<const ::uint8_t>> data)
{
    ...
    message->setData(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(NotShared<DOMUint8Array>(buffer)));
    if (src_addr) {
