...
class FakeRestrictedUDPSocket final : public GarbageCollected<FakeRestrictedUDPSocket>, public network::mojom::blink::RestrictedUDPSocket {
public:
    ...
    void ProvideRequestedDatagrams()
    {
        ...
        while (num_requested_datagrams > 0) {
            remote_->OnReceived(net::OK, net::IPEndPoint { net::IPAddress::IPv4Localhost(), 0U }, datagram_.Span8());
            num_requested_datagrams--;
        }
    }

    void Bind(mojo::PendingRemote<network::mojom::blink::UDPSocketListener> pending_remote, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    {
        remote_.Bind(std::move(pending_remote), task_runner);
    }

    const String& GetTestingDatagram() const
    {
        return datagram_;
    }
    void SetTestingDatagram(String datagram)
    {
...
public:
    explicit StreamCreator(const V8TestingScope& scope)
        : fake_udp_socket_(MakeGarbageCollected<FakeRestrictedUDPSocket>(scope.GetExecutionContext()))
    ...
    {
        scoped_refptr<base::SingleThreadTaskRunner> task_runner = scope.GetExecutionContext()->GetTaskRunner(TaskType::kNetworking);
        auto* udp_socket = MakeGarbageCollected<UDPSocketMojoRemote>(scope.GetExecutionContext());
        udp_socket->get().Bind(receiver_.BindNewPipeAndPassRemote(task_runner), task_runner);

        mojo::PendingReceiver<network::mojom::blink::UDPSocketListener> receiver;
        fake_udp_socket_->Bind(receiver.InitWithNewPipeAndPassRemote(), task_runner);

        auto* script_state = scope.GetScriptState();
        stream_wrapper_ = MakeGarbageCollected<UDPReadableStreamWrapper>(script_state, base::DoNothing(), udp_socket, std::move(receiver));
    ...
    FakeRestrictedUDPSocket& fake_udp_socket()
    {
        return *fake_udp_socket_;
    }
...
private:
    ...
    HeapMojoReceiver<network::mojom::blink::RestrictedUDPSocket, FakeRestrictedUDPSocket> receiver_;
};
...
public:
    ...
    ~ScopedStreamCreator()
    {
        stream_creator_->Cleanup();
    }

    StreamCreator* operator->() const
    {
...
std::pair<UDPMessage*, bool> UnpackPromiseResult(const V8TestingScope& scope, v8::Local<v8::Value> result)
{
    ...
    bool done = false;
    EXPECT_TRUE(V8UnpackIterationResult(scope.GetScriptState(), result.As<v8::Object>(), &udp_message_packed, &done));
    if (done) {
        return { nullptr, true };
    }
    auto* message = NativeValueTraits<UDPMessage>::NativeValue(scope.GetIsolate(), udp_message_packed, ASSERT_NO_EXCEPTION);
...
TEST(UDPReadableStreamWrapperTest, Create)
{
    ...
    ScopedStreamCreator stream_creator(MakeGarbageCollected<StreamCreator>(scope));
...
TEST(UDPReadableStreamWrapperTest, ReadUdpMessage)
{
    ...
    auto* script_state = scope.GetScriptState();
    auto* reader = udp_readable_stream_wrapper->Readable()->GetDefaultReaderForTesting(script_state, ASSERT_NO_EXCEPTION);
    ...
    ASSERT_EQ(UDPMessageDataToString(message), fake_udp_socket.GetTestingDatagram());
}
...
TEST(UDPReadableStreamWrapperTest, CancelStreamFromReader)
{
    ...
    ScriptPromiseTester cancel_tester(script_state, reader->cancel(script_state, ASSERT_NO_EXCEPTION));
    ...
    auto [message, done] = UnpackPromiseResult(scope, read_tester.Value().V8Value());
