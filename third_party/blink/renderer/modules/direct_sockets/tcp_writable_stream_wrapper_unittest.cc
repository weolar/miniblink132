...
public:
    ...
    TCPWritableStreamWrapper* Create(const V8TestingScope& scope, uint32_t capacity = 1)
    {
        ...
        stream_wrapper_ = MakeGarbageCollected<TCPWritableStreamWrapper>(script_state, base::DoNothing(), std::move(data_pipe_producer));
...
TEST(TCPWritableStreamWrapperTest, WriteArrayBuffer)
{
    ...
    auto* script_state = scope.GetScriptState();
    auto* writer = tcp_writable_stream_wrapper->Writable()->getWriter(script_state, ASSERT_NO_EXCEPTION);
...
TEST(TCPWritableStreamWrapperTest, AsyncWrite)
{
    ...
    auto* tcp_writable_stream_wrapper = stream_creator->Create(scope, kPipeCapacity);
...
TEST(TCPWritableStreamWrapperTest, DISABLED_TriggerHasAborted)
{
    ...
    EXPECT_EQ(tcp_writable_stream_wrapper->GetState(), StreamWrapper::State::kAborted);
}

class TCPWritableStreamWrapperCloseTestWithMaybePendingWrite : public testing::TestWithParam<bool> { };

INSTANTIATE_TEST_SUITE_P(/**/, TCPWritableStreamWrapperCloseTestWithMaybePendingWrite, testing::Bool());
...
TEST_P(TCPWritableStreamWrapperCloseTestWithMaybePendingWrite, TriggerClose)
{
    ...
    if (pending_write) {
        auto* script_state = scope.GetScriptState();
        auto* chunk = DOMArrayBuffer::Create(base::byte_span_with_nul_from_cstring("D"));
        auto write_promise = tcp_writable_stream_wrapper->Writable()
                                 ->getWriter(script_state, ASSERT_NO_EXCEPTION)
                                 ->write(script_state, ScriptValue::From(script_state, chunk), ASSERT_NO_EXCEPTION);
    ...
    ASSERT_EQ(tcp_writable_stream_wrapper->GetState(), StreamWrapper::State::kAborted);
}

TEST_P(TCPWritableStreamWrapperCloseTestWithMaybePendingWrite, TriggerCloseInReverseOrder)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    ScopedStreamCreator stream_creator(MakeGarbageCollected<StreamCreator>());
