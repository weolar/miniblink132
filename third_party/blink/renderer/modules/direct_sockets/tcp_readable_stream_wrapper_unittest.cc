...
public:
    ...
    TCPReadableStreamWrapper* Create(V8TestingScope& scope, uint32_t capacity = 0)
    {
        ...
        MojoResult result = mojo::CreateDataPipe(&options, data_pipe_producer_, data_pipe_consumer);
        ...
        stream_wrapper_ = MakeGarbageCollected<TCPReadableStreamWrapper>(
            script_state, WTF::BindOnce(&StreamCreator::Close, WrapWeakPersistent(this)), std::move(data_pipe_consumer));
    ...
    bool CloseCalledWith(bool error) const
    {
...
TEST(TCPReadableStreamWrapperTest, ReadArrayBuffer)
{
    ...
    auto* script_state = scope.GetScriptState();
    auto* reader = tcp_readable_stream_wrapper->Readable()->GetDefaultReaderForTesting(script_state, ASSERT_NO_EXCEPTION);
...
TEST(TCPReadableStreamWrapperTest, WriteToPipeWithPendingRead)
{
    ...
    StreamCreator::Iterator result = stream_creator->IteratorFromReadResult(scope, tester.Value().V8Value());
    EXPECT_FALSE(result.done);
    EXPECT_THAT(result.value, ElementsAre('A'));
}
...
INSTANTIATE_TEST_SUITE_P(/**/, TCPReadableStreamWrapperCloseTest, testing::Bool());
...
TEST_P(TCPReadableStreamWrapperCloseTest, TriggerClose)
{
    ...
    tcp_readable_stream_wrapper->ErrorStream(graceful ? net::OK : net::ERR_UNEXPECTED);
    ...
    ASSERT_TRUE(tester.IsFulfilled());
    ASSERT_EQ(tcp_readable_stream_wrapper->GetState(), graceful ? StreamWrapper::State::kClosed : StreamWrapper::State::kAborted);
}
...
TEST_P(TCPReadableStreamWrapperCloseTest, ErrorCancelReset)
{
    ...
    auto tester = ScriptPromiseTester(script_state, tcp_readable_stream_wrapper->Readable()->cancel(script_state, ASSERT_NO_EXCEPTION));
    tester.WaitUntilSettled();
    ASSERT_TRUE(tester.IsFulfilled());
...
TEST_P(TCPReadableStreamWrapperCloseTest, ResetCancelError)
{
    ...
    ASSERT_EQ(tcp_readable_stream_wrapper->GetState(), StreamWrapper::State::kClosed);
}
