...
mojom::blink::DirectTCPServerSocketOptionsPtr CreateTCPServerSocketOptions(
    const String& local_address, const TCPServerSocketOptions* options, ExceptionState& exception_state)
{
    ...
    net::IPEndPoint local_addr = net::IPEndPoint(std::move(address), options->hasLocalPort() ? options->localPort() : 0U);
...
TCPServerSocket::TCPServerSocket(ScriptState* script_state)
    : Socket(script_state)
    , opened_(MakeGarbageCollected<ScriptPromiseProperty<TCPServerSocketOpenInfo, DOMException>>(GetExecutionContext()))
{
}
...
// static
TCPServerSocket* TCPServerSocket::Create(
    ScriptState* script_state, const String& local_address, const TCPServerSocketOptions* options, ExceptionState& exception_state)
{
...
ScriptPromise<TCPServerSocketOpenInfo> TCPServerSocket::opened(ScriptState* script_state) const
{
    return opened_->Promise(script_state->World());
}

ScriptPromise<IDLUndefined> TCPServerSocket::close(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    auto readable_cancel = readable_stream_wrapper_->Readable()->cancel(script_state, ScriptValue::From(script_state, reason), exception_state);
...
bool TCPServerSocket::Open(const String& local_addr, const TCPServerSocketOptions* options, ExceptionState& exception_state)
{
    auto open_tcp_server_socket_options = CreateTCPServerSocketOptions(local_addr, options, exception_state);
    ...
    mojo::PendingReceiver<network::mojom::blink::TCPServerSocket> tcp_server_receiver = tcp_server_remote.InitWithNewPipeAndPassReceiver();

    GetServiceRemote()->OpenTCPServerSocket(std::move(open_tcp_server_socket_options), std::move(tcp_server_receiver),
        WTF::BindOnce(&TCPServerSocket::OnTCPServerSocketOpened, WrapPersistent(this), std::move(tcp_server_remote)));
    return true;
}

void TCPServerSocket::OnTCPServerSocketOpened(
    mojo::PendingRemote<network::mojom::blink::TCPServerSocket> tcp_server_remote, int32_t result, const std::optional<net::IPEndPoint>& local_addr)
{
    if (result == net::OK) {
        DCHECK(local_addr);
        readable_stream_wrapper_ = MakeGarbageCollected<TCPServerReadableStreamWrapper>(
            GetScriptState(), WTF::BindOnce(&TCPServerSocket::OnReadableStreamClosed, WrapPersistent(this)), std::move(tcp_server_remote));
