...
MODULES_EXPORT media::VideoCodec MediaVideoCodecFromCodecId(VideoTrackRecorder::CodecId id);

// Helper function to parse a codec string to codec/profile/level.
MODULES_EXPORT VideoTrackRecorder::CodecProfile VideoStringToCodecProfile(const String& codecs);
...
class MODULES_EXPORT MediaRecorderHandler final : public GarbageCollected<MediaRecorderHandler>,
                                                  public VideoTrackRecorder::CallbackInterface,
                                                  public AudioTrackRecorder::CallbackInterface,
                                                  public WebMediaStreamObserver {
public:
    MediaRecorderHandler(scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner, KeyFrameRequestProcessor::Configuration key_frame_config);
    ...
    bool Initialize(MediaRecorder* client, MediaStreamDescriptor* media_stream, const String& type, const String& codecs,
        AudioTrackRecorder::BitrateMode audio_bitrate_mode);

    AudioTrackRecorder::BitrateMode AudioBitrateMode();

    bool Start(int timeslice, const String& type, uint32_t audio_bits_per_second, uint32_t video_bits_per_second);
    void Stop();
    void Pause();
    void Resume();
    ...
    using OnMediaCapabilitiesEncodingInfoCallback = base::OnceCallback<void(std::unique_ptr<WebMediaCapabilitiesInfo>)>;
    void EncodingInfo(const WebMediaConfiguration& configuration, OnMediaCapabilitiesEncodingInfoCallback cb);
...
private:
    ...
    void OnEncodedVideo(const media::Muxer::VideoParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
        std::optional<media::VideoEncoder::CodecDescription> codec_description, base::TimeTicks timestamp) override;
    void OnPassthroughVideo(const media::Muxer::VideoParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data, base::TimeTicks timestamp) override;
    std::unique_ptr<media::VideoEncoderMetricsProvider> CreateVideoEncoderMetricsProvider() override;
    ...
    void OnEncodedAudio(const media::AudioParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
        std::optional<media::AudioEncoder::CodecDescription> codec_description, base::TimeTicks timestamp) override;
    ...
    void HandleEncodedVideo(const media::Muxer::VideoParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
    ...
    void OnVideoFrameForTesting(scoped_refptr<media::VideoFrame> frame, const base::TimeTicks& timestamp);
    void OnEncodedVideoFrameForTesting(scoped_refptr<EncodedVideoFrame> frame, const base::TimeTicks& timestamp);
    void OnAudioBusForTesting(const media::AudioBus& audio_bus, const base::TimeTicks& timestamp);
    void SetAudioFormatForTesting(const media::AudioParameters& params);
    void UpdateTrackLiveAndEnabled(const MediaStreamComponent& track, bool is_video);
    ...
    VideoTrackRecorder::CodecProfile video_codec_profile_ { VideoTrackRecorder::CodecId::kLast };

    // Audio Codec, OPUS is used by default.
    AudioTrackRecorder::CodecId audio_codec_id_ { AudioTrackRecorder::CodecId::kLast };
    ...
    // coercion.
    WeakCellFactory<AudioTrackRecorder::CallbackInterface> weak_audio_factory_ { this };
    WeakCellFactory<VideoTrackRecorder::CallbackInterface> weak_video_factory_ { this };
