...
class MODULES_EXPORT MediaRecorderEncoderWrapper final : public VideoTrackRecorder::Encoder {
public:
    using CreateEncoderCB = base::RepeatingCallback<std::unique_ptr<media::VideoEncoder>(media::GpuVideoAcceleratorFactories*)>;
    using OnErrorCB = base::OnceClosure;

    MediaRecorderEncoderWrapper(scoped_refptr<base::SequencedTaskRunner> encoding_task_runner, media::VideoCodecProfile profile, uint32_t bits_per_second,
        bool is_screencast, media::GpuVideoAcceleratorFactories* gpu_factories, CreateEncoderCB create_encoder_cb,
    ...
    MediaRecorderEncoderWrapper& operator=(const MediaRecorderEncoderWrapper&) = delete;
...
private:
    ...
    struct EncodeTask {
        EncodeTask(scoped_refptr<media::VideoFrame> frame, base::TimeTicks capture_timestamp, bool request_keyframe);
        ~EncodeTask();
    ...
    struct VideoParamsAndTimestamp {
        VideoParamsAndTimestamp(const media::Muxer::VideoParameters& params, base::TimeTicks timestamp);
    ...
    void EncodeFrame(scoped_refptr<media::VideoFrame> frame, base::TimeTicks capture_timestamp, bool request_keyframe) override;
    ...
    // the flush fails.
    void CreateAndInitialize(const gfx::Size& frame_size, bool encode_alpha, media::EncoderStatus status);
    void InitializeDone(media::EncoderStatus status);
    void EncodePendingTasks();
    void EncodeDone(media::EncoderStatus status);
    void OutputEncodeData(media::VideoEncoderOutput output, std::optional<media::VideoEncoder::CodecDescription> description);
