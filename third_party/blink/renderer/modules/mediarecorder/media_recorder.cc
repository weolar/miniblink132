...
V8BitrateMode::Enum BitrateModeToV8Enum(AudioTrackRecorder::BitrateMode bitrateMode)
{
    switch (bitrateMode) {
...
AudioTrackRecorder::BitrateMode GetBitrateModeFromOptions(const MediaRecorderOptions* const options)
{
...
uint32_t ClampAudioBitRate(ExecutionContext* context, uint32_t audio_bps)
{
    if (audio_bps > kLargestPossibleOpusBitRate) {
        LogConsoleMessage(context, String::Format("Clamping calculated audio bitrate (%dbps) to the maximum (%dbps)", audio_bps, kLargestPossibleOpusBitRate));
        return kLargestPossibleOpusBitRate;
    }
    if (audio_bps < kSmallestPossibleOpusBitRate) {
        LogConsoleMessage(context, String::Format("Clamping calculated audio bitrate (%dbps) to the minimum (%dbps)", audio_bps, kSmallestPossibleOpusBitRate));
...
MediaRecorderBitrates GetBitratesFromOptions(ExceptionState& exception_state, ExecutionContext* context, const MediaRecorderOptions* options)
{
    ...
    if (options->hasBitsPerSecond()) {
        ...
        video_bps = overall_bps.value() >= audio_bps.value() ? overall_bps.value() - audio_bps.value() : 0u;
...
MediaRecorder* MediaRecorder::Create(ExecutionContext* context, MediaStream* stream, ExceptionState& exception_state)
{
    return MakeGarbageCollected<MediaRecorder>(context, stream, MediaRecorderOptions::Create(), exception_state);
}

MediaRecorder* MediaRecorder::Create(ExecutionContext* context, MediaStream* stream, const MediaRecorderOptions* options, ExceptionState& exception_state)
{
    return MakeGarbageCollected<MediaRecorder>(context, stream, options, exception_state);
}

MediaRecorder::MediaRecorder(ExecutionContext* context, MediaStream* stream, const MediaRecorderOptions* options, ExceptionState& exception_state)
    ...
    , mime_type_(options->mimeType())
{
    if (context->IsContextDestroyed()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Execution context is detached.");
        return;
    }
    if (options->hasVideoKeyFrameIntervalDuration() && options->hasVideoKeyFrameIntervalCount()) {
    ...
    if (options->hasVideoKeyFrameIntervalDuration()) {
        key_frame_config = base::Milliseconds(options->videoKeyFrameIntervalDuration());
    } else if (options->hasVideoKeyFrameIntervalCount()) {
        key_frame_config = options->videoKeyFrameIntervalCount();
    }
    recorder_handler_ = MakeGarbageCollected<MediaRecorderHandler>(context->GetTaskRunner(TaskType::kInternalMediaRealTime), key_frame_config);
    if (!recorder_handler_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "No MediaRecorder handler can be created.");
        return;
    }

    const MediaRecorderBitrates bitrates = GetBitratesFromOptions(exception_state, context, options);
    const ContentType content_type(mime_type_);
    if (!recorder_handler_->Initialize(
            this, stream->Descriptor(), content_type.GetType(), content_type.Parameter("codecs"), GetBitrateModeFromOptions(options))) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kNotSupportedError, "Failed to initialize native MediaRecorder the type provided (" + mime_type_ + ") is not supported.");
    }
...
V8BitrateMode MediaRecorder::audioBitrateMode() const
{
    ...
    return V8BitrateMode(BitrateModeToV8Enum(recorder_handler_->AudioBitrateMode()));
}
...
void MediaRecorder::start(int time_slice, ExceptionState& exception_state)
{
    ...
    if (state_ != State::kInactive) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The MediaRecorder's state is '" + state().AsString() + "'.");
        return;
    }
    ...
    if (stream_->getAudioTracks().size() == 0) {
        ...
        if (overall_bits_per_second_.has_value()) {
            video_bits_per_second_ = ClampVideoBitRate(GetExecutionContext(), overall_bits_per_second_.value());
        }
    }
    ...
    if (stream_->getVideoTracks().size() == 0) {
        ...
        if (overall_bits_per_second_.has_value()) {
            audio_bits_per_second_ = ClampAudioBitRate(GetExecutionContext(), overall_bits_per_second_.value());
        }
    }

    const ContentType content_type(mime_type_);
    if (!recorder_handler_->Start(time_slice, content_type.GetType(), audio_bits_per_second_, video_bits_per_second_)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "There was an error starting the MediaRecorder.");
    }
}
...
void MediaRecorder::pause(ExceptionState& exception_state)
{
    ...
    if (state_ == State::kInactive) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The MediaRecorder's state is 'inactive'.");
        return;
    }
...
bool MediaRecorder::isTypeSupported(ExecutionContext* context, const String& type)
{
    MediaRecorderHandler* handler
        = MakeGarbageCollected<MediaRecorderHandler>(context->GetTaskRunner(TaskType::kInternalMediaRealTime), KeyFrameRequestProcessor::Configuration());
    if (!handler)
        return false;
    ...
    bool result = handler->CanSupportMimeType(content_type.GetType(), content_type.Parameter("codecs"));
    if (IdentifiabilityStudySettings::Get()->ShouldSampleType(blink::IdentifiableSurface::Type::kMediaRecorder_IsTypeSupported)) {
        blink::IdentifiabilityMetricBuilder(context->UkmSourceID())
            .Add(blink::IdentifiableSurface::FromTypeAndToken(
                     blink::IdentifiableSurface::Type::kMediaRecorder_IsTypeSupported, IdentifiabilityBenignStringToken(type)),
                result)
...
void MediaRecorder::ContextDestroyed()
{
    if (blob_data_) {
        ...
        CreateBlobEvent(MakeGarbageCollected<Blob>(BlobDataHandle::Create(std::move(blob_data_), blob_data_length)));
    }
...
void MediaRecorder::WriteData(base::span<const uint8_t> data, bool last_in_slice, ErrorEvent* error_event)
{
...
void MediaRecorder::OnError(DOMExceptionCode code, const String& message)
{
    ...
    ScriptState* script_state = ToScriptStateForMainWorld(DomWindow()->GetFrame());
    ScriptState::Scope scope(script_state);
    ScriptValue error_value = ScriptValue::From(script_state, MakeGarbageCollected<DOMException>(code, message));
    ErrorEventInit* event_init = ErrorEventInit::Create();
    event_init->setError(error_value);
    StopRecording(ErrorEvent::Create(script_state, event_type_names::kError, event_init));
}
...
{
    DVLOG(1) << __func__ << " message=" << message.Ascii() << " state_=" << static_cast<int>(state_);
    if (state_ != State::kInactive) {
...
void MediaRecorder::CreateBlobEvent(Blob* blob)
{
    ...
    if (!blob_event_first_chunk_timecode_.has_value()) {
        ...
        timecode = (now - blob_event_first_chunk_timecode_.value()).InMillisecondsF();
    }

    ScheduleDispatchEvent(MakeGarbageCollected<BlobEvent>(event_type_names::kDataavailable, blob, timecode));
}
    ...
    if (scheduled_events_.size() == 1) {
        if (auto* context = GetExecutionContext()) {
            ...
            context->GetTaskRunner(TaskType::kDOMManipulation)
                ->PostTask(FROM_HERE, WTF::BindOnce(&MediaRecorder::DispatchScheduledEvent, WrapPersistent(this)));
        }
    }
}
