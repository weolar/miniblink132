...
#if (BUILDFLAG(IS_MAC) || BUILDFLAG(IS_ANDROID)) && BUILDFLAG(USE_PROPRIETARY_CODECS)
#define HAS_AAC_ENCODER 1
#endif
...
{
    auto* source = MakeGarbageCollected<MediaStreamSource>("sourceId", MediaStreamSource::kTypeAudio, "sourceName", false,
        /*platform_source=*/nullptr);
    auto* component = MakeGarbageCollected<MediaStreamComponentImpl>("audioTrack", source, std::make_unique<MediaStreamAudioTrack>(/*is_local_track=*/true));

    auto* track = MakeGarbageCollected<MediaStreamTrackImpl>(scope.GetExecutionContext(), component);

    HeapVector<Member<MediaStreamTrack>> tracks;
    tracks.push_back(track);

    MediaStream* stream = MediaStream::Create(scope.GetExecutionContext(), tracks);
...
public:
    explicit MockMediaRecorder(V8TestingScope& scope)
        : MediaRecorder(scope.GetExecutionContext(), CreateMediaStream(scope), MediaRecorderOptions::Create(), scope.GetExceptionState())
    {
    }
...
public:
    MediaRecorderHandlerFixture(bool has_video, bool has_audio)
        ...
        , media_recorder_handler_(
              MakeGarbageCollected<MediaRecorderHandler>(scheduler::GetSingleThreadTaskRunnerForTesting(), KeyFrameRequestProcessor::Configuration()))
        , audio_source_(kTestAudioChannels, 440 /* freq */, kTestAudioSampleRate)
    {
    ...
    {
        return media_recorder_handler_->recording_;
    }
    ...
    {
        return media_recorder_handler_->audio_recorders_[0]->callback_interface_for_testing();
    }
    ...
    {
        media_recorder_handler_->OnVideoFrameForTesting(std::move(frame), base::TimeTicks::Now());
    }

    void OnEncodedVideoForTesting(const media::Muxer::VideoParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data, base::TimeTicks timestamp,
        std::optional<media::VideoEncoder::CodecDescription> codec_description = std::nullopt)
    {
        media_recorder_handler_->OnEncodedVideo(params, std::move(encoded_data), std::move(codec_description), timestamp);
    }

    void OnEncodedAudioForTesting(const media::AudioParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data, base::TimeTicks timestamp)
    {
        media::AudioEncoder::CodecDescription codec_description = { 99 };
        media_recorder_handler_->OnEncodedAudio(params, std::move(encoded_data), std::move(codec_description), timestamp);
    }

    void OnEncodedAudioNoCodeDescriptionForTesting(
        const media::AudioParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data, base::TimeTicks timestamp)
    {
        media_recorder_handler_->OnEncodedAudio(params, std::move(encoded_data), std::nullopt, timestamp);
    }

    void OnAudioBusForTesting(const media::AudioBus& audio_bus)
    {
        media_recorder_handler_->OnAudioBusForTesting(audio_bus, base::TimeTicks::Now());
    }
    ...
    {
        static_cast<media::WebmMuxer*>(media_recorder_handler_->muxer_adapter_->GetMuxerForTesting())->ForceOneLibWebmErrorForTesting();
    }

    std::unique_ptr<media::AudioBus> NextAudioBus()
    {
        std::unique_ptr<media::AudioBus> bus(media::AudioBus::Create(kTestAudioChannels, kTestAudioSampleRate * kTestAudioBufferDurationMs / 1000));
        audio_source_.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), {}, bus.get());
        return bus;
    }

    void OnEncodedH264VideoForTesting(base::TimeTicks timestamp, std::optional<media::VideoEncoder::CodecDescription> codec_description = std::nullopt)
    {
        ...
        if (h264_video_stream_.empty()) {
            ...
            h264_video_stream_ = base::HeapArray<uint8_t>::CopiedFrom(mapped_h264_file.bytes());
        }
        media::Muxer::VideoParameters video_params(gfx::Size(), 1, media::VideoCodec::kH264, gfx::ColorSpace());
        auto buffer = media::DecoderBuffer::CopyFrom(h264_video_stream_);
        std::string alpha_data = "alpha";
        buffer->WritableSideData().alpha_data = base::HeapArray<uint8_t>::CopiedFrom(base::as_byte_span(alpha_data));
        buffer->set_is_key_frame(true);
        OnEncodedVideoForTesting(video_params, buffer, timestamp, std::move(codec_description));
    }

#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    void PopulateAVCDecoderConfiguration(std::vector<uint8_t>& codec_description)
    {
        ...
        std::vector<uint8_t> test_data {
            ...
            0x67, 0x64, 0x0, 0xc, 0xac, 0xd9, 0x41, 0x41, 0xfb, 0x1, 0x10, 0x0, 0x0, 0x3, 0x0, 0x10, 0x0, 0x0, 0x3, 0x1, 0x40, 0xf1, 0x42, 0x99, 0x60,
        ...
        ASSERT_TRUE(avc_config.Parse(test_data.data(), static_cast<int>(test_data.size())));
    ...
    raw_ptr<MockMediaStreamVideoSource, DanglingUntriaged> video_source_ = nullptr;
    base::HeapArray<uint8_t> h264_video_stream_;

private:
    void LoadEncodedFile(std::string_view filename, base::MemoryMappedFile& mapped_stream)
    {
        base::FilePath file_path = GetTestDataFilePath(filename);

        ASSERT_TRUE(mapped_stream.Initialize(file_path)) << "Couldn't open stream file: " << file_path.MaybeAsASCII();
    }
    ...
    base::FilePath GetTestDataFilePath(std::string_view name)
    {
        ...
        file_path = file_path.Append(FILE_PATH_LITERAL("media")).Append(FILE_PATH_LITERAL("test")).Append(FILE_PATH_LITERAL("data")).AppendASCII(name);
        return file_path;
    }
};

class MediaRecorderHandlerTest : public TestWithParam<MediaRecorderTestParams>, public MediaRecorderHandlerFixture {
public:
    MediaRecorderHandlerTest()
        : MediaRecorderHandlerFixture(GetParam().has_video, GetParam().has_audio)
    {
    ...
    bool IsStreamWriteSupported()
    {
        ...
        if (EqualIgnoringASCIICase(GetParam().mime_type, "video/mp4") || EqualIgnoringASCIICase(GetParam().mime_type, "audio/mp4")) {
            return false;
        }
#endif
        return true;
...
TEST_P(MediaRecorderHandlerTest, CanSupportMimeType)
{
    ...
    EXPECT_FALSE(media_recorder_handler_->CanSupportMimeType(unsupported_mime_type, String()));

    const String mime_type_video("video/webm");
    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(mime_type_video, String()));
    const String mime_type_video_uppercase("video/WEBM");
    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(mime_type_video_uppercase, String()));
    const String example_good_codecs_1("vp8");
    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(mime_type_video, example_good_codecs_1));
    const String example_good_codecs_2("vp9,opus");
    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(mime_type_video, example_good_codecs_2));
    const String example_good_codecs_3("VP9,opus");
    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(mime_type_video, example_good_codecs_3));
    const String example_good_codecs_4("H264");
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(mime_type_video, example_good_codecs_4));
#else
    EXPECT_FALSE(media_recorder_handler_->CanSupportMimeType(mime_type_video, example_good_codecs_4));
#endif

    const String example_unsupported_codecs_1("daala");
    EXPECT_FALSE(media_recorder_handler_->CanSupportMimeType(mime_type_video, example_unsupported_codecs_1));

    const String mime_type_audio("audio/webm");
    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(mime_type_audio, String()));
    const String example_good_codecs_5("opus");
    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(mime_type_audio, example_good_codecs_5));
    const String example_good_codecs_6("OpUs");
    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(mime_type_audio, example_good_codecs_6));
    const String example_good_codecs_7("pcm");
    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(mime_type_audio, example_good_codecs_7));

    const String example_good_codecs_8("AV01,opus");
    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(mime_type_video, example_good_codecs_5));

    const String example_unsupported_codecs_2("vorbis");
    EXPECT_FALSE(media_recorder_handler_->CanSupportMimeType(mime_type_audio, example_unsupported_codecs_2));

    const String example_good_codecs_with_unsupported_tag("HEV1");
    EXPECT_FALSE(media_recorder_handler_->CanSupportMimeType(mime_type_video, example_good_codecs_with_unsupported_tag));

    const String example_good_codecs_with_supported_tag("hvc1");
    EXPECT_FALSE(media_recorder_handler_->CanSupportMimeType(mime_type_video, example_good_codecs_with_supported_tag));
}
...
TEST_P(MediaRecorderHandlerTest, SupportsBitrateMode)
{
    ...
    EXPECT_TRUE(media_recorder_handler_->Initialize(recorder, registry_.test_stream(), mime_type, codecs, AudioTrackRecorder::BitrateMode::kVariable));
    EXPECT_EQ(media_recorder_handler_->AudioBitrateMode(), AudioTrackRecorder::BitrateMode::kVariable);

    EXPECT_TRUE(media_recorder_handler_->Initialize(recorder, registry_.test_stream(), mime_type, codecs, AudioTrackRecorder::BitrateMode::kConstant));
    EXPECT_EQ(media_recorder_handler_->AudioBitrateMode(), AudioTrackRecorder::BitrateMode::kConstant);
}
...
TEST_P(MediaRecorderHandlerTest, EncodeVideoFrames)
{
    ...
    if ((GetParam().has_audio && !GetParam().use_mp4_muxer) || !IsCodecSupported()) {
        return;
    }
    ...
    if (GetParam().use_mp4_muxer) {
        {
            ...
            EXPECT_CALL(*recorder, WriteData(SizeIs(Lt(kMfraBoxSize)), _, _)).Times(AtLeast(1));
            EXPECT_CALL(*recorder, WriteData(SizeIs(Gt(kMfraBoxSize)), _, _)).Times(AtLeast(1));
            EXPECT_CALL(*recorder, WriteData(SizeIs(kMfraBoxSize), _, _)).Times(1).WillOnce(RunOnceClosure(run_loop.QuitClosure()));
    ...
    } else {
        {
            ...
            EXPECT_CALL(*recorder, WriteData(SizeIs(Lt(kEncodedSizeThreshold)), _, _)).Times(AtLeast(1));
            EXPECT_CALL(*recorder, WriteData(SizeIs(Gt(kEncodedSizeThreshold)), _, _)).Times(1).WillOnce(RunOnceClosure(run_loop.QuitClosure()));
        ...
        Mock::VerifyAndClearExpectations(recorder);
        {
            const scoped_refptr<media::VideoFrame> alpha_frame = media::VideoFrame::CreateTransparentFrame(gfx::Size(160, 80));
            const size_t kEncodedSizeThreshold = 16;
...
TEST_P(MediaRecorderHandlerTest, OpusEncodeAudioFrames)
{
    ...
    media::AudioParameters params(media::AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::Stereo(), kTestAudioSampleRate,
    ...
    if (GetParam().use_mp4_muxer) {
        ...
        EXPECT_CALL(*recorder, WriteData(SizeIs(Lt(kEncodedSizeThreshold)), _, _)).Times(AtLeast(1));
        EXPECT_CALL(*recorder, WriteData(SizeIs(Gt(kEncodedSizeThreshold)), _, _)).Times(2).WillOnce(RunOnceClosure(run_loop.QuitClosure()));

        media::AudioParameters audio_params(media::AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::Stereo(), kTestAudioSampleRate,
            kTestAudioSampleRate * kTestAudioBufferDurationMs / 1000);

        // Null codec_description is used for Opus.
        auto buffer = media::DecoderBuffer::CopyFrom(base::as_byte_span("audio"));
        OnEncodedAudioNoCodeDescriptionForTesting(audio_params, buffer, base::TimeTicks::Now());
...
TEST_P(MediaRecorderHandlerTest, WebmMuxerErrorWhileEncoding)
{
    ...
    if (GetParam().has_audio || !IsCodecSupported() || !IsStreamWriteSupported() || GetParam().use_mp4_muxer) {
        return;
    }
...

    {
        ...
        EXPECT_CALL(*recorder, OnError).Times(1).WillOnce(RunOnceClosure(run_loop.QuitClosure()));
...
TEST_P(MediaRecorderHandlerTest, ActualMimeType)
{
    ...
    EXPECT_EQ(media_recorder_handler_->ActualMimeType(), actual_mime_type.ToString());
}
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
        EXPECT_CALL(*recorder, WriteData).Times(AtLeast(1));
        media::Muxer::VideoParameters params(gfx::Size(), 1, media::VideoCodec::kH264, gfx::ColorSpace());
        std::vector<uint8_t> codec_description;
        PopulateAVCDecoderConfiguration(codec_description);
        OnEncodedH264VideoForTesting(base::TimeTicks::Now(), std::move(codec_description));
        media_recorder_handler_->Stop();
#endif
    } else {
        EXPECT_CALL(*recorder, WriteData).Times(AtLeast(1));
        media::Muxer::VideoParameters params(gfx::Size(), 1, media::VideoCodec::kVP9, gfx::ColorSpace());
        if (IsAvc1CodecSupported(codecs)) {
            OnEncodedH264VideoForTesting(base::TimeTicks::Now());
        } else {
            auto buffer = media::DecoderBuffer::CopyFrom(base::as_byte_span("vp9 frame"));
            std::string alpha_data = "alpha";
            buffer->WritableSideData().alpha_data = base::HeapArray<uint8_t>::CopiedFrom(base::as_byte_span(alpha_data));
            buffer->set_is_key_frame(true);
...
INSTANTIATE_TEST_SUITE_P(All, MediaRecorderHandlerTest, ValuesIn(kMediaRecorderTestParams));
class MediaRecorderHandlerTestForMp4 : public TestWithParam<MediaRecorderTestParams>, public MediaRecorderHandlerFixture {
public:
...
TEST_P(MediaRecorderHandlerTestForMp4, InitializeFailedWhenMP4MuxerFeatureDisabled)
{
    if (!IsTargetAudioCodecSupported(GetParam().codecs)) {
        return;
    }
    ...
    const String mime_type(GetParam().mime_type);
    const String codecs(GetParam().codecs);
    EXPECT_FALSE(media_recorder_handler_->Initialize(recorder, registry_.test_stream(), mime_type, codecs, AudioTrackRecorder::BitrateMode::kVariable));
}

INSTANTIATE_TEST_SUITE_P(All, MediaRecorderHandlerTestForMp4, ValuesIn(kMediaRecorderTestParamsForMp4));

class MediaRecorderHandlerIsSupportedTypeTestForMp4 : public TestWithParam<bool>, public MediaRecorderHandlerFixture {
public:
...
// Checks that canSupportMimeType() works as expected, by sending supported
// combinations and unsupported ones.
TEST_P(MediaRecorderHandlerIsSupportedTypeTestForMp4, CanSupportMimeTypeForMp4)
{
    // video types.
    const String good_mp4_video_mime_types[] = { "video/mp4" };
    const String bad_mp4_video_mime_types[] = { "video/MP4" };

    const String good_mp4_video_codecs[] = { "avc1", "avc1.420034", "vp9", "av01", "av01.2.19H.08.0.000.09.16.09.1" };
    const String bad_mp4_video_codecs[] = { "h264", "vp8", "avc11", "aVc1", "avc1.123456", "av1" };

    const String good_mp4_video_codecs_non_proprietory[] = { "vp9", "av01", "av01.2.19H.08.0.000.09.16.09.1" };
    const String bad_mp4_video_codecs_non_proprietory[] = { "avc1", "h264", "vp8", "avc11", "aVc1", "avc1.123456", "av1" };
    ...
    const String bad_mp4_audio_codecs[] = { "mp4a", "mp4a.40", "mP4a.40.2", "aac", "pcm" };

    const String good_mp4_audio_codecs_non_proprietory[] = { "opus" };
    const String bad_mp4_audio_codecs_non_proprietory[] = { "mp4a.40.2", "mp4a", "mp4a.40", "mP4a.40.2", "aac", "pcm" };
        ...
        for (const auto& type : good_mp4_video_mime_types) {
            for (const auto& video_codec : good_mp4_video_codecs) {
                ...
                for (const auto& audio_codec : good_mp4_audio_codecs) {
                    ...
                    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(type, codecs));

                    String codecs2 = audio_codec + "," + video_codec;
                    EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType(type, codecs2));
                }
            }
        }
        ...
        for (const auto& type : good_mp4_audio_mime_types) {
            for (const auto& video_codec : good_mp4_video_codecs) {
                ...
                for (const auto& audio_codec : good_mp4_audio_codecs) {
                    ...
                    String codecs2 = audio_codec + "," + video_codec;
                    EXPECT_FALSE(media_recorder_handler_->CanSupportMimeType(type, codecs2));
                }
            }
        }
#else
...
TEST_P(MediaRecorderHandlerIsSupportedTypeTestForMp4, CanSupportAacCodecForWinNSku)
{
    if (!GetParam()) {
        GTEST_SKIP();
    }
    ...
    {
        base::test::ScopedOSInfoOverride scoped_os_info_override(base::test::ScopedOSInfoOverride::Type::kWin11Home);
        EXPECT_TRUE(media_recorder_handler_->CanSupportMimeType("audio/mp4", "mp4a.40.2"));
    }

    {
        base::test::ScopedOSInfoOverride scoped_os_info_override(base::test::ScopedOSInfoOverride::Type::kWin11HomeN);
        EXPECT_FALSE(media_recorder_handler_->CanSupportMimeType("audio/mp4", "mp4a.40.2"));
    }
}
#endif // BUILDFLAG(IS_WIN) && BUILDFLAG(USE_PROPRIETARY_CODECS)

INSTANTIATE_TEST_SUITE_P(All, MediaRecorderHandlerIsSupportedTypeTestForMp4,
    ValuesIn({ /*MediaRecorderEnableMp4Muxer enabled=*/true,
        /*MediaRecorderEnableMp4Muxer disabled=*/false }));

class MediaRecorderHandlerAudioVideoTest : public testing::Test, public MediaRecorderHandlerFixture {
public:
    ...
    void FeedVideo()
    {
        media::Muxer::VideoParameters video_params(gfx::Size(), 1, media::VideoCodec::kVP9, gfx::ColorSpace());
...
TEST_F(MediaRecorderHandlerAudioVideoTest, IgnoresStaleEncodedMediaOnRestart)
{
    ...
    V8TestingScope scope;
    auto* recorder = MakeGarbageCollected<MockMediaRecorder>(scope);
    media_recorder_handler_->Initialize(recorder, registry_.test_stream(), "video/webm", "vp9,opus", AudioTrackRecorder::BitrateMode::kVariable);
    media_recorder_handler_->Start(std::numeric_limits<int>::max(), "video/webm", 0, 0);
...
TEST_F(MediaRecorderHandlerAudioVideoTest, CorrectH264LevelOnWrite)
{
    ...
    V8TestingScope scope;
    auto* recorder = MakeGarbageCollected<MockMediaRecorder>(scope);
    media_recorder_handler_->Initialize(recorder, registry_.test_stream(), "video/webm", "avc1.640022,opus", AudioTrackRecorder::BitrateMode::kVariable);

    EXPECT_EQ(media_recorder_handler_->ActualMimeType(), "video/x-matroska;codecs=avc1.640022,opus");
    media_recorder_handler_->Start(std::numeric_limits<int>::max(), "video/webm", 0, 0);
    ...
    EXPECT_EQ(media_recorder_handler_->ActualMimeType(), "video/x-matroska;codecs=avc1.64000d,opus");
    media_recorder_handler_ = nullptr;
    Mock::VerifyAndClearExpectations(recorder);
}
#endif

TEST_F(MediaRecorderHandlerAudioVideoTest, EmitsCachedAudioDataAfterVideoTrackEnded)
{
    AddTracks();
    V8TestingScope scope;
    auto* recorder = MakeGarbageCollected<MockMediaRecorder>(scope);
...
class MediaRecorderHandlerH264ProfileTest : public TestWithParam<H264ProfileTestParams>, public MediaRecorderHandlerFixture {
public:
    ...
    MediaRecorderHandlerH264ProfileTest(const MediaRecorderHandlerH264ProfileTest&) = delete;
    MediaRecorderHandlerH264ProfileTest& operator=(const MediaRecorderHandlerH264ProfileTest&) = delete;
...
TEST_P(MediaRecorderHandlerH264ProfileTest, ActualMimeType)
{
    ...
    String actual_mime_type = String(GetParam().mime_type) + ";codecs=" + GetParam().codecs;
...
INSTANTIATE_TEST_SUITE_P(All, MediaRecorderHandlerH264ProfileTest, ValuesIn(kH264ProfileTestParams));

#if BUILDFLAG(IS_WIN)
class MediaRecorderHandlerWinAacCodecTest : public TestWithParam<unsigned int>, public MediaRecorderHandlerFixture {
public:
    ...
    MediaRecorderHandlerWinAacCodecTest(const MediaRecorderHandlerWinAacCodecTest&) = delete;
    MediaRecorderHandlerWinAacCodecTest& operator=(const MediaRecorderHandlerWinAacCodecTest&) = delete;
...
TEST_P(MediaRecorderHandlerWinAacCodecTest, AudioBitsPerSeconds)
{
    ...
    EXPECT_EQ(media::MFAudioEncoder::ClampAccCodecBitrate(GetParam()), recorder->audioBitsPerSecond());

    media_recorder_handler_->Stop();
    media_recorder_handler_ = nullptr;
}

INSTANTIATE_TEST_SUITE_P(All, MediaRecorderHandlerWinAacCodecTest, ValuesIn({ 5000u, 96000u, 128000u, 160000u, 192000u, 256000u, 300000u }));
...
static const MediaRecorderPassthroughTestParams kMediaRecorderPassthroughTestParams[] = {
...
class MediaRecorderHandlerPassthroughTest : public TestWithParam<MediaRecorderPassthroughTestParams>, public ScopedMockOverlayScrollbars {
public:
    ...
    MediaRecorderHandlerPassthroughTest(const MediaRecorderHandlerPassthroughTest&) = delete;
    MediaRecorderHandlerPassthroughTest& operator=(const MediaRecorderHandlerPassthroughTest&) = delete;
    ...
    {
        media_recorder_handler_->OnEncodedVideoFrameForTesting(std::move(frame), base::TimeTicks::Now());
    }
...
TEST_P(MediaRecorderHandlerPassthroughTest, PassesThrough)
{
    ...
    V8TestingScope scope;
    auto* recorder = MakeGarbageCollected<MockMediaRecorder>(scope);
    media_recorder_handler_->Initialize(recorder, registry_.test_stream(), "", "", AudioTrackRecorder::BitrateMode::kVariable);
    media_recorder_handler_->Start(0, "", 0, 0);

    const size_t kFrameSize = 42;
    auto frame = FakeEncodedVideoFrame::Builder().WithKeyFrame(true).WithCodec(GetParam().codec).WithData(std::string(kFrameSize, 'P')).BuildRefPtr();
    {
        base::RunLoop run_loop;
        EXPECT_CALL(*recorder, WriteData).Times(AtLeast(1));
        EXPECT_CALL(*recorder, WriteData(SizeIs(Ge(kFrameSize)), _, _)).Times(1).WillOnce(RunOnceClosure(run_loop.QuitClosure()));
        OnVideoFrameForTesting(frame);
        run_loop.Run();
    }

    EXPECT_EQ(media_recorder_handler_->ActualMimeType(), String(GetParam().mime_type));
    Mock::VerifyAndClearExpectations(recorder);
...
TEST_F(MediaRecorderHandlerPassthroughTest, ErrorsOutOnCodecSwitch)
{
    V8TestingScope scope;
    auto* recorder = MakeGarbageCollected<MockMediaRecorder>(scope);
    EXPECT_TRUE(media_recorder_handler_->Initialize(recorder, registry_.test_stream(), "", "", AudioTrackRecorder::BitrateMode::kVariable));
    ...
    OnVideoFrameForTesting(
        FakeEncodedVideoFrame::Builder().WithKeyFrame(true).WithCodec(media::VideoCodec::kVP8).WithData(std::string("vp8 frame")).BuildRefPtr());
    // Switch to VP9 frames. This is expected to cause the call to OnError
    // above.
    OnVideoFrameForTesting(
        FakeEncodedVideoFrame::Builder().WithKeyFrame(true).WithCodec(media::VideoCodec::kVP9).WithData(std::string("vp9 frame")).BuildRefPtr());
...
INSTANTIATE_TEST_SUITE_P(All, MediaRecorderHandlerPassthroughTest, ValuesIn(kMediaRecorderPassthroughTestParams));
...
static const MediaRecorderCodecProfileTestParams kMediaRecorderCodecProfileTestParams[] = {
    { "vp8,mp4a.40.2", VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kVp8) },
    { "vp9,opus", VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kVp9) },
    { "av1,opus", VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kAv1) },
    { "av01,opus", VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kAv1) },
    { "av01.0.04M.08,opus", VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kAv1) },
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    { "h264,mp4a.40.2", VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kH264) },
    { "avc1,opus", VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kH264) },
    { "avc1.42E01E,opus", VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kH264, media::VideoCodecProfile::H264PROFILE_BASELINE, 30u) },
#endif // BUILDFLAG(USE_PROPRIETARY_CODECS)
#if BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
    { "hvc1,opus", VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kHevc) },
    { "hvc1.1.6.L93.B0,opus", VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kHevc, media::VideoCodecProfile::HEVCPROFILE_MAIN, 93u) },
#endif // BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
};

class MediaRecorderHandlerCodecProfileTest : public TestWithParam<MediaRecorderCodecProfileTestParams> { };

TEST_P(MediaRecorderHandlerCodecProfileTest, VideoStringToCodecProfile)
{
    EXPECT_EQ(VideoStringToCodecProfile(GetParam().codecs), GetParam().codec_profile);
}

INSTANTIATE_TEST_SUITE_P(All, MediaRecorderHandlerCodecProfileTest, ValuesIn(kMediaRecorderCodecProfileTestParams));
