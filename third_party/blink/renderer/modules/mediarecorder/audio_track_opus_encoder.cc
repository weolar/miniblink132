...
bool DoEncode(OpusEncoder* opus_encoder, float* data_in, int num_samples, base::span<uint8_t> data_out, size_t* actual_size)
{
    DCHECK_EQ(kOpusPreferredFramesPerBuffer, num_samples);
    CHECK_EQ(data_out.size(), kOpusMaxDataBytes);

    const opus_int32 result = opus_encode_float(opus_encoder, data_in, num_samples, data_out.data(), static_cast<int>(data_out.size()));
...
AudioTrackOpusEncoder::AudioTrackOpusEncoder(
    OnEncodedAudioCB on_encoded_audio_cb, OnEncodedAudioErrorCB on_encoded_audio_error_cb, uint32_t bits_per_second, bool vbr_enabled)
    : AudioTrackEncoder(std::move(on_encoded_audio_cb), std::move(on_encoded_audio_error_cb))
...
double AudioTrackOpusEncoder::ProvideInput(media::AudioBus* audio_bus, uint32_t frames_delayed, const media::AudioGlitchInfo& glitch_info)
{
    fifo_->Consume(audio_bus, 0, audio_bus->frames());
    return 1.0;
}

void AudioTrackOpusEncoder::OnSetFormat(const media::AudioParameters& input_params)
{
    DVLOG(1) << __func__;
    ...
    converted_params_ = media::AudioParameters(media::AudioParameters::AUDIO_PCM_LOW_LATENCY,
        media::ChannelLayoutConfig::Guess(std::min(input_params_.channels(), 2)), kOpusPreferredSamplingRate, kOpusPreferredFramesPerBuffer);
    DVLOG(1) << "|input_params_|:" << input_params_.AsHumanReadableString() << " -->|converted_params_|:" << converted_params_.AsHumanReadableString();

    converter_ = std::make_unique<media::AudioConverter>(input_params_, converted_params_, false /* disable_fifo */);
    converter_->AddInput(this);
    converter_->PrimeWithSilence();

    fifo_ = std::make_unique<media::AudioFifo>(input_params_.channels(), kMaxNumberOfFifoBuffers * input_params_.frames_per_buffer());

    buffer_.reset(new float[converted_params_.channels() * converted_params_.frames_per_buffer()]);

    // Initialize OpusEncoder.
    int opus_result;
    opus_encoder_ = opus_encoder_create(converted_params_.sample_rate(), converted_params_.channels(), OPUS_APPLICATION_AUDIO, &opus_result);
    if (opus_result < 0) {
        DLOG(ERROR) << "Couldn't init Opus encoder: " << opus_strerror(opus_result) << ", sample rate: " << converted_params_.sample_rate()
    ...
    // later versions.
    const opus_int32 bitrate = (bits_per_second_ > 0) ? base::saturated_cast<opus_int32>(bits_per_second_) : OPUS_AUTO;
    if (opus_encoder_ctl(opus_encoder_.get(), OPUS_SET_BITRATE(bitrate)) != OPUS_OK) {
    ...
    if (opus_encoder_ctl(opus_encoder_.get(), OPUS_SET_VBR(vbr_enabled)) != OPUS_OK) {
...
void AudioTrackOpusEncoder::EncodeAudio(std::unique_ptr<media::AudioBus> input_bus, base::TimeTicks capture_time)
{
    ...
    while (fifo_->frames() >= converter_->GetMaxInputFramesRequested(kOpusPreferredFramesPerBuffer)) {
        std::unique_ptr<media::AudioBus> audio_bus = media::AudioBus::Create(converted_params_.channels(), kOpusPreferredFramesPerBuffer);
        converter_->Convert(audio_bus.get());
        audio_bus->ToInterleaved<media::Float32SampleTypeTraits>(audio_bus->frames(), buffer_.get());

        if (packet_buffer_.empty()) {
            packet_buffer_ = base::HeapArray<uint8_t>::Uninit(kOpusMaxDataBytes);
        }
        size_t actual_size;
        if (DoEncode(opus_encoder_, buffer_.get(), kOpusPreferredFramesPerBuffer, packet_buffer_, &actual_size)) {
            const base::TimeTicks capture_time_of_first_sample
                = capture_time - media::AudioTimestampHelper::FramesToTime(input_bus->frames(), input_params_.sample_rate());

            auto buffer = media::DecoderBuffer::CopyFrom(packet_buffer_.first(actual_size));
            on_encoded_audio_cb_.Run(converted_params_, std::move(buffer), std::nullopt, capture_time_of_first_sample);
        } else {
