...
VpxEncoder::VpxEncoder(scoped_refptr<base::SequencedTaskRunner> encoding_task_runner, bool use_vp9,
    const VideoTrackRecorder::OnEncodedVideoCB& on_encoded_video_cb, uint32_t bits_per_second, bool is_screencast,
    const VideoTrackRecorder::OnErrorCB on_error_cb)
    : Encoder(std::move(encoding_task_runner), on_encoded_video_cb, bits_per_second)
    , use_vp9_(use_vp9)
...
void VpxEncoder::EncodeFrame(scoped_refptr<media::VideoFrame> frame, base::TimeTicks capture_timestamp, bool request_keyframe)
{
    using media::VideoFrame;
    TRACE_EVENT0("media", "VpxEncoder::EncodeFrame");

    if (frame->format() == media::PIXEL_FORMAT_NV12 && frame->storage_type() == media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER)
    ...
    if (!IsInitialized(codec_config_) || gfx::Size(codec_config_.g_w, codec_config_.g_h) != frame_size) {
    ...
    case media::PIXEL_FORMAT_NV12: {
        ...
        DoEncode(encoder_.get(), frame_size, frame->data(VideoFrame::Plane::kY), frame->visible_data(VideoFrame::Plane::kY),
            frame->stride(VideoFrame::Plane::kY), frame->visible_data(VideoFrame::Plane::kUV), frame->stride(VideoFrame::Plane::kUV),
            frame->visible_data(VideoFrame::Plane::kUV) + 1, frame->stride(VideoFrame::Plane::kUV), duration, force_keyframe, &output_data, /*is_alpha=*/false,
            VPX_IMG_FMT_NV12);
        break;
    }
    case media::PIXEL_FORMAT_I420: {
        last_frame_had_alpha_ = false;
        DoEncode(encoder_.get(), frame_size, frame->data(VideoFrame::Plane::kY), frame->visible_data(VideoFrame::Plane::kY),
            frame->stride(VideoFrame::Plane::kY), frame->visible_data(VideoFrame::Plane::kU), frame->stride(VideoFrame::Plane::kU),
            frame->visible_data(VideoFrame::Plane::kV), frame->stride(VideoFrame::Plane::kV), duration, force_keyframe, &output_data, /*is_alpha=*/false,
            VPX_IMG_FMT_I420);
        break;
    }
    case media::PIXEL_FORMAT_I420A: {
        ...
        duration = duration / 2;
        if ((!IsInitialized(alpha_codec_config_) || gfx::Size(alpha_codec_config_.g_w, alpha_codec_config_.g_h) != frame_size)) {
            if (!ConfigureEncoder(frame_size, &alpha_codec_config_, &alpha_encoder_)) {
                return;
            }
            u_plane_stride_ = media::VideoFrame::RowBytes(VideoFrame::Plane::kU, frame->format(), frame_size.width());
            v_plane_stride_ = media::VideoFrame::RowBytes(VideoFrame::Plane::kV, frame->format(), frame_size.width());
            v_plane_offset_ = media::VideoFrame::PlaneSize(frame->format(), VideoFrame::Plane::kU, frame_size).GetArea();
            alpha_dummy_planes_.resize(
                base::checked_cast<wtf_size_t>(v_plane_offset_ + media::VideoFrame::PlaneSize(frame->format(), VideoFrame::Plane::kV, frame_size).GetArea()));
        ...
        DoEncode(alpha_encoder_.get(), frame_size, frame->data(VideoFrame::Plane::kA), frame->visible_data(VideoFrame::Plane::kA),
            frame->stride(VideoFrame::Plane::kA), alpha_dummy_planes_.data(), base::checked_cast<int>(u_plane_stride_),
            alpha_dummy_planes_.data() + v_plane_offset_, base::checked_cast<int>(v_plane_stride_), duration, alpha_force_keyframe, &output_data,
            /*is_alpha=*/true, VPX_IMG_FMT_I420);
        break;
    }
    default:
    ...
    on_encoded_video_cb_.Run(video_params, std::move(output_data), std::nullopt, capture_timestamp);
}

void VpxEncoder::DoEncode(vpx_codec_ctx_t* const encoder, const gfx::Size& frame_size, const uint8_t* data, const uint8_t* y_plane, int y_stride,
    const uint8_t* u_plane, int u_stride, const uint8_t* v_plane, int v_stride, const base::TimeDelta& duration, bool force_keyframe,
    scoped_refptr<media::DecoderBuffer>* output_data, bool is_alpha, vpx_img_fmt_t img_fmt)
{
    CHECK(output_data);
    DCHECK(img_fmt == VPX_IMG_FMT_I420 || img_fmt == VPX_IMG_FMT_NV12);

    vpx_image_t vpx_image;
    vpx_image_t* const result = vpx_img_wrap(&vpx_image, img_fmt, frame_size.width(), frame_size.height(), 1 /* align */, const_cast<uint8_t*>(data));
    ...
    const vpx_codec_err_t ret
        = vpx_codec_encode(encoder, &vpx_image, 0 /* pts */, static_cast<unsigned long>(duration.InMicroseconds()), flags, VPX_DL_REALTIME);
    if (ret != VPX_CODEC_OK) {
        metrics_provider_->SetError({ media::EncoderStatus::Codes::kEncoderFailedEncode,
            base::StrCat({ "libvpx failed to encode: ", vpx_codec_err_to_string(ret), " - ", vpx_codec_error_detail(encoder) }) });
        on_error_cb_.Run();
        return;
    }
    ...
    while ((pkt = vpx_codec_get_cx_data(encoder, &iter))) {
        if (pkt->kind != VPX_CODEC_CX_FRAME_PKT)
            continue;
        if (is_alpha) {
            const auto* alpha_data = reinterpret_cast<const uint8_t*>(pkt->data.frame.buf);
            // If is_alpha is true, it needs *output_data to already be a valid
            // scoped_refptr.
            CHECK(*output_data);
            (*output_data)->WritableSideData().alpha_data = base::HeapArray<uint8_t>::CopiedFrom(base::span<const uint8_t>(alpha_data, pkt->data.frame.sz));
        } else {
            *output_data = media::DecoderBuffer::CopyFrom({ reinterpret_cast<const uint8_t*>(pkt->data.frame.buf), pkt->data.frame.sz });
        }
        (*output_data)->set_is_key_frame((pkt->data.frame.flags & VPX_FRAME_IS_KEY) != 0);
        break;
    }
}

bool VpxEncoder::ConfigureEncoder(const gfx::Size& size, vpx_codec_enc_cfg_t* codec_config, ScopedVpxCodecCtxPtr* encoder)
{
    if (IsInitialized(*codec_config)) {
        ...
        DVLOG(1) << "Destroying/Re-Creating encoder for new frame size: " << gfx::Size(codec_config->g_w, codec_config->g_h).ToString() << " --> "
                 << size.ToString() << (use_vp9_ ? " vp9" : " vp8");
        encoder->reset();
    }

    const vpx_codec_iface_t* codec_interface = use_vp9_ ? vpx_codec_vp9_cx() : vpx_codec_vp8_cx();
    vpx_codec_err_t result = vpx_codec_enc_config_default(codec_interface, codec_config, 0 /* reserved */);
    ...
    if (bits_per_second_ > 0) {
        ...
        codec_config->rc_target_bitrate = size.GetArea() * codec_config->rc_target_bitrate / codec_config->g_w / codec_config->g_h;
    }
    // Don't drop a frame.
    ...
    metrics_provider_->Initialize(use_vp9_ ? media::VP9PROFILE_MIN : media::VP8PROFILE_ANY, size,
    ...
    const vpx_codec_err_t ret = vpx_codec_enc_init(tmp_encoder.get(), codec_interface, codec_config, 0 /* flags */);
    if (ret != VPX_CODEC_OK) {
        metrics_provider_->SetError(
            { media::EncoderStatus::Codes::kEncoderInitializationError, base::StrCat({ "libvpx failed to initialize: ", vpx_codec_err_to_string(ret) }) });
    ...
    vpx_codec_control(encoder->get(), VP8E_SET_STATIC_THRESHOLD, (is_screencast_ && !use_vp9_) ? 100 : 1);
    if (is_screencast_) {
        if (use_vp9_) {
            vpx_codec_control(encoder->get(), VP9E_SET_TUNE_CONTENT, VP9E_CONTENT_SCREEN);
        } else {
...
base::TimeDelta VpxEncoder::EstimateFrameDuration(const media::VideoFrame& frame)
{
    ...
    base::TimeDelta predicted_frame_duration = frame.timestamp() - last_frame_timestamp_;
    base::TimeDelta frame_duration = frame.metadata().frame_duration.value_or(predicted_frame_duration);
    ...
    return std::min(kMaxFrameDuration, std::max(frame_duration, kMinFrameDuration));
}
