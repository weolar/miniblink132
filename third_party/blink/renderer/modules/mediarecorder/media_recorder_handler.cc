...
BASE_FEATURE(kMediaRecorderEnableMp4Muxer, "MediaRecorderEnableMp4Muxer", base::FEATURE_ENABLED_BY_DEFAULT);
namespace {
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS) || BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
std::optional<VideoTrackRecorder::CodecProfile> VideoStringTagToCodecProfile(const String& codecs, const StringView& codecs_tag)
{
    ...
    if (codecs_start != kNotFound) {
        ...
        auto codec_id = codecs.Substring(codecs_start, codecs_end == kNotFound ? UINT_MAX : codecs_end).StripWhiteSpace().Ascii();
        // Do not use lowercase `codecId` here, as `codecId` is case sensitive when
        // parsing.
        if (auto result = media::ParseCodec(codec_id)) {
            codec_profile = { CodecIdFromMediaVideoCodec(result->codec), result->profile, result->level };
        }
    }
...
{
    bool support = EqualIgnoringASCIICase(type, "video/webm") || EqualIgnoringASCIICase(type, "video/x-matroska");
    if (support) {
        return true;
    }
...
{
    return EqualIgnoringASCIICase(type, "video/mp4") || EqualIgnoringASCIICase(type, "audio/mp4");
}
...
VideoTrackRecorder::CodecProfile VideoStringToCodecProfile(const String& codecs)
{
    String codecs_str = codecs.LowerASCII();
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    if (codecs_str.Find("h264") != kNotFound || codecs_str.Find("avc1") != kNotFound) {
    ...
    if (codecs_str.Find("av01") != kNotFound || codecs_str.Find("av1") != kNotFound) {
...
MediaRecorderHandler::MediaRecorderHandler(
    scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner, KeyFrameRequestProcessor::Configuration key_frame_config)
    : key_frame_config_(key_frame_config)
    , main_thread_task_runner_(std::move(main_thread_task_runner))
{
}

bool MediaRecorderHandler::CanSupportMimeType(const String& type, const String& web_codecs)
{
    ...
    auto* const* relevant_codecs_begin = video ? std::begin(kVideoCodecs) : std::begin(kAudioCodecs);
    auto* const* relevant_codecs_end = video ? std::end(kVideoCodecs) : std::end(kAudioCodecs);
    ...
    if (mp4_mime_type) {
        ...
        relevant_codecs_begin = video ? std::begin(kVideoCodecsForMP4) : std::begin(kAudioCodecsForMp4);
        relevant_codecs_end = video ? std::end(kVideoCodecsForMP4) : std::end(kAudioCodecsForMp4);
    }
    ...
    for (const auto& codec : codecs_list) {
        ...
        bool match = std::any_of(relevant_codecs_begin, relevant_codecs_end, [&codec_string, &mp4_mime_type](const char* name) {
            if (mp4_mime_type) {
        ...
        if (!match && mp4_mime_type && video) {
            ...
            auto parsed_result = media::ParseVideoCodecString(type.Ascii(), codec_string.Ascii(),
                /*allow_ambiguous_matches=*/false);
            match = parsed_result && (parsed_result->codec == media::VideoCodec::kH264 || parsed_result->codec == media::VideoCodec::kAV1);
            ...
            if (parsed_result && parsed_result->profile == media::VideoCodecProfile::HEVCPROFILE_MAIN
        ...
        if (codec_string == "mp4a.40.2" && !media::MojoAudioEncoder::IsSupported(media::AudioCodec::kAAC)) {
            return false;
        }
        ...
        if (codec_string.StartsWith("hvc1", kTextCaseASCIIInsensitive)
            && (!base::FeatureList::IsEnabled(media::kMediaRecorderHEVCSupport) ||
                ...
                !media::IsEncoderSupportedVideoType({ media::VideoCodec::kHEVC, media::VideoCodecProfile::HEVCPROFILE_MAIN }))) {
            return false;
        }
#endif

        if (codec_string == "av01" || codec_string == "av1") {
            base::UmaHistogramBoolean("Media.MediaRecorder.HasCorrectAV1CodecString", codec_string == "av01");
            ...
            // doable.
            VideoTrackRecorder::CodecProfile codec_profile = VideoStringToCodecProfile(codec_string);
            if (!VideoTrackRecorderImpl::CanUseAcceleratedEncoder(
                    ...
                    // minimum size.
                    codec_profile, video_track_recorder::kVEAEncoderMinResolutionWidth, video_track_recorder::kVEAEncoderMinResolutionHeight)) {
                return false;
            }
#endif
        }
...
bool MediaRecorderHandler::Initialize(
    MediaRecorder* recorder, MediaStreamDescriptor* media_stream, const String& type, const String& codecs, AudioTrackRecorder::BitrateMode audio_bitrate_mode)
{
    DCHECK(main_thread_task_runner_->RunsTasksInCurrentSequence());
    ...
    if (video_codec_profile_.codec_id == VideoTrackRecorder::CodecId::kLast) {
        MediaTrackContainerType container_type = GetMediaContainerTypeFromString(type_);
        video_codec_profile_.codec_id = VideoTrackRecorderImpl::GetPreferredCodecId(container_type);
        DVLOG(1) << "Falling back to preferred video codec id " << static_cast<int>(video_codec_profile_.codec_id);
    }

    // Do the same for the audio codec(s).
    const AudioTrackRecorder::CodecId audio_codec_id = AudioStringToCodecId(codecs);
    ...
    DVLOG_IF(1, audio_codec_id == AudioTrackRecorder::CodecId::kLast) << "Falling back to preferred audio codec id " << static_cast<int>(audio_codec_id_);
...
bool MediaRecorderHandler::Start(int timeslice, const String& type, uint32_t audio_bits_per_second, uint32_t video_bits_per_second)
{
    DCHECK(main_thread_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(!recording_);
    ...
    const bool use_video_tracks = !video_tracks_.empty() && video_tracks_[0]->GetReadyState() != MediaStreamSource::kReadyStateEnded;
    const bool use_audio_tracks
        = !audio_tracks_.empty() && audio_tracks_[0]->GetPlatformTrack() && audio_tracks_[0]->GetReadyState() != MediaStreamSource::kReadyStateEnded;
    ...
    if (!type.empty()) {
        ...
        if (use_mp4_muxer && !base::FeatureList::IsEnabled(kMediaRecorderEnableMp4Muxer)) {
            return false;
        }
    }
    ...
    auto write_callback = WTF::BindRepeating(&MediaRecorderHandler::WriteData, WrapPersistent(weak_factory_.GetWeakCell()));
    if (use_mp4_muxer) {
        muxer = std::make_unique<media::Mp4Muxer>(audio_codec, use_video_tracks, use_audio_tracks,
            std::make_unique<media::Mp4MuxerDelegate>(audio_codec, MediaVideoCodecFromCodecId(video_codec_profile_.codec_id), video_codec_profile_.profile,
        ...
        if (audio_bits_per_second_ != 0u) {
            audio_bits_per_second_ = media::MFAudioEncoder::ClampAccCodecBitrate(audio_bits_per_second_);
            recorder_->UpdateAudioBitrate(audio_bits_per_second_);
        }
#endif
    } else {
        muxer = std::make_unique<media::WebmMuxer>(
            audio_codec, use_video_tracks, use_audio_tracks, std::make_unique<media::LiveWebmMuxerDelegate>(write_callback), optional_timeslice);
    }
    muxer_adapter_ = std::make_unique<media::MuxerTimestampAdapter>(std::move(muxer), use_video_tracks, use_audio_tracks);

    if (use_video_tracks) {
        // TODO(mcasas): The muxer API supports only one video track. Extend it to
        // several video tracks, see http://crbug.com/528523.
        LOG_IF(WARNING, video_tracks_.size() > 1u) << "Recording multiple video tracks is not implemented. "
                                                   << "Only recording first video track.";
        if (!video_tracks_[0])
            return false;
        UpdateTrackLiveAndEnabled(*video_tracks_[0], /*is_video=*/true);

        MediaStreamVideoTrack* const video_track = static_cast<MediaStreamVideoTrack*>(video_tracks_[0]->GetPlatformTrack());
        const bool use_encoded_source_output = video_track->source() != nullptr && video_track->source()->SupportsEncodedOutput();
        if (passthrough_enabled_ && use_encoded_source_output) {
            video_recorders_.emplace_back(std::make_unique<VideoTrackRecorderPassthrough>(
                main_thread_task_runner_, video_tracks_[0], weak_video_factory_.GetWeakCell(), key_frame_config_));
        } else {
            video_recorders_.emplace_back(std::make_unique<VideoTrackRecorderImpl>(main_thread_task_runner_, video_codec_profile_, video_tracks_[0],
                weak_video_factory_.GetWeakCell(), video_bits_per_second_, key_frame_config_));
        }
    }
    ...
    if (use_audio_tracks) {
        ...
        // several tracks.
        LOG_IF(WARNING, audio_tracks_.size() > 1u) << "Recording multiple audio"
        ...
        audio_recorders_.emplace_back(std::make_unique<AudioTrackRecorder>(
            main_thread_task_runner_, audio_codec_id_, audio_tracks_[0], weak_audio_factory_.GetWeakCell(), audio_bits_per_second_, audio_bitrate_mode_));
    }
...
void MediaRecorderHandler::EncodingInfo(const WebMediaConfiguration& configuration, OnMediaCapabilitiesEncodingInfoCallback callback)
{
    DCHECK(main_thread_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(configuration.video_configuration || configuration.audio_configuration);

    std::unique_ptr<WebMediaCapabilitiesInfo> info(new WebMediaCapabilitiesInfo());
    ...
    if (configuration.video_configuration && info->supported) {
        VideoTrackRecorder::CodecProfile codec_profile = VideoStringToCodecProfile(codec);
        const bool is_likely_accelerated = VideoTrackRecorderImpl::CanUseAcceleratedEncoder(
            codec_profile, configuration.video_configuration->width, configuration.video_configuration->height, configuration.video_configuration->framerate);

        const float pixels_per_second
            = configuration.video_configuration->width * configuration.video_configuration->height * configuration.video_configuration->framerate;
        // Encoding is considered |smooth| up and until the pixels per second
        // threshold or if it's likely to be accelerated.
        const float threshold = base::SysInfo::IsLowEndDevice() ? kNumPixelsPerSecondSmoothnessThresholdLow : kNumPixelsPerSecondSmoothnessThresholdHigh;
    ...
    DVLOG(1) << "type: " << mime_type.Ascii() << ", params:" << codec.Ascii() << " is" << (info->supported ? " supported" : " NOT supported") << " and"
...
String MediaRecorderHandler::ActualMimeType()
{
    ...
    if (!has_video_tracks && has_audio_tracks) {
        ...
        case VideoTrackRecorder::CodecId::kHevc:
#endif
            if (!passthrough_enabled_ && EqualIgnoringASCIICase(type_, "video/mp4")) {
                mime_type.Append(type_.Characters8(), type_.length());
            } else {
    ...
    if (has_video_tracks) {
        ...
        case VideoTrackRecorder::CodecId::kH264:
            ...
            if (video_codec_profile_.profile && video_codec_profile_.level) {
                mime_type.Append(media::BuildH264MimeSuffix(*video_codec_profile_.profile, *video_codec_profile_.level).c_str());
            }
            break;
#endif
        case VideoTrackRecorder::CodecId::kAv1:
    ...
    if (has_video_tracks && has_audio_tracks) {
        if (video_codec_profile_.codec_id != VideoTrackRecorder::CodecId::kLast && audio_codec_id_ != AudioTrackRecorder::CodecId::kLast) {
            mime_type.Append(",");
        }
    }
    if (has_audio_tracks) {
        ...
        case AudioTrackRecorder::CodecId::kAac:
            ...
            break;
        case AudioTrackRecorder::CodecId::kLast:
            DCHECK_NE(video_codec_profile_.codec_id, VideoTrackRecorder::CodecId::kLast);
        }
    }
...
void MediaRecorderHandler::OnStreamChanged(const String& message)
{
    if (recorder_) {
        ...
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, WTF::BindOnce(&MediaRecorder::OnStreamChanged, WrapWeakPersistent(recorder_.Get()), message));
    }
}

void MediaRecorderHandler::OnEncodedVideo(const media::Muxer::VideoParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
    std::optional<media::VideoEncoder::CodecDescription> codec_description, base::TimeTicks timestamp)
{
    ...
    media::VideoCodec video_codec = MediaVideoCodecFromCodecId(video_codec_profile_.codec_id);
    // Convert annex stream to avc/hevc bit stream for h264/h265.
    if ((video_codec == media::VideoCodec::kH264 || video_codec == media::VideoCodec::kHEVC) && encoded_data->is_key_frame()
        && !codec_description.has_value()) {
        bool first_key_frame = false;
        if (!h26x_converter_) {
            h26x_converter_ = std::make_unique<media::H26xAnnexBToBitstreamConverter>(video_codec);
        ...
        if (first_key_frame) {
            video_codec_profile_.level = h26x_converter_->GetCodecProfileLevel().level;
        }
    }
#endif

    auto params_with_codec = params;
    params_with_codec.codec = MediaVideoCodecFromCodecId(video_codec_profile_.codec_id);
    if (!params_with_codec.frame_rate) {
        params_with_codec.frame_rate = kDefaultVideoFrameRate;
    }

    HandleEncodedVideo(params_with_codec, std::move(encoded_data), std::move(codec_description), timestamp);
}

void MediaRecorderHandler::OnPassthroughVideo(
    const media::Muxer::VideoParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data, base::TimeTicks timestamp)
{
    DCHECK(main_thread_task_runner_->RunsTasksInCurrentSequence());
...
void MediaRecorderHandler::HandleEncodedVideo(const media::Muxer::VideoParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
    std::optional<media::VideoEncoder::CodecDescription> codec_description, base::TimeTicks timestamp)
{
    DCHECK(main_thread_task_runner_->RunsTasksInCurrentSequence());
    ...
    if (*last_seen_codec_ != params.codec) {
        recorder_->OnError(DOMExceptionCode::kUnknownError,
            String::Format("Video codec changed from %s to %s", media::GetCodecName(*last_seen_codec_).c_str(), media::GetCodecName(params.codec).c_str()));
        return;
    }
    if (!muxer_adapter_) {
        return;
    }
    if (!muxer_adapter_->OnEncodedVideo(params, std::move(encoded_data), std::move(codec_description), timestamp)) {
        recorder_->OnError(DOMExceptionCode::kUnknownError, "Error muxing video data");
    }
}

void MediaRecorderHandler::OnEncodedAudio(const media::AudioParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
    std::optional<media::AudioEncoder::CodecDescription> codec_description, base::TimeTicks timestamp)
{
    ...
    if (!muxer_adapter_->OnEncodedAudio(params, std::move(encoded_data), std::move(codec_description), timestamp)) {
        recorder_->OnError(DOMExceptionCode::kUnknownError, "Error muxing audio data");
    }
}

void MediaRecorderHandler::OnAudioEncodingError(media::EncoderStatus error_status)
{
    DCHECK(main_thread_task_runner_->RunsTasksInCurrentSequence());
    recorder_->OnError(DOMExceptionCode::kEncodingError, String(media::EncoderStatusCodeToString(error_status)));
}

std::unique_ptr<media::VideoEncoderMetricsProvider> MediaRecorderHandler::CreateVideoEncoderMetricsProvider()
{
    DCHECK(main_thread_task_runner_->RunsTasksInCurrentSequence());
    mojo::PendingRemote<media::mojom::VideoEncoderMetricsProvider> video_encoder_metrics_provider;
    recorder_->DomWindow()->GetFrame()->GetBrowserInterfaceBroker().GetInterface(video_encoder_metrics_provider.InitWithNewPipeAndPassReceiver());
    return base::MakeRefCounted<media::MojoVideoEncoderMetricsProviderFactory>(
        media::mojom::VideoEncoderUseCase::kMediaRecorder, std::move(video_encoder_metrics_provider))
...
void MediaRecorderHandler::WriteData(base::span<const uint8_t> data)
{
    ...
    const bool last_in_slice = timeslice_.is_zero() ? true : now > slice_origin_timestamp_ + timeslice_;
...
void MediaRecorderHandler::UpdateTrackLiveAndEnabled(const MediaStreamComponent& track, bool is_video)
{
    const bool track_live_and_enabled = track.GetReadyState() == MediaStreamSource::kReadyStateLive && track.Enabled();
    if (muxer_adapter_) {
...
void MediaRecorderHandler::OnVideoFrameForTesting(scoped_refptr<media::VideoFrame> frame, const TimeTicks& timestamp)
{
...
void MediaRecorderHandler::OnEncodedVideoFrameForTesting(scoped_refptr<EncodedVideoFrame> frame, const base::TimeTicks& timestamp)
{
    for (const auto& recorder : video_recorders_) {
        recorder->OnEncodedVideoFrameForTesting(base::TimeTicks::Now(), frame, timestamp);
    }
}

void MediaRecorderHandler::OnAudioBusForTesting(const media::AudioBus& audio_bus, const base::TimeTicks& timestamp)
{
    for (const auto& recorder : audio_recorders_)
        recorder->OnData(audio_bus, timestamp);
}

void MediaRecorderHandler::SetAudioFormatForTesting(const media::AudioParameters& params)
{
    for (const auto& recorder : audio_recorders_)
...
{
    if (recorder_) {
        recorder_->OnError(DOMExceptionCode::kUnknownError, "Video encoding failed.");
    }
}
