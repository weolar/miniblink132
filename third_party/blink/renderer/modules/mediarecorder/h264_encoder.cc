...
{
    static constexpr auto kProfileToEProfileIdc = base::MakeFixedFlatMap<media::VideoCodecProfile, EProfileIdc>({
...
{
    static constexpr auto kLevelToELevelIdc = base::MakeFixedFlatMap<uint8_t, ELevelIdc>({
        { 10, LEVEL_1_0 },
        { 9, LEVEL_1_B },
        { 11, LEVEL_1_1 },
        { 12, LEVEL_1_2 },
...
H264Encoder::H264Encoder(scoped_refptr<base::SequencedTaskRunner> encoding_task_runner, const VideoTrackRecorder::OnEncodedVideoCB& on_encoded_video_cb,
    VideoTrackRecorder::CodecProfile codec_profile, uint32_t bits_per_second, bool is_screencast, const VideoTrackRecorder::OnErrorCB on_error_cb)
...
void H264Encoder::EncodeFrame(scoped_refptr<media::VideoFrame> frame, base::TimeTicks capture_timestamp, bool request_keyframe)
{
    TRACE_EVENT0("media", "H264Encoder::EncodeFrame");
    using media::VideoFrame;
    DCHECK(frame->format() == media::VideoPixelFormat::PIXEL_FORMAT_NV12 || frame->format() == media::VideoPixelFormat::PIXEL_FORMAT_I420
    ...
    picture.pData[0] = const_cast<uint8_t*>(frame->visible_data(VideoFrame::Plane::kY));
    picture.pData[1] = const_cast<uint8_t*>(frame->visible_data(VideoFrame::Plane::kU));
    picture.pData[2] = const_cast<uint8_t*>(frame->visible_data(VideoFrame::Plane::kV));
    ...
    if (int ret = openh264_encoder_->EncodeFrame(&picture, &info); ret != cmResultSuccess) {
        metrics_provider_->SetError(
            { media::EncoderStatus::Codes::kEncoderFailedEncode, base::StrCat({ "OpenH264 failed to encode: ", base::NumberToString(ret) }) });
        on_error_cb_.Run();
        return;
    }
    ...
    on_encoded_video_cb_.Run(video_params, std::move(buffer), std::nullopt, capture_timestamp);
}
...
bool H264Encoder::ConfigureEncoder(const gfx::Size& size)
{
    ...
    init_params.iUsageType = is_screencast_ ? SCREEN_CONTENT_REAL_TIME : CAMERA_VIDEO_REAL_TIME;
    ...
    init_params.sSpatialLayers[0].uiProfileIdc = codec_profile_.profile ? ToOpenH264Profile(*codec_profile_.profile).value_or(PRO_UNKNOWN) : PRO_UNKNOWN;
    // Input level may be optional, fills LEVEL_UNKNOWN for auto-detection.
    init_params.sSpatialLayers[0].uiLevelIdc = codec_profile_.level ? ToOpenH264Level(*codec_profile_.level).value_or(LEVEL_UNKNOWN) : LEVEL_UNKNOWN;
    ...
    metrics_provider_->Initialize(codec_profile_.profile.value_or(media::H264PROFILE_BASELINE), configured_size_, /*is_hardware_encoder=*/false);
    if (int ret = openh264_encoder_->InitializeExt(&init_params); ret != cmResultSuccess) {
        metrics_provider_->SetError(
            { media::EncoderStatus::Codes::kEncoderInitializationError, base::StrCat({ "OpenH264 failed to initialize: ", base::NumberToString(ret) }) });
...
{
    DCHECK(openh264_encoder_) << "Call GetOption on uninitialized OpenH264 encoder";

    SEncParamExt params;
    if (openh264_encoder_->GetOption(ENCODER_OPTION_SVC_ENCODE_PARAM_EXT, &params) != 0) {
