...
class VEAEncoder final : public VideoTrackRecorder::Encoder, public media::VideoEncodeAccelerator::Client {
public:
    VEAEncoder(scoped_refptr<base::SequencedTaskRunner> encoding_task_runner, const VideoTrackRecorder::OnEncodedVideoCB& on_encoded_video_cb,
...
private:
    struct VideoFrameAndMetadata {
        VideoFrameAndMetadata(scoped_refptr<media::VideoFrame> frame, base::TimeTicks timestamp, bool request_keyframe)
            : frame(frame)
            , timestamp(timestamp)
    ...
    using VideoParamsAndTimestamp = std::pair<media::Muxer::VideoParameters, base::TimeTicks>;
    friend class base::SequenceBound<blink::VEAEncoder, WTF::internal::SequenceBoundBindTraits>;
