...
const TestFrameType kTestFrameTypes[] = { TestFrameType::kNv12GpuMemoryBuffer, TestFrameType::kNv12Software, TestFrameType::kI420 };
...
const gfx::Size kTrackRecorderTestSize[] = { gfx::Size(kVEAEncoderMinResolutionWidth / 2, kVEAEncoderMinResolutionHeight / 2),
    gfx::Size(kVEAEncoderMinResolutionWidth, kVEAEncoderMinResolutionHeight) };
static const int kTrackRecorderTestSizeDiff = 20;

constexpr media::VideoCodec MediaVideoCodecFromCodecId(VideoTrackRecorder::CodecId id)
{
    switch (id) {
...
media::VideoCodecProfile MediaVideoCodecProfileFromCodecId(VideoTrackRecorder::CodecId id)
{
    switch (id) {
    case VideoTrackRecorder::CodecId::kVp8:
...
public:
    ...
    MOCK_METHOD(media::GpuVideoAcceleratorFactories*, GetGpuFactories, (), (override));
};

class MockVideoTrackRecorderCallbackInterface : public GarbageCollected<MockVideoTrackRecorderCallbackInterface>, public VideoTrackRecorder::CallbackInterface {
public:
    virtual ~MockVideoTrackRecorderCallbackInterface() = default;
    MOCK_METHOD(void, OnPassthroughVideo,
        (const media::Muxer::VideoParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data, base::TimeTicks timestamp), (override));
    MOCK_METHOD(void, OnEncodedVideo,
        (const media::Muxer::VideoParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
            std::optional<media::VideoEncoder::CodecDescription> codec_description, base::TimeTicks timestamp),
        (override));
    MOCK_METHOD(std::unique_ptr<media::VideoEncoderMetricsProvider>, CreateVideoEncoderMetricsProvider, (), (override));

    MOCK_METHOD(void, OnVideoEncodingError, (), (override));
    MOCK_METHOD(void, OnSourceReadyStateChanged, (), (override));
    void Trace(Visitor* v) const override
    {
        v->Trace(weak_factory_);
    }
...
public:
    VideoTrackRecorderTestBase()
        : mock_callback_interface_(MakeGarbageCollected<MockVideoTrackRecorderCallbackInterface>())
    {
        ON_CALL(*mock_callback_interface_, CreateVideoEncoderMetricsProvider())
            .WillByDefault(::testing::Invoke(this, &VideoTrackRecorderTestBase::CreateMockVideoEncoderMetricsProvider));
    }
...
protected:
    ...
    std::unique_ptr<media::VideoEncoderMetricsProvider> CreateMockVideoEncoderMetricsProvider()
    {
...
public:
        ...
        : mock_source_(new MockMediaStreamVideoSource())
    {
        ...
        source_ = MakeGarbageCollected<MediaStreamSource>(
            track_id, MediaStreamSource::kTypeVideo, track_id, false /*remote*/, base::WrapUnique(mock_source_.get()));
        EXPECT_CALL(*mock_source_, OnRequestRefreshFrame()).Times(testing::AnyNumber());
        EXPECT_CALL(*mock_source_, OnCapturingLinkSecured(_)).Times(testing::AnyNumber());
        EXPECT_CALL(*mock_source_, GetSubCaptureTargetVersion()).Times(testing::AnyNumber()).WillRepeatedly(Return(0));
        EXPECT_CALL(*mock_source_, OnSourceCanDiscardAlpha(_)).Times(testing::AnyNumber());

        auto platform_track
            = std::make_unique<MediaStreamVideoTrack>(mock_source_, WebPlatformMediaStreamSource::ConstraintsOnceCallback(), true /* enabled */);
        track_ = platform_track.get();
        component_ = MakeGarbageCollected<MediaStreamComponentImpl>(source_, std::move(platform_track));

        // Paranoia checks.
        EXPECT_EQ(component_->Source()->GetPlatformSource(), source_->GetPlatformSource());
        EXPECT_TRUE(scheduler::GetSingleThreadTaskRunnerForTesting()->BelongsToCurrentThread());

        EXPECT_CALL(*platform_, GetGpuFactories()).Times(testing::AnyNumber()).WillRepeatedly(Return(nullptr));
    }
    ...
    void InitializeRecorder(
        VideoTrackRecorder::CodecId codec_id, KeyFrameRequestProcessor::Configuration keyframe_config = KeyFrameRequestProcessor::Configuration())
    {
        InitializeRecorder(VideoTrackRecorder::CodecProfile(codec_id), keyframe_config);
    }

    void InitializeRecorder(
        VideoTrackRecorder::CodecProfile codec_profile, KeyFrameRequestProcessor::Configuration keyframe_config = KeyFrameRequestProcessor::Configuration())
    {
        video_track_recorder_ = std::make_unique<VideoTrackRecorderImpl>(scheduler::GetSingleThreadTaskRunnerForTesting(), codec_profile,
            WebMediaStreamTrack(component_.Get()), mock_callback_interface_->GetWeakCell(),
            /*bits_per_second=*/1000000, keyframe_config,
            /*frame_buffer_pool_limit=*/10);
    }

    void Encode(scoped_refptr<media::VideoFrame> frame, base::TimeTicks capture_time, bool allow_vea_encoder = true)
    {
        EXPECT_TRUE(scheduler::GetSingleThreadTaskRunnerForTesting()->BelongsToCurrentThread());
        video_track_recorder_->OnVideoFrameForTesting(std::move(frame), capture_time, allow_vea_encoder);
    }
    ...
    bool CanEncodeAlphaChannel()
    {
        ...
        video_track_recorder_->encoder_.PostTaskWithThisObject(CrossThreadBindOnce(
            [](base::WaitableEvent* finished, bool* out_result, VideoTrackRecorder::Encoder* encoder) {
...
protected:
    scoped_refptr<media::VideoFrame> CreateFrameForTest(TestFrameType frame_type, const gfx::Size& frame_size, bool encode_alpha_channel, int padding)
    {
        const gfx::Size padded_size(frame_size.width() + padding, frame_size.height());
        if (frame_type == TestFrameType::kI420) {
            return media::VideoFrame::CreateZeroInitializedFrame(
                encode_alpha_channel ? media::PIXEL_FORMAT_I420A : media::PIXEL_FORMAT_I420, padded_size, gfx::Rect(frame_size), frame_size, base::TimeDelta());
        }

        scoped_refptr<media::VideoFrame> video_frame = blink::CreateTestFrame(padded_size, gfx::Rect(frame_size), frame_size,
            frame_type == TestFrameType::kNv12Software ? media::VideoFrame::STORAGE_OWNED_MEMORY : media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER,
        ...
        memset(video_frame2->writable_data(0), kBlackY, video_frame2->stride(0) * frame_size.height());
        memset(video_frame2->writable_data(1), kBlackUV, video_frame2->stride(1) * (frame_size.height() / 2));
        if (frame_type == TestFrameType::kNv12GpuMemoryBuffer) {
            return video_frame;
        }
        return video_frame2;
...
class VideoTrackRecorderTestWithAllCodecs : public ::testing::Test, public VideoTrackRecorderTest {
public:
...
TEST_F(VideoTrackRecorderTestWithAllCodecs, NoCrashInConfigureEncoder)
{
    ...
    for (auto [codec_id, can_sw_encode] : kCodecIds) {
        ...
        const scoped_refptr<media::VideoFrame> video_frame
            = CreateFrameForTest(TestFrameType::kI420, gfx::Size(kVEAEncoderMinResolutionWidth, kVEAEncoderMinResolutionHeight),
        ...
        InSequence s;
        if (can_sw_encode) {
            EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo).WillOnce(RunClosure(run_loop.QuitClosure()));
        } else {
            EXPECT_CALL(*mock_callback_interface_, OnVideoEncodingError).WillOnce(RunClosure(run_loop.QuitClosure()));
        }
...
class VideoTrackRecorderTestWithCodec : public TestWithParam<testing::tuple<VideoTrackRecorder::CodecId, bool>>,
                                        public VideoTrackRecorderTest,
                                        public ScopedMediaRecorderUseMediaVideoEncoderForTest {
public:
    VideoTrackRecorderTestWithCodec()
        : ScopedMediaRecorderUseMediaVideoEncoderForTest(testing::get<1>(GetParam()))
    {
    }
...
TEST_P(VideoTrackRecorderTestWithCodec, SoftwareEncoderInitializeErrorWithLargeFrame)
{
    ...
    auto too_large_frame = media::VideoFrame::CreateBlackFrame(kTooLargeResolution);
...
INSTANTIATE_TEST_SUITE_P(All, VideoTrackRecorderTestWithCodec, ::testing::Combine(ValuesIn(kTrackRecorderTestCodec), ::testing::Bool()));
...
// being tested.
class VideoTrackRecorderTestParam : public TestWithParam<testing::tuple<VideoTrackRecorder::CodecId, gfx::Size, bool, TestFrameType, bool>>,
                                    public VideoTrackRecorderTest,
                                    public ScopedMediaRecorderUseMediaVideoEncoderForTest {
public:
    VideoTrackRecorderTestParam()
        : ScopedMediaRecorderUseMediaVideoEncoderForTest(testing::get<4>(GetParam()))
    {
    }
...
TEST_P(VideoTrackRecorderTestParam, VideoEncoding)
{
    ...
    const scoped_refptr<media::VideoFrame> video_frame = CreateFrameForTest(test_frame_type, frame_size, encode_alpha_channel, /*padding=*/0);
    if (!video_frame) {
        ASSERT_TRUE(!!video_frame);
    }
    ...
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, timeticks_now)).Times(1).WillOnce(SaveArg<1>(&first_frame_encoded_data));

    const base::TimeTicks timeticks_later = base::TimeTicks::Now();
    scoped_refptr<media::DecoderBuffer> second_frame_encoded_data;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(false), _, timeticks_later)).Times(1).WillOnce(SaveArg<1>(&second_frame_encoded_data));

    const gfx::Size frame_size2(frame_size.width() + kTrackRecorderTestSizeDiff, frame_size.height());
    const scoped_refptr<media::VideoFrame> video_frame2 = CreateFrameForTest(test_frame_type, frame_size2, encode_alpha_channel, /*padding=*/0);

    base::RunLoop run_loop;

    scoped_refptr<media::DecoderBuffer> third_frame_encoded_data;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _))
        .Times(1)
        .WillOnce(DoAll(SaveArg<1>(&third_frame_encoded_data), RunClosure(run_loop.QuitClosure())));
    ...
    if (test_frame_type == TestFrameType::kI420 && encode_alpha_channel && CanEncodeAlphaChannel()) {
        EXPECT_GE(first_frame_encoded_data->side_data()->alpha_data.size(), kEncodedSizeThreshold);
        EXPECT_GE(second_frame_encoded_data->side_data()->alpha_data.size(), kEncodedSizeThreshold);
        EXPECT_GE(third_frame_encoded_data->side_data()->alpha_data.size(), kEncodedSizeThreshold);
    } else {
...
TEST_P(VideoTrackRecorderTestParam, ConfigureEncoderWithScreenContent)
{
    ...
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo).WillOnce(RunClosure(run_loop1.QuitClosure()));
    Encode(video_frame, base::TimeTicks::Now());
    run_loop1.Run();
    ...
    const bool is_media_recorder_encoder_wrapper = testing::get<4>(GetParam()) || testing::get<0>(GetParam()) == VideoTrackRecorder::CodecId::kAv1;
...
TEST_P(VideoTrackRecorderTestParam, CheckMetricsProviderInVideoEncoding)
{
    ...
    const media::VideoCodecProfile video_codec_profile = MediaVideoCodecProfileFromCodecId(testing::get<0>(GetParam()));

    auto metrics_provider = std::make_unique<media::MockVideoEncoderMetricsProvider>();
    media::MockVideoEncoderMetricsProvider* mock_metrics_provider = metrics_provider.get();
    int initialize_time = 1;
    if (encode_alpha_channel
        && (video_codec_profile == media::VideoCodecProfile::VP8PROFILE_ANY || video_codec_profile == media::VideoCodecProfile::VP9PROFILE_PROFILE0)
    ...
    base::RunLoop run_loop1;
    InSequence s;
    EXPECT_CALL(*mock_callback_interface_, CreateVideoEncoderMetricsProvider()).WillOnce(Return(::testing::ByMove(std::move(metrics_provider))));
    EXPECT_CALL(*mock_metrics_provider,
        MockInitialize(video_codec_profile, frame_size,
            /*is_hardware_encoder=*/false, media::SVCScalabilityMode::kL1T1))
        .Times(initialize_time);
    EXPECT_CALL(*mock_metrics_provider, MockIncrementEncodedFrameCount()).Times(2);
    ...
    EXPECT_CALL(*mock_metrics_provider,
        MockInitialize(video_codec_profile, frame_size2,
            /*is_hardware_encoder=*/false, media::SVCScalabilityMode::kL1T1))
        .Times(initialize_time);
    EXPECT_CALL(*mock_metrics_provider, MockIncrementEncodedFrameCount()).WillOnce(RunClosure(run_loop1.QuitClosure()));
    ...
    const base::TimeTicks timeticks_now = base::TimeTicks::Now();
    const base::TimeTicks timeticks_later = timeticks_now + base::Milliseconds(10);
    const base::TimeTicks timeticks_last = timeticks_later + base::Milliseconds(10);
    ...
    EXPECT_CALL(*mock_metrics_provider, MockDestroy()).WillOnce(RunClosure(run_loop2.QuitClosure()));
    video_track_recorder_.reset();
    run_loop2.Run();
...
TEST_P(VideoTrackRecorderTestParam, EncodeFrameWithPaddedCodedSize)
{
    ...
    const TestFrameType test_frame_type = testing::get<3>(GetParam());
    scoped_refptr<media::VideoFrame> video_frame = CreateFrameForTest(test_frame_type, frame_size,
        /*encode_alpha_channel=*/false, kCodedSizePadding);

    base::RunLoop run_loop;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _)).Times(1).WillOnce(RunClosure(run_loop.QuitClosure()));
    Encode(video_frame, base::TimeTicks::Now());
    run_loop.Run();
...
TEST_P(VideoTrackRecorderTestParam, EncodeFrameRGB)
{
    ...
    const bool encode_alpha_channel = testing::get<2>(GetParam());
    media::VideoPixelFormat pixel_format = encode_alpha_channel ? media::PIXEL_FORMAT_ARGB : media::PIXEL_FORMAT_XRGB;
    scoped_refptr<media::VideoFrame> video_frame = test_frame_type == TestFrameType::kI420
        ? media::VideoFrame::CreateZeroInitializedFrame(pixel_format, frame_size, gfx::Rect(frame_size), frame_size, base::TimeDelta())
        : blink::CreateTestFrame(frame_size, gfx::Rect(frame_size), frame_size, media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER, pixel_format, base::TimeDelta());
...
TEST_P(VideoTrackRecorderTestParam, EncoderHonorsKeyFrameRequests)
{
    ...
    base::RunLoop run_loop2;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _));
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(false), _, _)).WillOnce(RunClosure(run_loop2.QuitClosure()));
    Encode(frame, base::TimeTicks::Now());
    Encode(frame, base::TimeTicks::Now());
    run_loop2.Run();
...
TEST_P(VideoTrackRecorderTestParam, NoSubsequenceKeyFramesWithDefaultKeyFrameConfig)
{
    InitializeRecorder(testing::get<0>(GetParam()));
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _));
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(false), _, _)).Times(8);
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(false), _, _)).WillOnce(RunClosure(run_loop.QuitClosure()));
    auto frame = media::VideoFrame::CreateBlackFrame(kTrackRecorderTestSize[0]);
    for (int i = 0; i != 10; ++i) {
...
TEST_P(VideoTrackRecorderTestParam, KeyFramesGeneratedWithIntervalCount)
{
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _));
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(false), _, _)).Times(3);
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _));
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(false), _, _)).Times(2);
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(false), _, _)).WillOnce(RunClosure(run_loop.QuitClosure()));
    auto frame = media::VideoFrame::CreateBlackFrame(kTrackRecorderTestSize[0]);
    for (int i = 0; i != 8; ++i) {
        Encode(frame, origin);
...
TEST_P(VideoTrackRecorderTestParam, KeyFramesGeneratedWithIntervalDuration)
{
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _));
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(false), _, _));
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _));
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(false), _, _));
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _)).WillOnce(RunClosure(run_loop.QuitClosure()));
...
TEST_P(VideoTrackRecorderTestParam, UsesFrameTimestampsIfProvided)
{
    ...
    InSequence s;
    base::RunLoop run_loop;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, _, _, capture_begin_time));
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, _, _, reference_time));
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, _, _, estimated_capture_time)).WillOnce(RunClosure(run_loop.QuitClosure()));
...
std::string PrintTestParams(const testing::TestParamInfo<testing::tuple<VideoTrackRecorder::CodecId, gfx::Size, bool, TestFrameType, bool>>& info)
{
    std::stringstream ss;
    ss << "codec ";
    ...
    ss << " size " + testing::get<1>(info.param).ToString() << " encode alpha " << (testing::get<2>(info.param) ? "true" : "false") << " frame type ";
    ...
    ss << " mediaVideoEncoder " << (testing::get<4>(info.param) ? "true" : "false");
...
INSTANTIATE_TEST_SUITE_P(All, VideoTrackRecorderTestParam,
    ::testing::Combine(ValuesIn(kTrackRecorderTestCodec), ValuesIn(kTrackRecorderTestSize), ::testing::Bool(), ValuesIn(kTestFrameTypes), ::testing::Bool()),
    PrintTestParams);

class VideoTrackRecorderTestMediaVideoEncoderParam : public ::testing::TestWithParam<bool>,
                                                     public VideoTrackRecorderTest,
                                                     public ScopedMediaRecorderUseMediaVideoEncoderForTest {
public:
...
TEST_P(VideoTrackRecorderTestMediaVideoEncoderParam, RelaysReadyStateEnded)
{
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(*mock_callback_interface_, OnSourceReadyStateChanged).WillOnce(RunClosure(run_loop.QuitClosure()));
...
TEST_P(VideoTrackRecorderTestMediaVideoEncoderParam, ForceKeyframeOnAlphaSwitch)
{
    InitializeRecorder(VideoTrackRecorder::CodecId::kVp8);

    const gfx::Size& frame_size = kTrackRecorderTestSize[0];
    const scoped_refptr<media::VideoFrame> opaque_frame = media::VideoFrame::CreateBlackFrame(frame_size);

    InSequence s;
    auto first_frame_encoded_alpha = media::DecoderBuffer::CopyFrom(base::as_byte_span("test"));
    first_frame_encoded_alpha->set_is_key_frame(true);
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _)).Times(1).WillOnce(SaveArg<1>(&first_frame_encoded_alpha));
    Encode(opaque_frame, base::TimeTicks::Now());

    const scoped_refptr<media::VideoFrame> alpha_frame = media::VideoFrame::CreateTransparentFrame(frame_size);
    auto second_frame_encoded_alpha = media::DecoderBuffer::CopyFrom(base::as_byte_span("test"));
    second_frame_encoded_alpha->set_is_key_frame(true);
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _)).Times(1).WillOnce(SaveArg<1>(&second_frame_encoded_alpha));
    Encode(alpha_frame, base::TimeTicks::Now());

    base::RunLoop run_loop;
    auto third_frame_encoded_alpha = media::DecoderBuffer::CopyFrom(base::as_byte_span("test"));
    third_frame_encoded_alpha->set_is_key_frame(false);
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(false), _, _))
        .Times(1)
        .WillOnce(DoAll(SaveArg<1>(&third_frame_encoded_alpha), RunClosure(run_loop.QuitClosure())));
    Encode(alpha_frame, base::TimeTicks::Now());
    run_loop.Run();
    ...
    EXPECT_GT(second_frame_encoded_alpha->side_data()->alpha_data.size(), kEmptySize);
    EXPECT_TRUE(third_frame_encoded_alpha->has_side_data());
    EXPECT_GT(third_frame_encoded_alpha->side_data()->alpha_data.size(), kEmptySize);
...
TEST_P(VideoTrackRecorderTestMediaVideoEncoderParam, HandlesOnError)
{
    ...
    const gfx::Size& frame_size = kTrackRecorderTestSize[0];
    const scoped_refptr<media::VideoFrame> video_frame = media::VideoFrame::CreateBlackFrame(frame_size);

    InSequence s;
    base::RunLoop run_loop1;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _)).WillOnce(RunClosure(run_loop1.QuitClosure()));
    Encode(video_frame, base::TimeTicks::Now());
    run_loop1.Run();
    ...
    base::RunLoop run_loop2;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _)).WillOnce(RunClosure(run_loop2.QuitClosure()));
    Encode(video_frame, base::TimeTicks::Now());
    run_loop2.Run();
...
TEST_P(VideoTrackRecorderTestMediaVideoEncoderParam, HandleSoftwareEncoderFallback)
{
    ...
    EXPECT_CALL(*platform_, GetGpuFactories()).WillRepeatedly(Return(&mock_gpu_factories));
    EXPECT_CALL(mock_gpu_factories, NotifyEncoderSupportKnown).WillRepeatedly(base::test::RunOnceClosure<0>());
    EXPECT_CALL(mock_gpu_factories, GetTaskRunner).WillRepeatedly(Return(scheduler::GetSingleThreadTaskRunnerForTesting()));
    EXPECT_CALL(mock_gpu_factories, GetVideoEncodeAcceleratorSupportedProfiles)
        .WillRepeatedly(Return(std::vector<media::VideoEncodeAccelerator::SupportedProfile> {
            media::VideoEncodeAccelerator::SupportedProfile(media::VideoCodecProfile::VP8PROFILE_ANY, gfx::Size(1920, 1080)),
        }));
    EXPECT_CALL(mock_gpu_factories, DoCreateVideoEncodeAccelerator).WillRepeatedly([]() {
        return new media::FakeVideoEncodeAccelerator(scheduler::GetSingleThreadTaskRunnerForTesting());
    });
    InitializeRecorder(VideoTrackRecorder::CodecId::kVp8);

    const gfx::Size& frame_size = gfx::Size(kVEAEncoderMinResolutionWidth, kVEAEncoderMinResolutionHeight);
    const scoped_refptr<media::VideoFrame> video_frame = media::VideoFrame::CreateBlackFrame(frame_size);
    ...
    EXPECT_FALSE(HasEncoderInstance());
    base::RunLoop run_loop2;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo).WillOnce(RunClosure(run_loop2.QuitClosure()));
...
TEST_P(VideoTrackRecorderTestMediaVideoEncoderParam, ReleasesFrame)
{
    ...
    const gfx::Size& frame_size = kTrackRecorderTestSize[0];
    scoped_refptr<media::VideoFrame> video_frame = media::VideoFrame::CreateBlackFrame(frame_size);
...
TEST_P(VideoTrackRecorderTestMediaVideoEncoderParam, WaitForEncoderSupport)
{
    ...
    EXPECT_CALL(mock_gpu_factories, NotifyEncoderSupportKnown).WillOnce(base::test::RunOnceClosure<0>());
    InitializeRecorder(VideoTrackRecorder::CodecId::kVp8);
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(*mock_callback_interface_, OnEncodedVideo(_, IsKeyFrame(true), _, _)).WillOnce(RunClosure(run_loop.QuitWhenIdleClosure()));
    Encode(video_frame, base::TimeTicks::Now());
    run_loop.Run();
}
...
INSTANTIATE_TEST_SUITE_P(All, VideoTrackRecorderTestMediaVideoEncoderParam, ::testing::Bool());

class VideoTrackRecorderPassthroughTest : public TestWithParam<VideoTrackRecorder::CodecId>, public VideoTrackRecorderTestBase {
public:
        ...
        : mock_source_(new MockMediaStreamVideoSource())
    {
        ...
        component_ = MakeGarbageCollected<MediaStreamComponentImpl>(
            source_, std::make_unique<MediaStreamVideoTrack>(mock_source_, WebPlatformMediaStreamSource::ConstraintsOnceCallback(), true /* enabled */));
    ...
    void InitializeRecorder()
    {
        video_track_recorder_ = std::make_unique<VideoTrackRecorderPassthrough>(scheduler::GetSingleThreadTaskRunnerForTesting(),
            WebMediaStreamTrack(component_.Get()), mock_callback_interface_->GetWeakCell(), KeyFrameRequestProcessor::Configuration());
    }
...
scoped_refptr<FakeEncodedVideoFrame> CreateFrame(bool is_key_frame, VideoTrackRecorder::CodecId codec)
{
    return FakeEncodedVideoFrame::Builder().WithKeyFrame(is_key_frame).WithData("abc").WithCodec(MediaVideoCodecFromCodecId(codec)).BuildRefPtr();
}
...
TEST_P(VideoTrackRecorderPassthroughTest, HandlesFrames)
{
    ...
    EXPECT_CALL(*mock_callback_interface_, OnPassthroughVideo(IsSameCodec(GetParam()), IsKeyFrame(true), _)).WillOnce(DoAll(SaveArg<1>(&encoded_data)));
    ...
    EXPECT_CALL(*mock_callback_interface_, OnPassthroughVideo(IsSameCodec(GetParam()), IsKeyFrame(false), _));
...
TEST_F(VideoTrackRecorderPassthroughTest, DoesntForwardDeltaFrameFirst)
{
    ...
    EXPECT_CALL(*mock_callback_interface_, OnPassthroughVideo(_, IsKeyFrame(false), _)).Times(0);
    ...
    EXPECT_CALL(*mock_callback_interface_, OnPassthroughVideo(_, IsKeyFrame(true), _));
    now = base::TimeTicks::Now();
    video_track_recorder_->OnEncodedVideoFrameForTesting(now, frame, now);
    Mock::VerifyAndClearExpectations(this);
    ...
    EXPECT_CALL(*mock_callback_interface_, OnPassthroughVideo).WillOnce(RunClosure(run_loop.QuitClosure()));
    now = base::TimeTicks::Now();
    video_track_recorder_->OnEncodedVideoFrameForTesting(now, frame, now);
    run_loop.Run();
    EXPECT_CALL(*mock_source_, OnEncodedSinkDisabled);
...
INSTANTIATE_TEST_SUITE_P(All, VideoTrackRecorderPassthroughTest, ValuesIn(kTrackRecorderTestCodec));
...
public:
    ...
    media::VideoEncodeAccelerator::SupportedProfiles MakeVp9Profiles(bool vbr_support = false)
    {
        media::VideoEncodeAccelerator::SupportedProfiles profiles;
        auto rc_mode = media::VideoEncodeAccelerator::SupportedRateControlMode::kConstantMode;
        if (vbr_support) {
            rc_mode |= media::VideoEncodeAccelerator::SupportedRateControlMode::kVariableMode;
        }

        profiles.emplace_back(media::VP9PROFILE_PROFILE1, gfx::Size(1920, 1080), 60, 1, rc_mode);
        profiles.emplace_back(media::VP9PROFILE_PROFILE2, gfx::Size(1920, 1080), 30, 1, rc_mode);
        return profiles;
    }

    media::VideoEncodeAccelerator::SupportedProfiles MakeVp8Vp9Profiles()
    {
        media::VideoEncodeAccelerator::SupportedProfiles profiles = MakeVp8Profiles();
        media::VideoEncodeAccelerator::SupportedProfiles vp9_profiles = MakeVp9Profiles();
        profiles.insert(profiles.end(), vp9_profiles.begin(), vp9_profiles.end());
        return profiles;
    }

    media::VideoEncodeAccelerator::SupportedProfiles MakeH264Profiles(bool vbr_support = false)
    {
        media::VideoEncodeAccelerator::SupportedProfiles profiles;
        auto rc_mode = media::VideoEncodeAccelerator::SupportedRateControlMode::kConstantMode;
        if (vbr_support) {
        ...
        profiles.emplace_back(media::H264PROFILE_BASELINE, gfx::Size(1920, 1080), 24, 1, rc_mode);
        profiles.emplace_back(media::H264PROFILE_MAIN, gfx::Size(1920, 1080), 30, 1, rc_mode);
        profiles.emplace_back(media::H264PROFILE_HIGH, gfx::Size(1920, 1080), 60, 1, rc_mode);
        return profiles;
    }
    test::TaskEnvironment task_environment_;
};
...
TEST_F(CodecEnumeratorTest, GetPreferredCodecIdDefault)
{
    ...
    const CodecEnumerator emulator((media::VideoEncodeAccelerator::SupportedProfiles()));
...
{
    const CodecEnumerator emulator(MakeVp9Profiles());
    media::VideoEncodeAccelerator::SupportedProfiles profiles = emulator.GetSupportedProfiles(CodecId::kVp9);
...
{
    const CodecEnumerator emulator(MakeVp9Profiles());
    media::VideoEncodeAccelerator::SupportedProfiles profiles = emulator.GetSupportedProfiles(CodecId::kVp8);
...
{
    const CodecEnumerator emulator(MakeVp9Profiles());
    EXPECT_EQ(std::make_pair(media::VP9PROFILE_PROFILE1, /*vbr_support=*/false), emulator.GetFirstSupportedVideoCodecProfile(CodecId::kVp9));
}

TEST_F(CodecEnumeratorTest, GetFirstSupportedVideoCodecProfileNoVp8)
{
    const CodecEnumerator emulator(MakeVp9Profiles());
    EXPECT_EQ(std::make_pair(media::VIDEO_CODEC_PROFILE_UNKNOWN, /*vbr_support=*/false), emulator.GetFirstSupportedVideoCodecProfile(CodecId::kVp8));
}
...
TEST_F(CodecEnumeratorTest, GetFirstSupportedVideoCodecProfileVp9VBR)
{
    ...
    EXPECT_EQ(std::make_pair(media::VP9PROFILE_PROFILE1, /*vbr_support=*/true), emulator.GetFirstSupportedVideoCodecProfile(CodecId::kVp9));
}
...
TEST_F(CodecEnumeratorTest, FindSupportedVideoCodecProfileH264)
{
    ...
    EXPECT_EQ(std::make_pair(media::H264PROFILE_HIGH, /*vbr_support=*/false), emulator.FindSupportedVideoCodecProfile(CodecId::kH264, media::H264PROFILE_HIGH));
}
...
TEST_F(CodecEnumeratorTest, FindSupportedVideoCodecProfileH264VBR)
{
    ...
    EXPECT_EQ(std::make_pair(media::H264PROFILE_HIGH, /*vbr_support=*/true), emulator.FindSupportedVideoCodecProfile(CodecId::kH264, media::H264PROFILE_HIGH));
}

TEST_F(CodecEnumeratorTest, FindSupportedVideoCodecProfileNoProfileH264)
{
    const CodecEnumerator emulator(MakeH264Profiles());
    EXPECT_EQ(std::make_pair(media::VIDEO_CODEC_PROFILE_UNKNOWN, /*vbr_support=*/false),
        emulator.FindSupportedVideoCodecProfile(CodecId::kH264, media::H264PROFILE_HIGH422PROFILE));
}
