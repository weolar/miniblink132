...
const TestParam kH264EncoderParameterTestParam[] = {
    { media::VideoCodecProfile::H264PROFILE_BASELINE, 50, kFrameWidth* kFrameHeight * 2 },
    { media::VideoCodecProfile::H264PROFILE_MAIN, 51, kFrameWidth* kFrameHeight * 4 },
    { media::VideoCodecProfile::H264PROFILE_HIGH, 52, kFrameWidth* kFrameHeight * 8 },
    // Test optional input.
...
public:
    ...
    H264EncoderFixture(std::optional<media::VideoCodecProfile> profile, std::optional<uint8_t> level, uint32_t bitrate)
        : profile_(profile)
        , level_(level)
        , bitrate_(bitrate)
        , encoder_(scheduler::GetSingleThreadTaskRunnerForTesting(),
              ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&H264EncoderFixture::OnEncodedVideo, CrossThreadUnretained(this))),
              VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kH264, profile_, level_), bitrate_,
              /*is_screencast=*/false, base::BindRepeating(&H264EncoderFixture::OnError, CrossThreadUnretained(this)))
    {
...
protected:
    ...
    void EncodeFrame()
    {
        encoder_.StartFrameEncode(media::VideoFrame::CreateBlackFrame({ kFrameWidth, kFrameHeight }), base::TimeTicks::Now());
    }

    std::pair<media::VideoCodecProfile, uint8_t> GetProfileLevelForTesting()
    {
        static const HashMap<EProfileIdc, media::VideoCodecProfile> kEProfileIdcToProfile({
        ...
        return { kEProfileIdcToProfile.find(eProfileIdc)->value, kELevelIdcToLevel.find(eLevelIdc)->value };
    }
...
TEST_F(H264EncoderFixture, ErrorCallOnTooLargeFrame)
{
    ...
    static_assert(kTooLargeDimension <= media::limits::kMaxDimension, "kTooLargeDimension is more than media::limits::kMaxDimension");
    static_assert(kTooLargeDimension * kTooLargeDimension <= media::limits::kMaxCanvas,
        "kTooLargeDimension * kTooLargeDimension is more than "
        "media::limits::kMaxDimension");
    constexpr gfx::Size kTooLargeResolution(kTooLargeDimension, kTooLargeDimension);
    auto frame = media::VideoFrame::CreateBlackFrame(kTooLargeResolution);
    ASSERT_TRUE(frame);
    EXPECT_CALL(*mock_metrics_provider_,
        MockInitialize(media::VideoCodecProfile::H264PROFILE_BASELINE, kTooLargeResolution,
            /*hardware_video_encoder=*/false, media::SVCScalabilityMode::kL1T1));
...
class H264EncoderParameterTest : public H264EncoderFixture, public ::testing::WithParamInterface<TestParam> {
public:
    H264EncoderParameterTest()
        : H264EncoderFixture(GetParam().profile, GetParam().level, GetParam().bitrate)
    {
    }
...
TEST_P(H264EncoderParameterTest, CheckProfileLevel)
{
    ...
    // first frame.
    EXPECT_CALL(*mock_metrics_provider_,
        MockInitialize(GetParam().profile.value_or(media::H264PROFILE_BASELINE), gfx::Size(kFrameWidth, kFrameHeight),
            /*hardware_video_encoder=*/false, media::SVCScalabilityMode::kL1T1));
...
INSTANTIATE_TEST_SUITE_P(All, H264EncoderParameterTest, testing::ValuesIn(kH264EncoderParameterTestParam));
} // namespace blink
