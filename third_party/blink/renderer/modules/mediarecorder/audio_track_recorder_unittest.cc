...
{
    return kMediaStreamAudioTrackBufferDurationMs * sample_rate / base::Time::kMillisecondsPerSecond;
}
...
public:
    ...
    {
        receiver_.Bind(mojo::PendingReceiver<media::mojom::InterfaceFactory>(std::move(handle)));
        ...
        receiver_.set_disconnect_handler(WTF::BindOnce(&TestInterfaceFactory::OnConnectionError, base::Unretained(this)));
    }
    ...
    void CreateAudioEncoder(mojo::PendingReceiver<media::mojom::AudioEncoder> receiver) override
    {
...
#if BUILDFLAG(IS_MAC)
        auto platform_audio_encoder = std::make_unique<media::AudioToolboxAudioEncoder>();
#elif BUILDFLAG(IS_WIN)
        HRESULT hr = ::CoInitializeEx(nullptr, COINIT_APARTMENTTHREADED | COINIT_DISABLE_OLE1DDE);
        DCHECK(SUCCEEDED(hr));
        auto platform_audio_encoder = std::make_unique<media::MFAudioEncoder>(blink::scheduler::GetSequencedTaskRunnerForTesting());
#else
#error "Unknown platform encoder."
#endif
        audio_encoder_receivers_.Add(std::make_unique<media::MojoAudioEncoderService>(std::move(platform_audio_encoder)), std::move(receiver));
    }

    // Stub out other `mojom::InterfaceFactory` interfaces.
    void CreateVideoDecoder(
        mojo::PendingReceiver<media::mojom::VideoDecoder> receiver, mojo::PendingRemote<media::stable::mojom::StableVideoDecoder> dst_video_decoder) override
    {
    }
    void CreateAudioDecoder(mojo::PendingReceiver<media::mojom::AudioDecoder> receiver) override
    {
    }
    void CreateDefaultRenderer(const std::string& audio_device_id, mojo::PendingReceiver<media::mojom::Renderer> receiver) override
    {
    }
#if BUILDFLAG(ENABLE_CAST_RENDERER)
    void CreateCastRenderer(const base::UnguessableToken& overlay_plane_id, mojo::PendingReceiver<media::mojom::Renderer> receiver) override
    {
    }
#endif
#if BUILDFLAG(IS_ANDROID)
    void CreateMediaPlayerRenderer(mojo::PendingRemote<media::mojom::MediaPlayerRendererClientExtension> client_extension_remote,
        mojo::PendingReceiver<media::mojom::Renderer> receiver,
        mojo::PendingReceiver<media::mojom::MediaPlayerRendererExtension> renderer_extension_receiver) override
    {
    }
    void CreateFlingingRenderer(const std::string& presentation_id, mojo::PendingRemote<media::mojom::FlingingRendererClientExtension> client_extension,
    ...
    {
        std::move(callback).Run(mojo::NullRemote(), nullptr, media::CreateCdmStatus::kCdmNotSupported);
    }

#if BUILDFLAG(IS_WIN)
    void CreateMediaFoundationRenderer(mojo::PendingRemote<media::mojom::MediaLog> media_log_remote, mojo::PendingReceiver<media::mojom::Renderer> receiver,
        mojo::PendingReceiver<media::mojom::MediaFoundationRendererExtension> renderer_extension_receiver,
        mojo::PendingRemote<::media::mojom::MediaFoundationRendererClientExtension> client_extension_remote) override
    {
    }
#endif // BUILDFLAG(IS_WIN)
private:
...
const ATRTestParams kATRTestParams[] = {
    ...
    // Change to mono:
    { media::ChannelLayoutConfig::Mono(), kDefaultSampleRate, AudioTrackRecorder::CodecId::kOpus, AudioTrackRecorder::BitrateMode::kVariable },

    // Different sampling rate as well:
    { media::ChannelLayoutConfig::Mono(), 24000, AudioTrackRecorder::CodecId::kOpus, AudioTrackRecorder::BitrateMode::kVariable },
    { media::ChannelLayoutConfig::Stereo(), 8000, AudioTrackRecorder::CodecId::kOpus, AudioTrackRecorder::BitrateMode::kVariable },

    // Using a non-default Opus sampling rate (48, 24, 16, 12, or 8 kHz).
    { media::ChannelLayoutConfig::Mono(), 22050, AudioTrackRecorder::CodecId::kOpus, AudioTrackRecorder::BitrateMode::kVariable },
    { media::ChannelLayoutConfig::Stereo(), 44100, AudioTrackRecorder::CodecId::kOpus, AudioTrackRecorder::BitrateMode::kVariable },
    { media::ChannelLayoutConfig::Stereo(), 96000, AudioTrackRecorder::CodecId::kOpus, AudioTrackRecorder::BitrateMode::kVariable },

    // Use Opus in constant bitrate mode:
    { media::ChannelLayoutConfig::Stereo(), kDefaultSampleRate, AudioTrackRecorder::CodecId::kOpus, AudioTrackRecorder::BitrateMode::kConstant },

    // Use PCM encoder.
    { media::ChannelLayoutConfig::Mono(), kDefaultSampleRate, AudioTrackRecorder::CodecId::kPcm, AudioTrackRecorder::BitrateMode::kVariable },
    { media::ChannelLayoutConfig::Stereo(), kDefaultSampleRate, AudioTrackRecorder::CodecId::kPcm, AudioTrackRecorder::BitrateMode::kVariable },

#if HAS_AAC_ENCODER
    { media::ChannelLayoutConfig::Stereo(), kDefaultSampleRate, AudioTrackRecorder::CodecId::kAac, AudioTrackRecorder::BitrateMode::kVariable },
    { media::ChannelLayoutConfig::Mono(), kDefaultSampleRate, AudioTrackRecorder::CodecId::kAac, AudioTrackRecorder::BitrateMode::kVariable },
    { media::ChannelLayoutConfig::Stereo(), 44100, AudioTrackRecorder::CodecId::kAac, AudioTrackRecorder::BitrateMode::kVariable },
    { media::ChannelLayoutConfig::Mono(), 44100, AudioTrackRecorder::CodecId::kAac, AudioTrackRecorder::BitrateMode::kVariable },
    { media::ChannelLayoutConfig(media::CHANNEL_LAYOUT_5_1_BACK, 6), 44100, AudioTrackRecorder::CodecId::kAac, AudioTrackRecorder::BitrateMode::kVariable },
    { media::ChannelLayoutConfig(media::CHANNEL_LAYOUT_5_1_BACK, 6), kDefaultSampleRate, AudioTrackRecorder::CodecId::kAac,
        AudioTrackRecorder::BitrateMode::kVariable },
#endif // HAS_AAC_ENCODER
};

std::string ParamsToString(const ::testing::TestParamInfo<ATRTestParams>& info)
{
    ...
    if (info.param.channel_layout.channel_layout() == media::ChannelLayout::CHANNEL_LAYOUT_MONO) {
        test_suffix << "Mono";
    } else if (info.param.channel_layout.channel_layout() == media::ChannelLayout::CHANNEL_LAYOUT_STEREO) {
        test_suffix << "Stereo";
    } else if (info.param.channel_layout.channel_layout() == media::ChannelLayout::CHANNEL_LAYOUT_5_1_BACK) {
...
class MockAudioTrackRecorderCallbackInterface : public GarbageCollected<MockAudioTrackRecorderCallbackInterface>, public AudioTrackRecorder::CallbackInterface {
public:
    virtual ~MockAudioTrackRecorderCallbackInterface() = default;
    MOCK_METHOD(void, OnEncodedAudio,
        (const media::AudioParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
            std::optional<media::AudioEncoder::CodecDescription> codec_description, base::TimeTicks capture_time),
        (override));
    MOCK_METHOD(void, OnAudioEncodingError, (media::EncoderStatus status), (override));
...
public:
    ...
    AudioTrackRecorderTest()
        : mock_callback_interface_(MakeGarbageCollected<MockAudioTrackRecorderCallbackInterface>())
        , codec_(GetParam().codec)
        , first_params_(
              media::AudioParameters::AUDIO_PCM_LOW_LATENCY, GetParam().channel_layout, GetParam().sample_rate, FramesPerInputBuffer(GetParam().sample_rate))
        , second_params_(media::AudioParameters::AUDIO_PCM_LOW_LATENCY,
              GetParam().channel_layout.channel_layout() == media::ChannelLayout::CHANNEL_LAYOUT_MONO ? media::ChannelLayoutConfig::Stereo()
                                                                                                      : media::ChannelLayoutConfig::Mono(),
              kDefaultSampleRate, FramesPerInputBuffer(kDefaultSampleRate))
        , first_source_(first_params_.channels(),
              /*freq=*/440, first_params_.sample_rate())
        , second_source_(second_params_.channels(),
              /*freq=*/440, second_params_.sample_rate())
    {
    ...
    void SetUp() override
    {
        ...
        EXPECT_CALL(*mock_callback_interface_, OnEncodedAudio)
            .WillRepeatedly(Invoke([this](const media::AudioParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
                                       std::optional<media::AudioEncoder::CodecDescription> codec_description,
                                       base::TimeTicks capture_time) { OnEncodedAudio(params, encoded_data, std::move(codec_description), capture_time); }));
    }
    ...
    void TearDown() override
    {
        ...
        // DCHECK.
        PostCrossThreadTask(*encoder_task_runner_.get(), FROM_HERE, CrossThreadBindOnce([] {
            bool result = Platform::Current()->GetBrowserInterfaceBroker()->SetBinderForTesting(media::mojom::InterfaceFactory::Name_, {});
            DCHECK(result);
        }));
    }
    ...
    void InitializeRecorder()
    {
        ...
        audio_track_recorder_ = std::make_unique<AudioTrackRecorder>(scheduler::GetSingleThreadTaskRunnerForTesting(), codec_, media_stream_component_,
            mock_callback_interface_->GetWeakCell(), 0u /* bits_per_second */, GetParam().bitrate_mode, encoder_task_runner_);

#if HAS_AAC_ENCODER
        if (codec_ == AudioTrackRecorder::CodecId::kAac) {
            PostCrossThreadTask(*encoder_task_runner_.get(), FROM_HERE, CrossThreadBindOnce([] {
                auto interface_factory = std::make_unique<TestInterfaceFactory>();
                bool result = Platform::Current()->GetBrowserInterfaceBroker()->SetBinderForTesting(
                    media::mojom::InterfaceFactory::Name_, WTF::BindRepeating(&TestInterfaceFactory::BindRequest, base::Owned(std::move(interface_factory))));
                CHECK(result);
            }));
        }
#endif // HAS_AAC_ENCODER
    ...
    void InitializeDecoder(const AudioTrackRecorder::CodecId codec, const media::AudioParameters& params)
    {
        ShutdownDecoder();
    ...
    void CalculateBufferInformation()
    {
        ...
        case AudioTrackRecorder::CodecId::kOpus:
            ...
            opus_buffer_size_ = 120 * kDefaultSampleRate * std::max(first_params_.channels(), second_params_.channels()) / base::Time::kMillisecondsPerSecond;
            frames_per_buffer_ = kOpusBufferDurationMs * kDefaultSampleRate / base::Time::kMillisecondsPerSecond;
            break;
        case AudioTrackRecorder::CodecId::kAac:
    ...
    {
        int sample_rate = use_first_source ? first_params_.sample_rate() : second_params_.sample_rate();
        int num_inputs = GetNumInputsNeeded(kExpectedNumOutputs, sample_rate);
        for (int i = 0; i < num_inputs; ++i) {
            if (use_first_source) {
                audio_track_recorder_->OnData(*GetFirstSourceAudioBus(), base::TimeTicks::Now());
            } else {
                audio_track_recorder_->OnData(*GetSecondSourceAudioBus(), base::TimeTicks::Now());
            }
        }
    ...
    int GetNumInputsNeeded(int desired_num_outputs, int sample_rate)
    {
        ...
        if (codec_ == AudioTrackRecorder::CodecId::kOpus) {
            ...
            inputs_per_output = frames_per_buffer_ / FramesPerInputBuffer(kDefaultSampleRate);
        } else {
            inputs_per_output = frames_per_buffer_ / FramesPerInputBuffer(sample_rate);
        }
        ...
        int additional_input_frames = frames_per_buffer_ % FramesPerInputBuffer(sample_rate) * desired_num_outputs;
    ...
    {
        std::unique_ptr<media::AudioBus> bus(media::AudioBus::Create(first_params_.channels(), FramesPerInputBuffer(first_params_.sample_rate())));
        first_source_.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), {}, bus.get());
        ...
        if (codec_ == AudioTrackRecorder::CodecId::kPcm && !paused_) {
            std::unique_ptr<media::AudioBus> cache_bus(media::AudioBus::Create(bus->channels(), bus->frames()));
            bus->CopyTo(cache_bus.get());

            int current_size = first_source_cache_.size();
            first_source_cache_.resize(current_size + cache_bus->frames() * cache_bus->channels());
            cache_bus->ToInterleaved<media::Float32SampleTypeTraits>(cache_bus->frames(), &first_source_cache_[current_size]);
        }
    ...
    {
        std::unique_ptr<media::AudioBus> bus(media::AudioBus::Create(second_params_.channels(), FramesPerInputBuffer(second_params_.sample_rate())));
        second_source_.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), {}, bus.get());
        return bus;
    }
    ...
    void ExpectOutputsAndRunClosure(base::OnceClosure closure)
    {
        ...
        if (GetParam().codec == AudioTrackRecorder::CodecId::kAac) {
            EXPECT_CALL(*this, DoOnEncodedAudio).Times(kExpectedNumOutputs - 1).InSequence(s_);
#if HAS_AAC_DECODER
            ...
            EXPECT_CALL(*this, DecodeOutputCb).InSequence(s2_);
            EXPECT_CALL(*this, DecodeCb).InSequence(s2_).WillOnce(RunOnceClosure(std::move(closure)));
#else
            // If we don't have a decoder, we must run the closure after the final
            // call to `DoOnEncodedAudio`.
            EXPECT_CALL(*this, DoOnEncodedAudio).InSequence(s_).WillOnce(RunOnceClosure(std::move(closure)));
#endif // HAS_AAC_DECODER
            return;
        }
#endif // HAS_AAC_ENCODER
    ...
    MOCK_METHOD(void, DoOnEncodedAudio, (const media::AudioParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data, base::TimeTicks timestamp));

    void OnEncodedAudio(const media::AudioParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
        std::optional<media::AudioEncoder::CodecDescription> codec_description, base::TimeTicks timestamp)
    {
    ...
    void ValidateOpusData(scoped_refptr<media::DecoderBuffer> encoded_data)
    {
        ...
        EXPECT_EQ(kDefaultSampleRate * kOpusBufferDurationMs / 1000,
            opus_decode_float(opus_decoder_, encoded_data->data(), static_cast<wtf_size_t>(encoded_data->size()), opus_buffer_.get(), opus_buffer_size_, 0));
    }
    ...
    void ValidatePcmData(scoped_refptr<media::DecoderBuffer> encoded_data)
    {
        ...
        for (size_t b = 0; b + 3 < encoded_data->size() && first_source_cache_pos_ < first_source_cache_.size();
             b += sizeof(first_source_cache_[0]), ++first_source_cache_pos_) {
            float sample;
            memcpy(&sample, encoded_data->AsSpan().subspan(b).data(), 4);
            ASSERT_FLOAT_EQ(sample, first_source_cache_[first_source_cache_pos_]) << "(Sample " << first_source_cache_pos_ << ")";
        }
    }
    ...
    void ValidateAacData(scoped_refptr<media::DecoderBuffer> encoded_data)
    {
        ...
        // output is validated.
        media::AudioDecoder::DecodeCB decode_cb = WTF::BindOnce(&AudioTrackRecorderTest::OnDecode, WTF::Unretained(this));
        aac_decoder_->Decode(encoded_data, std::move(decode_cb));
    }

    void InitializeAacDecoder(int channels, int sample_rate)
    {
        aac_decoder_ = std::make_unique<media::FFmpegAudioDecoder>(scheduler::GetSequencedTaskRunnerForTesting(), &media_log_);
        ...
        media::AudioDecoderConfig config(media::AudioCodec::kAAC, media::SampleFormat::kSampleFormatS16, channel_layout, sample_rate,
            /*extra_data=*/std::vector<uint8_t>(), media::EncryptionScheme::kUnencrypted);
        EXPECT_CALL(*this, InitCb);
        media::AudioDecoder::InitCB init_cb = WTF::BindOnce(&AudioTrackRecorderTest::OnInit, WTF::Unretained(this));
        media::AudioDecoder::OutputCB output_cb = WTF::BindRepeating(&AudioTrackRecorderTest::OnDecodeOutput, WTF::Unretained(this));
        aac_decoder_->Initialize(config, /*cdm_context=*/nullptr, std::move(init_cb), std::move(output_cb),
...
private:
    ...
    void PrepareTrack()
    {
        auto audio_source = std::make_unique<MediaStreamAudioSource>(scheduler::GetSingleThreadTaskRunnerForTesting(), true);
        auto* source = MakeGarbageCollected<MediaStreamSource>(String::FromUTF8("dummy_source_id"), MediaStreamSource::kTypeAudio,
            String::FromUTF8("dummy_source_name"), false /* remote */, std::move(audio_source));
        media_stream_component_ = MakeGarbageCollected<MediaStreamComponentImpl>(
            String::FromUTF8("audio_track"), source, std::make_unique<MediaStreamAudioTrack>(/*is_local=*/true));
        CHECK(MediaStreamAudioSource::From(source)->ConnectToInitializedTrack(media_stream_component_));
    }
};
...
TEST_P(AudioTrackRecorderTest, PacketSize)
{
    ...
    EXPECT_CALL(*this, DoOnEncodedAudio)
        ...
        .InSequence(s_)
        .WillRepeatedly([&encodedPacketSizes](const media::AudioParameters&, scoped_refptr<media::DecoderBuffer> encoded_data, base::TimeTicks) {
            encodedPacketSizes.push_back(encoded_data->size());
        });
    EXPECT_CALL(*this, DoOnEncodedAudio)
        .InSequence(s_)
        .WillOnce(testing::DoAll(RunOnceClosure(run_loop_.QuitClosure()),
            [&encodedPacketSizes](const media::AudioParameters&, scoped_refptr<media::DecoderBuffer> encoded_data, base::TimeTicks) {
                encodedPacketSizes.push_back(encoded_data->size());
            }));
    GenerateAndRecordAudio(/*use_first_source=*/true);
    run_loop_.Run();
...
INSTANTIATE_TEST_SUITE_P(, AudioTrackRecorderTest, testing::ValuesIn(kATRTestParams), ParamsToString);
