...
bool KeyFrameRequestProcessor::OnFrameAndShouldRequestKeyFrame(base::TimeTicks now)
{
    frame_counter_++;
    bool request_keyframe = absl::visit(base::Overloaded { [&](uint64_t count) { return frame_counter_ > last_key_frame_received_.frame_counter + count; },
                                            [&](base::TimeDelta duration) { return now >= last_key_frame_received_.timestamp + duration; },
                                            [&](auto&) {
                                                constexpr size_t kDefaultKeyIntervalCount = 100;
                                                return frame_counter_ > last_key_frame_received_.frame_counter + kDefaultKeyIntervalCount;
                                            } },
        config_);
