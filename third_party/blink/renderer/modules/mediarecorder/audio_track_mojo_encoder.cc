...
AudioTrackMojoEncoder::AudioTrackMojoEncoder(scoped_refptr<base::SequencedTaskRunner> encoder_task_runner, AudioTrackRecorder::CodecId codec,
    OnEncodedAudioCB on_encoded_audio_cb, OnEncodedAudioErrorCB on_encoded_audio_error_cb, uint32_t bits_per_second)
...
void AudioTrackMojoEncoder::OnSetFormat(const media::AudioParameters& input_params)
{
    DVLOG(1) << __func__;
    ...
    blink::Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(pending_interface_factory.InitWithNewPipeAndPassReceiver());
    ...
    mojo_encoder_ = std::make_unique<media::MojoAudioEncoder>(std::move(encoder_remote));
    if (!mojo_encoder_) {
    ...
    auto output_cb = base::BindPostTask(encoder_task_runner_, WTF::BindRepeating(&AudioTrackMojoEncoder::OnEncodeOutput, weak_factory_.GetWeakPtr()));
    auto done_cb = base::BindPostTask(encoder_task_runner_, WTF::BindOnce(&AudioTrackMojoEncoder::OnInitializeDone, weak_factory_.GetWeakPtr()));
    mojo_encoder_->Initialize(options, std::move(output_cb), std::move(done_cb));
}

void AudioTrackMojoEncoder::EncodeAudio(std::unique_ptr<media::AudioBus> input_bus, base::TimeTicks capture_time)
{
...
void AudioTrackMojoEncoder::DoEncodeAudio(std::unique_ptr<media::AudioBus> input_bus, base::TimeTicks capture_time)
{
    auto done_cb = base::BindPostTask(encoder_task_runner_, WTF::BindOnce(&AudioTrackMojoEncoder::OnEncodeDone, weak_factory_.GetWeakPtr()));
...
void AudioTrackMojoEncoder::OnInitializeDone(media::EncoderStatus status)
{
    ...
    while (!input_queue_.empty()) {
        DoEncodeAudio(std::move(input_queue_.front().audio_bus), input_queue_.front().capture_time);
        input_queue_.pop();
    }
}
...
void AudioTrackMojoEncoder::OnEncodeOutput(media::EncodedAudioBuffer encoded_buffer, std::optional<media::AudioEncoder::CodecDescription> codec_desc)
{
    if (has_error_) {
    ...
    on_encoded_audio_cb_.Run(encoded_buffer.params, std::move(buffer), std::move(codec_desc), encoded_buffer.timestamp + accumulated_paused_duration_);
}
