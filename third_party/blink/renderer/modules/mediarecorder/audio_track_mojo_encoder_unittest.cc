...
public:
    ...
    {
        CHECK(init_cb_);
        std::move(init_cb_).Run(media::EncoderStatus::Codes::kEncoderInitializeTwice);
    }

    // media::mojom::AudioEncoder:
    void Initialize(mojo::PendingAssociatedRemote<media::mojom::AudioEncoderClient> client, const media::AudioEncoderConfig& /*config*/,
    ...
    void Encode(media::mojom::AudioBufferPtr buffer, EncodeCallback callback) override
    {
        ...
        if (!timestamp_helper_) {
            timestamp_helper_ = std::make_unique<media::AudioTimestampHelper>(buffer->sample_rate);
            timestamp_helper_->SetBaseTimestamp(capture_timestamp - base::TimeTicks());
        }
        client_->OnEncodedBufferReady(
            media::EncodedAudioBuffer(media::TestAudioParameters::Normal(), std::move(data), base::TimeTicks() + timestamp_helper_->GetTimestamp()),
            description);
...
public:
    ...
    {
        CHECK(!audio_encoder_receiver_.is_bound()) << "Expecting at most one encoder instance";
...
private:
    ...
    mojo::Receiver<media::mojom::AudioEncoder> audio_encoder_receiver_ { &audio_encoder_ };
...
public:
    AudioTrackMojoEncoderTest()
    {
        CHECK(Platform::Current()->GetBrowserInterfaceBroker()->SetBinderForTesting(
            media::mojom::InterfaceFactory::Name_, WTF::BindRepeating(&TestInterfaceFactory::BindRequest, base::Unretained(&interface_factory_))));
    ...
    {
        Platform::Current()->GetBrowserInterfaceBroker()->SetBinderForTesting(media::mojom::InterfaceFactory::Name_, {});
    }
    ...
    AudioTrackMojoEncoder& audio_track_encoder()
    {
        return audio_track_encoder_;
    }
    ...
    media::EncoderStatus::Codes error_code() const
    {
        return error_code_;
    }
...
private:
    ...
    AudioTrackMojoEncoder audio_track_encoder_ { scheduler::GetSequencedTaskRunnerForTesting(), AudioTrackRecorder::CodecId::kAac,
        /*on_encoded_audio_cb=*/
        base::BindLambdaForTesting([this](const media::AudioParameters& /*params*/, scoped_refptr<media::DecoderBuffer> /*encoded_data*/,
                                       std::optional<media::AudioEncoder::CodecDescription> /*codec_desc*/, base::TimeTicks capture_time) {
            ++output_count_;
...
TEST_F(AudioTrackMojoEncoderTest, TimeInPauseIsRespected)
{
    ...
    audio_track_encoder().EncodeAudio(GenerateInput(), base::TimeTicks() + helper.GetTimestamp());
    helper.AddFrames(input_frames);
    audio_track_encoder().EncodeAudio(GenerateInput(), base::TimeTicks() + helper.GetTimestamp());
    helper.AddFrames(input_frames);
    ...
    base::RunLoop().RunUntilIdle();
    ASSERT_THAT(capture_times(), ElementsAre(timestamp_frame_0, timestamp_frame_1, timestamp_frame_2));
}
...
TEST_F(AudioTrackMojoEncoderTest, OnSetFormatError)
{
    ...
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(error_code(), media::EncoderStatus::Codes::kEncoderUnsupportedConfig);
}
