...
public:
    ...
    public:
        ...
        // empty otherwise.
        virtual void OnEncodedVideo(const media::Muxer::VideoParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
            std::optional<media::VideoEncoder::CodecDescription> codec_description, base::TimeTicks timestamp)
            = 0;

        virtual std::unique_ptr<media::VideoEncoderMetricsProvider> CreateVideoEncoderMetricsProvider() = 0;
    ...
    struct MODULES_EXPORT CodecProfile {
        ...
        CodecProfile(CodecId codec_id, std::optional<media::VideoCodecProfile> opt_profile, std::optional<media::VideoCodecLevel> opt_level);
        CodecProfile(CodecId codec_id, media::VideoCodecProfile profile, media::VideoCodecLevel level);

        bool operator==(const CodecProfile& others) const
        {
            return (codec_id == others.codec_id) && (profile == others.profile) && (level == others.level);
        }
        bool operator!=(const CodecProfile& others) const
        {
            return !(*this == others);
    ...
    using OnEncodedVideoCB = base::RepeatingCallback<void(const media::Muxer::VideoParameters& params, scoped_refptr<media::DecoderBuffer> encoded_data,
    ...
    double GetRequiredMinFramesPerSec() const override
    {
        return 1;
    }
    ...
    public:
        Encoder(scoped_refptr<base::SequencedTaskRunner> encoding_task_runner, const VideoTrackRecorder::OnEncodedVideoCB& on_encoded_video_cb,
        ...
        void InitializeEncoder(KeyFrameRequestProcessor::Configuration key_frame_config, std::unique_ptr<media::VideoEncoderMetricsProvider> metrics_provider,
        ...
        void StartFrameEncode(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks capture_timestamp);

        using OnEncodedVideoInternalCB = WTF::CrossThreadFunction<void(const media::Muxer::VideoParameters& params, std::string encoded_data,
            std::string encoded_alpha, base::TimeTicks capture_timestamp, bool is_key_frame)>;

        void SetPaused(bool paused);
        virtual bool CanEncodeAlphaChannel() const;
        virtual bool IsScreenContentEncodingForTesting() const
        {
            return false;
        }
    ...
    protected:
        ...
        scoped_refptr<media::VideoFrame> MaybeProvideEncodableFrame(scoped_refptr<media::VideoFrame> video_frame);
        // Called shortly after wrapping the Encoder in a SequenceBound, on the
        // codec-specific task runner.
        virtual void Initialize();
        virtual void EncodeFrame(scoped_refptr<media::VideoFrame> frame, base::TimeTicks capture_timestamp, bool request_keyframe) = 0;
        ...
        scoped_refptr<media::VideoFrame> ConvertToI420ForSoftwareEncoder(scoped_refptr<media::VideoFrame> frame);
    ...
    public:
        explicit CodecEnumerator(const media::VideoEncodeAccelerator::SupportedProfiles& vea_supported_profiles);
        ...
        std::pair<media::VideoCodecProfile, bool> FindSupportedVideoCodecProfile(CodecId codec, media::VideoCodecProfile profile) const;
        ...
        std::pair<media::VideoCodecProfile, bool> GetFirstSupportedVideoCodecProfile(CodecId codec) const;

        // Returns a list of supported media::VEA::SupportedProfile for a given
        // CodecId, or empty vector if CodecId is unsupported.
        media::VideoEncodeAccelerator::SupportedProfiles GetSupportedProfiles(CodecId codec) const;

    private:
        // VEA-supported profiles grouped by CodecId.
        HashMap<CodecId, media::VideoEncodeAccelerator::SupportedProfiles> supported_profiles_;
        CodecId preferred_codec_id_ = CodecId::kLast;
    };

    VideoTrackRecorder(scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner, WeakCell<CallbackInterface>* callback_interface);

    virtual void Pause() = 0;
    virtual void Resume() = 0;
    virtual void OnVideoFrameForTesting(scoped_refptr<media::VideoFrame> frame, base::TimeTicks capture_time, bool allow_vea_encoder)
    {
    }
    virtual void OnEncodedVideoFrameForTesting(base::TimeTicks now, scoped_refptr<EncodedVideoFrame> frame, base::TimeTicks capture_time)
    {
    }
...
public:
    ...
    static bool CanUseAcceleratedEncoder(CodecProfile& codec_profile, size_t width, size_t height, double framerate = 0.0);

    VideoTrackRecorderImpl(scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner, CodecProfile codec, MediaStreamComponent* track,
        WeakCell<CallbackInterface>* callback_interface, uint32_t bits_per_second, KeyFrameRequestProcessor::Configuration key_frame_config,
        size_t frame_buffer_pool_limit = media::DeviceVideoCaptureMaxBufferPoolSize());
    ...
    void OnVideoFrameForTesting(scoped_refptr<media::VideoFrame> frame, base::TimeTicks capture_time, bool allow_vea_encoder) override;
    void ForceKeyFrameForNextFrameForTesting();
...
private:
    ...
    void OnVideoFrame(bool allow_vea_encoder, scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks capture_time);
    void ProcessOneVideoFrame(bool allow_vea_encoder, scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks capture_time);
    void OnEncoderSupportKnown();
    void InitializeEncoder(uint32_t bits_per_second, bool allow_vea_encoder, media::VideoFrame::StorageType frame_storage_type, gfx::Size input_size);
    ...
    std::unique_ptr<Encoder> CreateHardwareVideoEncoder(scoped_refptr<base::SequencedTaskRunner> encoding_task_runner, CodecProfile codec_profile,
        const gfx::Size& input_size, bool use_import_mode, bool is_screencast);
    ...
    bool should_pause_encoder_on_initialization_ GUARDED_BY_CONTEXT(main_sequence_checker_) = false;
    WTF::Vector<MediaStreamFrame> incoming_frame_queue_ GUARDED_BY_CONTEXT(main_sequence_checker_);
    bool encoder_support_known_ GUARDED_BY_CONTEXT(main_sequence_checker_) = false;
...
public:
    VideoTrackRecorderPassthrough(scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner, MediaStreamComponent* track,
    ...
    VideoTrackRecorderPassthrough& operator=(const VideoTrackRecorderPassthrough&) = delete;
    ...
    void Pause() override;
    void Resume() override;
    void OnEncodedVideoFrameForTesting(base::TimeTicks now, scoped_refptr<EncodedVideoFrame> frame, base::TimeTicks capture_time) override;
...
private:
    ...
    enum class KeyFrameState { kWaitingForKeyFrame, kKeyFrameReceivedOK, kPaused };
