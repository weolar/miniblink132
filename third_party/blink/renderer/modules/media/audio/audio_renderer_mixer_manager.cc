...
media::AudioParameters GetMixerOutputParams(
    const media::AudioParameters& input_params, const media::AudioParameters& hardware_params, media::AudioLatency::Type latency)
{
    ...
    if (!hardware_params.IsValid() || hardware_params.format() == media::AudioParameters::AUDIO_FAKE) {
        ...
        if (latency == media::AudioLatency::Type::kPlayback) {
            preferred_output_buffer_size = std::max(input_params.frames_per_buffer(), hardware_params.frames_per_buffer());
        } else {
    ...
    case media::AudioLatency::Type::kInteractive:
        output_buffer_size = media::AudioLatency::GetInteractiveBufferSize(hardware_params.frames_per_buffer());
        break;
    case media::AudioLatency::Type::kRtc:
        output_buffer_size = media::AudioLatency::GetRtcBufferSize(output_sample_rate, preferred_output_buffer_size);
        break;
    case media::AudioLatency::Type::kPlayback:
        output_buffer_size = media::AudioLatency::GetHighLatencyBufferSize(output_sample_rate, preferred_output_buffer_size);
        break;
    ...
    media::AudioParameters params(input_params.format(), input_params.channel_layout_config(), output_sample_rate, output_buffer_size);
...
AudioRendererMixerManager::AudioRendererMixerManager(CreateSinkCB create_sink_cb)
...
scoped_refptr<AudioRendererMixerInput> AudioRendererMixerManager::CreateInput(const LocalFrameToken& source_frame_token, const FrameToken& main_frame_token,
    const base::UnguessableToken& session_id, std::string_view device_id, media::AudioLatency::Type latency)
{
    ...
    return base::MakeRefCounted<AudioRendererMixerInput>(this, source_frame_token, main_frame_token, device_id, latency);
}

AudioRendererMixer* AudioRendererMixerManager::GetMixer(const LocalFrameToken& source_frame_token, const FrameToken& main_frame_token,
    const media::AudioParameters& input_params, media::AudioLatency::Type latency, const media::OutputDeviceInfo& sink_info,
    scoped_refptr<media::AudioRendererSink> sink)
{
    ...
    const MixerKey key(source_frame_token, main_frame_token, input_params, latency, sink_info.device_id());
    ...
    if (it != mixers_.end() && !it->second.mixer->HasSinkError()) {
        ...
        dead_mixers_.emplace_back(std::move(it->second.mixer), it->second.ref_count);
        mixers_.erase(it);
    }

    const auto mixer_output_params = GetMixerOutputParams(input_params, sink_info.output_params(), latency);
    auto mixer = std::make_unique<AudioRendererMixer>(mixer_output_params, std::move(sink));
    auto* mixer_ref = mixer.get();
    mixers_[key] = { std::move(mixer), 1 };
    DVLOG(1) << __func__ << " mixer: " << mixer << " latency: " << base::to_underlying(latency) << "\n input: " << input_params.AsHumanReadableString()
...
{
    base::AutoLock auto_lock(mixers_lock_);
    auto it = base::ranges::find(mixers_, mixer, [](const std::pair<MixerKey, AudioRendererMixerReference>& val) { return val.second.mixer.get(); });

    // If a mixer isn't in the normal map, check the map for mixers w/ errors.
    auto dead_it = dead_mixers_.end();
    if (it == mixers_.end()) {
        dead_it = base::ranges::find(dead_mixers_, mixer, [](const AudioRendererMixerReference& val) { return val.mixer.get(); });
...
scoped_refptr<media::AudioRendererSink> AudioRendererMixerManager::GetSink(
    const LocalFrameToken& source_frame_token, const FrameToken& main_frame_token, std::string_view device_id)
{
    std::string device_id_str = std::string(device_id);

    auto token_for_creation = source_frame_token;
    if (media::AudioDeviceDescription::IsDefaultDevice(device_id_str) && main_frame_token.Is<blink::LocalFrameToken>()) {
    ...
    return create_sink_cb_.Run(token_for_creation, media::AudioSinkParameters(base::UnguessableToken(), std::move(device_id_str)));
}

AudioRendererMixerManager::MixerKey::MixerKey(const LocalFrameToken& source_frame_token, const FrameToken& main_frame_token,
    const media::AudioParameters& params, media::AudioLatency::Type latency, std::string_view device_id)
