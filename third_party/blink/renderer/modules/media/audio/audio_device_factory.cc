...
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS_LACROS)
...
base::TimeDelta GetDefaultAuthTimeout()
{
    ...
    return std::min(Platform::Current()->GetHungRendererDelay() * 8 / 10, kMaxAuthorizationTimeout);
}
...
scoped_refptr<media::AudioOutputDevice> NewOutputDevice(
    const blink::LocalFrameToken& frame_token, const media::AudioSinkParameters& params, base::TimeDelta auth_timeout)
{
    CHECK(blink::AudioOutputIPCFactory::GetInstance().io_task_runner());
    auto device = base::MakeRefCounted<media::AudioOutputDevice>(blink::AudioOutputIPCFactory::GetInstance().CreateAudioOutputIPC(frame_token),
        blink::AudioOutputIPCFactory::GetInstance().io_task_runner(), params, auth_timeout);
...
// static
media::AudioLatency::Type AudioDeviceFactory::GetSourceLatencyType(blink::WebAudioDeviceSourceType source)
{
    switch (source) {
...
scoped_refptr<media::AudioRendererSink> AudioDeviceFactory::NewAudioRendererSink(
    blink::WebAudioDeviceSourceType source_type, const blink::LocalFrameToken& frame_token, const media::AudioSinkParameters& params)
{
    DCHECK(!IsMixable(source_type));
    return NewOutputDevice(frame_token, params, GetDefaultAuthTimeout());
}

scoped_refptr<media::SwitchableAudioRendererSink> AudioDeviceFactory::NewMixableSink(blink::WebAudioDeviceSourceType source_type,
    const blink::LocalFrameToken& frame_token, const blink::FrameToken& main_frame_token, const media::AudioSinkParameters& params)
{
    DCHECK(IsMixable(source_type));
    DCHECK(IsMainThread()) << __func__ << "() is called on a wrong thread.";
    if (!mixer_manager_) {
        auto create_sink_cb
            = base::BindRepeating([](const LocalFrameToken& frame_token, const media::AudioSinkParameters& params) -> scoped_refptr<media::AudioRendererSink> {
        ...
        mixer_manager_ = std::make_unique<AudioRendererMixerManager>(std::move(create_sink_cb));
    }
    return mixer_manager_->CreateInput(
        frame_token, main_frame_token, params.session_id, params.device_id, AudioDeviceFactory::GetSourceLatencyType(source_type));
}

scoped_refptr<media::AudioCapturerSource> AudioDeviceFactory::NewAudioCapturerSource(WebLocalFrame* web_frame, const media::AudioSourceParameters& params)
{
    return base::MakeRefCounted<media::AudioInputDevice>(
        blink::AudioInputIPCFactory::CreateAudioInputIPC(web_frame->GetLocalFrameToken(), web_frame->GetTaskRunner(TaskType::kInternalMedia), params),
        media::AudioInputDevice::Purpose::kUserInput, media::AudioInputDevice::DeadStreamDetection::kEnabled);
}

media::OutputDeviceInfo AudioDeviceFactory::GetOutputDeviceInfo(const blink::LocalFrameToken& frame_token, const std::string& device_id)
{
    DCHECK(IsMainThread()) << __func__ << "() is called on a wrong thread.";

    if (!sink_cache_) {
        auto create_sink_cb = base::BindRepeating(
            [](AudioDeviceFactory* factory, const LocalFrameToken& frame_token, const std::string& device_id) -> scoped_refptr<media::AudioRendererSink> {
                ...
                return factory->NewAudioRendererSink(
                    blink::WebAudioDeviceSourceType::kNone, frame_token, media::AudioSinkParameters(base::UnguessableToken(), device_id));
            },
            base::Unretained(this));

        constexpr base::TimeDelta kDeleteTimeout = base::Milliseconds(5000);
        sink_cache_ = std::make_unique<AudioRendererSinkCache>(base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::BEST_EFFORT,
