...
class AudioRendererMixerInputTest : public testing::Test, public AudioRendererMixerPool {
public:
    AudioRendererMixerInputTest()
    {
        ...
        fake_callback_ = std::make_unique<media::FakeAudioRenderCallback>(0, kSampleRate);
        audio_bus_ = media::AudioBus::Create(audio_parameters_);
    }

    AudioRendererMixerInputTest(const AudioRendererMixerInputTest&) = delete;
    AudioRendererMixerInputTest& operator=(const AudioRendererMixerInputTest&) = delete;

    void CreateMixerInput(const std::string& device_id)
    {
        mixer_input_ = base::MakeRefCounted<AudioRendererMixerInput>(this, LocalFrameToken(), FrameToken(), device_id, media::AudioLatency::Type::kPlayback);
        mixer_input_->GetOutputDeviceInfoAsync(base::DoNothing());
        task_environment_.RunUntilIdle();
    }

    AudioRendererMixer* GetMixer(const LocalFrameToken&, const FrameToken&, const media::AudioParameters& params, media::AudioLatency::Type,
        const media::OutputDeviceInfo& sink_info, scoped_refptr<media::AudioRendererSink> sink) override
    {
        EXPECT_TRUE(params.IsValid());
        size_t idx = (sink_info.device_id() == kDefaultDeviceId) ? 0 : 1;
        if (!mixers_[idx]) {
            EXPECT_CALL(*reinterpret_cast<media::MockAudioRendererSink*>(sink.get()), Start());

            mixers_[idx] = std::make_unique<AudioRendererMixer>(audio_parameters_, std::move(sink));
        }
    ...
    scoped_refptr<media::AudioRendererSink> GetSink(const LocalFrameToken&, const FrameToken&, std::string_view device_id) override
    {
        ...
        auto sink = base::MakeRefCounted<media::MockAudioRendererSink>(std::string(device_id), status);
    ...
    AudioRendererMixer* GetInputMixer()
    {
        return mixer_input_->mixer_;
    }
    media::MockAudioRendererSink* GetMockSink() const
    {
        return reinterpret_cast<media::MockAudioRendererSink*>(mixer_input_->sink_.get());
    }
...
TEST_F(AudioRendererMixerInputTest, SwitchOutputDevice)
{
    ...
    mixer_input_->SwitchOutputDevice(kDeviceId, base::BindOnce(&AudioRendererMixerInputTest::SwitchCallback, base::Unretained(this), &run_loop));
    run_loop.Run();
...
TEST_F(AudioRendererMixerInputTest, SwitchOutputDeviceToSameDevice)
{
    ...
    AudioRendererMixer* old_mixer = GetInputMixer();
    base::RunLoop run_loop;
    mixer_input_->SwitchOutputDevice(kDefaultDeviceId, base::BindOnce(&AudioRendererMixerInputTest::SwitchCallback, base::Unretained(this), &run_loop));
    run_loop.Run();
    AudioRendererMixer* new_mixer = GetInputMixer();
...
TEST_F(AudioRendererMixerInputTest, SwitchOutputDeviceToAnotherDevice)
{
    ...
    AudioRendererMixer* old_mixer = GetInputMixer();
    base::RunLoop run_loop;
    mixer_input_->SwitchOutputDevice(kAnotherDeviceId, base::BindOnce(&AudioRendererMixerInputTest::SwitchCallback, base::Unretained(this), &run_loop));
    run_loop.Run();
    AudioRendererMixer* new_mixer = GetInputMixer();
    EXPECT_NE(old_mixer, new_mixer);
    mixer_input_->Stop();
...
TEST_F(AudioRendererMixerInputTest, SwitchOutputDeviceToNonexistentDevice)
{
    mixer_input_->Initialize(audio_parameters_, fake_callback_.get());
    mixer_input_->Start();
    EXPECT_CALL(*this, SwitchCallbackCalled(media::OUTPUT_DEVICE_STATUS_ERROR_NOT_FOUND));
    base::RunLoop run_loop;
    mixer_input_->SwitchOutputDevice(kNonexistentDeviceId, base::BindOnce(&AudioRendererMixerInputTest::SwitchCallback, base::Unretained(this), &run_loop));
    run_loop.Run();
    mixer_input_->Stop();
}
...
TEST_F(AudioRendererMixerInputTest, SwitchOutputDeviceToUnauthorizedDevice)
{
    mixer_input_->Initialize(audio_parameters_, fake_callback_.get());
    mixer_input_->Start();
    EXPECT_CALL(*this, SwitchCallbackCalled(media::OUTPUT_DEVICE_STATUS_ERROR_NOT_AUTHORIZED));
    base::RunLoop run_loop;
    mixer_input_->SwitchOutputDevice(kUnauthorizedDeviceId, base::BindOnce(&AudioRendererMixerInputTest::SwitchCallback, base::Unretained(this), &run_loop));
    run_loop.Run();
    mixer_input_->Stop();
}
...
{
    mixer_input_->Stop();
    mixer_input_ = base::MakeRefCounted<AudioRendererMixerInput>(this, LocalFrameToken(), FrameToken(), kDefaultDeviceId, media::AudioLatency::Type::kPlayback);
...
TEST_F(AudioRendererMixerInputTest, SwitchOutputDeviceDuringGODIA)
{
    ...
    mixer_input_->GetOutputDeviceInfoAsync(base::BindOnce(&AudioRendererMixerInputTest::OnDeviceInfoReceived, base::Unretained(this)));
    mixer_input_->SwitchOutputDevice(kAnotherDeviceId, base::BindOnce(&AudioRendererMixerInputTest::SwitchCallbackCalled, base::Unretained(this)));
    {
        ...
        EXPECT_CALL(*this, OnDeviceInfoReceived(_)).WillOnce(testing::SaveArg<0>(&info));
        EXPECT_CALL(*this, SwitchCallbackCalled(media::OUTPUT_DEVICE_STATUS_OK));
        task_environment_.RunUntilIdle();
...
TEST_F(AudioRendererMixerInputTest, GODIADuringSwitchOutputDeviceWhichFails)
{
    ...
    mixer_input_->SwitchOutputDevice(kNonexistentDeviceId, base::BindOnce(&AudioRendererMixerInputTest::SwitchCallbackCalled, base::Unretained(this)));
    mixer_input_->GetOutputDeviceInfoAsync(base::BindOnce(&AudioRendererMixerInputTest::OnDeviceInfoReceived, base::Unretained(this)));
...
{
    EXPECT_CALL(*this, SwitchCallbackCalled(media::OUTPUT_DEVICE_STATUS_OK));
    mixer_input_->SwitchOutputDevice(std::string(), base::BindOnce(&AudioRendererMixerInputTest::SwitchCallbackCalled, base::Unretained(this)));
