...
AudioRendererMixer::AudioRendererMixer(const media::AudioParameters& output_params, scoped_refptr<media::AudioRendererSink> sink)
...
void AudioRendererMixer::AddMixerInput(const media::AudioParameters& input_params, media::AudioConverter::InputCallback* input)
{
    ...
    if (can_passthrough(input_sample_rate)) {
        ...
        if (converter == converters_.end()) {
            std::pair<AudioConvertersMap::iterator, bool> result = converters_.insert(std::make_pair(input_sample_rate,
...
void AudioRendererMixer::RemoveMixerInput(const media::AudioParameters& input_params, media::AudioConverter::InputCallback* input)
{
    base::AutoLock auto_lock(lock_);
...
int AudioRendererMixer::Render(base::TimeDelta delay, base::TimeTicks delay_timestamp, const media::AudioGlitchInfo& glitch_info, media::AudioBus* audio_bus)
{
    TRACE_EVENT("audio", "AudioRendererMixer::Render", "playout_delay (ms)", delay.InMillisecondsF(), "delay_timestamp (ms)",
    ...
    uint32_t frames_delayed = base::saturated_cast<uint32_t>(media::AudioTimestampHelper::TimeToFrames(delay, output_params_.sample_rate()));
