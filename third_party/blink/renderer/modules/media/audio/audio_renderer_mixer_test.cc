...
using AudioRendererMixerTestData = std::tuple<const int* const, size_t, int, double>;

class AudioRendererMixerTest : public testing::TestWithParam<AudioRendererMixerTestData>, public AudioRendererMixerPool {
public:
    AudioRendererMixerTest()
        : epsilon_(std::get<3>(GetParam()))
        , half_fill_(false)
    {
        ...
        for (size_t i = 0; i < sample_rates_count; ++i) {
            input_parameters_.emplace_back(
                media::AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::FromLayout<kChannelLayout>(), sample_rates[i], kHighLatencyBufferSize);
        }

        // Create output parameters based on test parameters.
        output_parameters_ = media::AudioParameters(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::FromLayout<kChannelLayout>(),
        ...
        expected_callback_ = std::make_unique<media::FakeAudioRenderCallback>(step, output_parameters_.sample_rate());
    ...
    AudioRendererMixer* GetMixer(const LocalFrameToken&, const FrameToken&, const media::AudioParameters&, media::AudioLatency::Type,
        const media::OutputDeviceInfo&, scoped_refptr<media::AudioRendererSink>) final
    {
        return mixer_.get();
    }
    ...
    scoped_refptr<media::AudioRendererSink> GetSink(const LocalFrameToken&, const FrameToken&, std::string_view) override
    {
        return sink_;
    }
    ...
    void InitializeInputs(int inputs_per_sample_rate)
    {
        ...
        for (size_t i = 0, input = 0; i < input_parameters_.size(); ++i) {
            ...
            for (int j = 0; j < inputs_per_sample_rate; ++j, ++input) {
                fake_callbacks_.push_back(std::make_unique<media::FakeAudioRenderCallback>(step, output_parameters_.sample_rate()));
                mixer_inputs_.push_back(CreateMixerInput());
                mixer_inputs_[input]->Initialize(input_parameters_[i], fake_callbacks_[input].get());
        ...
        for (int i = 0; i < audio_bus_->channels(); ++i) {
            for (int j = index; j < frames; j++) {
                ...
                if ((error > epsilon * scale) && (error > epsilon)) {
                    EXPECT_NEAR(expected_audio_bus_->channel(i)[j] * scale, audio_bus_->channel(i)[j], epsilon * scale) << " i=" << i << ", j=" << j;
                    return false;
                }
            }
        }
    ...
    bool RenderAndValidateAudioData(float scale)
    {
        ...
        int frames = mixer_callback_->Render(base::TimeDelta(), base::TimeTicks::Now(), {}, audio_bus_.get());
        if (frames != audio_bus_->frames()) {
            return false;
        }

        // Render expected audio data (without scaling).
        expected_callback_->Render(base::TimeDelta(), base::TimeTicks::Now(), {}, expected_audio_bus_.get());

        if (half_fill_) {
            // In this case, just verify that every frame was initialized, this will
            // only fail under tooling such as valgrind.
            return ValidateAudioData(0, frames, 0, std::numeric_limits<double>::max());
        } else {
    ...
    {
        for (int i = 0; i < audio_bus_->channels(); ++i) {
            std::fill(audio_bus_->channel(i), audio_bus_->channel(i) + audio_bus_->frames(), value);
        }
    }
    ...
    void MixedStopPlayTest(int inputs)
    {
        ...
        ASSERT_TRUE(RenderAndValidateAudioData(std::max(1.f, static_cast<float>(floor(mixer_inputs_.size() / 2.f)))));
    ...
    void GlitchInfoTest(int inputs)
    {
        ...
        media::AudioGlitchInfo glitch_info { .duration = base::Milliseconds(100), .count = 123 };
        media::AudioGlitchInfo expected_glitch_info;

        for (int i = 0; i < kMixerCycles; ++i) {
            expected_glitch_info += glitch_info;
            mixer_callback_->Render(base::TimeDelta(), base::TimeTicks::Now(), glitch_info, audio_bus_.get());
        }
        ...
        for (int i = 0; i < kMixerCycles; ++i) {
            mixer_callback_->Render(base::TimeDelta(), base::TimeTicks::Now(), {}, audio_bus_.get());
        }
    ...
    {
        auto input = base::MakeRefCounted<AudioRendererMixerInput>(this, LocalFrameToken(), FrameToken(),
            // default device ID.
...
TEST_P(AudioRendererMixerBehavioralTest, MixerPausesStream)
{
    ...
    base::WaitableEvent pause_event(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
...
INSTANTIATE_TEST_SUITE_P(All, AudioRendererMixerTest,
    testing::Values(
        ...
        std::make_tuple(static_cast<const int* const>(kTestInput3Rates), std::size(kTestInput3Rates), kTestInput3Rates[0], 0.01),

        // Upsampling, multiple sinput sample rates.
        std::make_tuple(static_cast<const int* const>(kTestInput3Rates), std::size(kTestInput3Rates), kTestInput3Rates[2], 0.01),

        // Both downsampling and upsampling, multiple input sample rates
        std::make_tuple(static_cast<const int* const>(kTestInput3Rates), std::size(kTestInput3Rates), kTestInput3Rates[1], 0.01)));
...
// parameter set.
INSTANTIATE_TEST_SUITE_P(All, AudioRendererMixerBehavioralTest,
    testing::ValuesIn(std::vector<AudioRendererMixerTestData>(1, std::make_tuple(&kTestInputLower, 1, kTestInputLower, 0.00000048))));
} // namespace blink
