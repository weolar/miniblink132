...
// thread.
class MODULES_EXPORT MojoAudioOutputIPC : public media::AudioOutputIPC, public media::mojom::blink::AudioOutputStreamProviderClient {
public:
    using FactoryAccessorCB = base::RepeatingCallback<blink::mojom::blink::RendererAudioOutputStreamFactory*()>;

    // |factory_accessor| is required to provide a
    // RendererAudioOutputStreamFactory* if IPC is possible.
    MojoAudioOutputIPC(FactoryAccessorCB factory_accessor, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner);
    ...
    void RequestDeviceAuthorization(media::AudioOutputIPCDelegate* delegate, const base::UnguessableToken& session_id, const std::string& device_id) override;
    ...
    void Created(mojo::PendingRemote<media::mojom::blink::AudioOutputStream> stream, media::mojom::blink::ReadWriteAudioDataPipePtr data_pipe) override;
...
private:
    ...
    void ProviderClientBindingDisconnected(uint32_t disconnect_reason, const std::string& description);

    mojo::PendingReceiver<media::mojom::blink::AudioOutputStreamProvider> MakeProviderReceiver();
    ...
    // asynchronously.
    void DoRequestDeviceAuthorization(const base::UnguessableToken& session_id, const std::string& device_id, AuthorizationCB callback);
    ...
    mojo::Receiver<media::mojom::blink::AudioOutputStreamProviderClient> receiver_ { this };
