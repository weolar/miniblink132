...
class AudioRendererSinkCache::WindowObserver final : public GarbageCollected<AudioRendererSinkCache::WindowObserver>,
...
const char AudioRendererSinkCache::WindowObserver::kSupplementName[] = "AudioRendererSinkCache::WindowObserver";
...
void AudioRendererSinkCache::InstallWindowObserver(LocalDOMWindow& window)
{
    if (Supplement<LocalDOMWindow>::From<WindowObserver>(window))
        return;
    Supplement<LocalDOMWindow>::ProvideTo(window, MakeGarbageCollected<WindowObserver>(window));
}

AudioRendererSinkCache::AudioRendererSinkCache(
    scoped_refptr<base::SequencedTaskRunner> cleanup_task_runner, CreateSinkCallback create_sink_cb, base::TimeDelta delete_timeout)
...
media::OutputDeviceInfo AudioRendererSinkCache::GetSinkInfo(const LocalFrameToken& source_frame_token, const std::string& device_id)
{
    TRACE_EVENT_BEGIN2("audio", "AudioRendererSinkCache::GetSinkInfo", "frame_token", source_frame_token.ToString(), "device id", device_id);
    {
        base::AutoLock auto_lock(cache_lock_);
        auto cache_iter = FindCacheEntry_Locked(source_frame_token, device_id);
        if (cache_iter != cache_.end()) {
            // A matching cached sink is found.
            TRACE_EVENT_END1("audio", "AudioRendererSinkCache::GetSinkInfo", "result", "Cache hit");
            return cache_iter->sink->GetOutputDeviceInfo();
        }
    }

    // No matching sink found, create a new one.
    scoped_refptr<media::AudioRendererSink> sink = create_sink_cb_.Run(source_frame_token, device_id);

    MaybeCacheSink(source_frame_token, device_id, sink);

    TRACE_EVENT_END1("audio", "AudioRendererSinkCache::GetSinkInfo", "result", "Cache miss");
    // |sink| is ref-counted, so it's ok if it is removed from cache before we
    // get here.
    return sink->GetOutputDeviceInfo();
}

void AudioRendererSinkCache::DeleteLater(scoped_refptr<media::AudioRendererSink> sink)
{
    PostDelayedCrossThreadTask(*cleanup_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&AudioRendererSinkCache::DeleteSink,
...
void AudioRendererSinkCache::DeleteSink(const media::AudioRendererSink* sink_ptr)
{
    DCHECK(sink_ptr);
...

    {
        ...
        auto cache_iter = base::ranges::find(cache_, sink_ptr, [](const CacheEntry& val) { return val.sink.get(); });
...
AudioRendererSinkCache::CacheContainer::iterator AudioRendererSinkCache::FindCacheEntry_Locked(
    const LocalFrameToken& source_frame_token, const std::string& device_id)
{
    cache_lock_.AssertAcquired();
    return base::ranges::find_if(cache_, [source_frame_token, &device_id](const CacheEntry& val) {
...
void AudioRendererSinkCache::MaybeCacheSink(
    const LocalFrameToken& source_frame_token, const std::string& device_id, scoped_refptr<media::AudioRendererSink> sink)
{
    if (!SinkIsHealthy(sink.get())) {
        TRACE_EVENT_INSTANT0("audio", "MaybeCacheSink: Unhealthy sink", TRACE_EVENT_SCOPE_THREAD);
...
void AudioRendererSinkCache::DropSinksForFrame(const LocalFrameToken& source_frame_token)
{
    base::AutoLock auto_lock(cache_lock_);
