...
blink::LocalFrameToken TokenFromInt(int i)
{
    ...
    return blink::LocalFrameToken(base::UnguessableToken::CreateForTesting(base_token.GetHighForSerialization() + i, base_token.GetLowForSerialization() + i));
}
...
class FakeRemoteFactory : public mojom::blink::RendererAudioOutputStreamFactory {
public:
    ...
    void RequestDeviceAuthorization(mojo::PendingReceiver<media::mojom::blink::AudioOutputStreamProvider> stream_provider,
    ...
    void Bind(mojo::ScopedMessagePipeHandle handle)
    {
        ...
        receiver_.Bind(mojo::PendingReceiver<mojom::blink::RendererAudioOutputStreamFactory>(std::move(handle)));
    }

private:
    mojo::Receiver<mojom::blink::RendererAudioOutputStreamFactory> receiver_ { this };
...
class FakeAudioOutputIPCDelegate : public media::AudioOutputIPCDelegate {
    ...
    void OnDeviceAuthorized(media::OutputDeviceStatus device_status, const media::AudioParameters& output_params, const std::string& matched_device_id) override
    {
    }
    void OnStreamCreated(base::UnsafeSharedMemoryRegion region, base::SyncSocket::ScopedHandle socket_handle, bool playing_automatically) override
    {
    }
...
public:
    ...
    void RequestAuthorizationOnIOThread(std::unique_ptr<media::AudioOutputIPC> output_ipc)
    {
        output_ipc->RequestDeviceAuthorization(&fake_delegate, base::UnguessableToken(), "");
...
TEST_F(AudioOutputIPCFactoryTest, CallFactoryFromIOThread)
{
    ...
    interface_broker.SetBinderForTesting(
        mojom::blink::RendererAudioOutputStreamFactory::Name_, base::BindRepeating(&FakeRemoteFactory::Bind, base::Unretained(&remote_factory)));

    AudioOutputIPCFactory ipc_factory(io_thread->task_runner());

    ipc_factory.RegisterRemoteFactory(TokenFromInt(kRenderFrameId), interface_broker);
    ...
    io_thread->task_runner()->PostTask(FROM_HERE,
        base::BindOnce(&AudioOutputIPCFactoryTest::RequestAuthorizationOnIOThread, base::Unretained(this),
    ...
    interface_broker.SetBinderForTesting(mojom::blink::RendererAudioOutputStreamFactory::Name_, {});
...
TEST_F(AudioOutputIPCFactoryTest, SeveralFactories)
{
    ...
    interface_broker.SetBinderForTesting(
        mojom::blink::RendererAudioOutputStreamFactory::Name_, base::BindLambdaForTesting([&](mojo::ScopedMessagePipeHandle handle) {
    ...
    for (int i = 0; i < n_factories; i++) {
        ipc_factory.RegisterRemoteFactory(TokenFromInt(kRenderFrameId + i), interface_broker);
    }
