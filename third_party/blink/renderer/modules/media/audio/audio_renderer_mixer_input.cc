...
AudioRendererMixerInput::AudioRendererMixerInput(AudioRendererMixerPool* mixer_pool, const LocalFrameToken& source_frame_token,
    const FrameToken& main_frame_token, std::string_view device_id, media::AudioLatency::Type latency)
...
void AudioRendererMixerInput::Initialize(const media::AudioParameters& params, AudioRendererSink::RenderCallback* callback)
{
    DCHECK(!started_);
    DCHECK(!mixer_);
    DCHECK(callback);
    ...
    total_fade_in_frames_ = static_cast<int>(media::AudioTimestampHelper::TimeToFrames(kFadeInDuration, params_.sample_rate()));
}
...
void AudioRendererMixerInput::Start()
{
    ...
    started_ = true;
    mixer_ = mixer_pool_->GetMixer(source_frame_token_, main_frame_token_, params_, latency_, *device_info_, std::move(sink_));
...
void AudioRendererMixerInput::GetOutputDeviceInfoAsync(OutputDeviceInfoCB info_cb)
{
    ...
    if (device_info_.has_value() && (sink_ || mixer_)) {
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(info_cb), *device_info_));
        return;
    }
    ...
    sink_ = mixer_pool_->GetSink(source_frame_token_, main_frame_token_, device_id_);
    ...
    // threading issues.
    sink_->GetOutputDeviceInfoAsync(base::BindOnce(&AudioRendererMixerInput::OnDeviceInfoReceived, base::RetainedRef(this), std::move(info_cb)));
}
...
void AudioRendererMixerInput::SwitchOutputDevice(const std::string& device_id, media::OutputDeviceStatusCB callback)
{
    ...
    if (godia_in_progress_) {
        ...
        if (pending_switch_cb_) {
            std::move(pending_switch_cb_).Run(media::OUTPUT_DEVICE_STATUS_ERROR_INTERNAL);
        }
    ...
    if (device_id == device_id_ || (media::AudioDeviceDescription::IsDefaultDevice(device_id_) && media::AudioDeviceDescription::IsDefaultDevice(device_id))) {
    ...
    // know it's a success.
    auto new_sink = mixer_pool_->GetSink(source_frame_token_, main_frame_token_, device_id);

    // Retain a ref to this sink to ensure it is not destructed while this occurs.
    // The callback is guaranteed to execute on this thread, so there are no
    // threading issues.
    new_sink->GetOutputDeviceInfoAsync(base::BindOnce(&AudioRendererMixerInput::OnDeviceSwitchReady, base::RetainedRef(this), std::move(callback), new_sink));
}

double AudioRendererMixerInput::ProvideInput(media::AudioBus* audio_bus, uint32_t frames_delayed, const media::AudioGlitchInfo& glitch_info)
{
    TRACE_EVENT("audio", "AudioRendererMixerInput::ProvideInput", "delay (frames)", frames_delayed);
    const base::TimeDelta delay = media::AudioTimestampHelper::FramesToTime(frames_delayed, params_.sample_rate());

    int frames_filled = callback_->Render(delay, base::TimeTicks::Now(), glitch_info, audio_bus);

    // AudioConverter expects unfilled frames to be zeroed.
    if (frames_filled < audio_bus->frames()) {
        audio_bus->ZeroFramesPartial(frames_filled, audio_bus->frames() - frames_filled);
    }
...
void AudioRendererMixerInput::OnDeviceInfoReceived(OutputDeviceInfoCB info_cb, media::OutputDeviceInfo device_info)
{
    DCHECK(godia_in_progress_);
    ...
    SwitchOutputDevice(std::move(pending_device_id_), std::move(pending_switch_cb_));
}

void AudioRendererMixerInput::OnDeviceSwitchReady(
    media::OutputDeviceStatusCB switch_cb, scoped_refptr<media::AudioRendererSink> sink, media::OutputDeviceInfo device_info)
{
    DCHECK(switch_output_device_in_progress_);
