...
public:
    AudioRendererSinkCacheTest()
        ...
        , cache_(std::make_unique<AudioRendererSinkCache>(
              task_runner_, base::BindRepeating(&AudioRendererSinkCacheTest::CreateSink, base::Unretained(this)), kDeleteTimeout))
    {
    }

    AudioRendererSinkCacheTest(const AudioRendererSinkCacheTest&) = delete;
    AudioRendererSinkCacheTest& operator=(const AudioRendererSinkCacheTest&) = delete;
...
protected:
    ...
    scoped_refptr<media::AudioRendererSink> CreateSink(const LocalFrameToken& frame_token, const std::string& device_id)
    {
    ...
    void ExpectNotToStop(media::AudioRendererSink* sink)
    {
        ...
        EXPECT_CALL(*static_cast<media::MockAudioRendererSink*>(sink), Stop()).Times(0);
    }

    // Posts the task to the specified thread and runs current message loop until
    // the task is completed.
    void PostAndWaitUntilDone(const base::Thread& thread, base::OnceClosure task)
    {
        base::WaitableEvent e { base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED };

        thread.task_runner()->PostTask(FROM_HERE, std::move(task));
        thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&e)));
...
TEST_F(AudioRendererSinkCacheTest, GetDeviceInfo)
{
    ...
    media::OutputDeviceInfo device_info = cache_->GetSinkInfo(kFrameToken, kDefaultDeviceId);
    EXPECT_EQ(1u, sink_count());

    // The info on the same device is requested, so no new sink is created.
    media::OutputDeviceInfo one_more_device_info = cache_->GetSinkInfo(kFrameToken, kDefaultDeviceId);
    EXPECT_EQ(1u, sink_count());
...
TEST_F(AudioRendererSinkCacheTest, GarbageCollection)
{
    ...
    media::OutputDeviceInfo another_device_info = cache_->GetSinkInfo(kFrameToken, kAnotherDeviceId);
...
{
    EXPECT_EQ(0u, sink_count());
    media::OutputDeviceInfo device_info = cache_->GetSinkInfo(kFrameToken, kUnhealthyDeviceId);
    EXPECT_EQ(0u, sink_count());
}
...
{
    scoped_refptr<media::MockAudioRendererSink> sink = new media::MockAudioRendererSink(kUnhealthyDeviceId, media::OUTPUT_DEVICE_STATUS_ERROR_INTERNAL);

    cache_.reset(); // Destruct first so there's only one cache at a time.
    cache_ = std::make_unique<AudioRendererSinkCache>(task_runner_,
        base::BindRepeating(
            [](scoped_refptr<media::AudioRendererSink> sink, const LocalFrameToken& frame_token, const std::string& device_id) {
...
TEST_F(AudioRendererSinkCacheTest, MultithreadedAccess)
{
    ...
    PostAndWaitUntilDone(
        thread1, base::BindOnce(base::IgnoreResult(&AudioRendererSinkCache::GetSinkInfo), base::Unretained(cache_.get()), kFrameToken, kDefaultDeviceId));

    EXPECT_EQ(1u, sink_count());

    // Request the device information again on the second thread.
    PostAndWaitUntilDone(
        thread2, base::BindOnce(base::IgnoreResult(&AudioRendererSinkCache::GetSinkInfo), base::Unretained(cache_.get()), kFrameToken, kDefaultDeviceId));
}
