...
class BLINK_MODULES_EXPORT AudioRendererMixer : public media::AudioRendererSink::RenderCallback {
public:
    AudioRendererMixer(const media::AudioParameters& output_params, scoped_refptr<media::AudioRendererSink> sink);
    ...
    void AddMixerInput(const media::AudioParameters& input_params, media::AudioConverter::InputCallback* input);
    void RemoveMixerInput(const media::AudioParameters& input_params, media::AudioConverter::InputCallback* input);
...
private:
    ...
    base::flat_set<raw_ptr<AudioRendererMixerInput, CtnExperimental>> error_callbacks_ GUARDED_BY(lock_);

    // Maps input sample rate to the dedicated converter.
    using AudioConvertersMap = base::flat_map<int, std::unique_ptr<media::LoopbackAudioConverter>>;
