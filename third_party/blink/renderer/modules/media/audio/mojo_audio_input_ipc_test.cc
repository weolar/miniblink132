...
{
    return media::AudioSourceParameters(base::UnguessableToken::CreateForTesting(1234, 5678));
}

media::AudioSourceParameters SourceParamsWithProcessing()
{
    media::AudioSourceParameters params(base::UnguessableToken::CreateForTesting(1234, 5678));
...
class MockAudioProcessorControls : public media::mojom::blink::AudioProcessorControls {
public:
...
public:
    ...
    void OnStreamCreated(base::ReadOnlySharedMemoryRegion mem_handle, base::SyncSocket::ScopedHandle socket_handle, bool initially_muted) override
    {
...
public:
    FakeStreamCreator(media::mojom::blink::AudioInputStream* stream, media::mojom::blink::AudioProcessorControls* controls, bool initially_muted,
    ...
    void Create(const media::AudioSourceParameters& source_params, mojo::PendingRemote<mojom::blink::RendererAudioInputStreamFactoryClient> factory_client,
        mojo::PendingReceiver<media::mojom::blink::AudioProcessorControls> pending_controls_receiver, const media::AudioParameters& params,
        bool automatic_gain_control, uint32_t total_segments)
    {
        ...
        factory_client_->StreamCreated(receiver_.BindNewPipeAndPassRemote(), stream_client_.BindNewPipeAndPassReceiver(),
            { std::in_place, base::ReadOnlySharedMemoryRegion::Create(kMemoryLength).region, mojo::PlatformHandle(foreign_socket.Take()) }, initially_muted_,
            base::UnguessableToken::Create());
    }

    MojoAudioInputIPC::StreamCreatorCB GetCallback()
    {
        return base::BindRepeating(&FakeStreamCreator::Create, base::Unretained(this));
    }
...
private:
    ...
    mojo::Remote<mojom::blink::RendererAudioInputStreamFactoryClient> factory_client_;
    mojo::Receiver<media::mojom::blink::AudioInputStream> receiver_;
    mojo::Receiver<media::mojom::blink::AudioProcessorControls> controls_receiver_;
    bool initially_muted_;
    bool expect_processing_config_;
    base::CancelableSyncSocket socket_;
};

void AssociateOutputForAec(const base::UnguessableToken& stream_id, const std::string& output_device_id)
{
...
TEST(MojoAudioInputIPC, OnStreamCreated_Propagates)
{
    ...
    const std::unique_ptr<media::AudioInputIPC> ipc
        = std::make_unique<MojoAudioInputIPC>(SourceParams(), creator.GetCallback(), base::BindRepeating(&AssociateOutputForAec));
...
TEST(MojoAudioInputIPC, OnStreamCreated_Propagates_WithProcessingConfig)
{
    ...
    const std::unique_ptr<media::AudioInputIPC> ipc
        = std::make_unique<MojoAudioInputIPC>(SourceParamsWithProcessing(), creator.GetCallback(), base::BindRepeating(&AssociateOutputForAec));
...
TEST(MojoAudioInputIPC, FactoryDisconnected_SendsError)
{
    ...
    const std::unique_ptr<media::AudioInputIPC> ipc = std::make_unique<MojoAudioInputIPC>(SourceParams(),
        base::BindRepeating([](const media::AudioSourceParameters&, mojo::PendingRemote<mojom::blink::RendererAudioInputStreamFactoryClient> factory_client,
                                mojo::PendingReceiver<media::mojom::blink::AudioProcessorControls> controls_receiver, const media::AudioParameters& params,
                                bool automatic_gain_control, uint32_t total_segments) {}),
        base::BindRepeating(&AssociateOutputForAec));

    EXPECT_CALL(delegate, OnError(media::AudioCapturerSource::ErrorCode::kUnknown));
...
TEST(MojoAudioInputIPC, Controls_NotCalled_BeforeStreamCreated_WithoutProcessing)
{
    test::TaskEnvironment task_environment;
    StrictMock<MockStream> stream;
    StrictMock<MockAudioProcessorControls> controls;
...
TEST(MojoAudioInputIPC, Controls_NotCalled_AfterStreamCreated_WithoutProcessing)
{
    test::TaskEnvironment task_environment;
    StrictMock<MockStream> stream;
    StrictMock<MockAudioProcessorControls> controls;
...
TEST(MojoAudioInputIPC, Controls_Called_AfterStreamCreated_WithProcessing)
{
    ...
    media_controls->SetPreferredNumCaptureChannels(1);
    media_controls->GetStats(base::BindOnce([](const media::AudioProcessingStats& stats) {}));
    base::RunLoop().RunUntilIdle();
