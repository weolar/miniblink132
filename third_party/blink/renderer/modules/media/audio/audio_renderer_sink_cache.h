...
public:
    ...
    using CreateSinkCallback
        = base::RepeatingCallback<scoped_refptr<media::AudioRendererSink>(const LocalFrameToken& frame_token, const std::string& device_id)>;
    ...
    AudioRendererSinkCache(
        scoped_refptr<base::SequencedTaskRunner> cleanup_task_runner, CreateSinkCallback create_sink_callback, base::TimeDelta delete_timeout);
    ...
    media::OutputDeviceInfo GetSinkInfo(const LocalFrameToken& source_frame_token, const std::string& device_id);
...
private:
    ...
    CacheContainer::iterator FindCacheEntry_Locked(const LocalFrameToken& source_frame_token, const std::string& device_id);

    void MaybeCacheSink(const LocalFrameToken& source_frame_token, const std::string& device_id, scoped_refptr<media::AudioRendererSink> sink);
