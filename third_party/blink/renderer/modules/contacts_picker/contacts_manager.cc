...
template <> struct TypeConverter<blink::ContactInfo*, blink::mojom::blink::ContactInfoPtr> {
    static blink::ContactInfo* Convert(const blink::mojom::blink::ContactInfoPtr& contact);
};

blink::ContactInfo* TypeConverter<blink::ContactInfo*, blink::mojom::blink::ContactInfoPtr>::Convert(const blink::mojom::blink::ContactInfoPtr& contact)
{
    ...
    if (contact->address) {
        ...
        for (auto& address : *contact->address) {
            auto* blink_address = blink::MakeGarbageCollected<blink::ContactAddress>(std::move(address));
...
mojom::blink::ContactsManager* ContactsManager::GetContactsManager(ScriptState* script_state)
{
    if (!contacts_manager_.is_bound()) {
        ExecutionContext::From(script_state)
            ->GetBrowserInterfaceBroker()
            .GetInterface(contacts_manager_.BindNewPipeAndPassReceiver(ExecutionContext::From(script_state)->GetTaskRunner(TaskType::kMiscPlatformAPI)));
    }
    return contacts_manager_.get();
}

const Vector<V8ContactProperty>& ContactsManager::GetProperties(ScriptState* script_state)
{
    if (properties_.empty()) {
        properties_ = { V8ContactProperty(V8ContactProperty::Enum::kEmail), V8ContactProperty(V8ContactProperty::Enum::kName),
            V8ContactProperty(V8ContactProperty::Enum::kTel) };

        if (RuntimeEnabledFeatures::ContactsManagerExtraPropertiesEnabled(ExecutionContext::From(script_state))) {
            properties_.push_back(V8ContactProperty(V8ContactProperty::Enum::kAddress));
...
ScriptPromise<IDLSequence<ContactInfo>> ContactsManager::select(
    ScriptState* script_state, const Vector<V8ContactProperty>& properties, ContactsSelectOptions* options, ExceptionState& exception_state)
{
    LocalFrame* frame = script_state->ContextIsValid() ? LocalDOMWindow::From(script_state)->GetFrame() : nullptr;

    if (!frame || !frame->IsOutermostMainFrame()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The contacts API can only be used in the top frame");
        return ScriptPromise<IDLSequence<ContactInfo>>();
    }

    if (!LocalFrame::HasTransientUserActivation(frame)) {
        exception_state.ThrowSecurityError("A user gesture is required to call this method");
        return ScriptPromise<IDLSequence<ContactInfo>>();
    }
    ...
    if (contact_picker_in_use_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Contacts Picker is already in use.");
        return ScriptPromise<IDLSequence<ContactInfo>>();
    }
    ...
    for (const auto& property : properties) {
        if (!RuntimeEnabledFeatures::ContactsManagerExtraPropertiesEnabled(execution_context)
            && (property == V8ContactProperty::Enum::kAddress || property == V8ContactProperty::Enum::kIcon)) {
            exception_state.ThrowTypeError("The provided value '" + property.AsString() + "' is not a valid enum value of type ContactProperty");
            return ScriptPromise<IDLSequence<ContactInfo>>();
        }
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<ContactInfo>>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();

    contact_picker_in_use_ = true;
    GetContactsManager(script_state)
        ->Select(options->multiple(), include_names, include_emails, include_tel, include_addresses, include_icons,
            WTF::BindOnce(&ContactsManager::OnContactsSelected, WrapPersistent(this), WrapPersistent(resolver)));

    return promise;
}

void ContactsManager::OnContactsSelected(
    ScriptPromiseResolver<IDLSequence<ContactInfo>>* resolver, std::optional<Vector<mojom::blink::ContactInfoPtr>> contacts)
{
    ...
    if (!contacts.has_value()) {
        resolver->Reject(V8ThrowException::CreateTypeError(script_state->GetIsolate(), "Unable to open a contact selector"));
        return;
    }
...
ScriptPromise<IDLSequence<V8ContactProperty>> ContactsManager::getProperties(ScriptState* script_state)
{
    return ToResolvedPromise<IDLSequence<V8ContactProperty>>(script_state, GetProperties(script_state));
}
