...
public:
    ...
    void SetBodyContentFromFuzzer(const uint8_t* data, size_t size)
    {
        ...
        GetDocument().documentElement()->setInnerHTML(String::FromUTF8(body_content));
    ...
    {
        DCHECK(!platform_);
        platform_ = std::make_unique<ScopedTestingPlatformSupport<TestingPlatformSupportWithMockScheduler>>();
    }
    const base::TickClock* GetTickClock()
    {
        return platform_ ? platform()->test_task_runner()->GetMockTickClock() : base::DefaultTickClock::GetInstance();
    }
...
private:
    ...
    std::unique_ptr<ScopedTestingPlatformSupport<TestingPlatformSupportWithMockScheduler>> platform_;
