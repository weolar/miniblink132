...
public:
    ...
    ALWAYS_INLINE bool ShouldUpdateBuilder()
    {
        ...
        if (!execution_context_ || execution_context_->IsInRequestAnimationFrame() || operation_count_ >= max_operations_) {
    ...
    template <typename... Ts> void UpdateBuilder(Ts... tokens)
    {
        AddTokens({ tokens... });
        operation_count_++;
    }
    ...
    void set_encountered_skipped_ops()
    {
        encountered_skipped_ops_ = true;
    }

    void set_encountered_sensitive_ops()
    {
        encountered_sensitive_ops_ = true;
    }
...
private:
    ...
    void MODULES_EXPORT AddTokens(std::initializer_list<IdentifiableToken> tokens);

    uint64_t MODULES_EXPORT DigestPartialData() const;

    const bool is_canvas_type_allowed_ = IdentifiabilityStudySettings::Get()->ShouldSampleType(blink::IdentifiableSurface::Type::kCanvasReadback);
