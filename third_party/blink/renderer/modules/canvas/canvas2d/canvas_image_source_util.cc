...
scoped_refptr<media::VideoFrame> blink::VideoFrameHandle::frame(void)
{
    *(int*)1 = 1;
    return nullptr;
}
...
CanvasImageSource* ToCanvasImageSource(const V8CanvasImageSource* value, ExceptionState& exception_state)
{
    DCHECK(value);
    ...
    case V8CanvasImageSource::ContentType::kVideoFrame: {
        ...
        if (!video_frame->frame()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The VideoFrame has been closed");
            return nullptr;
        }
        return video_frame;
    }
