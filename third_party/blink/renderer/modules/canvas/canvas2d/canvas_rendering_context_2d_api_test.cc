...
{
    PageTestBase::SetUp();
    GetDocument().documentElement()->setInnerHTML("<body><canvas id='c'></canvas></body>");
...
TEST_F(CanvasRenderingContext2DAPITest, SetShadowColor_Clamping)
{
    ...
    Context2D()->setShadowColor("rgb(0,"
    ...
    Context2D()->setShadowColor("rgba(7,"
                                "999999999999999999999999999999999999999999999999999999999999999999999999"
                                "9999999999,0,"
                                "999999999999999999999999999999999999999999999999999999999999999999999999"
                                "999999999999999999999999999999999999999999999999999999999999999999999999"
...
String TrySettingStrokeStyle(CanvasRenderingContext2DAPITest* test, const String& value)
{
...
String TrySettingFillStyle(CanvasRenderingContext2DAPITest* test, const String& value)
{
...
void TrySettingColor(CanvasRenderingContext2DAPITest* test, const String& value, const String& expected)
{
...
TEST_F(CanvasRenderingContext2DAPITest, DefaultAttributeValues)
{
    ...
    EXPECT_EQ(String("#000000"), GetStrokeStyleAsString(Context2D(), GetScriptState()));

    EXPECT_EQ(String("#000000"), GetFillStyleAsString(Context2D(), GetScriptState()));
...
TEST_F(CanvasRenderingContext2DAPITest, CreateImageData)
{
    ...
    ImageData* image_data = Context2D()->createImageData(100, 50, settings, exception_state);
    EXPECT_FALSE(exception_state.HadException());
    EXPECT_EQ(100, image_data->width());
    EXPECT_EQ(50, image_data->height());

    for (size_t i = 0; i < image_data->data()->GetAsUint8ClampedArray()->length(); ++i) {
    ...
    ImageData* same_size_image_data = Context2D()->createImageData(image_data, exception_state);
    EXPECT_FALSE(exception_state.HadException());
    EXPECT_EQ(100, same_size_image_data->width());
    EXPECT_EQ(50, same_size_image_data->height());
    EXPECT_EQ(0, same_size_image_data->data()->GetAsUint8ClampedArray()->Data()[32]);

    // createImageData(width, height) takes the absolute magnitude of the size
    // arguments

    ImageData* imgdata1 = Context2D()->createImageData(10, 20, settings, exception_state);
    EXPECT_FALSE(exception_state.HadException());
    ImageData* imgdata2 = Context2D()->createImageData(-10, 20, settings, exception_state);
    EXPECT_FALSE(exception_state.HadException());
    ImageData* imgdata3 = Context2D()->createImageData(10, -20, settings, exception_state);
    EXPECT_FALSE(exception_state.HadException());
    ImageData* imgdata4 = Context2D()->createImageData(-10, -20, settings, exception_state);
    EXPECT_FALSE(exception_state.HadException());
...
TEST_F(CanvasRenderingContext2DAPITest, CreateImageDataTooBig)
{
    ...
    ImageDataSettings* settings = ImageDataSettings::Create();
    ImageData* too_big_image_data = Context2D()->createImageData(1000000, 1000000, settings, exception_state);
...
TEST_F(CanvasRenderingContext2DAPITest, GetImageDataTooBig)
{
    ...
    DummyExceptionStateForTesting exception_state;
    ImageDataSettings* settings = ImageDataSettings::Create();
    ImageData* image_data = Context2D()->getImageData(0, 0, 1000000, 1000000, settings, exception_state);
    EXPECT_EQ(nullptr, image_data);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(ESErrorType::kRangeError, exception_state.CodeAs<ESErrorType>());
}

TEST_F(CanvasRenderingContext2DAPITest, GetImageDataIntegerOverflowNegativeParams)
{
    CreateContext(kNonOpaque);
    ImageDataSettings* settings = ImageDataSettings::Create();
    {
        DummyExceptionStateForTesting exception_state;
        ImageData* image_data = Context2D()->getImageData(1, -2147483647, 1, -2147483647, settings, exception_state);
        EXPECT_EQ(nullptr, image_data);
        EXPECT_TRUE(exception_state.HadException());
        EXPECT_EQ(ESErrorType::kRangeError, exception_state.CodeAs<ESErrorType>());
    }
    ...
    {
        DummyExceptionStateForTesting exception_state;
        ImageData* image_data = Context2D()->getImageData(-2147483647, 1, -2147483647, 1, settings, exception_state);
        EXPECT_EQ(nullptr, image_data);
        EXPECT_TRUE(exception_state.HadException());
        EXPECT_EQ(ESErrorType::kRangeError, exception_state.CodeAs<ESErrorType>());
    }
...
TEST_F(CanvasRenderingContext2DAPITest, UnclosedLayerCreateImageBitmap)
{
    ...
    DummyExceptionStateForTesting exception_state;
    ImageBitmapFactories::CreateImageBitmap(GetScriptState(), image, options, exception_state);
    EXPECT_EQ(exception_state.CodeAs<DOMExceptionCode>(), DOMExceptionCode::kInvalidStateError);

    Context2D()->endLayer(no_exception);

    // Doesn't throw outside layers:
    ImageBitmapFactories::CreateImageBitmap(GetScriptState(), image, options, no_exception);
}
...
TEST_F(CanvasRenderingContext2DAPITest, UnclosedLayerGetImageData)
{
    ...
    // Throws inside layers:
    DummyExceptionStateForTesting exception_state;
    Context2D()->getImageData(/*sx=*/0, /*sy=*/0, /*sw=*/1, /*sh=*/1, exception_state);
    EXPECT_EQ(exception_state.CodeAs<DOMExceptionCode>(), DOMExceptionCode::kInvalidStateError);

    Context2D()->endLayer(no_exception);

    // Doesn't throw outside layers:
    Context2D()->getImageData(/*sx=*/0, /*sy=*/0, /*sw=*/1, /*sh=*/1, no_exception);
}
...
TEST_F(CanvasRenderingContext2DAPITest, UnclosedLayerPutImageData)
{
    ...
    NonThrowableExceptionState no_exception;
    ImageData* image_data = ImageData::Create(Context2D()->Width(), Context2D()->Height(), no_exception);
...
TEST_F(CanvasRenderingContext2DAPITest, UnclosedLayerDrawMesh)
{
    ...
    const Mesh2DVertexBuffer* vbuf = MakeGarbageCollected<Mesh2DVertexBuffer>(
        base::MakeRefCounted<cc::RefCountedBuffer<SkPoint>>(std::vector<SkPoint> { { 0, 0 }, { 100, 0 }, { 100, 100 } }));
    const Mesh2DUVBuffer* uvbuf
        = MakeGarbageCollected<Mesh2DUVBuffer>(base::MakeRefCounted<cc::RefCountedBuffer<SkPoint>>(std::vector<SkPoint> { { 0, 0 }, { 1, 0 }, { 1, 1 } }));
    const Mesh2DIndexBuffer* ibuf
        = MakeGarbageCollected<Mesh2DIndexBuffer>(base::MakeRefCounted<cc::RefCountedBuffer<uint16_t>>(std::vector<uint16_t> { 0, 1, 2 }));
...
void ResetCanvasForAccessibilityRectTest(Document& document)
{
    ...
    auto* canvas = To<HTMLCanvasElement>(document.getElementById(AtomicString("canvas")));
...
public:
    explicit ActiveSettingsProvider(bool enabled)
        : enabled_(enabled)
    {
    }
...
public:
    ...
    {
        IdentifiabilityStudySettings::SetGlobalProvider(std::make_unique<ActiveSettingsProvider>(enabled));
    }
...
TEST_F(CanvasRenderingContext2DAPITest, IdentifiabilityStudyMaxOperations)
{
    ...
    IdentifiabilityStudyHelper::ScopedMaxOperationsSetter max_operations_setter(kMaxOperations);
    CreateContext(kNonOpaque);

    int64_t last_digest = INT64_C(0);
    for (int i = 0; i < kMaxOperations; i++) {
        Context2D()->setFont("Arial");
        EXPECT_NE(last_digest, Context2D()->IdentifiableTextToken().ToUkmMetricValue()) << i;
        last_digest = Context2D()->IdentifiableTextToken().ToUkmMetricValue();
    }

    Context2D()->setFont("Arial");
    EXPECT_EQ(last_digest, Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
#define MAYBE_IdentifiabilityStudyDigest_Font DISABLED_IdentifiabilityStudyDigest_Font
#else
...
TEST_F(CanvasRenderingContext2DAPITest, MAYBE_IdentifiabilityStudyDigest_Font)
{
    ...
    Context2D()->setFont("Arial");
    EXPECT_EQ(INT64_C(7339381412423806682), Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
TEST_F(CanvasRenderingContext2DAPITest, IdentifiabilityStudyDisabled)
{
    ...
    Context2D()->setFont("Arial");
    EXPECT_EQ(kTokenBuilderInitialDigest, Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
// TODO(crbug.com/1258605): Fix test on Windows and re-enable.
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)
#define MAYBE_IdentifiabilityStudyDigest_StrokeText DISABLED_IdentifiabilityStudyDigest_StrokeText
#else
#define MAYBE_IdentifiabilityStudyDigest_StrokeText IdentifiabilityStudyDigest_StrokeText
#endif // BUILDFLAG(IS_ANDROID)

TEST_F(CanvasRenderingContext2DAPITest, MAYBE_IdentifiabilityStudyDigest_StrokeText)
{
    StudyParticipationRaii study_participation_raii;
    CreateContext(kNonOpaque);

    Context2D()->strokeText("Sensitive message", 1.0, 1.0);
    EXPECT_EQ(INT64_C(8218678546639211996), Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
// TODO(crbug.com/1258605): Fix test on Windows and re-enable.
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)
#define MAYBE_IdentifiabilityStudyDigest_FillText DISABLED_IdentifiabilityStudyDigest_FillText
#else
#define MAYBE_IdentifiabilityStudyDigest_FillText IdentifiabilityStudyDigest_FillText
#endif // BUILDFLAG(IS_ANDROID)

TEST_F(CanvasRenderingContext2DAPITest, MAYBE_IdentifiabilityStudyDigest_FillText)
{
    StudyParticipationRaii study_participation_raii;
    CreateContext(kNonOpaque);

    Context2D()->fillText("Sensitive message", 1.0, 1.0);
    EXPECT_EQ(INT64_C(-7525055925911674050), Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
// TODO(crbug.com/1258605): Fix test on Windows and re-enable.
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)
#define MAYBE_IdentifiabilityStudyDigest_TextAlign DISABLED_IdentifiabilityStudyDigest_TextAlign
#else
#define MAYBE_IdentifiabilityStudyDigest_TextAlign IdentifiabilityStudyDigest_TextAlign
#endif // BUILDFLAG(IS_ANDROID)

TEST_F(CanvasRenderingContext2DAPITest, MAYBE_IdentifiabilityStudyDigest_TextAlign)
{
    StudyParticipationRaii study_participation_raii;
    CreateContext(kNonOpaque);

    Context2D()->setTextAlign("center");
    EXPECT_EQ(INT64_C(-5618040280239325003), Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
// TODO(crbug.com/1258605): Fix test on Windows and re-enable.
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)
#define MAYBE_IdentifiabilityStudyDigest_TextBaseline DISABLED_IdentifiabilityStudyDigest_TextBaseline
#else
#define MAYBE_IdentifiabilityStudyDigest_TextBaseline IdentifiabilityStudyDigest_TextBaseline
#endif // BUILDFLAG(IS_ANDROID)

TEST_F(CanvasRenderingContext2DAPITest, MAYBE_IdentifiabilityStudyDigest_TextBaseline)
{
    StudyParticipationRaii study_participation_raii;
    CreateContext(kNonOpaque);

    Context2D()->setTextBaseline("top");
    EXPECT_EQ(INT64_C(-6814889525293785691), Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
// TODO(crbug.com/1258605): Fix test on Windows and re-enable.
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)
#define MAYBE_IdentifiabilityStudyDigest_StrokeStyle DISABLED_IdentifiabilityStudyDigest_StrokeStyle
#else
#define MAYBE_IdentifiabilityStudyDigest_StrokeStyle IdentifiabilityStudyDigest_StrokeStyle
#endif // BUILDFLAG(IS_ANDROID)

TEST_F(CanvasRenderingContext2DAPITest, MAYBE_IdentifiabilityStudyDigest_StrokeStyle)
{
    v8::HandleScope handle_scope(GetIsolate());
    StudyParticipationRaii study_participation_raii;
    CreateContext(kNonOpaque);
    ...
    EXPECT_EQ(INT64_C(3577524355478740727), Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
// TODO(crbug.com/1258605): Fix test on Windows and re-enable.
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)
#define MAYBE_IdentifiabilityStudyDigest_FillStyle DISABLED_IdentifiabilityStudyDigest_FillStyle
#else
#define MAYBE_IdentifiabilityStudyDigest_FillStyle IdentifiabilityStudyDigest_FillStyle
#endif // BUILDFLAG(IS_ANDROID)

TEST_F(CanvasRenderingContext2DAPITest, MAYBE_IdentifiabilityStudyDigest_FillStyle)
{
    v8::HandleScope handle_scope(GetIsolate());
    StudyParticipationRaii study_participation_raii;
    CreateContext(kNonOpaque);
    ...
    EXPECT_EQ(INT64_C(7953663110297373742), Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
// TODO(crbug.com/1258605): Fix test on Windows and re-enable.
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)
#define MAYBE_IdentifiabilityStudyDigest_Combo DISABLED_IdentifiabilityStudyDigest_Combo
#else
#define MAYBE_IdentifiabilityStudyDigest_Combo IdentifiabilityStudyDigest_Combo
#endif // BUILDFLAG(IS_ANDROID)

TEST_F(CanvasRenderingContext2DAPITest, MAYBE_IdentifiabilityStudyDigest_Combo)
{
    v8::HandleScope handle_scope(GetIsolate());
    StudyParticipationRaii study_participation_raii;
    CreateContext(kNonOpaque);
    ...
    EXPECT_EQ(INT64_C(-7631959002534825456), Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
// TODO(crbug.com/1258605): Fix test on Windows and re-enable.
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)
#define MAYBE_IdentifiabilityStudyDigest_putImageData DISABLED_IdentifiabilityStudyDigest_putImageData
#else
#define MAYBE_IdentifiabilityStudyDigest_putImageData IdentifiabilityStudyDigest_putImageData
#endif // BUILDFLAG(IS_ANDROID)

TEST_F(CanvasRenderingContext2DAPITest, MAYBE_IdentifiabilityStudyDigest_putImageData)
{
    StudyParticipationRaii study_participation_raii;
    CreateContext(kNonOpaque);
    NonThrowableExceptionState exception_state;

    ImageData* image_data = Context2D()->createImageData(/*sw=*/1, /*sh=*/1, exception_state);
    EXPECT_FALSE(exception_state.HadException());
    Context2D()->putImageData(image_data, /*dx=*/1, /*dy=*/1, exception_state);
    EXPECT_EQ(INT64_C(2821795876044191773), Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
// TODO(crbug.com/1258605): Fix test on Windows and re-enable.
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)
#define MAYBE_IdentifiabilityStudyDigest_drawImage DISABLED_IdentifiabilityStudyDigest_drawImage
#else
#define MAYBE_IdentifiabilityStudyDigest_drawImage IdentifiabilityStudyDigest_drawImage
#endif // BUILDFLAG(IS_ANDROID)

TEST_F(CanvasRenderingContext2DAPITest, MAYBE_IdentifiabilityStudyDigest_drawImage)
{
    StudyParticipationRaii study_participation_raii;
    CreateContext(kNonOpaque);
    NonThrowableExceptionState exception_state;
    ...
    auto* image_source = MakeGarbageCollected<V8CanvasImageSource>(&CanvasElement());
    Context2D()->drawImage(image_source, /*x=*/1,
        /*y=*/1, exception_state);
    EXPECT_EQ(INT64_C(-4851825694092845811), Context2D()->IdentifiableTextToken().ToUkmMetricValue());
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DVertexBuffer0Floats)
{
    CreateContext(kNonOpaque);
    DummyExceptionStateForTesting exception_state;
    EXPECT_THAT(Context2D()->createMesh2DVertexBuffer(CreateFloat32Array({}), exception_state), IsNull());
    EXPECT_TRUE(exception_state.HadException());
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DVertexBuffer1Float)
{
    CreateContext(kNonOpaque);
    DummyExceptionStateForTesting exception_state;
    EXPECT_THAT(Context2D()->createMesh2DVertexBuffer(CreateFloat32Array({ 101 }), exception_state), IsNull());
    EXPECT_TRUE(exception_state.HadException());
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DVertexBuffer2Floats)
{
    ...
    NonThrowableExceptionState exception_state;
    EXPECT_THAT(
        Context2D()->createMesh2DVertexBuffer(CreateFloat32Array({ 101, 102 }), exception_state), Pointee(Mesh2dBufferIs(ElementsAre(SkPoint(101, 102)))));
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DVertexBuffer3Floats)
{
    CreateContext(kNonOpaque);
    DummyExceptionStateForTesting exception_state;
    EXPECT_THAT(Context2D()->createMesh2DVertexBuffer(CreateFloat32Array({ 101, 102, 103 }), exception_state), IsNull());
    EXPECT_TRUE(exception_state.HadException());
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DVertexBuffer4Floats)
{
    CreateContext(kNonOpaque);
    NonThrowableExceptionState exception_state;
    EXPECT_THAT(Context2D()->createMesh2DVertexBuffer(CreateFloat32Array({ 101, 102, 103, 104 }), exception_state),
        Pointee(Mesh2dBufferIs(ElementsAre(SkPoint(101, 102), SkPoint(103, 104)))));
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DUVBuffer0Floats)
{
    CreateContext(kNonOpaque);
    DummyExceptionStateForTesting exception_state;
    EXPECT_THAT(Context2D()->createMesh2DUVBuffer(CreateFloat32Array({}), exception_state), IsNull());
    EXPECT_TRUE(exception_state.HadException());
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DUVBuffer1Float)
{
    CreateContext(kNonOpaque);
    DummyExceptionStateForTesting exception_state;
    EXPECT_THAT(Context2D()->createMesh2DUVBuffer(CreateFloat32Array({ 101 }), exception_state), IsNull());
    EXPECT_TRUE(exception_state.HadException());
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DUVBuffer2Floats)
{
    CreateContext(kNonOpaque);
    NonThrowableExceptionState exception_state;
    EXPECT_THAT(Context2D()->createMesh2DUVBuffer(CreateFloat32Array({ 101, 102 }), exception_state), Pointee(Mesh2dBufferIs(ElementsAre(SkPoint(101, 102)))));
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DUVBuffer3Floats)
{
    CreateContext(kNonOpaque);
    DummyExceptionStateForTesting exception_state;
    EXPECT_THAT(Context2D()->createMesh2DUVBuffer(CreateFloat32Array({ 101, 102, 103 }), exception_state), IsNull());
    EXPECT_TRUE(exception_state.HadException());
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DUVBuffer4Floats)
{
    CreateContext(kNonOpaque);
    NonThrowableExceptionState exception_state;
    EXPECT_THAT(Context2D()->createMesh2DUVBuffer(CreateFloat32Array({ 101, 102, 103, 104 }), exception_state),
        Pointee(Mesh2dBufferIs(ElementsAre(SkPoint(101, 102), SkPoint(103, 104)))));
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DIndexBuffer0Uints)
{
    CreateContext(kNonOpaque);
    DummyExceptionStateForTesting exception_state;
    EXPECT_THAT(Context2D()->createMesh2DIndexBuffer(CreateUint16Array({}), exception_state), IsNull());
    EXPECT_TRUE(exception_state.HadException());
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DIndexBuffer1Uint)
{
    CreateContext(kNonOpaque);
    DummyExceptionStateForTesting exception_state;
    EXPECT_THAT(Context2D()->createMesh2DIndexBuffer(CreateUint16Array({ 1 }), exception_state), IsNull());
    EXPECT_TRUE(exception_state.HadException());
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DIndexBuffer2Uints)
{
    CreateContext(kNonOpaque);
    DummyExceptionStateForTesting exception_state;
    EXPECT_THAT(Context2D()->createMesh2DIndexBuffer(CreateUint16Array({ 1, 2 }), exception_state), IsNull());
    EXPECT_TRUE(exception_state.HadException());
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DUVBuffer3Uints)
{
    CreateContext(kNonOpaque);
    NonThrowableExceptionState exception_state;
    EXPECT_THAT(Context2D()->createMesh2DIndexBuffer(CreateUint16Array({ 1, 2, 3 }), exception_state), Pointee(Mesh2dBufferIs(ElementsAre(1, 2, 3))));
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DIndexBuffer4Uints)
{
    CreateContext(kNonOpaque);
    DummyExceptionStateForTesting exception_state;
    EXPECT_THAT(Context2D()->createMesh2DIndexBuffer(CreateUint16Array({ 1, 2, 3, 4 }), exception_state), IsNull());
    EXPECT_TRUE(exception_state.HadException());
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DIndexBuffer5Uints)
{
    CreateContext(kNonOpaque);
    DummyExceptionStateForTesting exception_state;
    EXPECT_THAT(Context2D()->createMesh2DIndexBuffer(CreateUint16Array({ 1, 2, 3, 4, 5 }), exception_state), IsNull());
    EXPECT_TRUE(exception_state.HadException());
}
...
TEST_F(CanvasRenderingContext2DAPITest, Mesh2DUVBuffer6Uints)
{
    ...
    NonThrowableExceptionState exception_state;
    EXPECT_THAT(
        Context2D()->createMesh2DIndexBuffer(CreateUint16Array({ 1, 2, 3, 4, 5, 6 }), exception_state), Pointee(Mesh2dBufferIs(ElementsAre(1, 2, 3, 4, 5, 6))));
}
...
TEST_F(CanvasRenderingContext2DAPITest, DrawMesh)
{
    ...
    V8CanvasImageSource* image_source = MakeGarbageCollected<V8CanvasImageSource>(&CanvasElement());

    DummyExceptionStateForTesting exception_state;
    const auto* vert_buffer = ctx->createMesh2DVertexBuffer(CreateFloat32Array({ 0, 0, 100, 0, 100, 100 }), exception_state);
    ASSERT_NE(vert_buffer, nullptr);

    const auto* uv_buffer = ctx->createMesh2DUVBuffer(CreateFloat32Array({ 0, 0, 1, 0, 1, 1 }), exception_state);
    ASSERT_NE(uv_buffer, nullptr);

    const auto* index_buffer = ctx->createMesh2DIndexBuffer(CreateUint16Array({ 0, 1, 2 }), exception_state);
    ASSERT_NE(index_buffer, nullptr);

    ASSERT_FALSE(exception_state.HadException());

    // valid call
    ctx->drawMesh(vert_buffer, uv_buffer, index_buffer, image_source, exception_state);
    EXPECT_FALSE(exception_state.HadException());
}
