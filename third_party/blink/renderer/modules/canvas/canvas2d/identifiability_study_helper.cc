...
void IdentifiabilityStudyHelper::AddTokens(std::initializer_list<IdentifiableToken> tokens)
{
...
uint64_t IdentifiabilityStudyHelper::DigestPartialData() const
{
    return base::legacy::CityHash64WithSeed(
        base::make_span(reinterpret_cast<const uint8_t*>(partial_.data()), reinterpret_cast<const uint8_t*>(partial_.data() + position_)), chaining_value_);
}
