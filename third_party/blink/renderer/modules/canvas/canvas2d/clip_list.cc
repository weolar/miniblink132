...
void ClipList::ClipPath(const SkPath& path, AntiAliasingMode anti_aliasing_mode, const SkMatrix& ctm)
{
    ClipOp new_clip;
...
void ClipList::Playback(cc::PaintCanvas* canvas) const
{
    for (const auto& clip : clip_list_) {
        canvas->clipPath(clip.path_, SkClipOp::kIntersect, clip.anti_aliasing_mode_ == kAntiAliased);
    }
}
...
ClipList::ClipOp::ClipOp()
    : anti_aliasing_mode_(kAntiAliased)
{
}
