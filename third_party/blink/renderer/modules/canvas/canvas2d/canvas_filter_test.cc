...
TEST_P(CanvasFilterTest, CreatesFilterOperations)
{
    ...
    EXPECT_THAT(CanvasFilter::CreateFilterOperations(CHECK_DEREF(ParseFilter(scope, GetParam().filter)), Font(),
                    /*style_resolution_host=*/nullptr, CHECK_DEREF(scope.GetExecutionContext()), scope.GetExceptionState())
                    .Operations(),
