...
{
    return ExplainMatchResult(testing::Eq(testing::ByRef(*matcher.Run())), *arg, result_listener);
}

template <typename T, typename... Args> auto GarbageCollectedIs(const Args&... args)
{
    return GarbageCollectedIsMatcher(base::BindRepeating([](const Args&... args) { return MakeGarbageCollected<T>(args...); }, args...));
}
