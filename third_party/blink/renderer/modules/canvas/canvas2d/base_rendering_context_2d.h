...
template <typename T> class NotShared;
...
public:
    ...
    void setStrokeStyle(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state);

    v8::Local<v8::Value> fillStyle(ScriptState* script_state) const;
    void setFillStyle(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state);
    ...
    void beginLayer(ScriptState* script_state, const BeginLayerOptions* options, ExceptionState& exception_state)
    {
    ...
    void stroke();
    void stroke(Path2D*);
    void clip(const V8CanvasFillRule& winding = V8CanvasFillRule(V8CanvasFillRule::Enum::kNonzero));
    void clip(Path2D*, const V8CanvasFillRule& winding = V8CanvasFillRule(V8CanvasFillRule::Enum::kNonzero));

    bool isPointInPath(const double x, const double y, const V8CanvasFillRule& winding = V8CanvasFillRule(V8CanvasFillRule::Enum::kNonzero));
    bool isPointInPath(Path2D*, const double x, const double y, const V8CanvasFillRule& winding = V8CanvasFillRule(V8CanvasFillRule::Enum::kNonzero));
    ...
    void placeElement(Element* element, double x, double y, ExceptionState& exception_state);
    void drawImage(const V8CanvasImageSource* image_source, double x, double y, ExceptionState& exception_state);
    void drawImage(const V8CanvasImageSource* image_source, double x, double y, double width, double height, ExceptionState& exception_state);
    void drawImage(const V8CanvasImageSource* image_source, double sx, double sy, double sw, double sh, double dx, double dy, double dw, double dh,
    ...
    CanvasPattern* createPattern(const V8CanvasImageSource* image_source, const String& repetition_type, ExceptionState& exception_state);
    CanvasPattern* createPattern(CanvasImageSource*, const String& repetition_type, ExceptionState&);

    Mesh2DVertexBuffer* createMesh2DVertexBuffer(NotShared<DOMFloat32Array>, ExceptionState&);
    Mesh2DUVBuffer* createMesh2DUVBuffer(NotShared<DOMFloat32Array>, ExceptionState&);
    Mesh2DIndexBuffer* createMesh2DIndexBuffer(NotShared<DOMUint16Array>, ExceptionState&);
    void drawMesh(const Mesh2DVertexBuffer* vertex_buffer, const Mesh2DUVBuffer* uv_buffer, const Mesh2DIndexBuffer* index_buffer,
        const V8CanvasImageSource* image, ExceptionState&);

    ImageData* createImageData(ImageData*, ExceptionState&) const;
    ImageData* createImageData(int sw, int sh, ExceptionState&) const;
    ImageData* createImageData(int sw, int sh, ImageDataSettings*, ExceptionState&) const;

    // For deferred canvases this will have the side effect of drawing recorded
    // commands in order to finalize the frame
    ImageData* getImageData(int sx, int sy, int sw, int sh, ExceptionState&);
    ImageData* getImageData(int sx, int sy, int sw, int sh, ImageDataSettings*, ExceptionState&);
    virtual ImageData* getImageDataInternal(int sx, int sy, int sw, int sh, ImageDataSettings*, ExceptionState&);

    void putImageData(ImageData*, int dx, int dy, ExceptionState&);
    void putImageData(ImageData*, int dx, int dy, int dirty_x, int dirty_y, int dirty_width, int dirty_height, ExceptionState&);
    ...
    // state.
    GPUTexture* transferToGPUTexture(const Canvas2dGPUTransferOption*, ExceptionState& exception_state);
    ...
    {
        return const_cast<cc::PaintCanvas*>(const_cast<const BaseRenderingContext2D*>(this)->GetPaintCanvas());
    }
    ...
    {
        return const_cast<MemoryManagedPaintRecorder*>(const_cast<const BaseRenderingContext2D*>(this)->Recorder());
    }

    // Called when about to draw. When this is called GetPaintCanvas() has already
    // been called and returned a non-null value.
    virtual void WillDraw(const SkIRect& dirty_rect, CanvasPerformanceMonitor::DrawType) = 0;
    ...
    HeapTaskRunnerTimer<BaseRenderingContext2D> dispatch_context_lost_event_timer_;
    HeapTaskRunnerTimer<BaseRenderingContext2D> dispatch_context_restored_event_timer_;
    HeapTaskRunnerTimer<BaseRenderingContext2D> try_restore_context_event_timer_;
    unsigned try_restore_context_attempt_count_ = 0;

protected:
    explicit BaseRenderingContext2D(scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    bool ComputeDirtyRect(const gfx::RectF& local_bounds, const SkIRect& transformed_clip_bounds, SkIRect*);

    template <OverdrawOp CurrentOverdrawOp, typename DrawFunc, typename DrawCoversClipBoundsFunc>
    void Draw(const DrawFunc&, const DrawCoversClipBoundsFunc&, const gfx::RectF& bounds, CanvasRenderingContext2DState::PaintType,
        CanvasRenderingContext2DState::ImageType, CanvasPerformanceMonitor::DrawType);
    ...
    virtual bool WritePixels(const SkImageInfo& orig_info, const void* pixels, size_t row_bytes, int x, int y)
    {
        NOTREACHED();
    }
    ...
    void CheckOverdraw(const cc::PaintFlags*, CanvasRenderingContext2DState::ImageType, BaseRenderingContext2D::OverdrawOp overdraw_op);
    ...
    CanvasRenderingContext::LostContextMode context_lost_mode_ { CanvasRenderingContext::kNotLostContext };

    // TODO(issues.chromium.org/issues/349835587): Add an observer to know if the
    // element is detached and then remove it.
    HeapHashMap<WeakMember<Element>, scoped_refptr<CanvasDeferredPaintRecord>> placed_elements_;

private:
    void FillImpl(SkPathFillType winding_rule);
    void FillPathImpl(Path2D* dom_path, SkPathFillType winding_rule);

    void DrawTextInternal(const String& text, double x, double y, CanvasRenderingContext2DState::PaintType paint_type, double* max_width = nullptr,
    ...
    bool ExtractColorFromV8StringAndUpdateCache(v8::Isolate* isolate, v8::Local<v8::String> v8_string, ExceptionState& exception_state, Color& color);
    ...
    void beginLayerImpl(ScriptState* script_state, const BeginLayerOptions* options, ExceptionState* exception_state);
    ...
    ALWAYS_INLINE bool ShouldUseDropShadowPaintFilter(
        CanvasRenderingContext2DState::PaintType paint_type, CanvasRenderingContext2DState::ImageType image_type) const
    {
        return (paint_type == CanvasRenderingContext2DState::kFillPaintType || paint_type == CanvasRenderingContext2DState::kStrokePaintType)
            && image_type == CanvasRenderingContext2DState::kNonOpaqueImage;
    }

    bool BlendModeRequiresCompositedDraw(const CanvasRenderingContext2DState& state) const;

    ALWAYS_INLINE bool ShouldUseCompositedDraw(CanvasRenderingContext2DState::PaintType paint_type, CanvasRenderingContext2DState::ImageType image_type)
    {
        ...
        if (StateHasFilter())
            return true;
        if (state.ShouldDrawShadows() && ShouldUseDropShadowPaintFilter(paint_type, image_type))
            return true;
        return false;
    }

    void ResetAlphaIfNeeded(cc::PaintCanvas* c, SkBlendMode blend_mode, const gfx::RectF* bounds = nullptr);

    // `paint_canvas` is null if this function is called asynchronously.
    template <OverdrawOp CurrentOverdrawOp, typename DrawFunc, typename DrawCoversClipBoundsFunc>
    void DrawInternal(cc::PaintCanvas* paint_canvas, const DrawFunc&, const DrawCoversClipBoundsFunc&, const gfx::RectF& bounds,
        CanvasRenderingContext2DState::PaintType, CanvasRenderingContext2DState::ImageType, const SkIRect& clip_bounds, CanvasPerformanceMonitor::DrawType);

    void DrawPathInternal(const CanvasPath&, CanvasRenderingContext2DState::PaintType, SkPathFillType, cc::UsePaintCache);
    void DrawImageInternal(
        cc::PaintCanvas*, CanvasImageSource*, Image*, const gfx::RectF& src_rect, const gfx::RectF& dst_rect, const SkSamplingOptions&, const cc::PaintFlags*);
    void ClipInternal(const Path&, const V8CanvasFillRule& winding_rule, cc::UsePaintCache);

    bool IsPointInPathInternal(const Path&, const double x, const double y, const V8CanvasFillRule& winding_rule);
    ...
    void CompositedDraw(const DrawFunc&, cc::PaintCanvas*, CanvasRenderingContext2DState::PaintType, CanvasRenderingContext2DState::ImageType);

    template <typename T> bool ValidateRectForCanvas(T x, T y, T width, T height);

    template <typename T> void AdjustRectForCanvas(T& x, T& y, T& width, T& height);
    ...
    void PutByteArray(const SkPixmap& source, const gfx::Rect& source_rect, const gfx::Vector2d& dest_offset);
    ...
    RespectImageOrientationEnum RespectImageOrientationInternal(CanvasImageSource*);

    // Updates the identifiability study before changing stroke or fill styles.
    void UpdateIdentifiabilityStudyBeforeSettingStrokeOrFill(const V8CanvasStyle& v8_style, CanvasOps op);
    void UpdateIdentifiabilityStudyBeforeSettingStrokeOrFill(v8::Local<v8::String> v8_string, CanvasOps op);

    // Parses the string as a color and returns the result of parsing.
    ColorParseResult ParseColorOrCurrentColor(const String& color_string, Color& color) const;
...
{
    return blendMode == SkBlendMode::kSrcOver || blendMode == SkBlendMode::kSrcATop || blendMode == SkBlendMode::kDstOut;
}

ALWAYS_INLINE bool BlendModeDoesntPreserveOpaqueDestinationAlpha(SkBlendMode blendMode)
{
    return blendMode == SkBlendMode::kSrc || blendMode == SkBlendMode::kSrcIn || blendMode == SkBlendMode::kDstIn || blendMode == SkBlendMode::kSrcOut
        || blendMode == SkBlendMode::kDstOut || blendMode == SkBlendMode::kSrcATop || blendMode == SkBlendMode::kDstATop || blendMode == SkBlendMode::kXor
        || blendMode == SkBlendMode::kModulate;
}

} // namespace

ALWAYS_INLINE bool BaseRenderingContext2D::BlendModeRequiresCompositedDraw(const CanvasRenderingContext2DState& state) const
{
    ...
    return state.ShouldDrawShadows() && !BlendModeSupportsShadowFilter(blend_mode);
}

ALWAYS_INLINE void BaseRenderingContext2D::ResetAlphaIfNeeded(cc::PaintCanvas* c, SkBlendMode blend_mode, const gfx::RectF* bounds)
{
    ...
    // being modified.
    if (!HasAlpha() && BlendModeDoesntPreserveOpaqueDestinationAlpha(blend_mode)) {
        cc::PaintFlags flags;
...
ALWAYS_INLINE void BaseRenderingContext2D::CheckOverdraw(
    const cc::PaintFlags* flags, CanvasRenderingContext2DState::ImageType image_type, BaseRenderingContext2D::OverdrawOp overdraw_op)
{
    if (base::FeatureList::IsEnabled(kDisableCanvasOverdrawOptimization)) [[unlikely]] {
        return;
    }
    ...
    if (overdraw_op == OverdrawOp::kDrawImage) { // static branch
        if (flags->getBlendMode() != SkBlendMode::kSrcOver || flags->getLooper() || flags->getImageFilter() || !flags->isOpaque()
            || image_type == CanvasRenderingContext2DState::kNonOpaqueImage) [[unlikely]] {
            return;
        }
    }

    if (overdraw_op == OverdrawOp::kClearRect || overdraw_op == OverdrawOp::kDrawImage) { // static branch
        ...
        if (!device_rect.contains(SkRect::MakeWH(image_info.width(), image_info.height()))) [[likely]] {
            return;
        }
    }
...
template <BaseRenderingContext2D::OverdrawOp CurrentOverdrawOp, typename DrawFunc, typename DrawCoversClipBoundsFunc>
void BaseRenderingContext2D::DrawInternal(cc::PaintCanvas* paint_canvas, const DrawFunc& draw_func, const DrawCoversClipBoundsFunc& draw_covers_clip_bounds,
    const gfx::RectF& bounds, CanvasRenderingContext2DState::PaintType paint_type, CanvasRenderingContext2DState::ImageType image_type,
    const SkIRect& clip_bounds, CanvasPerformanceMonitor::DrawType draw_type)
{
    ...
    if (ShouldUseCompositedDraw(paint_type, image_type)) {
        ...
        const cc::PaintFlags* flags = state.GetFlags(paint_type, kDrawForegroundOnly, image_type);
        WillDraw(clip_bounds, draw_type);
        ...
        if (ComputeDirtyRect(bounds, clip_bounds, &dirty_rect)) {
            const cc::PaintFlags* flags = state.GetFlags(paint_type, kDrawShadowAndForeground, image_type);
            if (paint_type != CanvasRenderingContext2DState::kStrokePaintType && draw_covers_clip_bounds(clip_bounds)) {
...
template <BaseRenderingContext2D::OverdrawOp CurrentOverdrawOp, typename DrawFunc, typename DrawCoversClipBoundsFunc>
void BaseRenderingContext2D::Draw(const DrawFunc& draw_func, const DrawCoversClipBoundsFunc& draw_covers_clip_bounds, const gfx::RectF& bounds,
    CanvasRenderingContext2DState::PaintType paint_type, CanvasRenderingContext2DState::ImageType image_type, CanvasPerformanceMonitor::DrawType draw_type)
{
    ...
    if (GetState().IsFilterUnresolved()) [[unlikely]] {
        ...
        DrawInternal<CurrentOverdrawOp, DrawFunc, DrawCoversClipBoundsFunc>(
            nullptr, draw_func, draw_covers_clip_bounds, bounds, paint_type, image_type, clip_bounds, draw_type);
    } else {
        DrawInternal<CurrentOverdrawOp, DrawFunc, DrawCoversClipBoundsFunc>(
            paint_canvas, draw_func, draw_covers_clip_bounds, bounds, paint_type, image_type, clip_bounds, draw_type);
    }
}

template <typename DrawFunc>
void BaseRenderingContext2D::CompositedDraw(
    const DrawFunc& draw_func, cc::PaintCanvas* c, CanvasRenderingContext2DState::PaintType paint_type, CanvasRenderingContext2DState::ImageType image_type)
{
    ...
    if (state.ShouldDrawShadows()) {
        ...
        cc::PaintFlags shadow_flags = *state.GetFlags(paint_type, kDrawShadowOnly, image_type);
        if (canvas_filter || ShouldUseDropShadowPaintFilter(paint_type, image_type)) {
            cc::PaintFlags foreground_flags = *state.GetFlags(paint_type, kDrawForegroundOnly, image_type);
            shadow_flags.setImageFilter(sk_make_sp<ComposePaintFilter>(
                sk_make_sp<ComposePaintFilter>(foreground_flags.getImageFilter(), shadow_flags.getImageFilter()), canvas_filter));
            ...
            DCHECK(IsFullCanvasCompositeMode(state.GlobalComposite()) || BlendModeRequiresCompositedDraw(state));
...
template <typename T> bool BaseRenderingContext2D::ValidateRectForCanvas(T x, T y, T width, T height)
{
    return (std::isfinite(x) && std::isfinite(y) && std::isfinite(width) && std::isfinite(height) && (width || height));
}

template <typename T> void BaseRenderingContext2D::AdjustRectForCanvas(T& x, T& y, T& width, T& height)
{
    if (width < 0) {
        width = -width;
        x -= width;
...
ALWAYS_INLINE void BaseRenderingContext2D::SetTransform(const AffineTransform& matrix)
{
    GetState().SetTransform(matrix);
    SetIsTransformInvertible(matrix.IsInvertible());
}

ALWAYS_INLINE bool BaseRenderingContext2D::IsFullCanvasCompositeMode(SkBlendMode op)
{
...
ALWAYS_INLINE bool BaseRenderingContext2D::ComputeDirtyRect(const gfx::RectF& local_rect, const SkIRect& transformed_clip_bounds, SkIRect* dirty_rect)
{
    DCHECK(dirty_rect);
    const CanvasRenderingContext2DState& state = GetState();
