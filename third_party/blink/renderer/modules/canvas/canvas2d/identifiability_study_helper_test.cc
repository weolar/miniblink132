...
TEST(IdentifiabilityStudyHelperTest, SameHashAsIdentifiableTokenBuilder_Aligned)
{
    std::array<uint64_t, 8> tokens = { 0, 1, max_uint, 45, 83, 123, 0, 3567 };
    IdentifiabilityStudyHelper helper1;
    IdentifiabilityStudyHelper helper2;
    IdentifiableTokenBuilder builder;
    helper1.UpdateBuilder(tokens[0], tokens[1], tokens[2], tokens[3], tokens[4], tokens[5], tokens[6], tokens[7]);
...
TEST(IdentifiabilityStudyHelperTest, SameHashAsIdentifiableTokenBuilder_Unaligned)
{
    std::array<uint64_t, 10> tokens = { 0, 1, max_uint, 45, 83, 123, 0, 3567, max_uint, 2 };
    IdentifiabilityStudyHelper helper1;
    IdentifiabilityStudyHelper helper2;
    IdentifiableTokenBuilder builder;
    helper1.UpdateBuilder(tokens[0], tokens[1], tokens[2], tokens[3], tokens[4], tokens[5], tokens[6], tokens[7], tokens[8], tokens[9]);
    for (uint64_t item : tokens) {
        helper2.UpdateBuilder(item);
        builder.AddToken(item);
    }
