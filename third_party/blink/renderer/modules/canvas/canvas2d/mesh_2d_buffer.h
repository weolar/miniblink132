...
namespace cc {
template <typename T> class RefCountedBuffer;
} // namespace cc
...
// paint op recording.
template <typename T> class Mesh2DBuffer : public ScriptWrappable {
public:
    ...
    {
        external_memory_accounter_.Decrease(v8::Isolate::GetCurrent(), base::checked_cast<int64_t>(buffer_->data().size() * sizeof(T)));
    }

    scoped_refptr<cc::RefCountedBuffer<T>> GetBuffer() const
    {
        return buffer_;
    }
...
protected:
    ...
    {
        external_memory_accounter_.Increase(v8::Isolate::GetCurrent(), base::checked_cast<int64_t>(buffer_->data().size() * sizeof(T)));
    }
