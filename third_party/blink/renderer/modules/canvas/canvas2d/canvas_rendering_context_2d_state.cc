...
bool StringToNumWithUnit(String spacing, float* number_spacing, CSSPrimitiveValue::UnitType* unit)
{
    ...
    if (result.GetType() == kDimensionToken && CSSPrimitiveValue::IsLength(result.GetUnitType())) {
...
FontSelectionValue CanvasFontStretchToSelectionValue(V8CanvasFontStretch font_stretch)
{
...
TextRenderingMode CanvasTextRenderingToTextRenderingMode(V8CanvasTextRendering text_rendering)
{
...
CanvasRenderingContext2DState::CanvasRenderingContext2DState(const CanvasRenderingContext2DState& other, ClipListCopyMode mode, SaveType save_type)
    ...
    , shadow_and_foreground_draw_looper_(other.shadow_and_foreground_draw_looper_)
    , shadow_only_image_filter_(other.shadow_only_image_filter_)
    , shadow_and_foreground_image_filter_(other.shadow_and_foreground_image_filter_)
...
void CanvasRenderingContext2DState::FontsNeedUpdate(FontSelector* font_selector, FontInvalidationReason)
{
...
void CanvasRenderingContext2DState::SetLineDash(const Vector<double>& dash)
{
    ...
    base::ranges::transform(line_dash_, line_dash_.begin(), [](double d) { return ClampTo<float>(d); });
...
ALWAYS_INLINE void CanvasRenderingContext2DState::UpdateLineDash() const
{
    ...
    if (!HasANonZeroElement(line_dash_)) {
        ...
        stroke_flags_.setPathEffect(cc::PathEffect::MakeDash(line_dash.data(), line_dash.size(), line_dash_offset_));
    }
    line_dash_dirty_ = false;
}
...
{
    DCHECK(fill_flags_.isAntiAlias() == stroke_flags_.isAntiAlias() && fill_flags_.isAntiAlias() == image_flags_.isAntiAlias());
...
void CanvasRenderingContext2DState::ClipPath(const SkPath& path, AntiAliasingMode anti_aliasing_mode)
{
    clip_list_.ClipPath(path, anti_aliasing_mode, AffineTransformToSkMatrix(transform_));
    has_clip_ = true;
    if (!path.isRect(nullptr))
        has_complex_clip_ = true;
}

void CanvasRenderingContext2DState::SetFont(const FontDescription& passed_font_description, FontSelector* selector)
{
    ...
    auto const font_size = CSSToLengthConversionData::FontSizes(
        font_description.ComputedSize(), font_description.ComputedSize(), &font, 1.0f /*Deliberately ignore zoom on the canvas element*/);
    ...
    if (word_spacing_is_set_) {
        ...
        float word_spacing_in_pixel = conversion_data.ZoomedComputedPixels(word_spacing_, word_spacing_unit_);
    ...
    if (letter_spacing_is_set_) {
        ...
        float letter_spacing_in_pixel = conversion_data.ZoomedComputedPixels(letter_spacing_, letter_spacing_unit_);
        font_description.SetLetterSpacing(letter_spacing_in_pixel);
    }
    font_description.SetKerning(font_kerning_);
    font_description.SetTextRendering(CanvasTextRenderingToTextRenderingMode(text_rendering_mode_));
    font_variant_caps_ = font_description.VariantCaps();
    std::optional<blink::V8CanvasFontStretch> font_value = V8CanvasFontStretch::Create(FontDescription::ToString(font_description.Stretch()).LowerASCII());
...
void CanvasRenderingContext2DState::SetFontInternal(const FontDescription& passed_font_description, FontSelector* selector)
{
    FontDescription font_description = passed_font_description;
    font_description.SetSubpixelAscentDescent(true);
...
const FontDescription& CanvasRenderingContext2DState::GetFontDescription() const
{
    DCHECK(realized_font_);
    return font_.GetFontDescription();
}

void CanvasRenderingContext2DState::SetFontKerning(FontDescription::Kerning font_kerning, FontSelector* selector)
{
    DCHECK(realized_font_);
...
void CanvasRenderingContext2DState::SetFontStretch(V8CanvasFontStretch font_stretch, FontSelector* selector)
{
    DCHECK(realized_font_);
    FontSelectionValue stretch_value = CanvasFontStretchToSelectionValue(font_stretch);
    FontDescription font_description(GetFontDescription());
...
void CanvasRenderingContext2DState::SetFontVariantCaps(FontDescription::FontVariantCaps font_variant_caps, FontSelector* selector)
{
    DCHECK(realized_font_);
    FontDescription font_description(GetFontDescription());
...
sk_sp<PaintFilter> CanvasRenderingContext2DState::GetFilterForOffscreenCanvas(gfx::Size canvas_size, BaseRenderingContext2D* context)
{
    ValidateFilterState();
    ...
    if (canvas_filter_) {
        ...
        operations = FilterOperationResolver::CreateOffscreenFilterOperations(*css_filter_value_, font_for_filter_);
    }
    ...
    FilterEffectBuilder filter_effect_builder(gfx::RectF(canvas_viewport), canvas_viewport,
        ...
        Color::kBlack, mojom::blink::ColorScheme::kLight, &fill_flags_for_filter, &stroke_flags_for_filter);

    FilterEffect* last_effect = filter_effect_builder.BuildFilterEffect(operations, !context->OriginClean());
    if (last_effect) {
        // TODO(chrishtr): Taint the origin if needed. crbug.com/792506.
        resolved_filter_ = paint_filter_builder::Build(last_effect, kInterpolationSpaceSRGB);
    }

    filter_state_ = resolved_filter_ ? FilterState::kResolved : FilterState::kInvalid;
    ValidateFilterState();
    return resolved_filter_;
}

sk_sp<PaintFilter> CanvasRenderingContext2DState::GetFilter(Element* style_resolution_host, gfx::Size canvas_size, CanvasRenderingContext2D* context)
{
    ...
    if (canvas_filter_) {
        ...
        operations = document.GetStyleResolver().ComputeFilterOperations(style_resolution_host, *font, *css_filter_value_);
    }
...
void CanvasRenderingContext2DState::ClearResolvedFilter()
{
    ...
    filter_state_ = (canvas_filter_ || css_filter_value_) ? FilterState::kUnresolved : FilterState::kNone;
    ValidateFilterState();
}
...
sk_sp<cc::DrawLooper>& CanvasRenderingContext2DState::ShadowOnlyDrawLooper() const
{
    if (!shadow_only_draw_looper_) {
        DrawLooperBuilder draw_looper_builder;
        draw_looper_builder.AddShadow(
            shadow_offset_, ClampTo<float>(shadow_blur_), shadow_color_, DrawLooperBuilder::kShadowIgnoresTransforms, DrawLooperBuilder::kShadowRespectsAlpha);
        shadow_only_draw_looper_ = draw_looper_builder.DetachDrawLooper();
    }
    return shadow_only_draw_looper_;
}

sk_sp<cc::DrawLooper>& CanvasRenderingContext2DState::ShadowAndForegroundDrawLooper() const
{
...
sk_sp<PaintFilter>& CanvasRenderingContext2DState::ShadowOnlyImageFilter() const
{
    ...
    if (!shadow_only_image_filter_) {
        ...
        shadow_only_image_filter_ = sk_make_sp<DropShadowPaintFilter>(
            shadow_offset_.x(), shadow_offset_.y(), sigma, sigma, shadow_color_.toSkColor4f(), ShadowMode::kDrawShadowOnly, nullptr);
    }
    return shadow_only_image_filter_;
}

sk_sp<PaintFilter>& CanvasRenderingContext2DState::ShadowAndForegroundImageFilter() const
{
    using ShadowMode = DropShadowPaintFilter::ShadowMode;
    if (!shadow_and_foreground_image_filter_) {
        ...
        shadow_and_foreground_image_filter_ = sk_make_sp<DropShadowPaintFilter>(
            shadow_offset_.x(), shadow_offset_.y(), sigma, sigma, shadow_color_.toSkColor4f(), ShadowMode::kDrawShadowAndForeground, nullptr);
    }
...
void CanvasRenderingContext2DState::SetCanvasFilter(CanvasFilter* canvas_filter)
{
...
void CanvasRenderingContext2DState::SetImageSmoothingQuality(const V8ImageSmoothingQuality& quality)
{
...
V8ImageSmoothingQuality CanvasRenderingContext2DState::ImageSmoothingQuality() const
{
...
void CanvasRenderingContext2DState::UpdateFilterQuality(cc::PaintFlags::FilterQuality filter_quality) const
{
...
const cc::PaintFlags* CanvasRenderingContext2DState::GetFlags(PaintType paint_type, ShadowMode shadow_mode, ImageType image_type) const
{
    cc::PaintFlags* flags;
    switch (paint_type) {
    ...
    if ((!ShouldDrawShadows() && shadow_mode == kDrawShadowAndForeground) || shadow_mode == kDrawForegroundOnly) {
...
void CanvasRenderingContext2DState::SetLetterSpacing(const String& letter_spacing)
{
    DCHECK(realized_font_);
    if (!letter_spacing_is_set_)
    ...
    CSSToLengthConversionData conversion_data = CSSToLengthConversionData();
    auto const font_size = CSSToLengthConversionData::FontSizes(
        font_description.ComputedSize(), font_description.ComputedSize(), &font_, 1.0f /*Deliberately ignore zoom on the canvas element*/);
    conversion_data.SetFontSizes(font_size);
    float letter_spacing_in_pixel = conversion_data.ZoomedComputedPixels(num_spacing, unit);
...
void CanvasRenderingContext2DState::SetWordSpacing(const String& word_spacing)
{
    ...
    auto const font_size = CSSToLengthConversionData::FontSizes(
        font_description.ComputedSize(), font_description.ComputedSize(), &font_, 1.0f /*Deliberately ignore zoom on the canvas element*/);
    conversion_data.SetFontSizes(font_size);
    float word_spacing_in_pixel = conversion_data.ZoomedComputedPixels(num_spacing, unit);
...
void CanvasRenderingContext2DState::SetTextRendering(V8CanvasTextRendering text_rendering, FontSelector* selector)
{
    DCHECK(realized_font_);
    TextRenderingMode text_rendering_mode = CanvasTextRenderingToTextRenderingMode(text_rendering);
    FontDescription font_description(GetFontDescription());
