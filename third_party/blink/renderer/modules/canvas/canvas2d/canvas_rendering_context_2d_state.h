...
enum ShadowMode { kDrawShadowAndForeground, kDrawShadowOnly, kDrawForegroundOnly };

class MODULES_EXPORT CanvasRenderingContext2DState final : public GarbageCollected<CanvasRenderingContext2DState>, public FontSelectorClient {
public:
    ...
    enum class SaveType { kSaveRestore, kBeginEndLayerOneSave, kBeginEndLayerTwoSaves, kBeginEndLayerThreeSaves, kInitial };

    CanvasRenderingContext2DState();
    CanvasRenderingContext2DState(const CanvasRenderingContext2DState&, ClipListCopyMode, SaveType);

    CanvasRenderingContext2DState(const CanvasRenderingContext2DState&) = delete;
    CanvasRenderingContext2DState& operator=(const CanvasRenderingContext2DState&) = delete;
    ...
    const Vector<double>& LineDash() const
    {
        return line_dash_;
    }
    ...
    void SetFont(const FontDescription& passed_font_description, FontSelector* selector);
    bool IsFontDirtyForFilter() const;
    const Font& GetFont() const;
    const FontDescription& GetFontDescription() const;
    inline bool HasRealizedFont() const
    {
        return realized_font_;
    }
    void SetUnparsedFont(const String& font)
    {
        unparsed_font_ = font;
    }
    const String& UnparsedFont() const
    {
        return unparsed_font_;
    }

    void SetFontForFilter(const Font& font)
    {
        font_for_filter_ = font;
    }
    ...
    const String& UnparsedCSSFilter() const
    {
        return unparsed_css_filter_;
    }
    void SetCanvasFilter(CanvasFilter* filter_value);
    CanvasFilter* GetCanvasFilter() const
    {
        return canvas_filter_.Get();
    }
    sk_sp<PaintFilter> GetFilter(Element*, gfx::Size canvas_size, CanvasRenderingContext2D*);
    sk_sp<PaintFilter> GetFilterForOffscreenCanvas(gfx::Size canvas_size, BaseRenderingContext2D*);
    ...
    const CanvasStyle& StrokeStyle() const
    {
    ...
    const CanvasStyle& FillStyle() const
    {
        return fill_style_;
    }
    ...
    {
        return text_baseline_;
    }

    void SetLetterSpacing(const String& letter_spacing);
    String GetLetterSpacing() const
    {
        return parsed_letter_spacing_;
    }

    void SetWordSpacing(const String& word_spacing);
    String GetWordSpacing() const
    {
        return parsed_word_spacing_;
    }

    void SetTextRendering(V8CanvasTextRendering text_rendering, FontSelector* selector);
    V8CanvasTextRendering GetTextRendering() const
    {
        return text_rendering_mode_;
    }

    void SetFontKerning(FontDescription::Kerning font_kerning, FontSelector* selector);
    FontDescription::Kerning GetFontKerning() const
    {
        return font_kerning_;
    }

    void SetFontStretch(V8CanvasFontStretch font_stretch, FontSelector* selector);
    V8CanvasFontStretch GetFontStretch() const
    {
        return font_stretch_;
    }

    void SetFontVariantCaps(FontDescription::FontVariantCaps font_kerning, FontSelector* selector);
    ...
    {
        return stroke_flags_.getStrokeWidth();
    }
    ...
    {
        return stroke_flags_.getStrokeMiter();
    }

    void SetShadowOffsetX(double);
    void SetShadowOffsetY(double);
    const gfx::Vector2dF& ShadowOffset() const
    {
    ...
    void SetUnparsedStrokeColor(v8::Isolate* isolate, v8::Local<v8::String> color)
    {
        unparsed_stroke_color_.Reset(isolate, color);
    }
    void ClearUnparsedStrokeColor()
    {
        unparsed_stroke_color_.Reset();
    }
    ...
    void ClearUnparsedFillColor()
    {
        unparsed_fill_color_.Reset();
    }
    ...
    // Opaque.
    const cc::PaintFlags* GetFlags(PaintType, ShadowMode, ImageType = kNoImage) const;

    static_assert(static_cast<int>(SaveType::kBeginEndLayerOneSave) + 1 == static_cast<int>(SaveType::kBeginEndLayerTwoSaves));
    static_assert(static_cast<int>(SaveType::kBeginEndLayerTwoSaves) + 1 == static_cast<int>(SaveType::kBeginEndLayerThreeSaves));
    ...
    {
        return save_type_ >= SaveType::kBeginEndLayerOneSave && save_type_ <= SaveType::kBeginEndLayerThreeSaves;
    }
    int LayerSaveCount()
    {
        if (!IsLayerSaveType()) {
        ...
        return static_cast<int>(save_type_) - static_cast<int>(SaveType::kBeginEndLayerOneSave) + 1;
    }
    static SaveType LayerSaveCountToSaveType(int save_count)
    {
        CHECK(1 <= save_count && save_count <= 3);
        return static_cast<SaveType>(static_cast<int>(SaveType::kBeginEndLayerOneSave) + save_count - 1);
    }
...
private:
    ...
    CSSPrimitiveValue::UnitType letter_spacing_unit_ { CSSPrimitiveValue::UnitType::kPixels };
    String parsed_letter_spacing_;

    float word_spacing_ { 0 };
    CSSPrimitiveValue::UnitType word_spacing_unit_ { CSSPrimitiveValue::UnitType::kPixels };
    String parsed_word_spacing_;
    V8CanvasTextRendering text_rendering_mode_ { V8CanvasTextRendering::Enum::kAuto };
    ...
    FontDescription::FontVariantCaps font_variant_caps_ { FontDescription::kCapsNormal };
...
{
    return (!shadow_color_.IsFullyTransparent()) && (shadow_blur_ || !shadow_offset_.IsZero());
}
