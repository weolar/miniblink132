...
OffscreenCanvas* HTMLCanvasElementModule::transferControlToOffscreen(ScriptState* script_state, HTMLCanvasElement& canvas, ExceptionState& exception_state)
{
    ...
    if (canvas.IsOffscreenCanvasRegistered()) {
        ...
        canvas.CreateLayer();
        offscreen_canvas = TransferControlToOffscreenInternal(script_state, canvas, exception_state);
    }
...
OffscreenCanvas* HTMLCanvasElementModule::TransferControlToOffscreenInternal(
    ScriptState* script_state, HTMLCanvasElement& canvas, ExceptionState& exception_state)
{
    OffscreenCanvas* offscreen_canvas = OffscreenCanvas::Create(script_state, canvas.width(), canvas.height());
