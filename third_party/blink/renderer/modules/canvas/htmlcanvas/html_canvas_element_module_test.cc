...
class HTMLCanvasElementModuleTest : public ::testing::Test, public ::testing::WithParamInterface<bool> {
protected:
    void SetUp() override
    {
    ...
    HTMLCanvasElement& canvas_element() const
    {
    ...
    OffscreenCanvas* TransferControlToOffscreen(ExceptionState& exception_state)
    {
        return HTMLCanvasElementModule::TransferControlToOffscreenInternal(
            ToScriptStateForMainWorld(GetWindow()->GetFrame()), canvas_element(), exception_state);
    }
...
TEST_F(HTMLCanvasElementModuleTest, TransferControlToOffscreen)
{
    ...
    const OffscreenCanvas* offscreen_canvas = TransferControlToOffscreen(exception_state);
...
TEST_P(HTMLCanvasElementModuleTest, LowLatencyCanvasCompositorFrameOpacity)
{
    ...
    context_provider->UnboundTestContextGL()->set_supports_gpu_memory_buffer_format(buffer_format, true);
