...
OffscreenCanvasRenderingContext2D* GetContext(V8TestingScope& scope, OffscreenCanvas* host)
{
    CanvasRenderingContext* context
        = host->GetCanvasRenderingContext(scope.GetExecutionContext(), CanvasRenderingContext::CanvasRenderingAPI::k2D, CanvasContextCreationAttributesCore());
    CHECK(context->IsRenderingContext2D());
    return static_cast<OffscreenCanvasRenderingContext2D*>(context);
}

TEST(OffscreenCanvasHostTest, TransferToOffscreenThrowsErrorsProducedByContext)
{
    ...
    host->transferToImageBitmap(scope.GetScriptState(), scope.GetExceptionState());
...
TEST(OffscreenCanvasHostTest, TransferToOffscreenThrowsUnknownErrorAsFallback)
{
    ...
    auto* host = OffscreenCanvas::Create(scope.GetScriptState(), /*width=*/100000000,
        /*height=*/100000000);
    // A context must exist for `transferToImageBitmap` to work.
    GetContext(scope, host);

    host->transferToImageBitmap(scope.GetScriptState(), scope.GetExceptionState());
    EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kUnknownError);
}
...
TEST(OffscreenCanvasRenderingContext2DTest, TransferToOffscreenThrowsInLayers)
{
    ...
    context->beginLayer(scope.GetScriptState(), BeginLayerOptions::Create(), no_exception);
    context->TransferToImageBitmap(scope.GetScriptState(), scope.GetExceptionState());
    EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kInvalidStateError);
}
...
TEST(OffscreenCanvasRenderingContext2DTest, UnclosedLayerConvertToBlob)
{
    ...
    host->convertToBlob(scope.GetScriptState(), options, scope.GetExceptionState());
    EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kInvalidStateError);
...
TEST(OffscreenCanvasRenderingContext2DTest, UnclosedLayerCreateImageBitmap)
{
    ...
    // Throws inside layers:
    ImageBitmapFactories::CreateImageBitmap(scope.GetScriptState(), image, options, scope.GetExceptionState());
    EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kInvalidStateError);

    context->endLayer(no_exception);

    // Doesn't throw outside layers:
    ImageBitmapFactories::CreateImageBitmap(scope.GetScriptState(), image, options, no_exception);
}
...
TEST(OffscreenCanvasRenderingContext2DTest, UnclosedLayerGetImageData)
{
    ...
    // Throws inside layers:
    context->getImageData(/*sx=*/0, /*sy=*/0, /*sw=*/1, /*sh=*/1, scope.GetExceptionState());
    EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kInvalidStateError);
...
TEST(OffscreenCanvasRenderingContext2DTest, UnclosedLayerPutImageData)
{
    ...
    NonThrowableExceptionState no_exception;
    ImageData* image_data = ImageData::Create(context->Width(), context->Height(), no_exception);

    context->beginLayer(scope.GetScriptState(), BeginLayerOptions::Create(), no_exception);

    // Throws inside layers:
    context->putImageData(image_data, /*dx=*/0, /*dy=*/0, scope.GetExceptionState());
    EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kInvalidStateError);
...
TEST(OffscreenCanvasRenderingContext2DTest, UnclosedLayerTransferToImageBitmap)
{
    test::TaskEnvironment task_environment;
    ScopedCanvas2dLayersForTest layer_feature(/*enabled=*/true);
    V8TestingScope scope;
