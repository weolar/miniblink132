...
public:
    static std::unique_ptr<SharedStorageWorkletThread> Create(WorkerReportingProxy& worker_reporting_proxy);

    ~SharedStorageWorkletThread() override;

    void InitializeSharedStorageWorkletService(mojo::PendingReceiver<mojom::blink::SharedStorageWorkletService> receiver, base::OnceClosure disconnect_handler);

    static std::optional<WorkerBackingThreadStartupData> CreateThreadStartupData();
