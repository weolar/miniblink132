...
class WindowSharedStorageImpl final : public GarbageCollected<WindowSharedStorageImpl>, public Supplement<LocalDOMWindow> {
public:
    static const char kSupplementName[];

    static WindowSharedStorageImpl& From(LocalDOMWindow& window)
    {
        WindowSharedStorageImpl* supplement = Supplement<LocalDOMWindow>::template From<WindowSharedStorageImpl>(window);
        if (!supplement) {
...
// static
const char WindowSharedStorageImpl::kSupplementName[] = "WindowSharedStorageImpl";

} // namespace

SharedStorage* WindowSharedStorage::sharedStorage(LocalDOMWindow& window, ExceptionState& exception_state)
{
