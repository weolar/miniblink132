...
constexpr base::Time kScriptResponseTime = base::Time::FromDeltaSinceWindowsEpoch(base::Days(100));
...
struct GetResult {
    blink::mojom::SharedStorageGetStatus status = blink::mojom::SharedStorageGetStatus::kSuccess;
    std::string error_message;
    std::u16string value;
};
...
std::vector<blink::mojom::SharedStorageKeyAndOrValuePtr> CreateBatchResult(std::vector<std::pair<std::u16string, std::u16string>> input)
{
    std::vector<blink::mojom::SharedStorageKeyAndOrValuePtr> result;
    for (const auto& p : input) {
        blink::mojom::SharedStorageKeyAndOrValuePtr e = blink::mojom::SharedStorageKeyAndOrValue::New(p.first, p.second);
...
public:
    explicit TestWorkletDevToolsHost(mojo::PendingReceiver<mojom::blink::WorkletDevToolsHost> receiver)
        : receiver_(this, std::move(receiver))
    {
    }

    void OnReadyForInspection(
        mojo::PendingRemote<mojom::blink::DevToolsAgent> agent, mojo::PendingReceiver<mojom::blink::DevToolsAgentHost> agent_host) override
    {
    ...
    bool ready_for_inspection() const
    {
        return ready_for_inspection_;
    }
...
public:
    explicit TestClient(mojo::PendingAssociatedReceiver<blink::mojom::SharedStorageWorkletServiceClient> receiver)
        : receiver_(this, std::move(receiver))
    {
    }
    ...
    void SharedStorageGet(const std::u16string& key, SharedStorageGetCallback callback) override
    {
        ...
        std::move(callback).Run(get_result_.status, get_result_.error_message, get_result_.value);
    }

    void SharedStorageKeys(mojo::PendingRemote<blink::mojom::SharedStorageEntriesListener> pending_listener) override
    {
        pending_keys_listeners_.push_back(std::move(pending_listener));
    }

    void SharedStorageEntries(mojo::PendingRemote<blink::mojom::SharedStorageEntriesListener> pending_listener) override
    {
    ...
    void SharedStorageLength(SharedStorageLengthCallback callback) override
    {
        ...
        std::move(callback).Run(length_result_.success, length_result_.error_message, length_result_.length);
    }

    void SharedStorageRemainingBudget(SharedStorageRemainingBudgetCallback callback) override
    {
        observed_remaining_budget_count_++;
        std::move(callback).Run(remaining_budget_result_.success, remaining_budget_result_.error_message, remaining_budget_result_.bits);
    }
    ...
    void DidAddMessageToConsole(blink::mojom::ConsoleMessageLevel level, const std::string& message) override
    {
        observed_console_log_messages_.push_back(message);
    }

    void RecordUseCounters(const std::vector<mojom::WebFeature>& features) override
    {
        base::ranges::for_each(features, [&](mojom::WebFeature feature) { observed_use_counters_.push_back(feature); });
    }

    mojo::Remote<blink::mojom::SharedStorageEntriesListener> TakeKeysListenerAtFront()
    {
        ...
        return mojo::Remote<blink::mojom::SharedStorageEntriesListener>(std::move(pending_listener));
    }

    mojo::Remote<blink::mojom::SharedStorageEntriesListener> TakeEntriesListenerAtFront()
    {
    ...
    std::deque<mojo::PendingRemote<blink::mojom::SharedStorageEntriesListener>> pending_keys_listeners_;

    std::deque<mojo::PendingRemote<blink::mojom::SharedStorageEntriesListener>> pending_entries_listeners_;

    std::vector<network::mojom::SharedStorageModifierMethodPtr> observed_update_params_;
...
private:
    mojo::AssociatedReceiver<blink::mojom::SharedStorageWorkletServiceClient> receiver_ { this };
};

class MockMojomPrivateAggregationHost : public blink::mojom::blink::PrivateAggregationHost {
public:
    ...
    mojo::ReceiverSet<blink::mojom::blink::PrivateAggregationHost>& receiver_set()
    {
        return receiver_set_;
    }

    // blink::mojom::blink::PrivateAggregationHost:
    MOCK_METHOD(void, ContributeToHistogram, (Vector<blink::mojom::blink::AggregatableReportHistogramContributionPtr>), (override));
    MOCK_METHOD(void, EnableDebugMode, (blink::mojom::blink::DebugKeyPtr), (override));
...
public:
    ...
    void DidGenerateCacheableMetadata(mojom::CodeCacheType cache_type, const KURL& url, base::Time expected_response_time, mojo_base::BigBuffer data) override
    {
    ...
    void FetchCachedCode(mojom::CodeCacheType cache_type, const KURL& url, FetchCachedCodeCallback callback) override
    {
        fetch_cached_code_count_++;
        std::move(callback).Run(response_time_, data_.Clone());
    }

    void ClearCodeCacheEntry(mojom::CodeCacheType cache_type, const KURL& url) override
    {
        clear_code_cache_entry_count_++;
    }

    void DidGenerateCacheableMetadataInCacheStorage(
        const KURL& url, base::Time expected_response_time, mojo_base::BigBuffer data, const String& cache_storage_cache_name) override
    {
        NOTREACHED();
    }

    void OverrideFetchCachedCodeResult(base::Time response_time, mojo_base::BigBuffer data)
    {
    ...
    {
        return fetch_cached_code_count_;
    }
...
{
    return std::make_unique<GlobalScopeCreationParams>(KURL("https://foo.com"),
...
public:
    ...
    AddModuleResult AddModule(const std::string& script_content, std::string mime_type = "application/javascript")
    {
        ...
        proxied_url_loader_factory.AddResponse(GURL(kModuleScriptSource), std::move(head),
        ...
        shared_storage_worklet_service_->AddModule(factory.Unbind(), GURL(kModuleScriptSource), future.GetCallback());

        return { future.Get<0>(), future.Get<1>() };
    }

    SelectURLResult SelectURL(const std::string& name, const std::vector<GURL>& urls, blink::CloneableMessage serialized_data)
    {
        InitializeWorkletServiceOnce();

        base::test::TestFuture<bool, const std::string&, uint32_t> future;
        shared_storage_worklet_service_->RunURLSelectionOperation(name, urls, std::move(serialized_data), MaybeInitPAOperationDetails(), future.GetCallback());

        return { future.Get<0>(), future.Get<1>(), future.Get<2>() };
    }

    RunResult Run(const std::string& name, blink::CloneableMessage serialized_data, int filtering_id_max_bytes = 1)
    {
        InitializeWorkletServiceOnce();

        base::test::TestFuture<bool, const std::string&> future;
        shared_storage_worklet_service_->RunOperation(
            name, std::move(serialized_data), MaybeInitPAOperationDetails(filtering_id_max_bytes), future.GetCallback());

        return { future.Get<0>(), future.Get<1>() };
    }

    // CrossVariantMojoRemote<mojom::blink::PrivateAggregationHostInterfaceBase>
    mojom::PrivateAggregationOperationDetailsPtr MaybeInitPAOperationDetails(int filtering_id_max_bytes = kPrivateAggregationApiDefaultFilteringIdMaxBytes)
    {
        CHECK_EQ(ShouldDefinePrivateAggregationInSharedStorage(), !!mock_private_aggregation_host_);

        if (!ShouldDefinePrivateAggregationInSharedStorage()) {
            return nullptr;
        }

        mojo::PendingRemote<mojom::blink::PrivateAggregationHost> pending_pa_host_remote;
        mojo::PendingReceiver<mojom::blink::PrivateAggregationHost> pending_pa_host_receiver = pending_pa_host_remote.InitWithNewPipeAndPassReceiver();

        mock_private_aggregation_host_->receiver_set().Add(mock_private_aggregation_host_.get(), std::move(pending_pa_host_receiver));

        return mojom::PrivateAggregationOperationDetails::New(
            CrossVariantMojoRemote<mojom::blink::PrivateAggregationHostInterfaceBase>(std::move(pending_pa_host_remote)), filtering_id_max_bytes);
    }
    ...
    CloneableMessage CreateSerializedDict(const std::map<std::string, std::string>& dict)
    {
        return CreateSerializedDictOrUndefined(&dict);
    }

protected:
    ScopedSharedStorageAPIM125ForTest shared_storage_m125_runtime_enabled_feature { /*enabled=*/true };

    ScopedInterestGroupsInSharedStorageWorkletForTest interest_groups_in_shared_storage_worklet_runtime_enabled_feature { /*enabled=*/true };

    ScopedSharedStorageWebLocksForTest shared_storage_web_locks_runtime_enabled_feature { /*enabled=*/true };

    mojo::Remote<mojom::SharedStorageWorkletService> shared_storage_worklet_service_;

    Persistent<SharedStorageWorkletMessagingProxy> messaging_proxy_;

    std::optional<std::u16string> embedder_context_;

    blink::mojom::SharedStorageWorkletPermissionsPolicyStatePtr permissions_policy_state_ = blink::mojom::SharedStorageWorkletPermissionsPolicyState::New(
    ...
    std::unique_ptr<MockMojomPrivateAggregationHost> mock_private_aggregation_host_;
...
private:
    CloneableMessage CreateSerializedDictOrUndefined(const std::map<std::string, std::string>* dict)
    {
        ScriptState* script_state = ToScriptStateForMainWorld(&GetFrame());
        ScriptState::Scope scope(script_state);
        v8::MicrotasksScope microtasksScope(script_state->GetContext(), v8::MicrotasksScope::kRunMicrotasks);
        ...
        if (dict) {
            ...
            serialized_value = SerializedScriptValue::SerializeAndSwallowExceptions(isolate, v8_value);
        } else {
        ...
        mojo::Message message = mojom::CloneableMessage::SerializeAsMessage(&original);
        ...
        mojom::CloneableMessage::DeserializeFromMessage(std::move(message), &converted);
        return converted;
    }
    ...
    void InitializeWorkletServiceOnce()
    {
        ...
        mojo::PendingReceiver<mojom::SharedStorageWorkletService> receiver = shared_storage_worklet_service_.BindNewPipeAndPassReceiver();

        mojo::PendingRemote<mojom::blink::WorkletDevToolsHost> pending_devtools_host_remote;
        mojo::PendingReceiver<mojom::blink::WorkletDevToolsHost> pending_devtools_host_receiver = pending_devtools_host_remote.InitWithNewPipeAndPassReceiver();
        test_worklet_devtools_host_ = std::make_unique<TestWorkletDevToolsHost>(std::move(pending_devtools_host_receiver));

        mojo::PendingRemote<mojom::blink::CodeCacheHost> pending_code_cache_host_remote;
        mojo::PendingReceiver<mojom::blink::CodeCacheHost> pending_code_cache_host_receiver = pending_code_cache_host_remote.InitWithNewPipeAndPassReceiver();

        mock_code_cache_host_->receiver_set().Add(mock_code_cache_host_.get(), std::move(pending_code_cache_host_receiver));

        messaging_proxy_ = MakeGarbageCollected<SharedStorageWorkletMessagingProxy>(base::SingleThreadTaskRunner::GetCurrentDefault(),
            CrossVariantMojoReceiver<mojom::blink::SharedStorageWorkletServiceInterfaceBase>(std::move(receiver)),
            mojom::blink::WorkletGlobalScopeCreationParams::New(KURL(kModuleScriptSource),
                /*starter_origin=*/
                SecurityOrigin::Create(KURL(kModuleScriptSource)), Vector<blink::mojom::OriginTrialFeature>(),
                /*devtools_worker_token=*/base::UnguessableToken(), std::move(pending_devtools_host_remote), std::move(pending_code_cache_host_remote),
                mojo::PendingRemote<mojom::blink::BrowserInterfaceBroker>(),
                /*wait_for_debugger=*/false),
            worklet_terminated_future_.GetCallback());

        mojo::PendingAssociatedRemote<mojom::SharedStorageWorkletServiceClient> pending_shared_storage_service_client_remote;
        mojo::PendingAssociatedReceiver<mojom::SharedStorageWorkletServiceClient> pending_shared_storage_service_client_receiver
            = pending_shared_storage_service_client_remote.InitWithNewEndpointAndPassReceiver();

        test_client_ = std::make_unique<TestClient>(std::move(pending_shared_storage_service_client_receiver));

        if (ShouldDefinePrivateAggregationInSharedStorage()) {
            mock_private_aggregation_host_ = std::make_unique<MockMojomPrivateAggregationHost>();
        }

        shared_storage_worklet_service_->Initialize(
            std::move(pending_shared_storage_service_client_remote), permissions_policy_state_->Clone(), embedder_context_);
...
TEST_F(SharedStorageWorkletTest, AddModule_SimpleScriptError)
{
    ...
    EXPECT_THAT(result.error_message, testing::HasSubstr("ReferenceError: a is not defined"));
...
TEST_F(SharedStorageWorkletTest, CodeCache_NoClearDueToEmptyCache_NoGenerateData)
{
...
TEST_F(SharedStorageWorkletTest, CodeCache_DidClearDueToUnmatchedTime_NoGenerateData)
{
...
TEST_F(SharedStorageWorkletTest, CodeCache_NoClearDueToMatchedTime_NoGenerateData)
{
...
TEST_F(SharedStorageWorkletTest, GlobalScopeObjectsAndFunctions_DuringAddModule)
{
...
TEST_F(SharedStorageWorkletTest, RegisterOperation_MissingOperationNameArgument)
{
    AddModuleResult result = AddModule(/*script_content=*/R"(
      register();
  )");
    ...
    EXPECT_FALSE(result.success);
    EXPECT_THAT(result.error_message, testing::HasSubstr("2 arguments required, but only 0 present"));
}
...
TEST_F(SharedStorageWorkletTest, RegisterOperation_MissingClassArgument)
{
    ...
    EXPECT_FALSE(result.success);
    EXPECT_THAT(result.error_message, testing::HasSubstr("2 arguments required, but only 1 present"));
}
...
TEST_F(SharedStorageWorkletTest, RegisterOperation_EmptyOperationName)
{
    ...
    EXPECT_FALSE(result.success);
    EXPECT_THAT(result.error_message, testing::HasSubstr("Operation name cannot be empty"));
}
...
TEST_F(SharedStorageWorkletTest, RegisterOperation_ClassArgumentNotAFunction)
{
    ...
    EXPECT_FALSE(result.success);
    EXPECT_THAT(result.error_message, testing::HasSubstr("parameter 2 is not of type 'Function'"));
}
...
TEST_F(SharedStorageWorkletTest, RegisterOperation_MissingRunFunction)
{
    ...
    EXPECT_FALSE(result.success);
    EXPECT_THAT(result.error_message, testing::HasSubstr("Property \"run\" doesn't exist"));
}

TEST_F(SharedStorageWorkletTest, RegisterOperation_ClassArgumentPrototypeNotAnObject)
{
    AddModuleResult result = AddModule(/*script_content=*/R"(
      function test() {};
      test.prototype = 123;
    ...
    EXPECT_FALSE(result.success);
    EXPECT_THAT(result.error_message, testing::HasSubstr("constructor prototype is not an object"));
}
...
TEST_F(SharedStorageWorkletTest, RegisterOperation_AlreadyRegistered)
{
    ...
    EXPECT_FALSE(result.success);
    EXPECT_THAT(result.error_message, testing::HasSubstr("Operation name already registered"));
}

TEST_F(SharedStorageWorkletTest, SelectURL_BeforeAddModuleFinish)
{
    SelectURLResult select_url_result = SelectURL("test-operation", /*urls=*/ {}, CreateSerializedUndefined());

    EXPECT_FALSE(select_url_result.success);
    EXPECT_THAT(select_url_result.error_message, testing::HasSubstr("The module script hasn't been loaded"));
...
TEST_F(SharedStorageWorkletTest, SelectURL_OperationNameNotRegistered)
{
    ...
    SelectURLResult select_url_result = SelectURL("unregistered-operation", /*urls=*/ {}, CreateSerializedUndefined());

    EXPECT_FALSE(select_url_result.success);
    EXPECT_THAT(select_url_result.error_message, testing::HasSubstr("Cannot find operation name"));
    EXPECT_EQ(select_url_result.index, 0u);
}
...
TEST_F(SharedStorageWorkletTest, SelectURL_FunctionError)
{
    ...
    EXPECT_FALSE(select_url_result.success);
    EXPECT_THAT(select_url_result.error_message, testing::HasSubstr("ReferenceError: a is not defined"));
    EXPECT_EQ(select_url_result.index, 0u);
}
...
TEST_F(SharedStorageWorkletTest, SelectURL_FulfilledSynchronously)
{
    ...
    SelectURLResult select_url_result = SelectURL("test-operation",
        /*urls=*/ { GURL("https://foo0.com"), GURL("https://foo1.com") }, CreateSerializedUndefined());
...
TEST_F(SharedStorageWorkletTest, SelectURL_RejectedAsynchronously)
{
    ...
    test_client_->length_result_ = LengthResult { .success = false, .error_message = "error 123", .length = 0 };
...
TEST_F(SharedStorageWorkletTest, SelectURL_FulfilledAsynchronously)
{
    ...
    test_client_->length_result_ = LengthResult { .success = true, .error_message = std::string(), .length = 1 };
...
TEST_F(SharedStorageWorkletTest, SelectURL_ReturnValueOutOfRange)
{
    ...
    EXPECT_FALSE(select_url_result.success);
    EXPECT_THAT(select_url_result.error_message, testing::HasSubstr("Promise resolved to a number outside the length of the input urls"));
    EXPECT_EQ(select_url_result.index, 0u);
}
...
TEST_F(SharedStorageWorkletTest, SelectURL_ReturnValueToUint32Error)
{
    ...
    EXPECT_FALSE(select_url_result.success);
    EXPECT_THAT(select_url_result.error_message, testing::HasSubstr("Promise did not resolve to an uint32 number"));
    EXPECT_EQ(select_url_result.index, 0u);
}

TEST_F(SharedStorageWorkletTest, SelectURL_ValidateUrlsAndDataParamViaConsoleLog)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
      class TestClass {
        async run(urls, data) {
    ...
    SelectURLResult select_url_result = SelectURL("test-operation",
        /*urls=*/ { GURL("https://foo0.com"), GURL("https://foo1.com") }, CreateSerializedDict({ { "customField", "customValue" } }));
    ...
    EXPECT_EQ(test_client_->observed_console_log_messages_[0], "[\"https://foo0.com/\",\"https://foo1.com/\"]");
    EXPECT_EQ(test_client_->observed_console_log_messages_[1], "{\"customField\":\"customValue\"}");
}
...
TEST_F(SharedStorageWorkletTest, Run_BeforeAddModuleFinish)
{
    ...
    EXPECT_THAT(run_result.error_message, testing::HasSubstr("The module script hasn't been loaded"));
}
...
TEST_F(SharedStorageWorkletTest, Run_OperationNameNotRegistered)
{
    ...
    RunResult run_result = Run("unregistered-operation", CreateSerializedUndefined());

    EXPECT_FALSE(run_result.success);
    EXPECT_THAT(run_result.error_message, testing::HasSubstr("Cannot find operation name"));
}
...
TEST_F(SharedStorageWorkletTest, Run_FunctionError)
{
    ...
    EXPECT_FALSE(run_result.success);
    EXPECT_THAT(run_result.error_message, testing::HasSubstr("ReferenceError: a is not defined"));
}
...
TEST_F(SharedStorageWorkletTest, Run_ValidateDataParamViaConsoleLog)
{
    ...
    RunResult run_result = Run("test-operation", CreateSerializedDict({ { "customField", "customValue" } }));

    EXPECT_TRUE(run_result.success);
    EXPECT_TRUE(run_result.error_message.empty());

    EXPECT_EQ(test_client_->observed_console_log_messages_.size(), 1u);
    EXPECT_EQ(test_client_->observed_console_log_messages_[0], "{\"customField\":\"customValue\"}");
}
...
TEST_F(SharedStorageWorkletTest, GlobalScopeObjectsAndFunctions_AfterAddModuleSuccess)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
      class TestClass {
        async run() {
...
TEST_F(SharedStorageWorkletTest, GlobalScopeObjectsAndFunctions_AfterAddModuleFailure)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
      class TestClass {
        async run() {
    ...
    EXPECT_THAT(add_module_result.error_message, testing::HasSubstr("ReferenceError: a is not defined"));
...
TEST_F(SharedStorageWorkletTest, Set_MissingKey)
{
    ...
    EXPECT_FALSE(run_result.success);
    EXPECT_THAT(run_result.error_message, testing::HasSubstr("2 arguments required, but only 0 present"));
...
TEST_F(SharedStorageWorkletTest, Set_InvalidKey_Empty)
{
    ...
    EXPECT_FALSE(run_result.success);
    EXPECT_THAT(run_result.error_message, testing::HasSubstr("Length of the \"key\" parameter is not valid"));
...
TEST_F(SharedStorageWorkletTest, Set_MissingValue)
{
    ...
    EXPECT_FALSE(run_result.success);
    EXPECT_THAT(run_result.error_message, testing::HasSubstr("2 arguments required, but only 1 present"));
...
TEST_F(SharedStorageWorkletTest, Set_InvalidValue_TooLong)
{
    ...
    EXPECT_FALSE(run_result.success);
    EXPECT_THAT(run_result.error_message, testing::HasSubstr("Length of the \"value\" parameter is not valid"));
...
TEST_F(SharedStorageWorkletTest, Set_InvalidOptions)
{
    ...
    EXPECT_FALSE(run_result.success);
    EXPECT_THAT(run_result.error_message, testing::HasSubstr("The provided value is not of type 'SharedStorageSetMethodOptions'"));
...
TEST_F(SharedStorageWorkletTest, Set_ClientError)
{
    ...
    network::mojom::SharedStorageSetMethodPtr& observed_params = test_client_->observed_update_params_[0]->get_set_method();
    EXPECT_EQ(observed_params->key, u"key0");
    EXPECT_EQ(observed_params->value, u"value0");
}

TEST_F(SharedStorageWorkletTest, InterestGroups_RunAdAuctionPermissionsPolicyNotAllowed)
{
    permissions_policy_state_ = blink::mojom::SharedStorageWorkletPermissionsPolicyState::New(
        /*private_aggregation_allowed=*/true,
        /*join_ad_interest_group_allowed=*/true,
    ...
    test_client_->interest_groups_result_ = blink::mojom::GetInterestGroupsResult::NewGroups({});
...
TEST_F(SharedStorageWorkletTest, InterestGroups_ClientError)
{
    ...
    test_client_->interest_groups_result_ = blink::mojom::GetInterestGroupsResult::NewErrorMessage("error 123");
...
TEST_F(SharedStorageWorkletTest, InterestGroups)
{
    ...
    blink::mojom::BiddingBrowserSignalsPtr bidding_browser_signals = blink::mojom::BiddingBrowserSignals::New(
        /*join_count=*/1,
    ...
    ig.all_sellers_capabilities = { blink::SellerCapabilities::kInterestGroupCounts, blink::SellerCapabilities::kLatencyStats };
    ig.seller_capabilities = { { url::Origin::Create(GURL("https://example.org")), { blink::SellerCapabilities::kInterestGroupCounts } } };
    ...
    ig.trusted_bidding_signals_slot_size_mode = InterestGroup::TrustedBiddingSignalsSlotSizeMode::kAllSlotsRequestedSizes;
    ig.max_trusted_bidding_signals_url_length = 100;
    ig.trusted_bidding_signals_coordinator = url::Origin::Create(GURL("https://example.test"));
    ig.user_bidding_signals = "\"hello\"";
    ig.ads = { { blink::InterestGroup::Ad(GURL("https://example.com/train"), "\"metadata\"", "sizegroup", "bid", "bsid",
                     std::vector<std::string> { "selectable_id1", "selectable_id2" }, "ad_render_id",
    ...
    ig.ad_sizes = { { "small", AdSize(100, AdSize::LengthUnit::kPixels, 5, AdSize::LengthUnit::kScreenHeight) } };
    ...
    ig.aggregation_coordinator_origin = url::Origin::Create(GURL("https://aggegator.example.org"));

    blink::mojom::StorageInterestGroupPtr storage_interest_group = blink::mojom::StorageInterestGroup::New(std::move(ig), std::move(bidding_browser_signals),
        /*joining_origin=*/
    ...
    test_client_->interest_groups_result_ = blink::mojom::GetInterestGroupsResult::NewGroups(std::move(groups));
...
TEST_F(SharedStorageWorkletTest, Set_IgnoreIfPresent_True)
{
    ...
    EXPECT_TRUE(test_client_->observed_update_params_[0]->get_set_method()->ignore_if_present);
    EXPECT_TRUE(test_client_->observed_update_params_[1]->get_set_method()->ignore_if_present);
    EXPECT_TRUE(test_client_->observed_update_params_[2]->get_set_method()->ignore_if_present);
}
...
TEST_F(SharedStorageWorkletTest, Set_IgnoreIfPresent_False)
{
    ...
    EXPECT_FALSE(test_client_->observed_update_params_[0]->get_set_method()->ignore_if_present);
    EXPECT_FALSE(test_client_->observed_update_params_[1]->get_set_method()->ignore_if_present);
    EXPECT_FALSE(test_client_->observed_update_params_[2]->get_set_method()->ignore_if_present);
    EXPECT_FALSE(test_client_->observed_update_params_[3]->get_set_method()->ignore_if_present);
    EXPECT_FALSE(test_client_->observed_update_params_[4]->get_set_method()->ignore_if_present);
}
...
TEST_F(SharedStorageWorkletTest, Set_KeyAndValueConvertedToString)
{
    ...
    network::mojom::SharedStorageSetMethodPtr& observed_params_0 = test_client_->observed_update_params_[0]->get_set_method();
    EXPECT_EQ(observed_params_0->key, u"123");
    EXPECT_EQ(observed_params_0->value, u"456");

    network::mojom::SharedStorageSetMethodPtr& observed_params_1 = test_client_->observed_update_params_[1]->get_set_method();
    EXPECT_EQ(observed_params_1->key, u"null");
    EXPECT_EQ(observed_params_1->value, u"null");

    network::mojom::SharedStorageSetMethodPtr& observed_params_2 = test_client_->observed_update_params_[2]->get_set_method();
    EXPECT_EQ(observed_params_2->key, u"undefined");
    EXPECT_EQ(observed_params_2->value, u"undefined");

    network::mojom::SharedStorageSetMethodPtr& observed_params_3 = test_client_->observed_update_params_[3]->get_set_method();
...
TEST_F(SharedStorageWorkletTest, Append_ClientError)
{
    ...
    EXPECT_EQ(test_client_->observed_update_params_.size(), 1u);
    network::mojom::SharedStorageAppendMethodPtr& observed_params = test_client_->observed_update_params_[0]->get_append_method();
    EXPECT_EQ(observed_params->key, u"key0");
    EXPECT_EQ(observed_params->value, u"value0");
}
...
TEST_F(SharedStorageWorkletTest, Delete_MissingKey)
{
    ...
    EXPECT_FALSE(run_result.success);
    EXPECT_THAT(run_result.error_message, testing::HasSubstr("1 argument required, but only 0 present"));
...
TEST_F(SharedStorageWorkletTest, Delete_ClientError)
{
    ...
    EXPECT_EQ(test_client_->observed_update_params_.size(), 1u);
    network::mojom::SharedStorageDeleteMethodPtr& observed_params = test_client_->observed_update_params_[0]->get_delete_method();
    EXPECT_EQ(observed_params->key, u"key0");
}
...
TEST_F(SharedStorageWorkletTest, Get_ClientError)
{
    ...
    test_client_->get_result_ = GetResult { .status = blink::mojom::SharedStorageGetStatus::kError, .error_message = "error 123", .value = std::u16string() };
...
TEST_F(SharedStorageWorkletTest, Get_NotFound)
{
    ...
    test_client_->get_result_
        = GetResult { .status = blink::mojom::SharedStorageGetStatus::kNotFound, .error_message = std::string(), .value = std::u16string() };
...
TEST_F(SharedStorageWorkletTest, Get_Success)
{
    ...
    test_client_->get_result_ = GetResult { .status = blink::mojom::SharedStorageGetStatus::kSuccess, .error_message = std::string(), .value = u"value0" };
...
TEST_F(SharedStorageWorkletTest, Length_Success)
{
    ...
    test_client_->length_result_ = LengthResult { .success = true, .error_message = std::string(), .length = 123 };
...
TEST_F(SharedStorageWorkletTest, Entries_OneEmptyBatch_Success)
{
    ...
    shared_storage_worklet_service_->RunOperation("test-operation", CreateSerializedUndefined(), MaybeInitPAOperationDetails(), run_future.GetCallback());
    ...
    mojo::Remote<blink::mojom::SharedStorageEntriesListener> listener = test_client_->TakeEntriesListenerAtFront();
...
TEST_F(SharedStorageWorkletTest, Entries_FirstBatchError_Failure)
{
    ...
    mojo::Remote<blink::mojom::SharedStorageEntriesListener> listener = test_client_->TakeEntriesListenerAtFront();
    listener->DidReadEntries(
        /*success=*/false, /*error_message=*/"Internal error 12345", CreateBatchResult({}),
...
TEST_F(SharedStorageWorkletTest, Entries_TwoBatches_Success)
{
    ...
    mojo::Remote<blink::mojom::SharedStorageEntriesListener> listener = test_client_->TakeEntriesListenerAtFront();
    listener->DidReadEntries(
        /*success=*/true, /*error_message=*/ {}, CreateBatchResult({ { u"key0", u"value0" } }),
    ...
    listener->DidReadEntries(
        /*success=*/true, /*error_message=*/ {}, CreateBatchResult({ { u"key1", u"value1" }, { u"key2", u"value2" } }),
...
TEST_F(SharedStorageWorkletTest, Keys_OneBatch_Success)
{
    ...
    mojo::Remote<blink::mojom::SharedStorageEntriesListener> listener = test_client_->TakeKeysListenerAtFront();
    listener->DidReadEntries(
        /*success=*/true, /*error_message=*/ {}, CreateBatchResult({ { u"key0", u"value0" }, { u"key1", u"value1" } }),
...
TEST_F(SharedStorageWorkletTest, Keys_ManuallyCallNext)
{
    ...
    mojo::Remote<blink::mojom::SharedStorageEntriesListener> listener = test_client_->TakeKeysListenerAtFront();
    listener->DidReadEntries(
        /*success=*/true, /*error_message=*/ {}, CreateBatchResult({ { u"key0", /*value=*/ {} } }),
    ...
    listener->DidReadEntries(
        /*success=*/true, /*error_message=*/ {}, CreateBatchResult({ { u"key1", /*value=*/ {} }, { u"key2", /*value=*/ {} } }),
        /*has_more_entries=*/true, /*total_queued_to_send=*/4);
    shared_storage_worklet_service_.FlushForTesting();

    EXPECT_FALSE(run_future.IsReady());
    EXPECT_EQ(test_client_->observed_console_log_messages_.size(), 1u);
    EXPECT_EQ(test_client_->observed_console_log_messages_[0], "{\"done\":false,\"value\":\"key2\"}");

    listener->DidReadEntries(
        /*success=*/true, /*error_message=*/ {}, CreateBatchResult({ { u"key3", /*value=*/ {} } }),
    ...
    EXPECT_EQ(test_client_->observed_console_log_messages_[1], "{\"done\":false,\"value\":\"key3\"}");
...
TEST_F(SharedStorageWorkletTest, Values_ManuallyCallNext)
{
    ...
    EXPECT_FALSE(run_future.IsReady());
    EXPECT_EQ(test_client_->observed_console_log_messages_.size(), 1u);
    EXPECT_EQ(test_client_->observed_console_log_messages_[0], "{\"done\":false,\"value\":\"value2\"}");

    listener->DidReadEntries(
        /*success=*/true, /*error_message=*/ {}, CreateBatchResult({ { u"key3", u"value3" } }),
        /*has_more_entries=*/false, /*total_queued_to_send=*/4);
    ...
    EXPECT_EQ(test_client_->observed_console_log_messages_.size(), 4u);
    EXPECT_EQ(test_client_->observed_console_log_messages_[1], "{\"done\":false,\"value\":\"value3\"}");
    EXPECT_EQ(test_client_->observed_console_log_messages_[2], "{\"done\":true}");
    EXPECT_EQ(test_client_->observed_console_log_messages_[3], "{\"done\":true}");
}
...
TEST_F(SharedStorageWorkletTest, RemainingBudget_ClientError)
{
    ...
    test_client_->remaining_budget_result_ = RemainingBudgetResult { .success = false, .error_message = "error 123", .bits = 0 };
...
TEST_F(SharedStorageWorkletTest, RemainingBudget_Success)
{
    ...
    test_client_->remaining_budget_result_ = RemainingBudgetResult { .success = true, .error_message = std::string(), .bits = 2.0 };
...
TEST_F(SharedStorageWorkletTest, ContextAttribute_Undefined)
{
    ...
    histogram_tester_.ExpectUniqueSample("Storage.SharedStorage.Worklet.Context.IsDefined", /*sample=*/false,
...
TEST_F(SharedStorageWorkletTest, ContextAttribute_String)
{
    ...
    EXPECT_EQ(test_client_->observed_console_log_messages_.size(), 1u);
    EXPECT_EQ(test_client_->observed_console_log_messages_[0], "some embedder context");

    histogram_tester_.ExpectUniqueSample("Storage.SharedStorage.Worklet.Context.IsDefined", /*sample=*/true,
        /*expected_bucket_count=*/1);
}
...
TEST_F(SharedStorageWorkletTest, AsyncFailuresDuringOperation_OperationSucceed)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
      class TestClass {
        async run() {
...
TEST_F(SharedStorageWorkletTest, Crypto_GetRandomValues)
{
    ...
    EXPECT_NE(test_client_->observed_console_log_messages_[0], test_client_->observed_console_log_messages_[1]);
}
...
TEST_F(SharedStorageWorkletTest, TextEncoderDecoderAndSubtleCryptoEncryptDecrypt)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
      class TestClass {
        async run() {
...
private:
    ScopedSharedStorageWebLocksForTest shared_storage_web_locks_runtime_enabled_feature { /*enabled=*/false };
};

TEST_F(SharedStorageWebLocksDisabledTest, InterfaceAndObjectExposure_DuringAddModule)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
    var expectedUndefinedVariables = [
      "SharedStorageWorkletNavigator",
...
class SharedStorageInterestGroupsDisabledTest : public SharedStorageWorkletTest {
private:
    ScopedInterestGroupsInSharedStorageWorkletForTest interest_groups_in_shared_storage_worklet_runtime_enabled_feature { /*enabled=*/false };
};
...
TEST_F(SharedStorageInterestGroupsDisabledTest, InterestGroups)
{
    ...
    EXPECT_FALSE(add_module_result.success);
    EXPECT_THAT(add_module_result.error_message, testing::HasSubstr("ReferenceError: interestGroups is not defined"));
}
...
class SharedStoragePrivateAggregationDisabledTest : public SharedStorageWorkletTest {
public:
    SharedStoragePrivateAggregationDisabledTest()
    {
        private_aggregation_feature_.InitAndDisableFeature(blink::features::kPrivateAggregationApi);
    }
...
TEST_F(SharedStoragePrivateAggregationDisabledTest, GlobalScopeObjectsAndFunctions_DuringAddModule)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
    var expectedObjects = [
      "console",
...
TEST_F(SharedStoragePrivateAggregationDisabledTest, GlobalScopeObjectsAndFunctions_AfterAddModuleSuccess)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
      class TestClass {
        async run() {
...
TEST_F(SharedStoragePrivateAggregationDisabledTest, GlobalScopeObjectsAndFunctions_AfterAddModuleFailure)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
      class TestClass {
        async run() {
...
public:
    SharedStoragePrivateAggregationTest()
    {
        private_aggregation_feature_.InitWithFeaturesAndParameters(
            /*enabled_features=*/
            { { blink::features::kPrivateAggregationApi, { { "enabled_in_shared_storage", "true" } } } },
            /*disabled_features=*/ {});
    }

    // `error_message` being `nullptr` indicates no error is expected.
    void ExecuteScriptAndValidateContribution(const std::string& script_body, absl::uint128 expected_bucket, int expected_value,
        mojom::blink::DebugModeDetailsPtr expected_debug_mode_details = mojom::blink::DebugModeDetails::New(),
        std::optional<uint64_t> filtering_id = std::nullopt, int filtering_id_max_bytes = 1, std::string* error_message = nullptr)
    {
        AddModuleResult add_module_result
            = AddModule(/*script_content=*/base::StrCat({ "class TestClass { async run() {", script_body, "}}; register(\"test-operation\", TestClass);" }));

        EXPECT_CALL(*mock_private_aggregation_host_, ContributeToHistogram)
            .WillOnce(testing::Invoke([&](Vector<blink::mojom::blink::AggregatableReportHistogramContributionPtr> contributions) {
        ...
        if (expected_debug_mode_details->is_enabled) {
            EXPECT_CALL(*mock_private_aggregation_host_, EnableDebugMode).WillOnce(testing::Invoke([&](mojom::blink::DebugKeyPtr debug_key) {
                EXPECT_TRUE(debug_key == expected_debug_mode_details->debug_key);
            }));
        }

        RunResult run_result = Run("test-operation", CreateSerializedUndefined(), filtering_id_max_bytes);
        ...
        std::vector<mojom::WebFeature> expected_use_counters
            = { mojom::WebFeature::kPrivateAggregationApiAll, mojom::WebFeature::kPrivateAggregationApiSharedStorage };
        if (expected_debug_mode_details->is_enabled) {
            expected_use_counters.push_back(mojom::WebFeature::kPrivateAggregationApiEnableDebugMode);
        }
        if (filtering_id.has_value()) {
            expected_use_counters.push_back(mojom::WebFeature::kPrivateAggregationApiFilteringIds);
        }

        EXPECT_THAT(test_client_->observed_use_counters_, testing::UnorderedElementsAreArray(expected_use_counters));

        mock_private_aggregation_host_->FlushForTesting();
    }

    std::string ExecuteScriptReturningError(
        const std::string& script_body, std::vector<mojom::WebFeature> expect_use_counters = {}, int filtering_id_max_bytes = 1)
    {
        AddModuleResult add_module_result
            = AddModule(/*script_content=*/base::StrCat({ "class TestClass { async run() {", script_body, "}}; register(\"test-operation\", TestClass);" }));
        ...
        if (mock_private_aggregation_host_) {
            EXPECT_CALL(*mock_private_aggregation_host_, ContributeToHistogram).Times(0);
        ...
        EXPECT_THAT(test_client_->observed_use_counters_, testing::UnorderedElementsAreArray(expect_use_counters));
...
TEST_F(SharedStoragePrivateAggregationTest, InterfaceAndObjectExposure_DuringAddModule)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
    // This will succeed.
    PrivateAggregation;
    ...
    EXPECT_FALSE(add_module_result.success);
    EXPECT_THAT(add_module_result.error_message, testing::HasSubstr("privateAggregation cannot be accessed during addModule()"));
}

TEST_F(SharedStoragePrivateAggregationTest, InterfaceAndObjectExposure_AfterAddModuleSuccess)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
      class TestClass {
        async run() {
...
TEST_F(SharedStoragePrivateAggregationTest, InterfaceAndObjectExposure_AfterAddModuleFailure)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
      class TestClass {
        async run() {
...
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram({bucket: 1n, value: 2});",
        /*expected_bucket=*/1, /*expected_value=*/2);
}

TEST_F(SharedStoragePrivateAggregationTest, ZeroBucket)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram({bucket: 0n, value: 2});",
        /*expected_bucket=*/0, /*expected_value=*/2);
}

TEST_F(SharedStoragePrivateAggregationTest, ZeroValue)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram({bucket: 1n, value: 0});",
        /*expected_bucket=*/1, /*expected_value=*/0);
}

TEST_F(SharedStoragePrivateAggregationTest, LargeBucket)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram("
...
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram({bucket: 1n, value: 2.3});",
        /*expected_bucket=*/1, /*expected_value=*/2);
}

TEST_F(SharedStoragePrivateAggregationTest, PrivateAggregationPermissionsPolicyNotAllowed_Rejected)
{
    permissions_policy_state_ = blink::mojom::SharedStorageWorkletPermissionsPolicyState::New(
    ...
    std::string error_str = ExecuteScriptReturningError("privateAggregation.contributeToHistogram({bucket: 1n, value: 2});",
        /*expect_use_counters=*/ { mojom::WebFeature::kPrivateAggregationApiAll, mojom::WebFeature::kPrivateAggregationApiSharedStorage });
...
{
    std::string error_str = ExecuteScriptReturningError("privateAggregation.contributeToHistogram({bucket: "
                                                        "340282366920938463463374607431768211456n, value: 2});",
        /*expect_use_counters=*/ { mojom::WebFeature::kPrivateAggregationApiAll, mojom::WebFeature::kPrivateAggregationApiSharedStorage });

    EXPECT_THAT(error_str, testing::HasSubstr("contribution['bucket'] is negative or does not fit in 128 bits"));
}
...
{
    std::string error_str = ExecuteScriptReturningError("privateAggregation.contributeToHistogram({bucket: 1, value: 2});",
        /*expect_use_counters=*/ {});
...
{
    std::string error_str = ExecuteScriptReturningError("privateAggregation.contributeToHistogram({bucket: 1n, value: -1});",
        /*expect_use_counters=*/ { mojom::WebFeature::kPrivateAggregationApiAll, mojom::WebFeature::kPrivateAggregationApiSharedStorage });

    EXPECT_THAT(error_str, testing::HasSubstr("contribution['value'] is negative"));
}

TEST_F(SharedStoragePrivateAggregationTest, InvalidEnableDebugModeArgument_Rejected)
{
    // The debug key is not wrapped in a dictionary.
    std::string error_str = ExecuteScriptReturningError("privateAggregation.enableDebugMode(1234n);",
        /*expect_use_counters=*/ {});
...
TEST_F(SharedStoragePrivateAggregationTest, EnableDebugModeCalledTwice_SecondCallFails)
{
    ...
    EXPECT_THAT(error_str, testing::HasSubstr("enableDebugMode may be called at most once"));
}

// Note that FLEDGE worklets have different behavior in this case.
TEST_F(SharedStoragePrivateAggregationTest, EnableDebugModeCalledAfterRequest_DoesntApply)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
    ...
    base::RepeatingClosure closure = base::BarrierClosure(2, run_loop.QuitClosure());
    ...
    EXPECT_CALL(*mock_private_aggregation_host_, ContributeToHistogram)
        .WillOnce(testing::Invoke([&](Vector<blink::mojom::blink::AggregatableReportHistogramContributionPtr> contributions) {
            ...
            contribute_to_histogram_pipe_id = mock_private_aggregation_host_->receiver_set().current_receiver();
            closure.Run();
        }));
    EXPECT_CALL(*mock_private_aggregation_host_, EnableDebugMode).WillOnce(testing::Invoke([&](blink::mojom::blink::DebugKeyPtr debug_key) {
        ASSERT_FALSE(debug_key.is_null());
        EXPECT_EQ(debug_key->value, 1234u);

        enable_debug_mode_pipe_id = mock_private_aggregation_host_->receiver_set().current_receiver();
        closure.Run();
    }));

    RunResult run_result = Run("contribute-to-histogram", CreateSerializedUndefined());
    EXPECT_TRUE(run_result.success);
...
TEST_F(SharedStoragePrivateAggregationTest, GlobalScopeDeletedBeforeOperationCompletes_ContributionsStillFlushed)
{
    AddModuleResult add_module_result = AddModule(/*script_content=*/R"(
      class TestClass {
        async run() {
    ...
    shared_storage_worklet_service_->RunOperation("test-operation", CreateSerializedUndefined(), MaybeInitPAOperationDetails(), base::DoNothing());
...
class SharedStoragePrivateAggregationFilteringIdTest : public SharedStoragePrivateAggregationTest {
public:
...
private:
    ...
    // enable both.
    base::test::ScopedFeatureList scoped_base_feature_ { features::kPrivateAggregationApiFilteringIds };
    ScopedPrivateAggregationApiFilteringIdsForTest scoped_rte_feature { /*enabled=*/true };
};
...
TEST_F(SharedStoragePrivateAggregationFilteringIdTest, FilteringIdWithDebugMode)
{
    ExecuteScriptAndValidateContribution(
...
TEST_F(SharedStoragePrivateAggregationFilteringIdTest, NoFilteringIdSpecified_FilteringIdNull)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram({bucket: 1n, value: 2});",
        /*expected_bucket=*/1, /*expected_value=*/2,
        /*expected_debug_mode_details=*/mojom::blink::DebugModeDetails::New(),
...
TEST_F(SharedStoragePrivateAggregationFilteringIdTest, ExplicitDefaultFilteringId_FilteringIdNotNull)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram("
...
TEST_F(SharedStoragePrivateAggregationFilteringIdTest, MaxFilteringIdForByteSize_Success)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram("
...
TEST_F(SharedStoragePrivateAggregationFilteringIdTest, FilteringIdTooBigForByteSize_Error)
{
    std::string error_str = ExecuteScriptReturningError("privateAggregation.contributeToHistogram("
...
TEST_F(SharedStoragePrivateAggregationFilteringIdTest, FilteringIdNegative_Error)
{
    std::string error_str = ExecuteScriptReturningError("privateAggregation.contributeToHistogram("
...
TEST_F(SharedStoragePrivateAggregationFilteringIdTest, NoFilteringIdWithCustomByteSize)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram({bucket: 1n, value: 2});",
        /*expected_bucket=*/1, /*expected_value=*/2,
        /*expected_debug_mode_details=*/mojom::blink::DebugModeDetails::New(),
...
TEST_F(SharedStoragePrivateAggregationFilteringIdTest, FilteringIdWithCustomByteSize_Success)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram("
                                         "{bucket: 1n, value: 2, filteringId: 3n});",
        /*expected_bucket=*/1, /*expected_value=*/2,
...
TEST_F(SharedStoragePrivateAggregationFilteringIdTest, MaxFilteringIdWithCustomByteSize_Success)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram("
...
TEST_F(SharedStoragePrivateAggregationFilteringIdTest, TooBigFilteringIdWithCustomByteSize_Error)
{
    std::string error_str = ExecuteScriptReturningError("privateAggregation.contributeToHistogram("
...
TEST_F(SharedStoragePrivateAggregationFilteringIdTest, TooBigFilteringIdWithMaxByteSize_Error)
{
    std::string error_str = ExecuteScriptReturningError("privateAggregation.contributeToHistogram("
...
class SharedStoragePrivateAggregationFilteringIdDisabledTest : public SharedStoragePrivateAggregationTest {
public:
    SharedStoragePrivateAggregationFilteringIdDisabledTest()
    {
        scoped_base_feature_.InitAndDisableFeature(features::kPrivateAggregationApiFilteringIds);
    }
...
private:
    ...
    ScopedPrivateAggregationApiFilteringIdsForTest scoped_rte_feature { /*enabled=*/false };
};

TEST_F(SharedStoragePrivateAggregationFilteringIdDisabledTest, ValidFilteringId_Ignored)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram("
                                         "{bucket: 1n, value: 2, filteringId: 3n});",
        /*expected_bucket=*/1, /*expected_value=*/2,
...
TEST_F(SharedStoragePrivateAggregationFilteringIdDisabledTest, InvalidFilteringId_Ignored)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram("
...
TEST_F(SharedStoragePrivateAggregationFilteringIdDisabledTest, CustomFilteringIdMaxBytes_Ignored)
{
    ExecuteScriptAndValidateContribution("privateAggregation.contributeToHistogram({bucket: 1n, value: 2});",
        /*expected_bucket=*/1, /*expected_value=*/2,
        /*expected_debug_mode_details=*/mojom::blink::DebugModeDetails::New(),
...
TEST_F(SharedStorageWorkletThreadTest, DedicatedBackingThread)
{
    ...
    feature_list.InitAndDisableFeature(blink::features::kSharedStorageWorkletSharedBackingThreadImplementation);
    ...
    EXPECT_NE(&thread1->GetWorkerBackingThread(), &thread2->GetWorkerBackingThread());
    ...
    thread_startup_data.atomics_wait_mode = WorkerBackingThreadStartupData::AtomicsWaitMode::kAllow;

    thread1->Start(MakeTestGlobalScopeCreationParams(), thread_startup_data, std::make_unique<WorkerDevToolsParams>());
    thread2->Start(MakeTestGlobalScopeCreationParams(), thread_startup_data, std::make_unique<WorkerDevToolsParams>());
...
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(blink::features::kSharedStorageWorkletSharedBackingThreadImplementation);
    ...
    auto thread1 = SharedStorageWorkletThread::Create(reporting_proxy1);
    auto thread2 = SharedStorageWorkletThread::Create(reporting_proxy2);
    EXPECT_EQ(&thread1->GetWorkerBackingThread(), &thread2->GetWorkerBackingThread());

    // Start and terminate the threads, so that the test can terminate gracefully.
    thread1->Start(MakeTestGlobalScopeCreationParams(),
        /*thread_startup_data=*/std::nullopt, std::make_unique<WorkerDevToolsParams>());
