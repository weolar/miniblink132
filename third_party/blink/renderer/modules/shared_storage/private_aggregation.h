...
public:
    struct OperationState : public GarbageCollected<OperationState> {
        explicit OperationState(ContextLifecycleNotifier* notifier, size_t filtering_id_max_bytes)
        ...
        HeapMojoRemote<mojom::blink::PrivateAggregationHost> private_aggregation_host;
    ...
    void contributeToHistogram(ScriptState*, const PrivateAggregationHistogramContribution*, ExceptionState&);
    void enableDebugMode(ScriptState*, ExceptionState&);
    void enableDebugMode(ScriptState*, const PrivateAggregationDebugModeOptions*, ExceptionState&);

    void OnOperationStarted(int64_t operation_id, mojom::blink::PrivateAggregationOperationDetailsPtr pa_operation_details);
...
private:
    ...
    HeapHashMap<int64_t, Member<OperationState>, IntWithZeroKeyHashTraits<int64_t>> operation_states_;
};
