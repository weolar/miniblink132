...
class MODULES_EXPORT SharedStorageWorkletGlobalScope final : public WorkletGlobalScope,
                                                             public Supplementable<SharedStorageWorkletGlobalScope>,
                                                             public mojom::blink::SharedStorageWorkletService {
    DEFINE_WRAPPERTYPEINFO();

public:
    SharedStorageWorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams> creation_params, WorkerThread* thread);
    ~SharedStorageWorkletGlobalScope() override;

    void BindSharedStorageWorkletService(mojo::PendingReceiver<mojom::blink::SharedStorageWorkletService> receiver, base::OnceClosure disconnect_handler);

    // SharedStorageWorkletGlobalScope IDL
    void Register(const String& name, V8NoArgumentConstructor* operation_ctor, ExceptionState&);

    // WorkletGlobalScope implementation:
    void OnConsoleApiMessage(mojom::ConsoleMessageLevel level, const String& message, SourceLocation* location) override;

    bool IsSharedStorageWorkletGlobalScope() const override
    {
        return true;
    }
    ...
    WorkletToken GetWorkletToken() const override
    {
        return token_;
    }
    ...
    void Initialize(mojo::PendingAssociatedRemote<mojom::blink::SharedStorageWorkletServiceClient> client,
        mojom::blink::SharedStorageWorkletPermissionsPolicyStatePtr permissions_policy_state, const String& embedder_context) override;
    void AddModule(mojo::PendingRemote<network::mojom::blink::URLLoaderFactory> pending_url_loader_factory, const KURL& script_source_url,
        AddModuleCallback callback) override;
    void RunURLSelectionOperation(const String& name, const Vector<KURL>& urls, BlinkCloneableMessage serialized_data,
        mojom::blink::PrivateAggregationOperationDetailsPtr pa_operation_details, RunURLSelectionOperationCallback callback) override;
    void RunOperation(const String& name, BlinkCloneableMessage serialized_data, mojom::blink::PrivateAggregationOperationDetailsPtr pa_operation_details,
    ...
    ScriptPromise<IDLSequence<StorageInterestGroup>> interestGroups(ScriptState*, ExceptionState&);
    ...
    mojom::blink::SharedStorageWorkletServiceClient* GetSharedStorageWorkletServiceClient()
    {
        return client_.get();
    }

    const String& embedder_context() const
    {
        return embedder_context_;
    }

    const mojom::blink::SharedStorageWorkletPermissionsPolicyStatePtr& permissions_policy_state() const
    {
        return permissions_policy_state_;
    }

private:
    void OnModuleScriptDownloaded(const KURL& script_source_url, mojom::blink::SharedStorageWorkletService::AddModuleCallback callback,
        std::unique_ptr<std::string> response_body, std::string error_message, network::mojom::URLResponseHeadPtr response_head);
    ...
    bool PerformCommonOperationChecks(const String& operation_name, String& error_message, SharedStorageOperationDefinition*& operation_definition);
    ...
    base::OnceClosure StartOperation(mojom::blink::PrivateAggregationOperationDetailsPtr pa_operation_details);
    ...
    HeapMojoReceiver<mojom::blink::SharedStorageWorkletService, SharedStorageWorkletGlobalScope> receiver_ { this, this };
    ...
    HeapHashMap<String, Member<SharedStorageOperationDefinition>> operation_definition_map_;
    ...
    HeapMojoAssociatedRemote<mojom::blink::SharedStorageWorkletServiceClient> client_ { this };

    // The state of the permissions policy features applicable to the shared
    // storage worklet.
    mojom::blink::SharedStorageWorkletPermissionsPolicyStatePtr permissions_policy_state_;

    const SharedStorageWorkletToken token_;
};

template <> struct DowncastTraits<SharedStorageWorkletGlobalScope> {
