...
v8::Local<v8::Value> IDBRecordArray::ToV8(ScriptState* script_state, IDBRecordArray source)
{
    ...
    CHECK(source.index_keys.size() == source.primary_keys.size() || source.index_keys.empty());
    ...
    for (wtf_size_t i = 0; i < record_count; ++i) {
        ...
        records.emplace_back(MakeGarbageCollected<IDBRecord>(std::move(source.primary_keys[i]), std::move(source.values[i]), std::move(index_key)));
    }
    return ToV8Traits<IDLSequence<IDBRecord>>::ToV8(script_state, std::move(records));
}
