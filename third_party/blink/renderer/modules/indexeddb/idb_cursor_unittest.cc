...
public:
    explicit MockCursorImpl(mojo::PendingAssociatedReceiver<mojom::blink::IDBCursor> receiver)
        : receiver_(this, std::move(receiver))
    {
        receiver_.set_disconnect_handler(WTF::BindOnce(&MockCursorImpl::CursorDestroyed, WTF::Unretained(this)));
    }

    void Prefetch(int32_t count, mojom::blink::IDBCursor::PrefetchCallback callback) override
    {
        ++prefetch_calls_;
    ...
    void Advance(uint32_t count, mojom::blink::IDBCursor::AdvanceCallback callback) override
    {
        ++advance_calls_;
        std::move(callback).Run(mojom::blink::IDBCursorResult::NewEmpty(true));
    }

    void Continue(std::unique_ptr<IDBKey> key, std::unique_ptr<IDBKey> primary_key, mojom::blink::IDBCursor::ContinueCallback callback) override
    {
        ++continue_calls_;
        std::move(callback).Run(mojom::blink::IDBCursorResult::NewEmpty(true));
    }
    ...
    int last_prefetch_count()
    {
        return last_prefetch_count_;
    }
    int reset_calls()
    {
        return reset_calls_;
    ...
    int last_used_count()
    {
        return last_used_count_;
    }
    int advance_calls()
    {
        return advance_calls_;
    }
    int continue_calls()
    {
        return continue_calls_;
    }
    bool destroyed()
    {
        return destroyed_;
...
public:
    ...
    void SetUpCursor(V8TestingScope& scope)
    {
        ...
        IDBTransaction* transaction = IDBTransaction::CreateNonVersionChange(scope.GetScriptState(), std::move(transaction_remote), kTransactionId,
            transaction_scope, mojom::IDBTransactionMode::ReadOnly, mojom::IDBTransactionDurability::Relaxed, db);
        ...
        IDBObjectStore* store = MakeGarbageCollected<IDBObjectStore>(store_metadata, transaction);

        // Create a `request` and a `source`.
        IDBRequest* request = IDBRequest::Create(scope.GetScriptState(), store, transaction, IDBRequest::AsyncTraceState());
        IDBCursor::Source* source = MakeGarbageCollected<IDBCursor::Source>(request->source(scope.GetScriptState())->GetAsIDBObjectStore());

        // Set up mojo endpoints.
        mojo::AssociatedRemote<mojom::blink::IDBCursor> remote;
        mock_cursor_ = std::make_unique<MockCursorImpl>(remote.BindNewEndpointAndPassDedicatedReceiver());
        cursor_ = MakeGarbageCollected<IDBCursor>(remote.Unbind(), mojom::blink::IDBCursorDirection::NextNoDuplicate, request, source, transaction);
    }
    ...
    for (int repetitions = 0; repetitions < kPrefetchRepetitions; ++repetitions) {
        ...
        for (int i = 0; i < prefetch_count; ++i) {
            ...
            for (int j = 0; j < expected_key + i; ++j) {
                blob_info.emplace_back(WebBlobInfo::BlobForTesting(WebString("blobuuid"), "text/plain", 123));
            }
            values.emplace_back(std::make_unique<IDBValue>(Vector<char>(), std::move(blob_info)));
        }
        cursor_->SetPrefetchData(std::move(keys), std::move(primary_keys), std::move(values));
        ...
        for (int i = 0; i < prefetch_count; ++i) {
            ...
            EXPECT_EQ(static_cast<int>(cursor_->prefetch_keys_.size()), prefetch_count - cursor_->used_prefetches_);
        }
    }
...
TEST_F(IDBCursorTest, AdvancePrefetchTest)
{
    ...
    EXPECT_EQ(static_cast<int>(IDBCursor::kPrefetchContinueThreshold), mock_cursor_->continue_calls());
...
TEST_F(IDBCursorTest, PrefetchReset)
{
    ...
    Vector<std::unique_ptr<IDBValue>> values;
    for (int i = 0; i < prefetch_count; ++i) {
        values.emplace_back(std::make_unique<IDBValue>(Vector<char>(), Vector<WebBlobInfo>()));
    }
    cursor_->SetPrefetchData(std::move(keys), std::move(primary_keys), std::move(values));
