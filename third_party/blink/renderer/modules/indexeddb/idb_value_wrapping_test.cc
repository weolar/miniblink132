...
public:
    void ReadVarInt(const char* start, uint32_t buffer_size)
    {
        ...
        ASSERT_EQ(unwrapper.end_, buffer_end) << "ReadVarInt should not change end_";
        ASSERT_LE(unwrapper.current_, unwrapper.end_) << "ReadVarInt should not move current_ past end_";
    ...
    void ReadBytes(const char* start, uint32_t buffer_size)
    {
        ...
        ASSERT_LE(unwrapper.current_, unwrapper.end_) << "ReadBytes should not move current_ past end_";
        consumed_bytes_ = static_cast<uint32_t>(unwrapper.current_ - buffer_start);
    }
    ...
    unsigned read_varint()
    {
        return read_varint_;
    }
    const Vector<uint8_t>& read_bytes()
    {
        return read_bytes_;
    }
...
TEST(IDBValueUnwrapperTest, ReadBytes)
{
    ...
    helper.ReadBytes(reinterpret_cast<char*>(long_output.data()), long_output.size());
    EXPECT_TRUE(helper.success());
    ASSERT_EQ(256U, helper.read_bytes().size());
    ASSERT_EQ(long_output.size() - 1, helper.consumed_bytes());
    EXPECT_TRUE(std::equal(helper.read_bytes().begin(), helper.read_bytes().end(), long_output.data() + 2));
...
TEST(IDBValueUnwrapperTest, IsWrapped)
{
    ...
    IDBValueWrapper wrapper(scope.GetIsolate(), v8_true, SerializedScriptValue::SerializeOptions::kSerialize, non_throwable_exception_state);
    ...
    auto wrapped_value = std::make_unique<IDBValue>(std::move(wrapped_marker_buffer), std::move(blob_infos));
    ...
    for (wtf_size_t i = 0; i < 3; ++i) {
        auto mutant_value = std::make_unique<IDBValue>(Vector<char>(base::span(wrapped_marker_bytes).subspan(0, i)), std::move(blob_infos));
    ...
    for (wtf_size_t i = 0; i < 3; ++i) {
        for (int j = 0; j < 8; ++j) {
            ...
            copy[i] ^= mask;
            auto mutant_value = std::make_unique<IDBValue>(std::move(copy), std::move(blob_infos));
            mutant_value->SetIsolate(scope.GetIsolate());
            EXPECT_FALSE(IDBValueUnwrapper::IsWrapped(mutant_value.get()));
        }
    }
...
TEST(IDBValueUnwrapperTest, Compression)
{
    ...
    for (const auto& test_case : test_cases) {
        ...
        enable_feature_list.InitAndEnableFeatureWithParameters(
            features::kIndexedDBCompressValuesWithSnappy, { { "compression-threshold", base::StringPrintf("%i", test_case.compression_threshold) } });

        V8TestingScope scope;
        NonThrowableExceptionState non_throwable_exception_state;
        v8::Local<v8::Value> v8_value = v8::String::NewFromUtf8(scope.GetIsolate(), test_case.bytes.c_str(), v8::NewStringType::kNormal).ToLocalChecked();
        IDBValueWrapper wrapper(scope.GetIsolate(), v8_value, SerializedScriptValue::SerializeOptions::kSerialize, non_throwable_exception_state);
        ...
        ASSERT_EQ(test_case.should_compress, IDBValueUnwrapper::Decompress(buffer, &decompressed));

        // Round trip to v8 value.
        auto value = std::make_unique<IDBValue>(std::move(buffer), std::move(blob_infos));
        value->SetIsolate(scope.GetIsolate());
        auto serialized_string = value->CreateSerializedValue();
        EXPECT_TRUE(serialized_string->Deserialize(scope.GetIsolate())->StrictEquals(v8_value));
    }
}
...
TEST(IDBValueUnwrapperTest, Decompression)
{
    ...
    {
        base::test::ScopedFeatureList enable_feature_list { features::kIndexedDBCompressValuesWithSnappy };
        NonThrowableExceptionState non_throwable_exception_state;
        std::string bytes = base::StrCat(std::vector<std::string>(100u, "abcd"));
        v8_value = v8::String::NewFromUtf8(scope.GetIsolate(), bytes.c_str(), v8::NewStringType::kNormal).ToLocalChecked();
        IDBValueWrapper wrapper(scope.GetIsolate(), v8_value, SerializedScriptValue::SerializeOptions::kSerialize, non_throwable_exception_state);
        wrapper.DoneCloning();
...

    {
        ...
        disable_feature_list.InitAndDisableFeature(features::kIndexedDBCompressValuesWithSnappy);
        EXPECT_FALSE(base::FeatureList::IsEnabled(features::kIndexedDBCompressValuesWithSnappy));
