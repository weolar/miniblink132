...
IDBValue::IDBValue(
    Vector<char>&& data, Vector<WebBlobInfo> blob_info, Vector<mojo::PendingRemote<mojom::blink::FileSystemAccessTransferToken>> file_system_access_tokens)
    : data_(std::move(data))
...
void IDBValue::SetIsolate(v8::Isolate* isolate)
{
    ...
    if (external_allocated_size) {
        external_memory_accounter_.Increase(isolate_.get(), external_allocated_size);
    }
}

void IDBValue::SetData(Vector<char>&& new_data)
{
    DCHECK(isolate_) << "Value unwrapping should be done after an isolate has been associated";
...
{
    DCHECK_GT(blob_info_.size(), 0U) << "The IDBValue does not have any attached Blob";

    scoped_refptr<BlobDataHandle> return_value = blob_info_.back().GetBlobHandle();
    blob_info_.pop_back();
...
// static
std::unique_ptr<IDBValue> IDBValue::ConvertReturnValue(const mojom::blink::IDBReturnValuePtr& input)
{
    if (!input) {
