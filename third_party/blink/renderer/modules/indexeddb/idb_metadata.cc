...
IDBIndexMetadata::IDBIndexMetadata(const String& name, int64_t id, const IDBKeyPath& key_path, bool unique, bool multi_entry)
    : name(name)
    , id(id)
    , key_path(key_path)
    , unique(unique)
...
IDBObjectStoreMetadata::IDBObjectStoreMetadata(const String& name, int64_t id, const IDBKeyPath& key_path, bool auto_increment, int64_t max_index_id)
    : name(name)
    , id(id)
    , key_path(key_path)
...
scoped_refptr<IDBObjectStoreMetadata> IDBObjectStoreMetadata::CreateCopy() const
{
    scoped_refptr<IDBObjectStoreMetadata> copy = base::AdoptRef(new IDBObjectStoreMetadata(name, id, key_path, auto_increment, max_index_id));

    for (const auto& it : indexes) {
        IDBIndexMetadata* index = it.value.get();
        scoped_refptr<IDBIndexMetadata> index_copy
            = base::AdoptRef(new IDBIndexMetadata(index->name, index->id, index->key_path, index->unique, index->multi_entry));
...
IDBDatabaseMetadata::IDBDatabaseMetadata(const String& name, int64_t id, int64_t version, int64_t max_object_store_id, bool was_cold_open)
    : name(name)
    , id(id)
    , version(version)
