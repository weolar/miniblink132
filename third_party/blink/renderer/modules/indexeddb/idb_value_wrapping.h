...
public:
    ...
    IDBValueWrapper(v8::Isolate*, v8::Local<v8::Value>, SerializedScriptValue::SerializeOptions::WasmSerializationPolicy, ExceptionState&);
    ~IDBValueWrapper();
    ...
    Vector<mojo::PendingRemote<mojom::blink::FileSystemAccessTransferToken>> TakeFileSystemAccessTransferTokens()
    {
#if DCHECK_IS_ON()
        DCHECK(done_cloning_) << __func__ << " called before DoneCloning()";
    ...
    size_t DataLengthBeforeWrapInBytes()
    {
        return original_data_length_;
    }

    // MIME type used for Blobs that wrap IDBValues.
    static constexpr const char* kWrapMimeType = "application/vnd.blink-idb-value-wrapper";
...
public:
    ...
    // `wrapped_value`.
    static void Unwrap(Vector<char>&& wrapper_blob_content, IDBValue& wrapped_value);
