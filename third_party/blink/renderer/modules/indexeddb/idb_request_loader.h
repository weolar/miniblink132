...
class IDBRequestLoader : public GarbageCollected<IDBRequestLoader>, public FileReaderClient {
public:
    // |values| contains the unwrapped values; null |error| indicates success.
    using LoadCompleteCallback = base::OnceCallback<void(Vector<std::unique_ptr<IDBValue>>&& values, DOMException* error)>;
    ...
    IDBRequestLoader(Vector<std::unique_ptr<IDBValue>>&& values, ExecutionContext* execution_context, LoadCompleteCallback&& load_complete_callback);
