...
// static
bool StructTraits<blink::mojom::IDBDatabaseMetadataDataView, blink::IDBDatabaseMetadata>::Read(
    blink::mojom::IDBDatabaseMetadataDataView data, blink::IDBDatabaseMetadata* out)
{
    out->id = data.id();
    String name;
    ...
    MapDataView<int64_t, blink::mojom::IDBObjectStoreMetadataDataView> object_stores;
    data.GetObjectStoresDataView(&object_stores);
    out->object_stores.ReserveCapacityForSize(base::checked_cast<wtf_size_t>(object_stores.size()));
...
// static
bool StructTraits<blink::mojom::IDBIndexKeysDataView, blink::IDBIndexKeys>::Read(blink::mojom::IDBIndexKeysDataView data, blink::IDBIndexKeys* out)
{
...
// static
bool StructTraits<blink::mojom::IDBIndexMetadataDataView, scoped_refptr<blink::IDBIndexMetadata>>::Read(
    blink::mojom::IDBIndexMetadataDataView data, scoped_refptr<blink::IDBIndexMetadata>* out)
{
    scoped_refptr<blink::IDBIndexMetadata> value = blink::IDBIndexMetadata::Create();
    value->id = data.id();
    String name;
    if (!data.ReadName(&name))
        return false;
...
// static
blink::mojom::IDBKeyDataView::Tag UnionTraits<blink::mojom::IDBKeyDataView, std::unique_ptr<blink::IDBKey>>::GetTag(const std::unique_ptr<blink::IDBKey>& key)
{
    DCHECK(key.get());
    switch (key->GetType()) {
...
// static
bool UnionTraits<blink::mojom::IDBKeyDataView, std::unique_ptr<blink::IDBKey>>::Read(blink::mojom::IDBKeyDataView data, std::unique_ptr<blink::IDBKey>* out)
{
    switch (data.tag()) {
    ...
    case blink::mojom::IDBKeyDataView::Tag::kBinary: {
        ...
        *out = blink::IDBKey::CreateBinary(
            base::MakeRefCounted<base::RefCountedData<Vector<char>>>(Vector<char>(base::span(reinterpret_cast<const char*>(bytes.data()), bytes.size()))));
        return true;
    }
...
// static
const Vector<std::unique_ptr<blink::IDBKey>>& UnionTraits<blink::mojom::IDBKeyDataView, std::unique_ptr<blink::IDBKey>>::key_array(
...
// static
base::span<const uint8_t> UnionTraits<blink::mojom::IDBKeyDataView, std::unique_ptr<blink::IDBKey>>::binary(const std::unique_ptr<blink::IDBKey>& key)
{
    return base::as_byte_span(key->Binary()->data);
}

// static
base::span<const uint8_t> StructTraits<blink::mojom::IDBValueDataView, std::unique_ptr<blink::IDBValue>>::bits(const std::unique_ptr<blink::IDBValue>& input)
{
    return base::as_byte_span(input->Data());
}

// static
Vector<blink::mojom::blink::IDBExternalObjectPtr> StructTraits<blink::mojom::IDBValueDataView, std::unique_ptr<blink::IDBValue>>::external_objects(
    const std::unique_ptr<blink::IDBValue>& input)
{
    Vector<blink::mojom::blink::IDBExternalObjectPtr> external_objects;
    external_objects.ReserveInitialCapacity(input->BlobInfo().size() + input->FileSystemAccessTokens().size());
    for (const blink::WebBlobInfo& info : input->BlobInfo()) {
        ...
        if (info.IsFile()) {
            ...
            blob_info->file->last_modified = info.LastModified().value_or(base::Time());
        }
        ...
        external_objects.push_back(blink::mojom::blink::IDBExternalObject::NewBlobOrFile(std::move(blob_info)));
    }
    for (auto& token : input->FileSystemAccessTokens()) {
        external_objects.push_back(blink::mojom::blink::IDBExternalObject::NewFileSystemAccessToken(std::move(token)));
    }
    return external_objects;
}

// static
bool StructTraits<blink::mojom::IDBValueDataView, std::unique_ptr<blink::IDBValue>>::Read(
    blink::mojom::IDBValueDataView data, std::unique_ptr<blink::IDBValue>* out)
{
    ...
    if (value_bits.empty()) {
        *out = std::make_unique<blink::IDBValue>(std::move(value_bits), Vector<blink::WebBlobInfo>());
        return true;
    }
    ...
    for (const auto& object : external_objects) {
        ...
        case blink::mojom::blink::IDBExternalObject::Tag::kBlobOrFile: {
            ...
            if (info->file) {
                value_blob_info.emplace_back(WTF::CreateCanonicalUUIDString(), info->file->name, info->mime_type,
                    blink::NullableTimeToOptionalTime(info->file->last_modified), info->size, std::move(info->blob));
            } else {
                value_blob_info.emplace_back(WTF::CreateCanonicalUUIDString(), info->mime_type, info->size, std::move(info->blob));
            }
            break;
        }
        case blink::mojom::blink::IDBExternalObject::Tag::kFileSystemAccessToken:
            file_system_access_tokens.push_back(std::move(object->get_file_system_access_token()));
            break;
        }
    }

    *out = std::make_unique<blink::IDBValue>(std::move(value_bits), std::move(value_blob_info), std::move(file_system_access_tokens));
    return true;
}

// static
blink::mojom::blink::IDBKeyPathDataPtr StructTraits<blink::mojom::IDBKeyPathDataView, blink::IDBKeyPath>::data(const blink::IDBKeyPath& key_path)
{
    ...
    case blink::mojom::IDBKeyPathType::Array: {
        ...
        result.ReserveInitialCapacity(base::checked_cast<wtf_size_t>(array.size()));
        for (const auto& item : array)
            result.push_back(item);
        return blink::mojom::blink::IDBKeyPathData::NewStringArray(std::move(result));
    }
...
// static
bool StructTraits<blink::mojom::IDBKeyPathDataView, blink::IDBKeyPath>::Read(blink::mojom::IDBKeyPathDataView data, blink::IDBKeyPath* out)
{
...
// static
bool StructTraits<blink::mojom::IDBObjectStoreMetadataDataView, scoped_refptr<blink::IDBObjectStoreMetadata>>::Read(
    blink::mojom::IDBObjectStoreMetadataDataView data, scoped_refptr<blink::IDBObjectStoreMetadata>* out)
{
    scoped_refptr<blink::IDBObjectStoreMetadata> value = blink::IDBObjectStoreMetadata::Create();
    value->id = data.id();
    String name;
    if (!data.ReadName(&name))
        return false;
    ...
    value->indexes.ReserveCapacityForSize(base::checked_cast<wtf_size_t>(indexes.size()));
...
// static
blink::mojom::blink::IDBKeyRangePtr TypeConverter<blink::mojom::blink::IDBKeyRangePtr, const blink::IDBKeyRange*>::Convert(const blink::IDBKeyRange* input)
{
    if (!input) {
        std::unique_ptr<blink::IDBKey> lower = blink::IDBKey::CreateNone();
        std::unique_ptr<blink::IDBKey> upper = blink::IDBKey::CreateNone();
        return blink::mojom::blink::IDBKeyRange::New(std::move(lower), std::move(upper), false /* lower_open */, false /* upper_open */);
    }

    return blink::mojom::blink::IDBKeyRange::New(
        blink::IDBKey::Clone(input->Lower()), blink::IDBKey::Clone(input->Upper()), input->lowerOpen(), input->upperOpen());
}

// static
blink::mojom::blink::IDBKeyRangePtr TypeConverter<blink::mojom::blink::IDBKeyRangePtr, blink::IDBKeyRange*>::Convert(blink::IDBKeyRange* input)
{
    if (!input) {
        std::unique_ptr<blink::IDBKey> lower = blink::IDBKey::CreateNone();
        std::unique_ptr<blink::IDBKey> upper = blink::IDBKey::CreateNone();
...
// static
blink::IDBKeyRange* TypeConverter<blink::IDBKeyRange*, blink::mojom::blink::IDBKeyRangePtr>::Convert(const blink::mojom::blink::IDBKeyRangePtr& input)
{
    if (!input)
        return nullptr;

    blink::IDBKeyRange::LowerBoundType lower_type = blink::IDBKeyRange::kLowerBoundClosed;
    if (input->lower_open)
        lower_type = blink::IDBKeyRange::kLowerBoundOpen;

    blink::IDBKeyRange::UpperBoundType upper_type = blink::IDBKeyRange::kUpperBoundClosed;
    if (input->upper_open)
        upper_type = blink::IDBKeyRange::kUpperBoundOpen;

    return blink::IDBKeyRange::Create(std::move(input->lower), std::move(input->upper), lower_type, upper_type);
}
