...
TEST(IDBMojomTraitsTest, IDBKeyBinary)
{
    ...
    scoped_refptr<base::RefCountedData<Vector<char>>> input_data = base::MakeRefCounted<base::RefCountedData<Vector<char>>>(Vector<char>(test_data));
    ...
    ASSERT_TRUE(mojom::blink::IDBKey::DeserializeFromMessage(std::move(mojo_message), &output));
    scoped_refptr<base::RefCountedData<Vector<char>>> output_data = output->Binary();
...
TEST(IDBMojomTraitsTest, IDBValue)
{
    ...
    auto input = std::make_unique<IDBValue>(std::move(input_data), Vector<WebBlobInfo>());
    mojo::Message mojo_message = mojom::blink::IDBValue::SerializeAsMessage(&input);

    // Deserialize the mojo message.
    std::unique_ptr<IDBValue> output;
    ASSERT_TRUE(mojom::blink::IDBValue::DeserializeFromMessage(std::move(mojo_message), &output));
