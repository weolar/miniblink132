...
class IDBDatabaseGetAllResultSinkImpl : public mojom::blink::IDBDatabaseGetAllResultSink {
public:
    IDBDatabaseGetAllResultSinkImpl(mojo::PendingAssociatedReceiver<mojom::blink::IDBDatabaseGetAllResultSink> receiver, IDBRequestQueueItem* owner,
    ...
    void ReceiveResults(WTF::Vector<mojom::blink::IDBRecordPtr> results, bool done) override
    {
        CHECK(active_);
        CHECK_LE(results.size(), static_cast<wtf_size_t>(mojom::blink::kIDBGetAllChunkSize));

        // Increase `Vector` capacities to make room for new `results`.
        records_.primary_keys.reserve(records_.primary_keys.size() + results.size());
        ...
        for (auto& result : results) {
            ...
            if (result->return_value) {
                std::unique_ptr<IDBValue> result_value = IDBValue::ConvertReturnValue(result->return_value);
    ...
    void OnError(mojom::blink::IDBErrorPtr error) override
    {
        ...
        owner_->error_ = MakeGarbageCollected<DOMException>(static_cast<DOMExceptionCode>(error->error_code), error->error_message);
        active_ = false;
        owner_->OnResultReady();
    }
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, DOMException* error, base::OnceClosure on_result_ready)
    : request_(request)
    , error_(error)
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, int64_t value, base::OnceClosure on_result_ready)
    : request_(request)
    , on_result_ready_(std::move(on_result_ready))
    , int64_value_(value)
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, base::OnceClosure on_result_ready)
    : request_(request)
    , on_result_ready_(std::move(on_result_ready))
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, std::unique_ptr<IDBKey> key, base::OnceClosure on_result_ready)
    : request_(request)
    , key_(std::move(key))
    , on_result_ready_(std::move(on_result_ready))
    , response_type_(kKey)
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, std::unique_ptr<IDBValue> value, base::OnceClosure on_result_ready)
    : request_(request)
    , on_result_ready_(std::move(on_result_ready))
...
IDBRequestQueueItem::IDBRequestQueueItem(
    IDBRequest* request, std::unique_ptr<IDBKey> key, std::unique_ptr<IDBKey> primary_key, std::unique_ptr<IDBValue> value, base::OnceClosure on_result_ready)
    : request_(request)
    , key_(std::move(key))
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, mojo::PendingAssociatedRemote<mojom::blink::IDBCursor> pending_cursor,
    std::unique_ptr<IDBKey> key, std::unique_ptr<IDBKey> primary_key, std::unique_ptr<IDBValue> value, base::OnceClosure on_result_ready)
    : request_(request)
    , key_(std::move(key))
    , primary_key_(std::move(primary_key))
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, mojom::blink::IDBGetAllResultType get_all_result_type,
    mojo::PendingAssociatedReceiver<mojom::blink::IDBDatabaseGetAllResultSink> receiver, base::OnceClosure on_result_ready)
    : request_(request)
    , on_result_ready_(std::move(on_result_ready))
{
    DCHECK_EQ(request->queue_item_, nullptr);
    request_->queue_item_ = this;
    get_all_sink_ = std::make_unique<IDBDatabaseGetAllResultSinkImpl>(std::move(receiver), this, get_all_result_type);
}
...
bool IDBRequestQueueItem::MaybeCreateLoader()
{
    if (IDBValueUnwrapper::IsWrapped(records_.values)) {
        loader_ = MakeGarbageCollected<IDBRequestLoader>(
            std::move(records_.values), request_->GetExecutionContext(), WTF::BindOnce(&IDBRequestQueueItem::OnLoadComplete, weak_factory_.GetWeakPtr()));
        return true;
    }
    return false;
}

void IDBRequestQueueItem::OnLoadComplete(Vector<std::unique_ptr<IDBValue>>&& values, DOMException* error)
{
...
void IDBRequestQueueItem::SendResult()
{
    ...
    case kCursorKeyPrimaryKeyValue: {
        ...
        request_->SendResultCursor(std::move(pending_cursor_), std::move(key_), std::move(primary_key_), std::move(records_.values.front()));
        break;
    }
    case kError: {
        DCHECK(error_);
    ...
    case kKeyPrimaryKeyValue: {
        CHECK_EQ(records_.values.size(), 1U);
        request_->SendResultAdvanceCursor(std::move(key_), std::move(primary_key_), std::move(records_.values.front()));
        break;
    }
    case kKey: {
        if (key_ && key_->IsValid()) {
            ...
            request_->SendResult(MakeGarbageCollected<IDBAny>(IDBAny::kUndefinedType));
        }
    ...
    case kKeyArray: {
        ...
        std::unique_ptr<IDBKey> key_array = IDBKey::CreateArray(std::move(records_.primary_keys));
    ...
    case kRecordArray: {
        ...
        CHECK(records_.index_keys.empty() || records_.index_keys.size() == records_.primary_keys.size());
    ...
    case kValueArray: {
        ...
        request_->SendResult(MakeGarbageCollected<IDBAny>(std::move(records_.values)));
        break;
    }
    case kVoid: {
        request_->SendResult(MakeGarbageCollected<IDBAny>(IDBAny::kUndefinedType));
