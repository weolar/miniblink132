...
IDBRecord::IDBRecord(std::unique_ptr<IDBKey> primary_key, std::unique_ptr<IDBValue> value, std::unique_ptr<IDBKey> index_key)
...
ScriptValue IDBRecord::primaryKey(ScriptState* script_state)
{
    ...
    return ScriptValue(script_state->GetIsolate(), primary_key_->ToV8(script_state));
}

ScriptValue IDBRecord::value(ScriptState* script_state)
{
    value_dirty_ = false;
    return ScriptValue(script_state->GetIsolate(), DeserializeIDBValue(script_state, value_.get()));
}
