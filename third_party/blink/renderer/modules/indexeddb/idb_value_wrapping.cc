...
bool ShouldTransmitCompressed(size_t uncompressed_length, size_t compressed_length)
{
...
IDBValueWrapper::IDBValueWrapper(v8::Isolate* isolate, v8::Local<v8::Value> value, SerializedScriptValue::SerializeOptions::WasmSerializationPolicy wasm_policy,
    ExceptionState& exception_state)
{
    ...
    serialized_value_ = SerializedScriptValue::Serialize(isolate, value, options, exception_state);
...
{
#if DCHECK_IS_ON()
    DCHECK(!had_exception_) << __func__ << " called on wrapper with serialization exception";
    DCHECK(!done_cloning_) << __func__ << " called after DoneCloning()";
#endif // DCHECK_IS_ON()

    *clone = DeserializeScriptValue(script_state, serialized_value_.get(), &blob_info_);
}
...
void IDBValueWrapper::MaybeCompress()
{
    ...
    wire_data_buffer_.resize(kHeaderSize + static_cast<wtf_size_t>(snappy::MaxCompressedLength(wire_data_size)));
    ...
    snappy::RawCompress(
        reinterpret_cast<const char*>(wire_data_.data()), wire_data_size, reinterpret_cast<char*>(wire_data_buffer_.data() + kHeaderSize), &compressed_length);
    if (ShouldTransmitCompressed(wire_data_size, compressed_length)) {
        // Truncate the excess space that was previously allocated.
        wire_data_buffer_.resize(kHeaderSize + static_cast<wtf_size_t>(compressed_length));
    } else {
...
{
    const unsigned wrapping_threshold = wrapping_threshold_override_.value_or(mojom::blink::kIDBWrapThreshold);
    ...
    const size_t wire_data_size = wire_data_.size();
    blob_info_.emplace_back(BlobDataHandle::Create(std::move(wrapper_blob_data), wire_data_size));
    ...
    IDBValueWrapper::WriteVarInt(base::checked_cast<unsigned>(wire_data_size), wire_data_buffer_);
...
Vector<char> IDBValueWrapper::TakeWireBytes()
{
    ...
    DCHECK_EQ(wire_data_buffer_.data(), reinterpret_cast<const char*>(wire_data_.data()));
...
bool IDBValueUnwrapper::IsWrapped(IDBValue* value)
{
    ...
    return data_span[0] == kVersionTag && data_span[1] == kRequiresProcessingSSVPseudoVersion && data_span[2] == kReplaceWithBlob;
}

// static
bool IDBValueUnwrapper::IsWrapped(const Vector<std::unique_ptr<IDBValue>>& values)
{
...
// static
void IDBValueUnwrapper::Unwrap(Vector<char>&& wrapper_blob_content, IDBValue& wrapped_value)
{
...
// static
bool IDBValueUnwrapper::Decompress(const Vector<char>& buffer, Vector<char>* out_buffer)
{
    ...
    if (data_span[0] != kVersionTag || data_span[1] != kRequiresProcessingSSVPseudoVersion || data_span[2] != kCompressedWithSnappy) {
        return false;
    }
    ...
    if (!snappy::GetUncompressedLength(compressed.data(), compressed.size(), &decompressed_length)) {
        return false;
    }
