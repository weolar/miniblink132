...
IDBRequestLoader::IDBRequestLoader(
    Vector<std::unique_ptr<IDBValue>>&& values, ExecutionContext* execution_context, LoadCompleteCallback&& load_complete_callback)
...
#endif // DCHECK_IS_ON()
    loader_ = MakeGarbageCollected<FileReaderLoader>(this, execution_context_->GetTaskRunner(TaskType::kDatabaseAccess));
...
{
    DCHECK_LE(wrapped_data_.size() + data.size(), wrapped_data_.capacity()) << "The reader returned more data than we were prepared for";
...
{
#if DCHECK_IS_ON()
    DCHECK(started_) << "FileReaderLoader called DidFinishLoading() before it was Start()ed";
    DCHECK(!canceled_) << "FileReaderLoader called DidFinishLoading() after it was Cancel()ed";

    DCHECK(file_reader_loading_);
    file_reader_loading_ = false;
#endif // DCHECK_IS_ON()

    base::UmaHistogramTimes("IndexedDB.WrappedBlobLoadTime", base::TimeTicks::Now() - start_loading_time_);
...
{
#if DCHECK_IS_ON()
    DCHECK(started_) << "FileReaderLoader called DidFail() before it was Start()ed";
    DCHECK(!canceled_) << "FileReaderLoader called DidFail() after it was Cancel()ed";
...
void IDBRequestLoader::OnLoadComplete(FileErrorCode error_code)
{
    ...
    case FileErrorCode::kNotFoundErr:
        ...
        exception = MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotReadableError,
    ...
    default:
        ...
        exception = MakeGarbageCollected<DOMException>(DOMExceptionCode::kUnknownError, "Failed to read large IndexedDB value");
        break;
    }
