...
CanvasRenderingContext* GPUCanvasContext::Factory::Create(CanvasRenderingContextHost* host, const CanvasContextCreationAttributesCore& attrs)
{
    CanvasRenderingContext* rendering_context = MakeGarbageCollected<GPUCanvasContext>(host, attrs);
    DCHECK(host);
    return rendering_context;
}

CanvasRenderingContext::CanvasRenderingAPI GPUCanvasContext::Factory::GetRenderingAPI() const
{
    return CanvasRenderingContext::CanvasRenderingAPI::kWebgpu;
}

GPUCanvasContext::GPUCanvasContext(CanvasRenderingContextHost* host, const CanvasContextCreationAttributesCore& attrs)
...
SkColorInfo GPUCanvasContext::CanvasRenderingContextSkColorInfo() const
{
    ...
    return SkColorInfo(viz::ToClosestSkColorType(
        ...
        alpha_mode_ == V8GPUCanvasAlphaMode::Enum::kOpaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType, PredefinedColorSpaceToSkColorSpace(color_space_));
}
...
void GPUCanvasContext::Reshape(int width, int height)
{
    ...
    texture_descriptor_.size = { static_cast<uint32_t>(width), static_cast<uint32_t>(height), 1 };
...
scoped_refptr<StaticBitmapImage> GPUCanvasContext::GetImage(FlushReason)
{
    ...
    return SnapshotInternal(mailbox_texture->GetTexture(), gfx::Size(mailbox_texture->GetTexture().GetWidth(), mailbox_texture->GetTexture().GetHeight()));
}

bool GPUCanvasContext::PaintRenderingResultsToCanvas(SourceDrawingBuffer source_buffer)
{
    if (!swap_buffers_) {
        return false;
    }

    if (Host()->ResourceProvider() && Host()->ResourceProvider()->Size() != swap_buffers_->Size()) {
    ...
    return CopyTextureToResourceProvider(texture_->GetHandle(), swap_buffers_->Size(), resource_provider);
}

bool GPUCanvasContext::CopyRenderingResultsToVideoFrame(WebGraphicsContext3DVideoFramePool* frame_pool, SourceDrawingBuffer src_buffer,
    const gfx::ColorSpace& dst_color_space, VideoFrameCopyCompletedCallback callback)
{
    ...
    return swap_buffers_->CopyToVideoFrame(frame_pool, src_buffer, dst_color_space, std::move(callback));
}

void GPUCanvasContext::SetFilterQuality(cc::PaintFlags::FilterQuality filter_quality)
{
...
bool GPUCanvasContext::PushFrame()
{
    ...
    if (!swap_buffers_->PrepareTransferableResource(nullptr, &transferable_resource, &release_callback)) {
        return false;
    }
    ...
    auto canvas_resource = ExternalCanvasResource::Create(std::move(client_si), transferable_resource, std::move(release_callback), GetContextProviderWeakPtr(),
    ...
    return Host()->PushFrame(std::move(canvas_resource), SkIRect::MakeWH(width, height));
}

ImageBitmap* GPUCanvasContext::TransferToImageBitmap(ScriptState* script_state, ExceptionState& exception_state)
{
    auto MakeFallbackImageBitmap = [this](V8GPUCanvasAlphaMode::Enum alpha_mode) -> ImageBitmap* {
        ...
        return MakeGarbageCollected<ImageBitmap>(UnacceleratedStaticBitmapImage::Create(SkImages::RasterFromBitmap(black_bitmap)));
    };
    ...
    const SkImageInfo sk_image_info = SkImageInfo::Make(texture_descriptor_.size.width, texture_descriptor_.size.height, sk_color_type, kPremul_SkAlphaType);

    CHECK_EQ(client_si->surface_origin(), kTopLeft_GrSurfaceOrigin);
    return MakeGarbageCollected<ImageBitmap>(AcceleratedStaticBitmapImage::CreateFromCanvasSharedImage(std::move(client_si), sk_image_sync_token,
        /* shared_image_texture_id = */ 0, sk_image_info, transferable_resource.texture_target(),
        /* is_origin_top_left = */ true, GetContextProviderWeakPtr(), base::PlatformThread::CurrentRef(), ThreadScheduler::Current()->CleanupTaskRunner(),
        std::move(release_callback),
        /*supports_display_compositing=*/true, transferable_resource.is_overlay_candidate));
}

// gpu_presentation_context.idl
V8UnionHTMLCanvasElementOrOffscreenCanvas* GPUCanvasContext::getHTMLOrOffscreenCanvas() const
{
...
void GPUCanvasContext::configure(const GPUCanvasConfiguration* descriptor, ExceptionState& exception_state)
{
    DCHECK(descriptor);
    ...
    if (stopped_ || !Host()) {
        ...
        // shutdown.
        exception_state.ThrowDOMException(DOMExceptionCode::kUnknownError, "canvas has been destroyed");
        return;
    }

    if (!descriptor->device()->ValidateTextureFormatUsage(descriptor->format(), exception_state)) {
        return;
    }

    for (auto view_format : descriptor->viewFormats()) {
        if (!descriptor->device()->ValidateTextureFormatUsage(view_format, exception_state)) {
            return;
        }
    }

    if (!IsContextFormatSupported(descriptor->format().AsEnum())) {
        exception_state.ThrowTypeError(String::Format("Unsupported canvas context format '%s'.", V8GPUTextureFormat(descriptor->format()).AsCStr()));
        return;
    }
    ...
    texture_descriptor_ = {
        ...
        .size = { static_cast<uint32_t>(host_size.width()), static_cast<uint32_t>(host_size.height()) },
    ...
    if (copy_to_swap_texture_required_ && GPU::preferred_canvas_format() == wgpu::TextureFormat::BGRA8Unorm
        && texture_descriptor_.usage & wgpu::TextureUsage::StorageBinding && !device_->adapter()->features()->has(V8GPUFeatureName::Enum::kBgra8UnormStorage)) {
    ...
    if (!copy_to_swap_texture_required_) {
        ...
        if (alpha_mode_ == V8GPUCanvasAlphaMode::Enum::kOpaque) {
            ...
            texture_internal_usage_.internalUsage |= wgpu::TextureUsage::TextureBinding | wgpu::TextureUsage::RenderAttachment;
        }
    ...
    if (copy_to_swap_texture_required_) {
        ...
        texture_internal_usage_ = { {
            .internalUsage = wgpu::TextureUsage::CopySrc | wgpu::TextureUsage::TextureBinding,
        } };
        ...
        swap_texture_descriptor_.usage = wgpu::TextureUsage::CopyDst | wgpu::TextureUsage::RenderAttachment;
    ...
    if (!ValidateAndConvertColorSpace(descriptor->colorSpace(), color_space_, exception_state)) {
        return;
    }
    ...
    if (descriptor->hasToneMapping() && descriptor->toneMapping()->hasMode()) {
        ...
        case V8GPUCanvasToneMappingMode::Enum::kExtended:
            hdr_metadata.extended_range.emplace(
                /*current_headroom=*/gfx::HdrMetadataExtendedRange::kDefaultHdrHeadroom,
                /*desired_headroom=*/gfx::HdrMetadataExtendedRange::kDefaultHdrHeadroom);
            break;
        }
    }

    const wgpu::DawnTextureInternalUsageDescriptor* internal_usage_desc = nullptr;
    if (const wgpu::ChainedStruct* next_in_chain = swap_texture_descriptor_.nextInChain) {
        // The internal usage descriptor is the only valid struct to chain.
        CHECK_EQ(next_in_chain->sType, wgpu::SType::DawnTextureInternalUsageDescriptor);
        internal_usage_desc = static_cast<const wgpu::DawnTextureInternalUsageDescriptor*>(next_in_chain);
    }
    auto internal_usage = internal_usage_desc ? internal_usage_desc->internalUsage : wgpu::TextureUsage::None;
    swap_buffers_ = base::AdoptRef(new WebGPUSwapBufferProvider(this, device_->GetDawnControlClient(), device_->GetHandle(), swap_texture_descriptor_.usage,
        internal_usage, swap_texture_descriptor_.format, color_space_, hdr_metadata));
    ...
    case V8GPUCanvasAlphaMode::Enum::kOpaque: {
        if (!alpha_clearer_ || !alpha_clearer_->IsCompatible(device_->GetHandle(), swap_texture_descriptor_.format)) {
            alpha_clearer_
                = base::MakeRefCounted<WebGPUTextureAlphaClearer>(device_->GetDawnControlClient(), device_->GetHandle(), swap_texture_descriptor_.format);
        }
        break;
    }
...
GPUTexture* GPUCanvasContext::getCurrentTexture(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!configured_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "context is not configured");
        return nullptr;
    }
    DCHECK(device_);
    ...
    if (!swap_buffers_) {
        device_->InjectError(wgpu::ErrorType::Validation, "context configuration is invalid.");
    ...
    SkAlphaType alpha_type = alpha_mode_ == V8GPUCanvasAlphaMode::Enum::kOpaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType;
    scoped_refptr<WebGPUMailboxTexture> mailbox_texture = swap_buffers_->GetNewTexture(swap_texture_descriptor_, alpha_type);
    if (!mailbox_texture) {
        // Try to give a helpful message for the most common cause for mailbox
        // texture creation failure.
        if (texture_descriptor_.size.width == 0 || texture_descriptor_.size.height == 0) {
            device_->InjectError(wgpu::ErrorType::Validation, "Could not create a swapchain texture of size 0.");
        } else {
            device_->InjectError(wgpu::ErrorType::Validation, "Could not create the swapchain texture.");
        }
        texture_ = swap_texture_ = GPUTexture::CreateError(device_, &texture_descriptor_);
        return texture_.Get();
    }
    ...
    swap_texture_ = MakeGarbageCollected<GPUTexture>(device_, swap_texture_descriptor_.format, static_cast<wgpu::TextureUsage>(swap_texture_descriptor_.usage),
        std::move(mailbox_texture), String::FromUTF8(swap_texture_descriptor_.label));

    if (copy_to_swap_texture_required_) {
        texture_
            = MakeGarbageCollected<GPUTexture>(device_, device_->GetHandle().CreateTexture(&texture_descriptor_), String::FromUTF8(texture_descriptor_.label));
    ...
    UseCounter::Count(execution_context, WebFeature::kWebGPUCanvasContextGetCurrentTexture);
...
void GPUCanvasContext::CopyToSwapTexture()
{
    ...
    wgpu::AlphaMode copy_alpha_mode = alpha_mode_ == V8GPUCanvasAlphaMode::Enum::kOpaque ? wgpu::AlphaMode::Opaque : wgpu::AlphaMode::Premultiplied;
    ...
    device_->queue()->GetHandle().CopyTextureForBrowser(&source, &destination, &copy_size, &options);
}

bool GPUCanvasContext::CopyTextureToResourceProvider(const wgpu::Texture& texture, const gfx::Size& size, CanvasResourceProvider* resource_provider) const
{
    ...
    DCHECK(resource_provider->GetSharedImageUsageFlags().Has(gpu::SHARED_IMAGE_USAGE_WEBGPU_WRITE));
    ...
    auto dst_client_si = resource_provider->GetBackingClientSharedImageForOverwrite();
    if (!dst_client_si) {
        return false;
    }
    ...
    // manually.
    gpu::webgpu::WebGPUInterface* webgpu = GetContextProviderWeakPtr()->ContextProvider()->WebGPUInterface();
    gpu::webgpu::ReservedTexture reservation = webgpu->ReserveTexture(device_->GetHandle().Get());
    ...
    wgpu::TextureUsage usage = wgpu::TextureUsage::CopyDst | wgpu::TextureUsage::RenderAttachment;
    webgpu->AssociateMailbox(
        reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation, static_cast<uint64_t>(usage), dst_client_si->mailbox());
    wgpu::ImageCopyTexture source = {
        .texture = texture,
        .aspect = wgpu::TextureAspect::All,
    };
    ...
    if (isOpaque || copy_to_swap_texture_required_) {
        wgpu::AlphaMode srcAlphaMode = isOpaque ? wgpu::AlphaMode::Opaque : wgpu::AlphaMode::Premultiplied;
        ...
        wgpu::CommandEncoder command_encoder = device_->GetHandle().CreateCommandEncoder(&command_encoder_desc);
...
scoped_refptr<StaticBitmapImage> GPUCanvasContext::SnapshotInternal(const wgpu::Texture& texture, const gfx::Size& size) const
{
    const auto canvas_context_color = CanvasRenderingContextSkColorInfo();
    const auto info = SkImageInfo::Make(gfx::SizeToSkISize(size), canvas_context_color);
    ...
    auto resource_provider = CanvasResourceProvider::CreateWebGPUImageProvider(info, swap_buffers_->GetSharedImageUsagesForDisplay());
    if (!resource_provider)
        return nullptr;
...
base::WeakPtr<WebGraphicsContext3DProviderWrapper> GPUCanvasContext::GetContextProviderWeakPtr() const
{
