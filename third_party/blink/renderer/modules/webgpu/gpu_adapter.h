...
public:
    GPUAdapter(GPU* gpu, wgpu::Adapter handle, scoped_refptr<DawnControlClientHolder> dawn_control_client, const GPURequestAdapterOptions* options);
    ...
    ScriptPromise<GPUDevice> requestDevice(ScriptState* script_state, GPUDeviceDescriptor* descriptor);
    ...
    void AddConsoleWarning(ExecutionContext* execution_context, const char* message);

    GPUAdapterInfo* CreateAdapterInfoForAdapter();

    bool isXRCompatible() const
    {
        return is_xr_compatible_;
    }

private:
    void OnRequestDeviceCallback(GPUDevice* device, const GPUDeviceDescriptor* descriptor, ScriptPromiseResolver<GPUDevice>* resolver,
