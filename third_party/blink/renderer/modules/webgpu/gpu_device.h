...
class GPUDevice final : public EventTarget, public ExecutionContextClient, public DawnObject<wgpu::Device> {
...
public:
    ...
    // happens.
    GPUDevice(ExecutionContext* execution_context, scoped_refptr<DawnControlClientHolder> dawn_control_client, GPUAdapter* adapter, const String& label);
    ...
    void Initialize(wgpu::Device handle, const GPUDeviceDescriptor* descriptor, GPUDeviceLostInfo* lost_info);
    ...
    GPUBuffer* createBuffer(const GPUBufferDescriptor* descriptor, ExceptionState& exception_state);
    GPUTexture* createTexture(const GPUTextureDescriptor* descriptor, ExceptionState& exception_state);
    GPUSampler* createSampler(const GPUSamplerDescriptor* descriptor);

    GPUExternalTexture* importExternalTexture(const GPUExternalTextureDescriptor* descriptor, ExceptionState& exception_state);

    GPUBindGroup* createBindGroup(const GPUBindGroupDescriptor* descriptor, ExceptionState& exception_state);
    GPUBindGroupLayout* createBindGroupLayout(const GPUBindGroupLayoutDescriptor* descriptor, ExceptionState& exception_state);
    ...
    GPURenderPipeline* createRenderPipeline(ScriptState* script_state, const GPURenderPipelineDescriptor* descriptor);
    GPUComputePipeline* createComputePipeline(const GPUComputePipelineDescriptor* descriptor, ExceptionState& exception_state);
    ScriptPromise<GPURenderPipeline> createRenderPipelineAsync(ScriptState* script_state, const GPURenderPipelineDescriptor* descriptor, ExceptionState&);
    ScriptPromise<GPUComputePipeline> createComputePipelineAsync(ScriptState* script_state, const GPUComputePipelineDescriptor* descriptor);

    GPUCommandEncoder* createCommandEncoder(const GPUCommandEncoderDescriptor* descriptor);
    GPURenderBundleEncoder* createRenderBundleEncoder(const GPURenderBundleEncoderDescriptor* descriptor, ExceptionState& exception_state);

    GPUQuerySet* createQuerySet(const GPUQuerySetDescriptor* descriptor, ExceptionState& exception_state);
    ...
    bool ValidateTextureFormatUsage(V8GPUTextureFormat format, ExceptionState& exception_state);
    bool ValidateBlendFactor(V8GPUBlendFactor blend_factor, ExceptionState& exception_state);
    ...
    WGPURepeatingCallback<void(const wgpu::Device&, wgpu::ErrorType, wgpu::StringView)>* error_callback();
    WGPURepeatingCallback<void(const wgpu::Device&, wgpu::DeviceLostReason, wgpu::StringView)>* lost_callback();
...
private:
    ...
    void OnUncapturedError(const wgpu::Device& device, wgpu::ErrorType errorType, wgpu::StringView message);
    void OnLogging(WGPULoggingType loggingType, WGPUStringView message);

    void OnDeviceLostError(const wgpu::Device& device, wgpu::DeviceLostReason reason, wgpu::StringView message);
    ...
    void OnCreateRenderPipelineAsyncCallback(const String& label, ScriptPromiseResolver<GPURenderPipeline>* resolver, wgpu::CreatePipelineAsyncStatus status,
        wgpu::RenderPipeline render_pipeline, wgpu::StringView message);
    void OnCreateComputePipelineAsyncCallback(const String& label, ScriptPromiseResolver<GPUComputePipeline>* resolver, wgpu::CreatePipelineAsyncStatus status,
        wgpu::ComputePipeline compute_pipeline, wgpu::StringView message);
    ...
    std::unique_ptr<WGPURepeatingCallback<void(const wgpu::Device&, wgpu::ErrorType, wgpu::StringView)>> error_callback_;
    std::unique_ptr<WGPURepeatingCallback<void(WGPULoggingType, WGPUStringView)>> logging_callback_;
    ...
    // free.
    std::unique_ptr<WGPURepeatingCallback<void(const wgpu::Device&, wgpu::DeviceLostReason, wgpu::StringView)>> lost_callback_;
    ...
    std::bitset<static_cast<size_t>(GPUSingletonWarning::kCount)> singleton_warning_fired_;
