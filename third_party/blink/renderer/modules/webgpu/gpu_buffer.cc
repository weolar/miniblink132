...
wgpu::BufferDescriptor AsDawnType(const GPUBufferDescriptor* webgpu_desc, std::string* label)
{
    DCHECK(webgpu_desc);
    DCHECK(label);
...
public:
    static GPUMappedDOMArrayBuffer* Create(v8::Isolate* isolate, GPUBuffer* owner, ArrayBufferContents contents)
    {
        auto* mapped_array_buffer = MakeGarbageCollected<GPUMappedDOMArrayBuffer>(owner, std::move(contents));
    ...
    GPUMappedDOMArrayBuffer(GPUBuffer* owner, ArrayBufferContents contents)
        : DOMArrayBuffer(std::move(contents))
        , owner_(owner)
    {
    }
    ...
    void DetachContents(v8::Isolate* isolate)
    {
        ...
        bool result = DOMArrayBuffer::Transfer(isolate, V8AtomicString(isolate, kWebGPUBufferMappingDetachKey), contents, exception_state);
...
// static
GPUBuffer* GPUBuffer::Create(GPUDevice* device, const GPUBufferDescriptor* webgpu_desc, ExceptionState& exception_state)
{
    DCHECK(device);
    ...
    bool is_mappable = dawn_desc.usage & (wgpu::BufferUsage::MapRead | wgpu::BufferUsage::MapWrite) || dawn_desc.mappedAtCreation;
    if (is_mappable) {
    ...
    if (wgpuBuffer == nullptr) {
        ...
        exception_state.ThrowRangeError(WTF::String::Format("createBuffer failed, size (%" PRIu64 ") is too large for "
    ...
    GPUBuffer* buffer = MakeGarbageCollected<GPUBuffer>(device, buffer_size, std::move(wgpuBuffer), webgpu_desc->label());
...
GPUBuffer::GPUBuffer(GPUDevice* device, uint64_t size, wgpu::Buffer buffer, const String& label)
    : DawnObject<wgpu::Buffer>(device, std::move(buffer), label)
    , size_(size)
{
}
...
ScriptPromise<IDLUndefined> GPUBuffer::mapAsync(ScriptState* script_state, uint32_t mode, uint64_t offset, ExceptionState& exception_state)
{
    return MapAsyncImpl(script_state, mode, offset, std::nullopt, exception_state);
}

ScriptPromise<IDLUndefined> GPUBuffer::mapAsync(ScriptState* script_state, uint32_t mode, uint64_t offset, uint64_t size, ExceptionState& exception_state)
{
    return MapAsyncImpl(script_state, mode, offset, size, exception_state);
}

DOMArrayBuffer* GPUBuffer::getMappedRange(ScriptState* script_state, uint64_t offset, ExceptionState& exception_state)
{
    return GetMappedRangeImpl(script_state, offset, std::nullopt, exception_state);
}

DOMArrayBuffer* GPUBuffer::getMappedRange(ScriptState* script_state, uint64_t offset, uint64_t size, ExceptionState& exception_state)
{
...
ScriptPromise<IDLUndefined> GPUBuffer::MapAsyncImpl(
    ScriptState* script_state, uint32_t mode, uint64_t offset, std::optional<uint64_t> size, ExceptionState& exception_state)
{
    ...
    // with it.
    size_t map_offset = static_cast<size_t>(std::min(offset, kGuaranteedBufferOOMSize));
    size_t map_size = static_cast<size_t>(std::min(size_defaulted, kGuaranteedBufferOOMSize));
    ...
    auto* callback = MakeWGPUOnceCallback(resolver->WrapCallbackInScriptScope(WTF::BindOnce(&GPUBuffer::OnMapAsyncCallback, WrapPersistent(this))));

    GetHandle().MapAsync(
        static_cast<wgpu::MapMode>(mode), map_offset, map_size, wgpu::CallbackMode::AllowSpontaneous, callback->UnboundCallback(), callback->AsUserdata());
...
DOMArrayBuffer* GPUBuffer::GetMappedRangeImpl(ScriptState* script_state, uint64_t offset, std::optional<uint64_t> size, ExceptionState& exception_state)
{
    // Compute the defaulted size which is "until the end of the buffer" or 0 if
    // offset is past the end of the buffer.
    uint64_t size_defaulted = 0;
    ...
    // offset with it.
    size_t range_offset = static_cast<size_t>(std::min(offset, kGuaranteedBufferOOMSize));
    size_t range_size = static_cast<size_t>(std::min(size_defaulted, kGuaranteedBufferOOMSize));

    if (range_size > std::numeric_limits<size_t>::max() - range_offset) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError,
            WTF::String::Format("getMappedRange failed, offset(%zu) + size(%zu) overflows size_t", range_offset, range_size));
        return nullptr;
    }
    ...
    // And send the command, leaving remaining validation to Dawn.
    const void* map_data_const = GetHandle().GetConstMappedRange(range_offset, range_size);

    if (!map_data_const) {
        // Ensure that GPU process error messages are bubbled back to the renderer process.
        EnsureFlush(ToEventLoop(script_state));
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "getMappedRange failed");
        return nullptr;
    }
    ...
    if (range_size > v8::TypedArray::kMaxByteLength) {
        exception_state.ThrowRangeError(WTF::String::Format("getMappedRange failed, size (%zu) is too large "
    ...
    return CreateArrayBufferForMappedData(script_state->GetIsolate(), map_data, range_size);
}

void GPUBuffer::OnMapAsyncCallback(ScriptPromiseResolver<IDLUndefined>* resolver, wgpu::MapAsyncStatus status, wgpu::StringView message)
{
    switch (status) {
    ...
    case wgpu::MapAsyncStatus::InstanceDropped:
        resolver->RejectWithDOMException(DOMExceptionCode::kAbortError, String::FromUTF8(message));
        break;
    case wgpu::MapAsyncStatus::Error:
        resolver->RejectWithDOMException(DOMExceptionCode::kOperationError, String::FromUTF8(message));
        break;
...
DOMArrayBuffer* GPUBuffer::CreateArrayBufferForMappedData(v8::Isolate* isolate, void* data, size_t data_length)
{
    DCHECK(data);
    DCHECK_LE(static_cast<uint64_t>(data_length), v8::TypedArray::kMaxByteLength);

    ArrayBufferContents contents(v8::ArrayBuffer::NewBackingStore(data, data_length, v8::BackingStore::EmptyDeleter, nullptr));
    GPUMappedDOMArrayBuffer* array_buffer = GPUMappedDOMArrayBuffer::Create(isolate, this, contents);
...
{
    for (Member<GPUMappedDOMArrayBuffer>& mapped_array_buffer : mapped_array_buffers_) {
