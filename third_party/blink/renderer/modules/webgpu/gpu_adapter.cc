...
std::optional<V8GPUFeatureName::Enum> ToV8FeatureNameEnum(wgpu::FeatureName f)
{
    ...
    case wgpu::FeatureName::ChromiumExperimentalTimestampQueryInsidePasses:
        return V8GPUFeatureName::Enum::kChromiumExperimentalTimestampQueryInsidePasses;
    ...
    case wgpu::FeatureName::ChromiumExperimentalSubgroupUniformControlFlow:
        return V8GPUFeatureName::Enum::kChromiumExperimentalSubgroupUniformControlFlow;
...
GPUSupportedFeatures* MakeFeatureNameSet(wgpu::Adapter adapter, ExecutionContext* execution_context)
{
    ...
    Vector<wgpu::FeatureName> feature_names(static_cast<wtf_size_t>(feature_count));
    ...
    for (wgpu::FeatureName f : feature_names) {
        ...
        if (feature_name_enum_optional) {
            V8GPUFeatureName::Enum feature_name_enum = feature_name_enum_optional.value();
            // Subgroups features are under OT.
            // TODO(crbug.com/349125474): remove this check after subgroups features
            // OT finished.
            if ((feature_name_enum_optional == V8GPUFeatureName::Enum::kSubgroups) || (feature_name_enum_optional == V8GPUFeatureName::Enum::kSubgroupsF16)) {
                if (!RuntimeEnabledFeatures::WebGPUSubgroupsFeaturesEnabled(execution_context)) {
                    continue;
                }
            }
...
GPUAdapter::GPUAdapter(GPU* gpu, wgpu::Adapter handle, scoped_refptr<DawnControlClientHolder> dawn_control_client, const GPURequestAdapterOptions* options)
    : DawnObject(dawn_control_client, std::move(handle), String())
    , gpu_(gpu)
{
    ...
    bool supportsPropertiesD3D = GetHandle().HasFeature(wgpu::FeatureName::AdapterPropertiesD3D);
    if (supportsPropertiesD3D) {
        *propertiesChain = &d3dProperties;
        propertiesChain = &(*propertiesChain)->nextInChain;
    }
    wgpu::AdapterPropertiesVk vkProperties = {};
    bool supportsPropertiesVk = GetHandle().HasFeature(wgpu::FeatureName::AdapterPropertiesVk);
    ...
    for (size_t i = 0; i < memoryHeapProperties.heapCount; ++i) {
        memory_heaps_.push_back(MakeGarbageCollected<GPUMemoryHeapInfo>(memoryHeapProperties.heapInfo[i]));
    }
    if (supportsPropertiesD3D) {
...
GPUAdapterInfo* GPUAdapter::CreateAdapterInfoForAdapter()
{
    ...
    if (RuntimeEnabledFeatures::WebGPUDeveloperFeaturesEnabled()) {
        ...
        // adapter info values.
        info = MakeGarbageCollected<GPUAdapterInfo>(vendor_, architecture_, device_, description_, driver_, FromDawnEnum(backend_type_),
...
void GPUAdapter::AddConsoleWarning(ExecutionContext* execution_context, const char* message)
{
...
void GPUAdapter::OnRequestDeviceCallback(GPUDevice* device, const GPUDeviceDescriptor* descriptor, ScriptPromiseResolver<GPUDevice>* resolver,
    wgpu::RequestDeviceStatus status, wgpu::Device dawn_device, wgpu::StringView error_message)
{
    switch (status) {
    case wgpu::RequestDeviceStatus::Success: {
        ...
        if (is_consumed_) {
            ...
            // error device.
            device_lost_info = MakeGarbageCollected<GPUDeviceLostInfo>(wgpu::DeviceLostReason::Unknown,
                StringFromASCIIAndUTF8("The adapter is invalid because it has already been used to "
        ...
        if (device_lost_info) {
            ...
            device->InjectError(wgpu::ErrorType::DeviceLost, "Device was marked as lost due to a stale adapter.");
        }

        resolver->Resolve(device);

        ukm::builders::ClientRenderingAPI(device->GetExecutionContext()->UkmSourceID())
    ...
    case wgpu::RequestDeviceStatus::InstanceDropped:
        if (dawn_device) {
            ...
            device->Initialize(
                dawn_device, descriptor, MakeGarbageCollected<GPUDeviceLostInfo>(wgpu::DeviceLostReason::Unknown, StringFromASCIIAndUTF8(error_message)));
            ...
            resolver->RejectWithDOMException(DOMExceptionCode::kOperationError, StringFromASCIIAndUTF8(error_message));
        }
        break;
    }
}

ScriptPromise<GPUDevice> GPUAdapter::requestDevice(ScriptState* script_state, GPUDeviceDescriptor* descriptor)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<GPUDevice>>(
        script_state, ExceptionContext(v8::ExceptionContext::kOperation, "GPUAdapter", "requestDevice"));
    ...
    if (descriptor->hasRequiredLimits()) {
        ...
        if (!GPUSupportedLimits::Populate(&required_limits, descriptor->requiredLimits(), resolver)) {
            return promise;
        }
    }
    ...
    if (RuntimeEnabledFeatures::WebGPUDeveloperFeaturesEnabled() && GetHandle().HasFeature(wgpu::FeatureName::ShaderModuleCompilationOptions)) {
        required_features.push_back(wgpu::FeatureName::ShaderModuleCompilationOptions);
    }
    if (descriptor->hasRequiredFeatures()) {
        ...
        for (const V8GPUFeatureName& f : descriptor->requiredFeatures()) {
            ...
            if (!features_->has(f.AsEnum())) {
                resolver->RejectWithTypeError(String::Format("Unsupported feature: %s", f.AsCStr()));
                return promise;
            }
        ...
        required_features.AppendRange(required_features_set.begin(), required_features_set.end());
    ...
    auto* device = MakeGarbageCollected<GPUDevice>(execution_context, GetDawnControlClient(), this, descriptor->label());
    dawn_desc.SetUncapturedErrorCallback(device->error_callback()->UnboundCallback(), device->error_callback()->AsUserdata());
    dawn_desc.SetDeviceLostCallback(wgpu::CallbackMode::AllowSpontaneous, device->lost_callback()->UnboundCallback(), device->lost_callback()->AsUserdata());

    auto* callback = MakeWGPUOnceCallback(resolver->WrapCallbackInScriptScope(
        WTF::BindOnce(&GPUAdapter::OnRequestDeviceCallback, WrapPersistent(this), WrapPersistent(device), WrapPersistent(descriptor))));

    GetHandle().RequestDevice(&dawn_desc, wgpu::CallbackMode::AllowSpontaneous, callback->UnboundCallback(), callback->AsUserdata());
    EnsureFlush(ToEventLoop(script_state));

    return promise;
}

ScriptPromise<GPUAdapterInfo> GPUAdapter::requestAdapterInfo(ScriptState* script_state)
{
