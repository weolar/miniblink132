...
public:
    explicit DawnObjectBase(scoped_refptr<DawnControlClientHolder> dawn_control_client, const String& label);
...
template <typename Obj> class DawnObject : public DawnObjectImpl {
public:
...
template <> class DawnObject<wgpu::Device> : public DawnObjectBase {
public:
    const wgpu::Device& GetHandle() const
    {
        return handle_;
    }
...
protected:
    ...
    void SetHandle(wgpu::Device handle)
    {
        handle_ = std::move(handle);
    }
...
template <> class DawnObject<wgpu::Adapter> : public DawnObjectBase {
public:
    DawnObject(scoped_refptr<DawnControlClientHolder> dawn_control_client, wgpu::Adapter handle, const String& label)
        : DawnObjectBase(dawn_control_client, label)
        , handle_(std::move(handle))
    {
    }

    const wgpu::Adapter& GetHandle() const
    {
        return handle_;
    }
