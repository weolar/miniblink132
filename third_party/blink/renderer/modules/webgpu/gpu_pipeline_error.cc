...
// static
GPUPipelineError* GPUPipelineError::Create(String message, const GPUPipelineErrorInit* options)
{
    // For consistency with `new DOMException()`, we don't AttachStackProperty.
    return MakeGarbageCollected<GPUPipelineError>(std::move(message), options->reason().AsEnum());
}

// static
v8::Local<v8::Value> GPUPipelineError::Create(v8::Isolate* isolate, const String& message, V8GPUPipelineErrorReason::Enum reason)
{
...
GPUPipelineError::GPUPipelineError(const String& message, V8GPUPipelineErrorReason::Enum reason)
