...
// static
GPUQuerySet* GPUQuerySet::Create(GPUDevice* device, const GPUQuerySetDescriptor* webgpu_desc)
{
    DCHECK(device);
    DCHECK(webgpu_desc);
    ...
    GPUQuerySet* query_set = MakeGarbageCollected<GPUQuerySet>(device, device->GetHandle().CreateQuerySet(&dawn_desc), webgpu_desc->label());
    return query_set;
}

GPUQuerySet::GPUQuerySet(GPUDevice* device, wgpu::QuerySet querySet, const String& label)
