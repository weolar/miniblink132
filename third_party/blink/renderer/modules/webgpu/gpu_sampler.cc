...
wgpu::SamplerDescriptor AsDawnType(const GPUSamplerDescriptor* webgpu_desc, std::string* label)
{
    DCHECK(webgpu_desc);
    DCHECK(label);
...
// static
GPUSampler* GPUSampler::Create(GPUDevice* device, const GPUSamplerDescriptor* webgpu_desc)
{
    DCHECK(device);
    DCHECK(webgpu_desc);
    std::string label;
    wgpu::SamplerDescriptor dawn_desc = AsDawnType(webgpu_desc, &label);
    GPUSampler* sampler = MakeGarbageCollected<GPUSampler>(device, device->GetHandle().CreateSampler(&dawn_desc), webgpu_desc->label());
    return sampler;
}

GPUSampler::GPUSampler(GPUDevice* device, wgpu::Sampler sampler, const String& label)
