...
class WebGPUContextProviderForTest : public WebGraphicsContext3DProviderForTests {
public:
    explicit WebGPUContextProviderForTest(base::MockCallback<base::OnceClosure>* destruction_callback)
        : WebGraphicsContext3DProviderForTests(std::make_unique<gpu::webgpu::WebGPUInterfaceStub>())
    ...
    static WebGPUContextProviderForTest* From(scoped_refptr<DawnControlClientHolder>& dawn_control_client)
    {
        return static_cast<WebGPUContextProviderForTest*>(dawn_control_client->GetContextProviderWeakPtr()->ContextProvider());
    }

    void ClearDestructionCallback()
    {
        destruction_callback_ = nullptr;
    }

    void SetLostContextCallback(base::RepeatingClosure lost_context_callback) override
    {
        lost_context_callback_ = std::move(lost_context_callback);
    }

    void CallLostContextCallback()
    {
...
protected:
    ...
    {
        ExecutionContext* execution_context = ExecutionContext::From(v8_test_scope->GetScriptState());
...
TEST_F(WebGPUContextLostTest, DestructedAfterLastRefDropped)
{
    ...
    auto context_provider = std::make_unique<WebGPUContextProviderForTest>(&destruction_callback);

    auto dawn_control_client = DawnControlClientHolder::Create(std::move(context_provider), execution_context->GetTaskRunner(TaskType::kWebGPU));
...
TEST_F(WebGPUContextLostTest, GPULostContext)
{
    ...
    WebGPUContextProviderForTest::From(dawn_control_client)->CallLostContextCallback();
    ...
    static_cast<WebGPUContextProviderForTest*>(context_provider_weak_ptr->ContextProvider())->ClearDestructionCallback();
}
...
TEST_F(WebGPUContextLostTest, RecreatedAfterGPULostContext)
{
    ...
    auto context_provider2 = std::make_unique<WebGPUContextProviderForTest>(&destruction_callback2);

    auto dawn_control_client2 = DawnControlClientHolder::Create(std::move(context_provider2), execution_context->GetTaskRunner(TaskType::kWebGPU));
    ...
    // Clear the destruction callback since it is stack-allocated in this frame.
    static_cast<WebGPUContextProviderForTest*>(dawn_control_client2->GetContextProviderWeakPtr()->ContextProvider())->ClearDestructionCallback();
}
...
TEST_F(WebGPUContextLostTest, ContextDestroyed)
{
    ...
    EXPECT_CALL(destruction_callback, Run()).Times(1);
    base::RunLoop loop;
    execution_context->GetTaskRunner(TaskType::kWebGPU)->PostTask(FROM_HERE, loop.QuitClosure());
    loop.Run();
    testing::Mock::VerifyAndClear(&destruction_callback);
}
