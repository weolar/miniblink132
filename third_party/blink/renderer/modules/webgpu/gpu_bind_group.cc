...
wgpu::BindGroupEntry AsDawnType(
    const GPUBindGroupEntry* webgpu_binding, Vector<std::unique_ptr<wgpu::ExternalTextureBindingEntry>>* externalTextureBindingEntries)
{
    ...
    case V8GPUBindingResource::ContentType::kGPUBufferBinding: {
        GPUBufferBinding* buffer = webgpu_binding->resource()->GetAsGPUBufferBinding();
    ...
    case V8GPUBindingResource::ContentType::kGPUSampler:
        dawn_binding.sampler = AsDawnType(webgpu_binding->resource()->GetAsGPUSampler());
        break;
    case V8GPUBindingResource::ContentType::kGPUTextureView:
        dawn_binding.textureView = AsDawnType(webgpu_binding->resource()->GetAsGPUTextureView());
        break;
    case V8GPUBindingResource::ContentType::kGPUExternalTexture:
        std::unique_ptr<wgpu::ExternalTextureBindingEntry> externalTextureBindingEntry = std::make_unique<wgpu::ExternalTextureBindingEntry>();
        externalTextureBindingEntry->externalTexture = AsDawnType(webgpu_binding->resource()->GetAsGPUExternalTexture());
        dawn_binding.nextInChain = externalTextureBindingEntry.get();
        externalTextureBindingEntries->push_back(std::move(externalTextureBindingEntry));
        break;
    }
...
base::HeapArray<wgpu::BindGroupEntry> AsDawnType(
    const HeapVector<Member<GPUBindGroupEntry>>& webgpu_objects, Vector<std::unique_ptr<wgpu::ExternalTextureBindingEntry>>* externalTextureBindingEntries)
{
    const wtf_size_t count = webgpu_objects.size();
    auto dawn_objects = base::HeapArray<wgpu::BindGroupEntry>::WithSize(count);
    for (wtf_size_t i = 0; i < count; ++i) {
        dawn_objects[i] = AsDawnType(webgpu_objects[i].Get(), externalTextureBindingEntries);
    }
    return dawn_objects;
}

// static
GPUBindGroup* GPUBindGroup::Create(GPUDevice* device, const GPUBindGroupDescriptor* webgpu_desc, ExceptionState& exception_state)
{
    DCHECK(device);
    DCHECK(webgpu_desc);
    ...
    Vector<std::unique_ptr<wgpu::ExternalTextureBindingEntry>> externalTextureBindingEntries;
    entry_count = static_cast<uint32_t>(webgpu_desc->entries().size());
    if (entry_count > 0) {
        entries = AsDawnType(webgpu_desc->entries(), &externalTextureBindingEntries);
    }
    ...
    GPUBindGroup* bind_group = MakeGarbageCollected<GPUBindGroup>(device, device->GetHandle().CreateBindGroup(&dawn_desc), webgpu_desc->label());
    return bind_group;
}

GPUBindGroup::GPUBindGroup(GPUDevice* device, wgpu::BindGroup bind_group, const String& label)
