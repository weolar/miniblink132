...
bool ConvertToDawn(const GPURenderPassColorAttachment* in, wgpu::RenderPassColorAttachment* out, ExceptionState& exception_state)
{
    DCHECK(in);
    DCHECK(out);
    ...
    *out = {
    ...
    if (in->hasClearValue() && !ConvertToDawn(in->clearValue(), &out->clearValue, exception_state)) {
        return false;
    }
...
std::string ValidateColorAttachmentsDepthSlice(const HeapVector<Member<GPURenderPassColorAttachment>>& in, const char* desc_label)
{
    for (wtf_size_t i = 0; i < in.size(); ++i) {
        ...
        if (attachment->hasDepthSlice() && attachment->depthSlice() == wgpu::kDepthSliceUndefined) {
            std::ostringstream error;
            error << "depthSlice (" << attachment->depthSlice() << ") is too large when validating [GPURenderPassDescriptor";
...
std::string ValidateAndConvertTimestampWrites(const GPUTimestampWrites* webgpu_desc, TimestampWrites* dawn_desc, const char* desc_type, const char* desc_label)
{
    DCHECK(webgpu_desc);
    ...
    if (webgpu_desc->hasBeginningOfPassWriteIndex()) {
        ...
        if (beginningOfPassWriteIndex == wgpu::kQuerySetIndexUndefined) {
            std::ostringstream error;
            error << "beginningOfPassWriteIndex (" << beginningOfPassWriteIndex << ") is too large when validating [" << desc_type;
            if (desc_label != nullptr && strlen(desc_label) != 0) {
                error << " '" << desc_label << "'";
            }
            error << "].";
    ...
    if (webgpu_desc->hasEndOfPassWriteIndex()) {
        ...
        if (endOfPassWriteIndex == wgpu::kQuerySetIndexUndefined) {
            std::ostringstream error;
            error << "endOfPassWriteIndex (" << endOfPassWriteIndex << ") is too large when validating [" << desc_type;
            if (desc_label != nullptr && strlen(desc_label) != 0) {
                error << " '" << desc_label << "'";
            }
            error << "].";
...
wgpu::RenderPassDepthStencilAttachment AsDawnType(GPUDevice* device, const GPURenderPassDepthStencilAttachment* webgpu_desc)
{
    DCHECK(webgpu_desc);
    ...
    wgpu::RenderPassDepthStencilAttachment dawn_desc = {
        ...
        .depthClearValue = webgpu_desc->getDepthClearValueOr(std::numeric_limits<float>::quiet_NaN()),
...
wgpu::ImageCopyBuffer ValidateAndConvertImageCopyBuffer(const GPUImageCopyBuffer* webgpu_view, const char** error)
{
    DCHECK(webgpu_view);
    DCHECK(webgpu_view->buffer());

    wgpu::ImageCopyBuffer dawn_view = { .buffer = webgpu_view->buffer()->GetHandle() };

    *error = ValidateTextureDataLayout(webgpu_view, &dawn_view.layout);
    return dawn_view;
}

wgpu::CommandEncoderDescriptor AsDawnType(const GPUCommandEncoderDescriptor* webgpu_desc, std::string* label)
{
    DCHECK(webgpu_desc);
    DCHECK(label);
...
// static
GPUCommandEncoder* GPUCommandEncoder::Create(GPUDevice* device, const GPUCommandEncoderDescriptor* webgpu_desc)
{
    DCHECK(device);
    DCHECK(webgpu_desc);
    ...
    GPUCommandEncoder* encoder = MakeGarbageCollected<GPUCommandEncoder>(device, device->GetHandle().CreateCommandEncoder(&dawn_desc), webgpu_desc->label());
    return encoder;
}

GPUCommandEncoder::GPUCommandEncoder(GPUDevice* device, wgpu::CommandEncoder command_encoder, const String& label)
    : DawnObject<wgpu::CommandEncoder>(device, std::move(command_encoder), label)
{
}

GPURenderPassEncoder* GPUCommandEncoder::beginRenderPass(const GPURenderPassDescriptor* descriptor, ExceptionState& exception_state)
{
    DCHECK(descriptor);
    ...
    if (dawn_desc.colorAttachmentCount > 0) {
        std::string error = ValidateColorAttachmentsDepthSlice(descriptor->colorAttachments(), label.c_str());
        if (!error.empty()) {
            GetHandle().InjectValidationError(error.c_str());
        }

        if (!ConvertToDawn(descriptor->colorAttachments(), &color_attachments, exception_state)) {
            return nullptr;
        }
    ...
    if (descriptor->hasDepthStencilAttachment()) {
        const GPURenderPassDepthStencilAttachment* depth_stencil = descriptor->depthStencilAttachment();
    ...
    if (descriptor->hasTimestampWrites()) {
        GPURenderPassTimestampWrites* timestamp_writes = descriptor->timestampWrites();
        std::string error = ValidateAndConvertTimestampWrites(timestamp_writes, &timestampWrites, "GPURenderPassDescriptor", label.c_str());
        if (!error.empty()) {
            GetHandle().InjectValidationError(error.c_str());
        } else {
    ...
    GPURenderPassEncoder* encoder = MakeGarbageCollected<GPURenderPassEncoder>(device_, GetHandle().BeginRenderPass(&dawn_desc), descriptor->label());
    return encoder;
}

GPUComputePassEncoder* GPUCommandEncoder::beginComputePass(const GPUComputePassDescriptor* descriptor, ExceptionState& exception_state)
{
    ...
    if (descriptor->hasTimestampWrites()) {
        GPUComputePassTimestampWrites* timestamp_writes = descriptor->timestampWrites();
        std::string error = ValidateAndConvertTimestampWrites(timestamp_writes, &timestampWrites, "GPUComputePassDescriptor", label.c_str());
        if (!error.empty()) {
            GetHandle().InjectValidationError(error.c_str());
        } else {
            dawn_desc.timestampWrites = &timestampWrites;
    ...
    GPUComputePassEncoder* encoder = MakeGarbageCollected<GPUComputePassEncoder>(device_, GetHandle().BeginComputePass(&dawn_desc), descriptor->label());
    return encoder;
}

void GPUCommandEncoder::copyBufferToTexture(
    GPUImageCopyBuffer* source, GPUImageCopyTexture* destination, const V8GPUExtent3D* copy_size, ExceptionState& exception_state)
{
    wgpu::Extent3D dawn_copy_size;
    wgpu::ImageCopyTexture dawn_destination;
    if (!ConvertToDawn(copy_size, &dawn_copy_size, device_, exception_state) || !ConvertToDawn(destination, &dawn_destination, exception_state)) {
        return;
    }

    const char* error = nullptr;
    wgpu::ImageCopyBuffer dawn_source = ValidateAndConvertImageCopyBuffer(source, &error);
    if (error) {
        GetHandle().InjectValidationError(error);
        return;
    }

    GetHandle().CopyBufferToTexture(&dawn_source, &dawn_destination, &dawn_copy_size);
}

void GPUCommandEncoder::copyTextureToBuffer(
    GPUImageCopyTexture* source, GPUImageCopyBuffer* destination, const V8GPUExtent3D* copy_size, ExceptionState& exception_state)
{
    wgpu::Extent3D dawn_copy_size;
    wgpu::ImageCopyTexture dawn_source;
    if (!ConvertToDawn(copy_size, &dawn_copy_size, device_, exception_state) || !ConvertToDawn(source, &dawn_source, exception_state)) {
        return;
    }

    const char* error = nullptr;
    wgpu::ImageCopyBuffer dawn_destination = ValidateAndConvertImageCopyBuffer(destination, &error);
    if (error) {
        GetHandle().InjectValidationError(error);
        return;
    }

    GetHandle().CopyTextureToBuffer(&dawn_source, &dawn_destination, &dawn_copy_size);
}

void GPUCommandEncoder::copyTextureToTexture(
    GPUImageCopyTexture* source, GPUImageCopyTexture* destination, const V8GPUExtent3D* copy_size, ExceptionState& exception_state)
{
    wgpu::Extent3D dawn_copy_size;
    wgpu::ImageCopyTexture dawn_source;
    wgpu::ImageCopyTexture dawn_destination;
    ...
    GetHandle().CopyTextureToTexture(&dawn_source, &dawn_destination, &dawn_copy_size);
}

void GPUCommandEncoder::writeTimestamp(DawnObject<wgpu::QuerySet>* querySet, uint32_t queryIndex, ExceptionState& exception_state)
{
    V8GPUFeatureName::Enum requiredFeatureEnum = V8GPUFeatureName::Enum::kTimestampQuery;
    if (!device_->features()->has(requiredFeatureEnum)) {
        exception_state.ThrowTypeError(String::Format("Use of the writeTimestamp() method requires the '%s' "
                                                      "feature to be enabled on %s.",
            V8GPUFeatureName(requiredFeatureEnum).AsCStr(), device_->formattedLabel().c_str()));
        return;
    }
...
GPUCommandBuffer* GPUCommandEncoder::finish(const GPUCommandBufferDescriptor* descriptor)
{
    ...
    GPUCommandBuffer* command_buffer = MakeGarbageCollected<GPUCommandBuffer>(device_, GetHandle().Finish(&dawn_desc), descriptor->label());
