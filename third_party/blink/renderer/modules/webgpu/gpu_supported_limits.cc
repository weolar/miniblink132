...
namespace {
template <typename T> constexpr T UndefinedLimitValue();

template <> constexpr uint32_t UndefinedLimitValue<uint32_t>()
{
    return wgpu::kLimitU32Undefined;
}

template <> constexpr uint64_t UndefinedLimitValue<uint64_t>()
{
    ...
    : limits_(limits.limits)
{
    for (auto* chain = limits.nextInChain; chain; chain = chain->nextInChain) {
        ...
        case (wgpu::SType::DawnExperimentalSubgroupLimits): {
            auto* t = static_cast<wgpu::DawnExperimentalSubgroupLimits*>(limits.nextInChain);
            subgroup_limits_ = *t;
...
{
#define X(name) out->limits.name = UndefinedLimitValue<decltype(wgpu::Limits::name)>();
    SUPPORTED_LIMITS(X)
#undef X
}

// static
bool GPUSupportedLimits::Populate(wgpu::RequiredLimits* out, const Vector<std::pair<String, uint64_t>>& in, ScriptPromiseResolverBase* resolver)
{
    ...
    for (const auto& [limitName, limitRawValue] : in) {
        if (limitName == "maxInterStageShaderComponents") {
            UseCounter::Count(resolver->GetExecutionContext(), WebFeature::kMaxInterStageShaderComponentsRequiredLimit);
        }
#define X(name)                                                                                                                                                \
    if (limitName == #name) {                                                                                                                                  \
        ...
        if (!value.IsValid() || value.ValueOrDie() == UndefinedLimitValue<T>()) {                                                                              \
            resolver->RejectWithDOMException(DOMExceptionCode::kOperationError,                                                                                \
                "Required " #name " limit (" + String::Number(limitRawValue) + ") exceeds the maximum representable value for its type.");                     \
            return false;                                                                                                                                      \
        }                                                                                                                                                      \
        ...
        SUPPORTED_LIMITS(X)
#undef X
        resolver->RejectWithDOMException(DOMExceptionCode::kOperationError, "The limit \"" + limitName + "\" is not recognized.");
        return false;
    }
    return true;
}
