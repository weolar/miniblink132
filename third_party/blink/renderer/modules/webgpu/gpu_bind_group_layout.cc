...
wgpu::BindGroupLayoutEntry AsDawnType(GPUDevice* device, const GPUBindGroupLayoutEntry* webgpu_binding,
    Vector<std::unique_ptr<wgpu::ExternalTextureBindingLayout>>* externalTextureBindingLayouts, ExceptionState& exception_state)
{
    wgpu::BindGroupLayoutEntry dawn_binding = {
        .binding = webgpu_binding->binding(),
        .visibility = AsDawnFlags<wgpu::ShaderStage>(webgpu_binding->visibility()),
    };
    ...
    if (webgpu_binding->hasStorageTexture()) {
        if (!device->ValidateTextureFormatUsage(webgpu_binding->storageTexture()->format(), exception_state)) {
            return {};
        }
        ...
        dawn_binding.storageTexture = {
            ...
            .viewDimension = AsDawnEnum(webgpu_binding->storageTexture()->viewDimension()),
        };
    }

    if (webgpu_binding->hasExternalTexture()) {
        std::unique_ptr<wgpu::ExternalTextureBindingLayout> externalTextureBindingLayout = std::make_unique<wgpu::ExternalTextureBindingLayout>();
        dawn_binding.nextInChain = externalTextureBindingLayout.get();
        externalTextureBindingLayouts->push_back(std::move(externalTextureBindingLayout));
    }
...
base::HeapArray<wgpu::BindGroupLayoutEntry> AsDawnType(GPUDevice* device, const HeapVector<Member<GPUBindGroupLayoutEntry>>& webgpu_objects,
    Vector<std::unique_ptr<wgpu::ExternalTextureBindingLayout>>* externalTextureBindingLayouts, ExceptionState& exception_state)
{
    const wtf_size_t count = webgpu_objects.size();
    auto dawn_objects = base::HeapArray<wgpu::BindGroupLayoutEntry>::WithSize(count);
    for (wtf_size_t i = 0; i < count; ++i) {
        dawn_objects[i] = AsDawnType(device, webgpu_objects[i].Get(), externalTextureBindingLayouts, exception_state);
    }
    return dawn_objects;
}

// static
GPUBindGroupLayout* GPUBindGroupLayout::Create(GPUDevice* device, const GPUBindGroupLayoutDescriptor* webgpu_desc, ExceptionState& exception_state)
{
    DCHECK(device);
    DCHECK(webgpu_desc);
    ...
    Vector<std::unique_ptr<wgpu::ExternalTextureBindingLayout>> externalTextureBindingLayouts;
    entry_count = static_cast<uint32_t>(webgpu_desc->entries().size());
    if (entry_count > 0) {
        entries = AsDawnType(device, webgpu_desc->entries(), &externalTextureBindingLayouts, exception_state);
    }
    ...
    GPUBindGroupLayout* layout = MakeGarbageCollected<GPUBindGroupLayout>(device, device->GetHandle().CreateBindGroupLayout(&dawn_desc), webgpu_desc->label());
    return layout;
}

GPUBindGroupLayout::GPUBindGroupLayout(GPUDevice* device, wgpu::BindGroupLayout bind_group_layout, const String& label)
    : DawnObject<wgpu::BindGroupLayout>(device, std::move(bind_group_layout), label)
{
}
