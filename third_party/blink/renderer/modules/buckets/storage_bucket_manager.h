...
class MODULES_EXPORT StorageBucketManager final : public ScriptWrappable, public Supplement<NavigatorBase>, public ExecutionContextClient {
...
public:
    ...
    ScriptPromise<StorageBucket> open(ScriptState* script_state, const String& name, const StorageBucketOptions* options, ExceptionState& exception_state);
    ScriptPromise<IDLSequence<IDLString>> keys(ScriptState* script_state, ExceptionState& exception_state);
    ScriptPromise<IDLUndefined> Delete(ScriptState* script_state, const String& name, ExceptionState& exception_state);

    // GarbageCollected
    void Trace(Visitor*) const override;

    // These are not exposed to the web applications and only used by DevTools.
    StorageBucket* GetBucketForDevtools(ScriptState* script_state, const String& name);

private:
    mojom::blink::BucketManagerHost* GetBucketManager(ScriptState* script_state);

    void DidOpen(ScriptPromiseResolver<StorageBucket>* resolver, const String& name, mojo::PendingRemote<mojom::blink::BucketHost> bucket_remote,
        mojom::blink::BucketError error);
    void DidGetKeys(ScriptPromiseResolver<IDLSequence<IDLString>>* resolver, const Vector<String>& keys, bool success);
