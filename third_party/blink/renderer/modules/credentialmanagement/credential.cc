...
Credential::Credential(const String& id, const String& type)
    ...
    , type_(type)
{
    DCHECK(!id_.empty() || type == kDigitalCredentialType || type == kFederatedCredentialType || type == kIdentityCredentialType || type == kOtpCredentialType);
    DCHECK(!type_.empty());
}

KURL Credential::ParseStringAsURLOrThrow(const String& url, ExceptionState& exception_state)
{
    if (url.empty())
        return KURL();
