...
mojom::blink::CredentialManager* CredentialManagerProxy::CredentialManager()
{
    if (!credential_manager_.is_bound()) {
        ...
        DCHECK(frame);
        frame->GetBrowserInterfaceBroker().GetInterface(credential_manager_.BindNewPipeAndPassReceiver(frame->GetTaskRunner(TaskType::kUserInteraction)));
    }
...
mojom::blink::Authenticator* CredentialManagerProxy::Authenticator()
{
    if (!authenticator_.is_bound()) {
        LocalFrame* frame = GetSupplementable()->GetFrame();
        DCHECK(frame);
        frame->GetBrowserInterfaceBroker().GetInterface(authenticator_.BindNewPipeAndPassReceiver(frame->GetTaskRunner(TaskType::kUserInteraction)));
    }
...
mojom::blink::WebOTPService* CredentialManagerProxy::WebOTPService()
{
    if (!webotp_service_.is_bound()) {
        LocalFrame* frame = GetSupplementable()->GetFrame();
        DCHECK(frame);
        frame->GetBrowserInterfaceBroker().GetInterface(webotp_service_.BindNewPipeAndPassReceiver(frame->GetTaskRunner(TaskType::kMiscPlatformAPI)));
    }
    return webotp_service_.get();
}

payments::mojom::blink::PaymentCredential* CredentialManagerProxy::PaymentCredential()
{
    if (!payment_credential_.is_bound()) {
        LocalFrame* frame = GetSupplementable()->GetFrame();
        DCHECK(frame);
        frame->GetBrowserInterfaceBroker().GetInterface(payment_credential_.BindNewPipeAndPassReceiver(frame->GetTaskRunner(TaskType::kMiscPlatformAPI)));
    }
    return payment_credential_.get();
}

template <typename Interface> void CredentialManagerProxy::BindRemoteForFedCm(HeapMojoRemote<Interface>& remote, base::OnceClosure disconnect_closure)
{
    if (remote.is_bound())
        return;
    ...
    frame->GetBrowserInterfaceBroker().GetInterface(remote.BindNewPipeAndPassReceiver(frame->GetTaskRunner(TaskType::kUserInteraction)));
    remote.set_disconnect_handler(std::move(disconnect_closure));
}

mojom::blink::FederatedAuthRequest* CredentialManagerProxy::FederatedAuthRequest()
{
    BindRemoteForFedCm(federated_auth_request_, WTF::BindOnce(&CredentialManagerProxy::OnFederatedAuthRequestConnectionError, WrapWeakPersistent(this)));
...
mojom::blink::DigitalIdentityRequest* CredentialManagerProxy::DigitalIdentityRequest()
{
    BindRemoteForFedCm(digital_identity_request_, WTF::BindOnce(&CredentialManagerProxy::OnDigitalIdentityRequestConnectionError, WrapWeakPersistent(this)));
...
// static
CredentialManagerProxy* CredentialManagerProxy::From(ScriptState* script_state)
{
...
{
    auto* supplement = Supplement<LocalDOMWindow>::From<CredentialManagerProxy>(*window);
    if (!supplement) {
...
// static
CredentialManagerProxy* CredentialManagerProxy::From(ExecutionContext* execution_context)
{
