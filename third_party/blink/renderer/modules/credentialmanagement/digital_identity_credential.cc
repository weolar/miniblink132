...
String ValidateAndStringifyObject(ScriptPromiseResolver<IDLNullable<Credential>>* resolver, const ScriptValue& input)
{
    v8::Local<v8::String> value;
    if (input.IsEmpty() || !input.V8Value()->IsObject()
        || !v8::JSON::Stringify(resolver->GetScriptState()->GetContext(), input.V8Value().As<v8::Object>()).ToLocal(&value)) {
        resolver->RejectWithTypeError("IdentityRequestProvider request objects should either by strings or "
                                      "JSON-Serializable objects.");
        return String();
    }

    String output = ToBlinkString<String>(resolver->GetScriptState()->GetIsolate(), value, kDoNotExternalize);
    ...
    if (output.length() > kMaxJSONStringLength) {
        resolver->RejectWithTypeError(String::Format("JSON serialization of IdentityRequestProvider request "
...
void OnCompleteRequest(ScriptPromiseResolver<IDLNullable<Credential>>* resolver, std::unique_ptr<ScopedAbortState> scoped_abort_state,
    RequestDigitalIdentityStatus status, const WTF::String& protocol, const WTF::String& token)
{
    ...
    case RequestDigitalIdentityStatus::kErrorNoProviders:
        resolver->RejectWithTypeError("Digital identity API needs at least one provider.");
        return;
    ...
    case RequestDigitalIdentityStatus::kSuccess: {
        UseCounter::Count(resolver->GetExecutionContext(), WebFeature::kIdentityDigitalCredentialsSuccess);

        DigitalCredential* credential = DigitalCredential::Create(protocol, token);
...
void DiscoverDigitalIdentityCredentialFromExternalSource(
    ScriptPromiseResolver<IDLNullable<Credential>>* resolver, ExceptionState& exception_state, const CredentialRequestOptions& options)
{
    CHECK(IsDigitalIdentityCredentialType(options));
    CHECK(RuntimeEnabledFeatures::WebIdentityDigitalCredentialsEnabled(resolver->GetExecutionContext()));

    if (!CheckGenericSecurityRequirementsForCredentialsContainerRequest(resolver)) {
        return;
    }

    if (!resolver->GetExecutionContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kDigitalCredentialsGet)) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotAllowedError,
    ...
    for (const auto& provider : options.digital()->providers()) {
        ...
        if (request_object_or_string->IsString()) {
            ...
            stringified_request = ValidateAndStringifyObject(resolver, request_object_or_string->GetAsObject());
            if (stringified_request.IsNull()) {
                continue;
            }
        }

        blink::mojom::blink::DigitalCredentialProviderPtr digital_credential_provider = blink::mojom::blink::DigitalCredentialProvider::New();
    ...
    UseCounter::Count(resolver->GetExecutionContext(), WebFeature::kIdentityDigitalCredentials);
    ...
    auto* request = CredentialManagerProxy::From(script_state)->DigitalIdentityRequest();
    request->Request(std::move(providers), WTF::BindOnce(&OnCompleteRequest, WrapPersistent(resolver), std::move(scoped_abort_state)));
}
