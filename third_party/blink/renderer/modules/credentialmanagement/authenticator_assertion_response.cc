...
AuthenticatorAssertionResponse::AuthenticatorAssertionResponse(const Vector<uint8_t> client_data_json, const Vector<uint8_t> authenticator_data,
    const Vector<uint8_t> signature, std::optional<Vector<uint8_t>> optional_user_handle)
    : AuthenticatorAssertionResponse(DOMArrayBuffer::Create(client_data_json), DOMArrayBuffer::Create(authenticator_data), DOMArrayBuffer::Create(signature),
        optional_user_handle && optional_user_handle->size() > 0 ? DOMArrayBuffer::Create(std::move(*optional_user_handle)) : nullptr)
{
}

AuthenticatorAssertionResponse::AuthenticatorAssertionResponse(
    DOMArrayBuffer* client_data_json, DOMArrayBuffer* authenticator_data, DOMArrayBuffer* signature, DOMArrayBuffer* user_handle)
...
absl::variant<AuthenticatorAssertionResponseJSON*, AuthenticatorAttestationResponseJSON*> AuthenticatorAssertionResponse::toJSON() const
{
