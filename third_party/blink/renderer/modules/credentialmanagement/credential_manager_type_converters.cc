...
// static
CredentialInfoPtr TypeConverter<CredentialInfoPtr, blink::Credential*>::Convert(blink::Credential* credential)
{
    auto info = CredentialInfo::New();
    info->id = credential->id();
    if (credential->IsPasswordCredential()) {
        ::blink::PasswordCredential* password_credential = static_cast<::blink::PasswordCredential*>(credential);
        ...
        ::blink::FederatedCredential* federated_credential = static_cast<::blink::FederatedCredential*>(credential);
        info->type = CredentialType::FEDERATED;
        info->password = g_empty_string;
        scoped_refptr<const blink::SecurityOrigin> origin = federated_credential->GetProviderAsOrigin();
        info->federation = url::SchemeHostPort(origin->Protocol().Utf8(), origin->Host().Utf8(), origin->Port());
...
// static
blink::Credential* TypeConverter<blink::Credential*, CredentialInfoPtr>::Convert(const CredentialInfoPtr& info)
{
    switch (info->type) {
    case CredentialType::FEDERATED:
        return blink::FederatedCredential::Create(info->id,
            blink::SecurityOrigin::CreateFromValidTuple(
                String::FromUTF8(info->federation.scheme()), String::FromUTF8(info->federation.host()), info->federation.port()),
            info->name, info->icon);
    case CredentialType::PASSWORD:
        return blink::PasswordCredential::Create(info->id, info->password, info->name, info->icon);
...
static Vector<Vector<uint32_t>> UvmEntryToArray(const Vector<blink::mojom::blink::UvmEntryPtr>& user_verification_methods)
{
    Vector<Vector<uint32_t>> uvm_array;
    for (const auto& uvm : user_verification_methods) {
        Vector<uint32_t> uvmEntry = { uvm->user_verification_method, uvm->key_protection_type, uvm->matcher_protection_type };
...
TypeConverter<blink::AuthenticationExtensionsClientOutputs*, blink::mojom::blink::AuthenticationExtensionsClientOutputsPtr>::Convert(
    const blink::mojom::blink::AuthenticationExtensionsClientOutputsPtr& extensions)
{
    auto* extension_outputs = blink::AuthenticationExtensionsClientOutputs::Create();
    ...
    if (extensions->echo_user_verification_methods) {
        extension_outputs->setUvm(UvmEntryToArray(std::move(*extensions->user_verification_methods)));
    }
#endif
    if (extensions->echo_large_blob) {
        DCHECK(blink::RuntimeEnabledFeatures::WebAuthenticationLargeBlobExtensionEnabled());
        blink::AuthenticationExtensionsLargeBlobOutputs* large_blob_outputs = blink::AuthenticationExtensionsLargeBlobOutputs::Create();
        if (extensions->large_blob) {
            large_blob_outputs->setBlob(blink::DOMArrayBuffer::Create(std::move(*extensions->large_blob)));
        }
    ...
    if (extensions->get_cred_blob) {
        extension_outputs->setGetCredBlob(blink::DOMArrayBuffer::Create(std::move(*extensions->get_cred_blob)));
    }
    if (extensions->supplemental_pub_keys) {
        extension_outputs->setSupplementalPubKeys(ConvertTo<blink::AuthenticationExtensionsSupplementalPubKeysOutputs*>(extensions->supplemental_pub_keys));
    }
    if (extensions->echo_prf) {
        ...
        if (extensions->prf_results) {
            ...
            values->setFirst(
                MakeGarbageCollected<blink::V8UnionArrayBufferOrArrayBufferView>(blink::DOMArrayBuffer::Create(std::move(extensions->prf_results->first))));
            if (extensions->prf_results->second) {
                values->setSecond(MakeGarbageCollected<blink::V8UnionArrayBufferOrArrayBufferView>(
                    blink::DOMArrayBuffer::Create(std::move(extensions->prf_results->second.value()))));
            }
...
TypeConverter<blink::AuthenticationExtensionsSupplementalPubKeysOutputs*, blink::mojom::blink::SupplementalPubKeysResponsePtr>::Convert(
    const blink::mojom::blink::SupplementalPubKeysResponsePtr& supplemental_pub_keys)
{
    ...
    auto* spk_outputs = blink::AuthenticationExtensionsSupplementalPubKeysOutputs::Create();
    spk_outputs->setSignatures(std::move(signatures));
    return spk_outputs;
}

// static
Vector<uint8_t> TypeConverter<Vector<uint8_t>, blink::V8UnionArrayBufferOrArrayBufferView*>::Convert(const blink::V8UnionArrayBufferOrArrayBufferView* buffer)
{
    DCHECK(buffer);
    ...
    case blink::V8UnionArrayBufferOrArrayBufferView::ContentType::kArrayBuffer:
        ...
        break;
    case blink::V8UnionArrayBufferOrArrayBufferView::ContentType::kArrayBufferView:
...
// static
std::optional<PublicKeyCredentialType> TypeConverter<std::optional<PublicKeyCredentialType>, String>::Convert(const String& type)
{
    if (type == "public-key") {
...
// static
std::optional<AuthenticatorTransport> TypeConverter<std::optional<AuthenticatorTransport>, String>::Convert(const String& transport)
{
    if (transport == "usb") {
...
// static
String TypeConverter<String, AuthenticatorTransport>::Convert(const AuthenticatorTransport& transport)
{
...
// static
std::optional<blink::mojom::blink::ResidentKeyRequirement> TypeConverter<std::optional<blink::mojom::blink::ResidentKeyRequirement>, String>::Convert(
...
// static
std::optional<UserVerificationRequirement> TypeConverter<std::optional<UserVerificationRequirement>, String>::Convert(const String& requirement)
{
    if (requirement == "required") {
...
// static
std::optional<AttestationConveyancePreference> TypeConverter<std::optional<AttestationConveyancePreference>, String>::Convert(const String& preference)
{
    if (preference == "none") {
...
// static
std::optional<AuthenticatorAttachment> TypeConverter<std::optional<AuthenticatorAttachment>, std::optional<String>>::Convert(
    const std::optional<String>& attachment)
{
...
// static
LargeBlobSupport TypeConverter<LargeBlobSupport, std::optional<String>>::Convert(const std::optional<String>& large_blob_support)
{
...
// static
AuthenticatorSelectionCriteriaPtr TypeConverter<AuthenticatorSelectionCriteriaPtr, blink::AuthenticatorSelectionCriteria>::Convert(
    const blink::AuthenticatorSelectionCriteria& criteria)
{
    auto mojo_criteria = blink::mojom::blink::AuthenticatorSelectionCriteria::New();

    mojo_criteria->authenticator_attachment = AuthenticatorAttachment::NO_PREFERENCE;
    if (criteria.hasAuthenticatorAttachment()) {
        std::optional<String> attachment = criteria.authenticatorAttachment();
        auto maybe_attachment = ConvertTo<std::optional<AuthenticatorAttachment>>(attachment);
        if (maybe_attachment) {
    ...
    if (criteria.hasResidentKey()) {
        resident_key = ConvertTo<std::optional<ResidentKeyRequirement>>(criteria.residentKey());
    }
    if (resident_key) {
        mojo_criteria->resident_key = *resident_key;
    } else {
        mojo_criteria->resident_key = criteria.requireResidentKey() ? ResidentKeyRequirement::REQUIRED : ResidentKeyRequirement::DISCOURAGED;
    }

    mojo_criteria->user_verification = UserVerificationRequirement::PREFERRED;
    if (criteria.hasUserVerification()) {
        std::optional<UserVerificationRequirement> user_verification = ConvertTo<std::optional<UserVerificationRequirement>>(criteria.userVerification());
...
// static
PublicKeyCredentialUserEntityPtr TypeConverter<PublicKeyCredentialUserEntityPtr, blink::PublicKeyCredentialUserEntity>::Convert(
...
// static
PublicKeyCredentialRpEntityPtr TypeConverter<PublicKeyCredentialRpEntityPtr, blink::PublicKeyCredentialRpEntity>::Convert(
...
// static
PublicKeyCredentialDescriptorPtr TypeConverter<PublicKeyCredentialDescriptorPtr, blink::PublicKeyCredentialDescriptor>::Convert(
    const blink::PublicKeyCredentialDescriptor& descriptor)
{
    std::optional<PublicKeyCredentialType> type = ConvertTo<std::optional<PublicKeyCredentialType>>(descriptor.type());
    if (!type) {
        return nullptr;
    }
    ...
    if (descriptor.hasTransports() && !descriptor.transports().empty()) {
        for (const auto& transport : descriptor.transports()) {
            auto maybe_transport(ConvertTo<std::optional<AuthenticatorTransport>>(transport));
            if (maybe_transport) {
                mojo_descriptor->transports.push_back(*maybe_transport);
            }
        }
    } else {
        mojo_descriptor->transports = { AuthenticatorTransport::USB, AuthenticatorTransport::BLE, AuthenticatorTransport::NFC, AuthenticatorTransport::HYBRID,
...
// static
PublicKeyCredentialParametersPtr TypeConverter<PublicKeyCredentialParametersPtr, blink::PublicKeyCredentialParameters>::Convert(
    const blink::PublicKeyCredentialParameters& parameter)
{
    std::optional<PublicKeyCredentialType> type = ConvertTo<std::optional<PublicKeyCredentialType>>(parameter.type());
    if (!type) {
        return nullptr;
    }
    auto mojo_parameter = PublicKeyCredentialParameters::New();
...
// static
PublicKeyCredentialCreationOptionsPtr TypeConverter<PublicKeyCredentialCreationOptionsPtr, blink::PublicKeyCredentialCreationOptions>::Convert(
    const blink::PublicKeyCredentialCreationOptions& options)
{
    auto mojo_options = blink::mojom::blink::PublicKeyCredentialCreationOptions::New();
    ...
    if (options.pubKeyCredParams().size() == 0) {
        ...
        for (auto& parameter : options.pubKeyCredParams()) {
            PublicKeyCredentialParametersPtr normalized_parameter = PublicKeyCredentialParameters::From(*parameter);
    ...
    for (auto& descriptor : options.excludeCredentials()) {
        PublicKeyCredentialDescriptorPtr mojo_descriptor = PublicKeyCredentialDescriptor::From(*descriptor);
        if (mojo_descriptor) {
    ...
    if (options.hasAuthenticatorSelection()) {
        mojo_options->authenticator_selection = AuthenticatorSelectionCriteria::From(*options.authenticatorSelection());
    }
    ...
    if (options.hasAttestation()) {
        std::optional<AttestationConveyancePreference> attestation = ConvertTo<std::optional<AttestationConveyancePreference>>(options.attestation());
        if (attestation) {
    ...
    if (options.hasExtensions()) {
        ...
        if (extensions->hasCredentialProtectionPolicy()) {
            ...
            if (policy == "userVerificationOptional") {
                ...
                mojo_options->protection_policy = blink::mojom::ProtectionPolicy::UV_REQUIRED;
            } else {
                return nullptr;
            }
        }
        if (extensions->hasEnforceCredentialProtectionPolicy() && extensions->enforceCredentialProtectionPolicy()) {
        ...
        if (extensions->hasCredBlob()) {
            mojo_options->cred_blob = ConvertTo<Vector<uint8_t>>(extensions->credBlob());
        }
        if (extensions->hasPayment() && extensions->payment()->hasIsPayment() && extensions->payment()->isPayment()) {
        ...
        if (extensions->hasRemoteDesktopClientOverride()) {
            mojo_options->remote_desktop_client_override = RemoteDesktopClientOverride::From(*extensions->remoteDesktopClientOverride());
        }
        if (extensions->hasSupplementalPubKeys()) {
            auto supplemental_pub_keys = ConvertTo<std::optional<SupplementalPubKeysRequestPtr>>(*extensions->supplementalPubKeys());
        ...
        if (extensions->hasPrf()) {
            ...
            if (extensions->prf()->hasEval()) {
                mojo_options->prf_input = ConvertTo<PRFValuesPtr>(*extensions->prf()->eval());
            }
        }
    }
...
static Vector<uint8_t> ConvertFixedSizeArray(const blink::V8BufferSource* buffer, unsigned length)
{
...
// static
CableAuthenticationPtr TypeConverter<CableAuthenticationPtr, blink::CableAuthenticationData>::Convert(const blink::CableAuthenticationData& data)
{
    ...
    case 1:
        ...
        entity->authenticator_eid = ConvertFixedSizeArray(data.authenticatorEid(), 16);
        entity->session_pre_key = ConvertFixedSizeArray(data.sessionPreKey(), 32);
        if (entity->client_eid->empty() || entity->authenticator_eid->empty() || entity->session_pre_key->empty()) {
            return nullptr;
        }
        break;

    case 2:
        entity->server_link_data = ConvertTo<Vector<uint8_t>>(data.sessionPreKey());
...
// static
PublicKeyCredentialRequestOptionsPtr TypeConverter<PublicKeyCredentialRequestOptionsPtr, blink::PublicKeyCredentialRequestOptions>::Convert(
    const blink::PublicKeyCredentialRequestOptions& options)
{
    auto mojo_options = blink::mojom::blink::PublicKeyCredentialRequestOptions::New();
    mojo_options->challenge = ConvertTo<Vector<uint8_t>>(options.challenge());
    ...
    if (options.hasUserVerification()) {
        std::optional<UserVerificationRequirement> user_verification = ConvertTo<std::optional<UserVerificationRequirement>>(options.userVerification());
        if (user_verification) {
    ...
    if (options.hasExtensions()) {
        mojo_options->extensions = ConvertTo<blink::mojom::blink::AuthenticationExtensionsClientInputsPtr>(*options.extensions());
    } else {
        mojo_options->extensions = blink::mojom::blink::AuthenticationExtensionsClientInputs::New();
    }
...
// static
AuthenticationExtensionsClientInputsPtr TypeConverter<AuthenticationExtensionsClientInputsPtr, blink::AuthenticationExtensionsClientInputs>::Convert(
    const blink::AuthenticationExtensionsClientInputs& inputs)
{
    auto mojo_inputs = blink::mojom::blink::AuthenticationExtensionsClientInputs::New();
    if (inputs.hasAppid()) {
    ...
    if (inputs.hasLargeBlob()) {
        ...
        if (inputs.largeBlob()->hasWrite()) {
            mojo_inputs->large_blob_write = ConvertTo<Vector<uint8_t>>(inputs.largeBlob()->write());
        }
    }
    ...
    if (inputs.hasRemoteDesktopClientOverride()) {
        mojo_inputs->remote_desktop_client_override = RemoteDesktopClientOverride::From(*inputs.remoteDesktopClientOverride());
    }
    if (inputs.hasSupplementalPubKeys()) {
        auto supplemental_pub_keys = ConvertTo<std::optional<SupplementalPubKeysRequestPtr>>(*inputs.supplementalPubKeys());
        if (supplemental_pub_keys) {
...
// static
RemoteDesktopClientOverridePtr TypeConverter<RemoteDesktopClientOverridePtr, blink::RemoteDesktopClientOverride>::Convert(
    const blink::RemoteDesktopClientOverride& blink_value)
{
    return RemoteDesktopClientOverride::New(blink::SecurityOrigin::CreateFromString(blink_value.origin()), blink_value.sameOriginWithAncestors());
}

// static
IdentityProviderConfigPtr TypeConverter<IdentityProviderConfigPtr, blink::IdentityProviderConfig>::Convert(const blink::IdentityProviderConfig& provider)
{
...
// static
IdentityProviderRequestOptionsPtr TypeConverter<IdentityProviderRequestOptionsPtr, blink::IdentityProviderRequestOptions>::Convert(
    const blink::IdentityProviderRequestOptions& options)
{
    ...
    if (blink::RuntimeEnabledFeatures::FedCmIdPRegistrationEnabled() && options.configURL() == "any") {
    ...
    mojo_options->domain_hint = blink::RuntimeEnabledFeatures::FedCmDomainHintEnabled() ? options.getDomainHintOr("") : "";
    ...
    if (options.hasParams()) {
        ...
        v8::MaybeLocal<v8::String> json = v8::JSON::Stringify(isolate->GetCurrentContext(), options.params().V8Value());
        if (json.IsEmpty()) {
            return nullptr;
        }
        mojo_options->params_json = blink::ToCoreString(isolate, json.ToLocalChecked());
    }
...
// static
RpContext TypeConverter<RpContext, blink::V8IdentityCredentialRequestOptionsContext>::Convert(const blink::V8IdentityCredentialRequestOptionsContext& context)
{
...
// static
RpMode TypeConverter<RpMode, blink::V8IdentityCredentialRequestOptionsMode>::Convert(const blink::V8IdentityCredentialRequestOptionsMode& mode)
{
    switch (mode.AsEnum()) {
...
IdentityUserInfoPtr TypeConverter<IdentityUserInfoPtr, blink::IdentityUserInfo>::Convert(const blink::IdentityUserInfo& user_info)
{
...
TypeConverter<std::optional<SupplementalPubKeysRequestPtr>, blink::AuthenticationExtensionsSupplementalPubKeysInputs>::Convert(
    const blink::AuthenticationExtensionsSupplementalPubKeysInputs& supplemental_pub_keys)
{
    ...
    ret->attestation
        = ConvertTo<std::optional<AttestationConveyancePreference>>(supplemental_pub_keys.attestation()).value_or(AttestationConveyancePreference::NONE);
    ret->attestation_formats = supplemental_pub_keys.attestationFormats();
    return ret;
}

// static
PRFValuesPtr TypeConverter<PRFValuesPtr, blink::AuthenticationExtensionsPRFValues>::Convert(const blink::AuthenticationExtensionsPRFValues& values)
{
...
// static
Vector<PRFValuesPtr> TypeConverter<Vector<PRFValuesPtr>, blink::AuthenticationExtensionsPRFInputs>::Convert(const blink::AuthenticationExtensionsPRFInputs& prf)
{
...
// static
IdentityCredentialDisconnectOptionsPtr TypeConverter<IdentityCredentialDisconnectOptionsPtr, blink::IdentityCredentialDisconnectOptions>::Convert(
    const blink::IdentityCredentialDisconnectOptions& options)
{
    ...
    mojo_disconnect_options->config->config_url = blink::KURL(options.configURL());
...
Vector<Hint> TypeConverter<Vector<Hint>, Vector<String>>::Convert(const Vector<String>& hints)
{
    Vector<Hint> ret;
...
TypeConverter<blink::mojom::blink::PublicKeyCredentialReportOptionsPtr, blink::UnknownCredentialOptions>::Convert(
    const blink::UnknownCredentialOptions& options)
{
    auto mojo_options = blink::mojom::blink::PublicKeyCredentialReportOptions::New();
    mojo_options->relying_party_id = options.rpId();
    // The fact that this decodes successfully has already been tested.
    mojo_options->unknown_credential_id = Base64UnpaddedURLDecodeOrCheck(options.credentialId());
    return mojo_options;
}

// static
blink::mojom::blink::PublicKeyCredentialReportOptionsPtr
TypeConverter<blink::mojom::blink::PublicKeyCredentialReportOptionsPtr, blink::AllAcceptedCredentialsOptions>::Convert(
    const blink::AllAcceptedCredentialsOptions& options)
{
    auto mojo_options = blink::mojom::blink::PublicKeyCredentialReportOptions::New();
    mojo_options->relying_party_id = options.rpId();
    mojo_options->all_accepted_credentials = blink::mojom::blink::AllAcceptedCredentialsOptions::New();
    // The fact that this decodes successfully has already been tested.
    mojo_options->all_accepted_credentials->user_id = Base64UnpaddedURLDecodeOrCheck(options.userId());
    for (WTF::String credential_id : options.allAcceptedCredentialIds()) {
        // The fact that this decodes successfully has already been tested.
        mojo_options->all_accepted_credentials->all_accepted_credentials_ids.push_back(Base64UnpaddedURLDecodeOrCheck(credential_id));
    }
    return mojo_options;
}
...
// static
blink::mojom::blink::PublicKeyCredentialReportOptionsPtr
TypeConverter<blink::mojom::blink::PublicKeyCredentialReportOptionsPtr, blink::CurrentUserDetailsOptions>::Convert(
    const blink::CurrentUserDetailsOptions& options)
{
    auto mojo_options = blink::mojom::blink::PublicKeyCredentialReportOptions::New();
    mojo_options->relying_party_id = options.rpId();
    mojo_options->current_user_details = blink::mojom::blink::CurrentUserDetailsOptions::New();
    // The fact that this decodes successfully has already been tested.
    mojo_options->current_user_details->user_id = Base64UnpaddedURLDecodeOrCheck(options.userId());
