...
PublicKeyCredentialUserEntity* PublicKeyCredentialUserEntityFromJSON(const PublicKeyCredentialUserEntityJSON& json, ExceptionState& exception_state)
{
    auto* result = PublicKeyCredentialUserEntity::Create();
    if (auto id = WebAuthnBase64UrlDecode(json.id()); id.has_value()) {
        result->setId(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(*id));
    } else {
        exception_state.ThrowDOMException(DOMExceptionCode::kEncodingError, "'user.id' contains invalid base64url data");
        return nullptr;
    }
...
PublicKeyCredentialDescriptor* PublicKeyCredentialDescriptorFromJSON(
    std::string_view field_name, const PublicKeyCredentialDescriptorJSON& json, ExceptionState& exception_state)
{
...
VectorOf<PublicKeyCredentialDescriptor> PublicKeyCredentialDescriptorVectorFromJSON(
    std::string_view field_name, const VectorOf<PublicKeyCredentialDescriptorJSON> json, ExceptionState& exception_state)
{
    VectorOf<PublicKeyCredentialDescriptor> result;
    for (const PublicKeyCredentialDescriptorJSON* json_descriptor : json) {
        auto* descriptor = PublicKeyCredentialDescriptorFromJSON(field_name, *json_descriptor, exception_state);
...
std::optional<AuthenticationExtensionsPRFValues*> AuthenticationExtensionsPRFValuesFromJSON(const AuthenticationExtensionsPRFValuesJSON& json)
{
    ...
    values->setFirst(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(first.value()));
    if (json.hasSecond()) {
        auto second = WebAuthnBase64UrlDecode(json.second());
        if (!second.has_value()) {
            return std::nullopt;
        }
        values->setSecond(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(second.value()));
    }
    return values;
}

AuthenticationExtensionsClientInputs* AuthenticationExtensionsClientInputsFromJSON(
    const AuthenticationExtensionsClientInputsJSON& json, ExceptionState& exception_state)
{
    ...
    if (json.hasEnforceCredentialProtectionPolicy()) {
        result->setEnforceCredentialProtectionPolicy(json.enforceCredentialProtectionPolicy());
    }
    ...
    if (json.hasLargeBlob()) {
        ...
        if (json.largeBlob()->hasWrite()) {
            if (auto write = WebAuthnBase64UrlDecode(json.largeBlob()->write()); write.has_value()) {
                large_blob->setWrite(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(write.value()));
            } else {
                exception_state.ThrowDOMException(DOMExceptionCode::kEncodingError, "'extensions.largeBlob.write' contains invalid base64url data");
                return nullptr;
            }
        }
    ...
    if (json.hasCredBlob()) {
        if (auto cred_blob = WebAuthnBase64UrlDecode(json.credBlob()); cred_blob.has_value()) {
            result->setCredBlob(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(cred_blob.value()));
        } else {
            exception_state.ThrowDOMException(DOMExceptionCode::kEncodingError, "'extensions.credBlob' contains invalid base64url data");
            return nullptr;
        }
    }
    ...
    if (json.hasPrf()) {
        ...
        if (json.prf()->hasEval()) {
            std::optional<AuthenticationExtensionsPRFValues*> eval = AuthenticationExtensionsPRFValuesFromJSON(*(json.prf()->eval()));
            if (!eval) {
                exception_state.ThrowDOMException(DOMExceptionCode::kEncodingError, "'extensions.prf.eval' contains invalid base64url data");
                return nullptr;
            }
        ...
        if (json.prf()->hasEvalByCredential()) {
            ...
            for (const auto& [key, json_values] : json.prf()->evalByCredential()) {
                std::optional<AuthenticationExtensionsPRFValues*> values = AuthenticationExtensionsPRFValuesFromJSON(*json_values);
                if (!values) {
                    exception_state.ThrowDOMException(DOMExceptionCode::kEncodingError,
...
AuthenticationExtensionsClientOutputsJSON* AuthenticationExtensionsClientOutputsToJSON(
    ScriptState* script_state, const blink::AuthenticationExtensionsClientOutputs& in)
{
    ...
    if (in.hasPrf()) {
        ...
        if (prf.hasResults()) {
            ...
            results_builder.AddString("first", WebAuthnBase64UrlEncode(prf.results()->first()));
            if (prf.results()->hasSecond()) {
                results_builder.AddString("second", WebAuthnBase64UrlEncode(prf.results()->second()));
            }
        }
        json->setPrf(builder.GetScriptValue());
    }
    if (in.hasSupplementalPubKeys()) {
        const AuthenticationExtensionsSupplementalPubKeysOutputs& supplemental_pub_keys = *in.supplementalPubKeys();
        V8ObjectBuilder builder(script_state);
        if (supplemental_pub_keys.hasSignatures()) {
            builder.AddVector<DOMArrayBuffer>("signatures", supplemental_pub_keys.signatures());
        }
...
PublicKeyCredentialCreationOptions* PublicKeyCredentialCreationOptionsFromJSON(
    const PublicKeyCredentialCreationOptionsJSON* json, ExceptionState& exception_state)
{
    ...
    result->setUser(user);
    if (auto challenge = WebAuthnBase64UrlDecode(json->challenge()); challenge.has_value()) {
        result->setChallenge(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(*challenge));
    } else {
        exception_state.ThrowDOMException(DOMExceptionCode::kEncodingError, "'challenge' contains invalid base64url data");
        return nullptr;
    }
    ...
    if (json->hasExcludeCredentials()) {
        VectorOf<PublicKeyCredentialDescriptor> credential_descriptors
            = PublicKeyCredentialDescriptorVectorFromJSON("excludeCredentials", json->excludeCredentials(), exception_state);
        if (exception_state.HadException()) {
            return nullptr;
        }
    ...
    if (json->hasExtensions()) {
        auto* extensions = AuthenticationExtensionsClientInputsFromJSON(*json->extensions(), exception_state);
        if (exception_state.HadException()) {
            return nullptr;
        }
...
PublicKeyCredentialRequestOptions* PublicKeyCredentialRequestOptionsFromJSON(const PublicKeyCredentialRequestOptionsJSON* json, ExceptionState& exception_state)
{
    ...
    if (json->hasAllowCredentials()) {
        VectorOf<PublicKeyCredentialDescriptor> credential_descriptors
            = PublicKeyCredentialDescriptorVectorFromJSON("allowCredentials", json->allowCredentials(), exception_state);
        if (exception_state.HadException()) {
            return nullptr;
        }
