...
class MODULES_EXPORT AuthenticatorAttestationResponse final : public AuthenticatorResponse {
    DEFINE_WRAPPERTYPEINFO();

public:
    AuthenticatorAttestationResponse(DOMArrayBuffer* client_data_json, DOMArrayBuffer* attestation_object, Vector<mojom::AuthenticatorTransport> transports,
        DOMArrayBuffer* authenticator_data, DOMArrayBuffer* public_key_der, int32_t public_key_algorithm);
    ...
    DOMArrayBuffer* getPublicKey() const
    {
        return public_key_der_.Get();
    }

    int32_t getPublicKeyAlgorithm() const
    {
    ...
    absl::variant<AuthenticatorAssertionResponseJSON*, AuthenticatorAttestationResponseJSON*> toJSON() const override;
