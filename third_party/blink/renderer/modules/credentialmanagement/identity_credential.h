...
public:
    static IdentityCredential* Create(const String& token, bool is_auto_selected = false, const String& config_url = WTF::g_empty_string);

    static bool IsRejectingPromiseDueToCSP(ContentSecurityPolicy* policy, ScriptPromiseResolverBase* resolver, const KURL& provider_url);

    explicit IdentityCredential(const String& token, bool is_auto_selected, const String& config_url);
    ...
    const bool& isAutoSelected() const
    {
        return is_auto_selected_;
    }
    ...
    static ScriptPromise<IDLUndefined> disconnect(ScriptState*, const IdentityCredentialDisconnectOptions* options, ExceptionState&);
