...
struct ExtensionsClientInputsValues {
    ...
    std::optional<std::vector<std::pair<std::string, PRFValues>>> prf_eval_by_credential;
};

AuthenticationExtensionsClientInputsJSON* MakeExtensionsInputsJSON(const ExtensionsClientInputsValues& in)
{
    ...
    if (in.cred_protect) {
        extensions->setCredentialProtectionPolicy(String(in.cred_protect->credential_protection_policy));
        extensions->setEnforceCredentialProtectionPolicy(in.cred_protect->enforce_credential_protection_policy);
    }
    if (in.cred_props) {
    ...
    if (in.prf_eval_by_credential) {
        HeapVector<std::pair<String, Member<AuthenticationExtensionsPRFValuesJSON>>> prf_values_by_cred;
        for (const auto& cred_and_values : *in.prf_eval_by_credential) {
            ...
            prf_values_by_cred.emplace_back(String(cred_and_values.first), prf_values);
        }
        auto* prf_inputs = AuthenticationExtensionsPRFInputsJSON::Create();
...
PublicKeyCredentialCreationOptionsJSON* MakeCreationOptionsJSON(const CredentialCreationOptionsValues& in)
{
    ...
    json->setPubKeyCredParams(VectorOf<PublicKeyCredentialParameters>({ pub_key_cred_params }));
    if (in.timeout) {
    ...
    if (in.authenticator_selection) {
        ...
        if (in.authenticator_selection->attachment) {
            authenticator_selection->setAuthenticatorAttachment(String(*in.authenticator_selection->attachment));
        }
        if (in.authenticator_selection->resident_key) {
            authenticator_selection->setResidentKey(String(*in.authenticator_selection->resident_key));
        }
        authenticator_selection->setRequireResidentKey(in.authenticator_selection->require_resident_key);
        authenticator_selection->setUserVerification(String(in.authenticator_selection->user_verification));
...
PublicKeyCredentialRequestOptionsJSON* MakeRequestOptionsJSON(const CredentialRequestOptionsValues& in)
{
...
template <typename T> Matcher<Member<T>> MemberField(Matcher<T*> matcher)
{
...
MATCHER(CredentialDescriptorsEq, "")
{
    ...
    return ExplainMatchResult(MemberField<PublicKeyCredentialDescriptor>(AllOf(Property("id", &PublicKeyCredentialDescriptor::id, Base64URL(StrEq(values.id))),
                                  Property("type", &PublicKeyCredentialDescriptor::type, StrEq(values.type)))),
...
{
    return ExplainMatchResult(AllOf(Property(&AuthenticationExtensionsPRFValues::first, Base64URL(StrEq(values.first))),
                                  Property(&AuthenticationExtensionsPRFValues::second, Base64URL(StrEq(values.second)))),
        arg, result_listener);
}
...
{
    const std::pair<String, Member<AuthenticationExtensionsPRFValues>>& actual = std::get<0>(arg);
    const std::pair<std::string, ExtensionsClientInputsValues::PRFValues>& expected = std::get<1>(arg);
    return ExplainMatchResult(AllOf(Field(&std::pair<String, Member<AuthenticationExtensionsPRFValues>>::first, StrEq(expected.first)),
                                  Field(&std::pair<String, Member<AuthenticationExtensionsPRFValues>>::second,
                                      MemberField<AuthenticationExtensionsPRFValues>(PRFValuesEq(expected.second)))),
...
void ExpectExtensionsMatch(const AuthenticationExtensionsClientInputs& extensions, const ExtensionsClientInputsValues& values)
{
    if (values.appid) {
    ...
    if (values.cred_protect) {
        EXPECT_THAT(extensions.credentialProtectionPolicy(), StrEq(values.cred_protect->credential_protection_policy));
        EXPECT_EQ(extensions.enforceCredentialProtectionPolicy(), values.cred_protect->enforce_credential_protection_policy);
    } else {
        EXPECT_FALSE(extensions.hasCredentialProtectionPolicy());
        EXPECT_FALSE(extensions.enforceCredentialProtectionPolicy()); // defaults to 'false'
    }
    ...
    if (values.prf_eval) {
        EXPECT_THAT(extensions.prf()->eval(), MemberField<AuthenticationExtensionsPRFValues>(PRFValuesEq(*values.prf_eval)));
    } else {
        EXPECT_TRUE(!extensions.hasPrf() || !extensions.prf()->hasEval());
    }
    if (values.prf_eval_by_credential) {
        EXPECT_THAT(extensions.prf()->evalByCredential(), Pointwise(PRFCredIdAndValuesEq(), *values.prf_eval_by_credential));
    } else {
        EXPECT_TRUE(!extensions.hasPrf() || !extensions.prf()->hasEvalByCredential());
    }
    EXPECT_EQ(extensions.hasPrf(), values.prf_eval || values.prf_eval_by_credential);
}

// Tests `PublicKeyCredentialCreationOptions` and `CreationOptionsValues` for
// equality. Invoke with SUBTEST().
void ExpectCreationOptionsMatches(const PublicKeyCredentialCreationOptions& options, const CredentialCreationOptionsValues& values)
{
    EXPECT_THAT(options.rp()->id(), StrEq(values.rp_id));
    EXPECT_THAT(options.user(),
        AllOf(Property("id", &PublicKeyCredentialUserEntity::id, Base64URL(StrEq(values.user_id))),
            Property("name", &PublicKeyCredentialUserEntity::name, StrEq(values.user_name))));
    EXPECT_THAT(options.pubKeyCredParams(),
        testing::ElementsAre(MemberField<PublicKeyCredentialParameters>(PubKeyCredParamsEq(values.pub_key_cred_params_type, values.pub_key_cred_params_alg))));
    ...
    EXPECT_THAT(options.excludeCredentials(), Pointwise(CredentialDescriptorsEq(), values.exclude_credentials));
    if (values.authenticator_selection) {
        if (values.authenticator_selection->attachment) {
            EXPECT_THAT(options.authenticatorSelection()->authenticatorAttachment(), StrEq(*values.authenticator_selection->attachment));
        } else {
            EXPECT_FALSE(options.authenticatorSelection()->hasAuthenticatorAttachment());
        }
        if (values.authenticator_selection->resident_key) {
            EXPECT_THAT(options.authenticatorSelection()->residentKey(), StrEq(*values.authenticator_selection->resident_key));
        } else {
            EXPECT_FALSE(options.authenticatorSelection()->hasResidentKey());
        }
        EXPECT_EQ(options.authenticatorSelection()->requireResidentKey(), values.authenticator_selection->require_resident_key);
        EXPECT_THAT(options.authenticatorSelection()->userVerification(), StrEq(values.authenticator_selection->user_verification));
    } else {
...
// equality. Invoke with SUBTEST().
void ExpectRequestOptionsMatches(const PublicKeyCredentialRequestOptions& options, const CredentialRequestOptionsValues& values)
{
    EXPECT_THAT(options.challenge(), Base64URL(StrEq(values.challenge)));
    if (values.timeout.has_value()) {
        EXPECT_EQ(options.timeout(), *values.timeout);
    ...
    EXPECT_THAT(options.allowCredentials(), Pointwise(CredentialDescriptorsEq(), values.allow_credentials));
...
TEST(PublicKeyCredentialTest, ParseCreationOptionsFromJSON)
{
    ...
    PublicKeyCredentialCreationOptionsJSON* json = MakeCreationOptionsJSON(options_values);
    DummyExceptionStateForTesting exception_state;
    const PublicKeyCredentialCreationOptions* options = PublicKeyCredential::parseCreationOptionsFromJSON(script_state, json, exception_state);
...
TEST(PublicKeyCredentialTest, ParseRequestOptions)
{
    ...
    PublicKeyCredentialRequestOptionsJSON* json = MakeRequestOptionsJSON(options_values);
    DummyExceptionStateForTesting exception_state;
    const PublicKeyCredentialRequestOptions* options = PublicKeyCredential::parseRequestOptionsFromJSON(script_state, json, exception_state);
    EXPECT_FALSE(exception_state.HadException());
    ASSERT_NE(options, nullptr);
...
TEST(PublicKeyCredentialTest, ParseCreationOptionsFromJSON_Full)
{
    ...
    CredentialCreationOptionsValues options_values {
        ...
        .authenticator_selection = CredentialCreationOptionsValues::AuthSelection { .attachment = "cross-platform",
...
TEST(PublicKeyCredentialTest, ParseCreationOptionsFromJSON_WithExtensions)
{
    ...
    DummyPageHolder holder;
    ScriptState* script_state = ToScriptStateForMainWorld(&holder.GetFrame());
    static const ExtensionsClientInputsValues::PRFValues kTestPRFValues { kTestB64URL, kTest2B64URL };
    static const ExtensionsClientInputsValues kTestCases[] = {
        { .appid_exclude = "https://example.com/appid.json" },
        { .cred_protect = ExtensionsClientInputsValues::CredProtect { "Level One", true } },
        { .cred_props = true },
...
TEST(PublicKeyCredentialTest, ParseCreationOptionsFromJSON_InvalidBase64URL)
{
    ...
    static const struct {
        ...
        { { .user_id = kInvalidB64URL }, "'user.id' contains invalid base64url data" },
        { { .challenge = kInvalidB64URL }, "'challenge' contains invalid base64url data" },
    ...
    for (const auto& t : kTestCases) {
        ...
        PublicKeyCredentialCreationOptionsJSON* json = MakeCreationOptionsJSON(t.in);
        DummyPageHolder holder;
        ScriptState* script_state = ToScriptStateForMainWorld(&holder.GetFrame());
        DummyExceptionStateForTesting exception_state;
        const PublicKeyCredentialCreationOptions* options = PublicKeyCredential::parseCreationOptionsFromJSON(script_state, json, exception_state);
