...
public:
    ...
    void Bind(mojo::PendingReceiver<::blink::mojom::blink::CredentialManager> receiver)
    {
        receiver_.Bind(std::move(receiver));
        receiver_.set_disconnect_handler(WTF::BindOnce(&MockCredentialManager::Disconnected, WTF::Unretained(this)));
    }
    ...
    void InvokeGetCallback()
    {
        ...
        std::move(get_callback_).Run(blink::mojom::blink::CredentialManagerError::SUCCESS, std::move(info));
    }

protected:
    void Store(blink::mojom::blink::CredentialInfoPtr credential, StoreCallback callback) override
    {
    }
    void PreventSilentAccess(PreventSilentAccessCallback callback) override
    {
    }
    void Get(blink::mojom::blink::CredentialMediationRequirement mediation, int requested_credential_types, const WTF::Vector<::blink::KURL>& federations,
...
public:
    MockAuthenticatorInterface()
    {
        loop_ = std::make_unique<base::RunLoop>();
    }

    MockAuthenticatorInterface(const MockAuthenticatorInterface&) = delete;
    MockAuthenticatorInterface& operator=(const MockAuthenticatorInterface&) = delete;

    void Bind(mojo::PendingReceiver<::blink::mojom::blink::Authenticator> receiver)
    {
        receiver_.Bind(std::move(receiver));
    }
    ...
    {
        EXPECT_TRUE(receiver_.is_bound());
        std::move(get_callback_).Run(blink::mojom::blink::AuthenticatorStatus::NOT_ALLOWED_ERROR, nullptr, nullptr);
    }
...
protected:
    ...
    void Report(blink::mojom::blink::PublicKeyCredentialReportOptionsPtr options, ReportCallback callback) override
    {
    }
...
public:
    explicit CredentialManagerTestingContext(MockCredentialManager* mock_credential_manager, MockAuthenticatorInterface* mock_authenticator = nullptr)
        : dummy_context_(KURL("https://example.test"))
    {
        if (mock_credential_manager) {
            DomWindow().GetBrowserInterfaceBroker().SetBinderForTesting(::blink::mojom::blink::CredentialManager::Name_,
                WTF::BindRepeating(
                    [](MockCredentialManager* mock_credential_manager, mojo::ScopedMessagePipeHandle handle) {
                        mock_credential_manager->Bind(mojo::PendingReceiver<::blink::mojom::blink::CredentialManager>(std::move(handle)));
                    },
                    WTF::Unretained(mock_credential_manager)));
        }
        if (mock_authenticator) {
            DomWindow().GetBrowserInterfaceBroker().SetBinderForTesting(::blink::mojom::blink::Authenticator::Name_,
                WTF::BindRepeating(
                    [](MockAuthenticatorInterface* mock_authenticator, mojo::ScopedMessagePipeHandle handle) {
                        mock_authenticator->Bind(mojo::PendingReceiver<::blink::mojom::blink::Authenticator>(std::move(handle)));
                    },
    ...
    {
        return dummy_context_.GetWindow();
    }
    ScriptState* GetScriptState()
    {
        return dummy_context_.GetScriptState();
    }
...
TEST(AuthenticationCredentialsContainerTest, PendingGetRequest_NoGCCycles)
{
...

    {
        ...
        AuthenticationCredentialsContainer::credentials(*context.DomWindow().navigator())
            ->get(context.GetScriptState(), CredentialRequestOptions::Create(), IGNORE_EXCEPTION_FOR_TESTING);
...
TEST(AuthenticationCredentialsContainerTest, PendingGetRequest_NoCrashOnResponseAfterDocumentShutdown)
{
    test::TaskEnvironment task_environment;
    MockCredentialManager mock_credential_manager;
    CredentialManagerTestingContext context(&mock_credential_manager);

    auto promise = AuthenticationCredentialsContainer::credentials(*context.DomWindow().navigator())
                       ->get(context.GetScriptState(), CredentialRequestOptions::Create(), IGNORE_EXCEPTION_FOR_TESTING);
    mock_credential_manager.WaitForCallToGet();
...
TEST(AuthenticationCredentialsContainerTest, RejectPublicKeyCredentialStoreOperation)
{
    ...
    auto promise = AuthenticationCredentialsContainer::credentials(*context.DomWindow().navigator())
                       ->store(context.GetScriptState(), MakeGarbageCollected<MockPublicKeyCredential>(), IGNORE_EXCEPTION_FOR_TESTING);

    EXPECT_EQ(v8::Promise::kRejected, promise.V8Promise()->State());
}

TEST(AuthenticationCredentialsContainerTest, GetPasswordAndFederatedCredentialUseCounters)
{
    test::TaskEnvironment task_environment;
    {
        // Password only.
        MockCredentialManager mock_credential_manager;
        CredentialManagerTestingContext context(&mock_credential_manager);
        context.DomWindow().document()->ClearUseCounterForTesting(WebFeature::kCredentialManagerGetPasswordCredential);
        context.DomWindow().document()->ClearUseCounterForTesting(WebFeature::kCredentialManagerGetLegacyFederatedCredential);
        auto* request_options = CredentialRequestOptions::Create();
        request_options->setPassword(true);
        AuthenticationCredentialsContainer::credentials(*context.DomWindow().navigator())
            ->get(context.GetScriptState(), request_options, IGNORE_EXCEPTION_FOR_TESTING);
        mock_credential_manager.WaitForCallToGet();
        EXPECT_TRUE(context.DomWindow().document()->IsUseCounted(WebFeature::kCredentialManagerGetPasswordCredential));
        EXPECT_FALSE(context.DomWindow().document()->IsUseCounted(WebFeature::kCredentialManagerGetLegacyFederatedCredential));
...

    {
        ...
        AuthenticationCredentialsContainer::credentials(*context.DomWindow().navigator())
            ->get(context.GetScriptState(), request_options, IGNORE_EXCEPTION_FOR_TESTING);
        mock_credential_manager.WaitForCallToGet();
        EXPECT_FALSE(context.DomWindow().document()->IsUseCounted(WebFeature::kCredentialManagerGetPasswordCredential));
        EXPECT_TRUE(context.DomWindow().document()->IsUseCounted(WebFeature::kCredentialManagerGetLegacyFederatedCredential));
...
TEST(AuthenticationCredentialsContainerTest, PublicKeyConditionalMediationUkm)
{
    ...
    CredentialManagerTestingContext context(/*mock_credential_manager=*/nullptr, &mock_authenticator);
    ...
    auto* public_key_request_options = PublicKeyCredentialRequestOptions::Create();
    ...
    public_key_request_options->setChallenge(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(DOMArrayBuffer::Create(challenge)));
    ...
    promise = AuthenticationCredentialsContainer::credentials(*context.DomWindow().navigator())
                  ->get(context.GetScriptState(), request_options, IGNORE_EXCEPTION_FOR_TESTING);
    mock_authenticator.WaitForCallToGet();
...
class AuthenticationCredentialsContainerActiveModeMultiIdpTest : public testing::Test,
...
TEST_F(AuthenticationCredentialsContainerActiveModeMultiIdpTest, RejectActiveModeWithMultipleIdps)
{
    test::TaskEnvironment task_environment;
    MockCredentialManager mock_credential_manager;
    CredentialManagerTestingContext context(&mock_credential_manager);
    ...
    IdentityCredentialRequestOptions* identity = IdentityCredentialRequestOptions::Create();
