...
public:
    static ScriptPromise<IDLSequence<IdentityUserInfo>> getUserInfo(ScriptState*, const blink::IdentityProviderConfig*, ExceptionState&);

    static void close(ScriptState*);
    static ScriptPromise<IDLBoolean> registerIdentityProvider(ScriptState*, const String&);
    static ScriptPromise<IDLUndefined> unregisterIdentityProvider(ScriptState*, const String&);
    static ScriptPromise<IDLUndefined> resolve(
        ScriptState*, const V8UnionIdentityProviderTokenOrUSVString* token, const IdentityResolveOptions* options = nullptr);
};
