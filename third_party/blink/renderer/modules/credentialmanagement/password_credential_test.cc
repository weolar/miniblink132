...
TEST_F(PasswordCredentialTest, CreateFromFormNoPassword)
{
    ...
    PasswordCredential* credential = PasswordCredential::Create(form, exception_state);
    EXPECT_EQ(nullptr, credential);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(ESErrorType::kTypeError, exception_state.CodeAs<ESErrorType>());
    EXPECT_EQ("Either 'current-password' or 'new-password' must be specified in the "
...
TEST_F(PasswordCredentialTest, CreateFromFormNoId)
{
    ...
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(ESErrorType::kTypeError, exception_state.CodeAs<ESErrorType>());
    EXPECT_EQ("'username' must be specified in the form's autocomplete attribute.", exception_state.Message());
}
