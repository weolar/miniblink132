...
public:
    ...
    MockDigitalIdentityRequest& operator=(const MockDigitalIdentityRequest&) = delete;
    ...
    void Request(std::vector<blink::mojom::DigitalCredentialProviderPtr> providers, RequestCallback callback) override
    {
        std::move(callback).Run(mojom::RequestDigitalIdentityStatus::kSuccess, "protocol", "token");
    }
    void Abort() override
    {
    }
...
CredentialRequestOptions* CreateOptionsWithProviders(const HeapVector<Member<IdentityRequestProvider>>& providers)
{
    DigitalCredentialRequestOptions* digital_credential_request = DigitalCredentialRequestOptions::Create();
...
{
    IdentityRequestProvider* identity_provider = IdentityRequestProvider::Create();
    identity_provider->setRequest(MakeGarbageCollected<V8UnionObjectOrString>(String()));
...
public:
    ...
    DigitalIdentityCredentialTest& operator=(const DigitalIdentityCredentialTest&) = delete;
...
TEST_F(DigitalIdentityCredentialTest, IdentityDigitalCredentialUseCounter)
{
    ...
    context.GetWindow().GetBrowserInterfaceBroker().SetBinderForTesting(mojom::DigitalIdentityRequest::Name_,
        WTF::BindRepeating(
            [](MockDigitalIdentityRequest* mock_request_ptr, mojo::ScopedMessagePipeHandle handle) {
                mock_request_ptr->Bind(mojo::PendingReceiver<mojom::DigitalIdentityRequest>(std::move(handle)));
            },
    ...
    DiscoverDigitalIdentityCredentialFromExternalSource(resolver, context.GetExceptionState(), *CreateValidOptions());

    test::RunPendingTasks();

    EXPECT_TRUE(context.GetWindow().document()->IsUseCounted(blink::mojom::WebFeature::kIdentityDigitalCredentials));
    EXPECT_TRUE(context.GetWindow().document()->IsUseCounted(blink::mojom::WebFeature::kIdentityDigitalCredentialsSuccess));
}
