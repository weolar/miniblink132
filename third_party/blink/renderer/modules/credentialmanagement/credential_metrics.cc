...
{
    Document* document = To<LocalDOMWindow>(ExecutionContext::From(script_state))->document();
    CredentialMetrics* supplement = Supplement<Document>::From<CredentialMetrics>(document);
    if (!supplement) {
...
void CredentialMetrics::RecordWebAuthnConditionalUiCall()
{
    ...
    if (!document->GetTiming().DomContentLoadedEventEnd().is_null()) {
        base::TimeDelta delta = base::TimeTicks::Now() - document->GetTiming().DomContentLoadedEventEnd();
        delta_ms = delta.InMilliseconds();
    }

    ukm::builders::WebAuthn_ConditionalUiGetCall(document->domWindow()->UkmSourceID()).SetTimeSinceDomContentLoaded(delta_ms).Record(document->UkmRecorder());
}
