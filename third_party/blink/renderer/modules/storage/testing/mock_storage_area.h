...
public:
    ...
    struct ObservedPut {
        ...
        {
            return std::tie(key, value, source) == std::tie(other.key, other.value, other.source);
        }
    };
    ...
    void AddObserver(mojo::PendingRemote<mojom::blink::StorageAreaObserver> observer) override;
    void Put(const Vector<uint8_t>& key, const Vector<uint8_t>& value, const std::optional<Vector<uint8_t>>& client_old_value, const String& source,
        PutCallback callback) override;
    void Delete(const Vector<uint8_t>& key, const std::optional<Vector<uint8_t>>& client_old_value, const String& source, DeleteCallback callback) override;
    void DeleteAll(const String& source, mojo::PendingRemote<mojom::blink::StorageAreaObserver> new_observer, DeleteAllCallback callback) override;
    void Get(const Vector<uint8_t>& key, GetCallback callback) override;
    void GetAll(mojo::PendingRemote<mojom::blink::StorageAreaObserver> new_observer, GetAllCallback callback) override;
    ...
    int observed_get_alls() const
    {
        return observed_get_alls_;
    }
    const Vector<ObservedPut>& observed_puts() const
    {
        return observed_puts_;
    }
    ...
    size_t observed_checkpoints() const
    {
        return observed_checkpoints_;
    }
