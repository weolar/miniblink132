...
public:
    ...
    void BindSessionStorageNamespace(const String& namespace_id, mojo::PendingReceiver<mojom::blink::SessionStorageNamespace> receiver) override
    {
    }
    void BindSessionStorageArea(const blink::BlinkStorageKey& storage_key, const blink::LocalFrameToken& local_frame_token, const String& namespace_id,
        mojo::PendingReceiver<mojom::blink::StorageArea> receiver) override
    {
...
TEST(StorageControllerTest, CacheLimit)
{
    ...
    LocalDOMWindow* local_dom_window_root = To<LocalDOMWindow>(web_view_helper_root.InitializeAndLoad(kRootString)->GetPage()->MainFrame()->DomWindow());
    Persistent<FakeAreaSource> source_area = MakeGarbageCollected<FakeAreaSource>(kRootUrl, local_dom_window_root);

    StorageController::DomStorageConnection connection;
    PostCrossThreadTask(*base::ThreadPool::CreateSequencedTaskRunner({}), FROM_HERE,
        CrossThreadBindOnce(
            [](mojo::PendingReceiver<mojom::blink::DomStorage> receiver) {
                mojo::MakeSelfOwnedReceiver(std::make_unique<MockDomStorage>(), std::move(receiver));
            },
    ...
    LocalDOMWindow* local_dom_window = To<LocalDOMWindow>(web_view_helper.InitializeAndLoad(kPageString)->GetPage()->MainFrame()->DomWindow());
    ...
    LocalDOMWindow* local_dom_window2 = To<LocalDOMWindow>(web_view_helper2.InitializeAndLoad(kPageString2)->GetPage()->MainFrame()->DomWindow());
    ...
    test::ScopedMockedURLLoad scoped_mocked_url_load3(kPageUrl3, test::CoreTestDataPath("foo.html"));
    frame_test_helpers::WebViewHelper web_view_helper3;
    LocalDOMWindow* local_dom_window3 = To<LocalDOMWindow>(web_view_helper3.InitializeAndLoad(kPageString3)->GetPage()->MainFrame()->DomWindow());
...
TEST(StorageControllerTest, CacheLimitSessionStorage)
{
    ...
    PostCrossThreadTask(*task_runner, FROM_HERE,
        CrossThreadBindOnce(
            [](std::unique_ptr<MockDomStorage> dom_storage_ptr, mojo::PendingReceiver<mojom::blink::DomStorage> receiver) {
                mojo::MakeSelfOwnedReceiver(std::move(dom_storage_ptr), std::move(receiver));
            },
            std::move(mock_dom_storage), connection.dom_storage_remote.BindNewPipeAndPassReceiver()));

    StorageController controller(std::move(connection), kTestCacheLimit);

    StorageNamespace* ns1 = controller.CreateSessionStorageNamespace(*local_dom_window_root->GetFrame()->GetPage(), kNamespace1);
    StorageNamespace* ns2 = controller.CreateSessionStorageNamespace(*local_dom_window_root->GetFrame()->GetPage(), kNamespace2);
    ...
    int32_t opens = 0;
    {
        ...
        task_runner->PostTaskAndReply(
            FROM_HERE, base::BindOnce(&MockDomStorage::GetSessionStorageUsage, base::Unretained(dom_storage_ptr), &opens), loop.QuitClosure());
        loop.Run();
    }
    EXPECT_EQ(opens, 3);
}
