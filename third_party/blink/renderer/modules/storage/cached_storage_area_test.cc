...
public:
    ...
    const BlinkStorageKey kRootStorageKey = BlinkStorageKey::CreateFromStringForTesting(kRootString.c_str());
    ...
    void SetUp() override
    {
        const CachedStorageArea::AreaType area_type
            = IsSessionStorage() ? CachedStorageArea::AreaType::kSessionStorage : CachedStorageArea::AreaType::kLocalStorage;
        test::ScopedMockedURLLoad scoped_mocked_url_load_root(kRootUrl, test::CoreTestDataPath("foo.html"));
        LocalDOMWindow* local_dom_window_root = To<LocalDOMWindow>(web_view_helper_root_.InitializeAndLoad(kRootString)->GetPage()->MainFrame()->DomWindow());
        cached_area_ = base::MakeRefCounted<CachedStorageArea>(area_type, kRootStorageKey, local_dom_window_root, nullptr,
            /*is_session_storage_for_prerendering=*/false);
        cached_area_->SetRemoteAreaForTesting(mock_storage_area_.GetInterfaceRemote());
        test::ScopedMockedURLLoad scoped_mocked_url_load(kPageUrl, test::CoreTestDataPath("foo.html"));
        LocalDOMWindow* local_dom_window = To<LocalDOMWindow>(web_view_helper_.InitializeAndLoad(kPageString)->GetPage()->MainFrame()->DomWindow());
        source_area_ = MakeGarbageCollected<FakeAreaSource>(kPageUrl, local_dom_window);
        source_area_id_ = cached_area_->RegisterSource(source_area_);
        source_ = kPageUrl.GetString() + "\n" + source_area_id_;
        test::ScopedMockedURLLoad scoped_mocked_url_load2(kPageUrl2, test::CoreTestDataPath("foo.html"));
        LocalDOMWindow* local_dom_window2 = To<LocalDOMWindow>(web_view_helper2_.InitializeAndLoad(kPageString2)->GetPage()->MainFrame()->DomWindow());
        source_area2_ = MakeGarbageCollected<FakeAreaSource>(kPageUrl2, local_dom_window2);
        cached_area_->RegisterSource(source_area2_);
    }

    virtual bool IsSessionStorage()
    {
        return false;
    }
    ...
    static Vector<uint8_t> StringToUint8Vector(const String& input, FormatOption format)
    {
        return CachedStorageArea::StringToUint8Vector(input, format);
    }

    static String Uint8VectorToString(const Vector<uint8_t>& input, FormatOption format)
    {
    ...
    {
        return StringToUint8Vector(key, IsSessionStorage() ? FormatOption::kSessionStorageForceUTF8 : FormatOption::kLocalStorageDetectFormat);
    }

    Vector<uint8_t> ValueToUint8Vector(const String& value)
    {
        return StringToUint8Vector(value, IsSessionStorage() ? FormatOption::kSessionStorageForceUTF16 : FormatOption::kLocalStorageDetectFormat);
    }

    String KeyFromUint8Vector(const Vector<uint8_t>& key)
    {
        return Uint8VectorToString(key, IsSessionStorage() ? FormatOption::kSessionStorageForceUTF8 : FormatOption::kLocalStorageDetectFormat);
    }

    String ValueFromUint8Vector(const Vector<uint8_t>& value)
    {
        return Uint8VectorToString(value, IsSessionStorage() ? FormatOption::kSessionStorageForceUTF16 : FormatOption::kLocalStorageDetectFormat);
    }

    MockStorageArea::ObservedPut ObservedPut(const String& key, const String& value, const String& source)
    {
        return MockStorageArea::ObservedPut { KeyToUint8Vector(key), ValueToUint8Vector(value), source };
    }

    MockStorageArea::ObservedDelete ObservedDelete(const String& key, const String& source)
    {
        return MockStorageArea::ObservedDelete { KeyToUint8Vector(key), source };
    }

    FakeAreaSource::Event Event(const String& key, const String& old_value, const String& new_value)
    {
    ...
    {
        mock_storage_area_.InjectKeyValue(KeyToUint8Vector(key), ValueToUint8Vector(value));
    }
...
class CachedStorageAreaTestWithParam : public CachedStorageAreaTest, public testing::WithParamInterface<bool> {
public:
    bool IsSessionStorage() override
    {
        return GetParam();
    }
};

INSTANTIATE_TEST_SUITE_P(CachedStorageAreaTest, CachedStorageAreaTestWithParam, ::testing::Bool());
...
TEST_P(CachedStorageAreaTestWithParam, SetItem)
{
    ...
    mock_storage_area_.Flush();
    EXPECT_THAT(mock_storage_area_.observed_puts(), ElementsAre(ObservedPut(kKey, kValue, source_)));
...
TEST_P(CachedStorageAreaTestWithParam, RemoveItem)
{
    ...
    EXPECT_TRUE(IsCacheLoaded());
    EXPECT_EQ(1, mock_storage_area_.observed_get_alls());
    EXPECT_THAT(mock_storage_area_.observed_deletes(), ElementsAre(ObservedDelete(kKey, source_)));
...
TEST_P(CachedStorageAreaTestWithParam, ResetConnectionWithKeyDiff)
{
    ...
    if (IsSessionStorage()) {
        ...
        EXPECT_THAT(mock_storage_area_.observed_puts(), ElementsAre(ObservedPut(kKey2, kCachedValue2, "\n")));
        EXPECT_TRUE(mock_storage_area_.observed_deletes().empty());
        EXPECT_TRUE(source_area_->events.empty());
    } else {
        ...
        EXPECT_TRUE(mock_storage_area_.observed_puts().empty());
        EXPECT_TRUE(mock_storage_area_.observed_deletes().empty());
        EXPECT_THAT(source_area_->events, ElementsAre(Event(kKey2, kCachedValue2, kPersistedValue2)));
    }
}
...
TEST_P(CachedStorageAreaTestWithParam, ResetConnectionWithMissingBackendKey)
{
    ...
    if (IsSessionStorage()) {
        ...
        EXPECT_EQ(2u, cached_area_->GetLength());
        EXPECT_EQ(kValue2, cached_area_->GetItem(kKey2));
        EXPECT_THAT(mock_storage_area_.observed_puts(), ElementsAre(ObservedPut(kKey2, kValue2, "\n")));
        EXPECT_TRUE(mock_storage_area_.observed_deletes().empty());
        EXPECT_TRUE(source_area_->events.empty());
    } else {
        // For Local Storage, we expect no mutations to the backend but instead a
        ...
        EXPECT_TRUE(mock_storage_area_.observed_puts().empty());
        EXPECT_TRUE(mock_storage_area_.observed_deletes().empty());
        EXPECT_THAT(source_area_->events, ElementsAre(Event(kKey2, kValue2, String())));
    }
}
...
TEST_P(CachedStorageAreaTestWithParam, ResetConnectionWithMissingLocalKey)
{
    ...
    if (IsSessionStorage()) {
        ...
        EXPECT_EQ(1u, cached_area_->GetLength());
        EXPECT_TRUE(cached_area_->GetItem(kKey2).IsNull());
        EXPECT_THAT(mock_storage_area_.observed_deletes(), ElementsAre(ObservedDelete(kKey2, "\n")));
        EXPECT_TRUE(mock_storage_area_.observed_puts().empty());
        EXPECT_TRUE(source_area_->events.empty());
    } else {
        // For Local Storage, we expect no mutations to the backend but instead a
        ...
        EXPECT_TRUE(mock_storage_area_.observed_puts().empty());
        EXPECT_TRUE(mock_storage_area_.observed_deletes().empty());
        EXPECT_THAT(source_area_->events, ElementsAre(Event(kKey2, String(), kValue2)));
    }
}
...
TEST_P(CachedStorageAreaTestWithParam, ResetConnectionWithComplexDiff)
{
    ...
    if (IsSessionStorage()) {
        ...
        EXPECT_TRUE(mock_storage_area_.observed_deletes().empty());
        EXPECT_THAT(
            source_area_->events, UnorderedElementsAre(Event(kKey2, kValue2, kAltValue2), Event(kKey3, kValue3, String()), Event(kKey4, String(), kValue4)));
    }
}
...
TEST_F(CachedStorageAreaTest, KeyMutationsAreIgnoredUntilCompletion)
{
    ...
    EXPECT_TRUE(IsIgnoringKeyMutations(kKey));
    EXPECT_EQ(kValue, cached_area_->GetItem(kKey));
    observer->KeyChanged(KeyToUint8Vector(kKey), ValueToUint8Vector(kValue), std::nullopt, source_);
    ...
    mock_storage_area_.Flush();
    EXPECT_TRUE(IsIgnoringKeyMutations(kKey));
    observer->KeyDeleted(KeyToUint8Vector(kKey), ValueToUint8Vector(kValue), source_);
    EXPECT_FALSE(IsIgnoringKeyMutations(kKey));
...
TEST_F(CachedStorageAreaTest, ChangeEvents)
{
    ...
    cached_area_->RemoveItem(kKey, source_area_);
    observer->KeyChanged(KeyToUint8Vector(kKey), ValueToUint8Vector(kValue), std::nullopt, source_);
    observer->KeyChanged(KeyToUint8Vector(kKey), ValueToUint8Vector(kValue2), ValueToUint8Vector(kValue), source_);
    observer->KeyDeleted(KeyToUint8Vector(kKey), ValueToUint8Vector(kValue2), source_);

    observer->KeyChanged(KeyToUint8Vector(kKey), ValueToUint8Vector(kValue), std::nullopt, kRemoteSource);
...
TEST_F(CachedStorageAreaTest, RevertOnChangeFailedWithSubsequentChanges)
{
    ...
    observer->KeyChangeFailed(KeyToUint8Vector(kKey), source_);
    EXPECT_EQ(kValue2, cached_area_->GetItem(kKey));
    observer->KeyChanged(KeyToUint8Vector(kKey), ValueToUint8Vector(kValue2), std::nullopt, source_);
    EXPECT_EQ(kValue2, cached_area_->GetItem(kKey));
}
...
TEST_F(CachedStorageAreaTest, RevertOnChangeFailedWithNonLocalChanges)
{
    ...
    EXPECT_EQ(kValue, cached_area_->GetItem(kKey));
    // Should be ignored.
    observer->KeyChanged(KeyToUint8Vector(kKey), ValueToUint8Vector(kValue2), std::nullopt, kRemoteSource);
    EXPECT_EQ(kValue, cached_area_->GetItem(kKey));
...
class StringEncoding : public CachedStorageAreaTest, public testing::WithParamInterface<FormatOption> { };

INSTANTIATE_TEST_SUITE_P(CachedStorageAreaTest, StringEncoding,
    ::testing::Values(FormatOption::kLocalStorageDetectFormat, FormatOption::kSessionStorageForceUTF16, FormatOption::kSessionStorageForceUTF8));

TEST_P(StringEncoding, RoundTrip_ASCII)
{
    String key("simplekey");
    EXPECT_EQ(Uint8VectorToString(StringToUint8Vector(key, GetParam()), GetParam()), key);
}
...
TEST_P(StringEncoding, RoundTrip_UTF16)
{
    ...
    EXPECT_EQ(Uint8VectorToString(StringToUint8Vector(key.ToString(), GetParam()), GetParam()), key);
}
...
TEST_P(StringEncoding, RoundTrip_InvalidUTF16)
{
    ...
    if (GetParam() != FormatOption::kSessionStorageForceUTF8) {
        ...
        validKey.Append("bar");
        EXPECT_EQ(Uint8VectorToString(StringToUint8Vector(key.ToString(), GetParam()), GetParam()), validKey.ToString());
    }
}
...
TEST_F(CachedStorageAreaTest, StringEncoding_LocalStorage)
{
    ...
    EXPECT_EQ(StringToUint8Vector(ascii_key, FormatOption::kLocalStorageDetectFormat).size(), ascii_key.length() + 1);
    EXPECT_EQ(StringToUint8Vector(non_ascii_key.ToString(), FormatOption::kLocalStorageDetectFormat).size(), non_ascii_key.length() * 2 + 1);
}
...
TEST_F(CachedStorageAreaTest, StringEncoding_UTF8)
{
    ...
    non_ascii_key.Append(UChar(0xd83d));
    non_ascii_key.Append(UChar(0xde00));
    EXPECT_EQ(StringToUint8Vector(ascii_key, FormatOption::kSessionStorageForceUTF8).size(), ascii_key.length());
    EXPECT_EQ(StringToUint8Vector(non_ascii_key.ToString(), FormatOption::kSessionStorageForceUTF8).size(), 7u);
}
...
TEST_F(CachedStorageAreaTest, StringEncoding_UTF16)
{
    ...
    non_ascii_key.Append(UChar(0xd83d));
    non_ascii_key.Append(UChar(0xde00));
    EXPECT_EQ(StringToUint8Vector(ascii_key, FormatOption::kSessionStorageForceUTF16).size(), ascii_key.length() * 2);
    EXPECT_EQ(StringToUint8Vector(non_ascii_key.ToString(), FormatOption::kSessionStorageForceUTF16).size(), non_ascii_key.length() * 2);
}

TEST_F(CachedStorageAreaTest, RecoveryWhenNoLocalDOMWindowPresent)
{
    frame_test_helpers::WebViewHelper web_view_helper;
    test::ScopedMockedURLLoad scoped_mocked_url_load(CachedStorageAreaTest::kPageUrl, test::CoreTestDataPath("foo.html"));
    auto* local_dom_window = To<LocalDOMWindow>(web_view_helper.InitializeAndLoad(CachedStorageAreaTest::kPageString)->GetPage()->MainFrame()->DomWindow());
    auto* source_area = MakeGarbageCollected<FakeAreaSource>(CachedStorageAreaTest::kPageUrl, local_dom_window);
    ...
    auto* sessionStorage = MakeGarbageCollected<StorageNamespace>(*local_dom_window->GetFrame()->GetPage(), &controller, "foo");

    // When no local DOM window is present this shouldn't fatal, just not bind
    auto cached_area
        = base::MakeRefCounted<CachedStorageArea>(CachedStorageArea::AreaType::kSessionStorage, CachedStorageAreaTest::kRootStorageKey, nullptr, sessionStorage,
            /*is_session_storage_for_prerendering=*/false);
