...
// static
const char DOMWindowStorageController::kSupplementName[] = "DOMWindowStorageController";

// static
DOMWindowStorageController& DOMWindowStorageController::From(LocalDOMWindow& window)
{
    DOMWindowStorageController* controller = Supplement<LocalDOMWindow>::From<DOMWindowStorageController>(window);
    if (!controller) {
...
void DOMWindowStorageController::DidAddEventListener(LocalDOMWindow* window, const AtomicString& event_type)
{
    if (event_type == event_type_names::kStorage) {
        ...
        DOMWindowStorage::From(*window).sessionStorage(IGNORE_EXCEPTION_FOR_TESTING);
    }
}
