...
// objects.
class MODULES_EXPORT CachedStorageArea : public mojom::blink::StorageAreaObserver,
...
public:
    ...
    public:
        ...
        virtual bool EnqueueStorageEvent(const String& key, const String& old_value, const String& new_value, const String& url) = 0;
        virtual blink::WebScopedVirtualTimePauser CreateWebScopedVirtualTimePauser(const char* name, WebScopedVirtualTimePauser::VirtualTaskDuration duration)
            = 0;
    ...
    CachedStorageArea(AreaType type, const BlinkStorageKey& storage_key, LocalDOMWindow* local_dom_window, StorageNamespace* storage_namespace,
    ...
    size_t quota_used() const
    {
        return map_ ? map_->quota_used() : 0;
    }
    size_t memory_used() const
    {
        return map_ ? map_->memory_used() : 0;
    }

    // Only public to allow tests to parametrize on this type.
    enum class FormatOption { kLocalStorageDetectFormat, kSessionStorageForceUTF16, kSessionStorageForceUTF8 };

    mojo::Remote<mojom::blink::StorageArea>& RemoteArea()
    {
        return remote_area_;
    }
    ...
    void ResetConnection(mojo::PendingRemote<mojom::blink::StorageArea> new_area = {});
    ...
    void SetRemoteAreaForTesting(mojo::PendingRemote<mojom::blink::StorageArea> area)
    {
...
private:
    ...
    void BindStorageArea(mojo::PendingRemote<mojom::blink::StorageArea> new_area = {}, LocalDOMWindow* local_dom_window = nullptr);

    // mojom::blink::StorageAreaObserver:
    void KeyChanged(
        const Vector<uint8_t>& key, const Vector<uint8_t>& new_value, const std::optional<Vector<uint8_t>>& old_value, const String& source) override;
    void KeyChangeFailed(const Vector<uint8_t>& key, const String& source) override;
    void KeyDeleted(const Vector<uint8_t>& key, const std::optional<Vector<uint8_t>>& old_value, const String& source) override;
    ...
    // |receiver_|.
    void EnqueuePendingMutation(const String& key, const String& new_value, const String& old_value, const String& source);
    ...
    void MaybeApplyNonLocalMutationForKey(const String& key, const String& new_value);
    ...
    void EnqueueStorageEvent(const String& key, const String& old_value, const String& new_value, const String& url, const String& storage_area_id);

    void EnqueueCheckpointMicrotask(Source* source);
    void NotifyCheckpoint();

    static String Uint8VectorToString(const Vector<uint8_t>& input, FormatOption format_option);
    static Vector<uint8_t> StringToUint8Vector(const String& input, FormatOption format_option);
