...
{
    DOMWindowStorage* supplement = Supplement<LocalDOMWindow>::From<DOMWindowStorage>(window);
    if (!supplement) {
...
// static
StorageArea* DOMWindowStorage::sessionStorage(LocalDOMWindow& window, ExceptionState& exception_state)
{
    return From(window).sessionStorage(exception_state);
}

// static
StorageArea* DOMWindowStorage::localStorage(LocalDOMWindow& window, ExceptionState& exception_state)
{
    return From(window).localStorage(exception_state);
}

StorageArea* DOMWindowStorage::sessionStorage(ExceptionState& exception_state) const
{
...
StorageArea* DOMWindowStorage::localStorage(ExceptionState& exception_state) const
{
...
void DOMWindowStorage::InitSessionStorage(mojo::PendingRemote<mojom::blink::StorageArea> storage_area) const
{
    // It's safe to ignore exceptions here since this is just an optimization to
    // avoid requesting the storage area later.
    GetOrCreateSessionStorage(IGNORE_EXCEPTION_FOR_TESTING, std::move(storage_area));
}

void DOMWindowStorage::InitLocalStorage(mojo::PendingRemote<mojom::blink::StorageArea> storage_area) const
{
    // It's safe to ignore exceptions here since this is just an optimization to
    // avoid requesting the storage area later.
    GetOrCreateLocalStorage(IGNORE_EXCEPTION_FOR_TESTING, std::move(storage_area));
}

StorageArea* DOMWindowStorage::GetOrCreateSessionStorage(
    ExceptionState& exception_state, mojo::PendingRemote<mojom::blink::StorageArea> storage_area_for_init) const
{
    LocalDOMWindow* window = GetSupplementable();
    if (!window->GetFrame())
        return nullptr;
    ...
    if (window->document()->IsPrerendering()) {
        cached_storage_area = storage_namespace->CreateCachedAreaForPrerender(window, std::move(storage_area_for_init));
    } else {
        cached_storage_area = storage_namespace->GetCachedArea(window, std::move(storage_area_for_init));
    }
    session_storage_ = StorageArea::Create(window, std::move(cached_storage_area), StorageArea::StorageType::kSessionStorage);
    ...
    ExceptionState& exception_state, mojo::PendingRemote<mojom::blink::StorageArea> storage_area_for_init) const
{
    ...
    auto storage_area = StorageController::GetInstance()->GetLocalStorageArea(window, std::move(storage_area_for_init));
    local_storage_ = StorageArea::Create(window, std::move(storage_area), StorageArea::StorageType::kLocalStorage);
    return local_storage_.Get();
}
