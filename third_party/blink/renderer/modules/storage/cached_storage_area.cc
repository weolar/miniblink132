...
void UnpackSource(const String& source, KURL* page_url, String* storage_area_id)
{
...
base::OnceCallback<void(bool)> MakeSuccessCallback(CachedStorageArea::Source* source)
{
    WebScopedVirtualTimePauser virtual_time_pauser
        = source->CreateWebScopedVirtualTimePauser("CachedStorageArea", WebScopedVirtualTimePauser::VirtualTaskDuration::kNonInstant);
    virtual_time_pauser.PauseVirtualTime();
    return WTF::BindOnce([](WebScopedVirtualTimePauser, bool) {}, std::move(virtual_time_pauser));
}
...
bool CachedStorageArea::SetItem(const String& key, const String& value, Source* source)
{
    DCHECK(areas_->Contains(source));

    // A quick check to reject obviously overbudget items to avoid priming the
    // cache.
    if ((key.length() + value.length()) * 2 > mojom::blink::StorageArea::kPerStorageAreaQuota) {
        return false;
    }
    ...
    if (!is_session_storage_for_prerendering_) {
        remote_area_->Put(
            StringToUint8Vector(key, GetKeyFormat()), StringToUint8Vector(value, value_format), optional_old_value, source_string, MakeSuccessCallback(source));
...
void CachedStorageArea::EnqueueCheckpointMicrotask(Source* source)
{
    ...
    window->GetAgent()->event_loop()->EnqueueMicrotask(WTF::BindOnce(&CachedStorageArea::NotifyCheckpoint, weak_factory_.GetWeakPtr()));
}
...
void CachedStorageArea::RemoveItem(const String& key, Source* source)
{
    ...
    String source_string = PackSource(page_url, source_id);
    if (!is_session_storage_for_prerendering_) {
        remote_area_->Delete(StringToUint8Vector(key, GetKeyFormat()), optional_old_value, source_string, MakeSuccessCallback(source));
        EnqueueCheckpointMicrotask(source);
    }
    if (!IsSessionStorage())
...
CachedStorageArea::CachedStorageArea(AreaType type, const BlinkStorageKey& storage_key, LocalDOMWindow* local_dom_window, StorageNamespace* storage_namespace,
    bool is_session_storage_for_prerendering, mojo::PendingRemote<mojom::blink::StorageArea> storage_area)
    : type_(type)
...
//         this, "DOMStorage", Thread::MainThread()->GetTaskRunner(MainThreadTaskRunnerRestricted()));
}
...
void CachedStorageArea::BindStorageArea(mojo::PendingRemote<mojom::blink::StorageArea> new_area, LocalDOMWindow* local_dom_window)
{
    ...
    auto task_runner = local_dom_window->GetFrame()->GetFrameScheduler()->GetAgentGroupScheduler()->DefaultTaskRunner();
    if (new_area) {
        remote_area_.Bind(std::move(new_area), task_runner);
    } else if (storage_namespace_) {
        storage_namespace_->BindStorageArea(storage_key_, local_dom_window->GetLocalFrameToken(), remote_area_.BindNewPipeAndPassReceiver(task_runner));
    } else {
        return;
    }
...
void CachedStorageArea::ResetConnection(mojo::PendingRemote<mojom::blink::StorageArea> new_area)
{
    ...
    if (!IsSessionStorage()) {
        ...
        for (const auto& delta : deltas) {
            EnqueueStorageEvent(delta.key, delta.value.previously_cached_value, delta.value.restored_value, "", "");
        }
        return;
    }
    ...
    for (const auto& delta : deltas) {
        if (delta.value.previously_cached_value.IsNull()) {
            remote_area_->Delete(StringToUint8Vector(delta.key, GetKeyFormat()), StringToUint8Vector(delta.value.restored_value, GetValueFormat()),
                /*source=*/"\n", base::DoNothing());
        } else {
            const FormatOption value_format = GetValueFormat();
            remote_area_->Put(StringToUint8Vector(delta.key, GetKeyFormat()), StringToUint8Vector(delta.value.previously_cached_value, value_format),
...
void CachedStorageArea::KeyChanged(
    const Vector<uint8_t>& key, const Vector<uint8_t>& new_value, const std::optional<Vector<uint8_t>>& old_value, const String& source)
{
    DCHECK(!IsSessionStorage());

    String key_string = Uint8VectorToString(key, FormatOption::kLocalStorageDetectFormat);
    String new_value_string = Uint8VectorToString(new_value, FormatOption::kLocalStorageDetectFormat);
    String old_value_string;
    if (old_value) {
        old_value_string = Uint8VectorToString(*old_value, FormatOption::kLocalStorageDetectFormat);
    }
    ...
    EnqueueStorageEvent(key_string, old_value_string, new_value_string, page_url, storage_area_id);
}

void CachedStorageArea::KeyChangeFailed(const Vector<uint8_t>& key, const String& source)
{
    DCHECK(!IsSessionStorage());
    ...
    if (key_queue_iter == pending_mutations_by_key_.end()) {
        ...
        String storage_area_id;
        UnpackSource(source, &page_url, &storage_area_id);
        EnqueueStorageEvent(key_string, invalid_cached_value, old_value, page_url, storage_area_id);
        return;
    }
...
void CachedStorageArea::KeyDeleted(const Vector<uint8_t>& key, const std::optional<Vector<uint8_t>>& old_value, const String& source)
{
    DCHECK(!IsSessionStorage());
    ...
    if (old_value) {
        ...
        String storage_area_id;
        UnpackSource(source, &page_url, &storage_area_id);
        EnqueueStorageEvent(key_string, Uint8VectorToString(*old_value, FormatOption::kLocalStorageDetectFormat), String(), page_url, storage_area_id);
    }
}
    ...
    if (!local_mutation) {
        ...
        for (const auto& key_mutations : pending_mutations_by_key_) {
            ...
            if (!most_recent_mutation->new_value.IsNull()) {
                map_->SetItemIgnoringQuota(key_mutations.key, most_recent_mutation->new_value);
            }
...
bool CachedStorageArea::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    using base::trace_event::MemoryAllocatorDump;

    WTF::String dump_name = WTF::String::Format(
        "site_storage/%s/0x%" PRIXPTR "/cache_size", IsSessionStorage() ? "session_storage" : "local_storage", reinterpret_cast<uintptr_t>(this));
    MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name.Utf8());
    dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, memory_used());
    pmd->AddSuballocation(dump->guid(), WTF::Partitions::kAllocatedObjectPoolName);
    return true;
}

void CachedStorageArea::EnqueuePendingMutation(const String& key, const String& new_value, const String& old_value, const String& source)
{
    ...
    if (!key.IsNull()) {
        pending_mutations_by_key_.insert(key, Deque<PendingMutation*>()).stored_value->value.push_back(mutation.get());
    }
    pending_mutations_by_source_.insert(source, OwnedPendingMutationQueue()).stored_value->value.push_back(std::move(mutation));
}

std::unique_ptr<CachedStorageArea::PendingMutation> CachedStorageArea::PopPendingMutation(const String& source)
{
    ...
    std::unique_ptr<PendingMutation> mutation = std::move(mutations_for_source.front());
    ...
    if (!key.IsNull()) {
        ...
        CHECK(key_queue_iter != pending_mutations_by_key_.end(), base::NotFatalUntil::M130);
...
void CachedStorageArea::MaybeApplyNonLocalMutationForKey(const String& key, const String& new_value)
{
    DCHECK(map_);
    auto key_queue_iter = pending_mutations_by_key_.find(key);
    if (key_queue_iter == pending_mutations_by_key_.end()) {
...
void CachedStorageArea::EnsureLoaded()
{
    ...
    UMA_HISTOGRAM_CUSTOM_COUNTS("LocalStorage.MojoSizeInKB", base::saturated_cast<base::Histogram::Sample>(local_storage_size_kb), 1, 6 * 1024, 50);
    if (local_storage_size_kb < 100) {
        UMA_HISTOGRAM_TIMES("LocalStorage.MojoTimeToPrimeForUnder100KB", time_to_prime);
    } else if (local_storage_size_kb < 1000) {
        UMA_HISTOGRAM_TIMES("LocalStorage.MojoTimeToPrimeFor100KBTo1MB", time_to_prime);
    } else {
        UMA_HISTOGRAM_TIMES("LocalStorage.MojoTimeToPrimeFor1MBTo5MB", time_to_prime);
    }
}

CachedStorageArea::FormatOption CachedStorageArea::GetKeyFormat() const
{
    return IsSessionStorage() ? FormatOption::kSessionStorageForceUTF8 : FormatOption::kLocalStorageDetectFormat;
}

CachedStorageArea::FormatOption CachedStorageArea::GetValueFormat() const
{
    return IsSessionStorage() ? FormatOption::kSessionStorageForceUTF16 : FormatOption::kLocalStorageDetectFormat;
}
...
void CachedStorageArea::EnqueueStorageEvent(
    const String& key, const String& old_value, const String& new_value, const String& url, const String& storage_area_id)
{
...
// static
String CachedStorageArea::Uint8VectorToString(const Vector<uint8_t>& input, FormatOption format_option)
{
    if (input.empty())
    ...
    case FormatOption::kSessionStorageForceUTF8: {
        ...
        result = String::FromUTF8(base::span(input.data(), input.size()));
        if (result.IsNull()) {
            corrupt = true;
            break;
        }
...
// static
Vector<uint8_t> CachedStorageArea::StringToUint8Vector(const String& input, FormatOption format_option)
{
    switch (format_option) {
    case FormatOption::kSessionStorageForceUTF16: {
    ...
    case FormatOption::kSessionStorageForceUTF8: {
        ...
        if (input.Is8Bit()) {
            ...
            WTF::unicode::ConversionResult result = WTF::unicode::ConvertLatin1ToUTF8(
                &characters, characters + length, reinterpret_cast<char**>(&buffer), reinterpret_cast<char*>(buffer + buffer_vector.size()));
            // (length * 3) should be sufficient for any conversion
            DCHECK_NE(result, WTF::unicode::kTargetExhausted);
            buffer_vector.Shrink(static_cast<wtf_size_t>(buffer - buffer_vector.data()));
            return buffer_vector;
        }

        // TODO(dmurph): Figure out how to avoid a copy here.
        // TODO(dmurph): Handle invalid UTF16 better. https://crbug.com/873280.
        StringUTF8Adaptor utf8(input, WTF::kStrictUTF8ConversionReplacingUnpairedSurrogatesWithFFFD);
    ...
    case FormatOption::kLocalStorageDetectFormat: {
        ...
        std::memcpy(result.data() + 1, input.Characters16(), input.length() * sizeof(UChar));
        return result;
    }
    }
    NOTREACHED();
