...
{
    Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(pending_remote_.InitWithNewPipeAndPassReceiver());
}
...
mojom::blink::WebDatabaseHost& WebDatabaseHost::GetWebDatabaseHost()
{
    if (!shared_remote_) {
        ...
        shared_remote_ = mojo::SharedRemote<mojom::blink::WebDatabaseHost>(
            std::move(pending_remote_), base::ThreadPool::CreateSequencedTaskRunner({ base::WithBaseSyncPrimitives() }));
    }

    return *shared_remote_;
}

base::File WebDatabaseHost::OpenFile(const String& vfs_file_name, int desired_flags)
{
    base::File file;
...
int64_t WebDatabaseHost::GetSpaceAvailableForOrigin(const SecurityOrigin& origin)
{
    int64_t rv = 0LL;
...
void WebDatabaseHost::DatabaseOpened(const SecurityOrigin& origin, const String& database_name, const String& database_display_name)
{
    DCHECK(IsMainThread());
    GetWebDatabaseHost().Opened(&origin, database_name, database_display_name);
}

void WebDatabaseHost::DatabaseModified(const SecurityOrigin& origin, const String& database_name)
{
    DCHECK(!IsMainThread());
    GetWebDatabaseHost().Modified(&origin, database_name);
}

void WebDatabaseHost::DatabaseClosed(const SecurityOrigin& origin, const String& database_name)
{
    DCHECK(!IsMainThread());
    GetWebDatabaseHost().Closed(&origin, database_name);
}

void WebDatabaseHost::ReportSqliteError(const SecurityOrigin& origin, const String& database_name, int error)
{
    DCHECK(!IsMainThread());
