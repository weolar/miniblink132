...
TEST(QuotaTrackerTest, UpdateAndGetSizeAndSpaceAvailable)
{
    ...
    scoped_refptr<const SecurityOrigin> origin = SecurityOrigin::CreateFromString("file:///a/b/c");
    ...
    tracker.GetDatabaseSizeAndSpaceAvailableToOrigin(origin.get(), database_name, &used, &available);
...
TEST(QuotaTrackerTest, LocalAccessBlocked)
{
    test::TaskEnvironment task_environment;
    QuotaTracker& tracker = QuotaTracker::Instance();
    scoped_refptr<SecurityOrigin> origin = SecurityOrigin::CreateFromString("file:///a/b/c");
