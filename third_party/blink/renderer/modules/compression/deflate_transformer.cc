...
DeflateTransformer::DeflateTransformer(ScriptState* script_state, CompressionFormat format, int level)
    ...
    , out_buffer_(kBufferSize)
{
    ...
    case CompressionFormat::kDeflate:
        err = deflateInit2(&stream_, level, Z_DEFLATED, kWindowBits, 8, Z_DEFAULT_STRATEGY);
        break;
    case CompressionFormat::kGzip:
        err = deflateInit2(&stream_, level, Z_DEFLATED, kWindowBits + kUseGzip, 8, Z_DEFAULT_STRATEGY);
        break;
...
ScriptPromise<IDLUndefined> DeflateTransformer::Transform(
    v8::Local<v8::Value> chunk, TransformStreamDefaultController* controller, ExceptionState& exception_state)
{
    ...
    Deflate(array_piece.Bytes(), static_cast<wtf_size_t>(array_piece.ByteLength()), IsFinished(false), controller, exception_state);
    return ToResolvedUndefinedPromise(script_state_.Get());
}

ScriptPromise<IDLUndefined> DeflateTransformer::Flush(TransformStreamDefaultController* controller, ExceptionState& exception_state)
{
...
void DeflateTransformer::Deflate(
    const uint8_t* start, wtf_size_t length, IsFinished finished, TransformStreamDefaultController* controller, ExceptionState& exception_state)
{
    ...
    do {
        ...
        DCHECK((finished && err == Z_STREAM_END) || err == Z_OK || err == Z_BUF_ERROR);

        wtf_size_t bytes = out_buffer_.size() - stream_.avail_out;
        if (bytes) {
            buffers.push_back(DOMUint8Array::Create(base::span(out_buffer_).first(bytes)));
        }
    ...
    for (DOMUint8Array* buffer : buffers) {
        controller->enqueue(script_state_, ScriptValue::From(script_state_, buffer), exception_state);
        if (exception_state.HadException()) {
            return;
        }
    }
