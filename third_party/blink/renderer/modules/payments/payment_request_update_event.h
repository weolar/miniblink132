...
class MODULES_EXPORT PaymentRequestUpdateEvent : public Event, public GarbageCollectedMixin {
    DEFINE_WRAPPERTYPEINFO();

public:
    PaymentRequestUpdateEvent(ExecutionContext*, const AtomicString& type, const PaymentRequestUpdateEventInit*);
    ~PaymentRequestUpdateEvent() override;

    static PaymentRequestUpdateEvent* Create(
        ExecutionContext*, const AtomicString& type, const PaymentRequestUpdateEventInit* = PaymentRequestUpdateEventInit::Create());

    void SetPaymentRequest(PaymentRequestDelegate* request);

    void updateWith(ScriptState*, ScriptPromise<PaymentDetailsUpdate>, ExceptionState&);

    void start_waiting_for_update(bool value)
    {
        wait_for_update_ = value;
    }
    bool is_waiting_for_update() const
    {
