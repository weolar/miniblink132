...
class MockPaymentRequest : public GarbageCollected<MockPaymentRequest>, public PaymentRequestDelegate {
public:
...
TEST(PaymentRequestUpdateEventTest, OnUpdatePaymentDetailsCalled)
{
    ...
    PaymentRequestUpdateEvent* event = PaymentRequestUpdateEvent::Create(scope.GetExecutionContext(), event_type_names::kShippingaddresschange);
    ...
    auto* payment_details = MakeGarbageCollected<ScriptPromiseResolver<PaymentDetailsUpdate>>(scope.GetScriptState());
    event->updateWith(scope.GetScriptState(), payment_details->Promise(), scope.GetExceptionState());
...
TEST(PaymentRequestUpdateEventTest, CannotUpdateWithoutDispatching)
{
    ...
    event->updateWith(scope.GetScriptState(), MakeGarbageCollected<ScriptPromiseResolver<PaymentDetailsUpdate>>(scope.GetScriptState())->Promise(),
...
TEST(PaymentRequestUpdateEventTest, AddressChangeUpdateWithTimeout)
{
    ...
    ScriptPromiseTester promise_tester(scope.GetScriptState(), request->show(scope.GetScriptState(), scope.GetExceptionState()));
    ...
    EXPECT_EQ("AbortError: Timed out waiting for a "
    ...
    EXPECT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ("PaymentRequest is no longer interactive", scope.GetExceptionState().Message());
}
...
TEST(PaymentRequestUpdateEventTest, OptionChangeUpdateWithTimeout)
{
    ...
    PaymentRequest* request
        = PaymentRequest::Create(scope.GetExecutionContext(), BuildPaymentMethodDataForTest(), BuildPaymentDetailsInitForTest(), scope.GetExceptionState());
    PaymentRequestUpdateEvent* event = PaymentRequestUpdateEvent::Create(scope.GetExecutionContext(), event_type_names::kShippingoptionchange);
    event->SetTrusted(true);
    event->SetPaymentRequest(request);
    EXPECT_FALSE(scope.GetExceptionState().HadException());
