...
TEST(AbortTest, FailedAbortShouldRejectAbortPromise)
{
    ...
    ScriptPromiseTester promise_tester(scope.GetScriptState(), request->abort(scope.GetScriptState(), ASSERT_NO_EXCEPTION));

    static_cast<payments::mojom::blink::PaymentRequestClient*>(request)->OnAbort(false);
...
TEST(AbortTest, SuccessfulAbortShouldRejectShowPromiseAndResolveAbortPromise)
{
    ...
    LocalFrame::NotifyUserActivation(&scope.GetFrame(), mojom::UserActivationNotificationType::kTest);
    ScriptPromiseTester show_promise_tester(scope.GetScriptState(), request->show(scope.GetScriptState(), ASSERT_NO_EXCEPTION));
    ScriptPromiseTester abort_promise_tester(scope.GetScriptState(), request->abort(scope.GetScriptState(), ASSERT_NO_EXCEPTION));
