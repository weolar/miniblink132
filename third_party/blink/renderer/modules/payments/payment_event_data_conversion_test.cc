...
static payments::mojom::blink::PaymentCurrencyAmountPtr CreatePaymentCurrencyAmountForTest()
{
...
static payments::mojom::blink::PaymentMethodDataPtr CreatePaymentMethodDataForTest()
{
    auto method_data = payments::mojom::blink::PaymentMethodData::New();
    method_data->supported_method = String::FromUTF8("foo");
    method_data->stringified_data = String::FromUTF8("{\"merchantId\":\"12345\"}");
    return method_data;
}

static payments::mojom::blink::CanMakePaymentEventDataPtr CreateCanMakePaymentEventDataForTest()
{
...
static payments::mojom::blink::PaymentOptionsPtr CreatePaymentOptionsForTest()
{
    ...
    payment_options->shipping_type = payments::mojom::PaymentShippingType::DELIVERY;
    return payment_options;
}

static payments::mojom::blink::PaymentShippingOptionPtr CreateShippingOptionForTest()
{
...
static payments::mojom::blink::PaymentRequestEventDataPtr CreatePaymentRequestEventDataForTest()
{
...
TEST(PaymentEventDataConversionTest, ToCanMakePaymentEventData)
{
    ...
    payments::mojom::blink::CanMakePaymentEventDataPtr event_data = CreateCanMakePaymentEventDataForTest();
    CanMakePaymentEventInit* data = PaymentEventDataConversion::ToCanMakePaymentEventInit(scope.GetScriptState(), std::move(event_data));
    ...
    String stringified_data = ToBlinkString<String>(scope.GetIsolate(),
        v8::JSON::Stringify(scope.GetContext(), data->methodData().front()->data().V8Value().As<v8::Object>()).ToLocalChecked(), kDoNotExternalize);
...
TEST(PaymentEventDataConversionTest, ToPaymentRequestEventData)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    payments::mojom::blink::PaymentRequestEventDataPtr event_data = CreatePaymentRequestEventDataForTest();
    PaymentRequestEventInit* data = PaymentEventDataConversion::ToPaymentRequestEventInit(scope.GetScriptState(), std::move(event_data));
