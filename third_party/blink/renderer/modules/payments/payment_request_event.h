...
public:
    static PaymentRequestEvent* Create(const AtomicString& type, const PaymentRequestEventInit*,
        mojo::PendingRemote<payments::mojom::blink::PaymentHandlerHost> host = mojo::NullRemote(),
        PaymentRequestRespondWithObserver* respond_with_observer = nullptr, WaitUntilObserver* wait_until_observer = nullptr,
        ExecutionContext* execution_context = nullptr);

    PaymentRequestEvent(const AtomicString& type, const PaymentRequestEventInit*, mojo::PendingRemote<payments::mojom::blink::PaymentHandlerHost> host,
        PaymentRequestRespondWithObserver*, WaitUntilObserver*, ExecutionContext* execution_context);
    ...
    std::optional<HeapVector<Member<PaymentShippingOption>>> shippingOptions() const;

    ScriptPromise<IDLNullable<ServiceWorkerWindowClient>> openWindow(ScriptState*, const String& url);
    ScriptPromise<IDLNullable<PaymentRequestDetailsUpdate>> changePaymentMethod(
        ScriptState*, const String& method_name, const ScriptValue& method_details, ExceptionState& exception_state);
    ScriptPromise<IDLNullable<PaymentRequestDetailsUpdate>> changeShippingAddress(ScriptState*, AddressInit*, ExceptionState&);
    ScriptPromise<IDLNullable<PaymentRequestDetailsUpdate>> changeShippingOption(ScriptState*, const String& shipping_option_id, ExceptionState&);
    void respondWith(ScriptState*, ScriptPromise<PaymentHandlerResponse>, ExceptionState&);

    void Trace(Visitor*) const override;

private:
    void OnChangePaymentRequestDetailsResponse(payments::mojom::blink::PaymentRequestDetailsUpdatePtr);
    ...
    Member<ScriptPromiseResolver<IDLNullable<PaymentRequestDetailsUpdate>>> change_payment_request_details_resolver_;
    Member<PaymentRequestRespondWithObserver> observer_;
    HeapMojoRemote<payments::mojom::blink::PaymentHandlerHost> payment_handler_host_;
};
