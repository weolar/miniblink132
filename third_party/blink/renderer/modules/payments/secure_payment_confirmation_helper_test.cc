...
WTF::Vector<uint8_t> CreateVector(const uint8_t* buffer, const unsigned length)
{
...
static V8UnionArrayBufferOrArrayBufferView* ArrayBufferOrView(const uint8_t* data, size_t size)
{
    DOMArrayBuffer* dom_array = DOMArrayBuffer::Create(UNSAFE_TODO(base::span(data, size)));
    return MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(dom_array);
}

static AuthenticationExtensionsPRFInputs* CreatePrfInputs(v8::Isolate* isolate)
{
    AuthenticationExtensionsPRFValues* prf_values = AuthenticationExtensionsPRFValues::Create(isolate);
    prf_values->setFirst(ArrayBufferOrView(kPrfInputData, sizeof(kPrfInputData)));
    AuthenticationExtensionsPRFInputs* prf_inputs = AuthenticationExtensionsPRFInputs::Create(isolate);
...
TEST(SecurePaymentConfirmationHelperTest, Parse_Success)
{
    ...
    SecurePaymentConfirmationRequest* request = CreateSecurePaymentConfirmationRequest(scope);

    ScriptValue script_value(scope.GetIsolate(), ToV8Traits<SecurePaymentConfirmationRequest>::ToV8(scope.GetScriptState(), request));
    ::payments::mojom::blink::SecurePaymentConfirmationRequestPtr parsed_request
        = SecurePaymentConfirmationHelper::ParseSecurePaymentConfirmationData(script_value, *scope.GetExecutionContext(), ASSERT_NO_EXCEPTION);
    ASSERT_TRUE(parsed_request);

    ASSERT_EQ(parsed_request->credential_ids.size(), 1u);
    EXPECT_EQ(parsed_request->credential_ids[0], CreateVector(kSecurePaymentConfirmationCredentialId, std::size(kSecurePaymentConfirmationCredentialId)));
    EXPECT_EQ(parsed_request->challenge, CreateVector(kSecurePaymentConfirmationChallenge, std::size(kSecurePaymentConfirmationChallenge)));
    EXPECT_EQ(parsed_request->instrument->display_name, "My Card");
    EXPECT_EQ(parsed_request->instrument->icon.GetString(), "https://bank.example/icon.png");
...
TEST(SecurePaymentConfirmationHelperTest, Parse_OptionalFields)
{
    ...
    NetworkOrIssuerInformation* networkInfo = NetworkOrIssuerInformation::Create(scope.GetIsolate());
    networkInfo->setName("Network Name");
    networkInfo->setIcon("https://network.example/icon.png");
    request->setNetworkInfo(networkInfo);

    NetworkOrIssuerInformation* issuerInfo = NetworkOrIssuerInformation::Create(scope.GetIsolate());
    ...
    EXPECT_EQ(parsed_request->payee_origin->ToString(), "https://merchant.example");
    ...
    EXPECT_EQ(parsed_request->network_info->icon.GetString(), "https://network.example/icon.png");
    EXPECT_EQ(parsed_request->issuer_info->name, "Issuer Name");
    EXPECT_EQ(parsed_request->issuer_info->icon.GetString(), "https://bank.example/icon.png");
}
...
TEST(SecurePaymentConfirmationHelperTest, Parse_EmptyIdCredentialIds)
{
    ...
    ScriptValue script_value(scope.GetIsolate(), ToV8Traits<SecurePaymentConfirmationRequest>::ToV8(scope.GetScriptState(), request));
    SecurePaymentConfirmationHelper::ParseSecurePaymentConfirmationData(script_value, *scope.GetExecutionContext(), scope.GetExceptionState());
    EXPECT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ(ESErrorType::kRangeError, scope.GetExceptionState().CodeAs<ESErrorType>());
}
...
TEST(SecurePaymentConfirmationHelperTest, Parse_EmptyId)
{
    ...
    credentialIds.push_back(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(DOMArrayBuffer::Create(kSecurePaymentConfirmationCredentialId)));
    const size_t num_elements = 0;
    const size_t byte_length = 0;
    credentialIds.push_back(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(DOMArrayBuffer::CreateOrNull(num_elements, byte_length)));
...
TEST(SecurePaymentConfirmationHelperTest, Parse_EmptyChallenge)
{
    ...
    const size_t num_elements = 0;
    const size_t byte_length = 0;
    request->setChallenge(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(DOMArrayBuffer::CreateOrNull(num_elements, byte_length)));
...
TEST(SecurePaymentConfirmationHelperTest, Parse_InvalidRpId)
{
    ...
    for (const String& rp_id : invalid_cases) {
        ...
        ScriptValue script_value(scope.GetIsolate(), ToV8Traits<SecurePaymentConfirmationRequest>::ToV8(scope.GetScriptState(), request));
        SecurePaymentConfirmationHelper::ParseSecurePaymentConfirmationData(script_value, *scope.GetExecutionContext(), scope.GetExceptionState());
        EXPECT_TRUE(scope.GetExceptionState().HadException()) << "RpId " << rp_id << " did not throw";
        EXPECT_EQ(ESErrorType::kTypeError, scope.GetExceptionState().CodeAs<ESErrorType>());
    }
}
...
TEST(SecurePaymentConfirmationHelperTest, Parse_MissingPayeeNameAndPayeeOrigin)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    SecurePaymentConfirmationRequest* request = CreateSecurePaymentConfirmationRequest(scope,
...
TEST(SecurePaymentConfirmationHelperTest, Parse_Extensions)
{
    ...
    V8TestingScope scope;
    SecurePaymentConfirmationRequest* request = CreateSecurePaymentConfirmationRequest(scope);
    AuthenticationExtensionsClientInputs* extensions = AuthenticationExtensionsClientInputs::Create(scope.GetIsolate());
    ...
    WTF::Vector<uint8_t> prf_expected = CreateVector(kPrfInputData, sizeof(kPrfInputData));
