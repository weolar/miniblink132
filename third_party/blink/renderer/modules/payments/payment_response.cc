...
v8::Local<v8::Value> BuildDetails(
    ScriptState* script_state, const String& json, mojom::blink::GetAssertionAuthenticatorResponsePtr get_assertion_authentication_response)
{
    if (get_assertion_authentication_response) {
        ...
        auto* authenticator_response
            = MakeGarbageCollected<AuthenticatorAssertionResponse>(std::move(info->client_data_json), std::move(info->authenticator_data),
                std::move(get_assertion_authentication_response->signature), get_assertion_authentication_response->user_handle);

        auto* result = MakeGarbageCollected<PublicKeyCredential>(get_assertion_authentication_response->info->id, DOMArrayBuffer::Create(info->raw_id),
            authenticator_response, get_assertion_authentication_response->authenticator_attachment,
            ConvertTo<AuthenticationExtensionsClientOutputs*>(get_assertion_authentication_response->extensions));
...
PaymentResponse::PaymentResponse(ScriptState* script_state, payments::mojom::blink::PaymentResponsePtr response, PaymentAddress* shipping_address,
    PaymentStateResolver* payment_state_resolver, const String& request_id)
    ...
    , payment_state_resolver_(payment_state_resolver)
{
    ...
    details_.Set(
        script_state->GetIsolate(), BuildDetails(script_state, response->stringified_details, std::move(response->get_assertion_authenticator_response)));
}

PaymentResponse::~PaymentResponse() = default;

void PaymentResponse::Update(ScriptState* script_state, payments::mojom::blink::PaymentResponsePtr response, PaymentAddress* shipping_address)
{
    DCHECK(response);
...
void PaymentResponse::UpdatePayerDetail(payments::mojom::blink::PayerDetailPtr detail)
{
    DCHECK(detail);
...
ScriptValue PaymentResponse::toJSONForBinding(ScriptState* script_state) const
{
    ...
    if (shippingAddress()) {
        result.AddV8Value("shippingAddress", shippingAddress()->toJSONForBinding(script_state).V8Value());
    } else {
...
{
    return ScriptValue(script_state->GetIsolate(), details_.GetAcrossWorld(script_state));
}

ScriptPromise<IDLUndefined> PaymentResponse::complete(ScriptState* script_state, const V8PaymentComplete& result, ExceptionState& exception_state)
{
    VLOG(2) << "Renderer: PaymentRequest (" << requestId().Utf8() << "): complete(" << result.AsString() << ")";
    PaymentStateResolver::PaymentComplete converted_result = PaymentStateResolver::PaymentComplete::kUnknown;
    ...
    return payment_state_resolver_->Complete(script_state, converted_result, exception_state);
}

ScriptPromise<IDLUndefined> PaymentResponse::retry(ScriptState* script_state, const PaymentValidationErrors* error_fields, ExceptionState& exception_state)
{
    VLOG(2) << "Renderer: PaymentRequest (" << requestId().Utf8() << "): retry()";
    return payment_state_resolver_->Retry(script_state, error_fields, exception_state);
}
