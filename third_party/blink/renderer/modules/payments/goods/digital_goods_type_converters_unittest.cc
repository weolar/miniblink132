...
TEST(DigitalGoodsTypeConvertersTest, MojoBillingResponseToIdl)
{
    ...
    EXPECT_EQ(mojo::ConvertTo<String>(BillingResponseCode::kItemAlreadyOwned), "itemAlreadyOwned");
    EXPECT_EQ(mojo::ConvertTo<String>(BillingResponseCode::kItemNotOwned), "itemNotOwned");
    EXPECT_EQ(mojo::ConvertTo<String>(BillingResponseCode::kItemUnavailable), "itemUnavailable");
    EXPECT_EQ(mojo::ConvertTo<String>(BillingResponseCode::kClientAppUnavailable), "clientAppUnavailable");
    EXPECT_EQ(mojo::ConvertTo<String>(BillingResponseCode::kClientAppError), "clientAppError");
}
...
TEST(DigitalGoodsTypeConvertersTest, MojoItemDetailsToIdl_WithOptionalFields)
{
    ...
    auto price = payments::mojom::blink::PaymentCurrencyAmount::New(price_currency, price_value);
    ...
    auto introductory_price = payments::mojom::blink::PaymentCurrencyAmount::New(introductory_price_currency, introductory_price_value);
    ...
    EXPECT_EQ(idl_item_details->introductoryPrice()->currency(), introductory_price_currency);
    EXPECT_EQ(idl_item_details->introductoryPrice()->value(), introductory_price_value);
    EXPECT_EQ(idl_item_details->introductoryPricePeriod(), introductory_price_period);
    EXPECT_EQ(idl_item_details->introductoryPriceCycles(), introductory_price_cycles);
...
TEST(DigitalGoodsTypeConvertersTest, MojoItemDetailsToIdl_WithoutOptionalFields)
{
    auto mojo_item_details = payments::mojom::blink::ItemDetails::New();
    const String item_id = "shiny-sword-id";
    const String title = "Shiny Sword";
...
{
    test::TaskEnvironment task_environment;
    auto mojo_purchase_reference = payments::mojom::blink::PurchaseReference::New();
    const String item_id = "shiny-sword-id";
