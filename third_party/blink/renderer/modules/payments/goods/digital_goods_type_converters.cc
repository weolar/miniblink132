...
WTF::String TypeConverter<WTF::String, CreateDigitalGoodsResponseCode>::Convert(const CreateDigitalGoodsResponseCode& input)
{
    switch (input) {
...
blink::ItemDetails* TypeConverter<blink::ItemDetails*, ItemDetailsPtr>::Convert(const ItemDetailsPtr& input)
{
    if (!input)
        return nullptr;
    ...
    output->setPrice(blink::PaymentEventDataConversion::ToPaymentCurrencyAmount(input->price));
    ...
    if (input->introductory_price) {
        output->setIntroductoryPrice(blink::PaymentEventDataConversion::ToPaymentCurrencyAmount(input->introductory_price));
    }
    if (input->introductory_price_period && !input->introductory_price_period.empty()) {
...
WTF::String TypeConverter<WTF::String, BillingResponseCode>::Convert(const BillingResponseCode& input)
{
    switch (input) {
...
blink::PurchaseDetails* TypeConverter<blink::PurchaseDetails*, PurchaseReferencePtr>::Convert(const PurchaseReferencePtr& input)
{
    if (!input)
        return nullptr;
