...
{
    if (!instruments_) {
        instruments_ = MakeGarbageCollected<PaymentInstruments>(*this, registration_->GetExecutionContext());
    }
...
ScriptPromise<IDLBoolean> PaymentManager::enableDelegations(
    ScriptState* script_state, const Vector<V8PaymentDelegation>& delegations, ExceptionState& exception_state)
{
    if (!script_state->ContextIsValid()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot enable payment delegations");
        return EmptyPromise();
    }
    ...
    manager_->EnableDelegations(std::move(mojo_delegations), WTF::BindOnce(&PaymentManager::OnEnableDelegationsResponse, WrapPersistent(this)));
    enable_delegations_resolver_ = MakeGarbageCollected<ScriptPromiseResolver<IDLBoolean>>(script_state, exception_state.GetContext());
    ...
    , instruments_(nullptr)
{
    ...
    if (ExecutionContext* context = registration->GetExecutionContext()) {
        context->GetBrowserInterfaceBroker().GetInterface(manager_.BindNewPipeAndPassReceiver(context->GetTaskRunner(TaskType::kUserInteraction)));
    }

    manager_.set_disconnect_handler(WTF::BindOnce(&PaymentManager::OnServiceConnectionError, WrapWeakPersistent(this)));
    manager_->Init(registration_->GetExecutionContext()->Url(), registration_->scope());
}

void PaymentManager::OnEnableDelegationsResponse(payments::mojom::blink::PaymentHandlerStatus status)
{
    DCHECK(enable_delegations_resolver_);
    enable_delegations_resolver_->Resolve(status == payments::mojom::blink::PaymentHandlerStatus::SUCCESS);
