...
PaymentItem* ToPaymentItem(payments::mojom::blink::PaymentItemPtr data)
{
    ...
    item->setAmount(PaymentEventDataConversion::ToPaymentCurrencyAmount(data->amount));
    item->setPending(data->pending);
    return item;
}

PaymentDetailsModifier* ToPaymentDetailsModifier(ScriptState* script_state, payments::mojom::blink::PaymentDetailsModifierPtr data)
{
    DCHECK(data);
...
ScriptValue StringDataToScriptValue(ScriptState* script_state, const String& stringified_data)
{
    if (!script_state->ContextIsValid())
        return ScriptValue();

    ScriptState::Scope scope(script_state);
    return ScriptValue(script_state->GetIsolate(), FromJSONString(script_state, stringified_data));
}

PaymentMethodData* ToPaymentMethodData(ScriptState* script_state, payments::mojom::blink::PaymentMethodDataPtr data)
{
    DCHECK(data);
    PaymentMethodData* method_data = PaymentMethodData::Create();
    method_data->setSupportedMethod(data->supported_method);
    ScriptValue v8_data = StringDataToScriptValue(script_state, data->stringified_data);
    if (!v8_data.IsEmpty())
        method_data->setData(std::move(v8_data));
    return method_data;
}

PaymentOptions* ToPaymentOptions(payments::mojom::blink::PaymentOptionsPtr options)
{
    DCHECK(options);
...
PaymentShippingOption* ToShippingOption(payments::mojom::blink::PaymentShippingOptionPtr option)
{
    DCHECK(option);
    PaymentShippingOption* shipping_option = PaymentShippingOption::Create();

    shipping_option->setAmount(PaymentEventDataConversion::ToPaymentCurrencyAmount(option->amount));
...
PaymentCurrencyAmount* PaymentEventDataConversion::ToPaymentCurrencyAmount(payments::mojom::blink::PaymentCurrencyAmountPtr& input)
{
...
PaymentRequestEventInit* PaymentEventDataConversion::ToPaymentRequestEventInit(
    ScriptState* script_state, payments::mojom::blink::PaymentRequestEventDataPtr event_data)
{
    DCHECK(script_state);
    DCHECK(event_data);
    ...
    event_init->setPaymentRequestOrigin(event_data->payment_request_origin.GetString());
    ...
    for (auto& modifier : event_data->modifiers) {
        modifiers.push_back(ToPaymentDetailsModifier(script_state, std::move(modifier)));
    }
    ...
    if (event_data->payment_options) {
        ...
        event_init->setPaymentOptions(ToPaymentOptions(std::move(event_data->payment_options)));
    }
...
CanMakePaymentEventInit* PaymentEventDataConversion::ToCanMakePaymentEventInit(
    ScriptState* script_state, payments::mojom::blink::CanMakePaymentEventDataPtr event_data)
{
    DCHECK(script_state);
    DCHECK(event_data);
