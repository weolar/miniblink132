...
TEST(OnPaymentResponseTest, RejectMissingShippingOption)
{
    ...
    static_cast<payments::mojom::blink::PaymentRequestClient*>(request)->OnPaymentResponse(std::move(response));
...
class PaymentResponseFunction : public ThenCallable<PaymentResponse, PaymentResponseFunction> {
public:
    void React(ScriptState*, PaymentResponse* response)
    {
        response_ = response;
    }
    PaymentResponse* Response() const
    {
        return response_;
    }
...
TEST(OnPaymentResponseTest, CanRequestShippingInformation)
{
    ...
    request->show(scope.GetScriptState(), ASSERT_NO_EXCEPTION).React(scope.GetScriptState(), response_function);

    static_cast<payments::mojom::blink::PaymentRequestClient*>(request)->OnPaymentResponse(std::move(response));

    scope.PerformMicrotaskCheckpoint();
    EXPECT_EQ("standardShipping", response_function->Response()->shippingOption());
}
