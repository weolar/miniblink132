...
void SetValues(
    PaymentItemOrPaymentShippingOption* original, PaymentTestDataToChange data, PaymentTestModificationType modification_type, const String& value_to_use)
{
...
void BuildPaymentDetailsBase(PaymentTestDetailToChange detail, PaymentTestDataToChange data, PaymentTestModificationType modification_type,
    const String& value_to_use, PaymentDetailsBase* details)
{
    ...
    if (detail == kPaymentTestDetailShippingOption) {
        shipping_option = BuildShippingOptionForTest(data, modification_type, value_to_use);
    } else {
    ...
    if (detail == kPaymentTestDetailModifierTotal || detail == kPaymentTestDetailModifierItem) {
        modifier = BuildPaymentDetailsModifierForTest(detail, data, modification_type, value_to_use);
    } else {
    ...
    details->setShippingOptions(HeapVector<Member<PaymentShippingOption>>(1, shipping_option));
    details->setModifiers(HeapVector<Member<PaymentDetailsModifier>>(1, modifier));
}

} // namespace

PaymentItem* BuildPaymentItemForTest(PaymentTestDataToChange data, PaymentTestModificationType modification_type, const String& value_to_use)
{
...
PaymentShippingOption* BuildShippingOptionForTest(PaymentTestDataToChange data, PaymentTestModificationType modification_type, const String& value_to_use)
{
...
PaymentDetailsModifier* BuildPaymentDetailsModifierForTest(
    PaymentTestDetailToChange detail, PaymentTestDataToChange data, PaymentTestModificationType modification_type, const String& value_to_use)
{
    ...
    PaymentTestDetailToChange detail, PaymentTestDataToChange data, PaymentTestModificationType modification_type, const String& value_to_use)
{
    ...
    BuildPaymentDetailsBase(detail, data, modification_type, value_to_use, details);

    if (detail == kPaymentTestDetailTotal) {
        details->setTotal(BuildPaymentItemForTest(data, modification_type, value_to_use));
    } else {
...
PaymentDetailsUpdate* BuildPaymentDetailsErrorMsgForTest(const String& value_to_use)
{
    return BuildPaymentDetailsUpdateForTest(kPaymentTestDetailError, kPaymentTestDataNone, kPaymentTestOverwriteValue, value_to_use);
}
...
SecurePaymentConfirmationRequest* CreateSecurePaymentConfirmationRequest(const V8TestingScope& scope, const bool include_payee_name)
{
    SecurePaymentConfirmationRequest* request = SecurePaymentConfirmationRequest::Create(scope.GetIsolate());
    ...
    request->setChallenge(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(DOMArrayBuffer::Create(kSecurePaymentConfirmationChallenge)));

    PaymentCredentialInstrument* instrument = PaymentCredentialInstrument::Create(scope.GetIsolate());
...
HeapVector<Member<PaymentMethodData>> BuildSecurePaymentConfirmationMethodDataForTest(const V8TestingScope& scope)
{
    SecurePaymentConfirmationRequest* spc_request = CreateSecurePaymentConfirmationRequest(scope);

    HeapVector<Member<PaymentMethodData>> method_data(1, PaymentMethodData::Create());
    method_data[0]->setSupportedMethod("secure-payment-confirmation");
    method_data[0]->setData(ScriptValue(scope.GetIsolate(), spc_request->ToV8(scope.GetScriptState())));
