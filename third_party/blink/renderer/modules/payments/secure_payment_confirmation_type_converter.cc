...
TypeConverter<payments::mojom::blink::SecurePaymentConfirmationRequestPtr, blink::SecurePaymentConfirmationRequest*>::Convert(
    const blink::SecurePaymentConfirmationRequest* input)
{
    auto output = payments::mojom::blink::SecurePaymentConfirmationRequest::New();
    output->credential_ids = mojo::ConvertTo<Vector<Vector<uint8_t>>>(input->credentialIds());
    ...
    output->instrument = blink::mojom::blink::PaymentCredentialInstrument::New(
        input->instrument()->displayName(), blink::KURL(input->instrument()->icon()), input->instrument()->iconMustBeShown());

    if (input->hasPayeeOrigin()) {
        output->payee_origin = blink::SecurityOrigin::CreateFromString(input->payeeOrigin());
    }
    ...
    if (input->hasExtensions()) {
        output->extensions = ConvertTo<blink::mojom::blink::AuthenticationExtensionsClientInputsPtr>(*input->extensions());
    }

    if (input->hasNetworkInfo()) {
        output->network_info = payments::mojom::blink::NetworkOrIssuerInformation::New(input->networkInfo()->name(), blink::KURL(input->networkInfo()->icon()));
    }

    if (input->hasIssuerInfo()) {
        output->issuer_info = payments::mojom::blink::NetworkOrIssuerInformation::New(input->issuerInfo()->name(), blink::KURL(input->issuerInfo()->icon()));
    }
