...
class MODULES_EXPORT PaymentRequest final : public EventTarget,
...
public:
    static PaymentRequest* Create(ExecutionContext*, const HeapVector<Member<PaymentMethodData>>&, const PaymentDetailsInit*, ExceptionState&);
    static PaymentRequest* Create(
        ExecutionContext*, const HeapVector<Member<PaymentMethodData>>&, const PaymentDetailsInit*, const PaymentOptions*, ExceptionState&);

    PaymentRequest(ExecutionContext*, const HeapVector<Member<PaymentMethodData>>&, const PaymentDetailsInit*, const PaymentOptions*,
        mojo::PendingRemote<payments::mojom::blink::PaymentRequest> mock_payment_provider, ExceptionState&);
    ...
    ScriptPromise<PaymentResponse> show(ScriptState*, ScriptPromise<PaymentDetailsUpdate>, ExceptionState&);
    ...
    PaymentAddress* getShippingAddress() const
    {
    ...
    ScriptPromise<IDLBoolean> hasEnrolledInstrument(ScriptState*, ExceptionState&);
    ...
    ScriptPromise<IDLUndefined> Complete(ScriptState*, PaymentComplete result, ExceptionState&) override;
    ScriptPromise<IDLUndefined> Retry(ScriptState*, const PaymentValidationErrors*, ExceptionState&) override;
...
private:
    ...
    void OnPaymentMethodChange(const String& method_name, const String& stringified_details) override;
    void OnShippingAddressChange(payments::mojom::blink::PaymentAddressPtr) override;
    ...
    void OnError(payments::mojom::blink::PaymentErrorReason, const String& error_message) override;
    void OnComplete() override;
    void OnAbort(bool aborted_successfully) override;
    void OnCanMakePayment(payments::mojom::blink::CanMakePaymentQueryResult) override;
    void OnHasEnrolledInstrument(payments::mojom::blink::HasEnrolledInstrumentQueryResult) override;
    void WarnNoFavicon() override;
    void AllowConnectToSource(
        const KURL& url, const KURL& url_before_redirects, bool did_follow_redirect, AllowConnectToSourceCallback response_callback) override;
    ...
    void DispatchPaymentRequestUpdateEvent(EventTarget* event_target, PaymentRequestUpdateEvent* event);
    ...
    Member<ScriptPromiseResolver<PaymentResponse>> accept_resolver_; // the resolver for the show() promise.
    ...
    HeapMojoReceiver<payments::mojom::blink::PaymentRequestClient, PaymentRequest> client_receiver_;
