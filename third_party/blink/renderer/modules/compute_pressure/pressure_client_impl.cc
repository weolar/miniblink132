...
PressureClientImpl::PressureClientImpl(ExecutionContext* context, PressureObserverManager* manager)
...
void PressureClientImpl::OnPressureUpdated(device::mojom::blink::PressureUpdatePtr update)
{
    ...
    for (const auto& observer : observers) {
        observer->OnUpdate(GetExecutionContext(), source, PressureStateToV8PressureState(update->state), CalculateTimestamp(update->timestamp));
    }
}
...
void PressureClientImpl::BindPressureClient(mojo::PendingReceiver<device::mojom::blink::PressureClient> pending_client_receiver)
{
    receiver_.Bind(std::move(pending_client_receiver), GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI));
    receiver_.set_disconnect_handler(WTF::BindOnce(&PressureClientImpl::Reset, WrapWeakPersistent(this)));
}
...
DOMHighResTimeStamp PressureClientImpl::CalculateTimestamp(base::TimeTicks timeticks) const
{
