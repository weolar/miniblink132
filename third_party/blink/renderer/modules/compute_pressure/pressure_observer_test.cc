...
class ThenClosureRunner final : public ThenCallable<IDLUndefined, ThenClosureRunner> {
public:
    ...
    {
        {
            ...
            const auto& updates
                = NativeValueTraits<IDLSequence<PressureRecord>>::NativeValue(script_value.GetIsolate(), script_value.V8Value(), exception_state);
            pressure_records_.AppendVector(updates);
        }
        return ScriptValue();
    }

    const auto& pressure_records() const
    {
        return pressure_records_;
    }
...
void WaitForPromiseFulfillment(ScriptState* script_state, ScriptPromise<IDLUndefined> promise)
{
    base::RunLoop run_loop;
    promise.React(script_state, MakeGarbageCollected<ThenClosureRunner>(run_loop.QuitClosure()));
...
TEST(PressureObserverTest, RateObfuscationMitigation)
{
    ...
    auto* pressure_record_accumulator = MakeGarbageCollected<PressureRecordAccumulator>();
    auto* callback = V8PressureUpdateCallback::Create(pressure_record_accumulator->ToV8Function(scope.GetScriptState()));

    constexpr size_t kNumPressureStates = static_cast<size_t>(PressureState::kMaxValue) + 1U;
    constexpr std::array<device::mojom::blink::PressureState, kNumPressureStates> kPressureStates = {
    ...
    observer->change_rate_monitor_for_testing().set_change_count_threshold_for_testing(kNumPressureStates + 1U);
    observer->change_rate_monitor_for_testing().set_penalty_duration_for_testing(kPenaltyDuration);

    auto* options = PressureObserverOptions::Create();
    options->setSampleInterval(kSamplingInterval.InMilliseconds());
    auto promise = observer->observe(scope.GetScriptState(), V8PressureSource(V8PressureSource::Enum::kCpu), options, scope.GetExceptionState());
    ...
    const auto& pressure_records = pressure_record_accumulator->pressure_records();
    ...
    // sent without delay.
    {
        for (wtf_size_t i = 0; i < kPressureStates.size(); ++i) {
            ...
            pressure_service.SendUpdate(device::mojom::blink::PressureUpdate::New(device::mojom::blink::PressureSource::kCpu, state, base::TimeTicks::Now()));
        ...
        for (wtf_size_t i = 0; i < (pressure_records.size() - 1U); ++i) {
            EXPECT_EQ(pressure_records[i + 1]->time() - pressure_records[i]->time(), kSamplingInterval.InMilliseconds());
        }
    }
    ...
    // mitigation for future updates.
    {
        ...
        pressure_service.SendUpdate(
            device::mojom::blink::PressureUpdate::New(device::mojom::blink::PressureSource::kCpu, PressureState::kNominal, base::TimeTicks::Now()));
        task_environment.FastForwardBy(base::Milliseconds(0));
        ...
        pressure_service.SendUpdate(
            device::mojom::blink::PressureUpdate::New(device::mojom::blink::PressureSource::kCpu, PressureState::kFair, base::TimeTicks::Now()));
        ...
        EXPECT_EQ(pressure_records.back()->state().AsString(), V8PressureState(V8PressureState::Enum::kFair).AsString());
        // This update was sent after fast-forwarding by 100ms once and by 200ms 5
        // times.
        EXPECT_EQ(pressure_records.back()->time(), (kSmallInterval + (5 * kSamplingInterval)).InMilliseconds());
    }
    ...
    // anymore.
    {
        ...
        pressure_service.SendUpdate(
            device::mojom::blink::PressureUpdate::New(device::mojom::blink::PressureSource::kCpu, PressureState::kSerious, base::TimeTicks::Now()));
        task_environment.FastForwardBy(base::Milliseconds(0));
        EXPECT_EQ(pressure_records.size(), original_callback_count + 1U);
        EXPECT_EQ(pressure_records.back()->state().AsString(), V8PressureState(V8PressureState::Enum::kSerious).AsString());
    }
...
TEST(PressureObserverTest, PressureObserverDisconnectBeforePenaltyEnd)
{
    ...
    auto* callback_function = MakeGarbageCollected<ClosureRunnerCallable>(callback_run_loop.QuitClosure());
    auto* callback = V8PressureUpdateCallback::Create(callback_function->ToV8Function(scope.GetScriptState()));

    V8PressureSource source(V8PressureSource::Enum::kCpu);
    auto* options = PressureObserverOptions::Create();
    auto* observer = PressureObserver::Create(callback);
    auto promise = observer->observe(scope.GetScriptState(), source, options, scope.GetExceptionState());

    WaitForPromiseFulfillment(scope.GetScriptState(), promise);

    observer->change_rate_monitor_for_testing().set_change_count_threshold_for_testing(kChangeCount);
    observer->change_rate_monitor_for_testing().set_penalty_duration_for_testing(kPenaltyDuration);

    // First update.
    task_environment.FastForwardBy(kDelayTime);
    pressure_service.SendUpdate(
        device::mojom::blink::PressureUpdate::New(device::mojom::blink::PressureSource::kCpu, PressureState::kCritical, base::TimeTicks::Now()));
