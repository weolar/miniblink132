...
constexpr char kFeaturePolicyBlocked[] = "Access to the feature \"compute pressure\" is disallowed by permissions "
                                         "policy.";
...
{
    return Vector<V8PressureSource>({ V8PressureSource(V8PressureSource::Enum::kCpu) });
}

ScriptPromise<IDLUndefined> PressureObserver::observe(
    ScriptState* script_state, V8PressureSource source, PressureObserverOptions* options, ExceptionState& exception_state)
{
    ...
    if (!execution_context->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kComputePressure, ReportOptions::kReportOnFailure)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, kFeaturePolicyBlocked);
        return EmptyPromise();
    }
...
void PressureObserver::unobserve(V8PressureSource source)
{
    ...
    RejectPendingResolvers(source.AsEnum(), DOMExceptionCode::kAbortError, "Called unobserve method.");
    records_.erase(base::ranges::remove_if(records_, [source](const auto& record) { return record->source() == source; }), records_.end());
}
...
void PressureObserver::disconnect()
{
    ...
    for (const auto& source : knownSources()) {
        RejectPendingResolvers(source.AsEnum(), DOMExceptionCode::kAbortError, "Called disconnect method.");
    }
    records_.clear();
}
...
void PressureObserver::OnUpdate(ExecutionContext* execution_context, V8PressureSource::Enum source, V8PressureState::Enum state, DOMHighResTimeStamp timestamp)
{
    ...
    if (base::FeatureList::IsEnabled(features::kComputePressureRateObfuscationMitigation)) {
        ...
        if (!PassesRateObfuscation(source)) {
            ...
            // https://w3c.github.io/compute-pressure/#dfn-data-delivery
            after_penalty_records_[source_index] = record;
            pending_delayed_report_to_callback_[source_index] = PostDelayedCancellableTask(*execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI),
                FROM_HERE, WTF::BindOnce(&PressureObserver::QueueAfterPenaltyRecord, WrapWeakPersistent(this), WrapWeakPersistent(execution_context), source),
...
// https://w3c.github.io/compute-pressure/#dfn-data-delivery
void PressureObserver::QueueAfterPenaltyRecord(ExecutionContext* execution_context, V8PressureSource::Enum source)
{
    const auto source_index = ToSourceIndex(source);
...
void PressureObserver::QueuePressureRecord(ExecutionContext* execution_context, V8PressureSource::Enum source, PressureRecord* record)
{
    ...
    pending_report_to_callback_ = PostCancellableTask(*execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI), FROM_HERE,
        WTF::BindOnce(&PressureObserver::ReportToCallback, WrapWeakPersistent(this), WrapWeakPersistent(execution_context)));
}
...
void PressureObserver::OnBindingFailed(V8PressureSource::Enum source, DOMExceptionCode exception_code)
{
    RejectPendingResolvers(source, exception_code, "Not available on this platform.");
}

void PressureObserver::OnConnectionError()
{
    for (const auto& source : knownSources()) {
        RejectPendingResolvers(source.AsEnum(), DOMExceptionCode::kNotSupportedError, "Connection error.");
    }
}
...
bool PressureObserver::PassesRateTest(V8PressureSource::Enum source, const DOMHighResTimeStamp& timestamp) const
{
...
bool PressureObserver::HasChangeInData(V8PressureSource::Enum source, V8PressureState::Enum state) const
{
    const auto& last_record = last_record_map_[ToSourceIndex(source)];
...
bool PressureObserver::PassesRateObfuscation(V8PressureSource::Enum source) const
{
...
void PressureObserver::RejectPendingResolvers(V8PressureSource::Enum source, DOMExceptionCode exception_code, const String& message)
{
    const auto source_index = ToSourceIndex(source);
    for (const auto& resolver : pending_resolvers_[source_index]) {
