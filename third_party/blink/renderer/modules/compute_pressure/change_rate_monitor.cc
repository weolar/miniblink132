...
{
    observation_window_time_ = base::Seconds(base::RandInt(kMinObservationWindowInSeconds, kMaxObservationWindowInSeconds));
    change_count_threshold_ = base::RandInt(kMinChangesThreshold, kMaxChangesThreshold);
    penalty_duration_ = base::Seconds(base::RandInt(kMinPenaltyDurationInSeconds, kMaxPenaltyDurationInSeconds));
...
bool ChangeRateMonitor::ChangeCountExceedsLimit(V8PressureSource::Enum source) const
{
