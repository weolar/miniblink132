...
// static
const char PressureObserverManager::kSupplementName[] = "PressureObserverManager";

// static
PressureObserverManager* PressureObserverManager::From(ExecutionContext* context)
{
    PressureObserverManager* manager = Supplement<ExecutionContext>::From<PressureObserverManager>(context);
    if (!manager) {
    ...
    , pressure_manager_(context)
{
    ...
    for (const auto& source : PressureObserver::knownSources()) {
        source_to_client_.insert(source.AsEnum(), MakeGarbageCollected<PressureClientImpl>(context, this));
    }
}

PressureObserverManager::~PressureObserverManager() = default;

void PressureObserverManager::AddObserver(V8PressureSource::Enum source, PressureObserver* observer)
{
    ...
    if (state == PressureClientImpl::State::kUninitialized) {
        ...
        pressure_manager_->AddClient(
            V8PressureSourceToPressureSource(source), WTF::BindOnce(&PressureObserverManager::DidAddClient, WrapWeakPersistent(this), source));
    } else if (state == PressureClientImpl::State::kInitialized) {
        observer->OnBindingSucceeded(source);
    }
}

void PressureObserverManager::RemoveObserver(V8PressureSource::Enum source, PressureObserver* observer)
{
    PressureClientImpl* client = source_to_client_.at(source);
...
void PressureObserverManager::RemoveObserverFromAllSources(PressureObserver* observer)
{
...
void PressureObserverManager::ContextLifecycleStateChanged(mojom::blink::FrameLifecycleState state)
{
...
void PressureObserverManager::EnsureConnection()
{
    ...
    auto task_runner = GetExecutionContext()->GetTaskRunner(TaskType::kUserInteraction);
    GetExecutionContext()->GetBrowserInterfaceBroker().GetInterface(pressure_manager_.BindNewPipeAndPassReceiver(task_runner));
    pressure_manager_.set_disconnect_handler(WTF::BindOnce(&PressureObserverManager::OnConnectionError, WrapWeakPersistent(this)));
}
...
void PressureObserverManager::ResetPressureManagerIfNeeded()
{
    if (base::ranges::all_of(
            source_to_client_.Values(), [](const PressureClientImpl* client) { return client->state() == PressureClientImpl::State::kUninitialized; })) {
...
void PressureObserverManager::DidAddClient(V8PressureSource::Enum source, device::mojom::blink::PressureManagerAddClientResultPtr result)
{
    PressureClientImpl* client = source_to_client_.at(source);
    ...
    HeapVector<Member<PressureObserver>> observers(client->observers());
    switch (result->which()) {
    case device::mojom::blink::PressureManagerAddClientResult::Tag::kPressureClient: {
