...
{
    mojo::PendingReceiver<mojom::blink::WebPressureManager> receiver(std::move(handle));
    DCHECK(!receiver_.is_bound());
    receiver_.Bind(std::move(receiver));
    receiver_.set_disconnect_handler(WTF::BindOnce(&FakePressureService::OnConnectionError, WTF::Unretained(this)));
}

void FakePressureService::AddClient(device::mojom::blink::PressureSource source, AddClientCallback callback)
{
    std::move(callback).Run(device::mojom::blink::PressureManagerAddClientResult::NewPressureClient(client_remote_.BindNewPipeAndPassReceiver()));
}

void FakePressureService::SendUpdate(device::mojom::blink::PressureUpdatePtr update)
{
...
ComputePressureTestingContext::ComputePressureTestingContext(FakePressureService* mock_pressure_service)
{
    DomWindow()->GetBrowserInterfaceBroker().SetBinderForTesting(
        mojom::blink::WebPressureManager::Name_, WTF::BindRepeating(&FakePressureService::BindRequest, WTF::Unretained(mock_pressure_service)));
}
...
ComputePressureTestingContext::~ComputePressureTestingContext()
{
    ...
    DomWindow()->GetBrowserInterfaceBroker().SetBinderForTesting(mojom::blink::WebPressureManager::Name_, {});
}
