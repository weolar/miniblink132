...
// static
bool ScreenDetailed::AreWebExposedScreenDetailedPropertiesEqual(const display::ScreenInfo& prev, const display::ScreenInfo& current)
{
    ...
    if (RuntimeEnabledFeatures::CanvasHDREnabled()) {
        ...
        if (prev.display_color_spaces.GetHDRMaxLuminanceRelative() != current.display_color_spaces.GetHDRMaxLuminanceRelative()) {
            return false;
        }
