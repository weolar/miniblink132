...
ScreenDetailed* ScreenDetails::currentScreen() const
{
    ...
    auto it = base::ranges::find(screens_, current_display_id_, &ScreenDetailed::DisplayId);
...
void ScreenDetails::UpdateScreenInfos(LocalDOMWindow* window, const display::ScreenInfos& new_infos)
{
    UpdateScreenInfosImpl(window, new_infos, /*dispatch_events=*/true);
}

void ScreenDetails::UpdateScreenInfosImpl(LocalDOMWindow* window, const display::ScreenInfos& new_infos, bool dispatch_events)
{
    ...
    for (WTF::wtf_size_t i = 0; i < screens_.size();
        ...
        if (base::Contains(new_infos.screen_infos, screens_[i]->DisplayId(), &display::ScreenInfo::display_id)) {
            ++i;
        } else {
            screens_.EraseAt(i);
    ...
    for (const auto& info : new_infos.screen_infos) {
        if (!base::Contains(screens_, info.display_id, &ScreenDetailed::DisplayId)) {
            screens_.push_back(MakeGarbageCollected<ScreenDetailed>(window, info.display_id));
            added_or_removed = true;
        }
    }
    ...
    if (dispatch_events) {
        ...
        if (prev_screen_infos_.screen_infos.empty() || prev_screen_infos_.current().display_id != new_infos.current().display_id
            || !ScreenDetailed::AreWebExposedScreenDetailedPropertiesEqual(prev_screen_infos_.current(), new_infos.current())) {
            EnqueueEvent(*Event::Create(event_type_names::kCurrentscreenchange), TaskType::kMiscPlatformAPI);
        }

        // Enqueue a change event if screens were added or removed.
        if (added_or_removed) {
            EnqueueEvent(*Event::Create(event_type_names::kScreenschange), TaskType::kMiscPlatformAPI);
        }
        ...
        for (Member<ScreenDetailed>& screen : screens_) {
            ...
            auto new_it = base::ranges::find(new_infos.screen_infos, id, &display::ScreenInfo::display_id);
            CHECK(new_it != new_infos.screen_infos.end(), base::NotFatalUntil::M130);
            auto old_it = base::ranges::find(prev_screen_infos_.screen_infos, id, &display::ScreenInfo::display_id);
            if (old_it != prev_screen_infos_.screen_infos.end() && !ScreenDetailed::AreWebExposedScreenDetailedPropertiesEqual(*old_it, *new_it)) {
                screen->EnqueueEvent(*Event::Create(event_type_names::kChange), TaskType::kMiscPlatformAPI);
            }
        }
    }
