...
class MODULES_EXPORT ScreenDetails final : public EventTarget, public ExecutionContextLifecycleObserver {
...
public:
    ...
    void UpdateScreenInfos(LocalDOMWindow* window, const display::ScreenInfos& new_infos);

private:
    // Update web-exposed data structures on screen information changes.
    // Enqueues events for dispatch if `dispatch_events` is true.
    void UpdateScreenInfosImpl(LocalDOMWindow* window, const display::ScreenInfos& new_infos, bool dispatch_events);
