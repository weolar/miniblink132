...
class AnimationAndPaintWorkletThreadTest : public PageTestBase, public ModuleTestBase {
public:
    void SetUp() override
    {
    ...
    {
        std::unique_ptr<base::WaitableEvent> wait_event = std::make_unique<base::WaitableEvent>();
        PostCrossThreadTask(*thread->GetWorkerBackingThread().BackingThread().GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(&AnimationAndPaintWorkletThreadTest::ExecuteScriptInWorklet, CrossThreadUnretained(this), CrossThreadUnretained(thread),
...
{
    std::unique_ptr<AnimationAndPaintWorkletThread> worklet = CreateThreadAndProvideAnimationWorkletProxyClient(&GetDocument(), reporting_proxy_.get());
...
TEST_F(AnimationAndPaintWorkletThreadTest, CreateSecondAndTerminateFirst)
{
    ...
    std::unique_ptr<AnimationAndPaintWorkletThread> first_worklet = CreateThreadAndProvideAnimationWorkletProxyClient(&GetDocument(), reporting_proxy_.get());
    Thread* first_thread = &first_worklet->GetWorkerBackingThread().BackingThread();
    ...
    std::unique_ptr<AnimationAndPaintWorkletThread> second_worklet = CreateThreadAndProvideAnimationWorkletProxyClient(&GetDocument(), reporting_proxy_.get());
    ...
    Thread* second_thread = &second_worklet->GetWorkerBackingThread().BackingThread();
...
TEST_F(AnimationAndPaintWorkletThreadTest, TerminateFirstAndCreateSecond)
{
    ...
    worklet = CreateThreadAndProvideAnimationWorkletProxyClient(&GetDocument(), reporting_proxy_.get());
...
TEST_F(AnimationAndPaintWorkletThreadTest, CreatingSecondDuringTerminationOfFirst)
{
    std::unique_ptr<AnimationAndPaintWorkletThread> first_worklet = CreateThreadAndProvideAnimationWorkletProxyClient(&GetDocument(), reporting_proxy_.get());
    CheckWorkletCanExecuteScript(first_worklet.get());
    v8::Isolate* first_isolate = first_worklet->GetIsolate();
...
TEST_F(AnimationAndPaintWorkletThreadTest, WorkletThreadHolderIsRefCountedProperly)
{
    EXPECT_FALSE(AnimationAndPaintWorkletThread::GetWorkletThreadHolderForTesting());

    std::unique_ptr<AnimationAndPaintWorkletThread> worklet = CreateThreadAndProvideAnimationWorkletProxyClient(&GetDocument(), reporting_proxy_.get());
    ASSERT_TRUE(worklet.get());
    WorkletThreadHolder<AnimationAndPaintWorkletThread>* holder = AnimationAndPaintWorkletThread::GetWorkletThreadHolderForTesting();
    EXPECT_TRUE(holder);

    std::unique_ptr<AnimationAndPaintWorkletThread> worklet2 = CreateThreadAndProvideAnimationWorkletProxyClient(&GetDocument(), reporting_proxy_.get());
    ASSERT_TRUE(worklet2.get());
    WorkletThreadHolder<AnimationAndPaintWorkletThread>* holder2 = AnimationAndPaintWorkletThread::GetWorkletThreadHolderForTesting();
    EXPECT_EQ(holder, holder2);

    worklet->Terminate();
    worklet->WaitForShutdownForTesting();
    worklet.reset();
    EXPECT_TRUE(AnimationAndPaintWorkletThread::GetWorkletThreadHolderForTesting());
    ...
    std::unique_ptr<AnimationAndPaintWorkletThread> worklet3 = CreateThreadAndProvideAnimationWorkletProxyClient(&GetDocument(), reporting_proxy_.get());
