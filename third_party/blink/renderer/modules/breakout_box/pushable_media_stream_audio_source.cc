...
PushableMediaStreamAudioSource::Broker::Broker(PushableMediaStreamAudioSource* source, scoped_refptr<base::SequencedTaskRunner> audio_task_runner)
    : source_(source)
...
void PushableMediaStreamAudioSource::Broker::PushAudioData(scoped_refptr<media::AudioBuffer> data)
{
    base::AutoLock locker(lock_);
    if (!source_)
        return;

    if (!should_deliver_audio_on_audio_task_runner_ || audio_task_runner_->RunsTasksInCurrentSequence()) {
        source_->DeliverData(std::move(data));
    } else {
        PostCrossThreadTask(
            *audio_task_runner_, FROM_HERE, CrossThreadBindOnce(&PushableMediaStreamAudioSource::Broker::PushAudioData, WrapRefCounted(this), std::move(data)));
    }
}
...
void PushableMediaStreamAudioSource::Broker::StopSource()
{
    if (main_task_runner_->RunsTasksInCurrentSequence()) {
        ...
        PostCrossThreadTask(
            *main_task_runner_, FROM_HERE, CrossThreadBindOnce(&PushableMediaStreamAudioSource::Broker::StopSourceOnMain, WrapRefCounted(this)));
    }
}

void PushableMediaStreamAudioSource::Broker::SetShouldDeliverAudioOnAudioTaskRunner(bool should_deliver_audio_on_audio_task_runner)
{
    base::AutoLock locker(lock_);
    should_deliver_audio_on_audio_task_runner_ = should_deliver_audio_on_audio_task_runner;
}

bool PushableMediaStreamAudioSource::Broker::ShouldDeliverAudioOnAudioTaskRunner()
{
    base::AutoLock locker(lock_);
...
PushableMediaStreamAudioSource::PushableMediaStreamAudioSource(
    scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, scoped_refptr<base::SequencedTaskRunner> audio_task_runner)
...
void PushableMediaStreamAudioSource::PushAudioData(scoped_refptr<media::AudioBuffer> data)
{
    broker_->PushAudioData(std::move(data));
}

void PushableMediaStreamAudioSource::DeliverData(scoped_refptr<media::AudioBuffer> data)
{
    DCHECK(data);
    ...
    if (!params.IsValid() || params.format() != media::AudioParameters::AUDIO_PCM_LOW_LATENCY || last_channels_ != channel_count
        || last_sample_rate_ != sample_rate || last_frames_ != frame_count) {
        params
            = media::AudioParameters(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Guess(channel_count), sample_rate, frame_count);
        SetFormat(params);
    ...
    std::unique_ptr<media::AudioBus> audio_bus = media::AudioBuffer::WrapOrCopyToAudioBus(data);
