...
const media::AudioParameters kMonoParams
    = media::AudioParameters(media::AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::Mono(), kSampleRate, kFramesPerBuffer);

const media::AudioParameters kStereoParams
    = media::AudioParameters(media::AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::Stereo(), kSampleRate, kFramesPerBuffer);
...
public:
    ...
    MediaStreamTrack* CreateTrack(ExecutionContext* execution_context)
    {
        auto pushable_audio_source
            = std::make_unique<PushableMediaStreamAudioSource>(scheduler::GetSingleThreadTaskRunnerForTesting(), platform_->GetIOTaskRunner());
        PushableMediaStreamAudioSource* pushable_audio_source_ptr = pushable_audio_source.get();
        MediaStreamSource* media_stream_source = MakeGarbageCollected<MediaStreamSource>(
            "dummy_source_id", MediaStreamSource::kTypeAudio, "dummy_source_name", false /* remote */, std::move(pushable_audio_source));
        MediaStreamComponent* component = MakeGarbageCollected<MediaStreamComponentImpl>(
            String::FromUTF8("audio_track"), media_stream_source, std::make_unique<MediaStreamAudioTrack>(true /* is_local_track */));
    ...
    MediaStreamAudioTrackUnderlyingSource* CreateSource(ScriptState* script_state, MediaStreamTrack* track, wtf_size_t buffer_size)
    {
        return MakeGarbageCollected<MediaStreamAudioTrackUnderlyingSource>(script_state, track->Component(), nullptr, buffer_size);
    }

    MediaStreamAudioTrackUnderlyingSource* CreateSource(ScriptState* script_state, MediaStreamTrack* track)
    {
...
protected:
    ...
    void PushData(MediaStreamTrack* track, const std::optional<base::TimeDelta>& timestamp = std::nullopt)
    {
        auto data = media::AudioBuffer::CreateEmptyBuffer(
            media::ChannelLayout::CHANNEL_LAYOUT_STEREO, /*channel_count=*/2, kSampleRate, kNumFrames, timestamp.value_or(base::Seconds(1)));
        PushableMediaStreamAudioSource* pushable_audio_source
            = static_cast<PushableMediaStreamAudioSource*>(MediaStreamAudioSource::From(track->Component()->Source()));
    ...
    bool DataMatches(scoped_refptr<media::AudioBuffer> buffer, const media::AudioBus& bus)
    {
        ...
        for (int ch = 0; ch < bus.channels(); ch++) {
            ...
            const float* buffer_channel = reinterpret_cast<float*>(buffer->channel_data()[ch]);
            for (int i = 0; i < bus.frames(); ++i) {
    ...
    std::unique_ptr<media::AudioBus> CreateTestData(const media::AudioParameters params, float channel_value_increment)
    {
...
TEST_F(MediaStreamAudioTrackUnderlyingSourceTest, AudioDataFlowsThroughStreamAndCloses)
{
...
TEST_F(MediaStreamAudioTrackUnderlyingSourceTest, CancelStreamDisconnectsFromTrack)
{
    V8TestingScope v8_scope;
    ScriptState* script_state = v8_scope.GetScriptState();
    auto* track = CreateTrack(v8_scope.GetExecutionContext());
...
TEST_F(MediaStreamAudioTrackUnderlyingSourceTest, DropOldFramesWhenQueueIsFull)
{
    V8TestingScope v8_scope;
    ScriptState* script_state = v8_scope.GetScriptState();
    ...
    WebMediaStreamAudioSink::AddToAudioTrack(&mock_sink, WebMediaStreamTrack(track->Component()));

    auto push_frame_sync = [&mock_sink, track, this](const base::TimeDelta timestamp) {
        base::RunLoop sink_loop;
        EXPECT_CALL(mock_sink, OnData(_, _)).WillOnce(base::test::RunOnceClosure(sink_loop.QuitClosure()));
    ...
    source->Close();
    WebMediaStreamAudioSink::RemoveFromAudioTrack(&mock_sink, WebMediaStreamTrack(track->Component()));
    track->stopTrack(v8_scope.GetExecutionContext());
}
