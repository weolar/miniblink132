...
VideoTrackGenerator* VideoTrackGenerator::Create(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    if (!ExecutionContext::From(script_state)->IsWindow()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "VideoTrackGenerator in worker does not work yet");
    }

    return MakeGarbageCollected<VideoTrackGenerator>(script_state, exception_state);
}

VideoTrackGenerator::VideoTrackGenerator(ScriptState* script_state, ExceptionState& exception_state)
{
    wrapped_generator_ = MakeGarbageCollected<MediaStreamTrackGenerator>(script_state, MediaStreamSource::kTypeVideo);
}
