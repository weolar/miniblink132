...
class AudioBufferPoolImpl : public MediaStreamAudioTrackUnderlyingSource::AudioBufferPool {
public:
    ...
    scoped_refptr<media::AudioBuffer> CopyIntoAudioBuffer(const media::AudioBus& audio_bus, base::TimeTicks capture_time) override
    {
    ...
    int GetSizeForTesting() override
    {
        return buffers_.size();
    }

private:
    scoped_refptr<media::AudioBuffer> AllocateAndSaveNewBuffer(const media::AudioBus& audio_bus, base::TimeTicks capture_time)
    {
        auto buffer = media::AudioBuffer::CopyFrom(params_.sample_rate(), capture_time - base::TimeTicks(), &audio_bus, nullptr);
        buffers_.push_back(buffer);
        return buffer;
    }
...
void MediaStreamAudioTrackUnderlyingSource::DisconnectFromTrack()
{
    ...
    WebMediaStreamAudioSink::RemoveFromAudioTrack(this, WebMediaStreamTrack(track_));
...
void MediaStreamAudioTrackUnderlyingSource::OnData(const media::AudioBus& audio_bus, base::TimeTicks estimated_capture_time)
{
    QueueFrame(buffer_pool_->CopyIntoAudioBuffer(audio_bus, estimated_capture_time));
}
...
void MediaStreamAudioTrackUnderlyingSource::OnSetFormat(const media::AudioParameters& params)
{
    buffer_pool_->SetFormat(params);
}

std::unique_ptr<ReadableStreamTransferringOptimizer> MediaStreamAudioTrackUnderlyingSource::GetTransferringOptimizer()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    return std::make_unique<AudioDataQueueTransferOptimizer>(this, GetRealmRunner(), MaxQueueSize(),
        CrossThreadBindOnce(&MediaStreamAudioTrackUnderlyingSource::OnSourceTransferStarted, WrapCrossThreadWeakPersistent(this)),
        CrossThreadBindOnce(&MediaStreamAudioTrackUnderlyingSource::ClearTransferredSource, WrapCrossThreadWeakPersistent(this)));
}

void MediaStreamAudioTrackUnderlyingSource::OnSourceTransferStarted(
    scoped_refptr<base::SequencedTaskRunner> transferred_runner, CrossThreadPersistent<TransferredAudioDataQueueUnderlyingSource> source)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
MediaStreamAudioTrackUnderlyingSource::AudioBufferPool* MediaStreamAudioTrackUnderlyingSource::GetAudioBufferPoolForTesting()
{
