...
public:
    FrameQueueTest()
        : io_task_runner_(Platform::Current()->GetIOTaskRunner())
    {
    }
...
TEST_F(FrameQueueTest, PushPopMatches)
{
    ...
    scoped_refptr<FrameQueue<int>> queue = base::MakeRefCounted<FrameQueue<int>>(kMaxSize);
...
{
    scoped_refptr<FrameQueue<int>> queue = base::MakeRefCounted<FrameQueue<int>>(5);
    std::optional<int> element = queue->Pop();
...
{
    const int kMaxSize = 5;
    scoped_refptr<FrameQueue<int>> original_queue = base::MakeRefCounted<FrameQueue<int>>(kMaxSize);
...
TEST_F(FrameQueueTest, PushValuesInOrderOnSeparateThread)
{
    ...
    PostCrossThreadTask(*io_task_runner_, FROM_HERE,
        CrossThreadBindOnce(
            [](FrameQueueHandle<int>* handle, base::WaitableEvent* start_event, base::WaitableEvent* end_event) {
            ...
            CrossThreadUnretained(&handle1), CrossThreadUnretained(&start_event), CrossThreadUnretained(&end_event)));
