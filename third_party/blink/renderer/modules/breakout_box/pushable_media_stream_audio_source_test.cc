...
public:
    FakeMediaStreamAudioSink(scoped_refptr<base::SingleThreadTaskRunner> main_thread, scoped_refptr<base::SingleThreadTaskRunner> audio_thread)
    ...
    void SetupNewAudioParameterExpectations(int channels, int frames, int sample_rate)
    {
    ...
    void SetDataTimeExpectation(base::TimeTicks time, media::AudioBus* expected_data, bool expect_data_on_audio_task_runner, base::OnceClosure on_data)
    {
        DCHECK(!on_data_);
    ...
    void OnData(const media::AudioBus& data, base::TimeTicks time) override
    {
        ...
        EXPECT_EQ(audio_task_runner_->BelongsToCurrentThread(), expect_data_on_audio_task_runner_);
    ...
    void OnSetFormat(const media::AudioParameters& params) override
    {
        ...
        // check above to fail).
        ASSERT_NE(audio_task_runner_->BelongsToCurrentThread(), main_task_runner_->BelongsToCurrentThread());
    ...
    void ClearDidReceiveFormatChange()
    {
        did_receive_format_change_ = false;
    }

    bool did_receive_format_change() const
    {
        return did_receive_format_change_;
    }
...
class PushableMediaStreamAudioSourceTest : public ::testing::TestWithParam<bool> {
public:
    PushableMediaStreamAudioSourceTest()
    {
        ...
        auto pushable_audio_source = std::make_unique<PushableMediaStreamAudioSource>(main_task_runner_, audio_task_runner_);
        pushable_audio_source_ = pushable_audio_source.get();
        broker_ = pushable_audio_source->GetBroker();
        broker_->SetShouldDeliverAudioOnAudioTaskRunner(ShouldDeliverAudioOnAudioTaskRunner());
        stream_source_ = MakeGarbageCollected<MediaStreamSource>(
            "dummy_source_id", MediaStreamSource::kTypeAudio, "dummy_source_name", false /* remote */, std::move(pushable_audio_source));
        stream_component_ = MakeGarbageCollected<MediaStreamComponentImpl>(
            stream_source_->Id(), stream_source_, std::make_unique<MediaStreamAudioTrack>(true /* is_local_track */));
    }
    ...
    void SendEmptyBufferAndVerifyParams(FakeMediaStreamAudioSink* fake_sink, int channels, int frames, int sample_rate, bool expect_format_change)
    {
        SendDataAndVerifyParams(fake_sink, channels, frames, sample_rate, expect_format_change, nullptr, nullptr);
    }

    void SendDataAndVerifyParams(FakeMediaStreamAudioSink* fake_sink, int channels, int frames, int sample_rate, bool expect_format_change,
        scoped_refptr<media::AudioBuffer> buffer, media::AudioBus* expected_data)
    {
        fake_sink->ClearDidReceiveFormatChange();

        if (expect_format_change) {
            fake_sink->SetupNewAudioParameterExpectations(channels, frames, sample_rate);
        }

        if (!buffer) {
            base::TimeTicks timestamp = base::TimeTicks::Now();
            buffer = media::AudioBuffer::CreateEmptyBuffer(media::GuessChannelLayout(channels), channels, sample_rate, frames, timestamp - base::TimeTicks());
        }

        base::RunLoop run_loop;
        fake_sink->SetDataTimeExpectation(
            base::TimeTicks() + buffer->timestamp(), expected_data, ShouldDeliverAudioOnAudioTaskRunner(), run_loop.QuitClosure());
    ...
    bool ShouldDeliverAudioOnAudioTaskRunner() const
    {
        return GetParam();
    }
...
protected:
    ...
    raw_ptr<PushableMediaStreamAudioSource, DanglingUntriaged> pushable_audio_source_;
...
TEST_P(PushableMediaStreamAudioSourceTest, FramesPropagateToSink)
{
    ...
    WebMediaStreamAudioSink::AddToAudioTrack(&fake_sink, WebMediaStreamTrack(stream_component_.Get()));
    ...
    WebMediaStreamAudioSink::RemoveFromAudioTrack(&fake_sink, WebMediaStreamTrack(stream_component_.Get()));
}
...
TEST_P(PushableMediaStreamAudioSourceTest, ConvertsFormatInternally)
{
    ...
    constexpr media::ChannelLayout kChannelLayout = media::ChannelLayout::CHANNEL_LAYOUT_STEREO;
    constexpr int kChannels = 2;
    constexpr int kSampleRate = 8000;
    constexpr int kFrames = 256;
    constexpr base::TimeDelta kDefaultTimeStamp = base::Milliseconds(123);

    auto interleaved_buffer = media::AudioBuffer::CreateBuffer(media::SampleFormat::kSampleFormatF32, kChannelLayout, kChannels, kSampleRate, kFrames);
    interleaved_buffer->set_timestamp(kDefaultTimeStamp);

    // Create interleaved data, with negative values on the second channel.
    float* interleaved_buffer_data = reinterpret_cast<float*>(interleaved_buffer->channel_data()[0]);
    ...
    SendDataAndVerifyParams(&fake_sink, kChannels, kFrames, kSampleRate,
        /*expect_format_change=*/true, std::move(interleaved_buffer), expected_data.get());

    auto planar_buffer = media::AudioBuffer::CopyFrom(kSampleRate, kDefaultTimeStamp, expected_data.get());

    // The pushable source shouldn't have to convert data internally, and should
    // just wrap it.
    SendDataAndVerifyParams(&fake_sink, kChannels, kFrames, kSampleRate,
        /*expect_format_change=*/false, std::move(planar_buffer), expected_data.get());
...
INSTANTIATE_TEST_SUITE_P(/*no prefix*/, PushableMediaStreamAudioSourceTest, ::testing::Bool());
