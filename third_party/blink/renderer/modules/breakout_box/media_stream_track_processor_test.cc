...
std::unique_ptr<PushableMediaStreamAudioSource> CreatePushableAudioSource()
{
    ...
    return std::make_unique<PushableMediaStreamAudioSource>(scheduler::GetSingleThreadTaskRunnerForTesting(), Platform::Current()->GetIOTaskRunner());
}

PushableMediaStreamVideoSource* CreatePushableVideoSource()
{
    PushableMediaStreamVideoSource* pushable_video_source = new PushableMediaStreamVideoSource(scheduler::GetSingleThreadTaskRunnerForTesting());
    ...
    MakeGarbageCollected<MediaStreamSource>("source_id", MediaStreamSource::kTypeVideo, "source_name",
        /*remote=*/false, base::WrapUnique(pushable_video_source));
    return pushable_video_source;
}

MediaStreamTrack* CreateAudioMediaStreamTrack(ExecutionContext* context, std::unique_ptr<MediaStreamAudioSource> source)
{
    auto* source_ptr = source.get();

    MediaStreamSource* media_stream_source = MakeGarbageCollected<MediaStreamSource>("source_id", MediaStreamSource::kTypeAudio, "source_name",
        /*remote=*/false, std::move(source));

    MediaStreamComponent* component
        = MakeGarbageCollected<MediaStreamComponentImpl>(media_stream_source, std::make_unique<MediaStreamAudioTrack>(true /* is_local_track */));
...
TEST_F(MediaStreamTrackProcessorTest, VideoFramesAreExposed)
{
    ...
    PushableMediaStreamVideoSource* pushable_video_source = CreatePushableVideoSource();
    MediaStreamTrackProcessor* track_processor
        = MediaStreamTrackProcessor::Create(script_state, CreateVideoMediaStreamTrack(v8_scope.GetExecutionContext(), pushable_video_source), exception_state);
    EXPECT_FALSE(exception_state.HadException());
    EXPECT_EQ(track_processor->InputTrack()->Component()->Source()->GetPlatformSource(), pushable_video_source);

    MockMediaStreamVideoSink mock_video_sink;
    mock_video_sink.ConnectToTrack(WebMediaStreamTrack(track_processor->InputTrack()->Component()));
    EXPECT_EQ(mock_video_sink.number_of_frames(), 0);
    EXPECT_EQ(mock_video_sink.last_frame(), nullptr);

    auto* reader = track_processor->readable(script_state)->GetDefaultReaderForTesting(script_state, exception_state);
    EXPECT_FALSE(exception_state.HadException());

    // Deliver a frame
    base::RunLoop sink_loop;
    EXPECT_CALL(mock_video_sink, OnVideoFrame(_)).WillOnce(base::test::RunOnceClosure(sink_loop.QuitClosure()));
    scoped_refptr<media::VideoFrame> frame = media::VideoFrame::CreateBlackFrame(gfx::Size(10, 5));
    ...
    histogram_tester.ExpectUniqueSample("Media.BreakoutBox.Usage", BreakoutBoxUsage::kReadableVideo, 1);
...
TEST_F(MediaStreamTrackProcessorTest, AudioDataAreExposed)
{
    ...
    ScriptState* script_state = v8_scope.GetScriptState();
    ExceptionState& exception_state = v8_scope.GetExceptionState();
    std::unique_ptr<PushableMediaStreamAudioSource> pushable_audio_source = CreatePushableAudioSource();
    auto* pushable_source_ptr = pushable_audio_source.get();
    MediaStreamTrackProcessor* track_processor = MediaStreamTrackProcessor::Create(
        script_state, CreateAudioMediaStreamTrack(v8_scope.GetExecutionContext(), std::move(pushable_audio_source)), exception_state);
    EXPECT_FALSE(exception_state.HadException());
    ...
    WebMediaStreamAudioSink::AddToAudioTrack(&mock_audio_sink, WebMediaStreamTrack(component));
    ...
    // Deliver data.
    base::RunLoop sink_loop;
    EXPECT_CALL(mock_audio_sink, OnData(_, _)).WillOnce(base::test::RunOnceClosure(sink_loop.QuitClosure()));
    pushable_source_ptr->PushAudioData(media::AudioBuffer::CreateEmptyBuffer(media::ChannelLayout::CHANNEL_LAYOUT_STEREO, /*channel_count=*/2,
        /*sample_rate=*/8000,
    ...
    WebMediaStreamAudioSink::RemoveFromAudioTrack(&mock_audio_sink, WebMediaStreamTrack(component));
    histogram_tester.ExpectUniqueSample("Media.BreakoutBox.Usage", BreakoutBoxUsage::kReadableAudio, 1);
    histogram_tester.ExpectTotalCount("Media.BreakoutBox.Usage", 1);
}
...
TEST_F(MediaStreamTrackProcessorTest, CanceledReadableDisconnects)
{
    ...
    MediaStreamVideoTrack* video_track = MediaStreamVideoTrack::From(track_processor->InputTrack()->Component());
    ...
    ScriptPromiseTester cancel_tester(script_state, readable->cancel(script_state, exception_state));
...
TEST_F(MediaStreamTrackProcessorTest, ProcessorConnectsToGenerator)
{
    ...
    init->setKind("video");
    MediaStreamTrackGenerator* track_generator = MediaStreamTrackGenerator::Create(script_state, init, exception_state);
    MockMediaStreamVideoSink mock_video_sink;
    mock_video_sink.ConnectToTrack(WebMediaStreamTrack(track_generator->Component()));
    EXPECT_EQ(mock_video_sink.number_of_frames(), 0);
    EXPECT_EQ(mock_video_sink.last_frame(), nullptr);

    // Connect the processor to the generator
    track_processor->readable(script_state)->pipeTo(script_state, track_generator->writable(script_state), exception_state);
...
TEST_F(MediaStreamTrackProcessorTest, NullInputTrack)
{
    ...
    MediaStreamTrackProcessor* track_processor = MediaStreamTrackProcessor::Create(script_state, track, exception_state);

    EXPECT_EQ(track_processor, nullptr);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_EQ(static_cast<ESErrorType>(v8_scope.GetExceptionState().Code()), ESErrorType::kTypeError);
}
...
TEST_F(MediaStreamTrackProcessorTest, EndedTrack)
{
    ...
    ExceptionState& exception_state = v8_scope.GetExceptionState();
    PushableMediaStreamVideoSource* pushable_video_source = CreatePushableVideoSource();
    MediaStreamTrack* track = CreateVideoMediaStreamTrack(v8_scope.GetExecutionContext(), pushable_video_source);
...
TEST_F(MediaStreamTrackProcessorTest, AudioCloseOnTrackEnd)
{
    ...
    ExceptionState& exception_state = v8_scope.GetExceptionState();
    std::unique_ptr<PushableMediaStreamAudioSource> pushable_audio_source = CreatePushableAudioSource();
    MediaStreamTrack* track = CreateAudioMediaStreamTrack(v8_scope.GetExecutionContext(), std::move(pushable_audio_source));
