...
class MODULES_EXPORT MediaStreamVideoTrackUnderlyingSource : public VideoFrameQueueUnderlyingSource, public MediaStreamVideoSink {
public:
    using CrossThreadFrameQueueSource = CrossThreadPersistent<TransferredVideoFrameQueueUnderlyingSource>;
    static const int kMaxMonitoredFrameCount;
    static const int kMinMonitoredFrameCount;

    explicit MediaStreamVideoTrackUnderlyingSource(ScriptState*, MediaStreamComponent*, ScriptWrappable* media_stream_track_processor, wtf_size_t queue_size);
    MediaStreamVideoTrackUnderlyingSource(const MediaStreamVideoTrackUnderlyingSource&) = delete;
    MediaStreamVideoTrackUnderlyingSource& operator=(const MediaStreamVideoTrackUnderlyingSource&) = delete;
    ...
    std::unique_ptr<ReadableStreamTransferringOptimizer> GetStreamTransferOptimizer();

private:
    FRIEND_TEST_ALL_PREFIXES(MediaStreamVideoTrackUnderlyingSourceTest, DeviceIdAndMaxFrameCountForMonitoring);
    FRIEND_TEST_ALL_PREFIXES(MediaStreamVideoTrackUnderlyingSourceTest, FrameLimiter);
    ...
    void OnSourceTransferStarted(scoped_refptr<base::SequencedTaskRunner>, CrossThreadPersistent<TransferredVideoFrameQueueUnderlyingSource>);

    void OnFrameFromTrack(scoped_refptr<media::VideoFrame> media_frame, base::TimeTicks estimated_capture_time);
