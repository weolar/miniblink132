...
class MODULES_EXPORT PushableMediaStreamAudioSource : public MediaStreamAudioSource {
public:
    ...
    class MODULES_EXPORT LOCKABLE Broker : public WTF::ThreadSafeRefCounted<Broker> {
    public:
        ...
        void StopSource();
        void SetShouldDeliverAudioOnAudioTaskRunner(bool should_deliver_audio_on_audio_task_runner);
        bool ShouldDeliverAudioOnAudioTaskRunner();

    private:
        friend class PushableMediaStreamAudioSource;

        explicit Broker(PushableMediaStreamAudioSource* source, scoped_refptr<base::SequencedTaskRunner> audio_task_runner);
    ...
    PushableMediaStreamAudioSource(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, scoped_refptr<base::SequencedTaskRunner> audio_task_runner);
    ...
    {
        return broker_->IsRunning();
    }
    scoped_refptr<Broker> GetBroker() const
    {
        return broker_;
    }
