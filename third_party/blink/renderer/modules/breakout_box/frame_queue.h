...
template <typename NativeFrameType> class FrameQueue : public WTF::ThreadSafeRefCounted<FrameQueue<NativeFrameType>> {
public:
    ...
    std::optional<NativeFrameType> PushLocked(NativeFrameType frame) EXCLUSIVE_LOCKS_REQUIRED(GetLock())
    {
    ...
    std::optional<NativeFrameType> PopLocked() EXCLUSIVE_LOCKS_REQUIRED(GetLock())
    {
        if (queue_.empty())
            return std::nullopt;
    ...
    std::optional<NativeFrameType> PeekLocked() EXCLUSIVE_LOCKS_REQUIRED(GetLock())
    {
        if (queue_.empty())
            return std::nullopt;
        return queue_.front();
...
// threads.
template <typename NativeFrameType> class FrameQueueHandle {
public:
    explicit FrameQueueHandle(scoped_refptr<FrameQueue<NativeFrameType>> frame_queue)
