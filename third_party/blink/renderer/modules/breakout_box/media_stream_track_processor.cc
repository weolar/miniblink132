...
class MediaStreamTrackProcessor::UnderlyingSourceCloser : public GarbageCollected<UnderlyingSourceCloser>, public MediaStreamTrack::Observer {
public:
    UnderlyingSourceCloser(MediaStreamTrack* track, MediaStreamTrackProcessor* processor)
        : track_(track)
...
MediaStreamTrackProcessor::MediaStreamTrackProcessor(ScriptState* script_state, MediaStreamTrack* input_track, uint16_t buffer_size)
    : input_track_(input_track)
    , buffer_size_(buffer_size)
{
    DCHECK(input_track_);
    UseCounter::Count(ExecutionContext::From(script_state), WebFeature::kMediaStreamTrackProcessor);
}
...
ReadableStream* MediaStreamTrackProcessor::readable(ScriptState* script_state)
{
    ...
    if (input_track_->Component()->GetSourceType() == MediaStreamSource::kTypeVideo) {
        CreateVideoSourceStream(script_state);
    } else {
        CreateAudioSourceStream(script_state);
    }

    source_closer_ = MakeGarbageCollected<UnderlyingSourceCloser>(input_track_, this);
    input_track_->AddObserver(source_closer_);

    return source_stream_.Get();
}

void MediaStreamTrackProcessor::CreateVideoSourceStream(ScriptState* script_state)
{
    DCHECK(!source_stream_);
    video_underlying_source_ = MakeGarbageCollected<MediaStreamVideoTrackUnderlyingSource>(script_state, input_track_->Component(), this, buffer_size_);
    source_stream_ = ReadableStream::CreateWithCountQueueingStrategy(script_state, video_underlying_source_,
        /*high_water_mark=*/0, AllowPerChunkTransferring(true), video_underlying_source_->GetStreamTransferOptimizer());
}

void MediaStreamTrackProcessor::CreateAudioSourceStream(ScriptState* script_state)
{
    DCHECK(!source_stream_);
    audio_underlying_source_ = MakeGarbageCollected<MediaStreamAudioTrackUnderlyingSource>(script_state, input_track_->Component(), this, buffer_size_);
    source_stream_ = ReadableStream::CreateWithCountQueueingStrategy(
        script_state, audio_underlying_source_, /*high_water_mark=*/0, AllowPerChunkTransferring(false), audio_underlying_source_->GetTransferringOptimizer());
}

MediaStreamTrackProcessor* MediaStreamTrackProcessor::Create(
    ScriptState* script_state, MediaStreamTrack* track, uint16_t buffer_size, ExceptionState& exception_state)
{
    if (!track) {
    ...
    if (!script_state->ContextIsValid()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The context has been destroyed");

        return nullptr;
    }

    return MakeGarbageCollected<MediaStreamTrackProcessor>(script_state, track, buffer_size);
}

MediaStreamTrackProcessor* MediaStreamTrackProcessor::Create(ScriptState* script_state, MediaStreamTrack* track, ExceptionState& exception_state)
{
    if (!track) {
        exception_state.ThrowTypeError("Input track cannot be null");
        return nullptr;
...
MediaStreamTrackProcessor* MediaStreamTrackProcessor::Create(ScriptState* script_state, MediaStreamTrackProcessorInit* init, ExceptionState& exception_state)
{
    if (init->hasMaxBufferSize()) {
        return Create(script_state, init->track(), init->maxBufferSize(), exception_state);
    }
