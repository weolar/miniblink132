...
{
    const scoped_refptr<media::VideoFrame> media_frame = media::VideoFrame::CreateBlackFrame(gfx::Size(10, 5));
    VideoFrame* video_frame = MakeGarbageCollected<VideoFrame>(std::move(media_frame), ExecutionContext::From(script_state));
    return ScriptValue(script_state->GetIsolate(), ToV8Traits<VideoFrame>::ToV8(script_state, video_frame));
}

ScriptValue CreateAudioDataChunk(ScriptState* script_state)
{
    AudioData* audio_data = MakeGarbageCollected<AudioData>(media::AudioBuffer::CreateEmptyBuffer(media::ChannelLayout::CHANNEL_LAYOUT_STEREO,
        /*channel_count=*/2,
        /*sample_rate=*/44100,
...
TEST_F(MediaStreamTrackGeneratorTest, VideoFramesAreWritten)
{
    ...
    MediaStreamTrackGenerator* generator = MediaStreamTrackGenerator::Create(script_state, "video", v8_scope.GetExceptionState());

    MockMediaStreamVideoSink media_stream_video_sink;
    media_stream_video_sink.ConnectToTrack(WebMediaStreamTrack(generator->Component()));
    ...
    EXPECT_CALL(media_stream_video_sink, OnVideoFrame(_)).WillOnce(base::test::RunOnceClosure(sink_loop.QuitClosure()));
    ExceptionState& exception_state = v8_scope.GetExceptionState();
    auto* writer = generator->writable(script_state)->getWriter(script_state, exception_state);
    ScriptPromiseTester write_tester(script_state, writer->write(script_state, CreateVideoFrameChunk(script_state), exception_state));
    ...
    ScriptPromiseTester close_tester(script_state, generator->writable(script_state)->close(script_state, exception_state));
    close_tester.WaitUntilSettled();
    EXPECT_FALSE(exception_state.HadException());
    EXPECT_TRUE(generator->Ended());
    histogram_tester.ExpectUniqueSample("Media.BreakoutBox.Usage", BreakoutBoxUsage::kWritableVideo, 1);
...
TEST_F(MediaStreamTrackGeneratorTest, AudioDataAreWritten)
{
    ...
    V8TestingScope v8_scope;
    ScriptState* script_state = v8_scope.GetScriptState();
    MediaStreamTrackGenerator* generator = MediaStreamTrackGenerator::Create(script_state, "audio", v8_scope.GetExceptionState());

    MockMediaStreamAudioSink media_stream_audio_sink;
    WebMediaStreamAudioSink::AddToAudioTrack(&media_stream_audio_sink, WebMediaStreamTrack(generator->Component()));

    base::RunLoop sink_loop;
    EXPECT_CALL(media_stream_audio_sink, OnData(_, _)).WillOnce(testing::WithArg<0>([&](const media::AudioBus& data) {
        EXPECT_NE(data.frames(), 0);
        sink_loop.Quit();
    }));
    ExceptionState& exception_state = v8_scope.GetExceptionState();
    auto* writer = generator->writable(script_state)->getWriter(script_state, exception_state);
    ScriptPromiseTester write_tester(script_state, writer->write(script_state, CreateAudioDataChunk(script_state), exception_state));
    EXPECT_FALSE(write_tester.IsFulfilled());
    write_tester.WaitUntilSettled();
    sink_loop.Run();
    EXPECT_TRUE(write_tester.IsFulfilled());
    ...
    WebMediaStreamAudioSink::RemoveFromAudioTrack(&media_stream_audio_sink, WebMediaStreamTrack(generator->Component()));
    histogram_tester.ExpectUniqueSample("Media.BreakoutBox.Usage", BreakoutBoxUsage::kWritableAudio, 1);
    histogram_tester.ExpectTotalCount("Media.BreakoutBox.Usage", 1);
}
...
TEST_F(MediaStreamTrackGeneratorTest, Clone)
{
    V8TestingScope v8_scope;
    ScriptState* script_state = v8_scope.GetScriptState();
    MediaStreamTrackGenerator* original = MediaStreamTrackGenerator::Create(script_state, "video", v8_scope.GetExceptionState());
    ...
    EXPECT_CALL(original_sink, OnVideoFrame(_)).WillOnce(base::test::RunOnceClosure(original_sink_loop.QuitClosure()));
    EXPECT_CALL(clone_sink, OnVideoFrame(_)).WillOnce(base::test::RunOnceClosure(clone_sink_loop.QuitClosure()));
    ExceptionState& exception_state = v8_scope.GetExceptionState();
    auto* writer = original->writable(script_state)->getWriter(script_state, exception_state);
    ScriptPromiseTester write_tester(script_state, writer->write(script_state, CreateVideoFrameChunk(script_state), exception_state));
    write_tester.WaitUntilSettled();
    ...
    EXPECT_CALL(clone_sink, OnVideoFrame(_)).WillOnce(base::test::RunOnceClosure(clone_sink_loop2.QuitClosure()));
    ScriptPromiseTester write_tester2(script_state, writer->write(script_state, CreateVideoFrameChunk(script_state), exception_state));
    ...
    ScriptPromiseTester write_tester3(script_state, writer->write(script_state, CreateVideoFrameChunk(script_state), exception_state));
...
TEST_F(MediaStreamTrackGeneratorTest, CloneStopSource)
{
    ...
    ScriptPromiseTester close_tester(script_state, original->writable(script_state)->close(script_state, v8_scope.GetExceptionState()));
    close_tester.WaitUntilSettled();
