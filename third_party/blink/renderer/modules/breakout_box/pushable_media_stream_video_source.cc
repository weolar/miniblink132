...
PushableMediaStreamVideoSource::Broker::Broker(PushableMediaStreamVideoSource* source)
    : source_(source)
...
void PushableMediaStreamVideoSource::Broker::PushFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks estimated_capture_time)
{
    base::AutoLock locker(lock_);
    ...
    PostCrossThreadTask(*video_task_runner_, FROM_HERE, CrossThreadBindOnce(frame_callback_, std::move(video_frame), estimated_capture_time));
}
...
void PushableMediaStreamVideoSource::Broker::StopSource()
{
    if (main_task_runner_->BelongsToCurrentThread()) {
        ...
        PostCrossThreadTask(
            *main_task_runner_, FROM_HERE, CrossThreadBindOnce(&PushableMediaStreamVideoSource::Broker::StopSourceOnMain, WrapRefCounted(this)));
    }
}
...
void PushableMediaStreamVideoSource::Broker::OnSourceStarted(VideoCaptureDeliverFrameCB frame_callback)
{
...
void PushableMediaStreamVideoSource::Broker::SetCanDiscardAlpha(bool can_discard_alpha)
{
    base::AutoLock locker(lock_);
    can_discard_alpha_ = can_discard_alpha;
}

void PushableMediaStreamVideoSource::Broker::ProcessFeedback(const media::VideoCaptureFeedback& feedback)
{
    base::AutoLock locker(lock_);
    feedback_ = feedback;
}

PushableMediaStreamVideoSource::PushableMediaStreamVideoSource(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner)
...
void PushableMediaStreamVideoSource::PushFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks estimated_capture_time)
{
    broker_->PushFrame(std::move(video_frame), estimated_capture_time);
}

void PushableMediaStreamVideoSource::StartSourceImpl(VideoCaptureDeliverFrameCB frame_callback, EncodedVideoFrameCB encoded_frame_callback,
...
base::WeakPtr<MediaStreamVideoSource> PushableMediaStreamVideoSource::GetWeakPtr()
{
    return weak_factory_.GetWeakPtr();
}

void PushableMediaStreamVideoSource::OnSourceCanDiscardAlpha(bool can_discard_alpha)
{
    broker_->SetCanDiscardAlpha(can_discard_alpha);
}

media::VideoCaptureFeedbackCB PushableMediaStreamVideoSource::GetFeedbackCallback() const
{
    return base::BindPostTask(GetTaskRunner(), WTF::BindRepeating(&PushableMediaStreamVideoSource::ProcessFeedbackInternal, weak_factory_.GetMutableWeakPtr()));
}

void PushableMediaStreamVideoSource::ProcessFeedbackInternal(const media::VideoCaptureFeedback& feedback)
{
