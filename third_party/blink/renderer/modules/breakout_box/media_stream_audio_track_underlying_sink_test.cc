...
namespace WTF {
template <> struct CrossThreadCopier<std::unique_ptr<blink::WritableStreamTransferringOptimizer>> {
...
public:
    MediaStreamAudioTrackUnderlyingSinkTest()
        : testing_thread_("TestingThread")
    {
        testing_thread_.Start();
        auto pushable_audio_source
            = std::make_unique<PushableMediaStreamAudioSource>(scheduler::GetSingleThreadTaskRunnerForTesting(), testing_thread_.task_runner());
        pushable_audio_source_ = pushable_audio_source.get();
        media_stream_source_ = MakeGarbageCollected<MediaStreamSource>("dummy_source_id", MediaStreamSource::kTypeAudio, "dummy_source_name",
    ...
    MediaStreamAudioTrackUnderlyingSink* CreateUnderlyingSink(ScriptState* script_state)
    {
        return MakeGarbageCollected<MediaStreamAudioTrackUnderlyingSink>(pushable_audio_source_->GetBroker());
    }

    void CreateTrackAndConnectToSource()
    {
        media_stream_component_ = MakeGarbageCollected<MediaStreamComponentImpl>(
            media_stream_source_->Id(), media_stream_source_, std::make_unique<MediaStreamAudioTrack>(true /* is_local_track */));
        pushable_audio_source_->ConnectToInitializedTrack(media_stream_component_);
    }

    ScriptValue CreateAudioData(ScriptState* script_state, AudioData** audio_data_out = nullptr)
    {
        const scoped_refptr<media::AudioBuffer> media_buffer = media::AudioBuffer::CreateEmptyBuffer(media::ChannelLayout::CHANNEL_LAYOUT_STEREO,
            /*channel_count=*/2,
            /*sample_rate=*/44100,
            /*frame_count=*/500, base::TimeDelta());
        AudioData* audio_data = MakeGarbageCollected<AudioData>(std::move(media_buffer));
        if (audio_data_out)
            *audio_data_out = audio_data;
        return ScriptValue(script_state->GetIsolate(), ToV8Traits<AudioData>::ToV8(script_state, audio_data));
    }

    static ScriptValue CreateInvalidAudioData(ScriptState* script_state, ExceptionState& exception_state)
    {
        ...
        init->setData(MakeGarbageCollected<AllowSharedBufferSource>(DOMArrayBuffer::Create(init->numberOfChannels() * init->numberOfFrames(), sizeof(float))));

        AudioData* audio_data = AudioData::Create(script_state, init, exception_state);
        return ScriptValue(script_state->GetIsolate(), ToV8Traits<AudioData>::ToV8(script_state, audio_data));
    }
...
TEST_F(MediaStreamAudioTrackUnderlyingSinkTest, WriteToStreamForwardsToMediaStreamSink)
{
    V8TestingScope v8_scope;
    ScriptState* script_state = v8_scope.GetScriptState();
    auto* underlying_sink = CreateUnderlyingSink(script_state);
    auto* writable_stream = WritableStream::CreateWithCountQueueingStrategy(script_state, underlying_sink, 1u);
    ...
    EXPECT_CALL(mock_sink, OnData(_, _)).WillOnce(base::test::RunOnceClosure(write_loop.QuitClosure()));

    WebMediaStreamAudioSink::AddToAudioTrack(&mock_sink, WebMediaStreamTrack(media_stream_component_.Get()));
    ...
    ScriptPromiseTester write_tester(script_state, writer->write(script_state, audio_data_chunk, exception_state));
    ...
    ScriptPromiseTester close_tester(script_state, writable_stream->close(script_state, exception_state));
    close_tester.WaitUntilSettled();

    // Writing to the sink after the stream closes should fail.
    DummyExceptionStateForTesting dummy_exception_state;
    underlying_sink->write(script_state, CreateAudioData(script_state), nullptr, dummy_exception_state);
    EXPECT_TRUE(dummy_exception_state.HadException());
    EXPECT_EQ(dummy_exception_state.Code(), static_cast<ExceptionCode>(DOMExceptionCode::kInvalidStateError));

    WebMediaStreamAudioSink::RemoveFromAudioTrack(&mock_sink, WebMediaStreamTrack(media_stream_component_.Get()));
}
...
TEST_F(MediaStreamAudioTrackUnderlyingSinkTest, WriteInvalidDataFails)
{
    ...
    // Writing a null value to the sink should fail.
    {
        ...
        sink->write(script_state, ScriptValue::CreateNull(v8_scope.GetIsolate()), nullptr, dummy_exception_state);
        EXPECT_TRUE(dummy_exception_state.HadException());
    }
...
TEST_F(MediaStreamAudioTrackUnderlyingSinkTest, WriteToAbortedSinkFails)
{
    ...
    NonThrowableExceptionState exception_state;
    ScriptPromiseTester abort_tester(script_state, writable_stream->abort(script_state, exception_state));
...
TEST_F(MediaStreamAudioTrackUnderlyingSinkTest, WriteInvalidAudioDataFails)
{
    ...
    sink->write(script_state, chunk, nullptr, dummy_exception_state);
    EXPECT_TRUE(dummy_exception_state.HadException());
    EXPECT_EQ(dummy_exception_state.Code(), static_cast<ExceptionCode>(DOMExceptionCode::kOperationError));
...
TEST_F(MediaStreamAudioTrackUnderlyingSinkTest, DeserializeWithOptimizer)
{
    ...
    auto* channel = MakeGarbageCollected<MessageChannel>(v8_scope.GetExecutionContext());
    writable_stream->Serialize(script_state, channel->port1(), ASSERT_NO_EXCEPTION);
    EXPECT_TRUE(writable_stream->IsLocked(writable_stream));

    // Deserialize the stream using the transfer optimizer.
    auto* transferred_stream = WritableStream::Deserialize(script_state, channel->port2(), std::move(transfer_optimizer), ASSERT_NO_EXCEPTION);
    EXPECT_TRUE(transferred_stream);
    EXPECT_TRUE(pushable_audio_source_->GetBroker()->ShouldDeliverAudioOnAudioTaskRunner());
}
...
TEST_F(MediaStreamAudioTrackUnderlyingSinkTest, TransferToWorkerWithOptimizer)
{
    ...
    worker_thread.StartWithSourceCode(v8_scope.GetWindow().GetSecurityOrigin(), "/* no worker script */");
    ...
    // while this occurs.
    scoped_refptr<base::SingleThreadTaskRunner> worker_task_runner = worker_thread.GetWorkerBackingThread().BackingThread().GetTaskRunner();
    PostCrossThreadTask(*worker_task_runner, FROM_HERE,
        CrossThreadBindOnce(
            [](WorkerThread* worker_thread, std::unique_ptr<WritableStreamTransferringOptimizer> transfer_optimizer) {
                ...
                ScriptState* worker_script_state = script_controller->GetScriptState();
                ScriptState::Scope worker_scope(worker_script_state);

                // Deserialize using the optimizer.
                auto* transferred_stream = WritableStream::Deserialize(worker_script_state, MakeGarbageCollected<MessageChannel>(worker_global_scope)->port2(),
                    std::move(transfer_optimizer), ASSERT_NO_EXCEPTION);
                EXPECT_TRUE(transferred_stream);
            },
            CrossThreadUnretained(&worker_thread), std::move(transfer_optimizer)));

    // Wait for another task on the worker to finish to ensure that the Oilpan
    // references held by the first task are dropped.
    base::WaitableEvent done;
    PostCrossThreadTask(*worker_task_runner, FROM_HERE, CrossThreadBindOnce(&base::WaitableEvent::Signal, CrossThreadUnretained(&done)));
    done.Wait();
    EXPECT_FALSE(pushable_audio_source_->GetBroker()->ShouldDeliverAudioOnAudioTaskRunner());
