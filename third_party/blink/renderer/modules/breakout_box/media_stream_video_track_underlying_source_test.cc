...
public:
    MediaStreamVideoTrackUnderlyingSourceTest()
        : pushable_video_source_(new PushableMediaStreamVideoSource(scheduler::GetSingleThreadTaskRunnerForTesting()))
        , media_stream_source_(MakeGarbageCollected<MediaStreamSource>(
              "dummy_source_id", MediaStreamSource::kTypeVideo, "dummy_source_name", false /* remote */, base::WrapUnique(pushable_video_source_.get())))
    {
    }
    ...
    MediaStreamTrack* CreateTrack(ExecutionContext* execution_context)
    {
        return MakeGarbageCollected<MediaStreamTrackImpl>(execution_context,
            MediaStreamVideoTrack::CreateVideoTrack(pushable_video_source_, MediaStreamVideoSource::ConstraintsOnceCallback(),
                /*enabled=*/true));
    }

    MediaStreamVideoTrackUnderlyingSource* CreateSource(ScriptState* script_state, MediaStreamTrack* track, wtf_size_t buffer_size)
    {
        return MakeGarbageCollected<MediaStreamVideoTrackUnderlyingSource>(script_state, track->Component(), nullptr, buffer_size);
    }

    MediaStreamVideoTrackUnderlyingSource* CreateSource(ScriptState* script_state, MediaStreamTrack* track)
    {
...
private:
    void RunIOUntilIdle() const
    {
        ...
        Platform::Current()->GetIOTaskRunner()->PostTaskAndReply(FROM_HERE, base::BindOnce([] {}), run_loop.QuitClosure());
        run_loop.Run();
        base::RunLoop().RunUntilIdle();
    }

protected:
    void PushFrame(const std::optional<base::TimeDelta>& timestamp = std::nullopt)
    {
        const scoped_refptr<media::VideoFrame> frame = media::VideoFrame::CreateBlackFrame(gfx::Size(10, 5));
        if (timestamp) {
    ...
    static MediaStreamSource* CreateDevicePushableSource(const std::string& device_id)
    {
        auto pushable_video_source = std::make_unique<PushableMediaStreamVideoSource>(scheduler::GetSingleThreadTaskRunnerForTesting());
        PushableMediaStreamVideoSource* pushable_video_source_ptr = pushable_video_source.get();
        auto* media_stream_source = MakeGarbageCollected<MediaStreamSource>(
            "dummy_source_id", MediaStreamSource::kTypeVideo, "dummy_source_name", false /* remote */, std::move(pushable_video_source));
        MediaStreamDevice device(mojom::MediaStreamType::DISPLAY_VIDEO_CAPTURE, device_id, "My window device");
...
TEST_F(MediaStreamVideoTrackUnderlyingSourceTest, VideoFrameFlowsThroughStreamAndCloses)
{
    V8TestingScope v8_scope;
    ScriptState* script_state = v8_scope.GetScriptState();
    auto* track = CreateTrack(v8_scope.GetExecutionContext());
    auto* source = CreateSource(script_state, track);
    auto* stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, source, 0);

    NonThrowableExceptionState exception_state;
    auto* reader = stream->GetDefaultReaderForTesting(script_state, exception_state);

    ScriptPromiseTester read_tester(script_state, reader->read(script_state, exception_state));
...
TEST_F(MediaStreamVideoTrackUnderlyingSourceTest, CancelStreamDisconnectsFromTrack)
{
    V8TestingScope v8_scope;
    MediaStreamTrack* track = CreateTrack(v8_scope.GetExecutionContext());
    MediaStreamVideoTrack* video_track = MediaStreamVideoTrack::From(track->Component());
    ...
    auto* stream = ReadableStream::CreateWithCountQueueingStrategy(v8_scope.GetScriptState(), source, 0);
...
TEST_F(MediaStreamVideoTrackUnderlyingSourceTest, DropOldFramesWhenQueueIsFull)
{
    V8TestingScope v8_scope;
    ScriptState* script_state = v8_scope.GetScriptState();
    auto* track = CreateTrack(v8_scope.GetExecutionContext());
    ...
    auto push_frame_sync = [&mock_sink, this](const base::TimeDelta timestamp) {
        ...
        EXPECT_CALL(mock_sink, OnVideoFrame(_)).WillOnce(base::test::RunOnceClosure(sink_loop.QuitClosure()));
        PushFrame(timestamp);
        sink_loop.Run();
    };
    ...
    for (wtf_size_t i = 1; i <= buffer_size; ++i) {
        VideoFrame* video_frame = ReadObjectFromStream<VideoFrame>(v8_scope, reader);
...
TEST_F(MediaStreamVideoTrackUnderlyingSourceTest, PlatformSourceAliveAfterGC)
{
    Persistent<MediaStreamComponent> component;
    {
        ...
        ReadableStream::CreateWithCountQueueingStrategy(v8_scope.GetScriptState(), source, 0);
...
TEST_F(MediaStreamVideoTrackUnderlyingSourceTest, DeviceIdAndMaxFrameCountForMonitoring)
{
    ...
    EXPECT_EQ(M::GetFramePoolSize(device),
        static_cast<size_t>(std::max(media::kVideoCaptureDefaultMaxBufferPoolSize / 2, media::DeviceVideoCaptureMaxBufferPoolSize() / 3)));
    ...
    device.id = screen_id;
    EXPECT_FALSE(M::GetDeviceIdForMonitoring(device).empty());
    EXPECT_EQ(M::GetFramePoolSize(device), static_cast<size_t>(media::kVideoCaptureDefaultMaxBufferPoolSize / 2));
    device.id = window_id;
    EXPECT_FALSE(M::GetDeviceIdForMonitoring(device).empty());
    EXPECT_EQ(M::GetFramePoolSize(device), static_cast<size_t>(media::kVideoCaptureDefaultMaxBufferPoolSize / 2));
...
TEST_F(MediaStreamVideoTrackUnderlyingSourceTest, FrameLimiter)
{
    ...
    auto* platform_video_source = static_cast<blink::PushableMediaStreamVideoSource*>(media_stream_source->GetPlatformSource());
    V8TestingScope v8_scope;
    ScriptState* script_state = v8_scope.GetScriptState();
    auto* track = MakeGarbageCollected<MediaStreamTrackImpl>(v8_scope.GetExecutionContext(),
        MediaStreamVideoTrack::CreateVideoTrack(platform_video_source, MediaStreamVideoSource::ConstraintsOnceCallback(),
            /*enabled=*/true));
    // Use a large buffer so that the effective buffer size is guaranteed to be
    // the one set by frame monitoring.
    auto* source = CreateSource(script_state, track, MediaStreamVideoTrackUnderlyingSource::kMaxMonitoredFrameCount);
    const wtf_size_t max_frame_count = MediaStreamVideoTrackUnderlyingSource::GetFramePoolSize(platform_video_source->device());
    ...
    auto push_frame_sync = [&](scoped_refptr<media::VideoFrame> video_frame) {
        base::RunLoop sink_loop;
        EXPECT_CALL(mock_sink, OnVideoFrame(_)).WillOnce(base::test::RunOnceClosure(sink_loop.QuitClosure()));
        platform_video_source->PushFrame(std::move(video_frame), base::TimeTicks::Now());
        sink_loop.Run();
    };
    ...
    auto* reader = stream->GetDefaultReaderForTesting(script_state, v8_scope.GetExceptionState());
    ...
    auto* platform_video_source2 = static_cast<blink::PushableMediaStreamVideoSource*>(media_stream_source2->GetPlatformSource());
    V8TestingScope v8_scope2;
    ScriptState* script_state2 = v8_scope2.GetScriptState();
    auto* track2 = MakeGarbageCollected<MediaStreamTrackImpl>(v8_scope2.GetExecutionContext(),
        MediaStreamVideoTrack::CreateVideoTrack(platform_video_source2, MediaStreamVideoSource::ConstraintsOnceCallback(),
            /*enabled=*/true));
    auto* source2 = CreateSource(script_state2, track2, MediaStreamVideoTrackUnderlyingSource::kMaxMonitoredFrameCount);
    ...
    auto push_frame_sync2 = [&](scoped_refptr<media::VideoFrame> video_frame) {
        base::RunLoop sink_loop;
        EXPECT_CALL(mock_sink2, OnVideoFrame(_)).WillOnce(base::test::RunOnceClosure(sink_loop.QuitClosure()));
        platform_video_source2->PushFrame(std::move(video_frame), base::TimeTicks::Now());
        sink_loop.Run();
    };
...
TEST_F(MediaStreamVideoTrackUnderlyingSourceTest, VideoFramePrefersCaptureTimestamp)
{
    ...
    scoped_refptr<media::VideoFrame> video_frame = media::VideoFrame::CreateBlackFrame(gfx::Size(10, 10));
    ...
    PushableMediaStreamVideoSource* pushable_source = static_cast<PushableMediaStreamVideoSource*>(track->Component()->Source()->GetPlatformSource());
    pushable_source->PushFrame(std::move(video_frame), base::TimeTicks::Now());

    VideoFrame* web_video_frame = ReadObjectFromStream<VideoFrame>(v8_scope, reader);
    ...
    video_frame->set_metadata(metadata);
    pushable_source->PushFrame(std::move(video_frame), base::TimeTicks::Now());
    VideoFrame* web_video_frame2 = ReadObjectFromStream<VideoFrame>(v8_scope, reader);
    EXPECT_EQ(web_video_frame2->timestamp(), kReferenceTimestamp.InMicroseconds());
    ...
    pushable_source->PushFrame(std::move(video_frame), base::TimeTicks::Now());
    VideoFrame* web_video_frame3 = ReadObjectFromStream<VideoFrame>(v8_scope, reader);

    if (base::FeatureList::IsEnabled(kBreakoutBoxInsertVideoCaptureTimestamp)) {
        scoped_refptr<media::VideoFrame> wrapped_video_frame3 = web_video_frame3->frame();
        ASSERT_TRUE(wrapped_video_frame3->metadata().capture_begin_time.has_value());
        EXPECT_EQ(web_video_frame3->timestamp(), (*wrapped_video_frame3->metadata().capture_begin_time - base::TimeTicks()).InMicroseconds());
        ASSERT_TRUE(wrapped_video_frame3->metadata().reference_time.has_value());
        EXPECT_EQ(web_video_frame3->timestamp(), (*wrapped_video_frame3->metadata().reference_time - base::TimeTicks()).InMicroseconds());
    }
