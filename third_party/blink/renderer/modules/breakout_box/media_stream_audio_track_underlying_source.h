...
class MODULES_EXPORT MediaStreamAudioTrackUnderlyingSource : public AudioDataQueueUnderlyingSource, public WebMediaStreamAudioSink {
    USING_PRE_FINALIZER(MediaStreamAudioTrackUnderlyingSource, DisconnectFromTrack);
...
public:
    ...
    public:
        ...
        virtual scoped_refptr<media::AudioBuffer> CopyIntoAudioBuffer(const media::AudioBus& audio_bus, base::TimeTicks capture_time) = 0;

        virtual int GetSizeForTesting() = 0;
    };

    explicit MediaStreamAudioTrackUnderlyingSource(ScriptState*, MediaStreamComponent*, ScriptWrappable* media_stream_track_processor, wtf_size_t queue_size);
    MediaStreamAudioTrackUnderlyingSource(const MediaStreamAudioTrackUnderlyingSource&) = delete;
    MediaStreamAudioTrackUnderlyingSource& operator=(const MediaStreamAudioTrackUnderlyingSource&) = delete;
    ...
    std::unique_ptr<ReadableStreamTransferringOptimizer> GetTransferringOptimizer();
