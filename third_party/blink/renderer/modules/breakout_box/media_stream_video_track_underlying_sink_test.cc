...
public:
    MediaStreamVideoTrackUnderlyingSinkTest()
    {
        ...
        media_stream_source_ = MakeGarbageCollected<MediaStreamSource>("dummy_source_id", MediaStreamSource::kTypeVideo, "dummy_source_name",
    ...
    MediaStreamVideoTrackUnderlyingSink* CreateUnderlyingSink(ScriptState* script_state)
    {
        return MakeGarbageCollected<MediaStreamVideoTrackUnderlyingSink>(pushable_video_source_->GetBroker());
    }

    WebMediaStreamTrack CreateTrack()
    {
        return MediaStreamVideoTrack::CreateVideoTrack(pushable_video_source_, MediaStreamVideoSource::ConstraintsOnceCallback(),
            /*enabled=*/true);
    }

    ScriptValue CreateVideoFrameChunk(ScriptState* script_state, VideoFrame** video_frame_out = nullptr, base::TimeDelta timestamp = base::Seconds(2))
    {
        const scoped_refptr<media::VideoFrame> media_frame = media::VideoFrame::CreateBlackFrame(gfx::Size(100, 50));
...
TEST_F(MediaStreamVideoTrackUnderlyingSinkTest, DISABLED_WriteToStreamForwardsToMediaStreamSink)
{
    ...
    ScriptPromiseTester write_tester(script_state, writer->write(script_state, video_frame_chunk, exception_state));
    ...
    underlying_sink->write(script_state, CreateVideoFrameChunk(script_state), nullptr, dummy_exception_state);
...
TEST_F(MediaStreamVideoTrackUnderlyingSinkTest, GetGmbManager)
{
    ...
    ScriptState* script_state = v8_scope.GetScriptState();
    auto* underlying_sink = CreateUnderlyingSink(script_state);
    EXPECT_EQ(!!underlying_sink->gmb_manager(), WebGraphicsContext3DVideoFramePool::IsGpuMemoryBufferReadbackFromTextureEnabled());
}

TEST_F(MediaStreamVideoTrackUnderlyingSinkTest, DeltaTimestampDoesNotWriteCaptureBeginTime)
{
    ...
    VideoFrame* video_frame = nullptr;
    ScriptValue video_frame_chunk = CreateVideoFrameChunk(script_state, &video_frame);
...
TEST_F(MediaStreamVideoTrackUnderlyingSinkTest, CaptureTimestampWritesCaptureBeginTime)
{
    if (IsTooEarlyForTest()) {
        return;
    }
    ...
    EXPECT_EQ((*media_frame->metadata().capture_begin_time - base::TimeTicks()).InMicroseconds(), web_exposed_timestamp);
}

TEST_F(MediaStreamVideoTrackUnderlyingSinkTest, DecisionToNotWriteCaptureTimeIsSticky)
{
    if (IsTooEarlyForTest()) {
        return;
    }
