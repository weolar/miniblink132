...
LockOrientationCallback::LockOrientationCallback(ScriptPromiseResolver<IDLUndefined>* resolver)
    : resolver_(resolver)
{
}
...
void LockOrientationCallback::OnSuccess()
{
    ...
    resolver_->GetExecutionContext()
        ...
        ->PostTask(FROM_HERE, WTF::BindOnce([](ScriptPromiseResolver<IDLUndefined>* resolver) { resolver->Resolve(); }, std::move(resolver_)));
}
...
void LockOrientationCallback::OnError(WebLockOrientationError error)
{
    ...
    case kWebLockOrientationErrorFullscreenRequired:
        ...
        message = "The page needs to be fullscreen in order to call "
                  "screen.orientation.lock().";
        break;
    case kWebLockOrientationErrorCanceled:
        code = DOMExceptionCode::kAbortError;
        message = "A call to screen.orientation.lock() or screen.orientation.unlock() "
                  "canceled this call.";
        break;
    }
