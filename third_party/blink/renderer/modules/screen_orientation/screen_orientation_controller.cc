...
const char ScreenOrientationController::kSupplementName[] = "ScreenOrientationController";

ScreenOrientationController* ScreenOrientationController::From(LocalDOMWindow& window)
{
...
ScreenOrientationController* ScreenOrientationController::FromIfExists(LocalDOMWindow& window)
{
    ...
    , screen_orientation_service_(&window)
{
    ...
    if (page && page->IsPrerendering()) {
        DomWindow()->document()->AddPostPrerenderingActivationStep(WTF::BindOnce(&ScreenOrientationController::BuildMojoConnection, WrapWeakPersistent(this)));
        return;
    }
    BuildMojoConnection();
}
...
// width / height.
display::mojom::blink::ScreenOrientation ScreenOrientationController::ComputeOrientation(const gfx::Rect& rect, uint16_t rotation)
{
    ...
    bool is_tall_display = rotation % 180 ? rect.height() < rect.width() : rect.height() > rect.width();
    ...
    switch (rotation) {
    case 0:
        return is_tall_display ? display::mojom::blink::ScreenOrientation::kPortraitPrimary : display::mojom::blink::ScreenOrientation::kLandscapePrimary;
    case 90:
        return is_tall_display ? display::mojom::blink::ScreenOrientation::kLandscapePrimary : display::mojom::blink::ScreenOrientation::kPortraitSecondary;
    case 180:
        return is_tall_display ? display::mojom::blink::ScreenOrientation::kPortraitSecondary : display::mojom::blink::ScreenOrientation::kLandscapeSecondary;
    case 270:
        return is_tall_display ? display::mojom::blink::ScreenOrientation::kLandscapeSecondary : display::mojom::blink::ScreenOrientation::kPortraitPrimary;
    default:
        NOTREACHED();
    }
}
...
void ScreenOrientationController::UpdateOrientation()
{
    ...
    display::mojom::blink::ScreenOrientation orientation_type = screen_info.orientation_type;
    if (orientation_type == display::mojom::blink::ScreenOrientation::kUndefined) {
        // The embedder could not provide us with an orientation, deduce it
        // ourselves.
        orientation_type = ComputeOrientation(screen_info.rect, screen_info.orientation_angle);
    }
    DCHECK(orientation_type != display::mojom::blink::ScreenOrientation::kUndefined);
...
void ScreenOrientationController::BuildMojoConnection()
{
    ...
    if (!DomWindow() || !DomWindow()->document())
        return;
    AssociatedInterfaceProvider* provider = DomWindow()->GetFrame()->GetRemoteNavigationAssociatedInterfaces();
    if (provider) {
        provider->GetInterface(screen_orientation_service_.BindNewEndpointAndPassReceiver(DomWindow()->GetTaskRunner(TaskType::kMiscPlatformAPI)));
    }
}
...
void ScreenOrientationController::PageVisibilityChanged()
{
    ...
    LocalFrame& frame = *DomWindow()->GetFrame();
    uint16_t current_angle = GetPage()->GetChromeClient().GetScreenInfo(frame).orientation_angle;
    ...
    // able to remove this.
    if (&frame == frame.LocalFrameRoot() && orientation_->angle() != current_angle)
...
void ScreenOrientationController::NotifyOrientationChanged()
{
    ...
    for (Frame* frame = DomWindow()->GetFrame(); frame; frame = frame->Tree().TraverseNext(DomWindow()->GetFrame())) {
        ...
        ->PostTask(FROM_HERE,
            WTF::BindOnce(
                [](ScreenOrientation* orientation) {
                    ScopedAllowFullscreen allow_fullscreen(ScopedAllowFullscreen::kOrientationChange);
                    orientation->DispatchEvent(*Event::Create(event_type_names::kChange));
                },
...
void ScreenOrientationController::lock(device::mojom::blink::ScreenOrientationLockType orientation, std::unique_ptr<WebLockOrientationCallback> callback)
{
    ...
    if (DomWindow()->document()->IsPrerendering()) {
        DomWindow()->document()->AddPostPrerenderingActivationStep(
            WTF::BindOnce(&ScreenOrientationController::LockOrientationInternal, WrapWeakPersistent(this), orientation, std::move(callback)));
        return;
    }
...
void ScreenOrientationController::unlock()
{
    ...
    if (DomWindow()->document()->IsPrerendering()) {
        DomWindow()->document()->AddPostPrerenderingActivationStep(
            WTF::BindOnce(&ScreenOrientationController::UnlockOrientationInternal, WrapWeakPersistent(this)));
        return;
    }
...
void ScreenOrientationController::SetScreenOrientationAssociatedRemoteForTests(HeapMojoAssociatedRemote<device::mojom::blink::ScreenOrientation> remote)
{
    screen_orientation_service_ = std::move(remote);
}

void ScreenOrientationController::OnLockOrientationResult(int request_id, ScreenOrientationLockResult result)
{
    if (!pending_callback_ || request_id != request_id_)
        return;

    if (IdentifiabilityStudySettings::Get()->ShouldSampleSurface(
            IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kWebFeature, WebFeature::kScreenOrientationLock))) {
        auto* context = GetExecutionContext();
        IdentifiabilityMetricBuilder(context->UkmSourceID())
            .AddWebFeature(WebFeature::kScreenOrientationLock, result == ScreenOrientationLockResult::SCREEN_ORIENTATION_LOCK_RESULT_SUCCESS)
...
void ScreenOrientationController::LockOrientationInternal(
    device::mojom::blink::ScreenOrientationLockType orientation, std::unique_ptr<WebLockOrientationCallback> callback)
{
    ...
    screen_orientation_service_->LockOrientation(
        orientation, WTF::BindOnce(&ScreenOrientationController::OnLockOrientationResult, WrapWeakPersistent(this), ++request_id_));
