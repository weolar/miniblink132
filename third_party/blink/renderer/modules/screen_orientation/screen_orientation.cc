...
V8OrientationType::Enum ScreenOrientation::OrientationTypeToV8Enum(display::mojom::blink::ScreenOrientation orientation)
{
    switch (orientation) {
...
static device::mojom::blink::ScreenOrientationLockType V8EnumToOrientationLock(V8OrientationLockType::Enum orientation_lock)
{
    ...
    case V8OrientationLockType::Enum::kPortraitSecondary:
        return device::mojom::blink::ScreenOrientationLockType::PORTRAIT_SECONDARY;
    ...
    case V8OrientationLockType::Enum::kLandscapeSecondary:
        return device::mojom::blink::ScreenOrientationLockType::LANDSCAPE_SECONDARY;
...
{
    DCHECK(window);
    ScreenOrientation* orientation = MakeGarbageCollected<ScreenOrientation>(window);
...
ScriptPromise<IDLUndefined> ScreenOrientation::lock(ScriptState* state, const V8OrientationLockType& orientation, ExceptionState& exception_state)
{
    if (!state->ContextIsValid() || !Controller()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The object is no longer associated to a window.");
        return EmptyPromise();
    }

    if (GetExecutionContext()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kOrientationLock)) {
        exception_state.ThrowSecurityError(To<LocalDOMWindow>(GetExecutionContext())->GetFrame()->IsInFencedFrameTree()
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(state);
    auto promise = resolver->Promise();
    Controller()->lock(V8EnumToOrientationLock(orientation.AsEnum()), std::make_unique<LockOrientationCallback>(resolver));
    return promise;
}
...
ScreenOrientationController* ScreenOrientation::Controller()
{
    ...
    return ScreenOrientationController::From(*To<LocalDOMWindow>(GetExecutionContext()));
}
