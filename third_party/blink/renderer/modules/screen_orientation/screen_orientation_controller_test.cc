...
using LockOrientationCallback = device::mojom::blink::ScreenOrientation::LockOrientationCallback;
...
protected:
    void SetUp() override
    {
        ...
        HeapMojoAssociatedRemote<device::mojom::blink::ScreenOrientation> screen_orientation(GetFrame().DomWindow());
        std::ignore = screen_orientation.BindNewEndpointAndPassDedicatedReceiver();
        Controller()->SetScreenOrientationAssociatedRemoteForTests(std::move(screen_orientation));
    }
    ...
    void LockOrientation(device::mojom::ScreenOrientationLockType orientation, std::unique_ptr<blink::WebLockOrientationCallback> callback)
    {
    ...
    int GetRequestId()
    {
        return Controller()->GetRequestIdForTests();
    }
...
TEST_F(ScreenOrientationControllerTest, CancelPending_Unlocking)
{
    ...
    LockOrientation(device::mojom::ScreenOrientationLockType::PORTRAIT_PRIMARY, std::make_unique<MockLockOrientationCallback>(&callback_results));
    UnlockOrientation();
...
TEST_F(ScreenOrientationControllerTest, CancelPending_DoubleLock)
{
    ...
    LockOrientation(device::mojom::ScreenOrientationLockType::PORTRAIT_PRIMARY, std::make_unique<MockLockOrientationCallback>(&callback_results2));
...
TEST_F(ScreenOrientationControllerTest, LockRequest_Error)
{
    ...
    errors.insert(LockResult::SCREEN_ORIENTATION_LOCK_RESULT_ERROR_NOT_AVAILABLE, blink::kWebLockOrientationErrorNotAvailable);
    errors.insert(LockResult::SCREEN_ORIENTATION_LOCK_RESULT_ERROR_FULLSCREEN_REQUIRED, blink::kWebLockOrientationErrorFullscreenRequired);
    errors.insert(LockResult::SCREEN_ORIENTATION_LOCK_RESULT_ERROR_CANCELED, blink::kWebLockOrientationErrorCanceled);
...
TEST_F(ScreenOrientationControllerTest, LockRequest_Success)
{
    ...
    RunLockResultCallback(GetRequestId(), LockResult::SCREEN_ORIENTATION_LOCK_RESULT_SUCCESS);
...
TEST_F(ScreenOrientationControllerTest, RaceScenario)
{
    ...
    LockOrientation(device::mojom::ScreenOrientationLockType::PORTRAIT_PRIMARY, std::make_unique<MockLockOrientationCallback>(&callback_results1));
    int request_id1 = GetRequestId();

    LockOrientation(device::mojom::ScreenOrientationLockType::LANDSCAPE_PRIMARY, std::make_unique<MockLockOrientationCallback>(&callback_results2));

    // callback_results1 must be rejected, tested in CancelPending_DoubleLock.

    RunLockResultCallback(request_id1, LockResult::SCREEN_ORIENTATION_LOCK_RESULT_SUCCESS);
...
TEST_F(ScreenOrientationControllerTest, PageVisibilityCrash)
{
    ...
    url_test_helpers::RegisterMockedURLLoadFromBase(WebString::FromUTF8(base_url), test::CoreTestDataPath(), WebString::FromUTF8(test_url));
    ...
    frame_test_helpers::CreateTestWebFrameWidgetCallback create_widget_callback
        = WTF::BindRepeating(&frame_test_helpers::WebViewHelper::CreateTestWebFrameWidget<ScreenInfoWebFrameWidget>);
    ...
    auto* child_orientation = ScreenOrientation::Create(To<LocalFrame>(frame->Tree().FirstChild())->DomWindow());
...
TEST_F(ScreenOrientationControllerTest, OrientationChangePropagationToGrandchild)
{
    std::string base_url("http://internal.test/");
    ...
    auto* grandchild_orientation = ScreenOrientation::Create(To<LocalFrame>(grandchild)->DomWindow());
    ...
    auto* web_frame_widget_base = static_cast<WebFrameWidgetImpl*>(frame->GetWidgetForLocalRoot());
