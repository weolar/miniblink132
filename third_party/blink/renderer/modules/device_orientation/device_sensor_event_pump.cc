...
void DeviceSensorEventPump::Stop()
{
    ...
    DCHECK((state_ == PumpState::kPendingStart && !timer_.IsActive()) || (state_ == PumpState::kRunning && timer_.IsActive()));
...
void DeviceSensorEventPump::SetSensorProviderForTesting(mojo::PendingRemote<mojom::blink::WebSensorProvider> sensor_provider)
{
...
DeviceSensorEventPump::PumpState DeviceSensorEventPump::GetPumpStateForTesting()
{
    return state_;
}
...
DeviceSensorEventPump::DeviceSensorEventPump(LocalFrame& frame)
    ...
    , timer_(frame.GetTaskRunner(TaskType::kSensor), this, &DeviceSensorEventPump::FireEvent)
{
}
...
void DeviceSensorEventPump::DidStartIfPossible()
{
    ...
    timer_.StartRepeating(base::Microseconds(kDefaultPumpDelayMicroseconds), FROM_HERE);
