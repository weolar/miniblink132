...
class MockDeviceMotionController final : public GarbageCollected<MockDeviceMotionController>, public PlatformEventController {
public:
    explicit MockDeviceMotionController(DeviceMotionEventPump* motion_pump, LocalDOMWindow& window)
    ...
    MockDeviceMotionController& operator=(const MockDeviceMotionController&) = delete;
    ...
    {
        motion_pump_->SetController(this);
    }
    ...
    DeviceMotionEventPump* motion_pump()
    {
        return motion_pump_.Get();
    }
...
public:
    ...
    DeviceMotionEventPumpTest& operator=(const DeviceMotionEventPumpTest&) = delete;
...
protected:
    void SetUp() override
    {
        ...
        auto* motion_pump = MakeGarbageCollected<DeviceMotionEventPump>(page_holder_->GetFrame());
        motion_pump->SetSensorProviderForTesting(ToCrossVariantMojoType(std::move(sensor_provider)));

        controller_ = MakeGarbageCollected<MockDeviceMotionController>(motion_pump, *page_holder_->GetFrame().DomWindow());

        ExpectAllThreeSensorsStateToBe(DeviceSensorEntry::State::kNotInitialized);
        EXPECT_EQ(DeviceMotionEventPump::PumpState::kStopped, controller_->motion_pump()->GetPumpStateForTesting());
    }

    void FireEvent()
    {
        controller_->motion_pump()->FireEvent(nullptr);
    }

    void ExpectAccelerometerStateToBe(DeviceSensorEntry::State expected_sensor_state)
    {
        EXPECT_EQ(expected_sensor_state, controller_->motion_pump()->accelerometer_->state());
    }

    void ExpectLinearAccelerationSensorStateToBe(DeviceSensorEntry::State expected_sensor_state)
    {
        EXPECT_EQ(expected_sensor_state, controller_->motion_pump()->linear_acceleration_sensor_->state());
    }

    void ExpectGyroscopeStateToBe(DeviceSensorEntry::State expected_sensor_state)
    {
        EXPECT_EQ(expected_sensor_state, controller_->motion_pump()->gyroscope_->state());
    }

    void ExpectAllThreeSensorsStateToBe(DeviceSensorEntry::State expected_sensor_state)
    {
    ...
    MockDeviceMotionController* controller()
    {
...
TEST_F(DeviceMotionEventPumpTest, AllSensorsAreActive)
{
    ...
    EXPECT_TRUE(received_data->GetAccelerationIncludingGravity()->HasAccelerationData());
    ...
    EXPECT_EQ(base::RadToDeg(7.0), received_data->GetRotationRate()->alpha().value());
    EXPECT_EQ(base::RadToDeg(8.0), received_data->GetRotationRate()->beta().value());
    EXPECT_EQ(base::RadToDeg(9.0), received_data->GetRotationRate()->gamma().value());
...
TEST_F(DeviceMotionEventPumpTest, TwoSensorsAreActive)
{
    ...
    ExpectLinearAccelerationSensorStateToBe(DeviceSensorEntry::State::kNotInitialized);
...
TEST_F(DeviceMotionEventPumpTest, SomeSensorDataFieldsNotAvailable)
{
    ...
    EXPECT_FALSE(received_data->GetAccelerationIncludingGravity()->x().has_value());
    EXPECT_EQ(2, received_data->GetAccelerationIncludingGravity()->y().value());
    EXPECT_EQ(3, received_data->GetAccelerationIncludingGravity()->z().value());
...
TEST_F(DeviceMotionEventPumpTest, FireAllNullEvent)
{
    ...
    EXPECT_FALSE(received_data->GetAccelerationIncludingGravity()->HasAccelerationData());
...
TEST_F(DeviceMotionEventPumpTest, NotFireEventWhenSensorReadingTimeStampIsZero)
{
    controller()->RegisterWithDispatcher();
    base::RunLoop().RunUntilIdle();
...
TEST_F(DeviceMotionEventPumpTest, PumpThrottlesEventRate)
{
    ...
    EXPECT_GE(60, base::Time::kMicrosecondsPerSecond / DeviceMotionEventPump::kDefaultPumpDelayMicroseconds);
    ...
    base::RunLoop loop;
    blink::scheduler::GetSingleThreadTaskRunnerForTesting()->PostDelayedTask(FROM_HERE, loop.QuitWhenIdleClosure(), base::Milliseconds(100));
    loop.Run();
    controller()->UnregisterWithDispatcher();
