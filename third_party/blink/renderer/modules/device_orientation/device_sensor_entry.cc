...
DeviceSensorEntry::DeviceSensorEntry(DeviceSensorEventPump* event_pump, ExecutionContext* context, device::mojom::blink::SensorType type)
...
void DeviceSensorEntry::Start(mojom::blink::WebSensorProvider* sensor_provider)
{
    ...
    if (!sensor_remote_.is_bound() && state_ != State::kInitializing && state_ != State::kShouldSuspend) {
        state_ = State::kNotInitialized;
    }

    if (state_ == State::kNotInitialized) {
        state_ = State::kInitializing;
        sensor_provider->GetSensor(type_, WTF::BindOnce(&DeviceSensorEntry::OnSensorCreated, WrapWeakPersistent(this)));
...
void DeviceSensorEntry::OnSensorCreated(device::mojom::blink::SensorCreationResult result, device::mojom::blink::SensorInitParamsPtr params)
{
    ...
    client_receiver_.Bind(std::move(params->client_receiver), event_pump_->task_runner_);
    ...
    device::mojom::blink::SensorConfigurationPtr config = std::move(params->default_configuration);
    config->frequency = std::min(static_cast<double>(DeviceSensorEventPump::kDefaultPumpFrequencyHz), params->maximum_frequency);

    sensor_remote_.set_disconnect_handler(WTF::BindOnce(&DeviceSensorEntry::HandleSensorError, WrapWeakPersistent(this)));
    sensor_remote_->ConfigureReadingChangeNotifications(/*enabled=*/false);
    sensor_remote_->AddConfiguration(std::move(config), WTF::BindOnce(&DeviceSensorEntry::OnSensorAddConfiguration, WrapWeakPersistent(this)));
}
