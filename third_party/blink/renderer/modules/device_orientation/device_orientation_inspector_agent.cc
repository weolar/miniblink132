...
constexpr char kInspectorConsoleMessage[] = "A reload is required so that the existing AbsoluteOrientationSensor "
...
DeviceOrientationInspectorAgent::DeviceOrientationInspectorAgent(InspectedFrames* inspected_frames)
...
{
    return DeviceOrientationController::From(*inspected_frames_->Root()->DomWindow());
}

protocol::Response DeviceOrientationInspectorAgent::setDeviceOrientationOverride(double alpha, double beta, double gamma)
{
    if (!enabled_.Get()) {
        ...
        inspected_frames_->Root()->DomWindow()->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kInfo, kInspectorConsoleMessage));
    }
    enabled_.Set(true);
    return protocol::Response::Success();
}

protocol::Response DeviceOrientationInspectorAgent::clearDeviceOrientationOverride()
{
    return disable();
}
