...
constexpr double kDefaultPumpDelayMilliseconds = blink::DeviceMotionEventPump::kDefaultPumpDelayMicroseconds / 1000;
...
{
    accelerometer_ = MakeGarbageCollected<DeviceSensorEntry>(this, frame.DomWindow(), device::mojom::blink::SensorType::ACCELEROMETER);
    linear_acceleration_sensor_ = MakeGarbageCollected<DeviceSensorEntry>(this, frame.DomWindow(), device::mojom::blink::SensorType::LINEAR_ACCELERATION);
    gyroscope_ = MakeGarbageCollected<DeviceSensorEntry>(this, frame.DomWindow(), device::mojom::blink::SensorType::GYROSCOPE);
}
...
{
    return accelerometer_->ReadyOrErrored() && linear_acceleration_sensor_->ReadyOrErrored() && gyroscope_->ReadyOrErrored();
}
...
DeviceMotionData* DeviceMotionEventPump::GetDataFromSharedMemory()
{
    ...
    if (accelerometer_->GetReading(&accelerometer_reading)) {
        ...
        acceleration_including_gravity
            = DeviceMotionEventAcceleration::Create(accelerometer_reading.accel.x, accelerometer_reading.accel.y, accelerometer_reading.accel.z);
    } else {
        acceleration_including_gravity = DeviceMotionEventAcceleration::Create(NAN, NAN, NAN);
    }

    device::SensorReading linear_acceleration_sensor_reading;
    if (linear_acceleration_sensor_->GetReading(&linear_acceleration_sensor_reading)) {
        if (linear_acceleration_sensor_reading.timestamp() == 0.0)
            return nullptr;

        acceleration = DeviceMotionEventAcceleration::Create(
            linear_acceleration_sensor_reading.accel.x, linear_acceleration_sensor_reading.accel.y, linear_acceleration_sensor_reading.accel.z);
    } else {
    ...
    if (gyroscope_->GetReading(&gyroscope_reading)) {
        ...
        rotation_rate = DeviceMotionEventRotationRate::Create(base::RadToDeg(gyroscope_reading.gyro.x.value()),
            base::RadToDeg(gyroscope_reading.gyro.y.value()), base::RadToDeg(gyroscope_reading.gyro.z.value()));
    } else {
    ...
    return DeviceMotionData::Create(acceleration, acceleration_including_gravity, rotation_rate, kDefaultPumpDelayMilliseconds);
}
