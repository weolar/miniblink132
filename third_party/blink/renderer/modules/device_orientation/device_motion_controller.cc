...
{
    DeviceMotionController* controller = Supplement<LocalDOMWindow>::From<DeviceMotionController>(window);
    if (!controller) {
...
void DeviceMotionController::DidAddEventListener(LocalDOMWindow* window, const AtomicString& event_type)
{
    ...
    if (!has_requested_permission_) {
        UseCounter::Count(GetWindow(), WebFeature::kDeviceMotionUsedWithoutPermissionRequest);
    }
    ...
    if (!has_event_listener_) {
        if (!CheckPolicyFeatures({ mojom::blink::PermissionsPolicyFeature::kAccelerometer, mojom::blink::PermissionsPolicyFeature::kGyroscope })) {
            DeviceOrientationController::LogToConsolePolicyFeaturesDisabled(*GetWindow().GetFrame(), EventTypeName());
            return;
        }
    }
...
{
    return motion_event_pump_ ? motion_event_pump_->LatestDeviceMotionData() != nullptr : false;
}

void DeviceMotionController::RegisterWithDispatcher()
{
    if (!motion_event_pump_) {
        motion_event_pump_ = MakeGarbageCollected<DeviceMotionEventPump>(*GetWindow().GetFrame());
    }
...
{
    return DeviceMotionEvent::Create(event_type_names::kDevicemotion, motion_event_pump_ ? motion_event_pump_->LatestDeviceMotionData() : nullptr);
}
...
ScriptPromise<V8DeviceOrientationPermissionState> DeviceMotionController::RequestPermission(ScriptState* script_state)
{
