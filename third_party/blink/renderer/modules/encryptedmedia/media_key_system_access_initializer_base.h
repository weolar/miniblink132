...
class MediaKeySystemAccessInitializerBase : public EncryptedMediaRequest, public ExecutionContextClient {
public:
    MediaKeySystemAccessInitializerBase(ExecutionContext*, ScriptPromiseResolverBase*, const String& key_system,
        const HeapVector<Member<MediaKeySystemConfiguration>>& supported_configurations, bool is_from_media_capabilities);

    MediaKeySystemAccessInitializerBase(const MediaKeySystemAccessInitializerBase&) = delete;
    MediaKeySystemAccessInitializerBase& operator=(const MediaKeySystemAccessInitializerBase&) = delete;

    ~MediaKeySystemAccessInitializerBase() override = default;

    // EncryptedMediaRequest implementation.
    WebString KeySystem() const override
    {
        return key_system_;
    }
    const WebVector<WebMediaKeySystemConfiguration>& SupportedConfigurations() const override
    {
