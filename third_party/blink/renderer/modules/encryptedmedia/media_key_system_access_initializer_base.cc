...
static WebVector<media::EmeInitDataType> ConvertInitDataTypes(const Vector<String>& init_data_types)
{
...
static WebMediaKeySystemMediaCapability::EncryptionScheme ConvertEncryptionScheme(const String& encryption_scheme)
{
...
static WebVector<WebMediaKeySystemMediaCapability> ConvertCapabilities(const HeapVector<Member<MediaKeySystemMediaCapability>>& capabilities)
{
    ...
    for (wtf_size_t i = 0; i < capabilities.size(); ++i) {
        ...
        result[i].encryption_scheme = (capabilities[i]->hasEncryptionScheme() && !capabilities[i]->encryptionScheme().IsNull())
...
static WebVector<WebEncryptedMediaSessionType> ConvertSessionTypes(const Vector<String>& session_types)
{
...
MediaKeySystemAccessInitializerBase::MediaKeySystemAccessInitializerBase(ExecutionContext* context, ScriptPromiseResolverBase* resolver,
    const String& key_system, const HeapVector<Member<MediaKeySystemConfiguration>>& supported_configurations, bool is_from_media_capabilities)
    ...
    , is_from_media_capabilities_(is_from_media_capabilities)
{
    for (wtf_size_t i = 0; i < supported_configurations.size(); ++i) {
        ...
        web_config.audio_capabilities = ConvertCapabilities(config->audioCapabilities());

        DCHECK(config->hasVideoCapabilities());
        web_config.video_capabilities = ConvertCapabilities(config->videoCapabilities());

        DCHECK(config->hasDistinctiveIdentifier());
        web_config.distinctive_identifier = EncryptedMediaUtils::ConvertToMediaKeysRequirement(config->distinctiveIdentifier().AsEnum());

        DCHECK(config->hasPersistentState());
        web_config.persistent_state = EncryptedMediaUtils::ConvertToMediaKeysRequirement(config->persistentState().AsEnum());
        ...
        if (config->hasSessionTypes()) {
            ...
            WebVector<WebEncryptedMediaSessionType> session_types(static_cast<size_t>(1));
...
const SecurityOrigin* MediaKeySystemAccessInitializerBase::GetSecurityOrigin() const
{
    return IsExecutionContextValid() ? GetExecutionContext()->GetSecurityOrigin() : nullptr;
}
...
void MediaKeySystemAccessInitializerBase::GenerateWarningAndReportMetrics() const
{
    ...
    if (has_video_capabilities) {
        base::UmaHistogramBoolean("Media.EME.Widevine.VideoCapability.HasEmptyRobustness", has_empty_robustness);
    }
    ...
    ukm::builders::Media_EME_RequestMediaKeySystemAccess builder(DomWindow()->UkmSourceID());
    ...
    builder.SetVideoCapabilities_HasEmptyRobustness(static_cast<int>(has_empty_robustness));
    builder.SetVideoCapabilities_HasHwSecureAllRobustness(static_cast<int>(has_hw_secure_all));
    builder.SetIsFromMediaCapabilities(static_cast<int>(is_from_media_capabilities_));
