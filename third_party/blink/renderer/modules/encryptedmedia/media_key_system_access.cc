...
public:
    NewCdmResultPromise(
        ScriptPromiseResolver<MediaKeys>* resolver, const MediaKeysConfig& config, const WebVector<WebEncryptedMediaSessionType>& supported_session_types)
        : ContentDecryptionModuleResultPromise(resolver, config, EmeApiType::kCreateMediaKeys)
        , config_(config)
    ...
    void CompleteWithContentDecryptionModule(std::unique_ptr<WebContentDecryptionModule> cdm) override
    {
        ...
        auto* media_keys = MakeGarbageCollected<MediaKeys>(GetExecutionContext(), supported_session_types_, std::move(cdm), config_);
...
Vector<String> ConvertInitDataTypes(const WebVector<media::EmeInitDataType>& init_data_types)
{
    Vector<String> result(base::checked_cast<wtf_size_t>(init_data_types.size()));
    for (wtf_size_t i = 0; i < result.size(); i++)
        result[i] = EncryptedMediaUtils::ConvertFromInitDataType(init_data_types[i]);
    return result;
}

HeapVector<Member<MediaKeySystemMediaCapability>> ConvertCapabilities(const WebVector<WebMediaKeySystemMediaCapability>& capabilities)
{
    HeapVector<Member<MediaKeySystemMediaCapability>> result(base::checked_cast<wtf_size_t>(capabilities.size()));
    for (wtf_size_t i = 0; i < result.size(); i++) {
        MediaKeySystemMediaCapability* capability = MediaKeySystemMediaCapability::Create();
        ...
        case WebMediaKeySystemMediaCapability::EncryptionScheme::kUnrecognized:
            NOTREACHED() << "Unrecognized encryption scheme should never be returned.";
        }
...
Vector<String> ConvertSessionTypes(const WebVector<WebEncryptedMediaSessionType>& session_types)
{
...
void ReportMetrics(ExecutionContext* execution_context, const String& key_system)
{
    ...
    builder.SetIsCrossOrigin(static_cast<int>(frame->IsCrossOriginToOutermostMainFrame()));
...
MediaKeySystemAccess::MediaKeySystemAccess(std::unique_ptr<WebContentDecryptionModuleAccess> access)
...
MediaKeySystemConfiguration* MediaKeySystemAccess::getConfiguration() const
{
    ...
    if (!configuration.init_data_types.empty())
        result->setInitDataTypes(ConvertInitDataTypes(configuration.init_data_types));
    if (!configuration.audio_capabilities.empty())
        result->setAudioCapabilities(ConvertCapabilities(configuration.audio_capabilities));
    if (!configuration.video_capabilities.empty())
        result->setVideoCapabilities(ConvertCapabilities(configuration.video_capabilities));

    // |distinctiveIdentifier|, |persistentState|, and |sessionTypes| are always
    // set by requestMediaKeySystemAccess().
    result->setDistinctiveIdentifier(EncryptedMediaUtils::ConvertMediaKeysRequirementToEnum(configuration.distinctive_identifier));
    result->setPersistentState(EncryptedMediaUtils::ConvertMediaKeysRequirementToEnum(configuration.persistent_state));
...
ScriptPromise<MediaKeys> MediaKeySystemAccess::createMediaKeys(ScriptState* script_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<MediaKeys>>(script_state);
    NewCdmResultPromise* helper = MakeGarbageCollected<NewCdmResultPromise>(resolver, config, configuration.session_types);
    ...
    access_->CreateContentDecryptionModule(helper->Result(), execution_context->GetTaskRunner(TaskType::kInternalMedia));
