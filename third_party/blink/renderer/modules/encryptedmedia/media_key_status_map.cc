...
class MediaKeyStatusMap::MapEntry final : public GarbageCollected<MediaKeyStatusMap::MapEntry> {
public:
    ...
    static bool CompareLessThan(MapEntry* a, MapEntry* b)
    {
        ...
        // Compare the bytes.
        int result = memcmp(a->KeyId()->Data(), b->KeyId()->Data(), std::min(a->KeyId()->ByteLength(), b->KeyId()->ByteLength()));
        if (result != 0)
            return result < 0;
...
class MapIterationSource final : public PairSyncIterable<MediaKeyStatusMap>::IterationSource {
public:
    ...
    bool FetchNextItem(ScriptState* script_state, V8BufferSource*& key, V8MediaKeyStatus& value, ExceptionState&) override
    {
...
void MediaKeyStatusMap::AddEntry(WebData key_id, const String& status)
{
    ...
    uint32_t index = 0;
    while (index < entries_.size() && MapEntry::CompareLessThan(entries_[index], entry))
        ++index;
...
bool MediaKeyStatusMap::has(const V8BufferSource* key_id)
{
    uint32_t index = IndexOf(key_id);
    return index < entries_.size();
}

ScriptValue MediaKeyStatusMap::get(ScriptState* script_state, const V8BufferSource* key_id)
{
    uint32_t index = IndexOf(key_id);
...
MediaKeyStatusMap::IterationSource* MediaKeyStatusMap::CreateIterationSource(ScriptState*, ExceptionState&)
{
