...
public:
    static ScriptPromise<IDLUndefined> Create(ScriptState*, HTMLMediaElement&, MediaKeys*, const ExceptionContext&);

    SetMediaKeysHandler(ScriptState*, HTMLMediaElement&, MediaKeys*, const ExceptionContext&);
...
private:
    ...
    void ClearFailed(WebContentDecryptionModuleException, const String& error_message);
    void SetFailed(WebContentDecryptionModuleException, const String& error_message);
...
public:
    ...
    void CompleteWithContentDecryptionModule(std::unique_ptr<WebContentDecryptionModule>) override
    {
        NOTREACHED();
    }
    ...
    void CompleteWithError(WebContentDecryptionModuleException code, uint32_t system_code, const WebString& message) override
    {
        ...
        DVLOG(EME_LOG_LEVEL) << __func__ << ": promise rejected with code " << code << " and message: " << result.ToString();
...
ScriptPromise<IDLUndefined> SetMediaKeysHandler::Create(
    ScriptState* script_state, HTMLMediaElement& element, MediaKeys* media_keys, const ExceptionContext& exception_context)
{
    SetMediaKeysHandler* handler = MakeGarbageCollected<SetMediaKeysHandler>(script_state, element, media_keys, exception_context);
    return handler->resolver_->Promise();
}

SetMediaKeysHandler::SetMediaKeysHandler(ScriptState* script_state, HTMLMediaElement& element, MediaKeys* media_keys, const ExceptionContext& exception_context)
    ...
    , keep_alive_(this)
    , timer_(ExecutionContext::From(script_state)->GetTaskRunner(TaskType::kMiscPlatformAPI), this, &SetMediaKeysHandler::TimerFired)
{
...
{
    DVLOG(EME_LOG_LEVEL) << __func__;
    HTMLMediaElementEncryptedMedia& this_element = HTMLMediaElementEncryptedMedia::From(*element_);
    ...
    if (new_media_keys_) {
        if (!new_media_keys_->ReserveForMediaElement(element_.Get())) {
            ...
            Fail(kWebContentDecryptionModuleExceptionQuotaExceededError, "The MediaKeys object is already in use by another media element.");
            return;
        }
    ...
    if (this_element.media_keys_) {
        ...
        if (media_player) {
            ...
            SuccessCallback success_callback = WTF::BindOnce(&SetMediaKeysHandler::SetNewMediaKeys, WrapPersistent(this));
            FailureCallback failure_callback = WTF::BindOnce(&SetMediaKeysHandler::ClearFailed, WrapPersistent(this));
            ContentDecryptionModuleResult* result
                = MakeGarbageCollected<SetContentDecryptionModuleResult>(std::move(success_callback), std::move(failure_callback));
...
void SetMediaKeysHandler::SetNewMediaKeys()
{
    ...
    if (new_media_keys_) {
        ...
        if (element_->GetWebMediaPlayer()) {
            SuccessCallback success_callback = WTF::BindOnce(&SetMediaKeysHandler::Finish, WrapPersistent(this));
            FailureCallback failure_callback = WTF::BindOnce(&SetMediaKeysHandler::SetFailed, WrapPersistent(this));
            ContentDecryptionModuleResult* result
                = MakeGarbageCollected<SetContentDecryptionModuleResult>(std::move(success_callback), std::move(failure_callback));
            element_->GetWebMediaPlayer()->SetContentDecryptionModule(new_media_keys_->ContentDecryptionModule(), result->Result());
...
void SetMediaKeysHandler::Fail(WebContentDecryptionModuleException code, const String& error_message)
{
    ...
    DCHECK(!HTMLMediaElementEncryptedMedia::From(*element_).is_attaching_media_keys_);

    // Reject promise with an appropriate error.
    WebCdmExceptionToPromiseRejection(resolver_, code, error_message);
}

void SetMediaKeysHandler::ClearFailed(WebContentDecryptionModuleException code, const String& error_message)
{
...
void SetMediaKeysHandler::SetFailed(WebContentDecryptionModuleException code, const String& error_message)
{
    DVLOG(EME_LOG_LEVEL) << __func__ << "(" << code << ", " << error_message << ")";
    HTMLMediaElementEncryptedMedia& this_element = HTMLMediaElementEncryptedMedia::From(*element_);
...
// static
const char HTMLMediaElementEncryptedMedia::kSupplementName[] = "HTMLMediaElementEncryptedMedia";
...
{
    HTMLMediaElementEncryptedMedia* supplement = Supplement<HTMLMediaElement>::From<HTMLMediaElementEncryptedMedia>(element);
    if (!supplement) {
...
{
    HTMLMediaElementEncryptedMedia& this_element = HTMLMediaElementEncryptedMedia::From(element);
    return this_element.media_keys_.Get();
}

ScriptPromise<IDLUndefined> HTMLMediaElementEncryptedMedia::setMediaKeys(
    ScriptState* script_state, HTMLMediaElement& element, MediaKeys* media_keys, ExceptionState& exception_state)
{
    HTMLMediaElementEncryptedMedia& this_element = HTMLMediaElementEncryptedMedia::From(element);
    DVLOG(EME_LOG_LEVEL) << __func__ << ": current(" << this_element.media_keys_.Get() << "), new(" << media_keys << ")";
    ...
    if (this_element.is_attaching_media_keys_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Another request is in progress.");
        return EmptyPromise();
    }
    ...
    return SetMediaKeysHandler::Create(script_state, element, media_keys, exception_state.GetContext());
}

// Create a MediaEncryptedEvent for WD EME.
static Event* CreateEncryptedEvent(media::EmeInitDataType init_data_type, const unsigned char* init_data, unsigned init_data_length)
{
    MediaEncryptedEventInit* initializer = MediaEncryptedEventInit::Create();
    initializer->setInitDataType(EncryptedMediaUtils::ConvertFromInitDataType(init_data_type));
    initializer->setInitData(DOMArrayBuffer::Create(UNSAFE_TODO(base::span(init_data, init_data_length))));
    initializer->setBubbles(false);
    initializer->setCancelable(false);

    return MakeGarbageCollected<MediaEncryptedEvent>(event_type_names::kEncrypted, initializer);
}

void HTMLMediaElementEncryptedMedia::Encrypted(media::EmeInitDataType init_data_type, const unsigned char* init_data, unsigned init_data_length)
{
    DVLOG(EME_LOG_LEVEL) << __func__;
