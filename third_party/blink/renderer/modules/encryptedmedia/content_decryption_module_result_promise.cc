...
void WebCdmExceptionToPromiseRejection(ScriptPromiseResolverBase* resolver, WebContentDecryptionModuleException cdm_exception, const String& message)
{
    ...
    case kWebContentDecryptionModuleExceptionNotSupportedError:
        resolver->RejectWithDOMException(DOMExceptionCode::kNotSupportedError, message);
        return;
    ...
    case kWebContentDecryptionModuleExceptionQuotaExceededError:
        resolver->RejectWithDOMException(DOMExceptionCode::kQuotaExceededError, message);
        return;
    }
...
ContentDecryptionModuleResultPromise::ContentDecryptionModuleResultPromise(
    ScriptPromiseResolverBase* resolver, const MediaKeysConfig& config, EmeApiType api_type)
    : resolver_(resolver)
    , config_(config)
    , api_type_(api_type)
{
...
ContentDecryptionModuleResultPromise::~ContentDecryptionModuleResultPromise() = default;
...
void ContentDecryptionModuleResultPromise::CompleteWithContentDecryptionModule(std::unique_ptr<WebContentDecryptionModule> cdm)
{
    NOTREACHED();
}
...
void ContentDecryptionModuleResultPromise::CompleteWithKeyStatus(WebEncryptedMediaKeyInformation::KeyStatus)
{
    if (!IsValidToFulfillPromise())
        return;
    resolver_->RejectWithDOMException(DOMExceptionCode::kInvalidStateError, "Unexpected completion.");
    resolver_.Clear();
}

void ContentDecryptionModuleResultPromise::CompleteWithError(
    WebContentDecryptionModuleException exception_code, uint32_t system_code, const WebString& error_message)
{
    if (!IsValidToFulfillPromise())
        return;
    ...
    if (IsA<LocalDOMWindow>(execution_context)) {
        ...
        if (document) {
            ukm::builders::Media_EME_ApiPromiseRejection builder(document->UkmSourceID());
            builder.SetKeySystem(media::GetKeySystemIntForUKM(config_.key_system.Ascii()));
            builder.SetUseHardwareSecureCodecs(static_cast<int>(config_.use_hardware_secure_codecs));
    ...
    WebCdmExceptionToPromiseRejection(resolver_, exception_code, result.ToString());
    resolver_.Clear();
}
