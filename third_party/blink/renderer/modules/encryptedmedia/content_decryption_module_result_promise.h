...
void WebCdmExceptionToPromiseRejection(ScriptPromiseResolverBase*, WebContentDecryptionModuleException, const String& message);
...
public:
    ...
    void CompleteWithKeyStatus(WebEncryptedMediaKeyInformation::KeyStatus) override;
    void CompleteWithError(WebContentDecryptionModuleException, uint32_t system_code, const WebString&) override;

    void Trace(Visitor*) const override;

protected:
    // |interface_name| and |property_name| must have static life time.
    ContentDecryptionModuleResultPromise(ScriptPromiseResolverBase*, const MediaKeysConfig&, EmeApiType api_type);

    // Resolves the promise with |value|. Used by subclasses to resolve the
    // promise.
    template <typename IDLType, typename... BlinkType> void Resolve(BlinkType&&... value)
    {
        DCHECK(IsValidToFulfillPromise());

        resolver_->DowncastTo<IDLType>()->Resolve(std::forward<BlinkType>(value)...);
        resolver_.Clear();
    }
