...
void CheckTrackAdapterSettingsEqualsResolution(const VideoCaptureSettings& settings)
{
    EXPECT_FALSE(settings.track_adapter_settings().target_size());
    EXPECT_EQ(1.0 / settings.Format().frame_size.height(), settings.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(settings.Format().frame_size.width(), settings.track_adapter_settings().max_aspect_ratio());
}

void CheckTrackAdapterSettingsEqualsFrameRate(const VideoCaptureSettings& settings, std::optional<double> value = std::nullopt)
{
    EXPECT_EQ(value, settings.track_adapter_settings().max_frame_rate());
}

void CheckTrackAdapterSettingsEqualsFormat(const VideoCaptureSettings& settings)
{
...
{
    return static_cast<double>(format.frame_size.width()) / static_cast<double>(format.frame_size.height());
}

VideoCaptureSettings SelectSettingsVideoDeviceCapture(const VideoDeviceCaptureCapabilities& capabilities, const MediaConstraints& constraints)
{
    return SelectSettingsVideoDeviceCapture(
        capabilities, constraints, MediaStreamVideoSource::kDefaultWidth, MediaStreamVideoSource::kDefaultHeight, MediaStreamVideoSource::kDefaultFrameRate);
}
...
public:
    void SetUp() override
    {
        ...
        device.formats = {
            media::VideoCaptureFormat(gfx::Size(200, 200), 40.0f, media::PIXEL_FORMAT_I420),
            // This entry is is the closest to defaults.
            media::VideoCaptureFormat(gfx::Size(500, 500), 40.0f, media::PIXEL_FORMAT_I420),
            media::VideoCaptureFormat(gfx::Size(1000, 1000), 20.0f, media::PIXEL_FORMAT_I420),
        };
        ...
        device.formats = {
            media::VideoCaptureFormat(gfx::Size(40, 30), 20.0f, media::PIXEL_FORMAT_I420),
            media::VideoCaptureFormat(gfx::Size(320, 240), 30.0f, media::PIXEL_FORMAT_I420),
            // This format has defaults for all settings
            media::VideoCaptureFormat(gfx::Size(MediaStreamVideoSource::kDefaultWidth, MediaStreamVideoSource::kDefaultHeight),
                MediaStreamVideoSource::kDefaultFrameRate, media::PIXEL_FORMAT_I420),
            media::VideoCaptureFormat(gfx::Size(800, 600), 20.0f, media::PIXEL_FORMAT_I420),
        };
        ...
        device.formats = {
            media::VideoCaptureFormat(gfx::Size(600, 400), 10.0f, media::PIXEL_FORMAT_I420),
            media::VideoCaptureFormat(gfx::Size(640, 480), 10.0f, media::PIXEL_FORMAT_I420),
            // This format has default for all settings, except that the resolution
            // is inverted.
            media::VideoCaptureFormat(gfx::Size(MediaStreamVideoSource::kDefaultHeight, MediaStreamVideoSource::kDefaultWidth),
                MediaStreamVideoSource::kDefaultFrameRate, media::PIXEL_FORMAT_I420),
            // This format has defaults for all settings
            media::VideoCaptureFormat(gfx::Size(MediaStreamVideoSource::kDefaultWidth, MediaStreamVideoSource::kDefaultHeight),
                MediaStreamVideoSource::kDefaultFrameRate, media::PIXEL_FORMAT_I420),
            media::VideoCaptureFormat(gfx::Size(1280, 720), 60.0f, media::PIXEL_FORMAT_I420),
            media::VideoCaptureFormat(gfx::Size(1920, 1080), 60.0f, media::PIXEL_FORMAT_I420),
            media::VideoCaptureFormat(gfx::Size(2304, 1536), 10.0f, media::PIXEL_FORMAT_I420),
        };
        device.control_support.pan = true;
        device.control_support.tilt = true;
        device.control_support.zoom = true;
        ...
        device.facing_mode = mojom::blink::FacingMode::kEnvironment;
        device.formats = { media::VideoCaptureFormat(gfx::Size(640, 480), 30.0f, media::PIXEL_FORMAT_Y16) };
        device.control_support.pan = true;
        device.control_support.tilt = true;
        device.control_support.zoom = true;
        capabilities_.device_capabilities.push_back(std::move(device));
        ...
        device.facing_mode = mojom::blink::FacingMode::kNone;
        device.formats = {
            media::VideoCaptureFormat(gfx::Size(MediaStreamVideoSource::kDefaultWidth, MediaStreamVideoSource::kDefaultHeight), 0.0f, media::PIXEL_FORMAT_I420),
            media::VideoCaptureFormat(gfx::Size(500, 500), 0.1f, media::PIXEL_FORMAT_I420),
        };
        device.control_support.pan = true;
        device.control_support.tilt = true;
        device.control_support.zoom = true;
...
protected:
    ...
    base::expected<Vector<VideoCaptureSettings>, std::string> SelectEligibleSettings()
    {
        MediaConstraints constraints = constraint_factory_.CreateMediaConstraints();
        return SelectEligibleSettingsVideoDeviceCapture(capabilities_, constraints, MediaStreamVideoSource::kDefaultWidth,
            MediaStreamVideoSource::kDefaultHeight, MediaStreamVideoSource::kDefaultFrameRate);
    }

    static WTF::Vector<BooleanConstraint MediaTrackConstraintSetPlatform::*> BooleanImageCaptureConstraints()
    {
        return {
    ...
    static WTF::Vector<DoubleConstraint MediaTrackConstraintSetPlatform::*> DoubleImageCaptureConstraints()
    {
        return {
    ...
    static WTF::Vector<DoubleConstraint MediaTrackConstraintSetPlatform::*> PanTiltZoomConstraints()
    {
        return {
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnDeviceID)
{
    ...
    EXPECT_EQ(constraint_factory_.basic().device_id.GetName(), result.failed_constraint_name());
}
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnGroupID)
{
    ...
    auto result = SelectSettings();
    EXPECT_FALSE(result.HasValue());
    EXPECT_EQ(constraint_factory_.basic().group_id.GetName(), result.failed_constraint_name());
}
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnFacingMode)
{
    ...
    auto result = SelectSettings();
    EXPECT_FALSE(result.HasValue());
    EXPECT_EQ(constraint_factory_.basic().facing_mode.GetName(), result.failed_constraint_name());
}

TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnEmptyFacingMode)
{
    constraint_factory_.Reset();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnInvalidResizeMode)
{
    constraint_factory_.Reset();
    constraint_factory_.basic().resize_mode.SetExact("invalid");
    auto result = SelectSettings();
    EXPECT_FALSE(result.HasValue());
    EXPECT_EQ(constraint_factory_.basic().resize_mode.GetName(), result.failed_constraint_name());
}

TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnEmptyResizeMode)
{
    constraint_factory_.Reset();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnHeight)
{
    ...
    auto result = SelectSettings();
    EXPECT_FALSE(result.HasValue());
    EXPECT_EQ(constraint_factory_.basic().height.GetName(), result.failed_constraint_name());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnWidth)
{
    ...
    auto result = SelectSettings();
    EXPECT_FALSE(result.HasValue());
    EXPECT_EQ(constraint_factory_.basic().width.GetName(), result.failed_constraint_name());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnAspectRatio)
{
    constraint_factory_.Reset();
    constraint_factory_.basic().aspect_ratio.SetExact(123467890.0);
    auto result = SelectSettings();
    EXPECT_FALSE(result.HasValue());
    EXPECT_EQ(constraint_factory_.basic().aspect_ratio.GetName(), result.failed_constraint_name());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnFrameRate)
{
    ...
    auto result = SelectSettings();
    EXPECT_FALSE(result.HasValue());
    EXPECT_EQ(constraint_factory_.basic().frame_rate.GetName(), result.failed_constraint_name());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnNoiseReduction)
{
    ...
    EXPECT_FALSE(result.HasValue());
    EXPECT_EQ(constraint_factory_.basic().goog_noise_reduction.GetName(), result.failed_constraint_name());
}

TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, OverconstrainedOnMandatoryPanTiltZoom)
{
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryFacingMode)
{
    ...
    EXPECT_EQ(low_res_device_->device_id.Utf8(), result.device_id());
    EXPECT_EQ(mojom::blink::FacingMode::kEnvironment, low_res_device_->facing_mode);
    EXPECT_EQ(*low_res_closest_format_, result.Format());
    CheckTrackAdapterSettingsEqualsFormat(result);
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryExactHeight)
{
    ...
    EXPECT_EQ(std::round(kLargeHeight * AspectRatio(*high_res_highest_format_)), result.track_adapter_settings().target_width());
}
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryMinHeight)
{
    ...
    EXPECT_FALSE(result.track_adapter_settings().target_size().has_value());
    EXPECT_EQ(static_cast<double>(result.Width()) / kHeight, result.track_adapter_settings().max_aspect_ratio());
    EXPECT_EQ(1.0 / result.Height(), result.track_adapter_settings().min_aspect_ratio());
    ...
    EXPECT_LE(kHeight, result.Height());
    EXPECT_FALSE(result.track_adapter_settings().target_size().has_value());
    EXPECT_EQ(static_cast<double>(result.Width()) / kLargeHeight, result.track_adapter_settings().max_aspect_ratio());
    EXPECT_EQ(1.0 / result.Height(), result.track_adapter_settings().min_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFrameRate(result);
}
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryMaxHeight)
{
    ...
    EXPECT_EQ(std::round(kLowHeight * AspectRatio(result.Format())), result.track_adapter_settings().target_width());
    EXPECT_EQ(static_cast<double>(result.Width()), result.track_adapter_settings().max_aspect_ratio());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryHeightRange)
{
    constraint_factory_.Reset();
    {
        ...
        EXPECT_EQ(*default_closest_format_, result.Format());
        EXPECT_FALSE(result.track_adapter_settings().target_size().has_value());
        EXPECT_EQ(static_cast<double>(result.Width()) / kMinHeight, result.track_adapter_settings().max_aspect_ratio());
        EXPECT_EQ(1.0 / result.Height(), result.track_adapter_settings().min_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFrameRate(result);
    }
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, IdealHeight)
{
...

    {
        ...
        EXPECT_EQ(std::round(kIdealHeight * AspectRatio(result.Format())), result.track_adapter_settings().target_width());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryExactWidth)
{
    ...
    EXPECT_EQ(static_cast<double>(kWidth) / result.Height(), result.track_adapter_settings().min_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFrameRate(result);
    ...
    EXPECT_EQ(high_res_device_->device_id.Utf8(), result.device_id());
    EXPECT_EQ(*high_res_highest_format_, result.Format());
    EXPECT_EQ(std::round(kLargeWidth / AspectRatio(result.Format())), result.track_adapter_settings().target_height());
    ...
    EXPECT_EQ(static_cast<double>(kLargeWidth) / result.Height(), result.track_adapter_settings().min_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFrameRate(result);
}
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryMaxWidth)
{
    ...
    EXPECT_EQ(std::round(kLowWidth / AspectRatio(result.Format())), result.track_adapter_settings().target_height());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryWidthRange)
{
    constraint_factory_.Reset();
    {
        ...
        EXPECT_FALSE(result.track_adapter_settings().target_size().has_value());
        EXPECT_EQ(result.Width(), result.track_adapter_settings().max_aspect_ratio());
        EXPECT_EQ(static_cast<double>(kMinWidth) / result.Height(), result.track_adapter_settings().min_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFrameRate(result);
    }
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, IdealWidth)
{
...

    {
        ...
        EXPECT_EQ(std::round(kIdealWidth / AspectRatio(result.Format())), result.track_adapter_settings().target_height());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryMaxFrameRate)
{
    ...
    {
        constraint_factory_.Reset();
        const double kMaxFrameRate = static_cast<double>(media::limits::kMaxFramesPerSecond) + 0.1;
        constraint_factory_.basic().frame_rate.SetMax(kMaxFrameRate);
        auto result = SelectSettings();
        EXPECT_TRUE(result.HasValue());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryExactAspectRatio)
{
    ...
    EXPECT_EQ(default_device_->device_id.Utf8(), result.device_id());
    EXPECT_EQ(*default_closest_format_, result.Format());
    EXPECT_EQ(std::round(result.Width() / kAspectRatio), result.track_adapter_settings().target_height());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryMaxAspectRatio)
{
    ...
    EXPECT_EQ(std::round(result.Height() * kAspectRatio), result.track_adapter_settings().target_width());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, MandatoryAspectRatioRange)
{
    constraint_factory_.Reset();
    {
        ...
        EXPECT_EQ(kMaxAspectRatio, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFrameRate(result);
    }

    {
        ...
        EXPECT_EQ(std::round(result.Width() / kMinAspectRatio), result.track_adapter_settings().target_height());
        EXPECT_EQ(result.Width(), result.track_adapter_settings().target_width());
        EXPECT_EQ(kMinAspectRatio, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(static_cast<double>(result.Width()) / kMinHeight, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFrameRate(result);
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, IdealAspectRatio)
{
    constraint_factory_.Reset();
    {
        ...
        EXPECT_EQ(result.Height(), result.track_adapter_settings().target_height());
        EXPECT_EQ(std::round(result.Height() * kIdealAspectRatio), result.track_adapter_settings().target_width());
        EXPECT_EQ(min_aspect_ratio, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(max_aspect_ratio, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFrameRate(result);
    }

    {
        ...
        EXPECT_EQ(std::round(result.Width() / kIdealAspectRatio), result.track_adapter_settings().target_height());
        EXPECT_EQ(result.Width(), result.track_adapter_settings().target_width());
        EXPECT_EQ(1.0 / result.Height(), result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(result.Width(), result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFrameRate(result);
...

    {
        ...
        EXPECT_EQ(kExactHeight * kIdealAspectRatio, result.track_adapter_settings().target_width());
        EXPECT_EQ(1.0 / kExactHeight, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(static_cast<double>(result.Width()) / kExactHeight, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFrameRate(result);
    }
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, IdealResizeModeResolutionGreaterThanNative)
{
    ...
    constraint_factory_.basic().width.SetIdeal(kIdealWidth);
    constraint_factory_.basic().height.SetIdeal(kIdealHeight);
    constraint_factory_.basic().resize_mode.SetIdeal(WebString::FromASCII("none"));
    auto result = SelectSettings();
    EXPECT_TRUE(result.HasValue());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, IdealResizeModeResolutionLessThanNative)
{
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, PresentPanTiltZoomOnSystemWithoutPanTiltZoomCamera)
{
...
// sets.
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedMinMaxResolutionFrameRate)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
    ...
    MediaTrackConstraintSetPlatform& advanced2 = constraint_factory_.AddAdvanced();
    ...
    MediaTrackConstraintSetPlatform& advanced3 = constraint_factory_.AddAdvanced();
    ...
    MediaTrackConstraintSetPlatform& advanced4 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedNoiseReduction)
{
    ...
    EXPECT_FALSE(result.track_adapter_settings().target_size().has_value());
    EXPECT_EQ(1920.0 / result.Height(), result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(result.Width() / 1080.0, result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFrameRate(result);
}

TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryNoiseReduction)
{
    {
        constraint_factory_.Reset();
        MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
        ...
        EXPECT_TRUE(result.noise_reduction() && *result.noise_reduction());
        EXPECT_FALSE(result.track_adapter_settings().target_size().has_value());
        EXPECT_EQ(640.0 / result.Height(), result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(result.Width() / 480.0, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFrameRate(result);
    }
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryExactResolution)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryMaxMinResolutionFrameRate)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryMinMaxResolutionFrameRate)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
    ...
    EXPECT_FALSE(result.track_adapter_settings().target_size().has_value());
    EXPECT_EQ(800.0 / result.Height(), result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(result.Width() / 600.0, result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFrameRate(result);
}

TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryExactAspectRatio)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
    ...
    EXPECT_EQ(result.Width(), result.track_adapter_settings().target_width());
    EXPECT_EQ(std::round(result.Height() / 2300.0), result.track_adapter_settings().target_height());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryAspectRatioRange)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryExactFrameRate)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryFrameRateRange)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryWidthFrameRate)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryHeightFrameRate)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryDeviceID)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
    Vector<String> id_vector1 = { kDeviceID1, kDeviceID2 };
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryDeviceIDAndResolution)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryGroupID)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
    Vector<String> id_vector1 = { kGroupID1, kGroupID2 };
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryAspectRatioWidth)
{
    {
        constraint_factory_.Reset();
        MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
        ...
        EXPECT_EQ(*default_closest_format_, result.Format());
        EXPECT_EQ(result.Width(), result.track_adapter_settings().target_width());
        EXPECT_EQ(std::round(result.Width() / 17.0), result.track_adapter_settings().target_height());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryImageCapture)
{
    for (auto& constraint : BooleanImageCaptureConstraints()) {
        ...
        EXPECT_EQ(result.image_capture_device_settings()->torch.has_value(), constraint == &MediaTrackConstraintSetPlatform::torch);
        if (result.image_capture_device_settings()->torch.has_value()) {
            EXPECT_FALSE(result.image_capture_device_settings()->torch.value());
        }
        EXPECT_EQ(result.image_capture_device_settings()->background_blur.has_value(), constraint == &MediaTrackConstraintSetPlatform::background_blur);
        if (result.image_capture_device_settings()->background_blur.has_value()) {
            EXPECT_FALSE(result.image_capture_device_settings()->background_blur.value());
        }
        EXPECT_EQ(result.image_capture_device_settings()->background_segmentation_mask.has_value(),
            constraint == &MediaTrackConstraintSetPlatform::background_segmentation_mask);
        if (result.image_capture_device_settings()->background_segmentation_mask.has_value()) {
            EXPECT_FALSE(result.image_capture_device_settings()->background_segmentation_mask.value());
        }
        EXPECT_EQ(result.image_capture_device_settings()->eye_gaze_correction.has_value(), constraint == &MediaTrackConstraintSetPlatform::eye_gaze_correction);
        if (result.image_capture_device_settings()->eye_gaze_correction.has_value()) {
            EXPECT_FALSE(result.image_capture_device_settings()->eye_gaze_correction.value());
        }
        EXPECT_EQ(result.image_capture_device_settings()->face_framing.has_value(), constraint == &MediaTrackConstraintSetPlatform::face_framing);
        if (result.image_capture_device_settings()->face_framing.has_value()) {
            EXPECT_FALSE(result.image_capture_device_settings()->face_framing.value());
        }
    }
    ...
    for (auto& constraint : DoubleImageCaptureConstraints()) {
        ...
        // ignored because it contradicts the third set.
        EXPECT_EQ(
            result.image_capture_device_settings()->exposure_compensation.has_value(), constraint == &MediaTrackConstraintSetPlatform::exposure_compensation);
        if (result.image_capture_device_settings()->exposure_compensation.has_value()) {
            EXPECT_EQ(1.0, result.image_capture_device_settings()->exposure_compensation.value());
        }
        EXPECT_EQ(result.image_capture_device_settings()->exposure_time.has_value(), constraint == &MediaTrackConstraintSetPlatform::exposure_time);
        if (result.image_capture_device_settings()->exposure_time.has_value()) {
            EXPECT_EQ(2.0, result.image_capture_device_settings()->exposure_time.value());
        }
        EXPECT_EQ(result.image_capture_device_settings()->color_temperature.has_value(), constraint == &MediaTrackConstraintSetPlatform::color_temperature);
        if (result.image_capture_device_settings()->color_temperature.has_value()) {
            EXPECT_EQ(3.0, result.image_capture_device_settings()->color_temperature.value());
        }
        EXPECT_EQ(result.image_capture_device_settings()->iso.has_value(), constraint == &MediaTrackConstraintSetPlatform::iso);
        if (result.image_capture_device_settings()->iso.has_value()) {
            EXPECT_EQ(4.0, result.image_capture_device_settings()->iso.value());
        }
        EXPECT_EQ(result.image_capture_device_settings()->brightness.has_value(), constraint == &MediaTrackConstraintSetPlatform::brightness);
        if (result.image_capture_device_settings()->brightness.has_value()) {
            EXPECT_EQ(5.0, result.image_capture_device_settings()->brightness.value());
        }
        EXPECT_EQ(result.image_capture_device_settings()->contrast.has_value(), constraint == &MediaTrackConstraintSetPlatform::contrast);
        ...
        EXPECT_EQ(result.image_capture_device_settings()->saturation.has_value(), constraint == &MediaTrackConstraintSetPlatform::saturation);
        if (result.image_capture_device_settings()->saturation.has_value()) {
            EXPECT_EQ(7.0, result.image_capture_device_settings()->saturation.value());
        }
        EXPECT_EQ(result.image_capture_device_settings()->sharpness.has_value(), constraint == &MediaTrackConstraintSetPlatform::sharpness);
        ...
        EXPECT_EQ(result.image_capture_device_settings()->focus_distance.has_value(), constraint == &MediaTrackConstraintSetPlatform::focus_distance);
        if (result.image_capture_device_settings()->focus_distance.has_value()) {
            EXPECT_EQ(9.0, result.image_capture_device_settings()->focus_distance.value());
        }
    }
}

TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedContradictoryPanTiltZoom)
{
    for (auto& constraint : PanTiltZoomConstraints()) {
        constraint_factory_.Reset();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, AdvancedResolutionResizeFrameRate)
{
    constraint_factory_.Reset();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, BasicContradictoryWidthAspectRatio)
{
    constraint_factory_.Reset();
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, BasicContradictoryImageCapture)
{
    for (auto& constraint : DoubleImageCaptureConstraints()) {
        constraint_factory_.Reset();
        ...
        auto result = SelectSettings();
        EXPECT_FALSE(result.HasValue());
        EXPECT_EQ((constraint_factory_.basic().*constraint).GetName(), result.failed_constraint_name());
    }
}

TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, BasicContradictoryPanTiltZoom)
{
    for (auto& constraint : PanTiltZoomConstraints()) {
        constraint_factory_.Reset();
        (constraint_factory_.basic().*constraint).SetMin(4);
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, NoDevicesNoConstraints)
{
    constraint_factory_.Reset();
    VideoDeviceCaptureCapabilities capabilities;
    auto result = SelectSettingsVideoDeviceCapture(capabilities, constraint_factory_.CreateMediaConstraints());
    EXPECT_FALSE(result.HasValue());
...
{
    constraint_factory_.Reset();
    constraint_factory_.basic().device_id.SetExact(invalid_frame_rate_device_->device_id);
    auto result = SelectSettings();
    EXPECT_TRUE(result.HasValue());
    EXPECT_EQ(invalid_frame_rate_device_->device_id.Utf8(), result.device_id());
    EXPECT_EQ(invalid_frame_rate_device_->formats[0].frame_rate, result.FrameRate());
    ...
    EXPECT_TRUE(result.HasValue());
    EXPECT_EQ(invalid_frame_rate_device_->device_id.Utf8(), result.device_id());
    EXPECT_EQ(invalid_frame_rate_device_->formats[1].frame_rate, result.FrameRate());
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, SelectEligibleSettingsVideoDeviceCapture_NoEligibleDevices)
{
    constraint_factory_.Reset();
    constraint_factory_.basic().device_id.SetExact("NONEXISTING");
...
TEST_F(MediaStreamConstraintsUtilVideoDeviceTest, SelectEligibleSettingsVideoDeviceCapture_IncludesEligibleDevices)
{
    constraint_factory_.Reset();
