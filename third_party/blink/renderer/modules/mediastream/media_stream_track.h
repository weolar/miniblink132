...
class MODULES_EXPORT MediaStreamTrack : public EventTarget, public ActiveScriptWrappable<MediaStreamTrack> {
...
public:
    ...
    using FromTransferredStateImplForTesting = base::RepeatingCallback<MediaStreamTrack*(const TransferredValues&)>;
    ...
    static MediaStreamTrack* FromTransferredState(ScriptState* script_state, const TransferredValues& data);
    ...
    virtual V8UnionMediaStreamTrackAudioStatsOrMediaStreamTrackVideoStats* stats() = 0;
    virtual CaptureHandle* getCaptureHandle() const = 0;
    virtual ScriptPromise<IDLUndefined> applyConstraints(ScriptState*, const MediaTrackConstraints*) = 0;

    virtual void applyConstraints(ScriptPromiseResolver<IDLUndefined>*, const MediaTrackConstraints*) = 0;
    ...
    void AddedEventListener(const AtomicString&, RegisteredEventListener&) override = 0;

    // ScriptWrappable
    bool HasPendingActivity() const override = 0;

    virtual std::unique_ptr<AudioSourceProvider> CreateWebAudioSource(int context_sample_rate, base::TimeDelta platform_buffer_duration) = 0;
...
private:
    ...
    friend void SetFromTransferredStateImplForTesting(FromTransferredStateImplForTesting impl);
    // Provides access to the global mock impl of FromTransferredState. Set to
    // base::NullCallback() to restore the real impl.
    static FromTransferredStateImplForTesting& GetFromTransferredStateImplForTesting();
};
