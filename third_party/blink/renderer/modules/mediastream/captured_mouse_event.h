...
public:
    static CapturedMouseEvent* Create(const AtomicString& type, const CapturedMouseEventInit* initializer, ExceptionState& exception_state);

    CapturedMouseEvent(const AtomicString& type, const CapturedMouseEventInit* initializer);
    ~CapturedMouseEvent() override = default;

    int surfaceX() const
    {
        return surface_coordinates_.x();
    }
    int surfaceY() const
    {
        return surface_coordinates_.y();
    }
