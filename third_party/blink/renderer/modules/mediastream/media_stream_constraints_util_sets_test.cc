...
constexpr double kDefaultAspectRatio = static_cast<double>(kDefaultWidth) / static_cast<double>(kDefaultHeight);
...
bool AreCounterclockwise(const Vector<Point>& vertices)
{
    ...
    for (auto vertex = vertices.begin() + 2; vertex != vertices.end(); ++vertex) {
        ...
        double det = prev_diagonal.height() * current_diagonal.width() - current_diagonal.height() * prev_diagonal.width();
        if (det <= 0)
            return false;
...
// segment |s1||s2|.
Point ProjectionOnSegmentLine(const Point& point, const Point& s1, const Point& s2)
{
    double segment_slope = (s2.width() - s1.width()) / (s2.height() - s1.height());
    double segment_angle = std::atan(segment_slope);
    double norm = std::sqrt(Point::Dot(point, point));
    double angle = (point.height() == 0 && point.width() == 0) ? 0.0 : std::atan(point.width() / point.height()) - segment_angle;
...
protected:
    ...
    {
        return set.SelectClosestPointToIdeal(factory_.CreateMediaConstraints().Basic(), kDefaultHeight, kDefaultWidth);
    }
...
TEST_F(MediaStreamConstraintsUtilSetsTest, VertexListValidity)
{
    ...
    // Not in counterclockwise order.
    EXPECT_FALSE(AreCounterclockwise({ P(1, 0), P(3, 0), P(2, 2), P(3.1, 1), P(0, 1) }));

    EXPECT_TRUE(AreCounterclockwise({ P(1, 0), P(3, 0), P(3.1, 1), P(3, 2), P(1, 2), P(0.9, 1) }));

    // Not in counterclockwise order.
    EXPECT_FALSE(AreCounterclockwise({ P(1, 0), P(3, 0), P(3.1, 1), P(1, 2), P(3, 2), P(0.9, 1) }));

    // Counterclockwise, but more than 6 vertices.
    EXPECT_FALSE(AreCounterclockwise({ P(1, 0), P(3, 0), P(3.1, 1), P(3, 2), P(2, 2.1), P(1, 2), P(0.9, 1) }));
}
...
TEST_F(MediaStreamConstraintsUtilSetsTest, PointOperations)
{
    ...
    EXPECT_EQ(P::SquareEuclideanDistance(P(50, 50), kZero), P::SquareEuclideanDistance(kZero, P(50, 50)));
    EXPECT_EQ(kZero, P::ClosestPointInSegment(kZero, kZero, P(50, 50)));
    EXPECT_EQ(kZero, P::ClosestPointInSegment(kZero, P(50, 50), kZero));
    EXPECT_EQ(P(50, 50), P::ClosestPointInSegment(P(50, 50), P(50, 50), P(50, 50)));
    ...
    EXPECT_EQ(2.0 * (P(100, 100) + P(100, 50)), 2.0 * P(100, 100) + 2.0 * P(100, 50));
    ...
    EXPECT_EQ(P(50, 0), P::ClosestPointInSegment(P(100, 100), P(10, 0), P(50, 0)));
    ...
    EXPECT_EQ(P(0, 50), P::ClosestPointInSegment(P(100, 100), P(0, 10), P(0, 50)));
...
TEST_F(MediaStreamConstraintsUtilSetsTest, ResolutionPointSetClosestPoint)
{
    ...
    ResolutionSet set(kHeight, kHeight, kWidth, kWidth, kAspectRatio, kAspectRatio);
...
TEST_F(MediaStreamConstraintsUtilSetsTest, ResolutionLineSetClosestPoint)
{
    ...
    {
        const double kAspectRatios[] = { 0.0, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0, HUGE_VAL };
        for (double aspect_ratio : kAspectRatios) {
            auto set = ResolutionSet::FromExactAspectRatio(aspect_ratio);
            for (int height = 0; height < 100; height += 10) {
                for (int width = 0; width < 100; width += 10) {
                    Point point(height, width);
                    Point expected = ProjectionOnSegmentLine(point, P(0, 0), P(1, aspect_ratio));
...
TEST_F(MediaStreamConstraintsUtilSetsTest, ResolutionGeneralSetClosestPoint)
{
    ...
    {
        Point point(12, 70);
        Point expected = ProjectionOnSegmentLine(point, P(100 / 1.5, 100), P(10, 15));
        Point actual = set.ClosestPointTo(point);
        EXPECT_POINT_EQ(expected, actual);
    }
...
TEST_F(MediaStreamConstraintsUtilSetsTest, ResolutionIdealIntersects)
{
    ...
    // Ideal height.
    {
        ...
        EXPECT_POINT_EQ(Point(kIdealHeight, kIdealHeight * kDefaultAspectRatio), point);
    }
    ...
    // Ideal width.
    {
        ...
        Point point = SelectClosestPointToIdeal(set);
        EXPECT_POINT_EQ(Point(kIdealWidth / kDefaultAspectRatio, kIdealWidth), point);
    }
    ...
    // Ideal height and aspect-ratio.
    {
        ...
        factory_.basic().aspect_ratio.SetIdeal(kIdealAspectRatio);
        Point point = SelectClosestPointToIdeal(set);
        EXPECT_POINT_EQ(Point(kIdealHeight, kIdealHeight * kIdealAspectRatio), point);
    }
...
TEST_F(MediaStreamConstraintsUtilSetsTest, ResolutionIdealOutsideSinglePoint)
{
    ...
    // Ideal height and aspect-ratio.
    {
        ...
        factory_.basic().aspect_ratio.SetIdeal(kIdealAspectRatio);
        Point point = SelectClosestPointToIdeal(set);
        Point expected = set.ClosestPointTo(Point(kIdealHeight, kIdealHeight * kIdealAspectRatio));
        EXPECT_POINT_EQ(expected, point);
    }
    ...
    // Ideal width and aspect-ratio.
    {
        ...
        factory_.basic().aspect_ratio.SetIdeal(kIdealAspectRatio);
        Point point = SelectClosestPointToIdeal(set);
        Point expected = set.ClosestPointTo(Point(kIdealWidth / kIdealAspectRatio, kIdealWidth));
        EXPECT_POINT_EQ(expected, point);
    }
...
TEST_F(MediaStreamConstraintsUtilSetsTest, ResolutionIdealOutsideMultiplePoints)
{
    // This set is a triangle with vertices (100,100), (1000,100) and (1000,1000).
    ResolutionSet set(100, 1000, 100, 1000, 0.0, 1.0);
...
TEST_F(MediaStreamConstraintsUtilSetsTest, ResolutionUnconstrainedExtremeIdeal)
{
    ResolutionSet set;
    ...
    // Ideal height.
    {
        ...
        Point point = SelectClosestPointToIdeal(set);
        EXPECT_POINT_EQ(Point(ResolutionSet::kMaxDimension, ResolutionSet::kMaxDimension), point);
    ...
    // Ideal width.
    {
        ...
        Point point = SelectClosestPointToIdeal(set);
        EXPECT_POINT_EQ(Point(ResolutionSet::kMaxDimension / kDefaultAspectRatio, ResolutionSet::kMaxDimension), point);
...
TEST_F(MediaStreamConstraintsUtilSetsTest, ResolutionVertices)
{
    ...
    // Extreme aspect ratios, for completeness.
    {
        ...
        set = ResolutionSet(0, ResolutionSet::kMaxDimension, 0, 100, HUGE_VAL, HUGE_VAL);
        vertices = set.ComputeVertices();
        EXPECT_EQ(2U, vertices.size());
        VerticesContain(vertices, Point(0, 0));
        VerticesContain(vertices, Point(0, 100));
...
TEST_F(MediaStreamConstraintsUtilSetsTest, ExactResolution)
{
    ...
    ResolutionSet set = ResolutionSet::FromExactResolution(kExactWidth, kExactHeight);
...
TEST_F(MediaStreamConstraintsUtilSetsTest, NumericRangeSetFromConstraint)
{
    ...
    media_constraints::NumericRangeSet<int> range = media_constraints::NumericRangeSet<int>::FromConstraint(constraint);
...
TEST_F(MediaStreamConstraintsUtilSetsTest, NumericRangeSetFromConstraintWithBounds)
{
    int upper_bound = 25;
    int lower_bound = 5;
    // Exact value translates in a range with a single value.
    LongConstraint constraint = LongConstraint("aConstraint");
    constraint.SetExact(10);
    media_constraints::NumericRangeSet<int> range = media_constraints::NumericRangeSet<int>::FromConstraint(constraint, lower_bound, upper_bound);
    EXPECT_FALSE(range.IsEmpty());
    EXPECT_TRUE(range.Min());
    EXPECT_EQ(*range.Min(), 10);
    EXPECT_TRUE(range.Max());
    ...
    constraint.SetMin(0);
    constraint.SetMax(100);
    range = media_constraints::NumericRangeSet<int>::FromConstraint(constraint, 0, 100);
    EXPECT_FALSE(range.IsEmpty());
    EXPECT_TRUE(range.Min());
    EXPECT_EQ(*range.Min(), 0);
    EXPECT_TRUE(range.Max());
    EXPECT_EQ(*range.Max(), 100);
    range = media_constraints::NumericRangeSet<int>::FromConstraint(constraint, lower_bound, upper_bound);
    EXPECT_FALSE(range.IsEmpty());
    EXPECT_FALSE(range.Min());
    EXPECT_FALSE(range.Max());
...
TEST_F(MediaStreamConstraintsUtilSetsTest, RescaleSetFromConstraints)
{
    ...
    BoolSet set = media_constraints::RescaleSetFromConstraint(factory_.basic().resize_mode);
    EXPECT_TRUE(set.is_universal());
    EXPECT_FALSE(set.HasExplicitElements());

    // Invalid exact value.
    factory_.basic().resize_mode.SetExact("invalid");
    set = media_constraints::RescaleSetFromConstraint(factory_.basic().resize_mode);
    EXPECT_TRUE(set.IsEmpty());
    ...
    // Rescaling
    factory_.basic().resize_mode.SetExact(WebMediaStreamTrack::kResizeModeRescale);
    set = media_constraints::RescaleSetFromConstraint(factory_.basic().resize_mode);
    EXPECT_TRUE(set.Contains(true));
    EXPECT_FALSE(set.Contains(false));

    // Both explicit
    Vector<String> rescale_modes = { WebMediaStreamTrack::kResizeModeRescale, WebMediaStreamTrack::kResizeModeNone };
