...
template <typename NumericConstraint> double NumericValueFitness(const NumericConstraint& constraint, decltype(constraint.Min()) value)
{
...
double NumericRangeSetFitness(const NumericConstraint& constraint, const media_constraints::NumericRangeSet<decltype(constraint.Min())>& range)
{
...
// Based on https://w3c.github.io/mediacapture-main/#dfn-fitness-distance.
template <typename NumericConstraint>
double NumericRangeSupportFitness(const NumericConstraint& constraint, const media_constraints::NumericRangeSet<decltype(constraint.Min())>& range,
    bool constraint_present, bool constraint_supported)
{
    DCHECK(!range.IsEmpty());
...
template <typename NumericConstraint>
double NumericRangeNativeFitness(
    const NumericConstraint& constraint, decltype(constraint.Min()) min, decltype(constraint.Min()) max, decltype(constraint.Min()) native_value)
{
    auto reference_value = constraint.HasIdeal() ? std::max(std::min(constraint.Ideal(), max), min) : max;
...
// Based on https://w3c.github.io/mediacapture-main/#dfn-fitness-distance.
double OptionalBoolFitness(const std::optional<bool>& value, const BooleanConstraint& constraint)
{
    if (!constraint.HasIdeal()) {
        return 0.0;
    }
...
// name of |constraint|.
void UpdateFailedConstraintName(const BaseConstraint& constraint, const char** failed_constraint_name)
{
...
public:
    ...
    explicit CandidateFormat(const media::VideoCaptureFormat& format)
        : format_(format)
        , resolution_set_(1, format.frame_size.height(), 1, format.frame_size.width(), 0.0, HUGE_VAL)
    {
    }
    ...
    const ResolutionSet& resolution_set() const
    {
        return resolution_set_;
    }
    ...
    int MinHeight() const
    {
        return resolution_set_.min_height();
    }
    int MaxHeight() const
    {
        return resolution_set_.max_height();
    }
    int MinWidth() const
    {
        return resolution_set_.min_width();
    }
    int MaxWidth() const
    {
        return resolution_set_.max_width();
    }
    double MinAspectRatio() const
    {
        return std::max(resolution_set_.min_aspect_ratio(), static_cast<double>(MinWidth()) / MaxHeight());
    }
    double MaxAspectRatio() const
    {
        return std::min(resolution_set_.max_aspect_ratio(), static_cast<double>(MaxWidth()) / MinHeight());
    }
    ...
    std::optional<ApplyConstraintSetResult> TryToApplyConstraintSet(
        const MediaTrackConstraintSetPlatform& constraint_set, const char** failed_constraint_name = nullptr) const
    {
        std::optional<ApplyConstraintSetResult> result(std::in_place);

        result->rescale_intersection_ = rescale_set_.Intersection(media_constraints::RescaleSetFromConstraint(constraint_set.resize_mode));
        if (result->rescale_intersection_.IsEmpty()) {
            UpdateFailedConstraintName(constraint_set.resize_mode, failed_constraint_name);
            return std::nullopt;
        }

        result->resolution_intersection_ = resolution_set_.Intersection(ResolutionSet::FromConstraintSet(constraint_set));
        if (!result->rescale_intersection_.Contains(true)) {
            // If rescaling is not allowed, only the native resolution is allowed.
            result->resolution_intersection_ = result->resolution_intersection_.Intersection(ResolutionSet::FromExactResolution(NativeWidth(), NativeHeight()));
        }
        ...
        if (result->resolution_intersection_.IsAspectRatioEmpty()) {
            UpdateFailedConstraintName(constraint_set.aspect_ratio, failed_constraint_name);
            return std::nullopt;
        }

        if (!SatisfiesFrameRateConstraint(constraint_set.frame_rate)) {
            UpdateFailedConstraintName(constraint_set.frame_rate, failed_constraint_name);
            return std::nullopt;
        }

        result->constrained_frame_rate_
            = constrained_frame_rate_.Intersection(DoubleRangeSet::FromConstraint(constraint_set.frame_rate, 0.0, media::limits::kMaxFramesPerSecond));
        result->constrained_width_ = constrained_width_.Intersection(IntRangeSet::FromConstraint(constraint_set.width, 1L, ResolutionSet::kMaxDimension));
        result->constrained_height_ = constrained_height_.Intersection(IntRangeSet::FromConstraint(constraint_set.height, 1L, ResolutionSet::kMaxDimension));
        result->constrained_aspect_ratio_ = constrained_aspect_ratio_.Intersection(DoubleRangeSet::FromConstraint(constraint_set.aspect_ratio, 0.0, HUGE_VAL));
    ...
    // https://w3c.github.io/mediacapture-main/#dfn-fitness-distance.
    double Fitness(const MediaTrackConstraintSetPlatform& basic_constraint_set, VideoTrackAdapterSettings* track_settings) const
    {
        ...
        if (rescale_set_.Contains(true)) {
            track_settings_with_rescale
                = SelectVideoTrackAdapterSettings(basic_constraint_set, resolution_set(), constrained_frame_rate(), format(), true /* enable_rescale */);
            DCHECK(track_settings_with_rescale.target_size().has_value());
            double target_aspect_ratio = static_cast<double>(track_settings_with_rescale.target_width()) / track_settings_with_rescale.target_height();
            DCHECK(!std::isnan(target_aspect_ratio));
            std::optional<double> best_supported_frame_rate = track_settings_with_rescale.max_frame_rate();
            if (!best_supported_frame_rate.has_value() || *best_supported_frame_rate > NativeFrameRate()) {
                best_supported_frame_rate = NativeFrameRate();
            }

            track_fitness_with_rescale = NumericValueFitness(basic_constraint_set.aspect_ratio, target_aspect_ratio)
                + NumericValueFitness(basic_constraint_set.height, track_settings_with_rescale.target_height())
                + NumericValueFitness(basic_constraint_set.width, track_settings_with_rescale.target_width())
                + NumericValueFitness(basic_constraint_set.frame_rate, *best_supported_frame_rate);
        }
        ...
        if (rescale_set_.Contains(false)) {
            bool can_use_native_resolution = constrained_width_.Contains(NativeWidth()) && constrained_height_.Contains(NativeHeight())
                && constrained_aspect_ratio_.Contains(NativeAspectRatio());
            if (can_use_native_resolution) {
                track_settings_without_rescale
                    = SelectVideoTrackAdapterSettings(basic_constraint_set, resolution_set(), constrained_frame_rate(), format(), false /* enable_rescale */);
                DCHECK(!track_settings_without_rescale.target_size().has_value());
                std::optional<double> best_supported_frame_rate = track_settings_without_rescale.max_frame_rate();
                if (!best_supported_frame_rate.has_value() || *best_supported_frame_rate > NativeFrameRate()) {
                    best_supported_frame_rate = NativeFrameRate();
                }
                track_fitness_without_rescale = NumericValueFitness(basic_constraint_set.aspect_ratio, NativeAspectRatio())
                    + NumericValueFitness(basic_constraint_set.height, NativeHeight()) + NumericValueFitness(basic_constraint_set.width, NativeWidth())
                    + NumericValueFitness(basic_constraint_set.frame_rate, *best_supported_frame_rate);
            }
        }

        if (basic_constraint_set.resize_mode.HasIdeal()) {
            if (!base::Contains(basic_constraint_set.resize_mode.Ideal(), WebMediaStreamTrack::kResizeModeNone)) {
                track_fitness_without_rescale += 1.0;
            }
            if (!base::Contains(basic_constraint_set.resize_mode.Ideal(), WebMediaStreamTrack::kResizeModeRescale)) {
    ...
    double NativeFitness(const MediaTrackConstraintSetPlatform& constraint_set) const
    {
        return NumericRangeNativeFitness(constraint_set.width, MinWidth(), MaxWidth(), NativeWidth())
            + NumericRangeNativeFitness(constraint_set.height, MinHeight(), MaxHeight(), NativeHeight())
            + NumericRangeNativeFitness(constraint_set.frame_rate, MinFrameRate(), MaxFrameRate(), NativeFrameRate());
    }
...
private:
    bool SatisfiesFrameRateConstraint(const DoubleConstraint& constraint) const
    {
        ...
        double constraint_max = ConstraintHasMax(constraint) ? ConstraintMax(constraint) : static_cast<double>(media::limits::kMaxFramesPerSecond);
        bool constraint_min_out_of_range = ((constraint_min > NativeFrameRate())
            || (constraint_min > MaxFrameRateConstraint().value_or(media::limits::kMaxFramesPerSecond) + DoubleConstraint::kConstraintEpsilon));
        bool constraint_max_out_of_range = ((constraint_max < kMinDeviceCaptureFrameRate)
            || (constraint_max < MinFrameRateConstraint().value_or(0.0) - DoubleConstraint::kConstraintEpsilon));
...
// otherwise.
bool FacingModeSatisfiesConstraint(mojom::blink::FacingMode value, const StringConstraint& constraint)
{
...
public:
    ...
    PTZDeviceState(const DoubleRangeSet& pan_set, const DoubleRangeSet& tilt_set, const DoubleRangeSet& zoom_set)
        : pan_set_(pan_set)
        , tilt_set_(tilt_set)
        , zoom_set_(zoom_set)
    {
    ...
    PTZDeviceState Intersection(const MediaTrackConstraintSetPlatform& constraint_set) const
    {
        DoubleRangeSet pan_intersection = pan_set_.Intersection(DoubleRangeSet::FromConstraint(constraint_set.pan));
        DoubleRangeSet tilt_intersection = tilt_set_.Intersection(DoubleRangeSet::FromConstraint(constraint_set.tilt));
        DoubleRangeSet zoom_intersection = zoom_set_.Intersection(DoubleRangeSet::FromConstraint(constraint_set.zoom));

        return PTZDeviceState(pan_intersection, tilt_intersection, zoom_intersection);
    }
    ...
    double Fitness(const MediaTrackConstraintSetPlatform& basic_set, const media::VideoCaptureControlSupport& control_support) const
    {
        return NumericRangeSupportFitness(basic_set.pan, pan_set_, basic_set.pan.IsPresent(), control_support.pan)
            + NumericRangeSupportFitness(basic_set.tilt, tilt_set_, basic_set.tilt.IsPresent(), control_support.tilt)
            + NumericRangeSupportFitness(basic_set.zoom, zoom_set_, basic_set.zoom.IsPresent(), control_support.zoom);
    }
    ...
    std::optional<double> SelectPan(const MediaTrackConstraintSetPlatform& basic_set) const
    {
        return SelectProperty(&PTZDeviceState::pan_set_, basic_set, &MediaTrackConstraintSetPlatform::pan);
    }

    std::optional<double> SelectTilt(const MediaTrackConstraintSetPlatform& basic_set) const
    {
        return SelectProperty(&PTZDeviceState::tilt_set_, basic_set, &MediaTrackConstraintSetPlatform::tilt);
    }

    std::optional<double> SelectZoom(const MediaTrackConstraintSetPlatform& basic_set) const
    {
        return SelectProperty(&PTZDeviceState::zoom_set_, basic_set, &MediaTrackConstraintSetPlatform::zoom);
    }
...
private:
    ...
    std::optional<double> SelectProperty(DoubleRangeSet PTZDeviceState::*ptz_field, const MediaTrackConstraintSetPlatform& basic_set,
        DoubleConstraint MediaTrackConstraintSetPlatform::*basic_set_field) const
    {
        if (!(basic_set.*basic_set_field).HasIdeal()) {
            return (this->*ptz_field).Min().has_value() ? (this->*ptz_field).Min() : (this->*ptz_field).Max();
        }

        auto ideal = (basic_set.*basic_set_field).Ideal();
        if ((this->*ptz_field).Min().has_value() && ideal < (this->*ptz_field).Min().value()) {
            return (this->*ptz_field).Min();
        }
        if ((this->*ptz_field).Max().has_value() && ideal > (this->*ptz_field).Max().value()) {
...
public:
        ...
        const MediaTrackConstraintSetPlatform& constraint_set, const char** failed_constraint_name = nullptr) const
    {
        ...
        if (!(TryToApplyConstraint(
                  constraint_set.exposure_compensation, exposure_compensation_set_, result->exposure_compensation_intersection_, failed_constraint_name)
                && TryToApplyConstraint(constraint_set.exposure_time, exposure_time_set_, result->exposure_time_intersection_, failed_constraint_name)
                && TryToApplyConstraint(
                    constraint_set.color_temperature, color_temperature_set_, result->color_temperature_intersection_, failed_constraint_name)
                && TryToApplyConstraint(constraint_set.iso, iso_set_, result->iso_intersection_, failed_constraint_name)
                && TryToApplyConstraint(constraint_set.brightness, brightness_set_, result->brightness_intersection_, failed_constraint_name)
                && TryToApplyConstraint(constraint_set.contrast, contrast_set_, result->contrast_intersection_, failed_constraint_name)
                && TryToApplyConstraint(constraint_set.saturation, saturation_set_, result->saturation_intersection_, failed_constraint_name)
                && TryToApplyConstraint(constraint_set.sharpness, sharpness_set_, result->sharpness_intersection_, failed_constraint_name)
                && TryToApplyConstraint(constraint_set.focus_distance, focus_distance_set_, result->focus_distance_intersection_, failed_constraint_name)
                && TryToApplyConstraint(constraint_set.torch, torch_set_, result->torch_intersection_, failed_constraint_name)
                && TryToApplyConstraint(constraint_set.background_blur, background_blur_set_, result->background_blur_intersection_, failed_constraint_name)
                && TryToApplyConstraint(constraint_set.background_segmentation_mask, background_segmentation_mask_set_,
                    result->background_segmentation_mask_intersection_, failed_constraint_name)
                && TryToApplyConstraint(
                    constraint_set.eye_gaze_correction, eye_gaze_correction_set_, result->eye_gaze_correction_intersection_, failed_constraint_name)
                && TryToApplyConstraint(constraint_set.face_framing, face_framing_set_, result->face_framing_intersection_, failed_constraint_name))) {
            result.reset();
        }
    ...
    void ApplyResult(const ApplyConstraintSetResult& result)
    {
        ...
        if (result.background_segmentation_mask_intersection_.has_value()) {
            background_segmentation_mask_set_ = *result.background_segmentation_mask_intersection_;
        }
    ...
    double Fitness(const MediaTrackConstraintSetPlatform& basic_constraint_set) const
    {
        return NumericRangeSetFitness(basic_constraint_set.exposure_compensation, exposure_compensation_set_)
            + NumericRangeSetFitness(basic_constraint_set.exposure_time, exposure_time_set_)
            + NumericRangeSetFitness(basic_constraint_set.color_temperature, color_temperature_set_)
            + NumericRangeSetFitness(basic_constraint_set.iso, iso_set_) + NumericRangeSetFitness(basic_constraint_set.brightness, brightness_set_)
            + NumericRangeSetFitness(basic_constraint_set.contrast, contrast_set_) + NumericRangeSetFitness(basic_constraint_set.saturation, saturation_set_)
            + NumericRangeSetFitness(basic_constraint_set.sharpness, sharpness_set_)
            + NumericRangeSetFitness(basic_constraint_set.focus_distance, focus_distance_set_) + BoolSetFitness(basic_constraint_set.torch, torch_set_)
            + BoolSetFitness(basic_constraint_set.background_blur, background_blur_set_)
            + BoolSetFitness(basic_constraint_set.background_segmentation_mask, background_segmentation_mask_set_)
            + BoolSetFitness(basic_constraint_set.eye_gaze_correction, eye_gaze_correction_set_)
            + BoolSetFitness(basic_constraint_set.face_framing, face_framing_set_);
    }

    std::optional<ImageCaptureDeviceSettings> SelectSettings(const MediaTrackConstraintSetPlatform& basic_constraint_set, const PTZDeviceState& ptz_state) const
    {
        std::optional<ImageCaptureDeviceSettings> settings(std::in_place);

        settings->exposure_compensation = SelectSetting(basic_constraint_set.exposure_compensation, exposure_compensation_set_);
        settings->exposure_time = SelectSetting(basic_constraint_set.exposure_time, exposure_time_set_);
        settings->color_temperature = SelectSetting(basic_constraint_set.color_temperature, color_temperature_set_);
        settings->iso = SelectSetting(basic_constraint_set.iso, iso_set_);
        settings->brightness = SelectSetting(basic_constraint_set.brightness, brightness_set_);
        settings->contrast = SelectSetting(basic_constraint_set.contrast, contrast_set_);
        settings->saturation = SelectSetting(basic_constraint_set.saturation, saturation_set_);
        settings->sharpness = SelectSetting(basic_constraint_set.sharpness, sharpness_set_);
        settings->focus_distance = SelectSetting(basic_constraint_set.focus_distance, focus_distance_set_);
        ...
        settings->background_blur = SelectSetting(basic_constraint_set.background_blur, background_blur_set_);
        settings->background_segmentation_mask = SelectSetting(basic_constraint_set.background_segmentation_mask, background_segmentation_mask_set_);
        settings->eye_gaze_correction = SelectSetting(basic_constraint_set.eye_gaze_correction, eye_gaze_correction_set_);
        settings->face_framing = SelectSetting(basic_constraint_set.face_framing, face_framing_set_);

        if (!(settings->exposure_compensation || settings->exposure_time || settings->color_temperature || settings->iso || settings->brightness
                || settings->contrast || settings->saturation || settings->sharpness || settings->focus_distance || settings->pan || settings->tilt
                || settings->zoom || settings->torch || settings->background_blur || settings->background_segmentation_mask || settings->eye_gaze_correction
...
private:
    std::optional<bool> SelectSetting(const BooleanConstraint& basic_constraint, const BoolSet& set) const
    {
    ...
    std::optional<double> SelectSetting(const DoubleConstraint& basic_constraint, const DoubleRangeSet& set) const
    {
        if (basic_constraint.HasIdeal()) {
            auto ideal = basic_constraint.Ideal();
            if (set.Contains(ideal)) {
    ...
    bool TryToApplyConstraint(
        const Constraint& constraint, const Set& current_set, std::optional<Set>& intersection, const char** failed_constraint_name = nullptr) const
    {
...
bool DeviceSatisfiesConstraintSet(
    const DeviceInfo& device, const MediaTrackConstraintSetPlatform& constraint_set, const char** failed_constraint_name = nullptr)
{
    if (!constraint_set.device_id.Matches(WebString(device.device_id))) {
        UpdateFailedConstraintName(constraint_set.device_id, failed_constraint_name);
        return false;
    }
    ...
    if (!FacingModeSatisfiesConstraint(device.facing_mode, constraint_set.facing_mode)) {
        UpdateFailedConstraintName(constraint_set.facing_mode, failed_constraint_name);
        return false;
    }
...
bool OptionalBoolSatisfiesConstraint(const std::optional<bool>& value, const BooleanConstraint& constraint, const char** failed_constraint_name = nullptr)
{
...
double DeviceFitness(const DeviceInfo& device, const MediaTrackConstraintSetPlatform& constraint_set)
{
    return StringConstraintFitnessDistance(WebString(device.device_id), constraint_set.device_id)
        + StringConstraintFitnessDistance(WebString(device.group_id), constraint_set.group_id)
        + StringConstraintFitnessDistance(ToWebString(device.facing_mode), constraint_set.facing_mode);
}
...
double CandidateFitness(const DeviceInfo& device, const PTZDeviceState& ptz_state, const CandidateFormat& candidate_format,
    const ImageCaptureDeviceState& image_capture_device_state, const std::optional<bool>& noise_reduction,
    const MediaTrackConstraintSetPlatform& constraint_set, VideoTrackAdapterSettings* track_settings)
{
    return DeviceFitness(device, constraint_set) + ptz_state.Fitness(constraint_set, device.control_support)
        + candidate_format.Fitness(constraint_set, track_settings) + image_capture_device_state.Fitness(constraint_set)
...
void AppendDistancesFromDefault(const DeviceInfo& device, const CandidateFormat& candidate_format, const std::optional<bool>& noise_reduction,
    const VideoDeviceCaptureCapabilities& capabilities, int default_width, int default_height, double default_frame_rate, DistanceVector* distance_vector)
{
    // Favor IDs that appear first in the enumeration.
    for (WTF::wtf_size_t i = 0; i < capabilities.device_capabilities.size(); ++i) {
    ...
    double resolution_distance = ResolutionSet::Point::SquareEuclideanDistance(
        ResolutionSet::Point(candidate_format.NativeHeight(), candidate_format.NativeWidth()), ResolutionSet::Point(default_height, default_width));
    distance_vector->push_back(resolution_distance);

    // Prefer a native frame rate close to the default.
    double frame_rate_distance = NumericConstraintFitnessDistance(candidate_format.NativeFrameRate(), default_frame_rate);
...
VideoInputDeviceCapabilities::VideoInputDeviceCapabilities(String device_id, String group_id, const media::VideoCaptureControlSupport& control_support,
    Vector<media::VideoCaptureFormat> formats, mojom::blink::FacingMode facing_mode)
...
VideoInputDeviceCapabilities::VideoInputDeviceCapabilities(VideoInputDeviceCapabilities&& other) = default;
VideoInputDeviceCapabilities& VideoInputDeviceCapabilities::operator=(VideoInputDeviceCapabilities&& other) = default;

VideoInputDeviceCapabilities::~VideoInputDeviceCapabilities() = default;

MediaStreamTrackPlatform::FacingMode ToPlatformFacingMode(mojom::blink::FacingMode video_facing)
{
    switch (video_facing) {
...
VideoDeviceCaptureCapabilities::VideoDeviceCaptureCapabilities(VideoDeviceCaptureCapabilities&& other) = default;
VideoDeviceCaptureCapabilities::~VideoDeviceCaptureCapabilities() = default;
VideoDeviceCaptureCapabilities& VideoDeviceCaptureCapabilities::operator=(VideoDeviceCaptureCapabilities&& other) = default;

VideoCaptureSettings SelectSettingsVideoDeviceCapture(
    const VideoDeviceCaptureCapabilities& capabilities, const MediaConstraints& constraints, int default_width, int default_height, double default_frame_rate)
{
    ...
    DistanceVector best_distance(constraints.Advanced().size() + 2 + kNumDefaultDistanceEntries);
    ...
    for (auto& device : capabilities.device_capabilities) {
        ...
        if (auto image_capture_device_result = image_capture_device_state.TryToApplyConstraintSet(constraints.Basic(), &failed_constraint_name)) {
        ...
        for (auto& format : device.formats) {
            ...
            if (auto candidate_format_result = candidate_format.TryToApplyConstraintSet(constraints.Basic(), &failed_constraint_name)) {
            ...
            for (auto& noise_reduction : capabilities.noise_reduction_capabilities) {
                if (!OptionalBoolSatisfiesConstraint(noise_reduction, constraints.Basic().goog_noise_reduction, &failed_constraint_name)) {
                    continue;
                }
                ...
                for (const auto& advanced_set : constraints.Advanced()) {
                    PTZDeviceState ptz_advanced_state = ptz_state_for_format.Intersection(advanced_set);
                    bool satisfies_advanced_set = false;

                    if (DeviceSatisfiesConstraintSet(device, advanced_set) && !ptz_advanced_state.IsEmpty()
                        && OptionalBoolSatisfiesConstraint(noise_reduction, advanced_set.goog_noise_reduction)) {
                        if (auto candidate_format_result = candidate_format.TryToApplyConstraintSet(advanced_set)) {
                            if (auto image_capture_device_result = image_capture_device_state.TryToApplyConstraintSet(advanced_set)) {
                ...
                candidate_distance_vector.push_back(CandidateFitness(
                    device, ptz_state_for_format, candidate_format, image_capture_device_state, noise_reduction, constraints.Basic(), &track_settings));

                // Third criterion is native fitness distance.
                candidate_distance_vector.push_back(candidate_format.NativeFitness(constraints.Basic()));

                // Final criteria are custom distances to default settings.
                AppendDistancesFromDefault(
                    device, candidate_format, noise_reduction, capabilities, default_width, default_height, default_frame_rate, &candidate_distance_vector);

                DCHECK_EQ(best_distance.size(), candidate_distance_vector.size());
                if (std::lexicographical_compare(
                        candidate_distance_vector.begin(), candidate_distance_vector.end(), best_distance.begin(), best_distance.end())) {
                    best_distance = candidate_distance_vector;

                    media::VideoCaptureParams capture_params;
                    capture_params.requested_format = candidate_format.format();
                    result = VideoCaptureSettings(device.device_id.Utf8(), capture_params, noise_reduction, track_settings,
                        candidate_format.constrained_frame_rate().Min(), candidate_format.constrained_frame_rate().Max(),
                        image_capture_device_state.SelectSettings(constraints.Basic(), ptz_state_for_format));
                }
            }
        }
    }
...
base::expected<Vector<VideoCaptureSettings>, std::string> SelectEligibleSettingsVideoDeviceCapture(
    const VideoDeviceCaptureCapabilities& capabilities, const MediaConstraints& constraints, int default_width, int default_height, double default_frame_rate)
{
    ...
    for (const auto& device : capabilities.device_capabilities) {
        ...
        device_capabilities.device_capabilities.emplace_back(device.device_id, device.group_id, device.control_support, device.formats, device.facing_mode);
        device_capabilities.noise_reduction_capabilities = capabilities.noise_reduction_capabilities;
        const auto device_settings = SelectSettingsVideoDeviceCapture(device_capabilities, constraints, default_width, default_height, default_frame_rate);
