...
// static
MediaStreamVideoSource* MediaStreamVideoSource::GetVideoSource(MediaStreamSource* source)
{
...
MediaStreamVideoSource::MediaStreamVideoSource(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
void MediaStreamVideoSource::AddTrack(MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& track_adapter_settings,
    const VideoCaptureDeliverFrameCB& frame_callback, const VideoCaptureNotifyFrameDroppedCB& notify_frame_dropped_callback,
    const EncodedVideoFrameCB& encoded_frame_callback, const VideoCaptureSubCaptureTargetVersionCB& sub_capture_target_version_callback,
    const VideoTrackSettingsCallback& settings_callback, const VideoTrackFormatCallback& format_callback, ConstraintsOnceCallback callback)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    ...
    pending_tracks_.push_back(
        PendingTrackInfo { track, frame_callback, notify_frame_dropped_callback, encoded_frame_callback, sub_capture_target_version_callback, settings_callback,
            format_callback, std::make_unique<VideoTrackAdapterSettings>(track_adapter_settings), std::move(callback) });

    switch (state_) {
    case NEW: {
        state_ = STARTING;
        auto deliver_frame_on_video_callback
            = ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&VideoTrackAdapter::DeliverFrameOnVideoTaskRunner, GetTrackAdapter()));
        auto deliver_encoded_frame_on_video_callback
            = ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&VideoTrackAdapter::DeliverEncodedVideoFrameOnVideoTaskRunner, GetTrackAdapter()));
        auto new_sub_capture_target_version_on_video_callback
            = ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&VideoTrackAdapter::NewSubCaptureTargetVersionOnVideoTaskRunner, GetTrackAdapter()));
        VideoCaptureNotifyFrameDroppedCB frame_dropped_callback
            = ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&VideoTrackAdapter::OnFrameDroppedOnVideoTaskRunner, GetTrackAdapter()));
        ...
        if (base::FeatureList::IsEnabled(features::kUseThreadPoolForMediaStreamVideoTaskRunner)) {
            StartSourceImpl(base::BindPostTask(video_task_runner(), std::move(deliver_frame_on_video_callback)),
                base::BindPostTask(video_task_runner(), std::move(deliver_encoded_frame_on_video_callback)),
                base::BindPostTask(video_task_runner(), std::move(new_sub_capture_target_version_on_video_callback)),
                base::BindPostTask(video_task_runner(), std::move(frame_dropped_callback)));
        } else {
            StartSourceImpl(std::move(deliver_frame_on_video_callback), std::move(deliver_encoded_frame_on_video_callback),
                std::move(new_sub_capture_target_version_on_video_callback), std::move(frame_dropped_callback));
        }
        break;
    }
    case STARTING:
    ...
    case ENDED: {
        FinalizeAddPendingTracks(mojom::blink::MediaStreamRequestResult::TRACK_START_FAILURE_VIDEO);
        break;
    }
    case STARTED: {
...
void MediaStreamVideoSource::RemoveTrack(MediaStreamVideoTrack* video_track, base::OnceClosure callback)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    {
    ...
    if (tracks_.empty()) {
        if (callback) {
            ...
            StopForRestart(WTF::BindOnce(&MediaStreamVideoSource::DidStopSource, GetWeakPtr()));
...
void MediaStreamVideoSource::ReconfigureTrack(MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& adapter_settings)
{
...
void MediaStreamVideoSource::StopForRestart(RestartCallback callback, bool send_black_frame)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    if (state_ != STARTED) {
        GetTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(std::move(callback), RestartResult::INVALID_STATE));
        return;
    }
    ...
    if (send_black_frame) {
        const std::optional<gfx::Size> source_size = GetTrackAdapter()->source_frame_size();
        scoped_refptr<media::VideoFrame> black_frame
            = media::VideoFrame::CreateBlackFrame(source_size.has_value() ? *source_size : gfx::Size(kDefaultWidth, kDefaultHeight));
        PostCrossThreadTask(*video_task_runner(), FROM_HERE,
            CrossThreadBindOnce(&VideoTrackAdapter::DeliverFrameOnVideoTaskRunner, GetTrackAdapter(), black_frame, base::TimeTicks::Now()));
    }
...
void MediaStreamVideoSource::OnStopForRestartDone(bool did_stop_for_restart)
{
    ...
    RestartResult result = did_stop_for_restart ? RestartResult::IS_STOPPED : RestartResult::IS_RUNNING;
    GetTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(std::move(restart_callback_), result));
}

void MediaStreamVideoSource::Restart(const media::VideoCaptureFormat& new_format, RestartCallback callback)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
...
void MediaStreamVideoSource::RestartSourceImpl(const media::VideoCaptureFormat& new_format)
{
    NOTREACHED();
}
...
void MediaStreamVideoSource::OnRestartDone(bool did_restart)
{
    ...
    DCHECK(restart_callback_);
    RestartResult result = did_restart ? RestartResult::IS_RUNNING : RestartResult::IS_STOPPED;
    GetTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(std::move(restart_callback_), result));
}
...
void MediaStreamVideoSource::UpdateHasConsumers(MediaStreamVideoTrack* track, bool has_consumers)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
...
void MediaStreamVideoSource::UpdateCapturingLinkSecure(MediaStreamVideoTrack* track, bool is_secure)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
...
void MediaStreamVideoSource::NotifyCapturingLinkSecured(size_t num_encoded_sinks)
{
    // Encoded sinks imply insecure sinks.
    OnCapturingLinkSecured(secure_tracker_.is_capturing_secure() && num_encoded_sinks == 0);
}
...
std::optional<media::VideoCaptureFormat> MediaStreamVideoSource::GetCurrentFormat() const
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
...
size_t MediaStreamVideoSource::CountEncodedSinks() const
{
    return std::accumulate(
        tracks_.begin(), tracks_.end(), size_t(0), [](size_t accum, MediaStreamVideoTrack* track) { return accum + track->CountEncodedSinks(); });
}
...
void MediaStreamVideoSource::DoChangeSource(const MediaStreamDevice& new_device)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    DVLOG(1) << "MediaStreamVideoSource::DoChangeSource: "
             << ", new device id = " << new_device.id << ", session id = " << new_device.session_id();
...
void MediaStreamVideoSource::OnStartDone(mojom::blink::MediaStreamRequestResult result)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    DVLOG(3) << "OnStartDone({result =" << result << "})";
    if (state_ == ENDED) {
        OnLog("MediaStreamVideoSource::OnStartDone dropping event because state_ == "
              "ENDED.");
        return;
    }
...
void MediaStreamVideoSource::FinalizeAddPendingTracks(mojom::blink::MediaStreamRequestResult result)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    ...
    for (auto& track_info : pending_track_descriptors) {
        if (result == mojom::blink::MediaStreamRequestResult::OK) {
            GetTrackAdapter()->AddTrack(track_info.track, track_info.frame_callback, track_info.notify_frame_dropped_callback,
                track_info.encoded_frame_callback, track_info.sub_capture_target_version_callback, track_info.settings_callback, track_info.format_callback,
        ...
        if (!track_info.callback.is_null()) {
            OnLog("MediaStreamVideoSource invoking callback indicating result of "
                  "starting track.");
            std::move(track_info.callback).Run(this, result, WebString());
        } else {
            OnLog("MediaStreamVideoSource dropping event indicating result of starting "
                  "track.");
        }
    }
}
...
void MediaStreamVideoSource::StartFrameMonitoring()
{
    ...
    GetTrackAdapter()->StartFrameMonitoring(frame_rate, WTF::BindRepeating(&MediaStreamVideoSource::SetMutedState, GetWeakPtr()));
}

void MediaStreamVideoSource::SetReadyState(WebMediaStreamSource::ReadyState state)
{
...
void MediaStreamVideoSource::SetMutedState(bool muted_state)
{
    ...
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    if (!Owner().IsNull()) {
        Owner().SetReadyState(muted_state ? WebMediaStreamSource::kReadyStateMuted : WebMediaStreamSource::kReadyStateLive);
    }
}

void MediaStreamVideoSource::UpdateTrackSettings(MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& adapter_settings)
{
    ...
    if (VideoTrackAdapter::CalculateDesiredSize(false /* is_rotated */, GetCurrentFormat()->frame_size, adapter_settings, &desired_size)) {
...
void MediaStreamVideoSource::ApplySubCaptureTarget(media::mojom::blink::SubCaptureTargetType type, const base::Token& sub_capture_target,
    uint32_t sub_capture_target_version, base::OnceCallback<void(media::mojom::ApplySubCaptureTargetResult)> callback)
{
    std::move(callback).Run(media::mojom::ApplySubCaptureTargetResult::kErrorGeneric);
}

std::optional<uint32_t> MediaStreamVideoSource::GetNextSubCaptureTargetVersion()
{
    return std::nullopt;
}
#endif
...
scoped_refptr<VideoTrackAdapter> MediaStreamVideoSource::GetTrackAdapter()
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    if (!track_adapter_) {
        track_adapter_ = base::MakeRefCounted<VideoTrackAdapter>(video_task_runner(), GetWeakPtr());
    }
