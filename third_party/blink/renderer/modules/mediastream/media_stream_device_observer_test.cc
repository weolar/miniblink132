...
public:
    ...
    void OnDeviceOpened(base::OnceClosure quit_closure, bool success, const String& label, const blink::MediaStreamDevice& device)
    {
        if (success) {
            stream_label_ = label;
    ...
    void AddStreams(const WTF::String& streams_label, WebMediaStreamDeviceObserver::OnDeviceStoppedCb device_stopped_callback,
        WebMediaStreamDeviceObserver::OnDeviceRequestStateChangeCb request_state_change_callback)
    {
        ...
        stream_devices_set.stream_devices.push_back(blink::mojom::blink::StreamDevices::New(
            std::nullopt, MediaStreamDevice(blink::mojom::blink::MediaStreamType::DISPLAY_VIDEO_CAPTURE_SET, "device_0_id", "device_0_name")));
        stream_devices_set.stream_devices.push_back(blink::mojom::blink::StreamDevices::New(
            std::nullopt, MediaStreamDevice(blink::mojom::blink::MediaStreamType::DISPLAY_VIDEO_CAPTURE_SET, "device_1_id", "device_1_name")));

        observer_->AddStreams(streams_label, stream_devices_set, {
            .on_device_stopped_cb = device_stopped_callback, .on_device_changed_cb = base::DoNothing(),
            .on_device_request_state_change_cb = request_state_change_callback, .on_device_capture_configuration_change_cb = base::DoNothing(),
    ...
    void CheckStreamDeviceIds(const WTF::Vector<MediaStreamDeviceObserver::Stream>& streams, const std::vector<std::string>& expected_labels) const
    {
        EXPECT_EQ(streams.size(), expected_labels.size());
        for (size_t stream_index = 0; stream_index < streams.size(); ++stream_index) {
            EXPECT_EQ(streams[static_cast<WTF::wtf_size_t>(stream_index)].video_devices.size(), 1u);
            EXPECT_EQ(streams[static_cast<WTF::wtf_size_t>(stream_index)].video_devices[0].id, expected_labels[stream_index]);
        }
    }

    const WTF::Vector<MediaStreamDeviceObserver::Stream>& GetStreams(const WTF::String& label) const
    {
    ...
    const MediaStreamDeviceObserver::Stream& GetStream(const WTF::String& label, WTF::wtf_size_t stream_index) const
    {
        return GetStreams(label)[stream_index];
    }

    void SetupMultiStreams(WebMediaStreamDeviceObserver::OnDeviceStoppedCb device_stopped_callback,
        WebMediaStreamDeviceObserver::OnDeviceRequestStateChangeCb request_state_change_callback)
    {
        const WTF::String streams_0_label = "label_0";
        AddStreams(streams_0_label, std::move(device_stopped_callback), std::move(request_state_change_callback));
        const WTF::Vector<MediaStreamDeviceObserver::Stream>& streams_0 = GetStreams(streams_0_label);
...
TEST_F(MediaStreamDeviceObserverTest, GetNonScreenCaptureDevices)
{
    ...
    mock_dispatcher_host_.OpenDevice(kRequestId1, "device_path", blink::mojom::MediaStreamType::DEVICE_VIDEO_CAPTURE,
        base::BindOnce(&MediaStreamDeviceObserverTest::OnDeviceOpened, base::Unretained(this), run_loop1.QuitClosure()));
    run_loop1.Run();
    String stream_label1 = stream_label_;

    // OpenDevice request 2
    base::RunLoop run_loop2;
    mock_dispatcher_host_.OpenDevice(kRequestId2, "screen_capture", blink::mojom::MediaStreamType::GUM_DESKTOP_VIDEO_CAPTURE,
        base::BindOnce(&MediaStreamDeviceObserverTest::OnDeviceOpened, base::Unretained(this), run_loop2.QuitClosure()));
    run_loop2.Run();
    ...
    blink::MediaStreamDevices video_devices = observer_->GetNonScreenCaptureDevices();
    EXPECT_EQ(video_devices.size(), 1u);
    EXPECT_EQ(video_devices[0].type, blink::mojom::MediaStreamType::DEVICE_VIDEO_CAPTURE);
...
TEST_F(MediaStreamDeviceObserverTest, OnDeviceStopped)
{
    ...
    // OpenDevice request.
    base::RunLoop run_loop1;
    mock_dispatcher_host_.OpenDevice(kRequestId, "device_path", blink::mojom::MediaStreamType::DEVICE_VIDEO_CAPTURE,
        base::BindOnce(&MediaStreamDeviceObserverTest::OnDeviceOpened, base::Unretained(this), run_loop1.QuitClosure()));
    run_loop1.Run();
...
TEST_F(MediaStreamDeviceObserverTest, OnDeviceChanged)
{
    ...
    // OpenDevice request.
    base::RunLoop run_loop1;
    mock_dispatcher_host_.OpenDevice(kRequestId1, example_video_id1, blink::mojom::MediaStreamType::DEVICE_VIDEO_CAPTURE,
        base::BindOnce(&MediaStreamDeviceObserverTest::OnDeviceOpened, base::Unretained(this), run_loop1.QuitClosure()));
    run_loop1.Run();
    ...
    blink::MediaStreamDevice fake_video_device(blink::mojom::MediaStreamType::DEVICE_VIDEO_CAPTURE, example_video_id2.Utf8(), "Fake Video Device");
...
TEST_F(MediaStreamDeviceObserverTest, OnDeviceChangedChangesDeviceAfterRebind)
{
    ...
    const blink::mojom::MediaStreamType kDeviceType = blink::mojom::MediaStreamType::DEVICE_VIDEO_CAPTURE;

    // Add a device to the |observer_|, to be changed using OnChangedDevice().
    blink::MediaStreamDevice initial_device(kDeviceType, "initial_device", kDeviceName);
    ...
    observer_->BindMediaStreamDeviceObserverReceiver(remote_observer.BindNewPipeAndPassReceiver());
    ...
    blink::MediaStreamDevice changed_device = blink::MediaStreamDevice(kDeviceType, "video_device-123", kDeviceName);
    remote_observer->OnDeviceChanged(kStreamLabel, initial_device, changed_device);
    ...
    observer_->BindMediaStreamDeviceObserverReceiver(remote_observer.BindNewPipeAndPassReceiver());
    remote_observer->OnDeviceChanged(kStreamLabel, changed_device, blink::MediaStreamDevice(kDeviceType, "video_device-456", kDeviceName));
    base::RunLoop().RunUntilIdle();
    video_devices = observer_->GetNonScreenCaptureDevices();
    ASSERT_EQ(video_devices.size(), 1u);
...
TEST_F(MediaStreamDeviceObserverTest, OnDeviceRequestStateChange)
{
    ...
    observer_->OnDeviceRequestStateChange(stream_label_, current_device_, mojom::blink::MediaStreamStateChange::PAUSE);

    EXPECT_EQ(observer_->label_stream_map_.size(), 1u);

    observer_->OnDeviceRequestStateChange(stream_label_, current_device_, mojom::blink::MediaStreamStateChange::PLAY);
...
TEST_F(MediaStreamDeviceObserverTest, MultiCaptureAddAndRemoveStreams)
{
    ...
    SetupMultiStreams(
        /*device_stopped_callback=*/base::BindLambdaForTesting(
            [&latest_stopped_device_id](const MediaStreamDevice& device) { latest_stopped_device_id = device.id; }),
        /*request_state_change_callback=*/base::DoNothing());

    MediaStreamDevice stopped_device_0(blink::mojom::blink::MediaStreamType::DISPLAY_VIDEO_CAPTURE_SET, "device_0_id", "device_0_name");
    observer_->OnDeviceStopped(streams_label, stopped_device_0);
    const WTF::Vector<MediaStreamDeviceObserver::Stream>& streams = GetStreams(streams_label);
    CheckStreamDeviceIds(streams, { "device_1_id" });
    EXPECT_EQ(latest_stopped_device_id, "device_0_id");

    MediaStreamDevice stopped_device_1(blink::mojom::blink::MediaStreamType::DISPLAY_VIDEO_CAPTURE_SET, "device_1_id", "device_1_name");
...
TEST_F(MediaStreamDeviceObserverTest, MultiCaptureChangeDeviceRequestState)
{
    ...
    blink::mojom::blink::MediaStreamStateChange latest_device_state = blink::mojom::blink::MediaStreamStateChange::PAUSE;
    SetupMultiStreams(
        /*device_stopped_callback=*/base::DoNothing(),
        /*request_state_change_callback=*/
        base::BindLambdaForTesting(
            [&latest_changed_device_id, &latest_device_state](const MediaStreamDevice& device, const mojom::MediaStreamStateChange new_state) {
                latest_changed_device_id = device.id;
                latest_device_state = new_state;
            }));
    EXPECT_EQ(latest_changed_device_id, "");

    observer_->OnDeviceRequestStateChange(streams_label, GetStream(streams_label, 0).video_devices[0], blink::mojom::blink::MediaStreamStateChange::PLAY);
    EXPECT_EQ(latest_changed_device_id, "device_0_id");
    EXPECT_EQ(latest_device_state, blink::mojom::blink::MediaStreamStateChange::PLAY);

    observer_->OnDeviceRequestStateChange(streams_label, GetStream(streams_label, 1).video_devices[0], blink::mojom::blink::MediaStreamStateChange::PAUSE);
    EXPECT_EQ(latest_changed_device_id, "device_1_id");
    EXPECT_EQ(latest_device_state, blink::mojom::blink::MediaStreamStateChange::PAUSE);
}
...
TEST_F(MediaStreamDeviceObserverTest, MultiCaptureRemoveStreamDevice)
{
    ...
    AddStreams(streams_1_label, /*device_stopped_callback=*/base::DoNothing(),
        /*request_state_change_callback=*/base::DoNothing());
    const WTF::Vector<MediaStreamDeviceObserver::Stream>& streams_1 = GetStreams(streams_1_label);
...
{
    const String kStreamLabel = "stream_label";
    blink::MediaStreamDevice device(blink::mojom::MediaStreamType::DEVICE_VIDEO_CAPTURE, "device_id", "device_name");

    blink::mojom::blink::StreamDevicesSet stream_devices_set;
    stream_devices_set.stream_devices.push_back(blink::mojom::blink::StreamDevices::New(std::nullopt, device));
    observer_->AddStreams(kStreamLabel, stream_devices_set,
        {
            ...
            .on_device_capture_handle_change_cb = base::DoNothing(),
            .on_zoom_level_change_cb = base::BindRepeating([](const MediaStreamDevice& device, int zoom_level) {
                EXPECT_EQ(device.type, blink::mojom::MediaStreamType::DEVICE_VIDEO_CAPTURE);
                EXPECT_EQ(device.id, "device_id");
                EXPECT_EQ(device.name, "device_name");
                EXPECT_EQ(zoom_level, CaptureController::getSupportedZoomLevels()[0]);
            }),
        });
    static_cast<mojom::blink::MediaStreamDeviceObserver*>(observer_.get())
        ->OnZoomLevelChange(kStreamLabel, device, CaptureController::getSupportedZoomLevels()[0]);
}
#endif
