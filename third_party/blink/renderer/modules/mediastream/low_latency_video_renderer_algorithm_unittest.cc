...
public:
    ...
    LowLatencyVideoRendererAlgorithmTest(const LowLatencyVideoRendererAlgorithmTest&) = delete;
    LowLatencyVideoRendererAlgorithmTest& operator=(const LowLatencyVideoRendererAlgorithmTest&) = delete;

    ~LowLatencyVideoRendererAlgorithmTest() override = default;

    scoped_refptr<media::VideoFrame> CreateFrame(int maximum_composition_delay_in_frames)
    {
        const gfx::Size natural_size(8, 8);
        scoped_refptr<media::VideoFrame> frame
            = frame_pool_.CreateFrame(media::PIXEL_FORMAT_I420, natural_size, gfx::Rect(natural_size), natural_size, base::TimeDelta());
        frame->metadata().maximum_composition_delay_in_frames = maximum_composition_delay_in_frames;
        return frame;
    }

    media::VideoFrame::ID CreateAndEnqueueFrame(int max_composition_delay_in_frames)
    {
        scoped_refptr<media::VideoFrame> frame = CreateFrame(max_composition_delay_in_frames);
    ...
    {
        constexpr base::TimeDelta kRenderInterval = base::Milliseconds(1000.0 / 60.0); // 60fps.
        return RenderAndStep(frames_dropped, kRenderInterval);
    }

    scoped_refptr<media::VideoFrame> RenderAndStep(size_t* frames_dropped, base::TimeDelta render_interval)
    {
    ...
    scoped_refptr<media::VideoFrame> RenderWithGlitchAndStep(size_t* frames_dropped, double deadline_begin_error, double deadline_end_error)
    {
        constexpr base::TimeDelta kRenderInterval = base::Milliseconds(1000.0 / 60.0); // 60fps.
        return RenderAndStep(frames_dropped, kRenderInterval);
    }
    ...
    scoped_refptr<media::VideoFrame> RenderWithGlitchAndStep(
        size_t* frames_dropped, base::TimeDelta render_interval, double deadline_begin_error, double deadline_end_error)
    {
        const base::TimeTicks start = current_render_time_ + deadline_begin_error * render_interval;
        current_render_time_ += render_interval;
        const base::TimeTicks end = current_render_time_ + deadline_end_error * render_interval;
        return algorithm_.Render(start, end, frames_dropped);
    }

    void StepUntilJustBeforeNextFrameIsRendered(base::TimeDelta render_interval, std::optional<media::VideoFrame::ID> expected_id = std::nullopt)
    {
        ...
        while (current_render_time_ - start_time + render_interval < FrameDuration()) {
            scoped_refptr<media::VideoFrame> rendered_frame = RenderAndStep(nullptr, render_interval);
            if (expected_id) {
...
TEST_F(LowLatencyVideoRendererAlgorithmTest, NormalMode60Hz)
{
    ...
    for (int i = 0; i < kNumberOfFrames; ++i) {
        ...
        scoped_refptr<media::VideoFrame> rendered_frame = RenderAndStep(&frames_dropped);
        ASSERT_TRUE(rendered_frame);
...
{
    constexpr base::TimeDelta kRenderInterval = base::Milliseconds(1000.0 / 30.0); // 30Hz.
    constexpr int kMaxCompositionDelayInFrames = 6;
    ...
    for (size_t i = 0; i < kNumberOfFrames; ++i) {
        ...
        size_t frames_dropped = 0;
        scoped_refptr<media::VideoFrame> rendered_frame = RenderAndStep(&frames_dropped, kRenderInterval);
        ASSERT_TRUE(rendered_frame);
...
{
    constexpr base::TimeDelta kRenderInterval = base::Milliseconds(1000.0 / 90.0); // 90Hz.
    constexpr int kMaxCompositionDelayInFrames = 6;
...
{
    constexpr base::TimeDelta kRenderInterval = base::Milliseconds(1000.0 / 120.0); // 120Hz.
    constexpr int kMaxCompositionDelayInFrames = 6;
...
{
    constexpr base::TimeDelta kRenderInterval = base::Milliseconds(1000.0 / 600.0 + 1.0e-3); // 600Hz.
    constexpr int kMaxCompositionDelayInFrames = 6;
...
TEST_F(LowLatencyVideoRendererAlgorithmTest, DropAllFramesIfQueueExceedsMaxSize)
{
...
TEST_F(LowLatencyVideoRendererAlgorithmTest, EnterDrainMode60Hz)
{
    ...
    for (int i = 0; i < kNumberOfFramesSubmitted; ++i) {
        enqueued_frame_ids.push(CreateAndEnqueueFrame(kMaxCompositionDelayInFrames));
    }
...
TEST_F(LowLatencyVideoRendererAlgorithmTest, SteadyStateQueueReduction60Hz)
{
    ...
    for (size_t i = kInitialQueueSize; i < kNumberOfFramesSubmitted; ++i) {
        ...
        if (frames_dropped > 0) {
            ...
            EXPECT_GE(processed_frames_since_last_frame_drop, kMinimumNumberOfFramesBetweenDrops);
            enqueued_frame_ids.pop();
...
TEST_F(LowLatencyVideoRendererAlgorithmTest, RenderFrameImmediatelyAfterOutage)
{
    constexpr base::TimeDelta kRenderInterval = base::Milliseconds(1000.0 / 600.0 + 1.0e-3); // 600Hz.
    constexpr int kMaxCompositionDelayInFrames = 6;
    ...
    for (int outage_length = 0; outage_length < 100; ++outage_length) {
        ...
        CreateAndEnqueueFrame(kMaxCompositionDelayInFrames);
        scoped_refptr<media::VideoFrame> rendered_frame = RenderAndStep(nullptr, kRenderInterval);
        ASSERT_TRUE(rendered_frame);
        media::VideoFrame::ID frame_id_0 = rendered_frame->unique_id();
        StepUntilJustBeforeNextFrameIsRendered(kRenderInterval, rendered_frame->unique_id());
...
TEST_F(LowLatencyVideoRendererAlgorithmTest, NormalModeWithGlitch60Hz)
{
    ...
    for (int i = 0; i < kNumberOfFrames; ++i) {
        media::VideoFrame::ID frame_id = CreateAndEnqueueFrame(kMaxCompositionDelayInFrames);
        size_t frames_dropped = 0u;
        scoped_refptr<media::VideoFrame> rendered_frame = RenderWithGlitchAndStep(&frames_dropped, kDeadlineBeginErrorRate[i], kDeadlineEndErrorRate[i]);
        ASSERT_TRUE(rendered_frame);
        EXPECT_EQ(rendered_frame->unique_id(), frame_id);
        EXPECT_EQ(frames_dropped, 0u);
    }
...
TEST_F(LowLatencyVideoRendererAlgorithmTest, NormalModeWithGlitch120Hz)
{
    ...
    for (size_t i = 0; i < kNumberOfFrames; ++i) {
        ...
        scoped_refptr<media::VideoFrame> rendered_frame
            = RenderWithGlitchAndStep(&frames_dropped, kRenderInterval, kDeadlineBeginErrorRate[i], kDeadlineEndErrorRate[i]);
        ASSERT_TRUE(rendered_frame);
        media::VideoFrame::ID rendered_frame_id = last_id;
        EXPECT_EQ(rendered_frame->unique_id(), rendered_frame_id);
        ...
        // The same frame should be rendered.
        rendered_frame = RenderWithGlitchAndStep(&frames_dropped, kRenderInterval, kDeadlineBeginErrorRate[i], kDeadlineEndErrorRate[i]);
        ASSERT_TRUE(rendered_frame);
        EXPECT_EQ(rendered_frame->unique_id(), rendered_frame_id);
    }
}
