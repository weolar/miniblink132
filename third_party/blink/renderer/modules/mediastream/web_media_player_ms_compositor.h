...
class MODULES_EXPORT WebMediaPlayerMSCompositor : public cc::VideoFrameProvider {
public:
    ...
    WebMediaPlayerMSCompositor(scoped_refptr<base::SingleThreadTaskRunner> task_runner, scoped_refptr<base::SequencedTaskRunner> video_task_runner,
        MediaStreamDescriptor* media_stream_descriptor, std::unique_ptr<WebVideoFrameSubmitter> submitter, bool use_surface_layer,
        const base::WeakPtr<WebMediaPlayerMS>& player);
    ~WebMediaPlayerMSCompositor() override;

    WebMediaPlayerMSCompositor(const WebMediaPlayerMSCompositor&) = delete;
    WebMediaPlayerMSCompositor& operator=(const WebMediaPlayerMSCompositor&) = delete;
    ...
    virtual void EnableSubmission(const viz::SurfaceId& id, media::VideoTransformation transformation, bool force_submit);
    ...
    std::unique_ptr<WebMediaPlayer::VideoFramePresentationMetadata> GetLastPresentedFrameMetadata();
...
private:
    ...
    // ctor.
    bool MapTimestampsToRenderTimeTicks(const std::vector<base::TimeDelta>& timestamps, std::vector<base::TimeTicks>* wall_clock_times);
    ...
    void RenderUsingAlgorithm(base::TimeTicks deadline_min, base::TimeTicks deadline_max);
    ...
    void RenderWithoutAlgorithm(scoped_refptr<media::VideoFrame> frame, bool is_copy);
    void RenderWithoutAlgorithmOnCompositor(scoped_refptr<media::VideoFrame> frame, bool is_copy);

    // Update |current_frame_| and |dropped_frame_count_|
    void SetCurrentFrame(scoped_refptr<media::VideoFrame> frame, bool is_copy, std::optional<base::TimeTicks> expected_presentation_time);
    ...
    raw_ptr<cc::VideoFrameProvider::Client, DanglingUntriaged> video_frame_provider_client_;
    ...
    base::TimeDelta last_preferred_render_interval_ GUARDED_BY(current_frame_lock_);
