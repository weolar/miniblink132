...
String ContentHintToString(const WebMediaStreamTrack::ContentHintType& content_hint)
{
    switch (content_hint) {
...
V8MediaStreamTrackState ReadyStateToV8TrackState(const MediaStreamSource::ReadyState& ready_state)
{
...
// static
MediaStreamTrack* MediaStreamTrack::FromTransferredState(ScriptState* script_state, const TransferredValues& data)
{
    ...
    auto* window = DynamicTo<LocalDOMWindow>(ExecutionContext::From(script_state));
    if (!window) {
        return nullptr;
    }
    ...
    MediaConstraints audio = (data.kind == "audio") ? media_constraints_impl::Create() : MediaConstraints();
    MediaConstraints video = (data.kind == "video") ? media_constraints_impl::Create() : MediaConstraints();
    UserMediaRequest* const request = MakeGarbageCollected<UserMediaRequest>(window, user_media_client, UserMediaRequestType::kDisplayMedia, audio, video,
        /*should_prefer_current_tab=*/false,
        /*capture_controller=*/nullptr, MakeGarbageCollected<GetOpenDeviceRequestCallbacks>(), IdentifiableSurface());
    if (!request) {
        return nullptr;
    }
    ...
    TransferredMediaStreamTrack* transferred_media_stream_track = MakeGarbageCollected<TransferredMediaStreamTrack>(ExecutionContext::From(script_state), data);

    request->SetTransferData(data.session_id, data.transfer_id, transferred_media_stream_track);
    request->Start();
    ...
    if (track->GetWrapperTypeInfo() != data.track_impl_subtype) {
        NOTREACHED() << "transferred track should be " << data.track_impl_subtype->interface_name << " but instead it is "
...
// static
MediaStreamTrack::FromTransferredStateImplForTesting& MediaStreamTrack::GetFromTransferredStateImplForTesting()
{
    static base::NoDestructor<MediaStreamTrack::FromTransferredStateImplForTesting> impl;
    return *impl;
}
