...
BASE_FEATURE(kGetDisplayMediaScreenScaleFactor, "GetDisplayMediaScreenScaleFactor",
...
MediaStreamTrack* MediaStreamUtils::CreateLocalAudioTrack(ExecutionContext* execution_context, MediaStreamSource* source)
{
    DCHECK_EQ(source->GetType(), MediaStreamSource::kTypeAudio);
    DCHECK(!source->Remote());
    auto* component = MakeGarbageCollected<MediaStreamComponentImpl>(source, std::make_unique<MediaStreamAudioTrack>(/*is_local=*/true));
    MediaStreamAudioSource::From(component->Source())->ConnectToInitializedTrack(component);
...
{
    const gfx::Size kDefaultScreenSize(kDefaultScreenCastWidth, kDefaultScreenCastHeight);
    ...
    for (const display::ScreenInfo& info : infos.screen_infos) {
        ...
        if (base::FeatureList::IsEnabled(kGetDisplayMediaScreenScaleFactor) && info.device_scale_factor > 0) {
