...
MODULES_EXPORT BASE_DECLARE_FEATURE(kApplyConstraintsRestartsVideoContentSources);
...
class MODULES_EXPORT ApplyConstraintsProcessor final : public GarbageCollected<ApplyConstraintsProcessor> {
public:
    ...
    ApplyConstraintsProcessor& operator=(const ApplyConstraintsProcessor&) = delete;
    ...
    // processed.
    void ProcessRequest(blink::ApplyConstraintsRequest* request, base::OnceClosure callback);
...
private:
    ...
    void MaybeStopVideoDeviceSourceForRestart(const Vector<media::VideoCaptureFormat>& formats);
    void MaybeDeviceSourceStoppedForRestart(blink::MediaStreamVideoSource::RestartResult result);
    void FindNewFormatAndRestartDeviceSource(const Vector<media::VideoCaptureFormat>& formats);
    blink::VideoCaptureSettings SelectVideoDeviceSettings(Vector<media::VideoCaptureFormat> formats);

    // Helpers for video content-capture requests.
    void ProcessVideoContentRequest();
    void MaybeStopVideoContentSourceForRestart();
    void MaybeRestartStoppedVideoContentSource(blink::MediaStreamVideoSource::RestartResult result);
    ...
    void MaybeSourceRestarted(blink::MediaStreamVideoSource::RestartResult result);
    ...
    using ScopedMediaStreamTrace = media::TypedScopedAsyncTrace<media::TraceCategory::kMediaStream>;
