...
TEST(MediaTrackConstraintsTest, DoubleConstraint)
{
    ...
    EXPECT_TRUE(range_constraint.Matches(5.0 - DoubleConstraint::kConstraintEpsilon / 2));
    EXPECT_TRUE(range_constraint.Matches(6.5 + DoubleConstraint::kConstraintEpsilon / 2));
    ...
    EXPECT_TRUE(exact_constraint.Matches(5.0 - DoubleConstraint::kConstraintEpsilon / 2));
    EXPECT_TRUE(exact_constraint.Matches(5.0 + DoubleConstraint::kConstraintEpsilon / 2));
...
TEST(MediaTrackConstraintsTest, SetToString)
{
    ...
    EXPECT_EQ("width: {max: 240}, echoCancellation: {ideal: true}", the_set.ToString().Utf8());
}
...
TEST(MediaTrackConstraintsTest, ConstraintsToString)
{
    ...
    EXPECT_EQ("{width: {max: 240}, advanced: [{echoCancellation: {exact: true}}]}", the_constraints.ToString().Utf8());
...
TEST(MediaTrackConstraintsTest, ConvertWebConstraintsBasic)
{
    ...
    [[maybe_unused]] MediaTrackConstraints* output = media_constraints_impl::ConvertConstraints(input);
}
...
TEST(MediaTrackConstraintsTest, ConvertWebSingleStringConstraint)
{
    ...
    MediaTrackConstraints* output = media_constraints_impl::ConvertConstraints(input);
...
TEST(MediaTrackConstraintsTest, ConvertBlinkStringConstraint)
{
    ...
    output = media_constraints_impl::ConvertTrackConstraintsToMediaConstraints(input, error_message);
...
TEST(MediaTrackConstraintsTest, ConvertBlinkComplexStringConstraint)
{
    ...
    MediaTrackConstraints* input = MediaTrackConstraints::Create();
    MediaConstraints output;
    ConstrainDOMStringParameters* subparameter = ConstrainDOMStringParameters::Create();
    subparameter->setIdeal(MakeGarbageCollected<V8UnionStringOrStringSequence>("foo"));
    ...
    MediaTrackConstraints* recycled = media_constraints_impl::ConvertConstraints(output);
...
TEST(MediaTrackConstraintsTest, NakedIsExactInAdvanced)
{
    ...
    auto* parameter = MakeGarbageCollected<V8ConstrainDOMString>("foo");
    input->setFacingMode(parameter);
    HeapVector<Member<MediaTrackConstraintSet>> advanced(1, MediaTrackConstraintSet::Create());
    advanced[0]->setFacingMode(parameter);
    input->setAdvanced(advanced);

    String error_message;
    MediaConstraints output = media_constraints_impl::ConvertTrackConstraintsToMediaConstraints(input, error_message);
    ASSERT_TRUE(error_message.empty());
    ASSERT_TRUE(output.Basic().facing_mode.HasIdeal());
...
TEST(MediaTrackConstraintsTest, AdvancedParameterFails)
{
    test::TaskEnvironment task_environment;
    MediaTrackConstraints* input = MediaTrackConstraints::Create();
    String str(std::string(media_constraints_impl::kMaxConstraintStringLength + 1, 'a').c_str());
...
TEST(MediaTrackConstraintsTest, MaxLengthStringConstraintPasses)
{
    test::TaskEnvironment task_environment;
    MediaTrackConstraints* input = MediaTrackConstraints::Create();
    String str(std::string(media_constraints_impl::kMaxConstraintStringLength, 'a').c_str());
    auto* parameter = MakeGarbageCollected<V8ConstrainDOMString>(str);
...
TEST(MediaTrackConstraintsTest, MaxLengthStringSequenceConstraintPasses)
{
    ...
    MediaConstraints output = media_constraints_impl::ConvertTrackConstraintsToMediaConstraints(input, error_message);
    EXPECT_TRUE(error_message.empty());
    EXPECT_EQ(output.Basic().group_id.Ideal().size(), media_constraints_impl::kMaxConstraintStringSeqLength);
}
...
TEST(MediaTrackConstraintsTest, TooLongStringSequenceConstraintFails)
{
    ...
    input->setGroupId(parameter);
    String error_message;
    media_constraints_impl::ConvertTrackConstraintsToMediaConstraints(input, error_message);
    ASSERT_FALSE(error_message.empty());
    EXPECT_EQ(error_message, "Constraint string sequence too long.");
}

TEST(MediaTrackConstraintsTest, TooLongStringSequenceForDeviceIdConstraintFails)
{
    MediaTrackConstraints* input = MediaTrackConstraints::Create();
    Vector<String> sequence;
    sequence.Fill("a", media_constraints_impl::kMaxConstraintStringSeqLength + 1);
...
TEST(MediaTrackConstraintsTest, TooLongStringSequenceForFacingModeConstraintFails)
{
    MediaTrackConstraints* input = MediaTrackConstraints::Create();
    Vector<String> sequence;
    sequence.Fill("a", media_constraints_impl::kMaxConstraintStringSeqLength + 1);
...
TEST(MediaTrackConstraintsTest, TooLongStringSequenceForResizeModeConstraintFails)
{
    MediaTrackConstraints* input = MediaTrackConstraints::Create();
    Vector<String> sequence;
    sequence.Fill("a", media_constraints_impl::kMaxConstraintStringSeqLength + 1);
...
TEST(MediaTrackConstraintsTest, TooLongStringSequenceForDisplaySurfaceConstraintFails)
{
    MediaTrackConstraints* input = MediaTrackConstraints::Create();
    Vector<String> sequence;
    sequence.Fill("a", media_constraints_impl::kMaxConstraintStringSeqLength + 1);
