...
public:
    VideoRendererAlgorithmWrapper(const media::TimeSource::WallClockTimeCB& wall_clock_time_cb, media::MediaLog* media_log);

    scoped_refptr<media::VideoFrame> Render(base::TimeTicks deadline_min, base::TimeTicks deadline_max, size_t* frames_dropped);
    void EnqueueFrame(scoped_refptr<media::VideoFrame> frame);
    void Reset(media::VideoRendererAlgorithm::ResetFlag reset_flag = media::VideoRendererAlgorithm::ResetFlag::kEverything);

    size_t frames_queued() const
    {
        return renderer_algorithm_ == RendererAlgorithm::kDefault ? default_rendering_frame_buffer_->frames_queued()
                                                                  : low_latency_rendering_frame_buffer_->frames_queued();
    }

    base::TimeDelta average_frame_duration() const
    {
        return renderer_algorithm_ == RendererAlgorithm::kDefault ? default_rendering_frame_buffer_->average_frame_duration()
...
private:
    ...
    std::unique_ptr<media::VideoRendererAlgorithm> default_rendering_frame_buffer_;
    std::unique_ptr<LowLatencyVideoRendererAlgorithm> low_latency_rendering_frame_buffer_;
};
