...
public:
    MockLocalMediaStreamVideoSource()
        : blink::MediaStreamVideoSource(blink::scheduler::GetSingleThreadTaskRunnerForTesting())
    {
    }
...
{
    auto platform_video_source = std::make_unique<MockLocalMediaStreamVideoSource>();
    auto* platform_video_source_ptr = platform_video_source.get();
    MediaStreamSource* const test_video_source = MakeGarbageCollected<MediaStreamSource>(
        /*id=*/"test_source_1_id", MediaStreamSource::StreamType::kTypeVideo,
        /*name=*/"test_source_1_name", /*remote=*/false, std::move(platform_video_source));

    return MakeGarbageCollected<MediaStreamComponentImpl>(test_video_source,
        std::make_unique<MediaStreamVideoTrack>(platform_video_source_ptr, MediaStreamVideoSource::ConstraintsOnceCallback(),
            /*enabled=*/true));
}
...
TEST_F(MediaStreamSetTest, GetAllScreensMediaSingleMediaStreamInitialized)
{
    ...
    MediaStreamComponentVector video_component_vector = { MakeMockVideoComponent() };
    MediaStreamDescriptor* const descriptor = MakeGarbageCollected<MediaStreamDescriptor>(audio_component_vector, video_component_vector);
    MediaStreamDescriptorVector descriptors = { descriptor };
    base::RunLoop run_loop;
    media_stream_set_ = MakeGarbageCollected<MediaStreamSet>(
        v8_scope.GetExecutionContext(), descriptors, UserMediaRequestType::kAllScreensMedia, base::BindLambdaForTesting([&run_loop](MediaStreamVector streams) {
...
TEST_F(MediaStreamSetTest, GetAllScreensMediaMultipleMediaStreamsInitialized)
{
    ...
    MediaStreamComponentVector video_component_vector = { MakeMockVideoComponent() };
    MediaStreamDescriptor* const descriptor = MakeGarbageCollected<MediaStreamDescriptor>(audio_component_vector, video_component_vector);
    MediaStreamDescriptorVector descriptors = { descriptor, descriptor, descriptor, descriptor };
    base::RunLoop run_loop;
    media_stream_set_ = MakeGarbageCollected<MediaStreamSet>(
        v8_scope.GetExecutionContext(), descriptors, UserMediaRequestType::kAllScreensMedia, base::BindLambdaForTesting([&run_loop](MediaStreamVector streams) {
...
TEST_F(MediaStreamSetTest, GetDisplayMediaSingleMediaStreamInitialized)
{
    ...
    base::RunLoop run_loop;
    media_stream_set_ = MakeGarbageCollected<MediaStreamSet>(
        v8_scope.GetExecutionContext(), descriptors, UserMediaRequestType::kDisplayMedia, base::BindLambdaForTesting([&run_loop](MediaStreamVector streams) {
            EXPECT_EQ(streams.size(), 1u);
            run_loop.Quit();
        }));
    run_loop.Run();
