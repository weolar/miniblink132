...
public:
    ...
    LowLatencyVideoRendererAlgorithm(const LowLatencyVideoRendererAlgorithm&) = delete;
    LowLatencyVideoRendererAlgorithm& operator=(const LowLatencyVideoRendererAlgorithm&) = delete;
...
private:
    size_t DetermineModeAndNumberOfFramesToRender(double fractional_frames_to_render);

    bool ReduceSteadyStateQueue(size_t number_of_frames_to_render);

    void SelectNextAvailableFrameAndUpdateLastDeadline(base::TimeTicks deadline_min);
