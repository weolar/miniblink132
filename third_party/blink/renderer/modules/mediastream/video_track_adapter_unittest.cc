...
TEST(VideoTrackAdapterTest, ZeroInputArea)
{
    ...
    static_assert(kSmallDimension < kMaxWidth && kSmallDimension < kMaxHeight, "kSmallDimension must be less than kMaxWidth and kMaxHeight");
    static_assert(kLargeDimension > kMaxWidth && kLargeDimension > kMaxHeight, "kLargeDimension must be greater than kMaxWidth and kMaxHeight");
    const VideoTrackAdapterSettings kVideoTrackAdapterSettings(
        gfx::Size(kMaxWidth, kMaxHeight), 0.1 /* min_aspect_ratio */, 2.0 /* max_aspect_ratio */, 0.0 /* max_frame_rate */);
    const bool kIsRotatedValues[] = { true, false };

    for (bool is_rotated : kIsRotatedValues) {
        gfx::Size desired_size;

        VideoTrackAdapter::CalculateDesiredSize(is_rotated, gfx::Size(0, 0), kVideoTrackAdapterSettings, &desired_size);
        EXPECT_EQ(desired_size.width(), 0);
        EXPECT_EQ(desired_size.height(), 0);

        // Zero width.
        VideoTrackAdapter::CalculateDesiredSize(is_rotated, gfx::Size(0, kSmallDimension), kVideoTrackAdapterSettings, &desired_size);
        EXPECT_EQ(desired_size.width(), 0);
        EXPECT_EQ(desired_size.height(), kSmallDimension);

        // Zero height.
        VideoTrackAdapter::CalculateDesiredSize(is_rotated, gfx::Size(kSmallDimension, 0), kVideoTrackAdapterSettings, &desired_size);
        EXPECT_EQ(desired_size.width(), kSmallDimension);
        EXPECT_EQ(desired_size.height(), 0);

        // Requires "cropping" of height.
        VideoTrackAdapter::CalculateDesiredSize(is_rotated, gfx::Size(0, kLargeDimension), kVideoTrackAdapterSettings, &desired_size);
        EXPECT_EQ(desired_size.width(), 0);
        EXPECT_EQ(desired_size.height(), is_rotated ? kMaxWidth : kMaxHeight);

        // Requires "cropping" of width.
        VideoTrackAdapter::CalculateDesiredSize(is_rotated, gfx::Size(kLargeDimension, 0), kVideoTrackAdapterSettings, &desired_size);
...
TEST(VideoTrackAdapterTest, ZeroOutputArea)
{
    ...
    static_assert(kSmallMaxDimension < kInputWidth && kSmallMaxDimension < kInputHeight, "kSmallMaxDimension must be less than kInputWidth and kInputHeight");
    static_assert(
        kLargeMaxDimension > kInputWidth && kLargeMaxDimension > kInputHeight, "kLargeMaxDimension must be greater than kInputWidth and kInputHeight");

    gfx::Size desired_size;

    VideoTrackAdapter::CalculateDesiredSize(false /* is_rotated */, gfx::Size(kInputWidth, kInputHeight),
        VideoTrackAdapterSettings(gfx::Size(0, 0), kMinAspectRatio, kMaxAspectRatio, 0.0 /* max_frame_rate */), &desired_size);
    EXPECT_EQ(desired_size.width(), 0);
    EXPECT_EQ(desired_size.height(), 0);
    ...
    VideoTrackAdapter::CalculateDesiredSize(false /* is_rotated */, gfx::Size(kInputWidth, kInputHeight),
        VideoTrackAdapterSettings(gfx::Size(0, kSmallMaxDimension), kMinAspectRatio, kMaxAspectRatio, 0.0 /* max_frame_rate */), &desired_size);
    EXPECT_EQ(desired_size.width(), 0);
    EXPECT_EQ(desired_size.height(), kSmallMaxDimension);

    // Height is cropped to zero.
    VideoTrackAdapter::CalculateDesiredSize(false /* is_rotated */, gfx::Size(kInputWidth, kInputHeight),
        VideoTrackAdapterSettings(gfx::Size(kLargeMaxDimension, 0), kMinAspectRatio, kMaxAspectRatio, 0.0 /* max_frame_rate */), &desired_size);
    ...
    VideoTrackAdapter::CalculateDesiredSize(false /* is_rotated */, gfx::Size(kInputWidth, kInputHeight),
        VideoTrackAdapterSettings(
            gfx::Size(kSmallMaxDimension /* max_width */, 0 /* max_height */), kMinAspectRatio, kMaxAspectRatio, 0.0 /* max_frame_rate */),
        &desired_size);
...
TEST(VideoTrackAdapterTest, LargeFrames)
{
    ...
    bool success = VideoTrackAdapter::CalculateDesiredSize(false /* is_rotated */, gfx::Size(kInputWidth, kInputHeight),
        VideoTrackAdapterSettings(gfx::Size(kMaxWidth, kMaxHeight), 0.0 /* max_frame_rate */), &desired_size);
    EXPECT_TRUE(success);
    ...
    success = VideoTrackAdapter::CalculateDesiredSize(false /* is_rotated */, gfx::Size(kInputWidth, kInputHeight), VideoTrackAdapterSettings(), &desired_size);
...
TEST(VideoTrackAdapterTest, NoRescaling)
{
    ...
    gfx::Size desired_size;
    bool success
        = VideoTrackAdapter::CalculateDesiredSize(false /* is_rotated */, gfx::Size(kInputWidth, kInputHeight), VideoTrackAdapterSettings(), &desired_size);
    EXPECT_TRUE(success);
    EXPECT_EQ(desired_size.width(), kInputWidth);
    EXPECT_EQ(desired_size.height(), kInputHeight);
}
...
public:
    VideoTrackAdapterFixtureTest()
        ...
        , frame_processed_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }
...
protected:
    void SetUp() override
    {
        testing_render_thread_.Start();
    }

    void TearDown() override
    {
        if (track_added_) {
            testing_render_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&VideoTrackAdapter::RemoveTrack, adapter_, null_track_.get()));
        }
        base::WaitableEvent source_deleted;
        testing_render_thread_.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
            mock_source_.reset();
    ...
    void CreateAdapter(media::VideoCaptureFormat capture_format)
    {
        ...
        base::WaitableEvent adapter_created(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
        testing_render_thread_.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
            mock_source_ = std::make_unique<NiceMock<MockMediaStreamVideoSource>>(capture_format, false);
            adapter_ = base::MakeRefCounted<VideoTrackAdapter>(platform_support_->GetIOTaskRunner(), mock_source_->GetWeakPtr());
    ...
    void ConfigureTrack(const VideoTrackAdapterSettings& adapter_settings)
    {
        if (!track_added_) {
            ...
            testing_render_thread_.task_runner()->PostTask(
                FROM_HERE, base::BindOnce(&VideoTrackAdapter::ReconfigureTrack, adapter_, null_track_.get(), adapter_settings));
        }
    }

    void AddTrackInternal(MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& adapter_settings)
    {
        testing_render_thread_.task_runner()->PostTask(FROM_HERE,
            base::BindOnce(&VideoTrackAdapter::AddTrack, adapter_, track,
                base::BindRepeating(&VideoTrackAdapterFixtureTest::OnFrameDelivered, base::Unretained(this)),
                base::BindRepeating(&VideoTrackAdapterFixtureTest::OnFrameDropped, base::Unretained(this)),
                base::BindRepeating(&VideoTrackAdapterFixtureTest::OnEncodedVideoFrameDelivered, base::Unretained(this)),
    ...
    void DeliverAndValidateFrame(scoped_refptr<media::VideoFrame> frame, base::TimeTicks estimated_capture_time)
    {
        auto deliver_frame = [&]() {
            platform_support_->GetIOTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&VideoTrackAdapter::DeliverFrameOnVideoTaskRunner, adapter_, frame, estimated_capture_time));
        };
        ...
        testing_render_thread_.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting(deliver_frame));
        frame_processed_.Wait();
    }

    void OnFrameDelivered(scoped_refptr<media::VideoFrame> frame, base::TimeTicks estimated_capture_time)
    {
    ...
    std::tuple<int, int> GenerateAndCountFrames(
        int num_frames, std::optional<double> target_frame_rate, base::RepeatingCallback<base::TimeDelta(int)> index_to_timestamp)
    {
        const gfx::Size resolution(10, 10);
        // Any capture format will work. Frames will generated at the
        // `actual_input_frame_rate`.
        const media::VideoCaptureFormat stream_format(resolution, /*frame_rate=*/10.0, media::PIXEL_FORMAT_I420);
        ...
        SetFrameValidationCallback(base::BindLambdaForTesting([&](scoped_refptr<media::VideoFrame> frame, base::TimeTicks estimated_capture_time) {
            num_delivered++;
            if (num_delivered + num_dropped == num_frames) {
                did_process_all_frames.Signal();
            }
        }));
        SetFrameDroppedCallback(base::BindLambdaForTesting([&](media::VideoCaptureFrameDropReason) {
            num_dropped++;
            if (num_delivered + num_dropped == num_frames) {
                did_process_all_frames.Signal();
            }
        ...
        for (int i = 0; i < num_frames; ++i) {
            auto frame = CreateTestFrame(
                ...
                /*storage_type=*/media::VideoFrame::STORAGE_OWNED_MEMORY, media::PIXEL_FORMAT_I420,
    ...
    // frames.
    std::tuple<int, int> GenerateAndCountFrames(int num_frames, std::optional<double> target_frame_rate, double actual_input_frame_rate)
    {
        auto index_to_timestamp = base::BindLambdaForTesting([actual_input_frame_rate](int i) { return i * base::Seconds(1.0 / actual_input_frame_rate); });
        return GenerateAndCountFrames(num_frames, target_frame_rate, index_to_timestamp);
    }

    void TestDeliversFrameWithVisibleRectWithEvenOriginAndSize(
        scoped_refptr<media::VideoFrame> frame, media::VideoPixelFormat pixel_format, gfx::Size desired_size)
    {
        const gfx::Size coded_size = frame->coded_size();
        const double kFrameRate = 30.0;
        const media::VideoCaptureFormat stream_format(coded_size, kFrameRate, pixel_format);
        CreateAdapter(stream_format);

        VideoTrackAdapterSettings cropped_settings(desired_size, kFrameRate);
        ConfigureTrack(cropped_settings);
        auto check_settings = [&](scoped_refptr<media::VideoFrame> frame, base::TimeTicks estimated_capture_time) {
    ...
    MOCK_METHOD2(OnEncodedVideoFrameDelivered, void(scoped_refptr<EncodedVideoFrame>, base::TimeTicks estimated_capture_time));

    ScopedTestingPlatformSupport<IOTaskRunnerTestingPlatformSupport> platform_support_;
...
TEST_F(VideoTrackAdapterFixtureTest, DeliverFrame_GpuMemoryBuffer)
{
    ...
    const double kFrameRate = 30.0;
    auto gmb_frame = CreateTestFrame(kCodedSize, kVisibleRect, kNaturalSize, media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER);

    // Initialize the VideoTrackAdapter to handle GpuMemoryBuffer. NV12 is the
    // only pixel format supported at the moment.
    const media::VideoCaptureFormat stream_format(kCodedSize, kFrameRate, media::PIXEL_FORMAT_NV12);
    CreateAdapter(stream_format);
    ...
    auto check_nonscaled = [&](scoped_refptr<media::VideoFrame> frame, base::TimeTicks estimated_capture_time) {
        // We should get the original frame as-is here.
        EXPECT_EQ(frame->storage_type(), media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER);
        EXPECT_EQ(frame->GetGpuMemoryBufferForTesting(), gmb_frame->GetGpuMemoryBufferForTesting());
    ...
    auto check_scaled = [&](scoped_refptr<media::VideoFrame> frame, base::TimeTicks estimated_capture_time) {
...
TEST_F(VideoTrackAdapterFixtureTest, DeliversHWFrameVisibleRectWithEvenOriginAndWidth)
{
    ...
    TestDeliversFrameWithVisibleRectWithEvenOriginAndSize(
        CreateTestFrame(kCodedSize, kVisibleRect, kNaturalSize, media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER), media::PIXEL_FORMAT_NV12, kDesiredSize);
}

TEST_F(VideoTrackAdapterFixtureTest, DeliversSWFrameVisibleRectWithEvenOriginAndWidth)
{
    const gfx::Size kCodedSize(1280, 720);
    const gfx::Rect kVisibleRect(0, 0, 1280, 720);
    const gfx::Size kDesiredSize(1277, 720);
    const gfx::Size kNaturalSize(1280, 720);
    TestDeliversFrameWithVisibleRectWithEvenOriginAndSize(
        CreateTestFrame(kCodedSize, kVisibleRect, kNaturalSize, media::VideoFrame::STORAGE_OWNED_MEMORY), media::PIXEL_FORMAT_I420, kDesiredSize);
}

TEST_F(VideoTrackAdapterFixtureTest, DeliversHWFrameVisibleRectWithEvenOriginAndHeight)
{
    const gfx::Size kCodedSize(1280, 720);
    const gfx::Rect kVisibleRect(0, 0, 1280, 720);
...
TEST_F(VideoTrackAdapterFixtureTest, DeliversSWFrameVisibleRectWithEvenOriginAndHeight)
{
    const gfx::Size kCodedSize(1280, 720);
    const gfx::Rect kVisibleRect(0, 0, 1280, 720);
    const gfx::Size kDesiredSize(1280, 718);
...
TEST_F(VideoTrackAdapterFixtureTest, DeliversSWFrameOddVisibleRectWithEvenOriginAndHeight)
{
    const gfx::Size kCodedSize(1280, 720);
...
// incoming frames.
TEST_F(VideoTrackAdapterFixtureTest, DeliverPortraitFrame_RunSettingsCallbackForSecondTrack)
{
    ...
    const double kFrameRate = 30.0;
    auto test_frame = CreateTestFrame(kCodedSize, kVisibleRect, kNaturalSize,
        /*storage_type=*/media::VideoFrame::STORAGE_OWNED_MEMORY);

    const media::VideoCaptureFormat stream_format(kCodedSize, kFrameRate, media::PIXEL_FORMAT_I420);
    CreateAdapter(stream_format);
    ...
    auto check_portrait = [&](scoped_refptr<media::VideoFrame> frame, base::TimeTicks estimated_capture_time) {
        // We should get the original frame as-is here.
...
TEST_F(VideoTrackAdapterFixtureTest, FrameRateReduction)
{
    ...
    auto [num_delivered, num_dropped] = GenerateAndCountFrames(kNumFrames, kTargetFrameRate, kInputFrameRate);
    EXPECT_EQ(num_delivered + num_dropped, kNumFrames);
    EXPECT_TRUE(base::IsApproximatelyEqual(static_cast<double>(num_delivered) / kNumFrames, kTargetFrameRate / kInputFrameRate, /*tolerance=*/0.05));
}
...
TEST_F(VideoTrackAdapterFixtureTest, ArbitraryFrameRateReduction)
{
    ...
    auto [num_delivered, num_dropped] = GenerateAndCountFrames(kNumFrames, kTargetFrameRate, kInputFrameRate);
    EXPECT_EQ(num_delivered + num_dropped, kNumFrames);
    EXPECT_NEAR(static_cast<double>(num_delivered) / kNumFrames * kInputFrameRate, kTargetFrameRate, /*abs_error=*/0.5);
}
...
TEST_F(VideoTrackAdapterFixtureTest, StreamWithJitterFrameRateReduction)
{
    ...
    auto index_to_timestamp = base::BindLambdaForTesting(
        [kTargetFrameRate](int i) { return (i / 2) * base::Seconds(1.0 / kTargetFrameRate) + (i % 2) * base::Seconds(0.05 / kTargetFrameRate); });
    auto [num_delivered, num_dropped] = GenerateAndCountFrames(kNumFrames, kTargetFrameRate, index_to_timestamp);
...
{
    const int kNumFrames = 1000;
    auto [num_delivered, num_dropped] = GenerateAndCountFrames(kNumFrames, /*target_frame_rate=*/5.0, /*actual_input_frame_rate=*/5.0);
...
TEST_F(VideoTrackAdapterFixtureTest, FrameRateReductionSlightMismatch)
{
    ...
    const double kTargetFrameRate = 10.0;
    const int kNumFrames = 1000;
    auto [num_delivered, num_dropped] = GenerateAndCountFrames(kNumFrames, kInputFrameRate, kTargetFrameRate);
    EXPECT_EQ(num_delivered + num_dropped, kNumFrames);
    EXPECT_NEAR(static_cast<double>(num_delivered) / kNumFrames * kInputFrameRate, kTargetFrameRate, /*abs_error=*/0.5);
}
...
TEST_F(VideoTrackAdapterFixtureTest, DoNotDropFramesIfNoTargetFrameRate)
{
    ...
    auto [num_delivered, num_dropped] = GenerateAndCountFrames(kNumFrames, /*target_frame_rate=*/std::nullopt,
...
{
    const int kNumFrames = 100;
    auto [num_delivered, num_dropped] = GenerateAndCountFrames(kNumFrames, /*target_frame_rate=*/std::numeric_limits<double>::infinity(),
        /*actual_input_frame_rate=*/10.0);
    EXPECT_EQ(num_delivered, kNumFrames);
    EXPECT_EQ(num_dropped, 0);
}

TEST_F(VideoTrackAdapterFixtureTest, DoNotDropFramesIfTimeBetweenFramesIsNegative)
{
    const int kNumFrames = 100;
    auto [num_delivered, num_dropped] = GenerateAndCountFrames(kNumFrames, /*target_frame_rate=*/10.0,
        /*actual_input_frame_rate=*/-10.0);
    EXPECT_EQ(num_delivered, kNumFrames);
    EXPECT_EQ(num_dropped, 0);
}

TEST_F(VideoTrackAdapterFixtureTest, DoNotDropFramesIfTimeBetweenFramesIsTooLong)
{
    const int kNumFrames = 100;
    auto [num_delivered, num_dropped] = GenerateAndCountFrames(kNumFrames, /*target_frame_rate=*/10.0,
...
public:
        ...
        : render_thread_("VideoTrackAdapterEncodedTest_RenderThread")
    {
        ...
        auto source = std::make_unique<MockMediaStreamVideoSource>(media::VideoCaptureFormat(), false);
        mock_source_ = source.get();
        web_source_.Initialize(blink::WebString::FromASCII("source_id"), blink::WebMediaStreamSource::kTypeVideo,
            blink::WebString::FromASCII("DeliverEncodedVideoFrameSource"), false /* remote */, std::move(source));
        RunSyncOnRenderThread([&] { adapter_ = base::MakeRefCounted<VideoTrackAdapter>(platform_support_->GetIOTaskRunner(), mock_source_->GetWeakPtr()); });
    }
    ...
    {
        auto track = std::make_unique<MediaStreamVideoTrack>(mock_source_, WebPlatformMediaStreamSource::ConstraintsOnceCallback(), true);
        RunSyncOnRenderThread([&] {
            adapter_->AddTrack(track.get(), base::BindRepeating(&VideoTrackAdapterEncodedTest::OnFrameDelivered, base::Unretained(this)), base::DoNothing(),
                base::BindRepeating(&VideoTrackAdapterEncodedTest::OnEncodedVideoFrameDelivered, base::Unretained(this)),
                /*sub_capture_target_version_callback=*/base::DoNothing(),
                /*settings_callback=*/base::DoNothing(),
    ...
    template <class Function> void RunSyncOnRenderThread(Function function)
    {
        base::RunLoop run_loop;
        base::OnceClosure quit_closure = run_loop.QuitClosure();
        render_thread_.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&] {
            std::move(function)();
    ...
    MOCK_METHOD2(OnFrameDelivered, void(scoped_refptr<media::VideoFrame> frame, base::TimeTicks estimated_capture_time));
    MOCK_METHOD2(OnEncodedVideoFrameDelivered, void(scoped_refptr<EncodedVideoFrame>, base::TimeTicks estimated_capture_time));
...
TEST_F(VideoTrackAdapterEncodedTest, DeliverEncodedVideoFrame)
{
    ...
    base::RunLoop run_loop;
    base::OnceClosure quit_closure = run_loop.QuitClosure();
    platform_support_->GetIOTaskRunner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
        adapter_->DeliverEncodedVideoFrameOnVideoTaskRunner(base::MakeRefCounted<MockEncodedVideoFrame>(), base::TimeTicks());
        std::move(quit_closure).Run();
    }));
    run_loop.Run();
    RunSyncOnRenderThread([&] {
