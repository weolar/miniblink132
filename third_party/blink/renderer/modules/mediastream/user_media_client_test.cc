...
MediaConstraints CreateDeviceConstraints(
    const String& basic_exact_value, const String& basic_ideal_value = g_empty_string, const String& advanced_exact_value = g_empty_string)
{
    blink::MockConstraintFactory factory;
...
MediaConstraints CreateFacingModeConstraints(const char* basic_exact_value, const char* basic_ideal_value = nullptr, const char* advanced_exact_value = nullptr)
{
    blink::MockConstraintFactory factory;
...
void CheckVideoSource(blink::MediaStreamVideoSource* source, int expected_source_width, int expected_source_height, double expected_source_frame_rate)
{
...
void CheckVideoSourceAndTrack(blink::MediaStreamVideoSource* source, int expected_source_width, int expected_source_height, double expected_source_frame_rate,
    MediaStreamComponent* component, int expected_track_width, int expected_track_height, double expected_track_frame_rate)
{
    CheckVideoSource(source, expected_source_width, expected_source_height, expected_source_frame_rate);
...
public:
    MockLocalMediaStreamAudioSource()
        : blink::MediaStreamAudioSource(blink::scheduler::GetSingleThreadTaskRunnerForTesting(), true /* is_local_source */)
    {
    }
...
class MockMediaStreamVideoCapturerSource : public blink::MockMediaStreamVideoSource {
public:
    MockMediaStreamVideoCapturerSource(const blink::MediaStreamDevice& device, SourceStoppedCallback stop_callback)
    ...
    MOCK_METHOD1(ChangeSourceImpl, void(const blink::MediaStreamDevice& new_device));
};

String MakeValidDeviceId(std::string_view id)
{
    std::string padding = base::ToLowerASCII(base::HexEncode(base::RandBytesAsVector(32)));
...
class MediaDevicesDispatcherHostMock : public mojom::blink::MediaDevicesDispatcherHost {
public:
    ...
    void SelectAudioOutput(const String& device_id, SelectAudioOutputCallback callback) override
    {
        NOTREACHED();
    }

    void AddMediaDevicesListener(bool subscribe_audio_input, bool subscribe_video_input, bool subscribe_audio_output,
        mojo::PendingRemote<blink::mojom::blink::MediaDevicesListener> listener) override
    {
        NOTREACHED();
    }
    ...
    void GetAllVideoInputDeviceFormats(const String& device_id, GetAllVideoInputDeviceFormatsCallback callback) override
    {
        devices_count_++;
    }

    void GetAvailableVideoInputDeviceFormats(const String& device_id, GetAvailableVideoInputDeviceFormatsCallback callback) override
    {
        devices_count_++;
    }

    size_t devices_count() const
    {
...
class MockMediaDevicesDispatcherHost : public mojom::blink::MediaDevicesDispatcherHost {
public:
    ...
    void SetVideoInputCapabilities(Vector<blink::mojom::blink::VideoInputDeviceCapabilitiesPtr> capabilities)
    {
    ...
    void GetVideoInputCapabilities(GetVideoInputCapabilitiesCallback client_callback) override
    {
        if (!video_input_capabilities_.empty()) {
            ...
            video_input_capabilities_ = Vector<blink::mojom::blink::VideoInputDeviceCapabilitiesPtr>();
            return;
        }
        blink::mojom::blink::VideoInputDeviceCapabilitiesPtr device = blink::mojom::blink::VideoInputDeviceCapabilities::New();
        ...
        if (!video_source_ || !video_source_->IsRunning() || !video_source_->GetCurrentFormat()) {
            device->formats.push_back(media::VideoCaptureFormat(gfx::Size(640, 480), 30.0f, media::PIXEL_FORMAT_I420));
            device->formats.push_back(media::VideoCaptureFormat(gfx::Size(800, 600), 30.0f, media::PIXEL_FORMAT_I420));
            device->formats.push_back(media::VideoCaptureFormat(gfx::Size(1024, 768), 20.0f, media::PIXEL_FORMAT_I420));
        } else {
    ...
    void GetAudioInputCapabilities(GetAudioInputCapabilitiesCallback client_callback) override
    {
        ...
        blink::mojom::blink::AudioInputDeviceCapabilitiesPtr device = blink::mojom::blink::AudioInputDeviceCapabilities::New();
    ...
    void GetAllVideoInputDeviceFormats(const String&, GetAllVideoInputDeviceFormatsCallback callback) override
    {
        Vector<media::VideoCaptureFormat> formats;
        formats.push_back(media::VideoCaptureFormat(gfx::Size(640, 480), 30.0f, media::PIXEL_FORMAT_I420));
        formats.push_back(media::VideoCaptureFormat(gfx::Size(800, 600), 30.0f, media::PIXEL_FORMAT_I420));
        formats.push_back(media::VideoCaptureFormat(gfx::Size(1024, 768), 20.0f, media::PIXEL_FORMAT_I420));
...
private:
    media::AudioParameters audio_parameters_ = media::AudioParameters::UnavailableDeviceParams();
    raw_ptr<blink::MediaStreamVideoSource, DanglingUntriaged> video_source_ = nullptr;
    // If set, overrides the default ones otherwise returned by
    // GetVideoInputCapabilities()
    Vector<blink::mojom::blink::VideoInputDeviceCapabilitiesPtr> video_input_capabilities_;
};
...
public:
    UserMediaProcessorUnderTest(LocalFrame* frame, std::unique_ptr<blink::WebMediaStreamDeviceObserver> media_stream_device_observer,
        mojo::PendingRemote<blink::mojom::blink::MediaDevicesDispatcherHost> media_devices_dispatcher, RequestState* state)
        : UserMediaProcessor(frame,
            WTF::BindRepeating(
                ...
                [](UserMediaProcessorUnderTest* processor) -> blink::mojom::blink::MediaDevicesDispatcherHost* {
        ...
        , state_(state)
    {
        media_devices_dispatcher_.Bind(std::move(media_devices_dispatcher), blink::scheduler::GetSingleThreadTaskRunnerForTesting());
    ...
    std::unique_ptr<blink::MediaStreamVideoSource> CreateVideoSource(
        const blink::MediaStreamDevice& device, blink::WebPlatformMediaStreamSource::SourceStoppedCallback stop_callback) override
    {
        video_source_ = new MockMediaStreamVideoCapturerSource(device, std::move(stop_callback));
        return base::WrapUnique(video_source_.get());
    }

    std::unique_ptr<blink::MediaStreamAudioSource> CreateAudioSource(
        const blink::MediaStreamDevice& device, blink::WebPlatformMediaStreamSource::ConstraintsRepeatingCallback source_ready) override
    {
        ...
        if (create_source_that_fails_) {
            ...
            public:
                FailedAtLifeAudioSource()
                    : blink::MediaStreamAudioSource(blink::scheduler::GetSingleThreadTaskRunnerForTesting(), true)
                {
                }
            ...
            source = std::make_unique<blink::MediaStreamAudioSource>(blink::scheduler::GetSingleThreadTaskRunnerForTesting(), true);
        }
        ...
        if (!create_source_that_fails_) {
            ...
            blink::scheduler::GetSingleThreadTaskRunnerForTesting()->PostTask(
                FROM_HERE, base::BindOnce(&UserMediaProcessorUnderTest::SignalSourceReady, std::move(source_ready), source.get()));
        }

        return source;
    }

    void GetUserMediaRequestSucceeded(MediaStreamDescriptorVector* descriptors, UserMediaRequest* request_info) override
    {
    ...
    void GetUserMediaRequestFailed(blink::mojom::blink::MediaStreamRequestResult result, const String& constraint_name) override
    {
        last_generated_descriptor_ = nullptr;
...
private:
    static void SignalSourceReady(blink::WebPlatformMediaStreamSource::ConstraintsOnceCallback source_ready, blink::WebPlatformMediaStreamSource* source)
    {
        std::move(source_ready).Run(source, blink::mojom::blink::MediaStreamRequestResult::OK, "");
    }

    std::unique_ptr<WebMediaStreamDeviceObserver> media_stream_device_observer_;
    HeapMojoRemote<blink::mojom::blink::MediaDevicesDispatcherHost> media_devices_dispatcher_;
    raw_ptr<MockMediaStreamVideoCapturerSource, DanglingUntriaged> video_source_ = nullptr;
    raw_ptr<MockLocalMediaStreamAudioSource, DanglingUntriaged> local_audio_source_ = nullptr;
    bool create_source_that_fails_ = false;
    Member<MediaStreamDescriptor> last_generated_descriptor_;
    blink::mojom::blink::MediaStreamRequestResult result_ = blink::mojom::blink::MediaStreamRequestResult::NUM_MEDIA_REQUEST_RESULTS;
...
public:
    UserMediaClientUnderTest(LocalFrame* frame, UserMediaProcessor* user_media_processor, UserMediaProcessor* display_user_media_processor, RequestState* state)
        : UserMediaClient(frame, user_media_processor, display_user_media_processor, blink::scheduler::GetSingleThreadTaskRunnerForTesting())
        , state_(state)
    {
    }
    ...
    {
        UserMediaRequest* user_media_request = UserMediaRequest::CreateForTesting(CreateDefaultConstraints(), CreateDefaultConstraints());
...
public:
    ...
    {
        screen_info_.rect = gfx::Rect(blink::kDefaultScreenCastWidth, blink::kDefaultScreenCastHeight);
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        dummy_page_holder_ = std::make_unique<DummyPageHolder>(gfx::Size(1, 1), chrome_client);

        user_media_processor_ = MakeGarbageCollected<UserMediaProcessorUnderTest>(
            &(dummy_page_holder_->GetFrame()), base::WrapUnique(msd_observer), user_media_processor_receiver_.BindNewPipeAndPassRemote(), &state_);
        user_media_processor_->set_media_stream_dispatcher_host_for_testing(mock_dispatcher_host_.CreatePendingRemoteAndBind());

        auto* display_msd_observer = new blink::WebMediaStreamDeviceObserver(nullptr);
        display_user_media_processor_ = MakeGarbageCollected<UserMediaProcessorUnderTest>(&(dummy_page_holder_->GetFrame()),
            base::WrapUnique(display_msd_observer), display_user_media_processor_receiver_.BindNewPipeAndPassRemote(), &state_);
        display_user_media_processor_->set_media_stream_dispatcher_host_for_testing(display_mock_dispatcher_host_.CreatePendingRemoteAndBind());

        user_media_client_impl_
            = MakeGarbageCollected<UserMediaClientUnderTest>(&(dummy_page_holder_->GetFrame()), user_media_processor_, display_user_media_processor_, &state_);

        user_media_client_impl_->SetMediaDevicesDispatcherForTesting(user_media_client_receiver_.BindNewPipeAndPassRemote());
    }
    ...
    MediaStreamDescriptor* RequestLocalMediaStream()
    {
        ...
        MediaStreamDescriptor* desc = user_media_processor_->last_generated_descriptor();
    ...
    {
        UserMediaRequest* user_media_request = UserMediaRequest::CreateForTesting(MediaConstraints(), CreateDefaultConstraints());
        user_media_client_impl_->RequestUserMediaForTest(user_media_request);
        StartMockedVideoSource(user_media_processor_);
        EXPECT_EQ(kRequestSucceeded, request_state());

        MediaStreamDescriptor* descriptor = user_media_processor_->last_generated_descriptor();
    ...
    MediaStreamComponent* RequestLocalAudioTrackWithAssociatedSink(bool render_to_associated_sink)
    {
        blink::MockConstraintFactory constraint_factory;
        constraint_factory.basic().render_to_associated_sink.SetExact(render_to_associated_sink);
        UserMediaRequest* user_media_request = UserMediaRequest::CreateForTesting(constraint_factory.CreateMediaConstraints(), MediaConstraints());
        user_media_client_impl_->RequestUserMediaForTest(user_media_request);
    ...
    void StartMockedVideoSource(UserMediaProcessorUnderTest* user_media_processor)
    {
        MockMediaStreamVideoCapturerSource* video_source = user_media_processor->last_created_video_source();
    ...
    {
        MockMediaStreamVideoCapturerSource* video_source = user_media_processor_->last_created_video_source();
        if (video_source->SourceHasAttemptedToStart())
            video_source->FailToStartMockedSource();
        blink::WebHeap::CollectGarbageForTesting();
    }

    void TestValidRequestWithConstraints(const MediaConstraints& audio_constraints, const MediaConstraints& video_constraints,
        const String& expected_audio_device_id, const String& expected_video_device_id)
    {
        DCHECK(!audio_constraints.IsNull());
        DCHECK(!video_constraints.IsNull());
        UserMediaRequest* request = UserMediaRequest::CreateForTesting(audio_constraints, video_constraints);
        ...
        EXPECT_EQ(expected_audio_device_id.Ascii(), mock_dispatcher_host_.devices().audio_device.value().id);
        EXPECT_EQ(expected_video_device_id.Ascii(), mock_dispatcher_host_.devices().video_device.value().id);
    }

    void ApplyConstraintsVideoMode(MediaStreamTrack* track, int width, int height, const std::optional<double>& frame_rate = std::optional<double>())
    {
        blink::MockConstraintFactory factory;
        ...
        auto* apply_constraints_request = MakeGarbageCollected<ApplyConstraintsRequest>(track, factory.CreateMediaConstraints(), nullptr);
    ...
    const MockMojoMediaStreamDispatcherHost& MediaStreamDispatcherHostForDisplayCapture()
    {
        return display_mock_dispatcher_host_;
    }

protected:
    ScopedTestingPlatformSupport<IOTaskRunnerTestingPlatformSupport> testing_platform_;
    ...
    mojo::Receiver<blink::mojom::blink::MediaDevicesDispatcherHost> user_media_processor_receiver_;
    mojo::Receiver<blink::mojom::blink::MediaDevicesDispatcherHost> display_user_media_processor_receiver_;
    mojo::Receiver<blink::mojom::blink::MediaDevicesDispatcherHost> user_media_client_receiver_;
...
TEST_F(UserMediaClientTest, GenerateTwoMediaStreamsWithSameSource)
{
    ...
    EXPECT_EQ(desc1_video_components[0]->Source()->GetName(), desc2_video_components[0]->Source()->GetName());

    EXPECT_EQ(desc1_video_components[0]->Source()->GetPlatformSource(), desc2_video_components[0]->Source()->GetPlatformSource());

    auto desc1_audio_components = desc1->AudioComponents();
    auto desc2_audio_components = desc2->AudioComponents();
    EXPECT_EQ(desc1_audio_components[0]->Source()->GetName(), desc2_audio_components[0]->Source()->GetName());

    EXPECT_EQ(MediaStreamAudioSource::From(desc1_audio_components[0]->Source()), MediaStreamAudioSource::From(desc2_audio_components[0]->Source()));
}
...
TEST_F(UserMediaClientTest, GenerateTwoMediaStreamsWithDifferentSources)
{
    ...
    auto desc1_video_components = desc1->VideoComponents();
    auto desc2_video_components = desc2->VideoComponents();
    EXPECT_NE(desc1_video_components[0]->Source()->GetName(), desc2_video_components[0]->Source()->GetName());

    EXPECT_NE(desc1_video_components[0]->Source()->GetPlatformSource(), desc2_video_components[0]->Source()->GetPlatformSource());

    auto desc1_audio_components = desc1->AudioComponents();
    auto desc2_audio_components = desc2->AudioComponents();
    EXPECT_NE(desc1_audio_components[0]->Source()->GetName(), desc2_audio_components[0]->Source()->GetName());

    EXPECT_NE(MediaStreamAudioSource::From(desc1_audio_components[0]->Source()), MediaStreamAudioSource::From(desc2_audio_components[0]->Source()));
}
...
TEST_F(UserMediaClientTest, StopLocalTracks)
{
    ...
    MediaStreamTrackPlatform* audio_track = MediaStreamTrackPlatform::GetTrack(WebMediaStreamTrack(audio_components[0]));
    audio_track->Stop();
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(1, mock_dispatcher_host_.stop_audio_device_counter());

    auto video_components = mixed_desc->VideoComponents();
    MediaStreamTrackPlatform* video_track = MediaStreamTrackPlatform::GetTrack(WebMediaStreamTrack(video_components[0]));
    video_track->Stop();
    base::RunLoop().RunUntilIdle();
...
TEST_F(UserMediaClientTest, StopLocalTracksWhenTwoStreamUseSameDevices)
{
    ...
    MediaStreamTrackPlatform* audio_track1 = MediaStreamTrackPlatform::GetTrack(WebMediaStreamTrack(audio_components1[0]));
    audio_track1->Stop();
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(0, mock_dispatcher_host_.stop_audio_device_counter());

    auto audio_components2 = desc2->AudioComponents();
    MediaStreamTrackPlatform* audio_track2 = MediaStreamTrackPlatform::GetTrack(WebMediaStreamTrack(audio_components2[0]));
    audio_track2->Stop();
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(1, mock_dispatcher_host_.stop_audio_device_counter());

    auto video_components1 = desc1->VideoComponents();
    MediaStreamTrackPlatform* video_track1 = MediaStreamTrackPlatform::GetTrack(WebMediaStreamTrack(video_components1[0]));
    video_track1->Stop();
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(0, mock_dispatcher_host_.stop_video_device_counter());

    auto video_components2 = desc2->VideoComponents();
    MediaStreamTrackPlatform* video_track2 = MediaStreamTrackPlatform::GetTrack(WebMediaStreamTrack(video_components2[0]));
    video_track2->Stop();
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(1, mock_dispatcher_host_.stop_video_device_counter());
}
...
TEST_F(UserMediaClientTest, MediaVideoSourceFailToStart)
{
    ...
    EXPECT_EQ(blink::mojom::blink::MediaStreamRequestResult::TRACK_START_FAILURE_VIDEO, user_media_processor_->error_reason());
    blink::WebHeap::CollectAllGarbageForTesting();
...
TEST_F(UserMediaClientTest, MediaAudioSourceFailToInitialize)
{
    ...
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(kRequestFailed, request_state());
    EXPECT_EQ(blink::mojom::blink::MediaStreamRequestResult::TRACK_START_FAILURE_AUDIO, user_media_processor_->error_reason());
    blink::WebHeap::CollectAllGarbageForTesting();
    EXPECT_EQ(1, mock_dispatcher_host_.request_stream_counter());
    EXPECT_EQ(1, mock_dispatcher_host_.stop_audio_device_counter());
    EXPECT_EQ(1, mock_dispatcher_host_.stop_video_device_counter());
...
{
    UserMediaRequest* request = UserMediaRequest::CreateForTesting(CreateDefaultConstraints(), CreateDefaultConstraints());
    user_media_client_impl_->RequestUserMediaForTest(request);
    blink::AudioCaptureSettings audio_capture_settings = user_media_processor_->AudioSettings();
    blink::VideoCaptureSettings video_capture_settings = user_media_processor_->VideoSettings();
    ...
    EXPECT_EQ(media::AudioDeviceDescription::kDefaultDeviceId, audio_capture_settings.device_id());
    EXPECT_TRUE(audio_capture_settings.disable_local_echo());
    EXPECT_FALSE(audio_capture_settings.render_to_associated_sink());

    const blink::AudioProcessingProperties& properties = audio_capture_settings.audio_processing_properties();
    EXPECT_EQ(EchoCancellationType::kEchoCancellationAec3, properties.echo_cancellation_type);
    ...
    EXPECT_EQ(video_capture_settings.Width(), blink::MediaStreamVideoSource::kDefaultWidth);
    EXPECT_EQ(video_capture_settings.Height(), blink::MediaStreamVideoSource::kDefaultHeight);
    EXPECT_EQ(video_capture_settings.FrameRate(), static_cast<float>(blink::MediaStreamVideoSource::kDefaultFrameRate));
    EXPECT_EQ(video_capture_settings.ResolutionChangePolicy(), media::ResolutionChangePolicy::FIXED_RESOLUTION);
    EXPECT_FALSE(video_capture_settings.noise_reduction());
    EXPECT_FALSE(video_capture_settings.min_frame_rate().has_value());

    const blink::VideoTrackAdapterSettings& track_settings = video_capture_settings.track_adapter_settings();
    EXPECT_FALSE(track_settings.target_size().has_value());
    EXPECT_EQ(track_settings.min_aspect_ratio(), 1.0 / static_cast<double>(blink::MediaStreamVideoSource::kDefaultHeight));
    EXPECT_EQ(track_settings.max_aspect_ratio(), static_cast<double>(blink::MediaStreamVideoSource::kDefaultWidth));
...
TEST_F(UserMediaClientTest, DefaultTabCapturePropagate)
{
    ...
    UserMediaRequest* request = UserMediaRequest::CreateForTesting(audio_constraints, video_constraints);
    user_media_client_impl_->RequestUserMediaForTest(request);
    blink::AudioCaptureSettings audio_capture_settings = UserMediaProcessorForDisplayCapture()->AudioSettings();
    blink::VideoCaptureSettings video_capture_settings = UserMediaProcessorForDisplayCapture()->VideoSettings();
    user_media_client_impl_->CancelUserMediaRequest(request);
    ...
    const blink::AudioProcessingProperties& properties = audio_capture_settings.audio_processing_properties();
    EXPECT_EQ(EchoCancellationType::kEchoCancellationDisabled, properties.echo_cancellation_type);
    EXPECT_FALSE(properties.goog_audio_mirroring);
    ...
    EXPECT_EQ(video_capture_settings.FrameRate(), blink::kDefaultScreenCastFrameRate);
    EXPECT_EQ(video_capture_settings.ResolutionChangePolicy(), media::ResolutionChangePolicy::FIXED_RESOLUTION);
    EXPECT_FALSE(video_capture_settings.noise_reduction());
    EXPECT_FALSE(video_capture_settings.min_frame_rate().has_value());
    ...
    EXPECT_EQ(track_settings.min_aspect_ratio(), 1.0 / blink::kMaxScreenCastDimension);
...
TEST_F(UserMediaClientTest, DefaultDesktopCapturePropagate)
{
    ...
    EXPECT_EQ(video_capture_settings.Height(), blink::kDefaultScreenCastHeight);
    EXPECT_EQ(video_capture_settings.FrameRate(), blink::kDefaultScreenCastFrameRate);
    EXPECT_EQ(video_capture_settings.ResolutionChangePolicy(), media::ResolutionChangePolicy::ANY_WITHIN_LIMIT);
    EXPECT_FALSE(video_capture_settings.noise_reduction());
    EXPECT_FALSE(video_capture_settings.min_frame_rate().has_value());
    EXPECT_FALSE(video_capture_settings.max_frame_rate().has_value());
...
TEST_F(UserMediaClientTest, NonDefaultAudioConstraintsPropagate)
{
    ...
    UserMediaRequest* request = UserMediaRequest::CreateForTesting(audio_constraints, MediaConstraints());
    user_media_client_impl_->RequestUserMediaForTest(request);
    blink::AudioCaptureSettings audio_capture_settings = user_media_processor_->AudioSettings();
#if !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_CHROMEOS) && !BUILDFLAG(IS_FUCHSIA)
    if (base::FeatureList::IsEnabled(features::kGetUserMediaDeferredDeviceSettingsSelection)) {
    ...
    EXPECT_TRUE(audio_capture_settings.HasValue());
    EXPECT_EQ(fake_ids_->audio_input_1.Ascii(), audio_capture_settings.device_id());
    EXPECT_TRUE(audio_capture_settings.disable_local_echo());
...
{
    MediaConstraints audio_constraints = CreateDeviceConstraints(fake_ids_->invalid_device);
    UserMediaRequest* request = UserMediaRequest::CreateForTesting(audio_constraints, MediaConstraints());
    user_media_client_impl_->RequestUserMediaForTest(request);
    EXPECT_EQ(kRequestFailed, request_state());
}
...
{
    MediaConstraints video_constraints = CreateDeviceConstraints(fake_ids_->invalid_device);
    UserMediaRequest* request = UserMediaRequest::CreateForTesting(MediaConstraints(), video_constraints);
    user_media_client_impl_->RequestUserMediaForTest(request);
    EXPECT_EQ(kRequestFailed, request_state());
}
...
{
    MediaConstraints audio_constraints = CreateDeviceConstraints(fake_ids_->audio_input_1);
    MediaConstraints video_constraints = CreateDeviceConstraints(fake_ids_->video_input_1);
    TestValidRequestWithConstraints(audio_constraints, video_constraints, fake_ids_->audio_input_1, fake_ids_->video_input_1);
}
...
TEST_F(UserMediaClientTest, CreateWithBasicIdealValidDeviceId)
{
    ...
    scoped_feature_list_.InitAndDisableFeature(features::kGetUserMediaDeferredDeviceSettingsSelection);
#endif
    MediaConstraints audio_constraints = CreateDeviceConstraints(g_empty_string, fake_ids_->audio_input_1);
    MediaConstraints video_constraints = CreateDeviceConstraints(g_empty_string, fake_ids_->video_input_1);
    TestValidRequestWithConstraints(audio_constraints, video_constraints, fake_ids_->audio_input_1, fake_ids_->video_input_1);
}
...
TEST_F(UserMediaClientTest, CreateWithAdvancedExactValidDeviceId)
{
    ...
    scoped_feature_list_.InitAndDisableFeature(features::kGetUserMediaDeferredDeviceSettingsSelection);
#endif
    MediaConstraints audio_constraints = CreateDeviceConstraints(g_empty_string, g_empty_string, fake_ids_->audio_input_1);
    MediaConstraints video_constraints = CreateDeviceConstraints(g_empty_string, g_empty_string, fake_ids_->video_input_1);
    TestValidRequestWithConstraints(audio_constraints, video_constraints, fake_ids_->audio_input_1, fake_ids_->video_input_1);
}

TEST_F(UserMediaClientTest, CreateWithAllOptionalInvalidDeviceId)
{
    MediaConstraints audio_constraints = CreateDeviceConstraints(g_empty_string, fake_ids_->invalid_device, fake_ids_->invalid_device);
    MediaConstraints video_constraints = CreateDeviceConstraints(g_empty_string, fake_ids_->invalid_device, fake_ids_->invalid_device);
    ...
    String expected_audio_device_id = String(media::AudioDeviceDescription::kDefaultDeviceId);
    TestValidRequestWithConstraints(audio_constraints, video_constraints, expected_audio_device_id, fake_ids_->video_input_1);
}
...
{
    MediaConstraints audio_constraints = CreateDeviceConstraints(fake_ids_->audio_input_1);
    MediaConstraints video_constraints = CreateFacingModeConstraints("environment");
    // fake_ids_->video_input_2 has kEnvironment facing mode.
    TestValidRequestWithConstraints(audio_constraints, video_constraints, fake_ids_->audio_input_1, fake_ids_->video_input_2);
}
...
TEST_F(UserMediaClientTest, ApplyConstraintsVideoDeviceSingleTrack)
{
    ...
    MediaStreamVideoTrack* platform_track = MediaStreamVideoTrack::From(component);
...
TEST_F(UserMediaClientTest, CameraCaptureCapabilityHistograms)
{
    // With two HD/FullHD capable devices.
    {
        ...
        blink::mojom::blink::VideoInputDeviceCapabilitiesPtr device1 = blink::mojom::blink::VideoInputDeviceCapabilities::New();
        ...
        device1->formats.push_back(media::VideoCaptureFormat(gfx::Size(640, 360), 30.0f, media::PIXEL_FORMAT_NV12));
        device1->formats.push_back(media::VideoCaptureFormat(gfx::Size(1280, 720), 30.0f, media::PIXEL_FORMAT_NV12));
        capabilities.push_back(std::move(device1));
        // The second device supports 480p and 1080p.
        blink::mojom::blink::VideoInputDeviceCapabilitiesPtr device2 = blink::mojom::blink::VideoInputDeviceCapabilities::New();
        ...
        device2->formats.push_back(media::VideoCaptureFormat(gfx::Size(640, 480), 30.0f, media::PIXEL_FORMAT_NV12));
        device2->formats.push_back(media::VideoCaptureFormat(gfx::Size(1920, 1080), 30.0f, media::PIXEL_FORMAT_NV12));
        capabilities.push_back(std::move(device2));
        media_devices_dispatcher_.SetVideoInputCapabilities(std::move(capabilities));

        // Perform getUserMedia() and verify one camera capability value per device.
        RequestLocalVideoTrack();
        histogram_tester.ExpectBucketCount("Media.MediaDevices.GetUserMedia.CameraCaptureCapability", CameraCaptureCapability::kHdOrFullHd, 0);
        histogram_tester.ExpectBucketCount("Media.MediaDevices.GetUserMedia.CameraCaptureCapability", CameraCaptureCapability::kHdOrFullHd_360p, 1);
        histogram_tester.ExpectBucketCount("Media.MediaDevices.GetUserMedia.CameraCaptureCapability", CameraCaptureCapability::kHdOrFullHd_480p, 1);
        histogram_tester.ExpectBucketCount("Media.MediaDevices.GetUserMedia.CameraCaptureCapability", CameraCaptureCapability::kHdOrFullHd_360p_480p, 0);
        histogram_tester.ExpectBucketCount("Media.MediaDevices.GetUserMedia.CameraCaptureCapability", CameraCaptureCapability::kHdAndFullHdMissing, 0);
    }
    ...
    // With three devices: No HD, HD with both 360p and 480p, and HD with neither.
    {
        ...
        device1->facing_mode = mojom::blink::FacingMode::kUser;
        device1->formats.push_back(media::VideoCaptureFormat(gfx::Size(640, 360), 30.0f, media::PIXEL_FORMAT_NV12));
        device1->formats.push_back(media::VideoCaptureFormat(gfx::Size(640, 480), 30.0f, media::PIXEL_FORMAT_NV12));
        capabilities.push_back(std::move(device1));
        ...
        device2->group_id = String("dummy2");
        device2->facing_mode = mojom::blink::FacingMode::kUser;
        device2->formats.push_back(media::VideoCaptureFormat(gfx::Size(640, 360), 30.0f, media::PIXEL_FORMAT_NV12));
        device2->formats.push_back(media::VideoCaptureFormat(gfx::Size(640, 480), 30.0f, media::PIXEL_FORMAT_NV12));
        device2->formats.push_back(media::VideoCaptureFormat(gfx::Size(1280, 720), 30.0f, media::PIXEL_FORMAT_NV12));
        capabilities.push_back(std::move(device2));
        // The third device supports 720p and 1080p.
        blink::mojom::blink::VideoInputDeviceCapabilitiesPtr device3 = blink::mojom::blink::VideoInputDeviceCapabilities::New();
        ...
        device3->formats.push_back(media::VideoCaptureFormat(gfx::Size(1280, 720), 30.0f, media::PIXEL_FORMAT_NV12));
        device3->formats.push_back(media::VideoCaptureFormat(gfx::Size(1920, 1080), 30.0f, media::PIXEL_FORMAT_NV12));
        capabilities.push_back(std::move(device3));
        media_devices_dispatcher_.SetVideoInputCapabilities(std::move(capabilities));

        // Perform getUserMedia() and verify one camera capability value per device.
        RequestLocalVideoTrack();
        histogram_tester.ExpectBucketCount("Media.MediaDevices.GetUserMedia.CameraCaptureCapability", CameraCaptureCapability::kHdOrFullHd, 1);
        histogram_tester.ExpectBucketCount("Media.MediaDevices.GetUserMedia.CameraCaptureCapability", CameraCaptureCapability::kHdOrFullHd_360p, 0);
        histogram_tester.ExpectBucketCount("Media.MediaDevices.GetUserMedia.CameraCaptureCapability", CameraCaptureCapability::kHdOrFullHd_480p, 0);
        histogram_tester.ExpectBucketCount("Media.MediaDevices.GetUserMedia.CameraCaptureCapability", CameraCaptureCapability::kHdOrFullHd_360p_480p, 1);
        histogram_tester.ExpectBucketCount("Media.MediaDevices.GetUserMedia.CameraCaptureCapability", CameraCaptureCapability::kHdAndFullHdMissing, 1);
    }
}
...
TEST_F(UserMediaClientTest, ApplyConstraintsVideoDeviceTwoTracks)
{
    MediaStreamTrack* track = RequestLocalVideoTrack();
    MediaStreamComponent* component = track->Component();
    MockMediaStreamVideoCapturerSource* source = user_media_processor_->last_created_video_source();
    CheckVideoSource(source, 0, 0, 0.0);
    media_devices_dispatcher_.SetVideoSource(source);
    ...
    CheckVideoSourceAndTrack(source, 1024, 768, 20.0, component2, 1024, 768, 20.0);
    ...
    blink::MediaStreamTrackPlatform::GetTrack(WebMediaStreamTrack(component2))->Stop();
    ApplyConstraintsVideoMode(track, 800, 600, 30.0);
    CheckVideoSourceAndTrack(source, 800, 600, 30.0, component, 800, 600, 30.0);
}
...
TEST_F(UserMediaClientTest, ApplyConstraintsVideoDeviceFailsToRestartAfterStop)
{
    MediaStreamTrack* track = RequestLocalVideoTrack();
    MediaStreamComponent* component = track->Component();
    MockMediaStreamVideoCapturerSource* source = user_media_processor_->last_created_video_source();
...
TEST_F(UserMediaClientTest, ApplyConstraintsVideoDeviceStopped)
{
    ...
    MediaStreamTrackPlatform* platform_track = MediaStreamTrackPlatform::GetTrack(WebMediaStreamTrack(component));
    platform_track->Stop();
    EXPECT_EQ(component->GetReadyState(), MediaStreamSource::kReadyStateEnded);
    EXPECT_FALSE(source->IsRunning());
    {
...
TEST_F(UserMediaClientTest, RenderToAssociatedSinkTrueAssociatedOutputDeviceId)
{
    MediaStreamComponent* component = RequestLocalAudioTrackWithAssociatedSink(true);
    MediaStreamAudioSource* source = MediaStreamAudioSource::From(component->Source());
    EXPECT_TRUE(source->device().matched_output_device_id);
}

TEST_F(UserMediaClientTest, RenderToAssociatedSinkFalseAssociatedOutputDeviceId)
{
    MediaStreamComponent* component = RequestLocalAudioTrackWithAssociatedSink(false);
    MediaStreamAudioSource* source = MediaStreamAudioSource::From(component->Source());
...
{
    blink::MockConstraintFactory factory;
    factory.basic().media_stream_source.SetExact(blink::WebString::FromASCII(blink::kMediaStreamSourceDesktop));
    MediaConstraints audio_constraints = factory.CreateMediaConstraints();
    MediaConstraints video_constraints = factory.CreateMediaConstraints();
    UserMediaRequest* request = UserMediaRequest::CreateForTesting(audio_constraints, video_constraints);
    user_media_client_impl_->RequestUserMediaForTest(request);
    ...
    MockMediaStreamVideoCapturerSource* video_source = UserMediaProcessorForDisplayCapture()->last_created_video_source();
    blink::MediaStreamDevice fake_video_device(
        blink::mojom::blink::MediaStreamType::GUM_DESKTOP_VIDEO_CAPTURE, fake_ids_->video_input_1.Ascii(), "Fake Video Device");
    EXPECT_CALL(*video_source, ChangeSourceImpl(_));
    UserMediaProcessorForDisplayCapture()->OnDeviceChanged(video_source->device(), fake_video_device);

    // Test changing audio source.
    MockLocalMediaStreamAudioSource* audio_source = UserMediaProcessorForDisplayCapture()->last_created_local_audio_source();
    EXPECT_NE(audio_source, nullptr);
    blink::MediaStreamDevice fake_audio_device(
        blink::mojom::blink::MediaStreamType::GUM_DESKTOP_AUDIO_CAPTURE, fake_ids_->video_input_1.Ascii(), "Fake Audio Device");
    EXPECT_CALL(*audio_source, EnsureSourceIsStopped()).Times(2);
    UserMediaProcessorForDisplayCapture()->OnDeviceChanged(audio_source->device(), fake_audio_device);
...
TEST_F(UserMediaClientTest, DesktopCaptureChangeSourceWithoutAudio)
{
    ...
    UserMediaRequest* request = UserMediaRequest::CreateForTesting(audio_constraints, video_constraints);
    user_media_client_impl_->RequestUserMediaForTest(request);
    EXPECT_NE(std::nullopt, MediaStreamDispatcherHostForDisplayCapture().devices().audio_device);
    EXPECT_NE(std::nullopt, MediaStreamDispatcherHostForDisplayCapture().devices().video_device);
    ...
    blink::MediaStreamDevice fake_audio_device(blink::mojom::blink::MediaStreamType::NO_SERVICE, "", "");
    UserMediaProcessorForDisplayCapture()->OnDeviceChanged(audio_source->device(), fake_audio_device);
    base::RunLoop().RunUntilIdle();
...
{
    EXPECT_FALSE(UserMediaProcessor::IsPanTiltZoomPermissionRequested(CreateDefaultConstraints()));

    blink::MockConstraintFactory basic_factory;
    basic_factory.basic().pan.SetIsPresent(true);
    EXPECT_TRUE(UserMediaProcessor::IsPanTiltZoomPermissionRequested(basic_factory.CreateMediaConstraints()));
    ...
    EXPECT_TRUE(UserMediaProcessor::IsPanTiltZoomPermissionRequested(advanced_factory.CreateMediaConstraints()));
}
...
TEST_F(UserMediaClientTest, MultiDeviceOnStreamsGenerated)
{
    ...
    std::unique_ptr<blink::MediaDevicesDispatcherHostMock> media_devices_dispatcher_host_mock = std::make_unique<blink::MediaDevicesDispatcherHostMock>();
    blink::Member<blink::UserMediaRequest> user_media_request
        = blink::UserMediaRequest::CreateForTesting(CreateDefaultConstraints(), CreateDefaultConstraints());
    user_media_request->set_request_id(request_id);
    user_media_processor_->ProcessRequest(user_media_request, base::DoNothing());
    user_media_processor_->media_devices_dispatcher_cb_ = base::BindLambdaForTesting(
        [&media_devices_dispatcher_host_mock]() -> blink::mojom::blink::MediaDevicesDispatcherHost* { return media_devices_dispatcher_host_mock.get(); });

    blink::mojom::blink::StreamDevicesSetPtr stream_devices_set = blink::mojom::blink::StreamDevicesSet::New();
    for (size_t stream_index = 0; stream_index < devices_count; ++stream_index) {
        stream_devices_set->stream_devices.emplace_back(blink::mojom::blink::StreamDevices::New(std::nullopt, blink::MediaStreamDevice()));
    }
    user_media_processor_->OnStreamsGenerated(
        request_id, blink::mojom::MediaStreamRequestResult::OK, "", std::move(stream_devices_set), /*pan_tilt_zoom_allowed=*/false);
...
TEST_F(UserMediaClientDeferredDeviceSelectionTest, CreateWithMandatoryInvalidAudioDeviceId)
{
    MediaConstraints audio_constraints = CreateDeviceConstraints(fake_ids_->invalid_device);
    UserMediaRequest* request = UserMediaRequest::CreateForTesting(audio_constraints, MediaConstraints());
    user_media_client_impl_->RequestUserMediaForTest(request);
...
TEST_F(UserMediaClientDeferredDeviceSelectionTest, CreateWithMandatoryInvalidVideoDeviceId)
{
    MediaConstraints video_constraints = CreateDeviceConstraints(fake_ids_->invalid_device);
    UserMediaRequest* request = UserMediaRequest::CreateForTesting(MediaConstraints(), video_constraints);
    user_media_client_impl_->RequestUserMediaForTest(request);
...
TEST_F(UserMediaClientDeferredDeviceSelectionTest, CreateWithMandatoryValidDeviceIds)
{
    MediaConstraints audio_constraints = CreateDeviceConstraints(fake_ids_->audio_input_1);
    MediaConstraints video_constraints = CreateDeviceConstraints(fake_ids_->video_input_1);
    TestValidRequestWithConstraints(audio_constraints, video_constraints, fake_ids_->audio_input_1, fake_ids_->video_input_1);
