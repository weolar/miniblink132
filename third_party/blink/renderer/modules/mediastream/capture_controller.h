...
class MODULES_EXPORT CaptureController final : public EventTarget, public ExecutionContextClient, public MediaStreamSource::Observer {
...
public:
    ...
    ScriptPromise<IDLUndefined> sendWheel(ScriptState* script_state, CapturedWheelAction* action);
    ScriptPromise<IDLUndefined> captureWheel(ScriptState* script_state, HTMLElement* element);
    ...
    ScriptPromise<IDLUndefined> setZoomLevel(ScriptState* script_state, int zoom_level);
    ...
    DEFINE_ATTRIBUTE_EVENT_LISTENER(capturedzoomlevelchange, kCapturedzoomlevelchange)
    ...
    void SendWheel(double relative_x, double relative_y, int32_t wheel_delta_x, int32_t wheel_delta_y);

    void SetMediaStreamDispatcherHostForTesting(mojo::PendingRemote<mojom::blink::MediaStreamDispatcherHost>);
#endif
...
private:
    ...
    void OnCaptureWheelPermissionResult(ScriptPromiseResolver<IDLUndefined>*, HTMLElement*, mojom::blink::CapturedSurfaceControlResult);
    ...
    HeapMojoRemote<mojom::blink::MediaStreamDispatcherHost> media_stream_dispatcher_host_;
