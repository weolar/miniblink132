...
public:
    ...
    FakeWebMediaPlayerDelegate& operator=(const FakeWebMediaPlayerDelegate&) = delete;
    ...
    void DidMediaMetadataChange(int delegate_id, bool has_audio, bool has_video, media::MediaContentType type) override
    {
        EXPECT_EQ(delegate_id_, delegate_id);
    }
...
public:
    ...
    base::OnceClosure GetClosure() const
    {
        return event_->GetClosure();
    }
...
public:
    MockMediaStreamVideoRenderer(const scoped_refptr<base::SingleThreadTaskRunner> task_runner, ReusableMessageLoopEvent* message_loop_controller,
        const MediaStreamVideoRenderer::RepaintCB& repaint_cb, raw_ptr<base::test::TaskEnvironment> task_environment)
        : started_(false)
    ...
    void QueueFrames(const Vector<int>& timestamps_or_frame_type, bool opaque_frame = true, bool odd_size_frame = false, int double_size_index = -1,
    ...
    void set_standard_size(const gfx::Size& size)
    {
        standard_size_ = size;
    }
    const gfx::Size& get_standard_size()
    {
...
private:
    ...
    const raw_ptr<ReusableMessageLoopEvent, DanglingUntriaged> message_loop_controller_;
...
public:
    ...
    void SwitchOutputDevice(const std::string& device_id, media::OutputDeviceStatusCB callback) override
    {
    }
...
void MockMediaStreamVideoRenderer::Start()
{
    started_ = true;
    paused_ = false;
    PostCrossThreadTask(*task_runner_, FROM_HERE, CrossThreadBindOnce(&MockMediaStreamVideoRenderer::InjectFrame, WrapRefCounted(this)));
}
...
void MockMediaStreamVideoRenderer::AddFrame(FrameType category, scoped_refptr<media::VideoFrame> frame)
{
    frames_.push_back(std::make_pair(category, std::move(frame)));
}

void MockMediaStreamVideoRenderer::QueueFrames(
    const Vector<int>& timestamp_or_frame_type, bool opaque_frame, bool odd_size_frame, int double_size_index, media::VideoRotation rotation)
{
    ...
    for (wtf_size_t i = 0; i < timestamp_or_frame_type.size(); i++) {
        ...
        if (static_cast<int>(i) == double_size_index) {
            standard_size = gfx::Size(standard_size_.width() * 2, standard_size_.height() * 2);
        }
        ...
        if (token >= 0) {
            gfx::Size frame_size;
            if (odd_size_frame) {
                frame_size.SetSize(standard_size.width() - kOddSizeOffset, standard_size.height() - kOddSizeOffset);
            } else {
                frame_size.SetSize(standard_size.width(), standard_size.height());
            }

            auto frame = media::VideoFrame::CreateZeroInitializedFrame(
                opaque_frame ? media::PIXEL_FORMAT_I420 : media::PIXEL_FORMAT_I420A, frame_size, gfx::Rect(frame_size), frame_size, base::Milliseconds(token));
            ...
            frame->metadata().reference_time = base::TimeTicks::Now() + base::Milliseconds(token);
...
void MockMediaStreamVideoRenderer::InjectFrame()
{
    ...
    PostDelayedCrossThreadTask(
        *task_runner_, FROM_HERE, CrossThreadBindOnce(&MockMediaStreamVideoRenderer::InjectFrame, WrapRefCounted(this)), delay_till_next_generated_frame_);
...
public:
    ...
    void Initialize(cc::VideoFrameProvider* provider, bool is_media_stream) override
    {
        provider_ = provider;
...
public:
    MockRenderFactory(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, ReusableMessageLoopEvent* message_loop_controller,
    ...
    scoped_refptr<MediaStreamVideoRenderer> GetVideoRenderer(const WebMediaStream& web_stream, const MediaStreamVideoRenderer::RepaintCB& repaint_cb,
        scoped_refptr<base::SequencedTaskRunner> video_task_runner, scoped_refptr<base::SingleThreadTaskRunner> main_render_task_runner) override;
    ...
    scoped_refptr<MediaStreamAudioRenderer> GetAudioRenderer(const WebMediaStream& web_stream, WebLocalFrame* web_frame, const WebString& device_id,
    ...
    bool support_video_renderer() const
    {
        return support_video_renderer_;
    }
...
scoped_refptr<MediaStreamVideoRenderer> MockRenderFactory::GetVideoRenderer(const WebMediaStream& web_stream,
    const MediaStreamVideoRenderer::RepaintCB& repaint_cb, scoped_refptr<base::SequencedTaskRunner> video_task_runner,
    scoped_refptr<base::SingleThreadTaskRunner> main_render_task_runner)
{
    if (!support_video_renderer_)
        return nullptr;

    provider_ = base::MakeRefCounted<MockMediaStreamVideoRenderer>(task_runner_, message_loop_controller_, repaint_cb, task_environment_);
...
class WebMediaPlayerMSTest
    : public testing::TestWithParam<testing::tuple<bool /* enable_surface_layer_for_video */, bool /* opaque_frame */, bool /* odd_size_frame */>>,
      public MediaPlayerClient,
      public cc::VideoFrameProvider::Client {
public:
    WebMediaPlayerMSTest()
        : render_factory_(new MockRenderFactory(scheduler::GetSingleThreadTaskRunnerForTesting(), &message_loop_controller_, &task_environment_))
    ...
    void RemotePlaybackCompatibilityChanged(const KURL& url, bool is_compatible) override
    {
    }
    bool WasAlwaysMuted() override
    {
        return false;
    }
    ...
    {
        return is_audio_element_;
    }
    bool IsInAutoPIP() const override
    {
        return false;
    }
    void MediaRemotingStarted(const WebString& remote_device_friendly_name) override
    {
    }
    ...
    void DidMediaMetadataChange(bool has_audio, bool has_video, media::AudioCodec audio_codec, media::VideoCodec video_codec,
        media::MediaContentType media_content_type, bool is_encrypted_media) override
    {
    }
    void DidPlayerMediaPositionStateChange(double playback_rate, base::TimeDelta duration, base::TimeDelta position, bool end_of_media) override
    {
    }
    ...
    {
#if BUILDFLAG(IS_WIN)
        render_factory_->provider()->set_standard_size(WebMediaPlayerMS::kUseGpuMemoryBufferVideoFramesMinResolution);
#endif // BUILDFLAG(IS_WIN)

        player_->SetGpuMemoryBufferVideoForTesting(new media::MockGpuMemoryBufferVideoFramePool(&frame_ready_cbs_));
    }
...
protected:
    ...
    raw_ptr<NiceMock<MockSurfaceLayerBridge>, DanglingUntriaged> surface_layer_bridge_ptr_ = nullptr;
    raw_ptr<NiceMock<MockWebVideoFrameSubmitter>, DanglingUntriaged> submitter_ptr_ = nullptr;
...
void WebMediaPlayerMSTest::InitializeWebMediaPlayerMS()
{
    ...
    player_ = std::make_unique<WebMediaPlayerMS>(nullptr, this, &delegate_, std::make_unique<media::NullMediaLog>(),
        scheduler::GetSingleThreadTaskRunnerForTesting(), scheduler::GetSingleThreadTaskRunnerForTesting(), scheduler::GetSingleThreadTaskRunnerForTesting(),
        scheduler::GetSingleThreadTaskRunnerForTesting(), scheduler::GetSingleThreadTaskRunnerForTesting(), gpu_factories_.get(), WebString(),
        WTF::BindOnce(&WebMediaPlayerMSTest::CreateMockSurfaceLayerBridge, WTF::Unretained(this)), std::move(submitter_), enable_surface_layer_for_video_);
    player_->SetMediaStreamRendererFactoryForTesting(std::unique_ptr<MediaStreamRendererFactory>(render_factory_));
}

MockMediaStreamVideoRenderer* WebMediaPlayerMSTest::LoadAndGetFrameProvider(bool algorithm_enabled)
{
    EXPECT_FALSE(!!render_factory_->provider()) << "There should not be a "
                                                   "FrameProvider yet.";

    EXPECT_CALL(*this, DoNetworkStateChanged(WebMediaPlayer::kNetworkStateLoading));
    EXPECT_CALL(*this, DoReadyStateChanged(WebMediaPlayer::kReadyStateHaveNothing));
    player_->Load(WebMediaPlayer::kLoadTypeURL, WebMediaPlayerSource(), WebMediaPlayer::kCorsModeUnspecified,
...
void WebMediaPlayerMSTest::NetworkStateChanged()
{
    ...
    if (state == WebMediaPlayer::NetworkState::kNetworkStateFormatError || state == WebMediaPlayer::NetworkState::kNetworkStateDecodeError
        || state == WebMediaPlayer::NetworkState::kNetworkStateNetworkError) {
        message_loop_controller_.GetPipelineStatusCB().Run(media::PIPELINE_ERROR_NETWORK);
    }
}
...
void WebMediaPlayerMSTest::ReadyStateChanged()
{
    ...
    if (state == WebMediaPlayer::ReadyState::kReadyStateHaveMetadata && !player_->HasAudio()) {
...
void WebMediaPlayerMSTest::StartRendering()
{
    if (!rendering_) {
        rendering_ = true;
        scheduler::GetSingleThreadTaskRunnerForTesting()->PostTask(FROM_HERE, WTF::BindOnce(&WebMediaPlayerMSTest::RenderFrame, weak_factory_.GetWeakPtr()));
    }
    DoStartRendering();
}
...
void WebMediaPlayerMSTest::RenderFrame()
{
    ...
    scheduler::GetSingleThreadTaskRunnerForTesting()->PostDelayedTask(
        FROM_HERE, WTF::BindOnce(&WebMediaPlayerMSTest::RenderFrame, weak_factory_.GetWeakPtr()), base::Seconds(1.0 / 60.0));
}
...
void WebMediaPlayerMSTest::TestGetVideoFramePresentationMetadata(bool algorithm_enabled)
{
    InitializeWebMediaPlayerMS();

    MockMediaStreamVideoRenderer* provider = LoadAndGetFrameProvider(algorithm_enabled);
    ...
    EXPECT_CALL(*this, OnRequestVideoFrameCallback()).Times(num_frames).WillRepeatedly([&]() {
        ...
        if (!algorithm_enabled && !enable_surface_layer_for_video_ && !deadline_min_.is_null()) {
...
void WebMediaPlayerMSTest::TestRequestFrameCallbackWithVideoFrameMetadata(bool algorithm_enabled)
{
    InitializeWebMediaPlayerMS();
    ...
    EXPECT_CALL(*this, OnRequestVideoFrameCallback()).Times(1).WillOnce(testing::Invoke([&]() {
...
{
    InitializeWebMediaPlayerMS();
    EXPECT_CALL(*this, DoReadyStateChanged(WebMediaPlayer::kReadyStateHaveMetadata)).Times(0);
    EXPECT_CALL(*this, DoReadyStateChanged(WebMediaPlayer::kReadyStateHaveEnoughData)).Times(0);
...
TEST_P(WebMediaPlayerMSTest, NoWaitForFrameForAudio)
{
    ...
    EXPECT_CALL(*this, DoReadyStateChanged(WebMediaPlayer::kReadyStateHaveMetadata));
    EXPECT_CALL(*this, DoReadyStateChanged(WebMediaPlayer::kReadyStateHaveEnoughData));
...
TEST_P(WebMediaPlayerMSTest, PictureInPictureStateChangeNotCalled)
{
    ...
    Vector<int> timestamps({ 0, 33, 66, 100, 133, 166, 200, 233, 266, 300, 333, 366, 400, 433, 466, 500, 533, 566, 600 });
    provider->QueueFrames(timestamps);

    if (enable_surface_layer_for_video_) {
        EXPECT_CALL(*submitter_ptr_, StartRendering());
        EXPECT_CALL(*this, GetDisplayType()).WillRepeatedly(Return(DisplayType::kPictureInPicture));
    ...
    EXPECT_CALL(*this, DoReadyStateChanged(WebMediaPlayer::kReadyStateHaveMetadata));
    EXPECT_CALL(*this, DoReadyStateChanged(WebMediaPlayer::kReadyStateHaveEnoughData));
    EXPECT_CALL(*this, CheckSizeChanged(gfx::Size(kStandardWidth, kStandardHeight)));
    message_loop_controller_.RunAndWaitForStatus(media::PIPELINE_OK);
...
TEST_P(WebMediaPlayerMSTest, PlayThenPause)
{
    ...
    const int kTestBrake = static_cast<int>(FrameType::TEST_BRAKE);
    Vector<int> timestamps({ 0, 33, 66, 100, 133, kTestBrake, 166, 200, 233, 266, 300, 333, 366, 400, 433, 466, 500, 533, 566, 600 });
    ...
    EXPECT_CALL(*this, DoReadyStateChanged(WebMediaPlayer::kReadyStateHaveMetadata));
    EXPECT_CALL(*this, DoReadyStateChanged(WebMediaPlayer::kReadyStateHaveEnoughData));
    gfx::Size frame_size = gfx::Size(kStandardWidth - (odd_size_frame ? kOddSizeOffset : 0), kStandardHeight - (odd_size_frame ? kOddSizeOffset : 0));
...
TEST_P(WebMediaPlayerMSTest, PlayThenPauseThenPlay)
{
    ...
    const int kTestBrake = static_cast<int>(FrameType::TEST_BRAKE);
    Vector<int> timestamps({ 0, 33, 66, 100, 133, kTestBrake, 166, 200, 233, 266, 300, 333, 366, 400, 433, kTestBrake, 466, 500, 533, 566, 600 });
    provider->QueueFrames(timestamps, opaque_frame, odd_size_frame);
...
TEST_P(WebMediaPlayerMSTest, RotationChange)
{
    ...
    // 3 rotation changes.
    EXPECT_CALL(*this, CheckSizeChanged(gfx::Size(kStandardWidth, kStandardHeight))).Times(3);
    message_loop_controller_.RunAndWaitForStatus(media::PIPELINE_OK);
    ...
    if (enable_surface_layer_for_video_) {
        ...
        EXPECT_CALL(*this, DoStopRendering()).WillOnce([&]() {
            scheduler::GetSingleThreadTaskRunnerForTesting()->PostTask(FROM_HERE,
                WTF::BindOnce(
    ...
    provider->QueueFrames(timestamps, false, false, 17, media::VIDEO_ROTATION_90);
    if (enable_surface_layer_for_video_) {
        EXPECT_CALL(*submitter_ptr_, SetTransform(media::VideoTransformation(media::VIDEO_ROTATION_90)));
    } else {
        EXPECT_CALL(*this, DoSetCcLayer(true));
        EXPECT_CALL(*this, DoStopRendering()).WillOnce([&]() {
            scheduler::GetSingleThreadTaskRunnerForTesting()->PostTask(FROM_HERE,
...
TEST_P(WebMediaPlayerMSTest, RequestVideoFrameCallback_ForcesBeginFrames)
{
    ...
    EXPECT_CALL(*submitter_ptr_, SetForceBeginFrames(false)).WillOnce(base::test::RunClosure(run_loop.QuitClosure()));
    run_loop.Run();
...
TEST_P(WebMediaPlayerMSTest, DuplicateFrameTimestamp)
{
    ...
    gfx::Size frame_size(kStandardWidth - (odd_size_frame ? kOddSizeOffset : 0), kStandardHeight - (odd_size_frame ? kOddSizeOffset : 0));

    constexpr auto kStep = base::Milliseconds(25);
    auto frame = media::VideoFrame::CreateZeroInitializedFrame(
        opaque_frame ? media::PIXEL_FORMAT_I420 : media::PIXEL_FORMAT_I420A, frame_size, gfx::Rect(frame_size), frame_size, kStep);
    ...
    auto frame3 = media::VideoFrame::CreateZeroInitializedFrame(
        opaque_frame ? media::PIXEL_FORMAT_I420 : media::PIXEL_FORMAT_I420A, frame_size, gfx::Rect(frame_size), frame_size, kStep * 2);
...
TEST_P(WebMediaPlayerMSTest, OutOfOrderEnqueue)
{
    ...
    frame2->metadata().reference_time = base::TimeTicks::Now() + kStep * 2;
    auto frame3 = media::VideoFrame::CreateZeroInitializedFrame(
        opaque_frame ? media::PIXEL_FORMAT_I420 : media::PIXEL_FORMAT_I420A, frame_size, gfx::Rect(frame_size), frame_size, kStep * 3);
...
TEST_P(WebMediaPlayerMSTest, ValidPreferredInterval)
{
    ...
    auto frame = media::VideoFrame::CreateZeroInitializedFrame(
        opaque_frame ? media::PIXEL_FORMAT_I420 : media::PIXEL_FORMAT_I420A, frame_size, gfx::Rect(frame_size), frame_size, base::Seconds(10));
    ...
    frame = media::VideoFrame::CreateZeroInitializedFrame(
        opaque_frame ? media::PIXEL_FORMAT_I420 : media::PIXEL_FORMAT_I420A, frame_size, gfx::Rect(frame_size), frame_size, base::Seconds(1));
    compositor_->EnqueueFrame(std::move(frame), true);
    base::RunLoop().RunUntilIdle();
    EXPECT_GE(compositor_->GetPreferredRenderInterval(), base::TimeDelta());
}
...
TEST_P(WebMediaPlayerMSTest, OnContextLost)
{
    ...
    auto non_gpu_frame
        = media::VideoFrame::CreateZeroInitializedFrame(media::PIXEL_FORMAT_I420, frame_size, gfx::Rect(frame_size), frame_size, base::Seconds(10));
    ...
    std::unique_ptr<gfx::GpuMemoryBuffer> gmb = std::make_unique<media::FakeGpuMemoryBuffer>(frame_size, gfx::BufferFormat::YUV_420_BIPLANAR);
    auto gpu_frame = media::VideoFrame::WrapExternalGpuMemoryBuffer(gfx::Rect(frame_size), frame_size, std::move(gmb), base::TimeDelta());
...
INSTANTIATE_TEST_SUITE_P(All, WebMediaPlayerMSTest, ::testing::Combine(::testing::Bool(), ::testing::Bool(), ::testing::Bool()));
} // namespace blink
