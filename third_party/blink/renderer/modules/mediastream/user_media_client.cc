...
class UserMediaClient::RequestQueue final : public GarbageCollected<UserMediaClient::RequestQueue> {
public:
    RequestQueue(LocalFrame* frame, UserMediaProcessor* user_media_processor, UserMediaClient* user_media_client,
    ...
    void KeepDeviceAliveForTransfer(
        base::UnguessableToken session_id, base::UnguessableToken transfer_id, UserMediaProcessor::KeepDeviceAliveForTransferCallback keep_alive_cb);
...
UserMediaClient::RequestQueue::RequestQueue(
    LocalFrame* frame, UserMediaProcessor* user_media_processor, UserMediaClient* user_media_client, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : frame_(frame)
    , user_media_processor_(user_media_processor)
    , apply_constraints_processor_(MakeGarbageCollected<ApplyConstraintsProcessor>(frame,
          WTF::BindRepeating(
              [](UserMediaClient* client) -> mojom::blink::MediaDevicesDispatcherHost* {
...
void UserMediaClient::RequestQueue::CancelUserMediaRequest(UserMediaRequest* user_media_request)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    {
        ...
        if (request == user_media_request) {
            blink::WebRtcLogMessage(base::StringPrintf("UMCI::CancelUserMediaRequest. request_id=%d", request->request_id()));
        }
    }
    ...
    if (!user_media_processor_->DeleteUserMediaRequest(user_media_request)) {
        for (auto it = pending_requests_.begin(); it != pending_requests_.end(); ++it) {
            if ((*it)->IsUserMedia() && (*it)->user_media_request() == user_media_request) {
    ...
    base::UnguessableToken session_id, base::UnguessableToken transfer_id, UserMediaProcessor::KeepDeviceAliveForTransferCallback keep_alive_cb)
{
    ...
    user_media_processor_->KeepDeviceAliveForTransfer(session_id, transfer_id, std::move(keep_alive_cb));
}
...
void UserMediaClient::RequestQueue::MaybeProcessNextRequestInfo()
{
    ...
    if (current_request->IsUserMedia()) {
        user_media_processor_->ProcessRequest(
            current_request->MoveUserMediaRequest(), WTF::BindOnce(&UserMediaClient::RequestQueue::CurrentRequestCompleted, WrapWeakPersistent(this)));
    } else if (current_request->IsApplyConstraints()) {
        apply_constraints_processor_->ProcessRequest(
            current_request->apply_constraints_request(), WTF::BindOnce(&UserMediaClient::RequestQueue::CurrentRequestCompleted, WrapWeakPersistent(this)));
    } else {
        DCHECK(current_request->IsStopTrack());
        MediaStreamTrackPlatform* track = MediaStreamTrackPlatform::GetTrack(WebMediaStreamTrack(current_request->track_to_stop()));
        if (track) {
            track->StopAndNotify(WTF::BindOnce(&UserMediaClient::RequestQueue::CurrentRequestCompleted, WrapWeakPersistent(this)));
        } else {
...
void UserMediaClient::RequestQueue::CurrentRequestCompleted()
{
    ...
    if (!pending_requests_.empty()) {
        frame_->GetTaskRunner(blink::TaskType::kInternalMedia)
            ->PostTask(FROM_HERE, WTF::BindOnce(&UserMediaClient::RequestQueue::MaybeProcessNextRequestInfo, WrapWeakPersistent(this)));
    }
}
...
UserMediaClient::UserMediaClient(LocalFrame* frame, UserMediaProcessor* user_media_processor, UserMediaProcessor* display_user_media_processor,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    ...
    , pending_device_requests_(MakeGarbageCollected<RequestQueue>(frame, user_media_processor, this, task_runner))
    , pending_display_requests_(MakeGarbageCollected<RequestQueue>(frame, display_user_media_processor, this, task_runner))
{
    CHECK(frame_);

    // WrapWeakPersistent is safe because the |frame_| owns UserMediaClient.
    frame_->SetIsCapturingMediaCallback(WTF::BindRepeating([](UserMediaClient* client) { return client && client->IsCapturing(); }, WrapWeakPersistent(this)));
}

UserMediaClient::UserMediaClient(LocalFrame* frame, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : UserMediaClient(frame,
        MakeGarbageCollected<UserMediaProcessor>(frame,
            WTF::BindRepeating(
                [](UserMediaClient* client) -> mojom::blink::MediaDevicesDispatcherHost* {
                    // |client| is guaranteed to be not null because |client|
...
void UserMediaClient::RequestUserMedia(UserMediaRequest* user_media_request)
{
    ...
    DCHECK(!user_media_request->GetWindow() || frame_ == user_media_request->GetWindow()->GetFrame());

    // Save histogram data so we can see how much GetUserMedia is used.
    UpdateAPICount(user_media_request->MediaRequestType());

    int32_t request_id = g_next_request_id++;
    blink::WebRtcLogMessage(base::StringPrintf("UMCI::RequestUserMedia({request_id=%d}, {audio constraints=%s}, "
                                               "{video constraints=%s})",
        request_id, user_media_request->AudioConstraints().ToString().Utf8().c_str(), user_media_request->VideoConstraints().ToString().Utf8().c_str()));
    ...
    if (LocalDOMWindow* window = user_media_request->GetWindow()) {
        has_transient_user_activation = LocalFrame::HasTransientUserActivation(window->GetFrame());
    }
    ...
    if (auto* window = user_media_request->GetWindow()) {
        if (user_media_request->MediaRequestType() == UserMediaRequestType::kUserMedia) {
            ...
            PeerConnectionTracker::From(*window).TrackGetDisplayMedia(user_media_request);
        }
    }

    user_media_request->set_has_transient_user_activation(has_transient_user_activation);
    mojom::blink::MediaStreamType type = user_media_request->Video() ? user_media_request->VideoMediaStreamType() : user_media_request->AudioMediaStreamType();
    auto* queue = GetRequestQueue(type);
    queue->EnqueueAndMaybeProcess(MakeGarbageCollected<Request>(user_media_request));
}

void UserMediaClient::ApplyConstraints(blink::ApplyConstraintsRequest* user_media_request)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(user_media_request);
    ...
    auto* queue = GetRequestQueue(user_media_request->Track()->Source()->GetPlatformSource()->device().type);
    queue->EnqueueAndMaybeProcess(MakeGarbageCollected<Request>(user_media_request));
}
...
void UserMediaClient::StopTrack(MediaStreamComponent* track)
{
    ...
    auto* queue = GetRequestQueue(track->Source()->GetPlatformSource()->device().type);
    queue->EnqueueAndMaybeProcess(MakeGarbageCollected<Request>(track));
}

bool UserMediaClient::IsCapturing()
{
    return pending_device_requests_->IsCapturing() || pending_display_requests_->IsCapturing();
}
...
void UserMediaClient::CancelUserMediaRequest(UserMediaRequest* user_media_request)
{
...
void UserMediaClient::SetMediaDevicesDispatcherForTesting(mojo::PendingRemote<blink::mojom::blink::MediaDevicesDispatcherHost> media_devices_dispatcher)
{
    media_devices_dispatcher_.Bind(std::move(media_devices_dispatcher), frame_->GetTaskRunner(blink::TaskType::kInternalMedia));
}

blink::mojom::blink::MediaDevicesDispatcherHost* UserMediaClient::GetMediaDevicesDispatcher()
{
    if (!media_devices_dispatcher_.is_bound()) {
        frame_->GetBrowserInterfaceBroker().GetInterface(
            media_devices_dispatcher_.BindNewPipeAndPassReceiver(frame_->GetTaskRunner(blink::TaskType::kInternalMedia)));
    }
...
UserMediaClient* UserMediaClient::From(LocalDOMWindow* window)
{
    ...
    if (!client) {
        ...
        client = MakeGarbageCollected<UserMediaClient>(window->GetFrame(), window->GetTaskRunner(TaskType::kInternalMedia));
...
void UserMediaClient::KeepDeviceAliveForTransfer(
    base::UnguessableToken session_id, base::UnguessableToken transfer_id, UserMediaProcessor::KeepDeviceAliveForTransferCallback keep_alive_cb)
{
    pending_display_requests_->KeepDeviceAliveForTransfer(session_id, transfer_id, std::move(keep_alive_cb));
}

UserMediaClient::RequestQueue* UserMediaClient::GetRequestQueue(mojom::blink::MediaStreamType media_stream_type)
{
    if (IsScreenCaptureMediaType(media_stream_type) || media_stream_type == mojom::blink::MediaStreamType::DISPLAY_AUDIO_CAPTURE) {
