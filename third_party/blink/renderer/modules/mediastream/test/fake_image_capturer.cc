...
{
    DynamicTo<LocalDOMWindow>(context)->GetBrowserInterfaceBroker().SetBinderForTesting(
        media::mojom::blink::ImageCapture::Name_, WTF::BindRepeating(&FakeImageCapture::Bind, weak_factory_.GetWeakPtr()));
}

void FakeImageCapture::Bind(mojo::ScopedMessagePipeHandle handle)
{
    receivers_.Add(this, mojo::PendingReceiver<media::mojom::blink::ImageCapture>(std::move(handle)));
}

void FakeImageCapture::GetPhotoState(const WTF::String& source_id, GetPhotoStateCallback callback)
{
    media::mojom::blink::PhotoStatePtr photo_capabilities = media::mojom::blink::PhotoState::New();
    ...
    photo_capabilities->red_eye_reduction = media::mojom::blink::RedEyeReduction::NEVER;
