...
protected:
    void Configure(int source_sample_rate, int source_buffer_size, int context_sample_rate, base::TimeDelta platform_buffer_duration)
    {
        source_params_.Reset(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Mono(), source_sample_rate, source_buffer_size);
        sink_params_.Reset(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), context_sample_rate,
            WebAudioMediaStreamAudioSink::kWebAudioRenderBufferSize);
        sink_bus_ = media::AudioBus::Create(sink_params_);
        auto* audio_source = MakeGarbageCollected<MediaStreamSource>(String::FromUTF8("dummy_source_id"), MediaStreamSource::kTypeAudio,
            String::FromUTF8("dummy_source_name"), /*remote=*/false,
            /*platform_source=*/nullptr);
        component_
            = MakeGarbageCollected<MediaStreamComponentImpl>(String::FromUTF8("audio_track"), audio_source, std::make_unique<MediaStreamAudioTrack>(true));
        source_provider_ = std::make_unique<WebAudioMediaStreamAudioSink>(component_, context_sample_rate, platform_buffer_duration);
...
TEST_F(WebAudioMediaStreamAudioSinkTest, VerifyDataFlow)
{
    ...
    std::fill(source_bus->channel(0), source_bus->channel(0) + source_bus->frames(), 0.5f);
    ...
    for (int i = sink_params_.frames_per_buffer(); i < source_params_.frames_per_buffer(); i += sink_params_.frames_per_buffer()) {
        sink_bus_->Zero();
        source_provider_->ProvideInput(audio_data, sink_params_.frames_per_buffer());
    ...
    estimated_capture_time += source_bus->frames() * base::Seconds(1) / source_params_.sample_rate();
    source_provider_->OnData(*source_bus, estimated_capture_time);

    // Verify that non-zero data samples are present in the results of the
    // following calls to provideInput().
    for (int i = 0; i < source_params_.frames_per_buffer(); i += sink_params_.frames_per_buffer()) {
        sink_bus_->Zero();
        source_provider_->ProvideInput(audio_data, sink_params_.frames_per_buffer());
...
TEST_F(WebAudioMediaStreamAudioSinkTest, DeleteSourceProviderBeforeStoppingTrack)
{
    Configure(/*source_sample_rate=*/48000, /*source_buffer_size=*/480,
        /*context_sample_rate=*/44100,
        /*platform_buffer_duration=*/base::Milliseconds(10));
...
TEST_F(WebAudioMediaStreamAudioSinkTest, StopTrackBeforeDeletingSourceProvider)
{
    Configure(/*source_sample_rate=*/48000, /*source_buffer_size=*/480,
        /*context_sample_rate=*/44100,
        /*platform_buffer_duration=*/base::Milliseconds(10));
...
class WebAudioMediaStreamAudioSinkFifoTest : public WebAudioMediaStreamAudioSinkTest,
                                             public testing::WithParamInterface<std::tuple<int, int, float, float, int>> { };
...
TEST_P(WebAudioMediaStreamAudioSinkFifoTest, VerifyFifo)
{
    ...
    int context_buffer_size = media::AudioLatency::GetHighLatencyBufferSize(context_sample_rate, 0);

    Configure(source_sample_rate, source_buffer_size, context_sample_rate, audio_utilities::FramesToTime(context_buffer_size, context_sample_rate));

    // 1. Source preparation.
    std::unique_ptr<media::AudioBus> source_bus = media::AudioBus::Create(source_params_);
    source_bus->Zero();
    ...
    auto pull_cb = base::BindLambdaForTesting([&](int frame_delay, media::AudioBus* audio_bus) {
        source_provider_->ProvideInput(audio_data, sink_params_.frames_per_buffer());
        sink_bus_->CopyTo(audio_bus);
    });
    media::AudioPullFifo pull_fifo(sink_params_.channels(), sink_params_.frames_per_buffer(), pull_cb);

    media::AudioParameters output_params(sink_params_.format(), sink_params_.channel_layout_config(), sink_params_.sample_rate(), context_buffer_size);

    std::unique_ptr<media::AudioBus> output_bus = media::AudioBus::Create(output_params);
    ...
    // errors.
    uint64_t counts_in_second = static_cast<uint64_t>(source_params_.sample_rate()) * output_params.sample_rate();

    // Values below are, in other words, frames_per_buffer() * counts_in_second /
    // sample_rate().
    uint64_t produce_step = static_cast<uint64_t>(source_params_.frames_per_buffer()) * output_params.sample_rate();
    uint64_t consume_step = static_cast<uint64_t>(output_params.frames_per_buffer()) * source_params_.sample_rate();

    uint64_t consume_counter = consume_step;
    uint64_t consume_delay = (1 + device_callback_irregularity_coefficient) * consume_step;
...
INSTANTIATE_TEST_SUITE_P(All, WebAudioMediaStreamAudioSinkFifoTest,
    testing::Combine(
        // source_sample_rate
