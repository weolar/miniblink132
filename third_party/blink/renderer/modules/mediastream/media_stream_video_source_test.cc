...
public:
    MediaStreamVideoSourceTest()
        ...
        , mock_stream_video_source_(
              new MockMediaStreamVideoSource(media::VideoCaptureFormat(gfx::Size(1280, 720), kSourceFrameRate, media::PIXEL_FORMAT_I420), false))
    {
        mock_stream_video_source_->DisableStopForRestart();
        stream_source_ = MakeGarbageCollected<MediaStreamSource>(String::FromUTF8("dummy_source_id"), MediaStreamSource::kTypeVideo,
            String::FromUTF8("dummy_source_name"), false /* remote */, base::WrapUnique(mock_stream_video_source_.get()));
        ON_CALL(*mock_stream_video_source_, OnSourceCanDiscardAlpha).WillByDefault(Return());
        ON_CALL(*mock_stream_video_source_, SupportsEncodedOutput).WillByDefault(Return(true));
    }
...
protected:
    ...
    WebMediaStreamTrack CreateTrack(const String& id)
    {
        ...
        return MediaStreamVideoTrack::CreateVideoTrack(
            mock_stream_video_source_, WTF::BindOnce(&MediaStreamVideoSourceTest::OnConstraintsApplied, base::Unretained(this)), enabled);
    }

    WebMediaStreamTrack CreateTrack(const String& id, const VideoTrackAdapterSettings& adapter_settings, const std::optional<bool>& noise_reduction,
        bool is_screencast, double min_frame_rate)
    {
        bool enabled = true;
        return MediaStreamVideoTrack::CreateVideoTrack(mock_stream_video_source_, adapter_settings, noise_reduction, is_screencast, min_frame_rate, nullptr,
            false, WTF::BindOnce(&MediaStreamVideoSourceTest::OnConstraintsApplied, base::Unretained(this)), enabled);
    }

    WebMediaStreamTrack CreateTrack()
    {
        return CreateTrack("123", VideoTrackAdapterSettings(gfx::Size(100, 100), 30.0), std::optional<bool>(), false, 0.0);
    }

    WebMediaStreamTrack CreateTrackAndStartSource(int width, int height, std::optional<double> frame_rate, bool detect_rotation = false)
    {
        WebMediaStreamTrack track = CreateTrack("123", VideoTrackAdapterSettings(gfx::Size(width, height), frame_rate), std::optional<bool>(), false, 0.0);
    ...
    mojom::MediaStreamRequestResult error_type() const
    {
        return result_;
    }
    WebString error_name() const
    {
        return result_name_;
    }
    ...
    MediaStreamSource* stream_source()
    {
        return stream_source_.Get();
    }

    void TestSourceCropFrame(int capture_width, int capture_height, int expected_width, int expected_height)
    {
        // Configure the track to crop to the expected resolution.
        WebMediaStreamTrack track = CreateTrackAndStartSource(expected_width, expected_height, 30.0);
    ...
    void DeliverVideoFrameAndWaitForRenderer(int width, int height, MockMediaStreamVideoSink* sink)
    {
        base::RunLoop run_loop;
        base::OnceClosure quit_closure = run_loop.QuitClosure();
        EXPECT_CALL(*sink, OnVideoFrame).WillOnce([&](base::TimeTicks) { std::move(quit_closure).Run(); });
        scoped_refptr<media::VideoFrame> frame = media::VideoFrame::CreateBlackFrame(gfx::Size(width, height));
        mock_source()->DeliverVideoFrame(frame);
        run_loop.Run();
    }

    void DeliverVideoFrameAndWaitForRenderer(int width, int height, base::TimeDelta timestamp, MockMediaStreamVideoSink* sink)
    {
        base::RunLoop run_loop;
        base::OnceClosure quit_closure = run_loop.QuitClosure();
        EXPECT_CALL(*sink, OnVideoFrame).WillOnce([&](base::TimeTicks) { std::move(quit_closure).Run(); });
    ...
    void DeliverRotatedVideoFrameAndWaitForRenderer(int width, int height, MockMediaStreamVideoSink* sink)
    {
        DeliverVideoFrameAndWaitForRenderer(height, width, sink);
    }

    void DeliverVideoFrameAndWaitForTwoRenderers(int width, int height, MockMediaStreamVideoSink* sink1, MockMediaStreamVideoSink* sink2)
    {
        base::RunLoop run_loop;
        base::OnceClosure quit_closure = run_loop.QuitClosure();
        EXPECT_CALL(*sink1, OnVideoFrame);
        EXPECT_CALL(*sink2, OnVideoFrame).WillOnce([&](base::TimeTicks) { std::move(quit_closure).Run(); });
        scoped_refptr<media::VideoFrame> frame = media::VideoFrame::CreateBlackFrame(gfx::Size(width, height));
        mock_source()->DeliverVideoFrame(frame);
        run_loop.Run();
    }

    void TestTwoTracksWithDifferentSettings(
        int capture_width, int capture_height, int expected_width1, int expected_height1, int expected_width2, int expected_height2)
    {
        WebMediaStreamTrack track1 = CreateTrackAndStartSource(expected_width1, expected_height1, MediaStreamVideoSource::kDefaultFrameRate);

        WebMediaStreamTrack track2
            = CreateTrack("dummy", VideoTrackAdapterSettings(gfx::Size(expected_width2, expected_height2), MediaStreamVideoSource::kDefaultFrameRate),
        ...
        DeliverVideoFrameAndWaitForTwoRenderers(capture_width, capture_height, &sink1, &sink2);
    ...
    void ReleaseTrackAndSourceOnAddTrackCallback(const WebMediaStreamTrack& track_to_release)
    {
        track_to_release_ = track_to_release;
    }

private:
    void OnConstraintsApplied(WebPlatformMediaStreamSource* source, mojom::MediaStreamRequestResult result, const WebString& result_name)
    {
    ...
    raw_ptr<MockMediaStreamVideoSource, DanglingUntriaged> mock_stream_video_source_;
};
...
TEST_F(MediaStreamVideoSourceTest, RotatedSourceDetectionDisabled)
{
    ...
    // Disable frame-rate adjustment in spec-compliant mode to ensure no frames
    // are dropped.
    WebMediaStreamTrack track = CreateTrackAndStartSource(1280, 720, std::nullopt, true);
...
TEST_F(MediaStreamVideoSourceTest, MutedSource)
{
    ...
    WebMediaStreamTrack track = CreateTrackAndStartSource(640, 480, media::limits::kMaxFramesPerSecond - 2);
    MockMediaStreamVideoSink sink;
    sink.ConnectToTrack(track);
    EXPECT_EQ(track.Source().GetReadyState(), WebMediaStreamSource::kReadyStateLive);

    base::RunLoop run_loop;
    base::OnceClosure quit_closure = run_loop.QuitClosure();
    bool muted_state = false;
    EXPECT_CALL(*mock_source(), DoSetMutedState(_)).WillOnce(DoAll(SaveArg<0>(&muted_state), [&](auto) { std::move(quit_closure).Run(); }));
    run_loop.Run();
    EXPECT_EQ(muted_state, true);

    EXPECT_EQ(track.Source().GetReadyState(), WebMediaStreamSource::kReadyStateMuted);

    base::RunLoop run_loop2;
    base::OnceClosure quit_closure2 = run_loop2.QuitClosure();
    EXPECT_CALL(*mock_source(), DoSetMutedState(_)).WillOnce(DoAll(SaveArg<0>(&muted_state), [&](auto) { std::move(quit_closure2).Run(); }));
    DeliverVideoFrameAndWaitForRenderer(640, 480, &sink);
    run_loop2.Run();
...
TEST_F(MediaStreamVideoSourceTest, SendAtMaxRateAndExpectAllFramesToBeDelivered)
{
    ...
    DeliverVideoFrameAndWaitForRenderer(100, 100, base::Milliseconds(100 + 10), &sink);
    DeliverVideoFrameAndWaitForRenderer(100, 100, base::Milliseconds(200 - 10), &sink);
    DeliverVideoFrameAndWaitForRenderer(100, 100, base::Milliseconds(300 + 5), &sink);
    DeliverVideoFrameAndWaitForRenderer(100, 100, base::Milliseconds(400 - 5), &sink);
    DeliverVideoFrameAndWaitForRenderer(100, 100, base::Milliseconds(500 + 20), &sink);
...
TEST_F(MediaStreamVideoSourceTest, NotifyFrameDroppedWhenStartAtTooHighRate)
{
    ...
    native_track->SetSinkNotifyFrameDroppedCallback(&sink, sink.GetNotifyFrameDroppedCB());
    ...
    base::RunLoop run_loop;
    base::OnceClosure quit_closure = run_loop.QuitClosure();
    EXPECT_CALL(sink, OnNotifyFrameDropped).WillOnce([&] { std::move(quit_closure).Run(); });
...
TEST_F(MediaStreamVideoSourceTest, ForwardsAtMaxFrameRateAndDropsWhenTooClose)
{
    ...
    EXPECT_CALL(sink, OnNotifyFrameDropped(media::VideoCaptureFrameDropReason::kResolutionAdapterFrameRateIsHigherThanRequested)).Times(1).WillOnce([&] {
...
TEST_F(MediaStreamVideoSourceTest, DropFrameAtTooHighRateAndThenStopDropping)
{
    ...
    constexpr base::TimeDelta kDeltaTimestampSlightlyTooHighRateDuration = base::Milliseconds(80);
    base::TimeDelta timestamp = base::TimeDelta() + kDeltaTimestampSlightlyTooHighRateDuration;
    for (int i = 0; i < 4; ++i) {
...
{
    WebMediaStreamTrack track = CreateTrackAndStartSource(640, 480, kSourceFrameRate - 2);
    MockMediaStreamVideoSink sink;
    sink.ConnectToTrack(track);
    EXPECT_EQ(track.Source().GetReadyState(), WebMediaStreamSource::kReadyStateLive);
    ...
    source()->ReconfigureTrack(native_track, VideoTrackAdapterSettings(gfx::Size(630, 470), 30.0));
    native_track->GetSettings(settings);
...
TEST_F(MediaStreamVideoSourceTest, ReconfigureStoppedTrack)
{
    ...
    native_track->Stop();
    EXPECT_EQ(track.Source().GetReadyState(), WebMediaStreamSource::kReadyStateEnded);
...
TEST_F(MediaStreamVideoSourceTest, FailedRestart)
{
    ...
    mock_source()->StopForRestart(
        WTF::BindOnce([](MediaStreamVideoSource::RestartResult result) { EXPECT_EQ(result, MediaStreamVideoSource::RestartResult::IS_RUNNING); }));
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(track.Source().GetReadyState(), WebMediaStreamSource::kReadyStateLive);

    // Verify that Restart() fails with INVALID_STATE when not called after a
    // successful StopForRestart().
    mock_source()->Restart(media::VideoCaptureFormat(),
        WTF::BindOnce([](MediaStreamVideoSource::RestartResult result) { EXPECT_EQ(result, MediaStreamVideoSource::RestartResult::INVALID_STATE); }));
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(track.Source().GetReadyState(), WebMediaStreamSource::kReadyStateLive);
...
TEST_F(MediaStreamVideoSourceTest, SuccessfulRestart)
{
    ...
    mock_source()->StopForRestart(
        WTF::BindOnce([](MediaStreamVideoSource::RestartResult result) { EXPECT_EQ(result, MediaStreamVideoSource::RestartResult::IS_STOPPED); }));
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(track.Source().GetReadyState(), WebMediaStreamSource::kReadyStateLive);
...
TEST_F(MediaStreamVideoSourceTest, StartStopAndNotifyRestartSupported)
{
    ...
    mock_source()->StartMockedSource();
    EXPECT_EQ(NumberOfSuccessConstraintsCallbacks(), 1);
    EXPECT_EQ(web_track.Source().GetReadyState(), WebMediaStreamSource::kReadyStateLive);

    EXPECT_CALL(*this, MockNotification());
    MediaStreamTrackPlatform* track = MediaStreamTrackPlatform::GetTrack(web_track);
    track->StopAndNotify(WTF::BindOnce(&MediaStreamVideoSourceTest::MockNotification, base::Unretained(this)));
    EXPECT_EQ(web_track.Source().GetReadyState(), WebMediaStreamSource::kReadyStateEnded);
    base::RunLoop().RunUntilIdle();
}
...
TEST_F(MediaStreamVideoSourceTest, AddTrackAfterStoppingSource)
{
    ...
    track1->StopAndNotify(WTF::BindOnce(&MediaStreamVideoSourceTest::MockNotification, base::Unretained(this)));
...
TEST_F(MediaStreamVideoSourceTest, RemovesEncodedSinkWhenEncodedConsumerDisappears)
{
...
TEST_F(MediaStreamVideoSourceTest, RemovesEncodedSinkWhenEncodedConsumersDisappear)
{
    EXPECT_CALL(*mock_source(), OnEncodedSinkDisabled).Times(1);
    WebMediaStreamTrack track1 = CreateTrack();
    MockMediaStreamVideoSink sink1;
...
TEST_F(MediaStreamVideoSourceTest, CapturingLinkSecureTracksAndEncodedSinks)
{
    ...
    EXPECT_CALL(*mock_source(), OnCapturingLinkSecured(true));
    WebMediaStreamTrack track = CreateTrack();
    mock_source()->UpdateCapturingLinkSecure(MediaStreamVideoTrack::From(track), true);
    ...
    sink.DisconnectEncodedFromTrack();
    EXPECT_CALL(*mock_source(), OnCapturingLinkSecured(false));
    mock_source()->UpdateCapturingLinkSecure(MediaStreamVideoTrack::From(track), false);
}
...
TEST_F(MediaStreamVideoSourceTest, CanDiscardAlphaIfOtherSinksDiscard)
{
    ...
    sink_depends.SetUsesAlpha(MediaStreamVideoSink::UsesAlpha::kDependsOnOtherSinks);
    MockMediaStreamVideoSink sink_alpha;
    sink_alpha.SetUsesAlpha(MediaStreamVideoSink::UsesAlpha::kDefault);
...
{
    WebMediaStreamTrack track = CreateTrackAndStartSource(640, 480, kSourceFrameRate);
    MockMediaStreamVideoSink sink;
    sink.ConnectToTrack(track);
    EXPECT_EQ(track.Source().GetReadyState(), WebMediaStreamSource::kReadyStateLive);
    ...
    source()->ReconfigureTrack(native_track, VideoTrackAdapterSettings(gfx::Size(640, 480), kSourceFrameRate + 1));
    native_track->GetSettings(settings);
