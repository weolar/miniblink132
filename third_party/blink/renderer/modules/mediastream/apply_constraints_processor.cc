...
void RequestFailed(blink::ApplyConstraintsRequest* request, const String& constraint, const String& message)
{
    DCHECK(request);
...
BASE_FEATURE(kApplyConstraintsRestartsVideoContentSources, "ApplyConstraintsRestartsVideoContentSources", base::FEATURE_ENABLED_BY_DEFAULT);
...
void ApplyConstraintsProcessor::ProcessRequest(blink::ApplyConstraintsRequest* request, base::OnceClosure callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (!request->Track()->Source()) {
        CannotApplyConstraints("Track has no source. ApplyConstraints not possible.");
        return;
    }
    request_completed_cb_ = std::move(callback);
    current_request_ = request;
    if (current_request_->Track()->GetSourceType() == MediaStreamSource::kTypeVideo) {
        ProcessVideoRequest();
    } else {
        DCHECK_EQ(current_request_->Track()->GetSourceType(), MediaStreamSource::kTypeAudio);
        ProcessAudioRequest();
    }
}
...
void ApplyConstraintsProcessor::ProcessAudioRequest()
{
    ...
    blink::AudioCaptureSettings settings = SelectSettingsAudioCapture(audio_source, current_request_->Constraints());
...
void ApplyConstraintsProcessor::ProcessVideoRequest()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(current_request_);
    DCHECK_EQ(current_request_->Track()->GetSourceType(), MediaStreamSource::kTypeVideo);
    DCHECK(request_completed_cb_);
    ...
    } else if (base::FeatureList::IsEnabled(kApplyConstraintsRestartsVideoContentSources) && video_source_->GetSubCaptureTargetVersion() == 0
        && (device_info.type == mojom::blink::MediaStreamType::GUM_DESKTOP_VIDEO_CAPTURE
            || device_info.type == mojom::blink::MediaStreamType::DISPLAY_VIDEO_CAPTURE
            || device_info.type == mojom::blink::MediaStreamType::DISPLAY_VIDEO_CAPTURE_THIS_TAB)) {
...
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    video_device_request_trace_ = ScopedMediaStreamTrace::CreateIfEnabled("VideoDeviceRequest");
    ...
    GetMediaDevicesDispatcher()->GetAllVideoInputDeviceFormats(
        String(video_source_->device().id.data()), WTF::BindOnce(&ApplyConstraintsProcessor::MaybeStopVideoDeviceSourceForRestart, WrapWeakPersistent(this)));
}
...
void ApplyConstraintsProcessor::MaybeStopVideoDeviceSourceForRestart(const Vector<media::VideoCaptureFormat>& formats)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (AbortIfVideoRequestStateInvalid())
        return;
    ...
    if (video_source_->GetCurrentFormat() == settings.Format()) {
        video_source_->ReconfigureTrack(GetCurrentVideoTrack(), settings.track_adapter_settings());
        ApplyConstraintsSucceeded();
        ...
        video_source_->StopForRestart(WTF::BindOnce(&ApplyConstraintsProcessor::MaybeDeviceSourceStoppedForRestart, WrapWeakPersistent(this)));
    }
}
...
void ApplyConstraintsProcessor::MaybeStopVideoContentSourceForRestart()
{
    ...
    if (video_source_->GetCurrentFormat() == settings.Format()) {
        if (settings.min_frame_rate().has_value()) {
            GetCurrentVideoTrack()->SetMinimumFrameRate(settings.min_frame_rate().value());
        }
        video_source_->ReconfigureTrack(GetCurrentVideoTrack(), settings.track_adapter_settings());
        ApplyConstraintsSucceeded();
        GetCurrentVideoTrack()->NotifyConstraintsConfigurationComplete();
    } else {
        video_source_->StopForRestart(WTF::BindOnce(&ApplyConstraintsProcessor::MaybeRestartStoppedVideoContentSource, WrapWeakPersistent(this)));
    }
}

void ApplyConstraintsProcessor::MaybeDeviceSourceStoppedForRestart(blink::MediaStreamVideoSource::RestartResult result)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    GetMediaDevicesDispatcher()->GetAvailableVideoInputDeviceFormats(
        String(video_source_->device().id.data()), WTF::BindOnce(&ApplyConstraintsProcessor::FindNewFormatAndRestartDeviceSource, WrapWeakPersistent(this)));
}

void ApplyConstraintsProcessor::MaybeRestartStoppedVideoContentSource(blink::MediaStreamVideoSource::RestartResult result)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    video_source_->Restart(settings.HasValue() ? settings.Format() : *video_source_->GetCurrentFormat(),
        WTF::BindOnce(&ApplyConstraintsProcessor::MaybeSourceRestarted, WrapWeakPersistent(this)));
}

void ApplyConstraintsProcessor::FindNewFormatAndRestartDeviceSource(const Vector<media::VideoCaptureFormat>& formats)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void ApplyConstraintsProcessor::MaybeSourceRestarted(blink::MediaStreamVideoSource::RestartResult result)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (AbortIfVideoRequestStateInvalid())
        return;
...
blink::VideoCaptureSettings ApplyConstraintsProcessor::SelectVideoDeviceSettings(Vector<media::VideoCaptureFormat> formats)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(current_request_);
    DCHECK_EQ(current_request_->Track()->GetSourceType(), MediaStreamSource::kTypeVideo);
    ...
    device_capabilities.facing_mode
        = GetCurrentVideoSource() ? static_cast<mojom::blink::FacingMode>(GetCurrentVideoSource()->device().video_facing) : mojom::blink::FacingMode::kNone;
    device_capabilities.formats = std::move(formats);

    blink::VideoDeviceCaptureCapabilities video_capabilities;
    video_capabilities.noise_reduction_capabilities.push_back(GetCurrentVideoTrack()->noise_reduction());
    video_capabilities.device_capabilities.push_back(std::move(device_capabilities));
    ...
    return SelectSettingsVideoDeviceCapture(video_capabilities, current_request_->Constraints(), settings.width, settings.height, settings.frame_rate);
}

blink::VideoCaptureSettings ApplyConstraintsProcessor::SelectVideoContentSettings()
{
    DCHECK(video_source_);
    gfx::Size screen_size = MediaStreamUtils::GetScreenSize(frame_);
    return blink::SelectSettingsVideoContentCapture(current_request_->Constraints(), video_source_->device().type, screen_size.width(), screen_size.height());
}

blink::MediaStreamAudioSource* ApplyConstraintsProcessor::GetCurrentAudioSource()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(current_request_);
...
blink::MediaStreamVideoTrack* ApplyConstraintsProcessor::GetCurrentVideoTrack()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    MediaStreamVideoTrack* track = MediaStreamVideoTrack::From(current_request_->Track());
    DCHECK(track);
    return track;
}

blink::MediaStreamVideoSource* ApplyConstraintsProcessor::GetCurrentVideoSource()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
bool ApplyConstraintsProcessor::AbortIfVideoRequestStateInvalid()
{
    ...
    if (GetCurrentVideoSource() != video_source_) {
        ...
        CannotApplyConstraints("Track stopped or source changed. ApplyConstraints not possible.");
        return true;
    }
    return false;
}
...
void ApplyConstraintsProcessor::ApplyConstraintsSucceeded()
{
    ...
    task_runner_->PostTask(FROM_HERE,
        WTF::BindOnce(
            &ApplyConstraintsProcessor::CleanupRequest, WrapWeakPersistent(this), WTF::BindOnce(&RequestSucceeded, WrapPersistent(current_request_.Get()))));
}

void ApplyConstraintsProcessor::ApplyConstraintsFailed(const char* failed_constraint_name)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    task_runner_->PostTask(FROM_HERE,
        WTF::BindOnce(&ApplyConstraintsProcessor::CleanupRequest, WrapWeakPersistent(this),
            WTF::BindOnce(&RequestFailed, WrapPersistent(current_request_.Get()), String(failed_constraint_name), String("Cannot satisfy constraints"))));
}
...
void ApplyConstraintsProcessor::CannotApplyConstraints(const String& message)
{
    ...
    task_runner_->PostTask(FROM_HERE,
        WTF::BindOnce(&ApplyConstraintsProcessor::CleanupRequest, WrapWeakPersistent(this),
            WTF::BindOnce(&RequestFailed, WrapPersistent(current_request_.Get()), String(), message)));
}

void ApplyConstraintsProcessor::CleanupRequest(base::OnceClosure user_media_request_callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(current_request_);
    DCHECK(request_completed_cb_);
...
blink::mojom::blink::MediaDevicesDispatcherHost* ApplyConstraintsProcessor::GetMediaDevicesDispatcher()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
