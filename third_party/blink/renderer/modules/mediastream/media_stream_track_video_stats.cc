...
MediaStreamTrackVideoStats::MediaStreamTrackVideoStats(MediaStreamTrackImpl* track)
    : track_(track)
{
}

uint64_t MediaStreamTrackVideoStats::deliveredFrames(ScriptState* script_state)
{
    PopulateStatsCache(script_state);
    return stats_.deliverable_frames;
}

uint64_t MediaStreamTrackVideoStats::discardedFrames(ScriptState* script_state)
{
    PopulateStatsCache(script_state);
...
{
    PopulateStatsCache(script_state);
    return stats_.deliverable_frames + stats_.discarded_frames + stats_.dropped_frames;
}
...
void MediaStreamTrackVideoStats::PopulateStatsCache(ScriptState* script_state)
{
    ...
    ToEventLoop(script_state).EnqueueMicrotask(WTF::BindOnce(&MediaStreamTrackVideoStats::InvalidateStatsCache, WrapWeakPersistent(this)));
}
