...
TransferredMediaStreamTrack::TransferredMediaStreamTrack(ExecutionContext* execution_context, const TransferredValues& data)
    : transferred_component_(MakeGarbageCollected<TransferredMediaStreamComponent>(TransferredMediaStreamComponent::TransferredValues { .id = data.id }))
...
MediaStreamTrack* TransferredMediaStreamTrack::clone(ExecutionContext* execution_context)
{
    if (track_) {
        return track_->clone(execution_context);
    }

    auto* cloned_tmst = MakeGarbageCollected<TransferredMediaStreamTrack>(execution_context, data_);
...
void TransferredMediaStreamTrack::stopTrack(ExecutionContext* execution_context)
{
    if (track_) {
...
V8UnionMediaStreamTrackAudioStatsOrMediaStreamTrackVideoStats* TransferredMediaStreamTrack::stats()
{
    if (track_) {
        return track_->stats();
    }
...
ScriptPromise<IDLUndefined> TransferredMediaStreamTrack::applyConstraints(ScriptState* script_state, const MediaTrackConstraints* constraints)
{
    if (track_) {
...
void TransferredMediaStreamTrack::applyConstraints(ScriptPromiseResolver<IDLUndefined>* resolver, const MediaTrackConstraints* constraints)
{
    setter_call_order_.push_back(APPLY_CONSTRAINTS);
    constraints_list_.push_back(MakeGarbageCollected<ConstraintsPair>(resolver, constraints));
}
...
void TransferredMediaStreamTrack::SetComponentImplementation(MediaStreamComponent* component)
{
    transferred_component_->SetImplementation(component);
}

void TransferredMediaStreamTrack::SetInitialConstraints(const MediaConstraints& constraints)
{
    if (track_) {
...
void TransferredMediaStreamTrack::SetConstraints(const MediaConstraints& constraints)
{
    if (track_) {
...
void TransferredMediaStreamTrack::AddedEventListener(const AtomicString& event_type, RegisteredEventListener& registered_listener)
{
    if (track_) {
...
std::unique_ptr<AudioSourceProvider> TransferredMediaStreamTrack::CreateWebAudioSource(int context_sample_rate, base::TimeDelta platform_buffer_duration)
{
    if (track_) {
        return track_->CreateWebAudioSource(context_sample_rate, platform_buffer_duration);
    }
...
std::optional<const MediaStreamDevice> TransferredMediaStreamTrack::device() const
{
    if (track_) {
...
void TransferredMediaStreamTrack::BeingTransferred(const base::UnguessableToken& transfer_id)
{
    if (track_) {
...
TransferredMediaStreamTrack::EventPropagator::EventPropagator(MediaStreamTrack* underlying_track, TransferredMediaStreamTrack* transferred_track)
    : transferred_track_(transferred_track)
{
    ...
    underlying_track->addEventListener(event_type_names::kCapturehandlechange, this);
}

void TransferredMediaStreamTrack::EventPropagator::Invoke(ExecutionContext*, Event* event)
{
    transferred_track_->DispatchEvent(*event);
}

void TransferredMediaStreamTrack::EventPropagator::Trace(Visitor* visitor) const
{
...
TransferredMediaStreamTrack::ConstraintsPair::ConstraintsPair(ScriptPromiseResolver<IDLUndefined>* resolver, const MediaTrackConstraints* constraints)
    : resolver(resolver)
    , constraints(constraints)
{
}

void TransferredMediaStreamTrack::ConstraintsPair::Trace(Visitor* visitor) const
{
