...
void AddSinkToMediaStreamTrack(const WebMediaStreamTrack& track, WebMediaStreamSink* sink, const VideoCaptureDeliverFrameCB& callback,
    MediaStreamVideoSink::IsSecure is_secure, MediaStreamVideoSink::UsesAlpha uses_alpha)
{
    static_cast<MediaStreamComponent*>(track)->AddSink(sink, callback, is_secure, uses_alpha);
}

void RemoveSinkFromMediaStreamTrack(const WebMediaStreamTrack& track, WebMediaStreamSink* sink)
{
...
void MediaStreamVideoSink::ConnectToTrack(const WebMediaStreamTrack& track, const VideoCaptureDeliverFrameCB& callback,
    MediaStreamVideoSink::IsSecure is_secure, MediaStreamVideoSink::UsesAlpha uses_alpha)
{
...
void MediaStreamVideoSink::ConnectEncodedToTrack(const WebMediaStreamTrack& track, const EncodedVideoFrameCB& callback)
{
...
{
    MediaStreamVideoTrack* const video_track = MediaStreamVideoTrack::From(connected_encoded_track_);
    if (video_track) {
...
void MediaStreamVideoSink::OnFrameDropped(media::VideoCaptureFrameDropReason reason)
{
