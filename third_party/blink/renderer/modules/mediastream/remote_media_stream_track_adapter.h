...
class MODULES_EXPORT RemoteMediaStreamTrackAdapter : public WTF::ThreadSafeRefCounted<RemoteMediaStreamTrackAdapter<WebRtcMediaStreamTrackType>> {
public:
    RemoteMediaStreamTrackAdapter(
        const scoped_refptr<base::SingleThreadTaskRunner>& main_thread, WebRtcMediaStreamTrackType* webrtc_track, ExecutionContext* track_execution_context)
    ...
    RemoteMediaStreamTrackAdapter& operator=(const RemoteMediaStreamTrackAdapter&) = delete;
...
protected:
    friend class WTF::ThreadSafeRefCounted<RemoteMediaStreamTrackAdapter<WebRtcMediaStreamTrackType>>;
    ...
    void InitializeTrack(MediaStreamSource::StreamType type, std::unique_ptr<WebPlatformMediaStreamSource> platform_source,
        std::unique_ptr<MediaStreamTrackPlatform> platform_track)
    {
        DCHECK(main_thread_->BelongsToCurrentThread());
        DCHECK(!component_);

        auto* source = MakeGarbageCollected<MediaStreamSource>(id_, type, id_, true /*remote*/, std::move(platform_source));
        component_ = MakeGarbageCollected<MediaStreamComponentImpl>(id_, source, std::move(platform_track));
        ...
        if (track_execution_context_ && track_execution_context_->IsWindow() && To<LocalDOMWindow>(track_execution_context_.Get())->GetFrame()) {
            // IsWindow() being true means that the ExecutionContext is a
            // LocalDOMWindow, so these casts should be safe.
            component_->SetCreationFrameGetter(
                WTF::BindRepeating([](LocalFrame* local_frame) { return local_frame ? WebFrame::FromCoreFrame(local_frame)->ToWebLocalFrame() : nullptr; },
                    WrapWeakPersistent(To<LocalDOMWindow>(track_execution_context_.Get())->GetFrame())));
        }
        DCHECK(component_);
    }
...
class MODULES_EXPORT RemoteVideoTrackAdapter : public RemoteMediaStreamTrackAdapter<webrtc::VideoTrackInterface> {
public:
    // Called on the signaling thread.
    RemoteVideoTrackAdapter(
        const scoped_refptr<base::SingleThreadTaskRunner>& main_thread, webrtc::VideoTrackInterface* webrtc_track, ExecutionContext* execution_context);

protected:
    ~RemoteVideoTrackAdapter() override;

private:
    void InitializeWebVideoTrack(std::unique_ptr<TrackObserver> observer, bool enabled);
};
...
// Blink.
class MODULES_EXPORT RemoteAudioTrackAdapter : public RemoteMediaStreamTrackAdapter<webrtc::AudioTrackInterface>, public webrtc::ObserverInterface {
public:
    // Called on the signaling thread.
    RemoteAudioTrackAdapter(
        const scoped_refptr<base::SingleThreadTaskRunner>& main_thread, webrtc::AudioTrackInterface* webrtc_track, ExecutionContext* execution_context);
...
private:
    void InitializeWebAudioTrack(const scoped_refptr<base::SingleThreadTaskRunner>& main_thread);

    // webrtc::ObserverInterface implementation.
    void OnChanged() override;

    void OnChangedOnMainThread(webrtc::MediaStreamTrackInterface::TrackState state);
