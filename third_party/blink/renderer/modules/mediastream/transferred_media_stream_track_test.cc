...
public:
    void CustomSetUp(V8TestingScope& scope)
    {
        ...
        transferred_track_ = MakeGarbageCollected<TransferredMediaStreamTrack>(scope.GetExecutionContext(),
...
TEST_F(TransferredMediaStreamTrackTest, InitialProperties)
{
    ...
    EXPECT_EQ(transferred_track_->GetReadyState(), MediaStreamSource::kReadyStateLive);
...
TEST_F(TransferredMediaStreamTrackTest, PropertiesInheritFromImplementation)
{
    ...
    const V8MediaStreamTrackState::Enum kReadyState = V8MediaStreamTrackState::Enum::kEnded;
    const MediaStreamSource::ReadyState kReadyStateEnum = MediaStreamSource::kReadyStateEnded;
    const bool kEnded = true;
    base::UnguessableToken kSerializableSessionId = base::UnguessableToken::Create();
    MediaStreamDevice kDevice = MediaStreamDevice();
    kDevice.set_session_id(kSerializableSessionId);

    Persistent<MediaTrackCapabilities> capabilities = MediaTrackCapabilities::Create();
    Persistent<MediaTrackConstraints> constraints = MediaTrackConstraints::Create();
    ...
    EXPECT_EQ(transferred_track_->device()->serializable_session_id(), kSerializableSessionId);
}
...
TEST_F(TransferredMediaStreamTrackTest, EventsArePropagated)
{
    ...
    transferred_track_->addEventListener(event_type_names::kEnded, mock_event_handler);
    ...
    ASSERT_EQ(mock_impl_->DispatchEvent(*Event::Create(event_type_names::kEnded)), DispatchEventResult::kNotCanceled);
}

TEST_F(TransferredMediaStreamTrackTest, ConstraintsAppliedBeforeImplementation)
{
    V8TestingScope scope;
    CustomSetUp(scope);

    mock_impl_->SetExecutionContext(scope.GetExecutionContext());
    transferred_track_->applyConstraints(scope.GetScriptState(), MediaTrackConstraints::Create());
    EXPECT_CALL(*mock_impl_, AddedEventListener(_, _)).Times(4);
...
TEST_F(TransferredMediaStreamTrackTest, SetImplementationTriggersObservers)
{
    ...
    transferred_track_->SetImplementation(MakeGarbageCollected<testing::NiceMock<MockMediaStreamTrack>>());
...
TEST_F(TransferredMediaStreamTrackTest, ObserversAddedToImpl)
{
    ...
    MockMediaStreamTrack* mock_impl_ = MakeGarbageCollected<testing::NiceMock<MockMediaStreamTrack>>();
...
TEST_F(TransferredMediaStreamTrackTest, CloneSetImplementation)
{
    ...
    MockMediaStreamTrack* mock_impl_ = MakeGarbageCollected<testing::NiceMock<MockMediaStreamTrack>>();
    EXPECT_CALL(*mock_impl_, clone(_)).WillOnce(Return(MakeGarbageCollected<testing::NiceMock<MockMediaStreamTrack>>()));
...
TEST_F(TransferredMediaStreamTrackTest, CloneDoesntIncludeLaterMutations)
{
    ...
    EXPECT_CALL(*mock_impl_, clone(_)).WillOnce([&](ExecutionContext*) {
        MockMediaStreamTrack* mock_clone_impl_ = MakeGarbageCollected<testing::NiceMock<MockMediaStreamTrack>>();
