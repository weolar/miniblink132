...
using StringSetFunction = void (blink::StringConstraint::*)(const blink::WebString&);
using MockFactoryAccessor = MediaTrackConstraintSetPlatform& (blink::MockConstraintFactory::*)();
...
const MockFactoryAccessor kFactoryAccessors[] = { &blink::MockConstraintFactory::basic, &blink::MockConstraintFactory::AddAdvanced };

const bool kBoolValues[] = { true, false };

const int kMinChannels = 1;

using AudioSettingsBoolMembers = WTF::Vector<bool (AudioCaptureSettings::*)() const>;
using AudioPropertiesBoolMembers = WTF::Vector<bool AudioProcessingProperties::*>;

template <typename T> static bool Contains(const WTF::Vector<T>& vector, T value)
{
...
protected:
    ...
    void ResetFactory()
    {
        ...
        constraint_factory_.basic().media_stream_source.SetExact(String::FromUTF8(GetMediaStreamSource()));
    }

    // If not overridden, this function will return device capture by default.
    virtual std::string GetMediaStreamSource()
    {
        return std::string();
    }
    bool IsDeviceCapture()
    {
        return GetMediaStreamSource().empty();
    }
    static AudioPropertiesBoolMembers GetAudioProcessingProperties()
    {
        return { &AudioProcessingProperties::goog_audio_mirroring, &AudioProcessingProperties::goog_auto_gain_control,
            &AudioProcessingProperties::goog_noise_suppression, &AudioProcessingProperties::goog_experimental_noise_suppression,
    ...
    std::unique_ptr<ProcessedLocalAudioSource> GetProcessedLocalAudioSource(
        const AudioProcessingProperties& properties, bool disable_local_echo, bool render_to_associated_sink, int effects, int num_requested_channels)
    {
        ...
        return std::make_unique<ProcessedLocalAudioSource>(*MainFrame().GetFrame(), device, disable_local_echo, properties, num_requested_channels,
            base::NullCallback(), blink::scheduler::GetSingleThreadTaskRunnerForTesting());
    }

    std::unique_ptr<ProcessedLocalAudioSource> GetProcessedLocalAudioSource(
        const AudioProcessingProperties& properties, bool disable_local_echo, bool render_to_associated_sink)
    {
        return GetProcessedLocalAudioSource(
            properties, disable_local_echo, render_to_associated_sink, media::AudioParameters::PlatformEffectsMask::NO_EFFECTS, 1 /* num_requested_channels */);
    }

    std::unique_ptr<blink::LocalMediaStreamAudioSource> GetLocalMediaStreamAudioSource(
        bool enable_system_echo_canceller, bool disable_local_echo, bool render_to_associated_sink, const int* requested_buffer_size = nullptr)
    {
        blink::MediaStreamDevice device;
        device.type = GetMediaStreamType();
        ...
        return std::make_unique<blink::LocalMediaStreamAudioSource>(
            ...
            /*enable_system_echo_canceller=*/enable_system_echo_canceller, blink::WebPlatformMediaStreamSource::ConstraintsRepeatingCallback(),
            blink::scheduler::GetSingleThreadTaskRunnerForTesting());
    }

    AudioCaptureSettings SelectSettings(bool is_reconfigurable = false, std::optional<AudioDeviceCaptureCapabilities> capabilities = std::nullopt)
    {
        MediaConstraints constraints = constraint_factory_.CreateMediaConstraints();
        if (capabilities) {
            return SelectSettingsAudioCapture(*capabilities, constraints, GetMediaStreamType(), false, is_reconfigurable);
        } else {
            return SelectSettingsAudioCapture(capabilities_, constraints, GetMediaStreamType(), false, is_reconfigurable);
        }
    }

    base::expected<Vector<blink::AudioCaptureSettings>, std::string> SelectEligibleSettings(bool is_reconfigurable = false)
    {
        MediaConstraints constraints = constraint_factory_.CreateMediaConstraints();
        return SelectEligibleSettingsAudioCapture(capabilities_, constraints, GetMediaStreamType(),
            /*should_disable_hardware_noise_suppression=*/false, is_reconfigurable);
    }

    void CheckBoolDefaultsDeviceCapture(
        const AudioSettingsBoolMembers& exclude_main_settings, const AudioPropertiesBoolMembers& exclude_audio_properties, const AudioCaptureSettings& result)
    {
        if (!Contains(exclude_main_settings, &AudioCaptureSettings::disable_local_echo)) {
            EXPECT_TRUE(result.disable_local_echo());
        }
        if (!Contains(exclude_main_settings, &AudioCaptureSettings::render_to_associated_sink)) {
            EXPECT_FALSE(result.render_to_associated_sink());
        }

        const auto& properties = result.audio_processing_properties();
        if (!Contains(exclude_audio_properties, &AudioProcessingProperties::goog_audio_mirroring)) {
            EXPECT_FALSE(properties.goog_audio_mirroring);
        }
        if (!Contains(exclude_audio_properties, &AudioProcessingProperties::goog_auto_gain_control)) {
            EXPECT_TRUE(properties.goog_auto_gain_control);
        }
        if (!Contains(exclude_audio_properties, &AudioProcessingProperties::goog_noise_suppression)) {
            EXPECT_TRUE(properties.goog_noise_suppression);
        }
        if (!Contains(exclude_audio_properties, &AudioProcessingProperties::goog_experimental_noise_suppression)) {
            EXPECT_TRUE(properties.goog_experimental_noise_suppression);
        }
        if (!Contains(exclude_audio_properties, &AudioProcessingProperties::goog_highpass_filter)) {
    ...
    {
        if (!Contains(exclude_main_settings, &AudioCaptureSettings::disable_local_echo)) {
            EXPECT_EQ(GetMediaStreamSource() != blink::kMediaStreamSourceDesktop, result.disable_local_echo());
        }
        if (!Contains(exclude_main_settings, &AudioCaptureSettings::render_to_associated_sink)) {
            EXPECT_FALSE(result.render_to_associated_sink());
        }
    ...
    {
        if (IsDeviceCapture()) {
            CheckBoolDefaultsDeviceCapture(exclude_main_settings, exclude_audio_properties, result);
        } else {
            CheckBoolDefaultsContentCapture(exclude_main_settings, exclude_audio_properties, result);
        }
    }
    ...
    void CheckEchoCancellationTypeDefault(const AudioCaptureSettings& result)
    {
        const auto& properties = result.audio_processing_properties();
        if (IsDeviceCapture()) {
            EXPECT_EQ(properties.echo_cancellation_type, EchoCancellationType::kEchoCancellationAec3);
        } else {
            EXPECT_EQ(properties.echo_cancellation_type, EchoCancellationType::kEchoCancellationDisabled);
        }
    }
    ...
    void CheckProcessingType(const AudioCaptureSettings& result)
    {
        ...
        if (properties.echo_cancellation_type == EchoCancellationType::kEchoCancellationAec3) {
            expected_type = ProcessingType::kApmProcessed;
        }
        EXPECT_EQ(result.processing_type(), expected_type);
    }

    void CheckDevice(const AudioDeviceCaptureCapability& expected_device, const AudioCaptureSettings& result)
    {
    ...
    void CheckAudioProcessingPropertiesForIdealEchoCancellationType(const AudioCaptureSettings& result)
    {
        const AudioProcessingProperties& properties = result.audio_processing_properties();

        EXPECT_EQ(EchoCancellationType::kEchoCancellationSystem, properties.echo_cancellation_type);
        EXPECT_TRUE(properties.goog_auto_gain_control);
        EXPECT_TRUE(properties.goog_noise_suppression);
        EXPECT_TRUE(properties.goog_experimental_noise_suppression);
        EXPECT_TRUE(properties.goog_highpass_filter);
    ...
    EchoCancellationType GetEchoCancellationTypeFromConstraintString(const blink::WebString& constraint_string)
    {
        ...
        ADD_FAILURE() << "Invalid echo cancellation type constraint: " << constraint_string.Ascii();
        return EchoCancellationType::kEchoCancellationDisabled;
    }

    void CheckLatencyConstraint(const AudioDeviceCaptureCapability* device, double min_latency, double max_latency)
    {
        constraint_factory_.Reset();
        ...
        CheckLocalMediaStreamAudioSourceLatency(device, 0.001, min_latency * device->Parameters().sample_rate());
        CheckLocalMediaStreamAudioSourceLatency(device, 1.0, max_latency * device->Parameters().sample_rate());
    }

    void CheckLocalMediaStreamAudioSourceLatency(const AudioDeviceCaptureCapability* device, double requested_latency, int expected_buffer_size)
    {
        constraint_factory_.Reset();
        constraint_factory_.basic().device_id.SetExact(device->DeviceID());
        constraint_factory_.basic().echo_cancellation.SetExact(false);
        ...
        std::unique_ptr<blink::LocalMediaStreamAudioSource> local_source = GetLocalMediaStreamAudioSource(false /* enable_system_echo_canceller */,
            false /* disable_local_echo */, false /* render_to_associated_sink */, base::OptionalToPtr(result.requested_buffer_size()));
        EXPECT_EQ(local_source->GetAudioParameters().frames_per_buffer(), expected_buffer_size);
    }
    ...
    raw_ptr<const AudioDeviceCaptureCapability> system_echo_canceller_device_ = nullptr;
    raw_ptr<const AudioDeviceCaptureCapability> four_channels_device_ = nullptr;
    raw_ptr<const AudioDeviceCaptureCapability> variable_latency_device_ = nullptr;
    ...
    const WTF::Vector<blink::WebString> kEchoCancellationTypeValues
        = { blink::WebString::FromASCII("browser"), blink::WebString::FromASCII("aec3"), blink::WebString::FromASCII("system") };

private:
    ScopedTestingPlatformSupport<IOTaskRunnerTestingPlatformSupport> platform_;
};

class MediaStreamConstraintsUtilAudioTest : public MediaStreamConstraintsUtilAudioTestBase, public testing::WithParamInterface<std::string> {
public:
    void SetUp() override
    {
        ...
        if (IsDeviceCapture()) {
            capabilities_.emplace_back("default_device", "fake_group1",
                media::AudioParameters(
                    media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), media::AudioParameters::kAudioCDSampleRate, 1000));

            media::AudioParameters system_echo_canceller_parameters(
                media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), media::AudioParameters::kAudioCDSampleRate, 1000);
            system_echo_canceller_parameters.set_effects(media::AudioParameters::ECHO_CANCELLER);
            capabilities_.emplace_back("system_echo_canceller_device", "fake_group2", system_echo_canceller_parameters);

            capabilities_.emplace_back("4_channels_device", "fake_group3",
                media::AudioParameters(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::FromLayout<media::CHANNEL_LAYOUT_4_0>(),
                    media::AudioParameters::kAudioCDSampleRate, 1000));

            capabilities_.emplace_back("8khz_sample_rate_device", "fake_group4",
                media::AudioParameters(
                    media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), webrtc::AudioProcessing::kSampleRate8kHz, 1000));

            capabilities_.emplace_back("variable_latency_device", "fake_group5",
                media::AudioParameters(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(),
                    media::AudioParameters::kAudioCDSampleRate, 512, media::AudioParameters::HardwareCapabilities(128, 4096)));
...
class MediaStreamConstraintsRemoteAPMTest : public MediaStreamConstraintsUtilAudioTestBase,
                                            public testing::WithParamInterface<std::tuple<std::string, ChromeWideAecExperiment>> {
protected:
    std::string GetMediaStreamSource() override
    {
    ...
    testing::Message GetMessageForScopedTrace()
    {
        ...
        return testing::Message() << "GetMediaStreamSource()=\"" << GetMediaStreamSource() << "\", GetChromeWideAecExperiment()=" << experiment_string;
    }

    // Indicates where and how audio processing is applied.
    enum class ApmLocation { kProcessedLocalAudioSource, kAudioService, kAudioServiceAvoidResampling };

    ApmLocation GetApmLocation()
    {
        if (GetMediaStreamType() != mojom::blink::MediaStreamType::DEVICE_AUDIO_CAPTURE) {
...
private:
    void SetUp() override
    {
        ...
        switch (GetChromeWideAecExperiment()) {
        case ChromeWideAecExperiment::kDisabled:
            scoped_feature_list_.InitAndDisableFeature(media::kChromeWideEchoCancellation);
            break;
        case ChromeWideAecExperiment::kEnabled:
            scoped_feature_list_.InitAndEnableFeature(media::kChromeWideEchoCancellation);
            break;
        }
#endif
...
TEST_P(MediaStreamConstraintsUtilAudioTest, Unconstrained)
{
    ...
    EXPECT_TRUE(result.HasValue());
    CheckAllDefaults(AudioSettingsBoolMembers(), AudioPropertiesBoolMembers(), result);
}
...
{
    AudioSettingsBoolMembers kMainSettings = { &AudioCaptureSettings::disable_local_echo, &AudioCaptureSettings::render_to_associated_sink };

    const WTF::Vector<blink::BooleanConstraint MediaTrackConstraintSetPlatform::*> kMainBoolConstraints
        = { &MediaTrackConstraintSetPlatform::disable_local_echo, &MediaTrackConstraintSetPlatform::render_to_associated_sink };
    ...
    for (auto set_function : kBoolSetFunctions) {
        for (auto accessor : kFactoryAccessors) {
            ...
            if (set_function == kBoolSetFunctions[1] && accessor == kFactoryAccessors[1]) {
                continue;
            }
            for (WTF::wtf_size_t i = 0; i < kMainSettings.size(); ++i) {
                for (bool value : kBoolValues) {
                    ResetFactory();
                    (((constraint_factory_.*accessor)().*kMainBoolConstraints[i]).*set_function)(value);
                    auto result = SelectSettings();
                    EXPECT_TRUE(result.HasValue());
                    EXPECT_EQ(value, (result.*kMainSettings[i])());
                    CheckAllDefaults({ kMainSettings[i] }, AudioPropertiesBoolMembers(), result);
                }
            }
        }
    }

    const WTF::Vector<blink::BooleanConstraint MediaTrackConstraintSetPlatform::*> kAudioProcessingConstraints = {
    ...
    ASSERT_EQ(GetAudioProcessingProperties().size(), kAudioProcessingConstraints.size());
    for (auto set_function : kBoolSetFunctions) {
        for (auto accessor : kFactoryAccessors) {
            // Ideal advanced is ignored by the SelectSettings algorithm.
            // Using array elements instead of pointer values due to the comparison
            ...
            for (WTF::wtf_size_t i = 0; i < GetAudioProcessingProperties().size(); ++i) {
                for (bool value : kBoolValues) {
                    ResetFactory();
                    (((constraint_factory_.*accessor)().*kAudioProcessingConstraints[i]).*set_function)(value);
                    auto result = SelectSettings();
                    EXPECT_TRUE(result.HasValue());
                    EXPECT_EQ(value, result.audio_processing_properties().*GetAudioProcessingProperties()[i]);
                    CheckAllDefaults(AudioSettingsBoolMembers(), { GetAudioProcessingProperties()[i] }, result);
                }
            }
        }
    }
...
TEST_P(MediaStreamConstraintsUtilAudioTest, Channels)
{
    ...
    // Only set a min value for the constraint.
    ResetFactory();
    constraint_factory_.basic().channel_count.SetMin(media::limits::kMaxChannels + 1);
    result = SelectSettings();
    EXPECT_FALSE(result.HasValue());
    ...
    constraint_factory_.basic().channel_count.SetMin(media::limits::kMaxChannels + 1);
    constraint_factory_.basic().channel_count.SetMax(media::limits::kMaxChannels + 10);
    result = SelectSettings();
    EXPECT_FALSE(result.HasValue());
...
TEST_P(MediaStreamConstraintsUtilAudioTest, MultiChannelEchoCancellation)
{
    ...
    // and 1 channel,
    EXPECT_EQ(result.audio_processing_properties().echo_cancellation_type, EchoCancellationType::kEchoCancellationAec3);
...
TEST_P(MediaStreamConstraintsUtilAudioTest, ChannelsWithSource)
{
    ...
    std::unique_ptr<blink::LocalMediaStreamAudioSource> source
        = GetLocalMediaStreamAudioSource(false /* enable_system_echo_canceller */, false /* disable_local_echo */, false /* render_to_associated_sink */);
    int channel_count = kMinChannels;
    for (; channel_count <= media::limits::kMaxChannels; ++channel_count) {
        ResetFactory();
        constraint_factory_.basic().channel_count.SetExact(channel_count);
        auto result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
        if (channel_count == 2)
            EXPECT_TRUE(result.HasValue());
        else
            EXPECT_FALSE(result.HasValue());
...
TEST_P(MediaStreamConstraintsUtilAudioTest, SampleRate)
{
    ...
    int max_sample_rate = std::max(static_cast<int>(media::AudioParameters::kAudioCDSampleRate), media::WebRtcAudioProcessingSampleRateHz());
    int ideal_sample_rate = webrtc::AudioProcessing::kSampleRate8kHz;
    if (!IsDeviceCapture()) {
        exact_sample_rate = media::AudioParameters::kAudioCDSampleRate;
        min_sample_rate = std::min(static_cast<int>(media::AudioParameters::kAudioCDSampleRate), media::WebRtcAudioProcessingSampleRateHz());
    ...
    if (IsDeviceCapture()) {
        constraint_factory_.basic().sample_rate.SetIdeal(webrtc::AudioProcessing::kSampleRate48kHz + 1000);
        result = SelectSettings();
        EXPECT_TRUE(result.HasValue());
        EXPECT_EQ(result.device_id(), "default_device");
    }
...
TEST_P(MediaStreamConstraintsUtilAudioTest, SampleRateWithSource)
{
    ...
    // Test set exact sampleRate.
    ResetFactory();
    constraint_factory_.basic().sample_rate.SetExact(media::AudioParameters::kAudioCDSampleRate);
    auto result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
    EXPECT_TRUE(result.HasValue());

    constraint_factory_.basic().sample_rate.SetExact(11111);
    result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
    EXPECT_FALSE(result.HasValue());

    // Test set min sampleRate.
    ResetFactory();
    constraint_factory_.basic().sample_rate.SetMin(media::AudioParameters::kAudioCDSampleRate);
    result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
    EXPECT_TRUE(result.HasValue());

    constraint_factory_.basic().sample_rate.SetMin(media::AudioParameters::kAudioCDSampleRate + 1);
    result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
    EXPECT_FALSE(result.HasValue());

    // Test set max sampleRate.
    ResetFactory();
    constraint_factory_.basic().sample_rate.SetMax(media::AudioParameters::kAudioCDSampleRate);
    result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
    EXPECT_TRUE(result.HasValue());

    constraint_factory_.basic().sample_rate.SetMax(media::AudioParameters::kAudioCDSampleRate - 1);
    result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
    EXPECT_FALSE(result.HasValue());

    // Test set ideal sampleRate.
    ResetFactory();
    constraint_factory_.basic().sample_rate.SetIdeal(media::AudioParameters::kAudioCDSampleRate);
    result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
    EXPECT_TRUE(result.HasValue());

    constraint_factory_.basic().sample_rate.SetIdeal(media::AudioParameters::kAudioCDSampleRate - 1);
    result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
    EXPECT_TRUE(result.HasValue());
}
...
TEST_P(MediaStreamConstraintsUtilAudioTest, Latency)
{
    ...
    constraint_factory_.basic().latency.SetExact(static_cast<double>(blink::kFallbackAudioLatencyMs) / 1000);
    result = SelectSettings();
    EXPECT_TRUE(result.HasValue());
    if (IsDeviceCapture())
        EXPECT_EQ(result.device_id(), "default_device");
...
TEST_P(MediaStreamConstraintsUtilAudioTest, LatencyWithSource)
{
    ...
    // Test set max sampleRate.
    ResetFactory();
    constraint_factory_.basic().latency.SetMax(static_cast<double>(blink::kFallbackAudioLatencyMs) / 1000);
    result = SelectSettings();
    EXPECT_TRUE(result.HasValue());
...
TEST_P(MediaStreamConstraintsUtilAudioTest, ExactArbitraryDeviceID)
{
    ...
    if (IsDeviceCapture()) {
        EXPECT_FALSE(result.HasValue());
        EXPECT_EQ(std::string(constraint_factory_.basic().device_id.GetName()), std::string(result.failed_constraint_name()));
    } else {
        EXPECT_TRUE(result.HasValue());
        EXPECT_EQ(kArbitraryDeviceID.Utf8(), result.device_id());
        CheckBoolDefaults(AudioSettingsBoolMembers(), AudioPropertiesBoolMembers(), result);
        CheckEchoCancellationTypeDefault(result);
    }
}
...
TEST_P(MediaStreamConstraintsUtilAudioTest, ExactValidDeviceID)
{
    for (const auto& device : capabilities_) {
        ...
        CheckProcessingType(result);
        CheckBoolDefaults(AudioSettingsBoolMembers(), AudioPropertiesBoolMembers(), result);
        EchoCancellationType expected_echo_cancellation_type = EchoCancellationType::kEchoCancellationDisabled;
        if (IsDeviceCapture()) {
            const bool has_system_echo_cancellation = device.Parameters().effects() & media::AudioParameters::ECHO_CANCELLER;
            expected_echo_cancellation_type
                = has_system_echo_cancellation ? EchoCancellationType::kEchoCancellationSystem : EchoCancellationType::kEchoCancellationAec3;
        }
        EXPECT_EQ(expected_echo_cancellation_type, result.audio_processing_properties().echo_cancellation_type);
    }
}
...
TEST_P(MediaStreamConstraintsUtilAudioTest, EchoCancellationWithWebRtc)
{
    for (auto set_function : kBoolSetFunctions) {
        for (auto accessor : kFactoryAccessors) {
            ...
            for (bool value : kBoolValues) {
                ResetFactory();
                ((constraint_factory_.*accessor)().echo_cancellation.*set_function)(value);
                auto result = SelectSettings();
                EXPECT_TRUE(result.HasValue());
                const AudioProcessingProperties& properties = result.audio_processing_properties();
                ...
                const EchoCancellationType expected_echo_cancellation_type
                    = value ? EchoCancellationType::kEchoCancellationAec3 : EchoCancellationType::kEchoCancellationDisabled;
                EXPECT_EQ(expected_echo_cancellation_type, properties.echo_cancellation_type);
                const bool enable_webrtc_audio_processing = IsDeviceCapture() ? value : false;
                EXPECT_EQ(enable_webrtc_audio_processing, properties.goog_auto_gain_control);
                EXPECT_EQ(enable_webrtc_audio_processing, properties.goog_noise_suppression);
                EXPECT_EQ(enable_webrtc_audio_processing, properties.goog_experimental_noise_suppression);
...
TEST_P(MediaStreamConstraintsUtilAudioTest, EchoCancellationWithSystem)
{
    ...
    for (auto set_function : kBoolSetFunctions) {
        for (auto accessor : kFactoryAccessors) {
            ...
            for (bool value : kBoolValues) {
                ResetFactory();
                constraint_factory_.basic().device_id.SetExact(system_echo_canceller_device_->DeviceID());
                ((constraint_factory_.*accessor)().echo_cancellation.*set_function)(value);
                auto result = SelectSettings();
                EXPECT_TRUE(result.HasValue());
                const AudioProcessingProperties& properties = result.audio_processing_properties();
                ...
                const EchoCancellationType expected_echo_cancellation_type
                    = value ? EchoCancellationType::kEchoCancellationSystem : EchoCancellationType::kEchoCancellationDisabled;
                EXPECT_EQ(expected_echo_cancellation_type, properties.echo_cancellation_type);
...
TEST_P(MediaStreamConstraintsUtilAudioTest, EchoCancellationAndSingleBoolConstraint)
{
    const WTF::Vector<blink::BooleanConstraint MediaTrackConstraintSetPlatform::*> kAudioProcessingConstraints = {
        &MediaTrackConstraintSetPlatform::goog_audio_mirroring,
        &MediaTrackConstraintSetPlatform::goog_auto_gain_control,
    ...
    for (auto set_function : kBoolSetFunctions) {
        for (auto accessor : kFactoryAccessors) {
            ...
            for (WTF::wtf_size_t i = 0; i < GetAudioProcessingProperties().size(); ++i) {
                ResetFactory();
                ((constraint_factory_.*accessor)().echo_cancellation.*set_function)(false);
                (((constraint_factory_.*accessor)().*kAudioProcessingConstraints[i]).*set_function)(true);
                auto result = SelectSettings();
                EXPECT_TRUE(result.HasValue());
                CheckProcessingType(result);
                EXPECT_EQ(EchoCancellationType::kEchoCancellationDisabled, result.audio_processing_properties().echo_cancellation_type);
                EXPECT_TRUE(result.audio_processing_properties().*GetAudioProcessingProperties()[i]);
                for (WTF::wtf_size_t j = 0; j < GetAudioProcessingProperties().size(); ++j) {
                    if (i == j)
                        continue;
                    EXPECT_FALSE(result.audio_processing_properties().*GetAudioProcessingProperties()[j]);
                }
            }
        }
    }
...
TEST_P(MediaStreamConstraintsUtilAudioTest, ReconfigurationSystemEchoCancellation)
{
    ...
    std::unique_ptr<ProcessedLocalAudioSource> system_echo_canceller_source = GetProcessedLocalAudioSource(properties, false /* disable_local_echo */,
        false /* render_to_associated_sink */, media::AudioParameters::PlatformEffectsMask::ECHO_CANCELLER, 1 /* num_requested_channels */);
    AudioDeviceCaptureCapabilities capabilities = { AudioDeviceCaptureCapability(system_echo_canceller_source.get()) };
    AudioDeviceCaptureCapability* system_echo_canceller_with_source = &capabilities[0];

    constraint_factory_.Reset();
    constraint_factory_.basic().device_id.SetExact(system_echo_canceller_with_source->DeviceID());
    constraint_factory_.basic().echo_cancellation.SetExact(true);
...
TEST_P(MediaStreamConstraintsUtilAudioTest, VoiceIsolationControl)
{
    ...
    EXPECT_EQ(settings.audio_processing_properties().voice_isolation, AudioProcessingProperties::VoiceIsolationType::kVoiceIsolationEnabled);

    constraint_factory_.Reset();
    constraint_factory_.basic().voice_isolation.SetExact(false);
    settings = SelectSettings(true, capabilities_);
    EXPECT_TRUE(settings.HasValue());
    EXPECT_EQ(settings.audio_processing_properties().voice_isolation, AudioProcessingProperties::VoiceIsolationType::kVoiceIsolationDisabled);
    constraint_factory_.Reset();
    settings = SelectSettings(true, capabilities_);
    EXPECT_TRUE(settings.HasValue());
    EXPECT_EQ(settings.audio_processing_properties().voice_isolation, AudioProcessingProperties::VoiceIsolationType::kVoiceIsolationDefault);
}
...
TEST_P(MediaStreamConstraintsUtilAudioTest, AdvancedCompatibleConstraints)
{
    ...
    CheckDeviceDefaults(result);
    CheckProcessingType(result);
    CheckBoolDefaults({ &AudioCaptureSettings::render_to_associated_sink }, { &AudioProcessingProperties::goog_audio_mirroring }, result);
    CheckEchoCancellationTypeDefault(result);
...
// applied.
TEST_P(MediaStreamConstraintsUtilAudioTest, AdvancedConflictingMiddleConstraints)
{
    ...
    CheckProcessingType(result);
    CheckDeviceDefaults(result);
    CheckBoolDefaults({}, { &AudioProcessingProperties::goog_audio_mirroring, &AudioProcessingProperties::goog_highpass_filter }, result);
    CheckEchoCancellationTypeDefault(result);
    EXPECT_TRUE(result.audio_processing_properties().goog_audio_mirroring);
...
TEST_P(MediaStreamConstraintsUtilAudioTest, NoDevicesNoConstraints)
{
    ...
    auto result = SelectSettingsAudioCapture(capabilities, constraint_factory_.CreateMediaConstraints(), GetMediaStreamType(), false);
    EXPECT_FALSE(result.HasValue());
...
TEST_P(MediaStreamConstraintsUtilAudioTest, SourceWithNoAudioProcessing)
{
    for (bool enable_properties : { true, false }) {
        std::unique_ptr<blink::LocalMediaStreamAudioSource> source = GetLocalMediaStreamAudioSource(enable_properties /* enable_system_echo_canceller */,
            enable_properties /* disable_local_echo */, enable_properties /* render_to_associated_sink */);

        // These constraints are false in |source|.
        const WTF::Vector<blink::BooleanConstraint MediaTrackConstraintSetPlatform::*> kConstraints = {
        ...
        for (WTF::wtf_size_t i = 0; i < kConstraints.size(); ++i) {
            constraint_factory_.Reset();
            (constraint_factory_.basic().*kConstraints[i]).SetExact(enable_properties);
            auto result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
            EXPECT_TRUE(result.HasValue());
...
TEST_P(MediaStreamConstraintsUtilAudioTest, SourceWithAudioProcessing)
{
    ...
    for (bool use_defaults : { true, false }) {
        AudioProcessingProperties properties;
        if (!use_defaults) {
            properties.echo_cancellation_type = EchoCancellationType::kEchoCancellationDisabled;
        ...
        std::unique_ptr<ProcessedLocalAudioSource> source
            = GetProcessedLocalAudioSource(properties, use_defaults /* disable_local_echo */, use_defaults /* render_to_associated_sink */);
        const WTF::Vector<blink::BooleanConstraint MediaTrackConstraintSetPlatform::*> kAudioProcessingConstraints = {
            &MediaTrackConstraintSetPlatform::goog_audio_mirroring,
            &MediaTrackConstraintSetPlatform::goog_auto_gain_control,
            &MediaTrackConstraintSetPlatform::goog_noise_suppression,
        ...
        ASSERT_EQ(kAudioProcessingConstraints.size(), GetAudioProcessingProperties().size());

        for (WTF::wtf_size_t i = 0; i < kAudioProcessingConstraints.size(); ++i) {
            constraint_factory_.Reset();
            (constraint_factory_.basic().*kAudioProcessingConstraints[i]).SetExact(properties.*GetAudioProcessingProperties()[i]);
            auto result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
            EXPECT_TRUE(result.HasValue());
            ...
            // Setting just ideal values should always succeed.
            constraint_factory_.Reset();
            (constraint_factory_.basic().*kAudioProcessingConstraints[i]).SetIdeal(true);
            result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
            EXPECT_TRUE(result.HasValue());

            constraint_factory_.Reset();
            (constraint_factory_.basic().*kAudioProcessingConstraints[i]).SetIdeal(false);
            result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
            EXPECT_TRUE(result.HasValue());
        }
        ...
        constraint_factory_.Reset();
        constraint_factory_.basic().echo_cancellation.SetExact(properties.echo_cancellation_type == EchoCancellationType::kEchoCancellationAec3);
        auto result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
        EXPECT_TRUE(result.HasValue());
        ...
        // These constraints are false in |source|.
        const WTF::Vector<blink::BooleanConstraint MediaTrackConstraintSetPlatform::*> kAudioBrowserConstraints = {
            &MediaTrackConstraintSetPlatform::disable_local_echo,
            &MediaTrackConstraintSetPlatform::render_to_associated_sink,
        };
        for (WTF::wtf_size_t i = 0; i < kAudioBrowserConstraints.size(); ++i) {
            constraint_factory_.Reset();
            (constraint_factory_.basic().*kAudioBrowserConstraints[i]).SetExact(use_defaults);
            result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
            EXPECT_TRUE(result.HasValue());
            ...
            constraint_factory_.Reset();
            (constraint_factory_.basic().*kAudioBrowserConstraints[i]).SetIdeal(false);
            result = SelectSettingsAudioCapture(source.get(), constraint_factory_.CreateMediaConstraints());
            EXPECT_TRUE(result.HasValue());
        }
...
TEST_P(MediaStreamConstraintsUtilAudioTest, UsedAndUnusedSources)
{
    ...
    std::unique_ptr<ProcessedLocalAudioSource> processed_source
        = GetProcessedLocalAudioSource(properties, false /* disable_local_echo */, false /* render_to_associated_sink */);

    const String kUnusedDeviceID = "unused_device";
    const String kGroupID = "fake_group";
    AudioDeviceCaptureCapabilities capabilities;
    capabilities.emplace_back(processed_source.get());
    capabilities.emplace_back(kUnusedDeviceID, kGroupID, media::AudioParameters::UnavailableDeviceParams());

    {
        ...
        auto result = SelectSettingsAudioCapture(
            capabilities, constraint_factory_.CreateMediaConstraints(), GetMediaStreamType(), false /* should_disable_hardware_noise_suppression */);
        EXPECT_TRUE(result.HasValue());
        EXPECT_EQ(result.device_id(), kUnusedDeviceID.Utf8());
        EXPECT_EQ(result.audio_processing_properties().echo_cancellation_type, EchoCancellationType::kEchoCancellationDisabled);
    }
...
TEST_P(MediaStreamConstraintsUtilAudioTest, SelectEligibleSettingsAudioDeviceCapture_NoEligibleDevices)
{
    if (!IsDeviceCapture()) {
...
TEST_P(MediaStreamConstraintsUtilAudioTest, SelectEligibleSettingsAudioDeviceCapture_IncludesEligibleDevices)
{
    if (!IsDeviceCapture()) {
        // This test is irrelevant for non-device captures.
        return;
...
TEST_P(MediaStreamConstraintsRemoteAPMTest, WebRtcSampleRateButNotDeviceSampleRate)
{
    SCOPED_TRACE(GetMessageForScopedTrace());

    AudioCaptureSettings result;
    ResetFactory();
    constraint_factory_.basic().sample_rate.SetExact(media::WebRtcAudioProcessingSampleRateHz());
    constraint_factory_.basic().echo_cancellation.SetExact(true);
    result = SelectSettings();
...
TEST_P(MediaStreamConstraintsUtilAudioTest, LatencyConstraint)
{
    ...
    double default_device_min = 10 / static_cast<double>(base::Time::kMillisecondsPerSecond);
    double default_device_max = 1000 / static_cast<double>(media::AudioParameters::kAudioCDSampleRate);

    CheckLatencyConstraint(default_device_, default_device_min, default_device_max);
    CheckLocalMediaStreamAudioSourceLatency(default_device_, 0.003, default_device_min * static_cast<double>(media::AudioParameters::kAudioCDSampleRate));
    CheckLocalMediaStreamAudioSourceLatency(default_device_, 0.015, default_device_min * static_cast<double>(media::AudioParameters::kAudioCDSampleRate));
    ...
    double variable_latency_device_min = 128 / static_cast<double>(media::AudioParameters::kAudioCDSampleRate);
    double variable_latency_device_max = 4096 / static_cast<double>(media::AudioParameters::kAudioCDSampleRate);

    CheckLatencyConstraint(variable_latency_device_, variable_latency_device_min, variable_latency_device_max);
    ...
    CheckLocalMediaStreamAudioSourceLatency(variable_latency_device_, 0.020, 1024);
#else
...
INSTANTIATE_TEST_SUITE_P(All, MediaStreamConstraintsUtilAudioTest,
    testing::Values("", blink::kMediaStreamSourceTab, blink::kMediaStreamSourceSystem, blink::kMediaStreamSourceDesktop));
#if BUILDFLAG(CHROME_WIDE_ECHO_CANCELLATION)
INSTANTIATE_TEST_SUITE_P(All, MediaStreamConstraintsRemoteAPMTest,
    testing::Combine(testing::Values("", blink::kMediaStreamSourceTab, blink::kMediaStreamSourceSystem, blink::kMediaStreamSourceDesktop),
        testing::Values(ChromeWideAecExperiment::kDisabled, ChromeWideAecExperiment::kEnabled)));
#else
INSTANTIATE_TEST_SUITE_P(All, MediaStreamConstraintsRemoteAPMTest,
    testing::Combine(testing::Values("", blink::kMediaStreamSourceTab, blink::kMediaStreamSourceSystem, blink::kMediaStreamSourceDesktop),
