...
LocalVideoCapturerSource::LocalVideoCapturerSource(
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, LocalFrame* frame, const base::UnguessableToken& session_id)
    : session_id_(session_id)
    , manager_(Platform::Current()->GetVideoCaptureImplManager())
    , frame_token_(frame->GetLocalFrameToken())
    , release_device_cb_(manager_->UseDevice(session_id_, frame->GetBrowserInterfaceBroker()))
...
void LocalVideoCapturerSource::StartCapture(const media::VideoCaptureParams& params, const VideoCaptureDeliverFrameCB& new_frame_callback,
    const VideoCaptureSubCaptureTargetVersionCB& sub_capture_target_version_callback, const VideoCaptureNotifyFrameDroppedCB& frame_dropped_callback,
    const RunningCallback& running_callback)
{
    ...
    stop_capture_cb_ = manager_->StartCapture(session_id_, params,
        base::BindPostTask(
            task_runner_, ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&LocalVideoCapturerSource::OnStateUpdate, weak_factory_.GetWeakPtr()))),
        new_frame_callback, sub_capture_target_version_callback, frame_dropped_callback);
}

media::VideoCaptureFeedbackCB LocalVideoCapturerSource::GetFeedbackCallback() const
{
...
void LocalVideoCapturerSource::OnStateUpdate(blink::VideoCaptureState state)
{
    ...
    case VIDEO_CAPTURE_STATE_STARTED:
        OnLog("LocalVideoCapturerSource::OnStateUpdate signaling to "
    ...
    case VIDEO_CAPTURE_STATE_ERROR_START_TIMEOUT:
        std::move(release_device_cb_).Run();
        release_device_cb_ = frame && frame->Client() ? manager_->UseDevice(session_id_, frame->GetBrowserInterfaceBroker()) : base::DoNothing();
        OnLog("LocalVideoCapturerSource::OnStateUpdate signaling to "
...
std::unique_ptr<VideoCapturerSource> LocalVideoCapturerSource::Create(
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, LocalFrame* frame, const base::UnguessableToken& session_id)
{
    return std::make_unique<LocalVideoCapturerSource>(std::move(task_runner), frame, session_id);
}
