...
template <typename T> struct CrossThreadCopier<std::optional<T>> : public CrossThreadCopierPassThrough<std::optional<T>> {
...
scoped_refptr<media::VideoFrame> CopyFrame(scoped_refptr<media::VideoFrame> frame, media::PaintCanvasVideoRenderer* video_renderer)
{
    scoped_refptr<media::VideoFrame> new_frame;
    if (frame->HasSharedImage()) {
        DCHECK(frame->format() == media::PIXEL_FORMAT_ARGB || frame->format() == media::PIXEL_FORMAT_XRGB || frame->format() == media::PIXEL_FORMAT_ABGR
            || frame->format() == media::PIXEL_FORMAT_XBGR || frame->format() == media::PIXEL_FORMAT_I420 || frame->format() == media::PIXEL_FORMAT_NV12);
        auto provider = Platform::Current()->SharedMainThreadContextProvider();
        if (!provider) {
            // Return a black frame (yuv = {0, 0x80, 0x80}).
            return media::VideoFrame::CreateColorFrame(frame->visible_rect().size(), 0u, 0x80, 0x80, frame->timestamp());
        }

        SkBitmap bitmap;
        bitmap.allocPixels(SkImageInfo::MakeN32Premul(frame->visible_rect().width(), frame->visible_rect().height()));
        ...
        bitmap.setImmutable();
        new_frame = media::CreateFromSkImage(bitmap.asImage(), frame->visible_rect(), frame->natural_size(), frame->timestamp());
        if (!new_frame) {
            return media::VideoFrame::CreateColorFrame(frame->visible_rect().size(), 0u, 0x80, 0x80, frame->timestamp());
        }
    } else {
        DCHECK(frame->IsMappable());
        if (frame->format() != media::PIXEL_FORMAT_I420A && frame->format() != media::PIXEL_FORMAT_I420 && frame->format() != media::PIXEL_FORMAT_NV12
        ...
        new_frame = media::VideoFrame::CreateFrame(frame->format(), coded_size, frame->visible_rect(), frame->natural_size(), frame->timestamp());
        if (!new_frame) {
            return media::VideoFrame::CreateColorFrame(frame->visible_rect().size(), 0u, 0x80, 0x80, frame->timestamp());
        }

        if (frame->format() == media::PIXEL_FORMAT_NV12) {
            libyuv::NV12Copy(frame->data(media::VideoFrame::Plane::kY), frame->stride(media::VideoFrame::Plane::kY), frame->data(media::VideoFrame::Plane::kUV),
                frame->stride(media::VideoFrame::Plane::kUV), new_frame->writable_data(media::VideoFrame::Plane::kY),
                new_frame->stride(media::VideoFrame::Plane::kY), new_frame->writable_data(media::VideoFrame::Plane::kUV),
                new_frame->stride(media::VideoFrame::Plane::kUV), coded_size.width(), coded_size.height());
        } else if (frame->format() == media::PIXEL_FORMAT_ARGB) {
            libyuv::ARGBCopy(frame->data(media::VideoFrame::Plane::kARGB), frame->stride(media::VideoFrame::Plane::kARGB),
                new_frame->writable_data(media::VideoFrame::Plane::kARGB), new_frame->stride(media::VideoFrame::Plane::kARGB), coded_size.width(),
                coded_size.height());
        } else {
            libyuv::I420Copy(frame->data(media::VideoFrame::Plane::kY), frame->stride(media::VideoFrame::Plane::kY), frame->data(media::VideoFrame::Plane::kU),
                frame->stride(media::VideoFrame::Plane::kU), frame->data(media::VideoFrame::Plane::kV), frame->stride(media::VideoFrame::Plane::kV),
                new_frame->writable_data(media::VideoFrame::Plane::kY), new_frame->stride(media::VideoFrame::Plane::kY),
                new_frame->writable_data(media::VideoFrame::Plane::kU), new_frame->stride(media::VideoFrame::Plane::kU),
                new_frame->writable_data(media::VideoFrame::Plane::kV), new_frame->stride(media::VideoFrame::Plane::kV), coded_size.width(),
                coded_size.height());
        }
        if (frame->format() == media::PIXEL_FORMAT_I420A) {
            libyuv::CopyPlane(frame->data(media::VideoFrame::Plane::kA), frame->stride(media::VideoFrame::Plane::kA),
                new_frame->writable_data(media::VideoFrame::Plane::kA), new_frame->stride(media::VideoFrame::Plane::kA), coded_size.width(),
                coded_size.height());
        }
    }
...
gfx::Size RotationAdjustedSize(media::VideoRotation rotation, const gfx::Size& size)
{
...
constexpr base::TimeDelta kMaximumVsyncDelayForLowLatencyRenderer = base::Milliseconds(50);

} // anonymous namespace

WebMediaPlayerMSCompositor::WebMediaPlayerMSCompositor(scoped_refptr<base::SingleThreadTaskRunner> video_frame_compositor_task_runner,
    scoped_refptr<base::SequencedTaskRunner> video_task_runner, MediaStreamDescriptor* media_stream_descriptor,
    std::unique_ptr<WebVideoFrameSubmitter> submitter, bool use_surface_layer, const base::WeakPtr<WebMediaPlayerMS>& player)
    ...
    , render_started_(!stopped_)
{
    ...
    if (use_surface_layer) {
        ...
        PostCrossThreadTask(*video_frame_compositor_task_runner_, FROM_HERE, CrossThreadBindOnce(&WebMediaPlayerMSCompositor::InitializeSubmitter, weak_this_));
        update_submission_state_callback_ = base::BindPostTask(video_frame_compositor_task_runner_,
            ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&WebMediaPlayerMSCompositor::SetIsSurfaceVisible, weak_this_)));
    }
    ...
    const bool remote_video = video_components.size() && video_components[0]->Remote();
    ...
    if (remote_video && Platform::Current()->RTCSmoothnessAlgorithmEnabled()) {
        ...
        rendering_frame_buffer_ = std::make_unique<VideoRendererAlgorithmWrapper>(
            ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&WebMediaPlayerMSCompositor::MapTimestampsToRenderTimeTicks, CrossThreadUnretained(this))),
            &media_log_);
    }

    // Just for logging purpose.
    std::string stream_id = media_stream_descriptor ? media_stream_descriptor->Id().Utf8() : std::string();
...
void WebMediaPlayerMSCompositor::SetIsSurfaceVisible(bool state, base::WaitableEvent* done_event)
{
    DCHECK(video_frame_compositor_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMSCompositor::EnableSubmission(const viz::SurfaceId& id, media::VideoTransformation transformation, bool force_submit)
{
    DCHECK(video_frame_compositor_task_runner_->BelongsToCurrentThread());

    // If we're switching to |submitter_| from some other client, then tell it.
    if (video_frame_provider_client_ && video_frame_provider_client_ != submitter_.get()) {
        video_frame_provider_client_->StopUsingProvider();
    }
...
void WebMediaPlayerMSCompositor::SetForceBeginFrames(bool enable)
{
    ...
    if (!video_frame_compositor_task_runner_->BelongsToCurrentThread()) {
        PostCrossThreadTask(
            *video_frame_compositor_task_runner_, FROM_HERE, CrossThreadBindOnce(&WebMediaPlayerMSCompositor::SetForceBeginFrames, weak_this_, enable));
        return;
    }
...
void WebMediaPlayerMSCompositor::SetVideoFrameProviderClient(cc::VideoFrameProvider::Client* client)
{
    DCHECK(video_frame_compositor_task_runner_->BelongsToCurrentThread());
    if (video_frame_provider_client_)
        video_frame_provider_client_->StopUsingProvider();
...
void WebMediaPlayerMSCompositor::RecordFrameDecodedStats(
    std::optional<base::TimeTicks> frame_received_time, std::optional<base::TimeDelta> frame_processing_time, std::optional<uint32_t> frame_rtp_timestamp)
{
    DCHECK(video_task_runner_->RunsTasksInCurrentSequence());
    if (frame_received_time && last_enqueued_frame_receive_time_) {
        base::TimeDelta frame_receive_time_delta = *frame_received_time - *last_enqueued_frame_receive_time_;
        LOCAL_HISTOGRAM_TIMES(UmaPrefix() + ".FrameReceivedTimeDelta", frame_receive_time_delta);
    }
    last_enqueued_frame_receive_time_ = frame_received_time;

    auto now = base::TimeTicks::Now();
    if (last_enqueued_frame_decoded_time_) {
        base::TimeDelta frame_decoded_time_delta = now - *last_enqueued_frame_decoded_time_;
        LOCAL_HISTOGRAM_TIMES(UmaPrefix() + ".FrameDecodedTimeDelta", frame_decoded_time_delta);
    }
    last_enqueued_frame_decoded_time_ = now;

    if (frame_processing_time) {
        LOCAL_HISTOGRAM_TIMES(UmaPrefix() + ".DecodeDuration", frame_processing_time.value());
    }

    if (frame_rtp_timestamp && last_enqueued_frame_rtp_timestamp_) {
        LOCAL_HISTOGRAM_COUNTS_10000(UmaPrefix() + ".FrameDecodedRtpTimestampDelta", *frame_rtp_timestamp - *last_enqueued_frame_rtp_timestamp_);
    }
...
void WebMediaPlayerMSCompositor::SetMetadata()
{
    ...
    current_metadata_.video_transform = current_frame_->metadata().transformation.value_or(media::kNoTransformation);
}

void WebMediaPlayerMSCompositor::EnqueueFrame(scoped_refptr<media::VideoFrame> frame, bool is_copy)
{
    DCHECK(video_task_runner_->RunsTasksInCurrentSequence());
    base::AutoLock auto_lock(current_frame_lock_);
    TRACE_EVENT_INSTANT1("media", "WebMediaPlayerMSCompositor::EnqueueFrame", TRACE_EVENT_SCOPE_THREAD, "Timestamp", frame->timestamp().InMicroseconds());
    ++total_frame_count_;
    ++frame_enqueued_since_last_vsync_;
    std::optional<uint32_t> enqueue_frame_rtp_timestamp;
    if (frame->metadata().rtp_timestamp) {
        enqueue_frame_rtp_timestamp = static_cast<uint32_t>(frame->metadata().rtp_timestamp.value());
    }
    RecordFrameDecodedStats(frame->metadata().receive_time, frame->metadata().processing_time, enqueue_frame_rtp_timestamp);
    ...
    // more information.
    if (!frame->metadata().reference_time.has_value() && !frame->metadata().maximum_composition_delay_in_frames) {
    ...
    base::TimeTicks render_time = frame->metadata().reference_time.value_or(base::TimeTicks());
    ...
    const bool is_out_of_order = pending_frames_info_.empty() ? false : frame->timestamp() < pending_frames_info_.back().timestamp;
    ...
    auto max_frame_delay = frame->metadata().maximum_composition_delay_in_frames ? kMaximumVsyncDelayForLowLatencyRenderer : base::TimeDelta();
    const bool is_update_overdue = deadline_offset > max_frame_delay;

    if (is_out_of_order || is_update_overdue) {
        base::UmaHistogramEnumeration(
            UmaPrefix() + ".ReasonToClearRenderBuffer", is_out_of_order ? ReasonToClearRenderBuffer::kOutOfOrder : ReasonToClearRenderBuffer::kUpdateOverdue);
    ...
    pending_frames_info_.emplace_back(frame->unique_id(), frame->timestamp(), render_time, is_copy);
    ...
    if (pending_frames_info_.size() != rendering_frame_buffer_->frames_queued()) {
        ...
        DCHECK_EQ(pending_frames_info_.size(), rendering_frame_buffer_->frames_queued());
    }
}

bool WebMediaPlayerMSCompositor::UpdateCurrentFrame(base::TimeTicks deadline_min, base::TimeTicks deadline_max)
{
    DCHECK(video_frame_compositor_task_runner_->BelongsToCurrentThread());

    TRACE_EVENT_BEGIN2(
        "media", "UpdateCurrentFrame", "Actual Render Begin", deadline_min.ToInternalValue(), "Actual Render End", deadline_max.ToInternalValue());
    if (stopped_)
        return false;
...

    {
        ...
        if (tracing_or_dcheck_enabled && current_frame_) {
            base::TimeTicks render_time = current_frame_->metadata().reference_time.value_or(base::TimeTicks());
            DCHECK(current_frame_->metadata().reference_time.has_value() || !rendering_frame_buffer_
                || (rendering_frame_buffer_ && !rendering_frame_buffer_->NeedsReferenceTime()))
                << "VideoFrames need REFERENCE_TIME to use "
                   "sophisticated video rendering algorithm.";
            TRACE_EVENT_END2("media", "UpdateCurrentFrame", "Ideal Render Instant", render_time.ToInternalValue(), "Serial", serial_);
        }
    }
...
scoped_refptr<media::VideoFrame> WebMediaPlayerMSCompositor::GetCurrentFrame()
{
    ...
    TRACE_EVENT_INSTANT1(
        "media", "WebMediaPlayerMSCompositor::GetCurrentFrame", TRACE_EVENT_SCOPE_THREAD, "Timestamp", current_frame_->timestamp().InMicroseconds());
    if (!render_started_)
        return nullptr;
...
void WebMediaPlayerMSCompositor::RecordFrameDisplayedStats(base::TimeTicks frame_displayed_time)
{
    DCHECK(video_frame_compositor_task_runner_->BelongsToCurrentThread());
    if (last_presented_frame_display_time_) {
        base::TimeDelta presentation_timestamp_delta = frame_displayed_time - *last_presented_frame_display_time_;
        LOCAL_HISTOGRAM_TIMES(UmaPrefix() + ".FramePresentationDelta", presentation_timestamp_delta);
    }
    ...
    if (current_frame_rtp_timestamp_ && last_presented_frame_rtp_timestamp_) {
        int32_t rtp_timestamp_delta = *current_frame_rtp_timestamp_ - *last_presented_frame_rtp_timestamp_;
        LOCAL_HISTOGRAM_COUNTS_10000(UmaPrefix() + ".MediaTimelineDelta", rtp_timestamp_delta);
    }
    last_presented_frame_rtp_timestamp_ = current_frame_rtp_timestamp_;

    if (current_frame_receive_time_) {
        base::TimeDelta frame_receive_to_display = frame_displayed_time - *current_frame_receive_time_;
        LOCAL_HISTOGRAM_TIMES(UmaPrefix() + ".FrameReceivedToDisplayedTime", frame_receive_to_display);
    }
}
...
base::TimeDelta WebMediaPlayerMSCompositor::GetPreferredRenderInterval()
{
    ...
    DCHECK_GE(rendering_frame_buffer_->average_frame_duration(), base::TimeDelta());
...
void WebMediaPlayerMSCompositor::OnContextLost()
{
    ...
    base::AutoLock auto_lock(current_frame_lock_);
    if (!current_frame_ || (!current_frame_->HasSharedImage() && !current_frame_->HasMappableGpuBuffer())) {
        return;
    }
    scoped_refptr<media::VideoFrame> black_frame = media::VideoFrame::CreateBlackFrame(current_frame_->natural_size());
...
void WebMediaPlayerMSCompositor::StartRendering()
{
    ...
    PostCrossThreadTask(*video_frame_compositor_task_runner_, FROM_HERE, CrossThreadBindOnce(&WebMediaPlayerMSCompositor::StartRenderingInternal, weak_this_));
}

void WebMediaPlayerMSCompositor::StopRendering()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    PostCrossThreadTask(*video_frame_compositor_task_runner_, FROM_HERE, CrossThreadBindOnce(&WebMediaPlayerMSCompositor::StopRenderingInternal, weak_this_));
}

bool WebMediaPlayerMSCompositor::MapTimestampsToRenderTimeTicks(const std::vector<base::TimeDelta>& timestamps, std::vector<base::TimeTicks>* wall_clock_times)
{
#if DCHECK_IS_ON()
    DCHECK(video_frame_compositor_task_runner_->BelongsToCurrentThread() || thread_checker_.CalledOnValidThread()
    ...
    for (const base::TimeDelta& timestamp : timestamps) {
        ...
        if (min_delta.is_positive()) {
            reference_time = reference_time + (min_delta / (timestamp + min_delta)) * (reference_time - base::TimeTicks());
        }
...
void WebMediaPlayerMSCompositor::RenderUsingAlgorithm(base::TimeTicks deadline_min, base::TimeTicks deadline_max)
{
    DCHECK(video_frame_compositor_task_runner_->BelongsToCurrentThread());
    current_frame_lock_.AssertAcquired();

    size_t frames_dropped = 0;
    scoped_refptr<media::VideoFrame> frame = rendering_frame_buffer_->Render(deadline_min, deadline_max, &frames_dropped);
    ...
    while (pending_frames_info_.size() != rendering_frame_buffer_->frames_queued()) {
...
void WebMediaPlayerMSCompositor::RenderWithoutAlgorithm(scoped_refptr<media::VideoFrame> frame, bool is_copy)
{
    DCHECK(video_task_runner_->RunsTasksInCurrentSequence());
    PostCrossThreadTask(*video_frame_compositor_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&WebMediaPlayerMSCompositor::RenderWithoutAlgorithmOnCompositor, weak_this_, std::move(frame), is_copy));
}

void WebMediaPlayerMSCompositor::RenderWithoutAlgorithmOnCompositor(scoped_refptr<media::VideoFrame> frame, bool is_copy)
{
    DCHECK(video_frame_compositor_task_runner_->BelongsToCurrentThread());
    {
        base::AutoLock auto_lock(current_frame_lock_);
        // Last timestamp in the stream might not have timestamp.
        if (current_frame_ && !frame->timestamp().is_zero() && frame->timestamp() > current_frame_->timestamp()) {
        ...
        TRACE_EVENT_INSTANT2("media", "RenderWithoutAlgorithm Difference From Deadline", TRACE_EVENT_SCOPE_THREAD, "diff_from_deadline_min",
            diff_from_deadline_min, "diff_from_deadline_max", diff_from_deadline_max);
...
void WebMediaPlayerMSCompositor::SetCurrentFrame(scoped_refptr<media::VideoFrame> frame, bool is_copy, std::optional<base::TimeTicks> expected_display_time)
{
    DCHECK(video_frame_compositor_task_runner_->BelongsToCurrentThread());
    current_frame_lock_.AssertAcquired();
    TRACE_EVENT_INSTANT1("media", "WebMediaPlayerMSCompositor::SetCurrentFrame", TRACE_EVENT_SCOPE_THREAD, "Timestamp", frame->timestamp().InMicroseconds());
    ...
    std::optional<media::VideoTransformation> new_transform = media::kNoTransformation;
    ...
    if (current_frame_) {
        ...
        auto current_video_transform = current_frame_->metadata().transformation.value_or(media::kNoTransformation);
        has_frame_size_changed = RotationAdjustedSize(new_transform->rotation, frame->natural_size())
            != RotationAdjustedSize(current_video_transform.rotation, current_frame_->natural_size());
    ...
    current_frame_rtp_timestamp_ = static_cast<uint32_t>(current_frame_->metadata().rtp_timestamp.value_or(0));
    LOCAL_HISTOGRAM_COUNTS_100(UmaPrefix() + ".DecoderThroughput", frame_enqueued_since_last_vsync_);
    ...
    last_expected_display_time_ = (expected_display_time.has_value() && !expected_display_time->is_null()) ? *expected_display_time : now;
    last_preferred_render_interval_ = GetPreferredRenderInterval();
    ++presented_frames_;

    TRACE_EVENT_INSTANT2("media", "SetCurrentFrame Timestamps", TRACE_EVENT_SCOPE_THREAD, "presentation_time", (last_presentation_time_),
        "last_expected_display_time", (last_expected_display_time_));
    ...
    PostCrossThreadTask(*video_frame_compositor_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&WebMediaPlayerMSCompositor::CheckForFrameChanges, weak_this_, is_first_frame, has_frame_size_changed, std::move(new_transform),
            std::move(new_opacity)));
}

void WebMediaPlayerMSCompositor::CheckForFrameChanges(
    bool is_first_frame, bool has_frame_size_changed, std::optional<media::VideoTransformation> new_frame_transform, std::optional<bool> new_frame_opacity)
{
    DCHECK(video_frame_compositor_task_runner_->BelongsToCurrentThread());

    if (is_first_frame) {
        PostCrossThreadTask(
            *main_task_runner_, FROM_HERE, CrossThreadBindOnce(&WebMediaPlayerMS::OnFirstFrameReceived, player_, *new_frame_transform, *new_frame_opacity));
        return;
    }

    if (new_frame_transform.has_value()) {
        PostCrossThreadTask(*main_task_runner_, FROM_HERE, CrossThreadBindOnce(&WebMediaPlayerMS::OnTransformChanged, player_, *new_frame_transform));
        if (submitter_)
            submitter_->SetTransform(*new_frame_transform);
    }
    if (new_frame_opacity.has_value()) {
        PostCrossThreadTask(*main_task_runner_, FROM_HERE, CrossThreadBindOnce(&WebMediaPlayerMS::OnOpacityChanged, player_, *new_frame_opacity));
    }
    if (has_frame_size_changed) {
        PostCrossThreadTask(*main_task_runner_, FROM_HERE, CrossThreadBindOnce(&WebMediaPlayerMS::TriggerResize, player_));
        PostCrossThreadTask(*main_task_runner_, FROM_HERE, CrossThreadBindOnce(&WebMediaPlayerMS::ResetCanvasCache, player_));
    }
}
...
void WebMediaPlayerMSCompositor::ReplaceCurrentFrameWithACopy()
{
    ...
    scoped_refptr<media::VideoFrame> copied_frame = CopyFrame(current_frame_ref, player_->GetPaintCanvasVideoRenderer());
...
void WebMediaPlayerMSCompositor::SetAlgorithmEnabledForTesting(bool algorithm_enabled)
{
    ...
    if (!rendering_frame_buffer_) {
        rendering_frame_buffer_ = std::make_unique<VideoRendererAlgorithmWrapper>(
            WTF::BindRepeating(&WebMediaPlayerMSCompositor::MapTimestampsToRenderTimeTicks, WTF::Unretained(this)), &media_log_);
    }
}

void WebMediaPlayerMSCompositor::SetOnFramePresentedCallback(OnNewFramePresentedCB presented_cb)
{
    base::AutoLock lock(new_frame_presented_cb_lock_);
    new_frame_presented_cb_ = std::move(presented_cb);
}

std::unique_ptr<WebMediaPlayer::VideoFramePresentationMetadata> WebMediaPlayerMSCompositor::GetLastPresentedFrameMetadata()
{
    auto frame_metadata = std::make_unique<WebMediaPlayer::VideoFramePresentationMetadata>();
