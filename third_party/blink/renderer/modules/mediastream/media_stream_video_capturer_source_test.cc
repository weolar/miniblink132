...
public:
    FakeMediaStreamVideoSink(base::TimeTicks* capture_time, media::VideoFrameMetadata* metadata, base::OnceClosure got_frame_cb)
    ...
    {
        MediaStreamVideoSink::ConnectToTrack(track,
            ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&FakeMediaStreamVideoSink::OnVideoFrame, WTF::CrossThreadUnretained(this))),
            MediaStreamVideoSink::IsSecure::kYes, MediaStreamVideoSink::UsesAlpha::kDefault);
    }
    ...
    void OnVideoFrame(scoped_refptr<media::VideoFrame> frame, base::TimeTicks capture_time)
    {
        *capture_time_ = capture_time;
...
public:
        ...
        : source_stopped_(false)
    {
        ...
        auto video_capturer_source = std::make_unique<MediaStreamVideoCapturerSource>(scheduler::GetSingleThreadTaskRunnerForTesting(),
            /*LocalFrame =*/nullptr, WTF::BindOnce(&MediaStreamVideoCapturerSourceTest::OnSourceStopped, WTF::Unretained(this)), std::move(delegate));
        video_capturer_source_ = video_capturer_source.get();
        video_capturer_source_->SetMediaStreamDispatcherHostForTesting(mock_dispatcher_host_.CreatePendingRemoteAndBind());
        stream_source_ = MakeGarbageCollected<MediaStreamSource>(
            "dummy_source_id", MediaStreamSource::kTypeVideo, "dummy_source_name", false /* remote */, std::move(video_capturer_source));
        stream_source_id_ = stream_source_->Id();

        MediaStreamVideoCapturerSource::DeviceCapturerFactoryCallback callback
            = WTF::BindRepeating(&MediaStreamVideoCapturerSourceTest::RecreateVideoCapturerSource, WTF::Unretained(this));
        video_capturer_source_->SetDeviceCapturerFactoryCallbackForTesting(std::move(callback));
    }
    ...
    WebMediaStreamTrack StartSource(
        const VideoTrackAdapterSettings& adapter_settings, const std::optional<bool>& noise_reduction, bool is_screencast, double min_frame_rate)
    {
        bool enabled = true;
        // CreateVideoTrack will trigger StartDone.
        return MediaStreamVideoTrack::CreateVideoTrack(video_capturer_source_, adapter_settings, noise_reduction, is_screencast, min_frame_rate, nullptr, false,
            WTF::BindOnce(&MediaStreamVideoCapturerSourceTest::StartDone, base::Unretained(this)), enabled);
    }

    MockVideoCapturerSource& mock_delegate()
    {
        return *delegate_;
    }
    ...
    void OnStarted(bool result)
    {
        ...
        video_capturer_source_->OnRunStateChanged(delegate_->capture_params(), run_state);
    }

    void SetStopCaptureFlag()
    {
    ...
    std::unique_ptr<VideoCapturerSource> RecreateVideoCapturerSource(const base::UnguessableToken& session_id)
    {
        auto delegate = std::make_unique<MockVideoCapturerSource>();
        delegate_ = delegate.get();
        EXPECT_CALL(*delegate_, MockStartCapture(_, _, _)).WillOnce(Return(RunState::kRunning));
        return delegate;
    }

protected:
    void StartDone(WebPlatformMediaStreamSource* source, MediaStreamRequestResult result, const WebString& result_name)
    {
    ...
    raw_ptr<MediaStreamVideoCapturerSource, DanglingUntriaged> video_capturer_source_; // owned by |stream_source_|.
    raw_ptr<MockVideoCapturerSource, DanglingUntriaged> delegate_; // owned by |source_|.
...
TEST_F(MediaStreamVideoCapturerSourceTest, StartAndStop)
{
    ...
    WebMediaStreamTrack track = StartSource(VideoTrackAdapterSettings(), std::nullopt, false, 0.0);
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(MediaStreamSource::kReadyStateLive, stream_source_->GetReadyState());
    ...
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(MediaStreamSource::kReadyStateEnded, stream_source_->GetReadyState());
...
TEST_F(MediaStreamVideoCapturerSourceTest, CaptureTimeAndMetadataPlumbing)
{
    ...
    InSequence s;
    EXPECT_CALL(mock_delegate(), MockStartCapture(_, _, _))
        .WillOnce(testing::DoAll(testing::SaveArg<1>(&deliver_frame_cb), testing::SaveArg<2>(&running_cb), Return(RunState::kRunning)));
    ...
    FakeMediaStreamVideoSink fake_sink(&capture_time, &metadata, base::BindPostTaskToCurrentDefault(run_loop.QuitClosure()));
    ...
    PostCrossThreadTask(*Platform::Current()->GetIOTaskRunner(), FROM_HERE, CrossThreadBindOnce(deliver_frame_cb, frame, reference_capture_time));
    run_loop.Run();
...
{
    InSequence s;
    EXPECT_CALL(mock_delegate(), MockStartCapture(_, _, _)).WillOnce(Return(RunState::kRunning));
    WebMediaStreamTrack track = StartSource(VideoTrackAdapterSettings(), std::nullopt, false, 0.0);
    base::RunLoop().RunUntilIdle();
    ...
    // triggered.
    EXPECT_EQ(stream_source_->GetReadyState(), MediaStreamSource::kReadyStateLive);
    EXPECT_FALSE(source_stopped_);
    ...
    EXPECT_CALL(mock_delegate(), MockStartCapture(_, _, _)).WillOnce(Return(RunState::kRunning));
    EXPECT_FALSE(video_capturer_source_->IsRunning());
    video_capturer_source_->Restart(media::VideoCaptureFormat(),
...
TEST_F(MediaStreamVideoCapturerSourceTest, StartStopAndNotify)
{
    InSequence s;
    EXPECT_CALL(mock_delegate(), MockStartCapture(_, _, _)).WillOnce(Return(RunState::kRunning));
    WebMediaStreamTrack web_track = StartSource(VideoTrackAdapterSettings(), std::nullopt, false, 0.0);
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(MediaStreamSource::kReadyStateLive, stream_source_->GetReadyState());
    EXPECT_FALSE(source_stopped_);
    ...
    EXPECT_CALL(mock_delegate(), MockStopCapture()).WillOnce(InvokeWithoutArgs(this, &MediaStreamVideoCapturerSourceTest::SetStopCaptureFlag));
    EXPECT_CALL(*this, MockNotification());
    MediaStreamTrackPlatform* track = MediaStreamTrackPlatform::GetTrack(web_track);
    track->StopAndNotify(WTF::BindOnce(&MediaStreamVideoCapturerSourceTest::MockNotification, base::Unretained(this)));
    EXPECT_EQ(MediaStreamSource::kReadyStateEnded, stream_source_->GetReadyState());
    EXPECT_TRUE(source_stopped_);
...
TEST_F(MediaStreamVideoCapturerSourceTest, ChangeSource)
{
    ...
    EXPECT_CALL(mock_delegate(), MockStopCapture());
    MediaStreamDevice fake_video_device(mojom::MediaStreamType::GUM_DESKTOP_VIDEO_CAPTURE, "Fake_Video_Device", "Fake Video Device");
...
{
    InSequence s;
    EXPECT_CALL(mock_delegate(), MockStartCapture(_, _, _)).WillOnce(Return(RunState::kSystemPermissionsError));
    WebMediaStreamTrack track = StartSource(VideoTrackAdapterSettings(), std::nullopt, false, 0.0);
    base::RunLoop().RunUntilIdle();
    EXPECT_TRUE(source_stopped_);
...
{
    InSequence s;
    EXPECT_CALL(mock_delegate(), MockStartCapture(_, _, _)).WillOnce(Return(RunState::kCameraBusyError));
    WebMediaStreamTrack track = StartSource(VideoTrackAdapterSettings(), std::nullopt, false, 0.0);
    base::RunLoop().RunUntilIdle();
    EXPECT_TRUE(source_stopped_);
