...
class MODULES_EXPORT LocalMediaStreamAudioSource final : public MediaStreamAudioSource, public media::AudioCapturerSource::CaptureCallback {
public:
    ...
    LocalMediaStreamAudioSource(LocalFrame* consumer_frame, const MediaStreamDevice& device, const int* requested_buffer_size, bool disable_local_echo,
        bool enable_system_echo_cancellation, ConstraintsRepeatingCallback started_callback, scoped_refptr<base::SingleThreadTaskRunner> task_runner);

    LocalMediaStreamAudioSource(const LocalMediaStreamAudioSource&) = delete;
    LocalMediaStreamAudioSource& operator=(const LocalMediaStreamAudioSource&) = delete;
    ...
    std::optional<AudioProcessingProperties> GetAudioProcessingProperties() const final;
...
private:
    ...
    void Capture(
        const media::AudioBus* audio_bus, base::TimeTicks audio_capture_time, const media::AudioGlitchInfo& glitch_info, double volume, bool key_pressed) final;
    void OnCaptureError(media::AudioCapturerSource::ErrorCode code, const std::string& message) final;
