...
bool AreSurfaceCoordinatesValid(const CapturedMouseEventInit& initializer)
{
    ...
    return (initializer.surfaceX() >= 0 && initializer.surfaceY() >= 0) || (initializer.surfaceX() == -1 && initializer.surfaceY() == -1);
}

} // namespace

// static
CapturedMouseEvent* CapturedMouseEvent::Create(const AtomicString& type, const CapturedMouseEventInit* initializer, ExceptionState& exception_state)
{
    CHECK(initializer);
    if (!AreSurfaceCoordinatesValid(*initializer)) {
        exception_state.ThrowRangeError("surfaceX and surfaceY must both be non-negative, or both of them "
                                        "must be equal to -1.");
        return nullptr;
    }
...
CapturedMouseEvent::CapturedMouseEvent(const AtomicString& type, const CapturedMouseEventInit* initializer)
