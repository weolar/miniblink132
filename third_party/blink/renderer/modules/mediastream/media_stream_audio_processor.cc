...
public:
    PlayoutListener(scoped_refptr<WebRtcAudioDeviceImpl> playout_data_source, WebRtcPlayoutDataSource::Sink* sink)
    ...
    ~PlayoutListener()
    {
        playout_data_source_->RemovePlayoutSink(sink_);
    }
...
MediaStreamAudioProcessor::MediaStreamAudioProcessor(DeliverProcessedAudioCallback deliver_processed_audio_callback,
    const media::AudioProcessingSettings& settings, const media::AudioParameters& capture_data_source_params,
    scoped_refptr<WebRtcAudioDeviceImpl> playout_data_source)
    : audio_processor_(media::AudioProcessor::Create(std::move(deliver_processed_audio_callback),
        /*log_callback=*/
        WTF::BindRepeating(&WebRtcLogStringPiece), settings, capture_data_source_params,
        media::AudioProcessor::GetDefaultOutputFormat(capture_data_source_params, settings)))
    ...
    , stopped_(false)
{
    ...
    if (audio_processor_->needs_playout_reference() && playout_data_source) {
        playout_listener_ = std::make_unique<PlayoutListener>(std::move(playout_data_source), this);
    }
    ...
    const media::AudioBus& audio_source, base::TimeTicks audio_capture_time, int num_preferred_channels, double volume, bool key_pressed)
{
    ...
    audio_processor_->ProcessCapturedAudio(audio_source, audio_capture_time, num_preferred_channels, volume, key_pressed);
}
...
const media::AudioParameters& MediaStreamAudioProcessor::GetInputFormatForTesting() const
{
...
bool MediaStreamAudioProcessor::WouldModifyAudio(const AudioProcessingProperties& properties)
{
    if (properties
...
void MediaStreamAudioProcessor::OnPlayoutData(media::AudioBus* audio_bus, int sample_rate, base::TimeDelta audio_delay)
{
...
webrtc::AudioProcessorInterface::AudioProcessorStatistics MediaStreamAudioProcessor::GetStats(bool has_remote_tracks)
{
