...
const double kDefaultScreenCastAspectRatio = static_cast<double>(kDefaultScreenCastWidth) / kDefaultScreenCastHeight;
...
void CheckTrackAdapterSettingsEqualsFormat(const VideoCaptureSettings& result, std::optional<double> frame_rate = std::nullopt)
{
...
void CheckTrackAdapterSettingsEqualsFormatDefaultAspectRatio(const VideoCaptureSettings& result, std::optional<double> frame_rate = std::nullopt)
{
    EXPECT_EQ(static_cast<double>(kMinScreenCastDimension) / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kMinScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
...
protected:
    VideoCaptureSettings SelectSettings(mojom::MediaStreamType stream_type = mojom::MediaStreamType::GUM_DESKTOP_VIDEO_CAPTURE)
    {
        MediaConstraints constraints = constraint_factory_.CreateMediaConstraints();
        return SelectSettingsVideoContentCapture(constraints, stream_type, kDefaultScreenCastWidth, kDefaultScreenCastHeight);
    }
...
{
    constraint_factory_.Reset();
    constraint_factory_.basic().frame_rate.SetExact(kMaxScreenCastFrameRate + 0.1);
    auto result = SelectSettings();
    EXPECT_FALSE(result.HasValue());
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, OverconstrainedOnInvalidResizeMode)
{
    constraint_factory_.Reset();
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, OverconstrainedOnEmptyResizeMode)
{
    constraint_factory_.Reset();
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryExactHeight)
{
    ...
    EXPECT_EQ(std::round(kHeight * kDefaultScreenCastAspectRatio), result.Width());
    CheckNonResolutionDefaults(result);
    EXPECT_EQ(1.0 / kHeight, result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kHeight, result.track_adapter_settings().max_aspect_ratio());
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryMinHeight)
{
    ...
    EXPECT_EQ(std::round(kHeight * kDefaultScreenCastAspectRatio), result.Width());
    CheckNonResolutionDefaults(result);
    EXPECT_EQ(1.0 / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kHeight, result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result);
    ...
    CheckNonResolutionDefaults(result);
    EXPECT_EQ(1.0 / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kSmallHeight, result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result);
}
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryMaxHeight)
{
    // kMaxHeight smaller than the default.
    {
        ...
        EXPECT_EQ(std::round(kMaxHeight * kDefaultScreenCastAspectRatio), result.Width());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(1.0 / kMaxHeight, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(kMaxScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
    }
    ...
    // kMaxHeight greater than the maximum allowed.
    {
        ...
        EXPECT_TRUE(result.HasValue());
        EXPECT_EQ(kDefaultScreenCastHeight, result.Height());
        EXPECT_EQ(std::round(kDefaultScreenCastHeight * kDefaultScreenCastAspectRatio), result.Width());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(1.0 / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(kMaxScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryHeightRange)
{
    // Range includes the default.
    {
        ...
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(1.0 / kMaxHeight, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kMinHeight, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
    }
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, IdealHeight)
{
    // Unconstrained.
    {
        ...
        EXPECT_EQ(std::round(kIdealHeight * kDefaultScreenCastAspectRatio), result.Width());
        CheckNonResolutionDefaults(result);
        CheckTrackAdapterSettingsEqualsFormatDefaultAspectRatio(result);
    }
    ...
    // Ideal less than minimum.
    {
        ...
        EXPECT_EQ(std::round(kMinHeight * kDefaultScreenCastAspectRatio), result.Width());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(1.0 / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kMinHeight, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
    ...
    // Ideal intersects a box.
    {
        ...
        EXPECT_EQ(kIdealHeight, result.Height());
        double default_aspect_ratio = static_cast<double>(constraint_factory_.basic().width.Max()) / constraint_factory_.basic().height.Max();
    ...
    // Ideal outside the box, closest to the side coinciding with max height.
    {
        ...
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(100.0 / kMaxHeight, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(500.0 / 500.0, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
    }
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryExactWidth)
{
    ...
    EXPECT_EQ(std::round(kWidth / kDefaultScreenCastAspectRatio), result.Height());
    CheckNonResolutionDefaults(result);
    EXPECT_EQ(static_cast<double>(kWidth) / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(static_cast<double>(kWidth) / kMinScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result);
}
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryMinWidth)
{
    ...
    EXPECT_EQ(kDefaultScreenCastHeight, result.Height());
    CheckNonResolutionDefaults(result);
    EXPECT_EQ(static_cast<double>(kSmallWidth) / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kMinScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result);
}
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryMaxWidth)
{
    // kMaxWidth less than the default.
    {
        ...
        EXPECT_EQ(std::round(kMaxWidth / kDefaultScreenCastAspectRatio), result.Height());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(1.0 / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(static_cast<double>(kMaxWidth) / kMinScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
    }
    ...
    // kMaxWidth greater than the maximum allowed (gets ignored).
    {
        ...
        EXPECT_EQ(kDefaultScreenCastWidth, result.Width());
        EXPECT_EQ(std::round(kDefaultScreenCastWidth / kDefaultScreenCastAspectRatio), result.Height());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(1.0 / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kMinScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryWidthRange)
{
    // The whole range is less than the default.
    {
        ...
        EXPECT_EQ(std::round(kMaxWidth / kDefaultScreenCastAspectRatio), result.Height());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(static_cast<double>(kMinWidth) / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(static_cast<double>(kMaxWidth) / kMinScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
    }
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, IdealWidth)
{
    // Unconstrained
    {
        ...
        EXPECT_EQ(std::round(kIdealWidth / kDefaultScreenCastAspectRatio), result.Height());
        CheckNonResolutionDefaults(result);
        CheckTrackAdapterSettingsEqualsFormatDefaultAspectRatio(result);
    }
    ...
    // Ideal less than minimum.
    {
        ...
        EXPECT_EQ(std::round(kMinWidth / kDefaultScreenCastAspectRatio), result.Height());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(static_cast<double>(kMinWidth) / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kMinScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
    ...
    // Ideal outside the box, closest to the side coinciding with max width.
    {
        ...
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(500.0 / 500.0, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(static_cast<double>(kMaxWidth) / 100.0, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
    }
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryExactAspectRatio)
{
    ...
    EXPECT_EQ(kDefaultScreenCastHeight, result.Height());
    EXPECT_EQ(std::round(kDefaultScreenCastHeight * kAspectRatio), result.Width());
    CheckNonResolutionDefaults(result);
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryMinAspectRatio)
{
    ...
    CheckNonResolutionDefaults(result);
    EXPECT_EQ(kAspectRatio, result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / static_cast<double>(kMinScreenCastDimension), result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result);
    ...
    EXPECT_EQ(kDefaultScreenCastWidth, result.Width());
    CheckNonResolutionDefaults(result);
    EXPECT_EQ(kSmallAspectRatio, result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / static_cast<double>(kMinScreenCastDimension), result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result);
}
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryMaxAspectRatio)
{
    ...
    EXPECT_EQ(kDefaultScreenCastWidth, result.Width());
    CheckNonResolutionDefaults(result);
    EXPECT_EQ(static_cast<double>(kMinScreenCastDimension) / static_cast<double>(kMaxScreenCastDimension), result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(kAspectRatio, result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result);
    ...
    EXPECT_EQ(std::round(kDefaultScreenCastWidth / kSmallAspectRatio), result.Height());
    EXPECT_EQ(kDefaultScreenCastWidth, result.Width());
    CheckNonResolutionDefaults(result);
    EXPECT_EQ(static_cast<double>(kMinScreenCastDimension) / static_cast<double>(kMaxScreenCastDimension), result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(kSmallAspectRatio, result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result);
}
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryRangeAspectRatio)
{
...

    {
        ...
        EXPECT_EQ(kDefaultScreenCastHeight, result.Height());
        EXPECT_EQ(std::round(kDefaultScreenCastHeight * kMinAspectRatio), result.Width());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(kMinAspectRatio, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(kMaxAspectRatio, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
...

    {
        ...
        EXPECT_EQ(std::round(kDefaultScreenCastWidth / kMaxAspectRatio), result.Height());
        EXPECT_EQ(kDefaultScreenCastWidth, result.Width());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(kMinAspectRatio, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(kMaxAspectRatio, result.track_adapter_settings().max_aspect_ratio());
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, IdealAspectRatio)
{
    // Unconstrained.
    {
        ...
        EXPECT_TRUE(result.HasValue());
        EXPECT_EQ(kDefaultScreenCastHeight, result.Height());
        EXPECT_EQ(std::round(kDefaultScreenCastHeight * kIdealAspectRatio), result.Width());
        CheckNonResolutionDefaults(result);
        CheckTrackAdapterSettingsEqualsFormatDefaultAspectRatio(result);
    }
    ...
    // Ideal less than minimum.
    {
        ...
        EXPECT_EQ(std::round(kDefaultScreenCastWidth / kMinAspectRatio), result.Height());
        EXPECT_EQ(kDefaultScreenCastWidth, result.Width());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(kMinAspectRatio, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kMinScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
    ...
    // Ideal intersects a box.
    {
        ...
        // width.
        EXPECT_EQ(std::round(constraint_factory_.basic().width.Max() / kIdealAspectRatio), result.Height());
        EXPECT_EQ(constraint_factory_.basic().width.Max(), result.Width());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(100.0 / 500.0, result.track_adapter_settings().min_aspect_ratio());
        ...
        EXPECT_EQ(constraint_factory_.basic().width.Max(), result.Width());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(1000.0 / 5000.0, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(5000.0 / 1000.0, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
        ...
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(250.0 / kMaxScreenCastDimension, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(kMaxScreenCastDimension / 250.0, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
    }
    ...
    // Ideal outside the constrained area, closest to min or max aspect ratio.
    {
        ...
        EXPECT_EQ(std::round(constraint_factory_.basic().width.Max() / kMaxAspectRatio), result.Height());
        EXPECT_EQ(constraint_factory_.basic().width.Max(), result.Width());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(kMinAspectRatio, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(kMaxAspectRatio, result.track_adapter_settings().max_aspect_ratio());
        ...
        EXPECT_EQ(constraint_factory_.basic().height.Max(), result.Height());
        EXPECT_EQ(std::round(constraint_factory_.basic().height.Max() * kMinAspectRatio), result.Width());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(kMinAspectRatio, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(kMaxAspectRatio, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
        ...
        EXPECT_EQ(std::round(constraint_factory_.basic().height.Min() * kMaxAspectRatio), result.Width());
        CheckNonResolutionDefaults(result);
        EXPECT_EQ(kMinAspectRatio, result.track_adapter_settings().min_aspect_ratio());
        EXPECT_EQ(kMaxAspectRatio, result.track_adapter_settings().max_aspect_ratio());
        CheckTrackAdapterSettingsEqualsFormat(result);
        ...
        EXPECT_TRUE(result.HasValue());
        // Ideal is closest to kMinAspectRatio.
        EXPECT_EQ(std::round(constraint_factory_.basic().width.Min() / kMinAspectRatio), result.Height());
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, MandatoryMaxFrameRate)
{
    ...
    // kMaxFrameRate greater than default
    {
        ...
        CheckNonFrameRateDefaults(result);
        CheckTrackAdapterSettingsEqualsFormatDefaultAspectRatio(result, kMaxFrameRate);
    }
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, IdealFrameRate)
{
    // Unconstrained.
    {
        ...
        EXPECT_EQ(std::optional<double>(), result.max_frame_rate());
        CheckNonFrameRateDefaults(result);
        CheckTrackAdapterSettingsEqualsFormatDefaultAspectRatio(result, kIdealFrameRate);
    }
    ...
    // Ideal less than minimum.
    {
        ...
        EXPECT_EQ(std::optional<double>(), result.max_frame_rate());
        CheckNonFrameRateDefaults(result);
        CheckTrackAdapterSettingsEqualsFormatDefaultAspectRatio(result, kMinFrameRate);
    }
...
// sets.
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedMinMaxResolutionFrameRate)
{
    constraint_factory_.Reset();
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedResolutionAndFrameRate)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedNoiseReduction)
{
    ...
    EXPECT_EQ(static_cast<int>(std::round(result.Width() / kDefaultScreenCastAspectRatio)), result.Height());
    EXPECT_TRUE(result.noise_reduction() && !*result.noise_reduction());
    EXPECT_EQ(kMinWidth / static_cast<double>(kMaxScreenCastDimension), result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kMinHeight, result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result);
}
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedContradictoryNoiseReduction)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedContradictoryExactResolution)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
    advanced1.width.SetExact(640);
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedContradictoryMaxMinResolutionFrameRate)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
    ...
    EXPECT_EQ(kDefaultScreenCastFrameRate, result.FrameRate());
    EXPECT_EQ(kMinScreenCastDimension / 480.0, result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(640.0 / kMinScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result);
}

TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedContradictoryMinMaxResolutionFrameRate)
{
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedContradictoryExactAspectRatio)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedContradictoryAspectRatioRange)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
    const double kMinAspectRatio = 5.0;
    ...
    CheckNonResolutionDefaults(result);
    EXPECT_EQ(kMinAspectRatio, result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(kMaxScreenCastDimension / static_cast<double>(kMinScreenCastDimension), result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result);
}

TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedContradictoryExactFrameRate)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedContradictoryFrameRateRange)
{
    constraint_factory_.Reset();
    MediaTrackConstraintSetPlatform& advanced1 = constraint_factory_.AddAdvanced();
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedContradictoryWidthFrameRate)
{
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedContradictoryHeightFrameRate)
{
    ...
    EXPECT_EQ(kMaxHeight, result.Height());
    EXPECT_EQ(60.0, result.FrameRate());
    EXPECT_EQ(static_cast<double>(kMinScreenCastDimension) / kMaxHeight, result.track_adapter_settings().min_aspect_ratio());
    EXPECT_EQ(static_cast<double>(kMaxScreenCastDimension) / kMinScreenCastDimension, result.track_adapter_settings().max_aspect_ratio());
    CheckTrackAdapterSettingsEqualsFormat(result, 60.0);
}
...
TEST_F(MediaStreamConstraintsUtilVideoContentTest, AdvancedContradictoryDeviceID)
{
    const String kDeviceID1 = "fake_device_1";
    const String kDeviceID2 = "fake_device_2";
    const String kDeviceID3 = "fake_device_3";
...
{
    {
        ...
        EXPECT_EQ(media::ResolutionChangePolicy::ANY_WITHIN_LIMIT, result.ResolutionChangePolicy());
        CheckTrackAdapterSettingsEqualsFormatDefaultAspectRatio(result);
    }
    {
        constraint_factory_.Reset();
        ...
        EXPECT_EQ(media::ResolutionChangePolicy::FIXED_RESOLUTION, result.ResolutionChangePolicy());
        CheckTrackAdapterSettingsEqualsFormatDefaultAspectRatio(result);
    }
    {
        constraint_factory_.Reset();
    ...
    }
    {
        ...
        EXPECT_EQ(media::ResolutionChangePolicy::FIXED_ASPECT_RATIO, result.ResolutionChangePolicy());
        CheckTrackAdapterSettingsEqualsFormat(result);
    }
    {
        constraint_factory_.Reset();
