...
using AnalogGainController = webrtc::AudioProcessing::Config::GainController1::AnalogGainController;
...
void ReadDataFromSpeechFile(base::HeapArray<int16_t>& data)
{
    ...
    EXPECT_EQ(base::checked_cast<int>(bytes.size_bytes()), base::ReadFile(file, bytes));
...
public:
    MediaStreamAudioProcessorTest()
        : params_(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), 48000, 480)
    {
    }

protected:
    // Helper method to save duplicated code.
    static void ProcessDataAndVerifyFormat(MediaStreamAudioProcessor& audio_processor, MockProcessedCaptureCallback& mock_capture_callback,
        int expected_output_sample_rate, int expected_output_channels, int expected_output_buffer_size)
    {
        // Read the audio data from a file.
        const media::AudioParameters& params = audio_processor.GetInputFormatForTesting();
        const int frames_per_packet = params.frames_per_buffer() * params.channels();
        ...
        const int16_t* data_ptr = reinterpret_cast<const int16_t*>(capture_data.data());
        ...
        for (int i = 0; i < kNumberOfPacketsForTest; ++i) {
            ...
            if (is_aec_enabled) {
                audio_processor.OnPlayoutData(data_bus.get(), params.sample_rate(), base::Milliseconds(10));
            }
            ...
            EXPECT_EQ(expected_output_sample_rate, audio_processor.output_format().sample_rate());
            EXPECT_EQ(expected_output_channels, audio_processor.output_format().channels());
            EXPECT_EQ(expected_output_buffer_size, audio_processor.output_format().frames_per_buffer());
...
class MediaStreamAudioProcessorTestMultichannel : public MediaStreamAudioProcessorTest, public ::testing::WithParamInterface<bool> { };
...
TEST_P(MediaStreamAudioProcessorTestMultichannel, MAYBE_WithAudioProcessing)
{
    ...
    scoped_refptr<WebRtcAudioDeviceImpl> webrtc_audio_device(new rtc::RefCountedObject<WebRtcAudioDeviceImpl>());
    blink::AudioProcessingProperties properties;
    scoped_refptr<MediaStreamAudioProcessor> audio_processor(new rtc::RefCountedObject<MediaStreamAudioProcessor>(
        mock_capture_callback_.Get(), properties.ToAudioProcessingSettings(use_multichannel_processing), params_, webrtc_audio_device));
    EXPECT_TRUE(audio_processor->has_webrtc_audio_processing());
    VerifyDefaultComponents(*audio_processor);

    const int expected_output_channels = use_multichannel_processing ? params_.channels() : 1;
    ProcessDataAndVerifyFormat(*audio_processor, mock_capture_callback_, media::WebRtcAudioProcessingSampleRateHz(), expected_output_channels,
...
TEST_F(MediaStreamAudioProcessorTest, TurnOffDefaultConstraints)
{
    ...
    scoped_refptr<WebRtcAudioDeviceImpl> webrtc_audio_device(new rtc::RefCountedObject<WebRtcAudioDeviceImpl>());
    scoped_refptr<MediaStreamAudioProcessor> audio_processor(new rtc::RefCountedObject<MediaStreamAudioProcessor>(mock_capture_callback_.Get(),
    ...
    ProcessDataAndVerifyFormat(*audio_processor, mock_capture_callback_, params_.sample_rate(), params_.channels(), params_.sample_rate() / 100);
...
TEST_P(MediaStreamAudioProcessorTestMultichannel, MAYBE_TestAllSampleRates)
{
    ...
    for (int sample_rate : kSupportedSampleRates) {
        ...
        media::AudioParameters params(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), sample_rate, buffer_size);
        scoped_refptr<MediaStreamAudioProcessor> audio_processor(new rtc::RefCountedObject<MediaStreamAudioProcessor>(
            mock_capture_callback_.Get(), properties.ToAudioProcessingSettings(use_multichannel_processing), params, webrtc_audio_device));
        EXPECT_TRUE(audio_processor->has_webrtc_audio_processing());
        VerifyDefaultComponents(*audio_processor);
...
#endif // BUILDFLAG(IS_CASTOS) || BUILDFLAG(IS_CAST_ANDROID)
        const int expected_output_channels = use_multichannel_processing ? params_.channels() : 1;
        ProcessDataAndVerifyFormat(*audio_processor, mock_capture_callback_, expected_sample_rate, expected_output_channels, expected_sample_rate / 100);
...
TEST_F(MediaStreamAudioProcessorTest, StartStopAecDump)
{
    ...
    ASSERT_TRUE(base::CreateTemporaryFileInDir(temp_directory.GetPath(), &temp_file_path));
    media::AudioParameters params(
        media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::FromLayout<media::CHANNEL_LAYOUT_STEREO_AND_KEYBOARD_MIC>(), 48000, 480);
    {
        scoped_refptr<MediaStreamAudioProcessor> audio_processor(new rtc::RefCountedObject<MediaStreamAudioProcessor>(mock_capture_callback_.Get(),
            properties.ToAudioProcessingSettings(
                /*multi_channel_capture_processing=*/true),
...
{
    const bool use_multichannel_processing = GetParam();
    SCOPED_TRACE(testing::Message() << "use_multichannel_processing=" << use_multichannel_processing);
    scoped_refptr<WebRtcAudioDeviceImpl> webrtc_audio_device(new rtc::RefCountedObject<WebRtcAudioDeviceImpl>());
    const media::AudioParameters source_params(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), 48000, 480);
    ...
    for (bool use_apm : kUseApmValues) {
        ...
        scoped_refptr<MediaStreamAudioProcessor> audio_processor(new rtc::RefCountedObject<MediaStreamAudioProcessor>(
            mock_capture_callback_.Get(), properties.ToAudioProcessingSettings(use_multichannel_processing), source_params, webrtc_audio_device));
...
INSTANTIATE_TEST_SUITE_P(MediaStreamAudioProcessorMultichannelAffectedTests, MediaStreamAudioProcessorTestMultichannel, ::testing::Values(false, true));

// When audio processing is performed, processed audio should be delivered as
// soon as 10 ms of audio has been received.
TEST(MediaStreamAudioProcessorCallbackTest, ProcessedAudioIsDeliveredAsSoonAsPossibleWithShortBuffers)
{
    test::TaskEnvironment task_environment_;
    ...
    media::AudioParameters params(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), 48000, 48000 * 4 / 1000);
    scoped_refptr<MediaStreamAudioProcessor> audio_processor(new rtc::RefCountedObject<MediaStreamAudioProcessor>(mock_capture_callback.Get(),
        properties.ToAudioProcessingSettings(
            /*multi_channel_capture_processing=*/true),
        params, webrtc_audio_device));
    ASSERT_TRUE(audio_processor->has_webrtc_audio_processing());
...
TEST(MediaStreamAudioProcessorCallbackTest, ProcessedAudioIsDeliveredAsSoonAsPossibleWithLongBuffers)
{
    test::TaskEnvironment task_environment_;
    MockProcessedCaptureCallback mock_capture_callback;
    blink::AudioProcessingProperties properties;
    ...
    media::AudioParameters params(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), 48000, 48000 * 35 / 1000);
    scoped_refptr<MediaStreamAudioProcessor> audio_processor(new rtc::RefCountedObject<MediaStreamAudioProcessor>(mock_capture_callback.Get(),
        properties.ToAudioProcessingSettings(
            /*multi_channel_capture_processing=*/true),
        params, webrtc_audio_device));
...
TEST(MediaStreamAudioProcessorCallbackTest, UnprocessedAudioIsDeliveredImmediatelyWithShortBuffers)
{
    test::TaskEnvironment task_environment_;
    MockProcessedCaptureCallback mock_capture_callback;
    blink::AudioProcessingProperties properties;
...
TEST(MediaStreamAudioProcessorCallbackTest, UnprocessedAudioIsDeliveredImmediatelyWithLongBuffers)
{
    test::TaskEnvironment task_environment_;
    MockProcessedCaptureCallback mock_capture_callback;
    blink::AudioProcessingProperties properties;
...
namespace {
scoped_refptr<MediaStreamAudioProcessor> CreateAudioProcessorWithProperties(AudioProcessingProperties properties)
{
    MockProcessedCaptureCallback mock_capture_callback;
    scoped_refptr<WebRtcAudioDeviceImpl> webrtc_audio_device(new rtc::RefCountedObject<WebRtcAudioDeviceImpl>());
    media::AudioParameters params(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), 48000, 480);
    scoped_refptr<MediaStreamAudioProcessor> audio_processor(new rtc::RefCountedObject<MediaStreamAudioProcessor>(mock_capture_callback.Get(),
        properties.ToAudioProcessingSettings(
            /*multi_channel_capture_processing=*/true),
        params, webrtc_audio_device));
...
TEST(MediaStreamAudioProcessorWouldModifyAudioTest, FalseWhenEverythingIsDisabled)
{
    test::TaskEnvironment task_environment_;
    blink::AudioProcessingProperties properties;
    properties.DisableDefaultProperties();
...
TEST(MediaStreamAudioProcessorWouldModifyAudioTest, FalseWhenOnlyHardwareEffectsAreUsed)
{
    test::TaskEnvironment task_environment_;
    blink::AudioProcessingProperties properties;
    properties.DisableDefaultProperties();
    properties.echo_cancellation_type = AudioProcessingProperties::EchoCancellationType::kEchoCancellationSystem;
...
#define MAYBE_TrueWhenSoftwareEchoCancellationIsEnabled DISABLED_TrueWhenSoftwareEchoCancellationIsEnabled
#else
#define MAYBE_TrueWhenSoftwareEchoCancellationIsEnabled TrueWhenSoftwareEchoCancellationIsEnabled
#endif // BUILDFLAG(IS_IOS)
TEST(MediaStreamAudioProcessorWouldModifyAudioTest, MAYBE_TrueWhenSoftwareEchoCancellationIsEnabled)
{
    test::TaskEnvironment task_environment_;
    blink::AudioProcessingProperties properties;
    properties.DisableDefaultProperties();
    properties.echo_cancellation_type = AudioProcessingProperties::EchoCancellationType::kEchoCancellationAec3;
...
// TODO(https://crbug.com/1417474): Remove legacy iOS case in
// AudioProcessingSettings::NeedWebrtcAudioProcessing().
#define MAYBE_TrueWhenStereoMirroringIsEnabled DISABLED_TrueWhenStereoMirroringIsEnabled
#else
#define MAYBE_TrueWhenStereoMirroringIsEnabled TrueWhenStereoMirroringIsEnabled
#endif // BUILDFLAG(IS_IOS)
TEST(MediaStreamAudioProcessorWouldModifyAudioTest, MAYBE_TrueWhenStereoMirroringIsEnabled)
{
    test::TaskEnvironment task_environment_;
    blink::AudioProcessingProperties properties;
    properties.DisableDefaultProperties();
...
#endif // BUILDFLAG(IS_IOS)
TEST(MediaStreamAudioProcessorWouldModifyAudioTest, MAYBE_TrueWhenGainControlIsEnabled)
{
    test::TaskEnvironment task_environment_;
    blink::AudioProcessingProperties properties;
    properties.DisableDefaultProperties();
...
TEST(MediaStreamAudioProcessorWouldModifyAudioTest, TrueWhenNoiseSuppressionIsEnabled)
{
    test::TaskEnvironment task_environment_;
    blink::AudioProcessingProperties properties;
    properties.DisableDefaultProperties();
...
TEST(MediaStreamAudioProcessorWouldModifyAudioTest, TrueWhenExperimentalNoiseSuppression)
{
    test::TaskEnvironment task_environment_;
    blink::AudioProcessingProperties properties;
    properties.DisableDefaultProperties();
...
TEST(MediaStreamAudioProcessorWouldModifyAudioTest, TrueWhenHighpassFilterIsEnabled)
{
    test::TaskEnvironment task_environment_;
    blink::AudioProcessingProperties properties;
    properties.DisableDefaultProperties();
