...
class MODULES_EXPORT MediaStreamDeviceObserver : public mojom::blink::MediaStreamDeviceObserver {
public:
    explicit MediaStreamDeviceObserver(LocalFrame* frame);

    MediaStreamDeviceObserver(const MediaStreamDeviceObserver&) = delete;
    MediaStreamDeviceObserver& operator=(const MediaStreamDeviceObserver&) = delete;
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(MediaStreamDeviceObserverTest, GetNonScreenCaptureDevices);
    ...
    FRIEND_TEST_ALL_PREFIXES(MediaStreamDeviceObserverTest, OnDeviceChangedChangesDeviceAfterRebind);
    FRIEND_TEST_ALL_PREFIXES(MediaStreamDeviceObserverTest, OnDeviceRequestStateChange);
    FRIEND_TEST_ALL_PREFIXES(MediaStreamDeviceObserverTest, MultiCaptureAddAndRemoveStreams);
    FRIEND_TEST_ALL_PREFIXES(MediaStreamDeviceObserverTest, MultiCaptureChangeDevices);
    FRIEND_TEST_ALL_PREFIXES(MediaStreamDeviceObserverTest, MultiCaptureChangeDeviceRequestState);
    FRIEND_TEST_ALL_PREFIXES(MediaStreamDeviceObserverTest, MultiCaptureRemoveStreamDevice);
    ...
    struct Stream {
        ...
        WebMediaStreamDeviceObserver::OnDeviceRequestStateChangeCb on_device_request_state_change_cb;
        WebMediaStreamDeviceObserver::OnDeviceCaptureConfigurationChangeCb on_device_capture_configuration_change_cb;
        WebMediaStreamDeviceObserver::OnDeviceCaptureHandleChangeCb on_device_capture_handle_change_cb;
    ...
    void OnDeviceStopped(const String& label, const MediaStreamDevice& device) override;
    void OnDeviceChanged(const String& label, const MediaStreamDevice& old_device, const MediaStreamDevice& new_device) override;
    void OnDeviceRequestStateChange(const String& label, const MediaStreamDevice& device, const mojom::blink::MediaStreamStateChange new_state) override;
    void OnDeviceCaptureConfigurationChange(const String& label, const MediaStreamDevice& device) override;
    void OnDeviceCaptureHandleChange(const String& label, const MediaStreamDevice& device) override;
    void OnZoomLevelChange(const String& label, const MediaStreamDevice& device, int zoom_level) override;

    void BindMediaStreamDeviceObserverReceiver(mojo::PendingReceiver<mojom::blink::MediaStreamDeviceObserver> receiver);
