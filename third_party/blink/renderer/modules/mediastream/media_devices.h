...
class MODULES_EXPORT MediaDevices final : public EventTarget,
...
public:
    ...
    ScriptPromise<IDLSequence<MediaDeviceInfo>> enumerateDevices(ScriptState*, ExceptionState&);
    MediaTrackSupportedConstraints* getSupportedConstraints() const;
    ScriptPromise<MediaStream> getUserMedia(ScriptState*, const UserMediaStreamConstraints*, ExceptionState&);
    ScriptPromise<IDLSequence<MediaStream>> getAllScreensMedia(ScriptState*, ExceptionState&);

    ScriptPromise<MediaStream> getDisplayMedia(ScriptState*, const DisplayMediaStreamOptions*, ExceptionState&);

    ScriptPromise<MediaDeviceInfo> selectAudioOutput(ScriptState*, const AudioOutputOptions* options, ExceptionState&);

    void setCaptureHandleConfig(ScriptState*, const CaptureHandleConfig*, ExceptionState&);
    ...
    ScriptPromise<CropTarget> ProduceCropTarget(ScriptState*, Element*, ExceptionState&);
    ScriptPromise<RestrictionTarget> ProduceRestrictionTarget(ScriptState*, Element*, ExceptionState&);
    ...
    void OnDevicesChanged(mojom::blink::MediaDeviceType, const Vector<WebMediaDeviceInfo>&) override;

    void SetDispatcherHostForTesting(mojo::PendingRemote<mojom::blink::MediaDevicesDispatcherHost>);
...
private:
    ...
    ScriptPromise<IDLResolvedType> SendUserMediaRequest(UserMediaRequestType, ScriptPromiseResolverWithTracker<UserMediaRequestResult, IDLResolvedType>*,
        const MediaStreamConstraints*, ExceptionState&, std::unique_ptr<ScopedMediaStreamTracer> tracer);

    void ScheduleDispatchEvent(Event*);
    void DispatchScheduledEvents();
    void StartObserving();
    void FinalizeStartObserving(const Vector<Vector<WebMediaDeviceInfo>>& enumeration,
    ...
    void StopObserving();
    void DevicesEnumerated(ScriptPromiseResolverWithTracker<EnumerateDevicesResult, IDLSequence<MediaDeviceInfo>>* result_tracker,
        std::unique_ptr<ScopedMediaStreamTracer> tracer, const Vector<Vector<WebMediaDeviceInfo>>&, Vector<mojom::blink::VideoInputDeviceCapabilitiesPtr>,
    ...
    void EnqueueMicrotaskToCloseFocusWindowOfOpportunity(const String&, CaptureController*);
    void CloseFocusWindowOfOpportunity(const String&, CaptureController*);

    bool MayProduceSubCaptureTarget(ScriptState* script_state, Element* element, ExceptionState& exception_state, SubCaptureTarget::Type type);
    ...
    HeapHashSet<Member<ScriptPromiseResolverWithTracker<EnumerateDevicesResult, IDLSequence<MediaDeviceInfo>>>> enumerate_device_requests_;

#if !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS)
    using ElementToCropTargetResolverMap = HeapHashMap<Member<Element>, Member<ScriptPromiseResolver<CropTarget>>>;
    using ElementToRestrictionTargetResolverMap = HeapHashMap<Member<Element>, Member<ScriptPromiseResolver<RestrictionTarget>>>;
