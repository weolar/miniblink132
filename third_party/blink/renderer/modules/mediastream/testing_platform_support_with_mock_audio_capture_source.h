...
public:
    ...
    MOCK_METHOD2(Initialize, void(const media::AudioParameters& params, CaptureCallback* callback));
...
class AudioCapturerSourceTestingPlatformSupport : public IOTaskRunnerTestingPlatformSupport {
public:
    AudioCapturerSourceTestingPlatformSupport() = default;

    scoped_refptr<media::AudioCapturerSource> NewAudioCapturerSource(WebLocalFrame* web_frame, const media::AudioSourceParameters& params) override;
...
private:
    scoped_refptr<MockAudioCapturerSource> mock_audio_capturer_source_ = base::MakeRefCounted<MockAudioCapturerSource>();
};
