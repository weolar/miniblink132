...
void SignalSourceReady(WebPlatformMediaStreamSource::ConstraintsOnceCallback source_ready, WebPlatformMediaStreamSource* source)
{
    std::move(source_ready).Run(source, mojom::blink::MediaStreamRequestResult::OK, "");
}
...
public:
    explicit MockUserMediaProcessor(LocalFrame* frame)
        : UserMediaProcessor(frame, base::BindRepeating(&UnusedMediaDevicesDispatcherHost), scheduler::GetSingleThreadTaskRunnerForTesting())
    {
    }

    // UserMediaProcessor overrides.
    std::unique_ptr<MediaStreamVideoSource> CreateVideoSource(
        const MediaStreamDevice& device, WebPlatformMediaStreamSource::SourceStoppedCallback stop_callback) override
    {
    ...
    std::unique_ptr<MediaStreamAudioSource> CreateAudioSource(
        const MediaStreamDevice& device, WebPlatformMediaStreamSource::ConstraintsRepeatingCallback source_ready) override
    {
        auto source = std::make_unique<MediaStreamAudioSource>(scheduler::GetSingleThreadTaskRunnerForTesting(),
            /*is_local_source=*/true);
        source->SetDevice(device);
        // RunUntilIdle is required for this task to complete.
        scheduler::GetSingleThreadTaskRunnerForTesting()->PostTask(FROM_HERE, base::BindOnce(&SignalSourceReady, std::move(source_ready), source.get()));
        return source;
    }
};
...
public:
    UserMediaClientUnderTest(LocalFrame* frame, UserMediaProcessor* user_media_processor, UserMediaProcessor* display_user_media_processor,
        scoped_refptr<base::SingleThreadTaskRunner> task_runner)
        : UserMediaClient(frame, user_media_processor, display_user_media_processor, task_runner)
    {
    }
};
...
public:
    ...
    {
        auto* user_media_processor = MakeGarbageCollected<MockUserMediaProcessor>(window->GetFrame());
        auto* display_user_media_processor = MakeGarbageCollected<MockUserMediaProcessor>(window->GetFrame());
        user_media_processor->set_media_stream_dispatcher_host_for_testing(mock_media_stream_dispatcher_host.CreatePendingRemoteAndBind());
        display_user_media_processor->set_media_stream_dispatcher_host_for_testing(display_mock_media_stream_dispatcher_host.CreatePendingRemoteAndBind());
        temp_ = MakeGarbageCollected<UserMediaClientUnderTest>(
            window->GetFrame(), user_media_processor, display_user_media_processor, scheduler::GetSingleThreadTaskRunnerForTesting());
        Supplement<LocalDOMWindow>::ProvideTo<UserMediaClient>(*window, temp_.Get());
    }

    ~ScopedMockUserMediaClient()
    {
        auto* window = temp_->GetSupplementable();
        if (Supplement<LocalDOMWindow>::From<UserMediaClient>(window) == temp_.Get()) {
            if (original_) {
                Supplement<LocalDOMWindow>::ProvideTo<UserMediaClient>(*window, original_.Get());
            } else {
                window->Supplementable<LocalDOMWindow>::RemoveSupplement<UserMediaClient>();
            }
        }
    }
...
MediaStreamTrack::TransferredValues TransferredValuesTabCaptureVideo()
{
    ...
    // them in sync.
    return MediaStreamTrack::TransferredValues { .track_impl_subtype = BrowserCaptureMediaStreamTrack::GetStaticWrapperTypeInfo(),
...
mojom::blink::StreamDevices DevicesTabCaptureVideo(base::UnguessableToken session_id)
{
...
#endif
    scoped_user_media_client.display_mock_media_stream_dispatcher_host.SetStreamDevices(DevicesTabCaptureVideo(data.session_id));

    auto* new_track = MediaStreamTrack::FromTransferredState(scope.GetScriptState(), data);

#if BUILDFLAG(IS_ANDROID)
    EXPECT_EQ(new_track->GetWrapperTypeInfo(), MediaStreamTrack::GetStaticWrapperTypeInfo());
#else
    EXPECT_EQ(new_track->GetWrapperTypeInfo(), BrowserCaptureMediaStreamTrack::GetStaticWrapperTypeInfo());
#endif
...
TEST(MediaStreamTrackTransferTest, TabCaptureAudioFromTransferredState)
{
    ...
    // keep them in sync.
    MediaStreamTrack::TransferredValues data { .track_impl_subtype = MediaStreamTrack::GetStaticWrapperTypeInfo(),
        .session_id = base::UnguessableToken::Create(),
        .transfer_id = base::UnguessableToken::Create(),
        .kind = "audio",
        .id = "component_id",
    ...
    MediaStreamDevice device(mojom::MediaStreamType::DISPLAY_AUDIO_CAPTURE, "device_id", "device_name");
    device.set_session_id(data.session_id);
    device.display_media_info = media::mojom::DisplayMediaInformation::New(media::mojom::DisplayCaptureSurfaceType::BROWSER,
        /*logical_surface=*/true, media::mojom::CursorCaptureType::NEVER,
        /*capture_handle=*/nullptr,
        /*zoom_level=*/100);
    scoped_user_media_client.display_mock_media_stream_dispatcher_host.SetStreamDevices({ std::nullopt, device });
