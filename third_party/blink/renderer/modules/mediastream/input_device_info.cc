...
InputDeviceInfo::InputDeviceInfo(const String& device_id, const String& label, const String& group_id, mojom::blink::MediaDeviceType device_type)
    : MediaDeviceInfo(device_id, label, group_id, device_type)
{
}

void InputDeviceInfo::SetVideoInputCapabilities(mojom::blink::VideoInputDeviceCapabilitiesPtr video_input_capabilities)
{
    ...
    platform_capabilities_.facing_mode = ToPlatformFacingMode(video_input_capabilities->facing_mode);
    if (!video_input_capabilities->formats.empty()) {
        ...
        platform_capabilities_.aspect_ratio = { 1.0 / max_height, static_cast<double>(max_width) };
        platform_capabilities_.frame_rate = { min_frame_rate, max_frame_rate };
    }
    platform_capabilities_.is_available
        = !video_input_capabilities->availability || (*video_input_capabilities->availability == media::mojom::CameraAvailability::kAvailable);
}

void InputDeviceInfo::SetAudioInputCapabilities(mojom::blink::AudioInputDeviceCapabilitiesPtr audio_input_capabilities)
{
    DCHECK_EQ(deviceId(), audio_input_capabilities->device_id);

    if (audio_input_capabilities->is_valid) {
        platform_capabilities_.channel_count = { 1, audio_input_capabilities->channels };

        platform_capabilities_.sample_rate = { std::min(media::WebRtcAudioProcessingSampleRateHz(), audio_input_capabilities->sample_rate),
            std::max(media::WebRtcAudioProcessingSampleRateHz(), audio_input_capabilities->sample_rate) };
        double fallback_latency = kFallbackAudioLatencyMs / 1000;
        platform_capabilities_.latency = { std::min(fallback_latency, audio_input_capabilities->latency.InSecondsF()),
            std::max(fallback_latency, audio_input_capabilities->latency.InSecondsF()) };
    }
}
...
MediaTrackCapabilities* InputDeviceInfo::getCapabilities() const
{
    ...
    if (DeviceType() == mojom::blink::MediaDeviceType::kMediaAudioInput) {
        ...
        sample_size->setMin(media::SampleFormatToBitsPerChannel(media::kSampleFormatS16));
        sample_size->setMax(media::SampleFormatToBitsPerChannel(media::kSampleFormatS16));
