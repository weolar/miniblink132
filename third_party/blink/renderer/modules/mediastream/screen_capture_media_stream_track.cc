...
ScreenCaptureMediaStreamTrack::ScreenCaptureMediaStreamTrack(
    ExecutionContext* context, MediaStreamComponent* component, ScreenDetails* screen_details, ScreenDetailed* screen_detailed)
    : MediaStreamTrackImpl(context, component, component->Source()->GetReadyState(),
...
ScreenDetailed* ScreenCaptureMediaStreamTrack::screenDetailed(ScriptState* script_state, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    DCHECK(script_state);
    ...
    if (!screen_detailed_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The ScreenDetailed object could not be created.");
    }
