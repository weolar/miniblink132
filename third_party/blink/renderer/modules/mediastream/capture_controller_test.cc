...
bool IsDOMException(ScriptState* script_state, const ScriptValue& value, DOMExceptionCode code)
{
    DOMException* const dom_exception = V8DOMException::ToWrappable(script_state->GetIsolate(), value.V8Value());
    return dom_exception && dom_exception->name() == DOMException(code).name();
}

bool IsDOMException(V8TestingScope& v8_scope, const ScriptValue& value, DOMExceptionCode code)
{
...
String GetDOMExceptionMessage(ScriptState* script_state, const ScriptValue& value)
{
    DOMException* const dom_exception = V8DOMException::ToWrappable(script_state->GetIsolate(), value.V8Value());
    CHECK(dom_exception) << "Malformed test.";
    return dom_exception->message();
}
String GetDOMExceptionMessage(V8TestingScope& v8_scope, const ScriptValue& value)
{
...
MediaStreamTrack* MakeTrack(ExecutionContext* execution_context, SurfaceType display_surface,
    int initial_zoom_level = CaptureController::getSupportedZoomLevels()[0], bool use_session_id = true)
{
    std::unique_ptr<MockMediaStreamVideoSource> media_stream_video_source = base::WrapUnique(
        new ::testing::NiceMock<MockMediaStreamVideoSource>(media::VideoCaptureFormat(gfx::Size(640, 480), 30.0, media::PIXEL_FORMAT_I420), true));

    // Set the reported SurfaceType.
    MediaStreamDevice device = media_stream_video_source->device();
    device.display_media_info = media::mojom::DisplayMediaInformation::New(display_surface,
    ...
    auto media_stream_video_track
        = std::make_unique<MediaStreamVideoTrack>(media_stream_video_source.get(), WebPlatformMediaStreamSource::ConstraintsOnceCallback(),
            /*enabled=*/true);

    MediaStreamSource* const source = MakeGarbageCollected<MediaStreamSource>("id", MediaStreamSource::StreamType::kTypeVideo, "name",
        /*remote=*/false, std::move(media_stream_video_source));

    MediaStreamComponent* const component = MakeGarbageCollected<MediaStreamComponentImpl>("component_id", source, std::move(media_stream_video_track));

    switch (display_surface) {
    case SurfaceType::BROWSER:
        return MakeGarbageCollected<BrowserCaptureMediaStreamTrack>(execution_context, component,
            /*callback=*/base::DoNothing());
    case SurfaceType::WINDOW:
    case SurfaceType::MONITOR:
        return MakeGarbageCollected<MediaStreamTrackImpl>(execution_context, component,
            /*callback=*/base::DoNothing());
    }
    NOTREACHED();
}

MediaStreamTrack* MakeTrack(V8TestingScope& testing_scope, SurfaceType display_surface, int initial_zoom_level = CaptureController::getSupportedZoomLevels()[0],
    bool use_session_id = true)
{
    return MakeTrack(testing_scope.GetExecutionContext(), display_surface, initial_zoom_level, use_session_id);
}

void SimulateFrameArrival(MediaStreamTrack* track, gfx::Size frame_size = gfx::Size(1000, 1000))
{
    GetMediaStreamVideoTrack(track)->SetTargetSize(frame_size.width(), frame_size.height());
}
...
protected:
    ...
    {
        auto* controller = MakeGarbageCollected<CaptureController>(execution_context);
        controller->SetMediaStreamDispatcherHostForTesting(mock_dispatcher_host_.CreatePendingRemoteAndBind());
        return controller;
    }
...
class CaptureControllerBaseTest : public testing::Test, public CaptureControllerTestSupport {
public:
...
class CaptureControllerGetSupportedZoomLevelsTest : public CaptureControllerBaseTest {
public:
    ~CaptureControllerGetSupportedZoomLevelsTest() override = default;
};

TEST_F(CaptureControllerGetSupportedZoomLevelsTest, ReturnsMonotonicallyIncreasingSequence)
{
    V8TestingScope v8_scope;
    const Vector<int> supported_levels = CaptureController::getSupportedZoomLevels();
...
TEST_F(CaptureControllerGetZoomLevelTest, GetZoomLevelFailsIfCaptureControllerNotBound)
{
    V8TestingScope v8_scope;
    CaptureController* controller = MakeController(v8_scope.GetExecutionContext());
    ...
    EXPECT_EQ(v8_scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kInvalidStateError);

    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    EXPECT_EQ(v8_scope.GetExceptionState().Message(), "getDisplayMedia() not called yet.");
}

TEST_F(CaptureControllerGetZoomLevelTest, GetZoomLevelFailsIfCaptureControllerBoundButNoVideoTrack)
{
    V8TestingScope v8_scope;
    CaptureController* controller = MakeController(v8_scope.GetExecutionContext());
    ...
    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    EXPECT_EQ(v8_scope.GetExceptionState().Message(), "Capture-session not started.");
}
...
TEST_F(CaptureControllerGetZoomLevelTest, GetZoomLevelSuccessZoomLevelUpdate)
{
    ...
    track->Component()->Source()->OnZoomLevelChange(MediaStreamDevice(), CaptureController::getSupportedZoomLevels()[1]);
...
TEST_F(CaptureControllerGetZoomLevelTest, GetZoomLevelFailsIfCapturingWindow)
{
    ...
    controller->getZoomLevel(v8_scope.GetExceptionState());
    ASSERT_TRUE(v8_scope.GetExceptionState().HadException());
    EXPECT_EQ(v8_scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kNotSupportedError);

    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    EXPECT_EQ(v8_scope.GetExceptionState().Message(), "Action only supported for tab-capture.");
}
...
class CaptureControllerOnCapturedZoomLevelChangeTest : public CaptureControllerBaseTest {
public:
...
TEST_F(CaptureControllerOnCapturedZoomLevelChangeTest, NoEventOnInit)
{
    ...
    StrictMock<MockEventListener>* event_listener = MakeGarbageCollected<StrictMock<MockEventListener>>();
    EXPECT_CALL(*event_listener, Invoke(_, _)).Times(0);
    controller->addEventListener(event_type_names::kCapturedzoomlevelchange, event_listener);
...
TEST_F(CaptureControllerOnCapturedZoomLevelChangeTest, EventWhenDifferentFromInitValue)
{
    V8TestingScope v8_scope;
    CaptureController* controller = MakeController(v8_scope.GetExecutionContext());
    StrictMock<MockEventListener>* event_listener = MakeGarbageCollected<StrictMock<MockEventListener>>();
...
TEST_F(CaptureControllerOnCapturedZoomLevelChangeTest, NoEventWhenSameAsInitValue)
{
    V8TestingScope v8_scope;
    CaptureController* controller = MakeController(v8_scope.GetExecutionContext());
    StrictMock<MockEventListener>* event_listener = MakeGarbageCollected<StrictMock<MockEventListener>>();
    ...
    EXPECT_CALL(*event_listener, Invoke(_, _)).Times(0);
    track->Component()->Source()->OnZoomLevelChange(MediaStreamDevice(), CaptureController::getSupportedZoomLevels()[0]);
}

TEST_F(CaptureControllerOnCapturedZoomLevelChangeTest, EventWhenDifferentFromPreviousUpdate)
{
    V8TestingScope v8_scope;
    CaptureController* controller = MakeController(v8_scope.GetExecutionContext());
    StrictMock<MockEventListener>* event_listener = MakeGarbageCollected<StrictMock<MockEventListener>>();
...
TEST_F(CaptureControllerOnCapturedZoomLevelChangeTest, EventWhenSameAsPreviousUpdate)
{
    V8TestingScope v8_scope;
    CaptureController* controller = MakeController(v8_scope.GetExecutionContext());
    StrictMock<MockEventListener>* event_listener = MakeGarbageCollected<StrictMock<MockEventListener>>();
...
TEST_F(CaptureControllerSetZoomLevelTest, SetZoomLevelFailsIfCaptureControllerNotBound)
{
    V8TestingScope v8_scope;
    CaptureController* controller = MakeController(v8_scope.GetExecutionContext());
    // Test avoids calling CaptureController::SetIsBound().
    ...
    EXPECT_TRUE(IsDOMException(v8_scope, promise_tester.Value(), DOMExceptionCode::kInvalidStateError));

    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    EXPECT_EQ(GetDOMExceptionMessage(v8_scope, promise_tester.Value()), "getDisplayMedia() not called yet.");
}

TEST_F(CaptureControllerSetZoomLevelTest, SetZoomLevelFailsIfCaptureControllerBoundButNoVideoTrack)
{
    V8TestingScope v8_scope;
    CaptureController* controller = MakeController(v8_scope.GetExecutionContext());
    controller->SetIsBound(true);
    ...
    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    EXPECT_EQ(GetDOMExceptionMessage(v8_scope, promise_tester.Value()), "Capture-session not started.");
}
...
TEST_F(CaptureControllerSetZoomLevelTest, SetZoomLevelFailsIfVideoTrackEnded)
{
    ...
    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    EXPECT_EQ(GetDOMExceptionMessage(v8_scope, promise_tester.Value()), "Video track ended.");
}
...
TEST_F(CaptureControllerSetZoomLevelTest, SetZoomLevelSuccessIfSupportedValue)
{
    ...
    for (int zoom_level : supported_levels) {
        EXPECT_CALL(DispatcherHost(), SetZoomLevel(_, zoom_level, _)).WillOnce(RunOnceCallback<2>(CscResult::kSuccess));
        const auto promise = controller->setZoomLevel(v8_scope.GetScriptState(), zoom_level);
...
TEST_F(CaptureControllerSetZoomLevelTest, SetZoomLevelFailsIfLevelTooLow)
{
    ...
    const auto promise = controller->setZoomLevel(v8_scope.GetScriptState(), controller->getSupportedZoomLevels().front() - 1);
    ScriptPromiseTester promise_tester(v8_scope.GetScriptState(), promise);
    promise_tester.WaitUntilSettled();
    EXPECT_TRUE(IsDOMException(v8_scope, promise_tester.Value(), DOMExceptionCode::kInvalidStateError));
    ...
    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    EXPECT_EQ(GetDOMExceptionMessage(v8_scope, promise_tester.Value()), "Only values returned by getSupportedZoomLevels() are valid.");
}
...
TEST_F(CaptureControllerSetZoomLevelTest, SetZoomLevelFailsIfLevelTooHigh)
{
    ...
    const auto promise = controller->setZoomLevel(v8_scope.GetScriptState(), controller->getSupportedZoomLevels().back() + 1);
    ScriptPromiseTester promise_tester(v8_scope.GetScriptState(), promise);
    promise_tester.WaitUntilSettled();
    EXPECT_TRUE(IsDOMException(v8_scope, promise_tester.Value(), DOMExceptionCode::kInvalidStateError));
...
TEST_F(CaptureControllerSetZoomLevelTest, SetZoomLevelFailsIfUnsupportedValue)
{
    ...
    const auto promise = controller->setZoomLevel(v8_scope.GetScriptState(), unsupported_level);
...
TEST_F(CaptureControllerSetZoomLevelTest, SetZoomLevelFailsIfCapturingWindow)
{
    ...
    promise_tester.WaitUntilSettled();
    EXPECT_TRUE(promise_tester.IsRejected());
    EXPECT_TRUE(IsDOMException(v8_scope, promise_tester.Value(), DOMExceptionCode::kNotSupportedError));

    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    EXPECT_EQ(GetDOMExceptionMessage(v8_scope, promise_tester.Value()), "Action only supported for tab-capture.");
}
...
TEST_F(CaptureControllerSetZoomLevelTest, SimulatedFailureFromDispatcherHost)
{
    ...
    EXPECT_CALL(DispatcherHost(), SetZoomLevel(_, _, _)).WillOnce(RunOnceCallback<2>(CscResult::kUnknownError));
    const auto promise = controller->setZoomLevel(v8_scope.GetScriptState(), 125);
    ScriptPromiseTester promise_tester(v8_scope.GetScriptState(), promise);
    promise_tester.WaitUntilSettled();
    EXPECT_TRUE(promise_tester.IsRejected());
    EXPECT_TRUE(IsDOMException(v8_scope, promise_tester.Value(), DOMExceptionCode::kUnknownError));

    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    EXPECT_EQ(GetDOMExceptionMessage(v8_scope, promise_tester.Value()), "Unknown error.");
}
...
TEST_F(CaptureControllerSetZoomLevelTest, SetZoomLevelFailsWithoutSessionId)
{
    ...
    CaptureController* controller = MakeController(v8_scope.GetExecutionContext());
    controller->SetIsBound(true);
    MediaStreamTrack* track = MakeTrack(v8_scope, SurfaceType::BROWSER, CaptureController::getSupportedZoomLevels()[0], /*use_session_id=*/false);
    controller->SetVideoTrack(track, "descriptor");
    ...
    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    EXPECT_EQ(GetDOMExceptionMessage(v8_scope, promise_tester.Value()), "Invalid capture");
}
...
TEST_F(CaptureControllerScrollTest, SendWheelFailsIfCaptureControllerNotBound)
{
    ...
    const auto promise = controller->sendWheel(v8_scope.GetScriptState(), CapturedWheelAction::Create());
...
TEST_F(CaptureControllerScrollTest, SendWheelFailsIfCaptureControllerBoundButNoVideoTrack)
{
    V8TestingScope v8_scope;
    CaptureController* controller = MakeController(v8_scope.GetExecutionContext());
    controller->SetIsBound(true);
...
TEST_F(CaptureControllerScrollTest, SendWheelSuccess)
{
    ...
    EXPECT_CALL(DispatcherHost(), SendWheel(_, _, _)).WillOnce(RunOnceCallback<2>(CscResult::kSuccess));
    const auto promise = controller->sendWheel(v8_scope.GetScriptState(), CapturedWheelAction::Create());
...
TEST_F(CaptureControllerScrollTest, SimulatedFailureFromDispatcherHost)
{
    ...
    EXPECT_CALL(DispatcherHost(), SendWheel(_, _, _)).WillOnce(RunOnceCallback<2>(CscResult::kUnknownError));
    const auto promise = controller->sendWheel(v8_scope.GetScriptState(), CapturedWheelAction::Create());
...
TEST_F(CaptureControllerScrollTest, SendWheelFailsBeforeReceivingFrames)
{
    ...
    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    EXPECT_EQ(GetDOMExceptionMessage(v8_scope, promise_tester.Value()), "No frames observed yet.");
}
...
TEST_F(CaptureControllerScrollTest, SendWheelScalesCorrectly)
{
    ...
    EXPECT_CALL(DispatcherHost(), SendWheel(_, _, _)).WillOnce(DoAll(SaveArgPointee<1>(&dispatcher_action), RunOnceCallbackRepeatedly<2>(CscResult::kSuccess)));
...
class CaptureConstrollerCaptureWheelTest : public PageTestBase, public CaptureControllerTestSupport { };

TEST_F(CaptureConstrollerCaptureWheelTest, Success)
{
    CaptureController* controller = MakeController(GetDocument().GetExecutionContext());
    controller->SetIsBound(true);
    MediaStreamTrack* track = MakeTrack(GetDocument().GetExecutionContext(), SurfaceType::BROWSER);
    controller->SetVideoTrack(track, "descriptor");
    ...
    EXPECT_CALL(DispatcherHost(), RequestCapturedSurfaceControlPermission(_, _)).WillOnce(RunOnceCallback<1>(CscResult::kSuccess));
    ...
    EXPECT_CALL(DispatcherHost(), SendWheel(_, _, _)).WillOnce(DoAll(Invoke(&run_loop, &base::RunLoop::Quit), RunOnceCallback<2>(CscResult::kSuccess)));
    element->DispatchEvent(*WheelEvent::Create(event_type_names::kWheel, WheelEventInit::Create()));
    run_loop.Run();
    ...
    EXPECT_CALL(*mock_listener, Invoke).WillOnce(Invoke(&run_loop2, &base::RunLoop::Quit));
    element->DispatchEvent(*WheelEvent::Create(event_type_names::kWheel, WheelEventInit::Create()));
    run_loop2.Run();
}
...
TEST_F(CaptureConstrollerCaptureWheelTest, DropUntrustedEvent)
{
    ...
    ScriptState::Scope scope(script_state);
    EXPECT_CALL(DispatcherHost(), RequestCapturedSurfaceControlPermission(_, _)).WillOnce(RunOnceCallback<1>(CscResult::kSuccess));
    ScriptPromiseTester(script_state, controller->captureWheel(script_state, element)).WaitUntilSettled();

    EXPECT_CALL(DispatcherHost(), SendWheel(_, _, _)).Times(0);
    DummyExceptionStateForTesting exception_state;
    // Events dispatched with dispatchEventForBindings are always untrusted.
    element->dispatchEventForBindings(WheelEvent::Create(event_type_names::kWheel, WheelEventInit::Create()), exception_state);
...
TEST_F(CaptureConstrollerCaptureWheelTest, SuccessWithNoElement)
{
    ...
    EXPECT_CALL(DispatcherHost(), RequestCapturedSurfaceControlPermission(_, _)).Times(0);
...
TEST_F(CaptureConstrollerCaptureWheelTest, BackendError)
{
    ...
    ScriptState::Scope scope(script_state);
    for (CscResult csc_error_result : { CscResult::kUnknownError, CscResult::kNoPermissionError, CscResult::kCapturerNotFoundError,
             CscResult::kCapturedSurfaceNotFoundError, CscResult::kDisallowedForSelfCaptureError, CscResult::kCapturerNotFocusedError }) {
        EXPECT_CALL(DispatcherHost(), RequestCapturedSurfaceControlPermission(_, _)).WillOnce(RunOnceCallback<1>(csc_error_result));
        ...
        EXPECT_CALL(*mock_listener, Invoke).WillRepeatedly(Invoke(&run_loop, &base::RunLoop::Quit));
        element->DispatchEvent(*WheelEvent::Create(event_type_names::kWheel, WheelEventInit::Create()));
        run_loop.Run();
    }
}
...
TEST_F(CaptureConstrollerCaptureWheelTest, NoSessionId)
{
    ...
    promise_tester.WaitUntilSettled();
    EXPECT_TRUE(promise_tester.IsRejected());
    EXPECT_TRUE(IsDOMException(script_state, promise_tester.Value(), DOMExceptionCode::kInvalidStateError));
    EXPECT_EQ(GetDOMExceptionMessage(script_state, promise_tester.Value()), "Invalid capture.");
}
...
class CaptureControllerScrollParametersValidationTest
    : public CaptureControllerScrollTest,
      public WithParamInterface<std::tuple<std::tuple<ScrollDirection, ScrollDirection>, std::tuple<gfx::Point, bool>>> {
public:
...
constexpr int kRightmost = CaptureControllerScrollParametersValidationTest::kWidth - 1;
constexpr int kTop = 0;
constexpr int kBottom = CaptureControllerScrollParametersValidationTest::kHeight - 1;

INSTANTIATE_TEST_SUITE_P(, CaptureControllerScrollParametersValidationTest,
    Combine(
        // Scroll direction.
        Combine(
            // Vertical scroll.
            Values(ScrollDirection::kNone, ScrollDirection::kForwards, ScrollDirection::kBackwards),
            // Horizontal scroll.
            Values(ScrollDirection::kNone, ScrollDirection::kForwards, ScrollDirection::kBackwards)),
        // Scroll coordinates and expectation.
        Values(
            // Corners
            std::make_tuple(gfx::Point(kLeftmost, kTop), true), std::make_tuple(gfx::Point(kLeftmost, kBottom), true),
            std::make_tuple(gfx::Point(kRightmost, kTop), true), std::make_tuple(gfx::Point(kRightmost, kBottom), true),
            // Just beyond top-left
            std::make_tuple(gfx::Point(kLeftmost - 1, kTop), false), std::make_tuple(gfx::Point(kLeftmost, kTop - 1), false),
            // Just beyond bottom-left
            std::make_tuple(gfx::Point(kLeftmost - 1, kBottom), false), std::make_tuple(gfx::Point(kLeftmost, kBottom + 1), false),
            // Just beyond top-right
            std::make_tuple(gfx::Point(kRightmost + 1, kTop), false), std::make_tuple(gfx::Point(kRightmost, kTop - 1), false),
            // Just beyond bottom-right
            std::make_tuple(gfx::Point(kRightmost + 1, kBottom), false), std::make_tuple(gfx::Point(kRightmost, kBottom + 1), false))));
} // namespace
...
TEST_P(CaptureControllerScrollParametersValidationTest, ValidateCoordinates)
{
    ...
    ScriptPromiseTester promise_tester(v8_scope.GetScriptState(), promise);
    promise_tester.WaitUntilSettled();
    EXPECT_TRUE(expect_success_ ? promise_tester.IsFulfilled() : promise_tester.IsRejected());

    // Avoid false-positives through different error paths terminating in
    // exception with the same code.
    if (!expect_success_) {
        EXPECT_EQ(GetDOMExceptionMessage(v8_scope, promise_tester.Value()), "Coordinates out of bounds.");
    }
}
