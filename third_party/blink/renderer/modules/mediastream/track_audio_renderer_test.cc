...
const media::AudioParameters kDefaultFormat(media::AudioParameters::Format::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::Stereo(), kSampleRate, kFrames);
...
class AudioRendererSinkTestingPlatformSupport : public IOTaskRunnerTestingPlatformSupport {
public:
...
private:
    scoped_refptr<media::FakeAudioRendererSink> fake_sink_ = base::MakeRefCounted<media::FakeAudioRendererSink>(kDefaultFormat);
};
...
public:
    ...
    {
        media::ChannelLayoutConfig layout = data.channels() == 2 ? media::ChannelLayoutConfig::Stereo() : media::ChannelLayoutConfig::Mono();

        media::AudioParameters format(media::AudioParameters::Format::AUDIO_PCM_LINEAR, layout, kSampleRate, data.frames());
...
public:
    ...
    void SetUp() override
    {
        ...
        auto platform_track = std::make_unique<MediaStreamAudioTrack>(/*is_local_track=*/true);

        auto* audio_component = MakeGarbageCollected<MediaStreamComponentImpl>(
            MakeGarbageCollected<MediaStreamSource>(
                String::FromUTF8("audio_id"), MediaStreamSource::kTypeAudio, String::FromUTF8("audio_track"), false /* remote */, std::move(source)),
            std::move(platform_track));

        static_cast<blink::MediaStreamAudioSource*>(audio_component->Source()->GetPlatformSource())->ConnectToInitializedTrack(audio_component);

        track_renderer_ = base::MakeRefCounted<TrackAudioRenderer>(
            audio_component, dummy_page_.GetFrame(), String(), base::BindRepeating(&TrackAudioRendererTest::OnRenderError, base::Unretained(this)));
    }
    ...
    void RunCaptureIntegrationTest(const base::RepeatingClosure& simulate_capture_callback, bool expect_dropped_frames)
    {
...
protected:
    void VerifyFrameCounts(bool expect_dropped_frames)
    {
        if (expect_dropped_frames) {
            ...
            EXPECT_GE(track_renderer_->TotalFramesPushedForTesting(), frames_captured_since_last_reconfig_);
            EXPECT_GE(track_renderer_->FramesInAudioShifterForTesting(), frames_captured_since_last_reconfig_);
        } else {
            EXPECT_EQ(track_renderer_->TotalFramesPushedForTesting(), total_frames_captured_);
            EXPECT_EQ(track_renderer_->FramesInAudioShifterForTesting(), total_frames_captured_);
        }
    }
    ...
    void SimulateDataCapture(int frames, int channels = kChannels)
    {
        ...
        IOTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&TrackAudioRendererTest::PushDataOnIO, base::Unretained(this), media::AudioBus::Create(channels, frames), base::TimeTicks::Now()));
    }
    ...
    void SyncAllSequences()
    {
        ...
        {
            base::RunLoop loop;
            scheduler::GetSequencedTaskRunnerForTesting()->PostTask(FROM_HERE, loop.QuitClosure());
            loop.Run();
        }
    }
...
private:
    ...
    void PushDataOnIO(std::unique_ptr<media::AudioBus> data, base::TimeTicks reference_time)
    {
    ...
    ScopedTestingPlatformSupport<AudioRendererSinkTestingPlatformSupport> platform_;
...
{
    RunCaptureIntegrationTest(base::BindLambdaForTesting([this]() { SimulateDataCapture(kFrames); }),
...
INSTANTIATE_TEST_SUITE_P(, TrackAudioRendererTest, testing::ValuesIn({ true, false }));
