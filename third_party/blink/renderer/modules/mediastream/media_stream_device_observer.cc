...
bool RemoveStreamDeviceFromArray(const MediaStreamDevice& device, MediaStreamDevices* devices)
{
...
MediaStreamDeviceObserver::MediaStreamDeviceObserver(LocalFrame* frame)
{
    ...
    if (frame) {
        frame->GetInterfaceRegistry()->AddInterface(
            WTF::BindRepeating(&MediaStreamDeviceObserver::BindMediaStreamDeviceObserverReceiver, weak_factory_.GetWeakPtr()));
    }
}
...
void MediaStreamDeviceObserver::OnDeviceStopped(const String& label, const MediaStreamDevice& device)
{
    ...
    for (Stream& stream : it->value) {
        ...
        if (stream.on_device_stopped_cb) {
            ...
            base::WeakPtr<MediaStreamDeviceObserver> weak_this = weak_factory_.GetWeakPtr();
...
void MediaStreamDeviceObserver::OnDeviceChanged(const String& label, const MediaStreamDevice& old_device, const MediaStreamDevice& new_device)
{
    DVLOG(1) << __func__ << " old_device_id=" << old_device.id << " new_device_id=" << new_device.id;
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (old_device.type != mojom::MediaStreamType::NO_SERVICE && new_device.type != mojom::MediaStreamType::NO_SERVICE) {
        if (RemoveStreamDeviceFromArray(old_device, &stream->audio_devices) || RemoveStreamDeviceFromArray(old_device, &stream->video_devices)) {
...
void MediaStreamDeviceObserver::OnDeviceRequestStateChange(
    const String& label, const MediaStreamDevice& device, const mojom::blink::MediaStreamStateChange new_state)
{
    DVLOG(1) << __func__ << " label=" << label << " device_id=" << device.id;
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (Stream& stream : it->value) {
        if (stream.ContainsDevice(device) && stream.on_device_request_state_change_cb) {
...
void MediaStreamDeviceObserver::OnDeviceCaptureConfigurationChange(const String& label, const MediaStreamDevice& device)
{
    DVLOG(1) << __func__ << " label=" << label << " device_id=" << device.id;
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (Stream& stream : it->value) {
        if (stream.ContainsDevice(device) && stream.on_device_capture_configuration_change_cb) {
...
void MediaStreamDeviceObserver::OnDeviceCaptureHandleChange(const String& label, const MediaStreamDevice& device)
{
    DVLOG(1) << __func__ << " label=" << label << " device_id=" << device.id;
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void MediaStreamDeviceObserver::OnZoomLevelChange(const String& label, const MediaStreamDevice& device, int zoom_level)
{
    DVLOG(1) << __func__ << " label=" << label << " device_id=" << device.id;
...
void MediaStreamDeviceObserver::BindMediaStreamDeviceObserverReceiver(mojo::PendingReceiver<mojom::blink::MediaStreamDeviceObserver> receiver)
{
    receiver_.reset();
...
void MediaStreamDeviceObserver::AddStreams(
    const String& label, const mojom::blink::StreamDevicesSet& stream_devices_set, const WebMediaStreamDeviceObserver::StreamCallbacks& stream_callbacks)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    Vector<Stream> streams;
    for (const mojom::blink::StreamDevicesPtr& stream_devices_ptr : stream_devices_set.stream_devices) {
        ...
        stream.on_device_request_state_change_cb = stream_callbacks.on_device_request_state_change_cb;
        stream.on_device_capture_configuration_change_cb = stream_callbacks.on_device_capture_configuration_change_cb;
        stream.on_device_capture_handle_change_cb = stream_callbacks.on_device_capture_handle_change_cb;
#if !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS)
...
void MediaStreamDeviceObserver::AddStream(const String& label, const MediaStreamDevice& device)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void MediaStreamDeviceObserver::RemoveStreamDevice(const MediaStreamDevice& device)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (auto& entry : label_stream_map_) {
        for (auto stream_it = entry.value.begin(); stream_it != entry.value.end();) {
            Stream& stream = *stream_it;
            MediaStreamDevices& audio_devices = stream.audio_devices;
            MediaStreamDevices& video_devices = stream.video_devices;
            if (RemoveStreamDeviceFromArray(device, &audio_devices) || RemoveStreamDeviceFromArray(device, &video_devices)) {
                device_found = true;
            }
...
base::UnguessableToken MediaStreamDeviceObserver::GetAudioSessionId(const String& label)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    auto it = label_stream_map_.find(label);
    if (it == label_stream_map_.end() || it->value.empty() || it->value[0].audio_devices.empty())
...
base::UnguessableToken MediaStreamDeviceObserver::GetVideoSessionId(const String& label)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    auto it = label_stream_map_.find(label);
    if (it == label_stream_map_.end() || it->value.empty() || it->value[0].video_devices.empty())
        return base::UnguessableToken();
...
bool MediaStreamDeviceObserver::Stream::ContainsDevice(const MediaStreamDevice& device) const
{
