...
public:
    ...
    {
        MediaStreamVideoSink::ConnectToTrack(track, GetDeliverFrameCB(), MediaStreamVideoSink::IsSecure::kYes, uses_alpha_);
    }

    void ConnectEncodedToTrack(const WebMediaStreamTrack& track)
    {
        MediaStreamVideoSink::ConnectEncodedToTrack(track, GetDeliverEncodedVideoFrameCB());
    }

    void ConnectToTrackWithCallback(const WebMediaStreamTrack& track, const VideoCaptureDeliverFrameCB& callback)
    {
        MediaStreamVideoSink::ConnectToTrack(track, callback, MediaStreamVideoSink::IsSecure::kYes, uses_alpha_);
    }
    ...
    void OnContentHintChanged(WebMediaStreamTrack::ContentHintType content_hint) override;
    MOCK_METHOD(void, OnVideoConstraintsChanged, (std::optional<double>, std::optional<double>), (override));
    ...
    scoped_refptr<media::VideoFrame> last_frame() const
    {
        return last_frame_;
    }
    ...
    WebMediaStreamSource::ReadyState state() const
    {
        return state_;
    }
...
private:
    void DeliverVideoFrame(scoped_refptr<media::VideoFrame> frame, base::TimeTicks estimated_capture_time);
    ...
    MediaStreamVideoSink::UsesAlpha uses_alpha_ = MediaStreamVideoSink::UsesAlpha::kDefault;
