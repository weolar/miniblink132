...
class MODULES_EXPORT BrowserCaptureMediaStreamTrack : public MediaStreamTrackImpl {
    DEFINE_WRAPPERTYPEINFO();

public:
    BrowserCaptureMediaStreamTrack(ExecutionContext* execution_context, MediaStreamComponent* component, base::OnceClosure callback);
    ...
    void OnSubCaptureTargetVersionObservedForTesting(uint32_t sub_capture_target_version)
    {
        OnSubCaptureTargetVersionObserved(sub_capture_target_version);
    }
#endif

    ScriptPromise<IDLUndefined> cropTo(ScriptState*, CropTarget*, ExceptionState&);
    ScriptPromise<IDLUndefined> restrictTo(ScriptState*, RestrictionTarget*, ExceptionState&);
...
private:
    ...
    ScriptPromise<IDLUndefined> ApplySubCaptureTarget(ScriptState*, SubCaptureTarget::Type, SubCaptureTarget*, ExceptionState&);

#if !BUILDFLAG(IS_ANDROID)
    struct PromiseInfo : GarbageCollected<PromiseInfo> {
        explicit PromiseInfo(ScriptPromiseResolverWithTracker<ApplySubCaptureTargetResult, IDLUndefined>* promise_resolver)
        ...
        {
            visitor->Trace(promise_resolver);
        }

        const Member<ScriptPromiseResolverWithTracker<ApplySubCaptureTargetResult, IDLUndefined>> promise_resolver;
        std::optional<media::mojom::ApplySubCaptureTargetResult> result;
        bool sub_capture_target_version_observed = false;
    };

    using SubCaptureTargetVersionToPromiseInfoMap = HeapHashMap<uint32_t, Member<BrowserCaptureMediaStreamTrack::PromiseInfo>>;
    ...
    void OnResultFromBrowserProcess(uint32_t sub_capture_target_version, media::mojom::ApplySubCaptureTargetResult result);
