...
public:
    FrameDeliverer(const RepaintCB& repaint_cb, base::WeakPtr<MediaStreamVideoRendererSink> media_stream_video_renderer_sink,
    ...
    void OnVideoFrame(scoped_refptr<media::VideoFrame> frame, base::TimeTicks /*current_time*/)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(video_sequence_checker_);
        DCHECK(frame);
        TRACE_EVENT_INSTANT1("webrtc",
            ...
            TRACE_EVENT_SCOPE_THREAD, "timestamp", frame->timestamp().InMilliseconds());

        if (state_ != kStarted) {
            if (emit_frame_drop_events_) {
                emit_frame_drop_events_ = false;
                PostCrossThreadTask(*main_render_task_runner_, FROM_HERE,
                    CrossThreadBindOnce(&MediaStreamVideoRendererSink::OnFrameDropped, media_stream_video_renderer_sink_,
                        media::VideoCaptureFrameDropReason::kRendererSinkFrameDelivererIsNotStarted));
            }
            return;
        }
    ...
    void RenderEndOfStream()
    {
        ...
        scoped_refptr<media::VideoFrame> video_frame
            = media::VideoFrame::CreateBlackFrame(state_ == kStopped ? gfx::Size(kMinFrameSize, kMinFrameSize) : frame_size_);
        if (!video_frame)
            return;
...
MediaStreamVideoRendererSink::MediaStreamVideoRendererSink(MediaStreamComponent* video_component, const RepaintCB& repaint_cb,
    scoped_refptr<base::SequencedTaskRunner> video_task_runner, scoped_refptr<base::SingleThreadTaskRunner> main_render_task_runner)
    : repaint_cb_(repaint_cb)
...
void MediaStreamVideoRendererSink::Start()
{
    ...
    frame_deliverer_ = std::make_unique<MediaStreamVideoRendererSink::FrameDeliverer>(repaint_cb_, weak_factory_.GetWeakPtr(), main_render_task_runner_);
    PostCrossThreadTask(*video_task_runner_, FROM_HERE, CrossThreadBindOnce(&FrameDeliverer::Start, WTF::CrossThreadUnretained(frame_deliverer_.get())));

    MediaStreamVideoSink::ConnectToTrack(WebMediaStreamTrack(video_component_.Get()),
        ...
        ConvertToBaseRepeatingCallback(WTF::CrossThreadBindRepeating(&FrameDeliverer::OnVideoFrame, WTF::CrossThreadUnretained(frame_deliverer_.get()))),
        // Local display video rendering is considered a secure link.
        MediaStreamVideoSink::IsSecure::kYes, MediaStreamVideoSink::UsesAlpha::kDependsOnOtherSinks);

    if (video_component_->GetReadyState() == MediaStreamSource::kReadyStateEnded || !video_component_->Enabled()) {
        PostCrossThreadTask(
            *video_task_runner_, FROM_HERE, CrossThreadBindOnce(&FrameDeliverer::RenderEndOfStream, CrossThreadUnretained(frame_deliverer_.get())));
    }
}
...
void MediaStreamVideoRendererSink::Resume()
{
    ...
    PostCrossThreadTask(*video_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&FrameDeliverer::Resume, WTF::CrossThreadUnretained(frame_deliverer_.get())));
}
...
void MediaStreamVideoRendererSink::Pause()
{
    ...
    PostCrossThreadTask(*video_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&FrameDeliverer::Pause, WTF::CrossThreadUnretained(frame_deliverer_.get())));
}

void MediaStreamVideoRendererSink::OnReadyStateChanged(WebMediaStreamSource::ReadyState state)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
    if (state == WebMediaStreamSource::kReadyStateEnded && frame_deliverer_) {
        PostCrossThreadTask(
            *video_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&FrameDeliverer::RenderEndOfStream, WTF::CrossThreadUnretained(frame_deliverer_.get())));
    }
}

MediaStreamVideoRendererSink::State MediaStreamVideoRendererSink::GetStateForTesting()
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
    if (!frame_deliverer_)
        return kStopped;
