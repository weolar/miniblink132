...
namespace {
const gfx::Size kTestScreenSize { kDefaultScreenCastWidth, kDefaultScreenCastHeight };
...
class TestObserver : public GarbageCollected<TestObserver>, public MediaStreamTrack::Observer {
public:
...
{
    return base::WrapUnique(new MockMediaStreamVideoSource(media::VideoCaptureFormat(gfx::Size(640, 480), 30.0, media::PIXEL_FORMAT_I420), true));
}

std::unique_ptr<blink::LocalMediaStreamAudioSource> MakeLocalMediaStreamAudioSource()
{
    ...
    return std::make_unique<blink::LocalMediaStreamAudioSource>(
        ...
        /*enable_system_echo_cancellation=*/false, blink::WebPlatformMediaStreamSource::ConstraintsRepeatingCallback(),
        blink::scheduler::GetSingleThreadTaskRunnerForTesting());
}

MediaStreamComponent* MakeMockVideoComponent()
{
    std::unique_ptr<MockMediaStreamVideoSource> platform_source = MakeMockMediaStreamVideoSource();
    MockMediaStreamVideoSource* platform_source_ptr = platform_source.get();
    MediaStreamSource* source = MakeGarbageCollected<MediaStreamSource>("id", MediaStreamSource::StreamType::kTypeVideo, "name",
        /*remote=*/false, std::move(platform_source));
    return MakeGarbageCollected<MediaStreamComponentImpl>(source,
...
{
    MediaStreamSource* source = MakeGarbageCollected<MediaStreamSource>("id", MediaStreamSource::StreamType::kTypeAudio, "name",
        /*remote=*/false, MakeLocalMediaStreamAudioSource());
    auto platform_track = std::make_unique<MediaStreamAudioTrack>(true /* is_local_track */);
    return MakeGarbageCollected<MediaStreamComponentImpl>(source, std::move(platform_track));
}
...
media::VideoCaptureFormat GetDefaultVideoContentCaptureFormat()
{
    ...
    return blink::SelectSettingsVideoContentCapture(
        constraints, mojom::MediaStreamType::DISPLAY_VIDEO_CAPTURE, kTestScreenSize.width(), kTestScreenSize.height())
        .Format();
}

std::tuple<MediaStreamComponent*, MockMediaStreamVideoSource*> MakeMockDisplayVideoCaptureComponent()
{
    auto platform_source = std::make_unique<MockMediaStreamVideoSource>(GetDefaultVideoContentCaptureFormat(), false);
    platform_source->SetDevice(MediaStreamDevice(mojom::MediaStreamType::DISPLAY_VIDEO_CAPTURE, "fakeSourceId", "fakeWindowCapturer"));
    MockMediaStreamVideoSource* platform_source_ptr = platform_source.get();
    MediaStreamSource* source
        = MakeGarbageCollected<MediaStreamSource>("id", MediaStreamSource::StreamType::kTypeVideo, "name", false /* remote */, std::move(platform_source));
    auto platform_track
        = std::make_unique<MediaStreamVideoTrack>(platform_source_ptr, WebPlatformMediaStreamSource::ConstraintsOnceCallback(), true /* enabled */);
    MediaStreamComponent* component = MakeGarbageCollected<MediaStreamComponentImpl>(source, std::move(platform_track));
    return std::make_tuple(component, platform_source_ptr);
}

MediaTrackConstraints* MakeMediaTrackConstraints(std::optional<int> exact_width, std::optional<int> exact_height, std::optional<float> min_frame_rate,
    std::optional<float> max_frame_rate, std::optional<float> aspect_ratio = std::nullopt)
{
    MediaConstraints constraints;
...
TEST_F(MediaStreamTrackImplTest, StopTrackTriggersObservers)
{
    ...
    MediaStreamSource* source = MakeGarbageCollected<MediaStreamSource>("id", MediaStreamSource::StreamType::kTypeVideo, "name",
        /*remote=*/false, std::move(platform_source));
    MediaStreamComponent* component = MakeGarbageCollected<MediaStreamComponentImpl>(source,
        std::make_unique<MediaStreamVideoTrack>(platform_source_ptr, MediaStreamVideoSource::ConstraintsOnceCallback(),
            /*enabled=*/true));
    MediaStreamTrack* track = MakeGarbageCollected<MediaStreamTrackImpl>(v8_scope.GetExecutionContext(), component);
...
{
    V8TestingScope v8_scope;
    MediaStreamTrack* track = MakeGarbageCollected<MediaStreamTrackImpl>(v8_scope.GetExecutionContext(), MakeMockVideoComponent());
...
TEST_F(MediaStreamTrackImplTest, CloningPreservesConstraints)
{
    ...
    auto platform_source
        = std::make_unique<MockMediaStreamVideoSource>(media::VideoCaptureFormat(gfx::Size(1280, 720), 1000.0, media::PIXEL_FORMAT_I420), false);
    MockMediaStreamVideoSource* platform_source_ptr = platform_source.get();
    MediaStreamSource* source
        = MakeGarbageCollected<MediaStreamSource>("id", MediaStreamSource::StreamType::kTypeVideo, "name", false /* remote */, std::move(platform_source));
    auto platform_track
...
TEST_F(MediaStreamTrackImplTest, ApplyConstraintsUpdatesSourceFormat)
{
    ...
    std::tie(component, platform_source_ptr) = MakeMockDisplayVideoCaptureComponent();
    MediaStreamTrack* track = MakeGarbageCollected<MediaStreamTrackImpl>(v8_scope.GetExecutionContext(), component);
    ...
    MediaTrackConstraints* track_constraints = MakeMediaTrackConstraints(kReducedWidth, kReducedHeight, kMinFrameRate, kMaxFrameRate);
    auto apply_constraints_promise = track->applyConstraints(v8_scope.GetScriptState(), track_constraints);

    ScriptPromiseTester tester(v8_scope.GetScriptState(), apply_constraints_promise);
...
TEST_F(MediaStreamTrackImplTest, ApplyConstraintsFramerateDoesNotAffectResolution)
{
    V8TestingScope v8_scope;
    MediaStreamComponent* component;
    MockMediaStreamVideoSource* platform_source_ptr;
    ...
    // Apply new frame rate constraints.
    MediaTrackConstraints* track_constraints = MakeMediaTrackConstraints(std::nullopt, std::nullopt, kMinFrameRate, kMaxFrameRate);
    auto apply_constraints_promise = track->applyConstraints(v8_scope.GetScriptState(), track_constraints);
...
TEST_F(MediaStreamTrackImplTest, ApplyConstraintsResolutionDoesNotAffectFramerate)
{
    V8TestingScope v8_scope;
    MediaStreamComponent* component;
    MockMediaStreamVideoSource* platform_source_ptr;
    ...
    // Apply new frame rate constraints.
    MediaTrackConstraints* track_constraints = MakeMediaTrackConstraints(kReducedWidth, kReducedHeight, std::nullopt, std::nullopt);
    auto apply_constraints_promise = track->applyConstraints(v8_scope.GetScriptState(), track_constraints);
...
TEST_F(MediaStreamTrackImplTest, ApplyConstraintsWidthDoesNotAffectAspectRatio)
{
    V8TestingScope v8_scope;
    MediaStreamComponent* component;
    MockMediaStreamVideoSource* platform_source_ptr;
    ...
    EXPECT_NE(initialHeight, kReducedHeight);
    // Apply new frame rate constraints.
    MediaTrackConstraints* track_constraints = MakeMediaTrackConstraints(kReducedWidth, std::nullopt, std::nullopt, std::nullopt);
    auto apply_constraints_promise = track->applyConstraints(v8_scope.GetScriptState(), track_constraints);
    ...
    // Verify updated settings and that the source was restarted.
    EXPECT_EQ(platform_source_ptr->restart_count(), 1);
    float aspect_ratio = static_cast<float>(initialWidth) / static_cast<float>(initialHeight);
    EXPECT_EQ(platform_source_ptr->max_requested_width(), kReducedWidth);
    EXPECT_EQ(platform_source_ptr->max_requested_height(), kReducedWidth / aspect_ratio);
    EXPECT_EQ(platform_source_ptr->max_requested_frame_rate(), initialFrameRate);
}
...
TEST_F(MediaStreamTrackImplTest, ApplyConstraintsWidthAndAspectRatio)
{
    ...
    EXPECT_NE(initialHeight, kReducedHeight);
    // Apply new frame rate constraints.
    MediaTrackConstraints* track_constraints = MakeMediaTrackConstraints(kReducedWidth, std::nullopt, std::nullopt, std::nullopt, kAspectRatio);
    auto apply_constraints_promise = track->applyConstraints(v8_scope.GetScriptState(), track_constraints);
    ...
    EXPECT_EQ(platform_source_ptr->restart_count(), 1);
    EXPECT_EQ(platform_source_ptr->max_requested_width(), kReducedWidth);
    EXPECT_EQ(platform_source_ptr->max_requested_height(), kReducedWidth / kAspectRatio);
    EXPECT_EQ(platform_source_ptr->max_requested_frame_rate(), initialFrameRate);
}

// cropTo() is not supported on Android.
#if BUILDFLAG(IS_ANDROID)
#define MAYBE_ApplyConstraintsDoesNotUpdateFormatForCroppedSources DISABLED_ApplyConstraintsDoesNotUpdateFormatForCroppedSources
#else
#define MAYBE_ApplyConstraintsDoesNotUpdateFormatForCroppedSources ApplyConstraintsDoesNotUpdateFormatForCroppedSources
#endif

TEST_F(MediaStreamTrackImplTest, MAYBE_ApplyConstraintsDoesNotUpdateFormatForCroppedSources)
{
    V8TestingScope v8_scope;
    MediaStreamComponent* component;
    MockMediaStreamVideoSource* platform_source_ptr;
    ...
    MediaTrackConstraints* track_constraints = MakeMediaTrackConstraints(kReducedWidth, kReducedHeight, kMinFrameRate, kMaxFrameRate);
    EXPECT_CALL(*platform_source_ptr, GetSubCaptureTargetVersion).WillRepeatedly(testing::Return(1));
    auto apply_constraints_promise = track->applyConstraints(v8_scope.GetScriptState(), track_constraints);
...
TEST_F(MediaStreamTrackImplTest, ApplyConstraintsDoesNotUpdateSourceFormatIfDisabled)
{
...
TEST_F(MediaStreamTrackImplTest, ApplyConstraintsWithUnchangedConstraints)
{
    ...
    MediaTrackConstraints* track_constraints = MakeMediaTrackConstraints(initialWidth, initialHeight, initialFrameRate, initialFrameRate);
    auto apply_constraints_promise = track->applyConstraints(v8_scope.GetScriptState(), track_constraints);
...
TEST_F(MediaStreamTrackImplTest, ApplyConstraintsUpdatesMinFps)
{
    ...
    // Apply new constraints.
    MediaTrackConstraints* track_constraints = MakeMediaTrackConstraints(std::nullopt, std::nullopt, kMinFrameRate, initialFrameRate);
    auto apply_constraints_promise = track->applyConstraints(v8_scope.GetScriptState(), track_constraints);
    ScriptPromiseTester tester(v8_scope.GetScriptState(), apply_constraints_promise);
    tester.WaitUntilSettled();
    EXPECT_TRUE(tester.IsFulfilled());
