...
public:
    MockMediaDevicesDispatcherHost()
        : enumeration_({
            ...
            {
                { "fake_video_input_1", "Fake Video Input 1", "common_group_1", media::VideoCaptureControlSupport(), blink::mojom::FacingMode::kNone,
                    media::CameraAvailability::kAvailable },
                { "fake_video_input_2", "Fake Video Input 2", "video_input_group", media::VideoCaptureControlSupport(), blink::mojom::FacingMode::kUser,
                    std::nullopt },
                { "fake_video_input_3", "Fake Video Input 3", "video_input_group 2", media::VideoCaptureControlSupport(), blink::mojom::FacingMode::kUser,
        ...
        })
    {
        ...
        mojom::blink::VideoInputDeviceCapabilitiesPtr capabilities = mojom::blink::VideoInputDeviceCapabilities::New();
        capabilities->device_id = String(enumeration_[1][0].device_id);
        capabilities->group_id = String(enumeration_[1][0].group_id);
        capabilities->facing_mode = enumeration_[1][0].video_facing; // mojom::blink::FacingMode::kNone;
        capabilities->formats.push_back(media::VideoCaptureFormat(gfx::Size(640, 480), 30.0, media::VideoPixelFormat::PIXEL_FORMAT_I420));
        capabilities->availability = static_cast<media::mojom::CameraAvailability>(*enumeration_[1][0].availability);
        ...
        capabilities->formats.push_back(media::VideoCaptureFormat(gfx::Size(640, 480), 30.0, media::VideoPixelFormat::PIXEL_FORMAT_I420));
        capabilities->formats.push_back(media::VideoCaptureFormat(gfx::Size(1920, 1080), 60.0, media::VideoPixelFormat::PIXEL_FORMAT_I420));
        capabilities->facing_mode = enumeration_[1][2].video_facing;
        capabilities->availability = static_cast<media::mojom::CameraAvailability>(*enumeration_[1][2].availability);
        video_input_capabilities_.push_back(std::move(capabilities));
    }
        ...
        bool request_audio_input_capabilities, EnumerateDevicesCallback callback) override
    {
        ...
        if (request_audio_input) {
            wtf_size_t index = static_cast<wtf_size_t>(blink::mojom::blink::MediaDeviceType::kMediaAudioInput);
            enumeration[index] = enumeration_[index];

            if (request_audio_input_capabilities) {
                for (const auto& c : audio_input_capabilities_) {
                    mojom::blink::AudioInputDeviceCapabilitiesPtr capabilities = mojom::blink::AudioInputDeviceCapabilities::New();
                    *capabilities = *c;
        ...
        if (request_video_input) {
            wtf_size_t index = static_cast<wtf_size_t>(blink::mojom::blink::MediaDeviceType::kMediaVideoInput);
            enumeration[index] = enumeration_[index];
    ...
    void SelectAudioOutput(const String& device_id, SelectAudioOutputCallback select_audio_output_callback) override
    {
        mojom::blink::SelectAudioOutputResultPtr result = mojom::blink::SelectAudioOutputResult::New();
    ...
    void SetCaptureHandleConfig(mojom::blink::CaptureHandleConfigPtr config) override
    {
        ...
        EXPECT_EQ(config->all_origins_permitted, expected_config->all_origins_permitted);
        CHECK_EQ(config->permitted_origins.size(), expected_config->permitted_origins.size());
        for (wtf_size_t i = 0; i < config->permitted_origins.size(); ++i) {
            EXPECT_TRUE(config->permitted_origins[i]->IsSameOriginWith(expected_config->permitted_origins[i].get()));
        }
    }
    ...
    void ExpectSetCaptureHandleConfig(mojom::blink::CaptureHandleConfigPtr config)
    {
        CHECK(config);
    ...
    mojo::PendingRemote<mojom::blink::MediaDevicesDispatcherHost> CreatePendingRemoteAndBind()
    {
    ...
    {
        listener()->OnDevicesChanged(MediaDeviceType::kMediaAudioInput, enumeration_[static_cast<wtf_size_t>(MediaDeviceType::kMediaAudioInput)]);
        listener()->OnDevicesChanged(MediaDeviceType::kMediaVideoInput, enumeration_[static_cast<wtf_size_t>(MediaDeviceType::kMediaVideoInput)]);
        listener()->OnDevicesChanged(MediaDeviceType::kMediaAudioOutput, enumeration_[static_cast<wtf_size_t>(MediaDeviceType::kMediaAudioOutput)]);
    }

    Vector<WebMediaDeviceInfo>& AudioInputDevices()
    {
        return enumeration_[static_cast<wtf_size_t>(MediaDeviceType::kMediaAudioInput)];
    }
    Vector<WebMediaDeviceInfo>& VideoInputDevices()
    {
        return enumeration_[static_cast<wtf_size_t>(MediaDeviceType::kMediaVideoInput)];
    }
    Vector<WebMediaDeviceInfo>& AudioOutputDevices()
    {
        return enumeration_[static_cast<wtf_size_t>(MediaDeviceType::kMediaAudioOutput)];
    }

    const Vector<mojom::blink::VideoInputDeviceCapabilitiesPtr>& VideoInputCapabilities()
    {
...
private:
    ...
    Vector<mojom::blink::VideoInputDeviceCapabilitiesPtr> video_input_capabilities_;
    Vector<mojom::blink::AudioInputDeviceCapabilitiesPtr> audio_input_capabilities_;
};
...
void VerifyFacingMode(const Vector<String>& js_facing_mode, blink::mojom::FacingMode cpp_facing_mode)
{
...
void VerifyDeviceInfo(const MediaDeviceInfo* device, const WebMediaDeviceInfo& expected, MediaDeviceType type)
{
...
void VerifyVideoInputCapabilities(
    const MediaDeviceInfo* device, const WebMediaDeviceInfo& expected_device_info, const mojom::blink::VideoInputDeviceCapabilitiesPtr& expected_capabilities)
{
    ...
    if (capabilities->hasFacingMode()) {
        VerifyFacingMode(capabilities->facingMode(), expected_device_info.video_facing);
    }
    ...
    if (expected_device_info.IsAvailable()) {
        ...
        for (const auto& format : expected_capabilities->formats) {
            max_expected_width = std::max(max_expected_width, format.frame_size.width());
            max_expected_height = std::max(max_expected_height, format.frame_size.height());
            max_expected_frame_rate = std::max(max_expected_frame_rate, format.frame_rate);
        }
...
{
    if (CropTarget* crop_target = V8CropTarget::ToWrappable(value.GetIsolate(), value.V8Value())) {
        return crop_target;
    }

    if (RestrictionTarget* restriction_target = V8RestrictionTarget::ToWrappable(value.GetIsolate(), value.V8Value())) {
...
bool ProduceSubCaptureTargetAndGetPromise(V8TestingScope& scope, SubCaptureTarget::Type type, MediaDevices* media_devices, Element* element)
{
    switch (type) {
    case SubCaptureTarget::Type::kCropTarget:
        return !media_devices->ProduceCropTarget(scope.GetScriptState(), element, scope.GetExceptionState()).IsEmpty();

    case SubCaptureTarget::Type::kRestrictionTarget:
        return !media_devices->ProduceRestrictionTarget(scope.GetScriptState(), element, scope.GetExceptionState()).IsEmpty();
    }
}

#if !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS)
void ProduceSubCaptureTargetAndGetTester(
    V8TestingScope& scope, SubCaptureTarget::Type type, MediaDevices* media_devices, Element* element, std::optional<ScriptPromiseTester>& tester)
{
    switch (type) {
    case SubCaptureTarget::Type::kCropTarget:
        tester.emplace(scope.GetScriptState(), media_devices->ProduceCropTarget(scope.GetScriptState(), element, scope.GetExceptionState()));
        return;
    case SubCaptureTarget::Type::kRestrictionTarget:
        tester.emplace(scope.GetScriptState(), media_devices->ProduceRestrictionTarget(scope.GetScriptState(), element, scope.GetExceptionState()));
        return;
    }
}
#endif // !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS)
...
public:
    ...
    MediaDevices* GetMediaDevices(LocalDOMWindow& window)
    {
        if (!media_devices_) {
            ...
            media_devices_->SetDispatcherHostForTesting(dispatcher_host_->CreatePendingRemoteAndBind());
        }
    ...
    bool listener_connection_error() const
    {
        return listener_connection_error_;
    }
    ...
    {
        GetMediaDevices(*GetDocument().domWindow())->addEventListener(event_type_names::kDevicechange, event_listener);
    ...
    void ExpectEnumerateDevicesHistogramReport(EnumerateDevicesResult expected_result)
    {
        histogram_tester_.ExpectTotalCount("Media.MediaDevices.EnumerateDevices.Result", 1);
        histogram_tester_.ExpectUniqueSample("Media.MediaDevices.EnumerateDevices.Result", expected_result, 1);
        histogram_tester_.ExpectTotalCount("Media.MediaDevices.EnumerateDevices.Latency", 1);
    }
...
TEST_F(MediaDevicesTest, GetUserMediaCanBeCalled)
{
    ...
    UserMediaStreamConstraints* constraints = UserMediaStreamConstraints::Create();
    auto promise = GetMediaDevices(scope.GetWindow())->getUserMedia(scope.GetScriptState(), constraints, scope.GetExceptionState());
    ...
    EXPECT_EQ(scope.GetExceptionState().Code(), ToExceptionCode(ESErrorType::kTypeError));
...
TEST_F(MediaDevicesTest, EnumerateDevices)
{
    ...
    ScriptPromiseTester tester(scope.GetScriptState(), media_devices->enumerateDevices(scope.GetScriptState(), scope.GetExceptionState()));
    tester.WaitUntilSettled();
    EXPECT_TRUE(tester.IsFulfilled());

    auto device_infos = NativeValueTraits<IDLArray<MediaDeviceInfo>>::NativeValue(scope.GetIsolate(), tester.Value().V8Value(), scope.GetExceptionState());
    ASSERT_FALSE(scope.GetExceptionState().HadException());

    ExpectEnumerateDevicesHistogramReport(EnumerateDevicesResult::kOk);

    const auto& video_input_capabilities = dispatcher_host().VideoInputCapabilities();
    for (wtf_size_t i = 0, result_index = 0, video_input_index = 0; i < static_cast<wtf_size_t>(MediaDeviceType::kNumMediaDeviceTypes); ++i) {
        for (const auto& expected_device_info : dispatcher_host().enumeration()[i]) {
            testing::Message message;
            message << "Verifying result index " << result_index;
            SCOPED_TRACE(message);
            VerifyDeviceInfo(device_infos[result_index], expected_device_info, static_cast<MediaDeviceType>(i));
            if (i == static_cast<wtf_size_t>(MediaDeviceType::kMediaVideoInput)) {
                VerifyVideoInputCapabilities(device_infos[result_index], expected_device_info, video_input_capabilities[video_input_index]);
                video_input_index++;
            }
            result_index++;
        }
...
TEST_F(MediaDevicesTest, EnumerateDevicesAfterConnectionError)
{
    ...
    ExpectEnumerateDevicesHistogramReport(EnumerateDevicesResult::kErrorMediaDevicesDispatcherHostDisconnected);
}
...
TEST_F(MediaDevicesTest, SetCaptureHandleConfigAfterConnectionError)
{
    ...
    CaptureHandleConfig* input_config = MakeGarbageCollected<CaptureHandleConfig>();
    media_devices->setCaptureHandleConfig(scope.GetScriptState(), input_config, scope.GetExceptionState());
    platform()->RunUntilIdle();
}
...
TEST_F(MediaDevicesTest, ObserveDeviceChangeEvent)
{
    ...
    StrictMock<MockDeviceChangeEventListener>* event_listener = MakeGarbageCollected<StrictMock<MockDeviceChangeEventListener>>();
    AddDeviceChangeListener(event_listener);
    EXPECT_TRUE(dispatcher_host().listener());
    dispatcher_host().listener().set_disconnect_handler(WTF::BindOnce(&MediaDevicesTest::OnListenerConnectionError, WTF::Unretained(this)));
    ...
    dispatcher_host().AudioInputDevices().push_back(WebMediaDeviceInfo("new_fake_audio_input_device", "new_fake_label", "new_fake_group"));
    NotifyDeviceChanges();
    ...
    EXPECT_CALL(*event_listener, Invoke(_, _));
    dispatcher_host().VideoInputDevices().begin()->availability = media::CameraAvailability::kUnavailableExclusivelyUsedByOtherApplication;
    NotifyDeviceChanges();

    // Changing facing mode does not file the event.
    EXPECT_CALL(*event_listener, Invoke(_, _)).Times(0);
    dispatcher_host().VideoInputDevices().begin()->video_facing = blink::mojom::FacingMode::kLeft;
    NotifyDeviceChanges();
    ...
    // event.
    dispatcher_host().AudioInputDevices().push_back(WebMediaDeviceInfo("yet_another_input_device", "yet_another_label", "yet_another_group"));
    NotifyDeviceChanges();
}
...
TEST_F(MediaDevicesTest, SetCaptureHandleConfigCaptureWithOverMaxHandleRejected)
{
    V8TestingScope scope;
    auto* media_devices = GetMediaDevices(*GetDocument().domWindow());
...
TEST_F(MediaDevicesTest, SetCaptureHandleConfigCaptureWithPermittedOriginsWildcard)
{
    V8TestingScope scope;
    auto* media_devices = GetMediaDevices(*GetDocument().domWindow());
...
TEST_F(MediaDevicesTest, SetCaptureHandleConfigCaptureWithPermittedOrigins)
{
    ...
    CaptureHandleConfig* input_config = MakeGarbageCollected<CaptureHandleConfig>();
    input_config->setPermittedOrigins({ "https://chromium.org", "ftp://chromium.org:1234" });
    ...
    expected_config->all_origins_permitted = false;
    expected_config->permitted_origins
        = { SecurityOrigin::CreateFromString("https://chromium.org"), SecurityOrigin::CreateFromString("ftp://chromium.org:1234") };
    dispatcher_host().ExpectSetCaptureHandleConfig(std::move(expected_config));
...
TEST_F(MediaDevicesTest, SetCaptureHandleConfigCaptureWithWildcardAndSomethingElseRejected)
{
    V8TestingScope scope;
    auto* media_devices = GetMediaDevices(*GetDocument().domWindow());
    ...
    ASSERT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ(scope.GetExceptionState().Code(), ToExceptionCode(DOMExceptionCode::kNotSupportedError));
}

TEST_F(MediaDevicesTest, SetCaptureHandleConfigCaptureWithMalformedOriginRejected)
{
    V8TestingScope scope;
    auto* media_devices = GetMediaDevices(*GetDocument().domWindow());
...
TEST_F(MediaDevicesTest, DistinctIdsForDistinctTypes)
{
    ...
    V8TestingScope scope;
    MediaDevices* const media_devices = GetMediaDevices(*GetDocument().domWindow());
    ASSERT_TRUE(media_devices);

    dispatcher_host().SetNextId(SubCaptureTarget::Type::kCropTarget, String("983bf2ff-7410-416c-808a-78421cbd8fdc"));
    dispatcher_host().SetNextId(SubCaptureTarget::Type::kRestrictionTarget, String("70db842e-5326-42c1-86b2-e3b2f74e97d2"));
    ...
    const auto first_promise = media_devices->ProduceCropTarget(scope.GetScriptState(), div, scope.GetExceptionState());
    ...
    const auto second_promise = media_devices->ProduceRestrictionTarget(scope.GetScriptState(), div, scope.GetExceptionState());
    ...
    const WTF::String first_result = ToSubCaptureTarget(first_tester.Value())->GetId();
    ASSERT_FALSE(first_result.empty());

    const WTF::String second_result = ToSubCaptureTarget(second_tester.Value())->GetId();
...
class ProduceSubCaptureTargetTest : public MediaDevicesTest, public testing::WithParamInterface<std::pair<SubCaptureTarget::Type, bool>> {
public:
...
INSTANTIATE_TEST_SUITE_P(_, ProduceSubCaptureTargetTest,
...
TEST_P(ProduceSubCaptureTargetTest, IdUnsupportedOnAndroid)
{
    ...
    dispatcher_host().SetNextId(type_, String(base::Uuid::GenerateRandomV4().AsLowercaseString()));
#endif
    ...
    Document& document = GetDocument();
    Element* const div = document.getElementById(AtomicString("test-div"));
    bool got_promise = ProduceSubCaptureTargetAndGetPromise(scope, type_, media_devices, div);
    platform()->RunUntilIdle();
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_IOS)
...
TEST_P(ProduceSubCaptureTargetTest, IdWithValidElement)
{
    ...
    Document& document = GetDocument();
    static const std::vector<const char*> kElementIds { "test-div", "test-iframe", "test-p", "test-var", "test-sup", "test-wbr", "test-sub", "test-u",
    ...
    for (const char* id : kElementIds) {
        ...
        ProduceSubCaptureTargetAndGetTester(scope, type_, media_devices, element, tester);
        ASSERT_TRUE(tester);
        tester->WaitUntilSettled();
        EXPECT_TRUE(tester->IsFulfilled()) << "Failed promise for element id=" << id;
        EXPECT_FALSE(scope.GetExceptionState().HadException());
    }
}
...
TEST_P(ProduceSubCaptureTargetTest, IdRejectedIfDifferentWindow)
{
    ...
    EXPECT_FALSE(got_promise);
    EXPECT_TRUE(scope.GetExceptionState().HadException());
    EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kNotSupportedError);
    EXPECT_EQ(scope.GetExceptionState().Message(), String("The Element and the MediaDevices object must be same-window."));
}
...
TEST_P(ProduceSubCaptureTargetTest, DuplicateId)
{
    ...
    dispatcher_host().SetNextId(type_, String("983bf2ff-7410-416c-808a-78421cbd8fdc"));

    // This ID should never be encountered.
    dispatcher_host().SetNextId(type_, String("70db842e-5326-42c1-86b2-e3b2f74e97d2"));
    ...
    ProduceSubCaptureTargetAndGetTester(scope, type_, media_devices, div, first_tester);
    ...
    ProduceSubCaptureTargetAndGetTester(scope, type_, media_devices, div, second_tester);
...
TEST_P(ProduceSubCaptureTargetTest, CorrectTokenClassInstantiated)
{
    ...
    EXPECT_EQ(!!V8CropTarget::ToWrappable(value.GetIsolate(), value.V8Value()), type_ == SubCaptureTarget::Type::kCropTarget);
    EXPECT_EQ(!!V8RestrictionTarget::ToWrappable(value.GetIsolate(), value.V8Value()), type_ == SubCaptureTarget::Type::kRestrictionTarget);
}
