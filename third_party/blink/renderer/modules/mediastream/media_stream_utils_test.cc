...
public:
    ScreenSizeTest()
    {
        scoped_feature_list_.InitAndEnableFeature(blink::kGetDisplayMediaScreenScaleFactor);
    }
    FakeChromeClient& GetChromeClient() const override
    {
        return *client_;
    }

protected:
    Persistent<FakeChromeClient> client_ = MakeGarbageCollected<FakeChromeClient>();
...
TEST_F(ScreenSizeTest, Basic)
{
    ...
    EXPECT_EQ(MediaStreamUtils::GetScreenSize(&GetFrame()), gfx::Size(1920, 1200));
}
...
TEST_F(ScreenSizeTest, ScaleFactor)
{
    ...
    client_->AddScreenInfo(screen);
    EXPECT_EQ(MediaStreamUtils::GetScreenSize(&GetFrame()), gfx::Size(1920, 1080));
}
...
TEST_F(ScreenSizeTest, MultiScreen)
{
    ...
    EXPECT_EQ(MediaStreamUtils::GetScreenSize(&GetFrame()), gfx::Size(1920, 2560));
}
...
TEST_F(ScreenSizeTest, MultiScreenScaleFactor)
{
    ...
    screen_2.rect = gfx::Rect(1440, 2560);
    client_->AddScreenInfo(screen_2);
    EXPECT_EQ(MediaStreamUtils::GetScreenSize(&GetFrame()), gfx::Size(3840, 2560));
}
