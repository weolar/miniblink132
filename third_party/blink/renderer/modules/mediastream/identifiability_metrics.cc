...
template <typename T> void Visit(IdentifiableTokenBuilder& builder, const T* range)
{
    if (!range)
        return;
...
void Visit(IdentifiableTokenBuilder& builder, const V8UnionStringOrStringSequence* s)
{
    if (!s) {
        builder.AddToken(IdentifiableToken());
        return;
...
void Visit(IdentifiableTokenBuilder& builder, const V8ConstrainDOMString* s)
{
    ...
    case V8ConstrainDOMString::ContentType::kConstrainDOMStringParameters: {
        const ConstrainDOMStringParameters* params = s->GetAsConstrainDOMStringParameters();
...
void Visit(IdentifiableTokenBuilder& builder, const V8ConstrainBoolean* b)
{
    ...
    case V8ConstrainBoolean::ContentType::kConstrainBooleanParameters: {
        ...
        builder.AddToken(params->hasIdeal() ? params->ideal() : IdentifiableToken());
        return;
    }
    }
    NOTREACHED();
...
void Visit(IdentifiableTokenBuilder& builder, const V8UnionBooleanOrConstrainDouble* x)
{
    if (!x) {
        builder.AddToken(IdentifiableToken());
        return;
...
void Visit(IdentifiableTokenBuilder& builder, const HeapVector<Member<Point2D>>& points)
{
...
void Visit(IdentifiableTokenBuilder& builder, const V8ConstrainPoint2D* p)
{
    ...
    case V8ConstrainPoint2D::ContentType::kConstrainPoint2DParameters: {
        const ConstrainPoint2DParameters* params = p->GetAsConstrainPoint2DParameters();
        if (params->hasExact()) {
...
void Visit(IdentifiableTokenBuilder& builder, const MediaTrackConstraintSet& set)
{
...
void Visit(IdentifiableTokenBuilder& builder, const V8UnionBooleanOrMediaTrackConstraints* constraint)
{
    if (!constraint) {
        builder.AddToken(IdentifiableToken());
        return;
    ...
    case V8UnionBooleanOrMediaTrackConstraints::ContentType::kMediaTrackConstraints: {
        const MediaTrackConstraints* constraints = constraint->GetAsMediaTrackConstraints();
        DCHECK(constraints);
...
IdentifiableToken TokenFromConstraints(const MediaStreamConstraints* constraints)
{
...
void RecordIdentifiabilityMetric(const IdentifiableSurface& surface, ExecutionContext* context, IdentifiableToken token)
{
    if (surface.IsValid() && context && IdentifiabilityStudySettings::Get()->ShouldSampleSurface(surface)) {
        IdentifiabilityMetricBuilder(context->UkmSourceID()).Add(surface, token).Record(context->UkmRecorder());
    }
}
