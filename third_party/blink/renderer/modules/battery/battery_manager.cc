...
// static
ScriptPromise<BatteryManager> BatteryManager::getBattery(ScriptState* script_state, Navigator& navigator)
{
    ...
    if (window->GetFrame()->IsInFencedFrameTree()) {
        return ScriptPromise<BatteryManager>::RejectWithDOMException(script_state,
            DOMException::Create("getBattery is not allowed in a fenced frame tree.", DOMException::GetErrorName(DOMExceptionCode::kNotAllowedError)));
    }
    window->GetFrame()->CountUseIfFeatureWouldBeBlockedByPermissionsPolicy(WebFeature::kBatteryStatusCrossOrigin, WebFeature::kBatteryStatusSameOriginABA);
...
BatteryManager::BatteryManager(Navigator& navigator)
    ...
    , battery_dispatcher_(MakeGarbageCollected<BatteryDispatcher>(navigator.DomWindow()))
{
    UpdateStateIfNeeded();
}

ScriptPromise<BatteryManager> BatteryManager::StartRequest(ScriptState* script_state)
{
    if (!battery_property_) {
        battery_property_ = MakeGarbageCollected<BatteryProperty>(ExecutionContext::From(script_state));
...
void BatteryManager::DidUpdateData()
{
    ...
    if (GetExecutionContext()->IsContextPaused() || GetExecutionContext()->IsContextDestroyed()) {
        return;
    }
...
void BatteryManager::ContextLifecycleStateChanged(mojom::FrameLifecycleState state)
{
...
bool BatteryManager::HasPendingActivity() const
{
    ...
    return HasEventListeners() || (battery_property_ && battery_property_->GetState() == BatteryProperty::kPending);
}
