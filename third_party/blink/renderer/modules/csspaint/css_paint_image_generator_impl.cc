...
CSSPaintImageGenerator* CSSPaintImageGeneratorImpl::Create(const String& name, const Document& document, Observer* observer)
{
    ...
    if (paint_worklet->GetDocumentDefinitionMap().Contains(name)) {
        generator = MakeGarbageCollected<CSSPaintImageGeneratorImpl>(paint_worklet, name);
    } else {
        generator = MakeGarbageCollected<CSSPaintImageGeneratorImpl>(observer, paint_worklet, name);
...
CSSPaintImageGeneratorImpl::CSSPaintImageGeneratorImpl(PaintWorklet* paint_worklet, const String& name)
    : CSSPaintImageGeneratorImpl(nullptr, paint_worklet, name)
{
}

CSSPaintImageGeneratorImpl::CSSPaintImageGeneratorImpl(Observer* observer, PaintWorklet* paint_worklet, const String& name)
    : observer_(observer)
...
scoped_refptr<Image> CSSPaintImageGeneratorImpl::Paint(const ImageResourceObserver& observer, const gfx::SizeF& container_size, const CSSStyleValueVector* data)
{
...
bool CSSPaintImageGeneratorImpl::GetValidDocumentDefinition(DocumentPaintDefinition*& definition) const
{
    ...
    if (definition && definition->GetRegisteredDefinitionCount() != PaintWorklet::kNumGlobalScopesPerThread) {
        definition = nullptr;
    }
    return definition;
}

unsigned CSSPaintImageGeneratorImpl::GetRegisteredDefinitionCountForTesting() const
{
    if (!HasDocumentDefinition())
        return 0;
    DocumentPaintDefinition* definition = paint_worklet_->GetDocumentDefinitionMap().at(name_);
...
const Vector<CSSSyntaxDefinition>& CSSPaintImageGeneratorImpl::InputArgumentTypes() const
{
