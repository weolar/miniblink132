...
protected:
    void SetUp() override
    {
        scoped_composite_clip_path_animation = std::make_unique<ScopedCompositeClipPathAnimationForTest>(true);
        scoped_composite_bgcolor_animation = std::make_unique<ScopedCompositeBGColorAnimationForTest>(false);
        PageTestBase::SetUp();
        GetDocument().GetSettings()->SetAcceleratedCompositingEnabled(true);
    }

private:
    std::unique_ptr<ScopedCompositeClipPathAnimationForTest> scoped_composite_clip_path_animation;
    std::unique_ptr<ScopedCompositeBGColorAnimationForTest> scoped_composite_bgcolor_animation;
};
...
TEST_F(ClipPathPaintDefinitionTest, SimpleClipPathAnimationNotFallback)
{
    ...
    start_keyframe->SetCSSPropertyValue(property_id, "circle(50% at 50% 50%)", SecureContextMode::kInsecureContext, nullptr);
    Persistent<StringKeyframe> end_keyframe = MakeGarbageCollected<StringKeyframe>();
    end_keyframe->SetCSSPropertyValue(property_id, "circle(30% at 30% 30%)", SecureContextMode::kInsecureContext, nullptr);
    ...
    EXPECT_EQ(element->GetElementAnimations()->CompositedClipPathStatus(), CompositedPaintStatus::kComposited);
    EXPECT_EQ(element->GetElementAnimations()->Animations().size(), 1u);
    EXPECT_EQ(ClipPathPaintDefinition::GetAnimationIfCompositable(element), animation);
}
...
TEST_F(ClipPathPaintDefinitionTest, ClipPathAnimationCancel)
{
    ...
    EXPECT_EQ(element->GetElementAnimations()->CompositedClipPathStatus(), CompositedPaintStatus::kNoAnimation);
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(ClipPathPaintDefinitionTest, FallbackOnNonCompositableSecondAnimation)
{
    ...
    Animation* animation2 = Animation::Create(MakeGarbageCollected<KeyframeEffect>(element, model, timing2), timeline, exception_state);
    animation2->play();
    ...
    // means in this case.
    EXPECT_EQ(ClipPathPaintDefinition::GetAnimationIfCompositable(element), nullptr);

    // After adding a second animation with a delay, we gracefully fallback.
    GetDocument().View()->UpdateLifecycleToCompositingInputsClean(DocumentUpdateReason::kTest);
    EXPECT_TRUE(lo->NeedsPaintPropertyUpdate());
    EXPECT_TRUE(lo->ShouldDoFullPaintInvalidation());
    EXPECT_EQ(element->GetElementAnimations()->CompositedClipPathStatus(), CompositedPaintStatus::kNeedsRepaint);
    UpdateAllLifecyclePhasesForTest();
    ...
    EXPECT_FALSE(lo->NeedsPaintPropertyUpdate());
    EXPECT_EQ(element->GetElementAnimations()->CompositedClipPathStatus(), CompositedPaintStatus::kNotComposited);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(lo->FirstFragment().PaintProperties()->ClipPathMask());
}
