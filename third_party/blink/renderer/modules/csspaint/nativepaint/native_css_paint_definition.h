...
public:
    ...
    using ValueFilter = bool (*)(const Element* element, const CSSValue* value, const InterpolableValue* interpolable_value);

    static Animation* GetAnimationForProperty(const Element* element, const CSSProperty& property, ValueFilter filter = DefaultValueFilter);

    static bool CanGetValueFromKeyframe(
        const Element* element, const PropertySpecificKeyframe* frame, const KeyframeEffectModelBase* model, ValueFilter filter);
    ...
    static bool DefaultValueFilter(const Element* element, const CSSValue* value, const InterpolableValue* interpolable_value);

    struct BaseKeyframe {
        BaseKeyframe(double offset, std::unique_ptr<gfx::TimingFunction>& tf)
            : offset(offset)
            , timing_function(tf.release())
        {
        }
        double offset;
    ...
    template <typename T> struct TypedKeyframe : public BaseKeyframe {
        TypedKeyframe(double offset, std::unique_ptr<gfx::TimingFunction>& tf, T v)
            : BaseKeyframe(offset, tf)
            , value(v)
        {
        }
        T value;
...
protected:
    NativeCssPaintDefinition(LocalFrame*, PaintWorkletInput::PaintWorkletInputType);
    NativeCssPaintDefinition() = default;

    template <typename T>
    KeyframeIndexAndProgress ComputeKeyframeIndexAndProgress(const std::optional<double>& main_thread_progress,
        const CompositorPaintWorkletJob::AnimatedPropertyValues& animated_property_values, const Vector<TypedKeyframe<T>>& keyframes)
    {
        ...
        double progress = Progress(main_thread_progress, animated_property_values).value_or(0);
        ...
        double local_progress = (progress - keyframes[result_index].offset) / (keyframes[result_index + 1].offset - keyframes[result_index].offset);
        ...
        // start delay.
        double transformed_progress = keyframes[result_index].timing_function
            ? keyframes[result_index].timing_function->GetValue(local_progress, TimingFunction::LimitDirection::RIGHT)
            : local_progress;
...
private:
    std::optional<double> Progress(
        const std::optional<double>& main_thread_progress, const CompositorPaintWorkletJob::AnimatedPropertyValues& animated_property_values);
};
