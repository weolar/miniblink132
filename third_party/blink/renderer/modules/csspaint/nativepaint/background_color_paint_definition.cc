...
Color InterpolateColor(unsigned index, double progress, const ColorKeyframeVector& keyframes)
{
    ...
    if (first.GetColorSpace() != Color::ColorSpace::kSRGBLegacy || second.GetColorSpace() != Color::ColorSpace::kSRGBLegacy) {
        first.ConvertToColorSpace(Color::ColorSpace::kOklab);
        second.ConvertToColorSpace(Color::ColorSpace::kOklab);
    }

    return Color::InterpolateColors(first.GetColorSpace(), std::nullopt, first, second, progress);
}
...
bool CompositorMayHaveIncorrectDamageRect(const Element* element)
{
    ...
    auto& first_fragment = layout_object->EnclosingLayer()->GetLayoutObject().FirstFragment();
...
public:
    BackgroundColorPaintWorkletInput(const gfx::SizeF& container_size, int worklet_id, ColorKeyframeVector keyframes,
        const std::optional<double>& main_thread_progress, cc::PaintWorkletInput::PropertyKeys property_keys)
    ...
    const ColorKeyframeVector& keyframes() const
    {
        return keyframes_;
    }
    ...
    bool KnownToBeOpaque() const override
    {
        return is_opaque_;
    }
...
Color GetColorFromKeyframe(const PropertySpecificKeyframe* frame, const KeyframeEffectModelBase* model, const Element* element)
{
    if (model->IsStringKeyframeEffectModel()) {
        const CSSValue* value = To<CSSPropertySpecificKeyframe>(frame)->Value();
        const CSSPropertyName property_name = CSSPropertyName(CSSPropertyID::kBackgroundColor);
    ...
    const auto* keyframe = To<TransitionKeyframe::PropertySpecificKeyframe>(frame);
    InterpolableValue* value = keyframe->GetValue()->Value().interpolable_value.Get();
...
void ExtractKeyframes(const Element* element, const Animation* compositable_animation, ColorKeyframeVector& color_keyframes)
{
    ...
    const PropertySpecificKeyframeVector* frames = model->GetPropertySpecificKeyframes(PropertyHandle(GetCSSPropertyBackgroundColor()));
    for (const auto& frame : *frames) {
        ...
        color_keyframes.push_back(ColorKeyframe(offset, timing_function_copy, color));
    }
}

bool ValidateColorValue(const Element* element, const CSSValue* value, const InterpolableValue* interpolable_value)
{
    if (value) {
        if (value->IsIdentifierValue()) {
            ...
            const cssvalue::CSSColorMixValue* color_mix = To<cssvalue::CSSColorMixValue>(value);
            if (!ValidateColorValue(element, &color_mix->Color1(), nullptr) || !ValidateColorValue(element, &color_mix->Color2(), nullptr)) {
...
template <> struct DowncastTraits<BackgroundColorPaintWorkletInput> {
    ...
    {
        return worklet_input.GetType() == PaintWorkletInput::PaintWorkletInputType::kBackgroundColor;
    }
};

Animation* BackgroundColorPaintDefinition::GetAnimationIfCompositable(const Element* element)
{
    if (CompositorMayHaveIncorrectDamageRect(element))
        return nullptr;

    return GetAnimationForProperty(element, GetCSSPropertyBackgroundColor(), ValidateColorValue);
}

// static
BackgroundColorPaintDefinition* BackgroundColorPaintDefinition::Create(LocalFrame& local_root)
{
...
BackgroundColorPaintDefinition::BackgroundColorPaintDefinition(LocalFrame& local_root)
    : NativeCssPaintDefinition(&local_root, PaintWorkletInput::PaintWorkletInputType::kBackgroundColor)
{
}
    ...
    const CompositorPaintWorkletInput* compositor_input, const CompositorPaintWorkletJob::AnimatedPropertyValues& animated_property_values)
{
    ...
    KeyframeIndexAndProgress keyframe_index_and_progress
        = ComputeKeyframeIndexAndProgress(input->MainThreadProgress(), animated_property_values, input->keyframes());

    Color color = InterpolateColor(keyframe_index_and_progress.index, keyframe_index_and_progress.progress, input->keyframes());
...
scoped_refptr<Image> BackgroundColorPaintDefinition::Paint(const gfx::SizeF& container_size, const Node* node)
{
    ...
    CompositorElementId element_id = CompositorElementIdFromUniqueObjectId(
        node->GetLayoutObject()->UniqueId(), CompositorAnimations::CompositorElementNamespaceForProperty(CSSPropertyID::kBackgroundColor));
    CompositorPaintWorkletInput::PropertyKeys input_property_keys;
    input_property_keys.emplace_back(CompositorPaintWorkletInput::NativePropertyType::kBackgroundColor, element_id);

    std::optional<double> main_thread_progress = compositable_animation->effect()->Progress();

    scoped_refptr<BackgroundColorPaintWorkletInput> input = base::MakeRefCounted<BackgroundColorPaintWorkletInput>(
        container_size, worklet_id_, std::move(color_keyframes), main_thread_progress, std::move(input_property_keys));
...
PaintRecord BackgroundColorPaintDefinition::PaintForTest(
    const Vector<Color>& animated_colors, const Vector<double>& offsets, const CompositorPaintWorkletJob::AnimatedPropertyValues& animated_property_values)
{
    ...
    for (unsigned i = 0; i < animated_colors.size(); i++) {
        ...
        color_keyframes.push_back(TypedKeyframe<Color>(offsets[i], tf, animated_colors[i]));
    }

    scoped_refptr<BackgroundColorPaintWorkletInput> input
        = base::MakeRefCounted<BackgroundColorPaintWorkletInput>(container_size, 1u, std::move(color_keyframes), progress, std::move(property_keys));
