...
BackgroundColorPaintImageGenerator* BackgroundColorPaintImageGeneratorImpl::Create(LocalFrame& local_root)
{
    BackgroundColorPaintDefinition* background_color_paint_definition = BackgroundColorPaintDefinition::Create(local_root);
    if (!background_color_paint_definition)
        return nullptr;

    BackgroundColorPaintImageGeneratorImpl* generator = MakeGarbageCollected<BackgroundColorPaintImageGeneratorImpl>(background_color_paint_definition);

    return generator;
}

BackgroundColorPaintImageGeneratorImpl::BackgroundColorPaintImageGeneratorImpl(BackgroundColorPaintDefinition* background_color_paint_definition)
    : background_color_paint_definition_(background_color_paint_definition)
{
}

scoped_refptr<Image> BackgroundColorPaintImageGeneratorImpl::Paint(const gfx::SizeF& container_size, const Node* node)
{
    return background_color_paint_definition_->Paint(container_size, node);
}

Animation* BackgroundColorPaintImageGeneratorImpl::GetAnimationIfCompositable(const Element* element)
{
    ...
    bool background_transfers_to_view = element->GetLayoutBoxModelObject() && element->GetLayoutBoxModelObject()->BackgroundTransfersToView();
    ...
    if (layout_object->IsLayoutTableCol() || layout_object->IsTableRow() || background_transfers_to_view) {
        return nullptr;
    }
