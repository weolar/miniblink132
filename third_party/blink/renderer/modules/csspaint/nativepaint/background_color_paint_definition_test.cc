...
public:
    ...
    void SetUp() override
    {
        EnableCompositing();
        RenderingTest::SetUp();
        FakeBackgroundColorPaintImageGenerator* generator = MakeGarbageCollected<FakeBackgroundColorPaintImageGenerator>();
        GetDocument().GetFrame()->SetBackgroundColorPaintImageGeneratorForTesting(generator);
    }

    // Crash testing of BackgroundColorPaintDefinition::Paint
    void RunPaintForTest(
        const Vector<Color>& animated_colors, const Vector<double>& offsets, const CompositorPaintWorkletJob::AnimatedPropertyValues& property_values)
    {
        BackgroundColorPaintDefinition* definition = MakeGarbageCollected<BackgroundColorPaintDefinition>(BackgroundColorPaintDefinition::KeyForTest());
...
TEST_F(BackgroundColorPaintDefinitionTest, SimpleBGColorAnimationNotFallback)
{
    ...
    Persistent<StringKeyframe> start_keyframe = MakeGarbageCollected<StringKeyframe>();
    start_keyframe->SetCSSPropertyValue(property_id, "red", SecureContextMode::kInsecureContext, nullptr);
    Persistent<StringKeyframe> end_keyframe = MakeGarbageCollected<StringKeyframe>();
    end_keyframe->SetCSSPropertyValue(property_id, "green", SecureContextMode::kInsecureContext, nullptr);
    ...
    Animation* animation = Animation::Create(MakeGarbageCollected<KeyframeEffect>(element, model, timing), timeline, exception_state);
    UpdateAllLifecyclePhasesForTest();
    animation->play();

    EXPECT_TRUE(element->GetElementAnimations());
    EXPECT_EQ(element->GetElementAnimations()->Animations().size(), 1u);
    EXPECT_EQ(element->GetElementAnimations()->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kNeedsRepaint);

    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(element->GetElementAnimations()->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kComposited);
}
...
TEST_F(BackgroundColorPaintDefinitionTest, FallbackWithPixelMovingFilter)
{
    ...
    EXPECT_EQ(BackgroundColorPaintDefinition::GetAnimationIfCompositable(element), animation);
    ...
    EXPECT_EQ(element_animations->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kNeedsRepaint);

    // Run paint.
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(element_animations->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kComposited);
    ...
    inline_style->setProperty(
        parent->GetExecutionContext(), CSSPropertyName(CSSPropertyID::kFilter).ToAtomicString(), "contrast(200%)", "", ASSERT_NO_EXCEPTION);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(BackgroundColorPaintDefinition::GetAnimationIfCompositable(element), animation);
    ...
    inline_style->setProperty(parent->GetExecutionContext(), CSSPropertyName(CSSPropertyID::kFilter).ToAtomicString(), "blur(5px)", "", ASSERT_NO_EXCEPTION);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(BackgroundColorPaintDefinition::GetAnimationIfCompositable(element), nullptr);
    element_animations = element->GetElementAnimations();
    EXPECT_EQ(element_animations->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kNotComposited);

    // Reset.
    inline_style->setProperty(parent->GetExecutionContext(), CSSPropertyName(CSSPropertyID::kFilter).ToAtomicString(), "none", "", ASSERT_NO_EXCEPTION);
    animation->cancel();
    EXPECT_EQ(element_animations->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kNoAnimation);
    animation->play();
    EXPECT_EQ(element_animations->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kNeedsRepaint);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(BackgroundColorPaintDefinition::GetAnimationIfCompositable(element), animation);
...
TEST_F(BackgroundColorPaintDefinitionTest, FallbackToMainCompositeAccumulate)
{
    ...
    EXPECT_EQ(element->GetElementAnimations()->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kNeedsRepaint);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(element->GetElementAnimations()->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kNotComposited);
}
...
TEST_F(BackgroundColorPaintDefinitionTest, MultipleAnimationsFallback)
{
    ...
    NonThrowableExceptionState exception_state;
    DocumentTimeline* timeline = MakeGarbageCollected<DocumentTimeline>(&GetDocument());
    Animation* animation1 = Animation::Create(MakeGarbageCollected<KeyframeEffect>(element, model1, timing), timeline, exception_state);

    start_keyframe->SetCSSPropertyValue(property_id, "blue", SecureContextMode::kInsecureContext, nullptr);
    end_keyframe->SetCSSPropertyValue(property_id, "yellow", SecureContextMode::kInsecureContext, nullptr);
    keyframes.clear();
    keyframes.push_back(start_keyframe);
    keyframes.push_back(end_keyframe);
    auto* model2 = MakeGarbageCollected<StringKeyframeEffectModel>(keyframes);
    Animation* animation2 = Animation::Create(MakeGarbageCollected<KeyframeEffect>(element, model2, timing), timeline, exception_state);
    UpdateAllLifecyclePhasesForTest();
    animation1->play();
    animation2->play();
...
TEST_F(BackgroundColorPaintDefinitionTest, FallbackToMainCurrentColor)
{
    ...
    EXPECT_EQ(element->GetElementAnimations()->Animations().size(), 1u);
    EXPECT_EQ(element->GetElementAnimations()->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kNotComposited);
    EXPECT_FALSE(element->GetElementAnimations()->Animations().begin()->key->HasActiveAnimationsOnCompositor());
}
...
TEST_F(BackgroundColorPaintDefinitionTest, CompositeColorMix)
{
    ...
    EXPECT_EQ(element->GetElementAnimations()->Animations().size(), 1u);
    EXPECT_EQ(element->GetElementAnimations()->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kComposited);
    EXPECT_TRUE(element->GetElementAnimations()->Animations().begin()->key->HasActiveAnimationsOnCompositor());
}
...
// animation is added.
TEST_F(BackgroundColorPaintDefinitionTest, TriggerRepaintCompositedToNonComposited)
{
    ScopedCompositeBGColorAnimationForTest composite_bgcolor_animation(true);
    SetBodyInnerHTML(R"HTML(
    <div id ="target" style="width: 100px; height: 100px">
    ...
    EXPECT_EQ(element->GetElementAnimations()->Animations().size(), 1u);
    style = GetDocument().GetStyleResolver().ResolveStyle(element, style_recalc_context);
...
TEST_F(BackgroundColorPaintDefinitionTest, NotStuckOnKNotComposited)
{
    ...
    Persistent<StringKeyframe> start_keyframe = MakeGarbageCollected<StringKeyframe>();
    start_keyframe->SetCSSPropertyValue(property_id, "red", SecureContextMode::kInsecureContext, nullptr);
    start_keyframe->SetCSSPropertyValue(nc_property_id, "0", SecureContextMode::kInsecureContext, nullptr);
    Persistent<StringKeyframe> end_keyframe = MakeGarbageCollected<StringKeyframe>();
    end_keyframe->SetCSSPropertyValue(property_id, "yellow", SecureContextMode::kInsecureContext, nullptr);
    end_keyframe->SetCSSPropertyValue(nc_property_id, "1", SecureContextMode::kInsecureContext, nullptr);
    ...
    Persistent<StringKeyframe> start_keyframe_2 = MakeGarbageCollected<StringKeyframe>();
    start_keyframe_2->SetCSSPropertyValue(property_id, "red", SecureContextMode::kInsecureContext, nullptr);
    Persistent<StringKeyframe> end_keyframe_2 = MakeGarbageCollected<StringKeyframe>();
    end_keyframe_2->SetCSSPropertyValue(property_id, "yellow", SecureContextMode::kInsecureContext, nullptr);
    keyframes.clear();
...
TEST_F(BackgroundColorPaintDefinitionTest, Rep)
{
    ...
    CSSPropertyID nc_property_id = CSSPropertyID::kTop; // Non-compositable
    Persistent<StringKeyframe> start_keyframe_2 = MakeGarbageCollected<StringKeyframe>();
    start_keyframe_2->SetCSSPropertyValue(nc_property_id, "0", SecureContextMode::kInsecureContext, nullptr);
    Persistent<StringKeyframe> end_keyframe_2 = MakeGarbageCollected<StringKeyframe>();
    end_keyframe_2->SetCSSPropertyValue(nc_property_id, "20", SecureContextMode::kInsecureContext, nullptr);
    keyframes.clear();
    keyframes.push_back(start_keyframe_2);
    keyframes.push_back(end_keyframe_2);
    To<KeyframeEffect>(animation->effect())->SetKeyframes(keyframes);
    ...
    EXPECT_TRUE(element->GetLayoutObject()->ShouldDoFullPaintInvalidation());
    EXPECT_EQ(element->GetElementAnimations()->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kNoAnimation);

    GetDocument().View()->UpdateLifecycleToCompositingInputsClean(DocumentUpdateReason::kTest);
    EXPECT_FALSE(element->ComputedStyleRef().HasCurrentBackgroundColorAnimation());
...
TEST_F(BackgroundColorPaintDefinitionTest, TriggerRepaintNewStartTime)
{
    ...
    CSSPropertyID property_id = CSSPropertyID::kBackgroundColor;
    Persistent<StringKeyframe> start_keyframe = MakeGarbageCollected<StringKeyframe>();
    start_keyframe->SetCSSPropertyValue(property_id, "green", SecureContextMode::kInsecureContext, nullptr);
    Persistent<StringKeyframe> end_keyframe = MakeGarbageCollected<StringKeyframe>();
    end_keyframe->SetCSSPropertyValue(property_id, "green", SecureContextMode::kInsecureContext, nullptr);
    ...
    auto* animation = Animation::Create(effect, &GetDocument().Timeline(), ASSERT_NO_EXCEPTION);
    ...
    animation->setStartTime(MakeGarbageCollected<V8CSSNumberish>(0.5), ASSERT_NO_EXCEPTION);
...
TEST_F(BackgroundColorPaintDefinitionTest, ProxyClientPaintWithNoPropertyValue)
{
    ScopedCompositeBGColorAnimationForTest composite_bgcolor_animation(true);
...
TEST_F(BackgroundColorPaintDefinitionTest, ProxyClientPaintWithNegativeProgress)
{
    ScopedCompositeBGColorAnimationForTest composite_bgcolor_animation(true);
    Vector<Color> animated_colors = { Color(0, 255, 0), Color(255, 0, 0) };
    Vector<double> offsets = { 0, 1 };
    CompositorPaintWorkletJob::AnimatedPropertyValues property_values;
    CompositorPaintWorkletInput::PropertyKey property_key(CompositorPaintWorkletInput::NativePropertyType::kBackgroundColor, CompositorElementId(1u));
...
TEST_F(BackgroundColorPaintDefinitionTest, ProxyClientPaintWithLargerThanOneProgress)
{
    ScopedCompositeBGColorAnimationForTest composite_bgcolor_animation(true);
    Vector<Color> animated_colors = { Color(0, 255, 0), Color(255, 0, 0) };
    Vector<double> offsets = { 0, 1 };
...
TEST_F(BackgroundColorPaintDefinitionTest, ProxyClientPaintWithCloseToOneOffset)
{
    ScopedCompositeBGColorAnimationForTest composite_bgcolor_animation(true);
    Vector<Color> animated_colors = { Color(0, 255, 0), Color(0, 255, 255), Color(255, 0, 0) };
...
TEST_F(BackgroundColorPaintDefinitionTest, ProxyClientPaintWithColorOfDifferingColorSpaces)
{
    ScopedCompositeBGColorAnimationForTest composite_bgcolor_animation(true);
...
// Test that BackgroundColorPaintDefinition::Paint handles colors with
// differing color spaces - i.e won't crash/DCHECK.
TEST_F(BackgroundColorPaintDefinitionTest, ProxyClientPaintWithColorOfDifferingColorSpacesReverse)
{
    ScopedCompositeBGColorAnimationForTest composite_bgcolor_animation(true);
    Vector<Color> animated_colors = {
