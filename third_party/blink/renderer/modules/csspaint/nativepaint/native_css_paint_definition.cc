...
NativeCssPaintDefinition::NativeCssPaintDefinition(LocalFrame* local_root, PaintWorkletInput::PaintWorkletInputType type)
...
Animation* NativeCssPaintDefinition::GetAnimationForProperty(const Element* element, const CSSProperty& property, ValueFilter filter)
{
    ...
    for (const auto& animation : element->GetElementAnimations()->Animations()) {
        if (animation.key->CalculateAnimationPlayState() == V8AnimationPlayState::Enum::kIdle || !animation.key->Affects(*element, property)) {
            continue;
        }
        count++;
    ...
    const PropertySpecificKeyframeVector* frames = model->GetPropertySpecificKeyframes(PropertyHandle(property));
...
bool NativeCssPaintDefinition::DefaultValueFilter(const Element* element, const CSSValue* value, const InterpolableValue* interpolable_value)
{
