...
{
    return SkPath::Rect(gfx::RectFToSkRect(ClipPathPaintImageGenerator::GetAnimationBoundingRect()));
}
...
public:
    ClipPathPaintWorkletInput(const gfx::RectF& reference_box, const gfx::SizeF& clip_area_size, const gfx::PointF& reference_origin, int worklet_id,
        float zoom, const Vector<std::optional<scoped_refptr<BasicShape>>>& animated_shapes, const Vector<double>& offsets,
        Vector<std::unique_ptr<gfx::TimingFunction>> timing_functions, const std::optional<double>& progress, const SkPath static_shape,
        ...
        , dy_(reference_origin.y())
    {
        ...
        for (const auto& basic_shape : animated_shapes) {
            ...
            if (paths_.size() > 0) {
                shape_compatibilities_.push_back((prev_type.has_value() && basic_shape.has_value()) ? (basic_shape->get()->GetType() == *prev_type) : false);
            }
    ...
    const Vector<SkPath>& Paths() const
    {
        return paths_;
    }
    const SkPath StaticPath() const
    {
    ...
    AnimationProgress GetAdjustedProgress(float progress) const
    {
        ...
        float local_progress = (progress - offsets_[result_index]) / (offsets_[result_index + 1] - offsets_[result_index]);
        ...
        return AnimationProgress(result_index, timing_functions_[result_index]->GetValue(local_progress, TimingFunction::LimitDirection::RIGHT));
    }

    bool ValueChangeShouldCauseRepaint(const PropertyValue& val1, const PropertyValue& val2) const override
    {
        return !val1.float_value.has_value() || !val2.float_value.has_value()
            || GetAdjustedProgress(*val1.float_value) != GetAdjustedProgress(*val2.float_value);
    }
...
scoped_refptr<BasicShape> CreateBasicShape(
    BasicShape::ShapeType type, const InterpolableValue& interpolable_value, const NonInterpolableValue& untyped_non_interpolable_value)
{
    if (type == BasicShape::kStylePathType) {
        return PathInterpolationFunctions::AppliedValue(interpolable_value, &untyped_non_interpolable_value);
    }
    CSSToLengthConversionData conversion_data;
    return basic_shape_interpolation_functions::CreateBasicShape(interpolable_value, untyped_non_interpolable_value, conversion_data);
}
...
std::optional<scoped_refptr<BasicShape>> GetAnimatedShapeFromKeyframe(
    const PropertySpecificKeyframe* frame, const KeyframeEffectModelBase* model, const Element* element)
{
    scoped_refptr<BasicShape> basic_shape;
    if (model->IsStringKeyframeEffectModel()) {
        DCHECK(frame->IsCSSPropertySpecificKeyframe());
        const CSSValue* value = static_cast<const CSSPropertySpecificKeyframe*>(frame)->Value();
        const CSSPropertyName property_name = CSSPropertyName(CSSPropertyID::kClipPath);
        const CSSValue* computed_value = StyleResolver::ComputeValue(const_cast<Element*>(element), property_name, *value);
        StyleResolverState state(element->GetDocument(), *const_cast<Element*>(element));

        // TODO(pdr): Support <geometry-box> (alone, or with a shape).
        if (CanExtractShapeOrPath(computed_value)) {
            basic_shape = BasicShapeForValue(state, DynamicTo<CSSValueList>(computed_value)->First());
        } else {
        ...
        const TransitionKeyframe::PropertySpecificKeyframe* keyframe = To<TransitionKeyframe::PropertySpecificKeyframe>(frame);
        const NonInterpolableValue* non_interpolable_value = keyframe->GetValue()->Value().non_interpolable_value.get();
        BasicShape::ShapeType type = PathInterpolationFunctions::IsPathNonInterpolableValue(*non_interpolable_value)
        ...
        basic_shape = CreateBasicShape(type, *keyframe->GetValue()->Value().interpolable_value.Get(), *non_interpolable_value);
    }
    CHECK(basic_shape);
    return basic_shape;
}

bool ValidateClipPathValue(const Element* element, const CSSValue* value, const InterpolableValue* interpolable_value)
{
    if (value) {
        const CSSPropertyName property_name = CSSPropertyName(CSSPropertyID::kClipPath);
        const CSSValue* computed_value = StyleResolver::ComputeValue(const_cast<Element*>(element), property_name, *value);
...
SkPath InterpolatePaths(const bool shapes_are_compatible, const SkPath& from, const SkPath& to, const float progress)
{
...
template <> struct DowncastTraits<ClipPathPaintWorkletInput> {
    ...
    {
        return worklet_input.GetType() == PaintWorkletInput::PaintWorkletInputType::kClipPath;
    }
};
...
// static
Animation* ClipPathPaintDefinition::GetAnimationIfCompositable(const Element* element)
{
    return GetAnimationForProperty(element, GetCSSPropertyClipPath(), ValidateClipPathValue);
}

// static
ClipPathPaintDefinition* ClipPathPaintDefinition::Create(LocalFrame& local_root)
{
...
ClipPathPaintDefinition::ClipPathPaintDefinition(LocalFrame& local_root)
    : NativeCssPaintDefinition(&local_root, PaintWorkletInput::PaintWorkletInputType::kClipPath)
{
}

PaintRecord ClipPathPaintDefinition::Paint(
    const CompositorPaintWorkletInput* compositor_input, const CompositorPaintWorkletJob::AnimatedPropertyValues& animated_property_values)
{
    ...
    if (input->MainThreadProgress().has_value() || !animated_property_values.empty()) {
        float progress = 0;
        ...
        cur_path = InterpolatePaths(input->CanAttemptInterpolation(result_index), paths[result_index], paths[result_index + 1], adjusted_progress);
    }

    cc::InspectablePaintRecorder paint_recorder;
    const gfx::Size clip_area_size(gfx::ToRoundedSize(gfx::RectF(InfiniteIntRect()).size()));
...
scoped_refptr<Image> ClipPathPaintDefinition::Paint(
    float zoom, const gfx::RectF& reference_box, const gfx::SizeF& clip_area_size, const Node& node, int worklet_id)
{
    ...
    const KeyframeEffectModelBase* model = static_cast<const KeyframeEffect*>(effect)->Model();

    const PropertySpecificKeyframeVector* frames = model->GetPropertySpecificKeyframes(PropertyHandle(GetCSSPropertyClipPath()));

    Vector<std::unique_ptr<gfx::TimingFunction>> timing_functions;

    for (const auto& frame : *frames) {
        animated_shapes.push_back(GetAnimatedShapeFromKeyframe(frame, model, element));
    ...
    case Timing::FillMode::FORWARDS: {
        ...
        if (element->GetLayoutObject()->StyleRef().HasClipPath()) {
            ClipPathOperation* static_shape = element->GetLayoutObject()->StyleRef().ClipPath();
            DCHECK_EQ(static_shape->GetType(), ClipPathOperation::kShape);
            Path path = To<ShapeClipPathOperation>(static_shape)->GetPath(reference_size, zoom);
    ...
    CompositorElementId element_id = CompositorElementIdFromUniqueObjectId(
        node.GetLayoutObject()->UniqueId(), CompositorAnimations::CompositorElementNamespaceForProperty(CSSPropertyID::kClipPath));
    CompositorPaintWorkletInput::PropertyKeys input_property_keys;
    input_property_keys.emplace_back(CompositorPaintWorkletInput::NativePropertyType::kClipPath, element_id);
    scoped_refptr<ClipPathPaintWorkletInput> input = base::MakeRefCounted<ClipPathPaintWorkletInput>(reference_size, clip_area_size, reference_box.origin(),
        worklet_id, zoom, animated_shapes, offsets, std::move(timing_functions), progress, static_path, std::move(input_property_keys));
