...
const char PaintWorkletProxyClient::kSupplementName[] = "PaintWorkletProxyClient";
...
// static
PaintWorkletProxyClient* PaintWorkletProxyClient::Create(LocalDOMWindow* window, int worklet_id)
{
    ...
    if (Thread::CompositorThread()) {
        if (WebLocalFrameImpl* local_frame = WebLocalFrameImpl::FromFrame(window->GetFrame())) {
            compositor_paint_dispatcher = local_frame->LocalRootFrameWidget()->EnsureCompositorPaintDispatcher(&compositor_host_queue);
        }
    }
    return MakeGarbageCollected<PaintWorkletProxyClient>(
        worklet_id, paint_worklet, window->GetTaskRunner(TaskType::kInternalDefault), std::move(compositor_paint_dispatcher), std::move(compositor_host_queue));
}

PaintWorkletProxyClient::PaintWorkletProxyClient(int worklet_id, PaintWorklet* paint_worklet, scoped_refptr<base::SingleThreadTaskRunner> main_thread_runner,
    base::WeakPtr<PaintWorkletPaintDispatcher> paint_dispatcher, scoped_refptr<base::SingleThreadTaskRunner> compositor_host_queue)
    : Supplement(nullptr)
...
void PaintWorkletProxyClient::AddGlobalScope(WorkletGlobalScope* global_scope)
{
    ...
    PostCrossThreadTask(*compositor_host_queue_, FROM_HERE,
        CrossThreadBindOnce(
            &PaintWorkletPaintDispatcher::RegisterPaintWorkletPainter, paint_dispatcher_, WrapCrossThreadPersistent(this), global_scope_runner));
}

void PaintWorkletProxyClient::RegisterCSSPaintDefinition(const String& name, CSSPaintDefinition* definition, ExceptionState& exception_state)
{
    if (document_definition_map_.Contains(name)) {
        DocumentPaintDefinition* document_definition = document_definition_map_.at(name);
        ...
        if (!document_definition->RegisterAdditionalPaintDefinition(*definition)) {
            ...
            exception_state.ThrowDOMException(
                DOMExceptionCode::kNotSupportedError, "A class with name:'" + name + "' was registered with a different definition.");
            return;
        }
    } else {
        auto document_definition = std::make_unique<DocumentPaintDefinition>(definition->NativeInvalidationProperties(),
            definition->CustomInvalidationProperties(), definition->InputArgumentTypes(), definition->GetPaintRenderingContext2DSettings()->alpha());
    ...
    if (document_definition->GetRegisteredDefinitionCount() == PaintWorklet::kNumGlobalScopesPerThread) {
        const Vector<AtomicString>& custom_properties = definition->CustomInvalidationProperties();
        ...
        PostCrossThreadTask(*main_thread_runner_, FROM_HERE,
            CrossThreadBindOnce(&PaintWorklet::RegisterMainThreadDocumentPaintDefinition, MakeUnwrappingCrossThreadWeakHandle(paint_worklet_), name,
                definition->NativeInvalidationProperties(), std::move(passed_custom_properties), definition->InputArgumentTypes(),
...
void PaintWorkletProxyClient::Dispose()
{
    if (state_ == RunState::kWorking) {
        PostCrossThreadTask(*compositor_host_queue_, FROM_HERE,
            CrossThreadBindOnce(&PaintWorkletPaintDispatcher::UnregisterPaintWorkletPainter, paint_dispatcher_, worklet_id_));
    }
...
{
    const PaintWorkletInput* worklet_input = To<PaintWorkletInput>(compositor_input);
    PaintDefinition* definition;
    if (worklet_input->GetType() != PaintWorkletInput::PaintWorkletInputType::kCSS) {
    ...
    PaintWorkletGlobalScope* global_scope = global_scopes_[base::RandInt(0, (PaintWorklet::kNumGlobalScopesPerThread)-1)];

    const CSSPaintWorkletInput* input = To<CSSPaintWorkletInput>(compositor_input);
...
void PaintWorkletProxyClient::RegisterForNativePaintWorklet(
    WorkerBackingThread* thread, NativePaintDefinition* definition, PaintWorkletInput::PaintWorkletInputType type)
{
    DCHECK(!native_definitions_.Contains(type));
    native_definitions_.insert(type, definition);
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = thread ? thread->BackingThread().GetTaskRunner() : nullptr;
    ...
    if (compositor_host_queue_) {
        PostCrossThreadTask(*compositor_host_queue_, FROM_HERE,
            CrossThreadBindOnce(&PaintWorkletPaintDispatcher::RegisterPaintWorkletPainter, paint_dispatcher_, WrapCrossThreadPersistent(this), task_runner));
    }
}
...
void ProvidePaintWorkletProxyClientTo(WorkerClients* clients, PaintWorkletProxyClient* client)
{
