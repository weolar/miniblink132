...
public:
    explicit TestPaintWorklet(LocalDOMWindow& window)
        : PaintWorklet(window)
    {
        ResetIsPaintOffThreadForTesting();
    }

    void SetPaintsToSwitch(int num)
    {
        paints_to_switch_ = num;
    }

    int GetPaintsBeforeSwitching() override
    {
        return paints_to_switch_;
    }
    ...
    {
        return (GetActiveGlobalScopeForTesting() + 1) % PaintWorklet::kNumGlobalScopesPerThread;
    }

    size_t GetActiveGlobalScope()
    {
        return GetActiveGlobalScopeForTesting();
    }
...
public:
    void SetUp() override
    {
        ...
        test_paint_worklet_ = MakeGarbageCollected<TestPaintWorklet>(*GetDocument().domWindow());
        proxy_ = test_paint_worklet_->CreateGlobalScope();
    }

    TestPaintWorklet* GetTestPaintWorklet()
    {
        return test_paint_worklet_.Get();
    }
    ...
    void ExpectSwitchGlobalScope(bool expect_switch_within_frame, size_t num_paint_calls, int paint_cnt_to_switch, size_t expected_num_paints_before_switch,
        TestPaintWorklet* paint_worklet_to_test)
    {
        paint_worklet_to_test->DomWindow()->GetFrame()->View()->UpdateAllLifecyclePhasesForTest();
        paint_worklet_to_test->SetPaintsToSwitch(paint_cnt_to_switch);
        size_t previously_selected_global_scope = paint_worklet_to_test->GetActiveGlobalScope();
...
TEST_F(PaintWorkletTest, PaintWithNullPaintArguments)
{
    ...
    ClassicScript::CreateUnspecifiedScript("registerPaint('foo', class { paint() { } });")
        ->RunScriptOnScriptState(global_scope->ScriptController()->GetScriptState());
    ...
    const LayoutObject& layout_object = static_cast<const LayoutObject&>(*observer);
    float zoom = layout_object.StyleRef().EffectiveZoom();
    StylePropertyMapReadOnly* style_map = MakeGarbageCollected<PrepopulatedComputedStylePropertyMap>(
        layout_object.GetDocument(), layout_object.StyleRef(), definition->NativeInvalidationProperties(), definition->CustomInvalidationProperties());
    scoped_refptr<Image> image = PaintGeneratedImage::Create(definition->Paint(container_size, zoom, style_map, nullptr), container_size);
...
{
    PaintWorklet* paint_worklet_to_test = PaintWorklet::From(*GetFrame().GetDocument()->domWindow());
    ...
    CSSPaintImageGeneratorImpl* generator = static_cast<CSSPaintImageGeneratorImpl*>(CSSPaintImageGeneratorImpl::Create("foo", GetDocument(), nullptr));
...
TEST_F(PaintWorkletTest, NativeAndCustomProperties)
{
    ...
    paint_worklet_to_test->RegisterMainThreadDocumentPaintDefinition(
        "foo", native_invalidation_properties, custom_invalidation_properties, Vector<CSSSyntaxDefinition>(), true);

    CSSPaintImageGeneratorImpl* generator = MakeGarbageCollected<CSSPaintImageGeneratorImpl>(paint_worklet_to_test, "foo");
...
class MainOrOffThreadPaintWorkletTest : public PageTestBase, public ::testing::WithParamInterface<bool>, private ScopedOffMainThreadCSSPaintForTest {
public:
...
INSTANTIATE_TEST_SUITE_P(All, MainOrOffThreadPaintWorkletTest, ::testing::Bool());
...
TEST_P(MainOrOffThreadPaintWorkletTest, ConsistentGlobalScopeOnMainThread)
{
    ...
    CSSPaintImageGeneratorImpl* generator_foo = MakeGarbageCollected<CSSPaintImageGeneratorImpl>(observer, paint_worklet_to_test, "foo");
    paint_worklet_to_test->AddPendingGenerator("foo", generator_foo);
    CSSPaintImageGeneratorImpl* generator_bar = MakeGarbageCollected<CSSPaintImageGeneratorImpl>(observer, paint_worklet_to_test, "bar");
    ...
    for (wtf_size_t i = 0; i < PaintWorklet::kNumGlobalScopesPerThread; ++i) {
        ...
        global_scopes.push_back(PaintWorkletGlobalScopeProxy::From(paint_worklet_to_test->GetGlobalScopesForTesting()[i])->global_scope());
    }
    ...
    ClassicScript::CreateUnspecifiedScript(foo0)->RunScriptOnScriptState(global_scopes[0]->ScriptController()->GetScriptState());

    EXPECT_TRUE(global_scopes[0]->FindDefinition("foo"));
    EXPECT_TRUE(paint_worklet_to_test->GetDocumentDefinitionMap().at("foo"));

    ClassicScript::CreateUnspecifiedScript(foo1)->RunScriptOnScriptState(global_scopes[1]->ScriptController()->GetScriptState());
    ...
    ClassicScript::CreateUnspecifiedScript(bar)->RunScriptOnScriptState(global_scopes[0]->ScriptController()->GetScriptState());
    ...
    ClassicScript::CreateUnspecifiedScript(bar)->RunScriptOnScriptState(global_scopes[1]->ScriptController()->GetScriptState());
...
{
    PaintWorklet* paint_worklet_to_test = MakeGarbageCollected<PaintWorklet>(*GetFrame().DomWindow());
    paint_worklet_to_test->ResetIsPaintOffThreadForTesting();

    EXPECT_TRUE(paint_worklet_to_test->GetGlobalScopesForTesting().empty());

    std::unique_ptr<PaintWorkletPaintDispatcher> dispatcher = std::make_unique<PaintWorkletPaintDispatcher>();
    Persistent<PaintWorkletProxyClient> proxy_client = MakeGarbageCollected<PaintWorkletProxyClient>(
        1, paint_worklet_to_test, GetFrame().GetTaskRunner(TaskType::kInternalDefault), dispatcher->GetWeakPtr(), nullptr);
    ...
    if (RuntimeEnabledFeatures::OffMainThreadCSSPaintEnabled()) {
        EXPECT_EQ(paint_worklet_to_test->GetGlobalScopesForTesting().size(), 2 * PaintWorklet::kNumGlobalScopesPerThread);
    } else {
        EXPECT_EQ(paint_worklet_to_test->GetGlobalScopesForTesting().size(), PaintWorklet::kNumGlobalScopesPerThread);
    }
}
...
TEST_F(PaintWorkletTest, ConsistentGlobalScopeCrossThread)
{
    ...
    CSSPaintImageGeneratorImpl* generator_bar = MakeGarbageCollected<CSSPaintImageGeneratorImpl>(observer, paint_worklet_to_test, "bar");
    paint_worklet_to_test->AddPendingGenerator("bar", generator_bar);
    CSSPaintImageGeneratorImpl* generator_loo = MakeGarbageCollected<CSSPaintImageGeneratorImpl>(observer, paint_worklet_to_test, "loo");
    paint_worklet_to_test->AddPendingGenerator("loo", generator_loo);
    CSSPaintImageGeneratorImpl* generator_gar = MakeGarbageCollected<CSSPaintImageGeneratorImpl>(observer, paint_worklet_to_test, "gar");
    ...
    paint_worklet_to_test->RegisterMainThreadDocumentPaintDefinition("foo", definition->NativeInvalidationProperties(), foo_custom_properties,
        definition->InputArgumentTypes(), definition->GetPaintRenderingContext2DSettings()->alpha());

    EXPECT_FALSE(paint_worklet_to_test->GetDocumentDefinitionMap().at("foo"));

    // Definition invalidated by cross thread check
    ClassicScript::CreateUnspecifiedScript(bar0)->RunScriptOnScriptState(global_scopes[0]->ScriptController()->GetScriptState());

    EXPECT_TRUE(global_scopes[0]->FindDefinition("bar"));
    EXPECT_TRUE(paint_worklet_to_test->GetDocumentDefinitionMap().at("bar"));

    ClassicScript::CreateUnspecifiedScript(bar0)->RunScriptOnScriptState(global_scopes[1]->ScriptController()->GetScriptState());
    ...
    paint_worklet_to_test->RegisterMainThreadDocumentPaintDefinition("bar", definition->NativeInvalidationProperties(), bar_custom_properties,
        definition->InputArgumentTypes(), definition->GetPaintRenderingContext2DSettings()->alpha());
    ...
    ClassicScript::CreateUnspecifiedScript(loo0)->RunScriptOnScriptState(global_scopes[0]->ScriptController()->GetScriptState());
    ...
    paint_worklet_to_test->RegisterMainThreadDocumentPaintDefinition("loo", definition->NativeInvalidationProperties(), loo_custom_properties,
        definition->InputArgumentTypes(), definition->GetPaintRenderingContext2DSettings()->alpha());

    EXPECT_TRUE(paint_worklet_to_test->GetDocumentDefinitionMap().at("loo"));

    ClassicScript::CreateUnspecifiedScript(loo1)->RunScriptOnScriptState(global_scopes[1]->ScriptController()->GetScriptState());
    ...
    ClassicScript::CreateUnspecifiedScript(gar0)->RunScriptOnScriptState(global_scopes[0]->ScriptController()->GetScriptState());
    ...
    paint_worklet_to_test->RegisterMainThreadDocumentPaintDefinition("gar", definition->NativeInvalidationProperties(), gar_custom_properties,
        definition->InputArgumentTypes(), definition->GetPaintRenderingContext2DSettings()->alpha());

    EXPECT_FALSE(paint_worklet_to_test->GetDocumentDefinitionMap().at("gar"));

    ClassicScript::CreateUnspecifiedScript(gar0)->RunScriptOnScriptState(global_scopes[1]->ScriptController()->GetScriptState());
...
TEST_F(PaintWorkletTest, GeneratorNotifiedAfterAllRegistrations)
{
    ...
    MockObserver* observer = MakeGarbageCollected<MockObserver>();
    CSSPaintImageGeneratorImpl* generator = MakeGarbageCollected<CSSPaintImageGeneratorImpl>(observer, paint_worklet_to_test, "foo");
    ...
    ClassicScript::CreateUnspecifiedScript(foo)->RunScriptOnScriptState(global_scopes[0]->ScriptController()->GetScriptState());

    EXPECT_TRUE(global_scopes[0]->FindDefinition("foo"));
    EXPECT_TRUE(paint_worklet_to_test->GetDocumentDefinitionMap().at("foo"));

    ClassicScript::CreateUnspecifiedScript(foo)->RunScriptOnScriptState(global_scopes[1]->ScriptController()->GetScriptState());
    ...
    paint_worklet_to_test->RegisterMainThreadDocumentPaintDefinition("foo", definition->NativeInvalidationProperties(), custom_properties,
        definition->InputArgumentTypes(), definition->GetPaintRenderingContext2DSettings()->alpha());
