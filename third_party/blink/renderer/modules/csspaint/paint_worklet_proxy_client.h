...
class MODULES_EXPORT PaintWorkletProxyClient : public GarbageCollected<PaintWorkletProxyClient>, public Supplement<WorkerClients>, public PaintWorkletPainter {
public:
    ...
    PaintWorkletProxyClient(int worklet_id, PaintWorklet*, scoped_refptr<base::SingleThreadTaskRunner> main_thread_runner,
        base::WeakPtr<PaintWorkletPaintDispatcher> compositor_paintee, scoped_refptr<base::SingleThreadTaskRunner> compositor_host_queue);
    ...
    int GetWorkletId() const override
    {
        return worklet_id_;
    }
    PaintRecord Paint(const CompositorPaintWorkletInput*, const CompositorPaintWorkletJob::AnimatedPropertyValues& animated_property_values) override;
    ...
    void RegisterCSSPaintDefinition(const String& name, CSSPaintDefinition*, ExceptionState&);
    ...
    // Hooks for testing.
    const Vector<CrossThreadPersistent<PaintWorkletGlobalScope>>& GetGlobalScopesForTesting() const
    {
        return global_scopes_;
    }
    const HashMap<String, std::unique_ptr<DocumentPaintDefinition>>& DocumentDefinitionMapForTesting() const
    {
        return document_definition_map_;
    }
    scoped_refptr<base::SingleThreadTaskRunner> MainThreadTaskRunnerForTesting() const
    {
        return main_thread_runner_;
    }
    void SetMainThreadTaskRunnerForTesting(scoped_refptr<base::SingleThreadTaskRunner> runner)
    {
    ...
    void RegisterForNativePaintWorklet(WorkerBackingThread* thread, NativePaintDefinition* definition, PaintWorkletInput::PaintWorkletInputType type);
    void UnregisterForNativePaintWorklet();

private:
    friend class PaintWorkletGlobalScopeTest;
    friend class PaintWorkletProxyClientTest;
    FRIEND_TEST_ALL_PREFIXES(PaintWorkletProxyClientTest, PaintWorkletProxyClientConstruction);
    ...
    HashMap<String, std::unique_ptr<DocumentPaintDefinition>> document_definition_map_;
    ...
    HashMap<PaintWorkletInput::PaintWorkletInputType, CrossThreadPersistent<NativePaintDefinition>> native_definitions_;
};

void MODULES_EXPORT ProvidePaintWorkletProxyClientTo(WorkerClients*, PaintWorkletProxyClient*);
