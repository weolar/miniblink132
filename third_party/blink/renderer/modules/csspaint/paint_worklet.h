...
class MODULES_EXPORT PaintWorklet : public Worklet, public Supplement<LocalDOMWindow> {
public:
    ...
    scoped_refptr<Image> Paint(const String& name, const ImageResourceObserver&, const gfx::SizeF& container_size, const CSSStyleValueVector*);

    int WorkletId() const
    {
        return worklet_id_;
    }
    bool IsOffMainThread() const
    {
        return is_paint_off_thread_;
    }
    ...
    typedef HashMap<String, std::unique_ptr<DocumentPaintDefinition>> DocumentDefinitionMap;
    ...
    // for the same |name|.
    void RegisterMainThreadDocumentPaintDefinition(const String& name, Vector<CSSPropertyID> native_properties, Vector<String> custom_properties,
...
protected:
    ...
    wtf_size_t GetActiveGlobalScopeForTesting()
    {
