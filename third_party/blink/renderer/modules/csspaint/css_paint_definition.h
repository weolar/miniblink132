...
// types as well.
class MODULES_EXPORT CSSPaintDefinition final : public GarbageCollected<CSSPaintDefinition>, public NameClient, public PaintDefinition {
public:
    CSSPaintDefinition(ScriptState*, V8NoArgumentConstructor* constructor, V8PaintCallback* paint, const Vector<CSSPropertyID>& native_invalidation_properties,
        const Vector<AtomicString>& custom_invalidation_properties, const Vector<CSSSyntaxDefinition>& input_argument_types,
        const PaintRenderingContext2DSettings*, PaintWorkletGlobalScope*);
    ~CSSPaintDefinition() override;

    // PaintDefinition override
    PaintRecord Paint(const CompositorPaintWorkletInput*, const CompositorPaintWorkletJob::AnimatedPropertyValues&) override;
    ...
    PaintRecord Paint(const gfx::SizeF& container_size, float zoom, StylePropertyMapReadOnly*, const CSSStyleValueVector*);
    ...
    const PaintRenderingContext2DSettings* GetPaintRenderingContext2DSettings() const
    {
