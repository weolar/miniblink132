...
gfx::SizeF GetSpecifiedSize(const gfx::SizeF& size, float zoom)
{
    ...
    auto un_zoom_fn = [un_zoom_factor](float a) -> float { return a * un_zoom_factor; };
    return gfx::SizeF(un_zoom_fn(size.width()), un_zoom_fn(size.height()));
}

} // namespace

CSSPaintDefinition::CSSPaintDefinition(ScriptState* script_state, V8NoArgumentConstructor* constructor, V8PaintCallback* paint,
    const Vector<CSSPropertyID>& native_invalidation_properties, const Vector<AtomicString>& custom_invalidation_properties,
    const Vector<CSSSyntaxDefinition>& input_argument_types, const PaintRenderingContext2DSettings* context_settings, PaintWorkletGlobalScope* global_scope)
    ...
    const CompositorPaintWorkletInput* compositor_input, const CompositorPaintWorkletJob::AnimatedPropertyValues& animated_property_values)
{
    ...
    PaintWorkletStylePropertyMap* style_map = MakeGarbageCollected<PaintWorkletStylePropertyMap>(input->StyleMapData());
    ...
    return Paint(input->GetSize(), input->EffectiveZoom(), style_map, &paint_arguments);
}

PaintRecord CSSPaintDefinition::Paint(
    const gfx::SizeF& container_size, float zoom, StylePropertyMapReadOnly* style_map, const CSSStyleValueVector* paint_arguments)
{
    ...
    auto* rendering_context = MakeGarbageCollected<PaintRenderingContext2D>(
        ToRoundedSize(container_size), context_settings_, zoom, global_scope_->GetTaskRunner(TaskType::kMiscPlatformAPI), global_scope_);
    ...
    // invalid image.
    if (paint_->Invoke(instance_.Get(isolate), rendering_context, paint_size, style_map, *paint_arguments).IsNothing()) {
        return PaintRecord();
    }
...
void CSSPaintDefinition::ApplyAnimatedPropertyOverrides(
    PaintWorkletStylePropertyMap* style_map, const CompositorPaintWorkletJob::AnimatedPropertyValues& animated_property_values)
{
    for (const auto& property_value : animated_property_values) {
        DCHECK(property_value.second.has_value());
        String property_name(property_value.first.custom_property_name.value().c_str());
        DCHECK(style_map->StyleMapData().Contains(property_name));
        CrossThreadStyleValue* old_value = style_map->StyleMapData().at(property_name);
        ...
        case CrossThreadStyleValue::StyleValueType::kUnitType: {
            ...
            std::unique_ptr<CrossThreadUnitValue> new_value
                = std::make_unique<CrossThreadUnitValue>(property_value.second.float_value.value(), DynamicTo<CrossThreadUnitValue>(old_value)->GetUnitType());
        ...
        case CrossThreadStyleValue::StyleValueType::kColorType: {
            ...
            std::unique_ptr<CrossThreadColorValue> new_value
                = std::make_unique<CrossThreadColorValue>(Color::FromSkColor4f(property_value.second.color_value.value()));
            style_map->StyleMapData().Set(property_name, std::move(new_value));
            break;
        }
        default:
