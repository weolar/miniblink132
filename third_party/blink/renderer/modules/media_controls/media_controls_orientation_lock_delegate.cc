...
std::ostream& device::mojom::operator<<(std::ostream& a, ::device::mojom::ScreenOrientationLockType b)
{
    *(int*)1 = 1;
    return a << (int)(b);
}
...
MediaControlsOrientationLockDelegate::MediaControlsOrientationLockDelegate(HTMLVideoElement& video)
    : monitor_(video.GetExecutionContext())
...
void MediaControlsOrientationLockDelegate::Attach()
{
    ...
    GetDocument().addEventListener(event_type_names::kFullscreenchange, this, true);
    VideoElement().addEventListener(event_type_names::kWebkitfullscreenchange, this, true);
    VideoElement().addEventListener(event_type_names::kLoadedmetadata, this, true);
}
...
void MediaControlsOrientationLockDelegate::Detach()
{
    ...
    GetDocument().removeEventListener(event_type_names::kFullscreenchange, this, true);
    VideoElement().removeEventListener(event_type_names::kWebkitfullscreenchange, this, true);
    VideoElement().removeEventListener(event_type_names::kLoadedmetadata, this, true);
}
...
void MediaControlsOrientationLockDelegate::MaybeLockOrientation()
{
    ...
    auto* controller = ScreenOrientationController::From(*GetDocument().domWindow());
    if (controller->MaybeHasActiveLock())
        return;

    locked_orientation_ = ComputeOrientationLock();
    DCHECK_NE(locked_orientation_, device::mojom::blink::ScreenOrientationLockType::DEFAULT);
    controller->lock(locked_orientation_, std::make_unique<DummyScreenOrientationCallback>());
...
void MediaControlsOrientationLockDelegate::ChangeLockToAnyOrientation()
{
    ...
    if (LocalDOMWindow* window = GetDocument().domWindow()) {
        ScreenOrientationController::From(*window)->lock(locked_orientation_, std::make_unique<DummyScreenOrientationCallback>());
    }
}
...
void MediaControlsOrientationLockDelegate::MaybeUnlockOrientation()
{
    ...
    if (locked_orientation_ == device::mojom::blink::ScreenOrientationLockType::DEFAULT /* unlocked */)
        return;

    monitor_.reset(); // Cancel any GotIsAutoRotateEnabledByUser Mojo callback.
    LocalDOMWindow* dom_window = GetDocument().domWindow();
    dom_window->removeEventListener(event_type_names::kDeviceorientation, this, false);
    ScreenOrientationController::From(*dom_window)->unlock();
    locked_orientation_ = device::mojom::blink::ScreenOrientationLockType::DEFAULT /* unlocked */;
...
void MediaControlsOrientationLockDelegate::MaybeListenToDeviceOrientation()
{
    ...
    if (is_auto_rotate_enabled_by_user_override_for_testing_ != std::nullopt) {
        GotIsAutoRotateEnabledByUser(is_auto_rotate_enabled_by_user_override_for_testing_.value());
        return;
    }
...
// Check whether the user locked screen orientation at the OS level.
    ...
    Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(
        monitor_.BindNewPipeAndPassReceiver(GetDocument().GetTaskRunner(TaskType::kMediaElementEvent)));
    monitor_->IsAutoRotateEnabledByUser(WTF::BindOnce(&MediaControlsOrientationLockDelegate::GotIsAutoRotateEnabledByUser, WrapPersistent(this)));
#else
    GotIsAutoRotateEnabledByUser(true); // Assume always enabled on other OSes.
#endif // BUILDFLAG(IS_ANDROID)
}

void MediaControlsOrientationLockDelegate::GotIsAutoRotateEnabledByUser(bool enabled)
{
    monitor_.reset();
    ...
    if (LocalDOMWindow* dom_window = GetDocument().domWindow()) {
        dom_window->addEventListener(event_type_names::kDeviceorientation, this, false);
    }
}
...
void MediaControlsOrientationLockDelegate::Invoke(ExecutionContext* execution_context, Event* event)
{
    if (event->type() == event_type_names::kFullscreenchange || event->type() == event_type_names::kWebkitfullscreenchange) {
    ...
    if (event->type() == event_type_names::kDeviceorientation) {
        if (event->isTrusted() && event->InterfaceName() == event_interface_names::kDeviceOrientationEvent) {
            MaybeLockToAnyIfDeviceOrientationMatchesVideo(To<DeviceOrientationEvent>(event));
        }
...
device::mojom::blink::ScreenOrientationLockType MediaControlsOrientationLockDelegate::ComputeOrientationLock() const
{
...
MediaControlsOrientationLockDelegate::DeviceOrientationType MediaControlsOrientationLockDelegate::ComputeDeviceOrientation(DeviceOrientationData* data) const
{
    LocalDOMWindow* dom_window = GetDocument().domWindow();
    if (!dom_window)
    ...
    double device_orientation_angle = std::fmod(Rad2deg(std::atan2(/* y= */ x, /* x= */ -y)) + 360, 360);
    ...
    int device_orientation_angle90 = static_cast<int>(std::lround(device_orientation_angle / 90) * 90);
    ...
    int screen_orientation_angle = ScreenScreenOrientation::orientation(*dom_window->screen())->angle();

    // This is equivalent to screen.orientation.type.startsWith('landscape').
    bool screen_orientation_is_portrait = dom_window->screen()->width() <= dom_window->screen()->height();
    ...
    bool screen_orientation_is_natural_or_flipped_natural = screen_orientation_angle % 180 == 0;
    bool natural_orientation_is_portrait = screen_orientation_is_portrait == screen_orientation_is_natural_or_flipped_natural;
    ...
    return device_orientation_angle90 % 180 == portrait_angle_mod_180 ? DeviceOrientationType::kPortrait : DeviceOrientationType::kLandscape;
}

void MediaControlsOrientationLockDelegate::MaybeLockToAnyIfDeviceOrientationMatchesVideo(DeviceOrientationEvent* event)
{
    DCHECK_EQ(state_, State::kMaybeLockedFullscreen);
    DCHECK(locked_orientation_ == device::mojom::blink::ScreenOrientationLockType::PORTRAIT
        || locked_orientation_ == device::mojom::blink::ScreenOrientationLockType::LANDSCAPE);

    DeviceOrientationType device_orientation = ComputeDeviceOrientation(event->Orientation());

    DeviceOrientationType video_orientation = locked_orientation_ == device::mojom::blink::ScreenOrientationLockType::PORTRAIT
    ...
    lock_to_any_task_ = PostDelayedCancellableTask(*GetDocument().GetTaskRunner(TaskType::kMediaElementEvent), FROM_HERE,
        ...
        WTF::BindOnce(&MediaControlsOrientationLockDelegate::ChangeLockToAnyOrientation, WrapPersistent(this)), kLockToAnyDelay);
}
