...
MediaControlsSharedHelpers::TransitionEventListener::TransitionEventListener(Element* element, Callback callback)
    : callback_(callback)
    , element_(element)
{
    DCHECK(callback_);
...
void MediaControlsSharedHelpers::TransitionEventListener::Invoke(ExecutionContext* context, Event* event)
{
...
void MediaControlsSharedHelpers::TransitionEventListener::Trace(blink::Visitor* visitor) const
{
    NativeEventListener::Trace(visitor);
    visitor->Trace(element_);
}

std::optional<unsigned> MediaControlsSharedHelpers::GetCurrentBufferedTimeRange(HTMLMediaElement& media_element)
{
    ...
    for (unsigned i = 0; i < buffered_time_ranges->length(); ++i) {
        ...
        if (!std::isnan(start) && !std::isnan(end) && start <= current_time + kCurrentTimeBufferedDelta && end > current_time) {
            return i;
        }
    }
...
bool MediaControlsSharedHelpers::ShouldShowFullscreenButton(const HTMLMediaElement& media_element)
{
    ...
    if (media_element.ControlsListInternal()->ShouldHideFullscreen() && !media_element.UserWantsControlsVisible()) {
        UseCounter::Count(media_element.GetDocument(), WebFeature::kHTMLMediaElementControlsListNoFullscreen);
        return false;
    }
