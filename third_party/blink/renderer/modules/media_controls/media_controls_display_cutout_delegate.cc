...
{
    return RuntimeEnabledFeatures::DisplayCutoutAPIEnabled() && RuntimeEnabledFeatures::MediaControlsExpandGestureEnabled();
}

MediaControlsDisplayCutoutDelegate::MediaControlsDisplayCutoutDelegate(HTMLVideoElement& video_element)
...
void MediaControlsDisplayCutoutDelegate::Attach()
{
    ...
    GetDocument().addEventListener(event_type_names::kWebkitfullscreenchange, this, true);
}
...
void MediaControlsDisplayCutoutDelegate::Detach()
{
    ...
    GetDocument().removeEventListener(event_type_names::kWebkitfullscreenchange, this, true);
}
...
void MediaControlsDisplayCutoutDelegate::DidExitFullscreen()
{
    ...
    video_element_->removeEventListener(event_type_names::kTouchstart, this, true);
    ...
    video_element_->removeEventListener(event_type_names::kTouchcancel, this, true);
}

void MediaControlsDisplayCutoutDelegate::Invoke(ExecutionContext* execution_context, Event* event)
{
...
void MediaControlsDisplayCutoutDelegate::HandleTouchEvent(TouchEvent* event)
{
    ...
    if (previous_.has_value() && previous_->second != direction) {
        ...
        UseCounter::Count(GetDocument(), WebFeature::kMediaControlsDisplayCutoutGesture);
        GetDocument().GetViewportData().SetExpandIntoDisplayCutout(direction == Direction::kExpanding);
    }

    // If we are finishing a touch then clear any stored value, otherwise store
    // the latest distance.
    if (event->type() == event_type_names::kTouchend || event->type() == event_type_names::kTouchcancel) {
