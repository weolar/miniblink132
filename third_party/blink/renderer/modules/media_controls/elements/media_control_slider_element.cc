...
void SetSegmentDivPosition(blink::HTMLDivElement* segment, blink::MediaControlSliderElement::Position position, int width, float zoom_factor)
{
    int segment_width = ClampTo<int>(floor((position.width * width) / zoom_factor));
    ...
    builder.Append("px;");
    segment->setAttribute(blink::html_names::kStyleAttr, builder.ToAtomicString());
}
...
class MediaControlSliderElement::MediaControlSliderElementResizeObserverDelegate final : public ResizeObserver::Delegate {
public:
    explicit MediaControlSliderElementResizeObserverDelegate(MediaControlSliderElement* element)
        : element_(element)
    {
        DCHECK(element);
    }
...
MediaControlSliderElement::MediaControlSliderElement(MediaControlsImpl& media_controls)
    ...
    , resize_observer_(ResizeObserver::Create(GetDocument().domWindow(), MakeGarbageCollected<MediaControlSliderElementResizeObserverDelegate>(this)))
{
...
Element& MediaControlSliderElement::GetTrackElement()
{
    ...
    Element* track = GetShadowRoot()->getElementById(shadow_element_names::kIdSliderTrack);
    DCHECK(track);
    return *track;
}
...
{
    DCHECK((segment_highlight_after_ && segment_highlight_before_) || (!segment_highlight_after_ && !segment_highlight_before_));

    if (segment_highlight_after_ || segment_highlight_before_)
        return;

    Element& track = GetTrackElement();
    track.SetShadowPseudoId(shadow_element_names::kPseudoMediaControlsSegmentedTrack);
    ...
    HTMLDivElement* background = MediaControlElementsHelper::CreateDiv(AtomicString("-internal-track-segment-background"), &track);
    segment_highlight_before_ = MediaControlElementsHelper::CreateDiv(AtomicString("-internal-track-segment-highlight-before"), background);
    segment_highlight_after_ = MediaControlElementsHelper::CreateDiv(AtomicString("-internal-track-segment-highlight-after"), background);
}

void MediaControlSliderElement::SetBeforeSegmentPosition(MediaControlSliderElement::Position position)
{
    DCHECK(segment_highlight_before_);
    before_segment_position_ = position;
    SetSegmentDivPosition(segment_highlight_before_, before_segment_position_, TrackWidth(), ZoomFactor());
}

void MediaControlSliderElement::SetAfterSegmentPosition(MediaControlSliderElement::Position position)
{
    DCHECK(segment_highlight_after_);
    after_segment_position_ = position;
    SetSegmentDivPosition(segment_highlight_after_, after_segment_position_, TrackWidth(), ZoomFactor());
}
