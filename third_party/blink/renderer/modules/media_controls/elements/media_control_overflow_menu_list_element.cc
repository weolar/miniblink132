...
MediaControlOverflowMenuListElement::MediaControlOverflowMenuListElement(MediaControlsImpl& media_controls)
    : MediaControlPopupMenuElement(media_controls)
{
    SetShadowPseudoId(AtomicString("-internal-media-controls-overflow-menu-list"));
...
void MediaControlOverflowMenuListElement::SetIsWanted(bool wanted)
{
    ...
    if (wanted) {
        OpenOverflowMenu();
    } else if (!GetMediaControls().TextTrackListIsWanted() && !GetMediaControls().PlaybackSpeedListIsWanted()) {
        CloseOverflowMenu();
    }
}
