...
public:
    ...
    MODULES_EXPORT static const HTMLMediaElement* ToParentMediaElement(const Node*);

    // Utility function for quickly creating div elements with a predefined shadow
    // ID.
    static HTMLDivElement* CreateDiv(const WTF::AtomicString& id, ContainerNode* parent);
    ...
    MODULES_EXPORT static gfx::Size GetSizeOrDefault(const Element&, const gfx::Size&);

    // Utility function for quickly creating div elements with a predefined
    // element ID.
    static HTMLDivElement* CreateDivWithId(const WTF::AtomicString& id, ContainerNode* parent);
