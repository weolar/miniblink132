...
public:
    ...
    HTMLMediaElement& GetElement() const
    {
...
TEST_F(MediaControlElementsHelperTest, DipSizeUnaffectedByPageZoom)
{
    ...
    EXPECT_EQ(test_size, MediaControlElementsHelper::GetSizeOrDefault(GetElement(), test_size));
...
TEST_F(MediaControlElementsHelperTest, LayoutSizeAffectedByPageZoom)
{
    ...
    gfx::Size test_size(123, 456);
    gfx::Size real_size = MediaControlElementsHelper::GetSizeOrDefault(GetElement(), test_size);
    EXPECT_NE(real_size, test_size);
    GetDocument().GetFrame()->SetLayoutZoomFactor(2.f);
    gfx::Size zoom_size = MediaControlElementsHelper::GetSizeOrDefault(GetElement(), test_size);
