...
public:
    void SetUp() final
    {
        ...
        media_element_->setAttribute(html_names::kPreloadAttr, AtomicString("none"));
...
protected:
    KeyboardEvent* KeyDownEvent(int key_code, Element* target = nullptr, WebInputEvent::Modifiers modifiers = WebInputEvent::kNoModifiers)
    {
    ...
    {
        return *media_controls_->playback_speed_button_->overflow_label_element_.Get();
    }
...
TEST_F(MediaControlPopupMenuElementTest, FocusMovesBackToPopupAnchorOnItemSelectedFromKeyboard)
{
    EXPECT_FALSE(GetPopupMenu().IsWanted());
    GetPopupMenu().SetIsWanted(true);
    EXPECT_TRUE(GetPopupMenu().IsWanted());

    GetDownloadButtonLabel().DispatchEvent(*KeyDownEvent(VKEY_RETURN, &GetDownloadButtonLabel()));
    EXPECT_FALSE(GetPopupMenu().IsWanted());
    EXPECT_EQ(GetPopupAnchor(), GetDocument().FocusedElement());
}

TEST_F(MediaControlPopupMenuElementTest, FocusDoesntMoveBackToPopupAnchorOnItemSelectedFromMouseClick)
{
    EXPECT_FALSE(GetPopupMenu().IsWanted());
    GetPopupMenu().SetIsWanted(true);
    EXPECT_TRUE(GetPopupMenu().IsWanted());
...
TEST_F(MediaControlPopupMenuElementTest, FocusDoesntMoveBackToPopupAnchorOnItemSelectedFromKeyboardButMenuStillOpened)
{
    EXPECT_FALSE(GetPopupMenu().IsWanted());
    GetPopupMenu().SetIsWanted(true);
    EXPECT_TRUE(GetPopupMenu().IsWanted());

    GetPlaybackSpeedButtonLabel().DispatchEvent(*KeyDownEvent(VKEY_RETURN, &GetPlaybackSpeedButtonLabel()));
    EXPECT_FALSE(GetPopupMenu().IsWanted());
