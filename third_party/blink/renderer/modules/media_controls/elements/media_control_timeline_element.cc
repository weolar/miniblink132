...
bool IsValidPointerEvent(const blink::Event& event)
{
    ...
    return pointer_event.isPrimary() && pointer_event.button() == static_cast<int16_t>(blink::WebPointerProperties::Button::kLeft);
}
...
MediaControlTimelineElement::MediaControlTimelineElement(MediaControlsImpl& media_controls)
    : MediaControlSliderElement(media_controls)
    , render_timeline_timer_(GetDocument().GetTaskRunner(TaskType::kInternalMedia), this, &MediaControlTimelineElement::RenderTimelineTimerFired)
{
...
{
    String aria_label = GetLocale().QueryString(IsA<HTMLVideoElement>(MediaElement()) ? IDS_AX_MEDIA_VIDEO_SLIDER_HELP : IDS_AX_MEDIA_AUDIO_SLIDER_HELP);
    ...
    setAttribute(html_names::kAriaValuetextAttr,
        AtomicString(GetLocale().QueryString(IDS_AX_MEDIA_CURRENT_TIME_DISPLAY, GetMediaControls().CurrentTimeDisplay().FormatTime())));
    ...
    setAttribute(html_names::kAriaDescriptionAttr,
        AtomicString(GetLocale().QueryString(
            IDS_AX_MEDIA_TIME_REMAINING_DISPLAY, GetMediaControls().RemainingTimeDisplay().MediaControlTimeDisplayElement::FormatTime())));
}

void MediaControlTimelineElement::SetPosition(double current_time, bool suppress_aria)
{
...
void MediaControlTimelineElement::SetDuration(double duration)
{
    ...
    SetFloatingPointAttribute(html_names::kMaxAttr, is_live_ ? 0.0 : duration_value);
...
void MediaControlTimelineElement::DefaultEventHandler(Event& event)
{
    ...
    if (IsA<MouseEvent>(event) || IsA<KeyboardEvent>(event) || IsA<GestureEvent>(event) || IsA<PointerEvent>(event)) {
    ...
    if (is_touching_ && event.type() == event_type_names::kTouchmove) {
        ...
        double position = max(0.0, fmin(1.0, touch->clientX() / TrackWidth() * ZoomFactor()));
...
{
    return MediaControlElementsHelper::IsUserInteractionEventForSlider(event, GetLayoutObject());
}
...
void MediaControlTimelineElement::MaybeUpdateTimelineInterval()
{
    ...
    double seekable_start = MediaElement().seekable()->start(last_seekable, ASSERT_NO_EXCEPTION);
    double seekable_end = MediaElement().seekable()->end(last_seekable, ASSERT_NO_EXCEPTION);
    double expected_media_time_now = live_anchor_time_->media_time_ + (base::TimeTicks::Now() - live_anchor_time_->clock_time_).InSecondsF();
...
void MediaControlTimelineElement::RenderBarSegments()
{
    ...
    if (is_live_) {
        current_time = Value().ToDouble() - GetFloatingPointAttribute(html_names::kMinAttr);
        duration = GetFloatingPointAttribute(html_names::kMaxAttr) - GetFloatingPointAttribute(html_names::kMinAttr);
    }
    ...
    std::optional<unsigned> current_buffered_time_range = MediaControlsSharedHelpers::GetCurrentBufferedTimeRange(MediaElement());

    if (current_buffered_time_range) {
        float end = buffered_time_ranges->end(current_buffered_time_range.value(), ASSERT_NO_EXCEPTION);
...
{
    if (is_touching_) {
        if (event.type() == event_type_names::kTouchend || event.type() == event_type_names::kTouchcancel || event.type() == event_type_names::kChange) {
            is_touching_ = false;
            return true;
        }
    } else if (event.type() == event_type_names::kPointerup || event.type() == event_type_names::kPointercancel) {
        return IsValidPointerEvent(event);
    }
