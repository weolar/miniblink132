...
class MediaControlPopupMenuElement::EventListener final : public NativeEventListener {
public:
    ...
    void StopListening()
    {
        ...
        popup_menu_->removeEventListener(event_type_names::kBeforetoggle, this, false);
        ...
        if (DOMWindow* outer_window = window->top()) {
            if (outer_window != window) {
                outer_window->removeEventListener(event_type_names::kScroll, this, true);
            }
...
private:
    void Invoke(ExecutionContext*, Event* event) final
    {
        if (event->type() == event_type_names::kKeydown) {
            ...
            case VKEY_TAB:
                keyboard_event->shiftKey() ? popup_menu_->SelectPreviousItem() : popup_menu_->SelectNextItem();
                break;
            case VKEY_UP:
        ...
        } else if (event->type() == event_type_names::kResize || event->type() == event_type_names::kScroll
...
void MediaControlPopupMenuElement::SetIsWanted(bool wanted)
{
    ...
    if (wanted) {
        ...
        if (popoverOpen()) {
            HidePopoverInternal(HidePopoverFocusBehavior::kNone, HidePopoverTransitionBehavior::kNoEventsNoWaiting, nullptr);
        }
    }
}
...
{
    if (event.type() == event_type_names::kPointermove && event.target() != this) {
        To<Element>(event.target()->ToNode())->Focus(FocusParams(FocusTrigger::kUserGesture));
        last_focused_element_ = To<Element>(event.target()->ToNode());
    } else if (event.type() == event_type_names::kFocusout) {
        GetDocument()
            .GetTaskRunner(TaskType::kMediaElementEvent)
            ->PostTask(FROM_HERE, WTF::BindOnce(&MediaControlPopupMenuElement::HideIfNotFocused, WrapWeakPersistent(this)));
    } else if (event.type() == event_type_names::kClick && event.target() != this) {
        ...
        event.stopPropagation();
        event.SetDefaultHandled();
    } else if (event.type() == event_type_names::kFocus && event.target() == this) {
        ...
        if (last_focused_element_) {
            ...
            last_focused_element_->Focus(
                FocusParams(SelectionBehaviorOnFocus::kNone, mojom::blink::FocusType::kNone, nullptr, focus_options, FocusTrigger::kUserGesture));
        }
    }
...
MediaControlPopupMenuElement::MediaControlPopupMenuElement(MediaControlsImpl& media_controls)
...
void MediaControlPopupMenuElement::SetPosition()
{
    ...
    WTF::String bottom_str_value = WTF::String::Number(dom_window->innerHeight() - bounding_client_rect->bottom() + kPopupMenuMarginPx) + kPx;
    WTF::String right_str_value = WTF::String::Number(dom_window->innerWidth() - bounding_client_rect->right() + kPopupMenuMarginPx) + kPx;

    style()->setProperty(dom_window, "bottom", bottom_str_value, kImportant, ASSERT_NO_EXCEPTION);
...
bool MediaControlPopupMenuElement::FocusListItemIfDisplayed(Node* node)
{
    ...
    if (!element->InlineStyle() || !element->InlineStyle()->HasProperty(CSSPropertyID::kDisplay)) {
...
void MediaControlPopupMenuElement::SelectNextItem()
{
    ...
    for (Node* target = focused_element->nextSibling(); target; target = target->nextSibling()) {
        if (FocusListItemIfDisplayed(target))
            break;
    }
}
...
void MediaControlPopupMenuElement::SelectPreviousItem()
{
    ...
    for (Node* target = focused_element->previousSibling(); target; target = target->previousSibling()) {
        if (FocusListItemIfDisplayed(target))
            break;
    }
}
...
{
    if (!GetMediaControls().OverflowMenuIsWanted() && !GetMediaControls().PlaybackSpeedListIsWanted() && !GetMediaControls().TextTrackListIsWanted()) {
        PopupAnchor()->Focus(FocusParams(FocusTrigger::kUserGesture));
    }
}
