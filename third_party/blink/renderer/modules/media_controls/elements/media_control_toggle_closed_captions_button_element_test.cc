...
public:
    void SetUp() final
    {
        ...
        media_element_ = static_cast<HTMLMediaElement*>(GetDocument().body()->firstChild());
        media_controls_ = static_cast<MediaControlsImpl*>(media_element_->GetMediaControls());
        captions_overflow_button_ = MakeGarbageCollected<MediaControlToggleClosedCaptionsButtonElement>(*media_controls_);
    }
...
private:
    ...
    Persistent<MediaControlToggleClosedCaptionsButtonElement> captions_overflow_button_;
};

TEST_F(MediaControlToggleClosedCaptionsButtonElementTest, SubtitleStringMatchesSelectedTrack)
{
    ...
    MediaElement()->addTextTrack(V8TextTrackKind(V8TextTrackKind::Enum::kSubtitles), AtomicString(kEnglishLabel), AtomicString("en"), ASSERT_NO_EXCEPTION);
