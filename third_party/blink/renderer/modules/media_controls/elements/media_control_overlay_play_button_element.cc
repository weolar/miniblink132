...
MediaControlOverlayPlayButtonElement::MediaControlOverlayPlayButtonElement(MediaControlsImpl& media_controls)
    ...
    , internal_button_(nullptr)
{
    ...
    internal_button_ = MediaControlElementsHelper::CreateDiv(AtomicString("-internal-media-controls-overlay-play-button-internal"), GetShadowRoot());
}
...
void MediaControlOverlayPlayButtonElement::UpdateDisplayType()
{
    ...
    int state = MediaElement().paused() ? IDS_AX_MEDIA_PLAY_BUTTON : IDS_AX_MEDIA_PAUSE_BUTTON;
    setAttribute(html_names::kAriaLabelAttr, WTF::AtomicString(GetLocale().QueryString(state)));
...
void MediaControlOverlayPlayButtonElement::MaybePlayPause()
{
    if (MediaElement().paused()) {
        ...
        Platform::Current()->RecordAction(UserMetricsAction("Media.Controls.PauseOverlay"));
    }
...
bool MediaControlOverlayPlayButtonElement::KeepEventInNode(const Event& event) const
{
...
gfx::Size MediaControlOverlayPlayButtonElement::GetSizeOrDefault() const
{
    ...
    // button.
    return MediaControlElementsHelper::GetSizeOrDefault(*internal_button_, gfx::Size(kInnerButtonSize, kInnerButtonSize));
}
