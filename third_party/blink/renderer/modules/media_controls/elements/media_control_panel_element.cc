...
MediaControlPanelElement::MediaControlPanelElement(MediaControlsImpl& media_controls)
...
void MediaControlPanelElement::EnsureTransitionEventListener()
{
    ...
    if (!event_listener_) {
        event_listener_ = MakeGarbageCollected<MediaControlsSharedHelpers::TransitionEventListener>(
            this, WTF::BindRepeating(&MediaControlPanelElement::HandleTransitionEndEvent, WrapWeakPersistent(this)));
    }
...
{
    return GetMediaControls().ShouldShowAudioControls() && MediaControlElementsHelper::IsUserInteractionEvent(event);
}
