...
// static
bool MediaControlInputElement::ShouldRecordDisplayStates(const HTMLMediaElement& media_element)
{
    ...
    if (media_element.EffectivePreloadType() == WebMediaPlayer::Preload::kPreloadNone) {
        return true;
    }
...
HTMLElement* MediaControlInputElement::CreateOverflowElement(MediaControlInputElement* button)
{
    if (!button)
        return nullptr;
    ...
    overflow_label_element_ = MakeGarbageCollected<HTMLLabelElement>(GetDocument());
    overflow_label_element_->SetShadowPseudoId(AtomicString("-internal-media-controls-overflow-menu-list-item"));
    overflow_label_element_->setAttribute(html_names::kRoleAttr, AtomicString("menuitem"));
    ...
    overflow_menu_container_ = MakeGarbageCollected<HTMLDivElement>(GetDocument());
    overflow_menu_container_->ParserAppendChild(overflow_menu_text_);
    overflow_menu_container_->setAttribute(html_names::kAriaHiddenAttr, keywords::kTrue);
    ...
    overflow_label_element_->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone);
...
void MediaControlInputElement::UpdateOverflowSubtitleElement(String text)
{
    ...
    if (overflow_menu_subtitle_) {
        ...
        overflow_menu_subtitle_ = MakeGarbageCollected<HTMLSpanElement>(GetDocument());
        overflow_menu_subtitle_->setInnerText(text);
        overflow_menu_subtitle_->setAttribute(html_names::kClassAttr, AtomicString(kOverflowSubtitleCSSClass));

        overflow_menu_container_->ParserAppendChild(overflow_menu_subtitle_);
        overflow_menu_container_->setAttribute(html_names::kClassAttr, AtomicString(kOverflowContainerWithSubtitleCSSClass));
    }
...
void MediaControlInputElement::UpdateOverflowLabelAriaLabel(String subtitle)
{
    ...
    overflow_label_element_->setAttribute(html_names::kAriaLabelAttr, WTF::AtomicString(full_aria_label));
}
...
MediaControlInputElement::MediaControlInputElement(MediaControlsImpl& media_controls)
...
void MediaControlInputElement::UpdateShownState()
{
    if (is_overflow_element_) {
        ...
        if (IsWanted() && DoesFit()) {
            ...
            parent->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone);
        }
    }
...
{
    if (!IsDisabled() && (event.type() == event_type_names::kClick || event.type() == event_type_names::kGesturetap)) {
...
{
    base::UmaHistogramEnumeration(base::StrCat({ "Media.Controls.CTR.", GetNameForHistograms() }), event);
}

void MediaControlInputElement::SetClass(const String& class_name, bool should_have_class)
{
...
gfx::Size MediaControlInputElement::GetSizeOrDefault() const
{
    if (IsControlPanelButton()) {
        return MediaControlElementsHelper::GetSizeOrDefault(*this, gfx::Size(kDefaultButtonSize, kDefaultButtonSize));
    }
