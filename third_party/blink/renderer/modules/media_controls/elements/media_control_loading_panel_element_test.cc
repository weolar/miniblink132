...
public:
    void SetUp() final
    {
        ...
        loading_element_ = MakeGarbageCollected<MediaControlLoadingPanelElement>(*media_controls_);
    }

protected:
    void ExpectStateIsHidden()
    {
        EXPECT_EQ(MediaControlLoadingPanelElement::kHidden, loading_element_->state_);
        CheckIsHidden();
    }

    void ExpectStateIsPlaying()
    {
        EXPECT_EQ(MediaControlLoadingPanelElement::kPlaying, loading_element_->state_);
        CheckIsShown();
    }

    void ExpectStateIsCoolingDown()
    {
        EXPECT_EQ(MediaControlLoadingPanelElement::kCoolingDown, loading_element_->state_);
        CheckIsShown();
    }

    void SimulateLoadingMetadata()
    {
        SetMediaElementState(HTMLMediaElement::kHaveNothing, HTMLMediaElement::kNetworkLoading);
        EXPECT_EQ(media_controls_->State(), MediaControlsImpl::kLoadingMetadataPaused);
        loading_element_->UpdateDisplayState();
    }

    void SimulateBuffering()
    {
        SetMediaElementState(HTMLMediaElement::kHaveCurrentData, HTMLMediaElement::kNetworkLoading, false);
    ...
    {
        SetMediaElementState(HTMLMediaElement::kHaveCurrentData, HTMLMediaElement::kNetworkIdle);
    ...
    {
        SetMediaElementState(HTMLMediaElement::kHaveCurrentData, HTMLMediaElement::kNetworkIdle, false);
    ...
    {
        SetMediaElementState(HTMLMediaElement::kHaveNothing, HTMLMediaElement::kNetworkNoSource);
    ...
    void SimulateAnimationEnd()
    {
        TriggerEvent(event_type_names::kAnimationend);
    }

    void SimulateControlsHidden()
    {
        loading_element_->OnControlsHidden();
    }

    void SimulateControlsShown()
    {
        loading_element_->OnControlsShown();
    }
...
private:
    void SetMediaElementState(HTMLMediaElement::ReadyState ready_state, HTMLMediaElement::NetworkState network_state, bool paused = true)
    {
    ...
    {
        EXPECT_EQ(value, element->style()->getPropertyValue("animation-iteration-count"));
    }
