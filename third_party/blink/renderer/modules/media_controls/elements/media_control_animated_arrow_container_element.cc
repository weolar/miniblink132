...
MediaControlAnimatedArrowContainerElement::AnimatedArrow::AnimatedArrow(const AtomicString& id, Document& document)
...
{
    DCHECK(!hidden_);
    svg_container_->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone);
    hidden_ = true;
}
...
void MediaControlAnimatedArrowContainerElement::AnimatedArrow::OnAnimationIteration()
{
    counter_--;
...
Element& MediaControlAnimatedArrowContainerElement::AnimatedArrow::WatchedAnimationElement() const
{
    return *last_arrow_;
}

void MediaControlAnimatedArrowContainerElement::AnimatedArrow::Trace(Visitor* visitor) const
{
...
MediaControlAnimatedArrowContainerElement::MediaControlAnimatedArrowContainerElement(MediaControlsImpl& media_controls)
    : MediaControlDivElement(media_controls)
    , left_jump_arrow_(nullptr)
    , right_jump_arrow_(nullptr)
{
    EnsureUserAgentShadowRoot();
    SetShadowPseudoId(AtomicString("-internal-media-controls-animated-arrow-container"));
}

void MediaControlAnimatedArrowContainerElement::ShowArrowAnimation(MediaControlAnimatedArrowContainerElement::ArrowDirection direction)
{
    ...
    if (!left_jump_arrow_) {
        ...
        style->setTextContent(MediaControlsResourceLoader::GetAnimatedArrowStyleSheet());
        shadow_root->ParserAppendChild(style);

        left_jump_arrow_ = MakeGarbageCollected<MediaControlAnimatedArrowContainerElement::AnimatedArrow>(AtomicString("left-arrow"), GetDocument());
        shadow_root->ParserAppendChild(left_jump_arrow_);

        right_jump_arrow_ = MakeGarbageCollected<MediaControlAnimatedArrowContainerElement::AnimatedArrow>(AtomicString("right-arrow"), GetDocument());
        shadow_root->ParserAppendChild(right_jump_arrow_);
    }

    DCHECK(left_jump_arrow_ && right_jump_arrow_);

    if (direction == MediaControlAnimatedArrowContainerElement::ArrowDirection::kLeft) {
