...
MediaControlPictureInPictureButtonElement::MediaControlPictureInPictureButtonElement(MediaControlsImpl& media_controls)
    : MediaControlInputElement(media_controls)
{
    setType(input_type_names::kButton);
    setAttribute(html_names::kRoleAttr, AtomicString("button"));

    bool isInPictureInPicture = PictureInPictureController::IsElementInPictureInPicture(&To<HTMLVideoElement>(MediaElement()));

    UpdateAriaString(isInPictureInPicture);

    SetShadowPseudoId(AtomicString("-internal-media-controls-picture-in-picture-button"));
    SetIsWanted(false);
}
...
int MediaControlPictureInPictureButtonElement::GetOverflowStringId() const
{
    ...
    return isInPictureInPicture ? IDS_MEDIA_OVERFLOW_MENU_EXIT_PICTURE_IN_PICTURE : IDS_MEDIA_OVERFLOW_MENU_ENTER_PICTURE_IN_PICTURE;
}
...
{
    if (event.type() == event_type_names::kClick || event.type() == event_type_names::kGesturetap) {
        PictureInPictureController& controller = PictureInPictureController::From(MediaElement().GetDocument());
...
void MediaControlPictureInPictureButtonElement::UpdateAriaString(bool isInPictureInPicture)
{
    String aria_string = isInPictureInPicture ? GetLocale().QueryString(IDS_AX_MEDIA_EXIT_PICTURE_IN_PICTURE_BUTTON)
                                              : GetLocale().QueryString(IDS_AX_MEDIA_ENTER_PICTURE_IN_PICTURE_BUTTON);
