...
public:
    void SetUp() final
    {
        ...
        PageTestBase::SetUp();
        arrow_element_ = MakeGarbageCollected<MediaControlAnimatedArrowContainerElement::AnimatedArrow>(AtomicString("test"), GetDocument());
        GetDocument().body()->AppendChild(arrow_element_);
    }

protected:
    void ExpectNotPresent()
    {
        EXPECT_FALSE(SVGElementIsPresent());
    }
...
private:
    ...
    {
        return GetElementById(AtomicString("jump"))->InlineStyle()->HasProperty(CSSPropertyID::kDisplay);
    }

    bool SVGElementIsPresent()
    {
    ...
    Persistent<MediaControlAnimatedArrowContainerElement::AnimatedArrow> arrow_element_;
};
