...
const QualifiedName& TrackIndexAttrName()
{
    ...
    // track.
    DEFINE_STATIC_LOCAL(QualifiedName, track_index_attr, (AtomicString("data-track-index")));
...
MediaControlTextTrackListElement::MediaControlTextTrackListElement(MediaControlsImpl& media_controls)
    : MediaControlPopupMenuElement(media_controls)
{
    setAttribute(html_names::kRoleAttr, AtomicString("menu"));
    setAttribute(html_names::kAriaLabelAttr, WTF::AtomicString(GetLocale().QueryString(IDS_MEDIA_OVERFLOW_MENU_CLOSED_CAPTIONS_SUBMENU_TITLE)));
...
void MediaControlTextTrackListElement::DefaultEventHandler(Event& event)
{
    if (event.type() == event_type_names::kClick) {
        ...
        int track_index = To<Element>(target)->GetIntegralAttribute(TrackIndexAttrName());
        if (track_index != kTrackIndexOffValue) {
            DCHECK_GE(track_index, 0);
            GetMediaControls().GetTextTrackManager().ShowTextTrackAtIndex(track_index);
...
Element* MediaControlTextTrackListElement::CreateTextTrackListItem(TextTrack* track)
{
    ...
    track_item->SetShadowPseudoId(AtomicString("-internal-media-controls-text-track-list-item"));
    auto* track_item_input = MakeGarbageCollected<HTMLInputElement>(GetDocument());
    track_item_input->SetShadowPseudoId(AtomicString("-internal-media-controls-text-track-list-item-input"));
    ...
    // contents twice.
    String track_label = GetMediaControls().GetTextTrackManager().GetTextTrackLabel(track);
    ...
    track_item->setAttribute(html_names::kAriaLabelAttr, WTF::AtomicString(track_label));
    ...
    if (track && (track->label().empty() || HasDuplicateLabel(track))) {
        auto* track_kind_marker = MakeGarbageCollected<HTMLSpanElement>(GetDocument());
        if (track->kind() == track->CaptionsKeyword()) {
            track_kind_marker->SetShadowPseudoId(AtomicString("-internal-media-controls-text-track-list-kind-captions"));
        } else if (track->kind() == track->DescriptionsKeyword()) {
            track_kind_marker->SetShadowPseudoId(AtomicString("-internal-media-controls-text-track-list-kind-descriptions"));
        } else {
            // Aside from Captions and Descriptions, Subtitles is the only other
            // supported keyword.
            DCHECK_EQ(track->kind(), track->SubtitlesKeyword());
            track_kind_marker->SetShadowPseudoId(AtomicString("-internal-media-controls-text-track-list-kind-subtitles"));
        }
...
Element* MediaControlTextTrackListElement::CreateTextTrackHeaderItem()
{
    ...
    header_item->SetShadowPseudoId(AtomicString("-internal-media-controls-text-track-list-header"));
    header_item->ParserAppendChild(Text::Create(GetDocument(), GetLocale().QueryString(IDS_MEDIA_OVERFLOW_MENU_CLOSED_CAPTIONS_SUBMENU_TITLE)));
...
void MediaControlTextTrackListElement::RefreshTextTrackListMenu()
{
    ...
    off_track->setAttribute(html_names::kAriaSetsizeAttr, WTF::AtomicString::Number(track_list->length() + 1));
    off_track->setAttribute(html_names::kAriaPosinsetAttr, WTF::AtomicString::Number(1));
    off_track->setAttribute(html_names::kRoleAttr, AtomicString("menuitemcheckbox"));
    ...
    for (unsigned i = 0; i < track_list->length(); i++) {
        ...
        track_item->setAttribute(html_names::kAriaSetsizeAttr, WTF::AtomicString::Number(track_list->length() + 1));
        ...
        track_item->setAttribute(html_names::kAriaPosinsetAttr, WTF::AtomicString::Number(i + 2));
        track_item->setAttribute(html_names::kRoleAttr, AtomicString("menuitemcheckbox"));
