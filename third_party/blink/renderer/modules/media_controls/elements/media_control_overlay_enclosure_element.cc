...
MediaControlOverlayEnclosureElement::MediaControlOverlayEnclosureElement(MediaControlsImpl& media_controls)
...
void MediaControlOverlayEnclosureElement::DefaultEventHandler(Event& event)
{
    ...
    // needs to be shown.
    if (event.type() == event_type_names::kGesturetap || event.type() == event_type_names::kClick) {
