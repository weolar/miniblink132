...
public:
    void SetUp() final
    {
        ...
        panel_element_ = MakeGarbageCollected<MediaControlPanelElement>(*media_controls_);
    }
...
protected:
    ...
    void ExpectPanelIsDisplayed()
    {
        EXPECT_TRUE(GetPanel().IsWanted());
    }

    void ExpectPanelIsNotDisplayed()
    {
        EXPECT_FALSE(GetPanel().IsWanted());
    }

    void EventListenerNotCreated()
    {
        EXPECT_FALSE(GetPanel().event_listener_);
    }
    ...
    MediaControlPanelElement& GetPanel()
    {
        return *panel_element_.Get();
    }
...
{
    auto* child_div = MakeGarbageCollected<HTMLDivElement>(GetPanel().GetDocument());
...
{
    EXPECT_TRUE(GetMediaElement().GetMediaControls()->PanelElement()->isConnected());
    GetMediaElement().remove();
    EXPECT_FALSE(GetMediaElement().GetMediaControls()->PanelElement()->isConnected());
}
