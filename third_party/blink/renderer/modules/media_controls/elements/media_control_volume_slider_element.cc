...
class MediaControlVolumeSliderElement::WheelEventListener : public NativeEventListener {
public:
    WheelEventListener(MediaControlVolumeSliderElement* volume_slider, MediaControlVolumeControlContainerElement* container)
...
MediaControlVolumeSliderElement::MediaControlVolumeSliderElement(MediaControlsImpl& media_controls, MediaControlVolumeControlContainerElement* container)
    : MediaControlSliderElement(media_controls)
    , wheel_event_listener_(MakeGarbageCollected<WheelEventListener>(this, container))
{
...
void MediaControlVolumeSliderElement::SetVolumeInternal(double volume)
{
    ...
    setAttribute(html_names::kAriaValuenowAttr, WTF::AtomicString::Number(percent_vol));
}

bool MediaControlVolumeSliderElement::KeepEventInNode(const Event& event) const
{
...
void MediaControlVolumeSliderElement::OnWheelEvent(WheelEvent* wheel_event)
{
    ...
    double new_volume = (wheel_event->wheelDelta() > 0) ? current_volume + kScrollVolumeDelta : current_volume - kScrollVolumeDelta;
