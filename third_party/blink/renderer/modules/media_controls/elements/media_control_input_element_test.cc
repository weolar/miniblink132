...
const char* kControlInputElementHistogramName = "Media.Controls.CTR.MediaControlInputElementImpl";
const char* kControlInputElementOverflowHistogramName = "Media.Controls.CTR.MediaControlInputElementImplOverflow";
...
protected:
    ...
    {
        return IsOverflowElement() ? "MediaControlInputElementImplOverflow" : "MediaControlInputElementImpl";
    }
...
public:
    void SetUp() final
    {
        ...
        control_input_element_ = MakeGarbageCollected<MediaControlInputElementImpl>(*media_controls_);
    }
...
TEST_F(MediaControlInputElementTest, ClickRecordsInteraction)
{
    ...
    ControlInputElement().DispatchSimulatedClick(Event::CreateBubble(event_type_names::kClick));
...
TEST_F(MediaControlInputElementTest, OverflowElement_DisplayFallback)
{
    ...
    Persistent<HTMLElement> overflow_container
        = ControlInputElement().CreateOverflowElement(MakeGarbageCollected<MediaControlInputElementImpl>(MediaControls()));
    ...
    histogram_tester_.ExpectTotalCount(kControlInputElementHistogramName, 0);
    histogram_tester_.ExpectUniqueSample(kControlInputElementOverflowHistogramName, 0, 1);
}
...
TEST_F(MediaControlInputElementTest, OverflowElement_DisplayRequiresWanted)
{
    ...
    histogram_tester_.ExpectTotalCount(kControlInputElementHistogramName, 0);
    histogram_tester_.ExpectTotalCount(kControlInputElementOverflowHistogramName, 0);
}
...
TEST_F(MediaControlInputElementTest, ShouldRecordDisplayStates_ReadyState)
{
    ...
    EXPECT_FALSE(MediaControlInputElement::ShouldRecordDisplayStates(MediaElement()));

    SetReadyState(HTMLMediaElement::kHaveMetadata);
    EXPECT_TRUE(MediaControlInputElement::ShouldRecordDisplayStates(MediaElement()));
...
TEST_F(MediaControlInputElementTest, ShouldRecordDisplayStates_Preload)
{
    ...
    MediaElement().setAttribute(html_names::kPreloadAttr, AtomicString("preload"));
    EXPECT_FALSE(MediaControlInputElement::ShouldRecordDisplayStates(MediaElement()));
