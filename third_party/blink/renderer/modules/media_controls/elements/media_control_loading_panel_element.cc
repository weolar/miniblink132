...
{
    return controls.State() == blink::MediaControlsImpl::kLoadingMetadataPaused || controls.State() == blink::MediaControlsImpl::kLoadingMetadataPlaying
...
MediaControlLoadingPanelElement::MediaControlLoadingPanelElement(MediaControlsImpl& media_controls)
    : MediaControlDivElement(media_controls)
{
    ...
    setAttribute(html_names::kAriaLabelAttr, WTF::AtomicString(GetLocale().QueryString(IDS_AX_MEDIA_LOADING_PANEL)));
...
void MediaControlLoadingPanelElement::PopulateShadowDOM()
{
    ...
    style->setTextContent(MediaControlsResourceLoader::GetShadowLoadingStyleSheet());
    ...
    // elements.
    HTMLDivElement* spinner_frame = MediaControlElementsHelper::CreateDivWithId(AtomicString("spinner-frame"), shadow_root);
    spinner_frame->SetShadowPseudoId(AtomicString("-internal-media-controls-loading-panel-spinner-frame"));

    // The spinner is responsible for rotating the elements below. The square
    // edges will be cut off by the frame above.
    HTMLDivElement* spinner = MediaControlElementsHelper::CreateDivWithId(AtomicString("spinner"), spinner_frame);

    // The layer performs a secondary "fill-unfill-rotate" animation.
    HTMLDivElement* layer = MediaControlElementsHelper::CreateDivWithId(AtomicString("layer"), spinner);
    ...
    HTMLDivElement* mask1 = MediaControlElementsHelper::CreateDivWithId(AtomicString("spinner-mask-1"), layer);
    mask1_background_ = MediaControlElementsHelper::CreateDiv(AtomicString("-internal-media-controls-loading-panel-spinner-mask-1-background"), mask1);
    HTMLDivElement* mask2 = MediaControlElementsHelper::CreateDivWithId(AtomicString("spinner-mask-2"), layer);
    mask2_background_ = MediaControlElementsHelper::CreateDiv(AtomicString("-internal-media-controls-loading-panel-spinner-mask-2-background"), mask2);

    event_listener_ = MakeGarbageCollected<MediaControlAnimationEventListener>(this);
}

void MediaControlLoadingPanelElement::RemovedFrom(ContainerNode& insertion_point)
{
    if (event_listener_) {
...
void MediaControlLoadingPanelElement::SetAnimationIterationCount(const String& count_value)
{
    if (mask1_background_) {
        mask1_background_->SetInlineStyleProperty(CSSPropertyID::kAnimationIterationCount, count_value);
    }
    if (mask2_background_) {
        mask2_background_->SetInlineStyleProperty(CSSPropertyID::kAnimationIterationCount, count_value);
    }
}
...
void MediaControlLoadingPanelElement::UpdateDisplayState()
{
    ...
    if (GetMediaControls().State() == MediaControlsImpl::kPlaying && state_ != State::kHidden) {
        HideAnimation();
        return;
    }
