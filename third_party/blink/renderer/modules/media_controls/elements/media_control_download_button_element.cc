...
MediaControlDownloadButtonElement::MediaControlDownloadButtonElement(MediaControlsImpl& media_controls)
    : MediaControlInputElement(media_controls)
{
    setType(input_type_names::kButton);
    setAttribute(html_names::kAriaLabelAttr, WTF::AtomicString(GetLocale().QueryString(IDS_AX_MEDIA_DOWNLOAD_BUTTON)));
...
bool MediaControlDownloadButtonElement::ShouldDisplayDownloadButton() const
{
    ...
    if (MediaElement().ControlsListInternal()->ShouldHideDownload() && !MediaElement().UserWantsControlsVisible()) {
        UseCounter::Count(MediaElement().GetDocument(), WebFeature::kHTMLMediaElementControlsListNoDownload);
        return false;
    }
...
void MediaControlDownloadButtonElement::DefaultEventHandler(Event& event)
{
    ...
    if ((event.type() == event_type_names::kClick || event.type() == event_type_names::kGesturetap) && !(url.IsNull() || url.IsEmpty())) {
        ...
        GetDocument().GetFrame()->DownloadURL(request, network::mojom::blink::RedirectMode::kError);
    }
