...
MediaControlOverflowMenuButtonElement::MediaControlOverflowMenuButtonElement(MediaControlsImpl& media_controls)
    : MediaControlInputElement(media_controls)
{
    setType(input_type_names::kButton);
    setAttribute(html_names::kAriaLabelAttr, WTF::AtomicString(GetLocale().QueryString(IDS_AX_MEDIA_OVERFLOW_BUTTON)));
    setAttribute(html_names::kTitleAttr, WTF::AtomicString(GetLocale().QueryString(IDS_AX_MEDIA_OVERFLOW_BUTTON_HELP)));
...
const char* MediaControlOverflowMenuButtonElement::GetNameForHistograms() const
{
