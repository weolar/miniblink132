...
MediaControlsRotateToFullscreenDelegate::MediaControlsRotateToFullscreenDelegate(HTMLVideoElement& video)
...
void MediaControlsRotateToFullscreenDelegate::Attach()
{
    ...
    video_element_->addEventListener(event_type_names::kWebkitfullscreenchange, this, true);
    video_element_->GetDocument().addEventListener(event_type_names::kFullscreenchange, this, true);
    ...
    dom_window->addEventListener(event_type_names::kOrientationchange, this, false);
...
void MediaControlsRotateToFullscreenDelegate::Detach()
{
    ...
    video_element_->removeEventListener(event_type_names::kWebkitfullscreenchange, this, true);
    video_element_->GetDocument().removeEventListener(event_type_names::kFullscreenchange, this, true);

    LocalDOMWindow* dom_window = video_element_->GetDocument().domWindow();
    if (!dom_window)
        return;
    dom_window->removeEventListener(event_type_names::kOrientationchange, this, false);
    dom_window->removeEventListener(event_type_names::kDeviceorientation, this, false);
}

void MediaControlsRotateToFullscreenDelegate::Invoke(ExecutionContext* execution_context, Event* event)
{
    if (event->type() == event_type_names::kPlay || event->type() == event_type_names::kPause || event->type() == event_type_names::kFullscreenchange
...
void MediaControlsRotateToFullscreenDelegate::OnStateChange()
{
    ...
    bool needs_intersection_observer = !video_element_->paused() && !video_element_->IsFullscreen();
    DVLOG(3) << __func__ << " " << !!intersection_observer_ << " -> " << needs_intersection_observer;

    if (needs_intersection_observer && !intersection_observer_) {
        intersection_observer_ = IntersectionObserver::Create(video_element_->GetDocument(),
            WTF::BindRepeating(&MediaControlsRotateToFullscreenDelegate::OnIntersectionChange, WrapWeakPersistent(this)),
            LocalFrameUkmAggregator::kMediaIntersectionObserver, IntersectionObserver::Params { .thresholds = { kIntersectionThreshold } });
...
void MediaControlsRotateToFullscreenDelegate::OnIntersectionChange(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
    bool is_visible = (entries.back()->intersectionRatio() > kIntersectionThreshold);
    DVLOG(3) << __func__ << " " << is_visible_ << " -> " << is_visible;
    is_visible_ = is_visible;
}

void MediaControlsRotateToFullscreenDelegate::OnDeviceOrientationAvailable(DeviceOrientationEvent* event)
{
    LocalDOMWindow* dom_window = video_element_->GetDocument().domWindow();
    if (!dom_window)
        return;
    ...
    device_orientation_supported_ = std::make_optional(data->CanProvideBeta() && data->CanProvideGamma() && (data->Beta() != 0.0 || data->Gamma() != 0.0));
}
...
void MediaControlsRotateToFullscreenDelegate::OnScreenOrientationChange()
{
    ...
    current_screen_orientation_ = ComputeScreenOrientation();
    DVLOG(3) << __func__ << " " << static_cast<int>(previous_screen_orientation) << " -> " << static_cast<int>(current_screen_orientation_);
    ...
    Element* fullscreen_element = Fullscreen::FullscreenElementFrom(video_element_->GetDocument());
    ...
    if (!video_element_->IsFullscreen() && (!is_visible_ || video_element_->paused())) {
        return;
    }

    // Ignore (unexpected) events where we have incomplete information.
    if (previous_screen_orientation == SimpleOrientation::kUnknown || current_screen_orientation_ == SimpleOrientation::kUnknown) {
        return;
    }
    ...
    MediaControlsImpl& media_controls = *static_cast<MediaControlsImpl*>(video_element_->GetMediaControls());

    {
        LocalFrame::NotifyUserActivation(video_element_->GetDocument().GetFrame(), mojom::blink::UserActivationNotificationType::kInteraction);

        bool should_be_fullscreen = current_screen_orientation_ == video_orientation;
        if (should_be_fullscreen && !video_element_->IsFullscreen()) {
            Platform::Current()->RecordAction(UserMetricsAction("Media.Video.RotateToFullscreen.Enter"));
            media_controls.EnterFullscreen();
        } else if (!should_be_fullscreen && video_element_->IsFullscreen()) {
            Platform::Current()->RecordAction(UserMetricsAction("Media.Video.RotateToFullscreen.Exit"));
            media_controls.ExitFullscreen();
        }
    }
}

MediaControlsRotateToFullscreenDelegate::SimpleOrientation MediaControlsRotateToFullscreenDelegate::ComputeVideoOrientation() const
{
...
MediaControlsRotateToFullscreenDelegate::SimpleOrientation MediaControlsRotateToFullscreenDelegate::ComputeScreenOrientation() const
{
