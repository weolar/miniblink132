...
MediaControlsMediaEventListener::MediaControlsMediaEventListener(MediaControlsImpl* media_controls)
...
//     GetMediaElement().addEventListener(event_type_names::kWebkitfullscreenchange, this, /*use_capture=*/false);
//     GetMediaElement().addEventListener(event_type_names::kFullscreenchange, this,
//         /*use_capture=*/false);
// 
//     // Picture-in-Picture events.
//     if (media_controls_->GetDocument().GetSettings() && media_controls_->GetDocument().GetSettings()->GetPictureInPictureEnabled()
//         && IsA<HTMLVideoElement>(GetMediaElement())) {
//         GetMediaElement().addEventListener(event_type_names::kEnterpictureinpicture, this, /*use_capture=*/false);
//         GetMediaElement().addEventListener(event_type_names::kLeavepictureinpicture, this, /*use_capture=*/false);
//     }
// 
//     // TextTracks events.
...
//         media_controls_->ButtonPanelElement()->addEventListener(event_type_names::kKeypress, this, false);
//     }
// 
...
//     DCHECK(!GetMediaElement().isConnected());
// 
//     media_controls_->GetDocument().removeEventListener(event_type_names::kFullscreenchange, this, /*use_capture=*/false);
// 
//     TextTrackList* text_tracks = GetMediaElement().textTracks();
...
// 
//     if (media_controls_->ButtonPanelElement()) {
//         media_controls_->ButtonPanelElement()->removeEventListener(event_type_names::kKeypress, this, /*use_capture=*/false);
//     }
// 
//     RemotePlayback& remote = RemotePlayback::From(GetMediaElement());
...
//     if (remote_playback_availability_callback_id_.has_value()
//         && remote_playback_availability_callback_id_.value() != RemotePlayback::kWatchAvailabilityNotSupported) {
//         remote.CancelWatchAvailabilityInternal(remote_playback_availability_callback_id_.value());
...
void MediaControlsMediaEventListener::Invoke(ExecutionContext* execution_context, Event* event)
{
    *(int*)1 = 1;
...
// 
//     // Picture-in-Picture events.
//     if (event->type() == event_type_names::kEnterpictureinpicture || event->type() == event_type_names::kLeavepictureinpicture) {
//         media_controls_->OnPictureInPictureChanged();
//         return;
//     }
// 
//     // TextTracks events.
//     if (event->type() == event_type_names::kAddtrack || event->type() == event_type_names::kRemovetrack) {
...
//     }
// 
//     if (event->type() == event_type_names::kKeypress || event->type() == event_type_names::kKeydown || event->type() == event_type_names::kKeyup) {
//         media_controls_->OnMediaKeyboardEvent(event);
//         return;
//     }
// 
//     // RemotePlayback state change events.
//     if (event->type() == event_type_names::kConnect || event->type() == event_type_names::kConnecting || event->type() == event_type_names::kDisconnect) {
//         media_controls_->RemotePlaybackStateChanged();
//         return;
//     }
// 
//     if (event->type() == event_type_names::kPointermove || event->type() == event_type_names::kPointerout || event->type() == event_type_names::kPointerenter) {
