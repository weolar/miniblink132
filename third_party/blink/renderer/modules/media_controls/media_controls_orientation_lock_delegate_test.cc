...
class MockWebMediaPlayerForOrientationLockDelegate final : public EmptyWebMediaPlayer {
public:
    ...
    {
        return mock_natural_size_;
    }

    gfx::Size& MockNaturalSize()
    {
        return mock_natural_size_;
    }
...
class MockScreenOrientation final : public GarbageCollected<MockScreenOrientation>, public device::mojom::blink::ScreenOrientation {
public:
    MockScreenOrientation() = default;

    // device::mojom::blink::ScreenOrientation overrides:
    void LockOrientation(device::mojom::ScreenOrientationLockType type, LockOrientationCallback callback) override
    {
    ...
    void BindPendingReceiver(
        mojo::PendingAssociatedReceiver<device::mojom::blink::ScreenOrientation> pending_receiver, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
    ...
    MOCK_METHOD(void, LockOrientation, (device::mojom::ScreenOrientationLockType));

private:
    // MockScreenOrientation is not tied to ExecutionContext.
    HeapMojoAssociatedReceiver<device::mojom::blink::ScreenOrientation, MockScreenOrientation> receiver_ { this, nullptr };
};
...
void DidEnterFullscreen(Document* document)
{
    ...
    PageAnimator::ServiceScriptedAnimations(base::TimeTicks::Now(), { { document->GetScriptedAnimationController(), false } });
}
...
class MockChromeClientForOrientationLockDelegate final : public EmptyChromeClient {
public:
    MockChromeClientForOrientationLockDelegate()
        : mock_screen_orientation_(MakeGarbageCollected<MockScreenOrientation>())
    {
    }
    ...
    void InstallSupplements(LocalFrame& frame) override
    {
        ...
        HeapMojoAssociatedRemote<device::mojom::blink::ScreenOrientation> screen_orientation(frame.DomWindow());
        ScreenOrientationClient().BindPendingReceiver(
            screen_orientation.BindNewEndpointAndPassDedicatedReceiver(), frame.GetTaskRunner(TaskType::kInternalNavigationAssociated));
        ScreenOrientationController::From(*frame.DomWindow())->SetScreenOrientationAssociatedRemoteForTests(std::move(screen_orientation));
    }
    ...
    void EnterFullscreen(LocalFrame& frame, const FullscreenOptions*, FullscreenRequestType) override
    {
        frame.GetTaskRunner(TaskType::kInternalNavigationAssociated)
            ->PostTask(FROM_HERE, WTF::BindOnce(DidEnterFullscreen, WrapPersistent(frame.GetDocument())));
    }
    void ExitFullscreen(LocalFrame& frame) override
    {
        frame.GetTaskRunner(TaskType::kInternalNavigationAssociated)
            ->PostTask(FROM_HERE, WTF::BindOnce(DidExitFullscreen, WrapPersistent(frame.GetDocument())));
    }
...
private:
    ...
    display::ScreenInfos mock_screen_infos_ = display::ScreenInfos(display::ScreenInfo());
};

class StubLocalFrameClientForOrientationLockDelegate final : public EmptyLocalFrameClient {
public:
...
class MediaControlsOrientationLockDelegateTest : public PageTestBase,
...
public:
    ...
    explicit MediaControlsOrientationLockDelegateTest(bool video_rotate_to_fullscreen)
        : ScopedVideoFullscreenOrientationLockForTest(true)
...
protected:
    using DeviceOrientationType = MediaControlsOrientationLockDelegate::DeviceOrientationType;
    ...
    void SetUp() override
    {
        chrome_client_ = MakeGarbageCollected<MockChromeClientForOrientationLockDelegate>();
        SetupPageWithClients(chrome_client_, MakeGarbageCollected<StubLocalFrameClientForOrientationLockDelegate>());
        previous_orientation_event_value_ = RuntimeEnabledFeatures::OrientationEventEnabled();

        GetDocument().write("<body><video></body>");
        video_ = To<HTMLVideoElement>(*GetDocument().QuerySelector(AtomicString("video")));
    }
    ...
    {
        return !!static_cast<const MediaControlsImpl*>(&media_controls)->orientation_lock_delegate_;
    }

    void SimulateEnterFullscreen()
    {
        LocalFrame::NotifyUserActivation(GetDocument().GetFrame(), mojom::UserActivationNotificationType::kTest);
    ...
    {
        ScreenOrientationController* controller = ScreenOrientationController::From(*GetDocument().domWindow());
        controller->lock(device::mojom::ScreenOrientationLockType::LANDSCAPE, std::make_unique<DummyScreenOrientationCallback>());
    ...
    {
        EXPECT_EQ(MediaControlsOrientationLockDelegate::State::kPendingFullscreen, MediaControls()->orientation_lock_delegate_->state_);
    }

    void CheckStatePendingMetadata() const
    {
        EXPECT_EQ(MediaControlsOrientationLockDelegate::State::kPendingMetadata, MediaControls()->orientation_lock_delegate_->state_);
    }

    void CheckStateMaybeLockedFullscreen() const
    {
        EXPECT_EQ(MediaControlsOrientationLockDelegate::State::kMaybeLockedFullscreen, MediaControls()->orientation_lock_delegate_->state_);
    }

    bool DelegateWillUnlockFullscreen() const
    {
        return DelegateOrientationLock() != device::mojom::ScreenOrientationLockType::DEFAULT /* unlocked */;
    }
    ...
    {
        return *static_cast<MockWebMediaPlayerForOrientationLockDelegate*>(Video().GetWebMediaPlayer());
    }
...
class MediaControlsOrientationLockAndRotateToFullscreenDelegateTest : public MediaControlsOrientationLockDelegateTest, private ScopedOrientationEventForTest {
public:
...
protected:
    ...
    {
        MediaControls()->orientation_lock_delegate_->is_auto_rotate_enabled_by_user_override_for_testing_ = enabled;
    }
    ...
    gfx::Rect ScreenRectFromAngle(uint16_t screen_orientation_angle)
    {
        ...
        bool screen_rect_is_portrait = screen_orientation_angle % 180 == portrait_angle_mod_180;
        return screen_rect_is_portrait ? gfx::Rect(0, 0, 1080, 1920) : gfx::Rect(0, 0, 1920, 1080);
    }
    ...
    void RotateDeviceTo(double beta, double gamma)
    {
        DeviceOrientationController::From(*GetFrame().DomWindow())
            .SetOverride(DeviceOrientationData::Create(0.0 /* alpha */, beta, gamma, false /* absolute */));
        test::RunPendingTasks();
    }

    // Calls must be wrapped in ASSERT_NO_FATAL_FAILURE.
    void RotateScreenTo(display::mojom::blink::ScreenOrientation screen_orientation_type, uint16_t screen_orientation_angle)
    {
        auto rect = ScreenRectFromAngle(screen_orientation_angle);
        ASSERT_TRUE(screen_orientation_type == ScreenOrientationController::ComputeOrientation(rect, screen_orientation_angle));
        ChromeClient().MockScreenInfo().orientation_type = screen_orientation_type;
        ChromeClient().MockScreenInfo().orientation_angle = screen_orientation_angle;
        ChromeClient().MockScreenInfo().rect = rect;

        // Screen Orientation API
        ScreenOrientationController::From(*GetDocument().domWindow())->NotifyOrientationChanged();
    ...
    void InitVideo(int video_width, int video_height)
    {
        ...
        // Set video size.
        MockWebMediaPlayer().MockNaturalSize() = gfx::Size(video_width, video_height);
    ...
    DeviceOrientationType ComputeDeviceOrientation(DeviceOrientationData* data) const
    {
        return MediaControls()->orientation_lock_delegate_->ComputeDeviceOrientation(data);
    }
...
TEST_F(MediaControlsOrientationLockDelegateTest, DelegateRequiresFlag)
{
    ...
    // Same with flag off.
    ScopedVideoFullscreenOrientationLockForTest video_fullscreen_orientation_lock(false);
...
TEST_F(MediaControlsOrientationLockDelegateTest, ReceivedMetadataAfterExitingFullscreen)
{
    EXPECT_CALL(ScreenOrientationClient(), LockOrientation(_)).Times(1);
...
TEST_F(MediaControlsOrientationLockDelegateTest, ComputeOrientationLock)
{
    ...
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::LANDSCAPE, ComputeOrientationLock());

    // 50x100
    MockWebMediaPlayer().MockNaturalSize() = gfx::Size(50, 100);
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::PORTRAIT, ComputeOrientationLock());
    ...
    ChromeClient().MockScreenInfo().orientation_type = display::mojom::blink::ScreenOrientation::kUndefined;
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::LANDSCAPE, ComputeOrientationLock());

    ChromeClient().MockScreenInfo().orientation_type = display::mojom::blink::ScreenOrientation::kPortraitPrimary;
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::PORTRAIT, ComputeOrientationLock());

    ChromeClient().MockScreenInfo().orientation_type = display::mojom::blink::ScreenOrientation::kPortraitPrimary;
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::PORTRAIT, ComputeOrientationLock());

    ChromeClient().MockScreenInfo().orientation_type = display::mojom::blink::ScreenOrientation::kLandscapePrimary;
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::LANDSCAPE, ComputeOrientationLock());

    ChromeClient().MockScreenInfo().orientation_type = display::mojom::blink::ScreenOrientation::kLandscapeSecondary;
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::LANDSCAPE, ComputeOrientationLock());
}

TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, ComputeDeviceOrientation)
{
    InitVideo(400, 400);
    ...
    for (int n_o_i_p = 0; n_o_i_p <= 1; n_o_i_p++) {
        ...
        SCOPED_TRACE(testing::Message() << "natural_orientation_is_portrait_=" << natural_orientation_is_portrait_);

        DeviceOrientationType natural_orientation = natural_orientation_is_portrait_ ? DeviceOrientationType::kPortrait : DeviceOrientationType::kLandscape;
        DeviceOrientationType perpendicular_to_natural_orientation
            = natural_orientation_is_portrait_ ? DeviceOrientationType::kLandscape : DeviceOrientationType::kPortrait;
        ...
        for (int screen_angle = 0; screen_angle < 360; screen_angle += 90) {
            ...
            display::mojom::blink::ScreenOrientation screen_type = display::mojom::blink::ScreenOrientation::kUndefined;
            switch (screen_angle) {
            case 0:
                screen_type = natural_orientation_is_portrait_ ? display::mojom::blink::ScreenOrientation::kPortraitPrimary
                                                               : display::mojom::blink::ScreenOrientation::kLandscapePrimary;
                break;
            case 90:
                screen_type = natural_orientation_is_portrait_ ? display::mojom::blink::ScreenOrientation::kLandscapePrimary
                                                               : display::mojom::blink::ScreenOrientation::kPortraitSecondary;
                break;
            case 180:
                screen_type = natural_orientation_is_portrait_ ? display::mojom::blink::ScreenOrientation::kPortraitSecondary
                                                               : display::mojom::blink::ScreenOrientation::kLandscapeSecondary;
                break;
            case 270:
                screen_type = natural_orientation_is_portrait_ ? display::mojom::blink::ScreenOrientation::kLandscapeSecondary
            ...
            EXPECT_EQ(DeviceOrientationType::kFlat, // face up
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 0. /* beta */, 0. /* gamma */, absolute)));
            EXPECT_EQ(DeviceOrientationType::kFlat, // face down
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 180. /* beta */, 0. /* gamma */, absolute)));
            EXPECT_EQ(DeviceOrientationType::kFlat, // face down
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, -180. /* beta */, 0. /* gamma */, absolute)));
            EXPECT_EQ(DeviceOrientationType::kFlat, // face up, angle=0
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 20. /* beta */, 0. /* gamma */, absolute)));
            EXPECT_EQ(DeviceOrientationType::kFlat, // face up, angle=90
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 0. /* beta */, -20. /* gamma */, absolute)));
            ...
            EXPECT_EQ(DeviceOrientationType::kDiagonal, // angle=45
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 135. /* beta */, 90. /* gamma */, absolute)));
            EXPECT_EQ(DeviceOrientationType::kDiagonal, // angle=45
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 45. /* beta */, -90. /* gamma */, absolute)));
            ...
            EXPECT_EQ(natural_orientation, ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 90. /* beta */, 0. /* gamma */, absolute)));
            EXPECT_EQ(natural_orientation, ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 90. /* beta */, 90. /* gamma */, absolute)));
            EXPECT_EQ(natural_orientation, ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 90. /* beta */, -90. /* gamma */, absolute)));
            EXPECT_EQ(natural_orientation, ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 85. /* beta */, 90. /* gamma */, absolute)));
            EXPECT_EQ(natural_orientation, ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 85. /* beta */, -90. /* gamma */, absolute)));
            EXPECT_EQ(natural_orientation, ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 95. /* beta */, 90. /* gamma */, absolute)));
            EXPECT_EQ(natural_orientation, ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 95. /* beta */, -90. /* gamma */, absolute)));

            // These beta and gamma values should all map to r == 1 and
            // device_orientation_angle == 90, hence orientation == kLandscape.
            EXPECT_EQ(perpendicular_to_natural_orientation,
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 0. /* beta */, -90. /* gamma */, absolute)));
            EXPECT_EQ(perpendicular_to_natural_orientation,
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 180. /* beta */, 90. /* gamma */, absolute)));
            EXPECT_EQ(perpendicular_to_natural_orientation,
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, -180. /* beta */, 90. /* gamma */, absolute)));
            ...
            EXPECT_EQ(
                perpendicular_to_natural_orientation, ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 0. /* beta */, 90. /* gamma */, absolute)));
            EXPECT_EQ(perpendicular_to_natural_orientation,
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, 180. /* beta */, -90. /* gamma */, absolute)));
            EXPECT_EQ(perpendicular_to_natural_orientation,
                ComputeDeviceOrientation(DeviceOrientationData::Create(alpha, -180. /* beta */, -90. /* gamma */, absolute)));
...
TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, PortraitInlineRotateToLandscapeFullscreen)
{
    // Naturally portrait device, initially portrait, with landscape video.
    natural_orientation_is_portrait_ = true;
    ASSERT_NO_FATAL_FAILURE(RotateScreenTo(display::mojom::blink::ScreenOrientation::kPortraitPrimary, 0));
    InitVideo(640, 480);
    ...
    // orientation change.
    ASSERT_NO_FATAL_FAILURE(RotateScreenTo(display::mojom::blink::ScreenOrientation::kLandscapePrimary, 90));
    ...
    CheckStateMaybeLockedFullscreen();
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::LANDSCAPE, DelegateOrientationLock());
    ...
    CheckStateMaybeLockedFullscreen();
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::ANY, DelegateOrientationLock());
    EXPECT_TRUE(DelegateWillUnlockFullscreen());
}

TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, PortraitInlineButtonToPortraitLockedLandscapeFullscreen)
{
    // Naturally portrait device, initially portrait, with landscape video.
    natural_orientation_is_portrait_ = true;
    ASSERT_NO_FATAL_FAILURE(RotateScreenTo(display::mojom::blink::ScreenOrientation::kPortraitPrimary, 0));
...
TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, PortraitLockedLandscapeFullscreenBackToPortraitInline)
{
    // Naturally portrait device, initially portrait device orientation but locked
    // to landscape screen orientation, with landscape video.
    natural_orientation_is_portrait_ = true;
...
TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, LandscapeInlineRotateToPortraitInline)
{
...
TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, LandscapeInlineButtonToLandscapeFullscreen)
{
    // Naturally portrait device, initially landscape, with landscape video.
    natural_orientation_is_portrait_ = true;
    ASSERT_NO_FATAL_FAILURE(RotateScreenTo(display::mojom::blink::ScreenOrientation::kLandscapePrimary, 90));
...
TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, LandscapeFullscreenRotateToPortraitInline)
{
    // Naturally portrait device, initially landscape, with landscape video.
    natural_orientation_is_portrait_ = true;
    ASSERT_NO_FATAL_FAILURE(RotateScreenTo(display::mojom::blink::ScreenOrientation::kLandscapePrimary, 90));
...
TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, AutoRotateDisabledPortraitInlineButtonToPortraitLockedLandscapeFullscreen)
{
    // Naturally portrait device, initially portrait, with landscape video.
    natural_orientation_is_portrait_ = true;
    ASSERT_NO_FATAL_FAILURE(RotateScreenTo(display::mojom::blink::ScreenOrientation::kPortraitPrimary, 0));
...
TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, AutoRotateDisabledPortraitLockedLandscapeFullscreenBackToPortraitInline)
{
    // Naturally portrait device, initially portrait device orientation but locked
    // to landscape screen orientation, with landscape video.
    natural_orientation_is_portrait_ = true;
...
TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, AutoRotateDisabledLandscapeLockedLandscapeFullscreenBackToPortraitInline)
{
    // Naturally portrait device, initially landscape device orientation yet also
    // locked to landscape screen orientation since the user had disabled auto
    // rotate, with landscape video.
...
TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, PortraitVideoRotateEnterExit)
{
    ...
    EXPECT_TRUE(Video().IsFullscreen());
    CheckStateMaybeLockedFullscreen();
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::PORTRAIT, DelegateOrientationLock());
...
TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, LandscapeDeviceRotateEnterExit)
{
    // Naturally *landscape* device, initially portrait, with landscape video.
    natural_orientation_is_portrait_ = false;
    ASSERT_NO_FATAL_FAILURE(RotateScreenTo(display::mojom::blink::ScreenOrientation::kPortraitPrimary, 270));
    InitVideo(640, 480);
    SetIsAutoRotateEnabledByUser(true);
    PlayVideo();
    UpdateVisibilityObserver();
    ...
    // Simulate user rotating their device to landscape triggering a screen
    // orientation change.
    ASSERT_NO_FATAL_FAILURE(RotateScreenTo(display::mojom::blink::ScreenOrientation::kLandscapePrimary, 0));
...
TEST_F(MediaControlsOrientationLockAndRotateToFullscreenDelegateTest, ScreenOrientationRaceCondition)
{
    ...
    static_assert(GetUnlockDelay() > kMinUnlockDelay, "GetUnlockDelay() should significantly exceed kMinUnlockDelay");
    ...
    ASSERT_NO_FATAL_FAILURE(RotateScreenTo(display::mojom::blink::ScreenOrientation::kLandscapeSecondary, 270));
