...
class MediaControlsDisplayCutoutDelegateTest : public PageTestBase, private ScopedDisplayCutoutAPIForTest, private ScopedMediaControlsExpandGestureForTest {
public:
    ...
    void SetUp() override
    {
        ...
        SetupPageWithClients(chrome_client_, MakeGarbageCollected<EmptyLocalFrameClient>());
    ...
    bool HasGestureState()
    {
        return GetDelegate().previous_.has_value();
    }

    bool DirectionIsExpanding()
    {
        return GetDelegate().previous_->second == MediaControlsDisplayCutoutDelegate::Direction::kExpanding;
    }

    bool DirectionIsUnknown()
    {
        return GetDelegate().previous_->second == MediaControlsDisplayCutoutDelegate::Direction::kUnknown;
    }
    ...
    TouchEvent* CreateTouchEventWithList(const AtomicString& type, TouchList* list)
    {
    ...
    {
        return Touch::Create(GetDocument().GetFrame(), &GetVideoElement(), 1 /* identifier */, gfx::PointF(x, y), gfx::PointF(x, y), gfx::SizeF(1, 1), 90, 0);
    }
...
private:
    ...
    {
        MediaControlsImpl* controls = static_cast<MediaControlsImpl*>(GetVideoElement().GetMediaControls());
        return *controls->display_cutout_delegate_;
    }

    HTMLVideoElement& GetVideoElement()
    {
        return *To<HTMLVideoElement>(GetDocument().getElementById(AtomicString("video")));
    }
...
TEST_F(MediaControlsDisplayCutoutDelegateTest, CombinedGesture)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kMediaControlsDisplayCutoutGesture));
}
...
{
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kMediaControlsDisplayCutoutGesture));
}
