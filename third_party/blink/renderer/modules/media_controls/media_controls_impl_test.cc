...
public:
    FakeChromeClient()
    {
        screen_info_.orientation_type = display::mojom::blink::ScreenOrientation::kLandscapePrimary;
    }
...
public:
    ...
    RemotePlaybackClient* CreateRemotePlaybackClient(HTMLMediaElement& element) override
    {
        return &RemotePlayback::From(element);
    }
};

Element* GetElementByShadowPseudoId(Node& root_node, const char* shadow_pseudo_id)
{
...
bool IsElementVisible(Element& element)
{
    ...
    if (inline_style->HasProperty(CSSPropertyID::kOpacity) && inline_style->GetPropertyValue(CSSPropertyID::kOpacity).ToDouble() == 0.0) {
        return false;
    }
...
class MediaControlsImplTest : public PageTestBase, private ScopedMediaCastOverlayButtonForTest {
public:
    explicit MediaControlsImplTest(base::test::TaskEnvironment::TimeSource time_source)
...
protected:
    ...
    void InitializePage()
    {
        SetupPageWithClients(MakeGarbageCollected<FakeChromeClient>(), MakeGarbageCollected<StubLocalFrameClientForImpl>());

        GetDocument().write("<video controls>");
        auto& video = To<HTMLVideoElement>(*GetDocument().QuerySelector(AtomicString("video")));
    ...
    {
        RemotePlayback::From(media_controls_->MediaElement()).AvailabilityChangedForTesting(/* screen_is_available */ true);
    }
    ...
    void EnsureSizing()
    {
        ...
        media_controls_->NotifyElementSizeChanged(media_controls_->MediaElement().GetBoundingClientRect());
    }
    ...
    void SimulateLoadedMetadata()
    {
        media_controls_->OnLoadedMetadata();
    }

    void SimulateOnSeeking()
    {
        media_controls_->OnSeeking();
    }
    void SimulateOnSeeked()
    {
        media_controls_->OnSeeked();
    }
    void SimulateOnWaiting()
    {
        media_controls_->OnWaiting();
    }
    void SimulateOnPlaying()
    {
    ...
    {
        MediaControls().MediaElement().SetReadyState(HTMLMediaElement::kHaveEnoughData);
        MediaControls().MediaElement().SetNetworkState(WebMediaPlayer::NetworkState::kNetworkStateLoading);
    }

    void SimulateMediaControlPlayingForFutureData()
    {
        MediaControls().MediaElement().SetReadyState(HTMLMediaElement::kHaveFutureData);
        MediaControls().MediaElement().SetNetworkState(WebMediaPlayer::NetworkState::kNetworkStateLoading);
    }

    void SimulateMediaControlBuffering()
    {
        MediaControls().MediaElement().SetReadyState(HTMLMediaElement::kHaveCurrentData);
        MediaControls().MediaElement().SetNetworkState(WebMediaPlayer::NetworkState::kNetworkStateLoading);
    }
    ...
    MediaControlRemainingTimeDisplayElement* GetRemainingTimeDisplayElement() const
    {
    ...
    {
        return static_cast<MockWebMediaPlayerForImpl*>(MediaControls().MediaElement().GetWebMediaPlayer());
    }
    ...
    {
        MediaControls().MediaElement().SetSrc(AtomicString("https://example.com/foo.mp4"));
        test::RunPendingTasks();
        WebTimeRange time_range(0.0, duration);
        WebMediaPlayer()->seekable_.Assign(base::span_from_ref(time_range));
        MediaControls().MediaElement().DurationChanged(duration, false /* requestSeek */);
        SimulateLoadedMetadata();
    }

    void SetHasAudio(bool has_audio)
    {
        WebMediaPlayer()->has_audio_ = has_audio;
    }

    void ClickOverflowButton()
    {
        MediaControls().download_button_->OverflowElementForTests()->DispatchSimulatedClick(nullptr);
    }
    ...
    {
        MediaControlInputElement* overflow_element = element.OverflowElementForTests();
        if (!overflow_element)
            return false;
        ...
        const CSSPropertyValueSet* inline_style = overflow_parent_label->InlineStyle();
        if (inline_style->GetPropertyValue(CSSPropertyID::kDisplay) == "none")
            return false;
...
{
    WebMouseEvent mouse_down_event(WebInputEvent::Type::kMouseDown, pos /* client pos */, pos /* screen pos */, WebPointerProperties::Button::kLeft, 1,
...
{
    WebMouseEvent mouse_move_event(WebInputEvent::Type::kMouseMove, pos /* client pos */, pos /* screen pos */, WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
...
{
    WebMouseEvent mouse_up_event(WebMouseEvent::Type::kMouseUp, pos /* client pos */, pos /* screen pos */, WebPointerProperties::Button::kLeft, 1,
...
{
    WebGestureEvent gesture_tap_event(WebInputEvent::Type::kGestureTap, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
    ...
    // Fire the event.
    GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(gesture_tap_event);
}
...
TEST_F(MediaControlsImplTest, CastButtonDisableRemotePlaybackAttr)
{
    ...
    MediaControls().MediaElement().SetBooleanAttribute(html_names::kDisableremoteplaybackAttr, true);
    test::RunPendingTasks();
    ASSERT_FALSE(IsOverflowElementVisible(*cast_button));

    MediaControls().MediaElement().SetBooleanAttribute(html_names::kDisableremoteplaybackAttr, false);
    test::RunPendingTasks();
    ASSERT_TRUE(IsOverflowElementVisible(*cast_button));
}
...
{
    MediaControls().MediaElement().SetBooleanAttribute(html_names::kControlsAttr, false);

    Element* cast_overlay_button = GetElementByShadowPseudoId(MediaControls(), "-internal-media-controls-overlay-cast-button");
...
TEST_F(MediaControlsImplTest, CastOverlayDisabledAutoplayMuted)
{
    ...
    MediaControls().MediaElement().GetDocument().GetSettings()->SetAutoplayPolicy(AutoplayPolicy::Type::kDocumentUserActivationRequired);
...
TEST_F(MediaControlsImplTest, CastButtonVisibilityDependsOnControlslistAttr)
{
    ...
    MediaControls().MediaElement().setAttribute(blink::html_names::kControlslistAttr, AtomicString("noremoteplayback"));
    test::RunPendingTasks();
...
{
    Element* overflow_list = GetElementByShadowPseudoId(MediaControls(), "-internal-media-controls-overflow-menu-list");
...
TEST_F(MediaControlsImplTest, DownloadButtonNotDisplayedInfiniteDuration)
{
    ...
    MediaControls().MediaElement().DurationChanged(std::numeric_limits<double>::infinity(), false /* requestSeek */);
    SimulateLoadedMetadata();
    EXPECT_FALSE(IsOverflowElementVisible(*download_button));

    // Download button should be shown if the duration changes back to finite.
    MediaControls().MediaElement().DurationChanged(20.0f, false /* requestSeek */);
    SimulateLoadedMetadata();
    EXPECT_TRUE(IsOverflowElementVisible(*download_button));
}
...
TEST_F(MediaControlsImplTest, DownloadButtonNotDisplayedHLS)
{
    ...
    MediaControls().MediaElement().SetSrc(AtomicString("https://example.com/foo.m3u8"));
    test::RunPendingTasks();
    SimulateLoadedMetadata();
    EXPECT_FALSE(IsOverflowElementVisible(*download_button));

    MediaControls().MediaElement().SetSrc(AtomicString("https://example.com/foo.m3u8?title=foo"));
    test::RunPendingTasks();
    SimulateLoadedMetadata();
    EXPECT_FALSE(IsOverflowElementVisible(*download_button));
    ...
    // the text 'm3u8'.
    MediaControls().MediaElement().SetSrc(AtomicString("https://example.com/foo.m3u8.mp4"));
    test::RunPendingTasks();
    SimulateLoadedMetadata();
    EXPECT_TRUE(IsOverflowElementVisible(*download_button));
}

TEST_F(MediaControlsImplTest, DownloadButtonVisibilityDependsOnControlslistAttr)
{
    EnsureSizing();
    ...
    MediaControls().MediaElement().SetSrc(AtomicString("https://example.com/foo.mp4"));
    MediaControls().MediaElement().setAttribute(blink::html_names::kControlslistAttr, AtomicString("nodownload"));
    test::RunPendingTasks();
    SimulateLoadedMetadata();
...
TEST_F(MediaControlsImplTest, FullscreenButtonDisabledDependsOnControlslistAttr)
{
    ...
    MediaControlFullscreenButtonElement* fullscreen_button = FullscreenButtonElement();
    ASSERT_NE(nullptr, fullscreen_button);

    MediaControls().MediaElement().SetSrc(AtomicString("https://example.com/foo.mp4"));
    MediaControls().MediaElement().setAttribute(blink::html_names::kControlslistAttr, AtomicString("nofullscreen"));
    test::RunPendingTasks();
    SimulateLoadedMetadata();
...
TEST_F(MediaControlsImplTest, PlaybackSpeedButtonVisibilityDependsOnControlslistAttr)
{
    EnsureSizing();

    MediaControlPlaybackSpeedButtonElement* playback_speed_button = PlaybackSpeedButtonElement();
    ASSERT_NE(nullptr, playback_speed_button);

    MediaControls().MediaElement().SetSrc(AtomicString("https://example.com/foo.mp4"));
    MediaControls().MediaElement().setAttribute(blink::html_names::kControlslistAttr, AtomicString("noplaybackrate"));
    test::RunPendingTasks();
    SimulateLoadedMetadata();
...
TEST_F(MediaControlsImplTest, TimelineImmediatelyUpdatesCurrentTime)
{
    ...
    MediaControlCurrentTimeDisplayElement* current_time_display = GetCurrentTimeDisplayElement();
    double duration = 600;
...
TEST_F(MediaControlsImplTest, TimeIsCorrectlyFormatted)
{
    ...
    MediaControlCurrentTimeDisplayElement* current_display = GetCurrentTimeDisplayElement();
    MediaControlRemainingTimeDisplayElement* duration_display = GetRemainingTimeDisplayElement();
...
public:
    MediaControlsImplTestWithMockScheduler()
        : MediaControlsImplTest(base::test::TaskEnvironment::TimeSource::MOCK_TIME)
    {
        EnablePlatform();
    }
...
TEST_F(MediaControlsImplTestWithMockScheduler, SeekingDoesNotShowControlsWhenNoControlsAttr)
{
    Element* panel = GetElementByShadowPseudoId(MediaControls(), "-webkit-media-controls-panel");
    ASSERT_NE(nullptr, panel);
...
TEST_F(MediaControlsImplTestWithMockScheduler, ControlsRemainVisibleDuringKeyboardInteraction)
{
    ...
    MuteButtonElement()->DispatchEvent(*Event::CreateBubble(event_type_names::kFocusin));
    FastForwardBy(base::Seconds(2));
    EXPECT_TRUE(IsElementVisible(*panel));

    // Seeking on the timeline or volume bar prevents controls from hiding.
    TimelineElement()->DispatchEvent(*Event::CreateBubble(event_type_names::kInput));
    FastForwardBy(base::Seconds(2));
    EXPECT_TRUE(IsElementVisible(*panel));

    // Pressing a key prevents controls from hiding.
    MuteButtonElement()->DispatchEvent(*Event::CreateBubble(event_type_names::kKeypress));
    FastForwardBy(base::Seconds(2));
    EXPECT_TRUE(IsElementVisible(*panel));
...
TEST_F(MediaControlsImplTestWithMockScheduler, ControlsHideAfterFocusedAndMouseMovement)
{
    EnsureSizing();
    ...
    MediaControls().MediaElement().SetFocused(true, mojom::blink::FocusType::kNone);
    MediaControls().DispatchEvent(*CreatePointerEvent(event_type_names::kPointermove));
...
TEST_F(MediaControlsImplTestWithMockScheduler, ControlsHideAfterFocusedAndMouseMoveout)
{
    EnsureSizing();
...
TEST_F(MediaControlsImplTest, RemovingFromDocumentRemovesListenersAndCallbacks)
{
    auto page_holder = std::make_unique<DummyPageHolder>();

    auto* element = MakeGarbageCollected<HTMLVideoElement>(page_holder->GetDocument());
...
TEST_F(MediaControlsImplTest, RemovingFromDocumentWhenResettingSrcAllowsReclamation)
{
    ...
    auto& video = To<HTMLVideoElement>(*page_holder->GetDocument().QuerySelector(AtomicString("video")));
...
TEST_F(MediaControlsImplTest, ReInsertingInDocumentRestoresListenersAndCallbacks)
{
    auto page_holder = std::make_unique<DummyPageHolder>();
...
TEST_F(MediaControlsImplTest, InitialInfinityDurationHidesDurationField)
{
    ...
    EXPECT_EQ(std::numeric_limits<double>::infinity(), duration_display->CurrentValue());
}
...
TEST_F(MediaControlsImplTest, InfinityDurationChangeHidesDurationField)
{
    ...
    MediaControls().MediaElement().DurationChanged(std::numeric_limits<double>::infinity(), false /* request_seek */);
    test::RunPendingTasks();
...
TEST_F(MediaControlsImplTestWithMockScheduler, ShowVolumeSliderAfterHoverTimerFired)
{
    ...
    gfx::PointF mute_btn_center(mute_btn_rect->left() + mute_btn_rect->width() / 2, mute_btn_rect->top() + mute_btn_rect->height() / 2);
    gfx::PointF edge(0, 0);
...
TEST_F(MediaControlsImplTestWithMockScheduler, VolumeSliderBehaviorWhenFocused)
{
    MediaControls().MediaElement().SetSrc(AtomicString("https://example.com/foo.mp4"));
    FastForwardBy(base::Seconds(1));
...
TEST_F(MediaControlsImplTestWithMockScheduler, VolumeSliderDoesNotOpenWithoutAudio)
{
    MediaControls().MediaElement().SetSrc(AtomicString("https://example.com/foo.mp4"));
    FastForwardBy(base::Seconds(1));
    SetHasAudio(false);
...
TEST_F(MediaControlsImplTest, CastOverlayShowsOnSomeEvents)
{
    ...
    Element* overlay_enclosure = GetElementByShadowPseudoId(MediaControls(), "-webkit-media-controls-overlay-enclosure");
    ...
    for (const AtomicString& event_name : { event_type_names::kGesturetap, event_type_names::kClick }) {
        overlay_enclosure->DispatchEvent(event_name == "gesturetap" ? *Event::Create(event_name) : *CreatePointerEvent(event_name));
        EXPECT_TRUE(IsElementVisible(*cast_overlay_button));
    ...
    overlay_enclosure->DispatchEvent(*CreatePointerEvent(event_type_names::kPointerover));
    EXPECT_FALSE(IsElementVisible(*cast_overlay_button));

    // The overlay button does not appear on click if the overlay button shouldn't
    // be shown.
    MediaControls().MediaElement().SetBooleanAttribute(html_names::kControlsAttr, true);
    overlay_enclosure->DispatchEvent(*CreatePointerEvent(event_type_names::kClick));
    EXPECT_FALSE(IsElementVisible(*cast_overlay_button));
}
...
TEST_F(MediaControlsImplTest, ControlsShouldUseSafeAreaInsets)
{
    ...
    GetStyleEngine().EnsureEnvironmentVariables().SetVariable(UADefinedVariable::kSafeAreaInsetTop, "1px");
    GetStyleEngine().EnsureEnvironmentVariables().SetVariable(UADefinedVariable::kSafeAreaInsetLeft, "2px");
    GetStyleEngine().EnsureEnvironmentVariables().SetVariable(UADefinedVariable::kSafeAreaInsetBottom, "3px");
    GetStyleEngine().EnsureEnvironmentVariables().SetVariable(UADefinedVariable::kSafeAreaInsetRight, "4px");
...
TEST_F(MediaControlsImplTest, MediaControlsDisabledWithNoSource)
{
    ...
    EXPECT_TRUE(OverflowMenuButtonElement()->FastHasAttribute(html_names::kDisabledAttr));
    EXPECT_TRUE(TimelineElement()->FastHasAttribute(html_names::kDisabledAttr));

    MediaControls().MediaElement().setAttribute(html_names::kPreloadAttr, AtomicString("none"));
    MediaControls().MediaElement().SetSrc(AtomicString("https://example.com/foo.mp4"));
    test::RunPendingTasks();
    SimulateLoadedMetadata();
    ...
    EXPECT_FALSE(PlayButtonElement()->FastHasAttribute(html_names::kDisabledAttr));
    EXPECT_FALSE(OverflowMenuButtonElement()->FastHasAttribute(html_names::kDisabledAttr));
    EXPECT_TRUE(TimelineElement()->FastHasAttribute(html_names::kDisabledAttr));
...
TEST_F(MediaControlsImplTest, DoubleTouchChangesTime)
{
    ...
    gfx::PointF leftOfCenter(videoRect->left() + (videoRect->width() / 2) - 5, videoRect->top() + 5);
    gfx::PointF rightOfCenter(videoRect->left() + (videoRect->width() / 2) + 5, videoRect->top() + 5);
...
TEST_F(MediaControlsImplTest, DoubleTouchChangesTimeWhenZoomed)
{
    ...
    DOMRect* videoRect = MediaControls().MediaElement().GetBoundingClientRect();
    ASSERT_LT(0, videoRect->width());
    gfx::PointF leftOfCenter(videoRect->left() + (videoRect->width() / 2) - 5, videoRect->top() + 10);
    gfx::PointF rightOfCenter(videoRect->left() + (videoRect->width() / 2) + 5, videoRect->top() + 10);
...
TEST_F(MediaControlsImplTest, HideControlsDefersStyleCalculationOnPlaying)
{
    ...
    SimulateOnPlaying();
    EXPECT_EQ(MediaControls().State(), MediaControlsImpl::ControlsState::kPlaying);
...
TEST_F(MediaControlsImplTest, HideControlsDefersStyleCalculationOnWaiting)
{
    ...
    SimulateOnWaiting();
    EXPECT_EQ(MediaControls().State(), MediaControlsImpl::ControlsState::kBuffering);
...
TEST_F(MediaControlsImplTest, OverflowMenuInPaintContainment)
{
    ...
    test::RunPendingTasks();
    UpdateAllLifecyclePhasesForTest();
    SetMediaControlsFromElement(To<HTMLMediaElement>(*page_holder->GetDocument().QuerySelector(AtomicString("audio"))));
