...
MediaControlsTextTrackManager::MediaControlsTextTrackManager(HTMLMediaElement& media_element)
    : media_element_(&media_element)
{
}

String MediaControlsTextTrackManager::GetTextTrackLabel(TextTrack* track) const
{
    if (!track) {
    ...
    if (track_label.empty()) {
        track_label = String(media_element_->GetLocale().QueryString(IDS_MEDIA_TRACKS_NO_LABEL, String::Number(track->TrackIndex() + 1)));
    }

    return track_label;
}

void MediaControlsTextTrackManager::ShowTextTrackAtIndex(unsigned index_to_enable)
{
