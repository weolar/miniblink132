...
ScriptPromise<DelegatedInkTrailPresenter> Ink::requestPresenter(ScriptState* state, InkPresenterParam* presenter_param)
{
    if (!state->ContextIsValid()) {
        V8ThrowException::ThrowError(state->GetIsolate(), "The object is no longer associated with a window.");
        return EmptyPromise();
    }

    if (presenter_param->presentationArea()
        && (presenter_param->presentationArea()->GetDocument() != GetSupplementable()->DomWindow()->GetFrame()->GetDocument())) {
        V8ThrowDOMException::Throw(state->GetIsolate(), DOMExceptionCode::kNotAllowedError, "Presentation area element does not belong to the document.");
        return EmptyPromise();
    }

    return ToResolvedPromise<DelegatedInkTrailPresenter>(
        state, MakeGarbageCollected<DelegatedInkTrailPresenter>(presenter_param->presentationArea(), GetSupplementable()->DomWindow()->GetFrame()));
}
