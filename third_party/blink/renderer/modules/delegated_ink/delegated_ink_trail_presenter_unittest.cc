...
public:
    ...
    explicit TestDelegatedInkMetadata(gfx::RectF area, float device_pixel_ratio = 1.0)
        : area_(area)
    {
...
DelegatedInkTrailPresenter* CreatePresenter(Element* element, LocalFrame* frame)
{
...
public:
    ...
    PointerEvent* CreatePointerMoveEvent(gfx::PointF pt, bool hovering)
    {
        ...
        if (!hovering) {
            init->setButtons(MouseEvent::WebInputEventModifiersToButtons(WebInputEvent::Modifiers::kLeftButtonDown));
        }
        init->setView(&Window());
        PointerEvent* event = PointerEvent::Create(event_type_names::kPointermove, init);
    ...
    {
        return TestDelegatedInkMetadata(WebView().MainFrameViewWidget()->LayerTreeHostForTesting()->DelegatedInkMetadataForTesting());
    }
...
class DelegatedInkTrailPresenterCanvasBeyondViewport : public DelegatedInkTrailPresenterUnitTest, public testing::WithParamInterface<bool> {
public:
    bool CanvasShouldBePastViewport()
    {
        return GetParam();
    }
    ...
    void SetWebViewSize()
    {
        DelegatedInkTrailPresenterUnitTest::SetWebViewSize(kViewportWidth, kViewportHeight);
    }
...
TEST_P(DelegatedInkTrailPresenterCanvasBeyondViewport, CollectAndPropagateMetadata)
{
    ...
    if (!CanvasShouldBePastViewport()) {
        ...
        SetWebViewSize();
        expected_metadata.SetArea(gfx::RectF(0, 0, GetViewportWidth(), GetViewportHeight()));
    }

    DelegatedInkTrailPresenter* presenter = CreatePresenter(GetDocument().getElementById(AtomicString("canvas")), GetDocument().GetFrame());
    DCHECK(presenter);
    ...
    presenter->updateInkTrailStartPoint(
        ToScriptStateForMainWorld(GetDocument().GetFrame()), CreatePointerMoveEvent(pt, /*hovering*/ true), style, exception_state);
...
TEST_P(DelegatedInkTrailPresenterCanvasBeyondViewport, NotDefaultDevicePixelRatio)
{
    ...
    if (!CanvasShouldBePastViewport()) {
        SetWebViewSizeGreaterThanCanvas(kCanvasWidth * kZoom, kCanvasHeight * kZoom);
        expected_metadata = TestDelegatedInkMetadata(gfx::RectF(0, 0, kCanvasWidth, kCanvasHeight), kZoom);
    } else {
        SetWebViewSize();
        expected_metadata.SetArea(gfx::RectF(0, 0, GetViewportWidth(), GetViewportHeight()));
    }
...
TEST_P(DelegatedInkTrailPresenterCanvasBeyondViewport, NotDefaultPageScaleFactor)
{
    SimRequest main_resource("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    main_resource.Complete(R"HTML(
    ...
    if (!CanvasShouldBePastViewport()) {
        SetWebViewSizeGreaterThanCanvas(kCanvasWidth * kScale, kCanvasHeight * kScale);
        expected_metadata = TestDelegatedInkMetadata(gfx::RectF(0, 0, kCanvasWidth, kCanvasHeight), kScale);
    } else {
        SetWebViewSize();
        expected_metadata.SetArea(gfx::RectF(0, 0, GetViewportWidth(), GetViewportHeight()));
    }
...
TEST_P(DelegatedInkTrailPresenterCanvasBeyondViewport, NotDefaultPageScaleFactorNonZeroOffset)
{
    SimRequest main_resource("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    main_resource.Complete(R"HTML(
    ...
    if (!CanvasShouldBePastViewport()) {
        SetWebViewSizeGreaterThanCanvas(kCanvasWidth * kScale, kCanvasHeight * kScale);
        expected_metadata = TestDelegatedInkMetadata(gfx::RectF(0, 0, kCanvasWidth - kOffsetX, kCanvasHeight - kOffsetY), kScale);
    } else {
        SetWebViewSize();
        expected_metadata.SetArea(gfx::RectF(0, 0, GetViewportWidth(), GetViewportHeight()));
    }
...
TEST_P(DelegatedInkTrailPresenterCanvasBeyondViewport, CanvasNotAtOrigin)
{
    ...
    if (!CanvasShouldBePastViewport()) {
        SetWebViewSizeGreaterThanCanvas(kCanvasWidth + kCanvasLeftOffset, kCanvasHeight + kCanvasTopOffset);
        expected_metadata.SetArea(gfx::RectF(kCanvasLeftOffset, kCanvasTopOffset, kCanvasWidth, kCanvasHeight));
    } else {
        SetWebViewSize();
        expected_metadata.SetArea(
            gfx::RectF(kCanvasLeftOffset, kCanvasTopOffset, GetViewportWidth() - kCanvasLeftOffset, GetViewportHeight() - kCanvasTopOffset));
    }
    ...
    presenter->updateInkTrailStartPoint(
        ToScriptStateForMainWorld(GetDocument().GetFrame()), CreatePointerMoveEvent(pt, /*hovering*/ false), style, exception_state);
...
TEST_P(DelegatedInkTrailPresenterCanvasBeyondViewport, CanvasInIFrame)
{
    ...
    if (!CanvasShouldBePastViewport()) {
        ...
        expected_metadata.SetArea(gfx::RectF(kIframeLeftOffset + kCanvasLeftOffset, kIframeTopOffset + kCanvasTopOffset, kCanvasWidth, kCanvasHeight));
    } else {
        SetWebViewSize();
        expected_metadata.SetArea(gfx::RectF(kIframeLeftOffset + kCanvasLeftOffset, kIframeTopOffset + kCanvasTopOffset,
            GetViewportWidth() - (kIframeLeftOffset + kCanvasLeftOffset), GetViewportHeight() - (kIframeTopOffset + kCanvasTopOffset)));
    }
    ...
    DelegatedInkTrailPresenter* presenter
        = CreatePresenter(iframe_localframe->GetDocument()->getElementById(AtomicString("canvas")), iframe_document->GetFrame());
    DCHECK(presenter);
    ...
    presenter->updateInkTrailStartPoint(
        ToScriptStateForMainWorld(iframe_document->GetFrame()), CreatePointerMoveEvent(pt, /*hovering*/ false), style, exception_state);
    expected_metadata.SetHovering(false);
    expected_metadata.SetPoint(gfx::PointF(pt.x() + kIframeLeftOffset, pt.y() + kIframeTopOffset));
...
TEST_P(DelegatedInkTrailPresenterCanvasBeyondViewport, NestedIframe)
{
    ...
    const float kInnerIframeLeftOffset = kOuterIframeLeftOffset + 18.f + kIframeBorder;
    const float kInnerIframeTopOffset = kOuterIframeTopOffset + 11.f + kIframeBorder;
    ...
    if (!CanvasShouldBePastViewport()) {
        ...
        expected_metadata.SetArea(
            gfx::RectF(kInnerIframeLeftOffset + kCanvasLeftOffset, kInnerIframeTopOffset + kCanvasTopOffset, kCanvasWidth, kCanvasHeight));
    } else {
        SetWebViewSize();
        expected_metadata.SetArea(gfx::RectF(kInnerIframeLeftOffset + kCanvasLeftOffset, kInnerIframeTopOffset + kCanvasTopOffset,
            GetViewportWidth() - (kInnerIframeLeftOffset + kCanvasLeftOffset), GetViewportHeight() - (kInnerIframeTopOffset + kCanvasTopOffset)));
    }

    auto* outer_iframe_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("OuterIframe")));
    auto* inner_iframe_element = To<HTMLIFrameElement>(outer_iframe_element->contentDocument()->getElementById(AtomicString("InnerIframe")));
    auto* iframe_localframe = To<LocalFrame>(inner_iframe_element->ContentFrame());
    ...
    presenter->updateInkTrailStartPoint(
        ToScriptStateForMainWorld(iframe_document->GetFrame()), CreatePointerMoveEvent(pt, /*hovering*/ true), style, exception_state);
    expected_metadata.SetHovering(true);
    expected_metadata.SetPoint(gfx::PointF(pt.x() + kInnerIframeLeftOffset, pt.y() + kInnerIframeTopOffset));
...
TEST_P(DelegatedInkTrailPresenterCanvasBeyondViewport, IFrameNoPresentationArea)
{
    SimRequest main_resource("https://example.com/test.html", "text/html");
    SimRequest frame_resource("https://example.com/iframe.html", "text/html");
    LoadURL("https://example.com/test.html");
    ...
    if (!CanvasShouldBePastViewport()) {
        SetWebViewSizeGreaterThanCanvas(kIframeWidth + kIframeLeftOffset, kIframeHeight + kIframeTopOffset);
        expected_metadata.SetArea(gfx::RectF(kIframeLeftOffset, kIframeTopOffset, kIframeWidth, kIframeHeight));
    } else {
        SetWebViewSize();
        expected_metadata.SetArea(
            gfx::RectF(kIframeLeftOffset, kIframeTopOffset, GetViewportWidth() - kIframeLeftOffset, GetViewportHeight() - kIframeTopOffset));
    }

    Document* iframe_document = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("iframe")))->contentDocument();

    DelegatedInkTrailPresenter* presenter = CreatePresenter(nullptr, iframe_document->GetFrame());
    DCHECK(presenter);
...
INSTANTIATE_TEST_SUITE_P(, DelegatedInkTrailPresenterCanvasBeyondViewport, testing::Bool());
...
TEST_F(DelegatedInkTrailPresenterUnitTest, PresentationAreaNotProvided)
{
    ...
    DelegatedInkTrailPresenter* presenter = CreatePresenter(nullptr, GetDocument().GetFrame());
    DCHECK(presenter);

    TestDelegatedInkMetadata expected_metadata(gfx::RectF(0, 0, kViewportWidth, kViewportHeight));
...
TEST_F(DelegatedInkTrailPresenterUnitTest, CanvasExtendsOutsideOfIframe)
{
    ...
    SetWebViewSize(kCanvasWidth + kIframeLeftOffset + 1, kCanvasHeight + kIframeTopOffset + 1);

    TestDelegatedInkMetadata expected_metadata(gfx::RectF(kIframeLeftOffset, kIframeTopOffset, kIframeWidth, kIframeHeight));
...
TEST_F(DelegatedInkTrailPresenterUnitTest, CanvasLeftAndAboveIframeBoundaries)
{
    ...
    SetWebViewSize(kIframeHeight + kIframeLeftOffset + 1, kIframeWidth + kIframeTopOffset + 1);

    TestDelegatedInkMetadata expected_metadata(
        gfx::RectF(kIframeLeftOffset, kIframeTopOffset, kCanvasWidth + kCanvasLeftOffset, kCanvasHeight + kCanvasTopOffset));
...
TEST_F(DelegatedInkTrailPresenterUnitTest, OuterIframeClipsInnerIframe)
{
    ...
    const float kOuterIframeHeight = 500.f;
    const float kOuterIframeWidth = 500.f;
    const float kInnerIframeLeftOffset = kOuterIframeLeftOffset + 334.f + kIframeBorder;
    const float kInnerIframeTopOffset = kOuterIframeTopOffset + 311.f + kIframeBorder;
    ...
    const float kViewportHeight = kOuterIframeHeight + kOuterIframeTopOffset + 1.f;
    SetWebViewSize(kViewportWidth, kViewportHeight);

    TestDelegatedInkMetadata expected_metadata(gfx::RectF(kInnerIframeLeftOffset + kCanvasLeftOffset, kInnerIframeTopOffset + kCanvasTopOffset,
        kOuterIframeWidth + kOuterIframeLeftOffset - kInnerIframeLeftOffset - kCanvasLeftOffset,
        kOuterIframeHeight + kOuterIframeTopOffset - kInnerIframeTopOffset - kCanvasTopOffset));
