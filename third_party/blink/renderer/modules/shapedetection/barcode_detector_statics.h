...
class BarcodeDetectorStatics final : public GarbageCollected<BarcodeDetectorStatics>, public Supplement<ExecutionContext> {
public:
    ...
    void CreateBarcodeDetection(
        mojo::PendingReceiver<shape_detection::mojom::blink::BarcodeDetection>, shape_detection::mojom::blink::BarcodeDetectorOptionsPtr);
    ScriptPromise<IDLSequence<V8BarcodeFormat>> EnumerateSupportedFormats(ScriptState*);

    void Trace(Visitor*) const override;

private:
    void EnsureServiceConnection();
    void OnEnumerateSupportedFormats(ScriptPromiseResolver<IDLSequence<V8BarcodeFormat>>*, const Vector<shape_detection::mojom::blink::BarcodeFormat>&);
    void OnConnectionError();

    HeapMojoRemote<shape_detection::mojom::blink::BarcodeDetectionProvider> service_;

    // Holds Promises returned by EnumerateSupportedFormats() so that they can be
    // resolve in the case of a Mojo connection error.
    HeapHashSet<Member<ScriptPromiseResolver<IDLSequence<V8BarcodeFormat>>>> get_supported_format_requests_;
};
