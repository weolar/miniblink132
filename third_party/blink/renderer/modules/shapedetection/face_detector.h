...
public:
    ...
    ScriptPromise<IDLSequence<DetectedFace>> detect(ScriptState* script_state, const V8ImageBitmapSource* image_source, ExceptionState&);

    void Trace(Visitor*) const override;

private:
    void OnDetectFaces(ScriptPromiseResolver<IDLSequence<DetectedFace>>*, Vector<shape_detection::mojom::blink::FaceDetectionResultPtr>);
    void OnFaceServiceConnectionError();

    HeapMojoRemote<shape_detection::mojom::blink::FaceDetection> face_service_;

    HeapHashSet<Member<ScriptPromiseResolver<IDLSequence<DetectedFace>>>> face_service_requests_;
};
