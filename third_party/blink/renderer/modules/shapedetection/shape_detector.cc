...
std::optional<SkBitmap> ShapeDetector::GetBitmapFromSource(ScriptState* script_state, const V8ImageBitmapSource* image_source, ExceptionState& exception_state)
{
    DCHECK(image_source);
    ...
    case V8ImageBitmapSource::ContentType::kImageData:
        ...
        return GetBitmapFromImageData(script_state, image_source->GetAsImageData(), exception_state);
    ...
    case V8ImageBitmapSource::ContentType::kVideoFrame:
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Unsupported source.");
        return std::nullopt;
    }
    DCHECK(canvas_image_source);

    if (canvas_image_source->IsNeutered()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The image source is detached.");
        return std::nullopt;
    }
    ...
    if (image_source->IsHTMLImageElement()) {
        return GetBitmapFromImageElement(script_state, image_source->GetAsHTMLImageElement(), exception_state);
    }
    ...
    const gfx::SizeF size = canvas_image_source->ElementSize(gfx::SizeF(), kRespectImageOrientation);

    SourceImageStatus source_image_status = kInvalidSourceImageStatus;
    scoped_refptr<Image> image = canvas_image_source->GetSourceImageForCanvas(FlushReason::kShapeDetector, &source_image_status, size, kPremultiplyAlpha);
    if (!image || source_image_status != kNormalSourceImageStatus) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Invalid element or state.");
        return std::nullopt;
    }
    if (size.IsEmpty()) {
        return SkBitmap();
    ...
    const sk_sp<SkImage> sk_image = image->PaintImageForCurrentFrame().GetSwSkImage();

    SkBitmap sk_bitmap;
    SkBitmap n32_bitmap;
    if (!sk_image || !sk_image->asLegacyBitmap(&sk_bitmap) || !skia::SkBitmapToN32OpaqueOrPremul(sk_bitmap, &n32_bitmap)) {
        // TODO(crbug.com/1467598): retrieve the pixels from elsewhere.
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Failed to get pixels for current frame.");
        return std::nullopt;
    }
...
std::optional<SkBitmap> ShapeDetector::GetBitmapFromImageData(ScriptState* script_state, ImageData* image_data, ExceptionState& exception_state)
{
    ...
    if (image_data->IsBufferBaseDetached()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The image data has been detached.");
        return std::nullopt;
    }
    ...
    if (!sk_bitmap.tryAllocPixels(image_data_pixmap.info().makeColorType(kN32_SkColorType),
            /*rowBytes=*/0)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Failed to allocate pixels for current frame.");
        return std::nullopt;
    }
    if (!sk_bitmap.writePixels(image_data_pixmap, 0, 0)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Failed to copy pixels for current frame.");
        return std::nullopt;
    }
...
std::optional<SkBitmap> ShapeDetector::GetBitmapFromImageElement(ScriptState* script_state, const HTMLImageElement* img, ExceptionState& exception_state)
{
    ImageResourceContent* const image_content = img->CachedImage();
    if (!image_content || !image_content->IsLoaded() || image_content->ErrorOccurred()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Failed to load or decode HTMLImageElement.");
        return std::nullopt;
    }

    if (!image_content->HasImage()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Failed to get image from resource.");
        return std::nullopt;
    }
    ...
    const sk_sp<SkImage> sk_image = blink_image->PaintImageForCurrentFrame().GetSwSkImage();
    ...
    if (!sk_image || !sk_image->asLegacyBitmap(&sk_bitmap)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Failed to get image from current frame.");
        return std::nullopt;
    }
