...
public:
    static BarcodeDetector* Create(ExecutionContext*, const BarcodeDetectorOptions*, ExceptionState& exception_state);

    // Barcode Detection API functions.
    static ScriptPromise<IDLSequence<V8BarcodeFormat>> getSupportedFormats(ScriptState*);

    static String BarcodeFormatToString(const shape_detection::mojom::BarcodeFormat format);

    ScriptPromise<IDLSequence<DetectedBarcode>> detect(ScriptState*, const V8ImageBitmapSource*, ExceptionState&);

    explicit BarcodeDetector(ExecutionContext*, const BarcodeDetectorOptions*, ExceptionState&);
    ~BarcodeDetector() override = default;

    void Trace(Visitor*) const override;

private:
    void OnDetectBarcodes(ScriptPromiseResolver<IDLSequence<DetectedBarcode>>*, Vector<shape_detection::mojom::blink::BarcodeDetectionResultPtr>);
