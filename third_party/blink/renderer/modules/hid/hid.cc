...
const char kFeaturePolicyBlocked[] = "Access to the feature \"hid\" is disallowed by permissions policy.";
...
bool IsContextSupported(ExecutionContext* context)
{
    ...
    DCHECK(!context->IsDedicatedWorkerGlobalScope() || RuntimeEnabledFeatures::WebHIDOnDedicatedWorkersEnabled());
    DCHECK(!context->IsServiceWorkerGlobalScope() || RuntimeEnabledFeatures::WebHIDOnServiceWorkersEnabled());
...
bool ShouldBlockHidServiceCall(LocalDOMWindow* window, ExecutionContext* context, ExceptionState* exception_state)
{
    ...
    if (security_origin->IsOpaque()) {
        if (exception_state) {
            exception_state->ThrowSecurityError("Access to the WebHID API is denied from contexts where the "
                                                "top-level "
    ...
    if (!context->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kHid, ReportOptions::kReportOnFailure)) {
        if (exception_state) {
...
void RejectWithTypeError(const String& message, ScriptPromiseResolverBase* resolver)
{
    ...
    , receiver_(this, navigator.GetExecutionContext())
{
    ...
    if (context) {
        feature_handle_for_scheduler_
            = context->GetScheduler()->RegisterFeature(SchedulingPolicy::Feature::kWebHID, { SchedulingPolicy::DisableBackForwardCache() });
    }
}
...
void HID::AddedEventListener(const AtomicString& event_type, RegisteredEventListener& listener)
{
    ...
    auto* context = GetExecutionContext();
    if (ShouldBlockHidServiceCall(GetSupplementable()->DomWindow(), context, nullptr)) {
        return;
    }
...
void HID::DeviceAdded(device::mojom::blink::HidDeviceInfoPtr device_info)
{
    ...
    DispatchEvent(*MakeGarbageCollected<HIDConnectionEvent>(event_type_names::kConnect, device));
}
...
void HID::DeviceRemoved(device::mojom::blink::HidDeviceInfoPtr device_info)
{
    ...
    DispatchEvent(*MakeGarbageCollected<HIDConnectionEvent>(event_type_names::kDisconnect, device));
}
...
ScriptPromise<IDLSequence<HIDDevice>> HID::getDevices(ScriptState* script_state, ExceptionState& exception_state)
{
    if (ShouldBlockHidServiceCall(GetSupplementable()->DomWindow(), GetExecutionContext(), &exception_state)) {
        return ScriptPromise<IDLSequence<HIDDevice>>();
    }

    auto* resolver = MakeGarbageCollected<HIDDeviceResolver>(script_state, exception_state.GetContext());
    get_devices_promises_.insert(resolver);

    EnsureServiceConnection();
    service_->GetDevices(WTF::BindOnce(&HID::FinishGetDevices, WrapPersistent(this), WrapPersistent(resolver)));
    return resolver->Promise();
}

ScriptPromise<IDLSequence<HIDDevice>> HID::requestDevice(ScriptState* script_state, const HIDDeviceRequestOptions* options, ExceptionState& exception_state)
{
    ...
    if (ShouldBlockHidServiceCall(window, GetExecutionContext(), &exception_state)) {
        return ScriptPromise<IDLSequence<HIDDevice>>();
    }
    ...
    if (options->hasExclusionFilters()) {
        if (options->exclusionFilters().size() == 0) {
            exception_state.ThrowTypeError("'exclusionFilters', if present, must contain at least one filter.");
            return ScriptPromise<IDLSequence<HIDDevice>>();
        }
        ...
        DCHECK_EQ(options->exclusionFilters().size(), mojo_exclusion_filters.size());
    }

    EnsureServiceConnection();
    service_->RequestDevice(
        std::move(mojo_filters), std::move(mojo_exclusion_filters), WTF::BindOnce(&HID::FinishRequestDevice, WrapPersistent(this), WrapPersistent(resolver)));
    return promise;
}

void HID::Connect(const String& device_guid, mojo::PendingRemote<device::mojom::blink::HidConnectionClient> client,
...
void HID::Forget(device::mojom::blink::HidDeviceInfoPtr device_info, mojom::blink::HidService::ForgetCallback callback)
{
    EnsureServiceConnection();
...
HIDDevice* HID::GetOrCreateDevice(device::mojom::blink::HidDeviceInfoPtr info)
{
    ...
    HIDDevice* device = MakeGarbageCollected<HIDDevice>(this, std::move(info), GetExecutionContext());
    device_cache_.insert(guid, device);
    return device;
}

void HID::FinishGetDevices(HIDDeviceResolver* resolver, Vector<device::mojom::blink::HidDeviceInfoPtr> device_infos)
{
...
void HID::FinishRequestDevice(HIDDeviceResolver* resolver, Vector<device::mojom::blink::HidDeviceInfoPtr> device_infos)
{
...
void HID::EnsureServiceConnection()
{
    ...
    service_.set_disconnect_handler(WTF::BindOnce(&HID::CloseServiceConnection, WrapWeakPersistent(this)));
...
mojom::blink::HidDeviceFilterPtr HID::ConvertDeviceFilter(const HIDDeviceFilter& filter)
{
    ...
    if (filter.hasVendorId()) {
        if (filter.hasProductId()) {
            mojo_filter->device_ids
                = mojom::blink::DeviceIdFilter::NewVendorAndProduct(mojom::blink::VendorAndProduct::New(filter.vendorId(), filter.productId()));
        } else {
            mojo_filter->device_ids = mojom::blink::DeviceIdFilter::NewVendor(filter.vendorId());
        }
    }
    if (filter.hasUsagePage()) {
        if (filter.hasUsage()) {
            mojo_filter->usage = mojom::blink::UsageFilter::NewUsageAndPage(device::mojom::blink::HidUsageAndPage::New(filter.usage(), filter.usagePage()));
        } else {
            mojo_filter->usage = mojom::blink::UsageFilter::NewPage(filter.usagePage());
        }
    }
    return mojo_filter;
}
...
{
    if (!filter.hasVendorId() && !filter.hasProductId() && !filter.hasUsagePage() && !filter.hasUsage()) {
