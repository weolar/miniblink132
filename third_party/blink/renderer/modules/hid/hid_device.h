...
class MODULES_EXPORT HIDDevice : public EventTarget,
...
public:
    ...
    public:
        virtual void Connect(const String& device_guid, mojo::PendingRemote<device::mojom::blink::HidConnectionClient> connection_client,
            device::mojom::blink::HidManager::ConnectCallback callback)
            = 0;
        virtual void Forget(device::mojom::blink::HidDeviceInfoPtr device_info, mojom::blink::HidService::ForgetCallback callback) = 0;
    };

    HIDDevice(ServiceInterface* parent, device::mojom::blink::HidDeviceInfoPtr info, ExecutionContext* execution_context);
    ~HIDDevice() override;
    ...
    ScriptPromise<IDLUndefined> open(ScriptState* script_state, ExceptionState& exception_state);
    ScriptPromise<IDLUndefined> close(ScriptState*);
    ScriptPromise<IDLUndefined> forget(ScriptState*, ExceptionState& exception_state);
    ScriptPromise<IDLUndefined> sendReport(ScriptState*, uint8_t report_id, const DOMArrayPiece& data);
    ScriptPromise<IDLUndefined> sendFeatureReport(ScriptState*, uint8_t report_id, const DOMArrayPiece& data);
    ScriptPromise<NotShared<DOMDataView>> receiveFeatureReport(ScriptState*, uint8_t report_id);
    ...
    static HIDReportItem* ToHIDReportItem(const device::mojom::blink::HidReportItem& report_item);

    void Trace(Visitor*) const override;

private:
    bool EnsureNoDeviceChangeInProgress(ScriptPromiseResolverBase* resolver) const;
    bool EnsureDeviceIsNotForgotten(ScriptPromiseResolverBase* resolver) const;

    void OnServiceConnectionError();

    void FinishOpen(ScriptPromiseResolver<IDLUndefined>*, mojo::PendingRemote<device::mojom::blink::HidConnection>);
    ...
    void FinishSendFeatureReport(ScriptPromiseResolver<IDLUndefined>*, bool success);
    void FinishReceiveFeatureReport(ScriptPromiseResolver<NotShared<DOMDataView>>*, bool success, const std::optional<Vector<uint8_t>>&);
    ...
    HeapMojoReceiver<device::mojom::blink::HidConnectionClient, HIDDevice> receiver_;
