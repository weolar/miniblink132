...
device::mojom::blink::HidReportItemPtr MakeReportItem()
{
    ...
    item->usages.push_back(device::mojom::blink::HidUsageAndPage::New(0x01, device::mojom::blink::kPageButton));
...
TEST(HIDDeviceTest, multiUsageItem)
{
    ...
    for (int i = 1; i < 9; ++i) {
        mojo_item->usages.push_back(device::mojom::blink::HidUsageAndPage::New(2 * i, device::mojom::blink::kPageButton));
    }
