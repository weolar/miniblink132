...
const char kReceiveFeatureReportFailed[] = "Failed to receive the feature report.";
const char kUnexpectedClose[] = "The device was closed unexpectedly.";
const char kArrayBufferTooBig[] = "The provided ArrayBuffer exceeds the maximum allowed size.";
...
bool IsProtectedReportType(const device::mojom::blink::HidUsageAndPage& hid_usage_and_page, ReportType report_type)
{
    ...
    if (usage == device::mojom::blink::kGenericDesktopPointer || usage == device::mojom::blink::kGenericDesktopMouse
        || usage == device::mojom::blink::kGenericDesktopKeyboard || usage == device::mojom::blink::kGenericDesktopKeypad) {
        return report_type != ReportType::kFeature;
    }

    if (usage >= device::mojom::blink::kGenericDesktopSystemControl && usage <= device::mojom::blink::kGenericDesktopSystemWarmRestart) {
        return true;
    }

    if (usage >= device::mojom::blink::kGenericDesktopSystemDock && usage <= device::mojom::blink::kGenericDesktopSystemDisplaySwap) {
        return true;
    }
...
uint32_t ConvertHidUsageAndPageToUint32(const device::mojom::blink::HidUsageAndPage& usage)
{
...
void UnpackUnitValues(uint32_t unit, String& unit_system, int8_t& length_exponent, int8_t& mass_exponent, int8_t& time_exponent, int8_t& temperature_exponent,
    int8_t& current_exponent, int8_t& luminous_intensity_exponent)
{
...
HIDReportInfo* ToHIDReportInfo(const device::mojom::blink::HidReportDescription& report_info)
{
...
HIDCollectionInfo* ToHIDCollectionInfo(const device::mojom::blink::HidCollectionInfo& collection)
{
...
HIDDevice::HIDDevice(ServiceInterface* parent, device::mojom::blink::HidDeviceInfoPtr info, ExecutionContext* context)
...
{
    DispatchEvent(*MakeGarbageCollected<HIDInputReportEvent>(event_type_names::kInputreport, this, report_id, buffer));
}
...
ScriptPromise<IDLUndefined> HIDDevice::open(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!GetExecutionContext()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, kContextGone);
        return EmptyPromise();
    }

    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state);
    auto promise = resolver->Promise();
    if (!EnsureNoDeviceChangeInProgress(resolver) || !EnsureDeviceIsNotForgotten(resolver)) {
        return promise;
    }

    if (opened()) {
        resolver->RejectWithDOMException(DOMExceptionCode::kInvalidStateError, "The device is already open.");
        return promise;
    }

    mojo::PendingRemote<device::mojom::blink::HidConnectionClient> client;
    receiver_.Bind(client.InitWithNewPipeAndPassReceiver(), ExecutionContext::From(script_state)->GetTaskRunner(TaskType::kMiscPlatformAPI));

    device_state_change_in_progress_ = true;
    device_requests_.insert(resolver);
    parent_->Connect(device_info_->guid, std::move(client), WTF::BindOnce(&HIDDevice::FinishOpen, WrapPersistent(this), WrapPersistent(resolver)));
    return promise;
}
...
ScriptPromise<IDLUndefined> HIDDevice::forget(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!GetExecutionContext()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, kContextGone);
        return EmptyPromise();
    ...
    device_state_change_in_progress_ = true;
    parent_->Forget(device_info_.Clone(), WTF::BindOnce(&HIDDevice::FinishForget, WrapPersistent(this), WrapPersistent(resolver)));
    return promise;
}

ScriptPromise<IDLUndefined> HIDDevice::sendReport(ScriptState* script_state, uint8_t report_id, const DOMArrayPiece& data)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state);
    auto promise = resolver->Promise();
    if (!EnsureNoDeviceChangeInProgress(resolver) || !EnsureDeviceIsNotForgotten(resolver)) {
    ...
    if (!opened()) {
        resolver->RejectWithDOMException(DOMExceptionCode::kInvalidStateError, kOpenRequired);
        return promise;
    }

    if (!base::CheckedNumeric<wtf_size_t>(data.ByteLength()).IsValid()) {
        resolver->RejectWithDOMException(DOMExceptionCode::kNotSupportedError, kArrayBufferTooBig);
        return promise;
    }
    ...
    device_requests_.insert(resolver);
    connection_->Write(report_id, vector, WTF::BindOnce(&HIDDevice::FinishSendReport, WrapPersistent(this), WrapPersistent(resolver)));
    return promise;
}

ScriptPromise<IDLUndefined> HIDDevice::sendFeatureReport(ScriptState* script_state, uint8_t report_id, const DOMArrayPiece& data)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state);
    auto promise = resolver->Promise();
    if (!EnsureNoDeviceChangeInProgress(resolver) || !EnsureDeviceIsNotForgotten(resolver)) {
    ...
    device_requests_.insert(resolver);
    connection_->SendFeatureReport(report_id, vector, WTF::BindOnce(&HIDDevice::FinishSendFeatureReport, WrapPersistent(this), WrapPersistent(resolver)));
    return promise;
}

ScriptPromise<NotShared<DOMDataView>> HIDDevice::receiveFeatureReport(ScriptState* script_state, uint8_t report_id)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<NotShared<DOMDataView>>>(script_state);
    auto promise = resolver->Promise();
    if (!EnsureNoDeviceChangeInProgress(resolver) || !EnsureDeviceIsNotForgotten(resolver)) {
        return promise;
    }
    ...
    if (!opened()) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, kOpenRequired));
        return promise;
    }

    device_requests_.insert(resolver);
    connection_->GetFeatureReport(report_id, WTF::BindOnce(&HIDDevice::FinishReceiveFeatureReport, WrapPersistent(this), WrapPersistent(resolver)));
    return promise;
}
...
bool HIDDevice::EnsureNoDeviceChangeInProgress(ScriptPromiseResolverBase* resolver) const
{
    if (device_state_change_in_progress_) {
        resolver->RejectWithDOMException(DOMExceptionCode::kInvalidStateError, kDeviceStateChangeInProgress);
        return false;
    }
    return true;
}

bool HIDDevice::EnsureDeviceIsNotForgotten(ScriptPromiseResolverBase* resolver) const
{
    if (device_is_forgotten_) {
        resolver->RejectWithDOMException(DOMExceptionCode::kInvalidStateError, kDeviceIsForgotten);
        return false;
    }
    return true;
}

void HIDDevice::FinishOpen(ScriptPromiseResolver<IDLUndefined>* resolver, mojo::PendingRemote<device::mojom::blink::HidConnection> connection)
{
    MarkRequestComplete(resolver);
    device_state_change_in_progress_ = false;

    if (connection && GetExecutionContext()) {
        connection_.Bind(std::move(connection), GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI));
        connection_.set_disconnect_handler(WTF::BindOnce(&HIDDevice::OnServiceConnectionError, WrapWeakPersistent(this)));
        resolver->Resolve();
    } else {
        // If the connection or the context is null, the open failed.
        receiver_.reset();
        resolver->RejectWithDOMException(DOMExceptionCode::kNotAllowedError, kOpenFailed);
    }
}
...
void HIDDevice::OnServiceConnectionError()
{
    for (auto& resolver : device_requests_) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, kUnexpectedClose));
    }
    device_requests_.clear();
}

void HIDDevice::FinishSendReport(ScriptPromiseResolver<IDLUndefined>* resolver, bool success)
{
    MarkRequestComplete(resolver);
    if (success) {
        resolver->Resolve();
    } else {
        resolver->RejectWithDOMException(DOMExceptionCode::kNotAllowedError, kSendReportFailed);
    }
}

void HIDDevice::FinishSendFeatureReport(ScriptPromiseResolver<IDLUndefined>* resolver, bool success)
{
    MarkRequestComplete(resolver);
    if (success) {
        resolver->Resolve();
    } else {
        resolver->RejectWithDOMException(DOMExceptionCode::kNotAllowedError, kSendFeatureReportFailed);
    }
}

void HIDDevice::FinishReceiveFeatureReport(ScriptPromiseResolver<NotShared<DOMDataView>>* resolver, bool success, const std::optional<Vector<uint8_t>>& data)
{
    MarkRequestComplete(resolver);
    if (success && data) {
        ...
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotAllowedError, kReceiveFeatureReportFailed));
    }
}
...
// static
HIDReportItem* HIDDevice::ToHIDReportItem(const device::mojom::blink::HidReportItem& report_item)
{
    ...
    result->setUnitExponent(UnitFactorExponentToInt(report_item.unit_exponent & 0x0f));
    ...
    if (report_item.is_range) {
        result->setUsageMinimum(ConvertHidUsageAndPageToUint32(*report_item.usage_minimum));
        result->setUsageMaximum(ConvertHidUsageAndPageToUint32(*report_item.usage_maximum));
    } else {
    ...
    UnpackUnitValues(report_item.unit, unit_system, unit_factor_length_exponent, unit_factor_mass_exponent, unit_factor_time_exponent,
        unit_factor_temperature_exponent, unit_factor_current_exponent, unit_factor_luminous_intensity_exponent);
    ...
    result->setUnitFactorLuminousIntensityExponent(unit_factor_luminous_intensity_exponent);
