...
class BackgroundFetchRegistration final : public EventTarget,
                                          public ActiveScriptWrappable<BackgroundFetchRegistration>,
                                          public blink::mojom::blink::BackgroundFetchRegistrationObserver {
    DEFINE_WRAPPERTYPEINFO();

public:
    BackgroundFetchRegistration(ServiceWorkerRegistration* service_worker_registration, mojom::blink::BackgroundFetchRegistrationPtr registration);

    ~BackgroundFetchRegistration() override;

    // BackgroundFetchRegistrationObserver implementation.
    void OnProgress(uint64_t upload_total, uint64_t uploaded, uint64_t download_total, uint64_t downloaded, mojom::BackgroundFetchResult result,
    ...
    // received, if any.
    void OnRequestCompleted(mojom::blink::FetchAPIRequestPtr request, mojom::blink::FetchAPIResponsePtr response) override;

    // Web Exposed attribute defined in the IDL file. Corresponds to the
    // |developer_id| used elsewhere in the codebase.
    String id() const;
    ScriptPromise<BackgroundFetchRecord> match(
        ScriptState* script_state, const V8RequestInfo* request, const CacheQueryOptions* options, ExceptionState& exception_state);
    ScriptPromise<IDLSequence<BackgroundFetchRecord>> matchAll(ScriptState* scrip_state, ExceptionState& exception_state);
    ...
    void UpdateUI(const String& in_title, const SkBitmap& in_icon, mojom::blink::BackgroundFetchRegistrationService::UpdateUICallback callback);

private:
    void DidAbort(ScriptPromiseResolver<IDLBoolean>* resolver, mojom::blink::BackgroundFetchError error);
    void MatchImpl(ScriptState* script_state, ScriptPromiseResolverBase*, const V8RequestInfo* request, mojom::blink::CacheQueryOptionsPtr cache_query_options,
        ExceptionState& exception_state, bool match_all);

    void DidGetMatchingRequests(ScriptPromiseResolverBase* resolver, bool return_all, Vector<mojom::blink::BackgroundFetchSettledFetchPtr> settled_fetches);

    // Updates the |record| with a |response|, if one is available, else marks
    // the |record|'s request as aborted or failed.
    void UpdateRecord(BackgroundFetchRecord* record, mojom::blink::FetchAPIResponsePtr& response);
    ...
    HeapMojoRemote<mojom::blink::BackgroundFetchRegistrationService> registration_service_;

    HeapMojoReceiver<blink::mojom::blink::BackgroundFetchRegistrationObserver, BackgroundFetchRegistration> observer_receiver_;
};
