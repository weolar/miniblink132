...
protected:
    ...
    Vector<mojom::blink::FetchAPIRequestPtr> CreateFetchAPIRequestVector(V8TestingScope& scope, const V8UnionRequestInfoOrRequestOrUSVStringSequence* requests)
    {
        return BackgroundFetchManager::CreateFetchAPIRequestVector(scope.GetScriptState(), requests, scope.GetExceptionState());
    }
...
TEST_F(BackgroundFetchManagerTest, SingleUSVString)
{
    ...
    auto* requests = MakeGarbageCollected<V8UnionRequestInfoOrRequestOrUSVStringSequence>(image_url.GetString());

    Vector<mojom::blink::FetchAPIRequestPtr> fetch_api_requests = CreateFetchAPIRequestVector(scope, requests);
...
TEST_F(BackgroundFetchManagerTest, SingleRequest)
{
    ...
    Request* request = Request::Create(scope.GetScriptState(), image_url.GetString(), request_init, scope.GetExceptionState());
    ASSERT_FALSE(scope.GetExceptionState().HadException());
    ASSERT_TRUE(request);

    auto* requests = MakeGarbageCollected<V8UnionRequestInfoOrRequestOrUSVStringSequence>(request);
...
TEST_F(BackgroundFetchManagerTest, Sequence)
{
    ...
    auto* image_request = MakeGarbageCollected<V8UnionRequestOrUSVString>(image_url.GetString());
    auto* icon_request = MakeGarbageCollected<V8UnionRequestOrUSVString>(icon_url.GetString());

    RequestInit* request_init = RequestInit::Create();
    request_init->setMethod("DELETE");
    Request* request = Request::Create(scope.GetScriptState(), cat_video_url.GetString(), request_init, scope.GetExceptionState());
    ASSERT_FALSE(scope.GetExceptionState().HadException());
    ASSERT_TRUE(request);

    auto* cat_video_request = MakeGarbageCollected<V8UnionRequestOrUSVString>(request);
    ...
    auto* requests = MakeGarbageCollected<V8UnionRequestInfoOrRequestOrUSVStringSequence>(request_sequence);
...
TEST_F(BackgroundFetchManagerTest, BlobsExtracted)
{
    ...
    RequestInit* request_init = RequestInit::Create();
    request_init->setMethod("POST");
    request_init->setBody(blink::ScriptValue(scope.GetIsolate(), ToV8Traits<IDLString>::ToV8(scope.GetScriptState(), body_text)));
    Request* image_request = Request::Create(scope.GetScriptState(), image_url.GetString(), request_init, scope.GetExceptionState());
    ASSERT_FALSE(scope.GetExceptionState().HadException());
    ...
    HeapVector<Member<V8UnionRequestOrUSVString>> request_sequence;
    request_sequence.push_back(MakeGarbageCollected<V8UnionRequestOrUSVString>(image_request));
    request_sequence.push_back(icon_request);
