...
class MODULES_EXPORT BackgroundFetchIconLoader final : public GarbageCollected<BackgroundFetchIconLoader> {
public:
    ...
    void Start(BackgroundFetchBridge* bridge, ExecutionContext* execution_context, HeapVector<Member<ManifestImageResource>> icons, IconCallback icon_callback);
...
private:
    ...
    void DidGetIconDisplaySizeIfSoLoadIcon(ExecutionContext* execution_context, IconCallback callback, const gfx::Size& icon_display_size_pixels);
    ...
    KURL PickBestIconForDisplay(ExecutionContext* execution_context, int ideal_size_pixels);
