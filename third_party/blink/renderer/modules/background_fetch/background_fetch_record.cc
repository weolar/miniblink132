...
BackgroundFetchRecord::BackgroundFetchRecord(Request* request, ScriptState* script_state)
    : request_(request)
    , script_state_(script_state)
{
    ...
    response_ready_property_ = MakeGarbageCollected<ResponseReadyProperty>(ExecutionContext::From(script_state));
}
...
{
    if (response_ready_property_->GetState() != ResponseReadyProperty::State::kPending) {
        return;
    }
    ...
    case State::kPending:
        return;
    case State::kAborted:
        response_ready_property_->Reject(
            MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "The fetch was aborted before the record was processed."));
        return;
    case State::kSettled:
        if (response) {
        ...
        response_ready_property_->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kUnknownError, "The response is not available."));
    }
}

ScriptPromise<Response> BackgroundFetchRecord::responseReady(ScriptState* script_state)
{
...
void BackgroundFetchRecord::UpdateState(BackgroundFetchRecord::State updated_state)
{
...
void BackgroundFetchRecord::SetResponseAndUpdateState(mojom::blink::FetchAPIResponsePtr& response)
{
...
void BackgroundFetchRecord::OnRequestCompleted(mojom::blink::FetchAPIResponsePtr response)
{
    if (!response.is_null())
