...
BackgroundFetchUpdateUIEvent::BackgroundFetchUpdateUIEvent(const AtomicString& type, const BackgroundFetchEventInit* initializer)
    : BackgroundFetchEvent(type, initializer, nullptr /* observer */)
{
}

BackgroundFetchUpdateUIEvent::BackgroundFetchUpdateUIEvent(
    const AtomicString& type, const BackgroundFetchEventInit* initializer, WaitUntilObserver* observer, ServiceWorkerRegistration* registration)
...
ScriptPromise<IDLUndefined> BackgroundFetchUpdateUIEvent::updateUI(
    ScriptState* script_state, const BackgroundFetchUIOptions* ui_options, ExceptionState& exception_state)
{
    if (observer_ && !observer_->IsEventActive()) {
        // Return a rejected promise as the event is no longer active.
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "ExtendableEvent is no longer active.");
        return EmptyPromise();
    }
    if (update_ui_called_) {
        // Return a rejected promise as this method should only be called once.
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "updateUI may only be called once.");
        return EmptyPromise();
    }
    ...
    if (ui_options->icons().empty()) {
        DidGetIcon(ui_options->title(), resolver, SkBitmap(), -1 /* ideal_to_chosen_icon_size */);
    } else {
        DCHECK(!loader_);
        loader_ = MakeGarbageCollected<BackgroundFetchIconLoader>();
        DCHECK(loader_);
        loader_->Start(BackgroundFetchBridge::From(service_worker_registration_), ExecutionContext::From(script_state), ui_options->icons(),
            resolver->WrapCallbackInScriptScope(WTF::BindOnce(&BackgroundFetchUpdateUIEvent::DidGetIcon, WrapPersistent(this), ui_options->title())));
    }
...
void BackgroundFetchUpdateUIEvent::DidGetIcon(
    const String& title, ScriptPromiseResolver<IDLUndefined>* resolver, const SkBitmap& icon, int64_t ideal_to_chosen_icon_size)
{
    registration()->UpdateUI(title, icon, resolver->WrapCallbackInScriptScope(WTF::BindOnce(&BackgroundFetchUpdateUIEvent::DidUpdateUI, WrapPersistent(this))));
}

void BackgroundFetchUpdateUIEvent::DidUpdateUI(ScriptPromiseResolver<IDLUndefined>* resolver, mojom::blink::BackgroundFetchError error)
{
    switch (error) {
    ...
    case mojom::blink::BackgroundFetchError::STORAGE_ERROR:
        resolver->RejectWithDOMException(DOMExceptionCode::kAbortError, "Failed to update UI due to I/O error.");
        return;
