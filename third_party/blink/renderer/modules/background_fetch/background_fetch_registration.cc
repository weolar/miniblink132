...
BackgroundFetchRegistration::BackgroundFetchRegistration(
    ServiceWorkerRegistration* service_worker_registration, mojom::blink::BackgroundFetchRegistrationPtr registration)
    ...
    , observer_receiver_(this, service_worker_registration->GetExecutionContext())
{
    ...
    registration_service_.Bind(std::move(registration->registration_interface), std::move(service_task_runner));

    auto observer_task_runner = context->GetTaskRunner(TaskType::kBackgroundFetch);
    registration_service_->AddRegistrationObserver(observer_receiver_.BindNewPipeAndPassRemote(observer_task_runner));
}

BackgroundFetchRegistration::~BackgroundFetchRegistration() = default;

void BackgroundFetchRegistration::OnProgress(uint64_t upload_total, uint64_t uploaded, uint64_t download_total, uint64_t downloaded,
    mojom::BackgroundFetchResult result, mojom::BackgroundFetchFailureReason failure_reason)
{
...
void BackgroundFetchRegistration::OnRequestCompleted(mojom::blink::FetchAPIRequestPtr request, mojom::blink::FetchAPIResponsePtr response)
{
...
ScriptPromise<IDLBoolean> BackgroundFetchRegistration::abort(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    registration_service_->Abort(resolver->WrapCallbackInScriptScope(WTF::BindOnce(&BackgroundFetchRegistration::DidAbort, WrapPersistent(this))));
...
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<BackgroundFetchRecord>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();
    MatchImpl(script_state, resolver, request, mojom::blink::CacheQueryOptions::From(options), exception_state,
        /* match_all = */ false);
    return promise;
}

ScriptPromise<IDLSequence<BackgroundFetchRecord>> BackgroundFetchRegistration::matchAll(ScriptState* script_state, ExceptionState& exception_state)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<BackgroundFetchRecord>>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();
...
ScriptPromise<IDLSequence<BackgroundFetchRecord>> BackgroundFetchRegistration::matchAll(
    ScriptState* script_state, const V8RequestInfo* request, const CacheQueryOptions* options, ExceptionState& exception_state)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<BackgroundFetchRecord>>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();
...
void BackgroundFetchRegistration::MatchImpl(ScriptState* script_state, ScriptPromiseResolverBase* resolver, const V8RequestInfo* request,
    mojom::blink::CacheQueryOptionsPtr cache_query_options, ExceptionState& exception_state, bool match_all)
{
    DCHECK(script_state);
    ...
    if (request) {
        ...
        case V8RequestInfo::ContentType::kUSVString: {
            Request* new_request = Request::Create(script_state, request->GetAsUSVString(), exception_state);
    ...
    DCHECK(registration_);
    DCHECK(registration_service_);
    registration_service_->MatchRequests(std::move(request_to_match), std::move(cache_query_options), match_all,
        WTF::BindOnce(&BackgroundFetchRegistration::DidGetMatchingRequests, WrapPersistent(this), WrapPersistent(resolver), match_all));
}

void BackgroundFetchRegistration::DidGetMatchingRequests(
    ScriptPromiseResolverBase* resolver, bool return_all, Vector<mojom::blink::BackgroundFetchSettledFetchPtr> settled_fetches)
{
    DCHECK(resolver);
    ...
    for (auto& fetch : settled_fetches) {
        Request* request = Request::Create(script_state, std::move(fetch->request), Request::ForServiceWorkerFetchEvent::kFalse);
        auto* record = MakeGarbageCollected<BackgroundFetchRecord>(request, script_state);
    ...
    resolver->DowncastTo<IDLSequence<BackgroundFetchRecord>>()->Resolve(to_return);
}

void BackgroundFetchRegistration::UpdateRecord(BackgroundFetchRecord* record, mojom::blink::FetchAPIResponsePtr& response)
{
    DCHECK(record);
...
{
    return failure_reason_ == mojom::BackgroundFetchFailureReason::CANCELLED_FROM_UI
        || failure_reason_ == mojom::BackgroundFetchFailureReason::CANCELLED_BY_DEVELOPER;
}

void BackgroundFetchRegistration::DidAbort(ScriptPromiseResolver<IDLBoolean>* resolver, mojom::blink::BackgroundFetchError error)
{
    switch (error) {
    ...
    case mojom::blink::BackgroundFetchError::STORAGE_ERROR:
        resolver->RejectWithDOMException(DOMExceptionCode::kAbortError, "Failed to abort registration due to I/O error.");
        return;
...
V8BackgroundFetchFailureReason BackgroundFetchRegistration::failureReason() const
{
    blink::IdentifiabilityMetricBuilder(GetExecutionContext()->UkmSourceID())
        .Add(blink::IdentifiableSurface::FromTypeAndToken(
                 blink::IdentifiableSurface::Type::kWebFeature, WebFeature::kV8BackgroundFetchRegistration_FailureReason_AttributeGetter),
            failure_reason_ == mojom::BackgroundFetchFailureReason::QUOTA_EXCEEDED)
        .Record(GetExecutionContext()->UkmRecorder());
    switch (failure_reason_) {
    case mojom::BackgroundFetchFailureReason::NONE:
        return V8BackgroundFetchFailureReason(V8BackgroundFetchFailureReason::Enum::k);
    case mojom::BackgroundFetchFailureReason::CANCELLED_FROM_UI:
    case mojom::BackgroundFetchFailureReason::CANCELLED_BY_DEVELOPER:
        return V8BackgroundFetchFailureReason(V8BackgroundFetchFailureReason::Enum::kAborted);
    case mojom::BackgroundFetchFailureReason::BAD_STATUS:
        return V8BackgroundFetchFailureReason(V8BackgroundFetchFailureReason::Enum::kBadStatus);
    case mojom::BackgroundFetchFailureReason::SERVICE_WORKER_UNAVAILABLE:
    case mojom::BackgroundFetchFailureReason::FETCH_ERROR:
        return V8BackgroundFetchFailureReason(V8BackgroundFetchFailureReason::Enum::kFetchError);
    case mojom::BackgroundFetchFailureReason::QUOTA_EXCEEDED:
        return V8BackgroundFetchFailureReason(V8BackgroundFetchFailureReason::Enum::kQuotaExceeded);
    case mojom::BackgroundFetchFailureReason::DOWNLOAD_TOTAL_EXCEEDED:
        return V8BackgroundFetchFailureReason(V8BackgroundFetchFailureReason::Enum::kDownloadTotalExceeded);
    }
    NOTREACHED();
}
...
void BackgroundFetchRegistration::UpdateUI(
    const String& in_title, const SkBitmap& in_icon, mojom::blink::BackgroundFetchRegistrationService::UpdateUICallback callback)
{
    DCHECK(registration_service_);
