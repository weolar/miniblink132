...
enum class BackgroundFetchLoadState { kNotLoaded, kLoadFailed, kLoadSuccessful };
...
constexpr char kBackgroundFetchImageLoaderIcon500x500FullPath[] = "http://test.com/500x500.png";
...
public:
    ...
    WebURL RegisterMockedURL(const String& file_name)
    {
        WebURL registered_url = url_test_helpers::RegisterMockedURLLoadFromBase(
            kBackgroundFetchImageLoaderBaseUrl, test::CoreTestDataPath(kBackgroundFetchImageLoaderBaseDir), file_name, "image/png");
    ...
    void IconLoaded(base::OnceClosure quit_closure, const SkBitmap& bitmap, int64_t ideal_to_chosen_icon_size)
    {
        bitmap_ = bitmap;
    ...
    ManifestImageResource* CreateTestIcon(const String& url_str, const String& size)
    {
    ...
    KURL PickRightIcon(HeapVector<Member<ManifestImageResource>> icons, const gfx::Size& ideal_display_size)
    {
        loader_->icons_ = std::move(icons);

        return loader_->PickBestIconForDisplay(GetContext(), ideal_display_size.height());
    }

    void LoadIcon(
        const KURL& url, const gfx::Size& maximum_size, base::OnceClosure quit_closure, const String& sizes = "500x500", const String& purpose = "ANY")
    {
        ManifestImageResource* icon = ManifestImageResource::Create();
        ...
        loader_->DidGetIconDisplaySizeIfSoLoadIcon(
            GetContext(), WTF::BindOnce(&BackgroundFetchIconLoaderTest::IconLoaded, WTF::Unretained(this), std::move(quit_closure)), maximum_size);
    }
...
TEST_F(BackgroundFetchIconLoaderTest, SuccessTest)
{
    ...
    LoadIcon(KURL(kBackgroundFetchImageLoaderIcon500x500FullPath), maximum_size, run_loop.QuitClosure());
...
TEST_F(BackgroundFetchIconLoaderTest, PickIconRelativePath)
{
    ...
    icons.push_back(CreateTestIcon(kBackgroundFetchImageLoaderIcon500x500, "500x500"));
...
{
    HeapVector<Member<ManifestImageResource>> icons;
    icons.push_back(CreateTestIcon(kBackgroundFetchImageLoaderIcon500x500FullPath, "500x500"));
...
{
    ManifestImageResource* icon0 = CreateTestIcon(kBackgroundFetchImageLoaderIcon500x500, "500x500");
    ManifestImageResource* icon1 = CreateTestIcon(kBackgroundFetchImageLoaderIcon48x48, "48x48");
    ManifestImageResource* icon2 = CreateTestIcon(kBackgroundFetchImageLoaderIcon3000x2000, "3000x2000");
    ...
    EXPECT_EQ(best_icon, KURL(KURL(kBackgroundFetchImageLoaderBaseUrl), kBackgroundFetchImageLoaderIcon48x48));
}
...
TEST_F(BackgroundFetchIconLoaderTest, EmptySizes)
{
    ...
    gfx::Size maximum_size { 192, 168 };
    LoadIcon(KURL(kBackgroundFetchImageLoaderIcon500x500FullPath), maximum_size, run_loop.QuitClosure(), "", "ANY");
...
TEST_F(BackgroundFetchIconLoaderTest, EmptyPurpose)
{
    ...
    gfx::Size maximum_size { 192, 168 };
    LoadIcon(KURL(kBackgroundFetchImageLoaderIcon500x500FullPath), maximum_size, run_loop.QuitClosure(), "500X500", "");
