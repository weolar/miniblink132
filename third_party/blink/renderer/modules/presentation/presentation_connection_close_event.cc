...
PresentationConnectionCloseEvent::PresentationConnectionCloseEvent(
    const AtomicString& event_type, const V8PresentationConnectionCloseReason& reason, const String& message)
...
PresentationConnectionCloseEvent::PresentationConnectionCloseEvent(const AtomicString& event_type, const PresentationConnectionCloseEventInit* initializer)
    : Event(event_type, initializer)
    , reason_(initializer->reason())
    , message_(initializer->hasMessage() ? initializer->message() : g_empty_string)
{
}
