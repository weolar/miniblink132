...
public:
    PresentationConnectionCallbacks(ScriptPromiseResolver<PresentationConnection>*, PresentationRequest*);
    PresentationConnectionCallbacks(ScriptPromiseResolver<PresentationConnection>*, ControllerPresentationConnection*);

    PresentationConnectionCallbacks(const PresentationConnectionCallbacks&) = delete;
    PresentationConnectionCallbacks& operator=(const PresentationConnectionCallbacks&) = delete;

    ~PresentationConnectionCallbacks() = default;

    void HandlePresentationResponse(mojom::blink::PresentationConnectionResultPtr, mojom::blink::PresentationErrorPtr);
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(PresentationConnectionCallbacksTest, HandleReconnect);

    void OnSuccess(const mojom::blink::PresentationInfo&, mojo::PendingRemote<mojom::blink::PresentationConnection> connection_remote,
