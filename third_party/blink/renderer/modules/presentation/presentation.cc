...
{
    return window->GetFrame()->IsMainFrame() && !window->GetFrame()->IsInFencedFrameTree();
}
...
void Presentation::setDefaultRequest(PresentationRequest* request)
{
    ...
    controller->GetPresentationService()->SetDefaultPresentationUrls(request ? request->Urls() : WTF::Vector<KURL>());
}

void Presentation::MaybeInitReceiver()
{
    LocalDOMWindow* window = GetSupplementable()->DomWindow();
    if (!receiver_ && window && IsOutermostDocument(window) && window->GetFrame()->GetSettings()->GetPresentationReceiver()) {
