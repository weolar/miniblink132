...
static constexpr char kNotSupportedErrorInfo[] = "getAvailability() isn't supported at the moment. It can be due to "
                                                 "a permanent or temporary system limitation. It is recommended to "
                                                 "try to blindly start a presentation in that case.";

} // namespace

PresentationAvailabilityState::PresentationAvailabilityState(mojom::blink::PresentationService* presentation_service)
...
void PresentationAvailabilityState::RequestAvailability(const Vector<KURL>& urls, ScriptPromiseResolver<PresentationAvailability>* resolver)
{
    ...
    if (screen_availability == mojom::blink::ScreenAvailability::DISABLED) {
        resolver->RejectWithDOMException(DOMExceptionCode::kNotSupportedError, kNotSupportedErrorInfo);
        return;
    }
    ...
    if (screen_availability != mojom::blink::ScreenAvailability::UNKNOWN) {
        resolver->Resolve(
            PresentationAvailability::Take(resolver->GetExecutionContext(), urls, screen_availability == mojom::blink::ScreenAvailability::AVAILABLE));
    } else {
...
void PresentationAvailabilityState::AddObserver(PresentationAvailabilityObserver* observer)
{
...
void PresentationAvailabilityState::RemoveObserver(PresentationAvailabilityObserver* observer)
{
    const auto& urls = observer->Urls();
    auto* listener = GetAvailabilityListener(urls);
    if (!listener) {
...
void PresentationAvailabilityState::UpdateAvailability(const KURL& url, mojom::blink::ScreenAvailability availability)
{
    ...
    for (auto& listener : listeners) {
        ...
        HeapVector<Member<PresentationAvailabilityObserver>> observers = listener->availability_observers;
        ...
        if (screen_availability == mojom::blink::ScreenAvailability::DISABLED) {
            for (auto& resolver_ptr : listener->availability_resolvers) {
                resolver_ptr->RejectWithDOMException(DOMExceptionCode::kNotSupportedError, kNotSupportedErrorInfo);
            }
        } else {
            for (auto& resolver_ptr : listener->availability_resolvers) {
                resolver_ptr->Resolve(PresentationAvailability::Take(
                    resolver_ptr->GetExecutionContext(), listener->urls, screen_availability == mojom::blink::ScreenAvailability::AVAILABLE));
            }
        }
...
void PresentationAvailabilityState::MaybeStopListeningToURL(const KURL& url)
{
    for (const auto& listener : availability_listeners_) {
        ...
        if (!listener->availability_resolvers.empty() || !listener->availability_observers.empty()) {
            return;
        }
    }
...
mojom::blink::ScreenAvailability PresentationAvailabilityState::GetScreenAvailability(const Vector<KURL>& urls) const
{
    ...
    for (const auto& url : urls) {
        ...
        auto screen_availability = status ? status->last_known_availability : mojom::blink::ScreenAvailability::UNKNOWN;
...
PresentationAvailabilityState::AvailabilityListener* PresentationAvailabilityState::GetAvailabilityListener(const Vector<KURL>& urls)
{
    auto listener_it = base::ranges::find(availability_listeners_, urls, &AvailabilityListener::urls);
    return listener_it == availability_listeners_.end() ? nullptr : *listener_it;
}

void PresentationAvailabilityState::TryRemoveAvailabilityListener(AvailabilityListener* listener)
{
    // URL is still observed by some availability object.
    if (!listener->availability_resolvers.empty() || !listener->availability_observers.empty()) {
        return;
...
PresentationAvailabilityState::ListeningStatus* PresentationAvailabilityState::GetListeningStatus(const KURL& url) const
{
    auto status_it = base::ranges::find(availability_listening_status_, url, &ListeningStatus::url);
    return status_it == availability_listening_status_.end() ? nullptr : status_it->get();
}

PresentationAvailabilityState::AvailabilityListener::AvailabilityListener(const Vector<KURL>& availability_urls)
    : urls(availability_urls)
{
}

PresentationAvailabilityState::AvailabilityListener::~AvailabilityListener() = default;

void PresentationAvailabilityState::AvailabilityListener::Trace(blink::Visitor* visitor) const
{
    visitor->Trace(availability_resolvers);
    visitor->Trace(availability_observers);
}

PresentationAvailabilityState::ListeningStatus::ListeningStatus(const KURL& availability_url)
