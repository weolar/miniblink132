...
{
    PresentationRequest* request = PresentationRequest::Create(scope->GetExecutionContext(), kPresentationUrl, scope->GetExceptionState());
...
TEST(PresentationConnectionCallbacksTest, HandleSuccess)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<PresentationConnection>>(scope.GetScriptState());
    ScriptPromiseTester promise_tester(scope.GetScriptState(), resolver->Promise());
    ...
    mojo::PendingReceiver<mojom::blink::PresentationConnection> connection_receiver = connection_remote.InitWithNewPipeAndPassReceiver();
    PresentationConnectionResultPtr result = PresentationConnectionResult::New(
        PresentationInfo::New(url_test_helpers::ToKURL(kPresentationUrl), kPresentationId), std::move(connection_remote), std::move(connection_receiver));
...
TEST(PresentationConnectionCallbacksTest, HandleReconnect)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    PresentationInfoPtr info = PresentationInfo::New(url_test_helpers::ToKURL(kPresentationUrl), kPresentationId);
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<PresentationConnection>>(scope.GetScriptState());
    ScriptPromiseTester promise_tester(scope.GetScriptState(), resolver->Promise());

    auto* connection = ControllerPresentationConnection::Take(resolver, *info, MakeRequest(&scope));
    ...
    mojo::PendingRemote<mojom::blink::PresentationConnection> connection_remote;
    mojo::PendingReceiver<mojom::blink::PresentationConnection> connection_receiver = connection_remote.InitWithNewPipeAndPassReceiver();
    PresentationConnectionResultPtr result = PresentationConnectionResult::New(std::move(info), std::move(connection_remote), std::move(connection_receiver));

    callbacks.HandlePresentationResponse(std::move(result), nullptr);

    // Previous connection was returned.
    ControllerPresentationConnection* new_connection = callbacks.connection_.Get();
    EXPECT_EQ(connection, new_connection);
    EXPECT_EQ(new_connection->GetState(), PresentationConnectionState::CONNECTING);
...
TEST(PresentationConnectionCallbacksTest, HandleError)
{
    ...
    callbacks.HandlePresentationResponse(nullptr, PresentationError::New(PresentationErrorType::NO_PRESENTATION_FOUND, "Something bad happened"));
