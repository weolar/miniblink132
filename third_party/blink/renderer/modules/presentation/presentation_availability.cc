...
// static
PresentationAvailability* PresentationAvailability::Take(ExecutionContext* context, const WTF::Vector<KURL>& urls, bool value)
{
    PresentationAvailability* presentation_availability = MakeGarbageCollected<PresentationAvailability>(context, urls, value);
    presentation_availability->UpdateStateIfNeeded();
    presentation_availability->UpdateListening();
    return presentation_availability;
}

PresentationAvailability::PresentationAvailability(ExecutionContext* execution_context, const WTF::Vector<KURL>& urls, bool value)
    : ActiveScriptWrappable<PresentationAvailability>({})
    , ExecutionContextLifecycleStateObserver(execution_context)
    , PageVisibilityObserver(To<LocalDOMWindow>(execution_context)->GetFrame()->GetPage())
    , urls_(urls)
    , value_(value)
...
void PresentationAvailability::AddedEventListener(const AtomicString& event_type, RegisteredEventListener& registered_listener)
{
    EventTarget::AddedEventListener(event_type, registered_listener);
    if (event_type == event_type_names::kChange) {
        UseCounter::Count(GetExecutionContext(), WebFeature::kPresentationAvailabilityChangeEventListener);
    }
}

void PresentationAvailability::AvailabilityChanged(blink::mojom::ScreenAvailability availability)
{
...
void PresentationAvailability::ContextLifecycleStateChanged(mojom::FrameLifecycleState state)
{
...
void PresentationAvailability::UpdateListening()
{
    ...
    if (state_ == State::kActive && (To<LocalDOMWindow>(GetExecutionContext())->document()->IsPageVisible()))
