...
Member<V8UnionPresentationSourceOrUSVString> CreatePresentationSource(const String& url)
{
...
HeapVector<Member<V8UnionPresentationSourceOrUSVString>> CreateUrlSources(const WTF::Vector<String>& urls)
{
    HeapVector<Member<V8UnionPresentationSourceOrUSVString>> sources;
    for (const String& url : urls) {
        sources.push_back(MakeGarbageCollected<V8UnionPresentationSourceOrUSVString>(url));
    }
    return sources;
}
...
TEST(PresentationRequestTest, TestMultipleUrlConstructor)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    HeapVector<Member<V8UnionPresentationSourceOrUSVString>> sources = CreateUrlSources({ "https://example.com", "cast://deadbeef?param=foo" });

    PresentationRequest* request = PresentationRequest::Create(scope.GetExecutionContext(), sources, scope.GetExceptionState());
    ASSERT_FALSE(scope.GetExceptionState().HadException());
...
TEST(PresentationRequestTest, TestMultipleUrlConstructorInvalidUrl)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    HeapVector<Member<V8UnionPresentationSourceOrUSVString>> sources = CreateUrlSources({ "https://example.com", "" });

    PresentationRequest::Create(scope.GetExecutionContext(), sources, scope.GetExceptionState());
    EXPECT_THAT(scope.GetExceptionState(), HadException(DOMExceptionCode::kSyntaxError));
}
...
TEST(PresentationRequestTest, TestMixedContentNotCheckedForNonHttpFamily)
{
    ...
    PresentationRequest* request = PresentationRequest::Create(scope.GetExecutionContext(), "cast://deadbeef?param=foo", scope.GetExceptionState());
    ASSERT_FALSE(scope.GetExceptionState().HadException());
...
TEST(PresentationRequestTest, TestSingleUrlConstructorMixedContent)
{
    ...
    PresentationRequest::Create(scope.GetExecutionContext(), "http://example.com", scope.GetExceptionState());
    EXPECT_THAT(scope.GetExceptionState(), HadException(DOMExceptionCode::kSecurityError));
}
...
TEST(PresentationRequestTest, TestMultipleUrlConstructorMixedContent)
{
    ...
    HeapVector<Member<V8UnionPresentationSourceOrUSVString>> sources = CreateUrlSources({ "http://example.com", "https://example1.com" });
...
TEST(PresentationRequestTest, TestMultipleUrlConstructorEmptySequence)
{
    ...
    PresentationRequest::Create(scope.GetExecutionContext(), sources, scope.GetExceptionState());
    EXPECT_THAT(scope.GetExceptionState(), HadException(DOMExceptionCode::kNotSupportedError));
}
...
TEST(PresentationRequestTest, TestSingleUrlConstructorUnknownScheme)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    PresentationRequest::Create(scope.GetExecutionContext(), "foobar:unknown", scope.GetExceptionState());
    EXPECT_THAT(scope.GetExceptionState(), HadException(DOMExceptionCode::kNotSupportedError));
}
...
TEST(PresentationRequestTest, TestMultipleUrlConstructorSomeUnknownSchemes)
{
    ...
    V8TestingScope scope;
    HeapVector<Member<V8UnionPresentationSourceOrUSVString>> sources
        = CreateUrlSources({ "foobar:unknown", "https://example.com", "cast://deadbeef?param=foo", "deadbeef:random" });
...
TEST(PresentationRequestTest, TestMultipleUrlConstructorAllUnknownSchemes)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    HeapVector<Member<V8UnionPresentationSourceOrUSVString>> sources = CreateUrlSources({ "foobar:unknown", "deadbeef:random" });
...
TEST(PresentationRequestTest, TestPresentationSourceNotAllowed)
{
    ...
    V8TestingScope scope;
    PresentationRequest::Create(scope.GetExecutionContext(), { CreatePresentationSource("https://example.com") }, scope.GetExceptionState());
    EXPECT_THAT(scope.GetExceptionState(), HadException(DOMExceptionCode::kNotSupportedError));
}
...
TEST(PresentationRequestTest, TestPresentationSourcesInConstructor)
{
    ...
    PresentationRequest* request = PresentationRequest::Create(
        scope.GetExecutionContext(), { CreatePresentationSource("https://example.com"), CreateMirroringSource() }, scope.GetExceptionState());
    CHECK(request);
    ASSERT_THAT(scope.GetExceptionState(), HadNoException());
    ...
    EXPECT_EQ("cast:0F5096E8?streamingCaptureAudio=1&streamingTargetPlayoutDelayMillis="
              "400",
...
TEST(PresentationRequestTest, TestInvalidPresentationSource)
{
    ...
    ScopedSiteInitiatedMirroringForTest site_initiated_mirroring_enabled { true };
    V8TestingScope scope;
    PresentationRequest::Create(scope.GetExecutionContext(), { CreatePresentationSource("invalid_url") }, scope.GetExceptionState());
    EXPECT_THAT(scope.GetExceptionState(), HadException(DOMExceptionCode::kNotSupportedError));
}
