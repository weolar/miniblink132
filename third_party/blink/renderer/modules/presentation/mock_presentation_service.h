...
public:
    void SetController(mojo::PendingRemote<mojom::blink::PresentationController>) override
    {
    }
    void SetReceiver(mojo::PendingRemote<mojom::blink::PresentationReceiver>) override
    {
    }
    ...
    void StartPresentation(const Vector<KURL>& presentation_urls, StartPresentationCallback callback) override
    {
        StartPresentationInternal(presentation_urls, callback);
    }
    MOCK_METHOD2(StartPresentationInternal, void(const Vector<KURL>&, StartPresentationCallback&));

    void ReconnectPresentation(const Vector<KURL>& presentation_urls, const String& presentation_id, ReconnectPresentationCallback callback) override
    {
        ReconnectPresentationInternal(presentation_urls, presentation_id, callback);
    }
    MOCK_METHOD3(ReconnectPresentationInternal, void(const Vector<KURL>& presentation_urls, const String& presentation_id, ReconnectPresentationCallback&));
