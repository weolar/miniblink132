...
// static
void PresentationMetrics::RecordPresentationConnectionResult(PresentationRequest* request, bool success)
{
    if (!request)
        return;
    ...
    ukm::builders::Presentation_StartResult(source_id).SetPresentationRequest(success).Record(ukm_recorder);
}
