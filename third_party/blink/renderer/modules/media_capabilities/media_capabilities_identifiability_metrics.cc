...
{
    return IdentifiabilityStudySettings::Get()->ShouldSampleType(IdentifiableSurface::Type::kMediaCapabilities_DecodingInfo);
}
...
void RecordDecodingIdentifiabilityMetric(ExecutionContext* context, IdentifiableToken input_token, IdentifiableToken output_token)
{
    DCHECK(IsDecodingInfoTypeAllowed());
    IdentifiabilityMetricBuilder(context->UkmSourceID())
        .Add(IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kMediaCapabilities_DecodingInfo, input_token), output_token)
...
IdentifiableToken ComputeToken(const VideoConfiguration* configuration)
{
    ...
    builder.AddToken(IdentifiabilityBenignStringToken(configuration->contentType()))
    ...
    if (configuration->hasHdrMetadataType()) {
        builder.AddToken(IdentifiabilityBenignStringToken(configuration->hdrMetadataType().AsString()));
    }
    if (configuration->hasColorGamut()) {
        builder.AddToken(IdentifiabilityBenignStringToken(configuration->colorGamut().AsString()));
    }
    if (configuration->hasTransferFunction()) {
        builder.AddToken(IdentifiabilityBenignStringToken(configuration->transferFunction().AsString()));
    }
    if (configuration->hasScalabilityMode()) {
        builder.AddToken(IdentifiabilityBenignStringToken(configuration->scalabilityMode()));
    }
...
IdentifiableToken ComputeToken(const AudioConfiguration* configuration)
{
    ...
    builder.AddValue(configuration->hasChannels()).AddValue(configuration->hasBitrate()).AddValue(configuration->hasSamplerate());
    if (configuration->hasChannels()) {
        builder.AddToken(IdentifiabilityBenignStringToken(configuration->channels()));
    }
...
IdentifiableToken ComputeToken(const KeySystemTrackConfiguration* configuration)
{
    DCHECK(IsDecodingInfoTypeAllowed());
    if (!configuration)
        return IdentifiableToken();
    ...
    IdentifiableTokenBuilder builder;
    builder.AddToken(IdentifiabilityBenignStringToken(configuration->robustness()));
    return builder.GetToken();
}

IdentifiableToken ComputeToken(const MediaKeySystemMediaCapability* capability)
{
    DCHECK(IsDecodingInfoTypeAllowed());
    if (!capability)
        return IdentifiableToken();
    ...
    builder.AddToken(IdentifiabilityBenignStringToken(capability->contentType()))
        ...
        .AddToken(IdentifiabilityBenignStringToken(capability->encryptionScheme()));
    return builder.GetToken();
}

IdentifiableToken ComputeToken(const MediaKeySystemConfiguration* configuration)
{
    DCHECK(IsDecodingInfoTypeAllowed());
    if (!configuration)
        return IdentifiableToken();
    ...
    builder.AddToken(IdentifiabilityBenignStringToken(configuration->label()))
        ...
        .AddToken(IdentifiabilityBenignStringToken(configuration->distinctiveIdentifier().AsString()))
        .AddToken(IdentifiabilityBenignStringToken(configuration->persistentState().AsString()))
        .AddValue(configuration->hasSessionTypes());
    if (configuration->hasInitDataTypes()) {
        builder.AddToken(IdentifiabilityBenignStringVectorToken(configuration->initDataTypes()));
    }
    if (configuration->hasAudioCapabilities()) {
        const HeapVector<Member<MediaKeySystemMediaCapability>>& audio_capabilities = configuration->audioCapabilities();
    ...
    if (configuration->hasVideoCapabilities()) {
        const HeapVector<Member<MediaKeySystemMediaCapability>>& video_capabilities = configuration->videoCapabilities();
        builder.AddValue(video_capabilities.size());
        for (const auto& elem : video_capabilities)
            builder.AddToken(ComputeToken(elem.Get()));
    }
    if (configuration->hasSessionTypes()) {
        builder.AddToken(IdentifiabilityBenignStringVectorToken(configuration->sessionTypes()));
    }
    return builder.GetToken();
}
...
IdentifiableToken ComputeToken(const MediaKeySystemAccess* access)
{
    ...
    IdentifiableTokenBuilder builder;
    builder.AddToken(IdentifiabilityBenignStringToken(access->keySystem())).AddToken(ComputeToken(access->getConfiguration()));
    return builder.GetToken();
}

IdentifiableToken ComputeToken(const MediaCapabilitiesKeySystemConfiguration* configuration)
{
    DCHECK(IsDecodingInfoTypeAllowed());
    if (!configuration)
        return IdentifiableToken();
...
IdentifiableToken ComputeToken(const MediaDecodingConfiguration* configuration)
{
    DCHECK(IsDecodingInfoTypeAllowed());
    if (!configuration)
        return IdentifiableToken();
    ...
    IdentifiableTokenBuilder builder;
    builder.AddToken(IdentifiabilityBenignStringToken(configuration->type().AsString()))
...
IdentifiableToken ComputeToken(const MediaCapabilitiesDecodingInfo* info)
{
    ...
    IdentifiableTokenBuilder builder;
    builder.AddValue(info->supported()).AddValue(info->smooth()).AddValue(info->powerEfficient()).AddToken(ComputeToken(info->keySystemAccess()));
    return builder.GetToken();
}

} // namespace

void ReportDecodingInfoResult(ExecutionContext* context, const MediaDecodingConfiguration* input, const MediaCapabilitiesDecodingInfo* output)
{
    if (!IsDecodingInfoTypeAllowed() || !ShouldSampleDecodingInfoType())
        return;

    RecordDecodingIdentifiabilityMetric(context, ComputeToken(input), ComputeToken(output));
}

void ReportDecodingInfoResult(ExecutionContext* context, std::optional<IdentifiableToken> input_token, const MediaCapabilitiesDecodingInfo* output)
{
    DCHECK_EQ(IsDecodingInfoTypeAllowed(), input_token.has_value());
    if (!input_token.has_value() || !ShouldSampleDecodingInfoType())
        return;

    RecordDecodingIdentifiabilityMetric(context, input_token.value(), IdentifiableToken());
}

std::optional<IdentifiableToken> ComputeDecodingInfoInputToken(const MediaDecodingConfiguration* input)
{
    if (!IsDecodingInfoTypeAllowed() || !ShouldSampleDecodingInfoType())
        return std::nullopt;
