...
class MODULES_EXPORT MediaCapabilities final : public ScriptWrappable, public Supplement<NavigatorBase> {
...
public:
    ...
    ScriptPromise<MediaCapabilitiesDecodingInfo> decodingInfo(ScriptState*, const MediaDecodingConfiguration*, ExceptionState&);
    ScriptPromise<MediaCapabilitiesInfo> encodingInfo(ScriptState*, const MediaEncodingConfiguration*, ExceptionState&);
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(MediaCapabilitiesTests, WebrtcDecodePowerEfficientIsSmooth);
    FRIEND_TEST_ALL_PREFIXES(MediaCapabilitiesTests, WebrtcDecodeOverridePowerEfficientIsSmooth);
    FRIEND_TEST_ALL_PREFIXES(MediaCapabilitiesTests, WebrtcEncodePowerEfficientIsSmooth);
    FRIEND_TEST_ALL_PREFIXES(MediaCapabilitiesTests, WebrtcEncodeOverridePowerEfficientIsSmooth);
    ...
    ScriptPromise<MediaCapabilitiesDecodingInfo> GetEmeSupport(ScriptState*, media::VideoCodec, media::VideoCodecProfile, media::VideoColorSpace,
        const MediaDecodingConfiguration*, const base::TimeTicks& request_time, ExceptionState&);
    ...
    void GetPerfInfo(media::VideoCodec, media::VideoCodecProfile, media::VideoColorSpace, const MediaDecodingConfiguration*,
        const base::TimeTicks& request_time, ScriptPromiseResolver<MediaCapabilitiesDecodingInfo>*, MediaKeySystemAccess*);

    // Gets ML perf predictions from remote LearingTaskControllers.
    void GetPerfInfo_ML(ExecutionContext* execution_context, int callback_id, media::VideoCodec video_codec, media::VideoCodecProfile video_profile, int width,
        double framerate);
    ...
    // is true.
    void GetGpuFactoriesSupport(
        int callback_id, media::VideoCodec video_codec, media::VideoCodecProfile video_profile, media::VideoColorSpace, const MediaDecodingConfiguration*);

    // Callback for perf info from the VideoDecodePerfHistory service.
    void OnPerfHistoryInfo(int callback_id, bool is_smooth, bool is_power_efficient);

    // Callback for predictions from |bad_window_predictor_|.
    void OnBadWindowPrediction(int callback_id, const std::optional<::media::learning::TargetHistogram>& histogram);

    // Callback for predictions from |nnr_predictor_|.
    void OnNnrPrediction(int callback_id, const std::optional<::media::learning::TargetHistogram>& histogram);

    // Callback for GetGpuFactoriesSupport().
    void OnGpuFactoriesSupport(int callback_id, bool is_supported, media::VideoCodec video_codec);
    ...
    void OnWebrtcSupportInfo(int callback_id, media::mojom::blink::WebrtcPredictionFeaturesPtr features, float frames_per_second, OperationType,
        bool is_supported, bool is_power_efficient);
    ...
    void set_webrtc_decoding_info_handler_for_test(WebrtcDecodingInfoHandler* handler)
    {
        webrtc_decoding_info_handler_for_test_ = handler;
    }

    void set_webrtc_encoding_info_handler_for_test(WebrtcEncodingInfoHandler* handler)
    {
        webrtc_encoding_info_handler_for_test_ = handler;
    }

    HeapMojoRemote<media::mojom::blink::VideoDecodePerfHistory> decode_history_service_;
    ...
    HeapMojoRemote<media::learning::mojom::blink::LearningTaskController> bad_window_predictor_;
    ...
    // media::SmoothnessHelper.
    HeapMojoRemote<media::learning::mojom::blink::LearningTaskController> nnr_predictor_;

    HeapMojoRemote<media::mojom::blink::WebrtcVideoPerfHistory> webrtc_history_service_;
    ...
    raw_ptr<WebrtcDecodingInfoHandler> webrtc_decoding_info_handler_for_test_ = nullptr;

    // Makes it possible to override the WebrtcEncodingInfoHandler in tests.
    raw_ptr<WebrtcEncodingInfoHandler> webrtc_encoding_info_handler_for_test_ = nullptr;
};
