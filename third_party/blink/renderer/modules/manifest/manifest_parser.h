...
public:
    ManifestParser(const String& data, const KURL& manifest_url, const KURL& document_url, ExecutionContext* execution_context);
...
private:
    ...
    struct PatternInit {
        PatternInit(std::optional<String> protocol, std::optional<String> username, std::optional<String> password, std::optional<String> hostname,
            std::optional<String> port, std::optional<String> pathname, std::optional<String> search, std::optional<String> hash, KURL base_url);
        ~PatternInit();
    ...
    bool ParseBoolean(const JSONObject* object, const String& key, bool default_value);
    ...
    std::optional<String> ParseString(const JSONObject* object, const String& key, Trim trim);
    ...
    std::optional<String> ParseStringForMember(const JSONObject* object, const String& member_name, const String& key, bool required, Trim trim);
    ...
    // parsed the same.
    KURL ParseURL(const JSONObject* object, const String& key, const KURL& base_url, ParseURLRestrictions origin_restriction, bool ignore_empty_string = false);
    ...
    template <typename Enum> Enum ParseFirstValidEnum(const JSONObject* object, const String& key, Enum (*parse_enum)(const std::string&), Enum invalid_value);
    ...
    std::pair<KURL, ParseIdResultType> ParseId(const JSONObject* object, const KURL& start_url);
    ...
    std::pair<KURL, ParseStartUrlResult> ParseStartURL(const JSONObject* object, const KURL& document_url);
    ...
    Vector<mojom::blink::DisplayMode> ParseDisplayOverride(const JSONObject* object);
    ...
    // failed.
    device::mojom::blink::ScreenOrientationLockType ParseOrientation(const JSONObject* object);
    ...
    std::optional<Vector<mojom::blink::ManifestImageResource::Purpose>> ParseIconPurpose(const JSONObject* icon);
    ...
    Vector<mojom::blink::ManifestImageResourcePtr> ParseIcons(const JSONObject* object);
    ...
    Vector<mojom::blink::ManifestScreenshotPtr> ParseScreenshots(const JSONObject* object);

    // Parse the 'form_factor' field of 'screenshots' as defined in:
    // https://www.w3.org/TR/manifest-app-info/#form_factor-member
    mojom::blink::ManifestScreenshot::FormFactor ParseScreenshotFormFactor(const JSONObject* screenshot);
    ...
    Vector<mojom::blink::ManifestImageResourcePtr> ParseImageResourceArray(const String& key, const JSONObject* object);

    std::optional<mojom::blink::ManifestImageResourcePtr> ParseImageResource(const JSONValue* object);
    ...
    Vector<mojom::blink::ManifestShortcutItemPtr> ParseShortcuts(const JSONObject* object);
    ...
    Vector<mojom::blink::ManifestFileFilterPtr> ParseTargetFiles(const String& key, const JSONObject* from);

    // Parses a single FileFilter (see above comment) and appends it to
    // the given |files| vector.
    void ParseFileFilter(const JSONObject* file_dictionary, Vector<mojom::blink::ManifestFileFilterPtr>* files);
    ...
    std::optional<mojom::blink::ManifestShareTarget::Method> ParseShareTargetMethod(const JSONObject* share_target_dict);

    // Parses the enctype field of a Share Target, as defined in:
    // https://wicg.github.io/web-share-target/#sharetarget-and-its-members
    // Returns an optional share target enctype enum object.
    std::optional<mojom::blink::ManifestShareTarget::Enctype> ParseShareTargetEnctype(const JSONObject* share_target_dict);
    ...
    mojom::blink::ManifestShareTargetParamsPtr ParseShareTargetParams(const JSONObject* share_target_params);
    ...
    std::optional<mojom::blink::ManifestShareTargetPtr> ParseShareTarget(const JSONObject* object);
    ...
    Vector<mojom::blink::ManifestUrlHandlerPtr> ParseUrlHandlers(const JSONObject* object);
    ...
    // This feature is experimental and is only enabled by the blink feature flag:
    // blink::features::kWebAppEnableUrlHandlers.
    std::optional<mojom::blink::ManifestUrlHandlerPtr> ParseUrlHandler(const JSONObject* object);
    ...
    Vector<mojom::blink::ManifestScopeExtensionPtr> ParseScopeExtensions(const JSONObject* object);
    ...
    // This feature is experimental and is only enabled by the blink feature flag:
    // blink::features::kWebAppEnableScopeExtensions.
    std::optional<mojom::blink::ManifestScopeExtensionPtr> ParseScopeExtension(const JSONObject* object);
    ...
    // This feature is experimental and is only enabled by the blink feature flag:
    // blink::features::kWebAppEnableScopeExtensions.
    std::optional<mojom::blink::ManifestScopeExtensionPtr> ParseScopeExtensionOrigin(const String& origin_string);
    ...
    // empty.
    Vector<mojom::blink::ManifestFileHandlerPtr> ParseFileHandlers(const JSONObject* object);
    ...
    std::optional<mojom::blink::ManifestFileHandlerPtr> ParseFileHandler(const JSONObject* file_handler_entry);

    // Parses the 'accept' field of a FileHandler, as defined in:
    // https://github.com/WICG/file-handling/blob/main/explainer.md.
    // Returns the parsed accept map. Invalid accept entries are ignored.
    HashMap<String, Vector<String>> ParseFileHandlerAccept(const JSONObject* accept);
    ...
    // parsed extension.
    bool ParseFileHandlerAcceptExtension(const JSONValue* extension, String* ouput);
    ...
    // empty.
    Vector<mojom::blink::ManifestProtocolHandlerPtr> ParseProtocolHandlers(const JSONObject* object);
    ...
    std::optional<mojom::blink::ManifestProtocolHandlerPtr> ParseProtocolHandler(const JSONObject* protocol_dictionary);
    ...
    // parsing failed.
    mojom::blink::ManifestLockScreenPtr ParseLockScreen(const JSONObject* manifest);
    ...
    // parsing failed.
    mojom::blink::ManifestNoteTakingPtr ParseNoteTaking(const JSONObject* manifest);
    ...
    // or empty.
    Vector<mojom::blink::ManifestRelatedApplicationPtr> ParseRelatedApplications(const JSONObject* object);
    ...
    Vector<blink::ParsedPermissionsPolicyDeclaration> ParseIsolatedAppPermissions(const JSONObject* object);
    Vector<String> ParseOriginAllowlist(const JSONArray* allowlist, const String& feature);

    // Parses the 'launch_handler' field of the manifest as defined in:
    // https://github.com/WICG/web-app-launch/blob/main/launch_handler.md
    // Returns default values if parsing fails.
    mojom::blink::ManifestLaunchHandlerPtr ParseLaunchHandler(const JSONObject* object);
    ...
    HashMap<String, mojom::blink::ManifestTranslationItemPtr> ParseTranslations(const JSONObject* object);
    ...
    mojom::blink::TabStripMemberVisibility ParseTabStripMemberVisibility(const JSONValue* json_value);
    ...
    std::optional<SafeUrlPattern> ParseScopePattern(const PatternInit& init, const KURL& base_url);

    std::optional<PatternInit> MaybeCreatePatternInit(const JSONObject* pattern_object);

    String ParseVersion(const JSONObject* object);

    void AddErrorInfo(const String& error_msg, bool critical = false, int error_line = 0, int error_column = 0);
