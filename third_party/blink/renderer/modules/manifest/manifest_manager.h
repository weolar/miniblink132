...
public:
    ...
    void ParseManifestFromString(
        const KURL& document_url, const KURL& manifest_url, const String& manifest_contents, ParseManifestFromStringCallback callback) override;
...
private:
    ...
    class Result {
    public:
        explicit Result(mojom::blink::ManifestRequestResult result, KURL manifest_url = KURL(), mojom::blink::ManifestPtr manifest = nullptr);
        Result(Result&&);
        Result& operator=(Result&&);

        mojom::blink::ManifestRequestResult result() const
        {
            return result_;
        }
        const KURL& manifest_url() const
        {
            return manifest_url_;
        }
        const mojom::blink::Manifest& manifest() const
        {
            return *manifest_;
        }
        ...
        mojom::blink::ManifestDebugInfo& debug_info()
        {
            return *debug_info_;
        }
    ...
    using InternalRequestManifestCallback = base::OnceCallback<void(const Result&)>;
    ...
    void FetchManifest();
    void OnManifestFetchComplete(const KURL& document_url, const ResourceResponse& response, const String& data);
    void ParseManifestFromPage(const KURL& document_url, std::optional<KURL> manifest_url, const String& data);
    void RecordMetrics(const mojom::blink::Manifest& manifest);
    void ResolveCallbacks(Result result);

    void BindReceiver(mojo::PendingReceiver<mojom::blink::ManifestManager> receiver);
    ...
    HeapMojoReceiverSet<mojom::blink::ManifestManager, ManifestManager> receivers_;
};
