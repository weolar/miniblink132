...
{
    WebVector<gfx::Size> parsed_sizes = blink::WebIconSizesParser::ParseIconSizes(WebString::FromASCII(sizes.Ascii()));
    WTF::HashSet<std::pair<int, int>, PairHashTraits<IntWithZeroKeyHashTraits<int>, IntWithZeroKeyHashTraits<int>>> unique_sizes;

    WTF::Vector<gfx::Size> results;
    for (const auto& size : parsed_sizes) {
        auto add_result = unique_sizes.insert(std::make_pair(size.width(), size.height()));
...
WTF::Vector<Purpose> ParsePurpose(const WTF::String& purpose)
{
    ...
    purpose.LowerASCII().Split(' ', false /* allow_empty_entries */, split_purposes);
...
blink::mojom::blink::ManifestImageResourcePtr TypeConverter<blink::mojom::blink::ManifestImageResourcePtr, blink::ManifestImageResource*>::Convert(
...
Manifest::ImageResource ConvertManifestImageResource(const ManifestImageResource* icon)
{
    ...
    if (icon->hasPurpose()) {
        ...
        for (auto purpose : mojo::ParsePurpose(icon->purpose())) {
            ...
            case mojo::Purpose::ANY:
                manifest_icon.purpose.emplace_back(mojom::ManifestImageResource_Purpose::ANY);
                break;
            case mojo::Purpose::MONOCHROME:
                manifest_icon.purpose.emplace_back(mojom::ManifestImageResource_Purpose::MONOCHROME);
                break;
            case mojo::Purpose::MASKABLE:
                manifest_icon.purpose.emplace_back(mojom::ManifestImageResource_Purpose::MASKABLE);
                break;
            }
        }
    }
