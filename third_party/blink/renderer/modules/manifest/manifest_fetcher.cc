...
void ManifestFetcher::Start(LocalDOMWindow& window, bool use_credentials, ResourceFetcher* resource_fetcher, ManifestFetcher::Callback callback)
{
    ...
    request.SetCredentialsMode(use_credentials ? network::mojom::CredentialsMode::kInclude : network::mojom::CredentialsMode::kOmit);

    ResourceLoaderOptions resource_loader_options(window.GetCurrentWorld());
    resource_loader_options.initiator_info.name = fetch_initiator_type_names::kLink;
    resource_loader_options.data_buffering_policy = kDoNotBufferData;

    loader_ = MakeGarbageCollected<ThreadableLoader>(window, this, resource_loader_options, resource_fetcher);
...
void ManifestFetcher::DidReceiveResponse(uint64_t, const ResourceResponse& response)
{
    response_ = response;
}
...
void ManifestFetcher::DidReceiveData(base::span<const char> data)
{
    ...
    if (!decoder_) {
        ...
        decoder_ = std::make_unique<TextResourceDecoder>(
            TextResourceDecoderOptions(TextResourceDecoderOptions::kPlainTextContent, encoding.empty() ? UTF8Encoding() : WTF::TextEncoding(encoding)));
    }
