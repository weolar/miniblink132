...
protected:
    ...
    mojom::blink::ManifestPtr& ParseManifestWithURLs(const String& data, const KURL& manifest_url, const KURL& document_url)
    {
    ...
    {
        return ParseManifestWithURLs(data, DefaultManifestUrl(), DefaultDocumentUrl());
    }

    const Vector<String>& errors() const
    {
        return errors_;
    }
    ...
    static KURL DefaultDocumentUrl()
    {
        return KURL("http://foo.com/index.html");
    }
    ...
    bool HasDefaultValuesWithUrls(
        const mojom::blink::ManifestPtr& manifest, const KURL& document_url = DefaultDocumentUrl(), const KURL& manifest_url = DefaultManifestUrl())
    {
    ...
    void VerifySafeUrlPatternSizes(const SafeUrlPattern& pattern, size_t protocol_size, size_t username_size, size_t password_size, size_t hostname_size,
        size_t port_size, size_t pathname_size, size_t search_size, size_t hash_size)
    {
...
TEST_F(ManifestParserTest, ValidNoContentParses)
{
    ...
    EXPECT_EQ(manifest->orientation, device::mojom::ScreenOrientationLockType::DEFAULT);
    ...
    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.name"), testing::IsEmpty());
    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.start_url"), testing::IsEmpty());
    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.short_name"), testing::IsEmpty());

    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.description"), testing::IsEmpty());
    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.start_url"), testing::IsEmpty());
    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.display"), testing::IsEmpty());
    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.orientation"), testing::IsEmpty());
    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.icons"), testing::IsEmpty());
    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.screenshots"), testing::IsEmpty());
    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.share_target"), testing::IsEmpty());

    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.protocol_handlers"), testing::IsEmpty());
    EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.gcm_sender_id"), testing::IsEmpty());
}
...
TEST_F(ManifestParserTest, MultipleErrorsReporting)
{
    ...
    EXPECT_THAT(errors(),
        testing::UnorderedElementsAre("unknown 'dir' value ignored.", "property 'name' ignored, type string expected.",
            "property 'short_name' ignored, type string expected.", "property 'start_url' ignored, type string expected.",
            "property 'id' ignored, type string expected.", "unknown 'display' value ignored.", "property 'orientation' ignored, type string expected.",
            "property 'icons' ignored, type array expected.", "property 'theme_color' ignored, type string expected.",
            "property 'background_color' ignored, type string expected.", "property 'shortcuts' ignored, type array expected."));
}
...
TEST_F(ManifestParserTest, DescriptionParseRules)
{
    // Smoke test.
    {
        auto& manifest = ParseManifest(R"({ "description": "foo is the new black" })");
    ...
    // Don't parse if description isn't a string.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'description' ignored, type string expected.", errors()[0]);
    }
...
TEST_F(ManifestParserTest, ShortNameParseRules)
{
    ...
    // Don't parse if name isn't a string.
    {
        ...
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'short_name' ignored, type string expected.", errors()[0]);
    }
...
TEST_F(ManifestParserTest, IdParseRules)
{
    ...
    // Invalid type.
    {
        auto& manifest = ParseManifest("{\"start_url\": \"/start?query=a\", \"id\": 1}");
        EXPECT_THAT(errors(), testing::ElementsAre("property 'id' ignored, type string expected."));
        EXPECT_EQ("http://foo.com/start?query=a", manifest->id);
        EXPECT_FALSE(manifest->has_custom_id);
    }
    // Empty string.
    {
        auto& manifest = ParseManifest(R"({ "start_url": "/start?query=a", "id": "" })");
        EXPECT_THAT(errors(), testing::IsEmpty());
        EXPECT_EQ("http://foo.com/start?query=a", manifest->id);
        EXPECT_FALSE(manifest->has_custom_id);
    }
    // Full url.
    {
        auto& manifest = ParseManifest("{ \"start_url\": \"/start?query=a\", \"id\": \"http://foo.com/foo\" "
                                       "}");
        EXPECT_THAT(errors(), testing::IsEmpty());
    ...
    {
        auto& manifest = ParseManifest("{ \"start_url\": \"/start?query=a\", \"id\": "
                                       "\"http://another.com/foo\" }");
        EXPECT_THAT(errors(), testing::ElementsAre("property 'id' ignored, should be same origin as document."));
        EXPECT_EQ("http://foo.com/start?query=a", manifest->id);
        EXPECT_FALSE(manifest->has_custom_id);
    }
    // Relative path
    ...
    // url with fragment
    {
        auto& manifest = ParseManifest("{ \"start_url\": \"/start?query=a\", \"id\": \"/#abc\" }");
        EXPECT_THAT(errors(), testing::IsEmpty());
        EXPECT_EQ("http://foo.com/", manifest->id);
        EXPECT_TRUE(manifest->has_custom_id);
    }
    // Smoke test.
    {
        auto& manifest = ParseManifest(R"({ "start_url": "/start?query=a", "id": "foo" })");
        EXPECT_THAT(errors(), testing::IsEmpty());
        EXPECT_EQ("http://foo.com/foo", manifest->id);
        EXPECT_TRUE(manifest->has_custom_id);
    }
    // Invalid UTF-8 character.
    {
        ...
        String manifest_str = String("{ \"start_url\": \"/start?query=a\", \"id\": \"") + String(invalid_utf8_chars) + String("\" }");

        auto& manifest = ParseManifest(manifest_str);
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_THAT(errors()[0].Utf8(),
...
TEST_F(ManifestParserTest, StartURLParseRules)
{
    // Smoke test.
    {
        ...
        EXPECT_FALSE(HasDefaultValuesWithUrls(manifest));
        EXPECT_THAT(histogram_tester.GetAllSamples("Manifest.HasProperty.start_url"), base::BucketsAre(base::Bucket(1, 1)));
    }
    ...
    // Don't parse if property isn't a string.
    {
        ...
        EXPECT_THAT(errors(), testing::ElementsAre("property 'start_url' ignored, type string expected."));
    ...
    {
        auto& manifest = ParseManifest(R"({ "start_url": "http://www.google.ca:a" })");
        EXPECT_EQ(manifest->start_url, DefaultDocumentUrl());
        EXPECT_THAT(errors(), testing::ElementsAre("property 'start_url' ignored, URL is invalid."));
        EXPECT_FALSE(manifest->has_valid_specified_start_url);
    }
    ...
    {
        auto& manifest
            = ParseManifestWithURLs(R"({ "start_url": "http://foo.com/land.html" })", KURL("http://foo.com/manifest.json"), KURL("http://foo.com/index.html"));
    ...
    {
        auto& manifest
            = ParseManifestWithURLs(R"({ "start_url": "http://bar.com/land.html" })", KURL("http://foo.com/manifest.json"), KURL("http://foo.com/index.html"));
        EXPECT_EQ(manifest->start_url, DefaultDocumentUrl());
        EXPECT_THAT(errors(),
    ...
    {
        auto& manifest
            = ParseManifestWithURLs(R"({ "start_url": "land.html" })", KURL("http://foo.com/landing/manifest.json"), KURL("http://foo.com/index.html"));
        EXPECT_EQ(manifest->start_url.GetString(), "http://foo.com/landing/land.html");
        EXPECT_THAT(errors(), testing::IsEmpty());
        EXPECT_TRUE(manifest->has_valid_specified_start_url);
    }
}
...
TEST_F(ManifestParserTest, ScopeParseRules)
{
    // Smoke test.
    {
        auto& manifest = ParseManifest(R"({ "scope": "land", "start_url": "land/landing.html" })");
    ...
    // Absolute scope, start URL is in scope.
    {
        ...
        "start_url": "http://foo.com/land/landing.html" })",
            KURL("http://foo.com/manifest.json"), KURL("http://foo.com/index.html"));
    ...
    // Absolute scope, start URL has different origin than scope URL.
    {
        ...
        EXPECT_EQ("property 'start_url' ignored, should be same origin as document.", errors()[0]);
        EXPECT_EQ("property 'scope' ignored. Start url should be within scope "
                  "of scope URL.",
            errors()[1]);
    }
    ...
    {
        auto& manifest
            = ParseManifestWithURLs(R"({ "scope": "http://foo.com/land" })", KURL("http://foo.com/manifest.json"), KURL("http://foo.com/land/site/index.html"));
        ASSERT_EQ(manifest->scope.GetString(), "http://foo.com/land");
    ...
    // No start URL. Document is out of scope.
    {
        ...
        auto& manifest
            = ParseManifestWithURLs(R"({ "scope": "http://foo.com/land" })", KURL("http://foo.com/manifest.json"), KURL("http://foo.com/index.html"));
        ASSERT_EQ(manifest->scope.GetString(), document_url.BaseAsString());
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'scope' ignored. Start url should be within scope "
                  "of scope URL.",
    ...
    {
        auto& manifest = ParseManifestWithURLs(
            R"({ "scope": "treasure" })", KURL("http://foo.com/map/manifest.json"), KURL("http://foo.com/map/treasure/island/index.html"));
    ...
    {
        auto& manifest = ParseManifestWithURLs(R"({ "scope": ".." })", KURL("http://foo.com/map/manifest.json"), KURL("http://foo.com/index.html"));
    ...
    {
        auto& manifest = ParseManifest(R"({ "start_url": "land/landing.html?query=test#abc" })");
        ASSERT_EQ(manifest->scope, KURL(DefaultDocumentUrl(), "land/"));
        EXPECT_EQ(0u, GetErrorCount());
    }

    {
        auto& manifest = ParseManifest(R"({ "start_url": "land/land/landing.html" })");
...
TEST_F(ManifestParserTest, DisplayParseRules)
{
    ...
    {
        auto& manifest = ParseManifest(R"({ "display": "window-controls-overlay" })");
        EXPECT_EQ(manifest->display, blink::mojom::DisplayMode::kUndefined);
        EXPECT_EQ(1u, GetErrorCount());
...
TEST_F(ManifestParserTest, DisplayOverrideParseRules)
{
    ...
    // Smoke test: if not array, value will be ignored
    {
        ...
        EXPECT_TRUE(manifest->display_override.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'display_override' ignored, type array expected.", errors()[0]);
    }
    ...
    // Case insensitive
    {
        ...
        EXPECT_EQ(manifest->display_override[0], blink::mojom::DisplayMode::kBrowser);
        EXPECT_FALSE(IsManifestEmpty(manifest));
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    {
        auto& manifest = ParseManifest(R"({ "display_override": [ "browser", "minimal-ui" ] })");
        EXPECT_FALSE(manifest->display_override.empty());
        EXPECT_EQ(manifest->display_override[0], blink::mojom::DisplayMode::kBrowser);
        EXPECT_EQ(manifest->display_override[1], blink::mojom::DisplayMode::kMinimalUi);
        EXPECT_FALSE(IsManifestEmpty(manifest));
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Accept 'browser', 'minimal-ui', 'standalone'
    {
        ...
        EXPECT_EQ(manifest->display_override[0], blink::mojom::DisplayMode::kBrowser);
        EXPECT_EQ(manifest->display_override[1], blink::mojom::DisplayMode::kMinimalUi);
        EXPECT_EQ(manifest->display_override[2], blink::mojom::DisplayMode::kStandalone);
        EXPECT_FALSE(IsManifestEmpty(manifest));
    }
    ...
    // Accept 'browser', 'minimal-ui', 'browser'
    {
        ...
        EXPECT_EQ(manifest->display_override[0], blink::mojom::DisplayMode::kBrowser);
        EXPECT_EQ(manifest->display_override[1], blink::mojom::DisplayMode::kMinimalUi);
        EXPECT_EQ(manifest->display_override[2], blink::mojom::DisplayMode::kBrowser);
        EXPECT_FALSE(IsManifestEmpty(manifest));
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    {
        auto& manifest = ParseManifest(R"({ "display_override": [ "window-controls-overlay" ] })");
        EXPECT_FALSE(manifest->display_override.empty());
        EXPECT_EQ(manifest->display_override[0], blink::mojom::DisplayMode::kWindowControlsOverlay);
        EXPECT_FALSE(IsManifestEmpty(manifest));
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Reject 'borderless' when Borderless flag is disabled.
    {
        base::test::ScopedFeatureList feature_list;
        feature_list.InitAndDisableFeature(blink::features::kWebAppBorderless);
        auto& manifest = ParseManifest(R"({ "display_override": [ "borderless" ] })");
        EXPECT_TRUE(manifest->display_override.empty());
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Accept 'borderless' when Borderless flag is enabled.
    {
        ...
        auto& manifest = ParseManifest(R"({ "display_override": [ "borderless" ] })");
        EXPECT_FALSE(manifest->display_override.empty());
        EXPECT_EQ(manifest->display_override[0], blink::mojom::DisplayMode::kBorderless);
        EXPECT_FALSE(IsManifestEmpty(manifest));
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Accept 'tabbed' when flag is enabled.
    {
        ...
        auto& manifest = ParseManifest(R"({ "display_override": [ "tabbed" ] })");
        EXPECT_FALSE(manifest->display_override.empty());
        EXPECT_EQ(manifest->display_override[0], blink::mojom::DisplayMode::kTabbed);
        EXPECT_FALSE(IsManifestEmpty(manifest));
        EXPECT_EQ(0u, GetErrorCount());
    }
}
...
TEST_F(ManifestParserTest, OrientationParseRules)
{
    // Smoke test.
    {
        ...
        EXPECT_EQ(manifest->orientation, device::mojom::ScreenOrientationLockType::NATURAL);
        EXPECT_FALSE(IsManifestEmpty(manifest));
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Don't parse if name isn't a string.
    {
        ...
        EXPECT_EQ(manifest->orientation, device::mojom::ScreenOrientationLockType::DEFAULT);
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'orientation' ignored, type string expected.", errors()[0]);
    }
    ...
    // Accept 'any'.
    {
        ...
        EXPECT_EQ(manifest->orientation, device::mojom::ScreenOrientationLockType::ANY);
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Accept 'landscape'.
    {
        ...
        EXPECT_EQ(manifest->orientation, device::mojom::ScreenOrientationLockType::LANDSCAPE);
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Accept 'landscape-primary'.
    {
        ...
        EXPECT_EQ(manifest->orientation, device::mojom::ScreenOrientationLockType::LANDSCAPE_PRIMARY);
        EXPECT_EQ(0u, GetErrorCount());
    }

    // Accept 'landscape-secondary'.
    {
        auto& manifest = ParseManifest(R"({ "orientation": "landscape-secondary" })");
        EXPECT_EQ(manifest->orientation, device::mojom::ScreenOrientationLockType::LANDSCAPE_SECONDARY);
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Accept 'portrait'.
    {
        ...
        EXPECT_EQ(manifest->orientation, device::mojom::ScreenOrientationLockType::PORTRAIT);
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Accept 'portrait-primary'.
    {
        ...
        EXPECT_EQ(manifest->orientation, device::mojom::ScreenOrientationLockType::PORTRAIT_PRIMARY);
        EXPECT_EQ(0u, GetErrorCount());
    }

    // Accept 'portrait-secondary'.
    {
        auto& manifest = ParseManifest(R"({ "orientation": "portrait-secondary" })");
        EXPECT_EQ(manifest->orientation, device::mojom::ScreenOrientationLockType::PORTRAIT_SECONDARY);
        EXPECT_EQ(0u, GetErrorCount());
    }
...
TEST_F(ManifestParserTest, ScreenshotsParseRules)
{
    ...
    {
        auto& manifest = ParseManifest(R"({ "screenshots": [ { "screenshots": [] } ] })");
        EXPECT_TRUE(manifest->screenshots.empty());
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Smoke test: if screenshot with empty src, it will be present in the list.
    {
        ...
        EXPECT_EQ(screenshots[0]->image->src.GetString(), "http://foo.com/manifest.json");
        EXPECT_FALSE(IsManifestEmpty(manifest));
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    {
        auto& manifest = ParseManifest(R"({ "screenshots": [{ "src": "foo.jpg" }] })");
        EXPECT_FALSE(manifest->screenshots.empty());
...
TEST_F(ManifestParserTest, ScreenshotFormFactorParseRules)
{
    // Smoke test.
    {
        auto& manifest = ParseManifest(R"({ "screenshots": [{ "src": "foo.jpg", "form_factor": "narrow" }] })");
        EXPECT_FALSE(manifest->screenshots.empty());

        auto& screenshots = manifest->screenshots;
        EXPECT_EQ(screenshots.size(), 1u);
        EXPECT_EQ(screenshots[0]->form_factor, mojom::blink::ManifestScreenshot::FormFactor::kNarrow);
        EXPECT_FALSE(IsManifestEmpty(manifest));
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Unspecified.
    {
        ...
        auto& screenshots = manifest->screenshots;
        EXPECT_EQ(screenshots.size(), 1u);
        EXPECT_EQ(screenshots[0]->form_factor, mojom::blink::ManifestScreenshot::FormFactor::kUnknown);
        EXPECT_FALSE(IsManifestEmpty(manifest));
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Invalid type.
    {
        auto& manifest = ParseManifest(R"({ "screenshots": [{ "src": "foo.jpg", "form_factor": 1}] })");
        EXPECT_FALSE(manifest->screenshots.empty());
    ...
    // Unrecognized string.
    {
        auto& manifest = ParseManifest(R"({ "screenshots": [{ "src": "foo.jpg", "form_factor": "windows"}] })");
        EXPECT_FALSE(manifest->screenshots.empty());
...
TEST_F(ManifestParserTest, ScreenshotLabelRules)
{
    // Smoke test.
    {
        auto& manifest = ParseManifest(R"({ "screenshots": [{ "src": "foo.jpg", "label": "example screenshot." }] })");
        EXPECT_FALSE(manifest->screenshots.empty());
    ...
    // Empty string.
    {
        auto& manifest = ParseManifest(R"({ "screenshots": [{ "src": "foo.jpg", "label": "" }] })");
        EXPECT_FALSE(manifest->screenshots.empty());
    ...
    // Invalid type.
    {
        auto& manifest = ParseManifest(R"({ "screenshots": [{ "src": "foo.jpg", "label": 2 }] })");
        EXPECT_FALSE(manifest->screenshots.empty());
...
TEST_F(ManifestParserTest, IconSrcParseRules)
{
    ...
    {
        auto& manifest
            = ParseManifestWithURLs(R"({ "icons": [ {"src": "icons/foo.png" } ] })", KURL("http://foo.com/landing/index.html"), DefaultManifestUrl());
        EXPECT_FALSE(manifest->icons.empty());
        EXPECT_EQ(manifest->icons[0]->src.GetString(), "http://foo.com/landing/icons/foo.png");
        EXPECT_EQ(0u, GetErrorCount());
    }
}
...
TEST_F(ManifestParserTest, IconTypeParseRules)
{
    // Smoke test.
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "type": "foo" } ] })");
        EXPECT_FALSE(manifest->icons.empty());
    ...
    // Don't parse if property isn't a string.
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "type": {} } ] })");
        EXPECT_FALSE(manifest->icons.empty());
    ...
    // Don't parse if property isn't a string.
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "type": 42 } ] })");
        EXPECT_FALSE(manifest->icons.empty());
        EXPECT_TRUE(manifest->icons[0]->type.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'type' ignored, type string expected.", errors()[0]);
...
TEST_F(ManifestParserTest, IconSizesParseRules)
{
    // Smoke test.
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "sizes": "42x42" } ] })");
        EXPECT_FALSE(manifest->icons.empty());
        EXPECT_EQ(manifest->icons[0]->sizes.size(), 1u);
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "sizes": {} } ] })");
        EXPECT_FALSE(manifest->icons.empty());
    ...
    // Ignore sizes if property isn't a string.
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "sizes": 42 } ] })");
        EXPECT_FALSE(manifest->icons.empty());
        EXPECT_EQ(manifest->icons[0]->sizes.size(), 0u);
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'sizes' ignored, type string expected.", errors()[0]);
    ...
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "sizes": "42x42  48x48" } ] })");
        EXPECT_FALSE(manifest->icons.empty());
    ...
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "sizes": "42X42  42x42" } ] })");
        EXPECT_FALSE(manifest->icons.empty());
    ...
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "sizes": "004X007  042x00" } ] })");
        EXPECT_FALSE(manifest->icons.empty());
        EXPECT_EQ(manifest->icons[0]->sizes.size(), 0u);
        EXPECT_EQ(1u, GetErrorCount());
    ...
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "sizes": "e4X1.0  55ax1e10" } ] })");
        EXPECT_FALSE(manifest->icons.empty());
        EXPECT_EQ(manifest->icons[0]->sizes.size(), 0u);
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("found icon with no valid size.", errors()[0]);
    ...
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "sizes": "any AnY ANY aNy" } ] })");
    ...
    {
        auto& manifest = ParseManifest(R"({ "icons": [ {"src": "", "sizes": "x 40xx 1x2x3 x42 42xx42" } ] })");
        EXPECT_FALSE(manifest->icons.empty());
        EXPECT_EQ(manifest->icons[0]->sizes.size(), 0u);
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("found icon with no valid size.", errors()[0]);
...
{
    const String kPurposeParseStringError = "property 'purpose' ignored, type string expected.";
    const String kPurposeInvalidValueError = "found icon with no valid purpose; ignoring it.";
    const String kSomeInvalidPurposeError = "found icon with one or more invalid purposes; those purposes are "
                                            "ignored.";
    ...
    // 'any' is added when property isn't present.
    {
        ...
        EXPECT_EQ(icons[0]->purpose[0], mojom::blink::ManifestImageResource::Purpose::ANY);
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Smoke test: values correctly parsed.
    {
        ...
        EXPECT_EQ(icons[0]->purpose[0], mojom::blink::ManifestImageResource::Purpose::ANY);
        EXPECT_EQ(icons[0]->purpose[1], mojom::blink::ManifestImageResource::Purpose::MONOCHROME);
        EXPECT_EQ(icons[0]->purpose[2], mojom::blink::ManifestImageResource::Purpose::MASKABLE);
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Twice the same value is parsed twice.
    {
        ...
        auto& icons = manifest->icons;
        ASSERT_EQ(icons[0]->purpose.size(), 2u);
        EXPECT_EQ(icons[0]->purpose[0], mojom::blink::ManifestImageResource::Purpose::MONOCHROME);
        EXPECT_EQ(icons[0]->purpose[1], mojom::blink::ManifestImageResource::Purpose::MONOCHROME);
        EXPECT_EQ(0u, GetErrorCount());
    }
...
TEST_F(ManifestParserTest, ShortcutsParseRules)
{
    ...
    // the list.
    {
        auto& manifest = ParseManifest(R"({ "shortcuts": [ { "shortcuts": [] } ] })");
        EXPECT_TRUE(manifest->icons.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'url' of 'shortcut' not present.", errors()[0]);
    }
    ...
    // the list.
    {
        auto& manifest = ParseManifest(R"({ "shortcuts": [ { "name": "", "url": "" } ] })");
        EXPECT_TRUE(manifest->shortcuts.empty());
        EXPECT_EQ(1u, GetErrorCount());
    ...
    // specifies 11 shortcuts, so the last one should not be in the result.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'shortcuts' contains more than 10 valid elements, "
...
TEST_F(ManifestParserTest, ShortcutNameParseRules)
{
    ...
    {
        auto& manifest = ParseManifest(R"({ "shortcuts": [ {"url": "NameParseTest" } ] })");
        EXPECT_TRUE(manifest->shortcuts.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'name' of 'shortcut' not present.", errors()[0]);
    }
    ...
    {
        auto& manifest = ParseManifest(R"({ "shortcuts": [ {"name": {}, "url": "NameParseTest" } ] })");
        EXPECT_TRUE(manifest->shortcuts.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'name' of 'shortcut' ignored, type string expected.", errors()[0]);
    }

    // Don't parse if shortcut->name isn't a string.
    {
        auto& manifest = ParseManifest(R"({ "shortcuts": [ {"name": 42, "url": "NameParseTest" } ] })");
        EXPECT_TRUE(manifest->shortcuts.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'name' of 'shortcut' ignored, type string expected.", errors()[0]);
    }
...
TEST_F(ManifestParserTest, ShortcutShortNameParseRules)
{
    ...
    {
        auto& manifest = ParseManifest(R"({ "shortcuts": [ {"name": "ShortNameParseTest", "url":
        "ShortNameParseTest" } ] })");
    ...
    // Don't parse short_name if it isn't a string.
    {
        ...
        ASSERT_TRUE(manifest->shortcuts[0]->short_name.IsNull());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'short_name' of 'shortcut' ignored, type string expected.", errors()[0]);
    }
...
TEST_F(ManifestParserTest, ShortcutDescriptionParseRules)
{
    ...
    // Don't parse description if it isn't a string.
    {
        ...
        ASSERT_TRUE(manifest->shortcuts[0]->description.IsNull());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'description' of 'shortcut' ignored, type string expected.", errors()[0]);
    }
...
TEST_F(ManifestParserTest, ShortcutUrlParseRules)
{
    ...
    // Whitespaces.
    {
        auto& manifest = ParseManifest(R"({ "shortcuts": [ {"name": "UrlParseTest", "url": "   foo   " } ] })");
        EXPECT_FALSE(manifest->shortcuts.empty());
        EXPECT_EQ(manifest->shortcuts[0]->url, KURL(DefaultDocumentUrl(), "foo"));
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    {
        auto& manifest = ParseManifest(R"({ "shortcuts": [ {"name": "UrlParseTest", "url": {} } ] })");
        EXPECT_TRUE(manifest->shortcuts.empty());
    ...
    // Don't parse if url isn't a string.
    {
        auto& manifest = ParseManifest(R"({ "shortcuts": [ {"name": "UrlParseTest", "url": 42 } ] })");
        EXPECT_TRUE(manifest->shortcuts.empty());
        EXPECT_EQ(2u, GetErrorCount());
        EXPECT_EQ("property 'url' ignored, type string expected.", errors()[0]);
        EXPECT_EQ("property 'url' of 'shortcut' not present.", errors()[1]);
    ...
    // Resolving has to happen based on the manifest_url.
    {
        ...
        EXPECT_FALSE(manifest->shortcuts.empty());
        EXPECT_EQ(manifest->shortcuts[0]->url.GetString(), "http://foo.com/landing/foo");
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Shortcut url should have same origin as the document url.
    {
        ...
        EXPECT_TRUE(manifest->shortcuts.empty());
        EXPECT_EQ(2u, GetErrorCount());
        EXPECT_EQ("property 'url' ignored, should be within scope of the manifest.", errors()[0]);
        EXPECT_EQ("property 'url' of 'shortcut' not present.", errors()[1]);
    }
    ...
    // The shortcut_url will be http://foo.com/shortcut which is in not in scope.
    {
        ...
        "UrlParseTest", "url": "shortcut" } ] })",
            KURL("http://foo.com/manifest.json"), KURL("http://foo.com/landing/index.html"));
        EXPECT_TRUE(manifest->shortcuts.empty());
    ...
    // The shortcut_url will be http://foo.com/land/shortcut which is in scope.
    {
        ...
        "UrlParseTest", "url": "shortcut" } ] })",
            KURL("http://foo.com/land/manifest.json"), KURL("http://foo.com/index.html"));
        EXPECT_FALSE(manifest->shortcuts.empty());
        ASSERT_EQ(manifest->scope.GetString(), "http://foo.com/land");
        EXPECT_EQ(manifest->shortcuts[0]->url.GetString(), "http://foo.com/land/shortcut");
        EXPECT_EQ(0u, GetErrorCount());
    }
}
...
{
    base::test::ScopedFeatureList feature_list(blink::features::kFileHandlingIcons);
    ...
    // file_handlers is not an array.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'file_handlers' ignored, type array expected.", errors()[0]);
        EXPECT_EQ(0u, manifest->file_handlers.size());
    }
    ...
    // Entry without an action is invalid.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("FileHandler ignored. Property 'action' is invalid.", errors()[0]);
        EXPECT_EQ(0u, manifest->file_handlers.size());
    }
    ...
    // Entry with an action on a different origin is invalid.
    {
        ...
        ASSERT_EQ(2u, GetErrorCount());
        EXPECT_EQ("property 'action' ignored, should be within scope of the manifest.", errors()[0]);
        EXPECT_EQ("FileHandler ignored. Property 'action' is invalid.", errors()[1]);
        EXPECT_EQ(0u, manifest->file_handlers.size());
    }
    ...
    // Entry without an accept is invalid.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("FileHandler ignored. Property 'accept' is invalid.", errors()[0]);
        EXPECT_EQ(0u, manifest->file_handlers.size());
    }
    ...
    // Entry where accept extensions are not an array or string is invalid.
    {
        ...
        ASSERT_EQ(2u, GetErrorCount());
        EXPECT_EQ("property 'accept' type ignored. File extensions must be type array or "
                  "type string.",
            errors()[0]);
        EXPECT_EQ("FileHandler ignored. Property 'accept' is invalid.", errors()[1]);
        EXPECT_EQ(0u, manifest->file_handlers.size());
    }
    ...
    // Extensions that do not start with a '.' are invalid.
    {
        ...
        ASSERT_EQ(2u, GetErrorCount());
        EXPECT_EQ("property 'accept' file extension ignored, must start with a '.'.", errors()[0]);
        EXPECT_EQ("FileHandler ignored. Property 'accept' is invalid.", errors()[1]);
    ...
    // Invalid MIME types and those with parameters are stripped.
    {
        ...
        EXPECT_EQ("invalid MIME type: application/foobar;parameter=25", errors()[2]);
        ASSERT_EQ(1u, file_handlers.size());

        EXPECT_EQ("Foo", file_handlers[0]->name);
        EXPECT_EQ("http://foo.com/foo.jpg", file_handlers[0]->icons[0]->src.GetString());
        ...
        EXPECT_EQ(0u, file_handlers[0]->accept.find("application/its+xml")->value.Contains(".foobar"));
    }
    ...
    // file_handlers with multiple entries is valid.
    {
        ...
        EXPECT_EQ("http://foo.com/graph.jpg", file_handlers[0]->icons[0]->src.GetString());
        ...
        EXPECT_EQ(".graph", file_handlers[0]->accept.find("text/svg+xml")->value[1]);

        EXPECT_EQ("Raw", file_handlers[1]->name);
        EXPECT_EQ("http://foo.com/raw.jpg", file_handlers[1]->icons[0]->src.GetString());
    ...
    // and including the file handler that hits the extension limit
    {
        ...
        ASSERT_EQ(2u, GetErrorCount());
        EXPECT_EQ("property 'accept': too many total file extensions, ignoring "
    ...
    // Test `launch_type` parsing and default.
    {
        ...
        EXPECT_EQ(mojom::blink::ManifestFileHandler::LaunchType::kMultipleClients, manifest->file_handlers[0]->launch_type);
        EXPECT_EQ(mojom::blink::ManifestFileHandler::LaunchType::kSingleClient, manifest->file_handlers[1]->launch_type);
        EXPECT_EQ(mojom::blink::ManifestFileHandler::LaunchType::kSingleClient, manifest->file_handlers[2]->launch_type);
        // This one has a typo.
        EXPECT_EQ(mojom::blink::ManifestFileHandler::LaunchType::kSingleClient, manifest->file_handlers[3]->launch_type);
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("launch_type value 'multiple-client' ignored, unknown value.", errors()[0]);
    }
}
...
TEST_F(ManifestParserTest, ProtocolHandlerParseRules)
{
    ...
    // protocol_handlers is not an array.
    {
        ...
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'protocol_handlers' ignored, type array expected.", errors()[0]);
        EXPECT_EQ(0u, manifest->protocol_handlers.size());
    }
    ...
    // Entries must be objects
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("protocol_handlers entry ignored, type object expected.", errors()[0]);
        EXPECT_EQ(0u, manifest->protocol_handlers.size());
    }
    ...
    // An invalid protocol handler with the URL not being from the same origin.
    {
        ...
        ASSERT_EQ(2u, GetErrorCount());
        EXPECT_EQ("property 'url' ignored, should be within scope of the manifest.", errors()[0]);
        EXPECT_EQ("protocol_handlers entry ignored, required property 'url' is invalid.", errors()[1]);
    ...
    // An invalid protocol handler with no value for protocol.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("protocol_handlers entry ignored, required property 'protocol' is "
                  "missing.",
            errors()[0]);
        ASSERT_EQ(0u, protocol_handlers.size());
    }
    ...
    // An invalid protocol handler with no url.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("protocol_handlers entry ignored, required property 'url' is missing.", errors()[0]);
        ASSERT_EQ(0u, protocol_handlers.size());
    }
    ...
    // An invalid protocol handler with a url that doesn't contain the %s token.
    {
        ...
        ASSERT_EQ(2u, GetErrorCount());
        EXPECT_EQ("The url provided ('http://foo.com/?profile=') does not contain '%s'.", errors()[0]);
        EXPECT_EQ("protocol_handlers entry ignored, required property 'url' is invalid.", errors()[1]);
        ASSERT_EQ(0u, protocol_handlers.size());
    }
    ...
    // An invalid protocol handler with a non-allowed protocol.
    {
        ...
        ASSERT_EQ(2u, GetErrorCount());
        EXPECT_EQ("The scheme 'github' doesn't belong to the scheme allowlist. Please "
...
TEST_F(ManifestParserTest, UrlHandlerParseRules)
{
    ...
    // 'url_handlers' is not an array.
    {
        ...
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'url_handlers' ignored, type array expected.", errors()[0]);
        EXPECT_EQ(0u, manifest->url_handlers.size());
    }
    ...
    // A valid url handler.
    {
        ...
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://foo.com")->IsSameOriginWith(url_handlers[0]->origin.get()));
    }
    ...
    // Scheme must be https.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("url_handlers entry ignored, required property 'origin' must use the "
                  "https scheme.",
            errors()[0]);
    ...
    // Origin must be valid.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("url_handlers entry ignored, required property 'origin' is invalid.", errors()[0]);
        ASSERT_EQ(0u, url_handlers.size());
    }
    ...
    // Parse multiple valid handlers.
    {
        ...
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://foo.com")->IsSameOriginWith(url_handlers[0]->origin.get()));
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://bar.com")->IsSameOriginWith(url_handlers[1]->origin.get()));
    }
    ...
    // Parse invalid handler where the origin is a TLD.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("url_handlers entry ignored, domain of required property 'origin' is "
                  "invalid.",
            errors()[0]);
        ASSERT_EQ(0u, url_handlers.size());
    }
    ...
    // Parse origin where the host is just the wildcard prefix.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        ASSERT_EQ("url_handlers entry ignored, domain of required property 'origin' is "
                  "invalid.",
            errors()[0]);
        ASSERT_EQ(0u, url_handlers.size());
    }
    ...
    // Parse valid origin with private registry.
    {
        ...
        ASSERT_EQ(0u, GetErrorCount());
        ASSERT_EQ(1u, url_handlers.size());
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://glitch.me")->IsSameOriginWith(url_handlers[0]->origin.get()));
        ASSERT_TRUE(url_handlers[0]->has_origin_wildcard);
    }
    ...
    // Parse valid IP address as origin.
    {
        ...
        ASSERT_EQ(0u, GetErrorCount());
        ASSERT_EQ(1u, url_handlers.size());
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://192.168.0.1:8888")->IsSameOriginWith(url_handlers[0]->origin.get()));
        ASSERT_FALSE(url_handlers[0]->has_origin_wildcard);
    }
    ...
    // specifies 11 handlers, so the last one should not be in the result.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'url_handlers' contains more than 10 valid elements, "
                  "only the first 10 are parsed.",
            errors()[0]);
        ASSERT_EQ(10u, url_handlers.size());
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://192.168.0.1:8010")->IsSameOriginWith(url_handlers[9]->origin.get()));
    }
}
...
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(blink::features::kWebAppEnableScopeExtensions);
    ...
    // 'scope_extensions' is not an array.
    {
        ...
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'scope_extensions' ignored, type array expected.", errors()[0]);
        EXPECT_EQ(0u, manifest->scope_extensions.size());
    }
    ...
    // Scope extension entry must be an object or a string.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("scope_extensions entry ignored, type string or object expected.", errors()[0]);
    ...
    // A valid scope extension.
    {
        ...
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://foo.com")->IsSameOriginWith(scope_extensions[0]->origin.get()));
    }
    ...
    // Origin field is missing from the scope extension entry.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("scope_extensions entry ignored, required property 'origin' is "
                  "missing.",
            errors()[0]);
        EXPECT_EQ(0u, scope_extensions.size());
    }
    ...
    // Scheme must be https.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("scope_extensions entry ignored, required property 'origin' must use "
                  "the https scheme.",
            errors()[0]);
    ...
    // Parse multiple valid scope extensions.
    {
        ...
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://foo.com")->IsSameOriginWith(scope_extensions[0]->origin.get()));
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://bar.com")->IsSameOriginWith(scope_extensions[1]->origin.get()));
    }
    ...
    // Parse invalid scope extensions list with entries in mixed formats.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("scope_extensions entry ignored, type object expected.", errors()[0]);
        ASSERT_EQ(1u, scope_extensions.size());
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://foo.com")->IsSameOriginWith(scope_extensions[0]->origin.get()));
    }
    ...
    // Parse invalid scope extension where the origin is a TLD.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("scope_extensions entry ignored, domain of required property 'origin' "
                  "is invalid.",
            errors()[0]);
        ASSERT_EQ(0u, scope_extensions.size());
    }
    ...
    // Parse origin where the host is just the wildcard prefix.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        ASSERT_EQ("scope_extensions entry ignored, domain of required property 'origin' "
                  "is invalid.",
            errors()[0]);
        ASSERT_EQ(0u, scope_extensions.size());
    }
    ...
    // Parse valid origin with private registry.
    {
        ...
        ASSERT_EQ(0u, GetErrorCount());
        ASSERT_EQ(1u, scope_extensions.size());
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://glitch.me")->IsSameOriginWith(scope_extensions[0]->origin.get()));
        ASSERT_TRUE(scope_extensions[0]->has_origin_wildcard);
    }
    ...
    // Parse valid IP address as origin.
    {
        ...
        ASSERT_EQ(0u, GetErrorCount());
        ASSERT_EQ(1u, scope_extensions.size());
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://192.168.0.1:8888")->IsSameOriginWith(scope_extensions[0]->origin.get()));
        ASSERT_FALSE(scope_extensions[0]->has_origin_wildcard);
    }
    ...
    // the result.
    {
        ...
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'scope_extensions' contains more than 10 valid elements, "
                  "only the first 10 are parsed.",
            errors()[0]);
        ASSERT_EQ(10u, scope_extensions.size());
        ASSERT_TRUE(blink::SecurityOrigin::CreateFromString("https://192.168.0.1:8010")->IsSameOriginWith(scope_extensions[9]->origin.get()));
    }
...
TEST_F(ManifestParserTest, LockScreenParseRules)
{
...

    {
        ...
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'lock_screen' ignored, type object expected.", errors()[0]);
        EXPECT_TRUE(manifest->lock_screen.is_null());
    }
...

    {
        ...
        auto& manifest = ParseManifest(R"({ "lock_screen": { "start_url": {} } } )");
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'start_url' ignored, type string expected.", errors()[0]);
        ASSERT_FALSE(manifest->lock_screen.is_null());
        EXPECT_TRUE(manifest->lock_screen->start_url.IsEmpty());
    }

    {
        ...
        auto& manifest = ParseManifest(R"({ "lock_screen": { "start_url": "https://bar.com" } } )");
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'start_url' ignored, should be within scope of the manifest.", errors()[0]);
        ASSERT_FALSE(manifest->lock_screen.is_null());
        EXPECT_TRUE(manifest->lock_screen->start_url.IsEmpty());
    }
...

    {
        ...
        ASSERT_EQ(0u, GetErrorCount());
        ASSERT_FALSE(manifest->lock_screen.is_null());
        EXPECT_EQ("https://foo.com/new_note", manifest->lock_screen->start_url.GetString());
    }
}
...
TEST_F(ManifestParserTest, NoteTakingParseRules)
{
...

    {
        ...
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'note_taking' ignored, type object expected.", errors()[0]);
        EXPECT_TRUE(manifest->note_taking.is_null());
    }
...

    {
        ...
        auto& manifest = ParseManifest(R"({ "note_taking": { "new_note_url": {} } } )");
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'new_note_url' ignored, type string expected.", errors()[0]);
        ASSERT_FALSE(manifest->note_taking.is_null());
        EXPECT_TRUE(manifest->note_taking->new_note_url.IsEmpty());
    }

    {
        ...
        auto& manifest = ParseManifest(R"({ "note_taking": { "new_note_url": "https://bar.com" } } )");
        ASSERT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'new_note_url' ignored, should be within scope of the "
                  "manifest.",
            errors()[0]);
        ASSERT_FALSE(manifest->note_taking.is_null());
        EXPECT_TRUE(manifest->note_taking->new_note_url.IsEmpty());
...

    {
        ...
        ASSERT_EQ(0u, GetErrorCount());
        ASSERT_FALSE(manifest->note_taking.is_null());
        EXPECT_EQ("https://foo.com/", manifest->note_taking->new_note_url.GetString());
    }

    {
        ...
        ASSERT_EQ(0u, GetErrorCount());
        ASSERT_FALSE(manifest->note_taking.is_null());
        EXPECT_EQ("https://foo.com/new_note", manifest->note_taking->new_note_url.GetString());
    }
}
...
TEST_F(ManifestParserTest, ShareTargetParseRules)
{
    // Contains share_target field but no keys.
    {
        ...
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'share_target' ignored. Property 'action' is invalid.", errors()[0]);
    }
    ...
    // Contains share_target field but no params key.
    {
        ...
        EXPECT_EQ("Method should be set to either GET or POST. It currently defaults to "
                  "GET.",
            errors()[0]);
        EXPECT_EQ("Enctype should be set to either application/x-www-form-urlencoded or "
                  "multipart/form-data. It currently defaults to "
                  "application/x-www-form-urlencoded",
            errors()[1]);
        EXPECT_EQ("property 'share_target' ignored. Property 'params' type "
                  "dictionary expected.",
            errors()[2]);
    }
    ...
    {
        auto& manifest = ParseManifest(R"({ "share_target": {"incorrect_key": "some_value" } })");
...
TEST_F(ManifestParserTest, ShareTargetUrlTemplateParseRules)
{
    ...
    {
        auto& manifest = ParseManifestWithURLs(R"({ "share_target": { "action": "", "params": {} } })", manifest_url, document_url);
    ...
    // Don't parse if action property isn't a string.
    {
        ...
        EXPECT_EQ("property 'share_target' ignored. Property 'action' is invalid.", errors()[1]);
    }

    // Don't parse if action property isn't a string.
    {
        auto& manifest = ParseManifestWithURLs(R"({ "share_target": { "action": 42, "params": {} } })", manifest_url, document_url);
        EXPECT_FALSE(manifest->share_target.get());
        EXPECT_EQ(2u, GetErrorCount());
        EXPECT_EQ("property 'action' ignored, type string expected.", errors()[0]);
        EXPECT_EQ("property 'share_target' ignored. Property 'action' is invalid.", errors()[1]);
    ...
    // Don't parse if params property isn't a dict.
    {
        auto& manifest = ParseManifestWithURLs(R"({ "share_target": { "action": "", "params": 42 } })", manifest_url, document_url);
        EXPECT_FALSE(manifest->share_target.get());
        EXPECT_EQ(3u, GetErrorCount());
        EXPECT_EQ("Method should be set to either GET or POST. It currently defaults to "
                  "GET.",
    ...
    // manifest.
    {
        ...
        EXPECT_EQ(2u, GetErrorCount());
        EXPECT_EQ("property 'action' ignored, should be within scope of the manifest.", errors()[0]);
        EXPECT_EQ("property 'share_target' ignored. Property 'action' is "
                  "invalid.",
            errors()[1]);
    }
    ...
    {
        auto& manifest = ParseManifestWithURLs(R"({ "share_target": {"action": "share/", "params": {} } })", manifest_url, document_url);
        ASSERT_TRUE(manifest->share_target.get());
        EXPECT_EQ(manifest->share_target->action.GetString(), "https://foo.com/share/");
        EXPECT_TRUE(manifest->share_target->params->text.IsNull());
        EXPECT_TRUE(manifest->share_target->params->title.IsNull());
        EXPECT_TRUE(manifest->share_target->params->url.IsNull());
        EXPECT_FALSE(IsManifestEmpty(manifest));
    ...
    // Return undefined if method or enctype is not string.
    {
        ...
        EXPECT_FALSE(manifest->share_target.get());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("invalid method. Allowed methods are:"
                  "GET and POST.",
            errors()[0]);
    }
    ...
    // Valid method and enctype.
    {
        ...
        { "title": "mytitle" } }
            ...
            manifest_url, document_url);
        EXPECT_TRUE(manifest->share_target.get());
        EXPECT_EQ(manifest->share_target->method, mojom::blink::ManifestShareTarget::Method::kGet);
        EXPECT_EQ(manifest->share_target->enctype, mojom::blink::ManifestShareTarget::Enctype::kFormUrlEncoded);
    }
    ...
    // "application/x-www-form-urlencoded".
    {
        ...
        EXPECT_FALSE(manifest->share_target.get());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("invalid enctype. Allowed enctypes are:"
    ...
    // Valid enctype for when method is "POST".
    {
        ...
        { "title": "mytitle" } }
            ...
            manifest_url, document_url);
        EXPECT_TRUE(manifest->share_target.get());
        EXPECT_EQ(manifest->share_target->method, mojom::blink::ManifestShareTarget::Method::kPost);
        EXPECT_EQ(manifest->share_target->enctype, mojom::blink::ManifestShareTarget::Enctype::kFormUrlEncoded);
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // Valid enctype for when method is "POST".
    {
        ...
        EXPECT_TRUE(manifest->share_target.get());
        EXPECT_EQ(manifest->share_target->method, mojom::blink::ManifestShareTarget::Method::kPost);
        EXPECT_EQ(manifest->share_target->enctype, mojom::blink::ManifestShareTarget::Enctype::kMultipartFormData);
        EXPECT_EQ(0u, GetErrorCount());
    }
    ...
    // GET method, for example, will cause an error in this case.
    {
        ...
        EXPECT_FALSE(manifest->share_target.get());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("invalid enctype for GET method. Only "
    ...
    // Enctype other than multipart/form-data will cause an error.
    {
        ...
        EXPECT_FALSE(manifest->share_target.get());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("files are only supported with multipart/form-data POST.", errors()[0]);
    }
    ...
    // Accept sequence contains non-string elements.
    {
        ...
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("'accept' entry ignored, expected to be of type string.", errors()[0]);
    }
    ...
    // Accept is neither a string nor an array of strings.
    {
        ...
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'accept' ignored, type array or string expected.", errors()[0]);
    }
    ...
    // Files is neither array nor FileFilter.
    {
        ...
        EXPECT_FALSE(share_target->params->files.has_value());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'files' ignored, type array or FileFilter expected.", errors()[0]);
    }
    ...
    // Files contains a non-dictionary entry.
    {
        ...
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("files must be a sequence of non-empty file entries.", errors()[0]);
    }
...
TEST_F(ManifestParserTest, RelatedApplicationsParseRules)
{
    ...
    // If empty application, empty list.
    {
        ...
        EXPECT_TRUE(manifest->related_applications.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("'platform' is a required field, related application ignored.", errors()[0]);
    }

    // If invalid platform, application is ignored.
    {
        auto& manifest = ParseManifest(R"({ "related_applications": [{"platform": 123}]})");
        EXPECT_TRUE(manifest->related_applications.empty());
        EXPECT_EQ(2u, GetErrorCount());
        EXPECT_EQ("property 'platform' ignored, type string expected.", errors()[0]);
    ...
    {
        auto& manifest = ParseManifest(R"({ "related_applications": [{"id": "foo"}]})");
        EXPECT_TRUE(manifest->related_applications.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("'platform' is a required field, related application ignored.", errors()[0]);
    }
    ...
    {
        auto& manifest = ParseManifest(R"({ "related_applications": [{"platform": "play"}]})");
        EXPECT_TRUE(manifest->related_applications.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("one of 'url' or 'id' is required, related application ignored.", errors()[0]);
    }
    ...
    // Application with an invalid url.
    {
        ...
        EXPECT_EQ("one of 'url' or 'id' is required, related application ignored.", errors()[1]);
    }
    ...
    // be in the list.
    {
        ...
        EXPECT_EQ(2u, GetErrorCount());
        EXPECT_EQ("one of 'url' or 'id' is required, related application ignored.", errors()[0]);
        EXPECT_EQ("'platform' is a required field, related application ignored.", errors()[1]);
    }
}
...
TEST_F(ManifestParserTest, ParsePreferRelatedApplicationsParseRules)
{
    // Smoke test.
    {
        auto& manifest = ParseManifest(R"({ "prefer_related_applications": true })");
    ...
    // Don't parse if the property isn't a boolean.
    {
        ...
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'prefer_related_applications' "
    ...
    {
        auto& manifest = ParseManifest(R"({ "prefer_related_applications": false })");
        EXPECT_FALSE(manifest->prefer_related_applications);
        EXPECT_EQ(0u, GetErrorCount());
    }
}
...
TEST_F(ManifestParserTest, ThemeColorParserRules)
{
    ...
    // Don't parse if theme_color isn't a string.
    {
        ...
        EXPECT_FALSE(manifest->has_theme_color);
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'theme_color' ignored, type string expected.", errors()[0]);
    }
    ...
    // Parse fails if string is not in a known format.
    {
        ...
        EXPECT_FALSE(manifest->has_theme_color);
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'theme_color' ignored, 'bleu' is not a valid color.", errors()[0]);
    }
    ...
    {
        auto& manifest = ParseManifest(R"~({ "theme_color": "rgba(255,0,0,0.4)" })~");
...
TEST_F(ManifestParserTest, BackgroundColorParserRules)
{
    ...
    // Don't parse if background_color isn't a string.
    {
        ...
        EXPECT_FALSE(manifest->has_background_color);
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'background_color' ignored, type string expected.", errors()[0]);
    }
    ...
    // Parse fails if multiple values for background_color are given.
    {
        auto& manifest = ParseManifest(R"({ "background_color": "#AABBCC #DDEEFF" })");
        EXPECT_FALSE(manifest->has_background_color);
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'background_color' ignored, "
                  "'#AABBCC #DDEEFF' is not a valid color.",
    ...
    // Accept translucent colors.
    {
        auto& manifest = ParseManifest(R"~({ "background_color": "rgba(255,0,0,0.4)" })~");
    ...
    // Accept transparent colors.
    {
        auto& manifest = ParseManifest(R"~({ "background_color": "rgba(0,0,0,0)" })~");
...
TEST_F(ManifestParserTest, GCMSenderIDParseRules)
{
    ...
    // Don't parse if the property isn't a string.
    {
        ...
        EXPECT_TRUE(manifest->gcm_sender_id.IsNull());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'gcm_sender_id' ignored, type string expected.", errors()[0]);
    }
    {
...
TEST_F(ManifestParserTest, PermissionsPolicyParsesSelf)
{
    ...
    EXPECT_EQ("http://foo.com", manifest->permissions_policy[0].self_if_matches->Serialize());
...
TEST_F(ManifestParserTest, PermissionsPolicyAsArray)
{
    ...
    EXPECT_EQ("property 'permissions_policy' ignored, type object expected.", errors()[0]);
}
...
TEST_F(ManifestParserTest, PermissionsPolicyInvalidAllowlistType)
{
    ...
    EXPECT_EQ(2u, GetErrorCount());
    EXPECT_EQ(1u, manifest->permissions_policy.size());
    EXPECT_EQ("permission 'microphone' ignored, invalid allowlist: type array "
              "expected.",
        errors()[0]);
    EXPECT_EQ("permission 'midi' ignored, invalid allowlist: type array expected.", errors()[1]);
}
...
TEST_F(ManifestParserTest, PermissionsPolicyInvalidAllowlistEntry)
{
    ...
    EXPECT_EQ(2u, GetErrorCount());
    EXPECT_EQ(0u, manifest->permissions_policy.size());
    EXPECT_EQ("permissions_policy entry ignored, required property 'origin' contains "
...
TEST_F(ManifestParserTest, LaunchHandlerParseRules)
{
    ...
    // Smoke test.
    {
      ...
      "launch_handler": {
        ...
        EXPECT_EQ(manifest->launch_handler->client_mode, ClientMode::kFocusExisting);
        EXPECT_EQ(0u, GetErrorCount());
    }
    {
        auto& manifest = ParseManifest(R"({
    ...
    // First known value in array is used.
    {
      ...
      "launch_handler": {
        ...
        EXPECT_EQ(manifest->launch_handler->client_mode, ClientMode::kNavigateExisting);
        EXPECT_EQ(0u, GetErrorCount());
    }
    {
        auto& manifest = ParseManifest(R"({
      "launch_handler": {
        ...
        EXPECT_EQ(manifest->launch_handler->client_mode, ClientMode::kFocusExisting);
        EXPECT_EQ(2u, GetErrorCount());
        EXPECT_EQ("client_mode value 'null' ignored, string expected.", errors()[0]);
...
{
    {
        ...
        auto& manifest = ParseManifest(R"({ "translations": {"fr": {"name": "french name"}} })");
    ...
    }
    {
        ...
        // Valid name, short_name and description should be parsed
        {
           ...
           "fr name", "description": "french description"}} })");
            ...
            EXPECT_EQ(manifest->translations.find("fr")->value->short_name, "fr name");
            EXPECT_EQ(manifest->translations.find("fr")->value->description, "french description");
            EXPECT_EQ(0u, GetErrorCount());
        }
        ...
        // Don't parse if the property isn't an object.
        {
            ...
            EXPECT_TRUE(manifest->translations.empty());
            EXPECT_EQ(1u, GetErrorCount());
            EXPECT_EQ("property 'translations' ignored, object expected.", errors()[0]);
        }
        ...
        {
            auto& manifest = ParseManifest(R"({ "translations": {"": {"name": "translated name"}} })");
            EXPECT_TRUE(manifest->translations.empty());
            EXPECT_EQ(1u, GetErrorCount());
            EXPECT_EQ("skipping translation, non-empty locale string expected.", errors()[0]);
        }
    }
}
...
TEST_F(ManifestParserTest, TranslationsStringsParseRules)
{
    ...
    {
        auto& manifest = ParseManifest(R"({ "translations": {"fr": {"name": {}}} })");
        EXPECT_TRUE(manifest->translations.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'name' of 'translations' ignored, type string expected.", errors()[0]);
    }

    // Ignore non-string translations short_name.
    {
        auto& manifest = ParseManifest(R"({ "translations": {"fr": {"short_name": []}} })");
        EXPECT_TRUE(manifest->translations.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'short_name' of 'translations' ignored, type string "
                  "expected.",
            errors()[0]);
    }
    ...
    {
        auto& manifest = ParseManifest(R"({ "translations": {"fr": {"description": 42}} })");
        EXPECT_TRUE(manifest->translations.empty());
        EXPECT_EQ(1u, GetErrorCount());
        EXPECT_EQ("property 'description' of 'translations' ignored, type string "
                  "expected.",
            errors()[0]);
    }
    ...
    // Translation with empty strings is ignored.
    {
        ...
        EXPECT_FALSE(manifest->translations.Contains("fr"));
        EXPECT_EQ(3u, GetErrorCount());
        EXPECT_EQ("property 'name' of 'translations' is an empty string.", errors()[0]);
        EXPECT_EQ("property 'short_name' of 'translations' is an empty string.", errors()[1]);
        EXPECT_EQ("property 'description' of 'translations' is an empty string.", errors()[2]);
    }
}
...
TEST_F(ManifestParserTest, TabStripParseRules)
{
    using Visibility = mojom::blink::TabStripMemberVisibility;
    {
        ...
        {
            auto& manifest = ParseManifest(R"({ "tab_strip": {"home_tab": "auto"} })");
    ...
    }
    {
        ...
        // 'tab_strip' object is empty.
        {
            ...
            EXPECT_FALSE(manifest->tab_strip.is_null());
            EXPECT_EQ(manifest->tab_strip->home_tab->get_visibility(), Visibility::kAuto);
        ...
        // Home tab and new tab button are empty objects.
        {
          ...
          "tab_strip": {"home_tab": {}, "new_tab_button": {}} })");
            ...
            EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns.size(), 0u);
            EXPECT_FALSE(manifest->tab_strip->new_tab_button->url.has_value());
            EXPECT_EQ(0u, GetErrorCount());
        }
        ...
        // Home tab with icons and new tab button with url are parsed.
        {
          ...
          "tab_strip": {
            ...
            EXPECT_EQ(manifest->tab_strip->new_tab_button->url, KURL(DefaultDocumentUrl(), "foo"));
            EXPECT_EQ(0u, GetErrorCount());
        }
        ...
        // Home tab set to 'absent'.
        {
          ...
          "tab_strip": {"home_tab": "absent"} })");
            EXPECT_FALSE(manifest->tab_strip.is_null());
            EXPECT_EQ(manifest->tab_strip->home_tab->get_visibility(), Visibility::kAbsent);
            EXPECT_FALSE(manifest->tab_strip->home_tab->is_params());
            EXPECT_FALSE(manifest->tab_strip->new_tab_button->url.has_value());
            EXPECT_EQ(0u, GetErrorCount());
        }
...
TEST_F(ManifestParserTest, TabStripHomeTabScopeParseRules)
{
    ...
    // URL.
    {
        ...
        EXPECT_FALSE(manifest->tab_strip.is_null());
        EXPECT_FALSE(manifest->tab_strip->home_tab->is_visibility());
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns.size(), 3u);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[0], 1, 0, 0, 0, 0, 0, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[1], 1, 0, 0, 1, 0, 0, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[2], 1, 0, 0, 1, 0, 0, 0, 0);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[0].protocol[0].value, "ftp");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].protocol[0].value, "http");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].hostname[0].value, "bar.com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].protocol[0].value, "ftp");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].hostname[0].value, "bar.com");
    ...
    // absolute, resolved relative to the manifest URL.
    {
        ...
        EXPECT_FALSE(manifest->tab_strip.is_null());
        EXPECT_FALSE(manifest->tab_strip->home_tab->is_visibility());
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns.size(), 4u);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[0], 1, 0, 0, 1, 0, 1, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[1], 1, 0, 0, 1, 0, 1, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[2], 1, 0, 0, 1, 0, 1, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[3], 1, 0, 0, 1, 0, 1, 0, 0);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[0].protocol[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[0].hostname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[0].hostname[0].value, "foo.com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[0].pathname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[0].pathname[0].value, "/static/foo");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].protocol[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].hostname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].hostname[0].value, "foo.com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].pathname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].pathname[0].value, "/static/foo/bar/");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].protocol[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].hostname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].hostname[0].value, "foo.com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].pathname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].pathname[0].value, "/foo/");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].protocol[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].hostname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].hostname[0].value, "foo.com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].pathname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].pathname[0].value, "/foo/bar/");
    ...
    // Base URL provided in scope patterns is respected if it is valid.
    {
        ...
        EXPECT_FALSE(manifest->tab_strip.is_null());
        EXPECT_FALSE(manifest->tab_strip->home_tab->is_visibility());
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns.size(), 5u);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[0], 1, 0, 0, 0, 0, 0, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[1], 1, 0, 0, 1, 0, 0, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[2], 1, 0, 0, 1, 0, 1, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[3], 1, 0, 0, 1, 0, 1, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[4], 1, 0, 0, 1, 0, 1, 0, 0);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[0].protocol[0].value, "ftp");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].protocol[0].value, "https");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].hostname[0].value, "bar.com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].protocol[0].value, "https");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].hostname[0].value, "bar.com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].pathname[0].value, "/foo/bar/");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].protocol[0].value, "https");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].hostname[0].value, "bar.com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].pathname[0].value, "/bar");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[4].protocol[0].value, "https");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[4].hostname[0].value, "bar.com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[4].pathname[0].value, "/foo/bar");
    ...
    // Allow patterns with wildcards and named groups in the pathname.
    {
        ...
        EXPECT_FALSE(manifest->tab_strip.is_null());
        EXPECT_FALSE(manifest->tab_strip->home_tab->is_visibility());
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns.size(), 6u);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[0], 1, 0, 0, 1, 0, 1, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[1], 1, 0, 0, 1, 0, 1, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[2], 1, 0, 0, 1, 0, 2, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[3], 1, 0, 0, 1, 0, 3, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[4], 1, 0, 0, 1, 0, 2, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[5], 1, 0, 0, 1, 0, 3, 0, 0);

        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[0].pathname[0].type, liburlpattern::PartType::kFullWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].pathname[0].type, liburlpattern::PartType::kSegmentWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].pathname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].pathname[0].value, "/foo");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].pathname[1].type, liburlpattern::PartType::kFullWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].pathname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].pathname[0].value, "/foo");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].pathname[1].type, liburlpattern::PartType::kFullWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].pathname[2].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].pathname[2].value, "/bar");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[4].pathname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[4].pathname[0].value, "/foo");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[4].pathname[1].type, liburlpattern::PartType::kSegmentWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[5].pathname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[5].pathname[0].value, "/foo");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[5].pathname[1].type, liburlpattern::PartType::kSegmentWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[5].pathname[2].type, liburlpattern::PartType::kFullWildcard);
    ...
    // Allow patterns with wildcards and named groups in the hostname.
    {
        ...
        EXPECT_FALSE(manifest->tab_strip.is_null());
        EXPECT_FALSE(manifest->tab_strip->home_tab->is_visibility());
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns.size(), 7u);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[0], 1, 0, 0, 1, 0, 0, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[1], 1, 0, 0, 1, 0, 0, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[2], 1, 0, 0, 3, 0, 0, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[3], 1, 0, 0, 2, 0, 0, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[4], 1, 0, 0, 3, 0, 0, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[5], 1, 0, 0, 3, 0, 0, 0, 0);
        VerifySafeUrlPatternSizes(manifest->tab_strip->home_tab->get_params()->scope_patterns[6], 1, 0, 0, 2, 0, 0, 0, 0);

        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[0].hostname[0].type, liburlpattern::PartType::kFullWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].hostname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[1].hostname[0].value, "bar.com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].hostname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].hostname[0].value, "bar");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].hostname[1].type, liburlpattern::PartType::kFullWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].hostname[2].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[2].hostname[2].value, ".com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].hostname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].hostname[0].value, "bar");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[3].hostname[1].type, liburlpattern::PartType::kFullWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[4].hostname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[4].hostname[0].value, "bar");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[4].hostname[1].type, liburlpattern::PartType::kFullWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[4].hostname[2].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[4].hostname[2].value, ".com");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[5].hostname[0].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[5].hostname[0].value, "foo");
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[5].hostname[1].type, liburlpattern::PartType::kSegmentWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[5].hostname[2].type, liburlpattern::PartType::kFullWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[6].hostname[0].type, liburlpattern::PartType::kFullWildcard);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[6].hostname[1].type, liburlpattern::PartType::kFixed);
        EXPECT_EQ(manifest->tab_strip->home_tab->get_params()->scope_patterns[6].hostname[1].value, ".com");
