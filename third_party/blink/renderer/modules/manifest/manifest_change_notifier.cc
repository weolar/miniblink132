...
ManifestChangeNotifier::ManifestChangeNotifier(LocalDOMWindow& window)
    : window_(window)
    , manifest_change_observer_(&window)
{
}
...
void ManifestChangeNotifier::DidChangeManifest()
{
    ...
    if (!window_->GetFrame()->IsLoading()) {
        ...
        window_->GetTaskRunner(TaskType::kInternalLoading)
            ->PostTask(FROM_HERE, WTF::BindOnce(&ManifestChangeNotifier::ReportManifestChange, WrapWeakPersistent(this)));
        return;
    }
...
void ManifestChangeNotifier::EnsureManifestChangeObserver()
{
    ...
    AssociatedInterfaceProvider* provider = window_->GetFrame()->GetRemoteNavigationAssociatedInterfaces();
    if (!provider)
        return;

    provider->GetInterface(manifest_change_observer_.BindNewEndpointAndPassReceiver(window_->GetTaskRunner(TaskType::kInternalLoading)));
}
