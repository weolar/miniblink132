...
{
    return SecurityOrigin::AreSameOrigin(url, scope) && url.GetPath().ToString().StartsWith(scope.GetPath());
}
...
bool IsHostValidForScopeExtension(String host)
{
    ...
    const size_t registry_length = net::registry_controlled_domains::PermissiveGetHostRegistryLength(host.Utf8(),
    ...
    if (registry_length == 0 || registry_length == std::string::npos || registry_length >= host.length()) {
        return false;
    }
...
std::optional<mojom::blink::ManifestFileHandler::LaunchType> FileHandlerLaunchTypeFromString(const std::string& launch_type)
{
...
bool IsDefaultManifest(const mojom::blink::Manifest& manifest, const KURL& document_url)
{
...
void ParseSucceeded(const mojom::blink::ManifestPtr& manifest, const KURL& document_url)
{
    if (IsDefaultManifest(*manifest, document_url)) {
        return;
    }

    base::UmaHistogramBoolean("Manifest.HasProperty.name", !manifest->name.empty());
    base::UmaHistogramBoolean("Manifest.HasProperty.short_name", !manifest->short_name.empty());
    base::UmaHistogramBoolean("Manifest.HasProperty.description", !manifest->description.empty());
    base::UmaHistogramBoolean("Manifest.HasProperty.start_url", !manifest->start_url.IsEmpty());
    base::UmaHistogramBoolean("Manifest.HasProperty.display", manifest->display != blink::mojom::DisplayMode::kUndefined);
    base::UmaHistogramBoolean("Manifest.HasProperty.orientation", manifest->orientation != device::mojom::blink::ScreenOrientationLockType::DEFAULT);
    base::UmaHistogramBoolean("Manifest.HasProperty.icons", !manifest->icons.empty());
    base::UmaHistogramBoolean("Manifest.HasProperty.screenshots", !manifest->screenshots.empty());
    base::UmaHistogramBoolean("Manifest.HasProperty.share_target", manifest->share_target.get());
    base::UmaHistogramBoolean("Manifest.HasProperty.protocol_handlers", !manifest->protocol_handlers.empty());
    base::UmaHistogramBoolean("Manifest.HasProperty.gcm_sender_id", !manifest->gcm_sender_id.empty());
}
...
std::optional<std::vector<liburlpattern::Part>> ParsePatternInitField(const std::optional<String>& field, const String default_field_value)
{
    ...
    auto parse_result = liburlpattern::Parse(utf8.AsStringView(), [](std::string_view input) { return std::string(input); });
...
ManifestParser::ManifestParser(const String& data, const KURL& manifest_url, const KURL& document_url, ExecutionContext* execution_context)
    : data_(data)
...
bool ManifestParser::Parse()
{
    ...
    std::unique_ptr<JSONValue> root = ParseJSONWithCommentsDeprecated(data_, &error, &has_comments);
    ...
    const auto& [start_url, start_url_parse_result] = ParseStartURL(root_object.get(), document_url_);
    manifest_->start_url = start_url;
    manifest_->has_valid_specified_start_url = start_url_parse_result == ParseStartUrlResult::kParsedFromJson;

    const auto& [id, id_parse_result] = ParseId(root_object.get(), manifest_->start_url);
    manifest_->id = id;
    ...
    manifest_->prefer_related_applications = ParsePreferRelatedApplications(root_object.get());
    ...
    std::optional<RGBA32> background_color = ParseBackgroundColor(root_object.get());
    ...
    manifest_->permissions_policy = ParseIsolatedAppPermissions(root_object.get());
    ...
    if (RuntimeEnabledFeatures::WebAppTabStripCustomizationsEnabled(execution_context_)) {
...
void ManifestParser::TakeErrors(Vector<mojom::blink::ManifestErrorPtr>* errors)
{
    errors->clear();
    errors->swap(errors_);
}
...
ManifestParser::PatternInit::PatternInit(std::optional<String> protocol, std::optional<String> username, std::optional<String> password,
    std::optional<String> hostname, std::optional<String> port, std::optional<String> pathname, std::optional<String> search, std::optional<String> hash,
    KURL base_url)
...
ManifestParser::PatternInit& ManifestParser::PatternInit::operator=(PatternInit&&) = default;
...
bool ManifestParser::ParseBoolean(const JSONObject* object, const String& key, bool default_value)
{
...
std::optional<String> ManifestParser::ParseString(const JSONObject* object, const String& key, Trim trim)
{
    JSONValue* json_value = object->Get(key);
    if (!json_value) {
        return std::nullopt;
...
std::optional<String> ManifestParser::ParseStringForMember(const JSONObject* object, const String& member_name, const String& key, bool required, Trim trim)
{
    JSONValue* json_value = object->Get(key);
    if (!json_value) {
        if (required) {
            AddErrorInfo("property '" + key + "' of '" + member_name + "' not present.");
        }
    ...
    if (!json_value->AsString(&value)) {
        AddErrorInfo("property '" + key + "' of '" + member_name + "' ignored, type string expected.");
        return std::nullopt;
    }
    if (trim) {
        value = value.StripWhiteSpace();
    ...
    if (value == "") {
        AddErrorInfo("property '" + key + "' of '" + member_name + "' is an empty string.");
        if (required) {
            return std::nullopt;
        }
    }
...
std::optional<RGBA32> ManifestParser::ParseColor(const JSONObject* object, const String& key)
{
    ...
    if (!CSSParser::ParseColor(color, *parsed_color, true)) {
        AddErrorInfo("property '" + key + "' ignored, '" + *parsed_color + "' is not a " + "valid color.");
        return std::nullopt;
    }
...
KURL ManifestParser::ParseURL(
    const JSONObject* object, const String& key, const KURL& base_url, ParseURLRestrictions origin_restriction, bool ignore_empty_string)
{
    ...
    case ParseURLRestrictions::kSameOriginOnly:
        if (!SecurityOrigin::AreSameOrigin(resolved, document_url_)) {
            AddErrorInfo("property '" + key + "' ignored, should be same origin as document.");
            return KURL();
        }
        return resolved;
    case ParseURLRestrictions::kWithinScope:
        if (!URLIsWithinScope(resolved, manifest_->scope)) {
            AddErrorInfo("property '" + key + "' ignored, should be within scope of the manifest.");
            return KURL();
        }
...
Enum ManifestParser::ParseFirstValidEnum(const JSONObject* object, const String& key, Enum (*parse_enum)(const std::string&), Enum invalid_value)
{
    ...
    if (!list) {
        AddErrorInfo("property '" + key + "' ignored, type string or array of strings expected.");
        return invalid_value;
    }
    ...
    for (wtf_size_t i = 0; i < list->size(); ++i) {
        ...
        if (!item->AsString(&string_value)) {
            AddErrorInfo(key + " value '" + item->ToJSONString() + "' ignored, string expected.");
            continue;
        }
...
mojom::blink::Manifest::TextDirection ManifestParser::ParseDir(const JSONObject* object)
{
    ...
    std::optional<TextDirection> textDirection = TextDirectionFromString(dir->Utf8());
...
{
    std::optional<String> short_name = ParseString(object, "short_name", Trim(true));
...
{
    std::optional<String> description = ParseString(object, "description", Trim(true));
    return description.has_value() ? *description : String();
}

std::pair<KURL, ManifestParser::ParseIdResultType> ManifestParser::ParseId(const JSONObject* object, const KURL& start_url)
{
    ...
    KURL id = ParseURL(object, "id", start_url_origin, ParseURLRestrictions::kSameOriginOnly,
...
std::pair<KURL, ManifestParser::ParseStartUrlResult> ManifestParser::ParseStartURL(const JSONObject* object, const KURL& document_url)
{
    KURL start_url = ParseURL(object, "start_url", manifest_url_, ParseURLRestrictions::kSameOriginOnly);
    if (start_url.IsEmpty()) {
        return std::make_pair(document_url, ParseStartUrlResult::kDefaultDocumentUrl);
    }
    return std::make_pair(start_url, ParseStartUrlResult::kParsedFromJson);
}

KURL ManifestParser::ParseScope(const JSONObject* object, const KURL& start_url)
{
    ...
    if (!URLIsWithinScope(default_value, scope)) {
        AddErrorInfo("property 'scope' ignored. Start url should be within scope "
                     "of scope URL.");
        return KURL(default_value.BaseAsString().ToString());
    }
...
blink::mojom::DisplayMode ManifestParser::ParseDisplay(const JSONObject* object)
{
    ...
    blink::mojom::DisplayMode display_enum = DisplayModeFromString(display->Utf8());
...
Vector<mojom::blink::DisplayMode> ManifestParser::ParseDisplayOverride(const JSONObject* object)
{
    ...
    for (wtf_size_t i = 0; i < display_override_list->size(); ++i) {
        ...
        mojom::blink::DisplayMode display_enum = DisplayModeFromString(display_enum_string.Utf8());

        if (!RuntimeEnabledFeatures::WebAppTabStripEnabled(execution_context_) && display_enum == mojom::blink::DisplayMode::kTabbed) {
            display_enum = mojom::blink::DisplayMode::kUndefined;
        }

        if (!base::FeatureList::IsEnabled(blink::features::kWebAppBorderless) && display_enum == mojom::blink::DisplayMode::kBorderless) {
            display_enum = mojom::blink::DisplayMode::kUndefined;
        }
...
device::mojom::blink::ScreenOrientationLockType ManifestParser::ParseOrientation(const JSONObject* object)
{
    std::optional<String> orientation = ParseString(object, "orientation", Trim(true));

    if (!orientation.has_value()) {
        return device::mojom::blink::ScreenOrientationLockType::DEFAULT;
    }

    device::mojom::blink::ScreenOrientationLockType orientation_enum = WebScreenOrientationLockTypeFromString(orientation->Utf8());
    if (orientation_enum == device::mojom::blink::ScreenOrientationLockType::DEFAULT) {
...
{
    return ParseURL(icon, "src", manifest_url_, ParseURLRestrictions::kNoRestrictions);
}
...
Vector<gfx::Size> ManifestParser::ParseIconSizes(const JSONObject* icon)
{
    ...
    WebVector<gfx::Size> web_sizes = WebIconSizesParser::ParseIconSizes(WebString(*sizes_str));
...
std::optional<Vector<mojom::blink::ManifestImageResource::Purpose>> ManifestParser::ParseIconPurpose(const JSONObject* icon)
{
    ...
    purpose_str.value().Split(/*separator=*/" ", /*allow_empty_entries=*/false, keywords);
    ...
    for (auto& keyword : keywords) {
        ...
        if (EqualIgnoringASCIICase(keyword, "any")) {
            ...
            purposes.push_back(mojom::blink::ManifestImageResource::Purpose::MONOCHROME);
        } else if (EqualIgnoringASCIICase(keyword, "maskable")) {
            purposes.push_back(mojom::blink::ManifestImageResource::Purpose::MASKABLE);
        } else {
    ...
    if (unrecognised_purpose) {
        AddErrorInfo("found icon with one or more invalid purposes; those purposes are "
                     "ignored.");
    }
...
mojom::blink::ManifestScreenshot::FormFactor ManifestParser::ParseScreenshotFormFactor(const JSONObject* screenshot)
{
    std::optional<String> form_factor_str = ParseString(screenshot, "form_factor", Trim(false));
    ...
    AddErrorInfo("property 'form_factor' on screenshots has an invalid value, ignoring "
                 "it.");
...
Vector<mojom::blink::ManifestImageResourcePtr> ManifestParser::ParseIcons(const JSONObject* object)
{
    return ParseImageResourceArray("icons", object);
}

Vector<mojom::blink::ManifestScreenshotPtr> ManifestParser::ParseScreenshots(const JSONObject* object)
{
...
Vector<mojom::blink::ManifestImageResourcePtr> ManifestParser::ParseImageResourceArray(const String& key, const JSONObject* object)
{
...
std::optional<mojom::blink::ManifestImageResourcePtr> ManifestParser::ParseImageResource(const JSONValue* object)
{
...
{
    std::optional<String> name = ParseStringForMember(shortcut, "shortcut", "name", true, Trim(true));
    return name.has_value() ? *name : String();
}

String ManifestParser::ParseShortcutShortName(const JSONObject* shortcut)
{
    std::optional<String> short_name = ParseStringForMember(shortcut, "shortcut", "short_name", false, Trim(true));
    return short_name.has_value() ? *short_name : String();
}

String ManifestParser::ParseShortcutDescription(const JSONObject* shortcut)
{
    std::optional<String> description = ParseStringForMember(shortcut, "shortcut", "description", false, Trim(true));
    return description.has_value() ? *description : String();
}

KURL ManifestParser::ParseShortcutUrl(const JSONObject* shortcut)
{
    KURL shortcut_url = ParseURL(shortcut, "url", manifest_url_, ParseURLRestrictions::kWithinScope);
...
Vector<mojom::blink::ManifestShortcutItemPtr> ManifestParser::ParseShortcuts(const JSONObject* object)
{
    ...
    for (wtf_size_t i = 0; i < shortcuts_list->size(); ++i) {
        if (i == kMaxShortcutsSize) {
            AddErrorInfo("property 'shortcuts' contains more than " + String::Number(kMaxShortcutsSize) + " valid elements, only the first "
...
Vector<mojom::blink::ManifestFileFilterPtr> ManifestParser::ParseTargetFiles(const String& key, const JSONObject* from)
{
    ...
    if (!file_list) {
        ...
        if (!file_object) {
            AddErrorInfo("property 'files' ignored, type array or FileFilter expected.");
            return files;
        }
...
void ManifestParser::ParseFileFilter(const JSONObject* file_object, Vector<mojom::blink::ManifestFileFilterPtr>* files)
{
...
std::optional<mojom::blink::ManifestShareTarget::Method> ManifestParser::ParseShareTargetMethod(const JSONObject* share_target_object)
{
    if (!share_target_object->Get("method")) {
        AddErrorInfo("Method should be set to either GET or POST. It currently defaults to "
                     "GET.");
...
std::optional<mojom::blink::ManifestShareTarget::Enctype> ManifestParser::ParseShareTargetEnctype(const JSONObject* share_target_object)
{
    if (!share_target_object->Get("enctype")) {
        AddErrorInfo("Enctype should be set to either application/x-www-form-urlencoded or "
...
mojom::blink::ManifestShareTargetParamsPtr ManifestParser::ParseShareTargetParams(const JSONObject* share_target_params)
{
    ...
    std::optional<String> text = ParseString(share_target_params, "text", Trim(true));
    params->text = text.has_value() ? *text : String();
    std::optional<String> title = ParseString(share_target_params, "title", Trim(true));
    params->title = title.has_value() ? *title : String();
    std::optional<String> url = ParseString(share_target_params, "url", Trim(true));
...
std::optional<mojom::blink::ManifestShareTargetPtr> ManifestParser::ParseShareTarget(const JSONObject* object)
{
    ...
    share_target->action = ParseURL(share_target_object, "action", manifest_url_, ParseURLRestrictions::kWithinScope);
    if (!share_target->action.IsValid()) {
        AddErrorInfo("property 'share_target' ignored. Property 'action' is "
                     "invalid.");
        return std::nullopt;
    }
    ...
    const JSONObject* share_target_params_object = share_target_object->GetJSONObject("params");
    if (!share_target_params_object) {
        AddErrorInfo("property 'share_target' ignored. Property 'params' type "
                     "dictionary expected.");
        return std::nullopt;
    }
    ...
    if (!method.has_value()) {
        AddErrorInfo("invalid method. Allowed methods are:"
                     "GET and POST.");
        return std::nullopt;
    }
    ...
    if (!enctype.has_value()) {
        AddErrorInfo("invalid enctype. Allowed enctypes are:"
    ...
    if (share_target->method == mojom::blink::ManifestShareTarget::Method::kGet) {
        if (share_target->enctype == mojom::blink::ManifestShareTarget::Enctype::kMultipartFormData) {
            AddErrorInfo("invalid enctype for GET method. Only "
    ...
    if (share_target->params->files.has_value()) {
        if (share_target->method != mojom::blink::ManifestShareTarget::Method::kPost
            || share_target->enctype != mojom::blink::ManifestShareTarget::Enctype::kMultipartFormData) {
            AddErrorInfo("files are only supported with multipart/form-data POST.");
            return std::nullopt;
        }
    }

    if (share_target->params->files.has_value() && !VerifyFiles(*share_target->params->files)) {
...
Vector<mojom::blink::ManifestFileHandlerPtr> ManifestParser::ParseFileHandlers(const JSONObject* object)
{
    ...
    for (wtf_size_t i = 0; i < entry_array->size(); ++i) {
        ...
        std::optional<mojom::blink::ManifestFileHandlerPtr> entry = ParseFileHandler(json_entry);
        if (!entry) {
            continue;
        }
...
std::optional<mojom::blink::ManifestFileHandlerPtr> ManifestParser::ParseFileHandler(const JSONObject* file_handler)
{
    mojom::blink::ManifestFileHandlerPtr entry = mojom::blink::ManifestFileHandler::New();
    entry->action = ParseURL(file_handler, "action", manifest_url_, ParseURLRestrictions::kWithinScope);
    ...
    const bool feature_enabled
        = base::FeatureList::IsEnabled(blink::features::kFileHandlingIcons) || RuntimeEnabledFeatures::FileHandlingIconsEnabled(execution_context_);
    if (feature_enabled) {
    ...
    entry->launch_type
        = ParseFirstValidEnum<std::optional<mojom::blink::ManifestFileHandler::LaunchType>>(file_handler, "launch_type", &FileHandlerLaunchTypeFromString,
            /*invalid_value=*/std::nullopt)
              .value_or(mojom::blink::ManifestFileHandler::LaunchType::kSingleClient);

    return entry;
}

HashMap<String, Vector<String>> ManifestParser::ParseFileHandlerAccept(const JSONObject* accept)
{
    ...
    const int kExtensionLimit = g_file_handler_extension_limit_for_testing > 0 ? g_file_handler_extension_limit_for_testing : kFileHandlerExtensionLimit;
    ...
    for (wtf_size_t i = 0; i < accept->size(); ++i) {
        ...
        if (!net::ParseMimeTypeWithoutParameter(mimetype.Utf8(), &top_level_mime_type, nullptr) || !net::IsValidTopLevelMimeType(top_level_mime_type)) {
        ...
        if (extensions_array) {
            for (wtf_size_t j = 0; j < extensions_array->size(); ++j) {
                ...
                if (!value->AsString(&extension)) {
                    AddErrorInfo("property 'accept' file extension ignored, type string "
                                 "expected.");
                    continue;
                }
        ...
        int extension_overflow = total_file_handler_extension_count_ - kExtensionLimit;
        if (extension_overflow > 0) {
            auto erase_iter = extensions.end() - extension_overflow;
            AddErrorInfo("property 'accept': too many total file extensions, ignoring "
...
bool ManifestParser::ParseFileHandlerAcceptExtension(const JSONValue* extension, String* output)
{
    if (!extension->AsString(output)) {
        AddErrorInfo("property 'accept' type ignored. File extensions must be type array or "
                     "type string.");
        return false;
    }

    if (!output->StartsWith(".")) {
        AddErrorInfo("property 'accept' file extension ignored, must start with a '.'.");
        return false;
    }
...
Vector<mojom::blink::ManifestProtocolHandlerPtr> ManifestParser::ParseProtocolHandlers(const JSONObject* from)
{
    ...
    for (wtf_size_t i = 0; i < protocol_list->size(); ++i) {
        ...
        std::optional<mojom::blink::ManifestProtocolHandlerPtr> protocol = ParseProtocolHandler(protocol_object);
        if (!protocol) {
            continue;
        }
...
std::optional<mojom::blink::ManifestProtocolHandlerPtr> ManifestParser::ParseProtocolHandler(const JSONObject* object)
{
    if (!object->Get("protocol")) {
        AddErrorInfo("protocol_handlers entry ignored, required property 'protocol' is "
                     "missing.");
        return std::nullopt;
    }
    ...
    if (is_valid_protocol && !VerifyCustomHandlerScheme(protocol.value(), error_message, ProtocolHandlerSecurityLevel::kStrict)) {
    ...
    if (!object->Get("url")) {
        AddErrorInfo("protocol_handlers entry ignored, required property 'url' is missing.");
        return std::nullopt;
    }
    protocol_handler->url = ParseURL(object, "url", manifest_url_, ParseURLRestrictions::kWithinScope);
    ...
    if (is_valid_url) {
        ...
        if (!VerifyCustomHandlerURLSyntax(full_url, manifest_url_, user_url, error_message)) {
            AddErrorInfo(error_message);
            is_valid_url = false;
        }
    }
    ...
    if (!is_valid_url) {
        AddErrorInfo("protocol_handlers entry ignored, required property 'url' is invalid.");
        return std::nullopt;
    }
...
Vector<mojom::blink::ManifestUrlHandlerPtr> ManifestParser::ParseUrlHandlers(const JSONObject* from)
{
    Vector<mojom::blink::ManifestUrlHandlerPtr> url_handlers;
    const bool feature_enabled
        = base::FeatureList::IsEnabled(blink::features::kWebAppEnableUrlHandlers) || RuntimeEnabledFeatures::WebAppUrlHandlingEnabled(execution_context_);
    ...
    for (wtf_size_t i = 0; i < handlers_list->size(); ++i) {
        if (i == kMaxUrlHandlersSize) {
            AddErrorInfo("property 'url_handlers' contains more than " + String::Number(kMaxUrlHandlersSize) + " valid elements, only the first "
        ...
        std::optional<mojom::blink::ManifestUrlHandlerPtr> url_handler = ParseUrlHandler(handler_object);
        if (!url_handler) {
            continue;
        }
...
std::optional<mojom::blink::ManifestUrlHandlerPtr> ManifestParser::ParseUrlHandler(const JSONObject* object)
{
    DCHECK(base::FeatureList::IsEnabled(blink::features::kWebAppEnableUrlHandlers) || RuntimeEnabledFeatures::WebAppUrlHandlingEnabled(execution_context_));
    if (!object->Get("origin")) {
        AddErrorInfo("url_handlers entry ignored, required property 'origin' is missing.");
        return std::nullopt;
    }
    const std::optional<String> origin_string = ParseString(object, "origin", Trim(true));
    if (!origin_string.has_value()) {
        AddErrorInfo("url_handlers entry ignored, required property 'origin' is invalid.");
        return std::nullopt;
    }
    ...
    if (origin_string->length() > kMaxOriginLength) {
        AddErrorInfo("url_handlers entry ignored, 'origin' exceeds maximum character length "
                     "of "
    ...
    if (origin->Protocol() != url::kHttpsScheme) {
        AddErrorInfo("url_handlers entry ignored, required property 'origin' must use the "
                     "https scheme.");
        return std::nullopt;
    }
    ...
    if (!host_valid) {
        AddErrorInfo("url_handlers entry ignored, domain of required property 'origin' is "
                     "invalid.");
        return std::nullopt;
    }

    if (url_handler->has_origin_wildcard) {
        origin = SecurityOrigin::CreateFromValidTuple(origin->Protocol(), host, origin->Port());
        if (!origin_string.has_value()) {
            AddErrorInfo("url_handlers entry ignored, required property 'origin' is invalid.");
            return std::nullopt;
        }
...
Vector<mojom::blink::ManifestScopeExtensionPtr> ManifestParser::ParseScopeExtensions(const JSONObject* from)
{
    Vector<mojom::blink::ManifestScopeExtensionPtr> scope_extensions;
    const bool feature_enabled = base::FeatureList::IsEnabled(blink::features::kWebAppEnableScopeExtensions)
    ...
    for (wtf_size_t i = 0; i < extensions_list->size(); ++i) {
        if (i == kMaxScopeExtensionsSize) {
            AddErrorInfo("property 'scope_extensions' contains more than " + String::Number(kMaxScopeExtensionsSize) + " valid elements, only the first "
        ...
        if (entry_type != JSONValue::kTypeString && entry_type != JSONValue::kTypeObject) {
            AddErrorInfo("scope_extensions entry ignored, type string or object expected.");
            continue;
        }
        ...
        if (expected_entry_type != JSONValue::kTypeString && expected_entry_type != JSONValue::kTypeObject) {
            expected_entry_type = entry_type;
        }

        std::optional<mojom::blink::ManifestScopeExtensionPtr> scope_extension = std::nullopt;
...
std::optional<mojom::blink::ManifestScopeExtensionPtr> ManifestParser::ParseScopeExtension(const JSONObject* object)
{
    DCHECK(base::FeatureList::IsEnabled(blink::features::kWebAppEnableScopeExtensions)
        || RuntimeEnabledFeatures::WebAppScopeExtensionsEnabled(execution_context_));
    if (!object->Get("origin")) {
        AddErrorInfo("scope_extensions entry ignored, required property 'origin' is "
                     "missing.");
        return std::nullopt;
    }
    const std::optional<String> origin_string = ParseString(object, "origin", Trim(true));
...
std::optional<mojom::blink::ManifestScopeExtensionPtr> ManifestParser::ParseScopeExtensionOrigin(const String& origin_string)
{
    DCHECK(base::FeatureList::IsEnabled(blink::features::kWebAppEnableScopeExtensions)
        || RuntimeEnabledFeatures::WebAppScopeExtensionsEnabled(execution_context_));
    ...
    if (origin_string.length() > kMaxOriginLength) {
        AddErrorInfo("scope_extensions entry ignored, 'origin' exceeds maximum character "
                     "length of "
            + String::Number(kMaxOriginLength) + " .");
        return std::nullopt;
    }
    ...
    if (origin->Protocol() != url::kHttpsScheme) {
        AddErrorInfo("scope_extensions entry ignored, required property 'origin' must use "
                     "the https scheme.");
        return std::nullopt;
    }
    ...
    bool host_valid = IsHostValidForScopeExtension(host);
    if (!host_valid) {
        AddErrorInfo("scope_extensions entry ignored, domain of required property 'origin' "
                     "is invalid.");
        return std::nullopt;
    }
...
KURL ManifestParser::ParseLockScreenStartUrl(const JSONObject* lock_screen)
{
    ...
    KURL start_url = ParseURL(lock_screen, "start_url", manifest_url_, ParseURLRestrictions::kWithinScope);
    if (!start_url.IsValid()) {
...
mojom::blink::ManifestLockScreenPtr ManifestParser::ParseLockScreen(const JSONObject* manifest)
{
...
KURL ManifestParser::ParseNoteTakingNewNoteUrl(const JSONObject* note_taking)
{
    ...
    KURL new_note_url = ParseURL(note_taking, "new_note_url", manifest_url_, ParseURLRestrictions::kWithinScope);
...
mojom::blink::ManifestNoteTakingPtr ManifestParser::ParseNoteTaking(const JSONObject* manifest)
{
...
String ManifestParser::ParseRelatedApplicationPlatform(const JSONObject* application)
{
    std::optional<String> platform = ParseString(application, "platform", Trim(true));
    return platform.has_value() ? *platform : String();
}

std::optional<KURL> ManifestParser::ParseRelatedApplicationURL(const JSONObject* application)
{
    return ParseURL(application, "url", manifest_url_, ParseURLRestrictions::kNoRestrictions);
}

String ManifestParser::ParseRelatedApplicationId(const JSONObject* application)
{
    std::optional<String> id = ParseString(application, "id", Trim(true));
    return id.has_value() ? *id : String();
}

Vector<mojom::blink::ManifestRelatedApplicationPtr> ManifestParser::ParseRelatedApplications(const JSONObject* object)
{
    ...
    for (wtf_size_t i = 0; i < applications_list->size(); ++i) {
        const JSONObject* application_object = JSONObject::Cast(applications_list->at(i));
        ...
        // left."
        if ((!application->url.has_value() || !application->url->IsValid()) && application->id.empty()) {
...
std::optional<RGBA32> ManifestParser::ParseThemeColor(const JSONObject* object)
{
    return ParseColor(object, "theme_color");
}

std::optional<RGBA32> ManifestParser::ParseBackgroundColor(const JSONObject* object)
{
...
{
    std::optional<String> gcm_sender_id = ParseString(object, "gcm_sender_id", Trim(true));
    return gcm_sender_id.has_value() ? *gcm_sender_id : String();
}

Vector<blink::ParsedPermissionsPolicyDeclaration> ManifestParser::ParseIsolatedAppPermissions(const JSONObject* object)
{
    PermissionsPolicyParser::Node policy { OriginWithPossibleWildcards::NodeType::kHeader };
    ...
    if (!permissions_dict) {
        AddErrorInfo("property 'permissions_policy' ignored, type object expected.");
        return Vector<blink::ParsedPermissionsPolicyDeclaration>();
    }
    ...
    for (wtf_size_t i = 0; i < permissions_dict->size(); ++i) {
        ...
        if (!origin_allowlist) {
            AddErrorInfo("permission '" + feature + "' ignored, invalid allowlist: type array expected.");
            continue;
        }
        ...
        for (const auto& origin : allowlist) {
            ...
            if (EqualIgnoringASCIICase(origin, "self") || EqualIgnoringASCIICase(origin, "none")) {
    ...
    PolicyParserMessageBuffer logger("Error with permissions_policy manifest field: ");
    blink::ParsedPermissionsPolicy parsed_policy
        = PermissionsPolicyParser::ParsePolicyFromNode(policy, SecurityOrigin::Create(manifest_url_), logger, execution_context_);
...
Vector<String> ManifestParser::ParseOriginAllowlist(const JSONArray* json_allowlist, const String& feature)
{
    Vector<String> out;
    for (wtf_size_t i = 0; i < json_allowlist->size(); ++i) {
        JSONValue* json_value = json_allowlist->at(i);
        if (!json_value) {
            AddErrorInfo("permissions_policy entry ignored, required property 'origin' is "
                         "invalid.");
        ...
        if (!json_value->AsString(&origin_string) || origin_string.IsNull()) {
            AddErrorInfo("permissions_policy entry ignored, required property 'origin' "
                         "contains "
        ...
        if (origin_string.length() > kMaxOriginLength) {
            AddErrorInfo("permissions_policy entry ignored, 'origin' exceeds maximum "
                         "character length "
                         "of "
                + String::Number(kMaxOriginLength) + " .");
            return Vector<String>();
...
mojom::blink::ManifestLaunchHandlerPtr ManifestParser::ParseLaunchHandler(const JSONObject* object)
{
    ...
    const JSONObject* launch_handler_object = JSONObject::Cast(launch_handler_value);
    ...
    return mojom::blink::ManifestLaunchHandler::New(ParseFirstValidEnum<std::optional<ClientMode>>(launch_handler_object, "client_mode", &ClientModeFromString,
        /*invalid_value=*/std::nullopt)
                                                        .value_or(ClientMode::kAuto));
}

HashMap<String, mojom::blink::ManifestTranslationItemPtr> ManifestParser::ParseTranslations(const JSONObject* object)
{
    ...
    for (wtf_size_t i = 0; i < translations_map->size(); ++i) {
        ...
        std::optional<String> name = ParseStringForMember(translation, "translations", "name", false, Trim(true));
        translation_item->name = name.has_value() && name->length() != 0 ? *name : String();

        std::optional<String> short_name = ParseStringForMember(translation, "translations", "short_name", false, Trim(true));
        translation_item->short_name = short_name.has_value() && short_name->length() != 0 ? *short_name : String();

        std::optional<String> description = ParseStringForMember(translation, "translations", "description", false, Trim(true));
        translation_item->description = description.has_value() && description->length() != 0 ? *description : String();
        ...
        if (!translation_item->name && !translation_item->short_name && !translation_item->description) {
            continue;
        }
...
mojom::blink::ManifestTabStripPtr ManifestParser::ParseTabStrip(const JSONObject* object)
{
    ...
    if (home_tab_value && home_tab_value->GetType() == JSONValue::kTypeObject) {
        ...
        String string_value;
        if (home_tab_icons && !(home_tab_icons->AsString(&string_value) && EqualIgnoringASCIICase(string_value, "auto"))) {
            home_tab_params->icons = ParseIcons(home_tab_object);
        }

        home_tab_params->scope_patterns = ParseScopePatterns(home_tab_object);

        result->home_tab = mojom::blink::HomeTabUnion::NewParams(std::move(home_tab_params));
    } else {
        result->home_tab = mojom::blink::HomeTabUnion::NewVisibility(ParseTabStripMemberVisibility(home_tab_value));
    }

    auto new_tab_button_params = mojom::blink::NewTabButtonParams::New();

    JSONObject* new_tab_button_object = tab_strip_object->GetJSONObject("new_tab_button");
    if (new_tab_button_object) {
        JSONValue* new_tab_button_url = new_tab_button_object->Get("url");

        String string_value;
        if (new_tab_button_url && !(new_tab_button_url->AsString(&string_value) && EqualIgnoringASCIICase(string_value, "auto"))) {
            KURL url = ParseURL(new_tab_button_object, "url", manifest_url_, ParseURLRestrictions::kWithinScope);
            if (!url.IsNull()) {
...
mojom::blink::TabStripMemberVisibility ManifestParser::ParseTabStripMemberVisibility(const JSONValue* json_value)
{
    if (!json_value) {
    ...
    String string_value;
    if (json_value->AsString(&string_value) && EqualIgnoringASCIICase(string_value, "absent")) {
...
Vector<SafeUrlPattern> ManifestParser::ParseScopePatterns(const JSONObject* object)
{
    ...
    for (wtf_size_t i = 0; i < scope_patterns_list->size(); ++i) {
        ...
        if (init.has_value()) {
            ...
            std::optional<SafeUrlPattern> pattern = ParseScopePattern(init.value(), base_url);
...
std::optional<SafeUrlPattern> ManifestParser::ParseScopePattern(const PatternInit& init, const KURL& base_url)
{
...

    {
        ...
        std::optional<std::vector<liburlpattern::Part>> part_list = ParsePatternInitField(init.protocol, base_url.Protocol());
        if (!part_list.has_value()) {
            AddErrorInfo("property 'protocol in home tab scope pattern could not be parsed or "
...

    {
        ...
        std::optional<std::vector<liburlpattern::Part>> part_list = ParsePatternInitField(init.username, default_username);
        if (!part_list.has_value()) {
            AddErrorInfo("property 'username'in home tab scope pattern could not be parsed or "
                         "contains banned regex.");
            return std::nullopt;
        }
...

    {
        ...
        std::optional<std::vector<liburlpattern::Part>> part_list = ParsePatternInitField(init.password, default_password);
        if (!part_list.has_value()) {
            AddErrorInfo("property 'password' in home tab scope pattern could not be parsed "
...

    {
        ...
        std::optional<std::vector<liburlpattern::Part>> part_list = ParsePatternInitField(init.hostname, default_hostname);
        if (!part_list.has_value()) {
            AddErrorInfo("property 'hostname' in home tab scope pattern could not be parsed "
                         "or contains banned regex.");
            return std::nullopt;
        }
...

    {
        ...
        std::optional<std::vector<liburlpattern::Part>> part_list = ParsePatternInitField(init.port, default_port);
        if (!part_list.has_value()) {
            AddErrorInfo("property 'port'in home tab scope pattern could not be parsed or "
                         "contains banned regex.");
            return std::nullopt;
        }
...

    {
        ...
        if (init.pathname.has_value()) {
            ...
            default_path = ResolveRelativePathnamePattern(base_url, init.pathname.value());
        ...
        std::optional<std::vector<liburlpattern::Part>> part_list = ParsePatternInitField(std::nullopt, default_path);
        if (!part_list.has_value()) {
            AddErrorInfo("property 'pathname'in home tab scope pattern could not be parsed or "
                         "contains banned regex.");
            return std::nullopt;
        }
...

    {
        ...
        std::optional<std::vector<liburlpattern::Part>> part_list = ParsePatternInitField(init.search, default_search);
        if (!part_list.has_value()) {
            AddErrorInfo("property 'search' in home tab scope pattern could not be parsed "
                         "or contains banned regex.");
            return std::nullopt;
        }
...

    {
        ...
        String default_hash;
        if (!init.IsAbsolute() && !init.pathname.has_value() && !init.search.has_value()) {
            default_hash = base_url.FragmentIdentifier().ToString();
        }
        std::optional<std::vector<liburlpattern::Part>> part_list = ParsePatternInitField(init.hash, default_hash);
        if (!part_list.has_value()) {
            AddErrorInfo("property 'hash' in home tab scope pattern could not be parsed "
                         "or contains banned regex.");
            return std::nullopt;
        }
...
std::optional<ManifestParser::PatternInit> ManifestParser::MaybeCreatePatternInit(const JSONObject* pattern_object)
{
    std::optional<String> protocol = ParseStringForMember(pattern_object, "scope_patterns", "protocol", false, Trim(true));
    std::optional<String> username = ParseStringForMember(pattern_object, "scope_patterns", "username", false, Trim(true));
    std::optional<String> password = ParseStringForMember(pattern_object, "scope_patterns", "password", false, Trim(true));
    std::optional<String> hostname = ParseStringForMember(pattern_object, "scope_patterns", "hostname", false, Trim(true));
    std::optional<String> port = ParseStringForMember(pattern_object, "scope_patterns", "port", false, Trim(true));
    std::optional<String> pathname = ParseStringForMember(pattern_object, "scope_patterns", "pathname", false, Trim(true));
    std::optional<String> search = ParseStringForMember(pattern_object, "scope_patterns", "search", false, Trim(true));
    std::optional<String> hash = ParseStringForMember(pattern_object, "scope_patterns", "hash", false, Trim(true));

    KURL base_url;

    if (pattern_object->Get("baseURL")) {
        base_url = ParseURL(pattern_object, "baseURL", KURL(), ParseURLRestrictions::kNoRestrictions);
        if (!base_url.IsValid()) {
            return std::nullopt;
        }
    }

    return std::make_optional<PatternInit>(std::move(protocol), std::move(username), std::move(password), std::move(hostname), std::move(port),
        std::move(pathname), std::move(search), std::move(hash), base_url);
}
...
void ManifestParser::AddErrorInfo(const String& error_msg, bool critical, int error_line, int error_column)
{
    mojom::blink::ManifestErrorPtr error = mojom::blink::ManifestError::New(error_msg, critical, error_line, error_column);
