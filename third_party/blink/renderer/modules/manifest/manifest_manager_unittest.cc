...
public:
    MockManifestChangeNotifier(LocalDOMWindow& window)
        : ManifestChangeNotifier(window)
        , manifest_change_count_(0)
    {
    }
    ...
    {
        ++manifest_change_count_;
    }

    int ManifestChangeCount()
    {
        return manifest_change_count_;
    }
...
TEST_F(ManifestManagerTest, ManifestURL)
{
    ...
    auto* link_manifest = MakeGarbageCollected<HTMLLinkElement>(GetDocument(), CreateElementFlags());
    link_manifest->setAttribute(blink::html_names::kRelAttr, AtomicString("manifest"));
    ...
    link_manifest->setAttribute(html_names::kHrefAttr, AtomicString("http://example.com/manifest.json"));
    ASSERT_EQ(link_manifest->Href(), GetManifestManager()->ManifestURL());

    // Set to some relative url.
    link_manifest->setAttribute(html_names::kHrefAttr, AtomicString("static/manifest.json"));
    ASSERT_EQ(link_manifest->Href(), GetManifestManager()->ManifestURL());
}
...
TEST_F(ManifestManagerTest, ManifestUseCredentials)
{
    ...
    link_manifest->setAttribute(html_names::kCrossoriginAttr, AtomicString("foobar"));
    ASSERT_FALSE(GetManifestManager()->ManifestUseCredentials());

    // Crossorigin set to 'anonymous' shouldn't trigger using credentials.
    link_manifest->setAttribute(html_names::kCrossoriginAttr, AtomicString("anonymous"));
    ASSERT_FALSE(GetManifestManager()->ManifestUseCredentials());

    // Crossorigin set to 'use-credentials' should trigger using credentials.
    link_manifest->setAttribute(html_names::kCrossoriginAttr, AtomicString("use-credentials"));
    ASSERT_TRUE(GetManifestManager()->ManifestUseCredentials());
}

class OverrideManifestChangeNotifierClient : public frame_test_helpers::TestWebFrameClient {
public:
    void DidCreateDocumentElement() override
    {
        if (!frame_)
            return;
        notifier_ = MakeGarbageCollected<MockManifestChangeNotifier>(*frame_->DomWindow());
        ManifestManager::From(*frame_->DomWindow())->SetManifestChangeNotifierForTest(notifier_);
    }

    void SetFrame(LocalFrame* frame)
    {
        frame_ = frame;
    }
    MockManifestChangeNotifier* GetNotifier()
    {
