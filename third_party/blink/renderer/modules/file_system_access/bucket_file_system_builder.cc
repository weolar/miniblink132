...
void BucketFileSystemBuilder::BuildDirectoryTree(
    ExecutionContext* execution_context, const String storage_key, const String name, DirectoryCallback callback, FileSystemDirectoryHandle* handle)
{
    CHECK(handle);
    ...
    BucketFileSystemBuilder* builder = MakeGarbageCollected<BucketFileSystemBuilder>(execution_context, storage_key, name, std::move(callback));

    handle->MojoHandle()->GetEntries(builder->GetListener());
}

BucketFileSystemBuilder::BucketFileSystemBuilder(
    ExecutionContext* execution_context, const String storage_key, const String name, DirectoryCallback completion_callback)
    ...
    , receiver_(this, execution_context)
{
    ...
    nested_files_ = std::make_unique<protocol::Array<protocol::FileSystem::File>>();
}
    ...
    mojom::blink::FileSystemAccessErrorPtr result, Vector<mojom::blink::FileSystemAccessEntryPtr> entries, bool has_more_entries)
{
    ...
    if (!execution_context || !result || result->status != mojom::blink::FileSystemAccessStatus::kOk) {
    ...
    auto barrier_callback
        = base::BarrierClosure(file_system_handle_queue_.size(), WTF::BindOnce(&BucketFileSystemBuilder::DidBuildDirectory, WrapWeakPersistent(this)));
    ...
    for (auto entry : file_system_handle_queue_) {
        if (entry->isFile()) {
            ...
            To<FileSystemFileHandle>(*entry).MojoHandle()->AsBlob(WTF::BindOnce(
                [](String name, base::OnceCallback<void(mojom::blink::FileSystemAccessErrorPtr, std::unique_ptr<protocol::FileSystem::File>)> callback,
                    mojom::blink::FileSystemAccessErrorPtr result, const base::File::Info& info, const scoped_refptr<BlobDataHandle>& blob) {
                    if (!result || result->status != mojom::blink::FileSystemAccessStatus::kOk) {
                        std::move(callback).Run(std::move(result),
                            /*std::unique_ptr<protocol::FileSystem::File>*/ nullptr);
                        return;
                    }

                    std::unique_ptr<protocol::FileSystem::File> file = protocol::FileSystem::File::create()
                                                                           .setName(name)
                                                                           .setLastModified(info.last_modified.InSecondsFSinceUnixEpoch())
                                                                           .setSize(info.size)
                                                                           .setType(blob->GetType())
                                                                           .build();
                    std::move(callback).Run(std::move(result), std::move(file));
                },
                entry->name(), WTF::BindOnce(&BucketFileSystemBuilder::DidBuildFile, WrapPersistent(this), barrier_callback)));
...
mojo::PendingRemote<mojom::blink::FileSystemAccessDirectoryEntriesListener> BucketFileSystemBuilder::GetListener()
{
    ExecutionContext* execution_context = GetExecutionContext();
    CHECK(execution_context);

    auto remote = receiver_.BindNewPipeAndPassRemote(execution_context->GetTaskRunner(TaskType::kInternalInspector));
    ...
    receiver_.set_disconnect_handler(WTF::BindOnce(&BucketFileSystemBuilder::OnMojoDisconnect, WrapWeakPersistent(this)));
...
void BucketFileSystemBuilder::DidBuildFile(
    base::OnceClosure barrier_callback, mojom::blink::FileSystemAccessErrorPtr result, std::unique_ptr<protocol::FileSystem::File> file)
{
    if (!result || !file) {
        std::move(completion_callback_)
            .Run(mojom::blink::FileSystemAccessError::New(
                     mojom::blink::FileSystemAccessStatus::kInvalidState, base::File::Error::FILE_ERROR_FAILED, "Failed to retrieve blob info"),
                /*std::unique_ptr<protocol::FileSystem::Directory>*/ nullptr);
        return;
    }
...
{
    std::move(completion_callback_)
        .Run(mojom::blink::FileSystemAccessError::New(mojom::blink::FileSystemAccessStatus::kOk, base::File::FILE_OK,
                 /*message=*/""),
