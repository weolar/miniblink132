...
FileSystemAccessCapacityTracker::FileSystemAccessCapacityTracker(ExecutionContext* context,
    mojo::PendingRemote<mojom::blink::FileSystemAccessFileModificationHost> file_modification_host_remote, int64_t file_size,
    base::PassKey<FileSystemAccessRegularFileDelegate>)
    : file_modification_host_(context)
    , file_size_(file_size)
    , file_capacity_(file_size)
{
    file_modification_host_.Bind(std::move(file_modification_host_remote), context->GetTaskRunner(TaskType::kStorage));
    DCHECK(file_modification_host_.is_bound());
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}

void FileSystemAccessCapacityTracker::RequestFileCapacityChange(int64_t required_capacity, base::OnceCallback<void(bool)> callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    static_assert(0 - std::numeric_limits<int64_t>::max() >= std::numeric_limits<int64_t>::min(), "The `capacity_delta` computation below may overflow");
    ...
    file_modification_host_->RequestCapacityChange(capacity_delta,
        WTF::BindOnce(&FileSystemAccessCapacityTracker::DidRequestCapacityChange, WrapPersistent(this), required_capacity, std::move(callback)));
}

bool FileSystemAccessCapacityTracker::RequestFileCapacityChangeSync(int64_t required_capacity)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK_GE(file_capacity_, 0);
    DCHECK_GE(required_capacity, 0);
    ...
    bool call_succeeded = file_modification_host_->RequestCapacityChange(capacity_delta, &granted_capacity);
    DCHECK(call_succeeded) << "Mojo call failed";

    bool capacity_change_successful = base::CheckAdd(file_capacity_, granted_capacity).AssignIfValid(&file_capacity_);
    DCHECK(capacity_change_successful) << "Mojo call returned out-of-bounds capacity";
...
void FileSystemAccessCapacityTracker::OnFileContentsModified(int64_t new_size)
{
    ...
    DCHECK_GE(file_capacity_, file_size_) << "A file's capacity should never be smaller than its size.";
...
void FileSystemAccessCapacityTracker::DidRequestCapacityChange(int64_t required_capacity, base::OnceCallback<void(bool)> callback, int64_t granted_capacity)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
// static
int64_t FileSystemAccessCapacityTracker::GetNextCapacityRequestSize(int64_t required_capacity)
{
    DCHECK_GE(required_capacity, 0);
    ...
    if (required_capacity <= kMaxAllocationDoublingSize) {
        ...
        static_assert(kMaxAllocationDoublingSize <= std::numeric_limits<uint32_t>::max(), "The allocation strategy will overflow.");
        ...
        return 1 << base::bits::Log2Ceiling(static_cast<uint32_t>(required_capacity));
    }
    ...
    if (!base::CheckAdd(required_capacity, kMaxAllocationDoublingSize).AssignIfValid(&numerator_plus_one)) {
        return required_capacity;
    }
    if (!base::CheckDiv(numerator_plus_one - 1, kMaxAllocationDoublingSize).AssignIfValid(&multiplier)) {
        return required_capacity;
    }
    if (!base::CheckMul(kMaxAllocationDoublingSize, multiplier).AssignIfValid(&requested_capacity)) {
        return required_capacity;
    }
