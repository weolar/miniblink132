...
FileSystemObservation::FileSystemObservation(
    ExecutionContext* context, FileSystemObserver* observer, mojo::PendingReceiver<mojom::blink::FileSystemAccessObserver> observation_receiver)
    : observer_(observer)
    , execution_context_(context)
    , receiver_(this, context)
{
    CHECK(execution_context_);
    receiver_.Bind(std::move(observation_receiver), execution_context_->GetTaskRunner(TaskType::kStorage));

    // Add a disconnect handler so we can cleanup appropriately.
    receiver_.set_disconnect_handler(WTF::BindOnce(&FileSystemObservation::OnRemoteDisconnected, WrapWeakPersistent(this)));
}
...
void FileSystemObservation::OnFileChanges(WTF::Vector<mojom::blink::FileSystemAccessChangePtr> mojo_changes)
{
...
{
    FileSystemObservationCollection::From(execution_context_)->RemoveObservation(observer_, this);
}
