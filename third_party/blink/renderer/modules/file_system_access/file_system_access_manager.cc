...
// static
const char FileSystemAccessManager::kSupplementName[] = "FileSystemAccessManager";

// static
FileSystemAccessManager& FileSystemAccessManager::From(ExecutionContext* context)
{
    FileSystemAccessManager* manager = Supplement<ExecutionContext>::From<FileSystemAccessManager>(context);
    if (!manager) {
...
void FileSystemAccessManager::EnsureConnection()
{
    ...
    GetExecutionContext()->GetBrowserInterfaceBroker().GetInterface(remote_.BindNewPipeAndPassReceiver(std::move(task_runner)));
}
