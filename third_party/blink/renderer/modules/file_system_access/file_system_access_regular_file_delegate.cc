...
FileSystemAccessFileDelegate* FileSystemAccessFileDelegate::Create(ExecutionContext* context, mojom::blink::FileSystemAccessRegularFilePtr regular_file)
{
    base::File backing_file = std::move(regular_file->os_file);
    int64_t backing_file_size = regular_file->file_size;
    mojo::PendingRemote<mojom::blink::FileSystemAccessFileModificationHost> file_modification_host_remote = std::move(regular_file->file_modification_host);
    return MakeGarbageCollected<FileSystemAccessRegularFileDelegate>(
        context, std::move(backing_file), backing_file_size, std::move(file_modification_host_remote), base::PassKey<FileSystemAccessFileDelegate>());
}

FileSystemAccessRegularFileDelegate::FileSystemAccessRegularFileDelegate(ExecutionContext* context, base::File backing_file, int64_t backing_file_size,
    mojo::PendingRemote<mojom::blink::FileSystemAccessFileModificationHost> file_modification_host_remote, base::PassKey<FileSystemAccessFileDelegate>)
    : backing_file_(std::move(backing_file))
    , capacity_tracker_(MakeGarbageCollected<FileSystemAccessCapacityTracker>(
          context, std::move(file_modification_host_remote), backing_file_size, base::PassKey<FileSystemAccessRegularFileDelegate>()))
    , task_runner_(context->GetTaskRunner(TaskType::kStorage))
{
}

base::FileErrorOr<int> FileSystemAccessRegularFileDelegate::Read(int64_t offset, base::span<uint8_t> data)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK_GE(offset, 0);

    int size = base::checked_cast<int>(data.size());
    int result = UNSAFE_TODO(backing_file_.Read(offset, reinterpret_cast<char*>(data.data()), size));
    if (result >= 0) {
        return result;
    }
...
base::FileErrorOr<int> FileSystemAccessRegularFileDelegate::Write(int64_t offset, base::span<const uint8_t> data)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK_GE(offset, 0);
    ...
    int result = UNSAFE_TODO(backing_file_.Write(offset, reinterpret_cast<const char*>(data.data()), write_size));
...
base::FileErrorOr<bool> FileSystemAccessRegularFileDelegate::SetLength(int64_t new_length)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
