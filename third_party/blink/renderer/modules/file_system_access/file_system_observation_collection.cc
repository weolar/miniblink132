...
// static
const char FileSystemObservationCollection::kSupplementName[] = "FileSystemObservationCollection";

// static
FileSystemObservationCollection* FileSystemObservationCollection::From(ExecutionContext* context)
{
    DCHECK(context);
    DCHECK(context->IsContextThread());

    FileSystemObservationCollection* data = Supplement<ExecutionContext>::From<FileSystemObservationCollection>(context);
    if (!data) {
...
FileSystemObservationCollection::FileSystemObservationCollection(ExecutionContext& context)
...
void FileSystemObservationCollection::AddObservation(
    FileSystemObserver* observer, mojo::PendingReceiver<mojom::blink::FileSystemAccessObserver> observer_receiver)
{
    if (!observation_map_.Contains(observer)) {
        observation_map_.insert(observer, MakeGarbageCollected<HeapHashSet<Member<FileSystemObservation>>>());
    }
    observation_map_.at(observer)->insert(MakeGarbageCollected<FileSystemObservation>(execution_context_, observer, std::move(observer_receiver)));
}

void FileSystemObservationCollection::RemoveObservation(FileSystemObserver* observer, FileSystemObservation* observation)
{
    if (!observation_map_.Contains(observer)) {
        return;
    }
...
void FileSystemObservationCollection::RemoveObserver(FileSystemObserver* observer)
{
    if (!observation_map_.Contains(observer)) {
        return;
    }
