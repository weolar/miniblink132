...
bool CreateDataPipeForSize(uint64_t data_size, mojo::ScopedDataPipeProducerHandle& producer, mojo::ScopedDataPipeConsumerHandle& consumer)
{
...
void WriteDataToProducer(mojo::ScopedDataPipeProducerHandle producer_handle, scoped_refptr<base::RefCountedData<Vector<uint8_t>>> data)
{
    DCHECK(!IsMainThread()) << "WriteDataToProducer must not be called on the main thread";

    auto data_source = std::make_unique<mojo::StringDataSource>(base::span<const char>(reinterpret_cast<const char*>(data->data.data()), data->data.size()),
    ...
    producer_raw->Write(std::move(data_source),
        WTF::BindOnce([](std::unique_ptr<mojo::DataPipeProducer>, scoped_refptr<base::RefCountedData<Vector<uint8_t>>>, MojoResult) {}, std::move(producer),
            std::move(data)));
}
...
FileSystemAccessFileDelegate* FileSystemAccessFileDelegate::CreateForIncognito(
    ExecutionContext* context, mojo::PendingRemote<mojom::blink::FileSystemAccessFileDelegateHost> incognito_file_remote)
{
    return MakeGarbageCollected<FileSystemAccessIncognitoFileDelegate>(
        context, std::move(incognito_file_remote), base::PassKey<FileSystemAccessFileDelegate>());
}

FileSystemAccessIncognitoFileDelegate::FileSystemAccessIncognitoFileDelegate(ExecutionContext* context,
    mojo::PendingRemote<mojom::blink::FileSystemAccessFileDelegateHost> incognito_file_remote, base::PassKey<FileSystemAccessFileDelegate>)
    : mojo_ptr_(context)
    , write_helper_task_runner_(base::ThreadPool::CreateSequencedTaskRunner({}))
{
    mojo_ptr_.Bind(std::move(incognito_file_remote), context->GetTaskRunner(TaskType::kStorage));
...
base::FileErrorOr<int> FileSystemAccessIncognitoFileDelegate::Read(int64_t offset, base::span<uint8_t> data)
{
...
base::FileErrorOr<int> FileSystemAccessIncognitoFileDelegate::Write(int64_t offset, base::span<const uint8_t> data)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK_GE(offset, 0);
    ...
    auto ref_counted_data = base::MakeRefCounted<base::RefCountedData<Vector<uint8_t>>>();
    ...
    PostCrossThreadTask(*write_helper_task_runner_, FROM_HERE, CrossThreadBindOnce(&WriteDataToProducer, std::move(producer_handle), ref_counted_data));

    base::File::Error file_error;
    int bytes_written;
    mojo_ptr_->Write(offset, std::move(consumer_handle), &file_error, &bytes_written);
...
base::FileErrorOr<bool> FileSystemAccessIncognitoFileDelegate::SetLength(int64_t length)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK_GE(length, 0);
    base::File::Error file_error;
