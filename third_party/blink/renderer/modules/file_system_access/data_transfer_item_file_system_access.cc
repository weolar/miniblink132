...
// static
ScriptPromise<IDLNullable<FileSystemHandle>> DataTransferItemFileSystemAccess::getAsFileSystemHandle(
    ScriptState* script_state, DataTransferItem& data_transfer_item, ExceptionState& exception_state)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLNullable<FileSystemHandle>>>(script_state, exception_state.GetContext());
    ...
    const DataObjectItem& data_object_item = *data_transfer_item.GetDataObjectItem();

    // Since tokens are move-only, we need to create a clone in order
    // to preserve the state of `data_object_item` for future calls.
    mojo::PendingRemote<mojom::blink::FileSystemAccessDataTransferToken> token_remote = data_object_item.CloneFileSystemAccessEntryToken();

    auto* execution_context = ExecutionContext::From(script_state);
    FileSystemAccessManager::From(execution_context)
        ->GetEntryFromDataTransferToken(std::move(token_remote),
            WTF::BindOnce(
                [](ScriptPromiseResolver<IDLNullable<FileSystemHandle>>* resolver, mojom::blink::FileSystemAccessErrorPtr result,
                    ...
                    resolver->Resolve(FileSystemHandle::CreateFromMojoEntry(std::move(entry), ExecutionContext::From(script_state)));
                },
