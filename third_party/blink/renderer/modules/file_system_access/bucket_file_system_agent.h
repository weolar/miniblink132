...
class MODULES_EXPORT BucketFileSystemAgent final : public InspectorBaseAgent<protocol::FileSystem::Metainfo> {
public:
    ...
    static protocol::Response HandleError(mojom::blink::FileSystemAccessErrorPtr error);

    // This method is called by DevTools to retrieve all nested files and
    // directories for a given `file_system_locator`.
    void getDirectory(std::unique_ptr<protocol::FileSystem::BucketFileSystemLocator> file_system_locator,
        std::unique_ptr<protocol::FileSystem::Backend::GetDirectoryCallback> callback) override;

    void Trace(Visitor*) const override;

private:
    void DidGetDirectoryHandle(ExecutionContext* execution_context, const String& storage_key, const String& directory_name,
    ...
    StorageBucket* GetStorageBucket(const String& storage_key, const String& bucket_name);
