...
class MockFileSystemAccessManager : public mojom::blink::FileSystemAccessManager {
public:
    MockFileSystemAccessManager(const BrowserInterfaceBrokerProxy& broker, base::OnceClosure reached_callback)
        : reached_callback_(std::move(reached_callback))
        , broker_(broker)
    {
        broker_->SetBinderForTesting(
            mojom::blink::FileSystemAccessManager::Name_, WTF::BindRepeating(&MockFileSystemAccessManager::BindFileSystemAccessManager, WTF::Unretained(this)));
    }
    explicit MockFileSystemAccessManager(const BrowserInterfaceBrokerProxy& broker)
        : broker_(broker)
    {
        broker_->SetBinderForTesting(
            mojom::blink::FileSystemAccessManager::Name_, WTF::BindRepeating(&MockFileSystemAccessManager::BindFileSystemAccessManager, WTF::Unretained(this)));
    ...
    using ChooseEntriesResponseCallback = base::OnceCallback<void(ChooseEntriesCallback callback)>;
    ...
    void GetSandboxedFileSystemForDevtools(const Vector<String>& directory_path_components, GetSandboxedFileSystemCallback callback) override
    {
    }

    void ChooseEntries(mojom::blink::FilePickerOptionsPtr options, ChooseEntriesCallback callback) override
    {
    ...
    void GetFileHandleFromToken(
        mojo::PendingRemote<mojom::blink::FileSystemAccessTransferToken>, mojo::PendingReceiver<mojom::blink::FileSystemAccessFileHandle>) override
    {
    }

    void GetDirectoryHandleFromToken(
        mojo::PendingRemote<mojom::blink::FileSystemAccessTransferToken>, mojo::PendingReceiver<mojom::blink::FileSystemAccessDirectoryHandle>) override
    {
    }
...
private:
    ...
    {
        receivers_.Add(this, mojo::PendingReceiver<mojom::blink::FileSystemAccessManager>(std::move(handle)));
    }
...
public:
    ...
    void Navigate(const String& destinationUrl)
    {
        ...
        auto navigation_params = WebNavigationParams::CreateWithEmptyHTMLForTesting(url);
        GetDocument().GetFrame()->Loader().CommitNavigation(std::move(navigation_params), /*extra_data=*/nullptr);
...
TEST_F(GlobalFileSystemAccessTest, UserActivationRequiredOtherwiseDenied)
{
    ...
    manager.SetChooseEntriesResponse(WTF::BindOnce([](MockFileSystemAccessManager::ChooseEntriesCallback callback) { FAIL(); }));
    ClassicScript::CreateUnspecifiedScript("window.showOpenFilePicker();")->RunScript(GetFrame().DomWindow());
...
TEST_F(GlobalFileSystemAccessTest, UserActivationChooseEntriesSuccessful)
{
    ...
    MockFileSystemAccessManager manager(frame->GetBrowserInterfaceBroker(), manager_run_loop.QuitClosure());
    manager.SetChooseEntriesResponse(WTF::BindOnce([](MockFileSystemAccessManager::ChooseEntriesCallback callback) {
        auto error = mojom::blink::FileSystemAccessError::New();
        error->status = mojom::blink::FileSystemAccessStatus::kOk;
        error->message = "";

        mojo::PendingRemote<mojom::blink::FileSystemAccessFileHandle> pending_remote;
        std::ignore = pending_remote.InitWithNewPipeAndPassReceiver();
        auto handle = mojom::blink::FileSystemAccessHandle::NewFile(std::move(pending_remote));
        auto entry = mojom::blink::FileSystemAccessEntry::New(std::move(handle), "foo.txt");
...
TEST_F(GlobalFileSystemAccessTest, UserActivationChooseEntriesErrors)
{
    ...
    for (const FileSystemAccessStatus& status : statuses) {
        ...
        manager.SetChooseEntriesResponse(WTF::BindOnce(
            [](mojom::blink::FileSystemAccessStatus status, MockFileSystemAccessManager::ChooseEntriesCallback callback) {
                auto error = mojom::blink::FileSystemAccessError::New();
                error->status = status;
                error->message = "";
                Vector<mojom::blink::FileSystemAccessEntryPtr> entries;
