...
BASE_FEATURE(kBlinkEnableInnerTextAgent, "BlinkEnableInnerTextAgent", base::FEATURE_ENABLED_BY_DEFAULT);

// Serves as a kill switch.
BASE_FEATURE(kBlinkEnableInnerHtmlAgent, "BlinkEnableInnerHtmlAgent", base::FEATURE_ENABLED_BY_DEFAULT);
...
void ModulesInitializer::InitLocalFrame(LocalFrame& frame) const
{
    ...
    if (base::FeatureList::IsEnabled(kBlinkEnableInnerTextAgent)) {
        frame.GetInterfaceRegistry()->AddInterface(WTF::BindRepeating(&InnerTextAgent::BindReceiver, WrapWeakPersistent(&frame)));
    }

    if (base::FeatureList::IsEnabled(kBlinkEnableInnerHtmlAgent)) {
        frame.GetInterfaceRegistry()->AddInterface(WTF::BindRepeating(&InnerHtmlAgent::BindReceiver, WrapWeakPersistent(&frame)));
    }
}
    ...
    const WebMediaPlayerSource& source, WebMediaPlayerClient* media_player_client) const
{
    ...
    return web_frame_client->CreateMediaPlayer(source, media_player_client, context_impl, &encrypted_media, encrypted_media.ContentDecryptionModule(), sink_id,
        frame_widget->GetLayerTreeSettings(), base::ThreadPool::CreateTaskRunner(base::TaskTraits {}));
}
...
void ModulesInitializer::DidUpdateScreens(LocalFrame& frame, const display::ScreenInfos& screen_infos)
{
    ...
    if (auto* supplement = Supplement<LocalDOMWindow>::From<WindowScreenDetails>(window)) {
...
void ModulesInitializer::RegisterInterfaces(mojo::BinderMap& binders)
{
    ...
    binders.Add<mojom::blink::WebDatabase>(
        ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&WebDatabaseImpl::Bind)), Platform::Current()->GetIOTaskRunner());
}
