...
public:
    BlobBytesStreamer(Vector<scoped_refptr<RawData>> data, mojo::ScopedDataPipeProducerHandle pipe)
        : data_(std::move(data))
        , pipe_(std::move(pipe))
        , watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::AUTOMATIC)
    {
        watcher_.Watch(pipe_.get(), MOJO_HANDLE_SIGNAL_WRITABLE, MOJO_WATCH_CONDITION_SATISFIED,
            WTF::BindRepeating(&BlobBytesStreamer::OnWritable, WTF::Unretained(this)));
    }
    ...
    void OnWritable(MojoResult result, const mojo::HandleSignalsState& state)
    {
        ...
        while (true) {
            scoped_refptr<RawData> data_current_item = data_[current_item_];
            base::span<const uint8_t> bytes
                = base::span<const uint8_t>((const uint8_t*)data_current_item->data(), data_current_item->size()).subspan(current_item_offset_);
            size_t actually_written_bytes = 0;
            MojoResult write_result = pipe_->WriteData(bytes, MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
...
private:
    ...
    mojo::ScopedDataPipeProducerHandle pipe_ GUARDED_BY_CONTEXT(sequence_checker_);
...
void BlobBytesProvider::AppendData(base::span<const char> data)
{
    ...
    if (data_.empty() || data_.back()->size() + data.size() > kMaxConsolidatedItemSizeInBytes) {
...
// static
void BlobBytesProvider::Bind(std::unique_ptr<BlobBytesProvider> provider, mojo::PendingReceiver<mojom::blink::BytesProvider> receiver)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(provider->sequence_checker_);
    DETACH_FROM_SEQUENCE(provider->sequence_checker_);

    auto task_runner = base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::TaskPriority::USER_VISIBLE });
    ...
    PostCrossThreadTask(*task_runner, FROM_HERE,
        CrossThreadBindOnce(
            [](std::unique_ptr<BlobBytesProvider> provider, mojo::PendingReceiver<mojom::blink::BytesProvider> receiver) {
                DCHECK_CALLED_ON_VALID_SEQUENCE(provider->sequence_checker_);
...
void BlobBytesProvider::RequestAsReply(RequestAsReplyCallback callback)
{
    ...
    for (const auto& d : data_)
        result.AppendSpan(base::span(d->data(), d->size()));
    std::move(callback).Run(result);
}

void BlobBytesProvider::RequestAsStream(mojo::ScopedDataPipeProducerHandle pipe)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void BlobBytesProvider::RequestAsFile(uint64_t source_offset, uint64_t source_size, base::File file, uint64_t file_offset, RequestAsFileCallback callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    int64_t seek_distance = file.Seek(base::File::FROM_BEGIN, base::checked_cast<int64_t>(file_offset));
    ...
    wtf_size_t data_index = static_cast<wtf_size_t>(std::upper_bound(offsets_.begin(), offsets_.end(), source_offset) - offsets_.begin());
    ...
    for (; data_index < data_.size(); ++data_index) {
        ...
        uint64_t data_size = std::min(data->size() - data_offset, source_offset + source_size - offset - data_offset);
        //auto partial_data = base::as_byte_span(*data).subspan(
        base::span<const uint8_t> partial_data = base::span<const uint8_t>((const uint8_t*)data->data(), data->size())
                                                     .subspan(base::checked_cast<size_t>(data_offset), base::checked_cast<size_t>(data_size));
        while (!partial_data.empty()) {
            std::optional<size_t> actual_written = file.WriteAtCurrentPos(partial_data);
