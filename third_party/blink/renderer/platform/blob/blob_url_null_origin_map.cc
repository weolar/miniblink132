...
SecurityOrigin* BlobURLNullOriginMap::Get(const KURL& blob_url)
{
    ...
    auto it = blob_url_null_origin_map_.find(blob_url_without_fragment.GetString());
...
void BlobURLOpaqueOriginNonceMap::Add(const KURL& blob_url, SecurityOrigin* origin)
{
    ...
    auto result = blob_url_opaque_origin_nonce_map_.insert(blob_url.GetString(), *origin->GetNonceForSerialization());
...
base::UnguessableToken BlobURLOpaqueOriginNonceMap::Get(const KURL& blob_url)
{
    ...
    KURL blob_url_without_fragment = blob_url;
    blob_url_without_fragment.RemoveFragmentIdentifier();
    auto it = blob_url_opaque_origin_nonce_map_.find(blob_url_without_fragment.GetString());
    return it != blob_url_opaque_origin_nonce_map_.end() ? it->value : base::UnguessableToken();
}
