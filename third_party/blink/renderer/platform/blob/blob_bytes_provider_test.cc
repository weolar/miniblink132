...
public:
    ...
    std::unique_ptr<BlobBytesProvider> CreateProvider(scoped_refptr<RawData> data = nullptr)
    {
...
TEST_F(BlobBytesProviderTest, Consolidation)
{
    ...
    auto large_data = base::HeapArray<char>::WithSize(BlobBytesProvider::kMaxConsolidatedItemSizeInBytes);
    data->AppendData(large_data);

    EXPECT_EQ(2u, data->data_.size());
    EXPECT_EQ(12u, data->data_[0]->size());
    EXPECT_EQ(BlobBytesProvider::kMaxConsolidatedItemSizeInBytes, data->data_[1]->size());
}
...
TEST_F(BlobBytesProviderTest, RequestAsReply)
{
    ...
    provider->RequestAsReply(base::BindOnce([](Vector<uint8_t>* bytes_out, const Vector<uint8_t>& bytes) { *bytes_out = bytes; }, &received_bytes));
...
class RequestAsFile : public BlobBytesProviderTest, public testing::WithParamInterface<FileTestData> {
public:
    void SetUp() override
    {
        ...
        auto combined_bytes_span = base::span(combined_bytes_).subspan(GetParam().offset, GetParam().size);
        sliced_data_.AppendRange(combined_bytes_span.begin(), combined_bytes_span.end());
    }

    base::File DoRequestAsFile(uint64_t source_offset, uint64_t source_length, uint64_t file_offset)
    {
        base::FilePath path;
        base::CreateTemporaryFile(&path);
        std::optional<base::Time> received_modified;
        test_provider_->RequestAsFile(source_offset, source_length, base::File(path, base::File::FLAG_OPEN | base::File::FLAG_WRITE), file_offset,
            base::BindOnce(
                [](std::optional<base::Time>* received_modified, std::optional<base::Time> modified) { *received_modified = modified; }, &received_modified));
        base::File file(path, base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_DELETE_ON_CLOSE);
        base::File::Info info;
...
TEST_P(RequestAsFile, OffsetInNonEmptyFile)
{
    ...
    test_provider_->RequestAsFile(test.offset, test.size, base::File(path, base::File::FLAG_OPEN | base::File::FLAG_WRITE), file_offset,
        base::BindOnce([](std::optional<base::Time> last_modified) { EXPECT_TRUE(last_modified); }));
...
INSTANTIATE_TEST_SUITE_P(BlobBytesProviderTest, RequestAsFile, testing::ValuesIn(file_tests));
...
TEST_F(BlobBytesProviderTest, RequestAsFile_MultipleChunks)
{
    ...
    for (size_t i = 0; i < combined_bytes_.size(); i += 16) {
        provider->RequestAsFile(i, 16, base::File(path, base::File::FLAG_OPEN | base::File::FLAG_WRITE), combined_bytes_.size() - i - 16,
            base::BindOnce([](std::optional<base::Time> last_modified) { EXPECT_TRUE(last_modified); }));
        auto combined_bytes_chunk = base::span(combined_bytes_).subspan(i, 16);
        expected_data.insert(0, combined_bytes_chunk.data(), combined_bytes_chunk.size());
    }
...
TEST_F(BlobBytesProviderTest, RequestAsFile_InvaldFile)
{
    ...
    provider->RequestAsFile(0, 16, base::File(), 0, base::BindOnce([](std::optional<base::Time> last_modified) { EXPECT_FALSE(last_modified); }));
}
...
TEST_F(BlobBytesProviderTest, RequestAsFile_UnwritableFile)
{
    ...
    base::FilePath path;
    base::CreateTemporaryFile(&path);
    provider->RequestAsFile(0, 16, base::File(path, base::File::FLAG_OPEN | base::File::FLAG_READ), 0,
        base::BindOnce([](std::optional<base::Time> last_modified) { EXPECT_FALSE(last_modified); }));
...
TEST_F(BlobBytesProviderTest, RequestAsStream)
{
    ...
    ASSERT_EQ(mojo::CreateDataPipe(7, producer_handle, consumer_handle), MOJO_RESULT_OK);
    provider->RequestAsStream(std::move(producer_handle));

    Vector<uint8_t> received_data;
    base::RunLoop loop;
    mojo::SimpleWatcher watcher(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::AUTOMATIC, blink::scheduler::GetSequencedTaskRunnerForTesting());
    watcher.Watch(consumer_handle.get(), MOJO_HANDLE_SIGNAL_READABLE, MOJO_WATCH_CONDITION_SATISFIED,
        base::BindRepeating(
            [](mojo::DataPipeConsumerHandle pipe, base::RepeatingClosure quit_closure, Vector<uint8_t>* bytes_out, MojoResult result,
                const mojo::HandleSignalsState& state) {
                if (result == MOJO_RESULT_CANCELLED || result == MOJO_RESULT_FAILED_PRECONDITION) {
                    quit_closure.Run();
                    return;
                }

                size_t num_bytes = 0;
                MojoResult query_result = pipe.ReadData(MOJO_READ_DATA_FLAG_QUERY, base::span<uint8_t>(), num_bytes);
                if (query_result == MOJO_RESULT_SHOULD_WAIT)
                    return;
                EXPECT_EQ(MOJO_RESULT_OK, query_result);

                Vector<uint8_t> bytes(num_bytes);
                EXPECT_EQ(MOJO_RESULT_OK, pipe.ReadData(MOJO_READ_DATA_FLAG_ALL_OR_NONE, base::as_writable_byte_span(bytes), num_bytes));
