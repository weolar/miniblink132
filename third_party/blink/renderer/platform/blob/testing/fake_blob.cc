...
public:
    explicit SimpleDataPipeGetter(const Vector<uint8_t>& bytes)
        : bytes_(bytes)
    {
    }
    ...
    {
        mojo::MakeSelfOwnedReceiver(std::make_unique<SimpleDataPipeGetter>(bytes_), std::move(receiver));
    }
...
FakeBlob::FakeBlob(const String& uuid, const Vector<uint8_t>& body_bytes, State* state)
    : uuid_(uuid)
    , body_(body_bytes)
    , state_(state)
{
...
{
    mojo::MakeSelfOwnedReceiver(std::make_unique<FakeBlob>(uuid_, body_, state_), std::move(receiver));
}

void FakeBlob::AsDataPipeGetter(mojo::PendingReceiver<network::mojom::blink::DataPipeGetter> receiver)
{
    if (state_)
        state_->did_initiate_read_operation = true;
    mojo::MakeSelfOwnedReceiver(std::make_unique<SimpleDataPipeGetter>(body_), std::move(receiver));
}

void FakeBlob::ReadRange(uint64_t offset, uint64_t length, mojo::ScopedDataPipeProducerHandle, mojo::PendingRemote<mojom::blink::BlobReaderClient>)
{
    NOTREACHED();
}

void FakeBlob::ReadAll(mojo::ScopedDataPipeProducerHandle handle, mojo::PendingRemote<mojom::blink::BlobReaderClient> client)
{
...
void FakeBlob::Load(mojo::PendingReceiver<network::mojom::blink::URLLoader>, const String& method, const net::HttpRequestHeaders&,
