...
public:
    ...
    FakeBlob(const String& uuid, const String& body = String(), State* state = nullptr);
    FakeBlob(const String& uuid, const Vector<uint8_t>& body_bytes, State* state = nullptr);

    void Clone(mojo::PendingReceiver<mojom::blink::Blob>) override;
    void AsDataPipeGetter(mojo::PendingReceiver<network::mojom::blink::DataPipeGetter>) override;
    void ReadRange(uint64_t offset, uint64_t length, mojo::ScopedDataPipeProducerHandle, mojo::PendingRemote<mojom::blink::BlobReaderClient>) override;
    void ReadAll(mojo::ScopedDataPipeProducerHandle, mojo::PendingRemote<mojom::blink::BlobReaderClient>) override;
    void Load(mojo::PendingReceiver<network::mojom::blink::URLLoader>, const String& method, const net::HttpRequestHeaders&,
