...
struct ExpectedElement {
    ...
    static ExpectedElement EmbeddedBytes(Vector<uint8_t> embedded_data)
    {
        ...
        return ExpectedElement { DataElement::NewBytes(DataElementBytes::New(size, std::move(embedded_data), mojo::NullRemote())) };
    }

    static ExpectedElement LargeBytes(Vector<uint8_t> data)
    {
        uint64_t size = data.size();
        return ExpectedElement { DataElement::NewBytes(DataElementBytes::New(size, std::nullopt, mojo::NullRemote())), String(), std::move(data) };
    }

    static ExpectedElement File(const String& path, uint64_t offset, uint64_t length, base::Time time)
    {
        return ExpectedElement { DataElement::NewFile(DataElementFile::New(WebStringToFilePath(path), offset, length, time)) };
    }

    static ExpectedElement Blob(const String& uuid, uint64_t offset, uint64_t length)
    {
        return ExpectedElement { DataElement::NewBlob(DataElementBlob::New(mojo::NullRemote(), offset, length)), uuid };
    }
};
...
public:
    BlobDataHandleTest()
        : blob_registry_receiver_(&mock_blob_registry_, blob_registry_remote_.BindNewPipeAndPassReceiver())
    {
    ...
    void SetUp() override
    {
        ...
        ASSERT_LT(small_test_data_.size(), BlobBytesProvider::kMaxConsolidatedItemSizeInBytes);
        ASSERT_LT(medium_test_data_.size(), DataElementBytes::kMaximumEmbeddedDataSize);
        ASSERT_GT(medium_test_data_.size(), BlobBytesProvider::kMaxConsolidatedItemSizeInBytes);
        ASSERT_GT(large_test_data_.size(), DataElementBytes::kMaximumEmbeddedDataSize);

        empty_blob_ = BlobDataHandle::Create();

        auto test_data = std::make_unique<BlobData>();
        test_data->AppendBytes(large_test_data_);
        test_blob_ = BlobDataHandle::Create(std::move(test_data), large_test_data_.size());
    ...
    void TestCreateBlob(std::unique_ptr<BlobData> data, Vector<ExpectedElement> expected_elements)
    {
        uint64_t blob_size = data->length();
        String type = data->ContentType();
        bool is_single_unknown_size_file = data->IsSingleUnknownSizeFile();

        scoped_refptr<BlobDataHandle> handle = BlobDataHandle::Create(std::move(data), blob_size);
        ...
        for (wtf_size_t i = 0; i < expected_elements.size(); ++i) {
            ...
            if (expected->is_bytes()) {
                ...
                EXPECT_EQ(expected->get_bytes()->embedded_data, actual->get_bytes()->embedded_data);

                base::RunLoop loop;
                Vector<uint8_t> received_bytes;
                mojo::Remote<mojom::blink::BytesProvider> actual_data(std::move(actual->get_bytes()->data));
                actual_data->RequestAsReply(WTF::BindOnce(
                    [](base::RepeatingClosure quit_closure, Vector<uint8_t>* bytes_out, const Vector<uint8_t>& bytes) {
                        *bytes_out = bytes;
                        quit_closure.Run();
                    },
                ...
                EXPECT_EQ(expected->get_file()->expected_modification_time, actual->get_file()->expected_modification_time);
                ...
                mojo::Remote<mojom::blink::Blob> blob(std::move(actual->get_blob()->blob));
                blob->GetInternalUUID(base::BindOnce(
                    [](base::RepeatingClosure quit_closure, String* uuid_out, const String& uuid) {
                        *uuid_out = uuid;
                        quit_closure.Run();
                    },
...
TEST_F(BlobDataHandleTest, CreateFromFile)
{
    ...
    mojo::Receiver<FileBackedBlobFactory> file_factory_receiver(&file_factory, file_factory_remote.BindNewPipeAndPassReceiver());

    scoped_refptr<BlobDataHandle> handle = BlobDataHandle::CreateForFile(file_factory_remote.get(), kPath, kOffset, kSize, kModificationTime, kType);
...
TEST_F(BlobDataHandleTest, CreateFromMergedBytes)
{
    ...
    Vector<ExpectedElement> expected_elements;
    expected_elements.push_back(ExpectedElement::EmbeddedBytes(std::move(expected_data)));
...
TEST_F(BlobDataHandleTest, CreateFromMergedLargeAndSmallBytes)
{
    ...
    Vector<ExpectedElement> expected_elements;
    expected_elements.push_back(ExpectedElement::LargeBytes(std::move(expected_data)));
...
TEST_F(BlobDataHandleTest, CreateFromManyMergedBytes)
{
    ...
    Vector<ExpectedElement> expected_elements;
    expected_elements.push_back(ExpectedElement::LargeBytes(std::move(merged_data)));
    expected_elements.push_back(ExpectedElement::Blob(test_blob_uuid_, 0, 10));
    expected_elements.push_back(ExpectedElement::EmbeddedBytes(medium_test_data_));
