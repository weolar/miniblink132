...
template <> struct PLATFORM_EXPORT StructTraits<blink::mojom::SerializedBlobDataView, scoped_refptr<blink::BlobDataHandle>> {
    ...
    static WTF::String content_type(const scoped_refptr<blink::BlobDataHandle>& input)
    {
    ...
    static mojo::PendingRemote<blink::mojom::blink::Blob> blob(const scoped_refptr<blink::BlobDataHandle>& input)
    {
        return input->CloneBlobRemote();
    }

    static bool Read(blink::mojom::SerializedBlobDataView, scoped_refptr<blink::BlobDataHandle>* out);
};
