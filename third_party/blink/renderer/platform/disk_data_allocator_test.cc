...
public:
    explicit DiskDataAllocatorTest(ThreadPoolExecutionMode thread_pool_execution_mode = ThreadPoolExecutionMode::DEFAULT)
        : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME, thread_pool_execution_mode)
    {
    }

    static std::vector<std::unique_ptr<DiskDataMetadata>> Allocate(InMemoryDataAllocator* allocator, size_t size, size_t count)
    {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            auto metadata = allocator->Write(std::move(reserved_chunk), base::as_byte_span(random_data));
...
TEST_F(DiskDataAllocatorTest, ReadWriteDiscardMultiple)
{
    ...
    std::vector<std::pair<std::unique_ptr<DiskDataMetadata>, std::string>> data_written;
    ...
    for (int i = 0; i < 10; i++) {
        ...
        ASSERT_TRUE(reserved_chunk);
        auto metadata = allocator.Write(std::move(reserved_chunk), base::as_byte_span(data));
        EXPECT_TRUE(metadata);
...
TEST_F(DiskDataAllocatorTest, CanReuseFreedChunk)
{
    ...
    auto reserved_chunk = allocator.TryReserveChunk(random_data.size());
    ASSERT_TRUE(reserved_chunk);
    auto new_metadata = allocator.Write(std::move(reserved_chunk), base::as_byte_span(random_data));
...
TEST_F(DiskDataAllocatorTest, ExactThenWorstFit)
{
    ...
    ASSERT_TRUE(reserved_chunk);
    metadata = allocator.Write(std::move(reserved_chunk), base::as_byte_span(random_data));
    EXPECT_TRUE(metadata);
...
TEST_F(DiskDataAllocatorTest, ProvideValidFile)
{
    ...
    int flags = base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_READ | base::File::FLAG_WRITE | base::File::FLAG_DELETE_ON_CLOSE;
...
TEST_F(DiskDataAllocatorTest, WriteWithLimitedCapacity)
{
    ...
    ASSERT_TRUE(reserved_chunk);
    auto metadata_1 = allocator.Write(std::move(reserved_chunk), base::as_byte_span(random_data_1));
    ...
    ASSERT_TRUE(reserved_chunk);
    auto metadata_2 = allocator.Write(std::move(reserved_chunk), base::as_byte_span(random_data_2));
    ...
    ASSERT_TRUE(reserved_chunk);
    auto metadata_3 = allocator.Write(std::move(reserved_chunk), base::as_byte_span(random_data_3));
