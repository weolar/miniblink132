...
void LinearTimingFunction::Range(double* min_value, double* max_value) const
{
    ...
    const auto [min, max] = std::minmax_element(min_it, max_it, [](const auto& a, const auto& b) { return a.output < b.output; });
...
CubicBezierTimingFunction* CubicBezierTimingFunction::Preset(EaseType ease_type)
{
    DEFINE_STATIC_REF(CubicBezierTimingFunction, ease, (base::AdoptRef(new CubicBezierTimingFunction(EaseType::EASE))));
    DEFINE_STATIC_REF(CubicBezierTimingFunction, ease_in, (base::AdoptRef(new CubicBezierTimingFunction(EaseType::EASE_IN))));
    DEFINE_STATIC_REF(CubicBezierTimingFunction, ease_out, (base::AdoptRef(new CubicBezierTimingFunction(EaseType::EASE_OUT))));
    DEFINE_STATIC_REF(CubicBezierTimingFunction, ease_in_out, (base::AdoptRef(new CubicBezierTimingFunction(EaseType::EASE_IN_OUT))));
...
String CubicBezierTimingFunction::ToString() const
{
    ...
    case CubicBezierTimingFunction::EaseType::CUSTOM:
        return "cubic-bezier(" + String::NumberToStringECMAScript(X1()) + ", " + String::NumberToStringECMAScript(Y1()) + ", "
            + String::NumberToStringECMAScript(X2()) + ", " + String::NumberToStringECMAScript(Y2()) + ")";
    default:
        NOTREACHED();
    }
}
...
scoped_refptr<TimingFunction> CreateCompositorTimingFunctionFromCC(const gfx::TimingFunction* timing_function)
{
    if (!timing_function)
