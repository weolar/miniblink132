...
class PLATFORM_EXPORT TimingFunction : public ThreadSafeRefCounted<TimingFunction> {
public:
...
public:
    ...
    {
        DEFINE_STATIC_REF(LinearTimingFunction, linear, (base::AdoptRef(new LinearTimingFunction())));
        return linear;
    }

    static scoped_refptr<LinearTimingFunction> Create(std::vector<gfx::LinearEasingPoint> points)
    {
        return base::AdoptRef(new LinearTimingFunction(std::move(points)));
    }

    static scoped_refptr<LinearTimingFunction> Create(Vector<gfx::LinearEasingPoint> points)
    {
        std::vector<gfx::LinearEasingPoint> temp_points(points.begin(), points.end());
...
public:
    ...
    static scoped_refptr<CubicBezierTimingFunction> Create(double x1, double y1, double x2, double y2)
    {
...
public:
    ...
    static scoped_refptr<StepsTimingFunction> Create(int steps, StepPosition step_position)
    {
    ...
    StepPosition GetStepPosition() const
    {
        return steps_->step_position();
    }
...
PLATFORM_EXPORT scoped_refptr<TimingFunction> CreateCompositorTimingFunctionFromCC(const gfx::TimingFunction*);
...
template <> struct DowncastTraits<LinearTimingFunction> {
    static bool AllowFrom(const TimingFunction& value)
    {
        return value.GetType() == TimingFunction::Type::LINEAR;
    }
};
template <> struct DowncastTraits<CubicBezierTimingFunction> {
    static bool AllowFrom(const TimingFunction& value)
    {
        return value.GetType() == TimingFunction::Type::CUBIC_BEZIER;
    }
};
template <> struct DowncastTraits<StepsTimingFunction> {
    static bool AllowFrom(const TimingFunction& value)
    {
