...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    for (const bool toggle : { false, true }) {
        ...
        std::optional<blink::StorageKey> maybe_storage_key = blink::StorageKey::Deserialize(serialized_storage_key);
        ...
        std::vector<uint8_t> mojom_serialized = blink::mojom::StorageKey::Serialize(&*maybe_storage_key);
        WTF::Vector<uint8_t> mojom_serialized_as_wtf;
        mojom_serialized_as_wtf.AppendRange(mojom_serialized.begin(), mojom_serialized.end());
        blink::BlinkStorageKey mojom_blink_storage_key;
        assert(blink::mojom::blink::StorageKey::Deserialize(mojom_serialized_as_wtf, &mojom_blink_storage_key));
        WTF::Vector<uint8_t> mojom_blink_serialized = blink::mojom::blink::StorageKey::Serialize(&mojom_blink_storage_key);
        std::vector<uint8_t> mojom_blink_serialized_as_std(mojom_blink_serialized.begin(), mojom_blink_serialized.end());
        blink::StorageKey mojom_storage_key;
        assert(blink::mojom::StorageKey::Deserialize(mojom_blink_serialized_as_std, &mojom_storage_key));
        ...
        assert(mojom_blink_storage_key.ExactMatchForTesting(type_blink_storage_key));
    }
    return 0;
}
