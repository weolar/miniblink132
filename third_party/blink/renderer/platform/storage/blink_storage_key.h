...
public:
    ...
    static BlinkStorageKey CreateFirstParty(scoped_refptr<const SecurityOrigin> origin);
    ...
    static BlinkStorageKey CreateWithNonce(scoped_refptr<const SecurityOrigin> origin, const base::UnguessableToken& nonce);
    ...
    // TODO(crbug.com/1159586): Remove when no longer needed.
    static bool FromWire(scoped_refptr<const SecurityOrigin> origin, const BlinkSchemefulSite& top_level_site,
    ...
    const BlinkSchemefulSite& GetTopLevelSite() const
    {
...
private:
    ...
    BlinkStorageKey(scoped_refptr<const SecurityOrigin> origin, const BlinkSchemefulSite& top_level_site, const base::UnguessableToken* nonce,
        mojom::blink::AncestorChainBit ancestor_chain_bit);

    // (7B) Operators.
    // Note that not all must be friends, but all are to consolidate the header.
    PLATFORM_EXPORT
    friend bool operator==(const BlinkStorageKey& lhs, const BlinkStorageKey& rhs);
    PLATFORM_EXPORT
    friend bool operator!=(const BlinkStorageKey& lhs, const BlinkStorageKey& rhs);
    // If there were a need for an operator< it would go here.
    PLATFORM_EXPORT
    friend std::ostream& operator<<(std::ostream& ostream, const BlinkStorageKey& sk);
    ...
    mojom::blink::AncestorChainBit ancestor_chain_bit_ { mojom::blink::AncestorChainBit::kCrossSite };

    // Stores the value `ancestor_chain_bit_` would have had if
    // `kThirdPartyStoragePartitioning` were enabled. This isn't used in
    // serialization or comparison.
    // TODO(crbug.com/1159586): Remove when no longer needed.
    mojom::blink::AncestorChainBit ancestor_chain_bit_if_third_party_enabled_ = ancestor_chain_bit_;
};
