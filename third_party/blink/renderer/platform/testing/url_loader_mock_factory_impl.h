...
public:
    ...
    void RegisterURL(const WebURL& url, const WebURLResponse& response, const WebString& file_path = WebString()) override;
    void RegisterErrorURL(const WebURL& url, const WebURLResponse& response, const WebURLError& error) override;
    void UnregisterURL(const WebURL& url) override;
    void RegisterURLProtocol(const WebString& protocol, const WebURLResponse& response, const WebString& file_path) override;
    ...
    void LoadSynchronously(std::unique_ptr<network::ResourceRequest> request, WebURLResponse* response, std::optional<WebURLError>* error,
        scoped_refptr<SharedBuffer>& data, int64_t* encoded_data_length);
    void LoadAsynchronouly(std::unique_ptr<network::ResourceRequest> request, URLLoaderMock* loader);
...
private:
    ...
    // accordingly.
    void LoadRequest(const WebURL& url, WebURLResponse* response, std::optional<WebURLError>* error, scoped_refptr<SharedBuffer>& data);
    ...
    bool LookupURL(const WebURL& url, std::optional<WebURLError>* error, ResponseInfo* response_info);

    // Reads 'file_path' and puts its content in 'data'.
    // Returns true if it successfully read the file.
    static bool ReadFile(const base::FilePath& file_path, scoped_refptr<SharedBuffer>& data);

    raw_ptr<URLLoaderTestDelegate> delegate_ = nullptr;

    // The loaders that have not being served data yet.
    using LoaderToRequestMap = HashMap<URLLoaderMock*, std::unique_ptr<network::ResourceRequest>>;
