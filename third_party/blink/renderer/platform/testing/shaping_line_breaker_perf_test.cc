...
{
    perf_test::PerfResultReporter reporter(kMetricPrefixShapingLineBreaker, story);
...
LayoutUnit ShapeText(ShapingLineBreaker* breaker, LayoutUnit available_space, unsigned string_length)
{
    unsigned break_offset = 0;
    LayoutUnit total_width;
    ShapingLineBreaker::Result result;
    while (break_offset < string_length) {
        const ShapeResultView* shape_result = breaker->ShapeLine(break_offset, available_space, &result);
...
TEST_F(ShapingLineBreakerPerfTest, ShapeLatinText)
{
    ...
    String string("\"Am I my brother's keeper?\""
                  "Yes, of a truth!"
    ...
    HarfBuzzShapingLineBreaker reference_breaker(&shaper, &font, reference_result, &break_iterator, nullptr);

    LayoutUnit available_width_px(500);
    LayoutUnit expected_width = ShapeText(&reference_breaker, available_width_px, len);
    ...
    SetUpReporter("latin_text").AddResult(kMetricThroughput, timer_.LapsPerSecond());
}
