...
void PrintTo(const DummyExceptionStateForTesting& exception_state, std::ostream* os);
...
MATCHER_P(HadException, code, internal::ExceptionCodeToString(ToExceptionCode(code)))
{
    return arg.HadException() && arg.Code() == ToExceptionCode(code);
}

MATCHER_P2(HadException, code, message, internal::ExceptionCodeToString(ToExceptionCode(code)) + ": " + testing::PrintToString(message))
{
    return arg.HadException() && arg.Code() == ToExceptionCode(code) && arg.Message() == message;
}
