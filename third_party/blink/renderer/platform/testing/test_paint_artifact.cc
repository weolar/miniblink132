...
{
    DEFINE_STATIC_LOCAL(Persistent<FakeDisplayItemClient>, client, (MakeGarbageCollected<FakeDisplayItemClient>()));
    client->Validate();
    return *client;
}
...
{
    Chunk(StaticDummyClient(), static_cast<DisplayItem::Type>(DisplayItem::kDrawingFirst + id));
...
TestPaintArtifact& TestPaintArtifact::Chunk(const DisplayItemClient& client, DisplayItem::Type type)
{
    auto& display_item_list = paint_artifact_->GetDisplayItemList();
    paint_artifact_->GetPaintChunks().emplace_back(
        display_item_list.size(), display_item_list.size(), client, PaintChunk::Id(client.Id(), type), PropertyTreeState::Root());
    paint_artifact_->RecordDebugInfo(client.Id(), client.DebugName(), client.OwnerNodeId());
    // Assume PaintController has processed this chunk.
    paint_artifact_->GetPaintChunks().back().client_is_just_created = false;
    return *this;
}

TestPaintArtifact& TestPaintArtifact::Properties(const PropertyTreeStateOrAlias& properties)
{
    paint_artifact_->GetPaintChunks().back().properties = properties;
    return *this;
}

TestPaintArtifact& TestPaintArtifact::RectDrawing(const gfx::Rect& bounds, Color color)
{
    return RectDrawing(NewClient(), bounds, color);
}

TestPaintArtifact& TestPaintArtifact::ForeignLayer(scoped_refptr<cc::Layer> layer, const gfx::Point& offset)
{
    DEFINE_STATIC_DISPLAY_ITEM_CLIENT(client, "ForeignLayer");
    paint_artifact_->GetDisplayItemList().AllocateAndConstruct<ForeignLayerDisplayItem>(
        client->Id(), DisplayItem::kForeignLayerFirst, std::move(layer), offset, RasterEffectOutset::kNone, client->GetPaintInvalidationReason());
    paint_artifact_->RecordDebugInfo(client->Id(), client->DebugName(), client->OwnerNodeId());
    DidAddDisplayItem();
    return *this;
}

TestPaintArtifact& TestPaintArtifact::RectDrawing(const DisplayItemClient& client, const gfx::Rect& bounds, Color color)
{
    ...
    paint_artifact_->GetDisplayItemList().AllocateAndConstruct<DrawingDisplayItem>(client.Id(), DisplayItem::kDrawingFirst, bounds,
        recorder.finishRecordingAsPicture(), client.VisualRectOutsetForRasterEffects(), client.GetPaintInvalidationReason());
    paint_artifact_->RecordDebugInfo(client.Id(), client.DebugName(), client.OwnerNodeId());
...
TestPaintArtifact& TestPaintArtifact::ScrollHitTestChunk(const DisplayItemClient& client, const PropertyTreeState& contents_state)
{
    const auto& scroll_translation = contents_state.Transform();
    DCHECK(scroll_translation.ScrollNode());
    Chunk(client, DisplayItem::kScrollHitTest).Properties(*scroll_translation.Parent(), *contents_state.Clip().Parent(), contents_state.Effect());
    auto& chunk = paint_artifact_->GetPaintChunks().back();
    chunk.hit_test_opaqueness = cc::HitTestOpaqueness::kOpaque;
    auto& hit_test_data = chunk.EnsureHitTestData();
    hit_test_data.scroll_hit_test_rect = scroll_translation.ScrollNode()->ContainerRect();
    hit_test_data.scroll_translation = &scroll_translation;
    return *this;
}

TestPaintArtifact& TestPaintArtifact::ScrollingContentsChunk(const DisplayItemClient& client, const PropertyTreeState& state, bool opaque)
{
...
TestPaintArtifact& TestPaintArtifact::ScrollChunks(const PropertyTreeState& contents_state, bool contents_opaque)
{
    return ScrollHitTestChunk(contents_state).ScrollingContentsChunk(contents_state, contents_opaque);
}

TestPaintArtifact& TestPaintArtifact::SetRasterEffectOutset(RasterEffectOutset outset)
{
...
TestPaintArtifact& TestPaintArtifact::TextKnownToBeOnOpaqueBackground()
{
    ...
    paint_artifact_->GetPaintChunks().back().text_known_to_be_on_opaque_background = true;
    return *this;
}
...
TestPaintArtifact& TestPaintArtifact::DrawableBounds(const gfx::Rect& drawable_bounds)
{
    auto& chunk = paint_artifact_->GetPaintChunks().back();
...
{
    paint_artifact_->GetPaintChunks().back().is_moved_from_cached_subsequence = true;
    return *this;
}
