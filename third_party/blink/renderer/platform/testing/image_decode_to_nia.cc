...
void write_nie_pixels(uint32_t width, uint32_t height, blink::ImageFrame* frame)
{
...
void write_nia_footer(int repetition_count, size_t frame_count)
{
    ...
    if (override_num_animation_loops || (repetition_count == blink::kAnimationNone) || (repetition_count == blink::kAnimationLoopInfinite)) {
        set_u32le(data + 0, 0);
    } else {
...
int main(int argc, char* argv[])
{
    ...
    std::unique_ptr<blink::Platform> platform = std::make_unique<blink::Platform>();
    blink::Platform::CreateMainThreadAndInitialize(platform.get());

    base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
    bool first_frame_only = command_line->HasSwitch("1") || command_line->HasSwitch("first-frame-only");
    ...
    std::unique_ptr<blink::ImageDecoder> decoder
        = blink::ImageDecoder::Create(WTF::SharedBuffer::Create(src.data(), src.size()), data_complete, blink::ImageDecoder::kAlphaNotPremultiplied,
            blink::ImageDecoder::kDefaultBitDepth, blink::ColorBehavior::kIgnore, cc::AuxImage::kDefault, blink::Platform::GetMaxDecodedImageBytes());
