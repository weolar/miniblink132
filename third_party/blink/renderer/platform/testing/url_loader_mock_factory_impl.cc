...
URLLoaderMockFactoryImpl::URLLoaderMockFactoryImpl(TestingPlatformSupport* platform)
    : platform_(platform)
{
}
...
void URLLoaderMockFactoryImpl::RegisterURL(const WebURL& url, const WebURLResponse& response, const WebString& file_path)
{
    ...
    if (!file_path.IsNull() && !file_path.IsEmpty()) {
        ...
        DCHECK(base::PathExists(response_info.file_path)) << response_info.file_path.MaybeAsASCII() << " does not exist.";
    }
...
void URLLoaderMockFactoryImpl::RegisterErrorURL(const WebURL& url, const WebURLResponse& response, const WebURLError& error)
{
    DCHECK(!base::Contains(url_to_response_info_, url));
...
void URLLoaderMockFactoryImpl::RegisterURLProtocol(const WebString& protocol, const WebURLResponse& response, const WebString& file_path)
{
...
void URLLoaderMockFactoryImpl::UnregisterURLProtocol(const WebString& protocol)
{
    ProtocolToResponseMap::iterator iter = protocol_to_response_info_.find(protocol);
...
void URLLoaderMockFactoryImpl::ServeAsynchronousRequests()
{
    ...
    while (!pending_loaders_.empty()) {
        ...
        while (response.HttpStatusCode() >= 300 && response.HttpStatusCode() < 400) {
            WebURL new_url = loader->ServeRedirect(WebString::FromLatin1(request->method), response);
            RunUntilIdle();
...
void URLLoaderMockFactoryImpl::FillNavigationParamsResponse(WebNavigationParams* params)
{
    ...
    if (kurl.ProtocolIsData()) {
        ...
        int result;
        std::tie(result, response, buffer) = network_utils::ParseDataURL(kurl, params->http_method);
        DCHECK(buffer);
        DCHECK_EQ(net::OK, result);
        params->response = WrappedResourceResponse(response);
        params->is_static_data = true;
        params->body_loader = StaticDataNavigationBodyLoader::CreateWithData(std::move(buffer));
        return;
    }
    ...
    DCHECK(!error);
    while (params->response.HttpStatusCode() >= 300 && params->response.HttpStatusCode() < 400) {
    ...
    params->is_static_data = true;
    params->body_loader = StaticDataNavigationBodyLoader::CreateWithData(std::move(data));
}
...
void URLLoaderMockFactoryImpl::LoadSynchronously(std::unique_ptr<network::ResourceRequest> request, WebURLResponse* response, std::optional<WebURLError>* error,
    scoped_refptr<SharedBuffer>& data, int64_t* encoded_data_length)
{
    LoadRequest(WebURL(KURL(request->url)), response, error, data);
    *encoded_data_length = data->size();
}

void URLLoaderMockFactoryImpl::LoadAsynchronouly(std::unique_ptr<network::ResourceRequest> request, URLLoaderMock* loader)
{
...
void URLLoaderMockFactoryImpl::LoadRequest(const WebURL& url, WebURLResponse* response, std::optional<WebURLError>* error, scoped_refptr<SharedBuffer>& data)
{
    ResponseInfo response_info;
...
bool URLLoaderMockFactoryImpl::LookupURL(const WebURL& url, std::optional<WebURLError>* error, ResponseInfo* response_info)
{
...
// static
bool URLLoaderMockFactoryImpl::ReadFile(const base::FilePath& file_path, scoped_refptr<SharedBuffer>& data)
{
