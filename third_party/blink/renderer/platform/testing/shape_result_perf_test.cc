...
protected:
    Font CreateFont(FontName font_name)
    {
        ...
        return CreateTestFont(AtomicString("TestFont"), test::PlatformTestDataPath(font_path.find(font_name)->value), 100, &ligatures);
    }

    TextRun CreateRun(const String& text, bool ltr)
    {
        return TextRun(text, ltr ? TextDirection::kLtr : TextDirection::kRtl, false);
    }

    void ReportResult(const std::string& metric_prefix, const std::string& story_prefix)
    {
...
class OffsetForPositionPerfTest : public ShapeResultPerfTest, public testing::TestWithParam<float> {
public:
    void OffsetForPosition(const Font& font, TextRun& run, IncludePartialGlyphsOption partial, BreakGlyphsOption breakopt)
    {
        timer.Reset();
...
protected:
    ...
    {
        ShapeResultPerfTest::ReportResult(kMetricPrefixOffsetForPosition, story_prefix);
    }
};

class CharacterRangePerfTest : public ShapeResultPerfTest, public testing::TestWithParam<int> {
public:
...
protected:
    void ReportResult(const std::string& story_prefix)
    {
        ShapeResultPerfTest::ReportResult(kMetricPrefixCharacterRange, story_prefix);
    }
};
...
INSTANTIATE_TEST_SUITE_P(OffsetForPosition, OffsetForPositionPerfTest, testing::Values(0, 10, 60, 100, 200, 350));
...
INSTANTIATE_TEST_SUITE_P(CharacterRange, CharacterRangePerfTest, testing::Values(0, 1, 2, 4, 8));
