...
public:
    ...
    {
        return Chunk(NewClient());
    }

    // Add a chunk with the specified client.
    TestPaintArtifact& Chunk(const DisplayItemClient&, DisplayItem::Type = DisplayItem::kDrawingFirst);
    ...
    TestPaintArtifact& ScrollHitTestChunk(const DisplayItemClient&, const PropertyTreeState& contents_state);
    TestPaintArtifact& ScrollHitTestChunk(const PropertyTreeState& contents_state)
    {
        return ScrollHitTestChunk(NewClient(), contents_state);
    }

    TestPaintArtifact& ScrollingContentsChunk(const DisplayItemClient&, const PropertyTreeState& state, bool opaque = false);
    TestPaintArtifact& ScrollingContentsChunk(const PropertyTreeState& state, bool opaque = false)
    {
        return ScrollingContentsChunk(NewClient(), state, opaque);
    }

    TestPaintArtifact& ScrollChunks(const PropertyTreeState& contents_state, bool contents_opaque = false);
    ...
    TestPaintArtifact& ForeignLayer(scoped_refptr<cc::Layer> layer, const gfx::Point& offset);

    // Add display item with the specified client in the chunk.
    TestPaintArtifact& RectDrawing(const DisplayItemClient&, const gfx::Rect& bounds, Color color);
