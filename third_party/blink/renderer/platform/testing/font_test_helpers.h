...
Font CreateTestFont(const AtomicString& family_name, const String& font_path, float size, const FontDescription::VariantLigatures* = nullptr,
    const FontVariantEmoji variant_emoji = kNormalVariantEmoji, void (*init_font_description)(FontDescription*) = nullptr);

// Reads a font from raw font data, for use in fuzzing test only.
Font CreateTestFont(const AtomicString& family_name, const uint8_t* data, size_t data_size, float size, const FontDescription::VariantLigatures* = nullptr);
