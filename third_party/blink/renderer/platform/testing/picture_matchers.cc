...
public:
    ...
    const Vector<RectWithColor>& RectsWithColor() const
    {
        return rects_;
    }
    ...
    void onDrawRect(const SkRect& rect, const SkPaint& paint) override
    {
        ...
        if (getDeviceClipBounds(&device_clip_bounds) && device_rect.intersect(SkRect::Make(device_clip_bounds)))
...
class DrawsRectanglesMatcher : public testing::MatcherInterface<const SkPicture&> {
public:
    ...
    bool MatchAndExplain(const SkPicture& picture, testing::MatchResultListener* listener) const override
    {
        ...
        for (unsigned index = 0; index < actual_rects.size(); index++) {
            ...
            if (gfx::ToEnclosingRect(actual_rect_with_color.rect) != gfx::ToEnclosingRect(expect_rect_with_color.rect)
                || actual_rect_with_color.color != expect_rect_with_color.color) {
                if (listener->IsInterested()) {
                    *listener << "at index " << index << " which draws " << actual_rect_with_color.rect.ToString() << " with color "
    ...
    void DescribeTo(::std::ostream* os) const override
    {
        *os << "\n";
        for (unsigned index = 0; index < rects_with_color_.size(); index++) {
            ...
            *os << "at index " << index << " rect draws " << rect_with_color.rect.ToString() << " with color " << rect_with_color.color.SerializeAsCSSColor()
                << "\n";
        }
    }
...
testing::Matcher<const SkPicture&> DrawsRectangle(const gfx::RectF& rect, Color color)
{
...
testing::Matcher<const SkPicture&> DrawsRectangles(const Vector<RectWithColor>& rects_with_color)
{
    return testing::MakeMatcher(new DrawsRectanglesMatcher(rects_with_color));
}
