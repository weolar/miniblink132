...
ScopedMainThreadOverrider::ScopedMainThreadOverrider(std::unique_ptr<MainThread> main_thread)
    : original_main_thread_(MainThread::SetMainThread(std::move(main_thread)))
{
    ...
    // isolate.
    if (auto* scheduler = original_main_thread_->Scheduler()->ToMainThreadScheduler()) {
