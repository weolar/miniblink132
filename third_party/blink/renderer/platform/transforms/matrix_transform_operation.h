...
public:
    explicit MatrixTransformOperation(const gfx::Transform& t)
        : matrix_(t)
    {
...
template <> struct DowncastTraits<MatrixTransformOperation> {
    static bool AllowFrom(const TransformOperation& transform)
    {
        return MatrixTransformOperation::IsMatchingOperationType(transform.GetType());
    }
};
