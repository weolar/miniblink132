...
TransformOperation::OperationType GetTypeForTranslate(const Length& x, const Length& y, double z)
{
...
TransformOperation* TranslateTransformOperation::Accumulate(const TransformOperation& other)
{
    ...
    return MakeGarbageCollected<TranslateTransformOperation>(new_x, new_y, new_z, GetTypeForTranslate(new_x, new_y, new_z));
}

TransformOperation* TranslateTransformOperation::Blend(const TransformOperation* from, double progress, bool blend_to_identity)
{
    DCHECK(!from || CanBlendWith(*from));

    const Length zero_length = Length::Fixed(0);
    if (blend_to_identity) {
        return MakeGarbageCollected<TranslateTransformOperation>(zero_length.Blend(x_, progress, Length::ValueRange::kAll),
            zero_length.Blend(y_, progress, Length::ValueRange::kAll), blink::Blend(z_, 0., progress), type_);
    }
    ...
    return MakeGarbageCollected<TranslateTransformOperation>(
        x_.Blend(from_x, progress, Length::ValueRange::kAll), y_.Blend(from_y, progress, Length::ValueRange::kAll), blink::Blend(from_z, z_, progress), type);
}

TranslateTransformOperation* TranslateTransformOperation::ZoomTranslate(double factor)
{
    return MakeGarbageCollected<TranslateTransformOperation>(x_.Zoom(factor), y_.Zoom(factor), z_ * factor, type_);
}

void TranslateTransformOperation::CommonPrimitiveForInterpolation(const TransformOperation* from, TransformOperation::OperationType& common_type) const
{
    bool is_3d = Is3DOperation() || (from && from->Is3DOperation());
    const auto* from_op = To<TranslateTransformOperation>(from);
    TransformOperation::OperationType from_type = from_op ? from_op->type_ : type_;
    if (type_ == from_type) {
        common_type = type_;
    } else if (is_3d) {
