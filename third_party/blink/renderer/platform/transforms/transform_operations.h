...
public:
    ...
    // |border_box_size| is used when computing styles that are size-dependent.
    void ApplyRemaining(const gfx::SizeF& border_box_size, wtf_size_t start, gfx::Transform& t) const;
    ...
    {
        for (auto& operation : operations_) {
            if (operation->Is3DOperation() && operation->GetType() != TransformOperation::kPerspective)
                return true;
        }
        return false;
    }
    ...
    TransformOperation::BoxSizeDependency BoxSizeDependencies(wtf_size_t start = 0) const;
    ...
    bool BlendedBoundsForBox(
        const gfx::BoxF&, const TransformOperations& from, const double& min_progress, const double& max_progress, gfx::BoxF* bounds) const;
    ...
    TransformOperation* BlendRemainingByUsingMatrixInterpolation(const TransformOperations& from, wtf_size_t matching_prefix_length, double progress,
        BoxSizeDependentMatrixBlending box_size_dependent = BoxSizeDependentMatrixBlending::kAllow) const;

    TransformOperations Blend(
        const TransformOperations& from, double progress, BoxSizeDependentMatrixBlending box_size_dependent = BoxSizeDependentMatrixBlending::kAllow) const;
