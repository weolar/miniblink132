...
{
    return Quaternion::FromAxisAngle(rotation.axis.x(), rotation.axis.y(), rotation.axis.z(), Deg2rad(rotation.angle));
}
...
Rotation ComputeRotation(Quaternion q)
{
    ...
    gfx::Vector3dF interpolated_axis = NormalizeAxis(gfx::Vector3dF(q.x(), q.y(), q.z()));
    return Rotation(interpolated_axis, interpolated_angle);
}

} // namespace

bool Rotation::GetCommonAxis(const Rotation& a, const Rotation& b, gfx::Vector3dF& result_axis, double& result_angle_a, double& result_angle_b)
{
