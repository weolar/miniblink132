...
TransformOperation::OperationType GetTypeForScale(double x, double y, double z)
{
...
TransformOperation* ScaleTransformOperation::Accumulate(const TransformOperation& other)
{
    ...
    return MakeGarbageCollected<ScaleTransformOperation>(new_x, new_y, new_z, GetTypeForScale(new_x, new_y, new_z));
}

TransformOperation* ScaleTransformOperation::Blend(const TransformOperation* from, double progress, bool blend_to_identity)
{
    DCHECK(!from || CanBlendWith(*from));

    if (blend_to_identity) {
        return MakeGarbageCollected<ScaleTransformOperation>(
            blink::Blend(x_, 1.0, progress), blink::Blend(y_, 1.0, progress), blink::Blend(z_, 1.0, progress), type_);
    }

    const ScaleTransformOperation* from_op = static_cast<const ScaleTransformOperation*>(from);
    ...
    return MakeGarbageCollected<ScaleTransformOperation>(
        blink::Blend(from_x, x_, progress), blink::Blend(from_y, y_, progress), blink::Blend(from_z, z_, progress), type);
}

void ScaleTransformOperation::CommonPrimitiveForInterpolation(const TransformOperation* from, TransformOperation::OperationType& common_type) const
{
