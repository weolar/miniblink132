...
class PLATFORM_EXPORT TransformOperation : public GarbageCollected<TransformOperation> {
public:
    enum OperationType {
        kScaleX,
        kScaleY,
    ...
    virtual void Apply(gfx::Transform&, const gfx::SizeF& border_box_size) const = 0;
    ...
    virtual TransformOperation* Blend(const TransformOperation* from, double progress, bool blend_to_identity = false) = 0;
    virtual TransformOperation* Zoom(double factor) = 0;

    virtual OperationType GetType() const = 0;

    // https://drafts.csswg.org/css-transforms/#transform-primitives
    virtual OperationType PrimitiveType() const
    {
        return GetType();
    }
    ...
    virtual bool IsIdentityOrTranslation() const
    {
        return false;
    }
    ...
    bool Is3DOperation() const
    {
        ...
        return op_type == kScaleZ || op_type == kScale3D || op_type == kTranslateZ || op_type == kTranslate3D || op_type == kRotateX || op_type == kRotateY
            || op_type == kRotate3D || op_type == kMatrix3D || op_type == kPerspective || op_type == kInterpolated;
    }

    virtual bool HasNonTrivial3DComponent() const
    {
        return Is3DOperation();
    }

    enum BoxSizeDependency { kDependsNone = 0, kDependsWidth = 0x01, kDependsHeight = 0x02, kDependsBoth = kDependsWidth | kDependsHeight };
    virtual BoxSizeDependency BoxSizeDependencies() const
    {
        return kDependsNone;
    }

    static inline BoxSizeDependency CombineDependencies(BoxSizeDependency a, BoxSizeDependency b)
    {
