...
public:
    ...
    {
        return CombineDependencies((x_.HasPercent() ? kDependsWidth : kDependsNone), (y_.HasPercent() ? kDependsHeight : kDependsNone));
    }
    ...
    void Apply(gfx::Transform& transform, const gfx::SizeF& border_box_size) const override
    {
    ...
    {
        return type == kTranslate || type == kTranslateX || type == kTranslateY || type == kTranslateZ || type == kTranslate3D;
    }
    ...
    OperationType PrimitiveType() const final
    {
        return kTranslate3D;
    }
...
private:
    ...
    void CommonPrimitiveForInterpolation(const TransformOperation* from, TransformOperation::OperationType& common_type) const;
...
template <> struct DowncastTraits<TranslateTransformOperation> {
    static bool AllowFrom(const TransformOperation& transform)
    {
        return TranslateTransformOperation::IsMatchingOperationType(transform.GetType());
    }
};
