...
TEST(AffineTransformTest, Inverse)
{
    ...
    EXPECT_EQ(AffineTransform().Translate(1, -2), AffineTransform().Translate(-1, 2).Inverse());
    EXPECT_EQ(AffineTransform().Translate(1, -2), AffineTransform().Translate(-1, 2).Inverse());
    EXPECT_EQ(AffineTransform().Scale(2, -0.25), AffineTransform().Scale(0.5, -4).Inverse());
    EXPECT_EQ(AffineTransform().Scale(2, -0.25).Translate(1, -2), AffineTransform().Translate(-1, 2).Scale(0.5, -4).Inverse());
}
...
TEST(AffineTransformTest, ValidRangedMatrix)
{
    ...
    for (double* entry : entries) {
        ...
        auto is_valid_point = [&](const gfx::PointF& p) -> bool { return std::isfinite(p.x()) && std::isfinite(p.y()); };
        auto is_valid_rect = [&](const gfx::RectF& r) -> bool { return is_valid_point(r.origin()) && std::isfinite(r.width()) && std::isfinite(r.height()); };
        auto is_valid_quad
            = [&](const gfx::QuadF& q) -> bool { return is_valid_point(q.p1()) && is_valid_point(q.p2()) && is_valid_point(q.p3()) && is_valid_point(q.p4()); };

        auto test = [&](const AffineTransform& m) {
            SCOPED_TRACE(String::Format("m: %s factor: %lg", m.ToString().Utf8().data(), factor));
...
TEST(AffineTransformTest, ToString)
{
    ...
    rotation.Rotate(180);
    EXPECT_EQ("translation(0,0), scale(1,1), angle(180deg), skewxy(0)", rotation.ToString());
