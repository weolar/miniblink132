...
public:
    ...
    {
        return type == kScale || type == kScaleX || type == kScaleY || type == kScaleZ || type == kScale3D;
    }
...
template <> struct DowncastTraits<ScaleTransformOperation> {
    static bool AllowFrom(const TransformOperation& transform)
    {
        return ScaleTransformOperation::IsMatchingOperationType(transform.GetType());
    }
};
