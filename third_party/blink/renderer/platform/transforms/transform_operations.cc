...
namespace {
using ApplyCallback = base::RepeatingCallback<TransformOperation*(TransformOperation* from, TransformOperation* to)>;

// Applies a given function (|ApplyCallback|) to matching pairs of operations.
TransformOperations ApplyFunctionToMatchingPrefix(
    ApplyCallback apply_cb, const TransformOperations& from, const TransformOperations& to, wtf_size_t matching_prefix_length, bool* success)
{
    ...
    for (wtf_size_t i = 0; i < matching_prefix_length; i++) {
        TransformOperation* from_operation = (i < from_size) ? from.Operations()[i].Get() : nullptr;
        TransformOperation* to_operation = (i < to_size) ? to.Operations()[i].Get() : nullptr;

        TransformOperation* result_operation = apply_cb.Run(from_operation, to_operation);
...
void TransformOperations::ApplyRemaining(const gfx::SizeF& border_box_size, wtf_size_t start, gfx::Transform& t) const
{
...
TransformOperation::BoxSizeDependency TransformOperations::BoxSizeDependencies(wtf_size_t start) const
{
    TransformOperation::BoxSizeDependency deps = TransformOperation::kDependsNone;
    for (wtf_size_t i = start; i < operations_.size(); i++) {
        deps = TransformOperation::CombineDependencies(deps, operations_[i]->BoxSizeDependencies());
    }
    return deps;
}

wtf_size_t TransformOperations::MatchingPrefixLength(const TransformOperations& other) const
{
    wtf_size_t num_operations = std::min(Operations().size(), other.Operations().size());
...
TransformOperation* TransformOperations::BlendRemainingByUsingMatrixInterpolation(
    const TransformOperations& from, wtf_size_t matching_prefix_length, double progress, BoxSizeDependentMatrixBlending box_size_dependent) const
{
    // Not safe to use a cached transform if any of the operations are size
    // dependent.
    if (BoxSizeDependencies(matching_prefix_length) || from.BoxSizeDependencies(matching_prefix_length)) {
        if (box_size_dependent == BoxSizeDependentMatrixBlending::kDisallow) {
            return nullptr;
        }
        return MakeGarbageCollected<InterpolatedTransformOperation>(from, *this, matching_prefix_length, progress);
    }
...
// interpolation.
TransformOperations TransformOperations::Blend(const TransformOperations& from, double progress, BoxSizeDependentMatrixBlending box_size_dependent) const
{
    if (from == *this || (!from.size() && !size()))
        return *this;

    wtf_size_t matching_prefix_length = MatchingPrefixLength(from);
    wtf_size_t max_path_length = std::max(Operations().size(), from.Operations().size());

    bool success = true;
    TransformOperations result
        = ApplyFunctionToMatchingPrefix(WTF::BindRepeating(
                                            [](double progress, TransformOperation* from, TransformOperation* to) {
                                                ...
                                                return to ? to->Blend(from, progress) : (from ? from->Blend(nullptr, progress, true) : nullptr);
                                            },
                                            progress),
            from, *this, matching_prefix_length, &success);
    if (success && matching_prefix_length < max_path_length) {
        TransformOperation* matrix_op = BlendRemainingByUsingMatrixInterpolation(from, matching_prefix_length, progress, box_size_dependent);
        if (matrix_op)
...
TransformOperations TransformOperations::Accumulate(const TransformOperations& to) const
{
    if (!to.size() && !size())
        return *this;
    ...
    wtf_size_t max_path_length = std::max(Operations().size(), to.Operations().size());

    // Accumulate matching pairs of transform functions.
    TransformOperations result = ApplyFunctionToMatchingPrefix(WTF::BindRepeating([](TransformOperation* from, TransformOperation* to) {
        if (to && from)
    ...
    if (success && matching_prefix_length < max_path_length) {
        ...
        TransformOperation* from_matrix = MakeGarbageCollected<Matrix3DTransformOperation>(from_transform);
        TransformOperation* to_matrix = MakeGarbageCollected<Matrix3DTransformOperation>(to_transform);
...
static void FindCandidatesInPlane(double px, double py, double nz, double* candidates, int* num_candidates)
{
...
static void BoundingBoxForArc(const gfx::Point3F& point, const RotateTransformOperation& from_transform, const RotateTransformOperation& to_transform,
    double min_progress, double max_progress, gfx::BoxF& box)
{
    double candidates[6];
    ...
    case TransformOperation::kRotateX:
        FindCandidatesInPlane(point.y(), point.z(), from_transform.X(), candidates, &num_candidates);
        break;
    case TransformOperation::kRotateY:
        FindCandidatesInPlane(point.z(), point.x(), from_transform.Y(), candidates, &num_candidates);
        break;
    case TransformOperation::kRotateZ:
    case TransformOperation::kRotate:
        FindCandidatesInPlane(point.x(), point.y(), from_transform.Z(), candidates, &num_candidates);
        break;
    default: {
        ...
        gfx::Point3F center = gfx::PointAtOffsetFromOrigin(gfx::ScaleVector3d(normal, gfx::DotProduct(to_point, normal)));
...
bool TransformOperations::BlendedBoundsForBox(
    const gfx::BoxF& box, const TransformOperations& from, const double& min_progress, const double& max_progress, gfx::BoxF* bounds) const
{
    ...
    *bounds = box;
    for (int i = size - 1; i >= 0; i--) {
        TransformOperation* from_operation = (i < from_size) ? from.Operations()[i] : nullptr;
        TransformOperation* to_operation = (i < to_size) ? Operations()[i] : nullptr;

        DCHECK(from_operation || to_operation);
        TransformOperation::OperationType interpolation_type = to_operation ? to_operation->GetType() : from_operation->GetType();
        if (from_operation && to_operation && !from_operation->CanBlendWith(*to_operation)) {
            return false;
        }
        ...
        case TransformOperation::kPerspective: {
            ...
            if (!to_operation) {
                from_transform = from_operation->Blend(to_operation, 1 - min_progress, false);
                to_transform = from_operation->Blend(to_operation, 1 - max_progress, false);
            } else {
                from_transform = to_operation->Blend(from_operation, min_progress, false);
                to_transform = to_operation->Blend(from_operation, max_progress, false);
            }
        ...
        case TransformOperation::kRotateZ: {
            ...
            if (from_operation) {
                from_rotation = static_cast<const RotateTransformOperation*>(from_operation);
                if (from_rotation->Axis().IsZero())
                    from_rotation = nullptr;
            }

            if (to_operation) {
                to_rotation = static_cast<const RotateTransformOperation*>(to_operation);
            ...
            double to_angle;
            gfx::Vector3dF axis;
            if (!RotateTransformOperation::GetCommonAxis(from_rotation, to_rotation, axis, from_angle, to_angle)) {
                return false;
            }

            if (!from_rotation) {
                identity_rotation = MakeGarbageCollected<RotateTransformOperation>(
                    axis.x(), axis.y(), axis.z(), 0, from_operation ? from_operation->GetType() : to_operation->GetType());
            ...
            for (size_t j = 0; j < 2; ++j) {
                for (size_t k = 0; k < 2; ++k) {
                    for (size_t m = 0; m < 2; ++m) {
                        ...
                        corner += gfx::Vector3dF(j * from_box.width(), k * from_box.height(), m * from_box.depth());
                        BoundingBoxForArc(corner, *from_rotation, *to_rotation, min_progress, max_progress, bounds_for_arc);
                        if (first) {
