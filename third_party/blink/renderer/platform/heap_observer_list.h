...
template <class ObserverType> class PLATFORM_EXPORT HeapObserverList final {
    DISALLOW_NEW();
...
public:
    ...
    {
        return mutation_state_ != kAllowAll;
    }
    ...
    //       observer->SomeMethod();
    //     });
    template <typename ForEachCallable> void ForEachObserver(const ForEachCallable& callable) const
    {
    ...
    void Trace(Visitor* visitor) const
    {
        ...
        visitor->RegisterWeakCallbackMethod<HeapObserverList, &HeapObserverList::CleanupDeadObservers>(this);
    }
...
private:
    ...
    void CleanupDeadObservers(const LivenessBroker& broker)
    {
        ...
        if (mutation_state_ == kNoMutationAllowed) {
            ...
            observers_.erase(std::remove_if(observers_.begin(), observers_.end(), [broker](auto& observer) { return !broker.IsHeapObjectAlive(observer); }),
                observers_.end());
