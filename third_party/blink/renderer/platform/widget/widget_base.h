...
class PLATFORM_EXPORT WidgetBase : public mojom::blink::Widget, public LayerTreeViewDelegate, public mojom::blink::RenderInputRouterClient {
public:
    WidgetBase(WidgetBaseClient* client, CrossVariantMojoAssociatedRemote<mojom::WidgetHostInterfaceBase> widget_host,
    ...
    void InitializeCompositing(PageScheduler& page_scheduler, const display::ScreenInfos& screen_infos, const cc::LayerTreeSettings* settings,
    ...
    void AddPresentationCallback(uint32_t frame_token, base::OnceCallback<void(const viz::FrameTimingDetails&)> callback);

    void WarmUpCompositor();

#if BUILDFLAG(IS_APPLE)
    void AddCoreAnimationErrorCodeCallback(uint32_t frame_token, base::OnceCallback<void(gfx::CALayerResult)> callback);
#endif

    // mojom::blink::RenderInputRouterClient overrides;
    void GetWidgetInputHandler(
        mojo::PendingReceiver<mojom::blink::WidgetInputHandler> request, mojo::PendingRemote<mojom::blink::WidgetInputHandlerHost> host) override;
    ...
    void WasShown(bool was_evicted, mojom::blink::RecordContentToVisibleTimeRequestPtr record_tab_switch_time_request) override;
    void RequestSuccessfulPresentationTimeForNextFrame(mojom::blink::RecordContentToVisibleTimeRequestPtr visible_time_request) override;
    void CancelSuccessfulPresentationTimeRequest() override;
    void SetupRenderInputRouterConnections(mojo::PendingReceiver<mojom::blink::RenderInputRouterClient> browser_request,
        mojo::PendingReceiver<mojom::blink::RenderInputRouterClient> viz_request) override;
    ...
    void OnDeferCommitsChanged(bool defer_status, cc::PaintHoldingReason reason, std::optional<cc::PaintHoldingCommitTrigger> trigger) override;
    void OnCommitRequested() override;
    void DidBeginMainFrame() override;
    void RequestNewLayerTreeFrameSink(LayerTreeFrameSinkCallback callback) override;
    ...
    void DidCommitCompositorFrame(base::TimeTicks commit_start_time, base::TimeTicks commit_finish_time) override;
    void DidCompletePageScaleAnimation() override;
    void RecordStartOfFrameMetrics() override;
    void RecordEndOfFrameMetrics(base::TimeTicks frame_begin_time, cc::ActiveFrameSequenceTrackers trackers) override;
    ...
    std::unique_ptr<cc::RenderFrameMetadataObserver> CreateRenderFrameObserver() override;
    ...
    WidgetBaseInputHandler& input_handler()
    {
    ...
    void QueueSyntheticEvent(std::unique_ptr<blink::WebCoalescedInputEvent> event);
    void SetEditCommandsForNextKeyEvent(Vector<mojom::blink::EditCommandPtr> edit_commands);
    ...
    void ImeCommitText(const String& text, const Vector<ui::ImeTextSpan>& ime_text_spans, const gfx::Range& replacement_range, int relative_cursor_pos);
    void ImeFinishComposingText(bool keep_selection);
    bool IsForProvisionalFrame();
    void FlushInputProcessedCallback();
    void CancelCompositionForPepper();

    void RequestPresentationAfterScrollAnimationEnd(mojom::blink::Widget::ForceRedrawCallback callback);
    ...
    void set_handling_select_range(bool value)
    {
        handling_select_range_ = value;
    }
    bool handling_select_range() const
    {
        return handling_select_range_;
    }
    ...
    const gfx::Rect& WidgetScreenRect() const
    {
        return widget_screen_rect_;
    }

    // Returns the bounds of the screen the widget is contained in (in screen
    // coordinates).
    const gfx::Rect& WindowScreenRect() const
    {
        return window_screen_rect_;
    }

    // Sets the screen rects (in screen coordinates).
    void SetScreenRects(const gfx::Rect& widget_screen_rect, const gfx::Rect& window_screen_rect);
    ...
    void BindWidgetCompositor(mojo::PendingReceiver<mojom::blink::WidgetCompositor> receiver);
    ...
    // Similar to UpdateSurfaceAndScreenInfo but the surface allocation
    // remains the same.
    void UpdateCompositorViewportAndScreenInfo(const gfx::Rect& compositor_viewport_pixel_rect, const display::ScreenInfos& new_screen_infos);
    ...
    const display::ScreenInfos& screen_infos() const
    {
...
private:
    ...
    void UpdateTextInputStateInternal(bool show_virtual_keyboard, bool immediate_request);
    ...
    bool ShouldUpdateCompositionInfo(const gfx::Range& range, const Vector<gfx::Rect>& bounds);
    ...
    void FinishRequestNewLayerTreeFrameSink(const KURL& url, mojo::PendingReceiver<viz::mojom::blink::CompositorFrameSink> compositor_frame_sink_receiver,
    ...
    mojo::Receiver<mojom::blink::RenderInputRouterClient> browser_input_receiver_ { this };
    mojo::Receiver<mojom::blink::RenderInputRouterClient> viz_input_receiver_ { this };
    ...
    ui::mojom::VirtualKeyboardPolicy vk_policy_ = ui::mojom::VirtualKeyboardPolicy::AUTO;
