...
public:
    ...
    virtual void RecordDispatchRafAlignedInputTime(base::TimeTicks raf_aligned_input_start_time)
    {
    }
    ...
    virtual void UpdateLifecycle(WebLifecycleUpdate requested_update, DocumentUpdateReason reason) = 0;
    ...
    virtual void RecordEndOfFrameMetrics(base::TimeTicks frame_begin_time, cc::ActiveFrameSequenceTrackers trackers)
    {
    }
    ...
    virtual std::unique_ptr<cc::BeginMainFrameMetrics> GetBeginMainFrameMetrics()
    {
        return nullptr;
    }
    ...
    virtual void EndCommitCompositorFrame(base::TimeTicks commit_start_time, base::TimeTicks commit_finish_time)
    {
    }
    ...
    virtual void UpdateCompositorScrollState(const cc::CompositorCommitData& commit_data)
    {
    }
    ...
    virtual std::unique_ptr<cc::LayerTreeFrameSink> AllocateNewLayerTreeFrameSink() = 0;
    ...
    virtual WebInputEventResult HandleInputEvent(const WebCoalescedInputEvent&) = 0;
    virtual bool SupportsBufferedTouchEvents() = 0;

    virtual void DidHandleKeyEvent()
    {
    }
    virtual void WillHandleGestureEvent(const WebGestureEvent& event, bool* suppress) = 0;
    ...
    virtual blink::FrameWidget* FrameWidget()
    {
        return nullptr;
    }
    ...
    virtual bool ShouldAckSyntheticInputImmediately()
    {
        return false;
    }

    // Apply the visual properties.
    virtual void UpdateVisualProperties(const VisualProperties& visual_properties) = 0;
    ...
    virtual bool UpdateScreenRects(const gfx::Rect& widget_screen_rect, const gfx::Rect& window_screen_rect)
    {
        return false;
    }
    ...
    virtual void DidUpdateSurfaceAndScreen(const display::ScreenInfos& previous_original_screen_infos)
    {
    }
    ...
    virtual std::optional<display::mojom::blink::ScreenOrientation> ScreenOrientationOverride()
    {
        return std::nullopt;
    }

    // Return the overridden device scale factor for testing.
    virtual float GetTestingDeviceScaleFactorOverride()
    {
        return 0.f;
    }
    ...
    virtual void RunPaintBenchmark(int repeat_count, cc::PaintBenchmarkResult& result)
    {
    }
    ...
    virtual bool AllowsScrollResampling()
    {
        return true;
    }

    virtual void ShowContextMenu(ui::mojom::blink::MenuSourceType source_type, const gfx::Point& location)
    {
    }
    virtual void BindInputTargetClient(mojo::PendingReceiver<viz::mojom::blink::InputTargetClient> receiver)
    {
    }
};
