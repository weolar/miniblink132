...
enum InertialPhaseState {
    UnknownMomentumPhase = static_cast<int>(WebGestureEvent::InertialPhaseState::kUnknownMomentum),
    NonMomentumPhase = static_cast<int>(WebGestureEvent::InertialPhaseState::kNonMomentum),
    MomentumPhase = static_cast<int>(WebGestureEvent::InertialPhaseState::kMomentum),
};
...
public:
    ...
    Vector2dF StretchAmount() const override
    {
    ...
    Size ScrollBounds() const override
    {
        return Size(800, 600);
    }
    ...
    {
        request_begin_frame_count_ += 1;
    }

    // Counters for number of times functions were called.
    int request_begin_frame_count() const
    {
        return request_begin_frame_count_;
    }
    int set_stretch_amount_count() const
    {
        return set_stretch_amount_count_;
    }

    void SetScrollOffsetAndMaxScrollOffset(const gfx::PointF& scroll_offset, const gfx::PointF& max_scroll_offset)
    {
...
public:
    ElasticOverscrollControllerExponentialTest()
        : controller_(&helper_)
        , current_time_(base::TimeTicks() + base::Microseconds(INT64_C(100000000)))
    {
    }
    ...
    {
        TickCurrentTime();
        WebGestureEvent event(WebInputEvent::Type::kGestureScrollBegin, WebInputEvent::kNoModifiers, current_time_, WebGestureDevice::kTouchpad);
        event.data.scroll_begin.inertial_phase = static_cast<WebGestureEvent::InertialPhaseState>(inertialPhase);

        controller_.ObserveGestureEventAndResult(event, cc::InputHandlerScrollResult());
    }

    void SendGestureScrollUpdate(InertialPhaseState inertialPhase, const Vector2dF& event_delta = Vector2dF(), const Vector2dF& overscroll_delta = Vector2dF(),
        const cc::OverscrollBehavior& overscroll_behavior = cc::OverscrollBehavior())
    {
        TickCurrentTime();
        WebGestureEvent event(WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers, current_time_, WebGestureDevice::kTouchpad);
        event.data.scroll_update.inertial_phase = static_cast<WebGestureEvent::InertialPhaseState>(inertialPhase);
    ...
    {
        TickCurrentTime();
        WebGestureEvent event(WebInputEvent::Type::kGestureScrollEnd, WebInputEvent::kNoModifiers, current_time_, WebGestureDevice::kTouchpad);
...
{
    helper_.SetScrollOffsetAndMaxScrollOffset(gfx::PointF(10, 10), gfx::PointF(10, 10));

    // If we push equally in the X and Y directions, we should see a stretch
    // in the Y direction.
    SendGestureScrollBegin(NonMomentumPhase);
    SendGestureScrollUpdate(NonMomentumPhase, Vector2dF(10, 10), Vector2dF(10, 10));
    ...
    // overscroll delta.
    helper_.SetScrollOffsetAndMaxScrollOffset(gfx::PointF(0, 10), gfx::PointF(10, 10));
    SendGestureScrollBegin(NonMomentumPhase);
    SendGestureScrollUpdate(NonMomentumPhase, Vector2dF(-25, 10), Vector2dF(-25, 10));
...
TEST_F(ElasticOverscrollControllerExponentialTest, MinimumDeltaBeforeStretch)
{
    ...
    helper_.SetScrollOffsetAndMaxScrollOffset(gfx::PointF(5, 5), gfx::PointF(10, 10));
    SendGestureScrollBegin(NonMomentumPhase);
    ...
    SendGestureScrollUpdate(NonMomentumPhase, Vector2dF(-10, 5), Vector2dF(-8, 5));
    EXPECT_EQ(0, helper_.set_stretch_amount_count());
...
TEST_F(ElasticOverscrollControllerExponentialTest, MomentumAnimate)
{
    ...
    helper_.SetScrollOffsetAndMaxScrollOffset(gfx::PointF(5, 0), gfx::PointF(10, 10));
...
TEST_F(ElasticOverscrollControllerExponentialTest, ReconcileStretchAndScroll)
{
    ...
    helper_.SetScrollOffsetAndMaxScrollOffset(gfx::PointF(5, 8), gfx::PointF(10, 10));
    ...
    helper_.SetScrollOffsetAndMaxScrollOffset(gfx::PointF(2, 3), gfx::PointF(10, 10));
...
TEST_F(ElasticOverscrollControllerExponentialTest, UserScrollableRequiredForStretch)
{
    helper_.SetScrollOffsetAndMaxScrollOffset(gfx::PointF(0, 0), gfx::PointF(10, 10));
...
TEST_F(ElasticOverscrollControllerExponentialTest, OverscrollBehavior)
{
    ...
    SendGestureScrollUpdate(
        NonMomentumPhase, Vector2dF(10, 0), Vector2dF(10, 0), cc::OverscrollBehavior(cc::OverscrollBehavior::Type::kNone, cc::OverscrollBehavior::Type::kAuto));
    EXPECT_EQ(0, helper_.set_stretch_amount_count());
    EXPECT_EQ(0.f, helper_.StretchAmount().x());
    EXPECT_EQ(0.f, helper_.StretchAmount().y());
    helper_.SetStretchAmount(Vector2dF());
    ...
    SendGestureScrollBegin(NonMomentumPhase);
    SendGestureScrollUpdate(
        NonMomentumPhase, Vector2dF(0, 10), Vector2dF(0, 10), cc::OverscrollBehavior(cc::OverscrollBehavior::Type::kNone, cc::OverscrollBehavior::Type::kAuto));
    EXPECT_EQ(2, helper_.set_stretch_amount_count());
    EXPECT_EQ(0.f, helper_.StretchAmount().x());
    EXPECT_LT(0.f, helper_.StretchAmount().y());
    helper_.SetStretchAmount(Vector2dF());
    ...
    SendGestureScrollBegin(NonMomentumPhase);
    SendGestureScrollUpdate(
        NonMomentumPhase, Vector2dF(0, 10), Vector2dF(0, 10), cc::OverscrollBehavior(cc::OverscrollBehavior::Type::kAuto, cc::OverscrollBehavior::Type::kNone));
    EXPECT_EQ(3, helper_.set_stretch_amount_count());
    EXPECT_EQ(0.f, helper_.StretchAmount().x());
    EXPECT_EQ(0.f, helper_.StretchAmount().y());
    helper_.SetStretchAmount(Vector2dF());
    ...
    SendGestureScrollBegin(NonMomentumPhase);
    SendGestureScrollUpdate(
        NonMomentumPhase, Vector2dF(10, 0), Vector2dF(10, 0), cc::OverscrollBehavior(cc::OverscrollBehavior::Type::kAuto, cc::OverscrollBehavior::Type::kNone));
...
TEST_F(ElasticOverscrollControllerExponentialTest, OverscrollBehaviorNonScrollable)
{
    int expected_stretch_count = 0;
    // Set up a scroller which is vertically scrollable scrolled to the bottom.
    helper_.SetScrollOffsetAndMaxScrollOffset(gfx::PointF(0, 10), gfx::PointF(0, 10));
