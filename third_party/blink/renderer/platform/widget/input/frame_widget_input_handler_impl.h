...
class PLATFORM_EXPORT FrameWidgetInputHandlerImpl : public mojom::blink::FrameWidgetInputHandler {
public:
    // The `widget` and `frame_widget_input_handler` should be invalidated
    // at the same time.
    FrameWidgetInputHandlerImpl(base::WeakPtr<WidgetBase> widget, base::WeakPtr<mojom::blink::FrameWidgetInputHandler> frame_widget_input_handler,
        scoped_refptr<MainThreadEventQueue> input_event_queue);
    FrameWidgetInputHandlerImpl(const FrameWidgetInputHandlerImpl&) = delete;
    FrameWidgetInputHandlerImpl& operator=(const FrameWidgetInputHandlerImpl&) = delete;
    ...
    void AdjustSelectionByCharacterOffset(int32_t start, int32_t end, blink::mojom::SelectionMenuBehavior selection_menu_behavior) override;
...
private:
    ...
    static void SetCompositionFromExistingTextMainThread(
        base::WeakPtr<WidgetBase> widget, int32_t start, int32_t end, const Vector<ui::ImeTextSpan>& ui_ime_text_spans);
    static void ExtendSelectionAndDeleteMainThread(base::WeakPtr<WidgetBase> widget, int32_t before, int32_t after);
    ...
    static void ExecuteCommandOnMainThread(
        base::WeakPtr<WidgetBase> widget, base::WeakPtr<mojom::blink::FrameWidgetInputHandler> handler, const char* command, UpdateState state);

    bool ThreadedCompositingEnabled()
    {
        return input_event_queue_ != nullptr;
    }

    // These should only be accessed on the main thread.
    base::WeakPtr<WidgetBase> widget_;
    base::WeakPtr<mojom::blink::FrameWidgetInputHandler> main_thread_frame_widget_input_handler_;
