...
public:
    ...
    MainThreadEventQueueTaskList& operator=(const MainThreadEventQueueTaskList&) = delete;
    ~MainThreadEventQueueTaskList();
...
private:
    using EventQueue = base::circular_deque<std::unique_ptr<MainThreadEventQueueTask>>;
    EventQueue queue_;
};
