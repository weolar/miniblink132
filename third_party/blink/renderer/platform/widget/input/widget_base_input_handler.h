...
public:
    ...
    using HandledEventCallback = base::OnceCallback<void(mojom::InputEventResultState ack_state, const ui::LatencyInfo& latency_info,
        std::unique_ptr<InputHandlerProxy::DidOverscrollParams>, std::optional<WebTouchAction>)>;
    ...
    void HandleInputEvent(const blink::WebCoalescedInputEvent& coalesced_event, std::unique_ptr<cc::EventMetrics> metrics, HandledEventCallback callback);
    ...
    bool handling_input_event() const
    {
        return handling_input_event_;
    }
...
private:
    ...
    WebInputEventResult HandleTouchEvent(const WebCoalescedInputEvent& coalesced_event);
    ...
    void HandleInjectedScrollGestures(Vector<InjectScrollGestureParams> injected_scroll_params, const WebInputEvent& input_event,
