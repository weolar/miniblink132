...
ElasticOverscrollControllerExponential::ElasticOverscrollControllerExponential(cc::ScrollElasticityHelper* helper)
...
gfx::Vector2d ElasticOverscrollControllerExponential::StretchAmountForTimeDelta(const base::TimeDelta& delta) const
{
    ...
    float critical_dampening_factor = expf((-delta.InSecondsF() * kRubberbandStiffness) / period);

    return gfx::ToRoundedVector2d(
        gfx::ScaleVector2d(momentum_animation_initial_stretch_ + gfx::ScaleVector2d(momentum_animation_initial_velocity_, delta.InSecondsF() * amplitude),
            critical_dampening_factor));
}

gfx::Vector2d ElasticOverscrollControllerExponential::StretchAmountForAccumulatedOverscroll(const gfx::Vector2dF& accumulated_overscroll) const
{
    const float stiffness = std::max(kRubberbandStiffness, 1.0);
    return gfx::ToRoundedVector2d(gfx::ScaleVector2d(accumulated_overscroll, 1.0f / stiffness));
}

gfx::Vector2d ElasticOverscrollControllerExponential::AccumulatedOverscrollForStretchAmount(const gfx::Vector2dF& delta) const
{
    return gfx::ToRoundedVector2d(gfx::ScaleVector2d(delta, kRubberbandStiffness));
}
} // namespace blink
