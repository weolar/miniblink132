...
public:
    InputEventPredictionTest()
    {
        ...
        ConfigureFieldTrialAndInitialize(blink::features::kResamplingInputEvents, ::features::kPredictorNameEmpty);
    }
    ...
    std::unique_ptr<ui::InputPredictor::InputData> GetPrediction(const WebPointerProperties& event) const
    {
        if (event.pointer_type == WebPointerProperties::PointerType::kMouse) {
            return event_predictor_->mouse_predictor_->GeneratePrediction(base::TimeTicks::Now());
        } else {
            auto predictor = event_predictor_->pointer_id_predictor_map_.find(event.id);
    ...
    void ConfigureFieldTrial(const base::Feature& feature, const std::string& predictor_type)
    {
        ...
        EXPECT_EQ(params["predictor"], GetFieldTrialParamValueByFeature(feature, "predictor"));
    }

    void ConfigureFieldTrialAndInitialize(const base::Feature& feature, const std::string& predictor_type)
    {
...
TEST_F(InputEventPredictionTest, PredictorType)
{
    ...
    EXPECT_EQ(event_predictor_->selected_predictor_type_, PredictorType::kScrollPredictorTypeEmpty);

    ConfigureFieldTrialAndInitialize(blink::features::kResamplingInputEvents, ::features::kPredictorNameEmpty);
    EXPECT_EQ(event_predictor_->selected_predictor_type_, PredictorType::kScrollPredictorTypeEmpty);

    ConfigureFieldTrialAndInitialize(blink::features::kResamplingInputEvents, ::features::kPredictorNameKalman);
    EXPECT_EQ(event_predictor_->selected_predictor_type_, PredictorType::kScrollPredictorTypeKalman);

    ConfigureFieldTrialAndInitialize(blink::features::kResamplingInputEvents, ::features::kPredictorNameKalman);
    EXPECT_EQ(event_predictor_->selected_predictor_type_, PredictorType::kScrollPredictorTypeKalman);

    ConfigureFieldTrialAndInitialize(blink::features::kResamplingInputEvents, ::features::kPredictorNameLsq);
    EXPECT_EQ(event_predictor_->selected_predictor_type_, PredictorType::kScrollPredictorTypeLsq);

    // Default to Kalman predictor.
    ConfigureFieldTrialAndInitialize(blink::features::kResamplingInputEvents, "");
    EXPECT_EQ(event_predictor_->selected_predictor_type_, PredictorType::kScrollPredictorTypeKalman);

    ConfigureFieldTrialAndInitialize(blink::features::kInputPredictorTypeChoice, ::features::kPredictorNameLsq);
...
TEST_F(InputEventPredictionTest, SingleTouchPoint)
{
    ...
    touch_event.touches[0].pointer_type = WebPointerProperties::PointerType::kTouch;
...
{
    WebMouseEvent pen_move = SyntheticWebMouseEventBuilder::Build(WebInputEvent::Type::kMouseMove, 10, 10, 0, WebPointerProperties::PointerType::kPen);
    ...
    WebMouseEvent pen_leave = SyntheticWebMouseEventBuilder::Build(WebInputEvent::Type::kMouseLeave, 10, 10, 0, WebPointerProperties::PointerType::kPen);
...
TEST_F(InputEventPredictionTest, TouchAndStylusResetMousePredictor)
{
    ...
    WebMouseEvent pen_move = SyntheticWebMouseEventBuilder::Build(WebInputEvent::Type::kMouseMove, 20, 20, 0, WebPointerProperties::PointerType::kPen);
    pen_move.id = 1;
...
TEST_F(InputEventPredictionTest, TouchScrollStartedRemoveAllTouchPoints)
{
    ...
    touch_event.touches[1].pointer_type = WebPointerProperties::PointerType::kTouch;
    HandleEvents(touch_event);
...
TEST_F(InputEventPredictionTest, ResamplingDisabled)
{
    ...
    HandleEvents(mouse_move);
    mouse_move = SyntheticWebMouseEventBuilder::Build(WebInputEvent::Type::kMouseMove, 11, 9, 0);
    HandleEvents(mouse_move);

    mouse_move = SyntheticWebMouseEventBuilder::Build(WebInputEvent::Type::kMouseMove, 12, 8, 0);
    HandleEvents(mouse_move);

    // The 4th move event should generate predicted events.
    mouse_move = SyntheticWebMouseEventBuilder::Build(WebInputEvent::Type::kMouseMove, 13, 7, 0);
    ...
    // change.
    const WebMouseEvent& event = static_cast<const blink::WebMouseEvent&>(coalesced_event.Event());
...
TEST_F(InputEventPredictionTest, NoResampleWhenExceedMaxResampleTime)
{
    ...
    base::TimeDelta predictor_max_resample_time = event_predictor_->mouse_predictor_->MaxResampleTime();
...

    {
        ...
        mouse_move.SetTimeStamp(event_time += base::Milliseconds(8));
        blink::WebCoalescedInputEvent coalesced_event(mouse_move, ui::LatencyInfo());
        base::TimeTicks frame_time = event_time + predictor_max_resample_time; // No cut off
        ...
        EXPECT_EQ(coalesced_event.PredictedEvent(0).TimeStamp(), event_time + base::Milliseconds(8));
    }

    {
        ...
        mouse_move = SyntheticWebMouseEventBuilder::Build(WebInputEvent::Type::kMouseMove, 14, 6, 0);
        mouse_move.SetTimeStamp(event_time += base::Milliseconds(8));
        blink::WebCoalescedInputEvent coalesced_event(mouse_move, ui::LatencyInfo());
        base::TimeTicks frame_time = event_time + predictor_max_resample_time + base::Milliseconds(10); // overpredict on purpose
        event_predictor_->HandleEvents(coalesced_event, frame_time);
...
TEST_F(InputEventPredictionTest, PredictedEventsTimeIntervalEqualRealEvents)
{
...

    {
        ...
        EXPECT_EQ(coalesced_event.PredictedEvent(0).TimeStamp(), event_time + base::Milliseconds(6));
    }
}
...
TEST_F(InputEventPredictionTest, TouchPointStates)
{
    ...
    for (size_t state = static_cast<size_t>(blink::WebTouchPoint::State::kStateUndefined); state <= static_cast<size_t>(blink::WebTouchPoint::State::kMaxValue);
         state++) {
        touch_event.touches[0].state = static_cast<blink::WebTouchPoint::State>(state);
        blink::WebCoalescedInputEvent coalesced_event(touch_event, ui::LatencyInfo());
        event_predictor_->HandleEvents(coalesced_event, base::TimeTicks::Now());
