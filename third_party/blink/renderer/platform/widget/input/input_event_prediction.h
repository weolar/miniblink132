...
public:
    ...
    void HandleEvents(blink::WebCoalescedInputEvent& coalesced_event, base::TimeTicks frame_time);
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(InputEventPredictionTest, NoResampleWhenExceedMaxResampleTime);
    ...
    base::TimeDelta GetPredictionTimeInterval(const WebPointerProperties& event) const;
    ...
    void UpdateSinglePointer(const WebPointerProperties& event, base::TimeTicks time);
    ...
    // available.
    bool GetPointerPrediction(base::TimeTicks predict_time, WebPointerProperties* event);
    ...
    std::unordered_map<PointerId, std::unique_ptr<ui::InputPredictor>> pointer_id_predictor_map_ ALLOW_DISCOURAGED_TYPE("TODO(crbug.com/1404327)");
