...
public:
    ...
    {
        scroll_predictor_->predictor_ = std::make_unique<ui::LeastSquaresPredictor>();
    }

    std::unique_ptr<WebInputEvent> CreateGestureScrollUpdate(float delta_x = 0, float delta_y = 0, double time_delta_in_milliseconds = 0,
        WebGestureEvent::InertialPhaseState phase = WebGestureEvent::InertialPhaseState::kNonMomentum)
    {
        auto gesture = std::make_unique<WebGestureEvent>(WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers,
            WebInputEvent::GetStaticTimeStampForTests() + base::Milliseconds(time_delta_in_milliseconds), WebGestureDevice::kTouchscreen);
        ...
        original_events_.emplace_back(std::make_unique<WebCoalescedInputEvent>(gesture->Clone(), ui::LatencyInfo()), nullptr, base::NullCallback());

        return gesture;
    }

    void CoalesceWith(const std::unique_ptr<WebInputEvent>& new_event, std::unique_ptr<WebInputEvent>& old_event)
    {
    ...
    {
        WebGestureEvent gesture_begin(
            WebInputEvent::Type::kGestureScrollBegin, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests(), WebGestureDevice::kTouchscreen);
        scroll_predictor_->ResetOnGestureScrollBegin(gesture_begin);
    }

    void HandleResampleScrollEvents(std::unique_ptr<WebInputEvent>& event, double time_delta_in_milliseconds = 0, double display_refresh_rate = 30)
    {
        auto event_with_callback
            = std::make_unique<EventWithCallback>(std::make_unique<WebCoalescedInputEvent>(std::move(event), ui::LatencyInfo()), base::NullCallback(), nullptr);
        ...
        event_with_callback = scroll_predictor_->ResampleScrollEvents(
            std::move(event_with_callback), WebInputEvent::GetStaticTimeStampForTests() + base::Milliseconds(time_delta_in_milliseconds), frame_interval);

        event = event_with_callback->event().Clone();
    }

    std::unique_ptr<ui::InputPredictor::InputData> PredictionAvailable(double time_delta_in_milliseconds = 0)
    {
        base::TimeTicks frame_time = WebInputEvent::GetStaticTimeStampForTests() + base::Milliseconds(time_delta_in_milliseconds);
        // Tests with 60Hz.
    ...
    {
        return scroll_predictor_->filtering_enabled_;
    }

    void ConfigurePredictorFieldTrialAndInitialize(const base::Feature& feature, const std::string& predictor_type)
    {
        ConfigurePredictorAndFilterInternal(feature, predictor_type, /* enable_filtering = */ false, blink::features::kFilteringScrollPrediction, "");
        scroll_predictor_ = std::make_unique<ScrollPredictor>();
    }

    void ConfigureFilterFieldTrialAndInitialize(const base::Feature& feature, const std::string& filter_name)
    {
        // We still need the resampler feature to construct the scroll predictor at
        // all but just initialize it to defaults.
        ConfigurePredictorAndFilterInternal(blink::features::kResamplingScrollEvents, "",
            /* enable_filtering = */ true, feature, filter_name);
        scroll_predictor_ = std::make_unique<ScrollPredictor>();
    }

    void ConfigurePredictorAndFilterFieldTrialAndInitialize(
        const base::Feature& pred_feature, const std::string& predictor_type, const base::Feature& filter_feature, const std::string& filter_type)
    {
        ConfigurePredictorAndFilterInternal(pred_feature, predictor_type,
            /* enable_filtering = */ true, filter_feature, filter_type);
        scroll_predictor_ = std::make_unique<ScrollPredictor>();
    }
    ...
    void ConfigurePredictorAndFilterInternal(const base::Feature& pred_feature, const std::string& predictor_type, bool enable_filtering,
        const base::Feature& filter_feature, const std::string& filter_type)
    {
        ...
        base::test::FeatureRefAndParams prediction_params = { pred_feature, pred_field_params };

        base::FieldTrialParams filter_field_params;
        filter_field_params["filter"] = filter_type;
        base::test::FeatureRefAndParams filter_params = { filter_feature, filter_field_params };

        enabled.emplace_back(base::test::FeatureRefAndParams(pred_feature, pred_field_params));
        if (enable_filtering) {
            enabled.emplace_back(base::test::FeatureRefAndParams(filter_feature, filter_field_params));
        } else {
        ...
        EXPECT_EQ(pred_field_params["predictor"], GetFieldTrialParamValueByFeature(pred_feature, "predictor"));
        if (enable_filtering) {
            EXPECT_EQ(filter_field_params["filter"], GetFieldTrialParamValueByFeature(filter_feature, "filter"));
        }
    }
    ...
    void InitLinearResamplingTest(bool use_frames_based_experimental_prediction)
    {
        ...
        base::test::FeatureRefAndParams prediction_params = { features::kResamplingScrollEvents, predictor_params };

        base::FieldTrialParams prediction_type_params;
        prediction_type_params["mode"]
            = use_frames_based_experimental_prediction ? ::features::kPredictionTypeFramesBased : ::features::kPredictionTypeTimeBased;
        base::test::FeatureRefAndParams experimental_prediction_params = { ::features::kResamplingScrollEventsExperimentalPrediction, prediction_type_params };

        base::FieldTrialParams filter_params;
        filter_params["filter"] = "";
        base::test::FeatureRefAndParams resampling_and_filter = { features::kFilteringScrollPrediction, filter_params };

        scoped_feature_list_.Reset();
        scoped_feature_list_.InitWithFeaturesAndParameters({ prediction_params, experimental_prediction_params, resampling_and_filter }, {});
        scroll_predictor_ = std::make_unique<ScrollPredictor>();
...
TEST_F(ScrollPredictorTest, ScrollResamplingStates)
{
    ...
    std::unique_ptr<WebInputEvent> gesture_update = CreateGestureScrollUpdate(0, 10, 10 /* ms */);
    HandleResampleScrollEvents(gesture_update, 15 /* ms */);
    EXPECT_TRUE(GetResamplingState());

    // after GSU with momentum phase
    gesture_update = CreateGestureScrollUpdate(0, 10, 10 /* ms */, WebGestureEvent::InertialPhaseState::kMomentum);
    HandleResampleScrollEvents(gesture_update, 15 /* ms */);
    EXPECT_FALSE(GetResamplingState());

    // after GSE
    std::unique_ptr<WebInputEvent> gesture_end = std::make_unique<WebGestureEvent>(
        WebInputEvent::Type::kGestureScrollEnd, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests(), WebGestureDevice::kTouchscreen);
...
{
    ConfigurePredictorFieldTrialAndInitialize(features::kResamplingScrollEvents, ::features::kPredictorNameEmpty);
    SendGestureScrollBegin();
    EXPECT_FALSE(PredictionAvailable());

    std::unique_ptr<WebInputEvent> gesture_update = CreateGestureScrollUpdate(0, -20);
    HandleResampleScrollEvents(gesture_update);
    EXPECT_EQ(-20, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    ...
    EXPECT_EQ(-60, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    HandleResampleScrollEvents(gesture_update);
    EXPECT_EQ(-60, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    ...
    HandleResampleScrollEvents(gesture_update);
    EXPECT_EQ(-35, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
...
TEST_F(ScrollPredictorTest, ScrollInDifferentDirection)
{
    ...
    HandleResampleScrollEvents(gesture_update);
    EXPECT_EQ(0, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_x);
    EXPECT_EQ(25, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    result = PredictionAvailable();
    EXPECT_TRUE(result);
    ...
    HandleResampleScrollEvents(gesture_update);
    EXPECT_EQ(25, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_x);
    EXPECT_EQ(0, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    result = PredictionAvailable();
    EXPECT_TRUE(result);
...
TEST_F(ScrollPredictorTest, ScrollUpdateWithEmptyOriginalEventList)
{
    ...
    HandleResampleScrollEvents(gesture_update);
    EXPECT_EQ(-30, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);

    // Send another GSU with empty original event list.
    gesture_update = CreateGestureScrollUpdate(0, -40);
    original_events_.clear();
    HandleResampleScrollEvents(gesture_update);
    EXPECT_EQ(-40, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
...
TEST_F(ScrollPredictorTest, LSQPredictorTest)
{
    ...
    std::unique_ptr<WebInputEvent> gesture_update = CreateGestureScrollUpdate(0, -30, 8 /* ms */);
    HandleResampleScrollEvents(gesture_update, 16 /* ms */);
    EXPECT_FALSE(PredictionAvailable(16 /* ms */));
    EXPECT_EQ(-30, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    EXPECT_EQ(
        WebInputEvent::GetStaticTimeStampForTests() + base::Milliseconds(8 /* ms */), static_cast<const WebGestureEvent*>(gesture_update.get())->TimeStamp());
    ...
    EXPECT_EQ(-30, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    EXPECT_EQ(
        WebInputEvent::GetStaticTimeStampForTests() + base::Milliseconds(16 /* ms */), static_cast<const WebGestureEvent*>(gesture_update.get())->TimeStamp());
    ...
    EXPECT_EQ(-60, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    EXPECT_EQ(
        WebInputEvent::GetStaticTimeStampForTests() + base::Milliseconds(32 /* ms */), static_cast<const WebGestureEvent*>(gesture_update.get())->TimeStamp());
    ...
    EXPECT_EQ(-30, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    EXPECT_EQ(
        WebInputEvent::GetStaticTimeStampForTests() + base::Milliseconds(40 /* ms */), static_cast<const WebGestureEvent*>(gesture_update.get())->TimeStamp());
...
TEST_F(ScrollPredictorTest, LinearResamplingPredictorTest)
{
    ...
    EXPECT_EQ(10, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    EXPECT_EQ(
        WebInputEvent::GetStaticTimeStampForTests() + base::Milliseconds(10 /* ms */), static_cast<const WebGestureEvent*>(gesture_update.get())->TimeStamp());
    ...
    ASSERT_FLOAT_EQ(10 + 3.3, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    EXPECT_EQ(WebInputEvent::GetStaticTimeStampForTests() + base::Milliseconds(23.3 /* ms */),
    ...
    ASSERT_FLOAT_EQ(10 - 5 + 8.333, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y);
    EXPECT_EQ(WebInputEvent::GetStaticTimeStampForTests() + base::Milliseconds(10 + 10 - 5 + 8.333 /* ms */),
        static_cast<const WebGestureEvent*>(gesture_update.get())->TimeStamp());
}
...
TEST_F(ScrollPredictorTest, ScrollPredictorNotChangeScrollDirection)
{
    ...
    EXPECT_NEAR(10, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y, kEpsilon);
    ...
    EXPECT_NEAR(-9, static_cast<const WebGestureEvent*>(gesture_update.get())->data.scroll_update.delta_y, kEpsilon);
...
TEST_F(ScrollPredictorTest, ScrollPredictorTypeSelection)
{
    ...
    ConfigurePredictorFieldTrialAndInitialize(features::kResamplingScrollEvents, ::features::kPredictorNameLsq);
    VerifyPredictorType(::features::kPredictorNameLsq);

    ConfigurePredictorFieldTrialAndInitialize(features::kResamplingScrollEvents, ::features::kPredictorNameKalman);
    VerifyPredictorType(::features::kPredictorNameKalman);

    ConfigurePredictorFieldTrialAndInitialize(features::kResamplingScrollEvents, ::features::kPredictorNameLinearFirst);
...
TEST_F(ScrollPredictorTest, DefaultFilter)
{
    ...
    ConfigureFilterFieldTrialAndInitialize(features::kFilteringScrollPrediction, ::features::kFilterNameEmpty);
    VerifyFilterType(::features::kFilterNameEmpty);
    EXPECT_TRUE(isFilteringEnabled());

    ConfigureFilterFieldTrialAndInitialize(features::kFilteringScrollPrediction, ::features::kFilterNameOneEuro);
...
TEST_F(ScrollPredictorTest, FilteringPrediction)
{
    ...
    ConfigurePredictorAndFilterFieldTrialAndInitialize(
        features::kResamplingScrollEvents, ::features::kPredictorNameKalman, features::kFilteringScrollPrediction, ::features::kFilterNameEmpty);
    scroll_predictor_ = std::make_unique<ScrollPredictor>();
