    ...
    : enable_resampling_(enable_resampling)
{
    ...
    // flag.
    std::string predictor_name = enable_resampling_ ? GetFieldTrialParamValueByFeature(blink::features::kResamplingInputEvents, "predictor")
                                                    : GetFieldTrialParamValueByFeature(blink::features::kInputPredictorTypeChoice, "predictor");

    if (predictor_name.empty()) {
        selected_predictor_type_ = input_prediction::PredictorType::kScrollPredictorTypeKalman;
    } else {
        selected_predictor_type_ = PredictorFactory::GetPredictorTypeFromName(predictor_name);
    }
...
void InputEventPrediction::HandleEvents(blink::WebCoalescedInputEvent& coalesced_event, base::TimeTicks frame_time)
{
...
std::unique_ptr<ui::InputPredictor> InputEventPrediction::CreatePredictor() const
{
...
void InputEventPrediction::UpdatePrediction(const WebInputEvent& event)
{
    if (WebInputEvent::IsTouchEventType(event.GetType())) {
        ...
        for (unsigned i = 0; i < touch_event.touches_length; ++i) {
            if (touch_event.touches[i].state == blink::WebTouchPoint::State::kStateMoved) {
                UpdateSinglePointer(touch_event.touches[i], touch_event.TimeStamp());
            }
        }
    } else {
        DCHECK(event.GetType() == WebInputEvent::Type::kMouseMove || event.GetType() == WebInputEvent::Type::kPointerMove);
        UpdateSinglePointer(*ToWebPointerProperties(&event), event.TimeStamp());
    }
    last_event_timestamp_ = event.TimeStamp();
}

void InputEventPrediction::ApplyResampling(base::TimeTicks frame_time, WebInputEvent* event)
{
...
void InputEventPrediction::ResetPredictor(const WebInputEvent& event)
{
    if (WebInputEvent::IsTouchEventType(event.GetType())) {
        ...
        for (unsigned i = 0; i < touch_event.touches_length; ++i) {
            if (touch_event.touches[i].state != blink::WebTouchPoint::State::kStateMoved
                && touch_event.touches[i].state != blink::WebTouchPoint::State::kStateStationary)
...
void InputEventPrediction::AddPredictedEvents(blink::WebCoalescedInputEvent& coalesced_event)
{
    base::TimeTicks predict_time = last_event_timestamp_;
    base::TimeTicks max_prediction_timestamp = last_event_timestamp_ + mouse_predictor_->MaxPredictionTime();
    bool success = true;
    while (success) {
        std::unique_ptr<WebInputEvent> predicted_event = coalesced_event.Event().Clone();
        success = false;
        if (predicted_event->GetType() == WebInputEvent::Type::kTouchMove) {
            ...
            for (unsigned i = 0; i < touch_event.touches_length; ++i) {
                touch_time_interval += GetPredictionTimeInterval(touch_event.touches[i]);
            }
            predict_time += touch_time_interval / touch_event.touches_length;
            if (predict_time <= max_prediction_timestamp) {
                for (unsigned i = 0; i < touch_event.touches_length; ++i) {
                    if (touch_event.touches[i].state == blink::WebTouchPoint::State::kStateMoved) {
                        success = GetPointerPrediction(predict_time, &touch_event.touches[i]);
                    }
                }
            }
        } else {
            WebPointerProperties* pointer_event = ToWebPointerProperties(predicted_event.get());
            predict_time += GetPredictionTimeInterval(*pointer_event);
            success = predict_time <= max_prediction_timestamp && GetPointerPrediction(predict_time, pointer_event);
        }
        if (success) {
...
ui::InputPredictor* InputEventPrediction::GetPredictor(const WebPointerProperties& event) const
{
...
base::TimeDelta InputEventPrediction::GetPredictionTimeInterval(const WebPointerProperties& event) const
{
...
void InputEventPrediction::UpdateSinglePointer(const WebPointerProperties& event, base::TimeTicks event_time)
{
...
bool InputEventPrediction::GetPointerPrediction(base::TimeTicks predict_time, WebPointerProperties* event)
{
...
void InputEventPrediction::ResetSinglePredictor(const WebPointerProperties& event)
{
    if (event.pointer_type == WebPointerProperties::PointerType::kMouse)
        mouse_predictor_->Reset();
    else
