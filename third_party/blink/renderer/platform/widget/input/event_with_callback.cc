...
EventWithCallback::EventWithCallback(
    std::unique_ptr<WebCoalescedInputEvent> event, InputHandlerProxy::EventDispositionCallback callback, std::unique_ptr<cc::EventMetrics> metrics)
    : event_(std::make_unique<WebCoalescedInputEvent>(*event))
{
    original_events_.emplace_back(std::move(event), std::move(metrics), std::move(callback));
}

EventWithCallback::EventWithCallback(std::unique_ptr<WebCoalescedInputEvent> event, OriginalEventList original_events)
...
void EventWithCallback::SetScrollbarManipulationHandledOnCompositorThread()
{
    for (auto& original_event : original_events_) {
        original_event.event_->EventPointer()->SetScrollbarManipulationHandledOnCompositorThread();
    }
}
...
void EventWithCallback::CoalesceWith(EventWithCallback* other)
{
    ...
    auto* metrics = original_events_.empty() ? nullptr : original_events_.front().metrics_.get();
    auto* scroll_update_metrics = metrics ? metrics->AsScrollUpdate() : nullptr;
    auto* other_metrics = other->original_events_.empty() ? nullptr : other->original_events_.front().metrics_.get();
    auto* other_scroll_update_metrics = other_metrics ? other_metrics->AsScrollUpdate() : nullptr;
...
static bool HandledOnCompositorThread(InputHandlerProxy::EventDisposition disposition)
{
    return (disposition != InputHandlerProxy::DID_NOT_HANDLE && disposition != InputHandlerProxy::DID_NOT_HANDLE_NON_BLOCKING_DUE_TO_FLING
        && disposition != InputHandlerProxy::DID_NOT_HANDLE_NON_BLOCKING);
}

void EventWithCallback::RunCallbacks(InputHandlerProxy::EventDisposition disposition, const ui::LatencyInfo& latency,
    std::unique_ptr<InputHandlerProxy::DidOverscrollParams> did_overscroll_params, const WebInputEventAttribution& attribution)
{
    ...
    std::move(oldest_event.callback_)
        .Run(disposition, std::move(oldest_event.event_),
            did_overscroll_params ? std::make_unique<InputHandlerProxy::DidOverscrollParams>(*did_overscroll_params) : nullptr, attribution,
            std::move(oldest_event.metrics_));
    ...
    for (auto& coalesced_event : original_events_) {
        ...
        std::move(coalesced_event.callback_)
            .Run(disposition, std::move(coalesced_event.event_),
                did_overscroll_params ? std::make_unique<InputHandlerProxy::DidOverscrollParams>(*did_overscroll_params) : nullptr, attribution,
                std::move(coalesced_event.metrics_));
    }
}
...
void EventWithCallback::WillStartProcessingForMetrics()
{
    ...
    for (auto& original_event : original_events_) {
        if (original_event.metrics_) {
            original_event.metrics_->SetDispatchStageTimestamp(cc::EventMetrics::DispatchStage::kRendererCompositorStarted);
        }
    }
}
...
void EventWithCallback::DidCompleteProcessingForMetrics()
{
    ...
    for (auto& original_event : original_events_) {
        if (original_event.metrics_) {
            original_event.metrics_->SetDispatchStageTimestamp(cc::EventMetrics::DispatchStage::kRendererCompositorFinished);
        }
    }
}

EventWithCallback::OriginalEventWithCallback::OriginalEventWithCallback(
    std::unique_ptr<WebCoalescedInputEvent> event, std::unique_ptr<cc::EventMetrics> metrics, InputHandlerProxy::EventDispositionCallback callback)
    : event_(std::move(event))
...
EventWithCallback::OriginalEventWithCallback::~OriginalEventWithCallback() = default;
