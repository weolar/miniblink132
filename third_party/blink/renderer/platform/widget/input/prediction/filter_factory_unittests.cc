...
public:
    FilterFactoryTest()
    {
        CreateNewFactory(blink::features::kFilteringScrollPrediction, PredictorType::kScrollPredictorTypeKalman, FilterType::kEmpty);
    }
    FilterFactoryTest(const FilterFactoryTest&) = delete;
    FilterFactoryTest& operator=(const FilterFactoryTest&) = delete;

    void GetFilterParams(const FilterType& filter_type, const PredictorType& predictor_type, FilterParams* filter_params)
    {
    ...
    void CreateNewFactory(const base::Feature& feature, const input_prediction::PredictorType predictor_type, const input_prediction::FilterType filter_type)
    {
        factory_ = std::make_unique<FilterFactory>(feature, predictor_type, filter_type);
    }
...
{
    EXPECT_EQ(input_prediction::FilterType::kEmpty, GetFilterTypeFromName(::features::kFilterNameEmpty));

    EXPECT_EQ(input_prediction::FilterType::kOneEuro, GetFilterTypeFromName(::features::kFilterNameOneEuro));
...
{
    CreateNewFactory(
        blink::features::kFilteringScrollPrediction, input_prediction::PredictorType::kScrollPredictorTypeEmpty, input_prediction::FilterType::kEmpty);
    ...
    CreateNewFactory(
        blink::features::kFilteringScrollPrediction, input_prediction::PredictorType::kScrollPredictorTypeEmpty, input_prediction::FilterType::kOneEuro);
...
TEST_F(FilterFactoryTest, TestOneEuroNoParams)
{
    ...
    GetFilterParams(FilterType::kOneEuro, PredictorType::kScrollPredictorTypeKalman, &filter_params);
...
TEST_F(FilterFactoryTest, TestOneEuroParams)
{
    ...
    scoped_feature_list.InitAndEnableFeatureWithParameters(blink::features::kFilteringScrollPrediction, field_trial_params);

    // Create a new factory to load fieldtrials params values
    CreateNewFactory(blink::features::kFilteringScrollPrediction, PredictorType::kScrollPredictorTypeKalman, FilterType::kOneEuro);
    ...
    filter_params.clear();
    GetFilterParams(FilterType::kOneEuro, PredictorType::kScrollPredictorTypeLsq, &filter_params);
