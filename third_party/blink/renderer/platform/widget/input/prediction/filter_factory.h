...
struct FilterParamMapKey {
    ...
    {
        return filter_type == other.filter_type && predictor_type == other.predictor_type;
    }
...
struct FilterParamMapKeyHash {
    ...
    {
        return std::hash<input_prediction::FilterType> {}(k.filter_type) ^ std::hash<input_prediction::PredictorType> {}(k.predictor_type);
    }
};

using FilterParams = std::unordered_map<std::string, double>;
using FilterParamsMap ALLOW_DISCOURAGED_TYPE("TODO(crbug.com/1404327)") = std::unordered_map<FilterParamMapKey, FilterParams, FilterParamMapKeyHash>;
...
public:
    FilterFactory(const base::Feature& feature, const input_prediction::PredictorType predictor_type, const input_prediction::FilterType filter_type);
    ~FilterFactory();

    // Returns the FilterType associated to the given filter
    // name if found, otherwise returns kFilterTypeEmpty
    static input_prediction::FilterType GetFilterTypeFromName(const std::string& filter_name);
...
private:
    ...
    // the future.
    void LoadFilterParams(const base::Feature& feature, const input_prediction::PredictorType predictor_type, const input_prediction::FilterType filter_type);
    ...
    void GetFilterParams(const input_prediction::FilterType filter_type, const input_prediction::PredictorType predictor_type, FilterParams* filter_params);
};
} // namespace blink
