...
// type of |event|.
void SetScrollOrPinchTraceId(EventWithCallback* event, int64_t* oldest_scroll_trace_id, int64_t* oldest_pinch_trace_id)
{
...
CompositorThreadEventQueue::~CompositorThreadEventQueue()
{
    while (!queue_.empty()) {
        ...
        event_with_callback->RunCallbacks(InputHandlerProxy::DROP_EVENT, event_with_callback->latency_info(),
            /*did_overscroll_params=*/nullptr,
            /*attribution=*/WebInputEventAttribution());
    }
}

void CompositorThreadEventQueue::Queue(std::unique_ptr<EventWithCallback> new_event)
{
    if (queue_.empty() || !IsContinuousGestureEvent(new_event->event().GetType())
        || !(queue_.back()->CanCoalesceWith(*new_event)
            || WebGestureEvent::IsCompatibleScrollorPinch(ToWebGestureEvent(new_event->event()), ToWebGestureEvent(queue_.back()->event())))) {
        if (new_event->first_original_event()) {
            ...
            TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("input", "CompositorThreadEventQueue::Queue", new_event->first_original_event());
        }
    ...
    SetScrollOrPinchTraceId(last_event.get(), &oldest_scroll_trace_id, &oldest_pinch_trace_id);
    oldest_latency = last_event->latency_info();
    EventWithCallback::OriginalEventList combined_original_events;
    combined_original_events.splice(combined_original_events.end(), last_event->original_events());
    combined_original_events.splice(combined_original_events.end(), new_event->original_events());
    ...
    if (!queue_.empty() && WebGestureEvent::IsCompatibleScrollorPinch(ToWebGestureEvent(new_event->event()), ToWebGestureEvent(queue_.back()->event()))) {
        second_last_event = std::move(queue_.back());
        queue_.pop_back();
        SetScrollOrPinchTraceId(second_last_event.get(), &oldest_scroll_trace_id, &oldest_pinch_trace_id);
        oldest_latency = second_last_event->latency_info();
        combined_original_events.splice(combined_original_events.begin(), second_last_event->original_events());
    }
    ...
    TRACE_EVENT2("input", "CoalesceScrollAndPinch", "coalescedTraceId", new_event->latency_info().trace_id(), "traceId",
        scroll_latency.trace_id() != -1 ? scroll_latency.trace_id() : pinch_latency.trace_id());
    std::pair<std::unique_ptr<WebGestureEvent>, std::unique_ptr<WebGestureEvent>> coalesced_events
        = WebGestureEvent::CoalesceScrollAndPinch(second_last_event ? &ToWebGestureEvent(second_last_event->event()) : nullptr,
            ToWebGestureEvent(last_event->event()), ToWebGestureEvent(new_event->event()));
    DCHECK(coalesced_events.first);
    DCHECK(coalesced_events.second);

    auto scroll_event = std::make_unique<EventWithCallback>(
        std::make_unique<WebCoalescedInputEvent>(std::move(coalesced_events.first), scroll_latency), std::move(scroll_original_events));
    scroll_event->set_coalesced_scroll_and_pinch();

    auto pinch_event = std::make_unique<EventWithCallback>(
        std::make_unique<WebCoalescedInputEvent>(std::move(coalesced_events.second), pinch_latency), std::move(pinch_original_events));
...
std::unique_ptr<EventWithCallback> CompositorThreadEventQueue::Pop()
{
    ...
    if (result->first_original_event()) {
        TRACE_EVENT_NESTABLE_ASYNC_END2("input", "CompositorThreadEventQueue::Queue", result->first_original_event(), "type", result->event().GetType(),
...
{
    return empty() ? WebInputEvent::Type::kUndefined : queue_.front()->event().GetType();
}
