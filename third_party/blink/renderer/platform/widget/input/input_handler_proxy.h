...
public:
    ...
    // scale state.
    virtual void UpdateRootLayerState(const gfx::PointF& total_scroll_offset, const gfx::PointF& max_scroll_offset, const gfx::SizeF& scrollable_size,
        float page_scale_factor, float min_page_scale_factor, float max_page_scale_factor)
        = 0;
};
...
// animations.
class PLATFORM_EXPORT InputHandlerProxy : public cc::InputHandlerClient, public cc::SnapFlingClient {
public:
    InputHandlerProxy(cc::InputHandler& input_handler, InputHandlerProxyClient* client);
    ...
    using EventDispositionCallback = base::OnceCallback<void(EventDisposition, std::unique_ptr<blink::WebCoalescedInputEvent> event,
        std::unique_ptr<DidOverscrollParams>, const blink::WebInputEventAttribution&, std::unique_ptr<cc::EventMetrics> metrics)>;
    ...
    // method.
    void ContinueScrollBeginAfterMainThreadHitTest(std::unique_ptr<blink::WebCoalescedInputEvent> event, std::unique_ptr<cc::EventMetrics> metrics,
        EventDispositionCallback callback, cc::ElementId hit_tests_result);
    ...
    void InjectScrollbarGestureScroll(const blink::WebInputEvent::Type type, const gfx::PointF& position_in_widget,
        const cc::InputHandlerPointerResult& pointer_result, const ui::LatencyInfo& latency_info, const base::TimeTicks now,
    ...
    blink::WebInputEventAttribution PerformEventAttribution(const blink::WebInputEvent& event);

    // SynchronousInputHandler needs to be informed of root layer updates.
    void SetSynchronousInputHandler(SynchronousInputHandler* synchronous_input_handler);
    ...
    // from what was sent.
    void SynchronouslySetRootScrollOffset(const gfx::PointF& root_offset);
    ...
    void SynchronouslyZoomBy(float magnify_delta, const gfx::Point& anchor);
    ...
    void SetScrollEventDispatchMode(cc::InputHandlerClient::ScrollEventDispatchMode mode) override;

    // SnapFlingClient implementation.
    bool GetSnapFlingInfoAndSetAnimatingSnapTarget(const gfx::Vector2dF& current_delta, const gfx::Vector2dF& natural_displacement, gfx::PointF* initial_offset,
...
private:
    ...
    EventDisposition HandleGestureScrollUpdate(
        const blink::WebGestureEvent& event, const blink::WebInputEventAttribution& original_attribution, cc::EventMetrics* metrics, int64_t trace_id);
    ...
    const cc::InputHandlerPointerResult HandlePointerDown(EventWithCallback* event_with_callback, const gfx::PointF& position);
    ...
    const cc::InputHandlerPointerResult HandlePointerUp(EventWithCallback* event_with_callback, const gfx::PointF& position);
    ...
    void HandleOverscroll(const gfx::PointF& causal_event_viewport_point, const cc::InputHandlerScrollResult& scroll_result);

    // Update the elastic overscroll controller with |gesture_event|.
    void HandleScrollElasticityOverscroll(const blink::WebGestureEvent& gesture_event, const cc::InputHandlerScrollResult& scroll_result);
    ...
    EventDisposition HitTestTouchEvent(const blink::WebTouchEvent& touch_event, bool* is_touching_scrolling_layer, cc::TouchAction* allowed_touch_action);

    EventDisposition RouteToTypeSpecificHandler(EventWithCallback* event_with_callback, const blink::WebInputEventAttribution& original_attribution);
    ...
    void RecordScrollBegin(blink::WebGestureDevice device, uint32_t main_thread_hit_tested_reasons, uint32_t main_thread_repaint_reasons);
    ...
    void GenerateAndDispatchSytheticScrollPrediction(const viz::BeginFrameArgs& args);
    ...
    // unification.
    uint32_t scroll_begin_main_thread_hit_test_reasons_ = cc::MainThreadScrollingReason::kNotScrollingOnMain;
    ...
    cc::InputHandlerClient::ScrollEventDispatchMode scroll_event_dispatch_mode_ = cc::InputHandlerClient::ScrollEventDispatchMode::kEnqueueScrollEvents;
};
