...
public:
    ElasticOverscrollControllerBezierTest()
        : controller_(&helper_)
    {
    }
    ...
    {
        WebGestureEvent event(WebInputEvent::Type::kGestureScrollBegin, WebInputEvent::kNoModifiers, base::TimeTicks(), WebGestureDevice::kTouchpad);
    ...
    void SendGestureScrollUpdate(PhaseState inertialPhase, const Vector2dF& unused_scroll_delta)
    {
        blink::WebGestureEvent event(
            WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers, base::TimeTicks(), blink::WebGestureDevice::kTouchpad);
    ...
    {
        WebGestureEvent event(WebInputEvent::Type::kGestureScrollEnd, WebInputEvent::kNoModifiers, time, WebGestureDevice::kTouchpad);
...
{
    // Test vertical overscroll.
    WebGestureEvent event(WebInputEvent::Type::kGestureScrollBegin, WebInputEvent::kNoModifiers, base::TimeTicks(), WebGestureDevice::kScrollbar);
    event.data.scroll_begin.inertial_phase = PhaseState::kNonMomentum;
    event.data.scroll_begin.synthetic = true;
    controller_.ObserveGestureEventAndResult(event, cc::InputHandlerScrollResult());
    EXPECT_EQ(controller_.state_, ElasticOverscrollController::State::kStateInactive);
}
...
TEST_F(ElasticOverscrollControllerBezierTest, ReconcileStretchAndScroll)
{
    // Test vertical overscroll.
    SendGestureScrollBegin(PhaseState::kNonMomentum);
    helper_.SetScrollOffsetAndMaxScrollOffset(gfx::PointF(0, 0), gfx::PointF(100, 100));
    SendGestureScrollUpdate(PhaseState::kNonMomentum, Vector2dF(0, -100));
    EXPECT_EQ(Vector2dF(0, -19), helper_.StretchAmount());
...
TEST_F(ElasticOverscrollControllerBezierTest, VerifyInitialStretchDelta)
{
    ...
    // diagonal stretch.
    controller_.state_ = ElasticOverscrollController::State::kStateMomentumAnimated;
    helper_.SetStretchAmount(Vector2dF(5, 10));
    helper_.SetScrollOffsetAndMaxScrollOffset(gfx::PointF(0, 20), gfx::PointF(100, 100));
    controller_.ReconcileStretchAndScroll();
    ...
    // animation ticks.
    const base::TimeTicks animation_start_time = base::TimeTicks() + base::Milliseconds(32);
    ...
    EXPECT_EQ(controller_.state_, ElasticOverscrollController::State::kStateMomentumAnimated);
...
{
    Vector2dF overscroll_bounce_distance(controller_.StretchAmountForAccumulatedOverscroll(Vector2dF(0, -100)));
    EXPECT_EQ(overscroll_bounce_distance.y(), -19);

    overscroll_bounce_distance = controller_.StretchAmountForAccumulatedOverscroll(Vector2dF(-100, 0));
...
TEST_F(ElasticOverscrollControllerBezierTest, VerifyBackwardAnimationTick)
{
    ...
    EXPECT_EQ(controller_.state_, ElasticOverscrollController::kStateMomentumAnimated);
...
TEST_F(ElasticOverscrollControllerBezierTest, VerifyForwardAnimationTick)
{
    ...
    const int stretch_amount_y[TOTAL_FRAMES]
        = { -19, -41, -55, -65, -72, -78, -82, -85, -88, -89, -78, -64, -53, -44, -37, -30, -25, -20, -16, -13, -10, -7, -5, -4, -2, -1, -1, 0 };

    for (int i = 0; i < TOTAL_FRAMES; i++) {
        controller_.Animate(now + base::Milliseconds(i * 16));
        EXPECT_EQ(
            controller_.state_, (stretch_amount_y[i] == 0 ? ElasticOverscrollController::kStateInactive : ElasticOverscrollController::kStateMomentumAnimated));
    ...
    const int stretch_amount_x[TOTAL_FRAMES]
        = { -9, -24, -34, -42, -48, -54, -58, -62, -66, -69, -62, -52, -43, -36, -30, -25, -20, -17, -13, -10, -8, -6, -4, -3, -2, -1, 0, 0 };

    for (int i = 0; i < TOTAL_FRAMES; i++) {
        controller_.Animate(now + base::Milliseconds(i * 16));
        EXPECT_EQ(
            controller_.state_, (stretch_amount_x[i] == 0 ? ElasticOverscrollController::kStateInactive : ElasticOverscrollController::kStateMomentumAnimated));
...
TEST_F(ElasticOverscrollControllerBezierTest, VerifyForwardAnimationIsNotPlayed)
{
    EXPECT_EQ(Vector2dF(), helper_.StretchAmount());
    ...
    controller_.DidEnterMomentumAnimatedState();
    EXPECT_EQ(gfx::Vector2dF(0, -16), gfx::ToRoundedVector2d(controller_.bounce_forwards_distance_));

    controller_.scroll_velocity_ = gfx::Vector2dF(-201.f, 0.f);
    controller_.DidEnterMomentumAnimatedState();
    EXPECT_EQ(gfx::Vector2dF(-16, 0), gfx::ToRoundedVector2d(controller_.bounce_forwards_distance_));
}
...
TEST_F(ElasticOverscrollControllerBezierTest, VerifyTwoAxisForwardAnimation)
{
    ...
    enum AnimationState { kBouncingForwardBoth, kBouncingForwardY, kBouncingBackwards };
    AnimationState state(kBouncingForwardBoth);
    for (int i = 1; i < kMaxFrames && (x_stretch_amount > 0 && y_stretch_amount > 0); i++) {
        controller_.Animate(now + base::Milliseconds(i * 16));
        ...
        if (state == kBouncingForwardBoth && new_x_stretch_amount == x_stretch_amount) {
            EXPECT_NE(new_x_stretch_amount, 0);
            state = kBouncingForwardY;
        }
        if (state == kBouncingForwardY && new_y_stretch_amount <= y_stretch_amount) {
