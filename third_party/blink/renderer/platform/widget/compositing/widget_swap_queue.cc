...
void WidgetSwapQueue::Queue(int source_frame_number, VisualStateRequestCallback callback, bool* is_first)
{
...
void WidgetSwapQueue::Drain(int source_frame_number)
{
    ...
    for (auto i = queue_.begin(); i != end; i++) {
        ...
        std::move(i->second.begin(), i->second.end(), std::back_inserter(next_callbacks_));
    }
    queue_.erase(queue_.begin(), end);
}

void WidgetSwapQueue::GetCallbacks(Vector<VisualStateRequestCallback>* callbacks)
{
    std::move(next_callbacks_.begin(), next_callbacks_.end(), std::back_inserter(*callbacks));
