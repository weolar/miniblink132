...
class PLATFORM_EXPORT LayerTreeView : public cc::LayerTreeHostClient, public cc::LayerTreeHostSingleThreadClient, public cc::LayerTreeHostSchedulingClient {
public:
    LayerTreeView(LayerTreeViewDelegate* delegate, scoped_refptr<scheduler::WidgetScheduler> scheduler);
    ...
    // only).
    void Initialize(const cc::LayerTreeSettings& settings, scoped_refptr<base::SingleThreadTaskRunner> main_thread,
    ...
    void ClearPreviousDelegateAndReattachIfNeeded(LayerTreeViewDelegate* delegate, scoped_refptr<scheduler::WidgetScheduler> scheduler);
    ...
    void DidCommit(int source_frame_number, base::TimeTicks commit_start_time, base::TimeTicks commit_finish_time) override;
...
private:
    ...
    void AddCallback(uint32_t frame_token, Callback callback, base::circular_deque<std::pair<uint32_t, std::vector<Callback>>>& callbacks);
    ...
    enum class FrameSinkState { kNoFrameSink, kRequestBufferedInvisible, kRequestPending, kInitializing, kInitialized };
    FrameSinkState frame_sink_state_ = FrameSinkState::kNoFrameSink;

    base::circular_deque<std::pair<uint32_t, std::vector<base::OnceCallback<void(const viz::FrameTimingDetails&)>>>> presentation_callbacks_;

#if BUILDFLAG(IS_APPLE)
    base::circular_deque<std::pair<uint32_t, std::vector<base::OnceCallback<void(gfx::CALayerResult error_code)>>>> core_animation_error_code_callbacks_;
#endif
