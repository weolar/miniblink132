...
LayerTreeView::LayerTreeView(LayerTreeViewDelegate* delegate, scoped_refptr<scheduler::WidgetScheduler> scheduler)
...
void LayerTreeView::Initialize(const cc::LayerTreeSettings& settings, scoped_refptr<base::SingleThreadTaskRunner> main_thread,
    scoped_refptr<base::SingleThreadTaskRunner> compositor_thread, cc::TaskGraphRunner* task_graph_runner)
{
    DCHECK(delegate_);
...
//     params.dark_mode_filter = nullptr;// &RasterDarkModeFilterImpl::Instance(); // weolar
//     params.ukm_recorder_factory = std::make_unique<UkmRecorderFactoryImpl>();
    ...
    if (base::ThreadPoolInstance::Get()) {
        ...
        // IO thread.
        params.image_worker_task_runner = base::ThreadPool::CreateSequencedTaskRunner(
            { base::WithBaseSyncPrimitives(), base::TaskPriority::USER_VISIBLE, base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN });
    }
    if (!is_threaded) {
        // Single-threaded web tests, and unit tests.
        layer_tree_host_ = cc::LayerTreeHost::CreateSingleThreaded(this, std::move(params));
    } else {
        layer_tree_host_ = cc::LayerTreeHost::CreateThreaded(std::move(compositor_thread), std::move(params));
    }

    first_source_frame_for_current_delegate_ = layer_tree_host_->SourceFrameNumber();
}
...
void LayerTreeView::ClearPreviousDelegateAndReattachIfNeeded(LayerTreeViewDelegate* delegate, scoped_refptr<scheduler::WidgetScheduler> scheduler)
{
    ...
    layer_tree_host_->StopDeferringCommits(cc::PaintHoldingCommitTrigger::kWidgetSwapped);
    for (uint32_t i = 0; i <= static_cast<uint32_t>(cc::EventListenerClass::kLast); ++i) {
        layer_tree_host_->SetEventListenerProperties(static_cast<cc::EventListenerClass>(i), cc::EventListenerProperties::kNone);
    }

    delegate_ = delegate;
    CHECK_GE(layer_tree_host_->SourceFrameNumber(), first_source_frame_for_current_delegate_) << "SourceFrameNumber() must be monotonically increasing";
    first_source_frame_for_current_delegate_ = layer_tree_host_->SourceFrameNumber();
    widget_scheduler_ = std::move(scheduler);
    ...
    case FrameSinkState::kInitialized:
        ...
        if (auto render_frame_observer = delegate_->CreateRenderFrameObserver()) {
            layer_tree_host_->SetRenderFrameObserver(std::move(render_frame_observer));
        }
        break;
    }
}
...
void LayerTreeView::SetVisible(bool visible)
{
    ...
    if (visible && frame_sink_state_ == FrameSinkState::kRequestBufferedInvisible) {
        DidFailToInitializeLayerTreeFrameSink();
    }
}
...
void LayerTreeView::SetLayerTreeFrameSink(
    std::unique_ptr<cc::LayerTreeFrameSink> layer_tree_frame_sink, std::unique_ptr<cc::RenderFrameMetadataObserver> render_frame_metadata_observer)
{
    DCHECK(delegate_);
    ...
    if (render_frame_metadata_observer) {
        layer_tree_host_->SetRenderFrameObserver(std::move(render_frame_metadata_observer));
    }
...
void LayerTreeView::OnDeferCommitsChanged(bool status, cc::PaintHoldingReason reason, std::optional<cc::PaintHoldingCommitTrigger> trigger)
{
    if (!delegate_)
        return;
...
void LayerTreeView::ApplyViewportChanges(const cc::ApplyViewportChangesArgs& args)
{
    if (!delegate_)
        return;
...
void LayerTreeView::UpdateCompositorScrollState(const cc::CompositorCommitData& commit_data)
{
    if (!delegate_)
        return;
...
void LayerTreeView::RequestNewLayerTreeFrameSink()
{
    ...
    CHECK(frame_sink_state_ == FrameSinkState::kNoFrameSink || frame_sink_state_ == FrameSinkState::kInitialized);
    ...
    delegate_->RequestNewLayerTreeFrameSink(base::BindOnce(&LayerTreeView::SetLayerTreeFrameSink, weak_factory_for_delegate_.GetWeakPtr()));
}
...
void LayerTreeView::DidFailToInitializeLayerTreeFrameSink()
{
    ...
    CHECK(frame_sink_state_ == FrameSinkState::kRequestBufferedInvisible || frame_sink_state_ == FrameSinkState::kInitializing
        || frame_sink_state_ == FrameSinkState::kRequestPending);
    ...
    if (!Platform::Current()->IsGpuRemoteDisconnected()) {
        layer_tree_host_->GetTaskRunnerProvider()->MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeView::RequestNewLayerTreeFrameSink, weak_factory_.GetWeakPtr()));
    }
}
...
void LayerTreeView::DidCommit(int source_frame_number, base::TimeTicks commit_start_time, base::TimeTicks commit_finish_time)
{
    if (!delegate_ || source_frame_number < first_source_frame_for_current_delegate_) {
        return;
    }
...
void LayerTreeView::DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& frame_timing_details)
{
    if (!delegate_)
        return;
    DCHECK(layer_tree_host_->GetTaskRunnerProvider()->MainThreadTaskRunner()->RunsTasksInCurrentSequence());
    ...
    while (!core_animation_error_code_callbacks_.empty()) {
        ...
        if (viz::FrameTokenGT(front->first, frame_token))
            break;
        for (auto& callback : front->second) {
            std::move(callback).Run(frame_timing_details.presentation_feedback.ca_layer_error_code);
        }
...
void LayerTreeView::RecordEndOfFrameMetrics(base::TimeTicks frame_begin_time, cc::ActiveFrameSequenceTrackers trackers)
{
    if (!delegate_)
        return;
...
std::unique_ptr<cc::BeginMainFrameMetrics> LayerTreeView::GetBeginMainFrameMetrics()
{
    if (!delegate_)
        return nullptr;
...
void LayerTreeView::NotifyThroughputTrackerResults(cc::CustomTrackerResults results)
{
    NOTREACHED();
}

void LayerTreeView::DidObserveFirstScrollDelay(int source_frame_number, base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp)
{
    if (!delegate_ || source_frame_number < first_source_frame_for_current_delegate_) {
        return;
    }
    delegate_->DidObserveFirstScrollDelay(first_scroll_delay, first_scroll_timestamp);
}

void LayerTreeView::RunPaintBenchmark(int repeat_count, cc::PaintBenchmarkResult& result)
{
    if (delegate_)
...
{
    return Platform::Current()->QueryLocalizedString(IDS_DEBUGGER_PAUSED_IN_ANOTHER_TAB).Utf8();
}
...
void LayerTreeView::AddPresentationCallback(uint32_t frame_token, base::OnceCallback<void(const viz::FrameTimingDetails&)> callback)
{
    AddCallback(frame_token, std::move(callback), presentation_callbacks_);
}

#if BUILDFLAG(IS_APPLE)
void LayerTreeView::AddCoreAnimationErrorCodeCallback(uint32_t frame_token, base::OnceCallback<void(gfx::CALayerResult)> callback)
{
    AddCallback(frame_token, std::move(callback), core_animation_error_code_callbacks_);
}
#endif

template <typename Callback>
void LayerTreeView::AddCallback(uint32_t frame_token, Callback callback, base::circular_deque<std::pair<uint32_t, std::vector<Callback>>>& callbacks)
{
    DCHECK(delegate_);
    if (!callbacks.empty()) {
    ...
    DCHECK_LE(callbacks.size(), cc::PresentationTimeCallbackBuffer::kMaxBufferSize);
}
