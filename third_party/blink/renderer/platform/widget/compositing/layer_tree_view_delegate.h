...
public:
    using LayerTreeFrameSinkCallback = base::OnceCallback<void(std::unique_ptr<cc::LayerTreeFrameSink>, std::unique_ptr<cc::RenderFrameMetadataObserver>)>;

    // Report viewport related properties during a commit from the compositor
    // thread.
    virtual void ApplyViewportChanges(const cc::ApplyViewportChangesArgs& args) = 0;

    virtual void UpdateCompositorScrollState(const cc::CompositorCommitData& commit_data) = 0;
    ...
    virtual void OnDeferCommitsChanged(bool defer_status, cc::PaintHoldingReason reason, std::optional<cc::PaintHoldingCommitTrigger> trigger) = 0;
    ...
    virtual void RequestNewLayerTreeFrameSink(LayerTreeFrameSinkCallback callback) = 0;

    // Notifies that the draw commands for a committed frame have been issued.
    virtual void DidCommitAndDrawCompositorFrame() = 0;

    virtual void DidObserveFirstScrollDelay(base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp) = 0;
    ...
    // the commit.
    virtual void DidCommitCompositorFrame(base::TimeTicks commit_start_time, base::TimeTicks commit_finish_time) = 0;
    ...
    virtual void RecordEndOfFrameMetrics(base::TimeTicks frame_begin_time, cc::ActiveFrameSequenceTrackers trackers) = 0;
    ...
    virtual std::unique_ptr<cc::BeginMainFrameMetrics> GetBeginMainFrameMetrics() = 0;
    ...
    virtual void RunPaintBenchmark(int repeat_count, cc::PaintBenchmarkResult& result) = 0;
    ...
    virtual std::unique_ptr<cc::RenderFrameMetadataObserver> CreateRenderFrameObserver() = 0;
