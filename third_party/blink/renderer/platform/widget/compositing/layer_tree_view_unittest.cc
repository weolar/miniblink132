...
public:
    ...
    FakeLayerTreeViewDelegate& operator=(const FakeLayerTreeViewDelegate&) = delete;
    ...
    void RequestNewLayerTreeFrameSink(LayerTreeFrameSinkCallback callback) override
    {
        ...
        if (num_failures_since_last_success_ < num_failures_before_success_) {
            context_provider->UnboundTestContextGL()->LoseContextCHROMIUM(GL_GUILTY_CONTEXT_RESET_ARB, GL_INNOCENT_CONTEXT_RESET_ARB);
        }
        std::move(callback).Run(cc::FakeLayerTreeFrameSink::Create3d(std::move(context_provider)), nullptr);
    }
...
public:
    LayerTreeViewWithFrameSinkTracking(FakeLayerTreeViewDelegate* delegate, PageScheduler& scheduler)
        : LayerTreeView(delegate, scheduler.CreateWidgetScheduler())
        , delegate_(delegate)
    {
    }
    LayerTreeViewWithFrameSinkTracking(const LayerTreeViewWithFrameSinkTracking&) = delete;
    LayerTreeViewWithFrameSinkTracking& operator=(const LayerTreeViewWithFrameSinkTracking&) = delete;
    ...
    void SynchronousComposite()
    {
        ...
        layer_tree_host()->CompositeForTest(some_time, true /* raster */, base::OnceClosure());
    }
    ...
    void DidInitializeLayerTreeFrameSink() override
    {
        ...
        if (delegate_->num_successes() == expected_successes_) {
            ...
            blink::scheduler::GetSingleThreadTaskRunnerForTesting()->PostTask(
                FROM_HERE, base::BindOnce(&LayerTreeViewWithFrameSinkTracking::SynchronousComposite, base::Unretained(this)));
        }
    }
    ...
    void SetUp(int expected_successes, int num_tries, FailureMode failure_mode, base::RunLoop* run_loop)
    {
        run_loop_ = run_loop;
...
public:
        ...
        , layer_tree_view_(&layer_tree_view_delegate_, *dummy_page_scheduler_)
    {
        ...
        layer_tree_view_.Initialize(settings, blink::scheduler::GetSingleThreadTaskRunnerForTesting(),
            /*compositor_thread=*/nullptr, &test_task_graph_runner_);
    }
    LayerTreeViewWithFrameSinkTrackingTest(const LayerTreeViewWithFrameSinkTrackingTest&) = delete;
    LayerTreeViewWithFrameSinkTrackingTest& operator=(const LayerTreeViewWithFrameSinkTrackingTest&) = delete;
    ...
    void RunTest(int expected_successes, FailureMode failure_mode)
    {
        ...
        case kNoFailure:
            ...
            break;
        case kBindContextFailure:
            layer_tree_view_delegate_.set_num_failures_before_success(tries_before_success);
            layer_tree_view_delegate_.set_num_requests_before_success(0);
            break;
        case kGpuChannelFailure:
            layer_tree_view_delegate_.set_num_failures_before_success(0);
            layer_tree_view_delegate_.set_num_requests_before_success(tries_before_success);
            break;
        }
        ...
        blink::scheduler::GetSingleThreadTaskRunnerForTesting()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeViewWithFrameSinkTracking::SynchronousComposite, base::Unretained(&layer_tree_view_)));
        run_loop.Run();
    }
...
public:
    VisibilityTestLayerTreeView(StubLayerTreeViewDelegate* delegate, PageScheduler& scheduler)
        : LayerTreeView(delegate, scheduler.CreateWidgetScheduler())
    {
    }
...
TEST(LayerTreeViewTest, VisibilityTest)
{
    ...
    VisibilityTestLayerTreeView layer_tree_view(&layer_tree_view_delegate, *page_scheduler);

    layer_tree_view.Initialize(cc::LayerTreeSettings(), blink::scheduler::GetSingleThreadTaskRunnerForTesting(),
...
TEST(LayerTreeViewTest, RunPresentationCallbackOnSuccess)
{
    ...
    cc::TestTaskGraphRunner test_task_graph_runner;
    std::unique_ptr<PageScheduler> dummy_page_scheduler = scheduler::CreateDummyPageScheduler();
    StubLayerTreeViewDelegate layer_tree_view_delegate;
    LayerTreeView layer_tree_view(&layer_tree_view_delegate, dummy_page_scheduler->CreateWidgetScheduler());
    ...
    layer_tree_view.AddPresentationCallback(1, base::BindLambdaForTesting([&](const viz::FrameTimingDetails& frame_timing_details) {
        callback_timestamp = frame_timing_details.presentation_feedback.timestamp;
    }));

    // Respond with a failed presentation feedback for frame 1 and verify that the
    // callback is not called
    base::TimeTicks fail_timestamp = base::TimeTicks::Now() + base::Microseconds(2);
    gfx::PresentationFeedback fail_feedback(fail_timestamp, base::TimeDelta(), gfx::PresentationFeedback::kFailure);
    ...
    gfx::PresentationFeedback success_feedback(success_timestamp, base::TimeDelta(), 0);
...
public:
    LayerTreeViewDelegateChangeTest()
        : dummy_page_scheduler_(scheduler::CreateDummyPageScheduler())
        , layer_tree_view_(&old_layer_tree_view_delegate_, dummy_page_scheduler_->CreateWidgetScheduler())
    {
        cc::LayerTreeSettings settings;
        settings.single_thread_proxy_scheduler = false;
        layer_tree_view_.Initialize(settings, blink::scheduler::GetSingleThreadTaskRunnerForTesting(),
    ...
    LayerTreeViewDelegateChangeTest(const LayerTreeViewDelegateChangeTest&) = delete;
    LayerTreeViewDelegateChangeTest& operator=(const LayerTreeViewDelegateChangeTest&) = delete;

    void SwapDelegate()
    {
        layer_tree_view_.ClearPreviousDelegateAndReattachIfNeeded(&new_layer_tree_view_delegate_, dummy_page_scheduler_->CreateWidgetScheduler());
    }
...
protected:
    ...
    public:
        ...
        bool commit_defer_status() const
        {
            return commit_defer_status_;
        }

        const std::optional<cc::PaintHoldingCommitTrigger>& last_paint_holding_trigger() const
        {
    ...
    public:
        LayerTreeViewForTesting(LayerTreeViewDelegate* delegate, scoped_refptr<scheduler::WidgetScheduler> scheduler)
...
TEST_F(LayerTreeViewDelegateChangeTest, NoFrameSink)
{
    ...
    base::TimeTicks some_time;
    layer_tree_view_.layer_tree_host()->CompositeForTest(some_time, true /* raster */, base::OnceClosure());
    EXPECT_FALSE(old_layer_tree_view_delegate_.GetAndResetDidRequestFrameSink());
    EXPECT_TRUE(new_layer_tree_view_delegate_.GetAndResetDidRequestFrameSink());
    EXPECT_FALSE(new_layer_tree_view_delegate_.GetAndResetDidRequestFrameObserver());
}
...
TEST_F(LayerTreeViewDelegateChangeTest, SwapDuringFrameSinkInitialization)
{
    ...
    EXPECT_FALSE(old_layer_tree_view_delegate_.GetAndResetDidRequestFrameSink());
    EXPECT_FALSE(new_layer_tree_view_delegate_.GetAndResetDidRequestFrameSink());
    EXPECT_TRUE(new_layer_tree_view_delegate_.GetAndResetDidRequestFrameObserver());
}
...
TEST_F(LayerTreeViewDelegateChangeTest, StopDeferringCommitsOnSwap)
{
    ...
    EXPECT_EQ(old_layer_tree_view_delegate_.last_paint_holding_trigger(), std::nullopt);

    layer_tree_view_.layer_tree_host()->StartDeferringCommits(base::Seconds(1), cc::PaintHoldingReason::kFirstContentfulPaint);
    EXPECT_TRUE(old_layer_tree_view_delegate_.commit_defer_status());
    EXPECT_EQ(old_layer_tree_view_delegate_.last_paint_holding_trigger(), std::nullopt);

    SwapDelegate();
    EXPECT_FALSE(old_layer_tree_view_delegate_.commit_defer_status());
    EXPECT_EQ(old_layer_tree_view_delegate_.last_paint_holding_trigger(), cc::PaintHoldingCommitTrigger::kWidgetSwapped);
}
...
TEST_F(LayerTreeViewDelegateChangeTest, ResetEventListenerPropertiesOnSwap)
{
    ...
    for (uint32_t i = 0; i <= static_cast<uint32_t>(cc::EventListenerClass::kLast); i++) {
        layer_tree_host->SetEventListenerProperties(static_cast<cc::EventListenerClass>(i), cc::EventListenerProperties::kBlocking);
    }

    SwapDelegate();

    for (uint32_t i = 0; i <= static_cast<uint32_t>(cc::EventListenerClass::kLast); i++) {
        EXPECT_EQ(layer_tree_host->event_listener_properties(static_cast<cc::EventListenerClass>(i)), cc::EventListenerProperties::kNone);
    }
}
