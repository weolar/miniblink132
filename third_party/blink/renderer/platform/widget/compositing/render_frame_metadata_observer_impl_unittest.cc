...
class MockRenderFrameMetadataObserverClient : public cc::mojom::blink::RenderFrameMetadataObserverClient {
public:
    MockRenderFrameMetadataObserverClient(mojo::PendingReceiver<cc::mojom::blink::RenderFrameMetadataObserverClient> client_receiver,
        mojo::PendingRemote<cc::mojom::blink::RenderFrameMetadataObserver> observer)
        : render_frame_metadata_observer_client_receiver_(this, std::move(client_receiver))
        , render_frame_metadata_observer_remote_(std::move(observer))
    {
    }
    MockRenderFrameMetadataObserverClient(const MockRenderFrameMetadataObserverClient&) = delete;
    MockRenderFrameMetadataObserverClient& operator=(const MockRenderFrameMetadataObserverClient&) = delete;

    MOCK_METHOD2(OnRenderFrameMetadataChanged, void(uint32_t frame_token, const cc::RenderFrameMetadata& metadata));
...
private:
    mojo::Receiver<cc::mojom::blink::RenderFrameMetadataObserverClient> render_frame_metadata_observer_client_receiver_;
    mojo::Remote<cc::mojom::blink::RenderFrameMetadataObserver> render_frame_metadata_observer_remote_;
};

class RenderFrameMetadataObserverImplTest : public testing::Test {
public:
    RenderFrameMetadataObserverImplTest() = default;
    RenderFrameMetadataObserverImplTest(const RenderFrameMetadataObserverImplTest&) = delete;
    RenderFrameMetadataObserverImplTest& operator=(const RenderFrameMetadataObserverImplTest&) = delete;
    ~RenderFrameMetadataObserverImplTest() override = default;

    RenderFrameMetadataObserverImpl& observer_impl()
    {
        return *observer_impl_;
    }

    MockRenderFrameMetadataObserverClient& client()
    {
        return *client_;
    }
    ...
    void SetUp() override
    {
        mojo::PendingRemote<cc::mojom::blink::RenderFrameMetadataObserver> observer_remote;
        mojo::PendingReceiver<cc::mojom::blink::RenderFrameMetadataObserver> receiver = observer_remote.InitWithNewPipeAndPassReceiver();
        mojo::PendingRemote<cc::mojom::blink::RenderFrameMetadataObserverClient> client_remote;

        client_ = std::make_unique<testing::NiceMock<MockRenderFrameMetadataObserverClient>>(
            client_remote.InitWithNewPipeAndPassReceiver(), std::move(observer_remote));
        observer_impl_ = std::make_unique<RenderFrameMetadataObserverImpl>(std::move(receiver), std::move(client_remote));
...
private:
    ...
    std::unique_ptr<testing::NiceMock<MockRenderFrameMetadataObserverClient>> client_;
...
TEST_F(RenderFrameMetadataObserverImplTest, ShouldSendFrameToken)
{
    ...
    observer_impl().OnRenderFrameSubmission(render_frame_metadata, &compositor_frame_metadata, false /* force_send */);
    ...
    EXPECT_TRUE(compositor_frame_metadata.send_frame_token_to_embedder);
    {
        ...
        EXPECT_CALL(client(), OnRenderFrameMetadataChanged(1337, render_frame_metadata)).WillOnce(InvokeClosure(run_loop.QuitClosure()));
        run_loop.Run();
    }
}
...
TEST_F(RenderFrameMetadataObserverImplTest, ShouldSendFrameTokenOnAndroid)
{
    ...
    EXPECT_FALSE(compositor_frame_metadata.send_frame_token_to_embedder);
    {
        ...
        EXPECT_CALL(client(), OnRenderFrameMetadataChanged(0u, render_frame_metadata)).WillOnce(InvokeClosure(run_loop.QuitClosure()));
        run_loop.Run();
    }
}
...
TEST_F(RenderFrameMetadataObserverImplTest, SendRootScrollsForAccessibility)
{
    ...
    {
        base::RunLoop run_loop;
        EXPECT_CALL(client(), OnRenderFrameMetadataChanged(expected_frame_token, render_frame_metadata)).WillOnce(InvokeClosure(run_loop.QuitClosure()));
        run_loop.Run();
    }
    ...
    observer_impl().OnRenderFrameSubmission(render_frame_metadata, &compositor_frame_metadata, false /* force_send */);
    render_frame_metadata.is_scroll_offset_at_top = !render_frame_metadata.is_scroll_offset_at_top;
    observer_impl().OnRenderFrameSubmission(render_frame_metadata, &compositor_frame_metadata, false /* force_send */);
    {
        base::RunLoop run_loop;
        EXPECT_CALL(client(), OnRenderFrameMetadataChanged(expected_frame_token, render_frame_metadata)).WillOnce(InvokeClosure(run_loop.QuitClosure()));
    ...
    // one notification.
    observer_impl().UpdateRootScrollOffsetUpdateFrequency(cc::mojom::RootScrollOffsetUpdateFrequency::kAllUpdates);
    {
        base::RunLoop run_loop;
        EXPECT_CALL(client(), OnRenderFrameMetadataChanged(expected_frame_token, render_frame_metadata)).WillOnce(InvokeClosure(run_loop.QuitClosure()));
        run_loop.Run();
    ...
    {
        base::RunLoop run_loop;
        EXPECT_CALL(client(), OnRootScrollOffsetChanged(*(render_frame_metadata.root_scroll_offset))).WillOnce(InvokeClosure(run_loop.QuitClosure()));
        run_loop.Run();
    }
...
// offsets by default.
TEST_F(RenderFrameMetadataObserverImplTest, DoNotSendRootScrollOffsetByDefault)
{
    const uint32_t expected_frame_token = 1337;
    viz::CompositorFrameMetadata compositor_frame_metadata;
    compositor_frame_metadata.send_frame_token_to_embedder = false;
    ...
    observer_impl().OnRenderFrameSubmission(render_frame_metadata, &compositor_frame_metadata, false /* force_send */);
    EXPECT_CALL(client(), OnRenderFrameMetadataChanged(expected_frame_token, render_frame_metadata)).Times(0);
}
...
TEST_F(RenderFrameMetadataObserverImplTest, DoNotSendExtraRootScrollOffset)
{
    ...
    render_frame_metadata.is_scroll_offset_at_top = !render_frame_metadata.is_scroll_offset_at_top;
    observer_impl().OnRenderFrameSubmission(render_frame_metadata, &compositor_frame_metadata, false /* force_send */);
    EXPECT_CALL(client(), OnRootScrollOffsetChanged(*(render_frame_metadata.root_scroll_offset))).Times(0);
}
...
TEST_F(RenderFrameMetadataObserverImplTest, SendRootScrollOffsetOnScrollEnd)
{
    ...
    // a notification.
    observer_impl().UpdateRootScrollOffsetUpdateFrequency(cc::mojom::RootScrollOffsetUpdateFrequency::kOnScrollEnd);
    {
        base::RunLoop run_loop;
        EXPECT_CALL(client(), OnRenderFrameMetadataChanged(expected_frame_token, render_frame_metadata)).WillOnce(InvokeClosure(run_loop.QuitClosure()));
        run_loop.Run();
...
TEST_F(RenderFrameMetadataObserverImplTest, SendRenderFrameMetadataOnUpdateFrequency)
{
    ScopedCCTNewRFMPushBehaviorForTest feature(true);
    const uint32_t expected_frame_token = 1337;
    viz::CompositorFrameMetadata compositor_frame_metadata;
    ...
    observer_impl().UpdateRootScrollOffsetUpdateFrequency(cc::mojom::RootScrollOffsetUpdateFrequency::kNone);
    {
        base::RunLoop run_loop;
        EXPECT_CALL(client(), OnRenderFrameMetadataChanged(expected_frame_token, render_frame_metadata)).WillOnce(InvokeClosure(run_loop.QuitClosure()));
        run_loop.Run();
...
TEST_F(RenderFrameMetadataObserverImplTest, ForceSendMetadata)
{
    ...
    observer_impl().OnRenderFrameSubmission(render_frame_metadata, &compositor_frame_metadata, false /* force_send */);
    observer_impl().OnRenderFrameSubmission(render_frame_metadata, &compositor_frame_metadata, true /* force_send */);
