...
public:
    ...
    void ObserveGestureEventAndResult(const WebGestureEvent&, const gfx::Vector2dF&, const cc::OverscrollBehavior&, bool) override
    {
    }
...
public:
    static scoped_refptr<FakeWidgetCompositor> Create(cc::LayerTreeHost* layer_tree_host, base::WeakPtr<WidgetBase> widget_base,
    ...
    {
        auto compositor = base::MakeRefCounted<FakeWidgetCompositor>(WidgetCompositorPassKeyProvider::GetPassKey(), layer_tree_host, std::move(widget_base),
            std::move(main_task_runner), std::move(compositor_task_runner));
        compositor->BindOnThread(std::move(receiver));
        return compositor;
    }

    FakeWidgetCompositor(base::PassKey<WidgetCompositorPassKeyProvider> pass_key, cc::LayerTreeHost* layer_tree_host, base::WeakPtr<WidgetBase> widget_base,
        scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, scoped_refptr<base::SingleThreadTaskRunner> compositor_task_runner)
        : WidgetCompositor(std::move(pass_key), widget_base, std::move(main_task_runner), std::move(compositor_task_runner))
        , layer_tree_host_(layer_tree_host)
    {
    }

    cc::LayerTreeHost* LayerTreeHost() const override
    {
...
public:
    ...
    void BeginTest() override
    {
        ...
        widget_base_ = std::make_unique<WidgetBase>(
            /*widget_base_client=*/&client_, widget_host_remote.Unbind(), std::move(widget_receiver), scheduler::GetSingleThreadTaskRunnerForTesting(),
            /*is_hidden=*/false,
        ...
        widget_compositor_
            = FakeWidgetCompositor::Create(layer_tree_host(), widget_base_->GetWeakPtr(), layer_tree_host()->GetTaskRunnerProvider()->MainThreadTaskRunner(),
                layer_tree_host()->GetTaskRunnerProvider()->ImplThreadTaskRunner(), remote_.BindNewPipeAndPassReceiver());

        remote_->VisualStateRequest(base::BindOnce(&WidgetCompositorTest::VisualStateResponse, base::Unretained(this)));
    ...
    {
        EXPECT_TRUE(is_callback_run_);
    }

protected:
    void set_second_run_with_null()
    {
        second_run_with_null_ = true;
    }
