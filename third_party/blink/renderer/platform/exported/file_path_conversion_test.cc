...
TEST(FilePathConversionTest, convert)
{
    ...
    base::FilePath path_utf16 = base::FilePath::FromUTF8Unsafe("\xE6\x96\x87\xE5\xAD\x97");
    ...
    EXPECT_EQ(FILE_PATH_LITERAL("path"), WebStringToFilePath(test8bit_string).value());
    EXPECT_EQ(path_latin1.value(), WebStringToFilePath(test8bit_latin1).value());
    EXPECT_EQ(FILE_PATH_LITERAL("path"), WebStringToFilePath(test16bit_string).value());
    ...
    EXPECT_EQ("path", FilePathToWebString(base::FilePath(FILE_PATH_LITERAL("path"))));
    ...
    EXPECT_TRUE(FilePathToWebString(base::FilePath(FILE_PATH_LITERAL("foo\337bar"))).IsEmpty());
#else
    EXPECT_FALSE(FilePathToWebString(base::FilePath(FILE_PATH_LITERAL("foo\337bar"))).IsEmpty());
#endif
}
