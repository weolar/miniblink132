...
TEST(WebVectorTest, Iterators)
{
    ...
    for (WebVector<int>::iterator it = web_vector.begin(); it != web_vector.end(); ++it)
        output.push_back(*it);
    ...
    output.clear();
    for (WebVector<int>::const_iterator it = const_web_vector.begin(); it != const_web_vector.end(); ++it)
        output.push_back(*it);
    ASSERT_EQ(input.size(), output.size());
    for (size_t i = 0; i < input.size(); ++i)
        EXPECT_EQ(input[i], output[i]);
