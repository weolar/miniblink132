...
static inline int PartialStringToInt(const String& string, wtf_size_t start, wtf_size_t end)
{
    return WTF::VisitCharacters(
        StringView(string, start, end - start), [](auto chars) { return CharactersToInt(chars, WTF::NumberParsingOptions(), nullptr); });
}

} // namespace

WebVector<gfx::Size> WebIconSizesParser::ParseIconSizes(const WebString& web_sizes_string)
{
    ...
    for (wtf_size_t i = 0; i < length; ++i) {
        ...
        if (sizes_string.Substring(i, 3).StartsWithIgnoringCase("any") && (i + 3 == length || IsWhitespace(sizes_string[i + 3]))) {
        ...
        icon_sizes.push_back(gfx::Size(PartialStringToInt(sizes_string, width_start, width_end), PartialStringToInt(sizes_string, height_start, height_end)));
    }
    return icon_sizes;
}
