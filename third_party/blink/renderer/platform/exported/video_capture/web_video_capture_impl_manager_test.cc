...
class MockVideoCaptureImpl : public VideoCaptureImpl, public media::mojom::blink::VideoCaptureHost {
public:
    MockVideoCaptureImpl(const media::VideoCaptureSessionId& session_id, PauseResumeCallback* pause_callback, base::OnceClosure destruct_callback)
        : VideoCaptureImpl(session_id, scheduler::GetSingleThreadTaskRunnerForTesting(), GetEmptyBrowserInterfaceBroker())
...
private:
        ...
        mojo::PendingRemote<media::mojom::blink::VideoCaptureObserver> observer) override
    {
        ...
        OnStateChanged(media::mojom::blink::VideoCaptureResult::NewState(media::mojom::VideoCaptureState::STARTED));
    }
    ...
    void GetDeviceSupportedFormats(const base::UnguessableToken&, const base::UnguessableToken&, GetDeviceSupportedFormatsCallback) override
    {
        NOTREACHED();
    }

    void GetDeviceFormatsInUse(const base::UnguessableToken&, const base::UnguessableToken&, GetDeviceFormatsInUseCallback) override
    {
        NOTREACHED();
    }
...
public:
    MockVideoCaptureImplManager(PauseResumeCallback* pause_callback, base::RepeatingClosure stop_capture_callback)
        : pause_callback_(pause_callback)
    ...
    MockVideoCaptureImplManager& operator=(const MockVideoCaptureImplManager&) = delete;
    ~MockVideoCaptureImplManager() override
    {
    }

private:
    std::unique_ptr<VideoCaptureImpl> CreateVideoCaptureImpl(const media::VideoCaptureSessionId& session_id, const BrowserInterfaceBrokerProxy&) const override
    {
        auto video_capture_impl = std::make_unique<MockVideoCaptureImpl>(session_id, pause_callback_, stop_capture_callback_);
...
class VideoCaptureImplManagerTest : public ::testing::Test, public PauseResumeCallback {
public:
    VideoCaptureImplManagerTest()
        : manager_(new MockVideoCaptureImplManager(this, base::BindPostTaskToCurrentDefault(cleanup_run_loop_.QuitClosure())))
    ...
    VideoCaptureImplManagerTest& operator=(const VideoCaptureImplManagerTest&) = delete;

protected:
    static constexpr size_t kNumClients = 3;
    std::array<base::UnguessableToken, kNumClients> session_ids_;

    std::array<base::OnceClosure, kNumClients> StartCaptureForAllClients(bool same_session_id)
    {
        ...
        EXPECT_CALL(*this, OnStarted(_)).WillOnce(RunOnceClosure(std::move(quit_closure))).RetiresOnSaturation();
        std::array<base::OnceClosure, kNumClients> stop_callbacks;
        media::VideoCaptureParams params;
        params.requested_format = media::VideoCaptureFormat(gfx::Size(176, 144), 30, media::PIXEL_FORMAT_I420);
        for (size_t i = 0; i < kNumClients; ++i) {
            stop_callbacks[i] = StartCapture(same_session_id ? session_ids_[0] : session_ids_[i], params);
        }
        run_loop.Run();
        return stop_callbacks;
    }

    void StopCaptureForAllClients(std::array<base::OnceClosure, kNumClients>* stop_callbacks)
    {
        base::RunLoop run_loop;
        base::RepeatingClosure quit_closure = base::BindPostTaskToCurrentDefault(run_loop.QuitClosure());
        EXPECT_CALL(*this, OnStopped(_)).Times(kNumClients - 1).RetiresOnSaturation();
        EXPECT_CALL(*this, OnStopped(_)).WillOnce(RunOnceClosure(std::move(quit_closure))).RetiresOnSaturation();
        for (auto& stop_callback : *stop_callbacks)
            std::move(stop_callback).Run();
        run_loop.Run();
    }

    MOCK_METHOD2(OnFrameReady, void(scoped_refptr<media::VideoFrame>, base::TimeTicks estimated_capture_time));
    ...
    void OnStateUpdate(const media::VideoCaptureSessionId& id, VideoCaptureState state)
    {
    ...
    base::OnceClosure StartCapture(const media::VideoCaptureSessionId& id, const media::VideoCaptureParams& params)
    {
        return manager_->StartCapture(id, params,
            ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&VideoCaptureImplManagerTest::OnStateUpdate, CrossThreadUnretained(this), id)),
            ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&VideoCaptureImplManagerTest::OnFrameReady, CrossThreadUnretained(this))),
...
TEST_F(VideoCaptureImplManagerTest, MultipleClients)
{
    ...
    for (size_t i = 0; i < kNumClients; ++i) {
        release_callbacks[i] = manager_->UseDevice(session_ids_[0], *browser_interface_broker_);
    }
    std::array<base::OnceClosure, kNumClients> stop_callbacks = StartCaptureForAllClients(true);
...
TEST_F(VideoCaptureImplManagerTest, SuspendAndResumeSessions)
{
    ...
    for (size_t i = 0; i < kNumClients; ++i) {
        release_callbacks[i] = manager_->UseDevice(session_ids_[i], *browser_interface_broker_);
        MediaStreamDevice video_device;
        video_device.set_session_id(session_ids_[i]);
        video_devices.push_back(video_device);
    }
    std::array<base::OnceClosure, kNumClients> stop_callbacks = StartCaptureForAllClients(false);
    ...
    // paused.
    {
        base::RunLoop run_loop;
        base::RepeatingClosure quit_closure = base::BindPostTaskToCurrentDefault(run_loop.QuitClosure());
        EXPECT_CALL(*this, OnPaused(session_ids_[0])).Times(1).RetiresOnSaturation();
        EXPECT_CALL(*this, OnPaused(session_ids_[1])).Times(1).RetiresOnSaturation();
        EXPECT_CALL(*this, OnPaused(session_ids_[2])).WillOnce(RunOnceClosure(std::move(quit_closure))).RetiresOnSaturation();
    ...
    // Call SuspendDevices(false) and expect all to be resumed.
    {
        base::RunLoop run_loop;
        base::RepeatingClosure quit_closure = base::BindPostTaskToCurrentDefault(run_loop.QuitClosure());
        EXPECT_CALL(*this, OnResumed(session_ids_[0])).Times(1).RetiresOnSaturation();
        EXPECT_CALL(*this, OnResumed(session_ids_[1])).Times(1).RetiresOnSaturation();
        EXPECT_CALL(*this, OnResumed(session_ids_[2])).WillOnce(RunOnceClosure(std::move(quit_closure))).RetiresOnSaturation();
    ...
    // paused.
    {
        base::RunLoop run_loop;
        base::RepeatingClosure quit_closure = base::BindPostTaskToCurrentDefault(run_loop.QuitClosure());
        EXPECT_CALL(*this, OnPaused(session_ids_[0])).WillOnce(RunOnceClosure(std::move(quit_closure))).RetiresOnSaturation();
