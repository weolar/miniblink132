...
WebPlatformMediaStreamSource::WebPlatformMediaStreamSource(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
void WebPlatformMediaStreamSource::SetSourceMuted(bool is_muted)
{
    ...
    Owner().SetReadyState(is_muted ? WebMediaStreamSource::kReadyStateMuted : WebMediaStreamSource::kReadyStateLive);
}
...
void WebPlatformMediaStreamSource::SetCaptureHandle(media::mojom::CaptureHandlePtr capture_handle)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...
void WebPlatformMediaStreamSource::SetStopCallback(SourceStoppedCallback stop_callback)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    DCHECK(stop_callback_.is_null());
...
void WebPlatformMediaStreamSource::ChangeSource(const MediaStreamDevice& new_device)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...
base::SingleThreadTaskRunner* WebPlatformMediaStreamSource::GetTaskRunner() const
{
