...
void WebFont::DrawText(cc::PaintCanvas* canvas, const WebTextRun& run, const gfx::PointF& left_baseline, SkColor color) const
{
...
{
    return private_->GetFont().OffsetForPosition(run, position, kIncludePartialGlyphs, BreakGlyphsOption(false));
}

gfx::RectF WebFont::SelectionRectForText(const WebTextRun& run, const gfx::PointF& left_baseline, int height, int from, int to) const
{
    return private_->GetFont().SelectionRectForText(run, left_baseline, height, from, to);
}
