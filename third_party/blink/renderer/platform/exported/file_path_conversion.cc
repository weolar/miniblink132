...
base::FilePath StringToFilePath(const String& str)
{
    ...
    if (!str.Is8Bit()) {
        return base::FilePath::FromUTF16Unsafe(std::u16string_view(str.Characters16(), str.length()));
    }
    ...
    return base::FilePath::FromUTF16Unsafe(std::u16string(data8, data8 + str.length()));
#endif
}
