...
WTF::FlushBehavior kFlushBehavior[] = { WTF::FlushBehavior::kDoNotFlush, WTF::FlushBehavior::kFetchEOF, WTF::FlushBehavior::kDataEOF };

WTF::UnencodableHandling kUnencodableHandlingOptions[]
    = { WTF::kEntitiesForUnencodables, WTF::kURLEncodedEntitiesForUnencodables, WTF::kCSSEncodedEntitiesForUnencodables };
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    WTF::UnencodableHandling unencodable_handling = fuzzed_data.PickValueInArray(kUnencodableHandlingOptions);
    WTF::FlushBehavior flush_behavior = fuzzed_data.PickValueInArray(kFlushBehavior);
    ...
    bool saw_error;
    const String decoded = codec->Decode(byte_span, flush_behavior, stop_on_error, saw_error);
    ...
    if (size % sizeof(UChar) == 0) {
        ...
        auto uchar_span = UNSAFE_BUFFERS(base::span(reinterpret_cast<const UChar*>(byte_span.data()), byte_span.size() / sizeof(UChar)));
