...
void DetectLanguageWithModel(const WTF::String& text, blink::DetectLanguageCallback on_complete, language_detection::LanguageDetectionModel& model)
{
    if (!model.IsAvailable()) {
        std::move(on_complete).Run(base::unexpected(blink::DetectLanguageError::kUnavailable));
        return;
    }
    ...
    while (pos < text.length()) {
        ...
        auto predictions = model.Predict(std::u16string(substring.Characters16(), substring.length()));
...
void DetectLanguage(const WTF::String& text, DetectLanguageCallback on_complete)
{
    auto& model = language_detection::GetLanguageDetectionModel();
    model.AddOnModelLoadedCallback(WTF::BindOnce(DetectLanguageWithModel, text, std::move(on_complete)));
}
