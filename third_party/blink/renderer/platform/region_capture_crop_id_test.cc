...
{
    const base::Uuid kGUID = base::Uuid::ParseLowercase("21abd97f-73e8-4b88-9389-a9fee6abda5e");
    ASSERT_TRUE(kGUID.is_valid());
    EXPECT_EQ(GUIDToToken(kGUID), base::Token(0x21abd97f'73e84b88u, 0x9389a9fe'e6abda5eu));

    const base::Uuid kMinGUID = base::Uuid::ParseLowercase("00000000-0000-0000-0000-000000000000");
    ASSERT_TRUE(kMinGUID.is_valid());
    EXPECT_EQ(GUIDToToken(kMinGUID), base::Token(0, 0));

    const base::Uuid kMaxGUID = base::Uuid::ParseLowercase("ffffffff-ffff-ffff-ffff-ffffffffffff");
    ASSERT_TRUE(kMaxGUID.is_valid());
    EXPECT_EQ(GUIDToToken(kMaxGUID), base::Token(0xffffffff'ffffffffu, 0xffffffff'ffffffffu));
...
TEST(RegionCaptureCropIdTest, TokenToGUID)
{
    ...
    EXPECT_EQ(TokenToGUID(kToken), base::Uuid::ParseLowercase("21abd97f-73e8-4b88-9389-a9fee6abda5e"));

    const base::Token kMinToken(0u, 0u);
    EXPECT_TRUE(TokenToGUID(kMinToken).is_valid());
    EXPECT_EQ(TokenToGUID(kMinToken), base::Uuid::ParseLowercase("00000000-0000-0000-0000-000000000000"));

    const base::Token kMaxToken(0xffffffff'ffffffffu, 0xffffffff'ffffffffu);
    EXPECT_TRUE(TokenToGUID(kMaxToken).is_valid());
    EXPECT_EQ(TokenToGUID(kMaxToken), base::Uuid::ParseLowercase("ffffffff-ffff-ffff-ffff-ffffffffffff"));
}
