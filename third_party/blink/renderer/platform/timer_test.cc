...
public:
    TimerTest()
    {
        scoped_refptr<MainThreadTaskQueue> task_queue
            = platform_->GetMainThreadScheduler()->NewTaskQueue(MainThreadTaskQueue::QueueCreationParams(MainThreadTaskQueue::QueueType::kTest));
    ...
    base::TimeTicks Now()
    {
        return platform_->test_task_runner()->NowTicks();
    }

    void CountingTask(TimerBase*)
    {
        run_times_.push_back(Now());
    }
    ...
    bool TimeTillNextDelayedTask(base::TimeDelta* time) const
    {
        ...
        auto* scheduler_helper = platform_->GetMainThreadScheduler()->GetSchedulerHelperForTesting();
...
public:
    ...
    public:
        static scoped_refptr<Record> Create()
        {
            return base::AdoptRef(new Record);
        }

        bool TimerHasFired() const
        {
            return timer_has_fired_;
        }
        bool IsDisposed() const
        {
            return is_disposed_;
        }
        bool OwnerIsDestructed() const
        {
            return owner_is_destructed_;
        }
        void SetTimerHasFired()
        {
            timer_has_fired_ = true;
        }
        void Dispose()
        {
            is_disposed_ = true;
        }
        void SetOwnerIsDestructed()
        {
            owner_is_destructed_ = true;
        }
    ...
    explicit OnHeapTimerOwner(scoped_refptr<Record> record, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
{
    TaskRunnerTimer<TimerTest> timer(GetTaskRunner(), this, &TimerTest::CountingTask);
...
TEST_F(TimerTest, StartOneShot_NonZero_RepostingAfterRunning)
{
    ...
    platform_->RunUntilIdle();
    EXPECT_THAT(run_times_, ElementsAre(start_time_ + base::Seconds(10), start_time_ + base::Seconds(30)));
}
...
TEST_F(TimerTest, StartRepeatingTask)
{
    ...
    EXPECT_THAT(run_times_,
        ElementsAre(start_time_ + base::Seconds(1), start_time_ + base::Seconds(2), start_time_ + base::Seconds(3), start_time_ + base::Seconds(4),
...
TEST_F(TimerTest, StartRepeatingTask_ThenCancel)
{
    ...
    EXPECT_THAT(run_times_, ElementsAre(start_time_ + base::Seconds(1), start_time_ + base::Seconds(2)));
...
TEST_F(TimerTest, StartRepeatingTask_ThenPostOneShot)
{
    ...
    EXPECT_THAT(run_times_, ElementsAre(start_time_ + base::Seconds(1), start_time_ + base::Seconds(2), start_time_ + base::Milliseconds(2500)));
}
...
TEST_F(TimerTest, AugmentRepeatInterval)
{
    ...
    EXPECT_THAT(run_times_, ElementsAre(start_time_ + base::Seconds(20), start_time_ + base::Seconds(40)));
}
...
TEST_F(TimerTest, RepeatingTimerDoesNotDrift)
{
    ...
    TaskRunnerTimer<TimerTest> timer(GetTaskRunner(), this, &TimerTest::RecordNextFireTimeTask);
    timer.StartRepeating(base::Seconds(2), FROM_HERE);

    RecordNextFireTimeTask(&timer); // Next scheduled task to run at |start_time_| + 2s
    ...
    EXPECT_THAT(next_fire_times_,
        ElementsAre(start_time_ + base::Seconds(2), start_time_ + base::Seconds(4), start_time_ + base::Seconds(6), start_time_ + base::Seconds(8),
            start_time_ + base::Seconds(10), start_time_ + base::Seconds(12), start_time_ + base::Seconds(14), start_time_ + base::Seconds(24)));
}

template <typename TimerFiredClass> class TimerForTest : public TaskRunnerTimer<TimerFiredClass> {
public:
    using TimerFiredFunction = typename TaskRunnerTimer<TimerFiredClass>::TimerFiredFunction;

    ~TimerForTest() override = default;

    TimerForTest(scoped_refptr<base::SingleThreadTaskRunner> task_runner, TimerFiredClass* timer_fired_class, TimerFiredFunction timer_fired_function)
        : TaskRunnerTimer<TimerFiredClass>(std::move(task_runner), timer_fired_class, timer_fired_function)
    {
    }
};
...
{
    scoped_refptr<MainThreadTaskQueue> task_queue(platform_->GetMainThreadScheduler()->NewThrottleableTaskQueueForTest(nullptr));
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = task_queue->CreateTaskRunner(TaskType::kInternalTest);
...
{
    scoped_refptr<OnHeapTimerOwner::Record> record = OnHeapTimerOwner::Record::Create();
    Persistent<OnHeapTimerOwner> owner = MakeGarbageCollected<OnHeapTimerOwner>(record, GetTaskRunner());
...
public:
    TaskObserver(scoped_refptr<base::SingleThreadTaskRunner> task_runner, Vector<scoped_refptr<base::SingleThreadTaskRunner>>* run_order)
...
TEST_F(TimerTest, MoveToNewTaskRunnerOneShot)
{
    ...
    scoped_refptr<MainThreadTaskQueue> task_queue1(platform_->GetMainThreadScheduler()->NewThrottleableTaskQueueForTest(nullptr));
    scoped_refptr<base::SingleThreadTaskRunner> task_runner1 = task_queue1->CreateTaskRunner(TaskType::kInternalTest);
    TaskObserver task_observer1(task_runner1, &run_order);
    task_queue1->AddTaskObserver(&task_observer1);

    scoped_refptr<MainThreadTaskQueue> task_queue2(platform_->GetMainThreadScheduler()->NewThrottleableTaskQueueForTest(nullptr));
    scoped_refptr<base::SingleThreadTaskRunner> task_runner2 = task_queue2->CreateTaskRunner(TaskType::kInternalTest);
...
TEST_F(TimerTest, MoveToNewTaskRunnerRepeating)
{
    ...
    EXPECT_THAT(
        run_times_, ElementsAre(start_time + base::Seconds(1), start_time + base::Seconds(2), start_time + base::Seconds(3), start_time + base::Seconds(4)));

    EXPECT_THAT(run_order, ElementsAre(task_runner1, task_runner1, task_runner2, task_runner2));
