...
private:
    std::optional<size_t> DoWrite(int64_t offset, base::span<const uint8_t> data) override
    {
        int64_t end_offset = offset + data.size();
        if (static_cast<size_t>(end_offset) > kMaxSize)
            return std::nullopt;

        base::as_writable_bytes(base::span(data_).subspan(offset, data.size())).copy_from(data);
