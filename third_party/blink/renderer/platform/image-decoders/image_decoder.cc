...
#if 0 //BUILDFLAG(ENABLE_AV1_DECODER)
...
wtf_size_t CalculateMaxDecodedBytes(
    ImageDecoder::HighBitDepthDecodingOption high_bit_depth_decoding_option, const SkISize& desired_size, size_t platform_max_decoded_bytes)
{
    const wtf_size_t max_decoded_bytes = base::saturated_cast<wtf_size_t>(platform_max_decoded_bytes);
...
gfx::Size ExtractDensityCorrectedSize(const SkExif::Metadata& metadata, const gfx::Size& physical_size)
{
    const unsigned kDefaultResolution = 72;
    const unsigned kResolutionUnitDpi = 2;

    gfx::SizeF resolution(metadata.fXResolution.value_or(0), metadata.fYResolution.value_or(0));
    gfx::Size size(metadata.fPixelXDimension.value_or(0), metadata.fPixelYDimension.value_or(0));
    if (metadata.fResolutionUnit != kResolutionUnitDpi || resolution.IsEmpty() || size.IsEmpty()) {
    ...
    gfx::SizeF size_from_resolution(
        physical_size.width() * kDefaultResolution / resolution.width(), physical_size.height() * kDefaultResolution / resolution.height());
...
String SniffMimeTypeInternal(scoped_refptr<SegmentReader> reader)
{
    ...
    const char* contents = fast_reader.GetConsecutiveData(0, kLongestSignatureLength, buffer);
...
{
    return EqualIgnoringASCIICase(mime_type, "image/jpeg") || EqualIgnoringASCIICase(mime_type, "image/jpg")
...
{
    return EqualIgnoringASCIICase(mime_type, "image/bmp") || EqualIgnoringASCIICase(mime_type, "image/gif") || EqualIgnoringASCIICase(mime_type, "image/png")
        || EqualIgnoringASCIICase(mime_type, "image/webp") || EqualIgnoringASCIICase(mime_type, "image/x-xbitmap")
        || EqualIgnoringASCIICase(mime_type, "image/x-png");
}

} // namespace

ImageDecoder::ImageDecoder(AlphaOption alpha_option, HighBitDepthDecodingOption high_bit_depth_decoding_option, ColorBehavior color_behavior,
    cc::AuxImage aux_image, wtf_size_t max_decoded_bytes)
...
std::unique_ptr<ImageDecoder> ImageDecoder::Create(scoped_refptr<SegmentReader> data, bool data_complete, AlphaOption alpha_option,
    HighBitDepthDecodingOption high_bit_depth_decoding_option, ColorBehavior color_behavior, cc::AuxImage aux_image, size_t platform_max_decoded_bytes,
    const SkISize& desired_size, AnimationOption animation_option)
{
    ...
    return CreateByMimeType(type, std::move(data), data_complete, alpha_option, high_bit_depth_decoding_option, color_behavior, aux_image,
        platform_max_decoded_bytes, desired_size, animation_option);
}

std::unique_ptr<ImageDecoder> ImageDecoder::CreateByMimeType(String mime_type, scoped_refptr<SegmentReader> data, bool data_complete, AlphaOption alpha_option,
    HighBitDepthDecodingOption high_bit_depth_decoding_option, ColorBehavior color_behavior, cc::AuxImage aux_image, size_t platform_max_decoded_bytes,
    const SkISize& desired_size, AnimationOption animation_option)
{
    const wtf_size_t max_decoded_bytes = CalculateMaxDecodedBytes(high_bit_depth_decoding_option, desired_size, platform_max_decoded_bytes);
    ...
    if (mime_type == "image/jpeg" || mime_type == "image/pjpeg" || mime_type == "image/jpg") {
        decoder = std::make_unique<JPEGImageDecoder>(alpha_option, color_behavior, aux_image, max_decoded_bytes);
    } else if (mime_type == "image/png" || mime_type == "image/x-png" || mime_type == "image/apng") {
        decoder = CreatePngImageDecoder(alpha_option, high_bit_depth_decoding_option, color_behavior, max_decoded_bytes);
    } else if (mime_type == "image/gif") {
        decoder = std::make_unique<GIFImageDecoder>(alpha_option, color_behavior, max_decoded_bytes);
    } else if (mime_type == "image/webp") {
        decoder = std::make_unique<WEBPImageDecoder>(alpha_option, color_behavior, max_decoded_bytes);
    } else if (mime_type == "image/x-icon" || mime_type == "image/vnd.microsoft.icon") {
        decoder = std::make_unique<ICOImageDecoder>(alpha_option, color_behavior, max_decoded_bytes);
    } else if (mime_type == "image/bmp" || mime_type == "image/x-xbitmap") {
        decoder = std::make_unique<BMPImageDecoder>(alpha_option, color_behavior, max_decoded_bytes);
#if 0 //BUILDFLAG(ENABLE_AV1_DECODER)
...
{
    return SniffMimeTypeInternal(SegmentReader::CreateFromSharedBuffer(std::move(image_data)));
}

// static
ImageDecoder::CompressionFormat ImageDecoder::GetCompressionFormat(scoped_refptr<SharedBuffer> image_data, String mime_type)
{
    ...
    if (EqualIgnoringASCIICase(mime_type, "image/webp") && available_data >= 16) {
        ...
        const FastSharedBufferReader fast_reader(SegmentReader::CreateFromSharedBuffer(image_data));
        char buffer[8];
        const unsigned char* contents = reinterpret_cast<const unsigned char*>(fast_reader.GetConsecutiveData(8, 8, buffer));
        ...
        if (!memcmp(contents, "WEBPVP8X", 8)) {
            ...
            contents = reinterpret_cast<const unsigned char*>(fast_reader.GetConsecutiveData(0, available_data, long_buffer.data()));
            WebPBitstreamFeatures webp_features {};
            VP8StatusCode status = WebPGetFeatures(contents, available_data, &webp_features);
            // It is possible that there is not have enough image data available to
            // make a determination.
            if (status == VP8_STATUS_OK) {
                DCHECK_LT(webp_features.format, CompressionFormat::kWebPAnimationFormat);
                return webp_features.has_animation ? CompressionFormat::kWebPAnimationFormat : static_cast<CompressionFormat>(webp_features.format);
...
bool ImageDecoder::IsSizeAvailable()
{
    ...
    if (ImageIsHighBitDepth() && high_bit_depth_decoding_option_ == kHighBitDepthToHalfFloat) {
        decoded_bytes_per_pixel = 8;
    }

    const gfx::Size size = DecodedSize();
    const wtf_size_t decoded_size_bytes = size.width() * size.height() * decoded_bytes_per_pixel;
    if (decoded_size_bytes > max_decoded_bytes_) {
        LOG(WARNING) << "Blocked decode of oversized image: " << size.width() << "x" << size.height();
        return SetFailed();
    }
#endif
...
bool ImageDecoder::GetGainmapInfoAndData(SkGainmapInfo& out_gainmap_info, scoped_refptr<SegmentReader>& out_gainmap_data) const
{
    return false;
}
...
cc::ImageHeaderMetadata ImageDecoder::MakeMetadataForDecodeAcceleration() const
{
...
ImageFrame* ImageDecoder::DecodeFrameBufferAtIndex(wtf_size_t index)
{
    ...
    if (frame->GetStatus() != ImageFrame::kFrameComplete) {
        TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "Decode Image", "imageType", FilenameExtension().Ascii());
        Decode(index);
    }
...
{
    return !FrameIsReceivedAtIndex(index) || frame_buffer_cache_[index].HasAlpha();
}
...
{
    return index < frame_buffer_cache_.size() && frame_buffer_cache_[index].GetStatus() == ImageFrame::kFrameComplete;
}

std::optional<base::TimeDelta> ImageDecoder::FrameTimestampAtIndex(wtf_size_t) const
{
    return std::nullopt;
}
...
{
    if (index >= frame_buffer_cache_.size() || frame_buffer_cache_[index].GetStatus() == ImageFrame::kFrameEmpty) {
        return 0;
    }

    wtf_size_t decoded_bytes_per_pixel = 4;
    if (frame_buffer_cache_[index].GetPixelFormat() == ImageFrame::PixelFormat::kRGBA_F16) {
        decoded_bytes_per_pixel = 8;
    }
...
wtf_size_t ImageDecoder::ClearCacheExceptFrame(wtf_size_t clear_except_frame)
{
    ...
    if (clear_except_frame < frame_buffer_cache_.size()) {
        ...
        if (!FrameStatusSufficientForSuccessors(clear_except_frame) || frame.GetDisposalMethod() == ImageFrame::kDisposeOverwritePrevious) {
    ...
    while (clear_except_frame2 < frame_buffer_cache_.size() && !FrameStatusSufficientForSuccessors(clear_except_frame2)) {
        clear_except_frame2 = frame_buffer_cache_[clear_except_frame2].RequiredPreviousFrameIndex();
    }
...
wtf_size_t ImageDecoder::ClearCacheExceptTwoFrames(wtf_size_t clear_except_frame1, wtf_size_t clear_except_frame2)
{
    wtf_size_t frame_bytes_cleared = 0;
    for (wtf_size_t i = 0; i < frame_buffer_cache_.size(); ++i) {
        if (frame_buffer_cache_[i].GetStatus() != ImageFrame::kFrameEmpty && i != clear_except_frame1 && i != clear_except_frame2) {
...
Vector<wtf_size_t> ImageDecoder::FindFramesToDecode(wtf_size_t index) const
{
    ...
    } while (index != kNotFound && frame_buffer_cache_[index].GetStatus() != ImageFrame::kFrameComplete);
...
void ImageDecoder::CorrectAlphaWhenFrameBufferSawNoAlpha(wtf_size_t index)
{
    ...
    if (buffer.OriginalFrameRect().Contains(gfx::Rect(Size()))) {
        ...
        const ImageFrame* prev_buffer = &frame_buffer_cache_[buffer.RequiredPreviousFrameIndex()];
        DCHECK(prev_buffer->GetDisposalMethod() != ImageFrame::kDisposeOverwritePrevious);

        if ((prev_buffer->GetDisposalMethod() == ImageFrame::kDisposeOverwriteBgcolor) && !prev_buffer->HasAlpha()
...
bool ImageDecoder::InitFrameBuffer(wtf_size_t frame_index)
{
    ...
    wtf_size_t required_previous_frame_index = buffer->RequiredPreviousFrameIndex();
    if (required_previous_frame_index == kNotFound) {
        // This frame doesn't rely on any previous data.
        if (!buffer->AllocatePixelData(Size().width(), Size().height(), ColorSpaceForSkImages())) {
            return false;
        }
        buffer->ZeroFillPixelData();
    } else {
        ImageFrame* const prev_buffer = &frame_buffer_cache_[required_previous_frame_index];
        ...
        if ((!CanReusePreviousFrameBuffer(frame_index) || !buffer->TakeBitmapDataIfWritable(prev_buffer)) && !buffer->CopyBitmapData(*prev_buffer)) {
            return false;
        }

        if (prev_buffer->GetDisposalMethod() == ImageFrame::kDisposeOverwriteBgcolor) {
    ...
    DCHECK_EQ(high_bit_depth_decoding_option_ == kHighBitDepthToHalfFloat && ImageIsHighBitDepth(), buffer->GetPixelFormat() == ImageFrame::kRGBA_F16);
...
void ImageDecoder::UpdateAggressivePurging(wtf_size_t index)
{
    ...
    if (frame_buffer_cache_.size() && frame_buffer_cache_[0].GetPixelFormat() == ImageFrame::PixelFormat::kRGBA_F16) {
        decoded_bytes_per_pixel = 8;
    }
    const uint64_t frame_memory_usage = DecodedSize().Area64() * decoded_bytes_per_pixel;
    ...
    DCHECK_EQ(frame_memory_usage / decoded_bytes_per_pixel, DecodedSize().Area64());
...
bool ImageDecoder::FrameStatusSufficientForSuccessors(wtf_size_t index)
{
    ...
    return frame_status == ImageFrame::kFramePartial || frame_status == ImageFrame::kFrameComplete;
}

wtf_size_t ImageDecoder::FindRequiredPreviousFrame(wtf_size_t frame_index, bool frame_rect_is_opaque)
{
    ...
    if ((frame_rect_is_opaque || curr_buffer->GetAlphaBlendSource() == ImageFrame::kBlendAtopBgcolor)
    ...
    while (prev_buffer->GetDisposalMethod() == ImageFrame::kDisposeOverwritePrevious) {
        if (prev_frame == 0) {
            return kNotFound;
        }
        prev_frame--;
    ...
    case ImageFrame::kDisposeOverwriteBgcolor:
        ...
        return (prev_buffer->OriginalFrameRect().Contains(gfx::Rect(Size())) || (prev_buffer->RequiredPreviousFrameIndex() == kNotFound)) ? kNotFound
                                                                                                                                          : prev_frame;
...
void ImageDecoder::ApplyExifMetadata(const SkData* exif_data, const gfx::Size& physical_size)
{
    DCHECK(IsDecodedSizeAvailable());
    SkExif::Metadata metadata;
    SkExif::Parse(metadata, exif_data);

    orientation_ = static_cast<ImageOrientationEnum>(metadata.fOrigin.value_or(kTopLeft_SkEncodedOrigin));
    density_corrected_size_ = ExtractDensityCorrectedSize(metadata, physical_size);
}
...
ImagePlanes::ImagePlanes(base::span<void*, cc::kNumYUVPlanes> planes, base::span<const wtf_size_t, cc::kNumYUVPlanes> row_bytes, SkColorType color_type)
...
ColorProfile::ColorProfile(const skcms_ICCProfile& profile, base::HeapArray<uint8_t> buffer)
    : profile_(profile)
...
std::unique_ptr<ColorProfile> ColorProfile::Create(base::span<const uint8_t> buffer)
{
...
ColorProfileTransform::ColorProfileTransform(const skcms_ICCProfile* src_profile, const skcms_ICCProfile* dst_profile)
{
    DCHECK(src_profile);
    DCHECK(dst_profile);
...
void ImageDecoder::SetEmbeddedColorProfile(std::unique_ptr<ColorProfile> profile)
{
...
void ImageDecoder::UpdateSkImageColorSpaceAndTransform()
{
    ...
    if (color_behavior_ == ColorBehavior::kTag) {
        ...
        if (embedded_color_profile_) {
            ...
            if (profile->has_CICP) {
                sk_image_color_space_ = skia::CICPGetSkColorSpace(profile->CICP.color_primaries, profile->CICP.transfer_characteristics,
    ...
    embedded_to_sk_image_transform_ = std::make_unique<ColorProfileTransform>(src_profile, &dst_profile);
}
