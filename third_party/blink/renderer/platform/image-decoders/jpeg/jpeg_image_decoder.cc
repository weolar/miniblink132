...
{
    if (info.jpeg_color_space == JCS_YCbCr && info.num_components == 3 && info.comp_info && info.comp_info[1].h_samp_factor == 1
        && info.comp_info[1].v_samp_factor == 1 && info.comp_info[2].h_samp_factor == 1 && info.comp_info[2].v_samp_factor == 1) {
...
bool SubsamplingSupportedByDecodeToYUV(cc::YUVSubsampling subsampling)
{
    ...
    return subsampling == cc::YUVSubsampling::k444 || subsampling == cc::YUVSubsampling::k422 || subsampling == cc::YUVSubsampling::k420;
}
...
jpg_boolean fill_input_buffer(j_decompress_ptr jd);
void skip_input_data(j_decompress_ptr jd, long num_bytes);
void term_source(j_decompress_ptr jd);
void error_exit(j_common_ptr cinfo);
void emit_message(j_common_ptr cinfo, int msg_level);

static gfx::Size ComputeYUVSize(const jpeg_decompress_struct* info, int component)
{
    return gfx::Size(info->comp_info[component].downsampled_width, info->comp_info[component].downsampled_height);
}

static wtf_size_t ComputeYUVWidthBytes(const jpeg_decompress_struct* info, int component)
{
...
public:
    ...
    void SkipBytes(long num_bytes)
    {
        ...
        if (bytes_to_skip < info_.src->bytes_in_buffer) {
            ...
            next_read_position_ = static_cast<wtf_size_t>(next_read_position_ + bytes_to_skip - info_.src->bytes_in_buffer);
            info_.src->bytes_in_buffer = 0;
            info_.src->next_input_byte = nullptr;
        }

        // This is a valid restart position.
        restart_position_ = static_cast<wtf_size_t>(next_read_position_ - info_.src->bytes_in_buffer);
    ...
    bool ShouldDecodeToOriginalSize() const
    {
        ...
        return info_.image_width % mcu_width != 0 || info_.image_height % mcu_height != 0;
    }
    ...
    bool AreValidSampleFactorsAvailable(int* max_h, int* max_v) const
    {
        ...
        *max_v = 0;
        for (int i = 0; i < info_.num_components; ++i) {
            if (comp_info[i].h_samp_factor < 1 || comp_info[i].h_samp_factor > 4 || comp_info[i].v_samp_factor < 1 || comp_info[i].v_samp_factor > 4) {
                return false;
            }
    ...
    bool Decode(JPEGImageDecoder::DecodingMode decoding_mode)
    {
        ...
        case kJpegHeader: {
            ...
            // now-complete header.
            {
                ...
                for (auto* marker = info_.marker_list; marker; marker = marker->next) {
                    segments.emplace_back(marker->marker, SkData::MakeWithoutCopy(marker->data, marker->data_length));
                }
            ...
            if (decoder_->ShouldGenerateAllSizes()) {
                ...
                if (max_numerator == g_scale_denominator && ShouldDecodeToOriginalSize()) {
                    sizes.push_back(SkISize::Make(info_.image_width, info_.image_height));
                } else {
                    ...
                    for (int numerator = 1; numerator <= max_numerator; ++numerator) {
                        ...
                        sizes.push_back(SkISize::Make(info_.output_width, info_.output_height));
                    }
                }
            ...
            decoder_->ApplyExifMetadata(metadata_decoder_->getExifMetadata(/*copyData=*/false).get(), gfx::Size(info_.output_width, info_.output_height));
            ...
            if (!decoder_->IgnoresColorSpace()) {
                ...
                sk_sp<SkData> profile_data = metadata_decoder_->getICCProfileData(/*copyData=*/false);
                if (profile_data) {
                    std::unique_ptr<ColorProfile> profile = ColorProfile::Create(base::span(profile_data->bytes(), profile_data->size()));
                    if (profile) {
        ...
        case kJpegStartDecompress:
            ...
            state_ = (info_.buffered_image) ? kJpegDecompressProgressive : kJpegDecompressSequential;
            [[fallthrough]];
        ...
        case kJpegDecompressProgressive:
            if (state_ == kJpegDecompressProgressive) {
                ...
                int status = 0;
                int first_scan_to_display = all_components_seen(info_) ? info_.input_scan_number : 0;
                do {
                    decoder_error_mgr* err = reinterpret_cast_ptr<decoder_error_mgr*>(info_.err);
                    if (err->num_corrupt_warnings) {
                        break;
                    }
                    status = jpeg_consume_input(&info_);
                    if (status == JPEG_REACHED_SOS || status == JPEG_REACHED_EOI || status == JPEG_SUSPENDED) {
                ...
                for (;;) {
                    if (!info_.output_scanline) {
                        ...
                        if (!info_.output_scan_number && (scan > first_scan_to_display) && (status != JPEG_REACHED_EOI)) {
                            --scan;
                        }
                    ...
                    if (info_.output_scanline == info_.output_height) {
                        ...
                        if (jpeg_input_complete(&info_) && (info_.input_scan_number == info_.output_scan_number)) {
                            break;
                        }
        ...
        case kJpegDone:
            ...
            if (info_.jpeg_color_space != JCS_GRAYSCALE && decoder_->IsAllDataReceived()) {
                static constexpr char kType[] = "Jpeg";
                ImageDecoder::UpdateBppHistogram<kType>(decoder_->Size(), data_->size());
            }
    ...
    gfx::Size UvSize() const
    {
        return uv_size_;
    }
    bool HasStartedDecompression() const
    {
        return state_ > kJpegStartDecompress;
    }
...
// case.
        ...
        if (info_.out_color_space != JCS_YCbCr) {
            return (*info_.mem->alloc_sarray)(reinterpret_cast_ptr<j_common_ptr>(&info_), JPOOL_IMAGE, 4 * info_.output_width, 1);
        }
        ...
        return (*info_.mem->alloc_sarray)(reinterpret_cast_ptr<j_common_ptr>(&info_), JPOOL_IMAGE, width_bytes, 1);
    }
...
JPEGImageDecoder::JPEGImageDecoder(
    AlphaOption alpha_option, ColorBehavior color_behavior, cc::AuxImage aux_image, wtf_size_t max_decoded_bytes, wtf_size_t offset)
    : ImageDecoder(alpha_option, ImageDecoder::kDefaultBitDepth, color_behavior, aux_image, max_decoded_bytes)
    , offset_(offset)
{
}
...
void JPEGImageDecoder::OnSetData(scoped_refptr<SegmentReader> data)
{
    ...
    if (data && aux_image_ == cc::AuxImage::kGainmap) {
        ...
        if (!base_metadata_decoder->findGainmapImage(base_image_data, gainmap_image_data, gainmap_info)) {
            SetFailed();
            return;
        }
    ...
    allow_decode_to_yuv_ =
        ...
        // data.
        !ColorTransform() && SubsamplingSupportedByDecodeToYUV(GetYUVSubsampling());
}
...
unsigned JPEGImageDecoder::DesiredScaleNumerator() const
{
    ...
    return JPEGImageDecoder::DesiredScaleNumerator(max_decoded_bytes_, original_bytes, g_scale_denominator);
}

// static
unsigned JPEGImageDecoder::DesiredScaleNumerator(wtf_size_t max_decoded_bytes, wtf_size_t original_bytes, unsigned scale_denominator)
{
    ...
    return static_cast<unsigned>(floor(sqrt(
        ...
        static_cast<float>(max_decoded_bytes) / original_bytes * scale_denominator * scale_denominator)));
}
...
void JPEGImageDecoder::DecodeToYUV()
{
    ...
    {
        TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "Decode Image", "imageType", "JPEG");
...
bool JPEGImageDecoder::GetGainmapInfoAndData(SkGainmapInfo& out_gainmap_info, scoped_refptr<SegmentReader>& out_gainmap_data) const
{
    ...
    sk_sp<SkData> gainmap_image_data;
    SkGainmapInfo gainmap_info;
    if (!metadata_decoder->findGainmapImage(base_image_data, gainmap_image_data, gainmap_info)) {
        return false;
    }
...
gfx::Size JPEGImageDecoder::GetImageCodedSize() const
{
    ...
    if (!reader_->AreValidSampleFactorsAvailable(&max_h_samp_factor, &max_v_samp_factor)) {
        return gfx::Size();
    }
...
cc::ImageHeaderMetadata JPEGImageDecoder::MakeMetadataForDecodeAcceleration() const
{
    cc::ImageHeaderMetadata image_metadata = ImageDecoder::MakeMetadataForDecodeAcceleration();
...
template <J_COLOR_SPACE colorSpace> void SetPixel(ImageFrame::PixelData*, JSAMPARRAY samples, int column) = delete;

// Used only for debugging with libjpeg (instead of libjpeg-turbo).
template <> void SetPixel<JCS_RGB>(ImageFrame::PixelData* pixel, JSAMPARRAY samples, int column)
{
    JSAMPLE* jsample = *samples + column * 3;
    ImageFrame::SetRGBARaw(pixel, jsample[0], jsample[1], jsample[2], 255);
}

template <> void SetPixel<JCS_CMYK>(ImageFrame::PixelData* pixel, JSAMPARRAY samples, int column)
{
    ...
    ImageFrame::SetRGBARaw(pixel, jsample[0] * k / 255, jsample[1] * k / 255, jsample[2] * k / 255, 255);
}
    ...
    while (info->output_scanline < info->output_height) {
        ...
        if (xform) {
            ...
            bool color_conversion_successful = skcms_Transform(
                row, XformColorFormat(), alpha_format, xform->SrcProfile(), row, XformColorFormat(), alpha_format, xform->DstProfile(), width);
...
static bool OutputRawData(JPEGImageReader* reader, ImagePlanes* image_planes)
{
    ...
    JSAMPROW output_y = static_cast<JSAMPROW>(image_planes->Plane(cc::YUVIndex::kY));
    JSAMPROW output_u = static_cast<JSAMPROW>(image_planes->Plane(cc::YUVIndex::kU));
    JSAMPROW output_v = static_cast<JSAMPROW>(image_planes->Plane(cc::YUVIndex::kV));
    ...
    while (info->output_scanline < info->output_height) {
        ...
        JDIMENSION scanlines_read = jpeg_read_raw_data(info, bufferraw, y_scanlines_to_read);
        if (!scanlines_read) {
            return false;
        }
    }
...
bool JPEGImageDecoder::OutputScanlines()
{
    ...
    if (buffer.GetStatus() == ImageFrame::kFrameEmpty) {
        DCHECK_EQ(info->output_width, static_cast<JDIMENSION>(decoded_size_.width()));
        DCHECK_EQ(info->output_height, static_cast<JDIMENSION>(decoded_size_.height()));

        if (!buffer.AllocatePixelData(info->output_width, info->output_height, ColorSpaceForSkImages())) {
            return SetFailed();
        }
    ...
    if (turboSwizzled(info->out_color_space)) {
        while (info->output_scanline < info->output_height) {
            ...
            if (xform) {
                skcms_AlphaFormat alpha_format = skcms_AlphaFormat_Unpremul;
                bool color_conversion_successful = skcms_Transform(
                    row, XformColorFormat(), alpha_format, xform->SrcProfile(), row, XformColorFormat(), alpha_format, xform->DstProfile(), info->output_width);
                DCHECK(color_conversion_successful);
            }
        }
        buffer.SetPixelsChanged(true);
...
inline bool IsComplete(const JPEGImageDecoder* decoder, JPEGImageDecoder::DecodingMode decoding_mode)
{
    if (decoding_mode == JPEGImageDecoder::DecodingMode::kDecodeToYuv) {
