...
{
    return std::make_unique<JPEGImageDecoder>(ImageDecoder::kAlphaNotPremultiplied, ColorBehavior::kTransformToSRGB, cc::AuxImage::kDefault, max_decoded_bytes);
}
...
void Downsample(size_t max_decoded_bytes, const char* image_file_path, const gfx::Size& expected_size)
{
...
void ReadYUV(size_t max_decoded_bytes, const char* image_file_path, const gfx::Size& expected_y_size, const gfx::Size& expected_uv_size,
    const bool expect_decoding_failure = false)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(image_file_path);
    ASSERT_TRUE(data);

    std::unique_ptr<JPEGImageDecoder> decoder = CreateJPEGDecoder(max_decoded_bytes);
    decoder->SetData(data.get(), true);
    ...
    decoder->SetImagePlanes(std::make_unique<ImagePlanes>(planes, row_bytes, kGray_8_SkColorType));
...
void TestJpegBppHistogram(const char* image_name, const char* histogram_name = nullptr, base::HistogramBase::Sample sample = 0)
{
    TestBppHistogram(CreateJPEGDecoder, "Jpeg", image_name, histogram_name, sample);
}
...
TEST(JPEGImageDecoderTest, yuv)
{
    ...
    ReadYUV(kLargeEnoughSize, jpeg_file, gfx::Size(256, 256), gfx::Size(128, 128));

    // Each plane is in its own scan.
    const char* jpeg_file_non_interleaved = "/images/resources/cs-uma-ycbcr-420-non-interleaved.jpg"; // 64x64
    ReadYUV(kLargeEnoughSize, jpeg_file_non_interleaved, gfx::Size(64, 64), gfx::Size(32, 32));

    const char* jpeg_file_image_size_not_multiple_of8 = "/images/resources/cropped_mandrill.jpg"; // 439x154
    ReadYUV(kLargeEnoughSize, jpeg_file_image_size_not_multiple_of8, gfx::Size(439, 154), gfx::Size(220, 77));
...
TEST(JPEGImageDecoderTest, missingEoi)
{
    ...
    ReadYUV((1599 * 899 * 4), jpeg_file, gfx::Size(1599, 899), gfx::Size(800, 450),
...
{
    TestByteByByteDecode(&CreateJPEGDecoder, "/images/resources/bug106024.jpg", 1u, kAnimationNone);
}

TEST(JPEGImageDecoderTest, byteByByteRGBJPEGWithAdobeMarkers)
{
    TestByteByByteDecode(&CreateJPEGDecoder, "/images/resources/rgb-jpeg-with-adobe-marker-only.jpg", 1u, kAnimationNone);
}
...
{
    scoped_refptr<SharedBuffer> test_data = ReadFileToSharedBuffer(kDecodersTestingDir, "many-progressive-scans.jpg");
...
{
    scoped_refptr<SharedBuffer> test_data = ReadFileToSharedBuffer(kDecodersTestingDir, "green-exif-ifd-last.jpg");
    ASSERT_TRUE(test_data.get());
...
TEST(JPEGImageDecoderTest, SupportedSizesSquare)
{
    ...
    std::unique_ptr<ImageDecoder> decoder = CreateJPEGDecoder(std::numeric_limits<int>::max());
    decoder->SetData(data.get(), true);
    // This will decode the size and needs to be called to avoid DCHECKs
    ASSERT_TRUE(decoder->IsSizeAvailable());
    Vector<SkISize> expected_sizes = { SkISize::Make(32, 32), SkISize::Make(64, 64), SkISize::Make(96, 96), SkISize::Make(128, 128), SkISize::Make(160, 160),
        SkISize::Make(192, 192), SkISize::Make(224, 224), SkISize::Make(256, 256) };
    auto sizes = decoder->GetSupportedDecodeSizes();
    ASSERT_EQ(expected_sizes.size(), sizes.size());
    for (size_t i = 0; i < sizes.size(); ++i) {
        EXPECT_TRUE(expected_sizes[i] == sizes[i]) << "Expected " << expected_sizes[i].width() << "x" << expected_sizes[i].height() << ". Got "
...
TEST(JPEGImageDecoderTest, SupportedSizesRectangle)
{
    ...
    // This will decode the size and needs to be called to avoid DCHECKs
    ASSERT_TRUE(decoder->IsSizeAvailable());
    Vector<SkISize> expected_sizes = { SkISize::Make(34, 25), SkISize::Make(68, 50), SkISize::Make(102, 75), SkISize::Make(136, 100), SkISize::Make(170, 125),
        SkISize::Make(204, 150), SkISize::Make(238, 175), SkISize::Make(272, 200) };
...
TEST(JPEGImageDecoderTest, SupportedSizesRectangleNotMultipleOfMCUIfMemoryBound)
{
    ...
    // This will decode the size and needs to be called to avoid DCHECKs
    ASSERT_TRUE(decoder->IsSizeAvailable());
    Vector<SkISize> expected_sizes = { SkISize::Make(35, 26), SkISize::Make(69, 52), SkISize::Make(104, 78), SkISize::Make(138, 104), SkISize::Make(172, 130),
        SkISize::Make(207, 156), SkISize::Make(241, 182) };
...
TEST(JPEGImageDecoderTest, SupportedSizesRectangleNotMultipleOfMCU)
{
    struct {
        ...
        SkISize expected_size;
    } recs[] = { { // This 264x192 image uses 4:2:0 sampling format. The MCU is therefore
                     ...
                     "/images/resources/icc-v2-gbr-420-width-not-whole-mcu.jpg", SkISize::Make(264, 192) },
        { // This 272x200 image uses 4:2:0 sampling format. The MCU is therefore
            ...
            "/images/resources/icc-v2-gbr-420-height-not-whole-mcu.jpg", SkISize::Make(272, 200) } };
    for (const auto& rec : recs) {
        ...
        EXPECT_EQ(rec.expected_size, sizes[0]) << "Expected " << rec.expected_size.width() << "x" << rec.expected_size.height() << ". Got " << sizes[0].width()
                                               << "x" << sizes[0].height();
    }
}
...
TEST(JPEGImageDecoderTest, SupportedSizesTruncatedIfMemoryBound)
{
    ...
    // This will decode the size and needs to be called to avoid DCHECKs
    ASSERT_TRUE(decoder->IsSizeAvailable());
    Vector<SkISize> expected_sizes = { SkISize::Make(32, 32), SkISize::Make(64, 64), SkISize::Make(96, 96), SkISize::Make(128, 128) };
    auto sizes = decoder->GetSupportedDecodeSizes();
    ASSERT_EQ(expected_sizes.size(), sizes.size());
    for (size_t i = 0; i < sizes.size(); ++i) {
        EXPECT_TRUE(expected_sizes[i] == sizes[i]) << "Expected " << expected_sizes[i].width() << "x" << expected_sizes[i].height() << ". Got "
...
TEST(JPEGImageDecoderTest, SupportedScaleNumeratorBound)
{
    ...
    auto numerator_normal = JPEGImageDecoder::DesiredScaleNumerator(1024, 2048, 8);
    ASSERT_EQ(numerator_normal, static_cast<unsigned>(5));

    auto numerator_overflow = JPEGImageDecoder::DesiredScaleNumerator(0x4000000, 0x4100000, 8);
...
{
    *os << "{\"" << param.file << "\", " << param.expected_success << "," << param.expected_uv_size.ToString() << "," << param.expect_yuv_decoding << "}";
}
...
TEST_P(ColorSpaceTest, YuvDecode)
{
    ...
    if (GetParam().expect_yuv_decoding) {
        ...
        ReadYUV(kLargeEnoughSize, jpeg_file.c_str(), gfx::Size(64, 64), GetParam().expected_uv_size,
...
TEST_P(ColorSpaceTest, RgbDecode)
{
    ...
    if (!GetParam().expect_yuv_decoding) {
        const auto jpeg_file = ("/images/resources/" + GetParam().file);
        scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(jpeg_file.c_str());
        ASSERT_TRUE(data);
...
const ColorSpaceTest::ParamType kColorSpaceTestParams[] = { { "cs-uma-grayscale.jpg", true }, { "cs-uma-rgb.jpg", true },
    // Each component is in a separate scan. Should not make a difference.
    { "cs-uma-rgb-non-interleaved.jpg", true }, { "cs-uma-cmyk.jpg", true },
    ...
    // CMYK.
    { "cs-uma-cmyk-jfif-marker.jpg", true }, { "cs-uma-ycck.jpg", true },
    // Contains CMYK data but uses a bad Adobe color transform, so libjpeg_turbo
    // will guess YCCK.
    { "cs-uma-cmyk-unknown-transform.jpg", true }, { "cs-uma-ycbcr-410.jpg", true, false }, { "cs-uma-ycbcr-411.jpg", true, false },
    ...
    // guess YCbCr.
    { "cs-uma-ycbcr-420-both-jfif-adobe.jpg", true, true, gfx::Size(32, 32) }, { "cs-uma-ycbcr-422.jpg", true, true, gfx::Size(32, 64) },
    { "cs-uma-ycbcr-440.jpg", true, false }, { "cs-uma-ycbcr-444.jpg", true, true, gfx::Size(64, 64) },
    // Contains RGB data but uses a bad Adobe color transform, so libjpeg_turbo
    // will guess YCbCr.
    { "cs-uma-rgb-unknown-transform.jpg", true, true, gfx::Size(64, 64) }, { "cs-uma-ycbcr-other.jpg", true, false },
    // Contains only 2 components. We expect the decode to fail and not produce
    // any samples.
    { "cs-uma-two-channels-jfif-marker.jpg", false } };

INSTANTIATE_TEST_SUITE_P(JPEGImageDecoderTest, ColorSpaceTest, ::testing::ValuesIn(kColorSpaceTestParams));
...
TEST(JPEGImageDecoderTest, PartialDataWithoutSize)
{
    ...
    scoped_refptr<SharedBuffer> partial_data = SharedBuffer::Create(full_data.data(), kDataLengthWithoutSize);
...
TEST(JPEGImageDecoderTest, PartialRgbDecodeBlocksYuvDecoding)
{
    ...
    auto partial_data = SharedBuffer::Create(full_data.data(), kJustEnoughDataToStartHeaderParsing);
...
TEST(JPEGImageDecoderTest, Gainmap)
{
    ...
    auto gainmap_decoder = std::make_unique<JPEGImageDecoder>(
        ImageDecoder::kAlphaNotPremultiplied, ColorBehavior::kTransformToSRGB, cc::AuxImage::kGainmap, ImageDecoder::kNoDecodedImageByteLimit);
...
TEST(JPEGImageDecoderTest, BppHistogramSmall)
{
    ...
    constexpr int kSample = (kFileSize * 100 * 8 + kImageArea / 2) / kImageArea; // = 245
    TestJpegBppHistogram("/images/resources/flowchart.jpg", "Blink.DecodedImage.JpegDensity.Count.0.4MP", kSample);
}
...
TEST(JPEGImageDecoderTest, BppHistogramSmall16x16)
{
    ...
    TestJpegBppHistogram("/images/resources/green.jpg", "Blink.DecodedImage.JpegDensity.Count.0.1MP", kSample);
}
...
TEST(JPEGImageDecoderTest, BppHistogramSmall900000)
{
    ...
    constexpr int kSample = (kFileSize * 100 * 8 + kImageArea / 2) / kImageArea; // = 12
    TestJpegBppHistogram("/images/resources/peach_900000.jpg", "Blink.DecodedImage.JpegDensity.Count.0.9MP", kSample);
}
...
TEST(JPEGImageDecoderTest, BppHistogramBig)
{
    ...
    TestJpegBppHistogram("/images/resources/bee.jpg", "Blink.DecodedImage.JpegDensity.Count.13MP", kSample);
}
...
TEST(JPEGImageDecoderTest, BppHistogramBig13000000)
{
    ...
    TestJpegBppHistogram("/images/resources/peach_13000000.jpg", "Blink.DecodedImage.JpegDensity.Count.13MP", kSample);
}
...
TEST(JPEGImageDecoderTest, BppHistogramHuge)
{
    ...
    constexpr int kSample = (kFileSize * 100 * 8 + kImageArea / 2) / kImageArea; // = 3
    TestJpegBppHistogram("/images/resources/peach.jpg", "Blink.DecodedImage.JpegDensity.Count.14+MP", kSample);
}
...
TEST(JPEGImageDecoderTest, BppHistogramHuge13000002)
{
    ...
    constexpr int kSample = (kFileSize * 100 * 8 + kImageArea / 2) / kImageArea; // = 3
    TestJpegBppHistogram("/images/resources/peach_13000002.jpg", "Blink.DecodedImage.JpegDensity.Count.14+MP", kSample);
}
...
TEST(JPEGImageDecoderTest, BppHistogramInvalid)
{
    ...
    std::unique_ptr<ImageDecoder> decoder = CreateJPEGDecoder();
    decoder->SetData(ReadFileToSharedBuffer("/images/resources/green-truncated.jpg"), true);
    ASSERT_TRUE(decoder->IsSizeAvailable());
    EXPECT_FALSE(decoder->Failed());
    ...
    EXPECT_THAT(histogram_tester.GetTotalCountsForPrefix("Blink.DecodedImage.JpegDensity.Count."), testing::ContainerEq(empty_counts));
}
