...
std::unique_ptr<ImageDecoder> CreatePNGDecoder(ImageDecoder::AlphaOption alpha_option)
{
    return CreatePngImageDecoder(alpha_option, ImageDecoder::kDefaultBitDepth, ColorBehavior::kTransformToSRGB, ImageDecoder::kNoDecodedImageByteLimit);
}
...
std::unique_ptr<ImageDecoder> Create16BitPNGDecoder()
{
    return CreatePngImageDecoder(
        ImageDecoder::kAlphaNotPremultiplied, ImageDecoder::kHighBitDepthToHalfFloat, ColorBehavior::kTag, ImageDecoder::kNoDecodedImageByteLimit);
}

std::unique_ptr<ImageDecoder> CreatePNGDecoderWithPngData(const char* png_file)
{
...
// data byte by byte.
void TestSizeByteByByte(const char* png_file, size_t bytes_needed_to_decode_size, gfx::Size expected_size)
{
    auto decoder = CreatePNGDecoder();
...
static PublicFrameInfo g_png_animated_frame_info[] = {
    { base::Milliseconds(500), { gfx::Point(0, 0), gfx::Size(5, 5) }, ImageFrame::kBlendAtopBgcolor, ImageFrame::kDisposeKeep },
    { base::Milliseconds(900), { gfx::Point(1, 1), gfx::Size(3, 1) }, ImageFrame::kBlendAtopBgcolor, ImageFrame::kDisposeOverwriteBgcolor },
    { base::Milliseconds(2000), { gfx::Point(1, 2), gfx::Size(3, 2) }, ImageFrame::kBlendAtopPreviousFrame, ImageFrame::kDisposeKeep },
    { base::Milliseconds(1500), { gfx::Point(1, 2), gfx::Size(3, 1) }, ImageFrame::kBlendAtopBgcolor, ImageFrame::kDisposeKeep },
};

void CompareFrameWithExpectation(const PublicFrameInfo& expected, ImageDecoder* decoder, size_t index)
{
...
void TestMissingDataBreaksDecoding(const char* png_file, size_t offset, size_t length)
{
    auto decoder = CreatePNGDecoder();
    Vector<char> data = ReadFile(png_file);
    ASSERT_FALSE(data.empty());

    scoped_refptr<SharedBuffer> invalid_data = SharedBuffer::Create(data.data(), offset);
    invalid_data->Append(data.data() + offset + length, data.size() - offset - length);
...
void TestInvalidFctlSize(const char* png_file, size_t offset_fctl, size_t expected_frame_count, bool should_fail)
{
    Vector<char> data = ReadFile(png_file);
    ASSERT_FALSE(data.empty());

    auto decoder = CreatePNGDecoder();
    scoped_refptr<SharedBuffer> invalid_data = SharedBuffer::Create(data.data(), offset_fctl);
    ...
    invalid_data->Append(data.data() + offset_post_fctl, data.size() - offset_post_fctl);
...
void TestProgressiveDecodingContinuesAfterFullData(const char* png_file, size_t offset_mid_first_frame)
{
    ...
    const ImageFrame* const frame_upfront = decoder_upfront->DecodeFrameBufferAtIndex(0);
    ASSERT_EQ(ImageFrame::kFrameComplete, frame_upfront->GetStatus());
    const unsigned hash_upfront = HashBitmap(frame_upfront->Bitmap());

    auto decoder = CreatePNGDecoder();
    scoped_refptr<SharedBuffer> partial_data = SharedBuffer::Create(full_data.data(), offset_mid_first_frame);
...
{
    TestRepetitionCount("/images/resources/"
                        "png-animated-idat-part-of-animation.png",
        6u);
...
{
    const char* png_file = "/images/resources/"
                           "png-animated-idat-part-of-animation.png";
...
{
    TestSizeByteByByte("/images/resources/"
                       "png-animated-idat-part-of-animation.png",
        141u, gfx::Size(5, 5));
    TestSizeByteByByte("/images/resources/"
                       "png-animated-idat-not-part-of-animation.png",
...
TEST_P(AnimatedPNGTests, ByteByByteMetaData)
{
    ...
    const char* source = data.data();
    scoped_refptr<SharedBuffer> partial_data = SharedBuffer::Create();
    for (size_t length = 1; length <= frame_offsets[kExpectedFrameCount - 1]; length++) {
        partial_data->Append(source++, 1u);
        decoder->SetData(partial_data.get(), false);
        EXPECT_FALSE(decoder->Failed());
        if (length < frame_offsets[frames_parsed]) {
            ...
            if (skia::IsRustyPngEnabled() && frames_parsed > 0) {
                ...
                EXPECT_NE(nullptr, decoder->DecodeFrameBufferAtIndex(frames_parsed - 1));
            }

            ASSERT_EQ(frames_parsed + 1, decoder->FrameCount());
            CompareFrameWithExpectation(g_png_animated_frame_info[frames_parsed], decoder.get(), frames_parsed);
            frames_parsed++;
        }
    }
...
TEST_P(AnimatedPNGTests, FailureDuringParsing)
{
    ...
    TestInvalidFctlSize("/images/resources/"
                        "png-animated-idat-part-of-animation.png",
        95u, 0u, false);
...
TEST_P(AnimatedPNGTests, ActlErrors)
{
    ...
    const size_t kAcTLSize = 20u;
    {
        ...
        scoped_refptr<SharedBuffer> no_actl_data = SharedBuffer::Create(data.data(), kOffsetActl);
        no_actl_data->Append(data.data() + kOffsetActl + kAcTLSize, data.size() - kOffsetActl - kAcTLSize);
    ...
    struct {
        size_t offset;
        bool should_fail;
    } kGRecs[] = { { 8u, false }, { kOffsetActl, false }, { 133u, false }, { 172u, true }, { 422u, true } };
    if (skia::IsRustyPngEnabled()) {
    ...
    for (const auto& rec : kGRecs) {
        ...
        scoped_refptr<SharedBuffer> extra_actl_data = SharedBuffer::Create(data.data(), offset);
               ...
               "cHRM_color_spin.png";
    {
        ...
        for (size_t times = 0; times < 2; times++) {
            scoped_refptr<SharedBuffer> extra_actl_data = SharedBuffer::Create(data2.data(), kPostIDATOffset);
            for (size_t i = 0; i < times; i++) {
                extra_actl_data->Append(ac_tl, kAcTLSize);
            }
            extra_actl_data->Append(data2.data() + kPostIDATOffset, data2.size() - kPostIDATOffset);
...
TEST_P(AnimatedPNGTests, fdatBeforeIdat)
{
    ...
    scoped_refptr<SharedBuffer> modified_data_buffer = SharedBuffer::Create(data.data(), kIdatOffset);
    // Copy fcTL and fdAT
...

    {
        ...
        const size_t kOffsetActl = 33u;
        const size_t kAcTLSize = 20u;
        scoped_refptr<SharedBuffer> modified_data_buffer2 = SharedBuffer::Create(modified_data.data(), kOffsetActl);
        modified_data_buffer2->Append(modified_data.data() + kOffsetActl + kAcTLSize, modified_data.size() - kOffsetActl - kAcTLSize);
        auto decoder = CreatePNGDecoder();
        ...
        scoped_refptr<SharedBuffer> modified_data3 = SharedBuffer::Create(modified_data2.data(), kInsertionOffset);
        modified_data3->Append(data.data() + kOffsetActl, kAcTLSize);
        modified_data3->Append(modified_data2.data() + kInsertionOffset, modified_data2.size() - kInsertionOffset);
...
TEST_P(AnimatedPNGTests, FrameOverflowX)
{
    ...
    scoped_refptr<SharedBuffer> modified_data = SharedBuffer::Create(data.data(), kFctlOffset);
...
TEST_P(AnimatedPNGTests, EmptyFdatFails)
{
    ...
    scoped_refptr<SharedBuffer> modified_data = SharedBuffer::Create(data.data(), kOffsetThirdFdat);
...
TEST_P(AnimatedPNGTests, VerifyFrameOutsideImageSizeFails)
{
    ...
    scoped_refptr<SharedBuffer> modified_data = SharedBuffer::Create(data.data(), kOffsetThirdFctl);
    const size_t kFctlSize = 38u;
    png_byte fctl[kFctlSize];
    ...
    modified_data->Append(const_cast<const char*>(reinterpret_cast<char*>(fctl)), kFctlSize);
    modified_data->Append(data.data() + kOffsetThirdFctl + kFctlSize, data.size() - kOffsetThirdFctl - kFctlSize);
...
TEST_P(AnimatedPNGTests, ProgressiveDecodingContinuesAfterFullData)
{
    ...
    TestProgressiveDecodingContinuesAfterFullData("/images/resources/"
                                                  "png-animated-idat-part-of-animation.png",
        160u);
}
...
{
    TestRandomDecodeAfterClearFrameBufferCache(&CreatePNGDecoder,
        "/images/resources/"
        "png-animated-idat-part-of-animation.png",
        2u);
}
...
{
    Vector<char> full_data = ReadFile("/images/resources/"
                                      "png-animated-idat-part-of-animation.png");
    ASSERT_FALSE(full_data.empty());
    auto decoder = CreatePNGDecoder();
    ...
    scoped_refptr<SharedBuffer> temp_data = SharedBuffer::Create(full_data.data(), kOffsetTwoFrames);
...
TEST_P(AnimatedPNGTests, VerifyIENDBeforeIDATInvalidatesDecoder)
{
    ...
    scoped_refptr<SharedBuffer> data = SharedBuffer::Create(full_data.data(), kOffsetIDAT);
...
TEST_P(AnimatedPNGTests, MixedDataChunks)
{
    ...
    scoped_refptr<SharedBuffer> data = SharedBuffer::Create(full_data.data(), kPostIDAT);
    const size_t kFcTLSize = 38u;
...
TEST_P(AnimatedPNGTests, VerifyInvalidDisposalAndBlending)
{
    ...
    scoped_refptr<SharedBuffer> data = SharedBuffer::Create(full_data.data(), kOffsetDisposalOp);
    ...
    data->Append(full_data.data() + kOffsetDisposalOp + 6u, full_data.size() - kOffsetDisposalOp - 6u);
...
TEST_P(AnimatedPNGTests, VerifySuccessfulFirstFrameDecodeAfterLaterFrame)
{
    ...
    scoped_refptr<SharedBuffer> data = SharedBuffer::Create(full_data.data(), kMiddleFirstFrame);
    decoder->SetData(data.get(), false);

    ASSERT_EQ(1u, decoder->FrameCount());
    ASSERT_EQ(ImageFrame::kFramePartial, decoder->DecodeFrameBufferAtIndex(0)->GetStatus());
    ...
    if (skia::IsRustyPngEnabled()) {
        ...
        // incremental decode (see http://review.skia.org/913917).  To make
        // progress, we need to finish the previous decode.
        EXPECT_EQ(ImageFrame::kFrameComplete, decoder->DecodeFrameBufferAtIndex(0)->GetStatus());
    }
    ASSERT_EQ(3u, decoder->FrameCount());
    ASSERT_EQ(ImageFrame::kFrameComplete, decoder->DecodeFrameBufferAtIndex(1)->GetStatus());
    ...
    ASSERT_EQ(kNotFound, decoder->DecodeFrameBufferAtIndex(1)->RequiredPreviousFrameIndex());
...
TEST_P(AnimatedPNGTests, DecodeFromIndependentFrame)
{
    ...
    data->Append(original_data.data() + data->size(), original_data.size() - data->size());
...
TEST_P(AnimatedPNGTests, SubsetFromIHDR)
{
    ...
    data->Append(original_data.data() + kFcTLOffset + kFcTLSize, original_data.size() - data->size());
    ASSERT_EQ(original_data.size(), data->size());
...
TEST_P(AnimatedPNGTests, Offset)
{
    ...
    scoped_refptr<SharedBuffer> original_data_buffer = SharedBuffer::Create(original_data);
    CreateDecodingBaseline(CreatePNGDecoder, original_data_buffer.get(), &baseline_hashes);
    ...
    auto decoder = CreatePngImageDecoder(
        ImageDecoder::kAlphaNotPremultiplied, ImageDecoder::kDefaultBitDepth, ColorBehavior::kTransformToSRGB, ImageDecoder::kNoDecodedImageByteLimit, kOffset);
...
TEST_P(AnimatedPNGTests, ExtraChunksBeforeIHDR)
{
    ...
    constexpr png_byte kExtraChunk[kExtraChunkSize] = { 0, 0, 0, 1, 't', 'R', 'c', 'N', 68, 82, 0, 87, 10 };
    data->Append(reinterpret_cast<const char*>(kExtraChunk), kExtraChunkSize);

    // Append the rest of the data from the original.
    data->Append(original_data.data() + kPngSignatureSize, original_data.size() - kPngSignatureSize);
...
TEST_P(StaticPNGTests, MetaDataTest)
{
    ...
    auto decoder = CreatePNGDecoderWithPngData("/images/resources/png-simple.png");
    EXPECT_EQ(kExpectedFrameCount, decoder->FrameCount());
...
{
    auto decoder = CreatePNGDecoderWithPngData("/images/resources/circle-trns-before-plte.png");
...
{
    TestProgressiveDecoding(&CreatePNGDecoder, "/images/resources/png-simple.png", 11u);
}

TEST_P(StaticPNGTests, ProgressiveDecodingContinuesAfterFullData)
{
    TestProgressiveDecodingContinuesAfterFullData("/images/resources/png-simple.png", 1000u);
}
...
static void TestHighBitDepthPNGDecoding(const PNGSample& png_sample, ImageDecoder* decoder)
{
    ...
    SkImageInfo info = SkImageInfo::Make(2, 2, kRGBA_F16_SkColorType, kUnpremul_SkAlphaType, image->refColorSpace());
    std::unique_ptr<uint8_t[]> decoded_pixels(new uint8_t[info.computeMinByteSize()]());
    ASSERT_TRUE(image->readPixels(info, decoded_pixels.get(), info.minRowBytes(), 0, 0));

    float decoded_pixels_float_32[16];
    ASSERT_TRUE(skcms_Transform(decoded_pixels.get(), skcms_PixelFormat_RGBA_hhhh, skcms_AlphaFormat_Unpremul, nullptr, decoded_pixels_float_32,
    ...
    for (int i = 0; i < 16; i++) {
        if (fabs(decoded_pixels_float_32[i] - expected_pixels[i]) > decoding_tolerance) {
            FAIL() << "Pixel comparison failed. File: " << png_sample.filename << ", component index: " << i << ", actual: " << decoded_pixels_float_32[i]
                   << ", expected: " << expected_pixels[i] << ", tolerance: " << decoding_tolerance;
        }
    }
}
...
static void FillPNGSamplesSourcePixels(Vector<PNGSample>& png_samples)
{
    ...
    static const Vector<float> source_pixels_opaque_srgb = { 0.4986953536, 0.5826657511, 0.7013199054, 1, // Top left pixel
    ...
    static const Vector<float> source_pixels_opaque_adobe_rgb = { 0.4448004883, 0.5216296635, 0.6506294347, 1, // Top left pixel
    ...
    static const Vector<float> source_pixels_opaque_p3 = { 0.515648127, 0.5802243076, 0.6912489509, 1, // Top left pixel
    ...
    static const Vector<float> source_pixels_opaque_e_srgb = { 0.6977539062, 0.5839843750, 0.4978027344, 1, // Top left pixel
    ...
    static const Vector<float> source_pixels_opaque_prophoto = { 0.5032883192, 0.5191271839, 0.6309147784, 1, // Top left pixel
    ...
    static const Vector<float> source_pixels_opaque_rec2020 = { 0.5390554665, 0.5766842145, 0.6851758602, 1, // Top left pixel
    ...
    static const Vector<float> source_pixels_transparent_srgb = { 0.3733272297, 0.4783093004, 0.6266422522, 0.8, // Top left pixel
    ...
    static const Vector<float> source_pixels_transparent_adobe_rgb = { 0.305943389, 0.4019836728, 0.5632867933, 0.8, // Top left pixel
    ...
    static const Vector<float> source_pixels_transparent_p3 = { 0.3945372702, 0.475257496, 0.6140383001, 0.8, // Top left pixel
    ...
    static const Vector<float> source_pixels_transparent_e_srgb = { 0.6230468750, 0.4782714844, 0.3723144531, 0.8, // Top left pixel
    ...
    static const Vector<float> source_pixels_transparent_prophoto = { 0.379064622, 0.3988708324, 0.5386282139, 0.8, // Top left pixel
    ...
    static const Vector<float> source_pixels_transparent_rec2020 = { 0.4237735561, 0.4708323796, 0.6064698253, 0.8, // Top left pixel
    ...
    for (PNGSample& png_sample : png_samples) {
        if (png_sample.color_space == "sRGB") {
            png_sample.expected_pixels = png_sample.is_transparent ? source_pixels_transparent_srgb : source_pixels_opaque_srgb;
        } else if (png_sample.color_space == "AdobeRGB") {
            png_sample.expected_pixels = png_sample.is_transparent ? source_pixels_transparent_adobe_rgb : source_pixels_opaque_adobe_rgb;
        } else if (png_sample.color_space == "DisplayP3") {
            png_sample.expected_pixels = png_sample.is_transparent ? source_pixels_transparent_p3 : source_pixels_opaque_p3;
        } else if (png_sample.color_space == "e-sRGB") {
            png_sample.expected_pixels = png_sample.is_transparent ? source_pixels_transparent_e_srgb : source_pixels_opaque_e_srgb;
        } else if (png_sample.color_space == "ProPhoto") {
            png_sample.expected_pixels = png_sample.is_transparent ? source_pixels_transparent_prophoto : source_pixels_opaque_prophoto;
        } else if (png_sample.color_space == "Rec2020") {
            png_sample.expected_pixels = png_sample.is_transparent ? source_pixels_transparent_rec2020 : source_pixels_opaque_rec2020;
        } else {
            NOTREACHED();
        }
    }
...
static Vector<PNGSample> GetPNGSamplesInfo(bool include_8bit_pngs)
{
    ...
    Vector<String> color_spaces = { "sRGB", "AdobeRGB", "DisplayP3", "e-sRGB", "ProPhoto", "Rec2020" };
    ...
    for (String color_space : color_spaces) {
        for (String alpha : alpha_status) {
            ...
            for (String interlace : interlace_status) {
                ...
                high_bit_depth_sample.filename = "2x2_16bit" + interlace + high_bit_depth_sample.filename;
            ...
            if (include_8bit_pngs) {
                ...
                regular_bit_depth_sample.filename = "2x2_8bit" + regular_bit_depth_sample.filename;
...
TEST_P(StaticPNGTests, ImageIsHighBitDepth)
{
    ...
    for (PNGSample& png_sample : png_samples) {
        ...
        std::unique_ptr<ImageDecoder> decoders[] = { CreatePNGDecoder(), Create16BitPNGDecoder() };
...
TEST_P(PNGTests, VerifyFrameCompleteBehavior)
{
    ...
    for (const auto& rec : g_recs) {
        ...
        auto decoder = CreatePNGDecoder();
        auto data = SharedBuffer::Create(full_data.data(), rec.offset_in_first_frame);
        decoder->SetData(data.get(), false);
...
{
    auto decoder = CreatePNGDecoderWithPngData("/images/resources/crbug702934.png");
    EXPECT_FALSE(decoder->IsSizeAvailable());
    EXPECT_TRUE(decoder->Failed());
}
...
{
    auto decoder = CreatePNGDecoderWithPngData("/images/resources/crbug807324.png");
...
TEST_P(PNGTests, cicp)
{
    ...
    if (skia::IsRustyPngEnabled()) {
        EXPECT_FALSE(skcms_TransferFunction_isPQish(&png_profile->trc[0].parametric));
...
TEST_P(PNGTests, HDRMetadata)
{
    ...
    ASSERT_TRUE(frame);
    ASSERT_FALSE(decoder->Failed());
    const std::optional<gfx::HDRMetadata> hdr_metadata = decoder->GetHDRMetadata();
...
TEST_P(AnimatedPNGTests, ApngTestSuiteDisposeOpNoneBasic)
{
    ...
    for (wtf_size_t i = 0; i < frame_count; i++) {
        SCOPED_TRACE(testing::Message() << "Testing DecodeFrameBufferAtIndex(" << i << ")");
        auto* frame = decoder->DecodeFrameBufferAtIndex(i);
        ASSERT_TRUE(frame);
        ASSERT_FALSE(decoder->Failed());
...
{
    auto decoder = CreatePNGDecoder();
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(kDecodersTestingDir, "private-critical-chunk-before-ihdr.png");
    EXPECT_FALSE(data->empty());
    decoder->SetData(data.get(), true);
    EXPECT_FALSE(decoder->IsSizeAvailable());
    EXPECT_TRUE(decoder->Failed());
...
INSTANTIATE_TEST_SUITE_P(RustEnabled, AnimatedPNGTests, ::testing::Values(RustFeatureState::kRustEnabled));
INSTANTIATE_TEST_SUITE_P(RustEnabled, PNGTests, ::testing::Values(RustFeatureState::kRustEnabled));
INSTANTIATE_TEST_SUITE_P(RustEnabled, StaticPNGTests, ::testing::Values(RustFeatureState::kRustEnabled));
#endif

INSTANTIATE_TEST_SUITE_P(RustDisabled, AnimatedPNGTests, ::testing::Values(RustFeatureState::kRustDisabled));
INSTANTIATE_TEST_SUITE_P(RustDisabled, PNGTests, ::testing::Values(RustFeatureState::kRustDisabled));
INSTANTIATE_TEST_SUITE_P(RustDisabled, StaticPNGTests, ::testing::Values(RustFeatureState::kRustDisabled));
