...
PNGImageReader::PNGImageReader(PNGImageDecoder* decoder, wtf_size_t initial_offset)
    : width_(0)
    , height_(0)
    , decoder_(decoder)
...
{
    png_ = png_create_read_struct(PNG_LIBPNG_VER_STRING, nullptr, pngFailed, nullptr);
    ...
    // remove this code.
    png_set_keep_unknown_chunks(png_, PNG_HANDLE_CHUNK_ALWAYS, reinterpret_cast<const png_byte*>("cICP\0cLLi\0mDCv"), 3);
    info_ = png_create_info_struct(png_);
    png_set_progressive_read_fn(png_, decoder_, nullptr, pngRowAvailable, pngFrameComplete);
...
{
    png_destroy_read_struct(png_ ? &png_ : nullptr, info_ ? &info_ : nullptr, nullptr);
    DCHECK(!png_ && !info_);
}
...
const png_byte* ReadAsConstPngBytep(const FastSharedBufferReader& reader, wtf_size_t read_offset, wtf_size_t length, char* buffer)
{
    DCHECK_LE(length, kPngReadBufferSize);
    return reinterpret_cast<const png_byte*>(reader.GetConsecutiveData(read_offset, length, buffer));
}
...
bool PNGImageReader::ShouldDecodeWithNewPNG(wtf_size_t index) const
{
    ...
    const bool frame_size_matches_ihdr = frame_info_[index].frame_rect == gfx::Rect(0, 0, width_, height_);
    if (index) {
...
bool PNGImageReader::Decode(SegmentReader& data, wtf_size_t index)
{
    ...
    if (!is_animated_) {
        ...
        DCHECK_EQ(0u, index);
        progressive_decode_offset_ += ProcessData(reader, frame_info_[0].start_offset + progressive_decode_offset_, 0);
        return true;
    }
    ...
    if (decode_with_new_png) {
        ...
        png_ = png_create_read_struct(PNG_LIBPNG_VER_STRING, nullptr, pngFailed, nullptr);
        info_ = png_create_info_struct(png_);
        png_set_progressive_read_fn(png_, decoder_, pngHeaderAvailable, pngRowAvailable, pngFrameComplete);
    }
...
void PNGImageReader::StartFrameDecoding(const FastSharedBufferReader& reader, wtf_size_t index)
{
    ...
    const png_byte* chunk = ReadAsConstPngBytep(reader, ihdr_offset_, kHeaderSize, read_buffer);
    ...
    ProcessData(reader, ihdr_offset_ + kHeaderSize, idat_offset_ - ihdr_offset_ - kHeaderSize);
}
...
bool PNGImageReader::ProgressivelyDecodeFirstFrame(const FastSharedBufferReader& reader)
{
...
void PNGImageReader::DecodeFrame(const FastSharedBufferReader& reader, wtf_size_t index)
{
...
static bool CheckCrc(const FastSharedBufferReader& reader, wtf_size_t chunk_start, wtf_size_t chunk_length)
{
    ...
    const png_byte* chunk = ReadAsConstPngBytep(reader, chunk_start + 4, chunk_length + 4, read_buffer);
...
bool PNGImageReader::Parse(SegmentReader& data, ParseQuery query)
{
    ...
    for (;;) {
        ...
        if (!base::CheckAdd(read_offset_, kChunkHeaderSize).AssignIfValid(&chunk_start_offset)) {
            // Overflow.
            return false;
        }
        ...
        const png_byte* chunk = ReadAsConstPngBytep(reader, read_offset_, kChunkHeaderSize, read_buffer);
        const wtf_size_t length = png_get_uint_32(chunk);
        if (length > PNG_UINT_31_MAX) {
            return false;
        }
        wtf_size_t chunk_end_offset;
        if (!base::CheckAdd(read_offset_, base::CheckAdd(12, length)).AssignIfValid(&chunk_end_offset)) {
            // Overflow.
            return false;
        }
        ...
        if (fdat || (idat && idat_is_part_of_animation_)) {
            ...
            if (fdat) {
                ...
                const png_byte* sequence_position = ReadAsConstPngBytep(reader, read_offset_ + 8, 4, read_buffer);
...
// processed.
wtf_size_t PNGImageReader::ProcessData(const FastSharedBufferReader& reader, wtf_size_t offset, wtf_size_t length)
{
    const char* segment;
    ...
    while (reader.size() > offset) {
        ...
        png_process_data(png_, info_, reinterpret_cast<png_byte*>(const_cast<char*>(segment)), segment_length);
...
bool PNGImageReader::ParseSize(const FastSharedBufferReader& reader)
{
    ...
    if (!parsed_signature_) {
        ...
        if (!base::CheckAdd(read_offset_, kNumSignatureBytes).AssignIfValid(&signature_end_offset)) {
            return false;
        }
        if (reader.size() < signature_end_offset) {
            return true;
        }
        const png_byte* chunk = ReadAsConstPngBytep(reader, read_offset_, kNumSignatureBytes, read_buffer);
        png_process_data(png_, info_, const_cast<png_byte*>(chunk), kNumSignatureBytes);
    ...
    for (png_uint_32 length = 0; reader.size() >= read_offset_ + 8;
        ...
        const png_byte* chunk = ReadAsConstPngBytep(reader, read_offset_, 8, read_buffer);
        ...
        if (IsChunk(chunk, "acTL")) {
            ...
            if (is_animated_ || length != 8 || !parsed_ihdr_ || !CheckCrc(reader, read_offset_, 8)) {
            ...
            if (length != 26 || !parsed_ihdr_ || !CheckCrc(reader, read_offset_, 26)) {
                ignore_animation_ = true;
                continue;
            }
            chunk = ReadAsConstPngBytep(reader, read_offset_ + 8, length, read_buffer);
            if (!ParseFrameInfo(chunk) || new_frame_.frame_rect != gfx::Rect(0, 0, width_, height_)) {
                ignore_animation_ = true;
                continue;
            }
            ...
            auto is_necessary_ancillary = [](const png_byte* chunk) {
                for (const char* tag : { "tRNS", "cHRM", "iCCP", "sRGB", "gAMA", "cICP", "cLLi", "mDCv", "eXIf" }) {
...
const PNGImageReader::FrameInfo& PNGImageReader::GetFrameInfo(wtf_size_t index) const
{
...
bool PNGImageReader::ParseFrameInfo(const png_byte* data)
{
    ...
    }
    {
        ...
        if (!base::CheckAdd(x_offset, frame_width).AssignIfValid(&frame_right) || frame_right > width_) {
            return false;
        }
    }
    {
        png_uint_32 frame_bottom;
        if (!base::CheckAdd(y_offset, frame_height).AssignIfValid(&frame_bottom) || frame_bottom > height_) {
            return false;
        }
    }

    new_frame_.frame_rect = gfx::Rect(x_offset, y_offset, frame_width, frame_height);
    ...
    case kAPNG_DISPOSE_OP_BACKGROUND:
        new_frame_.disposal_method = ImageFrame::DisposalMethod::kDisposeOverwriteBgcolor;
        break;
    case kAPNG_DISPOSE_OP_PREVIOUS:
        new_frame_.disposal_method = ImageFrame::DisposalMethod::kDisposeOverwritePrevious;
        break;
    default:
        return false;
    }
    ...
    case kAPNG_BLEND_OP_OVER:
        new_frame_.alpha_blend = ImageFrame::AlphaBlendSource::kBlendAtopPreviousFrame;
        break;
    default:
        return false;
    }
