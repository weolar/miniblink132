...
PNGImageDecoder::PNGImageDecoder(AlphaOption alpha_option, HighBitDepthDecodingOption high_bit_depth_decoding_option, ColorBehavior color_behavior,
    wtf_size_t max_decoded_bytes, wtf_size_t offset)
    : ImageDecoder(alpha_option, high_bit_depth_decoding_option, color_behavior, cc::AuxImage::kDefault, max_decoded_bytes)
    , offset_(offset)
    , current_frame_(0)
    ,
...
void PNGImageDecoder::Decode(wtf_size_t index)
{
    ...
    if (index >= frame_buffer_cache_.size() - 1 && IsAllDataReceived() && reader_ && !reader_->ParseCompleted()) {
        SetFailed();
    }
}
...
bool PNGImageDecoder::CanReusePreviousFrameBuffer(wtf_size_t index) const
{
    ...
    return frame_buffer_cache_[index].GetDisposalMethod() != ImageFrame::kDisposeOverwritePrevious;
}
...
static std::unique_ptr<ColorProfile> ParseCicpChunk(const png_unknown_chunk& chunk)
{
    ...
    const auto range = range_u8 == 1 ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED;
    ...
    sk_sp<SkColorSpace> sk_color_space = color_space.ToGfxColorSpace().GetAsFullRangeRGB().ToSkColorSpace();
    if (!sk_color_space) {
        return nullptr;
    }
...
static inline std::unique_ptr<ColorProfile> ReadColorProfile(png_structp png, png_infop info)
{
    png_unknown_chunkp unknown_chunks;
    size_t num_unknown_chunks = png_get_unknown_chunks(png, info, &unknown_chunks);
...
static inline void ReadHDRMetadata(png_structp png, png_infop info, std::optional<gfx::HDRMetadata>* hdr_metadata)
{
    std::optional<gfx::HdrMetadataCta861_3> clli;
    std::optional<gfx::HdrMetadataSmpteSt2086> mdcv;
    png_unknown_chunkp unknown_chunks;
    size_t num_unknown_chunks = png_get_unknown_chunks(png, info, &unknown_chunks);
    for (size_t chunk_index = 0; chunk_index < num_unknown_chunks; chunk_index++) {
        const auto& chunk = unknown_chunks[chunk_index];
        if (strcmp(reinterpret_cast<const char*>(chunk.name), "cLLi") == 0) {
            if (chunk.size != 8) {
                continue;
            }
            const uint32_t max_cll_times_10000 = (chunk.data[0] << 24) | (chunk.data[1] << 16) | (chunk.data[2] << 8) | chunk.data[3];
            const uint32_t max_fall_times_10000 = (chunk.data[4] << 24) | (chunk.data[5] << 16) | (chunk.data[6] << 8) | chunk.data[7];
        ...
        if (strcmp(reinterpret_cast<const char*>(chunk.name), "mDCv") == 0) {
            ...
            for (int i = 0; i < 8; ++i) {
                chromaticities_times_50000[i] = (chunk.data[2 * i] << 8) | chunk.data[2 * i + 1];
            }
            const uint32_t max_luminance_times_10000 = (chunk.data[16] << 24) | (chunk.data[17] << 16) | (chunk.data[18] << 8) | chunk.data[19];
            const uint32_t min_luminance_times_10000 = (chunk.data[20] << 24) | (chunk.data[21] << 16) | (chunk.data[22] << 8) | chunk.data[23];
            ...
            mdcv.emplace(primaries, max_luminance_times_10000 * 1e-4f, min_luminance_times_10000 * 1e-4f);
            continue;
        }
    }
    if (clli || mdcv) {
...
void PNGImageDecoder::SetBitDepth()
{
    ...
    decode_to_half_float_ = bit_depth_ == 16 && high_bit_depth_decoding_option_ == kHighBitDepthToHalfFloat &&
...
bool PNGImageDecoder::SetSize(unsigned width, unsigned height)
{
    ...
    return (width <= kMaxPNGSize) && (height <= kMaxPNGSize) && ImageDecoder::SetSize(width, height);
}
...
void PNGImageDecoder::HeaderAvailable()
{
    ...
    png_get_IHDR(png, info, &width, &height, &bit_depth, &color_type, &interlace_type, &compression_type, nullptr);
    ...
    if (png_get_eXIf_1(png, info, &exif_size, &exif_buffer) != 0) {
        ...
        if (exif_size != 0 && exif_buffer) {
            ApplyExifMetadata(SkData::MakeWithoutCopy(exif_buffer, exif_size).get(), gfx::Size(width, height));
        }
    }
#else
...
static inline void SetRGBAPremultiplyRowNeon(png_bytep src_ptr, const int pixel_count, ImageFrame::PixelData* dst_pixel, unsigned* const alpha_mask)
{
    assert(dst_pixel);
    assert(alpha_mask);
    ...
    uint64_t alpha_mask_u64 = vget_lane_u64(vreinterpret_u64_u8(alpha_mask_vector), 0);
    ...
    *alpha_mask &= alpha_mask_u64;
    ...
    for (; i > 0; i--, dst_pixel++, src_ptr += 4) {
        ImageFrame::SetRGBAPremultiply(dst_pixel, src_ptr[0], src_ptr[1], src_ptr[2], src_ptr[3]);
        *alpha_mask &= src_ptr[3];
    }
}

// Swizzle RGBA to SkPMColor order, and return the AND of all alpha channels.
static inline void SetRGBARawRowNeon(png_bytep src_ptr, const int pixel_count, ImageFrame::PixelData* dst_pixel, unsigned* const alpha_mask)
{
    assert(dst_pixel);
    assert(alpha_mask);
    ...
    uint64_t alpha_mask_u64 = vget_lane_u64(vreinterpret_u64_u8(vget_low_u8(alpha_mask_vector)), 0);
    alpha_mask_u64 &= vget_lane_u64(vreinterpret_u64_u8(vget_high_u8(alpha_mask_vector)), 0);
    alpha_mask_u64 &= (alpha_mask_u64 >> 32);
    alpha_mask_u64 &= (alpha_mask_u64 >> 16);
    alpha_mask_u64 &= (alpha_mask_u64 >> 8);
    *alpha_mask &= alpha_mask_u64;
    ...
    // Handle the tail elements.
    for (; i > 0; i--, dst_pixel++, src_ptr += 4) {
        ImageFrame::SetRGBARaw(dst_pixel, src_ptr[0], src_ptr[1], src_ptr[2], src_ptr[3]);
        *alpha_mask &= src_ptr[3];
    }
}
...
static inline void SetRGBARawRowNoAlphaNeon(png_bytep src_ptr, const int pixel_count, ImageFrame::PixelData* dst_pixel)
{
    assert(dst_pixel);
...
void PNGImageDecoder::RowAvailable(unsigned char* row_buffer, unsigned row_index, int)
{
    ...
    if (buffer.GetStatus() == ImageFrame::kFrameEmpty) {
        ...
        if (PNG_INTERLACE_ADAM7 == png_get_interlace_type(png, reader_->InfoPtr())) {
    ...
    if (!decode_to_half_float_) {
        ...
        if (has_alpha) {
            if (ColorProfileTransform* xform = ColorTransform()) {
                ...
                if (frame_buffer_cache_[current_frame_].GetAlphaBlendSource() == ImageFrame::kBlendAtopPreviousFrame) {
                    if (!color_transform_scanline_) {
                        ...
                        color_transform_scanline_.reset(new ImageFrame::PixelData[Size().width()]);
                    }
                ...
                bool color_conversion_successful = skcms_Transform(
                    src_ptr, color_format, alpha_format, xform->SrcProfile(), xform_dst, color_format, alpha_format, xform->DstProfile(), width);
                DCHECK(color_conversion_successful);
                src_ptr = png_bytep(xform_dst);
            }

            unsigned alpha_mask = 255;
            if (frame_buffer_cache_[current_frame_].GetAlphaBlendSource() == ImageFrame::kBlendAtopBgcolor) {
                if (buffer.PremultiplyAlpha()) {
#if (defined(__ARM_NEON__) || defined(__ARM_NEON))
                    SetRGBAPremultiplyRowNeon(src_ptr, width, dst_row, &alpha_mask);
#else
                    for (auto* dst_pixel = dst_row; dst_pixel < dst_row + width; dst_pixel++, src_ptr += 4) {
                        ImageFrame::SetRGBAPremultiply(dst_pixel, src_ptr[0], src_ptr[1], src_ptr[2], src_ptr[3]);
                        alpha_mask &= src_ptr[3];
                    }
#endif
                ...
                if (buffer.PremultiplyAlpha()) {
                    for (auto* dst_pixel = dst_row; dst_pixel < dst_row + width; dst_pixel++, src_ptr += 4) {
                        ImageFrame::BlendRGBAPremultiplied(dst_pixel, src_ptr[0], src_ptr[1], src_ptr[2], src_ptr[3]);
                        alpha_mask &= src_ptr[3];
                    }
                } else {
                    for (auto* dst_pixel = dst_row; dst_pixel < dst_row + width; dst_pixel++, src_ptr += 4) {
                        ImageFrame::BlendRGBARaw(dst_pixel, src_ptr[0], src_ptr[1], src_ptr[2], src_ptr[3]);
                        alpha_mask &= src_ptr[3];
                    }
                }
            }
...
#else
            for (auto* dst_pixel = dst_row; dst_pixel < dst_row + width; src_ptr += 3, ++dst_pixel) {
                ImageFrame::SetRGBARaw(dst_pixel, src_ptr[0], src_ptr[1], src_ptr[2], 255);
            }
#endif
            ...
            if (ColorProfileTransform* xform = ColorTransform()) {
                skcms_AlphaFormat alpha_format = skcms_AlphaFormat_Unpremul;
                bool color_conversion_successful = skcms_Transform(
                    dst_row, XformColorFormat(), alpha_format, xform->SrcProfile(), dst_row, XformColorFormat(), alpha_format, xform->DstProfile(), width);
                DCHECK(color_conversion_successful);
            }
        }
    } else { // for if (!decode_to_half_float_)
        ImageFrame::PixelDataF16* const dst_row_f16 = buffer.GetAddrF16(frame_rect.x(), y);
        ...
        // kBlendAtopBgcolor.
        DCHECK(frame_buffer_cache_[current_frame_].GetAlphaBlendSource() == ImageFrame::kBlendAtopBgcolor);
        ...
        auto src_format = has_alpha ? skcms_PixelFormat_RGBA_16161616BE : skcms_PixelFormat_RGB_161616BE;
        auto src_alpha_format = skcms_AlphaFormat_Unpremul;
        auto dst_alpha_format = (has_alpha && buffer.PremultiplyAlpha()) ? skcms_AlphaFormat_PremulAsEncoded : skcms_AlphaFormat_Unpremul;
        bool success = skcms_Transform(
            src_ptr, src_format, src_alpha_format, src_profile, dst_row_f16, skcms_PixelFormat_RGBA_hhhh, dst_alpha_format, dst_profile, width);
        DCHECK(success);
