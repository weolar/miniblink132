...
public:
    ...
    BMPImageReader(ImageDecoder* parent, wtf_size_t decoded_and_header_offset, wtf_size_t img_data_offset, bool is_in_ico);
...
private:
    ...
    {
        char buffer[2];
        const char* data = fast_reader_.GetConsecutiveData(decoded_offset_ + offset, 2, buffer);
    ...
    {
        char buffer[4];
        const char* data = fast_reader_.GetConsecutiveData(decoded_offset_ + offset, 4, buffer);
    ...
    {
        return is_top_down_ ? ((coord_.y() + num_rows) >= parent_->Size().height()) : ((coord_.y() - num_rows) < 0);
    }
    ...
    inline uint32_t ReadCurrentPixel(int bytes_per_pixel) const
    {
        ...
        char buffer[4];
        const char* encoded_pixel = fast_reader_.GetConsecutiveData(decoded_offset_, bytes_per_pixel, buffer);
    ...
    {
        uint8_t value = (pixel & bit_masks_[component]) >> bit_shifts_right_[component];
        return lookup_table_addresses_[component] ? lookup_table_addresses_[component][value] : value;
    }
    ...
    {
        SetRGBA(color_table_[color_index].rgb_red, color_table_[color_index].rgb_green, color_table_[color_index].rgb_blue, 0xff);
    }
    ...
    inline void FillRGBA(int end_coord, unsigned red, unsigned green, unsigned blue, unsigned alpha)
    {
