...
BMPImageDecoder::BMPImageDecoder(AlphaOption alpha_option, ColorBehavior color_behavior, wtf_size_t max_decoded_bytes)
...
void BMPImageDecoder::Decode(bool only_size)
{
    ...
    if (!DecodeHelper(only_size) && IsAllDataReceived()) {
        ...
        // has failed.
        SetFailed();
    } else if (!frame_buffer_cache_.empty() && (frame_buffer_cache_.front().GetStatus() == ImageFrame::kFrameComplete)) {
...
bool BMPImageDecoder::DecodeHelper(bool only_size)
{
    ...
    if ((decoded_offset_ < kSizeOfFileHeader) && !ProcessFileHeader(img_data_offset)) {
        return false;
    }

    if (!reader_) {
        reader_ = std::make_unique<BMPImageReader>(this, decoded_offset_, img_data_offset, false);
...
bool BMPImageDecoder::GetFileType(const FastSharedBufferReader& fast_reader, char* buffer, const char*& file_header, uint16_t& file_type) const
{
    if (data_->size() - decoded_offset_ < kSizeOfFileHeader) {
        return false;
    }
    file_header = fast_reader.GetConsecutiveData(decoded_offset_, kSizeOfFileHeader, buffer);
    file_type = (static_cast<uint16_t>(file_header[0]) << 8) | static_cast<uint8_t>(file_header[1]);
    return true;
}
