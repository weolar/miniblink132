...
scoped_refptr<SegmentReader> CopyToROBufferSegmentReader(scoped_refptr<SegmentReader> input)
{
    RWBuffer rw_buffer;
    size_t position = 0;
    for (base::span<const uint8_t> segment = input->GetSomeData(position); !segment.empty(); segment = input->GetSomeData(position)) {
...
scoped_refptr<SegmentReader> CopyToDataSegmentReader(scoped_refptr<SegmentReader> input)
{
...
struct SegmentReaders {
    ...
    {
        segment_readers[0] = SegmentReader::CreateFromSharedBuffer(std::move(input));
...
TEST(FastSharedBufferReaderTest, nonSequentialReads)
{
    ...
    for (auto segment_reader : reader_struct.segment_readers) {
        ...
        // read eventually.
        char temp_buffer[17];
        for (size_t data_position = 0; data_position + sizeof(temp_buffer) < sizeof(reference_data); data_position += sizeof(temp_buffer)) {
            const char* block = reader.GetConsecutiveData(data_position, sizeof(temp_buffer), temp_buffer);
            ASSERT_FALSE(memcmp(block, reference_data + data_position, sizeof(temp_buffer)));
        }
    }
}
...
TEST(FastSharedBufferReaderTest, readBackwards)
{
    ...
    for (auto segment_reader : reader_struct.segment_readers) {
        ...
        // read eventually.
        char temp_buffer[17];
        for (size_t data_offset = sizeof(temp_buffer); data_offset < sizeof(reference_data); data_offset += sizeof(temp_buffer)) {
            const char* block = reader.GetConsecutiveData(sizeof(reference_data) - data_offset, sizeof(temp_buffer), temp_buffer);
            ASSERT_FALSE(memcmp(block, reference_data + sizeof(reference_data) - data_offset, sizeof(temp_buffer)));
        }
    }
}
...
TEST(SegmentReaderTest, readPastEndThenRead)
{
    ...
    data->Append(base::span(reference_data).subspan(kDefaultSegmentTestSize, kDefaultSegmentTestSize));
...
TEST(SegmentReaderTest, getAsSkData)
{
    ...
    for (size_t i = 0; i < 4; ++i) {
        data->Append(base::span(reference_data).subspan(i * kDefaultSegmentTestSize, kDefaultSegmentTestSize));
    }
    SegmentReaders reader_struct(data);
    for (auto segment_reader : reader_struct.segment_readers) {
        ...
        size_t position = 0;
        for (base::span<const uint8_t> segment = segment_reader->GetSomeData(position); !segment.empty(); segment = segment_reader->GetSomeData(position)) {
...
TEST(SegmentReaderTest, variableSegments)
{
    ...
    scoped_refptr<SegmentReader> segment_reader;
    {
        ...
        rw_buffer.Append(reference_data + kDefaultSegmentTestSize, 2 * kDefaultSegmentTestSize);
        rw_buffer.Append(reference_data + 3 * kDefaultSegmentTestSize, .5 * kDefaultSegmentTestSize);

        segment_reader = SegmentReader::CreateFromROBuffer(rw_buffer.MakeROBufferSnapshot());
    }
