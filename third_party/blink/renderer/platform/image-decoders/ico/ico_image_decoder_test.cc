...
{
    return std::make_unique<ICOImageDecoder>(ImageDecoder::kAlphaNotPremultiplied, ColorBehavior::kTransformToSRGB, ImageDecoder::kNoDecodedImageByteLimit);
}
} // namespace
...
TEST(ICOImageDecoderTests, trunctedIco)
{
    ...
    scoped_refptr<SharedBuffer> truncated_data = SharedBuffer::Create(data.data(), data.size() / 2);
...
TEST(ICOImageDecoderTests, errorInPngInIco)
{
    ...
    scoped_refptr<SharedBuffer> modified_data = SharedBuffer::Create(data.data(), kCrcOffset);
    Vector<char> bad_crc(kCrcSize, 0);
    modified_data->Append(bad_crc);
    modified_data->Append(data.data() + kCrcOffset + kCrcSize, data.size() - kCrcOffset - kCrcSize);
...
{
    TestByteByByteDecode(&CreateICODecoder, "/images/resources/png-in-ico.ico", 1u, kAnimationNone);
    TestByteByByteDecode(&CreateICODecoder, "/images/resources/2entries.ico", 2u, kAnimationNone);
    TestByteByByteDecode(&CreateICODecoder, "/images/resources/greenbox-3frames.cur", 3u, kAnimationNone);
    TestByteByByteDecode(&CreateICODecoder, "/images/resources/icon-without-and-bitmap.ico", 1u, kAnimationNone);
    TestByteByByteDecode(&CreateICODecoder, "/images/resources/1bit.ico", 1u, kAnimationNone);
    TestByteByByteDecode(&CreateICODecoder, "/images/resources/bug653075.ico", 2u, kAnimationNone);
}
...
TEST(ICOImageDecoderTests, NullData)
{
    ...
    scoped_refptr<SharedBuffer> truncated_data = SharedBuffer::Create(ico_file_data.data(), kSizeOfBadBlock);
    auto decoder = CreateICODecoder();
...
protected:
    ...
    {
        return std::make_unique<ICOImageDecoder>(ImageDecoder::kAlphaPremultiplied, ColorBehavior::kTransformToSRGB, ImageDecoder::kNoDecodedImageByteLimit);
    }
};
...
TEST_F(ICOImageDecoderCorpusTest, ImageNonZeroFrameIndex)
{
    ...
    const base::FilePath md5_sum_path(GetMD5SumPath(multisize_icon_path).value() + FILE_PATH_LITERAL("2"));
