...
ICOImageDecoder::ICOImageDecoder(AlphaOption alpha_option, ColorBehavior color_behavior, wtf_size_t max_decoded_bytes)
...
{
    DEFINE_STATIC_LOCAL(const AtomicString, ico_mime_type, ("image/vnd.microsoft.icon"));
...
void ICOImageDecoder::OnSetData(scoped_refptr<SegmentReader> data)
{
    ...
    for (BMPReaders::iterator i(bmp_readers_.begin()); i != bmp_readers_.end(); ++i) {
        if (*i) {
            (*i)->SetData(data);
        }
    }
...
{
    return (index && (index < dir_entries_.size())) ? dir_entries_[index].size_ : Size();
}
...
bool ICOImageDecoder::SetSize(unsigned width, unsigned height)
{
    ...
    // the icon directory.
    return frame_size_.IsEmpty() ? ImageDecoder::SetSize(width, height) : ((gfx::Size(width, height) == frame_size_) || SetFailed());
}
...
bool ICOImageDecoder::HotSpotAtIndex(wtf_size_t index, gfx::Point& hot_spot) const
{
...
bool ICOImageDecoder::CompareEntries(const IconDirectoryEntry& a, const IconDirectoryEntry& b)
{
    ...
    return (a_entry_area == b_entry_area) ? (a.bit_count_ > b.bit_count_) : (a_entry_area > b_entry_area);
}
...
void ICOImageDecoder::Decode(wtf_size_t index, bool only_size)
{
    ...
    // has failed.
    if ((!DecodeDirectory() || (!only_size && !DecodeAtIndex(index))) && IsAllDataReceived()) {
        SetFailed();
        ...
        // cleared.)
    } else if ((frame_buffer_cache_.size() > index) && (frame_buffer_cache_[index].GetStatus() == ImageFrame::kFrameComplete)) {
...
bool ICOImageDecoder::DecodeDirectory()
{
    ...
    return (decoded_offset_ >= (kSizeOfDirectory + (dir_entries_count_ * kSizeOfDirEntry))) || ProcessDirectoryEntries();
}
...
bool ICOImageDecoder::DecodeAtIndex(wtf_size_t index)
{
    ...
    if (image_type == BMP) {
        if (!bmp_readers_[index]) {
            bmp_readers_[index] = std::make_unique<BMPImageReader>(this, dir_entry.image_offset_, 0, true);
    ...
    if (!png_decoders_[index]) {
        AlphaOption alpha_option = premultiply_alpha_ ? kAlphaPremultiplied : kAlphaNotPremultiplied;
        png_decoders_[index]
            = CreatePngImageDecoder(alpha_option, ImageDecoder::kDefaultBitDepth, color_behavior_, max_decoded_bytes_, dir_entry.image_offset_);
...
bool ICOImageDecoder::ProcessDirectoryEntries()
{
    ...
    if ((decoded_offset_ > data_->size()) || ((data_->size() - decoded_offset_) < (dir_entries_count_ * kSizeOfDirEntry))) {
        return false;
    }
    ...
    // entries.
    for (IconDirectoryEntries::iterator i(dir_entries_.begin()); i != dir_entries_.end(); ++i) {
    ...
    return SetSize(static_cast<unsigned>(dir_entry.size_.width()), static_cast<unsigned>(dir_entry.size_.height()));
}
