...
size_t ReadFromSegmentStream(SegmentStream&, size_t amount_to_read = kInsideBufferPosition);
size_t PeekIntoSegmentStream(SegmentStream&, size_t amount_to_peek = kInsideBufferPosition);
...
TEST(SegmentStreamTest, MoveConstructorShouldSetIsClearedWhenRhsIsCleared)
{
    ...
    SegmentStream move_constructed_segment_stream = std::move(cleared_segment_stream);

    ASSERT_TRUE(IsCleared(move_constructed_segment_stream));
}

TEST(SegmentStreamTest, MoveConstructorShouldUnsetIsClearedWhenRhsIsNotCleared)
{
    SegmentStream uncleared_segment_stream = CreatePopulatedSegmentStream();
    ASSERT_FALSE(IsCleared(uncleared_segment_stream));

    SegmentStream move_constructed_segment_stream = std::move(uncleared_segment_stream);
...
TEST(SegmentStreamTest, MoveConstructorShouldSetIsAtEndWhenRhsIsAtEnd)
{
    ...
    SegmentStream move_constructed_segment_stream = std::move(at_end_segment_stream);
...
TEST(SegmentStreamTest, MoveConstructorShouldUnsetIsAtEndWhenRhsIsNotAtEnd)
{
    ...
    SegmentStream move_constructed_segment_stream = std::move(not_at_end_segment_stream);
...
TEST(SegmentStreamTest, MoveContructorShouldCopyRhsPosition)
{
    ...
    ASSERT_EQ(kInsideBufferPosition, move_constructed_segment_stream.getPosition());
}
...
TEST(SegmentStreamTest, MoveAssignmentOperatorShouldSetIsClearedWhenRhsIsCleared)
{
    SegmentStream cleared_segment_stream;
    ASSERT_TRUE(IsCleared(cleared_segment_stream));
...
TEST(SegmentStreamTest, MoveAssignmentOperatorShouldUnsetIsClearedWhenRhsIsNotCleared)
{
    SegmentStream uncleared_segment_stream = CreatePopulatedSegmentStream();
    ASSERT_FALSE(IsCleared(uncleared_segment_stream));
...
TEST(SegmentStreamTest, MoveAssignmentOperatorShouldUnsetIsAtEndWhenRhsIsNotAtEnd)
{
    SegmentStream not_at_end_segment_stream = CreatePopulatedSegmentStream();
    ASSERT_FALSE(IsAtEnd(not_at_end_segment_stream));
...
TEST(SegmentStreamTest, ReadShouldConsumeBufferWithoutGoingPastTheEnd)
{
    ...
    const size_t amount_read = ReadFromSegmentStream(segment_stream, kPastEndOfBufferPosition);
...
TEST(SegmentStreamTest, PeekShouldConsumeBufferWithoutGoingPastTheEnd)
{
    ...
    const size_t amount_peeked = PeekIntoSegmentStream(segment_stream, kPastEndOfBufferPosition);
...
TEST(SegmentStreamTest, BasicReadingTest)
{
    ...
    scoped_refptr<SegmentReader> reader = SegmentReader::CreateFromSharedBuffer(buffer);
    ...
    std::string_view result = base::as_string_view(base::span(output).first(read_bytes));
...
::testing::AssertionResult PositionIsInsideBuffer(const SegmentStream& segment_stream)
{
    if (segment_stream.getPosition() == kInsideBufferPosition) {
        return ::testing::AssertionSuccess();
    }

    return ::testing::AssertionFailure() << "SegmentStream position is not inside the buffer";
}

::testing::AssertionResult PositionIsAtEndOfBuffer(const SegmentStream& segment_stream)
{
    if (segment_stream.getPosition() == kBufferAllocationSize) {
        return ::testing::AssertionSuccess();
    }

    return ::testing::AssertionFailure() << "SegmentStream position is not at the end of the buffer";
}
...
::testing::AssertionResult LengthIsAllocationSize(const SegmentStream& segment_stream)
{
    if (segment_stream.getLength() == kBufferAllocationSize) {
        return ::testing::AssertionSuccess();
    }

    return ::testing::AssertionFailure() << "SegmentStream length is not the allocation size";
}
...
scoped_refptr<SegmentReader> CreateSegmentReader()
{
    ...
    scoped_refptr<SharedBuffer> shared_buffer = SharedBuffer::Create(raw_buffer.data(), kBufferAllocationSize);

    scoped_refptr<SegmentReader> segment_reader = SegmentReader::CreateFromSharedBuffer(std::move(shared_buffer));

    return segment_reader;
}

size_t ReadFromSegmentStream(SegmentStream& segment_stream, size_t amount_to_read)
{
    Vector<char> read_buffer(kBufferAllocationSize);
    return segment_stream.read(read_buffer.data(), amount_to_read);
}

size_t PeekIntoSegmentStream(SegmentStream& segment_stream, size_t amount_to_peek)
{
