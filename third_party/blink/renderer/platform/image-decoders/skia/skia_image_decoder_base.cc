...
ImageFrame::DisposalMethod ConvertDisposalMethod(SkCodecAnimation::DisposalMethod disposal_method)
{
...
ImageFrame::AlphaBlendSource ConvertAlphaBlendSource(SkCodecAnimation::Blend blend)
{
    switch (blend) {
...
SkiaImageDecoderBase::SkiaImageDecoderBase(AlphaOption alpha_option, ColorBehavior color_behavior, wtf_size_t max_decoded_bytes, wtf_size_t reading_offset)
...
void SkiaImageDecoderBase::OnSetData(scoped_refptr<SegmentReader> data)
{
    ...
    if (segment_stream_) {
        ...
        auto segment_stream = std::make_unique<SegmentStream>(base::checked_cast<size_t>(reading_offset_));
        ...
        case SkCodec::kSuccess: {
            ...
            if (!SetSize(static_cast<unsigned>(image_info.width()), static_cast<unsigned>(image_info.height()))) {
                return;
            }
...
base::TimeDelta SkiaImageDecoderBase::FrameDurationAtIndex(wtf_size_t index) const
{
...
wtf_size_t SkiaImageDecoderBase::ClearCacheExceptFrame(wtf_size_t index)
{
    ...
    if (index < frame_buffer_cache_.size()) {
        ...
        if (frame.RequiredPreviousFrameIndex() != kNotFound
            && (!FrameStatusSufficientForSuccessors(index) || frame.GetDisposalMethod() == ImageFrame::kDisposeOverwritePrevious)) {
...
void SkiaImageDecoderBase::InitializeNewFrame(wtf_size_t index)
{
    ...
    if (frame_info.fRequiredFrame == SkCodec::kNoFrame) {
        ...
        required_previous_frame_index = static_cast<wtf_size_t>(frame_info.fRequiredFrame);
    }
...
void SkiaImageDecoderBase::Decode(wtf_size_t index)
{
    ...
    while (!frames_to_decode.empty()) {
        ...
        if (frame.GetStatus() == ImageFrame::kFrameEmpty) {
            wtf_size_t required_previous_frame_index = frame.RequiredPreviousFrameIndex();
            if (required_previous_frame_index == kNotFound) {
                frame.AllocatePixelData(Size().width(), Size().height(), ColorSpaceForSkImages());
                ...
                if ((!CanReusePreviousFrameBuffer(current_frame_index) || !frame.TakeBitmapDataIfWritable(&previous_frame))
        ...
        if (frame.GetStatus() == ImageFrame::kFrameInitialized) {
            ...
            SkImageInfo image_info = codec_->getInfo().makeColorType(kN32_SkColorType).makeColorSpace(ColorSpaceForSkImages()).makeAlphaType(alpha_type);
            ...
            SkCodec::Result start_incremental_decode_result
                = codec_->startIncrementalDecode(image_info, frame.Bitmap().getPixels(), frame.Bitmap().rowBytes(), &options);
        ...
        case SkCodec::kSuccess: {
            ...
            bool frame_info_received = codec_->getFrameInfo(current_frame_index, &frame_info);
            DCHECK(frame_info_received);
            frame.SetHasAlpha(frame_info.fAlphaType != SkAlphaType::kOpaque_SkAlphaType);
...
bool SkiaImageDecoderBase::CanReusePreviousFrameBuffer(wtf_size_t frame_index) const
{
    DCHECK_LT(frame_index, frame_buffer_cache_.size());
    return frame_buffer_cache_[frame_index].GetDisposalMethod() != ImageFrame::kDisposeOverwritePrevious;
}

wtf_size_t SkiaImageDecoderBase::GetViableReferenceFrameIndex(wtf_size_t dependent_index) const
{
    DCHECK_LT(dependent_index, frame_buffer_cache_.size());

    wtf_size_t required_previous_frame_index = frame_buffer_cache_[dependent_index].RequiredPreviousFrameIndex();
    ...
    // recent.
    for (wtf_size_t i = dependent_index - 1; i != required_previous_frame_index; i--) {
