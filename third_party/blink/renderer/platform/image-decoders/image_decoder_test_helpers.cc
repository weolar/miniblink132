...
scoped_refptr<SharedBuffer> ReadFileToSharedBuffer(const char* dir, const char* file_name)
{
...
{
    return StringHasher::HashMemory({ static_cast<const uint8_t*>(bitmap.getPixels()), bitmap.computeByteSize() });
}

void CreateDecodingBaseline(DecoderCreator create_decoder, SharedBuffer* data, Vector<unsigned>* baseline_hashes)
{
...
void TestByteByByteDecode(DecoderCreator create_decoder, SharedBuffer* shared_data, size_t expected_frame_count, int expected_repetition_count)
{
    ...
    scoped_refptr<SharedBuffer> source_data[2] = { SharedBuffer::Create(), SharedBuffer::Create() };
...
static void TestRandomFrameDecode(DecoderCreator create_decoder, SharedBuffer* full_data, size_t skipping_step)
{
    Vector<unsigned> baseline_hashes;
...
static void TestRandomDecodeAfterClearFrameBufferCache(DecoderCreator create_decoder, SharedBuffer* data, size_t skipping_step)
{
    Vector<unsigned> baseline_hashes;
    ...
    std::unique_ptr<ImageDecoder> decoder = create_decoder();
    decoder->SetData(data, true);
    for (size_t clear_except_frame = 0; clear_except_frame < frame_count; ++clear_except_frame) {
...
static void TestDecodeAfterReallocatingData(DecoderCreator create_decoder, SharedBuffer* data)
{
    std::unique_ptr<ImageDecoder> decoder = create_decoder();
    ...
    scoped_refptr<SharedBuffer> reallocated_data = SharedBuffer::Create(std::move(copy));
...
static void TestByteByByteSizeAvailable(
    DecoderCreator create_decoder, SharedBuffer* data, size_t frame_offset, bool has_color_space, int expected_repetition_count)
{
    std::unique_ptr<ImageDecoder> decoder = create_decoder();
...
static void TestProgressiveDecoding(DecoderCreator create_decoder, SharedBuffer* full_buffer, size_t increment)
{
...
void TestUpdateRequiredPreviousFrameAfterFirstDecode(DecoderCreator create_decoder, SharedBuffer* full_buffer)
{
    const Vector<char> full_data = full_buffer->CopyAs<Vector<char>>();
    std::unique_ptr<ImageDecoder> decoder = create_decoder();
    ...
    } while (!decoder->FrameCount() || decoder->DecodeFrameBufferAtIndex(0)->GetStatus() == ImageFrame::kFrameEmpty);

    EXPECT_EQ(kNotFound, decoder->DecodeFrameBufferAtIndex(0)->RequiredPreviousFrameIndex());
    unsigned frame_count = decoder->FrameCount();
    for (size_t i = 1; i < frame_count; ++i) {
        EXPECT_EQ(i - 1, decoder->DecodeFrameBufferAtIndex(i)->RequiredPreviousFrameIndex());
    }

    decoder->SetData(full_buffer, true);
    for (size_t i = 0; i < frame_count; ++i) {
        EXPECT_EQ(kNotFound, decoder->DecodeFrameBufferAtIndex(i)->RequiredPreviousFrameIndex());
    }
}

void TestByteByByteDecode(DecoderCreator create_decoder, const char* file, size_t expected_frame_count, int expected_repetition_count)
{
    SCOPED_TRACE(file);
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(file);
    ASSERT_TRUE(data.get());
    TestByteByByteDecode(create_decoder, data.get(), expected_frame_count, expected_repetition_count);
}
void TestByteByByteDecode(DecoderCreator create_decoder, const char* dir, const char* file, size_t expected_frame_count, int expected_repetition_count)
{
    SCOPED_TRACE(file);
...
void TestRandomFrameDecode(DecoderCreator create_decoder, const char* file, size_t skipping_step)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(file);
    ASSERT_TRUE(data.get());
    SCOPED_TRACE(file);
...
void TestRandomFrameDecode(DecoderCreator create_decoder, const char* dir, const char* file, size_t skipping_step)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(dir, file);
    ASSERT_TRUE(data.get());
    SCOPED_TRACE(file);
...
void TestRandomDecodeAfterClearFrameBufferCache(DecoderCreator create_decoder, const char* file, size_t skipping_step)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(file);
    ASSERT_TRUE(data.get());
    SCOPED_TRACE(file);
    TestRandomDecodeAfterClearFrameBufferCache(create_decoder, data.get(), skipping_step);
}

void TestRandomDecodeAfterClearFrameBufferCache(DecoderCreator create_decoder, const char* dir, const char* file, size_t skipping_step)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(dir, file);
    ASSERT_TRUE(data.get());
    SCOPED_TRACE(file);
...
void TestDecodeAfterReallocatingData(DecoderCreator create_decoder, const char* file)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(file);
    ASSERT_TRUE(data.get());
...
void TestDecodeAfterReallocatingData(DecoderCreator create_decoder, const char* dir, const char* file)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(dir, file);
    ASSERT_TRUE(data.get());
    TestDecodeAfterReallocatingData(create_decoder, data.get());
...
void TestByteByByteSizeAvailable(DecoderCreator create_decoder, const char* file, size_t frame_offset, bool has_color_space, int expected_repetition_count)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(file);
    ASSERT_TRUE(data.get());
    TestByteByByteSizeAvailable(create_decoder, data.get(), frame_offset, has_color_space, expected_repetition_count);
}

void TestByteByByteSizeAvailable(
    DecoderCreator create_decoder, const char* dir, const char* file, size_t frame_offset, bool has_color_space, int expected_repetition_count)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(dir, file);
    ASSERT_TRUE(data.get());
    TestByteByByteSizeAvailable(create_decoder, data.get(), frame_offset, has_color_space, expected_repetition_count);
...
void TestProgressiveDecoding(DecoderCreator create_decoder, const char* file, size_t increment)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(file);
    ASSERT_TRUE(data.get());
...
void TestProgressiveDecoding(DecoderCreator create_decoder, const char* dir, const char* file, size_t increment)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(dir, file);
    ASSERT_TRUE(data.get());
    TestProgressiveDecoding(create_decoder, data.get(), increment);
...
void TestUpdateRequiredPreviousFrameAfterFirstDecode(DecoderCreator create_decoder, const char* dir, const char* file)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(dir, file);
    ASSERT_TRUE(data.get());
...
void TestUpdateRequiredPreviousFrameAfterFirstDecode(DecoderCreator create_decoder, const char* file)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(file);
    ASSERT_TRUE(data.get());
    TestUpdateRequiredPreviousFrameAfterFirstDecode(create_decoder, data.get());
...
static void VerifyFramesMatch(const char* file, const ImageFrame* const a, const ImageFrame* const b)
{
...
void TestAlphaBlending(DecoderCreatorWithAlpha create_decoder, const char* file)
{
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(file);
    ASSERT_TRUE(data.get());

    std::unique_ptr<ImageDecoder> decoder_a = create_decoder(ImageDecoder::kAlphaPremultiplied);
    decoder_a->SetData(data.get(), true);

    std::unique_ptr<ImageDecoder> decoder_b = create_decoder(ImageDecoder::kAlphaNotPremultiplied);
    ...
    for (size_t i = 0; i < frame_count; ++i) {
        VerifyFramesMatch(file, decoder_a->DecodeFrameBufferAtIndex(i), decoder_b->DecodeFrameBufferAtIndex(i));
    }
}

void TestBppHistogram(
    DecoderCreator create_decoder, const char* image_type, const char* image_name, const char* histogram_name, base::HistogramBase::Sample sample)
{
    ...
    EXPECT_THAT(
        histogram_tester.GetTotalCountsForPrefix(base::StrCat({ "Blink.DecodedImage.", image_type, "Density.Count." })), testing::ContainerEq(expected_counts));
}
