...
gfx::ColorSpace GetColorSpace(crabbyavif::avifColorPrimaries color_primaries, crabbyavif::avifTransferCharacteristics transfer_characteristics,
    crabbyavif::avifMatrixCoefficients matrix_coefficients, crabbyavif::avifRange yuv_range, bool grayscale)
{
    ...
    const auto primaries = color_primaries == crabbyavif::AVIF_COLOR_PRIMARIES_UNSPECIFIED ? crabbyavif::AVIF_COLOR_PRIMARIES_BT709 : color_primaries;
    const auto transfer = transfer_characteristics == crabbyavif::AVIF_TRANSFER_CHARACTERISTICS_UNSPECIFIED ? crabbyavif::AVIF_TRANSFER_CHARACTERISTICS_SRGB
                                                                                                            : transfer_characteristics;
    const auto matrix = (grayscale || matrix_coefficients == crabbyavif::AVIF_MATRIX_COEFFICIENTS_UNSPECIFIED) ? crabbyavif::AVIF_MATRIX_COEFFICIENTS_BT601
                                                                                                               : matrix_coefficients;
    const auto range = yuv_range == crabbyavif::AVIF_RANGE_FULL ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED;
...
{
    const bool grayscale = image->yuvFormat == crabbyavif::AVIF_PIXEL_FORMAT_YUV400;
...
sk_sp<SkColorSpace> GetAltImageColorSpace(const crabbyavif::avifImage& image)
{
    ...
    } else if (gain_map->altColorPrimaries != crabbyavif::AVIF_COLOR_PRIMARIES_UNSPECIFIED) {
...
CrabbyAVIFImageDecoder::CrabbyAVIFImageDecoder(AlphaOption alpha_option, HighBitDepthDecodingOption hbd_option, ColorBehavior color_behavior,
    cc::AuxImage aux_image, wtf_size_t max_decoded_bytes, AnimationOption animation_option)
...
wtf_size_t CrabbyAVIFImageDecoder::DecodedYUVWidthBytes(cc::YUVIndex index) const
{
    DCHECK(IsDecodedSizeAvailable());
...
void CrabbyAVIFImageDecoder::DecodeToYUV()
{
    ...
    static_assert(cc::YUVIndex::kY == static_cast<cc::YUVIndex>(crabbyavif::AVIF_CHAN_Y), "");
    static_assert(cc::YUVIndex::kU == static_cast<cc::YUVIndex>(crabbyavif::AVIF_CHAN_U), "");
    static_assert(cc::YUVIndex::kV == static_cast<cc::YUVIndex>(crabbyavif::AVIF_CHAN_V), "");
...
bool CrabbyAVIFImageDecoder::FrameIsReceivedAtIndex(wtf_size_t index) const
{
    ...
    if (crabbyavif::crabby_avifDecoderNthImageMaxExtent(decoder_.get(), index, &data_extent) != crabbyavif::AVIF_RESULT_OK) {
        return false;
    }
    return data_extent.size == 0 || data_extent.offset + data_extent.size <= data_->size();
}

std::optional<base::TimeDelta> CrabbyAVIFImageDecoder::FrameTimestampAtIndex(wtf_size_t index) const
{
    return index < frame_buffer_cache_.size() ? frame_buffer_cache_[index].Timestamp() : std::nullopt;
}

base::TimeDelta CrabbyAVIFImageDecoder::FrameDurationAtIndex(wtf_size_t index) const
{
...
// static
bool CrabbyAVIFImageDecoder::MatchesAVIFSignature(const FastSharedBufferReader& fast_reader)
{
...
void CrabbyAVIFImageDecoder::InitializeNewFrame(wtf_size_t index)
{
    ...
    auto ret = crabbyavif::crabby_avifDecoderNthImageTiming(decoder_.get(), index, &timing);
...
void CrabbyAVIFImageDecoder::Decode(wtf_size_t index)
{
    ...
    if (progressive_) {
        ...
        for (frame_index = decoder_->imageIndex + 1; frame_index + 1 < decoder_->imageCount; ++frame_index) {
            crabbyavif::avifExtent data_extent;
            auto rv = crabbyavif::crabby_avifDecoderNthImageMaxExtent(decoder_.get(), frame_index + 1, &data_extent);
            if (rv != crabbyavif::AVIF_RESULT_OK) {
                DVLOG(1) << "crabbyavif::crabby_avifDecoderNthImageMaxExtent(" << frame_index + 1 << ") failed: " << crabbyavif::crabby_avifResultToString(rv)
    ...
    auto ret = DecodeImage(frame_index);
    if (ret != crabbyavif::AVIF_RESULT_OK && ret != crabbyavif::AVIF_RESULT_WAITING_ON_IO) {
        SetFailed();
        return;
    }
    const crabbyavif::avifImage* image = decoded_image_;
    ...
    int displayable_height = static_cast<int>(crabbyavif::crabby_avifDecoderDecodedRowCount(decoder_.get()));
...
bool CrabbyAVIFImageDecoder::CanReusePreviousFrameBuffer(wtf_size_t index) const
{
...
crabbyavif::avifResult CrabbyAVIFImageDecoder::ReadFromSegmentReader(
    crabbyavif::avifIO* io, uint32_t read_flags, uint64_t offset, size_t size, crabbyavif::avifROData* out)
{
    if (read_flags != 0) {
    ...
    if (offset > io_data->reader->size()) {
        ...
        return io_data->all_data_received ? crabbyavif::AVIF_RESULT_IO_ERROR : crabbyavif::AVIF_RESULT_WAITING_ON_IO;
    }
...
bool CrabbyAVIFImageDecoder::UpdateDemuxer()
{
    ...
    if (!decoder_) {
        ...
        if (animation_option_ != AnimationOption::kUnspecified
            && crabbyavif::crabby_avifDecoderSetSource(decoder_.get(),
                   animation_option_ == AnimationOption::kPreferAnimation ? crabbyavif::AVIF_DECODER_SOURCE_TRACKS
    ...
    if (ret != crabbyavif::AVIF_RESULT_OK) {
        DVLOG(1) << "crabbyavif::crabby_avifDecoderParse failed: " << crabbyavif::crabby_avifResultToString(ret) << ". " << decoder_->diag.error;
        return false;
    }
    ...
    progressive_ = decoder_->progressiveState == crabbyavif::AVIF_PROGRESSIVE_STATE_ACTIVE;
    ...
    // consecutive.
    static_assert(crabbyavif::AVIF_PIXEL_FORMAT_YUV422 == crabbyavif::AVIF_PIXEL_FORMAT_YUV444 + 1);
    static_assert(crabbyavif::AVIF_PIXEL_FORMAT_YUV420 == crabbyavif::AVIF_PIXEL_FORMAT_YUV422 + 1);
    static_assert(crabbyavif::AVIF_PIXEL_FORMAT_YUV400 == crabbyavif::AVIF_PIXEL_FORMAT_YUV420 + 1);
    ...
    CHECK(container->yuvFormat >= crabbyavif::AVIF_PIXEL_FORMAT_YUV444 && container->yuvFormat <= crabbyavif::AVIF_PIXEL_FORMAT_YUV400)
    ...
    if (!IgnoresColorSpace()) {
        ...
        if (container->icc.size) {
            ...
            const bool is_mono = container->yuvFormat == crabbyavif::AVIF_PIXEL_FORMAT_YUV400;
            if (is_mono) {
        ...
        } else if (container->colorPrimaries != crabbyavif::AVIF_COLOR_PRIMARIES_UNSPECIFIED
            || container->transferCharacteristics != crabbyavif::AVIF_TRANSFER_CHARACTERISTICS_UNSPECIFIED) {
    ...
    allow_decode_to_yuv_ = avif_yuv_format_ != crabbyavif::AVIF_PIXEL_FORMAT_YUV400 && !decoder_->alphaPresent && decoded_frame_count_ == 1
    ...
    // alpha.
    if (container->depth == 8 && avif_yuv_format_ != crabbyavif::AVIF_PIXEL_FORMAT_YUV400 && !decoder_->alphaPresent && decoded_frame_count_ == 1) {
    ...
    if (container->transformFlags & crabbyavif::AVIF_TRANSFORM_CLAP) {
        ...
        crabbyavif::avifBool valid_clap = crabbyavif::crabby_avifCropRectConvertCleanApertureBox(
            &crop_rect, &container->clap, container->width, container->height, container->yuvFormat, &diag);
        if (!valid_clap) {
...
{
    const auto ret = crabbyavif::crabby_avifDecoderNthImage(decoder_.get(), index);
    ...
    if (ret != crabbyavif::AVIF_RESULT_OK && ret != crabbyavif::AVIF_RESULT_WAITING_ON_IO) {
        DVLOG(1) << "crabbyavif::crabby_avifDecoderNthImage(" << index << ") failed: " << crabbyavif::crabby_avifResultToString(ret) << ": "
                 << AvifDecoderErrorMessage(decoder_.get());
        return ret;
    }
    ...
    if ((image->transformFlags & crabbyavif::AVIF_TRANSFORM_CLAP) && !ignore_clap_) {
        CropDecodedImage();
    }
...
void CrabbyAVIFImageDecoder::CropDecodedImage()
{
    ...
    const crabbyavif::avifResult result = crabbyavif::crabby_avifImageSetViewRect(cropped_image_.get(), decoded_image_, &rect);
    CHECK_EQ(result, crabbyavif::AVIF_RESULT_OK);
    decoded_image_ = cropped_image_.get();
}

bool CrabbyAVIFImageDecoder::RenderImage(const crabbyavif::avifImage* image, int from_row, int* to_row, ImageFrame* buffer)
{
    ...
    const bool use_libyuv_bilinear_upsampling = !decode_to_half_float_ && image->yuvFormat == crabbyavif::AVIF_PIXEL_FORMAT_YUV420;
    ...
    std::unique_ptr<crabbyavif::avifImage, decltype(&crabbyavif::crabby_avifImageDestroy)> view(nullptr, crabbyavif::crabby_avifImageDestroy);
    if (from_row > 0 || static_cast<uint32_t>(*to_row) < image->height) {
        const crabbyavif::avifCropRect rect = { 0, static_cast<uint32_t>(from_row), image->width, static_cast<uint32_t>(*to_row - from_row) };
        view.reset(crabbyavif::crabby_avifImageCreateEmpty());
        const crabbyavif::avifResult result = crabbyavif::crabby_avifImageSetViewRect(view.get(), image, &rect);
        CHECK_EQ(result, crabbyavif::AVIF_RESULT_OK);
        image = view.get();
    }
    ...
    const crabbyavif::avifResult result = crabbyavif::crabby_avifImageYUVToRGB(image, &rgb_image);
    if (save_top_row) {
...
void CrabbyAVIFImageDecoder::ColorCorrectImage(int from_row, int to_row, ImageFrame* buffer)
{
...
bool CrabbyAVIFImageDecoder::GetGainmapInfoAndData(SkGainmapInfo& out_gainmap_info, scoped_refptr<SegmentReader>& out_gainmap_data) const
{
...
CrabbyAVIFImageDecoder::AvifIOData::AvifIOData(scoped_refptr<const SegmentReader> reader, bool all_data_received)
