...
public:
    AVIFImageDecoder(AlphaOption, HighBitDepthDecodingOption, ColorBehavior, cc::AuxImage, wtf_size_t max_decoded_bytes, AnimationOption);
    ...
    bool GetGainmapInfoAndData(SkGainmapInfo& out_gainmap_info, scoped_refptr<SegmentReader>& out_gainmap_data) const override;
    ...
    std::optional<base::TimeDelta> FrameTimestampAtIndex(wtf_size_t) const override;
...
private:
    ...
    struct AvifIOData {
        AvifIOData();
        AvifIOData(scoped_refptr<const SegmentReader> reader, bool all_data_received);
        ~AvifIOData();
    ...
    static avifResult ReadFromSegmentReader(avifIO* io, uint32_t read_flags, uint64_t offset, size_t size, avifROData* out);
    ...
    // data.
    bool RenderImage(const avifImage* image, int from_row, int* to_row, ImageFrame* buffer);
    ...
    std::unique_ptr<avifImage, decltype(&avifImageDestroy)> cropped_image_ { nullptr, avifImageDestroy };
    ...
    avifIO avif_io_ = {};
    std::unique_ptr<avifDecoder, decltype(&avifDecoderDestroy)> decoder_ { nullptr, avifDecoderDestroy };
