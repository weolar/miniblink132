...
struct RWBuffer::BufferBlock {
    ...
    uint8_t* avail_data()
    {
        return const_cast<uint8_t*>(startData()) + used_;
    }

    static RWBuffer::BufferBlock* Alloc(size_t length)
    {
        size_t capacity = LengthToCapacity(length);
        void* buffer = WTF::Partitions::BufferMalloc(sizeof(RWBuffer::BufferBlock) + capacity, "blink::RWBuffer::BufferBlock");
...
struct RWBuffer::BufferHead {
    ...
    explicit BufferHead(size_t capacity)
        : ref_count_(1)
        , block_(capacity)
    {
    }
    ...
    static RWBuffer::BufferHead* Alloc(size_t length)
    {
        ...
        void* buffer = WTF::Partitions::BufferMalloc(size, "blink::RWBuffer::BufferHead");
    ...
    void unref() const
    {
        ...
        if (!ref_count_.Decrement()) {
            ...
            block_.~BufferBlock();
            WTF::Partitions::BufferFree(reinterpret_cast<void*>(const_cast<RWBuffer::BufferHead*>(this)));
            while (block) {
    ...
    void Validate(size_t minUsed, const RWBuffer::BufferBlock* tail = nullptr) const
    {
#if DCHECK_IS_ON()
...
//
ROBuffer::ROBuffer(const RWBuffer::BufferHead* head, size_t available, const RWBuffer::BufferBlock* tail)
    : head_(head)
...
RWBuffer::RWBuffer(base::OnceCallback<size_t(base::span<uint8_t>)> writer, size_t initial_capacity)
{
    if (initial_capacity) {
        head_ = RWBuffer::BufferHead::Alloc(initial_capacity);
        tail_ = &head_->block_;
