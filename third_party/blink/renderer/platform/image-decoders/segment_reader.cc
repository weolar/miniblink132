...
template <class Iter> base::span<const uint8_t> BufferGetSomeData(Iter& iter, size_t& position_of_block, size_t position)
{
    for (size_t size_of_block = iter.size(); size_of_block != 0; position_of_block += size_of_block, size_of_block = iter.size()) {
...
template <class Iter> sk_sp<SkData> BufferCopyAsSkData(Iter iter, size_t available)
{
...
public:
    ...
    SharedBufferSegmentReader& operator=(const SharedBufferSegmentReader&) = delete;
...
SharedBufferSegmentReader::SharedBufferSegmentReader(scoped_refptr<const SharedBuffer> buffer)
...
base::span<const uint8_t> SharedBufferSegmentReader::GetSomeData(size_t position) const
{
...
base::span<const uint8_t> DataSegmentReader::GetSomeData(size_t position) const
{
...
base::span<const uint8_t> ROBufferSegmentReader::GetSomeData(size_t position) const
{
    if (!ro_buffer_) {
        return {};
    }
...
sk_sp<SkData> ROBufferSegmentReader::GetAsSkData() const
{
    ...
    if (!multiple_blocks) {
        ...
        ro_buffer_->AddRef();
        return SkData::MakeWithProc(iter.data(), iter.size(), &UnrefROBuffer, ro_buffer_.get());
    }
...
scoped_refptr<SegmentReader> SegmentReader::CreateFromSharedBuffer(scoped_refptr<const SharedBuffer> buffer)
{
    return base::AdoptRef(new SharedBufferSegmentReader(std::move(buffer)));
}

scoped_refptr<SegmentReader> SegmentReader::CreateFromSkData(sk_sp<SkData> data)
{
    return base::AdoptRef(new DataSegmentReader(std::move(data)));
}

scoped_refptr<SegmentReader> SegmentReader::CreateFromROBuffer(scoped_refptr<ROBuffer> buffer)
{
    return base::AdoptRef(new ROBufferSegmentReader(std::move(buffer)));
}

// static
sk_sp<SkData> SegmentReader::RWBufferCopyAsSkData(RWBuffer::ROIter iter, size_t available)
{
    return BufferCopyAsSkData(iter, available);
}

// static
base::span<const uint8_t> SegmentReader::RWBufferGetSomeData(RWBuffer::ROIter& iter, size_t& position_of_block, size_t position)
{
