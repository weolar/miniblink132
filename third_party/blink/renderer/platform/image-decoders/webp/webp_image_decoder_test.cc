...
std::unique_ptr<ImageDecoder> CreateWEBPDecoder(ImageDecoder::AlphaOption alpha_option)
{
    return std::make_unique<WEBPImageDecoder>(alpha_option, ColorBehavior::kTransformToSRGB, ImageDecoder::kNoDecodedImageByteLimit);
}
...
void TestWebPBppHistogram(const char* image_name, const char* histogram_name = nullptr, base::HistogramBase::Sample sample = 0)
{
    TestBppHistogram(CreateWEBPDecoder, "WebP", image_name, histogram_name, sample);
}
...
TEST(AnimatedWebPTests, uniqueGenerationIDs)
{
    ...
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer("/images/resources/webp-animated.webp");
    ASSERT_TRUE(data.get());
    decoder->SetData(data.get(), true);
...
TEST(AnimatedWebPTests, verifyAnimationParametersTransparentImage)
{
    ...
    const AnimParam kFrameParameters[] = {
        { 0, 0, 11, 29, ImageFrame::kDisposeKeep, ImageFrame::kBlendAtopPreviousFrame, base::Milliseconds(1000), true },
        { 2, 10, 7, 17, ImageFrame::kDisposeKeep, ImageFrame::kBlendAtopPreviousFrame, base::Milliseconds(500), true },
        { 2, 2, 7, 16, ImageFrame::kDisposeKeep, ImageFrame::kBlendAtopPreviousFrame, base::Milliseconds(1000), true },
    };
    ...
    for (size_t i = 0; i < std::size(kFrameParameters); ++i) {
        ...
        EXPECT_EQ(kFrameParameters[i].alpha_blend_source, frame->GetAlphaBlendSource());
...
TEST(AnimatedWebPTests, verifyAnimationParametersOpaqueFramesTransparentBackground)
{
    ...
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer("/images/resources/webp-animated-opaque.webp");
    ASSERT_TRUE(data.get());
    decoder->SetData(data.get(), true);
    ...
    const AnimParam kFrameParameters[] = {
        { 4, 10, 33, 32, ImageFrame::kDisposeOverwriteBgcolor, ImageFrame::kBlendAtopPreviousFrame, base::Milliseconds(1000), true },
        { 34, 30, 33, 32, ImageFrame::kDisposeOverwriteBgcolor, ImageFrame::kBlendAtopPreviousFrame, base::Milliseconds(1000), true },
        { 62, 50, 32, 32, ImageFrame::kDisposeOverwriteBgcolor, ImageFrame::kBlendAtopPreviousFrame, base::Milliseconds(1000), true },
        { 10, 54, 32, 33, ImageFrame::kDisposeOverwriteBgcolor, ImageFrame::kBlendAtopPreviousFrame, base::Milliseconds(1000), true },
    };
...
TEST(AnimatedWebPTests, verifyAnimationParametersBlendOverwrite)
{
    ...
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer("/images/resources/webp-animated-no-blend.webp");
    ASSERT_TRUE(data.get());
    decoder->SetData(data.get(), true);
    ...
    const int kCanvasHeight = 87;
    const AnimParam kFrameParameters[] = {
        { 4, 10, 33, 32, ImageFrame::kDisposeOverwriteBgcolor, ImageFrame::kBlendAtopBgcolor, base::Milliseconds(1000), true },
        { 34, 30, 33, 32, ImageFrame::kDisposeOverwriteBgcolor, ImageFrame::kBlendAtopBgcolor, base::Milliseconds(1000), true },
        { 62, 50, 32, 32, ImageFrame::kDisposeOverwriteBgcolor, ImageFrame::kBlendAtopBgcolor, base::Milliseconds(1000), true },
        { 10, 54, 32, 33, ImageFrame::kDisposeOverwriteBgcolor, ImageFrame::kBlendAtopBgcolor, base::Milliseconds(1000), true },
    };
...
{
    TestByteByByteDecode(&CreateWEBPDecoder, "/images/resources/webp-animated.webp", 3u, kAnimationLoopInfinite);
    TestByteByByteDecode(&CreateWEBPDecoder, "/images/resources/webp-animated-icc-xmp.webp", 13u, 31999);
}
...
TEST(AnimatedWebPTests, truncatedLastFrame)
{
    ...
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer("/images/resources/invalid-animated-webp2.webp");
    ASSERT_TRUE(data.get());
    decoder->SetData(data.get(), true);
...
TEST(AnimatedWebPTests, truncatedInBetweenFrame)
{
    ...
    const Vector<char> full_data = ReadFile("/images/resources/invalid-animated-webp4.webp");
    scoped_refptr<SharedBuffer> data = SharedBuffer::Create(full_data.data(), full_data.size() - 1);
...
TEST(AnimatedWebPTests, reproCrash)
{
    ...
    const Vector<char> full_data = ReadFile("/images/resources/invalid_vp8_vp8x.webp");
    scoped_refptr<SharedBuffer> full_data_buffer = SharedBuffer::Create(full_data);
    ...
    scoped_refptr<SharedBuffer> data = SharedBuffer::Create(full_data.data(), kPartialSize);
    decoder->SetData(data.get(), false);
...
{
    TestProgressiveDecoding(&CreateWEBPDecoder, "/images/resources/webp-animated.webp");
}
...
TEST(AnimatedWebPTests, frameIsCompleteAndDuration)
{
    ...
    scoped_refptr<SharedBuffer> temp_data = SharedBuffer::Create(data.data(), data.size() - 10);
...
{
    TestUpdateRequiredPreviousFrameAfterFirstDecode(&CreateWEBPDecoder, "/images/resources/webp-animated.webp");
}

TEST(AnimatedWebPTests, randomFrameDecode)
{
    TestRandomFrameDecode(&CreateWEBPDecoder, "/images/resources/webp-animated.webp");
    TestRandomFrameDecode(&CreateWEBPDecoder, "/images/resources/webp-animated-opaque.webp");
    TestRandomFrameDecode(&CreateWEBPDecoder, "/images/resources/webp-animated-large.webp");
    TestRandomFrameDecode(&CreateWEBPDecoder, "/images/resources/webp-animated-icc-xmp.webp");
}

TEST(AnimatedWebPTests, randomDecodeAfterClearFrameBufferCache)
{
    TestRandomDecodeAfterClearFrameBufferCache(&CreateWEBPDecoder, "/images/resources/webp-animated.webp");
    TestRandomDecodeAfterClearFrameBufferCache(&CreateWEBPDecoder, "/images/resources/webp-animated-opaque.webp");
    TestRandomDecodeAfterClearFrameBufferCache(&CreateWEBPDecoder, "/images/resources/webp-animated-large.webp");
    TestRandomDecodeAfterClearFrameBufferCache(&CreateWEBPDecoder, "/images/resources/webp-animated-icc-xmp.webp");
}

TEST(AnimatedWebPTests, decodeAfterReallocatingData)
{
    TestDecodeAfterReallocatingData(&CreateWEBPDecoder, "/images/resources/webp-animated.webp");
    TestDecodeAfterReallocatingData(&CreateWEBPDecoder, "/images/resources/webp-animated-icc-xmp.webp");
}

TEST(AnimatedWebPTests, alphaBlending)
{
    TestAlphaBlending(&CreateWEBPDecoder, "/images/resources/webp-animated.webp");
    TestAlphaBlending(&CreateWEBPDecoder, "/images/resources/webp-animated-semitransparent1.webp");
    TestAlphaBlending(&CreateWEBPDecoder, "/images/resources/webp-animated-semitransparent2.webp");
    TestAlphaBlending(&CreateWEBPDecoder, "/images/resources/webp-animated-semitransparent3.webp");
    TestAlphaBlending(&CreateWEBPDecoder, "/images/resources/webp-animated-semitransparent4.webp");
}

TEST(AnimatedWebPTests, isSizeAvailable)
{
    TestByteByByteSizeAvailable(&CreateWEBPDecoder, "/images/resources/webp-animated.webp", 142u, false, kAnimationLoopInfinite);
    // FIXME: Add color profile support for animated webp images.
    TestByteByByteSizeAvailable(&CreateWEBPDecoder, "/images/resources/webp-animated-icc-xmp.webp", 1404u, false, 31999);
}
...
TEST(AnimatedWEBPTests, clearCacheExceptFrameWithAncestors)
{
    ...
    scoped_refptr<SharedBuffer> full_data = ReadFileToSharedBuffer("/images/resources/webp-animated.webp");
    ...
    EXPECT_EQ(buffer_sizes[0] + buffer_sizes[2], decoder->ClearCacheExceptFrame(1));
    ...
    for (size_t i = 0; i < decoder->FrameCount(); i++) {
        ASSERT_EQ(ImageFrame::kFrameComplete, decoder->DecodeFrameBufferAtIndex(i)->GetStatus());
    }
...
{
    TestByteByByteDecode(&CreateWEBPDecoder, "/images/resources/crbug.364830.webp", 1u, kAnimationNone);
    TestByteByByteDecode(&CreateWEBPDecoder, "/images/resources/size-failure.b186640109.webp", 1u, kAnimationNone);
}

TEST(StaticWebPTests, isSizeAvailable)
{
    TestByteByByteSizeAvailable(&CreateWEBPDecoder, "/images/resources/webp-color-profile-lossy.webp", 520u, true, kAnimationNone);
    TestByteByByteSizeAvailable(&CreateWEBPDecoder, "/images/resources/test.webp", 30u, false, kAnimationNone);
    TestByteByByteSizeAvailable(&CreateWEBPDecoder, "/images/resources/size-failure.b186640109.webp", 25u, false, kAnimationNone);
}

TEST(StaticWebPTests, notAnimated)
{
    std::unique_ptr<ImageDecoder> decoder = CreateWEBPDecoder();
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer("/images/resources/webp-color-profile-lossy.webp");
    ASSERT_TRUE(data.get());
    decoder->SetData(data.get(), true);
    EXPECT_EQ(1u, decoder->FrameCount());
...
TEST(StaticWebPTests, bppHistogramSmall)
{
    ...
    constexpr int kSample = (kFileSize * 100 * 8 + kImageArea / 2) / kImageArea; // = 24
    TestWebPBppHistogram("/images/resources/webp-color-profile-lossy.webp", "Blink.DecodedImage.WebPDensity.Count.0.7MP", kSample);
}
...
TEST(StaticWebPTests, bppHistogramSmall3x3)
{
    ...
    TestWebPBppHistogram("/images/resources/red3x3-lossy.webp", "Blink.DecodedImage.WebPDensity.Count.0.1MP", kSample);
}
...
TEST(StaticWebPTests, bppHistogramSmall900000)
{
    ...
    constexpr int kSample = (kFileSize * 100 * 8 + kImageArea / 2) / kImageArea; // = 10
    TestWebPBppHistogram("/images/resources/peach_900000.webp", "Blink.DecodedImage.WebPDensity.Count.0.9MP", kSample);
}
...
TEST(StaticWebPTests, bppHistogramBig)
{
    ...
    TestWebPBppHistogram("/images/resources/bee.webp", "Blink.DecodedImage.WebPDensity.Count.13MP", kSample);
}
...
TEST(StaticWebPTests, bppHistogramBig13000000)
{
    ...
    constexpr int kSample = (kFileSize * 100 * 8 + kImageArea / 2) / kImageArea; // = 4
    TestWebPBppHistogram("/images/resources/peach_13000000.webp", "Blink.DecodedImage.WebPDensity.Count.13MP", kSample);
}
...
TEST(StaticWebPTests, bppHistogramHuge)
{
    ...
    constexpr int kSample = (kFileSize * 100 * 8 + kImageArea / 2) / kImageArea; // = 3
    TestWebPBppHistogram("/images/resources/peach.webp", "Blink.DecodedImage.WebPDensity.Count.14+MP", kSample);
}
...
TEST(StaticWebPTests, bppHistogramHuge13000002)
{
    ...
    constexpr int kSample = (kFileSize * 100 * 8 + kImageArea / 2) / kImageArea; // = 3
    TestWebPBppHistogram("/images/resources/peach_13000002.webp", "Blink.DecodedImage.WebPDensity.Count.14+MP", kSample);
}
...
TEST(StaticWebPTests, bppHistogramInvalid)
{
    ...
    std::unique_ptr<ImageDecoder> decoder = CreateWEBPDecoder();
    decoder->SetData(ReadFileToSharedBuffer("/images/resources/truncated.webp"), true);
    ...
    EXPECT_THAT(histogram_tester.GetTotalCountsForPrefix("Blink.DecodedImage.WebPDensity.Count."), testing::ContainerEq(empty_counts));
}
