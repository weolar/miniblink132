...
{
    return std::make_unique<GIFImageDecoder>(ImageDecoder::kAlphaNotPremultiplied, ColorBehavior::kTransformToSRGB, ImageDecoder::kNoDecodedImageByteLimit);
}

void TestRepetitionCount(const char* dir, const char* file, int expected_repetition_count)
{
...
TEST(GIFImageDecoderTest, decodeTwoFrames)
{
    ...
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(kWebTestsResourcesDir, "animated.gif");
    ASSERT_TRUE(data.get());
    decoder->SetData(data.get(), true);
...
{
    std::unique_ptr<ImageDecoder> decoder = CreateDecoder();
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(kWebTestsResourcesDir, "crbug779261.gif");
    ASSERT_TRUE(data.get());
    decoder->SetData(data.get(), true);
...
TEST(GIFImageDecoderTest, parseByteByByte)
{
    ...
    for (size_t length = 1; length <= data.size(); ++length) {
        scoped_refptr<SharedBuffer> temp_data = SharedBuffer::Create(data.data(), length);
...
{
    TestByteByByteDecode(&CreateDecoder, kWebTestsResourcesDir, "animated-gif-with-offsets.gif", 5u, kAnimationLoopInfinite);
}
...
TEST(GIFImageDecoderTest, brokenSecondFrame)
{
    ...
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(kDecodersTestingDir, "broken.gif");
    ASSERT_TRUE(data.get());
    decoder->SetData(data.get(), true);
...
{
    scoped_refptr<SharedBuffer> reference_data = ReadFileToSharedBuffer(kDecodersTestingDir, "radient.gif");
    scoped_refptr<SharedBuffer> test_data = ReadFileToSharedBuffer(kDecodersTestingDir, "radient-bad-terminator.gif");
    ...
    EXPECT_EQ(HashBitmap(reference_frame->Bitmap()), HashBitmap(test_frame->Bitmap()));
}

TEST(GIFImageDecoderTest, updateRequiredPreviousFrameAfterFirstDecode)
{
    TestUpdateRequiredPreviousFrameAfterFirstDecode(&CreateDecoder, kWebTestsResourcesDir, "animated-10color.gif");
}
...
TEST(GIFImageDecoderTest, randomFrameDecode)
{
    ...
    TestRandomFrameDecode(&CreateDecoder, kWebTestsResourcesDir, "animated-gif-with-offsets.gif");
    TestRandomFrameDecode(&CreateDecoder, kWebTestsResourcesDir, "animated-10color.gif");
}

TEST(GIFImageDecoderTest, randomDecodeAfterClearFrameBufferCache)
{
    // Single frame image.
    TestRandomDecodeAfterClearFrameBufferCache(&CreateDecoder, kDecodersTestingDir, "radient.gif");
    // Multiple frame images.
    TestRandomDecodeAfterClearFrameBufferCache(&CreateDecoder, kWebTestsResourcesDir, "animated-gif-with-offsets.gif");
    TestRandomDecodeAfterClearFrameBufferCache(&CreateDecoder, kWebTestsResourcesDir, "animated-10color.gif");
}
...
{
    scoped_refptr<SharedBuffer> test_data = ReadFileToSharedBuffer(kDecodersTestingDir, "bad-initial-code.gif");
    ASSERT_TRUE(test_data.get());
...
{
    scoped_refptr<SharedBuffer> test_data = ReadFileToSharedBuffer(kDecodersTestingDir, "bad-code.gif");
    ASSERT_TRUE(test_data.get());
...
TEST(GIFImageDecoderTest, invalidDisposalMethod)
{
    ...
    scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(kDecodersTestingDir, "invalid-disposal-method.gif");
    ASSERT_TRUE(data.get());
    decoder->SetData(data.get(), true);
    ...
    EXPECT_EQ(ImageFrame::kDisposeOverwritePrevious, decoder->DecodeFrameBufferAtIndex(0)->GetDisposalMethod());
    // Unknown disposal methods (5 in this case) are converted to
    // ImageFrame::DisposeKeep.
    EXPECT_EQ(ImageFrame::kDisposeKeep, decoder->DecodeFrameBufferAtIndex(1)->GetDisposalMethod());
}

TEST(GIFImageDecoderTest, firstFrameHasGreaterSizeThanScreenSize)
{
    const Vector<char> full_data = ReadFile(kDecodersTestingDir, "first-frame-has-greater-size-than-screen-size.gif");
    ...
    for (size_t i = 1; i <= full_data.size(); ++i) {
        ...
        scoped_refptr<SharedBuffer> data = SharedBuffer::Create(full_data.data(), i);
        decoder->SetData(data.get(), i == full_data.size());

        if (decoder->IsSizeAvailable() && !frame_size.width() && !frame_size.height()) {
...
TEST(GIFImageDecoderTest, verifyRepetitionCount)
{
    ...
    TestRepetitionCount(kDecodersTestingDir, "radient.gif", kAnimationLoopInfinite);
}

TEST(GIFImageDecoderTest, repetitionCountChangesWhenSeen)
{
    const Vector<char> full_data = ReadFile(kWebTestsResourcesDir, "animated-10color.gif");
    ...
    scoped_refptr<SharedBuffer> partial_data = SharedBuffer::Create(full_data.data(), kTruncatedSize);

    std::unique_ptr<ImageDecoder> decoder
        = std::make_unique<GIFImageDecoder>(ImageDecoder::kAlphaPremultiplied, ColorBehavior::kTransformToSRGB, ImageDecoder::kNoDecodedImageByteLimit);
...
TEST(GIFImageDecoderTest, bitmapAlphaType)
{
    ...
    scoped_refptr<SharedBuffer> partial_data = SharedBuffer::Create(full_data.data(), kTruncateSize);

    std::unique_ptr<ImageDecoder> premul_decoder
        = std::make_unique<GIFImageDecoder>(ImageDecoder::kAlphaPremultiplied, ColorBehavior::kTransformToSRGB, ImageDecoder::kNoDecodedImageByteLimit);
    std::unique_ptr<ImageDecoder> unpremul_decoder
        = std::make_unique<GIFImageDecoder>(ImageDecoder::kAlphaNotPremultiplied, ColorBehavior::kTransformToSRGB, ImageDecoder::kNoDecodedImageByteLimit);
    ...
    EXPECT_TRUE(premul_frame && premul_frame->GetStatus() != ImageFrame::kFrameComplete);
    ...
    EXPECT_TRUE(unpremul_frame && unpremul_frame->GetStatus() != ImageFrame::kFrameComplete);
...
{
    const Vector<char> data = ReadFile(kWebTestsResourcesDir, "count-down-color-test.gif");
    scoped_refptr<SharedBuffer> data_buffer = SharedBuffer::Create(data);
    ...
    scoped_refptr<SharedBuffer> modified_data = SharedBuffer::Create(data.data(), kErrorOffset);
    modified_data->Append("A", 1u);
    modified_data->Append(data.data() + kErrorOffset + 1, data.size() - kErrorOffset - 1);
    {
        auto decoder = CreateDecoder();
...
{
    scoped_refptr<SharedBuffer> test_data = ReadFileToSharedBuffer(kDecodersTestingDir, "error_frame.gif");
    ASSERT_TRUE(test_data.get());
