...
bool ShouldSkipFile(const base::FilePath& path, blink::ImageDecoderBaseTest::FileSelection file_selection, const int64_t threshold)
{
    ...
    return (file_selection == blink::ImageDecoderBaseTest::FileSelection::kSmaller) == (image_size > threshold);
}
...
base::MD5Digest ComputeMD5Sum(const blink::ImageFrame& frame_buffer)
{
    ...
    base::MD5Sum(base::make_span(static_cast<const uint8_t*>(bitmap.getPixels()), bitmap.computeByteSize()), &digest);
    return digest;
}

#if defined(CALCULATE_MD5_SUMS)
void SaveMD5Sum(const base::FilePath& path, const blink::ImageFrame* frame_buffer)
{
    // Calculate MD5 sum.
...
#if !defined(CALCULATE_MD5_SUMS)
void VerifyImage(blink::ImageDecoder& decoder, const base::FilePath& path, const base::FilePath& md5_sum_path, size_t frame_index)
{
    SCOPED_TRACE(path.value());
    // Make sure decoding can complete successfully.
    EXPECT_TRUE(decoder.IsSizeAvailable());
    EXPECT_GE(decoder.FrameCount(), frame_index);
    blink::ImageFrame* const frame_buffer = decoder.DecodeFrameBufferAtIndex(frame_index);
    ASSERT_TRUE(frame_buffer);
...
void ImageDecoderBaseTest::SetUp()
{
    ...
    data_dir_ = data_dir.AppendASCII("webkit").AppendASCII("data").AppendASCII(format_.Utf8() + "_decoder");
}

base::FilePath ImageDecoderBaseTest::GetMD5SumPath(const base::FilePath& path)
{
    static const base::FilePath::StringType kDecodedDataExtension(FILE_PATH_LITERAL(".md5sum"));
...
Vector<base::FilePath> ImageDecoderBaseTest::GetImageFiles() const
{
    ...
    base::FileEnumerator enumerator(data_dir_, false, base::FileEnumerator::FILES);
    for (base::FilePath next_file_name = enumerator.Next(); !next_file_name.empty(); next_file_name = enumerator.Next()) {
...
bool ImageDecoderBaseTest::ShouldImageFail(const base::FilePath& path) const
{
    ...
    return (path.value().length() > (kBadSuffix.length() + format_.length())
        && !path.value().compare(path.value().length() - format_.length() - kBadSuffix.length(), kBadSuffix.length(), kBadSuffix));
}

void ImageDecoderBaseTest::TestDecoding(blink::ImageDecoderBaseTest::FileSelection file_selection, const int64_t threshold)
{
    const Vector<base::FilePath> image_files = GetImageFiles();
    if (image_files.empty()) {
        const testing::TestInfo* const test_info = testing::UnitTest::GetInstance()->current_test_info();
        VLOG(0) << "TestDecoding() in " << test_info->test_suite_name() << "." << test_info->name() << " not running because test data wasn't found.";
        return;
    }
...
void ImageDecoderBaseTest::TestImageDecoder(const base::FilePath& image_path, const base::FilePath& md5_sum_path, int desired_frame_index) const
{
#if defined(CALCULATE_MD5_SUMS)
    ...
    CHECK(ShouldImageFail(image_path) || base::PathExists(md5_sum_path)) << md5_sum_path;
    ...
    scoped_refptr<SharedBuffer> partial_data = SharedBuffer::Create(data_ptr, partial_size);
    ...
    scoped_refptr<SharedBuffer> data = SharedBuffer::Create(data_ptr, image_contents.size());
    decoder->SetData(data, true);
    if (ShouldImageFail(image_path)) {
        blink::ImageFrame* const frame_buffer = decoder->DecodeFrameBufferAtIndex(kFirstFrameIndex);
        ...
        EXPECT_FALSE(decoder->Failed()) << image_path.value();
#if defined(CALCULATE_MD5_SUMS)
        SaveMD5Sum(md5_sum_path, decoder->DecodeFrameBufferAtIndex(desired_frame_index));
#else
