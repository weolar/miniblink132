...
public:
    explicit TestImageDecoder(ImageDecoder::HighBitDepthDecodingOption high_bit_depth_decoding_option, wtf_size_t max_decoded_bytes = kNoDecodedImageByteLimit)
        : ImageDecoder(kAlphaNotPremultiplied, high_bit_depth_decoding_option, ColorBehavior::kTransformToSRGB, cc::AuxImage::kDefault, max_decoded_bytes)
    {
    }

    TestImageDecoder()
        : TestImageDecoder(ImageDecoder::kDefaultBitDepth)
    {
    }
    ...
    {
        return frame_buffer_cache_;
    }

    void ResetRequiredPreviousFrames(bool known_opaque = false)
    {
        for (size_t i = 0; i < frame_buffer_cache_.size(); ++i) {
            frame_buffer_cache_[i].SetRequiredPreviousFrameIndex(FindRequiredPreviousFrame(i, known_opaque));
        }
    }

    void InitFrames(wtf_size_t num_frames, unsigned width = 100, unsigned height = 100)
    {
    ...
    {
        return image_is_high_bit_depth_;
    }
    void SetImageToHighBitDepthForTest()
    {
        image_is_high_bit_depth_ = true;
    }
    ...
    for (bool high_bit_depth_decoder : high_bit_depth_decoder_status) {
        for (bool high_bit_depth_image : high_bit_depth_image_status) {
            ...
            if (high_bit_depth_decoder) {
                decoder = std::make_unique<TestImageDecoder>(ImageDecoder::kHighBitDepthToHalfFloat);
            } else {
            ...
            EXPECT_FALSE(decoder->SetSize(1 << overflow_dim_shift_half, 1 << overflow_dim_shift_half));
            EXPECT_TRUE(decoder->SetSize(1 << (overflow_dim_shift - 1), 1));
            EXPECT_TRUE(decoder->SetSize(1, 1 << (overflow_dim_shift - 1)));
            EXPECT_TRUE(decoder->SetSize(1 << (overflow_dim_shift_half - 1), 1 << (overflow_dim_shift_half - 1)));
        }
    }
}
...
TEST(ImageDecoderTest, requiredPreviousFrameIndexBlendAtopBgcolor)
{
    ...
    // any prior frames.
    for (int dispose_method = ImageFrame::kDisposeNotSpecified; dispose_method <= ImageFrame::kDisposeOverwritePrevious; ++dispose_method) {
        frame_buffers[1].SetDisposalMethod(static_cast<ImageFrame::DisposalMethod>(dispose_method));
        decoder->ResetRequiredPreviousFrames();
        EXPECT_EQ(kNotFound, frame_buffers[2].RequiredPreviousFrameIndex());
    }
...
TEST(ImageDecoderTest, clearCacheExceptFrameAll)
{
    ...
    for (size_t i = 0; i < kNumFrames; ++i) {
        frame_buffers[i].SetStatus(i % 2 ? ImageFrame::kFramePartial : ImageFrame::kFrameComplete);
    }
...
TEST(ImageDecoderTest, decodedSizeLimitBoundary)
{
    ...
    std::unique_ptr<TestImageDecoder> decoder(std::make_unique<TestImageDecoder>(ImageDecoder::kDefaultBitDepth, (kWidth * kHeight * kBitDepth)));
...
TEST(ImageDecoderTest, decodedSizeUnlimited)
{
    ...
    std::unique_ptr<TestImageDecoder> decoder(std::make_unique<TestImageDecoder>(ImageDecoder::kDefaultBitDepth, ImageDecoder::kNoDecodedImageByteLimit));
    EXPECT_TRUE(decoder->SetSize(kWidth, kHeight));
    EXPECT_TRUE(decoder->IsSizeAvailable());
    EXPECT_FALSE(decoder->Failed());
}
