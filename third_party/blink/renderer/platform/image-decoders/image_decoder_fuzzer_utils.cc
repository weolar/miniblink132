...
std::unique_ptr<ImageDecoder> CreateImageDecoder(DecoderType decoder_type, FuzzedDataProvider& fdp)
{
    ImageDecoder::AlphaOption option = fdp.ConsumeBool() ? ImageDecoder::kAlphaPremultiplied : ImageDecoder::kAlphaNotPremultiplied;
    ...
    case DecoderType::kBmpDecoder:
        return std::make_unique<BMPImageDecoder>(option, behavior, max_decoded_bytes);
    case DecoderType::kJpegDecoder: {
        cc::AuxImage aux_image_type = fdp.ConsumeBool() ? cc::AuxImage::kDefault : cc::AuxImage::kGainmap;
        wtf_size_t offset = fdp.ConsumeIntegral<uint32_t>();
        return std::make_unique<JPEGImageDecoder>(option, behavior, aux_image_type, max_decoded_bytes, offset);
    }
    case DecoderType::kPngDecoder: {
        ImageDecoder::HighBitDepthDecodingOption decoding_option = fdp.ConsumeBool() ? ImageDecoder::kDefaultBitDepth : ImageDecoder::kHighBitDepthToHalfFloat;
        wtf_size_t offset = fdp.ConsumeIntegral<uint32_t>();
        return std::make_unique<PNGImageDecoder>(option, decoding_option, behavior, max_decoded_bytes, offset);
    }
    }
}
...
void FuzzDecoder(DecoderType decoder_type, FuzzedDataProvider& fdp)
{
    ...
    auto buffer = SharedBuffer::Create(remaining_data.data(), remaining_data.size());
