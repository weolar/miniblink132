...
public:
    ...
    bool IsOpaque() const
    {
        return site_as_origin_->IsOpaque();
    }
...
private:
    ...
    friend struct mojo::StructTraits<network::mojom::SchemefulSiteDataView, blink::BlinkSchemefulSite>;

    FRIEND_TEST_ALL_PREFIXES(BlinkSchemefulSiteMojomTraitsTest, DeserializeFailure);
    ...
    static bool FromWire(const url::Origin& site_as_origin, BlinkSchemefulSite* out);
...
template <> struct HashTraits<blink::BlinkSchemefulSite> : OneFieldHashTraits<blink::BlinkSchemefulSite, &blink::BlinkSchemefulSite::site_as_origin_> { };
