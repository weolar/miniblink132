...
TEST_F(EncodedFormDataTest, DeepCopy)
{
    ...
    original->AppendFileRange("example.txt", 12345, 56789, base::Time::FromSecondsSinceUnixEpoch(9999.0));

    mojo::PendingRemote<mojom::blink::Blob> remote;
    mojo::PendingReceiver<mojom::blink::Blob> receiver = remote.InitWithNewPipeAndPassReceiver();
    original->AppendBlob(BlobDataHandle::Create("uuid", /*type=*/"", /*size=*/0, std::move(remote)));
    ...
    EXPECT_EQ(9999.0, copy_elements[1].expected_file_modification_time_->InSecondsFSinceUnixEpoch());
...
TEST_F(EncodedFormDataTest, GetType)
{
    ...
    EXPECT_EQ(EncodedFormData::FormDataType::kDataAndEncodedFileOrBlob, form_data->GetType());
...
TEST_F(EncodedFormDataTest, GetType2)
{
    ...
    form_data->AppendDataPipe(nullptr);
    EXPECT_EQ(EncodedFormData::FormDataType::kDataAndDataPipe, form_data->GetType());
