...
FormDataElement::FormDataElement(
    const String& filename, int64_t file_start, int64_t file_length, const std::optional<base::Time>& expected_file_modification_time)
    : type_(kEncodedFile)
    , filename_(filename)
...
FormDataElement::FormDataElement(scoped_refptr<BlobDataHandle> handle)
    : type_(kEncodedBlob)
    , blob_data_handle_(std::move(handle))
{
    CHECK(blob_data_handle_);
}

FormDataElement::FormDataElement(scoped_refptr<WrappedDataPipeGetter> data_pipe_getter)
    : type_(kDataPipe)
...
bool operator==(const FormDataElement& a, const FormDataElement& b)
{
    ...
    if (a.type_ == FormDataElement::kEncodedFile) {
        return a.filename_ == b.filename_ && a.file_start_ == b.file_start_ && a.file_length_ == b.file_length_
            && a.expected_file_modification_time_ == b.expected_file_modification_time_;
    }
...
scoped_refptr<EncodedFormData> EncodedFormData::Create(const void* data, wtf_size_t size)
{
...
scoped_refptr<EncodedFormData> EncodedFormData::Create(base::span<const char> string)
{
    scoped_refptr<EncodedFormData> result = Create();
    result->AppendData(string.data(), base::checked_cast<wtf_size_t>(string.size()));
    return result;
}

scoped_refptr<EncodedFormData> EncodedFormData::Create(const std::string& str)
{
    return EncodedFormData::Create(base::span<const char>(str.c_str(), str.size()));
}
...
scoped_refptr<EncodedFormData> EncodedFormData::DeepCopy() const
{
    ...
    for (const FormDataElement& e : elements_) {
        ...
        case FormDataElement::kEncodedFile:
            form_data->elements_.UncheckedAppend(FormDataElement(e.filename_, e.file_start_, e.file_length_, e.expected_file_modification_time_));
            break;
        case FormDataElement::kEncodedBlob:
            form_data->elements_.UncheckedAppend(FormDataElement(e.blob_data_handle_));
            break;
        case FormDataElement::kDataPipe:
            mojo::PendingRemote<network::mojom::blink::DataPipeGetter> data_pipe_getter;
            e.data_pipe_getter_->GetDataPipeGetter()->Clone(data_pipe_getter.InitWithNewPipeAndPassReceiver());
            auto wrapped = base::MakeRefCounted<WrappedDataPipeGetter>(std::move(data_pipe_getter));
            form_data->elements_.UncheckedAppend(FormDataElement(std::move(wrapped)));
            break;
        }
    }
    return form_data;
...
void EncodedFormData::AppendFile(const String& filename, const std::optional<base::Time>& expected_modification_time)
{
    elements_.push_back(FormDataElement(filename, 0, BlobData::kToEndOfFile, expected_modification_time));
}

void EncodedFormData::AppendFileRange(const String& filename, int64_t start, int64_t length, const std::optional<base::Time>& expected_modification_time)
{
    elements_.push_back(FormDataElement(filename, start, length, expected_modification_time));
}

void EncodedFormData::AppendBlob(scoped_refptr<BlobDataHandle> optional_handle)
{
    elements_.emplace_back(std::move(optional_handle));
}

void EncodedFormData::AppendDataPipe(scoped_refptr<WrappedDataPipeGetter> handle)
{
...
String EncodedFormData::FlattenToString() const
{
    Vector<char> bytes;
    Flatten(bytes);
    return Latin1Encoding().Decode(base::span<const uint8_t>((const uint8_t*)bytes.data(), bytes.size()));
}
