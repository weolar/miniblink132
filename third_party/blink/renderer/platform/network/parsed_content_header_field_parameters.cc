...
std::optional<ParsedContentHeaderFieldParameters> ParsedContentHeaderFieldParameters::Parse(HeaderFieldTokenizer tokenizer, Mode mode)
{
    ...
    while (!tokenizer.IsConsumed()) {
        ...
        if (!tokenizer.ConsumeTokenOrQuotedString(mode, value)) {
            DVLOG(1) << "Invalid content parameter value (at " << tokenizer.Index() << ", for '" << key.ToString() << "').";
            return std::nullopt;
        }
...
String ParsedContentHeaderFieldParameters::ParameterValueForName(const String& name) const
{
