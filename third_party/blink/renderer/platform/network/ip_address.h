...
template <> struct HashTraits<net::IPAddress> : GenericHashTraits<net::IPAddress> {
    static const bool kEmptyValueIsZero = true;

    static unsigned GetHash(const net::IPAddress& ip_address)
    {
        const net::IPAddressBytes& by = ip_address.bytes();
        return static_cast<unsigned>(base::FastHash(base::span(by.data(), by.size())));
    }
