...
public:
    ...
    static WTF::TextEncoding EncodingFromAcceptCharset(const String& accept_charset, const WTF::TextEncoding& fallback_encoding);

    // Helper functions used by HTMLFormElement for multi-part form data
    static Vector<char> GenerateUniqueBoundaryString();
    static void BeginMultiPartHeader(Vector<char>&, const std::string& boundary, const std::string& name);
    static void AddBoundaryToMultiPartHeader(Vector<char>&, const std::string& boundary, bool is_last_boundary = false);
    static void AddFilenameToMultiPartHeader(Vector<char>&, const WTF::TextEncoding&, const String& filename);
    static void AddContentTypeToMultiPartHeader(Vector<char>&, const String& mime_type);
    ...
    static void AddKeyValuePairAsFormData(Vector<char>&, const std::string& key, const std::string& value,
        EncodedFormData::EncodingType = EncodedFormData::kFormURLEncoded, Mode = kNormalizeCRLF);
