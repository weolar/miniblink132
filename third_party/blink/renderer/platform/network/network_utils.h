...
PLATFORM_EXPORT WTF::String GetDomainAndRegistry(const WTF::StringView& host, PrivateRegistryFilter);
...
PLATFORM_EXPORT std::tuple<int, ResourceResponse, scoped_refptr<SharedBuffer>> ParseDataURL(
    const KURL&, const WTF::String& method, ukm::SourceId source_id = ukm::kInvalidSourceId, ukm::UkmRecorder* recorder = nullptr);

// Returns true if the URL is a data URL and its MIME type is in the list of
// supported/recognized MIME types.
PLATFORM_EXPORT bool IsDataURLMimeTypeSupported(const KURL&, std::string* data = nullptr, std::string* mime_type = nullptr);
...
PLATFORM_EXPORT Vector<char> ParseMultipartBoundary(const AtomicString& content_type_header);
