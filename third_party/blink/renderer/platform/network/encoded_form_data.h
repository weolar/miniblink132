...
public:
    ...
    FormDataElement(const String& filename, int64_t file_start, int64_t file_length, const std::optional<base::Time>& expected_file_modification_time);
...
PLATFORM_EXPORT bool operator==(const FormDataElement& a, const FormDataElement& b);
...
public:
    ...
    static scoped_refptr<EncodedFormData> Create(const std::string&);
    ...
    void AppendFile(const String& file_path, const std::optional<base::Time>& expected_modification_time);
    void AppendFileRange(const String& filename, int64_t start, int64_t length, const std::optional<base::Time>& expected_modification_time);
    ...
    Vector<FormDataElement>& MutableElements()
    {
        return elements_;
    }
    ...
    void SetBoundary(Vector<char> boundary)
    {
        boundary_ = boundary;
    }
...
private:
    friend struct mojo::StructTraits<mojom::FetchAPIRequestBodyDataView, ResourceRequestBody>;
    EncodedFormData();
