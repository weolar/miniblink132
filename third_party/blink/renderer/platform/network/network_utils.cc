...
net::registry_controlled_domains::PrivateRegistryFilter getNetPrivateRegistryFilter(blink::network_utils::PrivateRegistryFilter filter)
{
    switch (filter) {
...
String GetDomainAndRegistry(const StringView& host, PrivateRegistryFilter filter)
{
    StringUTF8Adaptor host_utf8(host);
    std::string domain = net::registry_controlled_domains::GetDomainAndRegistry(host_utf8.AsStringView(), getNetPrivateRegistryFilter(filter));
    return String(domain);
}

std::tuple<int, ResourceResponse, scoped_refptr<SharedBuffer>> ParseDataURL(
    const KURL& url, const String& method, ukm::SourceId source_id, ukm::UkmRecorder* recorder)
{
    ...
    net::Error result = net::DataURL::BuildResponse(GURL(url), method.Ascii(), &utf8_mime_type, &utf8_charset, &data_string, &headers);
    if (result != net::OK)
    ...
    base::UmaHistogramMicrosecondsTimes("Blink.Network.ParseDataURLTime", elapsed);
    size_t length = url.GetString().length();
    base::UmaHistogramCounts10M("Blink.Network.DataUrlLength", static_cast<int>(length));
    if (length >= 0 && length < 1000) {
        base::UmaHistogramMicrosecondsTimes("Blink.Network.ParseDataURLTime.Under1000Char", elapsed);
    } else if (length >= 1000 && length < 100000) {
        base::UmaHistogramMicrosecondsTimes("Blink.Network.ParseDataURLTime.Under100000Char", elapsed);
    } else {
        base::UmaHistogramMicrosecondsTimes("Blink.Network.ParseDataURLTime.Over100000Char", elapsed);
    }
    bool is_image = utf8_mime_type.starts_with("image/");
    if (is_image) {
        base::UmaHistogramCounts10M("Blink.Network.DataUrlLength.Image", static_cast<int>(length));
        base::UmaHistogramMicrosecondsTimes("Blink.Network.ParseDataURLTime.Image", elapsed);
        if (length >= 0 && length < 1000) {
            base::UmaHistogramMicrosecondsTimes("Blink.Network.ParseDataURLTime.Image.Under1000Char", elapsed);
        } else if (length >= 1000 && length < 100000) {
            base::UmaHistogramMicrosecondsTimes("Blink.Network.ParseDataURLTime.Image.Under100000Char", elapsed);
        } else {
            base::UmaHistogramMicrosecondsTimes("Blink.Network.ParseDataURLTime.Image.Over100000Char", elapsed);
        }
    }
...
bool IsDataURLMimeTypeSupported(const KURL& url, std::string* data, std::string* mime_type)
{
    std::string utf8_mime_type;
    std::string utf8_charset;
...
{
    return WebString::FromUTF8(net::HttpUtil::GenerateAcceptLanguageHeader(lang.Utf8()));
}
...
Vector<char> ParseMultipartBoundary(const AtomicString& content_type_header)
{
    ...
    std::string boundary;
    net::HttpUtil::ParseContentType(utf8_string, &mime_type, &charset, &had_charset, &boundary);
    base::TrimString(boundary, " \"", &boundary);
    Vector<char> result;
    result.AppendSpan(base::span(boundary.c_str(), boundary.size()));
    return result;
}
