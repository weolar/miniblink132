...
void CheckValidity(bool expected, const String& input, Mode mode = Mode::kNormal)
{
    EXPECT_EQ(expected, !!ParsedContentHeaderFieldParameters::Parse(HeaderFieldTokenizer(input), mode)) << input;

    const String disposition_input = "attachment" + input;
    EXPECT_EQ(expected, ParsedContentDisposition(disposition_input, mode).IsValid()) << disposition_input;

    const String type_input = "text/plain" + input;
    EXPECT_EQ(expected, ParsedContentType(type_input, mode).IsValid()) << type_input;
}
...
TEST(ParsedContentHeaderFieldParametersTest, ParameterName)
{
    ...
    std::optional<ParsedContentHeaderFieldParameters> t = ParsedContentHeaderFieldParameters::Parse(HeaderFieldTokenizer(input), Mode::kNormal);
    ASSERT_TRUE(t);
...
TEST(ParsedContentHeaderFieldParametersTest, RelaxedParameterName)
{
    ...
    std::optional<ParsedContentHeaderFieldParameters> t = ParsedContentHeaderFieldParameters::Parse(HeaderFieldTokenizer(input), Mode::kRelaxed);
    ASSERT_TRUE(t);
