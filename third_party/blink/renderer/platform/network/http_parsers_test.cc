...
TEST(HTTPParsersTest, ParseCacheControl)
{
    ...
    header = ParseCacheControlDirectives(AtomicString("no-cache"), AtomicString());
    ...
    header = ParseCacheControlDirectives(AtomicString("no-cache no-store"), AtomicString());
    EXPECT_TRUE(header.parsed);
    EXPECT_TRUE(header.contains_no_cache);
    EXPECT_FALSE(header.contains_no_store);
    EXPECT_FALSE(header.contains_must_revalidate);
    ...
    header = ParseCacheControlDirectives(AtomicString("no-store must-revalidate"), AtomicString());
    EXPECT_TRUE(header.parsed);
    ...
    header = ParseCacheControlDirectives(AtomicString("max-age=0"), AtomicString());
    EXPECT_TRUE(header.parsed);
    EXPECT_FALSE(header.contains_no_cache);
    EXPECT_FALSE(header.contains_no_store);
    EXPECT_FALSE(header.contains_must_revalidate);
    ...
    header = ParseCacheControlDirectives(AtomicString("max-age=0 no-cache"), AtomicString());
    EXPECT_TRUE(header.parsed);
    EXPECT_FALSE(header.contains_no_cache);
    EXPECT_FALSE(header.contains_no_store);
    EXPECT_FALSE(header.contains_must_revalidate);
    ...
    header = ParseCacheControlDirectives(AtomicString("no-cache=foo"), AtomicString());
    EXPECT_TRUE(header.parsed);
    EXPECT_FALSE(header.contains_no_cache);
    EXPECT_FALSE(header.contains_no_store);
    EXPECT_FALSE(header.contains_must_revalidate);
    ...
    header = ParseCacheControlDirectives(AtomicString("nonsense"), AtomicString());
    EXPECT_TRUE(header.parsed);
    EXPECT_FALSE(header.contains_no_cache);
    EXPECT_FALSE(header.contains_no_store);
    EXPECT_FALSE(header.contains_must_revalidate);
    ...
    header = ParseCacheControlDirectives(AtomicString(), AtomicString("no-cache"));
    EXPECT_TRUE(header.parsed);
    EXPECT_TRUE(header.contains_no_cache);
    EXPECT_FALSE(header.contains_no_store);
    EXPECT_FALSE(header.contains_must_revalidate);
    ...
    header = ParseCacheControlDirectives(AtomicString("stale-while-revalidate=2,stale-while-revalidate=3"), AtomicString());
    EXPECT_TRUE(header.parsed);
    EXPECT_FALSE(header.contains_no_cache);
    EXPECT_FALSE(header.contains_no_store);
    EXPECT_FALSE(header.contains_must_revalidate);
...
TEST(HTTPParsersTest, ExtractMIMETypeFromMediaType)
{
    ...
    EXPECT_EQ(text_html, ExtractMIMETypeFromMediaType(AtomicString("text/html; charset=iso-8859-1")));

    // Quoted charset parameter
    EXPECT_EQ(text_html, ExtractMIMETypeFromMediaType(AtomicString("text/html; charset=\"quoted\"")));

    // Multiple parameters
    EXPECT_EQ(text_html, ExtractMIMETypeFromMediaType(AtomicString("text/html; charset=x; foo=bar")));
    ...
    // is accepted.
    EXPECT_EQ(text_html, ExtractMIMETypeFromMediaType(AtomicString("text/html,text/plain")));
    EXPECT_EQ(text_html, ExtractMIMETypeFromMediaType(AtomicString("text/html , text/plain")));
    EXPECT_EQ(text_html, ExtractMIMETypeFromMediaType(AtomicString("text/html\t,\ttext/plain")));
    EXPECT_EQ(text_html, ExtractMIMETypeFromMediaType(AtomicString("text/html,text/plain;charset=iso-8859-1")));

    // Preserves case.
    EXPECT_EQ("tExt/hTMl", ExtractMIMETypeFromMediaType(AtomicString("tExt/hTMl")));

    EXPECT_EQ(g_empty_string, ExtractMIMETypeFromMediaType(AtomicString(", text/html")));
    EXPECT_EQ(g_empty_string, ExtractMIMETypeFromMediaType(AtomicString("; text/html")));
...
{
    EXPECT_EQ("text/javascript", MinimizedMIMEType(AtomicString("application/javascript")));
    ...
    EXPECT_EQ("application/xml", MinimizedMIMEType(AtomicString("application/rss+xml")));
...
TEST(HTTPParsersTest, ParseHTTPRefresh)
{
    ...
    EXPECT_TRUE(ParseHTTPRefresh("1.5; url=dest", IsASCIISpace<UChar>, delay, url));
    EXPECT_EQ(base::Seconds(1), delay);
    EXPECT_EQ("dest", url);
    EXPECT_TRUE(ParseHTTPRefresh("1.5.9; url=dest", IsASCIISpace<UChar>, delay, url));
    EXPECT_EQ(base::Seconds(1), delay);
    EXPECT_EQ("dest", url);
    EXPECT_TRUE(ParseHTTPRefresh("7..; url=dest", IsASCIISpace<UChar>, delay, url));
...
TEST(HTTPParsersTest, ParseMultipartHeadersResult)
{
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        ...
        wtf_size_t end = 0;
        bool result = ParseMultipartHeadersFromBody(base::as_byte_span(tests[i].data), &response, &end);
...
TEST(HTTPParsersTest, ParseMultipartHeaders)
{
    ...
    response.AddHttpHeaderField(http_names::kLowerContentLength, AtomicString("999"));
    ...
    const char kData[] = "content-type: image/png\n"
                         ...
                         "\n";
    wtf_size_t end = 0;
    bool result = ParseMultipartHeadersFromBody(base::byte_span_from_cstring(kData), &response, &end);

    EXPECT_TRUE(result);
    EXPECT_EQ(strlen(kData), end);
    EXPECT_EQ("image/png", response.HttpHeaderField(http_names::kLowerContentType));
...
TEST(HTTPParsersTest, ParseMultipartHeadersContentCharset)
{
    ...
    EXPECT_TRUE(result);
    EXPECT_EQ(strlen(kData), end);
    EXPECT_EQ("text/html; charset=utf-8", response.HttpHeaderField(http_names::kLowerContentType));
    EXPECT_EQ("utf-8", response.TextEncodingName());
}

void testServerTimingHeader(const char* headerValue, Vector<Vector<String>> expectedResults)
{
    std::unique_ptr<ServerTimingHeaderVector> results = ParseServerTimingHeader(headerValue);
...
TEST(HTTPParsersTest, ParseServerTimingHeader)
{
  ...
  */
    ...
    // multiple entries
    testServerTimingHeader("metric1;dur=12.3;desc=description1,metric2;dur=45.6;desc=description2,"
                           "metric3;dur=78.9;desc=description3",
        { { "metric1", "12.3", "description1" }, { "metric2", "45.6", "description2" }, { "metric3", "78.9", "description3" } });
    testServerTimingHeader("metric1,metric2 ,metric3, metric4 , metric5",
        { { "metric1", "0", "" }, { "metric2", "0", "" }, { "metric3", "0", "" }, { "metric4", "0", "" }, { "metric5", "0", "" } });
    ...
    testServerTimingHeader(
        "metric;dur=12.3;desc=description1,metric;dur=45.6;desc=description2", { { "metric", "12.3", "description1" }, { "metric", "45.6", "description2" } });
    ...
    testServerTimingHeader(
        "metric1;foo=bar;desc=description;foo=bar;dur=123.4;foo=bar,metric2", { { "metric1", "123.4", "description" }, { "metric2", "0", "" } });
    ...
    testServerTimingHeader("metric==   \"\"foo;dur=123.4", { { "metric", "123.4", "" } });
    testServerTimingHeader("metric1==   \"\"foo,metric2", { { "metric1", "0", "" }, { "metric2", "0", "" } });
    ...
    testServerTimingHeader("}", { { "}", "0", "" } });
    testServerTimingHeader("{}", { { "{}", "0", "" } });
    testServerTimingHeader("{\"foo\":\"bar\"},metric", { { "{", "0", "" }, { "metric", "0", "" } });
}
...
TEST(HTTPParsersTest, ParseContentTypeOptionsTest)
{
    ...
    } cases[] = { { "nosniff", kContentTypeOptionsNosniff }, { "NOSNIFF", kContentTypeOptionsNosniff }, { "NOsniFF", kContentTypeOptionsNosniff },
        { "nosniff, nosniff", kContentTypeOptionsNosniff }, { "nosniff, not-nosniff", kContentTypeOptionsNosniff },
        { "nosniff, none", kContentTypeOptionsNosniff }, { " nosniff", kContentTypeOptionsNosniff }, { "NOSNIFF ", kContentTypeOptionsNosniff },
        { " NOsniFF ", kContentTypeOptionsNosniff }, { " nosniff, nosniff", kContentTypeOptionsNosniff },
        { "nosniff , not-nosniff", kContentTypeOptionsNosniff }, { " nosniff , none", kContentTypeOptionsNosniff }, { "", kContentTypeOptionsNone },
        { ",", kContentTypeOptionsNone }, { "none", kContentTypeOptionsNone }, { "none, nosniff", kContentTypeOptionsNone } };
...
TEST(HTTPParsersTest, ParseContentSecurityPoliciesmpty)
{
    auto csp = ParseContentSecurityPolicies(
        "", network::mojom::blink::ContentSecurityPolicyType::kEnforce, network::mojom::blink::ContentSecurityPolicySource::kHTTP, KURL("http://example.com"));
    EXPECT_TRUE(csp.empty());
}

TEST(HTTPParsersTest, ParseContentSecurityPoliciesMultiple)
{
    auto csp = ParseContentSecurityPolicies("frame-ancestors a.com, frame-ancestors b.com", network::mojom::blink::ContentSecurityPolicyType::kEnforce,
        network::mojom::blink::ContentSecurityPolicySource::kHTTP, KURL("http://example.com"));
...
{
    auto csp = ParseContentSecurityPolicies("frame-ancestors a.com", network::mojom::blink::ContentSecurityPolicyType::kEnforce,
        network::mojom::blink::ContentSecurityPolicySource::kHTTP, KURL("http://example.com"));
    ASSERT_EQ(1u, csp.size());

    // Header source:
    EXPECT_EQ(network::mojom::ContentSecurityPolicySource::kHTTP, csp[0]->header->source);

    // Header type:
    EXPECT_EQ(network::mojom::ContentSecurityPolicyType::kEnforce, csp[0]->header->type);
...
{
    auto csp = ParseContentSecurityPolicies("default-src a.com", network::mojom::blink::ContentSecurityPolicyType::kEnforce,
        network::mojom::blink::ContentSecurityPolicySource::kMeta, KURL("http://example.com"));
    ASSERT_EQ(1u, csp.size());

    // Header source:
    EXPECT_EQ(network::mojom::ContentSecurityPolicySource::kMeta, csp[0]->header->source);
...
{
    auto csp = ParseContentSecurityPolicies("frame-ancestors a.com", network::mojom::blink::ContentSecurityPolicyType::kReport,
        network::mojom::blink::ContentSecurityPolicySource::kHTTP, KURL("http://example.com"));
    ASSERT_EQ(1u, csp.size());
    ...
    // Header type:
    EXPECT_EQ(network::mojom::ContentSecurityPolicyType::kReport, csp[0]->header->type);
...
{
    auto policies = ParseContentSecurityPolicies("frame-ancestors 'none', "
...
{
    auto policies = ParseContentSecurityPolicies("report-to a b", network::mojom::blink::ContentSecurityPolicyType::kEnforce,
        network::mojom::blink::ContentSecurityPolicySource::kHTTP, KURL("http://example.com"));
...
{
    auto policies = ParseContentSecurityPolicies("report-uri ./report.py", network::mojom::blink::ContentSecurityPolicyType::kEnforce,
        network::mojom::blink::ContentSecurityPolicySource::kHTTP, KURL("http://example.com"));
...
{
    const std::string_view headers = "HTTP/1.1 200 OK\r\n"
                                     "Set-Cookie: a\r\n"
                                     "Set-Cookie: b\r\n\r\n";
    const auto parsed_headers = ParseHeaders(WTF::String::FromUTF8(headers), KURL("https://a.com"));

    ASSERT_TRUE(parsed_headers);
    ASSERT_TRUE(parsed_headers->no_vary_search_with_parse_error);
    ASSERT_TRUE(parsed_headers->no_vary_search_with_parse_error->is_parse_error());
    EXPECT_EQ(network::mojom::NoVarySearchParseError::kOk, parsed_headers->no_vary_search_with_parse_error->get_parse_error());
}
...
class NoVarySearchPrefetchEnabledTest : public ::testing::Test, public ::testing::WithParamInterface<NoVarySearchTestData> { };

TEST_P(NoVarySearchPrefetchEnabledTest, ParsingSuccess)
{
    const auto& test_data = GetParam();
    const auto parsed_headers = ParseHeaders(test_data.raw_headers, KURL("https://a.com"));

    ASSERT_TRUE(parsed_headers);
    ASSERT_TRUE(parsed_headers->no_vary_search_with_parse_error);
    ASSERT_TRUE(parsed_headers->no_vary_search_with_parse_error->is_no_vary_search());
    const auto& no_vary_search = parsed_headers->no_vary_search_with_parse_error->get_no_vary_search();
    ASSERT_TRUE(no_vary_search->search_variance);
    if (test_data.expected_vary_by_default) {
        EXPECT_THAT(no_vary_search->search_variance->get_no_vary_params(), test_data.expected_no_vary_params);
    } else {
        EXPECT_THAT(no_vary_search->search_variance->get_vary_params(), test_data.expected_vary_params);
    }
    EXPECT_EQ(no_vary_search->vary_on_key_order, test_data.expected_vary_on_key_order);
}

TEST(NoVarySearchHeaderValueParsingTest, ParsingSuccessForParseNoVarySearch)
{
    const auto no_vary_search_with_parse_error = blink::ParseNoVarySearch(R"(params=("a"))");

    ASSERT_TRUE(no_vary_search_with_parse_error);
    ASSERT_TRUE(no_vary_search_with_parse_error->is_no_vary_search());
    ASSERT_TRUE(no_vary_search_with_parse_error->get_no_vary_search()->search_variance);
    EXPECT_THAT(no_vary_search_with_parse_error->get_no_vary_search()->search_variance->get_no_vary_params(), Vector<String>({ "a" }));
    EXPECT_TRUE(no_vary_search_with_parse_error->get_no_vary_search()->vary_on_key_order);
}
...
INSTANTIATE_TEST_SUITE_P(NoVarySearchPrefetchEnabledTest, NoVarySearchPrefetchEnabledTest, testing::ValuesIn(GetNoVarySearchParsingSuccessTestData()));
