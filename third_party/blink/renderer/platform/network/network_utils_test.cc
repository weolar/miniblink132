...
TEST(NetworkUtilsTest, IsReservedIPAddress)
{
    ...
    EXPECT_FALSE(network_utils::IsReservedIPAddress("[FFC0:ba98:7654:3210:FEDC:BA98:7654:3210]"));
    EXPECT_FALSE(network_utils::IsReservedIPAddress("[2000:ba98:7654:2301:EFCD:BA98:7654:3210]"));
    // IPv4-mapped to IPv6
    ...
    EXPECT_TRUE(network_utils::IsReservedIPAddress("[FEC0:ba98:7654:3210:FEDC:BA98:7654:3210]"));
    ...
    for (int i = 0; i < 256; i++) {
        ...
        if (i == 0 || i == 10 || i == 127 || i == 192 || i > 223) {
            EXPECT_TRUE(network_utils::IsReservedIPAddress(String::FromUTF8(address_string)));
        } else {
            EXPECT_FALSE(network_utils::IsReservedIPAddress(String::FromUTF8(address_string)));
        }
    }
}
...
TEST(NetworkUtilsTest, GetDomainAndRegistry)
{
    ...
    EXPECT_EQ("", network_utils::GetDomainAndRegistry("127.0.0.1", network_utils::kIncludePrivateRegistries));
    ...
    EXPECT_EQ("example.notarealregistry", network_utils::GetDomainAndRegistry("www.example.notarealregistry", network_utils::kIncludePrivateRegistries));
}
