...
TEST(BlinkIPAddressTest, HashTraits)
{
    ...
    EXPECT_TRUE(WTF::IsHashTraitsDeletedValue<HashTraits<net::IPAddress>>(deleted_value));

    EXPECT_FALSE(WTF::IsHashTraitsDeletedValue<HashTraits<net::IPAddress>>(kEmptyIPAddr));
    EXPECT_FALSE(WTF::IsHashTraitsDeletedValue<HashTraits<net::IPAddress>>(kIPAddr1));
    EXPECT_FALSE(WTF::IsHashTraitsDeletedValue<HashTraits<net::IPAddress>>(kIPAddr2));

    EXPECT_TRUE(WTF::IsHashTraitsEmptyValue<HashTraits<net::IPAddress>>(kEmptyIPAddr));
    EXPECT_FALSE(WTF::IsHashTraitsEmptyValue<HashTraits<net::IPAddress>>(deleted_value));
    EXPECT_FALSE(WTF::IsHashTraitsEmptyValue<HashTraits<net::IPAddress>>(kIPAddr1));
    EXPECT_FALSE(WTF::IsHashTraitsEmptyValue<HashTraits<net::IPAddress>>(kIPAddr2));

    // Should be a 1 out of 4 billion chance these collide.
    EXPECT_NE(HashTraits<net::IPAddress>::GetHash(kIPAddr1), HashTraits<net::IPAddress>::GetHash(kIPAddr2));
}
