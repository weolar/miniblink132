...
TEST(MIMETypeRegistryTest, MimeTypeTest)
{
    ...
    EXPECT_TRUE(MIMETypeRegistry::IsSupportedImagePrefixedMIMEType("Image/Gif"));
    EXPECT_TRUE(MIMETypeRegistry::IsSupportedImageResourceMIMEType("Image/Gif"));
    static const UChar kUpper16[] = { 0x0049, 0x006d, 0x0061, 0x0067, 0x0065, 0x002f, 0x0067, 0x0069, 0x0066, 0 }; // Image/gif in UTF16
    EXPECT_TRUE(MIMETypeRegistry::IsSupportedImagePrefixedMIMEType(String(kUpper16)));
    EXPECT_TRUE(MIMETypeRegistry::IsSupportedImagePrefixedMIMEType("image/svg+xml"));
    EXPECT_FALSE(MIMETypeRegistry::IsSupportedImageResourceMIMEType("image/svg+xml"));
}
...
TEST(MIMETypeRegistryTest, PluginMimeTypes)
{
    ...
    EXPECT_EQ("application/pdf", MIMETypeRegistry::GetWellKnownMIMETypeForExtension("pdf").Utf8());
    EXPECT_EQ("application/x-shockwave-flash", MIMETypeRegistry::GetWellKnownMIMETypeForExtension("swf").Utf8());
}
...
{
    EXPECT_TRUE(MIMETypeRegistry::IsXMLExternalEntityMIMEType("application/xml-external-parsed-entity"));
    EXPECT_TRUE(MIMETypeRegistry::IsXMLExternalEntityMIMEType("text/xml-external-parsed-entity"));
    EXPECT_TRUE(MIMETypeRegistry::IsXMLExternalEntityMIMEType("application/XML-external-parsed-entity"));
    EXPECT_TRUE(MIMETypeRegistry::IsXMLExternalEntityMIMEType("text/XML-external-parsed-entity"));
