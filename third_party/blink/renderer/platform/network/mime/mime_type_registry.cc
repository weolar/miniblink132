...
public:
    ...
    {
        Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(mime_registry.BindNewPipeAndPassReceiver());
    }
...
template <typename CHARTYPE, typename SIZETYPE> std::string ToLowerASCIIInternal(CHARTYPE* str, SIZETYPE length)
{
...
STATIC_ASSERT_ENUM(MIMETypeRegistry::kNotSupported, media::SupportsType::kNotSupported);
STATIC_ASSERT_ENUM(MIMETypeRegistry::kSupported, media::SupportsType::kSupported);
STATIC_ASSERT_ENUM(MIMETypeRegistry::kMaybeSupported, media::SupportsType::kMaybeSupported);
...
String MIMETypeRegistry::GetMIMETypeForExtension(const String& ext)
{
    ...
    String mime_type;
    if (!registry_holder.mime_registry->GetMimeTypeFromExtension(ext.IsNull() ? "" : ext, &mime_type)) {
        return String();
    }
    return mime_type;
}
...
String MIMETypeRegistry::GetWellKnownMIMETypeForExtension(const String& ext)
{
    ...
    net::GetWellKnownMimeTypeFromExtension(WebStringToFilePath(ext).value(), &mime_type);
...
bool MIMETypeRegistry::IsSupportedImageResourceMIMEType(const String& mime_type)
{
    return IsSupportedImageMIMEType(mime_type);
}

bool MIMETypeRegistry::IsSupportedImagePrefixedMIMEType(const String& mime_type)
{
    std::string ascii_mime_type = ToLowerASCIIOrEmpty(mime_type);
    return (blink::IsSupportedImageMimeType(ascii_mime_type) || (ascii_mime_type.starts_with("image/") && blink::IsSupportedNonImageMimeType(ascii_mime_type)));
}

bool MIMETypeRegistry::IsSupportedImageMIMETypeForEncoding(const String& mime_type)
{
    return (
        EqualIgnoringASCIICase(mime_type, "image/jpeg") || EqualIgnoringASCIICase(mime_type, "image/png") || EqualIgnoringASCIICase(mime_type, "image/webp"));
}
...
bool MIMETypeRegistry::IsSupportedMediaMIMEType(const String& mime_type, const String& codecs)
{
    return SupportsMediaMIMEType(mime_type, codecs) != kNotSupported;
}

MIMETypeRegistry::SupportsType MIMETypeRegistry::SupportsMediaMIMEType(const String& mime_type, const String& codecs)
{
    const std::string ascii_mime_type = ToLowerASCIIOrEmpty(mime_type);
    std::vector<std::string> codec_vector;
    media::SplitCodecs(ToASCIIOrEmpty(codecs), &codec_vector);
    return static_cast<SupportsType>(media::IsSupportedMediaFormat(ascii_mime_type, codec_vector));
}

MIMETypeRegistry::SupportsType MIMETypeRegistry::SupportsMediaSourceMIMEType(const String& mime_type, const String& codecs)
{
    return kSupported;
//     const std::string ascii_mime_type = ToLowerASCIIOrEmpty(mime_type);
...
//         media::StreamParserFactory::IsTypeSupported(ascii_mime_type, base::span<const std::string>(parsed_codec_ids.data(), parsed_codec_ids.size())));
}
...
bool MIMETypeRegistry::IsJavaAppletMIMEType(const String& mime_type)
{
    ...
    // startsWith()
    return mime_type.StartsWithIgnoringASCIICase("application/x-java-applet") || mime_type.StartsWithIgnoringASCIICase("application/x-java-bean")
...
bool MIMETypeRegistry::IsSupportedFontMIMEType(const String& mime_type)
{
    ...
    return sub_type == "woff" || sub_type == "woff2" || sub_type == "otf" || sub_type == "ttf" || sub_type == "sfnt";
}
...
{
    if (EqualIgnoringASCIICase(mime_type, "text/xml") || EqualIgnoringASCIICase(mime_type, "application/xml")) {
        return true;
    }
    ...
    if (mime_type[0] == '/' || mime_type[length - 5] == '/' || !mime_type.EndsWithIgnoringASCIICase("+xml"))
        return false;
...
{
    return EqualIgnoringASCIICase(mime_type, "application/xml-external-parsed-entity") || EqualIgnoringASCIICase(mime_type, "text/xml-external-parsed-entity");
}

bool MIMETypeRegistry::IsPlainTextMIMEType(const String& mime_type)
{
    return mime_type.StartsWithIgnoringASCIICase("text/")
        && !(EqualIgnoringASCIICase(mime_type, "text/html") || EqualIgnoringASCIICase(mime_type, "text/xml") || EqualIgnoringASCIICase(mime_type, "text/xsl"));
}
