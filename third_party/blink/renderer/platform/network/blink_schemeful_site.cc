...
BlinkSchemefulSite::BlinkSchemefulSite(const net::SchemefulSite& site)
{
    ...
    DCHECK(site_as_origin_->Port() == std::max(url::DefaultPortForScheme(site_as_origin_->Protocol().Ascii()), 0));
}
...
// static
bool BlinkSchemefulSite::FromWire(const url::Origin& site_as_origin, BlinkSchemefulSite* out)
{
    ...
    scoped_refptr<const SecurityOrigin> security_origin = SecurityOrigin::CreateFromUrlOrigin(site_as_origin);
