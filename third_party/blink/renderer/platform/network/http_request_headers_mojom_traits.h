...
template <> struct StructTraits<network::mojom::HttpRequestHeadersDataView, blink::HTTPHeaderMap> {
    static WTF::Vector<network::mojom::blink::HttpRequestHeaderKeyValuePairPtr> headers(const blink::HTTPHeaderMap& map);

    static bool Read(network::mojom::HttpRequestHeadersDataView data, blink::HTTPHeaderMap* out);
};
