...
template <> struct PLATFORM_EXPORT StructTraits<network::mojom::SchemefulSiteDataView, blink::BlinkSchemefulSite> {
    static const scoped_refptr<const blink::SecurityOrigin>& site_as_origin(const blink::BlinkSchemefulSite& input)
    {
        return input.site_as_origin_;
    }

    static bool Read(network::mojom::SchemefulSiteDataView data, blink::BlinkSchemefulSite* out);
};
} // namespace mojo
