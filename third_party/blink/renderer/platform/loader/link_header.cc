...
static LinkHeader::LinkParameterName ParameterNameFromString(std::string_view name)
{
...
LinkHeaderSet::LinkHeaderSet(const String& header)
{
    ...
    std::string header_string(reinterpret_cast<const char*>(header.Characters8()), header.length());
