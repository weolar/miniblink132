...
void SubresourceIntegrity::ReportInfo::AddConsoleErrorMessage(const String& message)
{
...
bool SubresourceIntegrity::CheckSubresourceIntegrity(
    const IntegrityMetadataSet& metadata_set, const SegmentedBuffer* buffer, const KURL& resource_url, const Resource& resource, ReportInfo& report_info)
{
    // FetchResponseType::kError never arrives because it is a loading error.
    DCHECK_NE(resource.GetResponse().GetType(), network::mojom::FetchResponseType::kError);
    if (!resource.GetResponse().IsCorsSameOrigin()) {
        report_info.AddConsoleErrorMessage("Subresource Integrity: The resource '" + resource_url.ElidedString()
        ...
        report_info.AddUseCount(ReportInfo::UseCounterFeature::kSRIElementIntegrityAttributeButIneligible);
        return false;
    }

    return CheckSubresourceIntegrityImpl(metadata_set, buffer, resource_url, report_info);
}

bool SubresourceIntegrity::CheckSubresourceIntegrity(
    const String& integrity_metadata, IntegrityFeatures features, const SegmentedBuffer* buffer, const KURL& resource_url, ReportInfo& report_info)
{
    if (integrity_metadata.empty())
        return true;

    IntegrityMetadataSet metadata_set;
    ParseIntegrityAttribute(integrity_metadata, features, metadata_set, &report_info);
    return CheckSubresourceIntegrityImpl(metadata_set, buffer, resource_url, report_info);
}
...
blink::HashAlgorithm IntegrityAlgorithmToHashAlgorithm(IntegrityAlgorithm algorithm)
{
    switch (algorithm) {
    case IntegrityAlgorithm::kSha256:
...
bool SubresourceIntegrity::CheckSubresourceIntegrityImpl(
    const IntegrityMetadataSet& metadata_set, const SegmentedBuffer* buffer, const KURL& resource_url, ReportInfo& report_info)
{
    ...
    for (const IntegrityMetadata& metadata : metadata_set) {
        if (metadata.Algorithm() == max_algorithm && CheckSubresourceIntegrityDigest(metadata, buffer)) {
            report_info.AddUseCount(ReportInfo::UseCounterFeature::kSRIElementWithMatchingIntegrityAttribute);
            return true;
        }
    }
    ...
    DigestValue digest;
    if (ComputeDigest(IntegrityAlgorithmToHashAlgorithm(max_algorithm), buffer, digest)) {
        ...
        report_info.AddConsoleErrorMessage("Failed to find a valid digest in the 'integrity' attribute for "
                                           "resource '"
            + resource_url.ElidedString() + "' with computed " + IntegrityAlgorithmToString(max_algorithm) + " integrity '"
            + Base64Encode(base::span<const uint8_t>(digest.data(), digest.size())) + "'. The resource has been blocked.");
    } else {
        report_info.AddConsoleErrorMessage(
            "There was an error computing an integrity value for resource '" + resource_url.ElidedString() + "'. The resource has been blocked.");
    }
    report_info.AddUseCount(ReportInfo::UseCounterFeature::kSRIElementWithNonMatchingIntegrityAttribute);
    return false;
}

IntegrityAlgorithm SubresourceIntegrity::FindBestAlgorithm(const IntegrityMetadataSet& metadata_set)
{
    ...
    static_assert(IntegrityAlgorithm::kSha256 < IntegrityAlgorithm::kSha384 && IntegrityAlgorithm::kSha384 < IntegrityAlgorithm::kSha512,
...
bool SubresourceIntegrity::CheckSubresourceIntegrityDigest(const IntegrityMetadata& metadata, const SegmentedBuffer* buffer)
{
    blink::HashAlgorithm hash_algo = IntegrityAlgorithmToHashAlgorithm(metadata.Algorithm());
    ...
    converted_hash_vector.AppendSpan(base::span<const uint8_t>((const uint8_t*)hash_vector.data(), hash_vector.size()));
    return DigestsEqual(digest, converted_hash_vector);
}

SubresourceIntegrity::AlgorithmParseResult SubresourceIntegrity::ParseAttributeAlgorithm(
    const UChar*& begin, const UChar* end, IntegrityFeatures features, IntegrityAlgorithm& algorithm)
{
    static const AlgorithmPrefixPair kPrefixes[]
        = { { "sha256", IntegrityAlgorithm::kSha256 }, { "sha-256", IntegrityAlgorithm::kSha256 }, { "sha384", IntegrityAlgorithm::kSha384 },
              { "sha-384", IntegrityAlgorithm::kSha384 }, { "sha512", IntegrityAlgorithm::kSha512 }, { "sha-512", IntegrityAlgorithm::kSha512 } };
...
SubresourceIntegrity::AlgorithmParseResult SubresourceIntegrity::ParseAlgorithmPrefix(
    const UChar*& string_position, const UChar* string_end, const AlgorithmPrefixPair* prefix_table, size_t prefix_table_size, IntegrityAlgorithm& algorithm)
{
    for (size_t i = 0; i < prefix_table_size; i++) {
        const UChar* pos = string_position;
        if (SkipToken<UChar>(pos, string_end, prefix_table[i].first) && SkipExactly<UChar>(pos, string_end, '-')) {
            string_position = pos;
...
bool SubresourceIntegrity::ParseDigest(const UChar*& position, const UChar* end, String& digest)
{
    ...
    digest = NormalizeToBase64(String(begin, static_cast<wtf_size_t>(position - begin)));
    return true;
}

void SubresourceIntegrity::ParseIntegrityAttribute(const WTF::String& attribute, IntegrityFeatures features, IntegrityMetadataSet& metadata_set)
{
    return ParseIntegrityAttribute(attribute, features, metadata_set, nullptr);
}

void SubresourceIntegrity::ParseIntegrityAttribute(
    const WTF::String& attribute, IntegrityFeatures features, IntegrityMetadataSet& metadata_set, ReportInfo* report_info)
{
    ...
    while (position < end) {
        ...
        // them.
        AlgorithmParseResult parse_result = ParseAttributeAlgorithm(position, current_integrity_end, features, algorithm);
        if (parse_result == kAlgorithmUnknown) {
            ...
            if (report_info) {
                report_info->AddConsoleErrorMessage("Error parsing 'integrity' attribute ('" + attribute
        ...
        if (SkipExactly<UChar>(position, end, '?')) {
            ...
            if (begin != position && report_info) {
                report_info->AddConsoleErrorMessage(
                    "Ignoring unrecogized 'integrity' attribute option '" + String(begin, static_cast<wtf_size_t>(position - begin)) + "'.");
            }
        }
