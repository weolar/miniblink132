...
Vector<uint8_t> CreateTestSerializedDataWithMarker(uint32_t marker)
{
    ...
    serialized_data.ReserveInitialCapacity(sizeof(CachedMetadataHeader) + sizeof(kTestData));
    serialized_data.Append(reinterpret_cast<const uint8_t*>(&marker), sizeof(uint32_t));
    serialized_data.Append(reinterpret_cast<const uint8_t*>(&kTestDataTypeId), sizeof(uint32_t));
    serialized_data.Append(reinterpret_cast<const uint8_t*>(&kTestTag), sizeof(uint64_t));
...
{
    return CreateTestSerializedDataWithMarker(CachedMetadataHandler::kSingleEntryWithTag);
}
...
void CheckTestCachedMetadata(scoped_refptr<CachedMetadata> cached_metadata)
{
    ...
    EXPECT_THAT(cached_metadata->SerializedData(), testing::ElementsAreArray(CreateTestSerializedData()));
    ...
    if (absl::holds_alternative<Vector<uint8_t>>(drained_data)) {
        EXPECT_THAT(absl::get<Vector<uint8_t>>(drained_data), testing::ElementsAreArray(CreateTestSerializedData()));
        return;
    }
    CHECK(absl::holds_alternative<mojo_base::BigBuffer>(drained_data));
    mojo_base::BigBuffer drained_big_buffer = std::move(absl::get<mojo_base::BigBuffer>(drained_data));
    EXPECT_THAT(base::span(drained_big_buffer), testing::ElementsAreArray(CreateTestSerializedData()));
}

TEST(CachedMetadataTest, GetSerializedDataHeader)
{
    Vector<uint8_t> header_vector = CachedMetadata::GetSerializedDataHeader(kTestDataTypeId, /*estimated_body_size=*/10, kTestTag);
    EXPECT_EQ(header_vector.size(), sizeof(CachedMetadataHeader));

    const CachedMetadataHeader* header = reinterpret_cast<const CachedMetadataHeader*>(header_vector.data());
...
{
    CheckTestCachedMetadata(CachedMetadata::Create(kTestDataTypeId, kTestData, sizeof(kTestData), kTestTag));
}

TEST(CachedMetadataTest, CreateFromSerializedDataBuffer)
{
    Vector<uint8_t> data = CreateTestSerializedData();
    CheckTestCachedMetadata(CachedMetadata::CreateFromSerializedData(data.data(), data.size()));
}
...
TEST(CachedMetadataTest, CreateFromSerializedDataTooSmall)
{
    ...
    EXPECT_FALSE(CachedMetadata::CreateFromSerializedData(data.data(), data.size()));
...
TEST(CachedMetadataTest, CreateFromSerializedDataWithInvalidMarker)
{
    ...
    Vector<uint8_t> data = CreateTestSerializedDataWithMarker(CachedMetadataHandler::kSingleEntryWithTag + 1);
    EXPECT_FALSE(CachedMetadata::CreateFromSerializedData(data.data(), data.size()));
    EXPECT_FALSE(CachedMetadata::CreateFromSerializedData(data));
