...
public:
    ...
    virtual scoped_refptr<BlobDataHandle> DrainAsBlobDataHandle(BlobSizePolicy = BlobSizePolicy::kDisallowBlobWithInvalidSize)
    {
        return nullptr;
    }
    ...
    virtual scoped_refptr<EncodedFormData> DrainAsFormData()
    {
        return nullptr;
    }
...
PLATFORM_EXPORT std::ostream& operator<<(std::ostream& out, const BytesConsumer::PublicState& state);
PLATFORM_EXPORT std::ostream& operator<<(std::ostream& out, const BytesConsumer::Result& result);
