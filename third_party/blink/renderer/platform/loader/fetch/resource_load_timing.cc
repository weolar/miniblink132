...
{
    network::mojom::blink::LoadTimingInfoPtr timing = network::mojom::blink::LoadTimingInfo::New(false, 0, base::Time(), request_time_, proxy_start_,
        proxy_end_,
        network::mojom::blink::LoadTimingInfoConnectTiming::New(domain_lookup_start_, domain_lookup_end_, connect_start_, connect_end_, ssl_start_, ssl_end_),
        send_start_, send_end_, receive_headers_start_, receive_headers_end_, receive_non_informational_headers_start_, receive_early_hints_start_, push_start_,
        push_end_, worker_start_, worker_ready_, worker_fetch_start_, worker_respond_with_settled_, worker_router_evaluation_start_,
        worker_cache_lookup_start_);
    return timing;
}

void ResourceLoadTiming::SetDomainLookupStart(base::TimeTicks domain_lookup_start)
{
...
void ResourceLoadTiming::SetWorkerFetchStart(base::TimeTicks worker_fetch_start)
{
    worker_fetch_start_ = worker_fetch_start;
}

void ResourceLoadTiming::SetWorkerRespondWithSettled(base::TimeTicks worker_respond_with_settled)
{
    worker_respond_with_settled_ = worker_respond_with_settled;
}

void ResourceLoadTiming::SetWorkerRouterEvaluationStart(base::TimeTicks worker_router_evluation_start)
{
    worker_router_evaluation_start_ = worker_router_evluation_start;
}

void ResourceLoadTiming::SetWorkerCacheLookupStart(base::TimeTicks worker_cache_lookup_start)
{
...
void ResourceLoadTiming::SetReceiveHeadersStart(base::TimeTicks receive_headers_start)
{
    receive_headers_start_ = receive_headers_start;
}

void ResourceLoadTiming::SetReceiveNonInformationalHeaderStart(base::TimeTicks time)
{
...
double ResourceLoadTiming::CalculateMillisecondDelta(base::TimeTicks time) const
{
