...
public:
    ...
    {
        return *MakeGarbageCollected<FetchClientSettingsObjectSnapshot>(KURL("https://example.com/foo.html"), KURL("https://example.com/foo.html"),
            SecurityOrigin::Create(KURL("https://example.com/")), network::mojom::ReferrerPolicy::kDefault, "https://example.com/foo.html", HttpsState::kModern,
            AllowedByNosniff::MimeTypeCheck::kStrict, mojom::blink::InsecureRequestPolicy::kLeaveInsecureRequestsAlone,
...
{
    const auto& original_client_settings_object = CreateFetchClientSettingsObject();
    auto& properties
        = *MakeGarbageCollected<DetachableResourceFetcherProperties>(*MakeGarbageCollected<TestResourceFetcherProperties>(original_client_settings_object));
    ...
    EXPECT_EQ(properties.GetControllerServiceWorkerMode(), mojom::ControllerServiceWorkerMode::kNoController);
    ...
    EXPECT_NE(&client_settings_object, &properties.GetFetchClientSettingsObject());
    EXPECT_EQ(properties.GetFetchClientSettingsObject().BaseUrl(), KURL("https://example.com/foo.html"));
    EXPECT_FALSE(properties.IsOutermostMainFrame());
    EXPECT_EQ(properties.GetControllerServiceWorkerMode(), mojom::ControllerServiceWorkerMode::kNoController);
    // We cannot call ServiceWorkerId as the service worker mode is kNoController.
    EXPECT_FALSE(properties.IsPaused());
...
{
    const auto& original_client_settings_object = CreateFetchClientSettingsObject();
    auto& original_properties = *MakeGarbageCollected<TestResourceFetcherProperties>(original_client_settings_object);
    auto& properties = *MakeGarbageCollected<DetachableResourceFetcherProperties>(original_properties);
    ...
    EXPECT_EQ(properties.GetControllerServiceWorkerMode(), mojom::ControllerServiceWorkerMode::kControlled);
    ...
    EXPECT_EQ(scheduler::FrameStatus::kMainFrameVisible, properties.GetFrameStatus());
