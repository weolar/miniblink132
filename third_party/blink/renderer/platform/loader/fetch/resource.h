...
class PLATFORM_EXPORT Resource : public GarbageCollected<Resource>, public MemoryPressureListener {
public:
    ...
    virtual WTF::TextEncoding Encoding() const
    {
    ...
    virtual void AppendData(absl::variant<SegmentedBuffer, base::span<const char>>);
    virtual void FinishAsError(const ResourceError&, base::SingleThreadTaskRunner*);

    void SetLinkPreload(bool is_link_preload)
    {
        link_preload_ = is_link_preload;
    }
    bool IsLinkPreload() const
    {
        return link_preload_;
    }
    ...
    {
        return LastResourceRequest().InspectorId();
    }
    ...
    ResourceLoaderOptions& MutableOptions()
    {
        return options_;
    }
    ...
    {
        std::pair<ResourcePriority, ResourcePriority> result = ComputePriorityFromObservers();
    ...
    // on the given base::SingleThreadTaskRunner. Otherwise, the given
    // base::SingleThreadTaskRunner is unused.
    void AddFinishObserver(ResourceFinishObserver*, base::SingleThreadTaskRunner*);
    ...
    virtual ResourceStatus GetContentStatus() const
    {
        return status_;
    }
    ...
    size_t size() const
    {
        return EncodedSize() + DecodedSize() + OverheadSize();
    }
    ...
    {
        return overhead_size_;
    }
    virtual size_t CodeCacheSize() const
    {
        return 0;
    }
    ...
    virtual void Finish(base::TimeTicks finish_time, base::SingleThreadTaskRunner*);
    void FinishForTest()
    {
        Finish(base::TimeTicks(), nullptr);
    }
    ...
    virtual bool WillFollowRedirect(const ResourceRequest&, const ResourceResponse&);
    ...
    virtual void ResponseBodyReceived(ResponseBodyLoaderDrainableInterface& body_loader, scoped_refptr<base::SingleThreadTaskRunner> loader_task_runner)
    {
    }
    virtual void DidReceiveDecodedData(const String& data, std::unique_ptr<ParkableStringImpl::SecureDigest> digest)
    {
    }
    ...
    {
        return error_ && error_->IsCancellation();
    }
    bool ErrorOccurred() const
    {
        return status_ == ResourceStatus::kLoadError || status_ == ResourceStatus::kDecodeError;
    }
    ...
    bool MustRevalidateDueToCacheHeaders(bool allow_stale, UseCounter& use_counter) const;
    ...
    bool StaleRevalidationStarted() const
    {
        return stale_revalidation_started_;
    }
    void SetStaleRevalidationStarted()
    {
        stale_revalidation_started_ = true;
    }
    ...
    virtual void DidSendData(uint64_t /* bytesSent */, uint64_t /* totalBytesToBeSent */)
    {
    }
    ...
    static const char* ResourceTypeToString(ResourceType, const AtomicString& fetch_initiator_name);
    ...
    void SetIsPreloadedByEarlyHints()
    {
        is_preloaded_by_early_hints_ = true;
    }

    bool IsPreloadedByEarlyHints() const
    {
        return is_preloaded_by_early_hints_;
    }

    virtual std::unique_ptr<BackgroundResponseProcessorFactory> MaybeCreateBackgroundResponseProcessorFactory();

    virtual bool HasClientsOrObservers() const
    {
        return !clients_.empty() || !clients_awaiting_callback_.empty() || !finished_clients_.empty() || !finish_observers_.empty();
    }

protected:
    Resource(const ResourceRequestHead&, ResourceType, const ResourceLoaderOptions&);

    virtual std::pair<ResourcePriority, ResourcePriority> ComputePriorityFromObservers()
    {
    ...
    {
        return clients_.Contains(client) || clients_awaiting_callback_.Contains(client) || finished_clients_.Contains(client);
    }

    bool IsSuccessfulRevalidationResponse(const ResourceResponse& response) const
    {
    ...
    public:
        explicit RedirectPair(const ResourceRequestHead& request, const ResourceResponse& redirect_response)
            : request_(request)
    ...
    const Vector<RedirectPair>& RedirectChain() const
    {
...
private:
    ...
    std::set<net::SchemefulSite> existing_top_frame_sites_in_cache_ ALLOW_DISCOURAGED_TYPE("TODO(crbug.com/1404327)");
};
...
public:
    virtual Resource* Create(const ResourceRequest&, const ResourceLoaderOptions&, const TextResourceDecoderOptions&) const = 0;
...
protected:
    explicit ResourceFactory(ResourceType type, TextResourceDecoderOptions::ContentType content_type)
        : type_(type)
...
protected:
    ...
    virtual Resource* Create(const ResourceRequest&, const ResourceLoaderOptions&) const = 0;

    Resource* Create(const ResourceRequest& request, const ResourceLoaderOptions& options, const TextResourceDecoderOptions&) const final
    {
