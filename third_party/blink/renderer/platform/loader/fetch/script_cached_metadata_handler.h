...
class PLATFORM_EXPORT ScriptCachedMetadataHandler : public CachedMetadataHandler {
public:
    ScriptCachedMetadataHandler(const WTF::TextEncoding&, std::unique_ptr<CachedMetadataSender>);
    ~ScriptCachedMetadataHandler() override;
    void Trace(Visitor*) const override;
    void SetCachedMetadata(CodeCacheHost*, uint32_t, const uint8_t*, size_t) override;
    void ClearCachedMetadata(CodeCacheHost*, ClearCacheType) override;
    scoped_refptr<CachedMetadata> GetCachedMetadata(uint32_t, GetCachedMetadataBehavior = kCrashIfUnchecked) const override;
    ...
    void OnMemoryDump(WebProcessMemoryDump* pmd, const String& dump_prefix) const override;
...
protected:
    ...
    CachedMetadataSender* Sender() const
    {
        return sender_.get();
    }
...
struct CachedMetadataHeaderWithHash {
    ...
    uint32_t marker; // Must be CachedMetadataHandler::kSingleEntryWithHashAndPadding.
...
class PLATFORM_EXPORT ScriptCachedMetadataHandlerWithHashing final : public ScriptCachedMetadataHandler {
public:
    ScriptCachedMetadataHandlerWithHashing(const WTF::TextEncoding& encoding, std::unique_ptr<CachedMetadataSender> sender)
    ...
    bool HashRequired() const override
    {
        return true;
    }
