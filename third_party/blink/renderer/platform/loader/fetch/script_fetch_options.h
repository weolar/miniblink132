...
public:
    ...
    ScriptFetchOptions();
    ScriptFetchOptions(const String& nonce, const IntegrityMetadataSet& integrity_metadata, const String& integrity_attribute, ParserDisposition parser_state,
        network::mojom::CredentialsMode credentials_mode, network::mojom::ReferrerPolicy referrer_policy, mojom::blink::FetchPriorityHint fetch_priority_hint,
        RenderBlockingBehavior render_blocking_behavior, RejectCoepUnsafeNone reject_coep_unsafe_none = RejectCoepUnsafeNone(false));
    ~ScriptFetchOptions();
    ...
    const ParserDisposition& ParserState() const
    {
    ...
    void UpdateReferrerPolicyAfterResponseReceived(network::mojom::ReferrerPolicy response_referrer_policy) const
    {
        referrer_policy_ = response_referrer_policy;
    }

    void SetAttributionReportingEligibility(AttributionReportingEligibility eligibility)
    {
    ...
    // Steps 1 and 3.
    FetchParameters CreateFetchParameters(const KURL&, const SecurityOrigin*, const DOMWrapperWorld* world, CrossOriginAttributeValue, const WTF::TextEncoding&,
...
private:
    ...
    const RenderBlockingBehavior render_blocking_behavior_ = RenderBlockingBehavior::kUnset;
    ...
    const RejectCoepUnsafeNone reject_coep_unsafe_none_ = RejectCoepUnsafeNone(false);

    // https://wicg.github.io/attribution-reporting-api
    // TODO(crbug.com/1338976): make this member const once the attributionsrc
    // spec is drafted.
    AttributionReportingEligibility attribution_reporting_eligibility_ = AttributionReportingEligibility::kIneligible;
};
