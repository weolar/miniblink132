...
class FakeUseCounter : public GarbageCollected<FakeUseCounter>, public UseCounter {
private:
...
TEST_F(ResourceTest, Vary)
{
    ...
    // Irrelevant header
    response.SetHttpHeaderField(http_names::kVary, AtomicString("definitelynotarealheader"));
    resource->SetResponse(response);
    EXPECT_FALSE(resource->MustReloadDueToVaryHeader(new_request));

    // Header present on new but not old
    new_request.SetHttpHeaderField(http_names::kUserAgent, AtomicString("something"));
    ...
    old_request.SetHttpHeaderField(http_names::kUserAgent, AtomicString("something"));
    old_request.SetHttpHeaderField(http_names::kReferer, AtomicString("http://foo.com"));
    ...
    response.SetHttpHeaderField(http_names::kVary, AtomicString("User-Agent, Referer"));
    resource->SetResponse(response);
    EXPECT_TRUE(resource->MustReloadDueToVaryHeader(new_request));

    // Two matching
    new_request.SetHttpHeaderField(http_names::kReferer, AtomicString("http://foo.com"));
    EXPECT_FALSE(resource->MustReloadDueToVaryHeader(new_request));
}
...
TEST_F(ResourceTest, RevalidationSucceededUpdateHeaders)
{
    ...
    ResourceResponse response(url);
    response.SetHttpStatusCode(200);
    response.AddHttpHeaderField(AtomicString("keep-alive"), AtomicString("keep-alive value"));
    response.AddHttpHeaderField(http_names::kExpires, AtomicString("expires value"));
    response.AddHttpHeaderField(http_names::kLastModified, AtomicString("last-modified value"));
    response.AddHttpHeaderField(AtomicString("proxy-authenticate"), AtomicString("proxy-authenticate value"));
    response.AddHttpHeaderField(AtomicString("proxy-connection"), AtomicString("proxy-connection value"));
    response.AddHttpHeaderField(AtomicString("x-custom"), AtomicString("custom value"));
    resource->ResponseReceived(response);
    resource->FinishForTest();
    MemoryCache::Get()->Add(resource);
    ...
    EXPECT_EQ("keep-alive value", resource->GetResponse().HttpHeaderField(AtomicString("keep-alive")));
    EXPECT_EQ("expires value", resource->GetResponse().HttpHeaderField(http_names::kExpires));
    EXPECT_EQ("last-modified value", resource->GetResponse().HttpHeaderField(http_names::kLastModified));
    EXPECT_EQ("proxy-authenticate value", resource->GetResponse().HttpHeaderField(AtomicString("proxy-authenticate")));
    EXPECT_EQ("proxy-authenticate value", resource->GetResponse().HttpHeaderField(AtomicString("proxy-authenticate")));
    EXPECT_EQ("proxy-connection value", resource->GetResponse().HttpHeaderField(AtomicString("proxy-connection")));
    EXPECT_EQ("custom value", resource->GetResponse().HttpHeaderField(AtomicString("x-custom")));
    ...
    revalidating_response.AddHttpHeaderField(AtomicString("keep-alive"), AtomicString("garbage"));
    revalidating_response.AddHttpHeaderField(http_names::kExpires, AtomicString("garbage"));
    revalidating_response.AddHttpHeaderField(http_names::kLastModified, AtomicString("garbage"));
    revalidating_response.AddHttpHeaderField(AtomicString("proxy-authenticate"), AtomicString("garbage"));
    revalidating_response.AddHttpHeaderField(AtomicString("proxy-connection"), AtomicString("garbage"));
    // Header that is updated with 304 code.
    revalidating_response.AddHttpHeaderField(AtomicString("x-custom"), AtomicString("updated"));
    resource->ResponseReceived(revalidating_response);
    EXPECT_TRUE(resource->HasSuccessfulRevalidation());
    ...
    EXPECT_EQ("proxy-authenticate value", resource->GetResponse().HttpHeaderField(AtomicString("proxy-authenticate")));
    EXPECT_EQ("proxy-connection value", resource->GetResponse().HttpHeaderField(AtomicString("proxy-connection")));
    EXPECT_EQ("updated", resource->GetResponse().HttpHeaderField(AtomicString("x-custom")));
...
TEST_F(ResourceTest, RedirectDuringRevalidation)
{
    ...
    redirect_response.SetHttpHeaderField(http_names::kLocation, AtomicString(redirect_target_url.GetString()));
    redirect_response.SetHttpStatusCode(308);
    ResourceRequest redirected_revalidating_request(redirect_target_url);
    resource->WillFollowRedirect(redirected_revalidating_request, redirect_response);
    EXPECT_FALSE(resource->IsCacheValidator());
    EXPECT_FALSE(resource->HasSuccessfulRevalidation());
    EXPECT_EQ(url, resource->GetResourceRequest().Url());
...
TEST_F(ResourceTest, StaleWhileRevalidateCacheControlWithRedirect)
{
    ...
    ResourceResponse response(url);
    response.SetHttpHeaderField(http_names::kCacheControl, AtomicString("max-age=50"));
    response.SetHttpStatusCode(200);
    ...
    redirect_response.SetHttpHeaderField(http_names::kCacheControl, AtomicString("max-age=0, stale-while-revalidate=40"));
...
TEST_F(ResourceTest, FreshnessLifetime)
{
    ...
    redirect_response.SetHttpHeaderField(http_names::kLocation, AtomicString(redirect_target_url.GetString()));
    redirect_response.SetHttpStatusCode(302);
    redirect_response.SetHttpHeaderField(http_names::kCacheControl, AtomicString("max-age=10"));
    redirect_response.SetAsyncRevalidationRequested(true);
    ResourceRequest redirected_revalidating_request(redirect_target_url);

    auto* resource_redirected = MakeGarbageCollected<MockResource>(url);
    resource_redirected->WillFollowRedirect(redirected_revalidating_request, redirect_response);
    resource_redirected->ResponseReceived(response);
    resource_redirected->FinishForTest();

    EXPECT_EQ(resource_redirected->FreshnessLifetime(*use_counter), base::Seconds(10));
}
