...
void NotifyFinishObservers(HeapHashSet<WeakMember<ResourceFinishObserver>>* observers)
{
    for (const auto& observer : *observers)
        observer->NotifyFinished();
}

void GetSharedBufferMemoryDump(SharedBuffer* buffer, const String& dump_prefix, WebProcessMemoryDump* memory_dump)
{
    size_t dump_size;
    String dump_name;
    buffer->GetMemoryDumpNameAndSize(dump_name, dump_size);

    WebMemoryAllocatorDump* dump = memory_dump->CreateMemoryAllocatorDump(dump_prefix + dump_name);
    dump->AddScalar("size", "bytes", dump_size);
    memory_dump->AddSuballocation(dump->Guid(), String(WTF::Partitions::kAllocatedObjectPoolName));
}
...
const auto kHeaderPrefixesToIgnoreAfterRevalidation = std::to_array<const char*>({ "content-", "x-content-", "x-webkit-" });
...
inline bool ShouldUpdateHeaderAfterRevalidation(const AtomicString& header)
{
    ...
    for (const auto* header_prefix_to_ignore : kHeaderPrefixesToIgnoreAfterRevalidation) {
...
{
    const base::Clock* clock = g_clock_for_testing ? g_clock_for_testing : base::DefaultClock::GetInstance();
    return clock->Now();
}

Resource::Resource(const ResourceRequestHead& request, ResourceType type, const ResourceLoaderOptions& options)
    : type_(type)
...
{
    scoped_refptr<const SecurityOrigin> top_frame_origin = resource_request_.TopFrameOrigin();
    if (top_frame_origin) {
...
void Resource::CheckResourceIntegrity()
{
    ...
    if (SubresourceIntegrity::CheckSubresourceIntegrity(IntegrityMetadata(), Data(), Url(), *this, integrity_report_info_)) {
        integrity_disposition_ = ResourceIntegrityDisposition::kPassed;
    } else {
        integrity_disposition_ = ResourceIntegrityDisposition::kFailedIntegrityMetadata;
    }
...
void Resource::AppendData(absl::variant<SegmentedBuffer, base::span<const char>> data)
{
...
void Resource::TriggerNotificationForFinishObservers(base::SingleThreadTaskRunner* task_runner)
{
    if (finish_observers_.empty())
        return;

    auto* new_collections = MakeGarbageCollected<HeapHashSet<WeakMember<ResourceFinishObserver>>>(std::move(finish_observers_));
    finish_observers_.clear();

    task_runner->PostTask(FROM_HERE, WTF::BindOnce(&NotifyFinishObservers, WrapPersistent(new_collections)));
...
void Resource::FinishAsError(const ResourceError& error, base::SingleThreadTaskRunner* task_runner)
{
    error_ = error;
    revalidation_status_ = RevalidationStatus::kNoRevalidatingOrFailed;
    ...
    if (failed_during_start && !NeedsSynchronousCacheHit(GetType(), options_)) {
        task_runner->PostTask(FROM_HERE, WTF::BindOnce(&Resource::NotifyFinished, WrapWeakPersistent(this)));
    } else {
        NotifyFinished();
    }
}

void Resource::Finish(base::TimeTicks load_response_end, base::SingleThreadTaskRunner* task_runner)
{
    DCHECK(!IsCacheValidator());
...
bool Resource::MustRefetchDueToIntegrityMetadata(const FetchParameters& params) const
{
    if (params.IntegrityMetadata().empty())
        return false;

    return !IntegrityMetadata::SetsEqual(IntegrityMetadata(), params.IntegrityMetadata());
}
...
static base::TimeDelta CurrentAge(const ResourceResponse& response, base::Time response_timestamp, UseCounter& use_counter)
{
    // RFC2616 13.2.3
    ...
    base::TimeDelta corrected_received_age = age_value ? std::max(apparent_age, age_value.value()) : apparent_age;
    base::TimeDelta resident_time = Now() - response_timestamp;
    return corrected_received_age + resident_time;
}

static base::TimeDelta FreshnessLifetime(const ResourceResponse& response, base::Time response_timestamp, UseCounter& use_counter)
{
    ...
    if (!response.CurrentRequestUrl().ProtocolIsInHTTPFamily() && !response.CurrentRequestUrl().ProtocolIs("filesystem"))
...
{
    base::TimeDelta lifetime = blink::FreshnessLifetime(GetResponse(), response_timestamp_, use_counter);
    for (const auto& redirect : redirect_chain_) {
        base::TimeDelta redirect_lifetime = blink::FreshnessLifetime(redirect.redirect_response_, response_timestamp_, use_counter);
        lifetime = std::min(lifetime, redirect_lifetime);
    }
    return lifetime;
}

static bool CanUseResponse(const ResourceResponse& response, bool allow_stale, base::Time response_timestamp, UseCounter& use_counter)
{
    if (response.IsNull())
        return false;
    ...
    base::TimeDelta max_life = FreshnessLifetime(response, response_timestamp, use_counter);
    if (allow_stale)
...
bool Resource::WillFollowRedirect(const ResourceRequest& new_request, const ResourceResponse& redirect_response)
{
    if (IsCacheValidator()) {
        RevalidationFailed();
    }
...
void Resource::AddClient(ResourceClient* client, base::SingleThreadTaskRunner* task_runner)
{
    ...
    if ((ErrorOccurred() || !GetResponse().IsNull()) && !NeedsSynchronousCacheHit(GetType(), options_)) {
        clients_awaiting_callback_.insert(client);
        if (!async_finish_pending_clients_task_.IsActive()) {
            async_finish_pending_clients_task_
                = PostCancellableTask(*task_runner, FROM_HERE, WTF::BindOnce(&Resource::FinishPendingClients, WrapWeakPersistent(this)));
        }
        return;
    }
...
void Resource::RemoveClient(ResourceClient* client)
{
    ...
    if (clients_awaiting_callback_.empty() && async_finish_pending_clients_task_.IsActive()) {
...
void Resource::AddFinishObserver(ResourceFinishObserver* client, base::SingleThreadTaskRunner* task_runner)
{
    CHECK(!is_add_remove_client_prohibited_);
...
Resource::MatchStatus Resource::CanReuse(const FetchParameters& params) const
{
    ...
    scoped_refptr<const SecurityOrigin> existing_origin = GetResourceRequest().RequestorOrigin();
    scoped_refptr<const SecurityOrigin> new_origin = new_request.RequestorOrigin();
    ...
    if (GetResponse().WasFetchedViaServiceWorker() && GetResponse().GetType() == network::mojom::FetchResponseType::kOpaque
    ...
    if (current_request.GetCredentialsMode() != new_request.GetCredentialsMode()) {
    ...
    if (new_options.synchronous_policy == kRequestSynchronously || options_.synchronous_policy == kRequestSynchronously) {
    ...
    if (current_request.HttpMethod() != http_names::kGET || new_request.HttpMethod() != http_names::kGET) {
    ...
    if (new_request.GetCredentialsMode() != current_request.GetCredentialsMode()) {
        return MatchStatus::kRequestCredentialsModeDoesNotMatch;
    }
...
{
    if (base::FeatureList::IsEnabled(features::kReleaseResourceDecodedDataOnMemoryPressure)) {
        Prune();
    }
}

void Resource::OnMemoryDump(WebMemoryDumpLevelOfDetail level_of_detail, WebProcessMemoryDump* memory_dump) const
{
    ...
    WebMemoryAllocatorDump* dump = memory_dump->CreateMemoryAllocatorDump(dump_name);
    ...
    if (level_of_detail == WebMemoryDumpLevelOfDetail::kDetailed) {
        ...
        std::sort(client_names.begin(), client_names.end(), WTF::CodeUnitCompareLessThan);

        StringBuilder builder;
        for (wtf_size_t i = 0; i < client_names.size() && i < kMaxResourceClientToShowInMemoryInfra; ++i) {
            if (i > 0)
                builder.Append(" / ");
        ...
        if (client_names.size() > kMaxResourceClientToShowInMemoryInfra) {
            builder.Append(" / and ");
            builder.AppendNumber(client_names.size() - kMaxResourceClientToShowInMemoryInfra);
            builder.Append(" more");
        }
    ...
    WebMemoryAllocatorDump* overhead_dump = memory_dump->CreateMemoryAllocatorDump(overhead_name);
    overhead_dump->AddScalar("size", "bytes", OverheadSize());
    memory_dump->AddSuballocation(overhead_dump->Guid(), String(WTF::Partitions::kAllocatedObjectPoolName));
}

String Resource::GetMemoryDumpName() const
{
    return String::Format("web_cache/%s_resources/", ResourceTypeToString(GetType(), Options().initiator_info.name)) + String::Number(InspectorId());
}
...
void Resource::RevalidationSucceeded(const ResourceResponse& validating_response)
{
    SECURITY_CHECK(redirect_chain_.empty());
    SECURITY_CHECK(EqualIgnoringFragmentIdentifier(validating_response.CurrentRequestUrl(), GetResponse().CurrentRequestUrl()));
...
bool Resource::CanReuseRedirectChain(UseCounter& use_counter) const
{
    for (auto& redirect : redirect_chain_) {
        if (!CanUseResponse(redirect.redirect_response_, false /*allow_stale*/, response_timestamp_, use_counter)) {
            return false;
        }
        if (redirect.request_.CacheControlContainsNoCache() || redirect.request_.CacheControlContainsNoStore())
            return false;
    }
    return true;
}
...
{
    return GetResponse().CacheControlContainsNoStore() || GetResourceRequest().CacheControlContainsNoStore();
}

bool Resource::MustReloadDueToVaryHeader(const ResourceRequest& new_request) const
{
    ...
    for (const String& header : vary_headers) {
        ...
        if (GetResourceRequest().HttpHeaderField(atomic_header) != new_request.HttpHeaderField(atomic_header)) {
            return true;
        }
    }
    return false;
...
bool Resource::MustRevalidateDueToCacheHeaders(bool allow_stale, UseCounter& use_counter) const
{
    return !CanUseResponse(GetResponse(), allow_stale, response_timestamp_, use_counter) || GetResourceRequest().CacheControlContainsNoCache()
        || GetResourceRequest().CacheControlContainsNoStore();
}

static bool ShouldRevalidateStaleResponse(const ResourceResponse& response, base::Time response_timestamp, UseCounter& use_counter)
{
    base::TimeDelta staleness = response.CacheControlStaleWhileRevalidate();
    if (staleness.is_zero())
        return false;

    return CurrentAge(response, response_timestamp, use_counter) > FreshnessLifetime(response, response_timestamp, use_counter);
}
...
bool Resource::ShouldRevalidateStaleResponse(UseCounter& use_counter) const
{
    for (auto& redirect : redirect_chain_) {
        ...
        if (blink::ShouldRevalidateStaleResponse(redirect.redirect_response_, response_timestamp_, use_counter)) {
            return true;
        }
    }

    return blink::ShouldRevalidateStaleResponse(GetResponse(), response_timestamp_, use_counter);
}
...
bool Resource::CanUseCacheValidator() const
{
    ...
    return GetResponse().HasCacheValidatorFields() || GetResourceRequest().HasCacheValidatorFields();
}

size_t Resource::CalculateOverheadSize() const
{
    static const int kAverageClientsHashMapSize = 384;
    return sizeof(Resource) + GetResponse().MemoryUsage() + kAverageClientsHashMapSize + GetResourceRequest().Url().GetString().length() * 2;
}

void Resource::DidChangePriority(ResourceLoadPriority load_priority, int intra_priority_value)
{
...
{
    if (resource_width) {
        resource_request_.SetHttpHeaderField(AtomicString("sec-ch-width"), resource_width);
    } else {
...
static const char* InitiatorTypeNameToString(const AtomicString& initiator_type_name)
{
    ...
    static_assert(fetch_initiator_type_names::kNamesCount == 20, "New FetchInitiatorTypeNames should be handled correctly here.");

    return "Resource";
}

const char* Resource::ResourceTypeToString(ResourceType type, const AtomicString& fetch_initiator_name)
{
    switch (type) {
...
std::unique_ptr<BackgroundResponseProcessorFactory> Resource::MaybeCreateBackgroundResponseProcessorFactory()
{
    return nullptr;
}
