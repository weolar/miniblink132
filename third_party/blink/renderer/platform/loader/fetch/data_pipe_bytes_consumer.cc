...
void DataPipeBytesConsumer::CompletionNotifier::SignalError(const BytesConsumer::Error& error)
{
    if (bytes_consumer_)
...
DataPipeBytesConsumer::DataPipeBytesConsumer(
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, mojo::ScopedDataPipeConsumerHandle data_pipe, CompletionNotifier** notifier)
    : task_runner_(std::move(task_runner))
    , data_pipe_(std::move(data_pipe))
    , watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, task_runner_)
{
    DCHECK(data_pipe_.is_valid());
    *notifier = MakeGarbageCollected<CompletionNotifier>(this);
    watcher_.Watch(data_pipe_.get(), MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED,
...
BytesConsumer::Result DataPipeBytesConsumer::BeginRead(base::span<const char>& buffer)
{
...
BytesConsumer::Result DataPipeBytesConsumer::EndRead(size_t read)
{
    ...
    if (has_pending_notification_) {
        ...
        task_runner_->PostTask(FROM_HERE, WTF::BindOnce(&DataPipeBytesConsumer::Notify, WrapPersistent(this), MOJO_RESULT_OK));
    }
    return Result::kOk;
}
