...
ScriptFetchOptions::ScriptFetchOptions(const String& nonce, const IntegrityMetadataSet& integrity_metadata, const String& integrity_attribute,
    ParserDisposition parser_state, network::mojom::CredentialsMode credentials_mode, network::mojom::ReferrerPolicy referrer_policy,
    mojom::blink::FetchPriorityHint fetch_priority_hint, RenderBlockingBehavior render_blocking_behavior, RejectCoepUnsafeNone reject_coep_unsafe_none)
    : nonce_(nonce)
...
FetchParameters ScriptFetchOptions::CreateFetchParameters(const KURL& url, const SecurityOrigin* security_origin, const DOMWrapperWorld* world_for_csp,
    CrossOriginAttributeValue cross_origin, const WTF::TextEncoding& encoding, FetchParameters::DeferOption defer) const
{
    ...
    resource_loader_options.initiator_info.name = fetch_initiator_type_names::kScript;
    ...
    params.MutableResourceRequest().SetFetchIntegrity(GetIntegrityAttributeValue());
    ...
    if (attribution_reporting_eligibility_ == AttributionReportingEligibility::kEligible) {
        params.MutableResourceRequest().SetAttributionReportingEligibility(network::mojom::AttributionReportingEligibility::kEventSourceOrTrigger);
    }
