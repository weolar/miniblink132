...
{
    DCHECK_LE(network::GetClientHintToNameMap().size(), static_cast<size_t>(network::mojom::WebClientHintsType::kMaxValue) + 1);
}
...
void ClientHintsPreferences::CombineWith(const ClientHintsPreferences& preferences)
{
    for (const auto& elem : network::GetClientHintToNameMap()) {
        const auto& type = elem.first;
...
bool ClientHintsPreferences::UpdateFromMetaCH(
    const String& header_value, const KURL& url, Context* context, network::MetaCHType type, bool is_doc_preloader, bool is_sync_parser)
{
    ...
    case network::MetaCHType::HttpEquivAcceptCH: {
        ...
        std::optional<std::vector<network::mojom::WebClientHintsType>> parsed_ch = network::ParseClientHintsHeader(header_value.Latin1());
        ...
        for (network::mojom::WebClientHintsType newly_enabled : parsed_ch.value()) {
            ...
            if (context && !is_doc_preloader) {
                ukm::builders::ClientHints_AcceptCHMetaUsage(context->GetUkmSourceId())
    ...
    case network::MetaCHType::HttpEquivDelegateCH: {
        ...
        for (const auto& pair : parsed_ch.map) {
            ...
            if (context && !is_doc_preloader) {
                ukm::builders::ClientHints_DelegateCHMetaUsage(context->GetUkmSourceId())
...
bool ClientHintsPreferences::IsClientHintsAllowed(const KURL& url)
{
    ...
    // origin first.
    return (url.ProtocolIs("http") || url.ProtocolIs("https")) && network::IsOriginPotentiallyTrustworthy(url::Origin::Create(GURL(url)));
}
...
bool ClientHintsPreferences::ShouldSend(network::mojom::WebClientHintsType type) const
{
    return enabled_hints_.IsEnabled(type);
}

void ClientHintsPreferences::SetShouldSend(network::mojom::WebClientHintsType type)
{
