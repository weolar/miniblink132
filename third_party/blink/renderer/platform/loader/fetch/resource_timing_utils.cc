...
Vector<mojom::blink::ServerTimingInfoPtr> ParseServerTimingFromHeaderValueToMojo(const String& value)
{
    std::unique_ptr<ServerTimingHeaderVector> headers = ParseServerTimingHeader(value);
    Vector<mojom::blink::ServerTimingInfoPtr> result;
    result.reserve(headers->size());
    for (const auto& header : *headers) {
        result.emplace_back(mojom::blink::ServerTimingInfo::New(header->Name(), header->Duration(), header->Description()));
    }
    return result;
}
...
mojom::blink::ResourceTimingInfoPtr CreateResourceTimingInfo(base::TimeTicks start_time, const KURL& initial_url, const ResourceResponse* response)
{
    mojom::blink::ResourceTimingInfoPtr info = mojom::blink::ResourceTimingInfo::New();
    ...
    if (response->TimingAllowPassed()) {
        ...
        info->server_timing = ParseServerTimingFromHeaderValueToMojo(response->HttpHeaderField(http_names::kServerTiming));
        info->cache_state = response->CacheState();
        info->alpn_negotiated_protocol = response->AlpnNegotiatedProtocol().IsNull() ? g_empty_string : response->AlpnNegotiatedProtocol();
        info->connection_info = response->ConnectionInfoString().IsNull() ? g_empty_string : response->ConnectionInfoString();

        info->did_reuse_connection = response->ConnectionReused();
        // Use SecurityOrigin::Create to handle cases like blob:https://.
        info->is_secure_transport = base::Contains(url::GetSecureSchemes(), SecurityOrigin::Create(response->ResponseUrl())->Protocol().Ascii());
        info->timing = response->GetResourceLoadTiming() ? response->GetResourceLoadTiming()->ToMojo() : nullptr;
    } else {
    ...
    info->service_worker_router_info = response->GetServiceWorkerRouterInfo() ? response->GetServiceWorkerRouterInfo()->ToMojo() : nullptr;
    ...
    bool expose_body_sizes = RuntimeEnabledFeatures::ResourceTimingUseCORSForBodySizesEnabled() ? allow_response_details : info->allow_timing_details;
    ...
    if (expose_body_sizes && response) {
        ...
        info->service_worker_response_source = response->GetServiceWorkerResponseSource();
    }
