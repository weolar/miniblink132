...
protected:
    ...
    {
        MojoCreateDataPipeOptions data_pipe_options { sizeof(MojoCreateDataPipeOptions), MOJO_CREATE_DATA_PIPE_FLAG_NONE, 1, 0 };
        mojo::ScopedDataPipeConsumerHandle consumer_handle;
        mojo::ScopedDataPipeProducerHandle producer_handle;
        CHECK_EQ(MOJO_RESULT_OK, mojo::CreateDataPipe(&data_pipe_options, producer_handle, consumer_handle));
...
TEST_F(BufferingBytesConsumerTest, Read)
{
    ...
    auto* replaying_bytes_consumer = MakeGarbageCollected<ReplayingBytesConsumer>(task_runner);
    ...
    auto* bytes_consumer = BufferingBytesConsumer::Create(replaying_bytes_consumer);
...
TEST_F(BufferingBytesConsumerTest, ReadWithDelay)
{
    ...
    auto* bytes_consumer = BufferingBytesConsumer::CreateWithDelay(replaying_bytes_consumer, scheduler::GetSingleThreadTaskRunnerForTesting());

    task_runner->RunUntilIdle();

    // The underlying consumer should not have been read yet due to the delay.
    EXPECT_EQ(PublicState::kReadableOrWaiting, bytes_consumer->GetPublicState());
    EXPECT_EQ(PublicState::kReadableOrWaiting, replaying_bytes_consumer->GetPublicState());
...
TEST_F(BufferingBytesConsumerTest, DrainAsDataPipeFailsWithoutDelay)
{
    ...
    DataPipeBytesConsumer* data_pipe_consumer = MakeGarbageCollected<DataPipeBytesConsumer>(task_runner, MakeDataPipe(), &notifier);
...
TEST_F(BufferingBytesConsumerTest, DrainAsDataPipeSucceedsWithDelay)
{
    ...
    auto* bytes_consumer = BufferingBytesConsumer::CreateWithDelay(data_pipe_consumer, scheduler::GetSingleThreadTaskRunnerForTesting());
...
class BufferingBytesConsumerMaxBytesTest : public BufferingBytesConsumerTest, public ::testing::WithParamInterface<MaxBytesParams> {
protected:
    BufferingBytesConsumerMaxBytesTest()
        : task_runner_(base::MakeRefCounted<scheduler::FakeTaskRunner>())
        , replaying_bytes_consumer_(MakeGarbageCollected<ReplayingBytesConsumer>(task_runner_))
    {
    }

    size_t ChunkSize() const
    {
        return GetParam().chunk_size;
    }

    size_t TotalSize() const
    {
        return GetParam().total_size;
    }
...
TEST_P(BufferingBytesConsumerMaxBytesTest, ReadLargeResourceSuccessfully)
{
    ...
    auto* bytes_consumer = BufferingBytesConsumer::Create(replaying_bytes_consumer_);

    EXPECT_EQ(PublicState::kReadableOrWaiting, bytes_consumer->GetPublicState());
    EXPECT_EQ(PublicState::kReadableOrWaiting, replaying_bytes_consumer_->GetPublicState());
...
constexpr size_t kNextMultipleOfSixDigitPrimeAfterMaxBufferSize = ((kMaxBufferSize + kSixDigitPrime) / kSixDigitPrime) * kSixDigitPrime;

INSTANTIATE_TEST_SUITE_P(BufferingBytesConsumerMaxBytesTest, BufferingBytesConsumerMaxBytesTest,
    ::testing::Values(MaxBytesParams { 1024 * 1024, kMaxBufferSize + 1024 * 1024 },
        MaxBytesParams { kSixDigitPrime, kNextMultipleOfSixDigitPrimeAfterMaxBufferSize }, MaxBytesParams { 1024 * 1024, kMaxBufferSize },
