...
TEST_F(FetchApiRequestBodyMojomTraitsTest, RoundTripEmpty)
{
    ...
    EXPECT_TRUE(mojo::test::SerializeAndDeserialize<blink::mojom::blink::FetchAPIRequestBody>(src, dest));
...
TEST_F(FetchApiRequestBodyMojomTraitsTest, RoundTripBlobWithOpionalHandle)
{
    ...
    auto blob_data_handle = BlobDataHandle::Create("test_uuid", "type-test", 100, mojo::PendingRemote<mojom::blink::Blob>(std::move(pipe.handle0), 0));
...
TEST_F(FetchApiRequestBodyMojomTraitsTest, RoundTripDataPipeGetter)
{
    ...
    src.FormBody()->AppendDataPipe(base::MakeRefCounted<blink::WrappedDataPipeGetter>(std::move(data_pipe_getter)));
