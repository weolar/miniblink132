...
bool FetchUtils::IsForbiddenResponseHeaderName(const String& name)
{
    ...
    return EqualIgnoringASCIICase(name, "set-cookie") || EqualIgnoringASCIICase(name, "set-cookie2");
}
...
net::NetworkTrafficAnnotationTag FetchUtils::GetTrafficAnnotationTag(const network::ResourceRequest& request)
{
    ...
    case network::mojom::RequestDestination::kObject:
        return net::DefineNetworkTrafficAnnotation("blink_extension_resource_loader", R"(
        semantics {
          sender: "Blink Resource Loader"
          description:
...
void FetchUtils::LogFetchKeepAliveRequestMetric(
    const mojom::blink::RequestContextType& request_context_type, const FetchKeepAliveRequestState& request_state, bool is_context_detached)
{
    ...
    case FetchKeepAliveRequestState::kStarted:
        ...
        base::UmaHistogramBoolean("FetchKeepAlive.Requests2.Started.IsContextDetached.Renderer", is_context_detached);
        break;
    case FetchKeepAliveRequestState::kSucceeded:
        request_state_name = "Succeeded";
        base::UmaHistogramBoolean("FetchKeepAlive.Requests2.Succeeded.IsContextDetached.Renderer", is_context_detached);
        break;
    ...
    base::UmaHistogramEnumeration(base::StrCat({ "FetchKeepAlive.Requests2.", request_state_name, ".Renderer" }), sample_type);
}

void FetchUtils::LogFetchKeepAliveRequestSentToServiceMetric(const network::ResourceRequest& resource_request)
{
    auto resource_type = static_cast<mojom::blink::ResourceType>(resource_request.resource_type);
    FetchKeepAliveRequestMetricType sample_type;
    ...
    base::UmaHistogramEnumeration("FetchKeepAlive.Requests2.SentToService.Renderer", sample_type);
}
