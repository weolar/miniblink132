...
TEST_F(DataPipeBytesConsumerTest, TwoPhaseRead)
{
    ...
    MojoResult rv = producer_handle->WriteData(base::as_byte_span(kData), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
    ...
    DataPipeBytesConsumer* consumer = MakeGarbageCollected<DataPipeBytesConsumer>(task_runner_, std::move(consumer_handle), &notifier);
    notifier->SignalComplete();
    auto result = MakeGarbageCollected<BytesConsumerTestReader>(consumer)->Run(task_runner_.get());
...
TEST_F(DataPipeBytesConsumerTest, SignalSizeBeforeRead)
{
    ...
    const MojoCreateDataPipeOptions options { sizeof(MojoCreateDataPipeOptions), MOJO_CREATE_DATA_PIPE_FLAG_NONE, 1, 0 };
    ASSERT_EQ(MOJO_RESULT_OK, mojo::CreateDataPipe(&options, writable, readable));
    DataPipeBytesConsumer::CompletionNotifier* notifier = nullptr;
    DataPipeBytesConsumer* consumer = MakeGarbageCollected<DataPipeBytesConsumer>(task_runner_, std::move(readable), &notifier);

    const std::string_view kData = "hello";
    size_t actually_written_bytes = 0;
    MojoResult write_result = writable->WriteData(base::as_byte_span(kData), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
