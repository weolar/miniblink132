...
ScriptCachedMetadataHandler::ScriptCachedMetadataHandler(const WTF::TextEncoding& encoding, std::unique_ptr<CachedMetadataSender> sender)
...
void ScriptCachedMetadataHandler::SetCachedMetadata(CodeCacheHost* code_cache_host, uint32_t data_type_id, const uint8_t* data, size_t size)
{
...
void ScriptCachedMetadataHandler::ClearCachedMetadata(CodeCacheHost* code_cache_host, ClearCacheType cache_type)
{
...
scoped_refptr<CachedMetadata> ScriptCachedMetadataHandler::GetCachedMetadata(uint32_t data_type_id, GetCachedMetadataBehavior behavior) const
{
...
void ScriptCachedMetadataHandler::SetSerializedCachedMetadata(mojo_base::BigBuffer data)
{
...
void ScriptCachedMetadataHandler::OnMemoryDump(WebProcessMemoryDump* pmd, const String& dump_prefix) const
{
    if (!cached_metadata_)
        return;
    ...
    pmd->AddSuballocation(dump->Guid(), String(WTF::Partitions::kAllocatedObjectPoolName));
}
...
void ScriptCachedMetadataHandler::CommitToPersistentStorage(CodeCacheHost* code_cache_host)
{
    if (cached_metadata_) {
...
void ScriptCachedMetadataHandlerWithHashing::Check(CodeCacheHost* code_cache_host, const ParkableString& source_text)
{
...
void ScriptCachedMetadataHandlerWithHashing::SetSerializedCachedMetadata(mojo_base::BigBuffer data)
{
    // We only expect to receive cached metadata from the platform once. If this
    // triggers, it indicates an efficiency problem which is most likely
    // unexpected in code designed to improve performance.
    ...
    auto [header_bytes, payload_bytes] = base::span(data.data(), data.size()).split_at(sizeof(CachedMetadataHeaderWithHash));

    // Ensure the marker matches, otherwise discard the data.
    const CachedMetadataHeaderWithHash* header = reinterpret_cast<const CachedMetadataHeaderWithHash*>(header_bytes.data());
    ...
    cached_metadata_ = CachedMetadata::CreateFromSerializedData(payload_bytes.data(), payload_bytes.size());
}

scoped_refptr<CachedMetadata> ScriptCachedMetadataHandlerWithHashing::GetCachedMetadata(uint32_t data_type_id, GetCachedMetadataBehavior behavior) const
{
    ...
    scoped_refptr<CachedMetadata> result = ScriptCachedMetadataHandler::GetCachedMetadata(data_type_id, behavior);

    return result;
}

void ScriptCachedMetadataHandlerWithHashing::CommitToPersistentStorage(CodeCacheHost* code_cache_host)
{
    Vector<uint8_t> msg = GetSerializedCachedMetadata();
    base::span<const uint8_t> msg_span(msg.data(), msg.size());
    Sender()->Send(code_cache_host, msg_span);
}

Vector<uint8_t> ScriptCachedMetadataHandlerWithHashing::GetSerializedCachedMetadata() const
{
    Vector<uint8_t> serialized_data;
    if (cached_metadata_ && hash_state_ == kChecked) {
        uint32_t marker = CachedMetadataHandler::kSingleEntryWithHashAndPadding;
        CHECK_EQ(serialized_data.size(), offsetof(CachedMetadataHeaderWithHash, marker));
        serialized_data.Append(reinterpret_cast<uint8_t*>(&marker), sizeof(marker));
        uint32_t padding = 0;
        CHECK_EQ(serialized_data.size(), offsetof(CachedMetadataHeaderWithHash, padding));
        serialized_data.Append(reinterpret_cast<uint8_t*>(&padding), sizeof(padding));
        CHECK_EQ(serialized_data.size(), offsetof(CachedMetadataHeaderWithHash, hash));
