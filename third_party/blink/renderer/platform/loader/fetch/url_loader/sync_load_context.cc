...
public:
    SignalHelper(SyncLoadContext* context, base::WaitableEvent* redirect_or_response_event, base::WaitableEvent* abort_event, base::TimeDelta timeout)
        : context_(context)
    ...
    bool RestartAfterRedirect()
    {
        ...
        if (timeout_timer_) {
            timeout_remainder = timeout_timer_->desired_run_time() - base::TimeTicks::Now();
...
private:
    void Start(base::TimeDelta timeout)
    {
        ...
        if (abort_event_) {
            abort_watcher_.StartWatching(abort_event_, base::BindOnce(&SyncLoadContext::OnAbort, base::Unretained(context_)), context_->task_runner_);
        }
        if (timeout_timer_) {
            DCHECK_NE(base::TimeDelta::Max(), timeout);
            timeout_timer_->Start(FROM_HERE, timeout, context_.get(), &SyncLoadContext::OnTimeout);
        }
    }
...
// static
void SyncLoadContext::StartAsyncWithWaitableEvent(std::unique_ptr<network::ResourceRequest> request,
    scoped_refptr<base::SingleThreadTaskRunner> loading_task_runner, const net::NetworkTrafficAnnotationTag& traffic_annotation, uint32_t loader_options,
    std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_url_loader_factory, WebVector<std::unique_ptr<URLLoaderThrottle>> throttles,
    SyncLoadResponse* response, SyncLoadContext** context_for_redirect, base::WaitableEvent* redirect_or_response_event, base::WaitableEvent* abort_event,
    base::TimeDelta timeout, mojo::PendingRemote<mojom::blink::BlobRegistry> download_to_blob_registry, const Vector<String>& cors_exempt_header_list,
    std::unique_ptr<ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper)
{
    scoped_refptr<SyncLoadContext> context(base::AdoptRef(new SyncLoadContext(request.get(), std::move(pending_url_loader_factory), response,
        context_for_redirect, redirect_or_response_event, abort_event, timeout, std::move(download_to_blob_registry), loading_task_runner)));
    context->resource_request_sender_->SendAsync(std::move(request), std::move(loading_task_runner), traffic_annotation, loader_options,
        cors_exempt_header_list, context, context->url_loader_factory_, std::move(throttles), std::move(resource_load_info_notifier_wrapper),
...
SyncLoadContext::SyncLoadContext(network::ResourceRequest* request, std::unique_ptr<network::PendingSharedURLLoaderFactory> url_loader_factory,
    SyncLoadResponse* response, SyncLoadContext** context_for_redirect, base::WaitableEvent* redirect_or_response_event, base::WaitableEvent* abort_event,
    base::TimeDelta timeout, mojo::PendingRemote<mojom::blink::BlobRegistry> download_to_blob_registry, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : response_(response)
    ...
    , signals_(std::make_unique<SignalHelper>(this, redirect_or_response_event, abort_event, timeout))
{
    if (download_to_blob_registry_)
        mode_ = Mode::kBlob;

    url_loader_factory_ = network::SharedURLLoaderFactory::Create(std::move(url_loader_factory));
    ...
    has_authorization_header_ = request->headers.HasHeader(net::HttpRequestHeaders::kAuthorization);
}
    ...
    const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr head, FollowRedirectCallback follow_redirect_callback)
{
    ...
    if (has_authorization_header_ && !url::IsSameOriginWith(response_->url, redirect_info.new_url)) {
...
void SyncLoadContext::FollowRedirect(std::vector<std::string> removed_headers, net::HttpRequestHeaders modified_headers)
{
    ...
    *context_for_redirect_ = nullptr;
    std::move(follow_redirect_callback_).Run(std::move(removed_headers), std::move(modified_headers));
}
    ...
    network::mojom::URLResponseHeadPtr head, mojo::ScopedDataPipeConsumerHandle body, std::optional<mojo_base::BigBuffer> cached_metadata)
{
    ...
    if (mode_ == Mode::kBlob) {
        ...
        download_to_blob_registry_->RegisterFromStream(String(response_->head->mime_type), "", std::max<int64_t>(0, response_->head->content_length),
            std::move(body), mojo::NullAssociatedRemote(), base::BindOnce(&SyncLoadContext::OnFinishCreatingBlob, base::Unretained(this)));
        return;
    }
    ...
    body_watcher_.Watch(body_handle_.get(), MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
        base::BindRepeating(&SyncLoadContext::OnBodyReadable, base::Unretained(this)));
...
void SyncLoadContext::OnCompletedRequest(const network::URLLoaderCompletionStatus& status)
{
    if (Completed()) {
    ...
    response_->head->encoded_body_length = network::mojom::EncodedBodyLength::New(status.encoded_body_length);
...
void SyncLoadContext::OnFinishCreatingBlob(const scoped_refptr<BlobDataHandle>& blob)
{
    DCHECK(!Completed());
    blob_finished_ = true;
...
void SyncLoadContext::OnBodyReadable(MojoResult, const mojo::HandleSignalsState&)
{
    DCHECK_EQ(Mode::kDataPipe, mode_);
    DCHECK(body_handle_.is_valid());
    base::span<const uint8_t> buffer;
    MojoResult result = body_handle_->BeginReadData(MOJO_READ_DATA_FLAG_NONE, buffer);
