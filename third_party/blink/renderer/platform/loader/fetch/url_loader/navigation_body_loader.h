...
// was received.
class PLATFORM_EXPORT NavigationBodyLoader : public WebNavigationBodyLoader, public network::mojom::URLLoaderClient {
public:
    NavigationBodyLoader(const KURL& original_url, network::mojom::URLResponseHeadPtr response_head, mojo::ScopedDataPipeConsumerHandle response_body,
    ...
    void StartLoadingBodyInBackground(std::unique_ptr<BodyTextDecoder> decoder, bool should_keep_encoded_data);
...
private:
    ...
    BodyLoaderType GetType() const override
    {
        return BodyLoaderType::kNetwork;
    }
    ...
    mojo::Receiver<network::mojom::URLLoaderClient> url_loader_client_receiver_ { this };
    ...
    using OffThreadBodyReaderPtr = std::unique_ptr<OffThreadBodyReader, OffThreadBodyReaderDeleter>;
...
template <> struct DowncastTraits<NavigationBodyLoader> {
    static bool AllowFrom(const WebNavigationBodyLoader& body_loader)
    {
        return body_loader.GetType() == NavigationBodyLoader::BodyLoaderType::kNetwork;
    }
};
