...
class BLINK_PLATFORM_EXPORT CodeCacheFetcher : public WTF::RefCounted<CodeCacheFetcher> {
public:
    ...
    CodeCacheFetcher(CodeCacheHost& code_cache_host, mojom::blink::CodeCacheType code_cache_type, const KURL& url, base::OnceClosure done_closure);
    ...
    void SetCurrentUrl(const KURL& new_url)
    {
        current_url_ = new_url;
    }
    void DidReceiveCachedMetadataFromUrlLoader();
    std::optional<mojo_base::BigBuffer> TakeCodeCacheForResponse(const network::mojom::URLResponseHead& response_head);
...
private:
    ...
    void DidReceiveCachedCode(base::Time response_time, mojo_base::BigBuffer data);
