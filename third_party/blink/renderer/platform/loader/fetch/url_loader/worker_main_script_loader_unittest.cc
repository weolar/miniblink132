...
public:
    ...
    {
        scoped_feature_list_.InitWithFeatureState(blink::features::kPlzDedicatedWorker, true);
    }
...
protected:
    class TestClient final : public GarbageCollected<TestClient>, public WorkerMainScriptLoaderClient {
    ...
    public:
        explicit FakeURLLoader(mojo::PendingReceiver<network::mojom::URLLoader> url_loader_receiver)
        ...
        void FollowRedirect(
            const std::vector<std::string>&, const net::HttpRequestHeaders&, const net::HttpRequestHeaders&, const std::optional<GURL>&) override
        {
        }
    ...
    public:
        ...
        MOCK_METHOD6(WillSendRequest,
            void(const ResourceRequest&, const ResourceResponse& redirect_response, ResourceType, const ResourceLoaderOptions&, RenderBlockingBehavior,
                const Resource*));
        MOCK_METHOD3(DidChangePriority, void(uint64_t identifier, ResourceLoadPriority, int intra_priority_value));
        MOCK_METHOD5(DidReceiveResponse,
            void(uint64_t identifier, const ResourceRequest& request, const ResourceResponse& response, const Resource* resource, ResponseSource));
        MOCK_METHOD2(DidReceiveData, void(uint64_t identifier, base::SpanOrSize<const char> chunk));
        MOCK_METHOD2(DidReceiveTransferSizeUpdate, void(uint64_t identifier, int transfer_size_diff));
        MOCK_METHOD2(DidDownloadToBlob, void(uint64_t identifier, BlobDataHandle*));
        MOCK_METHOD4(DidFinishLoading, void(uint64_t identifier, base::TimeTicks finish_time, int64_t encoded_data_length, int64_t decoded_body_length));
        MOCK_METHOD5(DidFailLoading, void(const KURL&, uint64_t identifier, const ResourceError&, int64_t encoded_data_length, IsInternalRequest));
        MOCK_METHOD2(DidChangeRenderBlockingBehavior, void(Resource* resource, const FetchParameters& params));
        MOCK_METHOD0(InterestedInAllRequests, bool());
        MOCK_METHOD1(EvictFromBackForwardCache, void(mojom::blink::RendererEvictionReason));
    };
    ...
    std::unique_ptr<WorkerMainScriptLoadParameters> CreateMainScriptLoaderParams(const char* header, mojo::ScopedDataPipeProducerHandle* body_producer)
    {
        auto head = network::mojom::URLResponseHead::New();
        head->headers = base::MakeRefCounted<net::HttpResponseHeaders>(net::HttpUtil::AssembleRawHeaders(header));
        head->headers->GetMimeType(&head->mime_type);
        network::mojom::URLLoaderClientEndpointsPtr endpoints
            = network::mojom::URLLoaderClientEndpoints::New(std::move(pending_remote_loader_), loader_client_.BindNewPipeAndPassReceiver());

        std::unique_ptr<WorkerMainScriptLoadParameters> worker_main_script_load_params = std::make_unique<WorkerMainScriptLoadParameters>();
        worker_main_script_load_params->response_head = std::move(head);
        worker_main_script_load_params->url_loader_client_endpoints = std::move(endpoints);
        mojo::ScopedDataPipeConsumerHandle body_consumer;
        MojoCreateDataPipeOptions options = CreateDataPipeOptions();
        EXPECT_EQ(MOJO_RESULT_OK, mojo::CreateDataPipe(&options, *body_producer, body_consumer));
        worker_main_script_load_params->response_body = std::move(body_consumer);

        return worker_main_script_load_params;
    }

    WorkerMainScriptLoader* CreateWorkerMainScriptLoaderAndStartLoading(std::unique_ptr<WorkerMainScriptLoadParameters> worker_main_script_load_params,
        ResourceLoadObserver* observer, mojom::ResourceLoadInfoNotifier* resource_load_info_notifier)
    {
        ResourceRequest request(kTopLevelScriptURL);
        request.SetRequestContext(mojom::blink::RequestContextType::SHARED_WORKER);
        request.SetRequestDestination(network::mojom::RequestDestination::kSharedWorker);
        FetchParameters fetch_params(std::move(request), ResourceLoaderOptions(nullptr /* world */));
        WorkerMainScriptLoader* worker_main_script_loader = MakeGarbageCollected<WorkerMainScriptLoader>();
        ...
        worker_main_script_loader->Start(fetch_params, std::move(worker_main_script_load_params), fetch_context, observer, client_);
...
TEST_F(WorkerMainScriptLoaderTest, ResponseWithSucessThenOnComplete)
{
    ...
    std::unique_ptr<WorkerMainScriptLoadParameters> worker_main_script_load_params = CreateMainScriptLoaderParams(kHeader, &body_producer);
    MockResourceLoadObserver* mock_observer = MakeGarbageCollected<MockResourceLoadObserver>();
    ...
    Persistent<WorkerMainScriptLoader> worker_main_script_loader
        = CreateWorkerMainScriptLoaderAndStartLoading(std::move(worker_main_script_load_params), mock_observer, &fake_resource_load_info_notifier);
    ...
    EXPECT_EQ(KURL(kTopLevelScriptURL), worker_main_script_loader->GetRequestURL());
...
{
    mojo::ScopedDataPipeProducerHandle body_producer;
    std::unique_ptr<WorkerMainScriptLoadParameters> worker_main_script_load_params = CreateMainScriptLoaderParams(kFailHeader, &body_producer);
    MockResourceLoadObserver* mock_observer = MakeGarbageCollected<MockResourceLoadObserver>();
    FakeResourceLoadInfoNotifier fake_resource_load_info_notifier;
    EXPECT_CALL(*mock_observer, DidReceiveResponse(_, _, _, _, _));
