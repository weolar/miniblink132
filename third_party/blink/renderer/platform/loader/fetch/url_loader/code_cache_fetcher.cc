...
bool ShouldUseIsolatedCodeCache(
    const network::mojom::URLResponseHead& response_head, const KURL& initial_url, const KURL& current_url, base::Time code_cache_response_time)
{
    ...
    if (SchemeRegistry::SchemeSupportsCodeCacheWithHashing(initial_url.Protocol())) {
        // This resource should use a source text hash rather than a response time
        // comparison.
        if (!SchemeRegistry::SchemeSupportsCodeCacheWithHashing(current_url.Protocol())) {
            // This kind of Resource doesn't support requiring a hash, so we can't
            // send cached code to it.
            return false;
        }
        if (!Platform::Current()->ShouldUseCodeCacheWithHashing(WebURL(current_url))) {
        ...
        // cache.
        base::Time response_time = response_head.original_response_time.is_null() ? response_head.response_time : response_head.original_response_time;
        if (code_cache_response_time.is_null() || response_time.is_null() || code_cache_response_time != response_time) {
            return false;
        }
    }
    return true;
...
bool ShouldFetchCodeCache(const network::ResourceRequest& request)
{
    ...
    bool should_use_source_hash = SchemeRegistry::SchemeSupportsCodeCacheWithHashing(String(request.url.scheme()))
        && Platform::Current()->ShouldUseCodeCacheWithHashing(WebURL(KURL(request.url)));
    ...
    if (request.destination == network::mojom::RequestDestination::kScript
        || request.destination == network::mojom::RequestDestination::kSharedStorageWorklet) {
        return true;
    }
...
mojom::blink::CodeCacheType GetCodeCacheType(network::mojom::RequestDestination destination)
{
...
scoped_refptr<CodeCacheFetcher> CodeCacheFetcher::TryCreateAndStart(
    const network::ResourceRequest& request, CodeCacheHost& code_cache_host, base::OnceClosure done_closure)
{
    if (!ShouldFetchCodeCache(request)) {
        return nullptr;
    }
    auto fetcher = base::MakeRefCounted<CodeCacheFetcher>(code_cache_host, GetCodeCacheType(request.destination), KURL(request.url), std::move(done_closure));
    fetcher->Start();
    return fetcher;
}

CodeCacheFetcher::CodeCacheFetcher(CodeCacheHost& code_cache_host, mojom::blink::CodeCacheType code_cache_type, const KURL& url, base::OnceClosure done_closure)
...
void CodeCacheFetcher::Start()
{
    ...
    (*code_cache_host_)->FetchCachedCode(code_cache_type_, initial_url_, WTF::BindOnce(&CodeCacheFetcher::DidReceiveCachedCode, base::WrapRefCounted(this)));
}
...
std::optional<mojo_base::BigBuffer> CodeCacheFetcher::TakeCodeCacheForResponse(const network::mojom::URLResponseHead& response_head)
{
    CHECK(!is_waiting_);
    if (!ShouldUseIsolatedCodeCache(response_head, initial_url_, current_url_, code_cache_response_time_)) {
        ClearCodeCacheEntryIfPresent();
        return std::nullopt;
    }
...
void CodeCacheFetcher::DidReceiveCachedCode(base::Time response_time, mojo_base::BigBuffer data)
{
    is_waiting_ = false;
