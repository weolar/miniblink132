...
class NavigationBodyLoaderTest : public ::testing::Test, public WebNavigationBodyLoader::Client {
protected:
    NavigationBodyLoaderTest()
    {
    }

    ~NavigationBodyLoaderTest() override
    {
    ...
    void CreateBodyLoader()
    {
        ...
        ASSERT_EQ(mojo::CreateDataPipe(&options, producer_handle, consumer_handle), MOJO_RESULT_OK);
        ...
        common_params->request_destination = network::mojom::RequestDestination::kDocument;
        auto commit_params = CreateCommitNavigationParams();
        WebNavigationBodyLoader::FillNavigationParamsResponseAndBodyLoader(std::move(common_params), std::move(commit_params), /*request_id=*/1,
            network::mojom::URLResponseHead::New(), std::move(consumer_handle), std::move(endpoints), scheduler::GetSingleThreadTaskRunnerForTesting(),
    ...
    {
        To<NavigationBodyLoader>(loader_.get())->StartLoadingBodyInBackground(std::make_unique<UppercaseDecoder>(), false);
        base::RunLoop().RunUntilIdle();
    }
    ...
    void Write(const std::string& buffer)
    {
        ...
        MojoResult result = writer_->WriteData(base::as_byte_span(buffer), kNone, actually_written_bytes);
...
TEST_F(NavigationBodyLoaderTest, ProcessBackgroundData)
{
    ...
    process_background_data_callback_ = CrossThreadBindRepeating(
        [](String* background_data, const WebString& data) { *background_data = *background_data + String(data); }, CrossThreadUnretained(&background_data));
...
TEST_F(NavigationBodyLoaderTest, FillResponseWithSecurityDetails)
{
    ...
    response->ssl_info->cert = net::X509Certificate::CreateFromDERCertChain({ cert0_der, cert1_der });
    net::SSLConnectionStatusSetVersion(net::SSL_CONNECTION_VERSION_TLS1_2, &response->ssl_info->connection_status);
    ...
    mojo::ScopedDataPipeProducerHandle producer_handle;
    mojo::ScopedDataPipeConsumerHandle consumer_handle;
    MojoResult rv = mojo::CreateDataPipe(nullptr, producer_handle, consumer_handle);
    ASSERT_EQ(MOJO_RESULT_OK, rv);
    WebNavigationBodyLoader::FillNavigationParamsResponseAndBodyLoader(std::move(common_params), std::move(commit_params), /*request_id=*/1,
        std::move(response), std::move(consumer_handle), std::move(endpoints), scheduler::GetSingleThreadTaskRunnerForTesting(),
        std::make_unique<ResourceLoadInfoNotifierWrapper>(
            /*resource_load_info_notifier=*/nullptr),
        /*is_main_frame=*/true, &navigation_params, /*is_ad_frame=*/false);
    EXPECT_TRUE(navigation_params.response.ToResourceResponse().GetSSLInfo().has_value());
}
...
TEST_F(NavigationBodyLoaderTest, FillResponseReferrerRedirects)
{
    ...
    network::mojom::URLResponseHeadPtr first_redirect_response = network::mojom::URLResponseHead::New();
    network::mojom::URLResponseHeadPtr second_redirect_response = network::mojom::URLResponseHead::New();
    commit_params->redirect_infos.push_back(first_redirect_info);
    commit_params->redirect_infos.push_back(second_redirect_info);
    commit_params->redirect_response.push_back(std::move(first_redirect_response));
    commit_params->redirect_response.push_back(std::move(second_redirect_response));
    ...
    ASSERT_EQ(navigation_params.redirects[0].new_referrer, WebString(Referrer::NoReferrer()));
    ASSERT_EQ(navigation_params.redirects[1].new_referrer, WebString::FromUTF8(second_redirect_url.spec()));
}
...
public:
    ...
    void DecodedBodyDataReceived(const WebString& data, const WebEncodingData& encoding_data, base::SpanOrSize<const char> encoded_data) override
    {
        scheduler::GetSingleThreadTaskRunnerForTesting()->PostTask(FROM_HERE, base::BindOnce(&ChunkingLoaderClient::CreateNewChunk, base::Unretained(this)));
        chunks_.back() += data.Ascii();
    }
    void BodyLoadingFinished(base::TimeTicks completion_time, int64_t total_encoded_data_length, int64_t total_encoded_body_length,
        int64_t total_decoded_body_length, const std::optional<WebURLError>& error) override
    {
        scheduler::GetSingleThreadTaskRunnerForTesting()->PostTask(FROM_HERE, run_loop_.QuitClosure());
    }
...
TEST_F(NavigationBodyLoaderTest, MaxDataSize1)
{
    ...
    feature_list.InitAndEnableFeatureWithParameters(features::kThreadedBodyLoader, { { "max-data-to-process", "1" } });
    CreateBodyLoader();
    StartLoadingInBackground();
    ...
    EXPECT_THAT(client.TakeChunks(), ElementsAre("AB", "C", "D", "E", "F", "G", "H", ""));
}

TEST_F(NavigationBodyLoaderTest, MaxDataSize2)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeatureWithParameters(features::kThreadedBodyLoader, { { "max-data-to-process", "2" } });
    CreateBodyLoader();
    StartLoadingInBackground();
    for (const char* s : { "a", "b", "c", "d", "e", "f", "g", "h" })
        WriteAndFlush(std::string(s));
...
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeatureWithParameters(features::kThreadedBodyLoader, { { "max-data-to-process", "0" } });
    CreateBodyLoader();
    StartLoadingInBackground();
    for (const char* s : { "a", "b", "c", "d", "e", "f", "g", "h" })
        WriteAndFlush(std::string(s));
