...
template <> struct CrossThreadCopier<SegmentedBuffer> {
    STATIC_ONLY(CrossThreadCopier);
...
using MaybeStartFunction
    = CrossThreadOnceFunction<bool(network::mojom::URLResponseHeadPtr&, mojo::ScopedDataPipeConsumerHandle&, std::optional<mojo_base::BigBuffer>&,
        scoped_refptr<base::SingleThreadTaskRunner>, scoped_refptr<base::SequencedTaskRunner>, BackgroundResponseProcessor::Client*)>;

class BackgroundResponseProcessorTestUtil : public WTF::ThreadSafeRefCounted<BackgroundResponseProcessorTestUtil> {
public:
    BackgroundResponseProcessorTestUtil() = default;

    BackgroundResponseProcessorTestUtil(const BackgroundResponseProcessorTestUtil&) = delete;
    BackgroundResponseProcessorTestUtil& operator=(const BackgroundResponseProcessorTestUtil&) = delete;
    ...
    bool MaybeStartProcessingResponse(network::mojom::URLResponseHeadPtr& head, mojo::ScopedDataPipeConsumerHandle& body,
        std::optional<mojo_base::BigBuffer>& cached_metadata_buffer, scoped_refptr<base::SequencedTaskRunner> background_task_runner,
    ...
    void WaitUntilMaybeStartProcessingResponse()
    {
        run_loop_.Run();
    }
    ...
    bool processor_deleted() const
    {
        return processor_deleted_;
    }

private:
    class DummyProcessor final : public BackgroundResponseProcessor {
    public:
        explicit DummyProcessor(scoped_refptr<BackgroundResponseProcessorTestUtil> test_util)
        ...
        {
            return test_util_->MaybeStartProcessingResponse(head, body, cached_metadata_buffer, background_task_runner, client);
        }
    ...
    class DummyProcessorFactory final : public BackgroundResponseProcessorFactory {
    public:
        explicit DummyProcessorFactory(scoped_refptr<BackgroundResponseProcessorTestUtil> test_util)
            : test_util_(std::move(test_util))
        {
        }
...
network::mojom::URLResponseHeadPtr CreateTestResponse()
{
    ...
    response->headers = base::MakeRefCounted<net::HttpResponseHeaders>("HTTP/1.1 200 OK");
    response->mime_type = "text/html";
    return response;
}

class FakeBackForwardCacheLoaderHelper final : public BackForwardCacheLoaderHelper {
public:
    FakeBackForwardCacheLoaderHelper() = default;
    ~FakeBackForwardCacheLoaderHelper() = default;

    void EvictFromBackForwardCache(mojom::blink::RendererEvictionReason reason) override
    {
        evicted_reason_ = reason;
    }
    void DidBufferLoadWhileInBackForwardCache(bool update_process_wide_count, size_t num_bytes) override
    {
    ...
    const std::optional<mojom::blink::RendererEvictionReason>& evicted_reason() const
    {
        return evicted_reason_;
    }
    size_t total_bytes_buffered() const
    {
...
public:
    explicit FakeURLLoaderClient(scoped_refptr<base::SingleThreadTaskRunner> unfreezable_task_runner)
    ...
    using WillFollowRedirectCallback = base::OnceCallback<bool(const WebURL& new_url)>;
    void AddWillFollowRedirectCallback(WillFollowRedirectCallback will_follow_callback)
    {
        will_follow_callbacks_.push_back(std::move(will_follow_callback));
    }

    // URLLoaderClient implementation:
    bool WillFollowRedirect(const WebURL& new_url, const net::SiteForCookies& new_site_for_cookies, const WebString& new_referrer,
        ...
        bool& report_raw_headers, std::vector<std::string>*, net::HttpRequestHeaders&, bool insecure_scheme_was_upgraded) override
    {
        DCHECK(unfreezable_task_runner_->BelongsToCurrentThread());
        DCHECK(!will_follow_callbacks_.empty());
        WillFollowRedirectCallback will_follow_callback = std::move(will_follow_callbacks_.front());
    ...
    void DidReceiveResponse(const WebURLResponse& response, absl::variant<mojo::ScopedDataPipeConsumerHandle, SegmentedBuffer> body,
        std::optional<mojo_base::BigBuffer> cached_metadata) override
    {
        ...
        if (absl::holds_alternative<mojo::ScopedDataPipeConsumerHandle>(body)) {
            response_body_handle_ = std::move(absl::get<mojo::ScopedDataPipeConsumerHandle>(body));
        } else {
    ...
    void DidFinishLoading(base::TimeTicks finishTime, int64_t totalEncodedDataLength, uint64_t totalEncodedBodyLength, int64_t totalDecodedBodyLength) override
    {
        DCHECK(unfreezable_task_runner_->BelongsToCurrentThread());
        did_finish_ = true;
    }
    void DidFail(const WebURLError& error, base::TimeTicks finishTime, int64_t totalEncodedDataLength, uint64_t totalEncodedBodyLength,
    ...
    const std::optional<WebURLResponse>& response() const
    {
        return response_;
    }
    ...
    const std::optional<WebURLError>& error() const
    {
        return error_;
    }
...
public:
    explicit FakeURLLoader(mojo::PendingReceiver<network::mojom::URLLoader> pending_receiver)
    ...
    {
        set_priority_log_.push_back(PriorityInfo { .priority = priority, .intra_priority_value = intra_priority_value });
    }
    ...
    bool follow_redirect_called() const
    {
        return follow_redirect_called_;
    }
...
public:
    explicit BackgroundResourceFecherTest()
        : unfreezable_task_runner_(base::MakeRefCounted<scheduler::FakeTaskRunner>())
    {
    }

    BackgroundResourceFecherTest(const BackgroundResourceFecherTest&) = delete;
    BackgroundResourceFecherTest& operator=(const BackgroundResourceFecherTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        feature_list_.InitWithFeaturesAndParameters(
            { { blink::features::kLoadingTasksUnfreezable, { { "max_buffered_bytes_per_process", base::NumberToString(kMaxBufferedBytesPerProcess) } } } }, {});
        bfcache_loader_helper_ = MakeGarbageCollected<FakeBackForwardCacheLoaderHelper>();
    }
...
protected:
    std::unique_ptr<BackgroundURLLoader> CreateBackgroundURLLoaderAndStart(std::unique_ptr<network::ResourceRequest> request,
        URLLoaderClient* url_loader_client, std::unique_ptr<BackgroundResponseProcessorFactory> background_processor_factory = nullptr)
    {
        base::RunLoop run_loop;
        scoped_refptr<WebBackgroundResourceFetchAssets> background_resource_fetch_assets
            = base::MakeRefCounted<FakeBackgroundResourceFetchAssets>(background_task_runner_,
                base::BindLambdaForTesting(
                    [&](mojo::PendingReceiver<network::mojom::URLLoader> loader, mojo::PendingRemote<network::mojom::URLLoaderClient> client) {
                        DCHECK(background_task_runner_->RunsTasksInCurrentSequence());
                        loader_pending_receiver_ = std::move(loader);
                        loader_client_pending_remote_ = std::move(client);
                        run_loop.Quit();
                    }));
        std::unique_ptr<BackgroundURLLoader> background_url_loader = std::make_unique<BackgroundURLLoader>(std::move(background_resource_fetch_assets),
            /*cors_exempt_header_list=*/Vector<String>(), unfreezable_task_runner_, bfcache_loader_helper_,
        ...
        if (background_processor_factory) {
            background_url_loader->SetBackgroundResponseProcessorFactory(std::move(background_processor_factory));
        }
        background_url_loader->LoadAsynchronously(std::move(request), SecurityOrigin::Create(KURL(kTestURL)),
    ...
    mojo::PendingRemote<network::mojom::URLLoaderClient> loader_client_pending_remote_;
...
TEST_F(BackgroundResourceFecherTest, SimpleRequest)
{
    ...
    auto background_url_loader = CreateBackgroundURLLoaderAndStart(CreateTestRequest(), &client);

    mojo::Remote<network::mojom::URLLoaderClient> loader_client_remote(std::move(loader_client_pending_remote_));
    loader_client_remote->OnReceiveResponse(CreateTestResponse(), CreateTestBody(), CreateTestCachedMetaData());
...
TEST_F(BackgroundResourceFecherTest, FailedRequest)
{
    ...
    loader_client_remote->OnComplete(network::URLLoaderCompletionStatus(net::ERR_FAILED));
...
TEST_F(BackgroundResourceFecherTest, Redirect)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
    KURL redirected_url;
    client.AddWillFollowRedirectCallback(base::BindLambdaForTesting([&](const WebURL& new_url) {
    ...
    loader_client_remote->OnReceiveRedirect(redirect_info, network::mojom::URLResponseHead::New());
    ...
    loader_client_remote->OnReceiveResponse(CreateTestResponse(), CreateTestBody(),
...
TEST_F(BackgroundResourceFecherTest, CancelSoonAfterStart)
{
    ...
    background_task_runner_->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
        base::ScopedAllowBaseSyncPrimitivesForTesting allow_wait;
        waitable_event.Wait();
    }));

    scoped_refptr<WebBackgroundResourceFetchAssets> background_resource_fetch_assets = base::MakeRefCounted<FakeBackgroundResourceFetchAssets>(
        background_task_runner_,
        base::BindLambdaForTesting([&](mojo::PendingReceiver<network::mojom::URLLoader> loader, mojo::PendingRemote<network::mojom::URLLoaderClient> client) {
    ...
    std::unique_ptr<BackgroundURLLoader> background_url_loader = std::make_unique<BackgroundURLLoader>(std::move(background_resource_fetch_assets),
        ...
        /*background_code_cache_host*/ nullptr);
    FakeURLLoaderClient client(unfreezable_task_runner_);
    background_url_loader->LoadAsynchronously(CreateTestRequest(), SecurityOrigin::Create(KURL(kTestURL)),
        /*no_mime_sniffing=*/false,
        std::make_unique<ResourceLoadInfoNotifierWrapper>(
            /*resource_load_info_notifier=*/nullptr),
...
TEST_F(BackgroundResourceFecherTest, CancelAfterStart)
{
    ...
    loader_client_remote.set_disconnect_handler(base::BindLambdaForTesting([&]() { url_loader_client_dissconnected = true; }));
    loader.set_disconnect_handler(base::BindLambdaForTesting([&]() { url_loader_dissconnected = true; }));
...
TEST_F(BackgroundResourceFecherTest, BufferIncomingFreezeAndResume)
{
    ...
    EXPECT_EQ(kTestBodyString.size(), BackForwardCacheBufferLimitTracker::Get().total_bytes_buffered_for_testing());
    EXPECT_TRUE(BackForwardCacheBufferLimitTracker::Get().IsUnderPerProcessBufferLimit());
    ...
    unfreezable_task_runner_->RunUntilIdle();
    EXPECT_FALSE(bfcache_loader_helper_->evicted_reason());
    EXPECT_EQ(kTestBodyString.size(), bfcache_loader_helper_->total_bytes_buffered());
    EXPECT_FALSE(bfcache_loader_helper_->process_wide_count_updated());

    // Restore from BFCache.
    BackForwardCacheBufferLimitTracker::Get().DidRemoveFrameOrWorkerFromBackForwardCache(bfcache_loader_helper_->total_bytes_buffered());
    background_url_loader->Freeze(LoaderFreezeMode::kNone);
    task_environment_.RunUntilIdle();
    unfreezable_task_runner_->RunUntilIdle();
    EXPECT_TRUE(client.response());
...
TEST_F(BackgroundResourceFecherTest, BufferIncomingFreezeExceedMaxBufferedBytesPerProcess)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
    auto background_url_loader = CreateBackgroundURLLoaderAndStart(CreateTestRequest(), &client);
    ...
    mojo::Remote<network::mojom::URLLoaderClient> loader_client_remote(std::move(loader_client_pending_remote_));
    loader_client_remote->OnReceiveResponse(CreateTestResponse(), CreateDataPipeConsumerHandleFilledWithString(std::string(kBodySize, '*')),
        /*cached_metadata=*/std::nullopt);
    ...
    EXPECT_FALSE(BackForwardCacheBufferLimitTracker::Get().IsUnderPerProcessBufferLimit());

    EXPECT_EQ(kBodySize, BackForwardCacheBufferLimitTracker::Get().total_bytes_buffered_for_testing());
    ...
    EXPECT_EQ(0u, bfcache_loader_helper_->total_bytes_buffered());
    unfreezable_task_runner_->RunUntilIdle();
    EXPECT_THAT(bfcache_loader_helper_->evicted_reason(), mojom::blink::RendererEvictionReason::kNetworkExceedsBufferLimit);
...
TEST_F(BackgroundResourceFecherTest, BufferIncomingFreezeAndResumeBeforeExecutingUnfreezableTask)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
    auto background_url_loader = CreateBackgroundURLLoaderAndStart(CreateTestRequest(), &client);
    ...
    EXPECT_EQ(kTestBodyString.size(), BackForwardCacheBufferLimitTracker::Get().total_bytes_buffered_for_testing());
    unfreezable_task_runner_->RunUntilIdle();
    EXPECT_EQ(0u, BackForwardCacheBufferLimitTracker::Get().total_bytes_buffered_for_testing());
...
TEST_F(BackgroundResourceFecherTest, ChangePriority)
{
    ...
    background_url_loader->DidChangePriority(WebURLRequest::Priority::kVeryHigh, 100);

    // Call RunUntilIdle() to receive Mojo IPC.
    task_environment_.RunUntilIdle();
    ASSERT_EQ(1u, loader.set_priority_log().size());
    EXPECT_EQ(net::RequestPriority::HIGHEST, loader.set_priority_log()[0].priority);
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorSyncReturnFalse)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);

    auto test_util = base::MakeRefCounted<BackgroundResponseProcessorTestUtil>();
    test_util->SetSyncReturnFalse();
    auto background_url_loader = CreateBackgroundURLLoaderAndStart(CreateTestRequest(), &client, test_util->CreateProcessorFactory());
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorFinishWithPipeBeforeOtherIpc)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
    ...
    PostCrossThreadTask(*test_util->background_task_runner(), FROM_HERE,
        WTF::CrossThreadBindOnce(&BackgroundResponseProcessor::Client::DidFinishBackgroundResponseProcessor, WTF::CrossThreadUnretained(test_util->client()),
            std::move(test_util->head()), std::move(test_util->body()), std::move(test_util->cached_metadata_buffer())));
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorFinishWithRawDataBeforeOtherIpc)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
    ...
    PostCrossThreadTask(*test_util->background_task_runner(), FROM_HERE,
        WTF::CrossThreadBindOnce(&BackgroundResponseProcessor::Client::DidFinishBackgroundResponseProcessor, WTF::CrossThreadUnretained(test_util->client()),
            std::move(test_util->head()), CreateTestBodyRawData(), std::move(test_util->cached_metadata_buffer())));
    // RunUntilIdle() to run the FinishCallback.
    task_environment_.RunUntilIdle();
    ...
    EXPECT_TRUE(client.cached_metadata());
    EXPECT_FALSE(client.response_body_handle());
    EXPECT_THAT(client.response_body_buffer().CopyAs<Vector<char>>(), testing::ElementsAreArray(kTestBodyString));
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorFinishAfterOnTransferSizeUpdatedIpc)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorFinishAfterOnCompleteIpc)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorFreezeBeforeReceiveResponse)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorFreezeAfterReceiveResponse)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorExceedMaxBufferedBytesPerProcess)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
    ...
    PostCrossThreadTask(*test_util->background_task_runner(), FROM_HERE,
        WTF::CrossThreadBindOnce(&BackgroundResponseProcessor::Client::DidFinishBackgroundResponseProcessor, WTF::CrossThreadUnretained(test_util->client()),
            std::move(test_util->head()), std::move(body_raw_data), std::move(test_util->cached_metadata_buffer())));
    // RunUntilIdle() to run the FinishCallback.
    task_environment_.RunUntilIdle();
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorCancelBeforeReceiveResponse)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorCancelAfterReceiveResponse)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorCancelAfterReceiveResponseAndCallFinish)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
...
TEST_F(BackgroundResourceFecherTest, BackgroundResponseProcessorCompleteWithoutResponse)
{
    FakeURLLoaderClient client(unfreezable_task_runner_);
