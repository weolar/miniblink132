...
using RefCountedURLLoaderClientRemote = base::RefCountedData<mojo::Remote<network::mojom::URLLoaderClient>>;
...
static constexpr char kTestUrlForCodeCacheWithHashing[] = "codecachewithhashing://www.example.com/";
...
std::string ReadOneChunk(mojo::ScopedDataPipeConsumerHandle* handle)
{
    ...
    MojoResult result = (*handle)->ReadData(MOJO_READ_DATA_FLAG_NONE, base::as_writable_byte_span(buffer), actually_read_bytes);
...
{
    std::unique_ptr<network::ResourceRequest> request(new network::ResourceRequest());
...
mojo::ScopedDataPipeConsumerHandle CreateDataPipeConsumerHandleFilledWithString(const std::string& string)
{
    mojo::ScopedDataPipeProducerHandle producer_handle;
    mojo::ScopedDataPipeConsumerHandle consumer_handle;
    CHECK_EQ(mojo::CreateDataPipe(nullptr, producer_handle, consumer_handle), MOJO_RESULT_OK);
...
#endif
    SchemeRegistry::RegisterURLSchemeAsCodeCacheWithHashing("codecachewithhashing");
}
...
public:
        ...
        const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr head, FollowRedirectCallback follow_redirect_callback) override
    {
        ...
        std::move(on_received_redirect_callback_).Run(redirect_info, std::move(head), std::move(follow_redirect_callback));
    }
    ...
    bool upload_progress_called() const
    {
        return upload_progress_called_;
    }
    ...
    void SetOnReceivedRedirectCallback(base::OnceCallback<void(const net::RedirectInfo&, network::mojom::URLResponseHeadPtr, FollowRedirectCallback)> callback)
    {
...
private:
    ...
    base::OnceCallback<void(const net::RedirectInfo&, network::mojom::URLResponseHeadPtr, FollowRedirectCallback)> on_received_redirect_callback_;
};

class MockLoader : public network::mojom::URLLoader {
public:
    using RepeatingFollowRedirectCallback
        = base::RepeatingCallback<void(const std::vector<std::string>& removed_headers, const net::HttpRequestHeaders& modified_headers)>;
    MockLoader() = default;
...
using FetchCachedCodeCallback = mojom::blink::CodeCacheHost::FetchCachedCodeCallback;
using ProcessCodeCacheRequestCallback = base::RepeatingCallback<void(mojom::blink::CodeCacheType, const KURL&, FetchCachedCodeCallback)>;

class DummyCodeCacheHost final : public mojom::blink::CodeCacheHost {
public:
    explicit DummyCodeCacheHost(ProcessCodeCacheRequestCallback process_code_cache_request_callback)
        : process_code_cache_request_callback_(std::move(process_code_cache_request_callback))
    {
        mojo::PendingRemote<mojom::blink::CodeCacheHost> pending_remote;
        receiver_ = std::make_unique<mojo::Receiver<mojom::blink::CodeCacheHost>>(this, pending_remote.InitWithNewPipeAndPassReceiver());
        host_ = std::make_unique<blink::CodeCacheHost>(mojo::Remote<mojom::blink::CodeCacheHost>(std::move(pending_remote)));
    }

    // mojom::blink::CodeCacheHost implementations
    void DidGenerateCacheableMetadata(
        mojom::blink::CodeCacheType cache_type, const KURL& url, base::Time expected_response_time, mojo_base::BigBuffer data) override
    {
    }
    void FetchCachedCode(mojom::blink::CodeCacheType cache_type, const KURL& url, FetchCachedCodeCallback callback) override
    {
        process_code_cache_request_callback_.Run(cache_type, url, std::move(callback));
    }
    void ClearCodeCacheEntry(mojom::blink::CodeCacheType cache_type, const KURL& url) override
    {
        did_clear_code_cache_entry_ = true;
    }
    void DidGenerateCacheableMetadataInCacheStorage(
        const KURL& url, base::Time expected_response_time, mojo_base::BigBuffer data, const WTF::String& cache_storage_cache_name) override
    {
    }

    blink::CodeCacheHost* GetCodeCacheHost()
    {
        return host_.get();
    }
...
class ResourceRequestSenderTest : public testing::Test, public network::mojom::URLLoaderFactory {
public:
    ...
    void CreateLoaderAndStart(mojo::PendingReceiver<network::mojom::URLLoader> receiver, int32_t request_id, uint32_t options,
...
protected:
    ResourceRequestSender* sender()
    {
        return resource_request_sender_.get();
    }

    void StartAsync(std::unique_ptr<network::ResourceRequest> request, scoped_refptr<ResourceRequestClient> client, CodeCacheHost* code_cache_host = nullptr)
    {
        sender()->SendAsync(std::move(request), scheduler::GetSingleThreadTaskRunnerForTesting(), TRAFFIC_ANNOTATION_FOR_TESTS, false,
            /*cors_exempt_header_list=*/Vector<String>(), std::move(client), base::MakeRefCounted<network::WeakWrapperSharedURLLoaderFactory>(this),
    ...
    std::vector<std::pair<mojo::PendingReceiver<network::mojom::URLLoader>, mojo::PendingRemote<network::mojom::URLLoaderClient>>> loader_and_clients_;
...
TEST_F(ResourceRequestSenderTest, RedirectSyncFollow)
{
    ...
    mojo::Remote<network::mojom::URLLoaderClient> client(std::move(loader_and_clients_[0].second));
    std::unique_ptr<MockLoader> mock_loader = std::make_unique<MockLoader>();
    MockLoader* mock_loader_prt = mock_loader.get();
    mojo::MakeSelfOwnedReceiver(std::move(mock_loader), std::move(loader_and_clients_[0].first));

    base::RunLoop run_loop_for_redirect;
    mock_loader_prt->SetFollowRedirectCallback(
        base::BindLambdaForTesting([&](const std::vector<std::string>& removed_headers, const net::HttpRequestHeaders& modified_headers) {
    ...
    mock_client_->SetOnReceivedRedirectCallback(base::BindLambdaForTesting(
        [&](const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr head, ResourceRequestClient::FollowRedirectCallback callback) {
    ...
    client->OnReceiveRedirect(redirect_info, network::mojom::URLResponseHead::New());
    run_loop_for_redirect.Run();
    client->OnReceiveResponse(network::mojom::URLResponseHead::New(), mojo::ScopedDataPipeConsumerHandle(), std::nullopt);
    base::RunLoop().RunUntilIdle();
...
TEST_F(ResourceRequestSenderTest, RedirectSyncFollowWithRemovedHeaders)
{
    ...
    mock_loader_prt->SetFollowRedirectCallback(
        base::BindLambdaForTesting([&](const std::vector<std::string>& removed_headers, const net::HttpRequestHeaders& modified_headers) {
            ...
            EXPECT_THAT(removed_headers, ::testing::ElementsAreArray({ "Foo-Bar", "Hoge-Piyo" }));
            EXPECT_TRUE(modified_headers.IsEmpty());
            run_loop_for_redirect.Quit();
        }));
...
TEST_F(ResourceRequestSenderTest, RedirectSyncCancel)
{
    ...
    mock_loader_prt->SetFollowRedirectCallback(
        base::BindRepeating([](const std::vector<std::string>& removed_headers, const net::HttpRequestHeaders& modified_headers) {
...
TEST_F(ResourceRequestSenderTest, ReceiveResponseWithMetadata)
{
    ...
    client->OnReceiveResponse(network::mojom::URLResponseHead::New(), mojo::ScopedDataPipeConsumerHandle(), mojo_base::BigBuffer(metadata));
...
TEST_F(ResourceRequestSenderTest, EmptyCodeCacheThenReceiveResponse)
{
    ...
    auto code_cache_host = std::make_unique<DummyCodeCacheHost>(
        base::BindLambdaForTesting([&](mojom::blink::CodeCacheType cache_type, const KURL& url, FetchCachedCodeCallback callback) {
    ...
    StartAsync(std::move(request), mock_client_, code_cache_host->GetCodeCacheHost());
    ASSERT_EQ(1u, loader_and_clients_.size());
    mojo::Remote<network::mojom::URLLoaderClient> client(std::move(loader_and_clients_[0].second));

    run_loop.Run();
    // Send an empty cached data from CodeCacheHost.
    std::move(fetch_cached_code_callback).Run(base::Time(), mojo_base::BigBuffer());
    base::RunLoop().RunUntilIdle();
...
TEST_F(ResourceRequestSenderTest, ReceiveCodeCacheThenReceiveResponse)
{
    ...
    std::move(fetch_cached_code_callback).Run(response->response_time, mojo_base::BigBuffer(cache_data));
    base::RunLoop().RunUntilIdle();

    // Send a response without metadata.
    client->OnReceiveResponse(std::move(response), mojo::ScopedDataPipeConsumerHandle(), std::nullopt);
...
TEST_F(ResourceRequestSenderTest, ReceiveTimeMismatchCodeCacheThenReceiveResponse)
{
    mock_client_ = base::MakeRefCounted<MockRequestClient>();
    ...
    // Send a cached data from CodeCacheHost.
    std::vector<uint8_t> cache_data { 1, 2, 3, 4, 5, 6 };
    std::move(fetch_cached_code_callback).Run(response->response_time - base::Seconds(1), mojo_base::BigBuffer(cache_data));
    base::RunLoop().RunUntilIdle();
...
TEST_F(ResourceRequestSenderTest, ReceiveEmptyCodeCacheThenReceiveResponseWithMetadata)
{
    mock_client_ = base::MakeRefCounted<MockRequestClient>();
    ...
    // Send a response with metadata.
    std::vector<uint8_t> metadata { 1, 2, 3, 4, 5 };
    client->OnReceiveResponse(CreateResponse(), mojo::ScopedDataPipeConsumerHandle(), mojo_base::BigBuffer(metadata));
...
TEST_F(ResourceRequestSenderTest, ReceiveCodeCacheThenReceiveResponseWithMetadata)
{
    mock_client_ = base::MakeRefCounted<MockRequestClient>();
    ...
    // Send a response with metadata.
    std::vector<uint8_t> metadata { 1, 2, 3, 4, 5 };
    client->OnReceiveResponse(std::move(response), mojo::ScopedDataPipeConsumerHandle(), mojo_base::BigBuffer(metadata));
...
TEST_F(ResourceRequestSenderTest, ReceiveResponseWithMetadataThenReceiveCodeCache)
{
    mock_client_ = base::MakeRefCounted<MockRequestClient>();
    ...
    // Send a cached data from CodeCacheHost.
    std::vector<uint8_t> cache_data { 1, 2, 3, 4, 5, 6 };
    std::move(fetch_cached_code_callback).Run(response_time, mojo_base::BigBuffer(cache_data));
...
TEST_F(ResourceRequestSenderTest, ReceiveResponseWithMetadataThenReceiveEmptyCodeCache)
{
    mock_client_ = base::MakeRefCounted<MockRequestClient>();
...
TEST_F(ResourceRequestSenderTest, SlowCodeCache)
{
    ...
    mock_loader_prt->SetFollowRedirectCallback(base::BindLambdaForTesting(
        [&](const std::vector<std::string>& removed_headers, const net::HttpRequestHeaders& modified_headers) { follow_redirect_callback_called = true; }));

    mock_client_->SetOnReceivedRedirectCallback(
        base::BindLambdaForTesting([&](const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr head,
                                       ResourceRequestClient::FollowRedirectCallback callback) { std::move(callback).Run({}, {}); }));
    ...
    redirect_info.new_url = GURL(kRedirectedUrl);
    client->OnReceiveRedirect(redirect_info, network::mojom::URLResponseHead::New());
    client->OnUploadProgress(/*current_position=*/10, /*total_size=*/10, base::BindLambdaForTesting([]() {}));
    client->OnReceiveResponse(std::move(response), CreateDataPipeConsumerHandleFilledWithString(kTestData), std::nullopt);
...
TEST_F(ResourceRequestSenderTest, ReceiveCodeCacheThenReceiveSyntheticResponseFromServiceWorker)
{
    mock_client_ = base::MakeRefCounted<MockRequestClient>();
...
TEST_F(ResourceRequestSenderTest, ReceiveCodeCacheThenReceivePassThroughResponseFromServiceWorker)
{
    mock_client_ = base::MakeRefCounted<MockRequestClient>();
...
TEST_F(ResourceRequestSenderTest, ReceiveCodeCacheThenReceiveDifferentUrlResponseFromServiceWorker)
{
    mock_client_ = base::MakeRefCounted<MockRequestClient>();
...
TEST_F(ResourceRequestSenderTest, ReceiveCodeCacheThenReceiveResponseFromCacheStorageViaServiceWorker)
{
    mock_client_ = base::MakeRefCounted<MockRequestClient>();
...
TEST_F(ResourceRequestSenderTest, CodeCacheWithHashingEmptyCodeCache)
{
    ...
    // Send a response without metadata.
    client->OnReceiveResponse(CreateResponse(), mojo::ScopedDataPipeConsumerHandle(), std::nullopt);
...
TEST_F(ResourceRequestSenderTest, CodeCacheWithHashingWithCodeCache)
{
    ...
    // Send a cached data from CodeCacheHost.
    std::vector<uint8_t> cache_data { 1, 2, 3, 4, 5, 6 };
    std::move(fetch_cached_code_callback).Run(base::Time(), mojo_base::BigBuffer(cache_data));
    base::RunLoop().RunUntilIdle();
...
TEST_F(ResourceRequestSenderTest, CodeCacheWithHashingWithCodeCacheAfterRedirectedToDifferentScheme)
{
    RegisterURLSchemeAsCodeCacheWithHashing();
    mock_client_ = base::MakeRefCounted<MockRequestClient>();
    ...
    mojo::Remote<network::mojom::URLLoaderClient> second_client(std::move(loader_and_clients_[1].second));

    // Send a response without metadata.
    second_client->OnReceiveResponse(CreateResponse(), mojo::ScopedDataPipeConsumerHandle(), std::nullopt);
...
public:
    ...
    ResourceRequestSenderSyncTest& operator=(const ResourceRequestSenderSyncTest&) = delete;
    ~ResourceRequestSenderSyncTest() override = default;

protected:
    SyncLoadResponse SendSync(scoped_refptr<ResourceRequestClient> client, scoped_refptr<network::SharedURLLoaderFactory> loader_factory)
    {
        base::WaitableEvent terminate_sync_load_event(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
        SyncLoadResponse response;
        auto sender = std::make_unique<ResourceRequestSender>();
        sender->SendSync(CreateSyncResourceRequest(), TRAFFIC_ANNOTATION_FOR_TESTS, network::mojom::kURLLoadOptionSynchronous, &response,
...
{
    scoped_refptr<MockRequestClient> mock_client = base::MakeRefCounted<MockRequestClient>();
    auto loader_factory = base::MakeRefCounted<FakeURLLoaderFactoryForBackgroundThread>(
        base::BindOnce([](mojo::PendingReceiver<network::mojom::URLLoader> loader, mojo::PendingRemote<network::mojom::URLLoaderClient> client) {
            mojo::MakeSelfOwnedReceiver(std::make_unique<MockLoader>(), std::move(loader));
            mojo::Remote<network::mojom::URLLoaderClient> loader_client(std::move(client));
            loader_client->OnReceiveResponse(network::mojom::URLResponseHead::New(), CreateDataPipeConsumerHandleFilledWithString(kTestData), std::nullopt);
            loader_client->OnComplete(network::URLLoaderCompletionStatus(net::Error::OK));
        }));
    ...
    EXPECT_EQ(kTestData, std::string(response.data->begin()->data(), response.data->size()));
}
...
TEST_F(ResourceRequestSenderSyncTest, SendSyncRedirect)
{
    ...
    auto loader_factory = base::MakeRefCounted<FakeURLLoaderFactoryForBackgroundThread>(
        base::BindOnce([](mojo::PendingReceiver<network::mojom::URLLoader> loader, mojo::PendingRemote<network::mojom::URLLoaderClient> client) {
            ...
            net::RedirectInfo redirect_info;
            redirect_info.new_url = GURL(kRedirectedUrl);
            loader_client->OnReceiveRedirect(redirect_info, network::mojom::URLResponseHead::New());

            scoped_refptr<RefCountedURLLoaderClientRemote> refcounted_client = base::MakeRefCounted<RefCountedURLLoaderClientRemote>(std::move(loader_client));

            mock_loader_prt->SetFollowRedirectCallback(base::BindRepeating(
                [](scoped_refptr<RefCountedURLLoaderClientRemote> refcounted_client, const std::vector<std::string>& removed_headers,
                    ...
                    refcounted_client->data->OnReceiveResponse(
                        network::mojom::URLResponseHead::New(), CreateDataPipeConsumerHandleFilledWithString(kTestData), std::nullopt);

                    refcounted_client->data->OnComplete(network::URLLoaderCompletionStatus(net::Error::OK));
                },
...
TEST_F(ResourceRequestSenderSyncTest, SendSyncRedirectCancel)
{
    ...
    auto loader_factory = base::MakeRefCounted<FakeURLLoaderFactoryForBackgroundThread>(
        base::BindOnce([](mojo::PendingReceiver<network::mojom::URLLoader> loader, mojo::PendingRemote<network::mojom::URLLoaderClient> client) {
            ...
            mock_loader_prt->SetFollowRedirectCallback(base::BindRepeating(
                [](scoped_refptr<base::RefCountedData<mojo::Remote<network::mojom::URLLoaderClient>>> refcounted_client,
                    const std::vector<std::string>& removed_headers, const net::HttpRequestHeaders& modified_headers) {
                    // FollowRedirect() must not be called.
                    CHECK(false);
                },
                std::move(refcounted_client)));
...
public:
    void PerformTest(network::mojom::URLResponseHeadPtr response_head)
    {
        ...
        client->OnReceiveResponse(std::move(response_head), mojo::ScopedDataPipeConsumerHandle(), std::nullopt);
        base::RunLoop().RunUntilIdle();
    }
...
TEST_F(TimeConversionTest, ProperlyInitialized)
{
    ...
    response_head->load_timing.connect_timing.connect_start = TicksFromMicroseconds(13);
    ...
    EXPECT_EQ(base::TimeTicks(), received_load_timing().connect_timing.domain_lookup_start);
...
public:
    void PerformTest(base::TimeTicks remote_request_start, base::TimeTicks completion_time, base::TimeDelta delay)
    {
        std::unique_ptr<network::ResourceRequest> request(CreateResourceRequest());
        mock_client_ = base::MakeRefCounted<MockRequestClient>();
        StartAsync(std::move(request), mock_client_);
        ...
        // copied.
        response_head->load_timing.request_start_time = base::Time() + base::Seconds(99);

        mojo::ScopedDataPipeProducerHandle producer_handle;
        mojo::ScopedDataPipeConsumerHandle consumer_handle;
        ASSERT_EQ(mojo::CreateDataPipe(nullptr, producer_handle, consumer_handle), MOJO_RESULT_OK);

        client->OnReceiveResponse(std::move(response_head), std::move(consumer_handle), std::nullopt);
...
TEST_F(CompletionTimeConversionTest, Convert)
{
    ...
    const auto remote_completion_time = remote_request_start + base::Milliseconds(3);

    PerformTest(remote_request_start, remote_completion_time, base::Milliseconds(15));
