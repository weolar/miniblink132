...
public:
    ...
    MockResourceRequestSender& operator=(const MockResourceRequestSender&) = delete;
    ~MockResourceRequestSender() override = default;

    // ResourceRequestSender implementation:
    void SendSync(std::unique_ptr<network::ResourceRequest> request, const net::NetworkTrafficAnnotationTag& traffic_annotation, uint32_t loader_options,
        SyncLoadResponse* response, scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory, WebVector<std::unique_ptr<URLLoaderThrottle>> throttles,
        base::TimeDelta timeout, const Vector<String>& cors_exempt_header_list, base::WaitableEvent* terminate_sync_load_event,
        mojo::PendingRemote<mojom::blink::BlobRegistry> download_to_blob_registry, scoped_refptr<ResourceRequestClient> resource_request_client,
        std::unique_ptr<ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper) override
    {
        *response = std::move(sync_load_response_);
    }

    int SendAsync(std::unique_ptr<network::ResourceRequest> request, scoped_refptr<base::SequencedTaskRunner> loading_task_runner,
        ...
        scoped_refptr<ResourceRequestClient> resource_request_client, scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory,
        ...
        base::RepeatingCallback<void(size_t)> did_buffer_load_while_in_bfcache_callback) override
    {
    ...
    LoaderFreezeMode freeze_mode() const
    {
        return freeze_mode_;
    }
...
public:
    TestURLLoaderClient()
        : loader_(new URLLoader(
            ...
            /*terminate_sync_load_event=*/nullptr, scheduler::GetSingleThreadTaskRunnerForTesting(), scheduler::GetSingleThreadTaskRunnerForTesting(),
            base::MakeRefCounted<network::WeakWrapperSharedURLLoaderFactory>(&fake_url_loader_factory_),
    ...
    void set_delete_on_receive_redirect()
    {
        delete_on_receive_redirect_ = true;
    }
    void set_delete_on_receive_response()
    {
        delete_on_receive_response_ = true;
    }
    void set_delete_on_receive_data()
    {
        delete_on_receive_data_ = true;
    }
    void set_delete_on_finish()
    {
        delete_on_finish_ = true;
    }
    void set_delete_on_fail()
    {
        delete_on_fail_ = true;
    }

    bool did_receive_redirect() const
    {
        return did_receive_redirect_;
    }
    bool did_receive_response() const
    {
        return did_receive_response_;
    }
    bool did_receive_response_body() const
    {
...
public:
    ...
    void DoStartAsyncRequest()
    {
        ...
        client()->loader()->LoadAsynchronously(std::move(request), /*url_request_extra_data=*/nullptr,
    ...
    void DoReceiveRedirect()
    {
        ...
        redirect_info.new_site_for_cookies = net::SiteForCookies::FromUrl(GURL(kTestURL));
        std::vector<std::string> removed_headers;
        bool callback_called = false;
        resource_request_client()->OnReceivedRedirect(redirect_info, network::mojom::URLResponseHead::New(),
            /*follow_redirect_callback=*/
            WTF::BindOnce(
                [](bool* callback_called, std::vector<std::string> removed_headers, net::HttpRequestHeaders modified_headers) { *callback_called = true; },
    ...
    void DoReceiveResponse()
    {
        ...
        resource_request_client()->OnReceivedResponse(network::mojom::URLResponseHead::New(), std::move(handle_to_pass),
    ...
    MockResourceRequestSender* sender()
    {
        return sender_;
    }
    ResourceRequestClient* resource_request_client()
...
TEST_F(URLLoaderTest, ClientAddressSpace)
{
    ...
    EXPECT_EQ(network::mojom::IPAddressSpace::kPublic, response.ClientAddressSpace());
}
...
TEST_F(URLLoaderTest, SSLInfo)
{
    ...
    ASSERT_TRUE(net::LoadCertificateFiles({ "subjectAltName_sanity_check.pem", "root_ca_cert.pem" }, &certs));
    ASSERT_EQ(2U, certs.size());

    std::string_view cert0_der = net::x509_util::CryptoBufferAsStringPiece(certs[0]->cert_buffer());
    std::string_view cert1_der = net::x509_util::CryptoBufferAsStringPiece(certs[1]->cert_buffer());

    net::SSLInfo ssl_info;
    ssl_info.cert = net::X509Certificate::CreateFromDERCertChain({ cert0_der, cert1_der });
    net::SSLConnectionStatusSetVersion(net::SSL_CONNECTION_VERSION_TLS1_2, &ssl_info.connection_status);

    network::mojom::URLResponseHead head;
    head.ssl_info = ssl_info;
    WebURLResponse web_url_response = WebURLResponse::Create(url, head, true, -1);

    const std::optional<net::SSLInfo>& got_ssl_info = web_url_response.ToResourceResponse().GetSSLInfo();
...
TEST_F(URLLoaderTest, SyncLengths)
{
    ...
    sync_load_response.data = SharedBuffer::Create(kBodyData, sizeof(kBodyData) - 1);
    ASSERT_EQ(17u, sync_load_response.data->size());
    sync_load_response.head->encoded_body_length = network::mojom::EncodedBodyLength::New(kEncodedBodyLength);
    ...
    client()->loader()->LoadSynchronously(std::move(request), /*top_frame_origin=*/nullptr,
        /*download_to_blob=*/false,
        /*no_mime_sniffing=*/false, base::TimeDelta(), nullptr, response, error, data, encoded_data_length, encoded_body_length, downloaded_blob,
        std::make_unique<ResourceLoadInfoNotifierWrapper>(
            /*resource_load_info_notifier=*/nullptr));
...
TEST_F(URLLoaderTest, AuthChallengeInfo)
{
    ...
    blink::WebURLResponse response = WebURLResponse::Create(KURL(), head, true, -1);
