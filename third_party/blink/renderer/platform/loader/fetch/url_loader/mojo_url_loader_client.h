...
class BLINK_PLATFORM_EXPORT MojoURLLoaderClient final : public network::mojom::URLLoaderClient {
public:
    MojoURLLoaderClient(ResourceRequestSender* resource_request_sender, scoped_refptr<base::SequencedTaskRunner> task_runner, bool bypass_redirect_checks,
        const GURL& request_url, base::OnceCallback<void(mojom::blink::RendererEvictionReason)> evict_from_bfcache_callback,
...
private:
    ...
    const KURL& last_loaded_url() const
    {
        return last_loaded_url_;
    }
    ...
    const raw_ptr<ResourceRequestSender, DanglingUntriaged> resource_request_sender_;
    scoped_refptr<base::SequencedTaskRunner> task_runner_;
    bool bypass_redirect_checks_ = false;
    KURL last_loaded_url_;
    base::OnceCallback<void(mojom::blink::RendererEvictionReason)> evict_from_bfcache_callback_;
    base::RepeatingCallback<void(size_t)> did_buffer_load_while_in_bfcache_callback_;
