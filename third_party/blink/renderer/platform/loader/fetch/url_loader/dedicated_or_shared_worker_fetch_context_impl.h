...
class BLINK_PLATFORM_EXPORT DedicatedOrSharedWorkerFetchContextImpl final : public WebDedicatedOrSharedWorkerFetchContext,
...
public:
    ...
    DedicatedOrSharedWorkerFetchContextImpl(const RendererPreferences& renderer_preferences,
    ...
    // enabled by default.
    scoped_refptr<WebDedicatedOrSharedWorkerFetchContext> CloneForNestedWorkerDeprecated(scoped_refptr<base::SingleThreadTaskRunner> task_runner) override;
    ...
    scoped_refptr<WebDedicatedOrSharedWorkerFetchContext> CloneForNestedWorker(WebServiceWorkerProviderContext* service_worker_provider_context,
        std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_loader_factory,
        std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_fallback_factory,
    ...
    void set_site_for_cookies(const net::SiteForCookies& site_for_cookies) override;
    ...
    void SetSubresourceFilterBuilder(std::unique_ptr<WebDocumentSubresourceFilter::Builder>) override;
    std::unique_ptr<WebDocumentSubresourceFilter> TakeSubresourceFilter() override;
    std::unique_ptr<WebSocketHandshakeThrottle> CreateWebSocketHandshakeThrottle(scoped_refptr<base::SingleThreadTaskRunner> task_runner) override;
    ...
    //   worker.
    void set_controller_service_worker_mode(mojom::ControllerServiceWorkerMode mode);

    void set_client_id(const WebString& client_id);

    WebString GetAcceptLanguages() const override;

    std::unique_ptr<ResourceLoadInfoNotifierWrapper> CreateResourceLoadInfoNotifierWrapper() override;
...
private:
    ...
    scoped_refptr<DedicatedOrSharedWorkerFetchContextImpl> CloneForNestedWorkerInternal(
        mojo::PendingReceiver<mojom::blink::ServiceWorkerWorkerClient> service_worker_client_receiver,
    ...
    void UpdateSubresourceLoaderFactories(std::unique_ptr<PendingURLLoaderFactoryBundle> subresource_loader_factories) override;
    ...
    mojo::Remote<mojom::blink::ServiceWorkerWorkerClientRegistry> service_worker_worker_client_registry_;

    // Bound to |this| on the worker thread.
    mojo::PendingReceiver<mojom::blink::ServiceWorkerWorkerClient> service_worker_client_receiver_;

    // Consumed on the worker thread to create
    // |service_worker_worker_client_registry_|.
    mojo::PendingRemote<mojom::blink::ServiceWorkerWorkerClientRegistry> pending_service_worker_worker_client_registry_;
    ...
    std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_fallback_factory_;
    ...
    CrossVariantMojoRemote<mojom::ServiceWorkerContainerHostInterfaceBase> service_worker_container_host_;

    mojom::ControllerServiceWorkerMode controller_service_worker_mode_ = mojom::ControllerServiceWorkerMode::kNoController;
    ...
    // of recovery.
    mojo::PendingReceiver<mojom::blink::SubresourceLoaderUpdater> pending_subresource_loader_updater_;
    mojo::Receiver<mojom::blink::SubresourceLoaderUpdater> subresource_loader_updater_ { this };

    std::unique_ptr<WebDocumentSubresourceFilter::Builder> subresource_filter_builder_;
    ...
    mojo::Receiver<mojom::blink::RendererPreferenceWatcher> preference_watcher_receiver_ { this };
    // Kept while staring up the worker thread. Valid until
    // InitializeOnWorkerThread().
    mojo::PendingReceiver<mojom::blink::RendererPreferenceWatcher> preference_watcher_pending_receiver_;
    mojo::RemoteSet<mojom::blink::RendererPreferenceWatcher> child_preference_watchers_;
    ...
    std::unique_ptr<WebSocketHandshakeThrottleProvider> websocket_handshake_throttle_provider_;

    Vector<String> cors_exempt_header_list_;

    mojo::PendingRemote<mojom::ResourceLoadInfoNotifier> pending_resource_load_info_notifier_;
    ...
    std::unique_ptr<WeakWrapperResourceLoadInfoNotifier> weak_wrapper_resource_load_info_notifier_;

    WeakPersistent<AcceptLanguagesWatcher> accept_languages_watcher_;
};

template <> struct DowncastTraits<DedicatedOrSharedWorkerFetchContextImpl> {
