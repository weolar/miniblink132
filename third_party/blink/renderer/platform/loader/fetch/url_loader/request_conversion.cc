...
std::string TrimLWSAndCRLF(const std::string_view& input)
{
    ...
    while (last_crlf > 0 && (string[last_crlf - 1] == '\r' || string[last_crlf - 1] == '\n')) {
        --last_crlf;
    }
...
mojom::ResourceType RequestContextToResourceType(mojom::blink::RequestContextType request_context)
{
...
void PopulateResourceRequestBody(const EncodedFormData& src, network::ResourceRequestBody* dest)
{
    for (const auto& element : src.Elements()) {
        ...
        case FormDataElement::kEncodedFile:
            if (element.file_length_ == -1) {
                dest->AppendFileRange(WebStringToFilePath(element.filename_), 0, std::numeric_limits<uint64_t>::max(),
                    element.expected_file_modification_time_.value_or(base::Time()));
            } else {
                dest->AppendFileRange(WebStringToFilePath(element.filename_), static_cast<uint64_t>(element.file_start_),
                    static_cast<uint64_t>(element.file_length_), element.expected_file_modification_time_.value_or(base::Time()));
            }
            break;
        case FormDataElement::kEncodedBlob: {
            CHECK(element.blob_data_handle_);
            mojo::Remote<mojom::blink::Blob> blob_remote(element.blob_data_handle_->CloneBlobRemote());
            mojo::PendingRemote<network::mojom::blink::DataPipeGetter> data_pipe_getter_remote;
            blob_remote->AsDataPipeGetter(data_pipe_getter_remote.InitWithNewPipeAndPassReceiver());
            dest->AppendDataPipe(ToCrossVariantMojoType(std::move(data_pipe_getter_remote)));
            break;
        }
        case FormDataElement::kDataPipe: {
            mojo::PendingRemote<network::mojom::blink::DataPipeGetter> pending_data_pipe_getter;
            element.data_pipe_getter_->GetDataPipeGetter()->Clone(pending_data_pipe_getter.InitWithNewPipeAndPassReceiver());
            dest->AppendDataPipe(ToCrossVariantMojoType(std::move(pending_data_pipe_getter)));
            break;
        }
        }
    }
...
bool IsBannedCrossSiteAuth(network::ResourceRequest* resource_request, WebURLRequestExtraData* url_request_extra_data)
{
    ...
    if (url_request_extra_data) {
        allow_cross_origin_auth_prompt = url_request_extra_data->allow_cross_origin_auth_prompt();
    }

    if (first_party.IsFirstPartyWithSchemefulMode(request_url, /*compute_schemefully=*/false)) {
        // If the first party is secure but the subresource is not, this is
        // mixed-content. Do not allow the image.
        if (!allow_cross_origin_auth_prompt && network::IsUrlPotentiallyTrustworthy(first_party.RepresentativeUrl())
...
scoped_refptr<network::ResourceRequestBody> NetworkResourceRequestBodyFor(ResourceRequestBody src_body)
{
    ...
    if (const EncodedFormData* form_body = src_body.FormBody().get()) {
        ...
        mojo::PendingRemote<network::mojom::blink::ChunkedDataPipeGetter> stream_body = src_body.TakeStreamBody();
        dest_body = base::MakeRefCounted<network::ResourceRequestBody>();
        dest_body->SetToChunkedDataPipe(ToCrossVariantMojoType(std::move(stream_body)), network::ResourceRequestBody::ReadOnlyOnce(true));
    }
    return dest_body;
}

void PopulateResourceRequest(const ResourceRequestHead& src, ResourceRequestBody src_body, network::ResourceRequest* dest)
{
    ...
    if (src.GetDevToolsAcceptedStreamTypes()) {
        dest->devtools_accepted_stream_types
            = std::vector<net::SourceStream::SourceType>(src.GetDevToolsAcceptedStreamTypes()->data.begin(), src.GetDevToolsAcceptedStreamTypes()->data.end());
    }
    if (src.RequestorOrigin()->ToString() == "null") {
        ...
        // origin's precursor.
        dest->request_initiator = src.RequestorOrigin()->DeriveNewOpaqueOrigin()->ToUrlOrigin();
    } else {
    ...
    dest->referrer_policy = network::ReferrerPolicyForUrlRequest(src.GetReferrerPolicy());
    ...
    if (!src.GetRequestedWithHeader().empty()) {
        dest->cors_exempt_headers.SetHeader(kCorsExemptRequestedWithHeaderName, src.GetRequestedWithHeader().Utf8());
    }
    ...
    if (!src.GetPurposeHeader().empty()) {
        dest->cors_exempt_headers.SetHeader(kCorsExemptPurposeHeaderName, src.GetPurposeHeader().Utf8());
    }
    ...
    if (src.GetWebBundleTokenParams().has_value()) {
        dest->web_bundle_token_params = std::make_optional(network::ResourceRequest::WebBundleTokenParams(GURL(src.GetWebBundleTokenParams()->bundle_url),
            src.GetWebBundleTokenParams()->token, ToCrossVariantMojoType(src.GetWebBundleTokenParams()->CloneHandle())));
    }

    // TODO(kinuko): Deprecate this.
    dest->resource_type = static_cast<int>(RequestContextToResourceType(src.GetRequestContext()));

    if (src.IsFetchLikeAPI() && (dest->url.has_username() || dest->url.has_password())) {
        dest->do_not_prompt_for_login = true;
    }
    if (src.GetRequestContext() == mojom::blink::RequestContextType::PREFETCH || src.IsFavicon()) {
        dest->do_not_prompt_for_login = true;
    }
    ...
    dest->shared_storage_writable_eligible = src.GetSharedStorageWritableEligible();
    ...
    network::mojom::RequestDestination request_destination = src.GetRequestDestination();
    ...
    if (!dest->is_favicon && request_destination == network::mojom::RequestDestination::kImage
    ...
    dest->attribution_reporting_eligibility = src.GetAttributionReportingEligibility();
