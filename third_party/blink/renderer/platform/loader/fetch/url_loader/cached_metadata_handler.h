...
public:
    static std::unique_ptr<CachedMetadataSender> Create(
        const ResourceResponse&, mojom::blink::CodeCacheType, scoped_refptr<const SecurityOrigin> requestor_origin);

    static void SendToCodeCacheHost(CodeCacheHost*, mojom::blink::CodeCacheType, WTF::String, base::Time, const String&, base::span<const uint8_t>);

    virtual ~CachedMetadataSender() = default;
    virtual void Send(CodeCacheHost* code_cache_host, base::span<const uint8_t>) = 0;
...
PLATFORM_EXPORT bool ShouldUseIsolatedCodeCache(mojom::blink::RequestContextType, const ResourceResponse&);
...
public:
    ...
    virtual void OnMemoryDump(WebProcessMemoryDump* pmd, const String& dump_prefix) const = 0;
    ...
    virtual void SetCachedMetadata(CodeCacheHost*, uint32_t data_type_id, const uint8_t*, size_t) = 0;
    ...
    virtual scoped_refptr<CachedMetadata> GetCachedMetadata(uint32_t data_type_id, GetCachedMetadataBehavior behavior = kCrashIfUnchecked) const = 0;
    ...
    virtual bool HashRequired() const
    {
        return false;
    }
