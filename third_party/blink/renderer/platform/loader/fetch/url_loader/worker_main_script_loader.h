...
// renderer process.
class PLATFORM_EXPORT WorkerMainScriptLoader final : public GarbageCollected<WorkerMainScriptLoader>, public network::mojom::URLLoaderClient {
public:
    WorkerMainScriptLoader();
    ~WorkerMainScriptLoader() override;

    // Starts to load the main script.
    void Start(const FetchParameters& fetch_params, std::unique_ptr<WorkerMainScriptLoadParameters> worker_main_script_load_params, FetchContext* fetch_context,
        ResourceLoadObserver* resource_load_observer, WorkerMainScriptLoaderClient* client);
    ...
    void OnReceiveResponse(network::mojom::URLResponseHeadPtr response_head, mojo::ScopedDataPipeConsumerHandle handle,
    ...
    const KURL& GetRequestURL() const
    {
        return initial_request_url_;
    }
    ...
    WTF::TextEncoding GetScriptEncoding()
    {
        return script_encoding_;
    }
...
private:
    ...
    void HandleRedirections(std::vector<net::RedirectInfo>& redirect_infos, std::vector<network::mojom::URLResponseHeadPtr>& redirect_responses);
    ...
    std::unique_ptr<ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper_;
};
