...
public:
    using BodyVariant = absl::variant<mojo::ScopedDataPipeConsumerHandle, SegmentedBuffer>;
    class Client {
    public:
        virtual ~Client() = default;
        virtual void DidFinishBackgroundResponseProcessor(
            network::mojom::URLResponseHeadPtr head, BodyVariant body, std::optional<mojo_base::BigBuffer> cached_metadata)
            = 0;
    ...
    virtual bool MaybeStartProcessingResponse(network::mojom::URLResponseHeadPtr& head, mojo::ScopedDataPipeConsumerHandle& body,
        std::optional<mojo_base::BigBuffer>& cached_metadata, scoped_refptr<base::SequencedTaskRunner> background_task_runner, Client* client)
        = 0;
};
