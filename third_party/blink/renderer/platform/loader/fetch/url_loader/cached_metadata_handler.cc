...
public:
    CachedMetadataSenderImpl(const ResourceResponse&, mojom::blink::CodeCacheType);
...
CachedMetadataSenderImpl::CachedMetadataSenderImpl(const ResourceResponse& response, mojom::blink::CodeCacheType code_cache_type)
    ...
    , code_cache_type_(code_cache_type)
{
    ...
    DCHECK(response.CacheStorageCacheName().IsNull() || code_cache_type_ == mojom::blink::CodeCacheType::kWebAssembly);
    DCHECK(!response.WasFetchedViaServiceWorker() || response.IsServiceWorkerPassThrough() || code_cache_type_ == mojom::blink::CodeCacheType::kWebAssembly);
}

void CachedMetadataSenderImpl::Send(CodeCacheHost* code_cache_host, base::span<const uint8_t> data)
{
    if (!code_cache_host)
        return;
    // TODO(crbug.com/862940): This should use the Blink variant of the
    // interface.
    code_cache_host->get()->DidGenerateCacheableMetadata(code_cache_type_, response_url_, response_time_, mojo_base::BigBuffer(data));
}
...
public:
    ServiceWorkerCachedMetadataSender(const ResourceResponse&, scoped_refptr<const SecurityOrigin>);
...
ServiceWorkerCachedMetadataSender::ServiceWorkerCachedMetadataSender(const ResourceResponse& response, scoped_refptr<const SecurityOrigin> security_origin)
    : response_url_(response.CurrentRequestUrl())
    , response_time_(response.ResponseTime())
...
void ServiceWorkerCachedMetadataSender::Send(CodeCacheHost* code_cache_host, base::span<const uint8_t> data)
{
    if (!code_cache_host)
        return;
    code_cache_host->get()->DidGenerateCacheableMetadataInCacheStorage(response_url_, response_time_, mojo_base::BigBuffer(data), cache_storage_cache_name_);
}

// static
void CachedMetadataSender::SendToCodeCacheHost(CodeCacheHost* code_cache_host, mojom::blink::CodeCacheType code_cache_type, WTF::String url,
    base::Time response_time, const String& cache_storage_name, base::span<const uint8_t> data)
{
    if (!code_cache_host) {
        return;
    }
    if (cache_storage_name.IsNull()) {
        code_cache_host->get()->DidGenerateCacheableMetadata(code_cache_type, KURL(url), response_time, mojo_base::BigBuffer(data));
    } else {
        code_cache_host->get()->DidGenerateCacheableMetadataInCacheStorage(KURL(url), response_time, mojo_base::BigBuffer(data), cache_storage_name);
    }
}

// static
std::unique_ptr<CachedMetadataSender> CachedMetadataSender::Create(
    const ResourceResponse& response, mojom::blink::CodeCacheType code_cache_type, scoped_refptr<const SecurityOrigin> requestor_origin)
{
    // Non-ServiceWorker scripts and passthrough SW responses use the site
    // isolated code cache.
    if (!response.WasFetchedViaServiceWorker() || response.IsServiceWorkerPassThrough()) {
    ...
    if (!response.CacheStorageCacheName().IsNull()) {
        ...
        return std::make_unique<ServiceWorkerCachedMetadataSender>(response, std::move(requestor_origin));
    }
...
bool ShouldUseIsolatedCodeCache(mojom::blink::RequestContextType request_context, const ResourceResponse& response)
{
    ...
    // mechanism.
    return !response.WasFetchedViaServiceWorker() || response.IsServiceWorkerPassThrough();
}
