...
{
    static const base::FeatureParam<bool> kSendToScannerParam { &features::kThreadedBodyLoader, "send-to-scanner", true };
...
{
    static const base::FeatureParam<int> kMaxDataToProcessParam { &features::kThreadedBodyLoader, "max-data-to-process", 0 };
...
public:
    HeapArrayOrSize(base::span<const char> data, bool should_keep_encoded_data)
        : heap_array_or_size_(should_keep_encoded_data ? std::variant<base::HeapArray<char>, size_t>(base::HeapArray<char>::CopiedFrom(data)) : data.size())
    {
    }
    ...
    base::SpanOrSize<const char> AsSpanOrSize() const
    {
        ...
        //         heap_array_or_size_);
        const base::HeapArray<char>* heap_array = std::get_if<base::HeapArray<char>>(&heap_array_or_size_);
        if (heap_array)
            return base::SpanOrSize<const char>(base::span<const char>(heap_array->data(), heap_array->size()));
        const size_t* size = std::get_if<size_t>(&heap_array_or_size_);
        return base::SpanOrSize<const char>(*size);
    }

    size_t size() const
    {
        return AsSpanOrSize().size();
    }
...
void ReadFromDataPipeImpl(BodyReader& reader, mojo::ScopedDataPipeConsumerHandle& handle, mojo::SimpleWatcher& handle_watcher)
{
    ...
    while (reader.ShouldContinueReading()) {
        ...
        buffer = buffer.first(std::min<size_t>(buffer.size(), chunk_size - num_bytes_consumed));
        if (buffer.empty()) {
...
public:
    OffThreadBodyReader(mojo::ScopedDataPipeConsumerHandle response_body, std::unique_ptr<BodyTextDecoder> decoder,
        base::WeakPtr<NavigationBodyLoader> body_loader, scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner,
        scoped_refptr<base::SequencedTaskRunner> reader_task_runner, bool should_keep_encoded_data)
    ...
    {
        DCHECK(IsMainThread());
        PostCrossThreadTask(*reader_task_runner_, FROM_HERE, CrossThreadBindOnce(&OffThreadBodyReader::StartInBackground, CrossThreadUnretained(this)));
    }
    ...
    Vector<DataChunk> TakeData(size_t max_data_to_process)
    {
        ...
        if (!data_chunks_.empty()) {
            PostCrossThreadTask(*main_thread_task_runner_, FROM_HERE, CrossThreadBindOnce(&NavigationBodyLoader::ProcessOffThreadData, body_loader_));
        }
        return data;
    }
...
private:
    ...
    void StartInBackground()
    {
        ...
        DCHECK(reader_task_runner_->RunsTasksInCurrentSequence());
        response_body_watcher_ = std::make_unique<mojo::SimpleWatcher>(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL);
        response_body_watcher_->Watch(
            response_body_.get(), MOJO_HANDLE_SIGNAL_READABLE, base::BindRepeating(&OffThreadBodyReader::ReadFromDataPipe, base::Unretained(this)));
    ...
    void AddChunk(const String& decoded_data, base::span<const char> encoded_data, bool has_error)
    {
        DCHECK(reader_task_runner_->RunsTasksInCurrentSequence());
        HeapArrayOrSize encoded_data_or_size(encoded_data, should_keep_encoded_data_);
        ...
        bool post_task = false;
        {
            ...
            data_chunks_.push_back(DataChunk { .decoded_data = decoded_data,
    ...
    Client::ProcessBackgroundDataCallback process_background_data_callback_ GUARDED_BY(lock_);
    Vector<DataChunk> data_chunks_ GUARDED_BY(lock_);
};

void NavigationBodyLoader::OffThreadBodyReaderDeleter::operator()(const OffThreadBodyReader* ptr)
{
    if (ptr)
        ptr->Delete();
}
...
public:
    ...
    bool DataReceived(base::span<const char> data) override
    {
        base::WeakPtr<NavigationBodyLoader> weak_self = loader_->weak_factory_.GetWeakPtr();
...
NavigationBodyLoader::NavigationBodyLoader(const KURL& original_url, network::mojom::URLResponseHeadPtr response_head,
    mojo::ScopedDataPipeConsumerHandle response_body, network::mojom::URLLoaderClientEndpointsPtr endpoints,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, std::unique_ptr<ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper)
    : response_head_(std::move(response_head))
    , response_body_(std::move(response_body))
    , endpoints_(std::move(endpoints))
    , task_runner_(std::move(task_runner))
    , handle_watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, task_runner_)
    , resource_load_info_notifier_wrapper_(std::move(resource_load_info_notifier_wrapper))
    , original_url_(original_url)
    , should_send_directly_to_preload_scanner_(ShouldSendDirectlyToPreloadScanner())
...
NavigationBodyLoader::~NavigationBodyLoader()
{
    if (!has_received_completion_ || !has_seen_end_of_data_) {
        resource_load_info_notifier_wrapper_->NotifyResourceLoadCanceled(net::ERR_ABORTED);
    }
}

void NavigationBodyLoader::OnReceiveEarlyHints(network::mojom::EarlyHintsPtr early_hints)
{
...
void NavigationBodyLoader::OnReceiveRedirect(const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr head)
{
    // This has already happened in the browser process.
    NOTREACHED();
}

void NavigationBodyLoader::OnUploadProgress(int64_t current_position, int64_t total_size, OnUploadProgressCallback callback)
{
    // This has already happened in the browser process.
    NOTREACHED();
}
...
{
    network::RecordOnTransferSizeUpdatedUMA(network::OnTransferSizeUpdatedFrom::kNavigationBodyLoader);
    resource_load_info_notifier_wrapper_->NotifyResourceTransferSizeUpdated(transfer_size_diff);
}

void NavigationBodyLoader::OnComplete(const network::URLLoaderCompletionStatus& status)
{
...
void NavigationBodyLoader::StartLoadingBody(WebNavigationBodyLoader::Client* client)
{
    TRACE_EVENT1("loading", "NavigationBodyLoader::StartLoadingBody", "url", original_url_.GetString().Utf8());
    client_ = client;

    resource_load_info_notifier_wrapper_->NotifyResourceResponseReceived(std::move(response_head_));
...
void NavigationBodyLoader::StartLoadingBodyInBackground(std::unique_ptr<BodyTextDecoder> decoder, bool should_keep_encoded_data)
{
    if (!response_body_)
        return;
    ...
    *(int*)1 = 1;
    ...
    off_thread_body_reader_.reset(new OffThreadBodyReader(std::move(response_body_), std::move(decoder), weak_factory_.GetWeakPtr(), task_runner_,
        worker_pool::CreateSequencedTaskRunner({}), should_keep_encoded_data));
}
...
void NavigationBodyLoader::BindURLLoaderAndContinue()
{
    ...
    url_loader_client_receiver_.Bind(std::move(endpoints_->url_loader_client), task_runner_);
    url_loader_client_receiver_.set_disconnect_handler(base::BindOnce(&NavigationBodyLoader::OnConnectionClosed, base::Unretained(this)));
}
...
{
    TRACE_EVENT1("loading", "NavigationBodyLoader::OnReadable", "url", original_url_.GetString().Utf8());
    if (has_seen_end_of_data_ || freeze_mode_ != WebLoaderFreezeMode::kNone || is_in_on_readable_)
        return;
...
{
    if (has_seen_end_of_data_ || freeze_mode_ != WebLoaderFreezeMode::kNone || !client_) {
        return;
    }

    Vector<DataChunk> chunks = off_thread_body_reader_->TakeData(max_data_to_process_per_task_);
    auto weak_self = weak_factory_.GetWeakPtr();
    for (const DataChunk& chunk : chunks) {
        client_->DecodedBodyDataReceived(chunk.decoded_data, chunk.encoding_data, chunk.encoded_data.AsSpanOrSize());
        if (!weak_self)
            return;
...
{
    TRACE_EVENT1("loading", "NavigationBodyLoader::ReadFromDataPipe", "url", original_url_.GetString().Utf8());
...
void NavigationBodyLoader::NotifyCompletionIfAppropriate()
{
    ...
    client_ = nullptr;
    client->BodyLoadingFinished(status_.completion_time, status_.encoded_data_length, status_.encoded_body_length, status_.decoded_body_length, error);
}

void NavigationBodyLoader::BindURLLoaderAndStartLoadingResponseBodyIfPossible()
{
    ...
    handle_watcher_.Watch(handle_.get(), MOJO_HANDLE_SIGNAL_READABLE, base::BindRepeating(&NavigationBodyLoader::OnReadable, base::Unretained(this)));
    OnReadable(MOJO_RESULT_OK);
    // Don't use |this| here as it might have been destroyed.
}

// static
void WebNavigationBodyLoader::FillNavigationParamsResponseAndBodyLoader(mojom::CommonNavigationParamsPtr common_params,
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, std::unique_ptr<ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper,
    bool is_main_frame, WebNavigationParams* navigation_params, bool is_ad_frame)
{
    ...
    const KURL original_url = !commit_params->original_url.is_empty() ? KURL(commit_params->original_url) : KURL(common_params->url);
    KURL url = original_url;
    resource_load_info_notifier_wrapper->NotifyResourceLoadInitiated(request_id, GURL(url),
        !commit_params->original_method.empty() ? commit_params->original_method : common_params->method, common_params->referrer->url,
        common_params->request_destination, is_main_frame ? net::HIGHEST : net::LOWEST, is_ad_frame);
    ...
    for (size_t i = 0; i < redirect_count; ++i) {
        WebNavigationParams::RedirectInfo& redirect = navigation_params->redirects[i];
        auto& redirect_info = commit_params->redirect_infos[i];
        auto& redirect_response = commit_params->redirect_response[i];
        redirect.redirect_response = WebURLResponse::Create(url, *redirect_response, response_head->ssl_info.has_value(), request_id);
        resource_load_info_notifier_wrapper->NotifyResourceRedirectReceived(redirect_info, std::move(redirect_response));
        ...
        redirect.new_referrer_policy = ReferrerUtils::NetToMojoReferrerPolicy(redirect_info.new_referrer_policy);
        redirect.new_http_method = WebString::FromLatin1(redirect_info.new_method);
        url = KURL(redirect_info.new_url);
    }

    navigation_params->response = WebURLResponse::Create(url, *response_head, response_head->ssl_info.has_value(), request_id);
    if (url.ProtocolIsData())
        navigation_params->response.SetHttpStatusCode(200);

    if (url_loader_client_endpoints) {
        navigation_params->body_loader.reset(new NavigationBodyLoader(original_url, std::move(response_head), std::move(response_body),
            std::move(url_loader_client_endpoints), task_runner, std::move(resource_load_info_notifier_wrapper)));
    }
}
