...
public:
    ...
    void OnReceivedRedirect(const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr head, base::TimeTicks redirect_ipc_arrival_time) override
    {
    ...
    void OnReceivedResponse(network::mojom::URLResponseHeadPtr head, mojo::ScopedDataPipeConsumerHandle body,
        std::optional<mojo_base::BigBuffer> cached_metadata, base::TimeTicks response_ipc_arrival_time) override
    {
        EXPECT_FALSE(context_->cancelled);
    ...
    void OnRequestComplete(const network::URLLoaderCompletionStatus& status, base::TimeTicks complete_ipc_arrival_time) override
    {
        if (context_->cancelled)
            return;
        EXPECT_TRUE(context_->received_response);
...
std::string GetRequestPeerContextBody(MockResourceRequestSender::Context* context)
{
...
class WebMojoURLLoaderClientTest : public ::testing::Test, public network::mojom::URLLoaderFactory, public ::testing::WithParamInterface<bool> {
protected:
    WebMojoURLLoaderClientTest()
        : resource_request_sender_(new MockResourceRequestSender())
    {
        if (DeferWithBackForwardCacheEnabled()) {
            scoped_feature_list_.InitAndEnableFeature(blink::features::kLoadingTasksUnfreezable);
        }

        WebRuntimeFeatures::EnableBackForwardCache(DeferWithBackForwardCacheEnabled());

        auto url_loader_factory = base::MakeRefCounted<network::WeakWrapperSharedURLLoaderFactory>(this);
        auto request = std::make_unique<network::ResourceRequest>();
        auto loading_task_runner = blink::scheduler::GetSingleThreadTaskRunnerForTesting();

        client_ = std::make_unique<MojoURLLoaderClient>(resource_request_sender_.get(), loading_task_runner, url_loader_factory->BypassRedirectChecks(),
            request->url,
        ...
        url_loader_
            = ThrottlingURLLoader::CreateLoaderAndStart(std::move(url_loader_factory), std::vector<std::unique_ptr<blink::URLLoaderThrottle>>(), request_id_,
                /*loader_options=0*/ 0, request.get(), client_.get(), TRAFFIC_ANNOTATION_FOR_TESTS, std::move(loading_task_runner),
...
{
    url_loader_client_->OnReceiveResponse(network::mojom::URLResponseHead::New(), mojo::ScopedDataPipeConsumerHandle(), std::nullopt);
...
TEST_P(WebMojoURLLoaderClientTest, ResponseBody)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, mojo::CreateDataPipe(&options, data_pipe_producer, data_pipe_consumer));
    url_loader_client_->OnReceiveResponse(network::mojom::URLResponseHead::New(), std::move(data_pipe_consumer), std::nullopt);
    ...
    MojoResult result = data_pipe_producer->WriteData(base::byte_span_from_cstring("hello"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
...
TEST_P(WebMojoURLLoaderClientTest, OnReceiveRedirect)
{
    ...
    url_loader_client_->OnReceiveRedirect(redirect_info, network::mojom::URLResponseHead::New());
...
TEST_P(WebMojoURLLoaderClientTest, OnReceiveResponseWithCachedMetadata)
{
    ...
    url_loader_client_->OnReceiveResponse(network::mojom::URLResponseHead::New(), mojo::ScopedDataPipeConsumerHandle(), std::move(metadata));
...
TEST_P(WebMojoURLLoaderClientTest, DeferWithResponseBody)
{
    ...
    size_t actually_written_bytes = 0;
    ASSERT_EQ(MOJO_RESULT_OK, data_pipe_producer->WriteData(base::as_byte_span(msg1), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
...
TEST_P(WebMojoURLLoaderClientTest, DeferredAndDeferredWithBackForwardCacheTransitions)
{
    if (!DeferWithBackForwardCacheEnabled())
        return;
    ...
    ASSERT_EQ(MOJO_RESULT_OK, mojo::CreateDataPipe(nullptr, producer_handle, consumer_handle));
    url_loader_client_->OnReceiveResponse(network::mojom::URLResponseHead::New(), std::move(consumer_handle), std::nullopt);
    base::RunLoop().RunUntilIdle();
    EXPECT_FALSE(context_->received_response);
    EXPECT_FALSE(context_->complete);
    EXPECT_EQ("", GetRequestPeerContextBody(context_));
    ...
    std::string msg1 = "he";
    size_t actually_written_bytes = 0;
    ASSERT_EQ(MOJO_RESULT_OK, producer_handle->WriteData(base::as_byte_span(msg1), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
    EXPECT_EQ(msg1.size(), actually_written_bytes);
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ("", GetRequestPeerContextBody(context_));
    ...
    std::string msg2 = "ll";
    ASSERT_EQ(MOJO_RESULT_OK, producer_handle->WriteData(base::as_byte_span(msg2), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
    ...
    client_->Freeze(LoaderFreezeMode::kBufferIncoming);
    std::string msg3 = "o";
    ASSERT_EQ(MOJO_RESULT_OK, producer_handle->WriteData(base::as_byte_span(msg3), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
    ...
    ASSERT_EQ(MOJO_RESULT_OK, producer_handle->WriteData(base::as_byte_span(msg4), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
...
TEST_P(WebMojoURLLoaderClientTest, DeferredWithBackForwardCacheStoppedDeferringBeforeClosing)
{
    if (!DeferWithBackForwardCacheEnabled())
        return;
...
TEST_P(WebMojoURLLoaderClientTest, DeferredWithBackForwardCacheLongResponseBody)
{
    if (!DeferWithBackForwardCacheEnabled())
        return;
    // Call OnReceiveResponse, OnComplete while deferred.
    ...
    while (bytes_remaining > 0) {
        ...
        size_t actually_written_bytes = 0;
        MojoResult result = producer_handle->WriteData(bytes, MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
...
TEST_P(WebMojoURLLoaderClientTest, SetDeferredDuringFlushingDeferredMessageOnTransferSizeUpdated)
{
