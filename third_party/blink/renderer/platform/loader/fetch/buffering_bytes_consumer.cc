...
// static
BufferingBytesConsumer* BufferingBytesConsumer::CreateWithDelay(BytesConsumer* bytes_consumer, scoped_refptr<base::SingleThreadTaskRunner> timer_task_runner)
{
    return MakeGarbageCollected<BufferingBytesConsumer>(
        base::PassKey<BufferingBytesConsumer>(), bytes_consumer, std::move(timer_task_runner), base::Milliseconds(kDelayMilliseconds));
}

// static
BufferingBytesConsumer* BufferingBytesConsumer::Create(BytesConsumer* bytes_consumer)
{
    return MakeGarbageCollected<BufferingBytesConsumer>(base::PassKey<BufferingBytesConsumer>(), bytes_consumer, nullptr, base::TimeDelta());
}

BufferingBytesConsumer::BufferingBytesConsumer(base::PassKey<BufferingBytesConsumer> key, BytesConsumer* bytes_consumer,
    scoped_refptr<base::SingleThreadTaskRunner> timer_task_runner, base::TimeDelta buffering_start_delay)
    : bytes_consumer_(bytes_consumer)
    , timer_(std::move(timer_task_runner), this, &BufferingBytesConsumer::OnTimerFired)
    , is_limiting_total_buffer_size_(RuntimeEnabledFeatures::BufferedBytesConsumerLimitSizeEnabled())
{
...
BytesConsumer::Result BufferingBytesConsumer::BeginRead(base::span<const char>& buffer)
{
...
BytesConsumer::Result BufferingBytesConsumer::EndRead(size_t read_size)
{
    ...
    if (offset_for_first_chunk_ == first_chunk->size()) {
        const bool was_waiting_for_capacity = is_limiting_total_buffer_size_ && !has_seen_end_of_data_ && total_buffer_size_ >= kMaxBufferSize;
...
scoped_refptr<BlobDataHandle> BufferingBytesConsumer::DrainAsBlobDataHandle(BlobSizePolicy policy)
{
...
void BufferingBytesConsumer::BufferData()
{
    ...
    while (!is_limiting_total_buffer_size_ || total_buffer_size_ < kMaxBufferSize) {
