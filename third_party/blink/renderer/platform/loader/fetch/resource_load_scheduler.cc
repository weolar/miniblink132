...
constexpr char kRendererSideResourceScheduler[] = "RendererSideResourceScheduler";
...
constexpr char kTightLimitForRendererSideResourceSchedulerName[] = "tight_limit";
...
uint32_t GetFieldTrialUint32Param(const char* trial_name, const char* parameter_name, uint32_t default_param)
{
...
constexpr ResourceLoadScheduler::ClientId ResourceLoadScheduler::kInvalidClientId;

ResourceLoadScheduler::ResourceLoadScheduler(ThrottlingPolicy initial_throttling_policy, ThrottleOptionOverride throttle_option_override,
    const DetachableResourceFetcherProperties& resource_fetcher_properties, FrameOrWorkerScheduler* frame_or_worker_scheduler,
    DetachableConsoleLogger& console_logger, LoadingBehaviorObserver* loading_behavior_observer)
    : resource_fetcher_properties_(resource_fetcher_properties)
    , policy_(initial_throttling_policy)
    , outstanding_limit_for_throttled_frame_scheduler_(resource_fetcher_properties_->GetOutstandingThrottledLimit())
    ...
    , loading_behavior_observer_(loading_behavior_observer)
{
    ...
    normal_outstanding_limit_
        = GetFieldTrialUint32Param(kRendererSideResourceScheduler, kLimitForRendererSideResourceSchedulerName, kLimitForRendererSideResourceScheduler);
    tight_outstanding_limit_ = GetFieldTrialUint32Param(
        kRendererSideResourceScheduler, kTightLimitForRendererSideResourceSchedulerName, kTightLimitForRendererSideResourceScheduler);
    ...
    scheduler_observer_handle_ = frame_or_worker_scheduler->AddLifecycleObserver(
        FrameScheduler::ObserverType::kLoader, WTF::BindRepeating(&ResourceLoadScheduler::OnLifecycleStateChanged, WrapWeakPersistent(this)));
}
...
void ResourceLoadScheduler::Request(
    ResourceLoadSchedulerClient* client, ThrottleOption option, ResourceLoadPriority priority, int intra_priority, ResourceLoadScheduler::ClientId* id)
{
    *id = GenerateClientId();
    if (is_shutdown_) {
        return;
    ...
    if (option == ThrottleOption::kStoppable && throttle_option_override_ == ThrottleOptionOverride::kStoppableAsThrottleable) {
    ...
    DCHECK(ThrottleOption::kStoppable == option || ThrottleOption::kThrottleable == option);
    ...
    pending_request_map_.insert(*id, MakeGarbageCollected<ClientInfo>(client, option, priority, intra_priority));
...
void ResourceLoadScheduler::SetPriority(ClientId client_id, ResourceLoadPriority priority, int intra_priority)
{
    ...
    auto it = throttle_option_queue.find(ClientIdWithPriority(client_id, client_it->value->priority, client_it->value->intra_priority));
...
bool ResourceLoadScheduler::Release(
    ResourceLoadScheduler::ClientId id, ResourceLoadScheduler::ReleaseOption option, const ResourceLoadScheduler::TrafficReportHints& hints)
{
...
void ResourceLoadScheduler::SetOutstandingLimitForTesting(size_t tight_limit, size_t normal_limit, size_t tight_medium_limit)
{
...
void ResourceLoadScheduler::OnLifecycleStateChanged(scheduler::SchedulingLifecycleState state)
{
...
bool ResourceLoadScheduler::IsPendingRequestEffectivelyEmpty(ThrottleOption option)
{
...
bool ResourceLoadScheduler::GetNextPendingRequest(ClientId* id)
{
    ...
    bool has_runnable_stoppable_request = stoppable_it != stoppable_queue.end()
        && (!IsClientDelayable(ThrottleOption::kStoppable)
            || IsRunningThrottleableRequestsLessThanOutStandingLimit(GetOutstandingLimit(stoppable_it->priority), stoppable_it->priority));

    auto throttleable_it = throttleable_queue.begin();
    bool has_runnable_throttleable_request = throttleable_it != throttleable_queue.end()
        && (!IsClientDelayable(ThrottleOption::kThrottleable)
            || IsRunningThrottleableRequestsLessThanOutStandingLimit(GetOutstandingLimit(throttleable_it->priority), throttleable_it->priority));
    ...
    bool use_stoppable = has_runnable_stoppable_request && (!has_runnable_throttleable_request || compare(*stoppable_it, *throttleable_it));
...
void ResourceLoadScheduler::Run(ResourceLoadScheduler::ClientId id, ResourceLoadSchedulerClient* client, bool throttleable, ResourceLoadPriority priority)
{
...
size_t ResourceLoadScheduler::GetOutstandingLimit(ResourceLoadPriority priority) const
{
    ...
    switch (policy_) {
    case ThrottlingPolicy::kTight:
        limit = std::min(limit, priority < ResourceLoadPriority::kHigh ? tight_outstanding_limit_ : normal_outstanding_limit_);
        break;
    case ThrottlingPolicy::kNormal:
...
void ResourceLoadScheduler::ShowConsoleMessageIfNeeded()
{
    ...
    if ((pending_queue_update_times_[ThrottleOption::kThrottleable] >= limit || IsPendingRequestEffectivelyEmpty(ThrottleOption::kThrottleable))
        && (pending_queue_update_times_[ThrottleOption::kStoppable] >= limit || IsPendingRequestEffectivelyEmpty(ThrottleOption::kStoppable))) {
    ...
    console_logger_->AddConsoleMessage(mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kInfo,
...
bool ResourceLoadScheduler::IsRunningThrottleableRequestsLessThanOutStandingLimit(size_t out_standing_limit, ResourceLoadPriority priority)
{
    ...
    if (priority == ResourceLoadPriority::kMedium && running_medium_requests_.size() < tight_medium_limit_) {
        return true;
    }
