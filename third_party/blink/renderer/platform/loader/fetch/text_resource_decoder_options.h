...
public:
    ...
    // - Else, Latin-1.
    explicit TextResourceDecoderOptions(ContentType, const WTF::TextEncoding& default_encoding = WTF::TextEncoding());
    ...
    static TextResourceDecoderOptions CreateWithAutoDetection(
        ContentType, const WTF::TextEncoding& default_encoding, const WTF::TextEncoding& hint_encoding, const KURL& hint_url);

    void SetUseLenientXMLDecoding()
    {
        use_lenient_xml_decoding_ = true;
    }
    ...
    const WTF::TextEncoding& DefaultEncoding() const
    {
        return default_encoding_;
    }
    bool GetNoBOMDecoding() const
    {
        return no_bom_decoding_;
    }
    bool GetUseLenientXMLDecoding() const
    {
        return use_lenient_xml_decoding_;
    }

    const AtomicString& HintEncoding() const
    {
        return hint_encoding_;
    }
    const KURL& HintURL() const
    {
        return hint_url_;
    }
    const char* HintLanguage() const
    {
        return hint_language_.data();
    }

private:
    TextResourceDecoderOptions(
        EncodingDetectionOption, ContentType, const WTF::TextEncoding& default_encoding, const AtomicString& hint_encoding, const KURL& hint_url);
