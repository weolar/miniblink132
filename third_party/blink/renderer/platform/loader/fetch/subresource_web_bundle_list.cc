...
void SubresourceWebBundleList::Remove(SubresourceWebBundle& bundle)
{
    subresource_web_bundles_.erase(
        std::remove_if(subresource_web_bundles_.begin(), subresource_web_bundles_.end(), [&bundle](auto& item) { return item == &bundle; }),
        subresource_web_bundles_.end());
}

SubresourceWebBundle* SubresourceWebBundleList::GetMatchingBundle(const KURL& url) const
{
    for (auto it = subresource_web_bundles_.rbegin(); it != subresource_web_bundles_.rend(); ++it) {
...
SubresourceWebBundle* SubresourceWebBundleList::FindSubresourceWebBundleWhichWillBeReleased(
    const KURL& bundle_url, network::mojom::CredentialsMode credentials_mode) const
{
    for (auto& it : subresource_web_bundles_) {
        if (it->WillBeReleased() && it->GetBundleUrl() == bundle_url && it->GetCredentialsMode() == credentials_mode)
            return it.Get();
    }
    return nullptr;
}
