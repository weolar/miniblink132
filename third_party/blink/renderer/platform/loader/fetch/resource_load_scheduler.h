...
class PLATFORM_EXPORT ResourceLoadSchedulerClient : public GarbageCollectedMixin {
public:
...
class PLATFORM_EXPORT ResourceLoadScheduler final : public GarbageCollected<ResourceLoadScheduler> {
public:
    ...
    static constexpr size_t kOutstandingUnlimited = std::numeric_limits<size_t>::max();

    ResourceLoadScheduler(ThrottlingPolicy initial_throttling_poilcy, ThrottleOptionOverride throttle_option_override,
        const DetachableResourceFetcherProperties&, FrameOrWorkerScheduler*, DetachableConsoleLogger& console_logger,
    ...
    void Request(ResourceLoadSchedulerClient*, ThrottleOption, ResourceLoadPriority, int intra_priority, ClientId*);
    ...
    bool IsRunning(ClientId id)
    {
        return running_requests_.Contains(id);
    }
    ...
    void SetOutstandingLimitForTesting(size_t tight_limit, size_t normal_limit, size_t tight_medium_limit = 0);
    ...
    void SetThrottleOptionOverride(ThrottleOptionOverride throttle_option_override)
    {
...
private:
    ...
    public:
        struct Compare {
            bool operator()(const ClientIdWithPriority& x, const ClientIdWithPriority& y) const
            {
        ...
        ClientIdWithPriority(ClientId client_id, WebURLRequest::Priority priority, int intra_priority)
            : client_id(client_id)
            , priority(priority)
    ...
    struct ClientInfo : public GarbageCollected<ClientInfo> {
        ClientInfo(ResourceLoadSchedulerClient* client, ThrottleOption option, ResourceLoadPriority priority, int intra_priority)
            : client(client)
            , option(option)
            , priority(priority)
            , intra_priority(intra_priority)
    ...
    void Run(ClientId, ResourceLoadSchedulerClient*, bool throttleable, ResourceLoadPriority priority);

    size_t GetOutstandingLimit(ResourceLoadPriority priority) const;

    void ShowConsoleMessageIfNeeded();

    bool IsRunningThrottleableRequestsLessThanOutStandingLimit(size_t out_standing_limit, ResourceLoadPriority priority);

    const Member<const DetachableResourceFetcherProperties> resource_fetcher_properties_;
    ...
    scheduler::SchedulingLifecycleState frame_scheduler_lifecycle_state_ = scheduler::SchedulingLifecycleState::kNotThrottled;
    ...
    std::map<ThrottleOption, std::set<ClientIdWithPriority, ClientIdWithPriority::Compare>> pending_requests_ ALLOW_DISCOURAGED_TYPE("TODO(crbug.com/1404327)");

    // Remembers elapsed times in seconds when the top request in each queue is
    // processed.
    std::map<ThrottleOption, base::Time> pending_queue_update_times_ ALLOW_DISCOURAGED_TYPE("TODO(crbug.com/1404327)");

    // Handle to throttling observer.
    std::unique_ptr<FrameOrWorkerScheduler::LifecycleObserverHandle> scheduler_observer_handle_;
