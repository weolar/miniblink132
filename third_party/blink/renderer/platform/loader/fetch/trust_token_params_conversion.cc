...
network::OptionalTrustTokenParams ConvertTrustTokenParams(const std::optional<network::mojom::blink::TrustTokenParams>& maybe_in)
{
    if (!maybe_in)
        return std::nullopt;
    const network::mojom::blink::TrustTokenParams& in = *maybe_in;

    network::mojom::TrustTokenParamsPtr out = network::mojom::TrustTokenParams::New();
    ...
    if (!in.possibly_unsafe_additional_signing_data.IsNull()) {
        out->possibly_unsafe_additional_signing_data = in.possibly_unsafe_additional_signing_data.Utf8();
    }
