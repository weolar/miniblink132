...
// static
WTF::Vector<network::DataElement> StructTraits<blink::mojom::FetchAPIRequestBodyDataView, blink::ResourceRequestBody>::elements(
    blink::ResourceRequestBody& mutable_body)
{
    ...
    if (auto form_body = mutable_body.FormBody()) {
        DUMP_WILL_BE_CHECK_NE(blink::EncodedFormData::FormDataType::kInvalid, form_body->GetType());
        // Here we need to keep the original body, because other members such as
        // `identifier` are on the form body.
        network_body = NetworkResourceRequestBodyFor(blink::ResourceRequestBody(form_body));
...
// static
bool StructTraits<blink::mojom::FetchAPIRequestBodyDataView, blink::ResourceRequestBody>::Read(
    blink::mojom::FetchAPIRequestBodyDataView in, blink::ResourceRequestBody* out)
{
    if (in.is_null()) {
    ...
    if (elements_view.size() == 1) {
        ...
        if (view.tag() == network::DataElement::Tag::kChunkedDataPipe) {
            ...
            auto& chunked_data_pipe = element.As<network::DataElementChunkedDataPipe>();
            *out = blink::ResourceRequestBody(blink::ToCrossVariantMojoType(chunked_data_pipe.ReleaseChunkedDataPipeGetter()));
            return true;
        }
    }
    ...
    for (size_t i = 0; i < elements_view.size(); ++i) {
        ...
        case network::DataElement::Tag::kBytes: {
            ...
            form_data->AppendData(bytes.bytes().data(), base::checked_cast<wtf_size_t>(bytes.bytes().size()));
            break;
        }
        case network::DataElement::Tag::kFile: {
            ...
            form_data->AppendFileRange(blink::FilePathToString(file.path()), file.offset(), file.length(), expected_modification_time);
            break;
        }
        case network::DataElement::Tag::kDataPipe: {
            auto& datapipe = element.As<network::DataElementDataPipe>();
            form_data->AppendDataPipe(base::MakeRefCounted<blink::WrappedDataPipeGetter>(blink::ToCrossVariantMojoType(datapipe.ReleaseDataPipeGetter())));
            break;
        }
    ...
    DUMP_WILL_BE_CHECK_NE(blink::EncodedFormData::FormDataType::kInvalid, form_data->GetType());
    form_data->identifier_ = in.identifier();
    form_data->contains_password_data_ = in.contains_sensitive_info();
    form_data->SetBoundary(blink::FormDataEncoder::GenerateUniqueBoundaryString());
