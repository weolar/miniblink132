...
class PLATFORM_EXPORT ResourceFetcherProperties : public GarbageCollected<ResourceFetcherProperties> {
public:
    ...
    virtual const FetchClientSettingsObject& GetFetchClientSettingsObject() const = 0;
    ...
    // handler.
    virtual ControllerServiceWorkerMode GetControllerServiceWorkerMode() const = 0;
...
class PLATFORM_EXPORT DetachableResourceFetcherProperties final : public ResourceFetcherProperties {
public:
    explicit DetachableResourceFetcherProperties(const ResourceFetcherProperties& properties)
    ...
    const FetchClientSettingsObject& GetFetchClientSettingsObject() const override
    {
        return properties_ ? properties_->GetFetchClientSettingsObject() : *fetch_client_settings_object_;
    }
    bool IsOutermostMainFrame() const override
    {
        return properties_ ? properties_->IsOutermostMainFrame() : is_outermost_main_frame_;
    }
    ControllerServiceWorkerMode GetControllerServiceWorkerMode() const override
    {
        return properties_ ? properties_->GetControllerServiceWorkerMode() : ControllerServiceWorkerMode::kNoController;
    }
    ...
    {
        return properties_ ? properties_->IsSubframeDeprioritizationEnabled() : is_subframe_deprioritization_enabled_;
    }

    scheduler::FrameStatus GetFrameStatus() const override
    {
        return properties_ ? properties_->GetFrameStatus() : scheduler::FrameStatus::kNone;
    }

    int GetOutstandingThrottledLimit() const override
    {
        return properties_ ? properties_->GetOutstandingThrottledLimit() : outstanding_throttled_limit_;
    }
