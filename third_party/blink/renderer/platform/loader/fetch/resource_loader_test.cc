...
class MockUseCounter : public GarbageCollected<MockUseCounter>, public UseCounter {
public:
...
public:
    ...
    ResourceLoaderTest()
        : foo_url_("https://foo.test")
        , bar_url_("https://bar.test")
    {
    }
...
protected:
    ...
    class NoopLoaderFactory final : public ResourceFetcher::LoaderFactory {
        std::unique_ptr<URLLoader> CreateURLLoader(const network::ResourceRequest& request, const ResourceLoaderOptions& options,
            ...
            const std::optional<base::UnguessableToken>& service_worker_race_network_request_token, bool is_from_origin_dirty_style_sheet) override
        {
    ...
    ResourceFetcher* MakeResourceFetcher(TestResourceFetcherProperties* properties, FetchContext* context)
    {
        ResourceFetcherInit init(properties->MakeDetachable(), context, CreateTaskRunner(), CreateTaskRunner(), MakeGarbageCollected<NoopLoaderFactory>(),
    ...
    MockUseCounter* UseCounter() const
    {
        return use_counter_;
    }
...
TEST_F(ResourceLoaderTest, LoadResponseBody)
{
    ...
    loader->DidReceiveResponse(WrappedResourceResponse(response), std::move(consumer),
        /*cached_metadata=*/std::nullopt);
    loader->DidFinishLoading(base::TimeTicks(), 0, 0, 0);

    size_t actually_written_bytes = 0;
    result = producer->WriteData(base::byte_span_from_cstring("he"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
    ASSERT_EQ(result, MOJO_RESULT_OK);
    ASSERT_EQ(actually_written_bytes, 2u);

    static_cast<scheduler::FakeTaskRunner*>(fetcher->GetTaskRunner().get())->RunUntilIdle();

    result = producer->WriteData(base::byte_span_from_cstring("llo"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
    ASSERT_EQ(result, MOJO_RESULT_OK);
...
class TestRawResourceClient final : public GarbageCollected<TestRawResourceClient>, public RawResourceClient {
public:
    TestRawResourceClient() = default;

    // Implements RawResourceClient.
    void ResponseBodyReceived(Resource* resource, BytesConsumer& bytes_consumer) override
    {
        body_ = &bytes_consumer;
    }
    String DebugName() const override
    {
        return "TestRawResourceClient";
    }
...
TEST_F(ResourceLoaderTest, LoadDataURL_AsyncAndStream)
{
    ...
    auto* test_reader = MakeGarbageCollected<BytesConsumerTestReader>(raw_resource_client->body());
...
TEST_F(ResourceLoaderTest, LoadDataURL_Sync)
{
    ...
    request.SetRequestContext(mojom::blink::RequestContextType::FETCH);
    FetchParameters params = FetchParameters::CreateForTest(std::move(request));
    Resource* resource = RawResource::FetchSynchronously(params, fetcher, nullptr);
...
bool WillFollowRedirect(ResourceLoader* loader, KURL new_url)
{
    ...
    auto response = WebURLResponse::Create(new_url, *response_head,
        /*report_security_info=*/true, /*request_id=*/1);
    bool has_devtools_request_id = false;
    std::vector<std::string> removed_headers;
    net::HttpRequestHeaders modified_headers;
    return loader->WillFollowRedirect(new_url, net::SiteForCookies(), /*new_referrer=*/String(), network::mojom::ReferrerPolicy::kAlways, "GET", response,
...
TEST_F(ResourceLoaderTest, AuthorizationCrossOriginRedirect)
{
    ...
    ResourceRequest request(url);
    request.SetRequestContext(mojom::blink::RequestContextType::FETCH);
    request.SetHttpHeaderField(http_names::kAuthorization, AtomicString("Basic foo"));
    ...
    {
        EXPECT_CALL(*UseCounter(), CountUse(mojom::WebFeature::kAuthorizationCrossOrigin)).Times(1);
...
class ResourceLoaderSubresourceFilterCnameAliasTest : public ResourceLoaderTest {
public:
    ...
    void SetUp() override
    {
        feature_list_.InitAndEnableFeature(features::kSendCnameAliasesToSubresourceFilterFromRenderer);
        ResourceLoaderTest::SetUp();
    }

    void SetMockSubresourceFilterBlockLists(Vector<String> blocked_urls, Vector<String> tagged_urls)
    {
...
protected:
    FetchContext* MakeFetchContextWithMockSubresourceFilter(Vector<String> blocked_urls, Vector<String> tagged_urls)
    {
    ...
    {
        auto* properties = MakeGarbageCollected<TestResourceFetcherProperties>();
        FetchContext* context = MakeFetchContextWithMockSubresourceFilter(blocked_urls_, tagged_urls_);
    ...
    void ExpectCnameAliasInfoMatching(CnameAliasInfoForTesting info, ResourceLoader* loader)
    {
        EXPECT_EQ(loader->cname_alias_info_for_testing_.has_aliases, info.has_aliases);

        if (info.has_aliases) {
            EXPECT_EQ(loader->cname_alias_info_for_testing_.was_ad_tagged_based_on_alias, info.was_ad_tagged_based_on_alias);
            EXPECT_EQ(loader->cname_alias_info_for_testing_.was_blocked_based_on_alias, info.was_blocked_based_on_alias);
            EXPECT_EQ(loader->cname_alias_info_for_testing_.list_length, info.list_length);
            EXPECT_EQ(loader->cname_alias_info_for_testing_.invalid_count, info.invalid_count);
            EXPECT_EQ(loader->cname_alias_info_for_testing_.redundant_count, info.redundant_count);
        }
    }
...
TEST_F(ResourceLoaderSubresourceFilterCnameAliasTest, DnsAliasesCheckedBySubresourceFilterDisallowed_TaggedAndBlocked)
{
...
TEST_F(ResourceLoaderSubresourceFilterCnameAliasTest, DnsAliasesCheckedBySubresourceFilterDisallowed_BlockedOnly)
{
    // Set the blocklists: the first for blocking, the second for ad-tagging.
    Vector<String> blocked_urls = { "https://bad-ad.com/some_path.html" };
...
TEST_F(ResourceLoaderSubresourceFilterCnameAliasTest, DnsAliasesCheckedBySubresourceFilterDisallowed_TaggedOnly)
{
    // Set the blocklists: the first for blocking, the second for ad-tagging.
...
TEST_F(ResourceLoaderSubresourceFilterCnameAliasTest, DnsAliasesCheckedBySubresourceFilterAllowed_NotBlockedOrTagged)
{
    // Set the blocklists: the first for blocking, the second for ad-tagging.
    Vector<String> blocked_urls = {};
    Vector<String> tagged_urls = {};
    ...
    // Set the CNAME aliases.
    Vector<String> aliases({ "non-ad.com", "?", "alias3.com", "not-an-ad.com", "www.example.com" });
    response.SetDnsAliases(aliases);
...
TEST_F(ResourceLoaderSubresourceFilterCnameAliasTest, DnsAliasesCheckedBySubresourceFilterNoAliases_NoneDetected)
{
    // Set the blocklists: the first for blocking, the second for ad-tagging.
    Vector<String> blocked_urls = {};
    Vector<String> tagged_urls = {};
