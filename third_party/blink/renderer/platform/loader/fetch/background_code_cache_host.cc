...
BackgroundCodeCacheHost::BackgroundCodeCacheHost(mojo::PendingRemote<mojom::blink::CodeCacheHost> pending_remote)
...
CodeCacheHost& BackgroundCodeCacheHost::GetCodeCacheHost(scoped_refptr<base::SequencedTaskRunner> background_task_runner)
{
    CHECK(background_task_runner->RunsTasksInCurrentSequence());
    if (!code_cache_host_) {
        CHECK(pending_remote_);
        CHECK(!background_task_runner_);
        code_cache_host_ = std::make_unique<CodeCacheHost>(mojo::Remote<mojom::blink::CodeCacheHost>(std::move(pending_remote_)));
