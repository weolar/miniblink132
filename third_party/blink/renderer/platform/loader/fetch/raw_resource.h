...
public:
    static RawResource* FetchSynchronously(FetchParameters&, ResourceFetcher*, RawResourceClient* = nullptr);
    static RawResource* Fetch(FetchParameters&, ResourceFetcher*, RawResourceClient*);
    static RawResource* FetchMedia(FetchParameters&, ResourceFetcher*, RawResourceClient*);
    static RawResource* FetchTextTrack(FetchParameters&, ResourceFetcher*, RawResourceClient*);
    static RawResource* FetchManifest(FetchParameters&, ResourceFetcher*, RawResourceClient*);

    // Exposed for testing
    static RawResource* CreateForTest(const ResourceRequest& request, ResourceType type)
    {
        ResourceLoaderOptions options(nullptr /* world */);
        return MakeGarbageCollected<RawResource>(request, type, options);
    }
    static RawResource* CreateForTest(const KURL& url, scoped_refptr<const SecurityOrigin> origin, ResourceType type)
    {
    ...
    RawResource(const ResourceRequest&, ResourceType, const ResourceLoaderOptions&);
...
private:
    ...
    void AppendData(absl::variant<SegmentedBuffer, base::span<const char>>) override;
    ...
    void ResponseBodyReceived(ResponseBodyLoaderDrainableInterface&, scoped_refptr<base::SingleThreadTaskRunner> loader_task_runner) override;
...
{
    return type == ResourceType::kRaw || type == ResourceType::kTextTrack || type == ResourceType::kAudio || type == ResourceType::kVideo
...
public:
    bool IsRawResourceClient() const final
    {
        return true;
    }
    ...
    virtual void DataSent(Resource*, uint64_t /* bytesSent */, uint64_t /* totalBytesToBeSent */)
    {
    }
