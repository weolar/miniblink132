...
ResourceResponse CreateTestResponse()
{
    ...
    response.AddHttpHeaderField(http_names::kCacheControl, AtomicString("no-cache"));
    response.AddHttpHeaderField(http_names::kDate, AtomicString("Tue, 17 Jan 2017 04:01:00 GMT"));
    response.AddHttpHeaderField(http_names::kExpires, AtomicString("Tue, 17 Jan 2017 04:11:00 GMT"));
    response.AddHttpHeaderField(http_names::kLastModified, AtomicString("Tue, 17 Jan 2017 04:00:00 GMT"));
    ...
    response.AddHttpHeaderField(http_names::kContentDisposition, AtomicString("attachment; filename=a.txt"));
    return response;
}
...
TEST(ResourceResponseTest, AddHttpHeaderFieldWithMultipleValues)
{
    ...
    response.AddHttpHeaderFieldWithMultipleValues(http_names::kLowerSetCookie, empty_values);
    EXPECT_EQ(AtomicString(), response.HttpHeaderField(http_names::kLowerSetCookie));
    ...
    response.AddHttpHeaderFieldWithMultipleValues(http_names::kLowerSetCookie, values);

    EXPECT_EQ("a=1, b=2, c=3", response.HttpHeaderField(http_names::kLowerSetCookie));
}
...
TEST(ResourceResponseTest, TreatExpiresZeroAsExpired)
{
    ...
    std::optional<base::Time> expires = response.Expires(*MakeGarbageCollected<FakeUseCounter>());
