...
class ResponseBodyLoader::DelegatingBytesConsumer final : public BytesConsumer, public BytesConsumer::Client {
public:
    DelegatingBytesConsumer(BytesConsumer& bytes_consumer, ResponseBodyLoader& loader, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
            ...
            if (buffer.empty()) {
                ...
                if (result == Result::kOk) {
                    ...
                    if (in_on_state_change_) {
                        ...
                        task_runner_->PostTask(FROM_HERE, base::BindOnce(&DelegatingBytesConsumer::OnStateChange, WrapPersistent(this)));
                    }
                }
            }
        }
    ...
    scoped_refptr<BlobDataHandle> DrainAsBlobDataHandle(BlobSizePolicy policy) override
    {
        if (loader_->IsAborted()) {
            return nullptr;
        }
    ...
    {
        bytes_consumer_client_ = nullptr;
    }
    void Cancel() override
    {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&DelegatingBytesConsumer::CancelSync, WrapWeakPersistent(this)));
    }
    ...
    void OnStateChange() override
    {
        ...
        base::AutoReset<bool> auto_reset_for_in_on_state_change(&in_on_state_change_, true);
        base::AutoReset<bool> auto_reset_for_has_pending_state_change_signal(&has_pending_state_change_signal_, false);
        base::AutoReset<bool> auto_reset_for_waiting_for_lookahead_bytes(&waiting_for_lookahead_bytes_, false);
        ...
        if (loader_->IsAborted() || IsSuspendedButNotForBackForwardCache() || state_ == State::kCancelled) {
            return;
        }
        ...
        while (state_ == State::kLoading) {
            ...
            if ((result == Result::kOk || result == Result::kShouldWait) && lookahead_bytes_ == 0) {
    ...
    void HandleResult(Result result)
    {
        ...
        if (result == Result::kDone) {
            ...
            if (in_on_state_change_) {
                ...
                task_runner_->PostTask(FROM_HERE, base::BindOnce(&ResponseBodyLoader::DidFinishLoadingBody, WrapWeakPersistent(loader_.Get())));
            }
        }
        ...
        if (result == Result::kError) {
            ...
            if (in_on_state_change_) {
                ...
                task_runner_->PostTask(FROM_HERE, base::BindOnce(&ResponseBodyLoader::DidFailLoadingBody, WrapWeakPersistent(loader_.Get())));
            }
        }
    }
...
class ResponseBodyLoader::Buffer final : public GarbageCollected<ResponseBodyLoader::Buffer> {
public:
    ...
    {
        TRACE_EVENT2("loading", "ResponseBodyLoader::Buffer::AddChunk", "total_bytes_read", static_cast<int>(total_bytes_read_), "added_bytes",
    ...
    size_t DispatchChunk(size_t max_chunk_size)
    {
        ...
        base::span<const char> span(current_chunk.data(), current_chunk.size());
...
ResponseBodyLoader::ResponseBodyLoader(BytesConsumer& bytes_consumer, ResponseBodyLoaderClient& client, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
...
mojo::ScopedDataPipeConsumerHandle ResponseBodyLoader::DrainAsDataPipe(ResponseBodyLoaderClient** client)
{
    DCHECK(!started_);
...
BytesConsumer& ResponseBodyLoader::DrainAsBytesConsumer()
{
    ...
    delegating_bytes_consumer_ = MakeGarbageCollected<DelegatingBytesConsumer>(*bytes_consumer_, *this, task_runner_);
...
void ResponseBodyLoader::DidReceiveData(base::span<const char> data)
{
    ...
    if (IsSuspendedForBackForwardCache()) {
        ...
        if (!BackForwardCacheBufferLimitTracker::Get().IsUnderPerProcessBufferLimit()) {
            EvictFromBackForwardCache(mojom::blink::RendererEvictionReason::kNetworkExceedsBufferLimit);
        }
    }
...
void ResponseBodyLoader::DidReceiveDecodedData(const String& data, std::unique_ptr<ParkableStringImpl::SecureDigest> digest)
{
    if (aborted_)
        return;
...
void ResponseBodyLoader::EvictFromBackForwardCache(mojom::blink::RendererEvictionReason reason)
{
...
void ResponseBodyLoader::DidBufferLoadWhileInBackForwardCache(size_t num_bytes)
{
    if (!back_forward_cache_loader_helper_)
        return;
...
void ResponseBodyLoader::Suspend(LoaderFreezeMode mode)
{
    ...
    if (IsSuspendedForBackForwardCache()) {
        ...
        if (was_suspended) {
            task_runner_->PostTask(FROM_HERE, base::BindOnce(&ResponseBodyLoader::OnStateChange, WrapPersistent(this)));
        }
    }
}
...
void ResponseBodyLoader::EvictFromBackForwardCacheIfDrainedAsBytesConsumer()
{
    if (drained_as_bytes_consumer_) {
        if (!base::FeatureList::IsEnabled(features::kAllowDatapipeDrainedAsBytesConsumerInBFCache)) {
            EvictFromBackForwardCache(mojom::blink::RendererEvictionReason::kNetworkRequestDatapipeDrainedAsBytesConsumer);
        }
    }
}
...
void ResponseBodyLoader::Resume()
{
    ...
    if (finish_signal_is_pending_) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&ResponseBodyLoader::DidFinishLoadingBody, WrapPersistent(this)));
    } else if (fail_signal_is_pending_) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&ResponseBodyLoader::DidFailLoadingBody, WrapPersistent(this)));
    } else if (cancel_signal_is_pending_) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&ResponseBodyLoader::DidCancelLoadingBody, WrapPersistent(this)));
    } else {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&ResponseBodyLoader::OnStateChange, WrapPersistent(this)));
    }
}
...
void ResponseBodyLoader::OnStateChange()
{
    ...
    while (!aborted_ && (!IsSuspended() || IsSuspendedForBackForwardCache())) {
        ...
        if (!IsSuspended() && body_buffer_ && !body_buffer_->IsEmpty()) {
            ...
            // |bytes_consumer_|.
            num_bytes_consumed += body_buffer_->DispatchChunk(chunk_size - num_bytes_consumed);
            continue;
        }
        ...
        if (result == BytesConsumer::Result::kOk) {
            TRACE_EVENT1("blink", "ResponseBodyLoader::OnStateChange", "available", buffer.size());

            base::AutoReset<bool> auto_reset_for_in_two_phase_read(&in_two_phase_read_, true);
            buffer = buffer.first(std::min(buffer.size(), chunk_size - num_bytes_consumed));
            if (IsSuspendedForBackForwardCache()) {
        ...
        if (IsSuspendedForBackForwardCache() && result != BytesConsumer::Result::kOk) {
