...
SharedBufferBytesConsumer::SharedBufferBytesConsumer(scoped_refptr<const SharedBuffer> data)
    : data_(std::move(data))
    , iterator_(data_->begin())
{
}

BytesConsumer::Result SharedBufferBytesConsumer::BeginRead(base::span<const char>& buffer)
{
