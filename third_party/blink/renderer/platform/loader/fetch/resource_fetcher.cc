...
static constexpr char kCrossDocumentCachedResource[] = "Blink.MemoryCache.CrossDocumentCachedResource2";
...
bool ShouldResourceBeAddedToMemoryCache(const FetchParameters& params, Resource* resource)
{
    return IsMainThread() && params.GetResourceRequest().HttpMethod() == http_names::kGET && params.Options().data_buffering_policy != kDoNotBufferData
...
bool ShouldResourceBeKeptStrongReferenceByType(Resource* resource, const SecurityOrigin* settings_object_origin)
{
    // Image, fonts, stylesheets and scripts are the most commonly reused scripts.

    return resource->GetType() == ResourceType::kScript || resource->GetType() == ResourceType::kFont || resource->GetType() == ResourceType::kCSSStyleSheet
        || resource->GetType() == ResourceType::kMock; // For tests.
}

bool ShouldResourceBeKeptStrongReference(Resource* resource, const SecurityOrigin* settings_object_origin)
{
    return IsMainThread() && resource->IsLoaded() && resource->GetResourceRequest().HttpMethod() == http_names::kGET
        && resource->Options().data_buffering_policy != kDoNotBufferData && ShouldResourceBeKeptStrongReferenceByType(resource, settings_object_origin)
        && !resource->GetResponse().CacheControlContainsNoCache() && !resource->GetResponse().CacheControlContainsNoStore();
}

base::TimeDelta GetResourceStrongReferenceTimeout(Resource* resource, UseCounter& use_counter)
{
...
{
    DEFINE_STATIC_LOCAL(Persistent<ResourceFetcher::ResourceFetcherSet>, fetchers, (MakeGarbageCollected<ResourceFetcher::ResourceFetcherSet>()));
    return *fetchers;
}
...
std::unique_ptr<TracedValue> CreateTracedValueWithPriority(blink::ResourceLoadPriority priority)
{
...
std::unique_ptr<TracedValue> CreateTracedValueForUnusedPreload(const KURL& url, Resource::MatchStatus status, String request_id)
{
    auto value = std::make_unique<TracedValue>();
...
std::unique_ptr<TracedValue> CreateTracedValueForUnusedEarlyHintsPreload(const KURL& url)
{
...
void MaybeRecordBoostImagePriorityReason(const bool is_first_n, const bool is_potentially_lcp_element, const bool is_small_image)
{
    ...
    if (is_first_n && !is_potentially_lcp_element) {
        ...
        reason = is_small_image ? BoostImagePriorityReason::kLcppForSmallImage : BoostImagePriorityReason::kLcppForNotSmallImage;
    } else if (is_first_n && is_potentially_lcp_element) {
    ...
    if (reason) {
        base::UmaHistogramEnumeration("Blink.LCPP.BoostImagePriorityReason", *reason);
    }
}

constexpr char kLCPPDeferUnusedPreloadHistogramPrefix[] = "Blink.LCPP.DeferUnusedPreload.";
...
{
    base::UmaHistogramEnumeration(base::StrCat({ kLCPPDeferUnusedPreloadHistogramPrefix, "DeferredResource" }), resource->GetType());
    base::UmaHistogramEnumeration(
        base::StrCat({ kLCPPDeferUnusedPreloadHistogramPrefix, "DeferredResource.", LinkPreloadStrForHistogram(resource->IsLinkPreload()) }),
        resource->GetType());
    ...
    // versa.
    base::UmaHistogramBoolean(base::StrCat({ kLCPPDeferUnusedPreloadHistogramPrefix, "UnusedAtDeferredLoadTiming" }), resource->StillNeedsLoad());
    base::UmaHistogramBoolean(
        base::StrCat({ kLCPPDeferUnusedPreloadHistogramPrefix, "UnusedAtDeferredLoadTiming.", LinkPreloadStrForHistogram(resource->IsLinkPreload()) }),
    ...
    if (!resource->StillNeedsLoad()) {
        ...
        base::UmaHistogramEnumeration(base::StrCat({ kLCPPDeferUnusedPreloadHistogramPrefix, "PredictionFailed" }), resource->GetType());
        base::UmaHistogramEnumeration(
            base::StrCat({ kLCPPDeferUnusedPreloadHistogramPrefix, "PredictionFailed.", LinkPreloadStrForHistogram(resource->IsLinkPreload()) }),
            resource->GetType());
    }
}

int CompareResourcePriorities(const ResourcePriority& a, const ResourcePriority& b)
{
...
class ResourceFetcher::ResourcePrepareHelper final : public ResourceRequestContext {
    STACK_ALLOCATED();

public:
    ResourcePrepareHelper(ResourceFetcher& fetcher, FetchParameters& params, const ResourceFactory& factory);

    std::optional<ResourceRequestBlockedReason> PrepareRequestForCacheAccess(WebScopedVirtualTimePauser& pauser);
    ...
    ResourceLoadPriority ComputeLoadPriority(const FetchParameters& params) override;
...
ResourceFetcherInit::ResourceFetcherInit(DetachableResourceFetcherProperties& properties, FetchContext* context,
    ...
    ResourceFetcher::LoaderFactory* loader_factory, ContextLifecycleNotifier* context_lifecycle_notifier,
...
mojom::blink::RequestContextType ResourceFetcher::DetermineRequestContext(ResourceType type, IsImageSet is_image_set)
{
    DCHECK((is_image_set == kImageNotImageSet) || (type == ResourceType::kImage && is_image_set == kImageIsImageSet));
    switch (type) {
    case ResourceType::kXSLStyleSheet:
    case ResourceType::kCSSStyleSheet:
...
network::mojom::RequestDestination ResourceFetcher::DetermineRequestDestination(ResourceType type)
{
    switch (type) {
    case ResourceType::kXSLStyleSheet:
    case ResourceType::kCSSStyleSheet:
...
void ResourceFetcher::AddPriorityObserverForTesting(const KURL& resource_url, base::OnceCallback<void(int)> callback, bool new_load_only)
{
    KURL normalized_url = MemoryCache::RemoveFragmentIdentifierIfNeeded(resource_url);

    if (!new_load_only) {
        auto it = cached_resources_map_.find(normalized_url.GetString());
        if (it != cached_resources_map_.end()) {
            Resource* resource = it->value;
            std::move(callback).Run(static_cast<int>(resource->GetResourceRequest().InitialPriority()));
            return;
        }
    }
...
ResourceLoadPriority ResourceFetcher::ComputeLoadPriority(ResourceType type, const ResourceRequestHead& resource_request,
    ResourcePriority::VisibilityStatus visibility, FetchParameters::DeferOption defer_option, FetchParameters::SpeculativePreloadType speculative_preload_type,
    RenderBlockingBehavior render_blocking_behavior, mojom::blink::ScriptType script_type, bool is_link_preload, const std::optional<float> resource_width,
    const std::optional<float> resource_height, bool is_potentially_lcp_element, bool is_potentially_lcp_influencer)
{
    DCHECK(!resource_request.PriorityHasBeenSet() || type == ResourceType::kImage);
    ...
    if (speculative_preload_type == FetchParameters::SpeculativePreloadType::kInDocument && image_fetched_) {
    ...
    if (FetchParameters::kIdleLoad == defer_option) {
    ...
    } else if (type == ResourceType::kCSSStyleSheet && late_document_from_preload_scanner) {
        ...
        priority = ResourceLoadPriority::kVeryLow;
    } else if (resource_request.GetRequestContext() == mojom::blink::RequestContextType::BEACON
        || resource_request.GetRequestContext() == mojom::blink::RequestContextType::PING
        || resource_request.GetRequestContext() == mojom::blink::RequestContextType::CSP_REPORT) {
    ...
    priority = AdjustPriorityWithPriorityHintAndRenderBlocking(priority, type, resource_request.GetFetchPriorityHint(), render_blocking_behavior);

    priority = AdjustImagePriority(
        priority, type, resource_request, speculative_preload_type, is_link_preload, resource_width, resource_height, is_potentially_lcp_element);
    ...
    if (LcppEnabled()) {
        ...
        features::LcppResourceLoadPriority preferred_priority = features::LcppResourceLoadPriority::kMedium;

        if (is_potentially_lcp_element) {
            // Adjust priority of LCP image request.
            if (base::FeatureList::IsEnabled(features::kLCPCriticalPathPredictor) && features::kLCPCriticalPathAdjustImageLoadPriority.Get()) {
                should_modify_request_priority = true;
                preferred_priority = features::kLCPCriticalPathPredictorImageLoadPriority.Get();
            }

            if (base::FeatureList::IsEnabled(features::kLCPScriptObserver) && features::kLCPScriptObserverAdjustImageLoadPriority.Get()) {
                should_modify_request_priority = true;
                preferred_priority = features::kLCPScriptObserverImageLoadPriority.Get();
            }
        }

        if (is_potentially_lcp_influencer && base::FeatureList::IsEnabled(features::kLCPScriptObserver)) {
...
ResourceLoadPriority ResourceFetcher::AdjustImagePriority(const ResourceLoadPriority priority_so_far, const ResourceType type,
    const ResourceRequestHead& resource_request, const FetchParameters::SpeculativePreloadType speculative_preload_type, const bool is_link_preload,
    const std::optional<float> resource_width, const std::optional<float> resource_height, const bool is_potentially_lcp_element)
{
    ...
    if (speculative_preload_type == FetchParameters::SpeculativePreloadType::kInDocument && !is_link_preload && boosted_image_count_ < boosted_image_target_) {
        // Count all candidate images
        if (!is_small_image) {
            ++boosted_image_count_;

            // only boost the priority if one wasn't explicitly set
            if (new_priority < ResourceLoadPriority::kMedium && resource_request.GetFetchPriorityHint() == mojom::blink::FetchPriorityHint::kAuto) {
    ...
    // AdjustImagePriority.
    static const bool kOverrideFirstNBoost = base::FeatureList::IsEnabled(features::kLCPCriticalPathPredictor)
        && features::kLCPCriticalPathAdjustImageLoadPriority.Get() && features::kLCPCriticalPathAdjustImageLoadPriorityOverrideFirstNBoost.Get();
    if (kOverrideFirstNBoost && context_->DoesLCPPHaveLcpElementLocatorHintData()) {
    ...
    if (resource_request.Url().ProtocolIsInHTTPFamily()) {
        MaybeRecordBoostImagePriorityReason(priority_so_far != new_priority, is_potentially_lcp_element, is_small_image);
    }
...
ResourceFetcher::ResourceFetcher(const ResourceFetcherInit& init)
    ...
    , use_counter_(init.use_counter ? init.use_counter : MakeGarbageCollected<DetachableUseCounter>(nullptr))
    , console_logger_(init.console_logger ? init.console_logger : MakeGarbageCollected<DetachableConsoleLogger>())
    , loader_factory_(init.loader_factory)
    , scheduler_(MakeGarbageCollected<ResourceLoadScheduler>(init.initial_throttling_policy, init.throttle_option_override, *properties_,
          init.frame_or_worker_scheduler, *console_logger_, init.loading_behavior_observer))
    , back_forward_cache_loader_helper_(init.back_forward_cache_loader_helper)
    , archive_(init.archive)
    , resource_timing_report_timer_(freezable_task_runner_, this, &ResourceFetcher::ResourceTimingReportTimerFired)
    , frame_or_worker_scheduler_(init.frame_or_worker_scheduler ? init.frame_or_worker_scheduler->GetWeakPtr() : nullptr)
    ...
    , transparent_image_optimization_enabled_(base::FeatureList::IsEnabled(features::kSimplifyLoadingTransparentPlaceholderImage))
...
bool ResourceFetcher::ResourceHasBeenEmulatedLoadStartedForInspector(const KURL& resource_url) const
{
    if (resource_url.IsEmpty()) {
        return false;
    }
...
const HeapHashSet<Member<Resource>> ResourceFetcher::MoveResourceStrongReferences()
{
    document_resource_strong_refs_total_size_ = 0;
    return std::move(document_resource_strong_refs_);
}

mojom::ControllerServiceWorkerMode ResourceFetcher::IsControlledByServiceWorker() const
{
    return properties_->GetControllerServiceWorkerMode();
}

ResourceFetcher::DeferPolicy ResourceFetcher::GetDeferPolicy(ResourceType type, const FetchParameters& params) const
{
    ...
    if (type == ResourceType::kImage
        && (ShouldDeferImageLoad(params.Url()) || params.GetImageRequestBehavior() == FetchParameters::ImageRequestBehavior::kDeferImageLoad)) {
        return DeferPolicy::kDefer;
    }
...
bool ResourceFetcher::ResourceAlreadyLoadStarted(Resource* resource, RevalidationPolicy policy)
{
    return policy == RevalidationPolicy::kUse && resource && !resource->StillNeedsLoad();
}

bool ResourceFetcher::ResourceNeedsLoad(Resource* resource, RevalidationPolicy policy, DeferPolicy defer_policy) const
{
    switch (defer_policy) {
...
void ResourceFetcher::DidLoadResourceFromMemoryCache(
    Resource* resource, const ResourceRequest& request, bool is_static_data, RenderBlockingBehavior render_blocking_behavior)
{
    ...
    if (!is_static_data) {
        MarkEarlyHintConsumedIfNeeded(request.InspectorId(), resource, resource->GetResponse());
    }
    ...
    // `is_static_data`.
    if (!IsSimplifyLoadingTransparentPlaceholderImageEnabled() || (request.GetKnownTransparentPlaceholderImageIndex() == kNotFound)
        || (resource_load_observer_->InterestedInAllRequests())) {
        resource_load_observer_->WillSendRequest(
            request, ResourceResponse() /* redirects */, resource->GetType(), resource->Options(), render_blocking_behavior, resource);
        resource_load_observer_->DidReceiveResponse(
            request.InspectorId(), request, resource->GetResponse(), resource, ResourceLoadObserver::ResponseSource::kFromMemoryCache);
        if (resource->EncodedSize() > 0) {
            resource_load_observer_->DidReceiveData(request.InspectorId(), base::SpanOrSize<const char>(resource->EncodedSize()));
        }
        resource_load_observer_->DidFinishLoading(request.InspectorId(), base::TimeTicks(), 0, resource->GetResponse().DecodedBodyLength());
    }
    ...
    if (!is_static_data) {
        ...
        // they're used.
        KURL initial_url = resource->GetResourceRequest().GetRedirectInfo().has_value() ? resource->GetResourceRequest().GetRedirectInfo()->original_url
                                                                                        : resource->GetResourceRequest().Url();
        mojom::blink::ResourceTimingInfoPtr info = CreateResourceTimingInfo(now, initial_url, &final_response);
        info->response_end = now;
        info->render_blocking_status = render_blocking_behavior == RenderBlockingBehavior::kBlocking;
        ...
        if (resource->GetType() == ResourceType::kImage) {
            ...
            if (ResourceLoadTiming* resource_load_timing = resource->GetResponse().GetResourceLoadTiming()) {
        ...
        AtomicString initiator_type = resource->IsPreloadedByEarlyHints() ? AtomicString(kEarlyHintsInitiatorType) : resource->Options().initiator_info.name;
        // If the fetch originated from user agent CSS we do not emit a resource
        // timing entry.
        if (initiator_type != fetch_initiator_type_names::kUacss) {
            scheduled_resource_timing_reports_.push_back(ScheduledResourceTimingInfo { std::move(info), initiator_type });
...
Resource* ResourceFetcher::CreateResourceForStaticData(const FetchParameters& params, const ResourceFactory& factory)
{
    ...
    const String cache_identifier = GetCacheIdentifier(url, params.GetResourceRequest().GetSkipServiceWorker());
    // Most off-main-thread resource fetches use Resource::kRaw and don't reach
    // this point, but off-main-thread module fetches might.
    if (IsMainThread()) {
        if (Resource* old_resource = MemoryCache::Get()->ResourceForURL(url, cache_identifier)) {
    ...
    if (IsSimplifyLoadingTransparentPlaceholderImageEnabled() && (params.GetResourceRequest().GetKnownTransparentPlaceholderImageIndex() != kNotFound)) {
        ...
        std::tie(result, response, data) = network_utils::ParseDataURL(
            url, params.GetResourceRequest().HttpMethod(), params.GetResourceRequest().GetUkmSourceId(), /*UkmRecorder()*/ nullptr);
        if (result != net::OK) {
            return nullptr;
        }
        ...
        ArchiveResource* archive_resource = archive_->SubresourceForURL(params.Url());
    ...
    case ResourceStatus::kNotStarted:
        ...
        // fast-path.
        CHECK(!IsSimplifyLoadingTransparentPlaceholderImageEnabled() || (params.GetResourceRequest().GetKnownTransparentPlaceholderImageIndex() == kNotFound));
    ...
    case ResourceStatus::kCached:
        ...
        CHECK_NE(params.GetResourceRequest().GetKnownTransparentPlaceholderImageIndex(), kNotFound);

        use_counter_->CountUse(WebFeature::kSimplifyLoadingTransparentPlaceholderImage);
    ...
    default:
        CHECK(false) << "Unexpected resource status: " << (int)resource->GetStatus();
    }
...
Resource* ResourceFetcher::ResourceForBlockedRequest(
    const FetchParameters& params, const ResourceFactory& factory, ResourceRequestBlockedReason blocked_reason, ResourceClient* client)
{
    Resource* resource = factory.Create(params.GetResourceRequest(), params.Options(), params.DecoderOptions());
    if (client) {
        client->SetResource(resource, freezable_task_runner_.get());
    }
    resource->FinishAsError(ResourceError::CancelledDueToAccessCheckError(params.Url(), blocked_reason), freezable_task_runner_.get());
    return resource;
}

void ResourceFetcher::MakePreloadedResourceBlockOnloadIfNeeded(Resource* resource, const FetchParameters& params)
{
    ...
    if (resource && resource->Loader() && resource->IsLoadEventBlockingResourceType() && resource->IsLinkPreload() && !params.IsLinkPreload()
        ...
        if (resource_load_observer_) {
            resource_load_observer_->DidChangeRenderBlockingBehavior(resource, params);
        }
    }
}

ResourceFetcher::RevalidationPolicyForMetrics ResourceFetcher::MapToPolicyForMetrics(RevalidationPolicy policy, Resource* resource, DeferPolicy defer_policy)
{
    switch (defer_policy) {
    case DeferPolicy::kNoDefer:
        break;
...
void ResourceFetcher::UpdateMemoryCacheStats(Resource* resource, RevalidationPolicyForMetrics policy, const FetchParameters& params,
    const ResourceFactory& factory, bool is_static_data, bool same_top_frame_site_resource_cached) const
{
    ...
    if (params.Defer() != FetchParameters::DeferOption::kNoDefer && factory.GetType() == ResourceType::kScript) {
...
std::optional<ResourceRequestBlockedReason> ResourceFetcher::UpdateRequestForTransparentPlaceholderImage(FetchParameters& params)
{
    ResourceRequest& resource_request = params.MutableResourceRequest();
    // Should only be called if request has transparent-placholder-image.
    DCHECK(IsSimplifyLoadingTransparentPlaceholderImageEnabled() && (resource_request.GetKnownTransparentPlaceholderImageIndex() != kNotFound));
    ...
    if (Context().CheckAndEnforceCSPForRequest(resource_request.GetRequestContext(), resource_request.GetRequestDestination(), params.Url(), params.Options(),
            ReportingDisposition::kReport, params.Url(), ResourceRequestHead::RedirectStatus::kNoRedirect)
...
KURL ResourceFetcher::PrepareRequestForWebBundle(ResourceRequest& resource_request) const
{
    if (resource_request.GetWebBundleTokenParams()) {
        DCHECK_EQ(resource_request.GetRequestDestination(), network::mojom::RequestDestination::kWebBundle);
        return KURL();
    }
    if (SubresourceWebBundle* bundle = GetMatchingBundle(resource_request.Url())) {
        resource_request.SetWebBundleTokenParams(
            ResourceRequestHead::WebBundleTokenParams(bundle->GetBundleUrl(), bundle->WebBundleToken(), mojo::NullRemote()));
    ...
    if (resource_request.Url().Protocol() == "uuid-in-package" && resource_request.GetWebBundleTokenParams()) {
...
SubresourceWebBundleList* ResourceFetcher::GetOrCreateSubresourceWebBundleList()
{
...
Resource* ResourceFetcher::RequestResource(FetchParameters& params, const ResourceFactory& factory, ResourceClient* client)
{
    ...
    resource_request.SetFromOriginDirtyStyleSheet(params.IsFromOriginDirtyStyleSheet());
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(TRACE_DISABLED_BY_DEFAULT("network"), "ResourceLoad", TRACE_ID_WITH_SCOPE("BlinkResourceID", TRACE_ID_LOCAL(identifier)),
    ...
    absl::Cleanup record_times = [start = base::TimeTicks::Now(), &params] {
        ...
        base::UmaHistogramMicrosecondsTimes("Blink.Fetch.RequestResourceTime2", elapsed);
        if (params.Url().ProtocolIsData()) {
            base::UmaHistogramMicrosecondsTimes("Blink.Fetch.RequestResourceTime2.Data", elapsed);
            if (params.GetResourceRequest().GetKnownTransparentPlaceholderImageIndex() != kNotFound) {
                base::UmaHistogramMicrosecondsTimes("Blink.Fetch.RequestResourceTime2.TransparentPlaceholderImage", elapsed);
            }
        }
        if (params.IsSpeculativePreload() || params.IsLinkPreload()) {
            base::UmaHistogramMicrosecondsTimes("Blink.Fetch.RequestResourceTime2.Preload", elapsed);
        }
    };
    TRACE_EVENT1("blink,blink.resource", "ResourceFetcher::requestResource", "url", params.Url().ElidedString().Utf8());
    ...
    if (!resource_request.RequestorOrigin()) {
        resource_request.SetRequestorOrigin(properties_->GetFetchClientSettingsObject().GetSecurityOrigin());
    }
    ...
    std::optional<ResourceRequestBlockedReason> blocked_reason = prepare_helper.PrepareRequestForCacheAccess(pauser);
    if (blocked_reason) {
        auto* resource = ResourceForBlockedRequest(params, factory, blocked_reason.value(), client);
        StorePerformanceTimingInitiatorInformation(resource, params.GetRenderBlockingBehavior());
    ...
    // deferred.
    if (!is_stale_revalidation && (archive_ || (is_data_url && defer_policy != DeferPolicy::kDefer))) {
        prepare_helper.UpgradeForLoaderIfNecessary(pauser);
        resource = CreateResourceForStaticData(params, factory);
        if (resource) {
            policy = DetermineRevalidationPolicy(resource_type, params, *resource, true);
    ...
    bool in_cached_resources_map = cached_resources_map_.Contains(MemoryCache::RemoveFragmentIdentifierIfNeeded(params.Url()));

    if (!is_stale_revalidation && !resource) {
        if (!prepare_helper.WasUpgradeForLoaderCalled() && preloads_.find(PreloadKey(params.Url(), resource_type)) != preloads_.end()) {
            prepare_helper.UpgradeForLoaderIfNecessary(pauser);
        }
        ...
        if (resource) {
            ...
            resource = MemoryCache::Get()->ResourceForURL(params.Url(), GetCacheIdentifier(params.Url(), params.GetResourceRequest().GetSkipServiceWorker()));
            if (resource) {
                policy = DetermineRevalidationPolicy(resource_type, params, *resource, is_static_data);
                scoped_refptr<const SecurityOrigin> top_frame_origin = resource_request.TopFrameOrigin();
                if (top_frame_origin) {
                    same_top_frame_site_resource_cached = resource->AppendTopFrameSiteForMetrics(*top_frame_origin);
                }
            }
        }
    }
    if (!prepare_helper.WasUpgradeForLoaderCalled() && policy != RevalidationPolicy::kUse) {
        prepare_helper.UpgradeForLoaderIfNecessary(pauser);
    }

    UpdateMemoryCacheStats(
        resource, MapToPolicyForMetrics(policy, resource, defer_policy), params, factory, is_static_data, same_top_frame_site_resource_cached);
    ...
    case RevalidationPolicy::kRevalidate:
        ...
        break;
    case RevalidationPolicy::kUse:
        if (resource_request.AllowsStaleResponse() && resource->ShouldRevalidateStaleResponse(*use_counter_)) {
            ScheduleStaleRevalidate(resource);
        }
        if (resource->GetType() == ResourceType::kImage && resource->GetContentStatus() == ResourceStatus::kCached
    ...
    if (!is_static_data && policy == RevalidationPolicy::kUse && !in_cached_resources_map) {
        base::UmaHistogramEnumeration(kCrossDocumentCachedResource, resource->GetType());
    }
    ...
    if (policy == RevalidationPolicy::kUse && resource->GetStatus() == ResourceStatus::kCached && !in_cached_resources_map) {
        // Loaded from MemoryCache.
        DidLoadResourceFromMemoryCache(resource, resource_request, is_static_data, params.GetRenderBlockingBehavior());
    }
    if (!is_stale_revalidation) {
        String resource_url = MemoryCache::RemoveFragmentIdentifierIfNeeded(params.Url());
        cached_resources_map_.Set(resource_url, resource);
        MaybeSaveResourceToStrongReference(resource);
        if (PriorityObserverMapCreated() && PriorityObservers()->Contains(resource_url)) {
            // Resolve the promise.
            std::move(PriorityObservers()->Take(resource_url)).Run(static_cast<int>(resource->GetResourceRequest().InitialPriority()));
        }
    }
    ...
    // load-blocking.
    ImageLoadBlockingPolicy load_blocking_policy = ImageLoadBlockingPolicy::kDefault;
    if (resource->GetType() == ResourceType::kImage) {
        not_loaded_image_resources_.insert(resource);
        if (params.GetImageRequestBehavior() == FetchParameters::ImageRequestBehavior::kNonBlockingImage) {
    ...
    if (prepare_helper.WasUpgradeForLoaderCalled()) {
        resource->UpdateResourceWidth(resource_request.HttpHeaderField(AtomicString("sec-ch-width")));
    }

    if (needs_load) {
        if (!StartLoad(resource, std::move(params.MutableResourceRequest().MutableBody()), load_blocking_policy, params.GetRenderBlockingBehavior())) {
            resource->FinishAsError(ResourceError::CancelledError(params.Url()), freezable_task_runner_.get());
        }
    }
    ...
    if (resource->InspectorId() != identifier || (!resource->StillNeedsLoad() && !resource->IsLoading())) {
        TRACE_EVENT_NESTABLE_ASYNC_END1(
            TRACE_DISABLED_BY_DEFAULT("network"), "ResourceLoad", TRACE_ID_WITH_SCOPE("BlinkResourceID", TRACE_ID_LOCAL(identifier)), "outcome", "Fail");
    }
    return resource;
}
...
void ResourceFetcher::RemoveResourceStrongReference(Resource* resource)
{
    if (resource && document_resource_strong_refs_.Contains(resource)) {
        const size_t resource_size = static_cast<size_t>(resource->GetResponse().DecodedBodyLength());
...
void ResourceFetcher::ResourceTimingReportTimerFired(TimerBase* timer)
{
    ...
    for (auto& scheduled_report : timing_reports) {
        Context().AddResourceTiming(std::move(scheduled_report.info), scheduled_report.initiator_type);
    }
}

void ResourceFetcher::InitializeRevalidation(ResourceRequest& revalidating_request, Resource* resource)
{
    DCHECK(resource);
    ...
    DCHECK_EQ(properties_->GetControllerServiceWorkerMode(), mojom::ControllerServiceWorkerMode::kNoController);
    // RawResource doesn't support revalidation.
    CHECK(!IsRawResource(*resource));

    revalidating_request.SetIsRevalidating(true);

    const AtomicString& last_modified = resource->GetResponse().HttpHeaderField(http_names::kLastModified);
    const AtomicString& e_tag = resource->GetResponse().HttpHeaderField(http_names::kETag);
    if (!last_modified.empty() || !e_tag.empty()) {
        DCHECK_NE(mojom::blink::FetchCacheMode::kBypassCache, revalidating_request.GetCacheMode());
        if (revalidating_request.GetCacheMode() == mojom::blink::FetchCacheMode::kValidateCache) {
            revalidating_request.SetHttpHeaderField(http_names::kCacheControl, AtomicString("max-age=0"));
        }
    }
    if (!last_modified.empty()) {
        revalidating_request.SetHttpHeaderField(http_names::kIfModifiedSince, last_modified);
    }
    if (!e_tag.empty()) {
...
bool UseRenderBlockingTaskPriority(const mojom::blink::RequestContextType request_context, const RenderBlockingBehavior render_blocking_behavior)
{
    ...
    case mojom::blink::RequestContextType::IMAGE_SET:
        return base::FeatureList::IsEnabled(features::kBoostImageSetLoadingTaskPriority);
    case mojom::blink::RequestContextType::FONT:
        return base::FeatureList::IsEnabled(features::kBoostFontLoadingTaskPriority);
    case mojom::blink::RequestContextType::VIDEO:
        return base::FeatureList::IsEnabled(features::kBoostVideoLoadingTaskPriority);
    case mojom::blink::RequestContextType::STYLE:
        if (render_blocking_behavior == RenderBlockingBehavior::kBlocking) {
            return base::FeatureList::IsEnabled(features::kBoostRenderBlockingStyleLoadingTaskPriority);
        }
        return base::FeatureList::IsEnabled(features::kBoostNonRenderBlockingStyleLoadingTaskPriority);
    default:
        return false;
    }
}
...
std::unique_ptr<URLLoader> ResourceFetcher::CreateURLLoader(const network::ResourceRequest& network_request, const ResourceLoaderOptions& options,
    const mojom::blink::RequestContextType request_context, const RenderBlockingBehavior render_blocking_behavior,
    const std::optional<base::UnguessableToken>& service_worker_race_network_request_token, bool is_from_origin_dirty_style_sheet)
{
    DCHECK(!GetProperties().IsDetached());
    // TODO(http://crbug.com/1252983): Revert this to DCHECK.
    CHECK(loader_factory_);

    scoped_refptr<base::SingleThreadTaskRunner> task_runner = unfreezable_task_runner_;
    if (network_request.keepalive
        && (!base::FeatureList::IsEnabled(blink::features::kKeepAliveInBrowserMigration)
            || (network_request.attribution_reporting_eligibility != network::mojom::AttributionReportingEligibility::kUnset
                && !base::FeatureList::IsEnabled(features::kAttributionReportingInBrowserMigration)))) {
        ...
        if (auto* frame_or_worker_scheduler = GetFrameOrWorkerScheduler()) {
            if (auto* frame_scheduler = frame_or_worker_scheduler->ToFrameScheduler()) {
                task_runner = frame_scheduler->GetAgentGroupScheduler()->DefaultTaskRunner();
            }
        }
    } else if (UseRenderBlockingTaskPriority(request_context, render_blocking_behavior)) {
        if (auto* frame_or_worker_scheduler = GetFrameOrWorkerScheduler()) {
            if (auto* frame_scheduler = frame_or_worker_scheduler->ToFrameScheduler()) {
                task_runner = frame_scheduler->GetTaskRunner(TaskType::kNetworkingUnfreezableRenderBlockingLoading);
            }
        }
    }
    return loader_factory_->CreateURLLoader(network_request, options, freezable_task_runner_, task_runner, back_forward_cache_loader_helper_,
        service_worker_race_network_request_token, is_from_origin_dirty_style_sheet);
}
...
void ResourceFetcher::AddToMemoryCacheIfNeeded(const FetchParameters& params, Resource* resource)
{
...
Resource* ResourceFetcher::CreateResourceForLoading(const FetchParameters& params, const ResourceFactory& factory)
{
    const String cache_identifier = GetCacheIdentifier(params.GetResourceRequest().Url(), params.GetResourceRequest().GetSkipServiceWorker());
    DCHECK(!IsMainThread() || params.IsStaleRevalidation() || !MemoryCache::Get()->ResourceForURL(params.GetResourceRequest().Url(), cache_identifier));

    RESOURCE_LOADING_DVLOG(1) << "Loading Resource for " << params.GetResourceRequest().Url().ElidedString();
...
void ResourceFetcher::StorePerformanceTimingInitiatorInformation(Resource* resource, RenderBlockingBehavior render_blocking_behavior)
{
    ...
    resource_timing_info_map_.insert(resource, PendingResourceTimingInfo { base::TimeTicks::Now(), fetch_initiator, render_blocking_behavior });
}

void ResourceFetcher::RecordResourceTimingOnRedirect(Resource* resource, const ResourceResponse& redirect_response, const KURL& new_url)
{
    PendingResourceTimingInfoMap::iterator it = resource_timing_info_map_.find(resource);
    if (it != resource_timing_info_map_.end()) {
        if (ResourceLoadTiming* load_timing = redirect_response.GetResourceLoadTiming()) {
...
Resource* ResourceFetcher::MatchPreload(const FetchParameters& params, ResourceType type)
{
    ...
    if (IsImageResourceDisallowedToBeReused(*resource)) {
        PrintPreloadMismatch(resource, Resource::MatchStatus::kImageLoadingDisabled);
        return nullptr;
    }
...
void ResourceFetcher::PrintPreloadMismatch(Resource* resource, Resource::MatchStatus status)
{
    ...
    console_logger_->AddConsoleMessage(mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning, builder.ToString());

    TRACE_EVENT1("blink,blink.resource", "ResourceFetcher::PrintPreloadMismatch", "data",
        CreateTracedValueForUnusedPreload(resource->Url(), status, resource->GetResourceRequest().GetDevToolsId()));
}
...
void ResourceFetcher::InsertAsPreloadIfNecessary(Resource* resource, const FetchParameters& params, ResourceType type)
{
    ...
    if (resource->LoadFailedOrCanceled() && resource->GetResourceError().IsAccessCheck()) {
        return;
    }
...
bool ResourceFetcher::IsImageResourceDisallowedToBeReused(const Resource& existing_resource) const
{
...
ResourceFetcher::RevalidationPolicy ResourceFetcher::DetermineRevalidationPolicy(
    ResourceType type, const FetchParameters& fetch_params, const Resource& existing_resource, bool is_static_data) const
{
    RevalidationPolicy policy;
    const char* reason;
    std::tie(policy, reason) = DetermineRevalidationPolicyInternal(type, fetch_params, existing_resource, is_static_data);
    DCHECK(reason);

    RESOURCE_LOADING_DVLOG(1) << "ResourceFetcher::DetermineRevalidationPolicy "
                              << "url = " << fetch_params.Url() << ", policy = " << GetNameFor(policy) << ", reason = \"" << reason << "\"";

    TRACE_EVENT_INSTANT2("blink", "ResourceFetcher::DetermineRevalidationPolicy", TRACE_EVENT_SCOPE_THREAD, "policy", GetNameFor(policy), "reason", reason);
    return policy;
}
...
std::pair<ResourceFetcher::RevalidationPolicy, const char*> ResourceFetcher::DetermineRevalidationPolicyInternal(
    ResourceType type, const FetchParameters& fetch_params, const Resource& existing_resource, bool is_static_data) const
{
    ...
    if (IsDownloadOrStreamRequest(request)) {
        return { RevalidationPolicy::kReload, "It is for download or for streaming." };
    }

    if (IsImageResourceDisallowedToBeReused(existing_resource)) {
        return { RevalidationPolicy::kReload, "Reload due to 'allow image' settings." };
    }
    ...
    if (existing_resource.Loader() && existing_resource.Loader()->Fetcher() != this) {
        return { RevalidationPolicy::kReload, "The existing resource is loading in a foreign fetcher." };
    }
    ...
    if ((fetch_params.IsLinkPreload() || fetch_params.IsSpeculativePreload()) && existing_resource.IsUnusedPreload()) {
    ...
    if (request.GetCacheMode() == mojom::blink::FetchCacheMode::kForceCache) {
        return { RevalidationPolicy::kUse, "Use the existing resource due to cache-mode: 'force-cache'." };
    }

    // Don't reuse resources with Cache-control: no-store.
    if (existing_resource.HasCacheControlNoStoreHeader()) {
        return { RevalidationPolicy::kReload, "Reload due to cache-control: no-store." };
    }
    ...
    if (type != ResourceType::kRaw) {
        if (!properties_->IsLoadComplete() && cached_resources_map_.Contains(MemoryCache::RemoveFragmentIdentifierIfNeeded(existing_resource.Url()))) {
            return { RevalidationPolicy::kUse,
                "Avoid making multiple requests for the same URL "
                "during the initial load." };
        }
        if (existing_resource.IsLoading()) {
            return { RevalidationPolicy::kUse, "Use the existing resource because it's being loaded." };
        }
    }
    ...
    if (existing_resource.ErrorOccurred()) {
        return { RevalidationPolicy::kReload, "Reload because the existing resource has failed loading." };
    }
    ...
    if (type == ResourceType::kImage && &existing_resource == cached_resource_in_fetcher) {
        return { RevalidationPolicy::kUse, "Images can be reused without cache validation." };
    }
    ...
    if (!existing_resource.CanReuseRedirectChain(*use_counter_)) {
        return { RevalidationPolicy::kReload, "Reload due to an uncacheable redirect." };
    }
    ...
    if (request.GetCacheMode() == mojom::blink::FetchCacheMode::kValidateCache
        || existing_resource.MustRevalidateDueToCacheHeaders(request.AllowsStaleResponse(), *use_counter_) || request.CacheControlContainsNoCache()) {
        ...
        if (existing_resource.IsUnusedPreload()) {
            return { RevalidationPolicy::kReload, "Revalidation is harmful for non-matched preloads." };
        }
        ...
        if (existing_resource.CanUseCacheValidator() && properties_->GetControllerServiceWorkerMode() == mojom::ControllerServiceWorkerMode::kNoController) {
            ...
            if (existing_resource.IsCacheValidator()) {
                ...
                return { RevalidationPolicy::kUse, "Merged to the revalidate request which has not yet started." };
            }
            return { RevalidationPolicy::kRevalidate, "" };
        }

        // No, must reload.
        return { RevalidationPolicy::kReload, "Reload due to missing cache validators." };
    }

    return { RevalidationPolicy::kUse, "Use the existing resource because there is no reason not to do so." };
}
...
{
    return !Context().AllowImage() || (!auto_load_images_ && !url.ProtocolIsData());
}
...
void ResourceFetcher::ClearContext()
{
    ...
    if (!loaders_.empty() || !non_blocking_loaders_.empty()) {
        CHECK(!base::FeatureList::IsEnabled(blink::features::kKeepAliveInBrowserMigration)
            || !base::FeatureList::IsEnabled(blink::features::kAttributionReportingInBrowserMigration));
        ...
        keepalive_loaders_task_handle_ = PostDelayedCancellableTask(*freezable_task_runner_, FROM_HERE,
            WTF::BindOnce(&ResourceFetcher::StopFetchingIncludingKeepaliveLoaders, WrapPersistent(this)), kKeepaliveLoadersTimeout);
    }
}
...
void ResourceFetcher::ScheduleWarnUnusedPreloads(base::OnceCallback<void(Vector<KURL> unused_preloads)> callback)
{
    ...
    unused_preloads_timer_ = PostDelayedCancellableTask(*freezable_task_runner_, FROM_HERE,
        WTF::BindOnce(&ResourceFetcher::WarnUnusedPreloads, WrapWeakPersistent(this), std::move(callback)), kUnusedPreloadTimeout);
}

void ResourceFetcher::WarnUnusedPreloads(base::OnceCallback<void(Vector<KURL> unused_preloads)> callback)
{
    ...
    for (const auto& pair : preloads_) {
        ...
        if (resource->IsLinkPreload()) {
            String message = "The resource " + resource->Url().GetString() + " was preloaded "
                + "using link preload but not used within a few seconds from the " + "window's load event. Please make sure it has an appropriate `as` "
                + "value and it is preloaded intentionally.";
            console_logger_->AddConsoleMessage(mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kWarning, message);
            TRACE_EVENT1("blink,blink.resource", "ResourceFetcher::WarnUnusedPreloads", "data",
                CreateTracedValueForUnusedPreload(resource->Url(), Resource::MatchStatus::kOk, resource->GetResourceRequest().GetDevToolsId()));

            base::UmaHistogramCounts100("Renderer.Preload.UnusedResource", static_cast<int>(resource->GetType()));
        }
        base::UmaHistogramEnumeration("Renderer.Preload.UnusedResource2", resource->GetType());
        base::UmaHistogramEnumeration(
            base::StrCat({ "Renderer.Preload.UnusedResource2.", LinkPreloadStrForHistogram(resource->IsLinkPreload()) }), resource->GetType());
    }
    base::UmaHistogramCounts100("Renderer.Preload.UnusedResourceCount", unused_resource_count);
    // Record the total count of deferred preloads based on the LCPP signal.
    base::UmaHistogramCounts100(base::StrCat({ kLCPPDeferUnusedPreloadHistogramPrefix, "DeferredResourceCount" }), deferred_preloads_.size());
    ...
    for (auto& pair : unused_early_hints_preloaded_resources_) {
        ...
        console_logger_->AddConsoleMessage(mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kWarning, message);
        TRACE_EVENT1("blink,blink.resource", "ResourceFetcher::WarnUnusedEarlyHintsPreloads", "data", CreateTracedValueForUnusedEarlyHintsPreload(pair.key));
...
void ResourceFetcher::HandleLoaderFinish(Resource* resource, base::TimeTicks response_end, LoaderFinishType type, uint32_t inflight_keepalive_bytes)
{
    DCHECK(resource);
    ...
    if (resource->GetType() != ResourceType::kRaw) {
        ...
        if (resource->GetResponse().WasFetchedViaServiceWorker()) {
            ++subresource_load_metrics_.number_of_subresource_loads_handled_by_service_worker;
        }
    }

    if (IsControlledByServiceWorker() == mojom::blink::ControllerServiceWorkerMode::kControlled) {
        if (resource->GetResponse().WasFetchedViaServiceWorker()) {
            base::UmaHistogramEnumeration("ServiceWorker.Subresource.Handled.Type2", resource->GetType());
        } else {
            base::UmaHistogramEnumeration("ServiceWorker.Subresource.Fallbacked.Type2", resource->GetType());
        }
        UpdateServiceWorkerSubresourceMetrics(
            resource->GetType(), resource->GetResponse().WasFetchedViaServiceWorker(), resource->GetResponse().GetServiceWorkerRouterInfo());
    }
    ...
    // error).
    if (resource->GetResponse().GetType() == network::mojom::FetchResponseType::kOpaque && resource->GetResponse().HasRangeRequested()
        && !resource->GetResourceRequest().HttpHeaderFields().Contains(http_names::kRange)) {
    ...
    if (type == kDidFinishLoading) {
        ...
        if (resource->GetResourceRequest().AllowsStaleResponse() && resource->StaleRevalidationRequested()) {
            ScheduleStaleRevalidate(resource);
        }
    }
    if (resource_load_observer_) {
        DCHECK(!IsDetached());
        resource_load_observer_->DidFinishLoading(resource->InspectorId(), response_end, encoded_data_length, resource->GetResponse().DecodedBodyLength());
    }
    MaybeSaveResourceToStrongReference(resource);
}

void ResourceFetcher::HandleLoaderError(Resource* resource, base::TimeTicks finish_time, const ResourceError& error, uint32_t inflight_keepalive_bytes)
{
    DCHECK(resource);
    ...
    if (!info.is_null()) {
        if (resource->GetResourceRequest().Url().ProtocolIsInHTTPFamily()
            || (resource->GetResourceRequest().GetWebBundleTokenParams() && resource->GetResourceRequest().GetWebBundleTokenParams()->bundle_url.IsValid())) {
    ...
    if (network_utils::IsCertificateTransparencyRequiredError(error.ErrorCode())) {
        use_counter_->CountUse(mojom::WebFeature::kCertificateTransparencyRequiredErrorOnResourceLoad);
    }
    resource->FinishAsError(error, freezable_task_runner_.get());
    if (resource_load_observer_) {
        DCHECK(!IsDetached());
        resource_load_observer_->DidFailLoading(resource->LastResourceRequest().Url(), resource->InspectorId(), error,
            resource->GetResponse().EncodedDataLength(),
            ResourceLoadObserver::IsInternalRequest(resource->Options().initiator_info.name == fetch_initiator_type_names::kInternal));
    }
}
...
bool ResourceFetcher::StartLoad(Resource* resource, bool is_potentially_unused_preload)
{
    CHECK(resource->GetType() == ResourceType::kFont || resource->GetType() == ResourceType::kImage || is_potentially_unused_preload);
    ...
    if (resource->GetType() == ResourceType::kFont) {
        base::UmaHistogramEnumeration(RESOURCE_HISTOGRAM_PREFIX "Font", RevalidationPolicyForMetrics::kPreviouslyDeferredLoad);
    } else if (resource->GetType() == ResourceType::kImage) {
        base::UmaHistogramEnumeration(RESOURCE_HISTOGRAM_PREFIX "Image", RevalidationPolicyForMetrics::kPreviouslyDeferredLoad);
    }
    return StartLoad(resource, ResourceRequestBody(), ImageLoadBlockingPolicy::kDefault, RenderBlockingBehavior::kNonBlocking);
}

bool ResourceFetcher::StartLoad(
    Resource* resource, ResourceRequestBody request_body, ImageLoadBlockingPolicy policy, RenderBlockingBehavior render_blocking_behavior)
{
    DCHECK(resource);
...

    {
        ...
        Resource::RevalidationStartForbiddenScope revalidation_start_forbidden_scope(resource);
        ...
        if (resource_load_observer_) {
            ...
            ResourceResponse response;
            resource_load_observer_->WillSendRequest(request, response, resource->GetType(), resource->Options(), render_blocking_behavior, resource);
        }
        ...
        loader = MakeGarbageCollected<ResourceLoader>(this, scheduler_, resource, context_lifecycle_notifier_, std::move(request_body), size);
        ...
        if (!resource->IsLinkPreload() && resource->IsLoadEventBlockingResourceType() && policy != ImageLoadBlockingPolicy::kForceNonBlockingLoad) {
        ...
        StorePerformanceTimingInitiatorInformation(resource, render_blocking_behavior);
    }
...

    {
        ...
        Resource::ProhibitAddRemoveClientInScope prohibit_add_remove_client_in_scope(resource);
...
void ResourceFetcher::ScheduleLoadingPotentiallyUnusedPreload(Resource* resource)
{
    ...
    case features::LcppDeferUnusedPreloadTiming::kPostTask:
        ScheduleStartLoadAndFinishIfFailed(resource, /*is_potentially_unused_preload=*/true);
        break;
    case features::LcppDeferUnusedPreloadTiming::kLcpTimingPredictor:
        context_->AddLcpPredictedCallback(WTF::BindOnce(&ResourceFetcher::StartLoadAndFinishIfFailed, WrapWeakPersistent(this), WrapWeakPersistent(resource),
            /*is_potentially_unused_preload=*/true));
        break;
    case features::LcppDeferUnusedPreloadTiming::kLcpTimingPredictorWithPostTask:
        context_->AddLcpPredictedCallback(
            WTF::BindOnce(&ResourceFetcher::ScheduleStartLoadAndFinishIfFailed, WrapWeakPersistent(this), WrapWeakPersistent(resource),
                /*is_potentially_unused_preload=*/true));
        break;
    }
}

void ResourceFetcher::StartLoadAndFinishIfFailed(Resource* resource, bool is_potentially_unused_preload)
{
    if (!resource) {
        return;
    }
    ...
    if (!StartLoad(resource, is_potentially_unused_preload)) {
        resource->FinishAsError(ResourceError::CancelledError(resource->Url()), freezable_task_runner_.get());
    }
}

void ResourceFetcher::ScheduleStartLoadAndFinishIfFailed(Resource* resource, bool is_potentially_unused_preload)
{
    freezable_task_runner_->PostTask(FROM_HERE,
        WTF::BindOnce(&ResourceFetcher::StartLoadAndFinishIfFailed, WrapWeakPersistent(this), WrapWeakPersistent(resource), is_potentially_unused_preload));
}
...
void ResourceFetcher::UpdateAllImageResourcePriorities()
{
    ...
    speculative_decode_candidate_images_.erase_if(
        [](const WeakMember<Resource>& resource) -> bool { return resource->LastComputedPriority().visibility == ResourcePriority::kNotVisible; });
    MaybeStartSpeculativeImageDecode();
    ...
    for (Resource* resource : not_loaded_image_resources_) {
        ...
        ResourceLoadPriority computed_load_priority = ComputeLoadPriority(ResourceType::kImage, resource->GetResourceRequest(), resource_priority.visibility,
            FetchParameters::DeferOption::kNoDefer, FetchParameters::SpeculativePreloadType::kNotSpeculative, RenderBlockingBehavior::kNonBlocking,
            mojom::blink::ScriptType::kClassic, false, std::nullopt, std::nullopt, resource_priority.is_lcp_resource);

        ResourcePriority resource_priority_excluding_image_loader = priorities.second;
        ResourceLoadPriority computed_load_priority_excluding_image_loader
            = ComputeLoadPriority(ResourceType::kImage, resource->GetResourceRequest(), resource_priority_excluding_image_loader.visibility,
                FetchParameters::DeferOption::kNoDefer, FetchParameters::SpeculativePreloadType::kNotSpeculative, RenderBlockingBehavior::kNonBlocking,
                mojom::blink::ScriptType::kClassic, false, std::nullopt, std::nullopt, resource_priority_excluding_image_loader.is_lcp_resource);
        ...
        static const bool fix_enabled = base::FeatureList::IsEnabled(features::kImageLoadingPrioritizationFix);

        if (computed_load_priority != computed_load_priority_excluding_image_loader) {
            // Mark pages affected by this fix for performance evaluation.
            use_counter_->CountUse(WebFeature::kEligibleForImageLoadingPrioritizationFix);
        }
        if (!fix_enabled) {
        ...
        DCHECK_GT(computed_load_priority, resource->GetResourceRequest().Priority());
        resource->DidChangePriority(computed_load_priority, resource_priority.intra_priority_value);
        TRACE_EVENT_NESTABLE_ASYNC_INSTANT1(TRACE_DISABLED_BY_DEFAULT("network"), "ResourcePrioritySet",
            TRACE_ID_WITH_SCOPE("BlinkResourceID", TRACE_ID_LOCAL(resource->InspectorId())), "data", CreateTracedValueWithPriority(computed_load_priority));
        DCHECK(!IsDetached());
        resource_load_observer_->DidChangePriority(resource->InspectorId(), computed_load_priority, resource_priority.intra_priority_value);
    }
...
String ResourceFetcher::GetCacheIdentifier(const KURL& url, bool skip_service_worker) const
{
    if (!skip_service_worker && properties_->GetControllerServiceWorkerMode() != mojom::ControllerServiceWorkerMode::kNoController) {
...
std::optional<base::UnguessableToken> ResourceFetcher::GetSubresourceBundleToken(const KURL& url) const
{
    SubresourceWebBundle* bundle = GetMatchingBundle(url);
    if (!bundle) {
        return std::nullopt;
...
std::optional<KURL> ResourceFetcher::GetSubresourceBundleSourceUrl(const KURL& url) const
{
    SubresourceWebBundle* bundle = GetMatchingBundle(url);
    if (!bundle) {
        return std::nullopt;
...
void ResourceFetcher::EmulateLoadStartedForInspector(Resource* resource, mojom::blink::RequestContextType request_context,
    network::mojom::RequestDestination request_destination, const AtomicString& initiator_name)
{
    base::AutoReset<bool> r(&is_in_request_resource_, true);
    ...
    if (base::FeatureList::IsEnabled(features::kEmulateLoadStartedForInspectorOncePerResource)) {
        ...
        emulated_load_started_for_inspector_resources_map_.Set(resource_url, resource);
    }
    ...
    if (!resource_request.PriorityHasBeenSet()) {
        resource_request.SetPriority(ComputeLoadPriority(resource->GetType(), resource_request, ResourcePriority::kNotVisible));
    }
    resource_request.SetPriorityIncremental(ShouldLoadIncremental(resource->GetType()));
    ...
    Context().CanRequest(resource->GetType(), last_resource_request, last_resource_request.Url(), params.Options(), ReportingDisposition::kReport,
        last_resource_request.GetRedirectInfo());
    if (resource->GetStatus() == ResourceStatus::kNotStarted || resource->GetStatus() == ResourceStatus::kPending) {
    ...
    DidLoadResourceFromMemoryCache(resource, params.GetResourceRequest(), false /* is_static_data */, params.GetRenderBlockingBehavior());
}
...
void ResourceFetcher::StopFetchingInternal(StopFetchingTarget target)
{
    ...
    for (const auto& loader : non_blocking_loaders_) {
        if (target == StopFetchingTarget::kIncludingKeepaliveLoaders || !loader->ShouldBeKeptAliveWhenDetached()) {
...
void ResourceFetcher::ScheduleStaleRevalidate(Resource* stale_resource)
{
    ...
    freezable_task_runner_->PostTask(
        FROM_HERE, WTF::BindOnce(&ResourceFetcher::RevalidateStaleResource, WrapWeakPersistent(this), WrapPersistent(stale_resource)));
}
...
void ResourceFetcher::RevalidateStaleResource(Resource* stale_resource)
{
    ...
    FetchParameters params(std::move(request), ResourceLoaderOptions(/*world=*/nullptr));
    ...
    RawResource::Fetch(params, this, MakeGarbageCollected<StaleRevalidationResourceClient>(stale_resource));
}

mojom::blink::BlobRegistry* ResourceFetcher::GetBlobRegistry()
{
    if (!blob_registry_remote_.is_bound()) {
        Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(blob_registry_remote_.BindNewPipeAndPassReceiver(freezable_task_runner_));
    }
...
void ResourceFetcher::PopulateAndAddResourceTimingInfo(Resource* resource, const PendingResourceTimingInfo& pending_info, base::TimeTicks response_end)
{
    ...
    // are precluded.
    if (resource->Options().world_for_csp && resource->Options().world_for_csp->IsIsolatedWorld()) {
        return;
    }

    AtomicString initiator_type = resource->IsPreloadedByEarlyHints() ? AtomicString(kEarlyHintsInitiatorType) : pending_info.initiator_type;

    const KURL& initial_url = resource->GetResourceRequest().GetRedirectInfo().has_value() ? resource->GetResourceRequest().GetRedirectInfo()->original_url
                                                                                           : resource->GetResourceRequest().Url();

    mojom::blink::ResourceTimingInfoPtr info = CreateResourceTimingInfo(pending_info.start_time, initial_url, &resource->GetResponse());
    if (info->allow_timing_details) {
        info->last_redirect_end_time = pending_info.redirect_end_time;
    }
    info->render_blocking_status = pending_info.render_blocking_behavior == RenderBlockingBehavior::kBlocking;
...
SubresourceWebBundle* ResourceFetcher::GetMatchingBundle(const KURL& url) const
{
    return subresource_web_bundles_ ? subresource_web_bundles_->GetMatchingBundle(url) : nullptr;
}

void ResourceFetcher::CancelWebBundleSubresourceLoadersFor(const base::UnguessableToken& web_bundle_token)
{
...
void ResourceFetcher::MaybeSaveResourceToStrongReference(Resource* resource)
{
    ...
    const size_t total_size_threshold = static_cast<size_t>(features::kMemoryCacheStrongReferenceTotalSizeThresholdParam.Get());
    const size_t resource_size_threshold = static_cast<size_t>(features::kMemoryCacheStrongReferenceResourceSizeThresholdParam.Get());
    const size_t resource_size = static_cast<size_t>(resource->GetResponse().DecodedBodyLength());
    const bool size_is_small_enough = resource_size <= resource_size_threshold && resource_size <= total_size_threshold;

    if (!size_is_small_enough) {
        return;
    }

    const SecurityOrigin* settings_object_origin = properties_->GetFetchClientSettingsObject().GetSecurityOrigin();
    if (!ShouldResourceBeKeptStrongReference(resource, settings_object_origin)) {
        return;
    }

    if (base::FeatureList::IsEnabled(features::kResourceFetcherStoresStrongReferences)) {
        // If the size would take us over, don't store it.
        if (document_resource_strong_refs_total_size_ + resource_size > total_size_threshold) {
            return;
        }
        document_resource_strong_refs_.insert(resource);
        document_resource_strong_refs_total_size_ += resource_size;
        freezable_task_runner_->PostDelayedTask(FROM_HERE,
            WTF::BindOnce(&ResourceFetcher::RemoveResourceStrongReference, WrapWeakPersistent(this), WrapWeakPersistent(resource)),
...
{
    CHECK(base::FeatureList::IsEnabled(features::kSpeculativeImageDecodes) || !speculative_decode_in_flight_);
    CHECK(base::FeatureList::IsEnabled(features::kSpeculativeImageDecodes) || speculative_decode_candidate_images_.empty());
    ...
    for (Resource* resource : speculative_decode_candidate_images_) {
        ...
        if (!image_to_decode || CompareResourcePriorities(priority, image_to_decode->LastComputedPriority()) > 0) {
            image_to_decode = resource;
        }
    }
    if (image_to_decode) {
        speculative_decode_candidate_images_.erase(image_to_decode);
        Context().StartSpeculativeImageDecode(image_to_decode, WTF::BindOnce(&ResourceFetcher::SpeculativeImageDecodeFinished, WrapWeakPersistent(this)));
...
void ResourceFetcher::OnMemoryPressure(base::MemoryPressureListener::MemoryPressureLevel level)
{
    if (base::FeatureList::IsEnabled(features::kReleaseResourceStrongReferencesOnMemoryPressure)) {
        document_resource_strong_refs_.clear();
        document_resource_strong_refs_total_size_ = 0;
    }
}

void ResourceFetcher::MaybeRecordLCPPSubresourceMetrics(const KURL& document_url)
{
    ...
    base::UmaHistogramCounts100("Blink.LCPP.PotentiallyLCPResourcePriorityBoosts2", potentially_lcp_resource_priority_boosts_);
}

void ResourceFetcher::MarkEarlyHintConsumedIfNeeded(uint64_t inspector_id, Resource* resource, const ResourceResponse& response)
{
    const KURL& initial_url = resource->GetResourceRequest().GetRedirectInfo().has_value() ? resource->GetResourceRequest().GetRedirectInfo()->original_url
                                                                                           : resource->GetResourceRequest().Url();
    ...
    if (iter != unused_early_hints_preloaded_resources_.end()) {
        ...
        if (!response.NetworkAccessed() && (!response.WasFetchedViaServiceWorker() || response.IsServiceWorkerPassThrough())) {
            resource->SetIsPreloadedByEarlyHints();
        }
    }
}

bool ResourceFetcher::IsPotentiallyUnusedPreload(ResourceType type, const FetchParameters& params) const
{
    static const bool kDeferUnusedPreload = base::FeatureList::IsEnabled(features::kLCPPDeferUnusedPreload);
    ...
    const LcppDeferUnusedPreloadExcludedResourceType kExcludedResourceType = features::kLcppDeferUnusedPreloadExcludedResourceType.Get();
    LcppDeferUnusedPreloadExcludedResourceType excluded_resource_type;
    if (defer_unused_preload_enabled_for_testing_) {
        excluded_resource_type = defer_unused_preload_excluded_resource_type_for_testing_;
    } else {
...
// static
const ResourceFetcher::ResourceFetcherSet& ResourceFetcher::MainThreadFetchers()
{
...
void ResourceFetcher::RecordResourceHistogram(std::string_view prefix, ResourceType type, RevalidationPolicyForMetrics policy) const
{
    base::UmaHistogramEnumeration(base::StrCat({ RESOURCE_HISTOGRAM_PREFIX, prefix, ResourceTypeName(type) }), policy);
}

void ResourceFetcher::UpdateServiceWorkerSubresourceMetrics(
    ResourceType resource_type, bool handled_by_serviceworker, const blink::ServiceWorkerRouterInfo* router_info)
{
    if (!subresource_load_metrics_.service_worker_subresource_load_metrics) {
        subresource_load_metrics_.service_worker_subresource_load_metrics = blink::ServiceWorkerSubresourceLoadMetrics {};
    }
    auto& metrics = *subresource_load_metrics_.service_worker_subresource_load_metrics;
    ...
    metrics.total_router_evaluation_time_for_subresources += router_info->RouterEvaluationTime();

    switch (*router_info->MatchedSourceType()) {
    case network::mojom::ServiceWorkerRouterSourceType::kCache:
        metrics.total_cache_lookup_time_for_subresources += router_info->CacheLookupTime();
...
ResourceFetcher::ResourcePrepareHelper::ResourcePrepareHelper(ResourceFetcher& fetcher, FetchParameters& params, const ResourceFactory& factory)
    : fetcher_(fetcher)
    , params_(params)
    , factory_(factory)
    , has_transparent_placeholder_image_(fetcher.IsSimplifyLoadingTransparentPlaceholderImageEnabled()
          && (params.GetResourceRequest().GetKnownTransparentPlaceholderImageIndex() != kNotFound))
{
}

std::optional<ResourceRequestBlockedReason> ResourceFetcher::ResourcePrepareHelper::PrepareRequestForCacheAccess(WebScopedVirtualTimePauser& pauser)
{
#if DCHECK_IS_ON()
    ...
    bundle_url_for_uuid_resources_ = fetcher_.PrepareRequestForWebBundle(resource_request);

    ResourceType resource_type = factory_.GetType();
    const ResourceLoaderOptions& options = params_.Options();

    DCHECK(options.synchronous_policy == kRequestAsynchronously || resource_type == ResourceType::kRaw || resource_type == ResourceType::kXSLStyleSheet);

    if (!RuntimeEnabledFeatures::MinimimalResourceRequestPrepBeforeCacheLookupEnabled()) {
        params_.OverrideContentType(factory_.ContentType());
        return PrepareResourceRequest(
            resource_type, fetcher_.properties_->GetFetchClientSettingsObject(), params_, fetcher_.Context(), pauser, *this, bundle_url_for_uuid_resources_);
    }

    std::optional<ResourceRequestBlockedReason> blocked_reason = PrepareResourceRequestForCacheAccess(
        resource_type, fetcher_.properties_->GetFetchClientSettingsObject(), bundle_url_for_uuid_resources_, *this, fetcher_.Context(), params_);
    if (blocked_reason) {
...
void ResourceFetcher::ResourcePrepareHelper::UpgradeForLoaderIfNecessary(WebScopedVirtualTimePauser& pauser)
{
#if DCHECK_IS_ON()
    DCHECK(determined_initial_blocked_reason_);
#endif
    ...
    params_.OverrideContentType(factory_.ContentType());
    UpgradeResourceRequestForLoaderNew(factory_.GetType(), params_, fetcher_.Context(), *this, pauser);
}

ResourceLoadPriority ResourceFetcher::ResourcePrepareHelper::ComputeLoadPriority(const FetchParameters& params)
{
    return fetcher_.ComputeLoadPriority(factory_.GetType(), params.GetResourceRequest(), ResourcePriority::kNotVisible, params.Defer(),
        params.GetSpeculativePreloadType(), params.GetRenderBlockingBehavior(), params.GetScriptType(), params.IsLinkPreload(), params.GetResourceWidth(),
        params.GetResourceHeight(), params.IsPotentiallyLCPElement(), params.IsPotentiallyLCPInfluencer());
}
...
void ResourceFetcher::ResourcePrepareHelper::RecordTrace()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_INSTANT1(TRACE_DISABLED_BY_DEFAULT("network"), "ResourcePrioritySet",
        TRACE_ID_WITH_SCOPE("BlinkResourceID", TRACE_ID_LOCAL(resource_request.InspectorId())), "priority", resource_request.Priority());
}
