...
{
    const Vector<std::string> kData { "This is a expected data!", "This is another data!" };
    ...
    auto* bytes_consumer = MakeGarbageCollected<SharedBufferBytesConsumer>(std::move(shared_buffer));
    EXPECT_EQ(PublicState::kReadableOrWaiting, bytes_consumer->GetPublicState());

    auto task_runner = base::MakeRefCounted<scheduler::FakeTaskRunner>();
    auto* test_reader = MakeGarbageCollected<BytesConsumerTestReader>(bytes_consumer);
    ...
    EXPECT_EQ(flatten_expected_data, std::string(data_from_consumer.data(), data_from_consumer.size()));
}
