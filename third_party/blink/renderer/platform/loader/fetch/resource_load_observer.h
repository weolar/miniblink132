...
class PLATFORM_EXPORT ResourceLoadObserver : public GarbageCollected<ResourceLoadObserver> {
public:
    ...
    virtual void WillSendRequest(
        const ResourceRequest&, const ResourceResponse& redirect_response, ResourceType, const ResourceLoaderOptions&, RenderBlockingBehavior, const Resource*)
        = 0;

    // Called when the priority of the request changes.
    virtual void DidChangePriority(uint64_t identifier, ResourceLoadPriority, int intra_priority_value) = 0;
    ...
    virtual void DidReceiveData(uint64_t identifier, base::SpanOrSize<const char> chunk) = 0;

    // Called when receiving an update for "network transfer size" for a request.
    virtual void DidReceiveTransferSizeUpdate(uint64_t identifier, int transfer_size_diff) = 0;
    ...
    virtual void DidFinishLoading(uint64_t identifier, base::TimeTicks finish_time, int64_t encoded_data_length, int64_t decoded_body_length) = 0;

    using IsInternalRequest = base::StrongAlias<class IsInternalRequestTag, bool>;
    // Called when a request fails.
    virtual void DidFailLoading(const KURL&, uint64_t identifier, const ResourceError&, int64_t encoded_data_length, IsInternalRequest) = 0;

    // Called when the RenderBlockingBehavior given to WillSendRequest changes.
    virtual void DidChangeRenderBlockingBehavior(Resource* resource, const FetchParameters& params) = 0;
