...
const std::array<WebFeature, 2> kApplicationFeatures = { WebFeature::kCrossOriginApplicationScript, WebFeature::kSameOriginApplicationScript };

const std::array<WebFeature, 2> kTextFeatures = { WebFeature::kCrossOriginTextScript, WebFeature::kSameOriginTextScript };
...
static bool AllowMimeTypeAsScript(
    const String& mime_type, bool same_origin, AllowedByNosniff::MimeTypeCheck mime_type_check_mode, std::optional<WebFeature>& counter)
{
    ...
    if (mime_type_check_mode == MimeTypeCheck::kLaxForWorker && RuntimeEnabledFeatures::StrictMimeTypesForWorkersEnabled()) {
    ...
    DCHECK(mime_type_check_mode == MimeTypeCheck::kLaxForWorker || mime_type_check_mode == MimeTypeCheck::kLaxForElement);
    ...
    if (EqualIgnoringASCIICase(mime_type, "text/javascript1.6") || EqualIgnoringASCIICase(mime_type, "text/javascript1.7")) {
    ...
    } else if (mime_type.StartsWithIgnoringCase("text/json") || mime_type.StartsWithIgnoringCase("application/json")) {
...
bool AllowedByNosniff::MimeTypeAsScript(
    UseCounter& use_counter, ConsoleLogger* console_logger, const ResourceResponse& response, MimeTypeCheck mime_type_check_mode)
{
    // The content type is really only meaningful for `http:`-family schemes.
    if (!response.CurrentRequestUrl().ProtocolIsInHTTPFamily()) {
        String last_path_component = response.CurrentRequestUrl().LastPathComponent().ToString();
        if (last_path_component.EndsWith(".js") || last_path_component.EndsWith(".mjs")) {
            return true;
        }
    }

    // Exclude `data:`, `blob:` and `filesystem:` URLs from MIME checks.
    if (response.CurrentRequestUrl().ProtocolIsData() || response.CurrentRequestUrl().ProtocolIs(url::kBlobScheme)
    ...
    // Allowed by nosniff?
    if (!(ParseContentTypeOptionsHeader(response.HttpHeaderField(http_names::kXContentTypeOptions)) != kContentTypeOptionsNosniff
            || MIMETypeRegistry::IsSupportedJavaScriptMIMEType(mime_type))) {
        console_logger->AddConsoleMessage(mojom::ConsoleMessageSource::kSecurity, mojom::ConsoleMessageLevel::kError,
            "Refused to execute script from '" + response.CurrentRequestUrl().ElidedString() + "' because its MIME type ('" + mime_type
    ...
    // See:
    // https://fetch.spec.whatwg.org/#should-response-to-request-be-blocked-due-to-mime-type?
    const bool same_origin = response.GetType() == network::mojom::FetchResponseType::kBasic;
    ...
    bool allow = AllowMimeTypeAsScript(mime_type, same_origin, mime_type_check_mode, counter);
    ...
    if (!allow) {
        console_logger->AddConsoleMessage(mojom::blink::ConsoleMessageSource::kSecurity, mojom::blink::ConsoleMessageLevel::kError,
            "Refused to execute script from '" + response.CurrentRequestUrl().ElidedString() + "' because its MIME type ('" + mime_type
                + "') is not executable.");
    } else if (mime_type_check_mode == MimeTypeCheck::kLaxForWorker) {
        bool strict_allow = AllowMimeTypeAsScript(mime_type, same_origin, MimeTypeCheck::kStrict, counter);
        if (!strict_allow)
...
bool AllowedByNosniff::MimeTypeAsXMLExternalEntity(ConsoleLogger* console_logger, const ResourceResponse& response)
{
    if (ParseContentTypeOptionsHeader(response.HttpHeaderField(http_names::kXContentTypeOptions)) != kContentTypeOptionsNosniff) {
        return true;
    }

    if (MIMETypeRegistry::IsXMLExternalEntityMIMEType(response.HttpContentType())) {
        return true;
    }

    console_logger->AddConsoleMessage(mojom::blink::ConsoleMessageSource::kSecurity, mojom::blink::ConsoleMessageLevel::kError,
        "Refused to load XML external entity from '" + response.CurrentRequestUrl().ElidedString() + "' because its MIME type ('" + response.HttpContentType()
