...
class MockConsoleLogger : public GarbageCollected<MockConsoleLogger>, public ConsoleLogger {
public:
    MOCK_METHOD5(AddConsoleMessageImpl,
        void(mojom::ConsoleMessageSource, mojom::ConsoleMessageLevel, const String&, bool, std::optional<mojom::ConsoleMessageCategory>));
...
{
    RuntimeEnabledFeaturesTestHelpers::ScopedStrictMimeTypesForWorkers feature(GetParam());
    ...
    for (auto& testcase : data) {
        SCOPED_TRACE(testing::Message() << "\n  mime type: " << testcase.mimetype << "\n  allowed: " << (testcase.allowed ? "true" : "false")
                                        << "\n  strict_allowed: " << (testcase.strict_allowed ? "true" : "false"));
        ...
        response.SetHttpHeaderField(http_names::kContentType, AtomicString(testcase.mimetype));

        EXPECT_CALL(*use_counter, CountUse(_)).Times(::testing::AnyNumber());
        if (!testcase.allowed)
            EXPECT_CALL(*logger, AddConsoleMessageImpl(_, _, _, _, _));
        EXPECT_EQ(testcase.allowed, AllowedByNosniff::MimeTypeAsScript(*use_counter, logger, response, MimeTypeCheck::kLaxForElement));
        ::testing::Mock::VerifyAndClear(use_counter);

        EXPECT_CALL(*use_counter, CountUse(_)).Times(::testing::AnyNumber());
        bool expect_allowed = RuntimeEnabledFeatures::StrictMimeTypesForWorkersEnabled() ? testcase.strict_allowed : testcase.allowed;
        if (!expect_allowed)
            EXPECT_CALL(*logger, AddConsoleMessageImpl(_, _, _, _, _));
        EXPECT_EQ(expect_allowed, AllowedByNosniff::MimeTypeAsScript(*use_counter, logger, response, MimeTypeCheck::kLaxForWorker));
        ::testing::Mock::VerifyAndClear(use_counter);

        EXPECT_CALL(*use_counter, CountUse(_)).Times(::testing::AnyNumber());
        if (!testcase.strict_allowed)
            EXPECT_CALL(*logger, AddConsoleMessageImpl(_, _, _, _, _));
        EXPECT_EQ(testcase.strict_allowed, AllowedByNosniff::MimeTypeAsScript(*use_counter, logger, response, MimeTypeCheck::kStrict));
        ::testing::Mock::VerifyAndClear(use_counter);
    }
}
...
TEST_P(AllowedByNosniffTest, Counters)
{
    ...
    struct {
        ...
        { bla, blubb, "text/json", kOpaque, WebFeature::kCrossOriginJsonTypeForScript },
        { bla, blubb, "application/json", kOpaque, WebFeature::kCrossOriginJsonTypeForScript },
        ...
        { blubb, blubb, "application/octet-stream", kBasic, WebFeature::kSameOriginApplicationOctetStream },
        { blubb, blubb, "application/xml", kCors, WebFeature::kCrossOriginApplicationXml },
        { bla, bla, "text/html", kBasic, WebFeature::kSameOriginTextHtml },

        // Unknown
        { bla, bla, "not/script", kBasic, WebFeature::kSameOriginStrictNosniffWouldBlock },
        { bla, blubb, "not/script", kOpaque, WebFeature::kCrossOriginStrictNosniffWouldBlock },
    };
    ...
    for (auto& testcase : data) {
        ...
        EXPECT_CALL(*use_counter, CountUse(::testing::Ne(testcase.expected))).Times(::testing::AnyNumber());
        AllowedByNosniff::MimeTypeAsScript(*use_counter, logger, response, MimeTypeCheck::kLaxForElement);
        ::testing::Mock::VerifyAndClear(use_counter);
        ...
        // legitimate script.
        bool expect_worker_lax = (testcase.expected == WebFeature::kCrossOriginTextScript || testcase.expected == WebFeature::kSameOriginTextScript)
            || !RuntimeEnabledFeatures::StrictMimeTypesForWorkersEnabled();
        EXPECT_CALL(*use_counter, CountUse(testcase.expected)).Times(expect_worker_lax);
        EXPECT_CALL(*use_counter, CountUse(::testing::Ne(testcase.expected))).Times(::testing::AnyNumber());
        AllowedByNosniff::MimeTypeAsScript(*use_counter, logger, response, MimeTypeCheck::kLaxForWorker);
        ::testing::Mock::VerifyAndClear(use_counter);

        // The kStrictMimeTypeChecksWouldBlockWorker counter should only be active
        // is "lax" checking for workers is enabled.
        EXPECT_CALL(*use_counter, CountUse(WebFeature::kStrictMimeTypeChecksWouldBlockWorker))
            .Times(!RuntimeEnabledFeatures::StrictMimeTypesForWorkersEnabled());
        EXPECT_CALL(*use_counter, CountUse(::testing::Ne(WebFeature::kStrictMimeTypeChecksWouldBlockWorker))).Times(::testing::AnyNumber());
        AllowedByNosniff::MimeTypeAsScript(*use_counter, logger, response, MimeTypeCheck::kLaxForWorker);
        ::testing::Mock::VerifyAndClear(use_counter);
    }
}
...
TEST_P(AllowedByNosniffTest, AllTheSchemes)
{
    ...
    for (auto& testcase : data) {
        ...
        MockConsoleLogger* logger = MakeGarbageCollected<MockConsoleLogger>();
        EXPECT_CALL(*logger, AddConsoleMessageImpl(_, _, _, _, _)).Times(::testing::AnyNumber());
        SCOPED_TRACE(testing::Message() << "\n  url: " << testcase.url << "\n  allowed: " << testcase.allowed);
        ResourceResponse response(KURL(testcase.url));
        response.SetHttpHeaderField(http_names::kContentType, AtomicString("invalid"));
        response.SetHttpHeaderField(http_names::kXContentTypeOptions, AtomicString("nosniff"));
        EXPECT_EQ(testcase.allowed, AllowedByNosniff::MimeTypeAsScript(*use_counter, logger, response, MimeTypeCheck::kStrict));
        EXPECT_EQ(testcase.allowed, AllowedByNosniff::MimeTypeAsScript(*use_counter, logger, response, MimeTypeCheck::kLaxForElement));
        EXPECT_EQ(testcase.allowed, AllowedByNosniff::MimeTypeAsScript(*use_counter, logger, response, MimeTypeCheck::kLaxForWorker));
    }
}
...
TEST(AllowedByNosniffTest, XMLExternalEntity)
{
...

    {
        ...
        EXPECT_TRUE(AllowedByNosniff::MimeTypeAsXMLExternalEntity(logger, response));
    }

    {
        ResourceResponse response(KURL("https://example.com/"));
        response.SetHttpHeaderField(http_names::kContentType, AtomicString("text/plain"));
        EXPECT_TRUE(AllowedByNosniff::MimeTypeAsXMLExternalEntity(logger, response));
    }

    {
        ...
        response.SetHttpHeaderField(http_names::kXContentTypeOptions, AtomicString("nosniff"));
        EXPECT_CALL(*logger, AddConsoleMessageImpl(_, _, _, _, _));
        EXPECT_FALSE(AllowedByNosniff::MimeTypeAsXMLExternalEntity(logger, response));
    }
    ...
    {
        ResourceResponse response(KURL("https://example.com/"));
        response.SetHttpHeaderField(http_names::kContentType, AtomicString("application/xml-external-parsed-entity"));
        response.SetHttpHeaderField(http_names::kXContentTypeOptions, AtomicString("nosniff"));
        EXPECT_TRUE(AllowedByNosniff::MimeTypeAsXMLExternalEntity(logger, response));
    }
    ...
    {
        ResourceResponse response(KURL("https://example.com/"));
        response.SetHttpHeaderField(http_names::kContentType, AtomicString("text/xml-external-parsed-entity"));
        response.SetHttpHeaderField(http_names::kXContentTypeOptions, AtomicString("nosniff"));
        EXPECT_TRUE(AllowedByNosniff::MimeTypeAsXMLExternalEntity(logger, response));
    }
}
