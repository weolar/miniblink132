...
class BytesConsumerTestReader final : public GarbageCollected<BytesConsumerTestReader>, public BytesConsumer::Client {
public:
    ...
    {
        return "BytesConsumerTestReader";
    }
    std::pair<BytesConsumer::Result, Vector<char>> Run();
    std::pair<BytesConsumer::Result, Vector<char>> Run(scheduler::FakeTaskRunner*);

    // `max_chunk_size_` can be set to a larger value when efficiency is needed.
    void set_max_chunk_size(size_t bytes)
    {
        max_chunk_size_ = bytes;
    }
