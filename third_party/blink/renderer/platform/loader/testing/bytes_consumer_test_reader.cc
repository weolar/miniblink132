...
void BytesConsumerTestReader::OnStateChange()
{
    while (true) {
        ...
        if (result == BytesConsumer::Result::kOk) {
            wtf_size_t read = static_cast<wtf_size_t>(std::min(max_chunk_size_, buffer.size()));
...
{
    OnStateChange();
    while (result_ != BytesConsumer::Result::kDone && result_ != BytesConsumer::Result::kError)
        test::RunPendingTasks();
    test::RunPendingTasks();
    return std::make_pair(result_, std::move(data_));
}

std::pair<BytesConsumer::Result, Vector<char>> BytesConsumerTestReader::Run(scheduler::FakeTaskRunner* task_runner)
{
    OnStateChange();
    while (result_ != BytesConsumer::Result::kDone && result_ != BytesConsumer::Result::kError)
