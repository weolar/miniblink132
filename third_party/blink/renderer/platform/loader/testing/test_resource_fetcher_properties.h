...
public:
    ...
    {
        DCHECK_NE(GetControllerServiceWorkerMode(), ControllerServiceWorkerMode::kNoController);
    ...
    void SetIsOutermostMainFrame(bool value)
    {
        is_outermost_main_frame_ = value;
    }
    void SetControllerServiceWorkerMode(ControllerServiceWorkerMode mode)
    {
        service_worker_mode_ = mode;
    }
    void SetServiceWorkerId(int64_t id)
    {
        service_worker_id_ = id;
    }
    void SetIsPaused(bool value)
    {
        paused_ = value;
    }
    void SetIsLoadComplete(bool value)
    {
        load_complete_ = value;
    }
    ...
    void SetFrameStatus(scheduler::FrameStatus status)
    {
        frame_status_ = status;
    }

private:
    const Member<const FetchClientSettingsObject> fetch_client_settings_object_;
    bool is_outermost_main_frame_ = false;
    ControllerServiceWorkerMode service_worker_mode_ = ControllerServiceWorkerMode::kNoController;
