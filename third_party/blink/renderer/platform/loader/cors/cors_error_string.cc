...
String GetErrorStringForIssueSummary(const network::CorsErrorStatus& status, const AtomicString& initiator_name)
{
    StringBuilder builder;
    static constexpr char kNoCorsInformation[] = " Have the server send the header with a valid value, or, if an opaque "
                                                 "response serves your needs, set the request's mode to 'no-cors' to "
                                                 "fetch the resource with CORS disabled.";

    using CorsError = network::mojom::CorsError;
    const StringView hint(status.failed_parameter.data(), base::checked_cast<wtf_size_t>(status.failed_parameter.size()));

    builder.Append("Access blocked by CORS policy: ");
    if (IsPreflightError(status.cors_error)) {
        builder.Append("Response to preflight request doesn't pass access control check: ");
    }
    ...
    case CorsError::kInsecurePrivateNetwork:
        Append(builder,
            { "The request client is not a secure context and the "
    ...
    case CorsError::kPreflightWildcardOriginNotAllowed:
        builder.Append("The value of the 'Access-Control-Allow-Origin' header in the "
                       "response must not be the wildcard '*' when the request's "
                       "credentials mode is 'include'.");
        if (initiator_name == fetch_initiator_type_names::kXmlhttprequest) {
            builder.Append(" The credentials mode of requests initiated by the "
                           "XMLHttpRequest is controlled by the withCredentials attribute.");
        }
        break;
    case CorsError::kMissingAllowOriginHeader:
    case CorsError::kPreflightMissingAllowOriginHeader:
        builder.Append("No 'Access-Control-Allow-Origin' header is present on the "
                       "requested resource.");
        if (initiator_name == fetch_initiator_type_names::kFetch) {
            builder.Append(" If an opaque response serves your needs, set the request's "
    ...
    case CorsError::kPreflightAllowOriginMismatch:
        Append(builder, { "The 'Access-Control-Allow-Origin' header has a value '", EncodeHint(hint), "' that is not equal to the supplied origin." });
        if (initiator_name == fetch_initiator_type_names::kFetch) {
            builder.Append(kNoCorsInformation);
        }
        break;
    ...
    case CorsError::kCorsDisabledScheme:
        Append(builder, { "Cross origin requests are only supported for protocol schemes: ", SchemeRegistry::ListOfCorsEnabledURLSchemes(), "." });
        break;
    ...
    case CorsError::kPreflightMissingAllowPrivateNetwork:
        Append(builder,
            { "No 'Access-Control-Allow-Private-Network' header "
    ...
    case CorsError::kPreflightInvalidAllowPrivateNetwork:
        Append(builder,
            { "The 'Access-Control-Allow-Private-Network' header in the "
              ...
              "the `",
                ShortAddressSpace(status.target_address_space), "` address space had a value of '", EncodeHint(hint), "',  not 'true'." });
        break;
    case CorsError::kInvalidAllowMethodsPreflightResponse:
        builder.Append("Cannot parse Access-Control-Allow-Methods response header field in "
                       "preflight response.");
        break;
    case CorsError::kInvalidAllowHeadersPreflightResponse:
        builder.Append("Cannot parse Access-Control-Allow-Headers response header field in "
                       "preflight response.");
        break;
    case CorsError::kMethodDisallowedByPreflightResponse:
        Append(builder,
            { "Method ", EncodeHint(hint),
                " is not allowed by Access-Control-Allow-Methods in "
                "preflight response." });
        break;
    case CorsError::kHeaderDisallowedByPreflightResponse:
        Append(builder,
            { "Request header field ", EncodeHint(hint),
                " is not allowed by "
                "Access-Control-Allow-Headers in preflight response." });
        break;
    case CorsError::kRedirectContainsCredentials:
        Append(builder,
            { "Redirect location '", EncodeHint(hint),
                "' contains a username and password, which is "
                "disallowed for cross-origin requests." });
        break;
    case CorsError::kInvalidPrivateNetworkAccess:
        Append(builder,
            { "Request had a target IP address space of `", ShortAddressSpace(status.target_address_space), "` yet the resource is in address space `",
                ShortAddressSpace(status.resource_address_space), "`." });
        break;
    case CorsError::kUnexpectedPrivateNetworkAccess:
        Append(builder,
            { "Request had no target IP address space, yet the "
    ...
    case CorsError::kPrivateNetworkAccessPermissionUnavailable:
        Append(builder, { "Unable to ask for permission to access the `", ShortAddressSpace(status.target_address_space), "` IP address space." });
        break;
    case CorsError::kPrivateNetworkAccessPermissionDenied:
        Append(builder, { "Permission was denied for this request to access the `", ShortAddressSpace(status.target_address_space), "` address space." });
    }
    return builder.ToString();
}

String GetErrorStringForConsoleMessage(const network::CorsErrorStatus& status, const KURL& initial_request_url, const KURL& last_request_url,
    const SecurityOrigin& origin, ResourceType resource_type, const AtomicString& initiator_name)
{
    StringBuilder builder;
    static constexpr char kNoCorsInformation[] = " Have the server send the header with a valid value, or, if an opaque "
                                                 "response serves your needs, set the request's mode to 'no-cors' to "
    ...
    const char* resource_kind_raw = Resource::ResourceTypeToString(resource_type, initiator_name);
    ...
    Append(builder, { "Access to ", resource_kind, " at '", last_request_url.GetString(), "' " });
    if (initial_request_url != last_request_url) {
        Append(builder, { "(redirected from '", initial_request_url.GetString(), "') " });
    }
    Append(builder, { "from origin '", origin.ToString(), "' has been blocked by CORS policy: " });
