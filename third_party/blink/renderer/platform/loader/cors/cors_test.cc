...
public:
    ...
    HTTPHeaderSet Parse(CredentialsMode credentials_mode, const char* header) const
    {
        return Parse(credentials_mode, AtomicString(header));
    }

    HTTPHeaderSet Parse(CredentialsMode credentials_mode, const AtomicString& header) const
    {
        ResourceResponse response;
        response.AddHttpHeaderField(http_names::kAccessControlExposeHeaders, header);
...
TEST_F(CorsExposedHeadersTest, DuplicatedEntries)
{
    ...
    EXPECT_EQ(Parse(CredentialsMode::kOmit, "a, a, b"), HTTPHeaderSet({ "a", "b" }));
}
...
TEST_F(CorsExposedHeadersTest, InvalidInput)
{
    ...
    EXPECT_TRUE(Parse(CredentialsMode::kOmit, AtomicString(String::FromUTF8("\xC5\x81"))).empty());
}
...
{
    ResourceResponse response;
    response.AddHttpHeaderField(http_names::kAccessControlExposeHeaders, AtomicString("a, b, *"));
    ...
    EXPECT_EQ(
        cors::ExtractCorsExposedHeaderNamesList(CredentialsMode::kOmit, response), HTTPHeaderSet({ "Access-Control-Expose-Headers", "b", "c", "d", "*" }));

    EXPECT_EQ(cors::ExtractCorsExposedHeaderNamesList(CredentialsMode::kSameOrigin, response),
...
TEST_F(CorsExposedHeadersTest, Asterisk)
{
    ...
    EXPECT_EQ(cors::ExtractCorsExposedHeaderNamesList(CredentialsMode::kInclude, response), HTTPHeaderSet({ "a", "b", "*" }));
}
