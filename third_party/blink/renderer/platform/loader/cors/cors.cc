...
private:
    void ConsumePermittedCharacters(base::RepeatingCallback<bool(UChar)> is_permitted)
    {
        while (true) {
            if (pos_ == value_.length())
                return;
    ...
    void ConsumeSpaces()
    {
        ConsumePermittedCharacters(base::BindRepeating([](UChar c) { return c == ' ' || c == '\t'; }));
    }

    // Consumes zero or more comma from value_.
    void ConsumeComma()
    {
        ConsumePermittedCharacters(base::BindRepeating([](UChar c) { return c == ','; }));
    }

    // Consumes zero or more tchars from value_.
    void ConsumeTokenChars()
    {
        ConsumePermittedCharacters(base::BindRepeating([](UChar c) { return c <= 0x7F && net::HttpUtil::IsTokenChar(c); }));
    }
...
bool ContainsOnlyCorsSafelistedHeaders(const HTTPHeaderMap& header_map)
{
    ...
    for (const auto& entry : header_map) {
        in.push_back(net::HttpRequestHeaders::HeaderKeyValuePair(entry.key.Latin1(), entry.value.Latin1()));
    }

    return network::cors::CorsUnsafeRequestHeaderNames(in).empty();
}

bool CalculateCorsFlag(
    const KURL& url, const SecurityOrigin* initiator_origin, const SecurityOrigin* isolated_world_origin, network::mojom::RequestMode request_mode)
{
...
HTTPHeaderSet ExtractCorsExposedHeaderNamesList(network::mojom::CredentialsMode credentials_mode, const ResourceResponse& response)
{
    ...
    HTTPHeaderSet header_set;
    HTTPHeaderNameListParser parser(response.HttpHeaderField(http_names::kAccessControlExposeHeaders));
    parser.Parse(header_set);

    if (credentials_mode != network::mojom::CredentialsMode::kInclude && base::Contains(header_set, "*")) {
        header_set.clear();
...
bool IsCorsSafelistedResponseHeader(const String& name)
{
    ...
    DEFINE_THREAD_SAFE_STATIC_LOCAL(HTTPHeaderSet, allowed_cross_origin_response_headers,
        ({
            "cache-control",
            "content-language",
            "content-length",
