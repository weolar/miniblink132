...
public:
    ...
    NonMainThreadSchedulerBase& operator=(const NonMainThreadSchedulerBase&) = delete;
    ...
    virtual void OnTaskCompleted(NonMainThreadTaskQueue* worker_task_queue, const base::sequence_manager::Task& task,
    ...
    scoped_refptr<NonMainThreadTaskQueue> CreateTaskQueue(
        base::sequence_manager::QueueName name, NonMainThreadTaskQueue::QueueCreationParams params = NonMainThreadTaskQueue::QueueCreationParams());
...
protected:
    ...
    // this object.
    explicit NonMainThreadSchedulerBase(base::sequence_manager::SequenceManager* sequence_manager, TaskType default_task_type);

    friend class WorkerSchedulerImpl;

    NonMainThreadSchedulerHelper& GetHelper() override
    {
        return helper_;
    }
