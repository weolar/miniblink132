...
public:
    ...
    NonMainThreadSchedulerHelper(
        base::sequence_manager::SequenceManager* manager, NonMainThreadSchedulerBase* non_main_thread_scheduler, TaskType default_task_type);
    NonMainThreadSchedulerHelper(const NonMainThreadSchedulerHelper&) = delete;
    NonMainThreadSchedulerHelper& operator=(const NonMainThreadSchedulerHelper&) = delete;
    ~NonMainThreadSchedulerHelper() override;

    scoped_refptr<NonMainThreadTaskQueue> NewTaskQueue(const base::sequence_manager::TaskQueue::Spec& spec,
        NonMainThreadTaskQueue::QueueCreationParams params = NonMainThreadTaskQueue::QueueCreationParams());
    ...
    const scoped_refptr<base::SingleThreadTaskRunner>& ControlTaskRunner() override;
...
private:
    ...
    scoped_refptr<NonMainThreadTaskQueue> NewTaskQueueInternal(const base::sequence_manager::TaskQueue::Spec& spec,
        NonMainThreadTaskQueue::QueueCreationParams params = NonMainThreadTaskQueue::QueueCreationParams());
