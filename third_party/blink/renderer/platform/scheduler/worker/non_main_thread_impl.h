...
protected:
    virtual std::unique_ptr<NonMainThreadSchedulerBase> CreateNonMainThreadScheduler(base::sequence_manager::SequenceManager* sequence_manager);

private:
    class GCSupport;
    class SimpleThreadImpl final : public base::SimpleThread {
    public:
        using NonMainThreadSchedulerFactory
            = base::OnceCallback<std::unique_ptr<scheduler::NonMainThreadSchedulerBase>(base::sequence_manager::SequenceManager*)>;

        explicit SimpleThreadImpl(const WTF::String& name_prefix, const base::SimpleThread::Options& options, base::TimeDelta realtime_period, bool supports_gc,
            NonMainThreadImpl* worker_thread, base::MessagePumpType message_pump_type);
    ...
    private:
        ...
        std::unique_ptr<scheduler::NonMainThreadSchedulerBase> non_main_thread_scheduler_;
...
#if BUILDFLAG(IS_APPLE)
    FRIEND_TEST_ALL_PREFIXES(NonMainThreadImplRealtimePeriodTest, RealtimePeriodConfiguration);
#endif
};
