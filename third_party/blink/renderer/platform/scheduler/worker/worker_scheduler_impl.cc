...
std::unique_ptr<WorkerScheduler> WorkerScheduler::CreateWorkerScheduler(WorkerThreadScheduler* worker_thread_scheduler, WorkerSchedulerProxy* proxy)
{
    return std::make_unique<WorkerSchedulerImpl>(worker_thread_scheduler, proxy);
}

WorkerSchedulerImpl::PauseHandleImpl::PauseHandleImpl(base::WeakPtr<WorkerSchedulerImpl> scheduler)
...
WorkerSchedulerImpl::WorkerSchedulerImpl(WorkerThreadScheduler* worker_thread_scheduler, WorkerSchedulerProxy* proxy)
    : throttleable_task_queue_(worker_thread_scheduler->CreateTaskQueue(
        base::sequence_manager::QueueName::WORKER_THROTTLEABLE_TQ, NonMainThreadTaskQueue::QueueCreationParams().SetCanBeThrottled(true)))
    , pausable_task_queue_(worker_thread_scheduler->CreateTaskQueue(base::sequence_manager::QueueName::WORKER_PAUSABLE_TQ))
    , pausable_non_vt_task_queue_(worker_thread_scheduler->CreateTaskQueue(base::sequence_manager::QueueName::WORKER_PAUSABLE_TQ))
    , unpausable_task_queue_(worker_thread_scheduler->CreateTaskQueue(base::sequence_manager::QueueName::WORKER_UNPAUSABLE_TQ))
    , thread_scheduler_(worker_thread_scheduler)
    , back_forward_cache_disabling_feature_tracker_(&tracing_controller_, thread_scheduler_)
{
    task_runners_.emplace(throttleable_task_queue_, throttleable_task_queue_->CreateQueueEnabledVoter());
    task_runners_.emplace(pausable_task_queue_, pausable_task_queue_->CreateQueueEnabledVoter());
    task_runners_.emplace(pausable_non_vt_task_queue_, pausable_non_vt_task_queue_->CreateQueueEnabledVoter());
...
{
    if (!thread_scheduler_->wake_up_budget_pool() && !thread_scheduler_->cpu_time_budget_pool()) {
        return;
    }
    base::TimeTicks now = thread_scheduler_->GetTickClock()->NowTicks();

    WakeUpBudgetPool* wake_up_budget_pool = thread_scheduler_->wake_up_budget_pool();
    CPUTimeBudgetPool* cpu_time_budget_pool = thread_scheduler_->cpu_time_budget_pool();
...
SchedulingLifecycleState WorkerSchedulerImpl::CalculateLifecycleState(ObserverType) const
{
...
scoped_refptr<base::SingleThreadTaskRunner> WorkerSchedulerImpl::GetTaskRunner(TaskType type) const
{
    switch (type) {
    ...
    case TaskType::kNetworkingUnfreezableRenderBlockingLoading:
        return IsInflightNetworkRequestBackForwardCacheSupportEnabled() ? unpausable_task_queue_->CreateTaskRunner(type)
...
void WorkerSchedulerImpl::OnLifecycleStateChanged(SchedulingLifecycleState lifecycle_state)
{
    ...
    if (thread_scheduler_->cpu_time_budget_pool() || thread_scheduler_->wake_up_budget_pool()) {
...
scoped_refptr<NonMainThreadTaskQueue> WorkerSchedulerImpl::UnpausableTaskQueue()
{
...
scoped_refptr<NonMainThreadTaskQueue> WorkerSchedulerImpl::ThrottleableTaskQueue()
{
    return throttleable_task_queue_.get();
}

void WorkerSchedulerImpl::OnStartedUsingNonStickyFeature(SchedulingPolicy::Feature feature, const SchedulingPolicy& policy,
...
void WorkerSchedulerImpl::OnStoppedUsingNonStickyFeature(SchedulingAffectingFeatureHandle* handle)
{
...
base::WeakPtr<FrameOrWorkerScheduler> WorkerSchedulerImpl::GetFrameOrWorkerSchedulerWeakPtr()
{
    return weak_factory_.GetWeakPtr();
}

std::unique_ptr<WebSchedulingTaskQueue> WorkerSchedulerImpl::CreateWebSchedulingTaskQueue(WebSchedulingQueueType queue_type, WebSchedulingPriority priority)
{
    scoped_refptr<NonMainThreadTaskQueue> task_queue = thread_scheduler_->CreateTaskQueue(base::sequence_manager::QueueName::WORKER_WEB_SCHEDULING_TQ,
        NonMainThreadTaskQueue::QueueCreationParams().SetWebSchedulingQueueType(queue_type).SetWebSchedulingPriority(priority));
    return std::make_unique<NonMainThreadWebSchedulingTaskQueueImpl>(std::move(task_queue));
}

scoped_refptr<base::SingleThreadTaskRunner> WorkerSchedulerImpl::CompositorTaskRunner()
{
    return thread_scheduler_->CompositorTaskRunner();
}

WebScopedVirtualTimePauser WorkerSchedulerImpl::CreateWebScopedVirtualTimePauser(const String& name, WebScopedVirtualTimePauser::VirtualTaskDuration duration)
{
