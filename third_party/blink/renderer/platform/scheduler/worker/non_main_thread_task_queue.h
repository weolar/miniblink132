...
class PLATFORM_EXPORT NonMainThreadTaskQueue : public ThreadSafeRefCounted<NonMainThreadTaskQueue> {
public:
    ...
    NonMainThreadTaskQueue(base::sequence_manager::SequenceManager& sequence_manager, const TaskQueue::Spec& spec,
    ...
    void OnTaskCompleted(const base::sequence_manager::Task& task, base::sequence_manager::TaskQueue::TaskTiming* task_timing, base::LazyNow* lazy_now);

    scoped_refptr<base::SingleThreadTaskRunner> CreateTaskRunner(TaskType task_type);

    bool IsThrottled() const
    {
        return throttler_->IsThrottled();
    }
    ...
    const scoped_refptr<base::SingleThreadTaskRunner>& GetTaskRunnerWithDefaultTaskType() const
    {
