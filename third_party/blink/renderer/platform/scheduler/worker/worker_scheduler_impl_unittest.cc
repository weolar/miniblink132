...
void RunChainedTask(scoped_refptr<NonMainThreadTaskQueue> task_queue, int count, base::TimeDelta duration,
    scoped_refptr<base::TestMockTimeTaskRunner> environment, Vector<base::TimeTicks>* tasks)
{
    ...
    task_queue->GetTaskRunnerWithDefaultTaskType()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&RunChainedTask, task_queue, count - 1, duration, environment, base::Unretained(tasks)), base::Milliseconds(50));
}
...
public:
    ...
    // object.
    WorkerThreadSchedulerForTest(ThreadType thread_type, base::sequence_manager::SequenceManager* manager, WorkerSchedulerProxy* proxy)
...
public:
    explicit WorkerSchedulerForTest(WorkerThreadSchedulerForTest* thread_scheduler)
...
public:
    WorkerSchedulerImplTest()
        ...
        , sequence_manager_(base::sequence_manager::SequenceManagerForTest::Create(nullptr, mock_task_runner_, mock_task_runner_->GetMockTickClock(),
              base::sequence_manager::SequenceManager::Settings::Builder().SetPrioritySettings(CreatePrioritySettings()).Build()))
        , scheduler_(new WorkerThreadSchedulerForTest(ThreadType::kTestThread, sequence_manager_.get(), nullptr /* proxy */))
    {
    ...
    void SetUp() override
    {
        ...
        worker_scheduler_ = std::make_unique<WorkerSchedulerForTest>(scheduler_.get());
    }
    ...
    void PostTestTask(Vector<String>* run_order, const String& task_descriptor, TaskType task_type)
    {
        PostTestTask(run_order, task_descriptor, *worker_scheduler_->GetTaskRunner(task_type).get());
    }

    void PostTestTask(Vector<String>* run_order, const String& task_descriptor, base::SingleThreadTaskRunner& task_runner)
    {
        task_runner.PostTask(FROM_HERE, WTF::BindOnce(&AppendToVectorTestTask, WTF::Unretained(run_order), task_descriptor));
    }
...
TEST_F(WorkerSchedulerImplTest, TestPostTasks)
{
    ...
    // disposal.
    scoped_refptr<base::SingleThreadTaskRunner> test_task_runner = worker_scheduler_->GetTaskRunner(TaskType::kInternalTest);
...
{
    EXPECT_THAT(scheduler_->worker_schedulers(), testing::ElementsAre(worker_scheduler_.get()));

    std::unique_ptr<WorkerSchedulerForTest> worker_scheduler2 = std::make_unique<WorkerSchedulerForTest>(scheduler_.get());

    EXPECT_THAT(scheduler_->worker_schedulers(), testing::UnorderedElementsAre(worker_scheduler_.get(), worker_scheduler2.get()));

    worker_scheduler_->Dispose();
    worker_scheduler_.reset();

    EXPECT_THAT(scheduler_->worker_schedulers(), testing::ElementsAre(worker_scheduler2.get()));
...
TEST_F(WorkerSchedulerImplTest, ThrottleWorkerScheduler_RunThrottledTasks)
{
    ...
    worker_scheduler_->ThrottleableTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(FROM_HERE,
        base::BindOnce(&RunChainedTask, worker_scheduler_->ThrottleableTaskQueue(), 5, base::TimeDelta(), mock_task_runner_, base::Unretained(&tasks)));

    RunUntilIdle();

    EXPECT_THAT(tasks,
        ElementsAre(base::TimeTicks() + base::Seconds(1), base::TimeTicks() + base::Seconds(2), base::TimeTicks() + base::Seconds(3),
            base::TimeTicks() + base::Seconds(4), base::TimeTicks() + base::Seconds(5)));
}

TEST_F(WorkerSchedulerImplTest, ThrottleWorkerScheduler_RunThrottledTasks_CPUBudget)
{
    scheduler_->CreateBudgetPools();

    scheduler_->cpu_time_budget_pool()->SetTimeBudgetRecoveryRate(GetClock()->NowTicks(), 0.01);
    ...
    worker_scheduler_->ThrottleableTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(FROM_HERE,
        base::BindOnce(&RunChainedTask, worker_scheduler_->ThrottleableTaskQueue(), 5, base::Milliseconds(100), mock_task_runner_, base::Unretained(&tasks)));

    RunUntilIdle();

    EXPECT_THAT(tasks,
        ElementsAre(base::TimeTicks() + base::Seconds(1), start_time_ + base::Seconds(10), start_time_ + base::Seconds(20), start_time_ + base::Seconds(30),
...
{
    auto delegate = std::make_unique<testing::StrictMock<WorkerSchedulerDelegateForTesting>>();
    ...
    worker_scheduler_->GetTaskRunner(TaskType::kJavascriptTimerImmediate)
        ->PostTask(FROM_HERE,
            base::BindOnce(
                [](WorkerSchedulerImpl* worker_scheduler, testing::StrictMock<WorkerSchedulerDelegateForTesting>* delegate) {
...
class NonMainThreadWebSchedulingTaskQueueTest : public WorkerSchedulerImplTest, public WebSchedulingTestHelper::Delegate {
public:
    void SetUp() override
    {
...
TEST_F(NonMainThreadWebSchedulingTaskQueueTest, WebScheduingAndNonWebScheduingTasks)
{
    Vector<String> run_order;
    Vector<TestTaskSpecEntry> test_spec = { { .descriptor = "PostMessage", .type_info = TaskType::kPostedMessage },
        { .descriptor = "BG",
            .type_info = WebSchedulingParams({ .queue_type = WebSchedulingQueueType::kTaskQueue, .priority = WebSchedulingPriority::kBackgroundPriority }) },
        { .descriptor = "BG-C",
            .type_info
        ...
        { .descriptor = "UB-C",
            .type_info
            = WebSchedulingParams({ .queue_type = WebSchedulingQueueType::kContinuationQueue, .priority = WebSchedulingPriority::kUserBlockingPriority }) },
        { .descriptor = "Timer", .type_info = TaskType::kJavascriptTimerImmediate } };
    web_scheduling_test_helper_->PostTestTasks(&run_order, test_spec);

    RunUntilIdle();
    EXPECT_THAT(run_order, testing::ElementsAre("UB-C", "UB", "UV-C", "PostMessage", "UV", "Timer", "BG-C", "BG"));
}
...
{
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = worker_scheduler_->GetTaskRunner(TaskType::kInternalTest);
    int counter = 0;
