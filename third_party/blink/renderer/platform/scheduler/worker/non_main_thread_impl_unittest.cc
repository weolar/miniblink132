...
public:
    explicit TestObserver(StringBuilder* calls)
        : calls_(calls)
    {
    }
...
public:
    ...
    void SetUp() override
    {
        thread_ = NonMainThread::CreateThread(ThreadCreationParams(ThreadType::kTestThread));
    }

    void RunOnWorkerThread(const base::Location& from_here, base::OnceClosure task)
    {
        ...
        thread_->GetTaskRunner()->PostTask(
            from_here, base::BindOnce(&NonMainThreadImplTest::RunOnWorkerThreadTask, base::Unretained(this), std::move(task), &completion));
        completion.Wait();
    }

protected:
    void RunOnWorkerThreadTask(base::OnceClosure task, base::WaitableEvent* completion)
    {
        std::move(task).Run();
        completion->Signal();
    }
...
TEST_F(NonMainThreadImplTest, TestDefaultTask)
{
    ...
    ON_CALL(task, Run()).WillByDefault(Invoke([&completion]() { completion.Signal(); }));

    PostCrossThreadTask(*thread_->GetTaskRunner(), FROM_HERE, CrossThreadBindOnce(&MockTask::Run, WTF::CrossThreadUnretained(&task)));
    completion.Wait();
}
...
TEST_F(NonMainThreadImplTest, TestTaskObserver)
{
    ...
    RunOnWorkerThread(FROM_HERE, base::BindOnce(&AddTaskObserver, thread_.get(), &observer));
    PostCrossThreadTask(*thread_->GetTaskRunner(), FROM_HERE, CrossThreadBindOnce(&RunTestTask, WTF::CrossThreadUnretained(&calls)));
    RunOnWorkerThread(FROM_HERE, base::BindOnce(&RemoveTaskObserver, thread_.get(), &observer));
    ...
    EXPECT_THAT(calls.ToString().Utf8(), testing::HasSubstr("willProcessTask run didProcessTask"));
}
...
TEST_F(NonMainThreadImplTest, TestShutdown)
{
    ...
    RunOnWorkerThread(FROM_HERE, base::BindOnce(&ShutdownOnThread, thread_.get()));
    PostCrossThreadTask(*thread_->GetTaskRunner(), FROM_HERE, CrossThreadBindOnce(&MockTask::Run, WTF::CrossThreadUnretained(&task)));
    PostDelayedCrossThreadTask(
        *thread_->GetTaskRunner(), FROM_HERE, CrossThreadBindOnce(&MockTask::Run, WTF::CrossThreadUnretained(&delayed_task)), base::Milliseconds(50));
    thread_.reset();
}
...
TEST(NonMainThreadImplRealtimePeriodTest, RealtimePeriodConfiguration)
{
    ...
    EXPECT_EQ(static_cast<base::PlatformThread::Delegate*>(non_main_thread->thread_.get())->GetRealtimePeriod(), base::TimeDelta());
    ...
    EXPECT_EQ(static_cast<base::PlatformThread::Delegate*>(non_main_thread->thread_.get())->GetRealtimePeriod(), params.realtime_period);
}
#endif
