...
scheduler::CompositorThreadSchedulerImpl* g_compositor_thread_scheduler = nullptr;
...
CompositorThreadSchedulerImpl::CompositorThreadSchedulerImpl(base::sequence_manager::SequenceManager* sequence_manager)
    : NonMainThreadSchedulerBase(sequence_manager, TaskType::kCompositorThreadTaskQueueDefault)
...
scoped_refptr<NonMainThreadTaskQueue> CompositorThreadSchedulerImpl::DefaultTaskQueue()
{
    return GetHelper().DefaultNonMainThreadTaskQueue();
}

void CompositorThreadSchedulerImpl::OnTaskCompleted(NonMainThreadTaskQueue* worker_task_queue, const base::sequence_manager::Task& task,
...
scoped_refptr<scheduler::SingleThreadIdleTaskRunner> CompositorThreadSchedulerImpl::IdleTaskRunner()
{
    ...
    return base::MakeRefCounted<SingleThreadIdleTaskRunner>(GetHelper().DefaultTaskRunner(), GetHelper().ControlTaskRunner(), this);
}

scoped_refptr<base::SingleThreadTaskRunner> CompositorThreadSchedulerImpl::V8TaskRunner()
{
    NOTREACHED();
}

scoped_refptr<base::SingleThreadTaskRunner> CompositorThreadSchedulerImpl::CleanupTaskRunner()
{
    return DefaultTaskQueue()->GetTaskRunnerWithDefaultTaskType();
}

scoped_refptr<base::SingleThreadTaskRunner> CompositorThreadSchedulerImpl::InputTaskRunner()
{
    return GetHelper().InputTaskRunner();
}

scoped_refptr<base::SingleThreadTaskRunner> CompositorThreadSchedulerImpl::DefaultTaskRunner()
{
...
void CompositorThreadSchedulerImpl::AddTaskObserver(base::TaskObserver* task_observer)
{
    GetHelper().AddTaskObserver(task_observer);
}

void CompositorThreadSchedulerImpl::RemoveTaskObserver(base::TaskObserver* task_observer)
{
...
void CompositorThreadSchedulerImpl::PostIdleTask(const base::Location& location, Thread::IdleTask task)
{
    IdleTaskRunner()->PostIdleTask(location, std::move(task));
}

void CompositorThreadSchedulerImpl::PostNonNestableIdleTask(const base::Location& location, Thread::IdleTask task)
{
    IdleTaskRunner()->PostNonNestableIdleTask(location, std::move(task));
}

void CompositorThreadSchedulerImpl::PostDelayedIdleTask(const base::Location& location, base::TimeDelta delay, Thread::IdleTask task)
{
    IdleTaskRunner()->PostDelayedIdleTask(location, delay, std::move(task));
}

base::TimeTicks CompositorThreadSchedulerImpl::MonotonicallyIncreasingVirtualTime()
{
    return NowTicks();
}
