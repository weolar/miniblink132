...
TEST_F(MainThreadPerfTest, PostTaskPerformance)
{
    ...
    auto counter_closure = base::BarrierClosure(kTaskCount, run_loop.QuitClosure());
    ...
    perf_test::PerfResultReporter reporter(kMetricPrefix, "main_thread_post_task");
    ...
    size_t ns_per_post_task = static_cast<size_t>((after_post_task - before).InNanoseconds() / kTaskCount);
    reporter.AddResult(kTimePerPostTask, ns_per_post_task);

    size_t ns_per_iteration = static_cast<size_t>((after - before).InNanoseconds() / kTaskCount);
    reporter.AddResult(kTimePerTask, ns_per_iteration);

    size_t ns_per_task_iteration = static_cast<size_t>((after - after_post_task).InNanoseconds() / kTaskCount);
