...
public:
    ...
    void SetUp() override
    {
        ...
        scheduler_ = std::make_unique<MainThreadSchedulerImpl>(
            base::sequence_manager::CreateSequenceManagerOnCurrentThreadWithPump(base::MessagePump::Create(base::MessagePumpType::DEFAULT),
                base::sequence_manager::SequenceManager::Settings::Builder().SetTickClock(&clock_).SetPrioritySettings(CreatePrioritySettings()).Build()));
        scheduler_overrider_ = std::make_unique<ScopedSchedulerOverrider>(scheduler_.get(), scheduler_->DefaultTaskRunner());
    ...
    {
        scheduler_->GetSchedulerHelperForTesting()->SetWorkBatchSizeForTesting(work_batch_size);
    }
...
TEST_F(MainThreadTest, TestTaskObserver)
{
    ...
    scheduler_->DefaultTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&MockTask::Run, WTF::Unretained(&task)));
...
TEST_F(MainThreadTest, TestWorkBatchWithTwoTasks)
{
    ...
    scheduler_->DefaultTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&MockTask::Run, WTF::Unretained(&task1)));
    scheduler_->DefaultTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&MockTask::Run, WTF::Unretained(&task2)));
    base::RunLoop().RunUntilIdle();
    thread_->RemoveTaskObserver(&observer);
}
...
TEST_F(MainThreadTest, TestWorkBatchWithThreeTasks)
{
    ...
    scheduler_->DefaultTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&MockTask::Run, WTF::Unretained(&task1)));
    scheduler_->DefaultTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&MockTask::Run, WTF::Unretained(&task2)));
    scheduler_->DefaultTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&MockTask::Run, WTF::Unretained(&task3)));
    base::RunLoop().RunUntilIdle();
    thread_->RemoveTaskObserver(&observer);
}
...
void EnterRunLoop(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    ...
    task_runner->PostTask(FROM_HERE, WTF::BindOnce(&base::RunLoop::Quit, WTF::Unretained(&run_loop)));
    run_loop.Run();
}
...
TEST_F(MainThreadTest, TestNestedRunLoop)
{
    ...
    scheduler_->DefaultTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&EnterRunLoop, scheduler_->DefaultTaskRunner()));
    base::RunLoop().RunUntilIdle();
    thread_->RemoveTaskObserver(&observer);
}
