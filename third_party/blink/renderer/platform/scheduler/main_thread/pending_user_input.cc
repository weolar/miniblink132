...
void PendingUserInput::Monitor::OnEnqueue(WebInputEvent::Type type, const WebInputEventAttribution& attribution)
{
    ...
    auto result = pending_events_.insert(AttributionGroup(attribution), EventCounter());
...
void PendingUserInput::Monitor::OnDequeue(WebInputEvent::Type type, const WebInputEventAttribution& attribution)
{
    DCHECK_NE(type, WebInputEvent::Type::kUndefined);
    DCHECK_LE(type, WebInputEvent::Type::kTypeLast);
...
Vector<WebInputEventAttribution> PendingUserInput::Monitor::Info(bool include_continuous) const
{
    Vector<WebInputEventAttribution> attributions;
    for (const auto& entry : pending_events_) {
        if (entry.value.num_discrete > 0 || (entry.value.num_continuous > 0 && include_continuous)) {
