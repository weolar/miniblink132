...
public:
    static constexpr auto kFindInPageBudgetNotExhaustedPriority = TaskPriority::kVeryHighPriority;
    static constexpr auto kFindInPageBudgetExhaustedPriority = TaskPriority::kNormalPriority;

    explicit FindInPageBudgetPoolController(MainThreadSchedulerImpl* scheduler);
    ~FindInPageBudgetPoolController();

    void OnTaskCompleted(MainThreadTaskQueue* queue, TaskQueue::TaskTiming* task_timing);
