...
public:
    MainThreadMetricsHelper(MainThreadSchedulerImpl* main_thread_scheduler, bool has_cpu_timing_for_each_task, base::TimeTicks now, bool renderer_backgrounded);
...
private:
    ...
    static_assert(static_cast<size_t>(TaskPriority::kPriorityCount) == 11, "Queueing delay histograms must be kept in sync with TaskPriority.");
