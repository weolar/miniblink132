...
public:
    explicit MemoryPurgeManager(scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    void OnPageFrozen(base::MemoryReductionTaskContext called_from = base::MemoryReductionTaskContext::kDelayExpired);
    ...
    static constexpr base::TimeDelta kDefaultMaxTimeToPurgeAfterBackgrounded = base::Minutes(4);

    // Only one second, not to delay, but to make sure that it runs after the
    // non-delayed tasks.
    static constexpr base::TimeDelta kFreezePurgeDelay = base::TimeDelta(base::Seconds(1));
