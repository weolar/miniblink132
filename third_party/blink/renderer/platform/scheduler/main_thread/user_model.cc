...
void UserModel::DidStartProcessingInputEvent(blink::WebInputEvent::Type type, const base::TimeTicks now)
{
    last_input_signal_time_ = now;
    if (type == blink::WebInputEvent::Type::kTouchStart || type == blink::WebInputEvent::Type::kGestureScrollBegin
    ...
    if (type == blink::WebInputEvent::Type::kGestureScrollBegin || type == blink::WebInputEvent::Type::kGestureScrollEnd
        || type == blink::WebInputEvent::Type::kGestureScrollUpdate || type == blink::WebInputEvent::Type::kGestureFlingStart
        || type == blink::WebInputEvent::Type::kGestureFlingCancel || type == blink::WebInputEvent::Type::kGesturePinchBegin
        || type == blink::WebInputEvent::Type::kGesturePinchEnd || type == blink::WebInputEvent::Type::kGesturePinchUpdate) {
    ...
    if (type == blink::WebInputEvent::Type::kGestureScrollEnd || type == blink::WebInputEvent::Type::kGesturePinchEnd
        || type == blink::WebInputEvent::Type::kGestureFlingStart || type == blink::WebInputEvent::Type::kTouchEnd) {
        is_gesture_active_ = false;
    }

    TRACE_COUNTER1(TRACE_DISABLED_BY_DEFAULT("renderer.scheduler"), "is_gesture_active", is_gesture_active_);
...
base::TimeDelta UserModel::TimeLeftInContinuousUserGesture(base::TimeTicks now) const
{
    ...
    if (last_input_signal_time_.is_null() || last_input_signal_time_ + kGestureEstimationLimit < now) {
        return base::TimeDelta();
    }
    return last_input_signal_time_ + kGestureEstimationLimit - now;
}

base::TimeDelta UserModel::TimeLeftUntilDiscreteInputResponseDeadline(base::TimeTicks now) const
{
    if (last_discrete_input_time_.is_null() || last_discrete_input_time_ + kDiscreteInputResponseDeadline < now) {
        return base::TimeDelta();
    }
    return last_discrete_input_time_ + kDiscreteInputResponseDeadline - now;
}

bool UserModel::IsGestureExpectedSoon(const base::TimeTicks now, base::TimeDelta* prediction_valid_duration)
{
...
bool UserModel::IsGestureExpectedSoonImpl(const base::TimeTicks now, base::TimeDelta* prediction_valid_duration) const
{
    if (is_gesture_active_) {
        ...
        *prediction_valid_duration = kExpectSubsequentGestureDeadline;
        ...
        if (last_continuous_gesture_time_.is_null() || last_continuous_gesture_time_ + kExpectSubsequentGestureDeadline <= now) {
            return false;
        }
        *prediction_valid_duration = last_continuous_gesture_time_ + kExpectSubsequentGestureDeadline - now;
        return true;
    }
}

bool UserModel::IsGestureExpectedToContinue(const base::TimeTicks now, base::TimeDelta* prediction_valid_duration) const
{
    if (!is_gesture_active_)
        return false;

    base::TimeTicks expected_gesture_end_time = last_gesture_start_time_ + kMedianGestureDuration;
...
void UserModel::WriteIntoTrace(perfetto::TracedValue context) const
{
    ...
    dict.Add("last_gesture_expected_start_time", last_gesture_expected_start_time_);
