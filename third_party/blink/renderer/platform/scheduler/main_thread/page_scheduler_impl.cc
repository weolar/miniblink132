...
constexpr base::TimeDelta kThrottlingDelayAfterBackgrounding = base::Seconds(10);
...
constexpr base::TimeDelta kDefaultDelayForBackgroundTabFreezing = base::Minutes(5);
...
constexpr base::TimeDelta kTimeToInhibitIntensiveThrottlingOnTitleOrFaviconUpdate = base::Seconds(3);

constexpr base::TimeDelta kDefaultDelayForTrackingIPCsPostedToCachedFrames = base::Seconds(15);
...
double GetDoubleParameterFromMap(const base::FieldTrialParams& settings, const std::string& setting_name, double default_value)
{
...
BackgroundThrottlingSettings GetBackgroundThrottlingSettings()
{
    ...
    base::GetFieldTrialParams("ExpensiveBackgroundTimerThrottling", &background_throttling_settings);

    BackgroundThrottlingSettings settings;

    settings.budget_recovery_rate = GetDoubleParameterFromMap(background_throttling_settings, "cpu_budget", kDefaultBackgroundBudgetAsCPUFraction);

    settings.max_budget_level
        = DoubleToOptionalTime(GetDoubleParameterFromMap(background_throttling_settings, "max_budget", kDefaultMaxBackgroundBudgetLevelInSeconds));

    settings.max_throttling_delay
        = DoubleToOptionalTime(GetDoubleParameterFromMap(background_throttling_settings, "max_delay", kDefaultMaxBackgroundThrottlingDelayInSeconds));

    settings.initial_budget
        = DoubleToOptionalTime(GetDoubleParameterFromMap(background_throttling_settings, "initial_budget", kDefaultInitialBackgroundBudgetInSeconds));
...
{
    static const base::FeatureParam<int> kDelayForBackgroundTabFreezingMillis { &features::kStopInBackground, "DelayForBackgroundTabFreezingMills",
...
{
    if (base::FeatureList::IsEnabled(features::kLogUnexpectedIPCPostedToBackForwardCachedDocuments)) {
        static const base::FeatureParam<int> kDelayForLoggingUnexpectedIPCPostedToBckForwardCacheMillis {
            &features::kLogUnexpectedIPCPostedToBackForwardCachedDocuments, "delay_before_tracking_ms",
            static_cast<int>(kDefaultDelayForTrackingIPCsPostedToCachedFrames.InMilliseconds())
        };
        return base::Milliseconds(kDelayForLoggingUnexpectedIPCPostedToBckForwardCacheMillis.Get());
    }
...
PageSchedulerImpl::PageSchedulerImpl(PageScheduler::Delegate* delegate, AgentGroupSchedulerImpl& agent_group_scheduler)
    : main_thread_scheduler_(static_cast<MainThreadSchedulerImpl*>(&agent_group_scheduler.GetMainThreadScheduler()))
    ...
    , unimportant_timers_throttled_wake_up_interval_(base::Milliseconds(features::kUnimportantFrameTimersThrottledWakeUpIntervalMills.Get()))
{
    do_throttle_cpu_time_callback_.Reset(base::BindRepeating(&PageSchedulerImpl::DoThrottleCPUTime, base::Unretained(this)));
    do_intensively_throttle_wake_ups_callback_.Reset(base::BindRepeating(&PageSchedulerImpl::DoIntensivelyThrottleWakeUps, base::Unretained(this)));
    reset_had_recent_title_or_favicon_update_.Reset(base::BindRepeating(&PageSchedulerImpl::ResetHadRecentTitleOrFaviconUpdate, base::Unretained(this)));
    on_audio_silent_closure_.Reset(base::BindRepeating(&PageSchedulerImpl::OnAudioSilent, base::Unretained(this)));
}
...
{
    PolicyUpdater policy_updater;
    PageVisibilityState page_visibility = page_visible ? PageVisibilityState::kVisible : PageVisibilityState::kHidden;
...
void PageSchedulerImpl::SetPageFrozenImpl(bool frozen, PolicyUpdater& policy_updater, base::MemoryReductionTaskContext called_from)
{
...
void PageSchedulerImpl::SetPageBackForwardCached(bool is_in_back_forward_cache)
{
    is_stored_in_back_forward_cache_ = is_in_back_forward_cache;

    if (!is_stored_in_back_forward_cache_) {
        TRACE_EVENT_INSTANT("navigation", "PageSchedulerImpl::SetPageBackForwardCached_Restore");
        stored_in_back_forward_cache_timestamp_ = base::TimeTicks();
    } else {
        TRACE_EVENT_INSTANT("navigation", "PageSchedulerImpl::SetPageBackForwardCached_Store");
        stored_in_back_forward_cache_timestamp_ = main_thread_scheduler_->NowTicks();

        // Incorporate a delay of 15 seconds to allow for caching operations to
        // complete before tasks are logged.
        set_ipc_posted_handler_task_ = PostDelayedCancellableTask(*main_thread_scheduler_->ControlTaskRunner(), FROM_HERE,
            base::BindRepeating(&PageSchedulerImpl::SetUpIPCTaskDetection, GetWeakPtr()), GetTimeToDelayIPCTrackingWhileStoredInBackForwardCache());
    }
}
...
void PageSchedulerImpl::RegisterFrameSchedulerImpl(FrameSchedulerImpl* frame_scheduler)
{
...
{
    auto frame_scheduler = std::make_unique<FrameSchedulerImpl>(this, delegate, is_in_embedded_frame_tree, frame_type);
...
void PageSchedulerImpl::AudioStateChanged(bool is_audio_playing)
{
    if (is_audio_playing) {
        ...
        if (IsFrozen()) {
            ...
            main_thread_scheduler_->ControlTaskRunner()->PostDelayedTask(FROM_HERE, on_audio_silent_closure_.GetCallback(), kRecentAudioDelay);
...
void PageSchedulerImpl::OnThrottlingStatusUpdated()
{
    ...
    for (FrameSchedulerImpl* frame_scheduler : frame_schedulers_) {
        opted_out_from_aggressive_throttling |= frame_scheduler->opted_out_from_aggressive_throttling();
    }

    if (opted_out_from_aggressive_throttling_ != opted_out_from_aggressive_throttling) {
        opted_out_from_aggressive_throttling_ = opted_out_from_aggressive_throttling;
        base::LazyNow lazy_now(main_thread_scheduler_->GetTickClock());
...
{
    return base::ranges::any_of(frame_schedulers_, [](const FrameSchedulerImpl* fs) {
        return fs->IsWaitingForContentfulPaint() && !fs->IsInEmbeddedFrameTree() && fs->GetFrameType() == FrameScheduler::FrameType::kMainFrame;
    });
}
...
{
    return base::ranges::any_of(frame_schedulers_, [](const FrameSchedulerImpl* fs) {
        return fs->IsWaitingForMeaningfulPaint() && !fs->IsInEmbeddedFrameTree() && fs->GetFrameType() == FrameScheduler::FrameType::kMainFrame;
    });
}
...
{
    return base::ranges::any_of(frame_schedulers_, [](const FrameSchedulerImpl* fs) {
        return fs->IsLoading() && !fs->IsInEmbeddedFrameTree() && fs->GetFrameType() == FrameScheduler::FrameType::kMainFrame;
    });
}

void PageSchedulerImpl::WriteIntoTrace(perfetto::TracedValue context, base::TimeTicks now) const
{
    ...
    if (cpu_time_budget_pool_) {
        dict.Add("cpu_time_budget_pool", [&](perfetto::TracedValue context) { cpu_time_budget_pool_->WriteIntoTrace(std::move(context), now); });
    }
    if (unimportant_wake_up_budget_pool_) {
        dict.Add("unimportant_wake_up_budget_pool",
            [&](perfetto::TracedValue context) { unimportant_wake_up_budget_pool_->WriteIntoTrace(std::move(context), now); });
    }
    if (hidden_wake_up_budget_pool_) {
        dict.Add("hidden_wake_up_budget_pool", [&](perfetto::TracedValue context) { hidden_wake_up_budget_pool_->WriteIntoTrace(std::move(context), now); });
    }
    if (same_origin_intensive_wake_up_budget_pool_) {
        dict.Add("same_origin_intensive_wake_up_budget_pool",
            [&](perfetto::TracedValue context) { same_origin_intensive_wake_up_budget_pool_->WriteIntoTrace(std::move(context), now); });
    }
    if (cross_origin_intensive_wake_up_budget_pool_) {
        dict.Add("cross_origin_intensive_wake_up_budget_pool",
            [&](perfetto::TracedValue context) { cross_origin_intensive_wake_up_budget_pool_->WriteIntoTrace(std::move(context), now); });
    }

    dict.Add("frame_schedulers", frame_schedulers_);
}

void PageSchedulerImpl::AddQueueToWakeUpBudgetPool(MainThreadTaskQueue* task_queue, WakeUpBudgetPool* wake_up_budget_pool, base::LazyNow* lazy_now)
{
...
void PageSchedulerImpl::RemoveQueueFromWakeUpBudgetPool(MainThreadTaskQueue* task_queue, base::LazyNow* lazy_now)
{
    if (!task_queue->GetWakeUpBudgetPool())
        return;
    task_queue->RemoveFromBudgetPool(lazy_now->Now(), task_queue->GetWakeUpBudgetPool());
    task_queue->SetWakeUpBudgetPool(nullptr);
}

WakeUpBudgetPool* PageSchedulerImpl::GetWakeUpBudgetPool(MainThreadTaskQueue* task_queue, FrameOriginType frame_origin_type, ThrottlingType throttling_type)
{
...
void PageSchedulerImpl::MaybeInitializeBackgroundCPUTimeBudgetPool(base::LazyNow* lazy_now)
{
    if (cpu_time_budget_pool_)
        return;

    cpu_time_budget_pool_ = std::make_unique<CPUTimeBudgetPool>("background", &tracing_controller_, lazy_now->Now());

    BackgroundThrottlingSettings settings = GetBackgroundThrottlingSettings();

    cpu_time_budget_pool_->SetMaxBudgetLevel(lazy_now->Now(), settings.max_budget_level);
    cpu_time_budget_pool_->SetMaxThrottlingDelay(lazy_now->Now(), settings.max_throttling_delay);

    cpu_time_budget_pool_->SetTimeBudgetRecoveryRate(lazy_now->Now(), settings.budget_recovery_rate);

    if (settings.initial_budget) {
        cpu_time_budget_pool_->GrantAdditionalBudget(lazy_now->Now(), settings.initial_budget.value());
    }

    UpdateCPUTimeBudgetPool(lazy_now);
}

void PageSchedulerImpl::MaybeInitializeWakeUpBudgetPools(base::LazyNow* lazy_now)
{
    if (HasWakeUpBudgetPools())
        return;

    unimportant_wake_up_budget_pool_ = std::make_unique<WakeUpBudgetPool>("Page - Foreground Wake Up Throttling - Visible Unimportant & "
                                                                          "Cross-Origin to Main Frame");
    hidden_wake_up_budget_pool_ = std::make_unique<WakeUpBudgetPool>("Page - Hidden Wake Up Throttling - Hidden & Cross-Origin to Main "
                                                                     "Frame, Or Background Page");
    same_origin_intensive_wake_up_budget_pool_ = std::make_unique<WakeUpBudgetPool>("Page - Intensive Wake Up Throttling - Same-Origin as Main Frame");
    cross_origin_intensive_wake_up_budget_pool_ = std::make_unique<WakeUpBudgetPool>("Page - Intensive Wake Up Throttling - Cross-Origin to Main Frame");
    ...
    same_origin_intensive_wake_up_budget_pool_->AllowLowerAlignmentIfNoRecentWakeUp(kDefaultThrottledWakeUpInterval);

    UpdateWakeUpBudgetPools(lazy_now);
}

void PageSchedulerImpl::UpdatePolicyOnVisibilityChange(PolicyUpdater& policy_updater)
{
    base::LazyNow lazy_now(main_thread_scheduler_->GetTickClock());
    ...
    if (IsPageVisible()) {
        ...
        if (cpu_time_budget_pool_) {
            main_thread_scheduler_->ControlTaskRunner()->PostDelayedTask(
                FROM_HERE, do_throttle_cpu_time_callback_.GetCallback(), kThrottlingDelayAfterBackgrounding);
        }
        main_thread_scheduler_->ControlTaskRunner()->PostDelayedTask(
            FROM_HERE, do_intensively_throttle_wake_ups_callback_.GetCallback(), GetIntensiveWakeUpThrottlingGracePeriod(IsLoading()));
    }
...
void PageSchedulerImpl::OnTitleOrFaviconUpdated()
{
    if (!HasWakeUpBudgetPools())
        return;
    if (are_wake_ups_intensively_throttled_ && !opted_out_from_aggressive_throttling_) {
        ...
        main_thread_scheduler_->ControlTaskRunner()->PostDelayedTask(
            FROM_HERE, reset_had_recent_title_or_favicon_update_.GetCallback(), kTimeToInhibitIntensiveThrottlingOnTitleOrFaviconUpdate);
    }
}
...
base::TimeDelta PageSchedulerImpl::GetIntensiveWakeUpThrottlingInterval(bool is_same_origin) const
{
...
void PageSchedulerImpl::UpdateWakeUpBudgetPools(base::LazyNow* lazy_now)
{
    ...
    unimportant_wake_up_budget_pool_->SetWakeUpInterval(lazy_now->Now(), unimportant_timers_throttled_wake_up_interval_);
    hidden_wake_up_budget_pool_->SetWakeUpInterval(lazy_now->Now(), kDefaultThrottledWakeUpInterval);
    same_origin_intensive_wake_up_budget_pool_->SetWakeUpInterval(lazy_now->Now(), GetIntensiveWakeUpThrottlingInterval(true));
    cross_origin_intensive_wake_up_budget_pool_->SetWakeUpInterval(lazy_now->Now(), GetIntensiveWakeUpThrottlingInterval(false));
}
...
bool PageSchedulerImpl::HasWakeUpBudgetPools() const
{
    ...
    DCHECK_EQ(!!unimportant_wake_up_budget_pool_, !!same_origin_intensive_wake_up_budget_pool_);
    DCHECK_EQ(!!unimportant_wake_up_budget_pool_, !!cross_origin_intensive_wake_up_budget_pool_);

    return !!unimportant_wake_up_budget_pool_;
}

void PageSchedulerImpl::UpdateFrozenState(PolicyUpdater& policy_updater, base::MemoryReductionTaskContext called_from)
{
    ...
    if (IsBackgrounded()) {
        if (IsFrozen()) {
            ...
            if (called_from == base::MemoryReductionTaskContext::kProactive) {
                ...
                freeze_time = std::max(page_visibility_changed_time_, audio_state_changed_time_) + delay_for_background_tab_freezing_;
            }
        }
    }
    ...
    if (freeze_time > now) {
        ...
        if (!freeze_time.is_max()) {
            update_frozen_state_timer_.SetTaskRunner(main_thread_scheduler_->ControlTaskRunner());
            update_frozen_state_timer_.Start(FROM_HERE, freeze_time - now,
                base::BindOnce(
                    [](PageSchedulerImpl* page_scheduler, base::MemoryReductionTaskContext called_from) {
                        PolicyUpdater policy_updater;
...
{
    PageSchedulerImpl::UpdateFrozenState(policy_updater, base::MemoryReductionTaskContext::kDelayExpired);
}

std::array<WakeUpBudgetPool*, PageSchedulerImpl::kNumWakeUpBudgetPools> PageSchedulerImpl::AllWakeUpBudgetPools()
{
    return { unimportant_wake_up_budget_pool_.get(), hidden_wake_up_budget_pool_.get(), same_origin_intensive_wake_up_budget_pool_.get(),
