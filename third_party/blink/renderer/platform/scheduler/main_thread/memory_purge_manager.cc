...
BASE_FEATURE(kMemoryPurgeInBackground, "MemoryPurgeInBackground", base::FEATURE_ENABLED_BY_DEFAULT);
...
const base::FeatureParam<base::TimeDelta> kMemoryPurgeInBackgroundMinDelay { &kMemoryPurgeInBackground, "memory_purge_background_min_delay", base::Minutes(1) };
const base::FeatureParam<base::TimeDelta> kMemoryPurgeInBackgroundMaxDelay { &kMemoryPurgeInBackground, "memory_purge_background_max_delay",
    MemoryPurgeManager::kDefaultMaxTimeToPurgeAfterBackgrounded };

} // namespace

MemoryPurgeManager::MemoryPurgeManager(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
...
void MemoryPurgeManager::OnPageFrozen(base::MemoryReductionTaskContext called_from)
{
...
void MemoryPurgeManager::RequestMemoryPurgeWithDelay(base::TimeDelta delay)
{
    if (!purge_timer_.IsRunning()) {
        purge_timer_.Start(FROM_HERE, delay, this, &MemoryPurgeManager::PerformMemoryPurge);
    }
}
...
base::TimeDelta MemoryPurgeManager::GetTimeToPurgeAfterBackgrounded() const
{
    return base::Seconds(base::RandInt(
        static_cast<int>(kMemoryPurgeInBackgroundMinDelay.Get().InSeconds()), static_cast<int>(kMemoryPurgeInBackgroundMaxDelay.Get().InSeconds())));
}
