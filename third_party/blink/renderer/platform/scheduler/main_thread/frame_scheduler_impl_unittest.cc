...
constexpr base::TimeDelta kDefaultThrottledWakeUpInterval = PageSchedulerImpl::kDefaultThrottledWakeUpInterval;
constexpr base::TimeDelta kIntensiveThrottledWakeUpInterval = PageSchedulerImpl::kIntensiveThrottledWakeUpInterval;
...
std::unique_ptr<PageSchedulerImpl> CreatePageScheduler(
    PageScheduler::Delegate* page_scheduler_delegate, MainThreadSchedulerImpl* scheduler, AgentGroupScheduler& agent_group_scheduler)
{
...
class TestObject {
public:
    explicit TestObject(int* counter)
        : counter_(counter)
    {
    }
...
constexpr TaskType kAllFrameTaskTypes[] = { TaskType::kInternalContentCapture, TaskType::kJavascriptTimerImmediate, TaskType::kJavascriptTimerDelayedLowNesting,
    TaskType::kJavascriptTimerDelayedHighNesting, TaskType::kInternalLoading, TaskType::kNetworking, TaskType::kNetworkingUnfreezable,
    TaskType::kNetworkingUnfreezableRenderBlockingLoading, TaskType::kNetworkingControl, TaskType::kLowPriorityScriptExecution, TaskType::kDOMManipulation,
    TaskType::kHistoryTraversal, TaskType::kEmbed, TaskType::kCanvasBlobSerialization, TaskType::kRemoteEvent, TaskType::kWebSocket, TaskType::kMicrotask,
    TaskType::kUnshippedPortMessage, TaskType::kFileReading, TaskType::kPresentation, TaskType::kSensor, TaskType::kPerformanceTimeline, TaskType::kWebGL,
    TaskType::kIdleTask, TaskType::kInternalDefault, TaskType::kMiscPlatformAPI, TaskType::kFontLoading, TaskType::kApplicationLifeCycle,
    TaskType::kBackgroundFetch, TaskType::kPermission, TaskType::kPostedMessage, TaskType::kServiceWorkerClientMessage, TaskType::kWorkerAnimation,
    TaskType::kUserInteraction, TaskType::kMediaElementEvent, TaskType::kInternalWebCrypto, TaskType::kInternalMedia, TaskType::kInternalMediaRealTime,
    TaskType::kInternalUserInteraction, TaskType::kInternalIntersectionObserver, TaskType::kInternalFindInPage, TaskType::kInternalContinueScriptLoading,
    TaskType::kDatabaseAccess, TaskType::kInternalNavigationAssociated, TaskType::kInternalTest, TaskType::kWebLocks, TaskType::kInternalFrameLifecycleControl,
    TaskType::kInternalTranslation, TaskType::kInternalInspector, TaskType::kInternalNavigationAssociatedUnfreezable, TaskType::kInternalHighPriorityLocalFrame,
    TaskType::kInternalInputBlocking, TaskType::kWakeLock, TaskType::kStorage, TaskType::kClipboard, TaskType::kMachineLearning, TaskType::kWebGPU,
    TaskType::kInternalPostMessageForwarding, TaskType::kInternalNavigationCancellation };

static_assert(static_cast<int>(TaskType::kMaxValue) == 87, "When adding a TaskType, make sure that kAllFrameTaskTypes is updated.");
...
{
    bool vector_empty = arg.non_sticky_features_and_js_locations->details_list.empty();
    bool vector_has_ccns = arg.sticky_features_and_js_locations->details_list.Contains(
                               FeatureAndJSLocationBlockingBFCache(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore, nullptr))
        && arg.sticky_features_and_js_locations->details_list.Contains(
            FeatureAndJSLocationBlockingBFCache(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoCache, nullptr));
    return vector_empty && vector_has_ccns;
}

MATCHER_P(BlockingDetailsHasWebSocket, handle, "BlockingDetails has WebSocket.")
{
    bool handle_has_web_socket
        = (handle->GetFeatureAndJSLocationBlockingBFCache() == FeatureAndJSLocationBlockingBFCache(SchedulingPolicy::Feature::kWebSocket, nullptr));
...
{
    bool non_sticky_vector_empty = arg.non_sticky_features_and_js_locations->details_list.empty();
    bool sticky_vector_empty = arg.sticky_features_and_js_locations->details_list.empty();
    return non_sticky_vector_empty && sticky_vector_empty;
}
class FrameSchedulerImplTest : public testing::Test {
public:
    FrameSchedulerImplTest()
        : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME, base::test::TaskEnvironment::ThreadPoolExecutionMode::QUEUED)
    {
    }

    // Constructs a FrameSchedulerImplTest with a list of features to enable and a
    // list of features to disable.
    FrameSchedulerImplTest(std::vector<base::test::FeatureRef> features_to_enable, std::vector<base::test::FeatureRef> features_to_disable)
        : FrameSchedulerImplTest()
    {
    ...
    FrameSchedulerImplTest(const base::Feature& feature_to_enable, const base::FieldTrialParams& feature_to_enable_params,
        const std::vector<base::test::FeatureRef>& features_to_disable)
        : FrameSchedulerImplTest()
    {
        feature_list_.InitWithFeaturesAndParameters({ { feature_to_enable, feature_to_enable_params } }, features_to_disable);
    }
    ...
    void SetUp() override
    {
        scheduler_ = std::make_unique<MainThreadSchedulerImpl>(
            base::sequence_manager::SequenceManagerForTest::Create(nullptr, task_environment_.GetMainThreadTaskRunner(), task_environment_.GetMockTickClock(),
                base::sequence_manager::SequenceManager::Settings::Builder().SetPrioritySettings(CreatePrioritySettings()).Build()));
        agent_group_scheduler_ = scheduler_->CreateAgentGroupScheduler();
        page_scheduler_ = CreatePageScheduler(nullptr, scheduler_.get(), *agent_group_scheduler_);
        frame_scheduler_delegate_ = std::make_unique<testing::StrictMock<FrameSchedulerDelegateForTesting>>();
        frame_scheduler_ = CreateFrameScheduler(page_scheduler_.get(), frame_scheduler_delegate_.get(),
            /*is_in_embedded_frame_tree=*/false, FrameScheduler::FrameType::kSubframe);
    }

    void ResetFrameScheduler(bool is_in_embedded_frame_tree, FrameScheduler::FrameType frame_type)
    {
        auto new_delegate_ = std::make_unique<testing::StrictMock<FrameSchedulerDelegateForTesting>>();
        frame_scheduler_ = CreateFrameScheduler(page_scheduler_.get(), new_delegate_.get(), is_in_embedded_frame_tree, frame_type);
    ...
    // - 'B': Best-effort
    // - 'D': Database
    void PostTestTasksForPrioritisationType(Vector<String>* run_order, const String& task_descriptor)
    {
        ...
        while (!stream.eof()) {
            ...
            auto queue_traits = FrameSchedulerImpl::PausableTaskQueueTraits().SetPrioritisationType(prioritisation_type);
            GetTaskQueue(queue_traits)
                ->GetTaskRunnerWithDefaultTaskType()
                ->PostTask(FROM_HERE, base::BindOnce(&AppendToVectorTestTask, run_order, String::FromUTF8(task)));
        }
    }
    ...
    void PostTestTasksToQueuesWithTrait(Vector<String>* run_order, const String& task_descriptor)
    {
        std::istringstream stream(task_descriptor.Utf8());
        while (!stream.eof()) {
            std::string task;
    ...
    void FastForwardToAlignedTime(base::TimeDelta interval)
    {
        ...
        const base::TimeTicks aligned = now.SnappedToNextTick(base::TimeTicks(), interval);
        if (aligned != now)
    ...
    void PostAndRunTasks50MsInterval(scoped_refptr<base::SingleThreadTaskRunner> task_runner, std::vector<base::TimeTicks>* run_times)
    {
        for (int i = 0; i < 5; i++) {
            task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, run_times), base::Milliseconds(50) * i);
        }
        task_environment_.FastForwardBy(base::Seconds(5));
    }

    // Post and run tasks. Expect no alignment.
    void PostTasks_ExpectNoAlignment(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    {
        ...
        EXPECT_THAT(run_times,
            testing::ElementsAre(
                start, start + base::Milliseconds(50), start + base::Milliseconds(100), start + base::Milliseconds(150), start + base::Milliseconds(200)));
    }

    // Post and run tasks. Expect 32ms alignment.
    void PostTasks_Expect32msAlignment(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    {
        ...
        EXPECT_THAT(run_times,
            testing::ElementsAre(
                start, start + base::Milliseconds(64), start + base::Milliseconds(128), start + base::Milliseconds(160), start + base::Milliseconds(224)));
    }

    // Post and run tasks. Expect 1 second alignment.
    void PostTasks_Expect1sAlignment(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    {
        FastForwardToAlignedTime(base::Seconds(1));
        const base::TimeTicks start = base::TimeTicks::Now();
        ...
        EXPECT_THAT(
            run_times, testing::ElementsAre(start, start + base::Seconds(1), start + base::Seconds(1), start + base::Seconds(1), start + base::Seconds(1)));
    }
...
protected:
    ...
    scoped_refptr<MainThreadTaskQueue> GetTaskQueue(MainThreadTaskQueue::QueueTraits queue_traits)
    {
        return frame_scheduler_->FrameTaskQueueControllerForTest()->GetTaskQueue(queue_traits);
    }
    ...
    scoped_refptr<MainThreadTaskQueue> JavaScriptTimerNormalThrottleableTaskQueue()
    {
        return GetTaskQueue(FrameSchedulerImpl::ThrottleableTaskQueueTraits().SetPrioritisationType(PrioritisationType::kJavaScriptTimer));
    }

    scoped_refptr<MainThreadTaskQueue> JavaScriptTimerIntensivelyThrottleableTaskQueue()
    {
        return GetTaskQueue(
            FrameSchedulerImpl::ThrottleableTaskQueueTraits().SetPrioritisationType(PrioritisationType::kJavaScriptTimer).SetCanBeIntensivelyThrottled(true));
    }

    scoped_refptr<MainThreadTaskQueue> JavaScriptTimerNonThrottleableTaskQueue()
    {
        return GetTaskQueue(FrameSchedulerImpl::DeferrableTaskQueueTraits().SetPrioritisationType(PrioritisationType::kJavaScriptTimer));
    }
    ...
    SchedulingLifecycleState CalculateLifecycleState(FrameScheduler::ObserverType type)
    {
        return frame_scheduler_->CalculateLifecycleState(type);
    }

    void DidCommitProvisionalLoad(FrameScheduler::NavigationType navigation_type)
    {
        frame_scheduler_->DidCommitProvisionalLoad(
            /*is_web_history_inert_commit=*/false, navigation_type, { GetUnreportedTaskTime() });
    }
    ...
    std::unique_ptr<testing::StrictMock<FrameSchedulerDelegateForTesting>> frame_scheduler_delegate_;
    scoped_refptr<MainThreadTaskQueue> throttleable_task_queue_;
};

class FrameSchedulerImplStopInBackgroundDisabledTest : public FrameSchedulerImplTest, public ::testing::WithParamInterface<TaskType> {
public:
...
public:
    ...
    inline void CheckObserverState(base::Location from, size_t not_throttled_count_expectation, size_t hidden_count_expectation,
    ...
    {
        return base::BindRepeating(&MockLifecycleObserver::OnLifecycleStateChanged, base::Unretained(this));
    }
...
void RunTaskOfLength(base::test::TaskEnvironment* task_environment, base::TimeDelta length)
{
    task_environment->FastForwardBy(length);
}

class FrameSchedulerImplTestWithIntensiveWakeUpThrottlingBase : public FrameSchedulerImplTest {
public:
    using Super = FrameSchedulerImplTest;

    FrameSchedulerImplTestWithIntensiveWakeUpThrottlingBase()
        : FrameSchedulerImplTest({ features::kIntensiveWakeUpThrottling }, { features::kStopInBackground })
    {
    }
    ...
    const base::TimeDelta kGracePeriod = GetIntensiveWakeUpThrottlingGracePeriod(false);
};
...
class FrameSchedulerImplTestWithIntensiveWakeUpThrottling : public FrameSchedulerImplTestWithIntensiveWakeUpThrottlingBase,
...
public:
    ...
    // task type parameter.
    scoped_refptr<base::SingleThreadTaskRunner> GetTaskRunner(FrameSchedulerImpl* frame_scheduler)
    {
        ...
        if (task_type == TaskType::kWebSchedulingPostedTask) {
            test_web_scheduling_task_queues_.push_back(
                frame_scheduler->CreateWebSchedulingTaskQueue(WebSchedulingQueueType::kTaskQueue, WebSchedulingPriority::kUserVisiblePriority));
...
class FrameSchedulerImplTestWithIntensiveWakeUpThrottlingPolicyOverride : public FrameSchedulerImplTestWithIntensiveWakeUpThrottlingBase {
public:
    ...
    void SetPolicyOverride(bool enabled)
    {
        ...
        scoped_command_line_.GetProcessCommandLine()->AppendSwitchASCII(switches::kIntensiveWakeUpThrottlingPolicy,
...
TEST_F(FrameSchedulerImplTest, FrameHiddenThenVisible_CrossOrigin_ExplicitInit)
{
    LazyInitThrottleableTaskQueue();
    EXPECT_FALSE(IsThrottled());
...
{
    int counter = 0;
    LoadingTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)));
    ThrottleableTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)));
    DeferrableTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)));
    PausableTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)));
    UnpausableTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)));
...
TEST_F(FrameSchedulerImplTest, PauseAndResumeForCooperativeScheduling)
{
    ...
    frame_scheduler_->SetPreemptedForCooperativeScheduling(FrameOrWorkerScheduler::Preempted(true));
    ...
    frame_scheduler_->SetPreemptedForCooperativeScheduling(FrameOrWorkerScheduler::Preempted(false));
    EXPECT_TRUE(LoadingTaskQueue()->IsQueueEnabled());
    EXPECT_TRUE(ThrottleableTaskQueue()->IsQueueEnabled());
    EXPECT_TRUE(DeferrableTaskQueue()->IsQueueEnabled());
    EXPECT_TRUE(PausableTaskQueue()->IsQueueEnabled());
...
void RePostTask(scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::TimeDelta delay, int* num_remaining_tasks)
{
    --(*num_remaining_tasks);
    if (*num_remaining_tasks > 0) {
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RePostTask, task_runner, delay, base::Unretained(num_remaining_tasks)), delay);
    }
}
...
TEST_P(FrameSchedulerImplStopInBackgroundDisabledTest, DISABLED_ThrottledTaskExecution)
{
    // This TaskRunner is throttled.
    const scoped_refptr<base::SingleThreadTaskRunner> task_runner = frame_scheduler_->GetTaskRunner(GetParam());
    ...
    // grace period.
    int num_remaining_tasks = base::Seconds(kIntensiveWakeUpThrottling_GracePeriodSeconds_Default).IntDiv(kDefaultThrottledWakeUpInterval);
    task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RePostTask, task_runner, kShortDelay, base::Unretained(&num_remaining_tasks)), kShortDelay);
    ...
    const base::TimeDelta expected_period_after_grace_period
        = (GetParam() == TaskType::kJavascriptTimerDelayedLowNesting) ? kDefaultThrottledWakeUpInterval : kIntensiveThrottledWakeUpInterval;
...
INSTANTIATE_TEST_SUITE_P(AllTimerTaskTypes, FrameSchedulerImplStopInBackgroundDisabledTest,
    testing::Values(TaskType::kJavascriptTimerDelayedLowNesting, TaskType::kJavascriptTimerDelayedHighNesting),
    [](const testing::TestParamInfo<TaskType>& info) { return TaskTypeNames::TaskTypeToString(info.param); });

TEST_F(FrameSchedulerImplTest, FreezeForegroundOnlyTasks)
{
    int counter = 0;
    ForegroundOnlyTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)));
...
TEST_F(FrameSchedulerImplTest, PagePostsCpuTasks)
{
    ...
    UnpausableTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(
        FROM_HERE, base::BindOnce(&RunTaskOfLength, &task_environment_, base::Milliseconds(10)));
    base::RunLoop().RunUntilIdle();
    EXPECT_FALSE(GetUnreportedTaskTime().is_zero());
    EXPECT_EQ(0, GetTotalUpdateTaskTimeCalls());
    UnpausableTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(
        FROM_HERE, base::BindOnce(&RunTaskOfLength, &task_environment_, base::Milliseconds(100)));
    base::RunLoop().RunUntilIdle();
    EXPECT_TRUE(GetUnreportedTaskTime().is_zero());
...
TEST_F(FrameSchedulerImplTest, FramePostsCpuTasksThroughReloadRenavigate)
{
    ...
    } kTestCases[] = { { false, FrameScheduler::FrameType::kMainFrame, FrameScheduler::NavigationType::kOther, false, 0 },
        { false, FrameScheduler::FrameType::kMainFrame, FrameScheduler::NavigationType::kReload, false, 0 },
        { false, FrameScheduler::FrameType::kMainFrame, FrameScheduler::NavigationType::kSameDocument, true, 1 },
        { false, FrameScheduler::FrameType::kSubframe, FrameScheduler::NavigationType::kOther, true, 1 },
        { false, FrameScheduler::FrameType::kSubframe, FrameScheduler::NavigationType::kSameDocument, true, 1 },
        { true, FrameScheduler::FrameType::kMainFrame, FrameScheduler::NavigationType::kOther, true, 1 },
        { true, FrameScheduler::FrameType::kMainFrame, FrameScheduler::NavigationType::kSameDocument, true, 1 },
        { true, FrameScheduler::FrameType::kSubframe, FrameScheduler::NavigationType::kOther, true, 1 },
        { true, FrameScheduler::FrameType::kSubframe, FrameScheduler::NavigationType::kSameDocument, true, 1 } };
    for (const auto& test_case : kTestCases) {
        SCOPED_TRACE(String::Format("FrameType: %d, NavigationType: %d : TaskTime.is_zero %d, CallCount %d", static_cast<int>(test_case.frame_type),
            static_cast<int>(test_case.navigation_type), test_case.expect_unreported_task_time_zero, test_case.expected_total_calls));
        ...
        UnpausableTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(
            FROM_HERE, base::BindOnce(&RunTaskOfLength, &task_environment_, base::Milliseconds(60)));
        base::RunLoop().RunUntilIdle();
        EXPECT_FALSE(GetUnreportedTaskTime().is_zero());
        EXPECT_EQ(0, GetTotalUpdateTaskTimeCalls());
        ...
        UnpausableTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(
            FROM_HERE, base::BindOnce(&RunTaskOfLength, &task_environment_, base::Milliseconds(60)));
        base::RunLoop().RunUntilIdle();
        EXPECT_EQ(test_case.expect_unreported_task_time_zero, GetUnreportedTaskTime().is_zero());
        EXPECT_EQ(test_case.expected_total_calls, GetTotalUpdateTaskTimeCalls());
    }
}

class FrameSchedulerImplTestWithUnfreezableLoading : public FrameSchedulerImplTest {
public:
...
TEST_F(FrameSchedulerImplTestWithUnfreezableLoading, LoadingTasksKeepRunningWhenFrozen)
{
    int counter = 0;
    UnfreezableLoadingTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)));
    LoadingTaskQueue()->GetTaskRunnerWithDefaultTaskType()->PostTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)));
...
{
    std::unique_ptr<MockLifecycleObserver> observer = std::make_unique<MockLifecycleObserver>();
    ...
    observer->CheckObserverState(FROM_HERE, not_throttled_count, hidden_count, throttled_count, stopped_count);

    auto observer_handle = frame_scheduler_->AddLifecycleObserver(FrameScheduler::ObserverType::kLoader, observer->GetCallback());
...
{
    EXPECT_EQ(CalculateLifecycleState(FrameScheduler::ObserverType::kLoader), SchedulingLifecycleState::kNotThrottled);
    EXPECT_EQ(CalculateLifecycleState(FrameScheduler::ObserverType::kWorkerScheduler), SchedulingLifecycleState::kNotThrottled);
}

TEST_F(FrameSchedulerImplTest, SubesourceLoadingPaused)
{
    // A loader observer and related counts.
    std::unique_ptr<MockLifecycleObserver> loader_observer = std::make_unique<MockLifecycleObserver>();
    ...
    std::unique_ptr<MockLifecycleObserver> worker_observer = std::make_unique<MockLifecycleObserver>();
    ...
    loader_observer->CheckObserverState(FROM_HERE, loader_not_throttled_count, loader_hidden_count, loader_throttled_count, loader_stopped_count);

    worker_observer->CheckObserverState(FROM_HERE, worker_not_throttled_count, worker_hidden_count, worker_throttled_count, worker_stopped_count);

    // Adding the observers should recieve a non-throttled response
    auto loader_observer_handle = frame_scheduler_->AddLifecycleObserver(FrameScheduler::ObserverType::kLoader, loader_observer->GetCallback());

    auto worker_observer_handle = frame_scheduler_->AddLifecycleObserver(FrameScheduler::ObserverType::kWorkerScheduler, worker_observer->GetCallback());
...

    {
        ...
        std::unique_ptr<MockLifecycleObserver> loader_observer_added_after_stopped = std::make_unique<MockLifecycleObserver>();

        auto loader_observer_added_after_stopped_handle
            = frame_scheduler_->AddLifecycleObserver(FrameScheduler::ObserverType::kLoader, loader_observer_added_after_stopped->GetCallback());
        // This observer should see stopped when added.
        loader_observer_added_after_stopped->CheckObserverState(FROM_HERE, 0, 0, 0, 1u);
...
TEST_F(FrameSchedulerImplTest, LogIpcsPostedToFramesInBackForwardCache)
{
    ...
    const scoped_refptr<base::SingleThreadTaskRunner> task_runner = frame_scheduler_->GetTaskRunner(TaskType::kInternalTest);
...
TEST_F(FrameSchedulerImplTest, LogIpcsFromMultipleThreadsPostedToFramesInBackForwardCache)
{
    base::HistogramTester histogram_tester;
    ...
    base::RepeatingClosure restore_from_cache_callback = base::BindRepeating(&FrameSchedulerImplTest::RestorePageFromBackForwardCache, base::Unretained(this));
    ...
    base::ThreadPool::PostTask(FROM_HERE,
        base::BindOnce(
            [](scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::RepeatingClosure restore_from_cache_callback) {
                {
...
{
    page_scheduler_->SetPageVisible(false);
    EXPECT_EQ(InputBlockingTaskQueue()->GetQueuePriority(), TaskPriority::kHighestPriority);
    page_scheduler_->SetPageVisible(true);
    EXPECT_EQ(InputBlockingTaskQueue()->GetQueuePriority(), TaskPriority::kHighestPriority);
}
...
{
    auto render_blocking_task_queue = GetTaskQueue(TaskType::kNetworkingUnfreezableRenderBlockingLoading);
    page_scheduler_->SetPageVisible(false);
    EXPECT_EQ(render_blocking_task_queue->GetQueuePriority(), TaskPriority::kNormalPriority);
    page_scheduler_->SetPageVisible(true);
    EXPECT_EQ(render_blocking_task_queue->GetQueuePriority(), TaskPriority::kExtremelyHighPriority);
}
...
TEST_F(FrameSchedulerImplTest, TaskTypeToTaskQueueMapping)
{
    ...
    // default queues.
    EXPECT_EQ(GetTaskQueue(TaskType::kJavascriptTimerDelayedLowNesting), JavaScriptTimerNormalThrottleableTaskQueue());
    EXPECT_EQ(GetTaskQueue(TaskType::kJavascriptTimerDelayedHighNesting), JavaScriptTimerIntensivelyThrottleableTaskQueue());
    EXPECT_EQ(GetTaskQueue(TaskType::kJavascriptTimerImmediate), JavaScriptTimerNonThrottleableTaskQueue());
    ...
    EXPECT_EQ(GetTaskQueue(TaskType::kNetworkingControl), LoadingControlTaskQueue());
    EXPECT_EQ(GetTaskQueue(TaskType::kInternalTranslation), ForegroundOnlyTaskQueue());
}
...
TEST_F(FrameSchedulerImplTest, ThrottledTaskTypes)
{
    ...
    for (TaskType task_type : kAllFrameTaskTypes) {
        SCOPED_TRACE(testing::Message() << "TaskType is " << TaskTypeNames::TaskTypeToString(task_type));
        switch (task_type) {
...
class FrameSchedulerImplDatabaseAccessWithoutHighPriority : public FrameSchedulerImplTest {
public:
...
TEST_F(FrameSchedulerImplDatabaseAccessWithoutHighPriority, QueueTraits)
{
    ...
    EXPECT_EQ(da_queue->GetQueueTraits().prioritisation_type, MainThreadTaskQueue::QueueTraits::PrioritisationType::kRegular);
    EXPECT_EQ(da_queue->GetQueuePriority(), TaskPriority::kNormalPriority);
}

class FrameSchedulerImplDatabaseAccessWithHighPriority : public FrameSchedulerImplTest {
public:
...
{
    auto da_queue = GetTaskQueue(TaskType::kDatabaseAccess);
    EXPECT_EQ(da_queue->GetQueueTraits().prioritisation_type, MainThreadTaskQueue::QueueTraits::PrioritisationType::kExperimentalDatabase);
...
TEST_F(FrameSchedulerImplDatabaseAccessWithHighPriority, NormalPriorityInBackground)
{
    page_scheduler_->SetPageVisible(false);
...
class FrameSchedulerImplLowPriorityAsyncScriptExecutionTest : public FrameSchedulerImplTest, public testing::WithParamInterface<std::string> {
public:
    FrameSchedulerImplLowPriorityAsyncScriptExecutionTest()
        : FrameSchedulerImplTest(
            features::kLowPriorityAsyncScriptExecution, { { features::kLowPriorityAsyncScriptExecutionLowerTaskPriorityParam.name, specified_priority() } }, {})
    {
    }

    std::string specified_priority()
    {
        return GetParam();
    }
...
INSTANTIATE_TEST_SUITE_P(All, FrameSchedulerImplLowPriorityAsyncScriptExecutionTest, testing::Values("high", "low", "best_effort"));

TEST_P(FrameSchedulerImplLowPriorityAsyncScriptExecutionTest, LowPriorityScriptExecutionHasBestEffortPriority)
{
    EXPECT_EQ(GetExpectedPriority(), GetTaskQueue(TaskType::kLowPriorityScriptExecution)->GetQueuePriority()) << specified_priority();
}

TEST_F(FrameSchedulerImplTest, BackForwardCacheOptOut)
{
    EXPECT_THAT(frame_scheduler_->GetActiveFeaturesTrackedForBackForwardCacheMetrics(), testing::UnorderedElementsAre());

    auto feature_handle1 = frame_scheduler_->RegisterFeature(SchedulingPolicy::Feature::kWebSocket, { SchedulingPolicy::DisableBackForwardCache() });

    EXPECT_THAT(frame_scheduler_->GetActiveFeaturesTrackedForBackForwardCacheMetrics(), testing::UnorderedElementsAre(SchedulingPolicy::Feature::kWebSocket));

    auto feature_handle2 = frame_scheduler_->RegisterFeature(SchedulingPolicy::Feature::kWebRTC, { SchedulingPolicy::DisableBackForwardCache() });

    EXPECT_THAT(frame_scheduler_->GetActiveFeaturesTrackedForBackForwardCacheMetrics(),
        testing::UnorderedElementsAre(SchedulingPolicy::Feature::kWebSocket, SchedulingPolicy::Feature::kWebRTC));

    feature_handle1.reset();

    EXPECT_THAT(frame_scheduler_->GetActiveFeaturesTrackedForBackForwardCacheMetrics(), testing::UnorderedElementsAre(SchedulingPolicy::Feature::kWebRTC));
...
{
    ResetFrameScheduler(/*is_in_embedded_frame_tree=*/false, FrameScheduler::FrameType::kMainFrame);

    frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimerImmediate)
        ->PostTask(FROM_HERE,
            base::BindOnce(
                [](FrameSchedulerImpl* frame_scheduler, testing::StrictMock<FrameSchedulerDelegateForTesting>* delegate) {
                    ...
                    EXPECT_CALL(*delegate, UpdateBackForwardCacheDisablingFeatures(BlockingDetailsHasCCNS()));
                },
...
TEST_F(FrameSchedulerImplTest, FeatureUpload_FrameDestruction)
{
    ...
    FeatureHandle feature_handle(frame_scheduler_->RegisterFeature(SchedulingPolicy::Feature::kWebSocket, { SchedulingPolicy::DisableBackForwardCache() }));

    frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimerImmediate)
        ->PostTask(FROM_HERE,
            base::BindOnce(
                [](FrameSchedulerImpl* frame_scheduler, testing::StrictMock<FrameSchedulerDelegateForTesting>* delegate, FeatureHandle* feature_handle) {
                    // Ensure that the feature upload is delayed.
                    testing::Mock::VerifyAndClearExpectations(delegate);
                    EXPECT_CALL(*delegate, UpdateBackForwardCacheDisablingFeatures(BlockingDetailsHasWebSocket(feature_handle)));
                },
                frame_scheduler_.get(), frame_scheduler_delegate_.get(), &feature_handle));
    frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimerImmediate)
        ->PostTask(FROM_HERE,
            base::BindOnce(
                [](FrameSchedulerImpl* frame_scheduler, testing::StrictMock<FrameSchedulerDelegateForTesting>* delegate, FeatureHandle* feature_handle) {
                    ...
                    // destruction.
                    testing::Mock::VerifyAndClearExpectations(delegate);
                    EXPECT_CALL(*delegate, UpdateBackForwardCacheDisablingFeatures(BlockingDetailsIsEmpty())).Times(0);
                },
                frame_scheduler_.get(), frame_scheduler_delegate_.get(), &feature_handle));
...
{
    int counter = 0;
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimerImmediate);
    task_runner->PostTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)));
    task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)), base::Milliseconds(100));
    frame_scheduler_.reset();
...
TEST_F(FrameSchedulerImplTest, DetachedWebSchedulingTaskQueue)
{
    ...
    std::unique_ptr<WebSchedulingTaskQueue> web_scheduling_task_queue
        = frame_scheduler_->CreateWebSchedulingTaskQueue(WebSchedulingQueueType::kTaskQueue, WebSchedulingPriority::kUserVisiblePriority);
    frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimerImmediate)->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
        frame_scheduler_.reset();
        web_scheduling_task_queue->SetPriority(WebSchedulingPriority::kBackgroundPriority);
        web_scheduling_task_queue.reset();
    }));
    base::RunLoop().RunUntilIdle();
}

class WebSchedulingTaskQueueTest : public FrameSchedulerImplTest, public WebSchedulingTestHelper::Delegate {
public:
    void SetUp() override
    {
...
TEST_F(WebSchedulingTaskQueueTest, TasksRunInPriorityOrder)
{
    ...
    base::RunLoop().RunUntilIdle();
    EXPECT_THAT(run_order, testing::ElementsAre("UB1", "UB2", "UV1", "UV2", "BG1", "BG2"));
}
...
TEST_F(WebSchedulingTaskQueueTest, DynamicTaskPriorityOrder)
{
    ...
    base::RunLoop().RunUntilIdle();
    EXPECT_THAT(run_order, testing::ElementsAre("UV1", "UV2", "BG1", "BG2", "UB1", "UB2"));
}
...
TEST_F(WebSchedulingTaskQueueTest, TasksAndContinuations)
{
    ...
    Vector<TestTaskSpecEntry> test_spec = {
        ...
        { .descriptor = "BG-C",
            .type_info
            = WebSchedulingParams({ .queue_type = WebSchedulingQueueType::kContinuationQueue, .priority = WebSchedulingPriority::kBackgroundPriority }) },
        { .descriptor = "UV",
            .type_info = WebSchedulingParams({ .queue_type = WebSchedulingQueueType::kTaskQueue, .priority = WebSchedulingPriority::kUserVisiblePriority }) },
        { .descriptor = "UV-C",
            .type_info
            = WebSchedulingParams({ .queue_type = WebSchedulingQueueType::kContinuationQueue, .priority = WebSchedulingPriority::kUserVisiblePriority }) },
        { .descriptor = "UB",
            .type_info = WebSchedulingParams({ .queue_type = WebSchedulingQueueType::kTaskQueue, .priority = WebSchedulingPriority::kUserBlockingPriority }) },
        { .descriptor = "UB-C",
            .type_info
            = WebSchedulingParams({ .queue_type = WebSchedulingQueueType::kContinuationQueue, .priority = WebSchedulingPriority::kUserBlockingPriority }) }
    };
    web_scheduling_test_helper_->PostTestTasks(&run_order, test_spec);

    base::RunLoop().RunUntilIdle();
    EXPECT_THAT(run_order, testing::ElementsAre("UB-C", "UB", "UV-C", "UV", "BG-C", "BG"));
}

TEST_F(WebSchedulingTaskQueueTest, DynamicPriorityContinuations)
{
    Vector<String> run_order;
    Vector<TestTaskSpecEntry> test_spec = { { .descriptor = "BG-C",
                                                .type_info = WebSchedulingParams({ .queue_type = WebSchedulingQueueType::kContinuationQueue,
    ...
    web_scheduling_test_helper_->GetWebSchedulingTaskQueue(WebSchedulingQueueType::kContinuationQueue, WebSchedulingPriority::kUserBlockingPriority)
        ->SetPriority(WebSchedulingPriority::kBackgroundPriority);
...
{
    Vector<String> run_order;
    Vector<TestTaskSpecEntry> test_spec = { { .descriptor = "Idle", .type_info = TaskType::kLowPriorityScriptExecution },
        { .descriptor = "BG",
            .type_info = WebSchedulingParams({ .queue_type = WebSchedulingQueueType::kTaskQueue, .priority = WebSchedulingPriority::kBackgroundPriority }) },
        { .descriptor = "BG-C",
            .type_info
        ...
        { .descriptor = "UB-C",
            .type_info
            = WebSchedulingParams({ .queue_type = WebSchedulingQueueType::kContinuationQueue, .priority = WebSchedulingPriority::kUserBlockingPriority }) },
        { .descriptor = "Timer", .type_info = TaskType::kJavascriptTimerDelayedLowNesting },
        { .descriptor = "VH1", .type_info = TaskType::kInternalContinueScriptLoading },
        { .descriptor = "VH2", .type_info = TaskType::kInternalNavigationCancellation } };
    web_scheduling_test_helper_->PostTestTasks(&run_order, test_spec);

    base::RunLoop().RunUntilIdle();
    EXPECT_THAT(run_order, testing::ElementsAre("VH1", "VH2", "UB-C", "UB", "UV-C", "UV", "Timer", "BG-C", "BG", "Idle"));
}
...
TEST_F(FrameSchedulerImplTest, ThrottledJSTimerTasksRunTime)
{
    constexpr TaskType kJavaScriptTimerTaskTypes[]
        = { TaskType::kJavascriptTimerDelayedLowNesting, TaskType::kJavascriptTimerDelayedHighNesting, TaskType::kWebSchedulingPostedTask };
    ...
    for (TaskType task_type : kJavaScriptTimerTaskTypes) {
        const scoped_refptr<base::SingleThreadTaskRunner> task_runner
            = task_type == TaskType::kWebSchedulingPostedTask ? web_scheduling_task_queue->GetTaskRunner() : frame_scheduler_->GetTaskRunner(task_type);
        ...
        task_runner->PostTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times[task_type]));
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times[task_type]), base::Milliseconds(1000));
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times[task_type]), base::Milliseconds(1002));
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times[task_type]), base::Milliseconds(1004));
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times[task_type]), base::Milliseconds(2500));
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times[task_type]), base::Milliseconds(6000));
    }
    ...
    for (TaskType task_type : kJavaScriptTimerTaskTypes) {
        EXPECT_THAT(run_times[task_type],
            testing::ElementsAre(start + base::Milliseconds(0), start + base::Milliseconds(1000), start + base::Milliseconds(1002),
                start + base::Milliseconds(2000), start + base::Milliseconds(3000), start + base::Milliseconds(6000)));
    }
}
...
public:
    explicit MockMainThreadScheduler(base::test::TaskEnvironment& task_environment)
        : MainThreadSchedulerImpl(
            base::sequence_manager::SequenceManagerForTest::Create(nullptr, task_environment.GetMainThreadTaskRunner(), task_environment.GetMockTickClock(),
                base::sequence_manager::SequenceManager::Settings::Builder().SetPrioritySettings(CreatePrioritySettings()).Build()))
    {
    }
...
TEST_F(FrameSchedulerImplTest, ReportFMPAndFCPForMainFrames)
{
    ...
    AgentGroupScheduler* agent_group_scheduler = mock_main_thread_scheduler.CreateAgentGroupScheduler();
    std::unique_ptr<PageSchedulerImpl> page_scheduler = CreatePageScheduler(nullptr, &mock_main_thread_scheduler, *agent_group_scheduler);

    std::unique_ptr<FrameSchedulerImpl> main_frame_scheduler = CreateFrameScheduler(page_scheduler.get(), nullptr,
...
TEST_F(FrameSchedulerImplTest, DontReportFMPAndFCPForSubframes)
{
    ...
    {
        std::unique_ptr<FrameSchedulerImpl> subframe_scheduler = CreateFrameScheduler(page_scheduler.get(), nullptr,
            /*is_in_embedded_frame_tree=*/false, FrameScheduler::FrameType::kSubframe);
    ...
    {
        std::unique_ptr<FrameSchedulerImpl> subframe_scheduler = CreateFrameScheduler(page_scheduler.get(), nullptr,
            /*is_in_embedded_frame_tree=*/true, FrameScheduler::FrameType::kMainFrame);
...
TEST_P(FrameSchedulerImplTestWithIntensiveWakeUpThrottling, TaskExecutionSameOriginFrame)
{
    ASSERT_FALSE(frame_scheduler_->IsCrossOriginToNearestMainFrame());

    // Throttled TaskRunner to which tasks are posted in this test.
    const scoped_refptr<base::SingleThreadTaskRunner> task_runner = GetTaskRunner();
    ...
    // Initially, wake ups are not intensively throttled.
    {
        ...
        for (int i = 0; i < kNumTasks; ++i) {
            task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), kShortDelay + i * kDefaultThrottledWakeUpInterval);
        }

        task_environment_.FastForwardBy(kGracePeriod);
        EXPECT_THAT(run_times,
            testing::ElementsAre(scope_start + kDefaultThrottledWakeUpInterval, scope_start + 2 * kDefaultThrottledWakeUpInterval,
                scope_start + 3 * kDefaultThrottledWakeUpInterval, scope_start + 4 * kDefaultThrottledWakeUpInterval,
    ...
    // Test that wake ups are 1-second aligned if there is no recent wake up.
    {
        ...
        const base::TimeDelta delay = last_task_run_at + kIntensiveThrottledWakeUpInterval - scope_start;
        EXPECT_EQ(delay, base::Seconds(5));

        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), delay);
    ...
    //   Otherwise:                               Wake ups are 1-second aligned
    {
        ...
        if (IsIntensiveThrottlingExpected()) {
            ...
            EXPECT_EQ(aligned_time.SnappedToNextTick(base::TimeTicks(), kIntensiveThrottledWakeUpInterval), aligned_time);
            EXPECT_THAT(run_times, testing::ElementsAre(aligned_time, aligned_time, aligned_time, aligned_time, aligned_time));
        } else {
            EXPECT_THAT(run_times,
                testing::ElementsAre(scope_start + base::Seconds(1), scope_start + base::Seconds(2), scope_start + base::Seconds(3),
                    scope_start + base::Seconds(4), scope_start + base::Seconds(5)));
        }
    }
    ...
    // otherwise.
    {
        ...
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), kDefaultThrottledWakeUpInterval);

        task_environment_.FastForwardBy(kIntensiveThrottledWakeUpInterval);

        EXPECT_THAT(run_times, testing::ElementsAre(scope_start + GetExpectedWakeUpInterval()));
    }
    ...
    // minute.
    {
        ...
        const base::TimeDelta kLongDelay = kIntensiveThrottledWakeUpInterval * 5 + kDefaultThrottledWakeUpInterval;
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), kLongDelay);
    ...
    {
        const base::TimeTicks scope_start = base::TimeTicks::Now();
        EXPECT_EQ(scope_start, test_start + base::Minutes(8) + kDefaultThrottledWakeUpInterval);
        std::vector<base::TimeTicks> run_times;

        page_scheduler_->OnTitleOrFaviconUpdated();
        task_runner->PostDelayedTask(FROM_HERE, base::BindLambdaForTesting([&]() {
            RecordRunTime(&run_times);
            for (int i = 0; i < kNumTasks; ++i) {
                task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), kDefaultThrottledWakeUpInterval * (i + 1));
            }
        }),
        ...
        if (IsIntensiveThrottlingExpected()) {
            EXPECT_THAT(run_times,
                testing::ElementsAre(scope_start + base::Seconds(1), scope_start + base::Seconds(2), scope_start + base::Seconds(3),
                    scope_start - kDefaultThrottledWakeUpInterval + base::Minutes(1), scope_start - kDefaultThrottledWakeUpInterval + base::Minutes(1),
                    scope_start - kDefaultThrottledWakeUpInterval + base::Minutes(1)));
        } else {
            EXPECT_THAT(run_times,
                testing::ElementsAre(scope_start + base::Seconds(1), scope_start + base::Seconds(2), scope_start + base::Seconds(3),
                    scope_start + base::Seconds(4), scope_start + base::Seconds(5), scope_start + base::Seconds(6)));
        }
    }
}
...
TEST_P(FrameSchedulerImplTestWithIntensiveWakeUpThrottling, TaskExecutionCrossOriginFrame)
{
    frame_scheduler_->SetCrossOriginToNearestMainFrame(true);
    ...
    // Initially, wake ups are not intensively throttled.
    {
        ...
        task_environment_.FastForwardBy(kGracePeriod);
        EXPECT_THAT(run_times,
            testing::ElementsAre(scope_start + base::Seconds(1), scope_start + base::Seconds(2), scope_start + base::Seconds(3), scope_start + base::Seconds(4),
    ...
    // otherwise.
    {
        ...
        if (IsIntensiveThrottlingExpected()) {
            const base::TimeTicks aligned_time = scope_start + kIntensiveThrottledWakeUpInterval;
            EXPECT_THAT(run_times, testing::ElementsAre(aligned_time, aligned_time, aligned_time, aligned_time, aligned_time));
        } else {
            EXPECT_THAT(run_times,
                testing::ElementsAre(scope_start + base::Seconds(1), scope_start + base::Seconds(2), scope_start + base::Seconds(3),
    ...
    // should be 1-second aligned.
    {
        ...
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), kLongDelay - kShortDelay);
    ...
    // or favicon update. Otherwise, they should be 1-second aligned.
    {
        ...
        if (IsIntensiveThrottlingExpected()) {
            EXPECT_THAT(run_times,
                testing::ElementsAre(scope_start + base::Minutes(1), scope_start + base::Minutes(2), scope_start + base::Minutes(2),
                    scope_start + base::Minutes(2), scope_start + base::Minutes(2), scope_start + base::Minutes(2)));
        } else {
            EXPECT_THAT(run_times,
                testing::ElementsAre(scope_start + base::Seconds(1), scope_start + base::Seconds(2), scope_start + base::Seconds(3),
                    scope_start + base::Seconds(4), scope_start + base::Seconds(5), scope_start + base::Seconds(6)));
...
TEST_P(FrameSchedulerImplTestWithIntensiveWakeUpThrottling, ManySameOriginFrames)
{
    ASSERT_FALSE(frame_scheduler_->IsCrossOriginToNearestMainFrame());
    const scoped_refptr<base::SingleThreadTaskRunner> task_runner = GetTaskRunner();
    ...
    std::unique_ptr<FrameSchedulerImpl> other_frame_scheduler = CreateFrameScheduler(page_scheduler_.get(), frame_scheduler_delegate_.get(),
        /*is_in_embedded_frame_tree=*/false, FrameScheduler::FrameType::kSubframe);
    ASSERT_FALSE(other_frame_scheduler->IsCrossOriginToNearestMainFrame());
    const scoped_refptr<base::SingleThreadTaskRunner> other_task_runner = GetTaskRunner(other_frame_scheduler.get());
    ...
    std::vector<base::TimeTicks> run_times;
    task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), kDefaultThrottledWakeUpInterval + kShortDelay);
    other_task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), 2 * kDefaultThrottledWakeUpInterval + kShortDelay);
    task_environment_.FastForwardUntilNoTasksRemain();
    ...
    if (IsIntensiveThrottlingExpected()) {
        EXPECT_THAT(run_times, testing::ElementsAre(post_time + 2 * kDefaultThrottledWakeUpInterval, post_time + kIntensiveThrottledWakeUpInterval));
    } else {
        EXPECT_THAT(run_times, testing::ElementsAre(post_time + 2 * kDefaultThrottledWakeUpInterval, post_time + 3 * kDefaultThrottledWakeUpInterval));
    }
}

// Verify that intensive wake up throttling starts after 5 minutes instead of 1
// minute if the page is loading when hidden.
TEST_P(FrameSchedulerImplTestWithIntensiveWakeUpThrottling, TaskExecutionPageLoadingWhenHidden)
{
    ASSERT_FALSE(frame_scheduler_->IsCrossOriginToNearestMainFrame());
    ...
    std::unique_ptr<FrameSchedulerImpl> main_frame_scheduler = CreateFrameScheduler(page_scheduler_.get(), frame_scheduler_delegate_.get(),
        /*is_in_embedded_frame_tree=*/false, FrameScheduler::FrameType::kMainFrame);
    ...
    std::vector<base::TimeTicks> run_times;
    task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), base::Seconds(59));
    task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), base::Seconds(297));
    task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), base::Seconds(298));
    task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), base::Seconds(300));
    task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), base::Seconds(301));

    task_environment_.FastForwardBy(base::Minutes(7));

    if (IsIntensiveThrottlingExpected()) {
        EXPECT_THAT(run_times,
            testing::ElementsAre(test_start + base::Seconds(59), test_start + base::Seconds(297), test_start + base::Seconds(298),
                test_start + base::Seconds(300), test_start + base::Minutes(6)));
    } else {
        EXPECT_THAT(run_times,
            testing::ElementsAre(test_start + base::Seconds(59), test_start + base::Seconds(297), test_start + base::Seconds(298),
                test_start + base::Seconds(300), test_start + base::Seconds(301)));
    }
}

// Verify that intensive throttling is disabled when there is an opt-out.
TEST_P(FrameSchedulerImplTestWithIntensiveWakeUpThrottling, AggressiveThrottlingOptOut)
{
    ...
    // same page.
    const auto other_frame_scheduler = CreateFrameScheduler(page_scheduler_.get(), frame_scheduler_delegate_.get(),
        /*is_in_embedded_frame_tree=*/false, FrameScheduler::FrameType::kSubframe);
    const scoped_refptr<base::SingleThreadTaskRunner> other_task_runner = GetTaskRunner(other_frame_scheduler.get());
...

    {
        ...
        const base::TimeTicks scope_start = base::TimeTicks::Now();
        std::vector<base::TimeTicks> run_times;
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), kShortDelay);
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), kDefaultThrottledWakeUpInterval + kShortDelay);
        task_environment_.FastForwardUntilNoTasksRemain();
        if (IsIntensiveThrottlingExpected()) {
            // Note: Intensive throttling is not applied on the 1st task since there
            // is no recent wake up.
            EXPECT_THAT(run_times, testing::ElementsAre(scope_start + kDefaultThrottledWakeUpInterval, scope_start + kIntensiveThrottledWakeUpInterval));
        } else {
            EXPECT_THAT(run_times, testing::ElementsAre(scope_start + kDefaultThrottledWakeUpInterval, scope_start + 2 * kDefaultThrottledWakeUpInterval));
        }
    }

    {
        // Create an opt-out.
        auto handle = frame_scheduler_->RegisterFeature(SchedulingPolicy::Feature::kWebRTC, { SchedulingPolicy::DisableAggressiveThrottling() });

        {
            ...
            for (int i = 1; i < kNumTasks + 1; ++i) {
                task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), i * kShortDelay);
            }
            task_environment_.FastForwardUntilNoTasksRemain();
            EXPECT_THAT(run_times,
                testing::ElementsAre(scope_start + kDefaultThrottledWakeUpInterval, scope_start + kDefaultThrottledWakeUpInterval,
...

        {
            ...
            std::vector<base::TimeTicks> run_times;
            for (int i = 1; i < kNumTasks + 1; ++i) {
                other_task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), i * kShortDelay);
            }
            task_environment_.FastForwardUntilNoTasksRemain();
            EXPECT_THAT(run_times,
                testing::ElementsAre(scope_start + kDefaultThrottledWakeUpInterval, scope_start + kDefaultThrottledWakeUpInterval,
...
TEST_P(FrameSchedulerImplTestWithIntensiveWakeUpThrottling, FrameChangesOriginType)
{
    ...
    // throughout the test.
    std::unique_ptr<FrameSchedulerImpl> cross_origin_frame_scheduler = CreateFrameScheduler(page_scheduler_.get(), frame_scheduler_delegate_.get(),
        /*is_in_embedded_frame_tree=*/false, FrameScheduler::FrameType::kSubframe);
    cross_origin_frame_scheduler->SetCrossOriginToNearestMainFrame(true);
    const scoped_refptr<base::SingleThreadTaskRunner> cross_origin_task_runner = GetTaskRunner(cross_origin_frame_scheduler.get());
...

    {
        ...
        int counter = 0;
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)), kDefaultThrottledWakeUpInterval);
        int cross_origin_counter = 0;
        cross_origin_task_runner->PostDelayedTask(
            FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&cross_origin_counter)), kDefaultThrottledWakeUpInterval);
...

    {
        ...
        const base::TimeDelta kLongUnalignedDelay = 5 * kIntensiveThrottledWakeUpInterval + kDefaultThrottledWakeUpInterval;
        int counter = 0;
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)), kLongUnalignedDelay);
        int cross_origin_counter = 0;
        cross_origin_task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&cross_origin_counter)), kLongUnalignedDelay);
...
INSTANTIATE_TEST_SUITE_P(AllTimerTaskTypes, FrameSchedulerImplTestWithIntensiveWakeUpThrottling,
    testing::Values(IntensiveWakeUpThrottlingTestParam { /* task_type=*/TaskType::kJavascriptTimerDelayedLowNesting,
                        /* is_intensive_throttling_expected=*/false },
        IntensiveWakeUpThrottlingTestParam { /* task_type=*/TaskType::kJavascriptTimerDelayedHighNesting,
            /* is_intensive_throttling_expected=*/true },
        IntensiveWakeUpThrottlingTestParam { /* task_type=*/TaskType::kWebSchedulingPostedTask,
            /* is_intensive_throttling_expected=*/true }),
    [](const testing::TestParamInfo<IntensiveWakeUpThrottlingTestParam>& info) { return TaskTypeNames::TaskTypeToString(info.param.task_type); });

TEST_F(FrameSchedulerImplTestWithIntensiveWakeUpThrottlingPolicyOverride, PolicyForceEnable)
{
    SetPolicyOverride(/* enabled = */ true);
    EXPECT_TRUE(IsIntensiveWakeUpThrottlingEnabled());

    // The parameters should be the defaults.
    EXPECT_EQ(base::Seconds(kIntensiveWakeUpThrottling_GracePeriodSeconds_Default), GetIntensiveWakeUpThrottlingGracePeriod(false));
}

TEST_F(FrameSchedulerImplTestWithIntensiveWakeUpThrottlingPolicyOverride, PolicyForceDisable)
{
    SetPolicyOverride(/* enabled = */ false);
    EXPECT_FALSE(IsIntensiveWakeUpThrottlingEnabled());
}

class FrameSchedulerImplTestQuickIntensiveWakeUpThrottlingEnabled : public FrameSchedulerImplTest {
public:
    FrameSchedulerImplTestQuickIntensiveWakeUpThrottlingEnabled()
        : FrameSchedulerImplTest({ features::kQuickIntensiveWakeUpThrottlingAfterLoading }, {})
    {
    }
};

TEST_F(FrameSchedulerImplTestQuickIntensiveWakeUpThrottlingEnabled, LoadingPageGracePeriod)
{
    EXPECT_EQ(base::Seconds(kIntensiveWakeUpThrottling_GracePeriodSeconds_Default), GetIntensiveWakeUpThrottlingGracePeriod(true));
}

TEST_F(FrameSchedulerImplTestQuickIntensiveWakeUpThrottlingEnabled, LoadedPageGracePeriod)
{
    EXPECT_EQ(base::Seconds(kIntensiveWakeUpThrottling_GracePeriodSecondsLoaded_Default), GetIntensiveWakeUpThrottlingGracePeriod(false));
}
...
TEST_F(FrameSchedulerImplTest, ImmediateWebSchedulingTasksAreNotThrottled)
{
    ...
    queue->GetTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times));
...
class FrameSchedulerImplThrottleUnimportantFrameTimersEnabledTest : public FrameSchedulerImplTest {
public:
...
TEST_F(FrameSchedulerImplThrottleUnimportantFrameTimersEnabledTest, VisibleSizeChange_CrossOrigin_ExplicitInit)
{
    LazyInitThrottleableTaskQueue();
    EXPECT_FALSE(IsThrottled());
    frame_scheduler_->SetFrameVisible(true);
...
TEST_F(FrameSchedulerImplThrottleUnimportantFrameTimersEnabledTest, UserActivationChange_CrossOrigin_ExplicitInit)
{
    LazyInitThrottleableTaskQueue();
    EXPECT_FALSE(IsThrottled());
    frame_scheduler_->SetFrameVisible(true);
...
TEST_F(FrameSchedulerImplThrottleUnimportantFrameTimersEnabledTest, UnimportantFrameThrottling)
{
    page_scheduler_->SetPageVisible(true);
    frame_scheduler_->SetCrossOriginToNearestMainFrame(true);
    const scoped_refptr<base::SingleThreadTaskRunner> task_runner = frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimerDelayedLowNesting);
    frame_scheduler_->SetFrameVisible(true);
    frame_scheduler_->SetVisibleAreaLarge(false);
    frame_scheduler_->SetHadUserActivation(false);
...
TEST_F(FrameSchedulerImplThrottleUnimportantFrameTimersEnabledTest, HiddenCrossOriginFrameThrottling)
{
    page_scheduler_->SetPageVisible(true);
    frame_scheduler_->SetCrossOriginToNearestMainFrame(true);
    const scoped_refptr<base::SingleThreadTaskRunner> task_runner = frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimerDelayedLowNesting);
...
TEST_F(FrameSchedulerImplThrottleUnimportantFrameTimersEnabledTest, BackgroundPageTimerThrottling)
{
    page_scheduler_->SetPageVisible(false);
...
TEST_F(FrameSchedulerImplThrottleUnimportantFrameTimersEnabledTest, LargeCrossOriginFrameNoThrottling)
{
    page_scheduler_->SetPageVisible(true);
    frame_scheduler_->SetCrossOriginToNearestMainFrame(true);
    const scoped_refptr<base::SingleThreadTaskRunner> task_runner = frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimerDelayedLowNesting);
...
TEST_F(FrameSchedulerImplThrottleUnimportantFrameTimersEnabledTest, UserActivatedCrossOriginFrameNoThrottling)
{
    page_scheduler_->SetPageVisible(true);
    frame_scheduler_->SetCrossOriginToNearestMainFrame(true);
    const scoped_refptr<base::SingleThreadTaskRunner> task_runner = frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimerDelayedLowNesting);
...
class FrameSchedulerImplNoThrottlingVisibleAgentTest : public FrameSchedulerImplTest,
...
public:
    ...
    void SetUp() override
    {
        ...
        if (IsOtherFrameOnDifferentPage()) {
            other_page_scheduler_ = CreatePageScheduler(nullptr, scheduler_.get(), *agent_group_scheduler_);
            EXPECT_TRUE(other_page_scheduler_->IsPageVisible());
        }

        task_runner_ = frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimerDelayedLowNesting);
    ...
    bool IsOtherFrameOnDifferentPage()
    {
        return GetParam();
    }
    ...
    {
        return CreateFrameScheduler(GetOtherFramePageScheduler(), frame_scheduler_delegate_.get(),
            /*is_in_embedded_frame_tree=*/false, FrameScheduler::FrameType::kSubframe);
    }
...
class FrameSchedulerImplNoThrottlingVisibleAgentAndThrottleUnimportantTest : public FrameSchedulerImplNoThrottlingVisibleAgentTest {
public:
    FrameSchedulerImplNoThrottlingVisibleAgentAndThrottleUnimportantTest()
    {
        nested_scoped_feature_list_.InitAndEnableFeature(features::kThrottleUnimportantFrameTimers);
    }
...
// changes.
TEST_P(FrameSchedulerImplNoThrottlingVisibleAgentTest, SameAgentFramePageVisibilityChange)
{
    // Hidden frame with a visible same-agent frame: expect no throttling.
    frame_scheduler_->SetFrameVisible(false);
    auto other_frame_scheduler = CreateOtherFrameScheduler();
...
TEST_P(FrameSchedulerImplNoThrottlingVisibleAgentTest, VisibleFrameOnHiddenPageDeletion)
{
    // Hidden frame with a visible same-agent frame: expect no throttling.
    frame_scheduler_->SetFrameVisible(false);
    auto other_frame_scheduler = CreateOtherFrameScheduler();
...
// correctly.
TEST_P(FrameSchedulerImplNoThrottlingVisibleAgentTest, PageSchedulerWithSameAgentFrameDeleted)
{
    // This test is only relevant when the other frame is on a different page.
    if (!IsOtherFrameOnDifferentPage()) {
        return;
...
// main frame.
TEST_P(FrameSchedulerImplNoThrottlingVisibleAgentTest, SameOriginWithNearestMainFrame)
{
    // Hidden frame with a visible same-agent frame: expect no throttling.
    frame_scheduler_->SetFrameVisible(false);
    auto other_frame_scheduler = CreateOtherFrameScheduler();
...
TEST_P(FrameSchedulerImplNoThrottlingVisibleAgentAndThrottleUnimportantTest, SameAgentWithVisibleFrameIs32msThrottled)
{
...
INSTANTIATE_TEST_SUITE_P(, FrameSchedulerImplNoThrottlingVisibleAgentTest, ::testing::Bool(), &FrameSchedulerImplNoThrottlingVisibleAgentTest::GetSuffix);

INSTANTIATE_TEST_SUITE_P(
    , FrameSchedulerImplNoThrottlingVisibleAgentAndThrottleUnimportantTest, ::testing::Bool(), &FrameSchedulerImplNoThrottlingVisibleAgentTest::GetSuffix);

TEST_F(FrameSchedulerImplTest, DeleteSoonUsesBackupTaskRunner)
{
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = frame_scheduler_->GetTaskRunner(TaskType::kInternalTest);
    int counter = 0;
    task_runner->PostTask(FROM_HERE, base::BindOnce(&IncrementCounter, base::Unretained(&counter)));
    frame_scheduler_.reset();
    ...
    std::unique_ptr<TestObject> test_object = std::make_unique<TestObject>(&counter);
...
TEST_F(FrameSchedulerImplTest, DeleteSoonAfterShutdown)
{
    ...
    std::unique_ptr<TestObject> test_object1 = std::make_unique<TestObject>(&counter);
    ...
    std::unique_ptr<TestObject> test_object2 = std::make_unique<TestObject>(&counter);
