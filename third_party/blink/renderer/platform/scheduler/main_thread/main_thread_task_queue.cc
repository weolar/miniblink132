...
// static
QueueName MainThreadTaskQueue::NameForQueueType(MainThreadTaskQueue::QueueType queue_type)
{
    switch (queue_type) {
...
MainThreadTaskQueue::MainThreadTaskQueue(base::sequence_manager::SequenceManager& sequence_manager, const TaskQueue::Spec& spec,
    const QueueCreationParams& params, MainThreadSchedulerImpl* main_thread_scheduler)
...
{
    task_runner_with_default_task_type_ = WrapTaskRunner(task_queue_->task_runner());
    // Throttling needs |should_notify_observers| to get task timing.
    DCHECK(!params.queue_traits.can_be_throttled || spec.should_notify_observers) << "Throttled queue is not supported with |!should_notify_observers|";
    DCHECK_EQ(web_scheduling_priority_.has_value(), web_scheduling_queue_type_.has_value());
    DCHECK_EQ(web_scheduling_priority_.has_value(), queue_type_ == QueueType::kWebScheduling);
    if (spec.should_notify_observers) {
        if (params.queue_traits.can_be_throttled) {
            throttler_.emplace(task_queue_.get(), main_thread_scheduler_->GetTickClock());
        }
        ...
        task_queue_->SetOnTaskStartedHandler(base::BindRepeating(&MainThreadTaskQueue::OnTaskStarted, base::Unretained(this)));
        task_queue_->SetOnTaskCompletedHandler(base::BindRepeating(&MainThreadTaskQueue::OnTaskCompleted, base::Unretained(this)));
        task_queue_->SetTaskExecutionTraceLogger(base::BindRepeating(&MainThreadTaskQueue::LogTaskExecution, base::Unretained(this)));
    }
}
...
void MainThreadTaskQueue::OnTaskStarted(const base::sequence_manager::Task& task, const base::sequence_manager::TaskQueue::TaskTiming& task_timing)
{
...
void MainThreadTaskQueue::OnTaskCompleted(const base::sequence_manager::Task& task, TaskQueue::TaskTiming* task_timing, base::LazyNow* lazy_now)
{
    if (main_thread_scheduler_) {
...
void MainThreadTaskQueue::LogTaskExecution(perfetto::EventContext& ctx, const base::sequence_manager::Task& task)
{
    *(int*)1 = 1;
//     static const uint8_t* enabled = TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED("scheduler");
//     if (!*enabled)
//         return;
//     RendererMainThreadTaskExecution* execution = ctx.event<ChromeTrackEvent>()->set_renderer_main_thread_task_execution();
//     execution->set_task_type(TaskTypeToProto(static_cast<blink::TaskType>(task.task_type)));
//     if (frame_scheduler_) {
//         frame_scheduler_->WriteIntoTrace(ctx.Wrap(execution));
//     }
}

void MainThreadTaskQueue::OnTaskRunTimeReported(TaskQueue::TaskTiming* task_timing)
{
    if (throttler_) {
        throttler_->OnTaskRunTimeReported(task_timing->start_time(), task_timing->end_time());
    }
}
...
void MainThreadTaskQueue::SetOnIPCTaskPosted(base::RepeatingCallback<void(const base::sequence_manager::Task&)> on_ipc_task_posted_callback)
{
    // We use the frame_scheduler_ to track metrics so as to ensure that metrics
    // are not tied to individual task queues.
    on_ipc_task_posted_callback_handle_ = task_queue_->AddOnTaskPostedHandler(std::move(on_ipc_task_posted_callback));
}
...
void MainThreadTaskQueue::SetFrameSchedulerForTest(FrameSchedulerImpl* frame_scheduler)
{
    frame_scheduler_ = frame_scheduler;
}

void MainThreadTaskQueue::SetWebSchedulingPriority(WebSchedulingPriority priority)
{
...
void MainThreadTaskQueue::AddToBudgetPool(base::TimeTicks now, BudgetPool* pool)
{
    pool->AddThrottler(now, &throttler_.value());
}

void MainThreadTaskQueue::RemoveFromBudgetPool(base::TimeTicks now, BudgetPool* pool)
{
    pool->RemoveThrottler(now, &throttler_.value());
}

void MainThreadTaskQueue::SetWakeUpBudgetPool(WakeUpBudgetPool* wake_up_budget_pool)
{
...
void MainThreadTaskQueue::QueueTraits::WriteIntoTrace(perfetto::TracedValue context) const
{
    auto dict = std::move(context).WriteDictionary();
    ...
    dict.Add("can_run_when_virtual_time_paused", can_run_when_virtual_time_paused);
    dict.Add("can_be_paused_for_android_webview", can_be_paused_for_android_webview);
    dict.Add("prioritisation_type", prioritisation_type);
}

scoped_refptr<base::SingleThreadTaskRunner> MainThreadTaskQueue::CreateTaskRunner(TaskType task_type)
{
    CHECK(task_queue_);
    return WrapTaskRunner(task_queue_->CreateTaskRunner(static_cast<int>(task_type)));
}

scoped_refptr<BlinkSchedulerSingleThreadTaskRunner> MainThreadTaskQueue::WrapTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    ...
    scoped_refptr<base::SingleThreadTaskRunner> cleanup_runner
        = main_thread_scheduler_ && (frame_scheduler_ || agent_group_scheduler_) ? main_thread_scheduler_->CleanupTaskRunner() : nullptr;
    return base::MakeRefCounted<BlinkSchedulerSingleThreadTaskRunner>(std::move(task_runner), std::move(cleanup_runner));
}
