...
public:
    using TaskQueueAndEnabledVoterPair = std::pair<MainThreadTaskQueue*, base::sequence_manager::TaskQueue::QueueEnabledVoter*>;
    ...
    public:
        ...
        virtual void OnTaskQueueCreated(MainThreadTaskQueue*, base::sequence_manager::TaskQueue::QueueEnabledVoter*) = 0;
    };

    FrameTaskQueueController(MainThreadSchedulerImpl*, FrameSchedulerImpl*, Delegate*);
    ...
    scoped_refptr<MainThreadTaskQueue> GetTaskQueue(MainThreadTaskQueue::QueueTraits);

    scoped_refptr<MainThreadTaskQueue> NewWebSchedulingTaskQueue(MainThreadTaskQueue::QueueTraits, WebSchedulingQueueType, WebSchedulingPriority);
    ...
    base::sequence_manager::TaskQueue::QueueEnabledVoter* GetQueueEnabledVoter(const scoped_refptr<MainThreadTaskQueue>&);
...
private:
    ...
    static MainThreadTaskQueue::QueueType QueueTypeFromQueueTraits(MainThreadTaskQueue::QueueTraits);

    const raw_ptr<MainThreadSchedulerImpl, DanglingUntriaged> main_thread_scheduler_impl_;
    const raw_ptr<FrameSchedulerImpl> frame_scheduler_impl_;
    const raw_ptr<Delegate> delegate_;

    using TaskQueueMap = WTF::HashMap<MainThreadTaskQueue::QueueTraitsKeyType, scoped_refptr<MainThreadTaskQueue>>;

    // Map of all TaskQueues, indexed by QueueTraits.
    TaskQueueMap task_queues_;

    using TaskQueueEnabledVoterMap = WTF::HashMap<scoped_refptr<MainThreadTaskQueue>, std::unique_ptr<base::sequence_manager::TaskQueue::QueueEnabledVoter>>;
