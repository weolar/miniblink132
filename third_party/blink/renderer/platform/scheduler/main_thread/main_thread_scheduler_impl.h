...
FORWARD_DECLARE_TEST(MainThreadSchedulerImplTest, LogIpcsPostedToDocumentsInBackForwardCache);
FORWARD_DECLARE_TEST(MainThreadSchedulerImplTest, CanExceedIdleDeadlineIfRequired);
} // namespace main_thread_scheduler_impl_unittest
...
class PLATFORM_EXPORT MainThreadSchedulerImpl : public ThreadSchedulerBase,
...
public:
    ...
    // prioritization.
    static constexpr base::TimeDelta kRenderBlockingStarvationThreshold = base::Milliseconds(500);
    ...
    struct SchedulingSettings {
        ...
        // std::nullopt.
        std::optional<features::TaskDeferralPolicy> discrete_input_task_deferral_policy;
    ...
    explicit MainThreadSchedulerImpl(std::unique_ptr<base::sequence_manager::SequenceManager> sequence_manager);
    explicit MainThreadSchedulerImpl(base::sequence_manager::SequenceManager* sequence_manager);
    ...
    scoped_refptr<base::SingleThreadTaskRunner> DeprecatedDefaultTaskRunner() override;
    std::unique_ptr<MainThread> CreateMainThread() override;
    std::unique_ptr<WebAgentGroupScheduler> CreateWebAgentGroupScheduler() override;
    ...
    [[nodiscard]] std::unique_ptr<MainThreadScheduler::RendererPauseHandle> PauseScheduler() override;
    ...
    void ForEachMainThreadIsolate(base::RepeatingCallback<void(v8::Isolate* isolate)> callback) override;
    Vector<WebInputEventAttribution> GetPendingUserInputInfo(bool include_continuous) const override;
    void ExecuteAfterCurrentTaskForTesting(base::OnceClosure on_completion_task, ExecuteAfterCurrentTaskRestricted) override;
    ...
    void PostNonNestableIdleTask(const base::Location&, Thread::IdleTask) override;
    void PostDelayedIdleTask(const base::Location&, base::TimeDelta delay, Thread::IdleTask) override;
    scoped_refptr<base::SingleThreadTaskRunner> V8TaskRunner() override;
    scoped_refptr<base::SingleThreadTaskRunner> V8UserVisibleTaskRunner() override;
    ...
    MainThreadSchedulerHelper& GetHelper() override
    {
        return helper_;
    }
    ...
    void DidHandleInputEventOnCompositorThread(const WebInputEvent& web_input_event, WidgetScheduler::InputEventState event_state);
    void WillPostInputEventToMainThread(WebInputEvent::Type web_input_event_type, const WebInputEventAttribution& web_input_event_attribution);
    void WillHandleInputEventOnMainThread(WebInputEvent::Type web_input_event_type, const WebInputEventAttribution& web_input_event_attribution);
    void DidHandleInputEventOnMainThread(const WebInputEvent& web_input_event, WebInputEventResult result, bool frame_requested);
    ...
    scoped_refptr<base::SingleThreadTaskRunner> BackForwardCacheIpcTrackingTaskRunner()
    {
    ...
    scoped_refptr<MainThreadTaskQueue> NewTaskQueue(const MainThreadTaskQueue::QueueCreationParams& params);

    // Returns a new throttleable task queue to be used for tests.
    scoped_refptr<MainThreadTaskQueue> NewThrottleableTaskQueueForTest(FrameSchedulerImpl* frame_scheduler);
    ...
    // the main thread.
    void DidCommitProvisionalLoad(bool is_web_history_inert_commit, bool is_reload, bool is_outermost_main_frame);
    ...
    std::unique_ptr<CPUTimeBudgetPool> CreateCPUTimeBudgetPoolForTesting(const char* name);
    ...
    void OnTaskStarted(MainThreadTaskQueue* queue, const base::sequence_manager::Task& task, const base::sequence_manager::TaskQueue::TaskTiming& task_timing);

    // TODO(crbug.com/1143007): Pass `queue` by reference now that the queue is
    // guaranteed to be alive.
    void OnTaskCompleted(base::WeakPtr<MainThreadTaskQueue> queue, const base::sequence_manager::Task& task,
        base::sequence_manager::TaskQueue::TaskTiming* task_timing, base::LazyNow* lazy_now);

    void UpdateIpcTracking();
    void SetOnIPCTaskPostedWhileInBackForwardCacheIfNeeded();
    void OnIPCTaskPostedWhileInAllPagesBackForwardCache(uint32_t ipc_hash, const char* ipc_interface_name);
...
private:
    ...
    friend class main_thread_scheduler_impl_unittest::MainThreadSchedulerImplForTest;
    ...
    FRIEND_TEST_ALL_PREFIXES(main_thread_scheduler_impl_unittest::MainThreadSchedulerImplTest, ShouldIgnoreTaskForUkm);
    FRIEND_TEST_ALL_PREFIXES(main_thread_scheduler_impl_unittest::MainThreadSchedulerImplTest, Tracing);
    FRIEND_TEST_ALL_PREFIXES(main_thread_scheduler_impl_unittest::MainThreadSchedulerImplTest, LogIpcsPostedToDocumentsInBackForwardCache);
    FRIEND_TEST_ALL_PREFIXES(main_thread_scheduler_impl_unittest::MainThreadSchedulerImplTest, CanExceedIdleDeadlineIfRequired);
    ...
    void BeginAgentGroupSchedulerScope(AgentGroupScheduler* next_agent_group_scheduler);
    ...
    public:
        ...
        TaskPriority find_in_page_priority = FindInPageBudgetPoolController::kFindInPageBudgetNotExhaustedPriority;
    ...
    class RendererPauseHandleImpl : public MainThreadScheduler::RendererPauseHandle {
    public:
    ...
    bool CanEnterLongIdlePeriod(base::TimeTicks now, base::TimeDelta* next_long_idle_period_delay_out) override;
    ...
    // an input event.
    void UpdatePrioritizeCompositingAfterInputAfterTaskCompleted(MainThreadTaskQueue* queue);

    // Returns the serialized scheduler state for tracing.
    void WriteIntoTraceLocked(perfetto::TracedValue context, base::TimeTicks optional_now) const;
    ...
    // locked.
    void EnsureUrgentPolicyUpdatePostedOnMainThread(const base::Location& from_here);
    ...
    // any thread.
    UseCase ComputeCurrentUseCase(base::TimeTicks now, base::TimeDelta* expected_use_case_duration) const;
    ...
    void UpdateForInputEventOnCompositorThread(const WebInputEvent& event, WidgetScheduler::InputEventState input_event_state);
    ...
    void UpdateTaskQueueState(MainThreadTaskQueue* task_queue, base::sequence_manager::TaskQueue::QueueEnabledVoter* task_queue_enabled_voter,
    ...
    // policy if necessary.
    void MaybeUpdatePolicyOnTaskCompleted(MainThreadTaskQueue*, const base::sequence_manager::TaskQueue::TaskTiming&);
    ...
    void UpdateRenderingPrioritizationStateOnTaskCompleted(MainThreadTaskQueue*, const base::sequence_manager::TaskQueue::TaskTiming&);
    ...
    void RecordTaskUkm(MainThreadTaskQueue* queue, const base::sequence_manager::Task& task, const base::sequence_manager::TaskQueue::TaskTiming& task_timing);

    UkmRecordingStatus RecordTaskUkmImpl(MainThreadTaskQueue* queue, const base::sequence_manager::Task& task,
        const base::sequence_manager::TaskQueue::TaskTiming& task_timing, FrameSchedulerImpl* frame_scheduler, bool precise_attribution);
    ...
    std::unique_ptr<base::sequence_manager::SequenceManager> owned_sequence_manager_;
    MainThreadSchedulerHelper helper_;
    scoped_refptr<MainThreadTaskQueue> idle_helper_queue_;
    std::unique_ptr<base::sequence_manager::TaskQueue::QueueEnabledVoter> idle_queue_voter_;
    IdleHelper idle_helper_;
    RenderWidgetSignals render_widget_scheduler_signals_;

    std::unique_ptr<FindInPageBudgetPoolController> find_in_page_budget_pool_controller_;
    ...
    scoped_refptr<MainThreadTaskQueue> back_forward_cache_ipc_tracking_task_queue_;

    using TaskQueueVoterMap ALLOW_DISCOURAGED_TYPE("TODO(crbug.com/1404327)")
        = std::map<scoped_refptr<MainThreadTaskQueue>, std::unique_ptr<base::sequence_manager::TaskQueue::QueueEnabledVoter>>;
    ...
    scoped_refptr<base::SingleThreadTaskRunner> back_forward_cache_ipc_tracking_task_runner_;
    ...
    struct MainThreadOnly {
        MainThreadOnly(MainThreadSchedulerImpl* main_thread_scheduler_impl, const base::TickClock* time_source, base::TimeTicks now);
        ~MainThreadOnly();
        ...
        TraceableCounter<int, TracingCategory::kInfo> renderer_pause_count; // Renderer is paused if non-zero.

        TraceableObjectState<RAILMode,
            TracingCategory::kTopLevel> rail_mode_for_tracing; // Don't use except for tracing.
        ...
        TraceableObjectState<bool, TracingCategory::kTopLevel> renderer_backgrounded;
        TraceableState<bool, TracingCategory::kDefault> blocking_input_expected_soon;
        ...
        TraceableState<bool, TracingCategory::kDebug> compositor_will_send_main_frame_not_expected;
        ...
        base::ObserverList<RAILModeObserver>::Unchecked rail_mode_observers; // Not owned.
        MainThreadMetricsHelper metrics_helper;
        TraceableState<WebRendererProcessType, TracingCategory::kTopLevel> process_type;
        TraceableState<std::optional<TaskDescriptionForTracing>,
            TracingCategory::kInfo> task_description_for_tracing; // Don't use except for tracing.
        TraceableState<std::optional<TaskPriority>,
            TracingCategory::kInfo> task_priority_for_tracing; // Only used for tracing.

        // Holds task queues that are currently running.
        // The queue for the inmost task is at the top of stack when there are
        // nested RunLoops.
        std::stack<scoped_refptr<MainThreadTaskQueue>, std::vector<scoped_refptr<MainThreadTaskQueue>>> running_queues;
        ...
        TraceableState<RenderingPrioritizationState, TracingCategory::kDefault> main_frame_prioritization_state;
        ...
        Persistent<HeapHashSet<WeakMember<AgentGroupSchedulerImpl>>> agent_group_schedulers;
    ...
    struct AnyThread {
        ...
        TraceableState<bool, TracingCategory::kInfo> awaiting_discrete_input_response;
        TraceableState<bool, TracingCategory::kInfo> in_idle_period;
        TraceableState<bool, TracingCategory::kInfo> begin_main_frame_on_critical_path;
        TraceableState<bool, TracingCategory::kInfo> last_gesture_was_compositor_driven;
        ...
        TraceableState<bool, TracingCategory::kInfo> waiting_for_any_main_frame_contentful_paint;
        TraceableState<bool, TracingCategory::kInfo> waiting_for_any_main_frame_meaningful_paint;
        TraceableState<bool, TracingCategory::kInfo> is_any_main_frame_loading;
        TraceableState<bool, TracingCategory::kInfo> have_seen_input_since_navigation;
    };
