...
// no longer needed.
class PLATFORM_EXPORT MainThreadTaskQueue : public ThreadSafeRefCounted<MainThreadTaskQueue> {
public:
    ...
    // lifetime.
    static base::sequence_manager::QueueName NameForQueueType(QueueType queue_type);
    ...
    struct QueueTraits {
        ...
        static constexpr unsigned kPrioritisationTypeWidthBits = std::bit_width(static_cast<unsigned>(PrioritisationType::kMaxValue));

        // Ensure that the count of the enumeration does not exceed the
        // representable range
        static_assert(static_cast<unsigned>(PrioritisationType::kMaxValue) < (1u << kPrioritisationTypeWidthBits),
            "PrioritisationType count exceeds the bit width range");

        // Ensure that the count of the enumeration is not less than half the
        // representable range
        static_assert(static_cast<unsigned>(PrioritisationType::kMaxValue) >= (1u << (kPrioritisationTypeWidthBits - 1)),
    ...
    struct QueueCreationParams {
        ...
        QueueCreationParams SetWebSchedulingQueueType(std::optional<WebSchedulingQueueType> type)
        {
            web_scheduling_queue_type = type;
            return *this;
        }

        QueueCreationParams SetWebSchedulingPriority(std::optional<WebSchedulingPriority> priority)
        {
            web_scheduling_priority = priority;
            return *this;
        }

        QueueCreationParams SetAgentGroupScheduler(AgentGroupSchedulerImpl* scheduler)
        {
        ...
        QueueCreationParams SetPrioritisationType(QueueTraits::PrioritisationType type)
        {
    ...
    MainThreadTaskQueue(base::sequence_manager::SequenceManager& sequence_manager, const TaskQueue::Spec& spec, const QueueCreationParams& params,
    ...
    {
        return queue_traits_.can_be_deferred;
    }
    ...
    bool CanBeThrottled() const
    {
        return queue_traits_.can_be_throttled;
    }
    ...
    {
        return queue_traits_.can_be_paused;
    }
    ...
    {
        return queue_traits_.can_be_frozen;
    }
    ...
    QueueTraits GetQueueTraits() const
    {
        return queue_traits_;
    }
    ...
    void OnTaskCompleted(const base::sequence_manager::Task& task, TaskQueue::TaskTiming* task_timing, base::LazyNow* lazy_now);

    void LogTaskExecution(perfetto::EventContext& ctx, const base::sequence_manager::Task& task);

    void SetOnIPCTaskPosted(base::RepeatingCallback<void(const base::sequence_manager::Task&)> on_ipc_task_posted_callback);
    ...
    scoped_refptr<base::SingleThreadTaskRunner> CreateTaskRunner(TaskType task_type);
    ...
    const scoped_refptr<base::SingleThreadTaskRunner>& GetTaskRunnerWithDefaultTaskType()
    {
    ...
    WakeUpBudgetPool* GetWakeUpBudgetPool() const
    {
    ...
    {
        return task_queue_->IsQueueEnabled();
    }
    bool IsEmpty() const
    {
...
private:
    ...
    scoped_refptr<BlinkSchedulerSingleThreadTaskRunner> WrapTaskRunner(scoped_refptr<base::SingleThreadTaskRunner>);

    TaskQueue::Handle task_queue_;
    scoped_refptr<base::SingleThreadTaskRunner> task_runner_with_default_task_type_;
    ...
    std::unique_ptr<TaskQueue::OnTaskPostedCallbackHandle> on_ipc_task_posted_callback_handle_;
