...
FrameThrottlingState GetFrameThrottlingState(const FrameScheduler& frame_scheduler)
{
...
FrameStatus GetFrameStatus(FrameScheduler* frame_scheduler)
{
    ...
    FrameThrottlingState throttling_state = GetFrameThrottlingState(*frame_scheduler);
    FrameOriginState origin_state = GetFrameOriginState(*frame_scheduler);
    return static_cast<FrameStatus>(static_cast<int>(FrameStatus::kSpecialCasesCount)
        + static_cast<int>(origin_state) * static_cast<int>(FrameThrottlingState::kCount) + static_cast<int>(throttling_state));
}
