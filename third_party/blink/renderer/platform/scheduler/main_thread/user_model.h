...
public:
    ...
    static constexpr base::TimeDelta kGestureEstimationLimit = base::Milliseconds(100);

    // This is based on two weeks of Android usage data.
    static constexpr base::TimeDelta kMedianGestureDuration = base::Milliseconds(300);
    ...
    static constexpr base::TimeDelta kExpectSubsequentGestureDeadline = base::Milliseconds(2000);

    // The maximum amount of time tasks will be deferred in response to discrete
    // input.
    static constexpr base::TimeDelta kDiscreteInputResponseDeadline = base::Milliseconds(50);
    ...
    // gestures.
    void DidStartProcessingInputEvent(WebInputEvent::Type type, const base::TimeTicks now);
    ...
    bool IsGestureExpectedSoon(const base::TimeTicks now, base::TimeDelta* prediction_valid_duration);
    ...
    bool IsGestureExpectedToContinue(const base::TimeTicks now, base::TimeDelta* prediction_valid_duration) const;

    // Returns the time left before the deadline for UI response of the most
    // recent discrete input event.
    base::TimeDelta TimeLeftUntilDiscreteInputResponseDeadline(base::TimeTicks now) const;
...
private:
    bool IsGestureExpectedSoonImpl(const base::TimeTicks now, base::TimeDelta* prediction_valid_duration) const;
