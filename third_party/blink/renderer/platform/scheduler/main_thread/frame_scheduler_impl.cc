...
BASE_FEATURE(kRendererMainIsDefaultThreadTypeForWebRTC, "RendererMainIsNormalThreadTypeForWebRTC", base::FEATURE_ENABLED_BY_DEFAULT);
...
{
    switch (features::kLowPriorityAsyncScriptExecutionLowerTaskPriorityParam.Get()) {
...
FrameSchedulerImpl::PauseSubresourceLoadingHandleImpl::PauseSubresourceLoadingHandleImpl(base::WeakPtr<FrameSchedulerImpl> frame_scheduler)
...
FrameSchedulerImpl::PauseSubresourceLoadingHandleImpl::~PauseSubresourceLoadingHandleImpl()
{
    if (frame_scheduler_)
...
FrameSchedulerImpl::FrameSchedulerImpl(
    PageSchedulerImpl* parent_page_scheduler, FrameScheduler::Delegate* delegate, bool is_in_embedded_frame_tree, FrameScheduler::FrameType frame_type)
    : FrameSchedulerImpl(parent_page_scheduler->GetMainThreadScheduler(), parent_page_scheduler, delegate, is_in_embedded_frame_tree, frame_type)
{
}

FrameSchedulerImpl::FrameSchedulerImpl(MainThreadSchedulerImpl* main_thread_scheduler, PageSchedulerImpl* parent_page_scheduler,
    FrameScheduler::Delegate* delegate, bool is_in_embedded_frame_tree, FrameScheduler::FrameType frame_type)
    : frame_type_(frame_type)
    , is_in_embedded_frame_tree_(is_in_embedded_frame_tree)
    , main_thread_scheduler_(main_thread_scheduler)
    , parent_page_scheduler_(parent_page_scheduler)
    , delegate_(delegate)
    , page_visibility_(parent_page_scheduler_ && parent_page_scheduler_->IsPageVisible() ? PageVisibilityState::kVisible : PageVisibilityState::kHidden,
          "FrameScheduler.PageVisibility", &tracing_controller_, PageVisibilityStateToString)
    , frame_visible_(true, "FrameScheduler.FrameVisible", &tracing_controller_, VisibilityStateToString)
    , is_visible_area_large_(true, "FrameScheduler.IsVisibleAreaLarge", &tracing_controller_, IsVisibleAreaLargeStateToString)
    , had_user_activation_(false, "FrameScheduler.HadUserActivation", &tracing_controller_, UserActivationStateToString)
    , frame_paused_(false, "FrameScheduler.FramePaused", &tracing_controller_, PausedStateToString)
    , frame_origin_type_(frame_type == FrameType::kMainFrame ? FrameOriginType::kMainFrame : FrameOriginType::kSameOriginToMainFrame, "FrameScheduler.Origin",
          &tracing_controller_, FrameOriginTypeToString)
    , subresource_loading_paused_(false, "FrameScheduler.SubResourceLoadingPaused", &tracing_controller_, PausedStateToString)
    , url_tracer_("FrameScheduler.URL")
    , throttling_type_(ThrottlingType::kNone, "FrameScheduler.ThrottlingType", &tracing_controller_, ThrottlingTypeToString)
    , preempted_for_cooperative_scheduling_(false, "FrameScheduler.PreemptedForCooperativeScheduling", &tracing_controller_, YesNoStateToString)
    , aggressive_throttling_opt_out_count_(0)
    , opted_out_from_aggressive_throttling_(false, "FrameScheduler.AggressiveThrottlingDisabled", &tracing_controller_, YesNoStateToString)
    , subresource_loading_pause_count_(0u)
    , back_forward_cache_disabling_feature_tracker_(&tracing_controller_, main_thread_scheduler_)
    , low_priority_async_script_task_priority_(GetLowPriorityAsyncScriptTaskPriority())
    , page_frozen_for_tracing_(
          parent_page_scheduler_ ? parent_page_scheduler_->IsFrozen() : true, "FrameScheduler.PageFrozen", &tracing_controller_, FrozenStateToString)
    , waiting_for_contentful_paint_(true, "FrameScheduler.WaitingForContentfulPaint", &tracing_controller_, YesNoStateToString)
    , waiting_for_meaningful_paint_(true, "FrameScheduler.WaitingForMeaningfulPaint", &tracing_controller_, YesNoStateToString)
    , is_load_event_dispatched_(false, "FrameScheduler.IsLoadEventDispatched", &tracing_controller_, YesNoStateToString)
{
    frame_task_queue_controller_ = base::WrapUnique(new FrameTaskQueueController(main_thread_scheduler_, this, this));
...
FrameSchedulerImpl::FrameSchedulerImpl()
    : FrameSchedulerImpl(/*main_thread_scheduler=*/nullptr,
        ...
        /*delegate=*/nullptr,
        /*is_in_embedded_frame_tree=*/false, FrameType::kSubframe)
{
}
...
FrameSchedulerImpl::~FrameSchedulerImpl()
{
    ...
    for (const auto& task_queue_and_voter : frame_task_queue_controller_->GetAllTaskQueuesAndVoters()) {
    ...
    if (parent_page_scheduler_) {
        ...
        if (AreFrameAndPageVisible()) {
            ...
            GetAgentGroupSchedulerImpl().DecrementVisibleFramesForAgent(agent_cluster_id_, policy_updater);
        }
...
void FrameSchedulerImpl::OnPageVisibilityChange(PageVisibilityState page_visibility, PolicyUpdater& policy_updater)
{
    CHECK_NE(page_visibility, page_visibility_.get());

    const bool were_frame_and_page_visible = AreFrameAndPageVisible();
    page_visibility_ = page_visibility;
    CHECK_EQ(page_visibility_ == PageVisibilityState::kVisible, parent_page_scheduler_->IsPageVisible());
...
void FrameSchedulerImpl::OnPageSchedulerDeletion(PolicyUpdater& policy_updater)
{
    for (const auto& task_queue_and_voter : frame_task_queue_controller_->GetAllTaskQueuesAndVoters()) {
        if (task_queue_and_voter.first->CanBeThrottled()) {
            RemoveThrottleableQueueFromBudgetPools(task_queue_and_voter.first);
...
void FrameSchedulerImpl::OnFrameAndPageVisibleChanged(PolicyUpdater& policy_updater)
{
    if (AreFrameAndPageVisible()) {
        return GetAgentGroupSchedulerImpl().IncrementVisibleFramesForAgent(agent_cluster_id_, policy_updater);
    } else {
        return GetAgentGroupSchedulerImpl().DecrementVisibleFramesForAgent(agent_cluster_id_, policy_updater);
    }
}

void FrameSchedulerImpl::RemoveThrottleableQueueFromBudgetPools(MainThreadTaskQueue* task_queue)
{
    DCHECK(task_queue);
    ...
    CPUTimeBudgetPool* cpu_time_budget_pool = parent_page_scheduler_->background_cpu_time_budget_pool();

    // On tests, the scheduler helper might already be shut down and tick is not
    // available.
    base::LazyNow lazy_now
        = main_thread_scheduler_->GetTickClock() ? base::LazyNow(main_thread_scheduler_->GetTickClock()) : base::LazyNow(base::TimeTicks::Now());

    if (cpu_time_budget_pool) {
        task_queue->RemoveFromBudgetPool(lazy_now.Now(), cpu_time_budget_pool);
    }

    parent_page_scheduler_->RemoveQueueFromWakeUpBudgetPool(task_queue, &lazy_now);
}
...
void FrameSchedulerImpl::SetAgentClusterId(const base::UnguessableToken& agent_cluster_id)
{
    PolicyUpdater policy_updater;

    if (AreFrameAndPageVisible()) {
        GetAgentGroupSchedulerImpl().IncrementVisibleFramesForAgent(agent_cluster_id, policy_updater);
        GetAgentGroupSchedulerImpl().DecrementVisibleFramesForAgent(agent_cluster_id_, policy_updater);
    }
...
void FrameSchedulerImpl::AddTaskTime(base::TimeDelta time)
{
    ...
    constexpr base::TimeDelta kTaskDurationSendThreshold = base::Milliseconds(100);
    if (!delegate_)
        return;
...
QueueTraits FrameSchedulerImpl::CreateQueueTraitsForTaskType(TaskType type)
{
    ...
    case TaskType::kInternalContentCapture:
        return ThrottleableTaskQueueTraits().SetPrioritisationType(QueueTraits::PrioritisationType::kBestEffort);
    case TaskType::kJavascriptTimerDelayedLowNesting:
        return ThrottleableTaskQueueTraits().SetPrioritisationType(QueueTraits::PrioritisationType::kJavaScriptTimer);
    case TaskType::kJavascriptTimerDelayedHighNesting:
        return ThrottleableTaskQueueTraits()
            .SetPrioritisationType(QueueTraits::PrioritisationType::kJavaScriptTimer)
    ...
    case TaskType::kJavascriptTimerImmediate: {
        ...
        return DeferrableTaskQueueTraits().SetPrioritisationType(QueueTraits::PrioritisationType::kJavaScriptTimer);
    }
    case TaskType::kInternalLoading:
    case TaskType::kNetworking:
        return LoadingTaskQueueTraits();
    case TaskType::kNetworkingUnfreezable:
        return IsInflightNetworkRequestBackForwardCacheSupportEnabled() ? UnfreezableLoadingTaskQueueTraits() : LoadingTaskQueueTraits();
    case TaskType::kNetworkingUnfreezableRenderBlockingLoading: {
        QueueTraits queue_traits = IsInflightNetworkRequestBackForwardCacheSupportEnabled() ? UnfreezableLoadingTaskQueueTraits() : LoadingTaskQueueTraits();
        queue_traits.SetPrioritisationType(QueueTraits::PrioritisationType::kRenderBlocking);
        return queue_traits;
    }
    case TaskType::kNetworkingControl:
        return LoadingControlTaskQueueTraits();
    case TaskType::kLowPriorityScriptExecution:
        return LoadingTaskQueueTraits().SetPrioritisationType(QueueTraits::PrioritisationType::kAsyncScript);
    ...
    case TaskType::kInternalHighPriorityLocalFrame:
        return QueueTraits().SetPrioritisationType(QueueTraits::PrioritisationType::kHighPriorityLocalFrame);
    case TaskType::kInternalContinueScriptLoading:
        return PausableTaskQueueTraits().SetPrioritisationType(QueueTraits::PrioritisationType::kInternalScriptContinuation);
    case TaskType::kDatabaseAccess:
        if (base::FeatureList::IsEnabled(kHighPriorityDatabaseTaskType)) {
            return PausableTaskQueueTraits().SetPrioritisationType(QueueTraits::PrioritisationType::kExperimentalDatabase);
        } else {
            return PausableTaskQueueTraits();
        }
    ...
    case TaskType::kInternalNavigationCancellation:
        return FreezableTaskQueueTraits().SetPrioritisationType(QueueTraits::PrioritisationType::kInternalNavigationCancellation);
    ...
    case TaskType::kInternalPostMessageForwarding:
        ...
        return PausableTaskQueueTraits().SetCanBeFrozen(false).SetPrioritisationType(QueueTraits::PrioritisationType::kPostMessageForwarding);
...
scoped_refptr<base::SingleThreadTaskRunner> FrameSchedulerImpl::GetTaskRunner(TaskType type)
{
...
scoped_refptr<MainThreadTaskQueue> FrameSchedulerImpl::GetTaskQueue(TaskType type)
{
    QueueTraits queue_traits = CreateQueueTraitsForTaskType(type);
    queue_traits = queue_traits.SetCanBeDeferredForRendering(ComputeCanBeDeferredForRendering(queue_traits.can_be_deferred, type));
    return frame_task_queue_controller_->GetTaskQueue(queue_traits);
}

scoped_refptr<base::SingleThreadTaskRunner> FrameSchedulerImpl::ControlTaskRunner()
{
    DCHECK(parent_page_scheduler_);
...
{
    return parent_page_scheduler_ ? &parent_page_scheduler_->GetAgentGroupScheduler() : nullptr;
}
...
{
    main_thread_scheduler_->DidStartProvisionalLoad(frame_type_ == FrameScheduler::FrameType::kMainFrame && !is_in_embedded_frame_tree_);
}

void FrameSchedulerImpl::DidCommitProvisionalLoad(bool is_web_history_inert_commit, NavigationType navigation_type, DidCommitProvisionalLoadParams params)
{
    bool is_outermost_main_frame = GetFrameType() == FrameType::kMainFrame && !is_in_embedded_frame_tree_;
    ...
    main_thread_scheduler_->DidCommitProvisionalLoad(is_web_history_inert_commit, navigation_type == NavigationType::kReload, is_outermost_main_frame);
    if (!is_same_document)
        ResetForNavigation();
}

WebScopedVirtualTimePauser FrameSchedulerImpl::CreateWebScopedVirtualTimePauser(
    const WTF::String& name, WebScopedVirtualTimePauser::VirtualTaskDuration duration)
{
    return WebScopedVirtualTimePauser(main_thread_scheduler_, duration, name);
}

scoped_refptr<base::SingleThreadTaskRunner> FrameSchedulerImpl::CompositorTaskRunner()
{
    return parent_page_scheduler_->GetAgentGroupScheduler().CompositorTaskRunner();
}
...
void FrameSchedulerImpl::OnStartedUsingNonStickyFeature(SchedulingPolicy::Feature feature, const SchedulingPolicy& policy,
    std::unique_ptr<SourceLocation> source_location, SchedulingAffectingFeatureHandle* handle)
{
    if (policy.disable_aggressive_throttling)
        OnAddedAggressiveThrottlingOptOut();
    if (policy.disable_back_forward_cache) {
        back_forward_cache_disabling_feature_tracker_.AddNonStickyFeature(feature, std::move(source_location), handle);
    }
    ...
    if (feature == SchedulingPolicy::Feature::kWebRTC) {
        if (base::FeatureList::IsEnabled(kRendererMainIsDefaultThreadTypeForWebRTC)
            && base::PlatformThread::GetCurrentThreadType() == base::ThreadType::kDisplayCritical) {
...
void FrameSchedulerImpl::OnStartedUsingStickyFeature(
    SchedulingPolicy::Feature feature, const SchedulingPolicy& policy, std::unique_ptr<SourceLocation> source_location)
{
    if (policy.disable_aggressive_throttling)
        OnAddedAggressiveThrottlingOptOut();
    if (policy.disable_back_forward_cache) {
        back_forward_cache_disabling_feature_tracker_.AddStickyFeature(feature, std::move(source_location));
    }
    if (policy.disable_align_wake_ups) {
        DisableAlignWakeUpsForProcess();
    }
...
void FrameSchedulerImpl::OnStoppedUsingNonStickyFeature(SchedulingAffectingFeatureHandle* handle)
{
    ...
    if (handle->GetPolicy().disable_back_forward_cache) {
        back_forward_cache_disabling_feature_tracker_.Remove(handle->GetFeatureAndJSLocationBlockingBFCache());
    }
...
base::WeakPtr<FrameSchedulerImpl> FrameSchedulerImpl::GetInvalidatingOnBFCacheRestoreWeakPtr()
{
...
void FrameSchedulerImpl::OnAddedAggressiveThrottlingOptOut()
{
    ...
    opted_out_from_aggressive_throttling_ = static_cast<bool>(aggressive_throttling_opt_out_count_);
    if (parent_page_scheduler_)
        parent_page_scheduler_->OnThrottlingStatusUpdated();
}
...
{
    return parent_page_scheduler_ ? parent_page_scheduler_->IsPageVisible() : true;
}
...
void FrameSchedulerImpl::SetShouldReportPostedTasksWhenDisabled(bool should_report)
{
...
void FrameSchedulerImpl::UpdatePolicy()
{
    ...
    for (const auto& task_queue_and_voter : frame_task_queue_controller_->GetAllTaskQueuesAndVoters()) {
        ...
        auto* new_wake_up_budget_pool = parent_page_scheduler_->GetWakeUpBudgetPool(task_queue, frame_origin_type_, throttling_type_);
        if (task_queue->GetWakeUpBudgetPool() != new_wake_up_budget_pool) {
            parent_page_scheduler_->RemoveQueueFromWakeUpBudgetPool(task_queue, &lazy_now);
            parent_page_scheduler_->AddQueueToWakeUpBudgetPool(task_queue, new_wake_up_budget_pool, &lazy_now);
        }

        if (throttling_type_ != ThrottlingType::kNone && previous_throttling_type == ThrottlingType::kNone) {
            MainThreadTaskQueue::ThrottleHandle handle = task_queue_and_voter.first->Throttle();
...
void FrameSchedulerImpl::UpdateQueuePolicy(MainThreadTaskQueue* queue, TaskQueue::QueueEnabledVoter* voter)
{
    DCHECK(queue);
    ...
    bool queue_frozen = parent_page_scheduler_->IsFrozen() && queue->CanBeFrozen();
    ...
    if (queue_disabled) {
        TRACE_EVENT_INSTANT("renderer.scheduler", "FrameSchedulerImpl::UpdateQueuePolicy_QueueDisabled");
    } else {
        TRACE_EVENT_INSTANT("renderer.scheduler", "FrameSchedulerImpl::UpdateQueuePolicy_QueueEnabled");
    }
    voter->SetVoteToEnable(!queue_disabled);
}

SchedulingLifecycleState FrameSchedulerImpl::CalculateLifecycleState(ObserverType type) const
{
    ...
    if (subresource_loading_paused_ && type == ObserverType::kLoader)
        return SchedulingLifecycleState::kStopped;
    if (type == ObserverType::kLoader && parent_page_scheduler_->OptedOutFromAggressiveThrottling()) {
        return SchedulingLifecycleState::kNotThrottled;
    }
...
void FrameSchedulerImpl::OnFirstMeaningfulPaint(base::TimeTicks timestamp)
{
    ...
    if (GetFrameType() != FrameScheduler::FrameType::kMainFrame || is_in_embedded_frame_tree_) {
        return;
    }
...
bool FrameSchedulerImpl::IsLoading() const
{
    ...
    return base::TimeTicks::Now() - first_meaningful_paint_timestamp_ <= GetLoadingPhaseBufferTimeAfterFirstMeaningfulPaint();
}
...
ThrottlingType FrameSchedulerImpl::ComputeThrottlingType()
{
    ...
    const bool page_can_be_throttled_intensively = !parent_page_scheduler_->IsAudioPlaying() && !parent_page_scheduler_->IsPageVisible();

    const bool frame_can_be_throttled_background = !AreFrameAndPageVisible() && !parent_page_scheduler_->IsAudioPlaying()
        && !(parent_page_scheduler_->IsPageVisible() && !IsCrossOriginToNearestMainFrame())
        && !(base::FeatureList::IsEnabled(features::kNoThrottlingVisibleAgent) && GetAgentGroupSchedulerImpl().IsAgentVisible(agent_cluster_id_));

    const bool frame_can_be_throttled_foreground
        = IsCrossOriginToNearestMainFrame() && !IsImportant() && base::FeatureList::IsEnabled(features::kThrottleUnimportantFrameTimers);
...
TaskPriority FrameSchedulerImpl::ComputePriority(MainThreadTaskQueue* task_queue) const
{
    DCHECK(task_queue);
    ...
    case MainThreadTaskQueue::QueueTraits::PrioritisationType::kInternalScriptContinuation:
        return TaskPriority::kVeryHighPriority;
    case MainThreadTaskQueue::QueueTraits::PrioritisationType::kBestEffort:
        return TaskPriority::kBestEffortPriority;
    case MainThreadTaskQueue::QueueTraits::PrioritisationType::kPostMessageForwarding:
        return TaskPriority::kVeryHighPriority;
    case MainThreadTaskQueue::QueueTraits::PrioritisationType::kInternalNavigationCancellation:
        return TaskPriority::kVeryHighPriority;
    default:
        break;
    }
    ...
    if (std::optional<WebSchedulingQueueType> queue_type = task_queue->GetWebSchedulingQueueType()) {
        bool is_continuation = *queue_type == WebSchedulingQueueType::kContinuationQueue;
        switch (*task_queue->GetWebSchedulingPriority()) {
        case WebSchedulingPriority::kUserBlockingPriority:
            return is_continuation ? TaskPriority::kHighPriorityContinuation : TaskPriority::kHighPriority;
        case WebSchedulingPriority::kUserVisiblePriority:
            return is_continuation ? TaskPriority::kNormalPriorityContinuation : TaskPriority::kNormalPriority;
        case WebSchedulingPriority::kBackgroundPriority:
            return is_continuation ? TaskPriority::kLowPriorityContinuation : TaskPriority::kLowPriority;
        }
    }
    ...
    if (task_queue->GetPrioritisationType() == MainThreadTaskQueue::QueueTraits::PrioritisationType::kLoadingControl) {
        return TaskPriority::kHighPriority;
    }

    if (task_queue->GetPrioritisationType() == MainThreadTaskQueue::QueueTraits::PrioritisationType::kFindInPage) {
        return main_thread_scheduler_->find_in_page_priority();
    }

    if (task_queue->GetPrioritisationType() == MainThreadTaskQueue::QueueTraits::PrioritisationType::kHighPriorityLocalFrame) {
        return TaskPriority::kHighestPriority;
    }

    if (task_queue->GetPrioritisationType() == MainThreadTaskQueue::QueueTraits::PrioritisationType::kInput) {
        return TaskPriority::kHighestPriority;
    }

    if (task_queue->GetPrioritisationType() == MainThreadTaskQueue::QueueTraits::PrioritisationType::kExperimentalDatabase) {
        // TODO(shaseley): This decision should probably be based on Agent
        // visibility. Consider changing this before shipping anything.
        return parent_page_scheduler_->IsPageVisible() ? TaskPriority::kHighPriority : TaskPriority::kNormalPriority;
    }

    if (task_queue->GetPrioritisationType() == MainThreadTaskQueue::QueueTraits::PrioritisationType::kRenderBlocking) {
        return parent_page_scheduler_->IsPageVisible() ? TaskPriority::kExtremelyHighPriority : TaskPriority::kNormalPriority;
    }

    if (task_queue->GetPrioritisationType() == MainThreadTaskQueue::QueueTraits::PrioritisationType::kAsyncScript) {
...
std::unique_ptr<blink::mojom::blink::PauseSubresourceLoadingHandle> FrameSchedulerImpl::GetPauseSubresourceLoadingHandle()
{
    return std::make_unique<PauseSubresourceLoadingHandleImpl>(weak_factory_.GetWeakPtr());
}
...
void FrameSchedulerImpl::OnTaskQueueCreated(MainThreadTaskQueue* task_queue, base::sequence_manager::TaskQueue::QueueEnabledVoter* voter)
{
    DCHECK(parent_page_scheduler_);
    ...
    if (task_queue->CanBeThrottled()) {
        ...
        if (throttling_type_ != ThrottlingType::kNone) {
            parent_page_scheduler_->AddQueueToWakeUpBudgetPool(
                task_queue, parent_page_scheduler_->GetWakeUpBudgetPool(task_queue, frame_origin_type_, throttling_type_), &lazy_now);
...
void FrameSchedulerImpl::SetOnIPCTaskPostedWhileInBackForwardCacheHandler()
{
    ...
    for (const auto& task_queue_and_voter : frame_task_queue_controller_->GetAllTaskQueuesAndVoters()) {
        task_queue_and_voter.first->SetOnIPCTaskPosted(base::BindRepeating(
            [](scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::WeakPtr<FrameSchedulerImpl> frame_scheduler,
                ...
                base::ScopedDeferTaskPosting::PostOrDefer(task_runner, FROM_HERE,
                    base::BindOnce(&FrameSchedulerImpl::OnIPCTaskPostedWhileInBackForwardCache, frame_scheduler, task.ipc_hash, task.ipc_interface_name),
                    base::Seconds(1));
            },
            main_thread_scheduler_->BackForwardCacheIpcTrackingTaskRunner(), GetInvalidatingOnBFCacheRestoreWeakPtr()));
    }
}
...
void FrameSchedulerImpl::OnIPCTaskPostedWhileInBackForwardCache(uint32_t ipc_hash, const char* ipc_interface_name)
{
    ...
    if (!ipc_hash) {
        ...
        ipc_hash = base::TaskAnnotator::ScopedSetIpcHash::MD5HashMetricName(ipc_interface_name);
    }
    ...
    base::TimeDelta duration = main_thread_scheduler_->NowTicks() - parent_page_scheduler_->GetStoredInBackForwardCacheTimestamp();
    base::UmaHistogramCustomTimes("BackForwardCache.Experimental.UnexpectedIPCMessagePostedToCachedFrame."
                                  "TimeUntilIPCReceived",
        duration, base::TimeDelta(), base::Minutes(5), 100);
}

WTF::HashSet<SchedulingPolicy::Feature> FrameSchedulerImpl::GetActiveFeaturesTrackedForBackForwardCacheMetrics()
{
    return back_forward_cache_disabling_feature_tracker_.GetActiveFeaturesTrackedForBackForwardCacheMetrics();
}

base::WeakPtr<FrameOrWorkerScheduler> FrameSchedulerImpl::GetFrameOrWorkerSchedulerWeakPtr()
{
...
std::unique_ptr<WebSchedulingTaskQueue> FrameSchedulerImpl::CreateWebSchedulingTaskQueue(WebSchedulingQueueType queue_type, WebSchedulingPriority priority)
{
    bool can_be_deferred_for_rendering = ComputeCanBeDeferredForRendering(
        /*is_deferrable_for_touchstart=*/true, TaskType::kWebSchedulingPostedTask);
    ...
    scoped_refptr<MainThreadTaskQueue> immediate_task_queue = frame_task_queue_controller_->NewWebSchedulingTaskQueue(
        DeferrableTaskQueueTraits().SetCanBeDeferredForRendering(can_be_deferred_for_rendering), queue_type, priority);
    ...
    if (queue_type == WebSchedulingQueueType::kContinuationQueue) {
        return std::make_unique<MainThreadWebSchedulingTaskQueueImpl>(immediate_task_queue->AsWeakPtr(), nullptr);
    }
    scoped_refptr<MainThreadTaskQueue> delayed_task_queue = frame_task_queue_controller_->NewWebSchedulingTaskQueue(
        DeferrableTaskQueueTraits().SetCanBeThrottled(true).SetCanBeIntensivelyThrottled(true).SetCanBeDeferredForRendering(can_be_deferred_for_rendering),
        queue_type, priority);
    return std::make_unique<MainThreadWebSchedulingTaskQueueImpl>(immediate_task_queue->AsWeakPtr(), delayed_task_queue->AsWeakPtr());
}

void FrameSchedulerImpl::OnWebSchedulingTaskQueuePriorityChanged(MainThreadTaskQueue* queue)
{
    UpdateQueuePolicy(queue, frame_task_queue_controller_->GetQueueEnabledVoter(queue));
    main_thread_scheduler_->OnWebSchedulingTaskQueuePriorityChanged(queue);
}

void FrameSchedulerImpl::OnWebSchedulingTaskQueueDestroyed(MainThreadTaskQueue* queue)
{
...
bool FrameSchedulerImpl::ComputeCanBeDeferredForRendering(bool is_deferrable_for_touchstart, TaskType task_type) const
{
    if (!base::FeatureList::IsEnabled(features::kDeferRendererTasksAfterInput)) {
        return false;
    }
    std::optional<features::TaskDeferralPolicy> policy = main_thread_scheduler_->scheduling_settings().discrete_input_task_deferral_policy;
    CHECK(policy);
    switch (*policy) {
    case features::TaskDeferralPolicy::kMinimalTypes:
        return task_type == TaskType::kDOMManipulation || task_type == TaskType::kIdleTask || task_type == TaskType::kWebSchedulingPostedTask;
    ...
    case features::TaskDeferralPolicy::kAllDeferrableTypes:
        ...
        return is_deferrable_for_touchstart || task_type == TaskType::kPostedMessage;
...
// static
MainThreadTaskQueue::QueueTraits FrameSchedulerImpl::ThrottleableTaskQueueTraits()
{
    return QueueTraits()
...
// static
MainThreadTaskQueue::QueueTraits FrameSchedulerImpl::DeferrableTaskQueueTraits()
{
    return QueueTraits().SetCanBeDeferred(true).SetCanBeFrozen(true).SetCanBePaused(true).SetCanRunWhenVirtualTimePaused(false).SetCanBePausedForAndroidWebview(
        true);
}
...
{
    return QueueTraits().SetCanBeFrozen(true).SetCanBePaused(true).SetCanRunWhenVirtualTimePaused(false).SetCanBePausedForAndroidWebview(true);
}

// static
MainThreadTaskQueue::QueueTraits FrameSchedulerImpl::FreezableTaskQueueTraits()
{
...
// static
MainThreadTaskQueue::QueueTraits FrameSchedulerImpl::UnpausableTaskQueueTraits()
{
    return QueueTraits().SetCanRunWhenVirtualTimePaused(false);
}

MainThreadTaskQueue::QueueTraits FrameSchedulerImpl::ForegroundOnlyTaskQueueTraits()
{
    return ThrottleableTaskQueueTraits().SetCanRunInBackground(false).SetCanRunWhenVirtualTimePaused(false);
}

MainThreadTaskQueue::QueueTraits FrameSchedulerImpl::CanRunWhenVirtualTimePausedTaskQueueTraits()
{
    return QueueTraits().SetCanRunWhenVirtualTimePaused(true);
}

void FrameSchedulerImpl::SetPreemptedForCooperativeScheduling(Preempted preempted)
{
...
{
    return QueueTraits().SetCanBePaused(true).SetCanBeFrozen(true).SetCanBeDeferred(true).SetPrioritisationType(QueueTraits::PrioritisationType::kLoading);
}

MainThreadTaskQueue::QueueTraits FrameSchedulerImpl::UnfreezableLoadingTaskQueueTraits()
{
    return LoadingTaskQueueTraits().SetCanBeFrozen(false);
}

MainThreadTaskQueue::QueueTraits FrameSchedulerImpl::LoadingControlTaskQueueTraits()
{
    return QueueTraits().SetCanBePaused(true).SetCanBeFrozen(true).SetCanBeDeferred(true).SetPrioritisationType(
        QueueTraits::PrioritisationType::kLoadingControl);
}

MainThreadTaskQueue::QueueTraits FrameSchedulerImpl::FindInPageTaskQueueTraits()
{
    return PausableTaskQueueTraits().SetPrioritisationType(QueueTraits::PrioritisationType::kFindInPage);
}

MainThreadTaskQueue::QueueTraits FrameSchedulerImpl::InputBlockingQueueTraits()
{
    return QueueTraits().SetPrioritisationType(QueueTraits::PrioritisationType::kInput);
}
} // namespace scheduler
} // namespace blink
