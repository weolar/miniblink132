...
public:
    ...
    void SetUp() override
    {
        ...
        render_widget_signals_ = std::make_unique<RenderWidgetSignals>(mock_observer_.get());
    }

    void IgnoreWidgetCreationCallbacks()
    {
        EXPECT_CALL(*mock_observer_, SetAllRenderWidgetsHidden(false)).Times(AnyNumber());
    }

    void IgnoreWidgetDestructionCallbacks()
    {
        EXPECT_CALL(*mock_observer_, SetAllRenderWidgetsHidden(true)).Times(AnyNumber());
    }
...
TEST_F(RenderWidgetSignalsTest, RenderWidgetSchedulingStateLifeCycle)
{
    ...
    scoped_refptr<WidgetSchedulerImpl> widget1_scheduler = base::MakeRefCounted<WidgetSchedulerImpl>(
...
TEST_F(RenderWidgetSignalsTest, MultipleRenderWidgetsBecomeHiddenThenVisible)
{
    ...
    scoped_refptr<WidgetSchedulerImpl> widget1_scheduler = base::MakeRefCounted<WidgetSchedulerImpl>(
        /*main_thread_scheduler_impl=*/nullptr, render_widget_signals_.get());
    scoped_refptr<WidgetSchedulerImpl> widget2_scheduler = base::MakeRefCounted<WidgetSchedulerImpl>(
        /*main_thread_scheduler_impl=*/nullptr, render_widget_signals_.get());
    scoped_refptr<WidgetSchedulerImpl> widget3_scheduler = base::MakeRefCounted<WidgetSchedulerImpl>(
        /*main_thread_scheduler_impl=*/nullptr, render_widget_signals_.get());
    Mock::VerifyAndClearExpectations(mock_observer_.get());
