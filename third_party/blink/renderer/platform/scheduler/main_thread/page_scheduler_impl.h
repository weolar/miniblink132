...
public:
    ...
    static constexpr base::TimeDelta kDefaultThrottledWakeUpInterval = base::Seconds(1);

    // Interval between throttled wake ups, with intensive throttling.
    static constexpr base::TimeDelta kIntensiveThrottledWakeUpInterval = base::Minutes(1);
    ...
    bool has_ipc_detection_enabled()
    {
        return has_ipc_detection_enabled_;
    }

    std::unique_ptr<FrameScheduler> CreateFrameScheduler(
        FrameScheduler::Delegate* delegate, bool is_in_embedded_frame_tree, FrameScheduler::FrameType) override;
...
private:
    ...
    void SetPageFrozenImpl(
        bool frozen, PolicyUpdater& policy_updater, base::MemoryReductionTaskContext called_from = base::MemoryReductionTaskContext::kDelayExpired);

    // Adds `task_queue` to `wake_up_budget_pool`.
    void AddQueueToWakeUpBudgetPool(MainThreadTaskQueue* task_queue, WakeUpBudgetPool* wake_up_budget_pool, base::LazyNow* lazy_now);
    // Removes `task_queue` from its current `WakeUpBudgetPool`, if any.
    void RemoveQueueFromWakeUpBudgetPool(MainThreadTaskQueue* task_queue, base::LazyNow* lazy_now);
    ...
    WakeUpBudgetPool* GetWakeUpBudgetPool(MainThreadTaskQueue* task_queue, FrameOriginType frame_origin_type, ThrottlingType throttling_type);
    ...
    base::TimeDelta GetIntensiveWakeUpThrottlingInterval(bool is_same_origin) const;
    ...
    void UpdateFrozenState(PolicyUpdater& policy_updater, base::MemoryReductionTaskContext called_from);
