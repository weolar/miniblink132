...
class PLATFORM_EXPORT FrameSchedulerImpl : public FrameScheduler, FrameTaskQueueController::Delegate {
public:
    FrameSchedulerImpl(
    ...
    void SetAgentClusterId(const base::UnguessableToken& agent_cluster_id) override;
    ...
    void DidCommitProvisionalLoad(
        bool is_web_history_inert_commit, NavigationType navigation_type, DidCommitProvisionalLoadParams params = { base::TimeDelta() }) override;
    WebScopedVirtualTimePauser CreateWebScopedVirtualTimePauser(const WTF::String& name, WebScopedVirtualTimePauser::VirtualTaskDuration duration) override;
    ...
    std::unique_ptr<blink::mojom::blink::PauseSubresourceLoadingHandle> GetPauseSubresourceLoadingHandle() override;

    void OnStartedUsingNonStickyFeature(SchedulingPolicy::Feature feature, const SchedulingPolicy& policy, std::unique_ptr<SourceLocation> source_location,
        SchedulingAffectingFeatureHandle* handle) override;
    void OnStartedUsingStickyFeature(
        SchedulingPolicy::Feature feature, const SchedulingPolicy& policy, std::unique_ptr<SourceLocation> source_location) override;
    void OnStoppedUsingNonStickyFeature(SchedulingAffectingFeatureHandle* handle) override;
    ...
    void OnTaskQueueCreated(MainThreadTaskQueue*, base::sequence_manager::TaskQueue::QueueEnabledVoter*) override;

    void SetOnIPCTaskPostedWhileInBackForwardCacheHandler();
    void DetachOnIPCTaskPostedWhileInBackForwardCacheHandler();
    void OnIPCTaskPostedWhileInBackForwardCache(uint32_t ipc_hash, const char* ipc_interface_name);

    // Returns the list of active features which currently tracked by the
    // scheduler for back-forward cache metrics.
    WTF::HashSet<SchedulingPolicy::Feature> GetActiveFeaturesTrackedForBackForwardCacheMetrics() override;

    std::unique_ptr<WebSchedulingTaskQueue> CreateWebSchedulingTaskQueue(WebSchedulingQueueType, WebSchedulingPriority) override;
...
protected:
    FrameSchedulerImpl(MainThreadSchedulerImpl* main_thread_scheduler, PageSchedulerImpl* parent_page_scheduler, FrameScheduler::Delegate* delegate,
        bool is_in_embedded_frame_tree, FrameScheduler::FrameType frame_type);
...
private:
    ...
    class PauseSubresourceLoadingHandleImpl : public blink::mojom::blink::PauseSubresourceLoadingHandle {
    public:
        ...
        explicit PauseSubresourceLoadingHandleImpl(base::WeakPtr<FrameSchedulerImpl> frame_scheduler);
        PauseSubresourceLoadingHandleImpl(const PauseSubresourceLoadingHandleImpl&) = delete;
        PauseSubresourceLoadingHandleImpl& operator=(const PauseSubresourceLoadingHandleImpl&) = delete;
    ...
    void OnPageVisibilityChange(PageVisibilityState page_visibility, PolicyUpdater& policy_updater);
    ...
    SchedulingLifecycleState CalculateLifecycleState(ObserverType type) const override;
    void UpdateQueuePolicy(MainThreadTaskQueue* queue, base::sequence_manager::TaskQueue::QueueEnabledVoter* voter);
    ...
    static MainThreadTaskQueue::QueueTraits CreateQueueTraitsForTaskType(TaskType);
    ...
    base::WeakPtr<FrameOrWorkerScheduler> GetFrameOrWorkerSchedulerWeakPtr() override;
    ...
    bool ComputeCanBeDeferredForRendering(bool is_deferrable_for_touchstart, TaskType) const;
    ...
    static MainThreadTaskQueue::QueueTraits CanRunWhenVirtualTimePausedTaskQueueTraits();
    ...
    const raw_ptr<MainThreadSchedulerImpl, DanglingUntriaged> main_thread_scheduler_; // NOT OWNED
    ...
    TraceableState<bool, TracingCategory::kInfo> preempted_for_cooperative_scheduling_;
    // TODO(https://crbug.com/827113): Trace the count of opt-outs.
    int aggressive_throttling_opt_out_count_;
    TraceableState<bool, TracingCategory::kInfo> opted_out_from_aggressive_throttling_;
    size_t subresource_loading_pause_count_;

    BackForwardCacheDisablingFeatureTracker back_forward_cache_disabling_feature_tracker_;
    ...
    using TaskRunnerMap = WTF::HashMap<TaskType, scoped_refptr<base::SingleThreadTaskRunner>>;
    ...
    base::WeakPtrFactory<FrameSchedulerImpl> invalidating_on_bfcache_restore_weak_factory_ { this };
