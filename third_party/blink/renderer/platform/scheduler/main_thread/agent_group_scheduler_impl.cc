...
// static
std::unique_ptr<WebAgentGroupScheduler> WebAgentGroupScheduler::CreateForTesting()
{
    return std::make_unique<WebAgentGroupScheduler>(CreateDummyAgentGroupScheduler());
}

MainThreadTaskQueue::QueueCreationParams DefaultTaskQueueCreationParams(AgentGroupSchedulerImpl* agent_group_scheduler_impl)
{
    return MainThreadTaskQueue::QueueCreationParams(MainThreadTaskQueue::QueueType::kDefault)
        .SetShouldMonitorQuiescence(true)
        .SetAgentGroupScheduler(agent_group_scheduler_impl);
}

MainThreadTaskQueue::QueueCreationParams CompositorTaskRunnerCreationParams(AgentGroupSchedulerImpl* agent_group_scheduler_impl)
{
    return MainThreadTaskQueue::QueueCreationParams(MainThreadTaskQueue::QueueType::kCompositor)
        .SetShouldMonitorQuiescence(true)
        .SetPrioritisationType(MainThreadTaskQueue::QueueTraits::PrioritisationType::kCompositor)
        .SetAgentGroupScheduler(agent_group_scheduler_impl);
}

AgentGroupSchedulerImpl::AgentGroupSchedulerImpl(MainThreadSchedulerImpl& main_thread_scheduler)
    : default_task_queue_(main_thread_scheduler.NewTaskQueue(DefaultTaskQueueCreationParams(this)))
    , default_task_runner_(default_task_queue_->CreateTaskRunner(TaskType::kMainThreadTaskQueueDefault))
    , compositor_task_queue_(main_thread_scheduler.NewTaskQueue(CompositorTaskRunnerCreationParams(this)))
    , compositor_task_runner_(compositor_task_queue_->CreateTaskRunner(TaskType::kMainThreadTaskQueueCompositor))
...
std::unique_ptr<PageScheduler> AgentGroupSchedulerImpl::CreatePageScheduler(PageScheduler::Delegate* delegate)
{
...
scoped_refptr<base::SingleThreadTaskRunner> AgentGroupSchedulerImpl::DefaultTaskRunner()
{
    return default_task_runner_;
}

scoped_refptr<base::SingleThreadTaskRunner> AgentGroupSchedulerImpl::CompositorTaskRunner()
{
    return compositor_task_runner_;
}

scoped_refptr<MainThreadTaskQueue> AgentGroupSchedulerImpl::CompositorTaskQueue()
{
...
void AgentGroupSchedulerImpl::AddPageSchedulerForTesting(PageSchedulerImpl* page_scheduler)
{
    CHECK(!is_updating_policy_);
...
void AgentGroupSchedulerImpl::RemovePageScheduler(PageSchedulerImpl* page_scheduler)
{
    CHECK(!is_updating_policy_);
...
void AgentGroupSchedulerImpl::IncrementVisibleFramesForAgent(const base::UnguessableToken& agent_cluster_id, PolicyUpdater& policy_updater)
{
    // `agent_cluster_id` can be empty in tests.
    if (agent_cluster_id.is_empty()) {
        return;
    }
    auto [it, was_inserted] = num_visible_frames_per_agent_.emplace(agent_cluster_id, 0);
...
void AgentGroupSchedulerImpl::DecrementVisibleFramesForAgent(const base::UnguessableToken& agent_cluster_id, PolicyUpdater& policy_updater)
{
    // `agent_cluster_id` can be empty in tests.
    if (agent_cluster_id.is_empty()) {
        return;
...
bool AgentGroupSchedulerImpl::IsAgentVisible(const base::UnguessableToken& agent_cluster_id) const
{
    auto it = num_visible_frames_per_agent_.find(agent_cluster_id);
