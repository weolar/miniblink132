...
public:
    explicit AgentGroupSchedulerImpl(MainThreadSchedulerImpl& main_thread_scheduler);
    ...
    void IncrementVisibleFramesForAgent(const base::UnguessableToken& agent_cluster_id, PolicyUpdater& policy_updater);
    void DecrementVisibleFramesForAgent(const base::UnguessableToken& agent_cluster_id, PolicyUpdater& policy_updater);
...
private:
    ...
    const raw_ref<MainThreadSchedulerImpl, DanglingUntriaged> main_thread_scheduler_; // Not owned.
    HeapHashSet<WeakMember<Agent>> agents_;
    HashSet<PageSchedulerImpl*> page_schedulers_;
    std::map<base::UnguessableToken, int> num_visible_frames_per_agent_ ALLOW_DISCOURAGED_TYPE("There is no compelling reason to make base::UnguessableToken "
