...
public:
    ...
    void SetUp() override
    {
        ...
        estimator_ = std::make_unique<IdleTimeEstimator>(task_environment_.GetMockTickClock(), 10, 50);
        compositor_task_queue_1_ = NewTaskQueue();
        compositor_task_queue_2_ = NewTaskQueue();
        compositor_task_runner_1_ = compositor_task_queue_1_->CreateTaskRunner(TaskType::kMainThreadTaskQueueCompositor);
        compositor_task_runner_2_ = compositor_task_queue_2_->CreateTaskRunner(TaskType::kMainThreadTaskQueueCompositor);
    ...
    {
        return base::MakeRefCounted<MainThreadTaskQueue>(*manager_.get(), base::sequence_manager::TaskQueue::Spec(base::sequence_manager::QueueName::TEST_TQ),
            MainThreadTaskQueue::QueueCreationParams(MainThreadTaskQueue::QueueType::kCompositor), nullptr);
    }
    ...
    void SimulateFrameWithTwoCompositorTasks(int compositor_time1, int compositor_time2)
    {
    ...
    void PostTask(scoped_refptr<base::SingleThreadTaskRunner> task_runner, int compositor_time, bool commit)
    {
        task_runner->PostTask(FROM_HERE,
            base::BindOnce(
                [](base::test::TaskEnvironment* task_environment, IdleTimeEstimator* estimator, int compositor_time, bool commit) {
                    base::TimeDelta non_idle_time = base::Milliseconds(compositor_time);
...
TEST_F(IdleTimeEstimatorTest, BasicEstimation_SteadyState)
{
    ...
    EXPECT_EQ(base::Milliseconds(11), estimator_->GetExpectedIdleDuration(frame_length_));
}
...
TEST_F(IdleTimeEstimatorTest, BasicEstimation_Variable)
{
    ...
    EXPECT_EQ(base::Milliseconds(9), estimator_->GetExpectedIdleDuration(frame_length_));
}
...
TEST_F(IdleTimeEstimatorTest, NoIdleTime)
{
    ...
    EXPECT_EQ(base::Milliseconds(0), estimator_->GetExpectedIdleDuration(frame_length_));
}
...
TEST_F(IdleTimeEstimatorTest, Estimation_MultipleTasks_WithSingleObserver)
{
    ...
    EXPECT_EQ(base::Milliseconds(12), estimator_->GetExpectedIdleDuration(frame_length_));
}
