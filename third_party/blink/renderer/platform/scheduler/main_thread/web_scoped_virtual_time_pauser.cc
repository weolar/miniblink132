...
WebScopedVirtualTimePauser::WebScopedVirtualTimePauser(scheduler::ThreadSchedulerBase* scheduler, VirtualTaskDuration duration, const WebString& name)
    : duration_(duration)
...
WebScopedVirtualTimePauser::WebScopedVirtualTimePauser(WebScopedVirtualTimePauser&& other)
{
...
WebScopedVirtualTimePauser& WebScopedVirtualTimePauser::operator=(WebScopedVirtualTimePauser&& other)
{
...
void WebScopedVirtualTimePauser::PauseVirtualTime()
{
    ...
    if (virtual_time_enabled_when_paused_) {
        ...
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("renderer.scheduler", "WebScopedVirtualTimePauser::PauseVirtualTime", trace_id_, "name", debug_name_.Latin1());
    }
...
void WebScopedVirtualTimePauser::DecrementVirtualTimePauseCount()
{
    ...
    if (duration_ == VirtualTaskDuration::kNonInstant) {
        scheduler_->MaybeAdvanceVirtualTime(virtual_time_when_paused_ + base::Milliseconds(10));
    }
    if (virtual_time_enabled_when_paused_) {
        TRACE_EVENT_NESTABLE_ASYNC_END0("renderer.scheduler", "WebScopedVirtualTimePauser::PauseVirtualTime", trace_id_);
    }
}
