...
IdleTimeEstimator::IdleTimeEstimator(const base::TickClock* time_source, int sample_count, double estimation_percentile)
...
base::TimeDelta IdleTimeEstimator::GetExpectedIdleDuration(base::TimeDelta compositor_frame_interval) const
{
...
void IdleTimeEstimator::WillProcessTask(const base::PendingTask& pending_task, bool was_blocked_or_low_priority)
{
    nesting_level_++;
    if (nesting_level_ == 1)
...
void IdleTimeEstimator::DidProcessTask(const base::PendingTask& pending_task)
{
    ...
    if (did_commit_) {
        per_frame_compositor_task_runtime_.InsertSample(cumulative_compositor_runtime_);
        cumulative_compositor_runtime_ = base::TimeDelta();
        did_commit_ = false;
    }
}

void IdleTimeEstimator::AddCompositorTaskQueue(scoped_refptr<MainThreadTaskQueue> compositor_task_queue)
{
    compositor_task_queue->AddTaskObserver(this);
}

void IdleTimeEstimator::RemoveCompositorTaskQueue(scoped_refptr<MainThreadTaskQueue> compositor_task_queue)
{
