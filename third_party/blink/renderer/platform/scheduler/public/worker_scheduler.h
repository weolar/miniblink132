...
public:
    ...
    static std::unique_ptr<WorkerScheduler> CreateWorkerScheduler(WorkerThreadScheduler* worker_thread_scheduler, WorkerSchedulerProxy* proxy);
    ...
    virtual scoped_refptr<base::SingleThreadTaskRunner> GetTaskRunner(TaskType) const = 0;

    virtual WorkerThreadScheduler* GetWorkerThreadScheduler() const = 0;

    virtual void OnLifecycleStateChanged(SchedulingLifecycleState lifecycle_state) = 0;
