...
public:
    ...
    using OnLifecycleStateChangedCallback = base::RepeatingCallback<void(scheduler::SchedulingLifecycleState)>;
    ...
    public:
        ...
        SchedulingAffectingFeatureHandle(
            SchedulingPolicy::Feature feature, SchedulingPolicy policy, std::unique_ptr<SourceLocation> source_location, base::WeakPtr<FrameOrWorkerScheduler>);
        SchedulingAffectingFeatureHandle(SchedulingAffectingFeatureHandle&&);
        SchedulingAffectingFeatureHandle& operator=(SchedulingAffectingFeatureHandle&&);
        ...
        const FeatureAndJSLocationBlockingBFCache& GetFeatureAndJSLocationBlockingBFCache() const;
    ...
    struct BFCacheBlockingFeatureAndLocations {
        void MaybeAdd(FeatureAndJSLocationBlockingBFCache details)
        {
            ...
            // mojo message.
            if (details_list.Find(details) == kNotFound && details_list.size() < kMaxNumberOfBackForwardCacheBlockingDetails) {
    ...
    public:
        using BFCacheBlockingFeatureAndLocations = FrameOrWorkerScheduler::BFCacheBlockingFeatureAndLocations;

        struct BlockingDetails {
            const raw_ref<const BFCacheBlockingFeatureAndLocations> non_sticky_features_and_js_locations;
            const raw_ref<const BFCacheBlockingFeatureAndLocations> sticky_features_and_js_locations;
            BlockingDetails(BFCacheBlockingFeatureAndLocations& non_sticky, BFCacheBlockingFeatureAndLocations& sticky)
    ...
    [[nodiscard]] SchedulingAffectingFeatureHandle RegisterFeature(SchedulingPolicy::Feature feature, SchedulingPolicy policy);
    ...
    // document.
    void RegisterStickyFeature(SchedulingPolicy::Feature feature, SchedulingPolicy policy);
    ...
    // (crbug.com/1286570).
    [[nodiscard]] std::unique_ptr<LifecycleObserverHandle> AddLifecycleObserver(ObserverType, OnLifecycleStateChangedCallback);
    ...
    virtual std::unique_ptr<WebSchedulingTaskQueue> CreateWebSchedulingTaskQueue(WebSchedulingQueueType, WebSchedulingPriority) = 0;

    virtual FrameScheduler* ToFrameScheduler()
    {
        return nullptr;
    }
    ...
    virtual WebScopedVirtualTimePauser CreateWebScopedVirtualTimePauser(const String& name, WebScopedVirtualTimePauser::VirtualTaskDuration) = 0;
...
protected:
    ...
    virtual scheduler::SchedulingLifecycleState CalculateLifecycleState(ObserverType) const
    {
        return scheduler::SchedulingLifecycleState::kNotThrottled;
    }

    // |source_location| is nullptr when JS is not running.
    virtual void OnStartedUsingNonStickyFeature(SchedulingPolicy::Feature feature, const SchedulingPolicy& policy,
        std::unique_ptr<SourceLocation> source_location, SchedulingAffectingFeatureHandle* handle)
        = 0;
    // |source_location| is nullptr when JS is not running.
    virtual void OnStartedUsingStickyFeature(SchedulingPolicy::Feature feature, const SchedulingPolicy& policy, std::unique_ptr<SourceLocation> source_location)
        = 0;
    virtual void OnStoppedUsingNonStickyFeature(SchedulingAffectingFeatureHandle* handle) = 0;

    // Gets a weak pointer for this scheduler that is reset when the influence by
    // registered features to this scheduler is reset.
    virtual base::WeakPtr<FrameOrWorkerScheduler> GetFrameOrWorkerSchedulerWeakPtr() = 0;
...
private:
    ...
    public:
        ...
        {
            return observer_type_;
        }
        OnLifecycleStateChangedCallback& GetCallback()
        {
            return callback_;
        }
    ...
    HashMap<LifecycleObserverHandle*, std::unique_ptr<ObserverState>> lifecycle_observers_;
