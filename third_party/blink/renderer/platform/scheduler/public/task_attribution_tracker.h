...
public:
    ...
    private:
        ...
        TaskScope(TaskAttributionTracker* tracker, ScriptState* script_state, ScriptWrappableTaskState* previous_task_state)
    ...
    private:
        ...
        ObserverScope(TaskAttributionTracker* tracker, Observer* observer, Observer* previous_observer)
            : task_tracker_(tracker)
            , previous_observer_(previous_observer)
        {
        }

        Observer* PreviousObserver() const
        {
            return previous_observer_;
        }
    ...
    // continuations.
    virtual TaskScope CreateTaskScope(ScriptState*, TaskAttributionInfo* task_state, TaskScopeType type) = 0;
    ...
    virtual TaskScope CreateTaskScope(
        ScriptState*, TaskAttributionInfo* task_state, TaskScopeType type, AbortSignal* abort_source, DOMTaskSignal* priority_source)
        = 0;
    ...
    virtual std::optional<TaskScope> MaybeCreateTaskScopeForCallback(ScriptState*, TaskAttributionInfo* task_state) = 0;
    ...
    virtual TaskAttributionInfo* CommitSameDocumentNavigation(TaskAttributionId) = 0;
