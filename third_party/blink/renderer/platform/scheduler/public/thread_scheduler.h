...
public:
    ...
    // Takes ownership of |IdleTask|. Can be called from any thread.
    virtual void PostDelayedIdleTask(const base::Location&, base::TimeDelta delay, Thread::IdleTask) = 0;
    ...
    virtual void PostNonNestableIdleTask(const base::Location&, Thread::IdleTask) = 0;
    ...
    // thread.
    virtual scoped_refptr<base::SingleThreadTaskRunner> V8UserVisibleTaskRunner()
    {
        return nullptr;
    }
    ...
    virtual MainThreadScheduler* ToMainThreadScheduler()
    {
        return nullptr;
    }
};
