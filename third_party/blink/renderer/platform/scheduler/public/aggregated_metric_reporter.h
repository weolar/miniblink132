...
template <class TaskClass, class ValueType> class AggregatedMetricReporter {
    DISALLOW_NEW();
...
public:
    ...
    AggregatedMetricReporter(const char* metric_name, AggregatorFuncPtr aggregator)
        : AggregatedMetricReporter(base::Histogram::FactoryGet(metric_name, 1, static_cast<int>(TaskClass::kCount), static_cast<int>(TaskClass::kCount) + 1,
    ...
    void RecordTask(TaskClass task_class, ValueType value)
    {
        ...
        DCHECK_LT(static_cast<int>(task_class), static_cast<int>(TaskClass::kCount));

        ValueType& unreported_value = unreported_values_[static_cast<int>(task_class)];
        unreported_value += value;

        int value_to_report = aggregator_(unreported_value);
        if (value_to_report > 0) {
            value_per_type_histogram_->AddCount(static_cast<int>(task_class), value_to_report);
        }
    }

protected:
    AggregatedMetricReporter(base::HistogramBase* histogram, AggregatorFuncPtr aggregator)
        : value_per_type_histogram_(histogram)
        , aggregator_(aggregator)
    {
    }

    std::array<ValueType, static_cast<size_t>(TaskClass::kCount)> unreported_values_ = {};
