...
private:
    ...
    friend MainThreadTaskRunnerRestricted AccessMainThreadForGpuMemoryBufferManager();

    MainThreadTaskRunnerRestricted() = default;
};

class MainThreadTaskRunnerRestrictedForTesting : public MainThreadTaskRunnerRestricted { };
...
public:
    ...
    // instead.
    virtual scoped_refptr<base::SingleThreadTaskRunner> GetTaskRunner(MainThreadTaskRunnerRestricted) const = 0;
...
private:
    ...
    virtual bool IsSimpleMainThread() const
    {
        return false;
    }
};
