...
public:
    ...
    // May only be called after virtual time was enabled with EnableVirtualTime().
    virtual void GrantVirtualTimeBudget(base::TimeDelta budget, base::OnceClosure budget_exhausted_callback) = 0;
    ...
    // May only be called after virtual time was enabled with EnableVirtualTime().
    virtual void SetMaxVirtualTimeTaskStarvationCount(int max_task_starvation_count) = 0;
    ...
    // unpaused.
    virtual WebScopedVirtualTimePauser CreateWebScopedVirtualTimePauser(const WTF::String& name, WebScopedVirtualTimePauser::VirtualTaskDuration) = 0;
};
