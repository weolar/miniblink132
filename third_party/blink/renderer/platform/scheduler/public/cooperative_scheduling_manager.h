...
public:
    ...
    CooperativeSchedulingManager& operator=(const CooperativeSchedulingManager&) = delete;
    virtual ~CooperativeSchedulingManager() = default;

    // Returns true if reentry is allowed in the current C++ stack.
    bool InAllowedStackScope() const
    {
        return allowed_stack_scope_depth_ > 0;
    }
    ...
    void set_feature_enabled(bool enabled)
    {
