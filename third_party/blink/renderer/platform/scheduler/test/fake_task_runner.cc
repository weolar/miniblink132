...
class FakeTaskRunner::Data : public WTF::ThreadSafeRefCounted<Data>, public base::TickClock {
public:
    Data() = default;
    ...
    Deque<PendingTask>::iterator FindRunnableTask()
    {
        ...
        return base::ranges::find_if(task_queue_, [&](const PendingTask& item) { return item.second <= time_; });
    }
...
{
    data_->time_ += delta;
    for (auto it = data_->FindRunnableTask(); it != data_->task_queue_.end(); it = data_->FindRunnableTask()) {
...
Deque<std::pair<base::OnceClosure, base::TimeTicks>> FakeTaskRunner::TakePendingTasksForTesting()
{
    return std::move(data_->task_queue_);
}

bool FakeTaskRunner::PostDelayedTask(const base::Location& location, base::OnceClosure task, base::TimeDelta delay)
{
    data_->PostDelayedTask(std::move(task), delay);
    return true;
}

bool FakeTaskRunner::PostDelayedTaskAt(base::subtle::PostDelayedTaskPassKey, const base::Location& from_here, base::OnceClosure task,
    base::TimeTicks delayed_run_time, base::subtle::DelayPolicy deadline_policy)
{
    return PostDelayedTask(from_here, std::move(task), delayed_run_time.is_null() ? base::TimeDelta() : delayed_run_time - data_->NowTicks());
}

bool FakeTaskRunner::PostNonNestableDelayedTask(const base::Location& location, base::OnceClosure task, base::TimeDelta delay)
{
    data_->PostDelayedTask(std::move(task), delay);
    return true;
}
