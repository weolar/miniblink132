...
void ThreadPoolManager::CreateThread(
    const google::protobuf::RepeatedPtrField<SequenceManagerTestDescription::Action>& initial_thread_actions, base::TimeTicks time)
{
    SimpleThread* thread;
    {
        AutoLock lock(lock_);
        threads_.push_back(std::make_unique<SimpleThreadImpl>(this, time, BindOnce(&ThreadPoolManager::StartThread, Unretained(this), initial_thread_actions)));
...
void ThreadPoolManager::StartThread(
    const google::protobuf::RepeatedPtrField<SequenceManagerTestDescription::Action>& initial_thread_actions, ThreadManager* thread_manager)
{
    {
        AutoLock lock(lock_);
...
void ThreadPoolManager::AdvanceClockSynchronouslyByPendingTaskDelay(ThreadManager* thread_manager)
{
...

    {
        ...
        next_time_ = std::min(next_time_, thread_manager->NowTicks() + thread_manager->NextPendingTaskDelay());
...
void ThreadPoolManager::AdvanceClockSynchronouslyToTime(ThreadManager* thread_manager, base::TimeTicks time)
{
    ThreadReadyToComputeTime();
    {
        AutoLock lock(lock_);
