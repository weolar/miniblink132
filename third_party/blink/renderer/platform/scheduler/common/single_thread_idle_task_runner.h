...
// TODO(yutak): Fix this.
class SingleThreadIdleTaskRunner : public base::RefCountedThreadSafe<SingleThreadIdleTaskRunner> {
public:
    ...
    SingleThreadIdleTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> idle_priority_task_runner,
        scoped_refptr<base::SingleThreadTaskRunner> control_task_runner, Delegate* delegate);
    SingleThreadIdleTaskRunner(const SingleThreadIdleTaskRunner&) = delete;
    SingleThreadIdleTaskRunner& operator=(const SingleThreadIdleTaskRunner&) = delete;

    virtual void PostIdleTask(const base::Location& from_here, IdleTask idle_task);
    ...
    virtual void PostDelayedIdleTask(const base::Location& from_here, const base::TimeDelta delay, IdleTask idle_task);

    virtual void PostNonNestableIdleTask(const base::Location& from_here, IdleTask idle_task);
...
private:
    ...
    void PostDelayedIdleTaskOnAssociatedThread(const base::Location& from_here, const base::TimeTicks delayed_run_time, IdleTask idle_task);
    ...
    std::multimap<base::TimeTicks, DelayedIdleTask> delayed_idle_tasks_ ALLOW_DISCOURAGED_TYPE("TODO(crbug.com/1404327)");
