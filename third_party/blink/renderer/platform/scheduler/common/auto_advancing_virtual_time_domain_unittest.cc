...
public:
    ...
    void SetUp() override
    {
        sequence_manager_ = base::sequence_manager::CreateSequenceManagerOnCurrentThreadWithPump(base::MessagePump::Create(base::MessagePumpType::DEFAULT),
        ...
        auto_advancing_time_domain_ = std::make_unique<AutoAdvancingVirtualTimeDomain>(initial_time_, initial_time_ticks_, scheduler_helper_.get());
...
namespace {
void RepostingTask(scoped_refptr<NonMainThreadTaskQueue> task_queue, int max_count, int* count)
{
    if (++(*count) >= max_count)
        return;

    task_queue->GetTaskRunnerWithDefaultTaskType()->PostTask(FROM_HERE, base::BindOnce(&RepostingTask, task_queue, max_count, count));
}
...
TEST_F(AutoAdvancingVirtualTimeDomainTest, MaxVirtualTimeTaskStarvationCountOneHundred)
{
    ...
    task_queue_->GetTaskRunnerWithDefaultTaskType()->PostDelayedTask(
        FROM_HERE, base::BindOnce(DelayedTask, &count, &delayed_task_run_at_count), base::Milliseconds(10));
...
TEST_F(AutoAdvancingVirtualTimeDomainTest, MaxVirtualTimeTaskStarvationCountZero)
{
    auto_advancing_time_domain_->SetCanAdvanceVirtualTime(true);
...
TEST_F(AutoAdvancingVirtualTimeDomainTest, BaseTimeOverriden)
{
    ...
    bool task_run = false;
    task_queue_->GetTaskRunnerWithDefaultTaskType()->PostDelayedTask(FROM_HERE, base::BindOnce(NopTask, &task_run), delay);
    base::RunLoop().RunUntilIdle();
...
TEST_F(AutoAdvancingVirtualTimeDomainTest, GetNextWakeUpHandlesPastRunTime)
{
    ...
    bool task_run = false;
    task_queue_->GetTaskRunnerWithDefaultTaskType()->PostDelayedTask(FROM_HERE, base::BindOnce(NopTask, &task_run), base::Milliseconds(10));

    // Advance virtual time past task time to t+100ms.
    auto_advancing_time_domain_->MaybeAdvanceVirtualTime(initial_time_ticks_ + base::Milliseconds(100));

    // Task at t+10ms should be run immediately.
    EXPECT_GE(base::TimeTicks::Now(), sequence_manager_->GetNextDelayedWakeUp()->time);
