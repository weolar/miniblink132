...
void DeleteOrReleaseSoonImpl(const base::Location& from_here, void (*deleter)(const void*), const void* object,
    scoped_refptr<base::SingleThreadTaskRunner> preferred_task_runner, scoped_refptr<base::SingleThreadTaskRunner> fallback_task_runner);

class DeleteHelper {
public:
    DeleteHelper(const base::Location& from_here, void (*deleter)(const void*), const void* object,
        scoped_refptr<base::SingleThreadTaskRunner> preferred_task_runner, scoped_refptr<base::SingleThreadTaskRunner> fallback_task_runner)
    ...
    ~DeleteHelper()
    {
        ...
        if (fallback_task_runner_) {
            DeleteOrReleaseSoonImpl(from_here_, deleter_, object_, fallback_task_runner_, nullptr);
...
void DeleteOrReleaseSoonImpl(const base::Location& from_here, void (*deleter)(const void*), const void* object,
    scoped_refptr<base::SingleThreadTaskRunner> preferred_task_runner, scoped_refptr<base::SingleThreadTaskRunner> fallback_task_runner)
{
    auto delete_helper = std::make_unique<DeleteHelper>(from_here, deleter, object, preferred_task_runner, fallback_task_runner);
    preferred_task_runner->PostNonNestableTask(from_here, base::BindOnce(&DeleteHelper::Delete, std::move(delete_helper)));
}
...
BlinkSchedulerSingleThreadTaskRunner::~BlinkSchedulerSingleThreadTaskRunner() = default;

bool BlinkSchedulerSingleThreadTaskRunner::DeleteOrReleaseSoonInternal(const base::Location& from_here, void (*deleter)(const void*), const void* object)
{
    DCHECK(deleter);
    ...
    DeleteOrReleaseSoonImpl(from_here, deleter, object, outer_, thread_task_runner_);
    return true;
}
