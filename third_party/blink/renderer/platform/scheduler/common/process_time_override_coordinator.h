...
private:
    ...
    void RegisterOverride(ScopedOverride* handle, base::Time requested_time, base::TimeTicks requested_ticks);
    void UnregisterOverride(ScopedOverride* handle);
    // See `CreateOverride` above for the semantics of parameters.
    void EnableOverride(base::Time initial_time, base::TimeTicks initial_ticks) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    void DisableOverride() EXCLUSIVE_LOCKS_REQUIRED(lock_);

    base::TimeTicks TryAdvancingTime(ScopedOverride* handle, base::TimeTicks requested_ticks);
    ...
    base::Lock lock_;
    std::unique_ptr<base::subtle::ScopedTimeClockOverrides> clock_override_ GUARDED_BY(lock_);
    ...
    base::flat_map<ScopedOverride*, base::TimeTicks> requested_ticks_by_client_ GUARDED_BY(lock_);
};
