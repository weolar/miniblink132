...
class PLATFORM_EXPORT SchedulerHelper : public base::sequence_manager::SequenceManager::Observer {
public:
    // |sequence_manager| must remain valid until Shutdown() is called or the
    // object is destroyed.
    explicit SchedulerHelper(base::sequence_manager::SequenceManager* sequence_manager);
    ...
    void InitDefaultTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    virtual const scoped_refptr<base::SingleThreadTaskRunner>& ControlTaskRunner() = 0;
    ...
    void AddTaskTimeObserver(base::sequence_manager::TaskTimeObserver* task_time_observer);
    void RemoveTaskTimeObserver(base::sequence_manager::TaskTimeObserver* task_time_observer);
...
protected:
    ...
    raw_ptr<base::sequence_manager::SequenceManager> sequence_manager_; // NOT OWNED
