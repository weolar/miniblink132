...
// ends to avoid jank.
class PLATFORM_EXPORT IdleHelper : public base::TaskObserver, public SingleThreadIdleTaskRunner::Delegate {
public:
    ...
    public:
        ...
        // again.
        virtual bool CanEnterLongIdlePeriod(base::TimeTicks now, base::TimeDelta* next_long_idle_period_delay_out) = 0;
    ...
    // and must outlive it.
    IdleHelper(SchedulerHelper* helper, Delegate* delegate, const char* idle_period_tracing_name,
        base::TimeDelta required_quiescence_duration_before_long_idle_period, base::sequence_manager::TaskQueue* idle_queue);
    ...
    void StartIdlePeriod(IdlePeriodState new_idle_period_state, base::TimeTicks now, base::TimeTicks idle_period_deadline);
...
private:
    ...
    class State {
    public:
        State(SchedulerHelper* helper, Delegate* delegate, const char* idle_period_tracing_name);
        State(const State&) = delete;
        State& operator=(const State&) = delete;
        virtual ~State();

        void UpdateState(IdlePeriodState new_state, base::TimeTicks new_deadline, base::TimeTicks optional_now);
    ...
    private:
        void TraceEventIdlePeriodStateChange(IdlePeriodState new_state, bool new_running_idle_task, base::TimeTicks new_deadline, base::TimeTicks optional_now);
    ...
    IdlePeriodState ComputeNewLongIdlePeriodState(const base::TimeTicks now, base::TimeDelta* next_long_idle_period_delay_out);

    bool ShouldWaitForQuiescence();
    void OnIdleTaskPostedOnMainThread();
    void UpdateLongIdlePeriodStateAfterIdleTask();

    void SetIdlePeriodState(IdlePeriodState new_state, base::TimeTicks new_deadline, base::TimeTicks optional_now);
    ...
    raw_ptr<SchedulerHelper> helper_; // NOT OWNED
    raw_ptr<Delegate> delegate_; // NOT OWNED
    raw_ptr<base::sequence_manager::TaskQueue, DanglingUntriaged> idle_queue_; // NOT OWNED
