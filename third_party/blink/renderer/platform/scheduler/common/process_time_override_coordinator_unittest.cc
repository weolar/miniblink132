...
TEST(ProcessTimeOverrideCoordinatorTest, SingleClient)
{
    ...
    auto expect_never_called = base::BindRepeating([]() { FAIL() << "Schedule tasks callback should not be called for single client"; });

    auto time_override = ProcessTimeOverrideCoordinator::CreateOverride(kStartTime, start_ticks, std::move(expect_never_called));
    ...
    EXPECT_THAT(time_override->TryAdvancingTime(start_ticks + base::Seconds(1)), Eq(start_ticks + base::Seconds(1)));
    ...
    EXPECT_THAT(time_override->TryAdvancingTime(start_ticks + base::Seconds(2)), Eq(start_ticks + base::Seconds(2)));
...
TEST(ProcessTimeOverrideCoordinatorTest, MultipleClients)
{
    ...
    auto client1 = ProcessTimeOverrideCoordinator::CreateOverride(
        kStartTime, start_ticks, base::BindLambdaForTesting([&client1_callback_count] { client1_callback_count++; }));
    ...
    auto client2 = ProcessTimeOverrideCoordinator::CreateOverride(
        kStartTime + base::Seconds(1), start_ticks + base::Seconds(1), base::BindLambdaForTesting([&client2_callback_count] { client2_callback_count++; }));
    EXPECT_THAT(client1->NowTicks(), Eq(start_ticks));
    EXPECT_THAT(client2->NowTicks(), Eq(start_ticks));
    EXPECT_THAT(base::Time::Now(), Eq(kStartTime));

    // Nothing happens when first client tries to advance time.
    EXPECT_THAT(client1->TryAdvancingTime(start_ticks + base::Seconds(1)), Eq(start_ticks));
    EXPECT_THAT(base::TimeTicks::Now(), Eq(start_ticks));
    EXPECT_THAT(base::Time::Now(), Eq(kStartTime));
    ...
    EXPECT_THAT(client2->TryAdvancingTime(start_ticks + base::Milliseconds(100)), Eq(start_ticks + base::Milliseconds(100)));
    EXPECT_THAT(base::TimeTicks::Now(), Eq(start_ticks + base::Milliseconds(100)));
    ...
    EXPECT_THAT(client2->TryAdvancingTime(start_ticks + base::Milliseconds(2000)), Eq(start_ticks + base::Milliseconds(1000)));
    EXPECT_THAT(base::TimeTicks::Now(), Eq(start_ticks + base::Milliseconds(1000)));
    ...
    EXPECT_THAT(client2->TryAdvancingTime(start_ticks + base::Milliseconds(1500)), Eq(start_ticks + base::Milliseconds(1000)));
    EXPECT_THAT(client1_callback_count, Eq(2));
    EXPECT_THAT(client2_callback_count, Eq(0));

    // ... and now the second catches up.
    EXPECT_THAT(client1->TryAdvancingTime(start_ticks + base::Milliseconds(1500)), Eq(start_ticks + base::Milliseconds(1500)));
    EXPECT_THAT(base::TimeTicks::Now(), Eq(start_ticks + base::Milliseconds(1500)));
