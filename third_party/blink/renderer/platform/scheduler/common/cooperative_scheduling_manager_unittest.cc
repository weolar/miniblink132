...
{
    std::unique_ptr<CooperativeSchedulingManager> manager = std::make_unique<CooperativeSchedulingManager>();
    {
        ...
        {
            CooperativeSchedulingManager::AllowedStackScope nested_scope(manager.get());
            EXPECT_TRUE(manager->InAllowedStackScope());
        }
        EXPECT_TRUE(manager->InAllowedStackScope());
    }
...
public:
    MockCooperativeSchedulingManager()
        : CooperativeSchedulingManager()
    {
        set_feature_enabled(true);
        ON_CALL(*this, RunNestedLoop()).WillByDefault(testing::Invoke(this, &MockCooperativeSchedulingManager::RealRunNestedLoop));
    }
    ~MockCooperativeSchedulingManager() override = default;
    MOCK_METHOD0(RunNestedLoop, void());
    void RealRunNestedLoop()
    {
...
{
    {
        std::unique_ptr<MockCooperativeSchedulingManager> manager = std::make_unique<MockCooperativeSchedulingManager>();
