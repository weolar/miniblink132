...
EventLoop::EventLoop(EventLoop::Delegate* delegate, v8::Isolate* isolate, std::unique_ptr<v8::MicrotaskQueue> microtask_queue)
    : delegate_(delegate)
    , isolate_(isolate)
    , microtask_queue_(std::move(microtask_queue))
{
    ...
    microtask_queue_->AddMicrotasksCompletedCallback(&EventLoop::RunEndOfCheckpointTasks, this);
}
...
void EventLoop::Disable()
{
    ...
    for (auto* scheduler : schedulers_) {
        scheduler->SetPreemptedForCooperativeScheduling(FrameOrWorkerScheduler::Preempted(true));
    }
...
void EventLoop::Enable()
{
    ...
    for (auto* scheduler : schedulers_) {
        scheduler->SetPreemptedForCooperativeScheduling(FrameOrWorkerScheduler::Preempted(false));
    }
