...
public:
    ...
    BackForwardCacheDisablingFeatureTracker(TraceableVariableController* tracing_controller, ThreadSchedulerBase* scheduler);
    ...
    void AddNonStickyFeature(SchedulingPolicy::Feature feature, std::unique_ptr<SourceLocation> source_location = nullptr,
        FrameOrWorkerScheduler::SchedulingAffectingFeatureHandle* handle = nullptr);

    // Called when a usage of |feature| is added.
    // |feature| should be a sticky feature.
    void AddStickyFeature(SchedulingPolicy::Feature feature, std::unique_ptr<SourceLocation> source_location = nullptr);
    ...
    WTF::HashSet<SchedulingPolicy::Feature> GetActiveFeaturesTrackedForBackForwardCacheMetrics();

    // Gets a list of non sticky features and their JS locations.
    BFCacheBlockingFeatureAndLocations& GetActiveNonStickyFeaturesTrackedForBackForwardCache();

    // Gets a list of sticky features and their JS locations.
    const BFCacheBlockingFeatureAndLocations& GetActiveStickyFeaturesTrackedForBackForwardCache() const;
...
private:
    ...
    void NotifyDelegateAboutFeaturesAfterCurrentTask(TracingType tracing_type, SchedulingPolicy::Feature traced_feature);

    // Called when a usage of |feature| is added.
    void AddFeatureInternal(SchedulingPolicy::Feature feature);

    base::flat_map<SchedulingPolicy::Feature, int> back_forward_cache_disabling_feature_counts_ {};
    // TODO(crbug.com/1366675): Remove back_forward_cache_disabling_features_.
    std::bitset<static_cast<size_t>(SchedulingPolicy::Feature::kMaxValue) + 1> back_forward_cache_disabling_features_ {};
    TraceableState<bool, TracingCategory::kInfo> opted_out_from_back_forward_cache_;
    ...
    base::WeakPtrFactory<BackForwardCacheDisablingFeatureTracker> weak_factory_ { this };
};
