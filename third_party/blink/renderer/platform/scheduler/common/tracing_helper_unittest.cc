...
class TraceableStateForTest : public TraceableState<int, TracingCategory::kDefault> {
public:
...
{
    TraceableVariableController controller;
    TraceableState<int, TracingCategory::kDebug> x(-1, "X", &controller, SignOfInt);
    TraceableState<int, TracingCategory::kDebug> y(1, "Y", &controller, SignOfInt);
    EXPECT_EQ(0, x + y);
    EXPECT_FALSE(x == y);
    EXPECT_TRUE(x != y);
    x = 1;
