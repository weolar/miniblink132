...
public:
    ...
    void DidHandleInputEventOnCompositorThread(const WebInputEvent& web_input_event, InputEventState event_state) override
    {
    }
    void WillPostInputEventToMainThread(WebInputEvent::Type web_input_event_type, const WebInputEventAttribution& web_input_event_attribution) override
    {
    }
    void WillHandleInputEventOnMainThread(WebInputEvent::Type web_input_event_type, const WebInputEventAttribution& web_input_event_attribution) override
    {
    }
    void DidHandleInputEventOnMainThread(const WebInputEvent& web_input_event, WebInputEventResult result, bool frame_requested) override
    {
    }
...
public:
    ...
    {
        return FrameType::kMainFrame;
    }
    WebScopedVirtualTimePauser CreateWebScopedVirtualTimePauser(const String& name, WebScopedVirtualTimePauser::VirtualTaskDuration) override
    {
    ...
    void DidCommitProvisionalLoad(bool, FrameScheduler::NavigationType, DidCommitProvisionalLoadParams) override
    {
    }
...
public:
    ...
    std::unique_ptr<FrameScheduler> CreateFrameScheduler(FrameScheduler::Delegate* delegate, bool is_in_embedded_frame_tree, FrameScheduler::FrameType) override
    {
...
public:
    ...
    scoped_refptr<base::SingleThreadTaskRunner> GetTaskRunner(MainThreadTaskRunnerRestricted) const override
    {
    ...
    void SetMainThreadTaskRunnerForTesting(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    {
        main_thread_task_runner_for_testing_ = std::move(task_runner);
    }

private:
    bool IsSimpleMainThread() const override
    {
        return true;
    }

    raw_ptr<ThreadScheduler> scheduler_ptr_;
    scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner_for_testing_;
};

class SimpleMainThreadWithScheduler : public SimpleMainThread {
public:
    SimpleMainThreadWithScheduler()
        : SimpleMainThread(nullptr)
    {
    }
...
class DummyWebMainThreadScheduler : public WebThreadScheduler, public MainThreadScheduler {
public:
...
public:
    ...
    std::unique_ptr<PageScheduler> CreatePageScheduler(PageScheduler::Delegate*) override
    {
    ...
    v8::Isolate* Isolate() override
    {
        return main_thread_scheduler_->Isolate();
    }
...
std::unique_ptr<FrameScheduler> CreateDummyFrameScheduler(v8::Isolate* isolate)
{
    DCHECK(isolate);
...
{
    static_cast<SimpleMainThread*>(Thread::MainThread())->SetMainThreadTaskRunnerForTesting(base::SingleThreadTaskRunner::GetCurrentDefault());
}

void UnsetMainThreadTaskRunnerForTesting()
{
    static_cast<SimpleMainThread*>(Thread::MainThread())->SetMainThreadTaskRunnerForTesting(nullptr);
}
