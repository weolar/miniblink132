...
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(ThreadSpecific<CooperativeSchedulingManager>, manager, ());
    return &(*manager);
}

CooperativeSchedulingManager::AllowedStackScope::AllowedStackScope(CooperativeSchedulingManager* manager)
...
{
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("renderer.scheduler", "PreemptionAllowedStackScope", TRACE_ID_LOCAL(this));
...
{
    TRACE_EVENT_NESTABLE_ASYNC_END0("renderer.scheduler", "PreemptionAllowedStackScope", TRACE_ID_LOCAL(this));
    allowed_stack_scope_depth_--;
...
{
    TRACE_EVENT0("renderer.scheduler", "CooperativeSchedulingManager::RunNestedLoop");
...
void CooperativeSchedulingManager::SetTickClockForTesting(const base::TickClock* clock)
{
    clock_ = clock;
}
