...
TEST(WebTaskRunnerTest, PostCancellableTaskTest)
{
    ...
    int count = 0;
    TaskHandle handle = PostCancellableTask(*task_runner, FROM_HERE, WTF::BindOnce(&Increment, WTF::Unretained(&count)));
    EXPECT_EQ(0, count);
    ...
    count = 0;
    handle = PostDelayedCancellableTask(*task_runner, FROM_HERE, WTF::BindOnce(&Increment, WTF::Unretained(&count)), base::Milliseconds(1));
    EXPECT_EQ(0, count);
    EXPECT_TRUE(handle.IsActive());
    task_runner->RunUntilIdle();
    EXPECT_EQ(1, count);
    ...
    count = 0;
    handle = PostNonNestableCancellableTask(*task_runner, FROM_HERE, WTF::BindOnce(&Increment, WTF::Unretained(&count)));
    EXPECT_EQ(0, count);
    EXPECT_TRUE(handle.IsActive());
    task_runner->RunUntilIdle();
    EXPECT_EQ(1, count);
    ...
    count = 0;
    handle = PostNonNestableDelayedCancellableTask(*task_runner, FROM_HERE, WTF::BindOnce(&Increment, WTF::Unretained(&count)), base::Milliseconds(1));
    EXPECT_EQ(0, count);
    EXPECT_TRUE(handle.IsActive());
    task_runner->RunUntilIdle();
    EXPECT_EQ(1, count);
    ...
    // Cancel a task.
    count = 0;
    handle = PostCancellableTask(*task_runner, FROM_HERE, WTF::BindOnce(&Increment, WTF::Unretained(&count)));
    handle.Cancel();
    EXPECT_EQ(0, count);
    EXPECT_FALSE(handle.IsActive());
    task_runner->RunUntilIdle();
    ...
    {
        count = 0;
        TaskHandle handle2 = PostCancellableTask(*task_runner, FROM_HERE, WTF::BindOnce(&Increment, WTF::Unretained(&count)));
...
#pragma GCC diagnostic pop
    ...
    handle = PostCancellableTask(*task_runner, FROM_HERE, WTF::BindOnce(&GetIsActive, WTF::Unretained(&is_active), WTF::Unretained(&handle)));
    EXPECT_TRUE(handle.IsActive());
    task_runner->RunUntilIdle();
...
TEST(WebTaskRunnerTest, CancellationCheckerTest)
{
    ...
    handle = PostCancellableTask(*task_runner, FROM_HERE, WTF::BindOnce(&CancellationTestHelper::IncrementCounter, helper.GetWeakPtr()));
