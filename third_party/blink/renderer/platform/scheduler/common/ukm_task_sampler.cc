...
UkmTaskSampler::UkmTaskSampler(double thread_time_sampling_rate, double ukm_task_sampling_rate)
...
{
    if (thread_time_sampling_rate_ == 0.0 || ukm_task_sampling_rate_ == 0.0 || !(ukm_task_sampling_rate_ < 1.0)) {
    ...
    if (thread_time_sampling_rate_ < ukm_task_sampling_rate_) {
        if (has_thread_time) {
            ...
            return (ukm_task_sampling_rate_ - thread_time_sampling_rate_) / (1.0 - thread_time_sampling_rate_);
        }
    } else {
        if (has_thread_time) {
