...
WakeUpBudgetPool::WakeUpBudgetPool(const char* name)
    : BudgetPool(name)
    , wake_up_interval_(base::Seconds(1))
{
}
...
void WakeUpBudgetPool::SetWakeUpInterval(base::TimeTicks now, base::TimeDelta interval)
{
...
void WakeUpBudgetPool::AllowLowerAlignmentIfNoRecentWakeUp(base::TimeDelta alignment)
{
...
base::TimeTicks WakeUpBudgetPool::GetTimeTasksCanRunUntil(base::TimeTicks now) const
{
    if (!is_enabled_)
...
base::TimeTicks WakeUpBudgetPool::GetNextAllowedRunTime(base::TimeTicks desired_run_time) const
{
    if (!is_enabled_)
    ...
    // last wake up.
    if (last_wake_up_.has_value() && desired_run_time < last_wake_up_.value() + wake_up_duration_) {
        return desired_run_time;
    }
    ...
    if (!wake_up_alignment_if_no_recent_wake_up_.is_zero()) {
        ...
        if (!last_wake_up_.has_value()) {
            return desired_run_time.SnappedToNextTick(base::TimeTicks(), wake_up_alignment_if_no_recent_wake_up_);
        }

        // The next wake up is allowed at least |wake_up_interval_| after the last
        // wake up.
        auto next_aligned_wake_up = std::max(desired_run_time, last_wake_up_.value() + wake_up_interval_)
                                        .SnappedToNextTick(base::TimeTicks(), wake_up_alignment_if_no_recent_wake_up_);

        // A wake up is also allowed every |wake_up_interval_|.
        auto next_wake_up_at_interval = desired_run_time.SnappedToNextTick(base::TimeTicks(), wake_up_interval_);

        // Pick the earliest of the two allowed run times.
        return std::min(next_aligned_wake_up, next_wake_up_at_interval);
    }

    return desired_run_time.SnappedToNextTick(base::TimeTicks(), wake_up_interval_);
}
...
void WakeUpBudgetPool::WriteIntoTrace(perfetto::TracedValue context, base::TimeTicks now) const
{
    ...
    dict.Add("wake_up_alignment_if_no_recent_wake_up_in_seconds", wake_up_alignment_if_no_recent_wake_up_.InSecondsF());
    if (last_wake_up_) {
        dict.Add("last_wake_up_seconds_ago", (now - last_wake_up_.value()).InSecondsF());
    }
