...
TaskQueueThrottler::TaskQueueThrottler(base::sequence_manager::TaskQueue* task_queue, const base::TickClock* tick_clock)
...
void TaskQueueThrottler::IncreaseThrottleRefCount()
{
    ...
    TRACE_EVENT1("renderer.scheduler", "TaskQueueThrottler_TaskQueueThrottled", "task_queue", static_cast<void*>(task_queue_));
...
void TaskQueueThrottler::DecreaseThrottleRefCount()
{
    ...
    TRACE_EVENT1("renderer.scheduler", "TaskQueueThrottler_TaskQueueUnthrottled", "task_queue", static_cast<void*>(task_queue_));
    DisableThrottling();
}
...
std::optional<base::sequence_manager::WakeUp> TaskQueueThrottler::GetNextAllowedWakeUpImpl(
    LazyNow* lazy_now, std::optional<base::sequence_manager::WakeUp> next_wake_up, bool has_ready_task)
{
    DCHECK(IsThrottled());
    ...
    if (has_ready_task) {
        ...
        if (!allowed_run_time.is_null()) {
            ...
            return base::sequence_manager::WakeUp { allowed_run_time, base::MessagePump::GetLeewayForCurrentThread(),
                base::sequence_manager::WakeUpResolution::kLow, base::subtle::DelayPolicy::kFlexibleNoSooner };
        }
    }
    ...
    return base::sequence_manager::WakeUp { allowed_run_time, next_wake_up->leeway, base::sequence_manager::WakeUpResolution::kLow,
        next_wake_up->delay_policy == base::subtle::DelayPolicy::kPrecise ? base::subtle::DelayPolicy::kFlexibleNoSooner : next_wake_up->delay_policy };
}
...
std::optional<base::sequence_manager::WakeUp> TaskQueueThrottler::GetNextAllowedWakeUp(
    LazyNow* lazy_now, std::optional<base::sequence_manager::WakeUp> next_desired_wake_up, bool has_ready_task)
{
    TRACE_EVENT0("renderer.scheduler", "TaskQueueThrottler::OnNextWakeUpChanged");

    return GetNextAllowedWakeUpImpl(lazy_now, next_desired_wake_up, has_ready_task);
}

void TaskQueueThrottler::OnTaskRunTimeReported(base::TimeTicks start_time, base::TimeTicks end_time)
{
    if (!IsThrottled())
        return;
...
void TaskQueueThrottler::UpdateQueueState(base::TimeTicks now)
{
    ...
    if (CanRunTasksAt(now)) {
        ...
        case QueueBlockType::kAllTasks:
            task_queue_->InsertFence(TaskQueue::InsertFencePosition::kBeginningOfTime);
            break;
        ...
        TRACE_EVENT_INSTANT("renderer.scheduler", "TaskQueueThrottler::InsertFence");
    }
    task_queue_->UpdateWakeUp(&lazy_now);
}
...
std::optional<QueueBlockType> TaskQueueThrottler::GetBlockType(base::TimeTicks now) const
{
...
base::TimeTicks TaskQueueThrottler::GetNextAllowedRunTime(base::TimeTicks desired_runtime) const
{
    ...
    for (BudgetPool* budget_pool : budget_pools_) {
        if (budget_pool->CanRunTasksAt(desired_runtime))
            continue;
        result = std::max(result, budget_pool->GetNextAllowedRunTime(desired_runtime));
    }
    return result;
}

base::TimeTicks TaskQueueThrottler::GetTimeTasksCanRunUntil(base::TimeTicks now) const
{
