...
public:
    CPUTimeBudgetPool(const char* name, TraceableVariableController* tracing_controller, base::TimeTicks now);
    ...
    void SetMaxBudgetLevel(base::TimeTicks now, std::optional<base::TimeDelta> max_budget_level);
    ...
    void SetMaxThrottlingDelay(base::TimeTicks now, std::optional<base::TimeDelta> max_throttling_delay);
    ...
    void SetReportingCallback(base::RepeatingCallback<void(base::TimeDelta)> reporting_callback);

    // BudgetPool implementation:
    void RecordTaskRunTime(base::TimeTicks start_time, base::TimeTicks end_time) final;
    ...
    base::TimeTicks GetNextAllowedRunTime(base::TimeTicks desired_run_time) const final;
    void OnWakeUp(base::TimeTicks now) final;
    void WriteIntoTrace(perfetto::TracedValue context, base::TimeTicks) const final;
...
private:
    ...
    TraceableCounter<base::TimeDelta, TracingCategory::kInfo> current_budget_level_;
