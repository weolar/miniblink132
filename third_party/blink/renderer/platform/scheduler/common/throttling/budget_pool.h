...
public:
    ...
    virtual void RecordTaskRunTime(base::TimeTicks start_time, base::TimeTicks end_time) = 0;

    // Returns the earliest time when the next wake-up can be scheduled to run
    // new tasks.
    virtual base::TimeTicks GetNextAllowedRunTime(base::TimeTicks desired_run_time) const = 0;
    ...
    virtual base::TimeTicks GetTimeTasksCanRunUntil(base::TimeTicks now) const = 0;
    ...
    virtual void WriteIntoTrace(perfetto::TracedValue context, base::TimeTicks now) const = 0;
