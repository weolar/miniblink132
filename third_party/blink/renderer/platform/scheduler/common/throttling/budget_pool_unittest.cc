...
{
    std::unique_ptr<CPUTimeBudgetPool> pool = std::make_unique<CPUTimeBudgetPool>("test", &tracing_controller_, start_time_);

    pool->SetTimeBudgetRecoveryRate(SecondsAfterStart(0), 0.1);

    EXPECT_TRUE(pool->CanRunTasksAt(SecondsAfterStart(0)));
    EXPECT_EQ(SecondsAfterStart(0), pool->GetNextAllowedRunTime(SecondsAfterStart(0)));
    ...
    EXPECT_EQ(MillisecondsAfterStart(1000), pool->GetNextAllowedRunTime(SecondsAfterStart(0)));
    ...
    pool->RecordTaskRunTime(MillisecondsAfterStart(2000), MillisecondsAfterStart(2020));
    EXPECT_TRUE(pool->CanRunTasksAt(MillisecondsAfterStart(2020)));
    EXPECT_EQ(MillisecondsAfterStart(2020), pool->GetNextAllowedRunTime(SecondsAfterStart(0)));
}

TEST_F(BudgetPoolTest, WakeUpBudgetPool)
{
    std::unique_ptr<WakeUpBudgetPool> pool = std::make_unique<WakeUpBudgetPool>("test");
    ...
    EXPECT_EQ(base::TimeTicks() + base::Seconds(10), pool->GetNextAllowedRunTime(MillisecondsAfterStart(15)));
    ...
    pool->RecordTaskRunTime(MillisecondsAfterStart(12005), MillisecondsAfterStart(12007));
    ...
    EXPECT_EQ(base::TimeTicks() + base::Seconds(20), pool->GetNextAllowedRunTime(SecondsAfterStart(13)));
}
