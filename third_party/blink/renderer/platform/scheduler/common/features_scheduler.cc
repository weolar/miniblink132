...
PolicyOverride GetIntensiveWakeUpThrottlingPolicyOverride()
{
    ...
    std::string value = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kIntensiveWakeUpThrottlingPolicy);
...
base::TimeDelta GetIntensiveWakeUpThrottlingGracePeriod(bool loading)
{
    ...
    static const base::FeatureParam<int> kIntensiveWakeUpThrottling_GracePeriodSeconds { &features::kIntensiveWakeUpThrottling,
        features::kIntensiveWakeUpThrottling_GracePeriodSeconds_Name, kIntensiveWakeUpThrottling_GracePeriodSeconds_Default };

    // Controls the grace period for loaded pages.
    static const base::FeatureParam<int> kIntensiveWakeUpThrottling_GracePeriodSeconds_Loaded { &features::kQuickIntensiveWakeUpThrottlingAfterLoading,
        "grace_period_seconds_loaded", kIntensiveWakeUpThrottling_GracePeriodSecondsLoaded_Default };

    int seconds = kIntensiveWakeUpThrottling_GracePeriodSeconds_Default;
    if (GetIntensiveWakeUpThrottlingPolicyOverride() == PolicyOverride::kNoOverride) {
        seconds = kIntensiveWakeUpThrottling_GracePeriodSeconds.Get();
        if (!loading && base::FeatureList::IsEnabled(features::kQuickIntensiveWakeUpThrottlingAfterLoading))
...
MIRACLE_PARAMETER_FOR_INT(GetLoadingPhaseBufferTimeAfterFirstMeaningfulPaintMillis, features::kLoadingPhaseBufferTimeAfterFirstMeaningfulPaint,
    "LoadingPhaseBufferTimeAfterFirstMeaningfulPaintMillis", 0)

base::TimeDelta GetLoadingPhaseBufferTimeAfterFirstMeaningfulPaint()
{
    return base::Milliseconds(GetLoadingPhaseBufferTimeAfterFirstMeaningfulPaintMillis());
}

base::TimeDelta GetThreadedScrollRenderingStarvationThreshold()
{
    static const base::FeatureParam<int> kThreadedScrollRenderingStarvationThreshold { &features::kThreadedScrollPreventRenderingStarvation, "threshold_ms",
        100 };
    if (base::FeatureList::IsEnabled(features::kThreadedScrollPreventRenderingStarvation)) {
        return base::Milliseconds(kThreadedScrollRenderingStarvationThreshold.Get());
    }
    return base::TimeDelta::Max();
}

BASE_FEATURE(kPrioritizeCompositingAfterDelayTrials, "PrioritizeCompositingAfterDelayTrials", base::FEATURE_DISABLED_BY_DEFAULT);
