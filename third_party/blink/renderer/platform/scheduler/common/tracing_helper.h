...
template <TracingCategory category> class StateTracer {
    DISALLOW_NEW();

public:
    explicit StateTracer(const char* name)
        : name_(name)
    ...
    {
        if (slice_is_open_) {
            TRACE_EVENT_NESTABLE_ASYNC_END0(internal::TracingCategoryToString(category), name_, TRACE_ID_LOCAL(this));
        }
    }
...
protected:
    ...
    {
        bool result = false;
        TRACE_EVENT_CATEGORY_GROUP_ENABLED(internal::TracingCategoryToString(category), &result); // Cached.
        return result;
    }
...
private:
    void TraceImpl(const char* state, bool need_copy)
    {
        ...
        if (need_copy) {
            TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(internal::TracingCategoryToString(category), name_, TRACE_ID_LOCAL(this), "state", TRACE_STR_COPY(state));
        } else {
            TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(internal::TracingCategoryToString(category), name_, TRACE_ID_LOCAL(this), "state", state);
        }
        slice_is_open_ = true;
    }
...
template <typename T, TracingCategory category> class TraceableState : public TraceableVariable, private StateTracer<category> {
public:
    ...
    TraceableState(T initial_state, const char* name, TraceableVariableController* controller, ConverterFuncPtr converter)
    ...
    template <typename V = T> typename perfetto::check_traced_value_support<V>::type WriteIntoTrace(perfetto::TracedValue context) const
    {
...
template <TracingCategory category, typename TypedValue> class ProtoStateTracer {
    DISALLOW_NEW();
...
public:
    ...
    void TraceProto(TypedValue* value)
    {
        ...
        TRACE_EVENT_BEGIN(internal::TracingCategoryToString(category), perfetto::StaticString { name_ }, trace_track,
            [value](perfetto::EventContext ctx) { value->AsProtozeroInto(ctx.event()); });
...
template <typename T> using InitializeProtoFuncPtr = void (*)(void* event, T e);

template <typename T, TracingCategory category>
class TraceableObjectState : public TraceableVariable, public ProtoStateTracer<category, TraceableObjectState<T, category>> {
public:
    TraceableObjectState(T initial_state, const char* name, TraceableVariableController* controller, InitializeProtoFuncPtr<T> proto_init_func)
        : TraceableVariable(controller)
...
private:
    void Trace()
    {
        ProtoStateTracer<category, TraceableObjectState<T, category>>::TraceProto(this);
    }
...
template <typename T, TracingCategory category> class TraceableCounter : public TraceableVariable {
public:
    using ConverterFuncPtr = double (*)(const T&);

    TraceableCounter(T initial_value, const char* name, TraceableVariableController* controller, ConverterFuncPtr converter)
        : TraceableVariable(controller)
        , name_(name)
        , converter_(converter)
    ...
    TraceableCounter(T initial_value, const char* name, TraceableVariableController* controller)
        : TraceableCounter(initial_value, name, controller, [](const T& value) { return static_cast<double>(value); })
    {
    }
    ...
    void Trace() const
    {
        TRACE_COUNTER_ID1(internal::TracingCategoryToString(category), name_, this, converter_(value_));
    }
...
template <typename T, TracingCategory category> constexpr T operator-(const TraceableCounter<T, category>& counter)
{
    return -counter.value();
}

template <typename T, TracingCategory category> constexpr T operator/(const TraceableCounter<T, category>& lhs, const T& rhs)
{
    return lhs.value() / rhs;
}

template <typename T, TracingCategory category> constexpr bool operator>(const TraceableCounter<T, category>& lhs, const T& rhs)
{
    return lhs.value() > rhs;
}
...
template <typename T, TracingCategory category> constexpr T operator++(TraceableCounter<T, category>& counter)
{
...
template <typename T, TracingCategory category> constexpr T operator++(TraceableCounter<T, category>& counter, int)
{
