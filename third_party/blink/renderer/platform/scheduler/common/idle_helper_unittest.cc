...
void RepostingIdleTestTask(SingleThreadIdleTaskRunner* idle_task_runner, int* run_count, base::TimeTicks* deadline_out, base::TimeTicks deadline)
{
    if ((*run_count + 1) < g_max_idle_task_reposts) {
        idle_task_runner->PostIdleTask(FROM_HERE, base::BindOnce(&RepostingIdleTestTask, base::Unretained(idle_task_runner), run_count, deadline_out));
    }
    *deadline_out = deadline;
    (*run_count)++;
}
...
void RepeatingTask(base::SingleThreadTaskRunner* task_runner, int num_repeats, base::TimeDelta delay)
{
    if (num_repeats > 1) {
        task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RepeatingTask, base::Unretained(task_runner), num_repeats - 1, delay), delay);
    }
}

void UpdateClockIdleTestTask(scoped_refptr<base::TestMockTimeTaskRunner> task_runner, int* run_count, base::TimeTicks set_time, base::TimeTicks deadline)
{
...
void ShutdownIdleTask(IdleHelper* helper, bool* shutdown_task_run, base::TimeTicks deadline)
{
...
public:
    explicit IdleHelperForTest(
        SchedulerHelper* scheduler_helper, base::TimeDelta required_quiescence_duration_before_long_idle_period, TaskQueue* idle_task_queue)
        : IdleHelper(scheduler_helper, this, "TestSchedulerIdlePeriod", required_quiescence_duration_before_long_idle_period, idle_task_queue)
    {
    }
    ...
    MOCK_METHOD2(CanEnterLongIdlePeriod, bool(base::TimeTicks now, base::TimeDelta* next_long_idle_period_delay_out));
...
public:
    explicit BaseIdleHelperTest(base::TimeDelta required_quiescence_duration_before_long_idle_period)
        : test_task_runner_(base::MakeRefCounted<base::TestMockTimeTaskRunner>(base::TestMockTimeTaskRunner::Type::kStandalone))
    {
        auto settings = base::sequence_manager::SequenceManager::Settings::Builder().SetPrioritySettings(CreatePrioritySettings()).Build();
        sequence_manager_
            = base::sequence_manager::SequenceManagerForTest::Create(nullptr, test_task_runner_, test_task_runner_->GetMockTickClock(), std::move(settings));
        scheduler_helper_ = std::make_unique<NonMainThreadSchedulerHelper>(sequence_manager_.get(), nullptr, TaskType::kInternalTest);
        scheduler_helper_->AttachToCurrentThread();
        idle_helper_queue_ = scheduler_helper_->NewTaskQueue(TaskQueue::Spec(base::sequence_manager::QueueName::IDLE_TQ));
        idle_helper_ = std::make_unique<IdleHelperForTest>(
            scheduler_helper_.get(), required_quiescence_duration_before_long_idle_period, idle_helper_queue_->GetTaskQueue());
        default_task_queue_ = scheduler_helper_->DefaultNonMainThreadTaskQueue();
        default_task_runner_ = default_task_queue_->GetTaskRunnerWithDefaultTaskType();
    ...
    void SetUp() override
    {
        ...
        EXPECT_CALL(*idle_helper_, CanEnterLongIdlePeriod(_, _)).Times(AnyNumber()).WillRepeatedly(Return(true));
    ...
    template <typename E> static void CallForEachEnumValue(E first, E last, const char* (*function)(E))
    {
        for (E val = first; val < last; val = static_cast<E>(static_cast<int>(val) + 1)) {
            (*function)(val);
        }
    }
    ...
    static void CheckAllTaskQueueIdToString()
    {
        CallForEachEnumValue<IdleHelper::IdlePeriodState>(
            IdleHelper::IdlePeriodState::kFirstIdlePeriodState, IdleHelper::IdlePeriodState::kIdlePeriodStateCount, &IdleHelper::IdlePeriodStateToString);
    }

    bool IsInIdlePeriod() const
    {
        return idle_helper_->IsInIdlePeriod(idle_helper_->SchedulerIdlePeriodState());
    }
...
protected:
    ...
    {
        return base::Milliseconds(IdleHelper::kRetryEnableLongIdlePeriodDelayMillis);
    }
    ...
    const TaskQueue* idle_queue() const
    {
...
{
    int run_count = 0;
    base::TimeTicks expected_deadline = test_task_runner_->NowTicks() + base::Milliseconds(2300);
    ...
    idle_helper_->StartIdlePeriod(IdleHelper::IdlePeriodState::kInShortIdlePeriod, test_task_runner_->NowTicks(), expected_deadline);
    test_task_runner_->RunUntilIdle();
...
TEST_F(IdleHelperTest, TestPostIdleTask_EndIdlePeriod)
{
    ...
    idle_helper_->StartIdlePeriod(
        IdleHelper::IdlePeriodState::kInShortIdlePeriod, test_task_runner_->NowTicks(), test_task_runner_->NowTicks() + base::Milliseconds(10));
...
TEST_F(IdleHelperTest, TestRepostingIdleTask)
{
    ...
    idle_task_runner_->PostIdleTask(FROM_HERE, base::BindOnce(&RepostingIdleTestTask, base::RetainedRef(idle_task_runner_), &run_count, &actual_deadline));
    idle_helper_->StartIdlePeriod(
        IdleHelper::IdlePeriodState::kInShortIdlePeriod, test_task_runner_->NowTicks(), test_task_runner_->NowTicks() + base::Milliseconds(10));
    test_task_runner_->RunUntilIdle();
    EXPECT_EQ(1, run_count);
...
public:
    ...
    IdleHelperTestWithIdlePeriodObserver(const IdleHelperTestWithIdlePeriodObserver&) = delete;
    IdleHelperTestWithIdlePeriodObserver& operator=(const IdleHelperTestWithIdlePeriodObserver&) = delete;
...
TEST_F(IdleHelperTestWithIdlePeriodObserver, TestEnterAndExitIdlePeriod)
{
    ...
    ON_CALL(*idle_helper_, OnIdlePeriodStarted()).WillByDefault(Invoke([fixture]() { EXPECT_TRUE(fixture->IsInIdlePeriod()); }));
    ON_CALL(*idle_helper_, OnIdlePeriodEnded()).WillByDefault(Invoke([fixture]() { EXPECT_FALSE(fixture->IsInIdlePeriod()); }));
...
{
    base::TimeTicks expected_deadline = test_task_runner_->NowTicks() + maximum_idle_period_duration();
    base::TimeTicks deadline_in_task;
    int run_count = 0;

    idle_task_runner_->PostIdleTask(FROM_HERE, base::BindOnce(&IdleTestTask, &run_count, &deadline_in_task));

    EXPECT_CALL(*idle_helper_, CanEnterLongIdlePeriod(_, _)).Times(1).WillRepeatedly(Return(true));
    ExpectIdlePeriodStartsButNeverEnds();
...
TEST_F(IdleHelperTest, TestLongIdlePeriodWithPendingDelayedTask)
{
    ...
    base::TimeTicks expected_deadline = test_task_runner_->NowTicks() + pending_task_delay;
    base::TimeTicks deadline_in_task;
    int run_count = 0;
...
TEST_F(IdleHelperTestWithIdlePeriodObserver, TestLongIdlePeriodRepeating)
{
    ...
    EXPECT_CALL(*idle_helper_, CanEnterLongIdlePeriod(_, _)).Times(4).WillRepeatedly(Return(true));
    ...
    idle_task_runner_->PostIdleTask(FROM_HERE,
        base::BindOnce(
            &RepostingUpdateClockIdleTestTask, base::RetainedRef(idle_task_runner_), &run_count, test_task_runner_, idle_task_runtime, &actual_deadlines));
    idle_task_runner_->PostIdleTask(FROM_HERE, base::BindOnce(&EndIdlePeriodIdleTask, base::Unretained(idle_helper_.get())));
...
TEST_F(IdleHelperTestWithIdlePeriodObserver, TestLongIdlePeriodWhenNotCanEnterLongIdlePeriod)
{
    ...
    ON_CALL(*idle_helper_, CanEnterLongIdlePeriod(_, _))
        .WillByDefault(Invoke([delay, delay_over](base::TimeTicks now, base::TimeDelta* next_long_idle_period_delay_out) {
            if (now >= delay_over)
                return true;
...
TEST_F(IdleHelperTest, TestLongIdlePeriodDoesNotImmediatelyRestartIfMaxDeadline)
{
    Vector<base::TimeTicks> actual_deadlines;
    int run_count = 0;
    ...
    test_task_runner_->FastForwardUntilNoTasksRemain();
    EXPECT_EQ(2, run_count);
    EXPECT_THAT(actual_deadlines, testing::ElementsAre(clock_before + maximum_idle_period_duration(), clock_before + 2 * maximum_idle_period_duration()));
}
...
TEST_F(IdleHelperTest, TestLongIdlePeriodRestartWaitsIfNotMaxDeadline)
{
    ...
    base::TimeTicks expected_deadline(
        test_task_runner_->NowTicks() + pending_task_delay + maximum_idle_period_duration() + retry_enable_long_idle_period_delay());
    ...
    test_task_runner_->AdvanceMockTickClock(pending_task_delay - idle_task_duration);
    ...
    test_task_runner_->AdvanceMockTickClock(retry_enable_long_idle_period_delay());
    test_task_runner_->RunUntilIdle();
...
void TestCanExceedIdleDeadlineIfRequiredTask(IdleHelperForTest* idle_helper, bool* can_exceed_idle_deadline_out, int* run_count, base::TimeTicks deadline)
{
...
TEST_F(IdleHelperTest, CanExceedIdleDeadlineIfRequired)
{
    ...
    idle_task_runner_->PostIdleTask(
        FROM_HERE, base::BindOnce(&TestCanExceedIdleDeadlineIfRequiredTask, idle_helper_.get(), &can_exceed_idle_deadline, &run_count));
    idle_helper_->StartIdlePeriod(
        IdleHelper::IdlePeriodState::kInShortIdlePeriod, test_task_runner_->NowTicks(), test_task_runner_->NowTicks() + base::Milliseconds(10));
    test_task_runner_->RunUntilIdle();
    EXPECT_EQ(1, run_count);
    ...
    default_task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&NullTask), base::Milliseconds(10));
    idle_task_runner_->PostIdleTask(
        FROM_HERE, base::BindOnce(&TestCanExceedIdleDeadlineIfRequiredTask, idle_helper_.get(), &can_exceed_idle_deadline, &run_count));
    idle_helper_->EnableLongIdlePeriod();
    test_task_runner_->RunUntilIdle();
...
public:
    IdleHelperWithQuiescencePeriodTest(const IdleHelperWithQuiescencePeriodTest&) = delete;
    IdleHelperWithQuiescencePeriodTest& operator=(const IdleHelperWithQuiescencePeriodTest&) = delete;
    enum {
...
class IdleHelperWithQuiescencePeriodTestWithIdlePeriodObserver : public IdleHelperWithQuiescencePeriodTest {
public:
    ...
    IdleHelperWithQuiescencePeriodTestWithIdlePeriodObserver(const IdleHelperWithQuiescencePeriodTestWithIdlePeriodObserver&) = delete;
    IdleHelperWithQuiescencePeriodTestWithIdlePeriodObserver& operator=(const IdleHelperWithQuiescencePeriodTestWithIdlePeriodObserver&) = delete;
...
TEST_F(IdleHelperWithQuiescencePeriodTest, LongIdlePeriodStartsImmediatelyIfQuiescent)
{
    base::TimeTicks actual_deadline;
    int run_count = 0;
...
TEST_F(IdleHelperWithQuiescencePeriodTestWithIdlePeriodObserver, LongIdlePeriodDoesNotStartsImmediatelyIfBusy)
{
    MakeNonQuiescent();
...
TEST_F(IdleHelperWithQuiescencePeriodTest, LongIdlePeriodStartsAfterQuiescence)
{
    MakeNonQuiescent();

    // Run a repeating task so we're deemed to be busy for the next 400ms.
    default_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RepeatingTask, base::Unretained(default_task_runner_.get()), 10, base::Milliseconds(40)));

    int run_count = 0;
    // In this scenario EnableLongIdlePeriod deems us not to be quiescent 5x in
    // a row.
    base::TimeTicks expected_deadline = test_task_runner_->NowTicks() + base::Milliseconds(5 * kQuiescenceDelayMs + kLongIdlePeriodMs);
    base::TimeTicks deadline_in_task;
    idle_task_runner_->PostIdleTask(FROM_HERE, base::BindOnce(&IdleTestTask, &run_count, &deadline_in_task));
...
TEST_F(IdleHelperWithQuiescencePeriodTest, QuescienceCheckedForAfterLongIdlePeriodEnds)
{
    ...
    int run_count = 0;
    base::TimeTicks deadline_in_task;
    base::TimeTicks expected_deadline = test_task_runner_->NowTicks() + base::Milliseconds(kQuiescenceDelayMs + kLongIdlePeriodMs);
    idle_task_runner_->PostIdleTask(FROM_HERE, base::BindOnce(&IdleTestTask, &run_count, &deadline_in_task));
    idle_helper_->EnableLongIdlePeriod();
    test_task_runner_->FastForwardUntilNoTasksRemain();
...
TEST_F(IdleHelperTest, NoShortIdlePeriodWhenDeadlineTooClose)
{
    ...
    base::TimeTicks less_than_min_deadline(test_task_runner_->NowTicks() + minimum_idle_period_duration() - half_a_ms);
    base::TimeTicks more_than_min_deadline(test_task_runner_->NowTicks() + minimum_idle_period_duration() + half_a_ms);

    idle_helper_->StartIdlePeriod(IdleHelper::IdlePeriodState::kInShortIdlePeriod, test_task_runner_->NowTicks(), less_than_min_deadline);
    test_task_runner_->RunUntilIdle();
    EXPECT_EQ(0, run_count);

    idle_helper_->StartIdlePeriod(IdleHelper::IdlePeriodState::kInShortIdlePeriod, test_task_runner_->NowTicks(), more_than_min_deadline);
    test_task_runner_->RunUntilIdle();
    EXPECT_EQ(1, run_count);
}
...
TEST_F(IdleHelperTest, NoLongIdlePeriodWhenDeadlineTooClose)
{
    ...
    base::TimeDelta half_a_ms(base::Microseconds(50));
    base::TimeDelta less_than_min_deadline_duration(minimum_idle_period_duration() - half_a_ms);
    base::TimeDelta more_than_min_deadline_duration(minimum_idle_period_duration() + half_a_ms);

    idle_task_runner_->PostIdleTask(FROM_HERE, base::BindOnce(&IdleTestTask, &run_count, &deadline_in_task));
    default_task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&NullTask), less_than_min_deadline_duration);
    ...
    default_task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&NullTask), more_than_min_deadline_duration);
    idle_helper_->EnableLongIdlePeriod();
    test_task_runner_->RunUntilIdle();
    EXPECT_EQ(1, run_count);
}

TEST_F(IdleHelperWithQuiescencePeriodTest, PendingEnableLongIdlePeriodNotRunAfterShutdown)
{
    MakeNonQuiescent();
    ...
    int run_count = 0;
    base::TimeTicks deadline_in_task;
    idle_task_runner_->PostIdleTask(FROM_HERE, base::BindOnce(&ShutdownIdleTask, base::Unretained(idle_helper_.get()), &shutdown_task_run));
    idle_task_runner_->PostIdleTask(FROM_HERE, base::BindOnce(&IdleTestTask, &run_count, &deadline_in_task));
...
TEST_F(IdleHelperTest, TestPostDelayedIdleTask)
{
    ...
    idle_task_runner_->PostDelayedIdleTask(FROM_HERE, base::Milliseconds(200), base::BindOnce(&IdleTestTask, &run_count, &deadline_in_task));
...
public:
    ...
    void PostDelayedIdleTaskFromNewThread(base::TimeDelta delay, int* run_count)
    {
        std::unique_ptr<NonMainThread> thread
            = NonMainThread::CreateThread(ThreadCreationParams(ThreadType::kTestThread).SetThreadNameForTest("TestBackgroundThread"));
        PostCrossThreadTask(*thread->GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(&PostIdleTaskFromBackgroundThread, idle_task_runner_, delay, WTF::CrossThreadUnretained(run_count)));
        thread.reset();
    }

protected:
    static void PostIdleTaskFromBackgroundThread(scoped_refptr<SingleThreadIdleTaskRunner> idle_task_runner, base::TimeDelta delay, int* run_count)
    {
        auto callback = ConvertToBaseOnceCallback(CrossThreadBindOnce(&IdleTestTask, WTF::CrossThreadUnretained(run_count), nullptr));
        if (delay.is_zero()) {
            idle_task_runner->PostIdleTask(FROM_HERE, std::move(callback));
        } else {
            idle_task_runner->PostDelayedIdleTask(FROM_HERE, delay, std::move(callback));
        }
    }
};
...
TEST_F(MultiThreadedIdleHelperTest, DelayedAndNonDelayedIdleTasksFromMultipleThreads)
{
    int run_count = 0;

    PostIdleTaskFromNewThread(&run_count);

    idle_task_runner_->PostIdleTask(FROM_HERE, base::BindOnce(&IdleTestTask, &run_count, nullptr));

    PostDelayedIdleTaskFromNewThread(base::Milliseconds(200), &run_count);

    idle_task_runner_->PostDelayedIdleTask(FROM_HERE, base::Milliseconds(250), base::BindOnce(&IdleTestTask, &run_count, nullptr));
