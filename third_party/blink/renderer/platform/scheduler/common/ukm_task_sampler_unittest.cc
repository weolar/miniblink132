...
TEST(UkmTaskSamplerTest, GetConditionalSamplingProbability)
{
    for (double time_prob = 0; time_prob < 1.0; time_prob += 0.1) {
        ...
        for (double expected_ukm_rate = 0; expected_ukm_rate < 1.0; expected_ukm_rate += 0.1) {
            sampler.SetUkmTaskSamplingRate(expected_ukm_rate);
            double ukm_rate = sampler.GetConditionalSamplingProbability(true) * time_prob + sampler.GetConditionalSamplingProbability(false) * (1 - time_prob);
            EXPECT_THAT(ukm_rate, DoubleEq(expected_ukm_rate)) << "For time_prob: " << time_prob;
        }
    }
}
