...
ProcessTimeOverrideCoordinator::ScopedOverride::ScopedOverride(base::RepeatingClosure schedule_work_callback)
...
base::TimeTicks ProcessTimeOverrideCoordinator::ScopedOverride::TryAdvancingTime(base::TimeTicks requested_ticks)
{
    return ProcessTimeOverrideCoordinator::Instance().TryAdvancingTime(this, requested_ticks);
}

base::TimeTicks ProcessTimeOverrideCoordinator::ScopedOverride::NowTicks() const
{
...
std::unique_ptr<ProcessTimeOverrideCoordinator::ScopedOverride> ProcessTimeOverrideCoordinator::CreateOverride(
    base::Time requested_time, base::TimeTicks requested_ticks, base::RepeatingClosure schedule_work_callback)
{
    auto handle = base::WrapUnique(new ScopedOverride(std::move(schedule_work_callback)));

    Instance().RegisterOverride(handle.get(), requested_time, requested_ticks);
    return handle;
}

void ProcessTimeOverrideCoordinator::RegisterOverride(ScopedOverride* handle, base::Time requested_time, base::TimeTicks requested_ticks)
{
    base::AutoLock auto_lock(lock_);
    if (requested_ticks_by_client_.empty()) {
        EnableOverride(requested_time, requested_ticks);
    }
    bool inserted = requested_ticks_by_client_.insert({ handle, current_ticks_.load(std::memory_order_relaxed) }).second;
    DCHECK(inserted);
}

void ProcessTimeOverrideCoordinator::UnregisterOverride(ScopedOverride* handle)
{
    base::AutoLock auto_lock(lock_);
...
void ProcessTimeOverrideCoordinator::EnableOverride(base::Time initial_time, base::TimeTicks initial_ticks)
{
    ...
    clock_override_ = std::make_unique<base::subtle::ScopedTimeClockOverrides>(
        &ProcessTimeOverrideCoordinator::CurrentTime, &ProcessTimeOverrideCoordinator::CurrentTicks, nullptr);
}
...
base::TimeTicks ProcessTimeOverrideCoordinator::TryAdvancingTime(ScopedOverride* handle, base::TimeTicks requested_ticks)
{
    base::AutoLock auto_lock(lock_);
...
{
    auto ticks = ProcessTimeOverrideCoordinator::Instance().current_ticks_.load(std::memory_order_relaxed);
