...
BackForwardCacheDisablingFeatureTracker::BackForwardCacheDisablingFeatureTracker(
    TraceableVariableController* tracing_controller, ThreadSchedulerBase* scheduler)
...
void BackForwardCacheDisablingFeatureTracker::SetDelegate(FrameOrWorkerScheduler::Delegate* delegate)
{
...
void BackForwardCacheDisablingFeatureTracker::Reset()
{
    for (const auto& it : back_forward_cache_disabling_feature_counts_) {
        TRACE_EVENT_NESTABLE_ASYNC_END0(
            "renderer.scheduler", "ActiveSchedulerTrackedFeature", TRACE_ID_LOCAL(reinterpret_cast<intptr_t>(this) ^ static_cast<int>(it.first)));
    }
...
void BackForwardCacheDisablingFeatureTracker::AddFeatureInternal(SchedulingPolicy::Feature feature)
{
    ...
    NotifyDelegateAboutFeaturesAfterCurrentTask(BackForwardCacheDisablingFeatureTracker::TracingType::kBegin, feature);
}

void BackForwardCacheDisablingFeatureTracker::AddNonStickyFeature(
    SchedulingPolicy::Feature feature, std::unique_ptr<SourceLocation> source_location, FrameOrWorkerScheduler::SchedulingAffectingFeatureHandle* handle)
{
    DCHECK(!scheduler::IsFeatureSticky(feature));
    AddFeatureInternal(feature);

    DCHECK(handle);
    non_sticky_features_and_js_locations_.MaybeAdd(handle->GetFeatureAndJSLocationBlockingBFCache());

    NotifyDelegateAboutFeaturesAfterCurrentTask(BackForwardCacheDisablingFeatureTracker::TracingType::kBegin, feature);
}

void BackForwardCacheDisablingFeatureTracker::AddStickyFeature(SchedulingPolicy::Feature feature, std::unique_ptr<SourceLocation> source_location)
{
    DCHECK(scheduler::IsFeatureSticky(feature));
    AddFeatureInternal(feature);

    sticky_features_and_js_locations_.MaybeAdd(FeatureAndJSLocationBlockingBFCache(feature, source_location.get()));

    NotifyDelegateAboutFeaturesAfterCurrentTask(BackForwardCacheDisablingFeatureTracker::TracingType::kBegin, feature);
}

void BackForwardCacheDisablingFeatureTracker::Remove(FeatureAndJSLocationBlockingBFCache feature_and_js_location)
{
    ...
    opted_out_from_back_forward_cache_ = !back_forward_cache_disabling_feature_counts_.empty();
...
WTF::HashSet<SchedulingPolicy::Feature> BackForwardCacheDisablingFeatureTracker::GetActiveFeaturesTrackedForBackForwardCacheMetrics()
{
...
BFCacheBlockingFeatureAndLocations& BackForwardCacheDisablingFeatureTracker::GetActiveNonStickyFeaturesTrackedForBackForwardCache()
{
    return non_sticky_features_and_js_locations_;
}

const BFCacheBlockingFeatureAndLocations& BackForwardCacheDisablingFeatureTracker::GetActiveStickyFeaturesTrackedForBackForwardCache() const
{
    return sticky_features_and_js_locations_;
}

void BackForwardCacheDisablingFeatureTracker::NotifyDelegateAboutFeaturesAfterCurrentTask(TracingType tracing_type, SchedulingPolicy::Feature traced_feature)
{
    ...
    case TracingType::kBegin:
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("renderer.scheduler", "ActiveSchedulerTrackedFeature",
            TRACE_ID_LOCAL(reinterpret_cast<intptr_t>(this) ^ static_cast<int>(traced_feature)), "feature", FeatureToHumanReadableString(traced_feature));
        break;
    case TracingType::kEnd:
        TRACE_EVENT_NESTABLE_ASYNC_END0(
            "renderer.scheduler", "ActiveSchedulerTrackedFeature", TRACE_ID_LOCAL(reinterpret_cast<intptr_t>(this) ^ static_cast<int>(traced_feature)));
        break;
    }
}
...
void BackForwardCacheDisablingFeatureTracker::ReportFeaturesToDelegate()
{
    ...
    if (non_sticky_features_and_js_locations_ == last_reported_non_sticky_ && sticky_features_and_js_locations_ == last_reported_sticky_) {
        return;
    }
    last_reported_non_sticky_ = non_sticky_features_and_js_locations_;
    last_reported_sticky_ = sticky_features_and_js_locations_;
    FrameOrWorkerScheduler::Delegate::BlockingDetails details(non_sticky_features_and_js_locations_, sticky_features_and_js_locations_);
