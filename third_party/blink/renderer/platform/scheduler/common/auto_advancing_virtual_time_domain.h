...
// |-----------------------------> time
class PLATFORM_EXPORT AutoAdvancingVirtualTimeDomain : public base::sequence_manager::TimeDomain, public base::TaskObserver {
public:
    AutoAdvancingVirtualTimeDomain(base::Time initial_time, base::TimeTicks initial_time_ticks, SchedulerHelper* helper);
    AutoAdvancingVirtualTimeDomain(const AutoAdvancingVirtualTimeDomain&) = delete;
    AutoAdvancingVirtualTimeDomain& operator=(const AutoAdvancingVirtualTimeDomain&) = delete;
    ...
    int task_starvation_count() const
    {
        return task_starvation_count_;
    }

    base::TimeTicks InitialTicks() const
    {
        return initial_time_ticks_;
    }
...
private:
    ...
    bool MaybeFastForwardToWakeUp(std::optional<base::sequence_manager::WakeUp> wakeup, bool quit_when_idle_requested) override;
    ...
    std::unique_ptr<ProcessTimeOverrideCoordinator::ScopedOverride> time_override_;
