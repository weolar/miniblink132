...
void ThreadSchedulerBase::ExecuteAfterCurrentTask(base::OnceClosure on_completion_task)
{
...
base::TimeTicks ThreadSchedulerBase::EnableVirtualTime(base::Time initial_time)
{
    ...
    virtual_time_domain_ = std::make_unique<AutoAdvancingVirtualTimeDomain>(initial_time, initial_ticks, &GetHelper());
...
void ThreadSchedulerBase::GrantVirtualTimeBudget(base::TimeDelta budget, base::OnceClosure budget_exhausted_callback)
{
    GetVirtualTimeTaskRunner()->PostDelayedTask(FROM_HERE, std::move(budget_exhausted_callback), budget);
    // This can shift time forwards if there's a pending MaybeAdvanceVirtualTime,
    // so it's important this is called second.
    virtual_time_domain_->SetVirtualTimeFence(GetTickClock()->NowTicks() + budget);
}
...
void ThreadSchedulerBase::SetMaxVirtualTimeTaskStarvationCount(int max_task_starvation_count)
{
    DCHECK(IsVirtualTimeEnabled());
...
WebScopedVirtualTimePauser ThreadSchedulerBase::CreateWebScopedVirtualTimePauser(
...
void ThreadSchedulerBase::MaybeAdvanceVirtualTime(base::TimeTicks new_virtual_time)
{
    if (virtual_time_domain_)
...
namespace {
const char* VirtualTimePolicyToString(VirtualTimeController::VirtualTimePolicy virtual_time_policy)
{
...
void ThreadSchedulerBase::WriteVirtualTimeInfoIntoTrace(perfetto::TracedDictionary& dict) const
{
    dict.Add("virtual_time_stopped", virtual_time_stopped_);
    dict.Add("virtual_time_pause_count", virtual_time_pause_count_);
    dict.Add("virtual_time_policy", VirtualTimePolicyToString(virtual_time_policy_));
...
void ThreadSchedulerBase::ApplyVirtualTimePolicy()
{
    ...
    case VirtualTimePolicy::kAdvance:
        virtual_time_domain_->SetMaxVirtualTimeTaskStarvationCount(GetHelper().IsInNestedRunloop() ? 0 : max_virtual_time_task_starvation_count_);
    ...
    case VirtualTimePolicy::kDeterministicLoading:
        ...
        SetVirtualTimeStopped(virtual_time_pause_count_ != 0 || GetHelper().IsInNestedRunloop());
        break;
    }
}
