...
SchedulerHelper::SchedulerHelper(SequenceManager* sequence_manager)
    ...
    , observer_(nullptr)
    , ukm_task_sampler_(sequence_manager_->GetMetricRecordingSettings().task_sampling_rate_for_recording_cpu_time)
{
    sequence_manager_->SetWorkBatchSize(4);
}

void SchedulerHelper::InitDefaultTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
...
void SchedulerHelper::AttachToCurrentThread()
{
    ...
    CheckOnValidThread();
    DCHECK(default_task_runner_) << "Must be invoked after InitDefaultTaskRunner().";
}
...
void SchedulerHelper::AddTaskObserver(base::TaskObserver* task_observer)
{
    CheckOnValidThread();
    if (sequence_manager_) {
        static_cast<base::sequence_manager::internal::SequenceManagerImpl*>(sequence_manager_)->AddTaskObserver(task_observer);
    }
}
...
void SchedulerHelper::RemoveTaskObserver(base::TaskObserver* task_observer)
{
    CheckOnValidThread();
    if (sequence_manager_) {
        static_cast<base::sequence_manager::internal::SequenceManagerImpl*>(sequence_manager_)->RemoveTaskObserver(task_observer);
    }
}

void SchedulerHelper::AddTaskTimeObserver(TaskTimeObserver* task_time_observer)
{
    if (sequence_manager_)
        sequence_manager_->AddTaskTimeObserver(task_time_observer);
}

void SchedulerHelper::RemoveTaskTimeObserver(TaskTimeObserver* task_time_observer)
{
    if (sequence_manager_)
...
std::optional<base::sequence_manager::WakeUp> SchedulerHelper::GetNextWakeUp() const
{
    CheckOnValidThread();
    DCHECK(sequence_manager_);
...
void SchedulerHelper::SetTimeDomain(base::sequence_manager::TimeDomain* time_domain)
{
    CheckOnValidThread();
    DCHECK(sequence_manager_);
...
{
    if (sequence_manager_) {
        return sequence_manager_->GetMetricRecordingSettings().records_cpu_time_for_all_tasks();
    }
    return false;
}
