...
// runner.
class PLATFORM_EXPORT BlinkSchedulerSingleThreadTaskRunner : public base::SingleThreadTaskRunner {
public:
    ...
    BlinkSchedulerSingleThreadTaskRunner(
        scoped_refptr<base::SingleThreadTaskRunner> wrapped_task_runner, scoped_refptr<base::SingleThreadTaskRunner> thread_task_runner);
    ~BlinkSchedulerSingleThreadTaskRunner() override;

    BlinkSchedulerSingleThreadTaskRunner(BlinkSchedulerSingleThreadTaskRunner&&) = delete;
    BlinkSchedulerSingleThreadTaskRunner& operator=(BlinkSchedulerSingleThreadTaskRunner&&) = delete;
    ...
    {
        return outer_->PostNonNestableDelayedTask(from_here, std::move(task), delay);
    }
    ...
    {
        return outer_->PostCancelableDelayedTask(pass_key, from_here, std::move(task), delay);
    }

    [[nodiscard]] base::DelayedTaskHandle PostCancelableDelayedTaskAt(base::subtle::PostDelayedTaskPassKey pass_key, const base::Location& from_here,
        base::OnceClosure task, base::TimeTicks delayed_run_time, base::subtle::DelayPolicy delay_policy) override
    {
        return outer_->PostCancelableDelayedTaskAt(pass_key, from_here, std::move(task), delayed_run_time, delay_policy);
    }

    bool PostDelayedTaskAt(base::subtle::PostDelayedTaskPassKey pass_key, const base::Location& from_here, base::OnceClosure task,
        base::TimeTicks delayed_run_time, base::subtle::DelayPolicy delay_policy) override
    {
        return outer_->PostDelayedTaskAt(pass_key, from_here, std::move(task), delayed_run_time, delay_policy);
    }
...
protected:
    ...
    bool DeleteOrReleaseSoonInternal(const base::Location& from_here, void (*deleter)(const void*), const void* object) override;
