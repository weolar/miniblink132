...
ThreadCreationParams& ThreadCreationParams::SetThreadNameForTest(const char* thread_name)
{
    name = thread_name;
    return *this;
}

ThreadCreationParams& ThreadCreationParams::SetFrameOrWorkerScheduler(FrameOrWorkerScheduler* scheduler)
{
...
void Thread::CreateAndSetCompositorThread()
{
    ...
    auto compositor_thread = std::make_unique<scheduler::CompositorThread>(params);
    ...
    compositor_thread->GetTaskRunner()->PostTaskAndReplyWithResult(
        FROM_HERE, base::BindOnce(&base::PlatformThread::CurrentId), base::BindOnce([](base::PlatformThreadId compositor_thread_id) {
            ...
            // browser to do it.
            Platform::Current()->SetThreadType(compositor_thread_id, base::ThreadType::kDisplayCritical);
        }));
#endif
...
std::unique_ptr<MainThread> MainThread::SetMainThread(std::unique_ptr<MainThread> main_thread)
{
