...
class ThreadCPUThrottler::ThrottlingThread final : public base::PlatformThread::Delegate {
public:
...
#ifdef OS_WIN
    : throttled_thread_handle_(::OpenThread(THREAD_SUSPEND_RESUME, false, ::GetCurrentThreadId()))
{
#else
...
{
    throttling_rate_percent_.store(static_cast<int>(rate * 100), std::memory_order_release);
}
...
void ThreadCPUThrottler::ThrottlingThread::HandleSignal(int signal)
{
    ...
    uint32_t throttling_rate_percent = throttling_rate_percent_.load(std::memory_order_acquire);
    // Limit the observed run duration to 1000μs to deal with the first entrance
    // to the signal handler.
    uint32_t run_duration_us = static_cast<uint32_t>(std::min(run_duration.InMicroseconds(), static_cast<int64_t>(1000)));
    uint32_t sleep_duration_us = run_duration_us * throttling_rate_percent / 100 - run_duration_us;
...
void ThreadCPUThrottler::ThrottlingThread::Throttle()
{
    ...
    base::TimeDelta run_duration = base::Microseconds(static_cast<int>(quant_time_us / rate));
    base::TimeDelta sleep_duration = base::Microseconds(quant_time_us) - run_duration;
    Sleep(run_duration);
