...
template <typename VectorType> class NonTraceableSparseVectorFields {
    DISALLOW_NEW();
...
template <typename VectorType> class TraceableSparseVectorFields {
    DISALLOW_NEW();
...
template <typename FieldId, typename FieldType, wtf_size_t inline_capacity = 0,
    typename VectorType = std::conditional_t<WTF::IsMemberType<FieldType>::value || WTF::IsTraceable<FieldType>::value, HeapVector<FieldType, inline_capacity>,
        Vector<FieldType, inline_capacity>>,
    typename BitfieldType = std::conditional_t<static_cast<unsigned>(FieldId::kNumFields) <= std::numeric_limits<uint32_t>::digits, uint32_t, uint64_t>>
class SparseVector :
    ...
    public std::conditional_t<WTF::IsTraceable<VectorType>::value, internal::TraceableSparseVectorFields<VectorType>,
    ...
    static_assert(std::numeric_limits<BitfieldType>::digits >= static_cast<unsigned>(FieldId::kNumFields),
...
public:
    ...
    void SetField(FieldId field_id, FieldType&& field)
    {
        if (HasField(field_id)) {
            ...
            this->fields_.insert(GetFieldIndex(field_id), std::forward<FieldType>(field));
        }
    }
...
private:
    ...
    wtf_size_t GetFieldIndex(FieldId field_id) const
    {
        ...
        const BitfieldType mask = ~(~static_cast<BitfieldType>(0) << static_cast<unsigned>(field_id));
