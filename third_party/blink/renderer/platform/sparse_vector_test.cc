...
struct IntFieldTest {
    ...
    static Handle MakeTest()
    {
        return std::make_unique<IntFieldTest>();
    }

    static int CreateField(int value)
    {
        return value;
    }
    static int GetValue(int field)
    {
        return field;
    }
};
...
public:
    ...
    static Handle MakeTest()
    {
        return std::make_unique<UniquePtrFieldTest>();
    }
    ...
    static int GetValue(const std::unique_ptr<int>& field)
    {
        return *field;
    }
};
...
public:
    ...
    static Traceable CreateField(int value)
    {
        return Traceable { value };
    }
    static int GetValue(const Traceable& field)
    {
        return field.value;
    }
};
...
public:
    ...
    static int GetValue(const Member<GCObject>& field)
    {
        return field->value;
    }
};

template <typename TestType> class SparseVectorTest : public testing::Test {
protected:
    ...
    {
        for (auto id = FieldId::kFirst; id <= FieldId::kLast; id = static_cast<FieldId>(static_cast<unsigned>(id) + 1)) {
            EXPECT_EQ(std::count(field_ids.begin(), field_ids.end(), id), sparse_vector().HasField(id)) << static_cast<unsigned>(id);
        }
    }

    auto& sparse_vector()
    {
        return test_->sparse_vector;
    }
...
using TestTypes = ::testing::Types<IntFieldTest, UniquePtrFieldTest, TraceableFieldTest, MemberFieldTest>;
