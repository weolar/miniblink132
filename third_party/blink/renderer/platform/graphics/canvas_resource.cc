...
base::trace_event::MemoryAllocatorDumpGuid gpu::GetSharedImageGUIDForTracing(const gpu::Mailbox&)
{
    *(int*)1 = 1;
    return base::trace_event::MemoryAllocatorDumpGuid();
}

gpu::SharedImageInterface::SwapChainSharedImages::~SwapChainSharedImages(void)
{
    *(int*)1 = 1;
}
...
CanvasResource::CanvasResource(base::WeakPtr<CanvasResourceProvider> provider, cc::PaintFlags::FilterQuality filter_quality, const SkColorInfo& info)
    : owning_thread_ref_(base::PlatformThread::CurrentRef())
    , owning_thread_task_runner_(ThreadScheduler::Current()->CleanupTaskRunner())
...
static void ReleaseFrameResources(base::WeakPtr<CanvasResourceProvider> resource_provider, viz::ReleaseCallback&& viz_release_callback,
    scoped_refptr<CanvasResource>&& resource, const gpu::SyncToken& sync_token, bool lost_resource)
{
    ...
    if (resource_provider && !lost_resource && resource->IsRecycleable() && resource->HasOneRef()) {
        resource_provider->RecycleResource(std::move(resource));
    }
}

bool CanvasResource::PrepareTransferableResource(
    viz::TransferableResource* out_resource, CanvasResource::ReleaseCallback* out_callback, bool needs_verified_synctoken)
{
    DCHECK(IsValid());
    ...
    *out_callback = WTF::BindOnce(&ReleaseFrameResources, provider_, TakeVizReleaseCallback());

    if (!out_resource)
        return true;
    if (SupportsAcceleratedCompositing()) {
        return UsesClientSharedImage() ? PrepareAcceleratedTransferableResourceFromClientSI(out_resource, needs_verified_synctoken)
                                       : PrepareAcceleratedTransferableResourceWithoutClientSI(out_resource);
    }
    return PrepareUnacceleratedTransferableResource(out_resource);
}

bool CanvasResource::PrepareAcceleratedTransferableResourceFromClientSI(viz::TransferableResource* out_resource, bool needs_verified_synctoken)
{
    TRACE_EVENT0("blink", "CanvasResource::PrepareAcceleratedTransferableResource");
    ...
    *out_resource = viz::TransferableResource::MakeGpu(client_shared_image->mailbox(), client_shared_image->GetTextureTarget(),
        GetSyncTokenWithOptionalVerification(needs_verified_synctoken), Size(), GetSharedImageFormat(), IsOverlayCandidate(),
    ...
    if (!UsesAcceleratedRaster()) {
        out_resource->synchronization_type = viz::TransferableResource::SynchronizationType::kGpuCommandsCompleted;
    }
...
{
    return SkImageInfo::Make(SkISize::Make(Size().width(), Size().height()), info_);
}
...
CanvasResourceSharedBitmap::CanvasResourceSharedBitmap(const SkImageInfo& info, base::WeakPtr<CanvasResourceProvider> provider,
    base::WeakPtr<WebGraphicsSharedImageInterfaceProvider> shared_image_interface_provider, cc::PaintFlags::FilterQuality filter_quality)
    ...
    , size_(info.width(), info.height())
{
    if (features::IsCanvasSharedBitmapConversionEnabled()) {
        ...
        auto* shared_image_interface = shared_image_interface_provider->SharedImageInterface();
        if (!shared_image_interface) {
            return;
        }

        auto shared_image_mapping = shared_image_interface->CreateSharedImage(
            { viz::SinglePlaneFormat::kBGRA_8888, Size(), gfx::ColorSpace(), gpu::SHARED_IMAGE_USAGE_CPU_WRITE, "CanvasResourceSharedBitmap" });
        ...
        base::MappedReadOnlyRegion shm = viz::bitmap_allocation::AllocateSharedBitmap(Size(), viz::SinglePlaneFormat::kRGBA_8888);
        ...
        CanvasResourceDispatcher* resource_dispatcher = Provider() ? Provider()->ResourceDispatcher() : nullptr;
        if (resource_dispatcher) {
            resource_dispatcher->DidAllocateSharedBitmap(std::move(shm.region), shared_bitmap_id_);
        }
    }
}
...
scoped_refptr<StaticBitmapImage> CanvasResourceSharedBitmap::Bitmap()
{
    ...
    SkImageInfo image_info = SkImageInfo::Make(SkISize::Make(Size().width(), Size().height()), GetSkColorInfo());
    base::span<uint8_t> bytes(shared_mapping_.data(), shared_mapping_.size());
    ...
    sk_sp<SkImage> sk_image = SkImages::RasterFromPixmap(
        pixmap, [](const void*, SkImages::ReleaseContext resource_to_unref) { static_cast<CanvasResourceSharedBitmap*>(resource_to_unref)->Release(); }, this);
    auto image = UnacceleratedStaticBitmapImage::Create(sk_image);
    image->SetOriginClean(is_origin_clean_);
    return image;
}

scoped_refptr<CanvasResourceSharedBitmap> CanvasResourceSharedBitmap::Create(const SkImageInfo& info, base::WeakPtr<CanvasResourceProvider> provider,
    base::WeakPtr<WebGraphicsSharedImageInterfaceProvider> shared_image_interface_provider, cc::PaintFlags::FilterQuality filter_quality)
{
    auto resource = AdoptRef(new CanvasResourceSharedBitmap(info, std::move(provider), std::move(shared_image_interface_provider), filter_quality));
    return resource->IsValid() ? resource : nullptr;
}

bool CanvasResourceSharedBitmap::PrepareUnacceleratedTransferableResource(viz::TransferableResource* out_resource)
{
    TRACE_EVENT0("blink", "CanvasResourceSharedBitmap::PrepareUnacceleratedTransferableResource");
    ...
    if (shared_image_) {
        *out_resource = viz::TransferableResource::MakeSoftwareSharedImage(
            shared_image_, sync_token_, Size(), viz::SinglePlaneFormat::kBGRA_8888, viz::TransferableResource::ResourceSource::kCanvas);
    } else {
        *out_resource = viz::TransferableResource::MakeSoftwareSharedBitmap(
            shared_bitmap_id_, gpu::SyncToken(), Size(), viz::SinglePlaneFormat::kRGBA_8888, viz::TransferableResource::ResourceSource::kCanvas);
    }
...
void CanvasResourceSharedBitmap::TakeSkImage(sk_sp<SkImage> image)
{
    ...
    base::span<uint8_t> bytes(shared_mapping_.data(), shared_mapping_.size());
    CHECK_GE(bytes.size(), image_info.computeByteSize(image_info.minRowBytes()));
    bool read_pixels_successful = image->readPixels(image_info, bytes.data(), image_info.minRowBytes(), 0, 0);
...
CanvasResourceSharedImage::CanvasResourceSharedImage(const SkImageInfo& info, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper,
    base::WeakPtr<CanvasResourceProvider> provider, cc::PaintFlags::FilterQuality filter_quality, bool is_accelerated,
    gpu::SharedImageUsageSet shared_image_usage_flags)
    : CanvasResource(std::move(provider), filter_quality, info.colorInfo())
    , context_provider_wrapper_(std::move(context_provider_wrapper))
    , size_(info.width(), info.height())
    , is_accelerated_(is_accelerated)
    , is_overlay_candidate_(shared_image_usage_flags.Has(gpu::SHARED_IMAGE_USAGE_SCANOUT))
    , supports_display_compositing_(shared_image_usage_flags.Has(gpu::SHARED_IMAGE_USAGE_DISPLAY_READ))
    , use_oop_rasterization_(is_accelerated && context_provider_wrapper_->ContextProvider()->GetCapabilities().gpu_rasterization)
{
    auto* shared_image_interface = context_provider_wrapper_->ContextProvider()->SharedImageInterface();
    ...
    shared_image_usage_flags
        = shared_image_usage_flags | gpu::SHARED_IMAGE_USAGE_RASTER_READ | gpu::SHARED_IMAGE_USAGE_RASTER_WRITE | gpu::SHARED_IMAGE_USAGE_GLES2_READ;
    if (use_oop_rasterization_) {
        shared_image_usage_flags = shared_image_usage_flags | gpu::SHARED_IMAGE_USAGE_OOP_RASTERIZATION;
    } else {
        // The GLES2_WRITE flag is needed due to raster being over GL.
        shared_image_usage_flags = shared_image_usage_flags | gpu::SHARED_IMAGE_USAGE_GLES2_WRITE;
    }
    ...
    if (!is_accelerated_) {
        ...
        client_shared_image
            = shared_image_interface->CreateSharedImage({ GetSharedImageFormat(), Size(), GetColorSpace(), kTopLeft_GrSurfaceOrigin, surface_alpha_type,
                                                            gpu::SharedImageUsageSet(shared_image_usage_flags), "CanvasResourceRasterGmb" },
        ...
        client_shared_image
            = shared_image_interface->CreateSharedImage({ GetSharedImageFormat(), Size(), GetColorSpace(), kTopLeft_GrSurfaceOrigin, surface_alpha_type,
                                                            gpu::SharedImageUsageSet(shared_image_usage_flags), "CanvasResourceRaster" },
    ...
    owning_thread_data().texture_id_for_read_access = raster_interface->CreateAndConsumeForGpuRaster(client_shared_image);

    if (shared_image_usage_flags.Has(gpu::SHARED_IMAGE_USAGE_CONCURRENT_READ_WRITE)) {
        owning_thread_data().texture_id_for_write_access = raster_interface->CreateAndConsumeForGpuRaster(client_shared_image);
    } else {
        owning_thread_data().texture_id_for_write_access = owning_thread_data().texture_id_for_read_access;
    }
}

scoped_refptr<CanvasResourceSharedImage> CanvasResourceSharedImage::Create(const SkImageInfo& info,
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, base::WeakPtr<CanvasResourceProvider> provider,
    cc::PaintFlags::FilterQuality filter_quality, bool is_accelerated, gpu::SharedImageUsageSet shared_image_usage_flags)
{
    TRACE_EVENT0("blink", "CanvasResourceSharedImage::Create");
    auto resource = base::AdoptRef(new CanvasResourceSharedImage(
        info, std::move(context_provider_wrapper), std::move(provider), filter_quality, is_accelerated, shared_image_usage_flags));
    return resource->IsValid() ? resource : nullptr;
}
...
{
    RasterInterface()->BeginSharedImageAccessDirectCHROMIUM(GetTextureIdForWriteAccess(), GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM);
}

void CanvasResourceSharedImage::EndWriteAccess()
{
    RasterInterface()->EndSharedImageAccessDirectCHROMIUM(GetTextureIdForWriteAccess());
}
...
//     texture_info.fFormat = context_provider_wrapper_->ContextProvider()->GetGrGLTextureFormat(GetSharedImageFormat());
//     return GrBackendTextures::MakeGL(Size().width(), Size().height(), skgpu::Mipmapped::kNo, texture_info);
// }
...
CanvasResourceSharedImage::~CanvasResourceSharedImage()
{
    ...
    if (ContextProviderWrapper() && IsValid()) {
        auto* raster_interface = RasterInterface();
        auto* shared_image_interface = ContextProviderWrapper()->ContextProvider()->SharedImageInterface();
        if (raster_interface && shared_image_interface) {
            gpu::SyncToken shared_image_sync_token;
            raster_interface->GenUnverifiedSyncTokenCHROMIUM(shared_image_sync_token.GetData());
            owning_thread_data().client_shared_image->UpdateDestructionSyncToken(shared_image_sync_token);
        }
        if (raster_interface) {
            if (owning_thread_data().texture_id_for_read_access) {
                raster_interface->DeleteGpuRasterTexture(owning_thread_data().texture_id_for_read_access);
            }
            if (owning_thread_data().texture_id_for_write_access
                && owning_thread_data().texture_id_for_write_access != owning_thread_data().texture_id_for_read_access) {
                raster_interface->DeleteGpuRasterTexture(owning_thread_data().texture_id_for_write_access);
            }
        }
    }
...
{
    DCHECK(!is_cross_thread()) << "Write access is only allowed on the owning thread";
...
void CanvasResourceSharedImage::OnBitmapImageDestroyed(
    scoped_refptr<CanvasResourceSharedImage> resource, bool has_read_ref_on_texture, const gpu::SyncToken& sync_token, bool is_lost)
{
    ...
    if (has_read_ref_on_texture) {
        ...
        if (resource->owning_thread_data().bitmap_image_read_refs == 0u && resource->RasterInterface()) {
            resource->RasterInterface()->EndSharedImageAccessDirectCHROMIUM(resource->owning_thread_data().texture_id_for_read_access);
        }
    }

    auto weak_provider = resource->WeakProvider();
    ReleaseFrameResources(std::move(weak_provider), viz::ReleaseCallback(), std::move(resource), sync_token, is_lost);
}
...
//     if (!is_accelerated_) {
//         std::unique_ptr<gpu::ClientSharedImage::ScopedMapping> mapping = GetClientSharedImage()->Map();
//         if (!mapping) {
//             LOG(ERROR) << "MapSharedImage Failed.";
//             return nullptr;
//         }
//         auto sk_image = SkImages::RasterFromPixmapCopy(mapping->GetSkPixmapForPlane(0, CreateSkImageInfo()));
// 
//         // Unmap the underlying buffer.
//         mapping.reset();
//         return sk_image ? UnacceleratedStaticBitmapImage::Create(sk_image) : nullptr;
//     }
// 
...
//     // below.
//     const bool has_read_ref_on_texture = !is_cross_thread() && !use_oop_rasterization_;
...
//         if (owning_thread_data().bitmap_image_read_refs == 1u && RasterInterface()) {
//             RasterInterface()->BeginSharedImageAccessDirectCHROMIUM(texture_id_for_image, GL_SHARED_IMAGE_ACCESS_MODE_READ_CHROMIUM);
//         }
//     }
// 
...
//     auto release_callback = base::BindOnce(&OnBitmapImageDestroyed, scoped_refptr<CanvasResourceSharedImage>(this), has_read_ref_on_texture);
// 
...
//     image = AcceleratedStaticBitmapImage::CreateFromCanvasSharedImage(std::move(client_shared_image), GetSyncToken(), texture_id_for_image, image_info,
//         texture_target, /*is_origin_top_left=*/true, context_provider_wrapper_, owning_thread_ref_, owning_thread_task_runner_, std::move(release_callback),
...
void CanvasResourceSharedImage::CopyRenderingResultsToGpuMemoryBuffer(const sk_sp<SkImage>& image)
{
    *(int*)1 = 1;
...
//         return;
//     }
//     auto* sii = ContextProviderWrapper()->ContextProvider()->SharedImageInterface();
//     std::unique_ptr<gpu::ClientSharedImage::ScopedMapping> mapping = GetClientSharedImage()->Map();
//     if (!mapping) {
//         LOG(ERROR) << "MapSharedImage failed.";
//         return;
//     }
// 
//     auto surface = SkSurfaces::WrapPixels(mapping->GetSkPixmapForPlane(0, CreateSkImageInfo()));
//     SkPixmap pixmap;
...
scoped_refptr<gpu::ClientSharedImage> CanvasResourceSharedImage::GetClientSharedImage()
{
    CHECK(owning_thread_data_.client_shared_image);
    return owning_thread_data_.client_shared_image;
}

const scoped_refptr<gpu::ClientSharedImage>& CanvasResourceSharedImage::GetClientSharedImage() const
{
    CHECK(owning_thread_data_.client_shared_image);
    return owning_thread_data_.client_shared_image;
}

const gpu::SyncToken CanvasResourceSharedImage::GetSyncTokenWithOptionalVerification(bool needs_verified_token)
{
    if (is_cross_thread()) {
    ...
    if (mailbox_needs_new_sync_token()) {
        ...
        DCHECK(raster_interface); // caller should already have early exited if
            // !raster_interface.
        raster_interface->GenUnverifiedSyncTokenCHROMIUM(owning_thread_data().sync_token.GetData());
        owning_thread_data().mailbox_needs_new_sync_token = false;
    }

    if (needs_verified_token && !owning_thread_data().sync_token.verified_flush()) {
...
base::WeakPtr<WebGraphicsContext3DProviderWrapper> CanvasResourceSharedImage::ContextProviderWrapper() const
{
    DCHECK(!is_cross_thread());
    return context_provider_wrapper_;
}

void CanvasResourceSharedImage::OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, const std::string& parent_path, size_t bytes_per_pixel) const
{
    if (!IsValid())
        return;

    std::string dump_name = base::StringPrintf("%s/CanvasResource_0x%" PRIXPTR, parent_path.c_str(), reinterpret_cast<uintptr_t>(this));
    auto* dump = pmd->CreateAllocatorDump(dump_name);
    size_t memory_size = Size().height() * Size().width() * bytes_per_pixel;
    dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, memory_size);

    auto guid = GetClientSharedImage()->GetGUIDForTracing();
    pmd->CreateSharedGlobalAllocatorDump(guid);
    pmd->AddOwnershipEdge(dump->guid(), guid, static_cast<int>(gpu::TracingImportance::kClientOwner));
}

// ExternalCanvasResource
//==============================================================================
scoped_refptr<ExternalCanvasResource> ExternalCanvasResource::Create(scoped_refptr<gpu::ClientSharedImage> client_si,
    const viz::TransferableResource& transferable_resource, viz::ReleaseCallback release_callback,
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, base::WeakPtr<CanvasResourceProvider> provider,
    cc::PaintFlags::FilterQuality filter_quality, bool is_origin_top_left)
{
    TRACE_EVENT0("blink", "ExternalCanvasResource::Create");
    CHECK(client_si);
    CHECK(client_si->mailbox() == transferable_resource.mailbox());
    auto resource = AdoptRef(new ExternalCanvasResource(std::move(client_si), transferable_resource, std::move(release_callback),
        std::move(context_provider_wrapper), std::move(provider), filter_quality, is_origin_top_left));
    return resource->IsValid() ? resource : nullptr;
}
...
//         [](scoped_refptr<ExternalCanvasResource> resource, const gpu::SyncToken& sync_token, bool is_lost) {
//             // Do nothing but hold onto the refptr.
//         },
//         base::RetainedRef(this));
// 
//     return AcceleratedStaticBitmapImage::CreateFromCanvasSharedImage(client_si_, GetSyncToken(), /*shared_image_texture_id=*/0u, CreateSkImageInfo(),
//         transferable_resource_.texture_target(), is_origin_top_left_, context_provider_wrapper_, owning_thread_ref_, owning_thread_task_runner_,
//         std::move(release_callback),
//         /*supports_display_compositing=*/true, transferable_resource_.is_overlay_candidate);
}

const gpu::SyncToken ExternalCanvasResource::GetSyncTokenWithOptionalVerification(bool needs_verified_token)
{
    GenOrFlushSyncToken();
...
base::WeakPtr<WebGraphicsContext3DProviderWrapper> ExternalCanvasResource::ContextProviderWrapper() const
{
...
bool ExternalCanvasResource::PrepareAcceleratedTransferableResourceWithoutClientSI(viz::TransferableResource* out_resource)
{
    TRACE_EVENT0("blink", "ExternalCanvasResource::PrepareAcceleratedTransferableResource");
    GenOrFlushSyncToken();
    *out_resource = transferable_resource_;
    return true;
}

ExternalCanvasResource::ExternalCanvasResource(scoped_refptr<gpu::ClientSharedImage> client_si, const viz::TransferableResource& transferable_resource,
    viz::ReleaseCallback out_callback, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper,
    base::WeakPtr<CanvasResourceProvider> provider, cc::PaintFlags::FilterQuality filter_quality, bool is_origin_top_left)
    : CanvasResource(std::move(provider), filter_quality,
        SkColorInfo(viz::ToClosestSkColorType(/*gpu_compositing=*/true, transferable_resource.format), kPremul_SkAlphaType,
...
//==============================================================================
scoped_refptr<CanvasResourceSwapChain> CanvasResourceSwapChain::Create(const SkImageInfo& info,
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, base::WeakPtr<CanvasResourceProvider> provider,
    cc::PaintFlags::FilterQuality filter_quality)
{
    TRACE_EVENT0("blink", "CanvasResourceSwapChain::Create");
    auto resource = AdoptRef(new CanvasResourceSwapChain(info, std::move(context_provider_wrapper), std::move(provider), filter_quality));
    return resource->IsValid() ? resource : nullptr;
}
...
CanvasResourceSwapChain::~CanvasResourceSwapChain()
{
    ...
    if (!use_oop_rasterization_) {
        auto* raster_interface = context_provider_wrapper_->ContextProvider()->RasterInterface();
        DCHECK(raster_interface);
...
//         base::RetainedRef(this));
// 
//     return AcceleratedStaticBitmapImage::CreateFromCanvasSharedImage(back_buffer_shared_image_, GetSyncToken(), shared_texture_id, image_info,
//         back_buffer_shared_image_->GetTextureTarget(), true /*is_origin_top_left*/, context_provider_wrapper_, owning_thread_ref_, owning_thread_task_runner_,
//         std::move(release_callback), /*supports_display_compositing=*/true,
//         /*is_overlay_candidate=*/true);
}

scoped_refptr<gpu::ClientSharedImage> CanvasResourceSwapChain::GetClientSharedImage()
{
    return front_buffer_shared_image_;
}

const gpu::SyncToken CanvasResourceSwapChain::GetSyncTokenWithOptionalVerification(bool needs_verified_token)
{
...
void CanvasResourceSwapChain::PresentSwapChain()
{
    ...
    auto* sii = context_provider_wrapper_->ContextProvider()->SharedImageInterface();
    DCHECK(sii);
    ...
    raster_interface->CopySharedImage(front_buffer_shared_image_->mailbox(), back_buffer_shared_image_->mailbox(), 0, 0, 0, 0, size_.width(), size_.height());
...
base::WeakPtr<WebGraphicsContext3DProviderWrapper> CanvasResourceSwapChain::ContextProviderWrapper() const
{
    return context_provider_wrapper_;
}

CanvasResourceSwapChain::CanvasResourceSwapChain(const SkImageInfo& info, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper,
    base::WeakPtr<CanvasResourceProvider> provider, cc::PaintFlags::FilterQuality filter_quality)
    : CanvasResource(std::move(provider), filter_quality, info.colorInfo())
    , context_provider_wrapper_(std::move(context_provider_wrapper))
    , size_(info.width(), info.height())
    , use_oop_rasterization_(context_provider_wrapper_->ContextProvider()->GetCapabilities().gpu_rasterization)
{
    if (!context_provider_wrapper_)
        return;
    ...
    // Hence, GLES2_READ usage is necessary regardless of whether raster is over
    // GLES.
    gpu::SharedImageUsageSet usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_GLES2_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT
    ...
    gpu::SharedImageInterface::SwapChainSharedImages shared_images
        = sii->CreateSwapChain(GetSharedImageFormat(), Size(), GetColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage);
