...
public:
    explicit DarkModeImageClassifier(DarkModeImageClassifierPolicy image_classifier_policy);
...
private:
    ...
    std::optional<Features> GetFeatures(const SkPixmap& pixmap, const SkIRect& src) const;
    // Extracts a sample set of pixels (|sampled_pixels|), |transparency_ratio|,
    // and |background_ratio|.
    void GetSamples(const SkPixmap& pixmap, const SkIRect& src, std::vector<SkColor>* sampled_pixels, float* transparency_ratio, float* background_ratio) const;
    ...
    void GetBlockSamples(const SkPixmap& pixmap, const SkIRect& block, const int required_samples_count, std::vector<SkColor>* sampled_pixels,
    ...
    Features ComputeFeatures(const std::vector<SkColor>& sampled_pixels, const float transparency_ratio, const float background_ratio) const;
    ...
    float ComputeColorBucketsRatio(const std::vector<SkColor>& sampled_pixels, const ColorMode color_mode) const;
    ...
    FRIEND_TEST_ALL_PREFIXES(DarkModeImageClassifierTest, FeaturesAndClassification);
};
