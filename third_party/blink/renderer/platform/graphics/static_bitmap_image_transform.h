...
public:
    ...
    static scoped_refptr<StaticBitmapImage> Apply(FlushReason, scoped_refptr<StaticBitmapImage> image, const Params& params);

    // Create a copy of the input image, with a newly created backing.
    static scoped_refptr<StaticBitmapImage> Clone(FlushReason, scoped_refptr<StaticBitmapImage> image);

    // If `image` has unpremultiplied alpha, the multipl alpha. If `image` is
    // opaque or already premultiplied, return `image.
    static scoped_refptr<StaticBitmapImage> GetWithAlphaDisposition(FlushReason, scoped_refptr<StaticBitmapImage> image, AlphaDisposition);

    // Convert `image` to the specified color space.
    static scoped_refptr<StaticBitmapImage> ConvertToColorSpace(FlushReason, scoped_refptr<StaticBitmapImage> image, sk_sp<SkColorSpace> color_space);
...
private:
    ...
    static scoped_refptr<StaticBitmapImage> ApplyUsingPixmap(scoped_refptr<StaticBitmapImage> image, const Params& params);
    ...
    static scoped_refptr<StaticBitmapImage> ApplyWithBlit(FlushReason, scoped_refptr<StaticBitmapImage> image, const Params& params);
};
