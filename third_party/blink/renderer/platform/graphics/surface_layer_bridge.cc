...
SurfaceLayerBridge::SurfaceLayerBridge(viz::FrameSinkId parent_frame_sink_id, ContainsVideo contains_video, WebSurfaceLayerBridgeObserver* observer,
...
{
    Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(embedded_frame_sink_provider_.BindNewPipeAndPassReceiver());
    ...
    embedded_frame_sink_provider_->RegisterEmbeddedFrameSink(parent_frame_sink_id_, frame_sink_id_, receiver_.BindNewPipeAndPassRemote());
}
...
void SurfaceLayerBridge::SetLocalSurfaceId(const viz::LocalSurfaceId& local_surface_id)
{
...
void SurfaceLayerBridge::EmbedSurface(const viz::SurfaceId& surface_id)
{
    ...
    surface_layer_->SetSurfaceId(surface_id, cc::DeadlinePolicy::UseSpecifiedDeadline(0u));
...
void SurfaceLayerBridge::BindSurfaceEmbedder(mojo::PendingReceiver<mojom::blink::SurfaceEmbedder> receiver)
{
...
void SurfaceLayerBridge::CreateSurfaceLayer()
{
    ...
    current_surface_id_ = viz::SurfaceId(frame_sink_id_, parent_local_surface_id_allocator_.GetCurrentLocalSurfaceId());

    surface_layer_->SetSurfaceId(current_surface_id_, cc::DeadlinePolicy::UseDefaultDeadline());
...
void SurfaceLayerBridge::UpdateSurfaceLayerOpacity()
{
    ...
    surface_layer_->SetContentsOpaque(embedder_expects_opaque_ && frames_are_opaque_);
}
