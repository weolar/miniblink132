...
PLATFORM_EXPORT std::unique_ptr<WebGraphicsContext3DProvider> CreateOffscreenGraphicsContext3DProvider(
...
// Note if this method is not called on the main thread it will block waiting
// for the main thread to allocated an offscreen context provider.
PLATFORM_EXPORT std::unique_ptr<WebGraphicsContext3DProvider> CreateWebGPUGraphicsContext3DProvider(const KURL& url);

// Asynchronously creates a WebGPUGraphicsContext3DProvider on any thread.
PLATFORM_EXPORT void CreateWebGPUGraphicsContext3DProviderAsync(const KURL& url, scoped_refptr<base::SingleThreadTaskRunner> current_thread_task_runner,
