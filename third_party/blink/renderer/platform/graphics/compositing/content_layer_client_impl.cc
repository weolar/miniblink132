...
bool DrawingShouldFillScrollingContentsLayer(const PropertyTreeState& layer_state, const cc::PictureLayer& layer)
{
    ...
    if (const auto* scroll_node = layer_state.Transform().ScrollNode()) {
        ...
        return layer.bounds().width() >= scroll_node->ContentsRect().width() && layer.bounds().height() >= scroll_node->ContentsRect().height();
    }
    return false;
}
...
void ContentLayerClientImpl::AppendAdditionalInfoAsJSON(LayerTreeFlags flags, const cc::Layer& layer, JSONObject& json) const
{
    ...
    if ((flags & (kLayerTreeIncludesInvalidations | kLayerTreeIncludesDetailedInvalidations)) && raster_invalidator_->GetTracking()) {
        raster_invalidator_->GetTracking()->AsJSON(&json, flags & kLayerTreeIncludesDetailedInvalidations);
    }
    ...
    if (flags & kLayerTreeIncludesPaintRecords) {
        ...
        for (auto& [scroll_element_id, _] : cc_display_item_list_->raster_inducing_scrolls()) {
            raster_inducing_scroll_offsets[scroll_element_id]
                = layer.layer_tree_host()->property_trees()->scroll_tree().current_scroll_offset(scroll_element_id);
        }
        cc_display_item_list_->Raster(&canvas, /*image_provider=*/nullptr, &raster_inducing_scroll_offsets);
        json.SetValue("paintRecord", canvas.Log());
    }
#endif
}

void ContentLayerClientImpl::UpdateCcPictureLayer(const PendingLayer& pending_layer)
{
    ...
    for (auto it = paint_chunks.begin(); it != paint_chunks.end(); ++it) {
        ...
        json->SetArray("displayItems",
            DisplayItemList::DisplayItemsAsJSON(paint_chunks.GetPaintArtifact(), it->begin_index, it.DisplayItems(), DisplayItemList::kCompact));
    ...
    if (is_mask_layer) {
        AdjustMaskLayerGeometry(pending_layer.GetPropertyTreeState().Transform(), layer_offset, layer_bounds);
    }
    ...
    std::optional<RasterUnderInvalidationCheckingParams> raster_under_invalidation_params;
    if (RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled()) {
        raster_under_invalidation_params.emplace(
            *raster_invalidator_->GetTracking(), gfx::Rect(layer_bounds), paint_chunks.GetPaintArtifact().ClientDebugName(paint_chunks[0].id.client_id));
    }
    ...
    bool may_be_unchanged = cc_display_item_list_ && layer_bounds == old_layer_bounds && cc_picture_layer_->draws_content() == pending_layer.DrawsContent()
    ...
    PaintChunksToCcLayer::ConvertInto(paint_chunks, layer_state, layer_offset, base::OptionalToPtr(raster_under_invalidation_params), *cc_display_item_list_);

    if (RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        if ((had_raster_inducing_scroll || HasRasterInducingScroll()) && only_empty_invalidations_) {
            // See InvalidateRect().
    ...
    if (is_mask_layer || DrawingShouldFillScrollingContentsLayer(layer_state, *cc_picture_layer_)) {
    ...
    bool contents_opaque =
        ...
        && pending_layer.RectKnownToBeOpaque().Contains(
            gfx::RectF(gfx::PointAtOffsetFromOrigin(pending_layer.LayerOffset()), gfx::SizeF(pending_layer.LayerBounds())));
    cc_picture_layer_->SetContentsOpaque(contents_opaque);
    if (!contents_opaque) {
        cc_picture_layer_->SetContentsOpaqueForText(cc_display_item_list_->has_draw_text_ops() && pending_layer.TextKnownToBeOnOpaqueBackground());
    }
}

bool ContentLayerClientImpl::HasRasterInducingScroll() const
{
    return cc_display_item_list_ && !cc_display_item_list_->raster_inducing_scrolls().empty();
}
...
{
    return sizeof(*this) + raster_invalidator_->ApproximateUnsharedMemoryUsage() - sizeof(raster_invalidator_);
}
