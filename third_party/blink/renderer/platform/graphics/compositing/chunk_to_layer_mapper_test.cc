...
protected:
    ...
    {
        DEFINE_STATIC_LOCAL(Persistent<FakeDisplayItemClient>, fake_client, (MakeGarbageCollected<FakeDisplayItemClient>()));
        DEFINE_STATIC_LOCAL(std::optional<PaintChunk::Id>, id, (PaintChunk::Id(fake_client->Id(), DisplayItem::kDrawingFirst)));
    ...
    {
        if (!layer_transform_) {
            layer_transform_ = CreateTransform(t0(), MakeTranslationMatrix(123, 456), gfx::Point3F(1, 2, 3));
            layer_clip_ = CreateClip(c0(), *layer_transform_, FloatRoundedRect(12, 34, 56, 78));
            layer_effect_ = EffectPaintPropertyNode::Create(
                e0(), EffectPaintPropertyNode::State { layer_transform_, layer_clip_, CompositorFilterOperations(), nullptr, 0.789f, SkBlendMode::kSrcIn });
        }
...
TEST_F(ChunkToLayerMapperTest, OneChunkUsingLayerState)
{
    ...
    EXPECT_EQ(gfx::Rect(20, 10, 88, 99), mapper.MapVisualRect(gfx::Rect(30, 30, 88, 99)));
...
{
    ChunkToLayerMapper mapper(LayerState(), gfx::Vector2dF(10, 20));
    auto* transform = CreateTransform(LayerState().Transform(), MakeScaleMatrix(2));
    auto* clip = CreateClip(LayerState().Clip(), LayerState().Transform(), FloatRoundedRect(10, 10, 100, 100));
    ...
    EXPECT_EQ(gfx::Rect(50, 40, 50, 50), mapper.MapVisualRect(gfx::Rect(30, 30, 88, 99)));
    EXPECT_EQ(gfx::Rect(), mapper.MapVisualRect(gfx::Rect()));
...
{
    ChunkToLayerMapper mapper(LayerState(), gfx::Vector2dF(10, 20));
    auto* transform1 = CreateTransform(LayerState().Transform(), MakeScaleMatrix(2));
    auto* clip1 = CreateClip(LayerState().Clip(), LayerState().Transform(), FloatRoundedRect(10, 10, 100, 100));
    auto& effect = LayerState().Effect();
    auto chunk1 = Chunk(PropertyTreeState(*transform1, *clip1, effect));

    auto* transform2 = Create2DTranslation(*transform1, 20, 30);
    auto* clip2 = CreateClip(LayerState().Clip(), *transform2, FloatRoundedRect(0, 0, 20, 20));
    ...
    EXPECT_EQ(gfx::Rect(30, 40, 40, 40), mapper.MapVisualRect(gfx::Rect(0, 0, 200, 200)));
    EXPECT_EQ(gfx::Rect(), mapper.MapVisualRect(gfx::Rect()));
}
...
TEST_F(ChunkToLayerMapperTest, SlowPath)
{
    ...
    auto* clip_expander = CreatePixelMovingFilterClipExpander(LayerState().Clip(), *effect2);
    auto chunk2 = Chunk(PropertyTreeState(LayerState().Transform(), *clip_expander, *effect2));
    ...
    auto chunk3 = Chunk(PropertyTreeState(LayerState().Transform(), *clip_expander, *effect3));
    ...
    auto chunk4 = Chunk(PropertyTreeState(LayerState().Transform(), LayerState().Clip(), *effect4));
    ...
    EXPECT_EQ(gfx::Rect(-40, -50, 208, 219), mapper.MapVisualRect(gfx::Rect(30, 30, 88, 99)));
    EXPECT_EQ(gfx::Rect(), mapper.MapVisualRect(gfx::Rect()));
...
TEST_F(ChunkToLayerMapperTest, SwitchToSiblingEffect)
{
    ...
    auto chunk1 = Chunk(PropertyTreeState(LayerState().Transform(), LayerState().Clip(), *effect1));
    auto* effect2 = CreateOpacityEffect(LayerState().Effect(), 0.5f);
    auto chunk2 = Chunk(PropertyTreeState(LayerState().Transform(), LayerState().Clip(), *effect2));

    ChunkToLayerMapper mapper(chunk1.properties.Unalias(), gfx::Vector2dF(10, 20));
    mapper.SwitchToChunk(chunk2);
    EXPECT_FALSE(HasFilterThatMovesPixels(mapper));
}
