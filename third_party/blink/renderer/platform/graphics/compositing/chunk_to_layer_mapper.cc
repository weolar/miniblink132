...
ChunkToLayerMapper::ChunkToLayerMapper(const PropertyTreeState& layer_state, const gfx::Vector2dF& layer_offset)
...
void ChunkToLayerMapper::SwitchToChunkWithState(const PaintChunk& chunk, const PropertyTreeState& new_chunk_state)
{
    ...
    if (&new_chunk_state.Transform() != &chunk_state_.Transform()) {
        transform_ = GeometryMapper::SourceToDestinationProjection(new_chunk_state.Transform(), layer_state_.Transform());
        transform_.PostTranslate(-layer_offset_);
    }

    has_filter_that_moves_pixels_ = new_chunk_state.Clip().NearestPixelMovingFilterClip() != layer_state_.Clip().NearestPixelMovingFilterClip();

    if (has_filter_that_moves_pixels_) {
        clip_rect_ = InfiniteLooseFloatClipRect();
    } else if (&new_chunk_state.Clip() != &chunk_state_.Clip()) {
        clip_rect_ = GeometryMapper::LocalToAncestorClipRect(new_chunk_state, layer_state_);
...
gfx::Rect ChunkToLayerMapper::MapVisualRect(const gfx::Rect& rect) const
{
    ...
    if (result != slow_result) {
        ...
        LOG(WARNING) << "ChunkToLayerMapper::MapVisualRect: Different results from"
                     << "fast path (" << result.ToString() << ") and slow path (" << slow_result.ToString() << ")";
    }
#endif
    return result;
}
...
gfx::Rect ChunkToLayerMapper::MapUsingGeometryMapper(const gfx::Rect& rect) const
{
    return MapVisualRectFromState(rect, chunk_state_);
}

gfx::Rect ChunkToLayerMapper::MapVisualRectFromState(const gfx::Rect& rect, const PropertyTreeState& state) const
{
