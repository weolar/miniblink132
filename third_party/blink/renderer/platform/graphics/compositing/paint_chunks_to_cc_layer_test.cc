...
RegionCaptureData* MakeRegionCaptureData(std::initializer_list<std::pair<RegionCaptureCropId, gfx::Rect>> map_values)
{
...
class PaintChunksToCcLayerTest : public testing::Test, public PaintTestConfigurations { };

INSTANTIATE_PAINT_TEST_SUITE_P(PaintChunksToCcLayerTest);

const DisplayItemClient& DefaultClient()
{
    DEFINE_STATIC_LOCAL(Persistent<FakeDisplayItemClient>, fake_client, (MakeGarbageCollected<FakeDisplayItemClient>("FakeDisplayItemClient")));
    return *fake_client;
}
...
void UpdateLayerProperties(cc::Layer& layer, const PropertyTreeState& layer_state, const PaintChunkSubset& chunks)
{
    cc::LayerSelection layer_selection;
    PaintChunksToCcLayer::UpdateLayerProperties(layer, layer_state, chunks, layer_selection, /*selection_only=*/false);
}

class TestChunks {
public:
    // Add a paint chunk with a non-empty paint record and given property nodes.
    void AddChunk(const TransformPaintPropertyNodeOrAlias& t, const ClipPaintPropertyNodeOrAlias& c, const EffectPaintPropertyNodeOrAlias& e,
        const gfx::Rect& bounds = gfx::Rect(0, 0, 100, 100), const std::optional<gfx::Rect>& drawable_bounds = std::nullopt)
    {
        AddChunk(PropertyTreeStateOrAlias(t, c, e), bounds, drawable_bounds);
    }
    void AddChunk(const PropertyTreeStateOrAlias& state, const gfx::Rect& bounds = gfx::Rect(0, 0, 100, 100),
        const std::optional<gfx::Rect>& drawable_bounds = std::nullopt)
    {
        cc::PaintOpBuffer buffer;
        buffer.push<cc::DrawRectOp>(gfx::RectToSkRect(drawable_bounds.value_or(bounds)), cc::PaintFlags());
    ...
    void AddChunk(PaintRecord record, const TransformPaintPropertyNodeOrAlias& t, const ClipPaintPropertyNodeOrAlias& c,
        const EffectPaintPropertyNodeOrAlias& e, const gfx::Rect& bounds = gfx::Rect(0, 0, 100, 100),
        const std::optional<gfx::Rect>& drawable_bounds = std::nullopt)
    {
        AddChunk(record, PropertyTreeStateOrAlias(t, c, e), bounds, drawable_bounds);
    }
    void AddChunk(PaintRecord record, const PropertyTreeStateOrAlias& state, const gfx::Rect& bounds = gfx::Rect(0, 0, 100, 100),
        const std::optional<gfx::Rect>& drawable_bounds = std::nullopt)
    {
        auto& items = paint_artifact_->GetDisplayItemList();
        auto i = items.size();
        items.AllocateAndConstruct<DrawingDisplayItem>(
            DefaultId().client_id, DefaultId().type, drawable_bounds ? *drawable_bounds : bounds, std::move(record), RasterEffectOutset::kNone);
    ...
    void AddEmptyChunk(const TransformPaintPropertyNodeOrAlias& t, const ClipPaintPropertyNodeOrAlias& c, const EffectPaintPropertyNodeOrAlias& e,
        const gfx::Rect& bounds = gfx::Rect(0, 0, 100, 100))
    {
        AddEmptyChunk(PropertyTreeStateOrAlias(t, c, e), bounds);
    }
    void AddEmptyChunk(const PropertyTreeStateOrAlias& state, const gfx::Rect& bounds = gfx::Rect(0, 0, 100, 100))
    {
        auto& chunks = paint_artifact_->GetPaintChunks();
    ...
    PaintChunks& GetChunks()
    {
        return paint_artifact_->GetPaintChunks();
    }

    PaintChunkSubset Build()
    {
        return PaintChunkSubset(*paint_artifact_);
    }

private:
    Persistent<PaintArtifact> paint_artifact_ = MakeGarbageCollected<PaintArtifact>();
};
...
TEST_P(PaintChunksToCcLayerTest, EffectGroupingSimple)
{
    ...
    PaintRecord output = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState::Root());
    EXPECT_THAT(output,
...
TEST_P(PaintChunksToCcLayerTest, EffectGroupingNested)
{
    ...
    PaintRecord output = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState::Root());
    EXPECT_THAT(output,
        ElementsAre(PaintOpEq<cc::SaveLayerAlphaOp>(SkRect::MakeXYWH(0, 0, 444, 666),
                        0.5f), // <e1>
...
TEST_P(PaintChunksToCcLayerTest, EffectFilterGroupingNestedWithTransforms)
{
    ...
    expected_flags.setImageFilter(cc::RenderSurfaceFilters::BuildImageFilter(filter.AsCcFilterOperations()));
    EXPECT_THAT(output,
        ElementsAre(PaintOpIs<cc::SaveOp>(),
            PaintOpEq<cc::ConcatOp>(gfx::TransformToSkM44(t1->Matrix() * t2->Matrix())), // <t1*t2>
    ...
    EXPECT_THAT(output,
        ElementsAre(PaintOpIs<cc::SaveOp>(),
            ...
            PaintOpEq<cc::SaveLayerAlphaOp>(SkRect::MakeXYWH(0, 0, 100, 100),
                0.5f), // <e1>
            PaintOpIs<cc::SaveOp>(),
            PaintOpEq<cc::ConcatOp>(gfx::TransformToSkM44(t1->Matrix().GetCheckedInverse())), // <t1^-1>
            PaintOpIs<cc::DrawRecordOp>(), // <p0/>
            PaintOpIs<cc::RestoreOp>(), // </t1^-1>
            PaintOpIs<cc::DrawRecordOp>(), // <p1/>
            PaintOpIs<cc::RestoreOp>(), // </e1>
...
TEST_P(PaintChunksToCcLayerTest, FilterEffectSpaceInversion)
{
    ...
    auto output = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState::Root());
...
TEST_P(PaintChunksToCcLayerTest, NonRootLayerSimple)
{
    ...
    PaintRecord output = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState(*t1, *c1, *e1));
...
TEST_P(PaintChunksToCcLayerTest, EffectWithNoOutputClip)
{
    ...
    PaintRecord output = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState(t0(), *c1, e0()));
    EXPECT_THAT(output,
        ElementsAre(PaintOpEq<cc::SaveLayerAlphaOp>(SkRect::MakeXYWH(0, 0, 100, 100),
                        0.5f), // <e1>
            PaintOpIs<cc::SaveOp>(),
...
TEST_P(PaintChunksToCcLayerTest, EffectWithNoOutputClipNestedInDecompositedEffect)
{
    auto* c1 = CreateClip(c0(), t0(), FloatRoundedRect(0.f, 0.f, 1.f, 1.f));
...
TEST_P(PaintChunksToCcLayerTest, EffectWithNoOutputClipNestedInCompositedEffect)
{
    auto* c1 = CreateClip(c0(), t0(), FloatRoundedRect(0.f, 0.f, 1.f, 1.f));
    auto* e1 = CreateOpacityEffect(e0(), 0.5);
    auto* e2 = CreateOpacityEffect(*e1, t0(), nullptr, 0.5);
    ...
    PaintRecord output = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState(t0(), c0(), *e1));
    EXPECT_THAT(output,
        ElementsAre(PaintOpEq<cc::SaveLayerAlphaOp>(SkRect::MakeXYWH(0, 0, 100, 100),
                        0.5f), // <e2>
            PaintOpIs<cc::SaveOp>(),
...
TEST_P(PaintChunksToCcLayerTest, EffectWithNoOutputClipNestedInCompositedEffectAndClip)
{
    auto* c1 = CreateClip(c0(), t0(), FloatRoundedRect(0.f, 0.f, 1.f, 1.f));
    auto* e1 = CreateOpacityEffect(e0(), 0.5);
    auto* e2 = CreateOpacityEffect(*e1, t0(), nullptr, 0.5);
    ...
    PaintRecord output = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState(t0(), *c1, *e1));
    EXPECT_THAT(output,
        ElementsAre(PaintOpEq<cc::SaveLayerAlphaOp>(SkRect::MakeXYWH(0, 0, 100, 100),
                        0.5f), // <e2>
            PaintOpIs<cc::DrawRecordOp>(), // <p0/>
...
TEST_P(PaintChunksToCcLayerTest, VisualRect)
{
    ...
    PaintChunksToCcLayer::ConvertInto(chunks.Build(), PropertyTreeState(*layer_transform, c0(), e0()), gfx::Vector2dF(100, 200), nullptr, *cc_list);
...
TEST_P(PaintChunksToCcLayerTest, CombineClipsAcrossTransform)
{
    ...
    EXPECT_THAT(output,
        ElementsAre(PaintOpIs<cc::SaveOp>(),
            PaintOpEq<cc::ClipRectOp>(SkRect::MakeXYWH(50, 50, 50, 50), SkClipOp::kIntersect,
                /*antialias=*/true), // <c1+c2>
            PaintOpIs<cc::SaveOp>(),
            PaintOpEq<cc::ConcatOp>(gfx::TransformToSkM44(non_identity->Matrix())), // <non_identity
            PaintOpEq<cc::ClipRectOp>(SkRect::MakeXYWH(1, 2, 3, 4), SkClipOp::kIntersect,
                /*antialias=*/true), //  c3>
            PaintOpIs<cc::SaveOp>(),
            PaintOpEq<cc::ConcatOp>(gfx::TransformToSkM44(non_invertible->Matrix())), // <non_invertible
            PaintOpEq<cc::ClipRectOp>(SkRect::MakeXYWH(5, 6, 7, 8), SkClipOp::kIntersect,
                /*antialias=*/true), //  c4>
            PaintOpIs<cc::DrawRecordOp>(), // <p0/>
...
TEST_P(PaintChunksToCcLayerTest, CombineClipsWithRoundedRects)
{
    ...
    EXPECT_THAT(output,
        ElementsAre(PaintOpIs<cc::SaveOp>(),
            PaintOpEq<cc::ClipRRectOp>(SkRRect(small_rounded_rect), SkClipOp::kIntersect,
                /*antialias=*/true), // <c1+c2+c3>
            PaintOpIs<cc::SaveOp>(),
            PaintOpEq<cc::ClipRRectOp>(SkRRect(big_rounded_rect), SkClipOp::kIntersect,
                /*antialias=*/true), // <c4>
            PaintOpIs<cc::SaveOp>(),
            PaintOpEq<cc::ClipRectOp>(gfx::RectFToSkRect(rect.Rect()), SkClipOp::kIntersect,
                /*antialias=*/true), // <c5>
            PaintOpIs<cc::SaveOp>(),
            PaintOpEq<cc::ClipRRectOp>(SkRRect(big_rounded_rect), SkClipOp::kIntersect,
                /*antialias=*/true), // <c6>
...
TEST_P(PaintChunksToCcLayerTest, OnlyNoopTransformIsNotEmitted)
{
    ...
    EXPECT_THAT(output, ElementsAre(PaintOpIs<cc::DrawRecordOp>(), PaintOpIs<cc::DrawRecordOp>(), PaintOpIs<cc::DrawRecordOp>()));
}
...
static sk_sp<cc::PaintFilter> MakeFilter(gfx::RectF bounds)
{
    ...
    return sk_make_sp<ColorFilterPaintFilter>(cc::ColorFilter::MakeBlend(SkColors::kBlue, SkBlendMode::kSrc), nullptr, &rect);
}
...
TEST_P(PaintChunksToCcLayerTest, ReferenceFilterOnEmptyChunk)
{
    ...
    auto cc_list = base::MakeRefCounted<cc::DisplayItemList>();
    PaintChunksToCcLayer::ConvertInto(chunks.Build(), PropertyTreeState::Root(), gfx::Vector2dF(5, 10), nullptr, *cc_list);
...
TEST_P(PaintChunksToCcLayerTest, ReferenceFilterOnChunkWithDrawingDisplayItem)
{
    ...
    TestChunks chunks;
    chunks.AddChunk(t0(), *clip_expander, *e1, gfx::Rect(5, 10, 200, 300), gfx::Rect(10, 15, 20, 30));
...
{
    auto scroll_state = CreateScrollTranslationState(PropertyTreeState::Root(), -50, -60, gfx::Rect(5, 5, 20, 30), gfx::Size(100, 200));
    TestChunks chunks;
    chunks.AddChunk(t0(), c0(), e0());
    ...
    EXPECT_THAT(output,
        ElementsAre(PaintOpIs<cc::DrawRecordOp>(), // chunk 0
            PaintOpIs<cc::SaveOp>(),
            PaintOpEq<cc::ClipRectOp>(SkRect::MakeXYWH(5, 5, 20, 30), SkClipOp::kIntersect,
                /*antialias=*/true), // <overflow-clip>
            PaintOpIs<cc::SaveOp>(), PaintOpEq<cc::TranslateOp>(-50, -60), // <scroll-translation>
...
TEST_P(PaintChunksToCcLayerTest, ScrollingContentsIntoDisplayItemList)
{
    ...
    auto cc_list = base::MakeRefCounted<cc::DisplayItemList>();
    PaintChunksToCcLayer::ConvertInto(chunks.Build(), PropertyTreeState::Root(), gfx::Vector2dF(), nullptr, *cc_list);
    ...
    if (RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        ...
        EXPECT_EQ(gfx::Rect(5, 5, 20, 30), cc_list->raster_inducing_scrolls().at(scroll_state.Transform().ScrollNode()->GetCompositorElementId()).visual_rect);
        const auto& scrolling_contents_op = static_cast<const cc::DrawScrollingContentsOp&>(cc_list->paint_op_buffer().GetOpAtForTesting(3));
        ASSERT_EQ(cc::PaintOpType::kDrawScrollingContents, scrolling_contents_op.GetType());
...
TEST_P(PaintChunksToCcLayerTest, ScrollingContentsIntoDisplayItemListOverflowClipInLayerState)
{
    if (!RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        GTEST_SKIP();
    }
    ...
    auto cc_list = base::MakeRefCounted<cc::DisplayItemList>();
    PaintChunksToCcLayer::ConvertInto(chunks.Build(), layer_state, gfx::Vector2dF(), nullptr, *cc_list);

    EXPECT_THAT(cc_list->paint_op_buffer(),
        ElementsAre(PaintOpIs<cc::DrawRecordOp>(), // chunk 0
            PaintOpIs<cc::DrawScrollingContentsOp>(),
            PaintOpIs<cc::DrawRecordOp>())); // chunk 2
    const auto& scrolling_contents_op = static_cast<const cc::DrawScrollingContentsOp&>(cc_list->paint_op_buffer().GetOpAtForTesting(1));
    ASSERT_EQ(cc::PaintOpType::kDrawScrollingContents, scrolling_contents_op.GetType());
    EXPECT_THAT(scrolling_contents_op.display_item_list->paint_op_buffer(),
        ElementsAre(PaintOpIs<cc::DrawRecordOp>())); // chunk 1
}

TEST_P(PaintChunksToCcLayerTest, ScrollingContentsIntoDisplayItemListOverflowClipInLayerStateWithEffect)
{
    if (!RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        GTEST_SKIP();
    }
    ...
    auto scroll_state = CreateScrollTranslationState(PropertyTreeState(t0(), c0(), *effect), -50, -60, gfx::Rect(5, 5, 20, 30), gfx::Size(100, 200));
    TestChunks chunks;
    PropertyTreeState layer_state(t0(), scroll_state.Clip(), e0());
    chunks.AddChunk(layer_state);
    chunks.AddChunk(scroll_state);
    ...
    EXPECT_THAT(cc_list->paint_op_buffer(),
        ElementsAre(PaintOpIs<cc::DrawRecordOp>(), // chunk 0
            ...
            PaintOpIs<cc::DrawRecordOp>())); // chunk 2
    const auto& scrolling_contents_op = static_cast<const cc::DrawScrollingContentsOp&>(cc_list->paint_op_buffer().GetOpAtForTesting(2));
    ASSERT_EQ(cc::PaintOpType::kDrawScrollingContents, scrolling_contents_op.GetType());
    EXPECT_THAT(scrolling_contents_op.display_item_list->paint_op_buffer(),
        ElementsAre(PaintOpIs<cc::DrawRecordOp>())); // chunk 1
}

TEST_P(PaintChunksToCcLayerTest, ScrollingContentsIntoDisplayItemListStartingFromNestedState)
{
    if (!RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        GTEST_SKIP();
    }
    ...
    auto scroll_state = CreateScrollTranslationState(PropertyTreeState::Root(), -50, -60, gfx::Rect(5, 5, 20, 30), gfx::Size(100, 200));
    auto* transform_under_scroll = CreateTransform(scroll_state.Transform(), MakeScaleMatrix(2));
    auto* effect_under_scroll = CreateOpacityEffect(scroll_state.Effect(), *transform_under_scroll, &scroll_state.Clip(), 0.5f);
    auto* clip_under_scroll = CreateClip(scroll_state.Clip(), *transform_under_scroll, FloatRoundedRect(0.f, 0.f, 1.f, 1.f));

    TestChunks chunks;
    chunks.AddChunk(t0(), c0(), e0());
    chunks.AddChunk(*transform_under_scroll, *clip_under_scroll, *effect_under_scroll);
    chunks.AddChunk(scroll_state);
    chunks.AddChunk(t0(), c0(), e0());
    ...
    ASSERT_EQ(cc::PaintOpType::kDrawScrollingContents, scrolling_contents_op.GetType());
    EXPECT_THAT(scrolling_contents_op.display_item_list->paint_op_buffer(),
        ElementsAre(PaintOpIs<cc::SaveOp>(), PaintOpEq<cc::ConcatOp>(SkM44::Scale(2, 2)), // <transform_under_scroll>
            PaintOpIs<cc::SaveLayerAlphaOp>(), // <effect_under_scroll>
            PaintOpIs<cc::SaveOp>(),
            PaintOpEq<cc::ClipRectOp>(SkRect::MakeXYWH(0, 0, 1, 1), SkClipOp::kIntersect,
...
//   </div>
// </div>
TEST_P(PaintChunksToCcLayerTest, ScrollingContentsInterlacingNonScrollingIntoDisplayItemList)
{
    if (!RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        GTEST_SKIP();
    }
    ...
    auto* effect_under_scroll = CreateOpacityEffect(e0(), *transform_under_scroll, nullptr, 0.5f);
    ...
    EXPECT_EQ(gfx::Rect(5, 5, 20, 30), cc_list->raster_inducing_scrolls().at(scroll_state.Transform().ScrollNode()->GetCompositorElementId()).visual_rect);
    const auto& scrolling_contents_op1 = static_cast<const cc::DrawScrollingContentsOp&>(cc_list->paint_op_buffer().GetOpAtForTesting(4));
    ASSERT_EQ(cc::PaintOpType::kDrawScrollingContents, scrolling_contents_op1.GetType());
    ...
    const auto& scrolling_contents_op2 = static_cast<const cc::DrawScrollingContentsOp&>(cc_list->paint_op_buffer().GetOpAtForTesting(10));
    ASSERT_EQ(cc::PaintOpType::kDrawScrollingContents, scrolling_contents_op2.GetType());
...
TEST_P(PaintChunksToCcLayerTest, NestedScrollingContentsIntoDisplayItemList)
{
    ...
    auto scroll_state1 = CreateScrollTranslationState(PropertyTreeState::Root(), -50, -60, gfx::Rect(5, 5, 20, 30), gfx::Size(100, 200));
    auto scroll_state2 = CreateScrollTranslationState(scroll_state1, -70, -80, gfx::Rect(10, 20, 30, 40), gfx::Size(200, 300));
    ...
    if (RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        EXPECT_THAT(cc_list->paint_op_buffer(),
            ElementsAre(PaintOpIs<cc::DrawRecordOp>(), // chunk 0
                ...
                PaintOpIs<cc::DrawRecordOp>())); // chunk 3
        EXPECT_EQ(gfx::Rect(5, 5, 20, 30), cc_list->raster_inducing_scrolls().at(scroll_state1.Transform().ScrollNode()->GetCompositorElementId()).visual_rect);
        EXPECT_EQ(gfx::Rect(5, 5, 20, 30), cc_list->raster_inducing_scrolls().at(scroll_state2.Transform().ScrollNode()->GetCompositorElementId()).visual_rect);
        const auto& scrolling_contents_op1 = static_cast<const cc::DrawScrollingContentsOp&>(cc_list->paint_op_buffer().GetOpAtForTesting(3));
        ASSERT_EQ(cc::PaintOpType::kDrawScrollingContents, scrolling_contents_op1.GetType());
        EXPECT_THAT(scrolling_contents_op1.display_item_list->paint_op_buffer(),
            ElementsAre(PaintOpIs<cc::DrawRecordOp>(), // chunk 1
                PaintOpIs<cc::SaveOp>(),
                PaintOpEq<cc::ClipRectOp>(SkRect::MakeXYWH(10, 20, 30, 40), SkClipOp::kIntersect,
                    /*antialias=*/true), // <overflow-clip2>
                PaintOpIs<cc::DrawScrollingContentsOp>(),
                PaintOpIs<cc::RestoreOp>())); // </overflow-clip2>
        const auto& scrolling_contents_op2
            = static_cast<const cc::DrawScrollingContentsOp&>(scrolling_contents_op1.display_item_list->paint_op_buffer().GetOpAtForTesting(3));
        ASSERT_EQ(cc::PaintOpType::kDrawScrollingContents, scrolling_contents_op2.GetType());
        EXPECT_THAT(scrolling_contents_op2.display_item_list->paint_op_buffer(),
        ...
        EXPECT_THAT(cc_list->paint_op_buffer(),
            ElementsAre(PaintOpIs<cc::DrawRecordOp>(), // chunk 0
                ...
                PaintOpEq<cc::ClipRectOp>(SkRect::MakeXYWH(5, 5, 20, 30), SkClipOp::kIntersect,
                    /*antialias=*/true), // <overflow-clip1>
                PaintOpIs<cc::SaveOp>(), PaintOpEq<cc::TranslateOp>(-50, -60), // <scroll-translation1>
                PaintOpIs<cc::DrawRecordOp>(), // chunk 1
                PaintOpIs<cc::SaveOp>(),
                PaintOpEq<cc::ClipRectOp>(SkRect::MakeXYWH(10, 20, 30, 40), SkClipOp::kIntersect,
                    /*antialias=*/true), // <overflow-clip1>
                PaintOpIs<cc::SaveOp>(), PaintOpEq<cc::TranslateOp>(-70, -80), // <scroll-translation2>
                PaintOpIs<cc::DrawRecordOp>(), // chunk 2
...
TEST_P(PaintChunksToCcLayerTest, NestedScrollingContentsIntoDisplayItemListStartingFromNestedState)
{
    if (!RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        GTEST_SKIP();
    }
    ...
    auto scroll_state1 = CreateScrollTranslationState(PropertyTreeState::Root(), -50, -60, gfx::Rect(5, 5, 20, 30), gfx::Size(100, 200));
    auto scroll_state2 = CreateScrollTranslationState(scroll_state1, -70, -80, gfx::Rect(10, 20, 30, 40), gfx::Size(200, 300));
    auto* transform_under_scroll = CreateTransform(scroll_state2.Transform(), MakeScaleMatrix(2));
    auto* clip_under_scroll = CreateClip(scroll_state2.Clip(), *transform_under_scroll, FloatRoundedRect(0.f, 0.f, 1.f, 1.f));
    auto* effect_under_scroll = CreateOpacityEffect(scroll_state2.Effect(), *transform_under_scroll, clip_under_scroll, 0.5f);

    TestChunks chunks;
    chunks.AddChunk(t0(), c0(), e0());
    chunks.AddChunk(*transform_under_scroll, *clip_under_scroll, *effect_under_scroll, gfx::Rect(1, 2, 67, 82));
    chunks.AddChunk(scroll_state1);
    ...
    EXPECT_THAT(scrolling_contents_op1.display_item_list->paint_op_buffer(),
        ElementsAre(PaintOpIs<cc::SaveOp>(),
            ...
            PaintOpIs<cc::DrawRecordOp>())); // chunk 2
    const auto& scrolling_contents_op2
        = static_cast<const cc::DrawScrollingContentsOp&>(scrolling_contents_op1.display_item_list->paint_op_buffer().GetOpAtForTesting(2));
    ASSERT_EQ(cc::PaintOpType::kDrawScrollingContents, scrolling_contents_op2.GetType());
    EXPECT_THAT(scrolling_contents_op2.display_item_list->paint_op_buffer(),
        ElementsAre(PaintOpIs<cc::SaveOp>(), PaintOpEq<cc::ConcatOp>(SkM44::Scale(2, 2)), // <transform_under_scroll>
            PaintOpEq<cc::ClipRectOp>(SkRect::MakeXYWH(0, 0, 1, 1), SkClipOp::kIntersect,
...
TEST_P(PaintChunksToCcLayerTest, NonCompositedFixedAttachmentBackground)
{
    ...
    auto scroll_state = CreateScrollTranslationState(PropertyTreeState::Root(), -50, -60, gfx::Rect(5, 5, 20, 30), gfx::Size(100, 200));
    auto* background_clip = CreateClip(scroll_state.Clip(), scroll_state.Transform(), FloatRoundedRect(0.f, 0.f, 10.f, 10.f));
    ...
    EXPECT_THAT(cc_list->paint_op_buffer(),
        ElementsAre(PaintOpIs<cc::DrawRecordOp>(), // chunk 0
            ...
            PaintOpEq<cc::ClipRectOp>(SkRect::MakeXYWH(5, 5, 20, 30), SkClipOp::kIntersect,
                /*antialias=*/true), // <overflow-clip>
            PaintOpIs<cc::DrawScrollingContentsOp>(), PaintOpIs<cc::SaveOp>(), PaintOpEq<cc::TranslateOp>(-50, -60),
            PaintOpEq<cc::ClipRectOp>(SkRect::MakeXYWH(0, 0, 10, 10), SkClipOp::kIntersect,
    ...
    EXPECT_THAT(scrolling_contents_op1.display_item_list->paint_op_buffer(),
        ElementsAre(PaintOpIs<cc::DrawRecordOp>(), // chunk 1
            PaintOpIs<cc::SaveOp>(), PaintOpIs<cc::ClipRectOp>(), PaintOpIs<cc::RestoreOp>()));
    const auto& scrolling_contents_op2 = static_cast<const cc::DrawScrollingContentsOp&>(cc_list->paint_op_buffer().GetOpAtForTesting(12));
    ASSERT_EQ(cc::PaintOpType::kDrawScrollingContents, scrolling_contents_op2.GetType());
    EXPECT_THAT(scrolling_contents_op2.display_item_list->paint_op_buffer(),
        ElementsAre(PaintOpIs<cc::DrawRecordOp>())); // chunk 3
}

TEST_P(PaintChunksToCcLayerTest, UpdateLayerPropertiesRegionCaptureDataSetOnLayer)
{
    auto layer = cc::Layer::Create();

    TestChunks chunks;
    chunks.AddChunk(t0(), c0(), e0(), gfx::Rect(5, 10, 200, 300), gfx::Rect(10, 15, 20, 30));

    const auto kCropId = RegionCaptureCropId(base::Token::CreateRandom());
    chunks.GetChunks().back().region_capture_data = MakeRegionCaptureData({ { kCropId, gfx::Rect(50, 60, 100, 200) } });

    UpdateLayerProperties(*layer, PropertyTreeState::Root(), chunks.Build());

    const gfx::Rect actual_bounds = layer->capture_bounds().bounds().find(kCropId.value())->second;
    EXPECT_EQ((gfx::Rect { 50, 60, 100, 200 }), actual_bounds);
}

TEST_P(PaintChunksToCcLayerTest, UpdateLayerPropertiesRegionCaptureDataUsesLayerOffset)
{
    auto layer = cc::Layer::Create();
...
TEST_P(PaintChunksToCcLayerTest, UpdateLayerPropertiesRegionCaptureDataEmptyBounds)
{
    auto layer = cc::Layer::Create();
    ...
    const auto kCropId = RegionCaptureCropId(base::Token::CreateRandom());
    chunks.GetChunks().back().region_capture_data = MakeRegionCaptureData({ { kCropId, gfx::Rect() } });
...
TEST_P(PaintChunksToCcLayerTest, UpdateLayerPropertiesRegionCaptureDataMultipleChunks)
{
    auto layer = cc::Layer::Create();
    ...
    chunks.AddChunk(t0(), c0(), e0(), gfx::Rect(6, 12, 244, 366), gfx::Rect(20, 30, 40, 60));
    ...
    chunks.GetChunks().back().region_capture_data
        = MakeRegionCaptureData({ { kSecondCropId, gfx::Rect(51, 61, 101, 201) }, { kThirdCropId, gfx::Rect(52, 62, 102, 202) } });

    UpdateLayerProperties(*layer, PropertyTreeState::Root(), chunks.Build());

    EXPECT_EQ((gfx::Rect { 50, 60, 100, 200 }), layer->capture_bounds().bounds().find(kCropId.value())->second);
    EXPECT_EQ((gfx::Rect { 51, 61, 101, 201 }), layer->capture_bounds().bounds().find(kSecondCropId.value())->second);
    EXPECT_EQ((gfx::Rect { 52, 62, 102, 202 }), layer->capture_bounds().bounds().find(kThirdCropId.value())->second);
}
...
TEST_P(PaintChunksToCcLayerTest, NonCompositedBackdropFilter)
{
    ...
    // ignored.
    EXPECT_THAT(output, ElementsAre(PaintOpIs<cc::SaveLayerAlphaOp>(), PaintOpIs<cc::DrawRecordOp>(), PaintOpIs<cc::RestoreOp>()));
}
