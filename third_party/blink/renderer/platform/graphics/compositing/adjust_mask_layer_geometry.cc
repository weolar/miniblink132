...
void AdjustMaskLayerGeometry(const TransformPaintPropertyNode& transform, gfx::Vector2dF& layer_offset, gfx::Size& layer_bounds)
{
    ...
    GeometryMapper::SourceToDestinationRect(TransformPaintPropertyNode::Root(), transform, pixel_rect);
    // Don't expand too far in extreme cases.
    constexpr int kMaxOutset = 1000;
    int outset = ClampTo(std::ceil(std::max(pixel_rect.width(), pixel_rect.height()) * 2), 0, kMaxOutset);
