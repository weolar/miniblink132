...
bool DefaultIsCompositedScroll(const TransformPaintPropertyNode& scroll_translation)
{
    DCHECK(scroll_translation.ScrollNode());
    return scroll_translation.HasDirectCompositingReasons();
}

bool Merge(PendingLayer& home, const PendingLayer& guest, LCDTextPreference lcd_text_preference = LCDTextPreference::kIgnored,
    PendingLayer::IsCompositedScrollFunction is_composited_scroll = DefaultIsCompositedScroll)
{
...
TEST(PendingLayerTest, Merge)
{
    ...
    EXPECT_EQ(pending_layer.BoundsForTesting(), pending_layer.RectKnownToBeOpaque());

    ASSERT_TRUE(Merge(pending_layer, PendingLayer(artifact, artifact.GetPaintChunks()[1])));
    ...
    ASSERT_TRUE(Merge(pending_layer, PendingLayer(artifact, artifact.GetPaintChunks()[2])));
...
TEST(PendingLayerTest, MergeWithGuestTransform)
{
    ...
    auto& artifact = TestPaintArtifact().Chunk().Bounds(gfx::Rect(0, 0, 30, 40)).Chunk(*transform, c0(), e0()).Bounds(gfx::Rect(0, 0, 50, 60)).Build();
...
{
    auto* transform = Create2DTranslation(t0(), 20, 25);
    auto& artifact = TestPaintArtifact().Chunk(*transform, c0(), e0()).Bounds(gfx::Rect(0, 0, 30, 40)).Chunk().Bounds(gfx::Rect(0, 0, 50, 60)).Build();
...
TEST(PendingLayerTest, MergeWithBothTransforms)
{
    ...
    auto& artifact = TestPaintArtifact().Chunk(*t1, c0(), e0()).Bounds(gfx::Rect(0, 0, 30, 40)).Chunk(*t2, c0(), e0()).Bounds(gfx::Rect(0, 0, 50, 60)).Build();
...
TEST(PendingLayerTest, DontMergeSparse)
{
    ...
    PendingLayer pending_layer(artifact, artifact.GetPaintChunks()[0]);
    ASSERT_FALSE(Merge(pending_layer, PendingLayer(artifact, artifact.GetPaintChunks()[1])));
    EXPECT_EQ(gfx::RectF(0, 0, 30, 40), pending_layer.BoundsForTesting());
    EXPECT_EQ(artifact.GetPaintChunks()[0].properties, pending_layer.GetPropertyTreeState());
    EXPECT_THAT(ChunkIndices(pending_layer), ElementsAre(0));
}
...
{
    auto* t1 = Create2DTranslation(t0(), 20, 25);
    auto* e1 = CreateOpacityEffect(e0(), 1.0f, CompositingReason::kWillChangeOpacity);
    auto* t2 = Create2DTranslation(t0(), 1000, 1000);
    auto& artifact = TestPaintArtifact().Chunk(*t1, c0(), *e1).Bounds(gfx::Rect(0, 0, 30, 40)).Chunk(*t2, c0(), *e1).Bounds(gfx::Rect(0, 0, 50, 60)).Build();
...
TEST(PendingLayerTest, MergeSparseInNonCompositedEffect)
{
    ...
    PendingLayer pending_layer(artifact, artifact.GetPaintChunks()[0]);
    EXPECT_FALSE(Merge(pending_layer, PendingLayer(artifact, artifact.GetPaintChunks()[1])));
}
...
TEST(PendingLayerTest, KnownOpaque)
{
    ...
    EXPECT_NE(pending_layer.BoundsForTesting(), pending_layer.RectKnownToBeOpaque());
...
class PendingLayerTextOpaquenessTest : public testing::Test, public testing::WithParamInterface<LCDTextPreference> {
protected:
...
INSTANTIATE_TEST_SUITE_P(All, PendingLayerTextOpaquenessTest, ::testing::Values(LCDTextPreference::kStronglyPreferred, LCDTextPreference::kIgnored));
