...
public:
    SynthesizedClip()
        : layer_(nullptr)
    {
        mask_isolation_id_ = CompositorElementIdFromUniqueObjectId(NewUniqueObjectId());
        mask_effect_id_ = CompositorElementIdFromUniqueObjectId(NewUniqueObjectId());
    }
    ...
    void UpdateLayer(const ClipPaintPropertyNode&, const TransformPaintPropertyNode&);

    cc::PictureLayer* Layer()
    {
        return layer_.get();
    }
    CompositorElementId GetMaskIsolationId() const
    {
        return mask_isolation_id_;
    }
    CompositorElementId GetMaskEffectId() const
    {
        return mask_effect_id_;
    }

private:
    // ContentLayerClient implementation.
    scoped_refptr<cc::DisplayItemList> PaintContentsToDisplayList() final;
    bool FillsBoundsCompletely() const final
    {
        return false;
    }
...
class PLATFORM_EXPORT PaintArtifactCompositor final : public GarbageCollected<PaintArtifactCompositor>, private PropertyTreeManagerClient {
public:
    PaintArtifactCompositor(base::WeakPtr<CompositorScrollCallbacks> scroll_callbacks);
    ...
    void SetNeedsUpdate()
    {
        SetNeedsUpdateInternal(UpdateType::kFull);
    }
    void SetNeedsUpdateForRasterInducingScroll()
    {
        SetNeedsUpdateInternal(UpdateType::kRasterInducingScroll);
    }
    void SetNeedsUpdateAfterRepaint(const PaintArtifact& previous, const PaintArtifact& repainted);

    UpdateType NeedsUpdate() const
    {
        return needs_update_;
    }
    void ClearNeedsUpdateForTesting()
    {
    ...
    // property tree.
    using StackScrollTranslationVector = HeapVector<Member<const TransformPaintPropertyNode>, 32>;
    void Update(const PaintArtifact& artifact, const ViewportProperties& viewport_properties, const StackScrollTranslationVector& scroll_translation_nodes,
    ...
    bool DirectlySetScrollOffset(CompositorElementId, const gfx::PointF& scroll_offset);
    ...
    void ForAllContentLayersForTesting(base::FunctionRef<void(ContentLayerClientImpl*)> func) const;
...
private:
    ...
    void UpdateCompositorViewportProperties(const ViewportProperties&, PropertyTreeManager&, cc::LayerTreeHost*);

    const TransformPaintPropertyNode& ScrollTranslationStateForLayer(const PendingLayer&);
    ...
    SynthesizedClip& CreateOrReuseSynthesizedClipLayer(const ClipPaintPropertyNode&, const TransformPaintPropertyNode&, bool needs_layer,
        CompositorElementId& mask_isolation_id, CompositorElementId& mask_effect_id) final;

    bool NeedsCompositedScrolling(const TransformPaintPropertyNode& scroll_translation) const final;
    bool ShouldForceMainThreadRepaint(const TransformPaintPropertyNode& scroll_translation) const final;

    bool ComputeNeedsCompositedScrolling(const PaintArtifact&, PaintChunks::const_iterator chunk_cursor) const;
    void UpdatePaintedScrollTranslationsBeforeLayerization(const PaintArtifact&, PaintChunks::const_iterator chunk_cursor);
    PendingLayer::CompositingType ChunkCompositingType(const PaintArtifact&, const PaintChunk&) const;

    static void UpdateRenderSurfaceForEffects(cc::EffectTree&, const cc::LayerList&, const Vector<const EffectPaintPropertyNode*>&);

    bool CanDirectlyUpdateProperties() const;

    CompositingReasons GetCompositingReasons(const PendingLayer& layer, const PropertyTreeState& previous_layer_state) const;
    ...
    HeapHashMap<Member<const TransformPaintPropertyNode>, ScrollTranslationInfo> painted_scroll_translations_;
...
WTF_ALLOW_INIT_WITH_MEM_FUNCTIONS(blink::PaintArtifactCompositor::SynthesizedClipEntry)
