...
PaintArtifactCompositor::PaintArtifactCompositor(base::WeakPtr<CompositorScrollCallbacks> scroll_callbacks)
...
void PaintArtifactCompositor::SetLCDTextPreference(LCDTextPreference preference)
{
...
std::unique_ptr<JSONArray> PaintArtifactCompositor::GetPendingLayersAsJSON() const
{
...
std::unique_ptr<JSONObject> PaintArtifactCompositor::GetLayersAsJSON(LayerTreeFlags flags) const
{
    ...
    for (const auto& layer : root_layer_->children()) {
        ...
        if (!transform) {
            for (const auto& pending_layer : pending_layers_) {
                if (pending_layer.GetPropertyTreeState().Transform().CcNodeId(layer->property_tree_sequence_number()) == layer->transform_tree_index()) {
                    transform = &pending_layer.GetPropertyTreeState().Transform();
                    break;
                }
            }
...
const TransformPaintPropertyNode& PaintArtifactCompositor::ScrollTranslationStateForLayer(const PendingLayer& pending_layer)
{
    ...
    return RuntimeEnabledFeatures::HitTestOpaquenessEnabled() ? transform.ScrollTranslationState() : transform.NearestScrollTranslationNode();
}

bool PaintArtifactCompositor::NeedsCompositedScrolling(const TransformPaintPropertyNode& scroll_translation) const
{
...
bool PaintArtifactCompositor::ShouldForceMainThreadRepaint(const TransformPaintPropertyNode& scroll_translation) const
{
    DCHECK(!NeedsCompositedScrolling(scroll_translation));
    auto it = painted_scroll_translations_.find(&scroll_translation);
    return it != painted_scroll_translations_.end() && it->value.force_main_thread_repaint;
}

bool PaintArtifactCompositor::ComputeNeedsCompositedScrolling(const PaintArtifact& artifact, PaintChunks::const_iterator chunk_cursor) const
{
    ...
    const auto& scroll_translation = *chunk_cursor->hit_test_data->scroll_translation;
    DCHECK(scroll_translation.ScrollNode());
    if (scroll_translation.HasDirectCompositingReasons()) {
        return true;
    }
    ...
    auto preference = scroll_translation.ScrollNode()->GetCompositedScrollingPreference();
    ...
    for (auto next = chunk_cursor + 1; next != artifact.GetPaintChunks().end(); ++next) {
        if (&next->properties.Transform() == &chunk_cursor->properties.Transform()) {
        ...
        return &next->properties.Transform().Unalias() == &scroll_translation
            && &next->properties.Clip().Unalias() == scroll_translation.ScrollNode()->OverflowClipNode()
            && &next->properties.Effect().Unalias() == &chunk_cursor->properties.Effect().Unalias()
            && next->rect_known_to_be_opaque.Contains(scroll_translation.ScrollNode()->ContentsRect());
    }
    return true;
}

void PaintArtifactCompositor::UpdatePaintedScrollTranslationsBeforeLayerization(const PaintArtifact& artifact, PaintChunks::const_iterator chunk_cursor)
{
    ...
    if (hit_test_data && hit_test_data->scroll_translation) {
        ...
        bool is_composited = ComputeNeedsCompositedScrolling(artifact, chunk_cursor);
        auto it = painted_scroll_translations_.find(&scroll_translation);
        if (it == painted_scroll_translations_.end()) {
            painted_scroll_translations_.insert(&scroll_translation,
                ScrollTranslationInfo { .scrolling_contents_cull_rect = hit_test_data->scrolling_contents_cull_rect, .is_composited = is_composited });
        } else {
            // The node was added in the second half of this function before.
            // Update the is_composited field now.
            it->value.scrolling_contents_cull_rect = hit_test_data->scrolling_contents_cull_rect;
            if (is_composited) {
    ...
    if (RuntimeEnabledFeatures::RasterInducingScrollEnabled()
        && ((hit_test_data
                && (!hit_test_data->touch_action_rects.empty() || !hit_test_data->wheel_event_rects.empty() ||
                    // HitTestData of these types induce touch action regions.
                    chunk.id.type == DisplayItem::Type::kScrollbarHitTest || chunk.id.type == DisplayItem::Type::kResizerScrollHitTest))
        ...
        const auto* composited_ancestor = transform.NearestDirectlyCompositedAncestor();
        for (const auto* scroll_translation = &transform.NearestScrollTranslationNode(); scroll_translation;
             scroll_translation = scroll_translation->ParentScrollTranslationNode()) {
            if (scroll_translation->NearestDirectlyCompositedAncestor() != composited_ancestor) {
                break;
            }
            auto it = painted_scroll_translations_.find(scroll_translation);
            if (it == painted_scroll_translations_.end()) {
                ...
                painted_scroll_translations_.insert(scroll_translation, ScrollTranslationInfo { .force_main_thread_repaint = true });
            } else {
...
PendingLayer::CompositingType PaintArtifactCompositor::ChunkCompositingType(const PaintArtifact& artifact, const PaintChunk& chunk) const
{
    if (chunk.hit_test_data && chunk.hit_test_data->scroll_translation && NeedsCompositedScrolling(*chunk.hit_test_data->scroll_translation)) {
    ...
    if (chunk.size() == 1) {
        ...
        if (const auto* scrollbar = DynamicTo<ScrollbarDisplayItem>(item)) {
            if (const auto* scroll_translation = scrollbar->ScrollTranslation()) {
                if (RuntimeEnabledFeatures::RasterInducingScrollEnabled() || NeedsCompositedScrolling(*scroll_translation)) {
...
cc::Layer* ForeignLayer(const PaintChunk& chunk, const PaintArtifact& artifact)
{
    if (chunk.size() != 1)
        return nullptr;
    const auto& first_display_item = artifact.GetDisplayItemList()[chunk.begin_index];
...
bool NeedsFullUpdateAfterPaintingChunk(
    const PaintChunk& previous, const PaintArtifact& previous_artifact, const PaintChunk& repainted, const PaintArtifact& repainted_artifact)
{
    ...
    if (repainted.is_moved_from_cached_subsequence) {
        ...
        DCHECK_EQ(previous.rect_known_to_be_opaque, repainted.rect_known_to_be_opaque);
        DCHECK_EQ(previous.text_known_to_be_on_opaque_background, repainted.text_known_to_be_on_opaque_background);
    ...
    if (ForeignLayer(previous, previous_artifact) != ForeignLayer(repainted, repainted_artifact)) {
        return true;
    }
    ...
    if (previous.text_known_to_be_on_opaque_background != repainted.text_known_to_be_on_opaque_background) {
        return true;
    }
    // Whether background color is transparent affects cc::Layers's contents
    // opaque property.
    if ((previous.background_color.color == SkColors::kTransparent) != (repainted.background_color.color == SkColors::kTransparent)) {
        return true;
    }
    ...
    // type.
    if (previous.background_color.is_solid_color != repainted.background_color.is_solid_color) {
        return true;
    }
...
{
    if ((update_type == UpdateType::kRasterInducingScroll && needs_update_ == UpdateType::kRepaint)
        || (update_type == UpdateType::kRepaint && needs_update_ == UpdateType::kRasterInducingScroll)) {
...
void PaintArtifactCompositor::SetNeedsUpdateAfterRepaint(const PaintArtifact& previous, const PaintArtifact& repainted)
{
    ...
    for (wtf_size_t i = 0; i < previous.GetPaintChunks().size(); i++) {
        if (NeedsFullUpdateAfterPaintingChunk(previous.GetPaintChunks()[i], previous, repainted.GetPaintChunks()[i], repainted)) {
            SetNeedsUpdate();
            return;
        }
    }
...
bool PaintArtifactCompositor::HasComposited(CompositorElementId element_id) const
{
    ...
    DCHECK_EQ(needs_update_, UpdateType::kNone) << "This should only be called after an update";
    return root_layer_->layer_tree_host()->property_trees()->HasElement(element_id);
}
...
public:
    Layerizer(PaintArtifactCompositor& compositor, const PaintArtifact& artifact, wtf_size_t reserve_capacity)
...
private:
    ...
    bool DecompositeEffect(const EffectPaintPropertyNode& parent_effect, wtf_size_t first_layer_in_parent_group_index, const EffectPaintPropertyNode& effect,
    ...
    HeapHashSet<Member<const TransformPaintPropertyNode>> directly_composited_transforms_;
};
...
// 'node' itself.
static const EffectPaintPropertyNode* StrictUnaliasedChildOfAlongPath(const EffectPaintPropertyNode& ancestor, const EffectPaintPropertyNode& node)
{
    const auto* n = &node;
    while (n) {
...
bool PaintArtifactCompositor::Layerizer::DecompositeEffect(
    const EffectPaintPropertyNode& parent_effect, wtf_size_t first_layer_in_parent_group_index, const EffectPaintPropertyNode& effect, wtf_size_t layer_index)
{
    ...
    PropertyTreeState group_state(
        effect.LocalTransformSpace().Unalias(), effect.OutputClip() ? effect.OutputClip()->Unalias() : layer.GetPropertyTreeState().Clip(), effect);
    auto is_composited_scroll = [this](const TransformPaintPropertyNode& t) { return compositor_.NeedsCompositedScrolling(t); };
    std::optional<PropertyTreeState> upcast_state = group_state.CanUpcastWith(layer.GetPropertyTreeState(), is_composited_scroll);
    if (!upcast_state)
        return false;
    ...
    if (effect.BlendMode() != SkBlendMode::kSrcOver) {
        auto num_previous_siblings = layer_index - first_layer_in_parent_group_index;
        ...
        if (num_previous_siblings) {
            ...
            if (previous_sibling.DrawsContent() && !previous_sibling.CanMergeWithDecompositedBlendMode(layer, *upcast_state, is_composited_scroll)) {
                return false;
            }
        }
    }
...
void PaintArtifactCompositor::Layerizer::LayerizeGroup(const EffectPaintPropertyNode& current_group, bool force_draws_content)
{
    ...
    while (chunk_cursor_ != artifact_.GetPaintChunks().end()) {
        ...
        if (&chunk_effect == &current_group) {
            compositor_.UpdatePaintedScrollTranslationsBeforeLayerization(artifact_, chunk_cursor_);
            pending_layers_.emplace_back(artifact_, *chunk_cursor_, compositor_.ChunkCompositingType(artifact_, *chunk_cursor_));
            ++chunk_cursor_;
            ...
            const EffectPaintPropertyNode* subgroup = StrictUnaliasedChildOfAlongPath(current_group, chunk_effect);
            ...
            if (first_layer_in_subgroup != pending_layers_.size() - 1)
                continue;
            if (!DecompositeEffect(current_group, first_layer_in_current_group, *subgroup, first_layer_in_subgroup))
                continue;
        }
        ...
        if (const auto* composited_transform = new_layer.GetPropertyTreeState().Transform().NearestDirectlyCompositedAncestor()) {
            if (directly_composited_transforms_.insert(composited_transform).is_new_entry) {
                continue;
            }
        }
        ...
        // reverse.
        auto is_composited_scroll = [this](const TransformPaintPropertyNode& t) { return compositor_.NeedsCompositedScrolling(t); };
        for (wtf_size_t candidate_index = pending_layers_.size() - 1; candidate_index-- > first_layer_in_current_group;) {
            PendingLayer& candidate_layer = pending_layers_[candidate_index];
            if (candidate_layer.Merge(new_layer, compositor_.lcd_text_preference_, is_composited_scroll)) {
...
void SynthesizedClip::UpdateLayer(const ClipPaintPropertyNode& clip, const TransformPaintPropertyNode& transform)
{
    if (!layer_) {
    ...
    gfx::Transform new_projection = GeometryMapper::SourceToDestinationProjection(clip.LocalTransformSpace(), transform);
    ...
    if (!path && new_projection.IsIdentityOr2dTranslation()) {
        ...
        needs_display = rrect_is_local_ || new_rrect != rrect_ || new_projection != projection_ || !clip.ClipPathEquals(path_);
...
scoped_refptr<cc::DisplayItemList> SynthesizedClip::PaintContentsToDisplayList()
{
    ...
    if (rrect_is_local_) {
        ...
        if (path_) {
            cc_list->push<cc::ClipPathOp>(path_->GetSkPath(), SkClipOp::kIntersect, true);
        }
        cc_list->push<cc::DrawRRectOp>(rrect_, flags);
...
SynthesizedClip& PaintArtifactCompositor::CreateOrReuseSynthesizedClipLayer(const ClipPaintPropertyNode& clip, const TransformPaintPropertyNode& transform,
    bool needs_layer, CompositorElementId& mask_isolation_id, CompositorElementId& mask_effect_id)
{
    auto entry = base::ranges::find_if(synthesized_clip_cache_, [&clip](const auto& entry) { return entry.key == &clip && !entry.in_use; });
    if (entry == synthesized_clip_cache_.end()) {
        synthesized_clip_cache_.push_back(SynthesizedClipEntry { &clip, std::make_unique<SynthesizedClip>(), false });
    ...
    if (!should_always_update_on_scroll_) {
        ...
        if (&clip_transform != &transform && &clip_transform.NearestScrollTranslationNode() != &transform.NearestScrollTranslationNode()) {
...
void PaintArtifactCompositor::UpdateCompositorViewportProperties(
    const ViewportProperties& properties, PropertyTreeManager& property_tree_manager, cc::LayerTreeHost* layer_tree_host)
{
    // The inner and outer viewports' existence is linked. That is, either they're
    // both null or they both exist.
    CHECK_EQ(static_cast<bool>(properties.outer_scroll_translation), static_cast<bool>(properties.inner_scroll_translation));
    CHECK(!properties.outer_clip || static_cast<bool>(properties.inner_scroll_translation));

    cc::ViewportPropertyIds ids;
    if (properties.overscroll_elasticity_transform) {
        ids.overscroll_elasticity_transform = property_tree_manager.EnsureCompositorTransformNode(*properties.overscroll_elasticity_transform);
    }
    if (properties.page_scale) {
        ids.page_scale_transform = property_tree_manager.EnsureCompositorPageScaleTransformNode(*properties.page_scale);
    }
    if (properties.inner_scroll_translation) {
        ids.inner_scroll = property_tree_manager.EnsureCompositorInnerScrollAndTransformNode(*properties.inner_scroll_translation);
        if (properties.outer_clip) {
            ids.outer_clip = property_tree_manager.EnsureCompositorClipNode(*properties.outer_clip);
        }
        CHECK(properties.outer_scroll_translation);
        ids.outer_scroll = property_tree_manager.EnsureCompositorOuterScrollAndTransformNode(*properties.outer_scroll_translation);
        ...
        painted_scroll_translations_.insert(properties.inner_scroll_translation, ScrollTranslationInfo { InfiniteIntRect(), true });
        painted_scroll_translations_.insert(properties.outer_scroll_translation, ScrollTranslationInfo { InfiniteIntRect(), true });
    }

    layer_tree_host->RegisterViewportPropertyIds(ids);
}

void PaintArtifactCompositor::Update(const PaintArtifact& artifact, const ViewportProperties& viewport_properties,
    const StackScrollTranslationVector& scroll_translation_nodes, Vector<std::unique_ptr<cc::ViewTransitionRequest>> transition_requests)
{
    ...
    host->property_trees()->scroll_tree_mutable().SetScrollCallbacks(scroll_callbacks_);
    ...
    PropertyTreeManager property_tree_manager(*this, *host->property_trees(), *root_layer_, layer_list_builder, g_s_property_tree_sequence_number);

    UpdateCompositorViewportProperties(viewport_properties, property_tree_manager, host);
    ...
    for (auto& [node, info] : painted_scroll_translations_) {
        property_tree_manager.EnsureCompositorScrollAndTransformNode(*node, info.scrolling_contents_cull_rect);
    }

    cc::LayerSelection layer_selection;
    for (auto& pending_layer : pending_layers_) {
        pending_layer.UpdateCompositedLayer(
            old_pending_layer_matcher.Find(pending_layer), layer_selection, tracks_raster_invalidations_, root_layer_->layer_tree_host());
        ...
        int transform_id = property_tree_manager.EnsureCompositorTransformNode(transform);
        int effect_id = property_tree_manager.SwitchToEffectNodeWithSynthesizedClip(effect, clip, layer.draws_content());
        int clip_id = property_tree_manager.EnsureCompositorClipNode(clip);

        // We need additional bookkeeping for backdrop-filter mask.
        if (effect.RequiresCompositingForBackdropFilterMask() && effect.CcNodeId(g_s_property_tree_sequence_number) == effect_id) {
            ...
            effect_tree.Node(cc_node->parent_id)->backdrop_mask_element_id = effect.GetCompositorElementId();
        }

        int scroll_id = property_tree_manager.EnsureCompositorScrollAndTransformNode(ScrollTranslationStateForLayer(pending_layer), InfiniteIntRect());

        layer_list_builder.Add(&layer);

        layer.set_property_tree_sequence_number(root_layer_->property_tree_sequence_number());
    ...
    auto new_end = std::remove_if(synthesized_clip_cache_.begin(), synthesized_clip_cache_.end(), [](const auto& entry) { return !entry.in_use; });
    synthesized_clip_cache_.Shrink(static_cast<wtf_size_t>(new_end - synthesized_clip_cache_.begin()));

    // This should be done before
    // property_tree_manager.UpdateConditionalRenderSurfaceReasons() for which to
    // get property tree node ids from the layers.
    host->property_trees()->set_sequence_number(g_s_property_tree_sequence_number);
    ...
    for (auto& chunk : artifact.GetPaintChunks()) {
        ...
        if (chunk.hit_test_data && chunk.hit_test_data->scroll_translation) {
            chunk.hit_test_data->scroll_translation->ClearChangedToRoot(g_s_property_tree_sequence_number);
        }
    }

    DVLOG(2) << "PaintArtifactCompositor::Update() done\n"
             << "Composited layers:\n"
             << GetLayersAsJSON(VLOG_IS_ON(3) ? 0xffffffff : 0)->ToPrettyJSONString().Utf8();
}

bool PaintArtifactCompositor::TryFastPathUpdate(const PaintArtifact& repainted_artifact)
{
#if DCHECK_IS_ON()
    if (needs_update_ != UpdateType::kFull) {
        ...
        for (const auto& chunk : repainted_artifact.GetPaintChunks()) {
            ...
            DCHECK(!chunk.properties.Unalias().ChangedToRoot(PaintPropertyChangeType::kChangedOnlyNonRerasterValues));
        }
    }
#endif
    ...
    case UpdateType::kRepaint: {
        cc::LayerSelection layer_selection;
        for (auto& pending_layer : pending_layers_) {
            pending_layer.UpdateCompositedLayerForRepaint(repainted_artifact, layer_selection);
        }
        root_layer_->layer_tree_host()->RegisterSelection(layer_selection);
        UpdateDebugInfo();
        break;
...
bool PaintArtifactCompositor::DirectlyUpdateCompositedOpacityValue(const EffectPaintPropertyNode& effect)
{
    ...
    if (CanDirectlyUpdateProperties()) {
        return PropertyTreeManager::DirectlyUpdateCompositedOpacityValue(*root_layer_->layer_tree_host(), effect);
    }
    return false;
}

bool PaintArtifactCompositor::DirectlyUpdateScrollOffsetTransform(const TransformPaintPropertyNode& transform)
{
    if (CanDirectlyUpdateProperties()) {
        return PropertyTreeManager::DirectlyUpdateScrollOffsetTransform(*root_layer_->layer_tree_host(), transform);
    }
    return false;
}

bool PaintArtifactCompositor::DirectlyUpdateTransform(const TransformPaintPropertyNode& transform)
{
    // We can only directly-update compositor values if all content associated
    // with the node is known to be composited.
    ...
    if (CanDirectlyUpdateProperties()) {
        return PropertyTreeManager::DirectlyUpdateTransform(*root_layer_->layer_tree_host(), transform);
    }
    return false;
}

bool PaintArtifactCompositor::DirectlyUpdatePageScaleTransform(const TransformPaintPropertyNode& transform)
{
    // We can only directly-update compositor values if all content associated
    // with the node is known to be composited.
    DCHECK(transform.HasDirectCompositingReasons());
    if (CanDirectlyUpdateProperties()) {
        return PropertyTreeManager::DirectlyUpdatePageScaleTransform(*root_layer_->layer_tree_host(), transform);
    }
    return false;
}

bool PaintArtifactCompositor::DirectlySetScrollOffset(CompositorElementId element_id, const gfx::PointF& scroll_offset)
{
    ...
    if (!property_trees->scroll_tree().FindNodeFromElementId(element_id))
        return false;
    PropertyTreeManager::DirectlySetScrollOffset(*root_layer_->layer_tree_host(), element_id, scroll_offset);
    return true;
}

void PaintArtifactCompositor::DropCompositorScrollDeltaNextCommit(CompositorElementId element_id)
{
    if (!root_layer_ || !root_layer_->layer_tree_host()) {
        return;
    }
    ...
    PropertyTreeManager::DropCompositorScrollDeltaNextCommit(*root_layer_->layer_tree_host(), element_id);
}

uint32_t PaintArtifactCompositor::GetMainThreadRepaintReasons(const ScrollPaintPropertyNode& scroll) const
{
    CHECK(root_layer_);
    if (!root_layer_->layer_tree_host()) {
        return 0;
    }
    return PropertyTreeManager::GetMainThreadRepaintReasons(*root_layer_->layer_tree_host(), scroll);
}

bool PaintArtifactCompositor::UsesCompositedScrolling(const ScrollPaintPropertyNode& scroll) const
{
    CHECK(root_layer_);
    if (!root_layer_->layer_tree_host()) {
        return false;
    }
    return PropertyTreeManager::UsesCompositedScrolling(*root_layer_->layer_tree_host(), scroll);
}

bool PaintArtifactCompositor::UsesRasterInducingScroll(const ScrollPaintPropertyNode& scroll) const
{
    CHECK(root_layer_);
    if (!root_layer_->layer_tree_host()) {
        return false;
    }
    return PropertyTreeManager::UsesRasterInducingScroll(*root_layer_->layer_tree_host(), scroll);
}
...
void PaintArtifactCompositor::UpdateDebugInfo() const
{
    ...
    for (const auto& pending_layer : pending_layers_) {
        ...
        if (needs_update_ == UpdateType::kFull) {
            auto compositing_reasons = GetCompositingReasons(pending_layer, previous_layer_state);
            debug_info.compositing_reasons = CompositingReason::Descriptions(compositing_reasons);
            debug_info.compositing_reason_ids = CompositingReason::ShortNames(compositing_reasons);
        }
        debug_info.owner_node_id = pending_layer.OwnerNodeId();

        if (RasterInvalidationTracking::IsTracingRasterInvalidations() && tracking) {
...
CompositingReasons PaintArtifactCompositor::GetCompositingReasons(const PendingLayer& layer, const PropertyTreeState& previous_layer_state) const
{
    ...
    if (transform.IsBackfaceHidden() && !previous_layer_state.Transform().IsBackfaceHidden()) {
        reasons = CompositingReason::kBackfaceVisibilityHidden;
    }
    if (layer.GetCompositingType() == PendingLayer::kOverlap) {
        return reasons == CompositingReason::kNone ? CompositingReason::kOverlap : reasons;
    }

    auto composited_ancestor = [this](const TransformPaintPropertyNode& transform) -> const TransformPaintPropertyNode* {
        ...
        if (NeedsCompositedScrolling(scroll_translation) && (!ancestor || ancestor->IsAncestorOf(scroll_translation))) {
    ...
    auto transform_compositing_reasons
        = [composited_ancestor](const TransformPaintPropertyNode& transform, const TransformPaintPropertyNode& previous) -> CompositingReasons {
        CompositingReasons reasons = CompositingReason::kNone;
    ...
    auto clip_compositing_reasons
        = [transform_compositing_reasons](const ClipPaintPropertyNode& clip, const ClipPaintPropertyNode& previous) -> CompositingReasons {
        return transform_compositing_reasons(clip.LocalTransformSpace().Unalias(), previous.LocalTransformSpace().Unalias());
    };

    reasons |= transform_compositing_reasons(transform, previous_layer_state.Transform());
    ...
    if (&effect != &previous_layer_state.Effect()) {
        ...
        if (reasons == CompositingReason::kNone) {
            reasons = transform_compositing_reasons(effect.LocalTransformSpace().Unalias(), previous_layer_state.Effect().LocalTransformSpace().Unalias());
            if (reasons == CompositingReason::kNone && effect.OutputClip() && previous_layer_state.Effect().OutputClip()) {
                reasons = clip_compositing_reasons(effect.OutputClip()->Unalias(), previous_layer_state.Effect().OutputClip()->Unalias());
            }
        }
    }
    if (reasons == CompositingReason::kNone) {
        reasons = clip_compositing_reasons(layer.GetPropertyTreeState().Clip(), previous_layer_state.Clip());
    }

    return reasons;
}

Vector<cc::Layer*> PaintArtifactCompositor::SynthesizedClipLayersForTesting() const
{
...
{
    size_t result = sizeof(*this) + synthesized_clip_cache_.CapacityInBytes() + pending_layers_.CapacityInBytes();
...
bool PaintArtifactCompositor::SetScrollbarNeedsDisplay(CompositorElementId element_id)
{
    DCHECK(root_layer_);
...
bool PaintArtifactCompositor::SetScrollbarSolidColor(CompositorElementId element_id, SkColor4f color)
{
    DCHECK(root_layer_);
    CHECK(ScrollbarDisplayItem::IsScrollbarElementId(element_id));
    if (cc::LayerTreeHost* host = root_layer_->layer_tree_host()) {
        if (cc::Layer* layer = host->LayerByElementId(element_id)) {
            if (static_cast<cc::ScrollbarLayerBase*>(layer)->GetScrollbarLayerType() == cc::ScrollbarLayerBase::kSolidColor) {
...
{
    LOG(INFO) << GetLayersAsJSON(kLayerTreeIncludesDebugInfo | kLayerTreeIncludesDetailedInvalidations)->ToPrettyJSONString().Utf8();
}
#endif

void PaintArtifactCompositor::ForAllContentLayersForTesting(base::FunctionRef<void(ContentLayerClientImpl*)> func) const
{
    for (auto& pending_layer : pending_layers_) {
        if (auto* client = pending_layer.GetContentLayerClient()) {
            func(client);
...
ContentLayerClientImpl* PaintArtifactCompositor::ContentLayerClientForTesting(wtf_size_t i) const
{
    for (auto& pending_layer : pending_layers_) {
        if (auto* client = pending_layer.GetContentLayerClient()) {
            if (i == 0)
