...
public:
    MOCK_METHOD3(DidCompositorScroll, void(CompositorElementId, const gfx::PointF&, const std::optional<cc::TargetSnapAreaElementIds>&));
...
class PaintArtifactCompositorTest : public testing::Test, public PaintTestConfigurations {
protected:
    ...
    void SetUp() override
    {
        ...
        paint_artifact_compositor_ = MakeGarbageCollected<PaintArtifactCompositor>(scroll_callbacks_.GetWeakPtr());
        // Prefer lcd-text by default for tests.
        paint_artifact_compositor_->SetLCDTextPreference(LCDTextPreference::kStronglyPreferred);

        // Uses a LayerTreeHostClient that will make a LayerTreeFrameSink to allow
        // the compositor to run and submit frames.
        layer_tree_ = std::make_unique<LayerTreeHostEmbedder>(&layer_tree_host_client_,
            /*single_thread_client=*/nullptr);
        layer_tree_host_client_.SetLayerTreeHost(layer_tree_->layer_tree_host());
        layer_tree_->layer_tree_host()->SetRootLayer(paint_artifact_compositor_->RootLayer());
    }
    ...
    {
        return *GetPropertyTrees().transform_tree().Node(layer->transform_tree_index());
    }
    ...
    int ElementIdToEffectNodeIndex(CompositorElementId element_id)
    {
        ...
        const auto* node = property_trees->effect_tree().FindNodeFromElementId(element_id);
    ...
    {
        auto* property_trees = layer_tree_->layer_tree_host()->property_trees();
        const auto* node = property_trees->transform_tree().FindNodeFromElementId(element_id);
        return node ? node->id : -1;
    }
    ...
    {
        auto* property_trees = layer_tree_->layer_tree_host()->property_trees();
        const auto* node = property_trees->scroll_tree().FindNodeFromElementId(element_id);
        return node ? node->id : -1;
    }

    using ViewportProperties = PaintArtifactCompositor::ViewportProperties;

    void Update(const PaintArtifact& artifact, const ViewportProperties& viewport_properties = ViewportProperties(),
        const PaintArtifactCompositor::StackScrollTranslationVector& scroll_translation_nodes = {})
    {
        paint_artifact_compositor_->SetNeedsUpdate();
        paint_artifact_compositor_->Update(artifact, viewport_properties, scroll_translation_nodes, {});
    ...
    cc::Layer* RootLayer()
    {
        return paint_artifact_compositor_->RootLayer();
    }
    ...
    size_t LayerCount()
    {
        return RootLayer()->children().size();
    }
    ...
    void UpdateWithEffectivelyInvisibleChunk(bool include_preceding_chunk, bool include_subsequent_chunk)
    {
        TestPaintArtifact artifact;
        if (include_preceding_chunk)
            artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 10, 10), Color::kBlack);
        artifact.Chunk().EffectivelyInvisible().RectDrawing(gfx::Rect(10, 0, 10, 10), Color(255, 0, 0));
        if (include_subsequent_chunk)
            artifact.Chunk().RectDrawing(gfx::Rect(0, 10, 10, 10), Color::kWhite);
        Update(artifact.Build());
    }

    MockScrollCallbacks& ScrollCallbacks()
    {
        return scroll_callbacks_;
    }
...
private:
    ...
    base::SingleThreadTaskRunner::CurrentDefaultHandle task_runner_current_default_handle_;
...
const auto kNotScrollingOnMain = cc::MainThreadScrollingReason::kNotScrollingOnMain;
...
TEST_P(PaintArtifactCompositorTest, OneChunkWithAnOffset)
{
    ...
    EXPECT_THAT(child->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 100, 100), Color::kWhite)));
...
TEST_P(PaintArtifactCompositorTest, OneTransform)
{
    ...
    auto* transform = CreateTransform(t0(), MakeRotationMatrix(90), gfx::Point3F(100, 100, 0), CompositingReason::k3DTransform);

    TestPaintArtifact artifact;
    artifact.Chunk(*transform, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kGray);
    artifact.Chunk(*transform, c0(), e0()).RectDrawing(gfx::Rect(100, 0, 100, 100), Color::kBlack);
    Update(artifact.Build());
    ...
    ASSERT_EQ(2u, LayerCount());
    {
        ...
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 100, 100), Color::kWhite));
        rects_with_color.push_back(RectWithColor(gfx::RectF(100, 0, 100, 100), Color::kBlack));

        EXPECT_THAT(layer->GetPicture(), Pointee(DrawsRectangles(rects_with_color)));
        EXPECT_EQ(gfx::RectF(100, 0, 100, 100), layer->ScreenSpaceTransform().MapRect(gfx::RectF(0, 0, 100, 100)));
    }
    {
        const cc::Layer* layer = LayerAt(1);
        EXPECT_FALSE(GetTransformNode(layer).transform_changed);
        EXPECT_THAT(layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 100, 100), Color::kGray)));
...
{
    // A 90 degree clockwise rotation about (100, 100).
    auto* real_transform = CreateTransform(t0(), MakeRotationMatrix(90), gfx::Point3F(100, 100, 0), CompositingReason::k3DTransform);
...
TEST_P(PaintArtifactCompositorTest, TransformCombining)
{
    ...
    auto* transform1 = CreateTransform(t0(), MakeScaleMatrix(2), gfx::Point3F(10, 10, 0), CompositingReason::k3DTransform);
    auto* transform2 = CreateTransform(*transform1, MakeTranslationMatrix(5, 5), gfx::Point3F(), CompositingReason::kWillChangeTransform);

    TestPaintArtifact artifact;
    artifact.Chunk(*transform1, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 300, 200), Color::kWhite);
    artifact.Chunk(*transform2, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 300, 200), Color::kBlack);
    Update(artifact.Build());
    ...
    ASSERT_EQ(2u, LayerCount());
    {
        const cc::Layer* layer = LayerAt(0);
        EXPECT_TRUE(GetTransformNode(layer).transform_changed);
        EXPECT_THAT(layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 300, 200), Color::kWhite)));
        EXPECT_EQ(gfx::RectF(-10, -10, 600, 400), layer->ScreenSpaceTransform().MapRect(gfx::RectF(0, 0, 300, 200)));
    }
    {
        const cc::Layer* layer = LayerAt(1);
        EXPECT_TRUE(GetTransformNode(layer).transform_changed);
        EXPECT_THAT(layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 300, 200), Color::kBlack)));
        EXPECT_EQ(gfx::RectF(0, 0, 600, 400), layer->ScreenSpaceTransform().MapRect(gfx::RectF(0, 0, 300, 200)));
    }
    EXPECT_NE(LayerAt(0)->transform_tree_index(), LayerAt(1)->transform_tree_index());
}

TEST_P(PaintArtifactCompositorTest, BackfaceVisibility)
{
    TransformPaintPropertyNode::State backface_hidden_state;
    backface_hidden_state.backface_visibility = TransformPaintPropertyNode::BackfaceVisibility::kHidden;
    auto* backface_hidden_transform = TransformPaintPropertyNode::Create(t0(), std::move(backface_hidden_state));

    auto* backface_inherited_transform = TransformPaintPropertyNode::Create(*backface_hidden_transform, TransformPaintPropertyNode::State {});

    TransformPaintPropertyNode::State backface_visible_state;
    backface_visible_state.backface_visibility = TransformPaintPropertyNode::BackfaceVisibility::kVisible;
    auto* backface_visible_transform = TransformPaintPropertyNode::Create(*backface_hidden_transform, std::move(backface_visible_state));

    TestPaintArtifact artifact;
    artifact.Chunk(*backface_hidden_transform, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 300, 200), Color::kWhite);
    artifact.Chunk(*backface_inherited_transform, c0(), e0()).RectDrawing(gfx::Rect(100, 100, 100, 100), Color::kBlack);
    artifact.Chunk(*backface_visible_transform, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 300, 200), Color::kDarkGray);
    Update(artifact.Build());

    ASSERT_EQ(2u, LayerCount());
    EXPECT_THAT(LayerAt(0)->GetPicture(),
        Pointee(DrawsRectangles(
            Vector<RectWithColor> { RectWithColor(gfx::RectF(0, 0, 300, 200), Color::kWhite), RectWithColor(gfx::RectF(100, 100, 100, 100), Color::kBlack) })));
    EXPECT_THAT(LayerAt(1)->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 300, 200), Color::kDarkGray)));
}
...
TEST_P(PaintArtifactCompositorTest, FlattensInheritedTransform)
{
    for (bool transform_is_flattened : { true, false }) {
        ...
        auto* transform2 = CreateTransform(*transform1, MakeRotationMatrix(0, 45, 0));
        TransformPaintPropertyNode::State transform3_state { { MakeRotationMatrix(0, 45, 0) } };
        transform3_state.flattens_inherited_transform = transform_is_flattened;
        auto* transform3 = TransformPaintPropertyNode::Create(*transform2, std::move(transform3_state));

        TestPaintArtifact artifact;
        artifact.Chunk(*transform3, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 300, 200), Color::kWhite);
        Update(artifact.Build());
        ...
        // flattens.
        const cc::TransformNode* transform_node3 = GetPropertyTrees().transform_tree().Node(layer->transform_tree_index());
        EXPECT_EQ(transform_is_flattened, transform_node3->flattens_inherited_transform);
        ...
        gfx::RectF rect = layer->ScreenSpaceTransform().MapRect(gfx::RectF(0, 0, 100, 100));
...
TEST_P(PaintArtifactCompositorTest, FlattensInheritedTransformWithAlias)
{
    for (bool transform_is_flattened : { true, false }) {
        ...
        auto* real_transform2 = CreateTransform(*transform1, MakeRotationMatrix(0, 45, 0));
        auto* transform2 = TransformPaintPropertyNodeAlias::Create(*real_transform2);
        TransformPaintPropertyNode::State transform3_state { { MakeRotationMatrix(0, 45, 0) } };
        transform3_state.flattens_inherited_transform = transform_is_flattened;
        auto* real_transform3 = TransformPaintPropertyNode::Create(*transform2, std::move(transform3_state));
...
TEST_P(PaintArtifactCompositorTest, SortingContextID)
{
    ...
    transform2_state.direct_compositing_reasons = CompositingReason::kWillChangeTransform;
    auto* transform2 = TransformPaintPropertyNode::Create(*transform1, std::move(transform2_state));
    ...
    transform3_state.direct_compositing_reasons = CompositingReason::kWillChangeTransform;
    auto* transform3 = TransformPaintPropertyNode::Create(*transform2, std::move(transform3_state));
    ...
    transform4_state.direct_compositing_reasons = CompositingReason::kWillChangeTransform;
    auto* transform4 = TransformPaintPropertyNode::Create(*transform2, std::move(transform4_state));

    TestPaintArtifact artifact;
    artifact.Chunk(*transform1, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 300, 200), Color::kWhite);
    artifact.Chunk(*transform2, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 300, 200), Color::kLightGray);
    artifact.Chunk(*transform3, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 300, 200), Color::kDarkGray);
    artifact.Chunk(*transform4, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 300, 200), Color::kBlack);
    Update(artifact.Build());
    ...
    EXPECT_THAT(white_layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 300, 200), Color::kWhite)));
    int white_sorting_context_id = GetTransformNode(white_layer).sorting_context_id;
    EXPECT_EQ(0, white_sorting_context_id);

    // The light gray layer is 3D sorted.
    const cc::Layer* light_gray_layer = LayerAt(1);
    EXPECT_THAT(light_gray_layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 300, 200), Color::kLightGray)));
    int light_gray_sorting_context_id = GetTransformNode(light_gray_layer).sorting_context_id;
    ...
    EXPECT_THAT(dark_gray_layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 300, 200), Color::kDarkGray)));
    int dark_gray_sorting_context_id = GetTransformNode(dark_gray_layer).sorting_context_id;
    EXPECT_EQ(light_gray_sorting_context_id, dark_gray_sorting_context_id);
    EXPECT_NE(light_gray_layer->transform_tree_index(), dark_gray_layer->transform_tree_index());

    // The black layer is 3D sorted, but in a separate context from the previous
    // layers.
    const cc::Layer* black_layer = LayerAt(3);
    EXPECT_THAT(black_layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 300, 200), Color::kBlack)));
    int black_sorting_context_id = GetTransformNode(black_layer).sorting_context_id;
...
TEST_P(PaintArtifactCompositorTest, OneClip)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *clip, e0()).RectDrawing(gfx::Rect(220, 80, 300, 200), Color::kBlack);
    Update(artifact.Build());
    ...
    EXPECT_THAT(layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 300, 180), Color::kBlack)));
    EXPECT_EQ(Translation(220, 100), layer->ScreenSpaceTransform());

    const cc::ClipNode* clip_node = GetPropertyTrees().clip_tree().Node(layer->clip_tree_index());
...
{
    auto* transform1 = CreateTransform(t0(), gfx::Transform(), gfx::Point3F(), CompositingReason::kWillChangeTransform);
    auto* clip1 = CreateClip(c0(), *transform1, FloatRoundedRect(100, 100, 700, 700));

    auto* transform2 = CreateTransform(*transform1, gfx::Transform(), gfx::Point3F(), CompositingReason::kWillChangeTransform);
    auto* clip2 = CreateClip(*clip1, *transform2, FloatRoundedRect(200, 200, 700, 700));

    TestPaintArtifact artifact;
    artifact.Chunk(*transform1, *clip1, e0()).RectDrawing(gfx::Rect(300, 350, 100, 100), Color::kWhite);
    artifact.Chunk(*transform2, *clip2, e0()).RectDrawing(gfx::Rect(300, 350, 100, 100), Color::kLightGray);
    artifact.Chunk(*transform1, *clip1, e0()).RectDrawing(gfx::Rect(300, 350, 100, 100), Color::kDarkGray);
    artifact.Chunk(*transform2, *clip2, e0()).RectDrawing(gfx::Rect(300, 350, 100, 100), Color::kBlack);
    Update(artifact.Build());

    ASSERT_EQ(4u, LayerCount());

    const cc::Layer* white_layer = LayerAt(0);
    EXPECT_THAT(white_layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 100, 100), Color::kWhite)));
    EXPECT_EQ(Translation(300, 350), white_layer->ScreenSpaceTransform());

    const cc::Layer* light_gray_layer = LayerAt(1);
    EXPECT_THAT(light_gray_layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 100, 100), Color::kLightGray)));
    EXPECT_EQ(Translation(300, 350), light_gray_layer->ScreenSpaceTransform());

    const cc::Layer* dark_gray_layer = LayerAt(2);
    EXPECT_THAT(dark_gray_layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 100, 100), Color::kDarkGray)));
    EXPECT_EQ(Translation(300, 350), dark_gray_layer->ScreenSpaceTransform());

    const cc::Layer* black_layer = LayerAt(3);
    EXPECT_THAT(black_layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 100, 100), Color::kBlack)));
    ...
    const cc::ClipNode* outer_clip = GetPropertyTrees().clip_tree().Node(white_layer->clip_tree_index());
    EXPECT_TRUE(outer_clip->AppliesLocalClip());
    EXPECT_EQ(gfx::RectF(100, 100, 700, 700), outer_clip->clip);

    EXPECT_EQ(light_gray_layer->clip_tree_index(), black_layer->clip_tree_index());
    const cc::ClipNode* inner_clip = GetPropertyTrees().clip_tree().Node(black_layer->clip_tree_index());
...
{
    auto* transform1 = CreateTransform(t0(), gfx::Transform(), gfx::Point3F(), CompositingReason::kWillChangeTransform);
    auto* real_clip1 = CreateClip(c0(), *transform1, FloatRoundedRect(100, 100, 700, 700));
    auto* clip1 = ClipPaintPropertyNodeAlias::Create(*real_clip1);
    auto* transform2 = CreateTransform(*transform1, gfx::Transform(), gfx::Point3F(), CompositingReason::kWillChangeTransform);
    auto* real_clip2 = CreateClip(*clip1, *transform2, FloatRoundedRect(200, 200, 700, 700));
...
TEST_P(PaintArtifactCompositorTest, DeeplyNestedClips)
{
    ...
    for (unsigned i = 1; i <= 10; i++) {
        clips.push_back(CreateClip(clips.empty() ? c0() : *clips.back(), t0(), FloatRoundedRect(5 * i, 0, 100, 200 - 10 * i)));
    }

    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *clips.back(), e0()).RectDrawing(gfx::Rect(0, 0, 200, 200), Color::kWhite);
    Update(artifact.Build());
    ...
    EXPECT_THAT(drawing_layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 150, 200), Color::kWhite)));
    EXPECT_EQ(Translation(50, 0), drawing_layer->ScreenSpaceTransform());

    // Check the clip nodes.
    const cc::ClipNode* clip_node = GetPropertyTrees().clip_tree().Node(drawing_layer->clip_tree_index());
...
{
    auto* real_common_clip = CreateClip(c0(), t0(), FloatRoundedRect(0, 0, 80, 60));
    auto* common_clip = ClipPaintPropertyNodeAlias::Create(*real_common_clip);
    auto* real_clip1 = CreateClip(*common_clip, t0(), FloatRoundedRect(0, 0, 30, 20));
    auto* clip1 = ClipPaintPropertyNodeAlias::Create(*real_clip1);
    auto* real_clip2 = CreateClip(*common_clip, t0(), FloatRoundedRect(40, 0, 40, 60));
    auto* clip2 = ClipPaintPropertyNodeAlias::Create(*real_clip2);

    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *clip1, e0()).RectDrawing(gfx::Rect(0, 0, 11, 22), Color::kWhite);
    artifact.Chunk(t0(), *clip2, e0()).RectDrawing(gfx::Rect(33, 44, 55, 66), Color::kBlack);
    Update(artifact.Build());
    ...
    EXPECT_THAT(layer->GetPicture(),
        Pointee(DrawsRectangles(Vector<RectWithColor> { // This is the first RectDrawing with real_clip1 applied.
...
{
    auto* t1 = CreateTransform(t0(), gfx::Transform(), gfx::Point3F(), CompositingReason::kWillChangeTransform);
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *c1, e0()).RectDrawing(gfx::Rect(0, 0, 640, 480), Color::kWhite);
    artifact.Chunk(t0(), *c2, e0()).RectDrawing(gfx::Rect(0, 0, 640, 480), Color::kBlack);
    Update(artifact.Build());
...
{
    auto* real_common_transform = CreateTransform(t0(), MakeTranslationMatrix(5, 6));
    auto* common_transform = TransformPaintPropertyNodeAlias::Create(*real_common_transform);
    auto* real_transform1 = CreateTransform(*common_transform, MakeScaleMatrix(2));
    auto* transform1 = TransformPaintPropertyNodeAlias::Create(*real_transform1);
    auto* real_transform2 = CreateTransform(*common_transform, MakeScaleMatrix(0.5));
    auto* transform2 = TransformPaintPropertyNodeAlias::Create(*real_transform2);

    TestPaintArtifact artifact;
    artifact.Chunk(*transform1, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 111, 222), Color::kWhite);
    artifact.Chunk(*transform2, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 333, 444), Color::kBlack);
    Update(artifact.Build());
    ...
    EXPECT_THAT(layer->GetPicture(),
        Pointee(DrawsRectangles(
            Vector<RectWithColor> { RectWithColor(gfx::RectF(0, 0, 222, 444), Color::kWhite), RectWithColor(gfx::RectF(0, 0, 166.5, 222), Color::kBlack) })));
...
TEST_P(PaintArtifactCompositorTest, SiblingTransformsWithComposited)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(*t2, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 640, 480), Color::kWhite);
    artifact.Chunk(*t3, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 640, 480), Color::kBlack);
    Update(artifact.Build());
...
{
    Update(TestPaintArtifact().Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite).Build());
    auto root_element_id = GetPropertyTrees().effect_tree().Node(1)->element_id;

    auto* real_effect1 = CreateOpacityEffect(e0(), t0(), &c0(), 0.5, CompositingReason::kAll);
    auto* effect1 = EffectPaintPropertyNodeAlias::Create(*real_effect1);
    auto* real_effect2 = CreateOpacityEffect(*effect1, 0.3, CompositingReason::kAll);
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(t0(), c0(), *effect2).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    artifact.Chunk(t0(), c0(), *effect1).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    artifact.Chunk(t0(), c0(), *effect3).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    Update(artifact.Build());
    ...
    EXPECT_EQ(real_effect1->GetCompositorElementId(), converted_effect1.element_id);
...
static PropertyTreeState ScrollState1(const PropertyTreeState& parent_state = PropertyTreeState::Root(),
    CompositingReasons compositing_reasons = CompositingReason::kOverflowScrolling, MainThreadScrollingReasons main_thread_reasons = kNotScrollingOnMain)
{
    return CreateScrollTranslationState(parent_state, 7, 9, gfx::Rect(3, 5, 11, 13), gfx::Size(27, 31), compositing_reasons, main_thread_reasons);
}

// Returns a RefCountedPropertyTreeState for composited scrolling paint
// properties with another set of arbitrary values.
static PropertyTreeState ScrollState2(const PropertyTreeState& parent_state = PropertyTreeState::Root(),
    CompositingReasons compositing_reasons = CompositingReason::kOverflowScrolling, MainThreadScrollingReasons main_thread_reasons = kNotScrollingOnMain)
{
    return CreateScrollTranslationState(parent_state, 39, 31, gfx::Rect(0, 0, 19, 23), gfx::Size(27, 31), compositing_reasons, main_thread_reasons);
}

static void CheckCcScrollNode(const ScrollPaintPropertyNode& blink_scroll, const cc::ScrollNode& cc_scroll)
{
    ...
    EXPECT_EQ(blink_scroll.UserScrollableHorizontal(), cc_scroll.user_scrollable_horizontal);
    EXPECT_EQ(blink_scroll.UserScrollableVertical(), cc_scroll.user_scrollable_vertical);
    EXPECT_EQ(blink_scroll.GetCompositorElementId(), cc_scroll.element_id);
    EXPECT_EQ(blink_scroll.GetMainThreadRepaintReasons(), cc_scroll.main_thread_repaint_reasons);
}
...
TEST_P(PaintArtifactCompositorTest, OneScrollNodeComposited)
{
    ...
    Update(TestPaintArtifact().ScrollHitTestChunk(scroll_state).Chunk(scroll_state).RectDrawing(gfx::Rect(-110, 12, 170, 19), Color::kWhite).Build());
    ...
    const cc::TransformNode& transform_node = *transform_tree.Node(scroll_node.transform_id);
    ...
    EXPECT_THAT(layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 57, 19), Color::kWhite)));
    ...
    EXPECT_CALL(ScrollCallbacks(), DidCompositorScroll(scroll_node.element_id, gfx::PointF(1, 2), targets));
    GetPropertyTrees().scroll_tree_mutable().NotifyDidCompositorScroll(scroll_node.element_id, gfx::PointF(1, 2), targets);

    EXPECT_CALL(ScrollCallbacks(), DidChangeScrollbarsHidden(scroll_node.element_id, true));
    GetPropertyTrees().scroll_tree_mutable().NotifyDidChangeScrollbarsHidden(scroll_node.element_id, true);
}

TEST_P(PaintArtifactCompositorTest, OneScrollNodeNonComposited)
{
    auto scroll_state = ScrollState1(PropertyTreeState::Root(), CompositingReason::kNone);
...
{
    auto scroll_state = ScrollState1();
    auto* transform = CreateTransform(scroll_state.Transform(), gfx::Transform(), gfx::Point3F(), CompositingReason::kWillChangeTransform);
    ...
    EXPECT_THAT(layer0->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 37, 7), Color::kBlack)));
    ...
    EXPECT_THAT(layer1->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 5, 70), Color::kWhite)));

    const cc::TransformTree& transform_tree = GetPropertyTrees().transform_tree();
    const cc::TransformNode& scroll_transform_node = *transform_tree.Node(scroll_node.transform_id);
...
TEST_P(PaintArtifactCompositorTest, NestedScrollNodes)
{
    ...
    auto scroll_state_a = ScrollState1(PropertyTreeState(t0(), c0(), *effect), cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects);
    ...
    Update(TestPaintArtifact().ScrollChunks(scroll_state_a).ScrollChunks(scroll_state_b).Build());
    ...
    EXPECT_EQ(scroll_node_a.id, ElementIdToScrollNodeIndex(scroll_node_a.element_id));

    const cc::TransformTree& transform_tree = GetPropertyTrees().transform_tree();
    const cc::TransformNode& transform_node_a = *transform_tree.Node(scroll_node_a.transform_id);
    ...
    EXPECT_EQ(scroll_node_b.id, ElementIdToScrollNodeIndex(scroll_node_b.element_id));

    const cc::TransformNode& transform_node_b = *transform_tree.Node(scroll_node_b.transform_id);
...
TEST_P(PaintArtifactCompositorTest, ScrollHitTestLayerOrder)
{
    ...
    auto* transform = CreateTransform(scroll_state.Transform(), MakeTranslationMatrix(5, 5), gfx::Point3F(), CompositingReason::k3DTransform);
    ...
    EXPECT_LT(LayerIndex(NonScrollHitTestLayerAt(0)), LayerIndex(ScrollHitTestLayerAt(0)));
    const cc::ScrollTree& scroll_tree = GetPropertyTrees().scroll_tree();
    auto* scroll_node = scroll_tree.Node(ScrollHitTestLayerAt(0)->scroll_tree_index());
    ASSERT_EQ(scroll.GetCompositorElementId(), scroll_node->element_id);
    EXPECT_EQ(scroll.GetCompositorElementId(), ScrollHitTestLayerAt(0)->element_id());
    EXPECT_EQ(RuntimeEnabledFeatures::HitTestOpaquenessEnabled() ? cc::HitTestOpaqueness::kOpaque : cc::HitTestOpaqueness::kMixed,
        ScrollHitTestLayerAt(0)->hit_test_opaqueness());

    // The second content layer should appear after the first.
    EXPECT_LT(LayerIndex(ScrollHitTestLayerAt(0)), LayerIndex(NonScrollHitTestLayerAt(1)));
...
TEST_P(PaintArtifactCompositorTest, NestedScrollableLayerOrder)
{
    ...
    auto* scroll_1_node = scroll_tree.Node(ScrollHitTestLayerAt(0)->scroll_tree_index());
    ASSERT_EQ(scroll_1.GetCompositorElementId(), scroll_1_node->element_id);
    auto* scroll_1_clip_node = clip_tree.Node(ScrollHitTestLayerAt(0)->clip_tree_index());
    // The scroll is not under clip_1.
    EXPECT_EQ(gfx::RectF(0, 0, 0, 0), scroll_1_clip_node->clip);

    auto* scroll_2_node = scroll_tree.Node(ScrollHitTestLayerAt(1)->scroll_tree_index());
    ASSERT_EQ(scroll_2.GetCompositorElementId(), scroll_2_node->element_id);
    auto* scroll_2_clip_node = clip_tree.Node(ScrollHitTestLayerAt(1)->clip_tree_index());
    ...
    EXPECT_LT(LayerIndex(ScrollHitTestLayerAt(0)), LayerIndex(ScrollHitTestLayerAt(1)));

    // The non-scrollable content layer should be after the second scroll layer.
    EXPECT_LT(LayerIndex(ScrollHitTestLayerAt(1)), LayerIndex(NonScrollHitTestLayerAt(0)));

    auto expected_hit_test_opaqueness = RuntimeEnabledFeatures::HitTestOpaquenessEnabled() ? cc::HitTestOpaqueness::kOpaque : cc::HitTestOpaqueness::kMixed;
    EXPECT_EQ(expected_hit_test_opaqueness, ScrollHitTestLayerAt(0)->hit_test_opaqueness());
    EXPECT_EQ(expected_hit_test_opaqueness, ScrollHitTestLayerAt(1)->hit_test_opaqueness());
}
...
TEST_P(PaintArtifactCompositorTest, AncestorScrollNodes)
{
    ...
    auto& scroll_a = *scroll_state_a.Transform().ScrollNode();
    auto scroll_state_b = ScrollState2(scroll_state_a, CompositingReason::kNone, cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText);
    auto& scroll_b = *scroll_state_b.Transform().ScrollNode();
    ...
    EXPECT_TRUE(transform_node_a.local.IsIdentity());
    EXPECT_EQ(gfx::PointF(-7, -9), transform_node_a.scroll_offset);
    EXPECT_EQ(gfx::PointF(-7, -9), scroll_tree.current_scroll_offset(scroll_node_a.element_id));
    ...
    EXPECT_TRUE(transform_node_b.local.IsIdentity());
    EXPECT_EQ(gfx::PointF(-39, -31), transform_node_b.scroll_offset);
    EXPECT_EQ(gfx::PointF(-39, -31), scroll_tree.current_scroll_offset(scroll_node_b.element_id));
}

TEST_P(PaintArtifactCompositorTest, AncestorNonCompositedScrollNode)
{
    auto scroll_state_a = ScrollState1(PropertyTreeState::Root(), CompositingReason::kNone, cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText);
    auto& scroll_a = *scroll_state_a.Transform().ScrollNode();
    auto scroll_state_b = ScrollState2(scroll_state_a);
    auto& scroll_b = *scroll_state_b.Transform().ScrollNode();
...
TEST_P(PaintArtifactCompositorTest, AncestorScrollNodesInversedOrder)
{
    ...
    Update(TestPaintArtifact().ScrollChunks(scroll_state_b).ScrollChunks(scroll_state_a).Build());
...
TEST_P(PaintArtifactCompositorTest, DifferentTransformTreeAndScrollTreeHierarchy)
{
    auto scroll_state_a = ScrollState1();
    auto& scroll_a = *scroll_state_a.Transform().ScrollNode();
    auto scroll_state_b = ScrollState2(scroll_state_a);
    ...
    // scroll_b.
    auto scroll_state_c = CreateCompositedScrollTranslationState(PropertyTreeState::Root(), scroll_b, 11, 22, gfx::Rect(0, 0, 10, 20), gfx::Size(50, 60));
    auto& scroll_c = *scroll_state_c.Transform().ScrollNode();

    Update(TestPaintArtifact().ScrollChunks(scroll_state_a).ScrollChunks(scroll_state_b).ScrollChunks(scroll_state_c).Build());
    ...
    EXPECT_EQ(scroll_node_c.id, ElementIdToScrollNodeIndex(scroll_node_c.element_id));
    EXPECT_EQ(scroll_node_c.element_id, ScrollHitTestLayerAt(2)->element_id());

    const cc::TransformNode& transform_node_c = *transform_tree.Node(scroll_node_c.transform_id);
    ...
    EXPECT_EQ(gfx::PointF(-11, -22), scroll_tree.current_scroll_offset(scroll_node_c.element_id));
}

TEST_P(PaintArtifactCompositorTest, DifferentTransformTreeAndScrollTreeHierarchyInversedOrder)
{
    auto scroll_state_a = ScrollState1();
    auto& scroll_a = *scroll_state_a.Transform().ScrollNode();
    auto scroll_state_b = ScrollState2(scroll_state_a);
    ...
    Update(TestPaintArtifact().ScrollChunks(scroll_state_c).ScrollChunks(scroll_state_b).ScrollChunks(scroll_state_a).Build());
...
TEST_P(PaintArtifactCompositorTest, FixedPositionScrollState)
{
    auto scroll_state_a = ScrollState1();
    auto& scroll_a = *scroll_state_a.Transform().ScrollNode();
    auto* fixed_transform = CreateFixedPositionTranslation(t0(), 100, 200, scroll_state_a.Transform());
    PropertyTreeState fixed_state(*fixed_transform, scroll_state_a.Clip(), scroll_state_a.Effect());
    // scroll_state_b's has fixed transform space, while the scroll parent is
    // scroll_a.
    auto scroll_state_b = CreateCompositedScrollTranslationState(fixed_state, scroll_a, 11, 22, gfx::Rect(10, 20), gfx::Size(50, 60));
    auto& scroll_b = *scroll_state_b.Transform().ScrollNode();
...
TEST_P(PaintArtifactCompositorTest, NonScrollingChildUnderFixedPosition)
{
    ...
    auto* fixed_transform = CreateFixedPositionTranslation(scroll_state_b.Transform(), 100, 200, scroll_state_a.Transform());
    PropertyTreeState fixed_state(*fixed_transform, scroll_state_a.Clip(), scroll_state_a.Effect());
    // Use kActiveOpacityAnimation to avoid being decomposited with parent
    // transform.
    auto* child_transform
        = CreateTransform(*fixed_transform, MakeTranslationMatrix(50, 50), gfx::Point3F(100, 100, 0), CompositingReason::kActiveOpacityAnimation);
    // Use a different effect to get a different layer.
    auto* child_effect = CreateOpacityEffect(fixed_state.Effect(), 0.5f);
    PropertyTreeState child_state(*child_transform, fixed_state.Clip(), *child_effect);
...
TEST_P(PaintArtifactCompositorTest, MergeSimpleChunks)
{
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        Vector<RectWithColor> rects_with_color;
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 100, 100), Color::kWhite));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, MergeClip)
{
    ...
    TestPaintArtifact test_artifact;
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    test_artifact.Chunk(t0(), *clip, e0()).RectDrawing(gfx::Rect(0, 0, 200, 300), Color::kBlack);
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        rects_with_color.push_back(RectWithColor(gfx::RectF(10, 20, 50, 60), Color::kBlack));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 300, 400), Color::kGray));
...
{
    auto* transform = CreateTransform(t0(), MakeTranslationMatrix(50, 50), gfx::Point3F(100, 100, 0));

    TestPaintArtifact test_artifact;
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    test_artifact.Chunk(*transform, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 200, 300), Color::kGray);
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        rects_with_color.push_back(RectWithColor(gfx::RectF(50, 50, 100, 100), Color::kBlack));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
{
    auto* transform = CreateTransform(t0(), gfx::Transform(), gfx::Point3F(), CompositingReason::k3DTransform);
    auto* transform2 = CreateTransform(*transform, MakeTranslationMatrix(50, 50), gfx::Point3F(100, 100, 0));

    TestPaintArtifact test_artifact;
    test_artifact.Chunk(*transform, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    ...
    test_artifact.Chunk(*transform2, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
...
{
    auto* transform = CreateTransform(t0(), MakeRotationMatrix(45), gfx::Point3F(100, 100, 0));
    ...
    {
        Vector<RectWithColor> rects_with_color;
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 42, 100, 100), Color::kWhite));
        // Transform is applied to this PaintChunk.
        rects_with_color.push_back(RectWithColor(gfx::RectF(29.2893, 0.578644, 141.421, 141.421), Color::kBlack));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 42, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, MergeOpacity)
{
    ...
    TestPaintArtifact test_artifact;
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    test_artifact.Chunk(t0(), c0(), *effect).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 200, 300), Color::kGray);
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 100, 100), semi_transparent_black));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, MergeNestedWithAlias)
{
    ...
    // transform.
    auto* real_transform = CreateTransform(t0(), MakeTranslationMatrix(50, 50), gfx::Point3F(100, 100, 0));
    auto* transform = TransformPaintPropertyNodeAlias::Create(*real_transform);
    auto* real_clip = CreateClip(c0(), *transform, FloatRoundedRect(10, 20, 50, 60));
    auto* clip = ClipPaintPropertyNodeAlias::Create(*real_clip);
    float opacity = 2.0 / 255.0;
    ...
    TestPaintArtifact test_artifact;
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    test_artifact.Chunk(*transform, *clip, *effect).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 200, 300), Color::kGray);
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        // Transform is applied to this PaintChunk.
        Color semi_transparent_black = Color::FromSkColor4f({ 0, 0, 0, opacity });
        rects_with_color.push_back(RectWithColor(gfx::RectF(60, 70, 50, 60), semi_transparent_black));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, ClipPushedUp)
{
    ...
    auto* transform = CreateTransform(t0(), MakeTranslationMatrix(20, 25), gfx::Point3F(100, 100, 0));
    auto* transform2 = CreateTransform(*transform, MakeTranslationMatrix(20, 25), gfx::Point3F(100, 100, 0));
    auto* clip = CreateClip(c0(), *transform2, FloatRoundedRect(10, 20, 50, 60));

    TestPaintArtifact test_artifact;
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    test_artifact.Chunk(t0(), *clip, e0()).RectDrawing(gfx::Rect(0, 0, 300, 400), Color::kBlack);
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 200, 300), Color::kGray);
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        // before clipping.
        rects_with_color.push_back(RectWithColor(gfx::RectF(50, 70, 50, 60), Color(Color::kBlack)));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, EffectPushedUp)
{
    ...
    TestPaintArtifact test_artifact;
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    test_artifact.Chunk(t0(), c0(), *effect).RectDrawing(gfx::Rect(0, 0, 300, 400), Color::kBlack);
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 200, 300), Color::kGray);
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 100, 100), Color::kWhite));
        Color semi_transparent_black = Color::FromSkColor4f({ 0, 0, 0, opacity });
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 300, 400), semi_transparent_black));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, EffectAndClipPushedUp)
{
    ...
    TestPaintArtifact test_artifact;
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    test_artifact.Chunk(t0(), *clip, *effect).RectDrawing(gfx::Rect(0, 0, 300, 400), Color::kBlack);
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 200, 300), Color::kGray);
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        // of (20, 25) occurs.
        Color semi_transparent_black = Color::FromSkColor4f({ 0, 0, 0, opacity });
        rects_with_color.push_back(RectWithColor(gfx::RectF(30, 45, 50, 60), semi_transparent_black));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, ClipAndEffectNoTransform)
{
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 100, 100), Color::kWhite));
        Color semi_transparent_black = Color::FromSkColor4f({ 0, 0, 0, opacity });
        rects_with_color.push_back(RectWithColor(gfx::RectF(10, 20, 50, 60), semi_transparent_black));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, TwoClips)
{
    ...
    TestPaintArtifact test_artifact;
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    test_artifact.Chunk(t0(), *clip2, e0()).RectDrawing(gfx::Rect(0, 0, 300, 400), Color::kBlack);
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 200, 300), Color::kGray);
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        rects_with_color.push_back(RectWithColor(gfx::RectF(20, 30, 10, 20), Color(Color::kBlack)));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, TwoTransformsClipBetween)
{
    ...
    TestPaintArtifact test_artifact;
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    test_artifact.Chunk(*transform2, *clip, e0()).RectDrawing(gfx::Rect(0, 0, 300, 400), Color::kBlack);
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 200, 300), Color::kGray);
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        Vector<RectWithColor> rects_with_color;
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 100, 100), Color::kWhite));
        rects_with_color.push_back(RectWithColor(gfx::RectF(40, 50, 10, 10), Color(Color::kBlack)));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
        const cc::Layer* layer = LayerAt(0);
        EXPECT_THAT(layer->GetPicture(), Pointee(DrawsRectangles(rects_with_color)));
    }
...
{
    auto* transform = CreateTransform(t0(), MakeTranslationMatrix(50, 50), gfx::Point3F(100, 100, 0), CompositingReason::k3DTransform);
...
EffectPaintPropertyNode* CreateSampleEffectNodeWithElementId()
{
    ...
    state.compositor_element_id = CompositorElementIdFromUniqueObjectId(2, CompositorElementIdNamespace::kPrimaryEffect);
...
TransformPaintPropertyNode* CreateSampleTransformNodeWithElementId()
{
    ...
    state.compositor_element_id = CompositorElementIdFromUniqueObjectId(3, CompositorElementIdNamespace::kPrimaryTransform);
...
TEST_P(PaintArtifactCompositorTest, TransformWithElementId)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(*transform, c0(), e0()).RectDrawing(gfx::Rect(100, 100, 200, 100), Color::kBlack);
    Update(artifact.Build());

    EXPECT_EQ(2, ElementIdToTransformNodeIndex(transform->GetCompositorElementId()));
}
...
TEST_P(PaintArtifactCompositorTest, EffectWithElementId)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(t0(), c0(), *effect).RectDrawing(gfx::Rect(100, 100, 200, 100), Color::kBlack);
    Update(artifact.Build());
...
TEST_P(PaintArtifactCompositorTest, NonContiguousEffectWithElementId)
{
    ...
    EXPECT_EQ(element_id, GetPropertyTrees().effect_tree().Node(LayerAt(0)->effect_tree_index())->element_id);
    EXPECT_EQ(1, LayerAt(1)->effect_tree_index());
    EXPECT_EQ(3, LayerAt(2)->effect_tree_index());
    EXPECT_NE(element_id, GetPropertyTrees().effect_tree().Node(LayerAt(2)->effect_tree_index())->element_id);
}
...
TEST_P(PaintArtifactCompositorTest, EffectWithElementIdWithAlias)
{
    ...
    EXPECT_EQ(2, ElementIdToEffectNodeIndex(real_effect->GetCompositorElementId()));
}

TEST_P(PaintArtifactCompositorTest, NonCompositedSimpleMask)
{
    auto* masked = CreateOpacityEffect(e0(), 1.0, CompositingReason::kWillChangeOpacity);
    ...
    auto* masking = EffectPaintPropertyNode::Create(*masked, std::move(masking_state));

    TestPaintArtifact artifact;
    artifact.Chunk(t0(), c0(), *masked).RectDrawing(gfx::Rect(100, 100, 200, 200), Color::kGray);
    artifact.Chunk(t0(), c0(), *masking).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite);
    Update(artifact.Build());
    ASSERT_EQ(1u, LayerCount());
    ...
    EXPECT_THAT(*layer->GetPicture(),
        DrawsRectangles(
            Vector<RectWithColor> { RectWithColor(gfx::RectF(0, 0, 200, 200), Color::kGray), RectWithColor(gfx::RectF(50, 50, 100, 100), Color::kWhite) }));
    EXPECT_EQ(Translation(100, 100), layer->ScreenSpaceTransform());
    EXPECT_EQ(gfx::Size(200, 200), layer->bounds());
    const cc::EffectNode* masked_group = GetPropertyTrees().effect_tree().Node(layer->effect_tree_index());
...
TEST_P(PaintArtifactCompositorTest, CompositedMaskOneChild)
{
    ...
    masking_state.output_clip = &c0();
    masking_state.blend_mode = SkBlendMode::kDstIn;
    masking_state.direct_compositing_reasons = CompositingReason::kWillChangeOpacity;
    auto* masking = EffectPaintPropertyNode::Create(*masked, std::move(masking_state));
    ...
    EXPECT_TRUE(GetPropertyTrees().effect_tree().parent(masking_group)->HasRenderSurface());
}
...
TEST_P(PaintArtifactCompositorTest, CompositedMaskTwoChildren)
{
    ...
    auto* child_of_masked = CreateOpacityEffect(*masking, 1.0, CompositingReason::kWillChangeOpacity);

    TestPaintArtifact artifact;
    artifact.Chunk(t0(), c0(), *masked).RectDrawing(gfx::Rect(100, 100, 200, 200), Color::kGray);
    artifact.Chunk(t0(), c0(), *child_of_masked).RectDrawing(gfx::Rect(100, 100, 200, 200), Color::kGray);
    artifact.Chunk(t0(), c0(), *masking).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite);
    Update(artifact.Build());
    ASSERT_EQ(3u, LayerCount());
...
TEST_P(PaintArtifactCompositorTest, NonCompositedSimpleExoticBlendMode)
{
    ...
    const cc::Layer* layer = LayerAt(0);
    const cc::EffectNode* group = GetPropertyTrees().effect_tree().Node(layer->effect_tree_index());
...
TEST_P(PaintArtifactCompositorTest, CompositedExoticBlendModeOnTwoOpacityAnimationLayers)
{
    auto* masked = CreateOpacityEffect(e0(), 1.0, CompositingReason::kWillChangeOpacity);
    auto* masked_child1 = CreateOpacityEffect(*masked, 1.0, CompositingReason::kActiveOpacityAnimation);
    auto* masked_child2 = CreateOpacityEffect(*masked, 1.0, CompositingReason::kActiveOpacityAnimation);
    EffectPaintPropertyNode::State masking_state;
    masking_state.local_transform_space = &t0();
    masking_state.output_clip = &c0();
    masking_state.blend_mode = SkBlendMode::kXor;
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(t0(), c0(), *masked_child1).RectDrawing(gfx::Rect(100, 100, 200, 200), Color::kGray);
    artifact.Chunk(t0(), c0(), *masked_child2).RectDrawing(gfx::Rect(100, 100, 200, 200), Color::kBlack);
    artifact.Chunk(t0(), c0(), *masking).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite);
    Update(artifact.Build());
    ASSERT_EQ(3u, LayerCount());
...
TEST_P(PaintArtifactCompositorTest, CompositedExoticBlendModeOnTwo3DTransformLayers)
{
    auto* masked = CreateOpacityEffect(e0(), 1.0, CompositingReason::kWillChangeOpacity);
    auto* transform1 = CreateTransform(t0(), gfx::Transform(), gfx::Point3F(), CompositingReason::k3DTransform);
    auto* transform2 = CreateTransform(t0(), gfx::Transform(), gfx::Point3F(), CompositingReason::k3DTransform);
    EffectPaintPropertyNode::State masking_state;
    masking_state.local_transform_space = &t0();
    masking_state.output_clip = &c0();
    masking_state.blend_mode = SkBlendMode::kXor;
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(*transform1, c0(), *masked).RectDrawing(gfx::Rect(100, 100, 200, 200), Color::kGray);
    artifact.Chunk(*transform2, c0(), *masked).RectDrawing(gfx::Rect(100, 100, 200, 200), Color::kBlack);
    artifact.Chunk(t0(), c0(), *masking).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite);
    Update(artifact.Build());
    ASSERT_EQ(3u, LayerCount());
...
{
    auto* parent_effect = CreateOpacityEffect(e0(), 1.0, CompositingReason::kWillChangeOpacity);
    ...
    auto* blend_effect1 = EffectPaintPropertyNode::Create(*parent_effect, std::move(blend_state1));
    ...
    auto* blend_effect2 = EffectPaintPropertyNode::Create(*parent_effect, std::move(blend_state2));
    ...
    ASSERT_EQ(1u, LayerCount());
    const auto* effect = GetPropertyTrees().effect_tree().Node(LayerAt(0)->effect_tree_index());
...
TEST_P(PaintArtifactCompositorTest, DecompositeExoticBlendModeWithNonDrawingLayer)
{
    auto* parent_effect = CreateOpacityEffect(e0(), 1.0, CompositingReason::kWillChangeOpacity);
    EffectPaintPropertyNode::State blend_state1;
    blend_state1.local_transform_space = &t0();
    ...
    const auto* effect = GetPropertyTrees().effect_tree().Node(LayerAt(1)->effect_tree_index());
    EXPECT_EQ(1.0f, effect->opacity);
    EXPECT_EQ(SkBlendMode::kSrcOver, effect->blend_mode);
    // Don't need a render surface because all blend effects are decomposited.
    EXPECT_FALSE(effect->HasRenderSurface());
...
TEST_P(PaintArtifactCompositorTest, UpdateProducesNewSequenceNumber)
{
    ...
    TestPaintArtifact test_artifact;
    test_artifact.Chunk(*transform, *clip, *effect).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    test_artifact.Chunk().RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kGray);
    auto& artifact = test_artifact.Build();
...
TEST_P(PaintArtifactCompositorTest, DecompositeClip)
{
    ...
    artifact.Chunk(t0(), *clip, e0()).RectDrawing(gfx::Rect(100, 100, 100, 100), Color::kGray);
    Update(artifact.Build());
    ASSERT_EQ(1u, LayerCount());
...
TEST_P(PaintArtifactCompositorTest, DecompositeEffect)
{
    ...
    artifact.Chunk(t0(), c0(), *effect).RectDrawing(gfx::Rect(25, 75, 100, 100), Color::kGray);
...
TEST_P(PaintArtifactCompositorTest, DirectlyCompositedEffect)
{
    ...
    const cc::EffectNode* effect_node = GetPropertyTrees().effect_tree().Node(layer2->effect_tree_index());
...
TEST_P(PaintArtifactCompositorTest, DecompositeDeepEffect)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk().RectDrawing(gfx::Rect(50, 25, 100, 100), Color::kGray);
    artifact.Chunk(t0(), c0(), *effect3).RectDrawing(gfx::Rect(25, 75, 100, 100), Color::kGray);
    artifact.Chunk().RectDrawing(gfx::Rect(75, 75, 100, 100), Color::kGray);
    Update(artifact.Build());
    ASSERT_EQ(3u, LayerCount());
    ...
    EXPECT_EQ(gfx::Vector2dF(25.f, 75.f), layer2->offset_to_transform_parent());
    EXPECT_EQ(gfx::Size(100, 100), layer2->bounds());
    const cc::EffectNode* effect_node2 = GetPropertyTrees().effect_tree().Node(layer2->effect_tree_index());
    EXPECT_EQ(0.2f, effect_node2->opacity);
    const cc::EffectNode* effect_node1 = GetPropertyTrees().effect_tree().Node(effect_node2->parent_id);
...
TEST_P(PaintArtifactCompositorTest, IndirectlyCompositedEffect)
{
    ...
    artifact.Chunk().RectDrawing(gfx::Rect(50, 25, 100, 100), Color::kGray);
    artifact.Chunk(t0(), c0(), *effect).RectDrawing(gfx::Rect(25, 75, 100, 100), Color::kGray);
    artifact.Chunk(*transform, c0(), *effect).RectDrawing(gfx::Rect(75, 75, 100, 100), Color::kGray);
    Update(artifact.Build());
    ASSERT_EQ(3u, LayerCount());
...
TEST_P(PaintArtifactCompositorTest, DecompositedEffectNotMergingDueToOverlap)
{
    ...
    artifact.Chunk(t0(), c0(), *effect1).RectDrawing(gfx::Rect(50, 0, 40, 40), Color::kGray);
    // This chunk has a transform that must be composited, thus causing effect1
    // to be composited too.
    artifact.Chunk(*transform, c0(), *effect1).RectDrawing(gfx::Rect(100, 0, 40, 40), Color::kGray);
    artifact.Chunk(t0(), c0(), *effect2).RectDrawing(gfx::Rect(100, 50, 40, 40), Color::kGray);
    ...
    artifact.Chunk(t0(), c0(), *effect2).RectDrawing(gfx::Rect(50, 0, 40, 40), Color::kGray);
...
{
    TestPaintArtifact artifact;
    artifact.Chunk().EffectivelyInvisible().RectDrawing(gfx::Rect(10, 0, 10, 10), Color(255, 0, 0)).IsSolidColor();
    Update(artifact.Build());
    ASSERT_EQ(1u, LayerCount());
    EXPECT_EQ(gfx::Size(10, 10), LayerAt(0)->bounds());
...
TEST_P(PaintArtifactCompositorTest, EffectivelyInvisibleChunkWithPrecedingChunk)
{
    ...
    EXPECT_THAT(LayerAt(0)->GetPicture(), Pointee(DrawsRectangles({ RectWithColor(gfx::RectF(0, 0, 10, 10), Color::kBlack) })));
}

TEST_P(PaintArtifactCompositorTest, EffectivelyInvisibleChunkWithSubsequentChunk)
{
    UpdateWithEffectivelyInvisibleChunk(false, true);
    ASSERT_EQ(1u, LayerCount());
    EXPECT_EQ(gfx::Size(20, 20), LayerAt(0)->bounds());
    EXPECT_TRUE(LayerAt(0)->draws_content());
    EXPECT_THAT(LayerAt(0)->GetPicture(), Pointee(DrawsRectangles({ RectWithColor(gfx::RectF(0, 10, 10, 10), Color::kWhite) })));
}

TEST_P(PaintArtifactCompositorTest, EffectivelyInvisibleChunkWithPrecedingAndSubsequentChunks)
{
    ...
    EXPECT_TRUE(LayerAt(0)->draws_content());
    EXPECT_THAT(LayerAt(0)->GetPicture(),
        Pointee(DrawsRectangles({ RectWithColor(gfx::RectF(0, 0, 10, 10), Color::kBlack), RectWithColor(gfx::RectF(0, 10, 10, 10), Color::kWhite) })));
}
...
TEST_P(PaintArtifactCompositorTest, UpdateManagesLayerElementIds)
{
    ...
    {
        TestPaintArtifact artifact;
        artifact.Chunk(*transform, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);

        Update(artifact.Build());
        ASSERT_EQ(1u, LayerCount());
        ASSERT_TRUE(GetLayerTreeHost().IsElementInPropertyTrees(element_id, cc::ElementListType::ACTIVE));
    }

    {
        ...
        ASSERT_FALSE(GetLayerTreeHost().IsElementInPropertyTrees(element_id, cc::ElementListType::ACTIVE));
    }
}
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipSimple)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *c1, e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    Update(artifact.Build());
    ...
    const cc::EffectNode& mask_isolation_0 = *GetPropertyTrees().effect_tree().Node(mask_isolation_0_id);
    ...
    EXPECT_EQ(gfx::RRectF(50, 50, 300, 200, 5), mask_isolation_0.mask_filter_info.rounded_corner_bounds());
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipRotatedNotSupported)
{
    ...
    auto* transform = CreateTransform(t0(), MakeRotationMatrix(45), gfx::Point3F(100, 100, 0), CompositingReason::k3DTransform);

    FloatRoundedRect rrect(gfx::RectF(50, 50, 300, 200), 5);
    auto* c1 = CreateClip(c0(), *transform, rrect);

    TestPaintArtifact artifact;
    artifact.Chunk(*transform, *c1, e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    Update(artifact.Build());
    ...
    const cc::EffectNode& mask_effect_0 = *GetPropertyTrees().effect_tree().Node(mask_effect_0_id);
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipShaderBasedBorderRadiusNotSupported2)
{
    // This tests the simplest case that a single layer needs to be clipped
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipSimpleShaderBasedBorderRadiusNotSupportedMacNonEqualCorners)
{
    // Tests that on Mac, we fall back to a mask layer if the corners are not all
    // the same radii.
    gfx::SizeF corner(30, 30);
    FloatRoundedRect rrect(gfx::RectF(50, 50, 300, 200), corner, corner, corner, gfx::SizeF());
    auto* c1 = CreateClip(c0(), t0(), rrect);
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipNested)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *c1, *filter).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    artifact.Chunk(*t1, *c3, *filter).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    Update(artifact.Build());
    ...
    const cc::ClipNode& cc_c2 = *GetPropertyTrees().clip_tree().Node(cc_c3.parent_id);
    ...
    const cc::EffectNode& mask_isolation_2 = *GetPropertyTrees().effect_tree().Node(mask_isolation_2_id);
    const cc::EffectNode& mask_isolation_1 = *GetPropertyTrees().effect_tree().Node(mask_isolation_2.parent_id);
    const cc::EffectNode& cc_filter = *GetPropertyTrees().effect_tree().Node(mask_isolation_1.parent_id);
    const cc::EffectNode& mask_isolation_0 = *GetPropertyTrees().effect_tree().Node(cc_filter.parent_id);
    ...
    EXPECT_EQ(gfx::RRectF(50, 50, 300, 200, 5), mask_isolation_1.mask_filter_info.rounded_corner_bounds());
    ...
    EXPECT_EQ(gfx::RRectF(50, 50, 300, 200, 5), mask_isolation_2.mask_filter_info.rounded_corner_bounds());
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipIsNotDrawable)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *c1, e0()).RectDrawing(gfx::Rect(0, 0, 0, 0), Color::kBlack);
    Update(artifact.Build());
...
TEST_P(PaintArtifactCompositorTest, ReuseSyntheticClip)
{
    ...
    cc::ElementId old_element_id = GetPropertyTrees().effect_tree().Node(content0->effect_tree_index())->element_id;

    TestPaintArtifact repeated_artifact;
    repeated_artifact.Chunk(t0(), *c1, e0()).RectDrawing(gfx::Rect(0, 0, 0, 0), Color::kBlack);
    Update(repeated_artifact.Build());
    const cc::Layer* content1 = LayerAt(0);

    // Check that stable ids are reused across updates.
    EXPECT_EQ(GetPropertyTrees().effect_tree().Node(content1->effect_tree_index())->element_id, old_element_id);

    TestPaintArtifact changed_artifact;
    changed_artifact.Chunk(t0(), *c2, e0()).RectDrawing(gfx::Rect(0, 0, 0, 0), Color::kBlack);
    ...
    // not be reused.
    EXPECT_NE(GetPropertyTrees().effect_tree().Node(content2->effect_tree_index())->element_id, old_element_id);
}

TEST_P(PaintArtifactCompositorTest, SynthesizedClipIndirectlyCompositedClipPath)
{
    ...
    auto* e1 = CreateOpacityEffect(e0(), t0(), c1, 1, CompositingReason::kWillChangeOpacity);

    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *c1, *e1).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    Update(artifact.Build());
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipContiguous)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *c1, e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    artifact.Chunk(*t1, *c1, e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    Update(artifact.Build());
    ...
    const cc::TransformNode& cc_t1 = *GetPropertyTrees().transform_tree().Node(t1_id);
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipDiscontiguous)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *c1, e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    artifact.Chunk(*t1, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    artifact.Chunk(t0(), *c1, e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    Update(artifact.Build());
    ...
    const cc::EffectNode& mask_isolation_1 = *GetPropertyTrees().effect_tree().Node(mask_isolation_1_id);
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipRespectOutputClip)
{
    ...
    auto* e1 = CreateFilterEffect(e0(), non_trivial_filter, CompositingReason::kActiveFilterAnimation);
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipDelegateBackdropFilter)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(*t1, *c1, e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    artifact.Chunk(*t1, *c2, *e1).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    artifact.Chunk(*t1, *c1, e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    Update(artifact.Build());
    ...
    EXPECT_EQ(gfx::RRectF(40, 30, 300, 200, 5), mask_isolation_1.mask_filter_info.rounded_corner_bounds());
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipMultipleNonBackdropEffects)
{
    ...
    auto* e1 = CreateOpacityEffect(e0(), t0(), c2, 0.5, CompositingReason::kWillChangeOpacity);
    auto* e2 = CreateOpacityEffect(e0(), t0(), c1, 0.75, CompositingReason::kWillChangeOpacity);

    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *c2, *e1).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    artifact.Chunk(t0(), *c1, *e2).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    artifact.Chunk(t0(), c0(), e0()).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    Update(artifact.Build());
    ...
    const cc::EffectNode& mask_isolation = *GetPropertyTrees().effect_tree().Node(mask_isolation_id);
    ...
    EXPECT_EQ(gfx::RRectF(50, 50, 300, 200, 5), mask_isolation.mask_filter_info.rounded_corner_bounds());
...
{
    TestPaintArtifact artifact;
    artifact.Chunk().RectDrawing(gfx::Rect(100, 200, 300, 400), Color::kBlack).IsSolidColor();
    Update(artifact.Build());
    ASSERT_EQ(1u, LayerCount());
...
{
    TestPaintArtifact artifact;
    artifact.Chunk().RectDrawing(gfx::Rect(100, 100, 200, 200), Color::kBlack).RectKnownToBeOpaque(gfx::Rect(100, 100, 200, 200));
    Update(artifact.Build());
    ASSERT_EQ(1u, LayerCount());
...
TEST_P(PaintArtifactCompositorTest, ContentsOpaqueUnitedWithRoundedClip)
{
    ...
    // rounded clip.
    auto* clip1 = CreateClip(c0(), t0(), FloatRoundedRect(gfx::RectF(175, 175, 100, 100), 5));
    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *clip1, e0())
        .RectDrawing(gfx::Rect(100, 100, 210, 210), Color::kBlack)
        .RectKnownToBeOpaque(gfx::Rect(100, 100, 210, 210))
...
TEST_P(PaintArtifactCompositorTest, DecompositeEffectWithNoOutputClip)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk().RectDrawing(gfx::Rect(50, 50, 100, 100), Color::kGray);
    artifact.Chunk(t0(), *clip1, *effect1).RectDrawing(gfx::Rect(100, 100, 100, 100), Color::kGray);
    Update(artifact.Build());
    ASSERT_EQ(1u, LayerCount());
...
TEST_P(PaintArtifactCompositorTest, CompositedEffectWithNoOutputClip)
{
    ...
    auto* effect1 = CreateOpacityEffect(e0(), t0(), nullptr, 0.5, CompositingReason::kAll);

    TestPaintArtifact artifact;
    artifact.Chunk(t0(), c0(), *effect1).RectDrawing(gfx::Rect(50, 50, 100, 100), Color::kGray);
    artifact.Chunk(t0(), *clip1, *effect1).RectDrawing(gfx::Rect(100, 100, 100, 100), Color::kGray);
    Update(artifact.Build());
    ASSERT_EQ(1u, LayerCount());
...
TEST_P(PaintArtifactCompositorTest, LayerRasterInvalidationWithClip)
{
    ...
    cc::FakeLayerTreeHostImpl host_impl(&task_runner_provider_, &task_graph_runner_);
    ...
    artifact1.Chunk(t0(), *clip, e0()).RectDrawing(gfx::Rect(50, 50, 200, 200), Color::kBlack);
    ...
    EXPECT_EQ(gfx::Size(200, 200), layer->bounds());
    EXPECT_THAT(layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 200, 200), Color::kBlack)));
    ...
    auto& artifact2 = TestPaintArtifact()
                          ...
                          .RectDrawing(artifact1.Client(1), gfx::Rect(0, 0, 400, 200), Color::kBlack)
                          .Build();
    ...
    layer->PushPropertiesTo(layer->CreateLayerImpl(host_impl.sync_tree()).get(),
        *const_cast<const cc::LayerTreeHost&>(GetLayerTreeHost()).pending_commit_state(),
        const_cast<const cc::LayerTreeHost&>(GetLayerTreeHost()).thread_unsafe_commit_state());
    Update(artifact2);
    ASSERT_EQ(1u, LayerCount());
    ...
    EXPECT_THAT(layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 390, 180), Color::kBlack)));

    // The layer's painting overflows all edges of the clip.
    auto& artifact3 = TestPaintArtifact()
                          .Chunk(artifact1.Client(0))
                          .Properties(t0(), *clip, e0())
                          .RectDrawing(artifact1.Client(1), gfx::Rect(-100, -200, 500, 800), Color::kBlack)
                          .Build();
    // Simluate commit to the compositor thread.
    layer->PushPropertiesTo(layer->CreateLayerImpl(host_impl.sync_tree()).get(),
        *const_cast<const cc::LayerTreeHost&>(GetLayerTreeHost()).pending_commit_state(),
    ...
    EXPECT_EQ(gfx::Vector2dF(10, 20), layer->offset_to_transform_parent());
    EXPECT_EQ(gfx::Size(300, 400), layer->bounds());
    EXPECT_THAT(layer->GetPicture(), Pointee(DrawsRectangle(gfx::RectF(0, 0, 390, 580), Color::kBlack)));
}
...
TEST_P(PaintArtifactCompositorTest, CreatesViewportNodes)
{
    ...
    const CompositorElementId compositor_element_id = CompositorElementIdFromUniqueObjectId(1);
    transform_state.compositor_element_id = compositor_element_id;

    auto* scale_transform_node = TransformPaintPropertyNode::Create(TransformPaintPropertyNode::Root(), std::move(transform_state));
    ...
    const cc::TransformTree& transform_tree = GetPropertyTrees().transform_tree();
    const cc::TransformNode* cc_transform_node = transform_tree.FindNodeFromElementId(compositor_element_id);
...
TEST_P(PaintArtifactCompositorTest, InSubtreeOfPageScale)
{
    ...
    auto* ancestor_transform = TransformPaintPropertyNode::Create(TransformPaintPropertyNode::Root(), std::move(ancestor_transform_state));

    TransformPaintPropertyNode::State page_scale_transform_state;
    page_scale_transform_state.in_subtree_of_page_scale = false;
    const CompositorElementId page_scale_compositor_element_id = CompositorElementIdFromUniqueObjectId(1);
    page_scale_transform_state.compositor_element_id = page_scale_compositor_element_id;
    auto* page_scale_transform = TransformPaintPropertyNode::Create(*ancestor_transform, std::move(page_scale_transform_state));

    TransformPaintPropertyNode::State descendant_transform_state;
    const CompositorElementId descendant_compositor_element_id = CompositorElementIdFromUniqueObjectId(2);
    descendant_transform_state.compositor_element_id = descendant_compositor_element_id;
    descendant_transform_state.in_subtree_of_page_scale = true;
    descendant_transform_state.direct_compositing_reasons = CompositingReason::kWillChangeTransform;
    auto* descendant_transform = TransformPaintPropertyNode::Create(*page_scale_transform, std::move(descendant_transform_state));

    TestPaintArtifact artifact;
    artifact.Chunk(*descendant_transform, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 10, 10), Color::kBlack);
    ViewportProperties viewport_properties;
    viewport_properties.page_scale = page_scale_transform;
    Update(artifact.Build(), viewport_properties);

    const cc::TransformTree& transform_tree = GetPropertyTrees().transform_tree();
    const auto* cc_page_scale_transform = transform_tree.FindNodeFromElementId(page_scale_compositor_element_id);
    ...
    const auto* cc_descendant_transform = transform_tree.FindNodeFromElementId(descendant_compositor_element_id);
...
TEST_P(PaintArtifactCompositorTest, ViewportPageScale)
{
    ...
    transform_state.in_subtree_of_page_scale = false;
    transform_state.compositor_element_id = CompositorElementIdFromUniqueObjectId(1);
    auto* scale_transform_node = TransformPaintPropertyNode::Create(TransformPaintPropertyNode::Root(), std::move(transform_state));
    ...
    auto scroll_element_id = CompositorElementIdFromUniqueObjectId(NewUniqueObjectId(), CompositorElementIdNamespace::kScroll);
    scroll_state.compositor_element_id = scroll_element_id;

    auto* scroll = ScrollPaintPropertyNode::Create(ScrollPaintPropertyNode::Root(), std::move(scroll_state));
    auto* scroll_translation = CreateScrollTranslation(*scale_transform_node, 0, 0, *scroll);

    TestPaintArtifact artifact;
    artifact.Chunk(*scroll_translation, c0(), e0()).RectDrawing(gfx::Rect(0, 0, 10, 10), Color::kBlack);
    ViewportProperties viewport_properties;
    viewport_properties.page_scale = scale_transform_node;
    Update(artifact.Build(), viewport_properties);

    cc::ScrollTree& scroll_tree = GetPropertyTrees().scroll_tree_mutable();
    cc::ScrollNode* cc_scroll_node = scroll_tree.FindNodeFromElementId(scroll_element_id);
...
#define EXPECT_OPACITY(effect_id, expected_opacity, expected_flags)                                                                                            \
    do {                                                                                                                                                       \
        ...
        EXPECT_EQ(!!((expected_flags)&kHasRenderSurface), effect->HasRenderSurface());                                                                         \
    } while (false)
...
TEST_P(PaintArtifactCompositorTest, OpacityRenderSurfaces)
{
  ...
  //    aa ab L2 L3   ca          (L = layer)
    ...
    auto* aa = CreateOpacityEffect(*a, 0.5f, CompositingReason::kWillChangeOpacity);
    auto* ab = CreateOpacityEffect(*a, 0.6f, CompositingReason::kWillChangeOpacity);
    auto* ca = CreateOpacityEffect(*c, 0.7f, CompositingReason::kWillChangeOpacity);
    auto* t = CreateTransform(t0(), MakeRotationMatrix(90), gfx::Point3F(), CompositingReason::k3DTransform);
    ...
    EXPECT_OPACITY(effect_tree.Node(effect_ids[4])->parent_id, 0.1f, kHasRenderSurface);
}
...
TEST_P(PaintArtifactCompositorTest, OpacityRenderSurfacesWithFilterChildren)
{
    ...
    auto* filter1 = CreateFilterEffect(*opacity, filter, CompositingReason::kActiveFilterAnimation);
    auto* filter2 = CreateFilterEffect(*opacity, filter, CompositingReason::kActiveFilterAnimation);

    gfx::Rect r(150, 150, 100, 100);
    Update(TestPaintArtifact().Chunk(t0(), c0(), *filter1).RectDrawing(r, Color::kWhite).Chunk(t0(), c0(), *filter2).RectDrawing(r, Color::kWhite).Build());
    ASSERT_EQ(2u, LayerCount());
    ...
    EXPECT_OPACITY(GetPropertyTrees().effect_tree().Node(filter_id1)->parent_id, 0.1f, kHasRenderSurface);
}
...
TEST_P(PaintArtifactCompositorTest, OpacityAnimationRenderSurfaces)
{
  ...
  //    aa ab L2 L3   ca          (L = layer)
    ...
    EXPECT_OPACITY(effect_tree.Node(effect_ids[4])->parent_id, 1.f, kHasRenderSurface);
}
...
TEST_P(PaintArtifactCompositorTest, DirectTransformAnimationCausesRenderSurfaceFor2dAxisMisalignedClip)
{
...
TEST_P(PaintArtifactCompositorTest, IndirectTransformAnimationCausesRenderSurfaceFor2dAxisMisalignedClip)
{
    // When a clip is affected by an animated transform, we should get a render
    // surface for the effect node.
    auto* t1 = CreateAnimatingTransform(t0());
...
TEST_P(PaintArtifactCompositorTest, OpacityIndirectlyAffectingTwoLayers)
{
    ...
    auto* child_composited_transform = CreateTransform(t0(), gfx::Transform(), gfx::Point3F(), CompositingReason::k3DTransform);
    auto* grandchild_composited_transform = CreateTransform(*child_composited_transform, gfx::Transform(), gfx::Point3F(), CompositingReason::k3DTransform);
    auto* child_effect = CreateOpacityEffect(*opacity, 1.f);
    auto* grandchild_effect = CreateOpacityEffect(*child_effect, 1.f);

    TestPaintArtifact artifact;
    artifact.Chunk(*child_composited_transform, c0(), *child_effect).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite);
    artifact.Chunk(*grandchild_composited_transform, c0(), *grandchild_effect).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kGray);
    Update(artifact.Build());
    ASSERT_EQ(2u, LayerCount());
...
{
    auto* opacity = CreateOpacityEffect(e0(), 1.f, CompositingReason::kWillChangeOpacity);
    auto* child_composited_transform = CreateTransform(t0(), gfx::Transform(), gfx::Point3F(), CompositingReason::k3DTransform);
    auto* child_effect = CreateOpacityEffect(*opacity, 1.f);

    TestPaintArtifact artifact;
    artifact.Chunk(t0(), c0(), *opacity).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    artifact.Chunk(*child_composited_transform, c0(), *child_effect).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite);
    Update(artifact.Build());
    ASSERT_EQ(2u, LayerCount());
...
TEST_P(PaintArtifactCompositorTest, WillChangeOpacityRenderSurfaceWithoutLayer)
{
    auto* opacity = CreateOpacityEffect(e0(), 1.f, CompositingReason::kWillChangeOpacity);
    auto* child_composited_transform = CreateTransform(t0(), gfx::Transform(), gfx::Point3F(), CompositingReason::k3DTransform);
    auto* grandchild_composited_transform = CreateTransform(*child_composited_transform, gfx::Transform(), gfx::Point3F(), CompositingReason::k3DTransform);
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(*child_composited_transform, c0(), *child_effect).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kBlack);
    artifact.Chunk(*grandchild_composited_transform, c0(), *grandchild_effect).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite);
    Update(artifact.Build());
    ASSERT_EQ(2u, LayerCount());
...
TEST_P(PaintArtifactCompositorTest, OpacityIndirectlyAffectingTwoLayersWithOpacityAnimations)
{
    auto* opacity = CreateAnimatingOpacityEffect(e0());
    auto* child_composited_effect = CreateAnimatingOpacityEffect(*opacity);
    auto* grandchild_composited_effect = CreateAnimatingOpacityEffect(*child_composited_effect);

    TestPaintArtifact artifact;
    artifact.Chunk(t0(), c0(), *child_composited_effect).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite);
    artifact.Chunk(t0(), c0(), *grandchild_composited_effect).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kGray);
    Update(artifact.Build());
    ASSERT_EQ(2u, LayerCount());
...
TEST_P(PaintArtifactCompositorTest, FilterCreatesRenderSurface)
{
    CompositorFilterOperations filter;
    filter.AppendBlurFilter(5);
    auto* e1 = CreateFilterEffect(e0(), filter, CompositingReason::kActiveFilterAnimation);
    Update(TestPaintArtifact().Chunk(t0(), c0(), *e1).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite).Build());
    ASSERT_EQ(1u, LayerCount());
...
{
    auto* e1 = CreateFilterEffect(e0(), CompositorFilterOperations(), CompositingReason::kWillChangeFilter);
    Update(TestPaintArtifact().Chunk(t0(), c0(), *e1).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite).Build());
    ASSERT_EQ(1u, LayerCount());
    EXPECT_OPACITY(LayerAt(0)->effect_tree_index(), 1.f, kHasRenderSurface);
}
...
TEST_P(PaintArtifactCompositorTest, WillChangeBackdropFilterCreatesRenderSurface)
{
    auto* e1 = CreateBackdropFilterEffect(e0(), CompositorFilterOperations(), CompositingReason::kWillChangeBackdropFilter);
    Update(TestPaintArtifact().Chunk(t0(), c0(), *e1).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite).Build());
    ASSERT_EQ(1u, LayerCount());
    EXPECT_OPACITY(LayerAt(0)->effect_tree_index(), 1.f, kHasRenderSurface);
}

TEST_P(PaintArtifactCompositorTest, BackdropFilterAnimationCreatesRenderSurface)
{
...
TEST_P(PaintArtifactCompositorTest, Non2dAxisAlignedClip)
{
    ...
    auto* opacity = CreateOpacityEffect(e0(), 0.5f, CompositingReason::kActiveOpacityAnimation);

    TestPaintArtifact artifact;
    artifact.Chunk(t0(), *clip, *opacity).RectDrawing(gfx::Rect(50, 50, 50, 50), Color::kWhite);
    Update(artifact.Build());
    ASSERT_EQ(1u, LayerCount());
...
TEST_P(PaintArtifactCompositorTest, Non2dAxisAlignedRoundedRectClip)
{
    ...
    const auto* cc_mask = GetPropertyTrees().effect_tree().Node(mask_layer->effect_tree_index());
    EXPECT_EQ(SkBlendMode::kDstIn, cc_mask->blend_mode);
}

TEST_P(PaintArtifactCompositorTest, Non2dAxisAlignedClipUnderLaterRenderSurface)
{
    auto* rotate1 = CreateTransform(t0(), MakeRotationMatrix(45), gfx::Point3F(), CompositingReason::k3DTransform);
    auto* rotate2 = CreateTransform(*rotate1, MakeRotationMatrix(-45), gfx::Point3F(), CompositingReason::k3DTransform);
    auto* clip = CreateClip(c0(), *rotate2, FloatRoundedRect(50, 50, 50, 50));
    auto* opacity = CreateOpacityEffect(e0(), *rotate1, &c0(), 0.5f, CompositingReason::kActiveOpacityAnimation);
    ...
    // to make it succeed.
    ASSERT_TRUE(GeometryMapper::SourceToDestinationProjection(t0(), *rotate2).Preserves2dAxisAlignment());

    TestPaintArtifact artifact;
    artifact.Chunk(t0(), c0(), *opacity).RectDrawing(gfx::Rect(50, 50, 50, 50), Color::kWhite);
    artifact.Chunk(*rotate1, c0(), *opacity).RectDrawing(gfx::Rect(50, 50, 50, 50), Color::kWhite);
    artifact.Chunk(*rotate2, *clip, *opacity).RectDrawing(gfx::Rect(50, 50, 50, 50), Color::kWhite);
    Update(artifact.Build());
    ASSERT_EQ(3u, LayerCount());
...
static TransformPaintPropertyNode::State Transform3dState(const gfx::Transform& transform)
{
...
{
    auto* t1 = Create2DTranslation(t0(), 10, 20);
    auto* t2 = TransformPaintPropertyNode::Create(*t1, Transform3dState(MakeRotationMatrix(45)));
    FakeDisplayItemClient& client = *MakeGarbageCollected<FakeDisplayItemClient>();
    client.Validate();
    Update(TestPaintArtifact().Chunk(1).Properties(*t2, c0(), e0()).RectDrawing(client, gfx::Rect(100, 100, 200, 100), Color::kBlack).Build());
    ASSERT_EQ(1u, LayerCount());
    ...
    t1->Update(t0(), TransformPaintPropertyNode::State { { MakeTranslationMatrix(20, 30) } });
    EXPECT_EQ(PaintPropertyChangeType::kChangedOnlySimpleValues, t1->NodeChanged());
    EXPECT_EQ(PaintPropertyChangeType::kUnchanged, t2->NodeChanged());
    Update(TestPaintArtifact().Chunk(1).Properties(*t2, c0(), e0()).RectDrawing(client, gfx::Rect(100, 100, 200, 100), Color::kBlack).Build());

    ASSERT_EQ(1u, LayerCount());
    ASSERT_EQ(layer, LayerAt(0));
    EXPECT_EQ(display_item_list.get(), layer->client()->PaintContentsToDisplayList().get());
    ...
    EXPECT_TRUE(GetPropertyTrees().transform_tree().Node(GetTransformNode(layer).parent_id)->transform_changed);
    ...
    EXPECT_EQ(PaintPropertyChangeType::kChangedOnlySimpleValues, t2->NodeChanged());
    Update(TestPaintArtifact().Chunk(1).Properties(*t2, c0(), e0()).RectDrawing(client, gfx::Rect(100, 100, 200, 100), Color::kBlack).Build());
    ...
    EXPECT_TRUE(layer->subtree_property_changed());
    EXPECT_TRUE(GetTransformNode(layer).transform_changed);
    EXPECT_FALSE(GetPropertyTrees().transform_tree().Node(GetTransformNode(layer).parent_id)->transform_changed);
    ...
    Update(TestPaintArtifact().Chunk(1).Properties(*t2, c0(), e0()).RectDrawing(client, gfx::Rect(100, 100, 200, 100), Color::kWhite).Build());

    ASSERT_EQ(1u, LayerCount());
    ASSERT_EQ(layer, LayerAt(0));
    EXPECT_NE(display_item_list.get(), layer->client()->PaintContentsToDisplayList().get());
}

TEST_P(PaintArtifactCompositorTest, EffectChange)
{
    auto* e1 = CreateOpacityEffect(e0(), t0(), nullptr, 0.5f);
    auto* e2 = CreateOpacityEffect(*e1, t0(), nullptr, 0.6f, CompositingReason::kWillChangeOpacity);

    Update(TestPaintArtifact().Chunk(1).Properties(t0(), c0(), *e2).RectDrawing(gfx::Rect(100, 100, 200, 100), Color::kBlack).Build());
    ASSERT_EQ(1u, LayerCount());
    ...
    EXPECT_EQ(PaintPropertyChangeType::kChangedOnlySimpleValues, e1->NodeChanged());
    ...
    // This is set by PropertyTreeManager.
    EXPECT_TRUE(GetPropertyTrees().effect_tree().Node(GetEffectNode(layer).parent_id)->effect_changed);
    ...
    EXPECT_EQ(PaintPropertyChangeType::kChangedOnlySimpleValues, e2->NodeChanged());
    Update(TestPaintArtifact().Chunk(1).Properties(t0(), c0(), *e2).RectDrawing(gfx::Rect(100, 100, 200, 100), Color::kBlack).Build());
    ...
    EXPECT_TRUE(layer->subtree_property_changed());
    EXPECT_TRUE(GetEffectNode(layer).effect_changed);
    EXPECT_FALSE(GetPropertyTrees().effect_tree().Node(GetEffectNode(layer).parent_id)->effect_changed);
}
...
TEST_P(PaintArtifactCompositorTest, DirectlySetScrollOffset)
{
    ...
    const auto* scroll_node = scroll_tree.FindNodeFromElementId(scroll_element_id);
    ...
    EXPECT_EQ(gfx::PointF(-7, -9), scroll_tree.current_scroll_offset(scroll_element_id));
    EXPECT_EQ(gfx::PointF(-7, -9), transform_node->scroll_offset);

    auto& host = GetLayerTreeHost();
    host.CompositeForTest(base::TimeTicks::Now(), true, base::OnceClosure());
    ASSERT_FALSE(const_cast<const cc::LayerTreeHost&>(host).pending_commit_state()->layers_that_should_push_properties.contains(scroll_layer));
    ASSERT_FALSE(host.CommitRequested());
    ASSERT_FALSE(transform_tree.needs_update());

    ASSERT_TRUE(GetPaintArtifactCompositor().DirectlySetScrollOffset(scroll_element_id, gfx::PointF(-10, -20)));
    EXPECT_TRUE(const_cast<const cc::LayerTreeHost&>(host).pending_commit_state()->layers_that_should_push_properties.contains(scroll_layer));
    EXPECT_TRUE(host.CommitRequested());
    EXPECT_EQ(gfx::PointF(-10, -20), scroll_tree.current_scroll_offset(scroll_element_id));
...
TEST_P(PaintArtifactCompositorTest, NoCommitRequestForUnchangedScroll)
{
    ...
    EXPECT_EQ(PaintPropertyChangeType::kNodeAddedOrRemoved, scroll_node.NodeChanged());
...
{
    auto scroll_state = ScrollState1(PropertyTreeState::Root(), CompositingReason::kNone, cc::MainThreadScrollingReason::kNotScrollingOnMain);
    PaintArtifactCompositor::StackScrollTranslationVector scroll_translation_nodes = { &scroll_state.Transform() };
    ...
    const auto& scroll_tree = GetPropertyTrees().scroll_tree();
    auto* scroll_node = scroll_tree.FindNodeFromElementId(scroll_state.Transform().ScrollNode()->GetCompositorElementId());
    ASSERT_TRUE(scroll_node);
    EXPECT_TRUE(scroll_node->is_composited);
    EXPECT_EQ(cc::MainThreadScrollingReason::kNotScrollingOnMain, scroll_node->main_thread_repaint_reasons);
...
TEST_P(PaintArtifactCompositorTest, AddNonCompositedScrollNodes)
{
    ...
    Update(TestPaintArtifact().ScrollChunks(scroll_state).Build(), ViewportProperties(), scroll_translation_nodes);
    ...
    if (RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        ...
        EXPECT_EQ(cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText, scroll_node->main_thread_repaint_reasons);
        EXPECT_FALSE(scroll_tree.CanRealizeScrollsOnPendingTree(*scroll_node));
...
{
    auto scroll_state = ScrollState1(PropertyTreeState::Root(), CompositingReason::kNone, cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects);
    PaintArtifactCompositor::StackScrollTranslationVector scroll_translation_nodes = { &scroll_state.Transform() };
    ...
    EXPECT_FALSE(scroll_node->is_composited);
    if (RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        EXPECT_EQ(cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects, scroll_node->main_thread_repaint_reasons);
    } else {
        EXPECT_EQ(cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText | cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects,
...
TEST_P(PaintArtifactCompositorTest, AddIndirectlyCompositedMainThreadScrollNodes)
{
    auto scroll_state = ScrollState1(PropertyTreeState::Root(), CompositingReason::kNone, cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects);
    PaintArtifactCompositor::StackScrollTranslationVector scroll_translation_nodes = { &scroll_state.Transform() };
...
{
    const uint32_t main_thread_scrolling_reason = cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText;
    auto scroll_state = ScrollState1(PropertyTreeState::Root(), CompositingReason::kNone, main_thread_scrolling_reason);
    PaintArtifactCompositor::StackScrollTranslationVector scroll_translation_nodes = { &scroll_state.Transform() };

    Update(TestPaintArtifact().Build(), ViewportProperties(), scroll_translation_nodes);
    ...
    EXPECT_EQ(gfx::PointF(-7, -9), scroll_tree.current_scroll_offset(scroll_node->element_id));
    EXPECT_FALSE(scroll_tree.CanRealizeScrollsOnActiveTree(*scroll_node));
    EXPECT_FALSE(scroll_tree.CanRealizeScrollsOnPendingTree(*scroll_node));
    EXPECT_FALSE(scroll_tree.ShouldRealizeScrollsOnMain(*scroll_node));
}
...
TEST_P(PaintArtifactCompositorTest, ClearChangedStateWithIndirectTransform)
{
    ...
    auto& artifact = TestPaintArtifact().Chunk(1).Properties(*t2, *c2, e0()).RectDrawing(gfx::Rect(2, 2, 2, 2), Color::kBlack).Build();
    Update(artifact);
    EXPECT_EQ(PaintPropertyChangeType::kUnchanged, t1->NodeChanged());
    EXPECT_EQ(PaintPropertyChangeType::kUnchanged, t2->NodeChanged());
...
TEST_P(PaintArtifactCompositorTest, CompositedPixelMovingFilterWithClipExpander)
{
    CompositorFilterOperations filter_op;
    filter_op.AppendBlurFilter(5);
    auto* filter = CreateFilterEffect(e0(), filter_op, CompositingReason::kWillChangeFilter);
    auto* clip_expander = CreatePixelMovingFilterClipExpander(c0(), *filter);

    Update(TestPaintArtifact().Chunk(t0(), *clip_expander, *filter).RectDrawing(gfx::Rect(150, 150, 100, 100), Color::kWhite).Build());
    ASSERT_EQ(1u, LayerCount());
    const auto* cc_clip_expander = GetPropertyTrees().clip_tree().Node(LayerAt(0)->clip_tree_index());
    EXPECT_FALSE(cc_clip_expander->AppliesLocalClip());
    EXPECT_EQ(cc_clip_expander->pixel_moving_filter_id, LayerAt(0)->effect_tree_index());
}

TEST_P(PaintArtifactCompositorTest, NonCompositedPixelMovingFilterWithCompositedClipExpander)
{
    CompositorFilterOperations filter_op;
    filter_op.AppendBlurFilter(5);
    ...
    mask_state.direct_compositing_reasons = CompositingReason::kBackdropFilterMask;
...
TEST_P(PaintArtifactCompositorTest, CreatePictureLayerForSolidColorBackdropFilterMask)
{
    CompositorFilterOperations filter;
    filter.AppendBlurFilter(5);
    ...
    mask_state.blend_mode = SkBlendMode::kDstIn;
    mask_state.direct_compositing_reasons = CompositingReason::kBackdropFilterMask;
    auto* mask = EffectPaintPropertyNode::Create(*backdrop_filter, std::move(mask_state));
