...
public:
    ...
    PendingLayer(const PaintArtifact&, const PaintChunk& first_chunk, CompositingType = kOther);
    ...
    const gfx::RectF& BoundsForTesting() const
    {
        return bounds_;
    }
    ...
    CompositingType GetCompositingType() const
    {
        return compositing_type_;
    }
    ...
    using IsCompositedScrollFunction = PropertyTreeState::IsCompositedScrollFunction;
    ...
    bool Merge(const PendingLayer& guest, LCDTextPreference lcd_text_preference, IsCompositedScrollFunction);

    // Returns true if `guest` that could be upcasted with decomposited blend
    // mode can be merged into `this`.
    bool CanMergeWithDecompositedBlendMode(const PendingLayer& guest, const PropertyTreeState& upcast_state, IsCompositedScrollFunction) const;
    ...
    const TransformPaintPropertyNode& ScrollTranslationForScrollHitTestLayer() const;

    std::unique_ptr<JSONObject> ToJSON() const;
    String DebugName() const;
    DOMNodeId OwnerNodeId() const;

    void ForceDrawsContent()
    {
        draws_content_ = true;
    }
    ...
    void UpdateCompositedLayer(PendingLayer* old_pending_layer, cc::LayerSelection&, bool tracks_raster_invalidations, cc::LayerTreeHost*);
    ...
    void UpdateCompositedLayerForRepaint(const PaintArtifact& repainted_artifact, cc::LayerSelection&);
    ...
    {
        return solid_color_chunk_index_ != kNotFound;
    }
...
private:
    ...
    std::optional<PropertyTreeState> CanUpcastWith(
        const PendingLayer& guest, const PropertyTreeState& guest_state, IsCompositedScrollFunction is_comosited_scroll) const;

    bool CanMerge(const PendingLayer& guest, LCDTextPreference lcd_text_preference, IsCompositedScrollFunction, gfx::RectF& merged_bounds,
        PropertyTreeState& merged_state, gfx::RectF& merged_rect_known_to_be_opaque, bool& merged_text_known_to_be_on_opaque_background,
        wtf_size_t& merged_solid_color_chunk_index, cc::HitTestOpaqueness& merged_hit_test_opaqueness) const;

    gfx::RectF MapRectKnownToBeOpaque(const PropertyTreeState& new_state, const FloatClipRect& mapped_layer_bounds) const;
    ...
    void UpdateContentLayer(PendingLayer* old_pending_layer, bool tracks_raster_invalidations);
    ...
    PaintPropertyChangeType change_of_decomposited_transforms_ = PaintPropertyChangeType::kUnchanged;
    CompositingType compositing_type_ = kOther;
    cc::HitTestOpaqueness hit_test_opaqueness_ = cc::HitTestOpaqueness::kTransparent;
    ...
    // hashing.
    HeapVector<Member<const TransformPaintPropertyNode>> non_composited_scroll_translations_;
