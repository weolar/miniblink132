...
PropertyTreeManager::EffectState::EffectState(const CurrentEffectState& other)
    ...
    , may_be_2d_axis_misaligned_to_render_surface(other.may_be_2d_axis_misaligned_to_render_surface)
    , contained_by_non_render_surface_synthetic_rounded_clip(other.contained_by_non_render_surface_synthetic_rounded_clip)
{
}

PropertyTreeManager::CurrentEffectState::CurrentEffectState(const EffectState& other)
    : effect_id(other.effect_id)
    , effect(other.effect)
    , clip(other.clip)
    , transform(other.transform)
...
PropertyTreeManager::PropertyTreeManager(
    PropertyTreeManagerClient& client, cc::PropertyTrees& property_trees, cc::Layer& root_layer, LayerListBuilder& layer_list_builder, int new_sequence_number)
    : client_(client)
...
static void UpdateCcTransformLocalMatrix(cc::TransformNode& compositor_node, const TransformPaintPropertyNode& transform_node)
{
    if (transform_node.GetStickyConstraint() || transform_node.GetAnchorPositionScrollData()) {
        ...
        compositor_node.scroll_offset = gfx::PointAtOffsetFromOrigin(-transform_node.Get2dTranslation());
        DCHECK(compositor_node.local.IsIdentity());
        DCHECK_EQ(gfx::Point3F(), compositor_node.origin);
    } else {
...
static void SetTransformTreePageScaleFactor(cc::TransformTree& transform_tree, const cc::TransformNode& page_scale_node)
{
...
bool PropertyTreeManager::DirectlyUpdateCompositedOpacityValue(cc::LayerTreeHost& host, const EffectPaintPropertyNode& effect)
{
    host.WaitForProtectedSequenceCompletion();
    auto* property_trees = host.property_trees();
    auto* cc_effect = property_trees->effect_tree_mutable().Node(effect.CcNodeId(property_trees->sequence_number()));
    if (!cc_effect)
        return false;
...
bool PropertyTreeManager::DirectlyUpdateScrollOffsetTransform(cc::LayerTreeHost& host, const TransformPaintPropertyNode& transform)
{
    host.WaitForProtectedSequenceCompletion();
    ...
    auto* cc_scroll_node = scroll_tree.Node(scroll_node->CcNodeId(property_trees->sequence_number()));
    if (!cc_scroll_node || scroll_tree.ShouldRealizeScrollsOnMain(*cc_scroll_node)) {
        return false;
    }

    auto* cc_transform = property_trees->transform_tree_mutable().Node(transform.CcNodeId(property_trees->sequence_number()));
    if (!cc_transform)
        return false;

    DCHECK(!cc_transform->is_currently_animating);

    gfx::PointF scroll_offset = gfx::PointAtOffsetFromOrigin(-transform.Get2dTranslation());
    DirectlySetScrollOffset(host, scroll_node->GetCompositorElementId(), scroll_offset);
...
bool PropertyTreeManager::DirectlyUpdateTransform(cc::LayerTreeHost& host, const TransformPaintPropertyNode& transform)
{
    host.WaitForProtectedSequenceCompletion();
...
bool PropertyTreeManager::DirectlyUpdatePageScaleTransform(cc::LayerTreeHost& host, const TransformPaintPropertyNode& transform)
{
    host.WaitForProtectedSequenceCompletion();
    DCHECK(!transform.ScrollNode());
    ...
    UpdateCcTransformLocalMatrix(*cc_transform, transform);
    SetTransformTreePageScaleFactor(property_trees->transform_tree_mutable(), *cc_transform);
    cc_transform->transform_changed = true;
    property_trees->transform_tree_mutable().set_needs_update(true);
    return true;
}

void PropertyTreeManager::DirectlySetScrollOffset(cc::LayerTreeHost& host, CompositorElementId element_id, const gfx::PointF& scroll_offset)
{
    host.WaitForProtectedSequenceCompletion();
    auto* property_trees = host.property_trees();
    if (property_trees->scroll_tree_mutable().SetScrollOffset(element_id, scroll_offset)) {
...
void PropertyTreeManager::DropCompositorScrollDeltaNextCommit(cc::LayerTreeHost& host, CompositorElementId element_id)
{
    host.DropActiveScrollDeltaNextCommit(element_id);
}

uint32_t PropertyTreeManager::NonCompositedMainThreadRepaintReasons(const TransformPaintPropertyNode& scroll_translation) const
{
    if (scroll_translation.ScrollNode()->GetCompositedScrollingPreference() == CompositedScrollingPreference::kNotPreferred) {
        return cc::MainThreadScrollingReason::kPreferNonCompositedScrolling;
    }
    if (RuntimeEnabledFeatures::RasterInducingScrollEnabled() && !client_.ShouldForceMainThreadRepaint(scroll_translation)) {
        return cc::MainThreadScrollingReason::kNotScrollingOnMain;
    }
    return cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText;
}

uint32_t PropertyTreeManager::GetMainThreadRepaintReasons(const cc::LayerTreeHost& host, const ScrollPaintPropertyNode& scroll)
{
    const auto* property_trees = host.property_trees();
    const auto* cc_scroll = property_trees->scroll_tree().Node(scroll.CcNodeId(property_trees->sequence_number()));
    return cc_scroll ? cc_scroll->main_thread_repaint_reasons : cc::MainThreadScrollingReason::kPreferNonCompositedScrolling;
}

bool PropertyTreeManager::UsesCompositedScrolling(const cc::LayerTreeHost& host, const ScrollPaintPropertyNode& scroll)
{
...
bool PropertyTreeManager::UsesRasterInducingScroll(const cc::LayerTreeHost& host, const ScrollPaintPropertyNode& scroll)
{
    const auto* property_trees = host.property_trees();
    const auto* cc_scroll = property_trees->scroll_tree().Node(scroll.CcNodeId(property_trees->sequence_number()));
    return cc_scroll && property_trees->scroll_tree().CanRealizeScrollsOnPendingTree(*cc_scroll);
}
...
void PropertyTreeManager::SetupRootTransformNode()
{
    ...
    cc::TransformNode& transform_node = *transform_tree_.Node(transform_tree_.Insert(cc::TransformNode(), cc::kRootPropertyNodeId));
    ...
    // cc.
    float device_scale_factor = root_layer_.layer_tree_host()->device_scale_factor();
    ...
    TransformPaintPropertyNode::Root().SetCcNodeId(new_sequence_number_, transform_node.id);
...
void PropertyTreeManager::SetupRootClipNode()
{
    ...
    clip_tree_.clear();
    cc::ClipNode& clip_node = *clip_tree_.Node(clip_tree_.Insert(cc::ClipNode(), cc::kRootPropertyNodeId));
    ...
    clip_node.clip = gfx::RectF(root_layer_.layer_tree_host()->device_viewport_rect());
...
void PropertyTreeManager::SetupRootEffectNode()
{
    ...
    effect_tree_.clear();
    cc::EffectNode& effect_node = *effect_tree_.Node(effect_tree_.Insert(cc::EffectNode(), cc::kInvalidPropertyNodeId));
    ...
    EffectPaintPropertyNode::Root().SetCcNodeId(new_sequence_number_, effect_node.id);
    SetCurrentEffectState(
        effect_node, CcEffectType::kEffect, EffectPaintPropertyNode::Root(), ClipPaintPropertyNode::Root(), TransformPaintPropertyNode::Root());
}

void PropertyTreeManager::SetupRootScrollNode()
{
    scroll_tree_.clear();
    cc::ScrollNode& scroll_node = *scroll_tree_.Node(scroll_tree_.Insert(cc::ScrollNode(), cc::kRootPropertyNodeId));
    DCHECK_EQ(scroll_node.id, cc::kSecondaryRootPropertyNodeId);
    scroll_node.transform_id = cc::kSecondaryRootPropertyNodeId;

    ScrollPaintPropertyNode::Root().SetCcNodeId(new_sequence_number_, scroll_node.id);
    root_layer_.SetScrollTreeIndex(scroll_node.id);
}

static bool TransformsToAncestorHaveNonAxisAlignedActiveAnimation(const TransformPaintPropertyNode& descendant, const TransformPaintPropertyNode& ancestor)
{
    if (&descendant == &ancestor)
        return false;
    for (const auto* n = &descendant; n != &ancestor; n = n->UnaliasedParent()) {
        if (n->HasActiveTransformAnimation() && !n->TransformAnimationIsAxisAligned()) {
            return true;
        }
    }
    return false;
...
bool TransformsMayBe2dAxisMisaligned(const TransformPaintPropertyNode& a, const TransformPaintPropertyNode& b)
{
    if (&a == &b)
        return false;
    if (!GeometryMapper::SourceToDestinationProjection(a, b).Preserves2dAxisAlignment()) {
        return true;
    }
    const auto& lca = a.LowestCommonAncestor(b).Unalias();
    if (TransformsToAncestorHaveNonAxisAlignedActiveAnimation(a, lca) || TransformsToAncestorHaveNonAxisAlignedActiveAnimation(b, lca))
        return true;
    return false;
}
...
{
    return reason == cc::RenderSurfaceReason::kBlendModeDstIn || reason == cc::RenderSurfaceReason::kOpacity
        || reason == cc::RenderSurfaceReason::kOpacityAnimation;
}

void PropertyTreeManager::SetCurrentEffectState(const cc::EffectNode& cc_effect_node, CcEffectType effect_type, const EffectPaintPropertyNode& effect,
    const ClipPaintPropertyNode& clip, const TransformPaintPropertyNode& transform)
{
    const auto* previous_transform = effect.IsRoot() ? nullptr : current_.transform.Get();
    ...
    if (cc_effect_node.HasRenderSurface() && !IsConditionalRenderSurfaceReason(cc_effect_node.render_surface_reason)) {
        current_.may_be_2d_axis_misaligned_to_render_surface = EffectState::kAligned;
        current_.contained_by_non_render_surface_synthetic_rounded_clip = false;
    } else {
        if (current_.may_be_2d_axis_misaligned_to_render_surface == EffectState::kAligned && previous_transform != current_.transform) {
            current_.may_be_2d_axis_misaligned_to_render_surface = EffectState::kUnknown;
        }
        current_.contained_by_non_render_surface_synthetic_rounded_clip |= (effect_type & CcEffectType::kSyntheticForNonTrivialClip);
    }
}

int PropertyTreeManager::EnsureCompositorTransformNode(const TransformPaintPropertyNode& transform_node)
{
    ...
    int parent_id = EnsureCompositorTransformNode(transform_node.Parent()->Unalias());
    id = transform_tree_.Insert(cc::TransformNode(), parent_id);

    if (auto* scroll_translation_for_fixed = transform_node.ScrollTranslationForFixed()) {
    ...
    compositor_node.should_undo_overscroll = transform_node.RequiresCompositingForFixedToViewport();
    compositor_node.transform_changed = transform_node.NodeChangeAffectsRaster();
    compositor_node.flattens_inherited_transform = transform_node.FlattensInheritedTransform();
    compositor_node.sorting_context_id = transform_node.RenderingContextId();
    compositor_node.delegates_to_parent_for_backface = transform_node.DelegatesToParentForBackface();
    ...
    compositor_node.in_subtree_of_page_scale_layer = transform_node.IsInSubtreeOfPageScale();

    compositor_node.will_change_transform = transform_node.RequiresCompositingForWillChangeTransform() &&
    ...
    if (const auto* sticky_constraint = transform_node.GetStickyConstraint()) {
        cc::StickyPositionNodeData& sticky_data = transform_tree_.EnsureStickyPositionData(id);
        sticky_data.constraints = *sticky_constraint;
        const auto& scroll_ancestor = transform_node.NearestScrollTranslationNode();
        sticky_data.scroll_ancestor = EnsureCompositorScrollAndTransformNode(scroll_ancestor, InfiniteIntRect());
        const auto& scroll_ancestor_compositor_node = *scroll_tree_.Node(sticky_data.scroll_ancestor);
        if (scroll_ancestor_compositor_node.scrolls_outer_viewport)
            transform_tree_.AddNodeAffectedByOuterViewportBoundsDelta(id);
        if (auto shifting_sticky_box_element_id = sticky_data.constraints.nearest_element_shifting_sticky_box) {
            sticky_data.nearest_node_shifting_sticky_box = transform_tree_.FindNodeFromElementId(shifting_sticky_box_element_id)->id;
        }
        if (auto shifting_containing_block_element_id = sticky_data.constraints.nearest_element_shifting_containing_block) {
            // TODO(crbug.com/1224888): Get rid of the nullptr check below:
            if (cc::TransformNode* node = transform_tree_.FindNodeFromElementId(shifting_containing_block_element_id)) {
    ...
    if (transform_node.ScrollNode()) {
        ...
        scroll_node->is_composited = client_.NeedsCompositedScrolling(transform_node);
        if (!scroll_node->is_composited) {
            scroll_node->main_thread_repaint_reasons |= NonCompositedMainThreadRepaintReasons(transform_node);
        }
    }

    compositor_node.visible_frame_element_id = transform_node.GetVisibleFrameElementId();
...
int PropertyTreeManager::EnsureCompositorPageScaleTransformNode(const TransformPaintPropertyNode& node)
{
...
int PropertyTreeManager::EnsureCompositorClipNode(const ClipPaintPropertyNode& clip_node)
{
    ...
    compositor_node.transform_id = EnsureCompositorTransformNode(clip_node.LocalTransformSpace().Unalias());
...
int PropertyTreeManager::EnsureCompositorScrollNode(const TransformPaintPropertyNode& scroll_translation)
{
    const auto* scroll_node = scroll_translation.ScrollNode();
    CHECK(scroll_node);
    int scroll_id = EnsureCompositorScrollNodeInternal(*scroll_node);
    scroll_tree_.SetScrollOffset(scroll_node->GetCompositorElementId(), gfx::PointAtOffsetFromOrigin(-scroll_translation.Get2dTranslation()));
    return scroll_id;
}

int PropertyTreeManager::EnsureCompositorScrollNodeInternal(const ScrollPaintPropertyNode& scroll_node)
{
    ...
    compositor_node.user_scrollable_horizontal = scroll_node.UserScrollableHorizontal();
    compositor_node.user_scrollable_vertical = scroll_node.UserScrollableVertical();
    compositor_node.prevent_viewport_scrolling_from_inner = scroll_node.PreventViewportScrollingFromInner();

    compositor_node.max_scroll_offset_affected_by_page_scale = scroll_node.MaxScrollOffsetAffectedByPageScale();
    compositor_node.overscroll_behavior = cc::OverscrollBehavior(static_cast<cc::OverscrollBehavior::Type>(scroll_node.OverscrollBehaviorX()),
        static_cast<cc::OverscrollBehavior::Type>(scroll_node.OverscrollBehaviorY()));
    ...
    compositor_node.main_thread_repaint_reasons = scroll_node.GetMainThreadRepaintReasons();
    CHECK_EQ(compositor_node.main_thread_repaint_reasons, scroll_tree_.GetMainThreadRepaintReasons(compositor_node));
...
int PropertyTreeManager::EnsureCompositorScrollAndTransformNode(
    const TransformPaintPropertyNode& scroll_translation, const gfx::Rect& scrolling_contents_cull_rect)
{
...
int PropertyTreeManager::EnsureCompositorInnerScrollAndTransformNode(const TransformPaintPropertyNode& scroll_translation)
{
    int node_id = EnsureCompositorScrollAndTransformNode(scroll_translation, InfiniteIntRect());
    scroll_tree_.Node(node_id)->scrolls_inner_viewport = true;
    return node_id;
}

int PropertyTreeManager::EnsureCompositorOuterScrollAndTransformNode(const TransformPaintPropertyNode& scroll_translation)
{
    int node_id = EnsureCompositorScrollAndTransformNode(scroll_translation, InfiniteIntRect());
...
void PropertyTreeManager::EmitClipMaskLayer()
{
    ...
    bool needs_layer = !pending_synthetic_mask_layers_.Contains(mask_isolation->id) && mask_isolation->mask_filter_info.IsEmpty();

    CompositorElementId mask_isolation_id, mask_effect_id;
    SynthesizedClip& clip = client_.CreateOrReuseSynthesizedClipLayer(*current_.clip, *current_.transform, needs_layer, mask_isolation_id, mask_effect_id);
    ...
    cc::EffectNode& mask_effect = *effect_tree_.Node(effect_tree_.Insert(cc::EffectNode(), current_.effect_id));
    ...
    mask_layer->set_property_tree_sequence_number(root_layer_.property_tree_sequence_number());
    mask_layer->SetTransformTreeIndex(EnsureCompositorTransformNode(*current_.transform));
    int scroll_id = EnsureCompositorScrollAndTransformNode(current_.transform->NearestScrollTranslationNode(), InfiniteIntRect());
    ...
    if (!mask_isolation->backdrop_filters.IsEmpty()) {
        ...
        auto element_id = CompositorElementIdWithNamespace(mask_effect.element_id, CompositorElementIdNamespace::kEffectMask);
...
void PropertyTreeManager::CloseCcEffect()
{
    ...
    // closed now.
    bool clear_synthetic_effects = !IsCurrentCcEffectSynthetic() && current_.effect->MayHaveBackdropEffect();
...
int PropertyTreeManager::SwitchToEffectNodeWithSynthesizedClip(
    const EffectPaintPropertyNode& next_effect, const ClipPaintPropertyNode& next_clip, bool layer_draws_content)
{
    ...
    const auto& ancestor = current_.effect->LowestCommonAncestor(next_effect).Unalias();
...
static bool IsNodeOnAncestorChain(const ClipPaintPropertyNode& find, const ClipPaintPropertyNode& current, const ClipPaintPropertyNode& ancestor)
{
    ...
    for (const auto* node = &current; node != &ancestor; node = node->UnaliasedParent()) {
        if (node == &find)
            return true;
    }
    return false;
...
bool PropertyTreeManager::EffectStateMayBe2dAxisMisalignedToRenderSurface(EffectState& state, wtf_size_t index)
{
    if (state.may_be_2d_axis_misaligned_to_render_surface == EffectState::kUnknown) {
        // The root effect has render surface, so it's always kAligned.
        DCHECK_NE(0u, index);
        if (EffectStateMayBe2dAxisMisalignedToRenderSurface(effect_stack_[index - 1], index - 1)) {
            state.may_be_2d_axis_misaligned_to_render_surface = EffectState::kMisaligned;
        } else {
            state.may_be_2d_axis_misaligned_to_render_surface
                = TransformsMayBe2dAxisMisaligned(*effect_stack_[index - 1].transform, *current_.transform) ? EffectState::kMisaligned : EffectState::kAligned;
        }
    }
    return state.may_be_2d_axis_misaligned_to_render_surface == EffectState::kMisaligned;
}

bool PropertyTreeManager::CurrentEffectMayBe2dAxisMisalignedToRenderSurface()
{
    // |current_| is virtually the top of effect_stack_.
    return EffectStateMayBe2dAxisMisalignedToRenderSurface(current_, effect_stack_.size());
}

PropertyTreeManager::CcEffectType PropertyTreeManager::SyntheticEffectType(const ClipPaintPropertyNode& clip)
{
    ...
    if (CurrentEffectMayBe2dAxisMisalignedToRenderSurface() || TransformsMayBe2dAxisMisaligned(clip.LocalTransformSpace().Unalias(), *current_.transform))
        effect_type |= CcEffectType::kSyntheticFor2dAxisAlignment;
    return static_cast<CcEffectType>(effect_type);
}

void PropertyTreeManager::ForceRenderSurfaceIfSyntheticRoundedCornerClip(PropertyTreeManager::EffectState& state)
{
...
std::optional<gfx::RRectF> PropertyTreeManager::ShaderBasedRRect(const ClipPaintPropertyNode& clip, PropertyTreeManager::CcEffectType type,
    const TransformPaintPropertyNode& transform, const EffectPaintPropertyNode* next_effect)
{
    ...
    auto WidthAndHeightAreTheSame = [](const gfx::SizeF& size) { return size.width() == size.height(); };

    const FloatRoundedRect::Radii& radii = clip.PaintClipRect().GetRadii();
    if (!WidthAndHeightAreTheSame(radii.TopLeft()) || !WidthAndHeightAreTheSame(radii.TopRight()) || !WidthAndHeightAreTheSame(radii.BottomRight())
...
#if BUILDFLAG(IS_MAC)
    if (radii.TopLeft() != radii.TopRight() || radii.TopLeft() != radii.BottomRight() || radii.TopLeft() != radii.BottomLeft()) {
        return std::nullopt;
    }
#endif
    ...
    if (&transform != &clip.LocalTransformSpace()) {
        gfx::Transform projection = GeometryMapper::SourceToDestinationProjection(clip.LocalTransformSpace(), transform);
...
int PropertyTreeManager::SynthesizeCcEffectsForClipsIfNeeded(const ClipPaintPropertyNode& target_clip, const EffectPaintPropertyNode* next_effect)
{
    ...
    if (next_effect && next_effect->MayHaveBackdropEffect()) {
        ...
        effect_tree_.Node(current_.effect_id)->render_surface_reason = cc::RenderSurfaceReason::kBackdropScope;
        ...
        const auto& lca = current_.clip->LowestCommonAncestor(target_clip).Unalias();
    ...
    for (; clip_node && clip_node != current_.clip; clip_node = clip_node->UnaliasedParent()) {
    ...
    for (auto i = pending_clips.size(); i--;) {
        ...
        cc::EffectNode& synthetic_effect = *effect_tree_.Node(effect_tree_.Insert(cc::EffectNode(), current_.effect_id));

        const auto& transform
            = should_realize_backdrop_effect ? next_effect->LocalTransformSpace().Unalias() : pending_clip.clip->LocalTransformSpace().Unalias();
        ...
        if (pending_clip.type & CcEffectType::kSyntheticFor2dAxisAlignment) {
            if (should_realize_backdrop_effect) {
                ...
                pending_clip.type = static_cast<CcEffectType>(pending_clip.type | CcEffectType::kSyntheticForNonTrivialClip);
            } else {
                synthetic_effect.element_id = CompositorElementIdFromUniqueObjectId(NewUniqueObjectId());
                synthetic_effect.render_surface_reason = cc::RenderSurfaceReason::kClipAxisAlignment;
        ...
        if (pending_clip.type & CcEffectType::kSyntheticForNonTrivialClip) {
            ...
            if (std::optional<gfx::RRectF> rrect = ShaderBasedRRect(*pending_clip.clip, pending_clip.type, transform, next_effect)) {
                ...
                if (current_.contained_by_non_render_surface_synthetic_rounded_clip) {
                    ...
                    for (auto effect_it = effect_stack_.rbegin(); effect_it != effect_stack_.rend(); ++effect_it) {
                        auto& effect_node = *effect_tree_.Node(effect_it->effect_id);
                        if (effect_node.HasRenderSurface() && !IsConditionalRenderSurfaceReason(effect_node.render_surface_reason)) {
                            break;
                        }
                ...
                synthetic_effect.render_surface_reason
                    = pending_clip.clip->PaintClipRect().IsRounded() ? cc::RenderSurfaceReason::kRoundedCorner : cc::RenderSurfaceReason::kClipPath;
            }
        ...
        SetCurrentEffectState(synthetic_effect, pending_clip.type, *current_.effect, *pending_clip.clip, transform);
    }

    return cc_effect_id_for_backdrop_effect;
}

void PropertyTreeManager::BuildEffectNodesRecursively(const EffectPaintPropertyNode& next_effect)
{
    ...
    if (next_effect.OutputClip()) {
        ...
        real_effect_node_id = SynthesizeCcEffectsForClipsIfNeeded(*output_clip, &next_effect);
    ...
    auto& effect_node = *effect_tree_.Node(effect_tree_.Insert(cc::EffectNode(), current_.effect_id));
    if (real_effect_node_id == cc::kInvalidPropertyNodeId) {
        ...
        DCHECK((!next_effect.ViewTransitionElementResourceId().IsValid() && next_effect.ElementCaptureId()->is_zero()) || !has_multiple_groups)
    ...
    if (has_multiple_groups) {
        if (effect_node.element_id) {
            ...
            effect_node.element_id = CompositorElementIdFromUniqueObjectId(NewUniqueObjectId());
        }
    } else {
        next_effect.SetCcNodeId(new_sequence_number_, real_effect_node_id);
    }

    CompositorElementId compositor_element_id = next_effect.GetCompositorElementId();
    if (compositor_element_id && !has_multiple_groups) {
        DCHECK(!effect_tree_.FindNodeFromElementId(compositor_element_id));
        effect_tree_.SetElementIdForNodeId(real_effect_node_id, compositor_element_id);
    }

    effect_stack_.emplace_back(current_);
    SetCurrentEffectState(effect_node, CcEffectType::kEffect, next_effect, *output_clip, transform);
}

// See IsConditionalRenderSurfaceReason() for the definition of conditional
// render surface.
static cc::RenderSurfaceReason ConditionalRenderSurfaceReasonForEffect(const EffectPaintPropertyNode& effect)
{
...
static cc::RenderSurfaceReason RenderSurfaceReasonForEffect(const EffectPaintPropertyNode& effect)
{
    if (!effect.Filter().IsEmpty() || effect.RequiresCompositingForWillChangeFilter()) {
        return cc::RenderSurfaceReason::kFilter;
    }
    if (effect.HasActiveFilterAnimation())
        return cc::RenderSurfaceReason::kFilterAnimation;
    if (effect.BackdropFilter() || effect.RequiresCompositingForWillChangeBackdropFilter()) {
    ...
    DCHECK(conditional_reason == cc::RenderSurfaceReason::kNone || IsConditionalRenderSurfaceReason(conditional_reason));
    return conditional_reason;
}

void PropertyTreeManager::PopulateCcEffectNode(cc::EffectNode& effect_node, const EffectPaintPropertyNode& effect, int output_clip_id)
{
    ...
    effect_node.view_transition_element_resource_id = effect.ViewTransitionElementResourceId();

    effect_node.subtree_capture_id = viz::SubtreeCaptureId(*effect.ElementCaptureId());
}

void PropertyTreeManager::UpdateConditionalRenderSurfaceReasons(const cc::LayerList& layers)
{
    ...
    for (int id = tree_size - 1; id > cc::kSecondaryRootPropertyNodeId; id--) {
        ...
        if (effect_layer_counts[id] < 2 && IsConditionalRenderSurfaceReason(effect->render_surface_reason) &&
            // kBlendModeDstIn should create a render surface if the mask itself
            // has any child render surface.
            !(effect->render_surface_reason == cc::RenderSurfaceReason::kBlendModeDstIn && has_child_surface[id])) {
...
void PropertyTreeManager::UpdatePixelMovingFilterClipExpanders()
{
    for (const auto& clip : pixel_moving_filter_clip_expanders_) {
        ...
        cc::ClipNode* cc_clip = clip_tree_.Node(clip->CcNodeId(new_sequence_number_));
        DCHECK(cc_clip);
