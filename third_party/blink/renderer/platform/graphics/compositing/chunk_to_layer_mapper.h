...
public:
    ChunkToLayerMapper(const PropertyTreeState& layer_state, const gfx::Vector2dF& layer_offset);

    const PropertyTreeState& LayerState() const
    {
        return layer_state_;
    }
    ...
    const PropertyTreeState& ChunkState() const
    {
        return chunk_state_;
    }
    ...
    gfx::Rect MapVisualRectFromState(const gfx::Rect&, const PropertyTreeState&) const;
    ...
    // clip rect otherwise.
    const FloatClipRect& ClipRect() const
    {
        return clip_rect_;
    }
