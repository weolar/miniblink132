...
public:
    ...
    virtual SynthesizedClip& CreateOrReuseSynthesizedClipLayer(const ClipPaintPropertyNode&, const TransformPaintPropertyNode&, bool needs_layer,
        CompositorElementId& mask_isolation_id, CompositorElementId& mask_effect_id)
        = 0;
    virtual bool NeedsCompositedScrolling(const TransformPaintPropertyNode& scroll_translation) const = 0;
    virtual bool ShouldForceMainThreadRepaint(const TransformPaintPropertyNode& scroll_translation) const = 0;
};
...
public:
    PropertyTreeManager(
        PropertyTreeManagerClient&, cc::PropertyTrees& property_trees, cc::Layer& root_layer, LayerListBuilder& layer_list_builder, int new_sequence_number);
    ...
    int EnsureCompositorScrollAndTransformNode(const TransformPaintPropertyNode& scroll_translation, const gfx::Rect& scrolling_contents_cull_rect);

    // Same as above but marks the scroll nodes as being the viewport.
    int EnsureCompositorInnerScrollAndTransformNode(const TransformPaintPropertyNode& scroll_translation);
    int EnsureCompositorOuterScrollAndTransformNode(const TransformPaintPropertyNode& scroll_translation);
    ...
    int SwitchToEffectNodeWithSynthesizedClip(const EffectPaintPropertyNode& next_effect, const ClipPaintPropertyNode& next_clip, bool layer_draws_content);
    ...
    static bool DirectlyUpdateCompositedOpacityValue(cc::LayerTreeHost&, const EffectPaintPropertyNode&);
    ...
    static bool DirectlyUpdateScrollOffsetTransform(cc::LayerTreeHost&, const TransformPaintPropertyNode&);
    static bool DirectlyUpdateTransform(cc::LayerTreeHost&, const TransformPaintPropertyNode&);
    static bool DirectlyUpdatePageScaleTransform(cc::LayerTreeHost&, const TransformPaintPropertyNode&);

    // This function only updates the cc scroll tree scroll offset and does not
    // update the cc transform node's scroll offset.
    static void DirectlySetScrollOffset(cc::LayerTreeHost&, CompositorElementId, const gfx::PointF&);
    ...
    static void DropCompositorScrollDeltaNextCommit(cc::LayerTreeHost&, CompositorElementId);

    static uint32_t GetMainThreadRepaintReasons(const cc::LayerTreeHost&, const ScrollPaintPropertyNode&);
    // TODO(crbug.com/40517276): Remove this function after launching
    // RasterInducingScroll.
    static bool UsesCompositedScrolling(const cc::LayerTreeHost&, const ScrollPaintPropertyNode&);
    static bool UsesRasterInducingScroll(const cc::LayerTreeHost&, const ScrollPaintPropertyNode&);
    ...
    public:
        ...
        EffectState::Alignment may_be_2d_axis_misaligned_to_render_surface = EffectState::kAligned;
...
private:
    ...
    static std::optional<gfx::RRectF> ShaderBasedRRect(
        const ClipPaintPropertyNode&, CcEffectType type, const TransformPaintPropertyNode& transform, const EffectPaintPropertyNode* next_effect);
    ...
    int SynthesizeCcEffectsForClipsIfNeeded(const ClipPaintPropertyNode& target_clip, const EffectPaintPropertyNode* next_effect);

    void EmitClipMaskLayer();
    void CloseCcEffect();
    void PopulateCcEffectNode(cc::EffectNode&, const EffectPaintPropertyNode& effect, int output_clip_id);

    bool IsCurrentCcEffectSynthetic() const
    {
        return current_.effect_type;
    }
    ...
    bool EffectStateMayBe2dAxisMisalignedToRenderSurface(EffectState&, wtf_size_t index);
    bool CurrentEffectMayBe2dAxisMisalignedToRenderSurface();
    CcEffectType SyntheticEffectType(const ClipPaintPropertyNode&);

    void SetCurrentEffectState(
        const cc::EffectNode&, CcEffectType, const EffectPaintPropertyNode&, const ClipPaintPropertyNode&, const TransformPaintPropertyNode&);

    void UpdatePixelMovingFilterClipExpanders();

    uint32_t NonCompositedMainThreadRepaintReasons(const TransformPaintPropertyNode& scroll_translation) const;
    ...
    HeapVector<Member<const ClipPaintPropertyNode>, 16> pixel_moving_filter_clip_expanders_;
};

} // namespace blink

WTF_ALLOW_MOVE_AND_INIT_WITH_MEM_FUNCTIONS(blink::PropertyTreeManager::EffectState)
