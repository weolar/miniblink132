...
void PreserveNearIntegralBounds(gfx::RectF& bounds)
{
    ...
    if (std::abs(std::round(bounds.x()) - bounds.x()) <= kTolerance && std::abs(std::round(bounds.y()) - bounds.y()) <= kTolerance
        && std::abs(std::round(bounds.right()) - bounds.right()) <= kTolerance && std::abs(std::round(bounds.bottom()) - bounds.bottom()) <= kTolerance) {
...
PendingLayer::PendingLayer(const PaintArtifact& artifact, const PaintChunk& first_chunk, CompositingType compositing_type)
    ...
    , text_known_to_be_on_opaque_background_(first_chunk.text_known_to_be_on_opaque_background)
    , solid_color_chunk_index_(first_chunk.background_color.is_solid_color ? 0 : kNotFound)
    ...
    , hit_test_opaqueness_(first_chunk.hit_test_opaqueness)
{
    ...
    if (const std::optional<gfx::RectF>& visibility_limit = GeometryMapper::VisibilityLimit(GetPropertyTreeState())) {
    ...
    if (compositing_type == kOther && first_chunk.hit_test_data && first_chunk.hit_test_data->scroll_translation) {
        non_composited_scroll_translations_.push_back(first_chunk.hit_test_data->scroll_translation.Get());
    }
}
...
gfx::RectF PendingLayer::MapRectKnownToBeOpaque(const PropertyTreeState& new_state, const FloatClipRect& mapped_layer_bounds) const
{
    ...
    GeometryMapper::LocalToAncestorVisualRect(GetPropertyTreeState(), new_state, float_clip_rect);
...
std::unique_ptr<JSONObject> PendingLayer::ToJSON() const
{
    ...
    result->SetArray("rect_known_to_be_opaque", RectAsJSONArray(rect_known_to_be_opaque_));
    result->SetBoolean("text_known_to_be_on_opaque_background", text_known_to_be_on_opaque_background_);
    result->SetString("property_tree_state", GetPropertyTreeState().ToString());
    result->SetArray("offset_of_decomposited_transforms", VectorAsJSONArray(offset_of_decomposited_transforms_));
    ...
    result->SetString("hit_test_opaqueness", cc::HitTestOpaquenessToString(hit_test_opaqueness_));
    return result;
}

String PendingLayer::DebugName() const
{
    return Chunks().GetPaintArtifact().ClientDebugName(FirstPaintChunk().id.client_id);
}

DOMNodeId PendingLayer::OwnerNodeId() const
{
    return Chunks().GetPaintArtifact().ClientOwnerNodeId(FirstPaintChunk().id.client_id);
}
...
void PendingLayer::Upcast(const PropertyTreeState& new_state)
{
    ...
    DCHECK_EQ(&new_state.Effect(), property_tree_state_.Effect().UnaliasedParent());
...
bool PendingLayer::Matches(const PendingLayer& old_pending_layer) const
{
    if (ChunkRequiresOwnLayer() != old_pending_layer.ChunkRequiresOwnLayer())
        return false;
    if (ChunkRequiresOwnLayer() && compositing_type_ != old_pending_layer.compositing_type_)
        return false;
...
bool PendingLayer::CanMerge(const PendingLayer& guest, LCDTextPreference lcd_text_preference, IsCompositedScrollFunction is_composited_scroll,
    gfx::RectF& merged_bounds, PropertyTreeState& merged_state, gfx::RectF& merged_rect_known_to_be_opaque, bool& merged_text_known_to_be_on_opaque_background,
    wtf_size_t& merged_solid_color_chunk_index, cc::HitTestOpaqueness& merged_hit_test_opaqueness) const
{
    std::optional<PropertyTreeState> optional_merged_state = CanUpcastWith(guest, guest.GetPropertyTreeState(), is_composited_scroll);
    if (!optional_merged_state) {
        return false;
    }

    merged_state = *optional_merged_state;
    const std::optional<gfx::RectF>& merged_visibility_limit = GeometryMapper::VisibilityLimit(merged_state);
    ...
    if (!guest.has_decomposited_blend_mode_ && merged_visibility_limit && *merged_visibility_limit == bounds_ && merged_state == property_tree_state_
        ...
        merged_hit_test_opaqueness
            = cc::UnionHitTestOpaqueness(gfx::ToRoundedRect(bounds_), hit_test_opaqueness_, gfx::ToRoundedRect(guest.bounds_), guest.hit_test_opaqueness_);
        return true;
    }

    FloatClipRect new_home_bounds(bounds_);
    GeometryMapper::LocalToAncestorVisualRect(GetPropertyTreeState(), merged_state, new_home_bounds);
    ...
    GeometryMapper::LocalToAncestorVisualRect(guest.GetPropertyTreeState(), merged_state, new_guest_bounds);
    if (merged_visibility_limit) {
        new_guest_bounds.Rect().Intersect(*merged_visibility_limit);
    }

    merged_bounds = gfx::UnionRects(new_home_bounds.Rect(), new_guest_bounds.Rect());
    ...
    if (!guest.has_decomposited_blend_mode_) {
        float sum_area = new_home_bounds.Rect().size().GetArea() + new_guest_bounds.Rect().size().GetArea();
        if (merged_bounds.size().GetArea() - sum_area > kMergeSparsityAreaTolerance) {
            return false;
        }

        gfx::RectF home_rect_known_to_be_opaque = MapRectKnownToBeOpaque(merged_state, new_home_bounds);
        gfx::RectF guest_rect_known_to_be_opaque = guest.MapRectKnownToBeOpaque(merged_state, new_guest_bounds);
        merged_rect_known_to_be_opaque = gfx::MaximumCoveredRect(home_rect_known_to_be_opaque, guest_rect_known_to_be_opaque);
        merged_text_known_to_be_on_opaque_background = text_known_to_be_on_opaque_background_;
        if (text_known_to_be_on_opaque_background_ != guest.text_known_to_be_on_opaque_background_) {
        ...
        if (lcd_text_preference == LCDTextPreference::kStronglyPreferred && !merged_text_known_to_be_on_opaque_background) {
        ...
        if (IsSolidColor() && new_home_bounds.IsTight() && !guest.draws_content_ && new_home_bounds.Rect() == merged_bounds) {
            // Home's solid color fills the merged layer, and is the only drawing.
            merged_solid_color_chunk_index = solid_color_chunk_index_;
        } else if (guest.IsSolidColor() && new_guest_bounds.IsTight() && new_guest_bounds.Rect() == merged_bounds
            ...
            merged_solid_color_chunk_index = chunks_.size() + guest.solid_color_chunk_index_;
        }
    }

    cc::HitTestOpaqueness home_hit_test_opaqueness = hit_test_opaqueness_;
    if (home_hit_test_opaqueness == cc::HitTestOpaqueness::kOpaque && !new_home_bounds.IsTight()) {
    ...
    if (guest_hit_test_opaqueness == cc::HitTestOpaqueness::kOpaque && !new_guest_bounds.IsTight()) {
        guest_hit_test_opaqueness = cc::HitTestOpaqueness::kMixed;
    }
    merged_hit_test_opaqueness = cc::UnionHitTestOpaqueness(
        gfx::ToRoundedRect(new_home_bounds.Rect()), home_hit_test_opaqueness, gfx::ToRoundedRect(new_guest_bounds.Rect()), guest_hit_test_opaqueness);
...
bool PendingLayer::Merge(const PendingLayer& guest, LCDTextPreference lcd_text_preference, IsCompositedScrollFunction is_composited_scroll)
{
    ...
    cc::HitTestOpaqueness merged_hit_test_opaqueness = cc::HitTestOpaqueness::kMixed;

    if (!CanMerge(guest, lcd_text_preference, is_composited_scroll, merged_bounds, merged_state, merged_rect_known_to_be_opaque,
            merged_text_known_to_be_on_opaque_background, merged_solid_color_chunk_index, merged_hit_test_opaqueness)) {
        return false;
    }
    ...
    text_known_to_be_on_opaque_background_ = merged_text_known_to_be_on_opaque_background;
    has_text_ |= guest.has_text_;
    solid_color_chunk_index_ = merged_solid_color_chunk_index;
    change_of_decomposited_transforms_ = std::max(ChangeOfDecompositedTransforms(), guest.ChangeOfDecompositedTransforms());
    hit_test_opaqueness_ = merged_hit_test_opaqueness;
    non_composited_scroll_translations_.AppendVector(guest.non_composited_scroll_translations_);
    return true;
}

std::optional<PropertyTreeState> PendingLayer::CanUpcastWith(
    const PendingLayer& guest, const PropertyTreeState& guest_state, IsCompositedScrollFunction is_composited_scroll) const
{
    ...
    std::optional<PropertyTreeState> result = GetPropertyTreeState().CanUpcastWith(guest_state, is_composited_scroll);
    ...
    const auto& home_scroll_translation = property_tree_state_.Transform().NearestScrollTranslationNode();
    const auto& guest_scroll_translation = guest.property_tree_state_.Transform().NearestScrollTranslationNode();
    if (&home_scroll_translation == &guest_scroll_translation) {
        return result;
    }
    const auto& lca_scroll_translation = result->Transform().NearestScrollTranslationNode();
    if ((&guest_scroll_translation == &lca_scroll_translation || non_composited_scroll_translations_.Contains(&guest_scroll_translation))
        && (&home_scroll_translation == &lca_scroll_translation || guest.non_composited_scroll_translations_.Contains(&home_scroll_translation))) {
        return result;
    }
    return std::nullopt;
}
...
bool PendingLayer::CanMergeWithDecompositedBlendMode(
    const PendingLayer& guest, const PropertyTreeState& upcast_state, IsCompositedScrollFunction is_composited_scroll) const
{
    return CanUpcastWith(guest, upcast_state, is_composited_scroll).has_value();
}

const TransformPaintPropertyNode& PendingLayer::ScrollTranslationForScrollHitTestLayer() const
{
...
bool PendingLayer::PropertyTreeStateChanged(const PendingLayer* old_pending_layer) const
{
    if (!old_pending_layer || old_pending_layer->property_tree_state_ != property_tree_state_)
        return true;
...
{
    return GeometryMapper::MightOverlapForCompositing(bounds_, GetPropertyTreeState(), other.bounds_, other.GetPropertyTreeState());
}
...
{
    HeapHashMap<Member<const TransformPaintPropertyNode>, bool> can_be_decomposited;
    ...
    for (const PendingLayer& pending_layer : pending_layers) {
        ...
        // clips or effects.
        auto mark_not_decompositable = [&can_be_decomposited](const TransformPaintPropertyNode& transform_node) {
            for (const auto* node = &transform_node; node && !node->IsRoot(); node = node->UnaliasedParent()) {
        ...
        for (const auto* node = &property_state.Transform(); !node->IsRoot() && !can_be_decomposited.Contains(node); node = &node->Parent()->Unalias()) {
            if (!node->IsIdentityOr2dTranslation() || node->ScrollNode() || node->HasDirectCompositingReasonsOtherThan3dTransform()
                || !node->FlattensInheritedTransformSameAsParent() || !node->BackfaceVisibilitySameAsParent()) {
        ...
        for (const auto* node = &property_state.Clip(); !node->IsRoot() && !clips_and_effects_seen.Contains(node); node = &node->Parent()->Unalias()) {
            clips_and_effects_seen.insert(node);
            mark_not_decompositable(node->LocalTransformSpace().Unalias());
        }
        for (const auto* node = &property_state.Effect(); !node->IsRoot() && !clips_and_effects_seen.Contains(node); node = &node->Parent()->Unalias()) {
            clips_and_effects_seen.insert(node);
            mark_not_decompositable(node->LocalTransformSpace().Unalias());
        }
        ...
        if (pending_layer.GetCompositingType() == kScrollHitTestLayer) {
            ...
            mark_not_decompositable(pending_layer.ScrollTranslationForScrollHitTestLayer());
        }
    }
    ...
    for (PendingLayer& pending_layer : pending_layers) {
        ...
        while (!transform->IsRoot() && can_be_decomposited.at(transform)) {
            pending_layer.offset_of_decomposited_transforms_ += transform->Get2dTranslation();
            pending_layer.change_of_decomposited_transforms_ = std::max(pending_layer.ChangeOfDecompositedTransforms(), transform->NodeChanged());
            transform = &transform->Parent()->Unalias();
        }
        pending_layer.property_tree_state_.SetTransform(*transform);
        pending_layer.bounds_.Offset(pending_layer.OffsetOfDecompositedTransforms());
        pending_layer.rect_known_to_be_opaque_.Offset(pending_layer.OffsetOfDecompositedTransforms());
    }
}
...
void PendingLayer::UpdateForeignLayer()
{
    ...
    const auto& foreign_layer_display_item = To<ForeignLayerDisplayItem>(FirstDisplayItem());

    gfx::Vector2dF layer_offset(foreign_layer_display_item.VisualRect().OffsetFromOrigin());
    cc_layer_ = foreign_layer_display_item.GetLayer();
    cc_layer_->SetOffsetToTransformParent(layer_offset + offset_of_decomposited_transforms_);
}
...
void PendingLayer::UpdateScrollHitTestLayer(PendingLayer* old_pending_layer)
{
    ...
    const auto& scroll_node = *ScrollTranslationForScrollHitTestLayer().ScrollNode();
    ...
    cc_layer_->SetOffsetToTransformParent(gfx::Vector2dF(scroll_node.ContainerRect().OffsetFromOrigin()));
...
void PendingLayer::UpdateScrollbarLayer(PendingLayer* old_pending_layer)
{
    ...
    if (old_pending_layer) {
        scrollbar_layer = static_cast<cc::ScrollbarLayerBase*>(std::move(old_pending_layer->cc_layer_).get());
    }

    scrollbar_layer = scrollbar_item.CreateOrReuseLayer(scrollbar_layer.get(), offset_of_decomposited_transforms_);
    DCHECK(!cc_layer_);
    cc_layer_ = std::move(scrollbar_layer);
}

void PendingLayer::UpdateContentLayer(PendingLayer* old_pending_layer, bool tracks_raster_invalidations)
{
    DCHECK(!ChunkRequiresOwnLayer());
    DCHECK(!cc_layer_);
    ...
    if (!content_layer_client_) {
        ...
        content_layer_client_->GetRasterInvalidator().SetTracksRasterInvalidations(tracks_raster_invalidations);
    }
...
bool PendingLayer::UsesSolidColorLayer() const
{
    ...
    if (property_tree_state_.Effect().RequiresCompositingForBackdropFilterMask()) {
        return false;
    }
#if BUILDFLAG(IS_MAC)
...
void PendingLayer::UpdateCompositedLayer(
    PendingLayer* old_pending_layer, cc::LayerSelection& layer_selection, bool tracks_raster_invalidations, cc::LayerTreeHost* layer_tree_host)
{
    ...
    if (!layer.subtree_property_changed() && PropertyTreeStateChanged(old_pending_layer)) {
...
void PendingLayer::UpdateCompositedLayerForRepaint(const PaintArtifact& repainted_artifact, cc::LayerSelection& layer_selection)
{
    ...
    DCHECK_EQ(old_artifact.GetPaintChunks().size(), repainted_artifact.GetPaintChunks().size());
    ...
    if (!ChunkRequiresOwnLayer()) {
        if (UsesSolidColorLayer()) {
            ...
            if (chunks_unchanged) {
                ...
                content_layer_client_->GetRasterInvalidator().SetOldPaintArtifact(Chunks().GetPaintArtifact());
            } else {
                content_layer_client_->UpdateCcPictureLayer(*this);
            }
        }
...
{
    if (content_layer_client_ && content_layer_client_->HasRasterInducingScroll()) {
        content_layer_client_->GetRasterInvalidator().UpdateForRasterInducingScroll(Chunks());
    }
}

void PendingLayer::UpdateLayerProperties(cc::LayerSelection& layer_selection, bool selection_only)
{
    // Properties of foreign layers are managed by their owners.
    if (compositing_type_ == PendingLayer::kForeignLayer) {
        return;
    }
    PaintChunksToCcLayer::UpdateLayerProperties(CcLayer(), GetPropertyTreeState(), Chunks(), layer_selection, selection_only);
}
...
SkColor4f PendingLayer::ComputeBackgroundColor() const
{
    ...
    float min_background_area = kMinBackgroundColorCoverageRatio * bounds_.width() * bounds_.height();
    for (auto it = chunks_.end(); it != chunks_.begin();) {
        ...
        if (chunk.background_color.area >= min_background_area) {
            ...
            if (&chunk_effect != &property_tree_state_.Effect()) {
                if (chunk_effect.UnaliasedParent() != &property_tree_state_.Effect() || !chunk_effect.IsOpacityOnly()) {
                    continue;
                }
    ...
    for (const SkColor4f& color : base::Reversed(background_colors)) {
        background_color = SkColor4f::FromColor(color_utils::GetResultingPaintColor(color.toSkColor(), background_color.toSkColor()));
    }
