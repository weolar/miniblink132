...
std::unique_ptr<JSONObject> CCLayerAsJSON(const cc::Layer& layer, LayerTreeFlags flags)
{
    ...
    if (layer.offset_to_transform_parent() != gfx::Vector2dF()) {
        json->SetArray("position", VectorAsJSONArray(layer.offset_to_transform_parent()));
    }
    ...
    if (!Color::FromSkColor4f(layer.background_color()).IsFullyTransparent()
        ...
        json->SetString("backgroundColor", Color::FromSkColor4f(layer.background_color()).NameForLayoutTreeAsText());
    }

    if (flags & (kLayerTreeIncludesDebugInfo | kLayerTreeIncludesCompositingReasons)) {
        if (layer.debug_info()) {
    ...
    if (RuntimeEnabledFeatures::HitTestOpaquenessEnabled() && (flags & kLayerTreeIncludesDebugInfo)
        && layer.hit_test_opaqueness() != cc::HitTestOpaqueness::kOpaque) {
        json->SetString("hitTestOpaqueness", cc::HitTestOpaquenessToString(layer.hit_test_opaqueness()));
    }
...
int LayersAsJSON::AddTransformJSON(const TransformPaintPropertyNode& transform)
{
    ...
    if (!transform.IsIdentity()) {
        transform_json->SetArray("transform", TransformAsJSONArray(transform.Matrix()));
    }
...
void LayersAsJSON::AddLayer(const cc::Layer& layer, const TransformPaintPropertyNode& transform, const ContentLayerClientImpl* layer_client)
{
    if (!(flags_ & kLayerTreeIncludesAllLayers) && !layer.draws_content()) {
        std::string debug_name = layer.DebugName();
        if (debug_name == "LayoutView #document" || debug_name == "Inner Viewport Scroll Layer" || debug_name == "Scrolling Contents Layer") {
            return;
        }
    }
