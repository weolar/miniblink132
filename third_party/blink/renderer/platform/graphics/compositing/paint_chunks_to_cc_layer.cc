...
public:
    ...
    template <typename T, typename... Args> size_t push(Args&&... args)
    {
...
public:
    ...
    explicit StateEntry(PairedType type, const TransformPaintPropertyNode* transform, const ClipPaintPropertyNode* clip, const EffectPaintPropertyNode* effect,
...
template <typename Result> class ConversionContext {
    STACK_ALLOCATED();

public:
    ConversionContext(
        const PropertyTreeState& layer_state, const gfx::Vector2dF& layer_offset, Result& result, const HeapVector<StateEntry>* outer_state_stack = nullptr)
        ...
        , current_scroll_translation_(&current_transform_->NearestScrollTranslationNode())
        , result_(result)
...
private:
    void Convert(PaintChunkIterator& chunk_it, PaintChunkIterator end_chunk, const gfx::Rect* additional_cull_rect = nullptr);
...
public:
    ...
    void Convert(const PaintChunkSubset& chunks, const gfx::Rect* additional_cull_rect = nullptr)
    {
...
private:
    ...
    [[nodiscard]] ScrollTranslationAction SwitchToClip(const ClipPaintPropertyNode&);
    ...
    [[nodiscard]] ScrollTranslationAction SwitchToEffect(const EffectPaintPropertyNode&);

    // Switch the current transform to the target state.
    [[nodiscard]] ScrollTranslationAction SwitchToTransform(const TransformPaintPropertyNode&);
    ...
    ScrollTranslationAction ComputeScrollTranslationAction(const TransformPaintPropertyNode&) const
    {
        return {};
    }
    void EmitDrawScrollingContentsOp(PaintChunkIterator&, PaintChunkIterator, const TransformPaintPropertyNode&)
    {
        NOTREACHED();
    }
    ...
    gfx::Transform TargetToCurrentProjection(const TransformPaintPropertyNode& target_transform) const
    {
        return GeometryMapper::SourceToDestinationProjection(target_transform, *current_transform_);
    }
    ...
    [[nodiscard]] ScrollTranslationAction StartEffect(const EffectPaintPropertyNode&);
    ...
    [[nodiscard]] ScrollTranslationAction StartClip(const FloatRoundedRect& combined_clip_rect, const ClipPaintPropertyNode& lowest_combined_clip_node);
...
template <typename Result> ConversionContext<Result>::~ConversionContext()
{
    // End all states.
...
template <typename Result> void ConversionContext<Result>::TranslateForLayerOffsetOnce()
{
...
static bool CombineClip(const ClipPaintPropertyNode& clip, FloatRoundedRect& combined_clip_rect)
{
    ...
    if (&transform_space != &parent_transform_space) {
        if (transform_space.Parent() != &parent_transform_space || !transform_space.IsIdentity()) {
            return false;
        }
        // In RasterInducingScroll, don't combine clips across scroll translations.
        if (RuntimeEnabledFeatures::RasterInducingScrollEnabled() && transform_space.ScrollNode()) {
            return false;
        }
    }
    ...
    combined_clip_rect = FloatRoundedRect(IntersectRects(combined_clip_rect.Rect(), clip.PaintClipRect().Rect()));
    return true;
}

template <typename Result> ScrollTranslationAction ConversionContext<Result>::SwitchToClip(const ClipPaintPropertyNode& target_clip)
{
    ...
    {
        const auto* lca_clip = &target_clip.LowestCommonAncestor(*current_clip_).Unalias();
        const auto* clip = current_clip_;
        while (clip != lca_clip) {
            if (!state_stack_.size() && outer_state_stack_ && !outer_state_stack_->empty() && outer_state_stack_->back().IsClip()) {
    ...
    for (const auto* clip = &target_clip; clip != current_clip_; clip = clip->UnaliasedParent()) {
    ...
    for (auto i = pending_clips.size() - 1; i--;) {
        ...
        if (CombineClip(*sub_clip, pending_combined_clip_rect)) {
            ...
            if (auto action = StartClip(pending_combined_clip_rect, *lowest_combined_clip_node)) {
                return action;
            }
...
ScrollTranslationAction ConversionContext<Result>::StartClip(const FloatRoundedRect& combined_clip_rect, const ClipPaintPropertyNode& lowest_combined_clip_node)
{
    if (combined_clip_rect.Rect() == gfx::RectF(InfiniteIntRect())) {
        PushState(StateEntry::kClipOmitted);
    } else {
        const auto& local_transform = lowest_combined_clip_node.LocalTransformSpace().Unalias();
        ...
        if (combined_clip_rect.IsRounded()) {
            push<cc::ClipRRectOp>(SkRRect(combined_clip_rect), SkClipOp::kIntersect, antialias);
        } else {
            push<cc::ClipRectOp>(gfx::RectFToSkRect(combined_clip_rect.Rect()), SkClipOp::kIntersect, antialias);
        }
        if (const auto& clip_path = lowest_combined_clip_node.ClipPath()) {
            push<cc::ClipPathOp>(clip_path->GetSkPath(), SkClipOp::kIntersect, antialias);
        }
        result_.EndPaintOfPairedBegin();
...
bool HasRealEffects(const EffectPaintPropertyNode& current, const EffectPaintPropertyNode& ancestor)
{
...
template <typename Result> ScrollTranslationAction ConversionContext<Result>::SwitchToEffect(const EffectPaintPropertyNode& target_effect)
{
    ...
    const auto& lca_effect = target_effect.LowestCommonAncestor(*current_effect_).Unalias();
    ...
    for (const auto* effect = &target_effect; effect != &lca_effect; effect = effect->UnaliasedParent()) {
        // This should never happen unless the DCHECK in step 1 failed.
        if (!effect)
            break;
...
template <typename Result> ScrollTranslationAction ConversionContext<Result>::StartEffect(const EffectPaintPropertyNode& effect)
{
    ...
    if (!has_filter) {
        ...
        filter_flags.setImageFilter(cc::RenderSurfaceFilters::BuildImageFilter(effect.Filter().AsCcFilterOperations()));
    ...
    effect_bounds_stack_.emplace_back(EffectBoundsInfo { save_layer_id, current_transform_ });
    ...
    if (effect.Filter().HasReferenceFilter()) {
        ...
        gfx::RectF filtered_bounds = current_effect_->MapRect(gfx::RectF(effect.Filter().ReferenceBox().CenterPoint(), gfx::SizeF()));
        ...
        result_.StartPaint();
        result_.EndPaintOfUnpaired(chunk_to_layer_mapper_.MapVisualRect(gfx::ToEnclosingRect(filtered_bounds)));
    }
    return {};
}

template <typename Result> void ConversionContext<Result>::UpdateEffectBounds(const gfx::RectF& bounds, const TransformPaintPropertyNode& transform)
{
    ...
    GeometryMapper::SourceToDestinationRect(transform, *effect_bounds_info.transform, mapped_bounds);
    effect_bounds_info.bounds.Union(mapped_bounds);
}

template <typename Result> void ConversionContext<Result>::EndEffect()
{
#if DCHECK_IS_ON()
    ...
    if (current_effect_->Filter().IsEmpty()) {
        if (!bounds.IsEmpty()) {
            result_.UpdateSaveLayerBounds(bounds_info.save_layer_id, gfx::RectFToSkRect(bounds));
        }
    } else {
...
template <typename Result> ScrollTranslationAction ConversionContext<Result>::EndClips()
{
...
template <typename Result> void ConversionContext<Result>::EndClip()
{
...
template <typename Result> void ConversionContext<Result>::PushState(typename StateEntry::PairedType type)
{
    state_stack_.emplace_back(type, current_transform_, current_clip_, current_effect_, previous_transform_);
    previous_transform_ = nullptr;
}

template <typename Result> void ConversionContext<Result>::PopState()
{
...
template <typename Result> ScrollTranslationAction ConversionContext<Result>::SwitchToTransform(const TransformPaintPropertyNode& target_transform)
{
    if (&target_transform == current_transform_) {
        return {};
    }
...
template <typename Result> void ConversionContext<Result>::EndTransform()
{
...
void ConversionContext<cc::DisplayItemList>::EmitDrawScrollingContentsOp(
    PaintChunkIterator& chunk_it, PaintChunkIterator end_chunk, const TransformPaintPropertyNode& scroll_translation)
{
    ...
    ConversionContext<cc::DisplayItemList>(
        PropertyTreeState(scroll_translation, *current_clip_, *current_effect_), gfx::Vector2dF(), *scrolling_contents_list, &state_stack_)
        .Convert(chunk_it, end_chunk);

    EndTransform();
    scrolling_contents_list->Finalize();

    gfx::Rect visual_rect = chunk_to_layer_mapper_.MapVisualRectFromState(InfiniteIntRect(),
        PropertyTreeState(scroll_translation, *scroll_translation.ScrollNode()->OverflowClipNode(),
            // The effect state doesn't matter.
            chunk_to_layer_mapper_.LayerState().Effect()));
    result_.PushDrawScrollingContentsOp(scroll_translation.ScrollNode()->GetCompositorElementId(), std::move(scrolling_contents_list), visual_rect);
}

template <>
ScrollTranslationAction ConversionContext<cc::DisplayItemList>::ComputeScrollTranslationAction(const TransformPaintPropertyNode& target_transform) const
{
    if (!RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        return {};
    }

    const auto& target_scroll_translation = target_transform.NearestScrollTranslationNode();
    if (&target_scroll_translation == current_scroll_translation_) {
        return {};
    }

    const auto& chunk_scroll_translation = chunk_to_layer_mapper_.ChunkState().Transform().NearestScrollTranslationNode();
    ...
    if (current_scroll_translation_ == target_scroll_translation.ParentScrollTranslationNode()) {
...
template <typename Result> bool ConversionContext<Result>::HasDrawing(PaintChunkIterator chunk_it, const PropertyTreeState& chunk_state) const
{
...
template <typename Result>
void ConversionContext<Result>::Convert(PaintChunkIterator& chunk_it, PaintChunkIterator end_chunk, const gfx::Rect* additional_cull_rect)
{
    for (; chunk_it != end_chunk; ++chunk_it) {
        ...
        if (additional_cull_rect) {
            gfx::Rect chunk_visual_rect = chunk_to_layer_mapper_.MapVisualRect(chunk.drawable_bounds);
            if (additional_cull_rect && !additional_cull_rect->Intersects(chunk_visual_rect)) {
                continue;
            }
        }
        ...
        if (action.type == ScrollTranslationAction::kStart) {
            ...
            EmitDrawScrollingContentsOp(chunk_it, end_chunk, *action.scroll_translation_to_start);
        ...
        for (const auto& item : chunk_it.DisplayItems()) {
            ...
            bool can_ignore_record = &chunk_state.Effect() == &EffectPaintPropertyNode::Root();
            if (record.empty() && can_ignore_record) {
                continue;
            }

            gfx::Rect visual_rect = chunk_to_layer_mapper_.MapVisualRect(item.VisualRect());
            if (additional_cull_rect && can_ignore_record && !additional_cull_rect->Intersects(visual_rect)) {
                continue;
            }
        ...
        UpdateEffectBounds(gfx::RectF(chunk.drawable_bounds), chunk_state.Transform());
    }
}

} // unnamed namespace

void PaintChunksToCcLayer::ConvertInto(const PaintChunkSubset& chunks, const PropertyTreeState& layer_state, const gfx::Vector2dF& layer_offset,
    RasterUnderInvalidationCheckingParams* under_invalidation_checking_params, cc::DisplayItemList& cc_list)
{
    ...
    if (under_invalidation_checking_params) {
        ...
        params.tracking.CheckUnderInvalidations(params.debug_name, recorder.finishRecordingAsPicture(), params.interest_rect);
...
PaintRecord PaintChunksToCcLayer::Convert(const PaintChunkSubset& chunks, const PropertyTreeState& layer_state, const gfx::Rect* cull_rect)
{
    PaintOpBufferExt buffer;
    ConversionContext(layer_state, gfx::Vector2dF(), buffer).Convert(chunks, cull_rect);
...
public:
    ...
    void Trace(Visitor* visitor) const
    {
        visitor->Trace(scroll_translation);
    }
};
...
public:
    LayerPropertiesUpdater(
        cc::Layer& layer, const PropertyTreeState& layer_state, const PaintChunkSubset& chunks, cc::LayerSelection& layer_selection, bool selection_only)
        ...
        , layer_scroll_translation_(layer_state.Transform().NearestScrollTranslationNode())
    {
    }
...
private:
    ...
    const TransformPaintPropertyNode& TopNonCompositedScroll(const TransformPaintPropertyNode&) const;
    ...
    cc::LayerSelectionBound PaintedSelectionBoundToLayerSelectionBound(const PaintedSelectionBound&) const;
...
{
    const auto* scroll_node = chunk_to_layer_mapper_.ChunkState().Transform().NearestScrollTranslationNode().ScrollNode();
    ...
    for (; scroll_node; scroll_node = scroll_node->Parent()) {
        if (scroll_node->UserScrollableHorizontal() && scroll_node->ContainerRect().width() < scroll_node->ContentsRect().width()) {
            last_disable_cursor_control_ = TouchAction::kInternalPanXScrolls;
            break;
        }
        // If it is not kAuto, scroll can't propagate, so break here.
        if (scroll_node->OverscrollBehaviorX() != cc::OverscrollBehavior::Type::kAuto) {
            break;
        }
    }
    return last_disable_cursor_control_;
...
void LayerPropertiesUpdater::UpdateTouchActionRegion(const HitTestData& hit_test_data)
{
    ...
    for (const auto& touch_action_rect : hit_test_data.touch_action_rects) {
        gfx::Rect rect = chunk_to_layer_mapper_.MapVisualRect(touch_action_rect.rect);
        if (rect.IsEmpty()) {
            continue;
        }
...
void LayerPropertiesUpdater::UpdateWheelEventRegion(const HitTestData& hit_test_data)
{
    for (const auto& wheel_event_rect : hit_test_data.wheel_event_rects) {
        wheel_event_region_.Union(chunk_to_layer_mapper_.MapVisualRect(wheel_event_rect));
    }
}
...
void LayerPropertiesUpdater::UpdateScrollHitTestData(const PaintChunk& chunk)
{
    ...
    if (const auto scroll_translation = hit_test_data.scroll_translation) {
        ...
        auto& scroll_tree = layer_.layer_tree_host()->property_trees()->scroll_tree_mutable();
        if (hit_test_data.scrolling_contents_cull_rect.Contains(scroll_node->ContentsRect())) {
            scroll_tree.ClearScrollingContentsCullRect(scroll_element_id);
        } else {
            scroll_tree.SetScrollingContentsCullRect(scroll_element_id, hit_test_data.scrolling_contents_cull_rect);
        }
    ...
    if (RuntimeEnabledFeatures::FastNonCompositedScrollHitTestEnabled() && hit_test_data.scroll_translation) {
        CHECK_EQ(chunk.id.type, DisplayItem::Type::kScrollHitTest);
        AddNonCompositedScroll(chunk);
        return;
    }

    gfx::Rect rect = chunk_to_layer_mapper_.MapVisualRect(hit_test_data.scroll_hit_test_rect);
    if (rect.IsEmpty()) {
        return;
    }
    ...
    // touch action region.
    if (chunk.id.type == DisplayItem::Type::kScrollbarHitTest || chunk.id.type == DisplayItem::Type::kResizerScrollHitTest) {
        touch_action_region_.Union(TouchAction::kNone, rect);
    }
}

const TransformPaintPropertyNode& LayerPropertiesUpdater::TopNonCompositedScroll(const TransformPaintPropertyNode& scroll_translation) const
{
...
void LayerPropertiesUpdater::AddNonCompositedScroll(const PaintChunk& chunk)
{
    ...
    if (&top_scroll == &scroll_translation) {
        ...
        if (hit_test_opaqueness == cc::HitTestOpaqueness::kOpaque && !chunk_to_layer_mapper_.ClipRect().IsTight()) {
            hit_test_opaqueness = cc::HitTestOpaqueness::kMixed;
        }
        top_non_composited_scrolls_.emplace_back(
            &scroll_translation, chunk_to_layer_mapper_.MapVisualRect(chunk.hit_test_data->scroll_hit_test_rect), hit_test_opaqueness);
    } else {
...
void LayerPropertiesUpdater::UpdatePreviousNonCompositedScrolls(const PaintChunk& chunk)
{
    ...
    const auto* scroll_translation = &chunk.properties.Transform().Unalias().NearestScrollTranslationNode();
    if (scroll_translation == &layer_scroll_translation_) {
        ...
        gfx::Rect chunk_hit_test_rect = chunk_to_layer_mapper_.MapVisualRect(chunk.bounds);
    ...
    if (non_composited_scroll->hit_test_opaqueness == cc::HitTestOpaqueness::kTransparent) {
    ...
    if (non_composited_scroll->hit_test_opaqueness == cc::HitTestOpaqueness::kMixed) {
...
const ScrollbarDisplayItem* NonCompositedScrollbarDisplayItem(PaintChunkIterator chunk_it, const cc::Layer& layer)
{
    if (chunk_it->size() != 1) {
        return nullptr;
    }
    const auto* scrollbar = DynamicTo<ScrollbarDisplayItem>(*chunk_it.DisplayItems().begin());
    if (!scrollbar) {
        return nullptr;
    }
...
void LayerPropertiesUpdater::UpdateForNonCompositedScrollbar(const ScrollbarDisplayItem& scrollbar)
{
...
void LayerPropertiesUpdater::UpdateRegionCaptureData(const RegionCaptureData& region_capture_data)
{
    for (const std::pair<RegionCaptureCropId, gfx::Rect>& pair : region_capture_data.map) {
        capture_bounds_.Set(pair.first.value(), chunk_to_layer_mapper_.MapVisualRect(pair.second));
    }
}

gfx::Point LayerPropertiesUpdater::MapSelectionBoundPoint(const gfx::Point& point) const
{
    return gfx::ToRoundedPoint(chunk_to_layer_mapper_.Transform().MapPoint(gfx::PointF(point)));
}

cc::LayerSelectionBound LayerPropertiesUpdater::PaintedSelectionBoundToLayerSelectionBound(const PaintedSelectionBound& bound) const
{
...
void LayerPropertiesUpdater::UpdateLayerSelection(const LayerSelectionData& layer_selection_data)
{
    if (layer_selection_data.start) {
        layer_selection_.start = PaintedSelectionBoundToLayerSelectionBound(*layer_selection_data.start);
        layer_selection_.start.layer_id = layer_.id();
    }

    if (layer_selection_data.end) {
        layer_selection_.end = PaintedSelectionBoundToLayerSelectionBound(*layer_selection_data.end);
...
void LayerPropertiesUpdater::Update()
{
    ...
    for (auto it = chunks_.begin(); it != chunks_.end(); ++it) {
        ...
        const auto* non_composited_scrollbar = NonCompositedScrollbarDisplayItem(it, layer_);
        if ((!selection_only_ && (chunk.hit_test_data || non_composited_scrollbar || chunk.region_capture_data || !top_non_composited_scrolls_.empty()))
        ...
        if (chunk.layer_selection_data) {
            any_selection_was_painted |= chunk.layer_selection_data->any_selection_was_painted;
        ...
        for (const auto& scroll : top_non_composited_scrolls_) {
            if (scroll.hit_test_opaqueness == cc::HitTestOpaqueness::kMixed) {
                ...
                non_composited_scroll_hit_test_rects.emplace_back(
                    scroll.scroll_translation->ScrollNode()->GetCompositorElementId(), scroll.layer_hit_test_rect);
            }
        }
        layer_.SetMainThreadScrollHitTestRegion(std::move(main_thread_scroll_hit_test_region_));
        layer_.SetNonCompositedScrollHitTestRects(std::move(non_composited_scroll_hit_test_rects));
    }
...
void PaintChunksToCcLayer::UpdateLayerProperties(
    cc::Layer& layer, const PropertyTreeState& layer_state, const PaintChunkSubset& chunks, cc::LayerSelection& layer_selection, bool selection_only)
{
    LayerPropertiesUpdater(layer, layer_state, chunks, layer_selection, selection_only).Update();
}
