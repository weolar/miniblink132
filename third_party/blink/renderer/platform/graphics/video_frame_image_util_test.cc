...
public:
    explicit ScopedFakeGpuContext(bool disable_imagebitmap)
    {
        ...
        if (disable_imagebitmap) {
            ...
            feature_info.enabled_gpu_driver_bug_workarounds.push_back(DISABLE_IMAGEBITMAP_FROM_VIDEO_USING_GPU);
        }
...
private:
    ...
    ScopedTestingPlatformSupport<AcceleratedCompositingTestPlatform> accelerated_compositing_scope_;
};

// TODO(crbug.com/1186864): Remove |expect_broken_tagging| when fixed.
void TestOrientation(scoped_refptr<media::VideoFrame> frame, bool expect_broken_tagging = false)
{
    constexpr auto kTestTransform = media::VideoTransformation(media::VIDEO_ROTATION_90, /*mirrored=*/true);
    constexpr auto kTestOrientation = ImageOrientationEnum::kOriginLeftTop;

    frame->metadata().transformation = kTestTransform;
    auto image = CreateImageFromVideoFrame(frame, true, nullptr, nullptr, gfx::Rect(),
...
{
    for (int i = static_cast<int>(ImageOrientationEnum::kMinValue); i <= static_cast<int>(ImageOrientationEnum::kMaxValue); ++i) {
        auto blink_orientation = static_cast<ImageOrientationEnum>(i);
        auto media_transform = ImageOrientationToVideoTransformation(blink_orientation);
        EXPECT_EQ(blink_orientation, VideoTransformationToImageOrientation(media_transform));
    }
}
...
TEST(VideoFrameImageUtilTest, WillCreateAcceleratedImagesFromVideoFrame)
{
    ...
    {
        auto cpu_frame
            = CreateTestFrame(kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_OWNED_MEMORY, media::PIXEL_FORMAT_XRGB, base::TimeDelta());
    ...
    // GpuMemoryBuffer frames aren't supported.
    {
        auto cpu_frame = CreateTestFrame(
            kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER, media::PIXEL_FORMAT_XRGB, base::TimeDelta());
        EXPECT_FALSE(WillCreateAcceleratedImagesFromVideoFrame(cpu_frame.get()));
    }
    ...
    // Single mailbox shared images should be supported on most platforms.
    {
        auto shared_image_frame
            = CreateTestFrame(kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_OPAQUE, media::PIXEL_FORMAT_XRGB, base::TimeDelta());
        EXPECT_TRUE(shared_image_frame->HasSharedImage());
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_MAC)
        EXPECT_FALSE(WillCreateAcceleratedImagesFromVideoFrame(shared_image_frame.get()));
#else
        EXPECT_TRUE(WillCreateAcceleratedImagesFromVideoFrame(shared_image_frame.get()));
#endif
    }
}
...
TEST(VideoFrameImageUtilTest, CreateImageFromVideoFrameZeroCopy)
{
    ...
    EXPECT_EQ(memcmp(image->GetMailboxHolder().mailbox.name, shared_image_frame->shared_image()->mailbox().name, sizeof(gpu::Mailbox::Name)), 0);
}
#endif
...
TEST(VideoFrameImageUtilTest, CreateImageFromVideoFrameGpuMemoryBufferFrame)
{
    base::test::SingleThreadTaskEnvironment task_environment_;
    auto cpu_frame = CreateTestFrame(
        kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER, media::PIXEL_FORMAT_NV12, base::TimeDelta());
    auto image = CreateImageFromVideoFrame(cpu_frame);
...
TEST(VideoFrameImageUtilTest, CreateImageFromVideoFrameTextureFrame)
{
    base::test::SingleThreadTaskEnvironment task_environment_;
    auto cpu_frame
        = CreateTestFrame(kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_OPAQUE, media::PIXEL_FORMAT_NV12, base::TimeDelta());
    auto image = CreateImageFromVideoFrame(cpu_frame);
...
TEST(VideoFrameImageUtilTest, CreateAcceleratedImageFromVideoFrameBasicSoftwareFrame)
{
    ScopedFakeGpuContext fake_context(/*disable_imagebitmap=*/false);
    auto cpu_frame
        = CreateTestFrame(kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_OWNED_MEMORY, media::PIXEL_FORMAT_XRGB, base::TimeDelta());
...
TEST(VideoFrameImageUtilTest, CreateAcceleratedImageFromTextureFrame)
{
    ...
    auto texture_frame = media::CreateSharedImageRGBAFrame(fake_context.raster_context_provider(), kTestSize, gfx::Rect(kTestSize), base::DoNothing());
...
TEST(VideoFrameImageUtilTest, FlushedAcceleratedImage)
{
    ...
    auto texture_frame = media::CreateSharedImageRGBAFrame(raster_context_provider, kTestSize, gfx::Rect(kTestSize), base::DoNothing());

    auto provider = CreateResourceProviderForVideoFrame(kTestInfo, raster_context_provider);
    ASSERT_TRUE(provider);
    EXPECT_TRUE(provider->IsAccelerated());

    auto image = CreateImageFromVideoFrame(texture_frame,
        /*allow_zero_copy_images=*/false, provider.get());
...
TEST(VideoFrameImageUtilTest, DestRectWithoutCanvasResourceProvider)
{
    ...
    auto image = CreateImageFromVideoFrame(cpu_frame, true, nullptr, nullptr, gfx::Rect(0, 0, 10, 10));
    ASSERT_FALSE(image);
    task_environment_.RunUntilIdle();
}
...
TEST(VideoFrameImageUtilTest, CanvasResourceProviderTooSmallForDestRect)
{
    ...
    auto provider = CreateResourceProviderForVideoFrame(SkImageInfo::MakeN32Premul(16, 16), nullptr);
    ASSERT_TRUE(provider);
    EXPECT_FALSE(provider->IsAccelerated());

    auto image = CreateImageFromVideoFrame(cpu_frame, true, provider.get(), nullptr, gfx::Rect(kTestSize));
    ASSERT_FALSE(image);
    task_environment_.RunUntilIdle();
}
...
TEST(VideoFrameImageUtilTest, CanvasResourceProviderDestRect)
{
    ...
    auto provider = CreateResourceProviderForVideoFrame(SkImageInfo::MakeN32Premul(128, 128), nullptr);
    ASSERT_TRUE(provider);
    EXPECT_FALSE(provider->IsAccelerated());

    auto image = CreateImageFromVideoFrame(cpu_frame, true, provider.get(), nullptr, gfx::Rect(16, 16, 64, 64));
    ASSERT_TRUE(image);
    task_environment_.RunUntilIdle();
}
