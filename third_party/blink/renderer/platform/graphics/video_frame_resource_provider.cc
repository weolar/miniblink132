...
VideoFrameResourceProvider::VideoFrameResourceProvider(const cc::LayerTreeSettings& settings, bool use_sync_primitives)
    : settings_(settings)
    , use_sync_primitives_(use_sync_primitives)
{
}
...
void VideoFrameResourceProvider::Initialize(viz::RasterContextProvider* media_context_provider, viz::SharedBitmapReporter* shared_bitmap_reporter,
    scoped_refptr<gpu::ClientSharedImageInterface> shared_image_interface)
{
    ...
    if (context_provider_) {
        max_texture_size = context_provider_->ContextCapabilities().max_texture_size;
    } else {
        max_texture_size = media::limits::kMaxDimension;
    }

    resource_updater_ = std::make_unique<media::VideoResourceUpdater>(media_context_provider, shared_bitmap_reporter, resource_provider_.get(),
        std::move(shared_image_interface), settings_.use_stream_video_draw_quad, settings_.use_gpu_memory_buffer_resources, max_texture_size);
}
...
void VideoFrameResourceProvider::AppendQuads(
    viz::CompositorRenderPass* render_pass, scoped_refptr<media::VideoFrame> frame, media::VideoTransformation media_transform, bool is_opaque)
{
    ...
    resource_updater_->AppendQuad(render_pass, std::move(frame), transform, quad_rect, visible_quad_rect, mask_filter_info,
        /*clip_rect=*/std::nullopt, is_opaque, draw_opacity, sorting_context_id);
}
...
void VideoFrameResourceProvider::PrepareSendToParent(
    const WebVector<viz::ResourceId>& resource_ids, WebVector<viz::TransferableResource>* transferable_resources)
{
    std::vector<viz::TransferableResource> resources_list;
    resource_provider_->PrepareSendToParent(const_cast<WebVector<viz::ResourceId>&>(resource_ids).ReleaseVector(), &resources_list, context_provider_);
    *transferable_resources = std::move(resources_list);
}

void VideoFrameResourceProvider::ReceiveReturnsFromParent(Vector<viz::ReturnedResource> transferable_resources)
{
    std::vector<viz::ReturnedResource> returned_resources(
        std::make_move_iterator(transferable_resources.begin()), std::make_move_iterator(transferable_resources.end()));
