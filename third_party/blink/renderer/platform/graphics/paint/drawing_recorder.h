...
public:
    ...
    static bool UseCachedDrawingIfPossible(GraphicsContext& context, const DisplayItemClient& client, PaintPhase phase)
    {
        return UseCachedDrawingIfPossible(context, client, DisplayItem::PaintPhaseToDrawingType(phase));
    }

    // See DisplayItem::VisualRect() for the definition of visual rect.
    DrawingRecorder(GraphicsContext&, const DisplayItemClient&, DisplayItem::Type, const gfx::Rect& visual_rect);

    DrawingRecorder(GraphicsContext& context, const DisplayItemClient& client, PaintPhase phase, const gfx::Rect& visual_rect)
        : DrawingRecorder(context, client, DisplayItem::PaintPhaseToDrawingType(phase), visual_rect)
    {
    }
    ...
    // rect doesn't matter.
    DrawingRecorder(GraphicsContext& context, const DisplayItemClient& client, DisplayItem::Type type)
        : DrawingRecorder(context, client, type, gfx::Rect())
    {
        DCHECK(!context.GetPaintController().HasPersistentData());
    }
    DrawingRecorder(GraphicsContext& context, const DisplayItemClient& client, PaintPhase phase)
        : DrawingRecorder(context, client, DisplayItem::PaintPhaseToDrawingType(phase))
    {
    }
    ...
    // painting.
    void UniteVisualRect(const gfx::Rect& rect)
    {
        visual_rect_.Union(rect);
    }
...
public:
    ...
    DisableListModificationCheck& operator=(const DisableListModificationCheck&) = delete;
