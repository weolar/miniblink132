...
bool PaintChunk::EqualsForUnderInvalidationChecking(const PaintChunk& other) const
{
    return size() == other.size() && id == other.id && properties == other.properties && bounds == other.bounds
        && base::ValuesEquivalent(hit_test_data, other.hit_test_data) && base::ValuesEquivalent(region_capture_data, other.region_capture_data)
        && drawable_bounds == other.drawable_bounds && raster_effect_outset == other.raster_effect_outset && hit_test_opaqueness == other.hit_test_opaqueness
...
static String ToStringImpl(const PaintChunk& c, const String& id_string, bool concise)
{
    StringBuilder sb;
    sb.AppendFormat("PaintChunk(%u-%u id=%s cacheable=%d bounds=%s from_cache=%d", c.begin_index, c.end_index, id_string.Utf8().c_str(), c.is_cacheable,
        c.bounds.ToString().c_str(), c.is_moved_from_cached_subsequence);
    if (!concise) {
        sb.AppendFormat(" props=(%s) rect_known_to_be_opaque=%s hit_test_opaqueness=%s "
                        "effectively_invisible=%d drawscontent=%d",
            c.properties.ToString().Utf8().c_str(), c.rect_known_to_be_opaque.ToString().c_str(), cc::HitTestOpaquenessToString(c.hit_test_opaqueness),
...
String PaintChunk::ToString(const PaintArtifact& paint_artifact, bool concise) const
{
