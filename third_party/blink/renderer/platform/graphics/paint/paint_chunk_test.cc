...
TEST(PaintChunkTest, MatchesSame)
{
    ...
    EXPECT_TRUE(PaintChunk(0, 1, client, id, properties).Matches(PaintChunk(0, 1, client, id, properties)));
}
...
TEST(PaintChunkTest, MatchesEqual)
{
    ...
    EXPECT_TRUE(PaintChunk(0, 1, client, id, properties).Matches(PaintChunk(0, 1, client, id_equal, properties)));
    EXPECT_TRUE(PaintChunk(0, 1, client, id_equal, properties).Matches(PaintChunk(0, 1, client, id, properties)));
}
...
TEST(PaintChunkTest, IdNotMatches)
{
    ...
    EXPECT_FALSE(PaintChunk(0, 1, client2, id2, properties).Matches(PaintChunk(0, 1, client1, id1, properties)));
}
...
TEST(PaintChunkTest, IdNotMatchesUncacheable)
{
    ...
    DisplayItem::Id id(client.Id(), DisplayItem::kDrawingFirst);
    EXPECT_FALSE(PaintChunk(0, 1, client, id, properties).Matches(PaintChunk(0, 1, client, id, properties)));
}
