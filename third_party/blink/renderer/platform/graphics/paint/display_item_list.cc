...
std::unique_ptr<JSONArray> DisplayItemList::DisplayItemsAsJSON(
    const PaintArtifact& paint_artifact, wtf_size_t first_item_index, const DisplayItemRange& display_items, JsonOption option)
{
    auto json_array = std::make_unique<JSONArray>();
    wtf_size_t i = first_item_index;
    for (auto& item : display_items) {
        if (option == kCompact) {
            json_array->PushString(String::Format("%u: %s", i, item.IdAsString(paint_artifact).Utf8().c_str()));
        } else {
            ...
            if (option == kShowPaintRecords) {
                if (const auto* drawing_item = DynamicTo<DrawingDisplayItem>(item)) {
                    json->SetArray("record", RecordAsJSON(drawing_item->GetPaintRecord()));
                }
            }
