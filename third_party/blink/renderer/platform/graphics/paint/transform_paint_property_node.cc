...
bool Keeps2dAxisAlignmentStatus(const gfx::Transform& a, const gfx::Transform& b)
{
...
PaintPropertyChangeType TransformPaintPropertyNode::State::ComputeTransformChange(const TransformAndOrigin& other, const AnimationState& animation_state) const
{
    ...
    if (animation_state.is_running_animation_on_compositor) {
        ...
        return origin_changed ? PaintPropertyChangeType::kChangedOnlySimpleValues : PaintPropertyChangeType::kChangedOnlyCompositedValues;
    }

    if ((direct_compositing_reasons & CompositingReason::kStickyPosition) || (direct_compositing_reasons & CompositingReason::kAnchorPosition)) {
    ...
    if (matrix_changed && !Keeps2dAxisAlignmentStatus(transform_and_origin.matrix, other.matrix)) {
...
PaintPropertyChangeType TransformPaintPropertyNode::State::ComputeChange(const State& other, const AnimationState& animation_state) const
{
    // Whether or not a node is considered a frame root should be invariant.
    DCHECK_EQ(is_frame_paint_offset_translation, other.is_frame_paint_offset_translation);

    // Changes other than compositing reason and the transform are not simple.
    if (flattens_inherited_transform != other.flattens_inherited_transform || in_subtree_of_page_scale != other.in_subtree_of_page_scale
        || animation_is_axis_aligned != other.animation_is_axis_aligned || is_frame_paint_offset_translation != other.is_frame_paint_offset_translation
        || is_for_svg_child != other.is_for_svg_child || backface_visibility != other.backface_visibility || rendering_context_id != other.rendering_context_id
        ...
        RequiresCullRectExpansion() != other.RequiresCullRectExpansion() || scroll != other.scroll
        || scroll_translation_for_fixed != other.scroll_translation_for_fixed || !base::ValuesEquivalent(sticky_constraint, other.sticky_constraint)
        || !base::ValuesEquivalent(anchor_position_scroll_data, other.anchor_position_scroll_data)
        || visible_frame_element_id != other.visible_frame_element_id) {
        return PaintPropertyChangeType::kChangedOnlyValues;
    }

    auto change = ComputeTransformChange(other.transform_and_origin, animation_state);

    bool non_reraster_values_changed = direct_compositing_reasons != other.direct_compositing_reasons;
    if (non_reraster_values_changed) {
        ...
        return change != PaintPropertyChangeType::kUnchanged ? PaintPropertyChangeType::kChangedOnlyValues
...
TransformPaintPropertyNode::TransformPaintPropertyNode(RootTag)
    ...
    , state_ { .scroll = &ScrollPaintPropertyNode::Root(), .in_subtree_of_page_scale = false }
{
}

const TransformPaintPropertyNode& TransformPaintPropertyNode::Root()
{
    DEFINE_STATIC_LOCAL(Persistent<TransformPaintPropertyNode>, root, (MakeGarbageCollected<TransformPaintPropertyNode>(kRoot)));
    return *root;
}

PaintPropertyChangeType TransformPaintPropertyNode::DirectlyUpdateTransformAndOrigin(
    TransformAndOrigin&& transform_and_origin, const AnimationState& animation_state)
{
    auto change = state_.ComputeTransformChange(transform_and_origin, animation_state);
...
bool TransformPaintPropertyNodeOrAlias::Changed(PaintPropertyChangeType change, const TransformPaintPropertyNodeOrAlias& relative_to_node) const
{
...
void TransformPaintPropertyNodeOrAlias::ClearChangedToRoot(int sequence_number) const
{
    for (auto* n = this; n && n->ChangedSequenceNumber() != sequence_number; n = n->Parent()) {
        ...
        if (const auto* scroll = static_cast<const TransformPaintPropertyNode*>(n)->ScrollNode()) {
...
std::unique_ptr<JSONObject> TransformPaintPropertyNode::ToJSON() const
{
    ...
    if (state_.backface_visibility != BackfaceVisibility::kInherited) {
        json->SetString("backface", state_.backface_visibility == BackfaceVisibility::kVisible ? "visible" : "hidden");
    }
    if (state_.rendering_context_id) {
        json->SetString("renderingContextId", String::Format("%x", state_.rendering_context_id));
    }
    ...
    if (state_.compositor_element_id) {
        json->SetString("compositorElementId", String(state_.compositor_element_id.ToString()));
    }
    if (state_.scroll)
        json->SetString("scroll", String::Format("%p", state_.scroll.Get()));

    if (state_.scroll_translation_for_fixed) {
        json->SetString("scroll_translation_for_fixed", String::Format("%p", state_.scroll_translation_for_fixed.Get()));
    }
    return json;
}
