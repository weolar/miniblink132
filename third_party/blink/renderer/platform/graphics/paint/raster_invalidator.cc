...
const PaintChunk& RasterInvalidator::GetOldChunk(wtf_size_t index) const
{
    ...
    const auto& old_chunk = old_paint_artifact_->GetPaintChunks()[old_chunk_info.index_in_paint_artifact];
#if DCHECK_IS_ON()
...
wtf_size_t RasterInvalidator::MatchNewChunkToOldChunk(const PaintChunk& new_chunk, wtf_size_t old_index) const
{
...
PaintInvalidationReason RasterInvalidator::ChunkPropertiesChanged(const PaintChunk& new_chunk, const PaintChunk& old_chunk,
    const PaintChunkInfo& new_chunk_info, const PaintChunkInfo& old_chunk_info, const PropertyTreeState& layer_state,
    const float absolute_translation_tolerance, const float other_transform_tolerance) const
{
    ...
    if (!new_chunk_info.chunk_to_layer_transform.ApproximatelyEqual(
            old_chunk_info.chunk_to_layer_transform, absolute_translation_tolerance, other_transform_tolerance, other_transform_tolerance)) {
        return PaintInvalidationReason::kPaintProperty;
    }
    ...
    if (!new_chunk.is_moved_from_cached_subsequence) {
        clip_node_is_different = &new_chunk_state.Clip() != &old_chunk.properties.Clip();
        effect_node_is_different = &new_chunk_state.Effect() != &old_chunk.properties.Effect();
    }
    if (effect_node_is_different
        || new_chunk_state.Effect().Changed(PaintPropertyChangeType::kChangedOnlySimpleValues, layer_state, &new_chunk_state.Transform())) {
        return PaintInvalidationReason::kPaintProperty;
    }

    // Check for accumulated clip rect change, if the clip rects are tight.
    if (new_chunk_info.chunk_to_layer_clip.IsTight() && old_chunk_info.chunk_to_layer_clip.IsTight()) {
        ...
        if (!new_clip_in_layer_bounds.Contains(gfx::RectF(new_chunk_info.bounds_in_layer))
            || !old_clip_in_layer_bounds.Contains(gfx::RectF(old_chunk_info.bounds_in_layer))) {
    ...
    // chunk state.
    if (clip_node_is_different || new_chunk_state.Clip().Changed(PaintPropertyChangeType::kChangedOnlySimpleValues, layer_state, &new_chunk_state.Transform()))
        return PaintInvalidationReason::kPaintProperty;

    return PaintInvalidationReason::kNone;
}

static bool ShouldSkipForRasterInvalidation(const PaintChunkIterator& chunk_it)
{
...
static bool ScrollbarNeedsUpdateDisplay(const PaintChunkIterator& chunk_it)
{
    ...
    if (auto* scrollbar = DynamicTo<ScrollbarDisplayItem>(*chunk_it.DisplayItems().begin())) {
...
void RasterInvalidator::GenerateRasterInvalidations(
    const PaintChunkSubset& new_chunks, bool layer_offset_or_state_changed, bool layer_effect_changed, Vector<PaintChunkInfo>& new_chunks_info)
{
    ...
    for (auto it = new_chunks.begin(); it != new_chunks.end(); ++it) {
        ...
        if (matched_old_index == kNotFound) {
            ...
            AddRasterInvalidation(new_chunk_info.bounds_in_layer, new_chunk.id.client_id,
                new_chunk.is_cacheable ? PaintInvalidationReason::kChunkAppeared : PaintInvalidationReason::kChunkUncacheable, kClientIsNew);
            continue;
        }
        ...
        old_chunk_info.bounds_in_layer = ClipByLayerBounds(old_chunk_info.bounds_in_layer);
        ...
        if (!layer_offset_or_state_changed && reason == PaintInvalidationReason::kNone && new_chunk.is_moved_from_cached_subsequence
            && !new_chunk.properties.Changed(PaintPropertyChangeType::kChangedOnlySimpleValues, PropertyTreeState(layer_state_))) {
            ...
            if (reason == PaintInvalidationReason::kNone) {
                if (layer_effect_changed) {
                    ...
                    reason = ChunkPropertiesChanged(new_chunk, old_chunk, new_chunk_info, old_chunk_info, PropertyTreeState(layer_state_),
            ...
            if (IsFullPaintInvalidationReason(reason)) {
                ...
                AddRasterInvalidation(old_chunk_info.bounds_in_layer, new_chunk.id.client_id, reason, kClientIsNew);
                if (old_chunk_info.bounds_in_layer != new_chunk_info.bounds_in_layer) {
                    AddRasterInvalidation(new_chunk_info.bounds_in_layer, new_chunk.id.client_id, reason, kClientIsNew);
                }
                ...
                new_chunk_info.chunk_to_layer_transform = old_chunk_info.chunk_to_layer_transform;

                if (reason == PaintInvalidationReason::kIncremental) {
                    IncrementallyInvalidateChunk(old_chunk_info, new_chunk_info, new_chunk.id.client_id);
                }

                if (&new_chunks.GetPaintArtifact() != old_paint_artifact_ && !new_chunk.is_moved_from_cached_subsequence) {
                    DisplayItemRasterInvalidator(
                        *this, old_paint_artifact_->DisplayItemsInChunk(old_chunk_info.index_in_paint_artifact), it.DisplayItems(), mapper)
                        .Generate();
                }
            }
        }
    ...
    for (wtf_size_t i = 0; i < old_paint_chunks_info_.size(); ++i) {
        ...
        auto reason = old_chunk.is_cacheable ? PaintInvalidationReason::kChunkDisappeared : PaintInvalidationReason::kChunkUncacheable;
        AddRasterInvalidation(old_paint_chunks_info_[i].bounds_in_layer, old_chunk.id.client_id, reason, kClientIsOld);
    }
}

void RasterInvalidator::IncrementallyInvalidateChunk(const PaintChunkInfo& old_chunk_info, const PaintChunkInfo& new_chunk_info, DisplayItemClientId client_id)
{
    SkRegion diff(gfx::RectToSkIRect(old_chunk_info.bounds_in_layer));
    diff.op(gfx::RectToSkIRect(new_chunk_info.bounds_in_layer), SkRegion::kXOR_Op);
    for (SkRegion::Iterator it(diff); !it.done(); it.next()) {
        AddRasterInvalidation(gfx::SkIRectToRect(it.rect()), client_id, PaintInvalidationReason::kIncremental, kClientIsNew);
    }
}

void RasterInvalidator::TrackRasterInvalidation(
    const gfx::Rect& rect, DisplayItemClientId client_id, PaintInvalidationReason reason, ClientIsOldOrNew old_or_new)
{
    if (rect.IsEmpty()) {
        return;
    }
    DCHECK(tracking_);
    String debug_name = old_or_new == kClientIsOld ? old_paint_artifact_->ClientDebugName(client_id) : current_paint_artifact_->ClientDebugName(client_id);
...
void RasterInvalidator::Generate(
    const PaintChunkSubset& new_chunks, const gfx::Vector2dF& layer_offset, const gfx::Size& layer_bounds, const PropertyTreeState& layer_state)
{
    if (RasterInvalidationTracking::ShouldAlwaysTrack())
        EnsureTracking();

    bool layer_offset_or_state_changed = layer_offset_ != layer_offset || layer_state_ != layer_state;
    ...
    if (layer_bounds_was_empty || layer_bounds_.IsEmpty()) {
        ...
        PopulatePaintChunksInfo(new_chunks, layer_offset, layer_state, new_chunks_info);
        if (!layer_bounds.IsEmpty() && !new_chunks.IsEmpty()) {
            AddRasterInvalidation(gfx::Rect(layer_bounds), new_chunks.begin()->id.client_id, PaintInvalidationReason::kFullLayer, kClientIsNew);
        }
    } else {
        GenerateRasterInvalidations(new_chunks, layer_offset_or_state_changed, layer_effect_changed, new_chunks_info);
    }
...
void RasterInvalidator::UpdateForRasterInducingScroll(const PaintChunkSubset& chunks)
{
    old_paint_chunks_info_.Shrink(0);
    PopulatePaintChunksInfo(chunks, layer_offset_, PropertyTreeState(layer_state_), old_paint_chunks_info_);
}

void RasterInvalidator::PopulatePaintChunksInfo(
    const PaintChunkSubset& chunks, const gfx::Vector2dF& layer_offset, const PropertyTreeState& layer_state, Vector<PaintChunkInfo>& chunks_info)
{
...
void RasterInvalidator::SetOldPaintArtifact(const PaintArtifact& old_paint_artifact)
{
...
{
    return sizeof(*this) + old_paint_chunks_info_.capacity() * sizeof(PaintChunkInfo);
}
