...
namespace blink {
template <typename T> static String RectsAsString(const Vector<T>& rects)
{
    StringBuilder sb;
    sb.Append("[");
    bool first = true;
...
String HitTestData::ToString() const
{
    ...
    if (!touch_action_rects.empty()) {
        append_field("touch_action_rects: ", RectsAsString<TouchActionRect>(touch_action_rects));
    }

    if (!wheel_event_rects.empty()) {
        append_field("wheel_event_rects: ", RectsAsString<gfx::Rect>(wheel_event_rects));
    }

    if (!scroll_hit_test_rect.IsEmpty()) {
        append_field("scroll_hit_test_rect: ", String(scroll_hit_test_rect.ToString()));
    }

    if (scroll_translation) {
        append_field("scroll_translation: ", String::Format("%p", scroll_translation.Get()));
        if (scrolling_contents_cull_rect != InfiniteIntRect()) {
            append_field("scrolling_contents_cull_rect: ", String(scrolling_contents_cull_rect.ToString()));
        }
    }
