...
protected:
    ...
    PropertyTreeStateOrAliasData(
        const TransformPaintPropertyNodeOrAlias& transform, const ClipPaintPropertyNodeOrAlias& clip, const EffectPaintPropertyNodeOrAlias& effect)
...
// other objects.
template <typename Data> class PLATFORM_EXPORT PropertyTreeStateOrAliasBase : public Data {
public:
    ...
    explicit PropertyTreeStateOrAliasBase(const PropertyTreeStateOrAliasBase<Other>& other)
        : PropertyTreeStateOrAliasBase(other.Transform(), other.Clip(), other.Effect())
    {
    }

    template <typename Other> void operator=(const PropertyTreeStateOrAliasBase<Other>& other)
    {
    ...
    bool Changed(PaintPropertyChangeType, const PropertyTreeState& relative_to) const;
    ...
    template <typename Other> bool operator==(const PropertyTreeStateOrAliasBase<Other>& other) const
    {
        return this->transform_ == other.transform_ && this->clip_ == other.clip_ && this->effect_ == other.effect_;
    }

protected:
    template <typename Other> friend class PropertyTreeStateOrAliasBase;
};

class PLATFORM_EXPORT PropertyTreeStateOrAlias : public PropertyTreeStateOrAliasBase<PropertyTreeStateOrAliasData> {
public:
...
class TraceablePropertyTreeStateOrAlias : public PropertyTreeStateOrAliasBase<TraceablePropertyTreeStateOrAliasData> {
public:
    using PropertyTreeStateOrAliasBase::PropertyTreeStateOrAliasBase;
    using PropertyTreeStateOrAliasBase::operator=;
};

template <typename Super> class PLATFORM_EXPORT PropertyTreeStateBase : public Super {
public:
    PropertyTreeStateBase(const TransformPaintPropertyNode& transform, const ClipPaintPropertyNode& clip, const EffectPaintPropertyNode& effect)
    ...
    template <typename Other> void operator=(const PropertyTreeStateBase<Other>& other)
    {
        SetTransform(other.Transform());
        SetClip(other.Clip());
        SetEffect(other.Effect());
...
class PLATFORM_EXPORT PropertyTreeState : public PropertyTreeStateBase<PropertyTreeStateOrAlias> {
public:
    ...
    using IsCompositedScrollFunction = base::FunctionRef<bool(const TransformPaintPropertyNode&)>;
    std::optional<PropertyTreeState> CanUpcastWith(const PropertyTreeState& guest, IsCompositedScrollFunction) const;
};

class PLATFORM_EXPORT TraceablePropertyTreeState : public PropertyTreeStateBase<TraceablePropertyTreeStateOrAlias> {
public:
    using PropertyTreeStateBase::PropertyTreeStateBase;
    using PropertyTreeStateBase::operator=;
};

template <typename Data> inline PropertyTreeState PropertyTreeStateOrAliasBase<Data>::Root()
{
    return PropertyTreeState(TransformPaintPropertyNode::Root(), ClipPaintPropertyNode::Root(), EffectPaintPropertyNode::Root());
}

template <typename Data> inline PropertyTreeState PropertyTreeStateOrAliasBase<Data>::Unalias() const
{
    return PropertyTreeState(Transform().Unalias(), Clip().Unalias(), Effect().Unalias());
}

template <typename Data> inline bool PropertyTreeStateOrAliasBase<Data>::Changed(PaintPropertyChangeType change, const PropertyTreeState& relative_to) const
{
    return Transform().Changed(change, relative_to.Transform()) || Clip().Changed(change, relative_to, &Transform())
        || Effect().Changed(change, relative_to, &Transform());
}

template <typename Data> inline bool PropertyTreeStateOrAliasBase<Data>::ChangedToRoot(PaintPropertyChangeType change) const
{
    return Changed(change, Root());
}

template <typename Data> inline String PropertyTreeStateOrAliasBase<Data>::ToString() const
{
    return PropertyTreeStateOrAlias(Transform(), Clip(), Effect()).ToString();
}

#if DCHECK_IS_ON()
template <typename Data> inline String PropertyTreeStateOrAliasBase<Data>::ToTreeString() const
{
    return PropertyTreeStateOrAlias(Transform(), Clip(), Effect()).ToTreeString();
}
#endif

template <typename Data> inline std::unique_ptr<JSONObject> PropertyTreeStateOrAliasBase<Data>::ToJSON() const
{
    return PropertyTreeStateOrAlias(Transform(), Clip(), Effect()).ToJSON();
}

template <typename Data> inline std::ostream& operator<<(std::ostream& os, const PropertyTreeStateOrAliasBase<Data>& state)
{
