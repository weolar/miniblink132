...
ScrollbarDisplayItem::ScrollbarDisplayItem(DisplayItemClientId client_id, Type type, scoped_refptr<cc::Scrollbar> scrollbar, const gfx::Rect& visual_rect,
    const TransformPaintPropertyNode* scroll_translation, CompositorElementId element_id, cc::HitTestOpaqueness hit_test_opaqueness, RasterEffectOutset outset,
    PaintInvalidationReason paint_invalidation_reason)
    : DisplayItem(client_id, type, visual_rect, outset, paint_invalidation_reason,
        /*draws_content*/ true)
    , data_(new Data { std::move(scrollbar), scroll_translation, element_id, hit_test_opaqueness })
{
    DCHECK(IsScrollbar());
    CHECK(!data_->scroll_translation_ || data_->scroll_translation_->ScrollNode());
}
...
scoped_refptr<cc::ScrollbarLayerBase> ScrollbarDisplayItem::CreateOrReuseLayer(
    cc::ScrollbarLayerBase* existing_layer, gfx::Vector2dF offset_of_decomposited_transforms) const
{
    DCHECK(!IsTombstone());
    ...
    layer->SetScrollElementId(data_->scroll_translation_ ? data_->scroll_translation_->ScrollNode()->GetCompositorElementId() : CompositorElementId());
    layer->SetOffsetToTransformParent(gfx::Vector2dF(VisualRect().OffsetFromOrigin()) + offset_of_decomposited_transforms);
...
bool ScrollbarDisplayItem::EqualsForUnderInvalidationImpl(const ScrollbarDisplayItem& other) const
{
    ...
    return data_->scroll_translation_ == other.data_->scroll_translation_ && data_->element_id_ == other.data_->element_id_;
}

#if DCHECK_IS_ON()
void ScrollbarDisplayItem::PropertiesAsJSONImpl(JSONObject& json) const
{
    json.SetString("scrollTranslation", String::Format("%p", data_->scroll_translation_.Get()));
}
#endif

void ScrollbarDisplayItem::Record(GraphicsContext& context, const DisplayItemClient& client, DisplayItem::Type type, scoped_refptr<cc::Scrollbar> scrollbar,
    const gfx::Rect& visual_rect, const TransformPaintPropertyNode* scroll_translation, CompositorElementId element_id,
    cc::HitTestOpaqueness hit_test_opaqueness)
{
    ...
    DCHECK(RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled() || !paint_controller.UseCachedItemIfPossible(client, type));
    CHECK(IsScrollbarElementId(element_id));

    paint_controller.CreateAndAppend<ScrollbarDisplayItem>(client, type, std::move(scrollbar), visual_rect, scroll_translation, element_id, hit_test_opaqueness,
        client.VisualRectOutsetForRasterEffects(), client.GetPaintInvalidationReason());
}
