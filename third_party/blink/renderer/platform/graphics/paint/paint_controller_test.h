...
public:
    ...
    explicit PaintControllerForTest(PaintControllerPersistentData& persistent_data)
...
public:
    ...
    static DrawResult Draw(GraphicsContext& context, const DisplayItemClient& client, DisplayItem::Type type, base::FunctionRef<void()> draw_function);

    static DrawResult DrawNothing(GraphicsContext& context, const DisplayItemClient& client, DisplayItem::Type type)
    {
        return Draw(context, client, type, [&]() { DrawingRecorder recorder(context, client, type, gfx::Rect()); });
    }

    static DrawResult DrawRect(GraphicsContext& context, const DisplayItemClient& client, DisplayItem::Type type, const gfx::Rect& bounds)
    {
...
protected:
    ...
    {
        paint_controller.UpdateCurrentPaintChunkProperties(root_paint_chunk_id_, *root_paint_property_client_, DefaultPaintChunkProperties());
    }
    ...
    static const PaintArtifact& GetNewPaintArtifact(const PaintController& controller)
    {
    ...
    static wtf_size_t NumCachedNewSubsequences(const PaintController& controller)
    {
    ...
    void InvalidateAll()
    {
        persistent_data_->InvalidateAllForTesting();
    }

    const SubsequenceMarkers* GetSubsequenceMarkers(const DisplayItemClient& client)
    {
    ...
    static bool ClientCacheIsValid(const PaintController& paint_controller, const DisplayItemClient& client)
    {
...
private:
    ...
    Persistent<PaintControllerPersistentData> persistent_data_ = MakeGarbageCollected<PaintControllerPersistentData>();
};
...
inline bool CheckChunk(const PaintChunk& chunk, wtf_size_t begin, wtf_size_t end)
{
    return chunk.begin_index == begin && chunk.end_index == end;
}
inline bool CheckChunk(const PaintChunk& chunk, wtf_size_t begin, wtf_size_t end, const PaintChunk::Id& id, const PropertyTreeStateOrAlias& properties,
    const HitTestData* hit_test_data = nullptr, const gfx::Rect* bounds = nullptr)
{
    return chunk.begin_index == begin && chunk.end_index == end && chunk.id == id && chunk.properties == properties
        && ((!chunk.hit_test_data && !hit_test_data) || (chunk.hit_test_data && hit_test_data && *chunk.hit_test_data == *hit_test_data))
...
MATCHER_P6(IsPaintChunk, begin, end, id, properties, hit_test_data, bounds, "")
{
...
const DisplayItem::Type kForegroundType = static_cast<DisplayItem::Type>(DisplayItem::kDrawingPaintPhaseFirst + 5);
const DisplayItem::Type kClipType = DisplayItem::kClipPaintPhaseFirst;

#define EXPECT_SUBSEQUENCE(client, expected_start_chunk_index, expected_end_chunk_index)                                                                       \
    do {                                                                                                                                                       \
        auto* subsequence = GetSubsequenceMarkers(client);                                                                                                     \
        ASSERT_NE(nullptr, subsequence);                                                                                                                       \
        EXPECT_EQ(static_cast<wtf_size_t>(expected_start_chunk_index), subsequence->start_chunk_index);                                                        \
        EXPECT_EQ(static_cast<wtf_size_t>(expected_end_chunk_index), subsequence->end_chunk_index);                                                            \
    } while (false)

#define EXPECT_SUBSEQUENCE_FROM_CHUNK(client, start_chunk_iterator, chunk_count)                                                                               \
    EXPECT_SUBSEQUENCE(client, (start_chunk_iterator).IndexInPaintArtifact(), (start_chunk_iterator).IndexInPaintArtifact() + chunk_count)

#define EXPECT_NO_SUBSEQUENCE(client) EXPECT_EQ(nullptr, GetSubsequenceMarkers(client))
