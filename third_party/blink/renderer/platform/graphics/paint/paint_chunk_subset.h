...
public:
    ...
    {
        wtf_size_t chunk_index = base::checked_cast<wtf_size_t>(&chunk - &paint_artifact_->GetPaintChunks().front());
    ...
    public:
        const PaintChunk& operator*() const
        {
            return GetChunk();
        }
        const PaintChunk* operator->() const
        ...
        {
            auto& chunk = GetChunk();
            return subset_->paint_artifact_->GetDisplayItemList().ItemsInRange(chunk.begin_index, chunk.end_index);
        }
    ...
    bool IsEmpty() const
    {
        return subset_indices_.empty();
    }

    wtf_size_t size() const
    {
        return subset_indices_.size();
    }
    ...
    void SetPaintArtifact(const PaintArtifact& paint_artifact)
    {
        ...
        DCHECK_EQ(paint_artifact.GetPaintChunks().size(), paint_artifact_->GetPaintChunks().size());
        paint_artifact_ = &paint_artifact;
    }
...
PLATFORM_EXPORT std::ostream& operator<<(std::ostream&, const PaintChunkSubset&);
