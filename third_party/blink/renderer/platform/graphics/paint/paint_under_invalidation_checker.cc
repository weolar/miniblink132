...
PaintUnderInvalidationChecker::PaintUnderInvalidationChecker(PaintController& paint_controller)
...
bool PaintUnderInvalidationChecker::IsChecking() const
{
    if (old_item_index_ != kNotFound) {
        DCHECK(RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled());
        DCHECK(subsequence_client_id_ == kInvalidDisplayItemClientId || (old_chunk_index_ != kNotFound && new_chunk_index_ != kNotFound));
        return true;
    }
...
void PaintUnderInvalidationChecker::WouldUseCachedItem(wtf_size_t old_item_index)
{
    DCHECK(!IsChecking());
...
void PaintUnderInvalidationChecker::CheckNewItem()
{
    ...
    NewDisplayItemList().back().SetPaintInvalidationReason(old_item.IsCacheable() ? PaintInvalidationReason::kNone : PaintInvalidationReason::kUncacheable);
...
void PaintUnderInvalidationChecker::WouldUseCachedSubsequence(DisplayItemClientId client_id)
{
    DCHECK(!IsChecking());
...
void PaintUnderInvalidationChecker::WillEndSubsequence(DisplayItemClientId client_id, wtf_size_t start_chunk_index)
{
    DCHECK(IsChecking());
    if (!IsCheckingSubsequence())
        return;
    ...
    } else if (markers->end_chunk_index - markers->start_chunk_index != NewPaintChunks().size() - start_chunk_index) {
...
void PaintUnderInvalidationChecker::CheckNewChunkInternal()
{
    ...
    const auto* markers = paint_controller_.GetSubsequenceMarkers(subsequence_client_id_);
    DCHECK(markers);
    const auto& new_chunk = NewPaintChunks()[new_chunk_index_];
    if (old_chunk_index_ >= markers->end_chunk_index) {
        ShowSubsequenceError("extra chunk", kInvalidDisplayItemClientId, &new_chunk);
    } else {
        const auto& old_chunk = OldPaintChunks()[old_chunk_index_];
        if (!old_chunk.EqualsForUnderInvalidationChecking(new_chunk)) {
            ShowSubsequenceError("chunk changed", kInvalidDisplayItemClientId, &new_chunk, &old_chunk);
        }
    }
    new_chunk_index_++;
    old_chunk_index_++;
...
void PaintUnderInvalidationChecker::ShowItemError(const char* reason, const DisplayItem& new_item, const DisplayItem* old_item) const
{
    if (subsequence_client_id_ != kInvalidDisplayItemClientId) {
        LOG(ERROR) << "(In cached subsequence for " << paint_controller_.new_paint_artifact_->ClientDebugName(subsequence_client_id_) << ")";
    }
    LOG(ERROR) << "Under-invalidation: " << reason;
#if DCHECK_IS_ON()
    LOG(ERROR) << "New display item: " << new_item.AsDebugString(*paint_controller_.new_paint_artifact_);
    if (old_item) {
        LOG(ERROR) << "Old display item: " << old_item->AsDebugString(paint_controller_.CurrentPaintArtifact());
    }
...
void PaintUnderInvalidationChecker::ShowSubsequenceError(
    const char* reason, DisplayItemClientId client_id, const PaintChunk* new_chunk, const PaintChunk* old_chunk)
{
    if (subsequence_client_id_ != kInvalidDisplayItemClientId) {
        LOG(ERROR) << "(In cached subsequence for " << paint_controller_.new_paint_artifact_->ClientDebugName(subsequence_client_id_) << ")";
    }
    ...
    if (client_id != kInvalidDisplayItemClientId) {
        ...
        LOG(ERROR) << "Subsequence client: " << paint_controller_.new_paint_artifact_->ClientDebugName(client_id);
    }
    if (new_chunk) {
        LOG(ERROR) << "New paint chunk: " << new_chunk->ToString(*paint_controller_.new_paint_artifact_);
    }
    if (old_chunk) {
        LOG(ERROR) << "Old paint chunk: " << old_chunk->ToString(paint_controller_.CurrentPaintArtifact());
    }
#if DCHECK_IS_ON()
    paint_controller_.ShowDebugData();
#else
