...
TEST_F(DrawingRecorderTest, Nothing)
{
    ...
    EXPECT_THAT(GetPersistentData().GetDisplayItemList(), ElementsAre(IsSameId(client.Id(), kForegroundType)));
    EXPECT_TRUE(To<DrawingDisplayItem>(GetPersistentData().GetDisplayItemList()[0]).GetPaintRecord().empty());
}
