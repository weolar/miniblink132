...
inline bool operator==(const RasterInvalidationInfo& a, const RasterInvalidationInfo& b)
{
    return a.client_id == b.client_id && a.client_debug_name == b.client_debug_name && a.rect == b.rect && a.reason == b.reason;
}
inline bool operator!=(const RasterInvalidationInfo& a, const RasterInvalidationInfo& b)
{
    return !(a == b);
...
inline std::ostream& operator<<(std::ostream& os, const RasterInvalidationInfo& info)
{
    return os << info.client_id << ":" << info.client_debug_name << " rect=" << info.rect.ToString() << " reason=" << info.reason;
}
...
class PLATFORM_EXPORT RasterInvalidationTracking : public GarbageCollected<RasterInvalidationTracking> {
public:
    ...
    void AddInvalidation(DisplayItemClientId, const String& debug_name, const gfx::Rect&, PaintInvalidationReason);
    bool HasInvalidations() const
    {
    ...
    void CheckUnderInvalidations(const String& layer_debug_name, PaintRecord new_record, const gfx::Rect& new_interest_rect);
