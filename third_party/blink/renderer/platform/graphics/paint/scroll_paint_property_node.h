...
// geometry directly.
class PLATFORM_EXPORT ScrollPaintPropertyNode final : public PaintPropertyNodeBase<ScrollPaintPropertyNode, ScrollPaintPropertyNode> {
public:
    ...
    public:
        ...
        CompositedScrollingPreference composited_scrolling_preference = CompositedScrollingPreference::kDefault;
        MainThreadScrollingReasons main_thread_repaint_reasons = cc::MainThreadScrollingReason::kNotScrollingOnMain;
        ...
        cc::OverscrollBehavior overscroll_behavior = cc::OverscrollBehavior(cc::OverscrollBehavior::Type::kAuto);
    ...
    static ScrollPaintPropertyNode* Create(const ScrollPaintPropertyNode& parent, State&& state)
    {
        return MakeGarbageCollected<ScrollPaintPropertyNode>(kNonParentAlias, parent, std::move(state));
    }
    ...
    PaintPropertyChangeType Update(const ScrollPaintPropertyNode& parent, State&& state, const AnimationState& = AnimationState())
    {
    ...
    {
        return state_.main_thread_repaint_reasons & cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects;
    }
    ...
    ScrollPaintPropertyNode(NonParentAliasTag, const ScrollPaintPropertyNode& parent, State&& state)
...
private:
    ...
    {
#if DCHECK_IS_ON()
        DCHECK(!state_.compositor_element_id || NamespaceFromCompositorElementId(state_.compositor_element_id) == CompositorElementIdNamespace::kScroll);
        DCHECK(cc::MainThreadScrollingReason::AreRepaintReasons(state_.main_thread_repaint_reasons));
#endif
    }
