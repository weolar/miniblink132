...
public:
    ScrollbarDisplayItem(DisplayItemClientId, Type, scoped_refptr<cc::Scrollbar>, const gfx::Rect& visual_rect,
        const TransformPaintPropertyNode* scroll_translation, CompositorElementId element_id, cc::HitTestOpaqueness, RasterEffectOutset outset,
        PaintInvalidationReason paint_invalidation_reason = PaintInvalidationReason::kJustCreated);
    ...
    scoped_refptr<cc::ScrollbarLayerBase> CreateOrReuseLayer(cc::ScrollbarLayerBase* existing_layer, gfx::Vector2dF offset_of_decomposited_transforms) const;
    ...
    static void Record(GraphicsContext&, const DisplayItemClient&, DisplayItem::Type, scoped_refptr<cc::Scrollbar>, const gfx::Rect& visual_rect,
        const TransformPaintPropertyNode* scroll_translation, CompositorElementId element_id, cc::HitTestOpaqueness);

    bool IsOpaque() const;

    static bool IsScrollbarElementId(CompositorElementId element_id)
    {
        return NamespaceFromCompositorElementId(element_id) == CompositorElementIdNamespace::kHorizontalScrollbar
            || NamespaceFromCompositorElementId(element_id) == CompositorElementIdNamespace::kVerticalScrollbar;
    }
...
template <> struct DowncastTraits<ScrollbarDisplayItem> {
