...
protected:
    static const GeometryMapperTransformCache& GetTransformCache(const TransformPaintPropertyNode& transform)
    {
    ...
    static const GeometryMapperTransformCache::ScreenTransform* GetScreenTransform(const TransformPaintPropertyNode& node)
    {
        return base::OptionalToPtr(GetTransformCache(node).screen_transform_);
    }

    static void Check2dTranslationToRoot(const TransformPaintPropertyNode& node, double x, double y)
    {
    ...
    static void CheckRootAsPlaneRoot(const TransformPaintPropertyNode& node, const TransformPaintPropertyNode& root_of_2d_translation,
        const gfx::Transform& to_plane_root, double translate_x, double translate_y)
    {
        const auto& cache = GetTransformCache(node);
        EXPECT_EQ(&root_of_2d_translation, cache.root_of_2d_translation());
        EXPECT_EQ(gfx::Vector2dF(translate_x, translate_y), cache.to_2d_translation_root());
    ...
    static void CheckPlaneRootSameAs2dTranslationRoot(const TransformPaintPropertyNode& node, const gfx::Transform& to_screen,
        const TransformPaintPropertyNode& plane_root, double translate_x, double translate_y)
    {
        const auto& cache = GetTransformCache(node);
    ...
    static void CheckPlaneRootDifferent2dTranslationRoot(const TransformPaintPropertyNode& node, const gfx::Transform& to_screen,
        const TransformPaintPropertyNode& plane_root, const gfx::Transform& to_plane_root, const TransformPaintPropertyNode& root_of_2d_translation,
...
TEST_F(GeometryMapperTransformCacheTest, IntermediatePlaneRootSameAs2dTranslationRoot)
{
    auto* t1 = Create2DTranslation(t0(), 1, 2);
...
TEST_F(GeometryMapperTransformCacheTest, IntermediatePlaneRootDifferent2dTranslationRoot)
{
    auto* t1 = Create2DTranslation(t0(), 1, 2);
    auto* t2 = CreateTransform(*t1, MakeRotationMatrix(0, 45, 0));
    ...
    to_screen.Scale(3, 3);
    CheckPlaneRootDifferent2dTranslationRoot(*t3, to_screen, *t2, to_plane_root, *t3, 0, 0);

    to_plane_root.Translate(7, 8);
    to_screen.Translate(7, 8);
    CheckPlaneRootDifferent2dTranslationRoot(*t4, to_screen, *t2, to_plane_root, *t3, 7, 8);
}
...
TEST_F(GeometryMapperTransformCacheTest, TransformUpdate)
{
    ...
    GeometryMapperTransformCache::ClearCache();
    t2->Update(*t1, TransformPaintPropertyNode::State { { MakeRotationMatrix(0, 45, 0) } });
    Check2dTranslationToRoot(t0(), 0, 0);
    Check2dTranslationToRoot(*t1, 1, 2);
    ...
    GeometryMapperTransformCache::ClearCache();
    t3->Update(*t2, TransformPaintPropertyNode::State { { MakeTranslationMatrix(28, 27) } });
    ...
    GeometryMapperTransformCache::ClearCache();
    t2->Update(*t1, TransformPaintPropertyNode::State { { MakeTranslationMatrix(11, 12) } });
    Check2dTranslationToRoot(t0(), 0, 0);
    Check2dTranslationToRoot(*t1, 1, 2);
