...
// and transform.
int LocalPixelDistanceToExpand(const TransformPaintPropertyNode& root_transform, const TransformPaintPropertyNode& local_transform, float expansion_ratio)
{
    ...
    if (small_scrollers_use_min_cull_rect && !local_transform.RequiresCompositingForRootScroller() && local_transform.ScrollNode()
        && local_transform.ScrollNode()->ContainerRect().size().Area64() <= kSmallScrollerArea * expansion_ratio * expansion_ratio) {
        return min_expansion;
    }

    int local_pixel_distance_to_expand = pixel_distance_to_expand * expansion_ratio;
    float scale = GeometryMapper::SourceToDestinationApproximateMinimumScale(root_transform, local_transform);
...
bool CanExpandForScroll(const ScrollPaintPropertyNode& scroll)
{
    ...
    if (scroll.GetCompositedScrollingPreference() == CompositedScrollingPreference::kNotPreferred) {
        return false;
    }
    if (!scroll.UserScrollable()) {
        return false;
    }
    if (scroll.ContentsRect().width() <= scroll.ContainerRect().width() && scroll.ContentsRect().height() <= scroll.ContainerRect().height()) {
        return false;
    }
    return true;
}
...
bool CullRect::IntersectsTransformed(const AffineTransform& transform, const gfx::RectF& rect) const
{
    if (rect.IsEmpty())
        return false;
...
void CullRect::ApplyTransform(const TransformPaintPropertyNode& transform)
{
    ...
    GeometryMapper::SourceToDestinationRect(*transform.Parent(), transform, rect_);
}

std::pair<bool, bool> CullRect::ApplyScrollTranslation(
    const TransformPaintPropertyNode& root_transform, const TransformPaintPropertyNode& scroll_translation, float expansion_ratio)
{
    ...
    int outset_x = LocalPixelDistanceToExpand(root_transform, scroll_translation, expansion_ratio);
...
bool CullRect::ApplyPaintPropertiesWithoutExpansion(const PropertyTreeState& source, const PropertyTreeState& destination)
{
    FloatClipRect clip_rect = GeometryMapper::LocalToAncestorClipRect(destination, source);
    ...
    if (!IsInfinite()) {
        GeometryMapper::SourceToDestinationRect(source.Transform(), destination.Transform(), rect_);
    }
...
bool CullRect::ApplyPaintProperties(const PropertyTreeState& root, const PropertyTreeState& source, const PropertyTreeState& destination,
    const std::optional<CullRect>& old_cull_rect, float expansion_ratio)
{
    ...
    if (!abnormal_hierarchy) {
        for (const auto* t = &destination.Transform(); t != &source.Transform(); t = t->UnaliasedParent()) {
            if (t == &root.Transform()) {
    ...
    if (abnormal_hierarchy) {
        ...
        *this = Infinite();
        return root != destination && ApplyPaintProperties(root, root, destination, old_cull_rect, expansion_ratio);
    }
    ...
    for (const auto& scroll_translation : base::Reversed(scroll_translations)) {
        const auto* overflow_clip = scroll_translation->ScrollNode()->OverflowClipNode();
        if (!overflow_clip) {
            // This happens on the layout viewport scroll node when the viewport
            // doesn't clip contents (e.g. when printing).
            break;
        }
        if (!ApplyPaintPropertiesWithoutExpansion(PropertyTreeState(*last_transform, *last_clip, effect_root),
                PropertyTreeState(*scroll_translation->UnaliasedParent(), *overflow_clip, effect_root))) {
            return false;
        }
        last_clip = overflow_clip;

        // We only keep the expanded status of the last scroll translation.
        expanded = ApplyScrollTranslation(root.Transform(), *scroll_translation, expansion_ratio);
        last_transform = scroll_translation;
    }

    if (!ApplyPaintPropertiesWithoutExpansion(PropertyTreeState(*last_transform, *last_clip, effect_root), destination))
        return false;
    ...
    if (expanded.first || expanded.second) {
        ...
        if (last_transform != &destination.Transform() || last_clip != &destination.Clip()) {
            // Map expansion_bounds in the same way as we did for rect_ in the last
            // ApplyPaintPropertiesWithoutExpansion().
            FloatClipRect clip_rect = GeometryMapper::LocalToAncestorClipRect(destination, PropertyTreeState(*last_transform, *last_clip, effect_root));
            if (!clip_rect.IsInfinite())
                expansion_bounds->Intersect(gfx::ToEnclosingRect(clip_rect.Rect()));
            GeometryMapper::SourceToDestinationRect(*last_transform, destination.Transform(), *expansion_bounds);
        }
    }
    ...
    if (expansion_ratio > 0 && has_transform_requiring_expansion) {
        ...
        int pixel_distance_to_expand = LocalPixelDistanceToExpand(root.Transform(), destination.Transform(), expansion_ratio);
    ...
    if (old_cull_rect && !ChangedEnough(expanded, *old_cull_rect, expansion_bounds, expansion_ratio)) {
...
bool CullRect::ChangedEnough(
    const std::pair<bool, bool>& expanded, const CullRect& old_cull_rect, const std::optional<gfx::Rect>& expansion_bounds, float expansion_ratio) const
{
    ...
    if (!expanded.first && (rect_.x() != old_rect.x() || rect_.width() != old_rect.width())) {
        return true;
    }
    if (!expanded.second && (rect_.y() != old_rect.y() || rect_.height() != old_rect.height())) {
        return true;
    }
    ...
    if (rect_.x() == expansion_bounds->x() && old_rect.x() != expansion_bounds->x()) {
        return true;
    }
    if (rect_.y() == expansion_bounds->y() && old_rect.y() != expansion_bounds->y()) {
        return true;
    }
    if (rect_.right() == expansion_bounds->right() && old_rect.right() != expansion_bounds->right()) {
        return true;
    }
    if (rect_.bottom() == expansion_bounds->bottom() && old_rect.bottom() != expansion_bounds->bottom()) {
        return true;
    }
...
bool CullRect::HasScrolledEnough(const gfx::Vector2dF& delta, const TransformPaintPropertyNode& scroll_translation, float expansion_ratio)
{
    if (!scroll_translation.ScrollNode() || !CanExpandForScroll(*scroll_translation.ScrollNode())) {
        return !delta.IsZero();
    }
    int changed_enough_minimum_distance = ChangedEnoughMinimumDistance(expansion_ratio);
    if (std::abs(delta.x()) < changed_enough_minimum_distance && std::abs(delta.y()) < changed_enough_minimum_distance) {
        return false;
    }
    ...
    if (Rect().Contains(contents_rect))
        return false;
    return (delta.x() < 0 && Rect().x() != contents_rect.x()) || (delta.x() > 0 && Rect().right() != contents_rect.right())
        || (delta.y() < 0 && Rect().y() != contents_rect.y()) || (delta.y() > 0 && Rect().bottom() != contents_rect.bottom());
}
