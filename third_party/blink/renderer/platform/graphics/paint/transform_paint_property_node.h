...
class PLATFORM_EXPORT TransformPaintPropertyNodeOrAlias : public PaintPropertyNodeBase<TransformPaintPropertyNodeOrAlias, TransformPaintPropertyNode> {
public:
    ...
    bool Changed(PaintPropertyChangeType change, const TransformPaintPropertyNodeOrAlias& relative_to_node) const;
...
class TransformPaintPropertyNodeAlias final : public TransformPaintPropertyNodeOrAlias {
public:
    static TransformPaintPropertyNodeAlias* Create(const TransformPaintPropertyNodeOrAlias& parent)
    {
        return MakeGarbageCollected<TransformPaintPropertyNodeAlias>(kParentAlias, parent);
    }

    // This is public required by MakeGarbageCollected, but the protected tag
    // prevents this from being called from outside.
    TransformPaintPropertyNodeAlias(ParentAliasTag, const TransformPaintPropertyNodeOrAlias& parent)
        : TransformPaintPropertyNodeOrAlias(kParentAlias, parent)
    {
    }
};

class PLATFORM_EXPORT TransformPaintPropertyNode final : public TransformPaintPropertyNodeOrAlias {
public:
    ...
    public:
        ...
        PaintPropertyChangeType ComputeTransformChange(const TransformAndOrigin& other, const AnimationState& animation_state) const;
        ...
        {
            return direct_compositing_reasons & CompositingReason::kRequiresCullRectExpansion;
        }
    ...
    static TransformPaintPropertyNode* Create(const TransformPaintPropertyNodeOrAlias& parent, State&& state)
    {
        return MakeGarbageCollected<TransformPaintPropertyNode>(kNonParentAlias, parent, std::move(state));
    }
    ...
    PaintPropertyChangeType Update(const TransformPaintPropertyNodeOrAlias& parent, State&& state, const AnimationState& animation_state = AnimationState())
    {
    ...
    PaintPropertyChangeType DirectlyUpdateTransformAndOrigin(TransformAndOrigin&& transform_and_origin, const AnimationState& animation_state);
    ...
    {
        return DirectCompositingReasons() & CompositingReason::kAffectedByOuterViewportBoundsDelta;
    }
    ...
    bool BackfaceVisibilitySameAsParent() const
    {
        ...
        if (state_.backface_visibility == BackfaceVisibility::kInherited)
            return true;
        if (state_.backface_visibility == Parent()->Unalias().state_.backface_visibility)
            return true;
    ...
    bool FlattensInheritedTransformSameAsParent() const
    {
        if (IsRoot())
            return true;
        return state_.flattens_inherited_transform == Parent()->Unalias().state_.flattens_inherited_transform;
    }
    ...
    bool HasDirectCompositingReasonsOtherThan3dTransform() const
    {
        return DirectCompositingReasons()
            & ~(CompositingReason::k3DTransform | CompositingReason::k3DScale | CompositingReason::k3DRotate | CompositingReason::k3DTranslate
    ...
    {
        return state_.direct_compositing_reasons
            & (CompositingReason::kWillChangeTransform | CompositingReason::kWillChangeScale | CompositingReason::kWillChangeRotate
    ...
    unsigned RenderingContextId() const
    {
        return state_.rendering_context_id;
    }
    ...
    bool IsForSVGChild() const
    {
        return state_.is_for_svg_child;
    }
    ...
    TransformPaintPropertyNode(NonParentAliasTag, const TransformPaintPropertyNodeOrAlias& parent, State&& state)
