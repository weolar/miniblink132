...
PaintPropertyChangeType ComputeBackdropFilterChange(const EffectPaintPropertyNode::BackdropFilterInfo* a, const EffectPaintPropertyNode::BackdropFilterInfo* b,
    bool is_running_backdrop_filter_animation_on_compositor)
{
    if (!a && !b)
        return PaintPropertyChangeType::kUnchanged;
    if (!a || !b || a->bounds != b->bounds || a->mask_element_id != b->mask_element_id)
        return PaintPropertyChangeType::kChangedOnlyValues;
    if (a->operations != b->operations) {
        return is_running_backdrop_filter_animation_on_compositor ? PaintPropertyChangeType::kChangedOnlyCompositedValues
...
PaintPropertyChangeType EffectPaintPropertyNode::State::ComputeChange(const State& other, const AnimationState& animation_state) const
{
    if (local_transform_space != other.local_transform_space || output_clip != other.output_clip || blend_mode != other.blend_mode
        || view_transition_element_resource_id != other.view_transition_element_resource_id
        || self_or_ancestor_participates_in_view_transition != other.self_or_ancestor_participates_in_view_transition) {
        return PaintPropertyChangeType::kChangedOnlyValues;
    }
    bool opacity_changed = opacity != other.opacity;
    bool opacity_change_is_simple = IsOpacityChangeSimple(opacity, other.opacity, direct_compositing_reasons, other.direct_compositing_reasons);
    ...
    if (filter_changed && !animation_state.is_running_filter_animation_on_compositor) {
        return PaintPropertyChangeType::kChangedOnlyValues;
    }
    auto backdrop_filter_changed = ComputeBackdropFilterChange(
        backdrop_filter_info.get(), other.backdrop_filter_info.get(), animation_state.is_running_backdrop_filter_animation_on_compositor);
    if (backdrop_filter_changed == PaintPropertyChangeType::kChangedOnlyValues) {
        return PaintPropertyChangeType::kChangedOnlyValues;
    }
    bool non_reraster_values_changed = direct_compositing_reasons != other.direct_compositing_reasons || compositor_element_id != other.compositor_element_id;
    bool simple_values_changed = opacity_change_is_simple && !animation_state.is_running_opacity_animation_on_compositor;
    ...
    if (opacity_changed || filter_changed || backdrop_filter_changed != PaintPropertyChangeType::kUnchanged) {
...
bool EffectPaintPropertyNode::State::IsOpacityChangeSimple(
    float opacity, float new_opacity, CompositingReasons direct_compositing_reasons, CompositingReasons new_direct_compositing_reasons)
{
    bool opacity_changed = opacity != new_opacity;
    return opacity_changed
        && ((opacity != 1.f && new_opacity != 1.f)
            || ((direct_compositing_reasons & CompositingReason::kActiveOpacityAnimation)
                && (new_direct_compositing_reasons & CompositingReason::kActiveOpacityAnimation)));
}
...
EffectPaintPropertyNode::EffectPaintPropertyNode(RootTag)
    ...
    , state_ { TransformPaintPropertyNode::Root(), &ClipPaintPropertyNode::Root() }
{
}

const EffectPaintPropertyNode& EffectPaintPropertyNode::Root()
{
    DEFINE_STATIC_LOCAL(Persistent<EffectPaintPropertyNode>, root, (MakeGarbageCollected<EffectPaintPropertyNode>(kRoot)));
    return *root;
}

bool EffectPaintPropertyNodeOrAlias::Changed(
    PaintPropertyChangeType change, const PropertyTreeState& relative_to_state, const TransformPaintPropertyNodeOrAlias* transform_not_to_check) const
{
    ...
    // might change).
    for (const auto* node = this; node && node != &relative_effect; node = node->Parent()) {
        ...
        if (unaliased->HasFilterThatMovesPixels() && &local_transform != transform_not_to_check && local_transform.Changed(change, relative_transform)) {
            return true;
        }
...
void EffectPaintPropertyNodeOrAlias::ClearChangedToRoot(int sequence_number) const
{
    for (auto* n = this; n && n->ChangedSequenceNumber() != sequence_number; n = n->Parent()) {
...
PaintPropertyChangeType EffectPaintPropertyNode::State::ComputeOpacityChange(float new_opacity, const AnimationState& animation_state) const
{
    bool opacity_changed = opacity != new_opacity;
    bool opacity_change_is_simple = State::IsOpacityChangeSimple(opacity, new_opacity, direct_compositing_reasons, direct_compositing_reasons);
    if (opacity_changed && !opacity_change_is_simple) {
        DCHECK(!animation_state.is_running_opacity_animation_on_compositor);
        return PaintPropertyChangeType::kChangedOnlyValues;
    }
...
PaintPropertyChangeType EffectPaintPropertyNode::DirectlyUpdateOpacity(float opacity, const AnimationState& animation_state)
{
...
std::unique_ptr<JSONObject> EffectPaintPropertyNode::ToJSON() const
{
    ...
    json->SetString("localTransformSpace", String::Format("%p", state_.local_transform_space.Get()));
    ...
    if (state_.direct_compositing_reasons != CompositingReason::kNone) {
        json->SetString("directCompositingReasons", CompositingReason::ToString(state_.direct_compositing_reasons));
    }
    if (state_.compositor_element_id) {
        json->SetString("compositorElementId", state_.compositor_element_id.ToString().c_str());
    }
    return json;
}
