...
public:
    ...
    static_assert(kTypeLast < (1 << 8), "DisplayItem::Type should fit in uint8_t");
    ...
    struct Id {
        ...
        public:
            ...
            {
                return client_id == other.client_id && type == other.type && fragment == other.fragment;
            }
    ...
    Id GetId() const
    {
        return Id(client_id_, GetType(), fragment_);
    }
    ...
    const gfx::Rect& VisualRect() const
    {
        return visual_rect_;
    }
    ...
    wtf_size_t Fragment() const
    {
        return fragment_;
    }
    void SetFragment(wtf_size_t fragment)
    {
        fragment_ = fragment;
    }
...
#define DEFINE_CATEGORY_METHODS(Category)                                                                                                                      \
    ...
    bool Is##Category() const                                                                                                                                  \
    {                                                                                                                                                          \
        return Is##Category##Type(GetType());                                                                                                                  \
    }
...
#define DEFINE_PAINT_PHASE_CONVERSION_METHOD(Category)                                                                                                         \
    ...
    {                                                                                                                                                          \
        ...
        return static_cast<Type>(static_cast<int>(paint_phase) + k##Category##PaintPhaseFirst);                                                                \
    }
    ...
    {
        return static_cast<PaintInvalidationReason>(paint_invalidation_reason_) != PaintInvalidationReason::kUncacheable;
    }
...
protected:
    ...
    DisplayItem(const DisplayItemClientId client_id, Type type, const gfx::Rect& visual_rect, RasterEffectOutset raster_effect_outset,
        PaintInvalidationReason paint_invalidation_reason, bool draws_content)
        : client_id_(client_id)
        , visual_rect_(visual_rect)
        , fragment_(0)
        , paint_invalidation_reason_(static_cast<unsigned>(paint_invalidation_reason))
        , type_(type)
...
{
    return a.client_id == b.client_id && a.type == b.type && a.fragment == b.fragment;
}
...
template <> struct HashTraits<blink::DisplayItem::Id::HashKey> : GenericHashTraits<blink::DisplayItem::Id::HashKey> {
    ...
    static bool IsDeletedValue(const Key& id)
    {
        return id.fragment == kNotFound;
    }
