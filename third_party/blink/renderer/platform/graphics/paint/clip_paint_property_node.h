...
class PLATFORM_EXPORT ClipPaintPropertyNodeOrAlias : public PaintPropertyNodeBase<ClipPaintPropertyNodeOrAlias, ClipPaintPropertyNode> {
public:
...
public:
    static ClipPaintPropertyNodeAlias* Create(const ClipPaintPropertyNodeOrAlias& parent)
    {
        return MakeGarbageCollected<ClipPaintPropertyNodeAlias>(kParentAlias, parent);
    }

    // These are public required by MakeGarbageCollected, but the protected tags
    // prevent these from being called from outside.
    ClipPaintPropertyNodeAlias(ParentAliasTag, const ClipPaintPropertyNodeOrAlias& parent)
        : ClipPaintPropertyNodeOrAlias(kParentAlias, parent)
    {
    }
};

class PLATFORM_EXPORT ClipPaintPropertyNode final : public ClipPaintPropertyNodeOrAlias {
public:
    ...
    public:
        State(const TransformPaintPropertyNodeOrAlias& local_transform_space, const gfx::RectF& layout_clip_rect, const FloatRoundedRect& paint_clip_rect)
            : local_transform_space(&local_transform_space)
        {
            SetClipRect(layout_clip_rect, paint_clip_rect);
        }
        State(const TransformPaintPropertyNodeOrAlias& local_transform_space, const EffectPaintPropertyNode* pixel_moving_filter)
            : local_transform_space(&local_transform_space)
        ...
        void SetClipRect(const gfx::RectF& layout_clip_rect_arg, const FloatRoundedRect& paint_clip_rect_arg)
        {
    ...
    static ClipPaintPropertyNode* Create(const ClipPaintPropertyNodeOrAlias& parent, State&& state)
    {
        return MakeGarbageCollected<ClipPaintPropertyNode>(kNonParentAlias, parent, std::move(state));
    }
    ...
    PaintPropertyChangeType Update(const ClipPaintPropertyNodeOrAlias& parent, State&& state, const AnimationState& = AnimationState())
    {
    ...
    {
        return state_.layout_clip_rect_excluding_overlay_scrollbars ? *state_.layout_clip_rect_excluding_overlay_scrollbars : state_.layout_clip_rect_;
    }

    const std::optional<Path>& ClipPath() const
    {
        return state_.clip_path;
    }
    bool ClipPathEquals(const std::optional<Path>& p) const
    {
    ...
    ClipPaintPropertyNode(NonParentAliasTag, const ClipPaintPropertyNodeOrAlias& parent, State&& state)
