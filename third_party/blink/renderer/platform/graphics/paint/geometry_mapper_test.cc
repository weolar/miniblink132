...
class GeometryMapperTest : public testing::Test, public PaintTestConfigurations {
public:
    const GeometryMapperClipCache::ClipCacheEntry* GetCachedClip(
        const ClipPaintPropertyNode& descendant_clip, const PropertyTreeState& ancestor_property_tree_state)
    {
        GeometryMapperClipCache::ClipAndTransform clip_and_transform(
            &ancestor_property_tree_state.Clip(), &ancestor_property_tree_state.Transform(), kIgnoreOverlayScrollbarSize);
        return descendant_clip.GetClipCache().GetCachedClip(clip_and_transform);
    }

    static void LocalToAncestorVisualRectInternal(const PropertyTreeStateOrAlias& local, const PropertyTreeStateOrAlias& ancestor, FloatClipRect& mapping_rect)
    {
        GeometryMapper::LocalToAncestorVisualRectInternalForTesting(local.Unalias(), ancestor.Unalias(), mapping_rect);
    }

    static void LocalToAncestorVisualRectForCompositingOverlap(
        const PropertyTreeStateOrAlias& local, const PropertyTreeStateOrAlias& ancestor, FloatClipRect& mapping_rect)
    {
        GeometryMapper::LocalToAncestorVisualRectInternalForCompositingOverlapForTesting(local.Unalias(), ancestor.Unalias(), mapping_rect);
    }

    static bool MightOverlapForCompositing(const gfx::RectF& rect1, const PropertyTreeState& state1, const gfx::RectF& rect2, const PropertyTreeState& state2)
    {
        bool result = GeometryMapper::MightOverlapForCompositing(rect1, state1, rect2, state2);
        EXPECT_EQ(result, GeometryMapper::MightOverlapForCompositing(rect2, state2, rect1, state1));
        return result;
    }

    static gfx::RectF MapVisualRectAboveScrollForCompositingOverlap(
        const TransformPaintPropertyNode& scroll_translation, const gfx::RectF& rect, const PropertyTreeState& state)
    {
        PropertyTreeState new_state = state;
        gfx::RectF new_rect = rect;
        GeometryMapper::MapVisualRectAboveScrollForCompositingOverlap(scroll_translation, new_rect, new_state);
        EXPECT_EQ(&new_state.Transform(), scroll_translation.UnaliasedParent());
        EXPECT_EQ(&new_state.Clip(), scroll_translation.ScrollNode()->OverflowClipNode()->UnaliasedParent());
        return new_rect;
    }

    // For any rect |r|, MightOverlapForCompositing(rect1, state1, r, state2) is
    // expected to be true iff |r| intersects |rect2| in |state2|.
    void CheckOverlap(const gfx::RectF& rect1, const PropertyTreeState& state1, const gfx::RectF& rect2, const PropertyTreeState& state2);
    ...
    {
        return PropertyTreeStateOrAlias(*local_transform, *local_clip, *local_effect);
    }
    PropertyTreeStateOrAlias AncestorState() const
    {
        return PropertyTreeStateOrAlias(*ancestor_transform, *ancestor_clip, *ancestor_effect);
    }

    // Variables required by CheckMappings(). The tests should set these
    // variables with proper values before calling CheckMappings().
    Persistent<const TransformPaintPropertyNodeOrAlias> local_transform = &TransformPaintPropertyNode::Root();
    Persistent<const ClipPaintPropertyNodeOrAlias> local_clip = &ClipPaintPropertyNode::Root();
    Persistent<const EffectPaintPropertyNodeOrAlias> local_effect = &EffectPaintPropertyNode::Root();
    Persistent<const TransformPaintPropertyNodeOrAlias> ancestor_transform = &TransformPaintPropertyNode::Root();
    Persistent<const ClipPaintPropertyNodeOrAlias> ancestor_clip = &ClipPaintPropertyNode::Root();
    Persistent<const EffectPaintPropertyNodeOrAlias> ancestor_effect = &EffectPaintPropertyNode::Root();
...
#define EXPECT_CLIP_RECT_EQ(expected, actual) EXPECT_CLIP_RECT_NEAR(expected, actual, 0)

void GeometryMapperTest::CheckLocalToAncestorVisualRect()
{
    FloatClipRect actual_visual_rect(input_rect);
    LocalToAncestorVisualRectInternal(LocalState(), AncestorState(), actual_visual_rect);
    EXPECT_CLIP_RECT_EQ(expected_visual_rect, actual_visual_rect);

    actual_visual_rect = FloatClipRect(input_rect);
    LocalToAncestorVisualRectForCompositingOverlap(LocalState(), AncestorState(), actual_visual_rect);
    EXPECT_CLIP_RECT_EQ(
        expected_visual_rect_expanded_for_compositing ? *expected_visual_rect_expanded_for_compositing : expected_visual_rect, actual_visual_rect);
}

void GeometryMapperTest::CheckLocalToAncestorClipRect()
{
    FloatClipRect actual_clip_rect = GeometryMapper::LocalToAncestorClipRect(LocalState(), AncestorState());
...
void GeometryMapperTest::CheckSourceToDestinationRect()
{
    ...
    GeometryMapper::SourceToDestinationRect(*local_transform, *ancestor_transform, actual_transformed_rect);
    EXPECT_EQ(expected_transformed_rect, actual_transformed_rect);
}

void GeometryMapperTest::CheckSourceToDestinationProjection()
{
    gfx::Transform projection = GeometryMapper::SourceToDestinationProjection(*local_transform, *ancestor_transform);
...
void GeometryMapperTest::CheckCachedClip()
{
    ...
    const auto* cached_clip = GetCachedClip(unaliased_local_clip, AncestorState().Unalias());
    if (ancestor_clip == &unaliased_local_clip
        || (ancestor_clip == unaliased_local_clip.Parent() && ancestor_transform == &unaliased_local_clip.LocalTransformSpace())) {
        EXPECT_EQ(nullptr, cached_clip);
        return;
    }
    ASSERT_NE(nullptr, cached_clip);
    EXPECT_CLIP_RECT_EQ(expected_clip, cached_clip->clip_rect);
    EXPECT_EQ(expected_clip_has_transform_animation, cached_clip->has_transform_animation);
    EXPECT_EQ(expected_clip_has_sticky_transform, cached_clip->has_sticky_transform);
}
...
void GeometryMapperTest::CheckOverlap(const gfx::RectF& rect1, const PropertyTreeState& state1, const gfx::RectF& rect2, const PropertyTreeState& state2)
{
    ...
    EXPECT_FALSE(MightOverlapForCompositing(rect1, state1, top_left - gfx::Vector2dF(1, 0), state2));
    EXPECT_FALSE(MightOverlapForCompositing(rect1, state1, top_left - gfx::Vector2dF(1, 1), state2));
    EXPECT_FALSE(MightOverlapForCompositing(rect1, state1, top_left - gfx::Vector2dF(0, 1), state2));

    gfx::RectF top_right(rect2.top_right(), gfx::SizeF(1, 1));
    EXPECT_FALSE(MightOverlapForCompositing(rect1, state1, top_right, state2));
    EXPECT_TRUE(MightOverlapForCompositing(rect1, state1, top_right - gfx::Vector2dF(1, 0), state2));
    EXPECT_FALSE(MightOverlapForCompositing(rect1, state1, top_right - gfx::Vector2dF(1, 1), state2));
    EXPECT_FALSE(MightOverlapForCompositing(rect1, state1, top_right - gfx::Vector2dF(0, 1), state2));
    ...
    EXPECT_FALSE(MightOverlapForCompositing(rect1, state1, bottom_right - gfx::Vector2dF(1, 0), state2));
    EXPECT_TRUE(MightOverlapForCompositing(rect1, state1, bottom_right - gfx::Vector2dF(1, 1), state2));
    EXPECT_FALSE(MightOverlapForCompositing(rect1, state1, bottom_right - gfx::Vector2dF(0, 1), state2));
    ...
    EXPECT_FALSE(MightOverlapForCompositing(rect1, state1, bottom_left - gfx::Vector2dF(1, 0), state2));
    EXPECT_FALSE(MightOverlapForCompositing(rect1, state1, bottom_left - gfx::Vector2dF(1, 1), state2));
    EXPECT_TRUE(MightOverlapForCompositing(rect1, state1, bottom_left - gfx::Vector2dF(0, 1), state2));
}
...
TEST_P(GeometryMapperTest, RotationAndScaleTransformWithTransformOrigin)
{
    expected_transform = MakeRotationMatrix(45);
    expected_transform->Scale(2, 2);
    local_transform = CreateTransform(t0(), *expected_transform, gfx::Point3F(50, 50, 0));
...
TEST_P(GeometryMapperTest, NestedTransformsFlattening)
{
    ...
    local_transform = TransformPaintPropertyNode::Create(*transform1, std::move(inverse_state));
...
{
    auto* clip = CreateClip(c0(), t0(), gfx::RectF(10, 10, 50.5, 50.5), FloatRoundedRect(10, 10, 50, 51));
    local_clip = clip;
...
{
    ClipPaintPropertyNode::State clip_state(t0(), gfx::RectF(10, 10, 50, 50), FloatRoundedRect(10, 10, 50, 50));
    clip_state.layout_clip_rect_excluding_overlay_scrollbars = FloatClipRect(gfx::RectF(10, 10, 45, 43));
    local_clip = ClipPaintPropertyNode::Create(c0(), std::move(clip_state));

    input_rect = gfx::RectF(0, 0, 100, 100);

    FloatClipRect actual_visual_rect(input_rect);
    GeometryMapper::LocalToAncestorVisualRect(LocalState(), AncestorState(), actual_visual_rect, kExcludeOverlayScrollbarSizeForHitTesting);
    EXPECT_CLIP_RECT_EQ(FloatClipRect(gfx::RectF(10, 10, 45, 43)), actual_visual_rect);

    // Check that not passing kExcludeOverlayScrollbarSizeForHitTesting gives
    // a different result.
    actual_visual_rect = FloatClipRect(input_rect);
    GeometryMapper::LocalToAncestorVisualRect(LocalState(), AncestorState(), actual_visual_rect, kIgnoreOverlayScrollbarSize);
    EXPECT_CLIP_RECT_EQ(FloatClipRect(gfx::RectF(10, 10, 50, 50)), actual_visual_rect);

    FloatClipRect actual_clip_rect = GeometryMapper::LocalToAncestorClipRect(LocalState(), AncestorState(), kExcludeOverlayScrollbarSizeForHitTesting);
    EXPECT_CLIP_RECT_EQ(FloatClipRect(gfx::RectF(10, 10, 45, 43)), actual_clip_rect);

    // Check that not passing kExcludeOverlayScrollbarSizeForHitTesting gives
    // a different result.
    actual_clip_rect = GeometryMapper::LocalToAncestorClipRect(LocalState(), AncestorState(), kIgnoreOverlayScrollbarSize);
    EXPECT_CLIP_RECT_EQ(FloatClipRect(gfx::RectF(10, 10, 50, 50)), actual_clip_rect);
}
...
TEST_P(GeometryMapperTest, SimpleClipInclusiveIntersect)
{
    ...
    GeometryMapper::LocalToAncestorVisualRect(LocalState(), AncestorState(), actual_clip_rect, kIgnoreOverlayScrollbarSize, kEdgeInclusive);
    EXPECT_CLIP_RECT_EQ(FloatClipRect(gfx::RectF(60, 10, 0, 10)), actual_clip_rect);

    // Check that not passing kExcludeOverlayScrollbarSizeForHitTesting gives
    // a different result.
    actual_clip_rect.SetRect(gfx::RectF(60, 10, 10, 10));
    GeometryMapper::LocalToAncestorVisualRect(LocalState(), AncestorState(), actual_clip_rect);
...
TEST_P(GeometryMapperTest, SimpleClipPlusOpacity)
{
    ...
    FloatClipRect actual_clip_rect(gfx::RectF(60, 10, 10, 10));
    auto intersects = GeometryMapper::LocalToAncestorVisualRect(LocalState(), AncestorState(), actual_clip_rect);
...
TEST_P(GeometryMapperTest, SimpleClipPlusOpacityInclusiveIntersect)
{
    ...
    auto intersects = GeometryMapper::LocalToAncestorVisualRect(LocalState(), AncestorState(), actual_clip_rect, kIgnoreOverlayScrollbarSize, kEdgeInclusive);
...
{
    FloatRoundedRect rect(gfx::RectF(10, 10, 50, 50), FloatRoundedRect::Radii(gfx::SizeF(1, 1), gfx::SizeF(), gfx::SizeF(), gfx::SizeF()));
...
{
    FloatRoundedRect clip_rect1(gfx::RectF(10, 10, 30, 40), FloatRoundedRect::Radii(gfx::SizeF(1, 1), gfx::SizeF(), gfx::SizeF(), gfx::SizeF()));
...
TEST_P(GeometryMapperTest, TwoClipsTransformAbove)
{
    ...
    FloatRoundedRect clip_rect1(gfx::RectF(10, 10, 50, 50), FloatRoundedRect::Radii(gfx::SizeF(1, 1), gfx::SizeF(), gfx::SizeF(), gfx::SizeF()));
...
TEST_P(GeometryMapperTest, ClipBeforeTransform)
{
    expected_transform = MakeRotationMatrix(45);
    local_transform = CreateTransform(t0(), *expected_transform);
    auto* clip = CreateClip(c0(), *local_transform, FloatRoundedRect(10, 10, 50, 50));
    local_clip = clip;
...
TEST_P(GeometryMapperTest, ExpandVisualRectWithClipBeforeSticky)
{
    ...
    local_transform = CreateTransform(t0(), *expected_transform, gfx::Point3F(), CompositingReason::kStickyPosition);
    auto* clip = CreateClip(c0(), *local_transform, FloatRoundedRect(10, 10, 50, 50));
    local_clip = clip;
...
TEST_P(GeometryMapperTest, TwoClipsWithTransformBetween)
{
    ...
    expected_transform = MakeRotationMatrix(45);
    local_transform = CreateTransform(t0(), *expected_transform);
    auto* clip2 = CreateClip(*clip1, *local_transform, FloatRoundedRect(10, 10, 200, 200));
    local_clip = clip2;
...
TEST_P(GeometryMapperTest, ExpandVisualRectWithTwoClipsWithAnimatingTransformBetween)
{
    auto* clip1 = CreateClip(c0(), t0(), FloatRoundedRect(10, 10, 200, 200));
    expected_transform = MakeRotationMatrix(45);
    local_transform = CreateAnimatingTransform(t0(), *expected_transform);
...
TEST_P(GeometryMapperTest, SiblingTransforms)
{
    ...
    GeometryMapper::LocalToAncestorVisualRect(transform1_state, transform2_state, result_clip);
    ...
    GeometryMapper::LocalToAncestorVisualRect(transform2_state, transform1_state, result_clip);
...
TEST_P(GeometryMapperTest, SiblingTransformsWithClip)
{
    ...
    LocalToAncestorVisualRectInternal(transform1_state, transform2_and_clip_state, result);
    ...
    GeometryMapper::LocalToAncestorVisualRect(transform2_and_clip_state, transform1_state, result);
...
TEST_P(GeometryMapperTest, FilterWithClipsAndTransforms)
{
    ...
    auto* transform_below_effect = Create2DTranslation(*transform_above_effect, 20, 30);

    // This clip is between transformAboveEffect and the effect.
    auto* clip_above_effect = CreateClip(c0(), *transform_above_effect, FloatRoundedRect(-100, -100, 200, 200));

    CompositorFilterOperations filters;
    filters.AppendBlurFilter(20);
    auto* effect = CreateFilterEffect(e0(), *transform_above_effect, clip_above_effect, filters);
    auto* clip_expander = CreatePixelMovingFilterClipExpander(*clip_above_effect, *effect);

    // This clip is between the effect and transform_below_effect.
    auto* clip_below_effect = CreateClip(*clip_expander, *transform_above_effect, FloatRoundedRect(10, 10, 100, 100));
    ...
    expected_translation_2d = transform_above_effect->Get2dTranslation() + transform_below_effect->Get2dTranslation();
    expected_transformed_rect = input_rect;
    expected_transformed_rect.Offset(expected_translation_2d);
...
TEST_P(GeometryMapperTest, FilterWithClipsAndTransformsWithAlias)
{
    ...
    CompositorFilterOperations filters;
    filters.AppendBlurFilter(20);
    auto* real_effect = CreateFilterEffect(e0(), *transform_above_effect, clip_above_effect, filters);
    auto* clip_expander = CreatePixelMovingFilterClipExpander(*clip_above_effect, *real_effect);
...
TEST_P(GeometryMapperTest, ExpandVisualRectWithTwoClipsWithAnimatingFilterBetween)
{
    auto* clip1 = CreateClip(c0(), t0(), FloatRoundedRect(10, 10, 200, 200));
    auto* effect = CreateAnimatingFilterEffect(e0(), CompositorFilterOperations(), clip1);
    auto* clip_expander = CreatePixelMovingFilterClipExpander(*clip1, *effect);

    auto* clip2 = CreateClip(*clip_expander, t0(), FloatRoundedRect(50, 0, 200, 50));
    local_clip = clip2;
    local_effect = effect;
...
{
    CompositorFilterOperations filters;
    filters.AppendReferenceFilter(paint_filter_builder::BuildBoxReflectFilter(BoxReflection(BoxReflection::kHorizontalReflection, 0), nullptr));
...
TEST_P(GeometryMapperTest, IgnoreFilters)
{
    ...
    GeometryMapper::LocalToAncestorVisualRect(LocalState(), AncestorState(), actual_clip_rect, kIgnoreOverlayScrollbarSize, kIgnoreFilters);
...
TEST_P(GeometryMapperTest, Precision)
{
    ...
    EXPECT_TRUE(GeometryMapper::SourceToDestinationProjection(*t4, *t4).IsIdentity());
    EXPECT_TRUE(GeometryMapper::SourceToDestinationProjection(*t3, *t4).IsIdentity());
    EXPECT_TRUE(GeometryMapper::SourceToDestinationProjection(*t2, *t4).IsIdentity());
    EXPECT_TRUE(GeometryMapper::SourceToDestinationProjection(*t3, *t2).IsIdentity());
    EXPECT_TRUE(GeometryMapper::SourceToDestinationProjection(*t4, *t2).IsIdentity());
    EXPECT_TRUE(GeometryMapper::SourceToDestinationProjection(*t4, *t3).IsIdentity());
    EXPECT_TRUE(GeometryMapper::SourceToDestinationProjection(*t2, *t3).IsIdentity());
}
...
TEST_P(GeometryMapperTest, MightOverlapCommonClipAncestor)
{
    ...
    auto* c3 = CreateClip(*common_clip, t0(), FloatRoundedRect(100, 100, 100, 100));
...
TEST_P(GeometryMapperTest, MightOverlapFixed)
{
    ...
    auto scroll_state1 = CreateScrollTranslationState(PropertyTreeState(*viewport, c0(), e0()), -1234, -567, gfx::Rect(0, 0, 800, 600), gfx::Size(2400, 1800));
    auto* fixed_transform = CreateFixedPositionTranslation(*viewport, 100, 200, scroll_state1.Transform());
    ...
    // which is (100, 200, 1700, 1300) in the scrolling space.
    {
        ...
        CheckOverlap(gfx::RectF(0, 0, 100, 100), fixed_state, gfx::RectF(100, 200, 1700, 1300), scroll_state1);
    }

    {
        SCOPED_TRACE("fixed_state and scroll_state2");
        auto scroll_state2 = CreateScrollTranslationState(scroll_state1, -2345, -678, gfx::Rect(20, 10, 200, 100), gfx::Size(3000, 2000));
        ...
        EXPECT_FALSE(MightOverlapForCompositing(gfx::RectF(0, 0, 100, 100), fixed_state, gfx::RectF(1, 2, 3, 4), scroll_state2));
        EXPECT_FALSE(MightOverlapForCompositing(gfx::RectF(0, 0, 100, 100), fixed_state, gfx::RectF(0, 0, 1000, 1000), scroll_state2));
    }
    {
        SCOPED_TRACE("fixed_state and scroll_state3");
        auto scroll_state3 = CreateScrollTranslationState(scroll_state1, -234, -567, gfx::Rect(0, 300, 500, 500), gfx::Size(1000, 2000));
        EXPECT_FALSE(MightOverlapForCompositing(gfx::RectF(0, 0, 100, 100), fixed_state, gfx::RectF(1, 2, 3, 4), scroll_state3));
        EXPECT_TRUE(MightOverlapForCompositing(gfx::RectF(0, 0, 100, 100), fixed_state, gfx::RectF(0, 0, 500, 500), scroll_state3));
    }
}
...
{
    auto* viewport = CreateTransform(t0(), gfx::Transform());
    auto scroll_state = CreateScrollTranslationState(PropertyTreeState(*viewport, c0(), e0()), -1234, -567, gfx::Rect(0, 0, 800, 600), gfx::Size(2400, 1800));
    auto* fixed_transform = CreateFixedPositionTranslation(*viewport, 100, 200, scroll_state.Transform());
    ...
    CheckOverlap(gfx::RectF(0, 0, 100, 100), fixed_state, gfx::RectF(100, 200, 1800, 1500), scroll_state);
}
...
TEST_P(GeometryMapperTest, MightOverlapWithScrollingClip)
{
    ...
    auto scroll_state = CreateScrollTranslationState(PropertyTreeState(*viewport, c0(), e0()), -1234, -567, gfx::Rect(0, 0, 800, 600), gfx::Size(2400, 1800));
    auto* fixed_transform = CreateFixedPositionTranslation(*viewport, 100, 200, scroll_state.Transform());
    auto* scrolling_clip = CreateClip(scroll_state.Clip(), scroll_state.Transform(), FloatRoundedRect(0, 1000, 100, 100));
    ...
    CheckOverlap(gfx::RectF(0, 0, 100, 100), fixed_state, gfx::RectF(100, 200, 1700, 1300), scroll_state);
}
...
TEST_P(GeometryMapperTest, MightOverlapScroll)
{
    ...
    auto outer_scroll_state
        = CreateScrollTranslationState(PropertyTreeState(*viewport, c0(), e0()), -1234, -567, gfx::Rect(10, 20, 100, 200), gfx::Size(150, 300));
    auto inner_scroll_state = CreateScrollTranslationState(outer_scroll_state, -2345, -678, gfx::Rect(20, 10, 200, 100), gfx::Size(3000, 2000));

    auto* transform_outside = Create2DTranslation(*viewport, 100, 200);
    PropertyTreeState state_outside(*transform_outside, c0(), e0());

    auto* transform_under_outer_scroll = Create2DTranslation(outer_scroll_state.Transform(), 34, 56);
    PropertyTreeState state_under_outer_scroll(*transform_under_outer_scroll, outer_scroll_state.Clip(), outer_scroll_state.Effect());

    auto* transform_under_inner_scroll = Create2DTranslation(inner_scroll_state.Transform(), 45, 67);
    PropertyTreeState state_under_inner_scroll(*transform_under_inner_scroll, inner_scroll_state.Clip(), inner_scroll_state.Effect());
...

    {
        ...
        EXPECT_EQ(gfx::RectF(20, 20, 53, 64), MapVisualRectAboveScrollForCompositingOverlap(outer_scroll_state.Transform(), rect, outer_scroll_state));
        ...
        CheckOverlap(rect, outer_scroll_state, rect_mapped_in_state_outside, state_outside);
    }
    {
        ...
        EXPECT_EQ(gfx::RectF(54, 36, 53, 104), MapVisualRectAboveScrollForCompositingOverlap(outer_scroll_state.Transform(), rect, state_under_outer_scroll));
        CheckOverlap(rect, state_under_outer_scroll, gfx::RectF(-46, -164, 53, 104), state_outside);
    }
    {
        // `rect` is mapped through two scroll translations.
        SCOPED_TRACE("inner_scroll_state and state_outside");
        CheckOverlap(rect, inner_scroll_state, gfx::RectF(-90, -180, 63, 64), state_outside);
    }
    {
        // `rect` is mapped by local transform, then through two scroll
        // translations.
        SCOPED_TRACE("state_under_inner_scroll and state_outside");
        CheckOverlap(rect, state_under_inner_scroll, gfx::RectF(-90, -180, 100, 90), state_outside);
    }
    {
        SCOPED_TRACE("inner_scroll_state and outer_scroll_state");
        CheckOverlap(rect, inner_scroll_state, gfx::RectF(20, 20, 53, 64), outer_scroll_state);
    }
    {
        SCOPED_TRACE("state_under_inner_scroll and outer_scroll_state");
        CheckOverlap(rect, state_under_inner_scroll, gfx::RectF(20, 20, 98, 90), outer_scroll_state);
    }
    {
        SCOPED_TRACE("state_under_inner_scroll and state_under_outer_scroll");
        CheckOverlap(rect, state_under_inner_scroll, gfx::RectF(-14, -36, 98, 90), state_under_outer_scroll);
    }
}
