...
{
    return RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled() || IsTracingRasterInvalidations();
}

bool RasterInvalidationTracking::IsTracingRasterInvalidations()
{
    bool tracing_enabled;
    TRACE_EVENT_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("blink.invalidation"), &tracing_enabled);
    return tracing_enabled;
}

void RasterInvalidationTracking::AddInvalidation(DisplayItemClientId client_id, const String& debug_name, const gfx::Rect& rect, PaintInvalidationReason reason)
{
    if (rect.IsEmpty())
        return;
...
static bool CompareRasterInvalidationInfo(const RasterInvalidationInfo& a, const RasterInvalidationInfo& b)
{
    ...
    int name_compare_result = CodeUnitCompare(a.client_debug_name, b.client_debug_name);
...
void RasterInvalidationTracking::AsJSON(JSONObject* json, bool detailed) const
{
    if (!invalidations_.empty()) {
        ...
        for (auto it = sorted.begin(); it != sorted.end(); ++it) {
            ...
            if (detailed) {
                ...
                info_json->SetString("reason", PaintInvalidationReasonToString(info.reason));
                invalidations_json->PushObject(std::move(info_json));
            } else if (std::none_of(sorted.begin(), it, [&info](auto& previous) { return previous.rect.Contains(info.rect); })) {
    ...
    if (!under_invalidations_.empty()) {
        ...
        for (auto& under_invalidation : under_invalidations_) {
            ...
            under_invalidation_json->SetString("oldPixel", Color::FromSkColor(under_invalidation.old_pixel).NameForLayoutTreeAsText());
            under_invalidation_json->SetString("newPixel", Color::FromSkColor(under_invalidation.new_pixel).NameForLayoutTreeAsText());
...
void RasterInvalidationTracking::AddToLayerDebugInfo(cc::LayerDebugInfo& debug_info) const
{
    ...
    for (auto& info : invalidations_) {
        if (info.rect.IsEmpty())
            continue;
        debug_info.invalidations.push_back({ gfx::Rect(info.rect), PaintInvalidationReasonToString(info.reason), info.client_debug_name.Utf8() });
    }
}
...
{
    return PixelComponentsDiffer(SkColorGetA(p1), SkColorGetA(p2)) || PixelComponentsDiffer(SkColorGetR(p1), SkColorGetR(p2))
        || PixelComponentsDiffer(SkColorGetG(p1), SkColorGetG(p2)) || PixelComponentsDiffer(SkColorGetB(p1), SkColorGetB(p2));
}

void RasterInvalidationTracking::CheckUnderInvalidations(const String& layer_debug_name, PaintRecord new_record, const gfx::Rect& new_interest_rect)
{
    ...
    SkBitmap old_bitmap;
    if (!old_bitmap.tryAllocPixels(SkImageInfo::MakeN32Premul(rect.width(), rect.height())))
        return;
    {
    ...
    SkBitmap new_bitmap;
    if (!new_bitmap.tryAllocPixels(SkImageInfo::MakeN32Premul(rect.width(), rect.height())))
        return;
    {
    ...
    for (int bitmap_y = 0; bitmap_y < rect.height(); ++bitmap_y) {
        ...
        if (memcmp(old_bitmap.pixmap().addr(0, bitmap_y), new_row_addr, new_bitmap.rowBytes()) == 0) {
        ...
        for (int bitmap_x = 0; bitmap_x < rect.width(); ++bitmap_x) {
            ...
            if (PixelsDiffer(old_pixel, new_pixel) && !invalidation_region.Contains(gfx::Point(layer_x, layer_y))) {
                if (mismatching_pixels < kMaxMismatchesToReport) {
                    RasterUnderInvalidation under_invalidation = { layer_x, layer_y, old_pixel, new_pixel };
                    under_invalidations_.push_back(under_invalidation);
                    LOG(ERROR) << layer_debug_name << " Uninvalidated old/new pixels mismatch at " << layer_x << "," << layer_y << " old:" << std::hex
                               << old_pixel << " new:" << new_pixel;
                } else if (mismatching_pixels == kMaxMismatchesToReport) {
                    LOG(ERROR) << "and more...";
                }
                ++mismatching_pixels;
                *new_bitmap.getAddr32(bitmap_x, bitmap_y) = SkColorSetARGB(0xFF, 0xA0, 0, 0); // Dark red.
            } else {
    ...
    canvas->drawImage(cc::PaintImage::CreateFromBitmap(std::move(new_bitmap)), rect.x(), rect.y());
