...
class PLATFORM_EXPORT RasterInvalidator : public GarbageCollected<RasterInvalidator> {
public:
    ...
    explicit RasterInvalidator(Callback& callback)
        : callback_(callback)
    {
    }
    ...
    // paint artifact.
    void Generate(const PaintChunkSubset&, const gfx::Vector2dF& layer_offset, const gfx::Size& layer_bounds, const PropertyTreeState& layer_state);
    ...
    const gfx::Size& LayerBounds() const
    {
...
private:
    ...
    public:
        PaintChunkInfo(const RasterInvalidator& invalidator, const ChunkToLayerMapper& mapper, const PaintChunkIterator& chunk_it)
            ...
            ,
#endif
            bounds_in_layer(invalidator.ClipByLayerBounds(mapper.MapVisualRect(chunk_it->drawable_bounds)))
            , chunk_to_layer_clip(mapper.ClipRect())
            , chunk_to_layer_transform(mapper.Transform())
        {
        }

        PaintChunkInfo(const PaintChunkInfo& old_chunk_info, const PaintChunkIterator& chunk_it)
            : index_in_paint_artifact(chunk_it.IndexInPaintArtifact())
            ,
#if DCHECK_IS_ON()
            id(chunk_it->id)
    ...
    void GenerateRasterInvalidations(
        const PaintChunkSubset&, bool layer_offset_or_state_changed, bool layer_effect_changed, Vector<PaintChunkInfo>& new_chunks_info);

    ALWAYS_INLINE const PaintChunk& GetOldChunk(wtf_size_t index) const;
    ALWAYS_INLINE wtf_size_t MatchNewChunkToOldChunk(const PaintChunk& new_chunk, wtf_size_t old_index) const;

    ALWAYS_INLINE void IncrementallyInvalidateChunk(const PaintChunkInfo& old_chunk_info, const PaintChunkInfo& new_chunk_info, DisplayItemClientId);
    ...
    void AddRasterInvalidation(const gfx::Rect& rect, DisplayItemClientId client_id, PaintInvalidationReason reason, ClientIsOldOrNew old_or_new)
    {
        callback_.InvalidateRect(rect);
        if (tracking_)
            TrackRasterInvalidation(rect, client_id, reason, old_or_new);
    }
    void TrackRasterInvalidation(const gfx::Rect&, DisplayItemClientId, PaintInvalidationReason, ClientIsOldOrNew);

    ALWAYS_INLINE PaintInvalidationReason ChunkPropertiesChanged(const PaintChunk& new_chunk, const PaintChunk& old_chunk, const PaintChunkInfo& new_chunk_info,
        const PaintChunkInfo& old_chunk_info, const PropertyTreeState& layer_state, const float absolute_translation_tolerance,
        const float other_transform_tolerance) const;

    // Clip a rect in the layer space by the layer bounds.
    template <typename Rect> Rect ClipByLayerBounds(const Rect& r) const
    {
        return IntersectRects(r, Rect(gfx::Rect(layer_bounds_)));
    }

    void PopulatePaintChunksInfo(const PaintChunkSubset&, const gfx::Vector2dF& layer_offset, const PropertyTreeState& layer_state, Vector<PaintChunkInfo>&);
