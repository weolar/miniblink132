...
PLATFORM_EXPORT const char* PaintPropertyChangeTypeToString(PaintPropertyChangeType);

class PLATFORM_EXPORT PaintPropertyNode : public GarbageCollected<PaintPropertyNode> {
public:
    ...
    bool IsParentAlias() const
    {
        return is_parent_alias_;
    }
    ...
    {
        return cc_sequence_number_ == sequence_number ? cc_node_id_ : cc::kInvalidPropertyNodeId;
    }
    ...
    PaintPropertyChangeType NodeChanged() const
    {
        return changed_;
    }
    bool NodeChangeAffectsRaster() const
    {
        return changed_ != PaintPropertyChangeType::kUnchanged && changed_ != PaintPropertyChangeType::kChangedOnlyNonRerasterValues;
    }
...
protected:
    ...
    const PaintPropertyNode* Parent() const
    {
        return parent_.Get();
    }
    ...
    const PaintPropertyNode& LowestCommonAncestor(const PaintPropertyNode& other) const
    {
    ...
    int ChangedSequenceNumber() const
    {
        return changed_sequence_number_;
    }
...
private:
    ...
    const PaintPropertyNode& LowestCommonAncestorInternal(const PaintPropertyNode& other) const;
...
template <typename NodeTypeOrAlias, typename NodeType> class PaintPropertyNodeBase : public PaintPropertyNode {
public:
    ...
    const NodeTypeOrAlias& LowestCommonAncestor(const NodeTypeOrAlias& other) const
    {
        return static_cast<const NodeTypeOrAlias&>(PaintPropertyNode::LowestCommonAncestor(other));
    }
...
private:
    void BuildTreeString(StringBuilder& string_builder, const PaintPropertyNode& node, unsigned indent);
...
inline std::ostream& operator<<(std::ostream& os, const PaintPropertyNode& node)
{
    return os << node.ToString().Utf8();
}

inline std::ostream& operator<<(std::ostream& os, PaintPropertyChangeType change)
{
