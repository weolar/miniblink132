...
const TransformPaintPropertyNode* NearestCompositedScrollTranslation(
    const TransformPaintPropertyNode& scroll_translation, IsCompositedScrollFunction is_composited_scroll)
{
    for (auto* t = &scroll_translation; t->Parent(); t = &t->UnaliasedParent()->NearestScrollTranslationNode()) {
...
bool InSameTransformCompositingBoundary(
    const TransformPaintPropertyNode& t1, const TransformPaintPropertyNode& t2, IsCompositedScrollFunction is_composited_scroll)
{
    ...
    return NearestCompositedScrollTranslation(scroll_translation1, is_composited_scroll)
        == NearestCompositedScrollTranslation(scroll_translation2, is_composited_scroll);
}

bool ClipChainInTransformCompositingBoundary(const ClipPaintPropertyNode& node, const ClipPaintPropertyNode& ancestor,
    const TransformPaintPropertyNode& transform, IsCompositedScrollFunction is_composited_scroll)
{
    for (const auto* n = &node; n != &ancestor; n = n->UnaliasedParent()) {
        if (!InSameTransformCompositingBoundary(transform, n->LocalTransformSpace().Unalias(), is_composited_scroll)) {
            return false;
        }
    }
    return true;
...
std::optional<PropertyTreeState> PropertyTreeState::CanUpcastWith(const PropertyTreeState& guest, IsCompositedScrollFunction is_composited_scroll) const
{
    ...
    if (&Transform() == &guest.Transform()) {
        ...
        if (!InSameTransformCompositingBoundary(Transform(), guest.Transform(), is_composited_scroll)) {
            return std::nullopt;
        }
        if (Transform().IsBackfaceHidden() != guest.Transform().IsBackfaceHidden()) {
            return std::nullopt;
        }
        upcast_transform = &Transform().LowestCommonAncestor(guest.Transform()).Unalias();
    }
    ...
    if (&Clip() == &guest.Clip()) {
        ...
        if (!ClipChainInTransformCompositingBoundary(Clip(), *upcast_clip, *upcast_transform, is_composited_scroll)
            || !ClipChainInTransformCompositingBoundary(guest.Clip(), *upcast_clip, *upcast_transform, is_composited_scroll)) {
            return std::nullopt;
        }
    }
