...
size_t PaintControllerPersistentData::ApproximateUnsharedMemoryUsage() const
{
    ...
    // new_subsequences_.
    memory_usage += current_subsequences_.map.Capacity() * sizeof(decltype(current_subsequences_.map)::value_type);
    memory_usage += current_subsequences_.tree.CapacityInBytes();

    return memory_usage;
}

bool PaintControllerPersistentData::ClientCacheIsValid(const DisplayItemClient& client) const
{
...
wtf_size_t PaintControllerPersistentData::GetSubsequenceIndex(DisplayItemClientId client_id) const
{
...
const SubsequenceMarkers* PaintControllerPersistentData::GetSubsequenceMarkers(DisplayItemClientId client_id) const
{
...
void PaintControllerPersistentData::CommitNewDisplayItems(PaintArtifact& new_paint_artifact, SubsequencesData new_subsequences)
{
...
PaintController::CounterForTesting* PaintController::counter_for_testing_ = nullptr;

PaintController::PaintController(bool record_debug_info, PaintControllerPersistentData* persistent_data, PaintBenchmarkMode benchmark_mode)
    : persistent_data_(persistent_data)
    , paint_chunker_(new_paint_artifact_->GetPaintChunks(), persistent_data_ ? &clients_to_validate_ : nullptr)
    ...
    , benchmark_mode_(benchmark_mode)
{
    ...
    if (persistent_data) {
        ...
        new_subsequences_.map.ReserveCapacityForSize(CurrentSubsequences().map.size());
    }
    new_paint_artifact_->GetDisplayItemList().ReserveCapacity(display_item_list_capacity);
}
...
void PaintController::RecordHitTestData(const DisplayItemClient& client, const gfx::Rect& rect, TouchAction touch_action, bool blocking_wheel,
    cc::HitTestOpaqueness opaqueness, DisplayItem::Type type)
{
    if (rect.IsEmpty())
        return;
    ...
    if (paint_chunker_.AddHitTestDataToCurrentChunk(id, client, rect, touch_action, blocking_wheel, opaqueness)) {
        CheckNewChunk();
    }
}

void PaintController::RecordRegionCaptureData(const DisplayItemClient& client, const RegionCaptureCropId& crop_id, const gfx::Rect& rect)
{
    DCHECK(!crop_id->is_zero());
    PaintChunk::Id id(client.Id(), DisplayItem::kRegionCapture, current_fragment_);
    CheckNewChunkId(id);
    ValidateNewChunkClient(client);
    if (paint_chunker_.AddRegionCaptureDataToCurrentChunk(id, client, crop_id, rect))
        CheckNewChunk();
}

void PaintController::RecordScrollHitTestData(const DisplayItemClient& client, DisplayItem::Type type, const TransformPaintPropertyNode* scroll_translation,
    const gfx::Rect& scroll_hit_test_rect, cc::HitTestOpaqueness hit_test_opaqueness, const gfx::Rect& scrolling_contents_cull_rect)
{
    PaintChunk::Id id(client.Id(), type, current_fragment_);
    CheckNewChunkId(id);
    ValidateNewChunkClient(client);
    paint_chunker_.CreateScrollHitTestChunk(id, client, scroll_translation, scroll_hit_test_rect, hit_test_opaqueness, scrolling_contents_cull_rect);
    CheckNewChunk();
}

void PaintController::RecordSelection(std::optional<PaintedSelectionBound> start, std::optional<PaintedSelectionBound> end, String debug_info)
{
    DCHECK(start.has_value() || end.has_value());
    paint_chunker_.AddSelectionToCurrentChunk(start, end, debug_info);
}

bool PaintController::UseCachedItemIfPossible(const DisplayItemClient& client, DisplayItem::Type type)
{
    ...
    if (!ClientCacheIsValid(client)) {
        ...
        last_matching_client_invalidation_reason_ = client.GetPaintInvalidationReason();
        return false;
    }
    ...
    DisplayItem& new_item = new_paint_artifact_->GetDisplayItemList().AppendByMoving(CurrentDisplayItemList()[cached_item]);
...
#if DCHECK_IS_ON()
void PaintController::AssertLastCheckedCachedItem(const DisplayItemClient& client, DisplayItem::Type type)
{
    if (RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled() || benchmark_mode_ >= PaintBenchmarkMode::kForcePaint || !persistent_data_) {
        return;
    }

    DCHECK(last_checked_cached_item_id_ == DisplayItem::Id(client.Id(), type, current_fragment_).AsHashKey());
}
#endif

sk_sp<SkTextBlob> PaintController::CachedTextBlob() const
{
    if (!IsNonLayoutFullPaintInvalidationReason(last_matching_client_invalidation_reason_)) {
        return nullptr;
    }
    const auto* cached_item = DynamicTo<DrawingDisplayItem>(MatchingCachedItemToBeRepainted());
    if (!cached_item) {
        return nullptr;
    }
...
bool PaintController::UseCachedSubsequenceIfPossible(const DisplayItemClient& client)
{
    ...
    if (benchmark_mode_ == PaintBenchmarkMode::kCachingDisabled || benchmark_mode_ == PaintBenchmarkMode::kSubsequenceCachingDisabled) {
        return false;
    }
    ...
    wtf_size_t start_item_index = CurrentPaintChunks()[markers.start_chunk_index].begin_index;
    wtf_size_t end_item_index = CurrentPaintChunks()[markers.end_chunk_index - 1].end_index;
    if (end_item_index > start_item_index && CurrentDisplayItemList()[start_item_index].IsTombstone()) {
    ...
    AppendSubsequenceByMoving(client, subsequence_index, markers.start_chunk_index, markers.end_chunk_index);
    return true;
}
...
wtf_size_t PaintController::BeginSubsequence(const DisplayItemClient& client)
{
    ...
    new_subsequences_.tree.push_back(SubsequenceMarkers { client.Id(), NumNewChunks() });
...
void PaintController::EndSubsequence(wtf_size_t subsequence_index)
{
    ...
    if (IsCheckingUnderInvalidation()) {
        under_invalidation_checker_->WillEndSubsequence(markers.client_id, markers.start_chunk_index);
    }
    ...
    DCHECK(!new_subsequences_.map.Contains(markers.client_id))
        << "Multiple subsequences for client: " << new_paint_artifact_->ClientDebugName(markers.client_id);

    // Check tree integrity.
    if (subsequence_index > 0) {
        DCHECK_GE(markers.start_chunk_index, new_subsequences_.tree[subsequence_index - 1].end_chunk_index);
    }
...
void PaintController::CheckNewItem(DisplayItem& display_item)
{
    ...
    if (display_item.IsCacheable()) {
        ...
        auto index = FindItemFromIdIndexMap(id, new_display_item_id_index_map_, new_display_item_list);
        if (index != kNotFound) {
            const auto& chunks = new_paint_artifact_->GetPaintChunks();
            auto chunk
                = std::upper_bound(chunks.begin(), chunks.end(), index, [](wtf_size_t index, const PaintChunk& chunk) { return index < chunk.end_index; });
            DCHECK_NE(chunk, chunks.end());
            LOG(ERROR) << "DisplayItem " << display_item.AsDebugString(*new_paint_artifact_).Utf8() << " (index="
                       ...
                       << " last chunk " << chunks.back().ToString(*new_paint_artifact_, /*concise=*/true).Utf8() << ")\n has duplicated id with previous "
                       << new_display_item_list[index].AsDebugString(*new_paint_artifact_).Utf8() << " (index=" << index << " in chunk "
        ...
        AddToIdIndexMap(id, new_display_item_list.size() - 1, new_display_item_id_index_map_);
    }
#endif
...
void PaintController::ProcessNewItem(const DisplayItemClient& client, DisplayItem& display_item)
{
    if (IsSkippingCache() && persistent_data_) {
        client.Invalidate(PaintInvalidationReason::kUncacheable);
        display_item.SetPaintInvalidationReason(PaintInvalidationReason::kUncacheable);
    }
    ...
    if (!frame_first_paints_.back().first_painted && display_item.IsDrawing() &&
        ...
        // background.
        display_item.GetType() != DisplayItem::kDocumentBackground && display_item.DrawsContent()) {
        SetFirstPainted();
    }
...
void PaintController::CheckNewChunkId(const PaintChunk::Id& id)
{
    ...
    if (it != new_paint_chunk_id_index_map_.end()) {
        LOG(ERROR) << "New paint chunk id " << id.ToString(*new_paint_artifact_) << " is already used by a previous chuck "
                   << new_paint_artifact_->GetPaintChunks()[it->value].ToString(*new_paint_artifact_);
        LOG(ERROR) << DebugDataAsString().Utf8();
...
void PaintController::CheckNewChunk()
{
    ...
    if (persistent_data_) {
        ...
        if (chunks.back().is_cacheable) {
            AddToIdIndexMap(chunks.back().id, chunks.size() - 1, new_paint_chunk_id_index_map_);
        }
    }
#endif
...
void PaintController::RecordDebugInfo(const DisplayItemClient& client)
{
    if (record_debug_info_) {
        new_paint_artifact_->RecordDebugInfo(client.Id(), client.DebugName(), client.OwnerNodeId());
    }
}

void PaintController::UpdateCurrentPaintChunkProperties(const PaintChunk::Id& id, const DisplayItemClient& client, const PropertyTreeStateOrAlias& properties)
{
    PaintChunk::Id id_with_fragment(id, current_fragment_);
    CheckNewChunkId(id_with_fragment);
    ValidateNewChunkClient(client);
    paint_chunker_.UpdateCurrentPaintChunkProperties(id_with_fragment, client, properties);
}

void PaintController::UpdateCurrentPaintChunkProperties(const PropertyTreeStateOrAlias& properties)
{
...
wtf_size_t PaintController::FindItemFromIdIndexMap(const DisplayItem::Id& id, const IdIndexMap& display_item_id_index_map, const DisplayItemList& list)
{
...
void PaintController::AddToIdIndexMap(const DisplayItem::Id& id, wtf_size_t index, IdIndexMap& map)
{
    DCHECK(!map.Contains(id.AsHashKey()));
    map.insert(id.AsHashKey(), index);
}

wtf_size_t PaintController::FindCachedItem(const DisplayItemClient& client, const DisplayItem::Id& id)
{
    ...
    wtf_size_t found_index = FindItemFromIdIndexMap(id, out_of_order_item_id_index_map_, CurrentDisplayItemList());
...
wtf_size_t PaintController::FindOutOfOrderCachedItemForward(const DisplayItemClient& client, const DisplayItem::Id& id)
{
    ...
    if (RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled() && ClientCacheIsValid(client)) {
        // Ensure our paint invalidation tests don't trigger the less performant
        // situation which should be rare.
        RecordDebugInfo(client);
        DLOG(WARNING) << "Can't find cached display item: " << id.ToString(*new_paint_artifact_);
        ShowDebugData();
    }
#endif
    return kNotFound;
...
void PaintController::AppendSubsequenceByMoving(
    const DisplayItemClient& client, wtf_size_t subsequence_index, wtf_size_t start_chunk_index, wtf_size_t end_chunk_index)
{
#if DCHECK_IS_ON()
    DCHECK(!RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled());
    DCHECK_EQ(client.Id(), CurrentSubsequences().tree[subsequence_index].client_id);
    ...
    for (auto chunk_index = start_chunk_index; chunk_index < end_chunk_index; ++chunk_index) {
        ...
        if (record_debug_info_) {
            ...
            new_paint_artifact_->RecordDebugInfo(new_chunk.id.client_id, CurrentPaintArtifact().ClientDebugName(new_chunk.id.client_id),
    ...
    new_display_item_list.AppendSubsequenceByMoving(
        CurrentDisplayItemList(), current_chunks[start_chunk_index].begin_index, current_chunks[end_chunk_index - 1].end_index);

    bool skip_cache = IsSkippingCache();
    for (auto& item : new_display_item_list.ItemsInRange(new_item_start_index, new_display_item_list.size())) {
        DCHECK(!item.IsTombstone());
        // This item was copied from the cache, so client must already be valid,
        // hence we don't call MarkClientForValidation(client).
        item.SetPaintInvalidationReason(skip_cache || !item.IsCacheable() ? PaintInvalidationReason::kUncacheable : PaintInvalidationReason::kNone);
        if (record_debug_info_) {
            new_paint_artifact_->RecordDebugInfo(
                item.ClientId(), CurrentPaintArtifact().ClientDebugName(item.ClientId()), CurrentPaintArtifact().ClientOwnerNodeId(item.ClientId()));
        }
#if DCHECK_IS_ON()
        CheckNewItem(item);
#endif
    ...
    for (wtf_size_t i = subsequence_index + 1; i < CurrentSubsequences().tree.size(); i++) {
        auto& markers = CurrentSubsequences().tree[i];
        if (markers.start_chunk_index >= end_chunk_index)
            break;
        DCHECK(!new_subsequences_.map.Contains(markers.client_id))
            << "Multiple subsequences for client: " << CurrentPaintArtifact().ClientDebugName(markers.client_id);
        new_subsequences_.map.insert(markers.client_id, new_subsequences_.tree.size());
        new_subsequences_.tree.push_back(SubsequenceMarkers { markers.client_id, markers.start_chunk_index + new_start_chunk_index - start_chunk_index,
    ...
    new_subsequences_.tree[new_subsequence_index].is_moved_from_cached_subsequence = true;
...
{
    TRACE_EVENT2("blink,benchmark", "PaintController::commitNewDisplayItems", "current_display_list_size",
        persistent_data_ ? CurrentDisplayItemList().size() : 0, "num_non_cached_new_items",
    ...
    if (counter_for_testing_) {
        ...
        counter_for_testing_->num_cached_subsequences += num_cached_new_subsequences_;
    }
    ...
    if (persistent_data_) {
        ...
        persistent_data_->CommitNewDisplayItems(paint_artifact, std::move(new_subsequences_));
    }
...
PaintUnderInvalidationChecker& PaintController::EnsureUnderInvalidationChecker()
{
    DCHECK(RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled());
