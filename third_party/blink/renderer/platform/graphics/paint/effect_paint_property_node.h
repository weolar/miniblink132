...
class PLATFORM_EXPORT EffectPaintPropertyNodeOrAlias : public PaintPropertyNodeBase<EffectPaintPropertyNodeOrAlias, EffectPaintPropertyNode> {
public:
...
class EffectPaintPropertyNodeAlias final : public EffectPaintPropertyNodeOrAlias {
public:
    static EffectPaintPropertyNodeAlias* Create(const EffectPaintPropertyNodeOrAlias& parent)
    {
        return MakeGarbageCollected<EffectPaintPropertyNodeAlias>(kParentAlias, parent);
    }

    // These are public required by MakeGarbageCollected, but the protected tags
    // prevent these from being called from outside.
    explicit EffectPaintPropertyNodeAlias(ParentAliasTag, const EffectPaintPropertyNodeOrAlias& parent)
        : EffectPaintPropertyNodeOrAlias(kParentAlias, parent)
    {
    }
};

class PLATFORM_EXPORT EffectPaintPropertyNode final : public EffectPaintPropertyNodeOrAlias {
public:
    ...
    public:
        ...
        PaintPropertyChangeType ComputeChange(const State& other, const AnimationState& animation_state) const;

        PaintPropertyChangeType ComputeOpacityChange(float opacity, const AnimationState& animation_state) const;
    ...
    static EffectPaintPropertyNode* Create(const EffectPaintPropertyNodeOrAlias& parent, State&& state)
    {
        return MakeGarbageCollected<EffectPaintPropertyNode>(kNonParentAlias, parent, std::move(state));
    }
    ...
    PaintPropertyChangeType Update(const EffectPaintPropertyNodeOrAlias& parent, State&& state, const AnimationState& animation_state = AnimationState())
    {
    ...
    PaintPropertyChangeType DirectlyUpdateOpacity(float opacity, const AnimationState& animation_state);
    ...
    {
        return Opacity() != 1.0f || BlendMode() != SkBlendMode::kSrcOver || !Filter().IsEmpty() || BackdropFilter();
    }

    bool IsOpacityOnly() const
    {
        return BlendMode() == SkBlendMode::kSrcOver && Filter().IsEmpty() && !BackdropFilter();
    }
    ...
    {
        return state_.direct_compositing_reasons & CompositingReason::kBackdropFilterMask;
    }

    bool FlattensAtLeafOf3DScene() const
    {
        return state_.direct_compositing_reasons & CompositingReason::kTransform3DSceneLeaf;
    }

    bool HasActiveOpacityAnimation() const
    {
        return state_.direct_compositing_reasons & CompositingReason::kActiveOpacityAnimation;
    }
    bool HasActiveFilterAnimation() const
    {
        return state_.direct_compositing_reasons & CompositingReason::kActiveFilterAnimation;
    }
    bool HasActiveBackdropFilterAnimation() const
    {
        return state_.direct_compositing_reasons & CompositingReason::kActiveBackdropFilterAnimation;
    }

    bool RequiresCompositingForWillChangeOpacity() const
    {
        return state_.direct_compositing_reasons & CompositingReason::kWillChangeOpacity;
    }
    bool RequiresCompositingForWillChangeFilter() const
    {
        return state_.direct_compositing_reasons & CompositingReason::kWillChangeFilter;
    }
    bool RequiresCompositingForWillChangeBackdropFilter() const
    {
        return state_.direct_compositing_reasons & CompositingReason::kWillChangeBackdropFilter;
    }
    ...
    {
        return Opacity() != 1.0f || HasActiveOpacityAnimation() || RequiresCompositingForWillChangeOpacity();
    }
    ...
    {
        return !Filter().IsEmpty() || HasActiveFilterAnimation() || RequiresCompositingForWillChangeFilter();
    }
    ...
    {
        return BackdropFilter() || HasActiveBackdropFilterAnimation() || RequiresCompositingForWillChangeBackdropFilter();
    }
    ...
    {
        return MayHaveFilter() || MayHaveBackdropEffect() || ViewTransitionElementResourceId().IsValid() || !ElementCaptureId()->is_zero();
    }
    ...
    const viz::ViewTransitionElementResourceId& ViewTransitionElementResourceId() const
    {
    ...
    EffectPaintPropertyNode(NonParentAliasTag, const EffectPaintPropertyNodeOrAlias& parent, State&& state)
