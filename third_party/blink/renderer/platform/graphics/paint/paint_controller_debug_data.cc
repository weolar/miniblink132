...
public:
    PaintArtifactAsJSON(const PaintArtifact& artifact, const Vector<SubsequenceMarkers>& subsequences, DisplayItemList::JsonOption option)
        : artifact_(artifact)
    ...
    String ToString()
    {
        return ChunksAsJSONArrayRecursive(0, artifact_.GetPaintChunks().size())->ToPrettyJSONString();
    }
...
std::unique_ptr<JSONObject> PaintController::PaintArtifactAsJSON::SubsequenceAsJSONObjectRecursive()
{
    ...
    json_object->SetString(
        "subsequence", String::Format("client: %p ", reinterpret_cast<void*>(subsequence.client_id)) + artifact_.ClientDebugName(subsequence.client_id));
    json_object->SetArray("chunks", ChunksAsJSONArrayRecursive(subsequence.start_chunk_index, subsequence.end_chunk_index));

    return json_object;
}

std::unique_ptr<JSONArray> PaintController::PaintArtifactAsJSON::ChunksAsJSONArrayRecursive(wtf_size_t start_chunk_index, wtf_size_t end_chunk_index)
{
    auto array = std::make_unique<JSONArray>();
    auto chunk_index = start_chunk_index;

    while (next_subsequence_ != subsequences_.end() && next_subsequence_->start_chunk_index < end_chunk_index) {
        const auto& subsequence = *next_subsequence_;
        ...
        if (chunk_index < subsequence.start_chunk_index) {
            artifact_.AppendChunksAsJSON(chunk_index, subsequence.start_chunk_index, *array, option_);
        }
...
String PaintController::DebugDataAsString(DisplayItemList::JsonOption option) const
{
    StringBuilder sb;
    sb.Append("current paint artifact: ");
    if (persistent_data_) {
        sb.Append(PaintArtifactAsJSON(CurrentPaintArtifact(), CurrentSubsequences().tree, option).ToString());
    } else {
        sb.Append("null");
    }
    sb.Append("\nnew paint artifact: ");
    if (new_paint_artifact_) {
        sb.Append(PaintArtifactAsJSON(*new_paint_artifact_, new_subsequences_.tree, option).ToString());
    } else {
        sb.Append("null");
    }
    return sb.ToString();
...
void PaintController::ShowDebugDataInternal(DisplayItemList::JsonOption option) const
{
