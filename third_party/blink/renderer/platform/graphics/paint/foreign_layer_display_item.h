...
public:
    ForeignLayerDisplayItem(
        DisplayItemClientId client_id, Type, scoped_refptr<cc::Layer>, const gfx::Point& origin, RasterEffectOutset, PaintInvalidationReason);
...
template <> struct DowncastTraits<ForeignLayerDisplayItem> {
...
PLATFORM_EXPORT void RecordForeignLayer(GraphicsContext& context, const DisplayItemClient& client, DisplayItem::Type type, scoped_refptr<cc::Layer> layer,
    const gfx::Point& origin, const PropertyTreeStateOrAlias* properties = nullptr);
