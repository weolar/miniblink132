...
{
    size_t total_size = sizeof(*this) + display_item_list_.MemoryUsageInBytes() - sizeof(display_item_list_) + chunks_.CapacityInBytes();
...
PaintRecord PaintArtifact::GetPaintRecord(const PropertyTreeState& replay_state, const gfx::Rect* cull_rect) const
{
    return PaintChunksToCcLayer::Convert(PaintChunkSubset(*this), replay_state, cull_rect);
}

void PaintArtifact::RecordDebugInfo(DisplayItemClientId client_id, const String& name, DOMNodeId owner_node_id)
{
...
DOMNodeId PaintArtifact::ClientOwnerNodeId(DisplayItemClientId client_id) const
{
    auto iterator = debug_info_.find(client_id);
    if (iterator == debug_info_.end())
...
String PaintArtifact::IdAsString(const DisplayItem::Id& id) const
{
    ...
    if (!debug_name.empty()) {
        return String::Format("%p:%s:%s:%d", reinterpret_cast<void*>(id.client_id), ClientDebugName(id.client_id).Utf8().c_str(),
...
void PaintArtifact::AppendChunksAsJSON(
    wtf_size_t start_chunk_index, wtf_size_t end_chunk_index, JSONArray& json_array, DisplayItemList::JsonOption option) const
{
    ...
    for (auto i = start_chunk_index; i < end_chunk_index; ++i) {
        ...
        json_object->SetString("chunk", ClientDebugName(chunk.id.client_id) + " " + chunk.id.ToString(*this));
        ...
        json_object->SetArray("displayItems", DisplayItemList::DisplayItemsAsJSON(*this, chunk.begin_index, DisplayItemsInChunk(i), option));
#endif
