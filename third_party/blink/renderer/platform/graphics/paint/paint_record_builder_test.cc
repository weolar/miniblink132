...
TEST_F(PaintRecordBuilderTest, TransientPaintController)
{
    ...
    EXPECT_THAT(GetNewPaintArtifact(context.GetPaintController()).GetDisplayItemList(),
        ElementsAre(IsSameId(client.Id(), kBackgroundType), IsSameId(client.Id(), kForegroundType)));
...
TEST_F(PaintRecordBuilderTest, TransientAndAnotherPaintController)
{
    ...
    EXPECT_THAT(GetPersistentData().GetDisplayItemList(), ElementsAre(IsSameId(client.Id(), kBackgroundType), IsSameId(client.Id(), kForegroundType)));
...

    {
        ...
        DrawRect(builder.Context(), client, kBackgroundType, gfx::Rect(10, 10, 20, 20));
    ...
    EXPECT_FALSE(ClientCacheIsValid(builder.Context().GetPaintController(), client));
}
