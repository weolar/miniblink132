...
private:
    ...
    template <typename BaseIterator, typename ItemType> class IteratorWrapper {
        DISALLOW_NEW();
    ...
    public:
        ...
        {
            return reinterpret_cast<ItemType&>(*it_);
        }
        ItemType* operator->() const
        {
            return &operator*();
...
public:
    ...
    using const_iterator = IteratorWrapper<ItemVector::const_iterator, const DisplayItem>;
    iterator begin()
    {
    ...
    DisplayItem& operator[](wtf_size_t index)
    {
        return *(begin() + index);
    }
    ...
    Range<const_iterator> ItemsInRange(wtf_size_t begin_index, wtf_size_t end_index) const
    {
        return Range<const_iterator>(begin() + begin_index, begin() + end_index);
    }

    template <class DerivedItemType, typename... Args> DerivedItemType& AllocateAndConstruct(Args&&... args)
    {
        static_assert(WTF::IsSubclass<DerivedItemType, DisplayItem>::value, "Must use subclass of DisplayItem.");
        static_assert(sizeof(DerivedItemType) <= kMaxItemSize, "DisplayItem subclass is larger than kMaxItemSize.");
        static_assert(kAlignment % alignof(DerivedItemType) == 0, "Derived type requires stronger alignment.");
    ...
    void AppendSubsequenceByMoving(DisplayItemList& from, wtf_size_t begin_index, wtf_size_t end_index)
    {
        ...
        size_t bytes_to_move = reinterpret_cast<uint8_t*>(new_start_slot + count) - reinterpret_cast<uint8_t*>(new_start_slot);
        memcpy(static_cast<void*>(new_start_slot), static_cast<void*>(&from[begin_index]), bytes_to_move);
    ...
    static std::unique_ptr<JSONArray> DisplayItemsAsJSON(
        const PaintArtifact&, wtf_size_t first_item_index, const Range<const_iterator>& display_items, JsonOption);
#else // DCHECK_IS_ON()
...
private:
    ...
    {
        memcpy(static_cast<void*>(new_item_slot), static_cast<void*>(&item), kMaxItemSize);
