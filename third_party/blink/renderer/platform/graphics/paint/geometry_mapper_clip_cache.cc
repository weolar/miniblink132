...
void GeometryMapperClipCache::Update(const ClipPaintPropertyNode& node)
{
    ...
    if (node.PixelMovingFilter()) {
        ...
        nearest_pixel_moving_filter_clip_ = parent->GetClipCache().nearest_pixel_moving_filter_clip_;
    } else {
        nearest_pixel_moving_filter_clip_ = nullptr;
    }
}

const GeometryMapperClipCache::ClipCacheEntry* GeometryMapperClipCache::GetCachedClip(const ClipAndTransform& clip_and_transform)
{
    DCHECK(IsValid());
