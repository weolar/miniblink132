...
DrawingRecorder::DrawingRecorder(
    GraphicsContext& context, const DisplayItemClient& display_item_client, DisplayItem::Type display_item_type, const gfx::Rect& visual_rect)
    : context_(context)
...
DrawingRecorder::~DrawingRecorder()
{
    ...
    context_.GetPaintController().CreateAndAppend<DrawingDisplayItem>(
        client_, type_, visual_rect_, context_.EndRecording(), client_.VisualRectOutsetForRasterEffects(), client_.GetPaintInvalidationReason());
}
