...
public:
    DisplayItemClient()
        : paint_invalidation_reason_(static_cast<uint8_t>(PaintInvalidationReason::kJustCreated))
    ...
    virtual DOMNodeId OwnerNodeId() const
    {
    ...
    void Invalidate(PaintInvalidationReason reason = PaintInvalidationReason::kLayout) const
    {
    ...
    {
        return GetPaintInvalidationReason() == PaintInvalidationReason::kJustCreated;
    }
    ...
    {
        return GetPaintInvalidationReason() != PaintInvalidationReason::kUncacheable;
    }
...
private:
    ...
    void MarkForValidation() const
    {
        marked_for_validation_ = 1;
    }
    bool IsMarkedForValidation() const
    {
        return marked_for_validation_;
    }
    void Validate() const
    {
        paint_invalidation_reason_ = static_cast<uint8_t>(PaintInvalidationReason::kNone);
        marked_for_validation_ = 0;
    }
...
class StaticDisplayItemClient : public GarbageCollected<StaticDisplayItemClient>, public DisplayItemClient {
public:
    explicit StaticDisplayItemClient(const char* name)
        : name_(name)
    {
    }
...
#define DEFINE_STATIC_DISPLAY_ITEM_CLIENT(name, debug_name)                                                                                                    \
    DEFINE_STATIC_LOCAL(Persistent<StaticDisplayItemClient>, name, (MakeGarbageCollected<StaticDisplayItemClient>(debug_name)))

PLATFORM_EXPORT std::ostream& operator<<(std::ostream&, const DisplayItemClient*);
PLATFORM_EXPORT std::ostream& operator<<(std::ostream&, const DisplayItemClient&);
