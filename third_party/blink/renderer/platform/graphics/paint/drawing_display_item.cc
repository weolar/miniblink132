...
{
    SkBitmap bitmap;
    if (!bitmap.tryAllocPixels(SkImageInfo::MakeN32Premul(bounds.width(), bounds.height())))
        return bitmap;
...
bool BitmapsEqual(const PaintRecord& record1, const PaintRecord& record2, const gfx::Rect& bounds)
{
...
bool PaintFlagsMayChangeColorOrMovePixelsExceptShader(const cc::PaintFlags& flags)
{
    return flags.getStyle() != cc::PaintFlags::kFill_Style || flags.getLooper() || flags.getColorFilter() || flags.getImageFilter()
        || (flags.getBlendMode() != SkBlendMode::kSrc && flags.getBlendMode() != SkBlendMode::kSrcOver);
}
...
bool IsDrawAreaAnalysisCandidate(const cc::PaintOp& op)
{
    ...
    return !PaintFlagsMayChangeColorOrMovePixelsExceptShader(flags) && !flags.getShader();
}

} // anonymous namespace

bool DrawingDisplayItem::EqualsForUnderInvalidationImpl(const DrawingDisplayItem& other) const
{
    ...
    if (record.buffer().next_op_offset() == other_record.buffer().next_op_offset()
        && memcmp(&record.GetFirstOp(), &other_record.GetFirstOp(), record.buffer().next_op_offset()) == 0) {
        return true;
    }
#endif
...
DrawingDisplayItem::BackgroundColorInfo DrawingDisplayItem::BackgroundColor() const
{
    DCHECK(!IsTombstone());
    ...
    for (const cc::PaintOp& op : record_) {
        if (!IsDrawAreaAnalysisCandidate(op)) {
            if (GetType() != DisplayItem::kBoxDecorationBackground && GetType() != DisplayItem::kDocumentBackground
                && GetType() != DisplayItem::kDocumentRootBackdrop && GetType() != DisplayItem::kScrollCorner) {
        ...
        return { static_cast<const cc::PaintOpWithFlags&>(op).flags.getColor4f(), item_rect.width() * item_rect.height(),
            may_be_solid_color && item_rect.contains(gfx::RectToSkIRect(VisualRect())) };
    }
    return {};
}
...
gfx::Rect DrawingDisplayItem::CalculateRectKnownToBeOpaqueForRecord(const PaintRecord& record) const
{
    if (record.empty()) {
        return gfx::Rect();
    }
    ...
    for (const cc::PaintOp& op : record) {
        ...
        if (op.GetType() == cc::PaintOpType::kClipRect) {
            clip_rect.Intersect(gfx::ToEnclosedRect(gfx::SkRectToRectF(static_cast<const cc::ClipRectOp&>(op).rect)));
            continue;
        }
        ...
        if (op.GetType() == cc::PaintOpType::kDrawRecord) {
            op_opaque_rect = CalculateRectKnownToBeOpaqueForRecord(static_cast<const cc::DrawRecordOp&>(op).record);
        } else {
            if (!op.IsPaintOpWithFlags())
                continue;

            const auto& flags = static_cast<const cc::PaintOpWithFlags&>(op).flags;
            if (PaintFlagsMayChangeColorOrMovePixelsExceptShader(flags) || !flags.getColor4f().isOpaque()
            ...
            switch (op.GetType()) {
            case cc::PaintOpType::kDrawRect:
                op_opaque_rect = gfx::ToEnclosedRect(gfx::SkRectToRectF(static_cast<const cc::DrawRectOp&>(op).rect));
                break;
            case cc::PaintOpType::kDrawRRect: {
            ...
            case cc::PaintOpType::kDrawImage: {
                ...
                if (!image.IsOpaque())
                    continue;
                op_opaque_rect = gfx::Rect(draw_image_op.left, draw_image_op.top, image.width(), image.height());
                break;
            }
            case cc::PaintOpType::kDrawImageRect: {
                const auto& draw_image_rect_op = static_cast<const cc::DrawImageRectOp&>(op);
                const auto& image = draw_image_rect_op.image;
                DCHECK(gfx::RectF(image.width(), image.height()).Contains(gfx::SkRectToRectF(draw_image_rect_op.src)));
                if (!image.IsOpaque())
                    continue;
                op_opaque_rect = gfx::ToEnclosedRect(gfx::SkRectToRectF(draw_image_rect_op.dst));
                break;
            }
            default:
                continue;
...
gfx::Rect DrawingDisplayItem::TightenVisualRect(const gfx::Rect& visual_rect, const PaintRecord& record)
{
    ...
    switch (op.GetType()) {
    case cc::PaintOpType::kDrawRect:
        item_rect = gfx::ToEnclosingRect(gfx::SkRectToRectF(static_cast<const cc::DrawRectOp&>(op).rect));
        break;
    case cc::PaintOpType::kDrawIRect:
        item_rect = gfx::SkIRectToRect(static_cast<const cc::DrawIRectOp&>(op).rect);
        break;
    case cc::PaintOpType::kDrawRRect:
        item_rect = gfx::ToEnclosingRect(gfx::SkRectToRectF(static_cast<const cc::DrawRRectOp&>(op).rrect.rect()));
        break;
