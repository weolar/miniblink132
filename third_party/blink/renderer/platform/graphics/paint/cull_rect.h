...
public:
    CullRect() = default;
    explicit CullRect(const gfx::Rect& rect)
        : rect_(rect)
    {
    }
    ...
    {
        return CullRect(InfiniteIntRect());
    }

    bool IsInfinite() const
    {
        return rect_ == InfiniteIntRect();
    }
    ...
    bool ApplyPaintProperties(const PropertyTreeState& root, const PropertyTreeState& source, const PropertyTreeState& destination,
    ...
    bool HasScrolledEnough(const gfx::Vector2dF& delta, const TransformPaintPropertyNode&, float expansion_ratio);

    String ToString() const
    {
        return String(rect_.ToString());
    }
...
private:
    ...
    bool ApplyPaintPropertiesWithoutExpansion(const PropertyTreeState& source, const PropertyTreeState& destination);
