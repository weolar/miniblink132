...
static PaintRecord CreateRectRecord(const gfx::RectF& record_bounds, SkColor4f color = SkColors::kBlack)
{
...
static PaintRecord CreateRectRecordWithTranslate(const gfx::RectF& record_bounds, float dx, float dy, SkColor4f color = SkColors::kBlack)
{
    PaintRecorder recorder;
    cc::PaintCanvas* canvas = recorder.beginRecording();
    canvas->save();
...
TEST_F(DrawingDisplayItemTest, DrawsContent)
{
    ...
    DrawingDisplayItem item(client_->Id(), DisplayItem::Type::kDocumentBackground, ToEnclosingRect(record_bounds), CreateRectRecord(record_bounds),
...
{
    DrawingDisplayItem item(client_->Id(), DisplayItem::Type::kDocumentBackground, gfx::Rect(), PaintRecord(), RasterEffectOutset::kNone);
...
TEST_F(DrawingDisplayItemTest, EqualsForUnderInvalidation)
{
    ...
    DrawingDisplayItem item1(
        client_->Id(), DisplayItem::kDocumentBackground, ToEnclosingRect(bounds1), CreateRectRecord(bounds1), client_->VisualRectOutsetForRasterEffects());
    DrawingDisplayItem translated(client_->Id(), DisplayItem::kDocumentBackground, ToEnclosingRect(bounds1), CreateRectRecordWithTranslate(bounds1, 10, 20),
        client_->VisualRectOutsetForRasterEffects());
    // This item contains a DrawingRecord that is different from but visually
    // equivalent to item1's.
    DrawingDisplayItem zero_translated(client_->Id(), DisplayItem::kDocumentBackground, ToEnclosingRect(bounds1), CreateRectRecordWithTranslate(bounds1, 0, 0),
        client_->VisualRectOutsetForRasterEffects());

    gfx::RectF bounds2(100.5, 100.6, 100.7, 100.8);
    DrawingDisplayItem item2(
        client_->Id(), DisplayItem::kDocumentBackground, ToEnclosingRect(bounds1), CreateRectRecord(bounds2), client_->VisualRectOutsetForRasterEffects());

    DrawingDisplayItem empty_item(client_->Id(), DisplayItem::kDocumentBackground, gfx::Rect(), PaintRecord(), client_->VisualRectOutsetForRasterEffects());
...
TEST_F(DrawingDisplayItemTest, SolidColorRect)
{
    ...
    DrawingDisplayItem item(client_->Id(), DisplayItem::Type::kDocumentBackground, ToEnclosingRect(record_bounds),
        CreateRectRecord(record_bounds, SkColors::kGreen), client_->VisualRectOutsetForRasterEffects());
...
TEST_F(DrawingDisplayItemTest, NonSolidColorOval)
{
    ...
    DrawingDisplayItem item(client_->Id(), DisplayItem::Type::kDocumentBackground, ToEnclosingRect(record_bounds), recorder.finishRecordingAsPicture(),
        client_->VisualRectOutsetForRasterEffects());
    EXPECT_EQ(gfx::Rect(5, 6, 10, 10), item.VisualRect());
    // Not solid color if the drawing does not fully cover the visual rect.
    auto background = item.BackgroundColor();
...
static void CheckOpaqueRectPixels(const DrawingDisplayItem& item, SkBitmap& bitmap)
{
    ...
    for (int y = opaque_rect.y(); y < opaque_rect.bottom(); ++y) {
        for (int x = opaque_rect.x(); x < opaque_rect.right(); ++x) {
            ...
            EXPECT_EQ(SK_ColorWHITE, pixel) << " x=" << x << " y=" << y << " non-white pixel=" << std::hex << pixel;
        }
    }
}
...
TEST_F(DrawingDisplayItemTest, OpaqueRectForDrawRRectUniform)
{
    ...
    for (float r = kRadiusStep; r < kSize / 2; r += kRadiusStep) {
        PaintRecorder recorder;
        recorder.beginRecording()->drawRRect(SkRRect::MakeRectXY(SkRect::MakeWH(kSize, kSize), r, r), flags);
        DrawingDisplayItem item(client_->Id(), DisplayItem::Type::kDocumentBackground, gfx::Rect(0, 0, kSize, kSize), recorder.finishRecordingAsPicture(),
...
{
    auto image = cc::PaintImageBuilder::WithDefault().set_paint_record(PaintRecord(), gfx::Rect(), 0).set_id(1).TakePaintImage();
    PaintRecorder recorder;
    recorder.beginRecording()->drawImageRect(image, SkRect::MakeEmpty(), SkRect::MakeEmpty(), SkCanvas::kFast_SrcRectConstraint);
    DrawingDisplayItem item(
        client_->Id(), DisplayItem::kBoxDecorationBackground, gfx::Rect(10, 20), recorder.finishRecordingAsPicture(), RasterEffectOutset::kNone);
