...
public:
    ...
    explicit PaintChunker(PaintChunks& chunks, HeapVector<Member<const DisplayItemClient>>* clients_to_validate = nullptr)
        : chunks_(chunks)
    ...
    bool WillForceNewChunkForTesting() const
    {
    ...
    bool AddHitTestDataToCurrentChunk(
        const PaintChunk::Id&, const DisplayItemClient&, const gfx::Rect&, TouchAction, bool blocking_wheel, cc::HitTestOpaqueness);

    bool CurrentChunkIsNonEmptyAndTransparentToHitTest() const;

    void CreateScrollHitTestChunk(const PaintChunk::Id&, const DisplayItemClient&, const TransformPaintPropertyNode* scroll_translation,
        const gfx::Rect& scroll_hit_test_rect, cc::HitTestOpaqueness, const gfx::Rect& scrolling_contents_cull_rect);

    // The id will be used when we need to create a new current chunk.
    // Otherwise it's ignored. Returns true if a new chunk is added.
    bool AddRegionCaptureDataToCurrentChunk(
        const PaintChunk::Id& id, const DisplayItemClient& client, const RegionCaptureCropId& crop_id, const gfx::Rect& bounds);

    // The id will be used when we need to create a new current chunk.
    // Otherwise it's ignored. Returns true if a new chunk is added.
    void AddSelectionToCurrentChunk(std::optional<PaintedSelectionBound> start, std::optional<PaintedSelectionBound> end, String debug_info);
...
private:
    ...
    PropertyTreeStateOrAlias current_properties_ { PropertyTreeStateOrAlias::kUninitialized };
