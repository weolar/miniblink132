...
void GeometryMapperTransformCache::Update(const TransformPaintPropertyNode& node)
{
    ...
    const GeometryMapperTransformCache& parent = node.UnaliasedParent()->GetTransformCache();

    has_sticky_or_anchor_position_
        = node.RequiresCompositingForStickyPosition() || node.RequiresCompositingForAnchorPosition() || parent.has_sticky_or_anchor_position_;

    is_backface_hidden_ = node.IsBackfaceHiddenInternal(parent.is_backface_hidden_);

    nearest_scroll_translation_ = node.ScrollNode() ? &node : parent.nearest_scroll_translation_.Get();
    ...
    nearest_directly_composited_ancestor_ = node.HasDirectCompositingReasons() ? &node : parent.nearest_directly_composited_ancestor_.Get();
    ...
    if (node.IsIdentityOr2dTranslation() && !node.HasActiveTransformAnimation()) {
        ...
        if (parent.plane_root_transform_) {
            ...
            plane_root_transform_->to_plane_root.Translate(translation.x(), translation.y());
            plane_root_transform_->from_plane_root = parent.from_plane_root();
            plane_root_transform_->from_plane_root.PostTranslate(-translation.x(), -translation.y());
        ...
        if (is_plane_root) {
            ...
            plane_root_transform_->has_animation = parent.has_animation_to_plane_root() || node.HasActiveTransformAnimation();
        }
    }
...
void GeometryMapperTransformCache::UpdateScreenTransform(const TransformPaintPropertyNode& node)
{
    ...
    screen_transform_->projection_from_screen_is_valid = to_screen_flattened.GetInverse(&screen_transform_->projection_from_screen);
