...
{
    paint_controller_.UpdateCurrentPaintChunkProperties(PropertyTreeState::Root());
}
...
PaintRecord PaintRecordBuilder::EndRecording(const PropertyTreeState& replay_state)
{
    return paint_controller_.CommitNewDisplayItems().GetPaintRecord(replay_state);
}

void PaintRecordBuilder::EndRecording(cc::PaintCanvas& canvas, const PropertyTreeState& replay_state)
{
