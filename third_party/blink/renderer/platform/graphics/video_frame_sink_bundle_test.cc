...
MATCHER(IsFrame, "")
{
    return arg->data->which() == viz::mojom::blink::BundledFrameSubmissionData::Tag::kFrame;
}
MATCHER(IsDidNotProduceFrame, "")
{
    return arg->data->which() == viz::mojom::blink::BundledFrameSubmissionData::Tag::kDidNotProduceFrame;
}
...
{
    return viz::mojom::blink::BeginFrameInfo::New(sink_id,
        viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, 1, 1, base::TimeTicks(), base::TimeTicks(), base::TimeDelta(), viz::BeginFrameArgs::NORMAL),
        WTF::HashMap<uint32_t, viz::FrameTimingDetails>(),
        /*frame_ack=*/false, WTF::Vector<viz::ReturnedResource>());
}

class MockFrameSinkBundleClient : public viz::mojom::blink::FrameSinkBundleClient {
public:
    ...
    MOCK_METHOD3(FlushNotifications,
        void(WTF::Vector<viz::mojom::blink::BundledReturnedResourcesPtr> acks, WTF::Vector<viz::mojom::blink::BeginFrameInfoPtr> begin_frames,
            WTF::Vector<viz::mojom::blink::BundledReturnedResourcesPtr> reclaimed_resources));
    MOCK_METHOD2(OnBeginFramePausedChanged, void(uint32_t sink_id, bool paused));
    MOCK_METHOD2(OnCompositorFrameTransitionDirectiveProcessed, void(uint32_t sink_id, uint32_t sequence_id));
};

const viz::LocalSurfaceId kTestSurfaceId(1, base::UnguessableToken::CreateForTesting(1, 2));

class VideoFrameSinkBundleTest : public testing::Test {
public:
    VideoFrameSinkBundleTest()
    {
        VideoFrameSinkBundle::SetFrameSinkProviderForTesting(&frame_sink_provider());
    }
...
TEST_F(VideoFrameSinkBundleTest, GetOrCreateSharedInstance)
{
    ...
    EXPECT_CALL(frame_sink_provider(), CreateFrameSinkBundle_).Times(1);
    VideoFrameSinkBundle& bundle = VideoFrameSinkBundle::GetOrCreateSharedInstance(kTestClientId);

    // And that acquiring an instance with the same client ID reuses the existing
    // instance.
    VideoFrameSinkBundle& other_bundle = VideoFrameSinkBundle::GetOrCreateSharedInstance(kTestClientId);
...
TEST_F(VideoFrameSinkBundleTest, Reconnect)
{
    ...
    EXPECT_CALL(frame_sink_provider(), CreateFrameSinkBundle_).Times(1).WillOnce([&] { loop.Quit(); });
...
TEST_F(VideoFrameSinkBundleTest, PassThrough)
{
    ...
    bundle.SubmitCompositorFrame(2, kTestSurfaceId, viz::MakeDefaultCompositorFrame(), std::nullopt, 0);
    EXPECT_CALL(mock_frame_sink_bundle(), Submit(ElementsAre(AllOf(IsFrame(), ForSink(2u))))).Times(1);
    mock_frame_sink_bundle().FlushReceiver();

    bundle.DidNotProduceFrame(3, viz::BeginFrameAck(1, 2, false));
    EXPECT_CALL(mock_frame_sink_bundle(), Submit(ElementsAre(AllOf(IsDidNotProduceFrame(), ForSink(3u)))));
    mock_frame_sink_bundle().FlushReceiver();
}
...
TEST_F(VideoFrameSinkBundleTest, BatchSubmissionsDuringOnBeginFrame)
{
    ...
    mojo::Receiver<viz::mojom::blink::CompositorFrameSinkClient> receiver1 { &mock_client1 };
    mojo::Receiver<viz::mojom::blink::CompositorFrameSinkClient> receiver2 { &mock_client2 };
    mojo::Receiver<viz::mojom::blink::CompositorFrameSinkClient> receiver3 { &mock_client3 };
    std::ignore = provider.BindNewPipeAndPassReceiver();
    bundle.AddClient(kTestVideoSinkId1, &mock_client1, provider, receiver1, sink1);
    bundle.AddClient(kTestVideoSinkId2, &mock_client2, provider, receiver2, sink2);
    bundle.AddClient(kTestVideoSinkId3, &mock_client3, provider, receiver3, sink3);

    // All clients will submit a frame synchronously within OnBeginFrame.
    EXPECT_CALL(mock_client1, OnBeginFrame).Times(1).WillOnce([&] {
        bundle.SubmitCompositorFrame(kTestVideoSinkId1.sink_id(), kTestSurfaceId, viz::MakeDefaultCompositorFrame(), std::nullopt, 0);
    });
    EXPECT_CALL(mock_client2, OnBeginFrame).Times(1).WillOnce([&] { bundle.DidNotProduceFrame(kTestVideoSinkId2.sink_id(), viz::BeginFrameAck(1, 1, false)); });
    EXPECT_CALL(mock_client3, OnBeginFrame).Times(1).WillOnce([&] {
        bundle.SubmitCompositorFrame(kTestVideoSinkId3.sink_id(), kTestSurfaceId, viz::MakeDefaultCompositorFrame(), std::nullopt, 0);
    });
    ...
    EXPECT_CALL(mock_frame_sink_bundle(),
        Submit(UnorderedElementsAre(AllOf(IsFrame(), ForSink(kTestVideoSinkId1.sink_id())), AllOf(IsDidNotProduceFrame(), ForSink(kTestVideoSinkId2.sink_id())),
            AllOf(IsFrame(), ForSink(kTestVideoSinkId3.sink_id())))))
        .Times(1);
    mock_frame_sink_bundle().FlushReceiver();
}

TEST_F(VideoFrameSinkBundleTest, DeliversBeginFramesDisabledWithoutSinksOnRegistration)
{
    CreateTestBundle();
    VideoFrameSinkBundle& bundle = test_bundle();
...
TEST_F(VideoFrameSinkBundleTest, DeliversBeginFramesEnabledWithSinkOnRegistration)
{
    CreateTestBundle();
    VideoFrameSinkBundle& bundle = test_bundle();
    auto observer = std::make_unique<MockBeginFrameObserver>();
...
TEST_F(VideoFrameSinkBundleTest, DeliversBeginFrameCompletionOnFlushWithBeginFrames)
{
    CreateTestBundle();
    VideoFrameSinkBundle& bundle = test_bundle();
...
TEST_F(VideoFrameSinkBundleTest, OmitsBeginFrameCompletionOnceOnFlushWithoutBeginFrames)
{
    CreateTestBundle();
    VideoFrameSinkBundle& bundle = test_bundle();
    auto observer = std::make_unique<MockBeginFrameObserver>();
