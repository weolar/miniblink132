...
public:
    explicit MockCanvasResourceDispatcher(unsigned placeholder_id)
        : CanvasResourceDispatcher(
            /*client=*/nullptr, scheduler::GetSingleThreadTaskRunnerForTesting(), scheduler::GetSingleThreadTaskRunnerForTesting(), kClientId, kSinkId,
            placeholder_id,
            /*canvas_size=*/ { kWidth, kHeight })
    {
    }

    MOCK_METHOD2(ReclaimResource, void(viz::ResourceId, scoped_refptr<CanvasResource>&&));
};
...
public:
    ...
    OffscreenCanvasPlaceholder* placeholder()
    {
        return &placeholder_;
    }
...
void OffscreenCanvasPlaceholderTest::SetUp()
{
    ...
    resource_provider_ = CanvasResourceProvider::CreateSharedBitmapProvider(SkImageInfo::MakeN32Premul(kWidth, kHeight), cc::PaintFlags::FilterQuality::kLow,
...
{
    scoped_refptr<CanvasResource> resource = resource_provider_->ProduceCanvasResource(FlushReason::kTesting);
    CanvasResource* resource_raw_ptr = resource.get();
    dispatcher_->DispatchFrame(std::move(resource), base::TimeTicks(), SkIRect::MakeEmpty(), false /* needs_vertical_flip */, false /* is-opaque */);
...
TEST_F(OffscreenCanvasPlaceholderTest, OldFrameNotReclaimedUntilUnref)
{
    ...
    platform->RunUntilIdle();
    EXPECT_EQ(placeholder()->OffscreenCanvasFrame().get(), frame1_raw_ptr);
    scoped_refptr<CanvasResource> extra_ref = placeholder()->OffscreenCanvasFrame();
    Mock::VerifyAndClearExpectations(dispatcher());
