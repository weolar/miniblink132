...
public:
    ...
    {
        dark_mode_image_classifier_ = std::make_unique<DarkModeImageClassifier>(DarkModeImageClassifierPolicy::kNumColorsWithMlFallback);
    }
...
TEST_F(DarkModeImageClassifierTest, ValidImage)
{
    ...
    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeWH(image->width(), image->height())), DarkModeResult::kApplyFilter);
}
...
TEST_F(DarkModeImageClassifierTest, InvalidImage)
{
    ...
    EXPECT_EQ(image_classifier()->Classify(pixmap, src), DarkModeResult::kDoNotApplyFilter);

    // |src| larger than image size.
    image = GetImage("/images/resources/twitter_favicon.ico");
    bitmap = image->AsSkBitmapForCurrentFrame(kDoNotRespectImageOrientation);
    bitmap.peekPixels(&pixmap);
    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeWH(image->width() + 10, image->height() + 10)), DarkModeResult::kDoNotApplyFilter);

    // Empty src rect.
    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect()), DarkModeResult::kDoNotApplyFilter);
}
...
TEST_F(DarkModeImageClassifierTest, ImageSpriteAllFragmentsSame)
{
    ...
    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeXYWH(0, 0, 95, 36)), DarkModeResult::kApplyFilter);

    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeXYWH(0, 36, 95, 36)), DarkModeResult::kApplyFilter);

    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeXYWH(0, 72, 95, 36)), DarkModeResult::kApplyFilter);

    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeXYWH(0, 108, 95, 36)), DarkModeResult::kApplyFilter);

    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeXYWH(0, 144, 95, 36)), DarkModeResult::kApplyFilter);

    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeXYWH(0, 180, 95, 36)), DarkModeResult::kApplyFilter);
}
...
TEST_F(DarkModeImageClassifierTest, ImageSpriteAlternateFragmentsSame)
{
    ...
    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeXYWH(0, 36, 95, 36)), DarkModeResult::kDoNotApplyFilter);

    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeXYWH(0, 72, 95, 36)), DarkModeResult::kApplyFilter);

    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeXYWH(0, 108, 95, 36)), DarkModeResult::kDoNotApplyFilter);

    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeXYWH(0, 144, 95, 36)), DarkModeResult::kApplyFilter);

    EXPECT_EQ(image_classifier()->Classify(pixmap, SkIRect::MakeXYWH(0, 180, 95, 36)), DarkModeResult::kDoNotApplyFilter);
}
...
TEST_F(DarkModeImageClassifierTest, BlockSamples)
{
    ...
    bitmap.peekPixels(&pixmap);
    image_classifier()->GetBlockSamples(pixmap, SkIRect::MakeXYWH(0, 0, 4, 4), 16, &sampled_pixels, &transparent_pixels_count);
    ...
    // Red block.
    image_classifier()->GetBlockSamples(pixmap, SkIRect::MakeXYWH(0, 0, 2, 2), 4, &sampled_pixels, &transparent_pixels_count);
    EXPECT_EQ(sampled_pixels.size(), 4u);
    EXPECT_EQ(transparent_pixels_count, 0);
    for (auto color : sampled_pixels)
        EXPECT_EQ(color, SK_ColorRED);
    // Green block.
    image_classifier()->GetBlockSamples(pixmap, SkIRect::MakeXYWH(2, 0, 2, 2), 4, &sampled_pixels, &transparent_pixels_count);
    EXPECT_EQ(sampled_pixels.size(), 4u);
    EXPECT_EQ(transparent_pixels_count, 0);
    for (auto color : sampled_pixels)
        EXPECT_EQ(color, SK_ColorGREEN);
    // Blue block.
    image_classifier()->GetBlockSamples(pixmap, SkIRect::MakeXYWH(0, 2, 2, 2), 4, &sampled_pixels, &transparent_pixels_count);
    EXPECT_EQ(sampled_pixels.size(), 4u);
    EXPECT_EQ(transparent_pixels_count, 0);
    for (auto color : sampled_pixels)
        EXPECT_EQ(color, SK_ColorBLUE);
    // Alpha block.
    image_classifier()->GetBlockSamples(pixmap, SkIRect::MakeXYWH(2, 2, 2, 2), 4, &sampled_pixels, &transparent_pixels_count);
    EXPECT_EQ(sampled_pixels.size(), 0u);
    EXPECT_EQ(transparent_pixels_count, 4);
}
...
TEST_F(DarkModeImageClassifierTest, FeaturesAndClassification)
{
    ...
    bitmap = image->AsSkBitmapForCurrentFrame(kDoNotRespectImageOrientation);
    bitmap.peekPixels(&pixmap);
    features = image_classifier()->GetFeatures(pixmap, SkIRect::MakeWH(image->width(), image->height())).value();
    EXPECT_EQ(image_classifier()->ClassifyWithFeatures(features), DarkModeResult::kApplyFilter);
    EXPECT_EQ(image_classifier()->ClassifyUsingDecisionTree(features), DarkModeResult::kApplyFilter);
    ...
    bitmap.peekPixels(&pixmap);
    features = image_classifier()->GetFeatures(pixmap, SkIRect::MakeWH(image->width(), image->height())).value();
    EXPECT_EQ(image_classifier()->ClassifyWithFeatures(features), DarkModeResult::kDoNotApplyFilter);
    EXPECT_EQ(image_classifier()->ClassifyUsingDecisionTree(features), DarkModeResult::kNotClassified);
    EXPECT_FALSE(features.is_colorful);
    ...
    features = image_classifier()->GetFeatures(pixmap, SkIRect::MakeWH(image->width(), image->height())).value();
    EXPECT_EQ(image_classifier()->ClassifyWithFeatures(features), DarkModeResult::kDoNotApplyFilter);
    EXPECT_EQ(image_classifier()->ClassifyUsingDecisionTree(features), DarkModeResult::kDoNotApplyFilter);
    EXPECT_TRUE(features.is_colorful);
