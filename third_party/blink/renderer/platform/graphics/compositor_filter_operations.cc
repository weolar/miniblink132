...
void CompositorFilterOperations::AppendColorMatrixFilter(Vector<float> values)
{
    ...
    for (WTF::wtf_size_t i = 0; i < values.size(); ++i)
        matrix[i] = values[i];
    filter_operations_.Append(cc::FilterOperation::CreateColorMatrixFilter(matrix));
}
...
{
    filter_operations_.Append(cc::FilterOperation::CreateBrightnessFilter(amount));
}
...
void CompositorFilterOperations::AppendBlurFilter(float amount, SkTileMode tile_mode)
{
    filter_operations_.Append(cc::FilterOperation::CreateBlurFilter(amount, tile_mode));
}

void CompositorFilterOperations::AppendDropShadowFilter(gfx::Vector2d offset, float std_deviation, const Color& color)
{
    gfx::Point gfx_offset(offset.x(), offset.y());
    // TODO(crbug/1308932): Remove FromColor and make all SkColor4f.
    filter_operations_.Append(cc::FilterOperation::CreateDropShadowFilter(gfx_offset, std_deviation, SkColor4f::FromColor(color.Rgb())));
}

void CompositorFilterOperations::AppendColorMatrixFilter(const cc::FilterOperation::Matrix& matrix)
{
    filter_operations_.Append(cc::FilterOperation::CreateColorMatrixFilter(matrix));
}
...
{
    filter_operations_.Append(cc::FilterOperation::CreateZoomFilter(amount, inset));
}

void CompositorFilterOperations::AppendSaturatingBrightnessFilter(float amount)
{
    filter_operations_.Append(cc::FilterOperation::CreateSaturatingBrightnessFilter(amount));
}

void CompositorFilterOperations::AppendReferenceFilter(sk_sp<PaintFilter> image_filter)
{
    filter_operations_.Append(cc::FilterOperation::CreateReferenceFilter(std::move(image_filter)));
}
...
gfx::RectF CompositorFilterOperations::MapRect(const gfx::RectF& input_rect) const
{
...
bool CompositorFilterOperations::operator==(const CompositorFilterOperations& o) const
{
    return reference_box_ == o.reference_box_ && filter_operations_ == o.filter_operations_;
}

String CompositorFilterOperations::ToString() const
{
    return String(filter_operations_.ToString()) + " at " + String(reference_box_.ToString());
}
