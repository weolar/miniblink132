...
// new frame.
class PLATFORM_EXPORT AnimationWorkletMutatorDispatcherImpl final : public AnimationWorkletMutatorDispatcher {
public:
    ...
    static std::unique_ptr<CompositorMutatorClient> CreateCompositorThreadClient(
        base::WeakPtr<AnimationWorkletMutatorDispatcherImpl>& mutatee, scoped_refptr<base::SingleThreadTaskRunner> mutatee_runner);
    static std::unique_ptr<MainThreadMutatorClient> CreateMainThreadClient(
        base::WeakPtr<AnimationWorkletMutatorDispatcherImpl>& mutatee, scoped_refptr<base::SingleThreadTaskRunner> mutatee_runner);

    explicit AnimationWorkletMutatorDispatcherImpl(scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    AnimationWorkletMutatorDispatcherImpl(const AnimationWorkletMutatorDispatcherImpl&) = delete;
    AnimationWorkletMutatorDispatcherImpl& operator=(const AnimationWorkletMutatorDispatcherImpl&) = delete;
    ~AnimationWorkletMutatorDispatcherImpl() override;

    // AnimationWorkletMutatorDispatcher implementation.
    void MutateSynchronously(std::unique_ptr<AnimationWorkletDispatcherInput>) override;

    bool MutateAsynchronously(std::unique_ptr<AnimationWorkletDispatcherInput>, MutateQueuingStrategy, AsyncMutationCompleteCallback) override;
    ...
    void RegisterAnimationWorkletMutator(CrossThreadPersistent<AnimationWorkletMutator>, scoped_refptr<base::SingleThreadTaskRunner> mutator_runner);

    void UnregisterAnimationWorkletMutator(CrossThreadPersistent<AnimationWorkletMutator>);

    void SetClient(MutatorClient* client)
    {
        client_ = client;
    }
...
private:
    ...
    using AnimationWorkletMutatorToTaskRunnerMap = HashMap<CrossThreadPersistent<AnimationWorkletMutator>, scoped_refptr<base::SingleThreadTaskRunner>>;
    ...
    // cycle.
    void MutateAsynchronouslyInternal(base::TimeTicks request_time, AsyncMutationCompleteCallback done_callback);
    ...
    base::WeakPtrFactory<AnimationWorkletMutatorDispatcherImpl> weak_factory_ { this };
};
