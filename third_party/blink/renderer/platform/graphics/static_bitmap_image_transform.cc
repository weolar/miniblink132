    ...
    if (horizontal) {
        for (int i = 0; i < pm.height() - 1; i++) {
            for (int j = 0; j < pm.width() / 2; j++) {
                ...
                std::swap_ranges(&data[first_element], &data[last_element], &data[bottom_element]);
            }
        }
    } else {
        for (int i = 0; i < pm.height() / 2; i++) {
            ...
            std::swap_ranges(&data[top_first_element], &data[top_last_element], &data[bottom_first_element]);
        }
    }
}
...
ImageOrientation GetSourceOrientation(scoped_refptr<StaticBitmapImage> source, const StaticBitmapImageTransform::Params& params)
{
...
gfx::Size GetSourceSize(scoped_refptr<StaticBitmapImage> source, const StaticBitmapImageTransform::Params& params)
{
    const auto source_info = source->GetSkImageInfo();
    const auto source_orientation = GetSourceOrientation(source, params);

    return source_orientation.UsesWidthAsHeight() ? gfx::Size(source_info.height(), source_info.width()) : gfx::Size(source_info.width(), source_info.height());
}

void ComputeSubsetParameters(scoped_refptr<StaticBitmapImage> source, const StaticBitmapImageTransform::Params& params, SkIRect& source_skrect,
    SkIRect& source_skrect_valid, SkISize& dest_sksize)
{
    const gfx::Size source_size = GetSourceSize(source, params);
    const ImageOrientation source_orientation = GetSourceOrientation(source, params);
    ...
    auto t = source_orientation.TransformFromDefault(gfx::SizeF(unoriented_source_size));
    const gfx::Rect source_rect_valid = gfx::IntersectRects(params.source_rect, gfx::Rect(source_size));
...
scoped_refptr<StaticBitmapImage> StaticBitmapImageTransform::ApplyUsingPixmap(
    scoped_refptr<StaticBitmapImage> source, const StaticBitmapImageTransform::Params& options)
{
    ...
    SkISize dest_size;
    ComputeSubsetParameters(source, options, source_rect, source_rect_valid, dest_size);
    ...
    // Allocate the cropped source image.
    {
        ...
        const auto bm_color_space = options.dest_color_space ? options.dest_color_space : source_info.refColorSpace();
    ...
    // conversion.
    {
        ...
        SkIRect pm_valid_rect = SkIRect::MakeXYWH(
            source_rect_valid.x() - source_rect.x(), source_rect_valid.y() - source_rect.y(), source_rect_valid.width(), source_rect_valid.height());
        SkPixmap pm_valid;
        if (!source_rect_valid.isEmpty() && !bm.pixmap().extractSubset(&pm_valid, pm_valid_rect)) {
            NOTREACHED();
        }
        if (!source_rect_valid.isEmpty()) {
            if (!source_paint_image.readPixels(pm_valid.info(), pm_valid.writable_addr(), pm_valid.rowBytes(), source_rect_valid.x(), source_rect_valid.y())) {
                return nullptr;
            }
        }
    }
    ...
    // Return the result.
    auto dest_paint_image = PaintImageBuilder::WithDefault()
                                .set_id(cc::PaintImage::GetNextId())
                                .set_image(std::move(dest_image), cc::PaintImage::GetNextContentId())
                                .TakePaintImage();
    return UnacceleratedStaticBitmapImage::Create(std::move(dest_paint_image), source_orientation);
}
...
scoped_refptr<StaticBitmapImage> StaticBitmapImageTransform::ApplyWithBlit(
    FlushReason flush_reason, scoped_refptr<StaticBitmapImage> source, const StaticBitmapImageTransform::Params& options)
{
    ...
    const SkAlphaType dest_alpha_type = source_info.alphaType() == kOpaque_SkAlphaType ? kOpaque_SkAlphaType : kPremul_SkAlphaType;
    const auto dest_color_space = options.dest_color_space ? options.dest_color_space : source_info.refColorSpace();
    SkIRect source_rect;
    SkIRect source_rect_valid;
    SkISize dest_size;
    ComputeSubsetParameters(source, options, source_rect, source_rect_valid, dest_size);
    ...
    {
        SkImageInfo dest_info = SkImageInfo::Make(dest_size, dest_color_type, dest_alpha_type, dest_color_space);
        ...
        if (source_paint_image.IsTextureBacked()) {
            base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider = source->ContextProviderWrapper();
            if (context_provider) {
                const gpu::SharedImageUsageSet shared_image_usage_flags
                    = context_provider->ContextProvider()->SharedImageInterface()->UsageForMailbox(source->GetMailboxHolder().mailbox);
                resource_provider = CanvasResourceProvider::CreateSharedImageProvider(
                    dest_info, kFilterQuality, kShouldInitialize, context_provider, RasterMode::kGPU, shared_image_usage_flags);
            }
        }
        // If not (or if the SharedImage provider fails), fall back to software.
        if (!resource_provider) {
            resource_provider = CanvasResourceProvider::CreateBitmapProvider(dest_info, kFilterQuality, kShouldInitialize);
        }
    }
    ...
    canvas.drawImageRect(source_paint_image, SkRect::Make(source_rect), SkRect::Make(dest_size), options.sampling, &paint, SkCanvas::kStrict_SrcRectConstraint);
    ...
    FlushReason flush_reason, scoped_refptr<StaticBitmapImage> source, const StaticBitmapImageTransform::Params& options)
{
    ...
    const bool needs_crop = options.source_rect != gfx::Rect(GetSourceSize(source, options));
    ...
    const bool needs_convert_color_space = options.dest_color_space
        && !SkColorSpace::Equals(options.dest_color_space.get(), source_info.colorSpace() ? source_info.colorSpace() : SkColorSpace::MakeSRGB().get());
    const bool needs_alpha_change = (source->GetSkImageInfo().alphaType() == kUnpremul_SkAlphaType) != (!options.premultiply_alpha);

    // If we aren't doing anything (and this wasn't a forced copy), just return
    // the original.
    if (!options.force_copy && !needs_flip && !needs_crop && !needs_resize && !needs_strip_orientation && !needs_strip_color_space && !needs_convert_color_space
        && !needs_alpha_change) {
        return source;
    }
...
scoped_refptr<StaticBitmapImage> StaticBitmapImageTransform::Clone(FlushReason flush_reason, scoped_refptr<StaticBitmapImage> source)
{
    if (!source) {
        return nullptr;
    }
...
scoped_refptr<StaticBitmapImage> StaticBitmapImageTransform::GetWithAlphaDisposition(
    FlushReason flush_reason, scoped_refptr<StaticBitmapImage> source, AlphaDisposition alpha_disposition)
{
    if (!source) {
        return nullptr;
    }
...
scoped_refptr<StaticBitmapImage> StaticBitmapImageTransform::ConvertToColorSpace(
    FlushReason flush_reason, scoped_refptr<StaticBitmapImage> source, sk_sp<SkColorSpace> color_space)
{
    const auto info = source->GetSkImageInfo();
    StaticBitmapImageTransform::Params options;
    options.source_rect = gfx::Rect(GetSourceSize(source, options));
