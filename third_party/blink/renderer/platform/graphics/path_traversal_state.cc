...
static inline gfx::PointF MidPoint(const gfx::PointF& first, const gfx::PointF& second)
{
    return gfx::PointF((first.x() + second.x()) / 2.0f, (first.y() + second.y()) / 2.0f);
}

static inline float DistanceLine(const gfx::PointF& start, const gfx::PointF& end)
{
...
struct QuadraticBezier {
    ...
    QuadraticBezier(const gfx::PointF& s, const gfx::PointF& c, const gfx::PointF& e)
        : start(s)
        , control(c)
        , end(e)
        , split_depth(0)
    ...
    void Split(QuadraticBezier& left, QuadraticBezier& right) const
    {
        ...
        gfx::PointF left_control_to_right_control = MidPoint(left.control, right.control);
...
struct CubicBezier {
    DISALLOW_NEW();
    CubicBezier() = default;
    CubicBezier(const gfx::PointF& s, const gfx::PointF& c1, const gfx::PointF& c2, const gfx::PointF& e)
        : start(s)
        , control1(c1)
        , control2(c2)
        , end(e)
    ...
    void Split(CubicBezier& left, CubicBezier& right) const
    {
        ...
        gfx::PointF left_control2_to_right_control1 = MidPoint(left.control2, right.control1);
...
template <class CurveType> static float CurveLength(PathTraversalState& traversal_state, CurveType curve)
{
    ...
    do {
        ...
        if ((length_discrepancy * length_discrepancy) / curve_scale_for_tolerance_squared > kPathSegmentLengthToleranceSquared
            ...
            total_length += length;
            if (traversal_state.action_ == PathTraversalState::kTraversalPointAtLength
                || traversal_state.action_ == PathTraversalState::kTraversalNormalAngleAtLength) {
                traversal_state.previous_ = curve.start;
                traversal_state.current_ = curve.end;
                if (traversal_state.total_length_ + total_length > traversal_state.desired_length_)
                    return total_length;
            }
...
float PathTraversalState::CubicBezierTo(const gfx::PointF& new_control1, const gfx::PointF& new_control2, const gfx::PointF& new_end)
{
    float distance = CurveLength<CubicBezier>(*this, CubicBezier(current_, new_control1, new_control2, new_end));

    if (action_ != kTraversalPointAtLength && action_ != kTraversalNormalAngleAtLength)
        current_ = new_end;
...
{
    if ((action_ == kTraversalPointAtLength || action_ == kTraversalNormalAngleAtLength) && total_length_ >= desired_length_) {
