...
FEFlood::FEFlood(Filter* filter, const SkColor4f& flood_color, float flood_opacity)
    : FilterEffect(filter)
...
sk_sp<PaintFilter> FEFlood::CreateImageFilter()
{
    ...
    return sk_make_sp<ColorFilterPaintFilter>(cc::ColorFilter::MakeBlend(color, SkBlendMode::kSrc), nullptr, base::OptionalToPtr(crop_rect));
}

StringBuilder& FEFlood::ExternalRepresentation(StringBuilder& ts, wtf_size_t indent) const
{
    WriteIndent(ts, indent);
    ts << "[feFlood";
    FilterEffect::ExternalRepresentation(ts);
    // TODO(crbug.com/1308932): Color::NameForLayoutTreeAsText to SkColor4f
    ts << " flood-color=\"" << Color::FromSkColor4f(FloodColor()).NameForLayoutTreeAsText() << "\" "
