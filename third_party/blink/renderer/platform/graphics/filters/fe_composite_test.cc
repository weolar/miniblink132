...
protected:
    FEComposite* CreateComposite(CompositeOperationType type, float k1 = 0, float k2 = 0, float k3 = 0, float k4 = 0)
    {
        // Use big filter region to avoid it from affecting FEComposite's MapRect
        // results.
        gfx::RectF filter_region(-10000, -10000, 20000, 20000);
        auto* filter = MakeGarbageCollected<Filter>(gfx::RectF(), filter_region, 1, Filter::kUserSpace);
        ...
        auto* composite = MakeGarbageCollected<FEComposite>(filter, type, k1, k2, k3, k4);
...
#define EXPECT_INTERSECTION(c)                                                                                                                                 \
    do {                                                                                                                                                       \
        ...
        EXPECT_EQ(gfx::RectF(50, 0, 100, 50), c->MapRect(gfx::RectF(0, 0, 200, 200)));                                                                         \
    } while (false)
...
#define EXPECT_INPUT1(c)                                                                                                                                       \
    do {                                                                                                                                                       \
        ...
        EXPECT_EQ(gfx::RectF(kInput1Rect), c->MapRect(gfx::RectF(0, 0, 200, 200)));                                                                            \
    } while (false)
...
#define EXPECT_INPUT2(c)                                                                                                                                       \
    do {                                                                                                                                                       \
        ...
        EXPECT_EQ(gfx::RectF(0, 0, 200, 200), c->MapRect(gfx::RectF(0, 0, 200, 200)));                                                                         \
    } while (false)

#define EXPECT_UNION(c)                                                                                                                                        \
    do {                                                                                                                                                       \
        EXPECT_EQ(gfx::RectF(kInput1Rect), c->MapRect(gfx::RectF()));                                                                                          \
        EXPECT_EQ(gfx::RectF(0, -50, 150, 100), c->MapRect(gfx::RectF(0, 0, 50, 50)));                                                                         \
        EXPECT_EQ(gfx::RectF(0, -50, 200, 250), c->MapRect(gfx::RectF(0, 0, 200, 200)));                                                                       \
    } while (false)
...
TEST_F(FECompositeTest, MapRectArithmetic)
{
    ...
    EXPECT_INTERSECTION(CreateComposite(FECOMPOSITE_OPERATOR_ARITHMETIC, 1, 0, 0, 0));
