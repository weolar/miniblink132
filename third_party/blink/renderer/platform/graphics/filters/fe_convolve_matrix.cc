...
FEConvolveMatrix::FEConvolveMatrix(Filter* filter, const gfx::Size& kernel_size, float divisor, float bias, const gfx::Vector2d& target_offset,
...
sk_sp<PaintFilter> FEConvolveMatrix::CreateImageFilter()
{
    ...
    SkISize kernel_size(SkISize::Make(kernel_size_.width(), kernel_size_.height()));
    ...
    base::span<const SkScalar> kernel_span(kernel.data(), kernel.size());
    std::optional<PaintFilter::CropRect> crop_rect = GetCropRect();
    return sk_make_sp<MatrixConvolutionPaintFilter>(
        kernel_size, kernel_span, gain, bias, target, tile_mode, convolve_alpha, std::move(input), base::OptionalToPtr(crop_rect));
}

static StringBuilder& operator<<(StringBuilder& ts, const FEConvolveMatrix::EdgeModeType& type)
{
    switch (type) {
...
StringBuilder& FEConvolveMatrix::ExternalRepresentation(StringBuilder& ts, wtf_size_t indent) const
{
