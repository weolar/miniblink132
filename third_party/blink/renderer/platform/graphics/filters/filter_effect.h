...
enum FilterEffectType { kFilterEffectTypeUnknown, kFilterEffectTypeImage, kFilterEffectTypeTile, kFilterEffectTypeSourceInput };
...
public:
    ...
    {
        return input_effects_;
    }
    FilterEffect* InputEffect(unsigned) const;
    unsigned NumberOfEffectInputs() const
    {
        return input_effects_.size();
    }

    inline bool HasImageFilter() const
    {
        return image_filters_[0] || image_filters_[1] || image_filters_[2] || image_filters_[3];
    }
    ...
    virtual StringBuilder& ExternalRepresentation(StringBuilder&, wtf_size_t indention = 0) const;

    gfx::RectF FilterPrimitiveSubregion() const
    {
        return filter_primitive_subregion_;
    }
    void SetFilterPrimitiveSubregion(const gfx::RectF& filter_primitive_subregion)
    {
    ...
    virtual void SetOperatingInterpolationSpace(InterpolationSpace interpolation_space)
    {
        operating_interpolation_space_ = interpolation_space;
    }

    virtual bool AffectsTransparentPixels() const
    {
        return false;
    }
    ...
    virtual bool MayProduceInvalidPreMultipliedPixels()
    {
        return false;
    }

    PaintFilter* GetImageFilter(InterpolationSpace, bool requires_pm_color_validation) const;
    void SetImageFilter(InterpolationSpace, bool requires_pm_color_validation, sk_sp<PaintFilter>);

    bool OriginTainted() const
    {
        return origin_tainted_;
    }
    void SetOriginTainted()
    {
        origin_tainted_ = true;
    }
