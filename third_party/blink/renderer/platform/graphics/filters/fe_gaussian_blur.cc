...
{
    return static_cast<unsigned>(floorf(s * (3 / 4.f * sqrtf(kTwoPiFloat)) + 0.5f));
}
...
gfx::RectF FEGaussianBlur::MapEffect(const gfx::SizeF& std_deviation, const gfx::RectF& rect)
{
    ...
    result.Outset(gfx::OutsetsF::VH(3.0f * kernel_size.height() * 0.5f, 3.0f * kernel_size.width() * 0.5f));
...
{
    gfx::SizeF std_error(GetFilter()->ApplyHorizontalScale(std_x_), GetFilter()->ApplyVerticalScale(std_y_));
...
sk_sp<PaintFilter> FEGaussianBlur::CreateImageFilter()
{
    ...
    return sk_make_sp<BlurPaintFilter>(SkFloatToScalar(std_x), SkFloatToScalar(std_y), SkTileMode::kDecal, std::move(input), base::OptionalToPtr(crop_rect));
}

StringBuilder& FEGaussianBlur::ExternalRepresentation(StringBuilder& ts, wtf_size_t indent) const
{
