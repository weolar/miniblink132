...
public:
    static scoped_refptr<SpotLightSource> Create(
        const gfx::Point3F& position, const gfx::Point3F& points_at, float specular_exponent, float limiting_cone_angle)
    {
        return base::AdoptRef(new SpotLightSource(position, points_at, specular_exponent, limiting_cone_angle));
    }
    ...
    const gfx::Point3F& PointsAt() const
    {
        return points_at_;
    }
...
private:
    SpotLightSource(const gfx::Point3F& position, const gfx::Point3F& points_at, float specular_exponent, float limiting_cone_angle)
        : LightSource(kLsSpot)
        , position_(position)
