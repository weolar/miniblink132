...
sk_sp<PaintFilter> FEComposite::CreateImageFilterInternal(bool requires_pm_color_validation)
{
    sk_sp<PaintFilter> foreground(paint_filter_builder::Build(InputEffect(0), OperatingInterpolationSpace(), !MayProduceInvalidPreMultipliedPixels()));
    sk_sp<PaintFilter> background(paint_filter_builder::Build(InputEffect(1), OperatingInterpolationSpace(), !MayProduceInvalidPreMultipliedPixels()));
    std::optional<PaintFilter::CropRect> crop_rect = GetCropRect();

    if (type_ == FECOMPOSITE_OPERATOR_ARITHMETIC) {
        return sk_make_sp<ArithmeticPaintFilter>(SkFloatToScalar(k1_), SkFloatToScalar(k2_), SkFloatToScalar(k3_), SkFloatToScalar(k4_),
            requires_pm_color_validation, std::move(background), std::move(foreground), base::OptionalToPtr(crop_rect));
    }

    return sk_make_sp<XfermodePaintFilter>(ToBlendMode(type_), std::move(background), std::move(foreground), base::OptionalToPtr(crop_rect));
}

static StringBuilder& operator<<(StringBuilder& ts, const CompositeOperationType& type)
{
    switch (type) {
...
StringBuilder& FEComposite::ExternalRepresentation(StringBuilder& ts, wtf_size_t indent) const
{
    ...
    ts << " operation=\"" << type_ << "\"";
    if (type_ == FECOMPOSITE_OPERATOR_ARITHMETIC)
        ts << " k1=\"" << k1_ << "\" k2=\"" << k2_ << "\" k3=\"" << k3_ << "\" k4=\"" << k4_ << "\"";
    ts << "]\n";
