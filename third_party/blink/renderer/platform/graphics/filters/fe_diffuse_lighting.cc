...
FEDiffuseLighting::FEDiffuseLighting(
    Filter* filter, const Color& lighting_color, float surface_scale, float diffuse_constant, scoped_refptr<LightSource> light_source)
    : FELighting(filter, kDiffuseLighting, lighting_color, surface_scale, diffuse_constant, 0, 0, std::move(light_source))
{
}
...
StringBuilder& FEDiffuseLighting::ExternalRepresentation(StringBuilder& ts, wtf_size_t indent) const
{
