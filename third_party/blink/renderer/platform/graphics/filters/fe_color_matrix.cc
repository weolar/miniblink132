...
static void SaturateMatrix(float s, base::span<float, kColorMatrixSize> matrix)
{
...
static void HueRotateMatrix(float hue, base::span<float, kColorMatrixSize> matrix)
{
...
static sk_sp<cc::ColorFilter> CreateColorFilter(ColorMatrixType type, const Vector<float>& values)
{
    ...
    case FECOLORMATRIX_TYPE_MATRIX: {
        if (auto maybe_matrix = base::span(values.data(), values.size()).to_fixed_extent<kColorMatrixSize>()) {
...
bool FEColorMatrix::AffectsTransparentPixels() const
{
    ...
    return type_ == FECOLORMATRIX_TYPE_MATRIX && values_.size() >= kColorMatrixSize && values_[19] > 0;
}
...
sk_sp<PaintFilter> FEColorMatrix::CreateImageFilter()
{
    ...
    return sk_make_sp<ColorFilterPaintFilter>(std::move(filter), std::move(input), base::OptionalToPtr(crop_rect));
}

static StringBuilder& operator<<(StringBuilder& ts, const ColorMatrixType& type)
{
    switch (type) {
    case FECOLORMATRIX_TYPE_UNKNOWN:
        ts << "UNKNOWN";
...
StringBuilder& FEColorMatrix::ExternalRepresentation(StringBuilder& ts, wtf_size_t indent) const
{
