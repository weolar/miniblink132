...
FEMorphology::FEMorphology(Filter* filter, MorphologyOperatorType type, float radius_x, float radius_y)
    : FilterEffect(filter)
    , type_(type)
...
gfx::RectF FEMorphology::MapEffect(const gfx::RectF& rect) const
{
    ...
    result.Outset(gfx::OutsetsF::VH(GetFilter()->ApplyVerticalScale(radius_y_), GetFilter()->ApplyHorizontalScale(radius_x_)));
    return result;
}
...
sk_sp<PaintFilter> FEMorphology::CreateImageFilter()
{
    ...
    MorphologyPaintFilter::MorphType morph_type
        = type_ == FEMORPHOLOGY_OPERATOR_DILATE ? MorphologyPaintFilter::MorphType::kDilate : MorphologyPaintFilter::MorphType::kErode;
    return sk_make_sp<MorphologyPaintFilter>(morph_type, radius_x, radius_y, std::move(input), base::OptionalToPtr(crop_rect));
}

static StringBuilder& operator<<(StringBuilder& ts, const MorphologyOperatorType& type)
{
    switch (type) {
...
StringBuilder& FEMorphology::ExternalRepresentation(StringBuilder& ts, wtf_size_t indent) const
{
