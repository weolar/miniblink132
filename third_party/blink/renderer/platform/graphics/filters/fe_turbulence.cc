...
FETurbulence::FETurbulence(Filter* filter, TurbulenceType type, float base_frequency_x, float base_frequency_y, int num_octaves, float seed, bool stitch_tiles)
    : FilterEffect(filter)
    , type_(type)
...
sk_sp<PaintFilter> FETurbulence::CreateImageFilter()
{
    ...
    TurbulencePaintFilter::TurbulenceType type = GetType() == FETURBULENCE_TYPE_FRACTALNOISE ? TurbulencePaintFilter::TurbulenceType::kFractalNoise
                                                                                             : TurbulencePaintFilter::TurbulenceType::kTurbulence;
    const SkISize size = SkISize::Make(FilterPrimitiveSubregion().width(), FilterPrimitiveSubregion().height());
    ...
    return sk_make_sp<TurbulencePaintFilter>(type, SkFloatToScalar(base_frequency_x), SkFloatToScalar(base_frequency_y), capped_num_octaves,
        SkFloatToScalar(Seed()), StitchTiles() ? &size : nullptr, base::OptionalToPtr(crop_rect));
}

static StringBuilder& operator<<(StringBuilder& ts, const TurbulenceType& type)
{
    switch (type) {
...
StringBuilder& FETurbulence::ExternalRepresentation(StringBuilder& ts, wtf_size_t indent) const
{
