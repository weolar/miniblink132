...
FEDisplacementMap::FEDisplacementMap(Filter* filter, ChannelSelectorType x_channel_selector, ChannelSelectorType y_channel_selector, float scale)
    : FilterEffect(filter)
...
gfx::RectF FEDisplacementMap::MapEffect(const gfx::RectF& rect) const
{
    ...
    result.Outset(gfx::OutsetsF::VH(GetFilter()->ApplyVerticalScale(std::abs(scale_) / 2), GetFilter()->ApplyHorizontalScale(std::abs(scale_) / 2)));
    return result;
}
...
bool FEDisplacementMap::SetXChannelSelector(const ChannelSelectorType x_channel_selector)
{
...
bool FEDisplacementMap::SetYChannelSelector(const ChannelSelectorType y_channel_selector)
{
...
{
    sk_sp<PaintFilter> color = paint_filter_builder::Build(InputEffect(0), OperatingInterpolationSpace());
    ...
    sk_sp<PaintFilter> displ = paint_filter_builder::Build(InputEffect(1), OperatingInterpolationSpace());
    ...
    return sk_make_sp<DisplacementMapEffectPaintFilter>(
        type_x, type_y, SkFloatToScalar(GetFilter()->ApplyHorizontalScale(scale_)), std::move(displ), std::move(color), base::OptionalToPtr(crop_rect));
}

static StringBuilder& operator<<(StringBuilder& ts, const ChannelSelectorType& type)
{
    switch (type) {
    case CHANNEL_UNKNOWN:
        ts << "UNKNOWN";
...
StringBuilder& FEDisplacementMap::ExternalRepresentation(StringBuilder& ts, wtf_size_t indent) const
{
