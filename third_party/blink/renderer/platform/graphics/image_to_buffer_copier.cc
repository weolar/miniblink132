...
ImageToBufferCopier::ImageToBufferCopier(gpu::gles2::GLES2Interface* gl, gpu::SharedImageInterface* sii)
    : gl_(gl)
    , sii_(sii)
{
}
...
bool ImageToBufferCopier::EnsureDestImage(const gfx::Size& size)
{
    ...
    if (dest_image_size_ != size || !dest_shared_image_) {
        ...
        dest_shared_image_ = sii_->CreateSharedImage(
            { viz::SinglePlaneFormat::kRGBA_8888, size, gfx::ColorSpace(), gpu::SHARED_IMAGE_USAGE_GLES2_WRITE, "ImageToBufferCopier" },
...
std::pair<gfx::GpuMemoryBufferHandle, gpu::SyncToken> ImageToBufferCopier::CopyImage(Image* image)
{
    if (!image)
        return {};
    ...
    auto dest_scoped_si_access = dest_si_texture->BeginAccess(gpu::SyncToken(), /*readonly=*/false);
    ...
    auto source_scoped_si_access = source_si_texture->BeginAccess(gpu::SyncToken(), /*readonly=*/true);

    gl_->CopySubTextureCHROMIUM(source_scoped_si_access->texture_id(), 0, GL_TEXTURE_2D, dest_scoped_si_access->texture_id(), 0, 0, 0, 0, 0, size.width(),
        size.height(), false, false, false);

    // Cleanup the read framebuffer and texture.
    gpu::SharedImageTexture::ScopedAccess::EndAccess(std::move(source_scoped_si_access));
    source_si_texture.reset();

    // Cleanup the draw framebuffer and texture.
    gpu::SyncToken sync_token = gpu::SharedImageTexture::ScopedAccess::EndAccess(std::move(dest_scoped_si_access));
    dest_si_texture.reset();

    static_image->UpdateSyncToken(sync_token);

    return std::make_pair(dest_shared_image_ ? dest_shared_image_->CloneGpuMemoryBufferHandle() : gfx::GpuMemoryBufferHandle(), sync_token);
}
