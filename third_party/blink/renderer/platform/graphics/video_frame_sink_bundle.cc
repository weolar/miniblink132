...
mojom::blink::EmbeddedFrameSinkProvider* g_frame_sink_provider_override = nullptr;

std::unique_ptr<VideoFrameSinkBundle>& GetThreadFrameSinkBundlePtr()
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(ThreadSpecific<std::unique_ptr<VideoFrameSinkBundle>>, bundle, ());
    return *bundle;
}

} // namespace

VideoFrameSinkBundle::VideoFrameSinkBundle(base::PassKey<VideoFrameSinkBundle>, uint32_t client_id)
    : id_(GenerateFrameSinkBundleId(client_id))
{
    ...
    if (g_frame_sink_provider_override) {
        ...
        Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(host_provider.BindNewPipeAndPassReceiver());
        provider = host_provider.get();
    }
    provider->RegisterEmbeddedFrameSinkBundle(id_, bundle_.BindNewPipeAndPassReceiver(), receiver_.BindNewPipeAndPassRemote());
    bundle_.set_disconnect_handler(base::BindOnce(&VideoFrameSinkBundle::OnDisconnected, base::Unretained(this)));
}

VideoFrameSinkBundle::~VideoFrameSinkBundle() = default;

// static
VideoFrameSinkBundle& VideoFrameSinkBundle::GetOrCreateSharedInstance(uint32_t client_id)
{
    ...
    bundle_ptr = std::make_unique<VideoFrameSinkBundle>(base::PassKey<VideoFrameSinkBundle>(), client_id);
    return *bundle_ptr;
}
...
// static
void VideoFrameSinkBundle::SetFrameSinkProviderForTesting(mojom::blink::EmbeddedFrameSinkProvider* provider)
{
    g_frame_sink_provider_override = provider;
}

void VideoFrameSinkBundle::SetBeginFrameObserver(std::unique_ptr<BeginFrameObserver> observer)
{
    begin_frame_observer_ = std::move(observer);
    if (begin_frame_observer_) {
        begin_frame_observer_->OnBeginFrameCompletionEnabled(!sinks_needing_begin_frames_.empty());
    }
}

base::WeakPtr<VideoFrameSinkBundle> VideoFrameSinkBundle::AddClient(const viz::FrameSinkId& frame_sink_id, viz::mojom::blink::CompositorFrameSinkClient* client,
    mojo::Remote<mojom::blink::EmbeddedFrameSinkProvider>& frame_sink_provider, mojo::Receiver<viz::mojom::blink::CompositorFrameSinkClient>& receiver,
    mojo::Remote<viz::mojom::blink::CompositorFrameSink>& remote)
{
    ...
    frame_sink_provider->CreateBundledCompositorFrameSink(frame_sink_id, id_, receiver.BindNewPipeAndPassRemote(), remote.BindNewPipeAndPassReceiver());
...
void VideoFrameSinkBundle::InitializeCompositorFrameSinkType(uint32_t sink_id, viz::mojom::blink::CompositorFrameSinkType type)
{
    bundle_->InitializeCompositorFrameSinkType(sink_id, type);
}

void VideoFrameSinkBundle::SetNeedsBeginFrame(uint32_t sink_id, bool needs_begin_frame)
{
    DVLOG(2) << __func__ << " this " << this << " sink_id " << sink_id << " needs_begin_frame " << needs_begin_frame;
...
void VideoFrameSinkBundle::SubmitCompositorFrame(uint32_t sink_id, const viz::LocalSurfaceId& local_surface_id, viz::CompositorFrame frame,
    std::optional<viz::HitTestRegionList> hit_test_region_list, uint64_t submit_time)
{
    ...
    submission->data = viz::mojom::blink::BundledFrameSubmissionData::NewFrame(std::move(bundled_frame));
...
void VideoFrameSinkBundle::DidNotProduceFrame(uint32_t sink_id, const viz::BeginFrameAck& ack)
{
    auto submission = viz::mojom::blink::BundledFrameSubmission::New();
    submission->sink_id = sink_id;
    submission->data = viz::mojom::blink::BundledFrameSubmissionData::NewDidNotProduceFrame(ack);
...
void VideoFrameSinkBundle::DidAllocateSharedBitmap(uint32_t sink_id, base::ReadOnlySharedMemoryRegion region, const viz::SharedBitmapId& id)
{
    bundle_->DidAllocateSharedBitmap(sink_id, std::move(region), id);
}

void VideoFrameSinkBundle::DidDeleteSharedBitmap(uint32_t sink_id, const viz::SharedBitmapId& id)
{
    ...
    submission_queue_.push_back(
        viz::mojom::blink::BundledFrameSubmission::New(sink_id, viz::mojom::blink::BundledFrameSubmissionData::NewDidDeleteSharedBitmap(id)));
}

#if BUILDFLAG(IS_ANDROID)
void VideoFrameSinkBundle::SetThreads(uint32_t sink_id, const WTF::Vector<viz::Thread>& threads)
{
    bundle_->SetThreads(sink_id, threads);
}
#endif

void VideoFrameSinkBundle::FlushNotifications(WTF::Vector<viz::mojom::blink::BundledReturnedResourcesPtr> acks,
    WTF::Vector<viz::mojom::blink::BeginFrameInfoPtr> begin_frames, WTF::Vector<viz::mojom::blink::BundledReturnedResourcesPtr> reclaimed_resources)
{
    ...
    for (auto& entry : begin_frames) {
        ...
        if (it == clients_.end())
            continue;
        it->value->OnBeginFrame(std::move(entry->args), std::move(entry->details), entry->frame_ack, std::move(entry->resources));
    }
...
void VideoFrameSinkBundle::OnBeginFramePausedChanged(uint32_t sink_id, bool paused)
{
...
void VideoFrameSinkBundle::OnCompositorFrameTransitionDirectiveProcessed(uint32_t sink_id, uint32_t sequence_id)
{
    auto it = clients_.find(sink_id);
    if (it == clients_.end())
        return;
