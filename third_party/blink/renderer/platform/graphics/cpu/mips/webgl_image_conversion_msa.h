...
ALWAYS_INLINE void packOneRowOfRGBA8ToUnsignedShort4444MSA(const uint8_t*& source, uint16_t*& destination, unsigned& pixelsPerRow)
{
    unsigned i;
    v16u8 src0, src1, src2, src3, src4, src5, src6, src7;
    ...
    for (i = (pixelsPerRow >> 5); i--;) {
        ...
        PCKEV_B4_UH(vec1, vec0, vec3, vec2, vec5, vec4, vec7, vec6, dst0, dst1, dst2, dst3);
...
ALWAYS_INLINE void packOneRowOfRGBA8LittleToR8MSA(const uint8_t*& source, uint8_t*& destination, unsigned& pixelsPerRow)
{
    unsigned i;
    v16u8 src0, src1, src2, src3, src4, src5, src6, src7;
    ...
    const v16u8 alphaMask = { 0, 0, 0, 255, 0, 0, 0, 255, 0, 0, 0, 255, 0, 0, 0, 255 };
    ...
    for (i = (pixelsPerRow >> 5); i--;) {
        ...
        AND_V4_UB(src0A, src1A, src2A, src3A, alphaMask, src0A, src1A, src2A, src3A);
        AND_V4_UB(src4A, src5A, src6A, src7A, alphaMask, src4A, src5A, src6A, src7A);
        src0A = SLDI_UB(src0A, src0A, 3);
        src1A = SLDI_UB(src1A, src1A, 3);
        src2A = SLDI_UB(src2A, src2A, 3);
        src3A = SLDI_UB(src3A, src3A, 3);
        ...
        DIV4(vfCnst255, fsrc0A, vfCnst255, fsrc1A, vfCnst255, fsrc2A, vfCnst255, fsrc3A, fsrc0A, fsrc1A, fsrc2A, fsrc3A);
        DIV4(vfCnst255, fsrc4A, vfCnst255, fsrc5A, vfCnst255, fsrc6A, vfCnst255, fsrc7A, fsrc4A, fsrc5A, fsrc6A, fsrc7A);
        MUL4(fsrc0R, fsrc0A, fsrc1R, fsrc1A, fsrc2R, fsrc2A, fsrc3R, fsrc3A, fdst0R, fdst1R, fdst2R, fdst3R);
        MUL4(fsrc4R, fsrc4A, fsrc5R, fsrc5A, fsrc6R, fsrc6A, fsrc7R, fsrc7A, fdst4R, fdst5R, fdst6R, fdst7R);
        ...
        PCKEV_H4_UB(dst1, dst0, dst3, dst2, dst5, dst4, dst7, dst6, dst0, dst2, dst4, dst6);
...
ALWAYS_INLINE void packOneRowOfRGBA8LittleToRA8MSA(const uint8_t*& source, uint8_t*& destination, unsigned& pixelsPerRow)
{
    unsigned i;
    v16u8 src0, src1, src2, src3, src4, src5, src6, src7;
    v16u8 src0A, src1A, src2A, src3A, src4A, src5A, src6A, src7A;
    ...
    for (i = (pixelsPerRow >> 5); i--;) {
        ...
        ILVR_D4_UB(dst1, dst0, dst3, dst2, dst5, dst4, dst7, dst6, dst0, dst1, dst2, dst3);
