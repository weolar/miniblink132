...
ALWAYS_INLINE void UnpackOneRowOfRGBA4444LittleToRGBA8(const uint16_t*& source, uint8_t*& destination, unsigned& pixels_per_row)
{
    ...
    for (unsigned i = 0; i < pixels_per_row_trunc; i += 8) {
        ...
        __m128i component_rg = _mm_or_si128(_mm_slli_epi16(component_g, 8), component_r);
        __m128i component_ba = _mm_or_si128(_mm_slli_epi16(component_a, 8), component_b);
...
ALWAYS_INLINE void UnpackOneRowOfRGBA5551LittleToRGBA8(const uint16_t*& source, uint8_t*& destination, unsigned& pixels_per_row)
{
    ...
    for (unsigned i = 0; i < pixels_per_row_trunc; i += 8) {
        ...
        __m128i component_r = _mm_or_si128(_mm_slli_epi16(r, 3), _mm_and_si128(r, immediate0x7));
        __m128i component_g = _mm_or_si128(_mm_slli_epi16(g, 3), _mm_and_si128(g, immediate0x7));
        __m128i component_b = _mm_or_si128(_mm_slli_epi16(b, 3), _mm_and_si128(b, immediate0x7));
        __m128i component_a = _mm_cmpeq_epi16(_mm_and_si128(packed_value, immediate0x1), immediate0x1);
...
ALWAYS_INLINE void PackOneRowOfRGBA8LittleToRA8(const uint8_t*& source, uint8_t*& destination, unsigned& pixels_per_row)
{
    float tmp[4];
    ...
    for (unsigned i = 0; i < pixels_per_row_trunc; i += 4) {
        __m128 scale_factor
            = _mm_set_ps(source[15] ? source[15] : 255.0, source[11] ? source[11] : 255.0, source[7] ? source[7] : 255.0, source[3] ? source[3] : 255.0);
...
ALWAYS_INLINE void UnpackOneRowOfBGRA8LittleToRGBA8(const uint32_t*& source, uint32_t*& destination, unsigned& pixels_per_row)
{
    __m128i bgra, rgba;
    ...
    for (unsigned i = 0; i < pixels_per_row_trunc; i += 4) {
        ...
        rgba = _mm_or_si128(_mm_and_si128(rgba, br_mask), _mm_and_si128(bgra, ga_mask));
...
ALWAYS_INLINE void PackOneRowOfRGBA8LittleToR8(const uint8_t*& source, uint8_t*& destination, unsigned& pixels_per_row)
{
    float tmp[4];
    unsigned pixels_per_row_trunc = (pixels_per_row / 4) * 4;

    for (unsigned i = 0; i < pixels_per_row_trunc; i += 4) {
        __m128 scale = _mm_set_ps(source[15] ? source[15] : 255, source[11] ? source[11] : 255, source[7] ? source[7] : 255, source[3] ? source[3] : 255);
...
ALWAYS_INLINE void PackOneRowOfRGBA8LittleToRGBA8(const uint8_t*& source, uint8_t*& destination, unsigned& pixels_per_row)
{
    float tmp[4];
    float scale;
    ...
    for (unsigned i = 0; i < pixels_per_row; i++) {
        ...
        source_r = _mm_mul_ps(source_r, _mm_div_ps(_mm_set1_ps(255), _mm_set1_ps(scale)));
