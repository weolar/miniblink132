...
ALWAYS_INLINE void UnpackOneRowOfRGBA5551LittleToRGBA8(const uint16_t*& source, uint8_t*& destination, unsigned& pixels_per_row)
{
    ...
    for (unsigned i = 0; i < pixels_per_row_trunc; i += 8) {
        ...
        __m128i component_r = __lsx_vor_v(__lsx_vslli_b(r, 3), __lsx_vand_v(r, immediate0x7));
        __m128i component_g = __lsx_vor_v(__lsx_vslli_b(g, 3), __lsx_vand_v(g, immediate0x7));
        __m128i component_b = __lsx_vor_v(__lsx_vslli_b(b, 3), __lsx_vand_v(b, immediate0x7));
        __m128i component_a = __lsx_vseq_h(__lsx_vand_v(packed_value, immediate0x1), immediate0x1);
