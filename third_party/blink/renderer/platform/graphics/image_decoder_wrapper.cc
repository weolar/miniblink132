...
{
    return pixmap.alphaType() == kUnpremul_SkAlphaType ? ImageDecoder::kAlphaNotPremultiplied : ImageDecoder::kAlphaPremultiplied;
}
...
public:
    explicit ExternalMemoryAllocator(const SkPixmap& pixmap)
        : pixmap_(pixmap)
    {
    }
...
ImageDecoderWrapper::ImageDecoderWrapper(ImageFrameGenerator* generator, SegmentReader* data, const SkPixmap& pixmap, ColorBehavior decoder_color_behavior,
    cc::AuxImage aux_image, wtf_size_t index, bool all_data_received, cc::PaintImage::GeneratorClientId client_id)
...
bool IsLowEndDeviceOrPartialLowEndModeEnabled()
{
    ...
    return Platform::Current()->IsLowEndDevice() || base::SysInfo::IsLowEndDeviceOrPartialLowEndModeEnabled();
#else
...
bool ImageDecoderWrapper::Decode(ImageDecoderFactory* factory, wtf_size_t* frame_count, bool* has_alpha)
{
    DCHECK(frame_count);
    DCHECK(has_alpha);
    ...
    const bool resume_decoding = ImageDecodingStore::Instance().LockDecoder(generator_, pixmap_.dimensions(), PixmapAlphaOption(pixmap_), client_id_, &decoder);
    ...
    const bool decode_to_external_memory = ShouldDecodeToExternalMemory(*frame_count, resume_decoding);
    ...
    if (!has_decoded_frame) {
        ...
        if (resume_decoding) {
            ImageDecodingStore::Instance().UnlockDecoder(generator_, client_id_, decoder);
        }
        return false;
    }
    ...
    *has_alpha = !scaled_size_bitmap.isOpaque();
    ...
    if (resume_decoding) {
        ...
        // it to the cache.
        ImageDecodingStore::Instance().InsertDecoder(generator_, client_id_, std::move(new_decoder));
    }

    return true;
}

bool ImageDecoderWrapper::ShouldDecodeToExternalMemory(wtf_size_t frame_count, bool resume_decoding) const
{
...
bool ImageDecoderWrapper::ShouldRemoveDecoder(bool frame_was_completely_decoded, bool decoded_to_external_memory) const
{
...
void ImageDecoderWrapper::PurgeAllFramesIfNecessary(ImageDecoder* decoder, bool frame_was_completely_decoded, wtf_size_t frame_count) const
{
...
std::unique_ptr<ImageDecoder> ImageDecoderWrapper::CreateDecoderWithData(ImageDecoderFactory* factory) const
{
    if (factory) {
    ...
    const ImageDecoder::HighBitDepthDecodingOption high_bit_depth_decoding_option
        = pixmap_.colorType() == kRGBA_F16_SkColorType ? ImageDecoder::kHighBitDepthToHalfFloat : ImageDecoder::kDefaultBitDepth;

    // The newly created decoder just grabbed the data.  No need to reset it.
    return ImageDecoder::Create(data_, all_data_received_, PixmapAlphaOption(pixmap_), high_bit_depth_decoding_option, decoder_color_behavior_, aux_image_,
