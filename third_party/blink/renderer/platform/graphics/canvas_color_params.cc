...
gfx::ColorSpace PredefinedColorSpaceToGfxColorSpace(PredefinedColorSpace color_space)
{
    switch (color_space) {
    case PredefinedColorSpace::kSRGB:
        return gfx::ColorSpace::CreateSRGB();
    case PredefinedColorSpace::kRec2020:
        return gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT2020, gfx::ColorSpace::TransferID::GAMMA24);
    case PredefinedColorSpace::kP3:
        return gfx::ColorSpace::CreateDisplayP3D65();
    case PredefinedColorSpace::kRec2100HLG:
        return gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT2020, gfx::ColorSpace::TransferID::HLG);
    case PredefinedColorSpace::kRec2100PQ:
        return gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT2020, gfx::ColorSpace::TransferID::PQ);
...
sk_sp<SkColorSpace> PredefinedColorSpaceToSkColorSpace(PredefinedColorSpace color_space)
{
    return PredefinedColorSpaceToGfxColorSpace(color_space).ToSkColorSpace();
}

PredefinedColorSpace PredefinedColorSpaceFromSkColorSpace(const SkColorSpace* sk_color_space)
{
    ...
    PredefinedColorSpace color_spaces[] = {
        ...
        PredefinedColorSpace::kRec2100HLG,
        PredefinedColorSpace::kRec2100PQ,
        PredefinedColorSpace::kSRGBLinear,
    };
    for (const auto& color_space : color_spaces) {
        if (SkColorSpace::Equals(sk_color_space, PredefinedColorSpaceToGfxColorSpace(color_space).ToSkColorSpace().get())) {
            return color_space;
        }
    }
...
CanvasColorParams::CanvasColorParams(PredefinedColorSpace color_space, CanvasPixelFormat pixel_format, OpacityMode opacity_mode)
...
CanvasColorParams::CanvasColorParams(PredefinedColorSpace color_space, CanvasPixelFormat pixel_format, bool has_alpha)
    : color_space_(color_space)
    , pixel_format_(pixel_format)
    , opacity_mode_(has_alpha ? OpacityMode::kNonOpaque : OpacityMode::kOpaque)
{
}

SkColorInfo CanvasColorParams::GetSkColorInfo() const
{
    return SkColorInfo(GetSkColorType(), opacity_mode_ == kOpaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType, GetSkColorSpace());
}
...
{
    static_assert(kN32_SkColorType == kRGBA_8888_SkColorType || kN32_SkColorType == kBGRA_8888_SkColorType, "Unexpected kN32_SkColorType value.");
