...
sk_sp<SkImage> CreateFrameAtIndex(DeferredImageDecoder* decoder, size_t index)
{
    return SkImages::DeferredFromGenerator(
        std::make_unique<SkiaPaintImageGenerator>(decoder->CreateGenerator(), index, cc::PaintImage::kDefaultGeneratorClientId));
}
 ...
 */
static void MixImages(const char* file_name, size_t bytes_for_first_frame, size_t later_frame)
{
    base::test::SingleThreadTaskEnvironment task_environment;
    const Vector<char> file = ReadFile(file_name);

    scoped_refptr<SharedBuffer> partial_file = SharedBuffer::Create(base::span(file).first(bytes_for_first_frame));
    std::unique_ptr<DeferredImageDecoder> decoder
        = DeferredImageDecoder::Create(partial_file, false, ImageDecoder::kAlphaPremultiplied, ColorBehavior::kIgnore);
    ASSERT_NE(decoder, nullptr);
    sk_sp<SkImage> partial_image = CreateFrameAtIndex(decoder.get(), 0);

    scoped_refptr<SharedBuffer> almost_complete_file = SharedBuffer::Create(base::span(file).first(file.size() - 1));
    decoder->SetData(almost_complete_file, false);
    sk_sp<SkImage> image_with_more_data = CreateFrameAtIndex(decoder.get(), later_frame);
...
TEST(DeferredImageDecoderTestWoPlatform, fragmentedSignature)
{
    ...
    for (const auto* test_file : test_files) {
        ...
        EXPECT_EQ(nullptr, DeferredImageDecoder::Create(buffer, false, ImageDecoder::kAlphaPremultiplied, ColorBehavior::kIgnore));
        ...
        std::unique_ptr<DeferredImageDecoder> decoder = DeferredImageDecoder::Create(buffer, false, ImageDecoder::kAlphaPremultiplied, ColorBehavior::kIgnore);
        ASSERT_NE(decoder, nullptr);
