...
bool IsImageFromGpuMemoryBufferFormatSupported(gfx::BufferFormat format, const gpu::Capabilities& capabilities)
{
    *(int*)1 = 1;
    return false;
}

bool IsImageSizeValidForGpuMemoryBufferFormat(const gfx::Size& size, gfx::BufferFormat format)
{
    *(int*)1 = 1;
    return false;
}

uint32_t ClientSharedImage::GetTextureTarget(void)
{
    *(int*)1 = 1;
    return 0;
}
...
SharedImageFormat SkColorTypeToSinglePlaneSharedImageFormat(SkColorType color_type)
{
    switch (color_type) {
...
{
    return format == SinglePlaneFormat::kBGRA_8888 || format == SinglePlaneFormat::kR_8 || format == SinglePlaneFormat::kR_16
        || format == SinglePlaneFormat::kRG_1616 || format == SinglePlaneFormat::kRGBA_4444 || format == SinglePlaneFormat::kRGBA_8888
        || format == SinglePlaneFormat::kRGBA_F16 || format == SinglePlaneFormat::kBGR_565 || format == SinglePlaneFormat::kRG_88
        || format == SinglePlaneFormat::kRGBX_8888 || format == SinglePlaneFormat::kBGRX_8888 || format == SinglePlaneFormat::kRGBA_1010102
        || format == SinglePlaneFormat::kBGRA_1010102 || format == MultiPlaneFormat::kYV12 || format == MultiPlaneFormat::kNV12
        || format == MultiPlaneFormat::kNV12A || format == MultiPlaneFormat::kP010;
}

gfx::BufferFormat SinglePlaneSharedImageFormatToBufferFormat(SharedImageFormat format)
{
...
static FlushForImageListener* GetFlushForImageListener()
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(ThreadSpecific<FlushForImageListener>, flush_for_image_listener, ());
...
bool IsGMBAllowed(const SkImageInfo& info, const gpu::Capabilities& caps)
{
    ...
    const gfx::BufferFormat buffer_format = viz::SinglePlaneSharedImageFormatToBufferFormat(viz::SkColorTypeToSinglePlaneSharedImageFormat(info.colorType()));
    return gpu::IsImageSizeValidForGpuMemoryBufferFormat(size, buffer_format) && gpu::IsImageFromGpuMemoryBufferFormatSupported(buffer_format, caps);
}
...
public:
    CanvasImageProvider(cc::ImageDecodeCache* cache_n32, cc::ImageDecodeCache* cache_f16, const gfx::ColorSpace& target_color_space,
        SkColorType target_color_type, cc::PlaybackImageProvider::RasterMode raster_mode);
    ...
    cc::ImageProvider::ScopedResult GetRasterContent(const cc::DrawImage&) override;
...
public:
    CanvasResourceProviderBitmap(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
        base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher, CanvasResourceHost* resource_host)
        : CanvasResourceProvider(kBitmap, info, filter_quality,
            /*is_origin_top_left=*/true,
            /*context_provider_wrapper=*/nullptr, std::move(resource_dispatcher), resource_host)
    {
    }
...
private:
    ...
    scoped_refptr<StaticBitmapImage> Snapshot(FlushReason reason, ImageOrientation orientation) override
    {
...
class CanvasResourceProviderSharedBitmap : public CanvasResourceProviderBitmap, public BitmapGpuChannelLostObserver {
public:
    CanvasResourceProviderSharedBitmap(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
        base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher, WebGraphicsSharedImageInterfaceProvider* shared_image_interface_provider,
        CanvasResourceHost* resource_host)
        : CanvasResourceProviderBitmap(info, filter_quality, std::move(resource_dispatcher), resource_host)
        , shared_image_interface_provider_(shared_image_interface_provider ? shared_image_interface_provider->GetWeakPtr() : nullptr)
    {
    ...
    {
        resource_host()->NotifyGpuContextLost();
    }
    ...
    base::WeakPtr<WebGraphicsSharedImageInterfaceProvider> shared_image_interface_provider_;
    ...
    bool IsSharedBitmapGpuChannelLost() const override
    {
        ...
        return !shared_image_interface_provider_ || !shared_image_interface_provider_->SharedImageInterface();
    }
...
private:
    scoped_refptr<CanvasResource> CreateResource() final
    {
        ...
        if (!viz::SkColorTypeToSinglePlaneSharedImageFormat(info.colorType()).IsBitmapFormatSupported()) {
            // If the rendering format is not supported, downgrade to 8-bits.
            // TODO(junov): Should we try 12-12-12-12 and 10-10-10-2?
            info = info.makeColorType(kN32_SkColorType);
        }

        return CanvasResourceSharedBitmap::Create(info, CreateWeakPtr(), shared_image_interface_provider_, FilterQuality());
    }

    scoped_refptr<CanvasResource> ProduceCanvasResource(FlushReason reason) final
    {
        DCHECK(GetSkSurface());
        ...
        static_cast<CanvasResourceSharedBitmap*>(output_resource.get())->TakeSkImage(std::move(sk_image));
...
public:
    CanvasResourceProviderSharedImage(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
        base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, bool is_accelerated, gpu::SharedImageUsageSet shared_image_usage_flags,
        CanvasResourceHost* resource_host)
        : CanvasResourceProvider(kSharedImage, info, filter_quality,
            /*is_origin_top_left=*/true, std::move(context_provider_wrapper),
            /*resource_dispatcher=*/nullptr, resource_host)
        , is_accelerated_(is_accelerated)
        , shared_image_usage_flags_(shared_image_usage_flags)
        , use_oop_rasterization_(is_accelerated && ContextProviderWrapper()->ContextProvider()->GetCapabilities().gpu_rasterization)
    {
    ...
    {
        return shared_image_usage_flags_.Has(gpu::SHARED_IMAGE_USAGE_CONCURRENT_READ_WRITE);
    }
    scoped_refptr<gpu::ClientSharedImage> GetBackingClientSharedImageForExternalWrite(
        gpu::SyncToken* internal_access_sync_token, gpu::SharedImageUsageSet required_shared_image_usages, bool* was_copy_performed) override
    {
    ...
    void EndExternalWrite(const gpu::SyncToken& external_write_sync_token) override
    {
    ...
    bool WritePixels(const SkImageInfo& orig_info, const void* pixels, size_t row_bytes, int x, int y) override
    {
        *(int*)1 = 1;
        return false;

//         if (!use_oop_rasterization_) {
//             return CanvasResourceProvider::WritePixels(orig_info, pixels, row_bytes, x, y);
//         }
// 
...
//         RasterInterface()->WritePixels(GetBackingClientSharedImageForOverwrite()->mailbox(), x, y, resource()->GetClientSharedImage()->GetTextureTarget(),
...
//         if (x <= 0 && y <= 0 && orig_info.width() >= Size().width() && orig_info.height() >= Size().height())
//             is_cleared_ = true;
// 
//         return true;
    }
    ...
    scoped_refptr<CanvasResource> CreateResource() final
    {
        ...
        return CanvasResourceSharedImage::Create(
            GetSkImageInfo(), ContextProviderWrapper(), CreateWeakPtr(), FilterQuality(), is_accelerated_, shared_image_usage_flags_);
    }

    bool UseOopRasterization() final
    {
        return use_oop_rasterization_;
    }
...
//             auto tex = SkSurfaces::GetBackendTexture(surface_.get(), SkSurfaces::BackendHandleAccess::kFlushRead);
...
protected:
    scoped_refptr<CanvasResource> ProduceCanvasResource(FlushReason reason) override
    {
        TRACE_EVENT0("blink", "CanvasResourceProviderSharedImage::ProduceCanvasResource");
        if (IsGpuContextLost())
            return nullptr;
        ...
        if (ContextProviderWrapper()->ContextProvider()->GetCapabilities().disable_2d_canvas_copy_on_write) {
    ...
    scoped_refptr<StaticBitmapImage> Snapshot(FlushReason reason, ImageOrientation orientation) override
    {
        ...
        if (!cached_snapshot_) {
            ...
            if (cached_snapshot_) {
                cached_content_id_ = cached_snapshot_->PaintImageForCurrentFrame().GetContentIdForFrame(0u);
            }
        }
    ...
    void WillDrawInternal(bool write_to_local_texture)
    {
        ...
        if (is_accelerated_ && (ShouldReplaceTargetBuffer(cached_content_id_) || !IsResourceUsable(resource_.get()))) {
            cached_content_id_ = PaintImage::kInvalidContentId;
            DCHECK(!current_resource_has_write_access_) << "Write access must be released before sharing the resource";

            auto old_resource = std::move(resource_);
            auto* old_resource_shared_image = static_cast<CanvasResourceSharedImage*>(old_resource.get());
            ...
            if (mode_ == SkSurface::kRetain_ContentChangeMode) {
                auto old_mailbox = old_resource_shared_image->GetClientSharedImage()->mailbox();
                auto mailbox = resource()->GetClientSharedImage()->mailbox();

                RasterInterface()->CopySharedImage(old_mailbox, mailbox, 0, 0, 0, 0, Size().width(), Size().height());
            ...
            UMA_HISTOGRAM_BOOLEAN("Blink.Canvas.ContentChangeMode", mode_ == SkSurface::kRetain_ContentChangeMode);
    ...
    void RasterRecord(cc::PaintRecord last_recording) override
    {
        ...
        is_cleared_ = true;
        RasterRecordOOP(std::move(last_recording), needs_clear, resource()->GetClientSharedImage()->mailbox());
    }

    bool ShouldReplaceTargetBuffer(PaintImage::ContentId content_id = PaintImage::kInvalidContentId)
    {
    ...
    sk_sp<SkSurface> CreateSkSurface() const override
    {
        ...
        if (is_accelerated_) {
            *(int*)1 = 1;
            return nullptr;
//             return SkSurfaces::WrapBackendTexture(GetGrContext(), CreateGrTextureForResource(), GetGrSurfaceOrigin(), 0 /* msaa_sample_count */,
//                 GetSkImageInfo().colorType(), GetSkImageInfo().refColorSpace(), &props);
        }
...
//         GetGrContext()->flushAndSubmit();
    ...
    void EnsureWriteAccess()
    {
        ...
        DCHECK(resource_->HasOneRef() || IsSingleBuffered() || !is_accelerated_) << "Write access requires exclusive access to the resource";
        DCHECK(!resource()->is_cross_thread()) << "Write access is only allowed on the owning thread";
    ...
    void EndWriteAccess()
    {
        ...
        if (is_accelerated_) {
            ...
            resource()->CopyRenderingResultsToGpuMemoryBuffer(GetSkSurface()->makeImageSnapshot());
        }
    ...
    void OnDestroyRecyclableCanvasResource(const gpu::SyncToken& sync_token) override
    {
    ...
    void OnFlushForImage(cc::PaintImage::ContentId content_id) override
    {
        ...
        if (cached_snapshot_ && cached_snapshot_->PaintImageForCurrentFrame().GetContentIdForFrame(0) == content_id) {
...
private:
    bool IsResourceUsable(CanvasResource* resource) final
    {
        ...
        return resource->GetClientSharedImage()->usage().HasAll(shared_image_usage_flags_);
    }
...
public:
    CanvasResourceProviderPassThrough(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
        base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher,
        bool is_origin_top_left, CanvasResourceHost* resource_host)
        : CanvasResourceProvider(
            kPassThrough, info, filter_quality, is_origin_top_left, std::move(context_provider_wrapper), std::move(resource_dispatcher), resource_host)
    {
    }
...
private:
    ...
    scoped_refptr<StaticBitmapImage> Snapshot(FlushReason, ImageOrientation) override
    {
...
public:
    CanvasResourceProviderSwapChain(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
        base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher,
        CanvasResourceHost* resource_host)
        : CanvasResourceProvider(kSwapChain, info, filter_quality,
            /*is_origin_top_left=*/true, std::move(context_provider_wrapper), std::move(resource_dispatcher), resource_host)
        , use_oop_rasterization_(ContextProviderWrapper()->ContextProvider()->GetCapabilities().gpu_rasterization)
    {
        resource_ = CanvasResourceSwapChain::Create(GetSkImageInfo(), ContextProviderWrapper(), CreateWeakPtr(), FilterQuality());
...
private:
    ...
    {
        DCHECK(IsSingleBuffered());
        TRACE_EVENT0("blink", "CanvasResourceProviderSwapChain::ProduceCanvasResource");
        if (!IsValid())
            return nullptr;
    ...
    scoped_refptr<StaticBitmapImage> Snapshot(FlushReason reason, ImageOrientation) override
    {
...
//         texture_info.fTarget = resource_->GetBackBufferClientSharedImage()->GetTextureTarget();
//         texture_info.fFormat
//             = ContextProviderWrapper()->ContextProvider()->GetGrGLTextureFormat(viz::SkColorTypeToSinglePlaneSharedImageFormat(GetSkImageInfo().colorType()));
// 
//         auto backend_texture = GrBackendTextures::MakeGL(Size().width(), Size().height(), skgpu::Mipmapped::kNo, texture_info);
// 
//         const auto props = GetSkSurfaceProps();
//         return SkSurfaces::WrapBackendTexture(GetGrContext(), backend_texture, kTopLeft_GrSurfaceOrigin, 0 /* msaa_sample_count */,
//             GetSkImageInfo().colorType(), GetSkImageInfo().refColorSpace(), &props);
    }
    ...
    void RasterRecord(cc::PaintRecord last_recording) override
    {
        ...
        WillDraw();
        RasterRecordOOP(last_recording, initial_needs_clear_, resource_->GetBackBufferClientSharedImage()->mailbox());
...
// 
//         WillDraw();
//         RasterInterface()->WritePixels(resource_->GetBackBufferClientSharedImage()->mailbox(), x, y,
//             resource_->GetBackBufferClientSharedImage()->GetTextureTarget(), SkPixmap(orig_info, pixels, row_bytes));
        return true;
    }
...
std::unique_ptr<CanvasResourceProvider> CanvasResourceProvider::CreateBitmapProvider(
    const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality, ShouldInitialize should_initialize, CanvasResourceHost* resource_host)
{
    auto provider = std::make_unique<CanvasResourceProviderBitmap>(info, filter_quality, /*resource_dispatcher=*/nullptr, resource_host);
    if (provider->IsValid()) {
        if (should_initialize == CanvasResourceProvider::ShouldInitialize::kCallClear)
            provider->Clear();
        return provider;
    }
    return nullptr;
...
std::unique_ptr<CanvasResourceProvider> CanvasResourceProvider::CreateSharedBitmapProvider(const SkImageInfo& info,
    cc::PaintFlags::FilterQuality filter_quality, ShouldInitialize should_initialize, base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher,
    WebGraphicsSharedImageInterfaceProvider* shared_image_interface_provider, CanvasResourceHost* resource_host)
{
    ...
    auto provider = std::make_unique<CanvasResourceProviderSharedBitmap>(
        info, filter_quality, std::move(resource_dispatcher), shared_image_interface_provider, resource_host);
    if (provider->IsValid()) {
        if (should_initialize == CanvasResourceProvider::ShouldInitialize::kCallClear)
            provider->Clear();
        return provider;
...
std::unique_ptr<CanvasResourceProvider> CanvasResourceProvider::CreateSharedImageProvider(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
    ShouldInitialize should_initialize, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, RasterMode raster_mode,
    gpu::SharedImageUsageSet shared_image_usage_flags, CanvasResourceHost* resource_host)
{
    ...
    if (!context_provider_wrapper || context_provider_wrapper->ContextProvider()->IsContextLost())
        return nullptr;

    const auto& capabilities = context_provider_wrapper->ContextProvider()->GetCapabilities();
    if ((info.width() < 1 || info.height() < 1 || info.width() > capabilities.max_texture_size || info.height() > capabilities.max_texture_size)) {
        return nullptr;
    }
    ...
    if (!shared_image_usage_flags.HasAny(gpu::SHARED_IMAGE_USAGE_WEBGPU_READ | gpu::SHARED_IMAGE_USAGE_WEBGPU_WRITE)) {
        adjusted_info = adjusted_info.makeColorType(is_accelerated && info.colorType() != kRGBA_F16_SkColorType ? kRGBA_8888_SkColorType : info.colorType());
    }

    const bool is_gpu_memory_buffer_image_allowed
        = is_gpu_compositing_enabled && IsGMBAllowed(adjusted_info, capabilities) && SharedGpuContext::GetGpuMemoryBufferManager();
    ...
    const auto& shared_image_caps = context_provider_wrapper->ContextProvider()->SharedImageInterface()->GetCapabilities();
    if (!is_gpu_memory_buffer_image_allowed || (is_accelerated && !shared_image_caps.supports_scanout_shared_images)) {
        shared_image_usage_flags.RemoveAll(gpu::SHARED_IMAGE_USAGE_CONCURRENT_READ_WRITE | gpu::SHARED_IMAGE_USAGE_SCANOUT);
    }

    if (resource_host && resource_host->TransferToGPUTextureWasInvoked()) {
        shared_image_usage_flags.PutAll(gpu::SHARED_IMAGE_USAGE_WEBGPU_READ | gpu::SHARED_IMAGE_USAGE_WEBGPU_WRITE);
    }

#if BUILDFLAG(IS_MAC)
    if ((shared_image_usage_flags & gpu::SHARED_IMAGE_USAGE_SCANOUT) && is_accelerated && adjusted_info.colorType() == kRGBA_8888_SkColorType) {
    ...
    auto provider = std::make_unique<CanvasResourceProviderSharedImage>(
        adjusted_info, filter_quality, context_provider_wrapper, is_accelerated, shared_image_usage_flags, resource_host);
    if (provider->IsValid()) {
        if (should_initialize == CanvasResourceProvider::ShouldInitialize::kCallClear)
            provider->Clear();
        return provider;
...
std::unique_ptr<CanvasResourceProvider> CanvasResourceProvider::CreateWebGPUImageProvider(
    const SkImageInfo& info, gpu::SharedImageUsageSet shared_image_usage_flags, CanvasResourceHost* resource_host)
{
    ...
    return CreateSharedImageProvider(info, cc::PaintFlags::FilterQuality::kLow, CanvasResourceProvider::ShouldInitialize::kNo,
        std::move(context_provider_wrapper), RasterMode::kGPU,
        shared_image_usage_flags | gpu::SHARED_IMAGE_USAGE_WEBGPU_READ | gpu::SHARED_IMAGE_USAGE_WEBGPU_WRITE, resource_host);
}

std::unique_ptr<CanvasResourceProvider> CanvasResourceProvider::CreatePassThroughProvider(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher,
    bool is_origin_top_left, CanvasResourceHost* resource_host)
{
    ...
    const auto& capabilities = context_provider_wrapper->ContextProvider()->GetCapabilities();
    if (info.width() > capabilities.max_texture_size || info.height() > capabilities.max_texture_size) {
        return nullptr;
    }

    const auto& shared_image_capabilities = context_provider_wrapper->ContextProvider()->SharedImageInterface()->GetCapabilities();
    // Either swap_chain or gpu memory buffer should be enabled for this be used
    if (!shared_image_capabilities.shared_image_swap_chain && (!IsGMBAllowed(info, capabilities) || !Platform::Current()->GetGpuMemoryBufferManager())) {
        return nullptr;
    }
    ...
    auto provider = std::make_unique<CanvasResourceProviderPassThrough>(
        info, filter_quality, context_provider_wrapper, resource_dispatcher, is_origin_top_left, resource_host);
    CHECK(provider->IsValid());
    return provider;
}

std::unique_ptr<CanvasResourceProvider> CanvasResourceProvider::CreateSwapChainProvider(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
    ShouldInitialize should_initialize, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper,
    base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher, CanvasResourceHost* resource_host)
{
    // SharedGpuContext::IsGpuCompositingEnabled can potentially replace the
    // context_provider_wrapper, so it's important to call that first as it can
    ...
    if (info.width() > capabilities.max_texture_size || info.height() > capabilities.max_texture_size || !shared_image_capabilities.shared_image_swap_chain) {
        return nullptr;
    }

    auto provider = std::make_unique<CanvasResourceProviderSwapChain>(info, filter_quality, context_provider_wrapper, resource_dispatcher, resource_host);
    if (provider->IsValid()) {
        if (should_initialize == CanvasResourceProvider::ShouldInitialize::kCallClear)
            provider->Clear();
        return provider;
...
CanvasResourceProvider::CanvasImageProvider::CanvasImageProvider(cc::ImageDecodeCache* cache_n32, cc::ImageDecodeCache* cache_f16,
    const gfx::ColorSpace& target_color_space, SkColorType canvas_color_type, cc::PlaybackImageProvider::RasterMode raster_mode)
    : raster_mode_(raster_mode)
{
    std::optional<cc::PlaybackImageProvider::Settings> settings = cc::PlaybackImageProvider::Settings();
    ...
    playback_image_provider_n32_.emplace(cache_n32, target_color_params, std::move(settings));
    ...
    if (canvas_color_type == kRGBA_F16_SkColorType) {
        ...
        settings->raster_mode = raster_mode_;
        playback_image_provider_f16_.emplace(cache_f16, target_color_params, std::move(settings));
    }
}

cc::ImageProvider::ScopedResult CanvasResourceProvider::CanvasImageProvider::GetRasterContent(const cc::DrawImage& draw_image)
{
    ...
    if (paint_image.IsDeferredPaintRecord()) {
        ...
        scoped_refptr<CanvasDeferredPaintRecord> canvas_deferred_paint_record(
            static_cast<CanvasDeferredPaintRecord*>(paint_image.deferred_paint_record().get()));
        return cc::ImageProvider::ScopedResult(canvas_deferred_paint_record->GetPaintRecord());
    }
    ...
    if (playback_image_provider_f16_ && draw_image.paint_image().is_high_bit_depth()) {
        DCHECK(playback_image_provider_f16_);
        scoped_decoded_image = playback_image_provider_f16_->GetRasterContent(draw_image);
    } else {
        scoped_decoded_image = playback_image_provider_n32_->GetRasterContent(draw_image);
    }
    ...
    if (!scoped_decoded_image.decoded_image().is_budgeted() || locked_images_.size() > kMaxLockedImagesCount) {
    ...
    return ScopedResult(decoded_draw_image, base::BindOnce(&CanvasImageProvider::CanUnlockImage, weak_factory_.GetWeakPtr(), std::move(scoped_decoded_image)));
}

void CanvasResourceProvider::CanvasImageProvider::CanUnlockImage(ScopedResult image)
{
    ...
    if (!cleanup_task_pending_) {
        ...
        ThreadScheduler::Current()->CleanupTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&CanvasImageProvider::CleanupLockedImages, weak_factory_.GetWeakPtr()));
    }
...
bool CanvasResourceProvider::CanvasImageProvider::IsHardwareDecodeCache() const
{
    return raster_mode_ != cc::PlaybackImageProvider::RasterMode::kSoftware;
}

BASE_FEATURE(kCanvas2DAutoFlushParams, "Canvas2DAutoFlushParams", base::FEATURE_DISABLED_BY_DEFAULT);

// When enabled, unused resources (ready to be recycled) are reclaimed after a
// delay.
BASE_FEATURE(kCanvas2DReclaimUnusedResources, "Canvas2DReclaimUnusedResources", base::FEATURE_DISABLED_BY_DEFAULT);
...
const base::FeatureParam<int> kMaxRecordedOpKB(&kCanvas2DAutoFlushParams, "max_recorded_op_kb", 2 * 1024);

const base::FeatureParam<int> kMaxPinnedImageKB(&kCanvas2DAutoFlushParams, "max_pinned_image_kb", 32 * 1024);

// Graphite can generally handle more ops, increase the size accordingly.
const base::FeatureParam<int> kMaxRecordedOpGraphiteKB(&kCanvas2DAutoFlushParams, "max_recorded_op_graphite_kb", 6 * 1024);

CanvasResourceProvider::CanvasResourceProvider(const ResourceProviderType& type, const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
    bool is_origin_top_left, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper,
    base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher, CanvasResourceHost* resource_host)
    : type_(type)
...
//         const auto& caps = context_provider_wrapper_->ContextProvider()->GetCapabilities();
//         oopr_uses_dmsaa_ = !caps.msaa_is_slow && !caps.avoid_stencil_buffers;
        // Graphite can handle a large buffer size.
        //     if (context_provider_wrapper_->ContextProvider()->GetGpuFeatureInfo().status_values[gpu::GPU_FEATURE_TYPE_SKIA_GRAPHITE] == gpu::kGpuFeatureStatusEnabled) {
        //       max_recorded_op_bytes_ = static_cast<size_t>(kMaxRecordedOpGraphiteKB.Get()) * 1024;
...
{
    UMA_HISTOGRAM_EXACT_LINEAR("Blink.Canvas.MaximumInflightResources", max_inflight_resources_, 20);
    if (context_provider_wrapper_) {
        *(int*)1 = 1;
...
std::unique_ptr<MemoryManagedPaintRecorder> CanvasResourceProvider::ReleaseRecorder()
{
...
void CanvasResourceProvider::SetRecorder(std::unique_ptr<MemoryManagedPaintRecorder> recorder)
{
...
void CanvasResourceProvider::FlushIfRecordingLimitExceeded()
{
    ...
    if (recorder_->ReleasableOpBytesUsed() > max_recorded_op_bytes_ || recorder_->ReleasableImageBytesUsed() > max_pinned_image_bytes_) [[unlikely]] {
...
void CanvasResourceProvider::NotifyWillTransfer(cc::PaintImage::ContentId content_id)
{
...
bool CanvasResourceProvider::OverwriteImage(
    const gpu::Mailbox& shared_image_mailbox, const gfx::Rect& copy_rect, const gpu::SyncToken& ready_sync_token, gpu::SyncToken& completion_sync_token)
{
    ...
    raster->CopySharedImage(shared_image_mailbox, dst_client_si->mailbox(),
        /*xoffset=*/0,
        /*yoffset=*/0, copy_rect.x(), copy_rect.y(), copy_rect.width(), copy_rect.height());
...
void CanvasResourceProvider::EnsureSkiaCanvas()
{
    ...
    if (IsAccelerated() && ContextProviderWrapper()
        && !ContextProviderWrapper()->ContextProvider()->GetGpuFeatureInfo().IsWorkaroundEnabled(gpu::DISABLE_2D_CANVAS_AUTO_FLUSH)) {
        context_flushes.enable = true;
        context_flushes.max_draws_before_flush = kMaxDrawsBeforeContextFlush;
    }
    skia_canvas_ = std::make_unique<cc::SkiaPaintCanvas>(GetSkSurface()->getCanvas(), GetOrCreateCanvasImageProvider(), context_flushes);
}

CanvasResourceProvider::CanvasImageProvider* CanvasResourceProvider::GetOrCreateCanvasImageProvider()
{
    if (!canvas_image_provider_) {
        ...
        if (UseHardwareDecodeCache()) {
            raster_mode = UseOopRasterization() ? cc::PlaybackImageProvider::RasterMode::kOop : cc::PlaybackImageProvider::RasterMode::kGpu;
        }
        canvas_image_provider_ = std::make_unique<CanvasImageProvider>(ImageDecodeCacheRGBA8(), cache_f16, GetColorSpace(), info_.colorType(), raster_mode);
    }
    return canvas_image_provider_.get();
}

void CanvasResourceProvider::InitializeForRecording(cc::PaintCanvas* canvas) const
{
    if (resource_host_) {
...
scoped_refptr<StaticBitmapImage> CanvasResourceProvider::SnapshotInternal(ImageOrientation orientation, FlushReason reason)
{
    if (!IsValid())
        return nullptr;
    ...
    return UnacceleratedStaticBitmapImage::Create(std::move(paint_image), orientation);
}
...
cc::PaintImage CanvasResourceProvider::MakeImageSnapshot(FlushReason reason)
{
    ...
    if (snapshot_paint_image_content_id_ == PaintImage::kInvalidContentId || last_snapshot_sk_image_id != snapshot_sk_image_id_) {
        snapshot_paint_image_content_id_ = PaintImage::GetNextContentId();
    }

    return PaintImageBuilder::WithDefault().set_id(snapshot_paint_image_id_).set_image(std::move(sk_image), snapshot_paint_image_content_id_).TakePaintImage();
}
...
{
    const bool can_use_lcd_text = GetSkImageInfo().alphaType() == kOpaque_SkAlphaType;
...
gfx::ColorSpace CanvasResourceProvider::GetColorSpace() const
{
    ...
    return color_space ? gfx::ColorSpace(*color_space) : gfx::ColorSpace::CreateSRGB();
}

std::optional<cc::PaintRecord> CanvasResourceProvider::FlushCanvas(FlushReason reason)
{
    if (!recorder_->HasReleasableDrawOps()) {
        return std::nullopt;
    }
    ScopedRasterTimer timer(IsAccelerated() ? RasterInterface() : nullptr, *this, always_enable_raster_timers_for_testing_);
    DCHECK(reason != FlushReason::kNone);
    bool want_to_print
        = (IsPrinting() && reason != FlushReason::kClear) || reason == FlushReason::kPrinting || reason == FlushReason::kCanvasPushFrameWhilePrinting;
    ...
    ReleaseLockedImages();
    last_recording_ = preserve_recording ? std::optional(recording) : std::nullopt;
    return recording;
}
...
void CanvasResourceProvider::RasterRecordOOP(cc::PaintRecord last_recording, bool needs_clear, gpu::Mailbox mailbox)
{
    if (IsGpuContextLost())
        return;
    gpu::raster::RasterInterface* ri = RasterInterface();
    SkColor4f background_color = GetSkImageInfo().alphaType() == kOpaque_SkAlphaType ? SkColors::kBlack : SkColors::kTransparent;
    ...
    ri->BeginRasterCHROMIUM(background_color, needs_clear,
        /*msaa_sample_count=*/oopr_uses_dmsaa_ ? 1 : 0, oopr_uses_dmsaa_ ? gpu::raster::MsaaMode::kDMSAA : gpu::raster::MsaaMode::kNoMSAA, can_use_lcd_text,
        /*visible=*/true, GetColorSpace(),
        /*hdr_headroom=*/1.f, mailbox.name);

    ri->RasterCHROMIUM(list.get(), GetOrCreateCanvasImageProvider(), size, full_raster_rect, playback_rect, post_translate, post_scale,
...
bool CanvasResourceProvider::IsGpuContextLost() const
{
    ...
    return !raster_interface || raster_interface->GetGraphicsResetStatusKHR() != GL_NO_ERROR;
}
...
bool CanvasResourceProvider::WritePixels(const SkImageInfo& orig_info, const void* pixels, size_t row_bytes, int x, int y)
{
    ...
    bool wrote_pixels = GetSkSurface()->getCanvas()->writePixels(orig_info, pixels, row_bytes, x, y);
...
{
    if (UseHardwareDecodeCache()) {
        //return context_provider_wrapper_->ContextProvider()->ImageDecodeCache(kRGBA_F16_SkColorType);
        *(int*)1 = 1;
        return nullptr;
    }
...
void CanvasResourceProvider::RecycleResource(scoped_refptr<CanvasResource>&& resource)
{
    // We don't want to keep an arbitrary large number of canvases.
    if (canvas_resources_.size() > static_cast<unsigned int>(kMaxRecycledCanvasResources)) {
        return;
    }
    ...
    if (resource->HasOneRef() && resource_recycling_enabled_ && !is_single_buffered_ && IsResourceUsable(resource.get())) {
...
void CanvasResourceProvider::RegisterUnusedResource(scoped_refptr<CanvasResource>&& resource)
{
...
void CanvasResourceProvider::MaybePostUnusedResourcesReclaimTask()
{
    ...
    if (resource_recycling_enabled_ && !is_single_buffered_ && !unused_resources_reclaim_timer_.IsRunning() && !canvas_resources_.empty()) {
        unused_resources_reclaim_timer_.Start(
            FROM_HERE, kUnusedResourceExpirationTime, base::BindOnce(&CanvasResourceProvider::ClearOldUnusedResources, base::Unretained(this)));
    }
}

void CanvasResourceProvider::ClearOldUnusedResources()
{
    bool cleared_resources = WTF::EraseIf(
        canvas_resources_, [](const UnusedResource& resource) { return base::TimeTicks::Now() - resource.last_use >= kUnusedResourceExpirationTime; });
    ...
    if (cleared_resources && ContextProviderWrapper()) {
        if (gpu::ContextSupport* context_support = ContextProviderWrapper()->ContextProvider()->ContextSupport()) {
...
scoped_refptr<CanvasResource> CanvasResourceProvider::NewOrRecycledResource()
{
    ...
    scoped_refptr<CanvasResource> resource = std::move(canvas_resources_.back().resource);
...
bool CanvasResourceProvider::ImportResource(scoped_refptr<CanvasResource>&& resource)
{
    if (!IsSingleBuffered() || !SupportsSingleBuffering())
        return false;
    canvas_resources_.clear();
...
scoped_refptr<CanvasResource> CanvasResourceProvider::GetImportedResource() const
{
    if (!IsSingleBuffered() || !SupportsSingleBuffering())
        return nullptr;
...
void CanvasResourceProvider::OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd)
{
    *(int*)1 = 1;
//     if (!surface_)
//         return;
...
//     }
// 
//     std::string dump_name = base::StringPrintf("canvas/ResourceProvider/SkSurface/0x%" PRIXPTR, reinterpret_cast<uintptr_t>(surface_.get()));
//     auto* dump = pmd->CreateAllocatorDump(dump_name);
// 
//     dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, ComputeSurfaceSize());
//     dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameObjectCount, base::trace_event::MemoryAllocatorDump::kUnitsObjects, 1);
// 
...
//     if (const char* system_allocator_name = base::trace_event::MemoryDumpManager::GetInstance()->system_allocator_pool_name()) {
