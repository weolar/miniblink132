...
class VideoMockCompositorFrameSink : public viz::mojom::blink::CompositorFrameSink {
public:
    VideoMockCompositorFrameSink(mojo::PendingReceiver<viz::mojom::blink::CompositorFrameSink> receiver)
    {
        receiver_.Bind(std::move(receiver));
    }
    VideoMockCompositorFrameSink(const VideoMockCompositorFrameSink&) = delete;
    VideoMockCompositorFrameSink& operator=(const VideoMockCompositorFrameSink&) = delete;
    ...
    MOCK_METHOD2(DoSubmitCompositorFrame, void(const viz::LocalSurfaceId&, viz::CompositorFrame*));
    void SubmitCompositorFrame(
        const viz::LocalSurfaceId& id, viz::CompositorFrame frame, std::optional<viz::HitTestRegionList> hit_test_region_list, uint64_t submit_time) override
    {
        last_submitted_compositor_frame_ = std::move(frame);
        DoSubmitCompositorFrame(id, &last_submitted_compositor_frame_);
    }
    void SubmitCompositorFrameSync(const viz::LocalSurfaceId& id, viz::CompositorFrame frame, std::optional<viz::HitTestRegionList> hit_test_region_list,
        uint64_t submit_time, const SubmitCompositorFrameSyncCallback callback) override
    {
        last_submitted_compositor_frame_ = std::move(frame);
        DoSubmitCompositorFrame(id, &last_submitted_compositor_frame_);
    }
    ...
    MOCK_METHOD2(DidAllocateSharedBitmap, void(base::ReadOnlySharedMemoryRegion region, const viz::SharedBitmapId& id));
    MOCK_METHOD1(DidDeleteSharedBitmap, void(const viz::SharedBitmapId& id));
    MOCK_METHOD1(InitializeCompositorFrameSinkType, void(viz::mojom::CompositorFrameSinkType));
    MOCK_METHOD1(BindLayerContext, void(viz::mojom::blink::PendingLayerContextPtr));
...
class MockVideoFrameResourceProvider : public blink::VideoFrameResourceProvider {
public:
    MockVideoFrameResourceProvider(viz::RasterContextProvider* context_provider, viz::SharedBitmapReporter* shared_bitmap_reporter,
        scoped_refptr<gpu::ClientSharedImageInterface> shared_image_interface)
        : blink::VideoFrameResourceProvider(cc::LayerTreeSettings(), false)
    {
        blink::VideoFrameResourceProvider::Initialize(context_provider, shared_bitmap_reporter, shared_image_interface);
    }
    MockVideoFrameResourceProvider(const MockVideoFrameResourceProvider&) = delete;
    MockVideoFrameResourceProvider& operator=(const MockVideoFrameResourceProvider&) = delete;
    ~MockVideoFrameResourceProvider() override = default;

    MOCK_METHOD3(
        Initialize, void(viz::RasterContextProvider*, viz::SharedBitmapReporter*, scoped_refptr<gpu::ClientSharedImageInterface> shared_image_interface));
    MOCK_METHOD4(AppendQuads, void(viz::CompositorRenderPass*, scoped_refptr<media::VideoFrame>, media::VideoTransformation, bool));
    MOCK_METHOD0(ReleaseFrameResources, void());
    MOCK_METHOD2(PrepareSendToParent, void(const WebVector<viz::ResourceId>&, WebVector<viz::TransferableResource>*));
    MOCK_METHOD1(ReceiveReturnsFromParent, void(Vector<viz::ReturnedResource> transferable_resources));
...
class VideoFrameSubmitterTest : public testing::Test, public testing::WithParamInterface<bool> {
public:
    ...
    void MakeSubmitter()
    {
        MakeSubmitter(base::DoNothing());
    }

    void MakeSubmitter(cc::VideoPlaybackRoughnessReporter::ReportingCallback reporting_cb)
    {
        resource_provider_ = new StrictMock<MockVideoFrameResourceProvider>(context_provider_.get(), nullptr, nullptr);
        submitter_ = std::make_unique<VideoFrameSubmitter>(
            base::DoNothing(), reporting_cb, base::WrapUnique<MockVideoFrameResourceProvider>(resource_provider_.get()));
        ...
        sink_ = std::make_unique<StrictMock<VideoMockCompositorFrameSink>>(submitter_sink.InitWithNewPipeAndPassReceiver());
        ...
        submitter_->surface_embedder_.Bind(surface_embedder_->receiver_.BindNewPipeAndPassRemote());
        auto surface_id = viz::SurfaceId(viz::FrameSinkId(1, 1), viz::LocalSurfaceId(11, base::UnguessableToken::CreateForTesting(0x111111, 0)));
        submitter_->frame_sink_id_ = surface_id.frame_sink_id();
        submitter_->child_local_surface_id_allocator_.UpdateFromParent(surface_id.local_surface_id());
    }

    bool IsRendering() const
    {
        return submitter_->is_rendering_;
    }
    ...
    {
        return submitter_->ShouldSubmit();
    }
    ...
    const viz::ChildLocalSurfaceIdAllocator& child_local_surface_id_allocator() const
    {
    ...
    {
        task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE, task_environment_.QuitClosure());
        task_environment_.RunUntilQuit();
    }

    void OnReceivedContextProvider(bool use_gpu_compositing, scoped_refptr<viz::RasterContextProvider> context_provider,
        scoped_refptr<gpu::ClientSharedImageInterface> shared_image_interface)
    {
        submitter_->OnReceivedContextProvider(use_gpu_compositing, std::move(context_provider), std::move(shared_image_interface));
    }
    ...
    void OnBeginFrame(const viz::BeginFrameArgs& args, const WTF::HashMap<uint32_t, viz::FrameTimingDetails>& timing_details, bool frame_ack,
        WTF::Vector<viz::ReturnedResource> resources)
    {
        if (HasBeginFrameAcks() && frame_ack) {
            EXPECT_CALL(*resource_provider_, ReceiveReturnsFromParent(_));
        }
        submitter_->OnBeginFrame(args, timing_details, frame_ack, std::move(resources));
    }
...
#define EXPECT_GET_PUT_FRAME()                                                                                                                                 \
    do {                                                                                                                                                       \
        EXPECT_CALL(*video_frame_provider_, GetCurrentFrame())                                                                                                 \
            .WillOnce(Return(                                                                                                                                  \
                media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_YV12, gfx::Size(8, 8), gfx::Rect(gfx::Size(8, 8)), gfx::Size(8, 8), base::TimeDelta())));   \
...
#define EXPECT_SUBMISSION(type)                                                                                                                                \
    do {                                                                                                                                                       \
        if (type == SubmissionType::kBeginFrame) {                                                                                                             \
            EXPECT_CALL(*video_frame_provider_, UpdateCurrentFrame(_, _)).WillOnce(Return(true));                                                              \
        }                                                                                                                                                      \
...
TEST_P(VideoFrameSubmitterTest, StopRenderingSkipsUpdateCurrentFrame)
{
    ...
    viz::BeginFrameArgs args = begin_frame_source_->CreateBeginFrameArgs(BEGINFRAME_FROM_HERE, now_src_.get());
    ...
    args = begin_frame_source_->CreateBeginFrameArgs(BEGINFRAME_FROM_HERE, now_src_.get());
    OnBeginFrame(args, {}, false, WTF::Vector<viz::ReturnedResource>());
    task_environment_.RunUntilIdle();
}
...
TEST_P(VideoFrameSubmitterTest, StopUsingProviderSubmitsFrameAndStopsRendering)
{
    EXPECT_CALL(*sink_, SetNeedsBeginFrame(true));
    submitter_->StartRendering();
    task_environment_.RunUntilIdle();
...
TEST_P(VideoFrameSubmitterTest, ShouldSubmitPreventsSubmission)
{
    ...
    EXPECT_CALL(*video_frame_provider_, GetCurrentFrame())
        .WillOnce(
            Return(media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_YV12, gfx::Size(8, 8), gfx::Rect(gfx::Size(8, 8)), gfx::Size(8, 8), base::TimeDelta())));
...
TEST_P(VideoFrameSubmitterTest, RotationInformationPassedToResourceProvider)
{
    ...
    EXPECT_CALL(*sink_, DoSubmitCompositorFrame(_, _));
    EXPECT_CALL(*video_frame_provider_, PutCurrentFrame());
    EXPECT_CALL(*resource_provider_, AppendQuads(_, _, media::VideoTransformation(media::VideoRotation::VIDEO_ROTATION_90), _));
    EXPECT_CALL(*resource_provider_, PrepareSendToParent(_, _));
    EXPECT_CALL(*resource_provider_, ReleaseFrameResources());
    ...
    EXPECT_CALL(*sink_, DoSubmitCompositorFrame(_, _));
    EXPECT_CALL(*video_frame_provider_, PutCurrentFrame());
    EXPECT_CALL(*resource_provider_, AppendQuads(_, _, media::VideoTransformation(media::VideoRotation::VIDEO_ROTATION_180), _));
    EXPECT_CALL(*resource_provider_, PrepareSendToParent(_, _));
    EXPECT_CALL(*resource_provider_, ReleaseFrameResources());
    ...
    EXPECT_CALL(*sink_, DoSubmitCompositorFrame(_, _));
    EXPECT_CALL(*video_frame_provider_, PutCurrentFrame());
    EXPECT_CALL(*resource_provider_, AppendQuads(_, _, media::VideoTransformation(media::VideoRotation::VIDEO_ROTATION_270), _));
    EXPECT_CALL(*resource_provider_, PrepareSendToParent(_, _));
    EXPECT_CALL(*resource_provider_, ReleaseFrameResources());
...
TEST_P(VideoFrameSubmitterTest, FrameTransformTakesPrecedent)
{
    ...
    auto frame = media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_YV12, gfx::Size(8, 8), gfx::Rect(gfx::Size(8, 8)), gfx::Size(8, 8), base::TimeDelta());
    frame->metadata().transformation = media::VideoTransformation(media::VideoRotation::VIDEO_ROTATION_180, /*mirrored=*/true);

    EXPECT_CALL(*video_frame_provider_, UpdateCurrentFrame(_, _)).WillOnce(Return(true));
    EXPECT_CALL(*video_frame_provider_, GetCurrentFrame()).WillOnce(Return(frame));
    EXPECT_CALL(*sink_, DoSubmitCompositorFrame(_, _));
    EXPECT_CALL(*video_frame_provider_, PutCurrentFrame());
    EXPECT_CALL(*resource_provider_, AppendQuads(_, _, frame->metadata().transformation.value(), _));
    EXPECT_CALL(*resource_provider_, PrepareSendToParent(_, _));
    EXPECT_CALL(*resource_provider_, ReleaseFrameResources());
...
TEST_P(VideoFrameSubmitterTest, NoUpdateOnFrameDoesNotProduceFrame)
{
    ...
    EXPECT_CALL(*video_frame_provider_, UpdateCurrentFrame(_, _)).WillOnce(Return(false));
    EXPECT_CALL(*sink_, DidNotProduceFrame(_));
...
TEST_P(VideoFrameSubmitterTest, NotRenderingDoesNotProduceFrame)
{
    ...
    EXPECT_CALL(*video_frame_provider_, UpdateCurrentFrame(_, _)).Times(AnyNumber());
    EXPECT_CALL(*sink_, DidNotProduceFrame(_));
...
TEST_P(VideoFrameSubmitterTest, WaitingForAckPreventsNewFrame)
{
    ...
    std::unique_ptr<base::SimpleTestTickClock> new_time = std::make_unique<base::SimpleTestTickClock>();
    args = begin_frame_source_->CreateBeginFrameArgs(BEGINFRAME_FROM_HERE, new_time.get());
    OnBeginFrame(args, {}, false, WTF::Vector<viz::ReturnedResource>());
    task_environment_.RunUntilIdle();
}
...
TEST_P(VideoFrameSubmitterTest, RecreateCompositorFrameSinkAfterContextLost)
{
    ...
    mojo::ReceiverSet<mojom::blink::EmbeddedFrameSinkProvider> embedded_frame_sink_provider_receivers;
    auto override = mock_embedded_frame_sink_provider.CreateScopedOverrideMojoInterface(embedded_frame_sink_provider_receivers);

    EXPECT_CALL(*resource_provider_, Initialize(_, _, _));
    EXPECT_CALL(mock_embedded_frame_sink_provider, ConnectToEmbedder(_, _)).Times(0);
    EXPECT_CALL(mock_embedded_frame_sink_provider, CreateCompositorFrameSink_(_)).Times(1);
...
TEST_P(VideoFrameSubmitterTest, RecreateCompositorFrameSinkAfterContextLostSoftwareCompositing)
{
    MockEmbeddedFrameSinkProvider mock_embedded_frame_sink_provider;
    mojo::ReceiverSet<mojom::blink::EmbeddedFrameSinkProvider> embedded_frame_sink_provider_receivers;
    auto override = mock_embedded_frame_sink_provider.CreateScopedOverrideMojoInterface(embedded_frame_sink_provider_receivers);
...
{
    {
        viz::LocalSurfaceId local_surface_id = child_local_surface_id_allocator().GetCurrentLocalSurfaceId();
        EXPECT_TRUE(local_surface_id.is_valid());
        EXPECT_EQ(11u, local_surface_id.parent_sequence_number());
        EXPECT_EQ(viz::kInitialChildSequenceNumber, local_surface_id.child_sequence_number());
    ...
    EXPECT_CALL(*video_frame_provider_, GetCurrentFrame())
        .WillOnce(
            Return(media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_YV12, gfx::Size(2, 2), gfx::Rect(gfx::Size(2, 2)), gfx::Size(2, 2), base::TimeDelta())));
    EXPECT_CALL(*sink_, DoSubmitCompositorFrame(_, _));
    EXPECT_CALL(*video_frame_provider_, PutCurrentFrame());
    EXPECT_CALL(*resource_provider_, AppendQuads(_, _, _, _));
    EXPECT_CALL(*resource_provider_, PrepareSendToParent(_, _));
...

    {
        ...
        EXPECT_TRUE(local_surface_id.is_valid());
        EXPECT_EQ(11u, local_surface_id.parent_sequence_number());
        EXPECT_EQ(viz::kInitialChildSequenceNumber + 1, local_surface_id.child_sequence_number());
...
TEST_P(VideoFrameSubmitterTest, VideoRotationOutputRect)
{
...

    {
        ...
        EXPECT_CALL(*video_frame_provider_, UpdateCurrentFrame(_, _)).WillOnce(Return(true));
        EXPECT_CALL(*video_frame_provider_, GetCurrentFrame())
            .WillOnce(Return(media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_YV12, coded_size, gfx::Rect(coded_size), natural_size, base::TimeDelta())));
        EXPECT_CALL(*sink_, DoSubmitCompositorFrame(_, _));
        EXPECT_CALL(*video_frame_provider_, PutCurrentFrame());
        EXPECT_CALL(*resource_provider_, AppendQuads(_, _, media::VideoTransformation(media::VideoRotation::VIDEO_ROTATION_90), _));
        EXPECT_CALL(*resource_provider_, PrepareSendToParent(_, _));
        ...
        EXPECT_EQ(sink_->last_submitted_compositor_frame().size_in_pixels(), rotated_size);
...

    {
        ...
        EXPECT_EQ(sink_->last_submitted_compositor_frame().size_in_pixels(), natural_size);
...
TEST_P(VideoFrameSubmitterTest, PreferredInterval)
{
    ...
    EXPECT_EQ(sink_->last_submitted_compositor_frame().metadata.begin_frame_ack.preferred_frame_interval, video_frame_provider_->preferred_interval);
    const auto& frame_interval_inputs = sink_->last_submitted_compositor_frame().metadata.frame_interval_inputs;
    ASSERT_EQ(frame_interval_inputs.content_interval_info.size(), 1u);
    EXPECT_EQ(frame_interval_inputs.content_interval_info[0].frame_interval, video_frame_provider_->preferred_interval);
    EXPECT_EQ(frame_interval_inputs.content_interval_info[0].type, viz::ContentFrameIntervalType::kVideo);
...
TEST_P(VideoFrameSubmitterTest, NoDuplicateFramesOnBeginFrame)
{
    ...
    auto vf = media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_YV12, gfx::Size(8, 8), gfx::Rect(gfx::Size(8, 8)), gfx::Size(8, 8), base::TimeDelta());
...
TEST_P(VideoFrameSubmitterTest, ProcessTimingDetails)
{
    ...
    int frames_to_run = fps * (cc::VideoPlaybackRoughnessReporter::kMinWindowsBeforeSubmit + 1);
    WTF::HashMap<uint32_t, viz::FrameTimingDetails> timing_details;

    MakeSubmitter(base::BindLambdaForTesting([&](const cc::VideoPlaybackRoughnessReporter::Measurement& measurement) {
    ...
    auto sink_submit = [&](const viz::LocalSurfaceId&, viz::CompositorFrame* frame) {
        auto token = frame->metadata.frame_token;
        viz::FrameTimingDetails details;
        details.presentation_feedback.timestamp = base::TimeTicks() + frame_duration * token;
        details.presentation_feedback.flags = gfx::PresentationFeedback::kHWCompletion;
        timing_details.clear();
        timing_details.Set(token, details);
    };

    EXPECT_CALL(*video_frame_provider_, UpdateCurrentFrame).WillRepeatedly(Return(true));
    EXPECT_CALL(*video_frame_provider_, PutCurrentFrame).Times(AnyNumber());
    EXPECT_CALL(*sink_, DoSubmitCompositorFrame).WillRepeatedly(Invoke(sink_submit));
    ...
    for (int i = 0; i < frames_to_run; i++) {
        auto frame = media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_YV12, gfx::Size(8, 8), gfx::Rect(gfx::Size(8, 8)), gfx::Size(8, 8), i * frame_duration);
        frame->metadata().wallclock_frame_duration = frame_duration;
        EXPECT_CALL(*video_frame_provider_, GetCurrentFrame()).WillRepeatedly(Return(frame));

        auto args = begin_frame_source_->CreateBeginFrameArgs(BEGINFRAME_FROM_HERE, now_src_.get());
        OnBeginFrame(args, timing_details, false, WTF::Vector<viz::ReturnedResource>());
        task_environment_.RunUntilIdle();
        AckSubmittedFrame();
    }
    submitter_->StopRendering();
...
TEST_P(VideoFrameSubmitterTest, OpaqueFramesNotifyEmbedder)
{
    ...
    EXPECT_CALL(*resource_provider_, PrepareSendToParent(_, _)).Times(AnyNumber());
    EXPECT_CALL(*resource_provider_, ReleaseFrameResources()).Times(AnyNumber());

    // Send an opaque frame, and expect a callback immediately.
    EXPECT_CALL(*video_frame_provider_, GetCurrentFrame()).WillOnce(Return(media::VideoFrame::CreateBlackFrame(size)));
    ...
    EXPECT_CALL(*video_frame_provider_, GetCurrentFrame()).WillOnce(Return(media::VideoFrame::CreateTransparentFrame(size)));
...
INSTANTIATE_TEST_SUITE_P(, VideoFrameSubmitterTest, testing::Bool(), [](auto& info) { return info.param ? "BeginFrameAcks" : "CompositorFrameAcks"; });
