...
TEST(CanvasColorParamsTest, MatchSkColorSpaceWithGfxColorSpace)
{
    ...
    for (PredefinedColorSpace color_space : canvas_color_spaces) {
        CanvasColorParams color_params(color_space, CanvasPixelFormat::kF16, kNonOpaque);
        sk_sp<SkColorSpace> canvas_drawing_color_space = color_params.GetSkColorSpace();
        sk_sp<SkColorSpace> canvas_media_color_space = color_params.GetStorageGfxColorSpace().ToSkColorSpace();
        ASSERT_TRUE(ColorCorrectionTestUtils::MatchColorSpace(canvas_drawing_color_space, canvas_media_color_space));
    }
}
