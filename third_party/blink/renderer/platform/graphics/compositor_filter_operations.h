...
public:
    ...
    void AppendBlurFilter(float amount, SkTileMode tile_mode = SkTileMode::kDecal);
    void AppendDropShadowFilter(gfx::Vector2d offset, float std_deviation, const Color& color);
    ...
    size_t size() const
    {
        return filter_operations_.size();
    }
    ...
    void SetReferenceBox(const gfx::RectF& r)
    {
        reference_box_ = r;
    }
    const gfx::RectF& ReferenceBox() const
    {
        return reference_box_;
    }
