...
BitmapImageMetrics::DecodedImageType BitmapImageMetrics::StringToDecodedImageType(const String& type)
{
    if (type == "jpg")
...
{
    UMA_HISTOGRAM_ENUMERATION("Blink.DecodedImageType", StringToDecodedImageType(type));
}

void BitmapImageMetrics::CountDecodedImageType(const String& type, UseCounter* use_counter)
{
    if (use_counter) {
        if (type == "webp") {
...
void BitmapImageMetrics::CountDecodedImageDensity(const String& type, int image_min_side, uint64_t density_centi_bpp, size_t image_size_bytes)
{
    ...
    DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, jpeg_density_histogram, ("Blink.DecodedImage.JpegDensity.KiBWeighted", 1, 1000, 100));
    DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, webp_density_histogram, ("Blink.DecodedImage.WebPDensity.KiBWeighted2", 1, 1000, 100));
#if BUILDFLAG(ENABLE_AV1_DECODER)
    DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, avif_density_histogram, ("Blink.DecodedImage.AvifDensity.KiBWeighted2", 1, 1000, 100));
#endif

    CustomCountHistogram* density_histogram = nullptr;
    BitmapImageMetrics::DecodedImageType decoded_image_type = StringToDecodedImageType(type);
    ...
    density_histogram->CountMany(base::saturated_cast<base::Histogram::Sample>(density_centi_bpp), image_size_kib);
}
