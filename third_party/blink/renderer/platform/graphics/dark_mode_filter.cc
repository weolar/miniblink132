...
{
    static bool enabled = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableRasterSideDarkModeForImages);
    return enabled;
}
...
sk_sp<cc::ColorFilter> GetDarkModeFilterForImageOnMainThread(DarkModeFilter* filter, Image* image, const SkIRect& rounded_src)
{
    ...
    if (cache->Exists(rounded_src)) {
        ...
        SkBitmap bitmap = image->AsSkBitmapForCurrentFrame(kDoNotRespectImageOrientation);
        SkPixmap pixmap;
    ...
    , image_filter(nullptr)
{
    ...
    foreground_classifier = DarkModeColorClassifier::MakeForegroundColorClassifier(settings);
    background_classifier = DarkModeColorClassifier::MakeBackgroundColorClassifier(settings);
    image_classifier = std::make_unique<DarkModeImageClassifier>(settings.image_classifier_policy);
}
...
SkColor4f DarkModeFilter::AdjustDarkenColor(const SkColor4f& color, DarkModeFilter::ElementRole role, const SkColor4f& contrast_background)
{
    ...
    case ElementRole::kBorder: {
        ...
        if (color_utils::GetContrastRatio(color, background) < color_utils::kMinimumReadableContrastRatio)
            return color;

        return AdjustDarkenColor(Color::FromSkColor4f(color).Dark().toSkColor4f(), role, background);
    }
    case ElementRole::kSelection: {
        if (!immutable_.color_filter)
            return color;

        return immutable_.color_filter->AdjustColorForHigherConstrast(color, background, color_utils::kMinimumVisibleContrastRatio);
    }
    default:
        return color;
    }
...
SkColor4f DarkModeFilter::InvertColorIfNeeded(const SkColor4f& color, ElementRole role, const SkColor4f& contrast_background)
{
    return AdjustDarkenColor(InvertColorIfNeeded(color, role), role, InvertColorIfNeeded(contrast_background, ElementRole::kBackground));
}

SkColor4f DarkModeFilter::InvertColorIfNeeded(const SkColor4f& color, ElementRole role)
{
    if (!immutable_.color_filter)
        return color;

    if (ShouldApplyToColor(color, role)) {
        return inverted_color_cache_->GetInvertedColor(immutable_.color_filter.get(), color);
    }

    return color;
}

void DarkModeFilter::ApplyFilterToImage(Image* image, cc::PaintFlags* flags, const SkRect& src)
{
    DCHECK(image);
    DCHECK(flags);
    ...
    sk_sp<cc::ColorFilter> color_filter = GetDarkModeFilterForImageOnMainThread(this, image, src.roundOut());
    if (color_filter)
...
sk_sp<cc::ColorFilter> DarkModeFilter::GenerateImageFilter(const SkPixmap& pixmap, const SkIRect& src) const
{
    DCHECK(immutable_.settings.image_policy == DarkModeImagePolicy::kFilterSmart);
    DCHECK(immutable_.image_filter);

    return (immutable_.image_classifier->Classify(pixmap, src) == DarkModeResult::kApplyFilter) ? immutable_.image_filter : nullptr;
}
...
std::optional<cc::PaintFlags> DarkModeFilter::ApplyToFlagsIfNeeded(const cc::PaintFlags& flags, ElementRole role, SkColor4f contrast_background)
{
    ...
    if (ShouldApplyToColor(flags_color, role)) {
        flags_color = inverted_color_cache_->GetInvertedColor(immutable_.color_filter.get(), flags_color);
    }
    dark_mode_flags.setColor(AdjustDarkenColor(flags_color, role, InvertColorIfNeeded(contrast_background, ElementRole::kBackground)));

    return std::make_optional<cc::PaintFlags>(std::move(dark_mode_flags));
}

bool DarkModeFilter::ShouldApplyToColor(const SkColor4f& color, ElementRole role)
{
    switch (role) {
    case ElementRole::kBorder:
    ...
    case ElementRole::kListSymbol:
        ...
        return immutable_.foreground_classifier->ShouldInvertColor(color.toSkColor()) == DarkModeResult::kApplyFilter;
    case ElementRole::kBackground:
    case ElementRole::kSelection:
        DCHECK(immutable_.background_classifier);
        return immutable_.background_classifier->ShouldInvertColor(color.toSkColor()) == DarkModeResult::kApplyFilter;
    default:
        return false;
    }
    NOTREACHED();
