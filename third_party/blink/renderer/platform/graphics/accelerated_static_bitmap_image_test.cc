...
scoped_refptr<StaticBitmapImage> CreateBitmap()
{
    ...
    return AcceleratedStaticBitmapImage::CreateFromCanvasSharedImage(std::move(client_si), GenTestSyncToken(100), 0, SkImageInfo::MakeN32Premul(100, 100),
        GL_TEXTURE_2D, true, SharedGpuContext::ContextProviderWrapper(), base::PlatformThread::CurrentRef(), base::MakeRefCounted<base::NullTaskRunner>(),
        base::DoNothing(),
...
TEST_F(AcceleratedStaticBitmapImageTest, CopyToTextureSynchronization)
{
    ...
    // dest context.
    EXPECT_CALL(destination_gl, WaitSyncTokenCHROMIUM(Pointee(SyncTokenMatcher(bitmap->GetMailboxHolder().sync_token)))).Times(testing::Between(1, 2));
    ...
    EXPECT_CALL(destination_gl, GenSyncTokenCHROMIUM(_))
        .WillOnce(SetArrayArgument<0>(sync_token2.GetConstData(), sync_token2.GetConstData() + sizeof(gpu::SyncToken)));

    gfx::Point dest_point(0, 0);
    gfx::Rect source_sub_rectangle(0, 0, 10, 10);
    ASSERT_TRUE(bitmap->CopyToTexture(&destination_gl, GL_TEXTURE_2D, 1 /*dest_texture_id*/, 0 /*dest_texture_level*/, false /*unpack_premultiply_alpha*/,
