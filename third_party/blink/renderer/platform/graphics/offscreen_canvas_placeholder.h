...
public:
    ...
    virtual void SetOffscreenCanvasResource(scoped_refptr<CanvasResource>&&, viz::ResourceId resource_id);
    void SetOffscreenCanvasDispatcher(base::WeakPtr<CanvasResourceDispatcher>, scoped_refptr<base::SingleThreadTaskRunner>);

    void SetSuspendOffscreenCanvasAnimation(bool);

    static OffscreenCanvasPlaceholder* GetPlaceholderCanvasById(unsigned placeholder_id);
    ...
    void UpdateOffscreenCanvasFilterQuality(cc::PaintFlags::FilterQuality filter_quality);

    virtual bool HasCanvasCapture() const
    {
        return false;
    }
