...
class CanvasHibernationHandlerTest : public testing::TestWithParam<CanvasHibernationHandler::CompressionAlgorithm> {
public:
...
void SetPageVisible(FakeCanvasResourceHost* host, CanvasHibernationHandler* hibernation_handler,
    ScopedTestingPlatformSupport<GpuMemoryBufferTestPlatform>& platform, bool page_visible)
{
    ...
    if (!page_visible) {
        // Trigger hibernation.
        scoped_refptr<StaticBitmapImage> snapshot = host->ResourceProvider()->Snapshot(FlushReason::kHibernating);
        hibernation_handler->SaveForHibernation(snapshot->PaintImageForCurrentFrame().GetSwSkImage(), host->ResourceProvider()->ReleaseRecorder());
...
std::map<std::string, uint64_t> GetEntries(const base::trace_event::MemoryAllocatorDump& dump)
{
    std::map<std::string, uint64_t> result;
    for (const auto& entry : dump.entries()) {
        CHECK(entry.entry_type == base::trace_event::MemoryAllocatorDump::Entry::kUint64);
...
void Draw(CanvasResourceHost& host)
{
    CanvasResourceProvider* provider
        = host.GetOrCreateCanvasResourceProvider(host.GetRasterMode() == RasterMode::kGPU ? RasterModeHint::kPreferGPU : RasterModeHint::kPreferCPU);
...
public:
    ...
    std::list<base::OnceClosure>& delayed()
    {
        return delayed_;
    }
    std::list<base::OnceClosure>& immediate()
    {
        return immediate_;
    }
...
INSTANTIATE_TEST_SUITE_P(CompressionAlgorithm, CanvasHibernationHandlerTest,
    ::testing::Values(CanvasHibernationHandler::CompressionAlgorithm::kZlib, CanvasHibernationHandler::CompressionAlgorithm::kZstd));
...
TEST_P(CanvasHibernationHandlerTest, SimpleTest)
{
    ...
    histogram_tester.ExpectTotalCount("Blink.Canvas.2DLayerBridge.Compression.Ratio", 1);
    histogram_tester.ExpectTotalCount("Blink.Canvas.2DLayerBridge.Compression.ThreadTime", 1);
    histogram_tester.ExpectUniqueSample("Blink.Canvas.2DLayerBridge.Compression.SnapshotSizeKb", uncompressed_size / 1024, 1);
    histogram_tester.ExpectTotalCount("Blink.Canvas.2DLayerBridge.Compression.DecompressionTime", 0);

    // It should be possible to decompress the encoded image.
    EXPECT_TRUE(handler.GetImage());
    histogram_tester.ExpectTotalCount("Blink.Canvas.2DLayerBridge.Compression.DecompressionTime", 1);
...
TEST_P(CanvasHibernationHandlerTest, HibernationMemoryMetrics)
{
    ...
    base::trace_event::MemoryDumpArgs args = { base::trace_event::MemoryDumpLevelOfDetail::kDetailed };
    {
        ...
        EXPECT_TRUE(HibernatedCanvasMemoryDumpProvider::GetInstance().OnMemoryDump(args, &pmd));
