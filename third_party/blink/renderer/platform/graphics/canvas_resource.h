...
class PLATFORM_EXPORT CanvasResource : public WTF::ThreadSafeRefCounted<CanvasResource> {
public:
    using ReleaseCallback = base::OnceCallback<void(scoped_refptr<blink::CanvasResource>&& canvas_resource, const gpu::SyncToken& sync_token, bool is_lost)>;

    using LastUnrefCallback = base::OnceCallback<void(scoped_refptr<blink::CanvasResource> canvas_resource)>;
    ...
    bool HasLastUnrefCallback()
    {
        return !!last_unref_callback_;
    }
    ...
    virtual bool UsesClientSharedImage()
    {
        return false;
    }
    ...
    // with the compositor.
    bool PrepareTransferableResource(viz::TransferableResource*, ReleaseCallback*, bool needs_verified_synctoken);
    ...
    virtual bool HasDetailedMemoryDumpProvider() const
    {
        return false;
    }

protected:
    CanvasResource(base::WeakPtr<CanvasResourceProvider>, cc::PaintFlags::FilterQuality, const SkColorInfo&);

    // Returns true if the resource is backed by memory such that it can be used
    // for direct scanout by the display.
    virtual bool IsOverlayCandidate() const
    {
        return false;
    }
    ...
    virtual base::WeakPtr<WebGraphicsContext3DProviderWrapper> ContextProviderWrapper() const = 0;
    ...
    // true.
    bool PrepareAcceleratedTransferableResourceFromClientSI(viz::TransferableResource* out_resource, bool needs_verified_synctoken);
    ...
    // method.
    virtual bool PrepareAcceleratedTransferableResourceWithoutClientSI(viz::TransferableResource* out_resource)
    {
        NOTREACHED();
    }
    ...
    virtual bool PrepareUnacceleratedTransferableResource(viz::TransferableResource* out_resource)
    {
        NOTREACHED();
    }
    const SkColorInfo& GetSkColorInfo() const
    {
        return info_;
    }
    ...
    base::WeakPtr<CanvasResourceProvider> WeakProvider()
    {
        return provider_;
    }
...
private:
    ...
    // verified.
    virtual const gpu::SyncToken GetSyncTokenWithOptionalVerification(bool needs_verified_token)
    {
        NOTREACHED();
    }
...
public:
    static scoped_refptr<CanvasResourceSharedBitmap> Create(
        const SkImageInfo&, base::WeakPtr<CanvasResourceProvider>, base::WeakPtr<WebGraphicsSharedImageInterfaceProvider>, cc::PaintFlags::FilterQuality);
    ~CanvasResourceSharedBitmap() override;
    bool IsRecycleable() const final
    {
        return IsValid();
    }
    bool IsValid() const final;
    bool SupportsAcceleratedCompositing() const final
    {
        return false;
    }
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> ContextProviderWrapper() const override
    {
        return nullptr;
    }
    bool PrepareUnacceleratedTransferableResource(viz::TransferableResource* out_resource) final;
    ...
    {
        return is_origin_clean_;
    }
    void SetOriginClean(bool flag) final
    {
        is_origin_clean_ = flag;
    }
...
private:
    ...
    bool UsesAcceleratedRaster() const final
    {
        return false;
    }
...
public:
    static scoped_refptr<CanvasResourceSharedImage> Create(const SkImageInfo&, base::WeakPtr<WebGraphicsContext3DProviderWrapper>,
        base::WeakPtr<CanvasResourceProvider>, cc::PaintFlags::FilterQuality, bool is_accelerated, gpu::SharedImageUsageSet shared_image_usage_flags);
    ...
    bool IsLost() const
    {
        return owning_thread_data().is_lost;
    }
    void CopyRenderingResultsToGpuMemoryBuffer(const sk_sp<SkImage>& image);
    bool UsesClientSharedImage() override
    {
        return true;
    }
    ...
    void OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, const std::string& parent_path, size_t bytes_per_pixel) const;
    ...
    // double-countintg.
    bool HasDetailedMemoryDumpProvider() const override
    {
        return true;
    }
...
private:
    ...
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> ContextProviderWrapper() const override;
    const gpu::SyncToken GetSyncTokenWithOptionalVerification(bool needs_verified_token) override;
    ...
    CanvasResourceSharedImage(const SkImageInfo&, base::WeakPtr<WebGraphicsContext3DProviderWrapper>, base::WeakPtr<CanvasResourceProvider>,
        cc::PaintFlags::FilterQuality, bool is_accelerated, gpu::SharedImageUsageSet shared_image_usage_flags);
...
public:
    ...
    static scoped_refptr<ExternalCanvasResource> Create(scoped_refptr<gpu::ClientSharedImage> client_si, const viz::TransferableResource& transferable_resource,
        viz::ReleaseCallback release_callback, base::WeakPtr<WebGraphicsContext3DProviderWrapper>, base::WeakPtr<CanvasResourceProvider>,
        cc::PaintFlags::FilterQuality, bool is_origin_top_left);
    ...
    gfx::Size Size() const final
    {
        return transferable_resource_.size;
    }
...
private:
    ...
    const gpu::SyncToken GetSyncTokenWithOptionalVerification(bool needs_verified_token) override;
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> ContextProviderWrapper() const override;
    bool PrepareAcceleratedTransferableResourceWithoutClientSI(viz::TransferableResource* out_resource) override;
    void GenOrFlushSyncToken();

    ExternalCanvasResource(scoped_refptr<gpu::ClientSharedImage> client_si, const viz::TransferableResource& transferable_resource,
        viz::ReleaseCallback out_callback, base::WeakPtr<WebGraphicsContext3DProviderWrapper>, base::WeakPtr<CanvasResourceProvider>,
        cc::PaintFlags::FilterQuality, bool is_origin_top_left);

    scoped_refptr<gpu::ClientSharedImage> client_si_;
    const base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper_;
...
public:
    static scoped_refptr<CanvasResourceSwapChain> Create(
        const SkImageInfo&, base::WeakPtr<WebGraphicsContext3DProviderWrapper>, base::WeakPtr<CanvasResourceProvider>, cc::PaintFlags::FilterQuality);
    ...
    GLuint GetBackBufferTextureId() const
    {
