...
public:
    ...
    BoxReflection(ReflectionDirection direction, float offset, PaintRecord mask, const gfx::RectF& mask_bounds);
    ...
    const PaintRecord& Mask() const
    {
        return mask_;
    }
    const gfx::RectF& MaskBounds() const
    {
        return mask_bounds_;
    }
...
inline bool operator==(const BoxReflection& a, const BoxReflection& b)
{
    return a.Direction() == b.Direction() && a.Offset() == b.Offset() &&
        ...
        a.Mask().empty() && b.Mask().empty() && a.MaskBounds() == b.MaskBounds();
}
