...
// static
std::unique_ptr<PlatformPaintWorkletLayerPainter> PaintWorkletPaintDispatcher::CreateCompositorThreadPainter(
    base::WeakPtr<PaintWorkletPaintDispatcher>* paint_dispatcher)
{
    ...
    *paint_dispatcher = dispatcher->GetWeakPtr();
    ...
    return std::make_unique<PlatformPaintWorkletLayerPainter>(std::move(dispatcher));
}
...
void PaintWorkletPaintDispatcher::RegisterPaintWorkletPainter(PaintWorkletPainter* painter, scoped_refptr<base::SingleThreadTaskRunner> painter_runner)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    TRACE_EVENT0("cc", "PaintWorkletPaintDispatcher::RegisterPaintWorkletPainter");
...
void PaintWorkletPaintDispatcher::UnregisterPaintWorkletPainter(int worklet_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    TRACE_EVENT0("cc",
...
void PaintWorkletPaintDispatcher::DispatchWorklets(cc::PaintWorkletJobMap worklet_job_map, PlatformPaintWorkletLayerPainter::DoneCallback done_callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    scoped_refptr<base::SingleThreadTaskRunner> runner = GetCompositorTaskRunner();
    WTF::CrossThreadClosure on_done = CrossThreadBindRepeating(
        [](base::WeakPtr<PaintWorkletPaintDispatcher> dispatcher, scoped_refptr<base::SingleThreadTaskRunner> runner) {
            PostCrossThreadTask(*runner, FROM_HERE, CrossThreadBindOnce(&PaintWorkletPaintDispatcher::AsyncPaintDone, dispatcher));
        },
    ...
    base::RepeatingClosure repeating_on_done = base::BarrierClosure(ongoing_jobs_.size(), ConvertToBaseRepeatingCallback(std::move(on_done)));
    ...
    for (auto& job : ongoing_jobs_) {
        ...
        auto on_done_runner = std::make_unique<base::ScopedClosureRunner>(repeating_on_done);
        ...
        if (task_runner) {
            ...
            PostCrossThreadTask(*task_runner, FROM_HERE,
                CrossThreadBindOnce(
                    [](PaintWorkletPainter* painter, scoped_refptr<cc::PaintWorkletJobVector> jobs, std::unique_ptr<base::ScopedClosureRunner> on_done_runner) {
                        for (cc::PaintWorkletJob& job : jobs->data) {
                            job.SetOutput(painter->Paint(job.input().get(), job.GetAnimatedPropertyValues()));
                        }
                        on_done_runner->RunAndReset();
                    },
                    WrapCrossThreadPersistent(painter), std::move(jobs), std::move(on_done_runner)));
        } else {
            ...
            for (cc::PaintWorkletJob& native_job : jobs->data) {
                native_job.SetOutput(painter->Paint(native_job.input().get(), native_job.GetAnimatedPropertyValues()));
            }
            on_done_runner->RunAndReset();
        }
    }
...
scoped_refptr<base::SingleThreadTaskRunner> PaintWorkletPaintDispatcher::GetCompositorTaskRunner()
{
