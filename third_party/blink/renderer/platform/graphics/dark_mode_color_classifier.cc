...
public:
    ...
    {
        return std::unique_ptr<SimpleColorClassifier>(new SimpleColorClassifier(DarkModeResult::kDoNotApplyFilter));
    }

    static std::unique_ptr<SimpleColorClassifier> AlwaysInvert()
    {
        return std::unique_ptr<SimpleColorClassifier>(new SimpleColorClassifier(DarkModeResult::kApplyFilter));
    }
...
private:
    explicit SimpleColorClassifier(DarkModeResult value)
        : value_(value)
    {
    }
...
std::unique_ptr<DarkModeColorClassifier> DarkModeColorClassifier::MakeForegroundColorClassifier(const DarkModeSettings& settings)
{
    ...
    return std::make_unique<InvertLowBrightnessColorsClassifier>(settings.foreground_brightness_threshold);
}

std::unique_ptr<DarkModeColorClassifier> DarkModeColorClassifier::MakeBackgroundColorClassifier(const DarkModeSettings& settings)
{
    ...
    return std::make_unique<InvertHighBrightnessColorsClassifier>(settings.background_brightness_threshold);
}
