...
public:
    static std::unique_ptr<ColorFilterWrapper> Create(sk_sp<cc::ColorFilter> color_filter)
    {
        return std::unique_ptr<ColorFilterWrapper>(new ColorFilterWrapper(color_filter));
    }

    static std::unique_ptr<ColorFilterWrapper> Create(SkHighContrastConfig::InvertStyle invert_style, const DarkModeSettings& settings)
    {
        ...
        return std::unique_ptr<ColorFilterWrapper>(new ColorFilterWrapper(cc::ColorFilter::MakeHighContrast(config)));
    }
    ...
    sk_sp<cc::ColorFilter> ToColorFilter() const override
    {
        return filter_;
    }
...
public:
    ...
    SkColor4f AdjustColorForHigherConstrast(const SkColor4f& adjusted_color, const SkColor4f& background, float reference_contrast_ratio) override
    {
        if (color_utils::GetContrastRatio(adjusted_color, background) >= reference_contrast_ratio)
...
private:
    ...
    SkColor4f AdjustGray(const SkColor4f& color) const
    {
        ...
        if (IsWithinEpsilon(r, g) && IsWithinEpsilon(r, b) && r < kBrightnessThreshold && r > kAdjustedBrightness) {
            return SkColor4f { kAdjustedBrightness, kAdjustedBrightness, kAdjustedBrightness, color.fA };
        }

        return color;
    }

    SkColor4f AdjustColorByLightness(const SkColor4f& reference_color, int lightness)
    {
...
std::unique_ptr<DarkModeColorFilter> DarkModeColorFilter::FromSettings(const DarkModeSettings& settings)
{
    ...
    case DarkModeInversionAlgorithm::kSimpleInvertForTesting:
        ...
        return ColorFilterWrapper::Create(cc::ColorFilter::MakeTableARGB(identity.data(), invert.data(), invert.data(), invert.data()));

    case DarkModeInversionAlgorithm::kInvertBrightness:
        return ColorFilterWrapper::Create(SkHighContrastConfig::InvertStyle::kInvertBrightness, settings);

    case DarkModeInversionAlgorithm::kInvertLightness:
        return ColorFilterWrapper::Create(SkHighContrastConfig::InvertStyle::kInvertLightness, settings);
