...
public:
    ...
    static scoped_refptr<StaticBitmapImage> Create(PaintImage, ImageOrientation = ImageOrientationEnum::kDefault);
    static scoped_refptr<StaticBitmapImage> Create(sk_sp<SkData> data, const SkImageInfo&, ImageOrientation = ImageOrientationEnum::kDefault);

    StaticBitmapImage(ImageOrientation orientation)
        : orientation_(orientation)
    {
    }

    bool IsStaticBitmapImage() const override
    {
        return true;
    }
    ...
    virtual bool SupportsDisplayCompositing() const
    {
        return true;
    }
    ...
    // non-gpu.
    virtual scoped_refptr<StaticBitmapImage> MakeUnaccelerated()
    {
        return this;
    }
    ...
    virtual bool CopyToTexture(gpu::gles2::GLES2Interface*, GLenum, GLuint, GLint, bool, bool, const gfx::Point&, const gfx::Rect&)
    {
        NOTREACHED();
    }

    virtual bool CopyToResourceProvider(CanvasResourceProvider* resource_provider, const gfx::Rect& copy_rect) = 0;

    virtual void EnsureSyncTokenVerified()
    {
        NOTREACHED();
    }
    ...
    {
        return GetSkImageInfo().colorInfo();
    }
    ...
    // information.
    Vector<uint8_t> CopyImageData(const SkImageInfo& info, bool apply_orientation);
...
protected:
    ...
    void DrawHelper(cc::PaintCanvas*, const cc::PaintFlags&, const gfx::RectF&, const gfx::RectF&, const ImageDrawOptions&, const PaintImage&);
...
template <> struct DowncastTraits<StaticBitmapImage> {
