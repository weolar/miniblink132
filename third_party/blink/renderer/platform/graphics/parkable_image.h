...
class PLATFORM_EXPORT ParkableImageImpl final : public ThreadSafeRefCounted<ParkableImageImpl> {
public:
...
private:
    ...
    template <typename T, typename... Args> friend scoped_refptr<T> base::MakeRefCounted(Args&&... args);
    ...
    void Append(WTF::SharedBuffer* buffer, size_t offset = 0) LOCKS_EXCLUDED(lock_);
    ...
    bool MaybePark(scoped_refptr<base::SingleThreadTaskRunner> task_runner) LOCKS_EXCLUDED(lock_);
    ...
    static void WriteToDiskInBackground(scoped_refptr<ParkableImageImpl>, scoped_refptr<base::SingleThreadTaskRunner> callback_task_runner)
        LOCKS_EXCLUDED(lock_);

    // Writes the data referred to by |on_disk_metadata| from disk into the
    // provided |buffer|.
    static size_t ReadFromDiskIntoBuffer(DiskDataMetadata* on_disk_metadata, base::span<uint8_t> buffer);
...
class PLATFORM_EXPORT ParkableImage final : public ThreadSafeRefCounted<ParkableImage> {
public:
    ...
    void Append(WTF::SharedBuffer* buffer, size_t offset = 0) LOCKS_EXCLUDED(impl_->lock_);
