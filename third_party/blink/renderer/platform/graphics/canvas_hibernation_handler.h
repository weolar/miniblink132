...
public:
    ...
    void SaveForHibernation(sk_sp<SkImage>&& image, std::unique_ptr<MemoryManagedPaintRecorder> recorder);
    ...
    void SetTaskRunnersForTesting(
        scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner, scoped_refptr<base::SingleThreadTaskRunner> background_thread_task_runner)
    {
    ...
    void SetOnEncodedCallbackForTesting(base::RepeatingClosure on_encoded_callback)
    {
...
private:
    struct BackgroundTaskParams final {
        BackgroundTaskParams(sk_sp<SkImage> image, uint64_t epoch, CompressionAlgorithm algorithm, base::WeakPtr<CanvasHibernationHandler> weak_instance,
    ...
    void OnEncoded(std::unique_ptr<CanvasHibernationHandler::BackgroundTaskParams> params, sk_sp<SkData> encoded);
    ...
    scoped_refptr<base::SingleThreadTaskRunner> background_thread_task_runner_for_testing_;
...
// only.
class PLATFORM_EXPORT HibernatedCanvasMemoryDumpProvider : public base::trace_event::MemoryDumpProvider {
public:
