...
public:
    WebGraphicsContext3DProviderForTests(std::unique_ptr<gpu::gles2::GLES2Interface> gl)
        : gl_(std::move(gl))
        , test_shared_image_interface_(base::MakeRefCounted<gpu::TestSharedImageInterface>())
    {
    }
    WebGraphicsContext3DProviderForTests(std::unique_ptr<gpu::webgpu::WebGPUInterface> webgpu)
    ...
    void SetErrorMessageCallback(base::RepeatingCallback<void(const char*, int32_t id)>) override
    {
    }
    ...
    void CopyVideoFrame(media::PaintCanvasVideoRenderer* video_render, media::VideoFrame* video_frame, cc::PaintCanvas* canvas) override
    {
    }
    ...
    gpu::GpuFeatureInfo& GetMutableGpuFeatureInfo()
    {
        return gpu_feature_info_;
    }
...
class GLES2InterfaceForTests : public gpu::gles2::GLES2InterfaceStub, public DrawingBuffer::Client {
public:
    ...
    void ClearDepthf(GLfloat depth) override
    {
        state_.clear_depth = depth;
    }
        ...
        GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const void* pixels) override
    {
        if (target == GL_TEXTURE_2D && !level) {
            texture_sizes_.Set(bound_textures_.find(target)->value, gfx::Size(width, height));
        }
    }
    ...
    MOCK_METHOD1(CreateAndTexStorage2DSharedImageCHROMIUMMock, void(const GLbyte*));
    GLuint CreateAndTexStorage2DSharedImageCHROMIUM(const GLbyte* mailbox) override
    {
        ...
        last_imported_shared_image_.SetName(reinterpret_cast<const gpu::Mailbox*>(mailbox)->name);
        return texture_id;
    }
    ...
    void GenSyncTokenCHROMIUM(GLbyte* sync_token) override
    {
        ...
        gpu::SyncToken source(gpu::GPU_IO, command_buffer_id_generator.GenerateNextId(), 2);
    ...
    {
        memcpy(&most_recently_waited_sync_token_, sync_token, sizeof(most_recently_waited_sync_token_));
    ...
    {
        memcpy(state_.color_mask, saved_state_.color_mask, sizeof(state_.color_mask));
        state_.clear_depth = saved_state_.clear_depth;
        state_.clear_stencil = saved_state_.clear_stencil;

        memcpy(state_.clear_color, saved_state_.clear_color, sizeof(state_.clear_color));
    ...
    {
        state_.active_texturecubemap_binding = saved_state_.active_texturecubemap_binding;
    }
    ...
    {
        state_.pixel_unpack_buffer_binding = saved_state_.pixel_unpack_buffer_binding;
    }
    ...
    GLuint NextImageIdToBeCreated() const
    {
    ...
    void VerifyStateHasNotChangedSinceSave() const
    {
        ...
        EXPECT_EQ(state_.active_texture2d_binding, saved_state_.active_texture2d_binding);
        EXPECT_EQ(state_.active_texturecubemap_binding, saved_state_.active_texturecubemap_binding);
        EXPECT_EQ(state_.renderbuffer_binding, saved_state_.renderbuffer_binding);
        EXPECT_EQ(state_.draw_framebuffer_binding, saved_state_.draw_framebuffer_binding);
        EXPECT_EQ(state_.read_framebuffer_binding, saved_state_.read_framebuffer_binding);
        EXPECT_EQ(state_.pixel_unpack_buffer_binding, saved_state_.pixel_unpack_buffer_binding);
        EXPECT_EQ(state_.pixel_pack_buffer_binding, saved_state_.pixel_pack_buffer_binding);
    }
...
public:
    static scoped_refptr<DrawingBufferForTests> Create(std::unique_ptr<WebGraphicsContext3DProvider> context_provider,
        std::unique_ptr<TestWebGraphicsSharedImageInterfaceProvider> shared_image_interface_provider_for_shared_bitmap,
        const Platform::GraphicsInfo& graphics_info, DrawingBuffer::Client* client, const gfx::Size& size, PreserveDrawingBuffer preserve,
        UseMultisampling use_multisampling)
    {
        ...
        scoped_refptr<DrawingBufferForTests> drawing_buffer
            = base::AdoptRef(new DrawingBufferForTests(std::move(context_provider), graphics_info, std::move(extensions_util), client, preserve));
        if (!drawing_buffer->Initialize(size, use_multisampling != kDisableMultisampling)) {
            drawing_buffer->BeginDestruction();
            return nullptr;
        }
        drawing_buffer->SetSharedImageInterfaceProviderForBitmapTest(std::move(shared_image_interface_provider_for_shared_bitmap));

        return drawing_buffer;
    }

    DrawingBufferForTests(std::unique_ptr<WebGraphicsContext3DProvider> context_provider, const Platform::GraphicsInfo& graphics_info,
        std::unique_ptr<Extensions3DUtil> extensions_util, DrawingBuffer::Client* client, PreserveDrawingBuffer preserve)
        : DrawingBuffer(std::move(context_provider), graphics_info, false /* usingSwapChain */, false /* desynchronized */, std::move(extensions_util), client,
            false /* discardFramebufferSupported */, false /* textureStorageEnabled */, true /* wantAlphaChannel */, true /* premultipliedAlpha */, preserve,
            kWebGL1, false /* wantDepth */, false /* wantStencil */, DrawingBuffer::kAllowChromiumImage /* ChromiumImageUsage */,
            cc::PaintFlags::FilterQuality::kLow, PredefinedColorSpace::kSRGB, gl::GpuPreference::kHighPerformance)
        , live_(nullptr)
    {
    }
    ...
    {
        return static_cast<gpu::TestSharedImageInterface*>(ContextProvider()->SharedImageInterface());
    }

    raw_ptr<bool> live_;

    int RecycledBitmapCount()
    {
        return recycled_bitmaps_.size();
    }
};
