...
public:
    RecyclableCanvasResource(std::unique_ptr<CanvasResourceProvider> resource_provider, base::WeakPtr<WebGPURecyclableResourceCache> cache);
...
public:
    ...
    std::unique_ptr<RecyclableCanvasResource> GetOrCreateCanvasResource(const SkImageInfo& info);

    // When the holder is destroyed, move the resource provider to
    // |unused_providers_| if the cache is not full.
    void OnDestroyRecyclableResource(std::unique_ptr<CanvasResourceProvider> resource_provider, const gpu::SyncToken& completion_sync_token);
...
private:
    ...
    static constexpr int kMaxRecyclableResourceCachesInBytes = kMaxRecyclableResourceCachesInKB * 1024;
    ...
    static constexpr int kTimerIdDeltaForDeletion = kCleanUpDelayInSeconds / kTimerDurationInSeconds;

    struct PLATFORM_EXPORT Resource {
        Resource(std::unique_ptr<CanvasResourceProvider> resource_provider, unsigned int timer_id, int resource_size);
    ...
    std::unique_ptr<CanvasResourceProvider> AcquireCachedProvider(const SkImageInfo& image_info);
