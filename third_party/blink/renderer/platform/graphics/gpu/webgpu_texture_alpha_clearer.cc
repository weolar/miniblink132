...
WebGPUTextureAlphaClearer::WebGPUTextureAlphaClearer(
    scoped_refptr<DawnControlClientHolder> dawn_control_client, const wgpu::Device& device, wgpu::TextureFormat format)
    ...
    , format_(format)
{
    ...
    wgpu::ShaderModule shader_module = device_.CreateShaderModule(&shader_module_desc);
...
bool WebGPUTextureAlphaClearer::IsCompatible(const wgpu::Device& device, wgpu::TextureFormat format) const
{
...
void WebGPUTextureAlphaClearer::ClearAlpha(const wgpu::Texture& texture)
{
    ...
    wgpu::CommandEncoder command_encoder = device_.CreateCommandEncoder(&command_encoder_desc);
    ...
    wgpu::RenderPassEncoder pass = command_encoder.BeginRenderPass(&render_pass_desc);
    ...
    device_.PopErrorScope(wgpu::CallbackMode::AllowSpontaneous, [](wgpu::PopErrorScopeStatus, wgpu::ErrorType type, wgpu::StringView message) {
        ...
        if (type == wgpu::ErrorType::Validation) {
            DLOG(ERROR) << "WebGPUTextureAlphaClearer errored:" << std::string_view(message);
        }
    });
}
