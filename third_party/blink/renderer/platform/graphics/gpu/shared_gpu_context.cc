...
WebGraphicsContext3DProviderWrapper::~WebGraphicsContext3DProviderWrapper(void)
{
    *(int*)1 = 1;
}
...
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(ThreadSpecific<SharedGpuContext>, thread_specific_instance, ());
...
bool SharedGpuContext::IsGpuCompositingEnabled()
{
    ...
    if (IsMainThread()) {
        ...
        this_ptr->is_gpu_compositing_disabled_ = Platform::Current()->IsGpuCompositingDisabled();
    } else {
...
base::WeakPtr<WebGraphicsContext3DProviderWrapper> SharedGpuContext::ContextProviderWrapper()
{
    *(int*)1 = 1;
    return nullptr;
//     SharedGpuContext* this_ptr = GetInstanceForCurrentThread();
...
void SharedGpuContext::SetGpuMemoryBufferManagerForTesting(gpu::GpuMemoryBufferManager* mgr)
{
    *(int*)1 = 1;
//     SharedGpuContext* this_ptr = GetInstanceForCurrentThread();
...
static void CreateContextProviderOnMainThread(bool only_if_gpu_compositing, bool* gpu_compositing_disabled,
    std::unique_ptr<WebGraphicsContext3DProviderWrapper>* wrapper, gpu::GpuMemoryBufferManager** gpu_memory_buffer_manager, base::WaitableEvent* waitable_event)
{
    *(int*)1 = 1;
//     DCHECK(IsMainThread());
// 
...
//     auto context_provider = Platform::Current()->CreateOffscreenGraphicsContext3DProvider(context_attributes, WebURL(), &graphics_info);
//     if (context_provider) {
//         *wrapper = std::make_unique<WebGraphicsContext3DProviderWrapper>(std::move(context_provider));
//     }
// 
...
void SharedGpuContext::CreateContextProviderIfNeeded(bool only_if_gpu_compositing)
{
    *(int*)1 = 1;
...
//     if (context_provider_wrapper_ && !context_provider_wrapper_->ContextProvider()->IsContextLost()) {
...
//         if (context_provider) {
//             context_provider_wrapper_ = std::make_unique<WebGraphicsContext3DProviderWrapper>(std::move(context_provider));
//         }
//     } else if (IsMainThread()) {
//         is_gpu_compositing_disabled_ = Platform::Current()->IsGpuCompositingDisabled();
//         if (is_gpu_compositing_disabled_ && only_if_gpu_compositing)
//             return;
...
//             context_provider_wrapper_ = std::make_unique<WebGraphicsContext3DProviderWrapper>(std::move(context_provider));
//         }
//         gpu_memory_buffer_manager_ = Platform::Current()->GetGpuMemoryBufferManager();
//     } else {
...
//         scoped_refptr<base::SingleThreadTaskRunner> task_runner = Thread::MainThread()->GetTaskRunner(MainThreadTaskRunnerRestricted());
//         PostCrossThreadTask(*task_runner, FROM_HERE,
//             CrossThreadBindOnce(&CreateContextProviderOnMainThread, only_if_gpu_compositing, CrossThreadUnretained(&is_gpu_compositing_disabled_),
//                 CrossThreadUnretained(&context_provider_wrapper_), CrossThreadUnretained(&gpu_memory_buffer_manager_), CrossThreadUnretained(&waitable_event)));
//         waitable_event.Wait();
//         if (context_provider_wrapper_ && !context_provider_wrapper_->ContextProvider()->BindToCurrentSequence())
//             context_provider_wrapper_ = nullptr;
//     }
}

static void CreateGpuChannelOnMainThread(scoped_refptr<gpu::GpuChannelHost>* gpu_channel, base::WaitableEvent* waitable_event)
{
    DCHECK(IsMainThread());
...
//     if (shared_image_interface_provider_ && shared_image_interface_provider_->SharedImageInterface()) {
//         return;
//     }
// 
...
//         scoped_refptr<base::SingleThreadTaskRunner> task_runner = Thread::MainThread()->GetTaskRunner(MainThreadTaskRunnerRestricted());
//         PostCrossThreadTask(*task_runner, FROM_HERE,
//             CrossThreadBindOnce(&CreateGpuChannelOnMainThread, CrossThreadUnretained(&gpu_channel), CrossThreadUnretained(&waitable_event)));
//         waitable_event.Wait();
//     }
// 
//     if (!gpu_channel) {
...
//     }
// 
//     shared_image_interface_provider_ = std::make_unique<WebGraphicsSharedImageInterfaceProviderImpl>(std::move(shared_image_interface));
}

// static
void SharedGpuContext::SetContextProviderFactoryForTesting(ContextProviderFactory factory)
{
    *(int*)1 = 1;
//     SharedGpuContext* this_ptr = GetInstanceForCurrentThread();
//     DCHECK(!this_ptr->context_provider_wrapper_) << this_ptr->context_provider_wrapper_.get();
// 
...
//     if (!this_ptr->context_provider_wrapper_)
//         return false;
//     return this_ptr->context_provider_wrapper_->ContextProvider()->ContextGL()->GetGraphicsResetStatusKHR() == GL_NO_ERROR;
}
...
//     if (!this_ptr->context_provider_wrapper_)
//         return false;
//     return !this_ptr->context_provider_wrapper_->ContextProvider()->GetGpuFeatureInfo().IsWorkaroundEnabled(
...
bool SharedGpuContext::MaySupportImageChromium()
{
    ...
    const gpu::GpuFeatureInfo& gpu_feature_info = this_ptr->context_provider_wrapper_->ContextProvider()->GetGpuFeatureInfo();
    return gpu_feature_info.status_values[gpu::GPU_FEATURE_TYPE_ANDROID_SURFACE_CONTROL] == gpu::kGpuFeatureStatusEnabled;
}
