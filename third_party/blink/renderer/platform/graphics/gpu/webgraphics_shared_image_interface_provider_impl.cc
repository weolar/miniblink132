...
WebGraphicsSharedImageInterfaceProviderImpl::WebGraphicsSharedImageInterfaceProviderImpl(scoped_refptr<gpu::ClientSharedImageInterface> shared_image_interface)
    : shared_image_interface_(std::move(shared_image_interface))
{
    ...
    task_gpu_channel_lost_on_worker_thread_ = base::BindPostTaskToCurrentDefault(
        base::BindOnce(&WebGraphicsSharedImageInterfaceProviderImpl::GpuChannelLostOnWorkerThread, weak_ptr_factory_.GetWeakPtr()));

    shared_image_interface_->gpu_channel()->AddObserver(this);
}

// Destroyed on the same ctor thread.
WebGraphicsSharedImageInterfaceProviderImpl::~WebGraphicsSharedImageInterfaceProviderImpl()
{
...
void WebGraphicsSharedImageInterfaceProviderImpl::AddGpuChannelLostObserver(BitmapGpuChannelLostObserver* ob)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    observer_list_.push_back(ob);
}

void WebGraphicsSharedImageInterfaceProviderImpl::RemoveGpuChannelLostObserver(BitmapGpuChannelLostObserver* ob)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    Erase(observer_list_, ob);
}

gpu::SharedImageInterface* WebGraphicsSharedImageInterfaceProviderImpl::SharedImageInterface()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    return shared_image_interface_.get();
}

base::WeakPtr<blink::WebGraphicsSharedImageInterfaceProvider> WebGraphicsSharedImageInterfaceProviderImpl::GetWeakPtr()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void WebGraphicsSharedImageInterfaceProviderImpl::GpuChannelLostOnWorkerThread()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
