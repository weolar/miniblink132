...
public:
    MOCK_METHOD(gpu::webgpu::ReservedTexture, ReserveTexture, (WGPUDevice device, const WGPUTextureDescriptor* optionalDesc));

    // NOTE: Can switch to using mock if tracking state manually grows to be
    // unwieldy.
    void AssociateMailbox(GLuint, GLuint, GLuint, GLuint, uint64_t, uint64_t internal_usage, const WGPUTextureFormat*, GLuint, gpu::webgpu::MailboxFlags,
        const gpu::Mailbox&) override
    {
        internal_usage_from_most_recent_associate_mailbox_call = static_cast<wgpu::TextureUsage>(internal_usage);
    ...
    {
        most_recent_generated_token = gpu::SyncToken(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId(), ++token_id_);
    ...
    wgpu::TextureUsage internal_usage_from_most_recent_associate_mailbox_call = wgpu::TextureUsage::None;
...
public:
    WebGPUSwapBufferProviderForTests(bool* alive, FakeProviderClient* client, const wgpu::Device& device,
        scoped_refptr<DawnControlClientHolder> dawn_control_client, wgpu::TextureUsage usage, wgpu::TextureUsage internal_usage, wgpu::TextureFormat format,
        PredefinedColorSpace color_space, const gfx::HDRMetadata& hdr_metadata)
        : WebGPUSwapBufferProvider(client, dawn_control_client, device, usage, internal_usage, format, color_space, hdr_metadata)
        , alive_(alive)
        , client_(client)
    {
        texture_desc_ = {
    ...
    ~WebGPUSwapBufferProviderForTests() override
    {
        *alive_ = false;
    }
    ...
    scoped_refptr<WebGPUMailboxTexture> GetNewTexture(const gfx::Size& size)
    {
        ...
        client_->texture = WebGPUSwapBufferProvider::GetNewTexture(texture_desc_, kOpaque_SkAlphaType);
...
protected:
    ...
    static constexpr wgpu::TextureUsage kUsage = wgpu::TextureUsage::RenderAttachment;
    static constexpr wgpu::TextureUsage kInternalUsage = wgpu::TextureUsage::CopyDst;
    ...
    void SetUp() override
    {
        ...
        auto provider = std::make_unique<WebGraphicsContext3DProviderForTests>(std::move(webgpu));
        ...
        auto reservation = wire_client_.ReserveInstance(reinterpret_cast<WGPUInstanceDescriptor*>(&instance_desc));
        ...
        instance_.RequestAdapter(&options, wgpu::CallbackMode::AllowSpontaneous,
            [&](wgpu::RequestAdapterStatus status, wgpu::Adapter adapter, wgpu::StringView) { adapter_ = std::move(adapter); });
        ...
        adapter_.RequestDevice(&deviceDesc, wgpu::CallbackMode::AllowSpontaneous,
            [&](wgpu::RequestDeviceStatus, wgpu::Device device, wgpu::StringView) { device_ = std::move(device); });
        ASSERT_TRUE(c2s_serializer_.Flush());
        ASSERT_TRUE(s2c_serializer_.Flush());
        ASSERT_NE(device_, nullptr);

        dawn_control_client_ = base::MakeRefCounted<DawnControlClientHolder>(std::move(provider), scheduler::GetSingleThreadTaskRunnerForTesting());

        provider_ = base::MakeRefCounted<WebGPUSwapBufferProviderForTests>(
            &provider_alive_, &client_, device_.Get(), dawn_control_client_, kUsage, kInternalUsage, kFormat, PredefinedColorSpace::kSRGB, gfx::HDRMetadata());
    }
    ...
    gpu::webgpu::ReservedTexture ReserveTextureImpl(WGPUDevice device, const WGPUTextureDescriptor* desc)
    {
    ...
    dawn::wire::WireServer wire_server_ { { .procs = &GetDawnNativeProcs(), .serializer = &s2c_serializer_ } };
...
TEST_F(WebGPUSwapBufferProviderTest, VerifyDestructionCompleteAfterAllResourceReleased)
{
    ...
    // Produce resources.
    EXPECT_CALL(*webgpu_, ReserveTexture(device_.Get(), _)).WillOnce(Invoke([&](WGPUDevice device, const WGPUTextureDescriptor* desc) {
        return ReserveTextureImpl(device, desc);
    }));
    provider_->GetNewTexture(kSize);
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource1, &release_callback1));

    EXPECT_CALL(*webgpu_, ReserveTexture(device_.Get(), _)).WillOnce(Invoke([&](WGPUDevice device, const WGPUTextureDescriptor* desc) {
        return ReserveTextureImpl(device, desc);
    }));
    provider_->GetNewTexture(kSize);
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource2, &release_callback2));

    EXPECT_CALL(*webgpu_, ReserveTexture(device_.Get(), _)).WillOnce(Invoke([&](WGPUDevice device, const WGPUTextureDescriptor* desc) {
        return ReserveTextureImpl(device, desc);
    }));
    provider_->GetNewTexture(kSize);
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource3, &release_callback3));
...
TEST_F(WebGPUSwapBufferProviderTest, VerifyResizingProperlyAffectsResources)
{
    ...
    provider_->GetNewTexture(kSize);
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource, &release_callback));
...
TEST_F(WebGPUSwapBufferProviderTest, VerifyInsertAndWaitSyncTokenCorrectly)
{
    ...
    provider_->GetNewTexture(kSize);
    EXPECT_EQ(sii_->MostRecentGeneratedToken(), webgpu_->most_recent_waited_token);
...
TEST_F(WebGPUSwapBufferProviderTest, ReuseSwapBuffers)
{
    ...
    EXPECT_TRUE(shared_images.insert(provider_->GetCurrentMailboxForTesting()).second);

    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource, &release_callback_0));
    ...
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource, &release_callback_1));
    ...
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource, &release_callback_2));
    ...
    EXPECT_FALSE(shared_images.insert(provider_->GetCurrentMailboxForTesting()).second);
...
TEST_F(WebGPUSwapBufferProviderTest, ReuseSwapBufferResize)
{
    ...
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource, &release_callback_3));
    ...
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource, &release_callback_4));
}
...
// DCHECK.
TEST_F(WebGPUSwapBufferProviderTest, PrepareTransferableResourceTwiceAfterDestroy)
{
    EXPECT_CALL(*webgpu_, ReserveTexture(device_.Get(), _)).WillOnce(Invoke([&](WGPUDevice device, const WGPUTextureDescriptor* desc) {
        return ReserveTextureImpl(device, desc);
    }));
    ...
    viz::TransferableResource resource;
    viz::ReleaseCallback release_callback_1;
    EXPECT_FALSE(provider_->PrepareTransferableResource(nullptr, &resource, &release_callback_1));

    viz::ReleaseCallback release_callback_2;
    EXPECT_FALSE(provider_->PrepareTransferableResource(nullptr, &resource, &release_callback_2));
}
...
TEST_F(WebGPUSwapBufferProviderTest, ReserveTextureDescriptorForReflection)
{
    ...
    EXPECT_CALL(*webgpu_, ReserveTexture(device_.Get(), _)).WillOnce(Invoke([&](WGPUDevice device, const WGPUTextureDescriptor* desc) -> auto {
    ...
    EXPECT_CALL(*webgpu_, ReserveTexture(device_.Get(), _)).WillOnce(Invoke([&](WGPUDevice device, const WGPUTextureDescriptor* desc) -> auto {
        ...
        EXPECT_EQ(desc->size.height, static_cast<uint32_t>(kOtherSize.height()));
        return ReserveTextureImpl(device, desc);
    }));
    provider_->GetNewTexture(kOtherSize);
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource, &release_callback));
...
TEST_F(WebGPUSwapBufferProviderTest, GetLastWebGPUMailboxTextureReturnsEmptyWithoutSwapBuffer)
{
...
TEST_F(WebGPUSwapBufferProviderTest, GetLastWebGPUMailboxTextureReturnsValidTextureWithSwapBuffer)
{
    const gfx::Size kSize(10, 20);

    EXPECT_CALL(*webgpu_, ReserveTexture(device_.Get(), _)).WillRepeatedly(Invoke([&](WGPUDevice device, const WGPUTextureDescriptor* desc) -> auto {
        return ReserveTextureImpl(device, desc);
    }));
    provider_->GetNewTexture(kSize);
...
TEST_F(WebGPUSwapBufferProviderTest, GetNewTexturePassesClientSpecifiedInternalUsagePlusRenderAttachment)
{
    ASSERT_EQ(kInternalUsage & wgpu::TextureUsage::RenderAttachment, 0);

    const gfx::Size kSize(10, 20);

    EXPECT_EQ(webgpu_->internal_usage_from_most_recent_associate_mailbox_call, wgpu::TextureUsage::None);
    ...
    EXPECT_EQ(webgpu_->internal_usage_from_most_recent_associate_mailbox_call, kInternalUsage | wgpu::TextureUsage::RenderAttachment);
}
...
TEST_F(WebGPUSwapBufferProviderTest, GetLastMailboxTexturePassesClientSpecifiedInternalUsage)
{
    ASSERT_EQ(kInternalUsage & wgpu::TextureUsage::RenderAttachment, 0);
    ...
    EXPECT_EQ(webgpu_->internal_usage_from_most_recent_associate_mailbox_call, kInternalUsage);
}
