...
bool FrameIsValid(const SkBitmap& frame_bitmap)
{
    ...
    if (frame_bitmap.colorType() != kN32_SkColorType && frame_bitmap.colorType() != kRGBA_F16_SkColorType) {
        return false;
    }
    return true;
}
...
ImageExtractor::ImageExtractor(Image* image, bool premultiply_alpha, sk_sp<SkColorSpace> target_color_space)
{
    if (!image) {
        return;
    }
    ...
    if (image->HasData()) {
        bool paint_image_is_f16 = paint_image.GetColorType() == kRGBA_F16_SkColorType;
        ...
        if (skia_image) {
            ...
            // rounding errors.
            if (target_color_space && !SkColorSpace::Equals(skia_image->colorSpace(), target_color_space.get())) {
                needs_redecode = true;
            }
            ...
            if (skia_image->alphaType() == kPremul_SkAlphaType && !premultiply_alpha) {
                needs_redecode = true;
            }

            // If the image is high bit depth, but was not decoded as high bit depth,
            // then re-decode the image.
            if (paint_image_is_f16 && skia_image->colorType() != kRGBA_F16_SkColorType) {
                needs_redecode = true;
            }
        } else {
        ...
        if (needs_redecode) {
            ...
            // be applied later.
            const auto bit_depth = paint_image_is_f16 ? ImageDecoder::kHighBitDepthToHalfFloat : ImageDecoder::kDefaultBitDepth;

            // If we are not ignoring the color space, then tag the image with the
            // target color space. It will be converted later on.
            const auto color_behavior = target_color_space ? ColorBehavior::kTag : ColorBehavior::kIgnore;

            // Decode the image here on the main thread.
            std::unique_ptr<ImageDecoder> decoder(ImageDecoder::Create(
                image->Data(), data_complete, alpha_option, bit_depth, color_behavior, cc::AuxImage::kDefault, Platform::GetMaxDecodedImageBytes()));
    ...
    if (skia_image->width() != image->width() || skia_image->height() != image->height()) {
        return;
    }
