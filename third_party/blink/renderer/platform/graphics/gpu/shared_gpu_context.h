...
public:
    ...
    static base::WeakPtr<WebGraphicsContext3DProviderWrapper> ContextProviderWrapper();
    static bool AllowSoftwareToAcceleratedCanvasUpgrade();
    static bool IsValidWithoutRestoring();

    static WebGraphicsSharedImageInterfaceProvider* SharedImageInterfaceProvider();
    ...
    using ContextProviderFactory = base::RepeatingCallback<std::unique_ptr<WebGraphicsContext3DProvider>()>;
    ...
    static void SetGpuMemoryBufferManagerForTesting(gpu::GpuMemoryBufferManager* mgr);
...
private:
    ...
    std::unique_ptr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper_;

    std::unique_ptr<WebGraphicsSharedImageInterfaceProvider> shared_image_interface_provider_;

    // RAW_PTR_EXCLUSION: #addr-of
    RAW_PTR_EXCLUSION gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager_ = nullptr;
};
