...
template <class GLES2InterfaceType> class SharedGpuContextTestBase : public Test {
public:
    void SetUp() override
    {
        ...
        handle_ = std::make_unique<base::SingleThreadTaskRunner::CurrentDefaultHandle>(task_runner_);
        auto factory = [](GLES2InterfaceType* gl) -> std::unique_ptr<WebGraphicsContext3DProvider> {
            gl->SetIsContextLost(false);
            auto fake_context = std::make_unique<FakeWebGraphicsContext3DProvider>(gl);
    ...
    GLES2InterfaceType& GlInterface()
    {
        return gl_;
    }
...
class SharedGpuContextTest : public SharedGpuContextTestBase<TestGLES2Interface> { };
...
public:
    void SetUp() override
    {
        ...
        task_runner_ = base::MakeRefCounted<base::NullTaskRunner>();
        handle_ = std::make_unique<base::SingleThreadTaskRunner::CurrentDefaultHandle>(task_runner_);
        auto factory = []() -> std::unique_ptr<WebGraphicsContext3DProvider> { return nullptr; };
        SharedGpuContext::SetContextProviderFactoryForTesting(WTF::BindRepeating(factory));
    }
...
public:
    void SetUp() override
    {
        ...
        InitializeSharedGpuContextGLES2(test_context_provider_.get(),
            /*cache = */ nullptr, SetIsContextLost::kSetToFalse);
    }
...
TEST_F(SharedGpuContextTest, contextLossAutoRecovery)
{
    ...
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> context = SharedGpuContext::ContextProviderWrapper();
...
TEST_F(SharedGpuContextTest, GetRasterModeAutoRecovery)
{
    ...
    EXPECT_FALSE(SharedGpuContext::IsValidWithoutRestoring());
    gfx::Size size(10, 10);
    std::unique_ptr<FakeCanvasResourceHost> host = std::make_unique<FakeCanvasResourceHost>(size);
...
TEST_F(BadSharedGpuContextTest, AccelerateImageBufferSurfaceCreationFails)
{
    ...
    std::unique_ptr<CanvasResourceProvider> resource_provider
        = CanvasResourceProvider::CreateSharedImageProvider(SkImageInfo::MakeN32Premul(10, 10), cc::PaintFlags::FilterQuality::kLow,
            CanvasResourceProvider::ShouldInitialize::kNo, SharedGpuContext::ContextProviderWrapper(), RasterMode::kGPU, gpu::SharedImageUsageSet());
