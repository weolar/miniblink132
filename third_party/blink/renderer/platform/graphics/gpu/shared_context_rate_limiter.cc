...
SharedContextRateLimiter::SharedContextRateLimiter(unsigned max_pending_ticks)
    : max_pending_ticks_(max_pending_ticks)
    , can_use_sync_queries_(false)
{
    context_provider_ = Platform::Current()->CreateSharedOffscreenGraphicsContext3DProvider();
    if (!context_provider_)
        return;

    gpu::raster::RasterInterface* raster_interface = context_provider_->RasterInterface();
    if (raster_interface && raster_interface->GetGraphicsResetStatusKHR() == GL_NO_ERROR) {
...
void SharedContextRateLimiter::Tick()
{
    ...
    if (can_use_sync_queries_) {
        ...
        raster_interface->BeginQueryEXT(GL_COMMANDS_COMPLETED_CHROMIUM, queries_.back());
        raster_interface->EndQueryEXT(GL_COMMANDS_COMPLETED_CHROMIUM);
    }
    if (queries_.size() > max_pending_ticks_) {
        if (can_use_sync_queries_) {
            TRACE_EVENT0("blink", "GPU backpressure via GL_COMMANDS_COMPLETED_CHROMIUM");
            GLuint result;
            raster_interface->GetQueryObjectuivEXT(queries_.front(), GL_QUERY_RESULT_EXT, &result);
...
void SharedContextRateLimiter::Reset()
{
    ...
    gpu::raster::RasterInterface* raster_interface = context_provider_->RasterInterface();
    if (can_use_sync_queries_ && raster_interface && raster_interface->GetGraphicsResetStatusKHR() == GL_NO_ERROR) {
