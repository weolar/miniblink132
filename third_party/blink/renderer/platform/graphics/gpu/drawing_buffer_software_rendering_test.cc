...
class TestSharedBitmapIdRegistar : public cc::SharedBitmapIdRegistrar {
    cc::SharedBitmapIdRegistration RegisterSharedBitmapId(const viz::SharedBitmapId& id, scoped_refptr<cc::CrossThreadSharedBitmap> bitmap) override
    {
        return {};
    }
};
...
protected:
    void SetUp() override
    {
        ...
        auto sii_provider_for_bitmap = TestWebGraphicsSharedImageInterfaceProvider::Create();
        Platform::GraphicsInfo graphics_info;
        graphics_info.using_gpu_compositing = false;

        drawing_buffer_ = DrawingBufferForTests::Create(
            std::move(provider), std::move(sii_provider_for_bitmap), graphics_info, gl_, initial_size, DrawingBuffer::kPreserve, kDisableMultisampling);
...
TEST_F(DrawingBufferSoftwareCompositingTest, BitmapRecycling)
{
    ...
    drawing_buffer_->PrepareTransferableResource(&test_shared_bitmap_id_registrar_, &resource,
        &release_callback1); // create a bitmap.
    EXPECT_EQ(0, drawing_buffer_->RecycledBitmapCount());
    std::move(release_callback1).Run(gpu::SyncToken(), false /* lostResource */); // release bitmap to the recycling queue
    EXPECT_EQ(1, drawing_buffer_->RecycledBitmapCount());
    drawing_buffer_->MarkContentsChanged();
    drawing_buffer_->PrepareTransferableResource(&test_shared_bitmap_id_registrar_, &resource,
        &release_callback2); // recycle a bitmap.
    EXPECT_EQ(0, drawing_buffer_->RecycledBitmapCount());
    std::move(release_callback2).Run(gpu::SyncToken(), false /* lostResource */); // release bitmap to the recycling queue
    EXPECT_EQ(1, drawing_buffer_->RecycledBitmapCount());
...
TEST_F(DrawingBufferSoftwareCompositingTest, FramebufferBinding)
{
    ...
    drawing_buffer_->Resize(initial_size);
    drawing_buffer_->MarkContentsChanged();
    drawing_buffer_->PrepareTransferableResource(&test_shared_bitmap_id_registrar_, &resource, &release_callback);
