...
class PLATFORM_EXPORT XRFrameTransport final : public GarbageCollected<XRFrameTransport>, public device::mojom::blink::XRPresentationClient {
public:
    explicit XRFrameTransport(ContextLifecycleNotifier* context, scoped_refptr<base::SequencedTaskRunner> task_runner);
    ~XRFrameTransport() override;

    void BindSubmitFrameClient(mojo::PendingReceiver<device::mojom::blink::XRPresentationClient> receiver);

    void PresentChange();

    void SetTransportOptions(device::mojom::blink::XRPresentationTransportOptionsPtr);
    ...
    bool FrameSubmit(device::mojom::blink::XRPresentationProvider*, gpu::gles2::GLES2Interface*, gpu::SharedImageInterface*, DrawingBuffer::Client*,
        scoped_refptr<Image> image_ref, int16_t vr_frame_id);

    bool FrameSubmitWebGPU(device::mojom::blink::XRPresentationProvider*, scoped_refptr<DawnControlClientHolder>, wgpu::Device, int16_t vr_frame_id);

    void FrameSubmitMissing(device::mojom::blink::XRPresentationProvider*, gpu::gles2::GLES2Interface*, int16_t vr_frame_id);
    void FrameSubmitMissingWebGPU(device::mojom::blink::XRPresentationProvider*, scoped_refptr<DawnControlClientHolder>, int16_t vr_frame_id);
...
private:
    ...
    HeapMojoReceiver<device::mojom::blink::XRPresentationClient, XRFrameTransport> submit_frame_client_receiver_;
