...
public:
    ...
    // be removed.
    enum AlphaOp { kAlphaDoNothing = 0, kAlphaDoPremultiply = 1, kAlphaDoUnmultiply = 2 };
    ...
    static SkColorType DataFormatToSkColorType(DataFormat data_format, SkColorType default_color_type);
    ...
    static bool ComputeFormatAndTypeParameters(GLenum format, GLenum type, unsigned* components_per_pixel, unsigned* bytes_per_component);
    ...
    static GLenum ComputeImageSizeInBytes(GLenum format, GLenum type, GLsizei width, GLsizei height, GLsizei depth, const PixelStoreParams&,
    ...
    ALWAYS_INLINE static bool SrcFormatComesFromDOMElementOrImageData(DataFormat src_format)
    {
        return src_format == kDataFormatBGRA8 || src_format == kDataFormatRGBA8 || src_format == kDataFormatRGBA16F;
    }
    ...
    static bool PackSkPixmap(const SkPixmap* source, GLenum format, GLenum type, bool flip_y, AlphaOp, const gfx::Rect& source_image_sub_rectangle, int depth,
        unsigned source_unpack_alignment, int unpack_image_height, Vector<uint8_t>& data);
    ...
    static bool PackImageData(Image*, const void* pixels, GLenum format, GLenum type, bool flip_y, AlphaOp, DataFormat source_format,
        unsigned source_image_width, unsigned source_image_height, const gfx::Rect& source_image_sub_rectangle, int depth, unsigned source_unpack_alignment,
        int unpack_image_height, Vector<uint8_t>& data);

    // Extracts the contents of the given ImageData into the passed Vector,
    // packing the pixel data according to the given format and type,
    // and obeying the flipY and premultiplyAlpha flags. Returns true
    // upon success.
    static bool ExtractImageData(const void* image_data, DataFormat source_data_format, const gfx::Size& image_data_size,
        const gfx::Rect& source_image_sub_rectangle, int depth, int unpack_image_height, GLenum format, GLenum type, bool flip_y, bool premultiply_alpha,
        Vector<uint8_t>& data);
    ...
    static bool ExtractTextureData(unsigned width, unsigned height, GLenum format, GLenum type, const PixelStoreParams& unpack_params, bool flip_y,
        bool premultiply_alpha, const void* pixels, Vector<uint8_t>& data);
...
private:
    ...
    // GraphicsContext3DImagePacking.cpp.
    static bool PackPixels(const void* source_data, DataFormat source_data_format, unsigned source_data_width, unsigned source_data_height,
        const gfx::Rect& source_data_sub_rectangle, int depth, unsigned source_unpack_alignment, int unpack_image_height, unsigned destination_format,
        unsigned destination_type, AlphaOp, void* destination_data, bool flip_y);
    static void UnpackPixels(const uint16_t* source_data, DataFormat source_data_format, unsigned pixels_per_row, uint8_t* destination_data);
