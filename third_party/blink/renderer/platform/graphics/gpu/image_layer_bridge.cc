...
scoped_refptr<StaticBitmapImage> MakeAccelerated(
    const scoped_refptr<StaticBitmapImage>& source, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper)
{
#if BUILDFLAG(IS_MAC)
    ...
    const auto image_info = paint_image.GetSkImageInfo().makeWH(source->Size().width(), source->Size().height());
    ...
    constexpr gpu::SharedImageUsageSet kSharedImageUsageFlags = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ;
#else
    ...
    constexpr gpu::SharedImageUsageSet kSharedImageUsageFlags = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT;
#endif // BUILDFLAG(IS_LINUX)
    auto provider = CanvasResourceProvider::CreateSharedImageProvider(image_info, cc::PaintFlags::FilterQuality::kLow,
        CanvasResourceProvider::ShouldInitialize::kNo, context_provider_wrapper, RasterMode::kGPU, kSharedImageUsageFlags);
...
void ImageLayerBridge::SetFilterQuality(cc::PaintFlags::FilterQuality filter_quality)
{
    if (disposed_) {
        return;
    }
...
void ImageLayerBridge::SetUV(const gfx::PointF& left_top, const gfx::PointF& right_bottom)
{
    if (disposed_)
        return;
    ...
    cc::SharedBitmapIdRegistrar* bitmap_registrar, viz::TransferableResource* out_resource, viz::ReleaseCallback* out_release_callback)
{
    ...
    if (gpu_compositing) {
        scoped_refptr<StaticBitmapImage> image_for_compositor = MakeAccelerated(image_, SharedGpuContext::ContextProviderWrapper());
        ...
        const gfx::Size size(image_for_compositor->width(), image_for_compositor->height());

        auto* sii = image_for_compositor->ContextProvider()->SharedImageInterface();
        bool is_overlay_candidate = sii->UsageForMailbox(mailbox_holder.mailbox).Has(gpu::SHARED_IMAGE_USAGE_SCANOUT);

        SkColorType color_type = image_for_compositor->GetSkColorInfo().colorType();
        *out_resource = viz::TransferableResource::MakeGpu(mailbox_holder.mailbox, mailbox_holder.texture_target, mailbox_holder.sync_token, size,
            viz::SkColorTypeToSinglePlaneSharedImageFormat(color_type), is_overlay_candidate, viz::TransferableResource::ResourceSource::kImageLayerBridge);

        auto func = WTF::BindOnce(&ImageLayerBridge::ResourceReleasedGpu, WrapWeakPersistent(this), std::move(image_for_compositor));
        *out_release_callback = std::move(func);
    } else {
        sk_sp<SkImage> sk_image = image_->PaintImageForCurrentFrame().GetSwSkImage();
        if (!sk_image)
            return false;
        ...
        viz::SharedImageFormat format
            = features::IsCanvasSharedBitmapConversionEnabled() ? viz::SinglePlaneFormat::kBGRA_8888 : viz::SinglePlaneFormat::kRGBA_8888;
        RegisteredBitmap registered = CreateOrRecycleBitmap(size, format, bitmap_registrar);
        if (!registered.bitmap) {
            return false;
        }

        SkImageInfo dst_info = SkImageInfo::Make(size.width(), size.height(), dst_color_type, kPremul_SkAlphaType, sk_image->refColorSpace());
        ...
        if (registered.shared_image) {
            *out_resource = viz::TransferableResource::MakeSoftwareSharedImage(
                registered.shared_image, registered.sync_token, size, format, viz::TransferableResource::ResourceSource::kImageLayerBridge);
        } else {
            *out_resource = viz::TransferableResource::MakeSoftwareSharedBitmap(
                registered.bitmap->id(), gpu::SyncToken(), size, format, viz::TransferableResource::ResourceSource::kImageLayerBridge);
        }
        out_resource->color_space = sk_image->colorSpace() ? gfx::ColorSpace(*sk_image->colorSpace()) : gfx::ColorSpace::CreateSRGB();
        auto func = WTF::BindOnce(&ImageLayerBridge::ResourceReleasedSoftware, WrapWeakPersistent(this), std::move(registered));
        *out_release_callback = std::move(func);
    }
...
ImageLayerBridge::RegisteredBitmap ImageLayerBridge::CreateOrRecycleBitmap(
    const gfx::Size& size, viz::SharedImageFormat format, cc::SharedBitmapIdRegistrar* bitmap_registrar)
{
    if (features::IsCanvasSharedBitmapConversionEnabled()) {
        ...
        auto it = std::remove_if(recycled_bitmaps_.begin(), recycled_bitmaps_.end(),
            [&size](const RegisteredBitmap& registered) { return registered.bitmap->size() != size || !registered.sii_provider; });
        ...
        auto shared_image_mapping
            = shared_image_interface->CreateSharedImage({ format, size, gfx::ColorSpace(), gpu::SHARED_IMAGE_USAGE_CPU_WRITE, "ImageLayerBridgeBitmap" });
        ...
        auto it = std::remove_if(recycled_bitmaps_.begin(), recycled_bitmaps_.end(), [&size, &format](const RegisteredBitmap& registered) {
            unsigned src_bytes_per_pixel = registered.bitmap->format().BitsPerPixel() / 8;
            unsigned target_bytes_per_pixel = format.BitsPerPixel() / 8;
            return (registered.bitmap->size().GetArea() * src_bytes_per_pixel != size.GetArea() * target_bytes_per_pixel);
        });
        recycled_bitmaps_.Shrink(static_cast<wtf_size_t>(it - recycled_bitmaps_.begin()));
        ...
        base::MappedReadOnlyRegion shm = viz::bitmap_allocation::AllocateSharedBitmap(size, format);

        RegisteredBitmap registered;
        registered.bitmap = base::MakeRefCounted<cc::CrossThreadSharedBitmap>(id, std::move(shm.region), std::move(shm.mapping), size, format);
        registered.registration = bitmap_registrar->RegisterSharedBitmapId(id, registered.bitmap);

        return registered;
    }
}

void ImageLayerBridge::ResourceReleasedGpu(scoped_refptr<StaticBitmapImage> image, const gpu::SyncToken& token, bool lost_resource)
{
    if (image && image->IsValid()) {
        DCHECK(image->IsTextureBacked());
        if (token.HasData() && image->ContextProvider() && image->ContextProvider()->InterfaceBase()) {
            image->ContextProvider()->InterfaceBase()->WaitSyncTokenCHROMIUM(token.GetConstData());
        }
    }
    // let 'image' go out of scope to release gpu resources.
}

void ImageLayerBridge::ResourceReleasedSoftware(RegisteredBitmap registered, const gpu::SyncToken& sync_token, bool lost_resource)
{
...
ImageLayerBridge::RegisteredBitmap::RegisteredBitmap(RegisteredBitmap&& other) = default;
ImageLayerBridge::RegisteredBitmap& ImageLayerBridge::RegisteredBitmap::operator=(RegisteredBitmap&& other) = default;
