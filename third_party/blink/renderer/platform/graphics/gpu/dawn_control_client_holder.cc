...
scoped_refptr<DawnControlClientHolder> DawnControlClientHolder::Create(
    std::unique_ptr<WebGraphicsContext3DProvider> context_provider, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    auto dawn_control_client_holder = base::MakeRefCounted<DawnControlClientHolder>(std::move(context_provider), std::move(task_runner));
    ...
    // this memory.
    dawn_control_client_holder->context_provider_->ContextProvider()->SetLostContextCallback(
        WTF::BindRepeating(&DawnControlClientHolder::MarkContextLost, dawn_control_client_holder->weak_ptr_factory_.GetWeakPtr()));
...
void DawnControlClientHolder::Destroy()
{
    ...
    if (context_provider_) {
        ...
        task_runner_->PostTask(
            FROM_HERE, base::BindOnce([](std::unique_ptr<WebGraphicsContext3DProviderWrapper> context_provider) {}, std::move(context_provider_)));
    }
}

base::WeakPtr<WebGraphicsContext3DProviderWrapper> DawnControlClientHolder::GetContextProviderWeakPtr() const
{
    if (!context_provider_) {
        return nullptr;
    }
...
std::unique_ptr<RecyclableCanvasResource> DawnControlClientHolder::GetOrCreateCanvasResource(const SkImageInfo& info)
{
...
void DawnControlClientHolder::EnsureFlush(scheduler::EventLoop& event_loop)
{
    ...
    if (!context_provider->ContextProvider()->WebGPUInterface()->EnsureAwaitingFlush()) {
    ...
    event_loop.EnqueueMicrotask(WTF::BindOnce(
        [](scoped_refptr<DawnControlClientHolder> dawn_control_client) {
            if (auto context_provider = dawn_control_client->GetContextProviderWeakPtr()) {
                context_provider->ContextProvider()->WebGPUInterface()->FlushAwaitingCommands();
            }
        },
...
std::vector<wgpu::WGSLFeatureName> GatherWGSLFeatures()
{
    ...
    wgpu::DawnWireWGSLControl wgsl_control = { {
        .enableUnsafe = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableUnsafeWebGPU),
        // This can be changed to true for manual testing with the
        // chromium_testing_* WGSL features.
        .enableTesting = false,
    } };
    wgsl_control.enableExperimental = wgsl_control.enableUnsafe || RuntimeEnabledFeatures::WebGPUExperimentalFeaturesEnabled();
    ...
    if (!wgsl_control.enableUnsafe) {
        wgsl_unsafe_features_owned = base::SplitString(features::kWGSLUnsafeFeatures.Get(), ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    wgpu::Instance instance = wgpu::Instance::Acquire(client.ReserveInstance(&static_cast<const WGPUInstanceDescriptor&>(instance_desc)).instance);
