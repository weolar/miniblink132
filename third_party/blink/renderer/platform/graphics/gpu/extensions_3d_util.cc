...
{
    std::unique_ptr<Extensions3DUtil> out = base::WrapUnique(new Extensions3DUtil(gl));
...
{
    return enabled_extensions_.Contains(name) || requestable_extensions_.Contains(name);
}
