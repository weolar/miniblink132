...
RecyclableCanvasResource::RecyclableCanvasResource(
    std::unique_ptr<CanvasResourceProvider> resource_provider, base::WeakPtr<WebGPURecyclableResourceCache> cache)
...
RecyclableCanvasResource::~RecyclableCanvasResource()
{
    if (cache_ && resource_provider_) {
        cache_->OnDestroyRecyclableResource(std::move(resource_provider_), completion_sync_token_);
    }
}

WebGPURecyclableResourceCache::WebGPURecyclableResourceCache(
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : context_provider_(std::move(context_provider))
    , task_runner_(std::move(task_runner))
{
    weak_ptr_ = weak_ptr_factory_.GetWeakPtr();
    timer_func_ = WTF::BindRepeating(&WebGPURecyclableResourceCache::ReleaseStaleResources, weak_ptr_);

    DCHECK_LE(kTimerDurationInSeconds, kCleanUpDelayInSeconds);
}

std::unique_ptr<RecyclableCanvasResource> WebGPURecyclableResourceCache::GetOrCreateCanvasResource(const SkImageInfo& info)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    std::unique_ptr<CanvasResourceProvider> provider = AcquireCachedProvider(info);
    if (!provider) {
    ...
    return std::make_unique<RecyclableCanvasResource>(std::move(provider), weak_ptr_);
}

void WebGPURecyclableResourceCache::OnDestroyRecyclableResource(
    std::unique_ptr<CanvasResourceProvider> resource_provider, const gpu::SyncToken& completion_sync_token)
{
    int resource_size = resource_provider->Size().width() * resource_provider->Size().height() * resource_provider->GetSkImageInfo().bytesPerPixel();
    if (context_provider_) {
        ...
        unused_providers_.push_front(Resource(std::move(resource_provider), current_timer_id_, resource_size));
    }

    // If the cache is full, release LRU from the back.
    while (total_unused_resources_in_bytes_ > kMaxRecyclableResourceCachesInBytes) {
...
WebGPURecyclableResourceCache::Resource::Resource(std::unique_ptr<CanvasResourceProvider> resource_provider, unsigned int timer_id, int resource_size)
    : resource_provider_(std::move(resource_provider))
    , timer_id_(timer_id)
...
WebGPURecyclableResourceCache::Resource::Resource(Resource&& that) noexcept = default;

WebGPURecyclableResourceCache::Resource::~Resource() = default;

std::unique_ptr<CanvasResourceProvider> WebGPURecyclableResourceCache::AcquireCachedProvider(const SkImageInfo& image_info)
{
    ...
    if (it != unused_providers_.end()) {
        std::unique_ptr<CanvasResourceProvider> provider = (std::move(it->resource_provider_));
...
void WebGPURecyclableResourceCache::ReleaseStaleResources()
{
    ...
    for (const auto& unused_provider : base::Reversed(unused_providers_)) {
        if ((current_timer_id_ - unused_provider.timer_id_) < kTimerIdDeltaForDeletion) {
...
void WebGPURecyclableResourceCache::StartResourceCleanUpTimer()
{
    if (unused_providers_.size() > 0 && !timer_is_running_) {
        task_runner_->PostDelayedTask(FROM_HERE, timer_func_, base::Seconds(kTimerDurationInSeconds));
        timer_is_running_ = true;
    }
}

wtf_size_t WebGPURecyclableResourceCache::CleanUpResourcesAndReturnSizeForTesting()
{
