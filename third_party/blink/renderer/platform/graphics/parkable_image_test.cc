...
public:
    ParkableImageBaseTest()
        : task_env_(base::test::TaskEnvironment::TimeSource::MOCK_TIME, ThreadPoolExecutionMode::DEFAULT)
    {
    }
...
protected:
    ...
    {
        task_env_.FastForwardBy(base::Minutes(5));
    }

    void DescribeCurrentTasks()
    {
    ...
    static bool IsSameContent(scoped_refptr<ParkableImage> pi, base::span<const char> buffer)
    {
    ...
    void ExpectWriteStatistics(base::HistogramBase::Sample sample, base::HistogramBase::Count expected_count)
    {
        histogram_tester_.ExpectTotalCount("Memory.ParkableImage.Write.Latency", expected_count);
        histogram_tester_.ExpectBucketCount("Memory.ParkableImage.Write.Size", sample, expected_count);
    }
    ...
    // be easily tested.
    void ExpectReadStatistics(base::HistogramBase::Sample sample, base::HistogramBase::Count expected_count)
    {
        histogram_tester_.ExpectTotalCount("Memory.ParkableImage.Read.Latency", expected_count);
        histogram_tester_.ExpectTotalCount("Memory.ParkableImage.Read.Throughput", expected_count);
    }
...
public:
    ParkableImageWithLimitedDiskCapacityTest()
    {
        const std::vector<base::test::FeatureRefAndParams> enabled_features
            = { { features::kParkableImagesToDisk, {} }, { features::kCompressParkableStrings, { { "max_disk_capacity_mb", "1" } } } };
...
TEST_F(ParkableImageTest, ManagerStatistics5min)
{
    ...
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.OnDiskFootprintKb.5min", 0);
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.OnDiskSize.5min", 1);
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.TotalReadTime.5min", 1);
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.TotalSize.5min", 1);
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.TotalWriteTime.5min", 1);
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.UnparkedSize.5min", 1);
}
...
TEST_F(ParkableImageNoParkingTest, ManagerStatistics5min)
{
    ...
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.OnDiskFootprintKb.5min", 0);
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.OnDiskSize.5min", 1);
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.TotalReadTime.5min", 0);
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.TotalSize.5min", 1);
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.TotalWriteTime.5min", 0);
    histogram_tester_.ExpectTotalCount("Memory.ParkableImage.UnparkedSize.5min", 1);
}
...
TEST_F(ParkableImageTest, DestroyOnSeparateThread)
{
    ...
    LambdaThreadDelegate delegate { base::BindLambdaForTesting([parkable_image = std::move(pi)]() mutable {
