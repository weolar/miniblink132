...
scoped_refptr<ImagePattern> ImagePattern::Create(scoped_refptr<Image> image, RepeatMode repeat_mode)
{
...
ImagePattern::ImagePattern(scoped_refptr<Image> image, RepeatMode repeat_mode)
    : Pattern(repeat_mode)
    , tile_image_(image->PaintImageForCurrentFrame())
{
}

sk_sp<PaintShader> ImagePattern::CreateShader(const SkMatrix& local_matrix) const
{
    if (!tile_image_) {
    ...
    return PaintShader::MakeImage(
        tile_image_, IsRepeatX() ? SkTileMode::kRepeat : SkTileMode::kDecal, IsRepeatY() ? SkTileMode::kRepeat : SkTileMode::kDecal, &local_matrix);
}
