...
public:
    ...
    enum class ElementRole { kForeground, kListSymbol, kBackground, kSVG, kBorder, kSelection };
    enum class ImageType { kNone, kIcon, kSeparator, kPhoto };

    SkColor4f InvertColorIfNeeded(const SkColor4f& color, ElementRole element_role);
    SkColor4f InvertColorIfNeeded(const SkColor4f& color, ElementRole role, const SkColor4f& contrast_background);

    std::optional<cc::PaintFlags> ApplyToFlagsIfNeeded(const cc::PaintFlags& flags, ElementRole role, SkColor4f contrast_background);
    ...
    sk_sp<cc::ColorFilter> GenerateImageFilter(const SkPixmap& pixmap, const SkIRect& src) const;

    void ApplyFilterToImage(Image* image, cc::PaintFlags* flags, const SkRect& src);
...
private:
    ...
    SkColor4f AdjustDarkenColor(const SkColor4f& color, DarkModeFilter::ElementRole role, const SkColor4f& contrast_background);
