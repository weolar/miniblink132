...
BASE_FEATURE(kDelayParkingImages, "DelayParkingImages", base::FEATURE_ENABLED_BY_DEFAULT);

namespace {

void RecordReadStatistics(size_t size, base::TimeDelta duration, base::TimeDelta time_since_freeze)
{
    int throughput_mb_s = duration.is_zero() ? INT_MAX : base::saturated_cast<int>(size / duration.InSecondsF() / (1024 * 1024));
    ...
    // ranges.
    base::UmaHistogramCustomMicrosecondsTimes("Memory.ParkableImage.Read.Latency", duration, base::Microseconds(500), base::Seconds(1), 100);
    base::UmaHistogramCounts1000("Memory.ParkableImage.Read.Throughput", throughput_mb_s);
}
...
void RecordWriteStatistics(size_t size, base::TimeDelta duration)
{
    ...
    // single-digit MB/s to ~1000MiB/s depending on the CPU/disk, hence the
    // ranges.
    base::UmaHistogramCustomMicrosecondsTimes("Memory.ParkableImage.Write.Latency", duration, base::Microseconds(500), base::Seconds(1), 100);
}
...
ParkableImageSegmentReader::ParkableImageSegmentReader(scoped_refptr<ParkableImage> image)
    : parkable_image_(std::move(image))
    , available_(parkable_image_->size())
{
}
...
base::span<const uint8_t> ParkableImageSegmentReader::GetSomeData(size_t position) const
{
    if (!parkable_image_) {
        return {};
    }
...
BASE_FEATURE(kUseParkableImageSegmentReader, "UseParkableImageSegmentReader", base::FEATURE_ENABLED_BY_DEFAULT);
...
scoped_refptr<SegmentReader> ParkableImageImpl::GetROBufferSegmentReader()
{
    ...
    LockData();
    scoped_refptr<ROBuffer> ro_buffer(rw_buffer_->MakeROBufferSnapshot());
    scoped_refptr<SegmentReader> segment_reader = SegmentReader::CreateFromROBuffer(std::move(ro_buffer));
    UnlockData();
...
{
    DCHECK(!is_on_disk());
    return !TransientlyUnableToPark() && !is_locked() && rw_buffer_->HasNoSnapshots();
}
...
// static
scoped_refptr<ParkableImageImpl> ParkableImageImpl::Create(size_t initial_capacity)
{
...
void ParkableImageImpl::WriteToDiskInBackground(
    scoped_refptr<ParkableImageImpl> parkable_image, scoped_refptr<base::SingleThreadTaskRunner> callback_task_runner)
{
    DCHECK(!IsMainThread());
    ...
    scoped_refptr<ROBuffer> ro_buffer = parkable_image->rw_buffer_->MakeROBufferSnapshot();
    ROBuffer::Iter it(ro_buffer.get());

    Vector<char> vector;
    vector.ReserveInitialCapacity(base::checked_cast<wtf_size_t>(parkable_image->size()));

    do {
        vector.Append(reinterpret_cast<const char*>(it.data()), base::checked_cast<wtf_size_t>(it.size()));
    } while (it.Next());
    ...
    auto metadata = ParkableImageManager::Instance().data_allocator().Write(
        std::move(reserved_chunk), base::span<const uint8_t>((const uint8_t*)vector.data(), vector.size()));
    ...
    if (!parkable_image->on_disk_metadata_) {
        ...
        // way to proceed.
        PostCrossThreadTask(*callback_task_runner, FROM_HERE, CrossThreadBindOnce(&NotifyWriteToDiskFinished, std::move(parkable_image)));
    } else {
        ...
        PostCrossThreadTask(*callback_task_runner, FROM_HERE, CrossThreadBindOnce(&ParkableImageImpl::MaybeDiscardData, std::move(parkable_image)));
    }
}
...
bool ParkableImageImpl::TransientlyUnableToPark() const
{
    if (base::FeatureList::IsEnabled(kDelayParkingImages)) {
        ...
        return !is_frozen() || (base::TimeTicks::Now() - frozen_time_ <= kParkingDelay && !used_);
    } else {
        return !is_frozen();
    }
}

bool ParkableImageImpl::MaybePark(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    DCHECK(ParkableImageManager::IsParkableImagesToDiskEnabled());
    DCHECK(IsMainThread());
    ...
    auto reserved_chunk = ParkableImageManager::Instance().data_allocator().TryReserveChunk(size());
    if (!reserved_chunk) {
        return false;
    }
    ...
    worker_pool::PostTask(FROM_HERE, { base::MayBlock() },
        CrossThreadBindOnce(&ParkableImageImpl::WriteToDiskInBackground, scoped_refptr<ParkableImageImpl>(this), std::move(task_runner)));
    return true;
}

// static
size_t ParkableImageImpl::ReadFromDiskIntoBuffer(DiskDataMetadata* on_disk_metadata, base::span<uint8_t> buffer)
{
    size_t size = on_disk_metadata->size();
    DCHECK_LE(size, buffer.size());
    ParkableImageManager::Instance().data_allocator().Read(*on_disk_metadata, buffer);
    return size;
}
...
void ParkableImageImpl::Unpark()
{
    ...
    DCHECK(!rw_buffer_);
    rw_buffer_ = std::make_unique<RWBuffer>(base::BindOnce(&ParkableImageImpl::ReadFromDiskIntoBuffer, base::Unretained(on_disk_metadata_.get())), size());
