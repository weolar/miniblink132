...
#if 0 // BUILDFLAG(HAS_ZSTD_COMPRESSION)
...
BASE_FEATURE(kCanvasHibernationSnapshotZstd, "CanvasHibernationSnapshotZstd", base::FEATURE_DISABLED_BY_DEFAULT);

// static
HibernatedCanvasMemoryDumpProvider& HibernatedCanvasMemoryDumpProvider::GetInstance()
{
    static base::NoDestructor<HibernatedCanvasMemoryDumpProvider> instance;
    return *instance.get();
}

void HibernatedCanvasMemoryDumpProvider::Register(CanvasHibernationHandler* handler)
{
    DCHECK(IsMainThread());
...
void HibernatedCanvasMemoryDumpProvider::Unregister(CanvasHibernationHandler* handler)
{
    DCHECK(IsMainThread());
    base::AutoLock locker(lock_);
...
bool HibernatedCanvasMemoryDumpProvider::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    DCHECK(IsMainThread());
...

    {
        ...
        for (CanvasHibernationHandler* handler : handlers_) {
            ...
            if (args.level_of_detail == base::trace_event::MemoryDumpLevelOfDetail::kDetailed) {
                auto* canvas_dump = pmd->CreateAllocatorDump(base::StringPrintf("canvas/hibernated/canvas_%d", index));
                canvas_dump->AddScalar("memory_size", "bytes", handler->memory_size());
                canvas_dump->AddScalar("is_encoded", "boolean", handler->is_encoded());
                canvas_dump->AddScalar("original_memory_size", "bytes", handler->original_memory_size());
...
//         this, "hibernated_canvas", blink::Thread::MainThread()->GetTaskRunner(MainThreadTaskRunnerRestricted()));
}
...
void CanvasHibernationHandler::SaveForHibernation(sk_sp<SkImage>&& image, std::unique_ptr<MemoryManagedPaintRecorder> recorder)
{
    DCheckInvariant();
    DCHECK(image);
    epoch_++;
    ...
    // purpose.
    base::PostDelayedMemoryReductionTask(GetMainThreadTaskRunner(), FROM_HERE,
        base::BindOnce(&CanvasHibernationHandler::OnAfterHibernation, weak_ptr_factory_.GetWeakPtr(), epoch_), before_compression_delay_);
}
...
#endif
    auto params = std::make_unique<BackgroundTaskParams>(image_, epoch_, algorithm_, weak_ptr_factory_.GetWeakPtr(), task_runner);

    if (background_thread_task_runner_for_testing_) {
        background_thread_task_runner_for_testing_->PostTask(FROM_HERE, base::BindOnce(&CanvasHibernationHandler::Encode, std::move(params)));
    } else {
        worker_pool::PostTask(FROM_HERE, { base::TaskPriority::BEST_EFFORT }, CrossThreadBindOnce(&CanvasHibernationHandler::Encode, std::move(params)));
    }
}

void CanvasHibernationHandler::OnEncoded(std::unique_ptr<CanvasHibernationHandler::BackgroundTaskParams> params, sk_sp<SkData> encoded)
{
    DCheckInvariant();
...
scoped_refptr<base::SingleThreadTaskRunner> CanvasHibernationHandler::GetMainThreadTaskRunner() const
{
    return main_thread_task_runner_for_testing_ ? main_thread_task_runner_for_testing_ : Thread::MainThread()->GetTaskRunner(MainThreadTaskRunnerRestricted());
}

void CanvasHibernationHandler::Encode(std::unique_ptr<CanvasHibernationHandler::BackgroundTaskParams> params)
{
    ...
    case CompressionAlgorithm::kZstd: {
        ...
        sk_sp<SkData> encoded_uncompressed = SkPngEncoder::Encode(nullptr, params->image.get(), options);

        TRACE_EVENT_BEGIN2("blink", "ZstdCompression", "original_size", 0, "size", 0);
        ...
        size_t compressed_size = ZSTD_compress(
            compressed_buffer.data(), compressed_buffer.size(), encoded_uncompressed->data(), encoded_uncompressed->size(), kZstdCompressionLevel);
        CHECK(!ZSTD_isError(compressed_size)) << "Error: " << ZSTD_getErrorName(uncompressed_size);
        ...
        TRACE_EVENT_END2("blink", "ZstdCompression", "original_size", uncompressed_size, "size", compressed_size);
        break;
#else
        NOTREACHED();
    ...
    int compression_ratio_percentage = static_cast<int>((static_cast<size_t>(100) * encoded->size()) / original_memory_size);
    UMA_HISTOGRAM_TIMES("Blink.Canvas.2DLayerBridge.Compression.ThreadTime", thread_timer.Elapsed());
    UMA_HISTOGRAM_PERCENTAGE("Blink.Canvas.2DLayerBridge.Compression.Ratio", compression_ratio_percentage);
    UMA_HISTOGRAM_CUSTOM_COUNTS("Blink.Canvas.2DLayerBridge.Compression.SnapshotSizeKb", static_cast<int>(original_memory_size / 1024), 10, 500000, 50);

    auto* reply_task_runner = params->reply_task_runner.get();
    reply_task_runner->PostTask(FROM_HERE, base::BindOnce(&CanvasHibernationHandler::OnEncoded, params->weak_instance, std::move(params), encoded));
}
...
sk_sp<SkImage> CanvasHibernationHandler::GetImage()
{
    ...
    case CompressionAlgorithm::kZstd: {
#if 0 // BUILDFLAG(HAS_ZSTD_COMPRESSION)
        uint64_t content_size = ZSTD_getFrameContentSize(encoded_->data(), encoded_->size());
        ...
        size_t uncompressed_size
            = ZSTD_decompress(static_cast<char*>(png_data->writable_data()), png_data->size(), static_cast<const char*>(encoded_->data()), encoded_->size());
        CHECK(!ZSTD_isError(uncompressed_size)) << "Error: " << ZSTD_getErrorName(uncompressed_size);
        break;
#else
        NOTREACHED();
#endif
    ...
    UMA_HISTOGRAM_TIMES("Blink.Canvas.2DLayerBridge.Compression.DecompressionTime", after - before);
    return image;
}
...
{
    return static_cast<size_t>(image.height()) * image.width() * image.imageInfo().bytesPerPixel();
}
