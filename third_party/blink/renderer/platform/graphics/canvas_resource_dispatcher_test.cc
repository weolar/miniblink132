...
public:
    ...
    MOCK_METHOD2(PostImageToPlaceholder, void(scoped_refptr<CanvasResource>&&, viz::ResourceId resource_id));
};

} // namespace

class CanvasResourceDispatcherTest : public testing::Test, public ::testing::WithParamInterface<TestParams> {
public:
    scoped_refptr<CanvasResource> DispatchOneFrame()
    {
        scoped_refptr<CanvasResource> canvas_resource = resource_provider_->ProduceCanvasResource(FlushReason::kTesting);
        auto canvas_resource_extra = canvas_resource;
        dispatcher_->DispatchFrame(std::move(canvas_resource), base::TimeTicks(), SkIRect::MakeEmpty(), false /* needs_vertical_flip */, false /* is-opaque */);
...
protected:
    ...
    void CreateCanvasResourceDispatcher()
    {
        ...
        resource_provider_
            = CanvasResourceProvider::CreateSharedBitmapProvider(SkImageInfo::MakeN32Premul(kWidth, kHeight), cc::PaintFlags::FilterQuality::kLow,
                CanvasResourceProvider::ShouldInitialize::kCallClear, dispatcher_->GetWeakPtr(), test_web_shared_image_interface_provider_.get());
    }

    MockCanvasResourceDispatcher* Dispatcher()
    {
...
TEST_F(CanvasResourceDispatcherTest, PlaceholderBeingBlocked)
{
   ...
   * be saved. */
    EXPECT_CALL(*(Dispatcher()), PostImageToPlaceholder(_, _)).Times(CanvasResourceDispatcher::kMaxUnreclaimedPlaceholderFrames);

    // Attempt to post kMaxUnreclaimedPlaceholderFrames+1 times
    auto frame1 = DispatchOneFrame();
    auto frame2 = DispatchOneFrame();
    for (unsigned i = 0; i < CanvasResourceDispatcher::kMaxUnreclaimedPlaceholderFrames - 1; i++) {
        DispatchOneFrame();
    }
    viz::ResourceId post_resource_id(CanvasResourceDispatcher::kMaxUnreclaimedPlaceholderFrames + 1);
    EXPECT_EQ(CanvasResourceDispatcher::kMaxUnreclaimedPlaceholderFrames, GetNumUnreclaimedFramesPosted());
    EXPECT_EQ(NextId(post_resource_id), PeekNextResourceId());
   ...
   * Resource reclaim happens in the same order as frame posting. */
    ...
    reclaim_resource_id = NextId(reclaim_resource_id);
    Dispatcher()->ReclaimResource(reclaim_resource_id, std::move(frame2));
    EXPECT_EQ(CanvasResourceDispatcher::kMaxUnreclaimedPlaceholderFrames - 1, GetNumUnreclaimedFramesPosted());
    Mock::VerifyAndClearExpectations(Dispatcher());
}
...
TEST_P(CanvasResourceDispatcherTest, DispatchFrame)
{
    ...
    auto canvas_resource = CanvasResourceSharedBitmap::Create(SkImageInfo::MakeN32Premul(GetSize().width(), GetSize().height()),
    ...
    EXPECT_CALL(mock_embedded_frame_sink_provider.mock_compositor_frame_sink(), SubmitCompositorFrame_(_))
        .WillOnce(::testing::WithArg<0>(::testing::Invoke([context_alpha](const viz::CompositorFrame* frame) {
            const viz::CompositorRenderPass* render_pass = frame->render_pass_list[0].get();

            EXPECT_EQ(render_pass->transform_to_root_target, gfx::Transform());
            EXPECT_EQ(render_pass->output_rect, gfx::Rect(kWidth, kHeight));
            EXPECT_EQ(render_pass->damage_rect, gfx::Rect(kDamageWidth, kDamageHeight));
            ...
            const auto* texture_quad = static_cast<const viz::TextureDrawQuad*>(quad);
    ...
    Dispatcher()->DispatchFrame(canvas_resource, base::TimeTicks::Now(), damage_rect, vertical_flip, !context_alpha /* is_opaque */);
    platform->RunUntilIdle();
}

const TestParams kTestCases[] = { { false /* context_alpha */, false /* vertical_flip */ }, { false, true }, { true, false }, { true, true } };

INSTANTIATE_TEST_SUITE_P(All, CanvasResourceDispatcherTest, ValuesIn(kTestCases));
} // namespace blink
