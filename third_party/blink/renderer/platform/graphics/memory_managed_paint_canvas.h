...
class PLATFORM_EXPORT MemoryManagedPaintCanvas final : public cc::InspectableRecordPaintCanvas {
public:
    ...
    void drawImage(const cc::PaintImage& image, SkScalar left, SkScalar top, const SkSamplingOptions&, const cc::PaintFlags* flags) override;
    void drawImageRect(const cc::PaintImage& image, const SkRect& src, const SkRect& dst, const SkSamplingOptions&, const cc::PaintFlags* flags,
        SkCanvas::SrcRectConstraint constraint) override;

    bool IsCachingImage(const cc::PaintImage::ContentId content_id) const;
    size_t ImageBytesUsed() const
    {
        return image_bytes_used_;
    }
...
private:
    ...
    // constructor.
    MemoryManagedPaintCanvas(CreateChildCanvasTag, const MemoryManagedPaintCanvas& parent);

    void UpdateMemoryUsage(const cc::PaintImage& image);

    HashSet<cc::PaintImage::ContentId, IntWithZeroKeyHashTraits<cc::PaintImage::ContentId>> cached_image_ids_;
