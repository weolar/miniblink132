...
class MockCompositorFrameSinkClient : public viz::mojom::blink::CompositorFrameSinkClient {
public:
    // mojom::blink::CompositorFrameSinkClient implementation:
    MOCK_METHOD1(DidReceiveCompositorFrameAck, void(WTF::Vector<viz::ReturnedResource>));
    MOCK_METHOD4(OnBeginFrame,
        void(const viz::BeginFrameArgs&, const WTF::HashMap<uint32_t, viz::FrameTimingDetails>&, bool frame_ack, WTF::Vector<viz::ReturnedResource>));
    MOCK_METHOD1(ReclaimResources, void(WTF::Vector<viz::ReturnedResource>));
    MOCK_METHOD2(WillDrawSurface, void(const viz::LocalSurfaceId&, const gfx::Rect&));
    MOCK_METHOD1(OnBeginFramePausedChanged, void(bool paused));
    MOCK_METHOD1(OnCompositorFrameTransitionDirectiveProcessed, void(uint32_t sequence_id));
