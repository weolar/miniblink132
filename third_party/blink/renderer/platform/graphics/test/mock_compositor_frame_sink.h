...
public:
    MockCompositorFrameSink(mojo::PendingReceiver<viz::mojom::blink::CompositorFrameSink> receiver, int num_expected_set_needs_begin_frame_on_construction)
    {
        receiver_.Bind(std::move(receiver));
        EXPECT_CALL(*this, SetNeedsBeginFrame(true)).Times(num_expected_set_needs_begin_frame_on_construction);
    ...
    void SubmitCompositorFrame(
        const viz::LocalSurfaceId&, viz::CompositorFrame frame, std::optional<viz::HitTestRegionList> hit_test_region_list, uint64_t) override
    {
        SubmitCompositorFrame_(&frame);
    }
    MOCK_METHOD1(SubmitCompositorFrame_, void(viz::CompositorFrame*));
    void SubmitCompositorFrameSync(const viz::LocalSurfaceId&, viz::CompositorFrame frame, std::optional<viz::HitTestRegionList> hit_test_region_list, uint64_t,
    ...
    MOCK_METHOD2(DidAllocateSharedBitmap, void(base::ReadOnlySharedMemoryRegion, const viz::SharedBitmapId&));
