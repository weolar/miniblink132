...
scoped_refptr<SegmentReader> CreateSegmentReader(base::span<char> reference_data)
{
...
TEST_F(DecodingImageGeneratorTest, Create)
{
    ...
    scoped_refptr<SegmentReader> reader = SegmentReader::CreateFromSharedBuffer(std::move(reference_data));
    std::unique_ptr<SkImageGenerator> generator = DecodingImageGenerator::CreateAsSkImageGenerator(reader->GetAsSkData());
...
TEST_F(DecodingImageGeneratorTest, CreateWithNoSize)
{
    ...
    EXPECT_EQ(nullptr, DecodingImageGenerator::CreateAsSkImageGenerator(CreateSegmentReader(reference_data)->GetAsSkData()));
}
