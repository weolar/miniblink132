...
public:
    ...
    {
        return base::BindOnce(&PaintWorkletPaintDispatcherAsyncTest::VerifyResultAndFinish, base::Unretained(this));
    }
...
class MockPaintWorkletPainter : public GarbageCollected<MockPaintWorkletPainter>, public PaintWorkletPainter {
public:
    ...
    MOCK_METHOD2(Paint, PaintRecord(const cc::PaintWorkletInput*, const cc::PaintWorkletJob::AnimatedPropertyValues&));
};
...
public:
    ...
    MOCK_CONST_METHOD0(GetPropertyKeys, const std::vector<PaintWorkletInput::PropertyKey>&());
    MOCK_CONST_METHOD0(IsCSSPaintWorkletInput, bool());
};

cc::PaintWorkletInput* AddPaintWorkletInputToMap(cc::PaintWorkletJobMap& map, int worklet_id)
{
    ...
    map[worklet_id]->data.emplace_back(/*layer_id=*/1, std::move(input), animated_property_values);
    return input_ptr;
}

class PaintWorkletPaintDispatcherMainThread : public PaintWorkletPaintDispatcher {
protected:
...
TEST_F(PaintWorkletPaintDispatcherAsyncTest, DispatchedWorkletIsPainted)
{
    ...
    MockPaintWorkletPainter* mock_painter = MakeGarbageCollected<NiceMock<MockPaintWorkletPainter>>(worklet_id);
    std::unique_ptr<NonMainThread> worklet_thread = CreateTestThread("WorkletThread");
    dispatcher->RegisterPaintWorkletPainter(mock_painter, worklet_thread->GetTaskRunner());
...
TEST_F(PaintWorkletPaintDispatcherAsyncTest, DispatchHandlesEmptyInput)
{
    ...
    const int worklet_id = 4;
    auto* mock_painter = MakeGarbageCollected<NiceMock<MockPaintWorkletPainter>>(worklet_id);
    std::unique_ptr<NonMainThread> worklet_thread = CreateTestThread("WorkletThread");
    dispatcher->RegisterPaintWorkletPainter(mock_painter, worklet_thread->GetTaskRunner());
...
TEST_F(PaintWorkletPaintDispatcherAsyncTest, DispatchSelectsCorrectPainter)
{
    ...
    auto* first_mock_painter = MakeGarbageCollected<NiceMock<MockPaintWorkletPainter>>(first_worklet_id);
    std::unique_ptr<NonMainThread> first_thread = CreateTestThread("WorkletThread1");
    dispatcher->RegisterPaintWorkletPainter(first_mock_painter, first_thread->GetTaskRunner());

    const int second_worklet_id = 3;
    auto* second_mock_painter = MakeGarbageCollected<NiceMock<MockPaintWorkletPainter>>(second_worklet_id);
    std::unique_ptr<NonMainThread> second_thread = CreateTestThread("WorkletThread2");
    dispatcher->RegisterPaintWorkletPainter(second_mock_painter, second_thread->GetTaskRunner());
...
TEST_F(PaintWorkletPaintDispatcherAsyncTest, DispatchIgnoresNonMatchingInput)
{
    ...
    cc::PaintWorkletInput* matching_input = AddPaintWorkletInputToMap(job_map, worklet_id);
...
TEST_F(PaintWorkletPaintDispatcherAsyncTest, DispatchCorrectlyAssignsInputsToMultiplePainters)
{
    auto dispatcher = std::make_unique<PaintWorkletPaintDispatcherMainThread>();
    ...
    cc::PaintWorkletJobMap job_map;
    cc::PaintWorkletInput* first_input = AddPaintWorkletInputToMap(job_map, first_worklet_id);
    cc::PaintWorkletInput* second_input = AddPaintWorkletInputToMap(job_map, second_worklet_id);
...
TEST_F(PaintWorkletPaintDispatcherAsyncTest, HasOngoingDispatchIsTrackedCorrectly)
{
    auto dispatcher = std::make_unique<PaintWorkletPaintDispatcherMainThread>();
