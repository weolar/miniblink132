...
bool IsInflightNetworkRequestBackForwardCacheSupportEnabled()
{
    ...
    // in content/.
    return RuntimeEnabledFeatures::BackForwardCacheEnabled() && base::FeatureList::IsEnabled(features::kLoadingTasksUnfreezable);
}

int GetLoadingTasksUnfreezableParamAsInt(const std::string& param_name, int default_value)
{
    if (!IsInflightNetworkRequestBackForwardCacheSupportEnabled())
        return default_value;
    return base::GetFieldTrialParamByFeatureAsInt(blink::features::kLoadingTasksUnfreezable, param_name, default_value);
}
