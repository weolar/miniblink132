...
BASE_FEATURE(kWebRtcHwAv1Decoding, "WebRtcHwAv1Decoding", base::FEATURE_ENABLED_BY_DEFAULT);
...
std::optional<webrtc::SdpVideoFormat> VdcToWebRtcFormat(const media::VideoDecoderConfig& config)
{
    switch (config.codec()) {
    ...
    case media::VideoCodec::kVP9: {
        ...
        return webrtc::SdpVideoFormat(cricket::kVp9CodecName, { { webrtc::kVP9FmtpProfileId, webrtc::VP9ProfileToString(vp9_profile) } });
    }
    case media::VideoCodec::kH264: {
        ...
        const std::optional<webrtc::H264Level> h264_level = webrtc::H264SupportedLevel(width * height, kDefaultFps);
    ...
    case media::VideoCodec::kHEVC: {
        ...
        const webrtc::Resolution resolution = { .width = config.visible_rect().width(), .height = config.visible_rect().height() };
        const std::optional<webrtc::H265Level> h265_level = webrtc::GetSupportedH265Level(resolution, kDefaultFps);
...
public:
    ScopedVideoDecoder(const scoped_refptr<base::SequencedTaskRunner>& task_runner, std::unique_ptr<webrtc::VideoDecoder> decoder)
...
RTCVideoDecoderFactory::RTCVideoDecoderFactory(media::GpuVideoAcceleratorFactories* gpu_factories, const gfx::ColorSpace& render_color_space)
    : gpu_factories_(gpu_factories)
    , render_color_space_(render_color_space)
{
    if (gpu_factories_) {
        gpu_codec_support_waiter_ = std::make_unique<GpuCodecSupportWaiter>(gpu_factories_);
    }
    DVLOG(2) << __func__;
}
...
void RTCVideoDecoderFactory::CheckAndWaitDecoderSupportStatusIfNeeded() const
{
    ...
    if (!gpu_codec_support_waiter_->IsDecoderSupportKnown()) {
        DLOG(WARNING) << "Decoder support is unknown. Timeout " << gpu_codec_support_waiter_->wait_timeout_ms().value_or(base::TimeDelta()).InMilliseconds()
                      << "ms. Decoders might not be available.";
    }
}

std::vector<webrtc::SdpVideoFormat> RTCVideoDecoderFactory::GetSupportedFormats() const
{
    CheckAndWaitDecoderSupportStatusIfNeeded();

    std::vector<webrtc::SdpVideoFormat> supported_formats;
    for (auto& codec_config : kCodecConfigs) {
        media::VideoDecoderConfig config(codec_config.codec, codec_config.profile, media::VideoDecoderConfig::AlphaMode::kIsOpaque, media::VideoColorSpace(),
            media::kNoTransformation, kDefaultSize, gfx::Rect(kDefaultSize), kDefaultSize, media::EmptyExtraData(), media::EncryptionScheme::kUnencrypted);
        ...
        if (gpu_factories_ && gpu_factories_->IsDecoderConfigSupported(config) == media::GpuVideoAcceleratorFactories::Supported::kTrue) {
            format = VdcToWebRtcFormat(config);
        }

        if (format) {
            // For H.264 decoder, packetization-mode 0/1 should be both supported.
            media::VideoCodec codec = WebRtcToMediaVideoCodec(webrtc::PayloadStringToCodecType(format->name));
    ...
    cricket::AddH264ConstrainedBaselineProfileToSupportedFormats(&supported_formats);
    ...
    if (base::FeatureList::IsEnabled(::features::kWebRtcAllowH265Receive)) {
        ...
        if (configs) {
            for (auto& config : configs.value()) {
                ...
                if (config.profile_min <= media::HEVCPROFILE_MAIN && config.profile_max >= media::HEVCPROFILE_MAIN) {
                    hevc_main_supported = true;
                    hevc_main_max_size.SetSize(static_cast<float>(config.coded_size_max.width()), static_cast<float>(config.coded_size_max.height()));
                }
                if (config.profile_min <= media::HEVCPROFILE_MAIN10 && config.profile_max >= media::HEVCPROFILE_MAIN10) {
                    hevc_main10_supported = true;
                    hevc_main10_max_size.SetSize(static_cast<float>(config.coded_size_max.width()), static_cast<float>(config.coded_size_max.height()));
                }
            }
        }
        if (hevc_main_supported) {
            media::VideoDecoderConfig hevc_main_config(media::VideoCodec::kHEVC, media::HEVCPROFILE_MAIN, media::VideoDecoderConfig::AlphaMode::kIsOpaque,
                media::VideoColorSpace(), media::kNoTransformation, hevc_main_max_size, gfx::Rect(hevc_main_max_size), hevc_main_max_size,
        ...
        if (hevc_main10_supported) {
            media::VideoDecoderConfig hevc_main10_config(media::VideoCodec::kHEVC, media::HEVCPROFILE_MAIN10, media::VideoDecoderConfig::AlphaMode::kIsOpaque,
                media::VideoColorSpace(), media::kNoTransformation, hevc_main10_max_size, gfx::Rect(hevc_main10_max_size), hevc_main10_max_size,
                media::EmptyExtraData(), media::EncryptionScheme::kUnencrypted);
...
webrtc::VideoDecoderFactory::CodecSupport RTCVideoDecoderFactory::QueryCodecSupport(const webrtc::SdpVideoFormat& format, bool reference_scaling) const
{
    CheckAndWaitDecoderSupportStatusIfNeeded();
    ...
    if (codec == media::VideoCodec::kHEVC && !base::FeatureList::IsEnabled(::features::kWebRtcAllowH265Receive)) {
        return { false, false };
    }
    ...
    if (reference_scaling) {
        ...
        if (codec == media::VideoCodec::kVP9 && !media::IsVp9kSVCHWDecodingEnabled()) {
            return { false, false };
        }
    }

    media::VideoCodecProfile codec_profile = WebRtcVideoFormatToMediaVideoCodecProfile(format);
    media::VideoDecoderConfig config(codec, codec_profile, media::VideoDecoderConfig::AlphaMode::kIsOpaque, media::VideoColorSpace(), media::kNoTransformation,
        kDefaultSize, gfx::Rect(kDefaultSize), kDefaultSize, media::EmptyExtraData(), media::EncryptionScheme::kUnencrypted);

    webrtc::VideoDecoderFactory::CodecSupport codec_support;
    // Check gpu_factories for powerEfficient.
    if (gpu_factories_) {
        if (gpu_factories_->IsDecoderConfigSupported(config) == media::GpuVideoAcceleratorFactories::Supported::kTrue) {
...
std::unique_ptr<webrtc::VideoDecoder> RTCVideoDecoderFactory::Create(const webrtc::Environment& /*env*/, const webrtc::SdpVideoFormat& format)
{
    ...
    return decoder ? std::make_unique<ScopedVideoDecoder>(base::SequencedTaskRunner::GetCurrentDefault(), std::move(decoder)) : nullptr;
}
