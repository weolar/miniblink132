...
TEST_F(MetronomeSourceTest, InvokesRequestedCallbackOnTick)
{
    ...
    EXPECT_CALL(*tick_provider_ptr_, RequestCallOnNextTick).WillOnce(Invoke([&](base::OnceClosure closure) { do_tick = std::move(closure); }));
...
TEST_F(MetronomeSourceTest, InvokesRequestedCallbackOnNewTickFromCallbackOnTick)
{
    auto metronome = source_->CreateWebRtcMetronome();
    ...
    InSequence s;
    EXPECT_CALL(*tick_provider_ptr_, RequestCallOnNextTick).WillRepeatedly(Invoke([&](base::OnceClosure closure) { do_tick1 = std::move(closure); }));
    EXPECT_CALL(callback1, Call).WillOnce(Invoke([&] { metronome->RequestCallOnNextTick(callback2.AsStdFunction()); }));
    EXPECT_CALL(*tick_provider_ptr_, RequestCallOnNextTick).WillRepeatedly(Invoke([&](base::OnceClosure closure) { do_tick2 = std::move(closure); }));
...
TEST_F(MetronomeSourceTest, ReturnsTickProviderTickPeriod)
{
    ...
    EXPECT_EQ(kTickPeriod.InMicroseconds(), source_->CreateWebRtcMetronome()->TickPeriod().us());
}
