...
static const std::string kIpv4AddressString = base::StrCat({ kIpv4Address, ":", base::NumberToString(kPort) });
static const std::string kIpv6AddressString = base::StrCat({ "[", kIpv6Address, "]:", base::NumberToString(kPort) });
...
protected:
    ...
    {
        base::WaitableEvent ready(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
        webrtc::ThreadWrapper::EnsureForCurrentMessageLoop();
        EXPECT_NE(webrtc::ThreadWrapper::current(), nullptr);

        std::unique_ptr<FakeConnectionFactory> factory = std::make_unique<FakeConnectionFactory>(webrtc::ThreadWrapper::current(), &ready);

        // Factory doesn't work before initialization.
        EXPECT_EQ(factory->port_count(), 0);
        EXPECT_EQ(factory->CreateConnection(webrtc::IceCandidateType::kHost, kIpv4Address, kPort), nullptr);
        EXPECT_EQ(factory->CreateConnection(webrtc::IceCandidateType::kHost, kIpv6Address, kPort), nullptr);

        int flags = ipv6 ? cricket::PORTALLOCATOR_ENABLE_IPV6 | cricket::PORTALLOCATOR_ENABLE_IPV6_ON_WIFI : cricket::kDefaultPortAllocatorFlags;
...
TEST_F(FakeConnectionFactoryTest, CreateConnectionIPv4)
{
    ...
    const cricket::Connection* conn = factory->CreateConnection(webrtc::IceCandidateType::kHost, kIpv4Address, kPort);
    ...
    ASSERT_EQ(factory->CreateConnection(webrtc::IceCandidateType::kHost, kIpv6Address, kPort), nullptr);
}
...
TEST_F(FakeConnectionFactoryTest, CreateConnectionIPv6)
{
    if (rtc::HasIPv6Enabled()) {
        ...
        const cricket::Connection* conn = factory->CreateConnection(webrtc::IceCandidateType::kHost, kIpv6Address, kPort);
        ASSERT_NE(conn, nullptr);
        EXPECT_EQ(conn->remote_candidate().address().ToString(), kIpv6AddressString);
        EXPECT_EQ(conn->network_thread(), webrtc::ThreadWrapper::current());

        // Connection shouldn't be created to an IPv4 remote address if the factory
        // is not initialized for IPv6.
        ASSERT_EQ(factory->CreateConnection(webrtc::IceCandidateType::kHost, kIpv4Address, kPort), nullptr);
    }
}
...
TEST_F(FakeConnectionFactoryTest, ConvertToIceConnectionIPv4)
{
    ...
    EXPECT_EQ(iceConn.local_candidate().address().ToString(), conn->local_candidate().address().ToString());
    EXPECT_EQ(iceConn.remote_candidate().address().ToString(), conn->remote_candidate().address().ToString());
}
