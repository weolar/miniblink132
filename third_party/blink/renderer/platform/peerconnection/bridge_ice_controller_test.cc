...
public:
    ...
    MOCK_METHOD(void, OnObserverAttached, (scoped_refptr<IceInteractionInterface> agent), (override));
    MOCK_METHOD(void, OnObserverDetached, (), (override));
    MOCK_METHOD(void, OnConnectionAdded, (const IceConnection& connection), (override));
    MOCK_METHOD(void, OnConnectionUpdated, (const IceConnection& connection), (override));
    MOCK_METHOD(void, OnConnectionSwitched, (const IceConnection& connection), (override));
    MOCK_METHOD(void, OnConnectionDestroyed, (const IceConnection& connection), (override));
    MOCK_METHOD(void, OnPingProposal, (const IcePingProposal& ping_proposal), (override));
    MOCK_METHOD(void, OnSwitchProposal, (const IceSwitchProposal& switch_proposal), (override));
    MOCK_METHOD(void, OnPruneProposal, (const IcePruneProposal& prune_proposal), (override));
};
...
class BridgeIceControllerProposalTest : public BridgeIceControllerTest,
                                        public WithParamInterface<std::tuple<PingProposalResponse, SwitchProposalResponse, PruneProposalResponse>> {
protected:
    BridgeIceControllerProposalTest()
        : should_accept_ping_proposal(std::get<0>(GetParam()) == ProposalResponse::ACCEPT)
        , should_accept_switch_proposal(std::get<1>(GetParam()) == ProposalResponse::ACCEPT)
        , should_accept_prune_proposal(std::get<2>(GetParam()) == ProposalResponse::ACCEPT)
    {
    }
...
{
    return base::StrCat({ (response == ProposalResponse::ACCEPT ? "Accept" : "Reject"), "", type });
}

std::string MakeTestName(const TestParamInfo<BridgeIceControllerProposalTest::ParamType>& info)
{
    return base::StrCat({ ToTestSuffix("Ping", std::get<0>(info.param)), "_", ToTestSuffix("Switch", std::get<1>(info.param)), "_",
        ToTestSuffix("Prune", std::get<2>(info.param)) });
}

INSTANTIATE_TEST_SUITE_P(All, BridgeIceControllerProposalTest,
    Combine(Values(PingProposalResponse::ACCEPT, PingProposalResponse::REJECT), Values(SwitchProposalResponse::ACCEPT, SwitchProposalResponse::REJECT),
        Values(PruneProposalResponse::ACCEPT, PruneProposalResponse::REJECT)),
    MakeTestName);
...
TEST_F(BridgeIceControllerTest, ObserverAttached)
{
    ...
    std::unique_ptr<MockIceController> will_move = std::make_unique<MockIceController>(IceControllerFactoryArgs {});

    scoped_refptr<IceInteractionInterface> interaction_agent = nullptr;
    EXPECT_CALL(observer1, OnObserverAttached).WillOnce(WithArgs<0>([&](auto ia) { interaction_agent = std::move(ia); }));
    BridgeIceController controller(env.GetMainThreadTaskRunner(), &observer1, &agent, std::move(will_move));
...
TEST_F(BridgeIceControllerTest, PassthroughIceControllerInterface)
{
    ...
    EXPECT_CALL(*wrapped, GetUseCandidateAttr(conn, NominationMode::AGGRESSIVE, IceMode::ICEMODE_LITE)).WillOnce(Return(true));
    EXPECT_TRUE(controller.GetUseCandidateAttribute(conn, NominationMode::AGGRESSIVE, IceMode::ICEMODE_LITE));
...
TEST_F(BridgeIceControllerTest, HandlesImmediateSwitchRequest)
{
    ...
    scoped_refptr<IceInteractionInterface> interaction_agent = nullptr;
    EXPECT_CALL(observer, OnObserverAttached(_)).WillOnce(WithArgs<0>([&](auto ia) { interaction_agent = std::move(ia); }));
    BridgeIceController controller(env.GetMainThreadTaskRunner(), &observer, &agent, std::move(will_move));
    ...
    EXPECT_CALL(*wrapped, GetConnections()).WillRepeatedly(Return(connection_set));
    ...
    const IceControllerInterface::SwitchResult switch_result { conn, IceRecheckEvent(IceSwitchReason::ICE_CONTROLLER_RECHECK, recheck_delay_ms),
        conns_to_forget };

    // ICE controller should switch to given connection immediately.
    Sequence check_then_switch;
    EXPECT_CALL(*wrapped, ShouldSwitchConnection(reason, conn)).InSequence(check_then_switch).WillOnce(Return(switch_result));
    EXPECT_CALL(observer, OnSwitchProposal(SwitchProposalEq(reason, switch_result, /*reply_expected*/ false))).InSequence(check_then_switch);
    EXPECT_CALL(agent, SwitchSelectedConnection(conn, reason)).InSequence(check_then_switch);
    EXPECT_CALL(agent, ForgetLearnedStateForConnections(ElementsAreArray(conns_to_forget)));
    ...
    EXPECT_CALL(*wrapped, SortAndSwitchConnection(IceSwitchReason::ICE_CONTROLLER_RECHECK)).InSequence(recheck_sort).WillOnce(Return(kEmptySwitchResult));
    // Empty switch proposal could be eliminated, but reason may be interesting.
    EXPECT_CALL(observer, OnSwitchProposal(SwitchProposalEq(IceSwitchReason::ICE_CONTROLLER_RECHECK, kEmptySwitchResult, /*reply_expected*/ false)))
        .InSequence(recheck_sort);
    EXPECT_CALL(agent, ForgetLearnedStateForConnections(IsEmpty())).InSequence(recheck_sort);
    // Recheck should check if anything needs pruning.
    EXPECT_CALL(*wrapped, PruneConnections()).InSequence(recheck_sort).WillOnce(Return(kEmptyConnsList));
...
TEST_P(BridgeIceControllerProposalTest, HandlesImmediateSortAndSwitchRequest)
{
    ...
    // Set default native ICE controller behaviour.
    const std::vector<const Connection*> connection_set { conn, conn_two, conn_three };
    EXPECT_CALL(*wrapped, GetConnections()).WillRepeatedly(Return(connection_set));
    EXPECT_CALL(*wrapped, HasPingableConnection).WillRepeatedly(Return(false));

    const IceSwitchReason reason = IceSwitchReason::NEW_CONNECTION_FROM_LOCAL_CANDIDATE;
    const std::vector<const Connection*> conns_to_forget { conn_two };
    ...
    EXPECT_CALL(*wrapped, SortAndSwitchConnection(reason)).InSequence(sort_and_switch).WillOnce(Return(switch_result));
    EXPECT_CALL(observer, OnSwitchProposal(_)).InSequence(sort_and_switch).WillOnce(WithArgs<0>([&](auto switch_proposal) {
        EXPECT_THAT(switch_proposal,
    ...
    if (should_accept_switch_proposal) {
        EXPECT_CALL(agent, SwitchSelectedConnection(conn, reason)).InSequence(sort_and_switch);
    }
    EXPECT_CALL(*wrapped, PruneConnections()).InSequence(sort_and_switch).WillOnce(Return(conns_to_prune));
    EXPECT_CALL(observer, OnPruneProposal(_)).InSequence(sort_and_switch).WillOnce(WithArgs<0>([&](auto prune_proposal) {
        EXPECT_THAT(prune_proposal, PruneProposalEq(conns_to_prune, /*reply_expected*/ true));
    ...
    // occurs regardless.
    if (should_accept_prune_proposal) {
        EXPECT_CALL(agent, PruneConnections(ElementsAreArray(conns_to_prune))).InSequence(sort_and_switch);
    }
    ...
    Sequence recheck_sort;
    EXPECT_CALL(agent, UpdateConnectionStates()).InSequence(recheck_sort);
    EXPECT_CALL(*wrapped, SortAndSwitchConnection(IceSwitchReason::ICE_CONTROLLER_RECHECK))
        .InSequence(recheck_sort)
...
TEST_P(BridgeIceControllerProposalTest, HandlesSortAndSwitchRequest)
{
    ...
    EXPECT_CALL(*wrapped, PruneConnections()).InSequence(sort_and_switch).WillOnce(Return(kEmptyConnsList));
    // No need to propose pruning if nothing to do.
    EXPECT_CALL(observer, OnPruneProposal).Times(0);
...
TEST_P(BridgeIceControllerProposalTest, StartPingingAfterSortAndSwitch)
{
    ...
    EXPECT_CALL(*wrapped, SortAndSwitchConnection(IceSwitchReason::DATA_RECEIVED)).WillOnce(Return(kEmptySwitchResult));
    EXPECT_CALL(observer,
        OnSwitchProposal(SwitchProposalEq(IceSwitchReason::DATA_RECEIVED, kEmptySwitchResult,
    ...
    const IceControllerInterface::PingResult empty_ping_result(nullptr, recheck_delay_ms);

    // Pinging starts when there is a pingable connection.
    Sequence start_pinging;
    EXPECT_CALL(*wrapped, SortAndSwitchConnection(IceSwitchReason::DATA_RECEIVED)).InSequence(start_pinging).WillOnce(Return(kEmptySwitchResult));
    EXPECT_CALL(observer,
        OnSwitchProposal(SwitchProposalEq(IceSwitchReason::DATA_RECEIVED, kEmptySwitchResult,
            /*reply_expected*/ false)))
        .InSequence(start_pinging);
    EXPECT_CALL(*wrapped, PruneConnections()).InSequence(start_pinging).WillOnce(Return(kEmptyConnsList));
    // No need to propose pruning if nothing to do.
    EXPECT_CALL(observer, OnPruneProposal).Times(0);
    EXPECT_CALL(agent, PruneConnections(IsEmpty())).InSequence(start_pinging);
    EXPECT_CALL(*wrapped, HasPingableConnection()).InSequence(start_pinging).WillOnce(Return(true));
    EXPECT_CALL(agent, OnStartedPinging()).InSequence(start_pinging);
    EXPECT_CALL(agent, GetLastPingSentMs()).InSequence(start_pinging).WillOnce(Return(123));
    EXPECT_CALL(*wrapped, SelectConnectionToPing(123)).InSequence(start_pinging).WillOnce(Return(ping_result));
    EXPECT_CALL(observer, OnPingProposal(_)).InSequence(start_pinging).WillOnce(WithArgs<0>([&](auto ping_proposal) {
        EXPECT_THAT(ping_proposal,
            PingProposalEq(ping_result,
                /*reply_expected*/ true));
    ...
    EXPECT_CALL(*wrapped, SelectConnectionToPing(456)).WillOnce(Return(empty_ping_result));
    EXPECT_CALL(observer,
...
protected:
    BridgeIceControllerInvalidProposalTest()
        : recheck_event(IceSwitchReason::ICE_CONTROLLER_RECHECK, recheck_delay_ms)
    {
        std::unique_ptr<StrictMock<MockIceController>> will_move = std::make_unique<StrictMock<MockIceController>>(IceControllerFactoryArgs {});
        wrapped_controller = will_move.get();

        EXPECT_CALL(observer, OnObserverAttached(_)).WillOnce(WithArgs<0>([&](auto ia) { interaction_agent = std::move(ia); }));
        controller = std::make_unique<BridgeIceController>(env.GetMainThreadTaskRunner(), &observer, &agent, std::move(will_move));
        ...
        const std::vector<const Connection*> connection_set { conn };
        EXPECT_CALL(*wrapped_controller, GetConnections()).WillRepeatedly(Return(connection_set));
    ...
    // api.
    RAW_PTR_EXCLUSION const std::vector<const Connection*> empty_conns_to_forget {};
...
TEST_F(BridgeIceControllerDeathTest, AcceptUnsolicitedPingProposal)
{
    ...
    EXPECT_DCHECK_DEATH_WITH(interaction_agent->AcceptPingProposal(proposal), "unsolicited");
}
...
TEST_F(BridgeIceControllerDeathTest, RejectUnsolicitedPingProposal)
{
    const IceControllerInterface::PingResult ping_result(conn, recheck_delay_ms);
    const IcePingProposal proposal(ping_result, /*reply_expected=*/false);
    EXPECT_DCHECK_DEATH_WITH(interaction_agent->RejectPingProposal(proposal), "unsolicited");
}

TEST_F(BridgeIceControllerDeathTest, AcceptEmptyPingProposal)
{
    const IceControllerInterface::PingResult null_ping_result(nullptr, recheck_delay_ms);
    const IcePingProposal proposal(null_ping_result, /*reply_expected=*/true);
    EXPECT_DCHECK_DEATH_WITH(interaction_agent->AcceptPingProposal(proposal), "without a connection");
}

TEST_F(BridgeIceControllerNoopTest, AcceptUnknownPingProposal)
{
    const IceControllerInterface::PingResult ping_result(conn_two, recheck_delay_ms);
...
{
    const IceControllerInterface::SwitchResult switch_result { conn.get(), recheck_event, empty_conns_to_forget };
    const IceSwitchProposal proposal(reason, switch_result,
        /*reply_expected=*/false);
    EXPECT_DCHECK_DEATH_WITH(interaction_agent->AcceptSwitchProposal(proposal), "unsolicited");
}
...
TEST_F(BridgeIceControllerDeathTest, RejectUnsolicitedSwitchProposal)
{
    ...
    const IceSwitchProposal proposal(reason, switch_result,
        /*reply_expected=*/false);
    EXPECT_DCHECK_DEATH_WITH(interaction_agent->RejectSwitchProposal(proposal), "unsolicited");
}

TEST_F(BridgeIceControllerDeathTest, AcceptEmptySwitchProposal)
{
    const IceControllerInterface::SwitchResult switch_result { std::nullopt, recheck_event, empty_conns_to_forget };
    const IceSwitchProposal proposal(reason, switch_result,
        /*reply_expected=*/true);
    EXPECT_DCHECK_DEATH_WITH(interaction_agent->AcceptSwitchProposal(proposal), "without a connection");
}

TEST_F(BridgeIceControllerDeathTest, AcceptNullSwitchProposal)
{
    const IceControllerInterface::SwitchResult switch_result { std::optional<const Connection*>(nullptr), recheck_event, empty_conns_to_forget };
    const IceSwitchProposal proposal(reason, switch_result,
        /*reply_expected=*/true);
    EXPECT_DCHECK_DEATH_WITH(interaction_agent->AcceptSwitchProposal(proposal), "without a connection");
}
...
{
    const IceControllerInterface::SwitchResult switch_result { conn_two.get(), recheck_event, empty_conns_to_forget };
    const IceSwitchProposal proposal(reason, switch_result,
        /*reply_expected=*/true);
...
TEST_F(BridgeIceControllerDeathTest, AcceptUnsolicitedPruneProposal)
{
    ...
    EXPECT_DCHECK_DEATH_WITH(interaction_agent->RejectPruneProposal(proposal), "unsolicited");
}
...
TEST_F(BridgeIceControllerTest, HandlesPingRequest)
{
    ...
    EXPECT_EQ(interaction_agent->PingIceConnection(IceConnection(conn)).type(), webrtc::RTCErrorType::NONE);

    EXPECT_CALL(agent, SendPingRequest).Times(0);
    EXPECT_EQ(interaction_agent->PingIceConnection(IceConnection(conn_two)).type(), webrtc::RTCErrorType::INVALID_PARAMETER);
}
...
TEST_F(BridgeIceControllerTest, HandlesSwitchRequest)
{
    ...
    EXPECT_CALL(agent, SwitchSelectedConnection(conn, IceSwitchReason::APPLICATION_REQUESTED));
    EXPECT_EQ(interaction_agent->SwitchToIceConnection(IceConnection(conn)).type(), webrtc::RTCErrorType::NONE);

    EXPECT_CALL(agent, SwitchSelectedConnection).Times(0);
    EXPECT_EQ(interaction_agent->SwitchToIceConnection(IceConnection(conn_two)).type(), webrtc::RTCErrorType::INVALID_PARAMETER);
}
...
TEST_F(BridgeIceControllerTest, HandlesPruneRequest)
{
    ...
    const std::vector<IceConnection> valid_ice_conns_to_prune { IceConnection(conn) };
    const std::vector<const Connection*> partial_conns_to_prune { conn_two };
    const std::vector<IceConnection> mixed_ice_conns_to_prune { IceConnection(conn_two), IceConnection(conn_three) };
    const std::vector<IceConnection> invalid_ice_conns_to_prune { IceConnection(conn_three) };

    EXPECT_CALL(agent, PruneConnections(ElementsAreArray(conns_to_prune)));
    EXPECT_EQ(interaction_agent->PruneIceConnections(valid_ice_conns_to_prune).type(), webrtc::RTCErrorType::NONE);

    // Invalid/unknown connections are ignored in a prune request, but the request
    // itself doesn't fail.

    EXPECT_CALL(agent, PruneConnections(ElementsAreArray(partial_conns_to_prune)));
    EXPECT_EQ(interaction_agent->PruneIceConnections(mixed_ice_conns_to_prune).type(), webrtc::RTCErrorType::NONE);

    EXPECT_CALL(agent, PruneConnections).Times(0);
    EXPECT_EQ(interaction_agent->PruneIceConnections(invalid_ice_conns_to_prune).type(), webrtc::RTCErrorType::NONE);
}
