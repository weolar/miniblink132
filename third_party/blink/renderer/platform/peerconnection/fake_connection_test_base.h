...
protected:
    FakeConnectionTestBase()
    {
        ...
        connection_factory_ = std::make_unique<FakeConnectionFactory>(webrtc::ThreadWrapper::current(), &ready);
        connection_factory_->Prepare();
        ready.Wait();
    }

    const ::cricket::Connection* GetConnection(std::string_view remote_ip, int remote_port)
    {
        return connection_factory_->CreateConnection(webrtc::IceCandidateType::kHost, remote_ip, remote_port);
    }
