...
std::optional<int64_t> RTCRtpSource::CaptureTimestamp() const
{
    ...
    return webrtc::UQ32x32ToInt64Ms(source_.absolute_capture_time()->absolute_capture_timestamp);
}

std::optional<int64_t> RTCRtpSource::SenderCaptureTimeOffset() const
{
    if (!source_.absolute_capture_time().has_value() || !source_.absolute_capture_time()->estimated_capture_clock_offset.has_value()) {
        return std::nullopt;
    }
    return webrtc::Q32x32ToInt64Ms(source_.absolute_capture_time()->estimated_capture_clock_offset.value());
}
