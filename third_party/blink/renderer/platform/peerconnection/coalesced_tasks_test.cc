...
TEST(CoalescedTasksTest, TaskRunInOrder)
{
    ...
    EXPECT_CALL(first_callback, Call()).WillOnce([&]() { run_tasks.emplace_back("first"); });
    MockFunction<void()> second_callback;
    EXPECT_CALL(second_callback, Call()).WillOnce([&]() { run_tasks.emplace_back("second"); });
    MockFunction<void()> third_callback;
    EXPECT_CALL(third_callback, Call()).WillOnce([&]() { run_tasks.emplace_back("third"); });

    base::TimeTicks now;
    base::TimeTicks scheduled_time = now + base::Milliseconds(10);

    CoalescedTasks coalesced_tasks;
    coalesced_tasks.QueueDelayedTask(now + base::Milliseconds(5), second_callback.AsStdFunction(), scheduled_time);
    coalesced_tasks.QueueDelayedTask(now + base::Milliseconds(1), first_callback.AsStdFunction(), scheduled_time);
    coalesced_tasks.QueueDelayedTask(now + base::Milliseconds(9), third_callback.AsStdFunction(), scheduled_time);
...
TEST(CoalescedTasksTest, OnlyReadyTasksRun)
{
    ...
    CoalescedTasks coalesced_tasks;
    coalesced_tasks.QueueDelayedTask(now + base::Milliseconds(11), third_callback.AsStdFunction(), second_scheduled_time);
    coalesced_tasks.QueueDelayedTask(now + base::Milliseconds(9), first_callback.AsStdFunction(), first_scheduled_time);
    coalesced_tasks.QueueDelayedTask(now + base::Milliseconds(10), second_callback.AsStdFunction(), first_scheduled_time);
...
TEST(CoalescedTasksTest, QueueDelayedTaskReturnsTrueWhenSchedulingIsNeeded)
{
    ...
    EXPECT_TRUE(coalesced_tasks.QueueDelayedTask(second_scheduled_time, dummy_callback.AsStdFunction(), second_scheduled_time));
    // `second_scheduled_time` does not need to be scheduled multiple times.
    EXPECT_FALSE(coalesced_tasks.QueueDelayedTask(second_scheduled_time, dummy_callback.AsStdFunction(), second_scheduled_time));
    // `first_scheduled_time` needs to be scheduled.
    EXPECT_TRUE(coalesced_tasks.QueueDelayedTask(first_scheduled_time, dummy_callback.AsStdFunction(), first_scheduled_time));
    // `first_scheduled_time` does not need to be scheduled multiple times.
    EXPECT_FALSE(coalesced_tasks.QueueDelayedTask(first_scheduled_time, dummy_callback.AsStdFunction(), first_scheduled_time));
...
TEST(CoalescedTasksTest, PrepareAndFinalizeCallbacks)
{
    ...
    CoalescedTasks::PrepareRunTaskCallback prepare_callback = base::BindRepeating(
        [](std::vector<std::string>* run_tasks) {
            ...
            return std::optional<base::TimeTicks>(base::TimeTicks() + base::Milliseconds(1337));
        },
        base::Unretained(&run_tasks));
    MockFunction<void()> task_callback;
    EXPECT_CALL(task_callback, Call()).WillOnce([&]() { run_tasks.emplace_back("task"); });
    CoalescedTasks::FinalizeRunTaskCallback finalize_callback = base::BindRepeating(
        [](std::vector<std::string>* run_tasks, std::optional<base::TimeTicks> ticks) {
            run_tasks->emplace_back("finalize");
            // Ticks should be the same value that `prepare_callback` returned.
            EXPECT_TRUE(ticks.has_value());
            EXPECT_EQ(ticks.value(), base::TimeTicks() + base::Milliseconds(1337));
        },
        base::Unretained(&run_tasks));
