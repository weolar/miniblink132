...
namespace blink {
class InstrumentedSimulcastAdapter::EncoderFactoryAdapter : public webrtc::VideoEncoderFactory {
public:
    EncoderFactoryAdapter(webrtc::VideoEncoderFactory* encoder_factory, VideoEncoderStateObserver* state_observer, bool is_primary)
    ...
    CodecSupport QueryCodecSupport(const webrtc::SdpVideoFormat& format, std::optional<std::string> scalability_mode) const override
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
    ...
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
        std::unique_ptr<webrtc::VideoEncoder> encoder = encoder_factory_->Create(env, format);
        next_encoder_id_ += is_primary_ ? 1 : -1;
        return std::make_unique<InstrumentedVideoEncoderWrapper>(next_encoder_id_, std::move(encoder), state_observer_);
    }
...
std::unique_ptr<InstrumentedSimulcastAdapter> InstrumentedSimulcastAdapter::Create(const webrtc::Environment& env,
    webrtc::VideoEncoderFactory* primary_encoder_factory, webrtc::VideoEncoderFactory* secondary_encoder_factory,
    std::unique_ptr<VideoEncoderStateObserver> encoder_state_observer, const webrtc::SdpVideoFormat& format)
{
    ...
    if (primary_encoder_factory) {
        primary_factory_adapter = std::make_unique<EncoderFactoryAdapter>(primary_encoder_factory, encoder_state_observer.get(),
            /*is_primary=*/true);
    }
    if (secondary_encoder_factory) {
        secondary_factory_adapter = std::make_unique<EncoderFactoryAdapter>(secondary_encoder_factory, encoder_state_observer.get(),
            /*is_primary=*/false);
    }
    return std::unique_ptr<InstrumentedSimulcastAdapter>(new InstrumentedSimulcastAdapter(
        env, std::move(primary_factory_adapter), std::move(secondary_factory_adapter), std::move(encoder_state_observer), format));
}
...
InstrumentedSimulcastAdapter::InstrumentedSimulcastAdapter(const webrtc::Environment& env, std::unique_ptr<EncoderFactoryAdapter> primary_factory_adapter,
    std::unique_ptr<EncoderFactoryAdapter> secondary_factory_adapter, std::unique_ptr<VideoEncoderStateObserver> encoder_state_observer,
    const webrtc::SdpVideoFormat& format)
    : webrtc::SimulcastEncoderAdapter(env, primary_factory_adapter.get(), secondary_factory_adapter.get(), format)
