...
// compositor thread.
class VSyncProviderImpl::BeginFrameObserver : public VideoFrameSinkBundle::BeginFrameObserver {
public:
...
VSyncProviderImpl::VSyncProviderImpl(scoped_refptr<base::SequencedTaskRunner> task_runner, uint32_t frame_sink_client_id)
    : task_runner_(task_runner)
    , frame_sink_client_id_(frame_sink_client_id)
{
}
...
void VSyncProviderImpl::SetVSyncCallback(base::OnceClosure callback)
{
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            [](base::OnceClosure callback, base::WeakPtr<BeginFrameObserver> observer) {
                if (observer) {
...
void VSyncProviderImpl::Initialize(base::RepeatingCallback<void(bool /*visible*/)> vsync_enabled_callback)
{
    auto observer = std::make_unique<BeginFrameObserver>(std::move(vsync_enabled_callback));
    weak_observer_ = observer->GetWeakPtr();
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            [](uint32_t client_id, std::unique_ptr<BeginFrameObserver> observer) {
                VideoFrameSinkBundle::GetOrCreateSharedInstance(client_id).SetBeginFrameObserver(std::move(observer));
            },
