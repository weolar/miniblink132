...
// initialized.
template <typename PrimaryKey, typename SecondaryKey, typename Value> class TwoKeysAdapterMap {
public:
    ...
    Value* Insert(PrimaryKey primary, Value value)
    {
        ...
        auto* add_result = entries_by_primary_.insert(std::move(primary), std::unique_ptr<Entry>(new Entry(std::move(value)))).stored_value;
    ...
    void SetSecondaryKey(const PrimaryKey& primary, SecondaryKey secondary)
    {
        ...
        DCHECK(entries_by_secondary_.find(secondary) == entries_by_secondary_.end());
        Entry* entry = it->value.get();
        auto* add_result = entries_by_secondary_.insert(std::move(secondary), entry).stored_value;
    ...
    bool EraseByPrimary(const PrimaryKey& primary)
    {
        ...
        if (primary_it->value->secondary_key.has_value()) {
            auto secondary_it = entries_by_secondary_.find(*primary_it->value->secondary_key);
    ...
    bool EraseBySecondary(const SecondaryKey& secondary)
    {
        ...
        auto primary_it = entries_by_primary_.find(secondary_it->value->primary_key);
        entries_by_primary_.erase(primary_it);
        entries_by_secondary_.erase(secondary_it);
        return true;
    }
    ...
    {
        return entries_by_primary_.size();
    }
    // The number of elements in the map which have secondary keys.
    size_t SecondarySize() const
    {
        return entries_by_secondary_.size();
    }
    bool empty() const
    {
        return entries_by_primary_.empty();
    }
