...
webrtc::VideoFrame CreateMockFrame(int width, int height, uint32_t timestamp)
{
    return webrtc::VideoFrame::Builder()
        .set_video_frame_buffer(rtc::make_ref_counted<MockVideoFrameBuffer>(width, height))
...
public:
    ...
    {
        webrtc::VideoFrame video_frame = CreateMockFrame(input_image._encodedWidth, input_image._encodedHeight, input_image.RtpTimestamp());
...
public:
    ...
    void Decoded(webrtc::VideoFrame& decodedImage, std::optional<int32_t> decode_time_ms, std::optional<uint8_t> qp) override
    {
        ...
        webrtc::TimeDelta decode_time = webrtc::TimeDelta::Millis(frame_counter_ % 100 < 90 ? min_decode_time_ms_ : p90_decode_time_ms_);
...
protected:
    StatsCollectingDecoderTest()
        : decoded_image_callback_(kMinDecodingTimeMs, kExpectedP99ProcessingTimeMs)
        , stats_decoder_(kFormatVp9, std::make_unique<MockDecoder>(&is_hw_accelerated_),
              base::BindRepeating(&StatsCollectingDecoderTest::StoreProcessingStatsCB, base::Unretained(this)))
    {
    ...
    void CreateAndDecodeFrames(int width, int height, bool is_hw_accelerated, int frames, int key_frame_interval, int frame_rate)
    {
        CreateAndDecodeFrames(&stats_decoder_, width, height, is_hw_accelerated, frames, key_frame_interval, frame_rate);
    }

    void CreateAndDecodeFrames(
        StatsCollectingDecoder* decoder, int width, int height, bool is_hw_accelerated, int frames, int key_frame_interval, int frame_rate)
    {
        is_hw_accelerated_ = is_hw_accelerated;
        for (int i = 0; i < frames; ++i) {
            ...
            encoded_frame.SetRtpTimestamp(90000 * frame_counter / frame_rate); // RTP timestamp using 90 kHz clock.
            encoded_frame._frameType
                = frame_counter % key_frame_interval == 0 ? webrtc::VideoFrameType::kVideoFrameKey : webrtc::VideoFrameType::kVideoFrameDelta;
            ++frame_counter;
...
TEST_F(StatsCollectingDecoderTest, StoreProcessingStatsCallbackHdSw)
{
    ...
    CreateAndDecodeFrames(kHdWidth, kHdHeight, /*is_hw_accelerated=*/false, kFrames, kKeyframeInterval, kFramerate);
...
TEST_F(StatsCollectingDecoderTest, StoreProcessingStatsCallbackFullHdHw)
{
    ...
    constexpr int kFrames = 200;
    EXPECT_EQ(stats_callbacks_, 0);
    CreateAndDecodeFrames(kFullHdWidth, kFullHdHeight, /*is_hw_accelerated=*/true, kFrames, kKeyframeInterval, kFramerate);
    // Verify that there's been one stats callback and that the numbers are
    // reasonable.
    EXPECT_EQ(stats_callbacks_, 1);
    EXPECT_TRUE(last_stats_key_.is_decode);
...
TEST_F(StatsCollectingDecoderTest, CollectionStopsIfThereAreMultipleDecodersActive)
{
    constexpr int kMinutesToRun = 10;
    EXPECT_EQ(stats_callbacks_, 0);
    CreateAndDecodeFrames(kHdWidth, kHdHeight, /*is_hw_accelerated=*/false, kFramesPerMinute, kKeyframeInterval, kFramerate);
    int last_stats_callbacks = stats_callbacks_;

    // Create another decoder.
    MockDecodedImageCallback second_decoded_image_callback(kMinDecodingTimeMs, kExpectedP99ProcessingTimeMs);
    bool second_decoder_is_hw_accelerated = false;
    StatsCollectingDecoder second_stats_decoder(kFormatVp9, std::make_unique<MockDecoder>(&second_decoder_is_hw_accelerated), base::DoNothing());
    second_stats_decoder.RegisterDecodeCompleteCallback(&second_decoded_image_callback);
