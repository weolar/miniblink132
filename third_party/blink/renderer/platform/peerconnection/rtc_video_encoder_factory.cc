...
BASE_FEATURE(kMediaFoundationAV1Encoding, "MediaFoundationAV1Encoding", base::FEATURE_DISABLED_BY_DEFAULT);

// Enables H.264 CBP encode acceleration for Windows.
BASE_FEATURE(kMediaFoundationH264CbpEncoding, "MediaFoundationH264CbpEncoding", base::FEATURE_DISABLED_BY_DEFAULT);

// Enables VP9 encode acceleration for Windows.
BASE_FEATURE(kMediaFoundationVP9Encoding, "MediaFoundationVP9Encoding", base::FEATURE_ENABLED_BY_DEFAULT);
#endif

// Convert media::SVCScalabilityMode to webrtc::ScalabilityMode and fill
// format.scalability_modes.
void FillScalabilityModes(webrtc::SdpVideoFormat& format, const media::VideoEncodeAccelerator::SupportedProfile& profile)
{
    for (const media::SVCScalabilityMode& mode : profile.scalability_modes) {
        std::optional<webrtc::ScalabilityMode> scalability_mode = webrtc::ScalabilityModeFromString(media::GetScalabilityModeName(mode));
        if (!scalability_mode.has_value()) {
            LOG(WARNING) << "Unrecognized SVC scalability mode: " << media::GetScalabilityModeName(mode);
            continue;
        }
...
std::optional<webrtc::SdpVideoFormat> VEAToWebRTCFormat(const media::VideoEncodeAccelerator::SupportedProfile& profile)
{
    ...
    if (profile.profile >= media::H264PROFILE_MIN && profile.profile <= media::H264PROFILE_MAX) {
...
// software encoding.
        ...
        webrtc_h264_sw_enabled = base::FeatureList::IsEnabled(blink::features::kWebRtcH264WithOpenH264FFmpeg);
    ...
    // BUILDFLAG(ENABLE_OPENH264)
        ...
        const std::optional<webrtc::H264Level> h264_level = webrtc::H264SupportedLevel(width * height, fps);
        const webrtc::H264ProfileLevelId profile_level_id(h264_profile, h264_level.value_or(webrtc::H264Level::kLevel1));
        const std::optional<std::string> h264_profile_level_string = webrtc::H264ProfileLevelIdToString(profile_level_id);
        ...
        format.parameters = { { cricket::kH264FmtpProfileLevelId, *h264_profile_level_string }, { cricket::kH264FmtpLevelAsymmetryAllowed, "1" },
            { cricket::kH264FmtpPacketizationMode, "1" } };
        FillScalabilityModes(format, profile);
        return format;
    }

    if (profile.profile >= media::VP9PROFILE_MIN && profile.profile <= media::VP9PROFILE_MAX) {
        ...
        format.parameters = { { webrtc::kVP9FmtpProfileId, webrtc::VP9ProfileToString(vp9_profile) } };
        FillScalabilityModes(format, profile);
        return format;
    }

    if (profile.profile >= media::AV1PROFILE_MIN && profile.profile <= media::AV1PROFILE_MAX) {
        webrtc::SdpVideoFormat format("AV1");
        FillScalabilityModes(format, profile);
        return format;
    }

    if (profile.profile >= media::HEVCPROFILE_MIN && profile.profile <= media::HEVCPROFILE_MAX) {
#if BUILDFLAG(RTC_USE_H265)
        ...
        const std::optional<webrtc::H265Level> h265_level = webrtc::GetSupportedH265Level(resolution, fps);
        const webrtc::H265ProfileTierLevel profile_tier_level(h265_profile, webrtc::H265Tier::kTier0, h265_level.value_or(webrtc::H265Level::kLevel1));
        webrtc::SdpVideoFormat format("H265");
        format.parameters = { { cricket::kH265FmtpProfileId, webrtc::H265ProfileToString(profile_tier_level.profile) },
            { cricket::kH265FmtpTierFlag, webrtc::H265TierToString(profile_tier_level.tier) },
            { cricket::kH265FmtpLevelId, webrtc::H265LevelToString(profile_tier_level.level) }, { cricket::kH265FmtpTxMode, "SRST" } };
        FillScalabilityModes(format, profile);
        return format;
#else
        return std::nullopt;
...
struct SupportedFormats {
    bool unknown = true;
    std::vector<media::VideoCodecProfile> profiles ALLOW_DISCOURAGED_TYPE("Matches webrtc API");
    std::vector<webrtc::SdpVideoFormat> sdp_formats ALLOW_DISCOURAGED_TYPE("Matches webrtc API");
};
...
// scalability modes.
void InsertOrReplaceWithHigherLevelH265Format(SupportedFormats* supported_formats, const webrtc::SdpVideoFormat& format, media::VideoCodecProfile profile)
{
    std::optional<webrtc::H265ProfileTierLevel> new_profile_tier_level = webrtc::ParseSdpForH265ProfileTierLevel(format.parameters);
    if (!new_profile_tier_level.has_value()) {
        return;
    }

    DCHECK_EQ(supported_formats->profiles.size(), supported_formats->sdp_formats.size());

    std::optional<webrtc::H265ProfileTierLevel> existing_profile_tier_level;
    auto profile_it = std::find(supported_formats->profiles.begin(), supported_formats->profiles.end(), profile);

    if (profile_it != supported_formats->profiles.end()) {
        auto index = std::distance(supported_formats->profiles.begin(), profile_it);
        existing_profile_tier_level = webrtc::ParseSdpForH265ProfileTierLevel(supported_formats->sdp_formats[index].parameters);

        if (existing_profile_tier_level.has_value() && new_profile_tier_level->level > existing_profile_tier_level->level) {
...
SupportedFormats GetSupportedFormatsInternal(media::GpuVideoAcceleratorFactories* gpu_factories, const std::vector<media::VideoCodecProfile>& disabled_profiles)
{
    ...
    for (const auto& profile : *profiles) {
        ...
        if (format) {
            ...
            if (format->name == cricket::kH265CodecName) {
                ...
                // H.265 profile.
                InsertOrReplaceWithHigherLevelH265Format(&low_priority_formats, format.value(), profile.profile);
                continue;
            }
#endif // BUILDFLAG(RTC_USE_H265)
            ...
            const bool kShouldAddH264Cbp
                = base::FeatureList::IsEnabled(kMediaFoundationH264CbpEncoding) && profile.profile == media::VideoCodecProfile::H264PROFILE_BASELINE;
#elif BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
            const bool kShouldAddH264Cbp = profile.profile == media::VideoCodecProfile::H264PROFILE_BASELINE;
#endif
            if (kShouldAddH264Cbp) {
                supported_formats.profiles.push_back(profile.profile);
                cricket::AddH264ConstrainedBaselineProfileToSupportedFormats(&supported_formats.sdp_formats);
            }
#endif
        }
    }

    supported_formats.profiles.insert(supported_formats.profiles.end(), low_priority_formats.profiles.begin(), low_priority_formats.profiles.end());
    supported_formats.sdp_formats.insert(supported_formats.sdp_formats.end(), low_priority_formats.sdp_formats.begin(), low_priority_formats.sdp_formats.end());
...
bool IsConstrainedH264(const webrtc::SdpVideoFormat& format)
{
    ...
    if (format.name == cricket::kH264CodecName) {
        const std::optional<webrtc::H264ProfileLevelId> profile_level_id = webrtc::ParseSdpForH264ProfileLevelId(format.parameters);
        if (profile_level_id && profile_level_id->profile == webrtc::H264Profile::kProfileConstrainedBaseline) {
    ...
    , gpu_codec_support_waiter_(gpu_factories)
{
    ...
    if (
        ...
        disabled_profiles_.emplace_back(media::HEVCPROFILE_HIGH_THROUGHPUT_SCREEN_EXTENDED);
    }
#endif // BUILDFLAG(RTC_USE_H265)
}
...
RTCVideoEncoderFactory::~RTCVideoEncoderFactory()
{
    ...
    gpu_factories_->GetTaskRunner()->ReleaseSoon(FROM_HERE, std::move(encoder_metrics_provider_factory_));
}
...
void RTCVideoEncoderFactory::CheckAndWaitEncoderSupportStatusIfNeeded() const
{
    if (!gpu_codec_support_waiter_.IsEncoderSupportKnown()) {
        DLOG(WARNING) << "Encoder support is unknown. Timeout " << gpu_codec_support_waiter_.wait_timeout_ms().value_or(base::TimeDelta()).InMilliseconds()
                      << "ms. Encoders might not be available.";
    }
}

std::unique_ptr<webrtc::VideoEncoder> RTCVideoEncoderFactory::Create(const webrtc::Environment& env, const webrtc::SdpVideoFormat& format)
{
    CheckAndWaitEncoderSupportStatusIfNeeded();

    std::unique_ptr<webrtc::VideoEncoder> encoder;
    bool is_constrained_h264 = IsConstrainedH264(format);
    auto supported_formats = GetSupportedFormatsInternal(gpu_factories_, disabled_profiles_);
    if (!supported_formats.unknown) {
        for (size_t i = 0; i < supported_formats.sdp_formats.size(); ++i) {
            if (format.IsSameCodec(supported_formats.sdp_formats[i])) {
                encoder
                    = std::make_unique<RTCVideoEncoder>(supported_formats.profiles[i], is_constrained_h264, gpu_factories_, encoder_metrics_provider_factory_);
                break;
            }
        }
    } else {
        ...
        if (profile) {
            encoder = std::make_unique<RTCVideoEncoder>(*profile, is_constrained_h264, gpu_factories_, encoder_metrics_provider_factory_);
        }
    }
...
std::vector<webrtc::SdpVideoFormat> RTCVideoEncoderFactory::GetSupportedFormats() const
{
    CheckAndWaitEncoderSupportStatusIfNeeded();

    return GetSupportedFormatsInternal(gpu_factories_, disabled_profiles_).sdp_formats;
}

webrtc::VideoEncoderFactory::CodecSupport RTCVideoEncoderFactory::QueryCodecSupport(
    const webrtc::SdpVideoFormat& format, std::optional<std::string> scalability_mode) const
{
    CheckAndWaitEncoderSupportStatusIfNeeded();
    SupportedFormats supported_formats = GetSupportedFormatsInternal(gpu_factories_, disabled_profiles_);
    ...
    for (size_t i = 0; i < supported_formats.sdp_formats.size(); ++i) {
        if (format.IsSameCodec(supported_formats.sdp_formats[i])) {
            ...
            if (format.name == cricket::kH265CodecName) {
                const std::optional<webrtc::H265ProfileTierLevel> profile_tier_level = webrtc::ParseSdpForH265ProfileTierLevel(format.parameters);
                if (profile_tier_level) {
                    const std::optional<webrtc::H265ProfileTierLevel> supported_profile
                        = webrtc::ParseSdpForH265ProfileTierLevel(supported_formats.sdp_formats[i].parameters);
                    if (supported_profile && profile_tier_level->level > supported_profile->level) {
            ...
            std::optional<webrtc::ScalabilityMode> mode
                = scalability_mode.has_value() ? webrtc::ScalabilityModeFromString(scalability_mode.value()) : std::nullopt;
            if (!scalability_mode || (mode.has_value() && base::Contains(supported_formats.sdp_formats[i].scalability_modes, mode.value()))) {
