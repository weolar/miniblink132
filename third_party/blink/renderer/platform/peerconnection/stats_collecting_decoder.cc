...
StatsCollectingDecoder::StatsCollectingDecoder(
    const webrtc::SdpVideoFormat& format, std::unique_ptr<webrtc::VideoDecoder> decoder, StatsCollectingDecoder::StoreProcessingStatsCB stats_callback)
    : StatsCollector(
        /*is_decode=*/true, WebRtcVideoFormatToMediaVideoCodecProfile(format), stats_callback)
...
int32_t StatsCollectingDecoder::Decode(const webrtc::EncodedImage& input_image, bool missing_frames, int64_t render_time_ms)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoding_sequence_checker_);
    ...
    }
    {
        ...
        number_of_new_keyframes_ += input_image._frameType == webrtc::VideoFrameType::kVideoFrameKey;
    }
    return decoder_->Decode(input_image, missing_frames, render_time_ms);
}

int32_t StatsCollectingDecoder::RegisterDecodeCompleteCallback(DecodedImageCallback* callback)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoding_sequence_checker_);
...
webrtc::VideoDecoder::DecoderInfo StatsCollectingDecoder::GetDecoderInfo() const
{
...
void StatsCollectingDecoder::Decoded(webrtc::VideoFrame& decodedImage, std::optional<int32_t> decode_time_ms, std::optional<uint8_t> qp)
{
    ...
    // place.
    if ((now - last_check_for_simultaneous_decoders_) > kCheckSimultaneousDecodersInterval) {
    ...
    if (active_stats_collection() && decodedImage.processing_time()) {
        ...
        bool is_hardware_accelerated = decoder_->GetDecoderInfo().is_hardware_accelerated;
        float processing_time_ms = decodedImage.processing_time()->Elapsed().ms();

        AddProcessingTime(pixel_size, is_hardware_accelerated, processing_time_ms, number_of_new_keyframes, now);
    }
}
