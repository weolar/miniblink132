...
public:
    ...
    {
        tab_visible_callback_.Run(visible);
    }

    // VSyncProvider overrides.
    void Initialize(base::RepeatingCallback<void(bool /*visible*/)> callback) override
    {
...
public:
    ...
    {
        fake_default_tick_provider_ = base::MakeRefCounted<FakeDefaultTickProvider>();
        begin_frame_tick_provider_
            = VSyncTickProvider::Create(fake_begin_frame_provider_, base::SequencedTaskRunner::GetCurrentDefault(), fake_default_tick_provider_);
        DepleteTaskQueues();
    }
...
{
    EXPECT_EQ(begin_frame_tick_provider_->TickPeriod(), FakeDefaultTickProvider::kTickPeriod);
}
...
TEST_F(VSyncTickProviderTest, ReportsVSyncTickPeriod)
{
    ...
    SetTabVisible(true);
    RunVSyncCallback();
    EXPECT_EQ(begin_frame_tick_provider_->TickPeriod(), VSyncTickProvider::kVSyncTickPeriod);
}
...
TEST_F(VSyncTickProviderTest, DispatchesCallbackRequestedBeforeSwitchBackFromVSyncs)
{
...
{
    std::unique_ptr<MetronomeSource> source1 = std::make_unique<MetronomeSource>(begin_frame_tick_provider_);
    std::unique_ptr<MetronomeSource> source2 = std::make_unique<MetronomeSource>(begin_frame_tick_provider_);
