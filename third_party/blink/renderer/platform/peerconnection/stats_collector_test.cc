...
protected:
    StatsCollectorTest()
        ...
        , stats_collector_(
              /*is_decode=*/true, kCodecProfile, base::BindRepeating(&StatsCollectorTest::StoreProcessingStatsCB, base::Unretained(this)))
    {
    ...
    void ProcessFrames(int width, int height, bool is_hw_accelerated, int frames, int key_frame_interval, int frame_rate)
    {
        ...
        for (int i = 0; i < frames; ++i) {
            ...
            float processing_time_ms = i % 100 < 90 ? kMinProcessingTimeMs : kExpectedP99ProcessingTimeMs;

            mock_now_ += base::Milliseconds(1000 / frame_rate);
            if (!stats_collector_.stats_collection_finished()) {
                stats_collector_.AddProcessingTime(pixel_size, is_hw_accelerated, processing_time_ms, is_keyframe, mock_now_);
            }
        }
    }
...
TEST_F(StatsCollectorTest, OneCallbackAfterMinNumberOfFrames)
{
    ...
    ProcessFrames(kHdWidth, kHdHeight, /*is_hw_accelerated=*/false, kFrames, kKeyframeInterval, kFramerate);
    ...
    EXPECT_GE(last_video_stats_.frame_count, StatsCollector::kMinSamplesThreshold);
...
TEST_F(StatsCollectorTest, AtLeastOneCallbackEveryMinute)
{
    ...
    for (int minute = 0; minute < kMinutesToRun; ++minute) {
        ProcessFrames(kHdWidth, kHdHeight, /*is_hw_accelerated=*/false, kFramesPerMinute, kKeyframeInterval, kFramerate);
        ...
        EXPECT_EQ(last_stats_key_.pixel_size, kHdWidth * kHdHeight);
        EXPECT_FALSE(last_stats_key_.hw_accelerated);
        EXPECT_GE(last_video_stats_.frame_count, frames_processed - kFramesPerMinute / 2);
...
TEST_F(StatsCollectorTest, NewReportIfResolutionChanges)
{
    ...
    EXPECT_EQ(stats_callbacks_, 0);
    int last_stats_callbacks = stats_callbacks_;
    ProcessFrames(kHdWidth, kHdHeight, /*is_hw_accelerated=*/false, kNumberOfFramesDuringTenSeconds, kKeyframeInterval, kFramerate);
    ...
    ProcessFrames(kFullHdWidth, kFullHdHeight, /*is_hw_accelerated=*/false, kNumberOfFramesDuringTenSeconds, kKeyframeInterval, kFramerate);
    EXPECT_GT(stats_callbacks_, last_stats_callbacks);
...
TEST_F(StatsCollectorTest, NewReportIfHwAccelerationChanges)
{
    ...
    ProcessFrames(kHdWidth, kHdHeight, /*is_hw_accelerated=*/true, 1, kKeyframeInterval, kFramerate);
    ...
    // Continue with new resolution and expect another report.
    ProcessFrames(kHdWidth, kHdHeight, /*is_hw_accelerated=*/true, kNumberOfFramesDuringTenSeconds, kKeyframeInterval, kFramerate);
    EXPECT_GT(stats_callbacks_, last_stats_callbacks);
    EXPECT_EQ(last_stats_key_.pixel_size, kHdWidth * kHdHeight);
