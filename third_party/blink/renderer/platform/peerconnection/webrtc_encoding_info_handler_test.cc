...
public:
    ...
    MOCK_METHOD(std::unique_ptr<webrtc::VideoEncoder>, Create, (const webrtc::Environment&, const webrtc::SdpVideoFormat&), (override));
    MOCK_METHOD(std::vector<webrtc::SdpVideoFormat>, GetSupportedFormats, (), (const));
    MOCK_METHOD(webrtc::VideoEncoderFactory::CodecSupport, QueryCodecSupport,
...
public:
    void VerifyEncodingInfo(const std::optional<webrtc::SdpAudioFormat> sdp_audio_format, const std::optional<webrtc::SdpVideoFormat> sdp_video_format,
        const std::optional<String> video_scalability_mode, const CodecSupport support)
    {
        auto video_encoder_factory = std::make_unique<MockVideoEncoderFactory>();
        rtc::scoped_refptr<webrtc::AudioEncoderFactory> audio_encoder_factory = blink::CreateWebrtcAudioEncoderFactory();
        if (sdp_video_format) {
            const std::optional<std::string> expected_scalability_mode
                = video_scalability_mode ? std::make_optional(video_scalability_mode->Utf8()) : std::nullopt;

            ON_CALL(*video_encoder_factory, QueryCodecSupport)
                .WillByDefault(testing::Invoke(
                    [sdp_video_format, expected_scalability_mode, support](const webrtc::SdpVideoFormat& format, std::optional<std::string> scalability_mode) {
                        EXPECT_TRUE(format.IsSameCodec(*sdp_video_format));
                        EXPECT_EQ(scalability_mode, expected_scalability_mode);
                        return support;
                    }));
            EXPECT_CALL(*video_encoder_factory, QueryCodecSupport).Times(::testing::AtMost(1));
        }
        WebrtcEncodingInfoHandler encoding_info_handler(std::move(video_encoder_factory), audio_encoder_factory);
        MediaCapabilitiesEncodingInfoCallback encoding_info_callback;

        encoding_info_handler.EncodingInfo(sdp_audio_format, sdp_video_format, video_scalability_mode,
            base::BindOnce(&MediaCapabilitiesEncodingInfoCallback::OnWebrtcEncodingInfoSupport, base::Unretained(&encoding_info_callback)));
        ...
        EXPECT_EQ(encoding_info_callback.IsPowerEfficient(), support.is_power_efficient);
    }
};

TEST_F(WebrtcEncodingInfoHandlerTests, BasicAudio)
{
    VerifyEncodingInfo(kAudioFormatOpus, /*sdp_video_format=*/std::nullopt,
        /*video_scalability_mode=*/std::nullopt, CodecSupport { /*is_supported=*/true, /*is_power_efficient=*/true });
}

TEST_F(WebrtcEncodingInfoHandlerTests, UnsupportedAudio)
{
    VerifyEncodingInfo(kAudioFormatFoo, /*sdp_video_format=*/std::nullopt,
        /*video_scalability_mode=*/std::nullopt, CodecSupport { /*is_supported=*/false, /*is_power_efficient=*/false });
}
...
TEST_F(WebrtcEncodingInfoHandlerTests, BasicVideo)
{
    VerifyEncodingInfo(
        ...
        /*video_scalability_mode=*/std::nullopt, CodecSupport { /*is_supported=*/true, /*is_power_efficient=*/false });
}
...
{
    VerifyEncodingInfo(
        /*sdp_audio_format=*/std::nullopt, kVideoFormatVp9, "L1T3", CodecSupport { /*is_supported=*/true, /*is_power_efficient=*/false });
}

TEST_F(WebrtcEncodingInfoHandlerTests, SupportedAudioUnsupportedVideo)
{
    VerifyEncodingInfo(kAudioFormatOpus, kVideoFormatFoo,
        /*video_scalability_mode=*/std::nullopt, CodecSupport { /*is_supported=*/false, /*is_power_efficient=*/false });
}

TEST_F(WebrtcEncodingInfoHandlerTests, SupportedVideoUnsupportedAudio)
{
    VerifyEncodingInfo(kAudioFormatFoo, kVideoFormatVp9,
        /*video_scalability_mode=*/std::nullopt, CodecSupport { /*is_supported=*/false, /*is_power_efficient=*/false });
}
