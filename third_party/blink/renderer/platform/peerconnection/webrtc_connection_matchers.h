...
inline void PrintTo(const IceControllerInterface::PingResult& result, std::ostream* os)
{
    *os << "PingResult[";
...
inline void PrintTo(const IceControllerInterface::SwitchResult& result, std::ostream* os)
{
    *os << "SwitchResult[";
    PrintTo(result.connection, os);
    *os << ":" << (result.recheck_event.has_value() ? result.recheck_event->ToString() : "<no recheck>");
    int ctr = 1;
...
MATCHER_P(ConnectionEq,
    ...
    /* const cricket::Connection* */ conn, base::StrCat({ negation ? "doesn't match " : "matches ", PrintToString(conn) }))
{
    return conn != nullptr && arg.id() == conn->id() && arg.local_candidate() == conn->local_candidate() && arg.remote_candidate() == conn->remote_candidate()
        && arg.connected() == conn->connected() && arg.selected() == conn->selected() && arg.last_ping_sent() == conn->last_ping_sent()
        && arg.last_ping_received() == conn->last_ping_received() && arg.last_data_received() == conn->last_data_received()
        && arg.last_ping_response_received() == conn->last_ping_response_received() && arg.num_pings_sent() == conn->num_pings_sent();
}
...
{
    if (arg.has_value()) {
        return ExplainMatchResult(ConnectionEq(conn.value_or(nullptr)), arg.value(), result_listener);
    }
...
MATCHER(CricketBlinkConnectionTupleEq,
    ...
    "")
{
    return ExplainMatchResult(ConnectionEq(std::get<1>(arg)), std::get<0>(arg), result_listener);
}
    ...
    /* std::vector<const cricket::Connection*> */ connections, "")
{
    ...
    base::ranges::copy_if(connections, std::back_inserter(non_null_connections), [](auto conn) { return conn != nullptr; });
    return ExplainMatchResult(UnorderedPointwise(CricketBlinkConnectionTupleEq(), non_null_connections), arg, result_listener);
}
...
MATCHER_P2(PingProposalEq,
    ...
    /* bool */ reply_expected, base::StrCat({ negation ? "doesn't match " : "matches ", PrintToString(result) }))
{
    if (!ExplainMatchResult(ConnectionOptionalsEq(result.connection), arg.connection(), result_listener)) {
        return false;
    }
    if (!ExplainMatchResult(Optional(result.recheck_delay_ms), arg.recheck_delay_ms(), result_listener)) {
        return false;
    }
...
MATCHER_P(RecheckEventEq,
    ...
    /* const cricket::IceRecheckEvent& */ event, base::StrCat({ negation ? "doesn't match " : "matches ", PrintToString(event) }))
{
    return blink::ConvertFromWebrtcIceSwitchReason(event.reason) == arg.reason && event.recheck_delay_ms == arg.recheck_delay_ms;
}
    ...
    /* bool */ reply_expected, base::StrCat({ negation ? "doesn't match " : "matches ", PrintToString(result) }))
{
    ...
    if (result.recheck_event.has_value() && !ExplainMatchResult(Optional(RecheckEventEq(result.recheck_event.value())), arg.recheck_event(), result_listener)) {
        return false;
    }
    if (!ExplainMatchResult(ConnectionSequenceEq(result.connections_to_forget_state_on), arg.connections_to_forget_state_on(), result_listener)) {
        return false;
    }
    return arg.reply_expected() == reply_expected;
}
...
MATCHER_P2(PruneProposalEq,
    ...
    /* std::vector<const cricket::Connection*> */ connections,
    /* bool */ reply_expected, base::StrCat({ negation ? "doesn't match " : "matches ", PrintToString(connections) }))
{
    if (!ExplainMatchResult(ConnectionSequenceEq(connections), arg.connections_to_prune(), result_listener)) {
        return false;
    }
    return arg.reply_expected() == reply_expected;
}
