...
RTCIceCandidatePlatform::RTCIceCandidatePlatform(
    String candidate, String sdp_mid, std::optional<uint16_t> sdp_m_line_index, String username_fragment, std::optional<String> url)
...
RTCIceCandidatePlatform::RTCIceCandidatePlatform(String candidate, String sdp_mid, std::optional<uint16_t> sdp_m_line_index)
    : candidate_(std::move(candidate))
    , sdp_mid_(std::move(sdp_mid))
    , sdp_m_line_index_(std::move(sdp_m_line_index))
{
...
void RTCIceCandidatePlatform::PopulateFields(bool use_username_from_candidate)
{
    ...
    DCHECK(type == "host" || type == "srflx" || type == "prflx" || type == "relay");
    type_ = String(type);
    if (!c.tcptype().empty()) {
