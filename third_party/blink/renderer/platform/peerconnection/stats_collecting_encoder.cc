...
StatsCollectingEncoder::StatsCollectingEncoder(
    const webrtc::SdpVideoFormat& format, std::unique_ptr<webrtc::VideoEncoder> encoder, StatsCollector::StoreProcessingStatsCB stats_callback)
    : StatsCollector(
        /*is_decode=*/false, WebRtcVideoFormatToMediaVideoCodecProfile(format), stats_callback)
...
void StatsCollectingEncoder::SetFecControllerOverride(webrtc::FecControllerOverride* fec_controller_override)
{
    encoder_->SetFecControllerOverride(fec_controller_override);
}

int StatsCollectingEncoder::InitEncode(const webrtc::VideoCodec* codec_settings, const webrtc::VideoEncoder::Settings& settings)
{
    DVLOG(3) << __func__;
...
int32_t StatsCollectingEncoder::RegisterEncodeCompleteCallback(EncodedImageCallback* callback)
{
    DVLOG(3) << __func__;
    DCHECK(callback);
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoding_sequence_checker_);
...
int32_t StatsCollectingEncoder::Encode(const webrtc::VideoFrame& frame, const std::vector<webrtc::VideoFrameType>* frame_types)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoding_sequence_checker_);
    if (!first_frame_encoded_) {
...
void StatsCollectingEncoder::OnLossNotification(const LossNotification& loss_notification)
{
    encoder_->OnLossNotification(loss_notification);
}

webrtc::VideoEncoder::EncoderInfo StatsCollectingEncoder::GetEncoderInfo() const
{
    ...
    const webrtc::EncodedImage& encoded_image, const webrtc::CodecSpecificInfo* codec_specific_info)
{
    ...
    webrtc::EncodedImageCallback::Result result = encoded_callback_->OnEncodedImage(encoded_image, codec_specific_info);

    const size_t encoded_image_stream_index = encoded_image.SimulcastIndex().value_or(encoded_image.SpatialIndex().value_or(0));
    highest_observed_stream_index_ = std::max(highest_observed_stream_index_, encoded_image_stream_index);

    if (stats_collection_finished() || encoded_image_stream_index != highest_observed_stream_index_) {
    ...
    // place.
    if ((now - last_check_for_simultaneous_encoders_) > kCheckSimultaneousEncodersInterval) {
    ...
    if (active_stats_collection()) {
        std::optional<base::TimeTicks> encode_start;
        {
            ...
            // timestamp.
            base::AutoLock auto_lock(lock_);
            while (encode_start_info_.size() > 0 && encode_start_info_.front().rtp_timestamp != encoded_image.RtpTimestamp()) {
                encode_start_info_.pop_front();
            }
        ...
        if (encode_start) {
            ...
            int pixel_size = encoded_image._encodedWidth * encoded_image._encodedHeight;
            bool is_hardware_accelerated = encoder_->GetEncoderInfo().is_hardware_accelerated;
            bool is_keyframe = encoded_image._frameType == webrtc::VideoFrameType::kVideoFrameKey;
            AddProcessingTime(pixel_size, is_hardware_accelerated, encode_time_ms, is_keyframe, now);
        }
    }
    return result;
}
