...
public:
    MOCK_METHOD1(OnEncodedFrame, void(std::unique_ptr<webrtc::TransformableVideoFrameInterface>));
};

class MockMetronome : public webrtc::Metronome {
public:
    MOCK_METHOD(void, RequestCallOnNextTick, (absl::AnyInvocable<void() &&> callback), (override));
    MOCK_METHOD(webrtc::TimeDelta, TickPeriod, (), (const, override));
};

std::unique_ptr<webrtc::MockTransformableVideoFrame> CreateMockFrame()
{
    auto mock_frame = std::make_unique<NiceMock<webrtc::MockTransformableVideoFrame>>();
...
class RTCEncodedVideoStreamTransformerTest : public testing::TestWithParam<bool> {
public:
    RTCEncodedVideoStreamTransformerTest()
        ...
        , encoded_video_stream_transformer_(main_task_runner_, absl::WrapUnique(metronome_.get()))
    {
    }

    void SetUp() override
    {
        EXPECT_FALSE(encoded_video_stream_transformer_.HasTransformedFrameSinkCallback(kSSRC));
        encoded_video_stream_transformer_.RegisterTransformedFrameSinkCallback(webrtc_callback_, kSSRC);
        EXPECT_TRUE(encoded_video_stream_transformer_.HasTransformedFrameSinkCallback(kSSRC));
        EXPECT_FALSE(encoded_video_stream_transformer_.HasTransformedFrameSinkCallback(kNonexistentSSRC));
        if (GetParam()) {
            ON_CALL(*metronome_, RequestCallOnNextTick(_)).WillByDefault([](absl::AnyInvocable<void() &&> callback) { std::move(callback)(); });
        }
    }
    ...
    {
        metronome_ = nullptr;
        encoded_video_stream_transformer_.UnregisterTransformedFrameSinkCallback(kSSRC);
        EXPECT_FALSE(encoded_video_stream_transformer_.HasTransformedFrameSinkCallback(kSSRC));
    }
...
INSTANTIATE_TEST_SUITE_P(MetronomeAlignment, RTCEncodedVideoStreamTransformerTest, testing::Values(true, false));

TEST_P(RTCEncodedVideoStreamTransformerTest, TransformerForwardsFrameToTransformerCallback)
{
    ...
    encoded_video_stream_transformer_.SetTransformerCallback(
        WTF::CrossThreadBindRepeating(&MockTransformerCallbackHolder::OnEncodedFrame, WTF::CrossThreadUnretained(&mock_transformer_callback_holder_)));
    ...
    PostCrossThreadTask(*webrtc_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&webrtc::FrameTransformerInterface::Transform, encoded_video_stream_transformer_.Delegate(), CreateMockFrame()));
...
{
    EXPECT_CALL(*webrtc_callback_, OnTransformedFrame);
    std::unique_ptr<webrtc::MockTransformableVideoFrame> mock_frame = CreateMockFrame();
    EXPECT_CALL(*mock_frame.get(), GetSsrc).WillRepeatedly(Return(kNonexistentSSRC));
...
TEST_P(RTCEncodedVideoStreamTransformerTest, ShortCircuitingSetOnLateRegisteredCallback)
{
    EXPECT_CALL(*webrtc_callback_, StartShortCircuiting);
    encoded_video_stream_transformer_.StartShortCircuiting();

    rtc::scoped_refptr<MockWebRtcTransformedFrameCallback> webrtc_callback_2(new rtc::RefCountedObject<MockWebRtcTransformedFrameCallback>());
    EXPECT_CALL(*webrtc_callback_2, StartShortCircuiting);
    encoded_video_stream_transformer_.RegisterTransformedFrameSinkCallback(webrtc_callback_2, kSSRC + 1);
}
...
TEST_P(RTCEncodedVideoStreamTransformerTest, WaitsForMetronomeTick)
{
    ...
    EXPECT_CALL(*metronome_, RequestCallOnNextTick).WillOnce([&](absl::AnyInvocable<void() &&> c) { callback = std::move(c); });
    ...
    EXPECT_CALL(mock_transformer_callback_holder_, OnEncodedFrame).Times(transform_count);
    // Must be done on the same sequence as the transform calls.
    PostCrossThreadTask(*webrtc_task_runner_, FROM_HERE,
        CrossThreadBindOnce([](absl::AnyInvocable<void() &&>* callback) { std::move (*callback)(); }, CrossThreadUnretained(&callback)));

    task_environment_.RunUntilIdle();
}

TEST_P(RTCEncodedVideoStreamTransformerTest, FramesBufferedBeforeShortcircuiting)
{
...
TEST_P(RTCEncodedVideoStreamTransformerTest, FrameArrivingAfterShortcircuitingIsPassedBack)
{
    EXPECT_CALL(*webrtc_callback_, StartShortCircuiting);
    encoded_video_stream_transformer_.StartShortCircuiting();
...
TEST_P(RTCEncodedVideoStreamTransformerTest, FramesBufferedBeforeSettingTransform)
{
