...
public:
    ...
    using StoreProcessingStatsCB = base::RepeatingCallback<void(const StatsKey&, const VideoStats&)>;

    StatsCollector(bool is_decode, media::VideoCodecProfile codec_profile, StoreProcessingStatsCB stats_callback);

    bool active_stats_collection() const
    {
        return static_cast<bool>(processing_time_ms_histogram_);
    }
    bool stats_collection_finished() const
    {
        return stats_collection_finished_;
    }
    ...
    void AddProcessingTime(
        int pixel_size, bool is_hardware_accelerated, const float processing_time_ms, const size_t new_keyframes, const base::TimeTicks& now);
