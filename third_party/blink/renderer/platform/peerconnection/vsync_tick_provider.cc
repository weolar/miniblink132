...
scoped_refptr<VSyncTickProvider> VSyncTickProvider::Create(
    VSyncProvider& provider, scoped_refptr<base::SequencedTaskRunner> sequence, scoped_refptr<MetronomeSource::TickProvider> default_tick_provider)
{
    scoped_refptr<VSyncTickProvider> tick_provider(new VSyncTickProvider(provider, sequence, std::move(default_tick_provider)));
    sequence->PostTask(FROM_HERE, base::BindOnce(&VSyncTickProvider::Initialize, tick_provider->weak_factory_.GetWeakPtr()));
    return tick_provider;
}

VSyncTickProvider::VSyncTickProvider(
    VSyncProvider& vsync_provider, scoped_refptr<base::SequencedTaskRunner> sequence, scoped_refptr<MetronomeSource::TickProvider> default_tick_provider)
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    vsync_provider_->Initialize(
        base::BindPostTask(sequence_, base::BindRepeating(&VSyncTickProvider::OnTabVisibilityChange, weak_factory_.GetWeakPtr()), FROM_HERE));
}
...
void VSyncTickProvider::ScheduleVSync()
{
    ...
    vsync_provider_->SetVSyncCallback(base::BindPostTask(sequence_, base::BindOnce(&VSyncTickProvider::OnVSync, weak_tick_factory_.GetWeakPtr()), FROM_HERE));
}
...
void VSyncTickProvider::ScheduleDefaultTick()
{
    ...
    default_tick_provider_->RequestCallOnNextTick(base::BindOnce(&VSyncTickProvider::OnDefaultTick, weak_tick_factory_.GetWeakPtr()));
}
...
void VSyncTickProvider::OnTabVisibilityChange(bool visible)
{
    ...
    TRACE_EVENT_INSTANT1("webrtc", __func__, TRACE_EVENT_SCOPE_PROCESS, "visible", visible);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
