...
public:
    LowPrecisionTimerTest()
        : task_environment_(base::test::TaskEnvironment::ThreadingMode::MULTIPLE_THREADS, base::test::TaskEnvironment::TimeSource::MOCK_TIME)
    {
        // Ensure mock time is aligned with metronome tick.
        base::TimeTicks now = base::TimeTicks::Now();
        task_environment_.FastForwardBy(TimerBasedTickProvider::TimeSnappedToNextTick(now, TimerBasedTickProvider::kDefaultPeriod) - now);
    }
...
public:
    RecursiveStartOneShotter(size_t repeat_count, base::TimeDelta delay)
        : timer_(base::ThreadPool::CreateSequencedTaskRunner({}), base::BindRepeating(&RecursiveStartOneShotter::Callback, base::Unretained(this)))
        , repeat_count_(repeat_count)
        , delay_(delay)
    {
        timer_.StartOneShot(delay_);
    }
    ~RecursiveStartOneShotter()
    {
        timer_.Shutdown();
    }
...
public:
    explicit RecursiveStopper(base::TimeDelta delay)
        : timer_(base::ThreadPool::CreateSequencedTaskRunner({}), base::BindRepeating(&RecursiveStopper::Callback, base::Unretained(this)))
    {
...
public:
    IsActiveChecker()
        : timer_(base::ThreadPool::CreateSequencedTaskRunner({}), base::BindRepeating(&IsActiveChecker::Callback, base::Unretained(this)))
    {
    }
    ~IsActiveChecker()
    {
    ...
    void Callback()
    {
        was_active_in_last_callback_ = timer_.IsActive();
    }
...
TEST_F(LowPrecisionTimerTest, StartOneShot)
{
    ...
    LowPrecisionTimer timer(listener.task_runner(), base::BindRepeating(&CallbackListener::Callback, base::Unretained(&listener)));
...
TEST(LowPrecisionTimerRealThreadsTest, StopTimerWithRaceCondition)
{
    base::test::TaskEnvironment task_environment(
        base::test::TaskEnvironment::ThreadingMode::MULTIPLE_THREADS, base::test::TaskEnvironment::TimeSource::SYSTEM_TIME);
    ...
    dedicated_task_runner->PostTask(FROM_HERE,
        base::BindOnce(
