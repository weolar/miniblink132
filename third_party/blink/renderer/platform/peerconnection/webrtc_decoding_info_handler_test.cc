...
public:
    ...
    MOCK_METHOD(std::unique_ptr<webrtc::VideoDecoder>, Create, (const webrtc::Environment&, const webrtc::SdpVideoFormat& format), (override));
    MOCK_METHOD(std::vector<webrtc::SdpVideoFormat>, GetSupportedFormats, (), (const));
    MOCK_METHOD(
        webrtc::VideoDecoderFactory::CodecSupport, QueryCodecSupport, (const webrtc::SdpVideoFormat& format, bool spatial_scalability), (const, override));
};
...
public:
    void VerifyDecodingInfo(const std::optional<webrtc::SdpAudioFormat> sdp_audio_format, const std::optional<webrtc::SdpVideoFormat> sdp_video_format,
        const bool video_spatial_scalability, const CodecSupport support)
    {
        auto video_decoder_factory = std::make_unique<MockVideoDecoderFactory>();
        rtc::scoped_refptr<webrtc::AudioDecoderFactory> audio_decoder_factory = blink::CreateWebrtcAudioDecoderFactory();
        if (sdp_video_format) {
            ON_CALL(*video_decoder_factory, QueryCodecSupport)
                .WillByDefault(
                    testing::Invoke([sdp_video_format, video_spatial_scalability, support](const webrtc::SdpVideoFormat& format, bool spatial_scalability) {
                        EXPECT_TRUE(format.IsSameCodec(*sdp_video_format));
                        EXPECT_EQ(spatial_scalability, video_spatial_scalability);
                        return support;
                    }));
            EXPECT_CALL(*video_decoder_factory, QueryCodecSupport).Times(::testing::AtMost(1));
        }
        WebrtcDecodingInfoHandler decoding_info_handler(std::move(video_decoder_factory), audio_decoder_factory);
        MediaCapabilitiesDecodingInfoCallback decoding_info_callback;

        decoding_info_handler.DecodingInfo(sdp_audio_format, sdp_video_format, video_spatial_scalability,
            base::BindOnce(&MediaCapabilitiesDecodingInfoCallback::OnWebrtcDecodingInfoSupport, base::Unretained(&decoding_info_callback)));
        ...
        EXPECT_EQ(decoding_info_callback.IsPowerEfficient(), support.is_power_efficient);
    }
};

TEST_F(WebrtcDecodingInfoHandlerTests, BasicAudio)
{
    VerifyDecodingInfo(kAudioFormatOpus, /*sdp_video_format=*/std::nullopt,
        /*video_spatial_scalability=*/false, CodecSupport { /*is_supported=*/true, /*is_power_efficient=*/true });
}

TEST_F(WebrtcDecodingInfoHandlerTests, UnsupportedAudio)
{
    VerifyDecodingInfo(kAudioFormatFoo, /*sdp_video_format=*/std::nullopt,
        /*video_spatial_scalability=*/false, CodecSupport { /*is_supported=*/false, /*is_power_efficient=*/false });
}
...
TEST_F(WebrtcDecodingInfoHandlerTests, BasicVideo)
{
    VerifyDecodingInfo(
        ...
        /*video_spatial_scalability=*/false, CodecSupport { /*is_supported=*/true, /*is_power_efficient=*/false });
}
...
TEST_F(WebrtcDecodingInfoHandlerTests, VideoWithReferenceScaling)
{
    VerifyDecodingInfo(
        /*sdp _audio_format=*/std::nullopt, kVideoFormatVp9,
        /*video_spatial_scalability=*/true, CodecSupport { /*is_supported=*/true, /*is_power_efficient=*/false });
}

TEST_F(WebrtcDecodingInfoHandlerTests, SupportedAudioUnsupportedVideo)
{
    VerifyDecodingInfo(kAudioFormatOpus, kVideoFormatFoo,
        /*video_spatial_scalability=*/false, CodecSupport { /*is_supported=*/false, /*is_power_efficient=*/false });
}

TEST_F(WebrtcDecodingInfoHandlerTests, SupportedVideoUnsupportedAudio)
{
    VerifyDecodingInfo(kAudioFormatFoo, kVideoFormatVp9,
        /*video_spatial_scalability=*/false, CodecSupport { /*is_supported=*/false, /*is_power_efficient=*/false });
}
