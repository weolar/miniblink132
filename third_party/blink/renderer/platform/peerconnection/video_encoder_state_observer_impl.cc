...
public:
    ...
    bool FirstFrameEncodeCalled() const
    {
        return first_frame_encode_called_;
    }
    void MarkFirstFrameEncodeCalled()
    {
        first_frame_encode_called_ = true;
    }
    ...
    {
        while (!encode_starts_.empty() && encode_starts_.front().rtp_timestamp != rtp_timestamp) {
            encode_starts_.pop();
        }
    ...
    std::optional<VideoEncoderStateObserverImpl::TopLayerInfo> TopLayer() const
    {
        ...
        int active_vec_size = base::saturated_cast<int>(codec_config_.active_spatial_layers.size());

        using TopLayerInfo = VideoEncoderStateObserverImpl::TopLayerInfo;
        std::optional<TopLayerInfo> top_layer;
        if (codec.codecType == webrtc::VideoCodecType::kVideoCodecVP9 && codec.VP9().numberOfSpatialLayers > 0) {
            for (int i = 0; i < codec.VP9().numberOfSpatialLayers; ++i) {
                const webrtc::SpatialLayer& stream = codec.spatialLayers[i];
                int pixel_rate = (active_vec_size >= i + 1 && codec_config_.active_spatial_layers[i] ? 1 : 0) * (stream.active ? 1 : 0) * stream.width
                    * stream.height * base::checked_cast<int>(stream.maxFramerate);
                if (!top_layer || top_layer->pixel_rate <= pixel_rate) {
                    top_layer = TopLayerInfo { .encoder_id = 0, .spatial_id = i, .pixel_rate = pixel_rate };
                }
            }
        } else {
        ...
        if (!top_layer) {
            ...
            top_layer
                = TopLayerInfo { .encoder_id = 0, .spatial_id = 0, .pixel_rate = codec.width * codec.height * base::checked_cast<int>(codec.maxFramerate) };
        }
        return top_layer;
    }
...
VideoEncoderStateObserverImpl::VideoEncoderStateObserverImpl(
    media::VideoCodecProfile profile, const StatsCollector::StoreProcessingStatsCB& store_processing_stats_cb)
...
void VideoEncoderStateObserverImpl::OnEncoderCreated(int encoder_id, const webrtc::VideoCodec& config)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
    ...
    CHECK(encoder_state_by_id_
              .insert_or_assign(encoder_id, std::make_unique<EncoderState>(EncoderState::CodecConfig { config, std::move(active_spatial_layers) }))
              .second);
...
void VideoEncoderStateObserverImpl::OnEncoderDestroyed(int encoder_id)
{
    ...
    if (active_stats_collection() && samples_collected() >= kMinSamplesThreshold) {
        ReportStats();
...
void VideoEncoderStateObserverImpl::OnRatesUpdated(int encoder_id, const Vector<bool>& active_spatial_layers)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
    EncoderState* encoder_state = GetEncoderState(encoder_id);
    if (!encoder_state) {
...
VideoEncoderStateObserverImpl::EncoderState* VideoEncoderStateObserverImpl::GetEncoderState(int encoder_id, base::Location location)
{
    auto it = encoder_state_by_id_.find(encoder_id);
    if (it == encoder_state_by_id_.end()) {
        LOG(WARNING) << "No encoder id: " << encoder_id << " (" << location.function_name() << ")";
        return nullptr;
    }
    return it->second.get();
}

void VideoEncoderStateObserverImpl::OnEncode(int encoder_id, uint32_t rtp_timestamp)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
    EncoderState* encoder_state = GetEncoderState(encoder_id);
    if (!encoder_state) {
...
void VideoEncoderStateObserverImpl::OnEncodedImage(int encoder_id, const EncodeResult& result)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
    ...
    if (encoder_id != top_encoder_info_->encoder_id || result.spatial_index.value_or(0) != top_encoder_info_->spatial_id) {
        return;
    }
    ...
    const float encode_time_ms = (result.encode_end_time - encode_start->time).InMillisecondsF();
    const int pixel_size = result.width * result.height;
    AddProcessingTime(pixel_size, result.is_hardware_accelerated, encode_time_ms, result.keyframe, now);
}

void VideoEncoderStateObserverImpl::UpdateStatsCollection(base::TimeTicks now)
{
    constexpr base::TimeDelta kCheckUpdateStatsCollectionInterval = base::Seconds(5);
    if ((now - last_update_stats_collection_time_) < kCheckUpdateStatsCollectionInterval) {
        return;
    }
...
std::optional<VideoEncoderStateObserverImpl::TopLayerInfo> VideoEncoderStateObserverImpl::FindHighestActiveEncoding() const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
    std::optional<TopLayerInfo> top_info;
    for (const auto& kv : encoder_state_by_id_) {
        std::optional<TopLayerInfo> top_of_encoder = kv.second->TopLayer();
        if (top_of_encoder && (!top_info || top_info->pixel_rate < top_of_encoder->pixel_rate)) {
...
#if DCHECK_IS_ON()
    if (top_info
        && (!top_encoder_info_ || top_encoder_info_->encoder_id != top_info->encoder_id || top_encoder_info_->spatial_id != top_info->spatial_id
            || top_encoder_info_->pixel_rate != top_info->pixel_rate)) {
        DVLOG(3) << "New top resolution configured for video encoder: encoder id = " << top_info->encoder_id << ", spatial id = " << top_info->spatial_id
