...
private:
    ...
    const scoped_refptr<MetronomeSource::TickProvider> default_tick_provider_ GUARDED_BY_CONTEXT(sequence_checker_);

    // The currently scheduled tick callback.
    WTF::Vector<base::OnceClosure> tick_callbacks_ GUARDED_BY_CONTEXT(sequence_checker_);
