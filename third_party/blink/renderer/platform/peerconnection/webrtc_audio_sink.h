...
public:
    WebRtcAudioSink(const std::string& label, scoped_refptr<webrtc::AudioSourceInterface> track_source,
    ...
    void SetAudioProcessor(scoped_refptr<webrtc::AudioProcessorInterface> processor);
...
private:
    ...
    public:
        Adapter(const std::string& label, scoped_refptr<webrtc::AudioSourceInterface> source, scoped_refptr<base::SingleThreadTaskRunner> signaling_task_runner,
        ...
        void set_processor(scoped_refptr<webrtc::AudioProcessorInterface> processor)
        {
            audio_processor_ = std::move(processor);
        }
        void set_level(scoped_refptr<MediaStreamAudioLevelCalculator::Level> level)
        {
    ...
    template <typename> friend struct WTF::CrossThreadCopier;
    ...
    // WebMediaStreamAudioSink implementation.
    int NumPreferredChannels() override
    {
        return num_preferred_channels_;
    }
    ...
    void DeliverRebufferedAudio(const media::AudioBus& audio_bus, int frame_delay);
