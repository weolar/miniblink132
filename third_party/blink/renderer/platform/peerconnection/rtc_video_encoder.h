...
public:
    RTCVideoEncoder(media::VideoCodecProfile profile, bool is_constrained_h264, media::GpuVideoAcceleratorFactories* gpu_factories,
    ...
    void SetErrorCallbackForTesting(WTF::CrossThreadOnceClosure error_callback_for_testing)
    {
...
private:
    ...
    int32_t InitializeEncoder(const media::VideoEncodeAccelerator::Config& vea_config);
    void UpdateEncoderInfo(media::VideoEncoderInfo encoder_info, std::vector<webrtc::VideoFrameBuffer::Type> preferred_pixel_formats);
    void SetError(uint32_t impl_id);
    void ReleaseImpl();

    bool CodecSettingsUsableForFrameSizeChange(const webrtc::VideoCodec& codec_settings) const;

    int32_t DrainEncoderAndUpdateFrameSize(const gfx::Size& input_visible_size, const webrtc::VideoEncoder::RateControlParameters& params,
