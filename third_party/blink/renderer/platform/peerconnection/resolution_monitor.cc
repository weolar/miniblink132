...
public:
    ...
    std::optional<gfx::Size> GetResolution(const media::DecoderBuffer& buffer) override
    {
        ...
        if (!parser.ParseFrame(buffer.data(), buffer.size(), &frame_header)) {
            ...
            current_resolution_ = gfx::Size(base::saturated_cast<int>(frame_header.width), base::saturated_cast<int>(frame_header.height));
        }
...
public:
    ...
    std::optional<gfx::Size> GetResolution(const media::DecoderBuffer& buffer) override
    {
        ...
        parser_.SetStream(buffer.data(), base::checked_cast<off_t>(buffer.size()), frame_sizes, /*stream_config=*/nullptr);
        ...
        while (GetNextFrameSize(frame_size, parse_error)) {
            if (max_resolution.value_or(gfx::Size()).GetArea() < frame_size.GetArea()) {
...
private:
    bool GetNextFrameSize(gfx::Size& frame_size, bool& parse_error)
    {
        ...
        media::Vp9Parser::Result result = parser_.ParseNextFrame(&frame_header, &allocate_size, /*frame_decrypt_config=*/nullptr);
        switch (result) {
...
public:
    ...
    {
        auto parser
            = base::WrapUnique(new (std::nothrow) libgav1::ObuParser(buffer.data(), buffer.size(), kDefaultOperatingPoint, &buffer_pool_, &decoder_state_));
        ...
        while (parser->HasData()) {
            ...
            if (status_code != libgav1::kStatusOk) {
                DLOG(ERROR) << "Failed parsing av1 frame: " << static_cast<int>(status_code);
                return std::nullopt;
            }
            if (!current_frame) {
            ...
            if (parser->sequence_header_changed() && !UpdateCurrentSequenceHeader(parser->sequence_header())) {
                return std::nullopt;
            }

            std::optional<gfx::Size> frame_size = GetFrameSizeFromHeader(parser->frame_header());
            if (!frame_size) {
                return std::nullopt;
            }
            if (max_resolution.value_or(gfx::Size()).GetArea() < frame_size->GetArea()) {
            ...
            decoder_state_.UpdateReferenceFrames(current_frame, base::strict_cast<int>(parser->frame_header().refresh_frame_flags));
        }
...
private:
    ...
    bool UpdateCurrentSequenceHeader(const libgav1::ObuSequenceHeader& sequence_header)
    {
        int operating_point_idc = sequence_header.operating_point_idc[kDefaultOperatingPoint];
        if (IsSpatialLayerDecoding(operating_point_idc)) {
            ...
            DVLOG(1) << "Spatial layer decoding is not supported: "
                     << "operating_point_idc=" << std::bitset<kOperatingPointIdcBits>(operating_point_idc);
            return false;
        }
    ...
    std::optional<gfx::Size> GetFrameSizeFromHeader(const libgav1::ObuFrameHeader& frame_header) const
    {
        ...
        CHECK_LT(frame_to_show, static_cast<size_t>(libgav1::kNumReferenceFrameTypes));
        const libgav1::RefCountedBufferPtr show_frame = decoder_state_.reference_frame[frame_to_show];
        if (!show_frame) {
...
public:
    ...
    std::optional<gfx::Size> GetResolution(const media::DecoderBuffer& buffer) override
    {
        ...
        std::vector<webrtc::H264::NaluIndex> nalu_indices = webrtc::H264::FindNaluIndices(webrtc_buffer);
        for (const auto& nalu_index : nalu_indices) {
            ...
            auto nalu_payload = webrtc_buffer.subview(nalu_index.payload_start_offset, nalu_index.payload_size);
            if (webrtc::H264::ParseNaluType(nalu_payload[0]) == webrtc::H264::NaluType::kSps) {
                // Parse without NALU header.
                std::optional<webrtc::SpsParser::SpsState> sps = webrtc::SpsParser::ParseSps(nalu_payload.subview(webrtc::H264::kNaluTypeSize));
...
// static
std::unique_ptr<ResolutionMonitor> ResolutionMonitor::Create(media::VideoCodec codec)
{
    switch (codec) {
