...
// encoders.
class PLATFORM_EXPORT InstrumentedSimulcastAdapter : public webrtc::SimulcastEncoderAdapter {
public:
    static std::unique_ptr<InstrumentedSimulcastAdapter> Create(const webrtc::Environment& env, webrtc::VideoEncoderFactory* primary_encoder_factory,
        webrtc::VideoEncoderFactory* secondate_encoder_factory, std::unique_ptr<VideoEncoderStateObserver> encoder_state_observer,
...
private:
    ...
    InstrumentedSimulcastAdapter(const webrtc::Environment& env, std::unique_ptr<EncoderFactoryAdapter> primary_factory_adapter,
