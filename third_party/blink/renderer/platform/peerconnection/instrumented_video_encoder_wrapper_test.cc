...
public:
    ...
    MOCK_METHOD(void, OnEncoderCreated, (int, const webrtc::VideoCodec&), (override));
...
public:
    MOCK_METHOD(webrtc::EncodedImageCallback::Result, OnEncodedImage, (const webrtc::EncodedImage&, const webrtc::CodecSpecificInfo*), (override));
    MOCK_METHOD(void, OnDroppedFrame, (webrtc::EncodedImageCallback::DropReason), (override));
};
...
webrtc::VideoFrame CreateFrame(int width = kWidth, int height = kHeight, uint64_t timestamp_us = kTimestamp)
{
...
webrtc::VideoCodec CreateVideoCodec(int width = kWidth, int height = kHeight, webrtc::VideoCodecType codec_type = webrtc::kVideoCodecVP8)
{
...
const webrtc::VideoEncoder::Settings kEncoderSettings(webrtc::VideoEncoder::Capabilities(/*loss_notification=*/false),
    /*number_of_cores=*/1,
...
public:
    ...
    void SetUp() override
    {
        ...
        wrapper_ = std::make_unique<InstrumentedVideoEncoderWrapper>(
            /*id=*/kEncoderId, std::move(fake_encoder), static_cast<VideoEncoderStateObserver*>(mock_state_observer_.get()));
    }
...
{
    EXPECT_CALL(*mock_state_observer_, OnEncoderCreated(kEncoderId, Field(&webrtc::VideoCodec::width, kVideoCodec.width)));
    EXPECT_EQ(wrapper_->InitEncode(&kVideoCodec, kEncoderSettings), WEBRTC_VIDEO_CODEC_OK);
...
TEST_F(InstrumentedVideoEncoderWrapperTest, Encode)
{
    ...
    EXPECT_EQ(wrapper_->RegisterEncodeCompleteCallback(&encoded_image_callback), WEBRTC_VIDEO_CODEC_OK);
    auto frame = CreateFrame();
    std::vector<webrtc::VideoFrameType> frame_types { webrtc::VideoFrameType::kVideoFrameKey };
    EXPECT_CALL(*mock_state_observer_, OnEncode(kEncoderId, frame.rtp_timestamp()));
    EXPECT_CALL(*mock_state_observer_,
        OnEncodedImage(kEncoderId,
            AllOf(Field(&EncodeResult::width, kWidth), Field(&EncodeResult::height, kHeight), Field(&EncodeResult::keyframe, true),
                Field(&EncodeResult::spatial_index, std::nullopt), Field(&EncodeResult::rtp_timestamp, frame.rtp_timestamp()),
                Field(&EncodeResult::is_hardware_accelerated, false))));
    EXPECT_CALL(encoded_image_callback, OnEncodedImage(_, _)).WillOnce(Return(webrtc::EncodedImageCallback::Result(webrtc::EncodedImageCallback::Result::OK)));
...
TEST_F(InstrumentedVideoEncoderWrapperTest, SetRates)
{
    ...
    EXPECT_CALL(*mock_state_observer_, OnRatesUpdated(kEncoderId, kExpectedActiveLayers));
    wrapper_->SetRates(webrtc::VideoEncoder::RateControlParameters(bitrate_allocation, kNewFrameRate));
}
} // namespace blink
