...
class PLATFORM_EXPORT VideoEncoderStateObserverImpl : public VideoEncoderStateObserver, public StatsCollector {
public:
    struct TopLayerInfo {
        int encoder_id;
        int spatial_id;
    ...
    VideoEncoderStateObserverImpl(media::VideoCodecProfile profile, const StatsCollector::StoreProcessingStatsCB& store_processing_stats_cb);
    ~VideoEncoderStateObserverImpl() override;

    // VideoEncoderStateObserver implementation.
    void OnEncoderCreated(int encoder_id, const webrtc::VideoCodec& config) override;
    void OnEncoderDestroyed(int encoder_id) override;
    void OnRatesUpdated(int encoder_id, const Vector<bool>& active_spatial_layers) override;
...
private:
    ...
    EncoderState* GetEncoderState(int encoder_id, base::Location location = base::Location::Current()) VALID_CONTEXT_REQUIRED(encoder_sequence_);
    void UpdateStatsCollection(base::TimeTicks now) VALID_CONTEXT_REQUIRED(encoder_sequence_);

    base::flat_map<int, std::unique_ptr<EncoderState>> encoder_state_by_id_ GUARDED_BY_CONTEXT(encoder_sequence_);
    std::optional<TopLayerInfo> top_encoder_info_ GUARDED_BY_CONTEXT(encoder_sequence_);

    base::TimeTicks last_update_stats_collection_time_ GUARDED_BY_CONTEXT(encoder_sequence_);
