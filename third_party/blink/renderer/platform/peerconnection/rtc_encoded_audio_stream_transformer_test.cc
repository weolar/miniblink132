...
public:
    MOCK_METHOD1(OnEncodedFrame, void(std::unique_ptr<webrtc::TransformableAudioFrameInterface>));
};
...
public:
    ...
    void SetUp() override
    {
        EXPECT_FALSE(encoded_audio_stream_transformer_.HasTransformedFrameCallback());
        encoded_audio_stream_transformer_.RegisterTransformedFrameCallback(webrtc_callback_);
        EXPECT_TRUE(encoded_audio_stream_transformer_.HasTransformedFrameCallback());
    }
...
TEST_F(RTCEncodedAudioStreamTransformerTest, TransformerForwardsFrameToTransformerCallback)
{
    ...
    PostCrossThreadTask(*webrtc_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&webrtc::FrameTransformerInterface::Transform, encoded_audio_stream_transformer_.Delegate(), nullptr));
...
TEST_F(RTCEncodedAudioStreamTransformerTest, ShortCircuitingSetOnLateRegisteredCallback)
{
    EXPECT_CALL(*webrtc_callback_, StartShortCircuiting);
    encoded_audio_stream_transformer_.StartShortCircuiting();
    ...
    encoded_audio_stream_transformer_.RegisterTransformedFrameCallback(webrtc_callback_2);
}
