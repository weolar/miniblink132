...
public:
    ...
    static std::unique_ptr<RTCVideoDecoderAdapter> Create(media::GpuVideoAcceleratorFactories* gpu_factories, const webrtc::SdpVideoFormat& format,
...
private:
    ...
    std::optional<DecodeResult> DecodeInternal(const webrtc::EncodedImage& input_image, bool missing_frames, int64_t render_time_ms);
    bool ShouldReinitializeForSettingColorSpace(const webrtc::EncodedImage& input_image) const;
    ...
    const std::unique_ptr<ResolutionMonitor> resolution_monitor_ GUARDED_BY_CONTEXT(decoding_sequence_checker_);

    // Decoding thread members.
    // Has anything been sent to Decode() yet?
    Status status_ GUARDED_BY_CONTEXT(decoding_sequence_checker_) { Status::kNeedKeyFrame };

    // DecoderInfo is constant after InitializeSync() is complete.
    DecoderInfo decoder_info_;

    bool have_started_decoding_ GUARDED_BY_CONTEXT(decoding_sequence_checker_) { false };

    media::VideoDecoderType decoder_type_ GUARDED_BY_CONTEXT(decoding_sequence_checker_) { media::VideoDecoderType::kUnknown };
