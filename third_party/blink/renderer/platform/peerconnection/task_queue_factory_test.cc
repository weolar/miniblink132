...
public:
    TestTaskQueueFactory()
        : factory_(CreateWebRtcTaskQueueFactory())
    {
    }

    std::unique_ptr<webrtc::TaskQueueBase, webrtc::TaskQueueDeleter> CreateTaskQueue(std::string_view name, Priority priority) const override
    {
...
std::unique_ptr<webrtc::TaskQueueFactory> CreateTaskQueueFactory(const webrtc::FieldTrialsView*)
{
...
INSTANTIATE_TEST_SUITE_P(WebRtcTaskQueue, TaskQueueTest, ::testing::Values(CreateTaskQueueFactory));
...
public:
    ...
    {
        task_queue_ = CreateWebRtcTaskQueueFactory()->CreateTaskQueue("TestTaskQueue", webrtc::TaskQueueFactory::Priority::NORMAL);
    }

    base::TimeDelta DeltaToNextTick() const override
    {
        base::TimeTicks now = base::TimeTicks::Now();
        return TimerBasedTickProvider::TimeSnappedToNextTick(now, TimerBasedTickProvider::kDefaultPeriod) - now;
    }
...
INSTANTIATE_TEST_SUITE_P(WebRtcTaskQueue, MetronomeLikeTaskQueueTest, ::testing::Values(std::make_unique<TaskQueueProvider>));
