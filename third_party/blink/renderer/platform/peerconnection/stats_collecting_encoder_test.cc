...
constexpr media::VideoCodecProfile kCodecProfile = media::VideoCodecProfile::VP9PROFILE_PROFILE0;
...
webrtc::VideoFrame CreateMockFrame(int width, int height, uint32_t timestamp)
{
    return webrtc::VideoFrame::Builder()
        .set_video_frame_buffer(rtc::make_ref_counted<FakeVideoFrameBuffer>(width, height))
...
public:
    explicit MockEncoder(base::test::TaskEnvironment* task_environment, int* spatial_layers, bool* is_hw_accelerated)
    ...
    int32_t Encode(const webrtc::VideoFrame& frame, const std::vector<webrtc::VideoFrameType>* frame_types) override
    {
        ...
        encoded_frame._frameType = frame_types && !frame_types->empty() ? frame_types->at(0) : webrtc::VideoFrameType::kVideoFrameDelta;

        base::TimeDelta encode_time = base::Milliseconds(frame_counter_++ % 100 < 90 ? kMinEncodingTimeMs : kExpectedP99ProcessingTimeMs);
        task_environment_->AdvanceClock(encode_time);
        for (int i = 0; i < *spatial_layers_; ++i) {
            encoded_frame._encodedWidth = frame.width() / (1 << (*spatial_layers_ - i - 1));
            encoded_frame._encodedHeight = frame.height() / (1 << (*spatial_layers_ - i - 1));
    ...
    MOCK_METHOD1(SetFecControllerOverride, void(webrtc::FecControllerOverride* fec_controller_override));
    MOCK_METHOD2(InitEncode, int(const webrtc::VideoCodec* codec_settings, const webrtc::VideoEncoder::Settings& settings));
    ...
    MOCK_METHOD1(OnLossNotification, void(const LossNotification& loss_notification));
...
public:
    StatsCollectingEncoderTest()
        : internal_encoder_(new MockEncoder(&task_environment_, &spatial_layers_, &is_hw_accelerated_))
        , stats_encoder_(kFormatVp9, std::unique_ptr<MockEncoder>(internal_encoder_),
              base::BindRepeating(&StatsCollectingEncoderTest::StoreProcessingStatsCB, base::Unretained(this)))
    {
    ...
    void StoreProcessingStatsCB(const StatsCollector::StatsKey& stats_key, const StatsCollector::VideoStats& video_stats)
    {
        ++stats_callbacks_;
    ...
    void CreateAndEncodeFrames(int width, int height, int spatial_layers, bool is_hw_accelerated, int frames, int key_frame_interval, int frame_rate)
    {
        CreateAndEncodeFrames(&stats_encoder_, width, height, spatial_layers, is_hw_accelerated, frames, key_frame_interval, frame_rate);
    }

    void CreateAndEncodeFrames(
        StatsCollectingEncoder* encoder, int width, int height, int spatial_layers, bool is_hw_accelerated, int frames, int key_frame_interval, int frame_rate)
    {
        spatial_layers_ = spatial_layers;
        is_hw_accelerated_ = is_hw_accelerated;
        ...
        for (int i = 0; i < frames; ++i) {
            ...
            webrtc::VideoFrame video_frame = CreateMockFrame(width, height, 90000 * frame_counter_ / frame_rate);
            std::vector<webrtc::VideoFrameType> video_frame_types
                = { frame_counter_ % key_frame_interval == 0 ? webrtc::VideoFrameType::kVideoFrameKey : webrtc::VideoFrameType::kVideoFrameDelta };
            ++frame_counter_;
            ...
            if (begin_encode + frame_interval > end_encode) {
                task_environment_.AdvanceClock(frame_interval + begin_encode - end_encode);
            }
        }
    }
...
TEST_F(StatsCollectingEncoderTest, StoreProcessingStatsCallbackHdSw)
{
    ...
    CreateAndEncodeFrames(kHdWidth, kHdHeight, /*spatial_layers=*/1,
        /*is_hw_accelerated=*/false, kFrames, kKeyframeInterval, kFramerate);
    ...
    EXPECT_NEAR(last_video_stats_.key_frame_count, last_video_stats_.frame_count / kKeyframeInterval, 1);
    EXPECT_NEAR(last_video_stats_.p99_processing_time_ms, kExpectedP99ProcessingTimeMs, kP99ToleranceMs);
...
TEST_F(StatsCollectingEncoderTest, StoreProcessingStatsCallbackFullHdHw)
{
    ...
    CreateAndEncodeFrames(kFullHdWidth, kFullHdHeight, /*spatial_layers=*/1,
        /*is_hw_accelerated=*/true, kFrames, kKeyframeInterval, kFramerate);
    // Verify that there's been one stats callback and that the numbers are
    // reasonable.
    EXPECT_EQ(stats_callbacks_, 1);
    EXPECT_FALSE(last_stats_key_.is_decode);
...
TEST_F(StatsCollectingEncoderTest, StoreProcessingStatsCallbackFullHdSwSpatialLayers)
{
    // P99 not meaningful for less than 100 frames.
    constexpr int kMinimumNumberOfFrames = 100;
    constexpr int kFrames = 200;
    ...
    EXPECT_NEAR(last_video_stats_.key_frame_count, last_video_stats_.frame_count / kKeyframeInterval, 1);
    EXPECT_NEAR(last_video_stats_.p99_processing_time_ms, kExpectedP99ProcessingTimeMs, kP99ToleranceMs);
    EXPECT_EQ(encoded_image_callback_.get_frame_counter(), kFrames * kSpatialLayers);
}

TEST_F(StatsCollectingEncoderTest, CollectionStopsIfThereAreMultipleEncodersActive)
{
    constexpr int kMinutesToRun = 10;
    EXPECT_EQ(stats_callbacks_, 0);
    CreateAndEncodeFrames(kHdWidth, kHdHeight, /*spatial_layers=*/1,
        /*is_hw_accelerated=*/false, kFramesPerMinute, kKeyframeInterval, kFramerate);
    ...
    StatsCollectingEncoder second_stats_encoder(
        kFormatVp9, std::make_unique<MockEncoder>(&task_environment_, &second_encoder_spatial_layers, &second_encoder_is_hw_accelerated), base::DoNothing());
    second_stats_encoder.RegisterEncodeCompleteCallback(&second_encoded_image_callback);
    // Encode one frame to make it an active encoder.
    CreateAndEncodeFrames(&second_stats_encoder, kHdWidth, kHdHeight, /*spatial_layers=*/1,
...
TEST_F(StatsCollectingEncoderTest, MethodCallsForwardedToInternalEncoder)
{
    ...
    // correctly.
    webrtc::FecControllerOverride* fec_controller_override = reinterpret_cast<webrtc::FecControllerOverride*>(0x123456);
    EXPECT_CALL(*internal_encoder_, SetFecControllerOverride(fec_controller_override));
    stats_encoder_.SetFecControllerOverride(fec_controller_override);

    webrtc::VideoCodec* codec_settings_ptr = reinterpret_cast<webrtc::VideoCodec*>(0x123456);
    ...
    EXPECT_CALL(*internal_encoder_, InitEncode)
        .WillOnce([codec_settings_ptr, &kSettings](const webrtc::VideoCodec* codec_settings, const webrtc::VideoEncoder::Settings& settings) {
            EXPECT_EQ(codec_settings_ptr, codec_settings);
            EXPECT_EQ(kSettings.capabilities.loss_notification, settings.capabilities.loss_notification);
    ...
    EXPECT_EQ(stats_encoder_.InitEncode(codec_settings_ptr, kSettings), kExpectedInitReturnValue);

    const webrtc::VideoBitrateAllocation kVideoBitrateAllocation;
    const webrtc::VideoEncoder::RateControlParameters kParameters = { kVideoBitrateAllocation, /*framerate_fps=*/24.0 };
    ...
    const webrtc::VideoEncoder::LossNotification kLossNotification { /*timestamp_of_last_decodable=*/7u, /*timestamp_of_last_received=*/9u };
    EXPECT_CALL(*internal_encoder_, OnLossNotification).WillOnce([&kLossNotification](const webrtc::VideoEncoder::LossNotification& loss_notification) {
        EXPECT_EQ(loss_notification.timestamp_of_last_decodable, kLossNotification.timestamp_of_last_decodable);
        EXPECT_EQ(loss_notification.timestamp_of_last_received, kLossNotification.timestamp_of_last_received);
    });
