...
void ExpectUpdateRectEquals(const gfx::Rect& expected, const webrtc::VideoFrame::UpdateRect actual)
{
...
{
    bool called = false;
    scoped_refptr<WebRtcVideoTrackSource> track_source = new rtc::RefCountedObject<WebRtcVideoTrackSource>(
        /*is_screencast=*/false,
        /*needs_denoising=*/std::nullopt, base::BindLambdaForTesting([](const media::VideoCaptureFeedback&) {}),
...
class WebRtcVideoTrackSourceTest : public ::testing::TestWithParam<std::tuple<media::VideoFrame::StorageType, media::VideoPixelFormat>>,
                                   public media::FakeGpuMemoryBuffer::MapCallbackController {
public:
    WebRtcVideoTrackSourceTest()
        : shared_resources_(base::MakeRefCounted<WebRtcVideoFrameAdapter::SharedResources>(
            /*gpu_factories=*/nullptr))
        , track_source_(new rtc::RefCountedObject<WebRtcVideoTrackSource>(
              /*is_screencast=*/false,
              /*needs_denoising=*/std::nullopt, base::BindRepeating(&WebRtcVideoTrackSourceTest::ProcessFeedback, base::Unretained(this)),
              base::BindLambdaForTesting([] {}),
              /*gpu_factories=*/nullptr, shared_resources_))
    {
    ...
    void SendTestFrame(const FrameParameters& frame_parameters, base::TimeDelta timestamp)
    {
        scoped_refptr<media::VideoFrame> frame = CreateTestFrame(frame_parameters.coded_size, frame_parameters.visible_rect, frame_parameters.natural_size,
            frame_parameters.storage_type, frame_parameters.pixel_format, timestamp);
        track_source_->OnFrameCaptured(frame);
    }

    void SendTestFrameWithMappableGMB(const FrameParameters& frame_parameters, base::TimeDelta timestamp, bool premapped)
    {
        std::unique_ptr<media::FakeGpuMemoryBuffer> fake_gmb = std::make_unique<media::FakeGpuMemoryBuffer>(
            frame_parameters.coded_size, media::VideoPixelFormatToGfxBufferFormat(frame_parameters.pixel_format).value(), premapped, this);
        scoped_refptr<media::VideoFrame> frame = CreateTestFrame(frame_parameters.coded_size, frame_parameters.visible_rect, frame_parameters.natural_size,
            frame_parameters.storage_type, frame_parameters.pixel_format, timestamp, std::move(fake_gmb));
        track_source_->OnFrameCaptured(frame);
    }

    void SendTestFrameAndVerifyFeedback(const FrameParameters& frame_parameters, int max_pixels, float max_framerate)
    {
        scoped_refptr<media::VideoFrame> frame = CreateTestFrame(frame_parameters.coded_size, frame_parameters.visible_rect, frame_parameters.natural_size,
            frame_parameters.storage_type, frame_parameters.pixel_format, base::TimeDelta());
        track_source_->OnFrameCaptured(frame);
        EXPECT_EQ(feedback_.max_pixels, max_pixels);
        EXPECT_EQ(feedback_.max_framerate_fps, max_framerate);
    }

    void SendTestFrameWithUpdateRect(const FrameParameters& frame_parameters, int capture_counter, const gfx::Rect& update_rect)
    {
        scoped_refptr<media::VideoFrame> frame = CreateTestFrame(frame_parameters.coded_size, frame_parameters.visible_rect, frame_parameters.natural_size,
            frame_parameters.storage_type, frame_parameters.pixel_format, base::TimeDelta());
    ...
    void SendTestFrameWithColorSpace(const FrameParameters& frame_parameters, const gfx::ColorSpace& color_space)
    {
        scoped_refptr<media::VideoFrame> frame = CreateTestFrame(frame_parameters.coded_size, frame_parameters.visible_rect, frame_parameters.natural_size,
            frame_parameters.storage_type, frame_parameters.pixel_format, base::TimeDelta());
    ...
    WebRtcVideoTrackSource::FrameAdaptationParams FrameAdaptation_KeepAsIs(const gfx::Size& natural_size)
    {
        return WebRtcVideoTrackSource::FrameAdaptationParams {
            false /*should_drop_frame*/, 0 /*crop_x*/, 0 /*crop_y*/, natural_size.width() /*crop_width*/, natural_size.height() /*crop_height*/,
            natural_size.width() /*scale_to_width*/, natural_size.height() /*scale_to_height*/
        };
    }
    ...
    {
        return WebRtcVideoTrackSource::FrameAdaptationParams {
            true /*should_drop_frame*/, 0 /*crop_x*/, 0 /*crop_y*/, 0 /*crop_width*/, 0 /*crop_height*/, 0 /*scale_to_width*/, 0 /*scale_to_height*/
        };
    }

    WebRtcVideoTrackSource::FrameAdaptationParams FrameAdaptation_Scale(const gfx::Size& natural_size, const gfx::Size& scale_to_size)
    {
        return WebRtcVideoTrackSource::FrameAdaptationParams {
            false /*should_drop_frame*/, 0 /*crop_x*/, 0 /*crop_y*/, natural_size.width() /*crop_width*/, natural_size.height() /*crop_height*/,
            scale_to_size.width() /*scale_to_width*/, scale_to_size.height() /*scale_to_height*/
        };
    }

    void SetRequireMappedFrame(bool require_mapped_frame)
    {
        shared_resources_->SetFeedback(media::VideoCaptureFeedback().RequireMapped(require_mapped_frame));
    }
...
std::vector<WebRtcVideoTrackSourceTest::ParamType> TestParams()
{
    ...
    for (media::VideoPixelFormat format : GetPixelFormatsMappableToWebRtcVideoFrameBuffer()) {
        test_params.emplace_back(media::VideoFrame::StorageType::STORAGE_OWNED_MEMORY, format);
    }
    test_params.emplace_back(media::VideoFrame::StorageType::STORAGE_GPU_MEMORY_BUFFER, media::VideoPixelFormat::PIXEL_FORMAT_NV12);
    test_params.emplace_back(media::VideoFrame::STORAGE_OPAQUE, media::VideoPixelFormat::PIXEL_FORMAT_NV12);
    return test_params;
}
} // namespace
...
TEST_P(WebRtcVideoTrackSourceTest, TestTimestamps)
{
    ...
    Sequence s;
    EXPECT_CALL(mock_sink_, OnFrame(_)).InSequence(s).WillOnce(Invoke([](const webrtc::VideoFrame& frame) {
...
TEST_P(WebRtcVideoTrackSourceTest, CropFrameTo640360)
{
    ...
    track_source_->SetCustomFrameAdaptationParamsForTesting(FrameAdaptation_KeepAsIs(kNaturalSize));

    EXPECT_CALL(mock_sink_, OnFrame(_)).WillOnce(Invoke([kNaturalSize](const webrtc::VideoFrame& frame) {
...
TEST_P(WebRtcVideoTrackSourceTest, TestColorSpaceSettings)
{
    ...
    EXPECT_CALL(mock_sink_, OnFrame(_)).InSequence(s).WillOnce(Invoke([](const webrtc::VideoFrame& frame) {
        ...
        EXPECT_EQ(frame.color_space().value().matrix(), webrtc::ColorSpace::MatrixID::kSMPTE170M);
        EXPECT_EQ(frame.color_space().value().transfer(), webrtc::ColorSpace::TransferID::kBT709);
        EXPECT_EQ(frame.color_space().value().primaries(), webrtc::ColorSpace::PrimaryID::kBT709);
        EXPECT_EQ(frame.color_space().value().range(), webrtc::ColorSpace::RangeID::kLimited);
    }));
    EXPECT_CALL(mock_sink_, OnFrame(_)).InSequence(s).WillOnce(Invoke([](const webrtc::VideoFrame& frame) {
        ASSERT_TRUE(frame.color_space().has_value());
        EXPECT_EQ(frame.color_space().value().matrix(), webrtc::ColorSpace::MatrixID::kBT709);
        EXPECT_EQ(frame.color_space().value().transfer(), webrtc::ColorSpace::TransferID::kBT709);
        EXPECT_EQ(frame.color_space().value().primaries(), webrtc::ColorSpace::PrimaryID::kBT709);
        EXPECT_EQ(frame.color_space().value().range(), webrtc::ColorSpace::RangeID::kFull);
    }));
    ...
    // invalid.
    EXPECT_CALL(mock_sink_, OnFrame(_)).InSequence(s).WillOnce(Invoke([](const webrtc::VideoFrame& frame) { ASSERT_FALSE(frame.color_space().has_value()); }));

    gfx::ColorSpace color_range_limited(
        gfx::ColorSpace::PrimaryID::BT709, gfx::ColorSpace::TransferID::BT709, gfx::ColorSpace::MatrixID::SMPTE170M, gfx::ColorSpace::RangeID::LIMITED);
...
TEST_P(WebRtcVideoTrackSourceTest, SetsFeedback)
{
    ...
    SendTestFrameAndVerifyFeedback(frame_parameters, kScaleToSize.GetArea(), k5Fps);
}
...
{
    const gfx::Size kNaturalSize(640, 360);
    FrameParameters frame_parameters = { .coded_size = gfx::Size(1280, 720),
...
TEST_P(WebRtcVideoTrackSourceTest, UpdateRectWithNoTransform)
{
    ...
    FrameParameters frame_parameters = { .coded_size = gfx::Size(640, 480),
        ...
        .storage_type = std::get<0>(GetParam()),
        .pixel_format = std::get<1>(GetParam()) };
    track_source_->SetCustomFrameAdaptationParamsForTesting(FrameAdaptation_KeepAsIs(frame_parameters.natural_size));
    ...
    EXPECT_CALL(mock_sink_, OnFrame(_)).WillOnce(Invoke([](const webrtc::VideoFrame& frame) {
        ExpectUpdateRectEquals(gfx::Rect(0, 0, frame.width(), frame.height()), frame.update_rect());
    }));
    ...
    EXPECT_CALL(mock_sink_, OnFrame(_)).WillOnce(Invoke([kUpdateRect1](const webrtc::VideoFrame& frame) {
    ...
    track_source_->SetCustomFrameAdaptationParamsForTesting(FrameAdaptation_DropFrame());
    const gfx::Rect kUpdateRect2(2, 3, 4, 5);
    EXPECT_CALL(mock_sink_, OnFrame(_)).Times(0);
    SendTestFrameWithUpdateRect(frame_parameters, ++capture_counter, kUpdateRect2);
    Mock::VerifyAndClearExpectations(&mock_sink_);
    ...
    EXPECT_CALL(mock_sink_, OnFrame(_)).WillOnce(Invoke([kUpdateRect2, kUpdateRect3](const webrtc::VideoFrame& frame) {
    ...
    SendTestFrameWithUpdateRect(frame_parameters, ++capture_counter, kUpdateRect3);
    Mock::VerifyAndClearExpectations(&mock_sink_);
    ...
    EXPECT_CALL(mock_sink_, OnFrame(_)).WillOnce(Invoke([kVisibleRect](const webrtc::VideoFrame& frame) {
        ExpectUpdateRectEquals(kVisibleRect, frame.update_rect());
    }));
    SendTestFrameWithUpdateRect(frame_parameters, ++capture_counter, kUpdateRect4);
    Mock::VerifyAndClearExpectations(&mock_sink_);
    ...
    EXPECT_CALL(mock_sink_, OnFrame(_)).WillOnce(Invoke([](const webrtc::VideoFrame& frame) { EXPECT_TRUE(frame.update_rect().IsEmpty()); }));
    SendTestFrameWithUpdateRect(frame_parameters, ++capture_counter, kEmptyRectWithZeroOrigin);
    Mock::VerifyAndClearExpectations(&mock_sink_);

    const gfx::Rect kEmptyRectWithNonZeroOrigin(10, 20, 0, 0);
    EXPECT_CALL(mock_sink_, OnFrame(_)).WillOnce(Invoke([](const webrtc::VideoFrame& frame) { EXPECT_TRUE(frame.update_rect().IsEmpty()); }));
    SendTestFrameWithUpdateRect(frame_parameters, ++capture_counter, kEmptyRectWithNonZeroOrigin);
    Mock::VerifyAndClearExpectations(&mock_sink_);
...
TEST_P(WebRtcVideoTrackSourceTest, UpdateRectWithCropFromUpstream)
{
    ...
    EXPECT_CALL(mock_sink_, OnFrame(_)).WillOnce(Invoke([kUpdateRect1, kVisibleRect](const webrtc::VideoFrame& frame) {
    ...
    const gfx::Rect kUpdateRect3(kVisibleRect.x() + 10, kVisibleRect.y() + 8, kVisibleRect.width(), kVisibleRect.height());
    EXPECT_CALL(mock_sink_, OnFrame(_)).WillOnce(Invoke([kVisibleRect](const webrtc::VideoFrame& frame) {
        ExpectUpdateRectEquals(gfx::Rect(10, 8, kVisibleRect.width() - 10, kVisibleRect.height() - 8), frame.update_rect());
    }));
    SendTestFrameWithUpdateRect(frame_parameters, ++capture_counter, kUpdateRect3);
    Mock::VerifyAndClearExpectations(&mock_sink_);
    ...
    // changed.
    const gfx::Rect kVisibleRect2(kVisibleRect.x() + 1, kVisibleRect.y(), kVisibleRect.width(), kVisibleRect.height());
    ...
    // changed.
    const gfx::Rect kVisibleRect3(kVisibleRect2.x(), kVisibleRect2.y(), kVisibleRect2.width(), kVisibleRect2.height() - 1);
...
TEST_P(WebRtcVideoTrackSourceTest, UpdateRectWithScaling)
{
    ...
    track_source_->SetCustomFrameAdaptationParamsForTesting(FrameAdaptation_Scale(kNaturalSize, kScaleToSize));

    // Any UPDATE_RECT for the first received frame is expected to get
    // ignored and no update rect should be set.
    const gfx::Rect kUpdateRect1(120, 70, 160, 40);
    EXPECT_CALL(mock_sink_, OnFrame(_)).WillOnce(Invoke([](const webrtc::VideoFrame& frame) { EXPECT_FALSE(frame.has_update_rect()); }));
    int capture_counter = 101; // arbitrary absolute value
    SendTestFrameWithUpdateRect(frame_parameters, capture_counter, kUpdateRect1);
    Mock::VerifyAndClearExpectations(&mock_sink_);
    ...
    track_source_->SetCustomFrameAdaptationParamsForTesting(FrameAdaptation_Scale(kNaturalSize, kScaleToSize2));
    SendTestFrameWithUpdateRect(frame_parameters, ++capture_counter, gfx::Rect());
...
TEST_P(WebRtcVideoTrackSourceTest, PassesMappedFramesInOrder)
{
    ...
    FrameParameters frame_parameters = { .coded_size = gfx::Size(640, 480),
        ...
        .storage_type = std::get<0>(GetParam()),
        .pixel_format = std::get<1>(GetParam()) };
    if (frame_parameters.storage_type != media::VideoFrame::StorageType::STORAGE_GPU_MEMORY_BUFFER) {
...
INSTANTIATE_TEST_SUITE_P(WebRtcVideoTrackSourceTest, WebRtcVideoTrackSourceTest, testing::ValuesIn(TestParams()), [](const auto& info) {
    return base::StrCat({ media::VideoFrame::StorageTypeToString(std::get<0>(info.param)), "_", media::VideoPixelFormatToString(std::get<1>(info.param)) });
});
