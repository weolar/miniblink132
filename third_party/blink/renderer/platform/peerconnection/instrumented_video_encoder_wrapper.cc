...
InstrumentedVideoEncoderWrapper::InstrumentedVideoEncoderWrapper(
    int id, std::unique_ptr<webrtc::VideoEncoder> wrapped_encoder, VideoEncoderStateObserver* state_observer)
    : id_(id)
...
int InstrumentedVideoEncoderWrapper::InitEncode(const webrtc::VideoCodec* codec_settings, const webrtc::VideoEncoder::Settings& settings)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
    ...
    if (status == WEBRTC_VIDEO_CODEC_OK) {
        ...
        CHECK_EQ(wrapped_encoder_->RegisterEncodeCompleteCallback(this), WEBRTC_VIDEO_CODEC_OK);
    }
    return status;
}

int32_t InstrumentedVideoEncoderWrapper::RegisterEncodeCompleteCallback(webrtc::EncodedImageCallback* callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
    callback_ = callback;
...
int32_t InstrumentedVideoEncoderWrapper::Encode(const webrtc::VideoFrame& frame, const std::vector<webrtc::VideoFrameType>* frame_types)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
...
void InstrumentedVideoEncoderWrapper::SetRates(const RateControlParameters& parameters)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
...
void InstrumentedVideoEncoderWrapper::SetFecControllerOverride(webrtc::FecControllerOverride* fec_controller_override)
{
    wrapped_encoder_->SetFecControllerOverride(fec_controller_override);
}

void InstrumentedVideoEncoderWrapper::OnPacketLossRateUpdate(float packet_loss_rate)
{
...
void InstrumentedVideoEncoderWrapper::OnLossNotification(const LossNotification& loss_notification)
{
    wrapped_encoder_->OnLossNotification(loss_notification);
}

webrtc::VideoEncoder::EncoderInfo InstrumentedVideoEncoderWrapper::GetEncoderInfo() const
{
    return wrapped_encoder_->GetEncoderInfo();
}

void InstrumentedVideoEncoderWrapper::ReportEncodeResult(const VideoEncoderStateObserver::EncodeResult& result)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_);
    state_observer_->OnEncodedImage(id_, result);
}

webrtc::EncodedImageCallback::Result InstrumentedVideoEncoderWrapper::OnEncodedImage(
    const webrtc::EncodedImage& encoded_image, const webrtc::CodecSpecificInfo* codec_specific_info)
{
    VideoEncoderStateObserver::EncodeResult encode_result { .width = base::checked_cast<int>(encoded_image._encodedWidth),
        .height = base::checked_cast<int>(encoded_image._encodedHeight),
        .keyframe = encoded_image._frameType == webrtc::VideoFrameType::kVideoFrameKey,
    ...
    if (!encoder_sequence_runner_->RunsTasksInCurrentSequence()) {
        encoder_sequence_runner_->PostTask(
            FROM_HERE, base::BindOnce(&InstrumentedVideoEncoderWrapper::ReportEncodeResult, weak_this_factory_.GetWeakPtr(), encode_result));
    } else {
...
void InstrumentedVideoEncoderWrapper::OnDroppedFrame(webrtc::EncodedImageCallback::DropReason reason)
{
    if (callback_) {
