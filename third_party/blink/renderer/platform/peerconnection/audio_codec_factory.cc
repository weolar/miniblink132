...
template <typename T> struct NotAdvertisedEncoder {
    ...
    static std::unique_ptr<webrtc::AudioEncoder> MakeAudioEncoder(const Config& config, int payload_type, std::optional<webrtc::AudioCodecPairId> codec_pair_id)
    {
...
template <typename T> struct NotAdvertisedDecoder {
    using Config = typename T::Config;
    static std::optional<Config> SdpToConfig(const webrtc::SdpAudioFormat& audio_format)
    {
        return T::SdpToConfig(audio_format);
    ...
    static std::unique_ptr<webrtc::AudioDecoder> MakeAudioDecoder(const Config& config, std::optional<webrtc::AudioCodecPairId> codec_pair_id)
    {
...
{
    return webrtc::CreateAudioEncoderFactory<webrtc::AudioEncoderOpus, webrtc::AudioEncoderG722, webrtc::AudioEncoderG711,
        NotAdvertisedEncoder<webrtc::AudioEncoderL16>, NotAdvertisedEncoder<webrtc::AudioEncoderMultiChannelOpus>>();
}

rtc::scoped_refptr<webrtc::AudioDecoderFactory> CreateWebrtcAudioDecoderFactory()
{
    return webrtc::CreateAudioDecoderFactory<webrtc::AudioDecoderOpus, webrtc::AudioDecoderG722, webrtc::AudioDecoderG711,
        NotAdvertisedDecoder<webrtc::AudioDecoderL16>, NotAdvertisedDecoder<webrtc::AudioDecoderMultiChannelOpus>>();
}
