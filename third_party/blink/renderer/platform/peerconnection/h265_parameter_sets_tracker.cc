...
H265ParameterSetsTracker::PpsData& H265ParameterSetsTracker::PpsData::operator=(PpsData&& rhs) = default;
H265ParameterSetsTracker::PpsData::~PpsData() = default;

H265ParameterSetsTracker::SpsData::SpsData() = default;
H265ParameterSetsTracker::SpsData::SpsData(SpsData&& rhs) = default;
H265ParameterSetsTracker::SpsData& H265ParameterSetsTracker::SpsData::operator=(SpsData&& rhs) = default;
H265ParameterSetsTracker::SpsData::~SpsData() = default;

H265ParameterSetsTracker::VpsData::VpsData() = default;
H265ParameterSetsTracker::VpsData::VpsData(VpsData&& rhs) = default;
H265ParameterSetsTracker::VpsData& H265ParameterSetsTracker::VpsData::operator=(VpsData&& rhs) = default;
H265ParameterSetsTracker::VpsData::~VpsData() = default;

H265ParameterSetsTracker::FixedBitstream H265ParameterSetsTracker::MaybeFixBitstream(rtc::ArrayView<const uint8_t> bitstream)
{
    if (!bitstream.size()) {
    ...
    parser_.ParseBitstream(rtc::ArrayView<const uint8_t>(bitstream.data(), bitstream.size()));

    std::vector<webrtc::H265::NaluIndex> nalu_indices = webrtc::H265::FindNaluIndices(bitstream.data(), bitstream.size());
    for (const auto& nalu_index : nalu_indices) {
        if (nalu_index.payload_size < 2) {
            // H.265 NALU header is at least 2 bytes.
            return { PacketAction::kRequestKeyframe };
        }
        const uint8_t* payload_start = bitstream.data() + nalu_index.payload_start_offset;
        const uint8_t* nalu_start = bitstream.data() + nalu_index.start_offset;
        size_t nalu_size = nalu_index.payload_size + nalu_index.payload_start_offset - nalu_index.start_offset;
        ...
        case webrtc::H265::NaluType::kVps:
            ...
            vps = webrtc::H265VpsParser::ParseVps(payload_start + 2, nalu_index.payload_size - 2);
            // Always replace VPS with the same ID. Same for other parameter sets.
            if (vps) {
                std::unique_ptr<VpsData> current_vps_data = std::make_unique<VpsData>();
                // Copy with start code included. Same for other parameter sets.
                if (!current_vps_data.get() || !nalu_size || nalu_size > kMaxParameterSetSizeBytes) {
                    return { PacketAction::kRequestKeyframe };
                }
        ...
        case webrtc::H265::NaluType::kSps:
            sps = webrtc::H265SpsParser::ParseSps(payload_start + 2, nalu_index.payload_size - 2);
            if (sps) {
                std::unique_ptr<SpsData> current_sps_data = std::make_unique<SpsData>();
                if (!current_sps_data.get() || !nalu_size || nalu_size > kMaxParameterSetSizeBytes) {
                    return { PacketAction::kRequestKeyframe };
                }
        ...
        case webrtc::H265::NaluType::kPps:
            if (webrtc::H265PpsParser::ParsePpsIds(payload_start + 2, nalu_index.payload_size - 2, &slice_pps_id, &slice_sps_id)) {
                ...
                if (current_sps_data == sps_data_.end()) {
                    ...
                    std::unique_ptr<PpsData> current_pps_data = std::make_unique<PpsData>();
                    if (!current_pps_data.get() || !nalu_size || nalu_size > kMaxParameterSetSizeBytes) {
                        return { PacketAction::kRequestKeyframe };
                    }
        ...
        case webrtc::H265::NaluType::kCra:
            ...
            if (!prepend_vps && !prepend_sps && !prepend_pps) {
                ...
                required_size += vps_data->value->size + sps_data->value->size + pps_data->value->size;

                required_size += bitstream.size();
                size_t offset = 0;

                fixed.bitstream = webrtc::EncodedImageBuffer::Create(required_size);
                memcpy(fixed.bitstream->data(), vps_data->value->payload.get(), vps_data->value->size);
                offset += vps_data->value->size;
                memcpy(fixed.bitstream->data() + offset, sps_data->value->payload.get(), sps_data->value->size);
                offset += sps_data->value->size;
                memcpy(fixed.bitstream->data() + offset, pps_data->value->payload.get(), pps_data->value->size);
                offset += pps_data->value->size;
                memcpy(fixed.bitstream->data() + offset, bitstream.data(), bitstream.size());
