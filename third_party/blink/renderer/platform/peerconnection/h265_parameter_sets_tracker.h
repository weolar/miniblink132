...
public:
    ...
    virtual FixedBitstream MaybeFixBitstream(rtc::ArrayView<const uint8_t> bitstream);
...
private:
    ...
    // ID.
    WTF::HashMap<uint32_t, std::unique_ptr<PpsData>, IntWithZeroKeyHashTraits<uint32_t>> pps_data_;
    // Map from sps_video_parameter_set_id to the SPS payload associated with this
    // ID.
    WTF::HashMap<uint32_t, std::unique_ptr<SpsData>, IntWithZeroKeyHashTraits<uint32_t>> sps_data_;
    // Map from pps_pic_parameter_set_id to the PPS payload associated with this
    // ID.
    WTF::HashMap<uint32_t, std::unique_ptr<VpsData>, IntWithZeroKeyHashTraits<uint32_t>> vps_data_;
};
