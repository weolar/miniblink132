...
class VideoCodecFactoryTestWithSdpFormat : public testing::TestWithParam<webrtc::SdpVideoFormat> {
public:
...
protected:
    ...
    {
        std::optional<media::VideoCodecProfile> profile = WebRTCFormatToCodecProfile(sdp);
    ...
    {
        return CreateWebrtcVideoEncoderFactory(&mock_gpu_factories_, mock_encoder_metrics_provider_factory_, base::NullCallback());
    }

    testing::NiceMock<media::MockGpuVideoAcceleratorFactories> mock_gpu_factories_ { nullptr };
...
{
    std::unique_ptr<webrtc::VideoEncoderFactory> encoder_factory = CreateEncoderFactory();
    ...
    EXPECT_CALL(mock_gpu_factories_, GetVideoEncodeAcceleratorSupportedProfiles()).WillRepeatedly(Return(kSupportedProfiles));
    webrtc::EnvironmentFactory environment_factory;
    auto encoder = encoder_factory->Create(environment_factory.Create(), GetParam());
...
INSTANTIATE_TEST_SUITE_P(, VideoCodecFactoryTestWithSdpFormat, ValuesIn({
#if !BUILDFLAG(IS_ANDROID)
    webrtc::CreateH264Format(webrtc::H264Profile::kProfileBaseline, webrtc::H264Level::kLevel1,
        /*packetization_mode=*/"1"),
#endif
        webrtc::SdpVideoFormat::VP8(), webrtc::SdpVideoFormat::VP9Profile0(), webrtc::SdpVideoFormat::AV1Profile0(),
