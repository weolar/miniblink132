...
using TransformerCallback = WTF::CrossThreadRepeatingFunction<void(std::unique_ptr<webrtc::TransformableAudioFrameInterface>)>;
...
public:
    ...
    public:
        ...
        void TransformFrameOnSourceTaskRunner(std::unique_ptr<webrtc::TransformableAudioFrameInterface> frame);
        ...
        void SendFrameToSink(std::unique_ptr<webrtc::TransformableAudioFrameInterface> frame);
    ...
    private:
        ...
        raw_ptr<RTCEncodedAudioStreamTransformer> transformer_ GUARDED_BY(transformer_lock_);
    };

    explicit RTCEncodedAudioStreamTransformer(scoped_refptr<base::SingleThreadTaskRunner> realm_task_runner);
    ...
    void RegisterTransformedFrameCallback(rtc::scoped_refptr<webrtc::TransformedFrameCallback>);
    ...
    void TransformFrame(std::unique_ptr<webrtc::TransformableAudioFrameInterface>);
...
private:
    ...
    rtc::scoped_refptr<webrtc::TransformedFrameCallback> send_frame_to_sink_cb_ GUARDED_BY(sink_lock_);
