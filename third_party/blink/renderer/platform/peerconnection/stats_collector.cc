...
StatsCollector::StatsCollector(bool is_decode, media::VideoCodecProfile codec_profile, StoreProcessingStatsCB stats_callback)
    : is_decode_(is_decode)
    , codec_profile_(codec_profile)
    , stats_callback_(stats_callback)
{
    DVLOG(3) << __func__ << " (IsDecode: " << is_decode_ << ", " << media::GetProfileName(codec_profile_) << ")";
...
{
    DVLOG(3) << __func__;
    processing_time_ms_histogram_
        = std::make_unique<LinearHistogram>(kProcessingTimeHistogramMinValue_ms, kProcessingTimeHistogramMaxValue_ms, kProcessingTimeHistogramBuckets);
...
void StatsCollector::ReportStats() const
{
    ...
    VideoStats stats = { static_cast<int>(processing_time_ms_histogram_->NumValues()), static_cast<int>(number_of_keyframes_),
        processing_time_ms_histogram_->GetPercentile(kProcessingTimePercentileToReport) };
    DVLOG(3) << __func__ << " IsDecode: " << current_stats_key_.is_decode << ", Pixel size: " << current_stats_key_.pixel_size
             << ", HW: " << current_stats_key_.hw_accelerated << ", P99: " << stats.p99_processing_time_ms << " ms, frames: " << stats.frame_count
             << ", key frames:: " << stats.key_frame_count;

    stats_callback_.Run(current_stats_key_, stats);
}

void StatsCollector::AddProcessingTime(
    int pixel_size, bool is_hardware_accelerated, const float processing_time_ms, size_t new_keyframes, const base::TimeTicks& now)
{
    DCHECK(processing_time_ms_histogram_);
    if (pixel_size == current_stats_key_.pixel_size && is_hardware_accelerated == current_stats_key_.hw_accelerated) {
        // Store data.
    ...
    if (samples_collected() >= kMinSamplesThreshold && (now - last_report_) > kProcessingStatsReportingPeriod) {
