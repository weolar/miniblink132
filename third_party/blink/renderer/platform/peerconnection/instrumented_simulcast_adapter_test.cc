...
public:
    ...
    {
        ON_CALL(mock_gpu_factories_, GetTaskRunner()).WillByDefault(Return(base::SequencedTaskRunner::GetCurrentDefault()));
        ON_CALL(mock_gpu_factories_, IsEncoderSupportKnown()).WillByDefault(Return(true));
    }
    ~InstrumentedSimulcastAdapterTest() override = default;

    void UseHwEncoder()
    {
        rtc_video_encoder_factory_ = std::make_unique<RTCVideoEncoderFactory>(&mock_gpu_factories_, mock_encoder_metrics_provider_factory_);

        primary_encoder_factory_ = static_cast<webrtc::VideoEncoderFactory*>(rtc_video_encoder_factory_.get());
        secondate_encoder_factory_ = static_cast<webrtc::VideoEncoderFactory*>(&software_encoder_factory_);
    }
    void SetUp() override
    {
        primary_encoder_factory_ = static_cast<webrtc::VideoEncoderFactory*>(&software_encoder_factory_);
...
protected:
    ...
    std::unique_ptr<InstrumentedSimulcastAdapter> CreateInstrumentedSimulcastAdapter()
    {
        return InstrumentedSimulcastAdapter::Create(webrtc::EnvironmentFactory().Create(), primary_encoder_factory_.get(), secondate_encoder_factory_.get(),
            std::make_unique<VideoEncoderStateObserverImpl>(media::VideoCodecProfile::VP8PROFILE_ANY, base::NullCallback()), webrtc::SdpVideoFormat::VP8());
    }
...
TEST_F(InstrumentedSimulcastAdapterTest, CreateAndDestroyWithoutHardwareAcceleration)
{
    EXPECT_TRUE(CreateInstrumentedSimulcastAdapter());
}

TEST_F(InstrumentedSimulcastAdapterTest, CreateAndDestroyWithHardwareAcceleration)
{
    UseHwEncoder();
    EXPECT_TRUE(CreateInstrumentedSimulcastAdapter());
}
