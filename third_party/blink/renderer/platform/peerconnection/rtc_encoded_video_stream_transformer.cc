...
class RTCEncodedVideoStreamTransformerDelegate : public webrtc::FrameTransformerInterface {
public:
    RTCEncodedVideoStreamTransformerDelegate(scoped_refptr<base::SingleThreadTaskRunner> realm_task_runner,
        scoped_refptr<RTCEncodedVideoStreamTransformer::Broker> transformer_broker, std::unique_ptr<Metronome> metronome)
    ...
    void SetSourceTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    {
    ...
    void RegisterTransformedFrameSinkCallback(rtc::scoped_refptr<webrtc::TransformedFrameCallback> send_frame_to_sink_callback, uint32_t ssrc) override
    {
        transformer_broker_->RegisterTransformedFrameSinkCallback(std::move(send_frame_to_sink_callback), ssrc);
    }
    ...
    {
        auto video_frame = base::WrapUnique(static_cast<webrtc::TransformableVideoFrameInterface*>(frame.release()));
        if (metronome_) {
            ...
            if (!tick_scheduled_) {
                ...
                metronome_->RequestCallOnNextTick([delegate = weak_factory_.GetWeakPtr()] {
                    if (delegate) {
            ...
            base::AutoLock locker(source_task_runner_lock_);
            PostCrossThreadTask(*source_task_runner_, FROM_HERE,
                CrossThreadBindOnce(&RTCEncodedVideoStreamTransformer::Broker::TransformFrameOnSourceTaskRunner, transformer_broker_, std::move(video_frame)));
        }
    }
...
private:
    void InvokeQueuedTransforms()
    {
        ...
        for (std::unique_ptr<webrtc::TransformableVideoFrameInterface>& frame : queued_frames_) {
            PostCrossThreadTask(*source_task_runner_, FROM_HERE,
                CrossThreadBindOnce(&RTCEncodedVideoStreamTransformer::Broker::TransformFrameOnSourceTaskRunner, transformer_broker_, std::move(frame)));
        }
        queued_frames_.clear();
    }

    base::Lock source_task_runner_lock_;
    scoped_refptr<base::SingleThreadTaskRunner> source_task_runner_ GUARDED_BY(source_task_runner_lock_);
    ...
    Vector<std::unique_ptr<webrtc::TransformableVideoFrameInterface>> queued_frames_ GUARDED_BY_CONTEXT(metronome_sequence_checker_);

    base::WeakPtrFactory<RTCEncodedVideoStreamTransformerDelegate> weak_factory_ { this };
};

} // namespace

RTCEncodedVideoStreamTransformer::Broker::Broker(RTCEncodedVideoStreamTransformer* transformer_)
    : transformer_(transformer_)
{
}

void RTCEncodedVideoStreamTransformer::Broker::RegisterTransformedFrameSinkCallback(
    rtc::scoped_refptr<webrtc::TransformedFrameCallback> send_frame_to_sink_callback, uint32_t ssrc)
{
    base::AutoLock locker(transformer_lock_);
    if (transformer_) {
        transformer_->RegisterTransformedFrameSinkCallback(std::move(send_frame_to_sink_callback), ssrc);
    }
}

void RTCEncodedVideoStreamTransformer::Broker::UnregisterTransformedFrameSinkCallback(uint32_t ssrc)
{
    base::AutoLock locker(transformer_lock_);
    if (transformer_) {
...
void RTCEncodedVideoStreamTransformer::Broker::TransformFrameOnSourceTaskRunner(std::unique_ptr<webrtc::TransformableVideoFrameInterface> frame)
{
    base::AutoLock locker(transformer_lock_);
    if (transformer_) {
...
void RTCEncodedVideoStreamTransformer::Broker::SetTransformerCallback(TransformerCallback callback)
{
    base::AutoLock locker(transformer_lock_);
    if (transformer_) {
...
void RTCEncodedVideoStreamTransformer::Broker::SetSourceTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    base::AutoLock locker(transformer_lock_);
    if (transformer_) {
...
void RTCEncodedVideoStreamTransformer::Broker::SendFrameToSink(std::unique_ptr<webrtc::TransformableVideoFrameInterface> frame)
{
    base::AutoLock locker(transformer_lock_);
    if (transformer_) {
...
RTCEncodedVideoStreamTransformer::RTCEncodedVideoStreamTransformer(
    scoped_refptr<base::SingleThreadTaskRunner> realm_task_runner, std::unique_ptr<Metronome> metronome)
    : broker_(base::AdoptRef(new Broker(this)))
    , delegate_(new rtc::RefCountedObject<RTCEncodedVideoStreamTransformerDelegate>(std::move(realm_task_runner), broker_, std::move(metronome)))
{
}
...
void RTCEncodedVideoStreamTransformer::RegisterTransformedFrameSinkCallback(rtc::scoped_refptr<webrtc::TransformedFrameCallback> callback, uint32_t ssrc)
{
...
void RTCEncodedVideoStreamTransformer::UnregisterTransformedFrameSinkCallback(uint32_t ssrc)
{
    base::AutoLock locker(sink_lock_);
...
void RTCEncodedVideoStreamTransformer::TransformFrame(std::unique_ptr<webrtc::TransformableVideoFrameInterface> frame)
{
    ...
    if (!transformer_callback_) {
        {
            ...
            if (!short_circuit_) {
                ...
                if (buffered_frames_.size() < kMaxBufferedFrames) {
                    ...
                    LogMessage(base::StringPrintf("TransformFrame reached kMaxBufferedFrames, dropped %d frames.", dropped_frames_count_));
                }
                return;
            }
        }
        ...
        LogMessage("TransformFrame received frame after starting shortcircuiting. Sending "
                   "straight back.");
...
void RTCEncodedVideoStreamTransformer::SendFrameToSink(std::unique_ptr<webrtc::TransformableVideoFrameInterface> frame)
{
    base::AutoLock locker(sink_lock_);
    if (send_frame_to_sink_callbacks_.size() == 1) {
        // Only a single sink callback registered, so this frame must use it.
        send_frame_to_sink_callbacks_[0].second->OnTransformedFrame(std::move(frame));
        return;
    }
...
{
    Vector<std::unique_ptr<webrtc::TransformableVideoFrameInterface>> buffered_frames;
    {
        base::AutoLock locker(sink_lock_);
        short_circuit_ = true;
    ...
    // transform.
    LogMessage(base::StringPrintf("StartShortCircuiting replaying %d buffered frames", buffered_frames.size()));
    for (auto& buffered_frame : buffered_frames) {
        SendFrameToSink(std::move(buffered_frame));
    }
}

void RTCEncodedVideoStreamTransformer::SetTransformerCallback(TransformerCallback callback)
{
    base::AutoLock locker(source_lock_);
    ...
    LogMessage(base::StringPrintf("SetTransformerCallback replaying %d buffered frames", buffered_frames.size()));
    for (auto& buffered_frame : buffered_frames) {
...
bool RTCEncodedVideoStreamTransformer::HasTransformedFrameSinkCallback(uint32_t ssrc) const
{
    base::AutoLock locker(sink_lock_);
...
rtc::scoped_refptr<webrtc::FrameTransformerInterface> RTCEncodedVideoStreamTransformer::Delegate()
{
    return delegate_;
}

void RTCEncodedVideoStreamTransformer::SetSourceTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> realm_task_runner)
{
    static_cast<RTCEncodedVideoStreamTransformerDelegate*>(delegate_.get())->SetSourceTaskRunner(std::move(realm_task_runner));
}

scoped_refptr<RTCEncodedVideoStreamTransformer::Broker> RTCEncodedVideoStreamTransformer::GetBroker()
{
    return broker_;
}
...
{
    blink::WebRtcLogMessage(base::StringPrintf("EncodedVideoStreamTransformer::%s [this=0x%" PRIXPTR "]", message.c_str(), reinterpret_cast<uintptr_t>(this)));
}
