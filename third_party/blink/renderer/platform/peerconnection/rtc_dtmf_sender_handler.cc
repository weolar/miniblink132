...
class RtcDtmfSenderHandler::Observer : public WTF::ThreadSafeRefCounted<Observer>, public webrtc::DtmfSenderObserverInterface {
public:
    explicit Observer(scoped_refptr<base::SingleThreadTaskRunner> main_thread, const base::WeakPtr<RtcDtmfSenderHandler>& handler)
...
private:
    ...
    {
        PostCrossThreadTask(
            *main_thread_.get(), FROM_HERE, CrossThreadBindOnce(&Observer::OnToneChangeOnMainThread, scoped_refptr<Observer>(this), String(tone.data())));
    }
...
RtcDtmfSenderHandler::RtcDtmfSenderHandler(scoped_refptr<base::SingleThreadTaskRunner> main_thread, webrtc::DtmfSenderInterface* dtmf_sender)
    : dtmf_sender_(dtmf_sender)
    , webkit_client_(nullptr)
{
    DVLOG(1) << "::ctor";
    observer_ = base::MakeRefCounted<Observer>(std::move(main_thread), weak_factory_.GetWeakPtr());
...
bool RtcDtmfSenderHandler::InsertDTMF(const String& tones, int duration, int inter_tone_gap)
{
