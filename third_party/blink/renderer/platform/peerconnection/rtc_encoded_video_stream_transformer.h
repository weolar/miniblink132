...
public:
    using TransformerCallback = WTF::CrossThreadRepeatingFunction<void(std::unique_ptr<webrtc::TransformableVideoFrameInterface>)>;
    ...
    public:
        void RegisterTransformedFrameSinkCallback(rtc::scoped_refptr<webrtc::TransformedFrameCallback>, uint32_t ssrc);

        void UnregisterTransformedFrameSinkCallback(uint32_t ssrc);

        void TransformFrameOnSourceTaskRunner(std::unique_ptr<webrtc::TransformableVideoFrameInterface> frame);
        ...
        void SendFrameToSink(std::unique_ptr<webrtc::TransformableVideoFrameInterface> frame);
    ...
    private:
        ...
        raw_ptr<RTCEncodedVideoStreamTransformer> transformer_ GUARDED_BY(transformer_lock_);
    };
    ...
    explicit RTCEncodedVideoStreamTransformer(scoped_refptr<base::SingleThreadTaskRunner> realm_task_runner, std::unique_ptr<webrtc::Metronome> metronome);
    ...
    void TransformFrame(std::unique_ptr<webrtc::TransformableVideoFrameInterface>);
    ...
    void SetSourceTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> realm_task_runner);
...
private:
    ...
    Vector<std::pair<uint32_t, rtc::scoped_refptr<webrtc::TransformedFrameCallback>>> send_frame_to_sink_callbacks_ GUARDED_BY(sink_lock_);
    bool short_circuit_ GUARDED_BY(sink_lock_) = false;
    Vector<std::unique_ptr<webrtc::TransformableVideoFrameInterface>> buffered_frames_ GUARDED_BY(sink_lock_);
