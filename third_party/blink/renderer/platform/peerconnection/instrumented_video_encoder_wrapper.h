...
class PLATFORM_EXPORT InstrumentedVideoEncoderWrapper : public webrtc::VideoEncoder, public webrtc::EncodedImageCallback {
public:
    InstrumentedVideoEncoderWrapper(int id, std::unique_ptr<webrtc::VideoEncoder> wrapped_encoder, VideoEncoderStateObserver* state_observer);
...
private:
    void ReportEncodeResult(const VideoEncoderStateObserver::EncodeResult& result);

    const int id_;
    const raw_ptr<VideoEncoderStateObserver> state_observer_ GUARDED_BY_CONTEXT(encoder_sequence_);
    ...
    base::WeakPtrFactory<InstrumentedVideoEncoderWrapper> weak_this_factory_ { this };
};
