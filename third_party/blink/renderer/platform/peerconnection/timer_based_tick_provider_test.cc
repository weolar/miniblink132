...
public:
    void EnsureTickAboutToElapse()
    {
        ...
        auto next_tick = TimerBasedTickProvider::TimeSnappedToNextTick(now, kTickPeriod);
    ...
    TimerBasedTickProvider* tick_provider()
    {
        return tick_provider_.get();
    }
...
private:
    scoped_refptr<TimerBasedTickProvider> tick_provider_ = base::MakeRefCounted<TimerBasedTickProvider>(kTickPeriod);
};
...
TEST_F(TimerBasedTickProviderTest, SnapsTimeToNextTick)
{
    ...
    auto now = base::TimeTicks::Now();
    auto next_tick = TimerBasedTickProvider::TimeSnappedToNextTick(now, kTickPeriod);
    EXPECT_EQ(next_tick, TimerBasedTickProvider::TimeSnappedToNextTick(next_tick, kTickPeriod));
    EXPECT_EQ(next_tick + kTickPeriod, TimerBasedTickProvider::TimeSnappedToNextTick(next_tick + base::Microseconds(1), kTickPeriod));
}
...
TEST_F(TimerBasedTickProviderTest, InvokesRequestedCallbackOnTick)
{
    ...
    EXPECT_CALL(callback, Run).WillOnce(Invoke([&] { EXPECT_EQ(base::TimeTicks::Now(), expected_run_time); }));
...
TEST_F(TimerBasedTickProviderTest, InvokesTwoCallbacksOnSameTick)
{
    ...
    EXPECT_CALL(callback, Run).Times(2).WillRepeatedly(Invoke([&] { EXPECT_EQ(base::TimeTicks::Now(), expected_run_time); }));
    tick_provider()->RequestCallOnNextTick(callback.Get());
...
TEST_F(TimerBasedTickProviderTest, InvokesRequestedCallbackOnTickFromCallbackOnTick)
{
    base::MockOnceCallback<void()> callback;
    auto expected_run_time = SnapToNextTick(base::TimeTicks::Now());
...
TEST_F(TimerBasedTickProviderTest, InvokesRequestedCallbackOnNextTickFromCallbackOnTick)
{
    base::MockOnceCallback<void()> callback;
    auto expected_run_time = SnapToNextTick(SnapToNextTick(base::TimeTicks::Now()) + base::Microseconds(1));
...
{
    auto tick_provider2 = base::MakeRefCounted<TimerBasedTickProvider>(kTickPeriod);
    base::MockOnceCallback<void()> callback;
