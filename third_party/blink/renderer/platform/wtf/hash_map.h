...
template <typename Value, typename Traits, typename Allocator> class HashCountedSet;

struct KeyValuePairExtractor {
    STATIC_ONLY(KeyValuePairExtractor);
    template <typename T> static const typename T::KeyType& ExtractKey(const T& p)
    {
        return p.key;
    }
    template <typename T> static typename T::KeyType& ExtractKey(T& p)
    {
        return p.key;
    }
    // Assumes out points to a buffer of size at least sizeof(T::KeyType).
    template <typename T> static void ExtractKeyToMemory(const T& p, void* out)
    {
        AtomicReadMemcpy<sizeof(typename T::KeyType), alignof(typename T::KeyType)>(out, &p.key);
    }
    template <typename T> static void ClearValue(T& p)
    {
...
template <typename KeyArg, typename MappedArg, typename KeyTraitsArg = HashTraits<KeyArg>, typename MappedTraitsArg = HashTraits<MappedArg>,
    typename Allocator = PartitionAllocator>
class HashMap {
    USE_ALLOCATOR(HashMap, Allocator);
    template <typename T, typename U, typename V> friend class HashCountedSet;
...
private:
    ...
    typedef HashTable<KeyType, ValueType, KeyValuePairExtractor, ValueTraits, KeyTraits, Allocator> HashTableType;
...
public:
    ...
    HashMapKeysProxy& Keys()
    {
        return static_cast<HashMapKeysProxy&>(*this);
    }
    ...
    template <typename IncomingKeyType, typename IncomingMappedType> AddResult Set(IncomingKeyType&&, IncomingMappedType&&);
    ...
    // was actually added.
    template <typename IncomingKeyType, typename IncomingMappedType> AddResult insert(IncomingKeyType&&, IncomingMappedType&&);
    ...
    void clear();
    template <typename Collection> void RemoveAll(const Collection& to_be_removed)
    {
    ...
    template <typename IncomingKeyType> static bool IsValidKey(const IncomingKeyType&);
...
private:
    template <typename IncomingKeyType, typename IncomingMappedType> AddResult InlineAdd(IncomingKeyType&&, IncomingMappedType&&);
    ...
    struct TypeConstraints {
        constexpr TypeConstraints()
        {
            ...
            static_assert(Allocator::kIsGarbageCollected || !IsPointerToGarbageCollectedType<KeyArg>::value,
                "Cannot put raw pointers to garbage-collected classes into "
                "an off-heap HashMap.  Use HeapHashMap<> instead.");
            static_assert(Allocator::kIsGarbageCollected || !IsPointerToGarbageCollectedType<MappedArg>::value,
                "Cannot put raw pointers to garbage-collected classes into "
                "an off-heap HashMap.  Use HeapHashMap<> instead.");
        }
    };
...
template <typename KeyArg, typename MappedArg, typename KeyTraitsArg, typename MappedTraitsArg, typename Allocator>
class HashMap<KeyArg, MappedArg, KeyTraitsArg, MappedTraitsArg, Allocator>::HashMapKeysProxy
    : private HashMap<KeyArg, MappedArg, KeyTraitsArg, MappedTraitsArg, Allocator> {
    DISALLOW_NEW();

public:
    typedef HashMap<KeyArg, MappedArg, KeyTraitsArg, MappedTraitsArg, Allocator> HashMapType;
...
template <typename KeyArg, typename MappedArg, typename KeyTraitsArg, typename MappedTraitsArg, typename Allocator>
class HashMap<KeyArg, MappedArg, KeyTraitsArg, MappedTraitsArg, Allocator>::HashMapValuesProxy
    : private HashMap<KeyArg, MappedArg, KeyTraitsArg, MappedTraitsArg, Allocator> {
    DISALLOW_NEW();
...
template <typename KeyTraits, typename ValueTraits> struct HashMapValueTraits : KeyValuePairHashTraits<KeyTraits, ValueTraits> {
...
template <typename KeyTraits, typename ValueTraits> struct HashMapTranslator {
    ...
    template <typename T, typename U, typename V> static void Store(T& location, U&& key, V&& mapped)
    {
...
template <typename KeyArg, typename MappedArg, typename KeyTraitsArg, typename MappedTraitsArg, typename Allocator>
HashMap<KeyArg, MappedArg, KeyTraitsArg, MappedTraitsArg, Allocator>::HashMap(std::initializer_list<ValueType> elements)
{
    if (elements.size()) {
        impl_.ReserveCapacityForSize(base::checked_cast<wtf_size_t>(elements.size()));
    }
...
auto HashMap<T, U, V, W, X>::operator=(std::initializer_list<ValueType> elements) -> HashMap&
{
...
template <typename It>
    requires(std::forward_iterator<It>)
HashMap<KeyArg, MappedArg, KeyTraitsArg, MappedTraitsArg, Allocator>::HashMap(It begin, It end)
{
...
template <typename T, typename U, typename V, typename W, typename X> inline wtf_size_t HashMap<T, U, V, W, X>::size() const
{
    return impl_.size();
}

template <typename T, typename U, typename V, typename W, typename X> inline wtf_size_t HashMap<T, U, V, W, X>::Capacity() const
{
    return impl_.Capacity();
}

template <typename T, typename U, typename V, typename W, typename X> inline bool HashMap<T, U, V, W, X>::empty() const
{
    return impl_.empty();
}
...
template <typename T, typename U, typename V, typename W, typename X>
inline typename HashMap<T, U, V, W, X>::iterator HashMap<T, U, V, W, X>::find(KeyPeekInType key)
{
    return impl_.find(key);
}

template <typename T, typename U, typename V, typename W, typename X>
inline typename HashMap<T, U, V, W, X>::const_iterator HashMap<T, U, V, W, X>::find(KeyPeekInType key) const
{
    return impl_.find(key);
}

template <typename T, typename U, typename V, typename W, typename X> inline bool HashMap<T, U, V, W, X>::Contains(KeyPeekInType key) const
{
...
inline typename HashMap<T, U, V, W, X>::iterator HashMap<T, U, V, W, X>::Find(const TYPE& value)
{
...
template <typename T, typename U, typename V, typename W, typename X>
template <typename HashTranslator, typename TYPE>
inline typename HashMap<T, U, V, W, X>::const_iterator HashMap<T, U, V, W, X>::Find(const TYPE& value) const
{
    return impl_.template Find<HashTranslator>(value);
}
...
typename HashMap<T, U, V, W, X>::AddResult HashMap<T, U, V, W, X>::InlineAdd(IncomingKeyType&& key, IncomingMappedType&& mapped)
{
    return impl_.template insert<HashMapTranslator<KeyTraits, ValueTraits>>(std::forward<IncomingKeyType>(key), std::forward<IncomingMappedType>(mapped));
}

template <typename T, typename U, typename V, typename W, typename X>
template <typename IncomingKeyType, typename IncomingMappedType>
typename HashMap<T, U, V, W, X>::AddResult HashMap<T, U, V, W, X>::Set(IncomingKeyType&& key, IncomingMappedType&& mapped)
{
...
template <typename T, typename U, typename V, typename W, typename X>
template <typename IncomingKeyType, typename IncomingMappedType>
typename HashMap<T, U, V, W, X>::AddResult HashMap<T, U, V, W, X>::insert(IncomingKeyType&& key, IncomingMappedType&& mapped)
{
    return InlineAdd(std::forward<IncomingKeyType>(key), std::forward<IncomingMappedType>(mapped));
}

template <typename T, typename U, typename V, typename W, typename X>
typename HashMap<T, U, V, W, X>::MappedPeekType HashMap<T, U, V, W, X>::at(KeyPeekInType key) const
{
...
template <typename T, typename U, typename V, typename W, typename X> inline void HashMap<T, U, V, W, X>::erase(iterator it)
{
    impl_.erase(it.impl_);
}

template <typename T, typename U, typename V, typename W, typename X> inline void HashMap<T, U, V, W, X>::erase(KeyPeekInType key)
{
    erase(find(key));
}

template <typename T, typename U, typename V, typename W, typename X> template <typename Pred> inline void HashMap<T, U, V, W, X>::erase_if(Pred pred)
{
...
template <typename T, typename U, typename V, typename W, typename X> auto HashMap<T, U, V, W, X>::Take(KeyPeekInType key) -> MappedType
{
...
template <typename T, typename U, typename V, typename W, typename X, typename Z>
inline void CopyKeysToVector(const HashMap<T, U, V, W, X>& collection, Z& vector)
{
    typedef typename HashMap<T, U, V, W, X>::const_iterator::KeysIterator iterator;
...
template <typename T, typename U, typename V, typename W, typename X, typename Z>
inline void CopyValuesToVector(const HashMap<T, U, V, W, X>& collection, Z& vector)
{
    typedef typename HashMap<T, U, V, W, X>::const_iterator::ValuesIterator iterator;
