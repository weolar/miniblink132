...
// becomes available.
template <typename T> ALWAYS_INLINE std::atomic<T>* AsAtomicPtr(T* t)
{
    return reinterpret_cast<std::atomic<T>*>(t);
}

template <typename T> ALWAYS_INLINE const std::atomic<T>* AsAtomicPtr(const T* t)
{
...
template <size_t bytes, typename AlignmentType> ALWAYS_INLINE void AtomicReadMemcpyAligned(void* to, const void* from)
{
    ...
    if constexpr (bytes == sizeof(uint32_t)) {
        *reinterpret_cast<uint32_t*>(to) = AsAtomicPtr(reinterpret_cast<const uint32_t*>(from))->load(std::memory_order_relaxed);
        return;
    }
#endif // defined(ARCH_CPU_64_BITS)

    if constexpr (bytes % kAlignment == 0 && bytes >= kAlignment && bytes <= 3 * kAlignment) {
        *reinterpret_cast<AlignmentType*>(to) = AsAtomicPtr(reinterpret_cast<const AlignmentType*>(from))->load(std::memory_order_relaxed);
        if constexpr (bytes >= 2 * kAlignment) {
            *(reinterpret_cast<AlignmentType*>(to) + 1) = AsAtomicPtr(reinterpret_cast<const AlignmentType*>(from) + 1)->load(std::memory_order_relaxed);
        }
        if constexpr (bytes == 3 * kAlignment) {
            *(reinterpret_cast<AlignmentType*>(to) + 2) = AsAtomicPtr(reinterpret_cast<const AlignmentType*>(from) + 2)->load(std::memory_order_relaxed);
        }
    } else {
...
template <size_t bytes, size_t alignment> ALWAYS_INLINE void AtomicReadMemcpy(void* to, const void* from)
{
...
template <size_t bytes, typename AlignmentType> ALWAYS_INLINE void AtomicWriteMemcpyAligned(void* to, const void* from)
{
    static constexpr size_t kAlignment = sizeof(AlignmentType);
...
#if defined(ARCH_CPU_64_BITS)
    if constexpr (bytes == sizeof(uint32_t)) {
        AsAtomicPtr(reinterpret_cast<uint32_t*>(to))->store(*reinterpret_cast<const uint32_t*>(from), std::memory_order_relaxed);
        return;
    }
#endif // defined(ARCH_CPU_64_BITS)

    if constexpr (bytes % kAlignment == 0 && bytes >= kAlignment && bytes <= 3 * kAlignment) {
        AsAtomicPtr(reinterpret_cast<AlignmentType*>(to))->store(*reinterpret_cast<const AlignmentType*>(from), std::memory_order_relaxed);
        if constexpr (bytes >= 2 * kAlignment) {
            AsAtomicPtr(reinterpret_cast<AlignmentType*>(to) + 1)->store(*(reinterpret_cast<const AlignmentType*>(from) + 1), std::memory_order_relaxed);
        }
        if constexpr (bytes == 3 * kAlignment) {
            AsAtomicPtr(reinterpret_cast<AlignmentType*>(to) + 2)->store(*(reinterpret_cast<const AlignmentType*>(from) + 2), std::memory_order_relaxed);
        }
    } else {
...
template <size_t bytes, size_t alignment> ALWAYS_INLINE void AtomicWriteMemcpy(void* to, const void* from)
{
    static_assert(bytes > 0, "Number of copied bytes should be greater than 0");
    if constexpr (alignment == sizeof(size_t)) {
...
template <size_t bytes, typename AlignmentType> ALWAYS_INLINE void AtomicMemzeroAligned(void* buf)
{
    static constexpr size_t kAlignment = sizeof(AlignmentType);
...
#if defined(ARCH_CPU_64_BITS)
    if constexpr (bytes == sizeof(uint32_t)) {
        AsAtomicPtr(reinterpret_cast<uint32_t*>(buf))->store(0, std::memory_order_relaxed);
        return;
    }
#endif // defined(ARCH_CPU_64_BITS)

    if constexpr (bytes % kAlignment == 0 && bytes >= kAlignment && bytes <= 3 * kAlignment) {
        AsAtomicPtr(reinterpret_cast<AlignmentType*>(buf))->store(0, std::memory_order_relaxed);
        if constexpr (bytes >= 2 * kAlignment) {
            AsAtomicPtr(reinterpret_cast<AlignmentType*>(buf) + 1)->store(0, std::memory_order_relaxed);
        }
        if constexpr (bytes == 3 * kAlignment) {
            AsAtomicPtr(reinterpret_cast<AlignmentType*>(buf) + 2)->store(0, std::memory_order_relaxed);
        }
    } else {
...
template <size_t bytes, size_t alignment> ALWAYS_INLINE void AtomicMemzero(void* buf)
{
    static_assert(bytes > 0, "Number of copied bytes should be greater than 0");
    if constexpr (alignment == sizeof(size_t)) {
...
template <typename T> ALWAYS_INLINE void AtomicWriteSwap(T& lhs, T& rhs)
{
    T tmp_val = rhs;
