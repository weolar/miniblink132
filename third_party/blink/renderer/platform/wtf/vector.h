...
template <typename T, wtf_size_t inlineBuffer, typename Allocator> class Deque;
...
#define MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, buffer, capacity, old_size, new_size)                                                                    \
...
#else
#define MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, buffer, capacity, old_size, new_size) ANNOTATE_CHANGE_SIZE(buffer, capacity, old_size, new_size)
#define MARKING_AWARE_ANNOTATE_NEW_BUFFER(Allocator, buffer, capacity, size) ANNOTATE_NEW_BUFFER(buffer, capacity, size)
#endif // defined(ADDRESS_SANITIZER)

template <typename T> struct VectorElementComparer {
    STATIC_ONLY(VectorElementComparer);
    template <typename U> static bool CompareElement(const T& left, const U& right)
    {
        return left == right;
    }
};

template <typename T> struct VectorElementComparer<std::unique_ptr<T>> {
    STATIC_ONLY(VectorElementComparer);
    template <typename U> static bool CompareElement(const std::unique_ptr<T>& left, const U& right)
    {
...
template <typename T, typename Allocator> struct VectorTypeOperations {
    ...
    static void Initialize(T* begin, T* end)
    {
        if constexpr (VectorTraits<T>::kCanInitializeWithMemset) {
            size_t size = reinterpret_cast<char*>(end) - reinterpret_cast<char*>(begin);
    ...
    static void Move(T* const src, T* const src_end, T* const dst, VectorOperationOrigin origin)
    {
        if (!src || !dst) [[unlikely]] {
            return;
        }
        ...
        } else if constexpr (Allocator::kIsGarbageCollected && IsTraceable<T>::value) {
            static_assert(VectorTraits<T>::kCanMoveWithMemcpy);
            AtomicWriteMemcpy(dst, src, reinterpret_cast<const char*>(src_end) - reinterpret_cast<const char*>(src));
            if (origin != VectorOperationOrigin::kConstruction) {
                // SAFETY: TODO(359904345): VectorTypeOperations should operate on spans.
                base::span<T> UNSAFE_BUFFERS(elements(dst, static_cast<size_t>(src_end - src)));
                ConstructTraits::NotifyNewElements(elements);
            }
        } else {
            static_assert(VectorTraits<T>::kCanMoveWithMemcpy);
            // NOLINTNEXTLINE(bugprone-undefined-memory-manipulation)
            memcpy(dst, src, reinterpret_cast<const char*>(src_end) - reinterpret_cast<const char*>(src));
        }
    }

    static void MoveOverlapping(T* const src, T* const src_end, T* const dst, VectorOperationOrigin origin)
    {
        if (!src || !dst) [[unlikely]] {
            return;
        }
    ...
    static void Swap(T* const src, T* const src_end, T* const dst, VectorOperationOrigin src_origin)
    {
        if constexpr (!VectorTraits<T>::kCanMoveWithMemcpy) {
            ...
            static_assert(VectorTraits<T>::kCanMoveWithMemcpy);
            std::swap_ranges(reinterpret_cast<char*>(src), reinterpret_cast<char*>(src_end), reinterpret_cast<char*>(dst));
        }
    }

    static void Copy(const T* src, const T* src_end, T* dst, VectorOperationOrigin origin)
    {
    ...
    static void UninitializedCopy(const U* src, const U* src_end, T* dst, VectorOperationOrigin origin, Proj proj = {})
    {
        if (!dst || !src) [[unlikely]] {
            return;
        }
        if constexpr (std::is_same_v<T, U> && std::is_same_v<Proj, std::identity> && VectorTraits<T>::kCanCopyWithMemcpy) {
            ...
            while (src != src_end) {
                ConstructTraits::ConstructAndNotifyElement(dst, std::invoke(proj, *src));
                ++dst;
                ++src;
            }
        }
    ...
    static void UninitializedFill(T* dst, T* dst_end, const T& val, VectorOperationOrigin origin)
    {
        if (!dst) [[unlikely]] {
            return;
        }
        if constexpr (VectorTraits<T>::kCanFillWithMemset) {
            ...
            static_assert(!Allocator::kIsGarbageCollected, "memset is unsupported for garbage-collected vectors.");
    ...
    template <typename U> static bool CompareElement(const T& left, U&& right)
    {
...
template <typename T, typename Allocator> class VectorBufferBase {
    DISALLOW_NEW();
...
public:
    ...
    void CheckUnusedSlots(const T* from, const T* to)
    {
        ...
        if constexpr (NeedsToClearUnusedSlots()) {
            const unsigned char* unused_area = reinterpret_cast<const unsigned char*>(from);
            const unsigned char* end_address = reinterpret_cast<const unsigned char*>(to);
...
private:
    static constexpr bool NeedsToClearUnusedSlots()
    {
        ...
        return Allocator::kIsGarbageCollected && (IsTraceable<T>::value || VectorTraits<T>::kNeedsDestruction);
    }

    void AllocateBufferNoBarrier(wtf_size_t new_capacity)
    {
        DCHECK(new_capacity);
        DCHECK_LE(new_capacity, Allocator::template MaxElementCountInBackingStore<T>());
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator = PartitionAllocator> class VectorBuffer;

template <typename T, typename Allocator> class VectorBuffer<T, 0, Allocator> : protected VectorBufferBase<T, Allocator> {
private:
...
public:
    ...
    void SwapVectorBuffer(VectorBuffer<T, 0, Allocator>& other, OffsetRange this_hole, OffsetRange other_hole, VectorOperationOrigin this_origin)
    {
    ...
    const T* const* BufferSlot() const
    {
        return &buffer_;
    }
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator> class VectorBuffer : protected VectorBufferBase<T, Allocator> {
private:
    using Base = VectorBufferBase<T, Allocator>;
...
public:
    ...
    void SwapVectorBuffer(VectorBuffer<T, InlineCapacity, Allocator>& other, OffsetRange this_hole, OffsetRange other_hole, VectorOperationOrigin this_origin)
    {
        ...
        if (this_source_begin && !other_source_begin) { // Our buffer is inline, theirs is not.
            ...
            std::swap(size_, other.size_);
            MARKING_AWARE_ANNOTATE_NEW_BUFFER(Allocator, other.buffer_, InlineCapacity, other.size_);
            if (this_origin != VectorOperationOrigin::kConstruction) {
                Allocator::BackingWriteBarrier(&buffer_);
            }
            ...
            AsAtomicPtr(&buffer_)->store(InlineBuffer(), std::memory_order_relaxed);
            std::swap(size_, other.size_);
            MARKING_AWARE_ANNOTATE_NEW_BUFFER(Allocator, buffer_, InlineCapacity, size_);
            Allocator::BackingWriteBarrier(&other.buffer_);
            ...
            DCHECK_EQ(Buffer(), InlineBuffer());
            DCHECK_EQ(other.Buffer(), other.InlineBuffer());
            MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, buffer_, InlineCapacity, size_, other.size_);
            MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, other.buffer_, InlineCapacity, other.size_, size_);
            std::swap(size_, other.size_);
        }
        ...
        while (section_begin < InlineCapacity) {
            ...
            if (this_occupied && other_occupied) {
                ...
                TypeOperations::Swap(this_source_begin + section_begin, this_source_begin + section_end, other_source_begin + section_begin, this_origin);
            } else if (this_occupied) {
                // Move from ours to theirs.
                TypeOperations::Move(this_source_begin + section_begin, this_source_begin + section_end, this_destination_begin + section_begin,
                    VectorOperationOrigin::kRegularModification);
                Base::ClearUnusedSlots(this_source_begin + section_begin, this_source_begin + section_end);
            } else if (other_occupied) {
                // Move from theirs to ours.
                TypeOperations::Move(
                    other_source_begin + section_begin, other_source_begin + section_end, other_destination_begin + section_begin, this_origin);
                Base::ClearUnusedSlots(other_source_begin + section_begin, other_source_begin + section_end);
            } else {
...
private:
    ...
    template <typename U, wtf_size_t inlineBuffer, typename V> friend class Deque;
};
...
template <typename T> class UncheckedIterator {
public:
    ...
    friend constexpr bool operator==(const UncheckedIterator& lhs, const UncheckedIterator& rhs)
    {
        return lhs.current_ == rhs.current_;
    }
    friend auto operator<=>(const UncheckedIterator& lhs, const UncheckedIterator& rhs)
    {
        return lhs.current_ <=> rhs.current_;
    }
    ...
    UNSAFE_BUFFER_USAGE friend UncheckedIterator operator+(difference_type lhs, const UncheckedIterator& rhs)
    {
        return rhs + lhs;
    }
    UNSAFE_BUFFER_USAGE UncheckedIterator& operator-=(difference_type rhs)
    ...
    friend difference_type operator-(const UncheckedIterator& lhs, const UncheckedIterator& rhs)
    {
        return lhs.current_ - rhs.current_;
    }
    ...
    friend std::ostream& operator<<(std::ostream& out, const UncheckedIterator& rhs)
    {
...
private:
    ...
    template <typename> friend class UncheckedIterator;
...
template <typename T, wtf_size_t InlineCapacity, bool isGced> inline constexpr bool kVectorNeedsDestructor = true;

// For garbage collection, Vector does not require destruction when there's no
// inline capacity.
template <typename T> inline constexpr bool kVectorNeedsDestructor<T, 0, true> = false;
...
// destructors on heap.
template <typename T, wtf_size_t InlineCapacity> inline constexpr bool kVectorNeedsDestructor<T, InlineCapacity, true> = true;

template <typename T, wtf_size_t InlineCapacity, typename Allocator> class Vector : private VectorBuffer<T, INLINE_CAPACITY, Allocator> {
...
public:
    ...
    // Copying.
    Vector(const Vector&);
    template <wtf_size_t otherCapacity> explicit Vector(const Vector<T, otherCapacity, Allocator>&);

    Vector& operator=(const Vector&);
    template <wtf_size_t otherCapacity> Vector& operator=(const Vector<T, otherCapacity, Allocator>&);

    // Copying with projection.
    template <typename Proj, typename = std::enable_if<std::is_invocable_v<Proj, const_reference>>> Vector(const Vector&, Proj);
    template <typename U, wtf_size_t otherCapacity, typename Proj,
        typename = std::enable_if<std::is_invocable_v<Proj, typename Vector<U, otherCapacity, Allocator>::const_reference>>>
    ...
    explicit Vector(const Range& range)
        : Vector()
    {
        assign(range);
    }
    ...
    template <typename U> wtf_size_t Find(const U&) const;
    template <typename U> wtf_size_t ReverseFind(const U&) const;
    ...
    template <typename... Args> T& emplace_back(Args&&...);
    ALWAYS_INLINE T& emplace_back()
    {
        Grow(size_ + 1);
        return back();
    }
    template <typename U> void Append(const U*, wtf_size_t);
    template <typename U, wtf_size_t otherCapacity, typename V> void AppendVector(const Vector<U, otherCapacity, V>&);
    template <typename Iterator> void AppendRange(Iterator begin, Iterator end);
    template <typename U, size_t N, typename Ptr> void AppendSpan(base::span<U, N, Ptr>);
    ...
    //     Insert multiple elements represented by either |buffer| and |size|
    //     or |vector| at |position|. The elements will be copied.
    template <typename U> void insert(wtf_size_t position, U&&);
    template <typename U> void insert(wtf_size_t position, const U*, wtf_size_t);
    template <typename U> void InsertAt(iterator position, U&&);
    template <typename U> void InsertAt(iterator position, const U*, wtf_size_t);
    template <typename U, wtf_size_t otherCapacity, typename OtherAllocator>
    void InsertVector(wtf_size_t position, const Vector<U, otherCapacity, OtherAllocator>&);
    ...
    template <typename U> void push_front(const U*, wtf_size_t);
    template <typename U, wtf_size_t otherCapacity, typename OtherAllocator> void PrependVector(const Vector<U, otherCapacity, OtherAllocator>&);
    ...
    {
        Base::SwapVectorBuffer(other, OffsetRange(), OffsetRange(), VectorOperationOrigin::kRegularModification);
    }
    ...
    ~Vector()
        requires(!kVectorNeedsDestructor<T, INLINE_CAPACITY, Allocator::kIsGarbageCollected>)
    = default;
    ~Vector()
        requires(kVectorNeedsDestructor<T, INLINE_CAPACITY, Allocator::kIsGarbageCollected>)
    {
...
protected:
    ...
    T** GetBufferSlot()
    {
        return Base::BufferSlot();
    }
    const T* const* GetBufferSlot() const
    {
        return Base::BufferSlot();
    }

private:
    template <typename, wtf_size_t, typename> friend class Vector;
    ...
    template <typename U> U* ExpandCapacity(wtf_size_t new_min_capacity, U*);
    template <typename U> NOINLINE PRESERVE_MOST void AppendSlowCase(U&&);
    ...
    struct TypeConstraints {
        constexpr TypeConstraints()
        {
            ...
            static_assert(!std::is_polymorphic_v<T> || !VectorTraits<T>::kCanInitializeWithMemset, "Cannot initialize with memset if there is a vtable.");
            static_assert(Allocator::kIsGarbageCollected || !IsDisallowNew<T> || !IsTraceable<T>::value,
                "Cannot put DISALLOW_NEW() objects that have trace methods "
                "into an off-heap Vector.");
            static_assert(Allocator::kIsGarbageCollected || !IsMemberType<T>::value, "Cannot put Member into an off-heap Vector. Use HeapVector instead.");
            static_assert(Allocator::kIsGarbageCollected || !IsWeakMemberType<T>::value, "WeakMember is not allowed in Vector nor HeapVector.");
            static_assert(Allocator::kIsGarbageCollected || !IsPointerToGarbageCollectedType<T>::value,
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator> inline Vector<T, InlineCapacity, Allocator>::Vector()
{
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
inline Vector<T, InlineCapacity, Allocator>::Vector(wtf_size_t size, const T& val)
    : Base(size)
{
    ANNOTATE_NEW_BUFFER(data(), capacity(), size);
    size_ = size;
    TypeOperations::UninitializedFill(data(), DataEnd(), val, VectorOperationOrigin::kConstruction);
}
    ...
    : Base(other.capacity())
{
    ...
    size_ = other.size();
    TypeOperations::UninitializedCopy(other.data(), other.DataEnd(), data(), VectorOperationOrigin::kConstruction);
}
    ...
    : Base(other.capacity())
{
    ANNOTATE_NEW_BUFFER(data(), capacity(), other.size());
    size_ = other.size();
    TypeOperations::UninitializedCopy(other.data(), other.DataEnd(), data(), VectorOperationOrigin::kConstruction, std::move(proj));
}
...
Vector<T, InlineCapacity, Allocator>::Vector(const Vector<U, otherCapacity, Allocator>& other, Proj proj)
    : Base(other.capacity())
{
    ANNOTATE_NEW_BUFFER(data(), capacity(), other.size());
    size_ = other.size();
...
Vector<T, InlineCapacity, Allocator>& Vector<T, InlineCapacity, Allocator>::operator=(const Vector<T, InlineCapacity, Allocator>& other)
{
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, data(), capacity(), size_, other.size());
    TypeOperations::Copy(other.data(), other.data() + size(), data(), VectorOperationOrigin::kRegularModification);
    TypeOperations::UninitializedCopy(other.data() + size(), other.DataEnd(), DataEnd(), VectorOperationOrigin::kRegularModification);
    size_ = other.size();
...
{
    static_assert(!std::is_same_v<Vector<T, InlineCapacity, Allocator>, Range>, "This method is for copying from a collection of a different type.");
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator> Vector<T, InlineCapacity, Allocator>::Vector(Vector<T, InlineCapacity, Allocator>&& other)
{
    size_ = 0;
    ...
    : Base(base::checked_cast<wtf_size_t>(elements.size()))
{
    ...
    TypeOperations::UninitializedCopy(elements.begin(), elements.end(), data(), VectorOperationOrigin::kConstruction);
}
...
Vector<T, InlineCapacity, Allocator>& Vector<T, InlineCapacity, Allocator>::operator=(std::initializer_list<T> elements)
{
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, data(), capacity(), size_, input_size);
    TypeOperations::Copy(elements.begin(), elements.begin() + size_, data(), VectorOperationOrigin::kRegularModification);
    TypeOperations::UninitializedCopy(elements.begin() + size_, elements.end(), DataEnd(), VectorOperationOrigin::kRegularModification);
    size_ = input_size;
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
template <typename U>
wtf_size_t Vector<T, InlineCapacity, Allocator>::ReverseFind(const U& value) const
{
    const T* b = data();
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
void Vector<T, InlineCapacity, Allocator>::Fill(const T& val, wtf_size_t new_size)
    requires(!Allocator::kIsGarbageCollected)
{
    if (size() > new_size) {
        Shrink(new_size);
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, data(), capacity(), size_, new_size);
    std::fill(begin(), end(), val);
    TypeOperations::UninitializedFill(DataEnd(), data() + new_size, val, VectorOperationOrigin::kRegularModification);
    size_ = new_size;
}

template <typename T, wtf_size_t InlineCapacity, typename Allocator> void Vector<T, InlineCapacity, Allocator>::ExpandCapacity(wtf_size_t new_min_capacity)
{
    ...
    reserve(std::max(new_min_capacity, std::max(kInitialVectorSize, expanded_capacity)));
}

template <typename T, wtf_size_t InlineCapacity, typename Allocator>
T* Vector<T, InlineCapacity, Allocator>::ExpandCapacity(wtf_size_t new_min_capacity, T* ptr)
{
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
template <typename U>
inline U* Vector<T, InlineCapacity, Allocator>::ExpandCapacity(wtf_size_t new_min_capacity, U* ptr)
{
    ExpandCapacity(new_min_capacity);
    return ptr;
}

template <typename T, wtf_size_t InlineCapacity, typename Allocator> inline void Vector<T, InlineCapacity, Allocator>::resize(wtf_size_t size)
{
    if (size <= size_) {
        TypeOperations::Destruct(data() + size, DataEnd());
        ClearUnusedSlots(data() + size, DataEnd());
        MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, data(), capacity(), size_, size);
    } else {
        if (size > capacity())
            ExpandCapacity(size);
        MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, data(), capacity(), size_, size);
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator> void Vector<T, InlineCapacity, Allocator>::Shrink(wtf_size_t size)
{
    CHECK_LE(size, size_);
    TypeOperations::Destruct(data() + size, DataEnd());
    ClearUnusedSlots(data() + size, DataEnd());
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator> void Vector<T, InlineCapacity, Allocator>::Grow(wtf_size_t size)
{
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator> void Vector<T, InlineCapacity, Allocator>::reserve(wtf_size_t new_capacity)
{
    if (new_capacity <= capacity()) [[unlikely]] {
        return;
    }
    if (!data()) {
        Base::AllocateBuffer(new_capacity, VectorOperationOrigin::kRegularModification);
        return;
    }
    ...
    if constexpr (Allocator::kIsGarbageCollected) {
        ...
        MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, data(), old_capacity, size_, old_capacity);
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
inline void Vector<T, InlineCapacity, Allocator>::ReserveInitialCapacity(wtf_size_t initial_capacity)
{
    DCHECK(!size_);
    ...
    if (initial_capacity > INLINE_CAPACITY) {
        ...
        // constructor.
        Base::AllocateBuffer(initial_capacity, VectorOperationOrigin::kRegularModification);
        MARKING_AWARE_ANNOTATE_NEW_BUFFER(Allocator, data(), capacity(), size_);
    }
}

template <typename T, wtf_size_t InlineCapacity, typename Allocator> void Vector<T, InlineCapacity, Allocator>::ShrinkCapacity(wtf_size_t new_capacity)
{
...
ALWAYS_INLINE void Vector<T, InlineCapacity, Allocator>::push_back(U&& val)
{
    ...
    if (size() != capacity()) [[likely]] {
        MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, data(), capacity(), size_, size_ + 1);
        ConstructTraits<T, VectorTraits<T>, Allocator>::ConstructAndNotifyElement(DataEnd(), std::forward<U>(val));
        ++size_;
        return;
    }
...
ALWAYS_INLINE T& Vector<T, InlineCapacity, Allocator>::emplace_back(Args&&... args)
{
    DCHECK(Allocator::IsAllocationAllowed());
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, data(), capacity(), size_, size_ + 1);
    T* t = ConstructTraits<T, VectorTraits<T>, Allocator>::ConstructAndNotifyElement(DataEnd(), std::forward<Args>(args)...);
    ++size_;
    return *t;
}
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
template <typename U>
void Vector<T, InlineCapacity, Allocator>::Append(const U* data, wtf_size_t data_size)
{
    DCHECK(Allocator::IsAllocationAllowed());
    ...
    T* dest = DataEnd();
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, this->data(), capacity(), size_, new_size);
    TypeOperations::UninitializedCopy(data, &data[data_size], dest, VectorOperationOrigin::kRegularModification);
    size_ = new_size;
}

template <typename T, wtf_size_t InlineCapacity, typename Allocator>
template <typename U>
NOINLINE PRESERVE_MOST void Vector<T, InlineCapacity, Allocator>::AppendSlowCase(U&& val)
{
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, data(), capacity(), size_, size_ + 1);
    ConstructTraits<T, VectorTraits<T>, Allocator>::ConstructAndNotifyElement(DataEnd(), std::forward<U>(*ptr));
    ++size_;
}
...
void Vector<T, InlineCapacity, Allocator>::AppendRange(Iterator begin, Iterator end)
{
...
void Vector<T, InlineCapacity, Allocator>::AppendSpan(base::span<U, N, Ptr> data)
{
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
template <typename U>
inline void Vector<T, InlineCapacity, Allocator>::insert(wtf_size_t position, U&& val)
{
    DCHECK(Allocator::IsAllocationAllowed());
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, this->data(), capacity(), size_, size_ + 1);
    T* spot = this->data() + position;
    TypeOperations::MoveOverlapping(spot, DataEnd(), spot + 1, VectorOperationOrigin::kRegularModification);
    ConstructTraits<T, VectorTraits<T>, Allocator>::ConstructAndNotifyElement(spot, std::forward<U>(*data));
    ++size_;
}

template <typename T, wtf_size_t InlineCapacity, typename Allocator>
template <typename U>
void Vector<T, InlineCapacity, Allocator>::insert(wtf_size_t position, const U* data, wtf_size_t data_size)
{
    DCHECK(Allocator::IsAllocationAllowed());
    CHECK_LE(position, size());
    wtf_size_t new_size = size_ + data_size;
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, this->data(), capacity(), size_, new_size);
    T* spot = this->data() + position;
    TypeOperations::MoveOverlapping(spot, DataEnd(), spot + data_size, VectorOperationOrigin::kRegularModification);
    TypeOperations::UninitializedCopy(data, &data[data_size], spot, VectorOperationOrigin::kRegularModification);
    size_ = new_size;
}

template <typename T, wtf_size_t InlineCapacity, typename Allocator>
template <typename U>
void Vector<T, InlineCapacity, Allocator>::InsertAt(Vector::iterator position, U&& val)
{
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
template <typename U>
void Vector<T, InlineCapacity, Allocator>::InsertAt(Vector::iterator position, const U* data, wtf_size_t data_size)
{
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
template <typename U, wtf_size_t otherCapacity, typename OtherAllocator>
inline void Vector<T, InlineCapacity, Allocator>::InsertVector(wtf_size_t position, const Vector<U, otherCapacity, OtherAllocator>& val)
{
    insert(position, val.data(), val.size());
}

template <typename T, wtf_size_t InlineCapacity, typename Allocator> template <typename U> inline void Vector<T, InlineCapacity, Allocator>::push_front(U&& val)
{
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
template <typename U>
void Vector<T, InlineCapacity, Allocator>::push_front(const U* data, wtf_size_t data_size)
{
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
template <typename U, wtf_size_t otherCapacity, typename OtherAllocator>
inline void Vector<T, InlineCapacity, Allocator>::PrependVector(const Vector<U, otherCapacity, OtherAllocator>& val)
{
    insert(0, val.data(), val.size());
}

template <typename T, wtf_size_t InlineCapacity, typename Allocator> inline void Vector<T, InlineCapacity, Allocator>::EraseAt(wtf_size_t position)
{
    CHECK_LT(position, size());
    T* spot = data() + position;
    spot->~T();
    TypeOperations::MoveOverlapping(spot + 1, DataEnd(), spot, VectorOperationOrigin::kRegularModification);
    ClearUnusedSlots(DataEnd() - 1, DataEnd());
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, data(), capacity(), size_, size_ - 1);
    --size_;
}

template <typename T, wtf_size_t InlineCapacity, typename Allocator> inline auto Vector<T, InlineCapacity, Allocator>::erase(iterator position) -> iterator
{
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
inline auto Vector<T, InlineCapacity, Allocator>::erase(iterator first, iterator last) -> iterator
{
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator>
inline void Vector<T, InlineCapacity, Allocator>::EraseAt(wtf_size_t position, wtf_size_t length)
{
    ...
    TypeOperations::MoveOverlapping(end_spot, DataEnd(), begin_spot, VectorOperationOrigin::kRegularModification);
    ClearUnusedSlots(DataEnd() - length, DataEnd());
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, data(), capacity(), size_, size_ - length);
    size_ -= length;
}

template <typename T, wtf_size_t InlineCapacity, typename Allocator> inline void Vector<T, InlineCapacity, Allocator>::Reverse()
{
...
template <typename T, wtf_size_t InlineCapacityA, wtf_size_t InlineCapacityB, typename Allocator>
...
namespace internal {
template <typename Allocator, typename VisitorDispatcher, typename T> void TraceInlinedBuffer(VisitorDispatcher visitor, const T* buffer_begin, size_t capacity)
{
    ...
    for (const T* buffer_entry = buffer_begin; buffer_entry != buffer_end; buffer_entry++) {
        Allocator::template Trace<T, VectorTraits<T>>(visitor, *buffer_entry);
    }
}

template <typename Allocator, typename VisitorDispatcher, typename T, wtf_size_t InlineCapacity>
void DeferredTraceImpl(VisitorDispatcher visitor, const void* object)
{
    internal::TraceInlinedBuffer<Allocator>(visitor, reinterpret_cast<const T*>(object), InlineCapacity);
}
...
void Vector<T, InlineCapacity, Allocator>::Trace(auto visitor) const
    requires Allocator::kIsGarbageCollected
{
    static_assert(Allocator::kIsGarbageCollected, "Garbage collector must be enabled.");

    const T* buffer = BufferSafe();

    if (!buffer) {
        // Register the slot for heap compaction.
        Allocator::TraceVectorBacking(visitor, static_cast<T*>(nullptr), Base::BufferSlot());
        return;
    }
    ...
    if (Base::IsOutOfLineBuffer(buffer)) {
        ...
        if (!VectorTraits<T>::kCanTraceConcurrently) {
            if (Allocator::DeferTraceToMutatorThreadIfConcurrent(
                    visitor, buffer, internal::DeferredTraceImpl<Allocator, decltype(visitor), T, InlineCapacity>, InlineCapacity * sizeof(T))) {
                return;
            }
        }
...
template <typename T, wtf_size_t InlineCapacity, typename Allocator> void Vector<T, InlineCapacity, Allocator>::ReallocateBuffer(wtf_size_t new_capacity)
{
...
#endif
        Base::ResetBufferPointer();
        TypeOperations::Move(old_begin, old_end, data(), VectorOperationOrigin::kRegularModification);
    ...
    VectorBufferBase<T, Allocator> temp_buffer = Base::AllocateTemporaryBuffer(new_capacity);
    ...
    // white.
    TypeOperations::Move(data(), DataEnd(), temp_buffer.Buffer(), VectorOperationOrigin::kConstruction);
...
template <typename T, wtf_size_t inline_capacity, typename Allocator, typename U> wtf_size_t Erase(Vector<T, inline_capacity, Allocator>& v, const U& value)
{
...
template <typename T, wtf_size_t inline_capacity, typename Allocator, typename Pred> wtf_size_t EraseIf(Vector<T, inline_capacity, Allocator>& v, Pred pred)
{
