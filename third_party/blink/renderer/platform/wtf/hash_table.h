...
#define UPDATE_ACCESS_COUNTS()                                                                                                                                 \
    HashTableStats::instance().numAccesses.fetch_add(1, std::memory_order_relaxed);                                                                            \
    int probeCount = 0;                                                                                                                                        \
...
struct WTF_EXPORT HashTableStats {
    ...
    template <typename VisitorDispatcher> void trace(VisitorDispatcher) const
    {
    }
...
#if DUMP_HASHTABLE_STATS_PER_TABLE
template <typename Allocator, bool isGCType = Allocator::kIsGarbageCollected> class HashTableStatsPtr;

template <typename Allocator> class HashTableStatsPtr<Allocator, false> final {
...
public:
    ...
    static std::unique_ptr<HashTableStats> copy(const std::unique_ptr<HashTableStats>& other)
    {
        if (!other)
            return nullptr;
    ...
    static void swap(std::unique_ptr<HashTableStats>& stats, std::unique_ptr<HashTableStats>& other)
    {
        stats.swap(other);
    }
};

template <typename Allocator> class HashTableStatsPtr<Allocator, true> final {
    STATIC_ONLY(HashTableStatsPtr);
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator> class HashTable;
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator> class HashTableIterator;
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator> class HashTableConstIterator;
template <WeakHandlingFlag x, typename T, typename U, typename V, typename W, typename X, typename Y> struct WeakProcessingHashTableHelper;

typedef enum { kHashItemKnownGood } HashItemKnownGoodTag;

template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator> class HashTableConstIterator final {
    DISALLOW_NEW();

private:
    typedef HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator> HashTableType;
    typedef HashTableIterator<Key, Value, Extractor, Traits, KeyTraits, Allocator> iterator;
    typedef HashTableConstIterator<Key, Value, Extractor, Traits, KeyTraits, Allocator> const_iterator;
    ...
    friend class HashTableIterator<Key, Value, Extractor, Traits, KeyTraits, Allocator>;
    ...
    HashTableConstIterator(PointerType position, PointerType begin_position, PointerType end_position, const HashTableType* container)
        : position_(position)
    ...
    HashTableConstIterator(PointerType position, PointerType begin_position, PointerType end_position, const HashTableType* container, HashItemKnownGoodTag)
        : position_(position)
        , end_position_(end_position)
#if DCHECK_IS_ON()
        , begin_position_(begin_position)
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
std::ostream& operator<<(std::ostream& stream, const HashTableConstIterator<Key, Value, Extractor, Traits, KeyTraits, Allocator>& iterator)
{
    return iterator.PrintTo(stream);
}

template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator> class HashTableIterator final {
    DISALLOW_NEW();
...
private:
    ...
    HashTableIterator(PointerType pos, PointerType begin, PointerType end, const HashTableType* container)
        : iterator_(pos, begin, end, container)
    {
    }
    HashTableIterator(PointerType pos, PointerType begin, PointerType end, const HashTableType* container, HashItemKnownGoodTag tag)
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
std::ostream& operator<<(std::ostream& stream, const HashTableIterator<Key, Value, Extractor, Traits, KeyTraits, Allocator>& iterator)
{
    return iterator.PrintTo(stream);
}
...
template <typename T, typename Allocator, typename Traits, bool enterGCForbiddenScope> struct Mover {
    STATIC_ONLY(Mover);
...
template <typename T, typename Allocator, typename Traits> struct Mover<T, Allocator, Traits, true> {
    STATIC_ONLY(Mover);
    static void Move(T&& from, T& to)
    {
...
template <typename KeyTraits> class IdentityHashTranslator {
    STATIC_ONLY(IdentityHashTranslator);

public:
    template <typename T> static unsigned GetHash(const T& key)
    {
        return KeyTraits::GetHash(key);
    }
    template <typename T, typename U> static bool Equal(const T& a, const U& b)
    {
        return KeyTraits::Equal(a, b);
    }
    template <typename T, typename U, typename V> static void Store(T& location, U&&, V&& value)
    {
...
public:
    HashTableAddResult([[maybe_unused]] const HashTableType* container, ValueType* stored_value, bool is_new_entry)
...
template <typename HashTranslator, typename KeyTraits, bool safeToCompareToEmptyOrDeleted> struct HashTableKeyChecker {
    ...
    template <typename T> static bool CheckKey(const T&)
    {
        return true;
    }
};

template <typename HashTranslator, typename KeyTraits> struct HashTableKeyChecker<HashTranslator, KeyTraits, true> {
    STATIC_ONLY(HashTableKeyChecker);
    template <typename T> static bool CheckKey(const T& key)
    {
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator> class HashTable final {
    DISALLOW_NEW();
...
public:
    ...
    template <typename IncomingValueType> AddResult insert(IncomingValueType&& value)
    {
        return insert<IdentityTranslatorType>(Extractor::ExtractKey(value), std::forward<IncomingValueType>(value));
    }
    ...
    template <typename HashTranslator, typename T, typename Extra> AddResult insert(T&& key, Extra&&);
    ...
    template <typename HashTranslator, typename T, typename Extra> AddResult InsertPassingHashCode(T&& key, Extra&&);
    ...
    {
        return IsHashTraitsEmptyOrDeletedValue<KeyTraits>(Extractor::ExtractKey(value));
    }
    ...
    bool AccessForbidden() const
    {
        return access_forbidden_;
    }
    int64_t Modifications() const
    {
    ...
    ALWAYS_INLINE bool AccessForbidden() const
    {
        return false;
    }
    ALWAYS_INLINE int64_t Modifications() const
    {
        return 0;
    }
...
private:
    ...
    template <typename HashTranslator, typename T> LookupResult LookupForWriting(const T&);
    ...
    bool ShouldShrink() const
    {
        ...
        // expensive.
        return key_count_ * kMinLoad < table_size_ && table_size_ > KeyTraits::kMinimumTableSize && Allocator::IsAllocationAllowed();
    }
    ...
    ValueType* RehashTo(ValueType* new_table, unsigned new_table_size, ValueType* entry);
...
#if DUMP_HASHTABLE_STATS_PER_TABLE
public:
    mutable typename std::conditional<Allocator::kIsGarbageCollected, HashTableStats*, std::unique_ptr<HashTableStats>>::type stats_;
    void DumpStats()
    {
        if (stats_) {
            stats_->DumpStats();
    ...
    template <WeakHandlingFlag x, typename T, typename U, typename V, typename W, typename X, typename Y> friend struct WeakProcessingHashTableHelper;
    ...
    struct TypeConstraints {
        constexpr TypeConstraints()
        {
            ...
            static_assert(Allocator::kIsGarbageCollected || (!IsPointerToGarbageCollectedType<Key>::value && !IsPointerToGarbageCollectedType<Value>::value),
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
inline HashTable<Key, Value, Extractor,

    Traits, KeyTraits, Allocator>::HashTable()
    : table_(nullptr)
    , table_size_(0)
    , key_count_(0)
    , deleted_count_(0)
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
void HashTable<Key, Value, Extractor,

    Traits, KeyTraits, Allocator>::ReserveCapacityForSize(unsigned new_size)
{
    ...
    if (new_capacity > Capacity()) {
        CHECK(!static_cast<int>(new_capacity >> 31)); // HashTable capacity should not overflow 32bit int.
...
inline Value* HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::Lookup(const T& key)
{
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
template <typename HashTranslator, typename T>
inline const Value* HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::Lookup(const T& key) const
{
    DCHECK(!AccessForbidden());
    DCHECK((HashTableKeyChecker<HashTranslator, KeyTraits, KeyTraits::kSafeToCompareToEmptyOrDeleted>::CheckKey(key)));
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
template <typename HashTranslator, typename T>
inline typename HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::LookupResult
HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::LookupForWriting(const T& key)
{
    DCHECK(!AccessForbidden());
    DCHECK(table_);
    ...
    bool can_reuse_deleted_entry = Allocator::template CanReuseHashTableDeletedBucket<Traits>();
...
template <typename Traits, typename Allocator, typename Value, bool = Traits::kEmptyValueIsZero> struct HashTableBucketInitializer {
    STATIC_ONLY(HashTableBucketInitializer);
    static_assert(!Traits::kEmptyValueIsZero);

    static void Reinitialize(Value& bucket)
    {
        ConstructTraits<Value, Traits, Allocator>::ConstructAndNotifyElement(&bucket, Traits::EmptyValue());
        DCHECK(IsHashTraitsEmptyValue<Traits>(bucket));
    }

    template <typename HashTable> static Value* AllocateTable(unsigned size, size_t alloc_size)
    {
        Value* result = Allocator::template AllocateHashTableBacking<Value, HashTable>(alloc_size);
...
template <typename Traits, typename Allocator, typename Value> struct HashTableBucketInitializer<Traits, Allocator, Value, true> {
    STATIC_ONLY(HashTableBucketInitializer);
    static void Reinitialize(Value& bucket)
    {
    ...
    template <typename HashTable> static Value* AllocateTable(unsigned size, size_t alloc_size)
    {
        Value* result = Allocator::template AllocateZeroedHashTableBacking<Value, HashTable>(alloc_size);
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
inline void HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::ReinitializeBucket(ValueType& bucket)
{
...
typename HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::AddResult HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::insert(
    T&& key, Extra&& extra)
{
    DCHECK(!AccessForbidden());
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
template <typename HashTranslator, typename T, typename Extra>
typename HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::AddResult
HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::InsertPassingHashCode(T&& key, Extra&& extra)
{
    DCHECK(!AccessForbidden());
    DCHECK(Allocator::IsAllocationAllowed());
    if (!table_)
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
Value* HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::Reinsert(ValueType&& entry)
{
    DCHECK(table_);
    DCHECK(!AccessForbidden());
    RegisterModification();
#if DUMP_HASHTABLE_STATS
    HashTableStats::instance().numReinserts.fetch_add(1, std::memory_order_relaxed);
#endif
#if DUMP_HASHTABLE_STATS_PER_TABLE
    ...
    Mover<ValueType, Allocator, Traits, Traits::template NeedsToForbidGCOnMove<>::value>::Move(std::move(entry), *new_entry);
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
template <typename HashTranslator, typename T>
inline typename HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::iterator HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::Find(
    const T& key)
{
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
template <typename HashTranslator, typename T>
inline typename HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::const_iterator
HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::Find(const T& key) const
{
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
template <typename HashTranslator, typename T>
bool HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::Contains(const T& key) const
{
    return Lookup<HashTranslator>(key);
}

template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
void HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::erase(const ValueType* pos)
{
    RegisterModification();
#if DUMP_HASHTABLE_STATS
...
void HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::erase_if(Pred pred)
{
    RegisterModification();
    EnterAccessForbiddenScope();
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
inline void HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::erase(iterator it)
{
    if (it == end())
        return;
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
inline void HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::erase(const_iterator it)
{
    if (it == end())
        return;
    erase(it.position_);
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
inline void HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::erase(KeyPeekInType key)
{
    erase(find(key));
}

template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
Value* HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::AllocateTable(unsigned size)
{
    ...
    // polymorphic.
    static_assert(!Traits::kEmptyValueIsZero || !std::is_polymorphic<KeyType>::value, "empty value cannot be zero for things with a vtable");
    static_assert(Allocator::kIsGarbageCollected
            || ((!IsDisallowNew<KeyType> || !IsTraceable<KeyType>::value) && (!IsDisallowNew<ValueType> || !IsTraceable<ValueType>::value)),
    ...
    return HashTableBucketInitializer<Traits, Allocator, Value>::template AllocateTable<HashTable>(size, alloc_size);
}

template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
void HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::DeleteAllBucketsAndDeallocate(ValueType* table, unsigned size)
{
    ...
    const bool needs_bucket_deletion
        = !std::is_trivially_destructible<ValueType>::value || (WTF::IsWeak<ValueType>::value && Allocator::IsIncrementalMarking());
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
Value* HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::Expand(Value* entry)
{
    unsigned new_size;
    if (!table_size_) {
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
Value* HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::ExpandBuffer(unsigned new_table_size, Value* entry, bool& success)
{
    success = false;
    DCHECK_LT(table_size_, new_table_size);
    CHECK(Allocator::IsAllocationAllowed());
    if (!table_ || !Allocator::template ExpandHashTableBacking<ValueType, HashTable>(table_, new_table_size * sizeof(ValueType)))
        return nullptr;
    ...
    for (unsigned i = 0; i < old_table_size; i++) {
        ...
        if (IsEmptyOrDeletedBucket(table_[i])) {
            ...
            Mover<ValueType, Allocator, Traits, Traits::template NeedsToForbidGCOnMove<>::value>::Move(std::move(table_[i]), temporary_table[i]);
            table_[i].~ValueType();
        }
    }
    ...
    HashTableBucketInitializer<Traits, Allocator, Value>::InitializeTable(original_table, new_table_size);
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
Value* HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::RehashTo(ValueType* new_table, unsigned new_table_size, Value* entry)
{
#if DUMP_HASHTABLE_STATS
    if (table_size_ != 0) {
        HashTableStats::instance().numRehashes.fetch_add(1, std::memory_order_relaxed);
    }
#endif
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
Value* HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::Rehash(unsigned new_table_size, Value* entry)
{
    unsigned old_table_size = table_size_;
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::HashTable(const HashTable& other)
    : table_(nullptr)
    , table_size_(0)
    , key_count_(0)
    , deleted_count_(0)
...
#endif
{
    ...
    for (unsigned i = 0; i < table_size_; i++) {
        if (other.IsEmptyBucket(other.table_[i])) {
            ...
            ConstructHashTraitsDeletedValue<KeyTraits>(Extractor::ExtractKey(table_[i]));
        } else {
            new (&table_[i]) ValueType(other.table_[i]);
            ConstructTraits<ValueType, Traits, Allocator>::NotifyNewElement(&table_[i]);
        }
    }
}

template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::HashTable(HashTable&& other)
    : table_(nullptr)
    , table_size_(0)
    , key_count_(0)
    , deleted_count_(0)
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
void HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::swap(HashTable& other)
{
    DCHECK(!AccessForbidden());
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>& HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::operator=(
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>& HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::operator=(HashTable&& other)
{
    swap(other);
    return *this;
}

template <WeakHandlingFlag weakHandlingFlag, typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
struct WeakProcessingHashTableHelper<kWeakHandling, Key, Value, Extractor, Traits, KeyTraits, Allocator> {
    STATIC_ONLY(WeakProcessingHashTableHelper);

    using HashTableType = HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>;
    using ValueType = typename HashTableType::ValueType;

    // Used for purely weak and for weak-and-strong tables (ephemerons).
    static void Process(const typename Allocator::LivenessBroker& info, const void* parameter)
    {
        HashTableType* table = reinterpret_cast<HashTableType*>(const_cast<void*>(parameter));
        ...
        for (ValueType* element = table->table_ + table->table_size_ - 1; element >= table->table_; element--) {
            if (!HashTableType::IsEmptyOrDeletedBucket(*element)) {
                if (!TraceInCollectionTrait<kWeakHandling, ValueType, Traits>::IsAlive(info, *element)) {
...
template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
void HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::Trace(auto visitor) const
    requires Allocator::kIsGarbageCollected
{
    static_assert(WTF::IsWeak<ValueType>::value || IsTraceable<ValueType>::value, "Value should not be traced");
    TraceTable(visitor, AsAtomicPtr(&table_)->load(std::memory_order_relaxed));
}

template <typename Key, typename Value, typename Extractor, typename Traits, typename KeyTraits, typename Allocator>
void HashTable<Key, Value, Extractor, Traits, KeyTraits, Allocator>::TraceTable(auto visitor, const ValueType* table) const
    requires Allocator::kIsGarbageCollected
{
    if (!WTF::IsWeak<ValueType>::value) {
        // Strong HashTable.
        Allocator::template TraceHashTableBackingStrongly<ValueType, HashTable>(visitor, table, &table_);
    } else {
        ...
        Allocator::template TraceHashTableBackingWeakly<ValueType, HashTable>(visitor, table, &table_,
            WeakProcessingHashTableHelper<kWeakHandlingTrait<ValueType>, Key, Value, Extractor, Traits, KeyTraits, Allocator>::Process, this);
    }
}

// iterator adapters

template <typename HashTableType, typename Traits, typename Enable = void> struct HashTableConstIteratorAdapter {
...
struct HashTableConstIteratorAdapter<HashTableType, Traits, typename std::enable_if_t<IsTraceable<typename Traits::TraitType>::value>> {
    static_assert(IsTraceable<typename Traits::TraitType>::value);
    STACK_ALLOCATED();
...
std::ostream& operator<<(std::ostream& stream, const HashTableConstIteratorAdapter<HashTable, Traits, Enable>& iterator)
{
    return stream << iterator.impl_;
}

template <typename HashTableType, typename Traits, typename Enable = void> struct HashTableIteratorAdapter {
    static_assert(!IsTraceable<typename Traits::TraitType>::value);
...
template <typename HashTableType, typename Traits>
struct HashTableIteratorAdapter<HashTableType, Traits, typename std::enable_if_t<IsTraceable<typename Traits::TraitType>::value>> {
    static_assert(IsTraceable<typename Traits::TraitType>::value);
    STACK_ALLOCATED();
...
template <typename HashTable, typename Traits, typename Enable>
std::ostream& operator<<(std::ostream& stream, const HashTableIteratorAdapter<HashTable, Traits, Enable>& iterator)
{
    return stream << iterator.impl_;
}
...
template <typename T, typename U> inline bool operator==(const HashTableConstIteratorAdapter<T, U>& a, const HashTableIteratorAdapter<T, U>& b)
{
    return a.impl_ == b.impl_;
}
...
template <typename T, typename U> inline bool operator==(const HashTableIteratorAdapter<T, U>& a, const HashTableConstIteratorAdapter<T, U>& b)
{
    return a.impl_ == b.impl_;
}
...
template <typename Collection1, typename Collection2> inline void RemoveAll(Collection1& collection, const Collection2& to_be_removed)
{
