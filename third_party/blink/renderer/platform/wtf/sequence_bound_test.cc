...
{
    SequenceBound<Foo> sequence_bound(base::ThreadPool::CreateSingleThreadTaskRunner({}));
    ...
    sequence_bound.AsyncCall(&Foo::Baz).Then(
        CrossThreadBindOnce(&SequenceBoundTest::CheckValue, CrossThreadUnretained(this), CrossThreadUnretained(&run_loop), CrossThreadUnretained(&test_value)));
    run_loop.Run();
