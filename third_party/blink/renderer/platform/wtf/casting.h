...
// }
template <typename Derived> struct DowncastTraits;

namespace internal {

template <typename Derived, typename Base> struct DowncastTraitsHelper {
...
template <typename Derived, typename Base>
    requires(std::is_base_of_v<Derived, Base>)
struct DowncastTraitsHelper<Derived, Base> {
    static bool AllowFrom(const Base&)
    {
        return true;
    }
};
...
template <typename Derived, typename Base> bool IsA(const Base& from)
{
...
template <typename Derived, typename Base> bool IsA(Base& from)
{
    static_assert(std::is_base_of_v<Base, Derived>, "Unnecessary type check");
    return internal::DowncastTraitsHelper<Derived, const Base>::AllowFrom(const_cast<const Base&>(from));
}
...
template <typename Derived, typename Base> const Derived& To(const Base& from)
{
...
template <typename Derived, typename Base> Derived& To(Base& from)
{
    CHECK(IsA<Derived>(from));
...
template <typename Derived, typename Base> const Derived* DynamicTo(const Base* from)
{
...
template <typename Derived, typename Base> Derived* DynamicTo(Base* from)
{
    // TOOD(https://crbug.com/1449302): Figure out why IsA<T> + To<T> does not
    // optimize correctly.
...
template <typename Derived, typename Base> const Derived& UnsafeTo(const Base& from)
{
...
template <typename Derived, typename Base> Derived& UnsafeTo(Base& from)
{
    SECURITY_DCHECK(IsA<Derived>(from));
    return static_cast<Derived&>(from);
}
