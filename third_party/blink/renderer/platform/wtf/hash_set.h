...
template <typename ValueArg, typename TraitsArg = HashTraits<ValueArg>, typename Allocator = PartitionAllocator> class HashSet {
...
private:
    typedef HashTable<ValueType, ValueType, IdentityExtractor, ValueTraits, ValueTraits, Allocator> HashTableType;
...
private:
    ...
    struct TypeConstraints {
        constexpr TypeConstraints()
        {
            ...
            static_assert(Allocator::kIsGarbageCollected || !IsPointerToGarbageCollectedType<ValueArg>::value,
...
struct IdentityExtractor {
    ...
    template <typename T> static const T& ExtractKey(const T& t)
    {
        return t;
    }
    template <typename T> static T& ExtractKey(T& t)
    {
        return t;
    }
    // Assumes out points to a buffer of size at least sizeof(T).
    template <typename T> static void ExtractKeyToMemory(const T& t, void* out)
    {
        AtomicReadMemcpy<sizeof(T), alignof(T)>(out, &t);
    }
    template <typename T> static void ClearValue(const T&)
    {
    }
};

template <typename Translator> struct HashSetTranslatorAdapter {
    ...
    template <typename T, typename U, typename V> static void Store(T& location, U&& key, const V&, unsigned hash_code)
    {
        Translator::Store(location, std::forward<U>(key), hash_code);
    }
};

template <typename Value, typename Traits, typename Allocator> HashSet<Value, Traits, Allocator>::HashSet(std::initializer_list<ValueType> elements)
{
    if (elements.size()) {
...
auto HashSet<Value, Traits, Allocator>::operator=(std::initializer_list<ValueType> elements) -> HashSet&
{
...
template <typename T, typename U, typename V> bool operator==(const HashSet<T, U, V>& a, const HashSet<T, U, V>& b)
{
    if (a.size() != b.size())
        return false;
...
template <typename T, typename U, typename V> inline bool operator!=(const HashSet<T, U, V>& a, const HashSet<T, U, V>& b)
{
    return !(a == b);
}

template <typename T, typename U, typename V> inline unsigned HashSet<T, U, V>::size() const
{
    return impl_.size();
}

template <typename T, typename U, typename V> inline unsigned HashSet<T, U, V>::Capacity() const
{
    return impl_.Capacity();
}

template <typename T, typename U, typename V> inline bool HashSet<T, U, V>::empty() const
{
    return impl_.empty();
}

template <typename T, typename U, typename V> inline typename HashSet<T, U, V>::iterator HashSet<T, U, V>::begin() const
{
    return impl_.begin();
}

template <typename T, typename U, typename V> inline typename HashSet<T, U, V>::iterator HashSet<T, U, V>::end() const
{
    return impl_.end();
}

template <typename T, typename U, typename V> inline typename HashSet<T, U, V>::iterator HashSet<T, U, V>::find(ValuePeekInType value) const
{
    return impl_.find(value);
}

template <typename Value, typename Traits, typename Allocator> inline bool HashSet<Value, Traits, Allocator>::Contains(ValuePeekInType value) const
{
...
typename HashSet<Value, Traits, Allocator>::iterator inline HashSet<Value, Traits, Allocator>::Find(const T& value) const
{
...
{
    return impl_.template Contains<HashSetTranslatorAdapter<HashTranslator>>(value);
}

template <typename T, typename U, typename V>
template <typename IncomingValueType>
inline typename HashSet<T, U, V>::AddResult HashSet<T, U, V>::insert(IncomingValueType&& value)
{
...
template <typename Value, typename Traits, typename Allocator>
template <typename HashTranslator, typename T>
inline typename HashSet<Value, Traits, Allocator>::AddResult HashSet<Value, Traits, Allocator>::AddWithTranslator(T&& value)
{
    // Forward only the first argument, because the second argument isn't actually
    // used in HashSetTranslatorAdapter.
    return impl_.template InsertPassingHashCode<HashSetTranslatorAdapter<HashTranslator>>(std::forward<T>(value), value);
}

template <typename T, typename U, typename V> inline void HashSet<T, U, V>::erase(iterator it)
{
    impl_.erase(it.impl_);
}

template <typename T, typename U, typename V> inline void HashSet<T, U, V>::erase(ValuePeekInType value)
{
    erase(find(value));
}

template <typename T, typename U, typename V> template <typename Pred> inline void HashSet<T, U, V>::erase_if(Pred pred)
{
...
template <typename T, typename U, typename V> inline auto HashSet<T, U, V>::Take(iterator it) -> ValueType
{
    if (it == end())
...
template <typename T, typename U, typename V> inline auto HashSet<T, U, V>::Take(ValuePeekInType value) -> ValueType
{
    return Take(find(value));
}

template <typename T, typename U, typename V> inline auto HashSet<T, U, V>::TakeAny() -> ValueType
{
    return Take(begin());
}
