...
#elif BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_FREEBSD) || BUILDFLAG(IS_FUCHSIA)
...
void* GetStackStart()
{
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_FREEBSD) || BUILDFLAG(IS_FUCHSIA)
    pthread_attr_t attr;
    int error;
#if BUILDFLAG(IS_FREEBSD)
    pthread_attr_init(&attr);
...
void InitializeMainThreadStackEstimate()
{
    ...
    g_main_thread_stack_start = reinterpret_cast<uintptr_t>(GetStackStart()) - sizeof(void*);
