...
template <class BitFieldType> class BitFieldBase;
...
template <class ValueType, size_t offset, size_t size, class BitFieldType, BitFieldValueConstness is_const = BitFieldValueConstness::kNonConst>
...
public:
    ...
    template <class OtherValueType, int other_size, BitFieldValueConstness other_is_const = BitFieldValueConstness::kNonConst>
...
template <class BitFieldType> class WTF_EXPORT SingleThreadedBitField {
    static_assert(std::is_fundamental<BitFieldType>::value, "Bit fields must be of a primitive type.");
    static_assert(std::is_unsigned<BitFieldType>::value, "Bit field must be of an unsigned type");
...
public:
    ...
    explicit SingleThreadedBitField()
        : SingleThreadedBitField(0)
    {
    }
    explicit SingleThreadedBitField(BitFieldType bits)
        : bits_(bits)
    {
    }

    template <typename Value> typename Value::Type get() const
    {
        return Value::decode(bits_);
    }

    template <typename Value> void set(typename Value::Type value)
    {
        bits_ = Value::update(bits_, value);
    }

    BitFieldType bits() const
    {
        return bits_;
    }
...
template <class BitFieldType> class WTF_EXPORT ConcurrentlyReadBitField : public SingleThreadedBitField<BitFieldType> {
    using Base = SingleThreadedBitField<BitFieldType>;
    using Base::bits_;

public:
    explicit ConcurrentlyReadBitField()
        : Base(0)
    {
    }
    explicit ConcurrentlyReadBitField(BitFieldType bits)
        : Base(bits)
    {
    }

    template <typename Value> typename Value::Type get_concurrently() const
    {
