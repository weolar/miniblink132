...
template <typename Value, typename Traits = HashTraits<Value>, typename Allocator = PartitionAllocator> class HashCountedSet {
    USE_ALLOCATOR(HashCountedSet, Allocator);

private:
    typedef HashMap<Value, unsigned, Traits, HashTraits<unsigned>, Allocator> ImplType;
...
public:
    ...
    void swap(HashCountedSet& other)
    {
    ...
    void RemoveAll(const ValueType& value)
    {
...
inline typename HashCountedSet<T, U, V>::AddResult HashCountedSet<T, U, V>::insert(const ValueType& value, unsigned count)
{
    DCHECK_GT(count, 0u);
...
template <typename T, typename U, typename V> inline typename HashCountedSet<T, U, V>::AddResult HashCountedSet<T, U, V>::insert(const ValueType& value)
{
    return insert(value, 1u);
}

template <typename T, typename U, typename V> inline bool HashCountedSet<T, U, V>::erase(iterator it)
{
    if (it == end())
        return false;
...
template <typename T, typename U, typename V> inline void HashCountedSet<T, U, V>::RemoveAll(iterator it)
{
    if (it == end())
        return;
...
template <typename Value, typename Traits, typename Allocator, typename VectorType>
inline void CopyToVector(const HashCountedSet<Value, Traits, Allocator>& collection, VectorType& vector)
{
    {
...
template <typename T, typename U, typename V> inline Vector<T> HashCountedSet<T, U, V>::AsVector() const
{
    Vector<T> vector;
