...
static_assert(!WTF::IsTraceable<LinkedHashSet<int>>::value, "LinkedHashSet must not be traceable.");
static_assert(!WTF::IsTraceable<LinkedHashSet<String>>::value, "LinkedHashSet must not be traceable.");

template <typename T> int* const ValueInstanceCount<T>::kDeletedValue = reinterpret_cast<int*>(static_cast<uintptr_t>(-1));
...
struct EmptyString {
    EmptyString() = default;
    explicit EmptyString(WTF::HashTableDeletedValueType)
        : deleted_(true)
    {
    }
    ~EmptyString()
    ...
    {
        return str_ == other.str_ && deleted_ == other.deleted_ && empty_ == other.empty_;
    }
...
template <> struct HashTraits<EmptyString> : SimpleClassHashTraits<EmptyString> {
    static unsigned GetHash(const EmptyString&)
    {
        return 0;
    }
...
TEST(LinkedHashSetRefPtrTest, ExerciseValuePeekInType)
{
    ...
    scoped_refptr<DummyRefCounted> ptr = base::AdoptRef(new DummyRefCounted(is_deleted));
...
struct Complicated {
    ...
    explicit Complicated(int value)
        : simple_(value)
    {
    }
    Simple simple_;
...
struct ComplicatedHashTraits : GenericHashTraits<Complicated> {
    ...
    static Complicated EmptyValue()
    {
        return static_cast<Complicated>(0);
    }
    static Complicated DeletedValue()
    {
        return static_cast<Complicated>(-1);
    }
};

struct ComplexityTranslator {
    static unsigned GetHash(const Simple& key)
    {
        return key.value_;
    }
...
TEST(LinkedHashSetTranslatorTest, ComplexityTranslator)
{
    ...
    typename Set::const_iterator const_iterator = const_set.template Find<ComplexityTranslator>(Simple(42));
    EXPECT_NE(const_iterator, const_set.end());
