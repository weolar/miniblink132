...
template <typename VectorBackedLinkedListType> class VectorBackedLinkedListIterator;
template <typename VectorBackedLinkedListType> class VectorBackedLinkedListConstIterator;
template <typename VectorBackedLinkedListType> class VectorBackedLinkedListReverseIterator;
template <typename VectorBackedLinkedListType> class VectorBackedLinkedListConstReverseIterator;

template <typename ValueType, typename Allocator> class VectorBackedLinkedListNode {
...
public:
    ...
    VectorBackedLinkedListNode(wtf_size_t prev_index, wtf_size_t next_index, const ValueType& value)
        : prev_index_(prev_index)
        , next_index_(next_index)
        , value_(value)
    {
    ...
    VectorBackedLinkedListNode(wtf_size_t prev_index, wtf_size_t next_index, ValueType&& value)
        : prev_index_(prev_index)
        , next_index_(next_index)
    ...
    VectorBackedLinkedListNode& operator=(const VectorBackedLinkedListNode& other) = default;
    VectorBackedLinkedListNode& operator=(VectorBackedLinkedListNode&& other) = default;
...
struct VectorTraits<VectorBackedLinkedListNode<ValueType, Allocator>> : VectorTraitsBase<VectorBackedLinkedListNode<ValueType, Allocator>> {
    STATIC_ONLY(VectorTraits);

    static const bool kNeedsDestruction = VectorTraits<ValueType>::kNeedsDestruction;
    ...
    static const bool kCanClearUnusedSlotsWithMemset = VectorTraits<ValueType>::kCanClearUnusedSlotsWithMemset;
    static const bool kCanCopyWithMemcpy = VectorTraits<ValueType>::kCanCopyWithMemcpy;
    static const bool kCanMoveWithMemcpy = VectorTraits<ValueType>::kCanMoveWithMemcpy;

    static constexpr bool kCanTraceConcurrently = VectorTraits<ValueType>::kCanTraceConcurrently;
};

template <typename ValueType, typename Traits, typename Allocator> class ConstructTraits<VectorBackedLinkedListNode<ValueType, Allocator>, Traits, Allocator> {
    STATIC_ONLY(ConstructTraits);

    using Node = VectorBackedLinkedListNode<ValueType, Allocator>;

public:
    template <typename... Args> static Node* Construct(void* location, Args&&... args)
    {
        return new (NotNullTag::kNotNull, location) Node(std::forward<Args>(args)...);
    }
    ...
    template <typename... Args> static Node* ConstructAndNotifyElement(void* location, Args&&... args)
    {
        Node* object = ConstructAndNotifyElementImpl::Construct(location, std::forward<Args>(args)...);
...
private:
    struct ConstructAndNotifyElementImplNotGarbageCollected {
        template <typename... Args> static Node* Construct(void* location, Args&&... args)
        {
            return ConstructTraits<Node, Traits, Allocator>::Construct(location, std::forward<Args>(args)...);
        }
    };
    ...
    using ConstructAndNotifyElementImpl = typename std::conditional<Allocator::kIsGarbageCollected, ConstructAndNotifyElementImplGarbageCollected,
...
template <typename ValueType, typename Allocator = PartitionAllocator> class VectorBackedLinkedList {
...
public:
    ...
    using const_iterator = VectorBackedLinkedListConstIterator<VectorBackedLinkedList>;
    friend class VectorBackedLinkedListConstIterator<VectorBackedLinkedList>;
    using reverse_iterator = VectorBackedLinkedListReverseIterator<VectorBackedLinkedList>;
    using const_reverse_iterator = VectorBackedLinkedListConstReverseIterator<VectorBackedLinkedList>;
    ...
    iterator begin()
    {
        return MakeIterator(UsedFirstIndex());
    }
    const_iterator begin() const
    {
        return MakeConstIterator(UsedFirstIndex());
    }
    ...
    iterator end()
    {
        return MakeIterator(anchor_index_);
    }
    const_iterator end() const
    {
        return MakeConstIterator(anchor_index_);
    }
    ...
    {
        return MakeReverseIterator(UsedLastIndex());
    }
    ...
    {
        return MakeReverseIterator(anchor_index_);
    }
    ...
    template <typename IncomingValueType> iterator insert(const_iterator position, IncomingValueType&& value);

    template <typename IncomingValueType> void push_front(IncomingValueType&& value)
    {
        insert(cbegin(), std::forward<IncomingValueType>(value));
    }

    template <typename IncomingValueType> void push_back(IncomingValueType&& value)
    {
        ...
        requires Allocator::kIsGarbageCollected
    {
        ...
        if (WTF::IsWeak<ValueType>::value) {
            visitor->template RegisterWeakCallbackMethod<VectorBackedLinkedList, &VectorBackedLinkedList::ProcessCustomWeakness>(this);
        }
    }
...
private:
    ...
    {
        return free_head_index_ == anchor_index_;
    }
    ...
    {
        return nodes_[anchor_index_].prev_index_;
    }
    ...
    bool IsAnchor(wtf_size_t index) const
    {
        return index == anchor_index_;
    }
    ...
    template <typename T, typename U, typename V> friend class LinkedHashSet;
...
template <typename VectorBackedLinkedListType> class VectorBackedLinkedListIterator {
    ...
    using const_iterator = VectorBackedLinkedListConstIterator<VectorBackedLinkedListType>;
...
public:
    ...
    VectorBackedLinkedListIterator(const VectorBackedLinkedListIterator&) = default;
    VectorBackedLinkedListIterator& operator=(const VectorBackedLinkedListIterator&) = default;

    VectorBackedLinkedListIterator(VectorBackedLinkedListIterator&&) = default;
    VectorBackedLinkedListIterator& operator=(VectorBackedLinkedListIterator&&) = default;
...
private:
    template <typename T, typename U, typename V> friend class LinkedHashSet;
    template <typename T, typename Allocator> friend class VectorBackedLinkedList;

    VectorBackedLinkedListIterator(const wtf_size_t index, VectorBackedLinkedListType* container)
    ...
    pointer Get() const
    {
        return const_cast<pointer>(iterator_.Get());
    }
    wtf_size_t GetIndex() const
    {
        return iterator_.GetIndex();
    }
...
public:
    ...
    VectorBackedLinkedListConstIterator(const VectorBackedLinkedListConstIterator&) = default;
    VectorBackedLinkedListConstIterator& operator=(const VectorBackedLinkedListConstIterator&) = default;

    VectorBackedLinkedListConstIterator(VectorBackedLinkedListConstIterator&&) = default;
    VectorBackedLinkedListConstIterator& operator=(VectorBackedLinkedListConstIterator&&) = default;
...
protected:
    VectorBackedLinkedListConstIterator(const wtf_size_t index, const VectorBackedLinkedListType* container)
        : index_(index)
...
template <typename VectorBackedLinkedListType> class VectorBackedLinkedListReverseIterator {
    ...
    using const_reverse_iterator = VectorBackedLinkedListConstReverseIterator<VectorBackedLinkedListType>;
...
public:
    ...
    VectorBackedLinkedListReverseIterator(const VectorBackedLinkedListReverseIterator&) = default;
    VectorBackedLinkedListReverseIterator& operator=(const VectorBackedLinkedListReverseIterator&) = default;

    VectorBackedLinkedListReverseIterator(VectorBackedLinkedListReverseIterator&&) = default;
    VectorBackedLinkedListReverseIterator& operator=(VectorBackedLinkedListReverseIterator&&) = default;
...
private:
    ...
    VectorBackedLinkedListReverseIterator(const wtf_size_t index, VectorBackedLinkedListType* container)
        : iterator_(index, container)
    {
    }
    ...
    pointer Get() const
    {
        return const_cast<pointer>(&*iterator_);
    }
    wtf_size_t GetIndex() const
    {
        return iterator_.GetIndex();
...
class VectorBackedLinkedListConstReverseIterator : public VectorBackedLinkedListConstIterator<VectorBackedLinkedListType> {
    DISALLOW_NEW();

    using Superclass = VectorBackedLinkedListConstIterator<VectorBackedLinkedListType>;
...
public:
    ...
    VectorBackedLinkedListConstReverseIterator(const VectorBackedLinkedListConstReverseIterator&) = default;
    VectorBackedLinkedListConstReverseIterator& operator=(const VectorBackedLinkedListConstReverseIterator&) = default;

    VectorBackedLinkedListConstReverseIterator(VectorBackedLinkedListConstReverseIterator&&) = default;
    VectorBackedLinkedListConstReverseIterator& operator=(VectorBackedLinkedListConstReverseIterator&&) = default;
...
private:
    template <typename T, typename U, typename V> friend class LinkedHashSet;
    template <typename T, typename Allocator> friend class VectorBackedLinkedList;
    friend class VectorBackedLinkedListReverseIterator<VectorBackedLinkedListType>;

    VectorBackedLinkedListConstReverseIterator(const wtf_size_t index, const VectorBackedLinkedListType* container)
        : Superclass(index, container)
    {
    }
};

template <typename T, typename Allocator> VectorBackedLinkedList<T, Allocator>::VectorBackedLinkedList()
{
...
template <typename T, typename Allocator> inline void VectorBackedLinkedList<T, Allocator>::swap(VectorBackedLinkedList& other)
{
...
template <typename T, typename Allocator> T& VectorBackedLinkedList<T, Allocator>::front()
{
    DCHECK(!empty());
    return nodes_[UsedFirstIndex()].value_;
}

template <typename T, typename Allocator> const T& VectorBackedLinkedList<T, Allocator>::front() const
{
    DCHECK(!empty());
    return nodes_[UsedFirstIndex()].value_;
}

template <typename T, typename Allocator> T& VectorBackedLinkedList<T, Allocator>::back()
{
    DCHECK(!empty());
    return nodes_[UsedLastIndex()].value_;
}

template <typename T, typename Allocator> const T& VectorBackedLinkedList<T, Allocator>::back() const
{
    DCHECK(!empty());
    return nodes_[UsedLastIndex()].value_;
}
...
typename VectorBackedLinkedList<T, Allocator>::iterator VectorBackedLinkedList<T, Allocator>::insert(const_iterator position, IncomingValueType&& value)
{
    ...
    if (IsFreeListEmpty()) {
        ...
        nodes_.push_back(Node(prev_index, position_index, std::forward<IncomingValueType>(value)));
    } else {
        new_entry_index = free_head_index_;
        Node& free_head = nodes_[free_head_index_];
        free_head_index_ = free_head.next_index_;
        free_head = Node(prev_index, position_index, std::forward<IncomingValueType>(value));
    }
...
template <typename T, typename Allocator>
typename VectorBackedLinkedList<T, Allocator>::iterator VectorBackedLinkedList<T, Allocator>::MoveTo(const_iterator target, const_iterator new_position)
{
    DCHECK(target != end());
...
template <typename T, typename Allocator>
typename VectorBackedLinkedList<T, Allocator>::iterator VectorBackedLinkedList<T, Allocator>::erase(const_iterator position)
{
    DCHECK(position != end());
    wtf_size_t position_index = position.GetIndex();
...
template <typename T, typename Allocator> void VectorBackedLinkedList<T, Allocator>::Unlink(const Node& node)
{
