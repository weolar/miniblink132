...
inline bool LocaleIdMatchesLang(const AtomicString& locale_id, const StringView& lang)
{
    ...
    return maybe_delimiter == '-' || maybe_delimiter == '_' || maybe_delimiter == '@';
}

enum class CaseMapType { kLower, kUpper };

scoped_refptr<StringImpl> CaseConvert(CaseMapType type, StringImpl* source, const char* locale, TextOffsetMap* offset_map = nullptr)
{
    DCHECK(source);
    CHECK_LE(source->length(), static_cast<wtf_size_t>(std::numeric_limits<int32_t>::max()));
    ...
    scoped_refptr<StringImpl> output = StringImpl::CreateUninitialized(target_length, data16);
    while (true) {
        UErrorCode status = U_ZERO_ERROR;
        icu::Edits edits;
        switch (type) {
        case CaseMapType::kLower:
            target_length = icu::CaseMap::toLower(locale, /* options */ 0, reinterpret_cast<const char16_t*>(source16), source_length,
                reinterpret_cast<char16_t*>(data16), target_length, &edits, status);
            break;
        case CaseMapType::kUpper:
            target_length = icu::CaseMap::toUpper(locale, /* options */ 0, reinterpret_cast<const char16_t*>(source16), source_length,
                reinterpret_cast<char16_t*>(data16), target_length, &edits, status);
            break;
        }
        if (U_SUCCESS(status)) {
...
CaseMap::Locale::Locale(const AtomicString& locale)
{
    ...
    if (LocaleIdMatchesLang(locale, "tr") || LocaleIdMatchesLang(locale, "az")) [[unlikely]] {
...
scoped_refptr<StringImpl> CaseMap::TryFastToLowerInvariant(StringImpl* source)
{
    ...
    if (source->Is8Bit()) {
        ...
        LChar* data8;
        scoped_refptr<StringImpl> new_impl = StringImpl::CreateUninitialized(source->length(), data8);
    ...
    if (!(ored & ~0x7F)) {
        UChar* data16;
        scoped_refptr<StringImpl> new_impl = StringImpl::CreateUninitialized(source->length(), data16);
...
scoped_refptr<StringImpl> CaseMap::ToLowerInvariant(StringImpl* source, TextOffsetMap* offset_map)
{
    DCHECK(source);
...
scoped_refptr<StringImpl> CaseMap::ToUpperInvariant(StringImpl* source, TextOffsetMap* offset_map)
{
    DCHECK(source);
    DCHECK(!offset_map || offset_map->IsEmpty());
    ...
    if (source->Is8Bit()) {
        ...
        // special characters.
        new_impl = StringImpl::CreateUninitialized(source->length() + number_sharp_s_characters, data8);
...
scoped_refptr<StringImpl> CaseMap::ToLower(StringImpl* source, TextOffsetMap* offset_map) const
{
    DCHECK(source);
    DCHECK(!offset_map || offset_map->IsEmpty());
...
scoped_refptr<StringImpl> CaseMap::ToUpper(StringImpl* source, TextOffsetMap* offset_map) const
{
    DCHECK(source);
    DCHECK(!offset_map || offset_map->IsEmpty());
