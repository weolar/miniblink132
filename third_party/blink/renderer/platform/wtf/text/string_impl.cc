...
void StringImpl::DestroyIfNeeded() const
{
    if (hash_and_flags_.load(std::memory_order_acquire) & kIsAtomic) {
        ...
        if (AtomicStringTable::Instance().ReleaseAndRemoveIfNeeded(const_cast<StringImpl*>(this))) {
            delete this;
        } else {
...
{
    ASCIIStringAttributes ascii_attributes = Is8Bit() ? CharacterAttributes(Characters8(), length()) : CharacterAttributes(Characters16(), length());
...
scoped_refptr<StringImpl> StringImpl::CreateUninitialized(size_t length, base::span<LChar>& data)
{
    if (!length) {
        data = base::span<LChar>();
        return empty_;
    }
    ...
    StringImpl* string = static_cast<StringImpl*>(Partitions::BufferMalloc(AllocationSize<LChar>(narrowed_length), "WTF::StringImpl"));
    ...
    data = UNSAFE_BUFFERS(base::span(reinterpret_cast<LChar*>(string + 1), narrowed_length));
    return base::AdoptRef(new (string) StringImpl(narrowed_length, kForce8BitConstructor));
}

scoped_refptr<StringImpl> StringImpl::CreateUninitialized(wtf_size_t length, LChar*& data)
{
...
scoped_refptr<StringImpl> StringImpl::CreateUninitialized(size_t length, base::span<UChar>& data)
{
    if (!length) {
        data = base::span<UChar>();
        return empty_;
    }
    const wtf_size_t narrowed_length = base::checked_cast<wtf_size_t>(length);
    ...
    // struct as well as the data which it contains. This removes one
    // heap allocation from this call.
    StringImpl* string = static_cast<StringImpl*>(Partitions::BufferMalloc(AllocationSize<UChar>(narrowed_length), "WTF::StringImpl"));
    ...
    // required for the StringImpl.
    data = UNSAFE_BUFFERS(base::span(reinterpret_cast<UChar*>(string + 1), narrowed_length));
    return base::AdoptRef(new (string) StringImpl(narrowed_length));
}

scoped_refptr<StringImpl> StringImpl::CreateUninitialized(wtf_size_t length, UChar*& data)
{
...
StringImpl* StringImpl::CreateStatic(const char* string, wtf_size_t length)
{
    ...
    // struct as well as the data which it contains. This removes one
    // heap allocation from this call.
    CHECK_LE(length, ((std::numeric_limits<wtf_size_t>::max() - sizeof(StringImpl)) / sizeof(LChar)));
    ...
    DCHECK(IsMainThread());
    highest_static_string_length_ = std::max(highest_static_string_length_, length);
...
scoped_refptr<StringImpl> StringImpl::Create(base::span<const UChar> utf16_data)
{
    if (utf16_data.empty()) {
        return empty_;
    }
    base::span<UChar> string_data;
    scoped_refptr<StringImpl> string = CreateUninitialized(utf16_data.size(), string_data);
    string_data.copy_from(utf16_data);
    return string;
}

scoped_refptr<StringImpl> StringImpl::Create(base::span<const LChar> latin1_data)
{
    if (latin1_data.empty()) {
        return empty_;
    }
    base::span<LChar> string_data;
    scoped_refptr<StringImpl> string = CreateUninitialized(latin1_data.size(), string_data);
    string_data.copy_from(latin1_data);
    return string;
}

scoped_refptr<StringImpl> StringImpl::Create(base::span<const LChar> characters, ASCIIStringAttributes ascii_attributes)
{
...
scoped_refptr<StringImpl> StringImpl::Create8BitIfPossible(base::span<const UChar> characters)
{
    ...
    scoped_refptr<StringImpl> string = CreateUninitialized(characters.size(), data);
...
scoped_refptr<StringImpl> StringImpl::Substring(wtf_size_t start, wtf_size_t length) const
{
    if (start >= length_)
        return empty_;
...
UChar32 StringImpl::CharacterStartingAt(wtf_size_t i)
{
    ...
    if (i + 1 < length_ && U16_IS_LEAD(Characters16()[i]) && U16_IS_TRAIL(Characters16()[i + 1]))
        return U16_GET_SUPPLEMENTARY(Characters16()[i], Characters16()[i + 1]);
    return 0;
}

wtf_size_t StringImpl::CopyTo(UChar* buffer, wtf_size_t start, wtf_size_t max_length) const
{
    wtf_size_t number_of_characters_to_copy = std::min(length() - start, max_length);
...
public:
    ...
    template <typename CharType> scoped_refptr<StringImpl> Alloc(wtf_size_t length, CharType*& buffer)
    {
...
scoped_refptr<StringImpl> StringImpl::FoldCase()
{
    ...
    bool error;
    int32_t real_length = unicode::FoldCase(data, length, Characters16(), length_, &error);
...
template <class UCharPredicate> inline unsigned StringImpl::LengthWithStrippedMatchedCharacters(UCharPredicate predicate) const
{
    if (!length_) {
        return 0;
    }
...
template <class UCharPredicate> inline scoped_refptr<StringImpl> StringImpl::StripMatchedCharacters(UCharPredicate predicate)
{
    if (!length_)
        return empty_;
...
scoped_refptr<StringImpl> StringImpl::StripWhiteSpace(IsWhiteSpaceFunctionPtr is_white_space)
{
    return StripMatchedCharacters(UCharPredicate(is_white_space));
}

template <typename CharType>
ALWAYS_INLINE scoped_refptr<StringImpl> StringImpl::RemoveCharacters(const CharType* characters, CharacterMatchFunctionPtr find_match)
{
...
scoped_refptr<StringImpl> StringImpl::RemoveCharacters(CharacterMatchFunctionPtr find_match)
{
    if (Is8Bit())
...
scoped_refptr<StringImpl> StringImpl::Remove(wtf_size_t start, wtf_size_t length_to_remove)
{
    ...
    if (Is8Bit()) {
        ...
        CopyChars(buffer.Characters() + start, Characters8() + removed_end, length_ - removed_end);
        return buffer.Release();
    }
    StringBuffer<UChar> buffer(length_ - length_to_remove);
    CopyChars(buffer.Characters(), Characters16(), start);
    CopyChars(buffer.Characters() + start, Characters16() + removed_end, length_ - removed_end);
    return buffer.Release();
}

template <typename CharType, class UCharPredicate>
inline scoped_refptr<StringImpl> StringImpl::SimplifyMatchedCharactersToSpace(UCharPredicate predicate, StripBehavior strip_behavior)
{
    StringBuffer<CharType> data(length_);
...
scoped_refptr<StringImpl> StringImpl::SimplifyWhiteSpace(StripBehavior strip_behavior)
{
    if (Is8Bit())
        return StringImpl::SimplifyMatchedCharactersToSpace<LChar>(SpaceOrNewlinePredicate(), strip_behavior);
    return StringImpl::SimplifyMatchedCharactersToSpace<UChar>(SpaceOrNewlinePredicate(), strip_behavior);
}

scoped_refptr<StringImpl> StringImpl::SimplifyWhiteSpace(IsWhiteSpaceFunctionPtr is_white_space, StripBehavior strip_behavior)
{
    if (Is8Bit())
        return StringImpl::SimplifyMatchedCharactersToSpace<LChar>(UCharPredicate(is_white_space), strip_behavior);
    return StringImpl::SimplifyMatchedCharactersToSpace<UChar>(UCharPredicate(is_white_space), strip_behavior);
}
...
bool DeprecatedEqualIgnoringCase(const LChar* a, const LChar* b, wtf_size_t length)
{
    DCHECK_GE(length, 0u);
    if (a == b)
        return true;
    while (length--) {
        if (StringImpl::kLatin1CaseFoldTable[*a++] != StringImpl::kLatin1CaseFoldTable[*b++])
            return false;
    }
    return true;
}

bool DeprecatedEqualIgnoringCase(const UChar* a, const UChar* b, wtf_size_t length)
{
    DCHECK_GE(length, 0u);
    if (a == b)
        return true;
...
bool DeprecatedEqualIgnoringCase(const UChar* a, const LChar* b, wtf_size_t length)
{
    while (length--) {
...
wtf_size_t StringImpl::Find(CharacterMatchFunctionPtr match_function, wtf_size_t start)
{
    if (Is8Bit())
...
wtf_size_t StringImpl::Find(base::RepeatingCallback<bool(UChar)> match_callback, wtf_size_t index) const
{
    if (Is8Bit()) {
...
ALWAYS_INLINE static wtf_size_t FindInternal(const SearchCharacterType* search_characters, const MatchCharacterType* match_characters, wtf_size_t index,
    wtf_size_t search_length, wtf_size_t match_length)
{
    ...
    while (search_hash != match_hash || !Equal(search_characters + i, match_characters, match_length)) {
        if (i == delta)
            return kNotFound;
...
wtf_size_t StringImpl::Find(const StringView& match_string, wtf_size_t index)
{
    ...
    if (Is8Bit()) {
        if (match_string.Is8Bit())
            return FindInternal(Characters8() + index, match_string.Characters8(), index, search_length, match_length);
        return FindInternal(Characters8() + index, match_string.Characters16(), index, search_length, match_length);
    }
    if (match_string.Is8Bit())
        return FindInternal(Characters16() + index, match_string.Characters8(), index, search_length, match_length);
    return FindInternal(Characters16() + index, match_string.Characters16(), index, search_length, match_length);
}

template <typename SearchCharacterType, typename MatchCharacterType>
ALWAYS_INLINE static wtf_size_t FindIgnoringCaseInternal(const SearchCharacterType* search_characters, const MatchCharacterType* match_characters,
    wtf_size_t index, wtf_size_t search_length, wtf_size_t match_length)
{
    // delta is the number of additional times to test; delta == 0 means test only
    // once.
    wtf_size_t delta = search_length - match_length;
    ...
    wtf_size_t i = 0;
    // keep looping until we match
    while (!DeprecatedEqualIgnoringCase(search_characters + i, match_characters, match_length)) {
        if (i == delta)
            return kNotFound;
        ++i;
    }
...
wtf_size_t StringImpl::FindIgnoringCase(const StringView& match_string, wtf_size_t index)
{
    if (match_string.IsNull()) [[unlikely]] {
        return kNotFound;
    }
    ...
    if (Is8Bit()) {
        if (match_string.Is8Bit())
            return FindIgnoringCaseInternal(Characters8() + index, match_string.Characters8(), index, search_length, match_length);
        return FindIgnoringCaseInternal(Characters8() + index, match_string.Characters16(), index, search_length, match_length);
    }
    if (match_string.Is8Bit())
        return FindIgnoringCaseInternal(Characters16() + index, match_string.Characters8(), index, search_length, match_length);
    return FindIgnoringCaseInternal(Characters16() + index, match_string.Characters16(), index, search_length, match_length);
}

template <typename SearchCharacterType, typename MatchCharacterType>
ALWAYS_INLINE static wtf_size_t FindIgnoringASCIICaseInternal(const SearchCharacterType* search_characters, const MatchCharacterType* match_characters,
    wtf_size_t index, wtf_size_t search_length, wtf_size_t match_length)
{
    // delta is the number of additional times to test; delta == 0 means test only
    // once.
    ...
    wtf_size_t i = 0;
    // keep looping until we match
    while (!EqualIgnoringASCIICase(search_characters + i, match_characters, match_length)) {
        if (i == delta)
            return kNotFound;
        ++i;
    }
...
wtf_size_t StringImpl::FindIgnoringASCIICase(const StringView& match_string, wtf_size_t index)
{
    if (match_string.IsNull()) [[unlikely]] {
        return kNotFound;
    }
    ...
    if (Is8Bit()) {
        if (match_string.Is8Bit())
            return FindIgnoringASCIICaseInternal(Characters8() + index, match_string.Characters8(), index, search_length, match_length);
        return FindIgnoringASCIICaseInternal(Characters8() + index, match_string.Characters16(), index, search_length, match_length);
    }
    if (match_string.Is8Bit())
        return FindIgnoringASCIICaseInternal(Characters16() + index, match_string.Characters8(), index, search_length, match_length);
    return FindIgnoringASCIICaseInternal(Characters16() + index, match_string.Characters16(), index, search_length, match_length);
}
...
ALWAYS_INLINE static wtf_size_t ReverseFindInternal(
    const SearchCharacterType* search_characters, const MatchCharacterType* match_characters, wtf_size_t index, wtf_size_t length, wtf_size_t match_length)
{
    // Optimization: keep a running hash of the strings,
    // only call equal if the hashes match.
    ...
    // keep looping until we match
    while (search_hash != match_hash || !Equal(search_characters + delta, match_characters, match_length)) {
        if (!delta)
            return kNotFound;
        --delta;
...
wtf_size_t StringImpl::ReverseFind(const StringView& match_string, wtf_size_t index)
{
    if (match_string.IsNull()) [[unlikely]] {
        return kNotFound;
    }
    ...
    if (match_length == 1) {
        if (Is8Bit())
            return WTF::ReverseFind(Characters8(), our_length, match_string[0], index);
    ...
    if (Is8Bit()) {
        if (match_string.Is8Bit())
            return ReverseFindInternal(Characters8(), match_string.Characters8(), index, our_length, match_length);
        return ReverseFindInternal(Characters8(), match_string.Characters16(), index, our_length, match_length);
    }
    if (match_string.Is8Bit())
        return ReverseFindInternal(Characters16(), match_string.Characters8(), index, our_length, match_length);
    return ReverseFindInternal(Characters16(), match_string.Characters16(), index, our_length, match_length);
}
...
bool StringImpl::StartsWithIgnoringCase(const StringView& prefix) const
{
    ...
    if (Is8Bit()) {
        if (prefix.Is8Bit()) {
            return DeprecatedEqualIgnoringCase(Characters8(), prefix.Characters8(), prefix.length());
        }
        return DeprecatedEqualIgnoringCase(Characters8(), prefix.Characters16(), prefix.length());
    }
    if (prefix.Is8Bit()) {
        return DeprecatedEqualIgnoringCase(Characters16(), prefix.Characters8(), prefix.length());
    }
    return DeprecatedEqualIgnoringCase(Characters16(), prefix.Characters16(), prefix.length());
}

bool StringImpl::StartsWithIgnoringCaseAndAccents(const StringView& prefix) const
{
    std::u16string s = ToU16String();
    std::u16string p = ::WTF::ToU16String(prefix);
    size_t match_index = 1U;

    if (base::i18n::StringSearchIgnoringCaseAndAccents(p, s, &match_index,
...
bool StringImpl::StartsWithIgnoringASCIICase(const StringView& prefix) const
{
    ...
    if (Is8Bit()) {
        if (prefix.Is8Bit())
            return EqualIgnoringASCIICase(Characters8(), prefix.Characters8(), prefix.length());
        return EqualIgnoringASCIICase(Characters8(), prefix.Characters16(), prefix.length());
    }
    if (prefix.Is8Bit())
        return EqualIgnoringASCIICase(Characters16(), prefix.Characters8(), prefix.length());
    return EqualIgnoringASCIICase(Characters16(), prefix.Characters16(), prefix.length());
}
...
bool StringImpl::EndsWith(const StringView& suffix) const
{
    ...
    if (Is8Bit()) {
        if (suffix.Is8Bit())
            return Equal(Characters8() + start_offset, suffix.Characters8(), suffix.length());
        return Equal(Characters8() + start_offset, suffix.Characters16(), suffix.length());
    }
    if (suffix.Is8Bit())
        return Equal(Characters16() + start_offset, suffix.Characters8(), suffix.length());
    return Equal(Characters16() + start_offset, suffix.Characters16(), suffix.length());
}
...
bool StringImpl::EndsWithIgnoringCase(const StringView& suffix) const
{
    ...
    if (Is8Bit()) {
        if (suffix.Is8Bit()) {
            return DeprecatedEqualIgnoringCase(Characters8() + start_offset, suffix.Characters8(), suffix.length());
        }
        return DeprecatedEqualIgnoringCase(Characters8() + start_offset, suffix.Characters16(), suffix.length());
    }
    if (suffix.Is8Bit()) {
        return DeprecatedEqualIgnoringCase(Characters16() + start_offset, suffix.Characters8(), suffix.length());
    }
    return DeprecatedEqualIgnoringCase(Characters16() + start_offset, suffix.Characters16(), suffix.length());
}
...
bool StringImpl::EndsWithIgnoringASCIICase(const StringView& suffix) const
{
    ...
    if (Is8Bit()) {
        if (suffix.Is8Bit())
            return EqualIgnoringASCIICase(Characters8() + start_offset, suffix.Characters8(), suffix.length());
        return EqualIgnoringASCIICase(Characters8() + start_offset, suffix.Characters16(), suffix.length());
    }
    if (suffix.Is8Bit())
        return EqualIgnoringASCIICase(Characters16() + start_offset, suffix.Characters8(), suffix.length());
    return EqualIgnoringASCIICase(Characters16() + start_offset, suffix.Characters16(), suffix.length());
}
...
// behavior.
scoped_refptr<StringImpl> StringImpl::Replace(wtf_size_t position, wtf_size_t length_to_replace, const StringView& string)
{
    ...
    CHECK_LT((length() - length_to_replace), (numeric_limits<wtf_size_t>::max() - length_to_insert));

    if (Is8Bit() && (string.IsNull() || string.Is8Bit())) {
        LChar* data;
        scoped_refptr<StringImpl> new_impl = CreateUninitialized(length() - length_to_replace + length_to_insert, data);
        memcpy(data, Characters8(), position * sizeof(LChar));
        if (!string.IsNull())
            memcpy(data + position, string.Characters8(), length_to_insert * sizeof(LChar));
        memcpy(data + position + length_to_insert, Characters8() + position + length_to_replace, (length() - position - length_to_replace) * sizeof(LChar));
        return new_impl;
    }
    UChar* data;
    scoped_refptr<StringImpl> new_impl = CreateUninitialized(length() - length_to_replace + length_to_insert, data);
    ...
    if (!string.IsNull()) {
        if (string.Is8Bit())
            ...
            memcpy(data + position, string.Characters16(), length_to_insert * sizeof(UChar));
    }
    if (Is8Bit()) {
        for (wtf_size_t i = 0; i < length() - position - length_to_replace; ++i)
            data[i + position + length_to_insert] = Characters8()[i + position + length_to_replace];
    } else {
        memcpy(data + position + length_to_insert, Characters16() + position + length_to_replace, (length() - position - length_to_replace) * sizeof(UChar));
    }
    return new_impl;
}

scoped_refptr<StringImpl> StringImpl::Replace(UChar pattern, const StringView& replacement)
{
...
scoped_refptr<StringImpl> StringImpl::Replace(UChar pattern, const LChar* replacement, wtf_size_t rep_str_length)
{
    DCHECK(replacement);
    ...
    CHECK(!rep_str_length || match_count <= numeric_limits<wtf_size_t>::max() / rep_str_length);
    ...
    if (Is8Bit()) {
        ...
        while ((src_segment_end = Find(pattern, src_segment_start)) != kNotFound) {
            ...
            memcpy(data + dst_offset, Characters8() + src_segment_start, src_segment_length * sizeof(LChar));
    ...
    while ((src_segment_end = Find(pattern, src_segment_start)) != kNotFound) {
        src_segment_length = src_segment_end - src_segment_start;
        memcpy(data + dst_offset, Characters16() + src_segment_start, src_segment_length * sizeof(UChar));
...
scoped_refptr<StringImpl> StringImpl::Replace(UChar pattern, const UChar* replacement, wtf_size_t rep_str_length)
{
    DCHECK(replacement);
...
scoped_refptr<StringImpl> StringImpl::Replace(const StringView& pattern, const StringView& replacement)
{
    ...
    CHECK_LE(new_size, (numeric_limits<wtf_size_t>::max() - match_count * rep_str_length));
    ...
    if (src_is_8bit && replacement_is_8bit) {
        ...
        while ((src_segment_end = Find(pattern, src_segment_start)) != kNotFound) {
            ...
            memcpy(data + dst_offset, Characters8() + src_segment_start, src_segment_length * sizeof(LChar));
            dst_offset += src_segment_length;
            memcpy(data + dst_offset, replacement.Characters8(), rep_str_length * sizeof(LChar));
            dst_offset += rep_str_length;
    ...
    while ((src_segment_end = Find(pattern, src_segment_start)) != kNotFound) {
        ...
        if (replacement_is_8bit) {
            ...
            // Case 4
            memcpy(data + dst_offset, replacement.Characters16(), rep_str_length * sizeof(UChar));
        }
        dst_offset += rep_str_length;
        src_segment_start = src_segment_end + pattern_length;
    }
...
template <typename CharType> inline bool EqualInternal(const StringImpl* a, const CharType* b, wtf_size_t length)
{
    if (!a)
        return !b;
    if (!b)
...
template <typename StringType> bool EqualInternal(const StringType* a, const LChar* b)
{
    if (!a)
        return !b;
    if (!b)
...
int CodeUnitCompareIgnoringASCIICase(wtf_size_t l1, wtf_size_t l2, const CharacterType1* c1, const CharacterType2* c2)
{
...
template <typename CharacterType> int CodeUnitCompareIgnoringASCIICase(const StringImpl* string1, const CharacterType* string2, wtf_size_t length2)
{
    if (!string1)
    ...
    if (string1->Is8Bit()) {
        return CodeUnitCompareIgnoringASCIICase(length1, length2, string1->Characters8(), string2);
    }
    return CodeUnitCompareIgnoringASCIICase(length1, length2, string1->Characters16(), string2);
}

int CodeUnitCompareIgnoringASCIICase(const StringImpl* string1, const LChar* string2)
{
    return CodeUnitCompareIgnoringASCIICase(string1, string2, string2 ? strlen(reinterpret_cast<const char*>(string2)) : 0);
}

int CodeUnitCompareIgnoringASCIICase(const StringImpl* string1, const StringImpl* string2)
{
    if (!string2)
        return string1 && string1->length() > 0 ? 1 : 0;
    return VisitCharacters(*string2, [string1](auto chars) { return CodeUnitCompareIgnoringASCIICase(string1, chars.data(), chars.size()); });
}
