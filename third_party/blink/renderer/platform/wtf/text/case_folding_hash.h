...
struct CaseFoldingHashReader {
    ...
    static inline uint64_t Read64(const uint8_t* ptr)
    {
        ...
        return FoldCase(p[0]) | (FoldCase(p[1]) << 16) | (FoldCase(p[2]) << 32) | (FoldCase(p[3]) << 48);
    }
...
public:
    ...
    {
        return StringHasher::ComputeHashAndMaskTop8Bits<CaseFoldingHashReader<UChar>>(reinterpret_cast<const char*>(data), length * 2);
    }
    ...
    {
        return StringHasher::ComputeHashAndMaskTop8Bits<CaseFoldingHashReader<LChar>>(reinterpret_cast<const char*>(data), length * 2);
    }
    ...
    {
        return GetHash(reinterpret_cast<const LChar*>(data), static_cast<unsigned>(strlen(data)));
    }
    ...
    static bool Equal(const scoped_refptr<StringImpl>& a, const scoped_refptr<StringImpl>& b)
    {
...
template <typename T> struct CaseFoldingHashTraits : HashTraits<T>, CaseFoldingHash {
