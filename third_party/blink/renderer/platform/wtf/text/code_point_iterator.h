...
public:
    ...
    template <class T> static CodePointIterator End(const T& string)
    {
        return CodePointIterator(string.Is8Bit(),
            string.Is8Bit() ? static_cast<const void*>(string.Characters8() + string.length())
...
inline UChar32 CodePointIterator::operator*() const
{
    ...
    U16_NEXT(static_cast<const uint16_t*>(data_), code_point_length_, data_length_, ch);
    return ch;
}
...
inline void CodePointIterator::operator++()
{
    ...
    if (!code_point_length_) {
        ...
        U16_FWD_1(static_cast<const uint16_t*>(data_), code_point_length_, data_length_);
    }
