...
typedef HashMap<wtf_size_t, StringImpl*, AlreadyHashedTraits> StaticStringsTable;
...
private:
    ...
    explicit StringImpl(ConstructEmptyString16BitTag)
        : length_(0)
        , hash_and_flags_(kAsciiPropertyCheckDone | kContainsOnlyAscii | kIsLowerAscii | kIsStatic)
    {
        GetHash();
    }
    ...
    StringImpl(wtf_size_t length, wtf_size_t hash, StaticStringTag)
        : length_(length)
        , hash_and_flags_(hash << kHashShift | LengthToAsciiFlags(length) | kIs8Bit | kIsStatic)
    {
    }
...
public:
    ...
    static scoped_refptr<StringImpl> Create(base::span<const LChar>, ASCIIStringAttributes ascii_attributes);
    static scoped_refptr<StringImpl> Create8BitIfPossible(base::span<const UChar>);

    ALWAYS_INLINE static scoped_refptr<StringImpl> Create(base::span<const char> s)
    {
    ...
    // caller.
    static scoped_refptr<StringImpl> CreateUninitialized(size_t length, base::span<LChar>& data);
    static scoped_refptr<StringImpl> CreateUninitialized(wtf_size_t length, LChar*& data);
    // Create a StringImpl with space for `length` UChar characters. `data` will
    // be the character data allocated, and _must_be_completely_filled_in_ by the
    // caller.
    static scoped_refptr<StringImpl> CreateUninitialized(size_t length, base::span<UChar>& data);
    static scoped_refptr<StringImpl> CreateUninitialized(wtf_size_t length, UChar*& data);
    ...
    {
        return { reinterpret_cast<const uint8_t*>(this + 1), CharactersSizeInBytes() };
    }

    template <typename CharType> ALWAYS_INLINE const CharType* GetCharacters() const;
    ...
    void SetHash(wtf_size_t hash) const
    {
        ...
        // hash function.
        DCHECK_EQ(hash,
            (Is8Bit() ? StringHasher::ComputeHashAndMaskTop8Bits((const char*)Characters8(), length_) : ComputeHashForWideString(Characters16(), length_)));
    ...
    {
        if (!IsStatic()) {
            uint32_t previous_ref_count = ref_count_.fetch_add(1, std::memory_order_relaxed);
    ...
    ALWAYS_INLINE void Release() const
    {
        if (!IsStatic()) {
            ...
            } while (!ref_count_.compare_exchange_weak(current_ref, current_ref - 1, std::memory_order_acq_rel));
        }
    }
    ...
    void ResetRefCountChangeCountForTesting()
    {
        ref_count_change_count_ = 0;
    }
#endif
    ...
    template <typename T> static void CopyChars(T* destination, const T* source, wtf_size_t num_characters)
    {
        if (num_characters > 0) {
    ...
    ALWAYS_INLINE static void CopyChars(UChar* destination, const LChar* source, wtf_size_t num_characters)
    {
    ...
    scoped_refptr<StringImpl> Substring(wtf_size_t pos, wtf_size_t len = UINT_MAX) const;
    ...
    scoped_refptr<StringImpl> SimplifyWhiteSpace(StripBehavior = kStripExtraWhiteSpace);
    scoped_refptr<StringImpl> SimplifyWhiteSpace(IsWhiteSpaceFunctionPtr, StripBehavior = kStripExtraWhiteSpace);

    scoped_refptr<StringImpl> RemoveCharacters(CharacterMatchFunctionPtr);
    template <typename CharType> ALWAYS_INLINE scoped_refptr<StringImpl> RemoveCharacters(const CharType* characters, CharacterMatchFunctionPtr);
    ...
    scoped_refptr<StringImpl> Remove(wtf_size_t start, wtf_size_t length_to_remove = 1);
    ...
    wtf_size_t Find(base::RepeatingCallback<bool(UChar)> match_callback, wtf_size_t index = 0) const;
    ...
    scoped_refptr<StringImpl> Replace(UChar pattern, const StringView& replacement);
    scoped_refptr<StringImpl> Replace(const StringView& pattern, const StringView& replacement);
    scoped_refptr<StringImpl> Replace(wtf_size_t index, wtf_size_t length_to_replace, const StringView& replacement);
    ...
    // copied.
    wtf_size_t CopyTo(UChar* buffer, wtf_size_t start, wtf_size_t max_length) const;
    ...
    template <typename BufferType> void AppendTo(BufferType&, wtf_size_t start = 0, wtf_size_t length = UINT_MAX) const;
    ...
    template <typename BufferType> void PrependTo(BufferType&, wtf_size_t start = 0, wtf_size_t length = UINT_MAX) const;
...
private:
    ...
    template <typename CharType> static size_t AllocationSize(wtf_size_t length)
    {
        ...
        return base::CheckAdd(sizeof(StringImpl), base::CheckMul(length, sizeof(CharType))).ValueOrDie();
    }

    scoped_refptr<StringImpl> Replace(UChar pattern, const LChar* replacement, wtf_size_t replacement_length);
    scoped_refptr<StringImpl> Replace(UChar pattern, const UChar* replacement, wtf_size_t replacement_length);

    template <class UCharPredicate> unsigned LengthWithStrippedMatchedCharacters(UCharPredicate) const;
    template <class UCharPredicate> scoped_refptr<StringImpl> StripMatchedCharacters(UCharPredicate);
    template <typename CharType, class UCharPredicate> scoped_refptr<StringImpl> SimplifyMatchedCharactersToSpace(UCharPredicate, StripBehavior);
    ...
    {
        DCHECK(HasHash());
        DCHECK_EQ(ExistingHash(), StringHasher::ComputeHashAndMaskTop8Bits((const char*)Characters8(), length()));
    }
#endif
...
template <> ALWAYS_INLINE const LChar* StringImpl::GetCharacters<LChar>() const
{
    return Characters8();
}

template <> ALWAYS_INLINE const UChar* StringImpl::GetCharacters<UChar>() const
{
    return Characters16();
}
...
template <> ALWAYS_INLINE size_t StringImpl::AllocationSize<LChar>(wtf_size_t length)
{
...
template <typename CharType> ALWAYS_INLINE bool Equal(const CharType* a, const CharType* b, wtf_size_t length)
{
...
WTF_EXPORT bool DeprecatedEqualIgnoringCase(const LChar*, const LChar*, wtf_size_t length);
WTF_EXPORT bool DeprecatedEqualIgnoringCase(const UChar*, const LChar*, wtf_size_t length);
inline bool DeprecatedEqualIgnoringCase(const LChar* a, const UChar* b, wtf_size_t length)
{
    return DeprecatedEqualIgnoringCase(b, a, length);
}
WTF_EXPORT bool DeprecatedEqualIgnoringCase(const UChar*, const UChar*, wtf_size_t length);

WTF_EXPORT bool EqualIgnoringNullity(StringImpl*, StringImpl*);

template <typename CharacterTypeA, typename CharacterTypeB>
inline bool EqualIgnoringASCIICase(const CharacterTypeA* a, const CharacterTypeB* b, wtf_size_t length)
{
    for (wtf_size_t i = 0; i < length; ++i) {
...
WTF_EXPORT int CodeUnitCompareIgnoringASCIICase(const StringImpl*, const StringImpl*);
WTF_EXPORT int CodeUnitCompareIgnoringASCIICase(const StringImpl*, const LChar*);

inline wtf_size_t Find(const LChar* characters, wtf_size_t length, LChar match_character, wtf_size_t index = 0)
{
...
inline wtf_size_t Find(const UChar* characters, wtf_size_t length, UChar match_character, wtf_size_t index = 0)
{
    while (index < length) {
...
ALWAYS_INLINE wtf_size_t Find(const UChar* characters, wtf_size_t length, LChar match_character, wtf_size_t index = 0)
{
    return Find(characters, length, static_cast<UChar>(match_character), index);
}

inline wtf_size_t Find(const LChar* characters, wtf_size_t length, UChar match_character, wtf_size_t index = 0)
{
...
template <typename CharacterType> inline wtf_size_t Find(const CharacterType* characters, wtf_size_t length, char match_character, wtf_size_t index = 0)
{
    return Find(characters, length, static_cast<LChar>(match_character), index);
}

inline wtf_size_t Find(const LChar* characters, wtf_size_t length, CharacterMatchFunctionPtr match_function, wtf_size_t index = 0)
{
    while (index < length) {
...
inline wtf_size_t Find(const UChar* characters, wtf_size_t length, CharacterMatchFunctionPtr match_function, wtf_size_t index = 0)
{
    while (index < length) {
        if (match_function(characters[index]))
            return index;
...
inline wtf_size_t ReverseFind(const CharacterType* characters, wtf_size_t length, CharacterType match_character, wtf_size_t index = UINT_MAX)
{
    if (!length)
        return kNotFound;
    if (index >= length)
...
ALWAYS_INLINE wtf_size_t ReverseFind(const UChar* characters, wtf_size_t length, LChar match_character, wtf_size_t index = UINT_MAX)
{
    return ReverseFind(characters, length, static_cast<UChar>(match_character), index);
}

inline wtf_size_t ReverseFind(const LChar* characters, wtf_size_t length, UChar match_character, wtf_size_t index = UINT_MAX)
{
    if (match_character & ~0xFF)
        return kNotFound;
    return ReverseFind(characters, length, static_cast<LChar>(match_character), index);
}
...
template <wtf_size_t inlineCapacity> bool EqualIgnoringNullity(const Vector<UChar, inlineCapacity>& a, StringImpl* b)
{
    if (!b)
        return !a.size();
    if (a.size() != b->length())
...
static inline int CodeUnitCompare(wtf_size_t l1, wtf_size_t l2, const CharacterType1* c1, const CharacterType2* c2)
{
...
static inline int CodeUnitCompare8(const StringImpl* string1, const StringImpl* string2)
{
    return CodeUnitCompare(string1->length(), string2->length(), string1->Characters8(), string2->Characters8());
}

static inline int CodeUnitCompare16(const StringImpl* string1, const StringImpl* string2)
{
    return CodeUnitCompare(string1->length(), string2->length(), string1->Characters16(), string2->Characters16());
}

static inline int CodeUnitCompare8To16(const StringImpl* string1, const StringImpl* string2)
{
    return CodeUnitCompare(string1->length(), string2->length(), string1->Characters8(), string2->Characters16());
}

static inline int CodeUnitCompare(const StringImpl* string1, const StringImpl* string2)
{
    if (!string1)
...
template <typename BufferType> inline void StringImpl::AppendTo(BufferType& result, wtf_size_t start, wtf_size_t length) const
{
...
template <typename BufferType> inline void StringImpl::PrependTo(BufferType& result, wtf_size_t start, wtf_size_t length) const
{
    wtf_size_t number_of_characters_to_copy = std::min(length, length_ - start);
    if (!number_of_characters_to_copy)
        return;
...
template <> struct HashTraits<StringImpl*>;
template <> struct HashTraits<scoped_refptr<StringImpl>>;
