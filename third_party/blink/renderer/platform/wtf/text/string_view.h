...
public:
    ...
    public:
        ...
        // again.
        template <typename CharT> CharT* Realloc(int length)
        {
            size_t size = length * sizeof(CharT);
            if (size > sizeof(stackbuf16_)) [[unlikely]] {
                heapbuf_.reset(reinterpret_cast<char*>(WTF::Partitions::BufferMalloc(size, "StackBackingStore")));
            ...
            // another branch.
            static_assert(alignof(decltype(stackbuf16_)) % alignof(CharT) == 0, "stack buffer must be sufficiently aligned");
    ...
    inline StringView(const String& string LIFETIME_BOUND, unsigned offset, unsigned length);
    ...
    inline StringView(const AtomicString& string LIFETIME_BOUND, unsigned offset, unsigned length);
    ...
    StringView(const LChar* chars)
        : StringView(chars, chars ? base::checked_cast<unsigned>(strlen(reinterpret_cast<const char*>(chars))) : 0)
    {
    }
    ...
    bool IsAtomic() const
    {
        return SharedImpl() && SharedImpl()->IsAtomic();
    }
    ...
    {
        return { reinterpret_cast<const uint8_t*>(bytes_), length_ * (Is8Bit() ? sizeof(LChar) : sizeof(UChar)) };
    }
...
WTF_EXPORT bool DeprecatedEqualIgnoringCase(const StringView&, const StringView&);
WTF_EXPORT bool DeprecatedEqualIgnoringCaseAndNullity(const StringView&, const StringView&);

WTF_EXPORT bool EqualIgnoringASCIICase(const StringView&, const StringView&);

template <size_t N> inline bool EqualIgnoringASCIICase(const StringView& a, const char (&literal)[N])
{
    if (a.length() != N - 1 || (N == 1 && a.IsNull()))
        return false;
    return a.Is8Bit() ? EqualIgnoringASCIICase(a.Characters8(), literal, N - 1) : EqualIgnoringASCIICase(a.Characters16(), literal, N - 1);
}
...
inline wtf_size_t StringView::Find(CharacterMatchFunctionPtr match_function, wtf_size_t start) const
{
    return Is8Bit() ? WTF::Find(Characters8(), length_, match_function, start) : WTF::Find(Characters16(), length_, match_function, start);
}
...
template <bool isSpecialCharacter(UChar)> inline bool StringView::IsAllSpecialCharacters() const
{
    ...
    return Is8Bit() ? WTF::IsAllSpecialCharacters<isSpecialCharacter, LChar>(Characters8(), len)
                    : WTF::IsAllSpecialCharacters<isSpecialCharacter, UChar>(Characters16(), len);
}
