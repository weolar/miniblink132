...
#define EXPECT_INVALID(string, options)                                                                                                                        \
    ...
    constexpr auto kStrict = NumberParsingOptions::Strict();
    constexpr auto kAcceptMinusZeroForUnsigned = NumberParsingOptions().SetAcceptMinusZeroForUnsigned();
...
#define EXPECT_VALID(string, expectedValue)                                                                                                                    \
    do {                                                                                                                                                       \
        bool ok;                                                                                                                                               \
        unsigned value = HexCharactersToUInt(String(string).Span8(), NumberParsingOptions::Strict(), &ok);                                                     \
        EXPECT_TRUE(ok);                                                                                                                                       \
        EXPECT_EQ(value, expectedValue);                                                                                                                       \
    } while (false)
...
#define EXPECT_INVALID(string)                                                                                                                                 \
    do {                                                                                                                                                       \
        bool ok;                                                                                                                                               \
        HexCharactersToUInt(String(string).Span8(), NumberParsingOptions::Strict(), &ok);                                                                      \
        EXPECT_FALSE(ok);                                                                                                                                      \
    } while (false)
...
NumberParsingResult ParseUInt(const String str, unsigned* value)
{
    ...
    *value = CharactersToUInt(str.Span8(), NumberParsingOptions::Strict(), &result);
    return result;
}
...
{
    unsigned value;
    EXPECT_EQ(NumberParsingResult::kOverflowMax, ParseUInt("10000000000", &value));
