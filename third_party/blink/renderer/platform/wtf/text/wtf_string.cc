...
{
    return CodeUnitCompareIgnoringASCIICase(a.Impl(), reinterpret_cast<const LChar*>(b));
}

wtf_size_t String::Find(base::RepeatingCallback<bool(UChar)> match_callback, wtf_size_t index) const
{
...
String String::SimplifyWhiteSpace(IsWhiteSpaceFunctionPtr is_white_space, StripBehavior strip_behavior) const
{
    if (!impl_)
        return String();
...
String String::NumberToStringFixedWidth(double number, unsigned decimal_places)
{
    NumberToStringBuffer buffer;
...
void String::Split(const StringView& separator, bool allow_empty_entries, Vector<String>& result) const
{
    result.clear();
...
void String::Split(UChar separator, bool allow_empty_entries, Vector<String>& result) const
{
    result.clear();
...
String String::FromUTF8(const uint8_t* string_start, size_t string_length)
{
    ...
    if (unicode::ConvertUTF8ToUTF16(&string_current, reinterpret_cast<const char*>(string_start + length), &buffer_current, buffer_current + buffer.size())
...
//     StringUTF8Adaptor adaptor(Is8Bit() ? StringView(Span8()) : StringView(Span16()));
