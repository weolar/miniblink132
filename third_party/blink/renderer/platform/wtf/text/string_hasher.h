...
public:
    ...
    template <class Reader = PlainHashReader> static unsigned ComputeHashAndMaskTop8Bits(const char* data, unsigned length)
    {
    ...
    // HashReader.
    template <class Reader = PlainHashReader> ALWAYS_INLINE static unsigned ComputeHashAndMaskTop8BitsInline(const char* data, unsigned length)
    {
        return MaskTop8Bits(rapidhash<Reader>(reinterpret_cast<const uint8_t*>(data), length));
    }
    ...
    template <size_t Extent> static uint64_t HashMemory(base::span<const uint8_t, Extent> data)
    {
        return rapidhash(data.data(), data.size());
    }
