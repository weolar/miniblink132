...
public:
    explicit StackStringViewAllocator(StringView::StackBackingStore& backing_store)
        : backing_store_(backing_store)
    {
    }
    using ResultStringType = StringView;

    template <typename CharType> StringView Alloc(wtf_size_t length, CharType*& buffer)
    {
...
{
    DCHECK(impl_);
    DCHECK(!impl_->HasOneRef() || impl_->IsStatic()) << "StringView does not own the StringImpl, it "
...
std::string StringView::Utf8(UTF8ConversionMode mode) const
{
    ...
    if (Is8Bit()) {
        ...
        unicode::ConversionResult result = unicode::ConvertLatin1ToUTF8(&characters, characters + length, &buffer, buffer + buffer_vector.size());
        ...
        if (mode == kStrictUTF8ConversionReplacingUnpairedSurrogatesWithFFFD) {
            ...
            while (characters < characters_end) {
                ...
                unicode::ConversionResult result = unicode::ConvertUTF16ToUTF8(&characters, characters_end, &buffer, buffer_end, true);
                DCHECK_NE(result, unicode::kTargetExhausted);
            ...
            unicode::ConversionResult result = unicode::ConvertUTF16ToUTF8(&characters, characters + length, &buffer, buffer + buffer_vector.size(), strict);
            // (length * 3) should be sufficient for any conversion
            DCHECK_NE(result, unicode::kTargetExhausted);
...
bool StringView::ContainsOnlyASCIIOrEmpty() const
{
    ...
    if (empty())
        return true;
    ASCIIStringAttributes attrs = Is8Bit() ? CharacterAttributes(Characters8(), length()) : CharacterAttributes(Characters16(), length());
    return attrs.contains_only_ascii;
}

bool StringView::SubstringContainsOnlyWhitespaceOrEmpty(unsigned from, unsigned to) const
{
...
bool DeprecatedEqualIgnoringCaseAndNullity(const StringView& a, const StringView& b)
{
    if (a.length() != b.length())
        return false;
    if (a.Is8Bit()) {
        if (b.Is8Bit()) {
            return DeprecatedEqualIgnoringCase(a.Characters8(), b.Characters8(), a.length());
        }
        return DeprecatedEqualIgnoringCase(a.Characters8(), b.Characters16(), a.length());
    }
    if (b.Is8Bit()) {
        return DeprecatedEqualIgnoringCase(a.Characters16(), b.Characters8(), a.length());
    }
    return DeprecatedEqualIgnoringCase(a.Characters16(), b.Characters16(), a.length());
}
...
StringView StringView::LowerASCIIMaybeUsingBuffer(StackBackingStore& buffer) const
{
