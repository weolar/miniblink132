...
WTF::StringTypeAdapter<const char*>::StringTypeAdapter(const LChar* buffer, size_t length)
    : buffer_(buffer)
    , length_(base::checked_cast<unsigned>(length))
{
}
...
WTF::StringTypeAdapter<const UChar*>::StringTypeAdapter(const UChar* buffer)
    : buffer_(buffer)
    , length_(LengthOfNullTerminatedString(buffer))
{
}
