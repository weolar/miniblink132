...
template <typename CharType> bool IsAsciiWhitespace(CharType character)
{
...
bool Base64DecodeRaw(const StringView& in, Vector<char>& out, Base64DecodePolicy policy)
{
    ...
    size_t output_size = modp_b64_decode(out.data(), adaptor.data(), adaptor.size(), GetModpPolicy(policy));
...
String Base64Encode(base::span<const uint8_t> data)
{
    ...
    if (encode_len == 0)
        return String();
    const size_t output_size = modp_b64_encode_data(reinterpret_cast<char*>(result.Characters()), reinterpret_cast<const char*>(data.data()), data.size());
...
void Base64Encode(base::span<const uint8_t> data, Vector<char>& out)
{
    ...
    const size_t output_size = modp_b64_encode_data(out.data(), reinterpret_cast<const char*>(data.data()), data.size());
    DCHECK_EQ(output_size, encode_len);
}

bool Base64Decode(const StringView& in, Vector<char>& out, Base64DecodePolicy policy)
{
    switch (policy) {
    case Base64DecodePolicy::kForgiving: {
        ...
        return Base64DecodeRaw(in, out, policy) || Base64DecodeRaw(in.ToString().RemoveCharacters(&IsAsciiWhitespace), out, policy);
    }
    case Base64DecodePolicy::kNoPaddingValidation: {
