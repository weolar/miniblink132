...
template <int base> bool IsCharacterAllowedInBase(UChar);

template <> bool IsCharacterAllowedInBase<10>(UChar c)
{
    return IsASCIIDigit(c);
}

template <> bool IsCharacterAllowedInBase<16>(UChar c)
{
    return IsASCIIHexDigit(c);
}

template <typename IntegralType, typename CharType, int base>
static inline IntegralType ToIntegralType(base::span<const CharType> chars, NumberParsingOptions options, NumberParsingResult* parsing_result)
{
...
template <typename IntegralType, typename CharType, int base>
static inline IntegralType ToIntegralType(base::span<const CharType> data, NumberParsingOptions options, bool* ok)
{
    NumberParsingResult result;
    IntegralType value = ToIntegralType<IntegralType, CharType, base>(data, options, &result);
    if (ok)
        *ok = result == NumberParsingResult::kSuccess;
    return value;
}

unsigned CharactersToUInt(base::span<const LChar> data, NumberParsingOptions options, NumberParsingResult* result)
{
    return ToIntegralType<unsigned, LChar, 10>(data, options, result);
}

unsigned CharactersToUInt(base::span<const UChar> data, NumberParsingOptions options, NumberParsingResult* result)
{
    return ToIntegralType<unsigned, UChar, 10>(data, options, result);
}

unsigned HexCharactersToUInt(base::span<const LChar> data, NumberParsingOptions options, bool* ok)
{
    return ToIntegralType<unsigned, LChar, 16>(data, options, ok);
}

unsigned HexCharactersToUInt(base::span<const UChar> data, NumberParsingOptions options, bool* ok)
{
    return ToIntegralType<unsigned, UChar, 16>(data, options, ok);
}

uint64_t HexCharactersToUInt64(base::span<const LChar> data, NumberParsingOptions options, bool* ok)
{
    return ToIntegralType<uint64_t, LChar, 16>(data, options, ok);
}

uint64_t HexCharactersToUInt64(base::span<const UChar> data, NumberParsingOptions options, bool* ok)
{
    return ToIntegralType<uint64_t, UChar, 16>(data, options, ok);
}

int CharactersToInt(base::span<const LChar> data, NumberParsingOptions options, bool* ok)
{
    return ToIntegralType<int, LChar, 10>(data, options, ok);
}

int CharactersToInt(base::span<const UChar> data, NumberParsingOptions options, bool* ok)
{
    return ToIntegralType<int, UChar, 10>(data, options, ok);
}

int CharactersToInt(const StringView& string, NumberParsingOptions options, bool* ok)
{
    return WTF::VisitCharacters(string, [&](auto chars) { return CharactersToInt(chars, options, ok); });
}

unsigned CharactersToUInt(base::span<const LChar> data, NumberParsingOptions options, bool* ok)
{
    return ToIntegralType<unsigned, LChar, 10>(data, options, ok);
}

unsigned CharactersToUInt(base::span<const UChar> data, NumberParsingOptions options, bool* ok)
{
    return ToIntegralType<unsigned, UChar, 10>(data, options, ok);
}

int64_t CharactersToInt64(base::span<const LChar> data, NumberParsingOptions options, bool* ok)
{
    return ToIntegralType<int64_t, LChar, 10>(data, options, ok);
}

int64_t CharactersToInt64(base::span<const UChar> data, NumberParsingOptions options, bool* ok)
{
    return ToIntegralType<int64_t, UChar, 10>(data, options, ok);
}

uint64_t CharactersToUInt64(base::span<const LChar> data, NumberParsingOptions options, bool* ok)
{
    return ToIntegralType<uint64_t, LChar, 10>(data, options, ok);
}

uint64_t CharactersToUInt64(base::span<const UChar> data, NumberParsingOptions options, bool* ok)
{
...
template <typename CharType, TrailingJunkPolicy policy> static inline double ToDoubleType(base::span<const CharType> data, bool* ok, size_t& parsed_length)
{
    ...
    double number = ParseDouble(data.data() + leading_spaces_length, length - leading_spaces_length, parsed_length);
    if (!parsed_length) {
        if (ok)
            *ok = false;
        return 0.0;
...
float CharactersToFloat(base::span<const LChar> data, bool* ok)
{
    ...
    // not a float.
    size_t parsed_length;
    return static_cast<float>(ToDoubleType<LChar, kDisallowTrailingJunk>(data, ok, parsed_length));
}
...
float CharactersToFloat(base::span<const UChar> data, bool* ok)
{
    ...
    // not a float.
    size_t parsed_length;
    return static_cast<float>(ToDoubleType<UChar, kDisallowTrailingJunk>(data, ok, parsed_length));
}
...
float CharactersToFloat(base::span<const LChar> data, size_t& parsed_length)
{
    // FIXME: This will return ok even when the string fits into a double but
    // not a float.
    return static_cast<float>(ToDoubleType<LChar, kAllowTrailingJunk>(data, nullptr, parsed_length));
}
...
float CharactersToFloat(base::span<const UChar> data, size_t& parsed_length)
{
    // FIXME: This will return ok even when the string fits into a double but
    // not a float.
    return static_cast<float>(ToDoubleType<UChar, kAllowTrailingJunk>(data, nullptr, parsed_length));
}
