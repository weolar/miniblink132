...
#define DISPATCH_CASE_OP(caseSensitivity, op, args)                                                                                                            \
    ((caseSensitivity == kTextCaseSensitive) ? op args : (caseSensitivity == kTextCaseASCIIInsensitive) ? op##IgnoringASCIICase args : op##IgnoringCase args)
...
public:
    ...
    template <wtf_size_t inlineCapacity> explicit String(const Vector<UChar, inlineCapacity>&);
    ...
    explicit String(const std::string& s)
        : String(base::as_byte_span(s))
    {
    }
    explicit String(const Vector<char>& s)
        : String(base::span<const char>(s.data(), s.size()))
    {
    }
    ...
    String(scoped_refptr<StringImpl> impl)
        : impl_(std::move(impl))
    {
    }
    ...
    template <typename CharType> static String Adopt(StringBuffer<CharType>& buffer)
    {
        if (!buffer.length())
    ...
    scoped_refptr<StringImpl> ReleaseImpl()
    {
    ...
    template <typename CharacterType> inline const CharacterType* GetCharacters() const;
    ...
    [[nodiscard]] std::string Utf8(UTF8ConversionMode mode = kLenientUTF8Conversion) const
    {
    ...
    [[nodiscard]] static String NumberToStringFixedWidth(double, unsigned decimal_places);
    ...
    wtf_size_t Find(CharacterMatchFunctionPtr match_function, wtf_size_t start = 0) const
    {
    ...
    // Find substrings.
    wtf_size_t Find(const StringView& value, wtf_size_t start = 0, TextCaseSensitivity case_sensitivity = kTextCaseSensitive) const
    {
        return impl_ ? DISPATCH_CASE_OP(case_sensitivity, impl_->Find, (value, start)) : kNotFound;
    }

    // Unicode aware case insensitive string matching. Non-ASCII characters might
    // match to ASCII characters. This function is rarely used to implement web
    // platform features.
    wtf_size_t FindIgnoringCase(const StringView& value, unsigned start = 0) const
    {
        return impl_ ? impl_->FindIgnoringCase(value, start) : kNotFound;
    }

    // ASCII case insensitive string matching.
    wtf_size_t FindIgnoringASCIICase(const StringView& value, unsigned start = 0) const
    {
    ...
    bool Contains(const StringView& value, TextCaseSensitivity case_sensitivity = kTextCaseSensitive) const
    {
    ...
    wtf_size_t ReverseFind(const StringView& value, unsigned start = UINT_MAX) const
    {
    ...
    bool StartsWith(const StringView& prefix, TextCaseSensitivity case_sensitivity = kTextCaseSensitive) const
    {
        return impl_ ? DISPATCH_CASE_OP(case_sensitivity, impl_->StartsWith, (prefix)) : prefix.empty();
    }
    ...
    {
        return impl_ ? impl_->StartsWithIgnoringCaseAndAccents(prefix) : prefix.empty();
    }
    ...
    bool EndsWith(const StringView& suffix, TextCaseSensitivity case_sensitivity = kTextCaseSensitive) const
    {
        return impl_ ? DISPATCH_CASE_OP(case_sensitivity, impl_->EndsWith, (suffix)) : suffix.empty();
    }
    ...
    String& replace(unsigned index, unsigned length_to_replace, const StringView& replacement)
    {
        if (impl_)
    ...
    [[nodiscard]] String Left(unsigned len) const
    {
    ...
    [[nodiscard]] String SimplifyWhiteSpace(StripBehavior = kStripExtraWhiteSpace) const;
    [[nodiscard]] String SimplifyWhiteSpace(IsWhiteSpaceFunctionPtr, StripBehavior = kStripExtraWhiteSpace) const;
    ...
    [[nodiscard]] PRINTF_FORMAT(1, 2) static String Format(const char* format, ...);
    ...
    [[nodiscard]] static String CreateUninitialized(unsigned length, base::span<UChar>& data)
    {
        return StringImpl::CreateUninitialized(length, data);
    }
    [[nodiscard]] static String CreateUninitialized(unsigned length, base::span<LChar>& data)
    {
        return StringImpl::CreateUninitialized(length, data);
    }

    void Split(const StringView& separator, bool allow_empty_entries, Vector<String>& result) const;
    void Split(const StringView& separator, Vector<String>& result) const
    {
        Split(separator, false, result);
    }
    void Split(UChar separator, bool allow_empty_entries, Vector<String>& result) const;
    ...
    template <typename BufferType> void AppendTo(BufferType&, unsigned start = 0, unsigned length = UINT_MAX) const;
    template <typename BufferType> void PrependTo(BufferType&, unsigned start = 0, unsigned length = UINT_MAX) const;
    ...
    [[nodiscard]] static String FromUTF8WithLatin1Fallback(base::span<const uint8_t>);
    ...
    {
        return !impl_ || impl_->IsLowerASCII();
    }
...
template <wtf_size_t inlineCapacity> inline bool EqualIgnoringNullity(const Vector<UChar, inlineCapacity>& a, const String& b)
{
...
String::String(const Vector<UChar, inlineCapacity>& vector)
    : impl_(vector.size() ? StringImpl::Create(base::span<const UChar>(vector.data(), vector.size())) : StringImpl::empty_)
{
}

template <> inline const LChar* String::GetCharacters<LChar>() const
{
    DCHECK(Is8Bit());
    return Characters8();
}

template <> inline const UChar* String::GetCharacters<UChar>() const
{
    DCHECK(!Is8Bit());
    return Characters16();
}
...
inline bool CodeUnitCompareIgnoringASCIICaseLessThan(const String& a, const String& b)
{
...
template <typename BufferType> void String::AppendTo(BufferType& result, unsigned position, unsigned length) const
{
    if (!impl_)
        return;
...
template <typename BufferType> void String::PrependTo(BufferType& result, unsigned position, unsigned length) const
{
    if (!impl_)
        return;
...
inline StringView::StringView(const String& string LIFETIME_BOUND, unsigned offset, unsigned length)
    : StringView(string.Impl(), offset, length)
{
}
inline StringView::StringView(const String& string LIFETIME_BOUND, unsigned offset)
