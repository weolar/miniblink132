...
class UCharBuffer {
public:
    ALWAYS_INLINE static unsigned ComputeHashAndMaskTop8Bits(const UChar* chars, unsigned len, AtomicStringUCharEncoding encoding)
    {
        if (encoding == AtomicStringUCharEncoding::kIs8Bit || (encoding == AtomicStringUCharEncoding::kUnknown && IsOnly8Bit(chars, len))) {
            // This is a very common case from HTML parsing, so we take
            // the size penalty from inlining.
            return StringHasher::ComputeHashAndMaskTop8BitsInline<ConvertTo8BitHashReader>((const char*)chars, len);
        } else {
            return StringHasher::ComputeHashAndMaskTop8Bits((const char*)chars, len * 2);
        }
    }

    ALWAYS_INLINE UCharBuffer(const UChar* chars, unsigned len, AtomicStringUCharEncoding encoding)
        : characters_(chars)
        , length_(len)
    ...
    AtomicStringUCharEncoding encoding() const
    {
        return encoding_;
    }
    ...
    scoped_refptr<StringImpl> CreateStringImpl() const
    {
        ...
        case AtomicStringUCharEncoding::kIs8Bit:
            return String::Make8BitFrom16BitSource({ characters_, length_ }).ReleaseImpl();
...
struct UCharBufferTranslator {
    ...
    static void Store(StringImpl*& location, const UCharBuffer& buf, unsigned hash)
    {
...
struct StringViewLookupTranslator {
    static unsigned GetHash(const StringView& buf)
    {
        ...
        if (buf.Is8Bit()) {
            return StringHasher::ComputeHashAndMaskTop8Bits((const char*)buf.Characters8(), buf.length());
        } else {
            return StringHasher::ComputeHashAndMaskTop8Bits((const char*)buf.Characters16(), buf.length());
        }
    }
...
// on performance.
template <typename CharType> struct ASCIILowerHashReader {
    ...
    ALWAYS_INLINE static uint64_t Read64(const uint8_t* ptr)
    {
        ...
        if constexpr (sizeof(CharType) == 2) {
            return Lowercase(p[0]) | (Lowercase(p[1]) << 16) | (Lowercase(p[2]) << 32) | (Lowercase(p[3]) << 48);
        } else {
            return Lowercase(p[0]) | (Lowercase(p[1]) << 8) | (Lowercase(p[2]) << 16) | (Lowercase(p[3]) << 24) | (Lowercase(p[4]) << 32)
                | (Lowercase(p[5]) << 40) | (Lowercase(p[6]) << 48) | (Lowercase(p[7]) << 56);
        }
#endif
    }
    ALWAYS_INLINE static uint64_t Read32(const uint8_t* ptr)
    {
        ...
        if constexpr (sizeof(CharType) == 2) {
            ...
            return Lowercase(p[0]) | (Lowercase(p[1]) << 8) | (Lowercase(p[2]) << 16) | (Lowercase(p[3]) << 24);
        }
#endif
    }
    ...
    ALWAYS_INLINE static uint64_t ReadSmall(const uint8_t* p, size_t k)
    {
        if constexpr (sizeof(CharType) == 2) {
            ...
            DCHECK_EQ(k, 2u);
            k = 2;
            return (uint64_t { p[0] } << 56) | (uint64_t { p[k >> 1] } << 32) | uint64_t { p[k - 1] };
        } else {
            return (Lowercase(p[0]) << 56) | (Lowercase(p[k >> 1]) << 32) | Lowercase(p[k - 1]);
        }
    }
};
...
struct ASCIIConvertTo8AndLowerHashReader {
    ...
    static uint64_t Lowercase(uint16_t ch)
    {
        return ToASCIILower(ch);
    }
    ...
    static uint64_t Read64(const uint8_t* ptr)
    {
        ...
        return Lowercase(p[0]) | (Lowercase(p[1]) << 8) | (Lowercase(p[2]) << 16) | (Lowercase(p[3]) << 24) | (Lowercase(p[4]) << 32) | (Lowercase(p[5]) << 40)
...
public:
    explicit HashTranslatorLowercaseBuffer(const StringImpl* impl)
        : impl_(impl)
    {
        ...
        if (impl_->Is8Bit()) {
            hash_ = StringHasher::ComputeHashAndMaskTop8Bits<ASCIILowerHashReader<LChar>>((const char*)impl_->Characters8(), impl_->length());
        } else {
            if (IsOnly8Bit(impl_->Characters16(), impl_->length())) {
                hash_ = StringHasher::ComputeHashAndMaskTop8Bits<ASCIIConvertTo8AndLowerHashReader>((const char*)impl_->Characters16(), impl_->length());
            } else {
                hash_ = StringHasher::ComputeHashAndMaskTop8Bits<ASCIILowerHashReader<UChar>>((const char*)impl_->Characters16(), impl_->length() * 2);
            }
        }
    }
...
struct LowercaseLookupTranslator {
    ...
    static bool Equal(StringImpl* const& bucket, const HashTranslatorLowercaseBuffer& buf)
    {
...
template <typename T, typename HashTranslator> scoped_refptr<StringImpl> AtomicStringTable::AddToStringTable(const T& value)
{
    // Lock not only protects access to the table, it also guarantees
    // mutual exclusion with the refcount decrement on removal.
    base::AutoLock auto_lock(lock_);
    HashSet<StringImpl*>::AddResult add_result = table_.AddWithTranslator<HashTranslator>(value);

    // If the string is newly-translated, then we need to adopt it.
    // The boolean in the pair tells us if that is so.
    return add_result.is_new_entry ? base::AdoptRef(*add_result.stored_value) : base::WrapRefCounted(*add_result.stored_value);
}

scoped_refptr<StringImpl> AtomicStringTable::Add(const UChar* s, unsigned length, AtomicStringUCharEncoding encoding)
{
    if (!s)
        return nullptr;
...
public:
    ALWAYS_INLINE LCharBuffer(const LChar* chars, unsigned len)
        ...
        hash_(StringHasher::ComputeHashAndMaskTop8BitsInline((const char*)chars, len))
    {
    }

    base::span<const LChar> characters() const
    {
        return { characters_, length_ };
    }
    unsigned hash() const
    {
        return hash_;
...
struct LCharBufferTranslator {
    ...
    static void Store(StringImpl*& location, const LCharBuffer& buf, unsigned hash)
    {
...
scoped_refptr<StringImpl> AtomicStringTable::Add(const StringView& string_view)
{
    ...
    UCharBuffer buffer(string_view.Characters16(), string_view.length(), AtomicStringUCharEncoding::kUnknown);
    return AddToStringTable<UCharBuffer, UCharBufferTranslator>(buffer);
}

scoped_refptr<StringImpl> AtomicStringTable::Add(const LChar* s, unsigned length)
{
    if (!s)
        return nullptr;
...
scoped_refptr<StringImpl> AtomicStringTable::Add(scoped_refptr<StringImpl>&& string)
{
    if (!string->length())
        return StringImpl::empty_;
...
scoped_refptr<StringImpl> AtomicStringTable::AddUTF8(const char* characters_start, const char* characters_end)
{
    bool seen_non_ascii = false;
    bool seen_non_latin1 = false;
    unsigned utf16_length = unicode::CalculateStringLengthFromUTF8(characters_start, characters_end, seen_non_ascii, seen_non_latin1);
    if (!seen_non_ascii) {
    ...
    if (unicode::ConvertUTF8ToUTF16(&source, characters_end, &dptr, utf16_buf.get() + utf16_length) != unicode::kConversionOK) {
        NOTREACHED();
    }

    UCharBuffer buffer(utf16_buf.get(), utf16_length, seen_non_latin1 ? AtomicStringUCharEncoding::kIs16Bit : AtomicStringUCharEncoding::kIs8Bit);
    return AddToStringTable<UCharBuffer, UCharBufferTranslator>(buffer);
}

AtomicStringTable::WeakResult AtomicStringTable::WeakFindSlowForTesting(const StringView& string)
{
    DCHECK(string.length());
    base::AutoLock auto_lock(lock_);
...
AtomicStringTable::WeakResult AtomicStringTable::WeakFindLowercase(const AtomicString& string)
{
    DCHECK(!string.empty());
