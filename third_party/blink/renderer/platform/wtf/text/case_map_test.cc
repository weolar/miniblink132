...
static struct CaseMapTestData {
    ...
    { u"\u00DFyi\u0307z", "lt", u"\u00DFyi\u0307z", u"SSYIZ", {}, { { 1, 2 }, { 4, 4 } } },
...
class CaseMapTest : public testing::Test, public testing::WithParamInterface<CaseMapTestData> { };

INSTANTIATE_TEST_SUITE_P(CaseMapTest, CaseMapTest, testing::ValuesIn(case_map_test_data));
...
const char* g_greek_input = "\xCE\x9F\xCE\x94\xCE\x8C\xCE\xA3 \xCE\x9F\xCE\xB4\xCF\x8C\xCF\x82 "
                            "\xCE\xA3\xCE\xBF \xCE\xA3\xCE\x9F o\xCE\xA3 \xCE\x9F\xCE\xA3 \xCF\x83 "
                            "\xE1\xBC\x95\xCE\xBE";
const char* g_lithuanian_input = "I \xC3\x8F J J\xCC\x88 \xC4\xAE \xC4\xAE\xCC\x88 \xC3\x8C \xC3\x8D "
...
TEST(CaseMapTest, ToUpperLocale)
{
    ...
    for (size_t i = 0; i < sizeof(test_data_list) / sizeof(test_data_list[0]); ++i) {
        ...
        for (size_t j = 0; j < test_data_list[i].locale_list_length; ++j) {
            ...
            EXPECT_EQ(expected, case_map.ToUpper(source).Utf8()) << test_data_list[i].source_description << "; locale=" << locale;
        }
    }
}
...
TEST(CaseMapTest, ToLowerLocale)
{
    ...
    for (size_t i = 0; i < sizeof(test_data_list) / sizeof(test_data_list[0]); ++i) {
        ...
        for (size_t j = 0; j < test_data_list[i].locale_list_length; ++j) {
            const char* locale = test_data_list[i].locale_list[j];
            CaseMap case_map { AtomicString(locale) };
            EXPECT_EQ(expected, case_map.ToLower(source).Utf8()) << test_data_list[i].source_description << "; locale=" << locale;
        }
    }
}
