...
{
    return reinterpret_cast<T*>(reinterpret_cast<uintptr_t>(pointer) & ~kMachineWordAlignmentMask);
}
...
template <typename CharacterType> inline bool IsAllASCII(MachineWord word)
{
...
template <typename CharacterType> ALWAYS_INLINE ASCIIStringAttributes CharacterAttributes(const CharacterType* characters, size_t length)
{
    DCHECK_GT(length, 0u);
...
template <typename CharacterType> ALWAYS_INLINE bool IsLowerASCII(const CharacterType* characters, size_t length)
{
    bool contains_upper_case = false;
...
template <typename CharacterType> ALWAYS_INLINE bool IsUpperASCII(const CharacterType* characters, size_t length)
{
...
public:
    template <typename CharType> ALWAYS_INLINE static bool IsCorrectCase(CharType* characters, size_t length)
    {
        return IsLowerASCII(characters, length);
    }

    template <typename CharType> ALWAYS_INLINE static CharType Convert(CharType ch)
    {
...
ALWAYS_INLINE typename Allocator::ResultStringType ConvertASCIICase(const StringType& string, Converter&& converter, Allocator&& allocator)
{
