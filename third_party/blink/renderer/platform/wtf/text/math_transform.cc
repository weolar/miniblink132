...
static UChar32 mathVariantGreek(UChar32 code_point, UChar32 base_char)
{
    ...
    auto ret = base_char + kMathBoldUpperAlpha + (kMathItalicUpperAlpha - kMathBoldUpperAlpha);
    return ret;
}
...
static UChar32 mathVariantLatin(UChar32 code_point, UChar32 base_char)
{
    ...
    // within the number math_variant range. To this the base_char calculated
    // earlier is added to obtain the final code point.
    UChar32 transformed_char = base_char + kMathBoldUpperA + (kMathItalicUpperA - kMathBoldUpperA);
...
UChar32 ItalicMathVariant(UChar32 code_point)
{
    ...
    if (IsASCIIUpper(code_point)) {
        ...
        base_char = kMathBoldSmallA - kMathBoldUpperA + code_point - kASCIILowerStart;
        var_type = kLatin;
        ...
        base_char = kMathBoldSmallAlpha - kMathBoldUpperAlpha + code_point - kGreekLowerAlpha;
        var_type = kGreekish;
    } else {
        switch (code_point) {
