...
AtomicString::AtomicString(base::span<const LChar> chars)
    : string_(AtomicStringTable::Instance().Add(chars.data(), base::checked_cast<wtf_size_t>(chars.size())))
{
}

AtomicString::AtomicString(base::span<const UChar> chars, AtomicStringUCharEncoding encoding)
    : string_(AtomicStringTable::Instance().Add(chars.data(), base::checked_cast<wtf_size_t>(chars.size()), encoding))
{
}

AtomicString::AtomicString(const UChar* chars)
    : string_(AtomicStringTable::Instance().Add(chars, chars ? LengthOfNullTerminatedString(chars) : 0, AtomicStringUCharEncoding::kUnknown))
{
}
...
scoped_refptr<StringImpl> AtomicString::AddSlowCase(scoped_refptr<StringImpl>&& string)
{
