...
TEST(WeakResultTest, UTF8)
{
    ...
    AtomicStringTable::WeakResult result = WTF::AtomicStringTable::Instance().WeakFindLowercase(AtomicString("FOO"));
    ...
    result = WTF::AtomicStringTable::Instance().WeakFindLowercase(AtomicString(too_wide_string.Impl()));
    EXPECT_FALSE(result.IsNull());

    AtomicStringTable::WeakResult result_latin1 = WTF::AtomicStringTable::Instance().WeakFindLowercase(AtomicString::FromUTF8("Foó"));
    EXPECT_FALSE(result_latin1.IsNull());

    // Only ASCII is lowercased.
    result_latin1 = WTF::AtomicStringTable::Instance().WeakFindLowercase(AtomicString::FromUTF8("FoÓ"));
    EXPECT_TRUE(result_latin1.IsNull());

    AtomicStringTable::WeakResult result_unicode = WTF::AtomicStringTable::Instance().WeakFindLowercase(AtomicString::FromUTF8("foO😀"));
    EXPECT_FALSE(result_unicode.IsNull());

    result_unicode = WTF::AtomicStringTable::Instance().WeakFindLowercase(AtomicString::FromUTF8("Goo😀"));
