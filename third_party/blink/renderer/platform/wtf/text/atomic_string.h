...
public:
    ...
    explicit AtomicString(const LChar* chars)
        : AtomicString(base::span<const LChar> { chars, chars ? strlen(reinterpret_cast<const char*>(chars)) : 0 })
    {
    }
    ...
    explicit AtomicString(base::span<const UChar> chars, AtomicStringUCharEncoding encoding = AtomicStringUCharEncoding::kUnknown);
    ...
    // platform features.
    wtf_size_t FindIgnoringCase(const StringView& value, wtf_size_t start = 0) const
    {
        return string_.FindIgnoringCase(value, start);
    }

    // ASCII case insensitive string matching.
    wtf_size_t FindIgnoringASCIICase(const StringView& value, wtf_size_t start = 0) const
    {
    ...
    wtf_size_t ReverseFind(const StringView& value, wtf_size_t start = UINT_MAX) const
    {
    ...
    {
        return string_.EndsWith(character);
    }
    ...
    {
        return string_.IsLowerASCII();
    }

    // See comments in WTFString.h.
    int ToInt(bool* ok = nullptr) const
    {
        return string_.ToInt(ok);
    }
    double ToDouble(bool* ok = nullptr) const
    {
        return string_.ToDouble(ok);
    }
    float ToFloat(bool* ok = nullptr) const
    {
        return string_.ToFloat(ok);
    }

    template <typename IntegerType> static AtomicString Number(IntegerType number)
    {
    ...
    AtomicString(NSString* s)
        : string_(Add(base::apple::NSToCFPtrCast(s)))
    {
    }
...
private:
    ...
    ALWAYS_INLINE static scoped_refptr<StringImpl> Add(scoped_refptr<StringImpl>&& r)
    {
        if (!r || r->IsAtomic())
            return std::move(r);
...
template <> struct HashTraits<AtomicString>;

// Pretty printer for gtest and base/logging.*.  It prepends and appends
// double-quotes, and escapes characters other than ASCII printables.
WTF_EXPORT std::ostream& operator<<(std::ostream&, const AtomicString&);

inline StringView::StringView(const AtomicString& string LIFETIME_BOUND, unsigned offset, unsigned length)
    : StringView(string.Impl(), offset, length)
{
}
inline StringView::StringView(const AtomicString& string LIFETIME_BOUND, unsigned offset)
