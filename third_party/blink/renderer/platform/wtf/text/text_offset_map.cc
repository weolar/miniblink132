...
int ChunkLengthDifference(const Vector<TextOffsetMap::Entry>& entries, wtf_size_t index)
{
...
std::ostream& operator<<(std::ostream& stream, const TextOffsetMap::Entry& entry)
{
    return stream << "{" << entry.source << ", " << entry.target << "}";
}

std::ostream& operator<<(std::ostream& stream, const Vector<TextOffsetMap::Entry>& entries)
{
    stream << "{";
...
TextOffsetMap::TextOffsetMap(const TextOffsetMap& map12, const TextOffsetMap& map23, bool fix_crash)
{
    if (map12.IsEmpty()) {
    ...
    while (index12 < size12 && index23 < size23) {
        ...
        if (chunk_length_diff_12 < 0 && chunk_length_diff_23 < 0 && entry12.target + offset_diff_23 == entry23.target) {
            // No need to handle entry12 because it was overwritten by entry23.
            offset_diff_12 = entry12.target - entry12.source;
            ++index12;
        } else if (chunk_length_diff_12 > 0 && chunk_length_diff_23 > 0 && entry12.source == entry23.source - offset_diff_12) {
            offset_diff_12 = entry12.target - entry12.source;
...
{
    CHECK(IsEmpty() || (source > entries_.back().source && target > entries_.back().target));
...
void TextOffsetMap::Append(const icu::Edits& edits)
{
    ...
    while (edit.next(error)) {
        ...
        entries_.emplace_back(edit.sourceIndex() + edit.oldLength(), edit.destinationIndex() + edit.newLength());
    }
