...
double ParseDoubleFromLongString(const UChar* string, size_t length, size_t& parsed_length)
{
...
double ParseDouble(const LChar* string, size_t length, size_t& parsed_length)
{
    ...
    double d = GetDoubleConverter().StringToDouble(reinterpret_cast<const char*>(string), base::saturated_cast<int>(length), &int_parsed_length);
...
double ParseDouble(const UChar* string, size_t length, size_t& parsed_length)
{
    ...
    for (size_t i = 0; i < length; ++i) {
        conversion_buffer[i] = IsASCII(string[i]) ? static_cast<LChar>(string[i]) : 0;
    }
