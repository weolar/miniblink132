...
public:
    explicit DestructCounter(int i, int* destruct_number)
        : i_(i)
        , destruct_number_(destruct_number)
    {
    }
...
struct MoveOnlyHashTraits : public GenericHashTraits<MoveOnlyHashValue> {
    ...
    static bool Equal(const MoveOnlyHashValue& left, const MoveOnlyHashValue& right)
    {
        return left.Value() == right.Value();
    }
};

template <> struct HashTraits<MoveOnlyHashValue> : MoveOnlyHashTraits { };
...
struct CountCopyHashTraits : public GenericHashTraits<CountCopy> {
    static const bool kEmptyValueIsZero = false;
    static bool IsEmptyValue(const CountCopy& value)
    {
...
template <> struct HashTraits<CountCopy> : CountCopyHashTraits { };

template <typename T> class ValueInstanceCount final {
public:
    static int* const kDeletedValue;
...
template <typename T> struct ValueInstanceCountHashTraits : public GenericHashTraits<ValueInstanceCount<T>> {
    static const bool kEmptyValueIsZero = false;
    ...
    static bool Equal(const ValueInstanceCount<T>& left, const ValueInstanceCount<T>& right)
    {
        return left.Counter() == right.Counter();
    }
};

template <typename T> struct HashTraits<ValueInstanceCount<T>> : public ValueInstanceCountHashTraits<T> { };

class DummyRefCounted : public RefCounted<DummyRefCounted> {
public:
    DummyRefCounted(bool& is_deleted)
