...
protected:
    template <unsigned keyBits> size_t BloomFilterBitArrayIndex(unsigned key)
    {
        return BloomFilter<keyBits>::BitArrayIndex(key);
    }

    template <unsigned keyBits> unsigned BloomFilterBitMask(unsigned key)
    {
        return BloomFilter<keyBits>::BitMask(key);
    }

    template <unsigned keyBits> void TestBloomFilterKeyBoundary()
    {
        ...
        for (unsigned i = max_key_bits; i < max_key_bits + keyBits; i++) {
            unsigned hash = 1u << i;
            EXPECT_FALSE(filter.MayContain(hash)) << String::Format("BloomFilter<%d>.Add(0) Must not contain 0x%08x", keyBits, hash);
        }
    }
};
