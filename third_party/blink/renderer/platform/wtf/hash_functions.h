...
template <typename T> using IntHashBits = typename IntTypes<sizeof(T)>::UnsignedType;
...
template <typename T> unsigned HashInt(T key)
{
    static_assert(std::is_integral_v<T> || std::is_enum_v<T>);
    return internal::HashInt(static_cast<internal::IntHashBits<T>>(key));
}

template <typename T> unsigned HashFloat(T key)
{
    static_assert(std::is_floating_point_v<T>);
    return internal::HashInt(base::bit_cast<internal::IntHashBits<T>>(key));
}

template <typename T> bool FloatEqualForHash(T a, T b)
{
    static_assert(std::is_floating_point_v<T>);
    return base::bit_cast<internal::IntHashBits<T>>(a) == base::bit_cast<internal::IntHashBits<T>>(b);
}

template <typename T> unsigned HashPointer(T* key)
{
