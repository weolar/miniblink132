...
template <typename T, typename Traits> class RefCounted;
...
template <typename T, typename Traits = DefaultRefCountedTraits<T>> class RefCounted : public base::RefCounted<T, Traits> {
...
template <typename T> class RefCountedCopyable : public RefCounted<T> {
protected:
    RefCountedCopyable() = default;
    RefCountedCopyable(const RefCountedCopyable&)
        : RefCounted<T>()
    {
    }
};
