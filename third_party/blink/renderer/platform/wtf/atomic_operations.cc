...
template <typename AlignmentType> void AtomicReadMemcpyImpl(void* to, const void* from, size_t bytes)
{
    // Check alignment of |to| and |from|.
    DCHECK_EQ(0u, static_cast<AlignmentType>(reinterpret_cast<size_t>(to)) & (sizeof(AlignmentType) - 1));
    DCHECK_EQ(0u, static_cast<AlignmentType>(reinterpret_cast<size_t>(from)) & (sizeof(AlignmentType) - 1));
    auto* sizet_to = reinterpret_cast<AlignmentType*>(to);
    const auto* sizet_from = reinterpret_cast<const AlignmentType*>(from);
    for (; bytes >= sizeof(AlignmentType); bytes -= sizeof(AlignmentType), ++sizet_to, ++sizet_from) {
...
template <typename AlignmentType> void AtomicWriteMemcpyImpl(void* to, const void* from, size_t bytes)
{
    // Check alignment of |to| and |from|.
    DCHECK_EQ(0u, static_cast<AlignmentType>(reinterpret_cast<size_t>(to)) & (sizeof(AlignmentType) - 1));
    DCHECK_EQ(0u, static_cast<AlignmentType>(reinterpret_cast<size_t>(from)) & (sizeof(AlignmentType) - 1));
...
template <typename AlignmentType> void AtomicMemzeroImpl(void* buf, size_t bytes)
{
    // Check alignment of |buf|.
    DCHECK_EQ(0u, static_cast<AlignmentType>(reinterpret_cast<size_t>(buf)) & (sizeof(AlignmentType) - 1));
    auto* sizet_buf = reinterpret_cast<AlignmentType*>(buf);
    for (; bytes >= sizeof(AlignmentType); bytes -= sizeof(AlignmentType), ++sizet_buf) {
