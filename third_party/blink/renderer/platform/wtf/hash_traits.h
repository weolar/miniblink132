...
template <typename T> struct GenericHashTraitsBase {
    ...
    // as get.
    using PeekOutType = T;
    static const T& Peek(const T& value)
    {
        return value;
    }
    ...
    // forbidden scope.
    template <typename U = void> struct NeedsToForbidGCOnMove {
...
template <typename T, auto empty_value, auto deleted_value> struct IntOrEnumHashTraits : internal::GenericHashTraitsBase<T> {
    static_assert(std::is_integral_v<T> || std::is_enum_v<T>);
    static unsigned GetHash(T key)
    {
        return WTF::HashInt(key);
    }
    static constexpr bool kEmptyValueIsZero = static_cast<int64_t>(empty_value) == 0;
    static constexpr T EmptyValue()
    {
        return static_cast<T>(empty_value);
    }
    static constexpr T DeletedValue()
    {
        return static_cast<T>(deleted_value);
    }
};
...
struct IntHashTraits : internal::IntOrEnumHashTraits<T, empty_value, deleted_value> {
    static_assert(std::is_integral_v<T>);
};

// Default traits for an enum type.  0 is very popular, and -1 is also popular.
// So we use -128 and -127.
template <typename T> struct EnumHashTraits : internal::IntOrEnumHashTraits<T, -128, -127> {
    static_assert(std::is_enum_v<T>);
};

template <typename T> struct GenericHashTraits : internal::GenericHashTraitsBase<T> {
    static_assert(!std::is_integral_v<T>);
    static_assert(!std::is_enum_v<T>);
...
struct GenericHashTraits<T> : internal::GenericHashTraitsBase<T> {
    static unsigned GetHash(T key)
    {
        return HashFloat(key);
    }
    static bool Equal(T a, T b)
    {
        return FloatEqualForHash(a, b);
    }
    static constexpr T EmptyValue()
    {
...
template <typename T> struct IntWithZeroKeyHashTraits : IntHashTraits<T, std::numeric_limits<T>::max(), std::numeric_limits<T>::max() - 1> { };
...
struct AlreadyHashedTraits : GenericHashTraits<unsigned> {
    static unsigned GetHash(unsigned key)
    {
        return key;
    }
};
...
template <typename P> struct GenericHashTraits<P*> : internal::GenericHashTraitsBase<P*> {
    static unsigned GetHash(P* key)
    {
        return HashPointer(key);
    }
    static constexpr bool kEmptyValueIsZero = true;
    static constexpr P* DeletedValue()
    {
        return reinterpret_cast<P*>(-1);
    }
};

template <typename P> struct GenericHashTraits<scoped_refptr<P>> : internal::GenericHashTraitsBase<scoped_refptr<P>> {
    ...
    static bool Equal(P* a, const scoped_refptr<P>& b)
    {
        return a == b;
    }
    static bool Equal(const scoped_refptr<P>& a, P* b)
    {
        return a == b;
    }
    ...
    public:
        ALWAYS_INLINE RefPtrValuePeeker(P* p)
            : ptr_(p)
        {
        }
        template <typename U>
        RefPtrValuePeeker(const scoped_refptr<U>& p)
            : ptr_(p.get())
        {
        }

        ALWAYS_INLINE operator P*() const
        {
            return ptr_;
        }
    ...
    static constexpr bool kEmptyValueIsZero = true;
    static bool IsEmptyValue(const scoped_refptr<P>& value)
    {
        return !value;
    }
    ...
    {
        return *reinterpret_cast<void* const*>(&value) == reinterpret_cast<const void*>(-1);
    }
    ...
    static PeekOutType Peek(const scoped_refptr<P>& value)
    {
        return value.get();
    }

    template <typename = void> struct NeedsToForbidGCOnMove {
        static constexpr bool value = false;
    };
};

template <typename T> struct GenericHashTraits<std::unique_ptr<T>> : internal::GenericHashTraitsBase<std::unique_ptr<T>> {
    static unsigned GetHash(T* key)
    {
        return HashPointer(key);
    }
    ...
    static constexpr bool kEmptyValueIsZero = true;
    static bool IsEmptyValue(const std::unique_ptr<T>& value)
    {
        return !value;
    }
    ...
    static void ConstructDeletedValue(std::unique_ptr<T>& slot)
    {
        ...
        new (NotNullTag::kNotNull, &slot) std::unique_ptr<T>(reinterpret_cast<T*>(1u));
    }
...
template <> struct HashTraits<String>;

namespace internal {

template <typename Traits, typename Enabled = void> struct HashTraitsEmptyValueChecker {
...
struct HashTraitsEmptyValueChecker<Traits, std::enable_if_t<std::is_same_v<decltype(Traits::IsEmptyValue(std::declval<typename Traits::TraitType>())), bool>>> {
    static bool IsEmptyValue(const typename Traits::TraitType& value)
    {
...
template <typename Traits, typename Enabled = void> struct HashTraitsDeletedValueHelper {
    ...
    {
        new (NotNullTag::kNotNull, &slot) typename Traits::TraitType(Traits::DeletedValue());
    }
};
template <typename Traits>
struct HashTraitsDeletedValueHelper<Traits,
    std::enable_if_t<std::is_same_v<decltype(Traits::IsDeletedValue(std::declval<typename Traits::TraitType>())), bool>>> {
    static bool IsDeletedValue(const typename Traits::TraitType& value)
    {
...
template <typename Traits, typename T> inline bool IsHashTraitsDeletedValue(const T& value)
{
...
template <typename Traits, typename T> inline void ConstructHashTraitsDeletedValue(T& slot)
{
    internal::HashTraitsDeletedValueHelper<Traits>::ConstructDeletedValue(slot);
}

template <typename Traits, typename T> inline bool IsHashTraitsEmptyOrDeletedValue(const T& value)
{
    return IsHashTraitsEmptyValue<Traits, T>(value) || IsHashTraitsDeletedValue<Traits, T>(value);
}

// A HashTraits type for T to delegate all HashTraits API to a field.
template <typename T, auto field, typename FieldTraits = HashTraits<std::remove_reference_t<decltype(std::declval<T>().*field)>>>
struct OneFieldHashTraits : GenericHashTraits<T> {
    using TraitType = T;
    static unsigned GetHash(const T& p)
    {
        return FieldTraits::GetHash(p.*field);
    }
    static bool Equal(const T& a, const T& b)
    {
        return FieldTraits::Equal(a.*field, b.*field);
    }
    static constexpr bool kSafeToCompareToEmptyOrDeleted = FieldTraits::kSafeToCompareToEmptyOrDeleted;

    static constexpr bool kEmptyValueIsZero = FieldTraits::kEmptyValueIsZero;
    static T EmptyValue()
    {
        return T(FieldTraits::EmptyValue());
    }
    ...
    template <typename U = void> struct NeedsToForbidGCOnMove {
        static const bool value = FieldTraits::template NeedsToForbidGCOnMove<>::value;
    };
};

// A HashTraits type for T to delegate all HashTraits API to two fields.
template <typename T, auto first_field, auto second_field, typename FirstTraits = HashTraits<std::remove_reference_t<decltype(std::declval<T>().*first_field)>>,
    typename SecondTraits = HashTraits<std::remove_reference_t<decltype(std::declval<T>().*second_field)>>>
struct TwoFieldsHashTraits : OneFieldHashTraits<T, first_field, FirstTraits> {
    using TraitType = T;
    static unsigned GetHash(const T& p)
    {
        return HashInts(FirstTraits::GetHash(p.*first_field), SecondTraits::GetHash(p.*second_field));
    }
    static bool Equal(const T& a, const T& b)
    {
        return FirstTraits::Equal(a.*first_field, b.*first_field) && SecondTraits::Equal(a.*second_field, b.*second_field);
    }
    static constexpr bool kSafeToCompareToEmptyOrDeleted = FirstTraits::kSafeToCompareToEmptyOrDeleted && SecondTraits::kSafeToCompareToEmptyOrDeleted;

    static constexpr bool kEmptyValueIsZero = FirstTraits::kEmptyValueIsZero && SecondTraits::kEmptyValueIsZero;
    static T EmptyValue()
    {
    ...
    static bool IsEmptyValue(const T& value)
    {
        return IsHashTraitsEmptyValue<FirstTraits>(value.*first_field) && IsHashTraitsEmptyValue<SecondTraits>(value.*second_field);
    }
    ...
    template <typename U = void> struct NeedsToForbidGCOnMove {
        static const bool value = FirstTraits::template NeedsToForbidGCOnMove<>::value || SecondTraits::template NeedsToForbidGCOnMove<>::value;
    };
};

template <typename FirstTraitsArg, typename SecondTraitsArg, typename P = std::pair<typename FirstTraitsArg::TraitType, typename SecondTraitsArg::TraitType>>
struct PairHashTraits : TwoFieldsHashTraits<P, &P::first, &P::second, FirstTraitsArg, SecondTraitsArg> {
    using TraitType = P;
    using FirstTraits = FirstTraitsArg;
    using SecondTraits = SecondTraitsArg;
};

template <typename First, typename Second> struct HashTraits<std::pair<First, Second>> : public PairHashTraits<HashTraits<First>, HashTraits<Second>> { };

// Shortcut of HashTraits<T>::GetHash(), which can deduct T automatically.
template <typename T> unsigned GetHash(const T& key)
{
