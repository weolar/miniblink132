...
public:
    ...
    static inline bool ShrinkVectorBacking(void* address, size_t quantized_current_size, size_t quantized_shrunk_size)
    {
    ...
    template <typename T, typename HashTable> static void FreeHashTableBacking(void* address)
    {
        FreeBacking(address);
    }

    template <typename Return, typename Metadata> static Return Malloc(size_t size, const char* type_name)
    {
        return reinterpret_cast<Return>(WTF::Partitions::FastMalloc(size, type_name));
    }

    template <typename T, typename HashTable> static inline bool ExpandHashTableBacking(void*, size_t)
    {
        return false;
    }
    template <typename Traits> static inline bool CanReuseHashTableDeletedBucket()
    {
        return true;
    }
    ...
    template <typename T> static void* NewArray(size_t bytes)
    {
    ...
    template <typename T> static void TraceBackingStoreIfMarked(T*)
    {
    }
    template <typename T> static void BackingWriteBarrier(T**)
    {
    }
    ...
    template <typename T, typename Traits> static void NotifyNewObjects(base::span<T>)
    {
    }
