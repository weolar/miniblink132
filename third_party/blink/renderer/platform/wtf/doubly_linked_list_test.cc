...
DoublyLinkedList<TestNode>::AddResult DoublyLinkedListTest::CheckedInsert(int i)
{
    size_t current_size = list_.size();

    auto result = list_.Insert(std::make_unique<TestNode>(i), CompareInt);
    EXPECT_EQ(list_.size(), result.is_new_entry ? current_size + 1 : current_size);
    EXPECT_EQ(test_node_counter, result.is_new_entry ? current_size + 1 : current_size);
...
TEST_F(DoublyLinkedListTest, InsertAfter)
{
    ...
    auto end_result = list_.InsertAfter(std::make_unique<TestNode>(10), begin_result.node);
    ...
    auto center_result = list_.InsertAfter(std::make_unique<TestNode>(5), begin_result.node);
    ...
    auto new_end_result = list_.InsertAfter(std::make_unique<TestNode>(20), end_result.node);
