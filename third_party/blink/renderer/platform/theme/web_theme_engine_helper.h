...
public:
    ...
    static void DidUpdateRendererPreferences(const blink::RendererPreferences& renderer_prefs);

    // Swaps the current theme engine out returning the old one.
    static std::unique_ptr<WebThemeEngine> SwapNativeThemeEngineForTesting(std::unique_ptr<WebThemeEngine> new_theme);
