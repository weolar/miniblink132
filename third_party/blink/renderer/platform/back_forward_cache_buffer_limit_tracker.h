...
public:
    ...
    void DidRemoveFrameOrWorkerFromBackForwardCache(size_t total_bytes) LOCKS_EXCLUDED(lock_);

    bool IsUnderPerProcessBufferLimit() LOCKS_EXCLUDED(lock_);

    size_t total_bytes_buffered_for_testing() LOCKS_EXCLUDED(lock_);

    BackForwardCacheBufferLimitTracker(BackForwardCacheBufferLimitTracker&) = delete;
