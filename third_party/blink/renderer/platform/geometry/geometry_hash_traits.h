...
template <> struct HashTraits<gfx::SizeF> : GenericHashTraits<gfx::SizeF> {
    ...
    {
        return HashTraits<float>::Equal(a.width(), b.width()) && HashTraits<float>::Equal(a.height(), b.height());
    }
...
template <> struct HashTraits<SkIRect> : GenericHashTraits<SkIRect> {
    STATIC_ONLY(HashTraits);
    static unsigned GetHash(const SkIRect& key)
    {
        return HashInts(HashInts(key.x(), key.y()), HashInts(key.right(), key.bottom()));
    }

    static constexpr bool kEmptyValueIsZero = false;
    static SkIRect EmptyValue()
    {
        return SkIRect::MakeWH(-1, 0);
    }
    static SkIRect DeletedValue()
    {
        return SkIRect::MakeWH(0, -1);
    }
};
