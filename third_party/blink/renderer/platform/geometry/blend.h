...
// For unsigned types.
template <typename T> inline T Blend(T from, T to, double progress)
{
...
inline gfx::PointF Blend(const gfx::PointF& from, const gfx::PointF& to, double progress)
{
    return gfx::PointF(Blend(from.x(), to.x(), progress), Blend(from.y(), to.y(), progress));
}
