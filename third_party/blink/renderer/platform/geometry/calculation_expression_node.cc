...
float CalculationExpressionNumberNode::Evaluate(float max_value, const EvaluationInput&) const
{
    return value_;
}
...
scoped_refptr<const CalculationExpressionNode> CalculationExpressionNumberNode::Zoom(double) const
{
...
CalculationExpressionSizingKeywordNode::CalculationExpressionSizingKeywordNode(Keyword keyword)
    : keyword_(keyword)
{
    if (keyword != Keyword::kSize && keyword != Keyword::kAny) {
        if (keyword == Keyword::kAuto) {
            has_auto_ = true;
        } else if (keyword == Keyword::kWebkitFillAvailable || keyword == Keyword::kStretch) {
            has_stretch_ = true;
        } else {
...
float CalculationExpressionSizingKeywordNode::Evaluate(float max_value, const EvaluationInput& input) const
{
    ...
    case Keyword::kContent:
        intrinsic_type = input.calc_size_keyword_behavior == CalcSizeKeywordBehavior::kAsAuto ? Length::Type::kAuto : Length::Type::kContent;
        break;
    case Keyword::kMinContent:
    case Keyword::kWebkitMinContent:
        CHECK_EQ(input.calc_size_keyword_behavior, CalcSizeKeywordBehavior::kAsSpecified);
    ...
    case Keyword::kWebkitFitContent:
        intrinsic_type = input.calc_size_keyword_behavior == CalcSizeKeywordBehavior::kAsAuto ? Length::Type::kAuto : Length::Type::kFitContent;
        break;
    case Keyword::kStretch:
    case Keyword::kWebkitFillAvailable:
        intrinsic_type = input.calc_size_keyword_behavior == CalcSizeKeywordBehavior::kAsAuto ? Length::Type::kAuto : Length::Type::kStretch;
        break;
    }
...
CalculationExpressionColorChannelKeywordNode::CalculationExpressionColorChannelKeywordNode(ColorChannelKeyword channel)
    : channel_(channel)
{
}

float CalculationExpressionColorChannelKeywordNode::Evaluate(float max_value, const EvaluationInput& evaluation_input) const
{
...
float CalculationExpressionPixelsAndPercentNode::Evaluate(float max_value, const EvaluationInput&) const
{
...
{
    auto* other_pixels_and_percent = DynamicTo<CalculationExpressionPixelsAndPercentNode>(other);
    if (!other_pixels_and_percent) {
        return false;
    }
    return value_.pixels == other_pixels_and_percent->value_.pixels && value_.percent == other_pixels_and_percent->value_.percent;
}

scoped_refptr<const CalculationExpressionNode> CalculationExpressionPixelsAndPercentNode::Zoom(double factor) const
{
    PixelsAndPercent result(value_.pixels * factor, value_.percent, value_.has_explicit_pixels, value_.has_explicit_percent);
    return base::MakeRefCounted<CalculationExpressionPixelsAndPercentNode>(result);
}
...
// static
scoped_refptr<const CalculationExpressionNode> CalculationExpressionOperationNode::CreateSimplified(Children&& children, CalculationOperator op)
{
    switch (op) {
    ...
    case CalculationOperator::kSubtract: {
        ...
        if (!children[0]->IsPixelsAndPercent() || !children[1]->IsPixelsAndPercent()) {
            return base::MakeRefCounted<CalculationExpressionOperationNode>(Children({ std::move(children[0]), std::move(children[1]) }), op);
        }
        const auto& left_pixels_and_percent = To<CalculationExpressionPixelsAndPercentNode>(*children[0]);
        PixelsAndPercent right_pixels_and_percent = To<CalculationExpressionPixelsAndPercentNode>(*children[1]).GetPixelsAndPercent();
    ...
    case CalculationOperator::kMultiply: {
        ...
        PixelsAndPercent pixels_and_percent = To<CalculationExpressionPixelsAndPercentNode>(*maybe_pixels_and_percent_node).GetPixelsAndPercent();
        pixels_and_percent *= number.Value();
        return base::MakeRefCounted<CalculationExpressionPixelsAndPercentNode>(pixels_and_percent);
    }
    case CalculationOperator::kInvert: {
        ...
        if (number) {
            return base::MakeRefCounted<CalculationExpressionNumberNode>(1.0 / number->Value());
        }
        return base::MakeRefCounted<CalculationExpressionOperationNode>(Children({ std::move(children[0]) }), op);
    }
    ...
    case CalculationOperator::kMax: {
        ...
        if (can_simplify) {
            return base::MakeRefCounted<CalculationExpressionPixelsAndPercentNode>(PixelsAndPercent(simplified_px));
        }
        return base::MakeRefCounted<CalculationExpressionOperationNode>(std::move(children), op);
    }
    case CalculationOperator::kClamp: {
        ...
        if (can_simplify) {
            ...
            return base::MakeRefCounted<CalculationExpressionPixelsAndPercentNode>(PixelsAndPercent(clamped_px));
        }
        return base::MakeRefCounted<CalculationExpressionOperationNode>(std::move(children), op);
    }
    ...
    case CalculationOperator::kRem: {
        ...
        if (can_simplify) {
            ...
            return base::MakeRefCounted<CalculationExpressionPixelsAndPercentNode>(PixelsAndPercent(value));
        } else {
            return base::MakeRefCounted<CalculationExpressionOperationNode>(std::move(children), op);
        }
    }
    ...
    case CalculationOperator::kSign: {
        ...
        if (!pixels_and_percent || pixels_and_percent->Percent()) {
            ...
            if (op == CalculationOperator::kAbs) {
                return base::MakeRefCounted<CalculationExpressionPixelsAndPercentNode>(PixelsAndPercent(std::abs(value)));
            } else {
                if (value == 0 || std::isnan(value)) {
                    return base::MakeRefCounted<CalculationExpressionNumberNode>(value);
                }
                return base::MakeRefCounted<CalculationExpressionNumberNode>(value > 0 ? 1 : -1);
            }
        }
    }
...
float CalculationExpressionOperationNode::Evaluate(float max_value, const EvaluationInput& input) const
{
    switch (operator_) {
    case CalculationOperator::kAdd: {
...
bool CalculationExpressionOperationNode::Equals(const CalculationExpressionNode& other) const
{
    ...
    return base::ranges::equal(children_, other_operation->GetChildren(), [](const ValueType& a, const ValueType& b) { return *a == *b; });
}

scoped_refptr<const CalculationExpressionNode> CalculationExpressionOperationNode::Zoom(double factor) const
{
    switch (operator_) {
    case CalculationOperator::kAdd:
    case CalculationOperator::kSubtract:
    ...
    case CalculationOperator::kInvert: {
        DCHECK_EQ(children_.size(), 1u);
        return CreateSimplified(Children({ children_[0]->Zoom(factor) }), operator_);
    }
    case CalculationOperator::kCalcSize: {
        DCHECK_EQ(children_.size(), 2u);
