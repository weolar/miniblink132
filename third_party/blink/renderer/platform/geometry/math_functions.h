...
template <class ValueType> std::pair<ValueType, ValueType> GetNearestMultiples(ValueType a, ValueType b)
{
    using std::swap;
...
template <class OperatorType, typename ValueType> std::optional<ValueType> PreCheckSteppedValueFunctionArguments(OperatorType op, ValueType a, ValueType b)
{
    ...
    if (OperatorType::kRoundNearest <= op && op <= OperatorType::kRoundToZero && (std::fmod(a, b) == 0.0 || (std::isinf(a) && !std::isinf(b)))) {
        return a;
    }
...
template <class OperatorType, typename ValueType> ValueType EvaluateSteppedValueFunction(OperatorType op, ValueType a, ValueType b)
{
