...
public:
    using WebMemoryMeasurementMode = ::performance_manager::mojom::blink::WebMemoryMeasurement::Mode;
    using OnWebMemoryMeasurementRequestedCallback = ::performance_manager::mojom::blink::DocumentCoordinationUnit::OnWebMemoryMeasurementRequestedCallback;

    // Returns nullptr if instrumentation is not enabled.
    static std::unique_ptr<DocumentResourceCoordinator> MaybeCreate(const BrowserInterfaceBrokerProxy&);
    DocumentResourceCoordinator(const DocumentResourceCoordinator&) = delete;
    DocumentResourceCoordinator& operator=(const DocumentResourceCoordinator&) = delete;
    ...
    void OnWebMemoryMeasurementRequested(WebMemoryMeasurementMode mode, OnWebMemoryMeasurementRequestedCallback callback);

private:
    explicit DocumentResourceCoordinator(const BrowserInterfaceBrokerProxy&);

    mojo::Remote<performance_manager::mojom::blink::DocumentCoordinationUnit> service_;
