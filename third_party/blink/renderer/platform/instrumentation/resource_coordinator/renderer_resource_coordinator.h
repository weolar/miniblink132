...
public:
    ...
    RendererResourceCoordinator& operator=(const RendererResourceCoordinator&) = delete;
    ...
    // |execution_context|.
    virtual void OnScriptStateCreated(ScriptState* script_state, ExecutionContext* execution_context) = 0;
    ...
    virtual void OnBeforeContentFrameAttached(const Frame& frame, const HTMLFrameOwnerElement& owner) = 0;
    // Called when |frame| is about to be unset as the ContentFrame of |owner|.
    virtual void OnBeforeContentFrameDetached(const Frame& frame, const HTMLFrameOwnerElement& owner) = 0;
};
