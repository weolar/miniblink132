...
public:
    ...
    CustomCountHistogram(const char* name, base::HistogramBase::Sample min, base::HistogramBase::Sample max, int32_t bucket_count);
...
template <typename Derived> class ScopedUsHistogramTimerBase {
    USING_FAST_MALLOC(ScopedUsHistogramTimerBase);

public:
    explicit ScopedUsHistogramTimerBase(CustomCountHistogram& counter)
        : ScopedUsHistogramTimerBase(counter, base::DefaultTickClock::GetInstance())
    {
    }

    ScopedUsHistogramTimerBase(CustomCountHistogram& counter, const base::TickClock* clock)
        : clock_(*clock)
...
class ScopedUsHistogramTimer : public ScopedUsHistogramTimerBase<ScopedUsHistogramTimer> {
public:
...
class ScopedHighResUsHistogramTimer : public ScopedUsHistogramTimerBase<ScopedHighResUsHistogramTimer> {
public:
    using ScopedUsHistogramTimerBase::ScopedUsHistogramTimerBase;
    static bool ShouldRecord()
    {
...
static constexpr base::HistogramBase::Sample kTimeBasedHistogramMaxSample = static_cast<base::Histogram::Sample>(base::Seconds(10).InMicroseconds());
...
#define SCOPED_BLINK_UMA_HISTOGRAM_TIMER_IMPL(name, allow_cross_thread)                                                                                        \
    DEFINE_STATIC_LOCAL_IMPL(CustomCountHistogram, scoped_us_counter,                                                                                          \
        (name, kTimeBasedHistogramMinSample, kTimeBasedHistogramMaxSample, kTimeBasedHistogramBucketCount), allow_cross_thread);                               \
    ScopedUsHistogramTimer timer(scoped_us_counter);

#define SCOPED_BLINK_UMA_HISTOGRAM_TIMER_HIGHRES_IMPL(name, allow_cross_thread)                                                                                \
    DEFINE_STATIC_LOCAL_IMPL(CustomCountHistogram, scoped_us_counter, (name, 0, 10000000, 50), allow_cross_thread);                                            \
...
#define SCOPED_BLINK_UMA_HISTOGRAM_TIMER(name) SCOPED_BLINK_UMA_HISTOGRAM_TIMER_IMPL(name, false)

// Only record samples when we have a high resolution timer
#define SCOPED_BLINK_UMA_HISTOGRAM_TIMER_HIGHRES(name) SCOPED_BLINK_UMA_HISTOGRAM_TIMER_HIGHRES_IMPL(name, false)

// Thread-safe variant of SCOPED_BLINK_UMA_HISTOGRAM_TIMER.
// Use if the histogram can be accessed by multiple threads.
#define SCOPED_BLINK_UMA_HISTOGRAM_TIMER_THREAD_SAFE(name) SCOPED_BLINK_UMA_HISTOGRAM_TIMER_IMPL(name, true)
