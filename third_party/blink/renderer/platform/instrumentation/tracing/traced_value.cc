...
void TracedValue::SetStringWithCopiedName(const char* name, const String& value)
{
    StringUTF8Adaptor adaptor(value);
...
{
    return String(static_cast<base::trace_event::TracedValueJSON*>(traced_value_.get())->ToJSON().c_str());
}

String TracedValueJSON::ToFormattedJSON() const
{
    return String(static_cast<base::trace_event::TracedValueJSON*>(traced_value_.get())->ToFormattedJSON().c_str());
}
