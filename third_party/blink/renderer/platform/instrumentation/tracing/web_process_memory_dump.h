...
public:
    ...
    explicit WebProcessMemoryDump(base::trace_event::MemoryDumpLevelOfDetail level_of_detail, base::trace_event::ProcessMemoryDump* process_memory_dump);
    ...
    blink::WebMemoryAllocatorDump* CreateMemoryAllocatorDump(const String& absolute_name);
    blink::WebMemoryAllocatorDump* CreateMemoryAllocatorDump(const String& absolute_name, blink::WebMemoryAllocatorDumpGuid guid);

    // Gets a previously created MemoryAllocatorDump given its name.
    blink::WebMemoryAllocatorDump* GetMemoryAllocatorDump(const String& absolute_name) const;
    ...
    // memory.
    void AddOwnershipEdge(blink::WebMemoryAllocatorDumpGuid source, blink::WebMemoryAllocatorDumpGuid target, int importance);
    void AddOwnershipEdge(blink::WebMemoryAllocatorDumpGuid source, blink::WebMemoryAllocatorDumpGuid target);
    ...
    void AddSuballocation(blink::WebMemoryAllocatorDumpGuid source, const String& target_node_name);
    ...
    blink::WebMemoryAllocatorDump* CreateDiscardableMemoryAllocatorDump(const std::string& name, base::DiscardableMemory* discardable);
...
private:
    ...
    blink::WebMemoryAllocatorDump* CreateWebMemoryAllocatorDump(base::trace_event::MemoryAllocatorDump* memory_allocator_dump);
    ...
    raw_ptr<base::trace_event::ProcessMemoryDump> process_memory_dump_; // Not owned.
    ...
    HashMap<base::trace_event::MemoryAllocatorDump*, std::unique_ptr<WebMemoryAllocatorDump>> memory_allocator_dumps_;
