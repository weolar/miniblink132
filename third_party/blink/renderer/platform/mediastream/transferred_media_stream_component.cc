...
TransferredMediaStreamComponent::TransferredMediaStreamComponent(const TransferredValues& data)
    : data_(data)
{
}

void TransferredMediaStreamComponent::SetImplementation(MediaStreamComponent* component)
{
    MediaStreamTrackPlatform::CaptureHandle old_capture_handle = GetCaptureHandle();
    ...
    // now set.
    bool capture_handle_changed = old_capture_handle.origin != GetCaptureHandle().origin || old_capture_handle.handle != GetCaptureHandle().handle;
    ...
    for (const auto& call : add_video_sink_calls_) {
        component_->AddSink(call.sink, call.callback, call.is_secure, call.uses_alpha);
    }
...
MediaStreamSource::StreamType TransferredMediaStreamComponent::GetSourceType() const
{
    if (component_) {
...
MediaStreamSource::ReadyState TransferredMediaStreamComponent::GetReadyState() const
{
    if (component_) {
...
WebMediaStreamTrack::ContentHintType TransferredMediaStreamComponent::ContentHint()
{
    if (component_) {
...
void TransferredMediaStreamComponent::SetContentHint(WebMediaStreamTrack::ContentHintType hint)
{
    if (component_) {
...
MediaStreamTrackPlatform* TransferredMediaStreamComponent::GetPlatformTrack() const
{
    if (component_) {
...
void TransferredMediaStreamComponent::GetSettings(MediaStreamTrackPlatform::Settings& settings)
{
    if (component_) {
...
MediaStreamTrackPlatform::CaptureHandle TransferredMediaStreamComponent::GetCaptureHandle()
{
    if (component_) {
...
void TransferredMediaStreamComponent::SetCreationFrameGetter(base::RepeatingCallback<WebLocalFrame*()> creation_frame_getter)
{
    if (component_) {
...
void TransferredMediaStreamComponent::AddSourceObserver(MediaStreamSource::Observer* observer)
{
    if (component_) {
    ...
    WebMediaStreamSink* sink, const VideoCaptureDeliverFrameCB& callback, MediaStreamVideoSink::IsSecure is_secure, MediaStreamVideoSink::UsesAlpha uses_alpha)
{
    ...
    add_video_sink_calls_.emplace_back(AddSinkArgs { sink, std::move(callback), is_secure, uses_alpha });
}
