...
template <typename Consumer> class MediaStreamAudioDeliverer {
public:
    MediaStreamAudioDeliverer()
    {
    }
    MediaStreamAudioDeliverer(const MediaStreamAudioDeliverer&) = delete;
    MediaStreamAudioDeliverer& operator=(const MediaStreamAudioDeliverer&) = delete;
    ~MediaStreamAudioDeliverer()
    {
    }
    ...
    void AddConsumer(Consumer* consumer)
    {
        ...
        SendLogMessage(String::Format("%s => (number of consumer: active=%u, pending=%u)", __func__, consumers_.size(), pending_consumers_.size()));
    }
    ...
    bool RemoveConsumer(Consumer* consumer)
    {
        ...
        SendLogMessage(String::Format("%s => (number of consumers: active=%u, pending=%u)", __func__, consumers_.size(), pending_consumers_.size()));
    ...
    void GetConsumerList(Vector<Consumer*>* consumer_list) const
    {
        ...
        *consumer_list = consumers_;
        consumer_list->AppendRange(pending_consumers_.begin(), pending_consumers_.end());
    }
    ...
    void OnSetFormat(const media::AudioParameters& params)
    {
        ...
        base::AutoLock auto_lock(consumers_lock_);
        {
            ...
            if (params_.Equals(params))
                return;
            SendLogMessage(String::Format("%s({params=[%s]})", __func__, params.AsHumanReadableString().c_str()));
            params_ = params;
        }
    ...
    void OnData(const media::AudioBus& audio_bus, base::TimeTicks reference_time, const media::AudioGlitchInfo& glitch_info)
    {
        TRACE_EVENT("audio", "MediaStreamAudioDeliverer::OnData", "reference_time (ms)", (reference_time - base::TimeTicks()).InMillisecondsF(),
            "layover_delay (ms)", (base::TimeTicks::Now() - reference_time).InMillisecondsF());
        base::AutoLock auto_lock(consumers_lock_);
        ...
        if (!pending_consumers_.empty()) {
            ...
            consumers_.AppendRange(pending_consumers_.begin(), pending_consumers_.end());
            pending_consumers_.clear();
            SendLogMessage(String::Format("%s => (number of active consumers=%u)", __func__, consumers_.size()));
        }
    ...
    int NumPreferredChannels() const
    {
        ...
        for (Consumer* consumer : consumers_) {
            num_preferred_channels = std::max(num_preferred_channels, consumer->NumPreferredChannels());
        }
...
private:
    void SendLogMessage(const WTF::String& message)
    {
        WebRtcLogMessage(String::Format(
            //"MSAD::%s [this=0x%" PRIXPTR "]",
            "MSAD::%s [this=0x%X]", message.Utf8().c_str(), reinterpret_cast<uintptr_t>(this))
                             .Utf8());
    }
