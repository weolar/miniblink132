...
struct PLATFORM_EXPORT AudioProcessingProperties {
    ...
    bool HasSameReconfigurableSettings(const AudioProcessingProperties& other) const;

    bool HasSameNonReconfigurableSettings(const AudioProcessingProperties& other) const;
    ...
    media::AudioProcessingSettings ToAudioProcessingSettings(bool multi_channel_capture_processing) const;

    EchoCancellationType echo_cancellation_type = EchoCancellationType::kEchoCancellationAec3;
    ...
    VoiceIsolationType voice_isolation = VoiceIsolationType::kVoiceIsolationDefault;
};
} // namespace blink
