...
public:
    ...
    AudioServiceAudioProcessorProxy(const AudioServiceAudioProcessorProxy&) = delete;
    AudioServiceAudioProcessorProxy& operator=(const AudioServiceAudioProcessorProxy&) = delete;
...
private:
    ...
    raw_ptr<media::AudioProcessorControls> processor_controls_ GUARDED_BY_CONTEXT(main_thread_checker_) = nullptr;

    base::Lock stats_lock_;
    AudioProcessorStatistics latest_stats_ GUARDED_BY(stats_lock_);

    base::RepeatingTimer stats_update_timer_ GUARDED_BY_CONTEXT(main_thread_checker_);
