...
{
    UMA_HISTOGRAM_ENUMERATION("WebRTC.UserMediaRequest.Result2", result, mojom::blink::MediaStreamRequestResult::NUM_MEDIA_REQUEST_RESULTS);
}

void UpdateWebRTCMethodCount(RTCAPIName api_name)
{
    DVLOG(3) << "Incrementing WebRTC.webkitApiCount for " << static_cast<int>(api_name);
    UMA_HISTOGRAM_ENUMERATION("WebRTC.webkitApiCount", api_name, RTCAPIName::kInvalidName);
...
{
    DVLOG(3) << "Incrementing WebRTC.webkitApiCountPerSession for " << static_cast<int>(api_name);
    UMA_HISTOGRAM_ENUMERATION("WebRTC.webkitApiCountPerSession", api_name, RTCAPIName::kInvalidName);
}
