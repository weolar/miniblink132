...
MediaStreamAudioTrack::MediaStreamAudioTrack(bool is_local_track)
    : MediaStreamTrackPlatform(is_local_track)
    , is_enabled_(1)
{
    WebRtcLog(kTag, this, "%s({is_local_track=%s})", __func__, (is_local_track ? "true" : "false"));
}
...
std::unique_ptr<MediaStreamTrackPlatform> MediaStreamAudioTrack::CreateFromComponent(const MediaStreamComponent* component, const String& id)
{
    MediaStreamSource* source = component->Source();
    CHECK_EQ(source->GetType(), MediaStreamSource::kTypeAudio);
    return MediaStreamAudioSource::From(source)->CreateMediaStreamAudioTrack(id.Utf8());
}

// static
MediaStreamAudioTrack* MediaStreamAudioTrack::From(const MediaStreamComponent* component)
{
    if (!component || component->GetSourceType() != MediaStreamSource::kTypeAudio) {
        return nullptr;
    }
...
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    WebRtcLog(kTag, this, "%s({enabled=%s})", __func__, (enabled ? "true" : "false"));

    const bool previously_enabled = is_enabled_.exchange(enabled, std::memory_order_relaxed);
...
void MediaStreamAudioTrack::SetContentHint(WebMediaStreamTrack::ContentHintType content_hint)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
{
    WebRtcLog(kTag, this, "%s({params: [%s]})", __func__, params.AsHumanReadableString().c_str());
    deliverer_.OnSetFormat(params);
}

void MediaStreamAudioTrack::OnData(const media::AudioBus& audio_bus, base::TimeTicks reference_time, const media::AudioGlitchInfo& glitch_info)
{
    TRACE_EVENT2(TRACE_DISABLED_BY_DEFAULT("mediastream"), "MediaStreamAudioTrack::OnData", "this", static_cast<void*>(this), "frame", audio_bus.frames());
    ...
    if (deliver_data) {
        ...
        if (!silent_bus_ || silent_bus_->channels() != audio_bus.channels() || silent_bus_->frames() != audio_bus.frames()) {
            silent_bus_ = media::AudioBus::Create(audio_bus.channels(), audio_bus.frames());
            silent_bus_->Zero();
        }
...
void MediaStreamAudioTrack::TransferAudioFrameStatsTo(MediaStreamTrackPlatform::AudioFrameStats& destination)
{
    base::AutoLock auto_lock(mainthread_frame_stats_lock_);
    destination.Absorb(mainthread_frame_stats_);
}

void MediaStreamAudioTrack::UpdateFrameStats(const media::AudioBus& audio_bus, base::TimeTicks reference_time, const media::AudioGlitchInfo& glitch_info)
{
