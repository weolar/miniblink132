...
namespace {
void VerifyStats(const media::AudioProcessingStats& expected, scoped_refptr<AudioServiceAudioProcessorProxy> proxy)
{
    webrtc::AudioProcessorInterface::AudioProcessorStatistics received = proxy->GetStats(false);
    EXPECT_FALSE(received.typing_noise_detected);
    EXPECT_EQ(received.apm_statistics.echo_return_loss, expected.echo_return_loss);
    EXPECT_EQ(received.apm_statistics.echo_return_loss_enhancement, expected.echo_return_loss_enhancement);
...
void VerifyStatsFromAnotherThread(const media::AudioProcessingStats& expected, scoped_refptr<AudioServiceAudioProcessorProxy> proxy)
{
    base::RunLoop run_loop;
    base::ThreadPool::PostTaskAndReply(FROM_HERE, {}, base::BindOnce(&VerifyStats, expected, proxy), run_loop.QuitClosure());
    run_loop.Run();
}

void MaybeSetNumChannelsOnAnotherThread(scoped_refptr<AudioServiceAudioProcessorProxy> proxy, uint32_t num_channels)
{
    base::RunLoop run_loop;
    base::ThreadPool::PostTaskAndReply(
        FROM_HERE, {}, base::BindOnce(&AudioServiceAudioProcessorProxy::MaybeUpdateNumPreferredCaptureChannels, proxy, num_channels), run_loop.QuitClosure());
    run_loop.Run();
}
...
protected:
    ...
    {
        task_environment_.FastForwardBy(AudioServiceAudioProcessorProxy::kStatsUpdateInterval + base::Seconds(1));
    }
...
{
    scoped_refptr<AudioServiceAudioProcessorProxy> proxy = new rtc::RefCountedObject<AudioServiceAudioProcessorProxy>();
...
TEST_F(AudioServiceAudioProcessorProxyTest, DoesNotSetNumChannelsIfDoesNotChange)
{
    scoped_refptr<AudioServiceAudioProcessorProxy> proxy = new rtc::RefCountedObject<AudioServiceAudioProcessorProxy>();
    StrictMock<MockAudioProcessorControls> controls;
