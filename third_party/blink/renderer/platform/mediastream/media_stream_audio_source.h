...
//     LOG(ERROR) << "Failed!";
//   }
class PLATFORM_EXPORT MediaStreamAudioSource : public WebPlatformMediaStreamSource {
public:
    MediaStreamAudioSource(scoped_refptr<base::SingleThreadTaskRunner> task_runner, bool is_local_source);
    MediaStreamAudioSource(scoped_refptr<base::SingleThreadTaskRunner> task_runner, bool is_local_source, bool disable_local_echo);
    ...
    bool HasSameReconfigurableSettings(const blink::AudioProcessingProperties& selected_properties) const;
    ...
    bool HasSameNonReconfigurableSettings(MediaStreamAudioSource* other_source) const;

    // Returns the audio processing properties associated to this source if any,
    // or nullopt otherwise.
    virtual std::optional<blink::AudioProcessingProperties> GetAudioProcessingProperties() const
    {
        return std::nullopt;
    }
    ...
    virtual std::unique_ptr<MediaStreamAudioTrack> CreateMediaStreamAudioTrack(const std::string& id);
...
protected:
    ...
    void DeliverDataToTracks(const media::AudioBus& audio_bus, base::TimeTicks reference_time, const media::AudioGlitchInfo& glitch_info);
    ...
    void StopSourceOnError(media::AudioCapturerSource::ErrorCode code, const std::string& why);
...
private:
    ...
    void StopSourceOnErrorOnTaskRunner(media::AudioCapturerSource::ErrorCode code);
