...
public:
    ...
    struct Settings {
        ...
        {
            return aspect_ratio >= 0.0;
        }
        bool HasFacingMode() const
        {
            return facing_mode != FacingMode::kNone;
        }
    ...
    public:
        ...
        void Update(const media::AudioParameters& params, base::TimeTicks capture_time, const media::AudioGlitchInfo& glitch_info);
    ...
    struct CaptureHandle {
        bool IsEmpty() const
        {
            return origin.empty() && handle.empty();
        }
    ...
    // achieve this.
    virtual std::unique_ptr<MediaStreamTrackPlatform> CreateFromComponent(const MediaStreamComponent* component, const String& id) = 0;

    static MediaStreamTrackPlatform* GetTrack(const WebMediaStreamTrack& track);

    virtual void SetEnabled(bool enabled) = 0;

    virtual void SetContentHint(WebMediaStreamTrack::ContentHintType content_hint) = 0;
    ...
    void Stop()
    {
        StopAndNotify(base::OnceClosure());
    }
    ...
    virtual void AddSubCaptureTargetVersionCallback(uint32_t sub_capture_target_version, base::OnceClosure callback)
    {
    }

    // Removes the callback that was associated with this
    // |sub_capture_target_version|, if any.
    virtual void RemoveSubCaptureTargetVersionCallback(uint32_t sub_capture_target_version)
    {
    }

    bool is_local_track() const
    {
        return is_local_track_;
    }
    ...
    virtual void AddSink(WebMediaStreamSink* sink, const VideoCaptureDeliverFrameCB& callback, MediaStreamVideoSink::IsSecure is_secure,
