...
WebAudioMediaStreamSource::WebAudioMediaStreamSource(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : MediaStreamAudioSource(std::move(task_runner), false /* is_remote */)
    , is_registered_consumer_(false)
    , fifo_(ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&WebAudioMediaStreamSource::DeliverRebufferedAudio, WTF::CrossThreadUnretained(this))))
{
...
void WebAudioMediaStreamSource::SetFormat(int number_of_channels, float sample_rate)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    VLOG(1) << "WebAudio media stream source changed format to: channels=" << number_of_channels << ", sample_rate=" << sample_rate;
    ...
    media::ChannelLayout channel_layout = number_of_channels > 8 ? media::CHANNEL_LAYOUT_DISCRETE : media::GuessChannelLayout(number_of_channels);
    ...
    media::AudioParameters params(
        media::AudioParameters::AUDIO_PCM_LOW_LATENCY, { channel_layout, number_of_channels }, sample_rate, fifo_.frames_per_buffer());
...
void WebAudioMediaStreamSource::EnsureSourceIsStopped()
{
    ...
    VLOG(1) << "Stopped WebAudio media stream source. Final audio parameters={" << GetAudioParameters().AsHumanReadableString() << "}.";
}

void WebAudioMediaStreamSource::ConsumeAudio(const Vector<const float*>& audio_data, int number_of_frames)
{
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("mediastream"), "WebAudioMediaStreamSource::ConsumeAudio", "frames", number_of_frames);
    ...
    for (wtf_size_t i = 0; i < audio_data.size(); ++i) {
        wrapper_bus_->SetChannelData(static_cast<int>(i), const_cast<float*>(audio_data[i]));
    }
...
void WebAudioMediaStreamSource::DeliverRebufferedAudio(const media::AudioBus& audio_bus, int frame_delay)
{
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("mediastream"), "WebAudioMediaStreamSource::DeliverRebufferedAudio", "frames", audio_bus.frames());
    const base::TimeTicks reference_time = current_reference_time_
        + base::Microseconds(frame_delay * base::Time::kMicrosecondsPerSecond / MediaStreamAudioSource::GetAudioParameters().sample_rate());
