...
class MockSourceObserver : public GarbageCollected<MockSourceObserver>, public MediaStreamSource::Observer {
public:
...
public:
    void SetUp() override
    {
        transferred_component_ = MakeGarbageCollected<TransferredMediaStreamComponent>(TransferredMediaStreamComponent::TransferredValues { .id = "id" });

        component_ = MakeGarbageCollected<MockMediaStreamComponent>();
        EXPECT_CALL(*component_, GetCaptureHandle()).WillRepeatedly(Return(MediaStreamTrackPlatform::CaptureHandle()));
        EXPECT_CALL(*component_, GetReadyState()).WillRepeatedly(Return(MediaStreamSource::kReadyStateLive));
    }
...
TEST_F(TransferredMediaStreamComponentTest, ObserverStateChangeFiresWhenSettingImplementation)
{
    MockSourceObserver* observer = MakeGarbageCollected<MockSourceObserver>();

    transferred_component_->AddSourceObserver(observer);
    ASSERT_EQ(transferred_component_->GetReadyState(), MediaStreamSource::kReadyStateLive);

    EXPECT_CALL(*component_, GetReadyState()).WillRepeatedly(Return(MediaStreamSource::kReadyStateMuted));
...
TEST_F(TransferredMediaStreamComponentTest, ObserverCaptureHandleChangeFiresWhenSettingImplementation)
{
    MockSourceObserver* observer = MakeGarbageCollected<MockSourceObserver>();
    ...
    EXPECT_CALL(*component_, GetCaptureHandle()).WillRepeatedly(Return(MediaStreamTrackPlatform::CaptureHandle { .handle = "handle" }));
    EXPECT_CALL(*component_, AddSourceObserver(observer));
