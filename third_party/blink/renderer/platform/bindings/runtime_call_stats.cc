...
void RuntimeCallTimer::Start(RuntimeCallCounter* counter, RuntimeCallTimer* parent)
{
    DCHECK(!IsRunning());
    counter_ = counter;
    parent_ = parent;
...
String RuntimeCallStats::ToString() const
{
    ...
    builder.Append("Name                                                    Count     Time "
                   "(ms)\n\n");
    for (int i = 0; i < number_of_counters_; i++) {
        const RuntimeCallCounter* counter = &counters_[i];
        builder.AppendFormat(row_format, counter->GetName(), counter->GetCount(), counter->GetTime().InMillisecondsF());
    }
...
{
    DEFINE_STATIC_LOCAL(RuntimeCallStats, s_rcs_for_testing, (base::DefaultTickClock::GetInstance()));
    g_runtime_call_stats_for_testing = static_cast<RuntimeCallStats*>(&s_rcs_for_testing);
}
...
RuntimeCallCounter* RuntimeCallStats::GetCounter(const char* name)
{
    ...
    if (it != counter_map_.end())
        return it->value.get();
    return counter_map_.insert(name, std::make_unique<RuntimeCallCounter>(name)).stored_value->value.get();
}
...
Vector<RuntimeCallCounter*> RuntimeCallStats::CounterMapToSortedArray() const
{
    ...
    auto comparator = [](RuntimeCallCounter* a, RuntimeCallCounter* b) {
        return a->GetCount() == b->GetCount() ? strcmp(a->GetName(), b->GetName()) < 0 : a->GetCount() < b->GetCount();
    };
    std::sort(counters.begin(), counters.end(), comparator);
    return counters;
}

void RuntimeCallStats::AddCounterMapStatsToBuilder(StringBuilder& builder) const
{
...
constexpr const char* RuntimeCallStatsScopedTracer::s_category_group_ = TRACE_DISABLED_BY_DEFAULT("v8.runtime_stats");
constexpr const char* RuntimeCallStatsScopedTracer::s_name_ = "BlinkRuntimeCallStats";

void RuntimeCallStatsScopedTracer::AddBeginTraceEventIfEnabled(v8::Isolate* isolate)
{
    bool category_group_enabled;
    TRACE_EVENT_CATEGORY_GROUP_ENABLED(s_category_group_, &category_group_enabled);
...
void RuntimeCallStatsScopedTracer::AddEndTraceEvent()
{
    ...
    TRACE_EVENT_END1(s_category_group_, s_name_, "runtime-call-stats", std::move(value));
}
