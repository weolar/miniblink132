...
public:
    ObservableArrayBase(GarbageCollectedMixin* platform_object, ObservableArrayExoticObject* observable_array_exotic_object);
...
protected:
    GarbageCollectedMixin* GetPlatformObject()
    {
        return platform_object_.Get();
    }

    virtual v8::Local<v8::FunctionTemplate> GetProxyHandlerFunctionTemplate(ScriptState* script_state) = 0;
...
template <typename ElementType> class ObservableArrayImplHelper : public bindings::ObservableArrayBase {
public:
    ...
    using const_reverse_iterator = typename BackingListType::const_reverse_iterator;

    explicit ObservableArrayImplHelper(GarbageCollectedMixin* platform_object)
        : bindings::ObservableArrayBase(platform_object, MakeGarbageCollected<ObservableArrayExoticObject>(this))
    {
    }
    ...
    {
        return backing_list_.size();
    }
    size_type capacity() const
    {
        return backing_list_.capacity();
    }
    bool empty() const
    {
        return backing_list_.empty();
    }
    ...
    iterator begin()
    {
        return backing_list_.begin();
    }
    iterator end()
    {
        return backing_list_.end();
    }
    ...
    {
        return backing_list_.rbegin();
    }
    reverse_iterator rend()
    {
        return backing_list_.rend();
    }
    ...
    reference front()
    {
        return backing_list_.front();
    }
    reference back()
    {
        return backing_list_.back();
    }
...
// https://webidl.spec.whatwg.org/#backing-observable-array-exotic-object
class PLATFORM_EXPORT ObservableArrayExoticObject final : public ScriptWrappable {
    DEFINE_WRAPPERTYPEINFO();

public:
    // Returns the backing list object extracted from the proxy target object
    // of type JS Array.
    template <typename T> static T* ProxyTargetToObservableArray(v8::Isolate* isolate, v8::Local<v8::Array> v8_proxy_target)
    {
        return ToScriptWrappable<T>(isolate, GetBackingObjectFromProxyTarget(isolate, v8_proxy_target));
    }

    explicit ObservableArrayExoticObject(bindings::ObservableArrayBase* observable_array_backing_list_object);
    ...
    [[nodiscard]] v8::Local<v8::Object> AssociateWithWrapper(
        v8::Isolate* isolate, const WrapperTypeInfo* wrapper_type_info, v8::Local<v8::Object> wrapper) final;

    void Trace(Visitor* visitor) const final;

private:
    static v8::Local<v8::Object> GetBackingObjectFromProxyTarget(v8::Isolate*, v8::Local<v8::Array> v8_proxy_target);
