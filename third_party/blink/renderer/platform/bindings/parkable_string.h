...
class PLATFORM_EXPORT ParkableStringImpl : public WTF::ThreadSafeRefCounted<ParkableStringImpl> {
public:
    enum class ParkingMode { kSynchronousOnly, kCompress, kToDisk };
    enum class AgeOrParkResult { kSuccessOrTransientFailure, kNonTransientFailure };
    ...
    static scoped_refptr<ParkableStringImpl> MakeNonParkable(scoped_refptr<StringImpl>&& impl);
    // |digest| is as returned by |HashString()|, hence not nullptr.
    static scoped_refptr<ParkableStringImpl> MakeParkable(scoped_refptr<StringImpl>&& impl, std::unique_ptr<SecureDigest> digest);
    ...
    {
        return !!metadata_->compressed_;
    }
    bool has_on_disk_data() const
    {
        return !!metadata_->on_disk_metadata_;
    }
...
private:
    ...
    ParkableStringImpl(scoped_refptr<StringImpl>&& impl, std::unique_ptr<SecureDigest> digest);
    ...
    bool ParkInternal(ParkingMode mode) EXCLUSIVE_LOCKS_REQUIRED(metadata_->lock_);
    ...
    void PostBackgroundWritingTask(std::unique_ptr<ReservedChunk> reserved_chunk) EXCLUSIVE_LOCKS_REQUIRED(metadata_->lock_);
    static void WriteToDiskInBackground(std::unique_ptr<BackgroundTaskParams>, DiskDataAllocator* data_allocator);
    ...
    void OnWritingCompleteOnMainThread(std::unique_ptr<BackgroundTaskParams> params, std::unique_ptr<DiskDataMetadata> metadata, base::TimeDelta writing_time);
    ...
    bool is_compression_failed_no_lock() const EXCLUSIVE_LOCKS_REQUIRED(metadata_->lock_);
...
public:
    ...
    ParkableString(scoped_refptr<StringImpl>&& impl, std::unique_ptr<ParkableStringImpl::SecureDigest> digest);
    ParkableString(const ParkableString& rhs)
        : impl_(rhs.impl_)
    {
    }
    ~ParkableString();
    ...
    bool may_be_parked() const
    {
        return impl_ && impl_->may_be_parked();
    }
...
static_assert(sizeof(ParkableString) == sizeof(void*), "ParkableString should be small");
