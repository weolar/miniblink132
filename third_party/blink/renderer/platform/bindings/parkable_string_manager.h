...
class PLATFORM_EXPORT ParkableStringManagerDumpProvider : public base::trace_event::MemoryDumpProvider {
    USING_FAST_MALLOC(ParkableStringManagerDumpProvider);

public:
    static ParkableStringManagerDumpProvider* Instance();
    ParkableStringManagerDumpProvider(const ParkableStringManagerDumpProvider&) = delete;
    ParkableStringManagerDumpProvider& operator=(const ParkableStringManagerDumpProvider&) = delete;
...
public:
    ...
    struct SecureDigestHashTraits : GenericHashTraits<const ParkableStringImpl::SecureDigest*> {
        ...
        {
            return a == b || std::equal(a->data(), a->data() + ParkableStringImpl::kDigestSize, b->data());
        }
    ...
    using StringMap = WTF::HashMap<const ParkableStringImpl::SecureDigest*, ParkableStringImpl*, SecureDigestHashTraits>;
...
private:
    ...
    scoped_refptr<ParkableStringImpl> Add(scoped_refptr<StringImpl>&&, std::unique_ptr<ParkableStringImpl::SecureDigest> digest);
    ...
    void CompleteUnpark(ParkableStringImpl* string, base::TimeDelta elapsed, base::TimeDelta disk_elapsed);

    void CompleteUnparkOnMainThread(ParkableStringImpl* string, base::TimeDelta elapsed, base::TimeDelta disk_elapsed);
    ...
    void SetDataAllocatorForTesting(std::unique_ptr<DiskDataAllocator> allocator)
    {
        allocator_for_testing_ = std::move(allocator);
    }

    void SetTaskRunnerForTesting(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    {
    ...
    constexpr static base::TimeDelta kLessAggressiveAgingInterval = base::Seconds(10);
