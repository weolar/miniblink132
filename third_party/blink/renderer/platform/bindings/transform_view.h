...
template <typename Range, typename Transform>
    requires(std::forward_iterator<decltype(std::begin(std::declval<const Range&>()))>
        && std::regular_invocable<Transform, decltype(*std::begin(std::declval<const Range&>()))>)
...
public:
    ...
    public:
        ...
        using value_type = decltype(std::declval<Transform>()(std::declval<const inner_value_t&>()));

        TransformingIterator() = default;
        explicit TransformingIterator(inner_iterator it)
            : it_(it)
        {
        }
        ...
        {
            return TransformingIterator(it_++);
        }
...
template <typename TransformFunc, typename Range> auto Transform(const Range& range)
{
