...
public:
    explicit RuntimeCallTimer(const base::TickClock* clock)
        : clock_(clock)
    {
    }
    ~RuntimeCallTimer()
...
#define RUNTIME_CALL_STATS_ENTER_WITH_RCS(runtime_call_stats, timer, counterId)                                                                                \
...
#define RUNTIME_CALL_TIMER_SCOPE_WITH_OPTIONAL_RCS(optional_scope_name, runtime_call_stats, counterId)                                                         \
    if (RuntimeCallStats::IsEnabled()) [[unlikely]] {                                                                                                          \
...
#define RUNTIME_CALL_STATS_ENTER(isolate, timer, counterId) RUNTIME_CALL_STATS_ENTER_WITH_RCS(RuntimeCallStats::From(isolate), timer, counterId)

#define RUNTIME_CALL_STATS_LEAVE(isolate, timer) RUNTIME_CALL_STATS_LEAVE_WITH_RCS(RuntimeCallStats::From(isolate), timer)

#define RUNTIME_CALL_TIMER_SCOPE(isolate, counterId) RUNTIME_CALL_TIMER_SCOPE_WITH_RCS(RuntimeCallStats::From(isolate), counterId)

#define RUNTIME_CALL_TIMER_SCOPE_IF_ISOLATE_EXISTS(isolate, counterId)                                                                                         \
    std::optional<RuntimeCallTimerScope> rcs_scope;                                                                                                            \
    if (isolate) {                                                                                                                                             \
        RUNTIME_CALL_TIMER_SCOPE_WITH_OPTIONAL_RCS(rcs_scope, RuntimeCallStats::From(isolate), counterId)                                                      \
    }
...
// not set).
#if BUILDFLAG(RCS_COUNT_EVERYTHING)
#define RUNTIME_CALL_TIMER_SCOPE_DISABLED_BY_DEFAULT(isolate, counterName) RUNTIME_CALL_TIMER_SCOPE(isolate, counterName)
#else
...
#if BUILDFLAG(BLINK_BINDINGS_TRACE_ENABLED)
#define BLINK_BINDINGS_TRACE_EVENT(trace_event_name) TRACE_EVENT0("blink.bindings", trace_event_name)
#else
...
#undef ADD_ENUM_VALUE
    ...
    const base::TickClock* clock() const
    {
        return clock_;
    }
...
private:
    ...
    static const int number_of_counters_ = static_cast<int>(CounterId::kNumberOfCounters);
    raw_ptr<const base::TickClock> clock_ = nullptr;
...
public:
    RuntimeCallTimerScope(RuntimeCallStats* stats, RuntimeCallStats::CounterId counter)
        : call_stats_(stats)
        , timer_(stats->clock())
    {
