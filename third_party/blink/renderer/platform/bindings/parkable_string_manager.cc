...
class OnPurgeMemoryListener : public GarbageCollected<OnPurgeMemoryListener>, public MemoryPressureListener {
...
Vector<ParkableStringImpl*> EnumerateStrings(const ParkableStringManager::StringMap& strings)
{
...
void MoveString(ParkableStringImpl* string, ParkableStringManager::StringMap* from, ParkableStringManager::StringMap* to)
{
...
// static
ParkableStringManagerDumpProvider* ParkableStringManagerDumpProvider::Instance()
{
    DEFINE_STATIC_LOCAL(ParkableStringManagerDumpProvider, instance, ());
    return &instance;
}

bool ParkableStringManagerDumpProvider::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    return ParkableStringManager::Instance().OnMemoryDump(pmd);
}

ParkableStringManagerDumpProvider::~ParkableStringManagerDumpProvider() = default;
ParkableStringManagerDumpProvider::ParkableStringManagerDumpProvider() = default;
...
bool ParkableStringManager::OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd)
{
    DCHECK(IsMainThread());
    base::trace_event::MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(kAllocatorDumpName);
    ...
    // Has to be uint64_t.
    dump->AddScalar("savings_size", "bytes", stats.savings_size > 0 ? stats.savings_size : 0);
    dump->AddScalar("on_disk_size", "bytes", stats.on_disk_size);
    dump->AddScalar("on_disk_footprint", "bytes", data_allocator().disk_footprint());
    dump->AddScalar("on_disk_free_chunks", "bytes", data_allocator().free_chunks_size());
...
bool ParkableStringManager::ShouldPark(const StringImpl& string)
{
    ...
    return string.length() > kSizeThreshold && IsMainThread() && CompressionEnabled();
}

// static
base::TimeDelta ParkableStringManager::AgingInterval()
{
    return base::FeatureList::IsEnabled(features::kLessAggressiveParkableString) ? kLessAggressiveAgingInterval : kAgingInterval;
}

scoped_refptr<ParkableStringImpl> ParkableStringManager::Add(scoped_refptr<StringImpl>&& string, std::unique_ptr<ParkableStringImpl::SecureDigest> digest)
{
    DCHECK(IsMainThread());
    ...
    if (!digest) {
        ...
        std::unique_ptr<ParkableStringImpl::SecureDigest> expected_digest = ParkableStringImpl::HashString(string_impl.get());
    ...
    auto new_parkable = ParkableStringImpl::MakeParkable(std::move(string_impl), std::move(digest));
    auto insert_result = unparked_strings_.insert(new_parkable->digest(), new_parkable.get());
    DCHECK(insert_result.is_new_entry);
    ...
    if (!did_register_memory_pressure_listener_) {
        ...
        // lives forever.
        MemoryPressureListenerRegistry::Instance().RegisterClient(MakeGarbageCollected<OnPurgeMemoryListener>());
        did_register_memory_pressure_listener_ = true;
    }

    if (!has_posted_unparking_time_accounting_task_) {
        task_runner_->PostDelayedTask(
            FROM_HERE, base::BindOnce(&ParkableStringManager::RecordStatisticsAfter5Minutes, base::Unretained(this)), base::Minutes(5));
...
void ParkableStringManager::Remove(ParkableStringImpl* string)
{
    ...
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&ParkableStringManager::RemoveOnMainThread, base::Unretained(this), base::Unretained(string)));
}

void ParkableStringManager::CompleteUnparkOnMainThread(ParkableStringImpl* string, base::TimeDelta elapsed, base::TimeDelta disk_elapsed)
{
    DCHECK(IsMainThread());
...
void ParkableStringManager::CompleteUnpark(ParkableStringImpl* string, base::TimeDelta elapsed, base::TimeDelta disk_elapsed)
{
    ...
    task_runner_->PostTask(
        FROM_HERE, BindOnce(&ParkableStringManager::CompleteUnparkOnMainThread, base::Unretained(this), base::RetainedRef(string), elapsed, disk_elapsed));
}
...
void ParkableStringManager::OnWrittenToDisk(ParkableStringImpl* newly_written_string)
{
    DCHECK(IsMainThread());
...
void ParkableStringManager::OnUnparked(ParkableStringImpl* was_parked_string, bool was_on_disk)
{
    DCHECK(IsMainThread());
...
void ParkableStringManager::RecordStatisticsAfter5Minutes() const
{
    ...
    base::UmaHistogramTimes("Memory.ParkableString.TotalParkingThreadTime.5min", total_parking_thread_time_);
    base::UmaHistogramTimes("Memory.ParkableString.TotalUnparkingTime.5min", total_unparking_time_);

    // These metrics only make sense if the disk allocator is used.
    if (data_allocator().may_write()) {
        Statistics stats = ComputeStatistics();
        base::UmaHistogramTimes("Memory.ParkableString.DiskWriteTime.5min", total_disk_write_time_);
        base::UmaHistogramTimes("Memory.ParkableString.DiskReadTime.5min", total_disk_read_time_);
        base::UmaHistogramCounts100000("Memory.ParkableString.OnDiskSizeKb.5min", static_cast<int>(stats.on_disk_size / 1000));
    }
}
...
void ParkableStringManager::AgeStringsAndPark()
{
    ...
    for (ParkableStringImpl* str : unparked) {
        if (str->MaybeAgeOrParkString() == ParkableStringImpl::AgeOrParkResult::kSuccessOrTransientFailure) {
...
void ParkableStringManager::ScheduleAgingTaskIfNeeded()
{
    ...
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&ParkableStringManager::AgeStringsAndPark, base::Unretained(this)), delay);
...
ParkableStringManager::Statistics ParkableStringManager::ComputeStatistics() const
{
    ParkableStringManager::Statistics stats = {};
    // The digest has an inline capacity set to the digest size, hence sizeof() is
    // accurate.
    constexpr size_t kParkableStringImplActualSize = sizeof(ParkableStringImpl) + sizeof(ParkableStringImpl::ParkableMetadata);
    ...
    for (const auto& kv : unparked_strings_) {
        ...
        size_t memory_footprint = (str->has_compressed_data() ? str->compressed_size() : 0) + size + kParkableStringImplActualSize;
    ...
    for (const auto& kv : parked_strings_) {
        ...
        // See comment above.
        size_t memory_footprint = str->compressed_size() + kParkableStringImplActualSize;
        DCHECK_EQ(memory_footprint, str->MemoryFootprintForDump());
    }
    ...
    stats.total_size = stats.uncompressed_size + stats.compressed_size + stats.metadata_size + stats.overhead_size;
    size_t memory_footprint = stats.compressed_size + stats.uncompressed_size + stats.metadata_size + stats.overhead_size;
    stats.savings_size = stats.original_size - static_cast<int64_t>(memory_footprint);
...
ParkableStringManager::ParkableStringManager()
    : task_runner_(Thread::MainThread()->GetTaskRunner(MainThreadTaskRunnerRestricted()))
{
    // Should unregister in the destructor, but `this` is a NoDestructor static
    // local.
    ThreadScheduler::Current()->ToMainThreadScheduler()->AddRAILModeObserver(this);
}
