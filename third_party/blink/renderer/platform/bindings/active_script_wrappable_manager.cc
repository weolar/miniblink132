...
void ActiveScriptWrappableManager::RecomputeActiveScriptWrappables(RecomputeMode mode)
{
    ...
    for (auto& pair : active_script_wrappables_) {
        pair.second = ScriptWrappableIsActive(*pair.first) ? pair.first.Get() : nullptr;
    }
    recomputed_cnt_++;
}

void ActiveScriptWrappableManager::CleanupInactiveAndClearActiveScriptWrappables(const LivenessBroker& broker)
{
    active_script_wrappables_.erase(std::remove_if(active_script_wrappables_.begin(), active_script_wrappables_.end(),
                                        [broker](auto& pair) {
...
void ActiveScriptWrappableManager::Trace(Visitor* visitor) const
{
    ...
    visitor->RegisterWeakCallbackMethod<ActiveScriptWrappableManager, &ActiveScriptWrappableManager::CleanupInactiveAndClearActiveScriptWrappables>(this);
}
