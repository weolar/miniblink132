...
RuntimeCallStats::CounterId test_counter_1_id = RuntimeCallStats::CounterId::kTestCounter1;
RuntimeCallStats::CounterId test_counter_2_id = RuntimeCallStats::CounterId::kTestCounter2;
...
TEST_F(RuntimeCallStatsTest, StatsCounterNameIsCorrect)
{
    ...
    EXPECT_STREQ("Blink_TestCounter1", stats.GetCounter(test_counter_1_id)->GetName());
}

TEST_F(RuntimeCallStatsTest, TestBindingsCountersForMethods)
{
    RuntimeCallStats stats(clock());
    RuntimeCallCounter* method_counter = stats.GetCounter(RuntimeCallStats::CounterId::kBindingsMethodTestCounter);
...
{
    RuntimeCallStats stats(clock());
    RuntimeCallCounter* getter_counter = stats.GetCounter(RuntimeCallStats::CounterId::kBindingsReadOnlyAttributeTestCounter_Getter);
    EXPECT_STREQ("Blink_BindingsReadOnlyAttributeTestCounter_Getter", getter_counter->GetName());
}

TEST_F(RuntimeCallStatsTest, TestBindingsCountersForAttributes)
{
    RuntimeCallStats stats(clock());
    RuntimeCallCounter* getter_counter = stats.GetCounter(RuntimeCallStats::CounterId::kBindingsAttributeTestCounter_Getter);
    RuntimeCallCounter* setter_counter = stats.GetCounter(RuntimeCallStats::CounterId::kBindingsAttributeTestCounter_Setter);
    EXPECT_STREQ("Blink_BindingsAttributeTestCounter_Getter", getter_counter->GetName());
    EXPECT_STREQ("Blink_BindingsAttributeTestCounter_Setter", setter_counter->GetName());
}
...
static void RecursiveRCSTestHelper(int remaining_count, RuntimeCallStatsTest& fixture, RuntimeCallStats& stats)
{
    RuntimeCallTimerScope scope(&stats, test_counter_1_id);
...
TEST_F(RuntimeCallStatsTest, TestScopeWithOptionalMacroWithCallStatsDisabled)
{
...

    {
        ...
        RUNTIME_CALL_TIMER_SCOPE_WITH_OPTIONAL_RCS(scope, &stats, test_counter_1_id);
        AdvanceClock(25);
    }
