...
template <typename T> using TraceWrapperV8Reference = v8::TracedReference<T>;

} // namespace blink

namespace WTF {

template <typename T> struct IsTraceable<blink::TraceWrapperV8Reference<T>> {
    STATIC_ONLY(IsTraceable);
    static const bool value = true;
};

template <typename T> struct VectorTraits<blink::TraceWrapperV8Reference<T>> : VectorTraitsBase<blink::TraceWrapperV8Reference<T>> {
    STATIC_ONLY(VectorTraits);

    static constexpr bool kNeedsDestruction = !std::is_trivially_destructible<blink::TraceWrapperV8Reference<T>>::value;
    ...
    static_assert(!kNeedsDestruction, "TraceWrapperV8Reference should be trivially destructible.");
};

template <typename T> struct HashTraits<blink::TraceWrapperV8Reference<T>> : GenericHashTraits<blink::TraceWrapperV8Reference<T>> {
