...
using ThreadPoolExecutionMode = base::test::TaskEnvironment::ThreadPoolExecutionMode;
...
public:
    explicit LambdaThreadDelegate(base::OnceClosure f)
        : f_(std::move(f))
    {
    }
...
class ParkableStringTest : public testing::TestWithParam<ParkableStringImpl::CompressionAlgorithm> {
public:
    ParkableStringTest(ThreadPoolExecutionMode thread_pool_execution_mode = ThreadPoolExecutionMode::DEFAULT)
        : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME, thread_pool_execution_mode)
    {
        ParkableStringImpl::CompressionAlgorithm algorithm = GetParam();
        switch (algorithm) {
        case ParkableStringImpl::CompressionAlgorithm::kZlib:
            scoped_feature_list_.InitWithFeatures({}, { features::kUseSnappyForParkableStrings, features::kUseZstdForParkableStrings });
            break;
        ...
        case ParkableStringImpl::CompressionAlgorithm::kZstd:
            scoped_feature_list_.InitWithFeatures({ features::kUseZstdForParkableStrings }, { features::kUseSnappyForParkableStrings });
            break;
...
protected:
    ...
    {
        bool success = string.Impl()->Park(ParkableStringImpl::ParkingMode::kCompress);
        RunPostedTasks();
        return success;
    }
    ...
    void CheckOnlyCpuCostTaskRemains()
    {
        ...
        if (ParkableStringManager::Instance().has_posted_unparking_time_accounting_task_) {
            expected_count = 1;
        }
        EXPECT_EQ(expected_count, task_environment_.GetPendingMainThreadTaskCount());
    }
    ...
    void SetUp() override
    {
        ...
        manager.SetTaskRunnerForTesting(task_environment_.GetMainThreadTaskRunner());
        manager.SetDataAllocatorForTesting(std::make_unique<InMemoryDataAllocator>());
...
INSTANTIATE_TEST_SUITE_P(CompressionAlgorithm, ParkableStringTest,
    ::testing::Values(ParkableStringImpl::CompressionAlgorithm::kZlib, ParkableStringImpl::CompressionAlgorithm::kSnappy
#if BUILDFLAG(HAS_ZSTD_COMPRESSION)
        ,
...
TEST_P(ParkableStringTest, CheckCompressedSize)
{
    ...
    EXPECT_TRUE(parkable.Impl()->Park(ParkableStringImpl::ParkingMode::kCompress));
    RunPostedTasks();
    EXPECT_TRUE(parkable.Impl()->is_parked());
...
TEST_P(ParkableStringTest, Simple)
{
    ...
    EXPECT_FALSE(parkable_abc.may_be_parked()); // Small strings are not parkable.
...
TEST_P(ParkableStringTest, Park)
{
...

    {
        ...
        EXPECT_TRUE(parkable_c.Impl()->Park(ParkableStringImpl::ParkingMode::kCompress));
...
TEST_P(ParkableStringTest, AbortParking)
{
...

    {
        ...
        parkable.Lock();
        EXPECT_FALSE(parkable.Impl()->Park(ParkableStringImpl::ParkingMode::kCompress));
        parkable.Unlock();
        {
...
TEST_P(ParkableStringTest, BackgroundDestruct)
{
    ...
    auto parkable = std::make_unique<ParkableStringWrapper>(MakeLargeString().ReleaseImpl());
...
TEST_P(ParkableStringTest, LockUnlock)
{
    ...
    LambdaThreadDelegate delegate(base::BindLambdaForTesting([&]() { parkable.Lock(); }));
...
TEST_P(ParkableStringTest, ManagerMultipleStrings)
{
...

    {
        ...
        ParkableString other_parkable3_different_string(MakeLargeString('c').ReleaseImpl());
        EXPECT_EQ(1u, manager.Size());
...
#else
#define EXPECT_ASAN_DEATH(statement, regex) GTEST_UNSUPPORTED_DEATH_TEST(statement, regex, )
#endif
...
TEST_P(ParkableStringTest, Compression)
{
    ...
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.Compression.SizeKb", kSizeKb, 1);
    histogram_tester.ExpectTotalCount("Memory.ParkableString.Compression.Latency", 1);
    histogram_tester.ExpectTotalCount("Memory.ParkableString.Decompression.Latency", 2);
    histogram_tester.ExpectTotalCount("Memory.ParkableString.Decompression.ThroughputMBps", 2);
}
...
TEST_P(ParkableStringTest, CompressionFailed)
{
    ...
    EXPECT_EQ(ParkableStringImpl::AgeOrParkResult::kNonTransientFailure, parkable.Impl()->MaybeAgeOrParkString());
...
TEST_P(ParkableStringTest, ReportMemoryDump)
{
    ...
    constexpr size_t kActualSize = sizeof(ParkableStringImpl) + sizeof(ParkableStringImpl::ParkableMetadata);
    ...
    base::trace_event::MemoryDumpArgs args = { base::trace_event::MemoryDumpLevelOfDetail::kDetailed };
    base::trace_event::ProcessMemoryDump pmd(args);
    manager.OnMemoryDump(&pmd);
    base::trace_event::MemoryAllocatorDump* dump = pmd.GetAllocatorDump("parkable_strings");
    ASSERT_NE(nullptr, dump);

    constexpr size_t kStringSize = kSizeKb * 1000;
    MemoryAllocatorDump::Entry original("original_size", "bytes", 2 * kStringSize);
    EXPECT_THAT(dump->entries(), Contains(Eq(ByRef(original))));

    // |parkable1| is unparked.
    MemoryAllocatorDump::Entry uncompressed("uncompressed_size", "bytes", kStringSize);
    EXPECT_THAT(dump->entries(), Contains(Eq(ByRef(uncompressed))));

    MemoryAllocatorDump::Entry compressed("compressed_size", "bytes", kCompressedSize);
    EXPECT_THAT(dump->entries(), Contains(Eq(ByRef(compressed))));

    // |parkable1| compressed data is overhead.
    MemoryAllocatorDump::Entry overhead("overhead_size", "bytes", kCompressedSize);
    EXPECT_THAT(dump->entries(), Contains(Eq(ByRef(overhead))));

    MemoryAllocatorDump::Entry metadata("metadata_size", "bytes", 2 * kActualSize);
    EXPECT_THAT(dump->entries(), Contains(Eq(ByRef(metadata))));

    MemoryAllocatorDump::Entry savings("savings_size", "bytes", 2 * kStringSize - (kStringSize + 2 * kCompressedSize + 2 * kActualSize));
    ...
    ASSERT_NE(nullptr, dump);
    on_disk = MemoryAllocatorDump::Entry("on_disk_size", "bytes", kCompressedSize);
    EXPECT_THAT(dump->entries(), Contains(Eq(ByRef(on_disk))));
    // |parkable2| is on disk.
    on_disk_footprint = MemoryAllocatorDump::Entry("on_disk_footprint", "bytes", kCompressedSize);
    EXPECT_THAT(dump->entries(), Contains(Eq(ByRef(on_disk_footprint))));

    MemoryAllocatorDump::Entry on_disk_free_chunks = MemoryAllocatorDump::Entry("on_disk_free_chunks", "bytes", 0);
    EXPECT_THAT(dump->entries(), Contains(Eq(ByRef(on_disk_free_chunks))));

    // |parkable1| is compressed.
    compressed = MemoryAllocatorDump::Entry("compressed_size", "bytes", kCompressedSize);
    EXPECT_THAT(dump->entries(), Contains(Eq(ByRef(compressed))));
}
...
TEST_P(ParkableStringTest, MemoryFootprintForDump)
{
    ...
    memory_footprint = kActualSize + parkable1.Impl()->compressed_size() + parkable1.Impl()->CharactersSizeInBytes();
    ...
    memory_footprint = sizeof(ParkableStringImpl) + parkable3.Impl()->CharactersSizeInBytes();
...
{
    ParkableString parkable(MakeLargeString().ReleaseImpl());
    EXPECT_EQ(ParkableStringImpl::Age::kYoung, parkable.Impl()->age_for_testing());
    WaitForAging();
    EXPECT_EQ(ParkableStringImpl::Age::kOld, parkable.Impl()->age_for_testing());
...
TEST_P(ParkableStringTest, ReportTotalDiskTime)
{
    ...
    task_environment_.FastForwardUntilNoTasksRemain();
    int64_t mock_elapsed_time_ms = base::ScopedMockElapsedTimersForTest::kMockElapsedTime.InMilliseconds();
    // String does not get to disk at the first iteration, hence "-1".
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.DiskReadTime.5min", mock_elapsed_time_ms * (kNumIterations - 1), 1);

    // The string is only written once despite the multiple parking/unparking
    // calls.
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.DiskWriteTime.5min", mock_elapsed_time_ms, 1);

    histogram_tester.ExpectUniqueSample("Memory.ParkableString.OnDiskSizeKb.5min", kCompressedSize / 1000, 1);
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.TotalUnparkingTime.5min", mock_elapsed_time_ms * kNumIterations - 1, 1);
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.TotalParkingThreadTime.5min", mock_elapsed_time_ms, 1);
}
...
INSTANTIATE_TEST_SUITE_P(CompressionAlgorithm, ParkableStringTestWithQueuedThreadPool,
    ::testing::Values(ParkableStringImpl::CompressionAlgorithm::kZlib, ParkableStringImpl::CompressionAlgorithm::kSnappy
#if BUILDFLAG(HAS_ZSTD_COMPRESSION)
        ,
        ParkableStringImpl::CompressionAlgorithm::kZstd
...
TEST_P(ParkableStringTestWithQueuedThreadPool, AgingParkingInProgress)
{
    ...
    scheduler::GetSingleThreadTaskRunnerForTesting()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), ParkableStringManager::AgingInterval());
    run_loop.Run();
...
public:
    ...
    {
        const std::vector<base::test::FeatureRefAndParams> enabled_features = { { features::kCompressParkableStrings, { { "max_disk_capacity_mb", "1" } } } };
...
INSTANTIATE_TEST_SUITE_P(CompressionAlgorithm, ParkableStringTestWithLimitedDiskCapacity,
    ::testing::Values(ParkableStringImpl::CompressionAlgorithm::kZlib, ParkableStringImpl::CompressionAlgorithm::kSnappy
#if BUILDFLAG(HAS_ZSTD_COMPRESSION)
        ,
        ParkableStringImpl::CompressionAlgorithm::kZstd
...
TEST_P(ParkableStringTestLessAggressiveMode, NoParkingWhileLoadingOrInForeground)
{
    auto& manager = ParkableStringManager::Instance();
...
INSTANTIATE_TEST_SUITE_P(CompressionAlgorithm, ParkableStringTestLessAggressiveMode,
    ::testing::Values(ParkableStringImpl::CompressionAlgorithm::kZlib, ParkableStringImpl::CompressionAlgorithm::kSnappy
#if BUILDFLAG(HAS_ZSTD_COMPRESSION)
        ,
        ParkableStringImpl::CompressionAlgorithm::kZstd
