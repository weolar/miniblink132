...
static_assert(offsetof(struct WrapperTypeInfo, gin_embedder) == offsetof(struct gin::WrapperInfo, embedder),
    "offset of WrapperTypeInfo.ginEmbedder must be the same as "
    "gin::WrapperInfo.embedder");

v8::Local<v8::Template> WrapperTypeInfo::GetV8ClassTemplate(v8::Isolate* isolate, const DOMWrapperWorld& world) const
{
    V8PerIsolateData* per_isolate_data = V8PerIsolateData::From(isolate);
    v8::Local<v8::Template> v8_template = per_isolate_data->FindV8Template(world, this);
    if (!v8_template.IsEmpty())
        return v8_template;

    switch (idl_definition_kind) {
    case kIdlInterface:
        v8_template = v8::FunctionTemplate::New(isolate, V8ObjectConstructor::IsValidConstructorMode);
        break;
    case kIdlNamespace:
