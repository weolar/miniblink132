...
typedef HashMap<int, std::unique_ptr<V8DOMActivityLogger>, IntWithZeroKeyHashTraits<int>> DOMActivityLoggerMapForIsolatedWorld;
...
void V8DOMActivityLogger::LogMethod(ScriptState* script_state, const char* api_name, v8::FunctionCallbackInfo<v8::Value> info)
{
    ...
    LogMethod(script_state, api_name, base::span<const v8::Local<v8::Value>>(logger_args.data(), logger_args.size()));
}

void V8DOMActivityLogger::SetActivityLogger(int world_id, const String& extension_id, std::unique_ptr<V8DOMActivityLogger> logger)
{
    if (world_id)
...
V8DOMActivityLogger* V8DOMActivityLogger::ActivityLogger(int world_id, const String& extension_id)
{
    if (world_id) {
...
V8DOMActivityLogger* V8DOMActivityLogger::ActivityLogger(int world_id, const KURL& url)
{
...
V8DOMActivityLogger* V8DOMActivityLogger::CurrentActivityLogger(v8::Isolate* isolate)
{
    if (!isolate->InContext())
        return nullptr;

    v8::HandleScope handle_scope(isolate);
    V8PerContextData* context_data = ScriptState::ForCurrentRealm(isolate)->PerContextData();
    if (!context_data)
        return nullptr;
...
V8DOMActivityLogger* V8DOMActivityLogger::CurrentActivityLoggerIfIsolatedWorld(v8::Isolate* isolate)
{
    if (!isolate->InContext())
        return nullptr;
