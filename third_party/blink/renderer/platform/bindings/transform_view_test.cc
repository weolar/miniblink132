...
struct to_string {
    WTF::String operator()(int n) const
    {
        return WTF::String::Number(n);
    }
};

static_assert(std::forward_iterator<decltype(Transform<to_string>(std::vector<int>()).begin())>);
...
TEST(TransformView, Basic)
{
    ...
    EXPECT_THAT(Transform<std::negate<int>>(in), testing::ElementsAre(-1, -2, -3));
    static int arr[] = { 5, 6, 7 };
    EXPECT_THAT(Transform<std::negate<int>>(arr), testing::ElementsAre(-5, -6, -7));
}
...
TEST(TransformView, NonRandomAccessIterator)
{
    ...
    EXPECT_THAT(Transform<std::negate<int>>(in), testing::UnorderedElementsAre(-1, -2, -3));
}
