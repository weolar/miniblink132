...
CallbackFunctionBase::CallbackFunctionBase(v8::Local<v8::Object> callback_function)
{
    ...
    incumbent_script_state_ = ScriptState::From(isolate, isolate->GetIncumbentContext());
    ...
    if (callback_function->IsFunction()) {
        ...
        callback_relevant_script_state_ = ScriptState::ForRelevantRealm(isolate, callback_function);
    } else {
        v8::MaybeLocal<v8::Context> creation_context = callback_function->GetCreationContext();
        if (BindingSecurityForPlatform::ShouldAllowAccessToV8Context(incumbent_script_state_->GetContext(), creation_context)) {
            callback_relevant_script_state_ = ScriptState::From(isolate, creation_context.ToLocalChecked());
        }
    }
}
...
ScriptState* CallbackFunctionBase::CallbackRelevantScriptStateOrReportError(const char* interface_name, const char* operation_name) const
{
    ...
    ExceptionState exception_state(GetIsolate(), v8::ExceptionContext::kOperation, interface_name, operation_name);
    exception_state.ThrowSecurityError("An invocation of the provided callback failed due to cross origin "
                                       "access.");
    return nullptr;
}

ScriptState* CallbackFunctionBase::CallbackRelevantScriptStateOrThrowException(const char* interface_name, const char* operation_name) const
{
    if (callback_relevant_script_state_) [[likely]] {
        return callback_relevant_script_state_;
    }
...
void CallbackFunctionBase::EvaluateAsPartOfCallback(base::OnceCallback<void(ScriptState*)> closure)
{
    ...
    ScriptState::Scope callback_relevant_context_scope(callback_relevant_script_state_);
    // step 9: Prepare to run a callback with stored settings.
    v8::Context::BackupIncumbentScope backup_incumbent_scope(IncumbentScriptState()->GetContext());
