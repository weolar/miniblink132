...
// implement it.
class PLATFORM_EXPORT CallbackFunctionBase : public GarbageCollected<CallbackFunctionBase>, public NameClient {
public:
    ...
    // returns nullptr.
    ScriptState* CallbackRelevantScriptStateOrReportError(const char* interface_name, const char* operation_name) const;

    // Returns the ScriptState of the relevant realm of the callback object iff
    // the callback is the same origin-domain. Otherwise, throws an exception and
    // returns nullptr.
    ScriptState* CallbackRelevantScriptStateOrThrowException(const char* interface_name, const char* operation_name) const;

    ScriptState* IncumbentScriptState() const
    {
        return incumbent_script_state_;
    }
    ...
    {
        return CallbackFunction()->IsConstructor();
    }
...
class PLATFORM_EXPORT CallbackFunctionWithTaskAttributionBase : public CallbackFunctionBase {
public:
