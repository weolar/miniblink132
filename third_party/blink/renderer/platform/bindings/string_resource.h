...
public:
        ...
        : parkable_string_(string)
    {
        ...
        memory_accounter_.Increase(isolate, parkable_string_.CharactersSizeInBytes());
    }
    ...
    void Unaccount(v8::Isolate* isolate)
    {
        ...
        if (!parkable_string_.IsNull()) {
            ...
            if (plain_string_.Impl() != atomic_string_.Impl() && !atomic_string_.IsNull()) {
...
protected:
    ...
    const ParkableString& GetParkableString() const
    {
        return parkable_string_;
    }

    // Helper functions for derived constructors.
    template <typename Str> static inline Str Assert8Bit(Str&& str)
    {
        DCHECK(str.Is8Bit());
        return str;
    }

    template <typename Str> static inline Str Assert16Bit(Str&& str)
    {
        DCHECK(!str.Is8Bit());
        return str;
    }
...
class StringResource16Base : public StringResourceBase, public v8::String::ExternalStringResource {
public:
    ...
    explicit StringResource16Base(v8::Isolate* isolate, ParkableString parkable_string)
...
public:
    ...
    size_t length() const override
    {
        return GetParkableString().length();
    }

    const uint16_t* data() const override
    {
        return reinterpret_cast<const uint16_t*>(GetParkableString().Characters16());
    }
};

class StringResource8Base : public StringResourceBase, public v8::String::ExternalOneByteStringResource {
public:
    ...
    explicit StringResource8Base(v8::Isolate* isolate, ParkableString parkable_string)
