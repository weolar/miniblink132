...
public:
    MockNetworkManager()
        : mdns_responder_(new EmptyMdnsResponder())
    {
    }
...
class FilteringNetworkManagerTest : public testing::Test, public sigslot::has_slots<> {
public:
    ...
    {
        networks_.emplace_back("test_eth0", "Test Network Adapter 1", rtc::IPAddress(0x12345600U), 24, rtc::ADAPTER_TYPE_ETHERNET),
            networks_.back().AddIP(rtc::IPAddress(0x12345678));
        networks_.emplace_back("test_eth1", "Test Network Adapter 2", rtc::IPAddress(0x87654300U), 24, rtc::ADAPTER_TYPE_ETHERNET),
    ...
    void SetupNetworkManager(bool multiple_routes_requested)
    {
        ...
        if (multiple_routes_requested) {
            network_manager_.reset(new FilteringNetworkManager(base_network_manager_->AsWeakPtr(), media_permission_.get(), allow_mdns_obfuscation_));
            network_manager_->Initialize();
        } else {
            network_manager_.reset(new EmptyNetworkManager(base_network_manager_.get(), base_network_manager_->AsWeakPtr()));
        }
        network_manager_->SignalNetworksChanged.connect(this, &FilteringNetworkManagerTest::OnNetworksChanged);
    }

    void RunTests(TestEntry* tests, size_t size)
    {
        for (size_t i = 0; i < size; ++i) {
            EXPECT_EQ(tests[i].expected_result, ProcessEvent(tests[i].event)) << " in step: " << i;
        }
    }
...
protected:
    ...
    void clear_callback_called()
    {
        callback_called_ = false;
    }
    void set_allow_mdns_obfuscation(bool val)
    {
        allow_mdns_obfuscation_ = val;
    }
...
TEST_F(FilteringNetworkManagerTest, ProvideMdnsResponderForDefaultRouteAfterPermissionDenied)
{
    SetupNetworkManager(true);
    // By default, the enumeration is blocked if we provide |media_permission_|;
    EXPECT_EQ(rtc::NetworkManager::ENUMERATION_BLOCKED, network_manager_->enumeration_permission());
    ...
    for (const rtc::Network* network : networks) {
        EXPECT_EQ(network_manager_->GetMdnsResponder(), network->GetMdnsResponder());
    }
}
...
TEST_F(FilteringNetworkManagerTest, NullMdnsResponderWhenMdnsObfuscationDisallowedAfterPermissionDenied)
{
