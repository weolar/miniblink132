...
rtc::AdapterType ConvertConnectionTypeToAdapterType(net::NetworkChangeNotifier::ConnectionType type)
{
    switch (type) {
...
IpcNetworkManager::IpcNetworkManager(blink::NetworkListManager* network_list_manager, std::unique_ptr<webrtc::MdnsResponderInterface> mdns_responder)
...
base::WeakPtr<IpcNetworkManager> IpcNetworkManager::AsWeakPtrForSignalingThread()
{
...
void IpcNetworkManager::StartUpdating()
{
    ...
    if (network_list_received_) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, WTF::BindOnce(&IpcNetworkManager::SendNetworksChangedSignal, weak_factory_.GetWeakPtr()));
    } else {
...
void IpcNetworkManager::OnNetworkListChanged(
    const net::NetworkInterfaceList& list, const net::IPAddress& default_ipv4_local_address, const net::IPAddress& default_ipv6_local_address)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (auto it = list.begin(); it != list.end(); it++) {
        ...
        rtc::AdapterType adapter_type = ConvertConnectionTypeToAdapterType(it->type);
        ...
        auto network = CreateNetwork(it->name, it->name, prefix, it->prefix_length, adapter_type);
        ...
        if (it->address.IsIPv4()) {
            ...
            if (rtc::IPIsMacBased(iface_addr) || (it->ip_address_attributes & net::IP_ADDRESS_ATTRIBUTE_DEPRECATED) || rtc::IPIsLinkLocal(iface_addr)
                || rtc::IPIsLoopback(iface_addr)) {
                continue;
            }
    ...
    if (use_default_ipv4_address) {
        ipv4_default = webrtc::NetIPAddressToRtcIPAddress(default_ipv4_local_address);
    }
    if (use_default_ipv6_address) {
        ipv6_default = webrtc::NetIPAddressToRtcIPAddress(default_ipv6_local_address);
    }
    ...
    if (Platform::Current()->AllowsLoopbackInPeerConnection()) {
        ...
        auto network_v4 = CreateNetwork(name_v4, name_v4, ip_address_v4, 32, rtc::ADAPTER_TYPE_UNKNOWN);
        ...
        if (GetDefaultLocalAddress(AF_INET6, &ipv6_default_address)) {
            ...
            auto network_v6 = CreateNetwork(name_v6, name_v6, ip_address_v6, 64, rtc::ADAPTER_TYPE_UNKNOWN);
    ...
    UMA_HISTOGRAM_COUNTS_100("WebRTC.PeerConnection.IPv4Interfaces", stats.ipv4_network_count);
    UMA_HISTOGRAM_COUNTS_100("WebRTC.PeerConnection.IPv6Interfaces", stats.ipv6_network_count);
}
