...
class FilteringNetworkManager : public rtc::NetworkManagerBase, public sigslot::has_slots<> {
public:
    // This class is created by WebRTC's main thread but used by WebRTC's
    // worker thread |task_runner|.
    PLATFORM_EXPORT FilteringNetworkManager(IpcNetworkManager* network_manager, media::MediaPermission* media_permission, bool allow_mdns_obfuscation);
