...
{
    return (type == network::P2P_SOCKET_STUN_TCP_CLIENT) || (type == network::P2P_SOCKET_TCP_CLIENT) || (type == network::P2P_SOCKET_STUN_SSLTCP_CLIENT)
        || (type == network::P2P_SOCKET_SSLTCP_CLIENT) || (type == network::P2P_SOCKET_TLS_CLIENT) || (type == network::P2P_SOCKET_STUN_TLS_CLIENT);
}

bool JingleSocketOptionToP2PSocketOption(rtc::Socket::Option option, network::P2PSocketOption* ipc_option)
{
    switch (option) {
...
class IpcPacketSocket : public rtc::AsyncPacketSocket, public blink::P2PSocketClientDelegate {
public:
    IpcPacketSocket();
    ...
    bool Init(P2PSocketDispatcher* dispatcher, const net::NetworkTrafficAnnotationTag& traffic_annotation, network::P2PSocketType type,
        std::unique_ptr<P2PSocketClientImpl> client, const rtc::SocketAddress& local_address, uint16_t min_port, uint16_t max_port,
        const rtc::SocketAddress& remote_address,
        WTF::CrossThreadFunction<void(base::OnceCallback<void(std::optional<base::UnguessableToken>)>)>& devtools_token);
    ...
    void OnOpen(const net::IPEndPoint& local_address, const net::IPEndPoint& remote_address) override;
    void OnSendComplete(const network::P2PSendPacketMetrics& send_metrics) override;
    void OnError() override;
    void OnDataReceived(const net::IPEndPoint& address, base::span<const uint8_t> data, const base::TimeTicks& timestamp, rtc::EcnMarking ecn) override;

private:
    static void DoCreateSocket(network::P2PSocketType type, P2PSocketDispatcher* dispatcher, net::IPEndPoint local_endpoint, uint16_t min_port,
        uint16_t max_port, network::P2PHostAndIPEndPoint remote_info, net::NetworkTrafficAnnotationTag traffic_annotation,
        mojo::PendingRemote<network::mojom::blink::P2PSocketClient> remote, mojo::PendingReceiver<network::mojom::blink::P2PSocket> receiver,
        std::optional<base::UnguessableToken> devtools_token);
    int SendToInternal(const void* pv, size_t cb, const rtc::SocketAddress& addr, const rtc::PacketOptions& options);
...
class AsyncDnsAddressResolverImpl : public webrtc::AsyncDnsResolverInterface, public webrtc::AsyncDnsResolverResult {
public:
    ...
    void Start(const rtc::SocketAddress& addr, int address_family, absl::AnyInvocable<void()> callback) override;
    ...
    , current_discard_bytes_sequence_(0)
{
    ...
    std::fill_n(options_, static_cast<int>(network::P2P_SOCKET_OPT_MAX), kDefaultNonSetOptionValue);
}
...
{
    TRACE_COUNTER_ID1("p2p", "P2PSendBytesAvailable", local_address_.port(), send_bytes_available_);
    TRACE_COUNTER_ID1("p2p", "P2PSendPacketsInFlight", local_address_.port(), in_flight_packet_records_.size());
}
...
bool IpcPacketSocket::Init(P2PSocketDispatcher* dispatcher, const net::NetworkTrafficAnnotationTag& traffic_annotation, network::P2PSocketType type,
    std::unique_ptr<P2PSocketClientImpl> client, const rtc::SocketAddress& local_address, uint16_t min_port, uint16_t max_port,
    const rtc::SocketAddress& remote_address,
    WTF::CrossThreadFunction<void(base::OnceCallback<void(std::optional<base::UnguessableToken>)>)>& devtools_token_getter)
{
    ...
    network::P2PHostAndIPEndPoint remote_info(remote_address.hostname(), remote_endpoint);

    devtools_token_getter.Run(base::BindPostTaskToCurrentDefault(WTF::BindOnce(&IpcPacketSocket::DoCreateSocket, type_, WrapCrossThreadPersistent(dispatcher),
        local_endpoint, min_port, max_port, remote_info, traffic_annotation, client_->CreatePendingRemote(), client_->CreatePendingReceiver())));
...
void IpcPacketSocket::DoCreateSocket(network::P2PSocketType type, P2PSocketDispatcher* dispatcher, net::IPEndPoint local_endpoint, uint16_t min_port,
    uint16_t max_port, network::P2PHostAndIPEndPoint remote_info, net::NetworkTrafficAnnotationTag traffic_annotation,
    mojo::PendingRemote<network::mojom::blink::P2PSocketClient> remote, mojo::PendingReceiver<network::mojom::blink::P2PSocket> receiver,
    std::optional<base::UnguessableToken> devtools_token)
{
    ...
    dispatcher->GetP2PSocketManager()->CreateSocket(type, local_endpoint, network::P2PPortRange(min_port, max_port), remote_info,
        net::MutableNetworkTrafficAnnotationTag(traffic_annotation), devtools_token, std::move(remote), std::move(receiver));
}
...
int IpcPacketSocket::Send(const void* data, size_t data_size, const rtc::PacketOptions& options)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    return SendTo(data, data_size, remote_address_, options);
}

int IpcPacketSocket::SendTo(const void* data, size_t data_size, const rtc::SocketAddress& address, const rtc::PacketOptions& options)
{
...
int IpcPacketSocket::SendToInternal(const void* data, size_t data_size, const rtc::SocketAddress& address, const rtc::PacketOptions& options)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (data_size > send_bytes_available_) {
        TRACE_EVENT_INSTANT1("p2p", "MaxPendingBytesWouldBlock", TRACE_EVENT_SCOPE_THREAD, "id", client_->GetSocketID());
        if (!writable_signal_expected_) {
            blink::WebRtcLogMessage(
                base::StringPrintf("IpcPacketSocket: sending is blocked. %d packets_in_flight.", static_cast<int>(in_flight_packet_records_.size())));
    ...
    if (address.IsUnresolvedIP()) {
        ...
        if (!webrtc::SocketAddressToIPEndPoint(address, &address_chrome)) {
            LOG(WARNING) << "Failed to convert remote address to IPEndPoint: address=" << address.ipaddr().ToSensitiveString()
                         << ", remote_address_=" << remote_address_.ipaddr().ToSensitiveString();
            NOTREACHED();
        }
    }
    ...
    uint64_t packet_id = client_->Send(address_chrome, base::make_span(static_cast<const uint8_t*>(data), data_size), options);

    // Ensure packet_id is not 0. It can't be the case according to
    // P2PSocketClientImpl::Send().
    DCHECK_NE(packet_id, 0uL);
    in_flight_packet_records_.push_back(InFlightPacketRecord(packet_id, data_size));
...
int IpcPacketSocket::DoSetOption(network::P2PSocketOption option, int value)
{
    ...
    if (option == network::P2PSocketOption::P2P_SOCKET_OPT_SNDBUF && value > 0) {
        ...
        // reduce it.
        size_t new_limit = std::max(static_cast<size_t>(value), kDefaultMaximumInFlightBytes);
...
void IpcPacketSocket::OnOpen(const net::IPEndPoint& local_address, const net::IPEndPoint& remote_address)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (IsTcpClientSocket(type_)) {
        ...
        if (remote_address_.IsUnresolvedIP()) {
            ...
            // proxy.
            if (!remote_address.address().empty() && webrtc::IPEndPointToSocketAddress(remote_address, &jingle_socket_address)) {
...
void IpcPacketSocket::OnSendComplete(const network::P2PSendPacketMetrics& send_metrics)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    send_bytes_available_ = std::min(send_bytes_available_ + record.packet_size, max_in_flight_bytes_);

    in_flight_packet_records_.pop_front();
    TraceSendThrottlingState();

    SignalSentPacket(this, rtc::SentPacket(send_metrics.rtc_packet_id, send_metrics.send_time_ms));

    if (writable_signal_expected_ && send_bytes_available_ > (max_in_flight_bytes_ / 2)) {
        blink::WebRtcLogMessage(
            base::StringPrintf("IpcPacketSocket: sending is unblocked. %d packets in flight.", static_cast<int>(in_flight_packet_records_.size())));
...
void IpcPacketSocket::OnDataReceived(const net::IPEndPoint& address, base::span<const uint8_t> data, const base::TimeTicks& timestamp, rtc::EcnMarking ecn)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    NotifyPacketReceived(rtc::ReceivedPacket(data, address_lj, webrtc::Timestamp::Micros(timestamp.since_origin().InMicroseconds()), ecn));
}

AsyncDnsAddressResolverImpl::AsyncDnsAddressResolverImpl(P2PSocketDispatcher* dispatcher)
...
void AsyncDnsAddressResolverImpl::Start(const rtc::SocketAddress& addr, absl::AnyInvocable<void()> callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(!started_);
    started_ = true;
    ...
    resolver_->Start(addr, /*address_family=*/std::nullopt, WTF::BindOnce(&AsyncDnsAddressResolverImpl::OnAddressResolved, weak_factory_.GetWeakPtr()));
}

void AsyncDnsAddressResolverImpl::Start(const rtc::SocketAddress& addr, int address_family, absl::AnyInvocable<void()> callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(!started_);
    started_ = true;
    ...
    addr_ = addr;
    callback_ = std::move(callback);
    resolver_->Start(addr, std::make_optional(address_family), WTF::BindOnce(&AsyncDnsAddressResolverImpl::OnAddressResolved, weak_factory_.GetWeakPtr()));
}

bool AsyncDnsAddressResolverImpl::GetResolvedAddress(int family, rtc::SocketAddress* addr) const
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void AsyncDnsAddressResolverImpl::OnAddressResolved(const Vector<net::IPAddress>& addresses)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    for (wtf_size_t i = 0; i < addresses.size(); ++i) {
        rtc::SocketAddress socket_address;
        if (!webrtc::IPEndPointToSocketAddress(net::IPEndPoint(addresses[i], 0), &socket_address)) {
            NOTREACHED();
        }
...
IpcPacketSocketFactory::IpcPacketSocketFactory(
    WTF::CrossThreadFunction<void(base::OnceCallback<void(std::optional<base::UnguessableToken>)>)> devtools_token_getter,
    P2PSocketDispatcher* socket_dispatcher, const net::NetworkTrafficAnnotationTag& traffic_annotation, bool batch_udp_packets)
...
rtc::AsyncPacketSocket* IpcPacketSocketFactory::CreateUdpSocket(const rtc::SocketAddress& local_address, uint16_t min_port, uint16_t max_port)
{
    auto socket_dispatcher = socket_dispatcher_.Lock();
    DCHECK(socket_dispatcher);
    auto socket_client = std::make_unique<P2PSocketClientImpl>(batch_udp_packets_);
    std::unique_ptr<IpcPacketSocket> socket(new IpcPacketSocket());

    if (!socket->Init(socket_dispatcher, traffic_annotation_, network::P2P_SOCKET_UDP, std::move(socket_client), local_address, min_port, max_port,
            rtc::SocketAddress(), devtools_token_getter_)) {
        return nullptr;
    }
    return socket.release();
}

rtc::AsyncListenSocket* IpcPacketSocketFactory::CreateServerTcpSocket(const rtc::SocketAddress& local_address, uint16_t min_port, uint16_t max_port, int opts)
{
    NOTREACHED();
}

rtc::AsyncPacketSocket* IpcPacketSocketFactory::CreateClientTcpSocket(
    const rtc::SocketAddress& local_address, const rtc::SocketAddress& remote_address, const rtc::PacketSocketTcpOptions& opts)
{
    ...
    if (opts.opts & rtc::PacketSocketFactory::OPT_SSLTCP) {
        type = (opts.opts & rtc::PacketSocketFactory::OPT_STUN) ? network::P2P_SOCKET_STUN_SSLTCP_CLIENT : network::P2P_SOCKET_SSLTCP_CLIENT;
    } else if (opts.opts & rtc::PacketSocketFactory::OPT_TLS) {
        type = (opts.opts & rtc::PacketSocketFactory::OPT_STUN) ? network::P2P_SOCKET_STUN_TLS_CLIENT : network::P2P_SOCKET_TLS_CLIENT;
    } else {
        type = (opts.opts & rtc::PacketSocketFactory::OPT_STUN) ? network::P2P_SOCKET_STUN_TCP_CLIENT : network::P2P_SOCKET_TCP_CLIENT;
    }
    auto socket_dispatcher = socket_dispatcher_.Lock();
    DCHECK(socket_dispatcher);
    auto socket_client = std::make_unique<P2PSocketClientImpl>(/*batch_packets=*/false);
    std::unique_ptr<IpcPacketSocket> socket(new IpcPacketSocket());
    if (!socket->Init(socket_dispatcher, traffic_annotation_, type, std::move(socket_client), local_address, 0, 0, remote_address, devtools_token_getter_)) {
        return nullptr;
    }
    return socket.release();
}

std::unique_ptr<webrtc::AsyncDnsResolverInterface> IpcPacketSocketFactory::CreateAsyncDnsResolver()
{
    auto socket_dispatcher = socket_dispatcher_.Lock();
    DCHECK(socket_dispatcher);
