...
FilteringNetworkManager::FilteringNetworkManager(IpcNetworkManager* network_manager, media::MediaPermission* media_permission, bool allow_mdns_obfuscation)
    : FilteringNetworkManager(network_manager->AsWeakPtrForSignalingThread(), media_permission, allow_mdns_obfuscation)
{
}
...
FilteringNetworkManager::FilteringNetworkManager(
    base::WeakPtr<rtc::NetworkManager> network_manager_for_signaling_thread, media::MediaPermission* media_permission, bool allow_mdns_obfuscation)
    : network_manager_for_signaling_thread_(std::move(network_manager_for_signaling_thread))
...
void FilteringNetworkManager::StartUpdating()
{
    ...
    if (!start_updating_called_) {
        ...
        network_manager_for_signaling_thread_->SignalNetworksChanged.connect(this, &FilteringNetworkManager::OnNetworksChanged);
    }
...
void FilteringNetworkManager::CheckPermission()
{
    ...
    media_permission_->HasPermission(media::MediaPermission::Type::kAudioCapture, WTF::BindOnce(&FilteringNetworkManager::OnPermissionStatus, GetWeakPtr()));
    media_permission_->HasPermission(media::MediaPermission::Type::kVideoCapture, WTF::BindOnce(&FilteringNetworkManager::OnPermissionStatus, GetWeakPtr()));
}
...
void FilteringNetworkManager::OnPermissionStatus(bool granted)
{
    ...
    VLOG(1) << "FilteringNetworkManager received permission status: " << (granted ? "granted" : "denied");
...
void FilteringNetworkManager::OnNetworksChanged()
{
    ...
    network_manager_for_signaling_thread_->GetDefaultLocalAddress(AF_INET, &ipv4_default);
    network_manager_for_signaling_thread_->GetDefaultLocalAddress(AF_INET6, &ipv6_default);
    ...
    // known.
    std::vector<const rtc::Network*> networks = network_manager_for_signaling_thread_->GetNetworks();
...
blink::IPPermissionStatus FilteringNetworkManager::GetIPPermissionStatus() const
{
    if (enumeration_permission() == ENUMERATION_ALLOWED) {
        return media_permission_ ? blink::PERMISSION_GRANTED_WITH_CHECKING : blink::PERMISSION_GRANTED_WITHOUT_CHECKING;
    }
...
void FilteringNetworkManager::FireEventIfStarted()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, WTF::BindOnce(&FilteringNetworkManager::SendNetworksChangedSignal, GetWeakPtr()));
