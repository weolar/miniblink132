...
class IpcNetworkManager : public rtc::NetworkManagerBase, public blink::NetworkListObserver {
public:
    // Constructor doesn't take ownership of the |network_list_manager|.
    PLATFORM_EXPORT IpcNetworkManager(blink::NetworkListManager* network_list_manager, std::unique_ptr<webrtc::MdnsResponderInterface> mdns_responder);
    ...
    base::WeakPtr<IpcNetworkManager> PLATFORM_EXPORT AsWeakPtrForSignalingThread();
    ...
    void OnNetworkListChanged(
        const net::NetworkInterfaceList& list, const net::IPAddress& default_ipv4_local_address, const net::IPAddress& default_ipv6_local_address) override;
