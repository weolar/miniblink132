...
public:
    void SetUp() override
    {
        mojo_binding_context_ = MakeGarbageCollected<FakeMojoBindingContext>(task_environment_.GetMainThreadTaskRunner());
        socket_factory_
            = std::make_unique<IpcPacketSocketFactory>(WTF::CrossThreadBindRepeating([](base::OnceCallback<void(std::optional<base::UnguessableToken>)>) {}),
                &P2PSocketDispatcher::From(*mojo_binding_context_), TRAFFIC_ANNOTATION_FOR_TESTS, false);

        socket_.reset(socket_factory_->CreateUdpSocket(rtc::SocketAddress("127.0.0.1", 0), 0, 0));
...
TEST_F(IpcPacketSocketFactoryTest, SetOptions)
{
    ...
    EXPECT_EQ(0, socket_->GetOption(rtc::Socket::OPT_RECV_ECN, &recv_ecn_option));
    EXPECT_EQ(rtc::EcnMarking::kEct1, static_cast<rtc::EcnMarking>(recv_ecn_option));
}
