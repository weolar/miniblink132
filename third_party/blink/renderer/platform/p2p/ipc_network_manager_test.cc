...
class MockP2PSocketDispatcher : public GarbageCollected<MockP2PSocketDispatcher>, public NetworkListManager {
public:
    void AddNetworkListObserver(blink::NetworkListObserver* network_list_observer) override
    {
    }

    void RemoveNetworkListObserver(blink::NetworkListObserver* network_list_observer) override
    {
    }
...
public:
    void CreateNameForAddress(const rtc::IPAddress& addr, NameCreatedCallback callback) override
    {
    }
    void RemoveNameForAddress(const rtc::IPAddress& addr, NameRemovedCallback callback) override
    {
    }
};
...
public:
    IpcNetworkManagerTest()
        ...
        , network_manager_(std::make_unique<IpcNetworkManager>(network_list_manager_.Get(), std::make_unique<EmptyMdnsResponder>()))
    {
    }

    ~IpcNetworkManagerTest() override
    {
        network_manager_->ContextDestroyed();
    }
...
TEST_F(IpcNetworkManagerTest, TestMergeNetworkList)
{
    ...
    list.push_back(net::NetworkInterface("em1", "em1", 0, net::NetworkChangeNotifier::CONNECTION_UNKNOWN, ip, 64, net::IP_ADDRESS_ATTRIBUTE_NONE));

    EXPECT_TRUE(ip.AssignFromIPLiteral(kIPv6PublicAddrString2));
    list.push_back(net::NetworkInterface("em1", "em1", 0, net::NetworkChangeNotifier::CONNECTION_UNKNOWN, ip, 64, net::IP_ADDRESS_ATTRIBUTE_NONE));

    network_manager_->OnNetworkListChanged(list, net::IPAddress(), net::IPAddress());
    ...
    // a different network.
    list.push_back(net::NetworkInterface("em1", "em1", 0, net::NetworkChangeNotifier::CONNECTION_UNKNOWN, ip, 48, net::IP_ADDRESS_ATTRIBUTE_NONE));
    ...
    auto network_with_two_ips = base::ranges::find(networks, 64, &rtc::Network::prefix_length);
    ...
    EXPECT_EQ((*network_with_two_ips)->GetIPs()[0], rtc::InterfaceAddress(ip_address));
    EXPECT_TRUE(rtc::IPFromString(kIPv6PublicAddrString2, &ip_address));
    EXPECT_EQ((*network_with_two_ips)->GetIPs()[1], rtc::InterfaceAddress(ip_address));
    // Verify the network with prefix length of 48 has 1 IP address.
    auto network_with_one_ip = base::ranges::find(networks, 48, &rtc::Network::prefix_length);
    ASSERT_NE(networks.end(), network_with_one_ip);
    EXPECT_EQ(1uL, (*network_with_one_ip)->GetIPs().size());
    EXPECT_TRUE(rtc::IPFromString(kIPv6PublicAddrString2, &ip_address));
    EXPECT_EQ((*network_with_one_ip)->GetIPs()[0], rtc::InterfaceAddress(ip_address));
}
...
TEST_F(IpcNetworkManagerTest, DeterminesNetworkTypeFromNameIfUnknown)
{
    ...
    EXPECT_TRUE(ip.AssignFromIPLiteral(kIPv6PublicAddrString1));
    list.push_back(net::NetworkInterface("tun1", "tun1", 0, net::NetworkChangeNotifier::CONNECTION_UNKNOWN, ip, 64, net::IP_ADDRESS_ATTRIBUTE_NONE));

    EXPECT_TRUE(ip.AssignFromIPLiteral(kIPv6PublicAddrString2));
    list.push_back(net::NetworkInterface("tun2", "tun2", 0, net::NetworkChangeNotifier::CONNECTION_WIFI, ip, 64, net::IP_ADDRESS_ATTRIBUTE_NONE));
...
TEST_F(IpcNetworkManagerTest, DeterminesVPNFromMacAddress)
{
    ...
    net::IPAddress ip;
    rtc::IPAddress ip_address;
    std::optional<net::Eui48MacAddress> mac_address({ 0x0, 0x5, 0x9A, 0x3C, 0x7A, 0x0 });

    // Assign the magic MAC address known to be a Cisco Anyconnect VPN interface.
    EXPECT_TRUE(ip.AssignFromIPLiteral(kIPv6PublicAddrString1));
    list.push_back(
        net::NetworkInterface("eth0", "eth1", 0, net::NetworkChangeNotifier::CONNECTION_UNKNOWN, ip, 64, net::IP_ADDRESS_ATTRIBUTE_NONE, mac_address));
...
TEST_F(IpcNetworkManagerTest, DeterminesNotVPN)
{
    ...
    net::IPAddress ip;
    rtc::IPAddress ip_address;
    std::optional<net::Eui48MacAddress> mac_address({ 0x0, 0x5, 0x9A, 0x3C, 0x7A, 0x1 });
...
TEST_F(IpcNetworkManagerTest, ServeAsMdnsResponderProviderForNetworksEnumerated)
{
    net::NetworkInterfaceList list;
    // Add networks.
    net::IPAddress ip;
    ...
    webrtc::MdnsResponderInterface* const mdns_responder = network_manager_->GetMdnsResponder();
