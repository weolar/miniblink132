...
EmptyNetworkManager::EmptyNetworkManager(IpcNetworkManager* network_manager)
    : EmptyNetworkManager(network_manager, network_manager->AsWeakPtrForSignalingThread())
{
}
...
EmptyNetworkManager::EmptyNetworkManager(rtc::NetworkManager* network_manager, base::WeakPtr<rtc::NetworkManager> network_manager_for_signaling_thread)
    : network_manager_for_signaling_thread_(network_manager_for_signaling_thread)
{
    DCHECK(network_manager);
    DETACH_FROM_THREAD(thread_checker_);
    set_enumeration_permission(ENUMERATION_BLOCKED);
    network_manager->SignalNetworksChanged.connect(this, &EmptyNetworkManager::OnNetworksChanged);
}
...
bool EmptyNetworkManager::GetDefaultLocalAddress(int family, rtc::IPAddress* ipaddress) const
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(network_manager_for_signaling_thread_);
    return network_manager_for_signaling_thread_->GetDefaultLocalAddress(family, ipaddress);
}
