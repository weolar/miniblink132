...
void RecordNumberOfPacketsInBatch(int num_packets)
{
    ...
    UMA_HISTOGRAM_COUNTS("WebRTC.P2P.UDP.BatchingNumberOfSentPackets", num_packets);
}
...
void P2PSocketClientImpl::Init(blink::P2PSocketClientDelegate* delegate)
{
    ...
    receiver_.set_disconnect_handler(WTF::BindOnce(&P2PSocketClientImpl::OnConnectionError, WTF::Unretained(this)));
}

uint64_t P2PSocketClientImpl::Send(const net::IPEndPoint& address, base::span<const uint8_t> data, const rtc::PacketOptions& options)
{
...
void P2PSocketClientImpl::SendWithPacketId(
    const net::IPEndPoint& address, base::span<const uint8_t> data, const rtc::PacketOptions& options, uint64_t packet_id)
{
    ...
    if (batch_packets_ && (awaiting_batch_complete_ || (options.batchable && !options.last_packet_in_batch))) {
        awaiting_batch_complete_ = true;
        batched_packets_storage_.emplace_back(data);
        const auto& storage = batched_packets_storage_.back();
        batched_send_packets_.emplace_back(network::mojom::blink::P2PSendPacket::New(
            base::span<const uint8_t>(storage.begin(), storage.end()), network::P2PPacketInfo(address, options, packet_id)));
...
void P2PSocketClientImpl::SetOption(network::P2PSocketOption option, int value)
{
    DCHECK(state_ == kStateOpen || state_ == kStateError);
    if (state_ == kStateOpen)
...
void P2PSocketClientImpl::SetDelegate(blink::P2PSocketClientDelegate* delegate)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    delegate_ = delegate;
}

void P2PSocketClientImpl::SocketCreated(const net::IPEndPoint& local_address, const net::IPEndPoint& remote_address)
{
    state_ = kStateOpen;
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (delegate_)
...
void P2PSocketClientImpl::SendComplete(const network::P2PSendPacketMetrics& send_metrics)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (delegate_)
...
void P2PSocketClientImpl::SendBatchComplete(const WTF::Vector<::network::P2PSendPacketMetrics>& in_send_metrics_batch)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void P2PSocketClientImpl::DataReceived(WTF::Vector<P2PReceivedPacketPtr> packets)
{
    ...
    if (delegate_) {
        for (auto& packet : packets) {
            delegate_->OnDataReceived(packet->socket_address, packet->data, packet->timestamp, packet->ecn);
        }
    }
}
