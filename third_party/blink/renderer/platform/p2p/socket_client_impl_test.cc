...
{
    return a.packet_id == b.packet_id && a.rtc_packet_id == b.rtc_packet_id && a.send_time_ms == b.send_time_ms;
}
} // namespace network
...
public:
    MOCK_METHOD(void, Send, (base::span<const uint8_t>, const network::P2PPacketInfo&), (override));
    MOCK_METHOD(void, SendBatch, (WTF::Vector<network::mojom::blink::P2PSendPacketPtr>), (override));
    MOCK_METHOD(void, SetOption, (network::P2PSocketOption option, int32_t value), (override));
};

class MockDelegate : public P2PSocketClientDelegate {
public:
    MOCK_METHOD(void, OnOpen, (const net::IPEndPoint&, const net::IPEndPoint&), (override));
    MOCK_METHOD(void, OnSendComplete, (const network::P2PSendPacketMetrics&), (override));
    MOCK_METHOD(void, OnError, (), (override));
    MOCK_METHOD(void, OnDataReceived, (const net::IPEndPoint&, base::span<const uint8_t>, const base::TimeTicks&, rtc::EcnMarking), (override));
};
...
public:
    ...
    virtual ~SocketClientImplTestBase()
    {
        client_.Close();
    }
...
class SocketClientImplParametrizedTest : public SocketClientImplTestBase, public ::testing::TestWithParam<bool> {
public:
    SocketClientImplParametrizedTest()
        : SocketClientImplTestBase(GetParam())
    {
    }
};
...
TEST_P(SocketClientImplParametrizedTest, OnDataReceivedCalled)
{
    ...
    packets.push_back(P2PReceivedPacket::New(data, net::IPEndPoint(), first, ecn));
    packets.push_back(P2PReceivedPacket::New(data, net::IPEndPoint(), second, ecn));
    InSequence s;
...
TEST_P(SocketClientImplParametrizedTest, SendsWithIncreasingPacketId)
{
    ...
    EXPECT_CALL(socket_, Send).WillOnce(WithArgs<1>([&first_info](const network::P2PPacketInfo& info) {
        EXPECT_EQ(info.packet_id, first_info.packet_id + 1);
    }));
    client_.Send(net::IPEndPoint(), std::vector<uint8_t>(1), rtc::PacketOptions());
    client_.Send(net::IPEndPoint(), std::vector<uint8_t>(1), rtc::PacketOptions());
    task_environment_.RunUntilIdle();
}
...
TEST_P(SocketClientImplParametrizedTest, SetsOption)
{
    Open();
    InSequence s;
    EXPECT_CALL(socket_, SetOption(network::P2PSocketOption::P2P_SOCKET_OPT_DSCP, 1));
    EXPECT_CALL(socket_, SetOption(network::P2PSocketOption::P2P_SOCKET_OPT_RCVBUF, 2));
...
INSTANTIATE_TEST_SUITE_P(All, SocketClientImplParametrizedTest, Values(false, true),
    [](const testing::TestParamInfo<bool>& info) { return info.param ? "WithBatching" : "WithoutBatching"; });

class SocketClientImplBatchingTest : public SocketClientImplTestBase, public ::testing::Test {
public:
...
TEST_F(SocketClientImplBatchingTest, TwoPacketBatchUsesSendBatch)
{
    ...
    EXPECT_CALL(socket_,
        SendBatch(ElementsAre(Pointee(Field(&network::mojom::blink::P2PSendPacket::packet_info,
                                  Field(&network::P2PPacketInfo::packet_options, Field(&rtc::PacketOptions::packet_id, 1)))),
            Pointee(Field(&network::mojom::blink::P2PSendPacket::packet_info,
                Field(&network::P2PPacketInfo::packet_options, Field(&rtc::PacketOptions::packet_id, 2)))))));
...
TEST_F(SocketClientImplBatchingTest, TwoPacketBatchWithNonbatchableInterleavedUsesSendBatch)
{
    Open();
    ...
    EXPECT_CALL(socket_,
        SendBatch(ElementsAre(Pointee(Field(&network::mojom::blink::P2PSendPacket::packet_info,
                                  Field(&network::P2PPacketInfo::packet_options, Field(&rtc::PacketOptions::packet_id, 1)))),
            Pointee(Field(
                &network::mojom::blink::P2PSendPacket::packet_info, Field(&network::P2PPacketInfo::packet_options, Field(&rtc::PacketOptions::packet_id, 2)))),
            Pointee(Field(&network::mojom::blink::P2PSendPacket::packet_info,
                Field(&network::P2PPacketInfo::packet_options, Field(&rtc::PacketOptions::packet_id, 3)))))));
