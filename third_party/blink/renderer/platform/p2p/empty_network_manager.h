...
class EmptyNetworkManager : public rtc::NetworkManagerBase, public sigslot::has_slots<> {
public:
    // This class is created on the main thread but used by WebRTC's worker thread
    // |task_runner|.
    PLATFORM_EXPORT explicit EmptyNetworkManager(IpcNetworkManager* network_manager);
    ...
    bool GetDefaultLocalAddress(int family, rtc::IPAddress* ipaddress) const override;
...
private:
    ...
    PLATFORM_EXPORT EmptyNetworkManager(rtc::NetworkManager* network_manager, base::WeakPtr<rtc::NetworkManager> network_manager_for_signaling_thread);
