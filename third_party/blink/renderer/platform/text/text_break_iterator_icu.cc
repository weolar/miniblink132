    ...
    icu::BreakIterator* Take(const AtomicString& locale)
    {
        ...
        if (!iterator) {
            ...
            iterator = icu::BreakIterator::createLineInstance(
                locale_is_empty ? icu::Locale(CurrentTextBreakLocaleID()) : icu::Locale(locale.Utf8().c_str()), open_status);
            ...
            if (!locale_is_empty && U_FAILURE(open_status)) {
                ...
                iterator = icu::BreakIterator::createLineInstance(icu::Locale(CurrentTextBreakLocaleID()), open_status);
            }

            if (U_FAILURE(open_status)) {
                DLOG(ERROR) << "icu::BreakIterator construction failed with status " << open_status;
                return nullptr;
            }
        }
...
void TextFixPointer(const UText* source, UText* destination, const void*& pointer)
{
...
UText* TextClone(UText* destination, const UText* source, UBool deep, UErrorCode* status)
{
    DCHECK(!deep);
    if (U_FAILURE(*status)) {
        return nullptr;
    ...
    const void* chunk_contents = static_cast<const void*>(destination->chunkContents);
    TextFixPointer(source, destination, chunk_contents);
    destination->chunkContents = static_cast<const UChar*>(chunk_contents);
    return destination;
}

int32_t TextExtract(UText*, int64_t, int64_t, UChar*, int32_t, UErrorCode* error_code)
{
...
inline TextContext TextGetContext(const UText* text, int64_t native_index, UBool forward)
{
...
void TextLatin1MoveInPrimaryContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
    ...
    StringImpl::CopyChars(const_cast<UChar*>(text->chunkContents), static_cast<const LChar*>(text->p) + (text->chunkNativeStart - text->b),
        static_cast<unsigned>(text->chunkLength));
}

void TextLatin1SwitchToPrimaryContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
...
void TextLatin1MoveInPriorContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
    DCHECK_EQ(text->chunkContents, text->q);
    DCHECK(forward ? native_index < text->b : native_index <= text->b);
    DCHECK(forward ? native_index < native_length : native_index <= native_length);
    DCHECK(forward ? native_index < native_length : native_index <= native_length);
...
void TextLatin1SwitchToPriorContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
...
inline bool TextInChunkOrOutOfRange(UText* text, int64_t native_index, int64_t native_length, UBool forward, UBool& is_accessible)
{
    if (forward) {
...
void TextInit(UText* text, const UTextFuncs* funcs, const void* string, unsigned length, const UChar* prior_context, int prior_context_length)
{
    text->pFuncs = funcs;
...
UText* TextOpenLatin1(UTextWithBuffer* ut_with_buffer, base::span<const LChar> string, const UChar* prior_context, int prior_context_length, UErrorCode* status)
{
    if (U_FAILURE(*status)) {
        return nullptr;
    }
    ...
    UText* text = utext_setup(&ut_with_buffer->text, sizeof(ut_with_buffer->buffer), status);
    if (U_FAILURE(*status)) {
        DCHECK(!text);
        return nullptr;
    }
...
void TextUTF16MoveInPrimaryContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
...
void TextUTF16SwitchToPrimaryContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
    DCHECK(!text->chunkContents || text->chunkContents == text->q);
...
void TextUTF16MoveInPriorContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
    DCHECK_EQ(text->chunkContents, text->q);
    DCHECK(forward ? native_index < text->b : native_index <= text->b);
    DCHECK(forward ? native_index < native_length : native_index <= native_length);
...
void TextUTF16SwitchToPriorContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
...
UText* TextOpenUTF16(UText* text, base::span<const UChar> string, const UChar* prior_context, int prior_context_length, UErrorCode* status)
{
    if (U_FAILURE(*status)) {
        return nullptr;
    }
...
TextBreakIterator* WordBreakIterator(base::span<const LChar> string)
{
    ...
    if (!break_iter) {
        break_iter = icu::BreakIterator::createWordInstance(icu::Locale(CurrentTextBreakLocaleID()), error_code);
        DCHECK(U_SUCCESS(error_code)) << "ICU could not open a break iterator: " << u_errorName(error_code) << " (" << error_code << ")";
        if (!break_iter) {
            return nullptr;
        }
    }
    ...
    if (U_FAILURE(set_text_status)) {
        DLOG(ERROR) << "BreakIterator::seText failed with status " << set_text_status;
    }
...
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(ThreadSpecific<std::unique_ptr<TextBreakIterator>>, thread_specific, ());

    std::unique_ptr<TextBreakIterator>& iterator = *thread_specific;

    if (!iterator) {
        ICUError error_code;
        iterator = base::WrapUnique(icu::BreakIterator::createCharacterInstance(icu::Locale(CurrentTextBreakLocaleID()), error_code));
        DCHECK(U_SUCCESS(error_code) && iterator) << "ICU could not open a break iterator: " << u_errorName(error_code) << " (" << error_code << ")";
    }
...
PooledBreakIterator AcquireLineBreakIterator(
    base::span<const LChar> string, const AtomicString& locale, const UChar* prior_context = nullptr, unsigned prior_context_length = 0)
{
    PooledBreakIterator iterator { LineBreakIteratorPool::SharedPool().Take(locale) };
    if (!iterator) {
        return nullptr;
    }
...
PooledBreakIterator AcquireLineBreakIterator(
    base::span<const UChar> string, const AtomicString& locale, const UChar* prior_context = nullptr, unsigned prior_context_length = 0)
{
    PooledBreakIterator iterator { LineBreakIteratorPool::SharedPool().Take(locale) };
    if (!iterator) {
        return nullptr;
...
PooledBreakIterator AcquireLineBreakIterator(StringView string, const AtomicString& locale)
{
    if (string.Is8Bit()) {
...
NonSharedCharacterBreakIterator::NonSharedCharacterBreakIterator(base::span<const UChar> buffer)
    : is_8bit_(false)
    , charaters8_(nullptr)
    , offset_(0)
    , length_(0)
...
void NonSharedCharacterBreakIterator::CreateIteratorForBuffer(base::span<const UChar> buffer)
{
...
TextBreakIterator* SentenceBreakIterator(base::span<const UChar> string)
{
    ...
    if (!iterator) {
        iterator = icu::BreakIterator::createSentenceInstance(icu::Locale(CurrentTextBreakLocaleID()), open_status);
        DCHECK(U_SUCCESS(open_status)) << "ICU could not open a break iterator: " << u_errorName(open_status) << " (" << open_status << ")";
        if (!iterator) {
            return nullptr;
        }
    }
...
{
    icu::RuleBasedBreakIterator* rule_based_break_iterator = static_cast<icu::RuleBasedBreakIterator*>(iterator);
...
TextBreakIterator* CursorMovementIterator(base::span<const UChar> string)
{
    ...
    DEFINE_THREAD_SAFE_STATIC_LOCAL(ThreadSpecific<std::unique_ptr<icu::RuleBasedBreakIterator>>, thread_specific, ());
    ...
    if (!iterator) {
        ...
        iterator = std::make_unique<icu::RuleBasedBreakIterator>(icu::UnicodeString(kRules, -1, US_INV), parse_status, open_status);
        DCHECK(U_SUCCESS(open_status)) << "ICU could not open a break iterator: " << u_errorName(open_status) << " (" << open_status << ")";
    }
