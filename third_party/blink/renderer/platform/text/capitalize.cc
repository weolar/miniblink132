...
String Capitalize(const String& string, UChar previous_character)
{
    ...
    string_with_previous[0] = previous_character == kNoBreakSpaceCharacter ? kSpaceCharacter : previous_character;
    ...
    for (end_of_word = boundary->next(); end_of_word != kTextBreakDone; start_of_word = end_of_word, end_of_word = boundary->next()) {
        if (start_of_word) { // Ignore first char of previous string
            result.Append(
                input[start_of_word - 1] == kNoBreakSpaceCharacter ? kNoBreakSpaceCharacter : WTF::unicode::ToTitleCase(string_with_previous[start_of_word]));
        }
