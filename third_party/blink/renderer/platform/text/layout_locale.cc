...
struct PerThreadData {
    HashMap<AtomicString, scoped_refptr<LayoutLocale>, CaseFoldingHashTraits<AtomicString>> locale_map;
...
scoped_refptr<QuotesData> GetQuotesDataForLanguage(const char* locale)
{
    ...
    for (DelimiterConfig delim : delimiters) {
        delimResultLength = ulocdata_getDelimiter(uld, delim.type, delim.result, ucharDelimMaxLength, &status);
...
static hb_language_t ToHarfbuzLanguage(const AtomicString& locale)
{
    ...
    return hb_language_from_string(locale_as_latin1.data(), static_cast<int>(locale_as_latin1.length()));
}
...
// static
const LayoutLocale* LayoutLocale::LocaleForHan(const LayoutLocale* content_locale)
{
...
const LayoutLocale& LayoutLocale::GetDefault()
{
    ...
    if (!data.default_locale) [[unlikely]] {
        ...
        data.default_locale = LayoutLocale::Get(!language.empty() ? language : AtomicString("en"));
    }
...
const LayoutLocale& LayoutLocale::GetSystem()
{
    ...
    if (!data.system_locale) [[unlikely]] {
        ...
        data.system_locale = LayoutLocale::Get(AtomicString(name.Replace('_', '-')));
    }
    return *data.system_locale;
}

scoped_refptr<LayoutLocale> LayoutLocale::CreateForTesting(const AtomicString& locale)
{
...
void LayoutLocale::SetHyphenationForTesting(const AtomicString& locale_string, scoped_refptr<Hyphenation> hyphenation)
{
...
scoped_refptr<QuotesData> LayoutLocale::GetQuotesData() const
{
    ...
    UEnumeration* ulocales = uloc_openAvailableByType(ULOC_AVAILABLE_DEFAULT, &status);
    if (U_FAILURE(status)) {
...
AtomicString LayoutLocale::LocaleWithBreakKeyword(LineBreakStrictness strictness, bool use_phrase) const
{
    if (string_.empty())
        return string_;
    ...
    public:
        ...
        {
            ICUError status;
            int32_t length_needed = uloc_setKeywordValue(keyword_name, value, buffer_.data(), buffer_.size(), &status);
            if (U_SUCCESS(status)) {
