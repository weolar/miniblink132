...
TEST(BidiParagraph, GetLogicalRuns)
{
    ...
    EXPECT_THAT(runs, ElementsAre(BidiParagraph::Run(0, 4, 1), BidiParagraph::Run(4, 7, 2), BidiParagraph::Run(7, 11, 1)));
}
...
static struct BaseDirectionData {
...
} base_direction_data[] = { { u"A", TextDirection::kLtr }, { u"\u05D0", TextDirection::kRtl },
    ...
    { u"!A", TextDirection::kLtr }, { u"!A\u05D0", TextDirection::kLtr }, { u"!\u05D0Z", TextDirection::kRtl },
    // Strong characters after a segment break should be ignored.
    { u"!\nA", std::nullopt, TextDirection::kLtr }, { u"!\nA\u05D0", std::nullopt, TextDirection::kLtr },
    { u"!\n\u05D0Z", std::nullopt, TextDirection::kRtl } };
class BaseDirectionTest : public testing::TestWithParam<BaseDirectionData> { };
INSTANTIATE_TEST_SUITE_P(BidiParagraph, BaseDirectionTest, testing::ValuesIn(base_direction_data));
...
TEST_P(BaseDirectionTest, Data)
{
    ...
    EXPECT_EQ(BidiParagraph::BaseDirectionForString(text, Character::IsLineFeed), test.direction_line_feed) << text;

    // Test without stop characters.
    EXPECT_EQ(BidiParagraph::BaseDirectionForString(text), test.direction_no_stop ? test.direction_no_stop : test.direction_line_feed) << text;
    ...
    if (text.IsAllSpecialCharacters<[](UChar ch) { return ch <= 0x00FF; }>()) {
        ...
        // Test when the search stops at Line Feed.
        EXPECT_EQ(BidiParagraph::BaseDirectionForString(text8, Character::IsLineFeed), test.direction_line_feed) << text;

        // Test without stop characters.
        EXPECT_EQ(BidiParagraph::BaseDirectionForString(text8), test.direction_no_stop ? test.direction_no_stop : test.direction_line_feed) << text;
    }
}
