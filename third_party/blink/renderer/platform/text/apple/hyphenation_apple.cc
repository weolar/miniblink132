...
public:
    ...
    wtf_size_t LastHyphenLocation(const StringView& text, wtf_size_t before_index) const override
    {
        ...
        before_index = std::min<wtf_size_t>(before_index, text.length() - MinSuffixLength() + 1);

        const CFIndex result = CFStringGetHyphenationLocationBeforeIndex(
            text.ToString().Impl()->CreateCFString().get(), before_index, CFRangeMake(0, text.length()), 0, locale_cf_.get(), 0);
    ...
    wtf_size_t FirstHyphenLocation(const StringView& text, wtf_size_t after_index) const override
    {
        if (!ShouldHyphenateWord(text)) {
            return 0;
        }
...
{
    base::apple::ScopedCFTypeRef<CFStringRef> locale_cf_string(locale.Impl()->CreateCFString());
    base::apple::ScopedCFTypeRef<CFLocaleRef> locale_cf(CFLocaleCreate(kCFAllocatorDefault, locale_cf_string.get()));
    if (!CFStringIsHyphenationAvailableForLocale(locale_cf.get())) {
        return nullptr;
    }
    scoped_refptr<Hyphenation> hyphenation(base::AdoptRef(new HyphenationCF(locale_cf)));
