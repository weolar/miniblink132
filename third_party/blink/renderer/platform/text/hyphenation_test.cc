...
public:
    wtf_size_t LastHyphenLocation(const StringView&, wtf_size_t before_index) const override
    {
        return 0;
    }
};
...
protected:
    ...
    Vector<wtf_size_t> FirstHyphenLocations(StringView word, const Hyphenation& hyphenation) const
    {
    ...
    Vector<wtf_size_t> LastHyphenLocations(StringView word, const Hyphenation& hyphenation) const
    {
        Vector<wtf_size_t> indexes;
        const wtf_size_t word_len = word.length();
        for (wtf_size_t i = 0; i < word_len; ++i)
...
#if defined(USE_MINIKIN_HYPHENATION)
    void TestWordToHyphenate(StringView text, StringView expected, unsigned expected_num_leading_chars)
    {
        unsigned num_leading_chars;
        const StringView result = HyphenationMinikin::WordToHyphenate(text, &num_leading_chars);
        ...
        text16.Ensure16Bit();
        const StringView result16 = HyphenationMinikin::WordToHyphenate(text16, &num_leading_chars);
...
TEST_F(HyphenationTest, Get)
{
    ...
    EXPECT_EQ(hyphenation.get(), LayoutLocale::Get(local_en_us)->GetHyphenation());
...
TEST_F(HyphenationTest, MapLocale)
{
    ...
    EXPECT_EQ(HyphenationMinikin::MapLocale(AtomicString("de-de-xyz")), "de-1996");
    EXPECT_EQ(HyphenationMinikin::MapLocale(AtomicString("de-li")), "de-1996");
    EXPECT_EQ(HyphenationMinikin::MapLocale(AtomicString("de-li-1901")), "de-ch-1901");
...
{
    scoped_refptr<Hyphenation> hyphenation = GetHyphenation(AtomicString("en-us"));
#if BUILDFLAG(IS_ANDROID)
    ...
    for (wtf_size_t i = 1; i < locations.size(); i++) {
        ASSERT_GT(locations[i - 1], locations[i]) << "hyphenLocations must return locations in the descending order";
    }
...
TEST_F(HyphenationTest, LeadingSpaces)
{
    ...
    EXPECT_THAT(hyphenation->HyphenLocations(multi_leading_spaces), ElementsAre(9, 7));
...
TEST_F(HyphenationTest, NonLetters)
{
    ...
    String non_letters("**********");
    EXPECT_EQ(0u, hyphenation->LastHyphenLocation(non_letters, non_letters.length()));
...
TEST_F(HyphenationTest, English)
{
    ...
    EXPECT_THAT(locations, testing::AnyOf(ElementsAreArray({ 6, 2 }), ElementsAreArray({ 7, 6, 2 })));
}

TEST_F(HyphenationTest, German)
{
    scoped_refptr<Hyphenation> hyphenation = GetHyphenation(AtomicString("de-1996"));
#if BUILDFLAG(IS_ANDROID)
    // Hyphenation is available only for Android M MR1 or later.
    if (!hyphenation)
        return;
    ...
    Vector<wtf_size_t, 8> locations = hyphenation->HyphenLocations("konsonantien");
    ...
    locations = hyphenation->HyphenLocations("B"
...
TEST_F(HyphenationTest, SetLimits)
{
    ...
    EXPECT_THAT(en->MinWordLength(), std::max(prefix + suffix, Hyphenation::kDefaultMinWordLength));

    // If the `suffix` is missing, it is the same as the `prefix`.
    en->SetLimits(prefix, 0, 0);
    EXPECT_THAT(en->MinPrefixLength(), prefix);
    EXPECT_THAT(en->MinSuffixLength(), prefix);
    EXPECT_THAT(en->MinWordLength(), std::max(prefix + prefix, Hyphenation::kDefaultMinWordLength));

    // If the `prefix` is missing, it is `auto`.
    en->SetLimits(0, suffix, 0);
    EXPECT_THAT(en->MinPrefixLength(), Hyphenation::kDefaultMinPrefixLength);
    EXPECT_THAT(en->MinSuffixLength(), suffix);
    EXPECT_THAT(en->MinWordLength(), std::max(Hyphenation::kDefaultMinPrefixLength + suffix, Hyphenation::kDefaultMinWordLength));
...
TEST_F(HyphenationTest, Limits)
{
    ...
    EXPECT_THAT(FirstHyphenLocations("example", *en), ElementsAre(2, 2, 4, 4, 0, 0, 0));
    EXPECT_THAT(LastHyphenLocations("example", *en), ElementsAre(0, 0, 0, 2, 2, 4, 4));
    ...
    EXPECT_THAT(FirstHyphenLocations("example", *en), ElementsAre(4, 4, 4, 4, 0, 0, 0));
    EXPECT_THAT(LastHyphenLocations("example", *en), ElementsAre(0, 0, 0, 0, 0, 4, 4));

    // Limiting the suffix >= 4 disables the last hyphenation point.
    en->SetLimits(0, 4, 0);
    EXPECT_THAT(en->HyphenLocations("example"), ElementsAre(2));
    EXPECT_THAT(FirstHyphenLocations("example", *en), ElementsAre(2, 2, 0, 0, 0, 0, 0));
    EXPECT_THAT(LastHyphenLocations("example", *en), ElementsAre(0, 0, 0, 2, 2, 2, 2));

    // Applying both limitations results in no hyphenation points.
    en->SetLimits(3, 4, 0);
    EXPECT_THAT(en->HyphenLocations("example"), ElementsAre());
    EXPECT_THAT(FirstHyphenLocations("example", *en), ElementsAre(0, 0, 0, 0, 0, 0, 0));
    EXPECT_THAT(LastHyphenLocations("example", *en), ElementsAre(0, 0, 0, 0, 0, 0, 0));
