...
void Hyphenation::SetLimits(wtf_size_t min_prefix_length, wtf_size_t min_suffix_length, wtf_size_t min_word_length)
{
    if (min_prefix_length) {
        ...
        min_suffix_length_ = min_suffix_length ? min_suffix_length : min_prefix_length;
    } else {
        min_prefix_length_ = kDefaultMinPrefixLength;
        min_suffix_length_ = min_suffix_length ? min_suffix_length : kDefaultMinSuffixLength;
    }
    min_word_length_ = std::max(min_word_length ? min_word_length : kDefaultMinWordLength, min_prefix_length_ + min_suffix_length_);
...
wtf_size_t Hyphenation::FirstHyphenLocation(const StringView& text, wtf_size_t after_index) const
{
    DCHECK_GE(MinPrefixLength(), 1u);
    after_index = std::max(after_index, static_cast<wtf_size_t>(MinPrefixLength() - 1));
...
Vector<wtf_size_t, 8> Hyphenation::HyphenLocations(const StringView& text) const
{
    ...
    while ((hyphen_location = LastHyphenLocation(text, hyphen_location)) >= min_prefix_len) {
