...
//
using PooledBreakIterator = std::unique_ptr<TextBreakIterator, ReturnBreakIteratorToPool>;

//
// Returns a new instance from a pool, or create a new one if the pool is empty.
//
PLATFORM_EXPORT PooledBreakIterator AcquireLineBreakIterator(StringView, const AtomicString& locale);
...
PLATFORM_EXPORT TextBreakIterator* CursorMovementIterator(base::span<const UChar>);
...
PLATFORM_EXPORT TextBreakIterator* SentenceBreakIterator(base::span<const UChar>);
...
public:
    explicit LazyLineBreakIterator(const String& string, const LayoutLocale* locale = nullptr, LineBreakType break_type = LineBreakType::kNormal)
        : string_(string)
        , locale_(locale)
    ...
    LazyLineBreakIterator(const String& string, const AtomicString& locale, LineBreakType break_type = LineBreakType::kNormal)
    ...
    LazyLineBreakIterator(const LazyLineBreakIterator& other, String string)
        : LazyLineBreakIterator(std::move(string), other.Locale(), other.BreakType())
    {
    ...
    void ResetStringAndReleaseIterator(String string, const LayoutLocale* locale)
    {
        string_ = string;
        start_offset_ = 0;
        SetLocale(locale);
    ...
    LineBreakType BreakType() const
    {
        return break_type_;
    }
    void SetBreakType(LineBreakType break_type);
    BreakSpaceType BreakSpace() const
    {
        return break_space_;
    }
    void SetBreakSpace(BreakSpaceType break_space)
    {
        break_space_ = break_space;
    }
    ...
    bool IsSoftHyphenEnabled() const
    {
        return !disable_soft_hyphen_;
    }
    void EnableSoftHyphen(bool value)
    {
        disable_soft_hyphen_ = !value;
    }
    ...
    {
        return ch == kSpaceCharacter || ch == kTabulationCharacter || ch == kNewlineCharacter;
    }

private:
    FRIEND_TEST_ALL_PREFIXES(TextBreakIteratorTest, Strictness);

    template <typename CharacterType> struct Context;
    ...
    TextBreakIterator* GetIterator() const
    {
        ...
        iterator_ = AcquireLineBreakIterator(StringView { string_, start_offset_ }, locale);
        return iterator_.get();
    }

    template <typename CharacterType, LineBreakType, BreakSpaceType> unsigned NextBreakablePosition(unsigned pos, const CharacterType* str, unsigned len) const;
    template <typename CharacterType, LineBreakType> unsigned NextBreakablePosition(unsigned pos, const CharacterType* str, unsigned len) const;
    template <LineBreakType> unsigned NextBreakablePosition(unsigned pos, unsigned len) const;
...
inline const AtomicString& LazyLineBreakIterator::LocaleWithKeyword() const
{
    if (!locale_with_keyword_) {
        ...
        } else if (strictness_ == LineBreakStrictness::kDefault && break_type_ != LineBreakType::kPhrase) {
            locale_with_keyword_ = locale_->LocaleString();
        } else {
            locale_with_keyword_ = locale_->LocaleWithBreakKeyword(strictness_, break_type_ == LineBreakType::kPhrase);
        }
...
inline void LazyLineBreakIterator::SetBreakType(LineBreakType break_type)
{
    if (break_type_ != break_type) {
        if (break_type_ == LineBreakType::kPhrase || break_type == LineBreakType::kPhrase) {
            InvalidateLocaleWithKeyword();
        }
        break_type_ = break_type;
    }
...
inline void LazyLineBreakIterator::SetStrictness(LineBreakStrictness strictness)
{
...
public:
    ...
    NonSharedCharacterBreakIterator(const NonSharedCharacterBreakIterator&) = delete;
    NonSharedCharacterBreakIterator& operator=(const NonSharedCharacterBreakIterator&) = delete;
...
private:
    ...
    {
        DCHECK(is_8bit_);
        return charaters8_[offset] == '\r' && offset + 1 < length_ && charaters8_[offset + 1] == '\n';
    }
...
// rules.
PLATFORM_EXPORT void GraphemesClusterList(const StringView& text, Vector<unsigned>* graphemes);
