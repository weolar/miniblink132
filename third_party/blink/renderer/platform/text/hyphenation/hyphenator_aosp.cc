...
void Hyphenator::hyphenate(Vector<uint8_t>* result, const uint16_t* word, wtf_size_t len)
{
    result->clear();
    result->resize(len);
...
void Hyphenator::hyphenateSoft(uint8_t* result, const uint16_t* word, wtf_size_t len)
{
    result[0] = 0;
...
bool Hyphenator::alphabetLookup(uint16_t* alpha_codes, const uint16_t* word, wtf_size_t len)
{
 ...
 * end.
 **/
void Hyphenator::hyphenateFromCodes(uint8_t* result, const uint16_t* codes, wtf_size_t len)
{
    const Header* header = getHeader();
