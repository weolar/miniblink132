...
static mojo::Remote<mojom::blink::Hyphenation> ConnectToRemoteService()
{
    ...
    Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(service.BindNewPipeAndPassReceiver());
    return service;
}

static mojom::blink::Hyphenation* GetService()
{
    DEFINE_STATIC_LOCAL(mojo::Remote<mojom::blink::Hyphenation>, service, (ConnectToRemoteService()));
    return service.get();
}
...
Vector<uint8_t> HyphenationMinikin::Hyphenate(const StringView& text) const
{
    ...
    if (text.Is8Bit()) {
        ...
        hyphenator_->hyphenate(&result, reinterpret_cast<const uint16_t*>(text16_bit.Characters16()), text16_bit.length());
    } else {
        hyphenator_->hyphenate(&result, reinterpret_cast<const uint16_t*>(text.Characters16()), text.length());
    }
    return result;
}
...
using LocaleMap = HashMap<AtomicString, const HyphenatorLocaleData*, CaseFoldingHashTraits<AtomicString>>;
...
scoped_refptr<HyphenationMinikin> HyphenationMinikin::FromFileForTesting(const AtomicString& locale, base::File file)
{
    scoped_refptr<HyphenationMinikin> hyphenation(base::AdoptRef(new HyphenationMinikin));
