...
TEST(TextEncodingDetectorTest, RespectIso2022Jp)
{
    ...
    bool result = DetectTextEncoding(base::as_byte_span(iso2022jp), nullptr, NullURL(), nullptr, &encoding);
    EXPECT_TRUE(result);
...
TEST(TextEncodingDetectorTest, Ignore7BitEncoding)
{
    ...
    std::string hz_gb2312 = " ~{\x54\x42\x31\x7D\x37\x22\x55\x39\x35\x3D\x3D\x71~} abc";
    WTF::TextEncoding encoding;
    bool result = DetectTextEncoding(base::as_byte_span(hz_gb2312), nullptr, NullURL(), nullptr, &encoding);
    EXPECT_TRUE(result);
...
{
    std::string pseudo_jpg = "\xff\xd8\xff\xe0\x00\x10JFIF foo bar baz\xff\xe1\x00\xa5"
    ...
    WTF::TextEncoding encoding;
    bool result = DetectTextEncoding(base::as_byte_span(pseudo_jpg), nullptr, NullURL(), nullptr, &encoding);
    EXPECT_TRUE(result);
    EXPECT_EQ(WTF::TextEncoding("US-ASCII"), encoding);
}
...
TEST(TextEncodingDetectorTest, UrlHintHelpsEUCJP)
{
    std::string eucjp_bytes = "<TITLE>"
                              ...
                              "TITLE>";
    WTF::TextEncoding encoding;
    bool result = DetectTextEncoding(base::as_byte_span(eucjp_bytes), nullptr, NullURL(), nullptr, &encoding);
    EXPECT_TRUE(result);
    EXPECT_EQ(WTF::TextEncoding("GBK"), encoding) << "Without language hint, it's detected as GBK";

    KURL url_jp_domain("http://example.co.jp/");
    result = DetectTextEncoding(base::as_byte_span(eucjp_bytes), nullptr, url_jp_domain, nullptr, &encoding);
    EXPECT_TRUE(result);
    EXPECT_EQ(WTF::TextEncoding("EUC-JP"), encoding) << "With URL hint including '.jp', it's detected as EUC-JP";
}
...
TEST(TextEncodingDetectorTest, LanguageHintHelpsEUCJP)
{
    ...
    EXPECT_TRUE(result);
    EXPECT_EQ(WTF::TextEncoding("GBK"), encoding) << "Language hint doesn't help for normal URL. Should be detected as GBK";

    KURL file_url("file:///text.txt");
    result = DetectTextEncoding(base::as_byte_span(eucjp_bytes), nullptr, file_url, "ja", &encoding);
    EXPECT_TRUE(result);
    EXPECT_EQ(WTF::TextEncoding("EUC-JP"), encoding) << "Language hint works for file resource. Should be detected as EUC-JP";
}
...
TEST(TextEncodingDetectorTest, UTF8DetectionShouldFail)
{
    ...
    WTF::TextEncoding encoding;
    bool result = DetectTextEncoding(base::as_byte_span(utf8_bytes), nullptr, NullURL(), nullptr, &encoding);
    EXPECT_FALSE(result);
}
...
TEST(TextEncodingDetectorTest, RespectUTF8DetectionForFileResource)
{
    ...
    WTF::TextEncoding encoding;
    KURL file_url("file:///text.txt");
    bool result = DetectTextEncoding(base::as_byte_span(utf8_bytes), nullptr, file_url, nullptr, &encoding);
    EXPECT_TRUE(result);
}
