...
#define CHECK_U_ERROR(error, name) CHECK(U_SUCCESS(error)) << name << ": (" << error << ")" << u_errorName(error)
...
void InitializeIcu(const char* exec_path)
{
    ...
    std::copy(std::istreambuf_iterator<char>(data_ifstream), std::istreambuf_iterator<char>(), std::back_inserter(icu_data));
    UErrorCode error = U_ZERO_ERROR;
...
public:
    ...
    CharacterPropertyValues()
        : values_(new CharacterProperty[kSize])
    {
        Initialize();
    }

    CharacterProperty operator[](UChar32 index) const
    {
...
#define SET(name)                                                                                                                                              \
    SetForRanges(name##Ranges, std::size(name##Ranges), CharacterProperty::name);                                                                              \
    SetForValues(name##Array, std::size(name##Array), CharacterProperty::name);
        ...
        SET(kIsBidiControl);
#undef SET
        SetForRanges(kIsHangulRanges, std::size(kIsHangulRanges), CharacterProperty::kIsHangul);
        SetHanKerning();
    }
    ...
    void SetHanKerning()
    {
        ...
        SetForUnicodeSet("[[:blk=CJK_Symbols:][:ea=F:] & [:gc=Ps:]]", HanKerningCharType::kOpen);
        SetForUnicodeSet("[[:blk=CJK_Symbols:][:ea=F:] & [:gc=Pe:]]", HanKerningCharType::kClose);
        SetForUnicodeSet("[[:gc=Ps:] - [:blk=CJK_Symbols:] - [:ea=F:]]", HanKerningCharType::kOpenNarrow);
        SetForUnicodeSet("[[:gc=Pe:] - [:blk=CJK_Symbols:] - [:ea=F:]]", HanKerningCharType::kCloseNarrow);
    }

    static CharacterProperty ToCharacterProperty(HanKerningCharType value)
    {
        CHECK_EQ((static_cast<unsigned>(value) & ~static_cast<unsigned>(CharacterProperty::kHanKerningMask)), 0u);
        return static_cast<CharacterProperty>(static_cast<unsigned>(value) << static_cast<unsigned>(CharacterProperty::kHanKerningShift));
    }

    void SetForUnicodeSet(const char* pattern, HanKerningCharType type)
    {
        SetForUnicodeSet(pattern, ToCharacterProperty(type), CharacterProperty::kHanKerningShiftedMask);
    }

    // For `patterns`, see:
    // https://unicode-org.github.io/icu/userguide/strings/unicodeset.html#unicodeset-patterns
    void SetForUnicodeSet(const char* pattern, CharacterProperty value, CharacterProperty mask)
    {
        UErrorCode error = U_ZERO_ERROR;
    ...
    void SetForRanges(const UChar32* ranges, size_t length, CharacterProperty value)
    {
    ...
    void SetForValues(const UChar32* begin, size_t length, CharacterProperty value)
    {
    ...
    void Set(UChar32 ch, HanKerningCharType type)
    {
        ...
        CHECK_EQ(static_cast<unsigned>(values_[ch] & CharacterProperty::kHanKerningShiftedMask), 0u);
        values_[ch] |= value;
    }
...
static void GenerateUTrieSerialized(FILE* fp, int32_t size, base::span<uint8_t> array)
{
    fprintf(fp,
        "#include <cstdint>\n\n"
        "namespace blink {\n\n"
...
static void GenerateCharacterPropertyData(FILE* fp)
{
    ...
    UErrorCode error = U_ZERO_ERROR;
    std::unique_ptr<UMutableCPTrie, decltype(&umutablecptrie_close)> trie(umutablecptrie_open(0, 0, &error), umutablecptrie_close);
    ...
    std::unique_ptr<UCPTrie, decltype(&ucptrie_close)> immutable_trie(
        umutablecptrie_buildImmutable(trie.get(), UCPTrieType::UCPTRIE_TYPE_FAST, UCPTrieValueWidth::UCPTRIE_VALUE_BITS_16, &error), ucptrie_close);

    assert(error == U_ZERO_ERROR);

    int32_t serialized_size = ucptrie_toBinary(immutable_trie.get(), nullptr, 0, &error);
    error = U_ZERO_ERROR;

    base::HeapArray<uint8_t> serialized = base::HeapArray<uint8_t>::Uninit(serialized_size);
    ...
    serialized_size = ucptrie_toBinary(immutable_trie.get(), serialized.data(), serialized.size(), &error);
    assert(error == U_ZERO_ERROR);

    GenerateUTrieSerialized(fp, serialized_size, base::span<uint8_t>(serialized.data(), serialized.size()));
}
...
private:
    ...
    void FillFromIcu()
    {
        ...
        icu::BreakIterator* break_iterator = icu::BreakIterator::createLineInstance(locale, status);
...
void InvokeGenerator(int index, int argc, char** argv, void (*generator)(FILE*))
{
    if (index >= argc) {
        return;
    }
