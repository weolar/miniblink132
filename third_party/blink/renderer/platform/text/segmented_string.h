...
public:
    ...
    explicit SegmentedSubstring(const String& str)
        : string_(str)
    {
        ...
        if (len) {
            if (string_.Is8Bit()) {
                ...
                data_last_char_ = reinterpret_cast<const LChar*>(data_.string16_ptr + len - 1);
            }
        } else {
            is_8bit_ = true;
    ...
    {
        return !do_not_exclude_line_numbers_;
    }
    ...
    {
        do_not_exclude_line_numbers_ = false;
    }
...
public:
    ...
    ALWAYS_INLINE void UpdateLineNumber()
    {
        if (current_string_.DoNotExcludeLineNumbers()) [[likely]] {
            ...
            number_of_characters_consumed_prior_to_current_line_ = NumberOfCharactersConsumed() + 1;
        }
    }
    ...
    {
        DCHECK_EQ(WTF::unicode::FoldCase(CurrentChar()), WTF::unicode::FoldCase(expected_character));
        return Advance();
    }
    ...
    ALWAYS_INLINE int NumberOfCharactersConsumed() const
    {
        ...
        return number_of_characters_consumed_prior_to_current_string_ + current_string_.NumberOfCharactersConsumed() - number_of_pushed_characters;
    }
    ...
    void SetCurrentPosition(OrdinalNumber line, OrdinalNumber column_aftre_prolog, int prolog_length);
...
private:
    ...
    inline LookAheadResult LookAheadInline(const String& string, TextCaseSensitivity case_sensitivity)
    {
        if (string.length() <= static_cast<unsigned>(current_string_.length())) {
            StringView current_substring = current_string_.CurrentSubString(string.length());
    ...
    LookAheadResult LookAheadSlowCase(const String& string, TextCaseSensitivity case_sensitivity)
    {
        unsigned count = string.length();
        if (count > length())
            return kNotEnoughCharacters;
        base::span<UChar> consumed_characters;
        String consumed_string = String::CreateUninitialized(count, consumed_characters);
