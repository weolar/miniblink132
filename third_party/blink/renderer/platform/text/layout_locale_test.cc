...
TEST(LayoutLocaleTest, Get)
{
    ...
    EXPECT_STRCASEEQ("en-us", LayoutLocale::Get(AtomicString("en-us"))->LocaleString().Ascii().c_str());
    EXPECT_STRCASEEQ("ja-jp", LayoutLocale::Get(AtomicString("ja-jp"))->LocaleString().Ascii().c_str());
...
#define EXPECT_JAPANESE USCRIPT_KATAKANA_OR_HIRAGANA, "ja", USCRIPT_KATAKANA_OR_HIRAGANA
#define EXPECT_KOREAN USCRIPT_HANGUL, "ko", USCRIPT_HANGUL
#define EXPECT_SIMPLIFIED_CHINESE USCRIPT_SIMPLIFIED_HAN, "zh-Hans", USCRIPT_SIMPLIFIED_HAN
#define EXPECT_TRADITIONAL_CHINESE USCRIPT_TRADITIONAL_HAN, "zh-Hant", USCRIPT_TRADITIONAL_HAN
...
INSTANTIATE_TEST_SUITE_P(LayoutLocaleTest, LocaleTestDataFixture, testing::ValuesIn(locale_test_data));

TEST_P(LocaleTestDataFixture, Script)
{
    const auto& test = GetParam();
    scoped_refptr<LayoutLocale> locale = LayoutLocale::CreateForTesting(AtomicString(test.locale));
    EXPECT_EQ(test.script, locale->GetScript()) << test.locale;
    EXPECT_EQ(test.script_for_han.has_value(), locale->HasScriptForHan()) << test.locale;
...
TEST(LayoutLocaleTest, BreakKeyword)
{
    ...
    for (const auto& test : tests) {
        scoped_refptr<LayoutLocale> locale = LayoutLocale::CreateForTesting(AtomicString(test.locale));
        EXPECT_EQ(test.expected, locale->LocaleWithBreakKeyword(test.strictness, test.use_phrase))
            << String::Format("'%s' with line-break %d, phrase=%d should be '%s'", test.locale, static_cast<int>(test.strictness),
...
TEST(LayoutLocaleTest, GetQuotesData)
{
    ...
    struct {
        ...
        { "en", enQuotes }, { "fr", frQuotes }, { "fr-CA", frCAQuotes }, { "fr-DNE", frQuotes }, // use fr
    };
    for (const auto& test : tests) {
        scoped_refptr<LayoutLocale> locale = LayoutLocale::CreateForTesting(AtomicString(test.locale));
...
TEST(LayoutLocaleTest, ExistingKeywordName)
{
    ...
    for (auto* const test : tests) {
        scoped_refptr<LayoutLocale> locale = LayoutLocale::CreateForTesting(AtomicString(test));
        EXPECT_EQ(test, locale->LocaleWithBreakKeyword(LineBreakStrictness::kNormal));
    }
}
...
TEST(LayoutLocaleTest, AcceptLanguagesChanged)
{
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_STREQ(test.locale, locale->LocaleForHanForSkFontMgr()) << test.accept_languages;
    }
}
