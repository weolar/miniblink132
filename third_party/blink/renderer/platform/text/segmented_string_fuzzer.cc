...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    auto gen_str = [&fuzzed_data](size_t max_length) {
        ...
        while (fuzzed_data.ConsumeBool()) {
            auto offset = fuzzed_data.ConsumeIntegralInRange<unsigned int>(0, str.length() - 1);
            if (!offset) {
                break;
            }
    ...
    while (!finished) {
        operation op = static_cast<operation>(fuzzed_data.ConsumeIntegralInRange<int>(operation::kOpFinish, operation::kOpLast - 1));
        String character;
        switch (op) {
        case kOpFinish:
            finished = true;
