...
bool BidiParagraph::SetParagraph(const String& text, std::optional<TextDirection> base_direction)
{
    DCHECK(!text.IsNull());
    DCHECK(!ubidi_);
    ...
    ICUError error;
    ubidi_setPara(ubidi_.get(), text.Characters16(), text.length(), para_level, nullptr, &error);
    if (U_FAILURE(error)) {
        NOTREACHED();
    }
...
// static
template <> std::optional<TextDirection> BidiParagraph::BaseDirectionForString(base::span<const LChar> text, bool (*stop_at)(UChar))
{
...
// static
template <> std::optional<TextDirection> BidiParagraph::BaseDirectionForString(base::span<const UChar> text, bool (*stop_at)(UChar))
{
...
// static
std::optional<TextDirection> BidiParagraph::BaseDirectionForString(const StringView& text, bool (*stop_at)(UChar))
{
    return text.Is8Bit() ? BaseDirectionForString(text.Span8(), stop_at) : BaseDirectionForString(text.Span16(), stop_at);
}

// static
String BidiParagraph::StringWithDirectionalOverride(const StringView& text, TextDirection direction)
{
    StringBuilder builder;
    builder.Reserve16BitCapacity(text.length() + 2);
    builder.Append(IsLtr(direction) ? kLeftToRightOverrideCharacter : kRightToLeftOverrideCharacter);
    builder.Append(text);
...
// static
void BidiParagraph::IndicesInVisualOrder(const Vector<UBiDiLevel, 32>& levels, Vector<int32_t, 32>* indices_in_visual_order_out)
{
