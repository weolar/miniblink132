...
public:
    TrackObserver(const scoped_refptr<base::SingleThreadTaskRunner>& main_thread, const scoped_refptr<webrtc::MediaStreamTrackInterface>& track);
    TrackObserver(const TrackObserver&) = delete;
    TrackObserver& operator=(const TrackObserver&) = delete;
    ~TrackObserver();

    using OnChangedCallback = base::RepeatingCallback<void(webrtc::MediaStreamTrackInterface::TrackState)>;
