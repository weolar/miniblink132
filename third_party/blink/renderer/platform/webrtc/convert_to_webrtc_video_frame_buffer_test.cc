...
class ConvertToWebRtcVideoFrameBufferParamTest : public ::testing::TestWithParam<std::tuple<media::VideoFrame::StorageType, media::VideoPixelFormat>> {
protected:
    scoped_refptr<WebRtcVideoFrameAdapter::SharedResources> resources_ = base::MakeRefCounted<WebRtcVideoFrameAdapter::SharedResources>(nullptr);
};
...
TEST_P(ConvertToWebRtcVideoFrameBufferParamTest, ToI420)
{
    ...
    scoped_refptr<media::VideoFrame> frame = CreateTestFrame(kCodedSize, kVisibleRect, kNaturalSize, storage_type, pixel_format, base::TimeDelta());
    rtc::scoped_refptr<webrtc::VideoFrameBuffer> frame_buffer = ConvertToWebRtcVideoFrameBuffer(std::move(frame), resources_);
...
INSTANTIATE_TEST_SUITE_P(
    ConvertToWebRtcVideoFrameBufferParamTest, ConvertToWebRtcVideoFrameBufferParamTest, ::testing::ValuesIn(TestParams()), [](const auto& info) {
...
TEST(ConvertToWebRtcVideoFrameBufferTest, ToI420ADownScale)
{
    ...
    const gfx::Rect kVisibleRect(0, 120, 1280, 720);
    const gfx::Size kNaturalSize(640, 360);
    auto resources = base::MakeRefCounted<WebRtcVideoFrameAdapter::SharedResources>(nullptr);
    ...
    auto owned_memory_frame = CreateTestFrame(
        kCodedSize, kVisibleRect, kNaturalSize, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_I420A, base::TimeDelta());
    rtc::scoped_refptr<webrtc::VideoFrameBuffer> owned_memory_frame_buffer = ConvertToWebRtcVideoFrameBuffer(std::move(owned_memory_frame), resources);
...
TEST(ConvertToWebRtcVideoFrameBufferTest, Nv12WrapsGmbWhenNoScalingNeeededWithFeature)
{
    const gfx::Size kCodedSize(1280, 960);
    const gfx::Rect kVisibleRect(0, 120, 1280, 720);
    ...
    rtc::scoped_refptr<webrtc::VideoFrameBuffer> gmb_frame_buffer = ConvertToWebRtcVideoFrameBuffer(std::move(gmb_frame), resources);
...
TEST(ConvertToWebRtcVideoFrameBufferTest, Nv12ScalesGmbWithFeature)
{
    ...
    // The adapter should report width and height from the natural size for
    // VideoFrame backed by GpuMemoryBuffer.
    rtc::scoped_refptr<webrtc::VideoFrameBuffer> gmb_frame_buffer = ConvertToWebRtcVideoFrameBuffer(gmb_frame, resources);
    EXPECT_EQ(gmb_frame_buffer->width(), kNaturalSize.width());
    EXPECT_EQ(gmb_frame_buffer->height(), kNaturalSize.height());
...
TEST(ConvertToWebRtcVideoFrameBufferTest, Nv12OwnedMemoryFrame)
{
    ...
    // VideoFrame backed by owned memory.
    auto owned_memory_frame = CreateTestFrame(
        kCodedSize, kVisibleRect, kNaturalSize, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_NV12, base::TimeDelta());
    rtc::scoped_refptr<webrtc::VideoFrameBuffer> owned_memory_frame_buffer = ConvertToWebRtcVideoFrameBuffer(std::move(owned_memory_frame), resources);
    EXPECT_EQ(owned_memory_frame_buffer->width(), kNaturalSize.width());
    EXPECT_EQ(owned_memory_frame_buffer->height(), kNaturalSize.height());
...
TEST(ConvertToWebRtcVideoFrameBufferTest, TextureFrameIsBlackWithNoSharedResources)
{
    const gfx::Size kCodedSize(1280, 960);
    const gfx::Rect kVisibleRect(0, 120, 1280, 720);
    const gfx::Size kNaturalSize(640, 360);
    ...
    // VideoFrame backed by owned memory.
    auto owned_memory_frame = CreateTestFrame(
        kCodedSize, kVisibleRect, kNaturalSize, media::VideoFrame::STORAGE_OPAQUE, media::VideoPixelFormat::PIXEL_FORMAT_NV12, base::TimeDelta());
    rtc::scoped_refptr<webrtc::VideoFrameBuffer> frame_buffer = ConvertToWebRtcVideoFrameBuffer(std::move(owned_memory_frame), nullptr);
...
TEST(ConvertToWebRtcVideoFrameBufferTest, ConvertsTextureFrameWithSharedResources)
{
    const gfx::Size kCodedSize(1280, 960);
    const gfx::Rect kVisibleRect(0, 120, 1280, 720);
    const gfx::Size kNaturalSize(640, 360);

    scoped_refptr<MockSharedResources> resources = base::MakeRefCounted<MockSharedResources>();
    ...
    scoped_refptr<media::VideoFrame> memory_frame = CreateTestFrame(
        kCodedSize, kVisibleRect, kNaturalSize, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_ARGB, base::TimeDelta());
    // fill mock image with whilte color.
    memset(memory_frame->writable_data(media::VideoFrame::Plane::kARGB), 0xFF, kCodedSize.GetArea() * 4);
    ...
    EXPECT_CALL(*resources, ConstructVideoFrameFromTexture(_)).WillOnce(Return(memory_frame));

    rtc::scoped_refptr<webrtc::VideoFrameBuffer> frame_buffer = ConvertToWebRtcVideoFrameBuffer(std::move(owned_memory_frame), resources);
    EXPECT_EQ(frame_buffer->width(), kNaturalSize.width());
    EXPECT_EQ(frame_buffer->height(), kNaturalSize.height());
