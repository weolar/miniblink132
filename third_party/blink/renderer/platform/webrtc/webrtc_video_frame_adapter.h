...
class PLATFORM_EXPORT WebRtcVideoFrameAdapterInterface : public webrtc::VideoFrameBuffer {
public:
...
class PLATFORM_EXPORT WebRtcVideoFrameAdapter : public WebRtcVideoFrameAdapterInterface {
public:
    class PLATFORM_EXPORT SharedResources : public ThreadSafeRefCounted<SharedResources> {
    public:
        explicit SharedResources(media::GpuVideoAcceleratorFactories* gpu_factories);

        // Create frames for requested output format and resolution.
        virtual scoped_refptr<media::VideoFrame> CreateFrame(media::VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
        ...
        virtual media::EncoderStatus ConvertAndScale(const media::VideoFrame& src_frame, media::VideoFrame& dest_frame);

        virtual scoped_refptr<viz::RasterContextProvider> GetRasterContextProvider();
        ...
        virtual scoped_refptr<media::VideoFrame> ConstructVideoFrameFromTexture(scoped_refptr<media::VideoFrame> source_frame);
        ...
        // copy pixel data.
        virtual scoped_refptr<media::VideoFrame> ConstructVideoFrameFromGpu(scoped_refptr<media::VideoFrame> source_frame);
    ...
    private:
        ...
        std::unique_ptr<media::RenderableGpuMemoryBufferVideoFramePool> accelerated_frame_pool_;
        bool disable_gmb_frames_ = false;

        base::Lock context_provider_lock_;
        scoped_refptr<viz::RasterContextProvider> raster_context_provider_ GUARDED_BY(context_provider_lock_);
    ...
    struct PLATFORM_EXPORT ScaledBufferSize {
        ...
        ScaledBufferSize CropAndScale(int offset_x, int offset_y, int crop_width, int crop_height, int scaled_width, int scaled_height) const;
    ...
    public:
        ScaledBuffer(scoped_refptr<WebRtcVideoFrameAdapter> parent, ScaledBufferSize size);

        scoped_refptr<media::VideoFrame> getMediaVideoFrame() const override;

        int width() const override
        {
            return size_.natural_size.width();
        }
        int height() const override
        {
            return size_.natural_size.height();
        }
        ...
        rtc::scoped_refptr<webrtc::VideoFrameBuffer> GetMappedFrameBuffer(rtc::ArrayView<webrtc::VideoFrameBuffer::Type> types) override;
    ...
    WebRtcVideoFrameAdapter(scoped_refptr<media::VideoFrame> frame, scoped_refptr<SharedResources> shared_resources);
    ...
    // null is returned.
    scoped_refptr<media::VideoFrame> GetAdaptedVideoBufferForTesting(const ScaledBufferSize& size);
...
private:
    struct AdaptedFrame {
        AdaptedFrame(ScaledBufferSize size, scoped_refptr<media::VideoFrame> video_frame, rtc::scoped_refptr<webrtc::VideoFrameBuffer> frame_buffer)
            : size(std::move(size))
    ...
    rtc::scoped_refptr<webrtc::VideoFrameBuffer> GetOrCreateFrameBufferForSize(const ScaledBufferSize& size);
    AdaptedFrame AdaptBestFrame(const ScaledBufferSize& size) const EXCLUSIVE_LOCKS_REQUIRED(adapted_frames_lock_);
