...
{
    const WebRtcVideoFrameAdapter::ScaledBufferSize k720p(gfx::Rect(0, 0, 1280, 720), gfx::Size(1280, 720));

    // Crop away a 100 pixel border.
    const auto cropped_full_scale = k720p.CropAndScale(100, 100, 1080, 520, 1080, 520);
    ...
    const auto cropped_full_scale2 = cropped_full_scale.CropAndScale(0, 0, 1080, 520, 1080, 520);
    EXPECT_TRUE(cropped_full_scale2 == cropped_full_scale);

    // Cropping again is relative to the current crop. Crop on crop.
    const auto second_cropped_full_size = cropped_full_scale.CropAndScale(100, 100, 880, 320, 880, 320);
    ...
    // Applying the same size again should be a NO-OP.
    const auto second_cropped_full_size2 = second_cropped_full_size.CropAndScale(0, 0, 880, 320, 880, 320);
    ...
    const auto third_cropped_full_size = second_cropped_full_size.CropAndScale(100, 100, 680, 120, 680, 120);
...
TEST(ScaledBufferSizeTest, ScalingIsRelative)
{
    ...
    // Applying the same size again should be a NO-OP.
    const auto no_crop_half_size2 = no_crop_half_size.CropAndScale(0, 0, 640, 360, 640, 360);
    EXPECT_TRUE(no_crop_half_size2 == no_crop_half_size);

    // Scaling again is relative to the current scale. Half-size on half-size.
    const auto no_crop_quarter_size = no_crop_half_size.CropAndScale(0, 0, 640, 360, 320, 180);
    ...
    // Applying the same size again should be a NO-OP.
    const auto no_crop_quarter_size2 = no_crop_quarter_size.CropAndScale(0, 0, 320, 180, 320, 180);
    ...
    const auto no_crop_eighths_size = no_crop_quarter_size.CropAndScale(0, 0, 320, 180, 160, 90);
...
TEST(ScaledBufferSizeTest, CroppingAndScalingIsRelative)
{
    ...
    const auto crop_and_scale1 = k720p.CropAndScale(100, 100, 1080, 520, 540, 260);
    ...
    const auto crop_and_scale2 = crop_and_scale1.CropAndScale(50, 50, 440, 160, 440, 160);
    ...
    const auto crop_and_scale3 = crop_and_scale2.CropAndScale(0, 0, 440, 160, 220, 80);
...
{
    std::vector<webrtc::VideoFrameBuffer::Type> kNv12 = { webrtc::VideoFrameBuffer::Type::kNV12 };
    ...
    auto frame_720p = CreateTestFrame(
        kSize720p, kRect720p, kSize720p, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_NV12, base::TimeDelta());

    rtc::scoped_refptr<WebRtcVideoFrameAdapter> multi_buffer(new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(frame_720p, resources));
    ...
    auto adapted_frame = multi_buffer->GetAdaptedVideoBufferForTesting(WebRtcVideoFrameAdapter::ScaledBufferSize(kRect720p, kSize720p));
...
TEST(WebRtcVideoFrameAdapterTest, MapScaledFrameCreatesNewFrame)
{
    ...
    EXPECT_CALL(*resources, CreateFrame)
        .WillOnce(testing::Invoke([](media::VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
                                      base::TimeDelta timestamp) {
            return CreateTestFrame(coded_size, visible_rect, natural_size, media::VideoFrame::STORAGE_OWNED_MEMORY, format, base::TimeDelta());
        }));
    ...
    auto scaled_frame = multi_buffer->Scale(kSize360p.width(), kSize360p.height());
    ...
    auto adapted_frame = multi_buffer->GetAdaptedVideoBufferForTesting(WebRtcVideoFrameAdapter::ScaledBufferSize(kRect720p, kSize360p));
...
TEST(WebRtcVideoFrameAdapterTest, MapScaledFrameScalesFromClosestPreviouslyScaledFrameWithoutCropping)
{
    std::vector<webrtc::VideoFrameBuffer::Type> kNv12 = { webrtc::VideoFrameBuffer::Type::kNV12 };
    const gfx::Size kSize720p(1280, 720);
    const gfx::Rect kRect720p(0, 0, 1280, 720);
    ...
    // scale from 720p.
    auto scaled_frame_480p = multi_buffer->Scale(kSize480p.width(), kSize480p.height());
    ...
    EXPECT_TRUE(multi_buffer->GetAdaptedVideoBufferForTesting(WebRtcVideoFrameAdapter::ScaledBufferSize(kRect720p, kSize480p)));
    ...
    auto scaled_frame_360p = multi_buffer->Scale(kSize360p.width(), kSize360p.height());
    ...
    EXPECT_FALSE(multi_buffer->GetAdaptedVideoBufferForTesting(WebRtcVideoFrameAdapter::ScaledBufferSize(kRect720p, kSize360p)));
}

TEST(WebRtcVideoFrameAdapterTest, MapScaledFrameScalesFromClosestPreviouslyScaledFrameWithCropping)
{
    std::vector<webrtc::VideoFrameBuffer::Type> kNv12 = { webrtc::VideoFrameBuffer::Type::kNV12 };
    ...
    auto full_frame = CreateTestFrame(kFullCodedSize720p, kFullVisibleRect, kFullNaturalSize, media::VideoFrame::STORAGE_OWNED_MEMORY,
        media::VideoPixelFormat::PIXEL_FORMAT_NV12, base::TimeDelta());

    rtc::scoped_refptr<WebRtcVideoFrameAdapter> multi_buffer(new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(full_frame, resources));
    ...
    gfx::Rect kScaledFrameVisibleRect(kFullVisibleRect.x() + (10 * 2), kFullVisibleRect.y() + (10 * 2), (600 * 2), (320 * 2));
    EXPECT_TRUE(multi_buffer->GetAdaptedVideoBufferForTesting(WebRtcVideoFrameAdapter::ScaledBufferSize(kScaledFrameVisibleRect, kScaledFrameSize)));

    // Downscale by another factor of two.
    gfx::Size kTinyFrameSize(kScaledFrameSize.width() / 2, kScaledFrameSize.height() / 2);
    auto tiny_frame = scaled_frame->Scale(kTinyFrameSize.width(), kTinyFrameSize.height());
    ...
    // the full frame.
    EXPECT_FALSE(multi_buffer->GetAdaptedVideoBufferForTesting(WebRtcVideoFrameAdapter::ScaledBufferSize(kScaledFrameVisibleRect, kTinyFrameSize)));
}

TEST(WebRtcVideoFrameAdapterTest, MapScaledFrameDoesNotScaleFromPreviouslyScaledFrameWithOtherCrop)
{
    std::vector<webrtc::VideoFrameBuffer::Type> kNv12 = { webrtc::VideoFrameBuffer::Type::kNV12 };
    const gfx::Size kSize720p(1280, 720);
    const gfx::Rect kRect720p(0, 0, 1280, 720);
    ...
    EXPECT_CALL(*resources, CreateFrame)
        .Times(2)
        .WillRepeatedly(testing::Invoke([](media::VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    ...
    auto scaled_frame_480p
        = multi_buffer->CropAndScale(kCroppedRect.x(), kCroppedRect.y(), kCroppedRect.width(), kCroppedRect.height(), kSize480p.width(), kSize480p.height());
    auto mapped_frame_480p = scaled_frame_480p->GetMappedFrameBuffer(kNv12);
    EXPECT_EQ(mapped_frame_480p->width(), kSize480p.width());
    EXPECT_EQ(mapped_frame_480p->height(), kSize480p.height());
    // The 480p must have been scaled from a media::VideoFrame.
    EXPECT_TRUE(multi_buffer->GetAdaptedVideoBufferForTesting(WebRtcVideoFrameAdapter::ScaledBufferSize(kCroppedRect, kSize480p)));
    ...
    EXPECT_TRUE(multi_buffer->GetAdaptedVideoBufferForTesting(WebRtcVideoFrameAdapter::ScaledBufferSize(kRect720p, kSize360p)));
}
...
TEST(WebRtcVideoFrameAdapterTest, FrameFeedbackSetsRequireMappedFrame)
{
    ...
    scoped_refptr<WebRtcVideoFrameAdapter::SharedResources> resources = base::MakeRefCounted<WebRtcVideoFrameAdapter::SharedResources>(nullptr);
    ...
    EXPECT_FALSE(resources->GetFeedback().require_mapped_frame);
    {
        // Do map the buffer.
        rtc::scoped_refptr<WebRtcVideoFrameAdapter> multi_buffer(new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(frame_720p, resources));
        multi_buffer->Scale(kSize360p.width(), kSize360p.height())->GetMappedFrameBuffer(kNv12);
    }
