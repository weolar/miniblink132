...
template <typename Interface, typename Owner, HeapMojoWrapperMode Mode = HeapMojoWrapperMode::kWithContextObserver> class HeapMojoReceiver {
    DISALLOW_NEW();
...
public:
    ...
    {
        static_assert(std::is_base_of<Interface, Owner>::value, "Owner should implement Interface");
        static_assert(IsGarbageCollectedType<Owner>::value, "Owner needs to be a garbage collected object");
    }
    ...
    bool is_bound() const
    {
        return wrapper_->receiver().is_bound();
    }
    void reset()
    {
        wrapper_->receiver().reset();
    }
    void ResetWithReason(uint32_t custom_reason_code, const std::string& description)
    {
    ...
    void set_disconnect_with_reason_handler(mojo::ConnectionErrorWithReasonCallback error_handler)
    {
        wrapper_->receiver().set_disconnect_with_reason_handler(std::move(error_handler));
    }
    [[nodiscard]] mojo::PendingRemote<Interface> BindNewPipeAndPassRemote(scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
        return wrapper_->receiver().BindNewPipeAndPassRemote(std::move(task_runner));
    }
    void Bind(mojo::PendingReceiver<Interface> pending_receiver, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
        wrapper_->receiver().Bind(std::move(pending_receiver), std::move(task_runner));
    }
...
private:
    FRIEND_TEST_ALL_PREFIXES(HeapMojoReceiverGCWithContextObserverTest, NoResetOnConservativeGC);

    // Garbage collected wrapper class to add a prefinalizer.
    class Wrapper final : public GarbageCollected<Wrapper>, public ContextLifecycleObserver {
    ...
    public:
        ...
        mojo::Receiver<Interface>& receiver()
        {
            return receiver_;
        }
