...
public:
    explicit UrlTestImpl(mojo::PendingReceiver<url::mojom::blink::UrlTest> receiver)
    ...
    void BounceOrigin(const scoped_refptr<const SecurityOrigin>& in, BounceOriginCallback callback) override
    {
        std::move(callback).Run(in);
    }
...
TEST(KURLSecurityOriginStructTraitsTest, Basic)
{
    ...
    scoped_refptr<const SecurityOrigin> non_unique = SecurityOrigin::CreateFromValidTuple("http", "www.google.com", 80);
    ...
    scoped_refptr<const SecurityOrigin> unique = SecurityOrigin::CreateUniqueOpaque();
