...
template <HeapMojoWrapperMode Mode, typename ContextType> class HeapMojoReceiverSetGCBaseTest;

template <HeapMojoWrapperMode Mode, typename ContextType>
class GCOwner final : public GarbageCollected<GCOwner<Mode, ContextType>>, public sample::blink::Service {
public:
    explicit GCOwner(MockContextLifecycleNotifier* context, HeapMojoReceiverSetGCBaseTest<Mode, ContextType>* test)
        : receiver_set_(this, context)
        , test_(test)
    {
    ...
    HeapMojoReceiverSet<sample::blink::Service, GCOwner, Mode, ContextType>& receiver_set()
    {
        return receiver_set_;
    }

    void Frobinate(sample::blink::FooPtr foo, Service::BazOptions baz, mojo::PendingRemote<sample::blink::Port> port, FrobinateCallback callback) override
    {
    }
...
private:
    HeapMojoReceiverSet<sample::blink::Service, GCOwner, Mode, ContextType> receiver_set_;
    raw_ptr<HeapMojoReceiverSetGCBaseTest<Mode, ContextType>> test_;
};

template <HeapMojoWrapperMode Mode, typename ContextType> class HeapMojoReceiverSetGCBaseTest : public TestSupportingGC {
public:
    ...
    GCOwner<Mode, ContextType>* owner()
    {
        return owner_;
    }
    void set_is_owner_alive(bool alive)
    {
        is_owner_alive_ = alive;
    }
...
protected:
    ...
    scoped_refptr<base::NullTaskRunner> null_task_runner_ = base::MakeRefCounted<base::NullTaskRunner>();
};

template <HeapMojoWrapperMode Mode, typename ContextType>
class HeapMojoReceiverSetDisconnectHandlerBaseTest : public HeapMojoReceiverSetGCBaseTest<Mode, ContextType> {
public:
...
protected:
    void SetUp() override
    {
        ...
        this->owner_ = MakeGarbageCollected<GCOwner<Mode, ContextType>>(this->context(), this);

        mojo::PendingRemote<sample::blink::Service> pending_remote;
        this->owner_->receiver_set().Add(pending_remote.InitWithNewPipeAndPassReceiver(), this->task_runner());
...
template <HeapMojoWrapperMode Mode, typename ContextType>
class HeapMojoReceiverSetDisconnectWithReasonHandlerBaseTest : public HeapMojoReceiverSetDisconnectHandlerBaseTest<Mode, ContextType> {
public:
...
protected:
    void SetUp() override
    {
        ...
        this->remote_.set_disconnect_with_reason_handler(WTF::BindOnce(
            [](HeapMojoReceiverSetDisconnectWithReasonHandlerBaseTest* receiver_set_test, const uint32_t custom_reason, const std::string& description) {
                receiver_set_test->run_loop().Quit();
...
class HeapMojoReceiverSetGCWithContextObserverTest : public HeapMojoReceiverSetGCBaseTest<HeapMojoWrapperMode::kWithContextObserver, void> { };
class HeapMojoReceiverSetStringContextGCWithContextObserverTest : public HeapMojoReceiverSetGCBaseTest<HeapMojoWrapperMode::kWithContextObserver, std::string> {
};
class HeapMojoReceiverSetGCWithoutContextObserverTest : public HeapMojoReceiverSetGCBaseTest<HeapMojoWrapperMode::kForceWithoutContextObserver, void> { };
class HeapMojoReceiverSetDisconnectHandlerWithoutContextObserverTest
    : public HeapMojoReceiverSetDisconnectHandlerBaseTest<HeapMojoWrapperMode::kForceWithoutContextObserver, void> { };
class HeapMojoReceiverSetDisconnectWithReasonHandlerWithoutContextObserverTest
    : public HeapMojoReceiverSetDisconnectWithReasonHandlerBaseTest<HeapMojoWrapperMode::kForceWithoutContextObserver, void> { };
...
TEST_F(HeapMojoReceiverSetGCWithContextObserverTest, RemovesReceiver)
{
    ...
    auto receiver = mojo::PendingReceiver<sample::blink::Service>(mojo::MessagePipe().handle0);
...
TEST_F(HeapMojoReceiverSetGCWithContextObserverTest, NoClearOnConservativeGC)
{
    ...
    mojo::ReceiverId rid = owner()->receiver_set().Add(std::move(receiver), task_runner());
...
TEST_F(HeapMojoReceiverSetGCWithContextObserverTest, AddSeveralReceiverSet)
{
    ...
    auto receiver_1 = mojo::PendingReceiver<sample::blink::Service>(mojo::MessagePipe().handle0);
    mojo::ReceiverId rid_1 = receiver_set.Add(std::move(receiver_1), task_runner());
    EXPECT_TRUE(receiver_set.HasReceiver(rid_1));
    EXPECT_FALSE(receiver_set.empty());
    EXPECT_EQ(receiver_set.size(), 1u);

    auto receiver_2 = mojo::PendingReceiver<sample::blink::Service>(mojo::MessagePipe().handle0);
    mojo::ReceiverId rid_2 = receiver_set.Add(std::move(receiver_2), task_runner());
    EXPECT_TRUE(receiver_set.HasReceiver(rid_1));
...
// properly.
TEST_F(HeapMojoReceiverSetStringContextGCWithContextObserverTest, AddSeveralReceiverSetWithContext)
{
    auto& receiver_set = owner()->receiver_set();
    ...
    auto receiver_1 = mojo::PendingReceiver<sample::blink::Service>(mojo::MessagePipe().handle0);
    mojo::ReceiverId rid_1 = receiver_set.Add(std::move(receiver_1), std::string("context1"), task_runner());
    EXPECT_TRUE(receiver_set.HasReceiver(rid_1));
    EXPECT_FALSE(receiver_set.empty());
    EXPECT_EQ(receiver_set.size(), 1u);

    auto receiver_2 = mojo::PendingReceiver<sample::blink::Service>(mojo::MessagePipe().handle0);
    mojo::ReceiverId rid_2 = receiver_set.Add(std::move(receiver_2), std::string("context2"), task_runner());
    EXPECT_TRUE(receiver_set.HasReceiver(rid_1));
    EXPECT_TRUE(receiver_set.HasReceiver(rid_2));
    EXPECT_FALSE(receiver_set.empty());
    EXPECT_EQ(receiver_set.size(), 2u);
...
// handler is fired.
TEST_F(HeapMojoReceiverSetDisconnectWithReasonHandlerWithoutContextObserverTest, ClearWithReason)
{
