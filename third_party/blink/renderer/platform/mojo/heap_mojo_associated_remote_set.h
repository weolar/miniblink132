...
template <typename Interface, HeapMojoWrapperMode Mode = HeapMojoWrapperMode::kWithContextObserver> class HeapMojoAssociatedRemoteSet {
    DISALLOW_NEW();

public:
    using DisconnectHandler = typename mojo::AssociatedRemoteSet<Interface>::DisconnectHandler;
    ...
    HeapMojoAssociatedRemoteSet& operator=(const HeapMojoAssociatedRemoteSet&) = delete;
    ...
    mojo::RemoteSetElementId Add(mojo::PendingAssociatedRemote<Interface> associated_remote, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
        return wrapper_->associated_remote_set().Add(std::move(associated_remote), task_runner);
    }
    ...
    {
        wrapper_->associated_remote_set().set_disconnect_handler(std::move(handler));
    }

    void Clear()
    {
        wrapper_->associated_remote_set().Clear();
    }

    bool empty() const
    {
        return wrapper_->associated_remote_set().empty();
    }
    size_t size() const
    {
        return wrapper_->associated_remote_set().size();
    }

    Iterator begin()
    {
        return wrapper_->associated_remote_set().begin();
    }
    ...
    Iterator end()
    {
        return wrapper_->associated_remote_set().end();
    }
    Iterator end() const
    {
        return wrapper_->associated_remote_set().end();
...
private:
    FRIEND_TEST_ALL_PREFIXES(HeapMojoAssociatedRemoteSetGCWithContextObserverTest, NoClearOnConservativeGC);
