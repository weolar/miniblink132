...
template <HeapMojoWrapperMode Mode> class HeapMojoAssociatedRemoteSetGCBaseTest;

template <HeapMojoWrapperMode Mode> class GCOwner final : public GarbageCollected<GCOwner<Mode>> {
public:
    explicit GCOwner(MockContextLifecycleNotifier* context, HeapMojoAssociatedRemoteSetGCBaseTest<Mode>* test)
        : remote_set_(context)
        , test_(test)
    {
    ...
    {
        visitor->Trace(remote_set_);
    }

    HeapMojoAssociatedRemoteSet<sample::blink::Service, Mode>& associated_remote_set()
    {
        return remote_set_;
    }
...
template <HeapMojoWrapperMode Mode> class HeapMojoAssociatedRemoteSetGCBaseTest : public TestSupportingGC {
public:
...
class HeapMojoAssociatedRemoteSetGCWithContextObserverTest : public HeapMojoAssociatedRemoteSetGCBaseTest<HeapMojoWrapperMode::kWithContextObserver> { };
class HeapMojoAssociatedRemoteSetGCWithoutContextObserverTest
    : public HeapMojoAssociatedRemoteSetGCBaseTest<HeapMojoWrapperMode::kForceWithoutContextObserver> { };
...
// the wrapper.
TEST_F(HeapMojoAssociatedRemoteSetGCWithContextObserverTest, NoClearOnConservativeGC)
{
    ...
    mojo::RemoteSetElementId rid = owner()->associated_remote_set().Add(std::move(remote), task_runner());
...
// GC the HeapMojoAssociatedRemoteSet with context observer and verify that the
// remote is no longer part of the set, and that the service was deleted.
TEST_F(HeapMojoAssociatedRemoteSetGCWithContextObserverTest, ClearLeavesSetEmpty)
{
    auto& remote_set = owner()->associated_remote_set();
    mojo::PendingAssociatedRemote<sample::blink::Service> remote;
    std::ignore = remote.InitWithNewEndpointAndPassReceiver();
...
// GC the HeapMojoAssociatedRemoteSet without context observer and verify that
// the remote is no longer part of the set, and that the service was deleted.
TEST_F(HeapMojoAssociatedRemoteSetGCWithoutContextObserverTest, ClearLeavesSetEmpty)
{
    auto& remote_set = owner()->associated_remote_set();
    mojo::PendingAssociatedRemote<sample::blink::Service> remote;
    std::ignore = remote.InitWithNewEndpointAndPassReceiver();
...
TEST_F(HeapMojoAssociatedRemoteSetGCWithContextObserverTest, AddSeveralRemoteSet)
{
    auto& remote_set = owner()->associated_remote_set();
    ...
    mojo::RemoteSetElementId rid_1 = remote_set.Add(std::move(remote_1), task_runner());
    ...
    mojo::RemoteSetElementId rid_2 = remote_set.Add(std::move(remote_2), task_runner());
    EXPECT_TRUE(remote_set.Contains(rid_1));
