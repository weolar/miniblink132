...
template <typename Interface, HeapMojoWrapperMode Mode = HeapMojoWrapperMode::kWithContextObserver> class HeapMojoRemote {
    DISALLOW_NEW();
...
public:
    ...
    Proxy* get() const { return /*wrapper_->remote().get()*/ nullptr; }
#else
    ...
    Interface* get() const
    {
        return wrapper_->remote().get();
    }
#endif
    bool is_bound() const
    {
        return wrapper_->remote().is_bound();
    }
    ...
    {
        return wrapper_->remote().is_connected();
    }
    void reset()
    {
        wrapper_->remote().reset();
    }
    ...
    [[nodiscard]] mojo::PendingReceiver<Interface> BindNewPipeAndPassReceiver(scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
        return wrapper_->remote().BindNewPipeAndPassReceiver(std::move(task_runner));
    }
    void Bind(mojo::PendingRemote<Interface> pending_remote, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
    ...
    mojo::PendingFlush FlushAsync()
    {
        return wrapper_->remote().FlushAsync();
    }
    void FlushForTesting()
    {
        return wrapper_->remote().FlushForTesting();
    }
