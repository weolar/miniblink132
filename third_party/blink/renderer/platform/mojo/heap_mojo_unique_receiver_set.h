...
template <typename Interface, typename Deleter = std::default_delete<Interface>, HeapMojoWrapperMode Mode = HeapMojoWrapperMode::kWithContextObserver>
class HeapMojoUniqueReceiverSet {
    DISALLOW_NEW();

public:
    using ImplPointerType = typename mojo::Receiver<Interface, mojo::UniquePtrImplRefTraits<Interface, Deleter>>::ImplPointerType;
    ...
    HeapMojoUniqueReceiverSet& operator=(const HeapMojoUniqueReceiverSet&) = delete;

    // Methods to redirect to mojo::ReceiverSet:
    mojo::ReceiverId Add(ImplPointerType impl, mojo::PendingReceiver<Interface> receiver, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        return wrapper_->receiver_set().Add(std::move(impl), std::move(receiver), task_runner);
    }
