...
template <HeapMojoWrapperMode Mode> class HeapMojoRemoteSetGCBaseTest;

template <HeapMojoWrapperMode Mode> class GCOwner final : public GarbageCollected<GCOwner<Mode>> {
public:
    explicit GCOwner(MockContextLifecycleNotifier* context, HeapMojoRemoteSetGCBaseTest<Mode>* test)
        : remote_set_(context)
        , test_(test)
    {
...
template <HeapMojoWrapperMode Mode> class HeapMojoRemoteSetGCBaseTest : public TestSupportingGC {
public:
...
class HeapMojoRemoteSetGCWithContextObserverTest : public HeapMojoRemoteSetGCBaseTest<HeapMojoWrapperMode::kWithContextObserver> { };
class HeapMojoRemoteSetGCWithoutContextObserverTest : public HeapMojoRemoteSetGCBaseTest<HeapMojoWrapperMode::kForceWithoutContextObserver> { };
...
TEST_F(HeapMojoRemoteSetGCWithContextObserverTest, RemovesRemote)
{
    ...
    auto remote = mojo::PendingRemote<sample::blink::Service>(mojo::MessagePipe().handle0, 0);
...
TEST_F(HeapMojoRemoteSetGCWithContextObserverTest, NoClearOnConservativeGC)
{
    ...
    mojo::RemoteSetElementId rid = owner()->remote_set().Add(std::move(remote), task_runner());
...
TEST_F(HeapMojoRemoteSetGCWithContextObserverTest, AddSeveralRemoteSet)
{
    ...
    auto remote_1 = mojo::PendingRemote<sample::blink::Service>(mojo::MessagePipe().handle0, 0);
    ...
    auto remote_2 = mojo::PendingRemote<sample::blink::Service>(mojo::MessagePipe().handle0, 0);
