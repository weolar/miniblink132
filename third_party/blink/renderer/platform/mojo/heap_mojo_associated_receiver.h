...
template <typename Interface, typename Owner, HeapMojoWrapperMode Mode = HeapMojoWrapperMode::kWithContextObserver> class HeapMojoAssociatedReceiver {
    DISALLOW_NEW();
...
public:
    ...
    HeapMojoAssociatedReceiver& operator=(const HeapMojoAssociatedReceiver&) = delete;

    // Methods to redirect to mojo::AssociatedReceiver:
    bool is_bound() const
    {
        return wrapper_->associated_receiver().is_bound();
    }
    void reset()
    {
        wrapper_->associated_receiver().reset();
    }
    void set_disconnect_handler(base::OnceClosure handler)
    {
        wrapper_->associated_receiver().set_disconnect_handler(std::move(handler));
    }
    [[nodiscard]] mojo::PendingAssociatedRemote<Interface> BindNewEndpointAndPassRemote(scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
        return wrapper_->associated_receiver().BindNewEndpointAndPassRemote(std::move(task_runner));
    }
    void Bind(mojo::PendingAssociatedReceiver<Interface> pending_associated_receiver, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
        wrapper_->associated_receiver().Bind(std::move(pending_associated_receiver), std::move(task_runner));
    }
...
private:
    FRIEND_TEST_ALL_PREFIXES(HeapMojoAssociatedReceiverGCWithContextObserverTest, NoResetOnConservativeGC);
