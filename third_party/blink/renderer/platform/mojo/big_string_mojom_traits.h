...
template <> struct PLATFORM_EXPORT StructTraits<mojo_base::mojom::BigStringDataView, WTF::String> {
    ...
    static mojo_base::BigBuffer data(const WTF::String& input)
    {
        ...
        return mojo_base::BigBuffer(base::span((const uint8_t*)adaptor.data(), adaptor.size()));
    }
    ...
    static bool Read(mojo_base::mojom::BigStringDataView data, WTF::String* out)
    {
        ...
        if (!buffer.size()) {
            ...
            *out = WTF::String::FromUTF8(base::span(buffer.data(), buffer.size()));
        }
        return true;
    }
};
