...
template <typename Interface, typename Owner, HeapMojoWrapperMode Mode = HeapMojoWrapperMode::kWithContextObserver, typename ContextType = void>
...
public:
    ...
    mojo::ReceiverId Add(mojo::PendingReceiver<Interface> receiver, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
        return wrapper_->receiver_set().Add(wrapper_->owner(), std::move(receiver), task_runner);
    }

    mojo::ReceiverId Add(mojo::PendingReceiver<Interface> receiver, Context context, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
        return wrapper_->receiver_set().Add(wrapper_->owner(), std::move(receiver), std::move(context), task_runner);
    }
    ...
    bool empty() const
    {
        return wrapper_->receiver_set().empty();
    }
    size_t size() const
    {
        return wrapper_->receiver_set().size();
    }
...
private:
    FRIEND_TEST_ALL_PREFIXES(HeapMojoReceiverSetGCWithContextObserverTest, NoClearOnConservativeGC);
