...
class PLATFORM_EXPORT MojoBindingContext : public ContextLifecycleNotifier, public Supplementable<MojoBindingContext> {
public:
    MojoBindingContext()
        : mojo_js_interface_broker_(this)
    {
    }

    virtual const BrowserInterfaceBrokerProxy& GetBrowserInterfaceBroker() const = 0;
    virtual scoped_refptr<base::SingleThreadTaskRunner> GetTaskRunner(TaskType) = 0;

    bool use_mojo_js_interface_broker()
    {
        return use_mojo_js_interface_broker_;
    }
    ...
    void SetMojoJSInterfaceBroker(mojo::PendingRemote<mojom::blink::BrowserInterfaceBroker> broker_remote)
    {
        use_mojo_js_interface_broker_ = true;
        mojo_js_interface_broker_.Bind(std::move(broker_remote), GetTaskRunner(TaskType::kInternalDefault));
    }
