...
struct UrlOriginAdapter {
    static const base::UnguessableToken* nonce_if_opaque(const scoped_refptr<const ::blink::SecurityOrigin>& origin)
    {
        return origin->GetNonceForSerialization();
    }
    static scoped_refptr<blink::SecurityOrigin> CreateSecurityOrigin(
        const url::SchemeHostPort& tuple, const std::optional<base::UnguessableToken>& nonce_if_opaque)
    {
        scoped_refptr<blink::SecurityOrigin> tuple_origin;
        if (tuple.IsValid()) {
            tuple_origin = blink::SecurityOrigin::CreateFromValidTuple(String::FromUTF8(tuple.scheme()), String::FromUTF8(tuple.host()), tuple.port());
        }

        if (nonce_if_opaque) {
            tuple_origin = blink::SecurityOrigin::CreateOpaque(url::Origin::Nonce(*nonce_if_opaque), tuple_origin.get());
        }
        return tuple_origin;
    }
    static const ::blink::SecurityOrigin* GetOriginOrPrecursorOriginIfOpaque(const scoped_refptr<const ::blink::SecurityOrigin>& origin)
    {
        return origin->GetOriginOrPrecursorOriginIfOpaque();
    }
};

template <> struct StructTraits<url::mojom::OriginDataView, scoped_refptr<const ::blink::SecurityOrigin>> {
    static WTF::String scheme(const scoped_refptr<const ::blink::SecurityOrigin>& origin)
    {
        return UrlOriginAdapter::GetOriginOrPrecursorOriginIfOpaque(origin)->Protocol();
    }
    static WTF::String host(const scoped_refptr<const ::blink::SecurityOrigin>& origin)
    {
        return UrlOriginAdapter::GetOriginOrPrecursorOriginIfOpaque(origin)->Host();
    }
    static uint16_t port(const scoped_refptr<const ::blink::SecurityOrigin>& origin)
    {
        return UrlOriginAdapter::GetOriginOrPrecursorOriginIfOpaque(origin)->Port();
    }
    static mojo::OptionalAsPointer<const base::UnguessableToken> nonce_if_opaque(const scoped_refptr<const ::blink::SecurityOrigin>& origin)
    {
        return mojo::OptionalAsPointer(UrlOriginAdapter::nonce_if_opaque(origin));
    }
    static bool Read(url::mojom::OriginDataView data, scoped_refptr<const ::blink::SecurityOrigin>* out)
    {
        ...
        const url::SchemeHostPort& tuple = url::SchemeHostPort(scheme, host, data.port());
        if (!tuple.IsValid()) {
    ...
    static bool IsNull(const scoped_refptr<const ::blink::SecurityOrigin>& origin)
    {
        return !origin;
    }
