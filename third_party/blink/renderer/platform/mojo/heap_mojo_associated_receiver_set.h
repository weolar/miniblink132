...
template <typename Interface, typename Owner, HeapMojoWrapperMode Mode = HeapMojoWrapperMode::kWithContextObserver> class HeapMojoAssociatedReceiverSet {
    DISALLOW_NEW();

public:
    explicit HeapMojoAssociatedReceiverSet(Owner* owner, ContextLifecycleNotifier* context)
    ...
    HeapMojoAssociatedReceiverSet& operator=(const HeapMojoAssociatedReceiverSet&) = delete;

    // Methods to redirect to mojo::AssociatedReceiverSet:
    void set_disconnect_handler(base::RepeatingClosure handler)
    {
        wrapper_->associated_receiver_set().set_disconnect_handler(std::move(handler));
    }

    mojo::ReceiverId Add(mojo::PendingAssociatedReceiver<Interface> associated_receiver, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
        return wrapper_->associated_receiver_set().Add(wrapper_->owner(), std::move(associated_receiver), task_runner);
    }
    ...
    void Clear()
    {
        wrapper_->associated_receiver_set().Clear();
    }
    ...
    bool empty() const
    {
        return wrapper_->associated_receiver_set().empty();
    }
...
private:
    FRIEND_TEST_ALL_PREFIXES(HeapMojoAssociatedReceiverSetGCWithContextObserverTest, NoClearOnConservativeGC);
