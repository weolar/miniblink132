...
template <typename Interface, HeapMojoWrapperMode Mode = HeapMojoWrapperMode::kWithContextObserver> class HeapMojoRemoteSet {
    DISALLOW_NEW();

public:
    using DisconnectHandler = typename mojo::RemoteSet<Interface>::DisconnectHandler;
    ...
    mojo::RemoteSetElementId Add(mojo::PendingRemote<Interface> remote, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
    ...
    void Clear()
    {
        wrapper_->remote_set().Clear();
    }

    bool empty() const
    {
        return wrapper_->remote_set().empty();
    }
    size_t size() const
    {
        return wrapper_->remote_set().size();
    }

    Iterator begin()
    {
        return wrapper_->remote_set().begin();
    }
    ...
    Iterator end()
    {
        return wrapper_->remote_set().end();
    }
    Iterator end() const
    {
        return wrapper_->remote_set().end();
...
private:
    FRIEND_TEST_ALL_PREFIXES(HeapMojoRemoteSetGCWithContextObserverTest, NoClearOnConservativeGC);
    ...
    public:
        ...
        mojo::RemoteSet<Interface>& remote_set()
        {
            return remote_set_;
        }
