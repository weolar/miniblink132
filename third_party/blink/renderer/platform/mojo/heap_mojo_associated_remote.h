...
template <typename Interface, HeapMojoWrapperMode Mode = HeapMojoWrapperMode::kWithContextObserver> class HeapMojoAssociatedRemote {
    DISALLOW_NEW();
...
//     Proxy* get() const
//     {
//         //return wrapper_->associated_remote().get();
//         *(int*)1 = 1;
//         return nullptr;
//     }
#else
    ...
    bool is_bound() const
    {
        return wrapper_->associated_remote().is_bound();
    }
    ...
    void reset()
    {
        wrapper_->associated_remote().reset();
    }
    ...
    void set_disconnect_with_reason_handler(mojo::ConnectionErrorWithReasonCallback handler)
    {
        wrapper_->associated_remote().set_disconnect_with_reason_handler(std::move(handler));
    }
    [[nodiscard]] mojo::PendingAssociatedReceiver<Interface> BindNewEndpointAndPassReceiver(scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(task_runner);
        return wrapper_->associated_remote().BindNewEndpointAndPassReceiver(std::move(task_runner));
    }
    [[nodiscard]] mojo::PendingAssociatedReceiver<Interface> BindNewEndpointAndPassDedicatedReceiver()
    {
        return wrapper_->associated_remote().BindNewEndpointAndPassDedicatedReceiver();
    }
    ...
    {
        DCHECK(task_runner);
        wrapper_->associated_remote().Bind(std::move(pending_associated_remote), std::move(task_runner));
    }
