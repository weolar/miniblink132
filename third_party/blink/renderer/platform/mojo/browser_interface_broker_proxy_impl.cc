...
class EmptyBrowserInterfaceBrokerProxy : public TestableBrowserInterfaceBrokerProxy {
public:
...
TestableBrowserInterfaceBrokerProxy::TestBinder* TestableBrowserInterfaceBrokerProxy::FindTestBinder(std::string_view interface_name) const
{
    if (!binder_map_for_testing_.empty()) {
        auto it = binder_map_for_testing_.Find<InterfaceNameHashTranslator, std::string_view>(interface_name);
...
bool TestableBrowserInterfaceBrokerProxy::SetBinderForTesting(
    const std::string& name, base::RepeatingCallback<void(mojo::ScopedMessagePipeHandle)> binder) const
{
    ...
    auto result = binder_map_for_testing_.insert(std::move(wtf_name), std::move(binder));
    return result.is_new_entry;
}

BrowserInterfaceBrokerProxy& GetEmptyBrowserInterfaceBroker()
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(EmptyBrowserInterfaceBrokerProxy, empty_broker, ());
    return empty_broker;
}
...
BrowserInterfaceBrokerProxyImpl::BrowserInterfaceBrokerProxyImpl(ContextLifecycleNotifier* notifier)
    : broker_(notifier)
{
}

void BrowserInterfaceBrokerProxyImpl::Bind(
    CrossVariantMojoRemote<mojom::BrowserInterfaceBrokerInterfaceBase> broker, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    DCHECK(task_runner);
    broker_.Bind(std::move(broker), std::move(task_runner));
}

CrossVariantMojoReceiver<mojom::BrowserInterfaceBrokerInterfaceBase> BrowserInterfaceBrokerProxyImpl::Reset(
...
void BrowserInterfaceBrokerProxyImpl::GetInterface(mojo::GenericPendingReceiver receiver) const
{
    DCHECK(receiver.interface_name());
    TestBinder* binder = FindTestBinder(receiver.interface_name().value());
    if (binder) {
