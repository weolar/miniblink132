...
public:
    ...
    {
        return unowned_.data();
    }
    size_t size() const
    {
        return unowned_.size();
    }
...
template <> struct PLATFORM_EXPORT ArrayTraits<MaybeOwnedString16> {
    using Element = const uint16_t;

    static bool IsNull(const MaybeOwnedString16&)
    {
        return false;
    }
...
template <> struct PLATFORM_EXPORT StructTraits<mojo_base::mojom::String16DataView, WTF::String> {
...
template <> struct PLATFORM_EXPORT StructTraits<mojo_base::mojom::BigString16DataView, WTF::String> {
    static bool IsNull(const WTF::String& input)
    {
        return input.IsNull();
    }
