...
template <typename T> struct TraceIfNeeded {
...
template <WTF::WeakHandlingFlag weakness, typename T, typename Traits,
    bool = WTF::IsTraceable<typename Traits::TraitType>::value && !WTF::IsWeak<typename Traits::TraitType>::value,
    WTF::WeakHandlingFlag = WTF::kWeakHandlingTrait<T>>
struct TraceCollectionIfEnabled;

template <WTF::WeakHandlingFlag weakness, typename T, typename Traits> struct TraceCollectionIfEnabled<weakness, T, Traits, false, WTF::kNoWeakHandling> {
    ...
    {
        static_assert(!WTF::IsTraceable<typename Traits::TraitType>::value || WTF::IsWeak<typename Traits::TraitType>::value, "T should not be traced");
    }
};

template <typename T, typename Traits> struct TraceCollectionIfEnabled<WTF::kNoWeakHandling, T, Traits, false, WTF::kWeakHandling> {
    STATIC_ONLY(TraceCollectionIfEnabled);

    static void Trace(Visitor* visitor, const void* t)
    {
        WTF::TraceInCollectionTrait<WTF::kNoWeakHandling, T, Traits>::Trace(visitor, *reinterpret_cast<const T*>(t));
    }
};

template <WTF::WeakHandlingFlag weakness, typename T, typename Traits, bool, WTF::WeakHandlingFlag> struct TraceCollectionIfEnabled {
    STATIC_ONLY(TraceCollectionIfEnabled);

    static bool IsAlive(const blink::LivenessBroker& info, const T& traceable)
    {
        return WTF::TraceInCollectionTrait<weakness, T, Traits>::IsAlive(info, traceable);
    }

    static void Trace(Visitor* visitor, const void* t)
    {
        static_assert(
            (WTF::IsTraceable<typename Traits::TraitType>::value && !WTF::IsWeak<typename Traits::TraitType>::value) || weakness == WTF::kWeakHandling,
            "Traits should be traced");
        WTF::TraceInCollectionTrait<weakness, T, Traits>::Trace(visitor, *reinterpret_cast<const T*>(t));
    }
};
...
template <typename _KeyType, typename _ValueType, typename _KeyTraits, typename _ValueTraits, bool = WTF::IsWeak<_ValueType>::value>
...
public:
    ...
    static constexpr bool kNeedsEphemeronSemantics = WTF::IsWeak<KeyType>::value != WTF::IsWeak<ValueType>::value && WTF::IsTraceable<ValueType>::value;

    static_assert(!WTF::IsWeak<KeyType>::value || WTF::IsWeakMemberType<KeyType>::value, "Weakness must be encoded using WeakMember.");
    static_assert(!WTF::IsWeak<ValueType>::value || WTF::IsWeakMemberType<ValueType>::value, "Weakness must be encoded using WeakMember.");
...
template <typename _KeyType, typename _ValueType, typename _KeyTraits, typename _ValueTraits>
struct EphemeronKeyValuePair<_KeyType, _ValueType, _KeyTraits, _ValueTraits, true>
    : EphemeronKeyValuePair<_ValueType, _KeyType, _ValueTraits, _KeyTraits, false> {
    EphemeronKeyValuePair(const _KeyType& k, const _ValueType& v)
        : EphemeronKeyValuePair<_ValueType, _KeyType, _ValueTraits, _KeyTraits, false>(v, k)
    {
    }
};

template <WTF::WeakHandlingFlag WeakHandling, typename Key, typename Value, typename Traits> struct KeyValuePairInCollectionTrait {
    static bool IsAlive(const blink::LivenessBroker& info, const WTF::KeyValuePair<Key, Value>& kvp)
    {
        ...
        return blink::TraceCollectionIfEnabled<WTF::kWeakHandlingTrait<Key>, Key, typename Traits::KeyTraits>::IsAlive(info, kvp.key)
            && blink::TraceCollectionIfEnabled<WTF::kWeakHandlingTrait<Value>, Value, typename Traits::ValueTraits>::IsAlive(info, kvp.value);
    }

    static void Trace(blink::Visitor* visitor, const Key* key, const Value* value)
    {
        TraceImpl::Trace(visitor, key, value);
    }

    static void Trace(blink::Visitor* visitor, const WTF::KeyValuePair<Key, Value>& kvp)
    {
        TraceImpl::Trace(visitor, &kvp.key, &kvp.value);
    }

private:
    using EphemeronHelper = EphemeronKeyValuePair<Key, Value, typename Traits::KeyTraits, typename Traits::ValueTraits>;
    ...
    struct WeakTrait {
        static void Trace(blink::Visitor* visitor, const Key* key, const Value* value)
        {
            ...
            if (WeakHandling == WTF::kNoWeakHandling) {
                ...
                blink::TraceCollectionIfEnabled<WTF::kNoWeakHandling, typename EphemeronHelper::KeyType, typename EphemeronHelper::KeyTraits>::Trace(
                    visitor, &helper.key);
            }
    ...
    struct StrongTrait {
        static void Trace(blink::Visitor* visitor, const Key* key, const Value* value)
        {
            ...
            blink::TraceCollectionIfEnabled<WTF::kNoWeakHandling, Key, typename Traits::KeyTraits>::Trace(visitor, key);
            blink::TraceCollectionIfEnabled<WTF::kNoWeakHandling, Value, typename Traits::ValueTraits>::Trace(visitor, value);
        }
    };

    using TraceImpl = typename std::conditional<EphemeronHelper::kNeedsEphemeronSemantics, WeakTrait, StrongTrait>::type;
};
...
struct TraceInCollectionTrait<kNoWeakHandling, KeyValuePair<Key, Value>, Traits>
    : public blink::internal::KeyValuePairInCollectionTrait<kNoWeakHandling, Key, Value, Traits> { };

template <typename Key, typename Value, typename Traits>
struct TraceInCollectionTrait<kWeakHandling, KeyValuePair<Key, Value>, Traits>
    : public blink::internal::KeyValuePairInCollectionTrait<kWeakHandling, Key, Value, Traits> { };
...
// weak elements.
template <typename T, typename Traits> struct TraceInCollectionTrait<kNoWeakHandling, T, Traits> {
    ...
    {
        static_assert(WTF::IsTraceable<typename Traits::TraitType>::value && !WTF::IsWeak<typename Traits::TraitType>::value, "T should be traceable");
        visitor->Trace(t);
    }
};

template <typename T, typename Traits> struct TraceInCollectionTrait<kNoWeakHandling, blink::WeakMember<T>, Traits> {
...
template <typename T, typename Traits> struct TraceInCollectionTrait<kWeakHandling, T, Traits> { };

template <typename T, typename Traits> struct TraceInCollectionTrait<kWeakHandling, blink::WeakMember<T>, Traits> {
    static bool IsAlive(const blink::LivenessBroker& info, const blink::WeakMember<T>& value)
    {
...
template <typename T, typename U> struct TraceTrait<std::pair<T, U>> {
