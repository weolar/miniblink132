...
bool IsGCInProgress()
{
    return cppgc::subtle::HeapState::IsMarking(ThreadState::Current()->heap_handle())
        || cppgc::subtle::HeapState::IsSweeping(ThreadState::Current()->heap_handle());
}
...
{
    ThreadState::Current()->CollectAllGarbageForTesting(ThreadState::StackState::kMayContainHeapPointers);
}
...
void TestSupportingGC::ClearOutOldGarbage()
{
    ...
    size_t old_used = cpp_heap.CollectStatistics(cppgc::HeapStatistics::kDetailed).used_size_bytes;
    while (true) {
        PreciselyCollectGarbage();
        size_t used = cpp_heap.CollectStatistics(cppgc::HeapStatistics::kDetailed).used_size_bytes;
        if (used >= old_used)
            break;
        old_used = used;
    }
...
IncrementalMarkingTestDriver::IncrementalMarkingTestDriver(ThreadState* thread_state)
    : heap_(thread_state->heap_handle())
{
}
...
{
    CHECK(ThreadState::Current()->IsIncrementalMarking());
    while (!heap_.PerformMarkingStep(ThreadState::StackState::kMayContainHeapPointers)) {
    }
}
...
ConcurrentMarkingTestDriver::ConcurrentMarkingTestDriver(ThreadState* thread_state)
