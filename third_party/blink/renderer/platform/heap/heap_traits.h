...
namespace internal {
template <typename T> inline constexpr bool IsHeapVector = false;

template <typename T> inline constexpr bool IsHeapVector<HeapVector<T>> = true;
} // namespace internal

// Given a type T, returns a type that is either Member<T> or just T depending
// on whether T is a garbage-collected type.
template <typename T> using AddMemberIfNeeded = std::conditional_t<WTF::IsGarbageCollectedType<T>::value && !internal::IsHeapVector<T>, Member<T>, T>;

// Given a type T, returns a type that is either HeapVector<T>,
// HeapVector<Member<T>> or Vector<T> depending on T.
template <typename T> using VectorOf = std::conditional_t<WTF::IsTraceable<T>::value, HeapVector<AddMemberIfNeeded<T>>, Vector<T>>;
...
using VectorOfPairs = std::conditional_t<WTF::IsTraceable<T>::value || WTF::IsTraceable<U>::value,
    HeapVector<std::pair<AddMemberIfNeeded<T>, AddMemberIfNeeded<U>>>, Vector<std::pair<T, U>>>;
