...
class HeapHashCountedSet final : public GarbageCollected<HeapHashCountedSet<Value, Traits>>, public HashCountedSet<Value, Traits, HeapAllocator> {
    DISALLOW_NEW();
...
private:
    struct TypeConstraints {
        ...
        {
            static_assert(WTF::IsMemberOrWeakMemberType<Value>::value, "HeapHashCountedSet supports only Member and WeakMember.");
            static_assert(std::is_trivially_destructible<HeapHashCountedSet>::value, "HeapHashCountedSet must be trivially destructible.");
...
inline void CopyToVector(const blink::HeapHashCountedSet<Value, Traits>& set, VectorType& vector)
{
    CopyToVector(static_cast<const HashCountedSet<Value, Traits, blink::HeapAllocator>&>(set), vector);
}
