...
template <typename T, typename Traits = WTF::VectorTraits<T>> class HeapVectorBacking final : public GarbageCollected<HeapVectorBacking<T, Traits>> {
public:
    ...
    ALWAYS_INLINE NO_SANITIZE_UNRELATED_CAST static T* ToArray(ClassType* backing)
    {
...
HeapVectorBacking<T, Traits>::~HeapVectorBacking()
    requires(Traits::kNeedsDestruction)
{
    static_assert(Traits::kCanClearUnusedSlotsWithMemset || std::is_polymorphic<T>::value,
        "HeapVectorBacking doesn't support objects that cannot be cleared as "
        "unused with memset or don't have a vtable");
    static_assert(!std::is_trivially_destructible<T>::value, "Finalization of trivially destructible classes should not happen.");
    const size_t object_size = cppgc::subtle::ObjectSizeTrait<HeapVectorBacking<T, Traits>>::GetSize(*this);
...
template <typename T, typename Traits> struct ThreadingTrait<HeapVectorBacking<T, Traits>> {
...
template <typename T, typename Traits> struct TraceInCollectionTrait<kNoWeakHandling, blink::HeapVectorBacking<T, Traits>, void> {
    ...
    static void Trace(blink::Visitor* visitor, const void* self)
    {
        ...
        const size_t length = cppgc::subtle::ObjectSizeTrait<const Backing>::GetSize(*reinterpret_cast<const Backing*>(self)) / sizeof(T);
#ifdef ANNOTATE_CONTIGUOUS_CONTAINER
        ...
        if constexpr (IsTraceable<T>::value) {
            for (unsigned i = 0; i < length; ++i) {
                if (!std::is_polymorphic_v<T> || blink::internal::VTableInitialized(&array[i])) {
...
template <typename T> struct SpaceTrait<blink::HeapVectorBacking<T>, std::enable_if_t<blink::HeapVectorBacking<T>::TraitsType::kCanMoveWithMemcpy>> {
...
// backing array.
template <typename T> class MakeGarbageCollectedTrait<blink::HeapVectorBacking<T>> : public MakeGarbageCollectedTraitBase<blink::HeapVectorBacking<T>> {
...
public:
    template <typename... Args> static Backing* Call(AllocationHandle& handle, size_t num_elements)
    {
        ...
        void* memory = MakeGarbageCollectedTraitBase<Backing>::Allocate(handle, sizeof(T) * num_elements);
...
template <typename T, typename Traits> struct TraceTrait<blink::HeapVectorBacking<T, Traits>> {
    using Backing = blink::HeapVectorBacking<T, Traits>;
    ...
    static void Trace(Visitor* visitor, const void* self)
    {
        if (!Traits::kCanTraceConcurrently && self) {
            if (visitor->DeferTraceToMutatorThreadIfConcurrent(
                    self, &Trace, cppgc::subtle::ObjectSizeTrait<const Backing>::GetSize(*reinterpret_cast<const Backing*>(self)))) {
                return;
            }
        }

        static_assert(!WTF::IsWeak<T>::value, "Weakness is not supported in HeapVector and HeapDeque");
        if (WTF::IsTraceable<T>::value) {
            WTF::TraceInCollectionTrait<WTF::kNoWeakHandling, blink::HeapVectorBacking<T, Traits>, void>::Trace(visitor, self);
        }
    }
};
