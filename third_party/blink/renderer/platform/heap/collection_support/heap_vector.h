...
class HeapVector final : public GarbageCollected<HeapVector<T, inlineCapacity>>, public Vector<T, inlineCapacity, HeapAllocator> {
    DISALLOW_NEW();
...
public:
    ...
    template <typename Collection, typename = typename std::enable_if<std::is_class<Collection>::value>::type>
    explicit HeapVector(const Collection& other)
        : BaseVector(other)
    {
    }
    ...
    template <typename Proj, typename = std::enable_if_t<std::is_invocable_v<Proj, typename BaseVector::const_reference>>>
    ...
    template <typename U, wtf_size_t otherSize, typename Proj,
        typename = std::enable_if_t<std::is_invocable_v<Proj, typename HeapVector<U, otherSize>::const_reference>>>
    HeapVector(const HeapVector<U, otherSize>& other, Proj proj)
        : BaseVector(static_cast<const Vector<U, otherSize, HeapAllocator>&>(other), std::move(proj))
    {
    }
    ...
    template <typename Collection> HeapVector& operator=(const Collection& other)
    {
...
template <typename T, wtf_size_t inlineCapacity> constexpr HeapVector<T, inlineCapacity>::TypeConstraints::TypeConstraints()
{
    static_assert(std::is_trivially_destructible_v<HeapVector> || inlineCapacity, "HeapVector must be trivially destructible.");
    static_assert(!WTF::IsWeak<T>::value, "Weak types are not allowed in HeapVector.");
    static_assert(!WTF::IsGarbageCollectedType<T>::value || IsHeapVector<T>, "GCed types should not be inlined in a HeapVector.");
...
struct VectorTraits<blink::HeapVector<T, inlineCapacity>> : VectorTraitsBase<blink::HeapVector<T, inlineCapacity>> {
    STATIC_ONLY(VectorTraits);
    static const bool kNeedsDestruction = VectorTraits<T>::kNeedsDestruction;
    static const bool kCanInitializeWithMemset = VectorTraits<T>::kCanInitializeWithMemset;
    static const bool kCanClearUnusedSlotsWithMemset = VectorTraits<T>::kCanClearUnusedSlotsWithMemset;
