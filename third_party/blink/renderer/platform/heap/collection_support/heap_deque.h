...
template <typename T> class HeapDeque final : public GarbageCollected<HeapDeque<T>>, public Deque<T, 0, HeapAllocator> {
    DISALLOW_NEW();
...
public:
    ...
    HeapDeque(const HeapDeque<T>& other)
...
private:
    struct TypeConstraints {
        ...
        {
            static_assert(WTF::IsMemberType<T>::value, "HeapDeque supports only Member.");
            static_assert(std::is_trivially_destructible_v<HeapDeque>, "HeapDeque must be trivially destructible.");
...
template <typename T> struct VectorTraits<blink::HeapDeque<T>> : VectorTraitsBase<blink::HeapDeque<T>> {
