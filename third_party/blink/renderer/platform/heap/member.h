...
template <typename T> using Member = cppgc::Member<T>;

template <typename T> using WeakMember = cppgc::WeakMember<T>;

template <typename T> using UntracedMember = cppgc::UntracedMember<T>;

namespace subtle {
template <typename T> using UncompressedMember = cppgc::subtle::UncompressedMember<T>;
}

template <typename T> inline bool IsHashTableDeletedValue(const Member<T>& m)
{
...
template <typename T> struct ThreadingTrait<blink::Member<T>> {
    STATIC_ONLY(ThreadingTrait);
    static constexpr ThreadAffinity kAffinity = ThreadingTrait<T>::kAffinity;
};

template <typename T> struct ThreadingTrait<blink::WeakMember<T>> {
    STATIC_ONLY(ThreadingTrait);
    static constexpr ThreadAffinity kAffinity = ThreadingTrait<T>::kAffinity;
};

template <typename T> struct ThreadingTrait<blink::UntracedMember<T>> {
    STATIC_ONLY(ThreadingTrait);
    static constexpr ThreadAffinity kAffinity = ThreadingTrait<T>::kAffinity;
};

template <typename T> inline void swap(Member<T>& a, Member<T>& b)
{
    a.Swap(b);
}
...
static_assert(kBlinkMemberGCHasDebugChecks || sizeof(Member<void*>) <= sizeof(void*), "Member<> should stay small!");

} // namespace blink

namespace WTF {

template <typename T> struct IsTraceable<blink::Member<T>> {
    STATIC_ONLY(IsTraceable);
    static const bool value = true;
};

template <typename T> struct IsWeak<blink::WeakMember<T>> : std::true_type { };

template <typename T> struct IsTraceable<blink::WeakMember<T>> {
    STATIC_ONLY(IsTraceable);
    static const bool value = true;
};
...
template <typename T> class ValuePeeker final {
    DISALLOW_NEW();

public:
    // NOLINTNEXTLINE
    ALWAYS_INLINE ValuePeeker(T* ptr)
        : ptr_(ptr)
    {
    }
    template <typename U>
    // NOLINTNEXTLINE
    ALWAYS_INLINE ValuePeeker(const blink::Member<U>& m)
        : ptr_(m.Get())
    {
    }
    template <typename U>
    // NOLINTNEXTLINE
    ALWAYS_INLINE ValuePeeker(const blink::WeakMember<U>& m)
        : ptr_(m.Get())
    {
    }
    template <typename U>
    ...
    template <typename U>
    // NOLINTNEXTLINE
    ALWAYS_INLINE ValuePeeker(const blink::Persistent<U>& p)
        : ptr_(p.Get())
    {
    }
    template <typename U>
    ...
    // NOLINTNEXTLINE
    ALWAYS_INLINE operator blink::Member<T>() const
    {
        return ptr_;
    }
    // NOLINTNEXTLINE
    ALWAYS_INLINE operator blink::WeakMember<T>() const
    {
        return ptr_;
    }
    // NOLINTNEXTLINE
    ALWAYS_INLINE operator blink::UntracedMember<T>() const
    {
        return ptr_;
    }
...
template <typename T, typename MemberType> struct BaseMemberHashTraits : SimpleClassHashTraits<MemberType> {
    ...
    template <typename Member, std::enable_if_t<WTF::IsAnyMemberType<Member>::value>* = nullptr> static unsigned GetHash(const Member& m)
    {
    ...
    static PeekOutType Peek(const MemberType& value)
    {
        return value.Get();
    }
...
template <typename T> struct MemberHashTraits : BaseMemberHashTraits<T, blink::Member<T>> {
    static constexpr bool kCanTraceConcurrently = true;
};
template <typename T> struct HashTraits<blink::Member<T>> : MemberHashTraits<T> { };

// Custom HashTraits<WeakMember<Type>> can inherit this type.
template <typename T> struct WeakMemberHashTraits : BaseMemberHashTraits<T, blink::WeakMember<T>> {
    static constexpr bool kCanTraceConcurrently = true;
};
template <typename T> struct HashTraits<blink::WeakMember<T>> : WeakMemberHashTraits<T> { };

// Custom HashTraits<UntracedMember<Type>> can inherit this type.
template <typename T> struct UntracedMemberHashTraits : BaseMemberHashTraits<T, blink::UntracedMember<T>> { };
template <typename T> struct HashTraits<blink::UntracedMember<T>> : UntracedMemberHashTraits<T> { };

template <typename T> class MemberConstructTraits {
    STATIC_ONLY(MemberConstructTraits);

public:
    template <typename... Args> static T* Construct(void* location, Args&&... args)
    {
    ...
    template <typename... Args> static T* ConstructAndNotifyElement(void* location, Args&&... args)
    {
        ...
        T* object = new (NotNullTag::kNotNull, location) T(std::forward<Args>(args)..., typename T::AtomicInitializerTag());
    ...
    static void NotifyNewElements(base::span<T> members)
    {
        ...
        if (members.empty() || !blink::WriteBarrier::IsWriteBarrierNeeded(&members.front())) [[likely]] {
            return;
        }
...
class ConstructTraits<blink::Member<T>, Traits, Allocator> final : public MemberConstructTraits<blink::Member<T>> { };

template <typename T, typename Traits, typename Allocator>
class ConstructTraits<blink::WeakMember<T>, Traits, Allocator> final : public MemberConstructTraits<blink::WeakMember<T>> { };
