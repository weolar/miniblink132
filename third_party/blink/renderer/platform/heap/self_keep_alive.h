...
template <typename Self> class SelfKeepAlive final {
    DISALLOW_NEW();

public:
    explicit SelfKeepAlive(const PersistentLocation& loc = PERSISTENT_LOCATION_FROM_HERE)
        : keep_alive_(loc)
    {
    }
    explicit SelfKeepAlive(Self* self, const PersistentLocation& loc = PERSISTENT_LOCATION_FROM_HERE)
