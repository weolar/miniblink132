...
public:
    ...
    HeapTestingPlatformAdapter& operator=(const HeapTestingPlatformAdapter&) = delete;
    ...
    std::shared_ptr<v8::TaskRunner> GetForegroundTaskRunner(v8::Isolate* isolate, v8::TaskPriority priority) final
    {
        // Provides task runner that allows for incremental tasks even in detached
        // mode.
        return task_runner_;
    }
    void PostTaskOnWorkerThreadImpl(v8::TaskPriority priority, std::unique_ptr<v8::Task> task, const v8::SourceLocation& location) final
    {
        platform_->CallOnWorkerThread(std::move(task));
    }
    void PostDelayedTaskOnWorkerThreadImpl(
        v8::TaskPriority priority, std::unique_ptr<v8::Task> task, double delay_in_seconds, const v8::SourceLocation& location) final
    {
    ...
    std::unique_ptr<v8::JobHandle> CreateJobImpl(v8::TaskPriority priority, std::unique_ptr<v8::JobTask> job_task, const v8::SourceLocation& location) final
    {
