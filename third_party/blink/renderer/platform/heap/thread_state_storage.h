...
template <typename T, typename = void> struct ThreadingTrait {
...
extern constinit thread_local ThreadStateStorage* g_thread_specific_ __attribute__((tls_model(BLINK_HEAP_THREAD_LOCAL_MODEL)));
...
public:
    ...
    BLINK_HEAP_DECLARE_THREAD_LOCAL_GETTER(Current, ThreadStateStorage*, g_thread_specific_)
    ...
    ALWAYS_INLINE ThreadState& thread_state() const
    {
...
template <ThreadAffinity> class ThreadStateStorageFor;

template <> class ThreadStateStorageFor<kMainThreadOnly> {
...
template <> class ThreadStateStorageFor<kAnyThread> {
    STATIC_ONLY(ThreadStateStorageFor);
