...
TEST_F(HeapCompactTest, CompactVectorOfVector)
{
    ...
    Persistent<IntVectorVector> int_vector_vector = MakeGarbageCollected<IntVectorVector>();
...
TEST_F(HeapCompactTest, CompactHashPartVector)
{
    ...
    Persistent<IntVectorMap> int_vector_map = MakeGarbageCollected<IntVectorMap>();
...
TEST_F(HeapCompactTest, CompactInlinedBackingStore)
{
    ...
    Persistent<MapWithInlinedBacking> map = MakeGarbageCollected<MapWithInlinedBacking>();
    {
        // Create a map that is reclaimed during compaction.
        (MakeGarbageCollected<MapWithInlinedBacking>())->insert(IntWrapper::Create(1), MakeGarbageCollected<Value>());
...
struct NestedType final {
    ...
    void CheckValidInlineBuffer() const
    {
        ...
        // the vector itself.
        CHECK(reinterpret_cast<uintptr_t>(&vec) <= front && front < reinterpret_cast<uintptr_t>(&vec) + sizeof(vec));
    }
...
namespace WTF {
template <> struct VectorTraits<blink::NestedType> : VectorTraitsBase<blink::NestedType> {
...
TEST_F(HeapCompactTest, AvoidCompactionWhenTraitsProhibitMemcpy)
{
    ...
    static_assert(WTF::VectorTraits<NestedType>::kCanMoveWithMemcpy == false, "should not allow move using memcpy");
    ...
    Persistent<HeapVector<NestedType>> vec = MakeGarbageCollected<HeapVector<NestedType>>();
    vec->emplace_back();
    PerformHeapCompaction();
    vec = nullptr;
    PreciselyCollectGarbage();
