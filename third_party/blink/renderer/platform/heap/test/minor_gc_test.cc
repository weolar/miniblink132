...
template <size_t Size> class SimpleGCed final : public SimpleGCedBase {
    char array[Size];
};
...
template <typename Type> struct OtherType;
template <> struct OtherType<Small> {
    using Type = Large;
};
template <> struct OtherType<Large> {
    using Type = Small;
};
...
public:
    ...
    {
        ThreadState::Current()->CollectGarbageInYoungGenerationForTesting(ThreadState::StackState::kNoHeapPointers);
    }
...
template <typename SmallOrLarge> class MinorGCTestForType : public MinorGCTest {
public:
...
TYPED_TEST(MinorGCTestForType, InterGenerationalPointerInCollection)
{
    ...
    Persistent<HeapVector<Member<Type>>> old = MakeGarbageCollected<HeapVector<Member<Type>>>();
...
TYPED_TEST(MinorGCTestForType, InterGenerationalPointerNotifyingBunchOfElements)
{
    using Type = typename TestFixture::Type;
...
namespace {
template <typename T> class InlinedObjectBase {
    DISALLOW_NEW();

public:
    InlinedObjectBase()
        : value_(MakeGarbageCollected<T>())
    {
    }
...
template <typename T> class InlinedObject : public InlinedObjectBase<T> { };
} // namespace

TYPED_TEST(MinorGCTestForType, InterGenerationalPointerInPlaceBarrierForTraced)
{
    using Type = typename TestFixture::Type;
