...
template <typename Callback> base::TimeDelta TimedRun(Callback callback)
{
...
template <> struct ThreadingTrait<TinyObject> {
    STATIC_ONLY(ThreadingTrait);
    static const ThreadAffinity kAffinity = ThreadAffinity::kMainThreadOnly;
};

template <> struct ThreadingTrait<LargeObject> {
    STATIC_ONLY(ThreadingTrait);
    static const ThreadAffinity kAffinity = ThreadAffinity::kMainThreadOnly;
};
...
TEST_F(AllocationPerfTest, Allocate10MTiny)
{
    ...
    reporter.AddResult(kMetricThroughput, static_cast<double>(kNumObjects * kObjectBytes) / (1024 * 1024) / delta.InSecondsF());
}
