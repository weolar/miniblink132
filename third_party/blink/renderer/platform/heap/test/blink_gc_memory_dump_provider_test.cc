...
template <typename Callback> void IterateMemoryDumps(base::trace_event::ProcessMemoryDump& dump, const std::string dump_prefix, Callback callback)
{
    auto dump_prefix_depth = base::ranges::count(dump_prefix, '/');
    for (auto& it : dump.allocator_dumps()) {
        const std::string& key = it.first;
        if ((key.compare(0, dump_prefix.size(), dump_prefix) == 0) && (base::ranges::count(key, '/') == dump_prefix_depth)) {
            callback(it.second.get());
        }
    }
}

void CheckSpacesInDump(base::trace_event::ProcessMemoryDump& dump, const std::string dump_prefix)
{
    size_t custom_space_count = 0;
    IterateMemoryDumps(dump, dump_prefix + "CustomSpace", [&custom_space_count](base::trace_event::MemoryAllocatorDump*) { custom_space_count++; });
...
{
    base::trace_event::MemoryDumpArgs args = { base::trace_event::MemoryDumpLevelOfDetail::kLight };
    std::unique_ptr<base::trace_event::ProcessMemoryDump> dump(new base::trace_event::ProcessMemoryDump(args));
    std::unique_ptr<BlinkGCMemoryDumpProvider> dump_provider(new BlinkGCMemoryDumpProvider(
        ThreadState::Current(), scheduler::GetSingleThreadTaskRunnerForTesting(), BlinkGCMemoryDumpProvider::HeapType::kBlinkMainThread));
...
TEST_F(BlinkGCMemoryDumpProviderTest, WorkerLightDump)
{
    ...
    std::unique_ptr<base::trace_event::ProcessMemoryDump> dump(new base::trace_event::ProcessMemoryDump(args));
    std::unique_ptr<BlinkGCMemoryDumpProvider> dump_provider(new BlinkGCMemoryDumpProvider(
        ThreadState::Current(), scheduler::GetSingleThreadTaskRunnerForTesting(), BlinkGCMemoryDumpProvider::HeapType::kBlinkWorkerThread));
    dump_provider->OnMemoryDump(args, dump.get());
...
TEST_F(BlinkGCMemoryDumpProviderTest, WorkerDetailedDump)
{
    ...
    std::string worker_base_path = worker_path_prefix + "/worker_" + worker_suffix + worker_path_suffix;
    CheckBasicHeapDumpStructure(dump->GetAllocatorDump(worker_base_path));

    IterateMemoryDumps(*dump, worker_base_path + "/", CheckBasicHeapDumpStructure);
