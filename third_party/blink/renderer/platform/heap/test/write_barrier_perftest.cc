...
constexpr char kMetricRelativeSpeedDifferenceUnitless[] = "relative_speed_difference";
...
perf_test::PerfResultReporter SetUpReporter(const std::string& story_name)
{
    ...
    reporter.RegisterImportantMetric(kMetricRelativeSpeedDifferenceUnitless, "unitless");
    return reporter;
}
...
TEST_F(WriteBarrierPerfTest, MemberWritePerformance)
{
    ...
    Persistent<HeapVector<Member<PerfDummyObject>>> holder(MakeGarbageCollected<HeapVector<Member<PerfDummyObject>>>());
    ...
    reporter.AddResult(kMetricWritesDuringGcRunsPerS, kNumElements / during_gc_duration.InSecondsF());
    reporter.AddResult(kMetricWritesOutsideGcRunsPerS, kNumElements / outside_gc_duration.InSecondsF());
    reporter.AddResult(kMetricRelativeSpeedDifferenceUnitless, during_gc_duration / outside_gc_duration);
}
