...
template <typename T> class CollectionWrapper : public GarbageCollected<CollectionWrapper<T>> {
public:
    CollectionWrapper()
        : collection_(MakeGarbageCollected<T>())
    {
    }

    void Trace(Visitor* visitor) const
    {
        visitor->Trace(collection_);
    }
...
template <typename T> struct MethodAdapterBase {
    template <typename U> static void insert(T& t, U&& u)
    {
...
template <typename T> struct MethodAdapter : public MethodAdapterBase<T> { };

template <typename C> void AddToCollection()
{
    constexpr int kIterations = 10;
    ConcurrentMarkingTestDriver driver(ThreadState::Current());
    Persistent<CollectionWrapper<C>> persistent = MakeGarbageCollected<CollectionWrapper<C>>();
    ...
    for (int i = 0; i < kIterations; ++i) {
        ...
        for (int j = 0; j < kIterations; ++j) {
            ...
            MethodAdapter<C>::insert(*collection, MakeGarbageCollected<IntegerObject>(num));
        }
    }
    driver.FinishGC();
}

template <typename C, typename GetLocation> void RemoveFromCollectionAtLocation(GetLocation location)
{
    constexpr int kIterations = 10;
    ConcurrentMarkingTestDriver driver(ThreadState::Current());
    Persistent<CollectionWrapper<C>> persistent = MakeGarbageCollected<CollectionWrapper<C>>();
    ...
    for (int i = 0; i < (kIterations * kIterations); ++i) {
        MethodAdapter<C>::insert(*collection, MakeGarbageCollected<IntegerObject>(i));
    }
    driver.StartGC();
    for (int i = 0; i < kIterations; ++i) {
        driver.TriggerMarkingSteps();
...
template <typename C> void RemoveFromBeginningOfCollection()
{
    RemoveFromCollectionAtLocation<C>([](C* collection) { return collection->begin(); });
}

template <typename C> void RemoveFromMiddleOfCollection()
{
...
template <typename C> void RemoveFromEndOfCollection()
{
    RemoveFromCollectionAtLocation<C>([](C* collection) {
...
template <typename C> void ClearCollection()
{
    constexpr int kIterations = 10;
    ConcurrentMarkingTestDriver driver(ThreadState::Current());
    Persistent<CollectionWrapper<C>> persistent = MakeGarbageCollected<CollectionWrapper<C>>();
...
template <typename C> void SwapCollections()
{
    constexpr int kIterations = 10;
    ConcurrentMarkingTestDriver driver(ThreadState::Current());
    Persistent<CollectionWrapper<C>> persistent = MakeGarbageCollected<CollectionWrapper<C>>();
    ...
    for (int i = 0; i < (kIterations * kIterations); ++i) {
        ...
        for (int j = 0; j < kIterations * i; ++j) {
            MethodAdapter<C>::insert(*new_collection, MakeGarbageCollected<IntegerObject>(j));
        }
        driver.TriggerMarkingSteps();
...
template <typename T> using IdentityHashMap = HeapHashMap<T, T>;

template <typename T> struct MethodAdapter<HeapHashMap<T, T>> : public MethodAdapterBase<HeapHashMap<T, T>> {
    template <typename U> static void insert(HeapHashMap<T, T>& map, U&& u)
    {
        map.insert(u, u);
    }
};
...
template <typename T> struct MethodAdapter<HeapLinkedHashSet<T>> : public MethodAdapterBase<HeapLinkedHashSet<T>> {
...
template <typename V> void PopFromCollection()
{
    constexpr int kIterations = 10;
    ConcurrentMarkingTestDriver driver(ThreadState::Current());
    Persistent<CollectionWrapper<V>> persistent = MakeGarbageCollected<CollectionWrapper<V>>();
...
template <typename T, wtf_size_t inlineCapacity> struct MethodAdapter<HeapVector<T, inlineCapacity>> : public MethodAdapterBase<HeapVector<T, inlineCapacity>> {
    template <typename U> static void insert(HeapVector<T, inlineCapacity>& vector, U&& u)
    {
...
template <typename T> using HeapVectorWithInlineStorage = HeapVector<T, 10>;
...
{
    RemoveFromBeginningOfCollection<HeapVectorWithInlineStorage<Member<IntegerObject>>>();
}
TEST_F(ConcurrentMarkingTest, RemoveFromMiddleOfInlinedVector)
{
    RemoveFromMiddleOfCollection<HeapVectorWithInlineStorage<Member<IntegerObject>>>();
}
TEST_F(ConcurrentMarkingTest, RemoveFromEndOfInlinedVector)
{
    RemoveFromEndOfCollection<HeapVectorWithInlineStorage<Member<IntegerObject>>>();
}
...
template <typename T> using HeapVectorOfPairs = HeapVector<std::pair<T, T>>;

template <typename T, wtf_size_t inlineCapacity>
struct MethodAdapter<HeapVector<std::pair<T, T>, inlineCapacity>> : public MethodAdapterBase<HeapVector<std::pair<T, T>, inlineCapacity>> {
    template <typename U> static void insert(HeapVector<std::pair<T, T>, inlineCapacity>& vector, U&& u)
    {
...
template <typename T> struct MethodAdapter<HeapDeque<T>> : public MethodAdapterBase<HeapDeque<T>> {
    template <typename U> static void insert(HeapDeque<T>& deque, U&& u)
    {
    ...
    static void Swap(HeapDeque<T>& a, HeapDeque<T>& b)
    {
        a.Swap(b);
    }
};
...
public:
    CollectsMixins()
        : set_(MakeGarbageCollected<MixinSet>())
    {
    }
    void RegisterMixin(RegisteredMixin* mixin)
    {
        set_->insert(mixin);
    }
    void Trace(Visitor* visitor) const
...
public:
    RegisteredMixin(CollectsMixins* collector)
    {
        collector->RegisterMixin(this);
    }
};

class GCedWithRegisteredMixin : public GarbageCollected<GCedWithRegisteredMixin>, public RegisteredMixin {
public:
