...
template <typename T> class WeakCellFactory;
...
template <typename T> class WeakCell : public GarbageCollected<WeakCell<T>> {
public:
    ...
    explicit WeakCell(base::PassKey<WeakCellFactory<T>>, T* ptr)
        : ptr_(ptr)
    {
    }
    void Invalidate(base::PassKey<WeakCellFactory<T>>)
    {
        ptr_ = nullptr;
    }
...
public:
    ...
    {
        if (!weak_cell_) {
            weak_cell_ = MakeGarbageCollected<WeakCell<T>>(base::PassKey<WeakCellFactory>(), ptr_);
        }
        DCHECK(weak_cell_);
        return weak_cell_.Get();
    }

    bool HasWeakCells() const
    {
        return weak_cell_;
    }
...
template <typename T> struct IsWeakReceiver<blink::Persistent<blink::WeakCell<T>>> : std::true_type { };

template <typename T> struct BindUnwrapTraits<blink::Persistent<blink::WeakCell<T>>> {
...
template <typename T> struct MaybeValidTraits<blink::Persistent<blink::WeakCell<T>>> {
    static constexpr bool MaybeValid(const blink::Persistent<blink::WeakCell<T>>& p)
    {
...
template <typename T> struct IsWeakReceiver<blink::UnwrappingCrossThreadHandle<blink::WeakCell<T>>> : std::true_type { };

template <typename T> struct BindUnwrapTraits<blink::UnwrappingCrossThreadHandle<blink::WeakCell<T>>> {
    static T* Unwrap(const blink::UnwrappingCrossThreadHandle<blink::WeakCell<T>>& wrapped)
    {
        return wrapped.GetOnCreationThread()->Get();
    }
};

template <typename T> struct MaybeValidTraits<blink::UnwrappingCrossThreadHandle<blink::WeakCell<T>>> {
    static constexpr bool MaybeValid(const blink::UnwrappingCrossThreadHandle<blink::WeakCell<T>>& p)
    {
