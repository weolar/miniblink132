...
template <typename T, typename DoneArg> class BarrierCallbackInfo {
public:
    BarrierCallbackInfo(wtf_size_t num_callbacks, base::OnceCallback<void(DoneArg)> done_callback)
...
template <typename T> void ShouldNeverRun(T t)
{
    CHECK(false);
}
...
// objects.
template <typename T, typename RawArg = std::remove_cvref_t<T>, typename DoneArg = HeapVector<Member<RawArg>>, template <typename> class CallbackType>
    requires(std::same_as<HeapVector<Member<RawArg>>, std::remove_cvref_t<DoneArg>>)
base::RepeatingCallback<void(T*)> HeapBarrierCallback(wtf_size_t num_callbacks, CallbackType<void(DoneArg)> done_callback)
{
    if (num_callbacks == 0) {
    ...
    return base::BindRepeating(
        &internal::BarrierCallbackInfo<T, DoneArg>::Run, std::make_unique<internal::BarrierCallbackInfo<T, DoneArg>>(num_callbacks, std::move(done_callback)));
}
