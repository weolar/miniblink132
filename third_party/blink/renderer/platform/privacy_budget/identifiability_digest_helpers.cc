...
IdentifiableToken IdentifiabilitySensitiveStringToken(const String& in)
{
    ...
    return IdentifiableToken(((original_hash & 0xFFFF0000) >> 16) ^ (original_hash & 0xFFFF));
}

IdentifiableToken IdentifiabilityBenignCaseFoldingStringToken(const String& in)
{
    if (in.IsNull())
        return kNullStringDigest;
...
IdentifiableToken IdentifiabilitySensitiveCaseFoldingStringToken(const String& in)
{
    if (in.IsNull())
        return IdentifiableToken(kNullStringDigest);
...
IdentifiableToken IdentifiabilityBenignStringVectorToken(const Vector<String>& in)
{
