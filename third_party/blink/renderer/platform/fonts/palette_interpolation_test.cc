...
{
    base::FilePath wpt_palette_font_path(blink::StringToFilePath(blink::test::BlinkWebTestsDir()));
    wpt_palette_font_path = wpt_palette_font_path.Append(FILE_PATH_LITERAL("external/wpt/css/css-fonts/resources/COLR-palettes-test-font.ttf"));
...
protected:
    void SetUp() override
    {
        ...
        Font color_palette_font = blink::test::CreateTestFont(AtomicString("Ahem"), pathToColorPalettesTestFont(), 16, &ligatures);
        color_palette_typeface_ = sk_ref_sp(color_palette_font.PrimaryFont()->PlatformData().Typeface());

        Font non_color_font = blink::test::CreateTestFont(AtomicString("Ahem"), pathToNonColorTestFont(), 16, &ligatures);
        non_color_ahem_typeface_ = sk_ref_sp(non_color_font.PrimaryFont()->PlatformData().Typeface());
    }

    void ExpectColorsEqualInSRGB(Vector<FontPalette::FontPaletteOverride> overrides1, Vector<FontPalette::FontPaletteOverride> overrides2)
    {
...
TEST_F(PaletteInterpolationTest, RetrievePaletteIndexFromColorFont)
{
    ...
    scoped_refptr<FontPalette> palette = FontPalette::Create(FontPalette::kDarkPalette);
    std::optional<uint16_t> index = palette_interpolation.RetrievePaletteIndex(palette.get());
...
TEST_F(PaletteInterpolationTest, RetrievePaletteIndexFromNonColorFont)
{
    ...
    scoped_refptr<FontPalette> palette = FontPalette::Create(FontPalette::kLightPalette);
    std::optional<uint16_t> index = palette_interpolation.RetrievePaletteIndex(palette.get());
...
{
    PaletteInterpolation palette_interpolation(color_palette_typeface_);
    scoped_refptr<FontPalette> palette_start = FontPalette::Create(AtomicString("palette1"));
    ...
    scoped_refptr<FontPalette> palette_end = FontPalette::Create(AtomicString("palette2"));
    ...
    scoped_refptr<FontPalette> palette = FontPalette::Mix(palette_start, palette_end, 50, 50, 0.5, 1.0, Color::ColorSpace::kOklab, std::nullopt);
    Vector<FontPalette::FontPaletteOverride> actual_color_records = palette_interpolation.ComputeInterpolableFontPalette(palette.get());
...
TEST_F(PaletteInterpolationTest, MixCustomPalettesInOklab)
{
    ...
    scoped_refptr<FontPalette> palette = FontPalette::Mix(palette_start, palette_end, 70, 30, 0.3, 1.0, Color::ColorSpace::kOklab, std::nullopt);
    Vector<FontPalette::FontPaletteOverride> actual_color_records = palette_interpolation.ComputeInterpolableFontPalette(palette.get());
...
TEST_F(PaletteInterpolationTest, MixCustomPalettesInSRGB)
{
    ...
    scoped_refptr<FontPalette> palette = FontPalette::Mix(palette_start, palette_end, 70, 30, 0.3, 1.0, Color::ColorSpace::kSRGB, std::nullopt);
    Vector<FontPalette::FontPaletteOverride> actual_color_records = palette_interpolation.ComputeInterpolableFontPalette(palette.get());
    // We expect each color to be equal palette_start * 0.7 + palette_end * 0.3
    // after interpolation in the Oklab interpolation color space and conversion
    // back to sRGB.
