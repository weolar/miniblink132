...
// fontconfig lookups.
BASE_FEATURE(kOptimizeLinuxFonts, "OptimizeLinuxFonts", base::FEATURE_DISABLED_BY_DEFAULT);
...
FontPlatformData::FontPlatformData(const FontPlatformData& src, float text_size)
    : FontPlatformData(src.typeface_,
        ...
        std::string(),
#endif
        text_size, src.synthetic_bold_, src.synthetic_italic_, src.text_rendering_, src.resolved_font_features_, src.orientation_)
{
}

FontPlatformData::FontPlatformData(sk_sp<SkTypeface> typeface, const std::string& family, float text_size, bool synthetic_bold, bool synthetic_italic,
    TextRenderingMode text_rendering, ResolvedFontFeatures resolved_font_features, FontOrientation orientation)
    : typeface_(typeface)
    ,
#if !BUILDFLAG(IS_WIN) && !BUILDFLAG(IS_MAC)
    family_(family)
...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    bool override_font_name_and_size = base::FeatureList::IsEnabled(kOptimizeLinuxFonts);
#else
    bool override_font_name_and_size = false;
#endif // BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    if (override_font_name_and_size) {
        system_style = QuerySystemRenderStyle(FontFamilyName().Utf8(), 0, typeface_->fontStyle(), text_rendering);
    } else {
        system_style = QuerySystemRenderStyle(family, text_size, typeface_->fontStyle(), text_rendering);
    }

    // In web tests, ignore system preference for subpixel positioning,
    // or explicitly disable if requested.
    if (WebTestSupport::IsRunningWebTest()) {
        system_style.use_subpixel_positioning = WebTestSupport::IsTextSubpixelPositioningAllowedForTest() ? WebFontRenderStyle::kNoPreference : 0;
    }
#else
...
bool FontPlatformData::operator==(const FontPlatformData& a) const
{
    ...
    return typefaces_equal && text_size_ == a.text_size_ && is_hash_table_deleted_value_ == a.is_hash_table_deleted_value_
        && synthetic_bold_ == a.synthetic_bold_ && synthetic_italic_ == a.synthetic_italic_ && avoid_embedded_bitmaps_ == a.avoid_embedded_bitmaps_
        && text_rendering_ == a.text_rendering_ && resolved_font_features_ == a.resolved_font_features_ &&
#if !BUILDFLAG(IS_MAC)
        style_ == a.style_ &&
#endif
...
{
    DCHECK(Typeface());
    SkTypeface::LocalizedStrings* font_family_iterator = Typeface()->createFamilyNameIterator();
    SkTypeface::LocalizedString localized_string;
    while (font_family_iterator->next(&localized_string) && !localized_string.fString.size()) {
    }
...
{
    return EqualIgnoringASCIICase(FontFamilyName(), "ahem") || EqualIgnoringASCIICase(FontFamilyName(), "ahem (fontations)");
}
...
unsigned FontPlatformData::GetHash() const
{
    unsigned h = UniqueID();
    h ^= 0x01010101
        * ((static_cast<int>(is_hash_table_deleted_value_) << 3) | (static_cast<int>(orientation_) << 2) | (static_cast<int>(synthetic_bold_) << 1)
...
WebFontRenderStyle FontPlatformData::QuerySystemRenderStyle(
    const std::string& family, float text_size, SkFontStyle font_style, TextRenderingMode text_rendering)
{
    ...
    if (family.length() && Platform::Current()->GetSandboxSupport()) {
        ...
        Platform::Current()->GetSandboxSupport()->GetWebFontRenderStyleForStrike(
            family.data(), text_size, is_bold, is_italic, FontCache::DeviceScaleFactor(), &result);
    }
    ...
    if (text_rendering == TextRenderingMode::kGeometricPrecision && result.use_anti_alias) {
...
IdentifiableToken FontPlatformData::ComputeTypefaceDigest() const
{
    ...
    for (SkFontTableTag table_tag : kTablesToFullyDigest) {
        ...
        if (table_data) {
            table_data_span = base::span<const uint8_t>(table_data->bytes(), table_data->size());
        }
