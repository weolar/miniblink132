...
public:
    ...
    enum LigaturesState { kNormalLigaturesState, kDisabledLigaturesState, kEnabledLigaturesState };
    ...
    enum FontVariantCaps { kCapsNormal, kSmallCaps, kAllSmallCaps, kPetiteCaps, kAllPetiteCaps, kUnicase, kTitlingCaps };
    static String ToString(FontVariantCaps);
    static String ToStringForIdl(FontVariantCaps);

    enum FontSynthesisWeight { kAutoFontSynthesisWeight, kNoneFontSynthesisWeight };
    ...
    enum FontSynthesisSmallCaps { kAutoFontSynthesisSmallCaps, kNoneFontSynthesisSmallCaps };
    static String ToString(FontSynthesisSmallCaps);

    enum FontVariantPosition { kNormalVariantPosition, kSubVariantPosition, kSuperVariantPosition };
    ...
    const FontFamily& FirstFamily() const
    {
        return family_list_;
    }
    Size GetSize() const
    ...
    FontSizeAdjust SizeAdjust() const
    {
        return size_adjust_;
    }
    ...
    {
        return int(computed_size_ + 0.5f);
    }
    ...
    {
        return font_selection_request_.weight;
    }
    FontSelectionValue Style() const
    {
        return font_selection_request_.slope;
    }
    FontSelectionValue Stretch() const
    {
        return font_selection_request_.width;
    }
    ...
    {
        return Family().FamilyName() == font_family_names::kMonospace && Family().FamilyIsGeneric() && !Family().Next();
    }
    ...
    {
        return FontVariantEastAsian::InitializeFromUnsigned(fields_.variant_east_asian_);
    }
    ...
    const LayoutLocale* Locale() const
    {
        return locale_.get();
    }
    ...
    {
        return LocaleOrDefault().GetScript();
    }
    bool IsSyntheticBold() const
    {
        return fields_.synthetic_bold_;
    }
    bool IsSyntheticItalic() const
    {
        return fields_.synthetic_italic_;
    }
    bool IsSyntheticOblique() const
    {
        return fields_.synthetic_oblique_;
    }
    ...
    {
        return static_cast<FontSynthesisSmallCaps>(fields_.font_synthesis_small_caps_);
    }
    ...
    FontCacheKey CacheKey(const FontFaceCreationParams&, bool is_unique_match) const;
    ...
    {
        computed_size_ = ClampTo<float>(s);
    }
    void SetSpecifiedSize(float s)
    {
        specified_size_ = ClampTo<float>(s);
    }
    void SetAdjustedSize(float s)
    {
        adjusted_size_ = ClampTo<float>(s);
    }
    ...
    void SetWeight(FontSelectionValue w)
    {
        font_selection_request_.weight = w;
    }
    void SetStretch(FontSelectionValue s)
    {
        font_selection_request_.width = s;
    }
    ...
    void SetKeywordSize(unsigned s)
    {
        fields_.keyword_size_ = s;
    }
    ...
    void SetFontVariantAlternates(scoped_refptr<const FontVariantAlternates> alternates)
    {
    ...
    void SetFontSynthesisSmallCaps(FontSynthesisSmallCaps font_synthesis_small_caps)
    {
    ...
    void SetVariationSettings(scoped_refptr<const FontVariationSettings> settings)
    {
    ...
    {
        return use_subpixel_text_positioning_;
    }
    ...
    unsigned BitmapFields() const
    {
        return fields_as_unsigned_.parts[0];
    }
...
private:
    ...
    struct BitFields {
        ...
        unsigned is_absolute_size_ : 1; // Whether or not CSS specified an explicit size
    ...
    static_assert(sizeof(BitFields) == sizeof(FieldsAsUnsignedType), "Mapped bitfield datatypes must have identical size.");
    union {
        BitFields fields_;
...
template <> struct HashTraits<blink::FontDescription> : SimpleClassHashTraits<blink::FontDescription> {
...
template <> struct CrossThreadCopier<blink::FontDescription> : public CrossThreadCopierPassThrough<blink::FontDescription> {
