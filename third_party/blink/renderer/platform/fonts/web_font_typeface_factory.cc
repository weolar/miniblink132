...
sk_sp<SkTypeface> MakeVariationsTypeface(sk_sp<SkData> data, const WebFontTypefaceFactory::FontInstantiator& instantiator)
{
#if BUILDFLAG(IS_WIN)
...
sk_sp<SkTypeface> MakeSbixTypeface(sk_sp<SkData> data, const WebFontTypefaceFactory::FontInstantiator& instantiator)
{
    ...
    if ((IsFreeTypeSystemRasterizer() || IsWin())
        && (RuntimeEnabledFeatures::FontationsForSelectedFormatsEnabled() || RuntimeEnabledFeatures::FontationsFontBackendEnabled())) {
...
sk_sp<SkTypeface> MakeColrV0Typeface(sk_sp<SkData> data, const WebFontTypefaceFactory::FontInstantiator& instantiator)
{
...
sk_sp<SkTypeface> MakeColrV0VariationsTypeface(sk_sp<SkData> data, const WebFontTypefaceFactory::FontInstantiator& instantiator)
{
#if BUILDFLAG(IS_WIN)
    if (DWriteVersionSupportsVariations()) {
        return instantiator.make_system(data);
    ...
    if ((RuntimeEnabledFeatures::FontationsForSelectedFormatsEnabled() || RuntimeEnabledFeatures::FontationsFontBackendEnabled())) {
        return instantiator.make_fontations(data);
    } else {
...
sk_sp<SkTypeface> MakeUseFallbackIfNeeded(sk_sp<SkData> data, const WebFontTypefaceFactory::FontInstantiator& instantiator)
{
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_APPLE)
    return instantiator.make_fallback(data);
#else
...
sk_sp<SkTypeface> MakeFontationsFallbackPreferred(sk_sp<SkData> data, const WebFontTypefaceFactory::FontInstantiator& instantiator)
{
    if (RuntimeEnabledFeatures::FontationsForSelectedFormatsEnabled() || RuntimeEnabledFeatures::FontationsFontBackendEnabled()) {
        return instantiator.make_fontations(data);
    }
...
bool WebFontTypefaceFactory::CreateTypeface(sk_sp<SkData> data, sk_sp<SkTypeface>& typeface)
{
...
bool WebFontTypefaceFactory::CreateTypeface(
    sk_sp<SkData> data, sk_sp<SkTypeface>& typeface, const FontFormatCheck& format_check, const FontInstantiator& instantiator)
{
    CHECK(!typeface);

    if (!format_check.IsVariableFont() && !format_check.IsColorFont() && !format_check.IsCff2OutlineFont()) {
        typeface = instantiator.make_system(data);
        if (typeface) {
            ReportInstantiationResult(InstantiationResult::kSuccessConventionalWebFont);
            return true;
        }
    ...
    using InstantionFunctionWithInstantiator = sk_sp<SkTypeface> (*)(sk_sp<SkData>, const FontInstantiator& instantiator);
    ...
    struct {
    ...
    } instantiation_rules[] = { // We don't expect variable CBDT/CBLC or Sbix variable fonts for now.
        { &FontFormatCheck::IsCbdtCblcColorFont, &MakeFontationsFallbackPreferred, InstantiationResult::kSuccessCbdtCblcColorFont, std::nullopt },
        { &FontFormatCheck::IsColrCpalColorFontV1, &MakeFontationsFallbackPreferred, InstantiationResult::kSuccessColrV1Font, std::nullopt },
        { &FontFormatCheck::IsSbixColorFont, &MakeSbixTypeface, InstantiationResult::kSuccessSbixFont, std::nullopt },
        { &FontFormatCheck::IsCff2OutlineFont, &MakeFontationsFallbackPreferred, InstantiationResult::kSuccessCff2Font, std::nullopt },
        ...
        // variations.
        { &FontFormatCheck::IsVariableColrV0Font, &MakeColrV0VariationsTypeface, InstantiationResult::kSuccessColrCpalFont, std::nullopt },
        { &FontFormatCheck::IsVariableFont, &MakeVariationsTypeface, InstantiationResult::kSuccessVariableWebFont,
            InstantiationResult::kErrorInstantiatingVariableFont },
        { &FontFormatCheck::IsColrCpalColorFontV0, &MakeColrV0Typeface, InstantiationResult::kSuccessColrCpalFont, std::nullopt }
    };
...
void WebFontTypefaceFactory::ReportInstantiationResult(InstantiationResult result)
{
