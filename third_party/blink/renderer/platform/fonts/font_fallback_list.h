...
class PLATFORM_EXPORT FontFallbackList : public GarbageCollected<FontFallbackList> {
public:
    ...
    const SimpleFontData* PrimarySimpleFontData(const FontDescription& font_description)
    {
        if (nullify_primary_font_data_for_test_) {
            return nullptr;
        }
        if (!cached_primary_simple_font_data_) {
            cached_primary_simple_font_data_ = DeterminePrimarySimpleFontData(font_description);
    ...
    bool HasLoadingFallback() const
    {
        return has_loading_fallback_;
    }
    bool HasCustomFont() const
    {
        return has_custom_font_;
    }
...
private:
    ...
    const SimpleFontData* DeterminePrimarySimpleFontDataCore(const FontDescription&);
