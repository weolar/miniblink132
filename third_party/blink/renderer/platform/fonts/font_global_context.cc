...
ThreadSpecific<Persistent<FontGlobalContext>>& GetThreadSpecificFontGlobalContextPool()
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(ThreadSpecific<Persistent<FontGlobalContext>>, thread_specific_pool, ());
...
FontUniqueNameLookup* FontGlobalContext::GetFontUniqueNameLookup()
{
    if (!Get().font_unique_name_lookup_) {
        Get().font_unique_name_lookup_ = FontUniqueNameLookup::GetPlatformUniqueNameLookup();
    }
    return Get().font_unique_name_lookup_.get();
}

IdentifiableToken FontGlobalContext::GetOrComputeTypefaceDigest(const FontPlatformData& source)
{
...
IdentifiableToken FontGlobalContext::GetOrComputePostScriptNameDigest(const FontPlatformData& source)
{
    SkTypeface* typeface = source.Typeface();
    if (!typeface)
    ...
    if (iter == postscript_name_digest_cache_.end())
        iter = postscript_name_digest_cache_.Put(font_id, IdentifiabilityBenignStringToken(source.GetPostScriptName()));
    DCHECK(iter->second == IdentifiabilityBenignStringToken(source.GetPostScriptName()));
    return iter->second;
}
