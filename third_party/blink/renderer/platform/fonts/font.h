...
public:
    ...
    {
        visitor->Trace(font_fallback_list_);
    }
    ...
    enum CustomFontNotReadyAction { kDoNotPaintIfFontNotReady, kUseFallbackIfFontNotReady };

    void DrawText(cc::PaintCanvas*, const TextRunPaintInfo&, const gfx::PointF&, const cc::PaintFlags&, DrawType = DrawType::kGlyphsOnly) const;
    void DrawText(
        cc::PaintCanvas*, const TextRunPaintInfo&, const gfx::PointF&, cc::NodeId node_id, const cc::PaintFlags&, DrawType = DrawType::kGlyphsOnly) const;
    void DrawText(
        cc::PaintCanvas*, const TextFragmentPaintInfo&, const gfx::PointF&, cc::NodeId node_id, const cc::PaintFlags&, DrawType = DrawType::kGlyphsOnly) const;
    bool DrawBidiText(
        cc::PaintCanvas*, const TextRunPaintInfo&, const gfx::PointF&, CustomFontNotReadyAction, const cc::PaintFlags&, DrawType = DrawType::kGlyphsOnly) const;
    void DrawEmphasisMarks(cc::PaintCanvas*, const TextRunPaintInfo&, const AtomicString& mark, const gfx::PointF&, const cc::PaintFlags&) const;
    void DrawEmphasisMarks(cc::PaintCanvas*, const TextFragmentPaintInfo&, const AtomicString& mark, const gfx::PointF&, const cc::PaintFlags&) const;
    ...
    void GetTextIntercepts(const TextRunPaintInfo&, const cc::PaintFlags&, const std::tuple<float, float>& bounds, Vector<TextIntercept>&) const;
    void GetTextIntercepts(const TextFragmentPaintInfo&, const cc::PaintFlags&, const std::tuple<float, float>& bounds, Vector<TextIntercept>&) const;
    ...
    float SubRunWidth(const TextRun&, unsigned from, unsigned to, gfx::RectF* glyph_bounds = nullptr) const;

    int OffsetForPosition(const TextRun&, float position, IncludePartialGlyphsOption, BreakGlyphsOption) const;
    gfx::RectF SelectionRectForText(const TextRun&, const gfx::PointF&, float height, int from = 0, int to = -1) const;
    ...
    void ReportEmojiSegmentGlyphCoverage(unsigned num_clusters, unsigned num_broken_clusters) const;
...
public:
    ...
    FontFallbackIterator CreateFontFallbackIterator(FontFallbackPriority fallback_priority) const
    {
        EnsureFontFallbackList();
        return FontFallbackIterator(font_description_, font_fallback_list_, fallback_priority);
    }
...
inline float Font::TabWidth(const SimpleFontData* font_data, const TabSize& tab_size) const
{
    if (!font_data)
...
template <> struct CrossThreadCopier<blink::Font> : public CrossThreadCopierPassThrough<blink::Font> {
