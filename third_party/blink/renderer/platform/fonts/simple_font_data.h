...
public:
    GlyphData(Glyph g = 0, const SimpleFontData* f = nullptr, CanvasRotationInVertical rotation = CanvasRotationInVertical::kRegular)
        : glyph(g)
        , font_data(f)
...
public:
    ...
    SimpleFontData(const FontPlatformData*, const CustomFontData* custom_data = nullptr, bool subpixel_ascent_descent = false,
    ...
    FontHeight NormalizedTypoAscentAndDescent(FontBaseline baseline_type = kAlphabeticBaseline) const;
    ...
    const HanKerning::FontData& HanKerningData(const LayoutLocale& locale, bool is_horizontal) const;
...
private:
    ...
    SimpleFontData* CreateScaledFontData(const FontDescription&, float scale_factor) const;
...
ALWAYS_INLINE gfx::RectF SimpleFontData::BoundsForGlyph(Glyph glyph) const
{
    ...
    if (glyph_to_bounds_map_) {
        if (std::optional<gfx::RectF> glyph_bounds = glyph_to_bounds_map_->MetricsForGlyph(glyph)) {
            return *glyph_bounds;
        }
    }
...
template <> struct DowncastTraits<SimpleFontData> {
