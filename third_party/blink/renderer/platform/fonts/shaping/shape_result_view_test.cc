...
{
    auto GetExpandedRange = [](const String& text, bool ltr, unsigned from, unsigned to) -> Vector<unsigned> {
        FontDescription::VariantLigatures ligatures(FontDescription::kEnabledLigaturesState);
        Font font = test::CreateTestFont(AtomicString("roboto"), test::PlatformTestDataPath("third_party/Roboto/roboto-regular.woff2"), 100, &ligatures);

        HarfBuzzShaper shaper(text);
        const ShapeResultView* shape_result = ShapeResultView::Create(shaper.Shape(&font, ltr ? TextDirection::kLtr : TextDirection::kRtl));
...
TEST_F(ShapeResultViewTest, ExpandRangeToIncludePartialGlyphsWithCombiningCharacter)
{
    ...
    const ShapeResultView* view = ShapeResultView::Create(result, result->StartIndex(), result->EndIndex());
    unsigned from = 0;
...
TEST_F(ShapeResultViewTest, LatinSingleView)
{
    ...
    String string = To16Bit("Test run with multiple words and breaking opportunities.");
...
TEST_F(ShapeResultViewTest, ArabicSingleView)
{
    ...
    const ShapeResult* first_wortd_reference = first_reference_shaper.Shape(&font, direction);
    Vector<ShapeResultTestGlyphInfo> first_reference_glyphs;
    first_wortd_reference->ForEachGlyph(0, AddGlyphInfo, static_cast<void*>(&first_reference_glyphs));
    EXPECT_EQ(first_reference_glyphs.size(), 4u);

    EXPECT_TRUE(CompareResultGlyphs(first_glyphs, first_reference_glyphs, 0u, 4u));
...
TEST_F(ShapeResultViewTest, PreviousSafeToBreak)
{
    ...
    TextDirection direction = TextDirection::kRtl;
    HarfBuzzShaper shaper(string);
    const RunSegmenter::RunSegmenterRange range = { 51, 131, USCRIPT_HEBREW, blink::OrientationIterator::kOrientationKeep, blink::FontFallbackPriority::kText };
    const ShapeResult* shape_result = shaper.Shape(&font, direction, 51, 131, range);

    unsigned start_offset = 59;
    unsigned end_offset = 118;
    const ShapeResultView* result_view = ShapeResultView::Create(shape_result, start_offset, end_offset);
...
TEST_F(ShapeResultViewTest, LatinMultiRun)
{
    ...
    composite_view->ForEachGlyph(0, AddGlyphInfo, static_cast<void*>(&view_glyphs));
    ...
    reference_result->ForEachGlyph(0, AddGlyphInfo, static_cast<void*>(&reference_glyphs));

    ShapeResult* composite_copy = MakeGarbageCollected<ShapeResult>(&font, 0, 0, direction);
    ...
    composite_copy->ForEachGlyph(0, AddGlyphInfo, static_cast<void*>(&composite_copy_glyphs));

    EXPECT_TRUE(CompareResultGlyphs(view_glyphs, reference_glyphs, 0u, 16u));
    EXPECT_TRUE(CompareResultGlyphs(composite_copy_glyphs, reference_glyphs, 0u, 16u));
...
TEST_F(ShapeResultViewTest, LatinCompositeView)
{
    ...
    composite_copy->ForEachGlyph(0, AddGlyphInfo, static_cast<void*>(&reference_glyphs));
    ...
    composite_view->ForEachGlyph(0, AddGlyphInfo, static_cast<void*>(&composite_glyphs));
...
TEST_F(ShapeResultViewTest, MixedScriptsCompositeView)
{
    ...
    Vector<ShapeResultTestGlyphInfo> composite_glyphs;
    composite_view->ForEachGlyph(0, AddGlyphInfo, static_cast<void*>(&composite_glyphs));
    EXPECT_TRUE(CompareResultGlyphs(composite_glyphs, reference_glyphs, 0u, reference_glyphs.size()));
    EXPECT_EQ(composite_view->Width(), composite_copy->Width());
}
...
TEST_F(ShapeResultViewTest, MarkerAndTrailingSpace)
{
    ...
    const ShapeResult* result = ShapeResult::CreateForSpaces(&font, direction, 1, 2, symbol_width);
...
TEST_F(ShapeResultViewTest, SpacesInLTR)
{
    ...
    const auto* result = ShapeResult::CreateForSpaces(&font, TextDirection::kLtr, kStartIndex, kLength, kWidth);
...
TEST_F(ShapeResultViewTest, SpacesInRTL)
{
    ...
    constexpr unsigned kLength = 2;
    constexpr float kWidth = 8;
    const auto* result = ShapeResult::CreateForSpaces(&font, TextDirection::kRtl, kStartIndex, kLength, kWidth);
...
TEST_F(ShapeResultViewTest, TabulationCharactersInLTR)
{
    ...
    constexpr unsigned kStartIndex = 0;
    constexpr unsigned kLength = 2;
    const auto* result = ShapeResult::CreateForTabulationCharacters(&font, TextDirection::kLtr, TabSize(8), kPosition, kStartIndex, kLength);
...
TEST_F(ShapeResultViewTest, TabulationCharactersInRTL)
{
    ...
    constexpr unsigned kStartIndex = 0;
    constexpr unsigned kLength = 2;
    const auto* result = ShapeResult::CreateForTabulationCharacters(&font, TextDirection::kRtl, TabSize(8), kPosition, kStartIndex, kLength);
