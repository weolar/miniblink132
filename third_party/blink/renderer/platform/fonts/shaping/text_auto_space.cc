...
float TextAutoSpace::GetSpacingWidth(const Font* font)
{
    if (const SimpleFontData* font_data = font->PrimaryFont()) {
        return font_data->IdeographicInlineSize().value_or(font_data->PlatformData().size()) / 8;
    }
    NOTREACHED();
}

// static
TextAutoSpace::CharType TextAutoSpace::GetTypeAndNext(const String& text, wtf_size_t& offset)
{
    CHECK(!text.Is8Bit());
    UChar32 ch;
...
// static
TextAutoSpace::CharType TextAutoSpace::GetPrevType(const String& text, wtf_size_t offset)
{
    DCHECK_GT(offset, 0u);
    CHECK(!text.Is8Bit());
    UChar32 last_ch;
...
TextAutoSpace::CharType TextAutoSpace::GetType(UChar32 ch)
{
    ...
    if (gc_mask & (U_GC_L_MASK | U_GC_M_MASK | U_GC_ND_MASK)) {
        const UEastAsianWidth eaw = static_cast<UEastAsianWidth>(u_getIntPropertyValue(ch, UCHAR_EAST_ASIAN_WIDTH));
