...
{
    output << "{ start_index=" << x.start_index << ", num_characters=" << x.num_characters << ", num_glyphs=" << x.num_glyphs << ", script=" << x.script
           << " }";
}
...
protected:
    ...
    {
        // Mac
        scoped_refptr<SharedFontFamily> itf = SharedFontFamily::Create(AtomicString("ITF Devanagari"), FontFamily::Type::kFamilyName);
        // Linux
        scoped_refptr<SharedFontFamily> lohit = SharedFontFamily::Create(AtomicString("Lohit Devanagari"), FontFamily::Type::kFamilyName, std::move(itf));
        // Windows 7
        scoped_refptr<SharedFontFamily> mangal = SharedFontFamily::Create(AtomicString("Mangal"), FontFamily::Type::kFamilyName, std::move(lohit));
        // Windows 10
        font_description.SetFamily(FontFamily(AtomicString("Nirmala UI"), FontFamily::Type::kFamilyName, std::move(mangal)));
    }

    Font CreateAhem(float size)
    {
        FontDescription::VariantLigatures ligatures;
        return blink::test::CreateTestFont(AtomicString("Ahem"), blink::test::PlatformTestDataPath("Ahem.woff"), size, &ligatures);
    }

    Font CreateNotoColorEmoji(FontVariantEmoji variant_emoji = kNormalVariantEmoji)
    {
        return blink::test::CreateTestFont(
            AtomicString("NotoColorEmoji"), blink::test::BlinkWebTestsDir() + "/third_party/NotoColorEmoji/NotoColorEmoji.ttf", 12, nullptr, variant_emoji);
    }

    Font CreateNotoEmoji(FontVariantEmoji variant_emoji = kNormalVariantEmoji)
    {
        return blink::test::CreateTestFont(
            AtomicString("NotoEmoji"), blink::test::BlinkWebTestsDir() + "/third_party/NotoEmoji/NotoEmoji-Regular.subset.ttf", 12, nullptr, variant_emoji);
    }
    ...
    {
        DCHECK(text.length() == 1 || (text.length() == 2 && (text.EndsWith(u"\ufe0e") || text.EndsWith(u"\ufe0f"))));
    ...
    const ShapeResult* CreateMissingRunResult(TextDirection direction)
    {
        ...
        ShapeResult* result = MakeGarbageCollected<ShapeResult>(&font, 2, 8, direction);
...
public:
    ...
    {
        prev_subpixel_allowed_ = WebTestSupport::IsTextSubpixelPositioningAllowedForTest();
    ...
    ~ScopedSubpixelOverride()
    {
        ...
        WebTestSupport::SetTextSubpixelPositioningAllowedForTest(prev_subpixel_allowed_);
...
class ShapeParameterTest : public HarfBuzzShaperTest, public testing::WithParamInterface<TextDirection> {
protected:
...
INSTANTIATE_TEST_SUITE_P(HarfBuzzShaperTest, ShapeParameterTest, testing::Values(TextDirection::kLtr, TextDirection::kRtl));
...
TEST_F(HarfBuzzShaperTest, ResolveCandidateRunsUnicodeVariants)
{
    ...
    for (auto& test : testlist) {
        ...
        ASSERT_TRUE(TestInfo(result)->RunInfoForTesting(0, start_index_, num_glyphs_, script_)) << test.name;
        ...
        if (num_glyphs_ == 2) {
            ...
            EXPECT_EQ(TestInfo(result)->FontDataForTesting(0)->SpaceGlyph(), TestInfo(result)->GlyphForTesting(0, 1)) << test.name;
...
TEST_F(HarfBuzzShaperTest, ResolveCandidateRunsDevanagariCommonLatinCommon)
{
    ...
    for (unsigned i = 0; i < TestInfo(result)->NumberOfRunsForTesting(); ++i) {
        ...
        EXPECT_EQ(script_, finished_devanagari ? HB_SCRIPT_LATIN : HB_SCRIPT_DEVANAGARI);
    }
}
...
TEST_F(HarfBuzzShaperTest, ShapeLatinSegment)
{
    ...
    ASSERT_TRUE(TestInfo(first)->RunInfoForTesting(0, start_index_, num_characters_, num_glyphs_, script_));
    EXPECT_EQ(0u, start_index_);
    EXPECT_EQ(6u, num_characters_);
    ASSERT_TRUE(TestInfo(second)->RunInfoForTesting(0, start_index_, num_characters_, num_glyphs_, script_));
    EXPECT_EQ(6u, start_index_);
    EXPECT_EQ(5u, num_characters_);
    ASSERT_TRUE(TestInfo(third)->RunInfoForTesting(0, start_index_, num_characters_, num_glyphs_, script_));
...
TEST_F(HarfBuzzShaperTest, ShapeTabulationCharacters)
{
    ...
    const ShapeResult* result = ShapeResult::CreateForTabulationCharacters(&font, TextDirection::kLtr, TabSize(8), 0.f, 0, length);
...
TEST_F(HarfBuzzShaperTest, ShapeVerticalUpright)
{
    ...
    const ShapeResult* result2 = shaper.Shape(&font, direction, 3, string.length());

    ShapeResult* composite_result = MakeGarbageCollected<ShapeResult>(&font, 0, 0, direction);
...
class ShapeStringTest : public HarfBuzzShaperTest, public testing::WithParamInterface<const char16_t*> { };

INSTANTIATE_TEST_SUITE_P(HarfBuzzShaperTest, ShapeStringTest,
    testing::Values(
...
TEST_P(ShapeParameterTest, MaxGlyphsSimple)
{
    ...
    EXPECT_THAT(runs,
        IsLtr(GetParam())
            ? testing::ElementsAre(ShapeResultRunData { 0, length - 1, length - 1, HB_SCRIPT_LATIN }, ShapeResultRunData { length - 1, 1, 1, HB_SCRIPT_LATIN })
            : testing::ElementsAre(ShapeResultRunData { 1, length - 1, length - 1, HB_SCRIPT_LATIN }, ShapeResultRunData { 0, 1, 1, HB_SCRIPT_LATIN }));
}
...
TEST_P(ShapeParameterTest, MaxGlyphsClusterLatin)
{
    ...
    EXPECT_THAT(runs,
        IsLtr(GetParam())
            ? testing::ElementsAre(
                ShapeResultRunData { 0, length - 2, length - 2, HB_SCRIPT_LATIN }, ShapeResultRunData { length - 2, 2u, 2u, HB_SCRIPT_LATIN })
            : testing::ElementsAre(ShapeResultRunData { 2, length - 2, length - 2, HB_SCRIPT_LATIN }, ShapeResultRunData { 0, 2, 2, HB_SCRIPT_LATIN }));
}
...
TEST_P(ShapeParameterTest, MaxGlyphsClusterLatin2)
{
    ...
    EXPECT_THAT(runs,
        IsLtr(GetParam())
            ? testing::ElementsAre(
                ShapeResultRunData { 0, length - 2, length - 2, HB_SCRIPT_LATIN }, ShapeResultRunData { length - 2, 2u, 2u, HB_SCRIPT_LATIN })
            : testing::ElementsAre(ShapeResultRunData { 2, length - 2, length - 2, HB_SCRIPT_LATIN }, ShapeResultRunData { 0, 2u, 2u, HB_SCRIPT_LATIN }));
}
...
TEST_P(ShapeParameterTest, MaxGlyphsClusterDevanagari)
{
    ...
    Vector<ShapeResultRunData> runs = ShapeResultRunData::Get(result);
    EXPECT_THAT(runs,
        IsLtr(GetParam()) ? testing::ElementsAre(
            ShapeResultRunData { 0, length - 3, length - 3, HB_SCRIPT_DEVANAGARI }, ShapeResultRunData { length - 3, 3u, 3u, HB_SCRIPT_DEVANAGARI })
                          : testing::ElementsAre(
                              ShapeResultRunData { 3, length - 3, length - 3, HB_SCRIPT_DEVANAGARI }, ShapeResultRunData { 0, 3u, 3u, HB_SCRIPT_DEVANAGARI }));
}

TEST_P(ShapeParameterTest, ZeroWidthSpace)
{
    UChar string[] = { kZeroWidthSpaceCharacter, kZeroWidthSpaceCharacter, 0x0627, 0x0631, 0x062F, 0x0648, kZeroWidthSpaceCharacter, kZeroWidthSpaceCharacter };
...
TEST_F(HarfBuzzShaperTest, IdeographicSpace)
{
    ...
    String string(u"\u3001" // IDEOGRAPHIC COMMA
                  u"\u3000"); // IDEOGRAPHIC SPACE
    HarfBuzzShaper shaper(string);
    const ShapeResult* result = shaper.Shape(&font, TextDirection::kLtr);
    HeapVector<ShapeResult::RunFontData> run_font_data;
...
TEST_F(HarfBuzzShaperTest, SystemEmojiVS15)
{
    ...
    ScopedSystemFallbackEmojiVSSupportForTest scoped_feature_system_emoji_vs(true);
    ...
    for (String text : { text_default, emoji_default }) {
        EXPECT_EQ(MaybeStripFontationsSuffix(GetShapedFontFamilyNameForEmojiVS(mono_font, text)), String(kNotoEmojiFontName));
...
#endif
        EXPECT_EQ(MaybeStripFontationsSuffix(GetShapedFontFamilyNameForEmojiVS(color_font, text)), String(system_mono_font_name));
    }
}
...
TEST_F(HarfBuzzShaperTest, SystemEmojiVS16)
{
    ...
    String emoji_default(u"\u2614"
                         u"\ufe0f");
    for (String text : { text_default, emoji_default }) {
        EXPECT_EQ(GetShapedFontFamilyNameForEmojiVS(mono_font, text), kSystemColorEmojiFont);
        EXPECT_EQ(MaybeStripFontationsSuffix(GetShapedFontFamilyNameForEmojiVS(color_font, text)), kNotoColorEmojiFontName);
    }
}

const FontVariantEmoji variant_emoji_values[] = { kEmojiVariantEmoji, kTextVariantEmoji, kUnicodeVariantEmoji };

class FontVariantEmojiTest : public HarfBuzzShaperTest, public testing::WithParamInterface<FontVariantEmoji> { };

INSTANTIATE_TEST_SUITE_P(HarfBuzzShaperTest, FontVariantEmojiTest, testing::ValuesIn(variant_emoji_values));
...
TEST_P(FontVariantEmojiTest, FontVariantEmojiSystemFallback)
{
    ...
    for (String text : { text_default, emoji_default }) {
        bool is_text_presentation = (variant_emoji == kTextVariantEmoji) || (variant_emoji == kUnicodeVariantEmoji && text == text_default);
        bool is_emoji_presentation = (variant_emoji == kEmojiVariantEmoji) || (variant_emoji == kUnicodeVariantEmoji && text == emoji_default);

        const char* expected_name_for_mono_requested_font = is_text_presentation ? kNotoEmojiFontName : kSystemColorEmojiFont;
        const char* expected_name_for_color_requested_font = is_emoji_presentation ? kNotoColorEmojiFontName : kSystemMonoEmojiFont;
        ...
        EXPECT_EQ(MaybeStripFontationsSuffix(GetShapedFontFamilyNameForEmojiVS(mono_font, text)), String(expected_name_for_mono_requested_font));
        EXPECT_EQ(MaybeStripFontationsSuffix(GetShapedFontFamilyNameForEmojiVS(color_font, text)), String(expected_name_for_color_requested_font));
    }
}
...
TEST_F(HarfBuzzShaperTest, VSOverrideFontVariantEmoji)
{
    ...
    String text(u"\u2603\u2614\ufe0e\u2603\ufe0f");
    Font font = blink::test::CreateTestFont(AtomicString("Ahem"), blink::test::PlatformTestDataPath("Ahem.woff"), 12, nullptr, kEmojiVariantEmoji);
    ...
    EXPECT_EQ(run_font_data[0].font_data_->PlatformData().FontFamilyName(), kSystemColorEmojiFont);
    EXPECT_EQ(run_font_data[1].font_data_->PlatformData().FontFamilyName(), kSystemMonoEmojiFont);
    EXPECT_EQ(run_font_data[2].font_data_->PlatformData().FontFamilyName(), kSystemColorEmojiFont);
}
...
TEST_F(HarfBuzzShaperTest, FontVariantEmojiTextSystemFallback)
{
    ...
    EXPECT_EQ(GetShapedFontFamilyNameForEmojiVS(color_font, text), mono_font_name);
}
...
std::ostream& operator<<(std::ostream& ostream, const GlyphDataRangeTestData& data)
{
    return ostream << data.text;
}

class GlyphDataRangeTest : public HarfBuzzShaperTest, public testing::WithParamInterface<GlyphDataRangeTestData> { };

INSTANTIATE_TEST_SUITE_P(HarfBuzzShaperTest, GlyphDataRangeTest, testing::ValuesIn(glyph_data_range_test_data));
...
std::ostream& operator<<(std::ostream& ostream, const OffsetForPositionTestData& data)
{
    return ostream << data.position;
}

class OffsetForPositionTest : public HarfBuzzShaperTest, public testing::WithParamInterface<OffsetForPositionTestData> { };

INSTANTIATE_TEST_SUITE_P(HarfBuzzShaperTest, OffsetForPositionTest, testing::ValuesIn(offset_for_position_fixed_pitch_test_data));
...
TEST_P(OffsetForPositionTest, Data)
{
    ...
    const ShapeResult* result = SplitRun(shaper.Shape(&ahem, TextDirection::kLtr), 2);
    EXPECT_EQ(data.offset_ltr, result->OffsetForPosition(data.position, BreakGlyphsOption(false)));
    EXPECT_EQ(data.hit_test_ltr, result->CaretOffsetForHitTest(data.position, string, BreakGlyphsOption(false)));
    EXPECT_EQ(data.fit_ltr_ltr, result->OffsetToFit(data.position, TextDirection::kLtr));
    EXPECT_EQ(data.fit_ltr_rtl, result->OffsetToFit(data.position, TextDirection::kRtl));

    result = SplitRun(shaper.Shape(&ahem, TextDirection::kRtl), 3);
    EXPECT_EQ(data.offset_rtl, result->OffsetForPosition(data.position, BreakGlyphsOption(false)));
    EXPECT_EQ(data.hit_test_rtl, result->CaretOffsetForHitTest(data.position, string, BreakGlyphsOption(false)));
    EXPECT_EQ(data.fit_rtl_ltr, result->OffsetToFit(data.position, TextDirection::kLtr));
    EXPECT_EQ(data.fit_rtl_rtl, result->OffsetToFit(data.position, TextDirection::kRtl));
}
...
TEST_F(HarfBuzzShaperTest, PositionForOffsetLatin)
{
    ...
    ASSERT_NEAR(second->Width(), result->PositionForOffset(11) - result->PositionForOffset(6), 1);
...
class IncludePartialGlyphsTest : public HarfBuzzShaperTest, public ::testing::WithParamInterface<IncludePartialGlyphsOption> { };

INSTANTIATE_TEST_SUITE_P(HarfBuzzShaperTest, IncludePartialGlyphsTest,
    ::testing::Values(IncludePartialGlyphsOption::kOnlyFullGlyphs, IncludePartialGlyphsOption::kIncludePartialGlyphs));

TEST_P(IncludePartialGlyphsTest, OffsetForPositionMatchesPositionForOffsetLatin)
{
    Font font(font_description);
    ...
    EXPECT_EQ(0u, result->OffsetForPosition(result->PositionForOffset(0), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(1u, result->OffsetForPosition(result->PositionForOffset(1), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(2u, result->OffsetForPosition(result->PositionForOffset(2), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(3u, result->OffsetForPosition(result->PositionForOffset(3), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(4u, result->OffsetForPosition(result->PositionForOffset(4), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(5u, result->OffsetForPosition(result->PositionForOffset(5), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(6u, result->OffsetForPosition(result->PositionForOffset(6), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(7u, result->OffsetForPosition(result->PositionForOffset(7), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(8u, result->OffsetForPosition(result->PositionForOffset(8), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(9u, result->OffsetForPosition(result->PositionForOffset(9), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(10u, result->OffsetForPosition(result->PositionForOffset(10), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(11u, result->OffsetForPosition(result->PositionForOffset(11), string, partial, BreakGlyphsOption(false)));
    EXPECT_EQ(12u, result->OffsetForPosition(result->PositionForOffset(12), string, partial, BreakGlyphsOption(false)));
}

TEST_P(IncludePartialGlyphsTest, OffsetForPositionMatchesPositionForOffsetArabic)
{
    Font font(font_description);
...
TEST_P(IncludePartialGlyphsTest, OffsetForPositionMatchesPositionForOffsetMixed)
{
    Font font(font_description);
...
} shape_result_copy_range_test_data[] = { { u"ABC", TextDirection::kLtr, 1 }, { u"\u0648\u0644\u064A", TextDirection::kRtl, 1 },
    // These strings creates 3 runs. Split it in the middle of 2nd run.
    { u"\u65E5Hello\u65E5\u65E5", TextDirection::kLtr, 3 }, { u"\u0648\u0644\u064A AB \u0628\u062A", TextDirection::kRtl, 5 } };

std::ostream& operator<<(std::ostream& ostream, const ShapeResultCopyRangeTestData& data)
{
    return ostream << String(data.string) << " @ " << data.break_point << ", " << data.direction;
}

class ShapeResultCopyRangeTest : public HarfBuzzShaperTest, public testing::WithParamInterface<ShapeResultCopyRangeTestData> { };

INSTANTIATE_TEST_SUITE_P(HarfBuzzShaperTest, ShapeResultCopyRangeTest, testing::ValuesIn(shape_result_copy_range_test_data));
...
TEST_P(ShapeResultCopyRangeTest, Split)
{
    ...
    // Split the result.
    ShapeResult* result1 = MakeGarbageCollected<ShapeResult>(&font, 0, 0, direction);
    ...
    ShapeResult* result2 = MakeGarbageCollected<ShapeResult>(&font, 0, 0, direction);
    ...
    Vector<unsigned> expected_character_indexes = TestInfo(result)->CharacterIndexesForTesting();
    Vector<unsigned> composite_character_indexes = TestInfo(result)->CharacterIndexesForTesting();
...
TEST_P(ShapeResultCopyRangeTest, ShapeRange)
{
    ...
    // Shape each range.
    const ShapeResult* result1 = shaper.Shape(&font, direction, 0, test_data.break_point);
    EXPECT_EQ(test_data.break_point, result1->NumCharacters());
    const ShapeResult* result2 = shaper.Shape(&font, direction, test_data.break_point, string.length());
    EXPECT_EQ(string.length() - test_data.break_point, result2->NumCharacters());
...
TEST_F(HarfBuzzShaperTest, ShapeResultCopyRangeIntoLatin)
{
    ...
    EXPECT_EQ(result->SnappedStartPositionForOffset(0), composite_result->SnappedStartPositionForOffset(0));
    EXPECT_EQ(result->SnappedStartPositionForOffset(15), composite_result->SnappedStartPositionForOffset(15));
    EXPECT_EQ(result->SnappedStartPositionForOffset(30), composite_result->SnappedStartPositionForOffset(30));
    EXPECT_EQ(result->SnappedStartPositionForOffset(33), composite_result->SnappedStartPositionForOffset(33));
}
...
TEST_F(HarfBuzzShaperTest, ShapeResultCopyRangeIntoArabicThaiHanLatin)
{
    ...
    EXPECT_EQ(result->SnappedWidth(), composite_result->SnappedWidth());
    EXPECT_EQ(result->SnappedStartPositionForOffset(0), composite_result->SnappedStartPositionForOffset(0));
    EXPECT_EQ(result->SnappedStartPositionForOffset(1), composite_result->SnappedStartPositionForOffset(1));
    EXPECT_EQ(result->SnappedStartPositionForOffset(2), composite_result->SnappedStartPositionForOffset(2));
    EXPECT_EQ(result->SnappedStartPositionForOffset(3), composite_result->SnappedStartPositionForOffset(3));
    EXPECT_EQ(result->SnappedStartPositionForOffset(4), composite_result->SnappedStartPositionForOffset(4));
    EXPECT_EQ(result->SnappedStartPositionForOffset(5), composite_result->SnappedStartPositionForOffset(5));
    EXPECT_EQ(result->SnappedStartPositionForOffset(6), composite_result->SnappedStartPositionForOffset(6));
    EXPECT_EQ(result->SnappedStartPositionForOffset(7), composite_result->SnappedStartPositionForOffset(7));
    EXPECT_EQ(result->SnappedStartPositionForOffset(8), composite_result->SnappedStartPositionForOffset(8));
}
...
TEST_P(ShapeParameterTest, ShapeResultCopyRangeAcrossRuns)
{
    ...
    ShapeResult* target = MakeGarbageCollected<ShapeResult>(&font, 0, 0, direction);
...
TEST_F(HarfBuzzShaperTest, ShapeResultCopyRangeSegmentGlyphBoundingBox)
{
    ...
    const ShapeResult* result2 = shaper.Shape(&font, direction, 6, string.length());
...
TEST_F(HarfBuzzShaperTest, SafeToBreakLatinCommonLigatures)
{
    ...
    Font testFont = blink::test::CreateTestFont(
        AtomicString("MEgalopolis"), blink::test::PlatformTestDataPath("third_party/MEgalopolis/MEgalopolisExtra.woff"), 16, &ligatures);
    ...
    ShapeResult* copied_result = MakeGarbageCollected<ShapeResult>(&testFont, 0, 0, TextDirection::kLtr);
...
TEST_F(HarfBuzzShaperTest, SafeToBreakLatinDiscretionaryLigatures)
{
    ...
    for (unsigned i = 1; i < test_word.length() - 1; ++i) {
        EXPECT_EQ(safe_to_break_positions[compare_safe_to_break_position], result->NextSafeToBreakOffset(i));
    ...
    const ShapeResult* referenceResult = refShaper.Shape(&testFont, TextDirection::kLtr);
    ...
    for (unsigned i = 0; i < test_word.length(); ++i) {
        if (i == safe_to_break_positions[inserts_offset])
            inserts_offset++;
        EXPECT_EQ(result->SnappedStartPositionForOffset(i), referenceResult->SnappedStartPositionForOffset(i + inserts_offset));
    }
}
...
TEST_P(ShapeParameterTest, CopyRangeNoRuns)
{
    ...
    TextDirection direction = GetParam();
    const ShapeResult* result = MakeGarbageCollected<ShapeResult>(&font, 0, 2, direction);
...
static bool KerningIsHappening(const FontDescription& font_description, TextDirection direction, const String& str)
{
...
{
    EXPECT_TRUE(KerningIsHappening(font_description, TextDirection::kLtr, u"AVOID"));
    EXPECT_FALSE(KerningIsHappening(font_description, TextDirection::kLtr, u"NOID"));

    // We won't kern vertically with the default font.
    font_description.SetOrientation(FontOrientation::kVerticalUpright);

    EXPECT_FALSE(KerningIsHappening(font_description, TextDirection::kLtr, u"AVOID"));
    EXPECT_FALSE(KerningIsHappening(font_description, TextDirection::kLtr, u"NOID"));
}

TEST_F(HarfBuzzShaperTest, ShapeHorizontalWithoutSubpixelPositionWithoutKerningIsRounded)
{
    Font font(font_description);
    ...
    for (unsigned i = 0; i < string.length(); i++) {
        ...
        EXPECT_EQ(round(position), position) << "Position not rounded at offset " << i;
    }
}

#if BUILDFLAG(IS_ANDROID)
#define MAYBE_ShapeHorizontalWithSubpixelPositionWithoutKerningIsNotRounded DISABLED_ShapeHorizontalWithSubpixelPositionWithoutKerningIsNotRounded
#else
#define MAYBE_ShapeHorizontalWithSubpixelPositionWithoutKerningIsNotRounded ShapeHorizontalWithSubpixelPositionWithoutKerningIsNotRounded
#endif
TEST_F(HarfBuzzShaperTest, MAYBE_ShapeHorizontalWithSubpixelPositionWithoutKerningIsNotRounded)
{
    Font font(font_description);
...
TEST_F(HarfBuzzShaperTest, ShapeHorizontalWithoutSubpixelPositionWithKerningIsRounded)
{
    ScopedSubpixelOverride subpixel_override(false);
    ...
    font_description_copy.SetFamily(FontFamily(font_family_names::kArial, FontFamily::Type::kFamilyName));
...
#if BUILDFLAG(IS_ANDROID)
#define MAYBE_ShapeHorizontalWithSubpixelPositionWithKerningIsNotRounded DISABLED_ShapeHorizontalWithSubpixelPositionWithKerningIsNotRounded
#else
#define MAYBE_ShapeHorizontalWithSubpixelPositionWithKerningIsNotRounded ShapeHorizontalWithSubpixelPositionWithKerningIsNotRounded
#endif
TEST_F(HarfBuzzShaperTest, MAYBE_ShapeHorizontalWithSubpixelPositionWithKerningIsNotRounded)
{
    Font font(font_description);
    ScopedSubpixelOverride subpixel_override(true);
...
TEST_F(HarfBuzzShaperTest, MAYBE_EmojiPercentage)
{
    ...
    HarfBuzzShaper::EmojiMetricsCallback metrics_callback = base::BindLambdaForTesting([&](unsigned num_clusters, unsigned num_broken_clusters) {
        CHECK_EQ(num_clusters, expectations[num_calls].expected_clusters);
        CHECK_EQ(num_broken_clusters, expectations[num_calls].expected_broken_clusters);
...
TEST_F(HarfBuzzShaperTest, UnorderedClusterIndex)
{
    ...
    font_description.SetFamily(FontFamily(AtomicString("Geneva"), FontFamily::Type::kFamilyName));
    Font font(font_description);
