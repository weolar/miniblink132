...
public:
    ...
    void UpdateFallbackMetricsAndScale(const FontPlatformData& platform_data, HarfBuzzFace::VerticalLayoutCallbacks vertical_layout)
    {
        float ascent = 0;
        float descent = 0;
        ...
        if (vertical_layout == HarfBuzzFace::kPrepareForVerticalLayout) [[unlikely]] {
            FontMetrics::AscentDescentWithHacks(ascent, descent, platform_data, font_);
            ...
            int units_per_em = platform_data.GetHarfBuzzFace()->UnitsPerEmFromHeadTable();
            if (!units_per_em) {
                DLOG(ERROR) << "Units per EM is 0 for font used in vertical writing mode.";
            }
    ...
    OpenTypeVerticalData* VerticalData()
    {
        if (!vertical_data_) {
            ...
            vertical_data_ = MakeGarbageCollected<OpenTypeVerticalData>(font_.refTypeface());
        }
