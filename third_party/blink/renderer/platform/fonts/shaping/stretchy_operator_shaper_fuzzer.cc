...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    Font math = test::CreateTestFont(AtomicString("MathTestFont"), data, size, kFontSize, &ligatures);
    ...
    for (auto character : { kNAryWhiteVerticalBarCodePoint, kLeftBraceCodePoint, kOverBraceCodePoint }) {
        if (!math.PrimaryFont()->GlyphForCharacter(character))
            continue;
        StretchyOperatorShaper vertical_shaper(character, OpenTypeMathStretchData::StretchAxis::Vertical);
        StretchyOperatorShaper horizontal_shaper(character, OpenTypeMathStretchData::StretchAxis::Horizontal);
