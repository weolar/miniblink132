...
ShapingLineBreaker::ShapingLineBreaker(const ShapeResult* result, const LazyLineBreakIterator* break_iterator, const Hyphenation* hyphenation, const Font* font)
    : result_(result)
...
{
    return LazyLineBreakIterator::IsBreakableSpace(ch) || Character::IsOtherSpaceSeparator(ch);
}

bool IsAllSpaces(const String& text, unsigned start, unsigned end)
{
    return StringView(text, start, end - start).IsAllSpecialCharacters<IsBreakableSpace>();
}

bool ShouldHyphenate(const String& text, unsigned word_start, unsigned word_end, unsigned line_start)
{
...
inline ShapingLineBreaker::EdgeOffset ShapingLineBreaker::FirstSafeOffset(unsigned start) const
{
    if (!IsStartOfWrappedLine(start)) {
        // When it's not at the start of a wrapped line, disable reshaping.
        return { start };
    }
    if (ShouldTrimStartOfWrappedLine(text_spacing_trim_) && Character::MaybeHanKerningOpen(GetText()[start])) [[unlikely]] {
...
unsigned ShapingLineBreaker::Hyphenate(unsigned offset, unsigned word_start, unsigned word_end, bool backwards) const
{
    DCHECK(hyphenation_);
    ...
    if (backwards) {
        if (word_offset < hyphenation_->MinPrefixLength())
            return 0;
        unsigned prefix_length = hyphenation_->LastHyphenLocation(word, word_offset + 1);
        DCHECK(!prefix_length || prefix_length <= word_offset);
        return prefix_length;
    } else {
        if (word_len - word_offset < hyphenation_->MinSuffixLength())
            return 0;
        unsigned prefix_length = hyphenation_->FirstHyphenLocation(word, word_offset ? word_offset - 1 : 0);
        DCHECK(!prefix_length || prefix_length >= word_offset);
        return prefix_length;
    }
}

ShapingLineBreaker::BreakOpportunity ShapingLineBreaker::Hyphenate(unsigned offset, unsigned start, bool backwards) const
{
    const String& text = GetText();
    ...
    if (word_end == offset) {
        DCHECK(IsBreakableSpace(text[offset]) || offset == break_iterator_->PreviousBreakOpportunity(offset, start));
        return { word_end, false };
    }
    unsigned previous_break_opportunity = break_iterator_->PreviousBreakOpportunity(offset, start);
    ...
    while (word_start < text.length() && LazyLineBreakIterator::IsBreakableSpace(text[word_start]))
        word_start++;
    if (offset >= word_start && ShouldHyphenate(text, previous_break_opportunity, word_end, start)) {
...
ShapingLineBreaker::BreakOpportunity ShapingLineBreaker::PreviousBreakOpportunity(unsigned offset, unsigned start) const
{
    ...
    const String& text = GetText();
    unsigned break_offset = break_iterator_->PreviousBreakOpportunity(offset, start);
...
ShapingLineBreaker::BreakOpportunity ShapingLineBreaker::NextBreakOpportunity(unsigned offset, unsigned start, unsigned len) const
{
    if (hyphenation_) [[unlikely]] {
...
inline void ShapingLineBreaker::SetBreakOffset(unsigned break_offset, const String& text, Result* result)
{
    result->break_offset = break_offset;
    result->is_hyphenated = text[result->break_offset - 1] == kSoftHyphenCharacter;
}

inline void ShapingLineBreaker::SetBreakOffset(const BreakOpportunity& break_opportunity, const String& text, Result* result)
{
    result->break_offset = break_opportunity.offset;
    result->is_hyphenated = break_opportunity.is_hyphenated || text[result->break_offset - 1] == kSoftHyphenCharacter;
...
const ShapeResultView* ShapingLineBreaker::ShapeLine(unsigned start, LayoutUnit available_space, ShapingLineBreaker::Result* result_out)
{
    ...
    const String& text = GetText();
    const bool is_break_after_any_space = break_iterator_->BreakSpace() == BreakSpaceType::kAfterEverySpace;

    // The start position in the original shape results.
    const LayoutUnit start_position = result_->CachedPositionForOffset(start - range_start);
    ...
    if (first_safe.offset != start) [[unlikely]] {
        const LayoutUnit first_safe_position = result_->CachedPositionForOffset(first_safe.offset - range_start);
        line_start_result = Shape(start, first_safe.offset, { .is_line_start = true, .han_kerning_start = first_safe.han_kerning });
        // Adjust the available space to take the reshaping into account.
        const LayoutUnit old_width = FlipRtl(first_safe_position - start_position, direction);
    ...
    const LayoutUnit end_position = start_position + FlipRtl(available_space, direction);
    DCHECK_GE(FlipRtl(end_position - start_position, direction), LayoutUnit(0));
    unsigned candidate_break = result_->CachedOffsetForPosition(end_position) + range_start;
    if (candidate_break < range_end && result_->HasAutoSpacingAfter(candidate_break)) [[unlikely]] {
        // If there's an auto-space after the `candidate_break`, check if it can fit
        // without the auto-space.
        candidate_break = result_->AdjustOffsetForAutoSpacing(TextAutoSpace::GetSpacingWidth(font_), candidate_break, end_position);
    }
    ...
    if (candidate_break < range_end && ShouldTrimEnd(text_spacing_trim_) && Character::MaybeHanKerningClose(text[candidate_break])) [[unlikely]] {
        ...
        if (break_iterator_->IsBreakable(adjusted_candidate_break)) {
            ...
            line_end_result = Shape(last_safe, adjusted_candidate_break, { .han_kerning_end = true });
            const LayoutUnit last_safe_position = result_->CachedPositionForOffset(last_safe - range_start);
            const LayoutUnit width_to_last_safe = FlipRtl(last_safe_position - start_position, direction);
    ...
    if (candidate_break >= range_end) {
        ...
        return ShapeToEnd(start, line_start_result, first_safe.offset, range_start, range_end);
    }
    ...
    const bool use_previous_break_opportunity = !IsBreakableSpace(text[candidate_break]) || is_break_after_any_space;
    if (use_previous_break_opportunity) {
        ...
        if (result_out->is_overflow) {
            ...
            break_opportunity = NextBreakOpportunity(std::max(candidate_break, start + 1), start, range_end);
        }
    } else {
        break_opportunity = NextBreakOpportunity(std::max(candidate_break, start + 1), start, range_end);
        ...
        if (break_opportunity.offset > candidate_break
            && (!break_opportunity.non_hangable_run_end || *break_opportunity.non_hangable_run_end > candidate_break)) {
            BreakOpportunity previous_opportunity = PreviousBreakOpportunity(candidate_break, start);
        ...
        if (break_opportunity.non_hangable_run_end && break_opportunity.non_hangable_run_end <= start) {
    ...
    if (break_opportunity.offset >= range_end) {
        ...
        if (break_opportunity.non_hangable_run_end && range_end < break_opportunity.non_hangable_run_end) {
            break_opportunity.non_hangable_run_end = std::nullopt;
        }
        if (IsBreakableSpace(text[range_end - 1])) {
            break_opportunity.non_hangable_run_end = FindNonHangableEnd(text, range_end - 1);
        }
    }
    ...
    if (first_safe.offset >= break_opportunity.offset) [[unlikely]] {
        ...
        return ShapeResultView::Create(Shape(start, break_opportunity.offset, { .is_line_start = true, .han_kerning_start = first_safe.han_kerning }));
    }
    DCHECK_GE(first_safe.offset, start);
    ...
    if (reshape_line_end) {
        ...
        while (true) {
            ...
            last_safe = result_->CachedPreviousSafeToBreakOffset(break_opportunity.offset);
            ...
            const LayoutUnit safe_position = result_->CachedPositionForOffset(last_safe - range_start);
            line_end_result = Shape(last_safe, break_opportunity.offset);
            if (line_end_result->Width() <= FlipRtl(end_position - safe_position, direction))
                break;

            // Doesn't fit after the reshape. Try the previous break opportunity.
            line_end_result = nullptr;
            break_opportunity = PreviousBreakOpportunity(break_opportunity.offset - 1, start);
    ...
    if (!line_end_result) {
        ...
        if (result_->HasAutoSpacingBefore(last_safe)) [[unlikely]] {
            ...
            line_end_result = result_->UnapplyAutoSpacing(TextAutoSpace::GetSpacingWidth(font_), last_safe, break_opportunity.offset);
        }
    }
    ...
    DCHECK_EQ(break_opportunity.offset - start,
        (line_start_result ? line_start_result->NumCharacters() : 0) + (last_safe > first_safe.offset ? last_safe - first_safe.offset : 0)
    ...
    return ConcatShapeResults(start, break_opportunity.offset, first_safe.offset, last_safe, line_start_result, line_end_result);
}
const ShapeResultView* ShapingLineBreaker::ConcatShapeResults(
    unsigned start, unsigned end, unsigned first_safe, unsigned last_safe, const ShapeResult* line_start_result, const ShapeResult* line_end_result)
{
...
const ShapeResultView* ShapingLineBreaker::ShapeToEnd(
    unsigned start, const ShapeResult* line_start_result, unsigned first_safe, unsigned range_start, unsigned range_end)
{
    DCHECK(result_);
    ...
    ShapeResultView::Segment segments[2] = { { line_start_result, 0, std::numeric_limits<unsigned>::max() }, { result_, first_safe, range_end } };
    return ShapeResultView::Create(segments);
}

const ShapeResultView* ShapingLineBreaker::ShapeLineAt(unsigned start, unsigned end)
{
    DCHECK_GT(end, start);
    ...
    if (first_safe.offset != start) {
        const ShapeOptions options { .is_line_start = true, .han_kerning_start = first_safe.han_kerning };
    ...
    return ConcatShapeResults(start, end, first_safe.offset, last_safe, line_start_result, line_end_result);
}
