...
// ShapeResult.
template <typename TextContainerType> class PLATFORM_EXPORT ShapeResultSpacing final {
    STACK_ALLOCATED();

public:
    explicit ShapeResultSpacing(const TextContainerType& text, bool allow_word_spacing_anywhere = false)
        : text_(text)
        , allow_word_spacing_anywhere_(allow_word_spacing_anywhere)
    {
    }

    const TextContainerType& Text() const
    {
        return text_;
    }
    ...
    bool SetSpacing(TextRunLayoutUnit letter_spacing, TextRunLayoutUnit word_spacing);

    // Set the expansion for the justification.
    void SetExpansion(InlineLayoutUnit expansion, TextDirection, bool allows_leading_expansion = false, bool allows_trailing_expansion = false);
    ...
    TextRunLayoutUnit ComputeSpacing(const ComputeSpacingParameters& parameters, float& offset);

private:
    bool IsAfterExpansion() const
    {
        return is_after_expansion_;
    }

    void ComputeExpansion(bool allows_leading_expansion, bool allows_trailing_expansion, TextDirection);
...
template <> void ShapeResultSpacing<TextRun>::SetSpacingAndExpansion(const FontDescription&);
} // namespace blink
