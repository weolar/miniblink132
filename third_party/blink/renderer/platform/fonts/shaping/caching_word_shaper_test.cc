...
TEST_F(CachingWordShaperTest, LatinLeftToRightByWord)
{
    ...
    ASSERT_TRUE(TestInfo(result)->RunInfoForTesting(0, start_index, num_glyphs, script));
...
TEST_F(CachingWordShaperTest, SegmentEmojiSequences)
{
    ...
    std::vector<std::string> test_strings = { // A family followed by a couple with heart emoji sequence,
    ...
    for (auto test_string : test_strings) {
        ...
        ASSERT_TRUE(iterator.Next(&word_result));
        EXPECT_EQ(emoji_string.length(), word_result->NumCharacters()) << " Length mismatch for sequence: " << test_string;
...
TEST_F(CachingWordShaperTest, SegmentEmojiExtraZWJPrefix)
{
    ...
    const UChar kStr[] = { 0x200D, 0xD83D, 0xDC68, 0x200D, 0xD83D, 0xDC69, 0x200D, 0xD83D, 0xDC67, 0x200D, 0xD83D, 0xDC66, 0xD83D, 0xDC69, 0x200D, 0x2764,
        0xFE0F, 0x200D, 0xD83D, 0xDC8B, 0x200D, 0xD83D, 0xDC68, 0x0 };
...
TEST_F(CachingWordShaperTest, SegmentEmojiSubdivisionFlags)
{
    ...
    const UChar kStr[] = { 0xD83C, 0xDFF4, 0xDB40, 0xDC67, 0xDB40, 0xDC62, 0xDB40, 0xDC77, 0xDB40, 0xDC6C, 0xDB40, 0xDC73, 0xDB40, 0xDC7F, 0xD83C, 0xDFF4,
        0xDB40, 0xDC67, 0xDB40, 0xDC62, 0xDB40, 0xDC73, 0xDB40, 0xDC63, 0xDB40, 0xDC74, 0xDB40, 0xDC7F, 0xD83C, 0xDFF4, 0xDB40, 0xDC67, 0xDB40, 0xDC62, 0xDB40,
...
TEST_F(CachingWordShaperTest, GlyphBoundsWithSpaces)
{
    ...
    TextRun periods_and_spaces(reinterpret_cast<const LChar*>(". . . . . . . . . ."), 19);
    gfx::RectF periods_and_spaces_glyph_bounds;
    float periods_and_spaces_width = shaper.Width(periods_and_spaces, &periods_and_spaces_glyph_bounds);
