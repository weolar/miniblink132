...
void FontFeatures::Initialize(const FontDescription& description)
{
...

    {
        ...
        if (letter_spacing || (common == disabled || (common == normal && default_is_off))) {
            Append(no_liga);
            Append(no_clig);
        }
        ...
        if (letter_spacing || (contextual == disabled || (contextual == normal && default_is_off))) {
            Append(no_calt);
        }
    }
    ...
    if (description.VariantNumeric().NumericFigureValue() == FontVariantNumeric::kLiningNums)
        Append(lnum);

    static constexpr hb_feature_t onum = CreateFeature('o', 'n', 'u', 'm', 1);
    if (description.VariantNumeric().NumericFigureValue() == FontVariantNumeric::kOldstyleNums)
        Append(onum);

    static constexpr hb_feature_t pnum = CreateFeature('p', 'n', 'u', 'm', 1);
    if (description.VariantNumeric().NumericSpacingValue() == FontVariantNumeric::kProportionalNums)
        Append(pnum);
    static constexpr hb_feature_t tnum = CreateFeature('t', 'n', 'u', 'm', 1);
    if (description.VariantNumeric().NumericSpacingValue() == FontVariantNumeric::kTabularNums)
        Append(tnum);

    static constexpr hb_feature_t afrc = CreateFeature('a', 'f', 'r', 'c', 1);
    if (description.VariantNumeric().NumericFractionValue() == FontVariantNumeric::kStackedFractions)
        Append(afrc);
    static constexpr hb_feature_t frac = CreateFeature('f', 'r', 'a', 'c', 1);
    if (description.VariantNumeric().NumericFractionValue() == FontVariantNumeric::kDiagonalFractions)
        Append(frac);

    static constexpr hb_feature_t ordn = CreateFeature('o', 'r', 'd', 'n', 1);
    if (description.VariantNumeric().OrdinalValue() == FontVariantNumeric::kOrdinalOn)
        Append(ordn);

    static constexpr hb_feature_t zero = CreateFeature('z', 'e', 'r', 'o', 1);
    if (description.VariantNumeric().SlashedZeroValue() == FontVariantNumeric::kSlashedZeroOn)
        Append(zero);

    const hb_tag_t chws_or_vchw = is_horizontal ? HB_TAG('c', 'h', 'w', 's') : HB_TAG('v', 'c', 'h', 'w');
    bool default_enable_chws = ShouldTrimAdjacent(description.GetTextSpacingTrim());
    ...
    if (settings) [[unlikely]] {
        ...
        const hb_tag_t halt_or_vhal = is_horizontal ? HB_TAG('h', 'a', 'l', 't') : HB_TAG('v', 'h', 'a', 'l');
        const hb_tag_t palt_or_vpal = is_horizontal ? HB_TAG('p', 'a', 'l', 't') : HB_TAG('v', 'p', 'a', 'l');
        for (const FontFeature& setting : *settings) {
            ...
            if (default_enable_chws && (feature.tag == chws_or_vchw || (feature.value && (feature.tag == halt_or_vhal || feature.tag == palt_or_vpal))))
    ...
    const FontDescription::FontVariantPosition variant_position = description.VariantPosition();
