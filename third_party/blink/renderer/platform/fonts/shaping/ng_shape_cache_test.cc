...
protected:
    void SetUp() override
    {
        cache = MakeGarbageCollected<NGShapeCache>();
    }
...
TEST_F(NGShapeCacheTest, AddEntriesAndCacheHits)
{
    ...
    const auto* entry_A_LTR = cache->GetOrCreate("A", TextDirection::kLtr, ShapeResultFunc);
    ASSERT_TRUE(entry_A_LTR);

    // Adding the same entry again hits cache.
    EXPECT_EQ(cache->GetOrCreate("A", TextDirection::kLtr, ShapeResultFunc), entry_A_LTR);

    // Adding the an entry with different text does not hit cache.
    const auto* entry_B_LTR = cache->GetOrCreate("B", TextDirection::kLtr, ShapeResultFunc);
    ASSERT_TRUE(entry_B_LTR);
    EXPECT_NE(entry_B_LTR, entry_A_LTR);

    // Adding the same entry again hits cache.
    EXPECT_EQ(cache->GetOrCreate("B", TextDirection::kLtr, ShapeResultFunc), entry_B_LTR);

    // Adding the an entry with different direction does not hit cache.
    const auto* entry_A_RTL = cache->GetOrCreate("A", TextDirection::kRtl, ShapeResultFunc);
    ASSERT_TRUE(entry_A_RTL);
    EXPECT_NE(entry_A_RTL, entry_A_LTR);
    EXPECT_NE(entry_A_RTL, entry_B_LTR);

    // Adding the same entry again hits cache.
    EXPECT_EQ(cache->GetOrCreate("A", TextDirection::kRtl, ShapeResultFunc), entry_A_RTL);
}
