...
struct TypeData {
    ...
    { ' ', TextAutoSpace::kOther }, { '0', TextAutoSpace::kLetterOrNumeral }, { 'A', TextAutoSpace::kLetterOrNumeral },
...
class TextAutoSpaceTypeTest : public testing::Test, public testing::WithParamInterface<TypeData> { };

INSTANTIATE_TEST_SUITE_P(TextAutoSpaceTest, TextAutoSpaceTypeTest, testing::ValuesIn(g_type_data));
...
TEST_F(TextAutoSpaceTest, Unapply)
{
    ...
    EXPECT_THAT(GetAdvances(*result), ElementsAre(size, with_spacing, size, size, with_spacing));

    // Compute the line-end by unapplying the spacing.
    for (unsigned end_offset : { 2u, 5u }) {
        const ShapeResult* line_end = result->UnapplyAutoSpacing(spacing, end_offset - 1, end_offset);
