...
public:
    ...
    const hb_feature_t& operator[](wtf_size_t i) const
    {
        return features_[i];
    }
    const hb_feature_t* data() const
    {
    ...
    {
        features_.reserve(new_capacity);
    }

    void Append(const hb_feature_t& feature)
    {
        features_.push_back(feature);
    }
    void Insert(const hb_feature_t& feature)
    {
