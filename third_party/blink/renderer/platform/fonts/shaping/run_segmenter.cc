...
RunSegmenter::RunSegmenter(base::span<const UChar> buffer, FontOrientation run_orientation)
...
void RunSegmenter::ConsumeIteratorPastLastSplit(Iterator& iterator, unsigned* iterator_position, SegmentationCategory* segmentation_category)
{
...
bool RunSegmenter::Consume(RunSegmenterRange* next_range)
{
    ...
    ConsumeIteratorPastLastSplit(script_run_iterator_, &script_run_iterator_position_, &candidate_range_.script);
    ...
    if (orientation_iterator_) [[unlikely]] {
        ConsumeIteratorPastLastSplit(*orientation_iterator_, &orientation_iterator_position_, &candidate_range_.render_orientation);
        unsigned positions[] = { script_run_iterator_position_, symbols_iterator_position_, orientation_iterator_position_ };
        last_split_ = *base::ranges::min_element(positions);
    } else {
        last_split_ = std::min(script_run_iterator_position_, symbols_iterator_position_);
    }
