...
hb_codepoint_t GetGlyphForVariationSequenceFromFont(Font font, UChar32 character, UChar32 variation_selector)
{
    const FontPlatformData& font_without_char_platform_data = font.PrimaryFont()->PlatformData();
    HarfBuzzFace* face_without_char = font_without_char_platform_data.GetHarfBuzzFace();
    EXPECT_TRUE(face_without_char);
    return face_without_char->HarfBuzzGetGlyphForTesting(character, variation_selector);
}

hb_codepoint_t GetGlyphForEmojiVSFromFontWithVS15(UChar32 character, UChar32 variation_selector)
{
    Font font = test::CreateTestFont(AtomicString("Noto Emoji"), WPTFontPath("NotoEmoji-Regular_subset.ttf"), 11);
    return GetGlyphForVariationSequenceFromFont(font, character, variation_selector);
}

hb_codepoint_t GetGlyphForEmojiVSFromFontWithVS16(UChar32 character, UChar32 variation_selector)
{
    Font font = test::CreateTestFont(AtomicString("Noto Color Emoji"), WPTFontPath("NotoColorEmoji-Regular_subset.ttf"), 11);
    return GetGlyphForVariationSequenceFromFont(font, character, variation_selector);
}

hb_codepoint_t GetGlyphForEmojiVSFromFontWithBaseCharOnly(UChar32 character, UChar32 variation_selector)
{
    Font font = test::CreateTestFont(AtomicString("Noto Emoji Without VS"), WPTFontPath("NotoEmoji-Regular_without-cmap14-subset.ttf"), 11);
    return GetGlyphForVariationSequenceFromFont(font, character, variation_selector);
}
...
hb_codepoint_t GetGlyphForStandardizedVSFromFontWithBaseCharOnly()
{
    ...
    Font font
        = test::CreateTestFont(AtomicString("Noto Sans Mongolian"), blink::test::BlinkWebTestsFontsTestDataPath("noto/NotoSansMongolian-regular.woff2"), 11);
    return GetGlyphForVariationSequenceFromFont(font, character, variation_selector);
}
...
hb_codepoint_t GetGlyphForCJKVSFromFontWithVS()
{
    ...
    Font font = test::CreateTestFont(
        AtomicString("Noto Sans CJK JP"), blink::test::BlinkWebTestsFontsTestDataPath("noto/cjk/NotoSansCJKjp-Regular-subset-chws.otf"), 11);
    return GetGlyphForVariationSequenceFromFont(font, character, variation_selector);
}
...
TEST(HarfBuzzFaceTest, HarfBuzzGetNominalGlyph_TestFontWithBaseCharOnly)
{
    ...
    EXPECT_EQ(GetGlyphForStandardizedVSFromFontWithBaseCharOnly(), kUnmatchedVSGlyphId);
}

TEST(HarfBuzzFaceTest, HarfBuzzGetNominalGlyph_TestFontWithBaseCharOnly_IgnoreVS)
{
    ScopedFontVariationSequencesForTest scoped_feature(true);
    HarfBuzzFace::SetVariationSelectorMode(kIgnoreVariationSelector);
...
TEST(HarfBuzzFaceTest, HarfBuzzGetNominalGlyph_TestFontWithBaseCharOnly_VSFlagOff)
{
    ScopedFontVariationSequencesForTest scoped_feature(false);
    HarfBuzzFace::SetVariationSelectorMode(kUseSpecifiedVariationSelector);
...
TEST(HarfBuzzFaceTest, HarfBuzzGetNominalGlyph_TestVariantEmojiEmoji)
{
    ...
    hb_codepoint_t glyph_from_font_with_vs15 = GetGlyphForEmojiVSFromFontWithVS15(character, variation_selector);
    EXPECT_EQ(glyph_from_font_with_vs15, kUnmatchedVSGlyphId);

    hb_codepoint_t glyph_from_font_with_vs16 = GetGlyphForEmojiVSFromFontWithVS16(character, variation_selector);
    ...
    if (!RuntimeEnabledFeatures::SystemFallbackEmojiVSSupportEnabled()) {
        hb_codepoint_t glyph_from_font_without_vs = GetGlyphForEmojiVSFromFontWithBaseCharOnly(character, variation_selector);
...
TEST(HarfBuzzFaceTest, HarfBuzzGetNominalGlyph_TestSystemFallbackEmojiVS)
{
    ScopedFontVariationSequencesForTest scoped_variation_sequences_feature(true);
    ScopedFontVariantEmojiForTest scoped_variant_emoji_feature(true);
    ScopedSystemFallbackEmojiVSSupportForTest scoped_system_emoji_vs_feature(true);

    HarfBuzzFace::SetVariationSelectorMode(kUseSpecifiedVariationSelector);

    UChar32 character = kShakingFaceEmoji;

    hb_codepoint_t glyph_from_font_with_vs15 = GetGlyphForEmojiVSFromFontWithVS15(character, kVariationSelector15Character);
    EXPECT_TRUE(glyph_from_font_with_vs15);
    EXPECT_NE(glyph_from_font_with_vs15, kUnmatchedVSGlyphId);

    hb_codepoint_t glyph_from_font_with_vs16 = GetGlyphForEmojiVSFromFontWithVS16(character, kVariationSelector16Character);
    EXPECT_TRUE(glyph_from_font_with_vs16);
    EXPECT_NE(glyph_from_font_with_vs16, kUnmatchedVSGlyphId);

    hb_codepoint_t glyph_from_font_without_vs = GetGlyphForEmojiVSFromFontWithBaseCharOnly(character, 0);
