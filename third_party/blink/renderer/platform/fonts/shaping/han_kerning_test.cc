...
Font CreateNotoCjk()
{
    return blink::test::CreateTestFont(
        AtomicString("Noto Sans CJK"), blink::test::BlinkWebTestsFontsTestDataPath("noto/cjk/NotoSansCJKjp-Regular-subset-halt.otf"), 16.0);
}
...
TEST_F(HanKerningTest, MayApply)
{
    ...
    scoped_refptr<LayoutLocale> ja = LayoutLocale::CreateForTesting(AtomicString("ja"));
    ...
    for (UChar32 ch = 0; ch < kMaxCodepoint; ++ch) {
        ...
        for (wtf_size_t i = 0; i < text.length(); ++i) {
            const HanKerning::CharType type = HanKerning::GetCharType(text[i], ja_data);
            if (type == HanKerning::CharType::kOpen || type == HanKerning::CharType::kOpenQuote || type == HanKerning::CharType::kClose
                || type == HanKerning::CharType::kCloseQuote) {
                EXPECT_EQ(HanKerning::MayApply(text), true) << String::Format("U+%06X", ch);
                break;
            }
        }
    }
...
TEST_F(HanKerningTest, FontDataHorizontal)
{
    ...
    EXPECT_TRUE(noto_cjk_data);
    scoped_refptr<LayoutLocale> ja = LayoutLocale::CreateForTesting(AtomicString("ja"));
    scoped_refptr<LayoutLocale> zhs = LayoutLocale::CreateForTesting(AtomicString("zh-hans"));
    scoped_refptr<LayoutLocale> zht = LayoutLocale::CreateForTesting(AtomicString("zh-hant"));
    HanKerning::FontData ja_data(*noto_cjk_data, *ja, true);
...
TEST_F(HanKerningTest, FontDataSizeError)
{
    ...
    public:
        EnableAntialiasedText()
            : is_antialiased_text_enabled_(FontCache::Get().AntialiasedTextEnabled())
        {
    ...
    font_description.SetFamily(FontFamily(AtomicString("Yu Gothic"), FontFamily::Type::kFamilyName));
...
TEST_F(HanKerningTest, ResetFeatures)
{
    ...
    features.Append({ HB_TAG('T', 'E', 'S', 'T'), 1, 0, static_cast<unsigned>(-1) });
    EXPECT_EQ(features.size(), 1u);
    const String text(u"国）（国");
    {
        HanKerning han_kerning(text, 0, text.length(), *noto_cjk_data, noto_cjk.GetFontDescription(), HanKerning::Options(), &features);
