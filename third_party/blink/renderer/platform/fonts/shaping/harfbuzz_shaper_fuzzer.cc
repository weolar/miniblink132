...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    String string(UNSAFE_BUFFERS(base::span(reinterpret_cast<const UChar*>(data), std::min(kMaxInputLength, size / sizeof(UChar)))));
    ...
    for (unsigned from = 0; from < text_run.length(); from += word_length) {
        ...
        ShapeResultBloberizer::FillGlyphs bloberizer(font.GetFontDescription(), subrun_info, buffer, ShapeResultBloberizer::Type::kEmitText);
        bloberizer.Blobs();
    }
