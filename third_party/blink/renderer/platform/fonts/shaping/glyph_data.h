...
struct HarfBuzzRunGlyphData {
    ...
    HarfBuzzRunGlyphData(unsigned glyph, unsigned character_index, bool safe_to_break_before, TextRunLayoutUnit advance)
        : glyph(glyph)
    ...
    void SetAdvance(TextRunLayoutUnit value)
    {
        advance = value;
    }
    void AddAdvance(TextRunLayoutUnit value)
    {
        advance += value;
    }
...
struct GlyphDataRange {
    GlyphDataRange FindGlyphDataRange(bool is_rtl, unsigned start_character_index, unsigned end_character_index) const;
    unsigned size() const
    {
...
inline GlyphDataRange GlyphDataRange::FindGlyphDataRange(bool is_rtl, unsigned start_character_index, unsigned end_character_index) const
{
    const auto comparer = [](const HarfBuzzRunGlyphData& glyph_data, unsigned index) { return glyph_data.character_index < index; };
    if (!is_rtl) {
        const HarfBuzzRunGlyphData* start_glyph = std::lower_bound(begin, end, start_character_index, comparer);
        if (start_glyph == end) [[unlikely]] {
            return GlyphDataRange();
        }
        const HarfBuzzRunGlyphData* end_glyph = std::lower_bound(start_glyph, end, end_character_index, comparer);
        if (offsets) {
    ...
    const auto start_glyph_it = std::lower_bound(rbegin, rend, start_character_index, comparer);
    if (start_glyph_it == rend) [[unlikely]] {
        return GlyphDataRange();
    }
    const auto end_glyph_it = std::lower_bound(start_glyph_it, rend, end_character_index, comparer);
...
public:
    explicit GlyphOffsetArray(unsigned size)
        : size_(size)
    {
    }
        ...
        : size_(other.size_)
    {
        ...
        std::copy(other.storage_.get(), other.storage_.get() + other.size(), AllocateStorage());
    }

    // A return value of |GetOffsets()| to represent optional |GlyphOffset|
    // array.
    template <bool has_non_zero_glyph_offsets> struct iterator final { };

    template <bool has_non_zero_glyph_offsets> iterator<has_non_zero_glyph_offsets> GetIterator() const
    {
        return iterator<has_non_zero_glyph_offsets>(*this);
    }

    template <bool has_non_zero_glyph_offsets> iterator<has_non_zero_glyph_offsets> GetIteratorForRange(const GlyphDataRange& range) const
    {
    ...
    void CopyFrom(const GlyphOffsetArray& other1, const GlyphOffsetArray& other2)
    {
        ...
        if (other1.storage_) {
            ...
            std::copy(other1.storage_.get(), other1.storage_.get() + other1.size(), storage_.get());
        }
        if (other2.storage_) {
            if (!storage_) {
                AllocateStorage();
            }
            std::copy(other2.storage_.get(), other2.storage_.get() + other2.size(), storage_.get() + other1.size());
        }
    }
...
template <> struct GlyphOffsetArray::iterator<true> final {
...
template <> struct GlyphOffsetArray::iterator<false> final {
    explicit iterator(const GlyphOffsetArray& array)
    {
