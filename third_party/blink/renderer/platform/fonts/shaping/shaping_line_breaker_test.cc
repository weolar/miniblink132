...
public:
    HarfBuzzShapingLineBreaker(
        const HarfBuzzShaper* shaper, const Font* font, const ShapeResult* result, const LazyLineBreakIterator* break_iterator, const Hyphenation* hyphenation)
...
const ShapeResultView* ShapeLine(ShapingLineBreaker* breaker, unsigned start_offset, LayoutUnit available_space, unsigned* break_offset)
{
    ShapingLineBreaker::Result result;
    const ShapeResultView* shape_result = breaker->ShapeLine(start_offset, available_space, &result);
...
protected:
    ...
    {
        font_description.SetFamily(FontFamily(AtomicString("Lucida Grande"), FontFamily::Type::kFamilyName,
            SharedFontFamily::Create(AtomicString("Lucida Medium"), FontFamily::Type::kFamilyName)));
    }
    ...
    Vector<unsigned> BreakPositionsByNext(const ShapingLineBreaker& breaker, const String& string)
    {
        Vector<unsigned> break_positions;
        for (unsigned i = 0; i <= string.length(); i++) {
            unsigned next = breaker.NextBreakOpportunity(i, 0, string.length()).offset;
    ...
    Vector<unsigned> BreakPositionsByPrevious(const ShapingLineBreaker& breaker, const String& string)
    {
        Vector<unsigned> break_positions;
        for (unsigned i = string.length(); i; i--) {
            unsigned previous = breaker.PreviousBreakOpportunity(i, 0).offset;
            if (previous && (break_positions.empty() || break_positions.back() != previous))
...
TEST_F(ShapingLineBreakerTest, ShapeLineLatin)
{
    ...
    String string = To16Bit("Test run with multiple words and breaking "
                            "opportunities.");
    LazyLineBreakIterator break_iterator(string, AtomicString("en-US"), LineBreakType::kNormal);
    ...
    HarfBuzzShapingLineBreaker breaker(&shaper, &font, result, &break_iterator, nullptr);
...
TEST_F(ShapingLineBreakerTest, ShapeLineLatinBreakAll)
{
    ...
    LazyLineBreakIterator break_iterator(string, AtomicString("en-US"), LineBreakType::kBreakAll);
    TextDirection direction = TextDirection::kLtr;
...
TEST_F(ShapingLineBreakerTest, HanKerningCloseUnsafe)
{
    ...
    const LayoutUnit available_width = LayoutUnit::FromFloatFloor(sub_result->PositionForOffset(1)) - 1;

    LazyLineBreakIterator break_iterator(string);
    HarfBuzzShapingLineBreaker breaker(&shaper, &font, sub_result, &break_iterator, nullptr);
    unsigned break_offset = 0;
...
class BreakOpportunityTest : public ShapingLineBreakerTest, public testing::WithParamInterface<BreakOpportunityTestData> { };

INSTANTIATE_TEST_SUITE_P(ShapingLineBreakerTest, BreakOpportunityTest,
    testing::Values(BreakOpportunityTestData { u"x y z", { 2, 4, 5 } }, BreakOpportunityTestData { u"y\xADz", { 2, 3 }, { 3 } },
        BreakOpportunityTestData { u"\xADz", { 1, 2 }, { 2 } }, BreakOpportunityTestData { u"y\xAD", { 2 }, { 2 } },
...
TEST_P(BreakOpportunityTest, Next)
{
    ...
    HarfBuzzShapingLineBreaker breaker(&shaper, &font, result, &break_iterator, nullptr);
    EXPECT_THAT(BreakPositionsByNext(breaker, string), testing::ElementsAreArray(data.break_positions));

    if (!data.break_positions_with_soft_hyphen_disabled.empty()) {
        break_iterator.EnableSoftHyphen(false);
        EXPECT_THAT(BreakPositionsByNext(breaker, string), testing::ElementsAreArray(data.break_positions_with_soft_hyphen_disabled));
    }
}
...
TEST_P(BreakOpportunityTest, Previous)
{
    ...
    HarfBuzzShapingLineBreaker breaker(&shaper, &font, result, &break_iterator, nullptr);
    EXPECT_THAT(BreakPositionsByPrevious(breaker, string), testing::ElementsAreArray(data.break_positions));

    if (!data.break_positions_with_soft_hyphen_disabled.empty()) {
        break_iterator.EnableSoftHyphen(false);
        EXPECT_THAT(BreakPositionsByPrevious(breaker, string), testing::ElementsAreArray(data.break_positions_with_soft_hyphen_disabled));
    }
}
