...
inline float GetAdvance(const HarfBuzzShaper::GlyphData& glyph, bool is_horizontal)
{
...
HanKerning::CharType CharTypeFromBounds(float half_em, const SkRect& bound, bool is_horizontal)
{
    if (is_horizontal) {
...
HanKerning::CharType CharTypeFromBounds(base::span<HarfBuzzShaper::GlyphData> glyphs, base::span<SkRect> bounds, unsigned index, bool is_horizontal)
{
...
HanKerning::CharType CharTypeFromBounds(base::span<HarfBuzzShaper::GlyphData> glyphs, base::span<SkRect> bounds, bool is_horizontal)
{
    ...
    for (++i; i < glyphs.size(); ++i) {
        ...
        const HanKerning::CharType type = CharTypeFromBounds(half_advance0, bounds[i], is_horizontal);
        if (type != type0) {
            return HanKerning::CharType::kOther;
        }
    }
...
void HanKerning::ResetFeatures()
{
    ...
    for (wtf_size_t i = num_features_before_; i < features_->size(); ++i) {
        ...
        DCHECK(feature.tag == HB_TAG('h', 'a', 'l', 't') || feature.tag == HB_TAG('v', 'h', 'a', 'l'));
    }
#endif
...
HanKerning::CharType HanKerning::GetCharType(UChar ch, const FontData& font_data)
{
    ...
    case CharType::kOpenQuote:
        return font_data.is_quote_fullwidth ? CharType::kOpen : CharType::kOpenNarrow;
    case CharType::kCloseQuote:
        return font_data.is_quote_fullwidth ? CharType::kClose : CharType::kCloseNarrow;
    }
    NOTREACHED();
}
...
{
    return !text.Is8Bit() && !text.IsAllSpecialCharacters<[](UChar ch) { return !Character::MaybeHanKerningOpenOrCloseFast(ch); }>();
}

inline bool HanKerning::ShouldKern(CharType type, CharType last_type)
{
    return type == CharType::kOpen
        && (last_type == CharType::kOpen || last_type == CharType::kMiddle || last_type == CharType::kClose || last_type == CharType::kOpenNarrow);
}

inline bool HanKerning::ShouldKernLast(CharType type, CharType last_type)
{
    return last_type == CharType::kClose && (type == CharType::kClose || type == CharType::kMiddle || type == CharType::kCloseNarrow);
}
...
void HanKerning::Compute(const String& text, wtf_size_t start, wtf_size_t end, const SimpleFontData& font, const FontDescription& font_description,
    Options options, FontFeatures* features)
{
    DCHECK(!features_);
    DCHECK_GT(end, start);
    ...
    const FontData& font_data = font.HanKerningData(locale, options.is_horizontal);
    if (!font_data.has_alternate_spacing) {
        return;
    }
    if (font_description.GetTextSpacingTrim() == TextSpacingTrim::kSpaceAll) [[unlikely]] {
        return;
    }
    ...
    const hb_tag_t tag = options.is_horizontal ? HB_TAG('h', 'a', 'l', 't') : HB_TAG('v', 'h', 'a', 'l');
    features_ = features;
...
HanKerning::FontData::FontData(const SimpleFontData& font, const LayoutLocale& locale, bool is_horizontal)
{
    // Check if the font has `halt` (or `vhal` in vertical.)
    OpenTypeFeatures features(font);
    const hb_tag_t alt_tag = is_horizontal ? HB_TAG('h', 'a', 'l', 't') : HB_TAG('v', 'h', 'a', 'l');
    ...
    const hb_tag_t chws_tag = is_horizontal ? HB_TAG('c', 'h', 'w', 's') : HB_TAG('v', 'c', 'h', 'w');
    ...
    const UChar kChars[] = { // Dot (full stop and comma) characters.
        // https://drafts.csswg.org/css-text-4/#fullwidth-dot-punctuation
        kIdeographicCommaCharacter, kIdeographicFullStopCharacter, kFullwidthComma, kFullwidthFullStop,
        // Colon characters.
        ...
        kLeftDoubleQuotationMarkCharacter, kLeftSingleQuotationMarkCharacter, kRightDoubleQuotationMarkCharacter, kRightSingleQuotationMarkCharacter
    };
    ...
    shaper.GetGlyphData(font, locale, locale.GetScriptForHan(), is_horizontal, glyph_data_list);
    ...
    type_for_dot = CharTypeFromBounds(glyph_data_span.first(kDotSize), bounds_span.first(kDotSize), is_horizontal);
    type_for_colon = CharTypeFromBounds(glyph_data_span, bounds_span, kColonIndex, is_horizontal);
    type_for_semicolon = CharTypeFromBounds(glyph_data_span, bounds_span, kSemicolonIndex, is_horizontal);
    ...
    if (CharTypeFromBounds(glyph_data_span.first(2u), bounds_span.first(2u), is_horizontal) != CharType::kOpen
        || CharTypeFromBounds(glyph_data_span.subspan(2u), bounds_span.subspan(2u), is_horizontal) != CharType::kClose) {
