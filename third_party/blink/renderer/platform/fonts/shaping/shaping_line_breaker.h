...
public:
    ...
    ShapingLineBreaker(const ShapeResult* result, const LazyLineBreakIterator* break_iterator, const Hyphenation* hyphenation, const Font* font);
    ...
    const ShapeResultView* ShapeLine(unsigned start_offset, LayoutUnit available_space, Result* result_out);
...
protected:
    ...
    virtual const ShapeResult* Shape(unsigned start, unsigned end, ShapeOptions = ShapeOptions()) = 0;
...
private:
    ...
    BreakOpportunity PreviousBreakOpportunity(unsigned offset, unsigned start) const;
    BreakOpportunity NextBreakOpportunity(unsigned offset, unsigned start, unsigned len) const;
    BreakOpportunity Hyphenate(unsigned offset, unsigned start, bool backwards) const;
    unsigned Hyphenate(unsigned offset, unsigned word_start, unsigned word_end, bool backwards) const;

    const ShapeResultView* ShapeToEnd(unsigned start, const ShapeResult* line_start_result, unsigned first_safe, unsigned range_start, unsigned range_end);
