...
TEST_F(StretchyOperatorShaperTest, GlyphVariants)
{
    ...
    StretchyOperatorShaper vertical_shaper(kVerticalArrow, OpenTypeMathStretchData::StretchAxis::Vertical);
    StretchyOperatorShaper horizontal_shaper(kHorizontalArrow, OpenTypeMathStretchData::StretchAxis::Horizontal);

    auto vertical_arrow = math.PrimaryFont()->GlyphForCharacter(kVerticalArrow);
    auto horizontal_arrow = math.PrimaryFont()->GlyphForCharacter(kHorizontalArrow);
    ...
    for (unsigned i = 0; i < size_count; i++) {
        for (unsigned j = 1; j <= subdivision; j++) {
            ...
            // Shaping of horizontal size variants.
            {
                ...
                EXPECT_NEAR(TestInfo(result)->AdvanceForTesting(0, 0), (i + 1) * 1000, kSizeError);
            }
    ...
    //
    for (unsigned repetition_count = 1; repetition_count <= 5; repetition_count++) {
        ...
        // From left to right: h2, h1, h1, h1, ...
        {
            ...
            EXPECT_EQ(TestInfo(result)->NumberOfRunsForTesting(), 1u);
            EXPECT_EQ(TestInfo(result)->RunInfoForTesting(0).NumGlyphs(), repetition_count + 1);
            EXPECT_EQ(TestInfo(result)->GlyphForTesting(0, 0), h[2]);
            EXPECT_NEAR(TestInfo(result)->AdvanceForTesting(0, 0), 3000 - overlap, kSizeError);
            for (unsigned i = 0; i < repetition_count - 1; i++) {
                EXPECT_EQ(TestInfo(result)->GlyphForTesting(0, i + 1), h[1]);
                EXPECT_NEAR(TestInfo(result)->AdvanceForTesting(0, i + 1), 2000 - overlap, kSizeError);
            }
            EXPECT_EQ(TestInfo(result)->GlyphForTesting(0, repetition_count), h[1]);
            EXPECT_NEAR(TestInfo(result)->AdvanceForTesting(0, repetition_count), 2000, kSizeError);
        }
        ...
        // From bottom to top: v2, v1, v1, v1, ...
        {
            ...
            for (unsigned i = 0; i < repetition_count; i++) {
                ...
                EXPECT_NEAR(TestInfo(result)->AdvanceForTesting(0, i), 2000 - overlap, kSizeError);
            }
            EXPECT_EQ(TestInfo(result)->GlyphForTesting(0, repetition_count), v[2]);
            EXPECT_NEAR(TestInfo(result)->AdvanceForTesting(0, repetition_count), 3000, kSizeError);
        }
    }
    ...
    // These tests verify that it does not cause any assertion or crashes.
    {
        ...
        // more than the max number of glyphs.
        target_size = static_cast<float>(1500 * HarfBuzzRunGlyphData::kMaxGlyphs + 1750);
        horizontal_shaper.Shape(&math, target_size);
        vertical_shaper.Shape(&math, target_size);
    }
}
...
TEST_F(StretchyOperatorShaperTest, NonBMPCodePoint)
{
    ...
    StretchyOperatorShaper horizontal_shaper(kArabicMathematicalOperatorHahWithDal, OpenTypeMathStretchData::StretchAxis::Horizontal);
...
{
    {
        ...
        StretchyOperatorShaper shaper(kNAryWhiteVerticalBarCodePoint, OpenTypeMathStretchData::StretchAxis::Vertical);
