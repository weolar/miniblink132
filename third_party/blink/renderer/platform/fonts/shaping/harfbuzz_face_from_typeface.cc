...
{
    SkStreamAsset* stream_asset = reinterpret_cast<SkStreamAsset*>(stream_asset_ptr);
    delete stream_asset;
}
} // namespace
...
hb::unique_ptr<hb_face_t> HbFaceFromSkTypeface(sk_sp<SkTypeface> typeface)
{
    ...
    if (tf_stream && tf_stream->getMemoryBase()) {
        ...
        hb::unique_ptr<hb_blob_t> face_blob(hb_blob_create(reinterpret_cast<const char*>(tf_memory), base::checked_cast<unsigned int>(tf_size),
            HB_MEMORY_MODE_READONLY, tf_stream.release(), DeleteTypefaceStream));
