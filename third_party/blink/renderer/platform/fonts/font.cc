...
FontFallbackList* GetOrCreateFontFallbackList(const FontDescription& font_description, FontSelector* font_selector)
{
    FontFallbackMap& fallback_map = font_selector ? font_selector->GetFontFallbackMap() : FontCache::Get().GetFontFallbackMap();
...
Font::Font(const FontDescription& font_description, FontSelector* font_selector)
    ...
    , font_fallback_list_(font_selector ? GetOrCreateFontFallbackList(font_description, font_selector) : nullptr)
{
}
...
void DrawBlobs(cc::PaintCanvas* canvas, const cc::PaintFlags& flags, const ShapeResultBloberizer::BlobBuffer& blobs, const gfx::PointF& point,
...
void Font::DrawText(cc::PaintCanvas* canvas, const TextRunPaintInfo& run_info, const gfx::PointF& point, const cc::PaintFlags& flags, DrawType draw_type) const
{
    DrawText(canvas, run_info, point, cc::kInvalidNodeId, flags, draw_type);
}

void Font::DrawText(cc::PaintCanvas* canvas, const TextRunPaintInfo& run_info, const gfx::PointF& point, cc::NodeId node_id, const cc::PaintFlags& flags,
    DrawType draw_type) const
{
    ...
    ShapeResultBloberizer::FillGlyphs bloberizer(GetFontDescription(), run_info, buffer,
        draw_type == Font::DrawType::kGlyphsOnly ? ShapeResultBloberizer::Type::kNormal : ShapeResultBloberizer::Type::kEmitText);
    DrawBlobs(canvas, flags, bloberizer.Blobs(), point, node_id);
}

void Font::DrawText(cc::PaintCanvas* canvas, const TextFragmentPaintInfo& text_info, const gfx::PointF& point, cc::NodeId node_id, const cc::PaintFlags& flags,
    DrawType draw_type) const
{
    // Don't draw anything while we are using custom fonts that are in the process
    // of loading.
    ...
    ShapeResultBloberizer::FillGlyphsNG bloberizer(GetFontDescription(), text_info.text, text_info.from, text_info.to, text_info.shape_result,
        draw_type == Font::DrawType::kGlyphsOnly ? ShapeResultBloberizer::Type::kNormal : ShapeResultBloberizer::Type::kEmitText);
    DrawBlobs(canvas, flags, bloberizer.Blobs(), point, node_id);
}

bool Font::DrawBidiText(cc::PaintCanvas* canvas, const TextRunPaintInfo& run_info, const gfx::PointF& point,
    CustomFontNotReadyAction custom_font_not_ready_action, const cc::PaintFlags& flags, DrawType draw_type) const
{
    // Don't draw anything while we are using custom fonts that are in the process
    // of loading, except if the 'force' argument is set to true (in which case it
    // will use a fallback font).
    if (ShouldSkipDrawing() && custom_font_not_ready_action == kDoNotPaintIfFontNotReady)
        return false;
    ...
    if (run.DirectionalOverride()) [[unlikely]] {
        ...
        // override characters.
        const String text_with_override = BidiParagraph::StringWithDirectionalOverride(run.ToStringView(), run.Direction());
        ...
        return DrawBidiText(canvas, TextRunPaintInfo(run_with_override), point, custom_font_not_ready_action, flags, draw_type);
    }
    ...
    for (const BidiParagraph::Run& bidi_run : bidi_runs) {
        ...
        if (is_sub_run) [[unlikely]] {
            ...
            subrun_info.from = run_info.from < bidi_run.start ? 0 : run_info.from - bidi_run.start;
            subrun_info.to = run_info.to > bidi_run.end ? bidi_run.Length() : run_info.to - bidi_run.start;
            // The range provides information required for positioning the subrun.
            range = word_shaper.GetCharacterRange(subrun, subrun_info.from, subrun_info.to);
        }
        ...
        // crbug.com/1055652.
        STACK_UNINITIALIZED ShapeResultBloberizer::FillGlyphs bloberizer(GetFontDescription(), subrun_info, buffer,
            draw_type == Font::DrawType::kGlyphsOnly ? ShapeResultBloberizer::Type::kNormal : ShapeResultBloberizer::Type::kEmitText);
        if (is_sub_run) [[unlikely]] {
...
void Font::DrawEmphasisMarks(
    cc::PaintCanvas* canvas, const TextRunPaintInfo& run_info, const AtomicString& mark, const gfx::PointF& point, const cc::PaintFlags& flags) const
{
    if (ShouldSkipDrawing())
        return;
    ...
    ShapeResultBuffer buffer;
    word_shaper.FillResultBuffer(run_info, &buffer);
    ShapeResultBloberizer::FillTextEmphasisGlyphs bloberizer(GetFontDescription(), run_info, buffer, emphasis_glyph_data);
    DrawBlobs(canvas, flags, bloberizer.Blobs(), point);
}

void Font::DrawEmphasisMarks(
    cc::PaintCanvas* canvas, const TextFragmentPaintInfo& text_info, const AtomicString& mark, const gfx::PointF& point, const cc::PaintFlags& flags) const
{
    if (ShouldSkipDrawing())
        return;
    ...
    ShapeResultBloberizer::FillTextEmphasisGlyphsNG bloberizer(
        GetFontDescription(), text_info.text, text_info.from, text_info.to, text_info.shape_result, emphasis_glyph_data);
    DrawBlobs(canvas, flags, bloberizer.Blobs(), point);
}
...
float Font::SubRunWidth(const TextRun& run, unsigned from, unsigned to, gfx::RectF* glyph_bounds) const
{
    if (run.length() == 0) {
        return 0;
    }
    ...
    for (const BidiParagraph::Run& visual_run : runs) {
        ...
        unsigned run_to = to > visual_run.end ? visual_run.Length() : to - visual_run.start;

        // Measure the subrun.
        TextRun text_run(StringView(run.ToStringView(), visual_run.start, visual_run.Length()), visual_run.Direction(), /* directional_override */ false);
        text_run.SetNormalizeSpace(true);
        CharacterRange character_range = shaper.GetCharacterRange(text_run, run_from, run_to);
...
unsigned InterceptsFromBlobs(
    const ShapeResultBloberizer::BlobBuffer& blobs, const SkPaint& paint, const std::tuple<float, float>& bounds, SkScalar* intercepts_buffer)
{
    ...
    for (const auto& blob_info : blobs) {
        ...
        num_intervals += blob_info.blob->getIntercepts(bounds_array, offset_intercepts_buffer, &paint);
    }
    return num_intervals;
}

void GetTextInterceptsInternal(const ShapeResultBloberizer::BlobBuffer& blobs, const cc::PaintFlags& flags, const std::tuple<float, float>& bounds,
    Vector<Font::TextIntercept>& intercepts)
{
    ...
    InterceptsFromBlobs(blobs, paint, bounds, reinterpret_cast<SkScalar*>(intercepts.data()));
}

} // anonymous namespace

void Font::GetTextIntercepts(
    const TextRunPaintInfo& run_info, const cc::PaintFlags& flags, const std::tuple<float, float>& bounds, Vector<TextIntercept>& intercepts) const
{
    if (ShouldSkipDrawing())
        return;
    ...
    ShapeResultBuffer buffer;
    word_shaper.FillResultBuffer(run_info, &buffer);
    ShapeResultBloberizer::FillGlyphs bloberizer(GetFontDescription(), run_info, buffer, ShapeResultBloberizer::Type::kTextIntercepts);

    GetTextInterceptsInternal(bloberizer.Blobs(), flags, bounds, intercepts);
}

void Font::GetTextIntercepts(
    const TextFragmentPaintInfo& text_info, const cc::PaintFlags& flags, const std::tuple<float, float>& bounds, Vector<TextIntercept>& intercepts) const
{
    if (ShouldSkipDrawing())
        return;

    ShapeResultBloberizer::FillGlyphsNG bloberizer(
        GetFontDescription(), text_info.text, text_info.from, text_info.to, text_info.shape_result, ShapeResultBloberizer::Type::kTextIntercepts);
...
static inline gfx::RectF PixelSnappedSelectionRect(const gfx::RectF& rect)
{
    ...
    return gfx::RectF(rounded_x, rect.y(), roundf(rect.right() - rounded_x), rect.height());
}

gfx::RectF Font::SelectionRectForText(const TextRun& run, const gfx::PointF& point, float height, int from, int to) const
{
    ...
    return PixelSnappedSelectionRect(gfx::RectF(point.x() + range.start, point.y(), range.Width(), height));
}

int Font::OffsetForPosition(const TextRun& run, float x_float, IncludePartialGlyphsOption partial_glyphs, BreakGlyphsOption break_glyphs) const
{
    FontCachePurgePreventer purge_preventer;
    CachingWordShaper shaper(*this);
...
void Font::ReportEmojiSegmentGlyphCoverage(unsigned num_clusters, unsigned num_broken_clusters) const
{
    FontSelector* fontSelector = EnsureFontFallbackList()->GetFontSelector();
    // See ReportNotDefGlyph(), sometimes no fontSelector is available in non-DOM
    // usages of Font.
    if (fontSelector) {
        fontSelector->ReportEmojiSegmentGlyphCoverage(num_clusters, num_broken_clusters);
    }
}
...
float Font::TabWidth(const SimpleFontData* font_data, const TabSize& tab_size, float position) const
{
...
LayoutUnit Font::TabWidth(const TabSize& tab_size, LayoutUnit position) const
{
    ...
    LayoutUnit distance_to_tab_stop = LayoutUnit::FromFloatFloor(base_tab_width - fmodf(position, base_tab_width));
