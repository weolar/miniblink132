...
template <class T> T* advance_by_byte_size(T* p, unsigned byte_size)
{
    return reinterpret_cast<T*>(reinterpret_cast<uint8_t*>(p) + byte_size);
}

template <class T> const T* advance_by_byte_size(const T* p, unsigned byte_size)
{
    return reinterpret_cast<const T*>(reinterpret_cast<const uint8_t*>(p) + byte_size);
}

} // namespace

void SkFontGetGlyphWidthForHarfBuzz(const SkFont& font, hb_codepoint_t codepoint, hb_position_t* width)
{
...
void SkFontGetGlyphWidthForHarfBuzz(
    const SkFont& font, unsigned count, const hb_codepoint_t* glyphs, const unsigned glyph_stride, hb_position_t* advances, unsigned advance_stride)
{
    ...
    for (unsigned i = 0; i < count; i++, glyphs = advance_by_byte_size(glyphs, glyph_stride)) {
    ...
    for (unsigned i = 0; i < count; i++, advances = advance_by_byte_size(advances, advance_stride)) {
...
void SkFontGetGlyphExtentsForHarfBuzz(const SkFont& font, hb_codepoint_t codepoint, hb_glyph_extents_t* extents)
{
    // We don't want to compute glyph extents for kUnmatchedVSGlyphId
    // cases yet. Since we will do that during the second shaping pass,
    // when VariationSelectorMode is set to kIgnoreVariationSelector.
...
void SkFontGetBoundsForGlyphs(const SkFont& font, const Vector<Glyph, 256>& glyphs, SkRect* bounds)
{
#if BUILDFLAG(IS_APPLE)
