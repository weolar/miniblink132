...
FontFallbackIterator::FontFallbackIterator(const FontDescription& description, FontFallbackList* fallback_list, FontFallbackPriority font_fallback_priority)
...
bool FontFallbackIterator::RangeSetContributesForHint(const HintCharList& hint_list, const FontDataForRangeSet* segmented_face)
{
    for (const auto& hint : hint_list) {
        if (segmented_face->Contains(hint)) {
            ...
            // load them.
            if (!segmented_face->IsPendingCustomFont() || segmented_face->IsPendingDataUrlCustomFont() || !AlreadyLoadingRangeForHintChar(hint)) {
                return true;
            }
        }
    }
...
void FontFallbackIterator::WillUseRange(const AtomicString& family, const FontDataForRangeSet& range_set)
{
...
FontDataForRangeSet* FontFallbackIterator::UniqueOrNext(FontDataForRangeSet* candidate, const HintCharList& hint_list)
{
    if (!candidate->HasFontData())
        return Next(hint_list);

    SkTypeface* candidate_typeface = candidate->FontData()->PlatformData().Typeface();
...
bool FontFallbackIterator::NeedsHintList() const
{
    ...
    const FontData* font_data = font_fallback_list_->FontDataAt(font_description_, current_font_data_index_);
...
FontDataForRangeSet* FontFallbackIterator::Next(const HintCharList& hint_list)
{
    ...
    if (fallback_stage_ == kFallbackPriorityFonts) {
        ...
        FontDataForRangeSet* fallback_priority_font_range = MakeGarbageCollected<FontDataForRangeSet>(FallbackPriorityFont(hint_list[0]));
    ...
    if (fallback_stage_ == kSystemFonts) {
        ...
        if (system_font) {
            ...
            return UniqueOrNext(MakeGarbageCollected<FontDataForRangeSet>(system_font), hint_list);
        }
        ...
        const SimpleFontData* last_resort = font_cache.GetLastResortFallbackFont(font_description_);

        if (FontSelector* font_selector = font_fallback_list_->GetFontSelector()) {
            font_selector->ReportLastResortFallbackFontLookup(font_description_, last_resort);
        }

        return UniqueOrNext(MakeGarbageCollected<FontDataForRangeSet>(last_resort), hint_list);
    }
    ...
    DCHECK(fallback_stage_ == kFontGroupFonts || fallback_stage_ == kSegmentedFace);
    const FontData* font_data = font_fallback_list_->FontDataAt(font_description_, current_font_data_index_);
    ...
    if (!font_data) {
        ...
        fallback_stage_ = IsNonTextFallbackPriority(font_fallback_priority_) ? kFallbackPriorityFonts : kSystemFonts;
        return Next(hint_list);
    }
    ...
    if (!font_data->IsSegmented()) {
        ...
        if (!font_data->IsLoading()) {
            SimpleFontData* non_segmented = const_cast<SimpleFontData*>(To<SimpleFontData>(font_data));
            ...
            return UniqueOrNext(MakeGarbageCollected<FontDataForRangeSet>(non_segmented), hint_list);
        }
        return Next(hint_list);
    }
    ...
    FontDataForRangeSet* current_segmented_face = segmented->FaceAt(segmented_face_index_);
    ...
    if (RangeSetContributesForHint(hint_list, current_segmented_face)) {
        const SimpleFontData* current_segmented_face_font_data = current_segmented_face->FontData();
        if (const CustomFontData* current_segmented_face_custom_font_data = current_segmented_face_font_data->GetCustomFontData())
...
const SimpleFontData* FontFallbackIterator::FallbackPriorityFont(UChar32 hint)
{
    const SimpleFontData* font_data = FontCache::Get().FallbackFontForCharacter(
        font_description_, hint, font_fallback_list_->PrimarySimpleFontData(font_description_), font_fallback_priority_);

    if (FontSelector* font_selector = font_fallback_list_->GetFontSelector()) {
        font_selector->ReportFontLookupByFallbackCharacter(hint, font_fallback_priority_, font_description_, font_data);
    }
    return font_data;
}

static inline unsigned ChooseHintIndex(const FontFallbackIterator::HintCharList& hint_list)
{
...
const SimpleFontData* FontFallbackIterator::UniqueSystemFontForHintList(const HintCharList& hint_list)
{
    ...
    const SimpleFontData* font_data
        = font_cache.FallbackFontForCharacter(font_description_, hint, font_fallback_list_->PrimarySimpleFontData(font_description_));

    if (FontSelector* font_selector = font_fallback_list_->GetFontSelector()) {
        font_selector->ReportFontLookupByFallbackCharacter(hint, FontFallbackPriority::kText, font_description_, font_data);
    }
    return font_data;
}
...
{
    return fallback_stage_ == other.fallback_stage_ && font_fallback_priority_ == other.font_fallback_priority_
        && current_font_data_index_ == other.current_font_data_index_ && segmented_face_index_ == other.segmented_face_index_
        && font_description_ == other.font_description_ && previously_asked_for_hint_ == other.previously_asked_for_hint_
        && unique_font_data_for_range_sets_returned_ == other.unique_font_data_for_range_sets_returned_
