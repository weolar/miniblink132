...
inline UScriptCode GetScriptForOpenType(UChar32 ch, UErrorCode* status)
{
    ...
    if (script == USCRIPT_KATAKANA || script == USCRIPT_KATAKANA_OR_HIRAGANA) [[unlikely]] {
...
inline UScriptCode FirstHanScript(const ScriptRunIterator::UScriptCodeList& list)
{
...
ScriptRunIterator::UScriptCodeList GetHanScriptExtensions()
{
    ...
    // Punctuation block.
    int count = uscript_getScriptExtensions(kLeftCornerBracket, &list[0], list.size(), &status);
    if (U_SUCCESS(status)) {
        DCHECK_GT(count, 0);
        list.resize(count);
        return list;
...
void FixScriptsByEastAsianWidth(UChar32 ch, ScriptRunIterator::UScriptCodeList* set)
{
    ...
    const auto eaw = static_cast<UEastAsianWidth>(u_getIntPropertyValue(ch, UCHAR_EAST_ASIAN_WIDTH));
    if (eaw == U_EA_WIDE || eaw == U_EA_FULLWIDTH || eaw == U_EA_HALFWIDTH) {
        ...
        DEFINE_STATIC_LOCAL(ScriptRunIterator::UScriptCodeList, han_scripts, (GetHanScriptExtensions()));
...
void ICUScriptData::GetScripts(UChar32 ch, UScriptCodeList& dst) const
{
    ...
    if (primary_script != USCRIPT_INHERITED && primary_script != USCRIPT_COMMON && primary_script != USCRIPT_INVALID_CODE) {
...
{
    return static_cast<PairedBracketType>(u_getIntPropertyValue(ch, UCHAR_BIDI_PAIRED_BRACKET_TYPE));
}
...
ScriptRunIterator::ScriptRunIterator(base::span<const UChar> text, const ScriptData* data)
    : text_(text.data())
    ...
    while (Fetch(&pos, &ch)) {
        ...
        if (!MergeSets()) {
            ...
            *script = ResolveCurrentScript();
            ...
            const bool exclude_last = paired_type == PairedBracketType::kBracketTypeOpen;
    ...
    if (!brackets_.empty()) {
        ...
        for (auto it = brackets_.rbegin(); it != brackets_.rend(); ++it) {
            if (it->ch == target) {
                ...
                int num_popped = static_cast<int>(std::distance(brackets_.rbegin(), it));
                // TODO: No resize operation in WTF::Deque?
                for (int i = 0; i < num_popped; ++i)
                    brackets_.pop_back();
                brackets_fixup_depth_ = static_cast<wtf_size_t>(std::max(0, static_cast<int>(brackets_fixup_depth_) - num_popped));
                return;
            }
        }
    }
...
bool ScriptRunIterator::MergeSets()
{
    ...
    if (next_set_->at(0) <= USCRIPT_INHERITED) {
        if (next_set_->size() == 2 && priority_script <= USCRIPT_INHERITED && common_preferred_ == USCRIPT_COMMON) {
    ...
    if (!have_priority) {
        ...
        have_priority = std::find(current_set_it, current_end, priority_script) != current_end;
    }
    ...
    int written = static_cast<int>(std::distance(current_set_.begin(), current_write_it));
    if (written > 0) {
...
void ScriptRunIterator::FixupStack(UScriptCode resolved_script, bool exclude_last)
{
