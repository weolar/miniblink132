...
public:
    ...
    explicit UTF16RagelIterator(base::span<const UChar> buffer, unsigned cursor = 0)
        : buffer_(buffer)
        , cursor_(cursor)
        , cached_category_(EmojiSegmentationCategory::kInvalidCacheEntry)
    {
    ...
    {
        return buffer_.data() == other.buffer_.data() && buffer_.size() == other.buffer_.size() && cursor_ == other.cursor_;
    }
