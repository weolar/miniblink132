...
hb_direction_t HarfBuzzDirection(blink::OpenTypeMathStretchData::StretchAxis stretch_axis)
{
    return stretch_axis == blink::OpenTypeMathStretchData::StretchAxis::Horizontal ? HB_DIRECTION_LTR : HB_DIRECTION_BTT;
}
...
std::optional<float> OpenTypeMathSupport::MathConstant(const HarfBuzzFace* harfbuzz_face, MathConstants constant)
{
    ...
    hb_position_t harfbuzz_value = hb_ot_math_get_constant(font, static_cast<hb_ot_math_constant_t>(constant));
...
std::optional<float> OpenTypeMathSupport::MathItalicCorrection(const HarfBuzzFace* harfbuzz_face, Glyph glyph)
{
    if (!HasMathData(harfbuzz_face)) {
        return std::nullopt;
    }
    ...
    return std::optional<float>(HarfBuzzUnitsToFloat(hb_ot_math_get_glyph_italics_correction(font, glyph)));
}

template <typename HarfBuzzRecordType>
using GetHarfBuzzMathRecordGetter = base::OnceCallback<unsigned int(
    hb_font_t* font, hb_codepoint_t glyph, hb_direction_t direction, unsigned int start_offset, unsigned int* record_count, HarfBuzzRecordType* record_array)>;

template <typename HarfBuzzRecordType, typename RecordType> using HarfBuzzMathRecordConverter = base::RepeatingCallback<RecordType(HarfBuzzRecordType)>;

template <typename HarfBuzzRecordType, typename RecordType>
Vector<RecordType> GetHarfBuzzMathRecord(const HarfBuzzFace* harfbuzz_face, Glyph base_glyph, OpenTypeMathStretchData::StretchAxis stretch_axis,
    GetHarfBuzzMathRecordGetter<HarfBuzzRecordType> getter, HarfBuzzMathRecordConverter<HarfBuzzRecordType, RecordType> converter,
...
Vector<OpenTypeMathStretchData::GlyphVariantRecord> OpenTypeMathSupport::GetGlyphVariantRecords(
    const HarfBuzzFace* harfbuzz_face, Glyph base_glyph, OpenTypeMathStretchData::StretchAxis stretch_axis)
{
    DCHECK(harfbuzz_face);
    DCHECK(base_glyph);

    auto getter = WTF::BindOnce(&hb_ot_math_get_glyph_variants);
    auto converter = WTF::BindRepeating([](hb_ot_math_glyph_variant_t record) -> OpenTypeMathStretchData::GlyphVariantRecord { return record.glyph; });
    return GetHarfBuzzMathRecord(harfbuzz_face, base_glyph, stretch_axis, std::move(getter), std::move(converter),
        std::optional<OpenTypeMathStretchData::GlyphVariantRecord>(base_glyph));
}

Vector<OpenTypeMathStretchData::GlyphPartRecord> OpenTypeMathSupport::GetGlyphPartRecords(
    const HarfBuzzFace* harfbuzz_face, Glyph base_glyph, OpenTypeMathStretchData::StretchAxis stretch_axis, float* italic_correction)
{
    ...
    auto getter = WTF::BindOnce([](hb_font_t* font, hb_codepoint_t glyph, hb_direction_t direction, unsigned int start_offset, unsigned int* parts_count,
                                    hb_ot_math_glyph_part_t* parts) {
        hb_position_t italic_correction;
        return hb_ot_math_get_glyph_assembly(font, glyph, direction, start_offset, parts_count, parts, &italic_correction);
    });
    auto converter = WTF::BindRepeating([](hb_ot_math_glyph_part_t record) -> OpenTypeMathStretchData::GlyphPartRecord {
        return { static_cast<Glyph>(record.glyph), HarfBuzzUnitsToFloat(record.start_connector_length), HarfBuzzUnitsToFloat(record.end_connector_length),
            HarfBuzzUnitsToFloat(record.full_advance), !!(record.flags & HB_MATH_GLYPH_PART_FLAG_EXTENDER) };
    });
    Vector<OpenTypeMathStretchData::GlyphPartRecord> parts = GetHarfBuzzMathRecord(
        harfbuzz_face, base_glyph, stretch_axis, std::move(getter), std::move(converter), std::optional<OpenTypeMathStretchData::GlyphPartRecord>());
    if (italic_correction && !parts.empty()) {
        ...
        hb_ot_math_get_glyph_assembly(hb_font, base_glyph, HarfBuzzDirection(stretch_axis), 0, nullptr, nullptr, &harfbuzz_italic_correction);
