...
protected:
    void SetUp() override
    {
        ...
        Font colr_palette_font = blink::test::CreateTestFont(AtomicString("Ahem"), pathToColrPalettesTestFont(), 16, &ligatures);
        colr_palette_typeface_ = sk_ref_sp(colr_palette_font.PrimaryFont()->PlatformData().Typeface());

        Font non_colr_font = blink::test::CreateTestFont(AtomicString("Ahem"), pathToNonColrTestFont(), 16, &ligatures);
        non_colr_ahem_typeface_ = sk_ref_sp(non_colr_font.PrimaryFont()->PlatformData().Typeface());
    }
...
{
    for (auto& palette_use : { OpenTypeCpalLookup::kUsableWithLightBackground, OpenTypeCpalLookup::kUsableWithDarkBackground }) {
        std::optional<uint16_t> palette_result = OpenTypeCpalLookup::FirstThemedPalette(non_colr_ahem_typeface_, palette_use);
...
TEST_F(OpenTypeCpalLookupTest, DarkLightPalettes)
{
    ...
    std::vector<std::pair<OpenTypeCpalLookup::PaletteUse, uint16_t>> expectations { { OpenTypeCpalLookup::kUsableWithLightBackground, 2 },
        { OpenTypeCpalLookup::kUsableWithDarkBackground, 3 } };
    for (auto& expectation : expectations) {
        std::optional<uint16_t> palette_result = OpenTypeCpalLookup::FirstThemedPalette(colr_palette_typeface_, expectation.first);
...
TEST_F(OpenTypeCpalLookupTest, RetrieveColorRecordsFromExistingPalette)
{
    ...
    Vector<Color> actual_color_records = OpenTypeCpalLookup::RetrieveColorRecords(colr_palette_typeface_, 3);
...
TEST_F(OpenTypeCpalLookupTest, RetrieveColorRecordsFromNonExistingPalette)
{
    ...
    Vector<Color> actual_color_records = OpenTypeCpalLookup::RetrieveColorRecords(colr_palette_typeface_, 16);
