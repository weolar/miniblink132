...
Vector<VariationAxis> VariableAxesNames::GetVariationAxes(sk_sp<SkTypeface> typeface)
{
    Vector<VariationAxis> output;
    std::unique_ptr<SkStreamAsset> stream = typeface->openStream(nullptr);
    if (!stream)
        return output;
    sk_sp<SkData> sk_data = SkData::MakeFromStream(stream.get(), stream->getLength());
    ...
    std::unique_ptr<hb_ot_var_axis_info_t[]> axes = std::make_unique<hb_ot_var_axis_info_t[]>(axes_count);
    ...
    for (unsigned i = 0; i < axes_count; i++) {
        ...
        unsigned name_length = hb_ot_name_get_utf16(face.get(), axes[i].name_id, HB_LANGUAGE_INVALID, nullptr, nullptr);

        axis.name = "";
        if (name_length) {
            unsigned buffer_length = name_length + 1;
            std::unique_ptr<char16_t[]> buffer = std::make_unique<char16_t[]>(buffer_length);
            hb_ot_name_get_utf16(face.get(), axes[i].name_id, HB_LANGUAGE_INVALID, &buffer_length, reinterpret_cast<uint16_t*>(buffer.get()));
