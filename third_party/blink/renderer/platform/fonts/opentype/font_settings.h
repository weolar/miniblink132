...
template <typename T> class FontTagValuePair {
    DISALLOW_NEW();
...
public:
        ...
        , value_(value)
    {
        ...
        DCHECK(tag == 0
            || (tag & 0xff000000) < 0x7f000000 && (tag & 0xff000000) >= 0x20000000 && (tag & 0xff0000) < 0x7f0000 && (tag & 0xff0000) >= 0x200000
                && (tag & 0xff00) < 0x7f00 && (tag & 0xff00) >= 0x2000 && (tag & 0xff) < 0x7f && (tag & 0xff) >= 0x20);
    }
    ...
    {
        // ensure tag is valid
        DCHECK((tag_ & 0xff000000) < 0x7f000000 && (tag_ & 0xff000000) >= 0x20000000 && (tag_ & 0xff0000) < 0x7f0000 && (tag_ & 0xff0000) >= 0x200000
            && (tag_ & 0xff00) < 0x7f00 && (tag_ & 0xff00) >= 0x2000 && (tag_ & 0xff) < 0x7f && (tag_ & 0xff) >= 0x20);
    }
...
template <typename T> class FontSettings {
public:
    ...
    Vector<T, 0>::const_iterator begin() const
    {
        return list_.begin();
    }
    Vector<T, 0>::const_iterator end() const
    {
        return list_.end();
    }
    Vector<T, 0>::iterator begin()
    {
        return list_.begin();
    }
    Vector<T, 0>::iterator end()
    {
        return list_.end();
    }
...
class PLATFORM_EXPORT FontFeatureSettings : public FontSettings<FontFeature>, public RefCounted<FontFeatureSettings> {
public:
...
class PLATFORM_EXPORT FontVariationSettings : public FontSettings<FontVariationAxis>, public RefCounted<FontVariationSettings> {
public:
