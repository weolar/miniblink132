    ...
    : features_(kInitialSize)
{
    ...
    unsigned size = hb_ot_layout_table_get_feature_tags(hb_face, HB_OT_TAG_GPOS, 0, &get_size, features_.data());
    features_.resize(size);
    if (size > get_size) {
        hb_ot_layout_table_get_feature_tags(hb_face, HB_OT_TAG_GPOS, 0, &size, features_.data());
