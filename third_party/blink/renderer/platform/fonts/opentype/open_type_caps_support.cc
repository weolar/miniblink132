...
bool activationSelectorPresent(
    hb_face_t* hb_face, const hb_aat_layout_feature_type_t feature_type, const hb_aat_layout_feature_selector_t enabled_selector_expectation)
{
    Vector<hb_aat_layout_feature_selector_info_t> feature_selectors;
    unsigned num_feature_selectors = 0;
    unsigned default_index = 0;
    num_feature_selectors = hb_aat_layout_feature_type_get_selector_infos(hb_face, feature_type, 0, nullptr, nullptr, nullptr);
    feature_selectors.resize(num_feature_selectors);
    if (!hb_aat_layout_feature_type_get_selector_infos(hb_face, feature_type, 0, &num_feature_selectors, feature_selectors.data(), &default_index)) {
        return false;
    }
    for (hb_aat_layout_feature_selector_info_t selector_info : feature_selectors) {
...
OpenTypeCapsSupport::OpenTypeCapsSupport(const HarfBuzzFace* harfbuzz_face, FontDescription::FontVariantCaps requested_caps,
    FontDescription::FontSynthesisSmallCaps font_synthesis_small_caps, hb_script_t script)
...
FontDescription::FontVariantCaps OpenTypeCapsSupport::FontFeatureToUse(SmallCapsIterator::SmallCapsBehavior source_text_case)
{
    ...
    if (font_support_ == FontSupport::kFallback) {
        ...
        if (requested_caps_ == FontDescription::FontVariantCaps::kPetiteCaps
            || (requested_caps_ == FontDescription::FontVariantCaps::kUnicase && source_text_case == SmallCapsIterator::kSmallCapsSameCase))
...
bool OpenTypeCapsSupport::NeedsRunCaseSplitting()
{
    ...
    return font_support_ != FontSupport::kFull && requested_caps_ != FontDescription::kTitlingCaps && SyntheticSmallCapsAllowed();
}

bool OpenTypeCapsSupport::NeedsSyntheticFont(SmallCapsIterator::SmallCapsBehavior run_case)
{
    if (font_support_ == FontSupport::kFull)
        return false;
    ...
    if (font_support_ == FontSupport::kNone) {
        if (run_case == SmallCapsIterator::kSmallCapsUppercaseNeeded
            && (caps_synthesis_ == CapsSynthesis::kLowerToSmallCaps || caps_synthesis_ == CapsSynthesis::kBothToSmallCaps))
            return true;
...
CaseMapIntend OpenTypeCapsSupport::NeedsCaseChange(SmallCapsIterator::SmallCapsBehavior run_case)
{
    ...
    case SmallCapsIterator::kSmallCapsSameCase:
        case_map_intend = font_support_ == FontSupport::kFallback
                && (caps_synthesis_ == CapsSynthesis::kBothToSmallCaps || caps_synthesis_ == CapsSynthesis::kUpperToSmallCaps)
...
OpenTypeCapsSupport::FontFormat OpenTypeCapsSupport::GetFontFormat() const
{
    if (font_format_ == FontFormat::kUndetermined) {
        ...
        hb::unique_ptr<hb_blob_t> morx_blob(hb_face_reference_table(hb_face, HB_TAG('m', 'o', 'r', 'x')));
        hb::unique_ptr<hb_blob_t> mort_blob(hb_face_reference_table(hb_face, HB_TAG('m', 'o', 'r', 't')));
        ...
        // exposed in HarfBuzz.
        bool has_morx_or_mort = hb_blob_get_length(morx_blob.get()) || hb_blob_get_length(mort_blob.get());
        bool has_gsub = hb_ot_layout_has_substitution(hb_face);
        font_format_ = has_morx_or_mort && !has_gsub ? FontFormat::kAat : FontFormat::kOpenType;
    }
    return font_format_;
}

bool OpenTypeCapsSupport::SupportsFeature(hb_script_t script, uint32_t tag) const
{
...
bool OpenTypeCapsSupport::SupportsAatFeature(uint32_t tag) const
{
    ...
    unsigned feature_count = hb_aat_layout_get_feature_types(hb_face, 0, nullptr, nullptr);
    aat_features.resize(feature_count);
    if (!hb_aat_layout_get_feature_types(hb_face, 0, &feature_count, aat_features.data()))
        return false;
    ...
    if (tag == HB_TAG('s', 'm', 'c', 'p')) {
        ...
        if (!aat_features.Contains(HB_AAT_LAYOUT_FEATURE_TYPE_LETTER_CASE) && !aat_features.Contains(HB_AAT_LAYOUT_FEATURE_TYPE_LOWER_CASE))
            return false;

        // Check for new style small caps, feature id 38.
        if (aat_features.Contains(HB_AAT_LAYOUT_FEATURE_TYPE_LOWER_CASE)) {
            if (activationSelectorPresent(hb_face, HB_AAT_LAYOUT_FEATURE_TYPE_LOWER_CASE, HB_AAT_LAYOUT_FEATURE_SELECTOR_LOWER_CASE_SMALL_CAPS))
                return true;
        }
        ...
        if (aat_features.Contains(HB_AAT_LAYOUT_FEATURE_TYPE_LETTER_CASE)) {
            if (activationSelectorPresent(hb_face, HB_AAT_LAYOUT_FEATURE_TYPE_LETTER_CASE, HB_AAT_LAYOUT_FEATURE_SELECTOR_SMALL_CAPS))
                return true;
        }
    ...
    if (tag == HB_TAG('c', '2', 's', 'c')) {
        ...
        return activationSelectorPresent(hb_face, HB_AAT_LAYOUT_FEATURE_TYPE_UPPER_CASE, HB_AAT_LAYOUT_FEATURE_SELECTOR_UPPER_CASE_SMALL_CAPS);
    }
...
void OpenTypeCapsSupport::DetermineFontSupport(hb_script_t script)
{
    ...
    case FontDescription::kAllSmallCaps:
        if (!(SupportsFeature(script, HB_TAG('s', 'm', 'c', 'p')) && SupportsFeature(script, HB_TAG('c', '2', 's', 'c')))) {
            font_support_ = FontSupport::kNone;
    ...
    case FontDescription::kAllPetiteCaps:
        if (!(SupportsFeature(script, HB_TAG('p', 'c', 'a', 'p')) && SupportsFeature(script, HB_TAG('c', '2', 'p', 'c')))) {
            if (SupportsFeature(script, HB_TAG('s', 'm', 'c', 'p')) && SupportsFeature(script, HB_TAG('c', '2', 's', 'c'))) {
                font_support_ = FontSupport::kFallback;
            } else {
                font_support_ = FontSupport::kNone;
