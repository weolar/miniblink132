...
protected:
    Font CreateCanvasTestFont(float size)
    {
        ...
        return blink::test::CreateTestFont(AtomicString("CanvasTest"), blink::test::BlinkWebTestsFontsTestDataPath(kCanvasTestFontName), size, &ligatures);
    }

    Font CreateAhemFont(float size)
    {
        FontDescription::VariantLigatures ligatures;
        return blink::test::CreateTestFont(AtomicString("Ahem"), blink::test::BlinkWebTestsFontsTestDataPath(kAhemFontName), size, &ligatures);
    }
};
...
TEST_F(OpenTypeBaselineMetricsTest, AlphabeticBaseline)
{
    ...
    OpenTypeBaselineMetrics baseline_metrics(baseline_test_font.PrimaryFont()->PlatformData().GetHarfBuzzFace(), FontOrientation::kHorizontal);
    EXPECT_EQ(baseline_metrics.OpenTypeAlphabeticBaseline(), 0);

    baseline_test_font = CreateCanvasTestFont(200);
    baseline_metrics = OpenTypeBaselineMetrics(baseline_test_font.PrimaryFont()->PlatformData().GetHarfBuzzFace(), FontOrientation::kHorizontal);
    EXPECT_EQ(baseline_metrics.OpenTypeAlphabeticBaseline(), 0);
