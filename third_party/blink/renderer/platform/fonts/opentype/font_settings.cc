...
AtomicString FourByteTagToAtomicString(uint32_t tag)
{
    const std::array<LChar, 4> tag_string
        = { static_cast<LChar>(tag >> 24), static_cast<LChar>(tag >> 16), static_cast<LChar>(tag >> 8), static_cast<LChar>(tag) };
    return AtomicString(base::span<const LChar>(tag_string.data(), tag_string.size()));
}
