...
/* static */
std::optional<uint16_t> OpenTypeCpalLookup::FirstThemedPalette(sk_sp<SkTypeface> typeface, PaletteUse palette_use)
{
    ...
    const hb_ot_color_palette_flags_t desired_flag = palette_use == kUsableWithLightBackground ? HB_OT_COLOR_PALETTE_FLAG_USABLE_WITH_LIGHT_BACKGROUND
...
Vector<Color> OpenTypeCpalLookup::RetrieveColorRecords(sk_sp<SkTypeface> typeface, unsigned palette_index)
{
    hb::unique_ptr<hb_face_t> face(HbFaceFromSkTypeface(typeface));
    ...
    unsigned num_colors = hb_ot_color_palette_get_colors(face.get(), palette_index, 0, nullptr, nullptr);
    if (!num_colors) {
        return Vector<Color>();
    }

    std::unique_ptr<hb_color_t[]> colors = std::make_unique<hb_color_t[]>(num_colors);
    ...
    for (unsigned i = 0; i < num_colors; i++) {
        color_records[i] = Color(hb_color_get_red(colors[i]), hb_color_get_green(colors[i]), hb_color_get_blue(colors[i]), hb_color_get_alpha(colors[i]));
    }
