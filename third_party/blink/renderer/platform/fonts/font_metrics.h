...
public:
    using ApplyBaselineTable = base::StrongAlias<class ApplyBaselineTableTag, bool>;

    float FloatAscent(FontBaseline baseline_type = kAlphabeticBaseline, ApplyBaselineTable apply_baseline_table = ApplyBaselineTable(false)) const
    {
    ...
    float FloatDescent(FontBaseline baseline_type = kAlphabeticBaseline, ApplyBaselineTable apply_baseline_table = ApplyBaselineTable(false)) const
    {
        if (baseline_type == kAlphabeticBaseline)
            return float_descent_;
        return FloatHeight() - FloatAscentInternal(baseline_type, apply_baseline_table);
    }
    ...
    float FloatHeight() const
    {
        return float_ascent_ + float_descent_;
    }

    float ConvertBaseline(float value, FontBaseline to, FontBaseline from = kAlphabeticBaseline) const
    {
    ...
    void SetCapHeight(float cap_height)
    {
    ...
    int LineGap() const
    {
        return static_cast<int>(lroundf(line_gap_));
    }
    ...
    {
        return static_cast<int>(lroundf(line_spacing_));
    }
    ...
    {
        return has_x_height_ && x_height_ > 0;
    }
    void SetHasXHeight(bool has_x_height)
    {
        has_x_height_ = has_x_height;
    }

    // Integer variants of certain metrics, used for HTML rendering.
    int Ascent(FontBaseline baseline_type = kAlphabeticBaseline, ApplyBaselineTable apply_baseline_table = ApplyBaselineTable(false)) const
    {
        if (baseline_type == kAlphabeticBaseline)
            return int_ascent_;
    ...
    int Descent(FontBaseline baseline_type = kAlphabeticBaseline, ApplyBaselineTable apply_baseline_table = ApplyBaselineTable(false)) const
    {
        if (baseline_type == kAlphabeticBaseline)
            return int_descent_;
    ...
    int Height() const
    {
        return int_ascent_ + int_descent_;
    }
    ...
    LayoutUnit FixedAscent(FontBaseline baseline_type = kAlphabeticBaseline) const
    {
        return LayoutUnit::FromFloatRound(FloatAscent(baseline_type));
    }

    LayoutUnit FixedDescent(FontBaseline baseline_type = kAlphabeticBaseline) const
    {
    ...
    FontHeight GetFontHeight(FontBaseline baseline_type = kAlphabeticBaseline) const
    {
        // TODO(kojii): In future, we'd like to use LayoutUnit metrics to support
        // sub-CSS-pixel layout.
        return FontHeight(LayoutUnit(Ascent(baseline_type)), LayoutUnit(Descent(baseline_type)));
    }
    ...
    bool HasZeroWidth() const
    {
        return has_zero_width_;
    }
    ...
    std::optional<float> UnderlinePosition() const
    {
    ...
    static void AscentDescentWithHacks(float& ascent, float& descent, const FontPlatformData&, const SkFont&, bool subpixel_ascent_descent = false,
        std::optional<float> ascent_override = std::nullopt, std::optional<float> descent_override = std::nullopt);
...
private:
    ...
    float FloatAscentInternal(FontBaseline baseline_type, ApplyBaselineTable apply_baseline_table) const;
    int IntAscentInternal(FontBaseline baseline_type, ApplyBaselineTable apply_baseline_table) const;
