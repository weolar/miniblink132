...
public:
    ...
    float GetWidth(FontName font_name, const String& text, bool ltr, int start = 0, int end = -1)
    {
        FontDescription::VariantLigatures ligatures(FontDescription::kEnabledLigaturesState);
        Font font = CreateTestFont(AtomicString("TestFont"), test::PlatformTestDataPath(font_path.find(font_name)->value), 100, &ligatures);
        TextRun text_run(text, ltr ? TextDirection::kLtr : TextDirection::kRtl, false);
        ...
        gfx::RectF rect = font.SelectionRectForText(text_run, gfx::PointF(), 12, start, end);
        return rect.width();
    }

    int GetCharacter(FontName font_name, const String& text, bool ltr, float position, bool partial)
    {
        FontDescription::VariantLigatures ligatures(FontDescription::kEnabledLigaturesState);
        Font font = CreateTestFont(AtomicString("TestFont"), test::PlatformTestDataPath(font_path.find(font_name)->value), 100, &ligatures);
        TextRun text_run(text, ltr ? TextDirection::kLtr : TextDirection::kRtl, false);

        return font.OffsetForPosition(text_run, position, partial ? kIncludePartialGlyphs : kOnlyFullGlyphs, BreakGlyphsOption(true));
    }
...
TEST_F(CursorPositionTest, LTRLigatureMouse)
{
    ...
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth / 4 - 1, false), 0);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth / 4 - 1, true), 0);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth / 4 + 1, false), 0);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth / 4 + 1, true), 1);

    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth / 2 - 1, false), 0);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth / 2 - 1, true), 1);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth / 2 + 1, false), 1);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth / 2 + 1, true), 1);

    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth * 3 / 4 - 1, false), 1);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth * 3 / 4 - 1, true), 1);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth * 3 / 4 + 1, false), 1);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth * 3 / 4 + 1, true), 2);
    ...
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth / 4 - 1, false), 2);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth / 4 - 1, true), 2);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth / 4 + 1, false), 2);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth / 4 + 1, true), 3);

    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth / 2 - 1, false), 2);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth / 2 - 1, true), 3);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth / 2 + 1, false), 3);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth / 2 + 1, true), 3);

    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth * 3 / 4 - 1, false), 3);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth * 3 / 4 - 1, true), 3);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth * 3 / 4 + 1, false), 3);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth * 3 / 4 + 1, true), 4);

    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth - 1, false), 3);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth - 1, true), 4);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth + 1, false), 4);
    EXPECT_EQ(GetCharacter(kMegalopolis, "FURA", true, kFUWidth + kRAWidth + 1, true), 4);
}
...
TEST_F(CursorPositionTest, RTLMouse)
{
    ...
    EXPECT_EQ(GetCharacter(kAmiri, u"تخ", false, kAboveTaWidth * 2 / 3, false), 1);
    ...
    EXPECT_EQ(GetCharacter(kAmiri, u"تخ", false, kAboveTaWidth + kAboveKhaWidth / 4, false), 0);
    EXPECT_EQ(GetCharacter(kAmiri, u"تخ", false, kAboveTaWidth + kAboveKhaWidth / 4, true), 1);
    EXPECT_EQ(GetCharacter(kAmiri, u"تخ", false, kAboveTaWidth + kAboveKhaWidth * 2 / 3, false), 0);
    EXPECT_EQ(GetCharacter(kAmiri, u"تخ", false, kAboveTaWidth + kAboveKhaWidth * 2 / 3, true), 0);
    EXPECT_EQ(GetCharacter(kAmiri, u"تخ", false, kAboveTaWidth + kAboveKhaWidth + 1, false), 0);
    EXPECT_EQ(GetCharacter(kAmiri, u"تخ", false, kAboveTaWidth + kAboveKhaWidth + 1, true), 0);
    EXPECT_EQ(GetCharacter(kAmiri, u"تخ", false, 2 * (kAboveTaWidth + kAboveKhaWidth), false), 0);
    EXPECT_EQ(GetCharacter(kAmiri, u"تخ", false, 2 * (kAboveTaWidth + kAboveKhaWidth), true), 0);
}
...
TEST_F(CursorPositionTest, RTLLigatureMouse)
{
    ...
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth / 4 - 1, false), 3);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth / 4 - 1, true), 4);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth / 4 + 1, false), 3);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth / 4 + 1, true), 3);

    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth / 2 - 1, false), 3);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth / 2 - 1, true), 3);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth / 2 + 1, false), 2);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth / 2 + 1, true), 3);

    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth * 3 / 4 - 1, false), 2);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth * 3 / 4 - 1, true), 3);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth * 3 / 4 + 1, false), 2);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth * 3 / 4 + 1, true), 2);
    ...
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth / 4 - 1, false), 1);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth / 4 - 1, true), 2);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth / 4 + 1, false), 1);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth / 4 + 1, true), 1);

    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth / 2 - 1, false), 1);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth / 2 - 1, true), 1);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth / 2 + 1, false), 0);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth / 2 + 1, true), 1);

    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth * 3 / 4 - 1, false), 0);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth * 3 / 4 - 1, true), 1);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth * 3 / 4 + 1, false), 0);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth * 3 / 4 + 1, true), 0);

    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth - 1, false), 0);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth - 1, true), 0);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth + 1, false), 0);
    EXPECT_EQ(GetCharacter(kMegalopolis, "ARUF", false, kFUWidth + kRAWidth + 1, true), 0);
}
...
TEST_F(CursorPositionTest, LTRLigature)
{
    ...
    EXPECT_NEAR(GetWidth(kMegalopolis, "FURA", true, 0, 3), kFUWidth + kRAWidth / 2, 1.0);
    EXPECT_NEAR(GetWidth(kMegalopolis, "FURA", true, 0, 4), kFUWidth + kRAWidth, 1.0);

    EXPECT_NEAR(GetWidth(kMegalopolis, "FURA", true, 1, 2), kFUWidth / 2, 1.0);
    EXPECT_NEAR(GetWidth(kMegalopolis, "FURA", true, 1, 3), kFUWidth / 2 + kRAWidth / 2, 1.0);
    EXPECT_NEAR(GetWidth(kMegalopolis, "FURA", true, 1, 4), kFUWidth / 2 + kRAWidth, 1.0);
    ...
    EXPECT_NEAR(GetWidth(kRoboto, u"ffî", true, 1, 3), kFFWidth / 2.0 + kIWidth, 1.0);
...
TEST_F(CursorPositionTest, RTLText)
{
    ...
    EXPECT_NEAR(GetWidth(kAmiri, u"تخ", false, 0, 2), kAboveKhaWidth + kAboveTaWidth, 1.0);
    ...
    EXPECT_NEAR(GetWidth(kAmiri, u"الخط", false, 0, 2), kAlifWidth + kLamWidth, 1.0);
    EXPECT_NEAR(GetWidth(kAmiri, u"الخط", false, 0, 3), kAlifWidth + kLamWidth + kKhaWidth, 1.0);
    EXPECT_NEAR(GetWidth(kAmiri, u"الخط", false, 0, 4), kAlifWidth + kLamWidth + kKhaWidth + kTaWidth, 1.0);
    EXPECT_NEAR(GetWidth(kAmiri, u"الخط", false, 1, 2), kLamWidth, 1.0);
    EXPECT_NEAR(GetWidth(kAmiri, u"الخط", false, 1, 3), kLamWidth + kKhaWidth, 1.0);
    EXPECT_NEAR(GetWidth(kAmiri, u"الخط", false, 1, 4), kLamWidth + kKhaWidth + kTaWidth, 1.0);
    EXPECT_NEAR(GetWidth(kAmiri, u"الخط", false, 2, 3), kKhaWidth, 1.0);
    EXPECT_NEAR(GetWidth(kAmiri, u"الخط", false, 2, 4), kKhaWidth + kTaWidth, 1.0);
...
TEST_F(CursorPositionTest, RTLLigature)
{
    ...
    EXPECT_NEAR(GetWidth(kMegalopolis, "ARUF", false, 0, 3), kRAWidth + kFUWidth / 2, 1.0);
    EXPECT_NEAR(GetWidth(kMegalopolis, "ARUF", false, 0, 4), kRAWidth + kFUWidth, 1.0);

    EXPECT_NEAR(GetWidth(kMegalopolis, "ARUF", false, 1, 2), kRAWidth / 2, 1.0);
    EXPECT_NEAR(GetWidth(kMegalopolis, "ARUF", false, 1, 3), kRAWidth / 2 + kFUWidth / 2, 1.0);
    EXPECT_NEAR(GetWidth(kMegalopolis, "ARUF", false, 1, 4), kRAWidth / 2 + kFUWidth, 1.0);
