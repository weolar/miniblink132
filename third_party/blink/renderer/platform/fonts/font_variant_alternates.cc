...
const ResolvedFontFeatures& FontVariantAlternates::GetResolvedFontFeatures() const
{
#if DCHECK_IS_ON()
    DCHECK(is_resolved_);
#endif
...
scoped_refptr<FontVariantAlternates> FontVariantAlternates::Clone(const FontVariantAlternates& other)
{
...
{
    return !stylistic_ && !historical_forms_ && !swash_ && !ornaments_ && !annotation_ && styleset_.empty() && character_variant_.empty();
}
...
void FontVariantAlternates::SetCharacterVariant(Vector<AtomicString> character_variant)
{
    character_variant_ = std::move(character_variant);
}

scoped_refptr<FontVariantAlternates> FontVariantAlternates::Resolve(ResolverFunction resolve_stylistic, ResolverFunction resolve_styleset,
    ResolverFunction resolve_character_variant, ResolverFunction resolve_swash, ResolverFunction resolve_ornaments, ResolverFunction resolve_annotation) const
{
    ...
    if (!character_variant_.empty()) {
        for (const AtomicString& character_variant_alias : character_variant_) {
            Vector<uint32_t> character_variant_resolved = resolve_character_variant(character_variant_alias);
            if (!character_variant_resolved.empty() && character_variant_resolved.size() <= 2) {
                ...
                if (character_variant_resolved[0] <= kMaxTag) {
                    auto pair = std::make_pair(cvTag(character_variant_resolved[0]), feature_value);
                    clone->resolved_features_.push_back(pair);
                }
            }
        }
...
unsigned FontVariantAlternates::GetHash() const
{
    ...
    WTF::AddIntToHash(computed_hash, stylistic_.has_value() ? WTF::GetHash(*stylistic_) : -1);
    WTF::AddIntToHash(computed_hash, historical_forms_);
    WTF::AddIntToHash(computed_hash, swash_.has_value() ? WTF::GetHash(*swash_) : -1);
    WTF::AddIntToHash(computed_hash, ornaments_.has_value() ? WTF::GetHash(*ornaments_) : -1);
    WTF::AddIntToHash(computed_hash, annotation_.has_value() ? WTF::GetHash(*annotation_) : -1);
    if (!styleset_.empty()) {
        for (const AtomicString& styleset_alias : styleset_) {
...
bool FontVariantAlternates::operator==(const FontVariantAlternates& other) const
{
    return stylistic_ == other.stylistic_ && historical_forms_ == other.historical_forms_ && styleset_ == other.styleset_
        && character_variant_ == other.character_variant_ && swash_ == other.swash_ && ornaments_ == other.ornaments_ && annotation_ == other.annotation_
