...
struct SmallCapsExpectedRun {
    ...
    SmallCapsExpectedRun(unsigned the_limit, SmallCapsIterator::SmallCapsBehavior the_small_caps_behavior)
        : limit(the_limit)
        , small_caps_behavior(the_small_caps_behavior)
    {
    }
};
...
protected:
    ...
    void VerifyRuns(SmallCapsIterator* small_caps_iterator, const Vector<SmallCapsExpectedRun>& expect)
    {
        unsigned limit;
        SmallCapsIterator::SmallCapsBehavior small_caps_behavior;
        size_t run_count = 0;
...
TEST_F(SmallCapsIteratorTest, Empty)
{
    ...
    unsigned limit = 0;
    SmallCapsIterator::SmallCapsBehavior small_caps_behavior = SmallCapsIterator::kSmallCapsInvalid;
...
{
    CHECK_SMALL_CAPS_RUN({ { "A", SmallCapsIterator::kSmallCapsSameCase }, { "a", SmallCapsIterator::kSmallCapsUppercaseNeeded } });
}
...
{
    CHECK_SMALL_CAPS_RUN({ { "aaa", SmallCapsIterator::kSmallCapsUppercaseNeeded }, { "===", SmallCapsIterator::kSmallCapsSameCase } });
}
...
{
    CHECK_SMALL_CAPS_RUN({ { "աբգդ", SmallCapsIterator::kSmallCapsUppercaseNeeded }, { "ԵԶԷԸ", SmallCapsIterator::kSmallCapsSameCase } });
}
