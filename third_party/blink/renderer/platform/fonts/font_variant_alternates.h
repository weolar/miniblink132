...
class PLATFORM_EXPORT FontVariantAlternates : public RefCounted<FontVariantAlternates> {
public:
    ...
    using ResolverFunction = base::FunctionRef<Vector<uint32_t>(const AtomicString&)>;
   ...
   * feature. */
    scoped_refptr<FontVariantAlternates> Resolve(ResolverFunction resolve_stylistic, ResolverFunction resolve_styleset_,
        ResolverFunction resolve_character_variant, ResolverFunction resolve_swash, ResolverFunction resolve_ornaments,
...
private:
    ...
    // CSSFontSelector.
    static scoped_refptr<FontVariantAlternates> Clone(const FontVariantAlternates& other);
