    ...
    for (auto weight : weights) {
        for (auto slope : slopes) {
            for (auto width : widths) {
                FontSelectionRequest request = FontSelectionRequest(FontSelectionValue(weight), FontSelectionValue(width), FontSelectionValue(slope));
...
{
    FontSelectionRequest request(FontSelectionValue(42), FontSelectionValue(43), FontSelectionValue(44));
    EXPECT_EQ("weight=42.000000, width=43.000000, slope=44.000000", request.ToString());
}
