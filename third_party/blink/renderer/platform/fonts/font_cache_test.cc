...
TEST_F(FontCacheTest, getLastResortFallbackFont)
{
    ...
    for (FontDescription::GenericFamilyType family_type :
        { FontDescription::kStandardFamily, FontDescription::kWebkitBodyFamily, FontDescription::kSansSerifFamily }) {
        FontDescription font_description;
        font_description.SetGenericFamily(family_type);
        const SimpleFontData* font_data = font_cache.GetLastResortFallbackFont(font_description);
...
TEST_F(FontCacheTest, NoFallbackForPrivateUseArea)
{
    ...
    // TODO(crbug.com/1065468): Remove the test for kWebkitBodyFamily when
    // -webkit-body in unshipped.
    for (FontDescription::GenericFamilyType family_type : { FontDescription::kStandardFamily, FontDescription::kWebkitBodyFamily }) {
        FontDescription font_description;
        font_description.SetGenericFamily(family_type);
        for (UChar32 character : { 0xE000, 0xE401, 0xE402, 0xE403, 0xF8FF, 0xF0000, 0xFAAAA, 0x100000, 0x10AAAA }) {
            const SimpleFontData* font_data = font_cache.FallbackFontForCharacter(font_description, character, nullptr);
...
TEST_F(FontCacheTest, FallbackForEmojis)
{
    ...
    for (FontDescription::GenericFamilyType family_type : { FontDescription::kStandardFamily, FontDescription::kWebkitBodyFamily }) {
        ...
        for (auto info : {
            ...
            {
                const SimpleFontData* font_data = font_cache.FallbackFontForCharacter(font_description, character, nullptr, FontFallbackPriority::kEmojiEmoji);
                EXPECT_EQ(font_data->PlatformData().FontFamilyName(), kNotoColorEmoji)
                    << "Character " << character_utf8 << " doesn't match what we expected for kEmojiEmoji.";
            }
            {
                const SimpleFontData* font_data = font_cache.FallbackFontForCharacter(font_description, character, nullptr, FontFallbackPriority::kEmojiText);
                if (available_in_contour_font) {
                    EXPECT_NE(font_data->PlatformData().FontFamilyName(), kNotoColorEmoji)
                        << "Character " << character_utf8 << " doesn't match what we expected for kEmojiText.";
                } else {
                    EXPECT_EQ(font_data->PlatformData().FontFamilyName(), kNotoColorEmoji)
                        << "Character " << character_utf8 << " doesn't match what we expected for kEmojiText.";
                }
...
TEST_F(FontCacheTest, firstAvailableOrFirst)
{
    ...
    EXPECT_EQ("Arial", FontCache::FirstAvailableOrFirst("not exist, Arial, not exist"));

    EXPECT_EQ("not exist", FontCache::FirstAvailableOrFirst("not exist, not exist 2"));

    EXPECT_EQ("Arial", FontCache::FirstAvailableOrFirst(", not exist, Arial"));
    EXPECT_EQ("not exist", FontCache::FirstAvailableOrFirst(", not exist, not exist"));
}
...
TEST_F(FontCacheTest, MAYBE_GetLargerThanMaxUnsignedFont)
{
    ...
    font_description.SetComputedSize(static_cast<float>(std::numeric_limits<unsigned>::max()) + 1.f);
    FontFaceCreationParams creation_params;
    const blink::SimpleFontData* font_data = font_cache.GetFontData(font_description, AtomicString());
    EXPECT_TRUE(font_data);
}
