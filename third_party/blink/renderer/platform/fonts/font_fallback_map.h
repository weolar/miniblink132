...
// reference to them.
class PLATFORM_EXPORT FontFallbackMap : public FontCacheClient, public FontSelectorClient {
public:
...
private:
    ...
    template <typename Predicate> void InvalidateInternal(Predicate predicate);

    const Member<FontSelector> font_selector_;
    HeapHashMap<FontDescription, WeakMember<FontFallbackList>> fallback_list_for_description_;
};
