...
TEST_F(FontCacheAndroidTest, FallbackFontForCharacter)
{
    ...
    for (FontDescription::GenericFamilyType family_type : { FontDescription::kStandardFamily, FontDescription::kWebkitBodyFamily }) {
        ...
        FontCache& font_cache = FontCache::Get();
        const SimpleFontData* font_data = font_cache.FallbackFontForCharacter(font_description, kTestChar, 0);
...
TEST_F(FontCacheAndroidTest, FallbackFontForCharacterSerif)
{
    ...
    const SimpleFontData* font_data = font_cache.FallbackFontForCharacter(font_description, kTestChar, nullptr);
    EXPECT_TRUE(font_data);
...
TEST_F(FontCacheAndroidTest, LocaleSpecificTypeface)
{
    ...
    for (FontDescription::GenericFamilyType family_type : { FontDescription::kStandardFamily, FontDescription::kWebkitBodyFamily }) {
        ...
        std::string name;
        FontCache& font_cache = FontCache::Get();
        sk_sp<SkTypeface> standard_ja_typeface = font_cache.CreateTypeface(standard_ja_description, FontFaceCreationParams(), name);
...
{
    EXPECT_EQ(CreateSerifTypeface(LayoutLocale::Get(AtomicString("en"))), nullptr);
...
TEST(FontCacheAndroid, GenericFamilyNameForScript)
{
    ...
    EXPECT_EQ(fallback, FontCache::GetGenericFamilyNameForScript(font_family_names::kWebkitStandard, fallback, english));
    EXPECT_EQ(font_family_names::kMonospace, FontCache::GetGenericFamilyNameForScript(font_family_names::kMonospace, fallback, english));

    // For CJK, getGenericFamilyNameForScript should return CJK fonts except
    // monospace.
    EXPECT_NE(fallback, FontCache::GetGenericFamilyNameForScript(font_family_names::kWebkitStandard, fallback, chinese));
    EXPECT_EQ(font_family_names::kMonospace, FontCache::GetGenericFamilyNameForScript(font_family_names::kMonospace, fallback, chinese));
}
