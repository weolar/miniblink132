...
{
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Android.FontLookup.Blink.DLFontsLatencyFailure2", delta, base::Microseconds(1), base::Seconds(10), 50);
}

void LogFontLatencySuccess(base::TimeDelta delta)
{
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Android.FontLookup.Blink.DLFontsLatencySuccess2", delta, base::Microseconds(1), base::Seconds(10), 50);
}
} // namespace

FontUniqueNameLookupAndroid::~FontUniqueNameLookupAndroid() = default;

void FontUniqueNameLookupAndroid::PrepareFontUniqueNameLookup(NotifyFontUniqueNameLookupReady callback)
{
    ...
    firmware_font_lookup_service_->GetUniqueNameLookupTable(
        WTF::BindOnce(&FontUniqueNameLookupAndroid::ReceiveReadOnlySharedMemoryRegion, WTF::Unretained(this)));
}
...
bool FontUniqueNameLookupAndroid::IsFontUniqueNameLookupReadyForSyncLookup()
{
    ...
    firmware_font_lookup_service_->GetUniqueNameLookupTableIfAvailable(&sync_available_from_mojo, &shared_memory_region);
...
sk_sp<SkTypeface> FontUniqueNameLookupAndroid::MatchUniqueName(const String& font_unique_name)
{
    if (!IsFontUniqueNameLookupReadyForSyncLookup())
        return nullptr;
    sk_sp<SkTypeface> result_font = MatchUniqueNameFromFirmwareFonts(font_unique_name);
    if (result_font)
        return result_font;
...
void FontUniqueNameLookupAndroid::Init()
{
    ...
    if (RuntimeEnabledFeatures::AndroidDownloadableFontsMatchingEnabled()) {
        ...
        if (android_font_lookup_service_) {
            ...
            android_font_lookup_service_->FetchAllFontFiles(WTF::BindOnce(&FontUniqueNameLookupAndroid::FontsPrefetched, WTF::Unretained(this)));
        }
    }
    if (base::FeatureList::IsEnabled(features::kPrefetchFontLookupTables) && RuntimeEnabledFeatures::FontSrcLocalMatchingEnabled()) {
...
{
    if (firmware_font_lookup_service_ && (!RuntimeEnabledFeatures::AndroidDownloadableFontsMatchingEnabled() || android_font_lookup_service_))
        return;

    if (!firmware_font_lookup_service_) {
        Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(firmware_font_lookup_service_.BindNewPipeAndPassReceiver());
    }

    if (RuntimeEnabledFeatures::AndroidDownloadableFontsMatchingEnabled() && !android_font_lookup_service_) {
        Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(android_font_lookup_service_.BindNewPipeAndPassReceiver());
    }
}

void FontUniqueNameLookupAndroid::ReceiveReadOnlySharedMemoryRegion(base::ReadOnlySharedMemoryRegion shared_memory_region)
{
    font_table_matcher_ = std::make_unique<FontTableMatcher>(shared_memory_region.Map());
...
sk_sp<SkTypeface> FontUniqueNameLookupAndroid::MatchUniqueNameFromFirmwareFonts(const String& font_unique_name)
{
    std::optional<FontTableMatcher::MatchResult> match_result = font_table_matcher_->MatchName(font_unique_name.Utf8().c_str());
    if (!match_result) {
        return nullptr;
    }
    sk_sp<SkFontMgr> mgr = skia::DefaultFontMgr();
    return mgr->makeFromFile(match_result->font_path.c_str(), match_result->ttc_index);
}

bool FontUniqueNameLookupAndroid::RequestedNameInQueryableFonts(const String& font_unique_name)
{
    if (!queryable_fonts_) {
    ...
    return queryable_fonts_ && queryable_fonts_->Contains(String::FromUTF8(IcuFoldCase(font_unique_name.Utf8())));
}

sk_sp<SkTypeface> FontUniqueNameLookupAndroid::MatchUniqueNameFromDownloadableFonts(const String& font_unique_name)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    base::File font_file;
    String case_folded_unique_font_name = String::FromUTF8(IcuFoldCase(font_unique_name.Utf8()));
    ...
    } else if (!android_font_lookup_service_->MatchLocalFontByUniqueName(case_folded_unique_font_name, &font_file)) {
        LOG(ERROR) << "Mojo method returned false for case-folded unique font name: " << case_folded_unique_font_name;
        LogFontLatencyFailure(elapsed_timer.Elapsed());
        return nullptr;
    }

    if (!font_file.IsValid()) {
        LOG(ERROR) << "Received platform font handle invalid, fd: " << font_file.GetPlatformFile();
        LogFontLatencyFailure(elapsed_timer.Elapsed());
        return nullptr;
    }
...
void FontUniqueNameLookupAndroid::FontsPrefetched(HashMap<String, base::File> font_files)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
