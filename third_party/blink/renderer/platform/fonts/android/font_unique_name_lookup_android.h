...
public:
    ...
    FontUniqueNameLookupAndroid& operator=(const FontUniqueNameLookupAndroid&) = delete;
    ~FontUniqueNameLookupAndroid() override;

    bool IsFontUniqueNameLookupReadyForSyncLookup() override;

    void PrepareFontUniqueNameLookup(NotifyFontUniqueNameLookupReady callback) override;
...
private:
    ...
    void ReceiveReadOnlySharedMemoryRegion(base::ReadOnlySharedMemoryRegion shared_memory_region);

    sk_sp<SkTypeface> MatchUniqueNameFromFirmwareFonts(const String& font_unique_name);

    bool RequestedNameInQueryableFonts(const String& font_unique_name);
    sk_sp<SkTypeface> MatchUniqueNameFromDownloadableFonts(const String& font_unique_name);

    void FontsPrefetched(HashMap<String, base::File> font_files);

    mojo::Remote<mojom::blink::FontUniqueNameLookup> firmware_font_lookup_service_;
