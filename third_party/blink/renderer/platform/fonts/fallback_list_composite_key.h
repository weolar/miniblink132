...
public:
        ...
        , auxiliary_bitmap_fields_(font_description.AuxiliaryBitmapFields())
    {
        ...
        while (current_family) {
            if (!current_family->FamilyName().empty()) {
                Add(CaseFoldingHash::GetHash(AdjustFamilyNameToAvoidUnsupportedFonts(current_family->FamilyName())));
    ...
    {
        return hash_ == other.hash_ && computed_size_ == other.computed_size_ && letter_spacing_ == other.letter_spacing_
            && word_spacing_ == other.word_spacing_ && bitmap_fields_ == other.bitmap_fields_ && auxiliary_bitmap_fields_ == other.auxiliary_bitmap_fields_
            && font_cache_keys_ == other.font_cache_keys_;
    }

    bool IsHashTableDeletedValue() const
    {
        return hash_ == kDeletedValueHash;
    }
