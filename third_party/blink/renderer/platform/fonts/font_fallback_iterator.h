...
public:
    ...
    FontFallbackIterator(const FontDescription&, FontFallbackList*, FontFallbackPriority);
...
private:
    bool RangeSetContributesForHint(const HintCharList& hint_list, const FontDataForRangeSet*);
    ...
    FontDataForRangeSet* UniqueOrNext(FontDataForRangeSet* candidate, const HintCharList& hint_list);

    const SimpleFontData* FallbackPriorityFont(UChar32 hint);
    const SimpleFontData* UniqueSystemFontForHintList(const HintCharList& hint_list);
    ...
    enum FallbackStage { kFallbackPriorityFonts, kFontGroupFonts, kSegmentedFace, kPreferencesFonts, kSystemFonts, kFirstCandidateForNotdefGlyph, kOutOfLuck };
