...
public:
    explicit MockFontFormatCheck(sk_sp<SkData> data)
        : FontFormatCheck(data)
    {
    }
...
{
    .make_system = expect_called, .make_fontations = expect_not_called,
...
{
    .make_system = expect_not_called, .make_fontations = expect_called,
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_APPLE)
    .make_fallback = expect_not_called,
#endif // BUILDFLAG(IS_WIN) || BUILDFLAG(IS_APPLE)
};
...
TEST(WebFontTypefaceFactoryTest, DefaultAlwaysSystem)
{
    ...
    WebFontTypefaceFactory::CreateTypeface(SkData::MakeEmpty(), out_typeface, mock_font_format_check, g_expect_system);
}

TEST(WebFontTypefaceFactoryTest, ColrV1AlwaysFallback)
{
    ScopedFontationsForSelectedFormatsForTest scoped_fontations_selected_formats(false);
    ScopedFontationsFontBackendForTest scoped_fontations(false);
    sk_sp<SkData> data = SkData::MakeEmpty();
    MockFontFormatCheck mock_font_format_check(data);
    EXPECT_CALL(mock_font_format_check, IsColrCpalColorFontV1()).Times(AtLeast(1)).WillRepeatedly(Return(true));
    sk_sp<SkTypeface> out_typeface;
    WebFontTypefaceFactory::CreateTypeface(SkData::MakeEmpty(), out_typeface, mock_font_format_check,
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_APPLE)
        g_expect_fallback
#else
        g_expect_system
...
{
    ScopedFontationsForSelectedFormatsForTest scoped_fontations_selected_formats(true);
    sk_sp<SkData> data = SkData::MakeEmpty();
    MockFontFormatCheck mock_font_format_check(data);
    EXPECT_CALL(mock_font_format_check, IsColrCpalColorFontV1()).Times(AtLeast(1)).WillRepeatedly(Return(true));
    sk_sp<SkTypeface> out_typeface;
    WebFontTypefaceFactory::CreateTypeface(SkData::MakeEmpty(), out_typeface, mock_font_format_check, g_expect_fontations);
}
...
TEST(WebFontTypefaceFactoryTest, Cff2AlwaysFallback)
{
    ...
    sk_sp<SkData> data = SkData::MakeEmpty();
    MockFontFormatCheck mock_font_format_check(data);
    EXPECT_CALL(mock_font_format_check, IsCff2OutlineFont()).Times(AtLeast(1)).WillRepeatedly(Return(true));
    sk_sp<SkTypeface> out_typeface;
    WebFontTypefaceFactory::CreateTypeface(SkData::MakeEmpty(), out_typeface, mock_font_format_check,
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_APPLE)
        g_expect_fallback
...
TEST(WebFontTypefaceFactoryTest, CbdtCblcAlwaysFallback)
{
    ...
    sk_sp<SkData> data = SkData::MakeEmpty();
    MockFontFormatCheck mock_font_format_check(data);
    EXPECT_CALL(mock_font_format_check, IsCbdtCblcColorFont()).Times(AtLeast(1)).WillRepeatedly(Return(true));
    sk_sp<SkTypeface> out_typeface;
    WebFontTypefaceFactory::CreateTypeface(SkData::MakeEmpty(), out_typeface, mock_font_format_check,
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_APPLE)
        g_expect_fallback
...
TEST(WebFontTypefaceFactoryTest, ColrV0FallbackApple)
{
    ...
    sk_sp<SkData> data = SkData::MakeEmpty();
    MockFontFormatCheck mock_font_format_check(data);
    EXPECT_CALL(mock_font_format_check, IsColrCpalColorFontV0()).Times(AtLeast(1)).WillRepeatedly(Return(true));
    sk_sp<SkTypeface> out_typeface;
    WebFontTypefaceFactory::CreateTypeface(SkData::MakeEmpty(), out_typeface, mock_font_format_check,
...
TEST(WebFontTypefaceFactoryTest, VariableColrV0FallbackWindowsApple)
{
    ...
    MockFontFormatCheck mock_font_format_check(data);
    EXPECT_CALL(mock_font_format_check, IsColrCpalColorFontV0()).Times(AtLeast(1)).WillRepeatedly(Return(true));
    EXPECT_CALL(mock_font_format_check, IsVariableFont()).Times(AtLeast(1)).WillRepeatedly(Return(true));
    sk_sp<SkTypeface> out_typeface;
#if BUILDFLAG(IS_WIN)
    const WebFontTypefaceFactory::FontInstantiator& expectation = DWriteVersionSupportsVariations() ? g_expect_system : g_expect_fallback;
#elif BUILDFLAG(IS_APPLE)
    const WebFontTypefaceFactory::FontInstantiator& expectation = g_expect_fallback;
#else
    const WebFontTypefaceFactory::FontInstantiator& expectation = g_expect_system;
#endif
    WebFontTypefaceFactory::CreateTypeface(SkData::MakeEmpty(), out_typeface, mock_font_format_check, expectation);
}
...
#if BUILDFLAG(IS_WIN)
    const WebFontTypefaceFactory::FontInstantiator& expectation = DWriteVersionSupportsVariations() ? g_expect_system : g_expect_fontations;
#else
    const WebFontTypefaceFactory::FontInstantiator& expectation = g_expect_fontations;
#endif
    WebFontTypefaceFactory::CreateTypeface(SkData::MakeEmpty(), out_typeface, mock_font_format_check, expectation);
}
...
TEST(WebFontTypefaceFactoryTest, MAYBE_SbixFallbackWindows)
{
    ...
    sk_sp<SkData> data = SkData::MakeEmpty();
    MockFontFormatCheck mock_font_format_check(data);
    EXPECT_CALL(mock_font_format_check, IsSbixColorFont()).Times(AtLeast(1)).WillRepeatedly(Return(true));
    sk_sp<SkTypeface> out_typeface;
    WebFontTypefaceFactory::CreateTypeface(SkData::MakeEmpty(), out_typeface, mock_font_format_check,
#if BUILDFLAG(IS_WIN)
        g_expect_fallback
...
#if BUILDFLAG(IS_IOS)
// TODO(crbug.com/1499557): Currently fails on the platform.
#define MAYBE_VariationsWinFallbackIfNeeded DISABLED_VariationsWinFallbackIfNeeded
#else
