...
SymbolsIterator::SymbolsIterator(base::span<const UChar> buffer)
{
    if (!buffer.empty()) {
        ...
        next_token_end_
            = cursor_ + (scan_emoji_presentation(buffer_iterator_, buffer_iterator_.end(), &next_token_emoji_, &next_token_has_vs_) - buffer_iterator_);
    }
}

bool SymbolsIterator::Consume(unsigned* symbols_limit, FontFallbackPriority* font_fallback_priority)
{
    ...
    do {
        ...
        if (!current_token_emoji && !Character::MaybeEmojiPresentation(buffer_iterator_.PeekCodepoint())) {
            ++buffer_iterator_;
        ...
        next_token_end_
            = cursor_ + (scan_emoji_presentation(buffer_iterator_, buffer_iterator_.end(), &next_token_emoji_, &next_token_has_vs_) - buffer_iterator_);
    } while (current_token_emoji == next_token_emoji_ && (!RuntimeEnabledFeatures::FontVariantEmojiEnabled() || curr_has_vs == next_token_has_vs_));

    if (RuntimeEnabledFeatures::FontVariantEmojiEnabled() && curr_has_vs) {
        *font_fallback_priority = current_token_emoji ? FontFallbackPriority::kEmojiEmojiWithVS : FontFallbackPriority::kEmojiTextWithVS;
    } else {
        *font_fallback_priority = current_token_emoji ? FontFallbackPriority::kEmojiEmoji : FontFallbackPriority::kText;
    }
