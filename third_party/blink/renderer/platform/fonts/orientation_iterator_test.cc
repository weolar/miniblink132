...
struct OrientationExpectedRun {
    ...
    OrientationExpectedRun(unsigned the_limit, OrientationIterator::RenderOrientation the_render_orientation)
        : limit(the_limit)
...
protected:
    void CheckRuns(const Vector<OrientationTestRun>& runs)
    {
        ...
        OrientationIterator orientation_iterator(text.Span16(), FontOrientation::kVerticalMixed);
        VerifyRuns(&orientation_iterator, expect);
    }

    void VerifyRuns(OrientationIterator* orientation_iterator, const Vector<OrientationExpectedRun>& expect)
    {
        unsigned limit;
        OrientationIterator::RenderOrientation render_orientation;
        size_t run_count = 0;
...
TEST_F(OrientationIteratorTest, Empty)
{
    ...
    OrientationIterator orientation_iterator(empty.Span16(), FontOrientation::kVerticalMixed);
    unsigned limit = 0;
    OrientationIterator::RenderOrientation orientation = OrientationIterator::kOrientationInvalid;
...
{
    CHECK_ORIENTATION({ { "いろは", OrientationIterator::kOrientationKeep }, { "ℐℒℐℒℐℒℐℒℐℒℐℒℐℒ", OrientationIterator::kOrientationRotateSideways } });
}

TEST_F(OrientationIteratorTest, LetterlikeJapaneseEnd)
{
    CHECK_ORIENTATION({ { "ℐ", OrientationIterator::kOrientationRotateSideways }, { "いろは", OrientationIterator::kOrientationKeep } });
}
...
{
    CHECK_ORIENTATION({ { "いろはにほへと", OrientationIterator::kOrientationKeep } });
}

TEST_F(OrientationIteratorTest, IVS)
{
    CHECK_ORIENTATION({ { "愉\xF3\xA0\x84\x81", OrientationIterator::kOrientationKeep } });
}
...
TEST_F(OrientationIteratorTest, MarkAtFirstCharRotated)
{
    ...
    CHECK_ORIENTATION({ { "\xCC\x80", OrientationIterator::kOrientationRotateSideways } });
}
...
TEST_F(OrientationIteratorTest, MarksAtFirstCharUpright)
{
    // U+20DD COMBINING ENCLOSING CIRCLE is Me (Mark, Enclosing) with Upright.
    // U+0300 COMBINING GRAVE ACCENT is Mn (Mark, Nonspacing) with Rotated.
    CHECK_ORIENTATION({ { "\xE2\x83\x9D\xCC\x80", OrientationIterator::kOrientationKeep } });
}
...
TEST_F(OrientationIteratorTest, MarksAtFirstCharUprightThenBase)
{
    ...
    // U+0300 COMBINING GRAVE ACCENT is Mn (Mark, Nonspacing) with Rotated.
    CHECK_ORIENTATION(
        { { "\xE2\x83\x9D\xCC\x80", OrientationIterator::kOrientationKeep }, { "ABC\xE2\x83\x9D", OrientationIterator::kOrientationRotateSideways } });
}

TEST_F(OrientationIteratorTest, JapaneseLatinMixedInside)
{
    CHECK_ORIENTATION({ { "いろはに", OrientationIterator::kOrientationKeep }, { "Abc", OrientationIterator::kOrientationRotateSideways },
        { "ほへと", OrientationIterator::kOrientationKeep } });
}

TEST_F(OrientationIteratorTest, PunctuationJapanese)
{
    CHECK_ORIENTATION({ { ".…¡", OrientationIterator::kOrientationRotateSideways }, { "ほへと", OrientationIterator::kOrientationKeep } });
}

TEST_F(OrientationIteratorTest, JapaneseLatinMixedOutside)
{
    CHECK_ORIENTATION({ { "Abc", OrientationIterator::kOrientationRotateSideways }, { "ほへと", OrientationIterator::kOrientationKeep },
        { "Xyz", OrientationIterator::kOrientationRotateSideways } });
}

TEST_F(OrientationIteratorTest, JapaneseMahjonggMixed)
{
    CHECK_ORIENTATION({ { "いろはに🀤ほへと", OrientationIterator::kOrientationKeep } });
}
