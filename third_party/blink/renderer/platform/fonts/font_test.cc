...
Font CreateVerticalUprightTestFont(const AtomicString& family_name, const String& font_path, float size)
{
    return CreateTestFont(family_name, font_path, size, /* ligatures */ nullptr, kNormalVariantEmoji,
        [](FontDescription* font_description) { font_description->SetOrientation(FontOrientation::kVerticalUpright); });
}
...
public:
    Font CreateFontWithOrientation(const Font& base_font, FontOrientation orientation)
    {
...
TEST_F(FontTest, FonteMetricsCapHeight)
{
    const auto cap_height_of = [](const char* font_path, float size) {
        Font font = CreateTestFont(AtomicString("test"), test::PlatformTestDataPath(font_path), size);
...
#if BUILDFLAG(IS_WIN)
    EXPECT_FLOAT_EQ(70.9961f, cap_height_of("third_party/Roboto/roboto-regular.woff2", 100));
    EXPECT_FLOAT_EQ(141.99219f, cap_height_of("third_party/Roboto/roboto-regular.woff2", 200));
#else
    EXPECT_FLOAT_EQ(71.09375f, cap_height_of("third_party/Roboto/roboto-regular.woff2", 100));
    EXPECT_FLOAT_EQ(142.1875f, cap_height_of("third_party/Roboto/roboto-regular.woff2", 200));
#endif
}
...
{
    Font font = CreateTestFont(AtomicString("Ahem"), test::PlatformTestDataPath("Ahem.woff"), 16);
    const SimpleFontData* font_data = font.PrimaryFont();
...
{
    Font font = CreateTestFont(AtomicString("M PLUS 1p"), blink::test::BlinkWebTestsFontsTestDataPath("mplus-1p-regular.woff"), 16);
    const SimpleFontData* font_data = font.PrimaryFont();
    ASSERT_TRUE(font_data);
...
{
    Font font = CreateTestFont(AtomicString("CSSHWOrientationTest"), blink::test::BlinkWebTestsFontsTestDataPath("adobe-fonts/CSSHWOrientationTest.otf"), 16);
    const SimpleFontData* font_data = font.PrimaryFont();
    ASSERT_TRUE(font_data);
    EXPECT_TRUE(font_data->IdeographicInlineSize().has_value());
...
{
    Font font = CreateVerticalUprightTestFont(AtomicString("M PLUS 1p"), blink::test::BlinkWebTestsFontsTestDataPath("mplus-1p-regular.woff"), 16);
    const SimpleFontData* font_data = font.PrimaryFont();
    ASSERT_TRUE(font_data);
    ...
    EXPECT_EQ(*font_data->IdeographicInlineSize(), font_data->GetFontMetrics().Height());
}
...
TEST_F(FontTest, IdeographicFullWidthUprightCjkVmtx)
{
    Font font = CreateVerticalUprightTestFont(
        AtomicString("CSSHWOrientationTest"), blink::test::BlinkWebTestsFontsTestDataPath("adobe-fonts/CSSHWOrientationTest.otf"), 16);
    const SimpleFontData* font_data = font.PrimaryFont();
    ASSERT_TRUE(font_data);
    ASSERT_TRUE(font_data->IdeographicInlineSize().has_value());
    EXPECT_EQ(*font_data->IdeographicInlineSize(), 16);
...
TEST_F(FontTest, TextIntercepts)
{
    ...
    UChar ahem_above_below_baseline_string[] = { 0xc9, 0x70, 0xc9, 0x70, 0xc9, 0x70, 0xc9, 0x70, 0xc9 };
    ...
    font.GetTextIntercepts(text_run_paint_info, default_paint, below_baseline_bounds, text_intercepts);
...
{
    Font font = CreateTestFont(AtomicString("Ahem"), test::PlatformTestDataPath("Ahem.woff"), 0);
    TabSize tab_size(8);
