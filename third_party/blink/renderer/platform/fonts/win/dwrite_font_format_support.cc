...
bool DWriteVersionSupportsVariationsImpl()
{
    ...
    sk_sp<SkTypeface> probe_typeface = fm->legacyMakeTypeface(nullptr, SkFontStyle());
    if (!probe_typeface) {
        return false;
    }
    int variation_design_position_result = probe_typeface->getVariationDesignPosition(nullptr, 0);
...
public:
    ...
    DWriteVersionSupportsVariationsChecker(const DWriteVersionSupportsVariationsChecker&) = delete;
    DWriteVersionSupportsVariationsChecker& operator=(const DWriteVersionSupportsVariationsChecker&) = delete;
...
bool DWriteVersionSupportsVariations()
{
    ...
    // once in each thread.
    DEFINE_THREAD_SAFE_STATIC_LOCAL(ThreadSpecific<DWriteVersionSupportsVariationsChecker>, variations_supported, ());
