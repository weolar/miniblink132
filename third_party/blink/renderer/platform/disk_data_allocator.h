...
public:
    ...
    std::unique_ptr<DiskDataMetadata> Write(std::unique_ptr<ReservedChunk> chunk, base::span<const uint8_t> data);
...
private:
    ...
    void ReleaseChunk(const DiskDataMetadata& metadata) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    // Virtual for testing.
    virtual std::optional<size_t> DoWrite(int64_t offset, base::span<const uint8_t> data) LOCKS_EXCLUDED(lock_);
...
protected: // For testing.
    base::Lock lock_;
    std::map<int64_t, size_t> free_chunks_ GUARDED_BY(lock_) ALLOW_DISCOURAGED_TYPE("We rely on |{lower,upper}_bound()|");
...
#if DCHECK_IS_ON()
    std::map<int64_t, size_t> allocated_chunks_ GUARDED_BY(lock_) ALLOW_DISCOURAGED_TYPE("We rely on |{lower,upper}_bound()|");
#endif
