...
PushPullFIFO::PushPullFIFO(unsigned number_of_channels, uint32_t fifo_length, unsigned render_quantum_frames)
...
PushPullFIFO::~PushPullFIFO()
{
    ...
    base::UmaHistogramPercentageObsoleteDoNotUse("WebAudio.PushPullFIFO.UnderflowPercentage", static_cast<int32_t>(100.0 * underflow_count_ / pull_count_));
    ...
    base::UmaHistogramBoolean("WebAudio.PushPullFIFO.UnderflowGlitches", underflow_count_ > 0);
}
...
{
    TRACE_EVENT2("webaudio", "PushPullFIFO::Push", "this", static_cast<void*>(this), "frames", input_bus->length());
    ...
    for (unsigned i = 0; i < fifo_bus_->NumberOfChannels(); ++i) {
        ...
        if (remainder >= input_bus_length) {
            ...
            memcpy(fifo_bus_channel + index_write_, input_bus_channel, input_bus_length * sizeof(*fifo_bus_channel));
        } else {
            // The input data overflows the remainder size. Wrap around the index.
            memcpy(fifo_bus_channel + index_write_, input_bus_channel, remainder * sizeof(*fifo_bus_channel));
            memcpy(fifo_bus_channel, input_bus_channel + remainder, (input_bus_length - remainder) * sizeof(*fifo_bus_channel));
        }
    }
    ...
    if (input_bus_length > fifo_length_ - frames_available_) {
        ...
        if (++overflow_count_ < kMaxMessagesToLog) {
            LOG(WARNING) << "PushPullFIFO: overflow while pushing ("
                         << "overflowCount=" << overflow_count_ << ", availableFrames=" << frames_available_ << ", inputFrames=" << input_bus_length
                         << ", fifoLength=" << fifo_length_ << ")";
        }
        TRACE_EVENT_INSTANT2("webaudio", "PushPullFIFO overrun", TRACE_EVENT_SCOPE_THREAD, "extra frames", input_bus_length + frames_available_ - fifo_length_,
            "overflow_count_", overflow_count_);
    }

    // Update the number of frames available in FIFO.
    frames_available_ = std::min(frames_available_ + input_bus_length, fifo_length_);
...
{
    TRACE_EVENT2("webaudio", "PushPullFIFO::Pull", "this", static_cast<void*>(this), "frames", frames_requested);
    ...
    if (!output_bus) {
        ...
        LOG(WARNING) << "[WebAudio/PushPullFIFO::pull <" << static_cast<void*>(this) << ">] |outputBus| is invalid.";
    ...
    if (frames_requested > output_bus->length()) {
        LOG(WARNING) << "[WebAudio/PushPullFIFO::pull <" << static_cast<void*>(this) << ">] framesRequested > outputBus->length() (" << frames_requested
                     << " > " << output_bus->length() << ")";
    }
    if (frames_requested > fifo_length_) {
        LOG(WARNING) << "[WebAudio/PushPullFIFO::pull <" << static_cast<void*>(this) << ">] framesRequested > fifo_length_ (" << frames_requested << " > "
                     << fifo_length_ << ")";
    }
    if (index_read_ >= fifo_length_) {
        LOG(WARNING) << "[WebAudio/PushPullFIFO::pull <" << static_cast<void*>(this) << ">] index_read_ >= fifo_length_ (" << index_read_
                     << " >= " << fifo_length_ << ")";
    }
#endif
    ...
    for (unsigned i = 0; i < fifo_bus_->NumberOfChannels(); ++i) {
        ...
        if (remainder >= frames_to_fill) {
            ...
            memcpy(output_bus_channel, fifo_bus_channel + index_read_, frames_to_fill * sizeof(*fifo_bus_channel));
        } else {
            // The frames to pull is bigger than the remainder size.
            // Wrap around the index.
            memcpy(output_bus_channel, fifo_bus_channel + index_read_, remainder * sizeof(*fifo_bus_channel));
            memcpy(output_bus_channel + remainder, fifo_bus_channel, (frames_to_fill - remainder) * sizeof(*fifo_bus_channel));
        }
        ...
        if (frames_requested > frames_to_fill) {
            memset(output_bus_channel + frames_to_fill, 0, (frames_requested - frames_to_fill) * sizeof(*output_bus_channel));
        }
    }
    ...
    if (frames_requested > frames_to_fill) {
        ...
        if (underflow_count_++ < kMaxMessagesToLog) {
            LOG(WARNING) << "PushPullFIFO: underflow while pulling ("
                         << "underflowCount=" << underflow_count_ << ", availableFrames=" << frames_available_ << ", requestedFrames=" << frames_requested
                         << ", fifoLength=" << fifo_length_ << ")";
        }
        TRACE_EVENT_INSTANT2("webaudio", "PushPullFIFO::Pull underrun", TRACE_EVENT_SCOPE_THREAD, "missing frames", frames_requested - frames_to_fill,
    ...
    return frames_requested > frames_available_ ? frames_requested - frames_available_ : 0;
}

PushPullFIFO::PullResult PushPullFIFO::PullAndUpdateEarmark(AudioBus* output_bus, uint32_t frames_requested)
{
    TRACE_EVENT2("webaudio", "PushPullFIFO::PullAndUpdateEarmark", "this", static_cast<void*>(this), "frames_requested", frames_requested);

    CHECK(output_bus);
    SECURITY_CHECK(frames_requested <= output_bus->length());

    base::AutoLock locker(lock_);
    TRACE_EVENT2("webaudio", "PushPullFIFO::PullAndUpdateEarmark (under lock)", "pull_count_", pull_count_, "earmark_frames_", earmark_frames_);
    ...
    if (frames_requested > frames_available_) {
        ...
        TRACE_EVENT_INSTANT2("webaudio", "PushPullFIFO::PullAndUpdateEarmark underrun", TRACE_EVENT_SCOPE_THREAD, "missing frames", missing_frames,
            "underflow_count_", underflow_count_);
        ...
        for (unsigned i = 0; i < fifo_bus_->NumberOfChannels(); ++i) {
            float* output_bus_channel = output_bus->Channel(i)->MutableData();
            memset(output_bus_channel, 0, frames_requested * sizeof(*output_bus_channel));
        }

        // No frames were pulled; the producer (WebAudio) needs to prepare the next
        // pull plus what's missing.
        return PullResult { .frames_provided = 0, .frames_to_render = frames_requested + missing_frames };
    }
    ...
    return PullResult { .frames_provided = frames_to_fill, .frames_to_render = earmark_frames_ > frames_available_ ? earmark_frames_ - frames_available_ : 0 };
}

const PushPullFIFOStateForTest PushPullFIFO::GetStateForTest()
{
    base::AutoLock locker(lock_);
    return { length(), NumberOfChannels(), frames_available_, index_read_, index_write_, overflow_count_, underflow_count_ };
}
