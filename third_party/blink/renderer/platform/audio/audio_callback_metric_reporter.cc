...
void AudioCallbackMetricReporter::Initialize(int callback_buffer_size, float sample_rate)
{
    DCHECK_GT(callback_buffer_size, 0);
    DCHECK_GT(sample_rate, 0);

    metric_.expected_callback_interval = callback_buffer_size / static_cast<double>(sample_rate);
    ...
    alpha_ = audio_utilities::DiscreteTimeConstantForSampleRate(time_constant_, 1.0 / metric_.expected_callback_interval);
}
...
void AudioCallbackMetricReporter::BeginTrace()
{
    ...
    if (metric_.number_of_callbacks == 0) {
        previous_callback_start_time_ = callback_start_time_ - base::Seconds(metric_.expected_callback_interval);
...
void AudioCallbackMetricReporter::UpdateMetric()
{
    ...
    callback_interval_ = (callback_start_time_ - previous_callback_start_time_).InSecondsF();
    render_duration_ = (previous_render_end_time_ - previous_callback_start_time_).InSecondsF();
    ...
    metric_.variance_callback_interval = (1 - alpha_) * (metric_.variance_callback_interval + diff * increment);
}
