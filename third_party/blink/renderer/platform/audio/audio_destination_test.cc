...
public:
    TestPlatform()
    {
        webaudio_device_ = std::make_unique<MockWebAudioDevice>(AudioHardwareSampleRate(), AudioHardwareBufferSize());
    }
    ...
    {
        CHECK(webaudio_device_ != nullptr) << "Calling CreateAudioDevice (via AudioDestination::Create) multiple "
    ...
    {
        CHECK(webaudio_device_ != nullptr) << "Finish setting up expectations before calling CreateAudioDevice "
...
public:
    void Render(AudioBus*, uint32_t frames_to_process, const AudioIOPosition&, const AudioCallbackMetric&, base::TimeDelta delay,
...
class AudioDestinationTest : public ::testing::TestWithParam<std::optional<float>> {
public:
    void CountWASamplesProcessedForRate(std::optional<float> sample_rate)
    {
        ...
        scoped_refptr<AudioDestination> destination
            = AudioDestination::Create(callback_, sink_descriptor, channel_count, latency_hint, sample_rate, render_quantum_frames);
        destination->Start();

        destination->Render(base::TimeDelta::Min(), base::TimeTicks::Now(), {}, media::AudioBus::Create(channel_count, request_frames).get());
        ...
        if (destination->SampleRate() != Platform::Current()->AudioHardwareSampleRate()) {
            exact_frames_required = std::ceil(request_frames * destination->SampleRate() / Platform::Current()->AudioHardwareSampleRate());
            ...
            exact_frames_required += media::SincResampler::KernelSizeFromRequestFrames(request_frames) / 2;
        }
        const int expected_frames_processed = std::ceil(exact_frames_required / static_cast<double>(render_quantum_frames)) * render_quantum_frames;
...
TEST_P(AudioDestinationTest, GlitchAndDelay)
{
    ...
    media::AudioGlitchInfo glitches[] { { .duration = base::Milliseconds(120), .count = 3 }, {}, { .duration = base::Milliseconds(20), .count = 1 } };

    base::TimeDelta delays[] { base::Milliseconds(100), base::Milliseconds(90), base::Milliseconds(80) };
    ...
    // delay.
    int priming_frames = ceil(request_frames / static_cast<float>(render_quantum_frames)) * render_quantum_frames;
    base::TimeDelta priming_delay = audio_utilities::FramesToTime(priming_frames, Platform::Current()->AudioHardwareSampleRate());
    ...
    for (int i = 0; i < kRenderCount; ++i) {
        destination->Render(delays[i], base::TimeTicks::Now(), glitches[i], audio_bus.get());
        ...
        if (destination->SampleRate() != Platform::Current()->AudioHardwareSampleRate()) {
            ...
            EXPECT_LE(callback_.last_latency_, delays[i] + base::Milliseconds(1) + priming_delay);
        } else {
...
INSTANTIATE_TEST_SUITE_P(/* no label */, AudioDestinationTest, ::testing::Values(std::optional<float>(), 8000, 24000, 44100, 48000, 384000));
