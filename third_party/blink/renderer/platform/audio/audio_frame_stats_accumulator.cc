...
void AudioFrameStatsAccumulator::Update(size_t observed_frames, int sample_rate, base::TimeDelta latency, const media::AudioGlitchInfo& glitch_info)
{
    observed_frames_ += observed_frames;
    observed_frames_duration_ += media::AudioTimestampHelper::FramesToTime(base::saturated_cast<int64_t>(observed_frames), sample_rate);
    glitch_frames_ += media::AudioTimestampHelper::TimeToFrames(glitch_info.duration, sample_rate);
...
void AudioFrameStatsAccumulator::Absorb(AudioFrameStatsAccumulator& from)
{
    ...
    // high as |this|.
    CHECK_GE(static_cast<uint64_t>(from.observed_frames_), static_cast<uint64_t>(observed_frames_));
    CHECK_GE(from.observed_frames_duration_, observed_frames_duration_);
    CHECK_GE(static_cast<uint64_t>(from.glitch_frames_), static_cast<uint64_t>(glitch_frames_));
    ...
    MergeLatencyExtremes(from.interval_minimum_latency_, from.interval_maximum_latency_);
...
void AudioFrameStatsAccumulator::MergeLatencyExtremes(base::TimeDelta new_minumum, base::TimeDelta new_maximum)
{
    // If we already have latency stats, we need to merge them.
    if (interval_frames_ > 0) {
        interval_minimum_latency_ = std::min(interval_minimum_latency_, new_minumum);
        interval_maximum_latency_ = std::max(interval_maximum_latency_, new_maximum);
    } else {
