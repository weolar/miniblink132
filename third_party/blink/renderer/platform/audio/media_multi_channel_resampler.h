...
class PLATFORM_EXPORT MediaMultiChannelResampler {
    ...
    typedef WTF::CrossThreadRepeatingFunction<void(int frame_delay, blink::AudioBus*)> ReadCB;
...
public:
    ...
    MediaMultiChannelResampler(int channels, double io_sample_rate_ratio, uint32_t request_frames, ReadCB read_cb);

    MediaMultiChannelResampler(const MediaMultiChannelResampler&) = delete;
    MediaMultiChannelResampler& operator=(const MediaMultiChannelResampler&) = delete;
...
private:
    ...
    void ProvideResamplerInput(int resampler_frame_delay, media::AudioBus* resampler_output_bus);
