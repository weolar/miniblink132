...
public:
    SimpleFFTConvolver(unsigned input_block_size, const std::unique_ptr<AudioFloatArray>& convolution_kernel);
    ...
    uint32_t ConvolutionKernelSize() const
    {
        return convolution_kernel_size_;
    }

private:
    unsigned FftSize() const
    {
        return frame_.FftSize();
    }
