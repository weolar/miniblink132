...
SimpleFFTConvolver::SimpleFFTConvolver(unsigned input_block_size, const std::unique_ptr<AudioFloatArray>& convolution_kernel)
    : convolution_kernel_size_(convolution_kernel->size())
    , fft_kernel_(2 * input_block_size)
    , frame_(2 * input_block_size)
    , input_buffer_(2 * input_block_size)
...
void SimpleFFTConvolver::Process(const float* source_p, float* dest_p, uint32_t frames_to_process)
{
    ...
    // to destination.
    vector_math::Vadd(output_buffer_.Data(), 1, last_overlap_buffer_.Data(), 1, dest_p, 1, half_size);

    // Finally, save 2nd half for the next time.
    last_overlap_buffer_.CopyToRange(output_buffer_.Data() + half_size, 0, half_size);
}
