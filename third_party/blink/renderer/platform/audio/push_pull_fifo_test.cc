...
TEST(PushPullFIFOBasicTest, BasicTests)
{
    ...
    std::unique_ptr<PushPullFIFO> test_fifo = std::make_unique<PushPullFIFO>(2, 1024);

    // The input bus length must be |audio_utilities::kRenderQuantumFrames|.
    // i.e.) input_bus->length() == kRenderQuantumFrames
    scoped_refptr<AudioBus> input_bus_129_frames = AudioBus::Create(2, kRenderQuantumFrames + 1);
    EXPECT_DEATH_IF_SUPPORTED(test_fifo->Push(input_bus_129_frames.get()), "");
    scoped_refptr<AudioBus> input_bus_127_frames = AudioBus::Create(2, kRenderQuantumFrames - 1);
...
// channels.
bool VerifyBusValueAtIndex(AudioBus* target_bus, int index, float expected_value)
{
    for (unsigned c = 0; c < target_bus->NumberOfChannels(); ++c) {
        float* bus_channel = target_bus->Channel(c)->MutableData();
        if (bus_channel[index] != expected_value) {
            LOG(ERROR) << ">> [FAIL] expected " << expected_value << " at index " << index << " but got " << bus_channel[index] << ".";
            return false;
        }
    }
    return true;
...
{
    out << "fifoLength=" << param.setup.fifo_length << " numberOfChannels=" << param.setup.number_of_channels;
    return out;
}
...
TEST_P(PushPullFIFOFeatureTest, FeatureTests)
{
    ...
    std::unique_ptr<PushPullFIFO> fifo = std::make_unique<PushPullFIFO>(setup.number_of_channels, setup.fifo_length);
    ...
    for (const auto& action : setup.fifo_actions) {
        if (strcmp(action.action, "PUSH") == 0) {
            scoped_refptr<AudioBus> input_bus = AudioBus::Create(setup.number_of_channels, action.number_of_frames);
            frame_counter = FillBusWithLinearRamp(input_bus.get(), frame_counter);
            fifo->Push(input_bus.get());
            LOG(INFO) << "PUSH " << action.number_of_frames << " frames (frameCounter=" << frame_counter << ")";
        } else {
            output_bus = AudioBus::Create(setup.number_of_channels, action.number_of_frames);
    ...
    for (const auto& sample : expected_state.fifo_samples) {
        EXPECT_TRUE(VerifyBusValueAtIndex(fifo->GetFIFOBusForTest(), sample.index, sample.value));
    }

    // Verify samples from the most recent output bus.
    for (const auto& sample : expected_state.output_samples) {
        EXPECT_TRUE(VerifyBusValueAtIndex(output_bus.get(), sample.index, sample.value));
    }
}
...
FIFOTestParam g_feature_test_params[] = {
    ...
    { { 512, 1, { { "PUSH", 128 }, { "PUSH", 128 }, { "PULL", 256 } } }, { 256, 256, 0, 0, { { 0, 0 } }, { { 0, 0 }, { 255, 255 } } } },

    { { 512, 2, { { "PUSH", 128 }, { "PUSH", 128 }, { "PULL", 256 } } }, { 256, 256, 0, 0, { { 0, 0 } }, { { 0, 0 }, { 255, 255 } } } },

    { { 512, 4, { { "PUSH", 128 }, { "PUSH", 128 }, { "PULL", 256 } } }, { 256, 256, 0, 0, { { 0, 0 } }, { { 0, 0 }, { 255, 255 } } } },

    { { 512, 6, { { "PUSH", 128 }, { "PUSH", 128 }, { "PULL", 256 } } }, { 256, 256, 0, 0, { { 0, 0 } }, { { 0, 0 }, { 255, 255 } } } },
    ...
    { { 512, 7,
        ...
        //                                               (256, 0), (383, 0)
        { 128, 128, 0, 1, { { 128, 128 } }, { { 0, 384 }, { 255, 639 }, { 256, 0 }, { 383, 0 } } } },
    ...
    { { 523, 11,
        ...
        //                                               (257, 0), (382, 0)
        { 117, 117, 0, 1, { { 117, 117 } }, { { 0, 383 }, { 256, 639 }, { 257, 0 }, { 382, 0 } } } },
...
INSTANTIATE_TEST_SUITE_P(PushPullFIFOFeatureTest, PushPullFIFOFeatureTest, testing::ValuesIn(g_feature_test_params));
...
class PushPullFIFOEarmarkFramesTest : public testing::TestWithParam<FIFOEarmarkTestParam> { };
...
FIFOEarmarkTestParam g_earmark_test_params[] = {
    ...
    { { 8192, 2, { { "PUSH", 128 }, { "PUSH", 128 }, { "PULL_EARMARK", 256 }, { "PUSH", 128 }, { "PUSH", 128 }, { "PULL_EARMARK", 256 } } }, 256, 256 },
    // The first underrun increases the earmark by the callback size.
    { { 8192, 2,
          {
              { "PUSH", 128 }, { "PUSH", 128 }, { "PULL_EARMARK", 384 }, // udnerrun; updating earmark and skipping pull.
              { "PUSH", 128 }, { "PUSH", 128 }, { "PUSH", 128 }, { "PULL_EARMARK", 384 } // OK
          } },
        384, 768 },
    // Simulating "bursty and irregular" callbacks.
    { { 8192, 2,
          {
              { "PUSH", 128 }, { "PUSH", 128 }, { "PUSH", 128 }, { "PUSH", 128 }, { "PULL_EARMARK", 480 }, // OK
              { "PUSH", 128 }, { "PUSH", 128 }, { "PULL_EARMARK", 480 }, // underrun; updating earmark and skipping pull.
              { "PUSH", 128 }, { "PUSH", 128 }, { "PUSH", 128 }, { "PULL_EARMARK", 480 }, // OK
              { "PUSH", 128 }, { "PULL_EARMARK", 480 } // underrun; updating earmark and skipping pull.
          } },
        480, 1440 }
};

INSTANTIATE_TEST_SUITE_P(PushPullFIFOEarmarkFramesTest, PushPullFIFOEarmarkFramesTest, testing::ValuesIn(g_earmark_test_params));
