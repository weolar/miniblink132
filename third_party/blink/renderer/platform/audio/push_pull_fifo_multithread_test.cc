...
public:
    FIFOClient(PushPullFIFO* fifo, size_t bus_length, size_t jitter_range_ms)
        : fifo_(fifo)
        , bus_(AudioBus::Create(fifo->GetStateForTest().number_of_channels, bus_length))
        , client_thread_(NonMainThread::CreateThread(ThreadCreationParams(ThreadType::kTestThread).SetThreadNameForTest("FIFOClientThread")))
        , done_event_(std::make_unique<base::WaitableEvent>(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED))
    ...
    base::WaitableEvent* Start(double duration_ms, double interval_ms)
    {
        ...
        PostCrossThreadTask(*client_thread_->GetTaskRunner(), FROM_HERE, CrossThreadBindOnce(&FIFOClient::RunTaskOnOwnThread, CrossThreadUnretained(this)));
...
private:
    ...
    {
        double interval_with_jitter = interval_ms_ + (static_cast<double>(std::rand()) / RAND_MAX) * jitter_range_ms_;
        elapsed_ms_ += interval_with_jitter;
        ++counter_;
        RunTask();
        if (elapsed_ms_ < duration_ms_) {
            PostDelayedCrossThreadTask(*client_thread_->GetTaskRunner(), FROM_HERE,
                CrossThreadBindOnce(&FIFOClient::RunTaskOnOwnThread, CrossThreadUnretained(this)), base::Milliseconds(interval_with_jitter));
        } else {
            Stop(counter_);
            done_event_->Signal();
        }
...
class PushPullFIFOSmokeTest : public testing::TestWithParam<FIFOSmokeTestParam> { };
...
TEST_P(PushPullFIFOSmokeTest, SmokeTests)
{
    ...
    std::unique_ptr<PushPullFIFO> test_fifo = std::make_unique<PushPullFIFO>(param.number_of_channels, param.fifo_length);
    std::unique_ptr<PullClient> pull_client = std::make_unique<PullClient>(test_fifo.get(), param.pull_buffer_size, param.pull_jitter_range_ms);
    std::unique_ptr<PushClient> push_client = std::make_unique<PushClient>(test_fifo.get(), param.push_buffer_size, param.push_jitter_range_ms);

    Vector<base::WaitableEvent*> done_events;
    done_events.push_back(pull_client->Start(param.test_duration_ms, pull_interval_ms));
    done_events.push_back(push_client->Start(param.test_duration_ms, push_interval_ms));
...
INSTANTIATE_TEST_SUITE_P(PushPullFIFOSmokeTest, PushPullFIFOSmokeTest, testing::ValuesIn(smoke_test_params));
