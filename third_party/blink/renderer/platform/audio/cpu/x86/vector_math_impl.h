...
void PrepareFilterForConv(const float* filter_p, int filter_stride, size_t filter_size, AudioFloatArray* prepared_filter)
{
    ...
    prepared_filter->Allocate(kReversedFilterStride * kPackedFloatsPerRegister * filter_size);
...
void Conv(const float* source_p, const float* prepared_filter_p, float* dest_p, uint32_t frames_to_process, size_t filter_size)
{
    ...
    const MType* reversed_filter = reinterpret_cast<const MType*>(prepared_filter_p);
...
void Vadd(const float* source1p, const float* source2p, float* dest_p, uint32_t frames_to_process)
{
...
// dest[k] = source1[k] - source2[k]
void Vsub(const float* source1p, const float* source2p, float* dest_p, uint32_t frames_to_process)
{
    const float* const source1_end_p = source1p + frames_to_process;
...
void Vclip(const float* source_p, const float* low_threshold_p, const float* high_threshold_p, float* dest_p, uint32_t frames_to_process)
{
...
#define CLIP_ALL(storeDest)                                                                                                                                    \
    while (source_p < source_end_p) {                                                                                                                          \
        ...
        MType m_dest = MM_PS(max)(m_low_threshold, MM_PS(min)(m_high_threshold, m_source));                                                                    \
        MM_PS(storeDest)(dest_p, m_dest);                                                                                                                      \
        source_p += kPackedFloatsPerRegister;                                                                                                                  \
        dest_p += kPackedFloatsPerRegister;                                                                                                                    \
    }
...
// dest[k] = source1[k] * source2[k]
void Vmul(const float* source1p, const float* source2p, float* dest_p, uint32_t frames_to_process)
{
    const float* const source1_end_p = source1p + frames_to_process;
...
void Vsma(const float* source_p, const float* scale, float* dest_p, uint32_t frames_to_process)
{
    const float* const source_end_p = source_p + frames_to_process;
...
// dest[k] = scale * source[k]
void Vsmul(const float* source_p, const float* scale, float* dest_p, uint32_t frames_to_process)
{
    const float* const source_end_p = source_p + frames_to_process;
...
void Vsadd(const float* source_p, const float* addend, float* dest_p, uint32_t frames_to_process)
{
    const float* const source_end_p = source_p + frames_to_process;
...
#define MULTIPLY_ALL(loadOtherThanReal1, storeDest)                                                                                                            \
    ...
    if (IsAligned(imag1p) && IsAligned(real2p) && IsAligned(imag2p) && IsAligned(real_dest_p) && IsAligned(imag_dest_p)) {
        MULTIPLY_ALL(load, store);
    } else {
        MULTIPLY_ALL(loadu, storeu);
    }
