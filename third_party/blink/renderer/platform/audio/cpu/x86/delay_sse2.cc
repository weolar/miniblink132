...
ALWAYS_INLINE static __m128i WrapIndexVector(__m128i v_write_index, __m128i v_buffer_length)
{
...
ALWAYS_INLINE static __m128 WrapPositionVector(__m128 v_position, __m128 v_buffer_length)
{
...
std::tuple<unsigned, int> Delay::ProcessARateVector(float* destination, uint32_t frames_to_process) const
{
    ...
    __m128i v_write_index = _mm_set_epi32(w_index + 3, w_index + 2, w_index + 1, w_index + 0);
    ...
    for (int n = 0; n < number_of_loops; ++n, k += 4) {
        ...
        const __m128 v_delay_time = _mm_max_ps(_mm_loadu_ps(delay_times + k), v_all_zeros);
        ...
        __m128 v_read_position = _mm_add_ps(_mm_cvtepi32_ps(v_write_index), _mm_sub_ps(v_buffer_length_float, v_desired_delay_frames));
        ...
        const __m128i v_read_index1 = WrapIndexVector(_mm_cvttps_epi32(v_read_position), v_buffer_length_int);
        const __m128i v_read_index2 = WrapIndexVector(_mm_add_epi32(v_read_index1, _mm_set1_epi32(1)), v_buffer_length_int);

        const __m128 interpolation_factor = _mm_sub_ps(v_read_position, _mm_cvtepi32_ps(v_read_index1));

        const uint32_t* read_index1 = reinterpret_cast<const uint32_t*>(&v_read_index1);
        const uint32_t* read_index2 = reinterpret_cast<const uint32_t*>(&v_read_index2);
        ...
        const __m128 sample = _mm_add_ps(v_sample1, _mm_mul_ps(interpolation_factor, _mm_sub_ps(v_sample2, v_sample1)));
