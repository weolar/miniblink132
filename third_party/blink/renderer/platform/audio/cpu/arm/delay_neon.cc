...
ALWAYS_INLINE static int32x4_t WrapIndexVector(int32x4_t v_write_index, int32x4_t v_buffer_length)
{
    ...
    int32x4_t cmp = reinterpret_cast<int32x4_t>(vcgeq_s32(v_write_index, v_buffer_length));
...
ALWAYS_INLINE static float32x4_t WrapPositionVector(float32x4_t v_position, float32x4_t v_buffer_length)
{
    ...
    return vsubq_f32(v_position, reinterpret_cast<float32x4_t>(vandq_u32(reinterpret_cast<uint32x4_t>(v_buffer_length), cmp)));
}
...
std::tuple<unsigned, int> Delay::ProcessARateVector(float* destination, uint32_t frames_to_process) const
{
    ...
    int32x4_t v_write_index = { w_index + 0, w_index + 1, w_index + 2, w_index + 3 };
    ...
    for (int n = 0; n < number_of_loops; ++n, k += 4) {
        const float32x4_t v_delay_time = vmaxq_f32(vld1q_f32(delay_times + k), v_all_zeros);
        ...
        float32x4_t v_read_position = vaddq_f32(vcvtq_f32_s32(v_write_index), vsubq_f32(v_buffer_length_float, v_desired_delay_frames));
        v_read_position = WrapPositionVector(v_read_position, v_buffer_length_float);

        // Get indices into the buffer for the samples we need for interpolation.
        const int32x4_t v_read_index1 = WrapIndexVector(vcvtq_s32_f32(v_read_position), v_buffer_length_int);
        const int32x4_t v_read_index2 = WrapIndexVector(vaddq_s32(v_read_index1, vdupq_n_s32(1)), v_buffer_length_int);

        const float32x4_t interpolation_factor = vsubq_f32(v_read_position, vcvtq_f32_s32(v_read_index1));
        ...
        const float32x4_t sample = vaddq_f32(v_sample1, vmulq_f32(interpolation_factor, vsubq_f32(v_sample2, v_sample1)));
