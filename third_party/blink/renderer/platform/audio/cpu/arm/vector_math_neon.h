    ...
    const float* source1p, int source_stride1, const float* source2p, int source_stride2, float* dest_p, int dest_stride, uint32_t frames_to_process)
{
    ...
    scalar::Vsub(source1p, source_stride1, source2p, source_stride2, dest_p, dest_stride, n);
}
...
ALWAYS_INLINE static void Vmaxmgv(const float* source_p, int source_stride, float* max_p, uint32_t frames_to_process)
{
    ...
    if (source_stride == 1) {
        ...
        float32x2_t two_max = vmax_f32(vget_low_f32(four_max), vget_high_f32(four_max));
    ...
    const float* real1p, const float* imag1p, const float* real2p, const float* imag2p, float* real_dest_p, float* imag_dest_p, uint32_t frames_to_process)
{
    ...
    scalar::Zvmul(real1p + i, imag1p + i, real2p + i, imag2p + i, real_dest_p + i, imag_dest_p + i, frames_to_process - i);
}
