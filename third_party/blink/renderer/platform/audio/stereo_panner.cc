...
void StereoPanner::PanWithSampleAccurateValues(const AudioBus* input_bus, AudioBus* output_bus, const float* pan_values, uint32_t frames_to_process)
{
    DCHECK(input_bus);
...
void StereoPanner::PanToTargetValue(const AudioBus* input_bus, AudioBus* output_bus, float pan_value, uint32_t frames_to_process)
{
    DCHECK(input_bus);
    DCHECK_LE(frames_to_process, input_bus->length());
    DCHECK_GE(input_bus->NumberOfChannels(), 1u);
    ...
    if (number_of_input_channels == 1) { // For mono source case.
        ...
        // do nothing when [0; 1].
        double pan_radian = (target_pan <= 0 ? target_pan + 1 : target_pan) * kPiOverTwoDouble;
