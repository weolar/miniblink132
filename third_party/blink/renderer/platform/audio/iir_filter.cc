...
void IIRFilter::Process(const float* source_p, float* dest_p, uint32_t frames_to_process)
{
    // Compute
    //
...
void IIRFilter::GetFrequencyResponse(int n_frequencies, const float* frequency, float* mag_response, float* phase_response)
{
    ...
    for (int k = 0; k < n_frequencies; ++k) {
        if (frequency[k] < 0 || frequency[k] > 1) {
            ...
            std::complex<double> z_recip = std::complex<double>(fdlibm::cos(omega), fdlibm::sin(omega));

            std::complex<double> numerator = EvaluatePolynomial(feedforward_->Data(), z_recip, feedforward_->size() - 1);
            std::complex<double> denominator = EvaluatePolynomial(feedback_->Data(), z_recip, feedback_->size() - 1);
...
double IIRFilter::TailTime(double sample_rate, bool is_filter_stable, unsigned render_quantum_frames)
{
    ...
    int number_of_blocks = std::ceil(sample_rate * kMaxTailTime / render_quantum_frames);
    ...
    for (int k = 1; k < number_of_blocks; ++k) {
        Process(input.Data(), output.Data(), render_quantum_frames);
        vector_math::Vmaxmgv(output.Data(), 1, &magnitudes[k], render_quantum_frames);
    }
