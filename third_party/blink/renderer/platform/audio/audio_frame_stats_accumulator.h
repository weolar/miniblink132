...
public:
    ...
    AudioFrameStatsAccumulator& operator=(const AudioFrameStatsAccumulator&) = delete;
    ~AudioFrameStatsAccumulator() = default;

    // Updates the stats with information from a new buffer.
    void Update(size_t observed_frames, int sample_rate, base::TimeDelta latency, const media::AudioGlitchInfo& glitch_info);
    ...
    uint64_t observed_frames() const
    {
        return observed_frames_;
    }
    ...
    uint64_t glitch_frames() const
    {
        return glitch_frames_;
    }
    ...
    size_t glitch_event_count() const
    {
        return glitch_event_count_;
    }
    ...
    {
        return interval_frames_ > 0 ? interval_frames_latency_sum_ / interval_frames_ : last_latency_;
    }

    base::TimeDelta min_latency() const
    {
        return interval_minimum_latency_;
    }

    base::TimeDelta max_latency() const
    {
...
private:
    ...
    // extremes.
    void MergeLatencyExtremes(base::TimeDelta new_minumum, base::TimeDelta new_maximum);
};
