...
std::string GetResourceData(const HeapVector<Member<ArchiveResource>>& resources, size_t index)
{
...
public:
    ...
    HeapVector<Member<ArchiveResource>> ParseArchive(base::span<const char> mhtml_data)
    {
...
{
    const char mhtml_data[] = "From: <Saved by Blink>\r\n"
    ...
    HeapVector<Member<ArchiveResource>> resources = ParseArchive(base::span_with_nul_from_cstring(mhtml_data));
...
TEST_F(MHTMLParserTest, Base64ContentTransferEncoding)
{
    ...
    EXPECT_EQ(std::string("123abc\x01\x02\x03\x0D\x0E\x0F", 12), GetResourceData(resources, 1));
}
...
TEST_F(MHTMLParserTest, DateParsing_EmptyDate)
{
    ...
    base::Time creation_time = ParseArchiveTime(base::span_with_nul_from_cstring(mhtml_data));
...
TEST_F(MHTMLParserTest, DateParsing_ValidDate)
{
    ...
    ASSERT_TRUE(base::Time::FromUTCExploded({ 2017, 3 /* March */, 5 /* Friday */, 1, 22, 44, 17, 0 }, &expected_time));
