...
{
    const std::string hex_str = base::StringPrintf("%016" PRIx64 "%016" PRIx64, token.high(), token.low());
    const std::string_view hex_string_piece(hex_str);
    const std::string lowercase = base::StrCat({ hex_string_piece.substr(0, 8), "-", hex_string_piece.substr(8, 4), "-", hex_string_piece.substr(12, 4), "-",
