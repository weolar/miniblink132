...
TEST(JSONParserTest, Reading)
{
    ...
    root = ParseJSONWithCommentsDeprecated("/* comment */null", &error, &has_comments);
    EXPECT_EQ(has_comments, true);
    ASSERT_TRUE(root.get());
    EXPECT_EQ(JSONValue::kTypeNull, root->GetType());
    root = ParseJSONWithCommentsDeprecated("40 /* comment */", &error, &has_comments);
    EXPECT_EQ(has_comments, true);
    ASSERT_TRUE(root.get());
    EXPECT_EQ(JSONValue::kTypeInteger, root->GetType());
    EXPECT_TRUE(root->AsInteger(&int_val));
    EXPECT_EQ(40, int_val);
    root = ParseJSONWithCommentsDeprecated("/**/ 40 /* multi-line\n comment */ // more comment", &error, &has_comments);
    EXPECT_EQ(has_comments, true);
    ASSERT_TRUE(root.get());
    EXPECT_EQ(JSONValue::kTypeInteger, root->GetType());
    EXPECT_TRUE(root->AsInteger(&int_val));
    EXPECT_EQ(40, int_val);
    root = ParseJSONWithCommentsDeprecated("true // comment", &error, &has_comments);
    EXPECT_EQ(has_comments, true);
    ASSERT_TRUE(root.get());
    EXPECT_EQ(JSONValue::kTypeBoolean, root->GetType());
    root = ParseJSONWithCommentsDeprecated("/* comment */\"sample string\"", &error, &has_comments);
    EXPECT_EQ(has_comments, true);
    ASSERT_TRUE(root.get());
    ...
    EXPECT_TRUE(tmp_value->AsInteger(&int_val));
    EXPECT_EQ(3, int_val);
    root = ParseJSONWithCommentsDeprecated("[1, /*a*/2, 3]", &error, &has_comments);
    EXPECT_EQ(has_comments, true);
    ASSERT_TRUE(root.get());
    list = JSONArray::Cast(root.get());
    ASSERT_TRUE(list);
    EXPECT_EQ(3u, list->size());
    root = ParseJSONWithCommentsDeprecated("/* comment **/42", &error, &has_comments);
    EXPECT_EQ(has_comments, true);
    ASSERT_TRUE(root.get());
    EXPECT_EQ(JSONValue::kTypeInteger, root->GetType());
    EXPECT_TRUE(root->AsInteger(&int_val));
    EXPECT_EQ(42, int_val);
    root = ParseJSONWithCommentsDeprecated("/* comment **/\n"
                                           "// */ 43\n"
                                           "44",
        &error, &has_comments);
    EXPECT_EQ(has_comments, true);
    ...
    EXPECT_FALSE(root.get());
    EXPECT_EQ("Line: 1, column: 4, Unexpected data after root element.", error.message);
    ...
    EXPECT_FALSE(root.get());
    EXPECT_EQ("Line: 1, column: 2, Unexpected data after root element.", error.message);
    ...
    root = ParseJSON("\n\n    \"\\ud800\"", &error);
    EXPECT_FALSE(root.get());
    EXPECT_EQ("Line: 3, column: 5, Unsupported encoding. JSON and all string literals "
    ...
    EXPECT_FALSE(root.get());
    EXPECT_EQ("Line: 1, column: 1, Unsupported encoding. JSON and all string literals "
              "must contain valid Unicode characters.",
        error.message);
    ...
    EXPECT_FALSE(root.get());
    EXPECT_EQ("Line: 1, column: 2, Unsupported encoding. JSON and all string literals "
              "must contain valid Unicode characters.",
        error.message);
...
TEST(JSONParserTest, LimitedDepth)
{
    ...
    Vector<std::pair<const char*, int>> test_cases = { { "[[[[[]]]]]", 5 }, { "[[[[[\"a\"]]]]]", 6 }, { "[[],[],[],[],[]]", 2 },
        { "{\"a\":{\"a\":{\"a\":{\"a\":{\"a\": \"a\"}}}}}", 6 }, { "\"root\"", 1 } };
