...
public:
    ...
    ~JSONValueDeletionVerifier() override
    {
        ++(*counter_);
    }
...
TEST(JSONValuesTest, ArrayCastDoesNotLeak)
{
    ...
    std::unique_ptr<JSONValueDeletionVerifier> not_an_array(new JSONValueDeletionVerifier(deletion_count));
...
{
    int deletion_count = 0;
    std::unique_ptr<JSONValueDeletionVerifier> not_an_object(new JSONValueDeletionVerifier(deletion_count));
