...
String FormatErrorMessage(Error error, int line, int column)
{
    ...
    case Error::kUnsupportedEncoding:
        text = "Unsupported encoding. JSON and all string literals must contain "
               "valid Unicode characters.";
        break;
    }
    return "Line: " + String::Number(line) + ", column: " + String::Number(column) + ", " + text;
}
...
template <typename CharType> struct Cursor {
    int line;
...
template <typename CharType> Error ParseConstToken(Cursor<CharType>* cursor, const CharType* end, const char* token)
{
...
template <typename CharType> Error ReadInt(Cursor<CharType>* cursor, const CharType* end, bool can_have_leading_zeros)
{
    if (cursor->pos == end)
        return Error::kSyntaxError;
...
template <typename CharType> Error ParseNumberToken(Cursor<CharType>* cursor, const CharType* end)
{
...
template <typename CharType> Error ReadHexDigits(Cursor<CharType>* cursor, const CharType* end, int digits)
{
    const CharType* token_start = cursor->pos;
...
template <typename CharType> Error ParseStringToken(Cursor<CharType>* cursor, const CharType* end)
{
    if (cursor->pos == end)
        return Error::kSyntaxError;
    if (*(cursor->pos) != '"')
...
template <typename CharType> Error SkipComment(Cursor<CharType>* cursor, const CharType* end)
{
...
template <typename CharType> Error SkipWhitespaceAndComments(Cursor<CharType>* cursor, const CharType* end, JSONCommentState& comment_state)
{
    while (cursor->pos < end) {
        CharType c = *(cursor->pos);
        if (c == '\n') {
...
Error ParseToken(Cursor<CharType>* cursor, const CharType* end, Token* token, Cursor<CharType>* token_start, JSONCommentState& comment_state)
{
...
template <typename CharType> inline int HexToInt(CharType c)
{
    if ('0' <= c && c <= '9')
        return c - '0';
    if ('A' <= c && c <= 'F')
...
template <typename CharType> Error DecodeString(Cursor<CharType>* cursor, const CharType* end, String* output)
{
    ...
    while (cursor->pos < end - 1) {
        ...
        case 'v':
            ...
            break;
        case 'u':
            c = (HexToInt(*(cursor->pos)) << 12) + (HexToInt(*(cursor->pos + 1)) << 8) + (HexToInt(*(cursor->pos + 2)) << 4) + HexToInt(*(cursor->pos + 3));
            cursor->pos += 4;
            break;
        default:
            return Error::kInvalidEscape;
...
template <typename CharType>
Error BuildValue(Cursor<CharType>* cursor, const CharType* end, int max_depth, JSONCommentState& comment_state, std::unique_ptr<JSONValue>* result,
    Vector<String>* duplicate_keys)
{
    ...
    case kNumber: {
        bool ok;
        double value = CharactersToDouble(base::span<const CharType>(token_start.pos.get(), static_cast<size_t>(cursor->pos - token_start.pos)), &ok);
        if (!ok || std::isinf(value)) {
            *cursor = token_start;
            return Error::kSyntaxError;
        }
        if (base::IsValueInRangeForNumericType<int>(value) && static_cast<int>(value) == value)
    ...
    case kArrayBegin: {
        ...
        while (token != kArrayEnd) {
            *cursor = before_token;
            ...
            error = BuildValue(cursor, end, max_depth - 1, comment_state, &array_node, duplicate_keys);
            if (error != Error::kNoError)
                return error;
    ...
    case kObjectBegin: {
        ...
        while (token != kObjectEnd) {
            ...
            error = BuildValue(cursor, end, max_depth - 1, comment_state, &value, duplicate_keys);
            if (error != Error::kNoError)
                return error;
            if (!object->SetValue(key, std::move(value)) && !duplicate_keys->Contains(key)) {
...
template <typename CharType>
JSONParseError ParseJSONInternal(const CharType* start_ptr, unsigned length, int max_depth, JSONCommentState& comment_state, std::unique_ptr<JSONValue>* result)
{
    ...
    error.type = BuildValue(&cursor, end, max_depth, comment_state, result, &error.duplicate_keys);
...
std::unique_ptr<JSONValue> ParseJSON(const String& json, JSONParseError* opt_error)
{
...
std::unique_ptr<JSONValue> ParseJSONWithCommentsDeprecated(const String& json, JSONParseError* opt_error, bool* opt_has_comments)
{
...
std::unique_ptr<JSONValue> ParseJSON(const String& json, JSONCommentState& comment_state, int max_depth, JSONParseError* opt_error)
{
    if (max_depth < 0)
        max_depth = 0;
    ...
    if (json.empty()) {
        ...
        error = ParseJSONInternal(json.Characters8(), json.length(), max_depth, comment_state, &result);
    } else {
        error = ParseJSONInternal(json.Characters16(), json.length(), max_depth, comment_state, &result);
    }
