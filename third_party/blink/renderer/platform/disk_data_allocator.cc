...
DiskDataMetadata DiskDataAllocator::FindFreeChunk(size_t size)
{
    ...
    if (chosen_chunk.start_offset() != -1) {
        ...
        if (chosen_chunk.size() > size) {
            std::pair<int64_t, size_t> remainder_chunk = { chosen_chunk.start_offset() + size, chosen_chunk.size() - size };
...
void DiskDataAllocator::ReleaseChunk(const DiskDataMetadata& metadata)
{
    ...
    DCHECK(free_chunks_.upper_bound(chunk.start_offset()) == free_chunks_.lower_bound(chunk.start_offset()));
...
std::unique_ptr<ReservedChunk> DiskDataAllocator::TryReserveChunk(size_t size)
{
    ...
    return std::make_unique<ReservedChunk>(this, std::unique_ptr<DiskDataMetadata>(new DiskDataMetadata(chosen_chunk.start_offset(), chosen_chunk.size())));
}

std::unique_ptr<DiskDataMetadata> DiskDataAllocator::Write(std::unique_ptr<ReservedChunk> chunk, base::span<const uint8_t> data)
{
    std::unique_ptr<DiskDataMetadata> metadata = chunk->Take();
    DCHECK(metadata);

    std::optional<size_t> written = DoWrite(metadata->start_offset(), data.first(metadata->size()));
...
void DiskDataAllocator::Read(const DiskDataMetadata& metadata, base::span<uint8_t> data)
{
...
std::optional<size_t> DiskDataAllocator::DoWrite(int64_t offset, base::span<const uint8_t> data)
{
    std::optional<size_t> written = file_.Write(offset, data);

    // No PCHECK(), since a file writing error is recoverable.
    if (written != data.size()) {
        LOG(ERROR) << "DISK: Cannot write to disk. written = " << written.value_or(0u) << " " << base::File::ErrorToString(base::File::GetLastFileError());
    }
    return written;
}
...
// static
void DiskDataAllocator::Bind(mojo::PendingReceiver<mojom::blink::DiskAllocator> receiver)
{
