...
public:
    VectorWStream(Vector<unsigned char>* dst)
        : dst_(dst)
    {
        DCHECK(dst_);
...
public:
    static bool Encode(Vector<unsigned char>* dst, const SkPixmap& src, const SkJpegEncoder::Options&);

    static bool Encode(Vector<unsigned char>* dst, const SkPixmap& src, const SkPngEncoder::Options&);

    static bool Encode(Vector<unsigned char>* dst, const SkPixmap& src, const SkWebpEncoder::Options&);

    static int MaxDimension(ImageEncodingMimeType mime_type);

    static std::unique_ptr<ImageEncoder> Create(Vector<unsigned char>* dst, const SkPixmap& src, const SkJpegEncoder::Options&);

    static std::unique_ptr<ImageEncoder> Create(Vector<unsigned char>* dst, const SkPixmap& src, const SkPngEncoder::Options&);

    bool encodeRows(int numRows)
    {
        return encoder_->encodeRows(numRows);
    }
...
private:
    ImageEncoder(Vector<unsigned char>* dst)
        : dst_(dst)
    {
    }
