...
public:
    TestMultiBufferDataProvider(UrlData* url_data, MultiBuffer::BlockId pos, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
        : ResourceMultiBufferDataProvider(url_data, pos, false /* is_client_audio_element */, std::move(task_runner))
    {
    ...
    bool loading() const
    {
        return !!active_loader_;
    }
    bool deferred() const
    {
        return deferred_;
    }
    void RunOnStart(base::OnceClosure cb)
    {
...
public:
    TestResourceMultiBuffer(UrlData* url_data, int shift, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
        : ResourceMultiBuffer(url_data, shift, task_runner)
        , task_runner_(std::move(task_runner))
    {
    }

    std::unique_ptr<MultiBuffer::DataProvider> CreateWriter(const BlockId& pos, bool) override
    {
        auto writer = std::make_unique<TestMultiBufferDataProvider>(url_data_, pos, task_runner_);
        writer->Start();
        return writer;
    }
    ...
    {
        return test_data_providers.size() == 1U;
    }
    bool loading()
    {
...
public:
    TestUrlData(
        const KURL& url, CorsMode cors_mode, UrlIndex* url_index, UrlData::CacheMode cache_mode, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    ...
    ResourceMultiBuffer* multibuffer() override
    {
        if (!test_multibuffer_.get()) {
            test_multibuffer_ = std::make_unique<TestResourceMultiBuffer>(this, block_shift_, task_runner_);
        }
...
public:
    TestUrlIndex(ResourceFetchContext* fetch_context, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
        : UrlIndex(fetch_context, task_runner)
        , task_runner_(std::move(task_runner))
    {
    }

    scoped_refptr<UrlData> NewUrlData(const KURL& url, UrlData::CorsMode cors_mode, UrlData::CacheMode cache_mode) override
    {
        NotifyNewUrlData(url, cors_mode, cache_mode);
        last_url_data_ = base::MakeRefCounted<TestUrlData>(url, cors_mode, this, cache_mode, task_runner_);
    ...
    MOCK_METHOD3(NotifyNewUrlData, void(KURL, UrlData::CorsMode, UrlData::CacheMode));
...
public:
    ...
    MockBufferedDataSourceHost& operator=(const MockBufferedDataSourceHost&) = delete;
...
public:
    MockMultiBufferDataSource(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, scoped_refptr<UrlData> url_data, media::MediaLog* media_log,
        BufferedDataSourceHost* host)
        : MultiBufferDataSource(
            task_runner, std::move(url_data), media_log, host, base::BindRepeating(&MockMultiBufferDataSource::set_downloading, base::Unretained(this)))
        , downloading_(false)
    {
    }

    MockMultiBufferDataSource(const MockMultiBufferDataSource&) = delete;
    MockMultiBufferDataSource& operator=(const MockMultiBufferDataSource&) = delete;
    ...
    {
        return url_data_->range_supported();
    }
    void CallSeekTask()
    {
        SeekTask();
...
public:
    ...
    {
        ON_CALL(fetch_context_, CreateUrlLoader(_)).WillByDefault(Invoke([](const WebAssociatedURLLoaderOptions&) {
    ...
    MultiBufferDataSourceTest& operator=(const MultiBufferDataSourceTest&) = delete;

    MOCK_METHOD1(OnInitialize, void(bool));

    void InitializeWithCors(const char* url_string, bool expected, UrlData::CorsMode cors_mode, size_t file_size = kFileSize)
    {
        KURL url(url_string);
        media_log_ = std::make_unique<NiceMock<media::MockMediaLog>>();
        data_source_
            = std::make_unique<MockMultiBufferDataSource>(task_runner_, url_index_.GetByUrl(url, cors_mode, UrlData::kNormal), media_log_.get(), &host_);
        data_source_->SetPreload(preload_);

        response_generator_ = std::make_unique<TestResponseGenerator>(url, file_size);
        EXPECT_CALL(*this, OnInitialize(expected));
        data_source_->SetIsClientAudioElement(is_client_audio_element_);
        data_source_->Initialize(base::BindOnce(&MultiBufferDataSourceTest::OnInitialize, base::Unretained(this)));
    ...
    void Initialize(const char* url, bool expected, size_t file_size = kFileSize)
    {
    ...
    {
        data_source_->Read(position, howmuch, buffer_, base::BindOnce(&MultiBufferDataSourceTest::ReadCallback, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }

    void ExecuteMixedResponseSuccessTest(const WebURLResponse& response1, const WebURLResponse& response2)
    {
        EXPECT_CALL(host_, SetTotalBytes(kFileSize));
    ...
    void ExecuteMixedResponseFailureTest(const WebURLResponse& response1, const WebURLResponse& response2)
    {
        EXPECT_CALL(host_, SetTotalBytes(kFileSize));
        EXPECT_CALL(host_, AddBufferedByteRange(0, kDataSize));
        ...
        bool failed_ = false;
        EXPECT_CALL(*this, ReadCallback(media::DataSource::kReadError)).WillOnce(Assign(&failed_, true));
    ...
    {
        return data_source_->reader_.get();
    }
    ...
    bool loading()
    {
        return multibuffer()->loading();
    }

    MultiBufferDataSource::Preload preload()
    {
        return data_source_->preload_;
    }
    ...
    int data_source_bitrate()
    {
        return data_source_->bitrate_;
    }
    int64_t max_buffer_forward()
    {
        return loader()->max_buffer_forward_;
    }
    int64_t max_buffer_backward()
    {
        return loader()->max_buffer_backward_;
    }
    int64_t buffer_size()
    {
        return loader()->current_buffer_size_ * 32768 /* block size */;
    }
    double data_source_playback_rate()
    {
        return data_source_->playback_rate_;
    }
    bool is_local_source()
    {
        return data_source_->AssumeFullyBuffered();
    }
    bool is_client_audio_element()
    {
        return loader()->is_client_audio_element_;
    }
    scoped_refptr<UrlData> url_data()
    {
        return data_source_->url_data_;
    }
...
protected:
    ...
    const scoped_refptr<base::SingleThreadTaskRunner> task_runner_ = task_environment_.GetMainThreadTaskRunner();
...
TEST_F(MultiBufferDataSourceTest, Range_InstanceSizeUnknown)
{
    ...
    Respond(response_generator_->Generate206(0, TestResponseGenerator::kNoContentRangeInstanceSize));
...
TEST_F(MultiBufferDataSourceTest, Range_SupportedButReturned200)
{
    ...
    response.SetHttpHeaderField(WebString::FromUTF8("Accept-Ranges"), WebString::FromUTF8("bytes"));
    Respond(response);
...
{
    Initialize(kHttpUrl, false);
    Respond(response_generator_->Generate206(0, TestResponseGenerator::kNoContentRange));
...
TEST_F(MultiBufferDataSourceTest, Range_MissingContentLength)
{
    ...
    EXPECT_CALL(host_, SetTotalBytes(response_generator_->content_length()));
    Respond(response_generator_->Generate206(0, TestResponseGenerator::kNoContentLength));
...
TEST_F(MultiBufferDataSourceTest, Http_MixedResponse_RedirectedToDifferentPathResponse)
{
    Initialize(kHttpUrl, true);
    WebURLResponse response1 = response_generator_->GeneratePartial206(0, kDataSize - 1);
    WebURLResponse response2 = response_generator_->GeneratePartial206(kDataSize, kDataSize * 2 - 1);
...
TEST_F(MultiBufferDataSourceTest, Http_MixedResponse_RedirectedToDifferentOriginResponse)
{
    Initialize(kHttpUrl, true);
    WebURLResponse response1 = response_generator_->GeneratePartial206(0, kDataSize - 1);
    WebURLResponse response2 = response_generator_->GeneratePartial206(kDataSize, kDataSize * 2 - 1);
...
TEST_F(MultiBufferDataSourceTest, Http_MixedResponse_ServiceWorkerGeneratedResponseAndNormalResponse)
{
    Initialize(kHttpUrl, true);
    WebURLResponse response1 = response_generator_->GeneratePartial206(0, kDataSize - 1);
...
TEST_F(MultiBufferDataSourceTest, Http_MixedResponse_ServiceWorkerProxiedAndSameURLResponse)
{
    Initialize(kHttpUrl, true);
    WebURLResponse response1 = response_generator_->GeneratePartial206(0, kDataSize - 1);
    response1.SetWasFetchedViaServiceWorker(true);
...
TEST_F(MultiBufferDataSourceTest, Http_MixedResponse_ServiceWorkerProxiedAndDifferentPathResponse)
{
    Initialize(kHttpUrl, true);
    WebURLResponse response1 = response_generator_->GeneratePartial206(0, kDataSize - 1);
    response1.SetWasFetchedViaServiceWorker(true);
...
TEST_F(MultiBufferDataSourceTest, Http_MixedResponse_ServiceWorkerProxiedAndDifferentOriginResponse)
{
    Initialize(kHttpUrl, true);
    WebURLResponse response1 = response_generator_->GeneratePartial206(0, kDataSize - 1);
    response1.SetWasFetchedViaServiceWorker(true);
...
TEST_F(MultiBufferDataSourceTest, Http_MixedResponse_ServiceWorkerProxiedAndDifferentOriginResponseCors)
{
...
TEST_F(MultiBufferDataSourceTest, File_InstanceSizeUnknown)
{
    ...
    Respond(response_generator_->GenerateFileResponse(media::DataSource::kReadError));
    ReceiveData(kDataSize);
...
TEST_F(MultiBufferDataSourceTest, StopDuringRead)
{
    ...
    uint8_t buffer[256];
    data_source_->Read(kDataSize, std::size(buffer), buffer, base::BindOnce(&MultiBufferDataSourceTest::ReadCallback, base::Unretained(this)));
...
TEST_F(MultiBufferDataSourceTest, Http_Read)
{
    ...
    EXPECT_CALL(*this, ReadCallback(kDataSize));
    EXPECT_CALL(host_, AddBufferedByteRange(kDataSize, kDataSize + kDataSize / 2));
...
TEST_F(MultiBufferDataSourceTest, Http_ShareData)
{
    ...
    media_log_ = std::make_unique<NiceMock<media::MockMediaLog>>();
    MockMultiBufferDataSource source2(task_runner_, url_index_.GetByUrl(KURL(kHttpUrl), UrlData::CORS_UNSPECIFIED, UrlData::kNormal), media_log_.get(), &host2);
    ...
    source2.Initialize(base::BindOnce(&MultiBufferDataSourceTest::OnInitialize, base::Unretained(this)));
    base::RunLoop().RunUntilIdle();
...
TEST_F(MultiBufferDataSourceTest, Http_ShareData_AtLeastOneProgress)
{
    ...
    EXPECT_CALL(host_, AddBufferedByteRange(0, testing::Ge(total_bytes))).Times(testing::AtLeast(1));
...
TEST_F(MultiBufferDataSourceTest, ExternalResource_Response200_PreloadMetadata_DeferStrategy)
{
    set_preload(MultiBufferDataSource::METADATA);
    InitializeWith200Response();
...
TEST_F(MultiBufferDataSourceTest, ExternalResource_Response206_PreloadMetadata_DeferStrategy)
{
    set_preload(MultiBufferDataSource::METADATA);
    InitializeWith206Response();
...
TEST_F(MultiBufferDataSourceTest, ExternalResource_Response206_CancelAfterDefer)
{
    set_preload(MultiBufferDataSource::METADATA);
    InitializeWith206Response();
...
TEST_F(MultiBufferDataSourceTest, ExternalResource_Response206_CancelAfterDefer2)
{
    set_preload(MultiBufferDataSource::METADATA);
    InitializeWith206Response();
...
// This test tries to trigger an edge case where the read callback
// never happens because the reader is deleted before that happens.
TEST_F(MultiBufferDataSourceTest, ExternalResource_Response206_CancelAfterDefer3)
{
    set_preload(MultiBufferDataSource::METADATA);
    InitializeWith206Response();
    ...
    ReceiveData(kDataSize);
    EXPECT_EQ(data_source_->downloading(), false);
    data_source_->Read(kDataSize * 10, kDataSize, buffer_, base::BindOnce(&MultiBufferDataSourceTest::ReadCallback, base::Unretained(this)));
    data_source_->OnBufferingHaveEnough(false);
...
TEST_F(MultiBufferDataSourceTest, ExternalResource_Response206_CancelAfterPlay)
{
    set_preload(MultiBufferDataSource::METADATA);
    InitializeWith206Response();
...
// takes place.
TEST_F(MultiBufferDataSourceTest, ExternalResource_Response206_CheckIsClientAudioElement)
{
    set_preload(MultiBufferDataSource::METADATA);
    ...
    ReadAt(kFarReadPosition);
    EXPECT_CALL(*this, ReadCallback(kDataSize));
    EXPECT_CALL(host_, AddBufferedByteRange(kFarReadPosition, kFarReadPosition + kDataSize));
    EXPECT_TRUE(is_client_audio_element());
    Respond(response_generator_->Generate206(kFarReadPosition));
    ReceiveData(kDataSize);
...
TEST_F(MultiBufferDataSourceTest, SeekPastEOF)
{
    ...
    data_source_ = std::make_unique<MockMultiBufferDataSource>(
        task_runner_, url_index_.GetByUrl(url, UrlData::CORS_UNSPECIFIED, UrlData::kNormal), media_log_.get(), &host_);
    data_source_->SetPreload(preload_);

    response_generator_ = std::make_unique<TestResponseGenerator>(url, kDataSize + 1);
    EXPECT_CALL(*this, OnInitialize(true));
    data_source_->Initialize(base::BindOnce(&MultiBufferDataSourceTest::OnInitialize, base::Unretained(this)));
    base::RunLoop().RunUntilIdle();
...
TEST_F(MultiBufferDataSourceTest, PreserveCachingModeAfterRedirect)
{
    ...
    data_response.SetHttpHeaderField(WebString::FromUTF8("Accept-Ranges"), WebString::FromUTF8("bytes"));
    ...
    {
        EXPECT_CALL(url_index_, NotifyNewUrlData(start, _, UrlData::kCacheDisabled));
        auto data_source = std::make_unique<MockMultiBufferDataSource>(
            task_runner_, url_index_.GetByUrl(start, UrlData::CORS_UNSPECIFIED, UrlData::kCacheDisabled), media_log_.get(), &host_);
        data_source->SetPreload(preload_);
        auto response_generator = std::make_unique<TestResponseGenerator>(start, kFileSize);
        data_source->SetIsClientAudioElement(false);
        EXPECT_CALL(*this, OnInitialize(true));
        data_source->Initialize(base::BindOnce(&MultiBufferDataSourceTest::OnInitialize, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
        EXPECT_EQ(data_source->downloading(), false);
        EXPECT_CALL(url_index_, NotifyNewUrlData(redir, _, UrlData::kCacheDisabled));
    ...
    // was a 200.
    {
        ...
        auto data_source = std::make_unique<MockMultiBufferDataSource>(
            task_runner_, url_index_.GetByUrl(start, UrlData::CORS_UNSPECIFIED, UrlData::kNormal), media_log_.get(), &host_);
        data_source->SetPreload(preload_);
        auto response_generator = std::make_unique<TestResponseGenerator>(start, kFileSize);
        data_source->SetIsClientAudioElement(false);
        EXPECT_CALL(*this, OnInitialize(true));
...
TEST_F(MultiBufferDataSourceTest, FileSizeLessThanBlockSize)
{
    ...
    response.SetHttpHeaderField(WebString::FromUTF8("Content-Length"), WebString::FromUTF8(base::NumberToString(kDataSize / 2)));
...
TEST_F(MultiBufferDataSourceTest, EtagTest)
{
    ...
    response.SetHttpHeaderField(WebString::FromUTF8("Etag"), WebString::FromUTF8(etag));
    Respond(response);
    EXPECT_CALL(host_, AddBufferedByteRange(0, kDataSize));
    ReceiveData(kDataSize);
...
TEST_F(MultiBufferDataSourceTest, Http_CheckLoadingTransition)
{
    ...
    EXPECT_CALL(*this, ReadCallback(1));
    data_source_->Read(kDataSize, 2, buffer_, base::BindOnce(&MultiBufferDataSourceTest::ReadCallback, base::Unretained(this)));
    base::RunLoop().RunUntilIdle();
...
TEST_F(MultiBufferDataSourceTest, Http_Seek_Back)
{
    ...
    EXPECT_CALL(*this, ReadCallback(kDataSize));
    EXPECT_CALL(host_, AddBufferedByteRange(kFarReadPosition - kDataSize, kFarReadPosition + kDataSize));
