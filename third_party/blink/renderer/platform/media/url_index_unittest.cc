...
public:
    ...
    scoped_refptr<UrlData> GetByUrl(const KURL& url, UrlData::CorsMode cors_mode)
    {
        scoped_refptr<UrlData> ret = url_index_.GetByUrl(url, cors_mode, UrlData::kNormal);
...
TEST_F(UrlIndexTest, GetByUrlCacheDisabled)
{
    ...
    scoped_refptr<UrlData> url_data = url_index_.GetByUrl(url, cors, UrlData::kNormal);
    url_data->Use();
