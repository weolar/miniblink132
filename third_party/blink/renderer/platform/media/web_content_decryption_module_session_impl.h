...
class PLATFORM_EXPORT WebContentDecryptionModuleSessionImpl : public WebContentDecryptionModuleSession {
public:
    ...
    WebContentDecryptionModuleSessionImpl(const WebContentDecryptionModuleSessionImpl&) = delete;
    WebContentDecryptionModuleSessionImpl& operator=(const WebContentDecryptionModuleSessionImpl&) = delete;
    ...
    void InitializeNewSession(
        media::EmeInitDataType init_data_type, const unsigned char* initData, size_t initDataLength, WebContentDecryptionModuleResult result) override;
    void Load(const WebString& session_id, WebContentDecryptionModuleResult result) override;
    void Update(const uint8_t* response, size_t response_length, WebContentDecryptionModuleResult result) override;
    void Close(WebContentDecryptionModuleResult result) override;
    void Remove(WebContentDecryptionModuleResult result) override;

    // Callbacks.
    void OnSessionMessage(media::CdmMessageType message_type, const std::vector<uint8_t>& message);
    void OnSessionKeysChange(bool has_additional_usable_key, media::CdmKeysInfo keys_info);
...
private:
    ...
    void OnSessionInitialized(const std::string& session_id, SessionInitStatus* status);
    ...
    // actually fires.
    base::WeakPtrFactory<WebContentDecryptionModuleSessionImpl> weak_ptr_factory_ { this };
};
