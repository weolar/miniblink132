...
void CompleteWebContentDecryptionModuleResult(
    std::unique_ptr<WebContentDecryptionModuleResult> result, std::unique_ptr<WebContentDecryptionModule> cdm, media::CreateCdmStatus status)
{
    DCHECK(result);

    if (!cdm) {
        result->CompleteWithError(kWebContentDecryptionModuleExceptionNotSupportedError, 0, WebString::FromASCII(ConvertCreateCdmStatusToString(status)));
        return;
    }
...
public:
    ...
    void ReportResolveTime()
    {
        ...
        base::UmaHistogramTimes(uma_name_ + ".TimeTo.Resolve", base::TimeTicks::Now() - report_status_.request_start_time);
    ...
    void ReportRejectTime()
    {
        ...
        base::UmaHistogramTimes(uma_name_ + ".TimeTo.Reject", base::TimeTicks::Now() - report_status_.request_start_time);
        report_status_.is_result_reported = true;
    }
...
using PerProcessReporterMap = std::unordered_map<std::string, std::unique_ptr<PerProcessReporter>>;
...
static PerProcessReporter* GetPerProcessReporter(const WebString& key_system)
{
    ...
    std::unique_ptr<PerProcessReporter>& reporter = GetPerProcessReporterMap()[uma_name];
...
public:
    enum KeySystemSupportStatus { KEY_SYSTEM_REQUESTED = 0, KEY_SYSTEM_SUPPORTED = 1, KEY_SYSTEM_SUPPORT_STATUS_COUNT };
...
private:
    ...
    {
        base::UmaHistogramEnumeration(uma_name_, status, KEY_SYSTEM_SUPPORT_STATUS_COUNT);
    }
...
WebEncryptedMediaClientImpl::WebEncryptedMediaClientImpl(media::KeySystems* key_systems, media::CdmFactory* cdm_factory,
    media::MediaPermission* media_permission, std::unique_ptr<KeySystemConfigSelector::WebLocalFrameDelegate> web_frame_delegate)
    : key_systems_(key_systems)
    , cdm_factory_(cdm_factory)
    , key_system_config_selector_(key_systems_, media_permission, std::move(web_frame_delegate))
{
    DCHECK(cdm_factory_);
}
...
void WebEncryptedMediaClientImpl::RequestMediaKeySystemAccess(WebEncryptedMediaRequest request)
{
    ...
    key_systems_->UpdateIfNeeded(base::BindOnce(&WebEncryptedMediaClientImpl::OnKeySystemsUpdated, weak_factory_.GetWeakPtr()));
}

void WebEncryptedMediaClientImpl::CreateCdm(
    const WebSecurityOrigin& security_origin, const media::CdmConfig& cdm_config, std::unique_ptr<WebContentDecryptionModuleResult> result)
{
    WebContentDecryptionModuleImpl::Create(
        cdm_factory_, key_systems_, security_origin, cdm_config, base::BindOnce(&CompleteWebContentDecryptionModuleResult, std::move(result)));
}
...
void WebEncryptedMediaClientImpl::SelectConfig(WebEncryptedMediaRequest request)
{
    key_system_config_selector_.SelectConfig(request.KeySystem(), request.SupportedConfigurations(),
        base::BindOnce(&WebEncryptedMediaClientImpl::OnConfigSelected, weak_factory_.GetWeakPtr(), request));
}

void WebEncryptedMediaClientImpl::OnConfigSelected(WebEncryptedMediaRequest request, KeySystemConfigSelector::Status status,
    WebMediaKeySystemConfiguration* accumulated_configuration, media::CdmConfig* cdm_config)
{
    ...
    const char kUnsupportedKeySystemOrConfigMessage[] = "Unsupported keySystem or supportedConfigurations.";
    ...
    request.RequestSucceeded(WebContentDecryptionModuleAccessImpl::Create(origin, *accumulated_configuration, *cdm_config, weak_factory_.GetWeakPtr()));
}

WebEncryptedMediaClientImpl::Reporter* WebEncryptedMediaClientImpl::GetReporter(const WebString& key_system)
{
    // Assumes that empty will not be found by GetKeySystemNameForUMA().
    std::string key_system_ascii;
    if (key_system.ContainsOnlyASCII())
