...
template <typename... T> class PLATFORM_EXPORT CdmResultPromise : public media::CdmPromiseTemplate<T...> {
public:
    CdmResultPromise(const WebContentDecryptionModuleResult& result, const std::string& key_system_uma_prefix, const std::string& uma_name);
    ...
    void reject(media::CdmPromise::Exception exception_code, uint32_t system_code, const std::string& error_message) override;
...
template <typename... T>
CdmResultPromise<T...>::CdmResultPromise(const WebContentDecryptionModuleResult& result, const std::string& key_system_uma_prefix, const std::string& uma_name)
...
template <typename... T> CdmResultPromise<T...>::~CdmResultPromise()
{
    if (!IsPromiseSettled())
...
template <> inline void CdmResultPromise<>::resolve()
{
    MarkPromiseSettled();
    ReportCdmResultUMA(key_system_uma_prefix_ + uma_name_, 0, SUCCESS);

    base::UmaHistogramTimes(key_system_uma_prefix_ + kTimeToResolveUmaPrefix + uma_name_, base::TimeTicks::Now() - creation_time_);

    web_cdm_result_.Complete();
}

template <> inline void CdmResultPromise<media::CdmKeyInformation::KeyStatus>::resolve(const media::CdmKeyInformation::KeyStatus& key_status)
{
    MarkPromiseSettled();
    ReportCdmResultUMA(key_system_uma_prefix_ + uma_name_, 0, SUCCESS);
...
template <typename... T>
void CdmResultPromise<T...>::reject(media::CdmPromise::Exception exception_code, uint32_t system_code, const std::string& error_message)
{
    MarkPromiseSettled();
    ReportCdmResultUMA(key_system_uma_prefix_ + uma_name_, system_code, ConvertCdmExceptionToResultForUMA(exception_code));

    base::UmaHistogramTimes(key_system_uma_prefix_ + kTimeToRejectUmaPrefix + uma_name_, base::TimeTicks::Now() - creation_time_);

    web_cdm_result_.CompleteWithError(ConvertCdmException(exception_code), system_code, WebString::FromUTF8(error_message));
}
