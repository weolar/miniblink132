...
{
    auto key_system_name_for_uma = media::GetKeySystemNameForUMA(cdm_config.key_system, cdm_config.use_hw_secure_codecs);
    auto key_system_uma_prefix = "Media.EME." + key_system_name_for_uma + ".";
    return key_system_uma_prefix;
}

void ReportCreateCdmStatusUMA(const std::string& uma_prefix, bool is_cdm_created, media::CreateCdmStatus status)
{
...
void ReportCreateCdmTimeUMA(const std::string& uma_prefix, base::TimeDelta delta)
{
    DCHECK(uma_prefix.ends_with("."));
