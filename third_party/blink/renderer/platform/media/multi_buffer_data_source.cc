...
public:
    ...
    ReadOperation(int64_t position, int size, uint8_t* data, media::DataSource::ReadCB callback);
...
MultiBufferDataSource::ReadOperation::ReadOperation(int64_t position, int size, uint8_t* data, media::DataSource::ReadCB callback)
    : position_(position)
    , size_(size)
    , data_(data)
...
// static
void MultiBufferDataSource::ReadOperation::Run(std::unique_ptr<ReadOperation> read_op, int result)
{
    std::move(read_op->callback_).Run(result);
}

MultiBufferDataSource::MultiBufferDataSource(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, scoped_refptr<UrlData> url_data_arg,
    media::MediaLog* media_log, BufferedDataSourceHost* host, DownloadingCB downloading_cb)
...
void MultiBufferDataSource::SetReader(std::unique_ptr<MultiBufferReader> reader)
{
    DCHECK(render_task_runner_->BelongsToCurrentThread());
...
void MultiBufferDataSource::CreateResourceLoader(int64_t first_byte_position, int64_t last_byte_position)
{
    DCHECK(render_task_runner_->BelongsToCurrentThread());

    SetReader(std::make_unique<MultiBufferReader>(url_data_->multibuffer(), first_byte_position, last_byte_position, is_client_audio_element_,
        base::BindRepeating(&MultiBufferDataSource::ProgressCallback, weak_ptr_), render_task_runner_));
    UpdateBufferSizes();
}

void MultiBufferDataSource::CreateResourceLoader_Locked(int64_t first_byte_position, int64_t last_byte_position)
{
    DCHECK(render_task_runner_->BelongsToCurrentThread());
    lock_.AssertAcquired();

    reader_ = std::make_unique<MultiBufferReader>(url_data_->multibuffer(), first_byte_position, last_byte_position, is_client_audio_element_,
        base::BindRepeating(&MultiBufferDataSource::ProgressCallback, weak_ptr_), render_task_runner_);
    UpdateBufferSizes();
}
...
void MultiBufferDataSource::Initialize(InitializeCB init_cb)
{
    ...
    if (reader_->Available()) {
        render_task_runner_->PostTask(FROM_HERE, base::BindOnce(&MultiBufferDataSource::StartCallback, weak_ptr_));
        ...
        render_task_runner_->PostTask(FROM_HERE, base::BindOnce(&MultiBufferDataSource::UpdateProgress, weak_factory_.GetWeakPtr()));
    } else {
        reader_->Wait(1, base::BindOnce(&MultiBufferDataSource::StartCallback, weak_ptr_));
    }
}

void MultiBufferDataSource::OnRedirected(const scoped_refptr<UrlData>& new_destination)
{
    ...
    if (!SecurityOrigin::AreSameOrigin(url_data_->url(), new_destination->url())) {
    ...
    if (init_cb_) {
        ...
        if (reader_->Available()) {
            render_task_runner_->PostTask(FROM_HERE, base::BindOnce(&MultiBufferDataSource::ReadTask, weak_ptr_));
        } else {
            reader_->Wait(1, base::BindOnce(&MultiBufferDataSource::ReadTask, weak_ptr_));
        }
    }
...
void MultiBufferDataSource::Stop()
{
    ...
    render_task_runner_->PostTask(FROM_HERE, base::BindOnce(&MultiBufferDataSource::StopLoader, weak_factory_.GetWeakPtr()));
}
...
{
    render_task_runner_->PostTask(FROM_HERE, base::BindOnce(&MultiBufferDataSource::SetBitrateTask, weak_factory_.GetWeakPtr(), bitrate));
}
...
int64_t MultiBufferDataSource::GetMemoryUsage()
{
    ...
    return base::checked_cast<int64_t>(url_data_->CachedSize()) << url_data_->multibuffer()->block_size_shift();
}
...
void MultiBufferDataSource::Read(int64_t position, int size, uint8_t* data, media::DataSource::ReadCB read_cb)
{
...

    {
        ...
        if (reader_) {
            ...
            if (bytes_read > 0) {
                ...
                if (seek_positions_.size() == 1) {
                    render_task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&MultiBufferDataSource::SeekTask, weak_factory_.GetWeakPtr()), kSeekDelay);
                }
        ...
        read_op_ = std::make_unique<ReadOperation>(position, size, data, std::move(read_cb));
    }

    render_task_runner_->PostTask(FROM_HERE, base::BindOnce(&MultiBufferDataSource::ReadTask, weak_factory_.GetWeakPtr()));
}
...
void MultiBufferDataSource::ReadTask()
{
    ...
    if (available) {
        ...
        bytes_read = reader_->TryReadAt(read_op_->position(), read_op_->data(), bytes_read);
        ...
        reader_->Wait(1, base::BindOnce(&MultiBufferDataSource::ReadTask, weak_factory_.GetWeakPtr()));
...
void MultiBufferDataSource::StartCallback()
{
    ...
    bool success = reader_ && reader_->Available() > 0 && url_data_ && (!AssumeFullyBuffered() || url_data_->length() != kPositionNotSpecified);

    if (success) {
        total_bytes_ = url_data_->length();
        streaming_ = !AssumeFullyBuffered() && (total_bytes_ == kPositionNotSpecified || !url_data_->range_supported());
    ...
    if (success) {
        ...
        media_log_->SetProperty<media::MediaLogProperty::kIsSingleOrigin>(single_origin_);
        media_log_->SetProperty<media::MediaLogProperty::kIsRangeHeaderSupported>(url_data_->range_supported());
    }

    render_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(init_cb_), success));
...
void MultiBufferDataSource::UpdateBufferSizes()
{
    ...
    int64_t preload = std::clamp(preload_seconds_.value() * bytes_per_second, kMinBufferPreload, kMaxBufferPreload);

    // Increase buffering slowly at a rate of 10% of data downloaded so
    // far, maxing out at the preload size.
    int64_t extra_buffer = std::min(preload, url_data_->BytesReadFromCache() * kSlowPreloadPercentage / 100);
    ...
    int64_t pin_backward = std::clamp(keep_after_playback_seconds_.value() * bytes_per_second, kMinBufferPreload, kMaxBufferPreload);
    ...
    int64_t buffer_size = std::min(
        (preload_seconds_.value() + keep_after_playback_seconds_.value()) * bytes_per_second + extra_buffer * 3, preload_high + pin_backward + extra_buffer);

    if (url_data_->FullyCached() || (url_data_->length() != kPositionNotSpecified && url_data_->length() < kDefaultPinSize)) {
