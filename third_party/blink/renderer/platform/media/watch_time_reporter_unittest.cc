...
#define EXPECT_WATCH_TIME(key, value)                                                                                                                          \
    do {                                                                                                                                                       \
        EXPECT_CALL(*this,                                                                                                                                     \
            OnWatchTimeUpdate((has_video_ && has_audio_) ? WatchTimeKey::kAudioVideo##key                                                                      \
...
#define EXPECT_MUTED_WATCH_TIME_IF_AUDIO_VIDEO(key, value)                                                                                                     \
    do {                                                                                                                                                       \
        if (!has_video_ || !has_audio_)                                                                                                                        \
            break;                                                                                                                                             \
        EXPECT_CALL(*this, OnWatchTimeUpdate(WatchTimeKey::kAudioVideoMuted##key, value)).RetiresOnSaturation();                                               \
    } while (0)
...
#define EXPECT_WATCH_TIME_IF_VIDEO(key, value)                                                                                                                 \
    do {                                                                                                                                                       \
        if (!has_video_)                                                                                                                                       \
            break;                                                                                                                                             \
        EXPECT_CALL(*this, OnWatchTimeUpdate(has_audio_ ? WatchTimeKey::kAudioVideo##key : WatchTimeKey::kVideo##key, value)).RetiresOnSaturation();           \
    } while (0)

#define EXPECT_BACKGROUND_WATCH_TIME(key, value)                                                                                                               \
    do {                                                                                                                                                       \
        EXPECT_CALL(*this,                                                                                                                                     \
            OnWatchTimeUpdate((has_video_ && has_audio_) ? WatchTimeKey::kAudioVideoBackground##key                                                            \
...
#define EXPECT_WATCH_TIME_IF_AUDIO_VIDEO_MEDIAFOUNDATION(key, value)                                                                                           \
    do {                                                                                                                                                       \
        if (!has_video_ || !has_audio_)                                                                                                                        \
            break;                                                                                                                                             \
        EXPECT_CALL(*this, OnWatchTimeUpdate(WatchTimeKey::kAudioVideoMediaFoundation##key, value)).RetiresOnSaturation();                                     \
    } while (0)

#define EXPECT_WATCH_TIME_FINALIZED() EXPECT_CALL(*this, OnWatchTimeFinalized()).RetiresOnSaturation();

// The following macros have .Times() values equal to the number of keys that a
// finalize event is expected to finalize.
#define EXPECT_POWER_WATCH_TIME_FINALIZED() EXPECT_CALL(*this, OnPowerWatchTimeFinalized()).Times(2).RetiresOnSaturation();

#define EXPECT_CONTROLS_WATCH_TIME_FINALIZED() EXPECT_CALL(*this, OnControlsWatchTimeFinalized()).Times(2).RetiresOnSaturation();

#define EXPECT_DISPLAY_WATCH_TIME_FINALIZED() EXPECT_CALL(*this, OnDisplayWatchTimeFinalized()).Times(3).RetiresOnSaturation();

using WatchTimeReporterTestData = std::tuple<bool, bool>;
class WatchTimeReporterTest : public testing::TestWithParam<WatchTimeReporterTestData> {
public:
    class WatchTimeInterceptor : public media::mojom::WatchTimeRecorder {
    public:
        WatchTimeInterceptor(WatchTimeReporterTest* parent)
            : parent_(parent)
        {
        }
        ...
        void FinalizeWatchTime(const std::vector<WatchTimeKey>& watch_time_keys) override
        {
        ...
        void UpdateSecondaryProperties(media::mojom::SecondaryPlaybackPropertiesPtr secondary_properties) override
        {
        ...
        void UpdateVideoDecodeStats(uint32_t video_frames_decoded, uint32_t video_frames_dropped) override
        {
            parent_->OnUpdateVideoDecodeStats(video_frames_decoded, video_frames_dropped);
        }
    ...
    public:
        ...
        void AcquireWatchTimeRecorder(media::mojom::PlaybackPropertiesPtr properties, mojo::PendingReceiver<media::mojom::WatchTimeRecorder> receiver) override
        {
            mojo::MakeSelfOwnedReceiver(std::make_unique<WatchTimeInterceptor>(parent_), std::move(receiver));
        }
        void AcquireVideoDecodeStatsRecorder(mojo::PendingReceiver<media::mojom::VideoDecodeStatsRecorder> receiver) override
        {
            FAIL();
        }
        void AcquireLearningTaskController(const std::string& taskName, mojo::PendingReceiver<media::learning::mojom::LearningTaskController> receiver) override
        {
        }
...
protected:
    void Initialize(bool is_mse, bool is_encrypted, const gfx::Size& initial_video_size, media::RendererType renderer_type = media::RendererType::kRendererImpl)
    {
        if (wtr_ && IsMonitoring())
            EXPECT_WATCH_TIME_FINALIZED();

        wtr_ = std::make_unique<WatchTimeReporter>(media::mojom::PlaybackProperties::New(has_audio_, has_video_, false, false, is_mse, is_encrypted, false,
                                                       media::mojom::MediaStreamType::kNone, renderer_type),
            initial_video_size, base::BindRepeating(&WatchTimeReporterTest::GetCurrentMediaTime, base::Unretained(this)),
            base::BindRepeating(&WatchTimeReporterTest::GetPipelineStatistics, base::Unretained(this)), &fake_metrics_provider_,
            scheduler::GetSequencedTaskRunnerForTesting(), task_environment_.GetMockTickClock());
        reporting_interval_ = wtr_->reporting_interval_;

        // Most tests don't care about this.
        EXPECT_CALL(*this, GetPipelineStatistics()).WillRepeatedly(testing::Return(media::PipelineStatistics()));
        EXPECT_CALL(*this, OnUpdateVideoDecodeStats(_, _)).Times(testing::AnyNumber());
    }
    ...
    {
        return wtr_->reporting_timer_.IsRunning();
    }
    ...
    {
        return wtr_->muted_reporter_ && wtr_->muted_reporter_->reporting_timer_.IsRunning();
    }

    void DisableMutedReporting()
    {
    ...
    void OnPowerStateChange(bool on_battery_power)
    {
        base::PowerStateObserver::BatteryPowerStatus battery_power_status_
            = on_battery_power ? base::PowerStateObserver::BatteryPowerStatus::kBatteryPower : base::PowerStateObserver::BatteryPowerStatus::kExternalPower;
        wtr_->OnBatteryPowerStatusChange(battery_power_status_);
        if (wtr_->background_reporter_) {
            wtr_->background_reporter_->OnBatteryPowerStatusChange(battery_power_status_);
        }
    ...
    {
        enabled ? wtr_->OnNativeControlsEnabled() : wtr_->OnNativeControlsDisabled();
    }
    ...
    template <int TestFlags = 0, typename HysteresisTestCallback> void RunHysteresisTest(HysteresisTestCallback test_callback_func)
    {
        ...
        constexpr base::TimeDelta kWatchTime4 = base::Seconds(30);
        {
            ...
            EXPECT_CALL(*this, GetCurrentMediaTime()).WillOnce(testing::Return(base::TimeDelta())).WillOnce(testing::Return(kWatchTime1));
            ...
            if (TestFlags & kAccumulationContinuesAfterTest) {
                EXPECT_CALL(*this, GetCurrentMediaTime())
                    .Times(
                        TestFlags & (kFinalizeExitDoesNotRequireCurrentTime | kFinalizePowerWatchTime | kFinalizeControlsWatchTime | kFinalizeDisplayWatchTime)
                            ? 1
                            : 2)
                    .WillRepeatedly(testing::Return(kWatchTime2));
                EXPECT_CALL(*this, GetCurrentMediaTime()).WillOnce(testing::Return(kWatchTime3));
            } else {
            ...
            if (TestFlags & kTransitionPowerWatchTime) {
                EXPECT_CALL(*this, GetCurrentMediaTime()).WillOnce(testing::Return(kWatchTime4));
            }
        ...
        const base::TimeDelta kExpectedContolsWatchTime = TestFlags & kFinalizeControlsWatchTime ? kWatchTime2 : kExpectedWatchTime;
        const base::TimeDelta kExpectedDisplayWatchTime = TestFlags & kFinalizeDisplayWatchTime ? kWatchTime2 : kExpectedWatchTime;
        ...
        if (TestFlags & kTransitionPowerWatchTime) {
            ASSERT_TRUE(TestFlags & kAccumulationContinuesAfterTest) << "kTransitionPowerWatchTime tests must be done with "
            ...
            ASSERT_TRUE(TestFlags & kAccumulationContinuesAfterTest) << "kTransitionControlsWatchTime tests must be done with "
                                                                        "kAccumulationContinuesAfterTest";
            ...
            ASSERT_TRUE(TestFlags & kAccumulationContinuesAfterTest) << "kTransitionDisplayWatchTime tests must be done with "
                                                                        "kAccumulationContinuesAfterTest";
            ...
            if (TestFlags & kStartWithDisplayFullscreen) {
                ...
                EXPECT_WATCH_TIME_IF_VIDEO(DisplayFullscreen, kWatchTime4 - kWatchTime2);
            }
        }
    ...
    MOCK_METHOD1(OnUpdateSecondaryProperties, void(media::mojom::SecondaryPlaybackPropertiesPtr));
...
{
    EXPECT_CALL(*this, GetCurrentMediaTime()).WillRepeatedly(testing::Return(base::TimeDelta()));
    ...
    EXPECT_CALL(*this, OnError(media::HasStatusCode(media::PIPELINE_ERROR_DECODE))).Times((has_audio_ && has_video_) ? 3 : 2);
...
{
    EXPECT_CALL(*this, GetCurrentMediaTime()).WillRepeatedly(testing::Return(media::kInfiniteDuration));
    Initialize(false, false, kSizeJustRight);
    wtr_->OnPlaying();
...
TEST_P(WatchTimeReporterTest, WatchTimeReporterBasic)
{
    ...
    if (has_video_) {
        EXPECT_CALL(*this, GetPipelineStatistics()).WillOnce(testing::Return(media::PipelineStatistics())).WillRepeatedly(testing::Return(stats));
        EXPECT_CALL(*this, OnUpdateVideoDecodeStats(stats.video_frames_decoded, stats.video_frames_dropped));
    }
...
TEST_P(WatchTimeReporterTest, WatchTimeReporterStatsOffsetCorrectly)
{
    ...
    if (has_video_) {
        EXPECT_CALL(*this, GetPipelineStatistics()).WillOnce(testing::Return(initial_stats)).WillRepeatedly(testing::Return(stats));
        EXPECT_CALL(*this,
            OnUpdateVideoDecodeStats(
                stats.video_frames_decoded - initial_stats.video_frames_decoded, stats.video_frames_dropped - initial_stats.video_frames_dropped));
    }
...
TEST_P(WatchTimeReporterTest, WatchTimeReporterDuration)
{
    ...
    EXPECT_CALL(*this, OnDurationChanged(kDuration1)).Times((has_audio_ && has_video_) ? 3 : 2);
    wtr_->OnDurationChanged(kDuration1);
    CycleReportingTimer();

    EXPECT_CALL(*this, OnDurationChanged(kDuration2)).Times((has_audio_ && has_video_) ? 3 : 2);
...
TEST_P(WatchTimeReporterTest, WatchTimeReporterUnderflow)
{
    ...
    EXPECT_CALL(*this, OnUnderflowUpdate(1)).Times((has_audio_ && has_video_) ? 2 : 1);
    EXPECT_CALL(*this, OnUnderflowDurationUpdate(1, kUnderflowDuration));
    CycleReportingTimer();
...
TEST_P(WatchTimeReporterTest, WatchTimeReporterSecondaryProperties)
{
    ...
    auto properties = media::mojom::SecondaryPlaybackProperties::New(has_audio_ ? media::AudioCodec::kAAC : media::AudioCodec::kUnknown,
        has_video_ ? media::VideoCodec::kH264 : media::VideoCodec::kUnknown,
        has_audio_ ? media::AudioCodecProfile::kXHE_AAC : media::AudioCodecProfile::kUnknown,
        has_video_ ? media::H264PROFILE_MAIN : media::VIDEO_CODEC_PROFILE_UNKNOWN,
        has_audio_ ? media::AudioDecoderType::kMojo : media::AudioDecoderType::kUnknown,
        has_video_ ? media::VideoDecoderType::kMojo : media::VideoDecoderType::kUnknown,
        has_audio_ ? media::EncryptionScheme::kCenc : media::EncryptionScheme::kUnencrypted,
        has_video_ ? media::EncryptionScheme::kCbcs : media::EncryptionScheme::kUnencrypted, has_video_ ? gfx::Size(800, 600) : gfx::Size());
    ...
    EXPECT_CALL(*this, OnUpdateSecondaryProperties(_)).Times((has_audio_ && has_video_) ? 3 : 2).WillRepeatedly([properies_ptr](auto secondary_properties) {
...
TEST_P(WatchTimeReporterTest, SecondaryProperties_SizeIncreased)
{
    ...
    EXPECT_CALL(*this, OnUpdateSecondaryProperties(_)).Times((has_audio_ && has_video_) ? 3 : 2);
    wtr_->UpdateSecondaryProperties(media::mojom::SecondaryPlaybackProperties::New(media::AudioCodec::kUnknown, media::VideoCodec::kUnknown,
        media::AudioCodecProfile::kUnknown, media::VIDEO_CODEC_PROFILE_UNKNOWN, media::AudioDecoderType::kUnknown, media::VideoDecoderType::kUnknown,
        media::EncryptionScheme::kUnencrypted, media::EncryptionScheme::kUnencrypted, kSizeJustRight));
    EXPECT_TRUE(IsMonitoring());
...
TEST_P(WatchTimeReporterTest, SecondaryProperties_SizeDecreased)
{
    ...
    wtr_->UpdateSecondaryProperties(media::mojom::SecondaryPlaybackProperties::New(media::AudioCodec::kUnknown, media::VideoCodec::kUnknown,
        media::AudioCodecProfile::kUnknown, media::VIDEO_CODEC_PROFILE_UNKNOWN, media::AudioDecoderType::kUnknown, media::VideoDecoderType::kUnknown,
        media::EncryptionScheme::kUnencrypted, media::EncryptionScheme::kUnencrypted, kSizeTooSmall));
    EXPECT_WATCH_TIME_FINALIZED();
    CycleReportingTimer();
...
TEST_P(WatchTimeReporterTest, WatchTimeReporterAutoplayInitiated)
{
    ...
    EXPECT_CALL(*this, OnSetAutoplayInitiated(true)).Times((has_audio_ && has_video_) ? 3 : 2);
...
TEST_P(WatchTimeReporterTest, WatchTimeReporterShownHiddenBackground)
{
    ...
    const base::TimeDelta kExpectedForegroundWatchTime = kWatchTimeLate - kWatchTimeEarly;
    EXPECT_WATCH_TIME(Ac, kExpectedForegroundWatchTime);
    EXPECT_WATCH_TIME(All, kExpectedForegroundWatchTime);
    EXPECT_WATCH_TIME(Eme, kExpectedForegroundWatchTime);
    EXPECT_WATCH_TIME(Mse, kExpectedForegroundWatchTime);
...
TEST_P(DisplayTypeWatchTimeReporterTest, WatchTimeReporterHiddenDisplayTypeBackground)
{
    constexpr base::TimeDelta kWatchTime1 = base::Seconds(8);
    constexpr base::TimeDelta kWatchTime2 = base::Seconds(16);
    EXPECT_CALL(*this, GetCurrentMediaTime())
...
TEST_P(WatchTimeReporterTest, WatchTimeReporterMultiplePartialFinalize)
{
    ...
    if (has_video_) {
        ...
        EXPECT_WATCH_TIME(Eme, kWatchTime2);
        EXPECT_WATCH_TIME(Mse, kWatchTime2);
        EXPECT_WATCH_TIME_IF_VIDEO(DisplayPictureInPicture, kWatchTime2 - kWatchTime1);
        EXPECT_WATCH_TIME(NativeControlsOn, kWatchTime2 - kWatchTime1);
        EXPECT_WATCH_TIME(Battery, kWatchTime2 - kWatchTime1);
        EXPECT_WATCH_TIME_FINALIZED();
        CycleReportingTimer();
...
TEST_P(WatchTimeReporterTest, WatchTimeReporterNonZeroStart)
{
    ...
    EXPECT_CALL(*this, GetCurrentMediaTime()).WillOnce(testing::Return(kWatchTime1)).WillRepeatedly(testing::Return(kWatchTime2));
    Initialize(true, true, kSizeJustRight);
    wtr_->OnPlaying();
    EXPECT_TRUE(IsMonitoring());
...
TEST_P(WatchTimeReporterTest, SeekFinalizes)
{
    ...
    EXPECT_CALL(*this, GetCurrentMediaTime()).WillOnce(testing::Return(base::TimeDelta())).WillOnce(testing::Return(kWatchTime));
    Initialize(true, true, kSizeJustRight);
    wtr_->OnPlaying();
    EXPECT_TRUE(IsMonitoring());
...
{
    constexpr base::TimeDelta kWatchTime = base::Seconds(10);
    EXPECT_CALL(*this, GetCurrentMediaTime()).WillOnce(testing::Return(base::TimeDelta())).WillRepeatedly(testing::Return(kWatchTime));
    Initialize(true, true, kSizeJustRight);
    wtr_->OnPlaying();
    EXPECT_TRUE(IsMonitoring());
...
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizeExitDoesNotRequireCurrentTime | kStartOnBattery>([this]() {
...
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizePowerWatchTime>([this]() { OnPowerStateChange(true); });
}
...
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizePowerWatchTime | kTransitionPowerWatchTime>([this]() { OnPowerStateChange(true); });
}

TEST_P(WatchTimeReporterTest, OnControlsChangeHysteresisNativeContinuation)
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizeExitDoesNotRequireCurrentTime | kStartWithNativeControls>([this]() {
...
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizeControlsWatchTime | kStartWithNativeControls>([this]() { OnNativeControlsEnabled(false); });
}
...
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizeControlsWatchTime>([this]() { OnNativeControlsEnabled(true); });
}

TEST_P(WatchTimeReporterTest, OnControlsChangeToNativeOff)
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizeControlsWatchTime | kStartWithNativeControls | kTransitionControlsWatchTime>(
        [this]() { OnNativeControlsEnabled(false); });
}

TEST_P(WatchTimeReporterTest, OnControlsChangeToNative)
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizeControlsWatchTime | kTransitionControlsWatchTime>([this]() { OnNativeControlsEnabled(true); });
}

TEST_P(DisplayTypeWatchTimeReporterTest, OnDisplayTypeChangeHysteresisFullscreenContinuation)
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizeExitDoesNotRequireCurrentTime | kStartWithDisplayFullscreen>([this]() {
        OnDisplayTypeChanged(DisplayType::kInline);
        OnDisplayTypeChanged(DisplayType::kFullscreen);
    });
}

TEST_P(DisplayTypeWatchTimeReporterTest, OnDisplayTypeChangeHysteresisNativeFinalized)
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizeDisplayWatchTime | kStartWithDisplayFullscreen>(
        [this]() { OnDisplayTypeChanged(DisplayType::kInline); });
}

TEST_P(DisplayTypeWatchTimeReporterTest, OnDisplayTypeChangeHysteresisInlineContinuation)
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizeExitDoesNotRequireCurrentTime>([this]() {
        OnDisplayTypeChanged(DisplayType::kFullscreen);
        OnDisplayTypeChanged(DisplayType::kInline);
...
TEST_P(DisplayTypeWatchTimeReporterTest, OnDisplayTypeChangeHysteresisNativeOffFinalized)
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizeDisplayWatchTime>([this]() { OnDisplayTypeChanged(DisplayType::kFullscreen); });
}

TEST_P(DisplayTypeWatchTimeReporterTest, OnDisplayTypeChangeInlineToFullscreen)
{
    RunHysteresisTest<kAccumulationContinuesAfterTest | kFinalizeDisplayWatchTime | kStartWithDisplayFullscreen | kTransitionDisplayWatchTime>(
        [this]() { OnDisplayTypeChanged(DisplayType::kInline); });
}

TEST_P(DisplayTypeWatchTimeReporterTest, OnDisplayTypeChangeFullscreenToInline)
{
...
TEST_P(WatchTimeReporterTest, HysteresisPartialExitStillFinalizes)
{
    ...
    for (size_t i = 0; i < kTestSize; ++i) {
        for (size_t j = 0; j < kTestSize; ++j) {
            ...
            RunHysteresisTest<kFinalizeInterleavedStartEvent>([i, j, start_event, stop_event]() {
                stop_event(i);
                stop_event(j);
                start_event(i);
            });
...
TEST_P(WatchTimeReporterTest, WatchTimeReporterMediaFoundation)
{
    ...
    // added.
    if (has_audio_ && has_video_) {
        EXPECT_CALL(*this, GetCurrentMediaTime()).WillOnce(testing::Return(base::TimeDelta())).WillRepeatedly(testing::Return(kWatchTimeEarly));
        Initialize(true, true, kSizeJustRight, media::RendererType::kMediaFoundation);
        wtr_->OnPlaying();
...
TEST_P(WatchTimeReporterTest, WatchTimeReporterMediaFoundationNoEme)
{
    ...
    if (has_audio_ && has_video_) {
        EXPECT_CALL(*this, GetCurrentMediaTime()).WillOnce(testing::Return(base::TimeDelta())).WillRepeatedly(testing::Return(kWatchTimeEarly));
        Initialize(true, false, kSizeJustRight, media::RendererType::kMediaFoundation);
        wtr_->OnPlaying();
...
TEST_P(MutedWatchTimeReporterTest, MuteUnmute)
{
    ...
    const base::TimeDelta kExpectedUnmutedWatchTime = kWatchTimeLate - kWatchTimeEarly;
...
TEST_P(MutedWatchTimeReporterTest, MutedControls)
{
    ...
    EXPECT_MUTED_WATCH_TIME_IF_AUDIO_VIDEO(Mse, kWatchTime2);
    EXPECT_MUTED_WATCH_TIME_IF_AUDIO_VIDEO(DisplayInline, kWatchTime2);
    EXPECT_MUTED_WATCH_TIME_IF_AUDIO_VIDEO(NativeControlsOn, kWatchTime2 - kWatchTime1);
    EXPECT_WATCH_TIME_FINALIZED();
    CycleReportingTimer();
...
TEST_P(MutedWatchTimeReporterTest, MutedDisplayType)
{
    ...
    EXPECT_MUTED_WATCH_TIME_IF_AUDIO_VIDEO(Eme, kWatchTime2);
    EXPECT_MUTED_WATCH_TIME_IF_AUDIO_VIDEO(Mse, kWatchTime2);
    EXPECT_MUTED_WATCH_TIME_IF_AUDIO_VIDEO(DisplayFullscreen, kWatchTime2 - kWatchTime1);
    EXPECT_MUTED_WATCH_TIME_IF_AUDIO_VIDEO(NativeControlsOff, kWatchTime2);
    EXPECT_WATCH_TIME_FINALIZED();
    CycleReportingTimer();
...
INSTANTIATE_TEST_SUITE_P(WatchTimeReporterTest, WatchTimeReporterTest,
...
INSTANTIATE_TEST_SUITE_P(DisplayTypeWatchTimeReporterTest, DisplayTypeWatchTimeReporterTest,
    testing::ValuesIn({ // has_video, has_audio
        std::make_tuple(true, true),
        // has_video
        std::make_tuple(true, false) }));
...
INSTANTIATE_TEST_SUITE_P(MutedWatchTimeReporterTest, MutedWatchTimeReporterTest,
    testing::ValuesIn({
        // has_video, has_audio
        std::make_tuple(true, true),
    }));
