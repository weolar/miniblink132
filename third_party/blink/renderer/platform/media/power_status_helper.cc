...
namespace blink {
PowerStatusHelper::PowerStatusHelper(CreateBatteryMonitorCB create_battery_monitor_cb)
...
// static
std::optional<int> PowerStatusHelper::BucketFor(bool is_playing, bool has_video, media::VideoCodec codec, media::VideoCodecProfile profile,
    gfx::Size natural_size, bool is_fullscreen, std::optional<int> average_fps)
{
    if (!is_playing)
        return {};
...
void PowerStatusHelper::OnAnyStateChange()
{
    ...
    if (experiment_state_) {
        current_bucket_ = BucketFor(is_playing_, has_video_, codec_, profile_, natural_size_, is_fullscreen_, average_fps_);
    }
...
void PowerStatusHelper::OnBatteryStatus(device::mojom::blink::BatteryStatusPtr battery_status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void PowerStatusHelper::QueryNextStatus()
{
    ...
    battery_monitor_->QueryNextStatus(base::BindOnce(&PowerStatusHelper::OnBatteryStatus, base::Unretained(this)));
}
