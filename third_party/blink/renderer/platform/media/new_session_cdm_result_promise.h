...
using SessionInitializedCB = base::OnceCallback<void(const std::string& session_id, SessionInitStatus* status)>;
...
class PLATFORM_EXPORT NewSessionCdmResultPromise : public media::CdmPromiseTemplate<std::string> {
public:
    NewSessionCdmResultPromise(const WebContentDecryptionModuleResult& result, const std::string& key_system_uma_prefix, const std::string& uma_name,
        SessionInitializedCB new_session_created_cb, const std::vector<SessionInitStatus>& expected_statuses);
    NewSessionCdmResultPromise(const NewSessionCdmResultPromise&) = delete;
    NewSessionCdmResultPromise& operator=(const NewSessionCdmResultPromise&) = delete;
