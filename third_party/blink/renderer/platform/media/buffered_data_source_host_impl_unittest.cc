...
public:
    BufferedDataSourceHostImplTest()
        : host_(base::BindRepeating(&BufferedDataSourceHostImplTest::ProgressCallback, base::Unretained(this)), &clock_)
    {
    }
    BufferedDataSourceHostImplTest(const BufferedDataSourceHostImplTest&) = delete;
    BufferedDataSourceHostImplTest& operator=(const BufferedDataSourceHostImplTest&) = delete;

    void Add()
    {
        host_.AddBufferedTimeRanges(&ranges_, base::Seconds(10));
    }

    void ProgressCallback()
    {
        progress_callback_calls_++;
    }
...
TEST_F(BufferedDataSourceHostImplTest, CanPlayThrough)
{
    ...
    EXPECT_EQ(100000, host_.UnloadedBytesInInterval(Interval<int64_t>(0, 100000)));
    ...
    EXPECT_FALSE(host_.CanPlayThrough(base::TimeDelta(), base::Seconds(0.01), 1.0));
    // If the video is 1000s we can play through.
    EXPECT_TRUE(host_.CanPlayThrough(base::TimeDelta(), base::Seconds(1000.0), 1.0));
    // No more downloads for 1000 seconds...
    clock_.Advance(base::Seconds(1000));
    // Can't play through..
    EXPECT_FALSE(host_.CanPlayThrough(base::TimeDelta(), base::Seconds(100.0), 1.0));
    ...
    EXPECT_TRUE(host_.CanPlayThrough(base::TimeDelta(), base::Seconds(0.01), 1.0));
}
