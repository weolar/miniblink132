...
public:
    FakeMultiBufferDataProvider(
        MultiBufferBlockId pos, size_t file_size, int max_blocks_after_defer, bool must_read_whole_file, MultiBuffer* multibuffer, media::TestRandom* rnd)
        : pos_(pos)
    ...
    bool Advance()
    {
        ...
        bool ret = true;
        auto block = base::MakeRefCounted<media::DataBuffer>(static_cast<int>(kBlockSize));
        size_t x = 0;
...
public:
    explicit TestMultiBuffer(int32_t shift, const scoped_refptr<MultiBuffer::GlobalLRU>& lru, media::TestRandom* rnd)
    ...
    void CheckPresentState()
    {
        ...
        for (auto i = data_.begin(); i != data_.end(); ++i) {
            ...
            CHECK_NE(!!pinned_[i->first], lru_->Contains(this, i->first)) << " i->first = " << i->first;
        ...
        while (tmp_iterator != tmp.end() && present_iterator != present_.end()) {
            EXPECT_EQ(tmp_iterator.interval_begin(), present_iterator.interval_begin());
...
protected:
    std::unique_ptr<DataProvider> CreateWriter(const MultiBufferBlockId& pos, bool) override
    {
        DCHECK(create_ok_);
        writers_created_++;
        CHECK_LT(writers.size(), max_writers_);
        return std::make_unique<FakeMultiBufferDataProvider>(pos, file_size_, max_blocks_after_defer_, must_read_whole_file_, this, rnd_);
    }
...
public:
    MultiBufferTest()
        : rnd_(42)
        , task_runner_(base::MakeRefCounted<media::FakeSingleThreadTaskRunner>(&clock_))
...
TEST_F(MultiBufferTest, ReadAll)
{
    ...
    MultiBufferReader reader(&multibuffer_, pos, end,
        /*is_client_audio_element=*/false, base::NullCallback(), task_runner_);
...
class ReadHelper {
public:
    ReadHelper(size_t end, size_t max_read_size, MultiBuffer* multibuffer, media::TestRandom* rnd, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
        : pos_(0)
        , end_(end)
        ...
        , reader_(multibuffer, pos_, end_,
              /*is_client_audio_element=*/false, base::NullCallback(), std::move(task_runner))
    {
    ...
    void StartRead()
    {
        ...
        if (!Read()) {
            reader_.Wait(read_size_, base::BindOnce(&ReadHelper::WaitCB, base::Unretained(this)));
        }
    }
...
TEST_F(MultiBufferTest, RandomTest)
{
    ...
    for (size_t i = 0; i < 20; i++) {
        read_helpers.push_back(std::make_unique<ReadHelper>(file_size, 1000, &multibuffer_, &rnd_, task_runner_));
    }
    for (int i = 0; i < 100; i++) {
        for (int j = 0; j < 100; j++) {
            if (rnd_.Rand() & 1) {
