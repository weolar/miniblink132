...
public:
    ...
    public:
        ...
        mojo::PendingRemote<device::mojom::blink::BatteryMonitor> GetBatteryMonitor()
        {
            ...
            case RemoteType::kDisconnected: {
                ...
                receiver_.set_disconnect_handler(base::BindOnce(&MockBatteryMonitor::DidDisconnect, base::Unretained(this)));
        ...
        {
            return base::BindRepeating(&MockBatteryMonitor::GetBatteryMonitor, base::Unretained(this));
        }
        ...
        void ProvidePowerUpdate(bool is_charging, float current_level)
        {
            ...
            device::mojom::blink::BatteryStatusPtr status = device::mojom::blink::BatteryStatus::New(is_charging,
                /*charging_time=*/0,
                /*discharging_time=*/0, current_level);
    ...
    int MakeRecordable(bool alternate = false)
    {
        ...
        metadata.video_decoder_config = media::VideoDecoderConfig(media::VideoCodec::kH264, media::H264PROFILE_MAIN,
            media::VideoDecoderConfig::AlphaMode::kIsOpaque, media::VideoColorSpace(), media::VideoTransformation(), gfx::Size(0, 0), /* coded_size */
        ...
        return PowerStatusHelper::kCodecBitsH264 | PowerStatusHelper::kResolution360p | PowerStatusHelper::kFrameRate60
            | (alternate ? PowerStatusHelper::kFullScreenYes : PowerStatusHelper::kFullScreenNo);
    }
...
class PowerStatusHelperBucketTest : public testing::TestWithParam<PlaybackParamsTuple> {
public:
    std::optional<int> BucketFor(bool is_playing, bool has_video, media::VideoCodec codec, media::VideoCodecProfile profile, gfx::Size coded_size,
        bool is_fullscreen, std::optional<int> average_fps)
    {
        return PowerStatusHelper::BucketFor(is_playing, has_video, codec, profile, coded_size, is_fullscreen, average_fps);
    }
};
...
TEST_P(PowerStatusHelperBucketTest, TestBucket)
{
    ...
    bool is_fullscreen = (std::get<5>(GetParam()) == PowerStatusHelper::Bits::kFullScreenYes);

    auto bucket = BucketFor(is_playing, has_video, codec, profile, coded_size, is_fullscreen, average_fps);
    if (!expect_bucket) {
        EXPECT_FALSE(bucket);
    } else {
        EXPECT_EQ(*bucket, std::get<2>(GetParam()) | std::get<3>(GetParam()) | std::get<4>(GetParam()) | std::get<5>(GetParam()));
    }
}

// Instantiate all valid combinations, plus some that aren't.
INSTANTIATE_TEST_SUITE_P(All, PowerStatusHelperBucketTest,
    Combine(Bool(), Bool(),
        Values(PowerStatusHelper::Bits::kCodecBitsH264, PowerStatusHelper::Bits::kCodecBitsVP9Profile0, PowerStatusHelper::Bits::kCodecBitsVP9Profile2,
            PowerStatusHelper::Bits::kNotAValidBitForTesting),
        Values(PowerStatusHelper::Bits::kResolution360p, PowerStatusHelper::Bits::kResolution720p, PowerStatusHelper::Bits::kResolution1080p,
            PowerStatusHelper::Bits::kNotAValidBitForTesting),
        Values(PowerStatusHelper::Bits::kFrameRate30, PowerStatusHelper::Bits::kFrameRate60, PowerStatusHelper::Bits::kNotAValidBitForTesting),
        Values(PowerStatusHelper::Bits::kFullScreenNo, PowerStatusHelper::Bits::kFullScreenYes)));
