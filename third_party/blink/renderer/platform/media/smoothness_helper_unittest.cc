...
#define OPT_TARGET(m) ResultOf([](const std::optional<TargetValue>& v) { return (*v).value(); }, m)

// Same as above, but expects an ObservationCompletion.
#define COMPLETION_TARGET(m) ResultOf([](const ObservationCompletion& x) { return x.target_value.value(); }, m)
...
class SmoothnessHelperTest : public testing::Test {
    ...
    public:
        MOCK_METHOD4(BeginObservation,
            void(base::UnguessableToken id, const FeatureVector& features, const std::optional<TargetValue>& default_target,
...
public:
    void SetUp() override
    {
        ...
        helper_ = SmoothnessHelper::Create(std::move(bad_ltc), std::move(nnr_ltc), features_, &client_);
