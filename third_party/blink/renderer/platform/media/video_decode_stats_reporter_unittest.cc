...
const media::CdmConfig kDefaultCdmConfig = { kDefaultKeySystem, false, false, kDefaultUseHwSecureCodecs };
...
media::VideoDecoderConfig MakeVideoConfig(media::VideoCodec codec, media::VideoCodecProfile profile, gfx::Size natural_size)
{
    gfx::Size coded_size = natural_size;
    gfx::Rect visible_rect(coded_size.width(), coded_size.height());
    return media::VideoDecoderConfig(codec, profile, media::VideoDecoderConfig::AlphaMode::kIsOpaque, media::VideoColorSpace::JPEG(), media::kNoTransformation,
        coded_size, visible_rect, natural_size, media::EmptyExtraData(), media::EncryptionScheme::kUnencrypted);
}

media::PipelineStatistics MakeStats(int frames_decoded, int frames_dropped, int power_efficient_decoded_frames, double fps)
{
...
public:
    ...
    {
        MockStartNewRecord(features->profile, features->video_size, features->frames_per_sec, features->key_system, features->use_hw_secure_codecs);
    }

    MOCK_METHOD5(MockStartNewRecord,
        void(media::VideoCodecProfile profile, const gfx::Size& natural_size, int frames_per_sec, std::string key_system, bool use_hw_secure_codecs));

    void UpdateRecord(media::mojom::PredictionTargetsPtr targets) override
    {
        MockUpdateRecord(targets->frames_decoded, targets->frames_dropped, targets->frames_power_efficient);
    }

    MOCK_METHOD3(MockUpdateRecord, void(uint32_t frames_decoded, uint32_t frames_dropped, uint32_t frames_power_efficient));
...
public:
    ...
    media::PipelineStatistics MakeAdvancingDecodeStats()
    {
        ...
        return MakeStats(pipeline_decoded_frames_, pipeline_dropped_frames_, pipeline_power_efficient_frames_, pipeline_framerate_);
    }
    ...
    {
        return MakeStats(pipeline_decoded_frames_ + kDecodeCountIncrement, pipeline_dropped_frames_ + kDroppedCountIncrement,
            pipeline_power_efficient_frames_ + kDecodePowerEfficientCountIncrement, pipeline_framerate_);
    }
...
protected:
    ...
    mojo::PendingRemote<media::mojom::VideoDecodeStatsRecorder> SetupRecordInterceptor(raw_ptr<RecordInterceptor>* interceptor_ptr)
    {
        ...
        *interceptor_ptr = interceptor.get();
        ...
        mojo::MakeSelfOwnedReceiver(std::move(interceptor), recorder_remote.InitWithNewPipeAndPassReceiver());
        EXPECT_TRUE(recorder_remote.is_valid());
        return recorder_remote;
    }

    // Inject mock objects and create a new |reporter_| to test.
    void MakeReporter(media::VideoCodecProfile profile = kDefaultProfile, const gfx::Size& natural_size = gfx::Size(kDefaultWidth, kDefaultHeight),
        const std::optional<media::CdmConfig> cdm_config = kDefaultCdmConfig)
    {
        reporter_ = std::make_unique<VideoDecodeStatsReporter>(SetupRecordInterceptor(&interceptor_),
            base::BindRepeating(&VideoDecodeStatsReporterTest::GetPipelineStatsCB, base::Unretained(this)), profile, natural_size, cdm_config,
            task_environment_.GetMainThreadTaskRunner(), task_environment_.GetMockTickClock());
    }
    ...
    {
        return reporter_->ShouldBeReporting();
    }
    ...
    void StartPlayingAndStabilizeFramerate(media::VideoCodecProfile expected_profile = kDefaultProfile,
        gfx::Size expected_natural_size = gfx::Size(kDefaultWidth, kDefaultHeight), int expected_fps = kDefaultFps,
        std::string expected_key_system = kDefaultKeySystem, bool expected_use_hw_secure_codecs = kDefaultUseHwSecureCodecs)
    {
        ...
        // constant framerate.
        ON_CALL(*this, GetPipelineStatsCB()).WillByDefault(Invoke(this, &VideoDecodeStatsReporterTest::MakeAdvancingDecodeStats));
        ...
        StabilizeFramerateAndStartNewRecord(expected_profile, expected_natural_size, expected_fps, expected_key_system, expected_use_hw_secure_codecs);
    }
    ...
    void StabilizeFramerateAndStartNewRecord(media::VideoCodecProfile expected_profile, gfx::Size expected_natural_size, int expected_fps,
        std::string expected_key_system, bool expected_use_hw_secure_codecs, FpsStabiliaztionSpeed fps_timer_speed = FAST_STABILIZE_FPS)
    {
        ...
        while (CurrentStableFpsSamples() < kRequiredStableFpsSamples) {
            ...
            if (CurrentStableFpsSamples() == kRequiredStableFpsSamples - 1) {
                EXPECT_CALL(*interceptor_,
                    MockStartNewRecord(expected_profile, expected_natural_size, expected_fps, expected_key_system, expected_use_hw_secure_codecs));
            }

            if (fps_timer_speed == FAST_STABILIZE_FPS) {
                // Generally FPS is stabilized with a timer of ~3x the average frame
                // duration.
                base::TimeDelta frame_druation = base::Seconds(1.0 / pipeline_framerate_);
    ...
    void AdvanceTimeAndVerifyRecordUpdate(int decoded_frames_offset, int dropped_frames_offset, int decoded_power_efficient_offset)
    {
        DCHECK(ShouldBeReporting());
        ...
        EXPECT_GT(next_stats.video_frames_decoded_power_efficient, pipeline_power_efficient_frames_);
        ...
        EXPECT_CALL(*this, GetPipelineStatsCB());
        EXPECT_CALL(*interceptor_,
            MockUpdateRecord(next_stats.video_frames_decoded - decoded_frames_offset, next_stats.video_frames_dropped - dropped_frames_offset,
                next_stats.video_frames_decoded_power_efficient - decoded_power_efficient_offset));
        FastForward(kRecordingInterval);
    ...
    static const int kRequiredStableFpsSamples = VideoDecodeStatsReporter::kRequiredStableFpsSamples;
    static const int kMaxUnstableFpsChanges = VideoDecodeStatsReporter::kMaxUnstableFpsChanges;
    static const int kMaxTinyFpsWindows = VideoDecodeStatsReporter::kMaxTinyFpsWindows;
};

const base::TimeDelta VideoDecodeStatsReporterTest::kRecordingInterval = base::Milliseconds(VideoDecodeStatsReporter::kRecordingIntervalMs);
const base::TimeDelta VideoDecodeStatsReporterTest::kTinyFpsWindowDuration = base::Milliseconds(VideoDecodeStatsReporter::kTinyFpsWindowMs);
...
TEST_F(VideoDecodeStatsReporterTest, RecordWhilePlaying)
{
    ...
    // Verify that UpdateRecord calls come at the recording interval with
    // correct values.
    AdvanceTimeAndVerifyRecordUpdate(decoded_offset, dropped_offset, decoded_power_efficient_offset);
...
TEST_F(VideoDecodeStatsReporterTest, RecordingStopsWhenHidden)
{
    ...
    EXPECT_CALL(*this, GetPipelineStatsCB());
    EXPECT_CALL(*interceptor_, MockStartNewRecord(kDefaultProfile, kDefaultSize_, kDefaultFps, kDefaultKeySystem, kDefaultUseHwSecureCodecs));
    reporter_->OnShown();
...
TEST_F(VideoDecodeStatsReporterTest, RecordingStopsWhenNoDecodeProgress)
{
    ...
    ON_CALL(*this, GetPipelineStatsCB())
        .WillByDefault(Return(MakeStats(pipeline_decoded_frames_, pipeline_dropped_frames_, pipeline_power_efficient_frames_, pipeline_framerate_)));
...
TEST_F(VideoDecodeStatsReporterTest, NewRecordStartsForFpsChange)
{
    ...
    StabilizeFramerateAndStartNewRecord(kDefaultProfile, kDefaultSize_, kDefaultFps * 2, kDefaultKeySystem, kDefaultUseHwSecureCodecs);
...
TEST_F(VideoDecodeStatsReporterTest, FpsStabilizationFailed)
{
    ...
    for (; num_fps_samples < kMaxUnstableFpsChanges; num_fps_samples++) {
        pipeline_framerate_ = (pipeline_framerate_ == kDefaultFps) ? 2 * kDefaultFps : kDefaultFps;
        EXPECT_CALL(*this, GetPipelineStatsCB());
        FastForward(CurrentStatsCbInterval());
    }
...
TEST_F(VideoDecodeStatsReporterTest, FpsStabilizationFailed_TinyWindows)
{
    ...
    for (int i = 0; i < kMaxTinyFpsWindows; i++) {
        StabilizeFramerateAndStartNewRecord(kDefaultProfile, kDefaultSize_, pipeline_framerate_, kDefaultKeySystem, kDefaultUseHwSecureCodecs);
...
TEST_F(VideoDecodeStatsReporterTest, ThrottleFpsTimerIfNoDecodeProgress)
{
    ...
    for (; stable_fps_samples < kRequiredStableFpsSamples / 2; stable_fps_samples++) {
    ...
    StabilizeFramerateAndStartNewRecord(kDefaultProfile, kDefaultSize_, kDefaultFps, kDefaultKeySystem, kDefaultUseHwSecureCodecs, SLOW_STABILIZE_FPS);
...
TEST_F(VideoDecodeStatsReporterTest, FpsBucketing)
{
    ...
    StabilizeFramerateAndStartNewRecord(kDefaultProfile, kDefaultSize_, bucketed_fps, kDefaultKeySystem, kDefaultUseHwSecureCodecs);
...
TEST_F(VideoDecodeStatsReporterTest, ResolutionBucketing)
{
    ...
    slightly_larger_size = gfx::Size(slightly_larger_size.width() + 1, slightly_larger_size.height() + 1);
    EXPECT_TRUE(reporter_->MatchesBucketedNaturalSize(slightly_larger_size));
...
TEST_F(VideoDecodeStatsReporterTest, ResolutionTooSmall)
{
    ...
    StartPlayingAndStabilizeFramerate(kDefaultProfile, media::GetSizeBucket(small_size));
...
TEST_F(VideoDecodeStatsReporterTest, VaryEmeProperties)
{
    ...
    const media::CdmConfig kNonDefaultCdmConfig = { kFooKeySystem, false, false, kNonDefaultHwSecureCodecs };
    ...
    StartPlayingAndStabilizeFramerate(kDefaultProfile, kDefaultSize, kDefaultFps, kEmptyKeySystem, kNonDefaultHwSecureCodecs);
    ...
    StartPlayingAndStabilizeFramerate(kDefaultProfile, kDefaultSize, kDefaultFps, kFooKeySystem, kNonDefaultHwSecureCodecs);
}
...
TEST_F(VideoDecodeStatsReporterTest, SanitizeFrameCounts)
{
    ...
    EXPECT_CALL(*this, GetPipelineStatsCB())
        .WillOnce(Return(MakeStats(pipeline_decoded_frames_, pipeline_dropped_frames_, pipeline_power_efficient_frames_, pipeline_framerate_)));

    // Expect that record update caps dropped and power efficient counts to the
    // offset decoded count.
    EXPECT_CALL(*interceptor_,
        MockUpdateRecord(pipeline_decoded_frames_ - decoded_offset, pipeline_decoded_frames_ - decoded_offset, pipeline_decoded_frames_ - decoded_offset));
    FastForward(kRecordingInterval);
    testing::Mock::VerifyAndClearExpectations(this);
    testing::Mock::VerifyAndClearExpectations(interceptor_);
    ...
    EXPECT_CALL(*interceptor_,
        MockUpdateRecord(pipeline_decoded_frames_ - decoded_offset, pipeline_dropped_frames_ - dropped_offset,
            pipeline_power_efficient_frames_ - decoded_power_efficient_offset));
    FastForward(kRecordingInterval);
}
