...
CdmResultForUMA ConvertCdmExceptionToResultForUMA(media::CdmPromise::Exception exception_code)
{
...
WebContentDecryptionModuleException ConvertCdmException(media::CdmPromise::Exception exception_code)
{
    switch (exception_code) {
    case media::CdmPromise::Exception::NOT_SUPPORTED_ERROR:
...
WebEncryptedMediaKeyInformation::KeyStatus ConvertCdmKeyStatus(media::CdmKeyInformation::KeyStatus key_status)
{
    switch (key_status) {
...
void ReportCdmResultUMA(const std::string& uma_name, uint32_t system_code, CdmResultForUMA result)
{
    if (uma_name.empty())
        return;
