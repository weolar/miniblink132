...
MultiBufferReader::MultiBufferReader(MultiBuffer* multibuffer, int64_t start, int64_t end, bool is_client_audio_element,
    base::RepeatingCallback<void(int64_t, int64_t)> progress_callback, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
void MultiBufferReader::Seek(int64_t pos)
{
    ...
    if (pos == pos_)
        return;
    PinRange(block(pos - max_buffer_backward_), block_ceil(pos + max_buffer_forward_));
...
void MultiBufferReader::SetPinRange(int64_t backward, int64_t forward)
{
    ...
    PinRange(block(pos_ - max_buffer_backward_), block_ceil(pos_ + max_buffer_forward_));
}

int64_t MultiBufferReader::AvailableAt(int64_t pos) const
{
    int64_t unavailable_byte_pos = static_cast<int64_t>(multibuffer_->FindNextUnavailable(block(pos))) << multibuffer_->block_size_shift();
...
int64_t MultiBufferReader::TryReadAt(int64_t pos, uint8_t* data, int64_t len)
{
    ...
    multibuffer_->GetBlocksThreadsafe(block(pos), block_ceil(pos + len), &buffers);
...
void MultiBufferReader::CheckWait()
{
    if (!cb_.is_null() && (Available() >= current_wait_size_ || Available() == -1)) {
        ...
        current_wait_size_ = 0;
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&MultiBufferReader::Call, weak_factory_.GetWeakPtr(), std::move(cb_)));
    }
}
...
void MultiBufferReader::UpdateEnd(MultiBufferBlockId p)
{
    ...
    if (i != multibuffer_->map().end() && i->second->end_of_stream()) {
        ...
        int64_t size_upper_limit = static_cast<int64_t>(p) << multibuffer_->block_size_shift();
        end_ = std::min(end_, size_upper_limit);
    }
}

void MultiBufferReader::NotifyAvailableRange(const Interval<MultiBufferBlockId>& range)
{
    ...
    if (!progress_callback_.is_null()) {
        task_runner_->PostTask(FROM_HERE,
            base::BindOnce(progress_callback_, static_cast<int64_t>(range.begin) << multibuffer_->block_size_shift(),
                (static_cast<int64_t>(range.end) << multibuffer_->block_size_shift()) + multibuffer_->UncommittedBytesAt(range.end)));
    }
}
...
void MultiBufferReader::UpdateInternalState()
{
    ...
    MultiBuffer::BlockId max_preload = block_ceil(std::min(end_, pos_ + std::max(effective_preload, current_wait_size_)));

    DVLOG(3) << "UpdateInternalState"
             << " pp = " << preload_pos_ << " block_ceil(end_) = " << block_ceil(end_) << " end_ = " << end_ << " max_preload " << max_preload;
...
void MultiBufferReader::PinRange(MultiBuffer::BlockId begin, MultiBuffer::BlockId end)
{
