...
template <> struct hash<blink::MultiBufferGlobalBlockId> {
...
public:
    ...
    public:
        ...
        virtual void NotifyAvailableRange(const Interval<MultiBufferBlockId>& range) = 0;
    };
    ...
    void GetBlocksThreadsafe(const BlockId& from, const BlockId& to, std::vector<scoped_refptr<media::DataBuffer>>* output);
    ...
    // Accessors.
    const DataMap& map() const
    {
        return data_;
    }
    int32_t block_size_shift() const
    {
        return block_size_shift_;
    }
...
protected:
    ...
    virtual std::unique_ptr<DataProvider> CreateWriter(const BlockId& pos, bool is_client_audio_element) = 0;
...
private:
    ...
    enum ProviderState { ProviderStateDead, ProviderStateDefer, ProviderStateLoad };
    ...
    void NotifyAvailableRange(const Interval<MultiBufferBlockId>& observer_range, const Interval<MultiBufferBlockId>& new_range);
    ...
    std::map<MultiBufferBlockId, std::set<raw_ptr<Reader, SetExperimental>>> readers_ ALLOW_DISCOURAGED_TYPE("HashMap lacks key sorting");

    // Keeps track of writers by their position.
    // The writers are owned by this class.
    std::map<BlockId, std::unique_ptr<DataProvider>> writer_index_ ALLOW_DISCOURAGED_TYPE("HashMap lacks key sorting");
