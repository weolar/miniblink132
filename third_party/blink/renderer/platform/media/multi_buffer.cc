...
template <class T> static MultiBuffer::BlockId ClosestPreviousEntry(const std::map<MultiBuffer::BlockId, T>& index, MultiBuffer::BlockId pos)
{
...
// from |index|.
template <class T> static MultiBuffer::BlockId ClosestNextEntry(const std::map<MultiBuffer::BlockId, T>& index, MultiBuffer::BlockId pos)
{
...
//
MultiBuffer::GlobalLRU::GlobalLRU(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : max_size_(0)
    , data_size_(0)
...
void MultiBuffer::GlobalLRU::Use(MultiBuffer* multibuffer, MultiBufferBlockId block_id)
{
    lru_.Put(GlobalBlockId { multibuffer, block_id });
    SchedulePrune();
}

void MultiBuffer::GlobalLRU::Insert(MultiBuffer* multibuffer, MultiBufferBlockId block_id)
{
    lru_.Put(GlobalBlockId { multibuffer, block_id });
    SchedulePrune();
}

void MultiBuffer::GlobalLRU::Remove(MultiBuffer* multibuffer, MultiBufferBlockId block_id)
{
...
bool MultiBuffer::GlobalLRU::Contains(MultiBuffer* multibuffer, MultiBufferBlockId block_id)
{
...
void MultiBuffer::GlobalLRU::SchedulePrune()
{
    if (Pruneable() && !background_pruning_pending_) {
        task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&MultiBuffer::GlobalLRU::PruneTask, this), base::Seconds(kBlockPruneInterval));
...
// MultiBuffer
//
MultiBuffer::MultiBuffer(int32_t block_size_shift, scoped_refptr<GlobalLRU> global_lru)
    : max_size_(0)
...
{
    DCHECK(present_[pos] == 0 || present_[pos] == 1) << " pos = " << pos << " present_[pos] " << present_[pos];
...
void MultiBuffer::NotifyAvailableRange(const Interval<MultiBufferBlockId>& observer_range, const Interval<MultiBufferBlockId>& new_range)
{
    std::set<Reader*> tmp;
    for (auto i = readers_.lower_bound(observer_range.begin); i != readers_.end() && i->first < observer_range.end; ++i) {
...
std::unique_ptr<MultiBuffer::DataProvider> MultiBuffer::RemoveProvider(DataProvider* provider)
{
    BlockId pos = provider->Tell();
...
MultiBuffer::ProviderState MultiBuffer::SuggestProviderState(const BlockId& pos) const
{
...
void MultiBuffer::OnDataProviderEvent(DataProvider* provider_tmp)
{
    ...
    if (pos > start_pos) {
        ...
        NotifyAvailableRange(Interval<BlockId>(start_pos, start_pos + 1), Interval<BlockId>(start_pos, start_pos));
    }
...
void MultiBuffer::GetBlocksThreadsafe(const BlockId& from, const BlockId& to, std::vector<scoped_refptr<media::DataBuffer>>* output)
{
    base::AutoLock auto_lock(data_lock_);
...
void MultiBuffer::PinRange(const BlockId& from, const BlockId& to, int32_t how_much)
{
    ...
    while (true) {
        ...
        if (range.value() == 0 || range.value() == how_much) {
            ...
            for (auto present_block_range = present_.find(transition_range.end - 1); present_block_range != present_.begin(); --present_block_range) {
                ...
                if (present_transitioned_range.Empty())
                    break;
                for (BlockId block = present_transitioned_range.end - 1; block >= present_transitioned_range.begin; --block) {
                    DCHECK_GE(block, 0);
