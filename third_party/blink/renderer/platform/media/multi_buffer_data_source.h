...
class PLATFORM_EXPORT MultiBufferDataSource : public media::CrossOriginDataSource {
public:
    ...
    MultiBufferDataSource(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, scoped_refptr<UrlData> url_data, media::MediaLog* media_log,
        BufferedDataSourceHost* host, DownloadingCB downloading_cb);
    ...
    void Read(int64_t position, int size, uint8_t* data, media::DataSource::ReadCB read_cb) override;
    ...
    bool cancel_on_defer_for_testing() const
    {
        return cancel_on_defer_;
    }
...
protected:
    ...
    // parameters.
    void CreateResourceLoader(int64_t first_byte_position, int64_t last_byte_position);

    // Same as above, but called with |lock_| held.
    void CreateResourceLoader_Locked(int64_t first_byte_position, int64_t last_byte_position);
    ...
    media::Tuneable<int> preload_seconds_ = { "SrcMediaMultiBufferPreloadSeconds", 0, 10, 60 };

    // Keep this many seconds of data for going back by default.
    media::Tuneable<int> keep_after_playback_seconds_ = { "SrcMediaMultiBufferKeepAfterPlaybackSeconds", 0, 2, 60 };
