...
ResourceMultiBufferDataProvider::ResourceMultiBufferDataProvider(
    UrlData* url_data, MultiBufferBlockId pos, bool is_client_audio_element, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : pos_(pos)
    , url_data_(url_data)
    , retries_(0)
...
void ResourceMultiBufferDataProvider::Start()
{
    ...
    if (url_data_->length() > 0 && byte_pos() >= url_data_->length()) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&ResourceMultiBufferDataProvider::Terminate, weak_factory_.GetWeakPtr()));
        return;
    }

    // Prepare the request.
    WebURLRequest request(url_data_->url());
    request.SetRequestContext(is_client_audio_element_ ? mojom::RequestContextType::AUDIO : mojom::RequestContextType::VIDEO);
    request.SetRequestDestination(is_client_audio_element_ ? network::mojom::RequestDestination::kAudio : network::mojom::RequestDestination::kVideo);
    request.SetHttpHeaderField(
        WebString::FromUTF8(net::HttpRequestHeaders::kRange), WebString::FromUTF8(net::HttpByteRange::RightUnbounded(byte_pos()).GetHeaderValue()));
    ...
    request.SetHttpHeaderField(WebString::FromUTF8(net::HttpRequestHeaders::kAcceptEncoding), WebString::FromUTF8("identity;q=1, *;q=0"));
    ...
    if (url_data_->cors_mode() != UrlData::CORS_UNSPECIFIED) {
        ...
        options.preflight_policy = network::mojom::CorsPreflightPolicy::kPreventPreflight;
    ...
    active_loader_ = url_data_->url_index()->fetch_context()->CreateUrlLoader(options);
...
bool ResourceMultiBufferDataProvider::WillFollowRedirect(const WebURL& new_url, const WebURLResponse& redirect_response)
{
    DVLOG(1) << "willFollowRedirect";
    redirects_to_ = new_url;
    url_data_->set_valid_until(base::Time::Now() + GetCacheValidUntil(redirect_response));
...
void ResourceMultiBufferDataProvider::DidSendData(uint64_t bytes_sent, uint64_t total_bytes_to_be_sent)
{
    NOTIMPLEMENTED();
}

void ResourceMultiBufferDataProvider::DidReceiveResponse(const WebURLResponse& response)
{
#if DCHECK_IS_ON()
    std::string version;
    ...
    DVLOG(1) << "didReceiveResponse: HTTP/" << version << " " << response.HttpStatusCode();
#endif
    ...
    if (!redirects_to_.IsEmpty()) {
        destination_url_data = url_data_->url_index()->GetByUrl(redirects_to_, cors_mode_, url_data_->cache_lookup_mode());
        redirects_to_ = KURL();
    }

    base::Time last_modified;
    if (base::Time::FromString(response.HttpHeaderField("Last-Modified").Utf8().data(), &last_modified)) {
        destination_url_data->set_last_modified(last_modified);
    }

    destination_url_data->set_etag(response.HttpHeaderField("ETag").Utf8().data());
    ...
    if (!do_fail) {
        destination_url_data = url_data_->url_index()->TryInsert(destination_url_data);
    }

    // This is vital for security!
    destination_url_data->set_is_cors_cross_origin(network::cors::IsCorsCrossOriginResponseType(response_type));

    // Only used for metrics.
    {
        WebString access_control = response.HttpHeaderField("Access-Control-Allow-Origin");
    ...
    destination_url_data->set_passed_timing_allow_origin_check(response.TimingAllowPassed());
    ...
    if (destination_url_data != url_data_.get()) {
        ...
        std::unique_ptr<DataProvider> self(url_data_->multibuffer()->RemoveProvider(this));
    ...
    KURL response_url;
    if (!response.WasFetchedViaServiceWorker() || response.HasUrlListViaServiceWorker()) {
...
void ResourceMultiBufferDataProvider::DidReceiveData(base::span<const char> data)
{
    ...
    while (!data.empty()) {
        if (fifo_.empty() || fifo_.back()->data_size() == block_size()) {
            fifo_.push_back(base::MakeRefCounted<media::DataBuffer>(static_cast<int>(block_size())));
            fifo_.back()->set_data_size(0);
        }
        int last_block_size = fifo_.back()->data_size();
        auto to_append = std::min<int64_t>(data.size(), block_size() - last_block_size);
        DCHECK_GT(to_append, 0);
        memcpy(fifo_.back()->writable_data() + last_block_size, data.data(), static_cast<size_t>(to_append));
    ...
    if (url_data_->length() != kPositionNotSpecified && size < url_data_->length()) {
        if (retries_ < kMaxRetries) {
            ...
            task_runner_->PostDelayedTask(
                FROM_HERE, base::BindOnce(&ResourceMultiBufferDataProvider::Start, weak_factory_.GetWeakPtr()), base::Milliseconds(kLoaderPartialRetryDelayMs));
            return;
        } else {
            url_data_->Fail();
            return; // "this" may be deleted now.
...
void ResourceMultiBufferDataProvider::DidFail(const WebURLError& error)
{
    ...
    if (retries_ < kMaxRetries && pos_ != 0) {
        retries_++;
        task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&ResourceMultiBufferDataProvider::Start, weak_factory_.GetWeakPtr()),
            base::Milliseconds(kLoaderFailedRetryDelayMs + kAdditionalDelayPerRetryMs * retries_));
    } else {
...
bool ResourceMultiBufferDataProvider::ParseContentRange(
    const std::string& content_range_str, int64_t* first_byte_position, int64_t* last_byte_position, int64_t* instance_size)
{
...
bool ResourceMultiBufferDataProvider::VerifyPartialResponse(const WebURLResponse& response, const scoped_refptr<UrlData>& url_data)
{
