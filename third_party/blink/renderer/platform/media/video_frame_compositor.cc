...
VideoFrameCompositor::VideoFrameCompositor(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, std::unique_ptr<WebVideoFrameSubmitter> submitter)
    : task_runner_(task_runner)
    , tick_clock_(base::DefaultTickClock::GetInstance())
    , background_rendering_timer_(FROM_HERE, base::Milliseconds(kBackgroundRenderingTimeoutMs),
          base::BindRepeating(&VideoFrameCompositor::BackgroundRender, base::Unretained(this), RenderingMode::kBackground))
    , force_begin_frames_timer_(
          FROM_HERE, base::Milliseconds(kForceBeginFramesTimeoutMs), base::BindRepeating(&VideoFrameCompositor::StopForceBeginFrames, base::Unretained(this)))
    , submitter_(std::move(submitter))
{
    if (submitter_) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoFrameCompositor::InitializeSubmitter, weak_ptr_factory_.GetWeakPtr()));
        update_submission_state_callback_
            = base::BindPostTask(task_runner_, base::BindRepeating(&VideoFrameCompositor::SetIsSurfaceVisible, weak_ptr_factory_.GetWeakPtr()));
    }
}

cc::UpdateSubmissionStateCB VideoFrameCompositor::GetUpdateSubmissionStateCallback()
{
    return update_submission_state_callback_;
}

void VideoFrameCompositor::SetIsSurfaceVisible(bool is_visible, base::WaitableEvent* done_event)
{
...
void VideoFrameCompositor::EnableSubmission(const viz::SurfaceId& id, media::VideoTransformation transform, bool force_submit)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...
void VideoFrameCompositor::OnRendererStateUpdate(bool new_state)
{
    ...
    if (!auto_open_close_) {
        auto_open_close_ = std::make_unique<base::trace_event::AutoOpenCloseEvent<kTracingCategory>>(
            base::trace_event::AutoOpenCloseEvent<kTracingCategory>::Type::kAsync, "VideoPlayback");
    }
...
scoped_refptr<media::VideoFrame> VideoFrameCompositor::GetCurrentFrameOnAnyThread()
{
...
void VideoFrameCompositor::SetCurrentFrame_Locked(scoped_refptr<media::VideoFrame> frame, base::TimeTicks expected_display_time)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    TRACE_EVENT1("media", "VideoFrameCompositor::SetCurrentFrame", "frame", frame->AsHumanReadableString());
...
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    TRACE_EVENT2("media", "VideoFrameCompositor::UpdateCurrentFrame", "deadline_min", deadline_min, "deadline_max", deadline_max);
...
void VideoFrameCompositor::Start(RenderCallback* callback)
{
    ...
    DCHECK(!callback_);
    callback_ = callback;
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoFrameCompositor::OnRendererStateUpdate, weak_ptr_factory_.GetWeakPtr(), true));
}
...
void VideoFrameCompositor::Stop()
{
    ...
    DCHECK(callback_);
    callback_ = nullptr;
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoFrameCompositor::OnRendererStateUpdate, weak_ptr_factory_.GetWeakPtr(), false));
}

void VideoFrameCompositor::PaintSingleFrame(scoped_refptr<media::VideoFrame> frame, bool repaint_duplicate_frame)
{
    if (!task_runner_->BelongsToCurrentThread()) {
        task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&VideoFrameCompositor::PaintSingleFrame, weak_ptr_factory_.GetWeakPtr(), std::move(frame), repaint_duplicate_frame));
        return;
    }
    if (ProcessNewFrame(std::move(frame), tick_clock_->NowTicks(), repaint_duplicate_frame) && IsClientSinkAvailable()) {
...
void VideoFrameCompositor::UpdateCurrentFrameIfStale(UpdateType type)
{
    ...
    if (type != UpdateType::kBypassClient && IsClientSinkAvailable() && client_->IsDrivingFrameUpdates()) {
        return;
    }
...
void VideoFrameCompositor::SetOnNewProcessedFrameCallback(OnNewProcessedFrameCB cb)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    new_processed_frame_cb_ = std::move(cb);
}

void VideoFrameCompositor::SetOnFramePresentedCallback(OnNewFramePresentedCB present_cb)
{
    base::AutoLock lock(current_frame_lock_);
    new_presented_frame_cb_ = std::move(present_cb);

    task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoFrameCompositor::StartForceBeginFrames, weak_ptr_factory_.GetWeakPtr()));
}
...
std::unique_ptr<WebMediaPlayer::VideoFramePresentationMetadata> VideoFrameCompositor::GetLastPresentedFrameMetadata()
{
...

    {
        base::AutoLock lock(callback_lock_);
        if (callback_) {
            frame_metadata->average_frame_duration = callback_->GetPreferredRenderInterval();
        } else if (last_frame && last_frame->metadata().frame_duration) {
            frame_metadata->average_frame_duration = *last_frame->metadata().frame_duration;
        }
...
bool VideoFrameCompositor::ProcessNewFrame(scoped_refptr<media::VideoFrame> frame, base::TimeTicks presentation_time, bool repaint_duplicate_frame)
{
    DCHECK(task_runner_->BelongsToCurrentThread());

    if (!frame || (GetCurrentFrame() && !repaint_duplicate_frame && frame->unique_id() == GetCurrentFrame()->unique_id())) {
        return false;
    }
    ...
    if (new_processed_frame_cb_) {
        std::move(new_processed_frame_cb_).Run(tick_clock_->NowTicks(), is_frame_readable);
    }
...
base::TimeDelta VideoFrameCompositor::GetLastIntervalWithoutLock() NO_THREAD_SAFETY_ANALYSIS
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...
bool VideoFrameCompositor::CallRender(base::TimeTicks deadline_min, base::TimeTicks deadline_max, RenderingMode mode)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    ...
    if (have_unseen_frame && mode == RenderingMode::kNormal && !was_background_rendering) {
        callback_->OnFrameDropped();
    }

    const bool new_frame = ProcessNewFrame(callback_->Render(deadline_min, deadline_max, mode), deadline_min, false);
