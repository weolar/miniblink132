...
{
    std::string value = request.HttpHeaderField(WebString::FromUTF8(net::HttpRequestHeaders::kAcceptEncoding)).Utf8();
    return (base::Contains(value, "identity;q=1")) && (base::Contains(value, "*;q=0"));
}
...
public:
    ResourceMultiBufferDataProviderTest()
    {
        ...
        ON_CALL(fetch_context_, CreateUrlLoader(_)).WillByDefault(Invoke(this, &ResourceMultiBufferDataProviderTest::CreateUrlLoader));
    }

    ResourceMultiBufferDataProviderTest(const ResourceMultiBufferDataProviderTest&) = delete;
    ResourceMultiBufferDataProviderTest& operator=(const ResourceMultiBufferDataProviderTest&) = delete;
    ...
    void Initialize(const char* url, int first_position)
    {
        ...
        DCHECK(url_data_);
        url_data_->OnRedirect(base::BindOnce(&ResourceMultiBufferDataProviderTest::RedirectCallback, base::Unretained(this)));

        first_position_ = first_position;

        auto loader = std::make_unique<ResourceMultiBufferDataProvider>(
            url_data_.get(), first_position_, false /* is_client_audio_element */, task_environment_.GetMainThreadTaskRunner());
        loader_ = loader.get();
    ...
    void FullResponse(int64_t instance_size, bool ok = true)
    {
        ...
        response.SetHttpHeaderField(WebString::FromUTF8("Content-Length"), WebString::FromUTF8(base::StringPrintf("%" PRId64, instance_size)));
    ...
    void PartialResponse(int64_t first_position, int64_t last_position, int64_t instance_size)
    {
        PartialResponse(first_position, last_position, instance_size, false, true);
    }

    void PartialResponse(int64_t first_position, int64_t last_position, int64_t instance_size, bool chunked, bool accept_ranges)
    {
        WebURLResponse response(url_);
        ...
        if (chunked) {
            response.SetHttpHeaderField(WebString::FromUTF8("Transfer-Encoding"), WebString::FromUTF8("chunked"));
        } else {
    ...
    void Redirect(const char* url)
    {
        ...
        EXPECT_CALL(*this, RedirectCallback(_)).WillOnce(Invoke(this, &ResourceMultiBufferDataProviderTest::SetUrlData));
    ...
    {
        loader_->DidReceiveData(base::as_chars(base::span(data_).subspan(position, size)));
    }
...
protected:
    std::unique_ptr<WebAssociatedURLLoader> CreateUrlLoader(const WebAssociatedURLLoaderOptions& options)
    {
        auto url_loader = std::make_unique<NiceMock<MockWebAssociatedURLLoader>>();
        EXPECT_CALL(*url_loader.get(), LoadAsynchronously(Truly(CorrectAcceptEncoding), loader_.get()));
        return url_loader;
    }
    ...
    UrlIndex url_index_ { &fetch_context_, 0, task_environment_.GetMainThreadTaskRunner() };
...
TEST_F(ResourceMultiBufferDataProviderTest, PartialResponse_ChunkedNoAcceptRanges)
{
    Initialize(kHttpUrl, 100);
    Start();
