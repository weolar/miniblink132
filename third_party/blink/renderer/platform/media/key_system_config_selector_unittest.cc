...
const char kRequireIdentifierAndHwSecureCodecRobustness[] = "require_identifier_and_hw_secure_codec";
const char kRequireIdentifierPersistenceAndHwSecureCodecRobustness[] = "require_identifier_persistence_and_hw_secure_codec";
...
constexpr EncryptionScheme kDisallowHwSecureCodecEncryptionScheme = EncryptionScheme::kCbcs;

media::EncryptionScheme ConvertEncryptionScheme(EncryptionScheme encryption_scheme)
{
...
bool IsCompatibleWithEmeMediaType(EmeMediaType media_type, const std::string& type)
{
...
bool IsSupportedMediaType(const std::string& container_mime_type, const std::string& codecs, bool use_aes_decryptor)
{
    ...
    for (const std::string& codec : codec_vector) {
        DCHECK_NE(codec, kExtendedVideoCodecStripped) << "codecs passed into this function should not be stripped";
...
public:
    ...
    std::string GetBaseKeySystemName(const std::string& key_system) const override
    {
        DCHECK(IsSupportedKeySystem(key_system));
        return key_system == kSupportedSubKeySystem ? kSupportedKeySystem : key_system;
    }

    bool IsSupportedKeySystem(const std::string& key_system) const override
    {
        // Based on EME spec, Clear Key key system is always supported.
        return key_system == media::kClearKeyKeySystem || key_system == kSupportedKeySystem || key_system == kSupportedSubKeySystem;
    }
    ...
    bool IsSupportedInitDataType(const std::string& key_system, EmeInitDataType init_data_type) const override
    {
    ...
    EmeConfig::Rule GetEncryptionSchemeConfigRule(const std::string& key_system, media::EncryptionScheme encryption_scheme) const override
    {
        if (encryption_scheme == ConvertEncryptionScheme(kSupportedEncryptionScheme)) {
    ...
    EmeConfig::Rule GetContentTypeConfigRule(
        const std::string& key_system, EmeMediaType media_type, const std::string& container_mime_type, const std::vector<std::string>& codecs) const override
    {
        DCHECK(IsValidContainerMimeType(container_mime_type)) << "Invalid container mime type should not be passed in";

        if (container_mime_type == kUnsupportedContainer || !IsCompatibleWithEmeMediaType(media_type, container_mime_type)) {
            return EmeConfig::UnsupportedRule();
        }
        ...
        for (const std::string& codec : codecs) {
            ...
            if (codec == kUnsupportedCodec || !IsCompatibleWithEmeMediaType(media_type, codec)) {
                return EmeConfig::UnsupportedRule();
        ...
        const std::string& key_system, EmeMediaType media_type, const std::string& requested_robustness, const bool* hw_secure_requirement) const override
    {
        ...
        if (hw_secure_requirement && *hw_secure_requirement && distinctive_identifier == EmeFeatureSupport::NOT_SUPPORTED) {
            return EmeConfig::UnsupportedRule();
        }
        if (requested_robustness.empty() || requested_robustness == kSupportedRobustness) {
            return EmeConfig::SupportedRule();
        }
        ...
        if (requested_robustness == kRequireIdentifierAndHwSecureCodecRobustness) {
            return EmeConfig { .identifier = EmeConfigRuleState::kRequired, .hw_secure_codecs = EmeConfigRuleState::kRequired };
        }
        if (requested_robustness == kRequireIdentifierPersistenceAndHwSecureCodecRobustness) {
            return EmeConfig {
                .identifier = EmeConfigRuleState::kRequired, .persistence = EmeConfigRuleState::kRequired, .hw_secure_codecs = EmeConfigRuleState::kRequired
            };
        }
    ...
    EmeConfig::Rule GetPersistentLicenseSessionSupport(const std::string& key_system) const override
    {
        return persistent_license;
    }

    EmeFeatureSupport GetPersistentStateSupport(const std::string& key_system) const override
    {
        return persistent_state;
    }

    EmeFeatureSupport GetDistinctiveIdentifierSupport(const std::string& key_system) const override
    {
...
public:
    ...
    void HasPermission(Type type, PermissionStatusCB permission_status_cb) override
    {
        std::move(permission_status_cb).Run(is_granted);
    }

    void RequestPermission(Type type, PermissionStatusCB permission_status_cb) override
    {
        requests++;
        std::move(permission_status_cb).Run(is_granted);
    }
    ...
    {
        return is_encrypted_media_enabled;
    }

#if BUILDFLAG(IS_WIN)
    void IsHardwareSecureDecryptionAllowed(IsHardwareSecureDecryptionAllowedCB cb) override
    {
...
class FakeWebLocalFrameDelegate : public KeySystemConfigSelector::WebLocalFrameDelegate {
public:
    ...
    {
        return is_cross_origin_;
    }
    bool AllowStorageAccessSync(WebContentSettingsClient::StorageType storage_type) override
    {
...
public:
    ...
    KeySystemConfigSelectorTest& operator=(const KeySystemConfigSelectorTest&) = delete;
    ...
    void SelectConfig()
    {
        ...
        KeySystemConfigSelector key_system_config_selector(key_systems_.get(), media_permission_.get(), std::move(web_frame_delegate_));
        ...
        key_system_config_selector.SetIsSupportedMediaTypeCBForTesting(base::BindRepeating(&IsSupportedMediaType));

        key_system_config_selector.SelectConfig(key_system_, configs_, base::BindOnce(&KeySystemConfigSelectorTest::OnConfigSelected, base::Unretained(this)));
    }
    ...
    void OnConfigSelected(KeySystemConfigSelector::Status status, WebMediaKeySystemConfiguration* config, media::CdmConfig* cdm_config)
    {
...
TEST_F(KeySystemConfigSelectorTest, DistinctiveIdentifier_ForcedBlockedByCrossOrigin)
{
    media_permission_->is_granted = true;
    key_systems_->distinctive_identifier = EmeFeatureSupport::ALWAYS_ENABLED;
    web_frame_delegate_->is_cross_origin_ = true;
...
TEST_F(KeySystemConfigSelectorTest, PersistentState_RequiredBlockedByContentSettings)
{
    key_systems_->persistent_state = EmeFeatureSupport::REQUESTABLE;
...
TEST_F(KeySystemConfigSelectorTest, SessionTypes_AllSupported)
{
    ...
    EXPECT_EQ(WebEncryptedMediaSessionType::kTemporary, config_.session_types[0]);
    EXPECT_EQ(WebEncryptedMediaSessionType::kPersistentLicense, config_.session_types[1]);
}
...
TEST_F(KeySystemConfigSelectorTest, SessionTypes_PermissionCanBeRequired)
{
    ...
    key_systems_->distinctive_identifier = EmeFeatureSupport::REQUESTABLE;
    key_systems_->persistent_state = EmeFeatureSupport::REQUESTABLE;
    key_systems_->persistent_license = EmeConfig { .identifier = EmeConfigRuleState::kRequired, .persistence = EmeConfigRuleState::kRequired };
...
TEST_F(KeySystemConfigSelectorTest, VideoCapabilities_UnsupportedByAesDecryptorCodec_ClearKey)
{
    key_system_ = media::kClearKeyKeySystem;
...
TEST_F(KeySystemConfigSelectorTest, VideoCapabilities_UnsupportedByAesDecryptorCodec)
{
    std::vector<WebMediaKeySystemMediaCapability> video_capabilities(1);
    video_capabilities[0].content_type = "a";
    video_capabilities[0].mime_type = kSupportedVideoContainer;
...
TEST_F(KeySystemConfigSelectorTest, VideoCapabilities_Robustness_PermissionCanBeRequired)
{
    media_permission_->is_granted = true;
    key_systems_->distinctive_identifier = EmeFeatureSupport::REQUESTABLE;
...
TEST_F(KeySystemConfigSelectorTest, VideoCapabilities_Robustness_PermissionCanBeRecommended)
{
    media_permission_->is_granted = false;
    key_systems_->distinctive_identifier = EmeFeatureSupport::REQUESTABLE;
...
TEST_F(KeySystemConfigSelectorTest, VideoCapabilities_Robustness_PermissionCanBeRecommendedAndGranted)
{
    media_permission_->is_granted = true;
    key_systems_->distinctive_identifier = EmeFeatureSupport::REQUESTABLE;
...
TEST_F(KeySystemConfigSelectorTest, VideoCapabilities_Robustness_NoPermissionRecommendedCrossOrigin)
{
    key_systems_->distinctive_identifier = EmeFeatureSupport::REQUESTABLE;
    web_frame_delegate_->is_cross_origin_ = true;
...
TEST_F(KeySystemConfigSelectorTest, VideoCapabilities_EncryptionScheme_Supported)
{
    std::vector<WebMediaKeySystemMediaCapability> video_capabilities(1);
    video_capabilities[0].content_type = "a";
    video_capabilities[0].mime_type = kSupportedVideoContainer;
    ...
    SelectConfigReturnsConfig();
    ASSERT_EQ(1u, config_.video_capabilities.size());
    EXPECT_EQ(kSupportedEncryptionScheme, config_.video_capabilities[0].encryption_scheme);
}

TEST_F(KeySystemConfigSelectorTest, VideoCapabilities_EncryptionScheme_DisallowHwSecureCodec)
{
    std::vector<WebMediaKeySystemMediaCapability> video_capabilities(1);
    video_capabilities[0].content_type = "a";
    video_capabilities[0].mime_type = kSupportedVideoContainer;
    video_capabilities[0].codecs = kSupportedVideoCodec;
    video_capabilities[0].encryption_scheme = kDisallowHwSecureCodecEncryptionScheme;
    ...
    SelectConfigReturnsConfig();
    ASSERT_EQ(1u, config_.video_capabilities.size());
    EXPECT_EQ(kDisallowHwSecureCodecEncryptionScheme, config_.video_capabilities[0].encryption_scheme);
}
...
TEST_F(KeySystemConfigSelectorTest, HwSecureCodec_IncompatibleCodecAndRobustness)
{
    std::vector<WebMediaKeySystemMediaCapability> video_capabilities(1);
    video_capabilities[0].content_type = "a";
    video_capabilities[0].mime_type = kSupportedVideoContainer;
...
TEST_F(KeySystemConfigSelectorTest, HwSecureCodec_CompatibleCodecs)
{
    ...
    video_capabilities[0].content_type = "a";
    video_capabilities[0].mime_type = kSupportedVideoContainer;
    video_capabilities[0].codecs = MakeCodecs(kRequireHwSecureCodec, kSupportedVideoCodec);
...
TEST_F(KeySystemConfigSelectorTest, HwSecureCodec_IncompatibleCodecs)
{
    ...
    video_capabilities[0].content_type = "a";
    video_capabilities[0].mime_type = kSupportedVideoContainer;
    video_capabilities[0].codecs = MakeCodecs(kRequireHwSecureCodec, kDisallowHwSecureCodec);
...
TEST_F(KeySystemConfigSelectorTest, HwSecureCodec_RequireAndDisallow)
{
    ...
    SelectConfigReturnsConfig();
    ASSERT_EQ(1u, config_.video_capabilities.size());
    EXPECT_EQ("require_hw_secure_codec", config_.video_capabilities[0].content_type);
    EXPECT_TRUE(cdm_config_.use_hw_secure_codecs);
}
...
TEST_F(KeySystemConfigSelectorTest, HwSecureCodec_DisallowAndRequire)
{
    ...
    SelectConfigReturnsConfig();
    ASSERT_EQ(1u, config_.video_capabilities.size());
    EXPECT_EQ("disallow_hw_secure_codec", config_.video_capabilities[0].content_type);
    EXPECT_FALSE(cdm_config_.use_hw_secure_codecs);
}
...
TEST_F(KeySystemConfigSelectorTest, HwSecureCodec_UnsupportedCapabilityNotAffectingRules)
{
    std::vector<WebMediaKeySystemMediaCapability> video_capabilities(2);
...
TEST_F(KeySystemConfigSelectorTest, HwSecureCodec_EncryptionScheme_DisallowHwSecureCodec)
{
    std::vector<WebMediaKeySystemMediaCapability> video_capabilities(1);
    video_capabilities[0].content_type = "a";
    video_capabilities[0].mime_type = kSupportedVideoContainer;
...
TEST_F(KeySystemConfigSelectorTest, IdentifierAndHwSecureCodec_IncompatibleCodecAndRobustness)
{
    media_permission_->is_granted = true;
    key_systems_->distinctive_identifier = EmeFeatureSupport::REQUESTABLE;
    ...
    video_capabilities[0].mime_type = kSupportedVideoContainer;
    video_capabilities[0].codecs = kDisallowHwSecureCodec;
    video_capabilities[0].robustness = kRequireIdentifierAndHwSecureCodecRobustness;
...
TEST_F(KeySystemConfigSelectorTest, IdentifierAndHwSecureCodec_IncompatibleCapabilities)
{
    media_permission_->is_granted = true;
    key_systems_->distinctive_identifier = EmeFeatureSupport::REQUESTABLE;
...
TEST_F(KeySystemConfigSelectorTest, IdentifierAndHwSecureCodec_UnsupportedCapabilityNotAffectingRules)
{
    media_permission_->is_granted = true;
    key_systems_->distinctive_identifier = EmeFeatureSupport::REQUESTABLE;
    ...
    video_capabilities[1].mime_type = kSupportedVideoContainer;
    video_capabilities[1].codecs = kRequireHwSecureCodec;
    video_capabilities[1].robustness = kRequireIdentifierAndHwSecureCodecRobustness;
...
TEST_F(KeySystemConfigSelectorTest, HwSecureCodecAndIdentifier_IdentifierAndHwSecureCodecsDisjoint)
{
    media_permission_->is_granted = false;
    key_systems_->distinctive_identifier = EmeFeatureSupport::NOT_SUPPORTED;
...
TEST_F(KeySystemConfigSelectorTest, IdentifierPersistenceAndHwSecureCodec_Supported)
{
    media_permission_->is_granted = true;
    key_systems_->persistent_state = EmeFeatureSupport::REQUESTABLE;
    key_systems_->distinctive_identifier = EmeFeatureSupport::REQUESTABLE;
    ...
    video_capabilities[0].mime_type = kSupportedVideoContainer;
    video_capabilities[0].codecs = kSupportedVideoCodec;
    video_capabilities[0].robustness = kRequireIdentifierPersistenceAndHwSecureCodecRobustness;
...
TEST_F(KeySystemConfigSelectorTest, IdentifierPersistenceAndHwSecureCodec_NotSupported)
{
    media_permission_->is_granted = true;
...
TEST_F(KeySystemConfigSelectorTest, AudioAndVideoCapabilities_AudioUnsupported)
{
    std::vector<WebMediaKeySystemMediaCapability> audio_capabilities(1);
    audio_capabilities[0].content_type = "a";
...
TEST_F(KeySystemConfigSelectorTest, AudioAndVideoCapabilities_VideoUnsupported)
{
    std::vector<WebMediaKeySystemMediaCapability> audio_capabilities(1);
    audio_capabilities[0].content_type = "a";
    audio_capabilities[0].mime_type = kSupportedAudioContainer;
...
TEST_F(KeySystemConfigSelectorTest, Configurations_FirstRequiresPermission_Allowed)
{
    media_permission_->is_granted = true;
    key_systems_->distinctive_identifier = EmeFeatureSupport::REQUESTABLE;
...
TEST_F(KeySystemConfigSelectorTest, Configurations_FirstRequiresPermission_Rejected)
{
    media_permission_->is_granted = false;
    key_systems_->distinctive_identifier = EmeFeatureSupport::REQUESTABLE;
...
TEST_F(KeySystemConfigSelectorTest, DisableHardwareSecureDecryptionFallbackFeature)
{
    std::vector<WebMediaKeySystemMediaCapability> video_capabilities(1);
    video_capabilities[0].content_type = "a";
    video_capabilities[0].mime_type = kSupportedVideoContainer;
    ...
    feature_list.InitAndEnableFeatureWithParameters(media::kHardwareSecureDecryptionFallback, { { "per_site", "false" } });
...
TEST_F(KeySystemConfigSelectorTest, EnableHardwareSecureDecryptionFallbackFeature)
{
    std::vector<WebMediaKeySystemMediaCapability> video_capabilities(1);
    video_capabilities[0].content_type = "a";
    video_capabilities[0].mime_type = kSupportedVideoContainer;
    ...
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeatureWithParameters(media::kHardwareSecureDecryptionFallback, { { "per_site", "true" } });
