...
public:
    ResourceMultiBuffer(UrlData* url_data_, int block_shift, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ~ResourceMultiBuffer() override;

    // MultiBuffer implementation.
    std::unique_ptr<MultiBuffer::DataProvider> CreateWriter(const BlockId& pos, bool is_client_audio_element) override;
...
public:
    ...
    UrlData(base::PassKey<UrlIndex>, const KURL& url, CorsMode cors_mode, UrlIndex* url_index, CacheMode cache_lookup_mode,
    ...
    CacheMode cache_lookup_mode() const
    {
        return cache_lookup_mode_;
    }
    ...
    void AddBytesRead(int64_t b)
    {
        bytes_read_from_cache_ += b;
    }
    int64_t BytesReadFromCache() const
    {
        return bytes_read_from_cache_;
    }

protected:
    UrlData(const KURL& url, CorsMode cors_mode, UrlIndex* url_index, CacheMode cache_lookup_mode, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    virtual ~UrlData();
...
private:
    ...
    std::vector<RedirectCB> redirect_callbacks_ ALLOW_DISCOURAGED_TYPE("TODO(crbug.com/40760651)");
...
public:
    UrlIndex(ResourceFetchContext* fetch_context, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    UrlIndex(ResourceFetchContext* fetch_context, int block_shift, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    virtual ~UrlIndex();
    ...
    scoped_refptr<UrlData> GetByUrl(const KURL& gurl, UrlData::CorsMode cors_mode, UrlData::CacheMode cache_mode);
    ...
    ResourceFetchContext* fetch_context() const
    {
        return fetch_context_;
    }
    int block_shift() const
    {
        return block_shift_;
    }
...
protected:
    ...
    virtual scoped_refptr<UrlData> NewUrlData(const KURL& url, UrlData::CorsMode cors_mode, UrlData::CacheMode cache_lookup_mode);
