...
static WebContentDecryptionModuleResult::SessionStatus ConvertStatus(SessionInitStatus status)
{
    switch (status) {
    case SessionInitStatus::UNKNOWN_STATUS:
        break;
...
NewSessionCdmResultPromise::NewSessionCdmResultPromise(const WebContentDecryptionModuleResult& result, const std::string& key_system_uma_prefix,
    const std::string& uma_name, SessionInitializedCB new_session_created_cb, const std::vector<SessionInitStatus>& expected_statuses)
...
void NewSessionCdmResultPromise::resolve(const std::string& session_id)
{
    ...
    if (!base::Contains(expected_statuses_, status)) {
        reject(Exception::INVALID_STATE_ERROR, 0, "Cannot finish session initialization");
        return;
    }

    MarkPromiseSettled();
    ReportCdmResultUMA(key_system_uma_prefix_ + uma_name_, 0, ConvertStatusToUMAResult(status));
...
void NewSessionCdmResultPromise::reject(CdmPromise::Exception exception_code, uint32_t system_code, const std::string& error_message)
{
    DVLOG(1) << __func__ << ": system_code = " << system_code << ", error_message = " << error_message;
