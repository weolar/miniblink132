...
public:
    using WindowCB = base::RepeatingCallback<void(int64_t dropped_frames, int64_t decoded_frames)>;
        ...
        , cb_(std::move(cb))
    {
        ...
        update_timer_.Start(FROM_HERE, kSegmentSize, base::BindRepeating(&SmoothnessWindowMonitor::OnTimer, base::Unretained(this)));
    }
...
public:
    SmoothnessHelperImpl(std::unique_ptr<LearningTaskController> consecutive_controller, std::unique_ptr<LearningTaskController> nnr_controller,
        const FeatureVector& features, Client* player)
        : SmoothnessHelper(features)
        , consecutive_bad_(std::move(consecutive_controller))
        , consecutive_nnr_(std::move(nnr_controller))
        , player_(player)
    {
        monitor_ = std::make_unique<SmoothnessWindowMonitor>(player_, base::BindRepeating(&SmoothnessHelperImpl::OnWindow, base::Unretained(this)));
    }
    ...
    void NotifyNNR() override
    {
        ...
        if (num_consecutive_nnrs_ > max_num_consecutive_nnrs_) {
            ...
            if (consecutive_nnr_.is_started()) {
                consecutive_nnr_.UpdateObservation(features(), TargetValue(max_num_consecutive_nnrs_));
            }
        }
    }
    ...
    void OnWindow(int64_t dropped_frames, int64_t decoded_frames)
    {
        ...
        if (pct >= kMaxDroppedFramesPerWindow) {
            ...
            if (consecutive_bad_windows_ > max_consecutive_bad_windows_) {
                ...
                consecutive_bad_.UpdateObservation(features(), TargetValue(max_consecutive_bad_windows_));
            }
        } else {
    ...
    struct Task {
        ...
        void UpdateObservation(const FeatureVector& features, TargetValue current_target)
        {
        ...
        const TargetValue& target_value() const
        {
...
// static
std::unique_ptr<SmoothnessHelper> SmoothnessHelper::Create(std::unique_ptr<LearningTaskController> bad_controller,
    std::unique_ptr<LearningTaskController> nnr_controller, const FeatureVector& features, Client* player)
{
    return std::make_unique<SmoothnessHelperImpl>(std::move(bad_controller), std::move(nnr_controller), features, player);
}
