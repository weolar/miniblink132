...
class WebAudioSourceProviderImplTest : public testing::Test, public WebAudioSourceProviderClient {
public:
    WebAudioSourceProviderImplTest()
        : params_(media::AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::Stereo(), kTestSampleRate, 64)
        , fake_callback_(0.1, kTestSampleRate)
        , mock_sink_(base::MakeRefCounted<media::MockAudioRendererSink>())
        , wasp_impl_(base::MakeRefCounted<WebAudioSourceProviderImpl>(mock_sink_, &media_log_))
    {
    }

    WebAudioSourceProviderImplTest(const WebAudioSourceProviderImplTest&) = delete;
    WebAudioSourceProviderImplTest& operator=(const WebAudioSourceProviderImplTest&) = delete;
    ...
    bool CompareBusses(const media::AudioBus* bus1, const media::AudioBus* bus2)
    {
        ...
        for (int ch = 0; ch < bus1->channels(); ++ch) {
            if (memcmp(bus1->channel(ch), bus2->channel(ch), sizeof(*bus1->channel(ch)) * bus1->frames()) != 0) {
                return false;
            }
        }
        return true;
    ...
    MOCK_METHOD3(DoCopyAudioCB, void(std::unique_ptr<media::AudioBus> bus, uint32_t frames_delayed, int sample_rate));
...
TEST_F(WebAudioSourceProviderImplTest, ProvideInput)
{
    ...
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks::Now(), {}, bus2.get());
    ...
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks::Now(), {}, bus1.get());
    fake_callback_.reset();
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks::Now(), {}, bus2.get());
    bus2->ZeroFramesPartial(bus2->frames() / 2, bus2->frames() - bus2->frames() / 2);
    fake_callback_.reset();
...
TEST_F(WebAudioSourceProviderImplTest, CopyAudioCB)
{
    testing::InSequence s;
    wasp_impl_->Initialize(params_, &fake_callback_);
    wasp_impl_->SetCopyAudioCallback(WTF::BindRepeating(&WebAudioSourceProviderImplTest::DoCopyAudioCB, base::Unretained(this)));
...
TEST_F(WebAudioSourceProviderImplTest, CopyAudioCBTainted)
{
    ...
    const auto bus1 = media::AudioBus::Create(params_);
    EXPECT_CALL(*this, DoCopyAudioCB(testing::Not(IsMuted()), 0, params_.sample_rate())).Times(1);
    Render(bus1.get());

    wasp_impl_->TaintOrigin();
    EXPECT_CALL(*this, DoCopyAudioCB(IsMuted(), 0, params_.sample_rate())).Times(1);
    Render(bus1.get());
...
TEST_F(WebAudioSourceProviderImplTest, MultipleInitializeWithSetClient)
{
    ...
    auto stream_params = media::AudioParameters(media::AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::Mono(), kTestSampleRate * 2, 64);

    EXPECT_CALL(*this, SetFormat(stream_params.channels(), stream_params.sample_rate()));
...
TEST_F(WebAudioSourceProviderImplTest, ProvideInputDifferentChannelCount)
{
    ...
    auto stereo_params = media::AudioParameters(media::AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::Stereo(), kTestSampleRate * 2, 64);

    // When Initialize() is called after setClient(), the params should propagate
    // to the client via setFormat() during the call.
    EXPECT_CALL(*this, SetFormat(stereo_params.channels(), stereo_params.sample_rate()));
    wasp_impl_->SetClient(this);
    ...
    auto mono_params = media::AudioParameters(media::AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::Mono(), kTestSampleRate * 2, 64);
...
TEST_F(WebAudioSourceProviderImplTest, SetClientCallback)
{
    wasp_impl_ = base::MakeRefCounted<WebAudioSourceProviderImpl>(
        mock_sink_, &media_log_, base::BindOnce(&WebAudioSourceProviderImplTest::OnClientSet, weak_factory_.GetWeakPtr()));
