...
TestResponseGenerator::TestResponseGenerator(const KURL& url, int64_t content_length)
    : url_(url)
...
WebURLResponse TestResponseGenerator::Generate200()
{
    ...
    response.SetHttpHeaderField(WebString::FromUTF8("Content-Length"), WebString::FromUTF8(base::NumberToString(content_length_)));
...
WebURLResponse TestResponseGenerator::Generate206(int64_t first_byte_offset, Flags flags)
{
    return GeneratePartial206(first_byte_offset, content_length_ - 1, flags);
}

WebURLResponse TestResponseGenerator::GeneratePartial206(int64_t first_byte_offset, int64_t last_byte_offset)
{
    return GeneratePartial206(first_byte_offset, last_byte_offset, kNormal);
}

WebURLResponse TestResponseGenerator::GeneratePartial206(int64_t first_byte_offset, int64_t last_byte_offset, Flags flags)
{
    ...
    if ((flags & kNoContentRange) == 0) {
        std::string content_range = base::StringPrintf("bytes %" PRId64 "-%" PRId64 "/", first_byte_offset, last_byte_offset);
        if (flags & kNoContentRangeInstanceSize)
            content_range += "*";
        else
            content_range += base::StringPrintf("%" PRId64, content_length_);
        response.SetHttpHeaderField(WebString::FromUTF8("Content-Range"), WebString::FromUTF8(content_range));
    }

    if ((flags & kNoContentLength) == 0) {
        response.SetHttpHeaderField(WebString::FromUTF8("Content-Length"), WebString::FromUTF8(base::NumberToString(range_content_length)));
...
WebURLResponse TestResponseGenerator::GenerateFileResponse(int64_t first_byte_offset)
{
    WebURLResponse response(url_);
