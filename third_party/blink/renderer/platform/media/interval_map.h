...
template <typename KeyType, typename ValueType, class Compare = std::less<KeyType>, class NumericLimits = std::numeric_limits<KeyType>>
...
public:
    ...
    IntervalMapConstIterator(const MapType* map, typename MapType::const_iterator iter)
        : map_(map)
        , iter_(iter)
    {
    }
...
template <typename KeyType, typename ValueType, class Compare = std::less<KeyType>, class NumericLimits = std::numeric_limits<KeyType>> class IntervalMap {
public:
    typedef std::map<KeyType, ValueType, Compare> MapType;
    typedef IntervalMapConstIterator<KeyType, ValueType, Compare, NumericLimits> const_iterator;
    IntervalMap()
    {
    ...
    {
        return const_iterator(&map(), map_.begin());
    }

    // Returns an end marker iterator.
    const_iterator end() const
    {
        return const_iterator(&map(), map_.end());
    }
