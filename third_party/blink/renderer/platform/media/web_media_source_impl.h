...
public:
    ...
    std::unique_ptr<WebSourceBuffer> AddSourceBuffer(const WebString& content_type, const WebString& codecs, AddStatus& out_status /* out */) override;
    std::unique_ptr<WebSourceBuffer> AddSourceBuffer(std::unique_ptr<media::AudioDecoderConfig> audio_config, AddStatus& out_status /* out */) override;
    std::unique_ptr<WebSourceBuffer> AddSourceBuffer(std::unique_ptr<media::VideoDecoderConfig> video_config, AddStatus& out_status /* out */) override;
...
private:
    raw_ptr<media::ChunkDemuxer, DanglingUntriaged> demuxer_; // Owned by WebMediaPlayerImpl.
};
