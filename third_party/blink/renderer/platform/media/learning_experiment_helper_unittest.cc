...
public:
    explicit MockLearningTaskController(const LearningTask& task)
        : task_(task)
    {
    }
    MockLearningTaskController(const MockLearningTaskController&) = delete;
    MockLearningTaskController& operator=(const MockLearningTaskController&) = delete;
    ~MockLearningTaskController() override = default;

    MOCK_METHOD4(BeginObservation,
        void(base::UnguessableToken id, const FeatureVector& features, const std::optional<TargetValue>& default_value,
            const std::optional<ukm::SourceId>& source_id));
    MOCK_METHOD2(CompleteObservation, void(base::UnguessableToken id, const ObservationCompletion& completion));
    MOCK_METHOD1(CancelObservation, void(base::UnguessableToken id));
    MOCK_METHOD2(UpdateDefaultTarget, void(base::UnguessableToken id, const std::optional<TargetValue>& default_target));
    MOCK_METHOD2(PredictDistribution, void(const FeatureVector& features, PredictionCB callback));
...
public:
    void SetUp() override
    {
        ...
        std::unique_ptr<MockLearningTaskController> controller = std::make_unique<MockLearningTaskController>(task_);
...
TEST_F(LearningExperimentHelperTest, BeginComplete)
{
    ...
    EXPECT_CALL(*controller_raw_, CompleteObservation(_, ObservationCompletion(target))).Times(1);
    ...
    EXPECT_CALL(*controller_raw_, CompleteObservation(_, ObservationCompletion(target))).Times(0);
    helper_->CompleteObservationIfNeeded(target);
}
