...
void RunSetSinkIdCallback(blink::WebSetSinkIdCompleteCallback callback, media::OutputDeviceStatus result)
{
    switch (result) {
...
void ReportMetrics(WebMediaPlayer::LoadType load_type, const KURL& url, media::MediaLog* media_log)
{
    DCHECK(media_log);
    ...
    UMA_HISTOGRAM_ENUMERATION("Media.LoadType", load_type, WebMediaPlayer::kLoadTypeMax + 1);
}

media::OutputDeviceStatusCB ConvertToOutputDeviceStatusCB(WebSetSinkIdCompleteCallback callback)
{
    return base::BindPostTaskToCurrentDefault(WTF::BindOnce(RunSetSinkIdCallback, std::move(callback)));
}
