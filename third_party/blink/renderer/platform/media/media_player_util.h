...
PLATFORM_EXPORT media::mojom::blink::MediaURLScheme GetMediaURLScheme(const KURL& url);

// Report various metrics to UMA.
PLATFORM_EXPORT void ReportMetrics(WebMediaPlayer::LoadType load_type, const KURL& url, media::MediaLog* media_log);

// Wraps a WebSetSinkIdCompleteCallback into a
// media::OutputDeviceStatusCB and binds it to the current thread
PLATFORM_EXPORT media::OutputDeviceStatusCB ConvertToOutputDeviceStatusCB(WebSetSinkIdCompleteCallback completion_callback);
