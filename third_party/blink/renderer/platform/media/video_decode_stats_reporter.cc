...
VideoDecodeStatsReporter::VideoDecodeStatsReporter(mojo::PendingRemote<media::mojom::VideoDecodeStatsRecorder> recorder_remote,
    GetPipelineStatsCB get_pipeline_stats_cb, media::VideoCodecProfile codec_profile, const gfx::Size& natural_size, std::optional<media::CdmConfig> cdm_config,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TickClock* tick_clock)
    ...
    , use_hw_secure_codecs_(cdm_config ? cdm_config->use_hw_secure_codecs : false)
    ...
    , stats_cb_timer_(tick_clock_)
{
    ...
    recorder_remote_.set_disconnect_handler(base::BindOnce(&VideoDecodeStatsReporter::OnIpcConnectionError, base::Unretained(this)));
...
void VideoDecodeStatsReporter::OnShown()
{
    ...
    if (num_stable_fps_samples_ >= kRequiredStableFpsSamples) {
        ...
        StartNewRecord(stats.video_frames_decoded, stats.video_frames_dropped, stats.video_frames_decoded_power_efficient);
    }
...
bool VideoDecodeStatsReporter::MatchesBucketedNaturalSize(const gfx::Size& natural_size) const
{
...
void VideoDecodeStatsReporter::RunStatsTimerAtInterval(base::TimeDelta interval)
{
    ...
    stats_cb_timer_.Start(FROM_HERE, interval, this, &VideoDecodeStatsReporter::UpdateStats);
}

void VideoDecodeStatsReporter::StartNewRecord(uint32_t frames_decoded_offset, uint32_t frames_dropped_offset, uint32_t frames_decoded_power_efficient_offset)
{
    DVLOG(2) << __func__ << " "
             << " profile:" << codec_profile_ << " size:" << natural_size_.ToString() << " fps:" << last_observed_fps_ << " key_system:" << key_system_
    ...
    DCHECK_GE(frames_decoded_power_efficient_offset, frames_decoded_power_efficient_offset_);
    frames_decoded_offset_ = frames_decoded_offset;
    frames_dropped_offset_ = frames_dropped_offset;
    frames_decoded_power_efficient_offset_ = frames_decoded_power_efficient_offset;

    bool use_hw_secure_codecs = use_hw_secure_codecs_;
    auto features = media::mojom::PredictionFeatures::New(codec_profile_, natural_size_, last_observed_fps_, key_system_, use_hw_secure_codecs);
...
{
    return is_playing_ && !is_backgrounded_ && !fps_stabilization_failed_ && !natural_size_.IsEmpty() && is_ipc_connected_;
}
...
bool VideoDecodeStatsReporter::UpdateDecodeProgress(const media::PipelineStatistics& stats)
{
...
bool VideoDecodeStatsReporter::UpdateFrameRateStability(const media::PipelineStatistics& stats)
{
    ...
    int frame_rate = media::GetFpsBucket(1 / stats.video_frame_duration_average.InSecondsF());

    if (frame_rate != last_observed_fps_) {
        DVLOG(2) << __func__ << " fps changed: " << last_observed_fps_ << " -> " << frame_rate;
        ...
        if (was_stable) {
            if (tick_clock_->NowTicks() - last_fps_stabilized_ticks_ < kTinyFpsWindowDuration) {
                num_consecutive_tiny_fps_windows_++;
                DVLOG(2) << __func__ << " Last FPS window was 'tiny'. num_tiny:" << num_consecutive_tiny_fps_windows_;
    ...
    if (num_stable_fps_samples_ < kRequiredStableFpsSamples) {
        DVLOG(2) << __func__ << " fps held, awaiting stable (" << num_stable_fps_samples_ << ")";
        return false;
...
void VideoDecodeStatsReporter::UpdateStats()
{
    ...
    media::PipelineStatistics stats = get_pipeline_stats_cb_.Run();
    DVLOG(2) << __func__ << " Raw stats -- dropped:" << stats.video_frames_dropped << "/" << stats.video_frames_decoded
             << " power efficient:" << stats.video_frames_decoded_power_efficient << "/" << stats.video_frames_decoded
    ...
    uint32_t frames_dropped = std::min(stats.video_frames_dropped - frames_dropped_offset_, frames_decoded);
    uint32_t frames_power_efficient = std::min(stats.video_frames_decoded_power_efficient - frames_decoded_power_efficient_offset_, frames_decoded);

    auto targets = media::mojom::PredictionTargets::New(frames_decoded, frames_dropped, frames_power_efficient);

    DVLOG(2) << __func__ << " Recording -- dropped:" << targets->frames_dropped << "/" << targets->frames_decoded
             << " power efficient:" << targets->frames_power_efficient << "/" << targets->frames_decoded;
