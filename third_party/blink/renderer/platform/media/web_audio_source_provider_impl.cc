...
class WebAudioSourceProviderImpl::TeeFilter : public AudioRendererSink::RenderCallback {
public:
    TeeFilter()
        : copy_required_(false)
    {
    }
    ...
    void Initialize(AudioRendererSink::RenderCallback* renderer, int channels, int sample_rate)
    {
        DCHECK(renderer);
        renderer_ = renderer;
        channels_ = channels;
    ...
    int Render(base::TimeDelta delay, base::TimeTicks delay_timestamp, const media::AudioGlitchInfo& glitch_info, media::AudioBus* audio_bus) override
    {
        DCHECK(initialized());
        DCHECK_EQ(audio_bus->channels(), channels_);

        const int num_rendered_frames = renderer_->Render(delay, delay_timestamp, glitch_info, audio_bus);
        ...
        if (copy_required_) {
            ...
            if (!copy_audio_bus_callback_.is_null()) {
                const int64_t frames_delayed = media::AudioTimestampHelper::TimeToFrames(delay, sample_rate_);
                std::unique_ptr<media::AudioBus> bus_copy = media::AudioBus::Create(audio_bus->channels(), audio_bus->frames());
                ...
                // needs it.
                copy_audio_bus_callback_.Run(std::move(bus_copy), static_cast<uint32_t>(frames_delayed), sample_rate_);
            }
        }
...
WebAudioSourceProviderImpl::WebAudioSourceProviderImpl(
    scoped_refptr<media::SwitchableAudioRendererSink> sink, media::MediaLog* media_log, base::OnceClosure on_set_client_callback /* = base::OnceClosure()*/)
    : sink_(std::move(sink))
...
void WebAudioSourceProviderImpl::SetClient(WebAudioSourceProviderClient* client)
{
    ...
    if (client) {
        ...
        set_format_cb_ = base::BindPostTaskToCurrentDefault(WTF::BindRepeating(&WebAudioSourceProviderImpl::OnSetFormat, weak_factory_.GetWeakPtr()));
...
void WebAudioSourceProviderImpl::ProvideInput(const WebVector<float*>& audio_data, int number_of_frames)
{
    if (!bus_wrapper_ || static_cast<size_t>(bus_wrapper_->channels()) != audio_data.size()) {
        bus_wrapper_ = media::AudioBus::CreateWrapper(static_cast<int>(audio_data.size()));
    }
    ...
    // them through here.
    const int frames = tee_filter_->Render(base::TimeDelta(), base::TimeTicks::Now(), {}, bus_wrapper_.get());
...
void WebAudioSourceProviderImpl::Initialize(const media::AudioParameters& params, RenderCallback* renderer)
{
    base::AutoLock auto_lock(sink_lock_);
...
void WebAudioSourceProviderImpl::GetOutputDeviceInfoAsync(OutputDeviceInfoCB info_cb)
{
    base::AutoLock auto_lock(sink_lock_);
    if (sink_) {
    ...
    base::BindPostTaskToCurrentDefault(WTF::BindOnce(std::move(info_cb), media::OutputDeviceInfo(media::OUTPUT_DEVICE_STATUS_OK))).Run();
}
...
void WebAudioSourceProviderImpl::SwitchOutputDevice(const std::string& device_id, media::OutputDeviceStatusCB callback)
{
    base::AutoLock auto_lock(sink_lock_);
    if (client_ || !sink_)
...
{
    return tee_filter_->Render(base::TimeDelta(), base::TimeTicks::Now(), {}, audio_bus);
}
