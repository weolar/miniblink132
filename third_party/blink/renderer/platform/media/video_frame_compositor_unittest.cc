...
public:
    ...
    int did_receive_frame_count()
    {
        return did_receive_frame_count_;
    }
...
class VideoFrameCompositorTest : public media::VideoRendererSink::RenderCallback, public testing::Test {
public:
    ...
    void SetUp() override
    {
        ...
        constexpr auto kTestTransform = media::VideoTransformation(media::VideoRotation::VIDEO_ROTATION_90, /*mirrored=*/false);
    ...
    {
        return base::BindOnce(&VideoFrameCompositorTest::OnNewFramePresented, base::Unretained(this));
    }

protected:
    // VideoRendererSink::RenderCallback implementation.
    MOCK_METHOD3(Render, scoped_refptr<media::VideoFrame>(base::TimeTicks, base::TimeTicks, RenderingMode));
    ...
    std::unique_ptr<StrictMock<MockWebVideoFrameSubmitter>> client_ = std::make_unique<StrictMock<MockWebVideoFrameSubmitter>>();
...
{
    scoped_refptr<media::VideoFrame> expected = media::VideoFrame::CreateEOSFrame();
...
TEST_F(VideoFrameCompositorTest, RenderFiresPresentationCallback)
{
    ...
    EXPECT_CALL(*this, Render(_, _, RenderingMode::kStartup)).WillRepeatedly(Return(opaque_frame));
...
TEST_F(VideoFrameCompositorTest, PresentationCallbackForcesBeginFrames)
{
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(*submitter_, SetForceBeginFrames(false)).WillOnce(RunClosure(run_loop.QuitClosure()));
    run_loop.Run();
...
TEST_F(VideoFrameCompositorTest, MultiplePresentationCallbacks)
{
    ...
    scoped_refptr<media::VideoFrame> opaque_frame_1 = CreateOpaqueFrame(kSize1, kSize1);
    scoped_refptr<media::VideoFrame> opaque_frame_2 = CreateOpaqueFrame(kSize2, kSize2);
    scoped_refptr<media::VideoFrame> opaque_frame_3 = CreateOpaqueFrame(kSize3, kSize3);
...
TEST_F(VideoFrameCompositorTest, VideoRendererSinkFrameDropped)
{
    ...
    EXPECT_TRUE(compositor()->UpdateCurrentFrame(base::TimeTicks(), base::TimeTicks()));

    // Another call should trigger a dropped frame callback.
    EXPECT_CALL(*this, OnFrameDropped());
    EXPECT_FALSE(compositor()->UpdateCurrentFrame(base::TimeTicks(), base::TimeTicks()));
...
TEST_F(VideoFrameCompositorTest, BackgroundRenderTicks)
{
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(*this, Render(_, _, RenderingMode::kStartup)).WillOnce(Return(opaque_frame));
    EXPECT_CALL(*this, Render(_, _, RenderingMode::kBackground)).WillOnce(DoAll(RunClosure(run_loop.QuitClosure()), Return(opaque_frame)));
    StartVideoRendererSink();
    run_loop.Run();

    // UpdateCurrentFrame() calls should indicate they are not synthetic.
    EXPECT_CALL(*this, Render(_, _, RenderingMode::kNormal)).WillOnce(Return(opaque_frame));
    EXPECT_FALSE(compositor()->UpdateCurrentFrame(base::TimeTicks(), base::TimeTicks()));
...
TEST_F(VideoFrameCompositorTest, UpdateCurrentFrameWorksWhenBackgroundRendered)
{
    scoped_refptr<media::VideoFrame> opaque_frame = CreateOpaqueFrame();
    compositor_->set_background_rendering_for_testing(true);
    ...
    EXPECT_CALL(*this, Render(_, _, RenderingMode::kNormal)).WillOnce(Return(scoped_refptr<media::VideoFrame>(opaque_frame)));
    EXPECT_TRUE(compositor()->UpdateCurrentFrame(base::TimeTicks(), base::TimeTicks()));
    RenderFrame();
...
TEST_F(VideoFrameCompositorTest, UpdateCurrentFrameIfStale)
{
    ...
    EXPECT_CALL(*submitter_, IsDrivingFrameUpdates).Times(AnyNumber()).WillRepeatedly(Return(true));

    // Starting the video renderer should return a single frame.
    EXPECT_CALL(*this, Render(_, _, RenderingMode::kStartup)).WillOnce(Return(opaque_frame_1));
    StartVideoRendererSink();
    ...
    EXPECT_CALL(*submitter_, IsDrivingFrameUpdates).Times(AnyNumber()).WillRepeatedly(Return(false));

    // Wait for background rendering to tick.
    base::RunLoop run_loop;
    EXPECT_CALL(*this, Render(_, _, RenderingMode::kBackground)).WillOnce(DoAll(RunClosure(run_loop.QuitClosure()), Return(opaque_frame_2)));
    run_loop.Run();
    ...
    EXPECT_CALL(*this, Render(_, _, RenderingMode::kBackground)).WillOnce(Return(opaque_frame_1));
    compositor()->UpdateCurrentFrameIfStale();
    EXPECT_EQ(opaque_frame_1, compositor()->GetCurrentFrame());
    ...
    // Advancing the tick clock should allow a new frame to be requested.
    tick_clock_.Advance(base::Milliseconds(10));
    EXPECT_CALL(*this, Render(_, _, RenderingMode::kBackground)).WillOnce(Return(opaque_frame_2));
    compositor()->UpdateCurrentFrameIfStale();
    EXPECT_EQ(opaque_frame_2, compositor()->GetCurrentFrame());
...
TEST_F(VideoFrameCompositorTest, UpdateCurrentFrameIfStale_ClientBypass)
{
    ...
    compositor()->UpdateCurrentFrameIfStale(VideoFrameCompositor::UpdateType::kBypassClient);
    EXPECT_EQ(opaque_frame_2, compositor()->GetCurrentFrame());
...
TEST_F(VideoFrameCompositorTest, PreferredRenderInterval)
{
    ...
    EXPECT_EQ(compositor_->GetPreferredRenderInterval(), preferred_render_interval_);
    compositor_->Stop();
    EXPECT_EQ(compositor_->GetPreferredRenderInterval(), viz::BeginFrameArgs::MinInterval());
}
...
TEST_F(VideoFrameCompositorTest, OnContextLost)
{
    ...
    std::unique_ptr<gfx::GpuMemoryBuffer> gmb = std::make_unique<media::FakeGpuMemoryBuffer>(encode_size, gfx::BufferFormat::YUV_420_BIPLANAR);
    scoped_refptr<media::VideoFrame> gpu_frame
        = media::VideoFrame::WrapExternalGpuMemoryBuffer(gfx::Rect(encode_size), encode_size, std::move(gmb), base::TimeDelta());
    ...
    EXPECT_CALL(*this, Render(_, _, RenderingMode::kStartup)).WillOnce(Return(non_gpu_frame));
    StartVideoRendererSink();
