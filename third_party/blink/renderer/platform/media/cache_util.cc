...
uint32_t GetReasonsForUncacheability(const WebURLResponse& response)
{
    ...
    const HttpVersion http_version = version == WebURLResponse::kHTTPVersion_2_0 ? HttpVersion(2, 0)
    ...
    if (code == kHttpPartialContent
        && !net::HttpUtil::HasStrongValidators(
            http_version, response.HttpHeaderField("etag").Utf8(), response.HttpHeaderField("Last-Modified").Utf8(), response.HttpHeaderField("Date").Utf8())) {
    ...
    const base::TimeDelta kMinimumAgeForUsefulness = base::Seconds(3600); // Arbitrary value.

    const char kMaxAgePrefix[] = "max-age=";
    const size_t kMaxAgePrefixLen = std::size(kMaxAgePrefix) - 1;
    if (cache_control_header.starts_with(kMaxAgePrefix)) {
        int64_t max_age_seconds;
        base::StringToInt64(base::MakeStringPiece(cache_control_header.begin() + kMaxAgePrefixLen, cache_control_header.end()), &max_age_seconds);
    ...
    Time date;
    Time expires;
    if (Time::FromString(response.HttpHeaderField("Date").Utf8().data(), &date) && Time::FromString(response.HttpHeaderField("Expires").Utf8().data(), &expires)
...
base::TimeDelta GetCacheValidUntil(const WebURLResponse& response)
{
    ...
    if (base::Contains(cache_control_header, "no-cache") || base::Contains(cache_control_header, "must-revalidate")) {
    ...
    if (cache_control_header.starts_with(kMaxAgePrefix)) {
        ...
        if (Time::FromString(response.HttpHeaderField("Date").Utf8().data(), &date)
            && Time::FromString(response.HttpHeaderField("Expires").Utf8().data(), &expires) && date > Time() && expires > Time()) {
