...
void TrustedTypesCheckForHTMLThrows(const String& string)
{
    ...
    window->GetContentSecurityPolicy()->AddPolicies(ParseContentSecurityPolicies("require-trusted-types-for 'script'",
        network::mojom::ContentSecurityPolicyType::kEnforce, network::mojom::ContentSecurityPolicySource::kMeta, *(window->GetSecurityOrigin())));
    ASSERT_FALSE(exception_state.HadException());
...
void TrustedTypesCheckForScriptWorks(const V8UnionStringOrTrustedScript* string_or_trusted_script, String expected)
{
    auto dummy_page_holder = std::make_unique<DummyPageHolder>(gfx::Size(800, 600));
    LocalDOMWindow* window = dummy_page_holder->GetFrame().DomWindow();
    V8TestingScope scope;
    DummyExceptionStateForTesting exception_state;
    String s = TrustedTypesCheckForScript(string_or_trusted_script, window, "", "", exception_state);
...
TEST(TrustedTypesUtilTest, TrustedTypesCheckForScript_TrustedScript)
{
    ...
    auto* trusted_value = MakeGarbageCollected<V8UnionStringOrTrustedScript>(script);
