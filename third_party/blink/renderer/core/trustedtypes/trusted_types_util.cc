...
const char kFunctionConstructorFailureConsoleMessage[] = "The JavaScript Function constructor does not accept TrustedString "
...
const char kScriptExecutionTrustedTypeFailConsoleMessage[] = "This document requires 'TrustedScript' assignment. "
...
String GetSamplePrefix(const char* interface_name, const char* property_name, const String& value)
{
    ...
    if (!interface_name) {
        ...
        sample_prefix.Append(value.StartsWith(kAnonymousPrefix) ? "Function" : "eval");
    } else if ((strcmp("Worker", interface_name) == 0 || strcmp("SharedWorker", interface_name) == 0) && property_name) {
...
HeapVector<ScriptValue> GetDefaultCallbackArgs(
    v8::Isolate* isolate, const char* type, const char* interface_name, const char* property_name, const String& value = g_empty_string)
{
    HeapVector<ScriptValue> args;
    args.push_back(ScriptValue(isolate, V8String(isolate, type)));
    args.push_back(ScriptValue(isolate, V8String(isolate, GetSamplePrefix(interface_name, property_name, value))));
    return args;
}
...
bool TrustedTypeFail(TrustedTypeViolationKind kind, const ExecutionContext* execution_context, const char* interface_name, const char* property_name,
    ExceptionState& exception_state, const String& value)
{
    ...
    bool allow = execution_context->GetContentSecurityPolicy()->AllowTrustedTypeAssignmentFailure(
        GetMessage(kind), prefix == "Function" ? value.Substring(static_cast<wtf_size_t>(strlen(kAnonymousPrefix))) : value, prefix, issue_id);
    ...
    if (prefix == "Function" && !allow && !RuntimeEnabledFeatures::TrustedTypesUseCodeLikeEnabled()) {
        DCHECK(kind == kTrustedScriptAssignment || kind == kTrustedScriptAssignmentAndDefaultPolicyFailed
            || kind == kTrustedScriptAssignmentAndNoDefaultPolicyExisted);
        execution_context->GetContentSecurityPolicy()->LogToConsole(MakeGarbageCollected<ConsoleMessage>(
            mojom::blink::ConsoleMessageSource::kRecommendation, mojom::blink::ConsoleMessageLevel::kInfo, kFunctionConstructorFailureConsoleMessage));
    }
    probe::OnContentSecurityPolicyViolation(const_cast<ExecutionContext*>(execution_context), ContentSecurityPolicyViolationType::kTrustedTypesSinkViolation);

    if (!allow) {
        v8::Isolate* isolate = execution_context->GetIsolate();
        TryRethrowScope rethrow_scope(isolate, exception_state);
        auto exception = V8ThrowException::CreateTypeError(isolate, GetMessage(kind));
        MaybeAssociateExceptionMetaData(exception, "issueId", IdentifiersFactory::IdFromToken(issue_id));
...
TrustedTypePolicy* GetDefaultPolicy(const ExecutionContext* execution_context)
{
    ...
    return execution_context->GetTrustedTypes() ? execution_context->GetTrustedTypes()->defaultPolicy() : nullptr;
}
...
// for each case.
String GetStringFromScriptHelper(const String& script, ExecutionContext* context,
    // Parameters to customize error messages:
    const char* interface_name, const char* property_name, TrustedTypeViolationKind violation_kind,
    TrustedTypeViolationKind violation_kind_when_default_policy_failed)
{
    ...
    if (!default_policy) {
        if (TrustedTypeFail(violation_kind, context, interface_name, property_name, exception_state, script)) {
            return String();
        }
        return script;
    }
    ...
    TrustedScript* result = default_policy->createScript(
        context->GetIsolate(), script, GetDefaultCallbackArgs(context->GetIsolate(), "TrustedScript", interface_name, property_name, script), exception_state);
    if (!result) {
        return String();
    }

    if (result->toString().IsNull()) {
        if (TrustedTypeFail(violation_kind_when_default_policy_failed, context, interface_name, property_name, exception_state, script)) {
            return String();
        }
        return script;
    }
...
{
    return execution_context && execution_context->RequireTrustedTypes() && !ContentSecurityPolicy::ShouldBypassMainWorldDeprecated(execution_context);
}

String TrustedTypesCheckForHTML(
    const String& html, const ExecutionContext* execution_context, const char* interface_name, const char* property_name, ExceptionState& exception_state)
{
    ...
    if (!default_policy) {
        if (TrustedTypeFail(kTrustedHTMLAssignment, execution_context, interface_name, property_name, exception_state, html)) {
    ...
    if (!default_policy->HasCreateHTML()) {
        if (TrustedTypeFail(kTrustedHTMLAssignmentAndNoDefaultPolicyExisted, execution_context, interface_name, property_name, exception_state, html)) {
            return g_empty_string;
        } else {
            return html;
        }
    ...
    // Punt for now.
    TrustedHTML* result = default_policy->createHTML(execution_context->GetIsolate(), html,
        GetDefaultCallbackArgs(execution_context->GetIsolate(), "TrustedHTML", interface_name, property_name), exception_state);
    if (exception_state.HadException()) {
        return g_empty_string;
    }

    if (result->toString().IsNull()) {
        if (TrustedTypeFail(kTrustedHTMLAssignmentAndDefaultPolicyFailed, execution_context, interface_name, property_name, exception_state, html)) {
            return g_empty_string;
        } else {
            return html;
        }
...
String TrustedTypesCheckForScript(
    const String& script, const ExecutionContext* execution_context, const char* interface_name, const char* property_name, ExceptionState& exception_state)
{
    bool require_trusted_type = RequireTrustedTypesCheck(execution_context);
    if (!require_trusted_type) {
        return script;
    ...
    TrustedTypePolicy* default_policy = GetDefaultPolicy(execution_context);
    if (!default_policy) {
        if (TrustedTypeFail(kTrustedScriptAssignment, execution_context, interface_name, property_name, exception_state, script)) {
            return g_empty_string;
        }
        return script;
    }
    ...
    if (!default_policy->HasCreateScript()) {
        if (TrustedTypeFail(kTrustedScriptAssignmentAndNoDefaultPolicyExisted, execution_context, interface_name, property_name, exception_state, script)) {
            return g_empty_string;
        } else {
            return script;
        }
    ...
    // StringCache::CreateStringAndInsertIntoCache() also, but it's a hard mess.
    // Punt for now.
    TrustedScript* result = default_policy->createScript(execution_context->GetIsolate(), script,
        GetDefaultCallbackArgs(execution_context->GetIsolate(), "TrustedScript", interface_name, property_name, script), exception_state);
    ...
    if (result->toString().IsNull()) {
        if (TrustedTypeFail(kTrustedScriptAssignmentAndDefaultPolicyFailed, execution_context, interface_name, property_name, exception_state, script)) {
            return g_empty_string;
        } else {
            return script;
        }
...
String TrustedTypesCheckForScriptURL(
    const String& script_url, const ExecutionContext* execution_context, const char* interface_name, const char* property_name, ExceptionState& exception_state)
{
    bool require_trusted_type = RequireTrustedTypesCheck(execution_context);
    if (!require_trusted_type) {
        return script_url;
    ...
    TrustedTypePolicy* default_policy = GetDefaultPolicy(execution_context);
    if (!default_policy) {
        if (TrustedTypeFail(kTrustedScriptURLAssignment, execution_context, interface_name, property_name, exception_state, script_url)) {
            return g_empty_string;
        }
        return script_url;
    }
    ...
    if (!default_policy->HasCreateScriptURL()) {
        if (TrustedTypeFail(
                kTrustedScriptURLAssignmentAndNoDefaultPolicyExisted, execution_context, interface_name, property_name, exception_state, script_url)) {
            return g_empty_string;
        } else {
            return script_url;
        }
    ...
    // StringCache::CreateStringAndInsertIntoCache() also, but it's a hard mess.
    // Punt for now.
    TrustedScriptURL* result = default_policy->createScriptURL(execution_context->GetIsolate(), script_url,
        GetDefaultCallbackArgs(execution_context->GetIsolate(), "TrustedScriptURL", interface_name, property_name), exception_state);
    ...
    if (result->toString().IsNull()) {
        if (TrustedTypeFail(kTrustedScriptURLAssignmentAndDefaultPolicyFailed, execution_context, interface_name, property_name, exception_state, script_url)) {
            return g_empty_string;
        } else {
            return script_url;
        }
...
String TrustedTypesCheckFor(SpecificTrustedType type, const V8TrustedType* trusted, const ExecutionContext* execution_context, const char* interface_name,
    const char* property_name, ExceptionState& exception_state)
{
    ...
    return TrustedTypesCheckFor(type, std::move(value), execution_context, interface_name, property_name, exception_state);
}

String TrustedTypesCheckForScript(const V8UnionStringOrTrustedScript* value, const ExecutionContext* execution_context, const char* interface_name,
    const char* property_name, ExceptionState& exception_state)
{
    ...
    if (!value) {
        return TrustedTypesCheckForScript(g_empty_string, execution_context, interface_name, property_name, exception_state);
    }

    switch (value->GetContentType()) {
    case V8UnionStringOrTrustedScript::ContentType::kString:
        return TrustedTypesCheckForScript(value->GetAsString(), execution_context, interface_name, property_name, exception_state);
...
String TrustedTypesCheckForScript(const V8UnionStringLegacyNullToEmptyStringOrTrustedScript* value, const ExecutionContext* execution_context,
    const char* interface_name, const char* property_name, ExceptionState& exception_state)
{
    // To remain compatible with legacy behaviour, HTMLElement uses extended IDL
    // attributes to allow for nullable union of (DOMString or TrustedScript).
    // Thus, this method is required to handle the case where |!value|, unlike
    ...
    case V8UnionStringLegacyNullToEmptyStringOrTrustedScript::ContentType::kStringLegacyNullToEmptyString:
        return TrustedTypesCheckForScript(value->GetAsStringLegacyNullToEmptyString(), execution_context, interface_name, property_name, exception_state);
...
String TrustedTypesCheckFor(SpecificTrustedType type, String trusted, const ExecutionContext* execution_context, const char* interface_name,
    const char* property_name, ExceptionState& exception_state)
{
    switch (type) {
    case SpecificTrustedType::kHTML:
        return TrustedTypesCheckForHTML(std::move(trusted), execution_context, interface_name, property_name, exception_state);
    case SpecificTrustedType::kScript:
        return TrustedTypesCheckForScript(std::move(trusted), execution_context, interface_name, property_name, exception_state);
    case SpecificTrustedType::kScriptURL:
        return TrustedTypesCheckForScriptURL(std::move(trusted), execution_context, interface_name, property_name, exception_state);
...
String CORE_EXPORT GetStringForScriptExecution(const String& script, const ScriptElementBase::Type type, ExecutionContext* context)
{
    String value = GetStringFromScriptHelper(script, context, GetElementName(type), "text", kScriptExecution, kScriptExecutionAndDefaultPolicyFailed);
    if (!script.IsNull() && value.IsNull()) {
        context->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::blink::ConsoleMessageSource::kSecurity, mojom::blink::ConsoleMessageLevel::kError, kScriptExecutionTrustedTypeFailConsoleMessage));
    }
    return value;
}

String TrustedTypesCheckForJavascriptURLinNavigation(const String& javascript_url, ExecutionContext* context)
{
    return GetStringFromScriptHelper(
        std::move(javascript_url), context, "Location", "href", kNavigateToJavascriptURL, kNavigateToJavascriptURLAndDefaultPolicyFailed);
}

String TrustedTypesCheckForExecCommand(const String& html, const ExecutionContext* execution_context, ExceptionState& exception_state)
{
    return TrustedTypesCheckForHTML(html, execution_context, "Document", "execCommand", exception_state);
}

bool IsTrustedTypesEventHandlerAttribute(const QualifiedName& q_name)
{
    return q_name.NamespaceURI().IsNull() && TrustedTypePolicyFactory::IsEventHandlerAttributeName(q_name.LocalName());
}

String GetTrustedTypesLiteral(const ScriptValue& script_value, ScriptState* script_state)
{
    DCHECK(script_state);
    ...
    if (!context.IsEmpty() && !value.IsEmpty() && context->HasTemplateLiteralObject(value) && value->IsObject()) {
        v8::Local<v8::Object> value_as_object = v8::Local<v8::Object>::Cast(value);
        v8::Local<v8::Value> first_value;
        if (value_as_object->HasRealIndexedProperty(context, 0).FromMaybe(false) && !value_as_object->HasRealIndexedProperty(context, 1).FromMaybe(false)
            && value_as_object->Get(context, 0).ToLocal(&first_value) && first_value->IsString()) {
            v8::Local<v8::String> first_value_as_string = v8::Local<v8::String>::Cast(first_value);
