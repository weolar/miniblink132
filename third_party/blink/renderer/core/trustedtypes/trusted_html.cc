...
TrustedHTML* TrustedHTML::fromLiteral(ScriptState* script_state, const ScriptValue& templateLiteral, ExceptionState& exception_state)
{
    ...
    if (!window) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot find current DOM window.");
        return nullptr;
    }
    ...
    HTMLTemplateElement* template_element = MakeGarbageCollected<HTMLTemplateElement>(*window->document());
    DCHECK(template_element->content());
    template_element->content()->ParseHTML(literal, template_element, ParserContentPolicy::kAllowScriptingContent);
