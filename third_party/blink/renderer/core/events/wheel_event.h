...
public:
    ...
    {
        return MakeGarbageCollected<WheelEvent>();
    }

    static WheelEvent* Create(const WebMouseWheelEvent& native_event, LocalDOMWindow& window);

    static WheelEvent* Create(const WebMouseWheelEvent& native_event, const gfx::Vector2dF& delta_in_pixels, LocalDOMWindow& window);
    ...
    WheelEvent(const WebMouseWheelEvent&, const gfx::Vector2dF& delta_in_pixels, LocalDOMWindow& window);
    ...
    const WebMouseWheelEvent& NativeEvent() const
    {
...
template <> struct DowncastTraits<WheelEvent> {
    static bool AllowFrom(const Event& event)
    {
        return event.IsWheelEvent();
    }
};
