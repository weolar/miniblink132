...
PointerEvent::PointerEvent(const AtomicString& type, const PointerEventInit* initializer, base::TimeTicks platform_time_stamp,
    MouseEvent::SyntheticEventType synthetic_event_type, WebMenuSourceType menu_source_type, bool prevent_counting_as_interaction)
    : MouseEvent(type, initializer, platform_time_stamp, synthetic_event_type, menu_source_type)
    , pointer_id_(0)
    , width_(0)
    , height_(0)
    , pressure_(0)
    ...
    , prevent_counting_as_interaction_(prevent_counting_as_interaction)
{
    ...
    if ((initializer->hasTiltX() || initializer->hasTiltY()) && !initializer->hasAzimuthAngle() && !initializer->hasAltitudeAngle()) {
        azimuth_angle_
            = PointerEventUtil::AzimuthFromTilt(PointerEventUtil::TransformToTiltInValidRange(tilt_x_), PointerEventUtil::TransformToTiltInValidRange(tilt_y_));
        altitude_angle_ = PointerEventUtil::AltitudeFromTilt(
            PointerEventUtil::TransformToTiltInValidRange(tilt_x_), PointerEventUtil::TransformToTiltInValidRange(tilt_y_));
    }
    if ((initializer->hasAzimuthAngle() || initializer->hasAltitudeAngle()) && !initializer->hasTiltX() && !initializer->hasTiltY()) {
        tilt_x_ = PointerEventUtil::TiltXFromSpherical(
            PointerEventUtil::TransformToAzimuthInValidRange(azimuth_angle_), PointerEventUtil::TransformToAltitudeInValidRange(altitude_angle_));
...
{
    if (type() == event_type_names::kClick || type() == event_type_names::kAuxclick || type() == event_type_names::kContextmenu) {
        return true;
    }
...
{
    if (type() == event_type_names::kClick || type() == event_type_names::kContextmenu || type() == event_type_names::kAuxclick) {
        return true;
    }
    return false;
}
...
HeapVector<Member<PointerEvent>> PointerEvent::getCoalescedEvents()
{
    if (auto* local_dom_window = DynamicTo<LocalDOMWindow>(view())) {
        ...
        if (document && !local_dom_window->isSecureContext()) {
            UseCounter::Count(document, WebFeature::kGetCoalescedEventsInInsecureContext);
        }
    }
...
DispatchEventResult PointerEvent::DispatchEvent(EventDispatcher& dispatcher)
{
    ...
    if (isTrusted() && RuntimeEnabledFeatures::PointerEventTargetsInEventListsEnabled()) {
