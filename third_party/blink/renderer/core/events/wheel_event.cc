...
unsigned ConvertDeltaMode(const WebMouseWheelEvent& event)
{
    ...
    DCHECK(event.delta_units == ui::ScrollGranularity::kScrollByPage || event.delta_units == ui::ScrollGranularity::kScrollByPixel
        || event.delta_units == ui::ScrollGranularity::kScrollByPrecisePixel);
    return event.delta_units == ui::ScrollGranularity::kScrollByPage ? WheelEvent::kDomDeltaPage : WheelEvent::kDomDeltaPixel;
}

MouseEventInit* GetMouseEventInitForWheel(const WebMouseWheelEvent& event, LocalDOMWindow& window)
{
    MouseEventInit* initializer = MouseEventInit::Create();
    initializer->setBubbles(true);
    initializer->setCancelable(event.IsCancelable());
    MouseEvent::SetCoordinatesFromWebPointerProperties(event.FlattenTransform(), &window, initializer);
...
WheelEvent* WheelEvent::Create(const WebMouseWheelEvent& event, LocalDOMWindow& window)
{
    return MakeGarbageCollected<WheelEvent>(event, window);
}

WheelEvent* WheelEvent::Create(const WebMouseWheelEvent& event, const gfx::Vector2dF& delta_in_pixels, LocalDOMWindow& window)
{
...
WheelEvent::WheelEvent(const AtomicString& type, const WheelEventInit* initializer)
    ...
    , wheel_delta_(initializer->wheelDeltaX() ? initializer->wheelDeltaX() : ClampTo<int32_t>(initializer->deltaX()),
          initializer->wheelDeltaY() ? initializer->wheelDeltaY() : ClampTo<int32_t>(initializer->deltaY()))
    , delta_x_(initializer->deltaX() ? initializer->deltaX() : ClampTo<int32_t>(-static_cast<double>(initializer->wheelDeltaX())))
    , delta_y_(initializer->deltaY() ? initializer->deltaY() : ClampTo<int32_t>(-static_cast<double>(initializer->wheelDeltaY())))
...
WheelEvent::WheelEvent(const WebMouseWheelEvent& event, LocalDOMWindow& window)
    : MouseEvent(event_type_names::kWheel, GetMouseEventInitForWheel(event, window), event.TimeStamp())
    , wheel_delta_((event.wheel_ticks_x * kTickMultiplier) / window.devicePixelRatio(), (event.wheel_ticks_y * kTickMultiplier) / window.devicePixelRatio())
...
WheelEvent::WheelEvent(const WebMouseWheelEvent& event, const gfx::Vector2dF& delta_in_pixels, LocalDOMWindow& window)
    : MouseEvent(event_type_names::kWheel, GetMouseEventInitForWheel(event, window), event.TimeStamp())
    , wheel_delta_(event.wheel_ticks_x * kTickMultiplier, event.wheel_ticks_y * kTickMultiplier)
...
void WheelEvent::preventDefault()
{
    ...
    if (passive_mode == PassiveMode::kPassiveForcedDocumentLevel) {
        ...
        String message = "Unable to preventDefault inside passive event listener due to "
    ...
    if (passive_mode == PassiveMode::kPassiveForcedDocumentLevel || passive_mode == PassiveMode::kNotPassiveDefault) {
        if (ExecutionContext* context = currentTarget()->GetExecutionContext()) {
            UseCounter::Count(context, WebFeature::kDocumentLevelPassiveDefaultEventListenerPreventedWheel);
        }
    }
}
