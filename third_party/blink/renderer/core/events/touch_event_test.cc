...
public:
    ConsoleCapturingChromeClient()
        : EmptyChromeClient()
    {
    }

    // ChromeClient methods:
    void AddMessageToConsole(LocalFrame*, mojom::ConsoleMessageSource message_source, mojom::ConsoleMessageLevel, const String& message, unsigned line_number,
...
public:
    ...
    const Vector<String>& Messages()
    {
        return chrome_client_->Messages();
    }
    const Vector<mojom::ConsoleMessageSource>& MessageSources()
    {
    ...
    {
        WebTouchEvent web_touch_event(WebInputEvent::Type::kTouchStart, 0, base::TimeTicks());
        web_touch_event.dispatch_type = dispatch_type;
        return TouchEvent::Create(WebCoalescedInputEvent(web_touch_event, ui::LatencyInfo()), nullptr, nullptr, nullptr, event_type_names::kTouchstart,
            &Window(), TouchAction::kAuto);
    }
...
TEST_F(TouchEventTest, PreventDefaultPassiveDueToDocumentLevelScrollerIntervention)
{
    TouchEvent* event = EventWithDispatchType(WebInputEvent::DispatchType::kListenersNonBlockingPassive);
    ...
    EXPECT_THAT(MessageSources(), ElementsAre(mojom::ConsoleMessageSource::kIntervention));
}

TEST_F(TouchEventTest, DispatchWithEmptyDocTargetDoesntCrash)
{
    String script = "var empty_document = new Document();"
    ...
    ClassicScript::CreateUnspecifiedScript(script)->RunScript(GetDocument().domWindow());
}
