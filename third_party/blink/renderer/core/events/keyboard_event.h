...
public:
    enum KeyLocationCode { kDomKeyLocationStandard = 0x00, kDomKeyLocationLeft = 0x01, kDomKeyLocationRight = 0x02, kDomKeyLocationNumpad = 0x03 };
    ...
    static KeyboardEvent* Create(const WebKeyboardEvent& web_event, LocalDOMWindow* dom_window, bool cancellable = true)
    {
        return MakeGarbageCollected<KeyboardEvent>(web_event, dom_window, cancellable);
    }

    static KeyboardEvent* Create(ScriptState*, const AtomicString& type, const KeyboardEventInit*);

    KeyboardEvent();
    KeyboardEvent(const WebKeyboardEvent&, LocalDOMWindow*, bool cancellable = true);
    KeyboardEvent(const AtomicString&, const KeyboardEventInit*);
    ~KeyboardEvent() override;

    void initKeyboardEvent(ScriptState*, const AtomicString& type, bool bubbles, bool cancelable, AbstractView*, const String& key_identifier,
        unsigned location, bool ctrl_key, bool alt_key, bool shift_key, bool meta_key);
    ...
    const WebKeyboardEvent* KeyEvent() const
    {
        return key_event_.get();
    }
    ...
    bool repeat() const
    {
        return GetModifiers() & WebInputEvent::kIsAutoRepeat;
    }
...
template <> struct DowncastTraits<KeyboardEvent> {
