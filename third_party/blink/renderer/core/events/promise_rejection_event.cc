...
PromiseRejectionEvent::PromiseRejectionEvent(ScriptState* script_state, const AtomicString& type, const PromiseRejectionEventInit* initializer)
    : Event(type, initializer)
    , world_(&script_state->World())
{
    DCHECK(initializer->hasPromise());
    promise_.Reset(script_state->GetIsolate(), initializer->promise().V8Promise());
...
ScriptPromise<IDLAny> PromiseRejectionEvent::promise(ScriptState* script_state) const
{
    ...
    return ScriptPromise<IDLAny>::FromV8Promise(script_state->GetIsolate(), promise_.Get(script_state->GetIsolate()));
}
...
ScriptValue PromiseRejectionEvent::reason(ScriptState* script_state) const
{
    ...
    return ScriptValue(script_state->GetIsolate(), reason_.Get(script_state->GetIsolate()));
}
