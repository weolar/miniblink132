...
TEST_F(MessageEventTest, AccountForStringMemory)
{
    ...
    // irrelevant.
    base::span<UChar> tmp;
    String data = String::CreateUninitialized(static_cast<unsigned>(string_size), tmp);
    ...
    int64_t initial = scope.GetIsolate()->AdjustAmountOfExternalAllocatedMemory(0);
    MessageEvent::Create(data);

    int64_t size_with_event = scope.GetIsolate()->AdjustAmountOfExternalAllocatedMemory(0);
    ...
    int64_t size_after_gc = scope.GetIsolate()->AdjustAmountOfExternalAllocatedMemory(0);
...
TEST_F(MessageEventTest, AccountForArrayBufferMemory)
{
    ...
    {
        DOMArrayBuffer* array_buffer = DOMArrayBuffer::Create(static_cast<size_t>(buffer_size), 1);
