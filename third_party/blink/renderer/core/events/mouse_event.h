...
public:
    ...
    static MouseEvent* Create(
        const AtomicString& event_type, const MouseEventInit*, base::TimeTicks platform_time_stamp, SyntheticEventType, WebMenuSourceType);

    static MouseEvent* Create(ScriptState*, const AtomicString& event_type, const MouseEventInit*);

    MouseEvent(const AtomicString& type, const MouseEventInit*, base::TimeTicks platform_time_stamp = base::TimeTicks::Now(),
        SyntheticEventType = kRealOrIndistinguishable, WebMenuSourceType = kMenuSourceNone, LocalDOMWindow* fallback_dom_window = nullptr);
    MouseEvent();

    static uint16_t WebInputEventModifiersToButtons(unsigned modifiers);
    static void SetCoordinatesFromWebPointerProperties(const WebPointerProperties&, const LocalDOMWindow*, MouseEventInit*);

    void initMouseEvent(ScriptState*, const AtomicString& type, bool bubbles, bool cancelable, AbstractView*, int detail, int screen_x, int screen_y,
        int client_x, int client_y, bool ctrl_key, bool alt_key, bool shift_key, bool meta_key, int16_t button, EventTarget* related_target,
        uint16_t buttons = 0);

    void InitCoordinatesForTesting(double screen_x, double screen_y, double client_x, double client_y);
    ...
    virtual DataTransfer* getDataTransfer() const
    {
        return nullptr;
    }
    ...
    {
        return synthetic_event_type_ == kFromTouch;
    }
    ...
    int movementX() const
    {
        return movement_delta_.x();
    }
    int movementY() const
    {
        return movement_delta_.y();
    }
    ...
    {
        return position_type_ == PositionType::kPosition;
    }

    WebMenuSourceType GetMenuSourceType() const
    {
    ...
    // i.e. the local frame.
    const gfx::PointF& AbsoluteLocation() const
    {
        return absolute_location_;
    }

    DispatchEventResult DispatchEvent(EventDispatcher&) override;

    void InitCoordinates(const double client_x, const double client_y, const LocalDOMWindow* fallback_dom_window = nullptr);

    void Trace(Visitor*) const override;

protected:
    int16_t RawButton() const
    {
        return button_;
    }
...
private:
    void InitMouseEventInternal(const AtomicString& type, bool bubbles, bool cancelable, AbstractView*, int detail, double screen_x, double screen_y,
        double client_x, double client_y, WebInputEvent::Modifiers, int16_t button, EventTarget* related_target, InputDeviceCapabilities* source_capabilities,
        uint16_t buttons = 0);
...
template <> struct DowncastTraits<MouseEvent> {
