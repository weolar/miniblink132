...
const std::array<InputTypeStringNameMapEntry, static_cast<size_t>(InputEvent::InputType::kNumberOfInputTypes)> kInputTypeStringNameMap { {
    ...
    { InputEvent::InputType::kFormatSetBlockTextDirection, "formatSetBlockTextDirection" },
} };

static_assert(std::size(kInputTypeStringNameMap) == static_cast<size_t>(InputEvent::InputType::kNumberOfInputTypes), "must handle all InputEvent::InputType");
...
String ConvertInputTypeToString(InputEvent::InputType input_type)
{
    ...
    if (numeric_input_type >= 0 && numeric_input_type < static_cast<IntegerInputType>(kInputTypeStringNameMap.size())) {
        return AtomicString(kInputTypeStringNameMap[numeric_input_type].string_name);
    }
...
InputEvent::InputEvent(const AtomicString& type, const UIEventInit& init, InputType input_type, const String& data, DataTransfer* data_transfer,
    EventIsComposing is_composing, const StaticRangeVector* ranges)
    : UIEvent(type, &init)
...
/* static */
InputEvent* InputEvent::CreateBeforeInput(InputType input_type, const String& data, EventIsComposing is_composing, const StaticRangeVector* ranges)
{
    ...
    return MakeGarbageCollected<InputEvent>(event_type_names::kBeforeinput, *event_init, input_type, data, nullptr, is_composing, ranges);
}

/* static */
InputEvent* InputEvent::CreateBeforeInput(InputType input_type, DataTransfer* data_transfer, EventIsComposing is_composing, const StaticRangeVector* ranges)
{
    auto* event_init = UIEventInit::Create();
    event_init->setBubbles(true);
    event_init->setCancelable(InputTypeIsCancelable(input_type));
    event_init->setComposed(true);
    return MakeGarbageCollected<InputEvent>(event_type_names::kBeforeinput, *event_init, input_type, String(), data_transfer, is_composing, ranges);
}

/* static */
InputEvent* InputEvent::CreateInput(InputType input_type, const String& data, EventIsComposing is_composing, const StaticRangeVector* ranges)
{
    auto* event_init = UIEventInit::Create();
    event_init->setBubbles(true);
    event_init->setCancelable(false);
    event_init->setComposed(true);
    return MakeGarbageCollected<InputEvent>(event_type_names::kInput, *event_init, input_type, data, nullptr, is_composing, ranges);
}
