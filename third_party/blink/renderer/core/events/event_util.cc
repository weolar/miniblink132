...
{
    DEFINE_STATIC_LOCAL(const Vector<AtomicString>, mouse_button_event_types,
        ({ event_type_names::kClick, event_type_names::kMousedown, event_type_names::kMouseup, event_type_names::kDOMActivate }));
...
{
    return event_type == event_type_names::kGotpointercapture || event_type == event_type_names::kLostpointercapture
        || event_type == event_type_names::kPointercancel || event_type == event_type_names::kPointerdown || event_type == event_type_names::kPointerenter
        || event_type == event_type_names::kPointerleave || event_type == event_type_names::kPointermove || event_type == event_type_names::kPointerout
        || event_type == event_type_names::kPointerover || event_type == event_type_names::kPointerup;
}
...
MutationEventInfo IsDOMMutationEventType(const AtomicString& event_type)
{
    if (event_type == event_type_names::kDOMSubtreeModified) {
        ...
        return { .is_mutation_event = true,
            ...
            .event_fired_feature = WebFeature::kDOMNodeRemovedFromDocumentEventFired,
        ...
        return { .is_mutation_event = true,
            ...
            .event_fired_feature = WebFeature::kDOMNodeInsertedIntoDocumentEventFired,
...
{
    return event_type == event_type_names::kScrollsnapchanging || event_type == event_type_names::kScrollsnapchange;
}
