...
public:
    static CommandEvent* Create(const AtomicString& type, const CommandEventInit* initializer)
    {
        return MakeGarbageCollected<CommandEvent>(type, initializer);
    }

    static CommandEvent* Create(const AtomicString& type, const AtomicString& command, Element* source)
    {
    ...
    CommandEvent(const AtomicString& type, const String& command, Element* source);
