...
public:
    ...
    static DragEvent* Create(
        const AtomicString& type, const DragEventInit* initializer, base::TimeTicks platform_time_stamp, SyntheticEventType synthetic_event_type)
    {
        return MakeGarbageCollected<DragEvent>(type, initializer, platform_time_stamp, synthetic_event_type);
    }

    static DragEvent* Create(const AtomicString& type, const DragEventInit* initializer)
    {
        return MakeGarbageCollected<DragEvent>(type, initializer, base::TimeTicks::Now(), kRealOrIndistinguishable);
    }

    DragEvent();
    DragEvent(const AtomicString& type, const DragEventInit*, base::TimeTicks platform_time_stamp, SyntheticEventType);
...
template <> struct DowncastTraits<DragEvent> {
    static bool AllowFrom(const Event& event)
    {
        return event.IsDragEvent();
    }
};
