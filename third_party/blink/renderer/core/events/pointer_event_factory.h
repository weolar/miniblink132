...
public:
    ...
    static const AtomicString& PointerTypeNameForWebPointPointerType(WebPointerProperties::PointerType type);
    ...
    PointerEvent* Create(const WebPointerEvent& web_pointer_event, const Vector<WebPointerEvent>& coalesced_events,
        const Vector<WebPointerEvent>& predicted_events, LocalDOMWindow* view);

    PointerEvent* CreatePointerCancelEvent(const PointerId pointer_id, base::TimeTicks platfrom_time_stamp, const int32_t device_id);
    ...
    PointerEvent* CreatePointerBoundaryEvent(PointerEvent*, const AtomicString&, EventTarget*);
    ...
    gfx::PointF GetLastPointerPosition(PointerId pointer_id, const WebPointerProperties& event, WebInputEvent::Type event_type) const;

    void SetLastPosition(PointerId pointer_id, const gfx::PointF& position_in_screen, WebInputEvent::Type event_type);
...
private:
    ...
    PointerId AddOrUpdateIdAndActiveButtons(
        const IncomingId, bool is_active_buttons, bool hovering, WebInputEvent::Type event_type, uint32_t unique_touch_event_id);
    ...
    PointerEvent* CreatePointerEventFrom(PointerEvent*, const AtomicString&, EventTarget*);
    ...
    using IncomingIdToPointerIdMap = HashMap<IncomingId, PointerId, PairHashTraits<IntWithZeroKeyHashTraits<int>, IntWithZeroKeyHashTraits<int>>>;
    ...
    struct PointerAttributes {
        ...
        PointerAttributes(IncomingId incoming_id, bool is_active_buttons, bool hovering, uint32_t unique_touch_event_id,
            std::optional<gfx::PointF> last_position, std::optional<gfx::PointF> last_rawupdate_position)
    ...
    void SaveRecentlyRemovedPointer(PointerId pointer_id, PointerAttributes pointer_attributes);

    // We use int64_t to cover the whole range for PointerId with no
    // deleted hash value.
    HashMap<int64_t, PointerAttributes, IntWithZeroKeyHashTraits<int64_t>> pointer_id_to_attributes_;
    ...
    Deque<std::pair<PointerId, PointerAttributes>, kRemovedPointersCapacity> recently_removed_pointers_;
    ...
    static constexpr size_t kIdArraySize = static_cast<int>(WebPointerProperties::PointerType::kMaxValue) + 1;
    ...
    // current document.
    using BrowserDeviceIdToBlinkDeviceIdMap = HashMap<int32_t, int32_t, IntWithZeroKeyHashTraits<int64_t>>;
