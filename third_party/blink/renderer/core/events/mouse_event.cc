...
const LayoutObject* FindTargetLayoutObject(Node*& target_node)
{
    ...
    DCHECK(!target_node || (svg_element && svg_element->IsOutermostSVGSVGElement()));
    return layout_object;
}
...
MouseEvent* MouseEvent::Create(ScriptState* script_state, const AtomicString& type, const MouseEventInit* initializer)
{
    ...
    if (script_state) {
        ...
        if (!initializer || !initializer->view()) {
            if (auto* execution_context = ExecutionContext::From(script_state); execution_context && execution_context->IsWindow()) {
                fallback_dom_window = static_cast<LocalDOMWindow*>(execution_context);
            }
        }
    }
    return MakeGarbageCollected<MouseEvent>(type, initializer, base::TimeTicks::Now(), kRealOrIndistinguishable, kMenuSourceNone, fallback_dom_window);
}

MouseEvent* MouseEvent::Create(const AtomicString& event_type, const MouseEventInit* initializer, base::TimeTicks platform_time_stamp,
    SyntheticEventType synthetic_event_type, WebMenuSourceType menu_source_type)
{
    return MakeGarbageCollected<MouseEvent>(event_type, initializer, platform_time_stamp, synthetic_event_type, menu_source_type);
}
...
MouseEvent::MouseEvent(const AtomicString& event_type, const MouseEventInit* initializer, base::TimeTicks platform_time_stamp,
    SyntheticEventType synthetic_event_type, WebMenuSourceType menu_source_type, LocalDOMWindow* fallback_dom_window)
    ...
    , position_type_(synthetic_event_type == kPositionless ? PositionType::kPositionless : PositionType::kPosition)
...
{
    InitCoordinates(initializer->clientX(), initializer->clientY(), fallback_dom_window);
    modifiers_ |= ButtonsToWebInputEventModifiers(buttons_);
}

void MouseEvent::InitCoordinates(const double client_x, const double client_y, const LocalDOMWindow* fallback_dom_window)
{
    ...
    float zoom_factor = LayoutZoomFactor(local_dom_window ? local_dom_window : fallback_dom_window);
...
void MouseEvent::SetCoordinatesFromWebPointerProperties(
    const WebPointerProperties& web_pointer_properties, const LocalDOMWindow* dom_window, MouseEventInit* initializer)
{
    ...
    if (dom_window && dom_window->GetFrame() && dom_window->GetFrame()->View()) {
        ...
        if (Page* p = frame->GetPage()) {
            if (p->GetPointerLockController().GetElement() && !p->GetPointerLockController().LockPending()) {
                p->GetPointerLockController().GetPointerLockPosition(&root_frame_point, &screen_point);
            }
        }
        gfx::PointF frame_point = frame->View()->ConvertFromRootFrame(root_frame_point);
...
void MouseEvent::initMouseEvent(ScriptState* script_state, const AtomicString& type, bool bubbles, bool cancelable, AbstractView* view, int detail,
    int screen_x, int screen_y, int client_x, int client_y, bool ctrl_key, bool alt_key, bool shift_key, bool meta_key, int16_t button,
    EventTarget* related_target, uint16_t buttons)
{
    if (IsBeingDispatched())
        return;

    if (script_state && script_state->World().IsIsolatedWorld())
        UIEventWithKeyState::DidCreateEventInIsolatedWorld(ctrl_key, alt_key, shift_key, meta_key);

    InitModifiers(ctrl_key, alt_key, shift_key, meta_key);
    InitMouseEventInternal(
        type, bubbles, cancelable, view, detail, screen_x, screen_y, client_x, client_y, GetModifiers(), button, related_target, nullptr, buttons);
}

void MouseEvent::InitMouseEventInternal(const AtomicString& type, bool bubbles, bool cancelable, AbstractView* view, int detail, double screen_x,
    double screen_y, double client_x, double client_y, WebInputEvent::Modifiers modifiers, int16_t button, EventTarget* related_target,
    InputDeviceCapabilities* source_capabilities, uint16_t buttons)
{
    InitUIEventInternal(type, bubbles, cancelable, related_target, view, detail, source_capabilities);
...
void MouseEvent::InitCoordinatesForTesting(double screen_x, double screen_y, double client_x, double client_y)
{
    screen_x_ = screen_x;
    screen_y_ = screen_y;
    InitCoordinates(client_x, client_y);
...
int16_t MouseEvent::button() const
{
    ...
    if (button_ == -1 || event_name == event_type_names::kMousemove || event_name == event_type_names::kMouseleave
        || event_name == event_type_names::kMouseenter || event_name == event_type_names::kMouseover || event_name == event_type_names::kMouseout) {
        return 0;
    }
    return button_;
}
...
Node* MouseEvent::toElement() const
{
    ...
    // pointer"
    if (type() == event_type_names::kMouseout || type() == event_type_names::kMouseleave)
...
DispatchEventResult MouseEvent::DispatchEvent(EventDispatcher& dispatcher)
{
    ...
    if (is_click || type() == event_type_names::kMousedown || type() == event_type_names::kMouseup || type() == event_type_names::kDblclick) {
    ...
    if (is_click) {
        ...
        for (unsigned i = 0; i < path.size(); i++) {
            ...
            if (saw_disabled_control && node.WillRespondToMouseClickEvents()) {
                UseCounter::Count(node.GetDocument(), WebFeature::kParentOfDisabledFormControlRespondsToMouseEvents);
            }
    ...
    double_click_event.InitMouseEventInternal(event_type_names::kDblclick, bubbles(), cancelable(), view(), detail(), screenX(), screenY(), clientX(),
        clientY(), GetModifiers(), button(), related_target, sourceCapabilities(), buttons());
    ...
    DispatchEventResult double_click_dispatch_result = EventDispatcher::DispatchEvent(dispatcher.GetNode(), double_click_event);
...
void MouseEvent::ComputeRelativePosition()
{
    ...
    LocalDOMWindow* dom_window_for_zoom_factor = DynamicTo<LocalDOMWindow>(view());
    ...
    if (const LayoutObject* layout_object = FindTargetLayoutObject(target_node)) {
        gfx::PointF local_pos = layout_object->AbsoluteToLocalPoint(AbsoluteLocation());

        if (layout_object->IsInline()) {
            UseCounter::Count(target_node->GetDocument(), WebFeature::kMouseEventRelativePositionForInlineElement);
        }
    ...
    if (n) {
        ...
        PhysicalOffset physical_offset = layer->GetLayoutObject().LocalToAbsolutePoint(PhysicalOffset(), kIgnoreTransforms);
...
void MouseEvent::RecordLayerXYMetrics()
{
    ...
    if (IsA<HTMLMediaElement>(node)) {
        ...
        UseCounter::Count(node->GetDocument(), WebFeature::kLayerXYWithCanvasTarget);
