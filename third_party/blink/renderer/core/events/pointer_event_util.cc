...
namespace blink {
// static
double PointerEventUtil::AzimuthFromTilt(double tilt_x_degrees, double tilt_y_degrees)
{
    ...
    DCHECK(tilt_x_degrees != 0.0 && tilt_y_degrees != 0.0 && abs(tilt_x_degrees) != 90 && abs(tilt_y_degrees) != 90);
    ...
    azimuth_radians = (azimuth_radians >= 0) ? azimuth_radians : (azimuth_radians + kTwoPiDouble);
...
// static
double PointerEventUtil::AltitudeFromTilt(double tilt_x_degrees, double tilt_y_degrees)
{
    DCHECK(tilt_x_degrees >= -90 && tilt_x_degrees <= 90);
    DCHECK(tilt_y_degrees >= -90 && tilt_y_degrees <= 90);
    ...
    return atan(1.0 / sqrt(pow(tan(tilt_x_radians), 2) + pow(tan(tilt_y_radians), 2)));
}

// static
int32_t PointerEventUtil::TiltXFromSpherical(double azimuth_radians, double altitude_radians)
{
    DCHECK(azimuth_radians >= 0 && azimuth_radians <= kTwoPiDouble);
    ...
    if (altitude_radians != 0) {
        ...
        return std::floor(Rad2deg(atan(cos(azimuth_radians) / tan(altitude_radians))) + 0.5);
    }

    if (azimuth_radians == kPiOverTwoDouble || azimuth_radians == 3 * kPiOverTwoDouble) {
        return 0;
    } else if (azimuth_radians < kPiOverTwoDouble || azimuth_radians > 3 * kPiOverTwoDouble) {
...
// static
int32_t PointerEventUtil::TiltYFromSpherical(double azimuth_radians, double altitude_radians)
{
    DCHECK(azimuth_radians >= 0 && azimuth_radians <= kTwoPiDouble);
    DCHECK(altitude_radians >= 0 && altitude_radians <= kPiOverTwoDouble);
    if (altitude_radians != 0) {
        // Not using std::round because we need Javascript Math.round behaviour
        // here which is different
        return std::floor(Rad2deg(atan(sin(azimuth_radians) / tan(altitude_radians))) + 0.5);
    }
    if (azimuth_radians == 0 || azimuth_radians == kPiDouble || azimuth_radians == kTwoPiDouble) {
        return 0;
...
// static
double PointerEventUtil::TransformToAzimuthInValidRange(double azimuth_radians)
{
...
// static
double PointerEventUtil::TransformToAltitudeInValidRange(double altitude_radians)
{
