...
public:
    static ToggleEvent* Create(const AtomicString& type, const ToggleEventInit* initializer)
    {
        return MakeGarbageCollected<ToggleEvent>(type, initializer);
    }
    static ToggleEvent* Create(const AtomicString& type, Event::Cancelable cancelable, const String& old_state, const String& new_state)
    {
        auto* event = MakeGarbageCollected<ToggleEvent>(type, cancelable, old_state, new_state);
        DCHECK(!event->bubbles());
        return event;
    }

    ToggleEvent();
    ToggleEvent(const AtomicString& type, Event::Cancelable cancelable, const String& old_state, const String& new_state);
