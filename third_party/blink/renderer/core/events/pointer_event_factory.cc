...
void UpdateCommonPointerEventInit(
    const WebPointerEvent& web_pointer_event, const gfx::PointF& last_global_position, LocalDOMWindow* dom_window, PointerEventInit* pointer_event_init)
{
    ...
    WebPointerEvent web_pointer_event_in_root_frame = web_pointer_event.WebPointerEventInRootFrame();

    MouseEvent::SetCoordinatesFromWebPointerProperties(web_pointer_event_in_root_frame, dom_window, pointer_event_init);
    if (!web_pointer_event.is_raw_movement_event
        && (web_pointer_event.GetType() == WebInputEvent::Type::kPointerMove || web_pointer_event.GetType() == WebInputEvent::Type::kPointerRawUpdate)) {
        float device_scale_factor = 1;

        // movementX/Y is type int for pointerevent, so we still need to truncated
        // the coordinates before calculate movement.
        pointer_event_init->setMovementX(base::saturated_cast<int>(web_pointer_event.PositionInScreen().x() * device_scale_factor)
            - base::saturated_cast<int>(last_global_position.x() * device_scale_factor));
        pointer_event_init->setMovementY(base::saturated_cast<int>(web_pointer_event.PositionInScreen().y() * device_scale_factor)
            - base::saturated_cast<int>(last_global_position.y() * device_scale_factor));
    }
    ...
    if (web_pointer_event_in_root_frame.HasWidth() && web_pointer_event_in_root_frame.HasHeight()
        ...
        gfx::SizeF point_shape = gfx::ScaleSize(gfx::SizeF(web_pointer_event_in_root_frame.width, web_pointer_event_in_root_frame.height), scale_factor);
        pointer_event_init->setWidth(point_shape.width());
        pointer_event_init->setHeight(point_shape.height());
    }
    pointer_event_init->setPressure(GetPointerEventPressure(web_pointer_event.force, pointer_event_init->buttons()));
    pointer_event_init->setTiltX(round(web_pointer_event.tilt_x));
    pointer_event_init->setTiltY(round(web_pointer_event.tilt_y));
    pointer_event_init->setAltitudeAngle(PointerEventUtil::AltitudeFromTilt(web_pointer_event.tilt_x, web_pointer_event.tilt_y));
    pointer_event_init->setAzimuthAngle(PointerEventUtil::AzimuthFromTilt(web_pointer_event.tilt_x, web_pointer_event.tilt_y));
    pointer_event_init->setTangentialPressure(web_pointer_event.tangential_pressure);
...
// static
const AtomicString& PointerEventFactory::PointerTypeNameForWebPointPointerType(WebPointerProperties::PointerType type)
{
...
HeapVector<Member<PointerEvent>> PointerEventFactory::CreateEventSequence(
    const WebPointerEvent& web_pointer_event, const PointerEventInit* pointer_event_init, const Vector<WebPointerEvent>& event_list, LocalDOMWindow* view)
{
    ...
    if (!event_list.empty()) {
        ...
        gfx::PointF last_global_position = GetLastPointerPosition(pointer_event_init->pointerId(), event_list.front(), web_pointer_event.GetType());
        ...
        for (const auto& event : event_list) {
            ...
            UpdateCommonPointerEventInit(event, last_global_position, view, new_event_init);
            UIEventWithKeyState::SetFromWebInputEventModifiers(new_event_init, static_cast<WebInputEvent::Modifiers>(event.GetModifiers()));

            last_global_position = event.PositionInScreen();

            if (pointer_event_init->hasPersistentDeviceId()) {
                new_event_init->setPersistentDeviceId(pointer_event_init->persistentDeviceId());
            }

            PointerEvent* pointer_event = PointerEvent::Create(type, new_event_init, event.TimeStamp());
...
PointerEventInit* PointerEventFactory::ConvertIdTypeButtonsEvent(const WebPointerEvent& web_pointer_event)
{
    WebPointerProperties::PointerType pointer_type = web_pointer_event.pointer_type;

    unsigned buttons;
    if (web_pointer_event.hovering) {
        buttons = MouseEvent::WebInputEventModifiersToButtons(static_cast<WebInputEvent::Modifiers>(web_pointer_event.GetModifiers()));
    } else {
        ...
        buttons = static_cast<unsigned>(
            (web_pointer_event.GetType() == WebInputEvent::Type::kPointerUp || web_pointer_event.GetType() == WebInputEvent::Type::kPointerCancel)
    ...
    PointerId pointer_id = AddOrUpdateIdAndActiveButtons(
        incoming_id, buttons != 0, web_pointer_event.hovering, web_pointer_event.GetType(), web_pointer_event.unique_touch_event_id);
    ...
    pointer_event_init->setPointerType(PointerTypeNameForWebPointPointerType(pointer_type));
    pointer_event_init->setIsPrimary(IsPrimary(pointer_id));

    return pointer_event_init;
}

void PointerEventFactory::SetEventSpecificFields(PointerEventInit* pointer_event_init, const AtomicString& type)
{
    bool is_pointer_enter_or_leave = type == event_type_names::kPointerenter || type == event_type_names::kPointerleave;
    pointer_event_init->setBubbles(!is_pointer_enter_or_leave);
    pointer_event_init->setCancelable(type != event_type_names::kPointerenter && type != event_type_names::kPointerleave
        && type != event_type_names::kPointercancel && type != event_type_names::kPointerrawupdate && type != event_type_names::kGotpointercapture
...
PointerEvent* PointerEventFactory::Create(const WebPointerEvent& web_pointer_event, const Vector<WebPointerEvent>& coalesced_events,
    const Vector<WebPointerEvent>& predicted_events, LocalDOMWindow* view)
{
    const WebInputEvent::Type event_type = web_pointer_event.GetType();
    DCHECK(event_type == WebInputEvent::Type::kPointerDown || event_type == WebInputEvent::Type::kPointerUp || event_type == WebInputEvent::Type::kPointerMove
        || event_type == WebInputEvent::Type::kPointerRawUpdate || event_type == WebInputEvent::Type::kPointerCancel);

    PointerEventInit* pointer_event_init = ConvertIdTypeButtonsEvent(web_pointer_event);
    ...
    if (event_type == WebInputEvent::Type::kPointerDown || event_type == WebInputEvent::Type::kPointerUp) {
        ...
        // TODO(mustaq): Fix when the spec starts supporting hovering erasers.
        if (web_pointer_event.pointer_type == WebPointerProperties::PointerType::kEraser && button == WebPointerProperties::Button::kLeft) {
        ...
        if ((event_type == WebInputEvent::Type::kPointerDown && (pointer_event_init->buttons() & ~ButtonToButtonsBitfield(button)) != 0)
            || (event_type == WebInputEvent::Type::kPointerUp && pointer_event_init->buttons() != 0)) {
            type = event_type_names::kPointermove;
        }
    } else {
        pointer_event_init->setButton(static_cast<int16_t>(WebPointerProperties::Button::kNoButton));
    }

    pointer_event_init->setView(view);
    UpdateCommonPointerEventInit(
        web_pointer_event, GetLastPointerPosition(pointer_event_init->pointerId(), web_pointer_event, event_type), view, pointer_event_init);

    UIEventWithKeyState::SetFromWebInputEventModifiers(pointer_event_init, static_cast<WebInputEvent::Modifiers>(web_pointer_event.GetModifiers()));

    SetEventSpecificFields(pointer_event_init, type);

    HeapVector<Member<PointerEvent>> coalesced_pointer_events, predicted_pointer_events;
    if (type == event_type_names::kPointermove || type == event_type_names::kPointerrawupdate) {
        coalesced_pointer_events = CreateEventSequence(web_pointer_event, pointer_event_init, coalesced_events, view);
    }
    if (type == event_type_names::kPointermove) {
        predicted_pointer_events = CreateEventSequence(web_pointer_event, pointer_event_init, predicted_events, view);
    }
    ...
    SetLastPosition(pointer_event_init->pointerId(), web_pointer_event.PositionInScreen(), event_type);

    pointer_event_init->setPersistentDeviceId(GetBlinkDeviceId(web_pointer_event));

    return PointerEvent::Create(type, pointer_event_init, web_pointer_event.TimeStamp(), MouseEvent::kRealOrIndistinguishable, kMenuSourceNone,
        web_pointer_event.GetPreventCountingAsInteraction());
}

void PointerEventFactory::SetLastPosition(int pointer_id, const gfx::PointF& position_in_screen, WebInputEvent::Type event_type)
{
    PointerAttributes attributes = pointer_id_to_attributes_.Contains(pointer_id) ? pointer_id_to_attributes_.at(pointer_id) : PointerAttributes();
...
gfx::PointF PointerEventFactory::GetLastPointerPosition(int pointer_id, const WebPointerProperties& event, WebInputEvent::Type event_type) const
{
    if (event_type == WebInputEvent::Type::kPointerRawUpdate) {
        if (pointer_id_to_attributes_.Contains(pointer_id) && pointer_id_to_attributes_.at(pointer_id).last_rawupdate_position.has_value()) {
            return pointer_id_to_attributes_.at(pointer_id).last_rawupdate_position.value();
        }
    } else {
        if (pointer_id_to_attributes_.Contains(pointer_id) && pointer_id_to_attributes_.at(pointer_id).last_position.has_value()) {
...
PointerEvent* PointerEventFactory::CreatePointerCancelEvent(const int pointer_id, base::TimeTicks platfrom_time_stamp, const int32_t device_id)
{
    ...
    pointer_event_init->setPointerId(pointer_id);
    pointer_event_init->setPointerType(PointerTypeNameForWebPointPointerType(pointer_id_to_attributes_.at(pointer_id).incoming_id.GetPointerType()));
    pointer_event_init->setIsPrimary(IsPrimary(pointer_id));
    ...
    return PointerEvent::Create(event_type_names::kPointercancel, pointer_event_init, platfrom_time_stamp);
}

PointerEvent* PointerEventFactory::CreatePointerEventFrom(PointerEvent* pointer_event, const AtomicString& type, EventTarget* related_target)
{
    PointerEventInit* pointer_event_init = PointerEventInit::Create();
    ...
    pointer_event_init->setPersistentDeviceId(pointer_event->persistentDeviceId());

    SetEventSpecificFields(pointer_event_init, type);

    if (const UIEventWithKeyState* key_state_event = FindEventWithKeyState(pointer_event)) {
        UIEventWithKeyState::SetFromWebInputEventModifiers(pointer_event_init, key_state_event->GetModifiers());
    }
    ...
    return PointerEvent::Create(type, pointer_event_init, pointer_event->PlatformTimeStamp());
}

PointerEvent* PointerEventFactory::CreatePointerRawUpdateEvent(PointerEvent* pointer_event)
{
    ...
    DCHECK(pointer_event->type() == event_type_names::kPointermove
        && (pointer_event->buttons() & ~ButtonToButtonsBitfield(static_cast<WebPointerProperties::Button>(pointer_event->button()))) != 0
        && pointer_event->button() != 0);

    return CreatePointerEventFrom(pointer_event, event_type_names::kPointerrawupdate, pointer_event->relatedTarget());
}

PointerEvent* PointerEventFactory::CreatePointerCaptureEvent(PointerEvent* pointer_event, const AtomicString& type)
{
    DCHECK(type == event_type_names::kGotpointercapture || type == event_type_names::kLostpointercapture);

    return CreatePointerEventFrom(pointer_event, type, pointer_event->relatedTarget());
}

PointerEvent* PointerEventFactory::CreatePointerBoundaryEvent(PointerEvent* pointer_event, const AtomicString& type, EventTarget* related_target)
{
    DCHECK(type == event_type_names::kPointerout || type == event_type_names::kPointerleave || type == event_type_names::kPointerover
...
{
    for (int type = 0; type <= ToInt(WebPointerProperties::PointerType::kMaxValue); type++) {
    ...
    attributes.incoming_id = IncomingId(WebPointerProperties::PointerType::kMouse, 0);
...
PointerId PointerEventFactory::AddOrUpdateIdAndActiveButtons(
    const IncomingId p, bool is_active_buttons, bool hovering, WebInputEvent::Type event_type, uint32_t unique_touch_event_id)
{
    ...
    pointer_id_to_attributes_.insert(mapped_id,
...
Vector<PointerId> PointerEventFactory::GetPointerIdsOfNonHoveringPointers() const
{
...
bool PointerEventFactory::IsPrimary(const WebPointerProperties& properties) const
{
...
bool PointerEventFactory::IsActiveButtonsState(const PointerId pointer_id) const
{
    return pointer_id_to_attributes_.Contains(pointer_id) && pointer_id_to_attributes_.at(pointer_id).is_active_buttons;
}

WebPointerProperties::PointerType PointerEventFactory::GetPointerType(PointerId pointer_id) const
{
...
PointerId PointerEventFactory::GetPointerEventId(const WebPointerProperties& properties) const
{
    if (properties.pointer_type == WebPointerProperties::PointerType::kMouse) {
...
PointerId PointerEventFactory::GetPointerIdForTouchGesture(const uint32_t unique_touch_event_id)
{
    const auto& unique_touch_id_matcher = [](const uint32_t unique_touch_event_id, const blink::PointerEventFactory::PointerAttributes attributes) {
        return attributes.incoming_id.GetPointerType() == WebPointerProperties::PointerType::kTouch
    ...
    for (const auto& id : pointer_id_to_attributes_.Keys()) {
        if (unique_touch_id_matcher(unique_touch_event_id, pointer_id_to_attributes_.at(id))) {
    ...
    for (const auto& id_attributes_pair : recently_removed_pointers_) {
        if (unique_touch_id_matcher(unique_touch_event_id, id_attributes_pair.second)) {
    ...
    pointer_attributes.incoming_id = IncomingId(WebPointerProperties::PointerType::kTouch, kInvalidId);
    SaveRecentlyRemovedPointer(pointer_id, pointer_attributes);

    return pointer_id;
}

void PointerEventFactory::SaveRecentlyRemovedPointer(PointerId pointer_id, PointerAttributes pointer_attributes)
{
...
int32_t PointerEventFactory::GetBlinkDeviceId(const WebPointerEvent& web_pointer_event)
{
    ...
    TRACE_EVENT_INSTANT1("event", "PointerEventFactory::GetBlinkDeviceId", TRACE_EVENT_SCOPE_THREAD, "id", result.stored_value->value);
