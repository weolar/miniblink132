...
public:
    TestURLRegistrable(URLRegistry* registry, mojo::PendingRemote<mojom::blink::Blob> blob = mojo::NullRemote())
        : registry_(registry)
        , blob_(std::move(blob))
    {
    }
    ...
    bool IsMojoBlob() override
    {
        return bool { blob_ };
    }

    void CloneMojoBlob(mojo::PendingReceiver<mojom::blink::Blob> receiver) override
    {
        if (!blob_)
            return;
...
public:
    PublicURLManagerTest()
        : url_store_receiver_(&url_store_)
    {
    }
    ...
    void SetUp() override
    {
        ...
        HeapMojoAssociatedRemote<BlobURLStore> url_store_remote(GetExecutionContext());
        url_store_receiver_.Bind(url_store_remote.BindNewEndpointAndPassDedicatedReceiver());
    ...
    mojo::PendingRemote<mojom::blink::Blob> CreateMojoBlob(const String& uuid)
    {
        ...
        mojo::MakeSelfOwnedReceiver(std::make_unique<FakeBlob>(uuid), result.InitWithNewPipeAndPassReceiver());
        return result;
    }
    ...
    {
        page_holder_->GetDocument().SetURL(url);
    }
    ...
    void SetUpSecurityContextForTesting()
    {
        ...
        security_context.SetSecurityOriginForTesting(SecurityOrigin::Create(window->Url()));
...
TEST_F(PublicURLManagerTest, RegisterNonMojoBlob)
{
    ...
    EXPECT_TRUE(SecurityOrigin::CreateFromString(url)->IsSameOriginWith(GetExecutionContext()->GetSecurityOrigin()));
    EXPECT_EQ(GetExecutionContext()->GetSecurityOrigin(), SecurityOrigin::CreateFromString(url));

    url_manager().Revoke(KURL(url));
    EXPECT_FALSE(SecurityOrigin::CreateFromString(url)->IsSameOriginWith(GetExecutionContext()->GetSecurityOrigin()));
