...
{
    bytes_loaded_ = 0;
    raw_data_ = ArrayBufferContents(static_cast<unsigned>(total_bytes), 1, ArrayBufferContents::kNotShared, ArrayBufferContents::kDontInitialize);
...
FileErrorCode FileReaderAccumulator::DidReceiveData(base::span<const uint8_t> data)
{
    // Fill out the buffer
    ...
    raw_data_.ByteSpan().subspan(base::checked_cast<size_t>(bytes_loaded_)).copy_prefix_from(data);
...
std::pair<FileErrorCode, FileReaderData> SyncedFileReaderAccumulator::Load(
    scoped_refptr<BlobDataHandle> handle, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    auto* client = MakeGarbageCollected<SyncedFileReaderAccumulator>();
    auto* file_reader = MakeGarbageCollected<FileReaderLoader>(client, std::move(task_runner));
