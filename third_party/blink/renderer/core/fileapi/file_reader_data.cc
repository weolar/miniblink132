...
String ToTextString(ArrayBufferContents raw_data, const WTF::TextEncoding& encoding)
{
    ...
    StringBuilder builder;
    auto decoder
        = TextResourceDecoder(TextResourceDecoderOptions(TextResourceDecoderOptions::kPlainTextContent, encoding.IsValid() ? encoding : UTF8Encoding()));
...
String ToString(ArrayBufferContents raw_data, FileReadType read_type, const WTF::TextEncoding& encoding, const String& data_type)
{
    switch (read_type) {
...
String FileReaderData::AsString(FileReadType read_type, const String& encoding, const String& data_type) &&
{
    CHECK(raw_data_.IsValid());
    return ToString(std::move(raw_data_), read_type, WTF::TextEncoding(encoding), data_type);
}
