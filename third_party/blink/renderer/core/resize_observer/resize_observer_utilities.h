...
public:
    ...
    // device-pixel boxes.
    static gfx::SizeF ComputeZoomAdjustedBox(ResizeObserverBoxOptions box_option, const LayoutBox& layout_box, const ComputedStyle& style);

    // Compute a scaled and pixel snapped device pixel content box for svg
    // bounding boxes.
    static gfx::SizeF ComputeSnappedDevicePixelContentBox(LogicalSize box_size, const LayoutObject& layout_object, const ComputedStyle& style);
    static gfx::SizeF ComputeSnappedDevicePixelContentBox(const gfx::SizeF& box_size, const LayoutObject& layout_object, const ComputedStyle& style);

    static DOMRectReadOnly* ZoomAdjustedPhysicalRect(PhysicalRect content_rect, const ComputedStyle& style);
};
