...
gfx::SizeF ComputeZoomAdjustedSVGBox(ResizeObserverBoxOptions box_option, const LayoutObject& layout_object)
{
    ...
    case ResizeObserverBoxOptions::kDevicePixelContentBox: {
        ...
        return ResizeObserverUtilities::ComputeSnappedDevicePixelContentBox(scaled_bounding_box_size, layout_object, style);
    }
    }
}
...
ResizeObservation::ResizeObservation(Element* target, ResizeObserver* observer, ResizeObserverBoxOptions observed_box)
    : target_(target)
    , observer_(observer)
...
bool ResizeObservation::ObservationSizeOutOfSync()
{
    ...
    if (target_ && DisplayLockUtilities::IsInLockedSubtreeCrossingFrames(*target_)) [[unlikely]] {
        return false;
    }
    ...
    if (observer_->SkipNonAtomicInlineObservations() && target_->GetLayoutObject() && target_->GetLayoutObject()->IsInline()
...
void ResizeObservation::SetObservationSize(const LogicalSize& observation_size)
{
...
{
    unsigned depth = 0;
    for (Element* parent = target_; parent; parent = FlatTreeTraversal::ParentElement(*parent))
        ++depth;
    return depth;
}
...
LogicalSize ResizeObservation::ComputeTargetSize() const
{
    ...
    if (const auto* layout_box = DynamicTo<LayoutBox>(layout_object)) {
        gfx::SizeF size = ResizeObserverUtilities::ComputeZoomAdjustedBox(observed_box_, *layout_box, layout_box->StyleRef());
        return LogicalSize(LayoutUnit(size.width()), LayoutUnit(size.height()));
    }
    return LogicalSize();
}
