...
ResizeObserver* ResizeObserver::Create(ScriptState* script_state, V8ResizeObserverCallback* callback)
{
    return MakeGarbageCollected<ResizeObserver>(callback, LocalDOMWindow::From(script_state));
}

ResizeObserver* ResizeObserver::Create(LocalDOMWindow* window, Delegate* delegate)
{
    return MakeGarbageCollected<ResizeObserver>(delegate, window);
}

ResizeObserver::ResizeObserver(V8ResizeObserverCallback* callback, LocalDOMWindow* window)
...
ResizeObserverBoxOptions ResizeObserver::V8EnumToBoxOptions(V8ResizeObserverBoxOptions::Enum box_options)
{
    switch (box_options) {
...
void ResizeObserver::observeInternal(Element* target, ResizeObserverBoxOptions box_option)
{
    ...
    auto* observation = MakeGarbageCollected<ResizeObservation>(target, this, box_option);
...
{
    ResizeObserverBoxOptions box_option = V8EnumToBoxOptions(options->box().AsEnum());
...
void ResizeObserver::DeliverObservations()
{
    ...
    for (auto& observation : active_observations_) {
        ...
        auto* entry = MakeGarbageCollected<ResizeObserverEntry>(observation->Target());
