...
TEST_F(ResizeObserverUnitTest, ResizeObserverDOMContentBoxAndSVG)
{
    ...
    ResizeObserver::Delegate* delegate = MakeGarbageCollected<TestResizeObserverDelegate>(Window());
    ...
    ResizeObservation* dom_observation = MakeGarbageCollected<ResizeObservation>(dom_target, observer, ResizeObserverBoxOptions::kContentBox);
    ResizeObservation* svg_observation = MakeGarbageCollected<ResizeObservation>(svg_target, observer, ResizeObserverBoxOptions::kContentBox);
    ...
    // Target depths
    ASSERT_EQ(svg_observation->TargetDepth() - dom_observation->TargetDepth(), (size_t)1);
}
...
TEST_F(ResizeObserverUnitTest, ResizeObserverDOMBorderBox)
{
    ...
    ResizeObserver::Delegate* delegate = MakeGarbageCollected<TestResizeObserverDelegate>(Window());
    ResizeObserver* observer = ResizeObserver::Create(&Window(), delegate);
    Element* dom_border_target = GetDocument().getElementById(AtomicString("domBorderTarget"));
    auto* dom_border_observation = MakeGarbageCollected<ResizeObservation>(dom_border_target, observer, ResizeObserverBoxOptions::kBorderBox);
...
TEST_F(ResizeObserverUnitTest, ResizeObserverDOMDevicePixelContentBox)
{
    ...
    ResizeObserver* observer = ResizeObserver::Create(&Window(), delegate);
    Element* dom_target = GetDocument().getElementById(AtomicString("domTarget"));
    Element* dom_dp_target = GetDocument().getElementById(AtomicString("domDPTarget"));

    auto* dom_dp_nested_observation = MakeGarbageCollected<ResizeObservation>(dom_dp_target, observer, ResizeObserverBoxOptions::kDevicePixelContentBox);
    auto* dom_dp_observation = MakeGarbageCollected<ResizeObservation>(dom_target, observer, ResizeObserverBoxOptions::kDevicePixelContentBox);
...
{
    ResizeObserverController& controller = *ResizeObserverController::From(Window());
    const HeapLinkedHashSet<WeakMember<ResizeObserver>>& observers = controller.Observers();
    ASSERT_EQ(observers.size(), 0U);

    //
    // Test whether ResizeObserver is kept alive by direct JS reference
    //
    ClassicScript::CreateUnspecifiedScript("var ro = new ResizeObserver( entries => {});")->RunScript(&Window());
    ASSERT_EQ(observers.size(), 1U);
    ClassicScript::CreateUnspecifiedScript("ro = undefined;")->RunScript(&Window());
    ...
    //
    ClassicScript::CreateUnspecifiedScript("var ro = new ResizeObserver( () => {});"
    ...
    WebHeap::CollectAllGarbageForTesting();
    ASSERT_EQ(observers.size(), 1U);
    ClassicScript::CreateUnspecifiedScript("el = undefined;")->RunScript(&Window());
    ThreadState::Current()->CollectAllGarbageForTesting();
    WebHeap::CollectAllGarbageForTesting();
    ASSERT_EQ(observers.empty(), true);
}
