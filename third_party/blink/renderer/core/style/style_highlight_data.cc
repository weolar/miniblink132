...
bool HighlightStyleMapEquals(const CustomHighlightsStyleMap& a, const CustomHighlightsStyleMap& b)
{
    if (a.size() != b.size()) {
        return false;
    }
    ...
    for (CustomHighlightsStyleMap::const_iterator it = a.begin(); it != a_end; ++it) {
...
{
    return base::ValuesEquivalent(selection_, other.selection_) && base::ValuesEquivalent(target_text_, other.target_text_)
        && base::ValuesEquivalent(search_text_current_, other.search_text_current_)
        && base::ValuesEquivalent(search_text_not_current_, other.search_text_not_current_) && base::ValuesEquivalent(spelling_error_, other.spelling_error_)
        && base::ValuesEquivalent(grammar_error_, other.grammar_error_) && HighlightStyleMapEquals(custom_highlights_, other.custom_highlights_);
}

const ComputedStyle* StyleHighlightData::Style(PseudoId pseudo_id, const AtomicString& pseudo_argument) const
{
...
const ComputedStyle* StyleHighlightData::CustomHighlight(const AtomicString& highlight_name) const
{
    if (highlight_name) {
...
void StyleHighlightData::SetCustomHighlight(const AtomicString& highlight_name, const ComputedStyle* style)
{
...
{
    if ((selection_ && (selection_->DependsOnSizeContainerQueries() || selection_->HasContainerRelativeUnits()))
        || (target_text_ && (target_text_->DependsOnSizeContainerQueries() || target_text_->HasContainerRelativeUnits()))
        || (spelling_error_ && (spelling_error_->DependsOnSizeContainerQueries() || spelling_error_->HasContainerRelativeUnits()))
        || (grammar_error_ && (grammar_error_->DependsOnSizeContainerQueries() || grammar_error_->HasContainerRelativeUnits()))) {
        return true;
    }
    for (auto style : custom_highlights_) {
        if (style.value->DependsOnSizeContainerQueries() || style.value->HasContainerRelativeUnits()) {
            return true;
        }
    }
    return false;
