...
protected:
    ...
    CORE_EXPORT base::RefCountedData<Vector<AppliedTextDecoration, 1>>* EnsureAppliedTextDecorationsCache() const;
...
public:
    ...
    ALWAYS_INLINE ComputedStyle(BuilderPassKey, const ComputedStyle& initial_style);
    ...
    CORE_EXPORT static Difference ComputeDifference(const ComputedStyle* old_style, const ComputedStyle* new_style);

    // Returns true if the ComputedStyle change requires a LayoutObject re-attach.
    static bool NeedsReattachLayoutTree(const Element& element, const ComputedStyle* old_style, const ComputedStyle* new_style);

    ContentPosition ResolvedJustifyContentPosition(const StyleContentAlignmentData& normal_value_behavior) const;
    ContentDistributionType ResolvedJustifyContentDistribution(const StyleContentAlignmentData& normal_value_behavior) const;
    ContentPosition ResolvedAlignContentPosition(const StyleContentAlignmentData& normal_value_behavior) const;
    ContentDistributionType ResolvedAlignContentDistribution(const StyleContentAlignmentData& normal_value_behavior) const;
    StyleSelfAlignmentData ResolvedAlignSelf(const StyleSelfAlignmentData& normal_value_behavior, const ComputedStyle* parent_style = nullptr) const;
    StyleContentAlignmentData ResolvedAlignContent(const StyleContentAlignmentData& normal_behaviour) const;
    StyleSelfAlignmentData ResolvedJustifySelf(const StyleSelfAlignmentData& normal_value_behavior, const ComputedStyle* parent_style = nullptr) const;
    StyleContentAlignmentData ResolvedJustifyContent(const StyleContentAlignmentData& normal_behaviour) const;

    CORE_EXPORT StyleDifference VisualInvalidationDiff(const Document&, const ComputedStyle&) const;

    CORE_EXPORT const ComputedStyle* GetCachedPseudoElementStyle(PseudoId, const AtomicString& pseudo_argument = g_null_atom) const;
    const ComputedStyle* AddCachedPseudoElementStyle(const ComputedStyle*, PseudoId, const AtomicString&) const;
    const ComputedStyle* ReplaceCachedPseudoElementStyle(const ComputedStyle* pseudo_style, PseudoId pseudo_id, const AtomicString& pseudo_argument) const;
   ...
   */
    ...
    {
        return !Filter().Operations().empty();
    }
    ...
    {
        return !Bottom().IsAuto() && Top().IsAuto();
    }
    ...
    {
        if (ColumnRuleStyle() == EBorderStyle::kNone || ColumnRuleStyle() == EBorderStyle::kHidden) {
            return 0;
        }
    ...
    {
        if (OutlineStyle() == EBorderStyle::kNone && other.OutlineStyle() == EBorderStyle::kNone) {
            return true;
        }
        return OutlineWidthInternal() == other.OutlineWidthInternal() && ResolvedColor(OutlineColor()) == other.ResolvedColor(other.OutlineColor())
            && OutlineStyle() == other.OutlineStyle() && OutlineOffset() == other.OutlineOffset() && OutlineStyleIsAuto() == other.OutlineStyleIsAuto();
    }
    ...
    {
        return OutlineStyleIsAuto() ? OutlineType::kIncludeBlockInkOverflow : OutlineType::kDontIncludeBlockInkOverflow;
    }

    // Scroll properties.

    PhysicalToLogicalGetter<const Length&, ComputedStyle> PhysicalScrollPaddingToLogicalGetter() const
    {
        return PhysicalToLogicalGetter<const Length&, ComputedStyle>(GetWritingDirection(), *this, &ComputedStyleBase::ScrollPaddingTop,
            &ComputedStyleBase::ScrollPaddingRight, &ComputedStyleBase::ScrollPaddingBottom, &ComputedStyleBase::ScrollPaddingLeft);
    }
    ...
    {
        return ShapeOutsideInternal().Get();
    }
    ...
    {
        return EffectiveZIndexZero() ? 0 : ZIndex();
    }
    ...
    // value is `outside`.
    bool MarkerShouldBeInside(const Element& parent, const DisplayStyle& marker_style) const;
    ...
    {
        return HasEmUnits() || HasRootFontRelativeUnits() || HasGlyphRelativeUnits();
    }
    bool HasAnyRelativeUnits() const
    {
        return HasFontRelativeUnits() || HasContainerRelativeUnits() || HasLogicalDirectionRelativeUnits() || HasViewportUnits();
    }

    // If true, the ComputedStyle must be recalculated when fonts are updated.
    bool DependsOnFontMetrics() const
    {
        return HasGlyphRelativeUnits() || HasFontSizeAdjust() || CustomStyleCallbackDependsOnFont();
    }

    template <typename Functor> bool HasCachedPseudoElementStyle(Functor& func) const
    {
    ...
    bool HasFill() const
    {
        return !FillPaint().IsNone();
    }
    bool IsFillColorCurrentColor() const
    {
        return FillPaint().HasCurrentColor() || InternalVisitedFillPaint().HasCurrentColor();
    }
    ...
    {
        return StrokePaint().HasCurrentColor() || InternalVisitedStrokePaint().HasCurrentColor();
    }
    bool HasDashArray() const
    {
        return !StrokeDashArray()->data.empty();
    }
    ...
    {
        return ChildHasExplicitInheritance();
    }
    ...
    bool CounterDirectivesEqual(const ComputedStyle& other) const
    {
        ...
        return base::ValuesEquivalent(CounterDirectivesInternal().get(), other.CounterDirectivesInternal().get());
    }

    bool IsDeprecatedWebkitBox() const
    {
        return Display() == EDisplay::kWebkitBox || Display() == EDisplay::kWebkitInlineBox;
    }
    bool IsDeprecatedFlexboxUsingFlexLayout() const
    {
        ...
        return IsDeprecatedWebkitBox() && !IsDeprecatedWebkitBoxWithVerticalLineClamp();
    }
    bool IsDeprecatedWebkitBoxWithVerticalLineClamp() const
    {
        DCHECK(!RuntimeEnabledFeatures::CSSLineClampWebkitBoxBlockificationEnabled());
        return IsDeprecatedWebkitBox() && BoxOrient() == EBoxOrient::kVertical && HasLineClamp();
    }
    ...
    CSSVariableData* GetVariableData(const AtomicString&, bool is_inherited_property) const;

    const CSSValue* GetVariableValue(const AtomicString&) const;
    const CSSValue* GetVariableValue(const AtomicString&, bool is_inherited_property) const;
    ...
    {
        return ColumnRuleColor().GetLegacyValue().Resolve(GetCurrentColor(), UsedColorScheme()).IsFullyTransparent();
    }
    ...
    bool HasColumnRule() const
    {
        ...
        return ColumnRuleWidth() && !ColumnRuleIsTransparent() && BorderStyleIsVisible(ColumnRuleStyle());
    }
    ...
    bool ResolvedIsColumnFlexDirection() const
    {
        ...
        return FlexDirection() == EFlexDirection::kColumn || FlexDirection() == EFlexDirection::kColumnReverse;
    }
    bool ResolvedIsRowReverseFlexDirection() const
    {
        if (IsDeprecatedWebkitBox()) {
            return BoxOrient() == EBoxOrient::kHorizontal && BoxDirection() == EBoxDirection::kReverse;
        }
    ...
    bool ResolvedIsReverseFlexDirection() const
    {
        ...
        return FlexDirection() == EFlexDirection::kRowReverse || FlexDirection() == EFlexDirection::kColumnReverse;
    }
    bool HasBoxReflect() const
    {
        return BoxReflect();
    }
    ...
    bool HasMask() const
    {
        return MaskInternal().AnyLayerHasImage() || MaskBoxImageInternal().HasImage();
    }
    ...
    {
        return MaskBoxImageInternal();
    }
    bool MaskBoxImageSlicesFill() const
    {
        return MaskBoxImageInternal().Fill();
    }

    // Text-combine utility functions.
    bool HasTextCombine() const
    {
        return TextCombine() != ETextCombine::kNone;
    }
    ...
    {
        return (GridAutoFlowInternal() & static_cast<int>(kInternalAutoFlowDirectionRow)) == kInternalAutoFlowDirectionRow;
    }
    bool IsGridAutoFlowDirectionColumn() const
    {
        return (GridAutoFlowInternal() & static_cast<int>(kInternalAutoFlowDirectionColumn)) == kInternalAutoFlowDirectionColumn;
    }
    bool IsGridAutoFlowAlgorithmSparse() const
    {
        return (GridAutoFlowInternal() & static_cast<int>(kInternalAutoFlowAlgorithmSparse)) == kInternalAutoFlowAlgorithmSparse;
    }
    bool IsGridAutoFlowAlgorithmDense() const
    {
        return (GridAutoFlowInternal() & static_cast<int>(kInternalAutoFlowAlgorithmDense)) == kInternalAutoFlowAlgorithmDense;
    }
    ...
    {
        return WillChangeProperties().Contains(CSSPropertyID::kOpacity) || WillChangeProperties().Contains(CSSPropertyID::kAliasWebkitOpacity);
    }
    ...
    {
        return WillChangeProperties().Contains(CSSPropertyID::kOffsetPath) || WillChangeProperties().Contains(CSSPropertyID::kOffsetPosition);
    }
    ...
    {
        return WillChangeProperties().Contains(CSSPropertyID::kFilter) || WillChangeProperties().Contains(CSSPropertyID::kAliasWebkitFilter);
    }
    ...
    [[nodiscard]] String ApplyTextTransform(const String&, UChar previous_character = ' ', TextOffsetMap* offset_map = nullptr) const;
    ...
    {
        return IsHorizontalWritingMode() ? ContainIntrinsicWidth() : ContainIntrinsicHeight();
    }
    const StyleIntrinsicLength& ContainIntrinsicBlockSize() const
    {
        return IsHorizontalWritingMode() ? ContainIntrinsicHeight() : ContainIntrinsicWidth();
    }
    ...
    const Length& MarginBlockEnd() const
    {
        return MarginBlockEndUsing(*this);
    }
    const Length& MarginInlineStart() const
    {
        return MarginInlineStartUsing(*this);
    }
    const Length& MarginInlineEnd() const
    {
        return MarginInlineEndUsing(*this);
    }
    ...
    {
        return PaddingTop() == other.PaddingTop() && PaddingLeft() == other.PaddingLeft() && PaddingRight() == other.PaddingRight()
            && PaddingBottom() == other.PaddingBottom();
    }
    bool PaddingEqual(const LengthBox& other) const
    {
        return PaddingTop() == other.Top() && PaddingLeft() == other.Left() && PaddingRight() == other.Right() && PaddingBottom() == other.Bottom();
    }
    ...
    {
        return BorderImage().Fill();
    }

    bool BorderSizeEquals(const ComputedStyle& o) const
    {
        return BorderLeftWidth() == o.BorderLeftWidth() && BorderTopWidth() == o.BorderTopWidth() && BorderRightWidth() == o.BorderRightWidth()
    ...
    bool BorderVisuallyEqual(const ComputedStyle& o) const
    {
        auto BorderSideVisuallyEqual
            = [&](const StyleColor& color, const StyleColor& other_color, EBorderStyle style, EBorderStyle other_style, int width, int other_width) -> bool {
            ...
            return width == other_width && style == other_style && ResolvedColor(color) == o.ResolvedColor(other_color);
        };

        return BorderSideVisuallyEqual(
                   BorderTopColor(), o.BorderTopColor(), BorderTopStyle(), o.BorderTopStyle(), BorderTopWidthInternal(), o.BorderTopWidthInternal())
            && BorderSideVisuallyEqual(
                BorderRightColor(), o.BorderRightColor(), BorderRightStyle(), o.BorderRightStyle(), BorderRightWidthInternal(), o.BorderRightWidthInternal())
            && BorderSideVisuallyEqual(BorderBottomColor(), o.BorderBottomColor(), BorderBottomStyle(), o.BorderBottomStyle(), BorderBottomWidthInternal(),
                o.BorderBottomWidthInternal())
            && BorderSideVisuallyEqual(
                BorderLeftColor(), o.BorderLeftColor(), BorderLeftStyle(), o.BorderLeftStyle(), BorderLeftWidthInternal(), o.BorderLeftWidthInternal())
    ...
    {
        return Floating() != EFloat::kNone;
    }
    EFloat UnresolvedFloating() const
    {
        return Floating();
    }
    ...
    {
        return GetBlendMode() != BlendMode::kNormal;
    }

    // Motion utility functions.
    bool HasOffset() const
    {
        return (!OffsetPosition().X().IsAuto() && !OffsetPosition().X().IsNone()) || OffsetPath();
    }
    ...
    {
        return GetPosition() == EPosition::kRelative || GetPosition() == EPosition::kSticky;
    }
    bool HasStickyConstrainedPosition() const
    {
        return GetPosition() == EPosition::kSticky && (!Top().IsAuto() || !Left().IsAuto() || !Right().IsAuto() || !Bottom().IsAuto());
    }
    static EPosition GetPosition(EDisplay display, EPosition position_internal)
    {
        ...
        if (position_internal == EPosition::kSticky && (display == EDisplay::kTableColumnGroup || display == EDisplay::kTableColumn)) {
    ...
    bool HasClear() const
    {
        return Clear() != EClear::kNone;
    }
    EClear UnresolvedClear() const
    {
        return Clear();
    }
    ...
    {
        return !Top().IsAuto() || (PositionAreaOffsets() && PositionAreaOffsets()->top);
    }
    bool IsRightInsetNonAuto() const
    {
        return !Right().IsAuto() || (PositionAreaOffsets() && PositionAreaOffsets()->right);
    }
    bool IsBottomInsetNonAuto() const
    {
        return !Bottom().IsAuto() || (PositionAreaOffsets() && PositionAreaOffsets()->bottom);
    }
    bool IsLeftInsetNonAuto() const
    {
        return !Left().IsAuto() || (PositionAreaOffsets() && PositionAreaOffsets()->left);
    }
    ...
    static unsigned EffectiveContainment(unsigned contain, unsigned container_type, EContentVisibility content_visibility, bool skips_contents)
    {
    ...
    {
        return ComputedStyle::EffectiveContainment(Contain(), ContainerType(), ContentVisibility(), SkipsContents());
    }

    bool ContainsStyle() const
    {
        return EffectiveContainment() & kContainsStyle;
    }
    bool ContainsPaint() const
    {
        return EffectiveContainment() & kContainsPaint;
    }
    ...
    CORE_EXPORT static bool ShouldApplyAnyContainment(const Element& element, const DisplayStyle&, unsigned effective_containment);

    CORE_EXPORT bool ShouldApplyAnyContainment(const Element& element) const
    {
        return ShouldApplyAnyContainment(element, GetDisplayStyle(), EffectiveContainment());
    }
    ...
    {
        return DependsOnSizeContainerQueries() || DependsOnStyleContainerQueries() || DependsOnScrollStateContainerQueries();
    }

    static bool IsContentVisibilityVisible(EContentVisibility content_visibility)
    {
    ...
    bool IsDisplayListItem() const
    {
        return IsDisplayListItem(Display());
    }
    static bool IsDisplayListItem(EDisplay display)
    {
        return display == EDisplay::kListItem || display == EDisplay::kInlineListItem || display == EDisplay::kFlowRootListItem
            || display == EDisplay::kInlineFlowRootListItem;
    }
    bool IsDisplayTableBox() const
    {
        return IsDisplayTableBox(Display());
    }
    bool IsDisplayFlexibleBox() const
    {
        return IsDisplayFlexibleBox(Display());
    }
    bool IsDisplayGridBox() const
    {
    ...
    bool IsDisplayTableType() const
    {
        return IsDisplayTableType(Display());
    }

    bool IsDisplayMathType() const
    {
        return IsDisplayMathBox(Display());
    }

    bool BlockifiesChildren() const
    {
        return IsDisplayFlexibleOrGridBox() || IsDisplayMathType() || IsDisplayLayoutCustomBox()
    ...
    bool InlinifiesChildren() const
    {
        ...
        if (display == EDisplay::kRuby || display == EDisplay::kBlockRuby || display == EDisplay::kRubyText) {
            return true;
        }
        ...
        return IsInInlinifyingDisplay() && (display == EDisplay::kContents || display == EDisplay::kInline || display == EDisplay::kInlineListItem);
    }
    ...
    {
        return Isolation() != EIsolation::kAuto;
    }
    ...
    EResize UnresolvedResize() const
    {
        return Resize();
    }
    ...
    EResize UsedResize() const
    {
        ...
        switch (value) {
        case EResize::kBlock:
            return IsHorizontalWritingMode() ? EResize::kVertical : EResize::kHorizontal;
        case EResize::kInline:
            return IsHorizontalWritingMode() ? EResize::kHorizontal : EResize::kVertical;
        default:
            return value;
        }
    }
    ...
    {
        return !IsInert() && !(UserSelect() == EUserSelect::kNone && UserModify() == EUserModify::kReadOnly);
    }
    ...
    bool IsFocusable() const
    {
        ...
        return !IsEnsuredInDisplayNone() && !IsInert() && Visibility() == EVisibility::kVisible
            && (Display() != EDisplay::kContents || RuntimeEnabledFeatures::DisplayContentsFocusableEnabled());
    }
    ...
    bool ShouldTextBoxTrimStart() const
    {
        ...
        return text_box_trim == ETextBoxTrim::kTrimStart || text_box_trim == ETextBoxTrim::kTrimBoth;
    }
    bool ShouldTextBoxTrimEnd() const
    {
        const ETextBoxTrim text_box_trim = TextBoxTrim();
        return text_box_trim == ETextBoxTrim::kTrimEnd || text_box_trim == ETextBoxTrim::kTrimBoth;
    }
    ...
    CORE_EXPORT const Vector<AppliedTextDecoration, 1>& AppliedTextDecorations() const;
    CORE_EXPORT base::RefCountedData<Vector<AppliedTextDecoration, 1>>* AppliedTextDecorationData() const
    {
        return IsDecoratingBox() ? EnsureAppliedTextDecorationsCache() : BaseTextDecorationDataInternal().get();
    }
    ...
    bool IsOverflowVisibleAlongBothAxes() const
    {
        ...
        return OverflowX() == EOverflow::kVisible && OverflowY() == EOverflow::kVisible;
    }

    // Returns true if 'overflow' is 'visible' or 'clip' along both axes.
    bool IsOverflowVisibleOrClip() const
    {
        bool overflow_x = OverflowX() == EOverflow::kVisible || OverflowX() == EOverflow::kClip;
        DCHECK(!overflow_x || OverflowY() == EOverflow::kVisible || OverflowY() == EOverflow::kClip);
        return overflow_x;
    }
    ...
    {
        return OverflowX() != EOverflow::kVisible && OverflowX() != EOverflow::kClip;
    }
    ...
    {
        if (GetObjectFit() == EObjectFit::kNone || GetObjectFit() == EObjectFit::kCover) {
            return false;
        }

        if (ObjectPosition() != LengthPoint(Length::Percent(50.0), Length::Percent(50.0))) {
            return false;
        }
    ...
    {
        return Visibility() == EVisibility::kVisible && UsedPointerEvents() != EPointerEvents::kNone;
    }

    // Animation utility functions.
    bool HasCurrentTransformRelatedAnimation() const
    {
        return HasCurrentTransformAnimation() || HasCurrentScaleAnimation() || HasCurrentRotateAnimation() || HasCurrentTranslateAnimation();
    }
    ...
    {
        return IsRunningOpacityAnimationOnCompositor() || IsRunningTransformAnimationOnCompositor() || IsRunningScaleAnimationOnCompositor()
            || IsRunningRotateAnimationOnCompositor() || IsRunningTranslateAnimationOnCompositor() || IsRunningFilterAnimationOnCompositor()
    ...
    bool IsFixedTableLayout() const
    {
        ...
        return TableLayout() == ETableLayout::kFixed && (!LogicalWidth().HasAuto() && !LogicalWidth().HasMaxContent());
    }
    ...
    LogicalSize TableBorderSpacing() const
    {
        ...
        return LogicalSize(LayoutUnit(HorizontalBorderSpacing()), LayoutUnit(VerticalBorderSpacing()));
    }
    ...
    {
        return Transform().HasNonPerspective3DOperation() || (Translate() && Translate()->Z() != 0) || (Rotate() && (Rotate()->X() != 0 || Rotate()->Y() != 0))
            || (Scale() && Scale()->Z() != 1);
    }
    bool HasTransform() const
    {
        return HasTransformOperations() || HasOffset() || HasCurrentTransformRelatedAnimation() || Translate() || Rotate() || Scale();
    }
    ...
    ETransformStyle3D UsedTransformStyle3D() const
    {
        ...
        return HasGroupingPropertyForUsedTransformStyle3D() ? ETransformStyle3D::kFlat : ETransformStyle3D::kPreserve3d;
    }
    bool Preserves3D() const
    {
        return UsedTransformStyle3D() != ETransformStyle3D::kFlat;
    }
    enum ApplyTransformOrigin { kIncludeTransformOrigin, kExcludeTransformOrigin };
    enum ApplyMotionPath { kIncludeMotionPath, kExcludeMotionPath };
    enum ApplyIndependentTransformProperties { kIncludeIndependentTransformProperties, kExcludeIndependentTransformProperties };
    enum ApplyTransformOperations { kIncludeTransformOperations, kExcludeTransformOperations };
    void ApplyTransform(gfx::Transform&, const LayoutBox* box, const PhysicalRect& reference_box, ApplyTransformOperations, ApplyTransformOrigin,
        ApplyMotionPath, ApplyIndependentTransformProperties) const;
    void ApplyTransform(gfx::Transform&, const LayoutBox* box, const gfx::RectF& reference_box, ApplyTransformOperations, ApplyTransformOrigin, ApplyMotionPath,
    ...
    {
        return HasGroupingProperty(BoxReflect()) || !IsOverflowVisibleAlongBothAxes();
    }
    ...
    {
        return HasTransform() || Preserves3D() || HasPerspective() || HasWillChangeHintForAnyTransformProperty();
    }
    ...
    bool HasCSSPaintImagesUsingCustomProperty(const AtomicString& custom_property_name, const Document&) const;
    ...
    {
        return BoxShadow() || HasBorderImageOutsets() || HasOutline() || HasMaskBoxImageOutsets();
    }

    bool IsStackedWithoutContainment() const
    {
        return IsStackingContextWithoutContainment() || GetPosition() != EPosition::kStatic;
    }
    ...
    {
        return ComputedStyle::HasPseudoElementStyle(PseudoElementStylesInternal(), pseudo);
    }
    ...
    {
        return (ShouldPreserveWhiteSpaces() && ShouldWrapLine()) || GetLineBreak() == LineBreak::kAfterWhiteSpace;
    }
    ...
    void GetBorderEdgeInfo(BorderEdgeArray& edges, PhysicalBoxSides sides_to_include = PhysicalBoxSides()) const;

    bool HasBoxDecorations() const
    {
        return HasBorderDecoration() || HasBorderRadius() || HasOutline() || HasEffectiveAppearance() || BoxShadow() || HasFilterInducingProperty()
            || HasNonInitialBackdropFilter() || HasResize();
    }
    ...
    {
        return HasBackground() || HasBorderDecoration() || HasEffectiveAppearance() || BoxShadow();
    }
    ...
    {
        if (BackgroundColor().IsCurrentColor() || InternalVisitedBackgroundColor().IsCurrentColor() || InternalForcedBackgroundColor().IsCurrentColor()) {
            return true;
        }
        if (!BoxShadow()) {
            return false;
    ...
    CORE_EXPORT blink::Color VisitedDependentColor(const Longhand& color_property, bool* is_current_color = nullptr) const;

    // Used to resolve 'context-fill' and 'context-stroke' paints
    CORE_EXPORT blink::Color VisitedDependentContextFill(const SVGPaint& context_paint, const ComputedStyle& context_style) const;
    CORE_EXPORT blink::Color VisitedDependentContextStroke(const SVGPaint& context_paint, const ComputedStyle& context_style) const;
    ...
    template <class Property> blink::Color VisitedDependentColorFast(const Property& color_property, bool* is_current_color = nullptr) const
    {
        DCHECK(!Property().IsVisited());

        if (InsideLink() != EInsideLink::kInsideVisitedLink) {
            blink::Color color = color_property.ColorIncludingFallback(false, *this,
    ...
    bool RequireTransformOrigin(ApplyTransformOrigin apply_origin, ApplyMotionPath) const;
    ...
    bool CanGeneratePseudoElement(PseudoId pseudo) const
    {
        ...
        if (pseudo == kPseudoIdScrollMarkerGroupBefore) {
            return ScrollMarkerGroup() == EScrollMarkerGroup::kBefore && IsScrollContainer() && HasPseudoElementStyle(kPseudoIdScrollMarkerGroup);
        }
        if (pseudo == kPseudoIdScrollMarkerGroupAfter) {
            return ScrollMarkerGroup() == EScrollMarkerGroup::kAfter && IsScrollContainer() && HasPseudoElementStyle(kPseudoIdScrollMarkerGroup);
        }
        ...
        return pseudo == kPseudoIdCheck || pseudo == kPseudoIdBefore || pseudo == kPseudoIdAfter || pseudo == kPseudoIdSelectArrow;
    }
    ...
    {
        return { LayoutUnit(ScrollMarginTop()), LayoutUnit(ScrollMarginRight()), LayoutUnit(ScrollMarginBottom()), LayoutUnit(ScrollMarginLeft()) };
    }
    ...
    {
        return is_dark_color_scheme ? mojom::blink::ColorScheme::kDark : mojom::blink::ColorScheme::kLight;
    }
    ...
    {
        return IsDisplayListItem() && ListStyleImage() && !ListStyleImage()->ErrorOccurred() && (ListStyleImage()->IsLoading() || ListStyleImage()->IsLoaded());
    }
    ...
    {
        return ViewportUnitFlags() & static_cast<unsigned>(ViewportUnitFlag::kStatic);
    }
    bool HasDynamicViewportUnits() const
    {
        return ViewportUnitFlags() & static_cast<unsigned>(ViewportUnitFlag::kDynamic);
    }
    ...
    bool OverflowClipMarginHasAnEffect() const
    {
        return OverflowClipMargin()
            && (OverflowClipMargin()->GetReferenceBox() != StyleOverflowClipMargin::ReferenceBox::kPaddingBox
    ...
    {
        return (static_cast<int>(GetPositionVisibility()) & static_cast<int>(visibility)) == static_cast<int>(visibility);
    }
...
private:
    ...
    {
        return display == EDisplay::kBlock || display == EDisplay::kListItem || display == EDisplay::kInlineBlock || display == EDisplay::kFlowRoot
            || display == EDisplay::kFlowRootListItem || display == EDisplay::kInlineFlowRootListItem || display == EDisplay::kTableCell
    ...
    {
        return display == EDisplay::kLayoutCustom || display == EDisplay::kInlineLayoutCustom;
    }

    static bool IsDisplayReplacedType(EDisplay display)
    {
        return display == EDisplay::kInlineBlock || display == EDisplay::kInlineFlowRootListItem || display == EDisplay::kWebkitInlineBox
            || display == EDisplay::kInlineFlex || display == EDisplay::kInlineTable || display == EDisplay::kInlineGrid || display == EDisplay::kMath
            || display == EDisplay::kInlineLayoutCustom;
    }

    static bool IsDisplayInlineType(EDisplay display)
    {
        return display == EDisplay::kInline || display == EDisplay::kInlineListItem || display == EDisplay::kRuby || IsDisplayReplacedType(display);
    }

    static bool IsDisplayTableType(EDisplay display)
    {
        return display == EDisplay::kTable || display == EDisplay::kInlineTable || display == EDisplay::kTableRowGroup || display == EDisplay::kTableHeaderGroup
            || display == EDisplay::kTableFooterGroup || display == EDisplay::kTableRow || display == EDisplay::kTableColumnGroup
            || display == EDisplay::kTableColumn || display == EDisplay::kTableCell || display == EDisplay::kTableCaption;
    }

    bool BorderOutlineVisitedColorChanged(const ComputedStyle& other) const
    {
        // Only invalidate if the border/outline is present.
        if (BorderTopWidth() && InternalVisitedBorderTopColor() != other.InternalVisitedBorderTopColor()) {
            return true;
        }
        if (BorderRightWidth() && InternalVisitedBorderRightColor() != other.InternalVisitedBorderRightColor()) {
            return true;
        }
        if (BorderBottomWidth() && InternalVisitedBorderBottomColor() != other.InternalVisitedBorderBottomColor()) {
            return true;
        }
        if (BorderLeftWidth() && InternalVisitedBorderLeftColor() != other.InternalVisitedBorderLeftColor()) {
            return true;
        }
        if (OutlineWidth() && InternalVisitedOutlineColor() != other.InternalVisitedOutlineColor()) {
            return true;
        }
        return false;
    }
    ...
    {
        return Appearance() != kNoControlPart;
    }

    void ApplyMotionPathTransform(float origin_x, float origin_y, const LayoutBox* box, const gfx::RectF& bounding_box, gfx::Transform&) const;
    ...
    bool DiffNeedsFullLayoutAndPaintInvalidation(const ComputedStyle& other, uint64_t field_diff) const;
    bool DiffNeedsFullLayout(const Document&, const ComputedStyle& other, uint64_t field_diff) const;
    bool DiffNeedsFullLayoutForLayoutCustom(const Document&, const ComputedStyle& other) const;
    bool DiffNeedsFullLayoutForLayoutCustomChild(const Document&, const ComputedStyle& other) const;
    bool DiffNeedsNormalPaintInvalidation(const Document&, const ComputedStyle& other, uint64_t field_diff) const;
    bool DiffNeedsPaintInvalidationForPaintImage(const StyleImage&, const ComputedStyle& other, const Document&) const;
    bool DiffNeedsRecomputeVisualOverflow(const ComputedStyle& other, uint64_t field_diff) const;
    bool DiffCompositingReasonsChanged(const ComputedStyle& other, uint64_t field_diff) const;
    bool PotentialCompositingReasonsFor3DTransformChanged(const ComputedStyle& other) const;

    bool PropertiesEqual(const Vector<CSSPropertyID>& properties, const ComputedStyle& other) const;
    CORE_EXPORT bool CustomPropertiesEqual(const Vector<AtomicString>& properties, const ComputedStyle& other) const;

    blink::Color GetCurrentColor(bool* is_current_color = nullptr) const;
    blink::Color GetInternalVisitedCurrentColor(bool* is_current_color = nullptr) const;
    blink::Color GetInternalForcedCurrentColor(bool* is_current_color = nullptr) const;
    blink::Color GetInternalForcedVisitedCurrentColor(bool* is_current_color = nullptr) const;

    blink::Color VisitedDependentContextPaint(const SVGPaint& context_paint, const SVGPaint& context_visited_paint) const;
    ...
    blink::Color ResolvedColor(const StyleColor& color, bool* is_current_color = nullptr) const;

    static bool ShadowListHasCurrentColor(const ShadowList*);

    PhysicalToLogical<const Length&> PhysicalMarginToLogical(const ComputedStyle& other) const
    {
        return PhysicalToLogical<const Length&>(other.GetWritingDirection(), MarginTop(), MarginRight(), MarginBottom(), MarginLeft());
    }

    PhysicalToLogical<const Length&> PhysicalPaddingToLogical() const
    {
        return PhysicalToLogical<const Length&>(GetWritingDirection(), PaddingTop(), PaddingRight(), PaddingBottom(), PaddingLeft());
    }

    PhysicalToLogical<int> PhysicalBorderWidthToLogical() const
    {
        return PhysicalToLogical<int>(GetWritingDirection(), BorderTopWidth(), BorderRightWidth(), BorderBottomWidth(), BorderLeftWidth());
    }

    PhysicalToLogical<EBorderStyle> PhysicalBorderStyleToLogical() const
    {
        return PhysicalToLogical<EBorderStyle>(GetWritingDirection(), BorderTopStyle(), BorderRightStyle(), BorderBottomStyle(), BorderLeftStyle());
    }

    PhysicalToLogical<const Length&> PhysicalBoundsToLogical() const
    {
        return PhysicalToLogical<const Length&>(GetWritingDirection(), Top(), Right(), Bottom(), Left());
    }

    static Difference ComputeDifferenceIgnoringInheritedFirstLineStyle(const ComputedStyle& old_style, const ComputedStyle& new_style);

    static bool ShouldForceColor(bool in_forced_colors_mode, EForcedColorAdjust forced_color_adjust, const StyleColor& unforced_color)
    {
        return in_forced_colors_mode && forced_color_adjust == EForcedColorAdjust::kAuto && !unforced_color.IsSystemColorIncludingDeprecated();
    }
    bool ShouldForceColor(const StyleColor& unforced_color) const
    {
        ...
        return ShouldForceColor(InForcedColorsMode(), ForcedColorAdjust(), unforced_color);
    }
    ...
    FRIEND_TEST_ALL_PREFIXES(PermissionShadowElementTest, PropagateCSSPropertyInnerElement);
};
...
{
    static_assert(kPseudoIdSelection >= kFirstPublicPseudoId && kPseudoIdSelection <= kLastTrackedPublicPseudoId, "kPseudoIdSelection must be public");
    static_assert(kPseudoIdSearchText >= kFirstPublicPseudoId && kPseudoIdSearchText <= kLastTrackedPublicPseudoId, "kPseudoIdSearchText must be public");
    static_assert(kPseudoIdTargetText >= kFirstPublicPseudoId && kPseudoIdTargetText <= kLastTrackedPublicPseudoId, "kPseudoIdTargetText must be public");
    static_assert(
        kPseudoIdSpellingError >= kFirstPublicPseudoId && kPseudoIdSpellingError <= kLastTrackedPublicPseudoId, "kPseudoIdSpellingError must be public");
    static_assert(kPseudoIdGrammarError >= kFirstPublicPseudoId && kPseudoIdGrammarError <= kLastTrackedPublicPseudoId, "kPseudoIdGrammarError must be public");
    static_assert(kPseudoIdHighlight >= kFirstPublicPseudoId && kPseudoIdHighlight <= kLastTrackedPublicPseudoId, "kPseudoIdHighlight must be public");

    const unsigned mask = (1 << (kPseudoIdSelection - kFirstPublicPseudoId)) | (1 << (kPseudoIdSearchText - kFirstPublicPseudoId))
        | (1 << (kPseudoIdTargetText - kFirstPublicPseudoId)) | (1 << (kPseudoIdSpellingError - kFirstPublicPseudoId))
        | (1 << (kPseudoIdGrammarError - kFirstPublicPseudoId)) | (1 << (kPseudoIdHighlight - kFirstPublicPseudoId));
...
public:
    ...
    CORE_EXPORT ComputedStyleBuilder(
        const ComputedStyle& initial_style, const ComputedStyle& parent_style, IsAtShadowBoundary is_at_shadow_boundary = kNotAtShadowBoundary);
    ...
    void PropagateIndependentInheritedProperties(const ComputedStyle& parent_style);
    ...
    {
        return MutableBackgroundInternal();
    }
    ...
    int BorderTopWidth() const
    {
        return ComputedStyle::BorderWidth(BorderTopStyle(), BorderTopWidthInternal());
    }
    int BorderBottomWidth() const
    {
        return ComputedStyle::BorderWidth(BorderBottomStyle(), BorderBottomWidthInternal());
    }
    int BorderLeftWidth() const
    {
        return ComputedStyle::BorderWidth(BorderLeftStyle(), BorderLeftWidthInternal());
    }
    int BorderRightWidth() const
    {
        return ComputedStyle::BorderWidth(BorderRightStyle(), BorderRightWidthInternal());
    }
    ...
    ClipPathOperation* MutableClipPath()
    {
        return ClipPathInternal().Get();
    }
    ...
    {
        unsigned effective_containment = ComputedStyle::EffectiveContainment(Contain(), ContainerType(), ContentVisibility(), SkipsContents());
        return ComputedStyle::ShouldApplyAnyContainment(element, GetDisplayStyle(), effective_containment);
    }
    ...
    {
        std::unique_ptr<CounterDirectiveMap>& map = MutableCounterDirectivesInternal();
        if (!map) {
    ...
    // cursor
    void AddCursor(StyleImage* image, bool hot_spot_specified, const gfx::Point& hot_spot = gfx::Point())
    {
        if (!CursorDataInternal()) {
            SetCursorDataInternal(MakeGarbageCollected<CursorList>());
        }
        MutableCursorDataInternal()->push_back(CursorData(image, hot_spot_specified, hot_spot));
    }
    void SetCursorList(CursorList* list)
    {
        SetCursorDataInternal(list);
    }
    ...
    {
        return Display() == EDisplay::kTableRow || Display() == EDisplay::kTableRowGroup || Display() == EDisplay::kTableColumn
    ...
    {
        return LineHeightInternal() == ComputedStyleInitialValues::InitialLineHeight();
    }
    ...
    // orphans
    void SetOrphans(int16_t o)
    {
        SetOrphansInternal(ClampTo<int16_t>(o, 1));
    }

    // overflow
    bool ScrollsOverflow() const
    {
        return ComputedStyle::ScrollsOverflow(OverflowX()) || ComputedStyle::ScrollsOverflow(OverflowY());
    }
    ...
    {
        float clamped_shape_image_threshold = ClampTo<float>(shape_image_threshold, 0, 1);
    ...
    {
        SetTransformOrigin(TransformOrigin(v, GetTransformOrigin().Y(), GetTransformOrigin().Z()));
    }
    void SetTransformOriginY(const Length& v)
    {
        SetTransformOrigin(TransformOrigin(GetTransformOrigin().X(), v, GetTransformOrigin().Z()));
    }
    void SetTransformOriginZ(float f)
    {
        SetTransformOrigin(TransformOrigin(GetTransformOrigin().X(), GetTransformOrigin().Y(), f));
    }
    ...
    {
        std::unique_ptr<CSSTransitionData>& transitions = MutableTransitionsInternal();
        if (!transitions) {
    ...
    {
        SetBoxOrdinalGroupInternal(std::min(std::numeric_limits<unsigned>::max() - 1, ordinal_group));
    }
    ...
    // widows
    void SetWidows(int16_t w)
    {
        SetWidowsInternal(ClampTo<int16_t>(w, 1));
    }
    ...
    void AddDocumentRulesSelector(StyleRule* selector)
    {
        if (!DocumentRulesSelectors()) {
            MutableDocumentRulesSelectorsInternal() = MakeGarbageCollected<HeapHashSet<WeakMember<StyleRule>>>();
        }
    ...
    void SetCustomHighlightNames(const HashSet<AtomicString>& custom_highlight_names)
    {
        SetCustomHighlightNamesInternal(std::make_unique<HashSet<AtomicString>>(custom_highlight_names));
    }
    ...
    {
        return InForcedColorsMode() && ForcedColorAdjust() == EForcedColorAdjust::kPreserveParentColor;
    }
    bool ShouldForceColor(const StyleColor& unforced_color) const
    {
        return ComputedStyle::ShouldForceColor(InForcedColorsMode(), ForcedColorAdjust(), unforced_color);
    }
    ...
    void SetUsedColorScheme(ColorSchemeFlags flags, mojom::blink::PreferredColorScheme preferred_color_scheme, bool force_dark);
    ...
    CORE_EXPORT void SetVariableData(const AtomicString& name, CSSVariableData* value, bool is_inherited_property)
    {
        if (is_inherited_property) {
            ...
            if (!has_own_inherited_variables_ && InheritedVariables()) {
                if (auto existing_value = InheritedVariables()->GetData(name); existing_value && base::ValuesEquivalent(*existing_value, value)) {
                    return;
                }
            }
    ...
    CORE_EXPORT void SetVariableValue(const AtomicString& name, const CSSValue* value, bool is_inherited_property)
    {
        if (is_inherited_property) {
    ...
    {
        SetViewportUnitFlags(ViewportUnitFlags() | static_cast<unsigned>(ViewportUnitFlag::kStatic));
    }
    void SetHasDynamicViewportUnits()
    {
        SetViewportUnitFlags(ViewportUnitFlags() | static_cast<unsigned>(ViewportUnitFlag::kDynamic));
    }
