...
BorderEdge::BorderEdge(int edge_width, const Color& edge_color, EBorderStyle edge_style, bool edge_is_present)
    : color_(edge_color)
...
{
    if ((style == EBorderStyle::kDouble && width < 3) || ((style == EBorderStyle::kRidge || style == EBorderStyle::kGroove) && width <= 1)) {
...
bool BorderEdge::ObscuresBackground() const
{
    ...
    if (style_ == EBorderStyle::kDotted || style_ == EBorderStyle::kDashed || style_ == EBorderStyle::kDouble) {
        return false;
    }
...
{
    DCHECK(stripe == kDoubleBorderStripeOuter || stripe == kDoubleBorderStripeInner);

    return roundf(stripe == kDoubleBorderStripeOuter ? UsedWidth() / 3.0f : (UsedWidth() * 2.0f) / 3.0f);
}
