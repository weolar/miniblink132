...
public:
    ...
    public:
        ...
        static TimelineAxis DefaultAxis()
        {
        ...
        TimelineAxis GetAxis() const
        {
            return axis_;
        }
        const TimelineScroller& GetScroller() const
        {
            return scroller_;
        }

        bool HasDefaultAxis() const
        {
            return axis_ == DefaultAxis();
        }
        bool HasDefaultScroller() const
        {
            return scroller_ == DefaultScroller();
        }
    ...
    explicit StyleTimeline(CSSValueID keyword)
        : data_(keyword)
    {
    }
    ...
    explicit StyleTimeline(const ScrollData& scroll_data)
        : data_(scroll_data)
    {
    }
    explicit StyleTimeline(const ViewData& view_data)
        : data_(view_data)
    {
    }
    ...
    {
        return absl::holds_alternative<CSSValueID>(data_);
    }
    bool IsName() const
    {
    ...
    bool IsScroll() const
    {
        return absl::holds_alternative<ScrollData>(data_);
    }
    bool IsView() const
    {
        return absl::holds_alternative<ViewData>(data_);
    }

    const CSSValueID& GetKeyword() const
    {
        return absl::get<CSSValueID>(data_);
    }
    const ScopedCSSName& GetName() const
    {
        return *absl::get<Persistent<const ScopedCSSName>>(data_);
    }
    const ScrollData& GetScroll() const
    {
        return absl::get<ScrollData>(data_);
    }
    const ViewData& GetView() const
    {
        return absl::get<ViewData>(data_);
    }
