...
class StyleViewTransitionName : public GarbageCollected<StyleViewTransitionName> {
public:
    enum class Type { kAuto, kCustom };

    bool IsAuto() const
    {
        return type_ == Type::kAuto;
    }
    bool IsCustom() const
    {
        return type_ == Type::kCustom;
    }
    ...
    static StyleViewTransitionName* Create(const AtomicString& name, const TreeScope* tree_scope)
    {
        CHECK(name);
        CHECK_NE(name, "none");
        CHECK_NE(name, "auto");
    ...
    {
        return type_ == other.type_ && custom_name_ == other.custom_name_ && tree_scope_ == other.tree_scope_;
    }
    ...
    StyleViewTransitionName(const AtomicString& custom_name, const TreeScope* tree_scope)
        : type_(Type::kCustom)
