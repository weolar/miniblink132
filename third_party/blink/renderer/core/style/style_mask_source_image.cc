...
StyleMaskSourceImage::StyleMaskSourceImage(StyleFetchedImage* image, SVGResource* resource, CSSImageValue* resource_css_value)
    : image_(image)
    , resource_(resource)
...
StyleMaskSourceImage::StyleMaskSourceImage(SVGResource* resource, CSSImageValue* resource_css_value)
...
CSSValue* StyleMaskSourceImage::ComputedCSSValue(const ComputedStyle& style, bool allow_visited_style, CSSValuePhase value_phase) const
{
...
IntrinsicSizingInfo StyleMaskSourceImage::GetNaturalSizingInfo(float multiplier, RespectImageOrientationEnum respect_orientation) const
{
    if (!image_) {
...
gfx::SizeF StyleMaskSourceImage::ImageSize(float multiplier, const gfx::SizeF& default_object_size, RespectImageOrientationEnum respect_orientation) const
{
    if (!image_) {
        return gfx::SizeF();
    }
    return image_->ImageSize(multiplier, default_object_size, respect_orientation);
}
...
SVGResourceClient* StyleMaskSourceImage::GetSVGResourceClient(const ImageResourceObserver& observer) const
{
    return resource_ ? resource_->GetObserverResourceClient(const_cast<ImageResourceObserver&>(observer)) : nullptr;
}
...
bool StyleMaskSourceImage::KnownToBeOpaque(const Document& document, const ComputedStyle& style) const
{
...
bool StyleMaskSourceImage::HasSVGMask() const
{
    ...
    CHECK(resource_);
    LayoutSVGResourceContainer* container = resource_->ResourceContainerNoCycleCheck();
...
bool StyleMaskSourceImage::IsEqual(const StyleImage& other) const
{
    ...
    return other_mask_ref && base::ValuesEquivalent(image_, other_mask_ref->image_) && resource_ == other_mask_ref->resource_;
}
