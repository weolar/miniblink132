...
scoped_refptr<StylePath> StylePath::Create(SVGPathByteStream path_byte_stream, WindRule wind_rule)
{
...
{
    DEFINE_STATIC_REF(StylePath, empty_path, StylePath::Create(SVGPathByteStream()));
    return empty_path;
}
...
{
    return MakeGarbageCollected<cssvalue::CSSPathValue>(const_cast<StylePath*>(this), kTransformToAbsolute);
}
...
void StylePath::GetPath(Path& path, const gfx::RectF& offset_rect, float zoom) const
{
    path = GetPath();
    path.Transform(AffineTransform::Translation(offset_rect.x(), offset_rect.y()).Scale(zoom));
}
