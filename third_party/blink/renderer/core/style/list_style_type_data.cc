...
// static
ListStyleTypeData* ListStyleTypeData::CreateCounterStyle(const AtomicString& name, const TreeScope* tree_scope)
{
    return MakeGarbageCollected<ListStyleTypeData>(Type::kCounterStyle, name, tree_scope);
}
...
bool ListStyleTypeData::IsCounterStyleReferenceValid(Document& document) const
{
    ...
    return counter_style_ == &document.GetStyleEngine().FindCounterStyleAcrossScopes(GetCounterStyleName(), GetTreeScope());
}

const CounterStyle& ListStyleTypeData::GetCounterStyle(Document& document) const
{
    DCHECK(IsCounterStyle());
    if (!IsCounterStyleReferenceValid(document)) {
        counter_style_ = document.GetStyleEngine().FindCounterStyleAcrossScopes(GetCounterStyleName(), GetTreeScope());
    }
