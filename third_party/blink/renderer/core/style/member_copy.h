...
template <typename T> scoped_refptr<T> MemberCopy(const scoped_refptr<T>& v)
{
    return v;
}

template <typename T> Member<T> MemberCopy(const Member<T>& v)
{
    return v;
}

template <typename T> std::unique_ptr<T> MemberCopy(const std::unique_ptr<T>& v)
{
