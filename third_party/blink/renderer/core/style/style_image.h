...
public:
    ...
    // specification.
    virtual CSSValue* ComputedCSSValue(const ComputedStyle&, bool allow_visited_style, CSSValuePhase value_phase) const = 0;
    ...
    // supports it.
    virtual IntrinsicSizingInfo GetNaturalSizingInfo(float multiplier, RespectImageOrientationEnum) const = 0;
    ...
    // The size will respect the image orientation if requested and if the image
    // supports it.
    virtual gfx::SizeF ImageSize(float multiplier, const gfx::SizeF& default_object_size, RespectImageOrientationEnum) const = 0;
    ...
    virtual scoped_refptr<Image> GetImage(const ImageResourceObserver&, const Document&, const ComputedStyle&, const gfx::SizeF& target_size) const = 0;
    ...
    virtual ImageResourceContent* CachedImage() const
    {
        return nullptr;
    }
    ...
    // content.
    virtual RespectImageOrientationEnum ForceOrientationIfNecessary(RespectImageOrientationEnum default_orientation) const
    {
        return default_orientation;
    }

    ALWAYS_INLINE bool IsImageResource() const
    {
        return is_image_resource_;
    }
    ALWAYS_INLINE bool IsPendingImage() const
    {
        return is_pending_image_;
    }
    ALWAYS_INLINE bool IsGeneratedImage() const
    {
        return is_generated_image_;
    }
    ALWAYS_INLINE bool IsContentful() const
    {
        return !is_generated_image_;
    }
    ...
    ALWAYS_INLINE bool IsMaskSource() const
    {
        return is_mask_source_;
    }
    ALWAYS_INLINE bool IsPaintImage() const
    {
        return is_paint_image_;
    }
    ALWAYS_INLINE bool IsCrossfadeImage() const
    {
        return is_crossfade_;
    }
    ...
    virtual bool IsLoadedAfterMouseover() const
    {
        return false;
    }

    virtual bool IsOriginClean() const
    {
        return true;
    }
