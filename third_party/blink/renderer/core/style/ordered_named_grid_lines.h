...
struct NamedGridLine {
    explicit NamedGridLine(const AtomicString& line_name, bool is_in_repeat = false, bool is_first_repeat = false)
        : line_name(line_name)
    ...
    {
        return line_name == other.line_name && is_in_repeat == other.is_in_repeat && is_first_repeat == other.is_first_repeat;
    }
...
using OrderedNamedGridLines = HashMap<size_t, Vector<NamedGridLine>, IntWithZeroKeyHashTraits<size_t>>;
