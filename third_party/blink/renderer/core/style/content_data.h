...
private:
    ...
    friend std::ostream& operator<<(std::ostream& stream, const ContentData& content_data);
};

inline std::ostream& operator<<(std::ostream& stream, const ContentData& content_data)
{
...
public:
    explicit ImageContentData(StyleImage* image)
        : image_(image)
    {
        DCHECK(image_);
    }
...
template <> struct DowncastTraits<ImageContentData> {
...
template <> struct DowncastTraits<TextContentData> {
    static bool AllowFrom(const ContentData& content)
    {
...
public:
    explicit AltTextContentData(const String& text)
        : text_(text)
    {
    }
    ...
    String ConcatenateAltText() const
    {
        ...
        for (const ContentData* content_data = this; content_data; content_data = content_data->Next()) {
...
template <> struct DowncastTraits<AltTextContentData> {
    static bool AllowFrom(const ContentData& content)
    {
...
public:
    explicit CounterContentData(const AtomicString& identifier, const AtomicString& style, const AtomicString& separator, const TreeScope* tree_scope)
    ...
    const AtomicString& ListStyle() const
    {
        return list_style_;
    }
...
private:
    ContentData* CloneInternal() const override
    {
        return MakeGarbageCollected<CounterContentData>(identifier_, list_style_, separator_, tree_scope_);
    }
    ...
    bool Equals(const ContentData& data) const override
    {
        ...
        const CounterContentData& other = static_cast<const CounterContentData&>(data);
...
template <> struct DowncastTraits<CounterContentData> {
    static bool AllowFrom(const ContentData& content)
    {
...
template <> struct DowncastTraits<QuoteContentData> {
    static bool AllowFrom(const ContentData& content)
    {
...
template <> struct DowncastTraits<NoneContentData> {
    static bool AllowFrom(const ContentData& content)
    {
        return content.IsNone();
    }
};
