...
template <typename Enum> inline bool EnumHasFlags(Enum v, Enum mask)
{
...
inline bool UsesHighlightPseudoInheritance(PseudoId pseudo_id)
{
    ...
    return ((IsHighlightPseudoElement(pseudo_id) && RuntimeEnabledFeatures::HighlightInheritanceEnabled()) || pseudo_id == PseudoId::kPseudoIdSearchText
        || pseudo_id == PseudoId::kPseudoIdHighlight || pseudo_id == PseudoId::kPseudoIdSpellingError || pseudo_id == PseudoId::kPseudoIdGrammarError);
}
...
enum class EVerticalAlign : unsigned { kBaseline, kMiddle, kSub, kSuper, kTextTop, kTextBottom, kTop, kBottom, kBaselineMiddle, kLength };
...
inline EFillBox EnclosingFillBox(EFillBox box_a, EFillBox box_b)
{
    ...
    if (box_a == EFillBox::kBorder || box_a == EFillBox::kText || box_b == EFillBox::kBorder || box_b == EFillBox::kText) {
...
enum class ContentPosition : unsigned { kNormal, kBaseline, kLastBaseline, kCenter, kStart, kEnd, kFlexStart, kFlexEnd, kLeft, kRight };
