...
StyleCrossfadeImage::StyleCrossfadeImage(cssvalue::CSSCrossfadeValue& value, HeapVector<Member<StyleImage>> images)
...
CSSValue* StyleCrossfadeImage::ComputedCSSValue(const ComputedStyle& style, bool allow_visited_style, CSSValuePhase value_phase) const
{
    ...
    for (unsigned i = 0; i < images_.size(); ++i) {
        CSSValue* value
            = images_[i] ? images_[i]->ComputedCSSValue(style, allow_visited_style, value_phase) : original_value_->GetImagesAndPercentages()[i].first.Get();
        CSSPrimitiveValue* percentage = original_value_->GetImagesAndPercentages()[i].second;
        if (percentage && !percentage->IsNumericLiteralValue()) {
            ...
            percentage = CSSNumericLiteralValue::Create(val, CSSPrimitiveValue::UnitType::kPercentage);
        }
        image_and_percentages.emplace_back(value, percentage);
    }
    return MakeGarbageCollected<cssvalue::CSSCrossfadeValue>(original_value_->IsPrefixedVariant(), std::move(image_and_percentages));
}

bool StyleCrossfadeImage::CanRender() const
{
    return std::all_of(images_.begin(), images_.end(), [](StyleImage* image) { return !image || image->CanRender(); });
}

bool StyleCrossfadeImage::IsLoading() const
{
    return std::any_of(images_.begin(), images_.end(), [](StyleImage* image) { return image && image->IsLoading(); });
}

bool StyleCrossfadeImage::IsLoaded() const
{
    return std::all_of(images_.begin(), images_.end(), [](StyleImage* image) { return !image || image->IsLoaded(); });
}

bool StyleCrossfadeImage::ErrorOccurred() const
{
    return std::any_of(images_.begin(), images_.end(), [](StyleImage* image) { return image && image->ErrorOccurred(); });
}

bool StyleCrossfadeImage::IsAccessAllowed(String& failing_url) const
{
    return std::all_of(images_.begin(), images_.end(), [&](StyleImage* image) { return !image || image->IsAccessAllowed(failing_url); });
}

bool StyleCrossfadeImage::AnyImageIsNone() const
{
    return std::any_of(images_.begin(), images_.end(), [](StyleImage* image) { return !image; });
}
...
IntrinsicSizingInfo StyleCrossfadeImage::GetNaturalSizingInfo(float multiplier, RespectImageOrientationEnum respect_orientation) const
{
    if (AnyImageIsNone()) {
    ...
    for (StyleImage* image : images_) {
        if (ParticipatesInSizing(*image)) {
            sizing_info.push_back(image->GetNaturalSizingInfo(multiplier, kRespectImageOrientation));
        }
    }
    ...
    const bool all_equal
        = std::ranges::all_of(base::span(sizing_info).subspan(1u), [first_sizing_info { sizing_info[0] }](const IntrinsicSizingInfo& sizing_info) {
              return sizing_info.size == first_sizing_info.size && sizing_info.aspect_ratio == first_sizing_info.aspect_ratio
                  && sizing_info.has_width == first_sizing_info.has_width && sizing_info.has_height == first_sizing_info.has_height;
          });
    if (all_equal) {
        return sizing_info[0];
    }
    ...
    for (unsigned i = 0; i < sizing_info.size(); ++i) {
        result_sizing_info.size += gfx::SizeF(sizing_info[i].size.width() * weights[i], sizing_info[i].size.height() * weights[i]);
...
gfx::SizeF StyleCrossfadeImage::ImageSize(float multiplier, const gfx::SizeF& default_object_size, RespectImageOrientationEnum) const
{
    if (AnyImageIsNone()) {
        return gfx::SizeF();
    }
    ...
    for (StyleImage* image : images_) {
        if (ParticipatesInSizing(*image)) {
            image_sizes.push_back(image->ImageSize(multiplier, default_object_size, kRespectImageOrientation));
        }
    }
    ...
    const bool all_equal = std::ranges::all_of(
        base::span(image_sizes).subspan(1u), [first_image_size { image_sizes[0] }](const gfx::SizeF& image_size) { return image_size == first_image_size; });
    if (all_equal) {
        return image_sizes[0];
    }
    ...
    for (unsigned i = 0; i < image_sizes.size(); ++i) {
        size += gfx::SizeF(image_sizes[i].width() * weights[i], image_sizes[i].height() * weights[i]);
    }
    return size;
}
...
{
    return std::any_of(images_.begin(), images_.end(), [](StyleImage* image) { return image && image->HasIntrinsicSize(); });
}
...
scoped_refptr<Image> StyleCrossfadeImage::GetImage(
    const ImageResourceObserver& observer, const Document& document, const ComputedStyle& style, const gfx::SizeF& target_size) const
{
    ...
    const gfx::SizeF resolved_size = ImageSize(style.EffectiveZoom(), target_size, kRespectImageOrientation);
    const ImageResourceObserver* proxy_observer = original_value_->GetObserverProxy();
    ...
    for (unsigned i = 0; i < images_.size(); ++i) {
        scoped_refptr<Image> image = images_[i]->GetImage(*proxy_observer, document, style, target_size);
        images.push_back(CrossfadeGeneratedImage::WeightedImage { std::move(image), weights[i] });
    }
...
bool StyleCrossfadeImage::KnownToBeOpaque(const Document& document, const ComputedStyle& style) const
{
    return std::all_of(images_.begin(), images_.end(), [&](StyleImage* image) { return image && image->KnownToBeOpaque(document, style); });
}
...
std::vector<float> StyleCrossfadeImage::ComputeWeights(bool for_sizing) const
{
    ...
    for (const auto& [image, percentage] : original_value_->GetImagesAndPercentages()) {
