...
inline PhysicalAxes PhysicalAxisFromRegion(
    PositionAreaRegion region, const WritingDirectionMode& container_writing_direction, const WritingDirectionMode& self_writing_direction)
{
    switch (region) {
...
inline PhysicalAxes PhysicalAxisFromSpan(PositionAreaRegion start, PositionAreaRegion end, const WritingDirectionMode& container_writing_direction,
...
PositionAreaRegion ToPhysicalRegion(
    PositionAreaRegion region, PhysicalAxes axis, const WritingDirectionMode& container_writing_direction, const WritingDirectionMode& self_writing_direction)
{
    ...
    case PositionAreaRegion::kInlineStart:
    case PositionAreaRegion::kBlockStart:
        axis_region = is_horizontal ? PositionAreaRegion::kXStart : PositionAreaRegion::kYStart;
        break;
    case PositionAreaRegion::kEnd:
    case PositionAreaRegion::kInlineEnd:
    case PositionAreaRegion::kBlockEnd:
        axis_region = is_horizontal ? PositionAreaRegion::kXEnd : PositionAreaRegion::kYEnd;
        break;
    case PositionAreaRegion::kSelfStart:
    case PositionAreaRegion::kSelfInlineStart:
    case PositionAreaRegion::kSelfBlockStart:
        axis_region = is_horizontal ? PositionAreaRegion::kXSelfStart : PositionAreaRegion::kYSelfStart;
        break;
    case PositionAreaRegion::kSelfEnd:
    case PositionAreaRegion::kSelfInlineEnd:
    case PositionAreaRegion::kSelfBlockEnd:
        axis_region = is_horizontal ? PositionAreaRegion::kXSelfEnd : PositionAreaRegion::kYSelfEnd;
        break;
    default:
        break;
    }
    ...
    if (is_horizontal) {
        if ((axis_region == PositionAreaRegion::kXStart && container_writing_direction.IsFlippedX())
            || (axis_region == PositionAreaRegion::kXEnd && !container_writing_direction.IsFlippedX())
            || (axis_region == PositionAreaRegion::kXSelfStart && self_writing_direction.IsFlippedX())
            || (axis_region == PositionAreaRegion::kXSelfEnd && !self_writing_direction.IsFlippedX())) {
            return PositionAreaRegion::kRight;
        }
        return PositionAreaRegion::kLeft;
    }

    if ((axis_region == PositionAreaRegion::kYStart && container_writing_direction.IsFlippedY())
        || (axis_region == PositionAreaRegion::kYEnd && !container_writing_direction.IsFlippedY())
        || (axis_region == PositionAreaRegion::kYSelfStart && self_writing_direction.IsFlippedY())
        || (axis_region == PositionAreaRegion::kYSelfEnd && !self_writing_direction.IsFlippedY())) {
...
PositionArea PositionArea::ToPhysical(const WritingDirectionMode& container_writing_direction, const WritingDirectionMode& self_writing_direction) const
{
    if (IsNone()) {
        return *this;
    }
    PhysicalAxes first_axis = PhysicalAxisFromSpan(FirstStart(), FirstEnd(), container_writing_direction, self_writing_direction);
    PhysicalAxes second_axis = PhysicalAxisFromSpan(SecondStart(), SecondEnd(), container_writing_direction, self_writing_direction);
    ...
    auto regions
        = std::to_array<PositionAreaRegion>({ PositionAreaRegion::kTop, PositionAreaRegion::kBottom, PositionAreaRegion::kLeft, PositionAreaRegion::kRight });
    ...
    if (SecondStart() != PositionAreaRegion::kAll) {
        ...
        regions[index + 1] = ToPhysicalRegion(SecondEnd(), second_axis, container_writing_direction, self_writing_direction);
    }
...
std::pair<StyleSelfAlignmentData, StyleSelfAlignmentData> PositionArea::AlignJustifySelfFromPhysical(
    WritingDirectionMode container_writing_direction, bool is_containing_block_scrollable) const
{
    const OverflowAlignment overflow = is_containing_block_scrollable ? OverflowAlignment::kUnsafe : OverflowAlignment::kDefault;
    ...
    if ((FirstStart() == PositionAreaRegion::kTop && FirstEnd() == PositionAreaRegion::kBottom)
        || (FirstStart() == PositionAreaRegion::kCenter && FirstEnd() == PositionAreaRegion::kCenter)) {
    ...
    if ((SecondStart() == PositionAreaRegion::kLeft && SecondEnd() == PositionAreaRegion::kRight)
        || (SecondStart() == PositionAreaRegion::kCenter && SecondEnd() == PositionAreaRegion::kCenter)) {
        // 'center' or 'all' should align with anchor center.
    ...
    if ((FirstStart() == PositionAreaRegion::kTop && FirstEnd() == PositionAreaRegion::kTop)
        || (FirstStart() == PositionAreaRegion::kBottom && FirstEnd() == PositionAreaRegion::kBottom)) {
        align.SetOverflow(OverflowAlignment::kUnsafe);
        align_reverse.SetOverflow(OverflowAlignment::kUnsafe);
    }
    if ((SecondStart() == PositionAreaRegion::kLeft && SecondEnd() == PositionAreaRegion::kLeft)
        || (SecondStart() == PositionAreaRegion::kRight && SecondEnd() == PositionAreaRegion::kRight)) {
...
{
    return AnchorQuery(CSSAnchorQueryType::kAnchor, AnchorSpecifierValue::Default(), /* percentage */ 0, CSSAnchorValue::kTop);
}

AnchorQuery PositionArea::AnchorBottom()
{
    return AnchorQuery(CSSAnchorQueryType::kAnchor, AnchorSpecifierValue::Default(), /* percentage */ 0, CSSAnchorValue::kBottom);
}

AnchorQuery PositionArea::AnchorLeft()
{
    return AnchorQuery(CSSAnchorQueryType::kAnchor, AnchorSpecifierValue::Default(), /* percentage */ 0, CSSAnchorValue::kLeft);
}

AnchorQuery PositionArea::AnchorRight()
{
    return AnchorQuery(CSSAnchorQueryType::kAnchor, AnchorSpecifierValue::Default(), /* percentage */ 0, CSSAnchorValue::kRight);
}
