...
StyleInitialData::StyleInitialData(Document& document, const PropertyRegistry& registry)
{
    for (const auto& entry : registry) {
        ...
        const CSSValue* computed_initial_value = &StyleBuilderConverter::ConvertRegisteredPropertyInitialValue(document, *specified_initial_value);
        CSSVariableData* computed_initial_data
            = StyleBuilderConverter::ConvertRegisteredPropertyVariableData(*computed_initial_value, false /* is_animation_tainted */);
