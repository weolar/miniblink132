...
TEST(BlurFilterOperationTest, IsotropicStdDeviation)
{
    ...
    BlurFilterOperation* filter = MakeGarbageCollected<BlurFilterOperation>(fixedLength);
...
TEST(BlurFilterOperationTest, AnisotropicStdDeviation)
{
    ...
    BlurFilterOperation* filter = MakeGarbageCollected<BlurFilterOperation>(kFixedLength0, kFixedLength3);
    ...
    EXPECT_EQ(filter->StdDeviationXY(), LengthPoint(kFixedLength0, kFixedLength3));
}
