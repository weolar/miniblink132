...
{
    return ShadowData(gfx::Vector2dF(0, 0), 0, 0, ShadowStyle::kNormal, StyleColor(Color::kTransparent));
}
...
gfx::OutsetsF ShadowData::RectOutsets() const
{
    ...
    return gfx::OutsetsF().set_left(blur_and_spread - X()).set_right(blur_and_spread + X()).set_top(blur_and_spread - Y()).set_bottom(blur_and_spread + Y());
}
