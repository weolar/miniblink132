...
PhysicalSize StyleAspectRatio::LayoutRatioFromSizeF(gfx::SizeF ratio)
{
    ...
    if ((width.ToFloat() == ratio.width() && height.ToFloat() == ratio.height()) || ratio.IsEmpty()) {
        return { width, height };
    }
    ...
    for (wtf_size_t i = 0; i < 16; ++i) {
        ...
        if (h2 == std::numeric_limits<int>::max() || k2 == std::numeric_limits<int>::max()) {
            break;
        }
    ...
    return { LayoutUnit::FromRawValue(h1.RawValue()), LayoutUnit::FromRawValue(k1.RawValue()) };
}
