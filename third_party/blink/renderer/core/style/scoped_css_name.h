...
class CORE_EXPORT ScopedCSSNameList : public GarbageCollected<ScopedCSSNameList> {
public:
    ...
    {
        return base::ranges::equal(names_, other.names_, [](const auto& a, const auto& b) { return base::ValuesEquivalent(a, b); });
    }
    bool operator!=(const ScopedCSSNameList& other) const
    {
        return !operator==(other);
...
template <typename ScopedCSSNameWrapperType> struct ScopedCSSNameWrapperPtrHashTraits : MemberHashTraits<ScopedCSSNameWrapperType> {
    using TraitType = typename MemberHashTraits<ScopedCSSNameWrapperType>::TraitType;
    static unsigned GetHash(const TraitType& name)
    {
        return name->GetHash();
    }
...
template <> struct HashTraits<blink::Member<blink::ScopedCSSName>> : ScopedCSSNameWrapperPtrHashTraits<blink::ScopedCSSName> { };
template <> struct HashTraits<blink::Member<const blink::ScopedCSSName>> : ScopedCSSNameWrapperPtrHashTraits<const blink::ScopedCSSName> { };
