...
template <typename T> class CORE_EXPORT ValueRepeater : public GarbageCollected<ValueRepeater<T>> {
public:
    using VectorType = typename std::conditional<WTF::IsTraceable<T>::value, HeapVector<T, 1>, Vector<T>>::type;
    ValueRepeater() = default;

    explicit ValueRepeater(VectorType repeated_values, std::optional<wtf_size_t> repeat_count)
    ...
    {
        return repeated_values_ == other.repeated_values_ && repeat_count_ == other.repeat_count_;
    }

    bool IsAutoRepeater() const
    {
        return !repeat_count_.has_value();
    }
    const VectorType& RepeatedValues() const
    {
        return repeated_values_;
    }
...
template <typename T> class CORE_EXPORT GapData {
    DISALLOW_NEW();

public:
    GapData() = default;
    explicit GapData(T value)
        : value_(value)
    {
    }
    ...
    {
        return value_ == other.value_ && base::ValuesEquivalent(value_repeater_, other.value_repeater_);
    }
    ...
    bool IsRepeaterData() const
    {
        return value_repeater_ != nullptr;
    }
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::GapData<blink::StyleColor>)
