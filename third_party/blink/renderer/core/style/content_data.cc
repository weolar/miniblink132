...
ContentData* ContentData::Clone() const
{
    ...
    for (const ContentData* content_data = Next(); content_data; content_data = content_data->Next()) {
...
LayoutObject* ImageContentData::CreateLayoutObject(LayoutObject& owner) const
{
    ...
    if (image_) {
        image->SetImageResource(MakeGarbageCollected<LayoutImageResourceStyleImage>(image_.Get()));
    } else {
...
{
    LayoutObject* layout_object = LayoutTextFragment::CreateAnonymous(owner.GetDocument(), text_);
    layout_object->SetPseudoElementStyle(owner);
    return layout_object;
}

LayoutObject* AltTextContentData::CreateLayoutObject(LayoutObject& owner) const
{
...
LayoutObject* CounterContentData::CreateLayoutObject(LayoutObject& owner) const
{
    LayoutObject* layout_object = MakeGarbageCollected<LayoutCounter>(owner.GetDocument(), *this);
    layout_object->SetPseudoElementStyle(owner);
    return layout_object;
}
...
{
    LayoutObject* layout_object = MakeGarbageCollected<LayoutQuote>(owner, quote_);
    layout_object->SetPseudoElementStyle(owner);
    return layout_object;
}
