...
public:
    ...
    constexpr TextBoxEdge(Type over, Type under)
        : over_(over)
        , under_(under)
    {
    }
    ...
    explicit constexpr TextBoxEdge(unsigned value)
        : TextBoxEdge(static_cast<Type>(value & kTypeMask), static_cast<Type>((value >> kTypeBits) & kTypeMask))
    {
    }
    explicit constexpr operator unsigned() const
    {
        return static_cast<unsigned>(over_) | (static_cast<unsigned>(under_) << kTypeBits);
    }
