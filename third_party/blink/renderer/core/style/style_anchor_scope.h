...
public:
    ...
    StyleAnchorScope(Type type, const TreeScope* all_tree_scope, const ScopedCSSNameList* names)
        : type_(type)
        , all_tree_scope_(all_tree_scope)
        , names_(names)
    {
    }
    ...
    {
        return type_ == o.type_ && base::ValuesEquivalent(all_tree_scope_, o.all_tree_scope_) && base::ValuesEquivalent(names_, o.names_);
    }
    bool operator!=(const StyleAnchorScope& o) const
    {
        return !operator==(o);
    ...
    bool IsAll() const
    {
        return type_ == Type::kAll;
    }
    const TreeScope* AllTreeScope() const
    {
        return all_tree_scope_.Get();
    }
    const ScopedCSSNameList* Names() const
    {
        return names_.Get();
    }
