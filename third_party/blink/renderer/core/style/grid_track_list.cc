...
NGGridTrackRepeater::NGGridTrackRepeater(
    wtf_size_t repeat_index, wtf_size_t repeat_size, wtf_size_t repeat_count, wtf_size_t line_name_indices_count, RepeatType repeat_type)
...
{
    return repeat_index == other.repeat_index && repeat_size == other.repeat_size && repeat_count == other.repeat_count && repeat_type == other.repeat_type;
}

wtf_size_t NGGridTrackList::RepeatCount(wtf_size_t index, wtf_size_t auto_value) const
{
...
NGGridTrackRepeater::RepeatType NGGridTrackList::RepeatType(wtf_size_t index) const
{
    DCHECK_LT(index, RepeaterCount());
    return repeaters_[index].repeat_type;
}

const GridTrackSize& NGGridTrackList::RepeatTrackSize(wtf_size_t index, wtf_size_t n) const
{
...
bool NGGridTrackList::AddRepeater(const Vector<GridTrackSize, 1>& repeater_track_sizes, NGGridTrackRepeater::RepeatType repeat_type, wtf_size_t repeat_count,
    wtf_size_t repeat_number_of_lines, wtf_size_t line_name_indices_count)
{
    ...
    if (!IsSubgriddedAxis() && (repeat_count == 0u || repeater_track_sizes.empty())) {
        return false;
    }

    // If the repeater is auto or there isn't a repeater, the repeat_count should
    // be 1.
    DCHECK(repeat_type == NGGridTrackRepeater::RepeatType::kInteger || repeat_count == 1u);
    ...
    wtf_size_t repeat_size = IsSubgriddedAxis() ? repeat_number_of_lines : repeater_track_sizes.size();
    switch (repeat_type) {
    ...
    repeaters_.emplace_back(repeater_track_sizes_.size(), repeat_size, repeat_count, line_name_indices_count, repeat_type);
    if (!IsSubgriddedAxis()) {
...
{
    return TrackCountWithoutAutoRepeat() == other.TrackCountWithoutAutoRepeat() && RepeaterCount() == other.RepeaterCount()
        && auto_repeater_index_ == other.auto_repeater_index_ && repeaters_ == other.repeaters_ && repeater_track_sizes_ == other.repeater_track_sizes_
        && non_auto_repeat_line_count_ == other.non_auto_repeat_line_count_ && axis_type_ == other.axis_type_;
}
