...
{
    if (data_.size() != other.data_.size() || values_.size() != other.values_.size()) {
        return false;
    }

    if (equality_cache_partner_ == &other && other.equality_cache_partner_ == this) {
...
StyleVariables::OptionalData StyleVariables::GetData(const AtomicString& name) const
{
...
StyleVariables::OptionalValue StyleVariables::GetValue(const AtomicString& name) const
{
...
std::ostream& operator<<(std::ostream& stream, const StyleVariables& variables)
{
    stream << "[";
