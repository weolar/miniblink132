...
class CORE_EXPORT StyleNonInheritedVariables : public GarbageCollected<StyleNonInheritedVariables> {
public:
    ...
    const StyleVariables::DataMap& Data() const
    {
        return variables_.Data();
    }
    const StyleVariables::ValueMap& Values() const
    {
        return variables_.Values();
    }

    friend CORE_EXPORT std::ostream& operator<<(std::ostream& stream, const StyleNonInheritedVariables& variables);

private:
    StyleVariables variables_;
};

inline CORE_EXPORT std::ostream& operator<<(std::ostream& stream, const StyleNonInheritedVariables& variables)
{
