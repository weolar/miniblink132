...
StyleFetchedImage::StyleFetchedImage(ImageResourceContent* image, const Document& document, bool is_lazyload_possibly_deferred, bool origin_clean,
    bool is_ad_related, const KURL& url, const float override_image_resolution)
    : document_(document)
    , url_(url)
...
bool StyleFetchedImage::IsEqual(const StyleImage& other) const
{
    ...
    return image_ == other_image.image_ && url_ == other_image.url_ && EqualResolutions(override_image_resolution_, other_image.override_image_resolution_);
}
...
CSSValue* StyleFetchedImage::CssValue() const
{
    return MakeGarbageCollected<CSSImageValue>(
        CSSUrlData(AtomicString(url_.GetString()), url_, Referrer(), origin_clean_ ? OriginClean::kTrue : OriginClean::kFalse, is_ad_related_),
        const_cast<StyleFetchedImage*>(this));
}

CSSValue* StyleFetchedImage::ComputedCSSValue(const ComputedStyle&, bool allow_visited_style, CSSValuePhase value_phase) const
{
    return CssValue();
}
...
gfx::SizeF StyleFetchedImage::ImageSize(float multiplier, const gfx::SizeF& default_object_size, RespectImageOrientationEnum respect_orientation) const
{
    ...
    if (auto* svg_image = DynamicTo<SVGImage>(image)) {
        const SVGImageViewInfo* view_info = SVGImageForContainer::CreateViewInfo(*svg_image, url_);
        const gfx::SizeF unzoomed_default_object_size = gfx::ScaleSize(default_object_size, 1 / multiplier);
        size = SVGImageForContainer::ConcreteObjectSize(*svg_image, view_info, unzoomed_default_object_size);
    } else {
...
IntrinsicSizingInfo StyleFetchedImage::GetNaturalSizingInfo(float multiplier, RespectImageOrientationEnum respect_orientation) const
{
    Image& image = *image_->GetImage();
    IntrinsicSizingInfo intrinsic_sizing_info;
    if (auto* svg_image = DynamicTo<SVGImage>(image)) {
        const SVGImageViewInfo* view_info = SVGImageForContainer::CreateViewInfo(*svg_image, url_);
        if (!SVGImageForContainer::GetNaturalDimensions(*svg_image, view_info, intrinsic_sizing_info)) {
    ...
    multiplier = ApplyImageResolution(multiplier);
    intrinsic_sizing_info.size = ApplyZoom(intrinsic_sizing_info.size, multiplier);
...
scoped_refptr<Image> StyleFetchedImage::GetImage(
    const ImageResourceObserver&, const Document& document, const ComputedStyle& style, const gfx::SizeF& target_size) const
{
    Image* image = image_->GetImage();
    ...
    return SVGImageForContainer::Create(
        *svg_image, target_size, style.EffectiveZoom(), view_info, document.GetStyleEngine().ResolveColorSchemeForEmbedding(&style));
}

bool StyleFetchedImage::KnownToBeOpaque(const Document&, const ComputedStyle&) const
{
...
RespectImageOrientationEnum StyleFetchedImage::ForceOrientationIfNecessary(RespectImageOrientationEnum default_orientation) const
{
...
bool StyleFetchedImage::GetImageAnimationPolicy(mojom::blink::ImageAnimationPolicy& policy)
{
