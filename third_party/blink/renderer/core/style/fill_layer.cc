...
FillLayer::FillLayer(EFillLayerType type, bool use_initial_values)
    ...
    , attachment_(static_cast<unsigned>(FillLayer::InitialFillAttachment(type)))
    , clip_(static_cast<unsigned>(FillLayer::InitialFillClip(type)))
    , origin_(static_cast<unsigned>(FillLayer::InitialFillOrigin(type)))
    , compositing_operator_(static_cast<unsigned>(FillLayer::InitialFillCompositingOperator(type)))
    , size_type_(use_initial_values ? static_cast<unsigned>(FillLayer::InitialFillSizeType(type)) : static_cast<unsigned>(EFillSizeType::kSizeNone))
    ...
    , compositing_operator_set_(use_initial_values || type == EFillLayerType::kMask)
...
FillLayer::FillLayer(const FillLayer& o)
    : next_(o.next_ ? MakeGarbageCollected<FillLayerWrapper>(*o.next_) : nullptr)
    , image_(o.image_)
...
{
    return base::ValuesEquivalent(image_, o.image_) && position_x_ == o.position_x_ && position_y_ == o.position_y_
        && background_x_origin_ == o.background_x_origin_ && background_y_origin_ == o.background_y_origin_ && mask_mode_ == o.mask_mode_
        && attachment_ == o.attachment_ && clip_ == o.clip_ && compositing_operator_ == o.compositing_operator_ && blend_mode_ == o.blend_mode_
        && origin_ == o.origin_ && repeat_ == o.repeat_ && size_type_ == o.size_type_ && size_length_ == o.size_length_ && type_ == o.type_;
}

bool FillLayer::operator==(const FillLayer& o) const
{
    return LayerPropertiesEqual(o) && ((Next() && o.Next()) ? *Next() == *o.Next() : Next() == o.Next());
}
...
void FillLayer::FillUnsetProperties()
{
    ...
    for (curr = this; curr && curr->IsCompositingOperatorSet(); curr = curr->Next()) {
    }
    if (curr && curr != this) {
        // We need to fill in the remaining values with the pattern specified.
        for (FillLayer* pattern = this; curr; curr = curr->Next()) {
...
void FillLayer::ComputeCachedProperties() const
{
    ...
    any_layer_uses_content_box_ = effective_clip == EFillBox::kContent || Origin() == EFillBox::kContent;
    any_layer_has_image_ = !!GetImage();
    any_layer_has_url_image_ = any_layer_has_image_ && GetImage()->CssValue()->MayContainUrl();
    any_layer_has_local_attachment_ = Attachment() == EFillAttachment::kLocal;
    any_layer_has_fixed_attachment_image_ = any_layer_has_image_ && Attachment() == EFillAttachment::kFixed;
    any_layer_has_default_attachment_image_ = any_layer_has_image_ && Attachment() == EFillAttachment::kScroll;
    any_layer_uses_current_color_ = (image_ && image_->IsGeneratedImage() && To<StyleGeneratedImage>(image_.Get())->IsUsingCurrentColor());
    cached_properties_computed_ = true;

    if (auto* next = Next()) {
        next->ComputeCachedPropertiesIfNeeded();
        layers_clip_max_ = static_cast<unsigned>(EnclosingFillBox(LayersClipMax(), next->LayersClipMax()));
        ...
        any_layer_has_fixed_attachment_image_ |= next->any_layer_has_fixed_attachment_image_;
        any_layer_has_default_attachment_image_ |= next->any_layer_has_default_attachment_image_;
...
bool FillLayer::ImageIsOpaque(const Document& document, const ComputedStyle& style) const
{
    ...
    return image_->KnownToBeOpaque(document, style) && !image_->ImageSize(style.EffectiveZoom(), gfx::SizeF(), kRespectImageOrientation).IsEmpty();
}
...
bool FillLayer::ImageTilesLayer() const
{
    ...
    return (repeat.x == EFillRepeat::kRepeatFill || repeat.x == EFillRepeat::kRoundFill)
        && (repeat.y == EFillRepeat::kRepeatFill || repeat.y == EFillRepeat::kRoundFill);
}

bool FillLayer::ImageOccludesNextLayers(const Document& document, const ComputedStyle& style) const
{
    ...
    case kCompositeSourceOver:
        return GetBlendMode() == BlendMode::kNormal && ImageTilesLayer() && ImageIsOpaque(document, style);
    default: {
    }
    }
