...
static inline constexpr TryTacticList kNoTryTactics = { TryTactic::kNone, TryTactic::kNone, TryTactic::kNone };
...
public:
    ...
    const TryTacticList& GetTryTactic() const
    {
        return tactic_list_;
    }
    const ScopedCSSName* GetPositionTryName() const
    {
        return position_try_name_;
    }
...
class CORE_EXPORT PositionTryFallbacks : public GarbageCollected<PositionTryFallbacks> {
public:
...
template <> struct VectorTraits<blink::PositionTryFallback> : VectorTraitsBase<blink::PositionTryFallback> {
