...
MasonryLayoutAlgorithm::MasonryLayoutAlgorithm(const LayoutAlgorithmParams& params)
...
GridRangeVector ExpandRangesFromTemplateTracks(const NGGridTrackList& template_tracks, wtf_size_t auto_repetitions)
{
...
GridSizingTrackCollection MasonryLayoutAlgorithm::ComputeCrossAxisTrackSizes() const
{
    GridSizingTrackCollection cross_axis_tracks(ExpandRangesFromTemplateTracks(Style().MasonryTemplateTracks().track_list, ComputeAutomaticRepetitions()));
...
MinMaxSizesResult MasonryLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
