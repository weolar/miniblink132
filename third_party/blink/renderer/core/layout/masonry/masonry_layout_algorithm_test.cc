...
protected:
    ...
    {
        cross_axis_tracks_ = std::make_unique<GridSizingTrackCollection>(algorithm.ComputeCrossAxisTrackSizes());
    }

    const GridRangeVector& Ranges() const
    {
        return cross_axis_tracks_->ranges_;
    }
...
TEST_F(MasonryLayoutAlgorithmTest, TemplateTracksExpandedRanges)
{
    ...
    const auto fragment_geometry = CalculateInitialFragmentGeometry(space, node, /*break_token=*/nullptr);
    ...
    const Vector<wtf_size_t> expected_repeater_indices = { 0, 1, 1, 1, 1, 1, 1, 2, 2, 2 };
    const Vector<wtf_size_t> expected_repeater_offsets = { 0, 0, 1, 0, 1, 0, 1, 0, 1, 2 };
