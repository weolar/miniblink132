...
gfx::PointF StartingPoint(const PhysicalOffset& paint_offset, const LayoutBox& box, const PhysicalSize& size)
{
...
PhysicalRect RectBelowStartingPoint(
    const PhysicalRect& rect, const PhysicalOffset& starting_point, LayoutUnit logical_height, WritingDirectionMode writing_direction)
{
...
float GetMoveDistance(const gfx::PointF& old_starting_point, const gfx::PointF& new_starting_point)
{
...
bool EqualWithinMovementThreshold(const gfx::PointF& a, const gfx::PointF& b, float threshold_physical_px)
{
    return fabs(a.x() - b.x()) < threshold_physical_px && fabs(a.y() - b.y()) < threshold_physical_px;
}
...
void RectToTracedValue(const gfx::Rect& rect, TracedValue& value, const char* key = nullptr)
{
    if (key)
...
LayoutShiftTracker::LayoutShiftTracker(LocalFrameView* frame_view)
    ...
    is_active_(!frame_view->GetFrame().GetChromeClient().IsIsolatedSVGChromeClient())
    , score_(0.0)
    , weighted_score_(0.0)
    , timer_(frame_view->GetFrame().GetTaskRunner(TaskType::kInternalDefault), this, &LayoutShiftTracker::TimerFired)
...
bool LayoutShiftTracker::NeedsToTrack(const LayoutObject& object) const
{
    ...
    if (const auto* block = DynamicTo<LayoutBlock>(box)) {
        ...
        if (box->HasBoxDecorationBackground() || box->GetScrollableArea() || box->StyleRef().HasVisualOverflowingEffect()) {
            return true;
        }
        return false;
    }
...
void LayoutShiftTracker::ObjectShifted(const LayoutObject& object, const PropertyTreeStateOrAlias& property_tree_state, const PhysicalRect& old_rect,
    const PhysicalRect& new_rect, const gfx::PointF& old_starting_point, const gfx::Vector2dF& translation_delta, const gfx::Vector2dF& scroll_delta,
    const gfx::Vector2dF& scroll_anchor_adjustment, const gfx::PointF& new_starting_point)
{
    // The caller should ensure these conditions.
    DCHECK(!old_rect.IsEmpty());
    DCHECK(!new_rect.IsEmpty());

    float threshold_physical_px = kMovementThreshold * object.StyleRef().EffectiveZoom();

    // Check shift of starting point, including 2d-translation and scroll
    // deltas.
    if (EqualWithinMovementThreshold(old_starting_point, new_starting_point, threshold_physical_px))
        return;

    // Check shift of 2d-translation-indifferent starting point.
    if (!translation_delta.IsZero() && EqualWithinMovementThreshold(old_starting_point + translation_delta, new_starting_point, threshold_physical_px))
        return;

    // Check shift of scroll-indifferent starting point.
    if (!scroll_delta.IsZero() && EqualWithinMovementThreshold(old_starting_point + scroll_delta, new_starting_point, threshold_physical_px))
        return;

    if (!scroll_anchor_adjustment.IsZero()
        && EqualWithinMovementThreshold(old_starting_point + scroll_delta + scroll_anchor_adjustment, new_starting_point, threshold_physical_px)) {
        return;
    }

    // Check shift of 2d-translation-and-scroll-indifferent starting point.
    gfx::Vector2dF translation_and_scroll_delta = scroll_delta + translation_delta;
    if (!translation_and_scroll_delta.IsZero()
        && EqualWithinMovementThreshold(old_starting_point + translation_and_scroll_delta, new_starting_point, threshold_physical_px))
        return;

    const auto& root_state = object.View()->FirstFragment().LocalBorderBoxProperties();
    FloatClipRect clip_rect = GeometryMapper::LocalToAncestorClipRect(property_tree_state, root_state);
    if (frame_view_->GetFrame().IsMainFrame()) {
        // Apply the visual viewport clip.
        clip_rect.Intersect(FloatClipRect(frame_view_->GetPage()->GetVisualViewport().VisibleRect()));
    }
    ...
    auto transform = GeometryMapper::SourceToDestinationProjection(property_tree_state.Transform(), root_state.Transform());
    ...
    gfx::PointF old_starting_point_in_root = transform.MapPoint(old_starting_point + scroll_delta);
    gfx::PointF new_starting_point_in_root = transform.MapPoint(new_starting_point);

    if (EqualWithinMovementThreshold(old_starting_point_in_root, new_starting_point_in_root, threshold_physical_px))
        return;
    ...
    gfx::Rect visible_old_rect = gfx::ToRoundedRect(gfx::IntersectRects(old_rect_in_root, clip_rect.Rect()));
    gfx::Rect visible_new_rect = gfx::ToRoundedRect(gfx::IntersectRects(new_rect_in_root, clip_rect.Rect()));
    ...
    if (visible_old_rect.IsEmpty() || visible_new_rect.IsEmpty()) {
        gfx::PointF old_inline_direction_indifferent_starting_point_in_root = old_starting_point_in_root;
        if (object.IsHorizontalWritingMode()) {
            old_inline_direction_indifferent_starting_point_in_root.set_x(new_starting_point_in_root.x());
        } else {
            old_inline_direction_indifferent_starting_point_in_root.set_y(new_starting_point_in_root.y());
        }
        if (EqualWithinMovementThreshold(old_inline_direction_indifferent_starting_point_in_root, new_starting_point_in_root, threshold_physical_px)) {
            return;
        }
    }
    ...
    float move_distance = GetMoveDistance(old_starting_point_in_root, new_starting_point_in_root);
    ...
    if (ShouldLog(frame)) {
        VLOG(2) << "in " << (frame.IsOutermostMainFrame() ? "" : "subframe ") << frame.GetDocument()->Url() << ", " << object << " moved from "
                << old_rect_in_root.ToString() << " to " << new_rect_in_root.ToString() << " (visible from " << visible_old_rect.ToString() << " to "
                << visible_new_rect.ToString() << ")";
        if (old_starting_point_in_root != old_rect_in_root.origin() || new_starting_point_in_root != new_rect_in_root.origin() || !translation_delta.IsZero()
            || !scroll_delta.IsZero()) {
            VLOG(2) << " (starting point from " << old_starting_point_in_root.ToString() << " to " << new_starting_point_in_root.ToString() << ")";
        }
    }
    ...
    if (Node* node = object.GetNode()) {
        MaybeRecordAttribution({ node->GetDomNodeId(), visible_old_rect, visible_new_rect });
    }
}
...
{
    return old_visual_rect.Contains(other.old_visual_rect) && new_visual_rect.Contains(other.new_visual_rect);
}
...
uint64_t LayoutShiftTracker::Attribution::Area() const
{
    ...
    gfx::Rect intersection = gfx::IntersectRects(old_visual_rect, new_visual_rect);
    uint64_t shared_area = intersection.size().Area64();
    return old_area + new_area - shared_area;
}

bool LayoutShiftTracker::Attribution::MoreImpactfulThan(const Attribution& other) const
{
    return Area() > other.Area();
}

void LayoutShiftTracker::MaybeRecordAttribution(const Attribution& attribution)
{
...
void LayoutShiftTracker::NotifyBoxPrePaint(const LayoutBox& box, const PropertyTreeStateOrAlias& property_tree_state, const PhysicalRect& old_rect,
    const PhysicalRect& new_rect, const PhysicalOffset& old_paint_offset, const gfx::Vector2dF& translation_delta, const gfx::Vector2dF& scroll_delta,
    const gfx::Vector2dF& scroll_anchor_adjustment, const PhysicalOffset& new_paint_offset)
{
    DCHECK(NeedsToTrack(box));
    ObjectShifted(box, property_tree_state, old_rect, new_rect, StartingPoint(old_paint_offset, box, box.PreviousSize()), translation_delta, scroll_delta,
        scroll_anchor_adjustment, StartingPoint(new_paint_offset, box, box.Size()));
}

void LayoutShiftTracker::NotifyTextPrePaint(const LayoutText& text, const PropertyTreeStateOrAlias& property_tree_state,
    const LogicalOffset& old_starting_point, const LogicalOffset& new_starting_point, const PhysicalOffset& old_paint_offset,
    const gfx::Vector2dF& translation_delta, const gfx::Vector2dF& scroll_delta, const gfx::Vector2dF& scroll_anchor_adjustment,
    const PhysicalOffset& new_paint_offset, LayoutUnit logical_height)
{
    ...
    PhysicalOffset old_physical_starting_point = old_paint_offset + old_starting_point.ConvertToPhysical(writing_direction, block->old_size_, PhysicalSize());
    PhysicalOffset new_physical_starting_point = new_paint_offset + new_starting_point.ConvertToPhysical(writing_direction, block->new_size_, PhysicalSize());

    PhysicalRect old_rect = RectBelowStartingPoint(block->old_rect_, old_physical_starting_point, logical_height, writing_direction);
    if (old_rect.IsEmpty())
        return;
    PhysicalRect new_rect = RectBelowStartingPoint(block->new_rect_, new_physical_starting_point, logical_height, writing_direction);
    if (new_rect.IsEmpty())
        return;

    ObjectShifted(text, property_tree_state, old_rect, new_rect, gfx::PointF(old_physical_starting_point), translation_delta, scroll_delta,
        scroll_anchor_adjustment, gfx::PointF(new_physical_starting_point));
}
...
double LayoutShiftTracker::SubframeWeightingFactor() const
{
    ...
    GeometryMapper::LocalToAncestorVisualRect(frame_view_->GetLayoutView()->FirstFragment().LocalBorderBoxProperties(),
        local_root.ContentLayoutObject()->FirstFragment().LocalBorderBoxProperties(), subframe_cliprect);
    ...
    return static_cast<double>(subframe_visible_size.Area64()) / main_frame_size.Area64();
}
...
void LayoutShiftTracker::NotifyPrePaintFinishedInternal()
{
    ...
    double move_distance_factor = (frame_max_distance_ < viewport_max_dimension) ? double(frame_max_distance_) / viewport_max_dimension : 1.0;
    ...
    LocalFrame& frame = frame_view_->GetFrame();
    if (ShouldLog(frame)) {
        VLOG(2) << "in " << (frame.IsOutermostMainFrame() ? "" : "subframe ") << frame.GetDocument()->Url() << ", viewport was " << (impact_fraction * 100)
                << "% impacted with distance fraction " << move_distance_factor << " and subframe weighting factor " << SubframeWeightingFactor();
    }

    if (pointerdown_pending_data_.num_pointerdowns > 0 || pointerdown_pending_data_.num_pressed_mouse_buttons > 0) {
...
LayoutShift::AttributionList LayoutShiftTracker::CreateAttributionList() const
{
    ...
    for (const Attribution& att : attributions_) {
        ...
        list.push_back(LayoutShiftAttribution::Create(
            DOMNodeIds::NodeForId(att.node_id), DOMRectReadOnly::FromRect(att.old_visual_rect), DOMRectReadOnly::FromRect(att.new_visual_rect)));
    }
    return list;
}

void LayoutShiftTracker::SubmitPerformanceEntry(double score_delta, bool had_recent_input) const
{
    ...
    LayoutShift* entry = LayoutShift::Create(performance->now(), score_delta, had_recent_input, input_timestamp, CreateAttributionList(), window);
...
void LayoutShiftTracker::ReportShift(double score_delta, double weighted_score_delta)
{
    LocalFrame& frame = frame_view_->GetFrame();
    ...
    if (!had_recent_input) {
        ...
        if (weighted_score_delta > 0) {
            ...
            frame.Client()->DidObserveLayoutShift(weighted_score_delta, observed_input_or_scroll_);
        }
    }

    SubmitPerformanceEntry(score_delta, had_recent_input);

    TRACE_EVENT_INSTANT2("loading", "LayoutShift", TRACE_EVENT_SCOPE_THREAD, "data", PerFrameTraceData(score_delta, weighted_score_delta, had_recent_input),
        "frame", GetFrameIdForTracing(&frame));

    if (ShouldLog(frame)) {
        VLOG(2) << "in " << (frame.IsOutermostMainFrame() ? "" : "subframe ") << frame.GetDocument()->Url().GetString() << ", layout shift of " << score_delta
                << (had_recent_input ? " excluded by recent input" : " reported") << "; cumulative score is " << score_;
    }
}
...
void LayoutShiftTracker::NotifyInput(const WebInputEvent& event)
{
    ...
    if (type == WebInputEvent::Type::kMouseUp) {
        ...
        release_all_mouse_buttons = pointerdown_pending_data_.num_pressed_mouse_buttons == 0;
    }
    ...
    const bool event_type_stops_pointerdown_buffering = type == WebInputEvent::Type::kPointerCausedUaAction || type == WebInputEvent::Type::kPointerCancel;

    // Only non-hovering pointerdown requires buffering.
    const bool is_hovering_pointerdown = type == WebInputEvent::Type::kPointerDown && static_cast<const WebPointerEvent&>(event).hovering;

    const bool should_trigger_shift_exclusion = type == WebInputEvent::Type::kMouseDown || type == WebInputEvent::Type::kKeyDown
        ...
        type == WebInputEvent::Type::kGestureTap || is_hovering_pointerdown || release_all_pointers || release_all_mouse_buttons;
    ...
    if (event_type_stops_pointerdown_buffering || release_all_mouse_buttons || release_all_pointers) {
...
void LayoutShiftTracker::UpdateInputTimestamps(base::TimeTicks timestamp)
{
    ...
    most_recent_input_timestamp_ = std::max(timestamp, most_recent_input_timestamp_);
...
bool LayoutShiftTracker::HasRecentInput()
{
    ...
    base::TimeDelta time_since_last_input = blink::Thread::Current()->CurrentTaskStartTime() - most_recent_input_processing_timestamp_;
...
void LayoutShiftTracker::NotifyScroll(mojom::blink::ScrollType scroll_type, ScrollOffset delta)
{
...
double LayoutShiftTracker::LastInputTimestamp() const
{
    ...
    return most_recent_input_timestamp_initialized_ ? performance->MonotonicTimeToDOMHighResTimeStamp(most_recent_input_timestamp_) : 0.0;
}

std::unique_ptr<TracedValue> LayoutShiftTracker::PerFrameTraceData(double score_delta, double weighted_score_delta, bool input_detected) const
{
    ...
    value->SetBoolean("is_main_frame", frame_view_->GetFrame().IsOutermostMainFrame());
...
void LayoutShiftTracker::AttributionsToTracedValue(TracedValue& value) const
{
    ...
    TRACE_EVENT_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("layout_shift.debug"), &should_include_names);
...
void LayoutShiftTracker::SendLayoutShiftRectsToHud(const Vector<gfx::Rect>& int_rects)
{
    ...
    if (cc_layer && cc_layer->layer_tree_host()) {
        ...
        if (cc_layer->layer_tree_host()->hud_layer()) {
            ...
            cc_layer->layer_tree_host()->hud_layer()->SetLayoutShiftRects(rects.ReleaseVector());
...
void ReattachHookScope::NotifyDetach(const Node& node)
{
    ...
    if (!layout_object || layout_object->ShouldSkipNextLayoutShiftTracking() || !layout_object->IsBox())
        return;
    ...
    map.Set(&node, Geometry { fragment.PaintOffset(), box.PreviousSize(), visual_overflow_rect, has_paint_offset_transform });
}
...
void ReattachHookScope::NotifyAttach(const Node& node)
{
    ...
    To<LayoutBox>(layout_object)
        ...
        .SetPreviousGeometryForLayoutShiftTracking(iter->value.paint_offset, iter->value.size, iter->value.visual_overflow_rect);
    layout_object->SetShouldSkipNextLayoutShiftTracking(false);
    layout_object->SetShouldAssumePaintOffsetTranslationForLayoutShiftTracking(iter->value.has_paint_offset_translation);
}
