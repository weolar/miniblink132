...
protected:
    template <bool should_have_wrapper> void ExpectAnonymousInlineWrapperFor(Node*);
};
...
template <bool should_have_wrapper> void LayoutObjectTest::ExpectAnonymousInlineWrapperFor(Node* node)
{
    ASSERT_TRUE(node);
...
TEST_F(LayoutObjectTest, LayoutDecoratedNameCalledWithPositionedObject)
{
    ...
    DCHECK(obj);
    EXPECT_THAT(obj->DecoratedName().Ascii(), MatchesRegex("LayoutN?G?BlockFlow \\(positioned, children-inline\\)"));
}
...
TEST_F(LayoutObjectTest, BackdropFilterAsGroupingProperty)
{
    ...
    EXPECT_TRUE(GetLayoutObjectByElementId("target1")->StyleRef().HasGroupingPropertyForUsedTransformStyle3D());
    EXPECT_TRUE(GetLayoutObjectByElementId("target2")->StyleRef().HasGroupingPropertyForUsedTransformStyle3D());
    ...
    EXPECT_FALSE(GetLayoutObjectByElementId("target3")->StyleRef().HasGroupingPropertyForUsedTransformStyle3D());
...
TEST_F(LayoutObjectTest, BlendModeAsGroupingProperty)
{
    ...
    EXPECT_FALSE(GetLayoutObjectByElementId("target2")->StyleRef().HasGroupingPropertyForUsedTransformStyle3D());
...
TEST_F(LayoutObjectTest, CSSClipAsGroupingProperty)
{
    ...
    EXPECT_FALSE(GetLayoutObjectByElementId("target1")->StyleRef().HasGroupingPropertyForUsedTransformStyle3D());
...
TEST_F(LayoutObjectTest, UseCountContainWithoutContentVisibility)
{
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSContainAllWithoutContentVisibility));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSContainStrictWithoutContentVisibility));
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSContainAllWithoutContentVisibility));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSContainStrictWithoutContentVisibility));
...
{
    EXPECT_EQ(GetDocument().body()->GetLayoutObject()->ContainingBlock(), GetDocument().documentElement()->GetLayoutObject());
}

TEST_F(LayoutObjectTest, ContainingBlockDocumentElementShouldBeLayoutView)
{
    EXPECT_EQ(GetDocument().documentElement()->GetLayoutObject()->ContainingBlock(), GetLayoutView());
}
...
TEST_F(LayoutObjectTest, ContainingBlockAbsoluteLayoutObjectShouldBeLayoutView)
{
    ...
    LayoutObject* layout_object = GetDocument().body()->GetLayoutObject()->SlowFirstChild();
...
TEST_F(LayoutObjectTest, ContainingBlockAbsoluteLayoutObjectShouldBeNonStaticallyPositionedBlockAncestor)
{
    ...
    LayoutObject* containing_blocklayout_object = GetDocument().body()->GetLayoutObject()->SlowFirstChild();
    LayoutObject* layout_object = containing_blocklayout_object->SlowFirstChild();
    EXPECT_TRUE(containing_blocklayout_object->CanContainOutOfFlowPositionedElement(EPosition::kAbsolute));
    EXPECT_FALSE(containing_blocklayout_object->CanContainOutOfFlowPositionedElement(EPosition::kFixed));
    ...
    EXPECT_EQ(layout_object->ContainingBlockForAbsolutePosition(), containing_blocklayout_object);
    EXPECT_EQ(layout_object->ContainingBlockForFixedPosition(), GetLayoutView());
    auto offset = layout_object->OffsetFromContainer(containing_blocklayout_object);
...
TEST_F(LayoutObjectTest, ContainingBlockFixedLayoutObjectInTransformedDiv)
{
    ...
    LayoutObject* layout_object = containing_blocklayout_object->SlowFirstChild();
    EXPECT_TRUE(containing_blocklayout_object->CanContainOutOfFlowPositionedElement(EPosition::kAbsolute));
    EXPECT_TRUE(containing_blocklayout_object->CanContainOutOfFlowPositionedElement(EPosition::kFixed));
    EXPECT_EQ(layout_object->Container(), containing_blocklayout_object);
    EXPECT_EQ(layout_object->ContainingBlock(), containing_blocklayout_object);
    EXPECT_EQ(layout_object->ContainingBlockForAbsolutePosition(), containing_blocklayout_object);
    EXPECT_EQ(layout_object->ContainingBlockForFixedPosition(), containing_blocklayout_object);
}
...
TEST_F(LayoutObjectTest, ContainingBlockFixedLayoutObjectInBody)
{
    ...
    LayoutObject* layout_object = GetDocument().body()->GetLayoutObject()->SlowFirstChild();
    EXPECT_TRUE(layout_object->CanContainOutOfFlowPositionedElement(EPosition::kAbsolute));
    EXPECT_FALSE(layout_object->CanContainOutOfFlowPositionedElement(EPosition::kFixed));
    EXPECT_EQ(layout_object->Container(), GetLayoutView());
    EXPECT_EQ(layout_object->ContainingBlock(), GetLayoutView());
    EXPECT_EQ(layout_object->ContainingBlockForAbsolutePosition(), GetLayoutView());
    EXPECT_EQ(layout_object->ContainingBlockForFixedPosition(), GetLayoutView());
}
...
TEST_F(LayoutObjectTest, ContainingBlockAbsoluteLayoutObjectShouldNotBeNonStaticallyPositionedInlineAncestor)
{
    // Test note: We can't use a raw string literal here, since extra whitespace
    // causes failures.
    SetBodyInnerHTML("<span style='position:relative; top:1px; left:2px'><bar "
    ...
    EXPECT_TRUE(span_layout_object->CanContainOutOfFlowPositionedElement(EPosition::kAbsolute));
    EXPECT_FALSE(span_layout_object->CanContainOutOfFlowPositionedElement(EPosition::kFixed));
    ...
    EXPECT_EQ(layout_object->ContainingBlock(), body_layout_object);
    EXPECT_EQ(layout_object->ContainingBlockForAbsolutePosition(), body_layout_object);
    EXPECT_EQ(layout_object->ContainingBlockForFixedPosition(), GetLayoutView());
}
...
TEST_F(LayoutObjectTest, PaintingLayerOfOverflowClipLayerUnderColumnSpanAll)
{
    ...
    LayoutObject* overflow_clip_object = GetLayoutObjectByElementId("overflow-clip-layer");
...
TEST_F(LayoutObjectTest, FloatUnderBlock)
{
    ...
    auto* layered_div = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("layered-div"));
...
TEST_F(LayoutObjectTest, FloatUnderInline)
{
    ...
    auto* layered_div = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("layered-div"));
    auto* container = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("container"));
    auto* layered_span = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("layered-span"));
    LayoutObject* floating = GetLayoutObjectByElementId("floating");
...
TEST_F(LayoutObjectTest, SubtreePaintPropertyUpdateReasons)
{
    ...
    object->AddSubtreePaintPropertyUpdateReason(SubtreePaintPropertyUpdateReason::kPreviouslySkipped);
...
TEST_F(LayoutObjectTest, ShouldCheckLayoutForPaintInvalidation)
{
    ...
    object->SetShouldDoFullPaintInvalidation();
    EXPECT_TRUE(object->ShouldDoFullPaintInvalidation());
    EXPECT_EQ(PaintInvalidationReason::kLayout, object->PaintInvalidationReasonForPrePaint());
    EXPECT_TRUE(object->ShouldCheckLayoutForPaintInvalidation());
    ...
    object->SetShouldDoFullPaintInvalidationWithoutLayoutChange(PaintInvalidationReason::kStyle);
    EXPECT_EQ(PaintInvalidationReason::kStyle, object->PaintInvalidationReasonForPrePaint());
    EXPECT_TRUE(object->ShouldDoFullPaintInvalidation());
    EXPECT_FALSE(object->ShouldCheckLayoutForPaintInvalidation());
    EXPECT_TRUE(parent->ShouldCheckForPaintInvalidation());
    EXPECT_FALSE(parent->ShouldCheckLayoutForPaintInvalidation());
...
{
    const char* body_content = "<style>p:first-letter {color:red;}</style><p id=sample>(a)bc</p>";
    SetBodyInnerHTML(body_content);

    Node* sample = GetElementById("sample");
    Node* text = sample->firstChild();

    const auto* layout_object0 = To<LayoutTextFragment>(AssociatedLayoutObjectOf(*text, 0));
    EXPECT_FALSE(layout_object0->IsRemainingTextLayoutObject());

    const auto* layout_object1 = To<LayoutTextFragment>(AssociatedLayoutObjectOf(*text, 1));
    EXPECT_EQ(layout_object0, layout_object1) << "A character 'a' should be part of first letter.";

    const auto* layout_object2 = To<LayoutTextFragment>(AssociatedLayoutObjectOf(*text, 2));
    EXPECT_EQ(layout_object0, layout_object2) << "close parenthesis should be part of first letter.";

    const auto* layout_object3 = To<LayoutTextFragment>(AssociatedLayoutObjectOf(*text, 3));
...
TEST_F(LayoutObjectTest, AssociatedLayoutObjectOfFirstLetterSplit)
{
    ...
    const auto* layout_object0 = To<LayoutTextFragment>(AssociatedLayoutObjectOf(*first_letter, 0));
    EXPECT_FALSE(layout_object0->IsRemainingTextLayoutObject());

    const auto* layout_object1 = To<LayoutTextFragment>(AssociatedLayoutObjectOf(*first_letter, 1));
    EXPECT_EQ(layout_object0, layout_object1);
}

TEST_F(LayoutObjectTest, AssociatedLayoutObjectOfFirstLetterWithTrailingWhitespace)
{
...
TEST_F(LayoutObjectTest, DisplayContentsWrapperPerTextNode)
{
    ...
    SetBodyInnerHTML("<div id='div' style='display:contents;color:pink'>A<!-- -->B</div>");
    Element* div = GetElementById("div");
    ASSERT_TRUE(div);
    ...
    EXPECT_NE(text1->GetLayoutObject()->Parent(), text2->GetLayoutObject()->Parent());
}
...
TEST_F(LayoutObjectTest, DisplayContentsWrapperInTable)
{
    ...
    LayoutObject* wrapper_parent = contents->firstChild()->GetLayoutObject()->Parent()->Parent();
...
lime'>
    ...
    EXPECT_THAT(result.ToString().Utf8(),
        MatchesRegex("LayoutN?G?BlockFlow \\(children-inline\\)\tDIV id=\"block\" "
                     "style=\"background:\\\\nlime\""));
...
public:
    bool DocumentHasTouchActionRegion(const EventHandlerRegistry& registry)
    {
        ...
        return registry.HasEventHandlers(EventHandlerRegistry::EventHandlerClass::kTouchAction);
    }
...
TEST_F(LayoutObjectSimTest, TouchActionUpdatesSubframeEventHandler)
{
    ...
    EventHandlerRegistry& registry = iframe_doc->GetFrame()->GetEventHandlerRegistry();

    // We should add event handler if touch action is set on subframe.
    inner->setAttribute(html_names::kStyleAttr, AtomicString("touch-action: none"));
    EXPECT_TRUE(DocumentHasTouchActionRegion(registry));

    // We should remove event handler if touch action is removed on subframe.
    inner->setAttribute(html_names::kStyleAttr, AtomicString("touch-action: auto"));
    EXPECT_FALSE(DocumentHasTouchActionRegion(registry));

    // We should add event handler if touch action is set on main frame.
    container->setAttribute(html_names::kStyleAttr, AtomicString("touch-action: none"));
    EXPECT_TRUE(DocumentHasTouchActionRegion(registry));

    // We should keep event handler if touch action is set on subframe document
    // element.
    iframe_doc_element->setAttribute(html_names::kStyleAttr, AtomicString("touch-action: none"));
    EXPECT_TRUE(DocumentHasTouchActionRegion(registry));

    // We should keep the event handler if touch action is removed on subframe
    // document element.
    iframe_doc_element->setAttribute(html_names::kStyleAttr, AtomicString("touch-action: auto"));
    EXPECT_TRUE(DocumentHasTouchActionRegion(registry));

    // We should remove the handler if touch action is removed on main frame.
    container->setAttribute(html_names::kStyleAttr, AtomicString("touch-action: auto"));
    EXPECT_FALSE(DocumentHasTouchActionRegion(registry));
}
...
TEST_F(LayoutObjectSimTest, FirstLineBackgroundImage)
{
    ...
    auto* image_resource_content = target_object->FirstLineStyleRef().BackgroundLayers().GetImage()->CachedImage();

    // Simulate an image change notification, and we should invalidate the objects
    // in the first line.
    static_cast<ImageObserver*>(image_resource_content)->Changed(image_resource_content->GetImage());
    ...
    auto* first_line1 = GetDocument().getElementById(AtomicString("first-line1"))->GetLayoutObject();
    ...
    auto* first_line2 = GetDocument().getElementById(AtomicString("first-line2"))->GetLayoutObject();
    ...
    auto* second_line = GetDocument().getElementById(AtomicString("second-line"))->GetLayoutObject();
...
TEST_F(LayoutObjectTest, FirstLineBackgroundImageNestedCrash)
{
    ...
    GetDocument().documentElement()->setAttribute(html_names::kStyleAttr, AtomicString("display: none"));
    UpdateAllLifecyclePhasesForTest();
}
...
TEST_F(LayoutObjectTest, FirstLineBackgroundImageChangeStyleCrash)
{
    ...
    GetElementById("target")->setAttribute(html_names::kStyleAttr, AtomicString("color: blue"));
    UpdateAllLifecyclePhasesForTest();

    GetElementById("target")->setAttribute(html_names::kStyleAttr, AtomicString("display: none"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(LayoutObjectSimTest, FirstLineBackgroundImageDirtyStyleCrash)
{
    ...
    CSSStyleSheet* sheet = To<HTMLStyleElement>(GetDocument().getElementById(AtomicString("style")))->sheet();
    {
...
TEST_F(LayoutObjectTest, PerspectiveWithAnonymousTable)
{
    ...
    auto* ancestor = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("ancestor"));
...
TEST_F(LayoutObjectTest, LocalToAncestoRectIgnoreAncestorScroll)
{
    ...
    LayoutObject* target = GetLayoutObjectByElementId("target");
    LayoutBoxModelObject* ancestor = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("ancestor"));
    ancestor->GetScrollableArea()->ScrollBy(ScrollOffset(0, 100), mojom::blink::ScrollType::kUser);
    UpdateAllLifecyclePhasesForTest();

    PhysicalRect rect(0, 0, 100, 100);

    EXPECT_EQ(PhysicalRect(0, 2000, 100, 100), target->LocalToAncestorRect(rect, ancestor, kIgnoreScrollOffset));

    EXPECT_EQ(PhysicalRect(0, 1900, 100, 100), target->LocalToAncestorRect(rect, ancestor, 0));
}
...
TEST_F(LayoutObjectTest, LocalToAncestoRectViewIgnoreAncestorScroll)
{
    ...
    LayoutObject* target = GetLayoutObjectByElementId("target");
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 100), mojom::blink::ScrollType::kProgrammatic);
    UpdateAllLifecyclePhasesForTest();

    PhysicalRect rect(0, 0, 100, 100);

    EXPECT_EQ(PhysicalRect(0, 2000, 100, 100), target->LocalToAncestorRect(rect, nullptr, kIgnoreScrollOffset));

    EXPECT_EQ(PhysicalRect(0, 1900, 100, 100), target->LocalToAncestorRect(rect, nullptr, 0));
}

TEST_F(LayoutObjectTest, LocalToAncestoRectIgnoreAncestorScrollIntermediateScroller)
{
    SetBodyInnerHTML(R"HTML(
    <style>body { margin:0; }</style>
    <div id=ancestor style="overflow:scroll; width: 100px; height: 100px">
    ...
    LayoutObject* target = GetLayoutObjectByElementId("target");
    LayoutBoxModelObject* ancestor = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("ancestor"));
    LayoutBoxModelObject* intermediate = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("intermediate"));
    ancestor->GetScrollableArea()->ScrollBy(ScrollOffset(0, 100), mojom::blink::ScrollType::kUser);
    intermediate->GetScrollableArea()->ScrollBy(ScrollOffset(0, 100), mojom::blink::ScrollType::kUser);
    UpdateAllLifecyclePhasesForTest();
    ...
    EXPECT_EQ(PhysicalRect(0, 1800, 100, 100), target->LocalToAncestorRect(rect, ancestor, 0));
}

TEST_F(LayoutObjectTest, LocalToAncestoRectViewIgnoreAncestorScrollIntermediateScroller)
{
    SetBodyInnerHTML(R"HTML(
    <style>body { margin:0; }</style>
    <div id=intermediate style="overflow:scroll; width: 100px; height: 100px">
    ...
    EXPECT_EQ(PhysicalRect(0, 1800, 100, 100), target->LocalToAncestorRect(rect, nullptr, 0));
}
...
TEST_F(LayoutObjectTest, RemovePendingTransformUpdatesCorrectly)
{
  ...
  <div id="div2" style="transform:translateX(100px)">
    ...
    div2->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateX(200px)"));
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);

    auto* div1 = GetElementById("div1");
    div1->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateX(200px)"));
    div2->SetInlineStyleProperty(CSSPropertyID::kDisplay, "none");
    UpdateAllLifecyclePhasesForTest();
}

static const char* const kTransformsWith3D[] = { "transform: rotateX(20deg)", "transform: translateZ(30px)" };
static const char kTransformWithout3D[] = "transform: matrix(2, 2, 0, 2, 2, 2, 0, 2, 2, 2, 2, 2, 2, 2, 0, 2)";
static const char kPreserve3D[] = "transform-style: preserve-3d";

TEST_F(LayoutObjectTestWithCompositing, UseCountDifferentPerspectiveCBOrParent)
{
    ...
    target->setAttribute(html_names::kStyleAttr, AtomicString(kTransformsWith3D[0]));
    UpdateAllLifecyclePhasesForTest();
    target->scrollIntoView();
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kDifferentPerspectiveCBOrParent));
    ...
    target->setAttribute(html_names::kStyleAttr, AtomicString(kTransformWithout3D));
    UpdateAllLifecyclePhasesForTest();
    target->scrollIntoView();
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kDifferentPerspectiveCBOrParent));
    ...
    UpdateAllLifecyclePhasesForTest();
    target->scrollIntoView();
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kDifferentPerspectiveCBOrParent));
    GetDocument().ClearUseCounterForTesting(WebFeature::kDifferentPerspectiveCBOrParent);

    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kDifferentPerspectiveCBOrParent));

    target->setAttribute(html_names::kStyleAttr, AtomicString(kTransformsWith3D[1]));
    UpdateAllLifecyclePhasesForTest();
    target->scrollIntoView();
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kDifferentPerspectiveCBOrParent));
    GetDocument().ClearUseCounterForTesting(WebFeature::kDifferentPerspectiveCBOrParent);
...
TEST_F(LayoutObjectTest, HasTransformRelatedProperty)
{
    ...
    auto test = [&](const char* element_id, bool has_transform_related_property, bool has_transform, bool preserves_3d) {
        SCOPED_TRACE(element_id);
        const auto* object = GetLayoutObjectByElementId(element_id);
        EXPECT_EQ(has_transform_related_property, object->HasTransformRelatedProperty());
...
TEST_F(LayoutObjectTest, ContainingScrollContainer)
{
    ...
    EXPECT_EQ(scroller1, GetLayoutObjectByElementId("child1")->ContainingScrollContainer());
    EXPECT_EQ(scroller1, scroller2->ContainingScrollContainer());
    EXPECT_EQ(scroller2, GetLayoutObjectByElementId("child2")->ContainingScrollContainer());
    EXPECT_EQ(&GetLayoutView(), GetLayoutObjectByElementId("fixed")->ContainingScrollContainer());
    EXPECT_EQ(&GetLayoutView(), GetLayoutObjectByElementId("under-fixed")->ContainingScrollContainer());
    EXPECT_EQ(scroller1, GetLayoutObjectByElementId("absolute")->ContainingScrollContainer());
    EXPECT_EQ(scroller1, GetLayoutObjectByElementId("under-absolute")->ContainingScrollContainer());
}
...
TEST_F(LayoutObjectTest, ScrollOffsetMapping)
{
    ...
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(10, 20), mojom::blink::ScrollType::kProgrammatic);
    UpdateAllLifecyclePhasesForTest();
    ...
    offset = gfx::PointF();
    offset = inner->LocalToAncestorPoint(offset, /*ancestor=*/nullptr, kIgnoreScrollOffset);
    EXPECT_EQ(offset, gfx::PointF(58, 58));
    // And back again:
    offset = inner->AncestorToLocalPoint(/*ancestor=*/nullptr, offset, kIgnoreScrollOffset);
    EXPECT_EQ(offset, gfx::PointF());
}
