...
bool HasLineEvenIfEmpty(LayoutBox* box)
{
    ...
    if (AreNGBlockFlowChildrenInline(block_flow)) {
        return block_flow->HasLineIfEmpty() && InlineNode(block_flow).IsBlockLevel();
    }
    if (const auto* const flow_thread = block_flow->MultiColumnFlowThread()) {
        DCHECK(!flow_thread->ChildrenInline());
        for (const auto* child = flow_thread->FirstChild(); child; child = child->NextSibling()) {
            if (child->IsInline()) {
...
{
    for (const LayoutObject* next = box.NextSibling(); next; next = next->NextSibling()) {
...
inline const LayoutResult* LayoutBlockChild(
    const ConstraintSpace& space, const BreakToken* break_token, const EarlyBreak* early_break, const ColumnSpannerPath* column_spanner_path, BlockNode* node)
{
    ...
    return node->Layout(space, To<BlockBreakToken>(break_token), early_break_in_child, column_spanner_path);
}

inline const LayoutResult* LayoutInflow(const ConstraintSpace& space, const BreakToken* break_token, const EarlyBreak* early_break,
    const ColumnSpannerPath* column_spanner_path, LayoutInputNode* node, InlineChildLayoutContext* context)
{
    if (auto* inline_node = DynamicTo<InlineNode>(node)) {
        return inline_node->Layout(space, break_token, column_spanner_path, context);
    }
    return LayoutBlockChild(space, break_token, early_break, column_spanner_path, To<BlockNode>(node));
}
...
inline bool HasClearancePastAdjoiningFloats(AdjoiningObjectTypes adjoining_object_types, const ComputedStyle& child_style, const ComputedStyle& cb_style)
{
    return ToAdjoiningObjectTypes(child_style.Clear(cb_style)) & adjoining_object_types;
}
...
// ClearanceOffset().
bool ApplyClearance(const ConstraintSpace& constraint_space, LayoutUnit* bfc_block_offset)
{
    if (constraint_space.HasClearanceOffset() && *bfc_block_offset < constraint_space.ClearanceOffset()) {
...
LayoutUnit LogicalFromBfcLineOffset(
    LayoutUnit child_bfc_line_offset, LayoutUnit parent_bfc_line_offset, LayoutUnit child_inline_size, LayoutUnit parent_inline_size, TextDirection direction)
{
    // We need to respect the current text direction to calculate the logical
    // offset correctly.
    LayoutUnit relative_line_offset = child_bfc_line_offset - parent_bfc_line_offset;

    LayoutUnit inline_offset = direction == TextDirection::kLtr ? relative_line_offset : parent_inline_size - relative_line_offset - child_inline_size;

    return inline_offset;
}

LogicalOffset LogicalFromBfcOffsets(
    const BfcOffset& child_bfc_offset, const BfcOffset& parent_bfc_offset, LayoutUnit child_inline_size, LayoutUnit parent_inline_size, TextDirection direction)
{
    LayoutUnit inline_offset
        = LogicalFromBfcLineOffset(child_bfc_offset.line_offset, parent_bfc_offset.line_offset, child_inline_size, parent_inline_size, direction);

    return { inline_offset, child_bfc_offset.block_offset - parent_bfc_offset.block_offset };
}
...
LayoutUnit WebkitTextAlignAndJustifySelfOffset(const ComputedStyle& child_style, const ComputedStyle& style, LayoutUnit available_space,
    const BoxStrut& margins, const ChildInlineSizeFunc& child_inline_size_func)
{
    DCHECK(!child_style.MarginInlineStartUsing(style).IsAuto());
    DCHECK(!child_style.MarginInlineEndUsing(style).IsAuto());

    const StyleSelfAlignmentData alignment_data = child_style.ResolvedJustifySelf({ ItemPosition::kNormal, OverflowAlignment::kDefault }, &style);
    ItemPosition justify_self = alignment_data.GetPosition();
    OverflowAlignment safe = OverflowAlignment::kSafe;
    if (RuntimeEnabledFeatures::LayoutJustifySelfForBlocksEnabled() && justify_self != ItemPosition::kNormal) {
        safe = alignment_data.Overflow();
    } else {
        justify_self = WebkitTextToItemPosition(style.GetTextAlign());
    }
    auto FreeSpace = [&]() -> LayoutUnit {
        const LayoutUnit free_space = available_space - child_inline_size_func() - margins.InlineSum();
        return safe == OverflowAlignment::kSafe ? free_space.ClampNegativeToZero() : free_space;
    };

    auto self_start_end_converter = [&]() -> LogicalToLogical<LayoutUnit> {
        const LayoutUnit free_space = FreeSpace();
        return LogicalToLogical<LayoutUnit>(child_style.GetWritingDirection(), style.GetWritingDirection(),
...
BlockLayoutAlgorithm::BlockLayoutAlgorithm(const LayoutAlgorithmParams& params)
    ...
    , should_text_box_trim_node_start_(params.space.ShouldTextBoxTrimNodeStart())
    , should_text_box_trim_node_end_(params.space.ShouldTextBoxTrimNodeEnd())
    , should_text_box_trim_fragmentainer_start_(params.space.ShouldTextBoxTrimFragmentainerStart())
    , should_text_box_trim_fragmentainer_end_(params.space.ShouldTextBoxTrimFragmentainerEnd())
{
    ...
    replaced_child_percentage_size_
        = CalculateReplacedChildPercentageSize(GetConstraintSpace(), Node(), ChildAvailableSize(), BorderScrollbarPadding(), BorderPadding());
    ...
    if (const BlockNode marker_node = Node().ListMarkerBlockNodeIfListItem()) {
        if (ShouldPlaceUnpositionedListMarker() && !marker_node.ListMarkerOccupiesWholeLine()
    ...
    if (style.TextBoxTrim() != ETextBoxTrim::kNone) [[unlikely]] {
        ...
        // page area.
        if (!GetConstraintSpace().HasBlockFragmentation() || GetConstraintSpace().IsPaginated()) {
            ...
            if (IsBreakInside(GetBreakToken())) {
                should_text_box_trim_fragmentainer_start_ |= should_text_box_trim_node_start_;
            } else {
                should_text_box_trim_fragmentainer_start_ = false;
            }
            ...
            if (!GetConstraintSpace().IsAnonymous() && style.BoxDecorationBreak() != EBoxDecorationBreak::kClone) {
                should_text_box_trim_fragmentainer_start_ &= !style.ShouldTextBoxTrimStart();
...
void BlockLayoutAlgorithm::SetupRelayoutData(const BlockLayoutAlgorithm& previous, RelayoutType relayout_type)
{
    ...
    if (relayout_type == kRelayoutIgnoringLineClamp) {
        ...
        line_clamp_data_.data.lines_until_clamp = line_clamp_data_.initial_lines_until_clamp = previous.line_clamp_data_.data.lines_until_clamp;
    } else if (previous.line_clamp_data_.data.state == LineClampData::kClampByLines) {
        line_clamp_data_.data.state = LineClampData::kClampByLines;
        line_clamp_data_.data.lines_until_clamp = line_clamp_data_.initial_lines_until_clamp = previous.line_clamp_data_.initial_lines_until_clamp;
    } else if (previous.line_clamp_data_.data.state == LineClampData::kDontTruncate) {
        line_clamp_data_.data.state = LineClampData::kDontTruncate;
    }
    ...
    if (relayout_type == kRelayoutForTextBoxTrim) {
        ...
        override_text_box_trim_end_break_token_ = previous.last_non_empty_break_token_;
    } else {
        override_text_box_trim_end_child_ = previous.override_text_box_trim_end_child_;
        override_text_box_trim_end_break_token_ = previous.override_text_box_trim_end_break_token_;
...
MinMaxSizesResult BlockLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput& float_input)
{
    ...
    for (LayoutInputNode child = Node().FirstChild(); child; child = child.NextSibling()) {
        ...
        // affect this width.
        if (child.IsOutOfFlowPositioned() || (child.IsColumnSpanAll() && GetConstraintSpace().IsInColumnBfc())) {
            continue;
        }
        ...
        if (child.IsFloating() || child_is_new_fc) {
            LayoutUnit float_inline_size = float_left_inline_size + float_right_inline_size;
        ...
        MinMaxConstraintSpaceBuilder builder(GetConstraintSpace(), Style(), child, child_is_new_fc);
        ...
        builder.SetReplacedPercentageResolutionBlockSize(replaced_child_percentage_size_.block_size);
        ...
        if (child.IsInline()) {
            ...
            child_result = To<InlineNode>(child).ComputeMinMaxSizes(Style().GetWritingMode(), space, child_float_input);
        } else {
            child_result = ComputeMinAndMaxContentContribution(Style(), To<BlockNode>(child), space, child_float_input);
        }
        DCHECK_LE(child_result.sizes.min_size, child_result.sizes.max_size) << child.ToString();

        // Determine the max inline contribution of the child.
        BoxStrut margins = child.IsInline() ? BoxStrut() : ComputeMarginsFor(space, child_style, GetConstraintSpace());
        ...
        if (child.IsFloating()) {
            ...
            LayoutUnit line_left_inset
                = margin_line_left > LayoutUnit() ? std::max(float_left_inline_size, margin_line_left) : float_left_inline_size + margin_line_left;

            LayoutUnit line_right_inset
                = margin_line_right > LayoutUnit() ? std::max(float_right_inline_size, margin_line_right) : float_right_inline_size + margin_line_right;
        ...
        // "line".
        LayoutUnit min_inline_contribution = child_result.sizes.min_size + margins.InlineSum();
...
LogicalOffset BlockLayoutAlgorithm::CalculateLogicalOffset(
    const LogicalFragment& fragment, LayoutUnit child_bfc_line_offset, const std::optional<LayoutUnit>& child_bfc_block_offset)
{
    ...
    if (child_bfc_block_offset && container_builder_.BfcBlockOffset()) {
        return LogicalFromBfcOffsets({ child_bfc_line_offset, *child_bfc_block_offset }, ContainerBfcOffset(), fragment.InlineSize(), inline_size, direction);
    }

    LayoutUnit inline_offset
        = LogicalFromBfcLineOffset(child_bfc_line_offset, container_builder_.BfcLineOffset(), fragment.InlineSize(), inline_size, direction);
...
NOINLINE const LayoutResult* BlockLayoutAlgorithm::HandleNonsuccessfulLayoutResult(const LayoutResult* result)
{
    ...
    case LayoutResult::kNeedsEarlierBreak: {
        ...
        return RelayoutAndBreakEarlier<BlockLayoutAlgorithm>(*result->GetEarlyBreak());
    }
...
NOINLINE const LayoutResult* BlockLayoutAlgorithm::LayoutInlineChild(const InlineNode& node)
{
    ...
    if (wrap == TextWrapStyle::kPretty) [[unlikely]] {
        ...
        if (!node.IsScoreLineBreakDisabled()) {
            return LayoutWithOptimalInlineChildLayoutContext<kMaxLinesForOptimal>(node);
        }
    } else if (wrap == TextWrapStyle::kBalance) [[unlikely]] {
        UseCounter::Count(node.GetDocument(), WebFeature::kTextWrapBalance);
        if (!node.IsScoreLineBreakDisabled()) {
            return LayoutWithOptimalInlineChildLayoutContext<kMaxLinesForBalance>(node);
        }
    } else {
...
template <wtf_size_t capacity> NOINLINE const LayoutResult* BlockLayoutAlgorithm::LayoutWithOptimalInlineChildLayoutContext(const InlineNode& child)
{
...
NOINLINE const LayoutResult* BlockLayoutAlgorithm::RelayoutWithLineClampBlockSize(int lines_until_clamp)
{
    DCHECK_EQ(line_clamp_data_.data.state, LineClampData::kMeasureLinesUntilBfcOffset);
...
{
    DCHECK_EQ(!!inline_child_layout_context, Node().IsInlineFormattingContextRoot());
    ...
    if (Style().HasAutoStandardLineClamp()) {
        if (!line_clamp_data_.data.IsLineClampContext()) {
            ...
            if (clamp_bfc_offset == kIndefiniteSize) {
                const MinMaxSizes sizes = ComputeInitialMinMaxBlockSizes(constraint_space, Node(), BorderPadding());
                if (sizes.max_size != LayoutUnit::Max()) {
                    clamp_bfc_offset = (sizes.max_size - BorderScrollbarPadding().block_end).ClampNegativeToZero();
                }
            } else {
                clamp_bfc_offset = (BorderScrollbarPadding().block_start + clamp_bfc_offset).ClampNegativeToZero();
            }
            line_clamp_data_.UpdateClampOffsetFromStyle(clamp_bfc_offset, BorderScrollbarPadding().block_start);
        }
        ...
        if (Style().WebkitLineClamp() != 0) {
            UseCounter::Count(Node().GetDocument(), WebFeature::kWebkitLineClampWithoutWebkitBox);
        }
        ...
        if (line_clamp_data_.data.state == LineClampData::kMeasureLinesUntilBfcOffset) {
            MarginStrut end_margin_strut = constraint_space.LineClampEndMarginStrut();
            end_margin_strut.Append(ComputeMarginsForSelf(constraint_space, Style()).block_end,
                /* is_quirky */ false);
            ...
            line_clamp_data_.data.clamp_bfc_offset
                -= BorderScrollbarPadding().block_end + (end_margin_strut.Sum() - constraint_space.LineClampEndMarginStrut().Sum());
    ...
    PreviousInflowPosition previous_inflow_position
        = { LayoutUnit(), constraint_space.GetMarginStrut(), is_resuming_ ? LayoutUnit() : container_builder_.Padding().block_start,
              /* self_collapsing_child_had_clearance */ false };

    if (GetBreakToken()) {
        if (IsBreakInside(GetBreakToken()) && !GetBreakToken()->IsForcedBreak() && !GetBreakToken()->IsCausedByColumnSpanner()) {
    ...
    if (content_edge || is_resuming_ || constraint_space.IsNewFormattingContext()) {
        bool discard_subsequent_margins = previous_inflow_position.margin_strut.discard_margins && !content_edge;
    ...
    for (entry = child_iterator.NextChild(); LayoutInputNode child = entry.node; entry = child_iterator.NextChild(previous_inline_break_token)) {
        ...
        if (child.IsOutOfFlowPositioned()) {
            ...
            // followed by an OOF.
            DCHECK(!child_break_token || (child_break_token->IsBlockType() && To<BlockBreakToken>(child_break_token)->IsBreakBefore()));
            HandleOutOfFlowPositioned(previous_inflow_position, To<BlockNode>(child));
        } else if (child.IsFloating()) {
            HandleFloat(previous_inflow_position, To<BlockNode>(child), To<BlockBreakToken>(child_break_token));
        ...
        } else if (child.IsColumnSpanAll() && constraint_space.IsInColumnBfc() && constraint_space.HasBlockFragmentation()) {
            ...
            if (constraint_space.IsPastBreak() || container_builder_.HasInsertedChildBreak()) {
            ...
            const auto* child_spanner_path = MakeGarbageCollected<ColumnSpannerPath>(To<BlockNode>(child));
            const auto* container_spanner_path = MakeGarbageCollected<ColumnSpannerPath>(Node(), child_spanner_path);
            ...
            container_builder_.SetIsEmptySpannerParent(container_builder_.Children().empty() && is_resuming_);
            ...
            // children.
            for (entry = child_iterator.NextChild(); LayoutInputNode sibling = entry.node; entry = child_iterator.NextChild()) {
                DCHECK(!entry.token);
            ...
            if (early_break_ && IsEarlyBreakTarget(*early_break_, container_builder_, child)) [[unlikely]] {
                if (!ResolveBfcBlockOffset(&previous_inflow_position)) {
            ...
            if (child.CreatesNewFormattingContext()) {
                status = HandleNewFormattingContext(child, To<BlockBreakToken>(child_break_token), &previous_inflow_position);
                previous_inline_break_token = nullptr;
            } else {
                status = HandleInflow(child, child_break_token, &previous_inflow_position, inline_child_layout_context, &previous_inline_break_token);
            }
    ...
    if (auto* inline_token = DynamicTo<InlineBreakToken>(entry.token)) {
        ...
        if (!container_builder_.FoundColumnSpanner() && !container_builder_.ShouldForceSameFragmentationFlow()) {
...
const LayoutResult* BlockLayoutAlgorithm::FinishLayout(PreviousInflowPosition* previous_inflow_position, InlineChildLayoutContext* inline_child_layout_context)
{
    const auto& constraint_space = GetConstraintSpace();
    if (constraint_space.IsNewFormattingContext() && line_clamp_data_.ShouldRelayoutWithNoForcedTruncate()) [[unlikely]] {
    ...
    if (ShouldTextBoxTrimEnd() && last_non_empty_inflow_child_ && !line_clamp_data_.previous_inflow_position_when_clamped.has_value()) [[unlikely]] {
    ...
    // point.
    if (RuntimeEnabledFeatures::CSSLineClampEnabled() && line_clamp_data_.previous_inflow_position_when_clamped.has_value()) [[unlikely]] {
        previous_inflow_position = &*line_clamp_data_.previous_inflow_position_when_clamped;
    }
    ...
    if (container_builder_.HasSeenAllChildren() && HasLineEvenIfEmpty(Node().GetLayoutBox())) {
        intrinsic_block_size_ = std::max(intrinsic_block_size_, BorderScrollbarPadding().block_start + Node().EmptyLineBlockSize(GetBreakToken()));
        if (container_builder_.IsInitialColumnBalancingPass()) {
            container_builder_.PropagateTallestUnbreakableBlockSize(intrinsic_block_size_);
        }
        ...
        const LayoutBlock* const layout_block = To<LayoutBlock>(Node().GetLayoutBox());
    ...
    if (previous_inflow_position->block_end_annotation_space < LayoutUnit()) {
        const LayoutUnit annotation_overflow = -previous_inflow_position->block_end_annotation_space;
        previous_inflow_position->logical_block_offset -= std::min(container_builder_.Padding().block_end, annotation_overflow);
    }
    ...
    if (!RuntimeEnabledFeatures::CSSLineClampEnabled() && line_clamp_data_.previous_inflow_position_when_clamped) {
        DCHECK(container_builder_.BfcBlockOffset());
        intrinsic_block_size_ = line_clamp_data_.previous_inflow_position_when_clamped->logical_block_offset + BorderScrollbarPadding().block_end;
        end_margin_strut = MarginStrut();
    } else if (BorderScrollbarPadding().block_end || previous_inflow_position->self_collapsing_child_had_clearance
        ...
        // line-clamp.
        if (constraint_space.IsNewFormattingContext()) {
            LayoutUnit clearance = GetExclusionSpace().NonHiddenClearanceOffsetIncludingInitialLetter();
#ifdef DCHECK_ALWAYS_ON
            if (!RuntimeEnabledFeatures::CSSLineClampEnabled() || !line_clamp_data_.previous_inflow_position_when_clamped) {
                DCHECK_EQ(clearance, GetExclusionSpace().ClearanceOffsetIncludingInitialLetter(EClear::kBoth));
            }
#endif
        ...
        if (!container_builder_.BfcBlockOffset()) {
            ...
            // margin.
            LayoutUnit margin_strut_sum = node_.IsQuirkyContainer() ? end_margin_strut.QuirkyContainerSum() : end_margin_strut.Sum();

            if (constraint_space.HasKnownFragmentainerBlockSize()) {
                LayoutUnit new_margin_strut_sum
                    = AdjustedMarginAfterFinalChildFragment(container_builder_, previous_inflow_position->logical_block_offset, margin_strut_sum);
            ...
            intrinsic_block_size_ = std::max(intrinsic_block_size_, previous_inflow_position->logical_block_offset + margin_strut_sum);
        }
        ...
        intrinsic_block_size_ = std::max(intrinsic_block_size_, previous_inflow_position->logical_block_offset);
    }

    LayoutUnit unconstrained_intrinsic_block_size = intrinsic_block_size_;
    intrinsic_block_size_ = ClampIntrinsicBlockSize(
        constraint_space, Node(), GetBreakToken(), BorderScrollbarPadding(), intrinsic_block_size_, CalculateQuirkyBodyMarginBlockSum(end_margin_strut));
    ...
    if (GetBreakToken() && !container_builder_.IsFragmentainerBoxType()) [[unlikely]] {
        previously_consumed_block_size = GetBreakToken()->ConsumedBlockSize();
    }

    // Recompute the block-axis size now that we know our content size.
    border_box_size.block_size = ComputeBlockSizeForFragment(
        constraint_space, Node(), BorderPadding(), previously_consumed_block_size + intrinsic_block_size_, border_box_size.inline_size);
    ...
    if (!container_builder_.BfcBlockOffset() && (border_box_size.block_size || GetBreakToken() || container_builder_.FoundColumnSpanner())) {
        if (!ResolveBfcBlockOffset(previous_inflow_position))
            return container_builder_.Abort(LayoutResult::kBfcBlockOffsetResolved);
        DCHECK(container_builder_.BfcBlockOffset());
    }
    ...
    if (container_builder_.BfcBlockOffset()) {
        ...
        const LayoutUnit initial_block_size
            = ComputeInitialBlockSizeForFragment(constraint_space, Node(), BorderPadding(), kIndefiniteSize, border_box_size.inline_size);
        if (border_box_size.block_size != intrinsic_block_size_ || initial_block_size != kIndefiniteSize) {
            end_margin_strut = MarginStrut();
        }
    }
    ...
    // boxes.
    if (container_builder_.GetUnpositionedListMarker() && ShouldPlaceUnpositionedListMarker() &&
    ...
    if (container_builder_.BfcBlockOffset()) {
        ...
        if (constraint_space.ForcedBfcBlockOffset()) {
            container_builder_.SetBfcBlockOffset(*constraint_space.ForcedBfcBlockOffset());
    ...
    if (constraint_space.IsTableCell()) {
        ...
        AlignBlockContent(Style(), GetBreakToken(), unconstrained_intrinsic_block_size, container_builder_);
    }
    ...
    if (constraint_space.IsNewFormattingContext()) {
        ...
        container_builder_.SetLinesUntilClamp(line_clamp_data_.data.LinesUntilClamp(/*show_measured_lines*/ true));
    }
...
bool BlockLayoutAlgorithm::TryReuseFragmentsFromCache(
    InlineNode inline_node, PreviousInflowPosition* previous_inflow_position, const InlineBreakToken** inline_break_token_out)
{
    ...
    const auto& previous_fragment = To<PhysicalBoxFragment>(previous_result_->GetPhysicalFragment());
    ...
    const FragmentItem* end_item = previous_items->EndOfReusableItems(previous_fragment);
    DCHECK(end_item);
    if (!end_item || end_item == &previous_items->front())
        return false;

    wtf_size_t max_lines = 0;
    if (std::optional<int> lines_until_clamp = line_clamp_data_.LinesUntilClamp()) {
    ...
    const auto result = items_builder->AddPreviousItems(previous_fragment, *previous_items, &container_builder_, end_item, max_lines);
    ...
    } else if (line_clamp_data_.data.state == LineClampData::kMeasureLinesUntilBfcOffset) {
        line_clamp_data_.data.lines_until_clamp += result.line_count;
    }

    // |AddPreviousItems| may have added more than one lines. Propagate baselines
    // from them.
    //base::make_span(children).subspan(children_before);
    auto children_before_sub = base::span(children.data(), children.size()).subspan(children_before);
    for (const auto& child : children_before_sub) {
...
void BlockLayoutAlgorithm::HandleOutOfFlowPositioned(const PreviousInflowPosition& previous_inflow_position, BlockNode child)
{
    if (GetConstraintSpace().HasBlockFragmentation()) {
        ...
        EBreakBetween break_between = container_builder_.JoinedBreakBetweenValue(EBreakBetween::kAuto);
    ...
    LogicalOffset static_offset = { BorderScrollbarPadding().inline_start, previous_inflow_position.logical_block_offset };
    ...
    if (child.Style().IsOriginalDisplayInlineType()) {
        ...
        LayoutUnit origin_bfc_block_offset
            = container_builder_.BfcBlockOffset().value_or(GetConstraintSpace().ExpectedBfcBlockOffset()) + static_offset.block_offset;

        BfcOffset origin_bfc_offset
            = { GetConstraintSpace().GetBfcOffset().line_offset + BorderScrollbarPadding().LineLeft(Style().Direction()), origin_bfc_block_offset };

        static_offset.inline_offset
            += CalculateOutOfFlowStaticInlineLevelOffset(Style(), origin_bfc_offset, GetExclusionSpace(), ChildAvailableSize().inline_size);
    }

    container_builder_.AddOutOfFlowChildCandidate(
        child, static_offset, LogicalStaticPosition::kInlineStart, LogicalStaticPosition::kBlockStart, line_clamp_data_.ShouldHideForPaint());
}

void BlockLayoutAlgorithm::HandleFloat(const PreviousInflowPosition& previous_inflow_position, BlockNode child, const BlockBreakToken* child_break_token)
{
    // If we're resuming layout, we must always know our position in the BFC.
    DCHECK(!IsBreakInside(child_break_token) || container_builder_.BfcBlockOffset());
    const auto& constraint_space = GetConstraintSpace();

    // If we don't have a BFC block-offset yet, the "expected" BFC block-offset
    // is used to optimistically place floats.
    BfcOffset origin_bfc_offset = { constraint_space.GetBfcOffset().line_offset + BorderScrollbarPadding().LineLeft(constraint_space.Direction()),
        container_builder_.BfcBlockOffset() ? NextBorderEdge(previous_inflow_position) : constraint_space.ExpectedBfcBlockOffset() };
    ...
    if (child_break_token) {
        ...
        origin_bfc_offset.block_offset += child_break_token->MonolithicOverflow() - GetBreakToken()->MonolithicOverflow();
    }
    ...
    UnpositionedFloat unpositioned_float(child, child_break_token, ChildAvailableSize(), child_percentage_size_, replaced_child_percentage_size_,
        origin_bfc_offset, constraint_space, Style(), FragmentainerCapacityForChildren(), FragmentainerOffsetForChildren(),
        line_clamp_data_.ShouldHideForPaint());

    if (!container_builder_.BfcBlockOffset()) {
        container_builder_.AddAdjoiningObjectTypes(unpositioned_float.IsLineLeft(constraint_space.Direction()) ? kAdjoiningFloatLeft : kAdjoiningFloatRight);
    ...
    PositionedFloat positioned_float = PositionFloat(&unpositioned_float, &GetExclusionSpace());

    if (positioned_float.minimum_space_shortage > LayoutUnit()) {
        container_builder_.PropagateSpaceShortage(positioned_float.minimum_space_shortage);
    }
    ...
    const PhysicalFragment& physical_fragment = positioned_float.layout_result->GetPhysicalFragment();
    LayoutUnit float_inline_size = LogicalFragment(constraint_space.GetWritingDirection(), physical_fragment).InlineSize();

    BfcOffset bfc_offset
        = { constraint_space.GetBfcOffset().line_offset, container_builder_.BfcBlockOffset().value_or(constraint_space.ExpectedBfcBlockOffset()) };

    LogicalOffset logical_offset
        = LogicalFromBfcOffsets(positioned_float.bfc_offset, bfc_offset, float_inline_size, container_builder_.InlineSize(), constraint_space.Direction());
...
LayoutResult::EStatus BlockLayoutAlgorithm::HandleNewFormattingContext(
    LayoutInputNode child, const BlockBreakToken* child_break_token, PreviousInflowPosition* previous_inflow_position)
{
    DCHECK(child);
    ...
    InflowChildData child_data = ComputeChildData(*previous_inflow_position, child, child_break_token,
        /* is_new_fc */ true);

    LayoutUnit child_origin_line_offset = constraint_space.GetBfcOffset().line_offset + BorderScrollbarPadding().LineLeft(direction);
    ...
    adjoining_margin_strut.Append(child_data.margins.block_start, child_style.HasMarginBlockStartQuirk());
    LayoutUnit adjoining_bfc_offset_estimate = child_data.bfc_offset_estimate.block_offset + adjoining_margin_strut.Sum();
    LayoutUnit non_adjoining_bfc_offset_estimate = child_data.bfc_offset_estimate.block_offset + previous_inflow_position->margin_strut.Sum();
    ...
    if (!container_builder_.BfcBlockOffset()) {
        has_adjoining_floats = container_builder_.GetAdjoiningObjectTypes() & kAdjoiningFloatBoth;
        ...
        bool has_clearance_past_adjoining_floats = constraint_space.AncestorHasClearancePastAdjoiningFloats()
            || HasClearancePastAdjoiningFloats(container_builder_.GetAdjoiningObjectTypes(), child_style, Style());
        ...
        if (has_clearance_past_adjoining_floats) {
            ...
            DCHECK(child_bfc_offset_estimate == adjoining_bfc_offset_estimate || child_bfc_offset_estimate == non_adjoining_bfc_offset_estimate
            ...
            child_margin_got_separated = child_bfc_offset_estimate != adjoining_bfc_offset_estimate;
        }
        ...
        if (!child_margin_got_separated) {
            SetSubtreeModifiedMarginStrutIfNeeded(&child_style.MarginBlockStartUsing(Style()));
        }

        if (!ResolveBfcBlockOffset(previous_inflow_position, child_bfc_offset_estimate)) {
    ...
    bool abort_if_cleared = child_data.margins.block_start != LayoutUnit() && !child_margin_got_separated && child_determined_bfc_offset;
    BfcOffset child_bfc_offset;
    BoxStrut resolved_margins;
    const LayoutResult* layout_result = LayoutNewFormattingContext(
        child, child_break_token, child_data, { child_origin_line_offset, child_bfc_offset_estimate }, abort_if_cleared, &child_bfc_offset, &resolved_margins);
    ...
    if (!layout_result) {
        ...
        if (child_determined_bfc_offset) {
            ...
            if ((bfc_offset_already_resolved || has_adjoining_floats) && old_offset != *container_builder_.BfcBlockOffset()) {
        ...
        layout_result = LayoutNewFormattingContext(child, child_break_token, child_data, { child_origin_line_offset, child_bfc_offset_estimate },
            /* abort_if_cleared */ false, &child_bfc_offset, &resolved_margins);
    }

    if (constraint_space.HasBlockFragmentation()) {
        bool has_container_separation
            = has_break_opportunity_before_next_child_ || child_bfc_offset.block_offset > child_bfc_offset_estimate || layout_result->IsPushedByFloats();
        BreakStatus break_status
            = BreakBeforeChildIfNeeded(child, *layout_result, previous_inflow_position, child_bfc_offset.block_offset, has_container_separation);
    ...
    LogicalFragment fragment(constraint_space.GetWritingDirection(), physical_fragment);

    LogicalOffset logical_offset
        = LogicalFromBfcOffsets(child_bfc_offset, ContainerBfcOffset(), fragment.InlineSize(), container_builder_.InlineSize(), constraint_space.Direction());

    if (!PositionOrPropagateListMarker(*layout_result, &logical_offset, previous_inflow_position))
        return LayoutResult::kBfcBlockOffsetResolved;

    PropagateBaselineFromBlockChild(physical_fragment, resolved_margins, logical_offset.block_offset);

    container_builder_.AddResult(*layout_result, logical_offset, resolved_margins);

    if (!child_break_token || !child_break_token->IsInParallelFlow()) {
        *previous_inflow_position
            = ComputeInflowPosition(*previous_inflow_position, child, child_data, child_bfc_offset.block_offset, logical_offset, *layout_result, fragment,
                /* self_collapsing_child_had_clearance */ false);
    }

    // Update line-clamp data, and abort if needed
    if (!line_clamp_data_.UpdateAfterLayout(layout_result, *container_builder_.BfcBlockOffset(), *previous_inflow_position, Padding().block_end)) {
        container_builder_.SetLinesUntilClamp(line_clamp_data_.LinesUntilClamp(/*show_measured_lines*/ true));
        return LayoutResult::kNeedsLineClampRelayout;
    }

    if (constraint_space.HasBlockFragmentation() && !has_break_opportunity_before_next_child_) {
        has_break_opportunity_before_next_child_ = HasBreakOpportunityBeforeNextChild(physical_fragment, child_break_token);
    }

    return LayoutResult::kSuccess;
}

const LayoutResult* BlockLayoutAlgorithm::LayoutNewFormattingContext(LayoutInputNode child, const BlockBreakToken* child_break_token,
    const InflowChildData& child_data, BfcOffset origin_offset, bool abort_if_cleared, BfcOffset* out_child_bfc_offset, BoxStrut* out_resolved_margins)
{
    ...
    if (!IsBreakInside(child_break_token)) {
        ...
        AdjustToClearance(GetExclusionSpace().ClearanceOffsetIncludingInitialLetter(child_style.Clear(style)), &origin_offset);
    }
    DCHECK(container_builder_.BfcBlockOffset());

    LayoutOpportunityVector opportunities = GetExclusionSpace().AllLayoutOpportunities(origin_offset, ChildAvailableSize().inline_size);
    ...
    for (const auto& opportunity : opportunities) {
        ...
        bool has_floats_on_line_left = opportunity.rect.LineStartOffset() != origin_offset.line_offset;
        bool has_floats_on_line_right = opportunity.rect.LineEndOffset() != (origin_offset.line_offset + ChildAvailableSize().inline_size);
        bool can_expand_outside_opportunity = !has_floats_on_line_left && !has_floats_on_line_right;

        const LayoutUnit line_left_margin = child_data.margins.LineLeft(direction);
        const LayoutUnit line_right_margin = child_data.margins.LineRight(direction);
        ...
        if (can_expand_outside_opportunity) {
            ...
            DCHECK_EQ(line_right_offset, origin_offset.line_offset + ChildAvailableSize().inline_size);
            ...
            line_left_offset = std::max(line_left_offset, origin_offset.line_offset + line_left_margin.ClampNegativeToZero());
            line_right_offset
                = std::min(line_right_offset, origin_offset.line_offset + ChildAvailableSize().inline_size - line_right_margin.ClampNegativeToZero());
        }
        LayoutUnit opportunity_size = (line_right_offset - line_left_offset).ClampNegativeToZero();
        ...
        LayoutUnit child_available_inline_size = (opportunity_size + child_data.margins.InlineSum()).ClampNegativeToZero();

        ConstraintSpace child_space
            = CreateConstraintSpaceForChild(child, child_break_token, child_data, { child_available_inline_size, ChildAvailableSize().block_size },
        ...
        const LayoutResult* layout_result = LayoutBlockChild(child_space, child_break_token, early_break_,
        ...
        LogicalFragment fragment(writing_direction, layout_result->GetPhysicalFragment());
        ...
        if (child.IsListMarker()) {
            ...
            if (!marker_fragment.Children().empty()) {
                marker_inline_size = LogicalFragment(writing_direction, *marker_fragment.Children().front()).InlineSize();
            }
            auto_margins.inline_start = UnpositionedListMarker(To<BlockNode>(child)).InlineOffset(marker_inline_size);
            auto_margins.inline_end = opportunity.rect.InlineSize() - fragment.InlineSize() - auto_margins.inline_start;
        } else {
            if (child_style.MarginInlineStartUsing(style).IsAuto() || child_style.MarginInlineEndUsing(style).IsAuto()) {
                has_auto_margins = true;
                ResolveInlineAutoMargins(child_style, style, child_available_inline_size, fragment.InlineSize(), &auto_margins);
            } else {
                // Handle -webkit- values for text-align.
                text_align_offset = WebkitTextAlignAndJustifySelfOffset(
                    child_style, style, opportunity.rect.InlineSize(), child_data.margins, [&]() { return fragment.InlineSize(); });
            }
        }
        ...
        // difference.
        BfcOffset child_bfc_offset = { LayoutUnit(), opportunity.rect.BlockStartOffset() };
        if (direction == TextDirection::kLtr) {
            LayoutUnit auto_margin_line_left = auto_margins.LineLeft(direction) - line_left_margin;
            child_bfc_offset.line_offset = line_left_offset + auto_margin_line_left + text_align_offset;
        } else {
            LayoutUnit auto_margin_line_right = auto_margins.LineRight(direction) - line_right_margin;
            child_bfc_offset.line_offset = line_right_offset - text_align_offset - auto_margin_line_right - fragment.InlineSize();
        }

        // Check if we'll intersect any floats on our line-left/line-right.
        if (has_floats_on_line_left && child_bfc_offset.line_offset < opportunity.rect.LineStartOffset())
            continue;
        if (has_floats_on_line_right && child_bfc_offset.line_offset + fragment.InlineSize() > opportunity.rect.LineEndOffset())
            continue;

        // If we can't expand outside our opportunity, check if we fit in the
        // inline direction.
        if (!can_expand_outside_opportunity && fragment.InlineSize() > opportunity.rect.InlineSize())
            continue;
        ...
        if (has_auto_margins) {
            LayoutUnit inline_offset = LogicalFromBfcLineOffset(child_bfc_offset.line_offset, container_builder_.BfcLineOffset(), fragment.InlineSize(),
            ...
            if (child_style.MarginInlineEndUsing(style).IsAuto()) {
                resolved_margins.inline_end = ChildAvailableSize().inline_size - inline_offset - fragment.InlineSize();
            }
        }
...
LayoutResult::EStatus BlockLayoutAlgorithm::HandleInflow(LayoutInputNode child, const BreakToken* child_break_token,
    PreviousInflowPosition* previous_inflow_position, InlineChildLayoutContext* inline_child_layout_context,
    const InlineBreakToken** previous_inline_break_token)
{
    ...
    if (child_inline_node) {
        ...
        if (!abort_when_bfc_block_offset_updated_ && !child_break_token && previous_result_) {
            DCHECK(!*previous_inline_break_token);
            if (TryReuseFragmentsFromCache(*child_inline_node, previous_inflow_position, previous_inline_break_token))
                return LayoutResult::kSuccess;
        }
    }

    bool has_clearance_past_adjoining_floats = !container_builder_.BfcBlockOffset() && child.IsBlock()
        && HasClearancePastAdjoiningFloats(container_builder_.GetAdjoiningObjectTypes(), child.Style(), Style());
    ...
    if (has_clearance_past_adjoining_floats) {
        ...
        if (has_clearance_past_adjoining_floats) {
            forced_bfc_block_offset = GetExclusionSpace().ClearanceOffset(child.Style().Clear(Style()));
    ...
    ConstraintSpace child_space = CreateConstraintSpaceForChild(child, child_break_token, child_data, ChildAvailableSize(),
        /* is_new_fc */ false, forced_bfc_block_offset, has_clearance_past_adjoining_floats, previous_inflow_position->block_end_annotation_space);
    const LayoutResult* layout_result = LayoutInflow(child_space, child_break_token, early_break_, column_spanner_path_, &child, inline_child_layout_context);
    ...
    return FinishInflow(child, child_break_token, child_space, has_clearance_past_adjoining_floats, std::move(layout_result), &child_data,
        previous_inflow_position, inline_child_layout_context, previous_inline_break_token);
}

LayoutResult::EStatus BlockLayoutAlgorithm::FinishInflow(LayoutInputNode child, const BreakToken* child_break_token, const ConstraintSpace& child_space,
    bool has_clearance_past_adjoining_floats, const LayoutResult* layout_result, InflowChildData* child_data, PreviousInflowPosition* previous_inflow_position,
    InlineChildLayoutContext* inline_child_layout_context, const InlineBreakToken** previous_inline_break_token)
{
    ...
    std::optional<LayoutUnit> child_bfc_block_offset = layout_result->BfcBlockOffset();
    ...
    bool normal_child_had_clearance = layout_result->IsPushedByFloats() && !is_self_collapsing;
    ...
    if (layout_result->Status() == LayoutResult::kBfcBlockOffsetResolved && !container_builder_.BfcBlockOffset()) {
        ...
        if (normal_child_had_clearance) {
            ...
            if (GetConstraintSpace().ClearanceOffset() == child_space.ClearanceOffset()) {
                container_builder_.SetIsPushedByFloats();
            } else {
    ...
    bool self_collapsing_child_had_clearance = is_self_collapsing && has_clearance_past_adjoining_floats;
    ...
    if (!child_bfc_block_offset) {
        ...
        if (child_space.HasClearanceOffset() && child.Style().HasClear()) {
            ...
            // clearance before it.
            LayoutUnit child_block_offset_estimate = BfcBlockOffset() + layout_result->EndMarginStrut().Sum();
            if (child_block_offset_estimate < child_space.ClearanceOffset())
                self_collapsing_child_had_clearance = true;
        }
    }

    bool child_had_clearance = self_collapsing_child_had_clearance || normal_child_had_clearance;
    ...
    if (!child_bfc_block_offset) {
        ...
        if (container_builder_.BfcBlockOffset() && layout_result->Status() == LayoutResult::kSuccess) {
            // Since we know our own BFC block-offset, though, we can calculate that
            // of the child as well.
            child_bfc_block_offset = PositionSelfCollapsingChildWithParentBfc(child, child_space, *child_data, *layout_result);

            // We may need to relayout this child if it had any (adjoining) objects
            // which were positioned in the incorrect place.
            if (layout_result->GetPhysicalFragment().HasAdjoiningObjectDescendants() && *child_bfc_block_offset != child_space.ExpectedBfcBlockOffset()) {
        ...
        // resolved.
        if (!ResolveBfcBlockOffset(previous_inflow_position, *child_bfc_block_offset))
            return LayoutResult::kBfcBlockOffsetResolved;
    }
    ...
    if (self_collapsing_child_had_clearance) {
        ...
        margin_strut.Append(child_data->margins.block_start, child.Style().HasMarginBlockStartQuirk());
    ...
    if ((layout_result->Status() == LayoutResult::kBfcBlockOffsetResolved || self_collapsing_child_needs_relayout) && child_bfc_block_offset) {
        // Assert that any clearance previously detected isn't lost.
        DCHECK(!child_data->is_pushed_by_floats || layout_result->IsPushedByFloats());
        ...
        ConstraintSpace new_child_space = CreateConstraintSpaceForChild(child, child_break_token, *child_data, ChildAvailableSize(),
            /* is_new_fc */ false, child_bfc_block_offset);
        layout_result = LayoutInflow(new_child_space, child_break_token, early_break_, column_spanner_path_, &child, inline_child_layout_context);
        ...
        if (layout_result->Status() == LayoutResult::kBfcBlockOffsetResolved) {
            ...
            new_child_space = CreateConstraintSpaceForChild(child, child_break_token, *child_data, ChildAvailableSize(),
                /* is_new_fc */ false, child_bfc_block_offset);
            layout_result = LayoutInflow(new_child_space, child_break_token, early_break_, column_spanner_path_, &child, inline_child_layout_context);
        }
    ...
    const std::optional<LayoutUnit> line_box_bfc_block_offset = layout_result->LineBoxBfcBlockOffset();
    ...
    if (GetConstraintSpace().HasBlockFragmentation()) {
        ...
        // below.
        bool consider_breaking_before = container_builder_.BfcBlockOffset() && child_bfc_block_offset
            && (!child.IsInline() || !child_break_token || !To<InlineBreakToken>(child_break_token)->IsInParallelBlockFlow());

        if (consider_breaking_before) {
            bool is_line_box_pushed_by_floats = line_box_bfc_block_offset && *line_box_bfc_block_offset > *child_bfc_block_offset;
            ...
            // block-start margins).
            bool has_container_separation = has_break_opportunity_before_next_child_
                || (!container_builder_.IsPushedByFloats() && (layout_result->IsPushedByFloats() || is_line_box_pushed_by_floats));
            ...
            const LayoutResult& layout_result_to_use = container_builder_.LayoutResultForPropagation(*layout_result);

            BreakStatus break_status = BreakBeforeChildIfNeeded(
                child, layout_result_to_use, previous_inflow_position, line_box_bfc_block_offset.value_or(*child_bfc_block_offset), has_container_separation);
            ...
            if (break_status == BreakStatus::kBrokeBefore) {
                ...
                if (should_text_box_trim_fragmentainer_end_ && child.IsInline() && !child_space.ShouldForceTextBoxTrimEnd()) {
        ...
        if (inline_child_layout_context) {
            for (auto token : inline_child_layout_context->ParallelFlowBreakTokens()) {
    ...
    container_builder_.SetAdjoiningObjectTypes(layout_result->GetAdjoiningObjectTypes());
    ...
    // has been positioned.
    if (!container_builder_.BfcBlockOffset()) {
        abort_when_bfc_block_offset_updated_ |= layout_result->GetAdjoiningObjectTypes();
    ...
    const auto& physical_fragment = layout_result->GetPhysicalFragment();
    LogicalFragment fragment(GetConstraintSpace().GetWritingDirection(), physical_fragment);

    if (line_box_bfc_block_offset)
        child_bfc_block_offset = line_box_bfc_block_offset;

    LogicalOffset logical_offset = CalculateLogicalOffset(fragment, layout_result->BfcLineOffset(), child_bfc_block_offset);
    ...
    if (physical_fragment.IsLineBox()) {
        PropagateBaselineFromLineBox(physical_fragment, logical_offset.block_offset);
    } else {
        PropagateBaselineFromBlockChild(physical_fragment, child_data->margins, logical_offset.block_offset);
    }

    if (IsA<BlockNode>(child)) {
        container_builder_.AddResult(*layout_result, logical_offset, child_data->margins);
    } else {
        container_builder_.AddResult(*layout_result, logical_offset);
    }

    if (!child_break_token || !child_break_token->IsInParallelFlow()) {
        *previous_inflow_position = ComputeInflowPosition(*previous_inflow_position, child, *child_data, child_bfc_block_offset, logical_offset, *layout_result,
            fragment, self_collapsing_child_had_clearance);
    }

    const InlineBreakToken* outgoing_inline_break_token = nullptr;
    if (child.IsInline()) {
        outgoing_inline_break_token = To<InlineBreakToken>(physical_fragment.GetBreakToken());
    }
    *previous_inline_break_token = outgoing_inline_break_token;
    ...
    if (auto bfc_block_offset = container_builder_.BfcBlockOffset()) {
        if (!line_clamp_data_.UpdateAfterLayout(layout_result, *bfc_block_offset, *previous_inflow_position, Padding().block_end)) {
            container_builder_.SetLinesUntilClamp(line_clamp_data_.LinesUntilClamp(/*show_measured_lines*/ true));
            return LayoutResult::kNeedsLineClampRelayout;
        }
    }
    ...
    if (ShouldTextBoxTrim()) [[unlikely]] {
        ...
        if (should_text_box_trim_node_start_) {
            if (!child.IsInline() || !outgoing_inline_break_token || outgoing_inline_break_token->IsPastFirstFormattedLine()) {
                should_text_box_trim_node_start_ = false;
            }
        }
        if (should_text_box_trim_node_end_) {
            if (line_clamp_data_.data.state == LineClampData::kMeasureLinesUntilBfcOffset && layout_result->TrimBlockEndBy()
                ...
                // position.
                previous_inflow_position->logical_block_offset += *layout_result->TrimBlockEndBy();
            } else if (layout_result->IsBlockEndTrimmableLine() || (child.IsBlock() && IsLastInflowChild(*child.GetLayoutBox()))) {
                ClearShouldTextBoxTrimEnd();
            } else if (!layout_result->IsSelfCollapsing() && child.IsInline() && !override_text_box_trim_end_child_) {
    ...
    if (GetConstraintSpace().HasBlockFragmentation() && !has_break_opportunity_before_next_child_) {
        has_break_opportunity_before_next_child_ = HasBreakOpportunityBeforeNextChild(physical_fragment, child_break_token);
    }
...
InflowChildData BlockLayoutAlgorithm::ComputeChildData(
    const PreviousInflowPosition& previous_inflow_position, LayoutInputNode child, const BreakToken* child_break_token, bool is_new_fc)
{
    DCHECK(child);
    DCHECK(!child.IsFloating());
    ...
    if (child_block_break_token) [[unlikely]] {
        ...
        if (child_block_break_token->MonolithicOverflow() && (Node().IsPaginatedRoot() || !GetBreakToken()->MonolithicOverflow())) {
    ...
    TextDirection direction = GetConstraintSpace().Direction();
    BfcOffset child_bfc_offset = { GetConstraintSpace().GetBfcOffset().line_offset + BorderScrollbarPadding().LineLeft(direction) + additional_line_offset
...
PreviousInflowPosition BlockLayoutAlgorithm::ComputeInflowPosition(const PreviousInflowPosition& previous_inflow_position, const LayoutInputNode child,
    const InflowChildData& child_data, const std::optional<LayoutUnit>& child_bfc_block_offset, const LogicalOffset& logical_offset,
    const LayoutResult& layout_result, const LogicalFragment& fragment, bool self_collapsing_child_had_clearance)
{
    ...
    if (is_self_collapsing) {
        ...
        if (self_collapsing_child_had_clearance) {
            ...
            LayoutUnit margin_before_clearance = previous_inflow_position.margin_strut.Sum();
            logical_block_offset += margin_before_clearance;

            // Calculate and apply actual clearance.
            LayoutUnit clearance = *child_bfc_block_offset - layout_result.EndMarginStrut().Sum() - NextBorderEdge(previous_inflow_position);
        ...
        if (trim_block_end_by) {
            ...
            logical_block_offset += std::max(layout_result.AnnotationOverflow(), clearance_after_line.value_or(LayoutUnit()));
        }
    }
    ...
    bool is_quirky = (is_self_collapsing && child.Style().HasMarginBlockStartQuirk()) || child.Style().HasMarginBlockEndQuirk();
    ...
    if (GetConstraintSpace().HasBlockFragmentation()) [[unlikely]] {
        ...
        if (const auto* physical_fragment = DynamicTo<PhysicalBoxFragment>(&layout_result.GetPhysicalFragment())) {
    ...
    bool self_or_sibling_self_collapsing_child_had_clearance
        = self_collapsing_child_had_clearance || (previous_inflow_position.self_collapsing_child_had_clearance && is_self_collapsing);
    ...
    if (!is_self_collapsing && !trim_block_end_by) {
        ...
        if (layout_result.AnnotationOverflow() > LayoutUnit()) {
            ...
            annotation_space = -std::max(LayoutUnit(), layout_result.AnnotationOverflow() - clearance_after_line.value_or(LayoutUnit()));
        }
    }

    return { logical_block_offset, margin_strut, annotation_space, self_or_sibling_self_collapsing_child_had_clearance };
}

LayoutUnit BlockLayoutAlgorithm::PositionSelfCollapsingChildWithParentBfc(
    const LayoutInputNode& child, const ConstraintSpace& child_space, const InflowChildData& child_data, const LayoutResult& layout_result) const
{
    DCHECK(layout_result.IsSelfCollapsing());

    // The child must be an in-flow zero-block-size fragment, use its end margin
    // strut for positioning.
    LayoutUnit child_bfc_block_offset = child_data.bfc_offset_estimate.block_offset + layout_result.EndMarginStrut().Sum();
...
void BlockLayoutAlgorithm::ConsumeRemainingFragmentainerSpace(PreviousInflowPosition* previous_inflow_position)
{
    if (GetConstraintSpace().HasKnownFragmentainerBlockSize()) {
        ...
        // monolithic content).
        previous_inflow_position->logical_block_offset = std::max(previous_inflow_position->logical_block_offset, FragmentainerSpaceLeftForChildren());
    }
}
...
BreakStatus BlockLayoutAlgorithm::FinalizeForFragmentation()
{
    ...
    if (Node().IsInlineFormattingContextRoot() && !early_break_ && GetConstraintSpace().HasBlockFragmentation()) {
        if (container_builder_.HasInflowChildBreakInside() || first_overflowing_line_) {
            if (first_overflowing_line_ && first_overflowing_line_ < container_builder_.LineCount()) {
                int line_number;
                if (fit_all_lines_) {
                    ...
                    line_number = std::max(line_count - Style().Widows(), std::min(line_count, int(Style().Orphans())));
                }
                // We need to layout again, and stop at the right line number.
                const auto* breakpoint = MakeGarbageCollected<EarlyBreak>(line_number, kBreakAppealPerfect);
...
BreakStatus BlockLayoutAlgorithm::BreakBeforeChildIfNeeded(LayoutInputNode child, const LayoutResult& layout_result,
    PreviousInflowPosition* previous_inflow_position, LayoutUnit bfc_block_offset, bool has_container_separation)
{
    DCHECK(GetConstraintSpace().HasBlockFragmentation());
    ...
    LayoutUnit fragmentainer_block_offset = FragmentainerOffsetAtBfc(container_builder_) + bfc_block_offset - layout_result.AnnotationBlockOffsetAdjustment();

    if (has_container_separation) {
        EBreakBetween break_between = CalculateBreakBetweenValue(child, layout_result, container_builder_);
        if (IsForcedBreakValue(GetConstraintSpace(), break_between)) {
            BreakBeforeChild(GetConstraintSpace(), child, &layout_result, fragmentainer_block_offset, FragmentainerCapacityForChildren(), kBreakAppealPerfect,
    ...
    BreakAppeal appeal_before = CalculateBreakAppealBefore(GetConstraintSpace(), child, layout_result, container_builder_, has_container_separation);

    // Attempt to move past the break point, and if we can do that, also assess
    // the appeal of breaking there, even if we didn't.
    if (MovePastBreakpoint(child, layout_result, fragmentainer_block_offset, appeal_before)) {
    ...
    if (child.IsInline() && layout_result.Status() == LayoutResult::kSuccess) {
        if (!first_overflowing_line_) {
            ...
            // simplicity instead.
            PropagateSpaceShortage(GetConstraintSpace(), &layout_result, fragmentainer_block_offset, FragmentainerCapacityForChildren(), &container_builder_);
        }
        ...
        if (int line_count = container_builder_.LineCount()) {
            ...
            if (!is_first_fragment) {
                ...
                minimum_line_count = std::max(minimum_line_count, static_cast<int>(Style().Widows()));
            }
            if (line_count < minimum_line_count) {
                ...
                if (widows_found < Style().Widows()) {
                    ...
                    // trimming.
                    if (!should_text_box_trim_fragmentainer_end_ || override_text_box_trim_end_child_) {
                        return BreakStatus::kContinue;
                    }
                }
    ...
    if (!AttemptSoftBreak(
            GetConstraintSpace(), child, &layout_result, fragmentainer_block_offset, FragmentainerCapacityForChildren(), appeal_before, &container_builder_)) {
        return BreakStatus::kNeedsEarlierBreak;
    }
...
void BlockLayoutAlgorithm::UpdateEarlyBreakBetweenLines()
{
    ...
    int line_number = std::max(line_count - Style().Widows(), std::min(line_count - 1, static_cast<int>(Style().Orphans())));
    BreakAppeal appeal = kBreakAppealPerfect;
    if (line_number < Style().Orphans() || line_count - line_number < Style().Widows()) {
    ...
    if (container_builder_.HasEarlyBreak() && container_builder_.GetEarlyBreak().GetBreakAppeal() > appeal) {
        return;
    }
    const auto* breakpoint = MakeGarbageCollected<EarlyBreak>(line_number, appeal);
    container_builder_.SetEarlyBreak(breakpoint);
}

BoxStrut BlockLayoutAlgorithm::CalculateMargins(LayoutInputNode child, bool is_new_fc, LayoutUnit* additional_line_offset)
{
    DCHECK(child);
    if (child.IsInline())
        return {};
    ...
    const ComputedStyle& child_style = child.Style();
    BoxStrut margins = ComputeMarginsFor(child_style, child_percentage_size_.inline_size, GetConstraintSpace().GetWritingDirection());
    if (is_new_fc) {
        return margins;
    }
    ...
    auto ChildInlineSize = [&]() -> LayoutUnit {
        if (!child_inline_size) {
            ConstraintSpaceBuilder builder(GetConstraintSpace(), child_style.GetWritingDirection(),
                /* is_new_fc */ false);
            builder.SetAvailableSize(ChildAvailableSize());
            builder.SetPercentageResolutionSize(child_percentage_size_);

            const bool has_auto_margins = child_style.MarginInlineStartUsing(Style()).IsAuto() || child_style.MarginInlineEndUsing(Style()).IsAuto();

            const bool justify_self_affects_sizing = RuntimeEnabledFeatures::LayoutJustifySelfForBlocksEnabled() && !has_auto_margins;

            const ItemPosition justify_self = child_style.ResolvedJustifySelf({ ItemPosition::kNormal, OverflowAlignment::kDefault }, &Style()).GetPosition();
            ...
            } else if (justify_self_affects_sizing && justify_self != ItemPosition::kNormal) {
            ...
            BoxStrut child_border_padding = ComputeBorders(space, block_child) + ComputePadding(space, child_style);
            child_inline_size = ComputeInlineSizeForFragment(space, block_child, child_border_padding);
        }
    ...
    if (child_style.MarginInlineStartUsing(style).IsAuto() || child_style.MarginInlineEndUsing(style).IsAuto()) {
        ...
        ResolveInlineAutoMargins(child_style, style, available_space, ChildInlineSize(), &margins);
    } else {
        // Handle -webkit- values for text-align.
        text_align_offset = WebkitTextAlignAndJustifySelfOffset(child_style, style, available_space, margins, ChildInlineSize);
    }

    if (is_rtl) {
        *additional_line_offset = ChildAvailableSize().inline_size - text_align_offset - ChildInlineSize() - margins.InlineSum();
    } else {
...
ConstraintSpace BlockLayoutAlgorithm::CreateConstraintSpaceForChild(const LayoutInputNode child, const BreakToken* child_break_token,
    const InflowChildData& child_data, const LogicalSize child_available_size, bool is_new_fc, const std::optional<LayoutUnit> child_bfc_block_offset,
    bool has_clearance_past_adjoining_floats, LayoutUnit block_start_annotation_space)
{
    const ComputedStyle& child_style = child.Style();
    const auto child_writing_direction = child_style.GetWritingDirection();
    const auto& constraint_space = GetConstraintSpace();
    ConstraintSpaceBuilder builder(constraint_space, child_writing_direction, is_new_fc);

    const bool is_in_parallel_flow = IsParallelWritingMode(constraint_space.GetWritingMode(), child_writing_direction.GetWritingMode());
    ...
    if (child.IsInline()) {
        ...
        if (justify_self_affects_sizing && justify_self == ItemPosition::kStretch) {
            ...
            builder.SetInlineAutoBehavior(AutoSizeBehavior::kFitContent);
        } else if (is_in_parallel_flow && ShouldBlockContainerChildStretchAutoInlineSize(To<BlockNode>(child))) {
            builder.SetInlineAutoBehavior(AutoSizeBehavior::kStretchImplicit);
        }
    }
    ...
    if (constraint_space.IsTableCell()) {
        ...
        if (Node().GetDOMNode() && IsA<MathMLTableCellElement>(Node().GetDOMNode())) {
            builder.SetInlineAutoBehavior(AutoSizeBehavior::kFitContent);
        }
        ...
        if (constraint_space.IsRestrictedBlockSizeTableCell() && child_percentage_size_.block_size == kIndefiniteSize && !child.ShouldBeConsideredAsReplaced()
            && child_style.LogicalHeight().HasPercent()
            && (child_style.OverflowBlockDirection() == EOverflow::kAuto || child_style.OverflowBlockDirection() == EOverflow::kScroll)) {
    ...
    if (has_bfc_block_offset) {
        ...
        if (child.IsBlock()) {
            if (const LayoutResult* cached_result = child.GetLayoutBox()->GetCachedLayoutResult(To<BlockBreakToken>(child_break_token))) {
                ...
                // layout.
                LayoutUnit bfc_block_delta = child_data.bfc_offset_estimate.block_offset - prev_space.GetBfcOffset().block_offset;
                if (prev_space.ForcedBfcBlockOffset()) {
                    builder.SetOptimisticBfcBlockOffset(*prev_space.ForcedBfcBlockOffset() + bfc_block_delta);
                } else if (prev_space.OptimisticBfcBlockOffset()) {
                    builder.SetOptimisticBfcBlockOffset(*prev_space.OptimisticBfcBlockOffset() + bfc_block_delta);
                }
            }
        }
        ...
        // children.
        builder.SetOptimisticBfcBlockOffset(*constraint_space.OptimisticBfcBlockOffset());
    }
    ...
    if (!IsBreakInside(DynamicTo<BlockBreakToken>(child_break_token))) {
        ...
        if (child.IsBlock()) {
            LayoutUnit child_clearance_offset = GetExclusionSpace().ClearanceOffset(child_style.Clear(Style()));
    ...
    if (!is_new_fc) {
        ...
        if (!has_bfc_block_offset) {
            builder.SetAdjoiningObjectTypes(container_builder_.GetAdjoiningObjectTypes());
        }
        ...
        builder.SetShouldTextBoxTrimInsideWhenLineClamp(
            line_clamp_data_.data.IsLineClampContext() && (constraint_space.ShouldTextBoxTrimInsideWhenLineClamp() || should_text_box_trim_node_end_));
    }
    builder.SetBlockStartAnnotationSpace(block_start_annotation_space);

    // Propagate `text-box-trim` only for in-flow children.
    if (ShouldTextBoxTrim() && !child.IsFloatingOrOutOfFlowPositioned()) [[unlikely]] {
        builder.SetShouldTextBoxTrimNodeStart(should_text_box_trim_node_start_);
        builder.SetShouldTextBoxTrimFragmentainerStart(should_text_box_trim_fragmentainer_start_);
        builder.SetShouldTextBoxTrimFragmentainerEnd(should_text_box_trim_fragmentainer_end_);
        if (ShouldTextBoxTrimEnd()) {
            ...
            builder.SetShouldTextBoxTrimNodeEnd(should_text_box_trim_node_end_ && (child.IsInline() || IsLastInflowChild(*child.GetLayoutBox())));

            if (child.IsInline() && child == override_text_box_trim_end_child_
                && InlineBreakToken::IsStartEqual(To<InlineBreakToken>(override_text_box_trim_end_break_token_), To<InlineBreakToken>(child_break_token))) {
        ...
        const ComputedStyle& style = Node().Style();
        builder.SetEffectiveTextBoxEdge(style.TextBoxTrim() != ETextBoxTrim::kNone ? style.GetTextBoxEdge() : constraint_space.EffectiveTextBoxEdge());
    }
    ...
    if (constraint_space.HasBlockFragmentation()) {
        ...
        // to break).
        if (is_new_fc) {
            fragmentainer_offset_delta = *child_bfc_block_offset - constraint_space.ExpectedBfcBlockOffset();
        } else {
            fragmentainer_offset_delta = builder.ExpectedBfcBlockOffset() - constraint_space.ExpectedBfcBlockOffset();
        }
        SetupSpaceBuilderForFragmentation(container_builder_, child, fragmentainer_offset_delta, &builder);
...
void BlockLayoutAlgorithm::PropagateBaselineFromLineBox(const PhysicalFragment& child, LayoutUnit block_offset)
{
    ...
    if (line_box.IsBlockInInline()) [[unlikely]] {
        ...
        const auto& items = container_builder_.ItemsBuilder()->GetLogicalLineItems(line_box);
    ...
    LayoutUnit baseline = block_offset + (Style().IsFlippedLinesWritingMode() ? metrics.descent : metrics.ascent);
...
void BlockLayoutAlgorithm::PropagateBaselineFromBlockChild(const PhysicalFragment& child, const BoxStrut& margins, LayoutUnit block_offset)
{
    DCHECK(child.IsBox());
    const auto baseline_algorithm = GetConstraintSpace().GetBaselineAlgorithmType();
    ...
    LogicalBoxFragment fragment(GetConstraintSpace().GetWritingDirection(), physical_fragment);
    ...
    bool use_last_baseline = baseline_algorithm == BaselineAlgorithmType::kDefault || physical_fragment.UseLastBaselineForInlineBaseline();

    auto last_baseline = use_last_baseline ? fragment.LastBaseline() : fragment.FirstBaseline();
    ...
    if (baseline_algorithm == BaselineAlgorithmType::kInlineBlock && physical_fragment.ForceInlineBaselineSynthesis() && fragment.IsWritingModeEqual()) {
...
bool BlockLayoutAlgorithm::ResolveBfcBlockOffset(
    PreviousInflowPosition* previous_inflow_position, LayoutUnit bfc_block_offset, std::optional<LayoutUnit> forced_bfc_block_offset)
{
...
bool BlockLayoutAlgorithm::NeedsAbortOnBfcBlockOffsetChange() const
{
    ...
    return *container_builder_.BfcBlockOffset() != GetConstraintSpace().ExpectedBfcBlockOffset();
}

std::optional<LayoutUnit> BlockLayoutAlgorithm::CalculateQuirkyBodyMarginBlockSum(const MarginStrut& end_margin_strut)
{
    ...
    DCHECK(Node().IsBody());
    LayoutUnit block_end_margin = ComputeMarginsForSelf(GetConstraintSpace(), Style()).block_end;
    ...
    return *container_builder_.BfcBlockOffset() - GetConstraintSpace().GetBfcOffset().block_offset + body_strut.Sum();
}

bool BlockLayoutAlgorithm::PositionOrPropagateListMarker(
    const LayoutResult& layout_result, LogicalOffset* content_offset, PreviousInflowPosition* previous_inflow_position)
{
    ...
    UnpositionedListMarker list_marker = container_builder_.GetUnpositionedListMarker();
    if (!list_marker)
        return true;
    ...
    if (auto content_baseline = list_marker.ContentAlignmentBaseline(space, baseline_type, content)) {
        ...
        // separately.
        const LayoutResult* marker_layout_result = list_marker.Layout(space, container_builder_.Style(), baseline_type);
        DCHECK(marker_layout_result);
        // If the BFC block-offset of li is still not resolved, resolved it now.
        if (!container_builder_.BfcBlockOffset() && marker_layout_result->BfcBlockOffset()) {
        ...
        list_marker.AddToBox(space, baseline_type, content, BorderScrollbarPadding(), *marker_layout_result, *content_baseline, &content_offset->block_offset,
            &container_builder_);
        return true;
    }
...
bool BlockLayoutAlgorithm::PositionListMarkerWithoutLineBoxes(PreviousInflowPosition* previous_inflow_position)
{
    ...
    list_marker.AddToBoxWithoutLineBoxes(space, baseline_type, *marker_layout_result, &container_builder_, &intrinsic_block_size_);
    container_builder_.ClearUnpositionedListMarker();

    return true;
}

LayoutUnit BlockLayoutAlgorithm::HandleTextControlPlaceholder(BlockNode placeholder, const PreviousInflowPosition& previous_inflow_position)
{
    ...
    if (container_builder_.Children().size() > 0 && apply_fixed_size) {
        ...
        const PhysicalFragment& child = *container_builder_.Children()[kTextBlockIndex].fragment;
        if (child.IsTextControlContainer()) {
            ...
            if (begin != grand_children.end()) {
                LogicalFragment grand_child_fragment(GetConstraintSpace().GetWritingDirection(), *begin->fragment);
    ...
    const InflowChildData child_data = ComputeChildData(previous_inflow_position, placeholder,
        /* child_break_token */ nullptr, is_new_fc);
    const ConstraintSpace space = CreateConstraintSpaceForChild(placeholder, /* child_break_token */ nullptr, child_data, available_size, is_new_fc);

    const LayoutResult* result = placeholder.Layout(space);
    LogicalOffset offset = BorderScrollbarPadding().StartOffset();
    if (Node().IsTextArea()) {
        return FinishTextControlPlaceholder(result, offset, apply_fixed_size, previous_inflow_position);
    }
    ...
    LogicalBoxFragment fragment(GetConstraintSpace().GetWritingDirection(), To<PhysicalBoxFragment>(result->GetPhysicalFragment()));
    ...
    if (fragment.FirstBaseline().has_value()) {
        ...
        const LayoutUnit border_padding_block_start = BorderScrollbarPadding().block_start;
        ...
        if (!apply_fixed_size && offset.block_offset < border_padding_block_start) {
            ...
            const LayoutUnit new_baseline = placeholder_baseline + border_padding_block_start;
            container_builder_.SetFirstBaseline(new_baseline);
            container_builder_.SetLastBaseline(new_baseline);
            const LogicalFragmentLink& first_child = container_builder_.Children()[kTextBlockIndex];
            LogicalOffset first_child_offset = first_child.offset;
            first_child_offset.block_offset += new_baseline - first_baseline;
            container_builder_.ReplaceChild(kTextBlockIndex, *first_child.fragment, first_child_offset);
...
LayoutUnit BlockLayoutAlgorithm::FinishTextControlPlaceholder(
    const LayoutResult* result, const LogicalOffset& offset, bool apply_fixed_size, const PreviousInflowPosition& previous_inflow_position)
{
...
LogicalOffset BlockLayoutAlgorithm::AdjustSliderThumbInlineOffset(const LogicalFragment& fragment, const LogicalOffset& logical_offset)
{
    const LayoutUnit available_extent = ChildAvailableSize().inline_size - fragment.InlineSize();
    const auto* input = To<HTMLInputElement>(Node().GetDOMNode()->OwnerShadowHost());
