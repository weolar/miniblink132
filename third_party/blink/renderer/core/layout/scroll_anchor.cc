...
{
    return block_flow && block_flow->IsFragmentationContextRoot() && block_flow->IsLayoutNGObject();
}
...
LogicalOffset ToLogicalOffset(const gfx::PointF& point, const ScrollableArea& scroller)
{
    return ScrollerLayoutBox(&scroller)->IsHorizontalWritingMode() ? LogicalOffset(LayoutUnit(point.x()), LayoutUnit(point.y()))
...
SerializedAnchor::SerializedAnchor(const ScrollAnchorData& data, const ScrollableArea& scroller)
...
ScrollOffset SerializedAnchor::GetScrollOffset(const ScrollableArea& scroller) const
{
...
void ScrollAnchor::SetScroller(ScrollableArea* scroller)
{
    ...
    DCHECK(scroller);
    DCHECK(scroller->IsRootFrameViewport() || scroller->IsPaintLayerScrollableArea());
    scroller_ = scroller;
    ClearSelf();
}
...
static PhysicalOffset CornerPointOfRect(const PhysicalRect& rect, Corner which_corner)
{
    switch (which_corner) {
...
static PhysicalRect RelativeBounds(const LayoutObject* layout_object, const ScrollableArea* scroller)
{
    ...
    if (const auto* box = DynamicTo<LayoutBox>(layout_object)) {
        ...
        if (!layout_object->ShouldClipOverflowAlongEitherAxis()) {
            ...
            LayoutUnit max_y = std::max(local_bounds.Bottom(), box->ScrollableOverflowRect().Bottom());
    ...
    gfx::RectF relative_bounds = scroller->LocalToVisibleContentQuad(gfx::QuadF(gfx::RectF(local_bounds)), layout_object).BoundingBox();

    return PhysicalRect::FastAndLossyFromRectF(relative_bounds);
}

static LogicalOffset ComputeRelativeOffset(const LayoutObject* layout_object, const ScrollableArea* scroller, Corner corner)
{
    PhysicalOffset offset = CornerPointOfRect(RelativeBounds(layout_object, scroller), corner);
...
{
    if (candidate->IsFixedPositioned() || candidate->StyleRef().HasStickyConstrainedPosition())
        return false;
...
{
    DCHECK(element);
    return (1U == NthIndexCache::NthOfTypeIndex(*element)) && (1U == NthIndexCache::NthLastOfTypeIndex(*element));
}
...
static const AtomicString UniqueClassnameAmongSiblings(Element* element)
{
    ...
    Element* sibling_element = parent_element ? ElementTraversal::FirstChild(*parent_element) : element;
    // Add every classname of every sibling to our bloom filter, starting from the
    // leftmost sibling, but skipping |element|.
    for (; sibling_element; sibling_element = ElementTraversal::NextSibling(*sibling_element)) {
...
static const String UniqueSimpleSelectorAmongSiblings(Element* element)
{
    ...
    if (element->HasID() && !element->GetDocument().ContainsMultipleElementsWithId(element->GetIdAttribute())) {
    ...
    return ":nth-child("
        + String::Number(NthIndexCache::NthChildIndex(*element, /*filter=*/nullptr, /*selector_checker=*/nullptr,
            /*context=*/nullptr))
        + ")";
}
...
static const String ComputeUniqueSelector(Node* anchor_node)
{
    ...
    for (Element* element = ElementTraversal::FirstAncestorOrSelf(*anchor_node); element; element = ElementTraversal::FirstAncestor(*element)) {
    ...
    for (auto reverse_iterator = selector_list.rbegin(); reverse_iterator != selector_list.rend(); ++reverse_iterator, ++i) {
        if (i)
            builder.Append(">");
...
{
    auto visible_rect = ScrollerLayoutBox(scroller)->OverflowClipRect(PhysicalOffset());

    const ComputedStyle* style = ScrollerLayoutBox(scroller)->Style();
    visible_rect.ContractEdges(MinimumValueForLength(style->ScrollPaddingTop(), visible_rect.Height()),
        MinimumValueForLength(style->ScrollPaddingRight(), visible_rect.Width()), MinimumValueForLength(style->ScrollPaddingBottom(), visible_rect.Height()),
...
ScrollAnchor::ExamineResult ScrollAnchor::Examine(const LayoutObject* candidate) const
{
    ...
    bool occupies_space = candidate_rect.Width() > 0 && candidate_rect.Height() > 0;
    if (occupies_space && visible_rect.Intersects(candidate_rect)) {
        return ExamineResult(visible_rect.Contains(candidate_rect) ? kReturn : kConstrain, CornerToAnchor(scroller_));
    } else {
        return ExamineResult(kSkip);
    }
}
...
void ScrollAnchor::FindAnchor()
{
    ...
    if (anchor_object_) {
        ...
        saved_relative_offset_ = ComputeRelativeOffset(anchor_object_, scroller_, corner_);
        TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "FindAnchor", "anchor_object_", anchor_object_->DebugName());
        TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "FindAnchor", "saved_relative_offset_", saved_relative_offset_.ToString());
        anchor_is_cv_auto_without_layout_ = DisplayLockUtilities::IsAutoWithoutLayout(*anchor_object_);
    }
}
...
bool ScrollAnchor::FindAnchorInPriorityCandidates()
{
    ...
    candidate = PriorityCandidateFromNode(document.GetFindInPageActiveMatchNode());
    result = ExaminePriorityCandidate(candidate);
    if (IsViable(result.status)) {
        anchor_object_ = candidate;
        corner_ = result.corner;
...
LayoutObject* ScrollAnchor::PriorityCandidateFromNode(const Node* node) const
{
    while (node) {
        if (auto* layout_object = node->GetLayoutObject()) {
            if (!layout_object->IsAnonymous() && (!layout_object->IsInline() || layout_object->IsAtomicInlineLevel())) {
...
ScrollAnchor::ExamineResult ScrollAnchor::ExaminePriorityCandidate(const LayoutObject* candidate) const
{
...
ScrollAnchor::WalkStatus ScrollAnchor::FindAnchorRecursive(LayoutObject* candidate)
{
    ...
    bool is_block_fragmentation_context_root = IsNGBlockFragmentationRoot(DynamicTo<LayoutBlockFlow>(candidate));

    for (LayoutObject* child = candidate->SlowFirstChild(); child; child = child->NextSibling()) {
...
ScrollAnchor::WalkStatus ScrollAnchor::FindAnchorInOOFs(LayoutObject* candidate)
{
    ...
    bool is_block_fragmentation_context_root = IsNGBlockFragmentationRoot(DynamicTo<LayoutBlockFlow>(layout_block));
    for (const PhysicalBoxFragment& fragment : layout_block->PhysicalFragments()) {
        if (!fragment.HasOutOfFlowFragmentChild() && !is_block_fragmentation_context_root)
            continue;
        ...
        for (const PhysicalFragmentLink& child : fragment.Children()) {
            ...
            if (!is_block_fragmentation_context_root || !child->IsFragmentainerBox() || !child->HasOutOfFlowFragmentChild())
                continue;
...
{
    if (queued_) {
        scroll_anchor_disabling_style_changed_ |= ComputeScrollAnchorDisablingStyleChanged();
        return;
    }
    DCHECK(scroller_);
    ScrollOffset scroll_offset = scroller_->GetScrollOffset();
    float block_direction_scroll_offset = ScrollerLayoutBox(scroller_)->IsHorizontalWritingMode() ? scroll_offset.y() : scroll_offset.x();
    ...
    ScrollableArea* owning_scroller = root_frame_viewport ? &root_frame_viewport->LayoutViewport() : scroller_.Get();
...
gfx::Vector2d ScrollAnchor::ComputeAdjustment() const
{
    ...
    gfx::Vector2d delta = ToRoundedVector2d(ComputeRelativeOffset(anchor_object_, scroller_, corner_)) - ToRoundedVector2d(saved_relative_offset_);

    PhysicalRect anchor_rect = RelativeBounds(anchor_object_, scroller_);
    TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "ComputeAdjustment", "anchor_object_", anchor_object_->DebugName());
    TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "ComputeAdjustment", "delta", delta.ToString());
        ...
        if (scroller_box->IsHorizontalWritingMode()) {
            ...
            if (scroller_box->HasFlippedBlocksWritingMode()) {
                ...
                if (anchor_rect.X() > visible_rect.Right()) {
                    delta.set_x(delta.x() - (anchor_rect.X().ToInt() - visible_rect.Right().ToInt()));
                }
    ...
    // make it physical.
    if (!scroller_box->IsHorizontalWritingMode() && scroller_box->HasFlippedBlocksWritingMode()) {
        delta.set_x(-delta.x());
    }
    return delta;
}
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "ScrollAnchor::Adjust");
    if (!queued_)
        return;
    queued_ = false;
    DCHECK(scroller_);
    ...
    TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "Adjust", "adjustment", adjustment.ToString());
    ...
    ScrollOffset new_offset = scroller_->GetScrollOffset() + ScrollOffset(adjustment);

    TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "Adjust", "new_offset", new_offset.ToString());

    scroller_->SetScrollOffset(new_offset, mojom::blink::ScrollType::kAnchoring);

    UseCounter::Count(ScrollerLayoutBox(scroller_)->GetDocument(), WebFeature::kScrollAnchored);
}

bool ScrollAnchor::RestoreAnchor(const SerializedAnchor& serialized_anchor)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "ScrollAnchor::RestoreAnchor");
    ...
    StaticElementList* found_elements = document->QuerySelectorAll(AtomicString(serialized_anchor.selector), exception_state);
    ...
    TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "RestoreAnchor", "found_elements_length", found_elements->length());
    ...
    for (unsigned index = 0; index < found_elements->length(); index++) {
        ...
        gfx::PointF location_point = IsFlippedBlocksWritingMode(writing_mode) ? bounding_box.top_right()
        ...
        TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "RestoreAnchor", "anchor_object", anchor_object->DebugName());
        scroller_->SetScrollOffset(desired_offset, mojom::blink::ScrollType::kAnchoring);
        FindAnchor();

        // If the above FindAnchor call failed, reset the scroll position and try
        // again with the next found element.
        if (!anchor_object_) {
            scroller_->SetScrollOffset(current_offset, mojom::blink::ScrollType::kAnchoring);
            continue;
        }
...
{
    LayoutObject* layout_object = anchor_object_ ? anchor_object_ : ScrollerLayoutBox(scroller_);
