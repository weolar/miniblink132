...
LayoutCounter::LayoutCounter(Document& document, const CounterContentData& counter)
...
void LayoutCounter::UpdateCounter(Vector<int> counter_values)
{
    ...
    if (!counter_->Separator().IsNull()) {
        for (wtf_size_t i = 1u; i < counter_values.size(); ++i) {
            text = GenerateCounterText(counter_style, counter_values[i]) + counter_->Separator() + text;
        }
    }
...
const CounterStyle* LayoutCounter::NullableCounterStyle() const
{
    ...
    return &GetDocument().GetStyleEngine().FindCounterStyleAcrossScopes(counter_->ListStyle(), counter_->GetTreeScope());
}
...
bool LayoutCounter::IsDirectionalSymbolMarker() const
{
    ...
    return list_style == keywords::kDisclosureOpen || list_style == keywords::kDisclosureClosed;
}
...
// static
const AtomicString& LayoutCounter::ListStyle(const LayoutObject* object, const ComputedStyle& style)
{
