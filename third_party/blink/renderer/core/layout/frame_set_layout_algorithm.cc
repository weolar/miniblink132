...
LayoutUnit AdjustSizeToRemainingSize(LayoutUnit current, LayoutUnit remaining, int64_t total)
{
...
FrameSetLayoutAlgorithm::FrameSetLayoutAlgorithm(const LayoutAlgorithmParams& params)
...
const LayoutResult* FrameSetLayoutAlgorithm::Layout()
{
    ...
    PhysicalSize size = ToPhysicalSize(container_builder_.Size(), GetConstraintSpace().GetWritingMode());
    const wtf_size_t row_count = frame_set.TotalRows();
    layout_data->row_sizes
        = LayoutAxis(row_count, frame_set.RowLengths(), frame_set.RowDeltas(), size.height - (row_count - 1) * layout_data->border_thickness);
    const wtf_size_t col_count = frame_set.TotalCols();
    layout_data->col_sizes = LayoutAxis(col_count, frame_set.ColLengths(), frame_set.ColDeltas(), size.width - (col_count - 1) * layout_data->border_thickness);
...
MinMaxSizesResult FrameSetLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    MinMaxSizes sizes;
    ...
    sizes += (ComputeBorders(space, Node()) + ComputePadding(space, Style())).InlineSum();
...
Vector<LayoutUnit> FrameSetLayoutAlgorithm::LayoutAxis(
    wtf_size_t count, const Vector<HTMLDimension>& grid, const Vector<int>& deltas, LayoutUnit available_length)
{
    DCHECK_GT(count, 0u);
    ...
    for (auto i : fixed_indices) {
        sizes[i] = IntLayoutUnit(grid[i].Value() * effective_zoom).ClampNegativeToZero();
    ...
    for (auto i : percent_indices) {
        sizes[i] = IntLayoutUnit(grid[i].Value() * available_length / 100.0).ClampNegativeToZero();
    ...
    if (total_fixed > remaining_length.ToInt()) {
        ...
        for (auto i : fixed_indices) {
            sizes[i] = AdjustSizeToRemainingSize(sizes[i], remaining_fixed, total_fixed);
    ...
    if (total_percent > remaining_length.ToInt()) {
        ...
        for (auto i : percent_indices) {
            sizes[i] = AdjustSizeToRemainingSize(sizes[i], remaining_percent, total_percent);
            remaining_length -= sizes[i];
        }
    } else {
    ...
    if (!relative_indices.empty()) {
        ...
        for (auto i : relative_indices) {
            sizes[i] = IntLayoutUnit((ClampTo<int>(std::max(grid[i].Value(), 1.)) * remaining_relative) / total_relative);
            remaining_length -= sizes[i];
    ...
    if (remaining_length) {
        ...
        if (!percent_indices.empty() && total_percent) {
            LayoutUnit remaining_percent = remaining_length;
            for (auto i : percent_indices) {
                LayoutUnit change_percent = AdjustSizeToRemainingSize(sizes[i], remaining_percent, total_percent);
            ...
            LayoutUnit remaining_fixed = remaining_length;
            for (auto i : fixed_indices) {
                LayoutUnit change_fixed = AdjustSizeToRemainingSize(sizes[i], remaining_fixed, total_fixed);
...
void FrameSetLayoutAlgorithm::LayoutChildren(const FrameSetLayoutData& layout_data)
{
    ...
    for (wtf_size_t row = 0; row < layout_data.row_sizes.size(); ++row) {
        ...
        for (wtf_size_t col = 0; col < layout_data.col_sizes.size(); ++col) {
            ...
            const LogicalSize available_size(Style().GetWritingDirection().IsHorizontal() ? LogicalSize(col_size, row_size) : LogicalSize(row_size, col_size));
            LayoutChild(child, available_size, position, PhysicalSize(col_size, row_size));
...
void FrameSetLayoutAlgorithm::LayoutChild(const LayoutInputNode& child, LogicalSize available_size, PhysicalOffset position, PhysicalSize child_size)
{
    const PhysicalSize frameset_size = ToPhysicalSize(container_builder_.Size(), GetConstraintSpace().GetWritingMode());
    const auto container_direction = Style().GetWritingDirection();
    const bool kNewFormattingContext = true;
    ConstraintSpaceBuilder space_builder(container_direction.GetWritingMode(), child.Style().GetWritingDirection(), kNewFormattingContext);
    space_builder.SetAvailableSize(available_size);
    space_builder.SetIsFixedInlineSize(true);
    space_builder.SetIsFixedBlockSize(true);
    const LayoutResult* result = To<BlockNode>(child).Layout(space_builder.ToConstraintSpace());
    container_builder_.AddResult(*result, position.ConvertToLogical(container_direction, frameset_size, child_size));
}
