...
class CORE_EXPORT PaginatedRootLayoutAlgorithm : public LayoutAlgorithm<BlockNode, BoxFragmentBuilder, BlockBreakToken> {
public:
    struct PageContainerResult {
        STACK_ALLOCATED();

    public:
        PageContainerResult(const PhysicalBoxFragment& fragment, const BlockBreakToken* fragmentainer_break_token,
            const CountersAttachmentContext& counters_context, bool needs_total_page_count)
            : fragment(&fragment)
    ...
    static const PhysicalBoxFragment& CreateEmptyPage(const BlockNode& node, const ConstraintSpace& parent_space, wtf_size_t page_index,
        const PhysicalBoxFragment& previous_fragmentainer, bool* needs_total_page_count);

private:
    PageContainerResult LayoutPageContainer(wtf_size_t page_index, wtf_size_t total_page_count, const AtomicString& page_name,
        const CountersAttachmentContext& counters_context, const PageAreaLayoutParams& params,
        const PhysicalBoxFragment* existing_page_container = nullptr) const
    {
        return LayoutPageContainer(Node(), GetConstraintSpace(), page_index, total_page_count, page_name, counters_context, params, existing_page_container);
    }

    static PageContainerResult LayoutPageContainer(const BlockNode& root_node, const ConstraintSpace& parent_space, wtf_size_t page_index,
        wtf_size_t total_page_count, const AtomicString& page_name, const CountersAttachmentContext&, const PageAreaLayoutParams&,
