...
struct CORE_EXPORT PhysicalOffset {
    ...
    LogicalOffset ConvertToLogical(WritingDirectionMode writing_direction, PhysicalSize outer_size, PhysicalSize inner_size) const;
    ...
    // Conversions from/to existing code. New code prefers type safety for
    // logical/physical distinctions.
    constexpr LayoutPoint ToLayoutPoint() const
    {
        return { left, top };
    }
    ...
    {
        return { LayoutUnit::FromFloatFloor(point.x()), LayoutUnit::FromFloatFloor(point.y()) };
    }
    static PhysicalOffset FromPointFRound(const gfx::PointF& point)
    {
        return { LayoutUnit::FromFloatRound(point.x()), LayoutUnit::FromFloatRound(point.y()) };
    }
    static PhysicalOffset FromVector2dFFloor(const gfx::Vector2dF& vector)
    {
        return { LayoutUnit::FromFloatFloor(vector.x()), LayoutUnit::FromFloatFloor(vector.y()) };
    }
    static PhysicalOffset FromVector2dFRound(const gfx::Vector2dF& vector)
    {
        return { LayoutUnit::FromFloatRound(vector.x()), LayoutUnit::FromFloatRound(vector.y()) };
    }
