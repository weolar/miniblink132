...
TEST(PhysicalRectTest, AddOffset)
{
    ...
    EXPECT_EQ(PhysicalRect(1, 2, 3, 4) + PhysicalOffset(5, 6), PhysicalRect(6, 8, 3, 4));
}
...
struct PhysicalOffsetRectUniteTestData {
    ...
    { "saturated width", { -1000, 0, 200, 200 }, { 33554402, 500, 30, 100 }, { 0, 0, 99999999, 600 } },
    { "saturated height", { 0, -1000, 200, 200 }, { 0, 33554402, 100, 30 }, { 0, 0, 200, 99999999 } },
};

std::ostream& operator<<(std::ostream& os, const PhysicalOffsetRectUniteTestData& data)
{
    return os << "Unite " << data.test_case;
}

class PhysicalRectUniteTest : public testing::Test, public testing::WithParamInterface<PhysicalOffsetRectUniteTestData> { };

INSTANTIATE_TEST_SUITE_P(GeometryUnitsTest, PhysicalRectUniteTest, testing::ValuesIn(physical_offset_rect_unite_test_data));
...
TEST(PhysicalRectTest, SquaredDistanceTo)
{
    ...
    EXPECT_EQ(200, rect.SquaredDistanceTo(PhysicalOffset(-10, -10))) << "over the top-left corner";
    EXPECT_EQ(0, rect.SquaredDistanceTo(PhysicalOffset(0, 0))) << "on the top-left corner";
    EXPECT_EQ(100, rect.SquaredDistanceTo(PhysicalOffset(10, -10))) << "over the top edge";
    EXPECT_EQ(0, rect.SquaredDistanceTo(PhysicalOffset(10, 0))) << "on the top edge";
    EXPECT_EQ(200, rect.SquaredDistanceTo(PhysicalOffset(210, -10))) << "over the top-right corner";
    EXPECT_EQ(0, rect.SquaredDistanceTo(PhysicalOffset(200, 0))) << "on the top-right corner";
    EXPECT_EQ(100, rect.SquaredDistanceTo(PhysicalOffset(210, 10))) << "over the right edge";
    EXPECT_EQ(0, rect.SquaredDistanceTo(PhysicalOffset(200, 10))) << "on the right edge";
    EXPECT_EQ(200, rect.SquaredDistanceTo(PhysicalOffset(210, 210))) << "over the bottom-right corner";
    EXPECT_EQ(0, rect.SquaredDistanceTo(PhysicalOffset(200, 200))) << "on the bottom-right corner";
    EXPECT_EQ(10000, rect.SquaredDistanceTo(PhysicalOffset(100, 300))) << "over the bottom edge";
    EXPECT_EQ(0, rect.SquaredDistanceTo(PhysicalOffset(100, 200))) << "on the bottom edge";
    EXPECT_EQ(401, rect.SquaredDistanceTo(PhysicalOffset(-20, 201))) << "over the bottom-left corner";
    EXPECT_EQ(0, rect.SquaredDistanceTo(PhysicalOffset(0, 200))) << "on the bottom-left corner";
    EXPECT_EQ(9, rect.SquaredDistanceTo(PhysicalOffset(-3, 100))) << "over the left edge";
    EXPECT_EQ(0, rect.SquaredDistanceTo(PhysicalOffset(0, 3))) << "on the left edge";

    EXPECT_EQ(0, rect.SquaredDistanceTo(PhysicalOffset(10, 190))) << "contained";

    // Huge size
    rect = PhysicalRect(LayoutUnit(500), LayoutUnit(), LayoutUnit::Max(), LayoutUnit());
    EXPECT_GT(rect.SquaredDistanceTo(PhysicalOffset(10, 0)), 0);

    // Negative size
    rect = PhysicalRect(LayoutUnit(500), LayoutUnit(), LayoutUnit(-100), LayoutUnit());
...
TEST(PhysicalRectTest, ToEnclosingRect)
{
    ...
    LayoutUnit small;
    small.SetRawValue(1);
    PhysicalRect small_dimensions_rect(LayoutUnit(42.5f), LayoutUnit(84.5f), small, small);
    ...
    PhysicalRect fractional_pos_rect(LayoutUnit(100.6f), LayoutUnit(150.8f), LayoutUnit(200), LayoutUnit(350));
    EXPECT_EQ(gfx::Rect(100, 150, 201, 351), ToEnclosingRect(fractional_pos_rect));

    PhysicalRect fractional_dimensions_rect(LayoutUnit(100), LayoutUnit(150), LayoutUnit(200.6f), LayoutUnit(350.4f));
    EXPECT_EQ(gfx::Rect(100, 150, 201, 351), ToEnclosingRect(fractional_dimensions_rect));

    PhysicalRect fractional_both_rect1(LayoutUnit(100.6f), LayoutUnit(150.8f), LayoutUnit(200.4f), LayoutUnit(350.2f));
    EXPECT_EQ(gfx::Rect(100, 150, 201, 351), ToEnclosingRect(fractional_both_rect1));

    PhysicalRect fractional_both_rect2(LayoutUnit(100.5f), LayoutUnit(150.7f), LayoutUnit(200.3f), LayoutUnit(350.3f));
    EXPECT_EQ(gfx::Rect(100, 150, 201, 351), ToEnclosingRect(fractional_both_rect2));

    PhysicalRect fractional_both_rect3(LayoutUnit(100.3f), LayoutUnit(150.2f), LayoutUnit(200.8f), LayoutUnit(350.9f));
    EXPECT_EQ(gfx::Rect(100, 150, 202, 352), ToEnclosingRect(fractional_both_rect3));

    PhysicalRect fractional_negpos_rect1(LayoutUnit(-100.4f), LayoutUnit(-150.8f), LayoutUnit(200), LayoutUnit(350));
    EXPECT_EQ(gfx::Rect(-101, -151, 201, 351), ToEnclosingRect(fractional_negpos_rect1));

    PhysicalRect fractional_negpos_rect2(LayoutUnit(-100.5f), LayoutUnit(-150.7f), LayoutUnit(199.4f), LayoutUnit(350.3f));
    EXPECT_EQ(gfx::Rect(-101, -151, 200, 351), ToEnclosingRect(fractional_negpos_rect2));

    PhysicalRect fractional_negpos_rect3(LayoutUnit(-100.3f), LayoutUnit(-150.2f), LayoutUnit(199.6f), LayoutUnit(350.3f));
    EXPECT_EQ(gfx::Rect(-101, -151, 201, 352), ToEnclosingRect(fractional_negpos_rect3));
}
...
TEST(PhysicalRectTest, EdgesOnPixelBoundaries)
{
    ...
    EXPECT_TRUE(PhysicalRect(LayoutUnit(1.0f), LayoutUnit(5), LayoutUnit(10), LayoutUnit(3)).EdgesOnPixelBoundaries());

    EXPECT_FALSE(PhysicalRect(LayoutUnit(9.3f), LayoutUnit(5), LayoutUnit(10), LayoutUnit(3)).EdgesOnPixelBoundaries());
    EXPECT_FALSE(PhysicalRect(LayoutUnit(0.5f), LayoutUnit(5), LayoutUnit(10), LayoutUnit(3)).EdgesOnPixelBoundaries());
    EXPECT_FALSE(PhysicalRect(LayoutUnit(-0.5f), LayoutUnit(-5), LayoutUnit(10), LayoutUnit(3)).EdgesOnPixelBoundaries());
    EXPECT_FALSE(PhysicalRect(LayoutUnit(-0.5f), LayoutUnit(-2), LayoutUnit(10), LayoutUnit(3)).EdgesOnPixelBoundaries());
    EXPECT_FALSE(PhysicalRect(LayoutUnit(-0.5f), LayoutUnit(5.1f), LayoutUnit(10), LayoutUnit(3)).EdgesOnPixelBoundaries());
    EXPECT_FALSE(PhysicalRect(LayoutUnit(3), LayoutUnit(5.1f), LayoutUnit(10), LayoutUnit(3)).EdgesOnPixelBoundaries());
    EXPECT_FALSE(PhysicalRect(LayoutUnit(3), LayoutUnit(5), LayoutUnit(10.2f), LayoutUnit(3)).EdgesOnPixelBoundaries());
    EXPECT_FALSE(PhysicalRect(LayoutUnit(3), LayoutUnit(5), LayoutUnit(10), LayoutUnit(0.3f)).EdgesOnPixelBoundaries());
}
