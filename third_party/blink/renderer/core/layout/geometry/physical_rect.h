...
struct CORE_EXPORT PhysicalRect {
    ...
    {
        return px >= offset.left && px < Right() && py >= offset.top && py < Bottom();
    }
    ...
    {
        return !offset.left.HasFraction() && !offset.top.HasFraction() && !size.width.HasFraction() && !size.height.HasFraction();
    }
    ...
    gfx::Point PixelSnappedOffset() const
    {
    ...
    static PhysicalRect EnclosingRect(const gfx::RectF& rect)
    {
        ...
        PhysicalSize size(LayoutUnit::FromFloatCeil(rect.right()) - offset.left, LayoutUnit::FromFloatCeil(rect.bottom()) - offset.top);
    ...
    {
        return PhysicalRect(LayoutUnit(rect.x()), LayoutUnit(rect.y()), LayoutUnit(rect.width()), LayoutUnit(rect.height()));
    }
...
inline gfx::Rect ToEnclosingRect(const PhysicalRect& r)
{
    ...
    return gfx::Rect(location.x(), location.y(), max_point.x() - location.x(), max_point.y() - location.y());
}
...
CORE_EXPORT PhysicalRect UnionRectEvenIfEmpty(const Vector<PhysicalRect>& rects);
