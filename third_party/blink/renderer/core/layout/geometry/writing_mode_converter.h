...
public:
    ...
    WritingModeConverter(WritingDirectionMode writing_direction, const PhysicalSize& outer_size)
        : writing_direction_(writing_direction)
        , outer_size_(outer_size)
    {
    }

    WritingModeConverter(WritingDirectionMode writing_direction, const LogicalSize& outer_size)
        : writing_direction_(writing_direction)
        , outer_size_(ToPhysicalSize(outer_size, writing_direction.GetWritingMode()))
    {
    }
    ...
    bool IsLtr() const
    {
        return writing_direction_.IsLtr();
    }

    const PhysicalSize OuterSize() const
    {
        return outer_size_;
    }
    ...
    PhysicalOffset ToPhysical(const LogicalOffset& offset, const PhysicalSize& inner_size) const;
...
private:
    LogicalOffset SlowToLogical(const PhysicalOffset& offset, const PhysicalSize& inner_size) const;
    PhysicalOffset SlowToPhysical(const LogicalOffset& offset, const PhysicalSize& inner_size) const;

    LogicalRect SlowToLogical(const PhysicalRect& rect) const;
    PhysicalRect SlowToPhysical(const LogicalRect& rect) const;

    gfx::PointF SlowToLogical(const gfx::PointF& offset, const gfx::SizeF& inner_size) const;
...
inline LogicalOffset WritingModeConverter::ToLogical(const PhysicalOffset& offset, const PhysicalSize& inner_size) const
{
...
inline PhysicalOffset WritingModeConverter::ToPhysical(const LogicalOffset& offset, const PhysicalSize& inner_size) const
{
    if (writing_direction_.IsHorizontalLtr())
...
inline LogicalSize WritingModeConverter::ToLogical(const PhysicalSize& size) const
{
    return size.ConvertToLogical(GetWritingMode());
}

inline PhysicalSize WritingModeConverter::ToPhysical(const LogicalSize& size) const
{
    return ToPhysicalSize(size, GetWritingMode());
}

inline LogicalRect WritingModeConverter::ToLogical(const PhysicalRect& rect) const
{
    if (writing_direction_.IsHorizontalLtr())
...
inline PhysicalRect WritingModeConverter::ToPhysical(const LogicalRect& rect) const
{
    if (writing_direction_.IsHorizontalLtr()) {
        return PhysicalRect(rect.offset.inline_offset, rect.offset.block_offset, rect.size.inline_size, rect.size.block_size);
    }
    return SlowToPhysical(rect);
}

inline gfx::PointF WritingModeConverter::ToLogical(const gfx::PointF& offset) const
{
    if (writing_direction_.IsHorizontalLtr()) {
        return offset;
    }
...
inline gfx::SizeF WritingModeConverter::ToLogical(const gfx::SizeF& size) const
{
    return writing_direction_.IsHorizontal() ? size : gfx::TransposeSize(size);
}

inline gfx::RectF WritingModeConverter::ToLogical(const gfx::RectF& rect) const
{
    if (writing_direction_.IsHorizontalLtr()) {
        return rect;
    }
