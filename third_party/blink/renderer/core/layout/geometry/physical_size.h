...
struct CORE_EXPORT PhysicalSize {
    ...
    {
        return mode == WritingMode::kHorizontalTb ? LogicalSize(width, height) : LogicalSize(height, width);
    }
    ...
    {
        return PhysicalSize(LayoutUnit(this->width * scale), LayoutUnit(this->height * scale));
    }
    ...
    {
        return { std::min(this->width, other.width), std::min(this->height, other.height) };
    }
    ...
    PhysicalSize FitToAspectRatio(const PhysicalSize& aspect_ratio, AspectRatioFit fit) const;
    ...
    {
        return { LayoutUnit::FromFloatRound(size.width()), LayoutUnit::FromFloatRound(size.height()) };
    }
    static PhysicalSize FromSizeFFloor(const gfx::SizeF& size)
    {
        return { LayoutUnit::FromFloatFloor(size.width()), LayoutUnit::FromFloatFloor(size.height()) };
    }
...
{
    return mode == WritingMode::kHorizontalTb ? PhysicalSize(other.inline_size, other.block_size) : PhysicalSize(other.block_size, other.inline_size);
}
