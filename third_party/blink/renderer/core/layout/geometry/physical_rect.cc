...
{
    return offset.left <= other.offset.left && offset.top <= other.offset.top && Right() >= other.Right() && Bottom() >= other.Bottom();
}

bool PhysicalRect::Intersects(const PhysicalRect& other) const
{
    // Checking emptiness handles negative widths as well as zero.
    return !IsEmpty() && !other.IsEmpty() && offset.left < other.Right() && other.offset.left < Right() && offset.top < other.Bottom()
...
bool PhysicalRect::IntersectsInclusively(const PhysicalRect& other) const
{
    ...
    return offset.left <= other.Right() && other.offset.left <= Right() && offset.top <= other.Bottom() && other.offset.top <= Bottom();
}
...
void PhysicalRect::Intersect(const PhysicalRect& other)
{
    ...
    PhysicalOffset new_max_point(std::min(Right(), other.Right()), std::min(Bottom(), other.Bottom()));
    ...
    offset = new_offset;
    size = { new_max_point.left - new_offset.left, new_max_point.top - new_offset.top };
}
...
{
    return String::Format("%s %s", offset.ToString().Ascii().c_str(), size.ToString().Ascii().c_str());
}
