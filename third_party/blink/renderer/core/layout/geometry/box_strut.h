...
struct CORE_EXPORT BoxStrut {
    BoxStrut() = default;
    BoxStrut(LayoutUnit inline_start, LayoutUnit inline_end, LayoutUnit block_start, LayoutUnit block_end)
    ...
    LayoutUnit InlineSum() const
    {
        return inline_start + inline_end;
    }
    LayoutUnit BlockSum() const
    {
        return block_start + block_end;
    }

    LogicalOffset StartOffset() const
    {
    ...
    {
        return std::tie(other.inline_start, other.inline_end, other.block_start, other.block_end) == std::tie(inline_start, inline_end, block_start, block_end);
    }
    bool operator!=(const BoxStrut& other) const
    {
        return !(*this == other);
...
struct CORE_EXPORT LineBoxStrut {
    ...
    LineBoxStrut(LayoutUnit inline_start, LayoutUnit inline_end, LayoutUnit line_over, LayoutUnit line_under)
        : inline_start(inline_start)
        , inline_end(inline_end)
        , line_over(line_over)
    ...
    {
        return inline_start == other.inline_start && inline_end == other.inline_end && line_over == other.line_over && line_under == other.line_under;
    }
...
struct CORE_EXPORT PhysicalBoxStrut {
    ...
    PhysicalBoxStrut(LayoutUnit top, LayoutUnit right, LayoutUnit bottom, LayoutUnit left)
        : top(top)
        , right(right)
        , bottom(bottom)
        , left(left)
    ...
    // Create a strut based on an inner rectangle positioned within an area.
    PhysicalBoxStrut(const PhysicalSize& outer_size, const PhysicalRect& inner_rect);
    ...
    {
        return { LayoutUnit::FromFloatCeil(outsets.top()), LayoutUnit::FromFloatCeil(outsets.right()), LayoutUnit::FromFloatCeil(outsets.bottom()),
    ...
    LineBoxStrut ConvertToLineLogical(WritingDirectionMode writing_direction) const
    {
        return LineBoxStrut(ConvertToLogical(writing_direction), writing_direction.IsFlippedLines());
    }

    LayoutUnit HorizontalSum() const
    {
        return left + right;
    }
    LayoutUnit VerticalSum() const
    {
        return top + bottom;
    }
    ...
    {
        return gfx::OutsetsF().set_left(left.ToFloat()).set_right(right.ToFloat()).set_top(top.ToFloat()).set_bottom(bottom.ToFloat());
    }
...
inline PhysicalBoxStrut BoxStrut::ConvertToPhysical(WritingDirectionMode writing_direction) const
{
    ...
    switch (writing_direction.GetWritingMode()) {
    case WritingMode::kHorizontalTb:
        return PhysicalBoxStrut(block_start, direction_end, block_end, direction_start);
    case WritingMode::kVerticalRl:
    case WritingMode::kSidewaysRl:
        return PhysicalBoxStrut(direction_start, block_start, direction_end, block_end);
    case WritingMode::kVerticalLr:
        return PhysicalBoxStrut(direction_start, block_end, direction_end, block_start);
    case WritingMode::kSidewaysLr:
        return PhysicalBoxStrut(direction_end, block_end, direction_start, block_start);
    default:
        NOTREACHED();
    }
}
