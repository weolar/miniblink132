...
struct CORE_EXPORT BfcOffset {
    ...
    BfcOffset operator+(const BfcDelta& delta)
    {
        return { line_offset + delta.line_offset_delta, block_offset + delta.block_offset_delta };
    }

    bool operator==(const BfcOffset& other) const
    {
        return std::tie(other.line_offset, other.block_offset) == std::tie(line_offset, block_offset);
    }
