...
public:
    ...
    {
        SetBodyInnerHTML(String::Format("<div id='target' style='font: 10px Ahem; width: %uem'>%s</div>", width, message));
    }
    ...
    {
        return GetLayoutTextById("target");
    }

    void SetSelectionAndUpdateLayoutSelection(const std::string& selection_text)
    {
        const SelectionInDOMTree selection = SelectionSample::SetSelectionText(GetDocument().body(), selection_text);
    ...
    {
        for (const Node& node : NodeTraversal::DescendantsOf(*GetDocument().body())) {
    ...
    PhysicalRect GetSelectionRectFor(const std::string& selection_text)
    {
        ...
        stream << "<div style='font: 10px/10px Ahem;'>" << selection_text << "</div>";
        SetSelectionAndUpdateLayoutSelection(stream.str());
        const Node* target = GetElementById("target");
        const LayoutObject* layout_object = target ? target->GetLayoutObject() : FindFirstLayoutText();
        return layout_object->LocalSelectionVisualRect();
    }

    std::string GetSnapCode(const LayoutText& layout_text, const std::string& caret_text)
    {
        return GetSnapCode(layout_text, static_cast<unsigned>(caret_text.find('|')));
    }
    ...
    std::string GetItemsAsString(const LayoutText& layout_text, int num_glyphs = 0, unsigned flags = 0)
    {
        ...
        for (const InlineItem& item : data.items) {
            ...
            if (const auto* shape_result = item.TextShapeResult()) {
                stream << ", ShapeResult=" << shape_result->StartIndex() << "+" << shape_result->NumCharacters();
#if BUILDFLAG(IS_WIN)
    ...
    unsigned CountNumberOfGlyphs(const LayoutText& layout_text)
    {
        ...
        return std::accumulate(
            items->begin(), items->end(), 0u, [](unsigned sum, const InlineItem& item) { return sum + item.TextShapeResult()->NumGlyphs(); });
    }
};
...
TEST_F(LayoutTextTest, PrewarmFamily)
{
    ...
    EXPECT_TRUE(container->StyleRef().GetFont().GetFontDescription().Family().IsPrewarmed());
}
...
TEST_F(LayoutTextTest, PrewarmFontFace)
{
    ...
    LayoutObject* container = GetLayoutObjectByElementId("container");
    EXPECT_FALSE(container->StyleRef().GetFont().GetFontDescription().Family().IsPrewarmed());
}
...
struct OffsetMappingTestData {
    ...
    unsigned text_start;
    unsigned text_end;
} offset_mapping_test_data[] = { { "<div id=target> a  b  </div>", 0, 1, true, 0, 0 }, { "<div id=target> a  b  </div>", 1, 2, true, 0, 1 },
    { "<div id=target> a  b  </div>", 2, 3, true, 1, 2 }, { "<div id=target> a  b  </div>", 2, 4, true, 1, 2 },
    { "<div id=target> a  b  </div>", 2, 5, true, 1, 3 }, { "<div id=target> a  b  </div>", 3, 4, true, 2, 2 },
    { "<div id=target> a  b  </div>", 3, 5, true, 2, 3 }, { "<div id=target> a  b  </div>", 5, 6, true, 3, 3 },
    { "<div id=target> a  b  </div>", 5, 7, true, 3, 3 }, { "<div id=target> a  b  </div>", 6, 7, true, 3, 3 },
    { "<div>a <span id=target> </span>b</div>", 0, 1, false, 0, 1 } };

std::ostream& operator<<(std::ostream& out, OffsetMappingTestData data)
{
    return out << "\"" << data.text << "\" " << data.dom_start << "," << data.dom_end << " => " << (data.success ? "true " : "false ") << data.text_start << ","
               << data.text_end;
}

class MapDOMOffsetToTextContentOffset : public LayoutTextTest, public testing::WithParamInterface<OffsetMappingTestData> { };

INSTANTIATE_TEST_SUITE_P(LayoutTextTest, MapDOMOffsetToTextContentOffset, testing::ValuesIn(offset_mapping_test_data));
...
TEST_P(MapDOMOffsetToTextContentOffset, Basic)
{
    ...
    bool success = layout_text->MapDOMOffsetToTextContentOffset(*mapping, &start, &end);
...
TEST_F(LayoutTextTest, CharacterAfterWhitespaceCollapsing)
{
    ...
    SetBodyInnerHTML("<span style='white-space: pre'>a <span id=target> </span>b</span>");
    layout_text = GetLayoutTextById("target");
    EXPECT_EQ(' ', layout_text->FirstCharacterAfterWhitespaceCollapsing());
    EXPECT_EQ(' ', layout_text->LastCharacterAfterWhitespaceCollapsing());
    ...
    EXPECT_EQ(' ', layout_text->LastCharacterAfterWhitespaceCollapsing());
    layout_text = To<LayoutText>(GetLayoutObjectByElementId("target")->NextSibling());
    DCHECK(!layout_text->HasInlineFragments());
    EXPECT_EQ(0, layout_text->FirstCharacterAfterWhitespaceCollapsing());
    EXPECT_EQ(0, layout_text->LastCharacterAfterWhitespaceCollapsing());
...
TEST_F(LayoutTextTest, GetTextBoxInfoWithGeneratedContent)
{
    ...
    const Element& before = *GetElementById("target")->GetPseudoElement(kPseudoIdBefore);
    const auto& layout_text_xyz = *To<LayoutText>(target.firstChild()->GetLayoutObject());
    const auto& layout_text_remaining = To<LayoutText>(*before.GetLayoutObject()->SlowLastChild());
    const LayoutText& layout_text_first_letter = *layout_text_remaining.GetFirstLetterPart();
...
TEST_F(LayoutTextTest, GetTextBoxInfoWithHidden)
{
    ...
    const Element& target = *GetElementById("target");
    const LayoutText& layout_text = *To<Text>(target.firstChild())->GetLayoutObject();
...
TEST_F(LayoutTextTest, GetTextBoxInfoWithEllipsisForPseudoAfter)
{
    ...
    const auto& layout_text = *To<LayoutText>(after.GetLayoutObject()->SlowFirstChild());
...
TEST_F(LayoutTextTest, PlainTextInPseudo)
{
    ...
    const LayoutObject* before_parent = GetLayoutObjectByElementId("before_parent");
    EXPECT_EQ("123 456", GetPlainText(before_parent));
    const LayoutObject* before_parent_cjk = GetLayoutObjectByElementId("before_parent_cjk");
...
TEST_F(LayoutTextTest, AbsoluteQuads)
{
    ...
    EXPECT_THAT(quads, testing::ElementsAre(gfx::QuadF(gfx::RectF(30, 0, 30, 10)), gfx::QuadF(gfx::RectF(0, 10, 20, 10))));
}
...
TEST_F(LayoutTextTest, AbsoluteQuadsVRL)
{
    ...
    Vector<gfx::QuadF> quads;
    layout_text->AbsoluteQuads(quads);
    EXPECT_THAT(quads, testing::ElementsAre(gfx::QuadF(gfx::RectF(90, 30, 10, 30)), gfx::QuadF(gfx::RectF(80, 0, 10, 20))));
}
...
TEST_F(LayoutTextTest, PhysicalLinesBoundingBox)
{
    ...
    EXPECT_EQ(PhysicalRect(3, 6, 52, 13), To<LayoutText>(div.firstChild()->GetLayoutObject())->PhysicalLinesBoundingBox());
    EXPECT_EQ(PhysicalRect(55, 6, 39, 13), To<LayoutText>(one.firstChild()->GetLayoutObject())->PhysicalLinesBoundingBox());
    EXPECT_EQ(PhysicalRect(28, 25, 39, 13), To<LayoutText>(two.firstChild()->GetLayoutObject())->PhysicalLinesBoundingBox());
}

TEST_F(LayoutTextTest, PhysicalLinesBoundingBoxTextCombine)
{
    LoadAhem();
    InsertStyleElement("body { font: 100px/130px Ahem; }"
    ...
    EXPECT_EQ(PhysicalRect(-5, 0, 110, 100), text_01234.PhysicalLinesBoundingBox());
...
TEST_F(LayoutTextTest, PhysicalLinesBoundingBoxVerticalRL)
{
    ...
    const Element& one = *GetElementById("one");
    const Element& two = *GetElementById("two");
    EXPECT_EQ(PhysicalRect(25, 3, 13, 52), To<LayoutText>(div.firstChild()->GetLayoutObject())->PhysicalLinesBoundingBox());
    EXPECT_EQ(PhysicalRect(25, 55, 13, 39), To<LayoutText>(one.firstChild()->GetLayoutObject())->PhysicalLinesBoundingBox());
    EXPECT_EQ(PhysicalRect(6, 28, 13, 39), To<LayoutText>(two.firstChild()->GetLayoutObject())->PhysicalLinesBoundingBox());
}
...
TEST_F(LayoutTextTest, LocalSelectionRect)
{
    ...
    EXPECT_EQ(PhysicalRect(0, 0, 40, 20), GetSelectionRectFor("<div style='width: 2em'>f^oo ba|r</div>"));
    EXPECT_EQ(PhysicalRect(30, 0, 10, 10), GetSelectionRectFor("foo^<br id='target'>|bar"));
    EXPECT_EQ(PhysicalRect(10, 0, 20, 10), GetSelectionRectFor("f^oo<br>b|ar"));
    EXPECT_EQ(PhysicalRect(10, 0, 30, 10), GetSelectionRectFor("<div>f^oo</div><div>b|ar</div>"));
    EXPECT_EQ(PhysicalRect(30, 0, 10, 10), GetSelectionRectFor("foo^ |bar"));
    EXPECT_EQ(PhysicalRect(0, 0, 0, 0), GetSelectionRectFor("^ |foo"));
    EXPECT_EQ(PhysicalRect(0, 0, 0, 0), GetSelectionRectFor("fo^o<wbr id='target'>ba|r"));
    EXPECT_EQ(PhysicalRect(0, 0, 10, 10), GetSelectionRectFor("<style>:first-letter { float: right}</style>^fo|o"));
...
{
    LoadAhem();
    EXPECT_EQ(PhysicalRect(30, 0, 10, 10), GetSelectionRectFor("f^oo<br id='target'><br>ba|r"));
    EXPECT_EQ(PhysicalRect(0, 10, 10, 10), GetSelectionRectFor("f^oo<br><br id='target'>ba|r"));
}

TEST_F(LayoutTextTest, LocalSelectionRectLineBreakPre)
{
    LoadAhem();
    EXPECT_EQ(PhysicalRect(30, 0, 10, 10), GetSelectionRectFor("<div style='white-space:pre;'>foo^\n|\nbar</div>"));
    EXPECT_EQ(PhysicalRect(0, 10, 10, 10), GetSelectionRectFor("<div style='white-space:pre;'>foo\n^\n|bar</div>"));
}
...
{
    LoadAhem();
    EXPECT_EQ(PhysicalRect(0, 0, 20, 40),
        GetSelectionRectFor("<div style='writing-mode: vertical-lr; height: 2em'>"
                            "f^oo ba|r baz</div>"));
    EXPECT_EQ(PhysicalRect(10, 0, 20, 40),
...
TEST_F(LayoutTextTest, LocalSelectionRectVerticalRTL)
{
    ...
    EXPECT_EQ(PhysicalRect(0, -10, 20, 30),
        GetSelectionRectFor("<div style='writing-mode: vertical-lr; height: 2em' dir=rtl>"
                            "f^oo ba|r baz</div>"));
    EXPECT_EQ(PhysicalRect(10, -10, 20, 30),
        GetSelectionRectFor("<div style='writing-mode: vertical-rl; height: 2em' dir=rtl>"
                            "f^oo ba|r baz</div>"));
}
...
TEST_F(LayoutTextTest, PositionForPointAtLeading)
{
    ...
    EXPECT_EQ(Position(text->GetNode(), 1), text->PositionForPoint({ 10, 0 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), 1), text->PositionForPoint({ 10, 5 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), 1), text->PositionForPoint({ 10, 10 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), 1), text->PositionForPoint({ 10, 15 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), 1), text->PositionForPoint({ 10, 20 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), 1), text->PositionForPoint({ 10, 25 }).GetPosition());
    // line 2
    EXPECT_EQ(Position(text->GetNode(), 7), text->PositionForPoint({ 10, 30 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), 7), text->PositionForPoint({ 10, 35 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), 7), text->PositionForPoint({ 10, 40 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), 7), text->PositionForPoint({ 10, 45 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), 7), text->PositionForPoint({ 10, 50 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), 7), text->PositionForPoint({ 10, 55 }).GetPosition());
}
...
TEST_F(LayoutTextTest, FloatFirstLetterPlainText)
{
    ...
    LayoutText* text = To<LayoutText>(GetElementById("target")->firstChild()->GetLayoutObject());
...
TEST_F(LayoutTextTest, SetTextWithOffsetAppendEmojiWithZWJ)
{
    ...
    SetBodyInnerHTML(u"<pre id=target>&#x1F937;</pre>"
                     "<p id=checker>&#x1F937;&#x200D;&#x2640;&#xFE0F</p>");

    // Check whether we have "Woman Shrug glyph or not.
    const auto& checker = *To<LayoutText>(GetElementById("checker")->firstChild()->GetLayoutObject());
...
TEST_F(LayoutTextTest, SetTextWithOffsetInsertSameCharacters)
{
    ...
    EXPECT_EQ("*{'aaa', ShapeResult=0+3 width=150}\n"
...
TEST_F(LayoutTextTest, SetTextWithOffsetNoRelocation)
{
    ...
    EXPECT_EQ("LayoutText has NeedsCollectInlines", GetItemsAsString(*text.GetLayoutObject())) << "There are no optimization for setData()";
}
