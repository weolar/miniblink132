...
protected:
    void CheckQuoteLayoutObjectChildrenLang(const char* id, const char* lang, const char* parent_lang)
    {
        ...
        EXPECT_STREQ(child_before->SlowFirstChild()->StyleRef().Locale().Ascii().c_str(), parent_lang);

        const LayoutObject* child_after = o->SlowLastChild();
        ASSERT_EQ(child_after->StyleRef().StyleType(), PseudoId::kPseudoIdAfter);
        EXPECT_STREQ(child_after->SlowFirstChild()->StyleRef().Locale().Ascii().c_str(), parent_lang);
...
TEST_F(LayoutQuoteTest, Locale)
{
    ...
    // crbug.com/1366233
    To<CSSStyleSheet>(GetDocument().StyleSheets().item(0))->removeRule(0, ASSERT_NO_EXCEPTION);
