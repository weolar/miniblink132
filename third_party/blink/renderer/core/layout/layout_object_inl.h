...
void LayoutObject::MarkContainerChainForOverflowRecalcIfNeeded(bool mark_container_chain_scrollable_overflow_recalc)
{
    NOT_DESTROYED();
    ...
    do {
        ...
        object = object->IsTableCell() || object->IsTableRow() ? object->Parent() : object->Container();
        if (object) {
            bool already_needs_scrollable_overflow_recalc = false;
            if (mark_container_chain_scrollable_overflow_recalc) {
                already_needs_scrollable_overflow_recalc = object->ChildNeedsScrollableOverflowRecalc();
