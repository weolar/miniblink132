...
void PrepareMarginBoxSpaceBuilder(LogicalSize available_size, ConstraintSpaceBuilder* builder)
{
    ...
    builder->SetDecorationPercentageResolutionType(DecorationPercentageResolutionType::kContainingBlockSize);
...
LogicalRect SnappedBorderBoxRect(const LogicalRect& rect)
{
    ...
    return LogicalRect(LayoutUnit(rect.offset.inline_offset.Round()), LayoutUnit(rect.offset.block_offset.Round()), LayoutUnit(rect.size.inline_size.Ceil()),
        LayoutUnit(rect.size.block_size.Ceil()));
}

} // anonymous namespace

PageContainerLayoutAlgorithm::PageContainerLayoutAlgorithm(const LayoutAlgorithmParams& params, wtf_size_t page_index, wtf_size_t total_page_count,
    const AtomicString& page_name, const BlockNode& content_node, const CountersAttachmentContext& counters_context,
    const PageAreaLayoutParams& page_area_params, bool ignore_author_page_style, const PhysicalBoxFragment* existing_page_container)
...
const LayoutResult* PageContainerLayoutAlgorithm::Layout()
{
    ...
    BoxStrut margins;
    LogicalSize containing_block_size = DesiredPageContainingBlockSize(document, Style());
    ResolvePageBoxGeometry(Node(), containing_block_size, &unscaled_geometry, &margins);

    LogicalSize source_page_margin_box_size(
        unscaled_geometry.border_box_size.inline_size + margins.InlineSum(), unscaled_geometry.border_box_size.block_size + margins.BlockSum());
    LogicalRect target_page_border_box_rect = TargetPageBorderBoxLogicalRect(document, Style(), source_page_margin_box_size, margins);
    target_page_border_box_rect = SnappedBorderBoxRect(target_page_border_box_rect);
    ...
    // offset in the end.
    WritingModeConverter converter(Style().GetWritingDirection(), GetConstraintSpace().AvailableSize());
    PhysicalRect border_box_physical_rect = converter.ToPhysical(target_page_border_box_rect);
    ...
    border_box_physical_rect.size = converter.ToPhysical(unscaled_geometry.border_box_size);
    LogicalOffset target_offset = converter.ToLogical(border_box_physical_rect).offset;
    ...
    BoxStrut minimal_margins(GetConstraintSpace().AvailableSize(), target_page_border_box_rect);
...
void PageContainerLayoutAlgorithm::LayoutPageBorderBox(LogicalSize containing_block_size, LogicalOffset target_offset)
{
    if (existing_page_container_) {
        ...
        // re-layout.
        const PhysicalBoxFragment& page_border_box_fragment = GetPageBorderBox(*existing_page_container_);
        const LayoutResult* existing_border_box_result = page_border_box_fragment.OwnerLayoutBox()->GetLayoutResult(0);
    ...
    if (layout_scale != 1 && !ignore_author_page_style_) {
        ...
        content_scaled_style = document.GetStyleResolver().StyleForPage(page_index_, page_name_, layout_scale);
    }

    LayoutBlockFlow* page_border_box = document.View()->GetPaginationState()->CreateAnonymousPageLayoutObject(document, *content_scaled_style);
    BlockNode page_border_box_node(page_border_box);

    FragmentGeometry geometry;
    ResolvePageBoxGeometry(page_border_box_node, containing_block_size * layout_scale, &geometry);
    ...
    LayoutAlgorithmParams params(page_border_box_node, geometry, child_space,
        /*break_token=*/nullptr);
    PageBorderBoxLayoutAlgorithm child_algorithm(params, content_node_, page_area_params_);
...
void PageContainerLayoutAlgorithm::LayoutAllMarginBoxes(const BoxStrut& logical_margins)
{
    Document& document = Node().GetDocument();
    PageMarginsStyle margins_style;
    document.GetStyleResolver().StyleForPageMargins(Style(), page_index_, page_name_, &margins_style);
    ...
    PhysicalSize page_box_size = ToPhysicalSize(GetConstraintSpace().AvailableSize(), GetConstraintSpace().GetWritingMode());
    PhysicalBoxStrut margins = logical_margins.ConvertToPhysical(GetConstraintSpace().GetWritingDirection());
    LayoutUnit right_edge = page_box_size.width - margins.right;
    LayoutUnit bottom_edge = page_box_size.height - margins.bottom;

    PhysicalRect top_left_corner_rect(LayoutUnit(), LayoutUnit(), margins.left, margins.top);
    PhysicalRect top_right_corner_rect(right_edge, LayoutUnit(), margins.right, margins.top);
    PhysicalRect bottom_right_corner_rect(right_edge, bottom_edge, margins.right, margins.bottom);
    PhysicalRect bottom_left_corner_rect(LayoutUnit(), bottom_edge, margins.left, margins.bottom);
    PhysicalRect top_edge_rect(margins.left, LayoutUnit(), page_box_size.width - margins.HorizontalSum(), margins.top);
    PhysicalRect right_edge_rect(right_edge, margins.top, margins.right, page_box_size.height - margins.VerticalSum());
    PhysicalRect bottom_edge_rect(margins.left, bottom_edge, page_box_size.width - margins.HorizontalSum(), margins.bottom);
    PhysicalRect left_edge_rect(LayoutUnit(), margins.top, margins.left, page_box_size.height - margins.VerticalSum());
    ...
    LayoutCornerMarginNode(margins_style[PageMarginsStyle::TopLeftCorner], top_left_corner_rect, TopEdge | LeftEdge);
    LayoutEdgeMarginNodes(margins_style[PageMarginsStyle::TopLeft], margins_style[PageMarginsStyle::TopCenter], margins_style[PageMarginsStyle::TopRight],
        top_edge_rect, TopEdge);
    LayoutCornerMarginNode(margins_style[PageMarginsStyle::TopRightCorner], top_right_corner_rect, TopEdge | RightEdge);
    LayoutEdgeMarginNodes(margins_style[PageMarginsStyle::RightTop], margins_style[PageMarginsStyle::RightMiddle], margins_style[PageMarginsStyle::RightBottom],
        right_edge_rect, RightEdge);
    LayoutCornerMarginNode(margins_style[PageMarginsStyle::BottomRightCorner], bottom_right_corner_rect, BottomEdge | RightEdge);
    LayoutEdgeMarginNodes(margins_style[PageMarginsStyle::BottomLeft], margins_style[PageMarginsStyle::BottomCenter],
        margins_style[PageMarginsStyle::BottomRight], bottom_edge_rect, BottomEdge);
    LayoutCornerMarginNode(margins_style[PageMarginsStyle::BottomLeftCorner], bottom_left_corner_rect, BottomEdge | LeftEdge);
    LayoutEdgeMarginNodes(margins_style[PageMarginsStyle::LeftTop], margins_style[PageMarginsStyle::LeftMiddle], margins_style[PageMarginsStyle::LeftBottom],
        left_edge_rect, LeftEdge);

    container_builder_.SetFragmentsTotalBlockSize(container_builder_.FragmentBlockSize());
}

void PageContainerLayoutAlgorithm::LayoutCornerMarginNode(const ComputedStyle* corner_style, const PhysicalRect& rect, EdgeAdjacency edge_adjacency)
{
    ...
    ConstraintSpaceBuilder space_builder(GetConstraintSpace(), corner_style->GetWritingDirection(),
        /*is_new_fc=*/true);
    WritingModeConverter converter(Style().GetWritingDirection(), GetConstraintSpace().AvailableSize());
    ...
    PhysicalBoxStrut physical_margins = ResolveMargins(child_space, *corner_style, box_fragment.Size(), rect.size, edge_adjacency);
    BoxStrut logical_margins = physical_margins.ConvertToLogical(Style().GetWritingDirection());
    LogicalOffset offset = logical_rect.offset + logical_margins.StartOffset();
    container_builder_.AddResult(*result, offset);
}

void PageContainerLayoutAlgorithm::LayoutEdgeMarginNodes(const ComputedStyle* start_box_style, const ComputedStyle* center_box_style,
    const ComputedStyle* end_box_style, const PhysicalRect& edge_rect, EdgeAdjacency edge_adjacency)
{
    BlockNode nodes[3] = { CreateBlockNodeIfNeeded(start_box_style), CreateBlockNodeIfNeeded(center_box_style), CreateBlockNodeIfNeeded(end_box_style) };
    ...
    if (IsReverse(dir)) {
        LayoutEdgeMarginNode(nodes[EndMarginBox], edge_rect, main_axis_sizes[EndMarginBox], EndMarginBox, edge_adjacency, dir);
        LayoutEdgeMarginNode(nodes[CenterMarginBox], edge_rect, main_axis_sizes[CenterMarginBox], CenterMarginBox, edge_adjacency, dir);
        LayoutEdgeMarginNode(nodes[StartMarginBox], edge_rect, main_axis_sizes[StartMarginBox], StartMarginBox, edge_adjacency, dir);
    } else {
        LayoutEdgeMarginNode(nodes[StartMarginBox], edge_rect, main_axis_sizes[StartMarginBox], StartMarginBox, edge_adjacency, dir);
        LayoutEdgeMarginNode(nodes[CenterMarginBox], edge_rect, main_axis_sizes[CenterMarginBox], CenterMarginBox, edge_adjacency, dir);
        LayoutEdgeMarginNode(nodes[EndMarginBox], edge_rect, main_axis_sizes[EndMarginBox], EndMarginBox, edge_adjacency, dir);
...
BlockNode PageContainerLayoutAlgorithm::CreateBlockNodeIfNeeded(const ComputedStyle* page_margin_style)
{
    ...
    Document& document = Node().GetDocument();
    LayoutBlockFlow* margin_layout_box = document.View()->GetPaginationState()->CreateAnonymousPageLayoutObject(document, *page_margin_style);
        ...
        if (margin_layout_box->IsChildAllowed(child, *page_margin_style)) {
            ...
            if (auto* quote = DynamicTo<LayoutQuote>(child)) {
                ...
                if (counter_data->Identifier() == "pages") {
                    ...
                    values = counters_context_.GetCounterValues(*Node().GetLayoutBox(), counter->Identifier(), counter->Separator().IsNull());
                }
...
PageContainerLayoutAlgorithm::PreferredSizeInfo PageContainerLayoutAlgorithm::EdgeMarginNodePreferredSize(
    const BlockNode& child, LogicalSize containing_block_size, ProgressionDirection dir) const
{
    DCHECK(child);
    ConstraintSpaceBuilder space_builder(GetConstraintSpace(), child.Style().GetWritingDirection(),
        /*is_new_fc=*/true);

    space_builder.SetAvailableSize(containing_block_size);
    space_builder.SetDecorationPercentageResolutionType(DecorationPercentageResolutionType::kContainingBlockSize);
    ConstraintSpace child_space = space_builder.ToConstraintSpace();
    ...
    bool main_axis_is_inline_for_child = IsHorizontal(dir) == child.Style().IsHorizontalWritingMode();
    ...
    if (main_axis_is_inline_for_child) {
        ...
        if (main_axis_is_auto) {
            ConstraintSpaceBuilder intrinsic_space_builder(GetConstraintSpace(), child.Style().GetWritingDirection(),
                /*is_new_fc=*/true);
            intrinsic_space_builder.SetCacheSlot(LayoutResultCacheSlot::kMeasure);
            minmax = ComputeMinAndMaxContentContributionForSelf(child, intrinsic_space_builder.ToConstraintSpace()).sizes;
        } else {
            BoxStrut border_padding = ComputeBorders(child_space, child) + ComputePadding(child_space, child.Style());
        ...
        LogicalSize size = result->GetPhysicalFragment().Size().ConvertToLogical(child_space.GetWritingMode());
...
void PageContainerLayoutAlgorithm::CalculateEdgeMarginBoxSizes(
    PhysicalSize available_physical_size, const BlockNode nodes[3], ProgressionDirection dir, LayoutUnit final_main_axis_sizes[3]) const
{
    ...
    LogicalSize available_logical_size = available_physical_size.ConvertToLogical(Style().GetWritingMode());
    ...
    for (int i = 0; i < 3; i++) {
        ...
        preferred_main_axis_sizes[i] = EdgeMarginNodePreferredSize(nodes[i], available_logical_size, dir);
    ...
    if (nodes[CenterMarginBox]) {
        if (preferred_main_axis_sizes[CenterMarginBox].IsAuto()) {
            ...
            PreferredSizeInfo ac_sizes_for_start[3]
                = { preferred_main_axis_sizes[CenterMarginBox], PreferredSizeInfo(), preferred_main_axis_sizes[StartMarginBox].Doubled() };
            ...
            ResolveTwoEdgeMarginBoxLengths(ac_sizes_for_start, available_main_axis_size, &center_size1, &ignored_ac_size);
            ResolveTwoEdgeMarginBoxLengths(ac_sizes_for_end, available_main_axis_size, &center_size2, &ignored_ac_size);
            final_main_axis_sizes[CenterMarginBox] = std::min(center_size1, center_size2);
        }
        // Any auto start or end should receive half of the space not used by
        // center.
        LayoutUnit side_space = available_main_axis_size - final_main_axis_sizes[CenterMarginBox];
        ...
        ResolveTwoEdgeMarginBoxLengths(
            preferred_main_axis_sizes, available_main_axis_size, &final_main_axis_sizes[StartMarginBox], &final_main_axis_sizes[EndMarginBox]);
    }
...
void PageContainerLayoutAlgorithm::ResolveTwoEdgeMarginBoxLengths(const PreferredSizeInfo preferred_main_axis_sizes[3], LayoutUnit available_main_axis_size,
    LayoutUnit* first_main_axis_size, LayoutUnit* second_main_axis_size)
{
    ...
    for (int i = 0; i < 3; i++) {
        if (preferred_main_axis_sizes[i].IsAuto()) {
            ...
            // Fixed-size box.
            available_main_axis_size_for_flex -= preferred_main_axis_sizes[i].Length();
        }
    }
    ...
    if (available_main_axis_size_for_flex > total_auto_max_size) {
        ...
        if (flex_space > LayoutUnit()) {
            ...
            // length,
            for (int i = 0; i < 3; i++) {
                flex_factors[i] = preferred_main_axis_sizes[i].MaxLength() - preferred_main_axis_sizes[i].MinLength();
            }
        } else {
    ...
    *second_main_axis_size = unflexed_sizes[SecondResolvee];
    if (preferred_main_axis_sizes[FirstResolvee].IsAuto()) {
        if (preferred_main_axis_sizes[SecondResolvee].IsAuto()) {
            // Both have auto size.
            LayoutUnit total_flex = flex_factors[FirstResolvee] + flex_factors[SecondResolvee];
            if (total_flex > LayoutUnit()) {
                *first_main_axis_size += flex_space * flex_factors[FirstResolvee] / total_flex;
            }
        } else {
...
void PageContainerLayoutAlgorithm::LayoutEdgeMarginNode(const BlockNode& child, const PhysicalRect& edge_rect, LayoutUnit main_axis_size,
    EdgeMarginType edge_margin_type, EdgeAdjacency edge_adjacency, ProgressionDirection dir)
{
    if (!child) {
        return;
    }
    ...
    ConstraintSpaceBuilder space_builder(GetConstraintSpace(), child.Style().GetWritingDirection(),
        /*is_new_fc=*/true);
    LogicalSize available_size = edge_rect.size.ConvertToLogical(Style().GetWritingMode());
    bool main_axis_is_inline = IsHorizontal(dir) == Style().IsHorizontalWritingMode();
    ...
    const LayoutResult* result = child.Layout(child_space);
    const auto& box_fragment = To<PhysicalBoxFragment>(result->GetPhysicalFragment());
    PhysicalBoxStrut physical_margins = ResolveMargins(child_space, child.Style(), box_fragment.Size(), edge_rect.size, edge_adjacency);
    LayoutUnit main_axis_available_size;
    LayoutUnit main_axis_fragment_size;
    if (IsHorizontal(dir)) {
        main_axis_available_size = edge_rect.size.width;
        main_axis_fragment_size = box_fragment.Size().width + physical_margins.HorizontalSum();
    } else {
        main_axis_available_size = edge_rect.size.height;
        main_axis_fragment_size = box_fragment.Size().height + physical_margins.VerticalSum();
    }
    ...
    case CenterMarginBox:
        main_axis_offset = (main_axis_available_size - main_axis_fragment_size) / 2;
        break;
    case EndMarginBox:
    ...
    WritingModeConverter converter(Style().GetWritingDirection(), GetConstraintSpace().AvailableSize());
    LogicalOffset logical_offset = converter.ToLogical(offset, box_fragment.Size());
    container_builder_.AddResult(*result, logical_offset);
}

PhysicalBoxStrut PageContainerLayoutAlgorithm::ResolveMargins(const ConstraintSpace& child_space, const ComputedStyle& child_style, PhysicalSize child_size,
    PhysicalSize available_size, EdgeAdjacency edge_adjacency) const
{
    PhysicalBoxStrut margins = ComputePhysicalMargins(child_style, available_size);
    ...
    if (IsAtVerticalEdge(edge_adjacency)) {
        LayoutUnit additional_space = available_size.height - child_size.height - margins.VerticalSum();
        ResolveAutoMargins(child_style.MarginTop(), child_style.MarginBottom(), additional_space.ClampNegativeToZero(), &margins.top, &margins.bottom);
    ...
    if (IsAtHorizontalEdge(edge_adjacency)) {
        LayoutUnit additional_space = available_size.width - child_size.width - margins.HorizontalSum();
        ResolveAutoMargins(child_style.MarginLeft(), child_style.MarginRight(), additional_space.ClampNegativeToZero(), &margins.left, &margins.right);
