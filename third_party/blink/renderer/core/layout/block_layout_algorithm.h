...
struct InflowChildData {
    InflowChildData(BfcOffset bfc_offset_estimate, const MarginStrut& margin_strut, const BoxStrut& margins, bool is_pushed_by_floats = false)
...
struct BlockLineClampData {
    ...
    {
        return data.IsPastClampPoint();
    }

    bool ShouldHideForPaint() const
    {
        return data.ShouldHideForPaint();
    }
    ...
    void UpdateClampOffsetFromStyle(LayoutUnit clamp_bfc_offset, LayoutUnit content_edge)
    {
    ...
    bool UpdateAfterLayout(
        const LayoutResult* layout_result, LayoutUnit bfc_block_offset, const PreviousInflowPosition& previous_inflow_position, LayoutUnit block_end_padding)
    {
        if (data.state == LineClampData::kClampByLines) {
            ...
            if (IsPastClampPoint() && !previous_inflow_position_when_clamped.has_value()) {
        ...
        if (data.state == LineClampData::kMeasureLinesUntilBfcOffset) {
            ...
            collapsed_strut.positive_margin = std::max(collapsed_strut.positive_margin, end_margin_strut.positive_margin);
            collapsed_strut.quirky_positive_margin = std::max(collapsed_strut.quirky_positive_margin, end_margin_strut.quirky_positive_margin);
            collapsed_strut.negative_margin = std::max(collapsed_strut.negative_margin, end_margin_strut.negative_margin);
            ...
            if (previous_inflow_position.block_end_annotation_space < LayoutUnit()) {
                padding_annotation_overflow = std::max(previous_inflow_position.block_end_annotation_space, -block_end_padding);
            }

            LayoutUnit bfc_offset = bfc_block_offset + previous_inflow_position.logical_block_offset + padding_annotation_overflow
...
class CORE_EXPORT BlockLayoutAlgorithm : public LayoutAlgorithm<BlockNode, BoxFragmentBuilder, BlockBreakToken> {
public:
...
private:
    NOINLINE const LayoutResult* HandleNonsuccessfulLayoutResult(const LayoutResult*);

    NOINLINE const LayoutResult* LayoutInlineChild(const InlineNode& child);
    template <wtf_size_t capacity> NOINLINE const LayoutResult* LayoutWithOptimalInlineChildLayoutContext(const InlineNode& child);

    NOINLINE const LayoutResult* RelayoutIgnoringLineClamp();
    NOINLINE const LayoutResult* RelayoutWithLineClampBlockSize(int lines_until_clamp);
    NOINLINE const LayoutResult* RelayoutForTextBoxTrimEnd();

    inline const LayoutResult* Layout(InlineChildLayoutContext* inline_child_layout_context);

    const LayoutResult* FinishLayout(PreviousInflowPosition*, InlineChildLayoutContext*);
    ...
    LayoutUnit NextBorderEdge(const PreviousInflowPosition& previous_inflow_position) const
    {
        return BfcBlockOffset() + previous_inflow_position.logical_block_offset + previous_inflow_position.margin_strut.Sum();
    }

    BoxStrut CalculateMargins(LayoutInputNode child, bool is_new_fc, LayoutUnit* additional_line_offset);

    // Creates a new constraint space for the current child.
    ConstraintSpace CreateConstraintSpaceForChild(const LayoutInputNode child, const BreakToken* child_break_token, const InflowChildData& child_data,
        const LogicalSize child_available_size, bool is_new_fc, const std::optional<LayoutUnit> bfc_block_offset = std::nullopt,
        bool has_clearance_past_adjoining_floats = false, LayoutUnit block_start_annotation_space = LayoutUnit());

    // @return Estimated BFC block offset for the "to be layout" child.
    InflowChildData ComputeChildData(const PreviousInflowPosition&, LayoutInputNode, const BreakToken* child_break_token, bool is_new_fc);

    PreviousInflowPosition ComputeInflowPosition(const PreviousInflowPosition&, const LayoutInputNode child, const InflowChildData&,
        const std::optional<LayoutUnit>& child_bfc_block_offset, const LogicalOffset&, const LayoutResult&, const LogicalFragment&,
        bool self_collapsing_child_had_clearance);
    ...
    LayoutUnit PositionSelfCollapsingChildWithParentBfc(
        const LayoutInputNode& child, const ConstraintSpace& child_space, const InflowChildData& child_data, const LayoutResult&) const;
    ...
    bool TryReuseFragmentsFromCache(InlineNode child, PreviousInflowPosition*, const InlineBreakToken** inline_break_token_out);

    void HandleOutOfFlowPositioned(const PreviousInflowPosition&, BlockNode);
    void HandleFloat(const PreviousInflowPosition&, BlockNode, const BlockBreakToken*);
    ...
    LayoutResult::EStatus HandleNewFormattingContext(LayoutInputNode child, const BlockBreakToken* child_break_token, PreviousInflowPosition*);

    // Performs the actual layout of a new formatting context. This may be called
    // multiple times from HandleNewFormattingContext.
    const LayoutResult* LayoutNewFormattingContext(LayoutInputNode child, const BlockBreakToken* child_break_token, const InflowChildData&,
        BfcOffset origin_offset, bool abort_if_cleared, BfcOffset* out_child_bfc_offset, BoxStrut* out_resolved_margins);

    // Handle an in-flow child.
    // Returns false if we need to abort layout, because a previously unknown BFC
    // block offset has now been resolved. (Same as HandleNewFormattingContext).
    LayoutResult::EStatus HandleInflow(LayoutInputNode child, const BreakToken* child_break_token, PreviousInflowPosition*, InlineChildLayoutContext*,
        const InlineBreakToken** previous_inline_break_token);

    LayoutResult::EStatus FinishInflow(LayoutInputNode child, const BreakToken* child_break_token, const ConstraintSpace&,
        bool has_clearance_past_adjoining_floats, const LayoutResult*, InflowChildData*, PreviousInflowPosition*, InlineChildLayoutContext*,
        const InlineBreakToken** previous_inline_break_token);
    ...
    BreakStatus BreakBeforeChildIfNeeded(
        LayoutInputNode child, const LayoutResult&, PreviousInflowPosition*, LayoutUnit bfc_block_offset, bool has_container_separation);
    ...
    void PropagateBaselineFromLineBox(const PhysicalFragment& child, LayoutUnit block_offset);
    void PropagateBaselineFromBlockChild(const PhysicalFragment& child, const BoxStrut& margins, LayoutUnit block_offset);
    ...
    // true).
    bool ResolveBfcBlockOffset(PreviousInflowPosition*, LayoutUnit bfc_block_offset, const std::optional<LayoutUnit> forced_bfc_block_offset);

    // This passes in the |forced_bfc_block_offset| from the input constraints,
    // which is almost always desired.
    bool ResolveBfcBlockOffset(PreviousInflowPosition* previous_inflow_position, LayoutUnit bfc_block_offset)
    {
        return ResolveBfcBlockOffset(previous_inflow_position, bfc_block_offset, GetConstraintSpace().ForcedBfcBlockOffset());
    }
    ...
    {
        return ResolveBfcBlockOffset(previous_inflow_position, NextBorderEdge(*previous_inflow_position));
    }
    ...
    bool PositionOrPropagateListMarker(const LayoutResult&, LogicalOffset*, PreviousInflowPosition*);
    ...
    // function.
    std::optional<LayoutUnit> CalculateQuirkyBodyMarginBlockSum(const MarginStrut& end_margin_strut);
    ...
    LayoutUnit HandleTextControlPlaceholder(BlockNode placeholder, const PreviousInflowPosition& previous_inflow_position);
    ...
    // HTMLInputElement.
    LogicalOffset AdjustSliderThumbInlineOffset(const LogicalFragment& fragment, const LogicalOffset& logical_offset);

    bool ShouldTextBoxTrimStart() const
    {
        return should_text_box_trim_node_start_ || should_text_box_trim_fragmentainer_start_;
    }
    bool ShouldTextBoxTrimEnd() const
    {
        return should_text_box_trim_node_end_ || should_text_box_trim_fragmentainer_end_;
    }
