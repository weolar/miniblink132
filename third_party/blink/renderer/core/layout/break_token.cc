...
void AppendBreakTokenToString(const BreakToken* token, StringBuilder* string_builder, unsigned indent = 2)
{
    if (!token)
        return;
    ...
    if (auto* block_break_token = DynamicTo<BlockBreakToken>(token)) {
        ...
        if (auto* child_block_break_token = inline_break_token->GetBlockBreakToken()) {
            AppendBreakTokenToString(child_block_break_token, string_builder, indent + 2);
        }
    }
}
} // namespace
