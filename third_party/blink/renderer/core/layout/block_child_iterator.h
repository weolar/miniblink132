...
public:
    BlockChildIterator(LayoutInputNode first_child, const BlockBreakToken* break_token, bool calculate_child_idx = false);
    ...
    struct Entry;
    Entry NextChild(const InlineBreakToken* previous_inline_break_token = nullptr);
...
public:
    ...
    Entry(LayoutInputNode node, const BreakToken* token, std::optional<wtf_size_t> index = std::nullopt)
        : node(node)
        , token(token)
        , index(index)
    {
