...
static void CopyMarginProperties(ComputedStyleBuilder& placeholder_style_builder, const ComputedStyle& spanner_style)
{
...
LayoutMultiColumnSpannerPlaceholder* LayoutMultiColumnSpannerPlaceholder::CreateAnonymous(
    const ComputedStyle& parent_style, LayoutBox& layout_object_in_flow_thread)
{
    LayoutMultiColumnSpannerPlaceholder* new_spanner = MakeGarbageCollected<LayoutMultiColumnSpannerPlaceholder>(&layout_object_in_flow_thread);
...
LayoutMultiColumnSpannerPlaceholder::LayoutMultiColumnSpannerPlaceholder(LayoutBox* layout_object_in_flow_thread)
    : LayoutBox(nullptr)
...
void LayoutMultiColumnSpannerPlaceholder::LayoutObjectInFlowThreadStyleDidChange(const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    LayoutBox* object_in_flow_thread = layout_object_in_flow_thread_;
    if (FlowThread()->RemoveSpannerPlaceholderIfNoLongerValid(object_in_flow_thread)) {
        // No longer a valid spanner, due to style changes. |this| is now dead.
        if (object_in_flow_thread->StyleRef().HasOutOfFlowPosition() && !old_style->HasOutOfFlowPosition()) {
            ...
            object_in_flow_thread->Parent()->SetNeedsLayout(layout_invalidation_reason::kColumnsChanged);
        }
        return;
    }
...
void LayoutMultiColumnSpannerPlaceholder::UpdateProperties(const ComputedStyle& parent_style)
{
    NOT_DESTROYED();
    ComputedStyleBuilder new_style_builder = GetDocument().GetStyleResolver().CreateAnonymousStyleBuilderWithDisplay(parent_style, EDisplay::kBlock);
    CopyMarginProperties(new_style_builder, layout_object_in_flow_thread_->StyleRef());
...
void LayoutMultiColumnSpannerPlaceholder::InsertedIntoTree()
{
    ...
    layout_object_in_flow_thread_->SetNeedsLayoutAndIntrinsicWidthsRecalc(layout_invalidation_reason::kColumnsChanged);
}
...
void LayoutMultiColumnSpannerPlaceholder::WillBeRemovedFromTree()
{
    ...
    if (layout_object_in_flow_thread_) {
        ...
        // out.
        ex_spanner->SetNeedsLayoutAndIntrinsicWidthsRecalc(layout_invalidation_reason::kColumnsChanged);
    }
