...
const std::optional<PhysicalSize> LayoutEmbeddedContent::FrozenFrameSize() const
{
...
AffineTransform LayoutEmbeddedContent::EmbeddedContentTransform() const
{
    ...
    if (!frozen_size || frozen_size->IsEmpty()) {
        ...
        return AffineTransform().Translate(content_box_offset.left, content_box_offset.top);
    }
    ...
    translate_and_scale.Scale(replaced_rect.Width() / frozen_size->width, replaced_rect.Height() / frozen_size->height);
    return translate_and_scale;
}

PhysicalOffset LayoutEmbeddedContent::EmbeddedContentFromBorderBox(const PhysicalOffset& offset) const
{
    gfx::PointF point(offset);
    return PhysicalOffset::FromPointFRound(EmbeddedContentTransform().Inverse().MapPoint(point));
}

gfx::PointF LayoutEmbeddedContent::EmbeddedContentFromBorderBox(const gfx::PointF& point) const
{
    return EmbeddedContentTransform().Inverse().MapPoint(point);
}

PhysicalOffset LayoutEmbeddedContent::BorderBoxFromEmbeddedContent(const PhysicalOffset& offset) const
{
    gfx::PointF point(offset);
    return PhysicalOffset::FromPointFRound(EmbeddedContentTransform().MapPoint(point));
}

gfx::Rect LayoutEmbeddedContent::BorderBoxFromEmbeddedContent(const gfx::Rect& rect) const
{
...
bool LayoutEmbeddedContent::PointOverResizer(const HitTestResult& result, const HitTestLocation& location, const PhysicalOffset& accumulated_offset) const
{
    NOT_DESTROYED();
    if (const auto* scrollable_area = GetScrollableArea()) {
        const HitTestRequest::HitTestRequestType hit_type = result.GetHitTestRequest().GetType();
        const blink::ResizerHitTestType resizer_type = hit_type & HitTestRequest::kTouchEvent ? kResizerForTouch : kResizerForPointer;
        return scrollable_area->IsAbsolutePointInResizeControl(ToRoundedPoint(location.Point() - accumulated_offset), resizer_type);
    }
    return false;
}
...
bool LayoutEmbeddedContent::NodeAtPointOverEmbeddedContentView(
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    bool had_result = result.InnerNode();
    bool inside = LayoutReplaced::NodeAtPoint(result, hit_test_location, accumulated_offset, phase);
    ...
    if ((inside || hit_test_location.IsRectBasedTest()) && !had_result && result.InnerNode() == GetNode()) {
        bool is_over_content_view = PhysicalContentBoxRect().Contains(result.LocalPoint()) && !result.IsOverResizer();
    ...
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    bool skip_contents = (result.GetHitTestRequest().GetStopNode() == this || !result.GetHitTestRequest().AllowsChildFrameContent()
        || PointOverResizer(result, hit_test_location, accumulated_offset));

    if (!local_frame_view || skip_contents) {
        return NodeAtPointOverEmbeddedContentView(result, hit_test_location, accumulated_offset, phase);
    }
    ...
    if (local_frame_view->ShouldThrottleRendering() || !local_frame_view->GetFrame().GetDocument()
        || local_frame_view->GetFrame().GetDocument()->Lifecycle().GetState() < DocumentLifecycle::kPrePaintClean) {
        return NodeAtPointOverEmbeddedContentView(result, hit_test_location, accumulated_offset, phase);
    }
    ...
    if (phase == HitTestPhase::kForeground) {
        ...
        if (VisibleToHitTestRequest(result.GetHitTestRequest()) && child_layout_view) {
            PhysicalOffset content_offset(BorderLeft() + PaddingLeft(), BorderTop() + PaddingTop());
            HitTestLocation new_hit_test_location(hit_test_location, -accumulated_offset - content_offset);
            HitTestRequest new_hit_test_request(
                result.GetHitTestRequest().GetType() | HitTestRequest::kChildFrameHitTest, result.GetHitTestRequest().GetStopNode());
            HitTestResult child_frame_result(new_hit_test_request, new_hit_test_location);

            // The frame's layout and style must be up to date if we reach here.
            bool is_inside_child_frame = child_layout_view->HitTestNoLifecycleUpdate(new_hit_test_location, child_frame_result);
            ...
            if (is_inside_child_frame) {
                ...
                bool point_over_embedded_content_view
                    = NodeAtPointOverEmbeddedContentView(point_over_embedded_content_view_result, hit_test_location, accumulated_offset, phase);
...
void LayoutEmbeddedContent::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    if (old_style && new_style.VisibleToHitTesting() == old_style->VisibleToHitTesting()) {
        return;
    }
...
void LayoutEmbeddedContent::PaintReplaced(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
...
CursorDirective LayoutEmbeddedContent::GetCursor(const PhysicalOffset& point, ui::Cursor& cursor) const
{
    NOT_DESTROYED();
    if (Plugin()) {
...
PhysicalRect LayoutEmbeddedContent::ReplacedContentRectFrom(const PhysicalRect& base_content_rect) const
{
    NOT_DESTROYED();
    ...
    if (const std::optional<PhysicalSize> frozen_size = FrozenFrameSize()) {
        ...
        content_rect = ComputeReplacedContentRect(base_content_rect, &frozen_layout_size);
    }
...
void LayoutEmbeddedContent::UpdateGeometry(EmbeddedContentView& embedded_content_view)
{
    NOT_DESTROYED();
    ...
    TransformState transform_state(TransformState::kApplyTransformDirection, gfx::PointF(), gfx::QuadF(gfx::RectF(replaced_rect)));
    MapLocalToAncestor(nullptr, transform_state, 0);
    transform_state.Flatten();
    PhysicalOffset absolute_location = PhysicalOffset::FromPointFRound(transform_state.LastPlanarPoint());
    PhysicalRect absolute_replaced_rect = replaced_rect;
    absolute_replaced_rect.Move(absolute_location);
    gfx::RectF absolute_bounding_box = transform_state.LastPlanarQuad().BoundingBox();
    gfx::Rect frame_rect(gfx::Point(), ToPixelSnappedRect(absolute_replaced_rect).size());
