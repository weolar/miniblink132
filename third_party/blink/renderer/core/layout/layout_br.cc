...
PositionWithAffinity LayoutBR::PositionForPoint(const PhysicalOffset&) const
{
    ...
    DCHECK(!IsInLayoutNGInlineFormattingContext() || GetDocument().Lifecycle().GetState() >= DocumentLifecycle::kPrePaintClean);
...
Position LayoutBR::PositionForCaretOffset(unsigned offset) const
{
    ...
    DCHECK_LE(offset, 1u);
    DCHECK(GetNode());
    return offset ? Position::AfterNode(*GetNode()) : Position::BeforeNode(*GetNode());
}

std::optional<unsigned> LayoutBR::CaretOffsetForPosition(const Position& position) const
{
    NOT_DESTROYED();
