...
FragmentainerIterator::FragmentainerIterator(const LayoutFlowThread& flow_thread, const PhysicalRect& physical_bounding_box_in_flow_thread)
    : current_fragmentainer_group_index_(0)
{
    LogicalRect bounds_in_flow_thread = flow_thread.CreateWritingModeConverter().ToLogical(physical_bounding_box_in_flow_thread);
    ...
    current_column_set_ = flow_thread.ColumnSetAtBlockOffset(logical_top_in_flow_thread_, LayoutBox::kAssociateWithLatterPage);
    ...
    current_fragmentainer_group_index_
        = current_column_set_->FragmentainerGroupIndexAtFlowThreadOffset(logical_top_in_flow_thread_, LayoutBox::kAssociateWithLatterPage);
...
PhysicalRect FragmentainerIterator::ClipRectInFlowThread() const
{
    ...
    if (bounding_box_is_empty_) {
        clip_rect = CurrentGroup().FlowThreadPortionRectAt(current_fragmentainer_index_);
    } else {
        clip_rect = CurrentGroup().FlowThreadPortionOverflowRectAt(current_fragmentainer_index_);
    }
    return clip_rect;
}
...
{
    DCHECK(!AtEnd());
    return current_column_set_->FragmentainerGroups()[current_fragmentainer_group_index_];
}

void FragmentainerIterator::MoveToNextFragmentainerGroup()
{
    current_fragmentainer_group_index_++;
    if (current_fragmentainer_group_index_ >= current_column_set_->FragmentainerGroups().size()) {
        ...
        current_fragmentainer_group_index_ = 0;
        if (!current_column_set_ || current_column_set_->LogicalTopInFlowThread() >= logical_bottom_in_flow_thread_) {
            SetAtEnd();
            return; // No more sets or next set out of range. We're done.
        }
    }
    if (CurrentGroup().LogicalTopInFlowThread() >= logical_bottom_in_flow_thread_) {
...
void FragmentainerIterator::SetFragmentainersOfInterest()
{
    ...
    group.ColumnIntervalForBlockRangeInFlowThread(
        logical_top_in_flow_thread_, logical_bottom_in_flow_thread_, current_fragmentainer_index_, end_fragmentainer_index_);
