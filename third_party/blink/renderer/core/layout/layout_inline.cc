...
bool IsInChildRubyText(const LayoutInline& start_object, const LayoutObject* target)
{
...
void LayoutInline::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    if (!IsInLayoutNGInlineFormattingContext()) {
        if (!AlwaysCreateLineBoxes()) {
            bool always_create_line_boxes_new
                = HasSelfPaintingLayer() || HasBoxDecorationBackground() || new_style.MayHavePadding() || new_style.MayHaveMargin() || new_style.HasOutline();
            if (old_style && always_create_line_boxes_new) {
                SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kStyleChange);
            }
...
bool LayoutInline::ComputeInitialShouldCreateBoxFragment(const ComputedStyle& style) const
{
    NOT_DESTROYED();
    ...
    if (style.HasBoxDecorationBackground() || style.MayHavePadding() || style.MayHaveMargin())
        return true;
    ...
    return ComputeIsAbsoluteContainer(&style) || HasPaintedOutline(style, GetNode()) || CanBeHitTestTargetPseudoNodeStyle(style);
}
...
bool LayoutInline::ComputeInitialShouldCreateBoxFragment() const
{
    ...
    if (HasSelfPaintingLayer() || ComputeInitialShouldCreateBoxFragment(style) || ShouldApplyPaintContainment() || ShouldApplyLayoutContainment())
        return true;

    const ComputedStyle& first_line_style = FirstLineStyleRef();
    if (&style != &first_line_style && ComputeInitialShouldCreateBoxFragment(first_line_style)) [[unlikely]] {
        return true;
    }
...
PhysicalRect LayoutInline::LocalCaretRect(int) const
{
    ...
    LogicalRect logical_caret_rect = LocalCaretRectForEmptyElement(BorderAndPaddingInlineSize(), LayoutUnit());
    ...
    if (IsInLayoutNGInlineFormattingContext()) {
        ...
        if (cursor) {
            PhysicalRect caret_rect
                = WritingModeConverter({ StyleRef().GetWritingMode(), TextDirection::kLtr }, cursor.CurrentItem()->Size()).ToPhysical(logical_caret_rect);
            caret_rect.Move(cursor.Current().OffsetInContainerFragment());
            return caret_rect;
        }
    }

    return PhysicalRect(logical_caret_rect.offset.inline_offset, logical_caret_rect.offset.block_offset, logical_caret_rect.size.inline_size,
        logical_caret_rect.size.block_size);
}

void LayoutInline::AddChild(LayoutObject* new_child, LayoutObject* before_child)
{
    NOT_DESTROYED();
...
void LayoutInline::BlockInInlineBecameFloatingOrOutOfFlow(LayoutBlockFlow* anonymous_block_child)
{
    NOT_DESTROYED();
    // Look for in-flow children. Any in-flow child will prevent the wrapper from
    // being deleted.
    for (const LayoutObject* grandchild = anonymous_block_child->FirstChild(); grandchild; grandchild = grandchild->NextSibling()) {
...
void LayoutInline::AddChildIgnoringContinuation(LayoutObject* new_child, LayoutObject* before_child)
{
    NOT_DESTROYED();
    ...
    new_child->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kChildChanged);
}

void LayoutInline::AddChildAsBlockInInline(LayoutObject* new_child, LayoutObject* before_child)
{
    DCHECK(!new_child->IsInline());
    LayoutBlockFlow* anonymous_box;
    if (!before_child) {
        anonymous_box = DynamicTo<LayoutBlockFlow>(LastChild());
    } else if (before_child->IsInline() || before_child->IsFloatingOrOutOfFlowPositioned()) {
...
LayoutBlockFlow* LayoutInline::CreateAnonymousContainerForBlockChildren() const
{
    NOT_DESTROYED();
    ...
    ComputedStyleBuilder new_style_builder = GetDocument().GetStyleResolver().CreateAnonymousStyleBuilderWithDisplay(StyleRef(), EDisplay::kBlock);
    ...
    return LayoutBlockFlow::CreateAnonymous(&GetDocument(), new_style_builder.TakeStyle());
}

LayoutBox* LayoutInline::CreateAnonymousBoxToSplit(const LayoutBox* box_to_split) const
{
    NOT_DESTROYED();
...
template <typename PhysicalRectCollector> void LayoutInline::CollectLineBoxRects(const PhysicalRectCollector& yield) const
{
    NOT_DESTROYED();
    if (!IsInLayoutNGInlineFormattingContext()) {
...
bool LayoutInline::AbsoluteTransformDependsOnPoint(const LayoutObject& object) const
{
...
void LayoutInline::QuadsInAncestorInternal(Vector<gfx::QuadF>& quads, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode) const
{
    QuadsForSelfInternal(quads, ancestor, mode, true);
}

void LayoutInline::QuadsForSelfInternal(Vector<gfx::QuadF>& quads, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode, bool map_to_ancestor) const
{
    NOT_DESTROYED();
    ...
    auto PushAncestorQuad
        = [&transform_depends_on_point, &transform_depends_on_point_computed, &mapping_to_ancestor, &quads, ancestor, mode, this](const PhysicalRect& rect) {
              ...
              if (transform_depends_on_point) {
                  quads.push_back(LocalToAncestorQuad(gfx::QuadF(gfx::RectF(rect)), ancestor, mode));
              } else {
                  quads.push_back(mapping_to_ancestor->MapQuad(gfx::QuadF(gfx::RectF(rect))));
              }
          };

    CollectLineBoxRects([&PushAncestorQuad, &map_to_ancestor, &quads](const PhysicalRect& rect) {
        if (map_to_ancestor) {
...
std::optional<PhysicalOffset> LayoutInline::FirstLineBoxTopLeftInternal() const
{
    NOT_DESTROYED();
    if (IsInLayoutNGInlineFormattingContext()) {
        InlineCursor cursor;
...
PhysicalRect LayoutInline::AbsoluteBoundingBoxRectHandlingEmptyInline(MapCoordinatesFlags flags) const
{
    NOT_DESTROYED();
    Vector<PhysicalRect> rects = OutlineRects(nullptr, PhysicalOffset(), OutlineType::kIncludeBlockInkOverflow);
...
static LayoutUnit ComputeMargin(const LayoutInline* layout_object, const Length& margin)
{
    if (margin.IsFixed())
        return LayoutUnit(margin.Value());
    if (margin.IsPercent() || margin.IsCalculated()) {
        return MinimumValueForLength(margin, std::max(LayoutUnit(), layout_object->ContainingBlock()->AvailableLogicalWidth()));
    }
    return LayoutUnit();
}
...
bool LayoutInline::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    if (IsInLayoutNGInlineFormattingContext()) {
        ...
        DCHECK(!CanTraversePhysicalFragments() || target_fragment_idx >= 0 || !IsFragmented());
        ...
        for (; cursor; cursor.MoveToNextForSameLayoutObject()) {
            if (target_fragment_idx != -1 && wtf_size_t(target_fragment_idx) != cursor.ContainerFragmentIndex())
                continue;
            ...
            if (BoxFragmentPainter(cursor, item, *box_fragment, &inline_context)
                    .NodeAtPoint(result, hit_test_location, child_offset, accumulated_offset, phase)) {
                return true;
            }
        }
        return false;
...
bool LayoutInline::HitTestCulledInline(
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, const InlineCursor& parent_cursor)
{
    NOT_DESTROYED();
    ...
    if (intersected) {
        ...
        if (result.AddNodeToListBasedTestResult(GetNode(), adjusted_location, region_result) == kStopHitTesting)
            return true;
    }
    return false;
}

PositionWithAffinity LayoutInline::PositionForPoint(const PhysicalOffset& point) const
{
    NOT_DESTROYED();
...
PhysicalRect LayoutInline::VisualOverflowRect() const
{
    ...
    const ComputedStyle& style = StyleRef();
    LayoutUnit outline_outset(OutlinePainter::OutlineOutsetExtent(style, OutlineInfo::GetFromStyle(style)));
    if (outline_outset) {
        ...
        if (GetDocument().InNoQuirksMode()) {
            ...
            AddOutlineRectsForNormalChildren(collector, PhysicalOffset(), style.OutlineRectsShouldIncludeBlockInkOverflow());
        } else {
            ...
            AddOutlineRects(collector, nullptr, PhysicalOffset(), style.OutlineRectsShouldIncludeBlockInkOverflow());
        }
    ...
    const LayoutBoxModelObject* ancestor, TransformState& transform_state, VisualRectFlags visual_rect_flags) const
{
    ...
    if (container_box && container != ancestor && !container_box->MapContentsRectToBoxSpace(transform_state, accumulation, *this, visual_rect_flags))
        return false;

    return container->MapToVisualRectInAncestorSpaceInternal(ancestor, transform_state, visual_rect_flags);
}

PhysicalOffset LayoutInline::OffsetFromContainerInternal(const LayoutObject* container, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    ...
    if (container->IsScrollContainer()) {
        offset += OffsetFromScrollableContainer(container, mode & kIgnoreScrollOffset);
    }
...
{
    NOT_DESTROYED();
    return IsRelPositioned() || IsStickyPositioned() || CreatesGroup() || StyleRef().ShouldCompositeForCurrentAnimations() || ShouldApplyPaintContainment()
        ? kNormalPaintLayer
        : kNoPaintLayer;
}
...
void LayoutInline::UpdateHitTestResult(HitTestResult& result, const PhysicalOffset& point) const
{
    NOT_DESTROYED();
    if (result.InnerNode())
        return;
...
void LayoutInline::AddOutlineRects(
    OutlineRectCollector& collector, OutlineInfo* info, const PhysicalOffset& additional_offset, OutlineType include_block_overflows) const
{
    NOT_DESTROYED();
#if DCHECK_IS_ON()
    ...
    if (page && !page->GetSettings().GetSpatialNavigationEnabled()) {
        DCHECK_GE(GetDocument().Lifecycle().GetState(), DocumentLifecycle::kAfterPerformLayout);
    }
#endif // DCHECK_IS_ON()
    ...
    AddOutlineRectsForNormalChildren(collector, additional_offset, include_block_overflows);
    if (info) {
...
gfx::RectF LayoutInline::LocalBoundingBoxRectForAccessibility() const
{
    NOT_DESTROYED();
    UnionOutlineRectCollector collector;
    AddOutlineRects(collector, nullptr, PhysicalOffset(), OutlineType::kIncludeBlockInkOverflow);
...
void LayoutInline::AddDraggableRegions(Vector<DraggableRegionValue>& regions)
{
    ...
    region.bounds.offset += container->LocalToAbsolutePoint(PhysicalOffset(), kIgnoreTransforms);
    regions.push_back(region);
}

void LayoutInline::InvalidateDisplayItemClients(PaintInvalidationReason invalidation_reason) const
{
    NOT_DESTROYED();
