...
protected:
    ...
    {
        element->setAttribute(html_names::kStyleAttr, AtomicString(String::Format("height: %dpx", height)));
        Update();
    }
    ...
    void ScrollLayoutViewport(ScrollOffset delta)
    {
        ...
        if (delta.x()) {
            scrolling_element->setScrollLeft(scrolling_element->scrollLeft() + delta.x());
        }
        if (delta.y()) {
            scrolling_element->setScrollTop(scrolling_element->scrollTop() + delta.y());
        }
    }

    void ValidateSerializedAnchor(const String& expected_selector, const LogicalOffset& expected_offset)
    {
        SerializedAnchor serialized = GetScrollAnchor(LayoutViewport()).GetSerializedAnchor();
        ...
        StaticElementList* ele_list = GetDocument().QuerySelectorAll(AtomicString(serialized.selector));
    ...
    void MouseDownOnVerticalScrollbar(Scrollbar* scrollbar)
    {
        ...
        int thumb_center = scrollbar->GetTheme().ThumbPosition(*scrollbar) + scrollbar->GetTheme().ThumbLength(*scrollbar) / 2;
        scrollbar_drag_point_ = gfx::PointF(
            scrollbar->GetLayoutBox()->GetScrollableArea()->ConvertFromScrollbarToContainingEmbeddedContentView(*scrollbar, gfx::Point(0, thumb_center)));
        scrollbar->MouseDown(blink::WebMouseEvent(blink::WebInputEvent::Type::kMouseDown, *scrollbar_drag_point_, *scrollbar_drag_point_,
            blink::WebPointerProperties::Button::kLeft, 0, blink::WebInputEvent::kNoModifiers, base::TimeTicks::Now()));
    }
    ...
    void MouseDragVerticalScrollbar(Scrollbar* scrollbar, float scroll_delta_y)
    {
        ...
        scrollbar_drag_point_->Offset(0,
            scroll_delta_y * (scrollbar->GetTheme().TrackLength(*scrollbar) - scrollbar->GetTheme().ThumbLength(*scrollbar))
                / (scroller->MaximumScrollOffset().y() - scroller->MinimumScrollOffset().y()));
        scrollbar->MouseMoved(blink::WebMouseEvent(blink::WebInputEvent::Type::kMouseMove, *scrollbar_drag_point_, *scrollbar_drag_point_,
            blink::WebPointerProperties::Button::kLeft, 0, blink::WebInputEvent::kNoModifiers, base::TimeTicks::Now()));
    }
    ...
    {
        DCHECK(scrollbar_drag_point_);
        scrollbar->MouseDown(blink::WebMouseEvent(blink::WebInputEvent::Type::kMouseUp, *scrollbar_drag_point_, *scrollbar_drag_point_,
            blink::WebPointerProperties::Button::kLeft, 0, blink::WebInputEvent::kNoModifiers, base::TimeTicks::Now()));
...
TEST_F(ScrollAnchorTest, VisualViewportAnchors)
{
    ...
    int top = GetDocument().getElementById(AtomicString("text"))->GetBoundingClientRect()->top();
    v_viewport.SetLocation(gfx::PointF(0, top));

    SetHeight(GetDocument().getElementById(AtomicString("div")), 10);
    EXPECT_EQ(GetDocument().getElementById(AtomicString("text"))->GetLayoutObject(), GetScrollAnchor(l_viewport).AnchorObject());
...
TEST_F(ScrollAnchorTest, ClearScrollAnchorsOnAncestors)
{
    ...
    EXPECT_EQ(GetDocument().getElementById(AtomicString("anchor"))->GetLayoutObject(), GetScrollAnchor(viewport).AnchorObject());

    // Scrolling the nested scroller should clear the anchor on the main frame.
    ScrollableArea* scroller = ScrollerForElement(GetDocument().getElementById(AtomicString("scroller")));
...
TEST_F(ScrollAnchorTest, FractionalOffsetsAreRoundedBeforeComparing)
{
    ...
    GetDocument().getElementById(AtomicString("block1"))->setAttribute(html_names::kStyleAttr, AtomicString("height: 50.6px"));
    Update();
...
TEST_F(ScrollAnchorTest, AvoidStickyAnchorWhichMovesWithScroll)
{
    ...
    GetDocument().getElementById(AtomicString("block1"))->setAttribute(html_names::kStyleAttr, AtomicString("height: 100px"));
    Update();
...
TEST_F(ScrollAnchorTest, AnchorWithLayerInScrollingDiv)
{
    ...
    EXPECT_EQ(block2->GetLayoutObject(), GetScrollAnchor(scroller).AnchorObject());
...
TEST_F(ScrollAnchorTest, AnchorWhileDraggingScrollbar)
{
    ...
    Element* scroller_element = GetDocument().getElementById(AtomicString("scroller"));
...
TEST_F(ScrollAnchorTest, RemoveScrollerWithLayerInScrollingDiv)
{
    ...
    EXPECT_EQ(250, scroller->ScrollOffsetInt().y());
    EXPECT_EQ(anchor->GetLayoutObject(), GetScrollAnchor(scroller).AnchorObject());
    EXPECT_EQ(anchor->GetLayoutObject(), GetScrollAnchor(viewport).AnchorObject());
...
TEST_F(ScrollAnchorTest, FlexboxDelayedAdjustmentRespectsSANACLAP)
{
    ...
    GetDocument().getElementById(AtomicString("spacer"))->setAttribute(html_names::kStyleAttr, AtomicString("margin-top: 50px"));
    Update();
...
TEST_F(ScrollAnchorTest, SerializeAnchorUsesLeastSpecificSelector)
{
    ...
    ScrollLayoutViewport(ScrollOffset(0, 250));
    ValidateSerializedAnchor("#ancestor>:nth-child(3)>.foobar>div", LogicalOffset(0, -50));
}
...
TEST_F(ScrollAnchorTest, SerializeAnchorWithNoIdAttribute)
{
    ...
    ScrollLayoutViewport(ScrollOffset(0, 250));
    ValidateSerializedAnchor("html>body>div>:nth-child(3)>.foobar>div", LogicalOffset(0, -50));
}
...
TEST_F(ScrollAnchorTest, RestoreAnchorVerticalRlWritingMode)
{
    ...
    EXPECT_TRUE(GetScrollAnchor(LayoutViewport()).RestoreAnchor(serialized_anchor));
...
TEST_F(ScrollAnchorTest, SerializeAnchorIgnoresDuplicatedId)
{
    ...
    ScrollLayoutViewport(ScrollOffset(0, 150));
    ValidateSerializedAnchor("html>body>:nth-child(3)>.barbaz", LogicalOffset(0, -50));
}
...
TEST_F(ScrollAnchorTest, SerializeAnchorFailsForShadowDOMElement)
{
    ...
    LayoutObject* anchor_object = GetScrollAnchor(LayoutViewport()).AnchorObject();
    EXPECT_TRUE(anchor_object->GetNode()->IsInShadowTree());
}

TEST_F(ScrollAnchorTest, RestoreAnchorSimple)
{
    SetBodyInnerHTML("<style> body { height: 1000px; margin: 0; } div { height: 100px } "
                     "</style>"
                     "<div id='block1'>abc</div>"
                     "<div id='block2'>def</div>");
...
TEST_F(ScrollAnchorTest, RestoreAnchorNonTrivialSelector)
{
    ...
    SerializedAnchor serialized_anchor("#ancestor>:nth-child(3)>.foobar>div", LogicalOffset(0, -50));
...
TEST_F(ScrollAnchorTest, RestoreAnchorFailsForInvalidSelectors)
{
    ...
    EXPECT_FALSE(GetScrollAnchor(LayoutViewport()).RestoreAnchor(serialized_anchor));

    SerializedAnchor serialized_anchor_2("", LogicalOffset(0, 0));

    EXPECT_FALSE(GetScrollAnchor(LayoutViewport()).RestoreAnchor(serialized_anchor_2));

    SerializedAnchor serialized_anchor_3("foobar", LogicalOffset(0, 0));

    EXPECT_FALSE(GetScrollAnchor(LayoutViewport()).RestoreAnchor(serialized_anchor_3));
}
...
TEST_F(ScrollAnchorTest, ClampAdjustsAnchorAnimation)
{
    ...
    LayoutViewport()->SetScrollOffset(ScrollOffset(0, 2000), mojom::blink::ScrollType::kUser);
    Update();
    GetDocument().getElementById(AtomicString("hidden"))->setAttribute(html_names::kStyleAttr, AtomicString("display:block"));
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    EXPECT_EQ(gfx::Vector2d(0, 200), LayoutViewport()->GetScrollAnimator().ImplOnlyAnimationAdjustmentForTesting());
    GetDocument().getElementById(AtomicString("hidden"))->setAttribute(html_names::kStyleAttr, g_empty_atom);
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    // The clamping scroll after resizing scrollable overflow to be smaller
    // should adjust the animation back to 0.
    EXPECT_EQ(gfx::Vector2d(0, 0), LayoutViewport()->GetScrollAnimator().ImplOnlyAnimationAdjustmentForTesting());
}
...
public:
    ...
    void Find(String search_text, ScrollAnchorTestFindInPageClient& client, bool new_session = true)
    {
        client.Reset();
...
TEST_F(ScrollAnchorFindInPageTest, FindInPageResultPrioritized)
{
    ...
    LayoutViewport()->SetScrollOffset(ScrollOffset(0, 150), mojom::blink::ScrollType::kUser);
    ...
    auto* old_bounds = GetDocument().getElementById(AtomicString("target"))->GetBoundingClientRect();

    GetDocument().getElementById(AtomicString("growing"))->setAttribute(html_names::kStyleAttr, AtomicString("height: 3000px"));
    UpdateAllLifecyclePhasesForTest();

    auto* new_bounds = GetDocument().getElementById(AtomicString("target"))->GetBoundingClientRect();
...
TEST_F(ScrollAnchorFindInPageTest, FocusPrioritizedOverFindInPage)
{
    ...
    // Save the old bounds for comparison.
    auto* old_focus_bounds = GetDocument().getElementById(AtomicString("focus_target"))->GetBoundingClientRect();
    auto* old_find_bounds = GetDocument().getElementById(AtomicString("find_target"))->GetBoundingClientRect();

    GetDocument().getElementById(AtomicString("growing"))->setAttribute(html_names::kStyleAttr, AtomicString("height: 3000px"));
    UpdateAllLifecyclePhasesForTest();

    auto* new_focus_bounds = GetDocument().getElementById(AtomicString("focus_target"))->GetBoundingClientRect();
    auto* new_find_bounds = GetDocument().getElementById(AtomicString("find_target"))->GetBoundingClientRect();
...
TEST_F(ScrollAnchorFindInPageTest, FocusedUnderStickyIsSkipped)
{
    ...
    auto* old_bounds = GetDocument().getElementById(AtomicString("check"))->GetBoundingClientRect();

    GetDocument().getElementById(AtomicString("growing"))->setAttribute(html_names::kStyleAttr, AtomicString("height: 3000px"));
    UpdateAllLifecyclePhasesForTest();

    auto* new_bounds = GetDocument().getElementById(AtomicString("check"))->GetBoundingClientRect();
...
<div style="overflow:auto; columns:1; column-fill:auto; width:300px; height:350px;">
  ...
  <div id="scrollbarSummoner" style="display:none;">
    ...
    doc.getElementById(AtomicString("scrollbarSummoner"))->setAttribute(html_names::kStyleAttr, AtomicString("display:block; contain:size; height:0"));
