...
public:
    static const PhysicalBoxFragment* Create(BoxFragmentBuilder* builder, WritingMode block_or_line_writing_mode);
    ...
    static const PhysicalBoxFragment* CloneWithPostLayoutFragments(const PhysicalBoxFragment& other);

    using PassKey = base::PassKey<PhysicalBoxFragment>;
    PhysicalBoxFragment(PassKey, BoxFragmentBuilder* builder, bool has_scrollable_overflow, const PhysicalRect& scrollable_overflow,
    ...
    PhysicalBoxFragment(PassKey, const PhysicalBoxFragment& other, bool has_scrollable_overflow, const PhysicalRect& scrollable_overflow);
    ...
    base::span<const PhysicalFragmentLink> Children() const
    {
        ...
        return base::make_span(children_.data(), children_.size());
    }
    ...
    private:
        ...
        MutableChildrenForOutOfFlow(const PhysicalFragmentLink* buffer, wtf_size_t num_children)
    ...
    {
        return use_last_baseline_for_inline_baseline_ && IsScrollContainer() && !Style().ShouldIgnoreOverflowPropertyForInlineBlockBaseline();
    }
    ...
    const TableFragmentData::CollapsedBordersGeometry* TableCollapsedBordersGeometry() const
    {
        if (const auto* field = GetRareField(FieldId::kTableCollapsedBordersGeometry)) {
    ...
    {
        return rare_data_->GetField(FieldId::kTableCellColumnIndex)->table_cell_column_index;
    }
    ...
    PhysicalRect OverflowClipRect(const PhysicalOffset& location, OverlayScrollbarClipBehavior = kIgnoreOverlayScrollbarSize) const;
    PhysicalRect OverflowClipRect(
        const PhysicalOffset& location, const BlockBreakToken* incoming_break_token, OverlayScrollbarClipBehavior = kIgnoreOverlayScrollbarSize) const;
    gfx::Vector2d PixelSnappedScrolledContentOffset() const;
    PhysicalSize ScrollSize() const;

    InkOverflow::Type InkOverflowType() const
    {
        return static_cast<InkOverflow::Type>(bit_field_.get<InkOverflowTypeValue>());
    }
    bool IsInkOverflowComputed() const
    {
        return InkOverflowType() != InkOverflow::Type::kNotSet && InkOverflowType() != InkOverflow::Type::kInvalidated;
    }
    ...
    void AddSelfOutlineRects(
        const PhysicalOffset& additional_offset, OutlineType include_block_overflows, OutlineRectCollector& collector, LayoutObject::OutlineInfo* info) const;
    ...
    void AddOutlineRects(const PhysicalOffset& additional_offset, OutlineType include_block_overflows, OutlineRectCollector& collector) const;
    ...
    {
        return PhysicalBoxSides(IncludeBorderTop(), IncludeBorderRight(), IncludeBorderBottom(), IncludeBorderLeft());
    }
    ...
    {
        return bit_field_.get<IsFirstForNodeFlag>();
    }

    // Return true if this is the only fragment generated from a node.
    bool IsOnlyForNode() const
    {
        return IsFirstForNode() && !GetBreakToken();
    }
    ...
    {
        return bit_field_.get<IsMonolithicFlag>();
    }
...
#if DCHECK_IS_ON()
    void CheckSameForSimplifiedLayout(const PhysicalBoxFragment&, bool check_same_block_size, bool check_no_fragmentation) const;
#endif

    const FrameSetLayoutData* GetFrameSetLayoutData() const
    {
        return rare_data_->GetField(FieldId::kFrameSetLayoutData)->frame_set_layout_data.get();
    }
    ...
    public:
        MutableForStyleRecalc(base::PassKey<PhysicalBoxFragment>, PhysicalBoxFragment& fragment);
    ...
    public:
        MutableForContainerLayout(base::PassKey<PhysicalBoxFragment>, PhysicalBoxFragment& fragment);
    ...
    public:
        ...
        {
            fragment_.InvalidateInkOverflow();
        }
#endif
    ...
    public:
        ...
        void ClearPropagatedOOFs()
        {
            fragment_.ClearOofData();
        }
    ...
    public:
        ...
        void AddChildFragmentainer(const PhysicalBoxFragment& child_fragment, LogicalOffset child_offset);
...
private:
    ...
    using ConstHasFragmentItemsFlag = BitField::DefineFirstValue<bool, 1, WTF::BitFieldValueConstness::kConst>;
    using IsInlineFormattingContextFlag = ConstHasFragmentItemsFlag::DefineNextValue<bool, 1>;
    using IncludeBorderTopFlag = IsInlineFormattingContextFlag::DefineNextValue<bool, 1>;
    using IncludeBorderRightFlag = IncludeBorderTopFlag::DefineNextValue<bool, 1>;
    using IncludeBorderBottomFlag = IncludeBorderRightFlag::DefineNextValue<bool, 1>;
    using IncludeBorderLeftFlag = IncludeBorderBottomFlag::DefineNextValue<bool, 1>;
    using InkOverflowTypeValue = IncludeBorderLeftFlag::DefineNextValue<uint8_t, InkOverflow::kTypeBits>;
    using IsFirstForNodeFlag = InkOverflowTypeValue::DefineNextValue<bool, 1>;
    using HasDescendantsForTablePartFlag = IsFirstForNodeFlag::DefineNextValue<bool, 1>;
    using IsFragmentationContextRootFlag = HasDescendantsForTablePartFlag::DefineNextValue<bool, 1>;
    using IsMonolithicFlag = IsFragmentationContextRootFlag::DefineNextValue<bool, 1>;
    using IsMonolithicOverflowPropagationDisabledFlag = IsMonolithicFlag::DefineNextValue<bool, 1>;
    using HasMovedChildrenInBlockDirectionFlag = IsMonolithicOverflowPropagationDisabledFlag::DefineNextValue<bool, 1>;
    ...
    {
        return !!GetRareField(FieldId::kBorders);
    }
    bool HasPadding() const
    {
        return !!GetRareField(FieldId::kPadding);
    }
    ...
    ALWAYS_INLINE const PhysicalFragmentRareData::RareField* GetRareField(FieldId id) const
    {
        if (rare_data_) {
    ...
    {
        DCHECK(HasItems());
        return reinterpret_cast<const FragmentItems*>(base::bits::AlignUp(reinterpret_cast<const uint8_t*>(this + 1), alignof(FragmentItems)));
    }
    ...
    void AddOutlineRects(
        const PhysicalOffset& additional_offset, OutlineType include_block_overflows, bool inline_container_relative, OutlineRectCollector& collector) const;
    void AddOutlineRectsForInlineBox(
        PhysicalOffset additional_offset, OutlineType include_block_overflows, bool inline_container_relative, OutlineRectCollector& collector) const;

    PositionWithAffinity PositionForPointByClosestChild(PhysicalOffset point_in_contents) const;

    PositionWithAffinity PositionForPointInBlockFlowDirection(PhysicalOffset point_in_contents) const;

    PositionWithAffinity PositionForPointInTable(PhysicalOffset point_in_contents) const;

    PositionWithAffinity PositionForPointRespectingEditingBoundaries(const PhysicalBoxFragment& child, PhysicalOffset point_in_child) const;
...
template <> struct DowncastTraits<PhysicalBoxFragment> {
