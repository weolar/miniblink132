...
public:
    ...
    using MulticolCollection = HeapHashMap<Member<LayoutBox>, Member<MulticolWithPendingOofs<LogicalOffset>>>;
    ...
    {
        return writing_direction_.Direction();
    }
    ...
    const BreakToken* PreviousBreakToken() const
    {
    ...
    void SetBoxType(PhysicalFragment::BoxType box_type)
    {
        box_type_ = box_type;
    }
    bool IsFragmentainerBoxType() const
    {
        ...
        return box_type == PhysicalFragment::kColumnBox || box_type == PhysicalFragment::kPageArea;
    }
    ...
    void SetBlockSize(LayoutUnit block_size)
    {
    ...
    {
        return size_.block_size != kIndefiniteSize;
    }

    void SetIsHiddenForPaint(bool value)
    {
        is_hidden_for_paint_ = value;
    }
    void SetIsOpaque()
    {
        is_opaque_ = true;
    ...
    void SetHasCollapsedBorders(bool value)
    {
        has_collapsed_borders_ = value;
    }
    ...
    {
        return is_block_end_trimmable_line_;
    }
    void SetIsBlockEndTrimmableLine()
    {
        is_block_end_trimmable_line_ = true;
    }
    ...
    void ReplaceChild(wtf_size_t index, const PhysicalFragment& new_child, const LogicalOffset offset);
    ...
    void PropagateChildAnchors(const PhysicalFragment& child, const LogicalOffset& child_offset);
    ...
    void AddOutOfFlowChildCandidate(BlockNode, const LogicalOffset& child_offset, LogicalStaticPosition::InlineEdge = LogicalStaticPosition::kInlineStart,
        LogicalStaticPosition::BlockEdge = LogicalStaticPosition::kBlockStart, bool is_hidden_for_paint = false, bool allow_top_layer_nodes = false);
    ...
    void AddOutOfFlowInlineChildCandidate(
        BlockNode, const LogicalOffset& child_offset, TextDirection inline_container_direction, bool is_hidden_for_paint = false);

    void AddOutOfFlowFragmentainerDescendant(const LogicalOofNodeForFragmentation& descendant);
    void AddOutOfFlowFragmentainerDescendant(const LogicalOofPositionedNode& descendant);

    void AddOutOfFlowDescendant(const LogicalOofPositionedNode& descendant);

    void SwapOutOfFlowPositionedCandidates(HeapVector<LogicalOofPositionedNode>* candidates);
    ...
    void AddMulticolWithPendingOOFs(
        const BlockNode& multicol, MulticolWithPendingOofs<LogicalOffset>* multicol_info = MakeGarbageCollected<MulticolWithPendingOofs<LogicalOffset>>());

    void SwapMulticolsWithPendingOOFs(MulticolCollection* multicols_with_pending_oofs);
    ...
    void TransferOutOfFlowCandidates(
        FragmentBuilder* destination_builder, LogicalOffset additional_offset, const MulticolWithPendingOofs<LogicalOffset>* multicol = nullptr);
    ...
    LayoutUnit BlockOffsetAdjustmentForFragmentainer(LayoutUnit fragmentainer_consumed_block_size = LayoutUnit()) const;
    ...
    void SetHasOutOfFlowInFragmentainerSubtree(bool has_out_of_flow_in_fragmentainer_subtree)
    {
        has_out_of_flow_in_fragmentainer_subtree_ = has_out_of_flow_in_fragmentainer_subtree;
    }
    ...
    void PropagateOOFPositionedInfo(const PhysicalFragment& fragment, LogicalOffset offset, LogicalOffset relative_offset,
        LogicalOffset offset_adjustment = LogicalOffset(), const OofInlineContainer<LogicalOffset>* inline_container = nullptr,
        LayoutUnit containing_block_adjustment = LayoutUnit(), const OofContainingBlock<LogicalOffset>* containing_block = nullptr,
        const OofContainingBlock<LogicalOffset>* fixedpos_containing_block = nullptr,
        const OofInlineContainer<LogicalOffset>* fixedpos_inline_container = nullptr, LogicalOffset additional_fixedpos_offset = LogicalOffset());
    ...
    // builder.
    void PropagateOOFFragmentainerDescendants(const PhysicalFragment& fragment, LogicalOffset offset, LogicalOffset relative_offset,
        LayoutUnit containing_block_adjustment, const OofContainingBlock<LogicalOffset>* containing_block,
        const OofContainingBlock<LogicalOffset>* fixedpos_containing_block, HeapVector<LogicalOofNodeForFragmentation>* out_list = nullptr);

    void SetIsSelfCollapsing()
    {
    ...
    void SetIsBlockInInline()
    {
        is_block_in_inline_ = true;
    }
    void SetIsLineForParallelFlow()
    {
        is_line_for_parallel_flow_ = true;
    }

    void SetHasBlockFragmentation()
    {
    ...
    {
        return is_empty_spanner_parent_;
    }
    ...
    void PropagateTallestUnbreakableBlockSize(LayoutUnit unbreakable_block_size)
    {
        ...
        tallest_unbreakable_block_size_ = std::max(tallest_unbreakable_block_size_, unbreakable_block_size);
    }
...
protected:
    FragmentBuilder(const LayoutInputNode& node, const ComputedStyle* style, const ConstraintSpace& space, WritingDirectionMode writing_direction,
    ...
    void PropagateFromLayoutResultAndFragment(
        const LayoutResult&, LogicalOffset child_offset, LogicalOffset relative_offset, const OofInlineContainer<LogicalOffset>* = nullptr);

    void PropagateFromLayoutResult(const LayoutResult&);
    void PropagateScrollStartTarget(const PhysicalFragment& child);

    void PropagateFromFragment(const PhysicalFragment& child, LogicalOffset child_offset, LogicalOffset relative_offset,
    ...
    void AdjustFixedposContainerInfo(const PhysicalFragment* box_fragment, LogicalOffset relative_offset,
        OofInlineContainer<LogicalOffset>* fixedpos_inline_container, const PhysicalFragment** fixedpos_containing_block_fragment,
        const OofInlineContainer<LogicalOffset>* current_inline_container = nullptr) const;
