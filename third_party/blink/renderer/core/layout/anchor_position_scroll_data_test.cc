...
public:
    ...
    void ScrollTo(const char* id, int offset)
    {
        ...
        scroller->GetScrollableArea()->SetScrollOffset(ScrollOffset(0, offset), mojom::blink::ScrollType::kProgrammatic);
    }
};
...
TEST_F(AnchorPositionScrollDataTest, HasDataAndScrollAdjustment)
{
    ...
    EXPECT_FALSE(no_anchor_usage->GetAnchorPositionScrollData()->NeedsScrollAdjustment());

    const Element* no_anchor = GetElementById("no-anchor");
    EXPECT_TRUE(no_anchor->GetAnchorPositionScrollData());
    EXPECT_FALSE(no_anchor->GetAnchorPositionScrollData()->NeedsScrollAdjustment());

    const Element* not_anchor_positioned = GetElementById("not-anchor-positioned");
...
TEST_F(AnchorPositionScrollDataTest, Detach)
{
    ...
    remove->remove();
    display_none->setAttribute(html_names::kStyleAttr, AtomicString("display: none"));
    in_flow->setAttribute(html_names::kStyleAttr, AtomicString("position: static"));
...
TEST_F(AnchorPositionScrollDataTest, ScrollerSizeChange)
{
    ...
    EXPECT_TRUE(anchored->GetAnchorPositionScrollData());
    EXPECT_EQ(gfx::Vector2dF(0, 300), anchored->GetAnchorPositionScrollData()->AccumulatedAdjustment());
    ...
    SimulateFrame();
    EXPECT_TRUE(anchored->GetAnchorPositionScrollData());
    EXPECT_EQ(gfx::Vector2dF(0, 200), anchored->GetAnchorPositionScrollData()->AccumulatedAdjustment());
