...
typedef HeapHashMap<WeakMember<const LayoutBlock>, Member<TrackedLayoutBoxLinkedHashSet>> TrackedDescendantsMap;
...
protected:
    ...
    void AddChildBeforeDescendant(LayoutObject* new_child, LayoutObject* before_descendant);
...
public:
    ...
    static LayoutBlock* CreateAnonymousWithParentAndDisplay(const LayoutObject*, EDisplay = EDisplay::kBlock);
...
protected:
    bool HitTestChildren(HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset, HitTestPhase) override;
...
protected:
    PositionWithAffinity PositionForPointIfOutsideAtomicInlineLevel(const PhysicalOffset&) const;
