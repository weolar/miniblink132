...
bool LayoutMathMLBlock::IsChildAllowed(LayoutObject* child, const ComputedStyle&) const
{
...
void LayoutMathMLBlock::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    LayoutBlock::StyleDidChange(diff, old_style);
    if (!old_style)
        return;
    if (IsA<MathMLUnderOverElement>(GetNode()) && old_style->MathStyle() != StyleRef().MathStyle()) {
