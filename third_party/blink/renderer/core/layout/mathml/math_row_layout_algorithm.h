...
class CORE_EXPORT MathRowLayoutAlgorithm : public LayoutAlgorithm<BlockNode, BoxFragmentBuilder, BlockBreakToken> {
public:
    explicit MathRowLayoutAlgorithm(const LayoutAlgorithmParams& params);

    struct ChildWithOffsetAndMargins {
        DISALLOW_NEW();
        ChildWithOffsetAndMargins(const BlockNode& child, const BoxStrut& margins, LogicalOffset offset, const LayoutResult* result)
            : child(child)
            , margins(margins)
            , offset(offset)
...
private:
    void LayoutRowItems(ChildrenVector*, LayoutUnit* max_row_block_baseline, LogicalSize* row_total_size);
};

} // namespace blink

WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::MathRowLayoutAlgorithm::ChildWithOffsetAndMargins)
