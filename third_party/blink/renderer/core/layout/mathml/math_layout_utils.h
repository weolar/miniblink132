...
ConstraintSpace CreateConstraintSpaceForMathChild(const BlockNode& parent_node, const LogicalSize& child_available_size,
    const ConstraintSpace& parent_constraint_space, const LayoutInputNode&, const LayoutResultCacheSlot = LayoutResultCacheSlot::kLayout,
    const std::optional<ConstraintSpace::MathTargetStretchBlockSizes> target_stretch_block_sizes = std::nullopt,
...
MinMaxSizesResult ComputeMinAndMaxContentContributionForMathChild(
    const ComputedStyle& parent_style, const ConstraintSpace& parent_constraint_space, const BlockNode& child, LayoutUnit child_available_block_size);
...
inline std::optional<float> MathConstant(const ComputedStyle& style, OpenTypeMathSupport::MathConstants constant)
{
    const SimpleFontData* font_data = style.GetFont().PrimaryFont();
    return font_data ? OpenTypeMathSupport::MathConstant(font_data->PlatformData().GetHarfBuzzFace(), constant) : std::nullopt;
}
...
RadicalHorizontalParameters GetRadicalHorizontalParameters(const ComputedStyle&);
...
RadicalVerticalParameters GetRadicalVerticalParameters(const ComputedStyle&, bool has_index);

// https://w3c.github.io/mathml-core/#dfn-preferred-inline-size-of-a-glyph-stretched-along-the-block-axis
MinMaxSizes GetMinMaxSizesForVerticalStretchyOperator(const ComputedStyle&, UChar character);
...
std::optional<MathMLEmbellishedOperatorProperties> GetMathMLEmbellishedOperatorProperties(const BlockNode&);
