...
{
    return LayoutUnit(MathConstant(style, OpenTypeMathSupport::MathConstants::kDisplayOperatorMinHeight).value_or(0));
}

} // namespace

MathOperatorLayoutAlgorithm::MathOperatorLayoutAlgorithm(const LayoutAlgorithmParams& params)
...
const LayoutResult* MathOperatorLayoutAlgorithm::Layout()
{
    ...
    SimpleInlineChildLayoutContext context(To<InlineNode>(child), &container_builder_);
    const LayoutResult* child_layout_result = To<InlineNode>(child).Layout(GetConstraintSpace(), /* break_token */ nullptr,
    ...
    if (element->HasBooleanProperty(MathMLOperatorElement::kStretchy)) {
        ...
        if (!element->IsVertical()) {
            ...
            if (auto target_stretch_block_sizes = GetConstraintSpace().TargetStretchBlockSizes()) {
                ...
                if (element->HasBooleanProperty(MathMLOperatorElement::kSymmetric)) {
                    ...
                    // AxisHeight"
                    LayoutUnit half_target_stretch_size = std::max(target_stretch_ascent - axis, target_stretch_descent + axis);
                ...
                if (auto base_glyph = font_data->GlyphForCharacter(GetBaseCodePoint())) {
                ...
                LayoutUnit min_size
                    = (Style().GetMathMinSize().GetType() == Length::kAuto ? unstretched_size : ValueForLength(Style().GetMathMinSize(), unstretched_size))
                          .ClampNegativeToZero();
                // "If maxsize < minsize then set maxsize to minsize."
                LayoutUnit max_size
                    = std::max<LayoutUnit>((Style().GetMathMaxSize().GetType() == Length::kAuto ? LayoutUnit(LayoutUnit::kIntMax)
                ...
                if (operator_target_size <= LayoutUnit()) {
                    ...
                    target_stretch_ascent = (target_stretch_ascent - axis).ClampNegativeToZero().MulDiv(min_size, operator_target_size) + axis;
                    target_stretch_descent = min_size - target_stretch_ascent;
                } else if (max_size < operator_target_size) {
                    // "Otherwise, if maxsize < T
                    target_stretch_ascent = (target_stretch_ascent - axis).ClampNegativeToZero().MulDiv(max_size, operator_target_size) + axis;
    ...
    StretchyOperatorShaper shaper(
        GetBaseCodePoint(), element->IsVertical() ? OpenTypeMathStretchData::StretchAxis::Vertical : OpenTypeMathStretchData::StretchAxis::Horizontal);
    StretchyOperatorShaper::Metrics metrics;
    const ShapeResult* shape_result = shaper.Shape(&Style().GetFont(), operator_target_size, &metrics);
    const ShapeResultView* shape_result_view = ShapeResultView::Create(shape_result);

    if (metrics.italic_correction) {
        container_builder_.SetMathItalicCorrection(LayoutUnit(metrics.italic_correction));
    }
    ...
    container_builder_.SetMathMLPaintInfo(
        MakeGarbageCollected<MathMLPaintInfo>(GetBaseCodePoint(), shape_result_view, LayoutUnit(metrics.advance), operator_ascent, operator_descent));
    ...
    if (element->HasBooleanProperty(MathMLOperatorElement::kStretchy) && element->IsVertical()) {
        // "The stretchy glyph is shifted towards the line-under by a value Δ so
        // that its center aligns with the center of the target"
        LayoutUnit delta = ((operator_ascent - operator_descent) - (target_stretch_ascent - target_stretch_descent)) / 2;
        ascent -= delta;
        descent += delta;
    }
...
MinMaxSizesResult MathOperatorLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    MinMaxSizes sizes;
    // https://w3c.github.io/mathml-core/#layout-of-operators
    auto* element = DynamicTo<MathMLOperatorElement>(Node().GetDOMNode());
    if (element->HasBooleanProperty(MathMLOperatorElement::kStretchy)) {
        ...
        if (!element->IsVertical()) {
            ...
            StretchyOperatorShaper shaper(GetBaseCodePoint(), OpenTypeMathStretchData::Horizontal);
            StretchyOperatorShaper::Metrics metrics;
            shaper.Shape(&Style().GetFont(), 0, &metrics);
            sizes.Encompass(LayoutUnit(metrics.advance));
        } else {
            // "Otherwise, the stretch axis of the operator is block."
            sizes = GetMinMaxSizesForVerticalStretchyOperator(Style(), GetBaseCodePoint());
        }
    } else {
        // "If the operator has the largeop property and if math-style on the <mo>
        // element is normal."
        StretchyOperatorShaper shaper(GetBaseCodePoint(), OpenTypeMathStretchData::Vertical);
        StretchyOperatorShaper::Metrics metrics;
...
{
    return DynamicTo<MathMLOperatorElement>(Node().GetDOMNode())->GetTokenContent().code_point;
}
