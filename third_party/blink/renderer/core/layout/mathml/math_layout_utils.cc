...
ConstraintSpace CreateConstraintSpaceForMathChild(const BlockNode& parent_node, const LogicalSize& child_available_size, const ConstraintSpace& parent_space,
    const LayoutInputNode& child, LayoutResultCacheSlot cache_slot,
    const std::optional<ConstraintSpace::MathTargetStretchBlockSizes> target_stretch_block_sizes, const std::optional<LayoutUnit> target_stretch_inline_size)
{
    const ComputedStyle& parent_style = parent_node.Style();
    const ComputedStyle& child_style = child.Style();
    DCHECK(child.CreatesNewFormattingContext());
    ConstraintSpaceBuilder builder(parent_space, child_style.GetWritingDirection(), true /* is_new_fc */);
...
MinMaxSizesResult ComputeMinAndMaxContentContributionForMathChild(
    const ComputedStyle& parent_style, const ConstraintSpace& parent_space, const BlockNode& child, LayoutUnit child_available_block_size)
{
    DCHECK(child.CreatesNewFormattingContext());
    MinMaxConstraintSpaceBuilder builder(parent_space, parent_style, child, true /* is_new_fc */);
    ...
    result.sizes += ComputeMarginsFor(space, child.Style(), parent_space).InlineSum();
...
RadicalHorizontalParameters GetRadicalHorizontalParameters(const ComputedStyle& style)
{
    RadicalHorizontalParameters parameters;
    parameters.kern_before_degree
        = LayoutUnit(MathConstant(style, OpenTypeMathSupport::MathConstants::kRadicalKernBeforeDegree).value_or(5 * style.FontSize() * kMathUnitFraction));
    parameters.kern_after_degree
        = LayoutUnit(MathConstant(style, OpenTypeMathSupport::MathConstants::kRadicalKernAfterDegree).value_or(-10 * style.FontSize() * kMathUnitFraction));
    return parameters;
}

RadicalVerticalParameters GetRadicalVerticalParameters(const ComputedStyle& style, bool has_index)
{
    ...
    parameters.rule_thickness = LayoutUnit(MathConstant(style, OpenTypeMathSupport::MathConstants::kRadicalRuleThickness).value_or(rule_thickness));
    parameters.vertical_gap = LayoutUnit(MathConstant(
        style, has_display ? OpenTypeMathSupport::MathConstants::kRadicalDisplayStyleVerticalGap : OpenTypeMathSupport::MathConstants::kRadicalVerticalGap)
                                             .value_or(has_display ? rule_thickness + x_height / 4 : 5 * rule_thickness / 4));
    parameters.extra_ascender = LayoutUnit(MathConstant(style, OpenTypeMathSupport::MathConstants::kRadicalExtraAscender).value_or(parameters.rule_thickness));
    if (has_index) {
        parameters.degree_bottom_raise_percent = MathConstant(style, OpenTypeMathSupport::MathConstants::kRadicalDegreeBottomRaisePercent).value_or(.6);
    }
    return parameters;
}

MinMaxSizes GetMinMaxSizesForVerticalStretchyOperator(const ComputedStyle& style, UChar character)
{
    ...
    if (auto base_glyph = font_data->GlyphForCharacter(character)) {
        ...
        const HarfBuzzFace* harfbuzz_face = font_data->PlatformData().GetHarfBuzzFace();

        for (auto& variant : OpenTypeMathSupport::GetGlyphVariantRecords(harfbuzz_face, base_glyph, OpenTypeMathStretchData::Vertical)) {
            sizes.Encompass(LayoutUnit(font_data->WidthForGlyph(variant)));
        }

        for (auto& part : OpenTypeMathSupport::GetGlyphPartRecords(harfbuzz_face, base_glyph, OpenTypeMathStretchData::StretchAxis::Vertical)) {
...
bool IsOperatorWithSpecialShaping(const BlockNode& node)
{
    ...
    if (auto* element = DynamicTo<MathMLOperatorElement>(node.GetDOMNode())) {
        ...
        if (base_code_point == kNonCharacter || !node.Style().GetFont().PrimaryFont()
            || !node.Style().GetFont().PrimaryFont()->GlyphForCharacter(base_code_point))
            return false;

        if (element->HasBooleanProperty(MathMLOperatorElement::kStretchy))
            return true;

        if (element->HasBooleanProperty(MathMLOperatorElement::kLargeOp) && HasDisplayStyle(node.Style()))
            return true;
    }
    return false;
}
...
{
    return LayoutUnit(MathConstant(style, OpenTypeMathSupport::MathConstants::kFractionRuleThickness).value_or(RuleThicknessFallback(style)));
}
...
LayoutUnit MathAxisHeight(const ComputedStyle& style)
{
    const SimpleFontData* font_data = style.GetFont().PrimaryFont();
    float x_height = font_data ? font_data->GetFontMetrics().XHeight() : 0;
    return LayoutUnit(MathConstant(style, OpenTypeMathSupport::MathConstants::kAxisHeight).value_or(x_height / 2));
}

LayoutUnit FractionLineThickness(const ComputedStyle& style)
{
    return std::max<LayoutUnit>(ValueForLength(style.GetMathFractionBarThickness(), DefaultFractionLineThickness(style)), LayoutUnit());
}

LayoutUnit MathTableBaseline(const ComputedStyle& style, LayoutUnit block_offset)
{
...
static bool IsSpaceLike(const BlockNode& node)
{
    ...
    if (element && (element->HasTagName(mathml_names::kMtextTag) || element->HasTagName(mathml_names::kMspaceTag)))
        return true;
    ...
    if ((element && (element->IsGroupingElement() || element->HasTagName(mathml_names::kMpaddedTag))) || node.IsAnonymous()) {
        for (auto child = To<BlockNode>(FirstChildInFlow(node)); child; child = To<BlockNode>(NextSiblingInFlow(child))) {
...
MathMLOperatorElement* GetCoreOperator(const BlockNode& node)
{
    ...
    if (element && (IsA<MathMLScriptsElement>(element) || element->HasTagName(mathml_names::kMfracTag))) {
        ...
        return IsA<BlockNode>(first_child) ? GetCoreOperator(To<BlockNode>(first_child)) : nullptr;
    }
    if ((element && (element->IsGroupingElement() || element->HasTagName(mathml_names::kMpaddedTag))) || node.IsAnonymous()) {
...
std::optional<MathMLEmbellishedOperatorProperties> GetMathMLEmbellishedOperatorProperties(const BlockNode& node)
{
    auto* core_operator = GetCoreOperator(node);
    if (!core_operator)
        return std::nullopt;
    DCHECK(core_operator->GetLayoutObject());
    const auto& core_operator_style = core_operator->GetLayoutObject()->StyleRef();

    MathMLEmbellishedOperatorProperties properties;

    properties.has_movablelimits = core_operator->HasBooleanProperty(MathMLOperatorElement::kMovableLimits);

    properties.is_stretchy = core_operator->HasBooleanProperty(MathMLOperatorElement::kStretchy);

    properties.is_large_op = core_operator->HasBooleanProperty(MathMLOperatorElement::kLargeOp);

    properties.is_vertical = core_operator->IsVertical();

    LayoutUnit leading_space(core_operator->DefaultLeadingSpace() * core_operator_style.FontSize());
    properties.lspace = ValueForLength(core_operator_style.GetMathLSpace(), leading_space).ClampNegativeToZero();

    LayoutUnit trailing_space(core_operator->DefaultTrailingSpace() * core_operator_style.FontSize());
    properties.rspace = ValueForLength(core_operator_style.GetMathRSpace(), trailing_space).ClampNegativeToZero();
...
bool IsStretchyOperator(const BlockNode& node, bool stretch_axis_is_vertical)
{
    ...
    return properties && properties->is_stretchy && properties->is_vertical == stretch_axis_is_vertical;
}
