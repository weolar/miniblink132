...
{
    if (params.print_scaling_option == printing::mojom::blink::PrintScalingOption::kCenterShrinkToFitPaper) {
        return true;
    }
    DCHECK(params.print_scaling_option == printing::mojom::blink::PrintScalingOption::kSourceSize);
    return false;
}
...
LogicalSize PageBoxDefaultSizeWithSourceOrientation(const Document& document, const ComputedStyle& style, LogicalSize layout_size)
{
    DCHECK(ShouldCenterPageOnPaper(document.GetFrame()->GetPrintParams()));
    LogicalSize target_size = PageBoxDefaultSize(document).ConvertToLogical(style.GetWritingMode());
    if (layout_size.inline_size != layout_size.block_size
        && (target_size.inline_size > target_size.block_size) != (layout_size.inline_size > layout_size.block_size)) {
...
float TargetShrinkScaleFactor(LogicalSize target_size, LogicalSize source_size)
{
    if (source_size.IsEmpty()) {
        return 1.0f;
    }
    float inline_scale = target_size.inline_size.ToFloat() / source_size.inline_size.ToFloat();
    float block_scale = target_size.block_size.ToFloat() / source_size.block_size.ToFloat();
...
void SetUpSpaceBuilderForPageBox(LogicalSize available_size, ConstraintSpaceBuilder* builder)
{
...
LogicalSize DesiredPageContainingBlockSize(const Document& document, const ComputedStyle& style)
{
    ...
    case PageSizeType::kFixed: {
        ...
        if (document.GetFrame()->GetPrintParams().ignore_page_size) {
            ...
            if ((css_size.width > css_size.height) != (layout_size.width > layout_size.height)) {
                std::swap(layout_size.width, layout_size.height);
            }
            break;
        }
...
void ResolvePageBoxGeometry(const BlockNode& page_box, LogicalSize page_containing_block_size, FragmentGeometry* geometry, BoxStrut* margins)
{
    const ComputedStyle& style = page_box.Style();
    ConstraintSpaceBuilder space_builder(style.GetWritingMode(), style.GetWritingDirection(),
        /* is_new_fc */ true);
    ...
    *margins = ComputeMarginsForSelf(space, style);
    ...
    LayoutUnit additional_inline_space = space.AvailableSize().inline_size - (geometry->border_box_size.inline_size + margins->InlineSum());
    LayoutUnit additional_block_space = space.AvailableSize().block_size - (geometry->border_box_size.block_size + margins->BlockSum());
    ResolveAutoMargins(style.MarginInlineStart(), style.MarginInlineEnd(), style.MarginBlockStart(), style.MarginBlockEnd(), additional_inline_space,
        additional_block_space, margins);
}

PhysicalSize CalculateInitialContainingBlockSizeForPagination(Document& document)
{
    ...
    if (const PhysicalBoxFragment* first_page = GetPageContainer(layout_view, 0)) {
        ...
        page_style = document.GetStyleResolver().StyleForPage(0, /*page_name=*/g_null_atom);
    }
    ...
    LogicalSize containing_block_size = DesiredPageContainingBlockSize(document, *page_style);
    ResolvePageBoxGeometry(temporary_page_node, containing_block_size, &geometry);
    LogicalSize logical_size = ShrinkLogicalSize(geometry.border_box_size, geometry.border + geometry.padding);
    ...
    return ToPhysicalSize(logical_size, writing_mode) * layout_view.PaginationScaleFactor();
}
...
float TargetScaleForPage(const PhysicalBoxFragment& page_container)
{
    ...
    FragmentGeometry geometry;
    BoxStrut margins;
    ResolvePageBoxGeometry(page_node, DesiredPageContainingBlockSize(document, style), &geometry, &margins);
    LogicalSize source_size = geometry.border_box_size + margins;
    LogicalSize target_size = page_container.Size().ConvertToLogical(style.GetWritingMode());

    return layout_scale * TargetShrinkScaleFactor(target_size, source_size);
}

LogicalSize FittedPageContainerSize(const Document& document, const ComputedStyle& style, LogicalSize source_margin_box_size)
{
    if (!ShouldCenterPageOnPaper(document.GetFrame()->GetPrintParams())) {
    ...
    return PageBoxDefaultSizeWithSourceOrientation(document, style, source_margin_box_size);
}

LogicalRect TargetPageBorderBoxLogicalRect(
    const Document& document, const ComputedStyle& style, const LogicalSize& source_margin_box_size, const BoxStrut& margins)
{
    LogicalSize source_border_box_size(source_margin_box_size.inline_size - margins.InlineSum(), source_margin_box_size.block_size - margins.BlockSum());
    LogicalRect rect(LogicalOffset(margins.inline_start, margins.block_start), source_border_box_size);

    if (!ShouldCenterPageOnPaper(document.GetFrame()->GetPrintParams())) {
        return rect;
    }

    LogicalSize target_size = PageBoxDefaultSizeWithSourceOrientation(document, style, source_margin_box_size);

    float scale = TargetShrinkScaleFactor(target_size, source_margin_box_size);

    rect.offset.inline_offset = LayoutUnit(
        rect.offset.inline_offset.ToFloat() * scale + (target_size.inline_size.ToFloat() - source_margin_box_size.inline_size.ToFloat() * scale) / 2);
    rect.offset.block_offset
        = LayoutUnit(rect.offset.block_offset.ToFloat() * scale + (target_size.block_size.ToFloat() - source_margin_box_size.block_size.ToFloat() * scale) / 2);
...
const PhysicalBoxFragment* GetPageContainer(const LayoutView& view, wtf_size_t page_index)
{
...
const PhysicalBoxFragment* GetPageArea(const LayoutView& view, wtf_size_t page_index)
{
...
const PhysicalFragmentLink& GetPageBorderBoxLink(const PhysicalBoxFragment& page_container)
{
    DCHECK_EQ(page_container.GetBoxType(), PhysicalFragment::kPageContainer);
...
const PhysicalBoxFragment& GetPageBorderBox(const PhysicalBoxFragment& page_container)
{
    return *To<PhysicalBoxFragment>(GetPageBorderBoxLink(page_container).get());
}

const PhysicalBoxFragment& GetPageArea(const PhysicalBoxFragment& page_border_box)
{
    DCHECK_EQ(page_border_box.GetBoxType(), PhysicalFragment::kPageBorderBox);
    DCHECK_EQ(page_border_box.Children().size(), 1u);
    const auto& page_area = *DynamicTo<PhysicalBoxFragment>(page_border_box.Children()[0].get());
    DCHECK_EQ(page_area.GetBoxType(), PhysicalFragment::kPageArea);
    return page_area;
}

PhysicalRect StitchedPageContentRect(const LayoutView& layout_view, wtf_size_t page_index)
{
...
PhysicalRect StitchedPageContentRect(const PhysicalBoxFragment& page_container)
{
    ...
    if (const BlockBreakToken* previous_break_token = FindPreviousBreakTokenForPageArea(page_area)) {
        LayoutUnit consumed_block_size = previous_break_token->ConsumedBlockSize();
        PhysicalDirection block_end = page_container.Style().GetWritingDirection().BlockEnd();
...
const BlockBreakToken* FindPreviousBreakTokenForPageArea(const PhysicalBoxFragment& page_area)
{
    DCHECK_EQ(page_area.GetBoxType(), PhysicalFragment::kPageArea);
...
float CalculateOverflowShrinkForPrinting(const LayoutView& view, float maximum_shrink_factor)
{
    float overall_scale_factor = 1.0;
    for (const PhysicalFragmentLink& link : view.GetPhysicalFragment(0)->Children()) {
        ...
        for (const PhysicalFragmentLink& child : page_container.Children()) {
            if (child->GetBoxType() == PhysicalFragment::kPageBorderBox) {
                ...
                if (view.StyleRef().IsHorizontalWritingMode()) {
                    page_scale_factor = page.ScrollableOverflow().Right().ToFloat() / page.Size().width.ToFloat();
                } else {
                    page_scale_factor = page.ScrollableOverflow().Bottom().ToFloat() / page.Size().height.ToFloat();
                }
...
WebPrintPageDescription GetPageDescriptionFromLayout(const Document& document, wtf_size_t page_number)
{
    const PhysicalBoxFragment& page_container = *GetPageContainer(*document.GetLayoutView(), page_number);
    ...
    PhysicalRect page_border_box_rect(border_box.offset, border_box->Size() * scale);
    ...
    PhysicalRect top_edge_rect(LayoutUnit(), LayoutUnit(), page_container.Size().width, insets.top);
    PhysicalRect right_edge_rect(insets.left + page_border_box_rect.Width(), LayoutUnit(), insets.right, page_container.Size().height);
    PhysicalRect bottom_edge_rect(LayoutUnit(), insets.top + page_border_box_rect.Height(), page_container.Size().width, insets.bottom);
    PhysicalRect left_edge_rect(LayoutUnit(), LayoutUnit(), insets.left, page_container.Size().height);
