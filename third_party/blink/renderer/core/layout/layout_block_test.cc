...
{
    auto* element = MakeGarbageCollected<Element>(QualifiedName(AtomicString("div")), &GetDocument());
...
TEST_F(LayoutBlockTest, WidthAvailableToChildrenChanged)
{
    ...
    ASSERT_EQ(item_element->OffsetWidth(), 150 - list_box->ComputeScrollbars().HorizontalSum());

    DummyExceptionStateForTesting exception_state;
    list_element->style()->setCSSText(GetDocument().GetExecutionContext(), "width:150px;height:100px;", exception_state);
...
TEST_F(LayoutBlockTest, ContainmentStyleChange)
{
    ...
    EXPECT_TRUE(target->GetSingleCachedLayoutResult()->GetPhysicalFragment().HasOutOfFlowFragmentChild());
    ...
    target_element->setAttribute(html_names::kStyleAttr, AtomicString("contain:style"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(target->GetSingleCachedLayoutResult()->GetPhysicalFragment().HasOutOfFlowFragmentChild());
    const LayoutView* view = GetDocument().GetLayoutView();
    EXPECT_TRUE(view->GetSingleCachedLayoutResult()->GetPhysicalFragment().HasOutOfFlowFragmentChild());
}
