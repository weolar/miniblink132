...
public:
    ...
    public:
        PropagatedData(const HeapVector<Member<LayoutBoxModelObject>>* sticky_descendants, const HeapVector<Member<Element>>* snap_areas,
    ...
    PhysicalFragment(FragmentBuilder* builder, WritingMode block_or_line_writing_mode, FragmentType type, unsigned sub_type);

    PhysicalFragment(const PhysicalFragment& other);

    FragmentType Type() const
    {
        return static_cast<FragmentType>(type_);
    }
    bool IsContainer() const
    {
        return Type() == FragmentType::kFragmentBox || Type() == FragmentType::kFragmentLineBox;
    }
    bool IsBox() const
    {
    ...
    {
        return Type() == FragmentType::kFragmentLineBox;
    }
    ...
    {
        return IsCSSBox() && layout_object_->StyleRef().HasStickyConstrainedPosition();
    }
    ...
    {
        return IsCSSBox() && IsA<LayoutBox>(layout_object_.Get()) && layout_object_->StyleRef().GetScrollSnapAlign() != cc::ScrollSnapAlign();
    }
    ...
    {
        return IsBox() && is_math_fraction_;
    }

    bool IsMathMLOperator() const
    {
        return IsBox() && is_math_operator_;
    }
    ...
    bool IsCSSBox() const
    {
        return !IsLineBox() && !IsFragmentainerBox();
    }
    ...
    {
        return IsCSSBox() && layout_object_->IsFrameSet();
    }
    ...
    bool IsSvg() const
    {
        return layout_object_->IsSVG();
    }
    bool IsSvgText() const
    {
        return layout_object_->IsSVGText();
    }

    bool IsTablePart() const
    {
        return is_table_part_;
    }

    bool IsTable() const
    {
        return IsTablePart() && layout_object_->IsTable();
    }
    ...
    bool IsGrid() const
    {
        return layout_object_->IsLayoutGrid();
    }
    ...
    bool HasLayer() const
    {
        return IsCSSBox() && layout_object_->HasLayer();
    }
    ...
    {
        return HasLayer() && To<LayoutBoxModelObject>(layout_object_.Get())->HasSelfPaintingLayer();
    }
    ...
    bool IsLayoutObjectDestroyedOrMoved() const
    {
    ...
    [[nodiscard]] String DumpFragmentTree(
        DumpFlags, const PhysicalFragment* target = nullptr, std::optional<PhysicalOffset> = std::nullopt, unsigned indent = 2) const;
    ...
    [[nodiscard]] static String DumpFragmentTree(const LayoutObject& root, DumpFlags, const PhysicalFragment* target = nullptr);
    ...
    public:
        PostLayoutChildLinkList(wtf_size_t count, const PhysicalFragmentLink* buffer)
            : count_(count)
            , buffer_(buffer)
        {
        }
        ...
        public:
            ...
            const PhysicalFragmentLink& operator*() const
            {
                return post_layout_;
            }
            const PhysicalFragmentLink* operator->() const
        ...
        {
            return const_iterator(buffer_, count_);
        }
        const_iterator end() const
        {
            return const_iterator(buffer_ + count_, 0);
        }
    ...
    const BreakToken* GetBreakToken() const
    {
        return break_token_.Get();
    }
    ...
    {
        return children_valid_;
    }

    const HeapVector<Member<LayoutBoxModelObject>>* StickyDescendants() const
    {
        return propagated_data_ ? propagated_data_->sticky_descendants.Get() : nullptr;
    }
    const HeapVector<Member<LayoutBoxModelObject>>* PropagatedStickyDescendants() const
    {
    ...
    {
        return PropagatedStickyDescendants() || PropagatedScrollStartTarget() || PropagatedSnapAreas();
    }

    class OofData : public GarbageCollected<OofData>, private PhysicalAnchorQuery {
    public:
...
protected:
    ...
    void AddOutlineRectsForCursor(OutlineRectCollector& collector, const PhysicalOffset& additional_offset, OutlineType outline_type,
        const LayoutBoxModelObject* containing_block, InlineCursor* cursor) const;
    void AddOutlineRectsForDescendant(const PhysicalFragmentLink& descendant, OutlineRectCollector& collector, const PhysicalOffset& additional_offset,
        OutlineType outline_type, const LayoutBoxModelObject* containing_block) const;
...
CORE_EXPORT void ShowFragmentTree(const blink::LayoutObject& root, const blink::PhysicalFragment* target = nullptr);
