...
public:
    ...
    using HitNodeCb = base::RepeatingCallback<ListBasedHitTestBehavior(const Node& node)>;

    HitTestRequest(HitTestRequestType request_type, const LayoutObject* stop_node = nullptr, std::optional<HitNodeCb> hit_node_cb = std::nullopt)
        ...
        , stop_node_(stop_node)
    {
        ...
        DCHECK(!hit_node_cb || ((request_type & kPenetratingList) && (request_type & kAvoidCache)));
    ...
    bool UseHitNodeCb() const
    {
        return hit_node_cb_.has_value();
    }
    ...
    const LayoutObject* GetStopNode() const
    {
        return stop_node_.Get();
    }
    ...
    static const HitTestRequestType kCacheabilityBits = kReadOnly | kActive | kMove | kRelease | kTouchEvent;
    bool EqualForCacheability(const HitTestRequest& value) const
    {
        return (request_type_ | kCacheabilityBits) == (value.request_type_ | kCacheabilityBits) && stop_node_ == value.stop_node_;
    }
