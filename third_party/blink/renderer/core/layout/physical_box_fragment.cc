...
bool ShouldUsePositionForPointInBlockFlowDirection(const LayoutObject& layout_object)
{
    const LayoutBlockFlow* const layout_block_flow = DynamicTo<LayoutBlockFlow>(layout_object);
...
{
    return fragment.Size().height && fragment.Style().Visibility() == EVisibility::kVisible && !fragment.IsFloatingOrOutOfFlowPositioned();
}
...
// static
const PhysicalBoxFragment* PhysicalBoxFragment::Create(BoxFragmentBuilder* builder, WritingMode block_or_line_writing_mode)
{
    const auto writing_direction = builder->GetWritingDirection();
    const PhysicalBoxStrut borders = builder->ApplicableBorders().ConvertToPhysical(writing_direction);
    const PhysicalBoxStrut scrollbar = builder->ApplicableScrollbar().ConvertToPhysical(writing_direction);
    const PhysicalBoxStrut padding = builder->ApplicablePadding().ConvertToPhysical(writing_direction);

    const PhysicalSize physical_size = ToPhysicalSize(builder->Size(), builder->GetWritingMode());
...
#if DCHECK_IS_ON()
    if (builder->needs_inflow_bounds_explicitly_set_ && builder->node_ && builder->node_.IsScrollContainer() && !builder->IsFragmentainerBoxType())
    ...
    if (builder->ShouldCalculateScrollableOverflow()) {
        ScrollableOverflowCalculator calculator(To<BlockNode>(builder->node_),
            /* is_css_box */ !builder->IsFragmentainerBoxType(), builder->GetConstraintSpace().HasBlockFragmentation(), borders, scrollbar, padding,
            physical_size, writing_direction);
        ...
        for (auto& child : builder->children_) {
            ...
            calculator.AddChild(*box_fragment, child.offset.ConvertToPhysical(writing_direction, physical_size, box_fragment->Size()));
        }
    ...
    return MakeGarbageCollected<PhysicalBoxFragment>(AdditionalBytes(byte_size), PassKey(), builder, has_scrollable_overflow, scrollable_overflow,
        borders.IsZero() ? nullptr : &borders, scrollbar.IsZero() ? nullptr : &scrollbar, padding.IsZero() ? nullptr : &padding, inflow_bounds,
        has_fragment_items, block_or_line_writing_mode);
}

// static
const PhysicalBoxFragment* PhysicalBoxFragment::Clone(const PhysicalBoxFragment& other)
{
    // The size of the new fragment shouldn't differ from the old one.
    size_t byte_size = AdditionalByteSize(other.HasItems());

    return MakeGarbageCollected<PhysicalBoxFragment>(AdditionalBytes(byte_size), PassKey(), other, other.HasScrollableOverflow(), other.ScrollableOverflow());
}

// static
const PhysicalBoxFragment* PhysicalBoxFragment::CloneWithPostLayoutFragments(const PhysicalBoxFragment& other)
{
    ...
    const auto* cloned_fragment
        = MakeGarbageCollected<PhysicalBoxFragment>(AdditionalBytes(byte_size), PassKey(), other, has_scrollable_overflow, scrollable_overflow);
    ...
    for (PhysicalFragmentLink& child : cloned_fragment->GetMutableForCloning().Children()) {
        ...
        for (PhysicalFragmentLink& fragmentainer_child : fragmentainer.GetMutableForCloning().Children()) {
            auto& old_child = *To<PhysicalBoxFragment>(fragmentainer_child.fragment.Get());
...
namespace {
template <typename T> constexpr void AccountSizeAndPadding(size_t& current_size)
{
    const size_t current_size_with_padding = base::bits::AlignUp(current_size, alignof(T));
...
PhysicalBoxFragment::PhysicalBoxFragment(PassKey key, BoxFragmentBuilder* builder, bool has_scrollable_overflow, const PhysicalRect& scrollable_overflow,
    const PhysicalBoxStrut* borders, const PhysicalBoxStrut* scrollbar, const PhysicalBoxStrut* padding, const std::optional<PhysicalRect>& inflow_bounds,
    bool has_fragment_items, WritingMode block_or_line_writing_mode)
    : PhysicalFragment(builder, block_or_line_writing_mode, kFragmentBox, builder->GetBoxType())
    , bit_field_(ConstHasFragmentItemsFlag::encode(has_fragment_items) | HasDescendantsForTablePartFlag::encode(false)
          | IsFragmentationContextRootFlag::encode(builder->is_fragmentation_context_root_) | IsMonolithicFlag::encode(builder->is_monolithic_)
          | IsMonolithicOverflowPropagationDisabledFlag::encode(builder->GetConstraintSpace().IsMonolithicOverflowPropagationDisabled())
          | HasMovedChildrenInBlockDirectionFlag::encode(builder->has_moved_children_in_block_direction_))
{
    ...
    wtf_size_t i = 0;
    for (auto& child : builder->children_) {
        children_[i].offset = converter.ToPhysical(child.offset, child.fragment->Size());
    ...
    if (HasItems()) {
        ...
        std::optional<PhysicalSize> new_size = items_builder->ToFragmentItems(Size(), items);
        if (new_size)
            size_ = *new_size;
    }
    ...
    wtf_size_t rare_fields_size = has_scrollable_overflow + !!builder->frame_set_layout_data_ + !!builder->mathml_paint_info_ + !!builder->table_grid_rect_
        + !!builder->table_collapsed_borders_ + !!builder->table_collapsed_borders_geometry_ + !!builder->table_cell_column_index_
        + (builder->table_section_row_offsets_.empty() ? 0 : 2) + !!builder->page_name_ + !!borders + !!scrollbar + !!padding + inflow_bounds.has_value()
        + !!builder->Style().MayHaveMargin();

    if (rare_fields_size > 0 || !builder->table_column_geometries_.empty() || !builder->reading_flow_elements_.empty()) {
        rare_data_ = MakeGarbageCollected<PhysicalFragmentRareData>(
            has_scrollable_overflow ? &scrollable_overflow : nullptr, borders, scrollbar, padding, inflow_bounds, *builder, rare_fields_size);
    }
    ...
    PhysicalBoxSides sides_to_include(builder->sides_to_include_, builder->GetWritingMode());
    ...
    bit_field_.set<IsInlineFormattingContextFlag>(builder->is_inline_formatting_context_);
    is_math_fraction_ = builder->is_math_fraction_;
    is_math_operator_ = builder->is_math_operator_;

    const bool allow_baseline = !layout_object_->ShouldApplyLayoutContainment() || layout_object_->IsTableCell();
    ...
    use_last_baseline_for_inline_baseline_ = builder->use_last_baseline_for_inline_baseline_;

    bit_field_.set<HasDescendantsForTablePartFlag>(children_.size() || NeedsOOFPositionedInfoPropagation());
...
PhysicalBoxFragment::PhysicalBoxFragment(PassKey key, const PhysicalBoxFragment& other, bool has_scrollable_overflow, const PhysicalRect& scrollable_overflow)
    ...
    , children_(other.children_)
{
    ...
    if (other.rare_data_) {
        rare_data_ = MakeGarbageCollected<PhysicalFragmentRareData>(*other.rare_data_);
    }
}
...
const LayoutBox* PhysicalBoxFragment::OwnerLayoutBox() const
{
    ...
    // moved elsewhere.
    const LayoutBox* owner_box = DynamicTo<LayoutBox>(GetSelfOrContainerLayoutObject());
...
PhysicalOffset PhysicalBoxFragment::OffsetFromOwnerLayoutBox() const
{
    ...
    if (IsLtr(containing_block_style.Direction())) {
        ...
        const FragmentData* fragment_data = owner_box->FragmentDataFromPhysicalFragment(*this);
...
const PhysicalBoxFragment* PhysicalBoxFragment::PostLayout() const
{
    ...
    if (fragment_count == 1) {
        ...
        if (index < fragment_count) {
            ...
            DCHECK(post_layout);
            DCHECK(!post_layout->GetBreakToken() || post_layout->GetBreakToken()->SequenceNumber() == index);
        }
    } else {
...
PhysicalRect PhysicalBoxFragment::InkOverflowRect() const
{
    ...
    if (overflow_clip_axes == kOverflowClipBothAxis) {
        if (ShouldApplyOverflowClipMargin()) {
            const PhysicalRect& contents_rect = ink_overflow_.Contents(InkOverflowType(), Size());
...
PhysicalRect PhysicalBoxFragment::OverflowClipRect(const PhysicalOffset& location, OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior) const
{
...
PhysicalRect PhysicalBoxFragment::OverflowClipRect(
    const PhysicalOffset& location, const BlockBreakToken* incoming_break_token, OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior) const
{
    PhysicalRect clip_rect = OverflowClipRect(location, overlay_scrollbar_clip_behavior);
    ...
    LogicalRect logical_fragment_rect(stitched_offset, Size().ConvertToLogical(writing_direction.GetWritingMode()));
    PhysicalRect physical_fragment_rect = converter.ToPhysical(logical_fragment_rect);
    ...
    if (!GetDocument().Printing()) {
        ...
        if (overflow_clip != kOverflowClipBothAxis) {
            ApplyVisibleOverflowToClipRect(overflow_clip, overflow_physical_fragment_rect);
...
bool PhysicalBoxFragment::MayIntersect(const HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset) const
{
...
const PhysicalBoxFragment* PhysicalBoxFragment::InlineContainerFragmentIfOutlineOwner() const
{
    DCHECK(IsInlineBox());
...
PhysicalFragmentRareData::RareField& PhysicalBoxFragment::EnsureRareField(FieldId id)
{
    if (!rare_data_) {
...
PhysicalBoxFragment::MutableForStyleRecalc::MutableForStyleRecalc(base::PassKey<PhysicalBoxFragment>, PhysicalBoxFragment& fragment)
    : fragment_(fragment)
{
}

void PhysicalBoxFragment::MutableForStyleRecalc::SetScrollableOverflow(const PhysicalRect& scrollable_overflow)
{
    bool has_scrollable_overflow = scrollable_overflow != PhysicalRect({}, fragment_.Size());
    if (has_scrollable_overflow) {
        // This can be called even without rare_data_.
        fragment_.EnsureRareField(FieldId::kScrollableOverflow).scrollable_overflow = scrollable_overflow;
    } else if (fragment_.HasScrollableOverflow()) {
        fragment_.rare_data_->RemoveField(FieldId::kScrollableOverflow);
    }
}

PhysicalBoxFragment::MutableForStyleRecalc PhysicalBoxFragment::GetMutableForStyleRecalc() const
{
    DCHECK(layout_object_->GetDocument().Lifecycle().GetState() == DocumentLifecycle::kInStyleRecalc
        || layout_object_->GetDocument().Lifecycle().GetState() == DocumentLifecycle::kInPerformLayout);
    return MutableForStyleRecalc(base::PassKey<PhysicalBoxFragment>(), const_cast<PhysicalBoxFragment&>(*this));
}

PhysicalBoxFragment::MutableForContainerLayout::MutableForContainerLayout(base::PassKey<PhysicalBoxFragment>, PhysicalBoxFragment& fragment)
    : fragment_(fragment)
{
}

void PhysicalBoxFragment::MutableForContainerLayout::SetMargins(const PhysicalBoxStrut& margins)
{
    // This can be called even without rare_data_.
    fragment_.EnsureRareField(FieldId::kMargins).margins = margins;
}

PhysicalBoxFragment::MutableForContainerLayout PhysicalBoxFragment::GetMutableForContainerLayout() const
{
    DCHECK(layout_object_->GetFrameView()->IsInPerformLayout());
    return MutableForContainerLayout(base::PassKey<PhysicalBoxFragment>(), const_cast<PhysicalBoxFragment&>(*this));
}

void PhysicalBoxFragment::MutableForOofFragmentation::AddChildFragmentainer(const PhysicalBoxFragment& child_fragment, LogicalOffset child_offset)
{
    ...
    WritingModeConverter converter(fragment_.Style().GetWritingDirection(), fragment_.Size());
...
void PhysicalBoxFragment::MutableForOofFragmentation::Merge(const PhysicalBoxFragment& placeholder_fragmentainer)
{
    DCHECK(placeholder_fragmentainer.IsFragmentainerBox());

    // Copy all child fragments.
    for (const PhysicalFragmentLink& new_child : placeholder_fragmentainer.children_) {
    ...
    // overflow (printing).
    if (const BlockBreakToken* new_break_token = placeholder_fragmentainer.GetBreakToken()) {
    ...
    if (const PhysicalAnchorQuery* query = placeholder_fragmentainer.AnchorQuery()) {
...
{
    PhysicalRect overflow = ScrollableOverflowCalculator::RecalculateScrollableOverflowForFragment(fragment_, /* has_block_fragmentation */ true);
    fragment_.GetMutableForStyleRecalc().SetScrollableOverflow(overflow);
}

void PhysicalBoxFragment::SetInkOverflow(const PhysicalRect& self, const PhysicalRect& contents)
{
    SetInkOverflowType(ink_overflow_.Set(InkOverflowType(), self, contents, Size()));
}
...
void PhysicalBoxFragment::RecalcInkOverflow()
{
    ...
    DCHECK(layout_object);
    DCHECK(!DisplayLockUtilities::LockedAncestorPreventingPrePaint(*layout_object));
...
PhysicalRect PhysicalBoxFragment::RecalcContentsInkOverflow()
{
    ...
    DCHECK(!DisplayLockUtilities::LockedAncestorPreventingPrePaint(*GetSelfOrContainerLayoutObject()));
    DCHECK(!GetSelfOrContainerLayoutObject()->ChildPrePaintBlockedByDisplayLock());
    ...
    if (const FragmentItems* items = Items()) {
        ...
        // text.
        const auto* const text_combine = DynamicTo<LayoutTextCombine>(GetLayoutObject());
        if (text_combine) [[unlikely]] {
            // Reset the cursor for text combine to provide a current item for
            // decorations.
            InlineCursor text_combine_cursor(*this, *items);
            contents_rect.Unite(text_combine->RecalcContentsInkOverflow(text_combine_cursor));
        }
    ...
    for (const PhysicalFragmentLink& child : PostLayoutChildren()) {
        ...
        if (child_fragment->CanUseFragmentsForInkOverflow()) {
            ...
            DCHECK_EQ(child_fragment->IsColumnBox(), child_layout_object->IsLayoutFlowThread());
...
PhysicalRect PhysicalBoxFragment::ComputeSelfInkOverflow() const
{
    ...
    if (IsTableRow()) [[unlikely]] {
        ...
        for (const PhysicalFragmentLink& child : PostLayoutChildren()) {
            ...
            const auto* child_layout_object = To<LayoutTableCell>(child_fragment.GetLayoutObject());
    ...
    if (style.HasOutline() && IsOutlineOwner()) {
        ...
        AddSelfOutlineRects(PhysicalOffset(), style.OutlineRectsShouldIncludeBlockInkOverflow(), collector, &info);
...
void PhysicalBoxFragment::AddSelfOutlineRects(
    const PhysicalOffset& additional_offset, OutlineType outline_type, OutlineRectCollector& collector, LayoutObject::OutlineInfo* info) const
{
    if (info) {
        if (IsSvgText())
    ...
    if (ShouldIncludeBlockInkOverflow(outline_type) && IsA<HTMLAnchorElement>(GetNode())) {
...
void PhysicalBoxFragment::AddOutlineRects(const PhysicalOffset& additional_offset, OutlineType outline_type, OutlineRectCollector& collector) const
{
    AddOutlineRects(additional_offset, outline_type,
...
void PhysicalBoxFragment::AddOutlineRects(
    const PhysicalOffset& additional_offset, OutlineType outline_type, bool inline_container_relative, OutlineRectCollector& collector) const
{
    DCHECK_EQ(PostLayout(), this);

    if (IsInlineBox()) {
        AddOutlineRectsForInlineBox(additional_offset, outline_type, inline_container_relative, collector);
        return;
    }
    ...
    if (!IsAnonymousBlock() || GetBoxType() == kPageBorderBox) {
        if (IsSvgText()) {
            if (Items()) {
                collector.AddRect(PhysicalRect::EnclosingRect(GetLayoutObject()->ObjectBoundingBox()));
            }
        } else {
    ...
    if (ShouldIncludeBlockInkOverflow(outline_type) && !HasNonVisibleOverflow() && !HasControlClip(*this)) {
        ...
        std::unique_ptr<OutlineRectCollector> child_collector = collector.ForDescendantCollector();
        AddOutlineRectsForNormalChildren(*child_collector, PhysicalOffset(), outline_type, To<LayoutBoxModelObject>(GetLayoutObject()));
        ...
        if (ShouldIncludeBlockInkOverflowForAnchorOnly(outline_type)) {
            for (const auto& child : PostLayoutChildren()) {
                ...
                AddOutlineRectsForDescendant(child, collector, additional_offset, outline_type, To<LayoutBoxModelObject>(GetLayoutObject()));
            }
        }
    }
...
void PhysicalBoxFragment::AddOutlineRectsForInlineBox(
    PhysicalOffset additional_offset, OutlineType outline_type, bool container_relative, OutlineRectCollector& collector) const
{
    DCHECK_EQ(PostLayout(), this);
    DCHECK(IsInlineBox());

    const PhysicalBoxFragment* container = InlineContainerFragmentIfOutlineOwner();
    if (!container)
        return;
    ...
    std::unique_ptr<OutlineRectCollector> cursor_collector = collector.ForDescendantCollector();
    InlineCursor cursor(*container);
    cursor.MoveTo(*layout_object);
    DCHECK(cursor);
    const PhysicalOffset this_offset_in_container = cursor.Current()->OffsetInContainerFragment();
#if DCHECK_IS_ON()
    ...
    for (; cursor; cursor.MoveToNextForSameLayoutObject()) {
        ...
        AddOutlineRectsForCursor(*cursor_collector, PhysicalOffset(), outline_type, layout_object, &descendants);
    }
#if DCHECK_IS_ON()
    ...
    if (ShouldIncludeBlockInkOverflowForAnchorOnly(outline_type) && !HasNonVisibleOverflow() && !HasControlClip(*this)) {
        for (const auto& child : container->PostLayoutChildren()) {
            if (!child->IsOutOfFlowPositioned() || child->GetLayoutObject()->ContainerForAbsolutePosition() != layout_object) {
                continue;
            }

            AddOutlineRectsForDescendant(child, collector, additional_offset, outline_type, layout_object);
        }
    }
}

PositionWithAffinity PhysicalBoxFragment::PositionForPoint(PhysicalOffset point) const
{
    ...
    const PhysicalOffset point_in_contents = IsScrollContainer() ? point + PhysicalOffset(PixelSnappedScrolledContentOffset()) : point;

    if (!layout_object_->ChildPaintBlockedByDisplayLock()) {
        if (const FragmentItems* items = Items()) {
            InlineCursor cursor(*this, *items);
            if (const PositionWithAffinity position = cursor.PositionForPointInInlineFormattingContext(point_in_contents, *this))
                return AdjustForEditingBoundary(position);
            return layout_object_->CreatePositionWithAffinity(0);
        }
    }

    if (IsA<LayoutBlockFlow>(*layout_object_) && layout_object_->ChildrenInline()) {
...
PositionWithAffinity PhysicalBoxFragment::PositionForPointByClosestChild(PhysicalOffset point_in_contents) const
{
    if (layout_object_->ChildPaintBlockedByDisplayLock()) {
        // If this node is DisplayLocked, then Children() will have invalid layout
        // information.
        return AdjustForEditingBoundary(FirstPositionInOrBeforeNode(*layout_object_->GetNode()));
    }
    ...
    for (const PhysicalFragmentLink& child : Children()) {
        ...
        if (!is_hit_test_candidate) {
            ...
            if (box_fragment.Style().Visibility() != EVisibility::kVisible || (box_fragment.Children().empty() && !box_fragment.IsBlockFlow())) {
                continue;
            }
        }
        ...
        const LayoutUnit distance = horizontal_distance * horizontal_distance + vertical_distance * vertical_distance;

        if (shortest_distance > distance || (is_hit_test_candidate && !found_hit_test_candidate)) {
    ...
    return To<PhysicalBoxFragment>(*closest_child).PositionForPoint(point_in_contents - closest_child.offset);
}

PositionWithAffinity PhysicalBoxFragment::PositionForPointInBlockFlowDirection(PhysicalOffset point_in_contents) const
{
    // Note: Children of <table> and "columns" are not laid out in block flow
    // direction.
    DCHECK(!layout_object_->IsTable()) << this;
    DCHECK(ShouldUsePositionForPointInBlockFlowDirection(*layout_object_)) << this;
    ...
    const LogicalOffset logical_point_in_contents = converter.ToLogical(point_in_contents, PhysicalSize());
    ...
    for (const PhysicalFragmentLink& child : Children()) {
        ...
        // IE6/7 and FF3).
        const LogicalRect logical_child_rect = converter.ToLogical(PhysicalRect(child.offset, box_fragment.Size()));
        if (logical_point_in_contents.block_offset < logical_child_rect.BlockEndOffset()
            || (blocks_are_flipped && logical_point_in_contents.block_offset == logical_child_rect.BlockEndOffset())) {
            // |child| is logically below |point_in_contents|.
            return PositionForPointRespectingEditingBoundaries(To<PhysicalBoxFragment>(*child.fragment), point_in_contents - child.offset);
        }
    ...
    if (last_candidate_box) {
        ...
        return PositionForPointRespectingEditingBoundaries(
            To<PhysicalBoxFragment>(*last_candidate_box->fragment), point_in_contents - last_candidate_box->offset);
    }
...
PositionWithAffinity PhysicalBoxFragment::PositionForPointInTable(PhysicalOffset point_in_contents) const
{
    DCHECK(layout_object_->IsTable()) << this;
    ...
    const LayoutUnit adjusted_left = Style().IsFlippedBlocksWritingMode() ? Size().width - point_in_contents.left : point_in_contents.left;
    if (adjusted_left < 0 || adjusted_left > Size().width || point_in_contents.top < 0 || point_in_contents.top > Size().height) {
...
PositionWithAffinity PhysicalBoxFragment::PositionForPointRespectingEditingBoundaries(const PhysicalBoxFragment& child, PhysicalOffset point_in_child) const
{
    ...
    if (!ancestor || !ancestor->Parent() || (ancestor->HasLayer() && ancestor->Parent()->IsLayoutView())
    ...
    // closer.
    WritingModeConverter converter(child.Style().GetWritingDirection(), child.Size());
    const LogicalOffset logical_point_in_child = converter.ToLogical(point_in_child, PhysicalSize());
    const LayoutUnit logical_child_inline_size = converter.ToLogical(child.Size()).inline_size;
...
void PhysicalBoxFragment::CheckSameForSimplifiedLayout(const PhysicalBoxFragment& other, bool check_same_block_size, bool check_no_fragmentation) const
{
    DCHECK_EQ(layout_object_, other.layout_object_);

    LogicalSize size = size_.ConvertToLogical(Style().GetWritingMode());
    LogicalSize other_size = other.size_.ConvertToLogical(Style().GetWritingMode());
    ...
    DCHECK_EQ(has_adjoining_object_descendants_, other.has_adjoining_object_descendants_);
    DCHECK_EQ(may_have_descendant_above_block_start_, other.may_have_descendant_above_block_start_);
    DCHECK_EQ(bit_field_.get<HasDescendantsForTablePartFlag>(), other.bit_field_.get<HasDescendantsForTablePartFlag>());
    ...
    if (!IsOutOfFlowPositioned()) {
        DCHECK_EQ(depends_on_percentage_block_size_, other.depends_on_percentage_block_size_);
    }
    ...
    if (IsTable()) {
        ...
        if (TableCollapsedBordersGeometry()) {
            ...
            TableCollapsedBordersGeometry()->CheckSameForSimplifiedLayout(*other.TableCollapsedBordersGeometry());
        } else {
            DCHECK(!other.TableCollapsedBordersGeometry());
        }
    }
...
void PhysicalBoxFragment::AssertFragmentTreeChildren(bool allow_destroyed_or_moved) const
{
    if (const FragmentItems* items = Items()) {
    ...
    for (const PhysicalFragmentLink& child : Children()) {
        ...
        if (const auto* box = DynamicTo<PhysicalBoxFragment>(child.fragment.Get())) {
            box->AssertFragmentTreeSelf();
        }
    }
}
