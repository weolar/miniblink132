...
{
    box->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kAnonymousBlockChange);
}
...
bool NeedsAnchorPositionScrollData(Element& element, const ComputedStyle& style)
{
...
void LayoutBoxModelObject::WillBeDestroyed()
{
    ...
    if (!DocumentBeingDestroyed()) {
        GetDocument().GetFrame()->GetInputMethodController().LayoutObjectWillBeDestroyed(*this);
    }
...
void LayoutBoxModelObject::StyleWillChange(StyleDifference diff, const ComputedStyle& new_style)
{
    ...
    if (Style() && (IsStacked() != IsStacked(new_style) || IsStackingContext() != IsStackingContext(new_style)) &&
...
DISABLE_CFI_PERF
void LayoutBoxModelObject::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    if (old_style && IsOutOfFlowPositioned() && Parent() && (StyleRef().GetPosition() == old_style->GetPosition())
        && (StyleRef().IsOriginalDisplayInlineType() != old_style->IsOriginalDisplayInlineType()))
        Parent()->SetNeedsLayout(layout_invalidation_reason::kChildChanged, kMarkContainerChain);

    // Clear our sticky constraints if we are no longer sticky.
    if (Layer() && old_style->HasStickyConstrainedPosition() && !StyleRef().HasStickyConstrainedPosition()) {
    ...
    if (type != kNoPaintLayer) {
        ...
        if (EverHadLayout())
            SetChildNeedsLayout();
        if (had_transform_related_property || had_filter_inducing_property || had_non_initial_backdrop_filter) {
            SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kStyleChange);
        }
    }
    ...
    if (old_style && (could_contain_fixed != can_contain_fixed || could_contain_absolute != can_contain_absolute)) {
        ...
        AddSubtreePaintPropertyUpdateReason(SubtreePaintPropertyUpdateReason::kContainerChainMayChange);
    } else if (had_layer == HasLayer()
        && (had_transform_related_property != HasTransformRelatedProperty() || had_filter_inducing_property != HasFilterInducingProperty()
            || had_non_initial_backdrop_filter != HasNonInitialBackdropFilter())) {
    ...
    if (old_style && Parent()) {
        ...
        if ((could_contain_fixed && !can_contain_fixed) || (could_contain_absolute && !can_contain_absolute)) {
    ...
    if (Layer()) {
        ...
        if (old_style && (IsStacked() != IsStacked(*old_style) || IsStackingContext() != IsStackingContext(*old_style)) &&
            // ObjectPaintInvalidator requires this.
            IsRooted()) {
            ObjectPaintInvalidator(*this).SlowSetPaintingLayerNeedsRepaint();
        }
    ...
    if (IsDocumentElement()) {
        if (const HTMLBodyElement* body = GetDocument().FirstBodyElement()) {
            if (auto* body_object = DynamicTo<LayoutBoxModelObject>(body->GetLayoutObject())) {
                bool new_body_background_transfers = body_object->BackgroundTransfersToView(Style());
                bool old_body_background_transfers = old_style && body_object->BackgroundTransfersToView(old_style);
                if (new_body_background_transfers != old_body_background_transfers && body_object->Style() && body_object->StyleRef().HasBackground()) {
    ...
    if (old_style && old_style->BackfaceVisibility() != StyleRef().BackfaceVisibility()) {
        SetNeedsPaintPropertyUpdate();
    }
    ...
    if (old_style && ShouldApplyLayoutContainment(*old_style) != ShouldApplyLayoutContainment() && Layer()) {
...
void LayoutBoxModelObject::AddOutlineRectsForNormalChildren(
    OutlineRectCollector& collector, const PhysicalOffset& additional_offset, OutlineType include_block_overflows) const
{
    NOT_DESTROYED();
    for (LayoutObject* child = SlowFirstChild(); child; child = child->NextSibling()) {
        ...
        AddOutlineRectsForDescendant(*child, collector, additional_offset, include_block_overflows);
    }
}

void LayoutBoxModelObject::AddOutlineRectsForDescendant(
    const LayoutObject& descendant, OutlineRectCollector& collector, const PhysicalOffset& additional_offset, OutlineType include_block_overflows) const
{
    NOT_DESTROYED();
    ...
    if (descendant.HasLayer()) {
        ...
        descendant.AddOutlineRects(*descendant_collector, nullptr, PhysicalOffset(), include_block_overflows);
        collector.Combine(descendant_collector.get(), descendant, this, additional_offset);
        return;
    }

    if (descendant.IsBox()) {
        descendant.AddOutlineRects(collector, nullptr, additional_offset + To<LayoutBox>(descendant).PhysicalLocation(), include_block_overflows);
        return;
    }
    ...
    if (descendant.IsLayoutInline()) {
        ...
        // only.
        To<LayoutInline>(descendant).AddOutlineRectsForNormalChildren(collector, additional_offset, include_block_overflows);
        return;
    }

    descendant.AddOutlineRects(collector, nullptr, additional_offset, include_block_overflows);
}
...
bool LayoutBoxModelObject::ShouldBeHandledAsInline(const ComputedStyle& style) const
{
...
void LayoutBoxModelObject::UpdateFromStyle()
{
    ...
    SetCanContainAbsolutePositionObjects(ComputeIsAbsoluteContainer(&style_to_use));
    SetCanContainFixedPositionObjects(ComputeIsFixedContainer(&style_to_use));
    SetIsBackgroundAttachmentFixedObject(!BackgroundTransfersToView() && StyleRef().HasFixedAttachmentBackgroundImage());
}

void LayoutBoxModelObject::UpdateCanCompositeBackgroundAttachmentFixed(bool enable_composited_background_attachment_fixed)
{
    SetCanCompositeBackgroundAttachmentFixed(enable_composited_background_attachment_fixed && ComputeCanCompositeBackgroundAttachmentFixed());
}
...
PhysicalRect LayoutBoxModelObject::ApplyFiltersToRect(const PhysicalRect& rect) const
{
    NOT_DESTROYED();
    ...
    if (auto* layer = Layer()) {
        ...
        CHECK(IsSVGChild());
        const gfx::RectF filter_reference_box = SVGResources::ReferenceBoxForEffects(*this);
        if (!filter_reference_box.size().IsZero()) {
            float_rect.UnionEvenIfEmpty(filter_reference_box);
        }
...
StickyPositionScrollingConstraints* LayoutBoxModelObject::ComputeStickyPositionConstraints() const
{
    NOT_DESTROYED();
    DCHECK(StyleRef().HasStickyConstrainedPosition());

    StickyPositionScrollingConstraints* constraints = MakeGarbageCollected<StickyPositionScrollingConstraints>();

    bool is_fixed_to_view = false;
    const auto* scroll_container_layer = Layer()->ContainingScrollContainerLayer(&is_fixed_to_view);
    ...
    while (sticky_container->IsAnonymous()) {
        if (sticky_container->Parent() && sticky_container->Parent()->IsFieldset()) {
            break;
        }
    ...
    const PhysicalOffset scroll_container_border_offset(scroll_container->BorderLeft(), scroll_container->BorderTop());

    MapCoordinatesFlags flags = kIgnoreTransforms | kIgnoreScrollOffset | kIgnoreStickyOffset;
    ...
    // Compute the sticky-container rect.
    {
        ...
        if (sticky_container == scroll_container) {
            scroll_container_relative_containing_block_rect = sticky_container->ScrollableOverflowRect();
        } else {
            PhysicalRect local_rect = sticky_container->PhysicalPaddingBoxRect();
            scroll_container_relative_containing_block_rect = sticky_container->LocalToAncestorRect(local_rect, scroll_container, flags);
        }
        ...
        scroll_container_relative_containing_block_rect.Contract(sticky_container->PaddingOutsets());
        if (!RuntimeEnabledFeatures::LayoutIgnoreMarginsForStickyEnabled()) {
            ...
            scroll_container_relative_containing_block_rect.ContractEdges(MinimumValueForLength(StyleRef().MarginTop(), max_width),
                MinimumValueForLength(StyleRef().MarginRight(), max_width), MinimumValueForLength(StyleRef().MarginBottom(), max_width),
                MinimumValueForLength(StyleRef().MarginLeft(), max_width));
        }

        constraints->scroll_container_relative_containing_block_rect = scroll_container_relative_containing_block_rect;
    }

    // The location container for boxes is not always the containing block.
    LayoutObject* location_container = IsLayoutInline() ? Container() : To<LayoutBox>(this)->LocationContainer();
    ...
    PhysicalRect sticky_box_rect;
    {
        ...
        PhysicalRect scroll_container_relative_sticky_box_rect = location_container->LocalToAncestorRect(sticky_box_rect, scroll_container, flags);

        // Make relative to the padding-box instead of border-box.
        scroll_container_relative_sticky_box_rect.Move(-scroll_container_border_offset);
        constraints->scroll_container_relative_sticky_box_rect = scroll_container_relative_sticky_box_rect;
    }
    ...
    constraints->nearest_sticky_layer_shifting_sticky_box = location_container->FindFirstStickyContainer(sticky_container);
    constraints->nearest_sticky_layer_shifting_containing_block = sticky_container->FindFirstStickyContainer(scroll_container);

    constraints->constraining_rect = scroll_container->ComputeStickyConstrainingRect();

    // Compute the insets.
    {
        auto ResolveInset = [](const Length& length, LayoutUnit size) -> std::optional<LayoutUnit> {
            if (length.IsAuto()) {
                return std::nullopt;
            }
        ...
        std::optional<LayoutUnit> left = ResolveInset(style.Left(), available_size.width);
        std::optional<LayoutUnit> right = ResolveInset(style.Right(), available_size.width);
        std::optional<LayoutUnit> top = ResolveInset(style.Top(), available_size.height);
        std::optional<LayoutUnit> bottom = ResolveInset(style.Bottom(), available_size.height);
...
PhysicalOffset LayoutBoxModelObject::AdjustedPositionRelativeTo(const PhysicalOffset& start_point, const Element* offset_parent) const
{
    NOT_DESTROYED();
    ...
    if (const LayoutBoxModelObject* offset_parent_object = offset_parent->GetLayoutBoxModelObject()) {
        if (!IsOutOfFlowPositioned()) {
            ...
            for (const LayoutObject* current = Container(); current && current->GetNode() != offset_parent; current = current->Container()) {
            ...
            if (offset_parent_object->IsBox() && offset_parent_object->IsBody() && !offset_parent_object->IsPositioned()) {
                reference_point += To<LayoutBox>(offset_parent_object)->PhysicalLocation();
            }
        } else if (IsBox() && To<LayoutBox>(this)->NeedsAnchorPositionScrollAdjustment()) [[unlikely]] {
            reference_point += To<LayoutBox>(this)->AnchorPositionScrollTranslationOffset();
        }
...
LogicalRect LayoutBoxModelObject::LocalCaretRectForEmptyElement(LayoutUnit width, LayoutUnit text_indent_offset) const
{
    NOT_DESTROYED();
    ...
    BoxStrut border_padding = (BorderOutsets() + PaddingOutsets()).ConvertToLogical({ current_style.GetWritingMode(), TextDirection::kLtr });
...
void LayoutBoxModelObject::MoveChildTo(LayoutBoxModelObject* to_box_model_object, LayoutObject* child, LayoutObject* before_child, bool full_remove_insert)
{
    NOT_DESTROYED();
    DCHECK_EQ(this, child->Parent());
    DCHECK(!before_child || to_box_model_object == before_child->Parent());

    if (full_remove_insert && (to_box_model_object->IsLayoutBlock() || to_box_model_object->IsLayoutInline())) {
        // Takes care of adding the new child correctly if toBlock and fromBlock
        // have different kind of children (block vs inline).
        to_box_model_object->AddChild(VirtualChildren()->RemoveChildNode(this, child), before_child);
    } else {
        to_box_model_object->VirtualChildren()->InsertChildNode(
            to_box_model_object, VirtualChildren()->RemoveChildNode(this, child, full_remove_insert), before_child, full_remove_insert);
    }
}

void LayoutBoxModelObject::MoveChildrenTo(
    LayoutBoxModelObject* to_box_model_object, LayoutObject* start_child, LayoutObject* end_child, LayoutObject* before_child, bool full_remove_insert)
{
    NOT_DESTROYED();
...
LayoutObject* LayoutBoxModelObject::SplitAnonymousBoxesAroundChild(LayoutObject* before_child)
{
    NOT_DESTROYED();
    ...
    while (before_child->Parent() != this) {
        ...
        if (box_to_split->SlowFirstChild() != before_child && box_to_split->IsAnonymous()) {
            ...
            parent_box->VirtualChildren()->InsertChildNode(parent_box, post_box, box_to_split->NextSibling());
...
LayoutBox* LayoutBoxModelObject::CreateAnonymousBoxToSplit(const LayoutBox* box_to_split) const
{
    NOT_DESTROYED();
    return box_to_split->CreateAnonymousBoxWithSameTypeAs(this);
}

bool LayoutBoxModelObject::BackgroundTransfersToView(const ComputedStyle* document_element_style) const
{
    NOT_DESTROYED();
