...
public:
    AbsoluteUtilsTest()
        : ltr_space_(CreateConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }))
        , rtl_space_(CreateConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kRtl }))
        , vlr_space_(CreateConstraintSpace({ WritingMode::kVerticalLr, TextDirection::kLtr }))
        , vrl_space_(CreateConstraintSpace({ WritingMode::kVerticalRl, TextDirection::kLtr }))
    {
    }

protected:
    ConstraintSpace CreateConstraintSpace(WritingDirectionMode writing_direction)
    {
        ConstraintSpaceBuilder builder(WritingMode::kHorizontalTb, writing_direction,
            /* is_new_fc */ true);
    ...
    void SetHorizontalStyle(const String& left, const String& margin_left, const String& width, const String& margin_right, const String& right,
        const String& writing_mode = "horizontal-tb", const String& box_sizing = "border-box")
    {
    ...
    void SetVerticalStyle(const String& top, const String& margin_top, const String& height, const String& margin_bottom, const String& bottom,
        const String& writing_mode = "horizontal-tb", const String& box_sizing = "border-box")
    {
        ...
        element_->SetInlineStyleProperty(CSSPropertyID::kMarginBottom, margin_bottom);
    ...
    void ComputeOutOfFlowInlineDimensions(const BlockNode& node, const ConstraintSpace& space, const BoxStrut& border_padding,
        const LogicalStaticPosition& static_position, const WritingDirectionMode container_writing_direction, LogicalOofDimensions* dimensions)
    {
        ...
        WritingModeConverter container_converter(
            container_writing_direction, ToPhysicalSize(space.AvailableSize(), container_writing_direction.GetWritingMode()));
        LogicalAnchorQuery* anchor_query = MakeGarbageCollected<LogicalAnchorQuery>();
        AnchorEvaluatorImpl anchor_evaluator(*node.GetLayoutBox(), *anchor_query,
            ...
            /* available_size */
            PhysicalSize());
        WritingDirectionMode self_writing_direction = node.Style().GetWritingDirection();
        const LogicalOofInsets insets = ComputeOutOfFlowInsets(node.Style(), space.AvailableSize(), LogicalAlignment(), self_writing_direction);
        const InsetModifiedContainingBlock imcb = ComputeInsetModifiedContainingBlock(
            node, space.AvailableSize(), LogicalAlignment(), insets, static_position, container_writing_direction, node.Style().GetWritingDirection());
        ComputeOofInlineDimensions(node, node.Style(), space, imcb, LogicalAnchorCenterPosition(), LogicalAlignment(), border_padding, std::nullopt, BoxStrut(),
    ...
    void ComputeOutOfFlowBlockDimensions(const BlockNode& node, const ConstraintSpace& space, const BoxStrut& border_padding,
        const LogicalStaticPosition& static_position, const WritingDirectionMode container_writing_direction, LogicalOofDimensions* dimensions)
    {
        GetDocument().Lifecycle().AdvanceTo(DocumentLifecycle::kInStyleRecalc);
        GetDocument().Lifecycle().AdvanceTo(DocumentLifecycle::kStyleClean);
        ...
        const InsetModifiedContainingBlock imcb = ComputeInsetModifiedContainingBlock(
            node, space.AvailableSize(), LogicalAlignment(), insets, static_position, container_writing_direction, node.Style().GetWritingDirection());
        ComputeOofBlockDimensions(node, node.Style(), space, imcb, LogicalAnchorCenterPosition(), LogicalAlignment(), border_padding, std::nullopt, BoxStrut(),
            container_writing_direction, dimensions);
        GetDocument().Lifecycle().AdvanceTo(DocumentLifecycle::kAfterPerformLayout);
        GetDocument().Lifecycle().AdvanceTo(DocumentLifecycle::kLayoutClean);
    }
...
TEST_F(AbsoluteUtilsTest, Horizontal)
{
    ...
    element_->SetInlineStyleProperty(CSSPropertyID::kContainIntrinsicSize, "60px 4px");

    BoxStrut ltr_border_padding = ComputeBorders(ltr_space_, node) + ComputePadding(ltr_space_, node.Style());
    BoxStrut rtl_border_padding = ComputeBorders(rtl_space_, node) + ComputePadding(rtl_space_, node.Style());
    BoxStrut vlr_border_padding = ComputeBorders(vlr_space_, node) + ComputePadding(vlr_space_, node.Style());
    BoxStrut vrl_border_padding = ComputeBorders(vrl_space_, node) + ComputePadding(vrl_space_, node.Style());

    LogicalStaticPosition static_position = { { LayoutUnit(), LayoutUnit() }, LogicalStaticPosition::kInlineStart, LogicalStaticPosition::kBlockStart };
    // Same as regular static position, but with the inline-end edge.
    LogicalStaticPosition static_position_inline_end
        = { { LayoutUnit(), LayoutUnit() }, LogicalStaticPosition::kInlineEnd, LogicalStaticPosition::kBlockStart };

    LogicalOofDimensions dimensions;

    // All auto => width is content, left is 0.
    SetHorizontalStyle("auto", "auto", "auto", "auto", "auto");
    ComputeOutOfFlowInlineDimensions(node, ltr_space_, ltr_border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
    ...
    ComputeOutOfFlowInlineDimensions(
        node, ltr_space_, ltr_border_padding, static_position_inline_end, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
    EXPECT_EQ(116, dimensions.size.inline_size);
    EXPECT_EQ(200, dimensions.inset.inline_end);

    // All auto + RTL.
    SetHorizontalStyle("auto", "auto", "auto", "auto", "auto");
    ComputeOutOfFlowInlineDimensions(node, rtl_space_, rtl_border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
    EXPECT_EQ(116, dimensions.size.inline_size);
    // 200 = 0 + 0 + 116 + 84 + 0
    ...
    ComputeOutOfFlowBlockDimensions(node, vlr_space_, vlr_border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
    ...
    ComputeOutOfFlowBlockDimensions(node, vrl_space_, vrl_border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
    ...
    SetHorizontalStyle("5px", "auto", "200px", "auto", "13px");
    ComputeOutOfFlowInlineDimensions(node, rtl_space_, rtl_border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kRtl }, &dimensions);
    ...
    SetHorizontalStyle("auto", "7px", "104px", "15px", "13px", "horizontal-tb", "content-box");
    ComputeOutOfFlowInlineDimensions(node, ltr_space_, ltr_border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
    // 200 = 5 + 7 + 160 + 15 + 13
    EXPECT_EQ(5 + 7, dimensions.inset.inline_start);
...
TEST_F(AbsoluteUtilsTest, Vertical)
{
    ...
    LogicalStaticPosition static_position = { { LayoutUnit(), LayoutUnit() }, LogicalStaticPosition::kInlineStart, LogicalStaticPosition::kBlockStart };
    LogicalStaticPosition static_position_block_end = { { LayoutUnit(), LayoutUnit() }, LogicalStaticPosition::kInlineStart, LogicalStaticPosition::kBlockEnd };
    ...
    ComputeOutOfFlowBlockDimensions(node, ltr_space_, ltr_border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
    ...
    ComputeOutOfFlowBlockDimensions(
        node, ltr_space_, ltr_border_padding, static_position_block_end, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
    ...
    ComputeOutOfFlowInlineDimensions(node, vlr_space_, vlr_border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
    // 300 = 5 + 11 + 260 + 11 + 13
    ...
    ComputeOutOfFlowInlineDimensions(node, vrl_space_, vrl_border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
    // 300 = 5 + 11 + 260 + 11 + 13
    EXPECT_EQ(5 + 11, dimensions.inset.inline_start);
    EXPECT_EQ(11 + 13, dimensions.inset.inline_end);
...
{
    BlockNode node(element_->GetLayoutBox());
    LogicalStaticPosition static_position = { { LayoutUnit(150), LayoutUnit(200) }, LogicalStaticPosition::kInlineCenter, LogicalStaticPosition::kBlockCenter };
    ...
    ComputeOutOfFlowInlineDimensions(node, ltr_space_, border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
    ...
    ComputeOutOfFlowInlineDimensions(node, ltr_space_, border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kRtl }, &dimensions);
    EXPECT_EQ(100, dimensions.size.inline_size);
    EXPECT_EQ(100, dimensions.inset.inline_start);
    EXPECT_EQ(0, dimensions.inset.inline_end);

    ComputeOutOfFlowBlockDimensions(node, ltr_space_, border_padding, static_position, { WritingMode::kHorizontalTb, TextDirection::kLtr }, &dimensions);
