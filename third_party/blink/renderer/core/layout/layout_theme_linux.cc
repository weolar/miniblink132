...
{
    String stylesheet = LayoutThemeDefault::ExtraDefaultStyleSheet() + UncompressResourceAsASCIIString(IDR_UASTYLE_THEME_CHROMIUM_LINUX_CSS);
    if (RuntimeEnabledFeatures::CustomizableSelectEnabled()) {
        stylesheet = stylesheet + UncompressResourceAsASCIIString(IDR_UASTYLE_CUSTOMIZABLE_SELECT_LINUX_CSS);
    }
    return stylesheet;
}
