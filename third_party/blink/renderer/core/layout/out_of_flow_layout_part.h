...
class LayoutObject;
class LayoutResult;
template <typename OffsetType> class OofContainingBlock;
class SimplifiedOofLayoutAlgorithm;
struct LogicalOofPositionedNode;
template <typename OffsetType> struct MulticolWithPendingOofs;
...
public:
    ...
    public:
        ...
        void SwapOutOfFlowFragmentainerDescendants(HeapVector<LogicalOofNodeForFragmentation>* descendants)
        {
        ...
        HeapVector<LogicalOofNodeForFragmentation> out_of_flow_fragmentainer_descendants;
    ...
    void SetChildFragmentStorage(FragmentBuilder::ChildrenVector* child_fragment_storage)
    {
    ...
    void SetColumnBalancingInfo(ColumnBalancingInfo* column_balancing_info, FragmentBuilder::ChildrenVector* child_fragment_storage)
    {
    ...
    bool NeedsTotalPageCount()
    {
        return needs_total_page_count_;
    }
    ...
    public:
        ...
        NodeInfo(BlockNode node, const LogicalStaticPosition static_position, const ContainingBlockInfo base_container_info,
            const WritingDirectionMode default_writing_direction, bool is_fragmentainer_descendant, const OofContainingBlock<LogicalOffset>& containing_block,
            const OofContainingBlock<LogicalOffset>& fixedpos_containing_block, const OofInlineContainer<LogicalOffset>& fixedpos_inline_container,
            bool requires_content_before_breaking, bool is_hidden_for_paint)
            : node(node)
    ...
    static std::optional<LogicalSize> InitialContainingBlockFixedSize(BlockNode container);

private:
    const ContainingBlockInfo GetContainingBlockInfo(const LogicalOofPositionedNode&);
    ...
    void ComputeInlineContainingBlocks(const HeapVector<LogicalOofPositionedNode>&);
    void ComputeInlineContainingBlocksForFragmentainer(const HeapVector<LogicalOofNodeForFragmentation>&);
    ...
    void AddInlineContainingBlockInfo(const InlineContainingBlockUtils::InlineContainingBlockMap&, const WritingDirectionMode container_writing_direction,
        PhysicalSize container_builder_size, LogicalOffset containing_block_relative_offset = LogicalOffset(),
        LogicalOffset containing_block_offset = LogicalOffset(), bool adjust_for_fragmentation = false);
    ...
    void LayoutOOFsInMulticol(const BlockNode& multicol, const MulticolWithPendingOofs<LogicalOffset>* multicol_info);
    ...
    void LayoutFragmentainerDescendants(HeapVector<LogicalOofNodeForFragmentation>* descendants, LogicalOffset fragmentainer_progression,
        bool outer_context_has_fixedpos_container = false, HeapVector<MulticolChildInfo>* multicol_children = nullptr);
    ...
    ContainingBlockInfo ApplyPositionAreaOffsets(const PositionAreaOffsets& offsets, const ContainingBlockInfo& container_info) const;

    NodeInfo SetupNodeInfo(const LogicalOofPositionedNode& oof_node);

    const LayoutResult* LayoutOOFNode(
        NodeToLayout& oof_node_to_layout, const ConstraintSpace* fragmentainer_constraint_space = nullptr, bool is_last_fragmentainer_so_far = false);

    // TODO(almaher): We are calculating more than just the offset. Consider
    // changing this to a more accurate name.
    OffsetInfo CalculateOffset(const NodeInfo& node_info, const LogicalAnchorQueryMap* anchor_queries = nullptr);
    ...
    std::optional<OffsetInfo> TryCalculateOffset(
        const NodeInfo& node_info, const ComputedStyle& style, AnchorEvaluatorImpl&, bool try_fit_available_space, NonOverflowingScrollRange* out_scroll_range);
    ...
    // elements).
    void LayoutOOFsInFragmentainer(HeapVector<NodeToLayout>& pending_descendants, wtf_size_t index, LogicalOffset fragmentainer_progression,
    ...
    void AddOOFToFragmentainer(NodeToLayout& descendant, const ConstraintSpace* fragmentainer_space, LogicalOffset fragmentainer_offset, wtf_size_t index,
        bool is_last_fragmentainer_so_far, bool* has_actual_break_inside, SimplifiedOofLayoutAlgorithm* algorithm,
        HeapVector<NodeToLayout>* fragmented_descendants);
    ConstraintSpace GetFragmentainerConstraintSpace(wtf_size_t index);
    void ComputeStartFragmentIndexAndRelativeOffset(WritingMode default_writing_mode, LayoutUnit block_estimate,
    ...
    // paint-layer.
    void SaveStaticPositionOnPaintLayer(LayoutBox* layout_box, const LogicalStaticPosition& position) const;
    LogicalStaticPosition ToStaticPositionForLegacy(LogicalStaticPosition position) const;
    ...
    const FragmentBuilder::ChildrenVector& FragmentationContextChildren() const
    {
        ...
        return child_fragment_storage_ ? *child_fragment_storage_ : container_builder_->Children();
    }
    ...
    void AddFragmentainer(const PhysicalBoxFragment& fragmentainer, LogicalOffset fragmentainer_offset)
    {
        if (child_fragment_storage_) {
            child_fragment_storage_->push_back(LogicalFragmentLink { fragmentainer, fragmentainer_offset });
        } else {
    ...
    HeapHashMap<Member<const LayoutObject>, ContainingBlockInfo> containing_blocks_map_;
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::OutOfFlowLayoutPart::MulticolChildInfo)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::OutOfFlowLayoutPart::NodeToLayout)
