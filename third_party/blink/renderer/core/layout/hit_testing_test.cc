...
using HitNodeCb = base::MockRepeatingCallback<ListBasedHitTestBehavior(const Node& node)>;
using testing::_;
...
protected:
    PositionWithAffinity HitTest(const PhysicalOffset offset)
    {
        ...
        LayoutObject* const layout_object = hit_result.InnerPossiblyPseudoNode()->GetLayoutObject();
        if (!layout_object)
            return PositionWithAffinity();
...
public:
    explicit HitNodeCallbackStopper(Node* stop_node)
        : stop_node_(stop_node)
    {
    }
    ...
    bool DidStopHitTesting()
    {
        return did_stop_hit_testing_;
    }
...
TEST_F(HitTestingTest, HitTestWithCallback)
{
    ...
    // set to the target.
    EXPECT_CALL(hit_node_cb, Run(_)).WillRepeatedly(Return(ListBasedHitTestBehavior::kContinueHitTesting));

    LocalFrame* frame = GetDocument().GetFrame();
    DCHECK(!frame->View()->NeedsLayout());
    const PhysicalRect& hit_rect = VisualRectInDocument(*target->GetLayoutObject());
    ...
    HitTestResult result = frame->GetEventHandler().HitTestResultAtLocation(location, hit_type, target->GetLayoutObject(), true, hit_node_cb.Get());
    ...
    const int div_height = GetLayoutObjectByElementId("target")->StyleRef().Height().IntValue();
    occluder_1->SetInlineStyleProperty(CSSPropertyID::kMarginTop, "-10px");
    occluder_2->SetInlineStyleProperty(CSSPropertyID::kMarginTop, String::Format("%dpx", (-div_height * 1) - 10));
    occluder_3->SetInlineStyleProperty(CSSPropertyID::kMarginTop, String::Format("%dpx", (-div_height * 2) - 10));
    UpdateAllLifecyclePhasesForTest();

    // Set up HitNodeCb helper, and the HitNodeCb expectations.
    Node* stop_node = GetElementById("occluder_2");
    HitNodeCallbackStopper* hit_node_callback_stopper = MakeGarbageCollected<HitNodeCallbackStopper>(stop_node);
    EXPECT_CALL(hit_node_cb, Run(_)).WillRepeatedly(testing::Invoke(hit_node_callback_stopper, &HitNodeCallbackStopper::StopAtNode));
    ...
    result = frame->GetEventHandler().HitTestResultAtLocation(location, hit_type, target->GetLayoutObject(), true, hit_node_cb.Get());
...
line9</div>
    ...
    EXPECT_EQ(PositionWithAffinity(Position(text, 12)), HitTest(PhysicalOffset(5, 5)));
}
