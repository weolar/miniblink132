...
AnchorPositionVisibilityObserver::AnchorPositionVisibilityObserver(Element& anchored_element)
...
void AnchorPositionVisibilityObserver::MonitorAnchor(const Element* anchor)
{
    ...
    if (anchor_element_) {
        ...
        observer_ = IntersectionObserver::Create(anchor_element_->GetDocument(),
            WTF::BindRepeating(&AnchorPositionVisibilityObserver::OnIntersectionVisibilityChanged, WrapWeakPersistent(this)),
        ...
        SetLayerInvisible(LayerPositionVisibility::kAnchorsIntersectionVisible, false);
...
void AnchorPositionVisibilityObserver::UpdateForChainedAnchorVisibility(const HeapHashSet<WeakMember<ScrollSnapshotClient>>& clients)
{
    HeapVector<Member<AnchorPositionVisibilityObserver>> observers_with_chained_anchor;
    for (auto& client : clients) {
        if (auto* scroll_data = DynamicTo<AnchorPositionScrollData>(client.Get())) {
            if (auto* observer = scroll_data->GetAnchorPositionVisibilityObserver()) {
                observer->SetLayerInvisible(LayerPositionVisibility::kChainedAnchorsVisible, false);
    ...
    for (auto& observer : observers_with_chained_anchor) {
        observer->SetLayerInvisible(LayerPositionVisibility::kChainedAnchorsVisible, observer->IsInvisibleForChainedAnchorVisibility());
    }
}

bool AnchorPositionVisibilityObserver::IsInvisibleForChainedAnchorVisibility() const
{
    DCHECK(anchored_element_->GetAnchorPositionScrollData()->DefaultAnchorHasChainedAnchor());
    if (!anchor_element_ || !anchor_element_->GetLayoutObject()) {
        return false;
    }
    for (auto* layer = anchor_element_->GetLayoutObject()->EnclosingLayer(); layer; layer = layer->Parent()) {
        if (auto* box = layer->GetLayoutBox()) {
            if (auto* chained_data = box->GetAnchorPositionScrollData()) {
                ...
                if (auto* chained_observer = chained_data->GetAnchorPositionVisibilityObserver();
...
void AnchorPositionVisibilityObserver::SetLayerInvisible(LayerPositionVisibility position_visibility, bool invisible)
{
    LayoutBoxModelObject* layout_object = anchored_element_->GetLayoutBoxModelObject();
    if (!layout_object) {
        return;
    }
...
void AnchorPositionVisibilityObserver::OnIntersectionVisibilityChanged(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
    CHECK_EQ(entries.size(), 1u);
    CHECK_EQ(entries.front()->target(), anchor_element_);
    bool invisible = !entries.front()->isIntersecting();
    SetLayerInvisible(LayerPositionVisibility::kAnchorsIntersectionVisible, invisible);
}
