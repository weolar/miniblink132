...
LayoutUnit TextAreaIntrinsicInlineSize(const HTMLTextAreaElement& textarea, const LayoutBox& box)
{
    // Always add the scrollbar thickness for 'overflow:auto'.
    const auto& style = box.StyleRef();
    int scrollbar_thickness = 0;
    if (style.OverflowBlockDirection() == EOverflow::kScroll || style.OverflowBlockDirection() == EOverflow::kAuto) {
        scrollbar_thickness = layout_text_control::ScrollbarThickness(box);
    }

    return LayoutUnit(ceilf(layout_text_control::GetAvgCharWidth(style) * textarea.cols())) + scrollbar_thickness;
}

LayoutUnit TextFieldIntrinsicInlineSize(const HTMLInputElement& input, const LayoutBox& box)
{
    int factor;
    ...
    if (includes_decoration) {
        const auto* spin_button = To<HTMLElement>(input.UserAgentShadowRoot()->getElementById(shadow_element_names::kIdSpinButton));
        if (LayoutBox* spin_box = spin_button ? spin_button->GetLayoutBox() : nullptr) {
            ...
            if (logical_width.IsPercent()) {
                if (logical_width.Value() != 100.f) {
                    result += result * logical_width.Value() / (100 - logical_width.Value());
                }
            } else {
...
LayoutUnit TextAreaIntrinsicBlockSize(const HTMLTextAreaElement& textarea, const LayoutBox& box)
{
    ...
    const LayoutUnit line_height = inner_editor && inner_editor->GetLayoutBox() ? inner_editor->GetLayoutBox()->FirstLineHeight() : box.FirstLineHeight();

    return line_height * textarea.rows() + scrollbar_thickness;
}

LayoutUnit TextFieldIntrinsicBlockSize(const HTMLInputElement& input, const LayoutBox& box)
{
    ...
    const LayoutBox& target_box = (inner_editor && inner_editor->GetLayoutBox()) ? *inner_editor->GetLayoutBox() : box;
    return target_box.FirstLineHeight();
}

LayoutUnit FileUploadControlIntrinsicInlineSize(const HTMLInputElement& input, const LayoutBox& box)
{
    ...
    const float min_default_label_width = kDefaultWidthNumChars * ComputeTextWidth(character_as_string, box.StyleRef());

    const String label = input.GetLocale().QueryString(IDS_FORM_FILE_NO_FILE_LABEL);
    ...
    if (HTMLInputElement* button = input.UploadButton()) {
        if (auto* button_box = button->GetLayoutBox()) {
            ...
            ConstraintSpaceBuilder builder(mode, button_style.GetWritingDirection(),
                /* is_new_fc */ true);
            LayoutUnit max = BlockNode(button_box).ComputeMinMaxSizes(mode, SizeType::kIntrinsic, builder.ToConstraintSpace()).sizes.max_size;
            default_label_width += max + (kAfterButtonSpacing * box.StyleRef().EffectiveZoom());
        }
    }
    return LayoutUnit(ceilf(std::max(min_default_label_width, default_label_width)));
}
...
LogicalSize ThemePartIntrinsicSize(const LayoutBox& box, WebThemeEngine::Part part)
{
    const auto& style = box.StyleRef();
    PhysicalSize size(WebThemeEngineHelper::GetNativeThemeEngine()->GetSize(part));
...
LayoutUnit ListBoxDefaultItemHeight(const LayoutBox& box)
{
    ...
    if (!font_data)
        return LayoutUnit();
    return LayoutUnit(font_data->GetFontMetrics().Height() + kDefaultPaddingBottom);
}
...
LayoutUnit ListBoxItemBlockSize(const HTMLSelectElement& select, const LayoutBox& box)
{
    ...
    for (Element* element : items) {
        ...
        if (auto* layout_box = element->GetLayoutBox()) {
            item_block_size = box.StyleRef().IsHorizontalWritingMode() ? layout_box->Size().height : layout_box->Size().width;
        } else {
...
LayoutUnit MenuListIntrinsicInlineSize(const HTMLSelectElement& select, const LayoutBox& box)
{
    const ComputedStyle& style = box.StyleRef();
    float max_option_width = 0;
    if (!box.ShouldApplySizeContainment()) {
        for (auto* const option : select.GetOptionList()) {
            String text = style.ApplyTextTransform(option->TextIndentedToRespectGroupLabel());
            ...
            max_option_width = std::max(max_option_width, ComputeTextWidth(text, style));
        }
    }

    LayoutTheme& theme = LayoutTheme::GetTheme();
    int paddings = theme.PopupInternalPaddingStart(style) + theme.PopupInternalPaddingEnd(box.GetFrame(), style);
    return LayoutUnit(ceilf(max_option_width)) + LayoutUnit(paddings);
}

LayoutUnit MenuListIntrinsicBlockSize(const HTMLSelectElement& select, const LayoutBox& box)
{
    ...
    return (font_data ? font_data->GetFontMetrics().Height() : 0) + (inner_box ? inner_box->BorderAndPaddingBlockSize() : LayoutUnit());
}

#if DCHECK_IS_ON()
void CheckDidAddFragment(const LayoutBox& box, const PhysicalBoxFragment& new_fragment, wtf_size_t new_fragment_index = kNotFound)
{
...
#else
inline void CheckDidAddFragment(const LayoutBox& box, const PhysicalBoxFragment& fragment, wtf_size_t new_fragment_index = kNotFound)
{
}
#endif
...
// changed.
void ApplyOverflowClip(OverflowClipAxes overflow_clip_axes, const PhysicalRect& no_overflow_rect, PhysicalRect& result)
{
...
int HypotheticalScrollbarThickness(const LayoutBox& box, ScrollbarOrientation scrollbar_orientation, bool should_include_overlay_thickness)
{
    box.CheckIsNotDestroyed();

    if (PaintLayerScrollableArea* scrollable_area = box.GetScrollableArea()) {
        return scrollable_area->HypotheticalScrollbarThickness(scrollbar_orientation, should_include_overlay_thickness);
    } else {
        ...
        if (theme.UsesOverlayScrollbars() && !should_include_overlay_thickness) {
            ...
            float scale_from_dip = chrome_client.WindowToViewportScalar(document.GetFrame(), 1.0f);
            return theme.ScrollbarThickness(scale_from_dip, box.StyleRef().UsedScrollbarWidth());
        }
    }
}

void RecalcFragmentScrollableOverflow(RecalcScrollableOverflowResult& result, const PhysicalFragment& fragment)
{
...
std::tuple<LogicalOffset, WritingMode> LogicalLocation(const LayoutBox& box)
{
    ...
    WritingModeConverter converter({ writing_mode, TextDirection::kLtr }, PhysicalSize(container->Size()));
    return { converter.ToLogical(box.PhysicalLocation(), PhysicalSize(box.Size())), writing_mode };
}
...
{
    NOT_DESTROYED();
    if (IsStacked() || HasHiddenBackface() || (StyleRef().SpecifiesColumns() && !IsLayoutNGObject()))
...
void LayoutBox::StyleWillChange(StyleDifference diff, const ComputedStyle& new_style)
{
    NOT_DESTROYED();
    ...
    if (old_style) {
        ...
        if (diff.NeedsFullLayout() && Parent()) {
            ...
            if ((old_style->IsFloating() || old_style->HasOutOfFlowPosition()) && !new_style.IsFloating() && !new_style.HasOutOfFlowPosition()) {
...
void LayoutBox::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    if (auto* parent_flow_block = DynamicTo<LayoutBlockFlow>(Parent())) {
        if (IsFloatingOrOutOfFlowPositioned() && old_style && !old_style->IsFloating() && !old_style->HasOutOfFlowPosition()) {
    ...
    if (IsScrollContainer() && old_style && old_style->EffectiveZoom() != new_style.EffectiveZoom()) {
    ...
    if (old_style) {
        ...
        if (ShouldClipOverflowAlongEitherAxis()) {
            ...
            if (!old_style->BorderSizeEquals(new_style) || diff.BorderRadiusChanged() || (HasControlClip() && !old_style->PaddingEqual(new_style))) {
                SetNeedsPaintPropertyUpdate();
            }
        }

        if (old_style->OverscrollBehaviorX() != new_style.OverscrollBehaviorX() || old_style->OverscrollBehaviorY() != new_style.OverscrollBehaviorY()) {
            SetNeedsPaintPropertyUpdate();
        }

        if (old_style->OverflowX() != new_style.OverflowX() || old_style->OverflowY() != new_style.OverflowY()) {
            SetNeedsPaintPropertyUpdate();
        }
        ...
        if (IsInLayoutNGInlineFormattingContext() && IsAtomicInlineLevel() && old_style->Direction() != new_style.Direction()) {
            SetNeedsCollectInlines();
        }

        if (IsBackgroundAttachmentFixedObject() && new_style.BackgroundLayers().Clip() != old_style->BackgroundLayers().Clip()) {
            SetNeedsPaintPropertyUpdate();
        }
    }
...
void LayoutBox::UpdateShapeOutsideInfoAfterStyleChange(const ComputedStyle& style, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    const ShapeValue* shape_outside = style.ShapeOutside();
    const ShapeValue* old_shape_outside = old_style ? old_style->ShapeOutside() : ComputedStyleInitialValues::InitialShapeOutside();

    const Length& shape_margin = style.ShapeMargin();
    const Length& old_shape_margin = old_style ? old_style->ShapeMargin() : ComputedStyleInitialValues::InitialShapeMargin();

    float shape_image_threshold = style.ShapeImageThreshold();
    float old_shape_image_threshold = old_style ? old_style->ShapeImageThreshold() : ComputedStyleInitialValues::InitialShapeImageThreshold();

    // FIXME: A future optimization would do a deep comparison for equality. (bug
    // 100811)
    if (shape_outside == old_shape_outside && shape_margin == old_shape_margin && shape_image_threshold == old_shape_image_threshold)
        return;
...
bool GridStyleChanged(const ComputedStyle* old_style, const ComputedStyle& current_style)
{
    return old_style->GridColumnStart() != current_style.GridColumnStart() || old_style->GridColumnEnd() != current_style.GridColumnEnd()
        || old_style->GridRowStart() != current_style.GridRowStart() || old_style->GridRowEnd() != current_style.GridRowEnd()
        || old_style->Order() != current_style.Order() || old_style->HasOutOfFlowPosition() != current_style.HasOutOfFlowPosition();
}

bool AlignmentChanged(const ComputedStyle* old_style, const ComputedStyle& current_style)
{
    return old_style->AlignSelf() != current_style.AlignSelf() || old_style->JustifySelf() != current_style.JustifySelf();
}

} // namespace

void LayoutBox::UpdateGridPositionAfterStyleChange(const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    LayoutBlock* containing_block = ContainingBlock();
    if ((containing_block && containing_block->IsLayoutGrid()) && GridStyleChanged(old_style, StyleRef())) {
        ...
        if (is_out_of_flow) {
            containing_block->SetNeedsLayout(layout_invalidation_reason::kGridChanged, kMarkContainerChain);
        }
    }
    ...
    if (is_out_of_flow && AlignmentChanged(old_style, StyleRef())) {
        ...
        if (grid_ng_ancestor) {
            grid_ng_ancestor->SetNeedsLayout(layout_invalidation_reason::kGridChanged, kMarkContainerChain);
        }
    }
}

void LayoutBox::UpdateScrollSnapMappingAfterStyleChange(const ComputedStyle& old_style)
{
    NOT_DESTROYED();
    DCHECK(Style());
    // scroll-snap-type and scroll-padding invalidate the snap container.
    if (old_style.GetScrollSnapType() != StyleRef().GetScrollSnapType() || old_style.ScrollPaddingBottom() != StyleRef().ScrollPaddingBottom()
        || old_style.ScrollPaddingLeft() != StyleRef().ScrollPaddingLeft() || old_style.ScrollPaddingTop() != StyleRef().ScrollPaddingTop()
    ...
    if (old_style.GetScrollSnapAlign() != StyleRef().GetScrollSnapAlign()) {
        if (auto* containing_block = ContainingBlock()) {
            containing_block->SetNeedsLayout(layout_invalidation_reason::kStyleChange, kMarkContainerChain);
        }
    }
    ...
    if (old_style.ScrollSnapStop() != StyleRef().ScrollSnapStop() || old_style.ScrollMarginBottom() != StyleRef().ScrollMarginBottom()
        || old_style.ScrollMarginLeft() != StyleRef().ScrollMarginLeft() || old_style.ScrollMarginTop() != StyleRef().ScrollMarginTop()
...
void LayoutBox::UpdateFromStyle()
{
    ...
    SetFloating(style_to_use.IsFloating() && !IsOutOfFlowPositioned() && !style_to_use.IsInsideDisplayIgnoringFloatingChildren());
    SetHasTransformRelatedProperty(IsSVGChild() ? style_to_use.HasTransformRelatedPropertyForSVG() : style_to_use.HasTransformRelatedProperty());
    SetHasReflection(style_to_use.BoxReflect());

    bool should_clip_overflow = (!StyleRef().IsOverflowVisibleAlongBothAxes() || ShouldApplyPaintContainment()) && RespectsCSSOverflow();
...
void LayoutBox::LayoutSubtreeRoot()
{
    ...
    // optimization.
    const auto& previous_fragment = To<PhysicalBoxFragment>(previous_result->GetPhysicalFragment());
    const auto& fragment = To<PhysicalBoxFragment>(result->GetPhysicalFragment());
    if (previous_fragment.FirstBaseline() != fragment.FirstBaseline() || previous_fragment.LastBaseline() != fragment.LastBaseline()
        || fragment.HasPropagatedLayoutObjects()) {
        if (auto* containing_block = ContainingBlock()) {
            containing_block->SetNeedsLayout(layout_invalidation_reason::kChildChanged, kMarkContainerChain);
        }
    }
}
...
LayoutUnit LayoutBox::ClientWidth() const
{
    ...
    if (CanSkipComputeScrollbars()) {
        ...
        return (width - BorderLeft() - BorderRight() - ComputeScrollbarsInternal(kClampToContentBox).HorizontalSum()).ClampNegativeToZero();
    }
}
...
LayoutUnit LayoutBox::ClientHeight() const
{
    ...
    if (CanSkipComputeScrollbars()) {
        ...
        return (height - BorderTop() - BorderBottom() - ComputeScrollbarsInternal(kClampToContentBox).VerticalSum()).ClampNegativeToZero();
    }
}
...
LayoutUnit LayoutBox::ScrollWidth() const
{
    ...
    if (StyleRef().IsScrollbarGutterStable() && StyleRef().OverflowBlockDirection() == EOverflow::kHidden) {
    ...
    return ClientWidth() - std::min(LayoutUnit(), overflow_rect.X() - BorderLeft());
}
...
LayoutUnit LayoutBox::ScrollHeight() const
{
    ...
    return std::max(ClientHeight(), ScrollableOverflowRect().Bottom() - BorderTop());
}
...
void LayoutBox::QuadsInAncestorInternal(Vector<gfx::QuadF>& quads, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    ...
    quads.push_back(LocalRectToAncestorQuad(PhysicalBorderBoxRect(), ancestor, mode));
}
...
LayoutUnit LayoutBox::OverrideIntrinsicContentInlineSize() const
{
    ...
    const StyleIntrinsicLength& intrinsic_length = style.ContainIntrinsicInlineSize();
        ...
        if (context && context->IsLocked()) {
            if (const auto* elem = DynamicTo<Element>(GetNode())) {
                if (const auto inline_size = elem->LastRememberedInlineSize()) {
                    ...
                    return LayoutUnit::FromFloatRound(*inline_size * style.EffectiveZoom());
                }
            }
        }
    }
...
LayoutUnit LayoutBox::OverrideIntrinsicContentBlockSize() const
{
    ...
    const auto& style = StyleRef();
    const StyleIntrinsicLength& intrinsic_length = style.ContainIntrinsicBlockSize();
...
LayoutUnit LayoutBox::DefaultIntrinsicContentInlineSize() const
{
    ...
    if (select && select->UsesMenuList() && !select->IsAppearanceBaseButton()) [[unlikely]] {
        return apply_fixed_size ? MenuListIntrinsicInlineSize(*select, *this) : kIndefiniteSize;
    }
    ...
    if (input) [[unlikely]] {
        ...
        if (effective_appearance == kCheckboxPart) {
            return ThemePartIntrinsicSize(*this, WebThemeEngine::kPartCheckbox).inline_size;
        }
        if (effective_appearance == kRadioPart) {
            return ThemePartIntrinsicSize(*this, WebThemeEngine::kPartRadio).inline_size;
        }
        return kIndefiniteSize;
    }
...
LayoutUnit LayoutBox::DefaultIntrinsicContentBlockSize() const
{
    ...
    auto effective_appearance = StyleRef().EffectiveAppearance();
    if (effective_appearance == kCheckboxPart) {
        return ThemePartIntrinsicSize(*this, WebThemeEngine::kPartCheckbox).block_size;
    }
    if (effective_appearance == kRadioPart) {
    ...
    if (const auto* select = DynamicTo<HTMLSelectElement>(GetNode())) {
        if (!select->IsAppearanceBaseButton()) {
            ...
            return ListBoxItemBlockSize(*select, *this) * select->ListBoxSize() - ComputeLogicalScrollbars().BlockSum();
        }
    }
    if (IsTextField()) {
    ...
    if (IsTextArea()) {
        return TextAreaIntrinsicBlockSize(*To<HTMLTextAreaElement>(GetNode()), *this);
    }
...
LayoutUnit LayoutBox::LogicalLeft() const
{
    ...
    return IsParallelWritingMode(container_writing_mode, StyleRef().GetWritingMode()) ? offset.inline_offset : offset.block_offset;
}
...
LayoutUnit LayoutBox::LogicalTop() const
{
    NOT_DESTROYED();
    auto [offset, container_writing_mode] = LogicalLocation(*this);
    return IsParallelWritingMode(container_writing_mode, StyleRef().GetWritingMode()) ? offset.block_offset : offset.inline_offset;
}
...
PhysicalRect LayoutBox::PhysicalBackgroundRect(BackgroundRectType rect_type) const
{
    NOT_DESTROYED();
    ...
    for (const FillLayer* cur = &(StyleRef().BackgroundLayers()); cur; cur = cur->Next()) {
        EFillBox current_clip = cur->Clip();
        if (rect_type == kBackgroundKnownOpaqueRect) {
            if (current_clip == EFillBox::kText)
                continue;

            if (cur->GetBlendMode() != BlendMode::kNormal || cur->Composite() != kCompositeSourceOver)
                continue;

            bool layer_known_opaque = false;
            // Check if the image is opaque and fills the clip.
            if (const StyleImage* image = cur->GetImage()) {
                if ((cur->Repeat().x == EFillRepeat::kRepeatFill || cur->Repeat().x == EFillRepeat::kRoundFill)
                    && (cur->Repeat().y == EFillRepeat::kRepeatFill || cur->Repeat().y == EFillRepeat::kRoundFill)
            ...
            if (!cur->GetImage() && (cur->Next() || background_color.IsFullyTransparent())) {
                continue;
            }
            // A content-box clipped fill layer can be scrolled into the padding box
            // of the overflow container.
            if (current_clip == EFillBox::kContent && cur->Attachment() == EFillAttachment::kLocal) {
                current_clip = EFillBox::kPadding;
            }
        }

        // Restrict clip if attachment is local.
        if (current_clip == EFillBox::kBorder && cur->Attachment() == EFillAttachment::kLocal)
            current_clip = EFillBox::kPadding;

        background_box = background_box ? EnclosingFillBox(*background_box, current_clip) : current_clip;
    }
    ...
    if (rect_type == kBackgroundPaintedExtent && *background_box == EFillBox::kBorder && BackgroundClipBorderBoxIsEquivalentToPaddingBox()) {
...
void LayoutBox::AddOutlineRects(OutlineRectCollector& collector, OutlineInfo* info, const PhysicalOffset& additional_offset, OutlineType) const
{
    NOT_DESTROYED();
...
bool LayoutBox::HasScrollbarGutters(ScrollbarOrientation orientation) const
{
    ...
    if (orientation == kVerticalScrollbar) {
        ...
        return StyleRef().IsHorizontalWritingMode() && (overflow == EOverflow::kAuto || overflow == EOverflow::kScroll || overflow == EOverflow::kHidden)
            && !UsesOverlayScrollbars() && GetNode() != GetDocument().ViewportDefiningElement();
    } else {
...
PhysicalBoxStrut LayoutBox::ComputeScrollbarsInternal(ShouldClampToContentBox clamp_to_content_box,
    OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior, ShouldIncludeScrollbarGutter include_scrollbar_gutter) const
{
    NOT_DESTROYED();
    PhysicalBoxStrut scrollbars;
    PaintLayerScrollableArea* scrollable_area = GetScrollableArea();

    if (include_scrollbar_gutter == kIncludeScrollbarGutter && HasScrollbarGutters(kVerticalScrollbar)) {
        LayoutUnit gutter_size = LayoutUnit(HypotheticalScrollbarThickness(*this, kVerticalScrollbar, /* include_overlay_thickness */ true));
        ...
        if (ShouldPlaceVerticalScrollbarOnLeft()) {
            scrollbars.left = LayoutUnit(scrollable_area->VerticalScrollbarWidth(overlay_scrollbar_clip_behavior));
        } else {
            scrollbars.right = LayoutUnit(scrollable_area->VerticalScrollbarWidth(overlay_scrollbar_clip_behavior));
        }
    }

    if (include_scrollbar_gutter == kIncludeScrollbarGutter && HasScrollbarGutters(kHorizontalScrollbar)) {
        LayoutUnit gutter_size = LayoutUnit(HypotheticalScrollbarThickness(*this, kHorizontalScrollbar,
            /* include_overlay_thickness */ true));
        scrollbars.bottom = gutter_size;
        if (StyleRef().IsScrollbarGutterBothEdges())
            scrollbars.top = gutter_size;
    } else if (scrollable_area) {
        scrollbars.bottom = LayoutUnit(scrollable_area->HorizontalScrollbarHeight(overlay_scrollbar_clip_behavior));
    }
    ...
    if (scrollbars.left > 0 && clamp_to_content_box == kClampToContentBox) {
        ...
        scrollbars.left = std::min(scrollbars.left, max_width.ClampNegativeToZero());
    }
...
void LayoutBox::Autoscroll(const PhysicalOffset& position_in_root_frame)
{
    ...
    PhysicalOffset absolute_position = frame_view->ConvertFromRootFrame(position_in_root_frame);
    mojom::blink::ScrollIntoViewParamsPtr params = scroll_into_view_util::CreateScrollIntoViewParams(
        ScrollAlignment::ToEdgeIfNeeded(), ScrollAlignment::ToEdgeIfNeeded(), mojom::blink::ScrollType::kUser);
    scroll_into_view_util::ScrollRectToVisible(*this, PhysicalRect(absolute_position, PhysicalSize(LayoutUnit(1), LayoutUnit(1))), std::move(params));
}
...
PhysicalOffset LayoutBox::CalculateAutoscrollDirection(const gfx::PointF& point_in_root_frame) const
{
    NOT_DESTROYED();
    if (!GetFrame())
    ...
    ExcludeScrollbars(absolute_scrolling_box, kExcludeOverlayScrollbarSizeForHitTesting);

    PhysicalRect belt_box = View()->GetFrameView()->ConvertToRootFrame(absolute_scrolling_box);
...
LayoutBox* LayoutBox::FindAutoscrollable(LayoutObject* layout_object, bool is_middle_click_autoscroll)
{
    while (layout_object && !(layout_object->IsBox() && To<LayoutBox>(layout_object)->IsUserScrollable())) {
        // Do not start selection-based autoscroll when the node is inside a
        // fixed-position element.
        if (!is_middle_click_autoscroll && layout_object->IsBox() && To<LayoutBox>(layout_object)->IsFixedToView()) {
            return nullptr;
        }

        if (!layout_object->Parent() && layout_object->GetNode() == layout_object->GetDocument() && layout_object->GetDocument().LocalOwner()) {
            layout_object = layout_object->GetDocument().LocalOwner()->GetLayoutObject();
        } else {
...
{
    while (layout_object) {
        if (layout_object->IsBox() && To<LayoutBox>(layout_object)->HasScrollableOverflowX())
            return true;

        // Scroll is not propagating.
        if (layout_object->StyleRef().OverscrollBehaviorX() != EOverscrollBehavior::kAuto)
            break;
...
{
    NOT_DESTROYED();
    return GetScrollableArea() ? GetScrollableArea()->ScrollOrigin() : gfx::Point();
}
...
PhysicalOffset LayoutBox::ScrolledContentOffset() const
{
    ...
    return PhysicalOffset::FromVector2dFFloor(GetScrollableArea()->GetScrollOffset());
}
...
bool LayoutBox::MapVisualRectToContainer(const LayoutObject* container_object, const PhysicalOffset& container_offset, const LayoutObject* ancestor,
    VisualRectFlags visual_rect_flags, TransformState& transform_state) const
{
    NOT_DESTROYED();
    bool container_preserve_3d = container_object->StyleRef().Preserves3D() && container_object == NearestAncestorForElement();

    TransformState::TransformAccumulation accumulation = container_preserve_3d ? TransformState::kAccumulateTransform : TransformState::kFlattenTransform;
    ...
    if (!ShouldUseTransformFromContainer(container_object)) {
        ...
        if (container_object->IsBox() && container_object != ancestor
            && !To<LayoutBox>(container_object)->MapContentsRectToBoxSpace(transform_state, accumulation, *this, visual_rect_flags)) {
            return false;
        }
        return true;
    }
    ...
    if (!StyleRef().Preserves3D()) {
        ...
        transform_state.SetQuad(gfx::QuadF(gfx::RectF(gfx::ToEnclosingRect(transform_state.LastPlanarQuad().BoundingBox()))));
    }
    ...
    // b) Container offset.
    transform.PostTranslate(container_offset.left.ToFloat(), container_offset.top.ToFloat());

    // c) Container scroll offset.
    if (container_object->IsBox() && container_object != ancestor && To<LayoutBox>(container_object)->ContainedContentsScroll(*this)) {
        PhysicalOffset offset(-To<LayoutBox>(container_object)->ScrolledContentOffset());
    ...
    if (container_object->IsBox() && container_object != ancestor && container_object->HasClipRelatedProperty()) {
        return To<LayoutBox>(container_object)->ApplyBoxClips(transform_state, accumulation, visual_rect_flags);
    }
...
bool LayoutBox::MapContentsRectToBoxSpace(
    TransformState& transform_state, TransformState::TransformAccumulation accumulation, const LayoutObject& contents, VisualRectFlags visual_rect_flags) const
{
    NOT_DESTROYED();
...
{
    NOT_DESTROYED();
    if (IsA<LayoutView>(this) && contents.StyleRef().GetPosition() == EPosition::kFixed) {
        return false;
    }
    return IsScrollContainer();
}

bool LayoutBox::ApplyBoxClips(TransformState& transform_state, TransformState::TransformAccumulation accumulation, VisualRectFlags visual_rect_flags) const
{
    NOT_DESTROYED();
    ...
    transform_state.Flatten();
    PhysicalRect rect(gfx::ToEnclosingRect(transform_state.LastPlanarQuad().BoundingBox()));
    bool does_intersect;
...
{
    NOT_DESTROYED();
    return rare_data_ && rare_data_->has_override_containing_block_content_logical_width_;
}

// TODO (lajava) Shouldn't we implement these functions based on physical
// direction ?.
void LayoutBox::SetOverrideContainingBlockContentLogicalWidth(LayoutUnit logical_width)
{
    NOT_DESTROYED();
    DCHECK_GE(logical_width, LayoutUnit(-1));
    EnsureRareData().override_containing_block_content_logical_width_ = logical_width;
...
bool LayoutBox::HitTestAllPhases(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset)
{
    NOT_DESTROYED();
    if (!MayIntersect(result, hit_test_location, accumulated_offset))
        return false;
    return LayoutObject::HitTestAllPhases(result, hit_test_location, accumulated_offset);
}

bool LayoutBox::HitTestOverflowControl(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& adjusted_location) const
{
    NOT_DESTROYED();
    ...
    if (!scrollable_area->HitTestOverflowControls(result, ToRoundedPoint(local_point)))
        return false;

    UpdateHitTestResult(result, local_point);
    return result.AddNodeToListBasedTestResult(NodeForHitTest(), hit_test_location) == kStopHitTesting;
}

bool LayoutBox::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    if (!MayIntersect(result, hit_test_location, accumulated_offset))
        return false;

    if (phase == HitTestPhase::kForeground && !HasSelfPaintingLayer() && HitTestOverflowControl(result, hit_test_location, accumulated_offset))
        return true;

    bool skip_children = (result.GetHitTestRequest().GetStopNode() == this) || ChildPaintBlockedByDisplayLock();
    if (!skip_children && ShouldClipOverflowAlongEitherAxis()) {
        ...
        if (!HasSelfPaintingLayer() && !hit_test_location.Intersects(OverflowClipRect(accumulated_offset, kExcludeOverlayScrollbarSizeForHitTesting))) {
            skip_children = true;
        }
        if (!skip_children && StyleRef().HasBorderRadius()) {
            PhysicalRect bounds_rect(accumulated_offset, Size());
            skip_children = !hit_test_location.Intersects(RoundedBorderGeometry::PixelSnappedRoundedInnerBorder(StyleRef(), bounds_rect));
        }
    }

    if (!skip_children && HitTestChildren(result, hit_test_location, accumulated_offset, phase)) {
        return true;
    }

    if (StyleRef().HasBorderRadius() && HitTestClippedOutByBorder(hit_test_location, accumulated_offset))
        return false;

    // Now hit test ourselves.
    if (IsInSelfHitTestingPhase(phase) && VisibleToHitTestRequest(result.GetHitTestRequest())) {
        ...
        if (hit_test_location.Intersects(bounds_rect)) {
            ...
            if (result.AddNodeToListBasedTestResult(NodeForHitTest(), hit_test_location, bounds_rect) == kStopHitTesting)
                return true;
        }
    }
...
bool LayoutBox::HitTestChildren(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    for (LayoutObject* child = SlowLastChild(); child; child = child->PreviousSibling()) {
        if (child->HasLayer() && To<LayoutBoxModelObject>(child)->Layer()->IsSelfPaintingLayer())
            continue;
        ...
        if (child->NodeAtPoint(result, hit_test_location, child_accumulated_offset, phase))
            return true;
    }
...
bool LayoutBox::HitTestClippedOutByBorder(const HitTestLocation& hit_test_location, const PhysicalOffset& border_box_location) const
{
    NOT_DESTROYED();
    PhysicalRect border_rect = PhysicalBorderBoxRect();
    border_rect.Move(border_box_location);
    return !hit_test_location.Intersects(RoundedBorderGeometry::PixelSnappedRoundedBorder(StyleRef(), border_rect));
}
...
bool LayoutBox::BackgroundIsKnownToBeOpaqueInRect(const PhysicalRect& local_rect) const
{
    NOT_DESTROYED();
    ...
    if (StyleRef().HasBlendMode())
        return false;
    return PhysicalBackgroundRect(kBackgroundKnownOpaqueRect).Contains(local_rect);
}
...
static bool IsCandidateForOpaquenessTest(const LayoutBox& child_box)
{
    ...
    if (child_style.Visibility() != EVisibility::kVisible || child_style.ShapeOutside()) {
        return false;
    }
...
bool LayoutBox::ForegroundIsKnownToBeOpaqueInRect(const PhysicalRect& local_rect, unsigned max_depth_to_test) const
{
    NOT_DESTROYED();
    ...
    for (LayoutObject* child = SlowFirstChild(); child; child = child->NextSibling()) {
        ...
        if (child_local_rect.Bottom() > child_box->Size().height || child_local_rect.Right() > child_box->Size().width) {
            continue;
        }
        if (RuntimeEnabledFeatures::CompositeBGColorAnimationEnabled() && child->Style()->HasCurrentBackgroundColorAnimation()) {
            return false;
        }
        if (child_box->BackgroundIsKnownToBeOpaqueInRect(child_local_rect))
            return true;
        if (child_box->ForegroundIsKnownToBeOpaqueInRect(child_local_rect, max_depth_to_test - 1))
            return true;
    }
    return false;
}
...
bool LayoutBox::ComputeBackgroundIsKnownToBeObscured() const
{
    ...
    if (StyleRef().BoxShadow())
        return false;
    return ForegroundIsKnownToBeOpaqueInRect(BackgroundPaintedExtent(), kBackgroundObscurationTestMaxDepth);
}

void LayoutBox::ImageChanged(WrappedImagePtr image, CanDeferInvalidation defer)
{
    NOT_DESTROYED();
    bool is_box_reflect_image
        = (StyleRef().BoxReflect() && StyleRef().BoxReflect()->Mask().GetImage() && StyleRef().BoxReflect()->Mask().GetImage()->Data() == image);
    ...
    // images.
    if ((StyleRef().BorderImage().GetImage() && StyleRef().BorderImage().GetImage()->Data() == image)
        || (StyleRef().MaskBoxImage().GetImage() && StyleRef().MaskBoxImage().GetImage()->Data() == image) || is_box_reflect_image) {
        SetShouldDoFullPaintInvalidationWithoutLayoutChange(PaintInvalidationReason::kImage);
    } else {
    ...
    if (!BackgroundTransfersToView()) {
        for (const FillLayer* layer = &StyleRef().BackgroundLayers(); layer; layer = layer->Next()) {
            if (layer->GetImage() && image == layer->GetImage()->Data()) {
                bool maybe_animated = layer->GetImage()->CachedImage() && layer->GetImage()->CachedImage()->GetImage()
    ...
    if (!GetFrameView()->IsInPerformLayout() && IsFloating() && shape_outside_value && shape_outside_value->GetImage()
...
ResourcePriority LayoutBox::ComputeResourcePriority() const
{
    ...
    return ResourcePriority(is_visible ? ResourcePriority::kVisible : ResourcePriority::kNotVisible, screen_area);
}
...
bool LayoutBox::IntersectsVisibleViewport() const
{
    ...
    return rect.Intersects(PhysicalRect(layout_view->GetFrameView()->GetScrollableArea()->VisibleContentRect()));
}
...
void LayoutBox::ClearPaintFlags()
{
    ...
    if (auto* scrollable_area = GetScrollableArea()) {
        if (auto* scrollbar = DynamicTo<CustomScrollbar>(scrollable_area->HorizontalScrollbar()))
            scrollbar->ClearPaintFlags();
        if (auto* scrollbar = DynamicTo<CustomScrollbar>(scrollable_area->VerticalScrollbar()))
            scrollbar->ClearPaintFlags();
    }
}

PhysicalRect LayoutBox::OverflowClipRect(const PhysicalOffset& location, OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior) const
{
    NOT_DESTROYED();
    ...
    if (IsScrollContainer()) {
        ...
        ExcludeScrollbars(clip_rect, overlay_scrollbar_clip_behavior, kExcludeScrollbarGutter);
    }
...
void LayoutBox::ExcludeScrollbars(
    PhysicalRect& rect, OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior, ShouldIncludeScrollbarGutter include_scrollbar_gutter) const
{
    NOT_DESTROYED();
    if (CanSkipComputeScrollbars())
        return;

    PhysicalBoxStrut scrollbars = ComputeScrollbarsInternal(kDoNotClampToContentBox, overlay_scrollbar_clip_behavior, include_scrollbar_gutter);
...
PhysicalRect LayoutBox::ClipRect(const PhysicalOffset& location) const
{
    ...
    if (!StyleRef().ClipRight().IsAuto()) {
        clip_rect.size.width -= width - ValueForLength(StyleRef().ClipRight(), width);
    }
    ...
    if (!StyleRef().ClipBottom().IsAuto()) {
        clip_rect.size.height -= height - ValueForLength(StyleRef().ClipBottom(), height);
    }
...
LayoutUnit LayoutBox::ContainingBlockLogicalHeightForRelPositioned() const
{
    ...
    return ContainingBlockLogicalHeightForPositioned(cb) - cb->PaddingLogicalHeight();
}
...
PhysicalOffset LayoutBox::OffsetFromContainerInternal(const LayoutObject* o, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
...
wtf_size_t LayoutBox::PhysicalFragmentList::IndexOf(const PhysicalBoxFragment& fragment) const
{
    wtf_size_t index = 0;
...
bool LayoutBox::PhysicalFragmentList::Contains(const PhysicalBoxFragment& fragment) const
{
...
void LayoutBox::AddMeasureLayoutResult(const LayoutResult* result)
{
    ...
    if (result->GetConstraintSpaceForCaching().CacheSlot() != LayoutResultCacheSlot::kMeasure) {
        return;
    }
    DCHECK(To<PhysicalBoxFragment>(result->GetPhysicalFragment()).IsOnlyForNode());
...
void LayoutBox::SetCachedLayoutResult(const LayoutResult* result, wtf_size_t index)
{
    NOT_DESTROYED();
    if (result->GetConstraintSpaceForCaching().CacheSlot() == LayoutResultCacheSlot::kMeasure) {
...
void LayoutBox::SetLayoutResult(const LayoutResult* result, wtf_size_t index)
{
    ...
    const auto& box_fragment = To<PhysicalBoxFragment>(result->GetPhysicalFragment());
    ...
    if (index != WTF::kNotFound && layout_results_.size() > index) {
        if (layout_results_.size() > index + 1) {
            ...
            if (!box_fragment.GetBreakToken() || box_fragment.GetBreakToken()->IsCausedByColumnSpanner() || box_fragment.IsFragmentationContextRoot()) {
...
void LayoutBox::ReplaceLayoutResult(const LayoutResult* result, wtf_size_t index)
{
    NOT_DESTROYED();
...
void LayoutBox::FinalizeLayoutResults()
{
    ...
    if (HasFragmentItems()) {
        FragmentItems::FinalizeAfterLayout(layout_results_, *To<LayoutBlockFlow>(this));
    }
}
...
void LayoutBox::RebuildFragmentTreeSpine()
{
    ...
    SCOPED_BLINK_UMA_HISTOGRAM_TIMER_HIGHRES("Blink.Layout.RebuildFragmentTreeSpine");
    ...
    while (container && container->PhysicalFragmentCount() && !container->NeedsLayout()) {
...
void LayoutBox::InvalidateCachedGeometry()
{
    ...
    if (auto* block_flow = DynamicTo<LayoutBlockFlow>(this)) {
        if (auto* flow_thread = block_flow->MultiColumnFlowThread()) {
            ...
            for (auto* sibling = flow_thread->NextSiblingBox(); sibling; sibling = sibling->NextSiblingBox()) {
...
void LayoutBox::InvalidateItems(const LayoutResult& result)
{
    ...
    if (!box_fragment.HasItems())
        return;
    ObjectPaintInvalidator(*box_fragment.GetLayoutObject()).SlowSetPaintingLayerNeedsRepaint();
}

const LayoutResult* LayoutBox::GetCachedLayoutResult(const BlockBreakToken* break_token) const
{
    NOT_DESTROYED();
    ...
    DCHECK(!result->GetPhysicalFragment().IsLayoutObjectDestroyedOrMoved() || BeingDestroyed());
    return result;
}

const LayoutResult* LayoutBox::GetCachedMeasureResult(const ConstraintSpace& space, std::optional<FragmentGeometry>* fragment_geometry) const
{
    NOT_DESTROYED();
    if (!measure_cache_) {
        return nullptr;
    ...
    return measure_cache_ ? measure_cache_->Find(BlockNode(const_cast<LayoutBox*>(this)), space, fragment_geometry) : nullptr;
}
...
const PhysicalBoxFragment& LayoutBox::PhysicalFragmentList::Iterator::operator*() const
{
...
{
    return To<PhysicalBoxFragment>(layout_results_.front()->GetPhysicalFragment());
}
...
const FragmentData* LayoutBox::FragmentDataFromPhysicalFragment(const PhysicalBoxFragment& physical_fragment) const
{
    NOT_DESTROYED();
...
void LayoutBox::InflateVisualRectForFilterUnderContainer(
    TransformState& transform_state, const LayoutObject& container, const LayoutBoxModelObject* ancestor_to_stop_at) const
{
    NOT_DESTROYED();
    transform_state.Flatten();
    ...
    for (LayoutObject* parent = Parent(); parent && parent != container; parent = parent->Parent()) {
        if (parent->IsBox()) {
    ...
    const LayoutBoxModelObject* ancestor, TransformState& transform_state, VisualRectFlags visual_rect_flags) const
{
    ...
    if (skip_info.FilterSkipped() && !(visual_rect_flags & kIgnoreFilters)) {
        InflateVisualRectForFilterUnderContainer(transform_state, *container, ancestor);
    }

    if (!MapVisualRectToContainer(container, container_offset, ancestor, visual_rect_flags, transform_state))
        return false;
    ...
    if (skip_info.AncestorSkipped()) {
        ...
        PhysicalOffset ancestor_container_offset = ancestor->OffsetFromAncestor(container);
    ...
    return container->MapToVisualRectInAncestorSpaceInternal(ancestor, transform_state, visual_rect_flags);
}

void LayoutBox::InflateVisualRectForFilter(TransformState& transform_state) const
{
    NOT_DESTROYED();
    ...
    transform_state.Flatten();
    PhysicalRect rect = PhysicalRect::EnclosingRect(transform_state.LastPlanarQuad().BoundingBox());
    transform_state.SetQuad(gfx::QuadF(gfx::RectF(Layer()->MapRectForFilter(rect))));
}

bool LayoutBox::SkipContainingBlockForPercentHeightCalculation(const LayoutBox* containing_block)
{
    ...
    if (containing_block->IsAnonymous()) {
        if (!in_quirks_mode && containing_block->Parent() && containing_block->Parent()->IsFieldset()) {
            return false;
        }
        EDisplay display = containing_block->StyleRef().Display();
        return display == EDisplay::kBlock || display == EDisplay::kInlineBlock || display == EDisplay::kFlowRoot;
    }

    // For quirks mode, we skip most auto-height containing blocks when computing
    // percentages.
    if (!in_quirks_mode || !containing_block->StyleRef().LogicalHeight().HasAuto()) {
        return false;
    }
    ...
    return !containing_block->IsLayoutReplaced() && !containing_block->IsTableCell() && !containing_block->IsOutOfFlowPositioned()
        && !containing_block->IsLayoutGrid() && !containing_block->IsFlexibleBox() && !containing_block->IsLayoutCustom();
}

LayoutUnit LayoutBox::ContainingBlockLogicalHeightForPositioned(const LayoutBoxModelObject* containing_block) const
{
    NOT_DESTROYED();

    // Use viewport as container for top-level fixed-position elements.
    const auto* view = DynamicTo<LayoutView>(containing_block);
    if (StyleRef().GetPosition() == EPosition::kFixed && view && !GetDocument().Printing()) {
        if (LocalFrameView* frame_view = view->GetFrameView()) {
            // Don't use visibleContentRect since the PaintLayer's size has not been
            // set yet.
            gfx::Size viewport_size = frame_view->LayoutViewport()->ExcludeScrollbars(frame_view->Size());
            return LayoutUnit(containing_block->IsHorizontalWritingMode() ? viewport_size.height() : viewport_size.width());
        }
    }
    ...
    DCHECK(containing_block->CanContainOutOfFlowPositionedElement(StyleRef().GetPosition()));
    ...
    height_result -= (containing_block->BorderBlockStart() + containing_block->BorderBlockEnd());
...
PhysicalRect LayoutBox::LocalCaretRect(int caret_offset) const
{
    ...
    LayoutUnit font_height = LayoutUnit(font_data ? font_data->GetFontMetrics().Height() : 0);
    ...
    bool apply_border_padding = GetNode() && !(EditingIgnoresContent(*GetNode()) || IsDisplayInsideTable(GetNode()));
    ...
    if (RuntimeEnabledFeatures::SidewaysWritingModesEnabled()) {
        ...
        if (apply_border_padding) {
            BoxStrut border_padding = (BorderOutsets() + PaddingOutsets()).ConvertToLogical(writing_direction);
    ...
    PhysicalOffset offset = PhysicalLocation();
    PhysicalRect rect(offset, is_horizontal ? PhysicalSize(caret_width, caret_block_size) : PhysicalSize(caret_block_size, caret_width));
    bool ltr = StyleRef().IsLeftToRightDirection();

    if ((!caret_offset) ^ ltr) {
        rect.Move(is_horizontal ? PhysicalOffset(size.inline_size - caret_width, LayoutUnit()) : PhysicalOffset(LayoutUnit(), size.inline_size - caret_width));
    }
...
PositionWithAffinity LayoutBox::PositionForPointInFragments(const PhysicalOffset& target) const
{
    NOT_DESTROYED();
    ...
    for (const PhysicalBoxFragment& fragment : PhysicalFragments()) {
        ...
        const PhysicalSize distance = PhysicalRect(fragment_offset, fragment.Size()).DistanceAsSize(target);
        if (distance.IsZero())
            return fragment.PositionForPoint(target - fragment_offset);

        // Otherwise find the closest fragment.
        const LayoutUnit square_distance = distance.width * distance.width + distance.height * distance.height;
...
PhysicalBoxStrut LayoutBox::ComputeVisualEffectOverflowOutsets()
{
    ...
    if (style.HasOutline()) {
        OutlineInfo info;
        Vector<PhysicalRect> outline_rects = OutlineRects(&info, PhysicalOffset(), style.OutlineRectsShouldIncludeBlockInkOverflow());
        ...
        outsets.Unite(PhysicalBoxStrut(-rect.Y(), rect.Right() - Size().width, rect.Bottom() - Size().height, -rect.X()));
    }
...
void LayoutBox::SetScrollableOverflowFromLayoutResults()
{
    ...
    for (const auto& layout_result : layout_results_) {
        const auto& fragment = To<PhysicalBoxFragment>(layout_result->GetPhysicalFragment());
        ...
        case WritingMode::kSidewaysRl:
            ...
            offset_adjust = { -fragment.Size().width - consumed_block_size, LayoutUnit() };
            break;
        case WritingMode::kVerticalLr:
        case WritingMode::kSidewaysLr:
    ...
    if (scrollable_overflow->IsEmpty() || PhysicalPaddingBoxRect().Contains(*scrollable_overflow)) {
        return;
    }
...
RecalcScrollableOverflowResult LayoutBox::RecalcScrollableOverflowNG()
{
    ...
    bool should_recalculate_scrollable_overflow = SelfNeedsScrollableOverflowRecalc() || child_result.scrollable_overflow_changed;
    ...
    if (rebuild_fragment_tree || should_recalculate_scrollable_overflow) {
        for (auto& layout_result : layout_results_) {
            ...
            if (should_recalculate_scrollable_overflow) {
                const PhysicalRect old_scrollable_overflow = fragment.ScrollableOverflow();
                const bool has_block_fragmentation = layout_result->GetConstraintSpaceForCaching().HasBlockFragmentation();
#if DCHECK_IS_ON()
                PhysicalBoxFragment::AllowPostLayoutScope allow_post_layout_scope;
#endif
                const PhysicalRect new_scrollable_overflow
                    = ScrollableOverflowCalculator::RecalculateScrollableOverflowForFragment(fragment, has_block_fragmentation);
            ...
            if (scrollable_overflow) {
                fragment.GetMutableForStyleRecalc().SetScrollableOverflow(*scrollable_overflow);
            }
        }
    ...
    scrollable_overflow_changed = scrollable_overflow_changed && !ShouldApplyLayoutContainment() && !ShouldClipOverflowAlongBothAxis();
    ...
    for (auto& layout_result : layout_results_) {
        ...
        if (fragment.HasItems()) {
            for (InlineCursor cursor(fragment); cursor; cursor.MoveToNext()) {
                const PhysicalBoxFragment* child = cursor.Current()->PostLayoutBoxFragment();
                if (!child || !child->GetLayoutObject()->IsBox())
                    continue;
                result.Unite(child->MutableOwnerLayoutBox()->RecalcScrollableOverflow());
            }
        }
...
void LayoutBox::UpdateHasSubpixelVisualEffectOutsets(const PhysicalBoxStrut& outsets)
{
    if (!VisualOverflowIsSet()) {
        return;
    }
    overflow_->visual_overflow->SetHasSubpixelVisualEffectOutsets(
        !outsets.top.IsInteger() || !outsets.right.IsInteger() || !outsets.bottom.IsInteger() || !outsets.left.IsInteger());
}

void LayoutBox::SetVisualOverflow(const PhysicalRect& self, const PhysicalRect& contents)
{
    ClearVisualOverflow();
    ...
    const PhysicalRect overflow_rect = overflow_->visual_overflow->SelfVisualOverflowRect();
    const PhysicalSize box_size = Size();
    const PhysicalBoxStrut outsets(-overflow_rect.Y(), overflow_rect.Right() - box_size.width, overflow_rect.Bottom() - box_size.height, -overflow_rect.X());
    ...
    const ComputedStyle& style = StyleRef();
    if (style.HasOutline()) {
        const LayoutUnit outline_extent(OutlinePainter::OutlineOutsetExtent(style, OutlineInfo::GetFromStyle(style)));
        SetOutlineMayBeAffectedByDescendants(
            outsets.top != outline_extent || outsets.right != outline_extent || outsets.bottom != outline_extent || outsets.left != outline_extent);
    }
}
...
void LayoutBox::CopyVisualOverflowFromFragments()
{
    ...
    const PhysicalRect previous_visual_overflow = VisualOverflowRectAllowingUnset();
...
void LayoutBox::CopyVisualOverflowFromFragmentsWithoutInvalidations()
{
    ...
    if (PhysicalFragmentCount() == 1) {
        ...
        SetVisualOverflow(fragment.SelfInkOverflowRect(), fragment.ContentsInkOverflowRect());
        return;
    }
...
bool LayoutBox::IsMonolithic() const
{
    ...
    if (ShouldBeConsideredAsReplaced() || HasUnsplittableScrollingOverflow() || (Parent() && IsWritingModeRoot())
        || (IsFixedPositioned() && GetDocument().Printing() && IsA<LayoutView>(Container())) || ShouldApplySizeContainment() || IsFrameSet()
...
{
    if (IsAtomicInlineLevel()) {
        return FirstLineStyle()->IsHorizontalWritingMode() ? MarginHeight() + Size().height : MarginWidth() + Size().width;
    }
    return LayoutUnit();
}
...
PhysicalBoxStrut LayoutBox::BorderOutsetsForClipping() const
{
    ...
    return overflow_clip_margin.Inflate(StyleRef().OverflowClipMargin()->GetMargin());
}
...
PhysicalRect LayoutBox::VisualOverflowRect() const
{
    ...
    const PhysicalRect& self_visual_overflow_rect = overflow_->visual_overflow->SelfVisualOverflowRect();
    if (HasMask()) {
    ...
    if (ShouldApplyOverflowClipMargin()) {
        ...
        const PhysicalRect& contents_visual_overflow_rect = overflow_->visual_overflow->ContentsVisualOverflowRect();
    ...
    PhysicalRect result = overflow_->visual_overflow->ContentsVisualOverflowRect();
    result.Unite(self_visual_overflow_rect);
...
PhysicalSize LayoutBox::ComputeSize() const
{
    ...
    for (const auto& result : results) {
        ...
        LogicalSize fragment_logical_size = converter.ToLogical(physical_fragment.Size());
        if (physical_fragment.IsFirstForNode()) {
            ...
            size.block_size = fragment_logical_size.block_size + previous_break_token->ConsumedBlockSizeForLegacy();
        }
        ...
        // is a repeated one.
        if (!previous_break_token || previous_break_token->IsRepeated() || previous_break_token->IsAtBlockEnd()) {
            break;
        }
    }
...
void LayoutBox::AddCustomLayoutChildIfNeeded()
{
    ...
    const CSSLayoutDefinition* definition = worklet->Proxy()->FindDefinition(name);
    ...
    EnsureRareData().layout_child_ = MakeGarbageCollected<CustomLayoutChild>(*definition, BlockNode(this));
}
...
OverflowClipAxes LayoutBox::ComputeOverflowClipAxes() const
{
    ...
    if (IsScrollContainer())
        return kOverflowClipBothAxis;
    return (StyleRef().OverflowX() == EOverflow::kVisible ? kNoOverflowClip : kOverflowClipX)
        | (StyleRef().OverflowY() == EOverflow::kVisible ? kNoOverflowClip : kOverflowClipY);
}
...
void LayoutBox::MutableForPainting::SavePreviousOverflowData()
{
    ...
    previous_overflow->previous_scrollable_overflow_rect = GetLayoutBox().ScrollableOverflowRect();
    previous_overflow->previous_visual_overflow_rect = GetLayoutBox().VisualOverflowRect();
    previous_overflow->previous_self_visual_overflow_rect = GetLayoutBox().SelfVisualOverflowRect();
}

void LayoutBox::MutableForPainting::SetPreviousGeometryForLayoutShiftTracking(
    const PhysicalOffset& paint_offset, const PhysicalSize& size, const PhysicalRect& visual_overflow_rect)
{
...
{
    GetLayoutBox().SetBackgroundPaintLocation(GetLayoutBox().ComputeBackgroundPaintLocation());
}
...
RasterEffectOutset LayoutBox::VisualRectOutsetForRasterEffects() const
{
    ...
    return VisualOverflowIsSet() && overflow_->visual_overflow->HasSubpixelVisualEffectOutsets() ? RasterEffectOutset::kWholePixel : RasterEffectOutset::kNone;
}

TextDirection LayoutBox::ResolvedDirection() const
{
    NOT_DESTROYED();
    if (IsInline() && IsAtomicInlineLevel() && IsInLayoutNGInlineFormattingContext()) {
        InlineCursor cursor;
        cursor.MoveTo(*this);
        if (cursor) {
...
bool LayoutBox::BackgroundClipBorderBoxIsEquivalentToPaddingBox() const
{
    ...
    if (StyleRef().BorderTopWidth() && (!ResolveColor(GetCSSPropertyBorderTopColor()).IsOpaque() || StyleRef().BorderTopStyle() != EBorderStyle::kSolid)) {
        return false;
    }
    if (StyleRef().BorderRightWidth()
        && (!ResolveColor(GetCSSPropertyBorderRightColor()).IsOpaque() || StyleRef().BorderRightStyle() != EBorderStyle::kSolid)) {
        return false;
    }
    if (StyleRef().BorderBottomWidth()
        && (!ResolveColor(GetCSSPropertyBorderBottomColor()).IsOpaque() || StyleRef().BorderBottomStyle() != EBorderStyle::kSolid)) {
        return false;
    }
    if (StyleRef().BorderLeftWidth() && (!ResolveColor(GetCSSPropertyBorderLeftColor()).IsOpaque() || StyleRef().BorderLeftStyle() != EBorderStyle::kSolid)) {
        return false;
    }
...
BackgroundPaintLocation LayoutBox::ComputeBackgroundPaintLocation() const
{
    ...
    if (IsA<LayoutView>(this) && GetDocument().GetSettings()->GetLCDTextPreference() == LCDTextPreference::kStronglyPreferred) {
    ...
    for (; layer; layer = layer->Next()) {
        ...
        if (!layer->GetImage() && !layer->Next() && !background_color.IsFullyTransparent() && StyleRef().IsScrollbarGutterAuto()) {
            ...
            } else if (clip == EFillBox::kContent && StyleRef().PaddingTop().IsZero() && StyleRef().PaddingLeft().IsZero() && StyleRef().PaddingRight().IsZero()
    ...
    if (!StyleRef().Preserves3D() && Parent() && Parent()->StyleRef().Preserves3D()) {
        return kBackgroundPaintInBorderBoxSpace;
    }
...
bool LayoutBox::ComputeCanCompositeBackgroundAttachmentFixed() const
{
    ...
    if (GetDocument().GetSettings()->GetLCDTextPreference() == LCDTextPreference::kStronglyPreferred) {
        return false;
    }
    // The fixed attachment background must be the only background layer.
    if (StyleRef().BackgroundLayers().Next() || StyleRef().BackgroundLayers().Clip() == EFillBox::kText) {
        return false;
    }
...
bool LayoutBox::IsFixedToView(const LayoutObject* container_for_fixed_position) const
{
...
PhysicalRect LayoutBox::ComputeStickyConstrainingRect() const
{
    ...
    constraining_rect.Move(PhysicalOffset(-BorderLeft() + PaddingLeft(), -BorderTop() + PaddingTop()));
    constraining_rect.ContractEdges(LayoutUnit(), PaddingLeft() + PaddingRight(), PaddingTop() + PaddingBottom(), LayoutUnit());
...
bool LayoutBox::AnchorPositionScrollAdjustmentAfectedByViewportScrolling() const
{
    if (auto* data = GetAnchorPositionScrollData()) {
        return data->NeedsScrollAdjustment() && data->IsAffectedByViewportScrolling();
    }
    return false;
}
...
template <typename Function> void ForEachAnchorQueryOnContainer(const LayoutBox& box, Function func)
{
    const LayoutObject* container = box.Container();
    if (container->IsLayoutBlock()) {
        for (const PhysicalBoxFragment& fragment : To<LayoutBlock>(container)->PhysicalFragments()) {
...
#if EXPENSIVE_DCHECKS_ARE_ON()
template <typename Function> void AssertSameDataOnLayoutResults(const LayoutBox::LayoutResultList& layout_results, Function func)
{
...
const LayoutObject* LayoutBox::FindTargetAnchor(const ScopedCSSName& anchor_name) const
{
    if (!IsOutOfFlowPositioned()) {
        return nullptr;
    }
    ...
    auto search_for_anchor = [&](const PhysicalAnchorQuery& anchor_query) {
        if (const LayoutObject* current = anchor_query.AnchorLayoutObject(*this, &anchor_name)) {
            if (!anchor || (anchor != current && anchor->IsBeforeInPreOrder(*current))) {
                anchor = current;
            }
        }
    };
...
const LayoutObject* LayoutBox::AcceptableImplicitAnchor() const
{
    ...
    Element* anchor_element = element ? element->ImplicitAnchorElement() : nullptr;
    LayoutObject* anchor_layout_object = anchor_element ? anchor_element->GetLayoutObject() : nullptr;
...
const HeapVector<NonOverflowingScrollRange>* LayoutBox::NonOverflowingScrollRanges() const
{
...
#if EXPENSIVE_DCHECKS_ARE_ON()
    for (wtf_size_t i = 1; i < layout_results.size(); ++i) {
        DCHECK(base::ValuesEquivalent(layout_results[i]->NonOverflowingScrollRanges(), layout_results[i - 1]->NonOverflowingScrollRanges()));
    }
#endif
...
#if EXPENSIVE_DCHECKS_ARE_ON()
    AssertSameDataOnLayoutResults(layout_results, [](const auto& result) { return result->OutOfFlowInsetsForGetComputedStyle(); });
#endif
...
const HeapHashSet<Member<Element>>* LayoutBox::DisplayLocksAffectedByAnchors() const
{
    const auto& layout_results = GetLayoutResults();
    if (layout_results.empty()) {
        return nullptr;
...
void LayoutBox::NotifyContainingDisplayLocksForAnchorPositioning(
    const HeapHashSet<Member<Element>>* past_display_locks_affected_by_anchors, const HeapHashSet<Member<Element>>* display_locks_affected_by_anchors) const
{
    auto notify_display_locks = [](const HeapHashSet<Member<Element>>* display_locks) {
        if (!display_locks) {
            return;
        }
        for (auto& display_lock_element : *display_locks) {
            display_lock_element->GetDisplayLockContext()->SetAnchorPositioningRenderStateMayHaveChanged();
        }
    };
...
bool LayoutBox::NeedsAnchorPositionScrollAdjustmentInX() const
{
    ...
    // fragmented, position fallback results are duplicated on all fragments.
#if EXPENSIVE_DCHECKS_ARE_ON()
    AssertSameDataOnLayoutResults(layout_results, [](const auto& result) { return result->NeedsAnchorPositionScrollAdjustmentInX(); });
#endif
...
bool LayoutBox::NeedsAnchorPositionScrollAdjustmentInY() const
{
    ...
    // fragmented, position fallback results are duplicated on all fragments.
#if EXPENSIVE_DCHECKS_ARE_ON()
    AssertSameDataOnLayoutResults(layout_results, [](const auto& result) { return result->NeedsAnchorPositionScrollAdjustmentInY(); });
#endif
...
{
    return WritingModeConverter({ Style()->GetWritingMode(), TextDirection::kLtr }, Size());
}
...
const HeapVector<Member<Element>>& LayoutBox::ReadingFlowElements() const
{
    ...
    DEFINE_STATIC_LOCAL(Persistent<HeapVector<Member<Element>>>, empty_vector, (MakeGarbageCollected<HeapVector<Member<Element>>>()));
    return *empty_vector;
}
