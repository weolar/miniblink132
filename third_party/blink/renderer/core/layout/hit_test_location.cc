...
bool PointInRectangleStroke(const gfx::PointF& point, const gfx::RectF& rect, float stroke_width)
{
    ...
    if (!(abs_delta_x <= half_width + half_stroke_width && abs_delta_y <= half_height + half_stroke_width)) {
        return false;
    }

    return (half_width - half_stroke_width <= abs_delta_x) || (half_height - half_stroke_width <= abs_delta_y);
}

bool QuadIntersectsRectangleStroke(const gfx::QuadF& quad, const gfx::RectF& rect, float stroke_width)
{
    const float half_stroke_width = stroke_width / 2;
    ...
    if (inner_edge.InclusiveContains(quad.p1()) && inner_edge.InclusiveContains(quad.p2()) && inner_edge.InclusiveContains(quad.p3())
...
bool PointInEllipse(const gfx::PointF& point, const gfx::PointF& center, const gfx::SizeF& radii)
{
    const gfx::PointF point_to_center = gfx::PointF(center.x() - point.x(), center.y() - point.y());
...
bool PointInCircleStroke(const gfx::PointF& point, const gfx::PointF& center, float radius, float stroke_width)
{
    const float half_stroke_width = stroke_width / 2;
    return std::abs(DistanceBetween(point, center) - radius) <= half_stroke_width;
}

bool QuadIntersectsCircleStroke(const gfx::QuadF& quad, const gfx::PointF& center, float radius, float stroke_width)
{
    const float half_stroke_width = stroke_width / 2;
    // If the outer edge does not intersect the quad, then neither will the inner.
    ...
    if (DistanceBetween(quad.p1(), center) < inner_edge_radius && DistanceBetween(quad.p2(), center) < inner_edge_radius
        && DistanceBetween(quad.p3(), center) < inner_edge_radius && DistanceBetween(quad.p4(), center) < inner_edge_radius) {
        return false;
    }
    return true;
}
...
HitTestLocation::HitTestLocation(const gfx::PointF& point, const PhysicalRect& bounding_box)
    : point_(PhysicalOffset::FromPointFFloor(point))
...
HitTestLocation::HitTestLocation(const gfx::PointF& point, const gfx::QuadF& quad)
    : transformed_point_(point)
    , transformed_rect_(quad)
    , is_rect_based_(true)
{
    point_ = PhysicalOffset::FromPointFFloor(point);
...
HitTestLocation::HitTestLocation(const HitTestLocation& other, const PhysicalOffset& offset)
    : point_(other.point_)
...
HitTestLocation::HitTestLocation(const HitTestLocation& other, wtf_size_t fragment_index)
    : point_(other.point_)
    , bounding_box_(other.bounding_box_)
    , transformed_point_(other.transformed_point_)
    , transformed_rect_(other.transformed_rect_)
...
HitTestLocation& HitTestLocation::operator=(const HitTestLocation& other) = default;
...
bool HitTestLocation::Intersects(const Path& path, WindRule winding_rule) const
{
    if (is_rect_based_ && CanUseRectHitTestForPath(path)) {
...
bool HitTestLocation::IntersectsStroke(const gfx::RectF& rect, float stroke_width) const
{
    if (is_rect_based_) {
...
bool HitTestLocation::IntersectsEllipse(const gfx::PointF& center, const gfx::SizeF& radii) const
{
    if (is_rect_based_) {
...
bool HitTestLocation::IntersectsCircleStroke(const gfx::PointF& center, float radius, float stroke_width) const
{
    if (is_rect_based_) {
        return QuadIntersectsCircleStroke(transformed_rect_, center, radius, stroke_width);
    }
