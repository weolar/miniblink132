...
MultiColumnFragmentainerGroup::MultiColumnFragmentainerGroup(const LayoutMultiColumnSet& column_set)
...
LayoutUnit MultiColumnFragmentainerGroup::LogicalHeightInFlowThreadAt(unsigned column_index) const
{
...
PhysicalOffset MultiColumnFragmentainerGroup::FlowThreadTranslationAtOffset(LayoutUnit offset_in_flow_thread, LayoutBox::PageBoundaryRule rule) const
{
    LayoutMultiColumnFlowThread* flow_thread = column_set_->MultiColumnFlowThread();
    ...
    unsigned column_index = offset_in_flow_thread >= LogicalBottomInFlowThread() ? ActualColumnCount() - 1 : ColumnIndexAtOffset(offset_in_flow_thread, rule);
    ...
    PhysicalRect physical_column_rect = column_set_->CreateWritingModeConverter().ToPhysical(column_rect);
    physical_column_rect.offset += column_set_->PhysicalLocation();

    return physical_column_rect.offset - portion_rect.offset;
}

LogicalOffset MultiColumnFragmentainerGroup::VisualPointToFlowThreadPoint(const LogicalOffset& visual_point) const
{
    ...
    return LogicalOffset(local_point.inline_offset, local_point.block_offset + LogicalTopInFlowThreadAt(column_index));
}

PhysicalRect MultiColumnFragmentainerGroup::FragmentsBoundingBox(const PhysicalRect& bounding_box_in_flow_thread) const
{
    // Find the start and end column intersected by the bounding box.
    const LogicalRect logical_bounding_box = column_set_->FlowThread()->CreateWritingModeConverter().ToLogical(bounding_box_in_flow_thread);
    LayoutUnit bounding_box_logical_top = logical_bounding_box.offset.block_offset;
    LayoutUnit bounding_box_logical_bottom = logical_bounding_box.BlockEndOffset();
    if (bounding_box_logical_bottom <= LogicalTopInFlowThread() || bounding_box_logical_top >= LogicalBottomInFlowThread()) {
        // The bounding box doesn't intersect this fragmentainer group.
        return PhysicalRect();
    }
    unsigned start_column;
    unsigned end_column;
    ColumnIntervalForBlockRangeInFlowThread(bounding_box_logical_top, bounding_box_logical_bottom, start_column, end_column);
    ...
    start_column_rect.offset += PhysicalOffset(FlowThreadTranslationAtOffset(LogicalTopInFlowThreadAt(start_column), LayoutBox::kAssociateWithLatterPage));
    ...
    end_column_rect.offset += PhysicalOffset(FlowThreadTranslationAtOffset(LogicalTopInFlowThreadAt(end_column), LayoutBox::kAssociateWithLatterPage));
...
void MultiColumnFragmentainerGroup::SetColumnBlockSizeFromNG(LayoutUnit block_size)
{
    ...
    // is the case.
    DCHECK(!is_logical_height_known_ || logical_height_ == block_size || block_size <= LayoutUnit(1));
...
void MultiColumnFragmentainerGroup::ExtendColumnBlockSizeFromNG(LayoutUnit block_size)
{
    DCHECK(is_logical_height_known_);
    logical_height_ += block_size;
}

LogicalRect MultiColumnFragmentainerGroup::ColumnRectAt(unsigned column_index) const
{
    ...
    if (column_set_->StyleRef().IsLeftToRightDirection()) {
        ...
        column_logical_left += column_set_->ContentLogicalWidth() - column_logical_width - column_index * (column_logical_width + column_gap);
    }

    return LogicalRect(column_logical_left, column_logical_top, column_logical_width, column_logical_height);
}

LogicalRect MultiColumnFragmentainerGroup::LogicalFlowThreadPortionRectAt(unsigned column_index) const
{
    LayoutUnit logical_top = LogicalTopInFlowThreadAt(column_index);
    LayoutUnit portion_logical_height = LogicalHeightInFlowThreadAt(column_index);
    return LogicalRect(LayoutUnit(), logical_top, column_set_->PageLogicalWidth(), portion_logical_height);
}

PhysicalRect MultiColumnFragmentainerGroup::FlowThreadPortionRectAt(unsigned column_index) const
{
    return column_set_->FlowThread()->CreateWritingModeConverter().ToPhysical(LogicalFlowThreadPortionRectAt(column_index));
}

PhysicalRect MultiColumnFragmentainerGroup::FlowThreadPortionOverflowRectAt(unsigned column_index) const
{
    ...
    bool is_first_column_in_multicol_container
        = is_first_column_in_row && this == &column_set_->FirstFragmentainerGroup() && !column_set_->PreviousSiblingMultiColumnSet();
    bool is_last_column_in_multicol_container
        = is_last_column_in_row && this == &column_set_->LastFragmentainerGroup() && !column_set_->NextSiblingMultiColumnSet();
    ...
    LogicalRect overflow_rect(-kMulticolMaxClipPixels, -kMulticolMaxClipPixels, 2 * kMulticolMaxClipPixels, 2 * kMulticolMaxClipPixels);
    ...
    return column_set_->FlowThread()->CreateWritingModeConverter().ToPhysical(overflow_rect);
}

unsigned MultiColumnFragmentainerGroup::ColumnIndexAtOffset(LayoutUnit offset_in_flow_thread, LayoutBox::PageBoundaryRule page_boundary_rule) const
{
    ...
    if (!IsLogicalHeightKnown())
        return 0;
    LayoutUnit column_height = ColumnLogicalHeight();
    unsigned column_index = ((offset_in_flow_thread - logical_top_in_flow_thread_) / column_height).Floor();
    if (page_boundary_rule == LayoutBox::kAssociateWithFormerPage && column_index > 0 && LogicalTopInFlowThreadAt(column_index) == offset_in_flow_thread) {
...
unsigned MultiColumnFragmentainerGroup::ConstrainedColumnIndexAtOffset(LayoutUnit offset_in_flow_thread, LayoutBox::PageBoundaryRule page_boundary_rule) const
{
    unsigned index = ColumnIndexAtOffset(offset_in_flow_thread, page_boundary_rule);
    return std::min(index, ActualColumnCount() - 1);
}

unsigned MultiColumnFragmentainerGroup::ColumnIndexAtVisualPoint(const LogicalOffset& visual_point) const
{
    LayoutUnit column_length = column_set_->PageLogicalWidth();
    LayoutUnit offset_in_column_progression_direction = visual_point.inline_offset;
    if (!column_set_->StyleRef().IsLeftToRightDirection()) {
        offset_in_column_progression_direction = column_set_->LogicalWidth() - offset_in_column_progression_direction;
    }
    LayoutUnit column_gap = column_set_->ColumnGap();
    if (column_length + column_gap <= 0)
        return 0;
    // Column boundaries are in the middle of the column gap.
    int index = ((offset_in_column_progression_direction + column_gap / 2) / (column_length + column_gap)).ToInt();
    if (index < 0)
        return 0;
...
void MultiColumnFragmentainerGroup::ColumnIntervalForBlockRangeInFlowThread(
    LayoutUnit logical_top_in_flow_thread, LayoutUnit logical_bottom_in_flow_thread, unsigned& first_column, unsigned& last_column) const
{
    logical_top_in_flow_thread = std::max(logical_top_in_flow_thread, LogicalTopInFlowThread());
    logical_bottom_in_flow_thread = std::min(logical_bottom_in_flow_thread, LogicalBottomInFlowThread());
    first_column = ConstrainedColumnIndexAtOffset(logical_top_in_flow_thread, LayoutBox::kAssociateWithLatterPage);
    if (logical_bottom_in_flow_thread <= logical_top_in_flow_thread) {
        ...
        last_column = ConstrainedColumnIndexAtOffset(logical_bottom_in_flow_thread, LayoutBox::kAssociateWithFormerPage);
    }
}
...
MultiColumnFragmentainerGroupList::MultiColumnFragmentainerGroupList(LayoutMultiColumnSet& column_set)
    : column_set_(&column_set)
{
...
// operator=.
MultiColumnFragmentainerGroupList::~MultiColumnFragmentainerGroupList() = default;
