...
void SetOrthogonalFallbackInlineSize(const ComputedStyle& parent_style, const LayoutInputNode child, ConstraintSpaceBuilder* builder)
{
    DCHECK(!IsParallelWritingMode(parent_style.GetWritingMode(), child.Style().GetWritingMode()));

    PhysicalSize orthogonal_children_containing_block_size = child.InitialContainingBlockSize();

    LayoutUnit fallback_size
        = parent_style.IsHorizontalWritingMode() ? orthogonal_children_containing_block_size.height : orthogonal_children_containing_block_size.width;
    ...
    if (parent_style.LogicalMaxHeight().IsFixed()) {
        size = std::min(size, LayoutUnit(parent_style.LogicalMaxHeight().GetFloatValue()));
    }
    if (parent_style.LogicalMinHeight().IsFixed()) {
        size = std::max(size, LayoutUnit(parent_style.LogicalMinHeight().GetFloatValue()));
    }
    ...
    if (parent_style.BoxSizing() == EBoxSizing::kBorderBox) {
        ...
        LayoutUnit border_padding(parent_style.BorderBlockStartWidth() + parent_style.BorderBlockEndWidth() + parent_style.PaddingBlockStart().GetFloatValue()
...
bool ShouldBlockContainerChildStretchAutoInlineSize(const BlockNode& child)
{
    ...
    if (const auto* node = child.GetDOMNode()) {
        if (IsA<HTMLButtonElement>(node) || IsA<HTMLInputElement>(node) || IsA<HTMLSelectElement>(node) || IsA<HTMLTextAreaElement>(node)) {
            return false;
        }
    }
    return true;
