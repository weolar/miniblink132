...
CORE_EXPORT bool AdjustToClearance(LayoutUnit clearance_offset, BfcOffset* offset);
...
inline void SetOrthogonalFallbackInlineSizeIfNeeded(const ComputedStyle& parent_style, const LayoutInputNode child, ConstraintSpaceBuilder* builder)
{
    if (IsParallelWritingMode(parent_style.GetWritingMode(), child.Style().GetWritingMode())) [[likely]] {
        return;
    }
