    ...
    for (wtf_size_t idx = 0; idx < fragment_count; idx++, sequence_number++) {
        ...
        if (break_token) {
            ...
            if (break_token->SequenceNumber() != sequence_number) {
                break_token
                    = BlockBreakToken::CreateForBreakInRepeatedFragment(node, sequence_number, break_token->ConsumedBlockSize(), break_token->IsAtBlockEnd());
            }
        ...
        for (const auto& child_link : fragment.Children()) {
            if (!child_link->IsFragmentainerBox())
                continue;
            const auto& fragmentainer = *To<PhysicalBoxFragment>(child_link.fragment.Get());
            const BlockBreakToken* fragmentainer_break_token = fragmentainer.GetBreakToken();
            ...
            if (fragmentainer_break_token) {
                if (fragmentainer_break_token->SequenceNumber() != fragmentainer_sequence_number) {
                    fragmentainer_break_token
                        = BlockBreakToken::CreateForBreakInRepeatedFragment(node, fragmentainer_sequence_number, fragmentainer_break_token->ConsumedBlockSize(),
                            /* is_at_block_end */ false);
                    fragmentainer.GetMutableForCloning().SetBreakToken(fragmentainer_break_token);
                }
            } else {
                fragmentainer_break_token = BlockBreakToken::CreateRepeated(node, fragmentainer_sequence_number);
                fragmentainer.GetMutableForCloning().SetBreakToken(fragmentainer_break_token);
...
void FragmentRepeater::CloneChildFragments(const PhysicalBoxFragment& cloned_fragment)
{
    if (cloned_fragment.HasItems()) {
        ...
        for (auto& cloned_item : cloned_fragment.Items()->Items()) {
            ...
            if (!child_box_fragment)
                continue;
            const auto* child_layout_box = DynamicTo<LayoutBox>(child_box_fragment->GetLayoutObject());
            if (!child_layout_box) {
                // We don't need to clone non-atomic inlines.
                DCHECK(child_box_fragment->GetLayoutObject()->IsLayoutInline());
                continue;
            }
            const LayoutResult* child_result = GetClonableLayoutResult(*child_layout_box, *child_box_fragment);
            child_result = Repeat(*child_result);
            child_box_fragment = &To<PhysicalBoxFragment>(child_result->GetPhysicalFragment());
            cloned_item.GetMutableForCloning().ReplaceBoxFragment(*child_box_fragment);
        }
    }

    for (PhysicalFragmentLink& child : cloned_fragment.GetMutableForCloning().Children()) {
        if (const auto* child_box = DynamicTo<PhysicalBoxFragment>(child.fragment.Get())) {
            if (child_box->IsCSSBox()) {
                const auto* child_layout_box = To<LayoutBox>(child_box->GetLayoutObject());
                const LayoutResult* child_result = GetClonableLayoutResult(*child_layout_box, *child_box);
                child_result = Repeat(*child_result);
            ...
            child.fragment = PhysicalLineBoxFragment::Clone(To<PhysicalLineBoxFragment>(*child.fragment.Get()));
        }
    }
}
...
const LayoutResult* FragmentRepeater::Repeat(const LayoutResult& other)
{
    ...
    const auto& cloned_fragment = To<PhysicalBoxFragment>(cloned_result->GetPhysicalFragment());
    ...
    if (is_last_fragment_ && (!cloned_fragment.GetBreakToken() || cloned_fragment.GetBreakToken()->IsRepeated())) {
...
const LayoutResult* FragmentRepeater::GetClonableLayoutResult(const LayoutBox& layout_box, const PhysicalBoxFragment& fragment) const
{
    ...
    for (const LayoutResult* result : layout_box.GetLayoutResults()) {
        const BlockBreakToken* break_token = To<PhysicalBoxFragment>(result->GetPhysicalFragment()).GetBreakToken();
        if (!break_token || break_token->IsRepeated())
            return result;
    }
    NOTREACHED();
