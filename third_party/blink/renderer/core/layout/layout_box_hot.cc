...
bool LayoutBox::HasHitTestableOverflow() const
{
    ...
    return ShouldApplyOverflowClipMargin() && StyleRef().OverflowClipMargin()->GetMargin() > 0;
}

// Hit Testing
bool LayoutBox::MayIntersect(const HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset) const
{
    NOT_DESTROYED();
...
const LayoutResult* LayoutBox::CachedLayoutResult(const ConstraintSpace& new_space, const BlockBreakToken* break_token, const EarlyBreak* early_break,
    const ColumnSpannerPath* column_spanner_path, std::optional<FragmentGeometry>* initial_fragment_geometry, LayoutCacheStatus* out_cache_status)
{
    NOT_DESTROYED();
    *out_cache_status = LayoutCacheStatus::kNeedsLayout;
    ...
    const bool use_layout_cache_slot = new_space.CacheSlot() == LayoutResultCacheSlot::kLayout && !layout_results_.empty();
    const LayoutResult* cached_layout_result
        = use_layout_cache_slot ? GetCachedLayoutResult(break_token) : GetCachedMeasureResult(new_space, initial_fragment_geometry);
    ...
    const PhysicalBoxFragment& physical_fragment = To<PhysicalBoxFragment>(cached_layout_result->GetPhysicalFragment());

    // No fun allowed for repeated content.
    if ((physical_fragment.GetBreakToken() && physical_fragment.GetBreakToken()->IsRepeated()) || (break_token && break_token->IsRepeated())) {
        return nullptr;
    }
    ...
    bool child_needs_layout = !is_blocked_by_display_lock && ChildNeedsFullLayout();
    ...
    if (NeedsSimplifiedLayoutOnly()) {
        ...
        if (!cached_layout_result->GetExclusionSpace().IsEmpty() || new_space.HasFloats()) {
            return nullptr;
        }
    ...
    if (use_layout_cache_slot) {
        size_cache_status = CalculateSizeBasedLayoutCacheStatus(node, break_token, *cached_layout_result, new_space, initial_fragment_geometry);
    }
    ...
    if (cached_layout_result->HasOrthogonalFallbackSizeDescendant() && View()->AffectedByResizedInitialContainingBlock(*cached_layout_result)) {
    ...
    if (!physical_fragment.ChildrenValid()
        && (size_cache_status == LayoutCacheStatus::kNeedsSimplifiedLayout || cache_status == LayoutCacheStatus::kNeedsSimplifiedLayout)) {
        return nullptr;
    }

    // Update our temporary cache status, if the size cache check indicated we
    // might need simplified layout.
    if (size_cache_status == LayoutCacheStatus::kNeedsSimplifiedLayout && cache_status == LayoutCacheStatus::kHit) {
        cache_status = LayoutCacheStatus::kNeedsSimplifiedLayout;
    }
    ...
    std::optional<LayoutUnit> bfc_block_offset = cached_layout_result->BfcBlockOffset();
    ...
    bool is_fragmented = IsBreakInside(break_token) || physical_fragment.GetBreakToken() || PhysicalFragmentCount() > 1;

    {
        ...
        are_bfc_offsets_equal = new_space.GetBfcOffset() == old_space.GetBfcOffset() && new_space.ExpectedBfcBlockOffset() == old_space.ExpectedBfcBlockOffset()
            && new_space.ForcedBfcBlockOffset() == old_space.ForcedBfcBlockOffset();

        is_margin_strut_equal = new_space.GetMarginStrut() == old_space.GetMarginStrut();
        is_exclusion_space_equal = new_space.GetExclusionSpace() == old_space.GetExclusionSpace();
        bool is_clearance_offset_equal = new_space.ClearanceOffset() == old_space.ClearanceOffset();
        ...
        // layout result.
        if (!is_new_formatting_context && (!are_bfc_offsets_equal || !is_exclusion_space_equal || !is_margin_strut_equal || !is_clearance_offset_equal)) {
            ...
            if (cache_status == LayoutCacheStatus::kNeedsSimplifiedLayout || cache_status == LayoutCacheStatus::kCanReuseLines) {
                return nullptr;
            }

            DCHECK_EQ(cache_status, LayoutCacheStatus::kHit);

            if (!MaySkipLayoutWithinBlockFormattingContext(*cached_layout_result, new_space, &bfc_block_offset, &block_offset_delta, &end_margin_strut))
                return nullptr;
        }
        ...
        if (new_space.HasBlockFragmentation()) [[unlikely]] {
            ...
            if (break_token && break_token->IsBreakBefore() && cached_layout_result->GetBreakAppeal() < kBreakAppealPerfect) {
                return nullptr;
            }
            ...
            if (new_space.IsInitialColumnBalancingPass()) {
                ...
                // additional checks.)
            } else if (new_space.FragmentainerBlockSize() != old_space.FragmentainerBlockSize()
                ...
                if (node.IsOutOfFlowPositioned()) {
                    ...
                    std::optional<LayoutUnit> space_shortage = cached_layout_result->MinimalSpaceShortage();
                ...
                auto DoFloatsCrossFragmentationLine = [&]() -> bool {
                    const auto& result_exclusion_space = cached_layout_result->GetExclusionSpace();
                    if (result_exclusion_space != old_space.GetExclusionSpace()) {
                        LayoutUnit block_end_offset = FragmentainerOffsetAtBfc(new_space) + result_exclusion_space.ClearanceOffset(EClear::kBoth);
                ...
                if (!bfc_block_offset && cached_layout_result->IsSelfCollapsing()) {
                    ...
                    if (physical_fragment.IsInlineFormattingContext() && !is_new_formatting_context) {
                        if (DoFloatsCrossFragmentationLine())
                            return nullptr;
                    }
                    ...
                    LayoutUnit block_size_for_fragmentation = cached_layout_result->BlockSizeForFragmentation();

                    LayoutUnit block_end_offset = FragmentainerOffsetAtBfc(new_space) + bfc_block_offset.value_or(LayoutUnit()) + block_size_for_fragmentation;
                    if (block_end_offset > new_space.FragmentainerBlockSize())
                        return nullptr;
                }
    ...
    *out_cache_status = cache_status;
    ...
    if (use_layout_cache_slot && !is_blocked_by_display_lock && NeedsScrollableOverflowRecalc()) {
#if DCHECK_IS_ON()
        const LayoutResult* cloned_cached_layout_result = LayoutResult::CloneWithPostLayoutFragments(*cached_layout_result);
#endif
        ...
        cloned_cached_layout_result->CheckSameForSimplifiedLayout(*cached_layout_result, /* check_same_block_size */ true,
    ...
    if (IsTableRow() && IsLayoutNGObject()) {
        const_cast<ConstraintSpace&>(cached_layout_result->GetConstraintSpaceForCaching())
    ...
    bool needs_cached_result_update = node.IsOutOfFlowPositioned()
        && new_space.PercentageResolutionSize() != cached_layout_result->GetConstraintSpaceForCaching().PercentageResolutionSize();

    // We can safely reuse this result if our BFC and "input" exclusion spaces
    // were equal.
    if (are_bfc_offsets_equal && is_exclusion_space_equal && is_margin_strut_equal && !needs_cached_result_update) {
        ...
        cached_layout_result->GetExclusionSpace().MoveAndUpdateDerivedGeometry(new_space.GetExclusionSpace());
        return cached_layout_result;
    }

    const auto* new_result
        = MakeGarbageCollected<LayoutResult>(*cached_layout_result, new_space, end_margin_strut, bfc_line_offset, bfc_block_offset, block_offset_delta);

    if (needs_cached_result_update && !DisableLayoutSideEffectsScope::IsDisabled()) {
