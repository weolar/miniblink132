...
void SetUpSpaceBuilderForPageBox(LogicalSize available_size, ConstraintSpaceBuilder*);
...
LogicalSize DesiredPageContainingBlockSize(const Document&, const ComputedStyle&);
...
void ResolvePageBoxGeometry(const BlockNode& page_box, LogicalSize page_containing_block_size, FragmentGeometry*, BoxStrut* margins = nullptr);
...
LogicalRect TargetPageBorderBoxLogicalRect(
    const Document& document, const ComputedStyle& page_style, const LogicalSize& source_margin_box_size, const BoxStrut& margins);
...
const PhysicalBoxFragment* GetPageContainer(const LayoutView&, wtf_size_t page_number);

// Get the page area (BoxType::kPageArea) for a given page.
const PhysicalBoxFragment* GetPageArea(const LayoutView&, wtf_size_t page_number);
...
float CalculateOverflowShrinkForPrinting(const LayoutView&, float maximum_shrink_factor);
...
WebPrintPageDescription GetPageDescriptionFromLayout(const Document&, wtf_size_t page_number);
