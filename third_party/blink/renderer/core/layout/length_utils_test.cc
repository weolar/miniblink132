...
static ConstraintSpace ConstructConstraintSpace(
    int inline_size, int block_size, bool fixed_inline = false, bool fixed_block = false, WritingMode writing_mode = WritingMode::kHorizontalTb)
{
    LogicalSize size = { LayoutUnit(inline_size), LayoutUnit(block_size) };

    ConstraintSpaceBuilder builder(writing_mode, { writing_mode, TextDirection::kLtr },
        /* is_new_fc */ false);
...
protected:
    ...
    LayoutUnit ResolveMainInlineLength(
        const Length& length, const std::optional<MinMaxSizes>& sizes = std::nullopt, ConstraintSpace constraint_space = ConstructConstraintSpace(200, 300))
    {
    ...
    LayoutUnit ResolveMainBlockLength(const Length& length, LayoutUnit content_size = LayoutUnit())
    {
        ...
        return ::blink::ResolveMainBlockLength(constraint_space, *initial_style_,
            /* border_padding */ BoxStrut(), length, /* auto_length */ nullptr, content_size);
    }
...
public:
    LayoutUnit ComputeInlineSizeForFragment(
        const BlockNode& node, ConstraintSpace constraint_space = ConstructConstraintSpace(200, 300), const MinMaxSizes& sizes = MinMaxSizes())
    {
        BoxStrut border_padding = ComputeBorders(constraint_space, node) + ComputePadding(constraint_space, node.Style());
        return ::blink::ComputeInlineSizeForFragment(constraint_space, node, border_padding, &sizes);
    }

    LayoutUnit ComputeBlockSizeForFragment(const BlockNode& node, ConstraintSpace constraint_space = ConstructConstraintSpace(200, 300),
        LayoutUnit content_size = LayoutUnit(), LayoutUnit inline_size = kIndefiniteSize)
    {
        BoxStrut border_padding = ComputeBorders(constraint_space, node) + ComputePadding(constraint_space, node.Style());
        return ::blink::ComputeBlockSizeForFragment(constraint_space, node, border_padding, content_size, inline_size);
    }
};
...
TEST_F(LengthUtilsTest, TestResolveInlineLength)
{
    ...
    EXPECT_EQ(LayoutUnit(30), ResolveMainInlineLength(Length::MinContent(), sizes));
    EXPECT_EQ(LayoutUnit(40), ResolveMainInlineLength(Length::MaxContent(), sizes));
    EXPECT_EQ(LayoutUnit(40), ResolveMainInlineLength(Length::FitContent(), sizes));
    sizes.max_size = LayoutUnit(800);
    EXPECT_EQ(LayoutUnit(200), ResolveMainInlineLength(Length::FitContent(), sizes));
...
TEST_F(LengthUtilsTest, TestIndefiniteResolveInlineLength)
{
    ...
    EXPECT_EQ(LayoutUnit(0), ResolveMinInlineLength(Length::Auto(), std::nullopt, space));
    EXPECT_EQ(LayoutUnit::Max(), ResolveMaxInlineLength(Length::Percent(30), std::nullopt, space));
    EXPECT_EQ(LayoutUnit::Max(), ResolveMaxInlineLength(Length::Stretch(), std::nullopt, space));
}
...
TEST_F(LengthUtilsTestWithNode, TestComputeContentContribution)
{
    ...
    const auto space = ConstraintSpaceBuilder(WritingMode::kHorizontalTb, { WritingMode::kHorizontalTb, TextDirection::kLtr },
        /* is_new_fc */ false)
                           .ToConstraintSpace();

    MinMaxSizes expected = sizes;
    BlockNode node(To<LayoutBox>(GetLayoutObjectByElementId("test1")));
    EXPECT_EQ(expected, ComputeMinAndMaxContentContributionForTest(WritingMode::kHorizontalTb, node, space, sizes));
    ...
    EXPECT_EQ(expected, ComputeMinAndMaxContentContributionForTest(WritingMode::kHorizontalTb, node, space, sizes_padding400));
    ...
    EXPECT_EQ(expected, ComputeMinAndMaxContentContributionForTest(WritingMode::kHorizontalTb, node, space, sizes_padding50));
...
TEST_F(LengthUtilsTestWithNode, TestComputeInlineSizeForFragment)
{
    ...
    ConstraintSpace constraint_space = ConstructConstraintSpace(120, 120, true, true);
    node = BlockNode(To<LayoutBox>(GetLayoutObjectByElementId("test6")));
    EXPECT_EQ(LayoutUnit(120), ComputeInlineSizeForFragment(node, constraint_space));
    ...
    node = BlockNode(To<LayoutBox>(GetLayoutObjectByElementId("test14")));
    EXPECT_EQ(LayoutUnit(430), ComputeInlineSizeForFragment(node, constraint_space, sizes_padding400));

    //  Due to padding and box-sizing, width computes to 400px and max-width to
    //  440px, so the result is 400.
    node = BlockNode(To<LayoutBox>(GetLayoutObjectByElementId("test15")));
    EXPECT_EQ(LayoutUnit(400), ComputeInlineSizeForFragment(node, constraint_space, sizes_padding400));

    node = BlockNode(To<LayoutBox>(GetLayoutObjectByElementId("test16")));
    EXPECT_EQ(LayoutUnit(40), ComputeInlineSizeForFragment(node, constraint_space, sizes));
}
...
TEST_F(LengthUtilsTestWithNode, TestComputeBlockSizeForFragment)
{
    ...
    ConstraintSpace constraint_space = ConstructConstraintSpace(200, 300);
    EXPECT_EQ(LayoutUnit(120), ComputeBlockSizeForFragment(node, constraint_space, LayoutUnit(120)));
    ...
    node = BlockNode(To<LayoutBox>(GetLayoutObjectByElementId("test6")));
    EXPECT_EQ(LayoutUnit(200), ComputeBlockSizeForFragment(node, constraint_space));
    ...
    node = BlockNode(To<LayoutBox>(GetLayoutObjectByElementId("test14")));
    EXPECT_EQ(LayoutUnit(50), ComputeBlockSizeForFragment(node, ConstructConstraintSpace(200, 300), LayoutUnit(), LayoutUnit(100)));

    // Default box-sizing
    // Should be (100 - 10) / 2 + 20 = 65.
    node = BlockNode(To<LayoutBox>(GetLayoutObjectByElementId("test15")));
    EXPECT_EQ(LayoutUnit(65), ComputeBlockSizeForFragment(node, ConstructConstraintSpace(200, 300), LayoutUnit(20), LayoutUnit(100)));

    // With box-sizing: border-box, should be 50.
    node = BlockNode(To<LayoutBox>(GetLayoutObjectByElementId("test16")));
    EXPECT_EQ(LayoutUnit(50), ComputeBlockSizeForFragment(node, ConstructConstraintSpace(200, 300), LayoutUnit(20), LayoutUnit(100)));
}
...
TEST_F(LengthUtilsTestWithNode, TestIndefinitePercentages)
{
    ...
    EXPECT_EQ(kIndefiniteSize, ComputeBlockSizeForFragment(node, ConstructConstraintSpace(200, -1), LayoutUnit(-1)));
    EXPECT_EQ(LayoutUnit(20), ComputeBlockSizeForFragment(node, ConstructConstraintSpace(200, -1), LayoutUnit(10)));
    EXPECT_EQ(LayoutUnit(120), ComputeBlockSizeForFragment(node, ConstructConstraintSpace(200, -1), LayoutUnit(120)));
}
...
TEST_F(LengthUtilsTest, TestPadding)
{
    ...
    ConstraintSpace constraint_space = ConstructConstraintSpace(200, 300, false, false, WritingMode::kVerticalRl);
...
TEST_F(LengthUtilsTest, TestAutoMargins)
{
    ...
    BoxStrut margins;
    ResolveInlineAutoMargins(*style, *style, kAvailableInlineSize, kInlineSize, &margins);
...
int GetUsedColumnWidth(int computed_column_count, int computed_column_width, int used_column_gap, int available_inline_size)
{
    LayoutUnit column_width(computed_column_width);
    if (!computed_column_width)
        column_width = LayoutUnit(kIndefiniteSize);
    return ResolveUsedColumnInlineSize(computed_column_count, column_width, LayoutUnit(used_column_gap), LayoutUnit(available_inline_size)).ToInt();
}
int GetUsedColumnCount(int computed_column_count, int computed_column_width, int used_column_gap, int available_inline_size)
{
    LayoutUnit column_width(computed_column_width);
    if (!computed_column_width)
        column_width = LayoutUnit(kIndefiniteSize);
    return ResolveUsedColumnCount(computed_column_count, column_width, LayoutUnit(used_column_gap), LayoutUnit(available_inline_size));
}
...
{
    return InlineSizeFromAspectRatio(BoxStrut(), aspect_ratio, EBoxSizing::kBorderBox, block_size);
}
TEST_F(LengthUtilsTest, AspectRatio)
{
    EXPECT_EQ(LayoutUnit(8000), ComputeInlineSize(LogicalSize(8000, 8000), LayoutUnit(8000)));
    EXPECT_EQ(LayoutUnit(1), ComputeInlineSize(LogicalSize(1, 10000), LayoutUnit(10000)));
    EXPECT_EQ(LayoutUnit(4), ComputeInlineSize(LogicalSize(1, 1000000), LayoutUnit(4000000)));
    EXPECT_EQ(LayoutUnit(0), ComputeInlineSize(LogicalSize(3, 5000000), LayoutUnit(5)));
    // The literals are 8 million, 20 million, 10 million, 4 million.
    EXPECT_EQ(LayoutUnit(8000000), ComputeInlineSize(LogicalSize(20000000, 10000000), LayoutUnit(4000000)));
    // If you specify an aspect ratio of 10000:1 with a large block size,
    // LayoutUnit saturates.
    EXPECT_EQ(LayoutUnit::Max(), ComputeInlineSize(LogicalSize(10000, 1), LayoutUnit(10000)));
}
