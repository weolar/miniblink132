...
protected:
    ...
    {
        const auto* layout_box = To<LayoutBox>(GetDocument().getElementsByTagName(AtomicString("html"))->item(0)->GetLayoutObject());
        return To<PhysicalBoxFragment>(&layout_box->GetSingleCachedLayoutResult()->GetPhysicalFragment());
    }
    ...
    MinMaxSizes RunComputeMinMaxSizes(BlockNode node)
    {
        ...
        ConstraintSpace space
            = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(), LayoutUnit()));
    ...
    const LayoutResult* RunCachedLayoutResult(const ConstraintSpace& space, const BlockNode& node)
    {
        LayoutCacheStatus cache_status;
        std::optional<FragmentGeometry> initial_fragment_geometry;
        return To<LayoutBlockFlow>(node.GetLayoutBox())->CachedLayoutResult(space, nullptr, nullptr, nullptr, &initial_fragment_geometry, &cache_status);
    }

    String DumpFragmentTree(const PhysicalBoxFragment* fragment)
    {
        PhysicalFragment::DumpFlags flags = PhysicalFragment::DumpHeaderText | PhysicalFragment::DumpSubtree | PhysicalFragment::DumpIndentation
            | PhysicalFragment::DumpOffset | PhysicalFragment::DumpSize;

        return fragment->DumpFragmentTree(flags);
    }

    template <typename UpdateFunc> void UpdateStyleForElement(Element* element, const UpdateFunc& update)
    {
...
TEST_F(BlockLayoutAlgorithmTest, FixedSize)
{
    ...
    ConstraintSpace space
        = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(100), kIndefiniteSize));
...
TEST_F(BlockLayoutAlgorithmTest, Caching)
{
    ...
    ConstraintSpace space
        = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(100), LayoutUnit(100)));
    ...
    space = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(100), LayoutUnit(100)));
    result = RunCachedLayoutResult(space, node);
    EXPECT_NE(result, nullptr);

    // Test different constraint space.
    space = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(200), LayoutUnit(100)));
    result = RunCachedLayoutResult(space, node);
    EXPECT_NE(result, nullptr);

    // Test a different constraint space that will actually result in a different
    // sized fragment.
    space = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(200), LayoutUnit(200)));
    result = RunCachedLayoutResult(space, node);
...
TEST_F(BlockLayoutAlgorithmTest, MinInlineSizeCaching)
{
    ...
    // Test different constraint space.
    space = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(100), LayoutUnit(200)));
    result = RunCachedLayoutResult(space, node);
    EXPECT_NE(result, nullptr);
...
TEST_F(BlockLayoutAlgorithmTest, PercentageBlockSizeQuirkDescendantsCaching)
{
    ...
    auto create_space = [&](auto size) -> ConstraintSpace {
        ConstraintSpaceBuilder builder(WritingMode::kHorizontalTb, { WritingMode::kHorizontalTb, TextDirection::kLtr },
    ...
    ConstraintSpace space100 = create_space(LogicalSize(LayoutUnit(100), LayoutUnit(100)));
    ConstraintSpace space200 = create_space(LogicalSize(LayoutUnit(100), LayoutUnit(200)));
...
TEST_F(BlockLayoutAlgorithmTest, LineOffsetCaching)
{
    ...
    ConstraintSpace space200 = create_space(LogicalSize(LayoutUnit(300), LayoutUnit(100)), BfcOffset(LayoutUnit(50), LayoutUnit()));
...
TEST_F(BlockLayoutAlgorithmTest, LayoutBlockChildren)
{
    ...
    const PhysicalBoxFragment* fragment = RunBlockLayoutAlgorithm(container, space);

    EXPECT_EQ(LayoutUnit(kWidth), fragment->Size().width);
    EXPECT_EQ(LayoutUnit(kHeight1 + kHeight2 + kMarginTop), fragment->Size().height);
...
TEST_F(BlockLayoutAlgorithmTest, LayoutBlockChildrenWithWritingMode)
{
    ...
    BlockNode container(GetLayoutBoxByElementId("container"));
    ConstraintSpace space
        = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(500), LayoutUnit(500)));
    const PhysicalBoxFragment* fragment = RunBlockLayoutAlgorithm(container, space);

    const PhysicalFragmentLink& child = fragment->Children()[0];
    const PhysicalFragmentLink& child2 = static_cast<const PhysicalBoxFragment*>(child.get())->Children()[0];
...
TEST_F(BlockLayoutAlgorithmTest, CollapsingMarginsCase1WithFloats)
{
    ...
    auto* container_fragment = To<PhysicalBoxFragment>(body_fragment->Children()[0].get());
...
TEST_F(BlockLayoutAlgorithmTest, CollapsingMarginsCase3)
{
    ...
    auto run_test = [&](const Length& container_height) {
        UpdateStyleForElement(GetElementById("container"), [&](ComputedStyleBuilder& builder) { builder.SetHeight(container_height); });
        ...
        container_fragment = To<PhysicalBoxFragment>(body_fragment->Children()[0].get());
        ASSERT_EQ(1UL, container_fragment->Children().size());
        child_fragment = To<PhysicalBoxFragment>(container_fragment->Children()[0].get());
    };
...
TEST_F(BlockLayoutAlgorithmTest, CollapsingMarginsCase4)
{
    ...
    auto run_test = [&](const Length& container_padding_top) {
        UpdateStyleForElement(GetElementById("container"), [&](ComputedStyleBuilder& builder) { builder.SetPaddingTop(container_padding_top); });
        fragment = GetHtmlPhysicalFragment();
        ASSERT_EQ(1UL, fragment->Children().size());
        const auto* body_fragment = To<PhysicalBoxFragment>(fragment->Children()[0].get());
        body_offset = fragment->Children()[0].Offset();
        const auto* container_fragment = To<PhysicalBoxFragment>(body_fragment->Children()[0].get());
...
TEST_F(BlockLayoutAlgorithmTest, CollapsingMarginsWithText)
{
    ...
    const auto* body_fragment = To<PhysicalBoxFragment>(html_fragment->Children()[0].get());
...
TEST_F(BlockLayoutAlgorithmTest, CollapsingMarginsEmptyBlockWithClearance)
{
    ...
    auto run_test = [&](const Length& zero_top_margin_bottom, const Length& zero_inner_margin_top, const Length& zero_inner_margin_bottom,
                        const Length& zero_margin_bottom, const Length& inflow_margin_top) {
        // Set the style of the elements we care about.
        UpdateStyleForElement(GetElementById("zero-top"), [&](ComputedStyleBuilder& builder) { builder.SetMarginBottom(zero_top_margin_bottom); });
        UpdateStyleForElement(GetElementById("zero-inner"), [&](ComputedStyleBuilder& builder) {
            builder.SetMarginTop(zero_inner_margin_top);
            builder.SetMarginBottom(zero_inner_margin_bottom);
        });
        UpdateStyleForElement(GetElementById("zero"), [&](ComputedStyleBuilder& builder) { builder.SetMarginBottom(zero_margin_bottom); });
        UpdateStyleForElement(GetElementById("inflow"), [&](ComputedStyleBuilder& builder) { builder.SetMarginTop(inflow_margin_top); });
        UpdateAllLifecyclePhasesForTest();
...
TEST_F(BlockLayoutAlgorithmTest, NewFormattingContextAutoMargins)
{
    ...
    const auto* fragment = &To<PhysicalBoxFragment>(GetLayoutBoxByElementId("container")->GetSingleCachedLayoutResult()->GetPhysicalFragment());
...
TEST_F(BlockLayoutAlgorithmTest, BorderAndPadding)
{
    ...
    ConstraintSpace space
        = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(1000), kIndefiniteSize));
    ...
    EXPECT_EQ(kBorderLeft + kPaddingLeft + kWidth + kPaddingRight + kBorderRight, child->Size().width);
    EXPECT_EQ(kBorderTop + kPaddingTop + kHeight + kPaddingBottom + kBorderBottom, child->Size().height);

    ASSERT_TRUE(child->IsBox());
    ASSERT_EQ(static_cast<const PhysicalBoxFragment*>(child)->Children().size(), 1UL);

    PhysicalOffset div2_offset = static_cast<const PhysicalBoxFragment*>(child)->Children()[0].Offset();
...
TEST_F(BlockLayoutAlgorithmTest, PositionFloatInsideEmptyBlocks)
{
    ...
    EXPECT_THAT(offset, PhysicalOffset(LayoutUnit(10) + right_float_offset, LayoutUnit(15)));
...
TEST_F(BlockLayoutAlgorithmTest, PositionFragmentsWithClear)
{
    ...
    auto run_with_clearance = [&](EClear clear_value) {
        UpdateStyleForElement(GetElementById("clearance"), [&](ComputedStyleBuilder& builder) { builder.SetClear(clear_value); });
        const auto* fragment = GetHtmlPhysicalFragment();
        ASSERT_EQ(1UL, fragment->Children().size());
        const auto* body_fragment = To<PhysicalBoxFragment>(fragment->Children()[0].get());
        body_offset = fragment->Children()[0].Offset();
...
TEST_F(BlockLayoutAlgorithmTest, ComputeMinMaxContentNewFormattingContextNegativeMargins)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #f1 { float: left; width: 20px; }
...
TEST_F(BlockLayoutAlgorithmTest, ComputeMinMaxContentSingleNewFormattingContextNegativeMargins)
{
    SetBodyInnerHTML(R"HTML(
    <style>
...
TEST_F(BlockLayoutAlgorithmTest, PositionEmptyBlocksInNewBfc)
{
    ...
    const auto* html_fragment = GetHtmlPhysicalFragment();
    auto* body_fragment = To<PhysicalBoxFragment>(html_fragment->Children()[0].get());
    auto* container_fragment = To<PhysicalBoxFragment>(body_fragment->Children()[0].get());
    PhysicalOffset empty_block1_offset = container_fragment->Children()[1].Offset();
    // empty-block1's margin == 8
    EXPECT_THAT(empty_block1_offset, PhysicalOffset(8, 8));

    PhysicalOffset empty_block2_offset = container_fragment->Children()[2].Offset();
...
// intruding floats.
TEST_F(BlockLayoutAlgorithmTest, PositionBlocksWithClearanceAndIntrudingFloats)
{
    SetBodyInnerHTML(R"HTML(
    <!DOCTYPE html>
    <style>
    ...
    auto* container_clear_fragment = To<PhysicalBoxFragment>(body_fragment->Children()[3].get());
    ...
    PhysicalOffset clears_right_offset = container_clear_fragment->Children()[1].Offset();
...
TEST_F(BlockLayoutAlgorithmTest, NoFragmentation)
{
    ...
    ConstraintSpace space
        = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(1000), kIndefiniteSize),
            /* stretch_inline_size_if_auto */ true, node.CreatesNewFormattingContext(), kFragmentainerSpaceAvailable);
...
TEST_F(BlockLayoutAlgorithmTest, DISABLED_FloatFragmentationParallelFlows)
{
    ...
    space = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(1000), kIndefiniteSize),
        /* stretch_inline_size_if_auto */ true, node.CreatesNewFormattingContext(), kFragmentainerSpaceAvailable);
...
TEST_F(BlockLayoutAlgorithmTest, FloatFragmentationOrthogonalFlows)
{
    ...
    const BreakToken* break_token = To<PhysicalBoxFragment>(float2)->GetBreakToken();
...
TEST_F(BlockLayoutAlgorithmTest, NewFcBlockWithAdjoiningFloatCollapsesMargins)
{
    ...
    auto run_test = [&](const Length& block_width) {
        UpdateStyleForElement(GetElementById("new-fc"), [&](ComputedStyleBuilder& builder) { builder.SetWidth(block_width); });
        const auto* fragment = GetHtmlPhysicalFragment();
        ASSERT_EQ(1UL, fragment->Children().size());
        const auto* body_fragment = To<PhysicalBoxFragment>(fragment->Children()[0].get());
        const auto* container_fragment = To<PhysicalBoxFragment>(body_fragment->Children()[0].get());
...
TEST_F(BlockLayoutAlgorithmTest, RootFragmentOffsetInsideLegacy)
{
    ...
    const PhysicalBoxFragment* fragment = CurrentFragmentFor(To<LayoutBlockFlow>(innerNGRoot));
