...
struct CORE_EXPORT ExclusionShapeData final : public GarbageCollected<ExclusionShapeData> {
    ExclusionShapeData(const LayoutBox* layout_box, const BoxStrut& margins, const BoxStrut& shape_insets)
...
struct CORE_EXPORT ExclusionArea final : public GarbageCollected<ExclusionArea> {
    enum Kind {
        kFloat,
        kInitialLetterBox,
    };

    ExclusionArea(const BfcRect& rect, const EFloat type, const Kind kind, bool is_hidden_for_paint, const ExclusionShapeData* shape_data)
        : rect(rect)
        , type(type)
        , kind(kind)
    ...
    static const ExclusionArea* Create(const BfcRect& rect, const EFloat type, bool is_hidden_for_paint, ExclusionShapeData* shape_data = nullptr)
    {
        return MakeGarbageCollected<ExclusionArea>(rect, type, kFloat, is_hidden_for_paint, std::move(shape_data));
    }

    static const ExclusionArea* CreateForInitialLetterBox(const BfcRect& rect, const EFloat type, bool is_hidden_for_paint)
    {
        return MakeGarbageCollected<ExclusionArea>(rect, type, kInitialLetterBox, is_hidden_for_paint, nullptr);
    }
    ...
    const ExclusionArea* CopyWithOffset(const BfcDelta& offset_delta) const
    {
        ...
        return MakeGarbageCollected<ExclusionArea>(
            new_rect, type, kind, is_hidden_for_paint, shape_data ? MakeGarbageCollected<ExclusionShapeData>(*shape_data) : nullptr);
    }

    bool IsForInitialLetterBox() const
    {
        return kind == kInitialLetterBox;
    }

    void Trace(Visitor* visitor) const
    {
        visitor->Trace(shape_data);
    }
