...
public:
    ...
    LayoutOpportunity FindLayoutOpportunity(const BfcOffset& offset, const LayoutUnit available_inline_size, const LayoutUnit minimum_inline_size) const
    {
        const LayoutUnit max_clear_offset
            = std::max({ left_clear_offset_, right_clear_offset_, initial_letter_left_clear_offset_, initial_letter_right_clear_offset_ });
        ...
        if (offset.block_offset >= max_clear_offset) {
            BfcOffset end_offset(offset.line_offset + available_inline_size.ClampNegativeToZero(), LayoutUnit::Max());
            return LayoutOpportunity(BfcRect(offset, end_offset), nullptr);
        }

        return GetDerivedGeometry(offset.block_offset).FindLayoutOpportunity(offset, available_inline_size, minimum_inline_size);
    }

    LayoutOpportunityVector AllLayoutOpportunities(const BfcOffset& offset, const LayoutUnit available_inline_size) const
    {
        const LayoutUnit max_clear_offset
            = std::max({ left_clear_offset_, right_clear_offset_, initial_letter_left_clear_offset_, initial_letter_right_clear_offset_ });
        ...
        if (offset.block_offset >= max_clear_offset) {
            BfcOffset end_offset(offset.line_offset + available_inline_size.ClampNegativeToZero(), LayoutUnit::Max());
            return LayoutOpportunityVector({ LayoutOpportunity(BfcRect(offset, end_offset), nullptr) });
        }

        return GetDerivedGeometry(offset.block_offset).AllLayoutOpportunities(offset, available_inline_size);
    }
    ...
    {
        return std::max(ClearanceOffset(clear_type), InitialLetterClearanceOffset(EClear::kBoth));
    }
    ...
    LayoutUnit InitialLetterClearanceOffset(EClear clear_type) const
    {
        ...
        case EClear::kBoth:
            return std::max(initial_letter_left_clear_offset_, initial_letter_right_clear_offset_);
        default:
            NOTREACHED();
        }
    }
    ...
    bool NeedsClearancePastFragmentainer(EClear type) const
    {
        ...
        case EClear::kLeft:
        case EClear::kBoth:
            needs_clearance |= has_break_inside_left_float_ || has_break_before_left_float_;
            if (type == EClear::kLeft)
                break;
            [[fallthrough]];
        case EClear::kRight:
            needs_clearance |= has_break_inside_right_float_ || has_break_before_right_float_;
            break;
        }
    ...
    {
        return last_float_block_start_;
    }
    ...
    void MergeExclusionSpaces(const BfcDelta& offset_delta, const ExclusionSpaceInternal& previous_output, const ExclusionSpaceInternal* previous_input)
    {
        // We need to copy all the exclusions over which were added by the cached
        // layout result.
        for (wtf_size_t i = previous_input ? previous_input->num_exclusions_ : 0; i < previous_output.num_exclusions_; ++i) {
    ...
    public:
        Shelf(LayoutUnit block_offset, bool track_shape_exclusions)
            ...
            , shape_exclusions(track_shape_exclusions ? MakeGarbageCollected<ShapeExclusions>() : nullptr)
        ...
        Shelf(const Shelf& other)
            ...
            , shape_exclusions(other.shape_exclusions ? MakeGarbageCollected<ShapeExclusions>(*other.shape_exclusions) : nullptr)
        ...
        {
            visitor->Trace(shape_exclusions);
        }
    ...
    public:
        ClosedArea(LayoutOpportunity opportunity, const Vector<ShelfEdge>& line_left_edges, const Vector<ShelfEdge>& line_right_edges)
...
private:
    ...
    struct CORE_EXPORT DerivedGeometry final : public GarbageCollected<DerivedGeometry> {
    public:
        ...
        void IterateAllLayoutOpportunities(const BfcOffset& offset, const LayoutUnit available_inline_size, const LambdaFunc&) const;
    ...
    const DerivedGeometry& GetDerivedGeometry(LayoutUnit block_offset_limit) const;
...
public:
    ...
    ExclusionSpace(const ExclusionSpace& other)
        : exclusion_space_(other.exclusion_space_ ? std::make_unique<ExclusionSpaceInternal>(*other.exclusion_space_) : nullptr)
    {
    }
    ...
    bool NeedsClearancePastFragmentainer(EClear type) const
    {
        return exclusion_space_ && exclusion_space_->NeedsClearancePastFragmentainer(type);
    }
    ...
    LayoutOpportunity FindLayoutOpportunity(
        const BfcOffset& offset, const LayoutUnit available_inline_size, const LayoutUnit minimum_inline_size = LayoutUnit()) const
    {
        if (!exclusion_space_) {
            BfcOffset end_offset(offset.line_offset + available_inline_size.ClampNegativeToZero(), LayoutUnit::Max());
            return LayoutOpportunity(BfcRect(offset, end_offset), nullptr);
        }
        return exclusion_space_->FindLayoutOpportunity(offset, available_inline_size, minimum_inline_size);
    }
    ...
    LayoutOpportunityVector AllLayoutOpportunities(const BfcOffset& offset, const LayoutUnit available_inline_size) const
    {
        ...
        return exclusion_space_->AllLayoutOpportunities(offset, available_inline_size);
    }
    ...
    static ExclusionSpace MergeExclusionSpaces(
        const ExclusionSpace& old_output, const ExclusionSpace& old_input, const ExclusionSpace& new_input, const BfcDelta& offset_delta)
    {
        ...
        new_output.exclusion_space_->MergeExclusionSpaces(offset_delta, *old_output.exclusion_space_, old_input.exclusion_space_.get());
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::ExclusionSpaceInternal::ShelfEdge)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::ExclusionSpaceInternal::Shelf)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::ExclusionSpaceInternal::ClosedArea)
