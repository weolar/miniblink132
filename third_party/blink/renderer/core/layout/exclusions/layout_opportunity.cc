...
LineSegment ExcludedSegment(const ExclusionArea& exclusion, LayoutUnit bfc_block_offset, LayoutUnit line_block_size)
{
    DCHECK(exclusion.shape_data);
    const ExclusionShapeData& shape_data = *exclusion.shape_data;
    const Shape& shape = shape_data.layout_box->GetShapeOutsideInfo()->ComputedShape();

    // Determine the block offset (relative to the shape) at which we need to
    // test for.
    LayoutUnit shape_relative_block_offset
        = bfc_block_offset - (exclusion.rect.BlockStartOffset() + shape_data.margins.block_start + shape_data.shape_insets.block_start);
    ...
    LayoutUnit clamped_line_block_size
        = std::min(line_block_size, exclusion.rect.BlockSize() - shape_data.shape_insets.BlockSum() - shape_data.margins.BlockSum());

    LineSegment segment = shape.GetExcludedInterval(shape_relative_block_offset, clamped_line_block_size);

    // Adjust the segment offsets to be relative to the line-left margin edge.
    LayoutUnit margin_delta = shape_data.margins.LineLeft(TextDirection::kLtr) + shape_data.shape_insets.LineLeft(TextDirection::kLtr);
    ...
    segment.logical_left = ClampTo<LayoutUnit>(segment.logical_left, LayoutUnit(), exclusion.rect.InlineSize());
    segment.logical_right = ClampTo<LayoutUnit>(segment.logical_right, LayoutUnit(), exclusion.rect.InlineSize());
...
// exclusion.
bool IntersectsExclusion(const ExclusionArea& exclusion, LayoutUnit bfc_block_offset, LayoutUnit line_block_size)
{
    return bfc_block_offset < exclusion.rect.BlockEndOffset() && bfc_block_offset + line_block_size > exclusion.rect.BlockStartOffset();
}
...
bool LayoutOpportunity::IsBlockDeltaBelowShapes(LayoutUnit block_delta) const
{
    ...
    for (const auto& exclusion : shape_exclusions->line_left_shapes) {
        if (rect.BlockStartOffset() + block_delta < exclusion->rect.BlockEndOffset())
            return false;
    }
...
LayoutUnit LayoutOpportunity::ComputeLineLeftOffset(const ConstraintSpace& space, LayoutUnit line_block_size, LayoutUnit block_delta) const
{
    ...
    for (auto& exclusion : shape_exclusions->line_left_shapes) {
        ...
        if (exclusion->shape_data) {
            LineSegment segment = ExcludedSegment(*exclusion, bfc_block_offset, line_block_size);
            if (segment.is_valid)
...
LayoutUnit LayoutOpportunity::ComputeLineRightOffset(const ConstraintSpace& space, LayoutUnit line_block_size, LayoutUnit block_delta) const
{
    ...
    LayoutUnit line_right = space.GetBfcOffset().line_offset + space.AvailableSize().inline_size;
...
std::ostream& operator<<(std::ostream& ostream, const LayoutOpportunity& opportunity)
{
