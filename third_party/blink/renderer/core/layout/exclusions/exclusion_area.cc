...
{
    return type == other.type && kind == other.kind && rect == other.rect && is_hidden_for_paint == other.is_hidden_for_paint && shape_data == other.shape_data;
}

namespace {

struct PrintableEFloat {
    explicit PrintableEFloat(EFloat value)
        : value(value)
    {
    }
    EFloat value;
...
struct PrintableKind {
    explicit PrintableKind(ExclusionArea::Kind value)
        : value(value)
    {
    }
...
{
    return os << "ExclusionArea(" << PrintableKind(exclusion.kind) << ", " << PrintableEFloat(exclusion.type) << ", " << exclusion.rect << ")";
}
