...
void InsertClosedArea(const ExclusionSpaceInternal::ClosedArea area, HeapVector<ExclusionSpaceInternal::ClosedArea, 4>& areas)
{
    if (areas.empty()) {
    ...
    for (wtf_size_t i = areas.size(); i--;) {
        ...
        if (other.opportunity.rect.BlockStartOffset() <= area.opportunity.rect.BlockStartOffset()) {
            ...
            if (other.opportunity.rect.BlockStartOffset() == area.opportunity.rect.BlockStartOffset()) {
                DCHECK_LE(other.opportunity.rect.BlockSize(), area.opportunity.rect.BlockSize());
                DCHECK_GE(other.opportunity.rect.InlineSize(), area.opportunity.rect.InlineSize());
            }
#endif
...
bool HasSolidEdges(const Vector<ExclusionSpaceInternal::ShelfEdge>& edges, LayoutUnit block_start, LayoutUnit block_end)
{
...
void CollectSolidEdges(Vector<ExclusionSpaceInternal::ShelfEdge>* edges, LayoutUnit block_offset, Vector<ExclusionSpaceInternal::ShelfEdge>* out_edges)
{
    *out_edges = std::move(*edges);
    auto it = std::remove_if(out_edges->begin(), out_edges->end(), [&](const auto& edge) { return edge.block_end <= block_offset; });
...
bool Intersects(const LayoutOpportunity& opportunity, const BfcOffset& offset, const LayoutUnit inline_size)
{
    return opportunity.rect.LineEndOffset() >= offset.line_offset && opportunity.rect.LineStartOffset() <= offset.line_offset + inline_size
...
LayoutOpportunity CreateLayoutOpportunity(const LayoutOpportunity& other, const BfcOffset& offset, const LayoutUnit inline_size)
{
    DCHECK(Intersects(other, offset, inline_size));

    BfcOffset start_offset(std::max(other.rect.LineStartOffset(), offset.line_offset), std::max(other.rect.start_offset.block_offset, offset.block_offset));

    BfcOffset end_offset(std::min(other.rect.LineEndOffset(), offset.line_offset + inline_size), other.rect.BlockEndOffset());

    return LayoutOpportunity(
        BfcRect(start_offset, end_offset), other.shape_exclusions ? MakeGarbageCollected<ShapeExclusions>(*other.shape_exclusions) : nullptr);
}

// Creates a new layout opportunity. The given shelf *must* intersect with the
// given area (defined by offset and inline_size).
LayoutOpportunity CreateLayoutOpportunity(const ExclusionSpaceInternal::Shelf& shelf, const BfcOffset& offset, const LayoutUnit inline_size)
{
    BfcOffset start_offset(std::max(shelf.line_left, offset.line_offset), std::max(shelf.block_offset, offset.block_offset));

    // Max with |start_offset.line_offset| in case the shelf has a negative
    // inline-size.
    BfcOffset end_offset(std::max(std::min(shelf.line_right, offset.line_offset + inline_size), start_offset.line_offset), LayoutUnit::Max());

    return LayoutOpportunity(
        BfcRect(start_offset, end_offset), shelf.has_shape_exclusions ? MakeGarbageCollected<ShapeExclusions>(*shelf.shape_exclusions) : nullptr);
}
...
ExclusionSpaceInternal::ExclusionSpaceInternal(const ExclusionSpaceInternal& other)
...
ExclusionSpaceInternal& ExclusionSpaceInternal::operator=(const ExclusionSpaceInternal& other)
{
    CopyFrom(other);
    derived_geometry_ = std::move(other.derived_geometry_);
    other.derived_geometry_ = nullptr;
...
ExclusionSpaceInternal::DerivedGeometry::DerivedGeometry(LayoutUnit block_offset_limit, bool track_shape_exclusions)
    ...
    , track_shape_exclusions_(track_shape_exclusions)
{
    ...
    shelves_.emplace_back(/* block_offset */ LayoutUnit::Min(), track_shape_exclusions_);
}
...
void ExclusionSpaceInternal::Add(const ExclusionArea* exclusion)
{
    ...
    const_cast<ExclusionArea*>(exclusion)->is_past_other_exclusions
        = exclusion_block_offset >= std::max({ left_clear_offset_, exclusion_block_offset, right_clear_offset_, exclusion_block_offset,
              initial_letter_left_clear_offset_, exclusion_block_offset, initial_letter_right_clear_offset_ });
    ...
    if (exclusion->IsForInitialLetterBox()) [[unlikely]] {
        if (exclusion->type == EFloat::kLeft) {
            initial_letter_left_clear_offset_ = std::max(initial_letter_left_clear_offset_, clear_offset);
        } else if (exclusion->type == EFloat::kRight) {
            initial_letter_right_clear_offset_ = std::max(initial_letter_right_clear_offset_, clear_offset);
        }
        ...
        if (!already_exists) {
            ...
            const auto source_span = base::span(source_exclusions.data(), source_exclusions.size()).first(num_exclusions_);
            ...
            for (auto it = source_span.begin(); it != source_end; ++it) {
                if (exclusion->rect.BlockStartOffset() < (*it)->rect.BlockStartOffset()) {
    ...
    last_float_block_start_ = std::max(last_float_block_start_, exclusion_block_offset);
...
void ExclusionSpaceInternal::DerivedGeometry::Add(const ExclusionArea& exclusion)
{
    ...
    for (wtf_size_t i = 0; i < shelves_.size(); ++i) {
        ...
        // A new scope is created as shelf may be removed.
        {
            ...
            is_between_shelves
                = exclusion_end_offset >= shelf.block_offset && (i + 1 >= shelves_.size() || exclusion_end_offset < shelves_[i + 1].block_offset);
            ...
            if (is_below) {
                ...
                // against that edge.
                bool has_solid_edges = HasSolidEdges(shelf.line_left_edges, shelf.block_offset, exclusion.rect.BlockStartOffset())
                    && HasSolidEdges(shelf.line_right_edges, shelf.block_offset, exclusion.rect.BlockStartOffset());
                ...
                bool is_overlapping;
                if (exclusion.type == EFloat::kLeft) {
                    is_overlapping = exclusion.rect.LineStartOffset() <= shelf.line_right && exclusion.rect.LineEndOffset() > shelf.line_left;
                } else {
                ...
                if (has_solid_edges && is_overlapping) {
                    LayoutOpportunity opportunity(BfcRect(
                                                      /* start_offset */ { shelf.line_left, shelf.block_offset },
                                                      /* end_offset */ { shelf.line_right, exclusion.rect.BlockStartOffset() }),
                        shelf.has_shape_exclusions ? MakeGarbageCollected<ShapeExclusions>(*shelf.shape_exclusions) : nullptr);

                    InsertClosedArea(ClosedArea(opportunity, shelf.line_left_edges, shelf.line_right_edges), areas_);
                }
            }
            ...
            //    X--------|NEW|--X
            //             +---+
            bool is_intersecting = !is_below && exclusion_end_offset > shelf.block_offset;
            ...
            if (is_below || is_intersecting) {
                if (exclusion.type == EFloat::kLeft) {
                    if (exclusion.rect.LineEndOffset() >= shelf.line_left) {
                        ...
                        shelf.line_left_edges.emplace_back(exclusion.rect.BlockStartOffset(), exclusion.rect.BlockEndOffset());
                    }
                    ...
                    if (exclusion.rect.LineStartOffset() <= shelf.line_right) {
                        ...
                        shelf.line_right_edges.emplace_back(exclusion.rect.BlockStartOffset(), exclusion.rect.BlockEndOffset());
                    }
                    if (shelf.shape_exclusions)
                ...
                bool is_same_as_previous = (i > 0) && shelf.line_left == shelves_[i - 1].line_left && shelf.line_right == shelves_[i - 1].line_right;
        ...
        if (is_between_shelves) {
            ...
            if (exclusion_end_offset != shelf_copy->block_offset) {
                Shelf new_shelf(/* block_offset */ exclusion_end_offset, track_shape_exclusions_);

                // shelf_copy->line_{left,right}_edges will not valid after these calls.
                CollectSolidEdges(&shelf_copy->line_left_edges, new_shelf.block_offset, &new_shelf.line_left_edges);

                CollectSolidEdges(&shelf_copy->line_right_edges, new_shelf.block_offset, &new_shelf.line_right_edges);
                ...
                new_shelf.line_left = new_shelf.line_left_edges.empty() ? LayoutUnit::Min() : shelf_copy->line_left;
                new_shelf.line_right = new_shelf.line_right_edges.empty() ? LayoutUnit::Max() : shelf_copy->line_right;
...
LayoutOpportunity ExclusionSpaceInternal::DerivedGeometry::FindLayoutOpportunity(
    const BfcOffset& offset, const LayoutUnit available_inline_size, const LayoutUnit minimum_inline_size) const
{
    ...
    IterateAllLayoutOpportunities(offset, available_inline_size,
        [&return_opportunity, &offset, &available_inline_size, &minimum_inline_size](const LayoutOpportunity opportunity) -> bool {
            ...
            if (opportunity.rect.InlineSize() >= minimum_inline_size
                || (opportunity.rect.InlineSize() == available_inline_size && opportunity.rect.LineStartOffset() == offset.line_offset)) {
...
LayoutOpportunityVector ExclusionSpaceInternal::DerivedGeometry::AllLayoutOpportunities(const BfcOffset& offset, const LayoutUnit available_inline_size) const
{
    DCHECK_GE(offset.block_offset, block_offset_limit_);
    LayoutOpportunityVector opportunities;

    // This method is only used for determining the position of line-boxes.
    IterateAllLayoutOpportunities(offset, available_inline_size, [&opportunities](const LayoutOpportunity opportunity) -> bool {
...
void ExclusionSpaceInternal::DerivedGeometry::IterateAllLayoutOpportunities(
    const BfcOffset& offset, const LayoutUnit available_inline_size, const LambdaFunc& lambda) const
{
    auto shelves_span = base::span(shelves_.data(), shelves_.size());
    auto areas_span = base::span(areas_.data(), areas_.size());
    ...
    while (shelves_it != shelves_end || areas_it != areas_end) {
        ...
        if (areas_it != areas_end) {
            ...
            LayoutUnit block_start_offset = std::max(area.opportunity.rect.BlockStartOffset(), offset.block_offset);

            // We always prefer the closed-off area opportunity, instead of the shelf
            // opportunity if they exist at the some offset.
            if (block_start_offset <= std::max(shelf.block_offset, offset.block_offset)) {
                LayoutUnit block_end_offset = area.opportunity.rect.BlockEndOffset();

                bool has_solid_edges = HasSolidEdges(area.line_left_edges, block_start_offset, block_end_offset)
                    && HasSolidEdges(area.line_right_edges, block_start_offset, block_end_offset);
                if (has_solid_edges) {
                    if (lambda(CreateLayoutOpportunity(area.opportunity, offset, available_inline_size)))
                        return;
                }
        ...
        bool has_solid_edges = HasSolidEdges(shelf.line_left_edges, offset.block_offset, LayoutUnit::Max())
            && HasSolidEdges(shelf.line_right_edges, offset.block_offset, LayoutUnit::Max());
        if (has_solid_edges) {
...
const ExclusionSpaceInternal::DerivedGeometry& ExclusionSpaceInternal::GetDerivedGeometry(LayoutUnit block_offset_limit) const
{
    // We might have a geometry, but built at a lower block-offset limit.
    if (derived_geometry_ && block_offset_limit < derived_geometry_->block_offset_limit_)
        derived_geometry_ = nullptr;
    ...
    if (!derived_geometry_) {
        ...
        // |block_offset_limit|.
        auto it = std::lower_bound(begin, end, block_offset_limit,
            [](const auto& exclusion, const auto& block_offset) -> bool { return exclusion->rect.BlockStartOffset() < block_offset; });
        ...
        derived_geometry_ = MakeGarbageCollected<DerivedGeometry>(block_offset_limit, track_shape_exclusions_);
        for (; it < end; ++it)
...
bool ExclusionSpaceInternal::operator==(const ExclusionSpaceInternal& other) const
{
    if (num_exclusions_ == 0 && other.num_exclusions_ == 0)
        return true;
    return num_exclusions_ == other.num_exclusions_ && exclusions_ == other.exclusions_ && has_break_before_left_float_ == other.has_break_before_left_float_
        && has_break_before_right_float_ == other.has_break_before_right_float_ && has_break_inside_left_float_ == other.has_break_inside_left_float_
        && has_break_inside_right_float_ == other.has_break_inside_right_float_;
}
