...
public:
    LayoutOpportunity()
        : rect(BfcOffset(LayoutUnit::Min(), LayoutUnit::Min()), BfcOffset(LayoutUnit::Max(), LayoutUnit::Max()))
    {
    }
    explicit LayoutOpportunity(const BfcRect& rect, const ShapeExclusions* shape_exclusions = nullptr)
        : rect(rect)
        , shape_exclusions(shape_exclusions)
    {
    }
    ...
    // opportunity.
    bool HasShapeExclusions() const
    {
        return shape_exclusions != nullptr;
    }
    ...
    LineLayoutOpportunity ComputeLineLayoutOpportunity(const ConstraintSpace& space, LayoutUnit line_block_size, LayoutUnit block_delta) const
    {
        return LineLayoutOpportunity(ComputeLineLeftOffset(space, line_block_size, block_delta), ComputeLineRightOffset(space, line_block_size, block_delta),
            rect.LineStartOffset(), rect.LineEndOffset(), rect.BlockStartOffset() + block_delta, line_block_size);
    }
...
private:
    LayoutUnit ComputeLineLeftOffset(const ConstraintSpace&, LayoutUnit line_block_size, LayoutUnit block_delta) const;
    LayoutUnit ComputeLineRightOffset(const ConstraintSpace&, LayoutUnit line_block_size, LayoutUnit block_delta) const;
};

CORE_EXPORT std::ostream& operator<<(std::ostream& os, const LayoutOpportunity& opportunity);
