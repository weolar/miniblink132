...
LayoutReplaced::LayoutReplaced(Element* element, const PhysicalSize& intrinsic_size)
    : LayoutBox(element)
...
void LayoutReplaced::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    float old_zoom = had_style ? old_style->EffectiveZoom() : ComputedStyleInitialValues::InitialZoom();
    if (Style() && StyleRef().EffectiveZoom() != old_zoom)
        IntrinsicSizeChanged();

    if ((IsLayoutImage() || IsVideo() || IsCanvas()) && !ClipsToContentBox() && !StyleRef().ObjectPropertiesPreventReplacedOverflow()) {
        static constexpr const char kErrorMessage[] = "Specifying 'overflow: visible' on img, video and canvas tags may "
        ...
        auto* console_message
            = MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning, kErrorMessage);
...
{
    NOT_DESTROYED();
    LayoutUnit scaled_width = LayoutUnit(static_cast<int>(kDefaultWidth * StyleRef().EffectiveZoom()));
    LayoutUnit scaled_height = LayoutUnit(static_cast<int>(kDefaultHeight * StyleRef().EffectiveZoom()));
    intrinsic_size_ = PhysicalSize(scaled_width, scaled_height);
    SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kSizeChanged);
}
...
static inline bool LayoutObjectHasIntrinsicAspectRatio(const LayoutObject* layout_object)
{
    DCHECK(layout_object);
    return layout_object->IsImage() || layout_object->IsCanvas() || IsA<LayoutVideo>(layout_object) || IsA<LayoutViewTransitionContent>(layout_object);
}
...
std::optional<PhysicalRect> LayoutReplaced::ComputeObjectViewBoxRect(const PhysicalSize* overridden_intrinsic_size) const
{
    ...
    const auto& intrinsic_size = overridden_intrinsic_size ? *overridden_intrinsic_size : intrinsic_size_;
    ...
    Path path;
    gfx::RectF bounding_box(0, 0, intrinsic_size.width.ToFloat(), intrinsic_size.height.ToFloat());
    object_view_box->GetPath(path, bounding_box, 1.f);

    const PhysicalRect view_box_rect = PhysicalRect::EnclosingRect(path.BoundingRect());
...
PhysicalRect LayoutReplaced::ComputeReplacedContentRect(const PhysicalRect& base_content_rect, const PhysicalSize* overridden_intrinsic_size) const
{
    ...
    // overflow.
    const auto& intrinsic_size_for_object_view_box = overridden_intrinsic_size ? *overridden_intrinsic_size : intrinsic_size_;
    const auto view_box = ComputeObjectViewBoxRect(&intrinsic_size_for_object_view_box);

    // If no view box override was applied, then we don't need to adjust the
    // view-box paint rect.
    if (!view_box) {
        return ComputeObjectFitAndPositionRect(base_content_rect, overridden_intrinsic_size);
    }
    ...
    const auto view_box_paint_rect = ComputeObjectFitAndPositionRect(base_content_rect, &view_box_size);
    ...
    auto scaled_width = intrinsic_size_for_object_view_box.width.MulDiv(view_box_paint_rect.Width(), view_box->Width());
    auto scaled_height = intrinsic_size_for_object_view_box.height.MulDiv(view_box_paint_rect.Height(), view_box->Height());
    ...
    // box.
    auto scaled_x_offset = view_box->X().MulDiv(view_box_paint_rect.Width(), view_box->Width());
    auto scaled_y_offset = view_box->Y().MulDiv(view_box_paint_rect.Height(), view_box->Height());
    const PhysicalOffset scaled_offset(scaled_x_offset, scaled_y_offset);

    return PhysicalRect(view_box_paint_rect.offset - scaled_offset, scaled_image_size);
}

PhysicalRect LayoutReplaced::ComputeObjectFitAndPositionRect(const PhysicalRect& base_content_rect, const PhysicalSize* overridden_intrinsic_size) const
{
    NOT_DESTROYED();
    EObjectFit object_fit = StyleRef().GetObjectFit();

    if (object_fit == EObjectFit::kFill && StyleRef().ObjectPosition() == ComputedStyleInitialValues::InitialObjectPosition()) {
    ...
    PhysicalSize intrinsic_size(overridden_intrinsic_size ? *overridden_intrinsic_size : IntrinsicSize());
    if (intrinsic_size.IsEmpty())
        return base_content_rect;
    ...
    case EObjectFit::kCover:
        final_rect.size = final_rect.size.FitToAspectRatio(intrinsic_size, object_fit == EObjectFit::kCover ? kAspectRatioFitGrow : kAspectRatioFitShrink);
        if (object_fit != EObjectFit::kScaleDown || final_rect.Width() <= scaled_intrinsic_size.width)
            break;
        [[fallthrough]];
    ...
    LayoutUnit x_offset = MinimumValueForLength(StyleRef().ObjectPosition().X(), base_content_rect.Width() - final_rect.Width());
    LayoutUnit y_offset = MinimumValueForLength(StyleRef().ObjectPosition().Y(), base_content_rect.Height() - final_rect.Height());
...
PhysicalRect LayoutReplaced::ReplacedContentRectFrom(const PhysicalRect& base_content_rect) const
{
    NOT_DESTROYED();
    return ComputeReplacedContentRect(base_content_rect);
}

PhysicalRect LayoutReplaced::PreSnappedRectForPersistentSizing(const PhysicalRect& rect)
{
    return PhysicalRect(rect.offset, PhysicalSize(ToRoundedSize(rect.size)));
}

void LayoutReplaced::ComputeIntrinsicSizingInfo(IntrinsicSizingInfo& intrinsic_sizing_info) const
{
    NOT_DESTROYED();
...
static std::pair<LayoutUnit, LayoutUnit> SelectionTopAndBottom(const LayoutReplaced& layout_replaced)
{
    ...
    // |layout_replaced|.
    const std::pair<LayoutUnit, LayoutUnit> fallback(layout_replaced.LogicalTop(), layout_replaced.LogicalBottom());

    if (layout_replaced.IsInline() && layout_replaced.IsInLayoutNGInlineFormattingContext()) {
        ...
        const WritingModeConverter converter(writing_direction, line_box.ContainerFragment().Size());
        ...
        physical_rect.offset += OffsetInStitchedFragments(line_box.ContainerFragment());
...
PositionWithAffinity LayoutReplaced::PositionForPoint(const PhysicalOffset& point) const
{
    NOT_DESTROYED();
    ...
    if (RuntimeEnabledFeatures::SidewaysWritingModesEnabled()) {
        LogicalOffset logical_point = LocationContainer()->CreateWritingModeConverter().ToLogical(point + PhysicalLocation(), {});
        block_direction_position = logical_point.block_offset;
        line_direction_position = logical_point.inline_offset;
    } else {
        LayoutPoint flipped_point_in_container = LocationContainer()->FlipForWritingMode(point + PhysicalLocation());
        block_direction_position = IsHorizontalWritingMode() ? flipped_point_in_container.Y() : flipped_point_in_container.X();
        line_direction_position = IsHorizontalWritingMode() ? flipped_point_in_container.X() : flipped_point_in_container.Y();
    }
    ...
    if (GetNode()) {
        const bool is_at_left_side = line_direction_position <= LogicalLeft() + (LogicalWidth() / 2);
...
{
    NOT_DESTROYED();
    if (GetSelectionState() == SelectionState::kNone || GetSelectionState() == SelectionState::kContain) {
        return PhysicalRect();
    }
...
bool LayoutReplaced::ClipsToContentBox() const
{
    ...
    return GetOverflowClipAxes() == kOverflowClipBothAxis && overflow_clip_margin
        && overflow_clip_margin->GetReferenceBox() == StyleOverflowClipMargin::ReferenceBox::kContentBox && !overflow_clip_margin->GetMargin();
}
