...
void GetImageSizeChangeTracingData(perfetto::TracedValue context, Node* node, LocalFrame* frame)
{
...
void LayoutImage::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    TRACE_EVENT_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), &tracing_enabled);

    if (tracing_enabled) {
        bool is_unsized = this->IsUnsizedImage();
        if (is_unsized) {
            Node* node = GetNode();
            TRACE_EVENT_INSTANT_WITH_TIMESTAMP1("devtools.timeline", "LayoutImageUnsized", TRACE_EVENT_SCOPE_THREAD, base::TimeTicks::Now(), "data",
                [&](perfetto::TracedValue ctx) { GetImageSizeChangeTracingData(std::move(ctx), node, GetFrame()); });
        }
    }
}
...
void LayoutImage::ImageChanged(WrappedImagePtr new_image, CanDeferInvalidation defer)
{
    NOT_DESTROYED();
    DCHECK(View());
    ...
    if (IsGeneratedContent() && html_image_element && image_resource_->ErrorOccurred()) {
    ...
    if (image_resource_->CachedImage() && image_resource_->CachedImage()->HasDevicePixelRatioHeaderValue()) {
        UseCounter::Count(GetDocument(), WebFeature::kClientHintsContentDPR);
        image_device_pixel_ratio_ = 1 / image_resource_->CachedImage()->DevicePixelRatioHeaderValue();
    }

    if (!did_increment_visually_non_empty_pixel_count_) {
        // At a zoom level of 1 the image is guaranteed to have an integer size.
        View()->GetFrameView()->IncrementVisuallyNonEmptyPixelCount(gfx::ToFlooredSize(image_resource_->ImageSize(1.0f)));
...
bool LayoutImage::NeedsLayoutOnIntrinsicSizeChange() const
{
    ...
    bool is_fixed_sized = style.LogicalWidth().IsFixed() && style.LogicalHeight().IsFixed()
        && (style.LogicalMinWidth().IsFixed() || style.LogicalMinWidth().IsAuto()) && (style.LogicalMaxWidth().IsFixed() || style.LogicalMaxWidth().IsNone());
    return !is_fixed_sized;
}

void LayoutImage::InvalidatePaintAndMarkForLayoutIfNeeded(CanDeferInvalidation defer)
{
    NOT_DESTROYED();
    PhysicalSize old_intrinsic_size = IntrinsicSize();

    PhysicalSize new_intrinsic_size = PhysicalSize::FromSizeFRound(image_resource_->ImageSize(StyleRef().EffectiveZoom()));
    ...
    if (old_intrinsic_size != new_intrinsic_size) {
        ...
        if (NeedsLayoutOnIntrinsicSizeChange()) {
            SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kSizeChanged);
            return;
        }
    }
    ...
    if (defer == CanDeferInvalidation::kYes && ImageResource() && ImageResource()->MaybeAnimated())
        SetShouldDelayFullPaintInvalidation();
}

void LayoutImage::PaintReplaced(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
...
bool LayoutImage::ForegroundIsKnownToBeOpaqueInRect(const PhysicalRect& local_rect, unsigned) const
{
    NOT_DESTROYED();
    if (ChildPaintBlockedByDisplayLock())
        return false;
    ...
    if (background_clip == EFillBox::kBorder && StyleRef().HasBorder() && !StyleRef().BorderObscuresBackground())
        return false;
    // Background shows in padding area.
    if ((background_clip == EFillBox::kBorder || background_clip == EFillBox::kPadding) && StyleRef().MayHavePadding())
        return false;
    // Object-position may leave parts of the content box empty, regardless of the
    // value of object-fit.
    if (StyleRef().ObjectPosition() != ComputedStyleInitialValues::InitialObjectPosition())
        return false;
    ...
    DEVTOOLS_TIMELINE_TRACE_EVENT_WITH_CATEGORIES(
        TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "PaintImage", inspector_paint_image_event::Data, this, *image_content);
...
HTMLMapElement* LayoutImage::ImageMap() const
{
    ...
    return i ? i->GetTreeScope().GetImageMap(i->FastGetAttribute(html_names::kUsemapAttr)) : nullptr;
}

bool LayoutImage::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    HitTestResult temp_result(result);
    bool inside = LayoutReplaced::NodeAtPoint(temp_result, hit_test_location, accumulated_offset, phase);
...
bool LayoutImage::CanApplyObjectViewBox() const
{
    ...
    const IntrinsicSizingInfo info = image_resource_->GetNaturalDimensions(StyleRef().EffectiveZoom());
    return info.has_width && info.has_height;
}

void LayoutImage::ComputeIntrinsicSizingInfo(IntrinsicSizingInfo& intrinsic_sizing_info) const
{
    NOT_DESTROYED();
    DCHECK(!ShouldApplySizeContainment());
    if (EmbeddedSVGImage()) {
        intrinsic_sizing_info = image_resource_->GetNaturalDimensions(StyleRef().EffectiveZoom());
    ...
    if (image_resource_ && image_resource_->ErrorOccurred() && !IsA<LayoutVideo>(this)) {
...
bool LayoutImage::IsUnsizedImage() const
{
    ...
    bool has_aspect_ratio = style.AspectRatio().GetType() == EAspectRatioType::kRatio;
    const bool is_fixed_size = (explicit_width && explicit_height) || (has_aspect_ratio && (explicit_width || explicit_height));
    return !is_fixed_size;
}

void LayoutImage::MutableForPainting::UpdatePaintedRect(const PhysicalRect& paint_rect)
{
