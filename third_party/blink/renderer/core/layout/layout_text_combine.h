...
public:
    ...
    PhysicalOffset AdjustOffsetForHitTest(const PhysicalOffset& offset_in_container) const;

    // Map non-scaled |offset_in_container| to scaled offset if |this| uses
    // scale x, otherwise return |offset_in_container|.
    PhysicalOffset AdjustOffsetForLocalCaretRect(const PhysicalOffset& offset_in_container) const;
    ...
    PhysicalRect ComputeTextBoundsRectForHitTest(const FragmentItem& text_item, const PhysicalOffset& inline_root_offset) const;
...
// static
inline bool LayoutTextCombine::ShouldBeParentOf(const LayoutObject& layout_object)
{
    if (layout_object.IsHorizontalWritingMode() || !layout_object.IsText() || layout_object.IsSVGInlineText()) [[likely]] {
        return false;
    }
    if (layout_object.StyleRef().HasTextCombine() && layout_object.IsLayoutNGObject()) [[unlikely]] {
        return true;
    }
    return false;
}

template <> struct DowncastTraits<LayoutTextCombine> {
