...
{
    test::TaskEnvironment task_environment;
    Vector<Vector<PhysicalRect>> tests = { Vector<PhysicalRect> { PhysicalRect(-1, -1, 10, 10), PhysicalRect(10, 20, 30, 40), PhysicalRect(1, 2, 3, 4),
                                               PhysicalRect(1, -1, 10, 15), PhysicalRect(-31, -15, 11, 16) },
        Vector<PhysicalRect> { PhysicalRect(1, 2, 3, 4) }, Vector<PhysicalRect> { PhysicalRect(10, 20, 30, 40), PhysicalRect(15, 25, 35, 45) },
        Vector<PhysicalRect> { PhysicalRect(-100, -200, 30, 40), PhysicalRect(-150, -250, 35, 45) } };
...
TEST(OutlineRectCollectorTest, CombineWithOffset)
{
    ...
    std::unique_ptr<OutlineRectCollector> u_descendant = u.ForDescendantCollector();
    std::unique_ptr<OutlineRectCollector> v_descendant = v.ForDescendantCollector();
    ...
    EXPECT_EQ(vector_result, (Vector<PhysicalRect> { PhysicalRect(10, 20, 30, 40), PhysicalRect(25, -5, 30, 40) }));
}
...
TEST_F(OutlineRectCollectorRenderingTest, CombineWithAncestor)
{
    ...
    LayoutBoxModelObject* parent = DynamicTo<LayoutBoxModelObject>(GetLayoutObjectByElementId("parent"));
    ...
    EXPECT_EQ(vector_result, (Vector<PhysicalRect> { PhysicalRect(10, 20, 30, 40), PhysicalRect(40, 20, 30, 40) }));
}
