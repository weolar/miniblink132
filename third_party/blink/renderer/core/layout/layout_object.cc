...
LayoutObject* FindAncestorByPredicate(const LayoutObject* descendant, LayoutObject::AncestorSkipInfo* skip_info, Predicate predicate)
{
    for (auto* object = descendant->Parent(); object; object = object->Parent()) {
        ...
        if (object->IsColumnSpanAll()) [[unlikely]] {
            ...
            const auto* multicol_container = object->SpannerPlaceholder()->MultiColumnBlockFlow();
...
bool HasNativeBackgroundPainter(Node* node)
{
    ...
    return element_animations->CompositedBackgroundColorStatus() == ElementAnimations::CompositedPaintStatus::kComposited;
}
...
bool HasClipPathPaintWorklet(Node* node)
{
    ...
    return element_animations->CompositedClipPathStatus() == ElementAnimations::CompositedPaintStatus::kComposited;
}

StyleDifference AdjustForCompositableAnimationPaint(const ComputedStyle* old_style, const ComputedStyle* new_style, Node* node, StyleDifference diff)
{
    DCHECK(new_style);

    bool skip_background_color_paint_invalidation = !diff.BackgroundColorChanged() || HasNativeBackgroundPainter(node);
    if (!skip_background_color_paint_invalidation)
        diff.SetNeedsNormalPaintInvalidation();

    bool skip_clip_path_paint_invalidation = !diff.ClipPathChanged() || HasClipPathPaintWorklet(node);
...
void ApplyVisibleOverflowToClipRect(OverflowClipAxes overflow_clip, PhysicalRect& clip_rect)
{
...
AllowDestroyingLayoutObjectInFinalizerScope::AllowDestroyingLayoutObjectInFinalizerScope()
{
...
LayoutObject::SetLayoutNeededForbiddenScope::SetLayoutNeededForbiddenScope(LayoutObject& layout_object)
...
struct SameSizeAsLayoutObject : public GarbageCollected<SameSizeAsLayoutObject>, ImageResourceObserver, DisplayItemClient {
...
LayoutObject* LayoutObject::CreateObject(Element* element, const ComputedStyle& style)
{
    ...
    if (!element->IsPseudoElement() && ShouldUseContentDataForElement(content_data)) {
        ...
        if (const StyleImage* style_image = To<ImageContentData>(content_data)->GetImage()) {
            image->SetImageResource(MakeGarbageCollected<LayoutImageResourceStyleImage>(const_cast<StyleImage*>(style_image)));
        ...
        if (parent->GetComputedStyle()->MarkerShouldBeInside(*parent, style.GetDisplayStyle())) {
    ...
    case EDisplay::kWebkitInlineBox:
        if (!RuntimeEnabledFeatures::CSSLineClampWebkitBoxBlockificationEnabled() && style.IsDeprecatedWebkitBoxWithVerticalLineClamp()) {
            return MakeGarbageCollected<LayoutBlockFlow>(element);
        }
        UseCounter::Count(element->GetDocument(), WebFeature::kWebkitBoxWithoutWebkitLineClamp);
...
// static
LayoutBlockFlow* LayoutObject::CreateBlockFlowOrListItem(Element* element, const ComputedStyle& style)
{
    if (style.IsDisplayListItem() && element && element->GetPseudoId() != kPseudoIdBackdrop) {
...
LayoutObject::LayoutObject(Node* node)
    : paint_invalidation_reason_for_pre_paint_(static_cast<unsigned>(PaintInvalidationReason::kNone))
    , positioned_state_(kIsStaticallyPositioned)
    , selection_state_(static_cast<unsigned>(SelectionState::kNone))
    , selection_state_for_paint_(static_cast<unsigned>(SelectionState::kNone))
    , subtree_paint_property_update_reasons_(static_cast<unsigned>(SubtreePaintPropertyUpdateReason::kNone))
...
{
    for (LayoutObject* runner = this; runner && !runner->MayHaveAnchorQuery(); runner = runner->Parent()) {
        runner->SetSelfMayHaveAnchorQuery();
    }
}

void LayoutObject::SetIsInsideFlowThreadIncludingDescendants(bool inside_flow_thread)
{
    NOT_DESTROYED();
    LayoutObject* next;
...
bool LayoutObject::RequiresAnonymousTableWrappers(const LayoutObject* new_child) const
{
    NOT_DESTROYED();
    ...
    if (new_child->IsLayoutTableCol()) {
        bool is_column_in_column_group = new_child->StyleRef().Display() == EDisplay::kTableColumn && IsLayoutTableCol();
...
DISABLE_CFI_PERF
void LayoutObject::AddChild(LayoutObject* new_child, LayoutObject* before_child)
{
    NOT_DESTROYED();
#if DCHECK_IS_ON()
    DCHECK(IsAllowedToModifyLayoutTreeStructure(GetDocument()) || IsInDetachedNonDomTree());
#endif
    ...
    if (RequiresAnonymousTableWrappers(new_child)) {
        ...
        LayoutObject* after_child = before_child ? before_child->PreviousSibling() : children->LastChild();
        if (after_child && after_child->IsAnonymous() && after_child->IsTable() && !after_child->IsBeforeContent()) {
            table = after_child;
        } else {
    ...
    } else if (new_child->IsHorizontalWritingMode() || !new_child->IsText()) [[likely]] {
        ...
        children->InsertChildNode(this, new_child, before_child);
    } else if (!IsHorizontalTypographicMode() && LayoutTextCombine::ShouldBeParentOf(*new_child)) {
        if (before_child) {
            if (IsA<LayoutTextCombine>(before_child)) {
                DCHECK(!DynamicTo<LayoutTextCombine>(before_child->PreviousSibling())) << before_child->PreviousSibling();
                before_child->AddChild(new_child, before_child->SlowFirstChild());
            } else if (auto* const previous_sibling = DynamicTo<LayoutTextCombine>(before_child->PreviousSibling())) {
                previous_sibling->AddChild(new_child);
            } else {
                children->InsertChildNode(this, LayoutTextCombine::CreateAnonymous(To<LayoutText>(new_child)), before_child);
            }
        } else if (auto* const last_child = DynamicTo<LayoutTextCombine>(SlowLastChild())) {
            last_child->AddChild(new_child);
        } else {
            children->AppendChildNode(this, LayoutTextCombine::CreateAnonymous(To<LayoutText>(new_child)));
        }
    } else {
...
void LayoutObject::NotifyPriorityScrollAnchorStatusChanged()
{
    ...
    if (!Parent())
        return;
    for (auto* layer = Parent()->EnclosingLayer(); layer; layer = layer->Parent()) {
...
{
    NOT_DESTROYED();
    if (!bitfields_.SubtreeChangeListenerRegistered() || bitfields_.NotifiedOfSubtreeChange()) {
        return false;
    }
...
bool LayoutObject::HasClipRelatedProperty() const
{
    ...
    if (ShouldApplyPaintContainment() || (ShouldApplyStyleContainment() && ShouldApplyLayoutContainment())) {
        return true;
    }
...
bool LayoutObject::IsRenderedLegendInternal() const
{
    ...
    return parent_layout_block && IsA<HTMLFieldSetElement>(parent->GetNode()) && LayoutFieldset::FindInFlowLegend(*parent_layout_block) == this;
}
...
LayoutObject* LayoutObject::GetScrollMarkerGroup() const
{
    ...
    if (IsFieldset()) {
        const LayoutBlock* fieldset_content = To<LayoutFieldset>(this)->FindAnonymousFieldsetContentBox();
    ...
    if (auto* element = DynamicTo<Element>(GetNode())) {
        if (PseudoElement* pseudo = element->GetPseudoElement(kPseudoIdScrollMarkerGroupBefore)) {
            return pseudo->GetLayoutObject();
        }
        if (PseudoElement* pseudo = element->GetPseudoElement(kPseudoIdScrollMarkerGroupAfter)) {
            return pseudo->GetLayoutObject();
        }
    }
    return nullptr;
...
bool LayoutObject::IsListMarkerForSummary() const
{
    ...
    if (const auto* summary = DynamicTo<HTMLSummaryElement>(Parent()->GetNode())) {
        if (!summary->IsMainSummary())
            return false;
        if (ListMarker::GetListStyleCategory(GetDocument(), StyleRef()) != ListMarker::ListStyleCategory::kSymbol)
            return false;
        const AtomicString& name = StyleRef().ListStyleType()->GetCounterStyleName();
        return name == keywords::kDisclosureOpen || name == keywords::kDisclosureClosed;
    }
    return false;
}
...
LayoutObject* LayoutObject::NextInPreOrder(const LayoutObject* stay_within) const
{
    NOT_DESTROYED();
    if (LayoutObject* o = SlowFirstChild())
        return o;
...
LayoutObject* LayoutObject::PreviousInPostOrder(const LayoutObject* stay_within) const
{
    NOT_DESTROYED();
...
LayoutObject* LayoutObject::NextInPreOrderAfterChildren(const LayoutObject* stay_within) const
{
    NOT_DESTROYED();
    if (this == stay_within)
        return nullptr;
...
LayoutObject* LayoutObject::PreviousInPostOrderBeforeChildren(const LayoutObject* stay_within) const
{
    NOT_DESTROYED();
    if (this == stay_within)
        return nullptr;
...
LayoutObject* LayoutObject::PreviousInPreOrder(const LayoutObject* stay_within) const
{
    NOT_DESTROYED();
    if (this == stay_within)
        return nullptr;
...
LayoutObject* LayoutObject::CommonAncestor(const LayoutObject& other, CommonAncestorData* data) const
{
    if (this == &other)
...
bool LayoutObject::IsBeforeInPreOrder(const LayoutObject& other) const
{
    ...
    for (const LayoutObject* child = common_ancestor->SlowFirstChild(); child; child = child->NextSibling()) {
        if (child == data.last)
            return true;
...
static void AddLayers(LayoutObject* obj, PaintLayer* parent_layer, LayoutObject*& new_object, PaintLayer*& before_child)
{
    if (obj->HasLayer()) {
        if (!before_child && new_object) {
            ...
            before_child = new_object->Parent()->FindNextLayer(parent_layer, new_object);
            new_object = nullptr;
        }
        parent_layer->AddChild(To<LayoutBoxModelObject>(obj)->Layer(), before_child);
        return;
    }
...
PaintLayer* LayoutObject::FindNextLayer(PaintLayer* parent_layer, LayoutObject* start_point, bool check_parent)
{
    NOT_DESTROYED();
    ...
    // layer.
    PaintLayer* our_layer = HasLayer() ? To<LayoutBoxModelObject>(this)->Layer() : nullptr;
    ...
    if (!our_layer || our_layer == parent_layer) {
        for (LayoutObject* curr = start_point ? start_point->NextSibling() : SlowFirstChild(); curr; curr = curr->NextSibling()) {
            PaintLayer* next_layer = curr->FindNextLayer(parent_layer, nullptr, false);
            if (next_layer)
                return next_layer;
        }
    }
...
PaintLayer* LayoutObject::PaintingLayer(int max_depth) const
{
    ...
    for (const LayoutObject* current = this; current; outermost = current, current = FindContainer(*current)) {
        if (max_depth != -1 && ++depth > max_depth) {
            return nullptr;
        }
        if (current->HasLayer() && To<LayoutBoxModelObject>(current)->Layer()->IsSelfPaintingLayer())
            return To<LayoutBoxModelObject>(current)->Layer();
    }
    ...
    if (const auto* box = DynamicTo<LayoutBox>(outermost)) {
        if (box->PhysicalFragmentCount()) {
            ...
            if (fragment.GetBoxType() == PhysicalFragment::kPageContainer || fragment.GetBoxType() == PhysicalFragment::kPageBorderBox
...
LayoutBlockFlow* LayoutObject::FragmentItemsContainer() const
{
    ...
    for (const LayoutObject* walker = Parent(); walker != block_flow; walker = walker->Parent())
...
LayoutBlock* LayoutObject::ContainingFragmentationContextRoot() const
{
    ...
    for (LayoutBlock* ancestor = ContainingBlock(); ancestor; ancestor = ancestor->ContainingBlock()) {
...
LayoutFlowThread* LayoutObject::LocateFlowThreadContainingBlock() const
{
    ...
    return LayoutFlowThread::LocateFlowThreadContainingBlockOf(*this, LayoutFlowThread::kAnyAncestor);
}
...
static inline bool ObjectIsRelayoutBoundary(const LayoutObject* object)
{
    ...
    if (box->IsOutOfFlowPositioned() && (style->HasAutoLeftAndRightIgnoringPositionArea() || style->HasAutoTopAndBottomIgnoringPositionArea())) {
        return false;
    }
    ...
    if (box->ShouldApplyLayoutContainment() && box->ShouldApplySizeContainment()) {
        return true;
    }
...
void LayoutObject::SetNeedsCollectInlines()
{
    ...
    if (IsSVGChild() && !IsSVGText() && !IsSVGInline() && !IsSVGInlineText() && !IsSVGForeignObject()) [[unlikely]] {
        return;
    }
...
void LayoutObject::MarkContainerChainForLayout(bool schedule_relayout)
{
    ...
    while (object) {
        ...
        if (!last->SelfNeedsFullLayout() && last->ChildLayoutBlockedByDisplayLock() && !HasPropagatedLayoutObjects(last)) {
            return;
        }
...
{
    NOT_DESTROYED();
    return StyleRef().GetFont().IsFallbackValid() && FirstLineStyle()->GetFont().IsFallbackValid();
}
...
void LayoutObject::InvalidateSubtreeLayoutForFontUpdates()
{
    ...
    if (!IsFontFallbackValid()) {
        SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kFontsChanged);
    }
...
static inline bool ShouldInvalidateBeyond(LayoutObject* o)
{
    ...
    if (o->IsLayoutGrid()) {
        ...
        if (style.GridTemplateColumns().IsSubgriddedAxis() || style.GridTemplateRows().IsSubgriddedAxis()) {
            return true;
        }
    }
...
inline void LayoutObject::InvalidateContainerIntrinsicLogicalWidths()
{
    ...
    while (o && (!o->IntrinsicLogicalWidthsDirty() || ShouldInvalidateBeyond(o))) {
...
LayoutObject* LayoutObject::ContainerForAbsolutePosition(AncestorSkipInfo* skip_info) const
{
    NOT_DESTROYED();
    return FindAncestorByPredicate(this, skip_info, [](LayoutObject* candidate) { return candidate->CanContainAbsolutePositionObjects(); });
}

LayoutObject* LayoutObject::ContainerForFixedPosition(AncestorSkipInfo* skip_info) const
{
    NOT_DESTROYED();
    DCHECK(!IsText());
    return FindAncestorByPredicate(this, skip_info, [](LayoutObject* candidate) { return candidate->CanContainFixedPositionObjects(); });
}

LayoutBlock* LayoutObject::ContainingBlockForAbsolutePosition(AncestorSkipInfo* skip_info) const
{
    NOT_DESTROYED();
...
LayoutBlock* LayoutObject::ContainingBlockForFixedPosition(AncestorSkipInfo* skip_info) const
{
    NOT_DESTROYED();
...
LayoutBlock* LayoutObject::InclusiveContainingBlock(AncestorSkipInfo* skip_info)
{
    NOT_DESTROYED();
...
const PaintLayer* LayoutObject::ContainingScrollContainerLayer(bool ignore_layout_view_for_fixed_pos) const
{
    NOT_DESTROYED();
    ...
    if (auto* scroll_container_layer = layer->ContainingScrollContainerLayer(&is_fixed_to_view)) {
...
const LayoutBox* LayoutObject::ContainingScrollContainer(bool ignore_layout_view_for_fixed_pos) const
{
    NOT_DESTROYED();
    if (const PaintLayer* scroll_container_layer = ContainingScrollContainerLayer(ignore_layout_view_for_fixed_pos)) {
...
bool LayoutObject::ComputeIsFixedContainer(const ComputedStyle* style) const
{
    ...
    if (IsAnonymous() && Parent() && Parent()->IsFieldset() && Parent()->CanContainFixedPositionObjects()) {
        return true;
    }
    ...
    if (style->HasTransformRelatedProperty() || style->TransformStyle3D() == ETransformStyle3D::kPreserve3d) {
        if (!IsInline() || IsAtomicInlineLevel())
            return true;
    }
    // https://www.w3.org/TR/css-contain-1/#containment-layout
    if (IsEligibleForPaintOrLayoutContainment()
        && (ShouldApplyPaintContainment(*style) || ShouldApplyLayoutContainment(*style) || style->WillChangeProperties().Contains(CSSPropertyID::kContain)))
        return true;
...
bool LayoutObject::ComputeIsAbsoluteContainer(const ComputedStyle* style) const
{
    ...
    if (!style)
        return false;
    return style->CanContainAbsolutePositionObjects() || ComputeIsFixedContainer(style) ||
        // crbug.com/1153042: If <fieldset> is an absolute container, its
        // anonymous content box should be an absolute container.
        (IsAnonymous() && Parent() && Parent()->IsFieldset() && Parent()->StyleRef().CanContainAbsolutePositionObjects());
}

const LayoutBoxModelObject* LayoutObject::FindFirstStickyContainer(const LayoutBox* below) const
{
    ...
    while (maybe_sticky_ancestor && maybe_sticky_ancestor != below) {
        ...
        maybe_sticky_ancestor
            = maybe_sticky_ancestor->IsLayoutInline() ? maybe_sticky_ancestor->Container() : To<LayoutBox>(maybe_sticky_ancestor)->LocationContainer();
    }
    return nullptr;
}

gfx::RectF LayoutObject::AbsoluteBoundingBoxRectF(MapCoordinatesFlags flags) const
{
    NOT_DESTROYED();
...
gfx::Rect LayoutObject::AbsoluteBoundingBoxRect(MapCoordinatesFlags flags) const
{
    NOT_DESTROYED();
    DCHECK(!(flags & kIgnoreTransforms));
    Vector<gfx::QuadF> quads;
...
PhysicalRect LayoutObject::AbsoluteBoundingBoxRectHandlingEmptyInline(MapCoordinatesFlags flags) const
{
    NOT_DESTROYED();
...
PhysicalRect LayoutObject::AbsoluteBoundingBoxRectForScrollIntoView() const
{
    ...
    const MapCoordinatesFlags flag = (RuntimeEnabledFeatures::CSSPositionStickyStaticScrollPositionEnabled()) ? kIgnoreStickyOffset : 0;

    if (const auto* scroll_marker = DynamicTo<ScrollMarkerPseudoElement>(GetNode())) {
        // Scroll markers are reparented into a scroll marker group. We want the
        // rectangle of the originating element (or column).
        const Element* originating_element = scroll_marker->UltimateOriginatingElement();
        const auto* originating_object = originating_element->GetLayoutObject();
        const auto* column_pseudo = DynamicTo<ColumnPseudoElement>(scroll_marker->parentNode());
        if (!column_pseudo) {
        ...
        bounds.offset -= PhysicalOffset::FromVector2dFRound(scroller->GetScrollableArea()->GetScrollOffset());
...
RecalcScrollableOverflowResult LayoutObject::RecalcScrollableOverflow()
{
    ...
    for (LayoutObject* current = SlowFirstChild(); current; current = current->NextSibling()) {
        children_scrollable_overflow_changed |= current->RecalcScrollableOverflow().scrollable_overflow_changed;
    }
...
void LayoutObject::RecalcVisualOverflow()
{
    NOT_DESTROYED();
    for (LayoutObject* current = SlowFirstChild(); current; current = current->NextSibling()) {
        if (current->HasLayer() && To<LayoutBoxModelObject>(current)->HasSelfPaintingLayer())
            continue;
...
{
    NOT_DESTROYED();
    if (IsOutOfFlowPositioned() || (HasLayer() && To<LayoutBoxModelObject>(this)->HasSelfPaintingLayer()))
        return;
...
{
    if (!IsInLayoutNGInlineFormattingContext() && !IsLayoutNGObject() && !IsLayoutBlock() && !NeedsLayout()) {
        ...
        SetNeedsLayoutAndIntrinsicWidthsRecalc(layout_invalidation_reason::kStyleChange);
    } else {
...
bool LayoutObject::HasDistortingVisualEffects() const
{
    ...
    for (const auto* effect = &paint_properties.Effect().Unalias(); effect; effect = effect->UnaliasedParent()) {
    ...
    gfx::Transform projection = GeometryMapper::SourceToDestinationProjection(paint_properties.Transform(), root_properties.Transform());
...
{
    NOT_DESTROYED();
    const FragmentData& fragment = EnclosingLayer()->GetLayoutObject().FirstFragment();
...
DISABLE_CFI_PERF
void LayoutObject::InvalidatePaint(const PaintInvalidatorContext& context) const
{
    NOT_DESTROYED();
...
bool LayoutObject::MapToVisualRectInAncestorSpaceInternalFastPath(
    const LayoutBoxModelObject* ancestor, gfx::RectF& rect, VisualRectFlags visual_rect_flags, bool& intersects) const
{
    NOT_DESTROYED();
    intersects = true;
    if (!(visual_rect_flags & kUseGeometryMapper) || !ancestor || !ancestor->FirstFragment().HasLocalBorderBoxProperties())
        return false;
    ...
    const LayoutObject* property_container = GetPropertyContainer(&skip_info, &container_properties, visual_rect_flags);
    ...
    if (property_container != ancestor) {
        ...
        intersects = GeometryMapper::LocalToAncestorVisualRect(
            container_properties, ancestor->FirstFragment().ContentsProperties(), clip_rect, kIgnoreOverlayScrollbarSize, visual_rect_flags);
        rect = clip_rect.Rect();
    }
...
bool LayoutObject::MapToVisualRectInAncestorSpace(const LayoutBoxModelObject* ancestor, PhysicalRect& rect, VisualRectFlags visual_rect_flags) const
{
    NOT_DESTROYED();
    gfx::RectF float_rect(rect);

    bool intersects = true;
    if (MapToVisualRectInAncestorSpaceInternalFastPath(ancestor, float_rect, visual_rect_flags, intersects)) {
        rect = PhysicalRect::EnclosingRect(float_rect);
        return intersects;
    }
    TransformState transform_state(TransformState::kApplyTransformDirection, gfx::QuadF(float_rect));
    intersects = MapToVisualRectInAncestorSpaceInternal(ancestor, transform_state, visual_rect_flags);
    transform_state.Flatten();
    rect = PhysicalRect::EnclosingRect(transform_state.LastPlanarQuad().BoundingBox());
    return intersects;
}

bool LayoutObject::MapToVisualRectInAncestorSpace(const LayoutBoxModelObject* ancestor, gfx::RectF& rect, VisualRectFlags visual_rect_flags) const
{
    NOT_DESTROYED();
    bool intersects = true;
    if (MapToVisualRectInAncestorSpaceInternalFastPath(ancestor, rect, visual_rect_flags, intersects)) {
        return intersects;
    }

    TransformState transform_state(TransformState::kApplyTransformDirection, gfx::QuadF(rect));
    intersects = MapToVisualRectInAncestorSpaceInternal(ancestor, transform_state, visual_rect_flags);
    transform_state.Flatten();
...
bool LayoutObject::MapToVisualRectInAncestorSpaceInternal(
    const LayoutBoxModelObject* ancestor, TransformState& transform_state, VisualRectFlags visual_rect_flags) const
{
    NOT_DESTROYED();
    ...
    if (LayoutObject* parent = Parent()) {
        if (parent->IsBox()) {
            ...
            TransformState::TransformAccumulation accumulation = preserve3d ? TransformState::kAccumulateTransform : TransformState::kFlattenTransform;

            if (parent != ancestor && !To<LayoutBox>(parent)->MapContentsRectToBoxSpace(transform_state, accumulation, *this, visual_rect_flags))
                return false;
        }
        return parent->MapToVisualRectInAncestorSpaceInternal(ancestor, transform_state, visual_rect_flags);
    }
    return true;
}

const LayoutObject* LayoutObject::GetPropertyContainer(
    AncestorSkipInfo* skip_info, PropertyTreeStateOrAlias* container_properties, VisualRectFlags visual_rect_flags) const
{
    NOT_DESTROYED();
    ...
    while (!property_container->FirstFragment().HasLocalBorderBoxProperties()) {
        ...
        if (!property_container || (skip_info && skip_info->AncestorSkipped()) || property_container->IsFragmented()) {
            return nullptr;
        }
    }
        ...
        if (property_container == this) {
            *container_properties = FirstFragment().LocalBorderBoxProperties();
            ...
            if (visual_rect_flags & kIgnoreLocalClipPath) {
                if (auto* properties = property_container->FirstFragment().PaintProperties()) {
            ...
            *container_properties = property_container->FirstFragment().ContentsProperties();
        }
    }
...
HitTestResult LayoutObject::HitTestForOcclusion(const PhysicalRect& hit_rect) const
{
    NOT_DESTROYED();
    LocalFrame* frame = GetDocument().GetFrame();
    DCHECK(!frame->View()->NeedsLayout());
    HitTestRequest::HitTestRequestType hit_type = HitTestRequest::kIgnorePointerEventsNone | HitTestRequest::kReadOnly | HitTestRequest::kIgnoreClipping
        | HitTestRequest::kIgnoreZeroOpacityObjects | HitTestRequest::kHitTestVisualOverflow;
...
void LayoutObject::DumpLayoutObject(StringBuilder& string_builder, bool dump_address, unsigned show_tree_character_offset) const
{
    ...
    if (IsText() && To<LayoutText>(this)->IsTextFragment()) {
        string_builder.AppendFormat(" \"%s\" ", To<LayoutText>(this)->TransformedText().Ascii().c_str());
    }
...
void LayoutObject::DumpLayoutTreeAndMark(StringBuilder& string_builder, const LayoutObject* marked_object1, const char* marked_label1,
    const LayoutObject* marked_object2, const char* marked_label2, unsigned depth) const
{
    NOT_DESTROYED();
...
const ComputedStyle& LayoutObject::SlowEffectiveStyle(StyleVariant style_variant) const
{
    NOT_DESTROYED();
...
static inline void HandleDynamicFloatPositionChange(LayoutObject* object)
{
    ...
    if (object->IsInline() != object->Parent()->ChildrenInline()) {
        if (!object->IsInline()) {
            ...
            LayoutBlock* block = To<LayoutBlock>(object->Parent())->CreateAnonymousBlock();
...
StyleDifference LayoutObject::AdjustStyleDifference(StyleDifference diff) const
{
    ...
    if (diff.TextDecorationOrColorChanged() && !diff.NeedsNormalPaintInvalidation() && !diff.NeedsSimplePaintInvalidation()) {
        if (StyleRef().HasOutlineWithCurrentColor() || StyleRef().HasBackgroundRelatedColorReferencingCurrentColor() ||
            ...
            (IsText() && !IsBR() && To<LayoutText>(this)->HasInlineFragments()) || (IsSVG() && StyleRef().IsFillColorCurrentColor())
    ...
    if (!diff.NeedsNormalPaintInvalidation() && IsLayoutView() && Style() && !Style()->GetFont().IsFallbackValid()) {
        diff.SetNeedsNormalPaintInvalidation();
    }
    ...
    if (!diff.NeedsFullLayout() && Style() && IsBoxModelObject()) {
        bool requires_layer = To<LayoutBoxModelObject>(this)->LayerTypeRequired() != kNoPaintLayer;
...
void LayoutObject::SetPseudoElementStyle(const LayoutObject& owner, bool match_parent_size)
{
    NOT_DESTROYED();
    const ComputedStyle* pseudo_style = owner.Style();
    DCHECK(pseudo_style->StyleType() == kPseudoIdCheck || pseudo_style->StyleType() == kPseudoIdBefore || pseudo_style->StyleType() == kPseudoIdAfter
        || pseudo_style->StyleType() == kPseudoIdSelectArrow || pseudo_style->StyleType() == kPseudoIdMarker
        || pseudo_style->StyleType() == kPseudoIdFirstLetter || pseudo_style->StyleType() == kPseudoIdScrollMarkerGroup || pseudo_style->IsPageMarginBox()
        || pseudo_style->StyleType() == kPseudoIdScrollMarker || pseudo_style->StyleType() == kPseudoIdColumnScrollMarker
        || pseudo_style->StyleType() == kPseudoIdScrollNextButton || pseudo_style->StyleType() == kPseudoIdScrollPrevButton);
    ...
    //
    if (IsImage() || IsQuote()) {
        ComputedStyleBuilder builder = GetDocument().GetStyleResolver().CreateComputedStyleBuilderInheritingFrom(*pseudo_style);
    ...
    if (IsText() && Parent() && Parent()->IsInitialLetterBox()) [[unlikely]] {
        ...
        const ComputedStyle* initial_letter_text_style
            = GetDocument().GetStyleResolver().StyleForInitialLetterText(*pseudo_style, Parent()->ContainingBlock()->StyleRef());
    ...
    if (IsText() && IsA<LayoutTextCombine>(Parent())) [[unlikely]] {
        ...
        ComputedStyleBuilder combined_text_style_builder = GetDocument().GetStyleResolver().CreateComputedStyleBuilderInheritingFrom(*pseudo_style);
...
DISABLE_CFI_PERF
void LayoutObject::SetStyle(const ComputedStyle* style, ApplyStyleChanges apply_changes)
{
    NOT_DESTROYED();
    if (style_ == style)
        return;
    ...
    if (style_) {
        ...
        if (const auto* cached_inherited_first_line_style = style_->GetCachedPseudoElementStyle(kPseudoIdFirstLineInherited)) {
            ...
            diff.Merge(cached_inherited_first_line_style->VisualInvalidationDiff(GetDocument(), *style));
        }

        auto HighlightPseudoUpdateDiff
            = [this, style, &diff](const PseudoId pseudo, const ComputedStyle* pseudo_old_style, const ComputedStyle* pseudo_new_style) {
                  DCHECK(pseudo == kPseudoIdSearchText || pseudo == kPseudoIdTargetText || pseudo == kPseudoIdSpellingError || pseudo == kPseudoIdGrammarError);

                  if (style_->HasPseudoElementStyle(pseudo) || style->HasPseudoElementStyle(pseudo)) {
                      if (pseudo_old_style && pseudo_new_style) {
                          diff.Merge(pseudo_old_style->VisualInvalidationDiff(GetDocument(), *pseudo_new_style));
                      } else {
                          diff.SetNeedsNormalPaintInvalidation();
                      }
                  }
        ...
        if (RuntimeEnabledFeatures::SearchTextHighlightPseudoEnabled() && UsesHighlightPseudoInheritance(kPseudoIdSearchText)) {
            HighlightPseudoUpdateDiff(kPseudoIdSearchText, style_->HighlightData().SearchTextCurrent(), style->HighlightData().SearchTextCurrent());
            HighlightPseudoUpdateDiff(kPseudoIdSearchText, style_->HighlightData().SearchTextNotCurrent(), style->HighlightData().SearchTextNotCurrent());
        }
        if (UsesHighlightPseudoInheritance(kPseudoIdTargetText)) {
            HighlightPseudoUpdateDiff(kPseudoIdTargetText, style_->HighlightData().TargetText(), style->HighlightData().TargetText());
        }
        if (UsesHighlightPseudoInheritance(kPseudoIdSpellingError)) {
            HighlightPseudoUpdateDiff(kPseudoIdSpellingError, style_->HighlightData().SpellingError(), style->HighlightData().SpellingError());
        }
        if (UsesHighlightPseudoInheritance(kPseudoIdGrammarError)) {
            HighlightPseudoUpdateDiff(kPseudoIdGrammarError, style_->HighlightData().GrammarError(), style->HighlightData().GrammarError());
        }
    }
    ...
    if (!diff.NeedsFullLayout()) {
        if (updated_diff.NeedsFullLayout()) {
            SetNeedsLayoutAndIntrinsicWidthsRecalc(layout_invalidation_reason::kStyleChange);
        } else if (updated_diff.NeedsPositionedMovementLayout() || StyleRef().HasAnchorFunctionsWithoutEvaluator()) {
    ...
    if (diff.NeedsNormalPaintInvalidation() || updated_diff.NeedsNormalPaintInvalidation()) {
        if (IsSVGRoot()) {
            ...
            SetShouldDoFullPaintInvalidationWithoutLayoutChange(PaintInvalidationReason::kStyle);
        }
    }

    // Clip Path animations need a property update when they're composited, as it
    // changes between mask based and path based clip.
    if (old_style && diff.NeedsNormalPaintInvalidation() && diff.ClipPathChanged()) {
        SetNeedsPaintPropertyUpdate();
        PaintingLayer()->SetNeedsCompositingInputsUpdate();
    }

    if (!IsLayoutNGObject() && old_style && old_style->Visibility() != style_->Visibility()) {
        SetShouldDoFullPaintInvalidation();
    }
    ...
    if (!IsText() && !HasLayer()
        && (diff.TransformChanged() || diff.OpacityChanged() || diff.ZIndexChanged() || diff.FilterChanged() || diff.CssClipChanged() || diff.BlendModeChanged()
            || diff.MaskChanged() || diff.CompositingReasonsChanged())) {
        SetNeedsPaintPropertyUpdate();
    }
...
void LayoutObject::UpdateFirstLineImageObservers(const ComputedStyle* new_style)
{
    NOT_DESTROYED();
    bool has_new_first_line_style = new_style && new_style->HasPseudoElementStyle(kPseudoIdFirstLine) && BehavesLikeBlockContainer();
    DCHECK(!has_new_first_line_style || new_style == Style());

    if (!bitfields_.RegisteredAsFirstLineImageObserver() && !has_new_first_line_style)
        return;

    using FirstLineStyleMap = HeapHashMap<WeakMember<const LayoutObject>, Member<const ComputedStyle>>;
    DEFINE_STATIC_LOCAL(Persistent<FirstLineStyleMap>, first_line_style_map, (MakeGarbageCollected<FirstLineStyleMap>()));
    DCHECK_EQ(bitfields_.RegisteredAsFirstLineImageObserver(), first_line_style_map->Contains(this));
    const auto* old_first_line_style = bitfields_.RegisteredAsFirstLineImageObserver() ? first_line_style_map->at(this) : nullptr;
    ...
    const ComputedStyle* new_first_line_style = has_new_first_line_style ? FirstLineStyleWithoutFallback() : nullptr;
    ...
    if (old_first_line_style || new_first_line_style) {
        UpdateFillImages(old_first_line_style ? &old_first_line_style->BackgroundLayers() : nullptr,
            new_first_line_style ? &new_first_line_style->BackgroundLayers() : nullptr);
        if (new_first_line_style) {
            ...
            DCHECK(FillLayer::ImagesIdentical(&new_first_line_style->BackgroundLayers(), &FirstLineStyleWithoutFallback()->BackgroundLayers()));
...
void LayoutObject::StyleWillChange(StyleDifference diff, const ComputedStyle& new_style)
{
    NOT_DESTROYED();
    if (style_) {
        ...
        if (visibility_changed || style_->EffectiveZIndex() != new_style.EffectiveZIndex() || IsStackingContext(*style_) != IsStackingContext(new_style)) {
            GetDocument().SetDraggableRegionsDirty(true);
        }

        bool background_color_changed = ResolveColorFast(GetCSSPropertyBackgroundColor()) != ResolveColorFast(new_style, GetCSSPropertyBackgroundColor());

        if (diff.TextDecorationOrColorChanged() || background_color_changed || style_->GetFontDescription() != new_style.GetFontDescription()
            || style_->GetWritingDirection() != new_style.GetWritingDirection() || style_->InsideLink() != new_style.InsideLink()
            || style_->VerticalAlign() != new_style.VerticalAlign() || style_->GetTextAlign() != new_style.GetTextAlign()
        ...
        if (visibility_changed) {
            ...
            GetDocument().GetFrame()->GetInputMethodController().DidChangeVisibility(*this);
        }

        affects_parent_block_ = IsFloatingOrOutOfFlowPositioned()
            && ((!new_style.IsFloating() || new_style.IsInsideDisplayIgnoringFloatingChildren()) && !new_style.HasOutOfFlowPosition()) && Parent()
    ...
    if (GetNode() && !GetNode()->IsTextNode() && (old_touch_action == TouchAction::kAuto) != (new_touch_action == TouchAction::kAuto)) {
        EventHandlerRegistry& registry = GetDocument().GetFrame()->GetEventHandlerRegistry();
        if (new_touch_action != TouchAction::kAuto) {
            registry.DidAddEventHandler(*GetNode(), EventHandlerRegistry::kTouchAction);
        } else {
            registry.DidRemoveEventHandler(*GetNode(), EventHandlerRegistry::kTouchAction);
        }
        MarkEffectiveAllowedTouchActionChanged();
    }
    if (is_document_element && style_ && style_->Opacity() == 0.0f && new_style.Opacity() != 0.0f) {
        if (LocalFrameView* frame_view = GetFrameView())
...
void LayoutObject::SetScrollAnchorDisablingStyleChangedOnAncestor()
{
    ...
    while (object) {
        ...
        if (block && (block->IsScrollContainer() || block->GetNode() == viewport_defining_element)) {
...
bool LayoutObject::BelongsToElementChangingOverflowBehaviour() const
{
    ...
    return IsA<HTMLVideoElement>(element) || IsA<HTMLCanvasElement>(element) || IsA<HTMLImageElement>(element);
}

void LayoutObject::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    if (HasHiddenBackface()) {
        if (Parent() && Parent()->StyleRef().UsedTransformStyle3D() == ETransformStyle3D::kPreserve3d) {
            UseCounter::Count(GetDocument(), WebFeature::kHiddenBackfaceWithPossible3D);
            UseCounter::Count(GetDocument(), WebFeature::kHiddenBackfaceWith3D);
            UseCounter::Count(GetDocument(), WebFeature::kHiddenBackfaceWithPreserve3D);
    ...
    if (ShouldApplyStrictContainment() && style_->IsContentVisibilityVisible()) {
        if (ShouldApplyStyleContainment()) {
            UseCounter::Count(GetDocument(), WebFeature::kCSSContainAllWithoutContentVisibility);
        }
        UseCounter::Count(GetDocument(), WebFeature::kCSSContainStrictWithoutContentVisibility);
    }
    ...
    if (BelongsToElementChangingOverflowBehaviour()) {
        if ((StyleRef().HasExplicitOverflowXVisible() && StyleRef().OverflowX() == EOverflow::kVisible)
            || (StyleRef().HasExplicitOverflowYVisible() && StyleRef().OverflowY() == EOverflow::kVisible)) {
            UseCounter::Count(GetDocument(), WebFeature::kExplicitOverflowVisibleOnReplacedElement);

            Deprecation::CountDeprecation(GetDocument().GetExecutionContext(), WebFeature::kExplicitOverflowVisibleOnReplacedElement);
            if (!StyleRef().ObjectPropertiesPreventReplacedOverflow()) {
                UseCounter::Count(GetDocument(), WebFeature::kExplicitOverflowVisibleOnReplacedElementWithObjectProp);
            }
        }
    }
    ...
    if (diff.NeedsFullLayout()) {
        ...
        if (old_style->HasOutOfFlowPosition() != style_->HasOutOfFlowPosition()) {
            ...
            if (old_style->HasOutOfFlowPosition()) {
                if (auto* box = DynamicTo<LayoutBox>(this)) {
                    box->NotifyContainingDisplayLocksForAnchorPositioning(box->DisplayLocksAffectedByAnchors(), nullptr);
                }
            }
    ...
    if (diff.NeedsNormalPaintInvalidation() && old_style) {
        if (ResolveColor(*old_style, GetCSSPropertyBackgroundColor()) != ResolveColor(GetCSSPropertyBackgroundColor())
    ...
    if (old_style && old_style->UsedTransformStyle3D() != StyleRef().UsedTransformStyle3D()) {
        // Change of transform-style may affect descendant transform property nodes.
        AddSubtreePaintPropertyUpdateReason(SubtreePaintPropertyUpdateReason::kTransformStyleChanged);
    }
    ...
    if (RuntimeEnabledFeatures::HitTestOpaquenessEnabled() && old_style && old_style->UsedPointerEvents() != StyleRef().UsedPointerEvents()) {
...
void LayoutObject::ApplyPseudoElementStyleChanges(const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ApplyFirstLineChanges(old_style);

    if ((old_style && old_style->HasPseudoElementStyle(kPseudoIdSelection)) || StyleRef().HasPseudoElementStyle(kPseudoIdSelection))
...
{
    NOT_DESTROYED();
    bool has_old_first_line_style = old_style && old_style->HasPseudoElementStyle(kPseudoIdFirstLine);
    bool has_new_first_line_style = StyleRef().HasPseudoElementStyle(kPseudoIdFirstLine);
    ...
    if (Parent() && has_old_first_line_style && has_new_first_line_style) {
        if (const auto* old_first_line_style = old_style->GetCachedPseudoElementStyle(kPseudoIdFirstLine)) {
            if (const auto* new_first_line_style = FirstLineStyleWithoutFallback()) {
                diff = old_first_line_style->VisualInvalidationDiff(GetDocument(), *new_first_line_style);
                diff = AdjustForCompositableAnimationPaint(old_first_line_style, new_first_line_style, GetNode(), diff);
                has_diff = true;
            }
        }
    }
    ...
    if (BehavesLikeBlockContainer() && (diff.NeedsNormalPaintInvalidation() || diff.TextDecorationOrColorChanged())) {
        if (auto* first_line_container = To<LayoutBlock>(this)->NearestInnerBlockWithFirstLine())
...
void LayoutObject::UpdateFillImages(const FillLayer* old_layers, const FillLayer* new_layers)
{
    NOT_DESTROYED();
    ...
    for (const FillLayer* curr_new = new_layers; curr_new; curr_new = curr_new->Next())
        AddAsImageObserver(curr_new->GetImage());

    for (const FillLayer* curr_old = old_layers; curr_old; curr_old = curr_old->Next())
        RemoveAsImageObserver(curr_old->GetImage());
}

void LayoutObject::UpdateCursorImages(const CursorList* old_cursors, const CursorList* new_cursors)
{
    NOT_DESTROYED();
...
void LayoutObject::UpdateShapeImage(const ShapeValue* old_shape_value, const ShapeValue* new_shape_value)
{
    NOT_DESTROYED();
    if (old_shape_value || new_shape_value) {
        UpdateImage(old_shape_value ? old_shape_value->GetImage() : nullptr, new_shape_value ? new_shape_value->GetImage() : nullptr);
    }
}
...
gfx::PointF LayoutObject::AncestorToLocalPoint(const LayoutBoxModelObject* ancestor, const gfx::PointF& container_point, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    TransformState transform_state(TransformState::kUnapplyInverseTransformDirection, container_point);
...
gfx::QuadF LayoutObject::AncestorToLocalQuad(const LayoutBoxModelObject* ancestor, const gfx::QuadF& quad, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    TransformState transform_state(TransformState::kUnapplyInverseTransformDirection, quad.BoundingBox().CenterPoint(), quad);
    MapAncestorToLocal(ancestor, transform_state, mode);
    transform_state.Flatten();
    return transform_state.LastPlanarQuad();
}

void LayoutObject::MapLocalToAncestor(const LayoutBoxModelObject* ancestor, TransformState& transform_state, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    if (ancestor == this)
        return;
    ...
    const bool preserve3d = use_transforms && container_preserves_3d && !container->IsText() && path_preserves_3d;

    if (use_transforms && ShouldUseTransformFromContainer(container)) {
        gfx::Transform t;
        GetTransformFromContainer(container, container_offset, t);
        transform_state.ApplyTransform(t, preserve3d ? TransformState::kAccumulateTransform : TransformState::kFlattenTransform);
    } else {
        transform_state.Move(container_offset, preserve3d ? TransformState::kAccumulateTransform : TransformState::kFlattenTransform);
    }
    ...
    if (skip_info.AncestorSkipped()) {
        ...
        transform_state.Move(-ancestor->OffsetFromAncestor(container), preserve3d ? TransformState::kAccumulateTransform : TransformState::kFlattenTransform);
        return;
    }
...
void LayoutObject::MapAncestorToLocal(const LayoutBoxModelObject* ancestor, TransformState& transform_state, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    if (this == ancestor)
        return;
    ...
    if (IsLayoutFlowThread()) {
        ...
        transform_state.Move(visual_point - To<LayoutFlowThread>(this)->VisualPointToFlowThreadPoint(visual_point));
    }
...
bool LayoutObject::ShouldUseTransformFromContainer(const LayoutObject* container_object) const
{
    NOT_DESTROYED();
    ...
    // transform directly.
    return (HasLayer() && To<LayoutBoxModelObject>(this)->Layer()->Transform()) || (container_object && container_object->StyleRef().HasPerspective());
}

void LayoutObject::GetTransformFromContainer(const LayoutObject* container_object, const PhysicalOffset& offset_in_container, gfx::Transform& transform,
    const PhysicalSize* size, const gfx::Transform* fragment_transform) const
{
    NOT_DESTROYED();
    ...
    if (fragment_transform) {
        ...
        PaintLayer* layer = HasLayer() ? To<LayoutBoxModelObject>(this)->Layer() : nullptr;
        if (layer && layer->Transform()) {
            transform.PreConcat(layer->CurrentTransform());
        }
    }

    transform.PostTranslate(offset_in_container.left.ToFloat(), offset_in_container.top.ToFloat());

    bool has_perspective = container_object && container_object->HasLayer() && container_object->StyleRef().HasPerspective();
    if (has_perspective && container_object != NearestAncestorForElement()) {
        has_perspective = false;

        if (StyleRef().Preserves3D() || transform.Creates3d()) {
            UseCounter::Count(GetDocument(), WebFeature::kDifferentPerspectiveCBOrParent);
        }
    }
    ...
    if (has_perspective) {
        ...
        perspective_matrix.ApplyPerspectiveDepth(container_object->StyleRef().UsedPerspective());
        perspective_matrix.ApplyTransformOrigin(perspective_origin.x(), perspective_origin.y(), 0);

        transform = perspective_matrix * transform;
    }
}

gfx::PointF LayoutObject::LocalToAncestorPoint(const gfx::PointF& local_point, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    TransformState transform_state(TransformState::kApplyTransformDirection, local_point);
...
PhysicalRect LayoutObject::LocalToAncestorRect(const PhysicalRect& rect, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    return PhysicalRect::EnclosingRect(LocalToAncestorQuad(gfx::QuadF(gfx::RectF(rect)), ancestor, mode).BoundingBox());
}

gfx::QuadF LayoutObject::LocalToAncestorQuad(const gfx::QuadF& local_quad, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    ...
    TransformState transform_state(TransformState::kApplyTransformDirection, local_quad.BoundingBox().CenterPoint(), local_quad);
    MapLocalToAncestor(ancestor, transform_state, mode);
    transform_state.Flatten();
...
void LayoutObject::LocalToAncestorRects(
    Vector<PhysicalRect>& rects, const LayoutBoxModelObject* ancestor, const PhysicalOffset& pre_offset, const PhysicalOffset& post_offset) const
{
    NOT_DESTROYED();
    for (wtf_size_t i = 0; i < rects.size(); ++i) {
        PhysicalRect& rect = rects[i];
        rect.Move(pre_offset);
        gfx::QuadF container_quad = LocalToAncestorQuad(gfx::QuadF(gfx::RectF(rect)), ancestor);
        PhysicalRect container_rect = PhysicalRect::EnclosingRect(container_quad.BoundingBox());
...
gfx::Transform LayoutObject::LocalToAncestorTransform(const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
...
bool LayoutObject::OffsetForContainerDependsOnPoint(const LayoutObject* container) const
{
    return IsLayoutFlowThread() || (container->StyleRef().IsFlippedBlocksWritingMode() && container->IsBox());
}

PhysicalOffset LayoutObject::OffsetFromContainer(const LayoutObject* o, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    return OffsetFromContainerInternal(o, mode);
}

PhysicalOffset LayoutObject::OffsetFromContainerInternal(const LayoutObject* o, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    DCHECK_EQ(o, Container());
    return o->IsScrollContainer() ? OffsetFromScrollableContainer(o, mode & kIgnoreScrollOffset) : PhysicalOffset();
}

PhysicalOffset LayoutObject::OffsetFromScrollableContainer(const LayoutObject* container, bool ignore_scroll_offset) const
{
    NOT_DESTROYED();
...
PhysicalOffset LayoutObject::OffsetFromAncestor(const LayoutObject* ancestor_container) const
{
    NOT_DESTROYED();
    ...
    do {
        ...
        PhysicalOffset current_offset = curr_container->OffsetFromContainer(next_container);
        offset += current_offset;
        reference_point += current_offset;
        curr_container = next_container;
    } while (curr_container != ancestor_container && !skip_info.AncestorSkipped());
    if (skip_info.AncestorSkipped()) {
...
RespectImageOrientationEnum LayoutObject::GetImageOrientation(const LayoutObject* layout_object)
{
    return layout_object ? layout_object->StyleRef().ImageOrientation() : ComputedStyleInitialValues::InitialImageOrientation();
}
...
void LayoutObject::WillBeDestroyed()
{
    ...
    if (GetNode() && !GetNode()->IsTextNode() && style_ && style_->GetTouchAction() != TouchAction::kAuto) {
        EventHandlerRegistry& registry = GetDocument().GetFrame()->GetEventHandlerRegistry();
        if (registry.EventHandlerTargets(EventHandlerRegistry::kTouchAction)->Contains(GetNode())) {
            registry.DidRemoveEventHandler(*GetNode(), EventHandlerRegistry::kTouchAction);
        }
    }
...
void LayoutObject::InsertedIntoTree()
{
    ...
    bitfields_.SetMightTraversePhysicalFragments(MightTraversePhysicalFragments(*this));
    ...
    // can't be used
    if (Parent()->StyleRef().Visibility() != EVisibility::kVisible && StyleRef().Visibility() == EVisibility::kVisible && !HasLayer()) {
        if (!layer)
            layer = Parent()->EnclosingLayer();
        if (layer)
            layer->DirtyVisibleContentStatus();
    ...
    if (const Element* element = DynamicTo<Element>(GetNode()); element && element->HasImplicitlyAnchoredElement()) {
        MarkMayHaveAnchorQuery();
...
static bool FindReferencingScrollAnchors(LayoutObject* layout_object, FindReferencingScrollAnchorsBehavior behavior)
{
    PaintLayer* layer = nullptr;
    if (LayoutObject* parent = layout_object->Parent())
        layer = parent->EnclosingLayer();
...
{
    NOT_DESTROYED();
    for (auto* ancestor = this; ancestor && !ancestor->DescendantNeedsPaintPropertyUpdate(); ancestor = ancestor->Parent()) {
...
void LayoutObject::MaybeClearIsScrollAnchorObject()
{
    ...
    if (!bitfields_.IsScrollAnchorObject())
        return;
    bitfields_.SetIsScrollAnchorObject(FindReferencingScrollAnchors(this, kDontClear));
}
...
void LayoutObject::RemoveFromLayoutFlowThread()
{
    ...
    LayoutFlowThread* flow_thread = IsColumnSpanAll() ? Parent()->FlowThreadContainingBlock() : FlowThreadContainingBlock();
    RemoveFromLayoutFlowThreadRecursive(flow_thread);
}

void LayoutObject::RemoveFromLayoutFlowThreadRecursive(LayoutFlowThread* layout_flow_thread)
{
    NOT_DESTROYED();
    if (const LayoutObjectChildList* children = VirtualChildren()) {
        for (LayoutObject* child = children->FirstChild(); child; child = child->NextSibling()) {
            ...
            child->RemoveFromLayoutFlowThreadRecursive(child->IsLayoutFlowThread() ? To<LayoutFlowThread>(child) : layout_flow_thread);
        }
    }
...
void LayoutObject::DestroyAndCleanupAnonymousWrappers(bool performing_reattach)
{
    NOT_DESTROYED();
    ...
    for (; destroy_root_parent && destroy_root_parent->IsAnonymous(); destroy_root = destroy_root_parent, destroy_root_parent = destroy_root_parent->Parent()) {
        ...
        if (destroy_root_parent->Parent() && destroy_root_parent->Parent()->IsFieldset()) {
            break;
        }
...
{
    NOT_DESTROYED();
    DCHECK(g_allow_destroying_layout_object_in_finalizer || !ThreadState::IsSweepingOnOwningThread(*ThreadStateStorage::Current()));
...
PositionWithAffinity LayoutObject::PositionForPoint(const PhysicalOffset&) const
{
    NOT_DESTROYED();
    // NG codepath requires |kPrePaintClean|.
    // |SelectionModifier| calls this only in legacy codepath.
    DCHECK(!IsLayoutNGObject() || GetDocument().Lifecycle().GetState() >= DocumentLifecycle::kPrePaintClean);
...
bool LayoutObject::HitTestAllPhases(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset)
{
    NOT_DESTROYED();
    if (NodeAtPoint(result, hit_test_location, accumulated_offset, HitTestPhase::kForeground)) {
        return true;
    }
    if (NodeAtPoint(result, hit_test_location, accumulated_offset, HitTestPhase::kFloat)) {
        return true;
    }
    if (NodeAtPoint(result, hit_test_location, accumulated_offset, HitTestPhase::kDescendantBlockBackgrounds)) {
        return true;
    }
    if (NodeAtPoint(result, hit_test_location, accumulated_offset, HitTestPhase::kSelfBlockBackground)) {
        return true;
    }
    return false;
}
...
Node* LayoutObject::NodeForHitTest() const
{
    ...
    if (const LayoutObject* parent = Parent()) {
        if (parent->IsBeforeOrAfterContent() || parent->IsMarkerContent() || parent->StyleRef().StyleType() == kPseudoIdFirstLetter) {
...
void LayoutObject::UpdateHitTestResult(HitTestResult& result, const PhysicalOffset& point) const
{
    NOT_DESTROYED();
    if (result.InnerNode())
        return;
...
bool LayoutObject::NodeAtPoint(HitTestResult&, const HitTestLocation&, const PhysicalOffset&, HitTestPhase)
{
    NOT_DESTROYED();
    return false;
}
...
const ComputedStyle* LayoutObject::FirstLineStyleWithoutFallback() const
{
    ...
    if (BehavesLikeBlockContainer()) {
        if (const ComputedStyle* cached = StyleRef().GetCachedPseudoElementStyle(kPseudoIdFirstLine)) {
        ...
        if (Element* element = DynamicTo<Element>(GetNode())) {
            if (element->ShadowPseudoId() == shadow_element_names::kPseudoInternalInputSuggested) {
        ...
        for (const LayoutBlock* first_line_block = To<LayoutBlock>(this); first_line_block; first_line_block = first_line_block->FirstLineStyleParentBlock()) {
            ...
            if (first_line_block == this) {
                if (const ComputedStyle* cached = first_line_block->GetCachedPseudoElementStyle(kPseudoIdFirstLine)) {
                    return cached;
                }
                continue;
            }
            ...
            // it.
            if (const ComputedStyle* first_line_style = first_line_block->GetUncachedPseudoElementStyle(StyleRequest(kPseudoIdFirstLine, Style()))) {
                return StyleRef().ReplaceCachedPseudoElementStyle(std::move(first_line_style), kPseudoIdFirstLine, g_null_atom);
            }
        }
    } else if (!IsAnonymous() && IsLayoutInline() && !GetNode()->IsFirstLetterPseudoElement()) {
        if (const ComputedStyle* cached = StyleRef().GetCachedPseudoElementStyle(kPseudoIdFirstLineInherited))
            return cached;

        if (const ComputedStyle* parent_first_line_style = Parent()->FirstLineStyleWithoutFallback()) {
            ...
            if (const ComputedStyle* first_line_style = GetUncachedPseudoElementStyle(StyleRequest(kPseudoIdFirstLineInherited, parent_first_line_style))) {
                return StyleRef().AddCachedPseudoElementStyle(std::move(first_line_style), kPseudoIdFirstLineInherited, g_null_atom);
            }
        }
    }
    return nullptr;
...
const ComputedStyle* LayoutObject::GetCachedPseudoElementStyle(PseudoId pseudo) const
{
    NOT_DESTROYED();
...
const ComputedStyle* LayoutObject::GetUncachedPseudoElementStyle(const StyleRequest& request) const
{
    NOT_DESTROYED();
    ...
    if (!element)
        return nullptr;
    if (element->IsPseudoElement() && request.pseudo_id != kPseudoIdFirstLineInherited)
        return nullptr;
...
void LayoutObject::AddDraggableRegions(Vector<DraggableRegionValue>& regions)
{
    ...
    region.draggable = StyleRef().DraggableRegionMode() == EDraggableRegionMode::kDrag;
...
bool LayoutObject::GetImageAnimationPolicy(mojom::blink::ImageAnimationPolicy& policy)
{
    NOT_DESTROYED();
...
void LayoutObject::ImageChanged(ImageResourceContent* image, CanDeferInvalidation defer)
{
    NOT_DESTROYED();
    DCHECK(node_);
    ...
    DCHECK_NE(GetDocument().Lifecycle().GetState(), DocumentLifecycle::LifecycleState::kInPaint);
...
Element* LayoutObject::OffsetParent(const Element* base) const
{
    ...
    for (LayoutObject* ancestor = Parent(); ancestor; ancestor = ancestor->Parent()) {
        ...
        if (!IsPositioned() && (IsA<HTMLTableElement>(*node) || IsA<HTMLTableCellElement>(*node)))
            break;
...
PositionWithAffinity LayoutObject::CreatePositionWithAffinity(int offset, TextAffinity affinity) const
{
    NOT_DESTROYED();
    ...
    if (!node)
        return FindPosition();
    return AdjustForEditingBoundary(PositionWithAffinity(Position(node, offset), affinity));
}
...
PositionWithAffinity LayoutObject::FindPosition() const
{
    ...
    while (const LayoutObject* parent = child->Parent()) {
        ...
        for (const LayoutObject* layout_object = child->NextInPreOrder(parent); layout_object; layout_object = layout_object->NextInPreOrder(parent)) {
        ...
        for (const LayoutObject* layout_object = child->PreviousInPreOrder(); layout_object; layout_object = layout_object->PreviousInPreOrder()) {
...
PositionWithAffinity LayoutObject::CreatePositionWithAffinity(int offset) const
{
    NOT_DESTROYED();
    return CreatePositionWithAffinity(offset, TextAffinity::kDownstream);
}

CursorDirective LayoutObject::GetCursor(const PhysicalOffset&, ui::Cursor&) const
{
    NOT_DESTROYED();
...
void LayoutObject::SetShouldInvalidateSelection()
{
    ...
    if (IsText()) {
        if (auto* computed_style = GetSelectionStyle()) {
            if (computed_style->HasAppliedTextDecorations() || computed_style->HasVisualOverflowingEffect()) {
                InvalidateVisualOverflow();
            }
        }
    }
...
void LayoutObject::SetShouldDoFullPaintInvalidationWithoutLayoutChange(PaintInvalidationReason reason)
{
    NOT_DESTROYED();
...
void LayoutObject::SetShouldDoFullPaintInvalidationWithoutLayoutChangeInternal(PaintInvalidationReason reason)
{
    NOT_DESTROYED();
    ...
    const bool should_upgrade_reason = reason > PaintInvalidationReasonForPrePaint();
...
void LayoutObject::SetShouldInvalidatePaintForHitTest()
{
    ...
    if (PaintInvalidationReasonForPrePaint() < PaintInvalidationReason::kHitTest) {
        SetShouldCheckForPaintInvalidationWithoutLayoutChange();
        paint_invalidation_reason_for_pre_paint_ = static_cast<unsigned>(PaintInvalidationReason::kHitTest);
...
void LayoutObject::SetShouldCheckForPaintInvalidation()
{
    ...
    DCHECK(GetDocument().Lifecycle().GetState() != DocumentLifecycle::kInPrePaint);

    for (LayoutObject* ancestor = Parent(); ancestor && !ancestor->DescendantShouldCheckLayoutForPaintInvalidation(); ancestor = ancestor->Parent()) {
        ancestor->bitfields_.SetShouldCheckForPaintInvalidation(true);
        ancestor->bitfields_.SetDescendantShouldCheckLayoutForPaintInvalidation(true);
    }
}
...
void LayoutObject::SetShouldCheckForPaintInvalidationWithoutLayoutChange()
{
    ...
    bitfields_.SetShouldCheckForPaintInvalidation(true);
    for (LayoutObject* ancestor = Parent(); ancestor && !ancestor->ShouldCheckForPaintInvalidation(); ancestor = ancestor->Parent()) {
        ancestor->bitfields_.SetShouldCheckForPaintInvalidation(true);
    }
}
...
void LayoutObject::ClearShouldDelayFullPaintInvalidation()
{
    ...
    SetShouldDoFullPaintInvalidationWithoutLayoutChangeInternal(PaintInvalidationReasonForPrePaint());
}
...
{
    NOT_DESTROYED();
    return BackgroundNeedsFullPaintInvalidation() || ShouldCheckForPaintInvalidation() || ShouldInvalidateSelection() || ShouldCheckLayoutForPaintInvalidation()
        || DescendantShouldCheckLayoutForPaintInvalidation() || ShouldDoFullPaintInvalidation() || SubtreeShouldDoFullPaintInvalidation()
...
void LayoutObject::EnsureIsReadyForPaintInvalidation()
{
    ...
    if (bitfields_.OutlineMayBeAffectedByDescendants() || bitfields_.PreviousOutlineMayBeAffectedByDescendants()) {
        SetShouldDoFullPaintInvalidationWithoutLayoutChange(PaintInvalidationReason::kOutline);
    }
    bitfields_.SetPreviousOutlineMayBeAffectedByDescendants(bitfields_.OutlineMayBeAffectedByDescendants());
}

void LayoutObject::ClearPaintFlags()
{
    NOT_DESTROYED();
    DCHECK_EQ(GetDocument().Lifecycle().GetState(), DocumentLifecycle::kInPrePaint);
...
{
    return document.Lifecycle().StateAllowsLayoutTreeMutations() || document.GetStyleEngine().InContainerQueryStyleRecalc()
        || document.GetStyleEngine().InScrollMarkersAttachment();
}

void LayoutObject::SetSubtreeShouldDoFullPaintInvalidation(PaintInvalidationReason reason)
{
    NOT_DESTROYED();
...
void LayoutObject::SetIsBackgroundAttachmentFixedObject(bool is_background_attachment_fixed_object)
{
    NOT_DESTROYED();
    DCHECK(GetFrameView());
    DCHECK(IsBoxModelObject());
    if (bitfields_.IsBackgroundAttachmentFixedObject() == is_background_attachment_fixed_object) {
        return;
    }
    bitfields_.SetIsBackgroundAttachmentFixedObject(is_background_attachment_fixed_object);
    if (is_background_attachment_fixed_object) {
        GetFrameView()->AddBackgroundAttachmentFixedObject(To<LayoutBoxModelObject>(*this));
    } else {
        SetCanCompositeBackgroundAttachmentFixed(false);
        GetFrameView()->RemoveBackgroundAttachmentFixedObject(To<LayoutBoxModelObject>(*this));
    }
}

void LayoutObject::SetCanCompositeBackgroundAttachmentFixed(bool can_composite)
{
...
void LayoutObject::InvalidateSelectedChildrenOnStyleChange()
{
    ...
    LayoutBlock* block = IsLayoutBlock() ? To<LayoutBlock>(this) : ContainingBlock();
    if (!block)
        return;
    if (!block->IsSelected())
        return;
...
const LayoutObject* AssociatedLayoutObjectOf(const Node& node, int offset_in_node, LayoutObjectSide object_side)
{
    DCHECK_GE(offset_in_node, 0);
    LayoutObject* layout_object = node.GetLayoutObject();
    if (!node.IsTextNode() || !layout_object || !To<LayoutText>(layout_object)->IsTextFragment())
        return layout_object;
    auto* layout_text_fragment = To<LayoutTextFragment>(layout_object);
    if (!layout_text_fragment->IsRemainingTextLayoutObject()) {
        DCHECK_LE(static_cast<unsigned>(offset_in_node), layout_text_fragment->Start() + layout_text_fragment->FragmentLength());
        return layout_text_fragment;
    }
    if (layout_text_fragment->FragmentLength()) {
        const unsigned threshold
            = object_side == LayoutObjectSide::kRemainingTextIfOnBoundary ? layout_text_fragment->Start() : layout_text_fragment->Start() + 1;
...
{
    NOT_DESTROYED();
    if (SlowFirstChild() || StyleRef().Visibility() != EVisibility::kVisible || DisplayLockUtilities::LockedAncestorPreventingPaint(*this)) {
        return false;
    }
    return CanBeSelectionLeafInternal();
}

Vector<PhysicalRect> LayoutObject::CollectOutlineRectsAndAdvance(OutlineType outline_type, AccompaniedFragmentIterator& iterator) const
{
    NOT_DESTROYED();
    ...
    if (iterator.Cursor()) {
        ...
        do {
            ...
            if (const PhysicalBoxFragment* box_fragment = item->BoxFragment()) {
                box_fragment->AddSelfOutlineRects(paint_offset + item->OffsetInContainerFragment(), outline_type, collector, nullptr);
            } else {
                PhysicalRect rect;
        ...
        } while (iterator.Advance() && iterator.Cursor()->ContainerFragmentIndex() == fragment_index);
        outline_rects = collector.TakeRects();
    } else {
        if (const auto* box_fragment = iterator.GetPhysicalBoxFragment()) {
            box_fragment->AddSelfOutlineRects(paint_offset, outline_type, collector, nullptr);
            outline_rects = collector.TakeRects();
        } else {
...
Vector<PhysicalRect> LayoutObject::OutlineRects(OutlineInfo* info, const PhysicalOffset& additional_offset, OutlineType outline_type) const
{
    NOT_DESTROYED();
    VectorOutlineRectCollector collector;
...
void LayoutObject::SetModifiedStyleOutsideStyleRecalc(const ComputedStyle* style, ApplyStyleChanges apply_changes)
{
    NOT_DESTROYED();
...
LayoutUnit LayoutObject::FlipForWritingModeInternal(LayoutUnit position, LayoutUnit width, const LayoutBox* box_for_flipping) const
{
    NOT_DESTROYED();
    DCHECK(!IsBox());
    ...
    if (IsSVG())
        return position;
    return (box_for_flipping ? box_for_flipping : ContainingBlock())->FlipForWritingMode(position, width);
}
    ...
    if (mark_style_dirty) {
        if (Node* node = GetNode()) {
            ...
            if (invalidate && mark_style_dirty) {
                ...
                // fallback styles.
                node->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kPositionTryChange));
            }
        }
    }
...
void ShowLayoutTree(const blink::LayoutObject* object1, const blink::LayoutObject* object2)
{
    ...
    if (object1) {
        ...
        if (object1) {
            StringBuilder string_builder;
            root->DumpLayoutTreeAndMark(string_builder, object1, "*", object2, "-", 0);
            DLOG(INFO) << "\n" << string_builder.ToString().Utf8();
        }
    } else {
