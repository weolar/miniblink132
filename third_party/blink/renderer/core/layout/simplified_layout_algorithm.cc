...
SimplifiedLayoutAlgorithm::SimplifiedLayoutAlgorithm(const LayoutAlgorithmParams& params, const LayoutResult& result, bool keep_old_size)
    ...
    , writing_direction_(Style().GetWritingDirection())
{
    ...
    container_builder_.SetIsNewFormattingContext(physical_fragment.IsFormattingContextRoot());
    ...
    if (is_block_flow && !physical_fragment.IsFieldsetContainer()) {
        container_builder_.SetIsInlineFormattingContext(Node().IsInlineFormattingContextRoot());
        ...
        DCHECK_EQ(GetConstraintSpace().GetBfcOffset(), previous_result_.GetConstraintSpaceForCaching().GetBfcOffset());
        ...
        if (GetConstraintSpace().IsTableCell()) {
            container_builder_.SetHasCollapsedBorders(physical_fragment.HasCollapsedBorders());
            container_builder_.SetTableCellColumnIndex(physical_fragment.TableCellColumnIndex());
        } else {
    ...
    if (physical_fragment.IsTable()) {
        ...
        if (const auto* table_column_geometries = physical_fragment.TableColumnGeometries())
            container_builder_.SetTableColumnGeometries(*table_column_geometries);

        if (const auto* table_collapsed_borders = physical_fragment.TableCollapsedBorders())
            container_builder_.SetTableCollapsedBorders(*table_collapsed_borders);

        if (const auto* table_collapsed_borders_geometry = physical_fragment.TableCollapsedBordersGeometry()) {
            container_builder_.SetTableCollapsedBordersGeometry(
                std::make_unique<TableFragmentData::CollapsedBordersGeometry>(*table_collapsed_borders_geometry));
        }
    } else if (physical_fragment.IsTableSection()) {
        if (const auto section_start_row_index = physical_fragment.TableSectionStartRowIndex()) {
            Vector<LayoutUnit> section_row_offsets = *physical_fragment.TableSectionRowOffsets();
            container_builder_.SetTableSectionCollapsedBordersGeometry(*section_start_row_index, std::move(section_row_offsets));
        }
    }

    if (physical_fragment.IsGrid()) {
        container_builder_.TransferGridLayoutData(std::make_unique<GridLayoutData>(*result.GetGridLayoutData()));
    } else if (physical_fragment.IsFrameSet()) {
        container_builder_.TransferFrameSetLayoutData(std::make_unique<FrameSetLayoutData>(*physical_fragment.GetFrameSetLayoutData()));
    }
    ...
    if (keep_old_size) {
        LayoutUnit old_block_size = LogicalFragment(writing_direction_, physical_fragment).BlockSize();
        ...
        auto ComputeNewBlockSize = [&]() -> LayoutUnit {
            return ComputeBlockSizeForFragment(
                GetConstraintSpace(), Node(), BorderPadding(), result.IntrinsicBlockSize(), container_builder_.InitialBorderBoxSize().inline_size);
        };
        ...
        if (is_block_flow) {
            ...
            if (!physical_fragment.IsTable() && !physical_fragment.IsTableSection() && !physical_fragment.IsTableRow()) {
...
void SimplifiedLayoutAlgorithm::AppendNewChildFragment(const PhysicalFragment& fragment, LogicalOffset offset)
{
...
const LayoutResult* SimplifiedLayoutAlgorithm::Layout()
{
    ...
    for (const auto& child_link : previous_fragment.Children()) {
        ...
        const LayoutResult* result = BlockNode(To<LayoutBox>(child_fragment.GetMutableLayoutObject())).SimplifiedLayout(child_fragment);
        ...
        DCHECK(!result->GetPhysicalFragment().IsFormattingContextRoot() || end_margin_strut.IsEmpty());

        AddChildFragment(child_link, result->GetPhysicalFragment(), &end_margin_strut, result->IsSelfCollapsing());
    }
    ...
    for (LayoutInputNode child = Node().FirstChild(); child; child = child.NextSibling()) {
        ...
        container_builder_.AddOutOfFlowChildCandidate(To<BlockNode>(child), position.offset, position.inline_edge, position.block_edge);
    }
    ...
    if (const FragmentItems* previous_items = previous_fragment.Items()) {
        ...
        const auto result = items_builder->AddPreviousItems(previous_fragment, *previous_items);
        if (!result.succeeded)
            return nullptr;
    }
    ...
    if (previous_fragment.InflowBounds()) {
        LogicalRect inflow_bounds = WritingModeConverter(writing_direction_, previous_physical_container_size_).ToLogical(*previous_fragment.InflowBounds());
        container_builder_.SetInflowBounds(inflow_bounds);
    }
    container_builder_.SetHasAdjoiningObjectDescendants(previous_fragment.HasAdjoiningObjectDescendants());
    container_builder_.SetMayHaveDescendantAboveBlockStart(previous_fragment.MayHaveDescendantAboveBlockStart());
    container_builder_.SetHasDescendantThatDependsOnPercentageBlockSize(previous_result_.HasDescendantThatDependsOnPercentageBlockSize());
    container_builder_.SetInitialBreakBefore(previous_result_.InitialBreakBefore());
...
NOINLINE const LayoutResult* SimplifiedLayoutAlgorithm::LayoutWithItemsBuilder()
{
...
void SimplifiedLayoutAlgorithm::AddChildFragment(
    const PhysicalFragmentLink& old_fragment, const PhysicalFragment& new_fragment, const MarginStrut* margin_strut, bool is_self_collapsing)
{
    DCHECK_EQ(old_fragment->Size(), new_fragment.Size());

    // Determine the previous position in the logical coordinate system.
    LogicalOffset child_offset
        = WritingModeConverter(writing_direction_, previous_physical_container_size_).ToLogical(old_fragment.Offset(), new_fragment.Size());
    ...
    container_builder_.AddChild(new_fragment, child_offset, margin_strut, is_self_collapsing, relative_offset);
}
