...
bool ShouldCloneBlockStartBorderPadding(const BoxFragmentBuilder& builder)
{
    ...
    if (previous_break_token->MonolithicOverflow()) {
        LayoutUnit space_left = FragmentainerSpaceLeft(builder, /*is_for_children=*/false);
...
EBreakBetween JoinFragmentainerBreakValues(EBreakBetween first_value, EBreakBetween second_value)
{
    if (FragmentainerBreakPrecedence(second_value) >= FragmentainerBreakPrecedence(first_value))
        return second_value;
    return first_value;
}

bool IsForcedBreakValue(const ConstraintSpace& constraint_space, EBreakBetween break_value)
{
    ...
    if (break_value == EBreakBetween::kLeft || break_value == EBreakBetween::kPage || break_value == EBreakBetween::kRecto
        || break_value == EBreakBetween::kRight || break_value == EBreakBetween::kVerso)
        return constraint_space.BlockFragmentationType() == kFragmentPage;
    return false;
}

template <typename Property> bool IsAvoidBreakValue(const ConstraintSpace& constraint_space, Property break_value)
{
...
template bool CORE_TEMPLATE_EXPORT IsAvoidBreakValue(const ConstraintSpace&, EBreakBetween);
template bool CORE_TEMPLATE_EXPORT IsAvoidBreakValue(const ConstraintSpace&, EBreakInside);

EBreakBetween CalculateBreakBetweenValue(LayoutInputNode child, const LayoutResult& layout_result, const BoxFragmentBuilder& builder)
{
    if (child.IsInline())
    ...
    if (layout_result.Status() == LayoutResult::kSuccess) {
        box_fragment = &To<PhysicalBoxFragment>(layout_result.GetPhysicalFragment());
    ...
    EBreakBetween break_before = JoinFragmentainerBreakValues(child.Style().BreakBefore(), layout_result.InitialBreakBefore());
    break_before = builder.JoinedBreakBetweenValue(break_before);
    const auto& space = builder.GetConstraintSpace();
    if (space.IsPaginated() && box_fragment && !IsForcedBreakValue(builder.GetConstraintSpace(), break_before)) {
...
bool IsBreakableAtStartOfResumedContainer(const ConstraintSpace& space, const LayoutResult& child_layout_result, const BoxFragmentBuilder& builder)
{
    if (child_layout_result.Status() != LayoutResult::kSuccess) {
        return false;
    }
    bool is_first_for_node = true;
    if (const auto* box_fragment = DynamicTo<PhysicalBoxFragment>(child_layout_result.GetPhysicalFragment())) {
        is_first_for_node = box_fragment->IsFirstForNode();
    }
    return IsBreakableAtStartOfResumedContainer(space, builder, is_first_for_node);
}

bool IsBreakableAtStartOfResumedContainer(const ConstraintSpace& space, const BoxFragmentBuilder& builder, bool is_first_for_node)
{
    return space.MinBreakAppeal() != kBreakAppealLastResort && IsBreakInside(builder.PreviousBreakToken()) && is_first_for_node;
}

BreakAppeal CalculateBreakAppealBefore(
    const ConstraintSpace& space, LayoutInputNode child, const LayoutResult& layout_result, const BoxFragmentBuilder& builder, bool has_container_separation)
{
    bool breakable_at_start_of_container = IsBreakableAtStartOfResumedContainer(space, layout_result, builder);
    EBreakBetween break_between = CalculateBreakBetweenValue(child, layout_result, builder);
    return CalculateBreakAppealBefore(space, layout_result.Status(), break_between, has_container_separation, breakable_at_start_of_container);
}

BreakAppeal CalculateBreakAppealBefore(const ConstraintSpace& space, LayoutResult::EStatus layout_result_status, EBreakBetween break_between,
    bool has_container_separation, bool breakable_at_start_of_container)
{
    DCHECK(layout_result_status == LayoutResult::kSuccess || layout_result_status == LayoutResult::kOutOfFragmentainerSpace);
    BreakAppeal break_appeal = kBreakAppealPerfect;
    if (!has_container_separation && layout_result_status == LayoutResult::kSuccess) {
...
BreakAppeal CalculateBreakAppealInside(const ConstraintSpace& space, const LayoutResult& layout_result, std::optional<BreakAppeal> hypothetical_appeal)
{
    if (layout_result.HasForcedBreak())
        return kBreakAppealPerfect;
    const auto& physical_fragment = layout_result.GetPhysicalFragment();
    const auto* break_token = DynamicTo<BlockBreakToken>(physical_fragment.GetBreakToken());
    BreakAppeal appeal;
    ...
    if (consider_break_inside_avoidance && appeal > kBreakAppealViolatingBreakAvoid && IsAvoidBreakValue(space, physical_fragment.Style().BreakInside()))
        appeal = kBreakAppealViolatingBreakAvoid;
    return appeal;
}

LogicalSize FragmentainerLogicalCapacity(const PhysicalBoxFragment& fragmentainer)
{
    DCHECK(fragmentainer.IsFragmentainerBox());
    LogicalSize logical_size = WritingModeConverter(fragmentainer.Style().GetWritingDirection()).ToLogical(fragmentainer.Size());
    ...
    if (fragmentainer.HasScrollableOverflow()) {
        ...
        // zero-height column.
        logical_size.block_size = ClampedToValidFragmentainerCapacity(logical_size.block_size);
    }
    return logical_size;
}

LogicalOffset GetFragmentainerProgression(const BoxFragmentBuilder& builder, FragmentationType type)
{
    if (type == kFragmentColumn) {
        LayoutUnit column_inline_progression = ColumnInlineProgression(builder.ChildAvailableSize().inline_size, builder.Style());
...
void SetupSpaceBuilderForFragmentation(const ConstraintSpace& parent_space, const LayoutInputNode& child, LayoutUnit fragmentainer_offset,
    LayoutUnit fragmentainer_block_size, bool requires_content_before_breaking, ConstraintSpaceBuilder* builder)
{
    ...
    DCHECK(!requires_content_before_breaking || !parent_space.IsInitialColumnBalancingPass());
    ...
    if ((parent_space.IsInitialColumnBalancingPass() && child.IsOutOfFlowPositioned()) || parent_space.ShouldIgnoreForcedBreaks())
...
void SetupSpaceBuilderForFragmentation(
    const BoxFragmentBuilder& parent_fragment_builder, const LayoutInputNode& child, LayoutUnit fragmentainer_offset_delta, ConstraintSpaceBuilder* builder)
{
    LayoutUnit fragmentainer_block_size = FragmentainerCapacity(parent_fragment_builder, /*is_for_children=*/true);
    LayoutUnit fragmentainer_block_offset = FragmentainerOffset(parent_fragment_builder, /*is_for_children=*/true) + fragmentainer_offset_delta;
    return SetupSpaceBuilderForFragmentation(parent_fragment_builder.GetConstraintSpace(), child, fragmentainer_block_offset, fragmentainer_block_size,
        parent_fragment_builder.RequiresContentBeforeBreaking(), builder);
}

void SetupFragmentBuilderForFragmentation(
    const ConstraintSpace& space, const LayoutInputNode& node, const BlockBreakToken* previous_break_token, BoxFragmentBuilder* builder)
{
    ...
    builder->SetIsMonolithic(!space.IsAnonymous() && space.IsBlockFragmentationForcedOff() && !IsBreakInside(previous_break_token));
    ...
    bool clone_box_start_decorations = ShouldCloneBlockStartBorderPadding(*builder);
    ...
    if (space.HasBlockFragmentation() && !space.IsAnonymous() && !space.IsInitialColumnBalancingPass()) {
        bool requires_content_before_breaking = space.RequiresContentBeforeBreaking();
        ...
        if (!node.IsTable() && builder->InitialBorderBoxSize().inline_size != kIndefiniteSize) {
            ...
            LayoutUnit max_block_size = ComputeBlockSizeForFragment(
                space, To<BlockNode>(node), builder->BorderPadding(), LayoutUnit::Max(), builder->InitialBorderBoxSize().inline_size);
...
{
    return !IsBreakInside(builder.PreviousBreakToken()) || ShouldCloneBlockStartBorderPadding(builder);
}

bool ShouldIncludeBlockEndBorderPadding(const BoxFragmentBuilder& builder)
{
    if (builder.PreviousBreakToken() && builder.PreviousBreakToken()->IsAtBlockEnd()) {
        // Past the block-end, and therefore past block-end border+padding.
        return false;
    }
    if (!builder.ShouldBreakInside() || builder.IsKnownToFitInFragmentainer() || builder.ShouldCloneBoxEndDecorations()) {
        return true;
    }
...
BreakStatus FinishFragmentation(BoxFragmentBuilder* builder)
{
    ...
    if (previous_break_token && !previous_break_token->IsBreakBefore())
        previously_consumed_block_size = previous_break_token->ConsumedBlockSize();
    bool is_past_end = previous_break_token && previous_break_token->IsAtBlockEnd();

    LayoutUnit fragments_total_block_size = builder->FragmentsTotalBlockSize();
    LayoutUnit desired_block_size = fragments_total_block_size - previously_consumed_block_size;
    ...
    LayoutUnit trailing_border_padding = builder->BorderScrollbarPadding().block_end;
    LayoutUnit subtractable_border_padding;
    if (!builder->ShouldPreventBreakBeforeBlockEndDecorations()) {
        if (desired_block_size > trailing_border_padding || (previous_break_token && previous_break_token->MonolithicOverflow())) {
    ...
    if (space_left != kIndefiniteSize) {
        ...
        // node is concerned.
        space_left = std::max(space_left, desired_intrinsic_block_size - subtractable_border_padding);
    }
    ...
    if (is_past_end) {
        ...
        final_block_size = std::min(final_block_size, desired_intrinsic_block_size) - trailing_border_padding;
    ...
    } else if (space_left != kIndefiniteSize && desired_block_size > space_left && space.HasBlockFragmentation()) {
        ...
        LayoutUnit modified_intrinsic_block_size = std::max(space_left, desired_intrinsic_block_size - subtractable_border_padding);
        builder->SetIntrinsicBlockSize(modified_intrinsic_block_size);
        final_block_size = std::min(desired_block_size - subtractable_border_padding, modified_intrinsic_block_size);
    ...
    if (previously_consumed_block_size && (node.Style().BoxDecorationBreak() == EBoxDecorationBreak::kSlice || is_past_end)) {
    ...
    if (!final_block_size && previous_break_token && previous_break_token->MonolithicOverflow()) {
        ...
        // content.
        LayoutUnit remaining_overflow = previous_break_token->MonolithicOverflow() - FragmentainerCapacity(*builder, /*is_for_children=*/false);
    ...
    if (builder->ShouldBreakInside()) {
        ...
        if (builder->IsAtBlockEnd()) {
            ...
            builder->SetConsumedBlockSize(previously_consumed_block_size + std::max(final_block_size, space_left));
        } else {
    ...
    if (desired_block_size > space_left) {
        ...
        if (!previously_consumed_block_size) {
            ...
            LayoutUnit block_start_unbreakable_space = geometry.border.block_start + geometry.scrollbar.block_start + geometry.padding.block_start;
            if (space_left < block_start_unbreakable_space)
                builder->ClampBreakAppeal(kBreakAppealLastResort);
        }
        if (space.BlockFragmentationType() == kFragmentColumn && !space.IsInitialColumnBalancingPass())
...
BreakStatus FinishFragmentationForFragmentainer(BoxFragmentBuilder* builder)
{
    ...
    const BlockBreakToken* previous_break_token = builder->PreviousBreakToken();
    LayoutUnit consumed_block_size = previous_break_token ? previous_break_token->ConsumedBlockSize() : LayoutUnit();
    if (space.HasKnownFragmentainerBlockSize()) {
        ...
        LayoutUnit fragmentainer_capacity = FragmentainerCapacity(*builder, /*is_for_children=*/false);
        ...
        LayoutUnit consumed_block_size_for_legacy = previous_break_token ? previous_break_token->ConsumedBlockSizeForLegacy() : LayoutUnit();
        LayoutUnit legacy_fragmentainer_block_size = (builder->IntrinsicBlockSize() > LayoutUnit()) ? fragmentainer_capacity : block_size;
        LayoutUnit consumed_block_size_legacy_adjustment = consumed_block_size_for_legacy + legacy_fragmentainer_block_size - consumed_block_size;
        builder->SetConsumedBlockSizeLegacyAdjustment(consumed_block_size_legacy_adjustment);
        ...
        builder->SetConsumedBlockSize(fragments_total_block_size + consumed_block_size);
    }
...
bool HasBreakOpportunityBeforeNextChild(const PhysicalFragment& child_fragment, const BreakToken* incoming_child_break_token)
{
    ...
    if (IsA<PhysicalBoxFragment>(&child_fragment)) {
        const auto* block_break_token = To<BlockBreakToken>(incoming_child_break_token);
    ...
    LogicalFragment fragment(child_fragment.Style().GetWritingDirection(), child_fragment);
    return fragment.BlockSize() != LayoutUnit();
}

BreakStatus BreakBeforeChildIfNeeded(const ConstraintSpace& space, LayoutInputNode child, const LayoutResult& layout_result,
    LayoutUnit fragmentainer_block_offset, LayoutUnit fragmentainer_block_size, bool has_container_separation, BoxFragmentBuilder* builder, bool is_row_item,
    FlexColumnBreakInfo* flex_column_break_info)
{
    ...
    if (has_container_separation && !is_row_item) {
        ...
        if (IsForcedBreakValue(space, break_between)) {
            BreakBeforeChild(space, child, &layout_result, fragmentainer_block_offset, fragmentainer_block_size, kBreakAppealPerfect,
                /*is_forced_break=*/true, builder);
            return BreakStatus::kBrokeBefore;
        }
    }

    BreakAppeal appeal_before = CalculateBreakAppealBefore(space, child, layout_result, *builder, has_container_separation);

    // Attempt to move past the break point, and if we can do that, also assess
    // the appeal of breaking there, even if we didn't.
    if (MovePastBreakpoint(
            space, child, layout_result, fragmentainer_block_offset, fragmentainer_block_size, appeal_before, builder, is_row_item, flex_column_break_info)) {
        return BreakStatus::kContinue;
    }
    ...
    if (!AttemptSoftBreak(space, child, &layout_result, fragmentainer_block_offset, fragmentainer_block_size, appeal_before, builder,
...
void BreakBeforeChild(const ConstraintSpace& space, LayoutInputNode child, const LayoutResult* layout_result, LayoutUnit fragmentainer_block_offset,
    LayoutUnit fragmentainer_block_size, std::optional<BreakAppeal> appeal, bool is_forced_break, BoxFragmentBuilder* builder,
    std::optional<LayoutUnit> block_size_override)
{
    ...
    if (layout_result && layout_result->Status() == LayoutResult::kSuccess) {
        ...
        // fragment.
        const auto& physical_fragment = layout_result->GetPhysicalFragment();
        DCHECK(!physical_fragment.IsBox() || To<PhysicalBoxFragment>(physical_fragment).IsFirstForNode());
    }
#endif

    if (space.HasKnownFragmentainerBlockSize()) {
        PropagateSpaceShortage(space, layout_result, fragmentainer_block_offset, fragmentainer_block_size, builder, block_size_override);
    }
...
void PropagateSpaceShortage(const ConstraintSpace& space, const LayoutResult* layout_result, LayoutUnit fragmentainer_block_offset,
    LayoutUnit fragmentainer_block_size, FragmentBuilder* builder, std::optional<LayoutUnit> block_size_override)
{
    // Only multicol cares about space shortage.
    if (space.BlockFragmentationType() != kFragmentColumn)
        return;

    LayoutUnit space_shortage = CalculateSpaceShortage(space, layout_result, fragmentainer_block_offset, fragmentainer_block_size, block_size_override);
...
LayoutUnit CalculateSpaceShortage(const ConstraintSpace& space, const LayoutResult* layout_result, LayoutUnit fragmentainer_block_offset,
    LayoutUnit fragmentainer_block_size, std::optional<LayoutUnit> block_size_override)
{
    ...
    if (block_size_override) {
        space_shortage = fragmentainer_block_offset + block_size_override.value() - fragmentainer_block_size;
        ...
        LogicalFragment fragment(space.GetWritingDirection(), layout_result->GetPhysicalFragment());
        space_shortage = fragmentainer_block_offset + fragment.BlockSize() - fragmentainer_block_size;
    } else {
...
void UpdateMinimalSpaceShortage(std::optional<LayoutUnit> new_space_shortage, LayoutUnit* minimal_space_shortage)
{
    ...
    if (*minimal_space_shortage == kIndefiniteSize) {
        ...
        *minimal_space_shortage = std::min(*minimal_space_shortage, *new_space_shortage);
    }
}

bool MovePastBreakpoint(const ConstraintSpace& space, LayoutInputNode child, const LayoutResult& layout_result, LayoutUnit fragmentainer_block_offset,
    LayoutUnit fragmentainer_block_size, BreakAppeal appeal_before, BoxFragmentBuilder* builder, bool is_row_item, FlexColumnBreakInfo* flex_column_break_info)
{
    if (layout_result.Status() != LayoutResult::kSuccess) {
    ...
    if (child.IsBlock()) {
        const auto& box_fragment = To<PhysicalBoxFragment>(layout_result.GetPhysicalFragment());
        ...
        if (builder && builder->GetExclusionSpace().NeedsClearancePastFragmentainer(child.Style().Clear(space.Direction()))) {
            return false;
        }
    }

    if (!space.HasKnownFragmentainerBlockSize() && space.IsInitialColumnBalancingPass() && builder) {
        if (layout_result.GetPhysicalFragment().IsMonolithic() || (child.IsBlock() && IsAvoidBreakValue(space, child.Style().BreakInside()))) {
            ...
            LayoutUnit block_size = BlockSizeForFragmentation(layout_result, space.GetWritingDirection());
            PropagateUnbreakableBlockSize(block_size, fragmentainer_block_offset, builder);
        }
    }

    bool move_past = MovePastBreakpoint(
        space, layout_result, fragmentainer_block_offset, fragmentainer_block_size, appeal_before, builder, is_row_item, flex_column_break_info);
    ...
    if (move_past && builder && child.IsBlock() && !is_row_item) {
        ...
        UpdateEarlyBreakAtBlockChild(space, To<BlockNode>(child), layout_result, appeal_before, builder, flex_column_break_info);
    }

    return move_past;
}

bool MovePastBreakpoint(const ConstraintSpace& space, const LayoutResult& layout_result, LayoutUnit fragmentainer_block_offset,
    LayoutUnit fragmentainer_block_size, BreakAppeal appeal_before, BoxFragmentBuilder* builder, bool is_row_item, FlexColumnBreakInfo* flex_column_break_info)
{
    ...
    bool refuse_break_before = space_left >= fragmentainer_block_size && (!builder || !IsBreakableAtStartOfResumedContainer(space, layout_result, *builder));
    ...
    if (space_left < LayoutUnit()) {
        ...
        // fragmentainer.
        must_break_before = !layout_result.GetColumnSpannerPath() && IsBreakInside(break_token) && !break_token->IsAtBlockEnd();
    }
    ...
    if (!move_past) {
            ...
            if (IsBreakInside(break_token) || appeal_inside < kBreakAppealPerfect) {
                ...
                if (appeal_inside >= appeal_before) {
                    if (flex_column_break_info) {
                        if (!flex_column_break_info->early_break || appeal_inside >= flex_column_break_info->early_break->GetBreakAppeal()) {
                            move_past = true;
                        }
                    } else if (!builder || !builder->HasEarlyBreak() || appeal_inside >= builder->GetEarlyBreak().GetBreakAppeal()) {
                        move_past = true;
                    }
                }
            } else {
        ...
        } else if (appeal_before == kBreakAppealLastResort && builder && builder->RequiresContentBeforeBreaking()) {
    ...
    if (move_past) {
        if (builder) {
            if (block_size > space_left) {
                ...
                PropagateSpaceShortage(space, &layout_result, fragmentainer_block_offset, fragmentainer_block_size, builder);
            }
        }
        return true;
    }
...
void UpdateEarlyBreakAtBlockChild(const ConstraintSpace& space, BlockNode child, const LayoutResult& layout_result, BreakAppeal appeal_before,
    BoxFragmentBuilder* builder, FlexColumnBreakInfo* flex_column_break_info)
{
    ...
    const auto* break_token = To<BlockBreakToken>(layout_result.GetPhysicalFragment().GetBreakToken());
    ...
    if (const auto* breakpoint = layout_result.GetEarlyBreak()) {
        ...
        appeal_inside = CalculateBreakAppealInside(space, layout_result, breakpoint->GetBreakAppeal());
        if (flex_column_break_info) {
            if (!flex_column_break_info->early_break || flex_column_break_info->early_break->GetBreakAppeal() <= breakpoint->GetBreakAppeal()) {
                // Found a good breakpoint inside the child. Add the child to the early
                // break chain for the current column.
                auto* parent_break = MakeGarbageCollected<EarlyBreak>(child, appeal_inside, breakpoint);
                flex_column_break_info->early_break = parent_break;
            }
        } else if (!builder->HasEarlyBreak() || builder->GetEarlyBreak().GetBreakAppeal() <= breakpoint->GetBreakAppeal()) {
            // Found a good breakpoint inside the child. Add the child to the early
    ...
    if (flex_column_break_info) {
        if (flex_column_break_info->early_break && flex_column_break_info->early_break->GetBreakAppeal() > appeal_before) {
            return;
        }
        flex_column_break_info->early_break = MakeGarbageCollected<EarlyBreak>(child, appeal_before);
        return;
    }

    if (builder->HasEarlyBreak() && builder->GetEarlyBreak().GetBreakAppeal() > appeal_before) {
        return;
    }

    builder->SetEarlyBreak(MakeGarbageCollected<EarlyBreak>(child, appeal_before));
}

bool AttemptSoftBreak(const ConstraintSpace& space, LayoutInputNode child, const LayoutResult* layout_result, LayoutUnit fragmentainer_block_offset,
    LayoutUnit fragmentainer_block_size, BreakAppeal appeal_before, BoxFragmentBuilder* builder, std::optional<LayoutUnit> block_size_override,
    FlexColumnBreakInfo* flex_column_break_info)
{
    DCHECK(layout_result || block_size_override);
    ...
    if (flex_column_break_info) {
        found_earlier_break = flex_column_break_info->early_break && flex_column_break_info->early_break->GetBreakAppeal() > appeal_before;
    } else {
        found_earlier_break = builder->HasEarlyBreak() && builder->GetEarlyBreak().GetBreakAppeal() > appeal_before;
    }
    ...
    BreakBeforeChild(space, child, layout_result, fragmentainer_block_offset, fragmentainer_block_size, appeal_before,
        /* is_forced_break */ false, builder, block_size_override);
    return true;
}

const EarlyBreak* EnterEarlyBreakInChild(const BlockNode& child, const EarlyBreak& early_break)
{
    if (early_break.Type() != EarlyBreak::kBlock || early_break.GetBlockNode() != child) {
        return nullptr;
    }
...
bool IsEarlyBreakTarget(const EarlyBreak& early_break, const BoxFragmentBuilder& builder, const LayoutInputNode& child)
{
...
ConstraintSpace CreateConstraintSpaceForFragmentainer(const ConstraintSpace& parent_space, FragmentationType fragmentation_type, LogicalSize fragmentainer_size,
    LogicalSize percentage_resolution_size, bool balance_columns, BreakAppeal min_break_appeal)
{
    ConstraintSpaceBuilder space_builder(parent_space, parent_space.GetWritingDirection(), /* is_new_fc */ true);
...
BoxFragmentBuilder CreateContainerBuilderForMulticol(const BlockNode& multicol, const ConstraintSpace& space, const FragmentGeometry& fragment_geometry)
{
    const ComputedStyle* style = &multicol.Style();
    BoxFragmentBuilder multicol_container_builder(multicol, style, space, style->GetWritingDirection(),
...
{
    WritingDirectionMode writing_direction_mode = multicol.Style().GetWritingDirection();
    ConstraintSpaceBuilder space_builder(writing_direction_mode.GetWritingMode(), writing_direction_mode,
        /* is_new_fc */ true);
...
const BlockBreakToken* FindPreviousBreakToken(const PhysicalBoxFragment& fragment)
{
    ...
    if (const BlockBreakToken* break_token = fragment.GetBreakToken()) {
        ...
        previous_fragment = box->GetPhysicalFragment(break_token->SequenceNumber() - 1);
    } else {
        // This is the last fragment, so its incoming break token will be the
        // outgoing one from the penultimate fragment.
        previous_fragment = box->GetPhysicalFragment(box->PhysicalFragmentCount() - 2);
    }
...
wtf_size_t PreviousInnerFragmentainerIndex(const PhysicalBoxFragment& fragment)
{
...
PhysicalOffset OffsetInStitchedFragments(const PhysicalBoxFragment& fragment, PhysicalSize* out_stitched_fragments_size)
{
    ...
    if (first_fragment.GetBreakToken() && first_fragment.GetBreakToken()->IsRepeated()) {
        // Repeated content isn't stitched.
        stitched_block_size = LogicalFragment(writing_direction, first_fragment).BlockSize();
    } else {
        ...
        if (fragment.IsOnlyForNode()) {
            stitched_block_size = LogicalFragment(writing_direction, fragment).BlockSize();
        } else {
            ...
            while (idx) {
                const PhysicalBoxFragment* walker = layout_box->GetPhysicalFragment(idx);
                stitched_block_size = LogicalFragment(writing_direction, *walker).BlockSize();

                // Look at the preceding break token.
                idx--;
                const BlockBreakToken* break_token = layout_box->GetPhysicalFragment(idx)->GetBreakToken();
    ...
    LogicalSize stitched_fragments_logical_size(LogicalFragment(writing_direction, fragment).InlineSize(), stitched_block_size);
    PhysicalSize stitched_fragments_physical_size(ToPhysicalSize(stitched_fragments_logical_size, writing_direction.GetWritingMode()));
    ...
    WritingModeConverter converter(writing_direction, stitched_fragments_physical_size);
    return converter.ToPhysical(offset_in_stitched_box, fragment.Size());
}

LayoutUnit BlockSizeForFragmentation(const LayoutResult& result, WritingDirectionMode container_writing_direction)
{
    ...
    if (block_size == kIndefiniteSize) {
        ...
        LogicalSize logical_size = result.GetPhysicalFragment().Size().ConvertToLogical(writing_mode);
...
bool CanPaintMultipleFragments(const LayoutObject& layout_object)
{
    ...
    // elements...).
    if (layout_box->IsScrollContainer() && !layout_object.GetDocument().Printing())
        return false;
