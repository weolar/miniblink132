...
BlockContentAlignment ComputeContentAlignment(const ComputedStyle& style, bool behave_like_table_cell, UseCounter* use_counter)
{
    ...
    if (use_counter) {
        if (!behave_like_table_cell) {
            if (position != ContentPosition::kNormal && position != ContentPosition::kStart && position != ContentPosition::kBaseline
                && position != ContentPosition::kFlexStart) {
                UseCounter::Count(*use_counter, WebFeature::kEffectiveAlignContentForBlock);
            }
        } else if (position != ContentPosition::kNormal && position != ContentPosition::kCenter) {
            UseCounter::Count(*use_counter, WebFeature::kEffectiveAlignContentForTableCell);
        }
    }
    ...
    case ContentPosition::kCenter:
        return is_safe ? BlockContentAlignment::kSafeCenter : BlockContentAlignment::kUnsafeCenter;

    case ContentPosition::kEnd:
    case ContentPosition::kFlexEnd:
        return is_safe ? BlockContentAlignment::kSafeEnd : BlockContentAlignment::kUnsafeEnd;
...
LayoutUnit CalculateOutOfFlowStaticInlineLevelOffset(
    const ComputedStyle& container_style, const BfcOffset& origin_bfc_offset, const ExclusionSpace& exclusion_space, LayoutUnit child_available_inline_size)
{
    ...
    LayoutOpportunity opportunity = exclusion_space.FindLayoutOpportunity(origin_bfc_offset, child_available_inline_size);

    LayoutUnit child_line_offset = IsLtr(direction) ? opportunity.rect.LineStartOffset() : opportunity.rect.LineEndOffset();

    LayoutUnit relative_line_offset = child_line_offset - origin_bfc_offset.line_offset;
    ...
    LayoutUnit inline_offset = IsLtr(direction) ? relative_line_offset : child_available_inline_size - relative_line_offset;

    // Adjust for text alignment, within the layout opportunity.
    LayoutUnit line_offset = LineOffsetForTextAlign(container_style.GetTextAlign(), direction, opportunity.rect.InlineSize());
    ...
    inline_offset += MinimumValueForLength(container_style.TextIndent(), child_available_inline_size);

    return inline_offset;
}

BlockContentAlignment ComputeContentAlignmentForBlock(const ComputedStyle& style, UseCounter* use_counter)
{
...
BlockContentAlignment ComputeContentAlignmentForTableCell(const ComputedStyle& style, UseCounter* use_counter)
{
    return ComputeContentAlignment(style, /*behave_like_table_cell=*/true, use_counter);
}

void AlignBlockContent(const ComputedStyle& style, const BlockBreakToken* break_token, LayoutUnit content_block_size, BoxFragmentBuilder& builder)
{
    ...
    BlockContentAlignment alignment = ComputeContentAlignmentForBlock(style, &builder.Node().GetDocument());
