...
PhysicalFragmentRareData::PhysicalFragmentRareData(const PhysicalRect* scrollable_overflow, const PhysicalBoxStrut* borders, const PhysicalBoxStrut* scrollbar,
    const PhysicalBoxStrut* padding, std::optional<PhysicalRect> inflow_bounds, BoxFragmentBuilder& builder, wtf_size_t num_fields)
    ...
    , reading_flow_elements_(
          builder.reading_flow_elements_.size() ? MakeGarbageCollected<HeapVector<Member<Element>>>(builder.reading_flow_elements_) : nullptr)
{
    field_list_.ReserveInitialCapacity(num_fields);
    ...
    if (scrollable_overflow) {
        SetField(FieldId::kScrollableOverflow).scrollable_overflow = *scrollable_overflow;
    }
    if (borders) {
    ...
    if (builder.frame_set_layout_data_) {
        SetField(FieldId::kFrameSetLayoutData).frame_set_layout_data = std::move(builder.frame_set_layout_data_);
    }
    if (builder.table_grid_rect_) {
        SetField(FieldId::kTableGridRect).table_grid_rect = *builder.table_grid_rect_;
    }
    if (builder.table_collapsed_borders_geometry_) {
        SetField(FieldId::kTableCollapsedBordersGeometry).table_collapsed_borders_geometry = std::move(builder.table_collapsed_borders_geometry_);
    }
    if (builder.table_cell_column_index_) {
        SetField(FieldId::kTableCellColumnIndex).table_cell_column_index = *builder.table_cell_column_index_;
    }
    if (!builder.table_section_row_offsets_.empty()) {
        SetField(FieldId::kTableSectionStartRowIndex).table_section_start_row_index = builder.table_section_start_row_index_;
        SetField(FieldId::kTableSectionRowOffsets).table_section_row_offsets = std::move(builder.table_section_row_offsets_);
    }
    ...
    if (!builder.table_column_geometries_.empty()) {
        table_column_geometries_ = MakeGarbageCollected<TableFragmentData::ColumnGeometries>(builder.table_column_geometries_);
    }
...
#define CLONE_IF_EXISTS(id, name, source)                                                                                                                      \
    if (const auto* field = source.GetField(FieldId::id)) {                                                                                                    \
        SetField(FieldId::id).name = std::make_unique<decltype(field->name)::element_type>(*field->name);                                                      \
    }

PhysicalFragmentRareData::PhysicalFragmentRareData(const PhysicalFragmentRareData& other)
    ...
    , table_column_geometries_(other.table_column_geometries_)
{
    ...
    CLONE_IF_EXISTS(kTableCollapsedBordersGeometry, table_collapsed_borders_geometry, other);
    SET_IF_EXISTS(kTableCellColumnIndex, table_cell_column_index, other);
    SET_IF_EXISTS(kTableSectionStartRowIndex, table_section_start_row_index, other);
...
PhysicalFragmentRareData::RareField::RareField(PhysicalFragmentRareData::FieldId field_id)
    : type(field_id)
{
...
PhysicalFragmentRareData::RareField::RareField(PhysicalFragmentRareData::RareField&& other)
    : type(other.type)
{
