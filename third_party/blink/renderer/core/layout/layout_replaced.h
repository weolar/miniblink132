...
public:
    ...
    virtual PhysicalRect ReplacedContentRectFrom(const PhysicalRect& base_content_rect) const;
    ...
    virtual void PaintReplaced(const PaintInfo&, const PhysicalOffset& paint_offset) const
    {
        NOT_DESTROYED();
    }
    ...
    {
        NOT_DESTROYED();
        return StyleRef().GetObjectFit() != ComputedStyleInitialValues::InitialObjectFit();
    }
...
protected:
    ...
    bool IsInSelfHitTestingPhase(HitTestPhase phase) const override
    {
        ...
        return element && element->IsReplacedElementRespectingCSSOverflow() && phase == HitTestPhase::kSelfBlockBackground;
    }
    ...
    PhysicalRect ComputeReplacedContentRect(const PhysicalRect& base_content_rect, const PhysicalSize* overridden_intrinsic_size = nullptr) const;
    ...
    std::optional<PhysicalRect> ComputeObjectViewBoxRect(const PhysicalSize* overridden_intrinsic_size = nullptr) const;

private:
    PhysicalRect ComputeObjectFitAndPositionRect(const PhysicalRect& base_content_rect, const PhysicalSize* overridden_intrinsic_size) const;
...
template <> struct DowncastTraits<LayoutReplaced> {
