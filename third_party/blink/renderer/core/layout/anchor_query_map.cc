...
struct StitchedAnchorReference : public GarbageCollected<StitchedAnchorReference> {
    StitchedAnchorReference(const LayoutObject& layout_object, const LogicalRect& rect, const FragmentainerContext& fragmentainer)
    ...
    LogicalAnchorReference* GetStitchedAnchorReference() const
    {
        ...
        return MakeGarbageCollected<LogicalAnchorReference>(*layout_object, StitchedRect(), /* is_out_of_flow */ false, nullptr);
    }

    void Unite(const LogicalRect& other_rect, const LogicalOffset& fragmentainer_offset)
    {
        ...
        // fragmentainer.
        const LogicalRect other_rect_in_first_fragmentainer = other_rect + (fragmentainer_offset - first_fragmentainer_offset);
...
struct StitchedAnchorQuery : public GarbageCollected<StitchedAnchorQuery>, public AnchorQueryBase<StitchedAnchorReference> {
    ...
    void AddAnchorQuery(const PhysicalFragment& fragment, const PhysicalOffset& offset_from_fragmentainer, const FragmentainerContext& fragmentainer)
    {
        ...
        for (auto entry : *anchor_query) {
            ...
            AddAnchorReference(
                entry.key, *entry.value->layout_object, entry.value->rect + offset_from_fragmentainer, fragmentainer, Conflict::kLastInCallOrder);
        }
    }

    void AddAnchorReference(const AnchorKey& key, const LayoutObject& new_object, const PhysicalRect& physical_rect_in_fragmentainer,
        const FragmentainerContext& fragmentainer, Conflict conflict)
    {
        const LogicalRect rect_in_fragmentainer = fragmentainer.converter.ToLogical(physical_rect_in_fragmentainer);
        auto* new_value = MakeGarbageCollected<StitchedAnchorReference>(new_object, rect_in_fragmentainer, fragmentainer);
...
public:
    StitchedAnchorQueries(const LayoutBox& root, const HeapHashSet<Member<const LayoutObject>>& anchored_oof_containers_and_ancestors)
        : anchored_oof_containers_and_ancestors_(anchored_oof_containers_and_ancestors)
        , root_(root)
    {
    }

    void AddChildren( //base::span<const LogicalFragmentLink> children,
        const LogicalFragmentLink* children, size_t children_size, const FragmentItemsBuilder::ItemWithOffsetList* items, const WritingModeConverter& converter)
    {
        const FragmentainerContext fragmentainer { {}, {}, converter };
        if (items) {
            for (const FragmentItemsBuilder::ItemWithOffset& item_with_offset : *items) {
                const FragmentItem& item = item_with_offset.item;
                if (const PhysicalBoxFragment* fragment = item.BoxFragment()) {
                    AddBoxChild(*fragment, item.OffsetInContainerFragment(), fragmentainer);
                }
            }
        }
        ...
        for (size_t i = 0; i < children_size; ++i) {
            const LogicalFragmentLink& child = *(children + i);
            DCHECK(!child->IsFragmentainerBox());
            DCHECK(!child->IsColumnSpanAll());
            const PhysicalOffset child_offset = converter.ToPhysical(child.offset, child->Size());
            AddChild(*child, child_offset, fragmentainer);
        }
    }

    void AddFragmentainerChildren(
        /*base::span<const LogicalFragmentLink>*/ const LogicalFragmentLink* children, size_t children_size, WritingDirectionMode writing_direction)
    {
        ...
        for (size_t i = 0; i < children_size; ++i) {
            ...
            if (child->IsFragmentainerBox()) {
                const FragmentainerContext fragmentainer { child.offset, fragmentainer_stitched_offset, { writing_direction, child->Size() } };
                AddChild(*child, /* offset_from_fragmentainer */ {}, fragmentainer);
                fragmentainer_stitched_offset += child->Size().ConvertToLogical(writing_direction.GetWritingMode()).block_size;
                continue;
            }
    ...
    void AddChild(const PhysicalFragment& fragment, const PhysicalOffset& offset_from_fragmentainer, const FragmentainerContext& fragmentainer)
    {
    ...
    void AddBoxChild(const PhysicalBoxFragment& fragment, const PhysicalOffset& offset_from_fragmentainer, const FragmentainerContext& fragmentainer)
    {
        ...
        if (const LayoutObject* layout_object = fragment.GetLayoutObject()) {
            if (!anchored_oof_containers_and_ancestors_.Contains(layout_object))
                return;
            if (layout_object->CanContainAbsolutePositionObjects() || layout_object->CanContainFixedPositionObjects()) {
                EnsureStitchedAnchorQuery(*layout_object).AddAnchorQuery(fragment, offset_from_fragmentainer, fragmentainer);
            }
        }

        if (fragment.IsFragmentationContextRoot()) {
            AddFragmentationContextRootChild(fragment, offset_from_fragmentainer, fragmentainer);
            return;
        }
        ...
        if (const FragmentItems* items = fragment.Items()) {
            for (InlineCursor cursor(fragment, *items); cursor; cursor.MoveToNext()) {
                if (cursor.Current().IsInlineBox()) {
                    ...
                    AddBoxChild(*cursor.Current().BoxFragment(), offset_from_fragmentainer + cursor.Current()->OffsetInContainerFragment(), fragmentainer);
                }
            }
        }
        ...
        for (const PhysicalFragmentLink& child : fragment.Children()) {
            DCHECK(!child->IsFragmentainerBox());
            const auto child_offset_from_fragmentainer = offset_from_fragmentainer + child.offset;
            AddChild(*child, child_offset_from_fragmentainer, fragmentainer);
        }
    }

    void AddFragmentationContextRootChild(
        const PhysicalBoxFragment& fragment, const PhysicalOffset& offset_from_fragmentainer, const FragmentainerContext& fragmentainer)
    {
        ...
        for (const PhysicalFragmentLink& child : fragment.Children()) {
            const LogicalOffset child_offset
                = fragmentainer.converter.ToLogical(offset_from_fragmentainer + child.offset, child->Size()) + fragmentainer.offset;
            children.push_back(LogicalFragmentLink { child.fragment, child_offset });
        }
        AddFragmentainerChildren(children.data(), children.size(), fragmentainer.converter.GetWritingDirection());
    }

    void AddOutOfFlowChild(const PhysicalBoxFragment& fragment, const PhysicalOffset& offset_from_fragmentainer, const FragmentainerContext& fragmentainer)
    {
        DCHECK(fragment.IsOutOfFlowPositioned());
        if (!fragment.Style().AnchorName() && !fragment.IsImplicitAnchor() && !fragment.AnchorQuery()) {
            return;
        }
        ...
        while (containing_block && containing_block != root_ && !skip_info.AncestorSkipped()) {
            StitchedAnchorQuery& query = EnsureStitchedAnchorQuery(*containing_block);
            if (fragment.Style().AnchorName()) {
                for (const ScopedCSSName* name : fragment.Style().AnchorName()->GetNames()) {
                    query.AddAnchorReference(name, *fragment.GetLayoutObject(), { offset_from_fragmentainer, fragment.Size() }, fragmentainer,
                        StitchedAnchorQuery::Conflict::kOverwriteIfAfter);
                }
            }
            if (fragment.IsImplicitAnchor()) {
                query.AddAnchorReference(layout_object, *fragment.GetLayoutObject(), { offset_from_fragmentainer, fragment.Size() }, fragmentainer,
                    StitchedAnchorQuery::Conflict::kOverwriteIfAfter);
            }
    ...
    StitchedAnchorQuery& EnsureStitchedAnchorQuery(const LayoutObject& containing_block)
    {
        const auto result = anchor_queries_.insert(&containing_block, MakeGarbageCollected<StitchedAnchorQuery>());
        DCHECK(result.stored_value->value);
        return *result.stored_value->value;
    }

    HeapHashMap<Member<const LayoutObject>, Member<StitchedAnchorQuery>> anchor_queries_;
    // The set of |LayoutObject| to traverse. When adding children, children not
    // in this set are skipped.
    const HeapHashSet<Member<const LayoutObject>>& anchored_oof_containers_and_ancestors_;
    const LayoutBox& root_;
};

} // namespace

LogicalAnchorQueryMap::LogicalAnchorQueryMap(const LayoutBox& root_box, const LogicalFragmentLinkVector& children, WritingDirectionMode writing_direction)
    : root_box_(root_box)
...
void LogicalAnchorQueryMap::SetChildren(const LogicalFragmentLinkVector& children)
{
    children_ = &children;
...
const LogicalAnchorQuery& LogicalAnchorQueryMap::AnchorQuery(const LayoutObject& containing_block) const
{
    DCHECK(&containing_block);
    DCHECK(containing_block.CanContainAbsolutePositionObjects() || containing_block.CanContainFixedPositionObjects());
...
void LogicalAnchorQueryMap::Update(const LayoutObject& layout_object) const
{
    ...
    for (const LayoutObject* runner = &layout_object; runner && runner != &root_box_; runner = runner->Parent()) {
    ...
    StitchedAnchorQueries stitched_anchor_queries(root_box_, anchored_oof_containers_and_ancestors);
    if (converter_) {
        stitched_anchor_queries.AddChildren(children_->data(), children_->size(), items_, *converter_);
    } else {
        stitched_anchor_queries.AddFragmentainerChildren(children_->data(), children_->size(), writing_direction_);
    }
    ...
    for (const auto& it : stitched_anchor_queries.anchor_queries_) {
        const auto result = queries_.insert(it.key, it.value->GetStitchedAnchorQuery());
