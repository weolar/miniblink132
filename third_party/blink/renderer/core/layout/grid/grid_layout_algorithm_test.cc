...
#define EXPECT_GRID_AREA(area, expected_column_start, expected_column_end, expected_row_start, expected_row_end)                                               \
...
protected:
    ...
    void BuildGridItemsAndTrackCollections(GridLayoutAlgorithm& algorithm)
    {
        ...
        algorithm.ComputeGridGeometry(grid_sizing_tree, &unused_intrinsic_block_size);
    ...
    const GridSizingTrackCollection& TrackCollection(GridTrackSizingDirection track_direction)
    {
        const auto& track_collection = (track_direction == kForColumns) ? layout_data_.Columns() : layout_data_.Rows();
    ...
    LayoutUnit BaseRowSizeForChild(const GridLayoutAlgorithm& algorithm, wtf_size_t index)
    {
        return algorithm.ComputeGridItemAvailableSize(GridItem(index), layout_data_.Rows());
    }
    ...
    {
        return cached_grid_items_.Size();
    }
    ...
    Vector<wtf_size_t> GridItemsWithColumnSpanProperty(TrackSpanProperties::PropertyId property)
    {
    ...
    Vector<wtf_size_t> GridItemsWithRowSpanProperty(TrackSpanProperties::PropertyId property)
    {
        Vector<wtf_size_t> results;
        for (wtf_size_t i = 0; i < GridItemCount(); ++i) {
    ...
    Vector<LayoutUnit> BaseSizes(GridTrackSizingDirection track_direction)
    {
        ...
        for (auto set_iterator = collection.GetConstSetIterator(); !set_iterator.IsAtEnd(); set_iterator.MoveToNextSet()) {
...
TEST_F(GridLayoutAlgorithmTest, GridLayoutAlgorithmResolveFixedTrackSizes)
{
    ...
    Vector<LayoutUnit> expected_column_base_sizes = { LayoutUnit(25), LayoutUnit(60), LayoutUnit(15) };
    Vector<LayoutUnit> expected_column_growth_limits = { LayoutUnit(25), LayoutUnit(60), LayoutUnit(15) };
    ...
    Vector<LayoutUnit> expected_row_base_sizes = { LayoutUnit(80), LayoutUnit(50), LayoutUnit(70) };
    Vector<LayoutUnit> expected_row_growth_limits = { LayoutUnit(100), LayoutUnit(50), LayoutUnit(70) };
...
TEST_F(GridLayoutAlgorithmTest, GridLayoutAlgorithmDetermineGridItemsSpanningIntrinsicOrFlexTracks)
{
    SetBodyInnerHTML(R"HTML(
    <style>
    #grid {
    ...
    Vector<wtf_size_t> actual_items = GridItemsWithColumnSpanProperty(TrackSpanProperties::kHasIntrinsicTrack);
    EXPECT_EQ(expected_grid_items_spanning_intrinsic_track.size(), actual_items.size());
    for (wtf_size_t i = 0; i < actual_items.size(); ++i)
        EXPECT_EQ(expected_grid_items_spanning_intrinsic_track[i], actual_items[i]);

    actual_items = GridItemsWithColumnSpanProperty(TrackSpanProperties::kHasFlexibleTrack);
    EXPECT_EQ(expected_grid_items_spanning_flex_track.size(), actual_items.size());
    for (wtf_size_t i = 0; i < actual_items.size(); ++i)
    ...
    actual_items = GridItemsWithRowSpanProperty(TrackSpanProperties::kHasIntrinsicTrack);
    EXPECT_EQ(expected_grid_items_spanning_intrinsic_track.size(), actual_items.size());
    for (wtf_size_t i = 0; i < actual_items.size(); ++i)
        EXPECT_EQ(expected_grid_items_spanning_intrinsic_track[i], actual_items[i]);

    actual_items = GridItemsWithRowSpanProperty(TrackSpanProperties::kHasFlexibleTrack);
    EXPECT_EQ(expected_grid_items_spanning_flex_track.size(), actual_items.size());
    for (wtf_size_t i = 0; i < actual_items.size(); ++i)
        EXPECT_EQ(expected_grid_items_spanning_flex_track[i], actual_items[i]);
}
...
TEST_F(GridLayoutAlgorithmTest, NGGridAxisType)
{
    ...
    EXPECT_EQ(grid_style.GridTemplateColumns().axis_type, GridAxisType::kStandaloneAxis);
    EXPECT_EQ(grid_style.GridTemplateRows().axis_type, GridAxisType::kStandaloneAxis);
    EXPECT_EQ(subgrid_style.GridTemplateColumns().axis_type, GridAxisType::kSubgriddedAxis);
    EXPECT_EQ(subgrid_style.GridTemplateRows().axis_type, GridAxisType::kSubgriddedAxis);
}
...
TEST_F(GridLayoutAlgorithmTest, SubgridLineNameList)
{
    ...
    BlockNode subgrid_node(GetLayoutBoxByElementId("subgrid"));
    const ComputedStyle& subgrid_style = subgrid_node.Style();
    const ComputedGridTrackList& computed_grid_column_track_list = subgrid_style.GridTemplateColumns();
    const ComputedGridTrackList& computed_grid_row_track_list = subgrid_style.GridTemplateRows();

    EXPECT_EQ(computed_grid_column_track_list.axis_type, GridAxisType::kSubgriddedAxis);
    EXPECT_EQ(computed_grid_row_track_list.axis_type, GridAxisType::kSubgriddedAxis);

    EXPECT_TRUE(computed_grid_column_track_list.ordered_named_grid_lines.empty());

    const OrderedNamedGridLines& ordered_named_grid_row_lines = computed_grid_row_track_list.ordered_named_grid_lines;
    EXPECT_EQ(ordered_named_grid_row_lines.size(), 3u);

    const Vector<NamedGridLine> row_named_lines = { NamedGridLine(AtomicString("a")), NamedGridLine(AtomicString("b")), NamedGridLine(AtomicString("c")) };
    for (wtf_size_t i = 0; i < 3; ++i) {
        EXPECT_EQ(ordered_named_grid_row_lines.find(i)->value[0], row_named_lines[i]);
    }
}
...
TEST_F(GridLayoutAlgorithmTest, SubgridLineNameListWithRepeaters)
{
    ...
    const OrderedNamedGridLines& ordered_named_grid_column_lines = computed_grid_column_track_list.ordered_named_grid_lines;
    const OrderedNamedGridLines& auto_repeat_ordered_named_grid_column_lines = computed_grid_column_track_list.auto_repeat_ordered_named_grid_lines;
    ...
    const Vector<NamedGridLine> column_named_lines
        = { NamedGridLine(AtomicString("a")), NamedGridLine(AtomicString("b")), NamedGridLine(AtomicString("c")), NamedGridLine(AtomicString("d")) };

    EXPECT_EQ(ordered_named_grid_column_lines.find(0)->value[0], column_named_lines[0]);
    EXPECT_EQ(ordered_named_grid_column_lines.find(2)->value[0], column_named_lines[3]);
    for (wtf_size_t i = 0; i < 2; ++i) {
        EXPECT_EQ(auto_repeat_ordered_named_grid_column_lines.find(i)->value[0], column_named_lines[i + 1]);
    }

    const OrderedNamedGridLines& ordered_named_grid_row_lines = computed_grid_row_track_list.ordered_named_grid_lines;
    EXPECT_EQ(ordered_named_grid_row_lines.size(), 6u);

    const Vector<NamedGridLine> row_named_lines = { NamedGridLine(AtomicString("a")),
        NamedGridLine(AtomicString("b"), /* is_in_repeat */ true,
            /* is_first_repeat */ true),
        NamedGridLine(AtomicString("c"), /* is_in_repeat */ true,
            /* is_first_repeat */ true),
        NamedGridLine(AtomicString("b"), /* is_in_repeat */ true), NamedGridLine(AtomicString("c"), /* is_in_repeat */ true),
