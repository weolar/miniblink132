...
public:
    ...
    inline bool HasProperty(PropertyId id) const
    {
        return bitmask_ & id;
    }
    inline void Reset()
    {
        bitmask_ &= kIsCollapsed | kIsImplicit;
    }
    inline void SetProperty(PropertyId id)
    {
        bitmask_ |= id;
    }
...
public:
    ...
    GridRangeBuilder(const ComputedStyle& grid_style, const GridLineResolver& line_resolver, GridTrackSizingDirection track_direction, wtf_size_t start_offset);
    ...
    void EnsureTrackCoverage(wtf_size_t start_line, wtf_size_t span_length, wtf_size_t* grid_item_start_range_index, wtf_size_t* grid_item_end_range_index);
...
private:
    ...
    struct TrackBoundaryToRangePair {
        explicit TrackBoundaryToRangePair(wtf_size_t grid_line, wtf_size_t* grid_item_range_index_to_cache = nullptr)
            : grid_line(grid_line)
    ...
    GridRangeBuilder(const NGGridTrackList& explicit_tracks, const NGGridTrackList& implicit_tracks, wtf_size_t auto_repetitions);
...
public:
    ...
    virtual bool IsForSizing() const
    {
        return false;
    }
    ...
    LayoutUnit ComputeSetSpanSize(wtf_size_t begin_set_index, wtf_size_t end_set_index) const;
    ...
    GridLayoutTrackCollection CreateSubgridTrackCollection(wtf_size_t begin_range_index, wtf_size_t end_range_index, LayoutUnit subgrid_gutter_size,
        const BoxStrut& subgrid_margin, const BoxStrut& subgrid_border_scrollbar_padding, GridTrackSizingDirection subgrid_track_direction,
        bool is_opposite_direction_in_root_grid) const;

    GridTrackSizingDirection Direction() const
    {
        return track_direction_;
    }
    LayoutUnit GutterSize() const
    {
        return gutter_size_;
    }
...
protected:
    ...
    bool IsSpanningIndefiniteSet(wtf_size_t begin_set_index, wtf_size_t end_set_index) const;
...
struct CORE_EXPORT GridSet {
    ...
    GridSet(wtf_size_t track_count, const GridTrackSize& track_definition, bool is_available_size_indefinite);
...
class CORE_EXPORT GridSizingTrackCollection final : public GridLayoutTrackCollection {
    USING_FAST_MALLOC(GridSizingTrackCollection);

public:
    template <bool is_const> class CORE_EXPORT SetIteratorBase {
    public:
        using TrackCollectionPtr = typename std::conditional<is_const, const GridSizingTrackCollection*, GridSizingTrackCollection*>::type;
        using GridSetRef = typename std::conditional<is_const, const GridSet&, GridSet&>::type;

        SetIteratorBase(TrackCollectionPtr track_collection, wtf_size_t begin_set_index, wtf_size_t end_set_index)
    ...
    GridSizingTrackCollection& operator=(const GridSizingTrackCollection&) = delete;

    explicit GridSizingTrackCollection(GridRangeVector&& ranges, bool must_create_baselines = false, GridTrackSizingDirection track_direction = kForColumns);

    // This class should be specifically used for grid sizing.
    bool IsForSizing() const override
    {
        return true;
    }
    ...
    SetIterator GetSetIterator(wtf_size_t begin_set_index, wtf_size_t end_set_index);
    ...
    void BuildSets(const ComputedStyle& grid_style, LayoutUnit grid_available_size, LayoutUnit gutter_size);
    ...
    void FinalizeSetsGeometry(LayoutUnit first_set_offset, LayoutUnit override_gutter_size);
...
private:
    ...
    void BuildSets(const NGGridTrackList& explicit_track_list, const NGGridTrackList& implicit_track_list, bool is_available_size_indefinite = true);
...
template <> struct DowncastTraits<GridSizingTrackCollection> {
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::GridLayoutTrackCollection::SetGeometry)
