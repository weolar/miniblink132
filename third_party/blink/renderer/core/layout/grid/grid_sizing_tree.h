...
public:
    ...
    SubgriddedItemData(const GridItemData& item_data_in_parent, const GridLayoutData& parent_layout_data, WritingMode parent_writing_mode)
    ...
    {
        return item_data_in_parent_ != nullptr;
    }
    ...
    const GridLayoutTrackCollection& Columns(std::optional<WritingMode> container_writing_mode = std::nullopt) const
    {
        return (!container_writing_mode || IsParallelWritingMode(*container_writing_mode, parent_writing_mode_)) ? parent_layout_data_->Columns()
                                                                                                                 : parent_layout_data_->Rows();
    }

    const GridLayoutTrackCollection& Rows(std::optional<WritingMode> container_writing_mode = std::nullopt) const
    {
        return (!container_writing_mode || IsParallelWritingMode(*container_writing_mode, parent_writing_mode_)) ? parent_layout_data_->Rows()
...
public:
    ...
    GridTreeNode& TreeRootData() const
    {
        return At(0);
    }
    ...
    SubgriddedItemData LookupSubgriddedItemData(const GridItemData& grid_item) const;
...
private:
    ...
    HeapHashMap<Member<const LayoutBox>, SubgriddedItemData> subgridded_item_data_lookup_map_;
...
class GridSizingSubtree : public GridSubtree<GridSizingSubtree, const GridSizingTree*> {
    STACK_ALLOCATED();

public:
    GridSizingSubtree() = default;

    explicit GridSizingSubtree(const GridSizingTree& sizing_tree, wtf_size_t subtree_root = 0)
        : GridSubtree(&sizing_tree, subtree_root)
    {
    }

    GridSizingSubtree(const GridSizingTree* sizing_tree, wtf_size_t parent_end_index, wtf_size_t subtree_root)
    ...
    GridSizingSubtree SubgridSizingSubtree(const GridItemData& subgrid_data) const
    {
        DCHECK(grid_tree_);
        DCHECK(subgrid_data.IsSubgrid());

        return GridSizingSubtree(*grid_tree_,
    ...
    {
        return grid_tree_ && grid_tree_->LookupSubgridIndex(grid_node) == subtree_root_;
    }
    ...
    {
        DCHECK(grid_tree_);
        return grid_tree_->At(subtree_root_).layout_data.SizingCollection(track_direction);
    }
};
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::GridSizingTree::GridTreeNode)
