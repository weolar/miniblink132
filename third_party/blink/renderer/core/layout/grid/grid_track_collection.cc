...
wtf_size_t GridTrackCollectionBase::RangeIndexFromGridLine(wtf_size_t grid_line) const
{
    wtf_size_t upper = RangeCount();
    DCHECK_GT(upper, 0u);

    const wtf_size_t last_grid_line = RangeStartLine(upper - 1) + RangeTrackCount(upper - 1);
...
GridRangeBuilder::GridRangeBuilder(
    const ComputedStyle& grid_style, const GridLineResolver& line_resolver, GridTrackSizingDirection track_direction, wtf_size_t start_offset)
    : auto_repetitions_(line_resolver.AutoRepetitions(track_direction))
    , start_offset_(start_offset)
    , must_sort_grid_lines_(false)
    , explicit_tracks_((track_direction == kForColumns) ? grid_style.GridTemplateColumns().track_list : grid_style.GridTemplateRows().track_list)
    , implicit_tracks_((track_direction == kForColumns) ? grid_style.GridAutoColumns() : grid_style.GridAutoRows())
{
    ...
    for (wtf_size_t i = 0; i < repeater_count; ++i) {
        const wtf_size_t repeater_track_count = explicit_tracks_.RepeatCount(i, auto_repetitions_) * explicit_tracks_.RepeatSize(i);
    ...
    if (const auto& grid_template_areas = grid_style.GridTemplateAreas()) {
        named_grid_area_end_line += (track_direction == kForColumns) ? grid_template_areas->column_count : grid_template_areas->row_count;
    }
...
void GridRangeBuilder::EnsureTrackCoverage(
    wtf_size_t start_line, wtf_size_t span_length, wtf_size_t* grid_item_start_range_index, wtf_size_t* grid_item_end_range_index)
{
...
GridRangeVector GridRangeBuilder::FinalizeRanges()
{
    ...
    if (must_sort_grid_lines_) {
        auto CompareTrackBoundaries = [](const TrackBoundaryToRangePair& a, const TrackBoundaryToRangePair& b) { return a.grid_line < b.grid_line; };
    ...
    wtf_size_t next_explicit_repeater_start = explicit_repeater_count ? start_offset_ : kNotFound;
    ...
    while (true) {
        ...
        while (start_line_index < grid_line_count && current_range_start_line >= start_lines_[start_line_index].grid_line) {
            ++start_line_index;
            ++open_items_or_repeaters;
        }

        // Identify ending tracks index.
        while (end_line_index < grid_line_count && current_range_start_line >= end_lines_[end_line_index].grid_line) {
            ++end_line_index;
            --open_items_or_repeaters;
        ...
        const wtf_size_t next_start_line = (start_line_index < grid_line_count) ? start_lines_[start_line_index].grid_line : kNotFound;
        ...
        while (current_range_start_line == next_explicit_repeater_start) {
            ...
            is_in_auto_fit_range = explicit_tracks_.RepeatType(current_explicit_repeater_index) == NGGridTrackRepeater::RepeatType::kAutoFit;
            next_explicit_repeater_start += explicit_tracks_.RepeatSize(current_explicit_repeater_index)
                * explicit_tracks_.RepeatCount(current_explicit_repeater_index, auto_repetitions_);
        }
        ...
        range.track_count = std::min(next_start_line, next_end_line) - current_range_start_line;
        ...
        if (explicit_tracks_.IsSubgriddedAxis()) {
            ...
            current_repeater_size = explicit_tracks_.RepeatSize(current_explicit_repeater_index);
            ...
            range.repeater_offset = (current_range_start_line - current_explicit_grid_line) % current_repeater_size;
        } else {
            range.SetIsImplicit();
            if (!implicit_tracks_.RepeaterCount()) {
                ...
                range.repeater_offset
                    = (current_range_start_line + current_repeater_size - current_explicit_grid_line % current_repeater_size) % current_repeater_size;
            }
        }
        ...
        if (start_line_index != 0) {
            ...
            for (wtf_size_t line_index = start_line_index - 1; start_lines_[line_index].grid_line == range.start_line; --line_index) {
                if (start_lines_[line_index].grid_item_range_index_to_cache) {
                    *start_lines_[line_index].grid_item_range_index_to_cache = ranges.size();
                }
        ...
        for (wtf_size_t line_index = end_line_index; line_index < grid_line_count && end_lines_[line_index].grid_line == end_line; ++line_index) {
...
GridRangeBuilder::GridRangeBuilder(const NGGridTrackList& explicit_tracks, const NGGridTrackList& implicit_tracks, wtf_size_t auto_repetitions)
...
GridSet::GridSet(wtf_size_t track_count, const GridTrackSize& track_definition, bool is_available_size_indefinite)
    ...
    , fit_content_limit(kIndefiniteSize)
{
    if (track_size.IsFitContent()) {
        ...
        if (is_available_size_indefinite && track_size.FitContentTrackBreadth().HasPercent()) {
        ...
        const auto normalized_min_track_sizing_function
            = ((is_available_size_indefinite && track_size.MinTrackBreadth().HasPercent()) || track_size.HasFlexMinTrackBreadth())
            ? Length::Auto()
            : track_size.MinTrackBreadth();

        const auto normalized_max_track_sizing_function
            = (is_available_size_indefinite && track_size.MaxTrackBreadth().HasPercent()) ? Length::Auto() : track_size.MaxTrackBreadth();

        track_size = GridTrackSize(normalized_min_track_sizing_function, normalized_max_track_sizing_function);
    }
    DCHECK(track_size.GetType() == kFitContentTrackSizing || track_size.GetType() == kMinMaxTrackSizing);
}
...
void GridSet::IncreaseGrowthLimit(LayoutUnit new_growth_limit)
{
    ...
    DCHECK(!IsGrowthLimitLessThanBaseSize() && (growth_limit == kIndefiniteSize || growth_limit <= new_growth_limit));
...
bool GridLayoutTrackCollection::operator==(const GridLayoutTrackCollection& other) const
{
    return gutter_size_ == other.gutter_size_ && track_direction_ == other.track_direction_
        && accumulated_gutter_size_delta_ == other.accumulated_gutter_size_delta_ && accumulated_start_extra_margin_ == other.accumulated_start_extra_margin_
        && accumulated_end_extra_margin_ == other.accumulated_end_extra_margin_ && baselines_.has_value() == other.baselines_.has_value()
        && (!baselines_ || (baselines_->major == other.baselines_->major && baselines_->minor == other.baselines_->minor))
        && last_indefinite_index_ == other.last_indefinite_index_ && ranges_ == other.ranges_ && sets_geometry_ == other.sets_geometry_;
}

wtf_size_t GridLayoutTrackCollection::RangeStartLine(wtf_size_t range_index) const
{
    DCHECK_LT(range_index, ranges_.size());
    return ranges_[range_index].start_line;
}

wtf_size_t GridLayoutTrackCollection::RangeTrackCount(wtf_size_t range_index) const
{
    DCHECK_LT(range_index, ranges_.size());
    return ranges_[range_index].track_count;
}

wtf_size_t GridLayoutTrackCollection::RangeSetCount(wtf_size_t range_index) const
{
    DCHECK_LT(range_index, ranges_.size());
    return ranges_[range_index].set_count;
}

wtf_size_t GridLayoutTrackCollection::RangeBeginSetIndex(wtf_size_t range_index) const
{
    DCHECK_LT(range_index, ranges_.size());
    return ranges_[range_index].begin_set_index;
}

TrackSpanProperties GridLayoutTrackCollection::RangeProperties(wtf_size_t range_index) const
{
    DCHECK_LT(range_index, ranges_.size());
...
bool GridLayoutTrackCollection::IsGridLineWithinImplicitGrid(wtf_size_t grid_line) const
{
...
wtf_size_t GridLayoutTrackCollection::GetSetTrackCount(wtf_size_t set_index) const
{
    DCHECK_LT(set_index + 1, sets_geometry_.size());
    return sets_geometry_[set_index + 1].track_count;
}

LayoutUnit GridLayoutTrackCollection::StartExtraMargin(wtf_size_t set_index) const
{
    return set_index ? accumulated_gutter_size_delta_ / 2 : accumulated_start_extra_margin_;
}

LayoutUnit GridLayoutTrackCollection::EndExtraMargin(wtf_size_t set_index) const
{
    return (set_index < sets_geometry_.size() - 1) ? accumulated_gutter_size_delta_ / 2 : accumulated_end_extra_margin_;
}

LayoutUnit GridLayoutTrackCollection::MajorBaseline(wtf_size_t set_index) const
{
    if (!baselines_) {
...
LayoutUnit GridLayoutTrackCollection::MinorBaseline(wtf_size_t set_index) const
{
    if (!baselines_) {
        return LayoutUnit::Min();
    }
...
void GridLayoutTrackCollection::AdjustSetOffsets(wtf_size_t set_index, LayoutUnit delta)
{
    DCHECK_LT(set_index, sets_geometry_.size());
...
LayoutUnit GridLayoutTrackCollection::ComputeSetSpanSize(wtf_size_t begin_set_index, wtf_size_t end_set_index) const
{
    ...
    return (GetSetOffset(end_set_index) - gutter_size_ - GetSetOffset(begin_set_index)).ClampNegativeToZero();
}

bool GridLayoutTrackCollection::IsSpanningIndefiniteSet(wtf_size_t begin_set_index, wtf_size_t end_set_index) const
{
    ...
    const wtf_size_t last_indefinite_index = last_indefinite_index_[end_set_index];

    return last_indefinite_index != kNotFound && begin_set_index <= last_indefinite_index;
}

GridLayoutTrackCollection GridLayoutTrackCollection::CreateSubgridTrackCollection(wtf_size_t begin_range_index, wtf_size_t end_range_index,
    LayoutUnit subgrid_gutter_size, const BoxStrut& subgrid_margin, const BoxStrut& subgrid_border_scrollbar_padding,
    GridTrackSizingDirection subgrid_track_direction, bool is_opposite_direction_in_root_grid) const
{
    ...
    const wtf_size_t end_set_index = ranges_[end_range_index].begin_set_index + ranges_[end_range_index].set_count;
    ...
    // Copy and translate the ranges in the subgrid's span.
    {
        ...
        for (wtf_size_t i = 0; i <= range_count; ++i) {
            ...
            const wtf_size_t current_index = is_opposite_direction_in_root_grid ? end_range_index - i : begin_range_index + i;

            auto& subgrid_translated_range = subgrid_ranges.emplace_back(ranges_[current_index]);
    ...
    const auto subgrid_margin_start = is_for_columns ? subgrid_margin.inline_start : subgrid_margin.block_start;

    const auto subgrid_border_scrollbar_padding_start
        = is_for_columns ? subgrid_border_scrollbar_padding.inline_start : subgrid_border_scrollbar_padding.block_start;

    const auto subgrid_margin_border_scrollbar_padding_start = subgrid_margin_start + subgrid_border_scrollbar_padding_start;
    const auto subgrid_margin_border_scrollbar_padding_end = is_for_columns ? subgrid_margin.inline_end + subgrid_border_scrollbar_padding.inline_end
                                                                            : subgrid_margin.block_end + subgrid_border_scrollbar_padding.block_end;
    ...
    {
        subgrid_track_collection.accumulated_gutter_size_delta_ = subgrid_gutter_size_delta + accumulated_gutter_size_delta_;
        ...
        subgrid_track_collection.accumulated_start_extra_margin_ = subgrid_margin_border_scrollbar_padding_start
            + (is_opposite_direction_in_root_grid ? EndExtraMargin(end_set_index) : StartExtraMargin(begin_set_index));

        subgrid_track_collection.accumulated_end_extra_margin_ = subgrid_margin_border_scrollbar_padding_end
            + (is_opposite_direction_in_root_grid ? StartExtraMargin(begin_set_index) : EndExtraMargin(end_set_index));

        // Opposite direction subgrids iterate backwards.
        const wtf_size_t first_set_index = is_opposite_direction_in_root_grid ? end_set_index : begin_set_index;
        ...
        for (wtf_size_t i = 1; i < set_span_size; ++i) {
            // Opposite direction subgrids need to iterate backwards.
            const wtf_size_t current_index = is_opposite_direction_in_root_grid ? end_set_index - i : begin_set_index + i;
            auto& set = subgrid_sets_geometry.emplace_back(sets_geometry_[current_index]);
            if (is_opposite_direction_in_root_grid) {
        ...
        const wtf_size_t last_set_index = is_opposite_direction_in_root_grid ? begin_set_index : end_set_index;
        auto& last_set = subgrid_sets_geometry.emplace_back(sets_geometry_[last_set_index]);
        ...
        last_set.offset += subgrid_gutter_size_delta - subgrid_margin_border_scrollbar_padding_end;
        DCHECK_GT(last_set.track_count, 0U);
    }

    // Copy the last indefinite indices in the subgrid's span.
    if (!last_indefinite_index_.empty()) {
        auto& subgrid_last_indefinite_index = subgrid_track_collection.last_indefinite_index_;
        ...
        for (wtf_size_t i = 0; i < set_span_size; ++i) {
            // Opposite direction subgrids need to iterate backwards.
            const wtf_size_t current_index = is_opposite_direction_in_root_grid ? end_set_index - i - 1 : begin_set_index + i;
            ...
            if (last_indefinite_index_[current_index + 1] != last_indefinite_index_[current_index]) {
    ...
    if (baselines_ && !baselines_->major.empty()) {
        ...
        for (wtf_size_t i = 0; i < set_span_size; ++i) {
            LayoutUnit major_adjust = (i == 0) ? subgrid_margin_border_scrollbar_padding_start : subgrid_gutter_size_delta / 2;
            LayoutUnit minor_adjust = (i == set_span_size - 1) ? subgrid_margin_border_scrollbar_padding_end : subgrid_gutter_size_delta / 2;
            if (is_opposite_direction_in_root_grid) {
                std::swap(major_adjust, minor_adjust);
            }
            const wtf_size_t current_index = is_opposite_direction_in_root_grid ? end_set_index - i - 1 : begin_set_index + i;
            subgrid_baselines.major.emplace_back(baselines_->major[current_index] - major_adjust);
            subgrid_baselines.minor.emplace_back(baselines_->minor[current_index] - minor_adjust);
        }
...
{
    return properties_.HasProperty(TrackSpanProperties::kIsDependentOnAvailableSize);
}

bool GridLayoutTrackCollection::HasIndefiniteSet() const
{
    return !last_indefinite_index_.empty() && last_indefinite_index_.back() != kNotFound;
}

GridSizingTrackCollection::GridSizingTrackCollection(GridRangeVector&& ranges, bool must_create_baselines, GridTrackSizingDirection track_direction)
...
GridSizingTrackCollection::SetIterator GridSizingTrackCollection::GetSetIterator()
{
    return SetIterator(this, 0, sets_.size());
}

GridSizingTrackCollection::ConstSetIterator GridSizingTrackCollection::GetConstSetIterator() const
{
    return ConstSetIterator(this, 0, sets_.size());
}

GridSizingTrackCollection::SetIterator GridSizingTrackCollection::GetSetIterator(wtf_size_t begin_set_index, wtf_size_t end_set_index)
{
...
void GridSizingTrackCollection::CacheInitializedSetsGeometry(LayoutUnit first_set_offset)
{
...
void GridSizingTrackCollection::FinalizeSetsGeometry(LayoutUnit first_set_offset, LayoutUnit override_gutter_size)
{
...
void GridSizingTrackCollection::SetMajorBaseline(wtf_size_t set_index, LayoutUnit candidate_baseline)
{
...
void GridSizingTrackCollection::SetMinorBaseline(wtf_size_t set_index, LayoutUnit candidate_baseline)
{
...
void GridSizingTrackCollection::BuildSets(const ComputedStyle& grid_style, LayoutUnit grid_available_size, LayoutUnit gutter_size)
{
    const bool is_for_columns = track_direction_ == kForColumns;
    gutter_size_ = gutter_size;

    BuildSets(is_for_columns ? grid_style.GridTemplateColumns().track_list : grid_style.GridTemplateRows().track_list,
        is_for_columns ? grid_style.GridAutoColumns() : grid_style.GridAutoRows(), grid_available_size == kIndefiniteSize);
    InitializeSets(grid_available_size);
}

void GridSizingTrackCollection::BuildSets(
    const NGGridTrackList& explicit_track_list, const NGGridTrackList& implicit_track_list, bool is_available_size_indefinite)
{
    properties_.Reset();
    sets_.Shrink(0);
    ...
    for (auto& range : ranges_) {
        ...
        auto CacheSetProperties = [&range](const GridSet& set) {
            ...
            if (set_track_size.HasFlexMaxTrackBreadth()) {
                ...
                range.properties.SetProperty(TrackSpanProperties::kIsDependentOnAvailableSize);
            }

            if (set_track_size.HasIntrinsicMinTrackBreadth() || set_track_size.HasIntrinsicMaxTrackBreadth()) {
        ...
        if (range.repeater_index == kNotFound) {
            ...
            const auto& specified_track_list = range.IsImplicit() ? implicit_track_list : explicit_track_list;

            const wtf_size_t current_repeater_size = specified_track_list.RepeatSize(range.repeater_index);
            ...
            const wtf_size_t floor_set_track_count = range.track_count / current_repeater_size;
            ...
            const wtf_size_t remaining_track_count = range.track_count % current_repeater_size;

            for (wtf_size_t i = 0; i < range.set_count; ++i) {
                const wtf_size_t set_track_count = floor_set_track_count + ((i < remaining_track_count) ? 1 : 0);
                const wtf_size_t set_repeater_offset = (range.repeater_offset + i) % current_repeater_size;
                const auto& set_track_size = specified_track_list.RepeatTrackSize(range.repeater_index, set_repeater_offset);
                ...
                CacheSetProperties(sets_.emplace_back(set_track_count, set_track_size, is_available_size_indefinite));
            }
        }
...
void GridSizingTrackCollection::InitializeSets(LayoutUnit grid_available_size)
{
    for (auto& set : sets_) {
        ...
        if (track_size.IsFitContent()) {
            ...
            DCHECK(!track_size.FitContentTrackBreadth().HasPercent() || grid_available_size != kIndefiniteSize);

            LayoutUnit fit_content_argument = MinimumValueForLength(track_size.FitContentTrackBreadth(), grid_available_size);
            set.fit_content_limit = fit_content_argument * set.track_count;
        }

        if (track_size.HasFixedMaxTrackBreadth()) {
            DCHECK(!track_size.MaxTrackBreadth().HasPercent() || grid_available_size != kIndefiniteSize);
            ...
            LayoutUnit fixed_max_breadth = MinimumValueForLength(track_size.MaxTrackBreadth(), grid_available_size);
        ...
        if (track_size.HasFixedMinTrackBreadth()) {
            DCHECK(!track_size.MinTrackBreadth().HasPercent() || grid_available_size != kIndefiniteSize);

            // A fixed sizing function: Resolve to an absolute length and use that
            // size as the track’s initial base size.
            LayoutUnit fixed_min_breadth = MinimumValueForLength(track_size.MinTrackBreadth(), grid_available_size);
