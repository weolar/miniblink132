...
public:
    ...
    {
        return grid_item_positions == other.grid_item_positions && column_start_offset == other.column_start_offset
            && row_start_offset == other.row_start_offset && line_resolver == other.line_resolver;
    }
    ...
    {
        return (track_direction == kForColumns) ? column_start_offset : row_start_offset;
    }
...
bool AreEqual(const std::unique_ptr<GridLayoutTrackCollection>& lhs, const std::unique_ptr<GridLayoutTrackCollection>& rhs)
{
...
public:
    ...
    {
        return (track_direction == kForColumns) ? !(columns_ && columns_->IsForSizing()) : !(rows_ && rows_->IsForSizing());
    }

    bool IsSubgridWithStandaloneAxis(GridTrackSizingDirection track_direction) const
    {
        return columns_ && rows_
            && ((track_direction == kForColumns) ? columns_->IsForSizing() && !rows_->IsForSizing() : rows_->IsForSizing() && !columns_->IsForSizing());
    }
    ...
    GridSizingTrackCollection& SizingCollection(GridTrackSizingDirection track_direction) const
    {
        DCHECK(!HasSubgriddedAxis(track_direction));

        return To<GridSizingTrackCollection>((track_direction == kForColumns) ? Columns() : Rows());
    }
    ...
    void SetTrackCollection(std::unique_ptr<GridLayoutTrackCollection> track_collection)
    {
...
public:
    struct GridTreeNode {
        GridTreeNode(const GridLayoutData& layout_data, wtf_size_t subtree_size)
            ...
            , has_unresolved_geometry(layout_data.Columns().HasIndefiniteSet() || layout_data.Rows().HasIndefiniteSet())
        {
        }
    ...
    bool AreSubtreesEqual(wtf_size_t subtree_root, const GridLayoutTree& other, wtf_size_t other_subtree_root) const
    {
        ...
        for (wtf_size_t i = 0; i < subtree_size; ++i) {
            if (other.LayoutData(other_subtree_root + i) != LayoutData(subtree_root + i)) {
                return false;
            }
        }
        return true;
...
class GridLayoutSubtree : public GridSubtree<GridLayoutSubtree, scoped_refptr<const GridLayoutTree>> {
    DISALLOW_NEW();

public:
    GridLayoutSubtree() = default;

    explicit GridLayoutSubtree(scoped_refptr<const GridLayoutTree> layout_tree, wtf_size_t subtree_root = 0)
        : GridSubtree(std::move(layout_tree), subtree_root)
    {
    }

    GridLayoutSubtree(const scoped_refptr<const GridLayoutTree>& layout_tree, wtf_size_t parent_end_index, wtf_size_t subtree_root)
    ...
    {
        return (grid_tree_ && other.grid_tree_) ? grid_tree_->AreSubtreesEqual(subtree_root_, *other.grid_tree_, other.subtree_root_)
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::GridLayoutTree::GridTreeNode)
