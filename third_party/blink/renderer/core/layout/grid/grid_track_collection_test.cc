...
#define EXPECT_SET(expected_size, expected_count, iterator)                                                                                                    \
    {                                                                                                                                                          \
        GridSet expected_set(expected_count, expected_size, /* is_content_box_size_definite */ false);                                                         \
...
protected:
    GridRangeBuilder CreateRangeBuilder(const NGGridTrackList& explicit_tracks, const NGGridTrackList& implicit_tracks, wtf_size_t auto_repetitions)
    {
    ...
    void InitializeSetsForSizingCollection(
        const NGGridTrackList& explicit_tracks, const NGGridTrackList& implicit_tracks, GridSizingTrackCollection* sizing_collection)
    {
        sizing_collection->BuildSets(explicit_tracks, implicit_tracks);
        sizing_collection->InitializeSets();
    }

    const GridRangeVector& GetRangesFrom(const GridSizingTrackCollection& sizing_collection)
    {
        return sizing_collection.ranges_;
    }

    GridSizingTrackCollection::SetIterator IteratorForRange(GridSizingTrackCollection& track_collection, wtf_size_t range_index)
    {
        const wtf_size_t begin_set_index = track_collection.RangeBeginSetIndex(range_index);

        return track_collection.GetSetIterator(begin_set_index, begin_set_index + track_collection.RangeSetCount(range_index));
    }
};
...
TEST_F(GridTrackCollectionTest, TestNGGridTrackList)
{
    ...
    EXPECT_TRUE(track_list.AddRepeater(CreateTrackSizes(2), NGGridTrackRepeater::RepeatType::kInteger, 4));
    ...
    EXPECT_FALSE(track_list.AddRepeater(CreateTrackSizes(0), NGGridTrackRepeater::RepeatType::kAutoFit));
    EXPECT_FALSE(track_list.AddRepeater(CreateTrackSizes(3), NGGridTrackRepeater::RepeatType::kNoRepeat, 0));
    ASSERT_EQ(1u, track_list.RepeaterCount());

    EXPECT_TRUE(track_list.AddRepeater(CreateTrackSizes(1), NGGridTrackRepeater::RepeatType::kInteger, kNotFound - 20));
    ...
    EXPECT_FALSE(track_list.AddRepeater(CreateTrackSizes(5), NGGridTrackRepeater::RepeatType::kInteger, 7));
    EXPECT_FALSE(track_list.AddRepeater(CreateTrackSizes(7), NGGridTrackRepeater::RepeatType::kInteger, 5));
    EXPECT_FALSE(track_list.AddRepeater(CreateTrackSizes(31), NGGridTrackRepeater::RepeatType::kAutoFill));
    ASSERT_EQ(2u, track_list.RepeaterCount());

    EXPECT_TRUE(track_list.AddRepeater(CreateTrackSizes(3), NGGridTrackRepeater::RepeatType::kAutoFit));
    ...
    EXPECT_FALSE(track_list.AddRepeater(CreateTrackSizes(3), NGGridTrackRepeater::RepeatType::kAutoFill));
    ASSERT_EQ(3u, track_list.RepeaterCount());
}
...
TEST_F(GridTrackCollectionTest, TestGridRangeBuilder)
{
    ...
    ASSERT_TRUE(explicit_tracks.AddRepeater(CreateTrackSizes(2), NGGridTrackRepeater::RepeatType::kInteger, 4));
    ASSERT_TRUE(explicit_tracks.AddRepeater(CreateTrackSizes(3), NGGridTrackRepeater::RepeatType::kAutoFill));
...
TEST_F(GridTrackCollectionTest, TestGridRangeBuilderCollapsed)
{
    NGGridTrackList explicit_tracks, implicit_tracks;
    ASSERT_TRUE(explicit_tracks.AddRepeater(CreateTrackSizes(2), NGGridTrackRepeater::RepeatType::kInteger, 4));
    ASSERT_TRUE(explicit_tracks.AddRepeater(CreateTrackSizes(3), NGGridTrackRepeater::RepeatType::kAutoFit));
    ASSERT_TRUE(explicit_tracks.AddRepeater(CreateTrackSizes(3), NGGridTrackRepeater::RepeatType::kInteger, 7));
...
TEST_F(GridTrackCollectionTest, TestGridRangeBuilderImplicit)
{
    ...
    ASSERT_TRUE(explicit_tracks.AddRepeater(CreateTrackSizes(2), NGGridTrackRepeater::RepeatType::kInteger, 4));
    ASSERT_TRUE(explicit_tracks.AddRepeater(CreateTrackSizes(3), NGGridTrackRepeater::RepeatType::kInteger, 3));
    ASSERT_TRUE(explicit_tracks.AddRepeater(CreateTrackSizes(3), NGGridTrackRepeater::RepeatType::kInteger, 7));
    ASSERT_EQ(3u, explicit_tracks.RepeaterCount());

    NGGridTrackList implicit_tracks;
    ASSERT_TRUE(implicit_tracks.AddRepeater(CreateTrackSizes(8), NGGridTrackRepeater::RepeatType::kInteger, 2));
...
{
    auto TestTrackSizeNormalization = [](const GridTrackSize& track_definition, const GridTrackSize& expected_track_size_in_definite_container,
        ...
        EXPECT_EQ(expected_track_size_in_definite_container, normalized_set.track_size);

        normalized_set = GridSet(/* track_count */ 1, track_definition,
            /* is_content_box_size_indefinite */ true);
        EXPECT_EQ(expected_track_size_in_indefinite_container, normalized_set.track_size);
    };

    // auto
    TestTrackSizeNormalization(GridTrackSize(Length::Auto()), GridTrackSize(Length::Auto(), Length::Auto()), GridTrackSize(Length::Auto(), Length::Auto()));
    // 10%
    TestTrackSizeNormalization(
        GridTrackSize(Length::Percent(10)), GridTrackSize(Length::Percent(10), Length::Percent(10)), GridTrackSize(Length::Auto(), Length::Auto()));
    // minmax(50px, 20%)
    TestTrackSizeNormalization(GridTrackSize(Length::Fixed(50), Length::Percent(20)), GridTrackSize(Length::Fixed(50), Length::Percent(20)),
        GridTrackSize(Length::Fixed(50), Length::Auto()));
    // min-content
    TestTrackSizeNormalization(GridTrackSize(Length::MinContent()), GridTrackSize(Length::MinContent(), Length::MinContent()),
        GridTrackSize(Length::MinContent(), Length::MinContent()));
    // max-content
    TestTrackSizeNormalization(GridTrackSize(Length::MaxContent()), GridTrackSize(Length::MaxContent(), Length::MaxContent()),
        GridTrackSize(Length::MaxContent(), Length::MaxContent()));
    // minmax(1fr, 3fr)
    TestTrackSizeNormalization(GridTrackSize(Length::Flex(1.0), Length::Flex(3.0)), GridTrackSize(Length::Auto(), Length::Flex(3.0)),
        GridTrackSize(Length::Auto(), Length::Flex(3.0)));
    // fit-content(40%)
    TestTrackSizeNormalization(GridTrackSize(Length::Percent(40), kFitContentTrackSizing), GridTrackSize(Length::Percent(40), kFitContentTrackSizing),
...
TEST_F(GridTrackCollectionTest, TestGridSizingTrackCollectionSetIterator)
{
    ...
    for (wtf_size_t set_count : set_counts) {
        ...
        ASSERT_TRUE(explicit_tracks.AddRepeater(track_sizes, NGGridTrackRepeater::RepeatType::kNoRepeat, 1));
    }
    ...
    for (wtf_size_t i = 0; i < ranges.size(); ++i) {
        ...
        for (auto set_iterator = IteratorForRange(track_collection, i); !set_iterator.IsAtEnd(); set_iterator.MoveToNextSet()) {
            EXPECT_SET(GridTrackSize(Length::Flex(set_count++)), 1u, set_iterator);
...
TEST_F(GridTrackCollectionTest, TestGridSizingTrackCollectionExplicitTracks)
{
    ...
    Vector<GridTrackSize, 1> track_sizes = { GridTrackSize(Length::Fixed(1)), GridTrackSize(Length::Fixed(2)), GridTrackSize(Length::Fixed(3)) };
    ASSERT_TRUE(explicit_tracks.AddRepeater(track_sizes, NGGridTrackRepeater::RepeatType::kInteger, 3));

    // repeat(auto-fit, 4px 5px)
    track_sizes = { GridTrackSize(Length::Fixed(4)), GridTrackSize(Length::Fixed(5)) };
    ASSERT_TRUE(explicit_tracks.AddRepeater(track_sizes, NGGridTrackRepeater::RepeatType::kAutoFit));
    ASSERT_EQ(2u, explicit_tracks.RepeaterCount());

    auto range_builder = CreateRangeBuilder(explicit_tracks, implicit_tracks,
        /* auto_repetitions */ 5);

    wtf_size_t range1_start, range1_end, range2_start, range2_end, range3_start, range3_end, range4_start, range4_end;
...
TEST_F(GridTrackCollectionTest, TestGridSizingTrackCollectionImplicitTracks)
{
    ...
    Vector<GridTrackSize, 1> track_sizes
        = { GridTrackSize(Length::Fixed(1)), GridTrackSize(Length::Fixed(2)), GridTrackSize(Length::Fixed(3)), GridTrackSize(Length::Fixed(4)) };
    ASSERT_TRUE(explicit_tracks.AddRepeater(track_sizes, NGGridTrackRepeater::RepeatType::kNoRepeat, 1));
    ASSERT_EQ(1u, explicit_tracks.RepeaterCount());

    // Implicit grid: 5px 6px 7px
    track_sizes = { GridTrackSize(Length::Fixed(5)), GridTrackSize(Length::Fixed(6)), GridTrackSize(Length::Fixed(7)) };
    ASSERT_TRUE(implicit_tracks.AddRepeater(track_sizes, NGGridTrackRepeater::RepeatType::kNoRepeat, 1));
...
TEST_F(GridTrackCollectionTest, TestGridSizingTrackCollectionIntrinsicAndFlexTracks)
{
    ...
    Vector<GridTrackSize, 1> track_sizes
        = { GridTrackSize(Length::MinContent()), GridTrackSize(Length::Flex(1.0)), GridTrackSize(Length::Fixed(2)), GridTrackSize(Length::Fixed(3)) };
    ASSERT_TRUE(explicit_tracks.AddRepeater(track_sizes, NGGridTrackRepeater::RepeatType::kInteger, 2));
    ASSERT_EQ(1u, explicit_tracks.RepeaterCount());
    ...
    EXPECT_FALSE(set_iterator.MoveToNextSet());
    EXPECT_FALSE(ranges[0].properties.HasProperty(TrackSpanProperties::kHasFlexibleTrack));
    EXPECT_TRUE(ranges[0].properties.HasProperty(TrackSpanProperties::kHasIntrinsicTrack));
    ...
    EXPECT_SET(GridTrackSize(Length::Fixed(2)), 1u, set_iterator);
    EXPECT_FALSE(set_iterator.MoveToNextSet());
    EXPECT_TRUE(ranges[1].properties.HasProperty(TrackSpanProperties::kHasFlexibleTrack));
    EXPECT_TRUE(ranges[1].properties.HasProperty(TrackSpanProperties::kHasIntrinsicTrack));
    ...
    EXPECT_SET(GridTrackSize(Length::Fixed(2)), 1u, set_iterator);
    EXPECT_FALSE(set_iterator.MoveToNextSet());
    EXPECT_TRUE(ranges[2].properties.HasProperty(TrackSpanProperties::kHasFlexibleTrack));
    EXPECT_TRUE(ranges[2].properties.HasProperty(TrackSpanProperties::kHasIntrinsicTrack));

    EXPECT_RANGE(7u, 1u, ranges[3]);
    set_iterator = IteratorForRange(track_collection, /* range_index */ 3);
    EXPECT_SET(GridTrackSize(Length::Fixed(3)), 1u, set_iterator);
    EXPECT_FALSE(set_iterator.MoveToNextSet());
    EXPECT_FALSE(ranges[3].properties.HasProperty(TrackSpanProperties::kHasFlexibleTrack));
    EXPECT_FALSE(ranges[3].properties.HasProperty(TrackSpanProperties::kHasIntrinsicTrack));

    EXPECT_RANGE(8u, 3u, ranges[4]);
    set_iterator = IteratorForRange(track_collection, /* range_index */ 4);
    EXPECT_SET(GridTrackSize(Length::Auto()), 3u, set_iterator);
    EXPECT_FALSE(set_iterator.MoveToNextSet());
    EXPECT_FALSE(ranges[4].properties.HasProperty(TrackSpanProperties::kHasFlexibleTrack));
    EXPECT_TRUE(ranges[4].properties.HasProperty(TrackSpanProperties::kHasIntrinsicTrack));
}
