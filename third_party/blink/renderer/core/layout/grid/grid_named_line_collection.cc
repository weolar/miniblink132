...
GridNamedLineCollection::GridNamedLineCollection(const String& named_line, GridTrackSizingDirection track_direction,
    const NamedGridLinesMap& implicit_grid_line_names, const NamedGridLinesMap& explicit_grid_line_names, const ComputedGridTrackList& computed_grid_track_list,
    wtf_size_t last_line, wtf_size_t auto_repeat_tracks_count, bool is_subgridded_to_parent)
    : last_line_(last_line)
    , auto_repeat_total_tracks_(auto_repeat_tracks_count)
{
    is_standalone_grid_ = computed_grid_track_list.axis_type == GridAxisType::kStandaloneAxis;
    ...
    const NamedGridLinesMap& auto_repeat_grid_line_names = computed_grid_track_list.auto_repeat_named_grid_lines;

    if (!explicit_grid_line_names.empty() && are_named_lines_valid) {
        auto it = explicit_grid_line_names.find(named_line);
        named_lines_indexes_ = (it == explicit_grid_line_names.end()) ? nullptr : &it->value;
    }

    if (!auto_repeat_grid_line_names.empty() && are_named_lines_valid) {
        auto it = auto_repeat_grid_line_names.find(named_line);
        auto_repeat_named_lines_indexes_ = it == auto_repeat_grid_line_names.end() ? nullptr : &it->value;
    }

    if (!implicit_grid_line_names.empty()) {
        auto it = implicit_grid_line_names.find(named_line);
        implicit_named_lines_indexes_ = it == implicit_grid_line_names.end() ? nullptr : &it->value;
    }

    insertion_point_ = computed_grid_track_list.auto_repeat_insertion_point;
    auto_repeat_track_list_length_ = computed_grid_track_list.track_list.AutoRepeatTrackCount();
...
bool GridNamedLineCollection::Contains(wtf_size_t line) const
{
    ...
    auto find = [](const Vector<wtf_size_t>* indexes, wtf_size_t line) { return indexes && indexes->Find(line) != kNotFound; };
    ...
    if (auto_repeat_track_list_length_ == 0 || has_collapsed_auto_repeat || line < insertion_point_) {
    ...
    if (line == insertion_point_) {
        return find(named_lines_indexes_, line) || find(auto_repeat_named_lines_indexes_, 0);
    }

    // Search the final auto repetition line name.
    if (line == insertion_point_ + auto_repeat_total_tracks_) {
        return find(auto_repeat_named_lines_indexes_, auto_repeat_track_list_length_) || find(named_lines_indexes_, insertion_point_ + 1);
    }

    // Search repeated line names.
    wtf_size_t auto_repeat_index_in_first_repetition = (line - insertion_point_) % auto_repeat_track_list_length_;
    if (!auto_repeat_index_in_first_repetition && find(auto_repeat_named_lines_indexes_, auto_repeat_track_list_length_)) {
        return true;
    }
    return find(auto_repeat_named_lines_indexes_, auto_repeat_index_in_first_repetition);
}
...
wtf_size_t GridNamedLineCollection::FirstExplicitPosition() const
{
    ...
    // the auto-repeat().
    if ((is_standalone_grid_ && auto_repeat_track_list_length_ == 0) || (named_lines_indexes_ && named_lines_indexes_->at(first_line) <= insertion_point_)) {
    ...
    const wtf_size_t auto_repeat_counted_tracks = auto_repeat_total_tracks_ ? auto_repeat_total_tracks_ - 1 : 0;
...
wtf_size_t GridNamedLineCollection::FirstPosition() const
{
    ...
    return std::min(FirstExplicitPosition(), implicit_named_lines_indexes_->at(first_line));
}
