...
public:
    ...
    GridItemData(BlockNode item_node, const ComputedStyle& parent_grid_style, const ComputedStyle& root_grid_style,
        bool parent_must_consider_grid_items_for_column_sizing = false, bool parent_must_consider_grid_items_for_row_sizing = false);
    ...
    void SetAlignmentFallback(GridTrackSizingDirection track_direction, bool has_synthesized_baseline);

    AxisEdge Alignment(GridTrackSizingDirection track_direction) const
    {
        return (track_direction == kForColumns) ? column_fallback_alignment.value_or(column_alignment) : row_fallback_alignment.value_or(row_alignment);
    }

    bool IsOverflowSafe(GridTrackSizingDirection track_direction) const
    {
        return (track_direction == kForColumns) ? column_fallback_alignment || is_overflow_safe_for_columns
    ...
    bool IsBaselineAligned(GridTrackSizingDirection track_direction) const
    {
        ...
        return (axis_alignment == AxisEdge::kFirstBaseline || axis_alignment == AxisEdge::kLastBaseline);
    }

    bool IsBaselineSpecified(GridTrackSizingDirection track_direction) const
    {
        const auto& axis_alignment = (track_direction == kForColumns) ? column_alignment : row_alignment;
        return (axis_alignment == AxisEdge::kFirstBaseline || axis_alignment == AxisEdge::kLastBaseline);
    }

    bool IsLastBaselineSpecified(GridTrackSizingDirection track_direction) const
    {
        return (track_direction == kForColumns) ? column_alignment == AxisEdge::kLastBaseline : row_alignment == AxisEdge::kLastBaseline;
    }
    ...
    enum BaselineGroup BaselineGroup(GridTrackSizingDirection track_direction) const
    {
        return (track_direction == kForColumns) ? column_baseline_group : row_baseline_group;
    }

    WritingDirectionMode BaselineWritingDirection(GridTrackSizingDirection track_direction) const
    {
        // NOTE: For reading the baseline from a fragment the direction doesn't
        // matter - just use the default.
        return { (track_direction == kForColumns) ? column_baseline_writing_mode : row_baseline_writing_mode, TextDirection::kLtr };
    }

    const GridItemIndices& SetIndices(GridTrackSizingDirection track_direction) const
    {
        return (track_direction == kForColumns) ? column_set_indices : row_set_indices;
    }

    GridItemIndices& RangeIndices(GridTrackSizingDirection track_direction)
    {
        return (track_direction == kForColumns) ? column_range_indices : row_range_indices;
    }
    ...
    {
        return (track_direction == kForColumns) ? is_considered_for_column_sizing : is_considered_for_row_sizing;
    }

    bool IsOppositeDirectionInRootGrid(GridTrackSizingDirection track_direction) const
    {
        return (track_direction == kForColumns) ? is_opposite_direction_in_root_grid_columns : is_opposite_direction_in_root_grid_rows;
    }

    bool MustCachePlacementIndices(GridTrackSizingDirection track_direction) const
    {
        return !is_subgridded_to_parent_grid || IsConsideredForSizing(track_direction) || MustConsiderGridItemsForSizing(track_direction);
    }

    bool MustConsiderGridItemsForSizing(GridTrackSizingDirection track_direction) const
    {
        return (track_direction == kForColumns) ? must_consider_grid_items_for_column_sizing : must_consider_grid_items_for_row_sizing;
    }
    ...
    const TrackSpanProperties& GetTrackSpanProperties(GridTrackSizingDirection track_direction) const
    {
        return (track_direction == kForColumns) ? column_span_properties : row_span_properties;
    }

    void SetTrackSpanProperty(const TrackSpanProperties::PropertyId property, GridTrackSizingDirection track_direction)
    {
    ...
    {
        return GetTrackSpanProperties(track_direction).HasProperty(TrackSpanProperties::kHasFlexibleTrack);
    }
    bool IsSpanningIntrinsicTrack(GridTrackSizingDirection track_direction) const
    {
        return GetTrackSpanProperties(track_direction).HasProperty(TrackSpanProperties::kHasIntrinsicTrack);
    }
    bool IsSpanningAutoMinimumTrack(GridTrackSizingDirection track_direction) const
    {
        return GetTrackSpanProperties(track_direction).HasProperty(TrackSpanProperties::kHasAutoMinimumTrack);
    }
    bool IsSpanningFixedMinimumTrack(GridTrackSizingDirection track_direction) const
    {
        return GetTrackSpanProperties(track_direction).HasProperty(TrackSpanProperties::kHasFixedMinimumTrack);
    }
    bool IsSpanningFixedMaximumTrack(GridTrackSizingDirection track_direction) const
    {
        return GetTrackSpanProperties(track_direction).HasProperty(TrackSpanProperties::kHasFixedMaximumTrack);
    }
...
public:
    ...
    public:
        using value_type = typename std::conditional<is_const, const GridItemData, GridItemData>::type;

        using GridItemDataVectorPtr = typename std::conditional<is_const, const GridItemDataVector*, GridItemDataVector*>::type;
        ...
        {
            return current_index_ != other.current_index_ || item_data_ != other.item_data_;
        }
    ...
    public:
        ...
        Iterator<is_const> begin() const
        {
            return begin_;
        }
        Iterator<is_const> end() const
        {
            return end_;
        }
