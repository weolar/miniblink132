...
AutoPlacementType AutoPlacement(const GridArea& position, GridTrackSizingDirection major_direction)
{
    const GridTrackSizingDirection minor_direction = (major_direction == kForColumns) ? kForRows : kForColumns;
    DCHECK(!position.Span(major_direction).IsUntranslatedDefinite() && !position.Span(minor_direction).IsUntranslatedDefinite());

    const bool is_major_indefinite = position.Span(major_direction).IsIndefinite();
    const bool is_minor_indefinite = position.Span(minor_direction).IsIndefinite();
...
GridPlacement::GridPlacement(const ComputedStyle& grid_style, const GridLineResolver& line_resolver)
    : placement_data_(line_resolver)
    , packing_behavior_(grid_style.IsGridAutoFlowAlgorithmSparse() ? PackingBehavior::kSparse : PackingBehavior::kDense)
    ,
    // The major direction is the one specified in the 'grid-auto-flow'
    // property (row or column), the minor direction is its opposite.
    major_direction_(grid_style.IsGridAutoFlowDirectionRow() ? kForRows : kForColumns)
    , minor_direction_(grid_style.IsGridAutoFlowDirectionRow() ? kForColumns : kForRows)
{
}

// https://drafts.csswg.org/css-grid/#auto-placement-algo
GridPlacementData GridPlacement::RunAutoPlacementAlgorithm(const GridItems& grid_items)
{
#if DCHECK_IS_ON()
    DCHECK(!auto_placement_algorithm_called_) << "Auto-placement algorithm should only be called once.";
    ...
    if (!PlaceNonAutoGridItems(grid_items, &placed_items, &positions_locked_to_major_axis, &positions_not_locked_to_major_axis)) {
...
bool GridPlacement::PlaceNonAutoGridItems(const GridItems& grid_items, PlacedGridItemsList* placed_items, PositionVector* positions_locked_to_major_axis,
    PositionVector* positions_not_locked_to_major_axis)
{
    DCHECK(placed_items && positions_locked_to_major_axis && positions_not_locked_to_major_axis);
    ...
    for (const auto& grid_item : grid_items) {
        ...
        GridArea position;
        position.columns = placement_data_.line_resolver.ResolveGridPositionsFromStyle(item_style, kForColumns);
        DCHECK(!position.columns.IsTranslatedDefinite());

        position.rows = placement_data_.line_resolver.ResolveGridPositionsFromStyle(item_style, kForRows);
        ...
        if (position.columns.IsUntranslatedDefinite()) {
            placement_data_.column_start_offset = std::max<int>(placement_data_.column_start_offset, -position.columns.UntranslatedStartLine());
        }

        if (position.rows.IsUntranslatedDefinite()) {
            placement_data_.row_start_offset = std::max<int>(placement_data_.row_start_offset, -position.rows.UntranslatedStartLine());
        }
    ...
    for (auto& position : placement_data_.grid_item_positions) {
        ...
        if (!has_indefinite_major_span) {
            item_major_span.Translate((major_direction_ == kForColumns) ? placement_data_.column_start_offset : placement_data_.row_start_offset);
            position.SetSpan(item_major_span, major_direction_);
        }

        if (!has_indefinite_minor_span) {
            item_minor_span.Translate((minor_direction_ == kForColumns) ? placement_data_.column_start_offset : placement_data_.row_start_offset);
            position.SetSpan(item_minor_span, minor_direction_);
        }

        minor_max_end_line_
            = std::max<wtf_size_t>(minor_max_end_line_, has_indefinite_minor_span ? item_minor_span.IndefiniteSpanSize() : item_minor_span.EndLine());
        ...
        if (!has_indefinite_major_span && !has_indefinite_minor_span) {
            auto placed_item = std::make_unique<PlacedGridItem>(position, major_direction_, minor_direction_);
            ...
            placed_items->needs_to_sort_item_vector |= !non_auto_placed_items.empty() && *placed_item < *non_auto_placed_items.back();
    ...
    return !positions_not_locked_to_major_axis->empty() || !positions_locked_to_major_axis->empty();
}

void GridPlacement::PlaceGridItemsLockedToMajorAxis(const PositionVector& positions_locked_to_major_axis, PlacedGridItemsList* placed_items)
{
    DCHECK(placed_items);
    ...
    HashMap<wtf_size_t, wtf_size_t, IntWithZeroKeyHashTraits<wtf_size_t>> minor_cursors;

    for (auto* position : positions_locked_to_major_axis) {
        DCHECK_EQ(AutoPlacement(*position, major_direction_), AutoPlacementType::kMinor);

        const wtf_size_t minor_span_size = position->Span(minor_direction_).IndefiniteSpanSize();
        ...
        placement_cursor.MoveCursorToFitGridSpan(
            position->SpanSize(major_direction_), minor_span_size, minor_max_end_line_, CursorMovementBehavior::kForceMajorLine);
        ...
        GridSpan grid_item_span = GridSpan::TranslatedDefiniteGridSpan(placement_cursor.MinorLine(), minor_end_line);
...
void GridPlacement::PlaceAutoMajorAxisGridItem(GridArea* position, PlacedGridItemsList* placed_items, AutoPlacementCursor* placement_cursor) const
{
    DCHECK(position && placed_items && placement_cursor);
    const wtf_size_t major_span_size = position->Span(major_direction_).IndefiniteSpanSize();

    placement_cursor->MoveToMinorLine(position->StartLine(minor_direction_));
    placement_cursor->MoveCursorToFitGridSpan(
        major_span_size, position->SpanSize(minor_direction_), minor_max_end_line_, CursorMovementBehavior::kForceMinorLine);

    // Update grid item placement for major axis.
    GridSpan grid_item_span = GridSpan::TranslatedDefiniteGridSpan(placement_cursor->MajorLine(), placement_cursor->MajorLine() + major_span_size);
    position->SetSpan(grid_item_span, major_direction_);

    PlaceGridItemAtCursor(*position, placed_items, placement_cursor);
}

void GridPlacement::PlaceAutoBothAxisGridItem(GridArea* position, PlacedGridItemsList* placed_items, AutoPlacementCursor* placement_cursor) const
{
    DCHECK(position && placed_items && placement_cursor);
    ...
    placement_cursor->MoveCursorToFitGridSpan(major_span_size, minor_span_size, minor_max_end_line_, CursorMovementBehavior::kAuto);

    // Update grid item placement for both axis.
    GridSpan grid_item_span = GridSpan::TranslatedDefiniteGridSpan(placement_cursor->MajorLine(), placement_cursor->MajorLine() + major_span_size);
    position->SetSpan(grid_item_span, major_direction_);

    grid_item_span = GridSpan::TranslatedDefiniteGridSpan(placement_cursor->MinorLine(), placement_cursor->MinorLine() + minor_span_size);
    position->SetSpan(grid_item_span, minor_direction_);

    PlaceGridItemAtCursor(*position, placed_items, placement_cursor);
}

void GridPlacement::PlaceGridItemAtCursor(const GridArea& position, PlacedGridItemsList* placed_items, AutoPlacementCursor* placement_cursor) const
{
    DCHECK(placed_items && placement_cursor);

    auto new_placed_item = std::make_unique<PlacedGridItem>(position, major_direction_, minor_direction_);
    const auto* next_placed_item = placement_cursor->NextPlacedItem();

    placed_items->ordered_list.InsertAfter(new_placed_item.get(), next_placed_item ? next_placed_item->Prev() : placed_items->ordered_list.Tail());
...
void GridPlacement::ClampGridItemsToFitSubgridArea(GridTrackSizingDirection track_direction)
{
    const wtf_size_t subgrid_span_size = placement_data_.SubgridSpanSize(track_direction);
    ...
    for (auto& resolved_position : placement_data_.grid_item_positions) {
        ...
        if ((track_direction == kForColumns && !resolved_position.columns.IsTranslatedDefinite())
            || (track_direction == kForRows && !resolved_position.rows.IsTranslatedDefinite())) {
            continue;
        }

        int start_line = resolved_position.StartLine(track_direction) - start_offset;
        int end_line = resolved_position.EndLine(track_direction) - start_offset;

        resolved_position.SetSpan(
            GridSpan::TranslatedDefiniteGridSpan(ClampTo<int>(start_line, 0, subgrid_span_size - 1), ClampTo<int>(end_line, 1, subgrid_span_size)),
            track_direction);
    }
...
wtf_size_t GridPlacement::IntrinsicEndLine(GridTrackSizingDirection track_direction) const
{
    return (track_direction == kForColumns) ? placement_data_.column_start_offset + placement_data_.ExplicitGridTrackCount(kForColumns)
                                            : placement_data_.row_start_offset + placement_data_.ExplicitGridTrackCount(kForRows);
}
...
{
    return (major_line == other.major_line) ? minor_line <= other.minor_line : major_line < other.major_line;
}
bool GridPlacement::GridPosition::operator<(const GridPosition& other) const
{
    return (major_line != other.major_line) ? major_line < other.major_line : minor_line < other.minor_line;
}

GridPlacement::PlacedGridItem::PlacedGridItem(const GridArea& position, GridTrackSizingDirection major_direction, GridTrackSizingDirection minor_direction)
    : start_ { position.StartLine(major_direction), position.StartLine(minor_direction) }
    , end_ { position.EndLine(major_direction), position.EndLine(minor_direction) }
{
}

GridPlacement::GridPosition GridPlacement::PlacedGridItem::EndOnPreviousMajorLine() const
{
...
void GridPlacement::AutoPlacementCursor::MoveCursorToFitGridSpan(
    const wtf_size_t major_span_size, const wtf_size_t minor_span_size, const wtf_size_t minor_max_end_line, const CursorMovementBehavior movement_behavior)
{
    DCHECK_LE(minor_span_size, minor_max_end_line);

    wtf_size_t next_minor_line;
    const bool allow_minor_line_movement = movement_behavior != CursorMovementBehavior::kForceMinorLine;
    ...
    const wtf_size_t minor_max_start_line
        = (movement_behavior == CursorMovementBehavior::kForceMajorLine) ? minor_max_end_line : minor_max_end_line - minor_span_size;
    ...
    auto NeedsToMoveToNextMajorLine = [&]() -> bool {
        ...
        return next_minor_line > minor_max_start_line || (!allow_minor_line_movement && next_minor_line != current_position_.minor_line);
    };
    ...
    while (true) {
        ...
        for (const auto* placed_item : items_overlapping_major_line_) {
            ...
            if (next_minor_line < item_minor_end_line && placed_item->MinorStartLine() < minor_span_end_line) {
        ...
        while (upcoming_item) {
            const wtf_size_t major_span_end_line = current_position_.major_line + major_span_size;
            const wtf_size_t minor_span_end_line = next_minor_line + minor_span_size;
            ...
            if (next_minor_line < item_minor_end_line && current_position_.major_line < upcoming_item->MajorEndLine()
                && upcoming_item->MajorStartLine() < major_span_end_line && upcoming_item->MinorStartLine() < minor_span_end_line) {
                next_minor_line = item_minor_end_line;
                if (NeedsToMoveToNextMajorLine())
                    break;
            }
...
{
    DCHECK(std::is_heap(items_overlapping_major_line_.begin(), items_overlapping_major_line_.end(), ComparePlacedGridItemsByEnd));
    ...
    while (!items_overlapping_major_line_.empty()) {
        ...
        auto last_overlapping_position = items_overlapping_major_line_.front()->EndOnPreviousMajorLine();
        ...
        std::pop_heap(items_overlapping_major_line_.begin(), items_overlapping_major_line_.end(), ComparePlacedGridItemsByEnd);
        items_overlapping_major_line_.pop_back();
    }

    while (next_placed_item_ && next_placed_item_->Start() <= current_position_) {
        auto last_overlapping_position = next_placed_item_->EndOnPreviousMajorLine();
        ...
        if (current_position_ < last_overlapping_position) {
            ...
            std::push_heap(items_overlapping_major_line_.begin(), items_overlapping_major_line_.end(), ComparePlacedGridItemsByEnd);
        }
        next_placed_item_ = next_placed_item_->Next();
    }
}

void GridPlacement::AutoPlacementCursor::MoveToMajorLine(const wtf_size_t major_line)
{
    DCHECK_LE(current_position_.major_line, major_line);
    current_position_.major_line = major_line;
}

void GridPlacement::AutoPlacementCursor::MoveToMinorLine(const wtf_size_t minor_line)
{
...
void GridPlacement::AutoPlacementCursor::MoveToNextMajorLine(bool allow_minor_line_movement)
{
    ++current_position_.major_line;

    if (should_move_to_next_item_major_end_line_ && !items_overlapping_major_line_.empty()) {
        DCHECK_GE(items_overlapping_major_line_.front()->MajorEndLine(), current_position_.major_line);
        current_position_.major_line = items_overlapping_major_line_.front()->MajorEndLine();
    }
...
void GridPlacement::AutoPlacementCursor::InsertPlacedItemAtCurrentPosition(const PlacedGridItem* new_placed_item)
{
...
void GridPlacement::PlacedGridItemsList::AppendCurrentItemsToOrderedList()
{
    ...
    auto ComparePlacedGridItemPointers = [](const std::unique_ptr<PlacedGridItem>& lhs, const std::unique_ptr<PlacedGridItem>& rhs) { return *lhs < *rhs; };

    if (needs_to_sort_item_vector) {
        std::sort(item_vector.begin(), item_vector.end(), ComparePlacedGridItemPointers);
    }
    DCHECK(std::is_sorted(item_vector.begin(), item_vector.end(), ComparePlacedGridItemPointers));
...
// static
void GridPlacement::ResolveOutOfFlowItemGridLines(const GridLayoutTrackCollection& track_collection, const GridLineResolver& line_resolver,
    const ComputedStyle& grid_style, const ComputedStyle& item_style, wtf_size_t start_offset, wtf_size_t* start_line, wtf_size_t* end_line)
{
    ...
    *end_line = kNotFound;
    ...
    const auto span = line_resolver.ResolveGridPositionsFromStyle(item_style, track_direction);
    ...
    if (span_start_line >= 0 && (is_for_columns ? !item_style.GridColumnStart().IsAuto() : !item_style.GridRowStart().IsAuto())
        && track_collection.IsGridLineWithinImplicitGrid(span_start_line)) {
        *start_line = span_start_line;
    }

    if (span_end_line >= 0 && (is_for_columns ? !item_style.GridColumnEnd().IsAuto() : !item_style.GridRowEnd().IsAuto())
