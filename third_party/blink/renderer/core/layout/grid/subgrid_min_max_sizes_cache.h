...
class SubgridMinMaxSizesCache : public GarbageCollected<SubgridMinMaxSizesCache> {
public:
    ...
    SubgridMinMaxSizesCache(MinMaxSizes&& min_max_sizes, const GridLayoutData& layout_data)
        : opposite_axis_subgridded_tracks_(layout_data.OnlySubgriddedCollection())
        , cached_min_max_sizes_(std::move(min_max_sizes))
    {
    }

    const MinMaxSizes& operator*() const
    {
    ...
    {
        return layout_data.OnlySubgriddedCollection() == opposite_axis_subgridded_tracks_;
    }
