...
public:
    ...
    GridPlacement(const ComputedStyle& grid_style, const GridLineResolver& line_resolver);
    ...
    static void ResolveOutOfFlowItemGridLines(const GridLayoutTrackCollection& track_collection, const GridLineResolver& line_resolver,
...
private:
    ...
    public:
        PlacedGridItem(const GridArea& position, GridTrackSizingDirection major_direction, GridTrackSizingDirection minor_direction);
        ...
        GridPosition Start() const
        {
            return start_;
        }
        GridPosition End() const
        {
            return end_;
        }
        GridPosition EndOnPreviousMajorLine() const;

        wtf_size_t MajorStartLine() const
        {
            return start_.major_line;
        }
        wtf_size_t MinorStartLine() const
        {
            return start_.minor_line;
        }
        wtf_size_t MajorEndLine() const
        {
            return end_.major_line;
        }
        wtf_size_t MinorEndLine() const
        {
            return end_.minor_line;
        }
    ...
    public:
        ...
        void MoveCursorToFitGridSpan(const wtf_size_t major_span_size, const wtf_size_t minor_span_size, const wtf_size_t minor_max_end_line,
        ...
        void InsertPlacedItemAtCurrentPosition(const PlacedGridItem* new_placed_item);

        wtf_size_t MajorLine() const
        {
            return current_position_.major_line;
        }
        wtf_size_t MinorLine() const
        {
            return current_position_.minor_line;
        }

        const PlacedGridItem* NextPlacedItem() const
        {
            return next_placed_item_;
        }

    private:
        // Comparer needed to use |items_overlapping_major_line_| as a heap.
        struct {
            bool operator()(const PlacedGridItem* lhs, const PlacedGridItem* rhs) const
            {
    ...
    struct PlacedGridItemsList {
        ...
        PlacedGridItem* FirstPlacedItem()
        {
    ...
    bool PlaceNonAutoGridItems(const GridItems& grid_items, PlacedGridItemsList* placed_items, PositionVector* positions_locked_to_major_axis,
    ...
    void PlaceGridItemsLockedToMajorAxis(const PositionVector& positions_locked_to_major_axis, PlacedGridItemsList* placed_items);
    // Place an item that has a definite position on the minor axis but need
    // auto-placement on the major axis.
    void PlaceAutoMajorAxisGridItem(GridArea* position, PlacedGridItemsList* placed_items, AutoPlacementCursor* placement_cursor) const;
    // Place an item that needs auto-placement on both the major and minor axis.
    void PlaceAutoBothAxisGridItem(GridArea* position, PlacedGridItemsList* placed_items, AutoPlacementCursor* placement_cursor) const;
    // Update the list of placed grid items and auto-placement cursor using the
    // resolved position of the specified grid item.
    void PlaceGridItemAtCursor(const GridArea& position, PlacedGridItemsList* placed_items, AutoPlacementCursor* placement_cursor) const;
