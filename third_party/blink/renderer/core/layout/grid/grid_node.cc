...
GridItems GridNode::ConstructGridItems(const GridLineResolver& line_resolver, bool* must_invalidate_placement_cache,
    HeapVector<Member<LayoutBox>>* opt_oof_children, bool* opt_has_nested_subgrid) const
{
    return ConstructGridItems(line_resolver, /*root_grid_style=*/Style(),
        /*parent_grid_style=*/Style(), line_resolver.HasStandaloneAxis(kForColumns), line_resolver.HasStandaloneAxis(kForRows), must_invalidate_placement_cache,
        opt_oof_children, opt_has_nested_subgrid);
}

GridItems GridNode::ConstructGridItems(const GridLineResolver& line_resolver, const ComputedStyle& root_grid_style, const ComputedStyle& parent_grid_style,
    bool must_consider_grid_items_for_column_sizing, bool must_consider_grid_items_for_row_sizing, bool* must_invalidate_placement_cache,
    HeapVector<Member<LayoutBox>>* opt_oof_children, bool* opt_has_nested_subgrid) const
{
    ...
    if (layout_grid->HasCachedPlacementData()) {
        ...
        grid_items.ReserveInitialCapacity(cached_placement_data->grid_item_positions.size());

        if (*must_invalidate_placement_cache || line_resolver != cached_placement_data->line_resolver) {
    ...
    *must_invalidate_placement_cache |= !cached_placement_data;

    {
        ...
        for (auto child = FirstChild(); child; child = child.NextSibling()) {
            ...
            auto grid_item = std::make_unique<GridItemData>(
                To<BlockNode>(child), parent_grid_style, root_grid_style, must_consider_grid_items_for_column_sizing, must_consider_grid_items_for_row_sizing);

            // We'll need to sort when we encounter a non-initial order property.
            should_sort_grid_items_by_order_property |= child.Style().Order() != initial_order;
    ...
    if (cached_placement_data) {
        ...
        DCHECK(*cached_placement_data == grid_placement.RunAutoPlacementAlgorithm(grid_items));
    }
#endif

    if (!cached_placement_data) {
        GridPlacement grid_placement(Style(), line_resolver);
        layout_grid->SetCachedPlacementData(grid_placement.RunAutoPlacementAlgorithm(grid_items));
        cached_placement_data = &layout_grid->CachedPlacementData();
    }
...
void GridNode::AppendSubgriddedItems(GridItems* grid_items) const
{
    ...
    for (wtf_size_t i = 0; i < grid_items->Size(); ++i) {
        ...
        if (!current_item.must_consider_grid_items_for_column_sizing && !current_item.must_consider_grid_items_for_row_sizing) {
            continue;
        }
        ...
        auto subgridded_items = subgrid.ConstructGridItems(subgrid.CachedLineResolver(), root_grid_style, subgrid.Style(),
            current_item.must_consider_grid_items_for_column_sizing, current_item.must_consider_grid_items_for_row_sizing, &must_invalidate_placement_cache);
        ...
        auto TranslateSubgriddedItem = [&current_item](GridSpan& subgridded_item_span, GridTrackSizingDirection track_direction) {
            if (current_item.MustConsiderGridItemsForSizing(track_direction)) {
                ...
                if (current_item.IsOppositeDirectionInRootGrid(track_direction)) {
                    const wtf_size_t subgrid_span_size = current_item.SpanSize(track_direction);

                    DCHECK_LE(subgridded_item_span.EndLine(), subgrid_span_size);

                    subgridded_item_span = GridSpan::TranslatedDefiniteGridSpan(
                        subgrid_span_size - subgridded_item_span.EndLine(), subgrid_span_size - subgridded_item_span.StartLine());
                }
                subgridded_item_span.Translate(current_item.StartLine(track_direction));
            }
        };
...
MinMaxSizesResult GridNode::ComputeSubgridMinMaxSizes(const GridSizingSubtree& sizing_subtree, const ConstraintSpace& space) const
{
    ...
    if (!layout_grid->HasCachedSubgridMinMaxSizes()) {
        const auto fragment_geometry = CalculateInitialFragmentGeometry(space, *this, /*break_token=*/nullptr, /*is_intrinsic=*/true);

        layout_grid->SetSubgridMinMaxSizesCache(
            GridLayoutAlgorithm({ *this, fragment_geometry, space }).ComputeSubgridMinMaxSizes(sizing_subtree), sizing_subtree.LayoutData());
    }
...
LayoutUnit GridNode::ComputeSubgridIntrinsicBlockSize(const GridSizingSubtree& sizing_subtree, const ConstraintSpace& space) const
{
    DCHECK(sizing_subtree.HasValidRootFor(*this));
    ...
    if (!layout_grid->HasCachedSubgridMinMaxSizes()) {
        ...
        const auto intrinsic_block_size = GridLayoutAlgorithm({ *this, fragment_geometry, space }).ComputeSubgridIntrinsicBlockSize(sizing_subtree);
        ...
        layout_grid->SetSubgridMinMaxSizesCache({ intrinsic_block_size, intrinsic_block_size }, sizing_subtree.LayoutData());
    }
