...
public:
    ...
    void SetSubgridMinMaxSizesCache(MinMaxSizes&& min_max_sizes, const GridLayoutData& layout_data);
    bool ShouldInvalidateSubgridMinMaxSizesCacheFor(const GridLayoutData& layout_data) const;

    wtf_size_t AutoRepeatCountForDirection(GridTrackSizingDirection track_direction) const;
    wtf_size_t ExplicitGridStartForDirection(GridTrackSizingDirection track_direction) const;
    wtf_size_t ExplicitGridEndForDirection(GridTrackSizingDirection track_direction) const;
    ...
    Vector<LayoutUnit, 1> TrackSizesForComputedStyle(GridTrackSizingDirection track_direction) const;
...
private:
    ...
    Vector<LayoutUnit> ComputeTrackSizeRepeaterForRange(const GridLayoutTrackCollection& track_collection, wtf_size_t range_index) const;
    Vector<LayoutUnit> ComputeExpandedPositions(GridTrackSizingDirection track_direction) const;

    void AddChild(LayoutObject* new_child, LayoutObject* before_child) override;
    void RemoveChild(LayoutObject* child) override;
    void StyleDidChange(StyleDifference diff, const ComputedStyle* old_style) override;
...
template <> struct DowncastTraits<LayoutGrid> {
