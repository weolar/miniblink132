...
GridSizingTree GridSizingTree::CopyForFragmentation() const
{
    ...
    for (const auto& sizing_data : tree_data_) {
        DCHECK(sizing_data);
        tree_copy.tree_data_.emplace_back(std::make_unique<GridTreeNode>(*sizing_data));
    }
    return tree_copy;
}
...
scoped_refptr<const GridLayoutTree> GridSizingTree::FinalizeTree() const
{
    ...
    for (const auto& grid_tree_node : tree_data_) {
        layout_tree_data.emplace_back(grid_tree_node->layout_data, grid_tree_node->subtree_size);
    }
    ...
    for (wtf_size_t i = layout_tree_data.size(); i; --i) {
        ...
        for (wtf_size_t j = i; !subtree_data.has_unresolved_geometry && j < next_subtree_index; j += layout_tree_data[j].subtree_size) {
            subtree_data.has_unresolved_geometry = layout_tree_data[j].has_unresolved_geometry;
        }
    }
    return base::MakeRefCounted<GridLayoutTree>(std::move(layout_tree_data));
}

GridSizingTree::GridTreeNode& GridSizingTree::CreateSizingData(const BlockNode& grid_node)
{
#if DCHECK_IS_ON()
...
void GridSizingTree::AddSubgriddedItemLookupData(SubgriddedItemData&& subgridded_item_data)
{
    ...
    subgridded_item_data_lookup_map_.insert(item_layout_box, std::move(subgridded_item_data));
}

SubgriddedItemData GridSizingTree::LookupSubgriddedItemData(const GridItemData& grid_item) const
{
...
wtf_size_t GridSizingTree::LookupSubgridIndex(const BlockNode& grid_node) const
{
    const auto* grid_layout_box = grid_node.GetLayoutBox();
