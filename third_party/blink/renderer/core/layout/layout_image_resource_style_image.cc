...
LayoutImageResourceStyleImage::LayoutImageResourceStyleImage(StyleImage* style_image)
...
scoped_refptr<Image> LayoutImageResourceStyleImage::GetImage(const gfx::SizeF& size) const
{
    ...
    if (style_image_->IsPendingImage())
        return nullptr;
    return style_image_->GetImage(*layout_object_, layout_object_->GetDocument(), layout_object_->StyleRef(), size);
}
...
gfx::SizeF LayoutImageResourceStyleImage::ImageSize(float multiplier) const
{
    ...
    gfx::SizeF default_size = list_marker ? list_marker->DefaultSize() : gfx::SizeF(LayoutReplaced::kDefaultWidth, LayoutReplaced::kDefaultHeight);
    return ConcreteObjectSize(multiplier, default_size);
}

gfx::SizeF LayoutImageResourceStyleImage::ConcreteObjectSize(float multiplier, const gfx::SizeF& default_object_size) const
{
    return style_image_->ImageSize(multiplier, default_object_size, ImageOrientation());
}

IntrinsicSizingInfo LayoutImageResourceStyleImage::GetNaturalDimensions(float multiplier) const
{
...
RespectImageOrientationEnum LayoutImageResourceStyleImage::ImageOrientation() const
{
    // Always respect the orientation of opaque origin images to avoid leaking
    // image data. Otherwise pull orientation from the layout object's style.
    RespectImageOrientationEnum respect_orientation = layout_object_->StyleRef().ImageOrientation();
