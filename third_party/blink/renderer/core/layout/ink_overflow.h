...
public:
    ...
    {
        return Reset(type, Type::kNone);
    }
    // Reset to |kInvalidated|.
    Type Invalidate(Type type)
    {
        return Reset(type, Type::kInvalidated);
    }
    ...
    Type SetContents(Type type, const PhysicalRect& contents, const PhysicalSize& size);

    // Set self and contents ink overflow rects.
    Type Set(Type type, const PhysicalRect& self, const PhysicalRect& contents, const PhysicalSize& size);

    // Compute and set ink overflow for text.
    Type SetTextInkOverflow(Type type, const InlineCursor& cursor, const TextFragmentPaintInfo& text_info, const ComputedStyle& style,
    ...
    // unscaled rectangle.
    Type SetSvgTextInkOverflow(Type type, const InlineCursor& cursor, const TextFragmentPaintInfo& text_info, const ComputedStyle& style,
    ...
    static std::optional<PhysicalRect> ComputeTextInkOverflow(const InlineCursor& cursor, const TextFragmentPaintInfo& text_info, const ComputedStyle& style,
    ...
    // logical direction.
    static LogicalRect ComputeEmphasisMarkOverflow(const ComputedStyle& style, const PhysicalSize& size, const LogicalRect& ink_overflow);

    // Expands the given overflow to account for shadows.
    static void ExpandForShadowOverflow(LogicalRect& ink_overflow, const ShadowList& text_shadow, const WritingMode writing_mode);
    ...
    static LogicalRect ComputeDecorationOverflow(const InlineCursor& cursor, const ComputedStyle& style, const Font& scaled_font,
    ...
    public:
        ...
        {
            return read_unset_as_none_;
        }
    };
#endif

private:
    static LogicalRect ComputeAppliedDecorationOverflow(const ComputedStyle& style, const Font& scaled_font, const PhysicalOffset& offset_in_container,
        const LogicalRect& ink_overflow, const InlinePaintContext* inline_context, const AppliedTextDecoration* decoration_override = nullptr);
    ...
    static LogicalRect ComputeMarkerOverflow(const DocumentMarkerVector& markers, const DocumentMarker::MarkerType type, const FragmentItem* fragment_item,
        const TextOffsetRange& fragment_dom_offsets, Text* node, const ComputedStyle& style, const Font& scaled_font, const PhysicalOffset& offset_in_container,
        const LogicalRect& ink_overflow, const InlinePaintContext* inline_context, const WritingMode writing_mode);

    static LogicalRect ComputeCustomHighlightOverflow(const DocumentMarkerVector& markers, const FragmentItem* fragment_item,
    ...
    bool TrySetOutsets(Type type, LayoutUnit left_outset, LayoutUnit top_outset, LayoutUnit right_outset, LayoutUnit bottom_outset);
    Type SetSingle(Type type, const PhysicalRect& ink_overflow, const PhysicalSize& size, Type new_type, Type new_small_type);
