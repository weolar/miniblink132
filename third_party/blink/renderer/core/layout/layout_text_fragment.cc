...
LayoutTextFragment::LayoutTextFragment(Node* node, const String& str, int start_offset, int length)
...
LayoutTextFragment* LayoutTextFragment::Create(Node* node, const String& str, int start_offset, int length)
{
    return MakeGarbageCollected<LayoutTextFragment>(node, str, start_offset, length);
}

LayoutTextFragment* LayoutTextFragment::CreateAnonymous(Document& doc, const String& text, unsigned start, unsigned length)
{
    LayoutTextFragment* fragment = LayoutTextFragment::Create(nullptr, text, start, length);
...
LayoutTextFragment* LayoutTextFragment::CreateAnonymous(Document& doc, const String& text)
{
...
// or remaining part.
void LayoutTextFragment::SetTextFragment(String text, unsigned start, unsigned length)
{
    NOT_DESTROYED();
...
Text* LayoutTextFragment::AssociatedTextNode() const
{
    ...
    if (auto* pseudo = DynamicTo<FirstLetterPseudoElement>(node)) {
        LayoutObject* next_layout_object = FirstLetterPseudoElement::FirstLetterTextLayoutObject(*pseudo);
...
LayoutText* LayoutTextFragment::GetFirstLetterPart() const
{
    ...
    if (!is_remaining_text_layout_object_)
        return nullptr;
    LayoutObject* const first_letter_container = GetFirstLetterPseudoElement()->GetLayoutObject();
...
void LayoutTextFragment::UpdateHitTestResult(HitTestResult& result, const PhysicalOffset& point) const
{
    NOT_DESTROYED();
    if (result.InnerNode())
        return;
...
std::optional<unsigned> LayoutTextFragment::CaretOffsetForPosition(const Position& position) const
{
    NOT_DESTROYED();
...
String LayoutTextFragment::PlainText() const
{
    ...
    if (first_letter_mapping && remaining_text_mapping && first_letter_mapping != remaining_text_mapping)
