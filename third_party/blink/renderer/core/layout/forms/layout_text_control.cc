...
void StyleDidChange(HTMLElement* inner_editor, const ComputedStyle* old_style, const ComputedStyle& new_style)
{
    ...
    LayoutBlock* inner_editor_layout_object = To<LayoutBlock>(inner_editor->GetLayoutObject());
    if (inner_editor_layout_object) {
        ...
        {
            StyleEngine::AllowMarkStyleDirtyFromRecalcScope scope(inner_editor->GetDocument().GetStyleEngine());
        ...
        if (new_style.HasPseudoElementStyle(kPseudoIdSelection) || (old_style && old_style->HasPseudoElementStyle(kPseudoIdSelection))) {
...
int ScrollbarThickness(const LayoutBox& box)
{
    ...
    return page.GetScrollbarTheme().ScrollbarThickness(
        page.GetChromeClient().WindowToViewportScalar(box.GetFrame(), 1.0f), box.StyleRef().UsedScrollbarWidth());
}

void HitInnerEditorElement(
    const LayoutBox& box, HTMLElement& inner_editor, HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset)
{
    if (!inner_editor.GetLayoutObject()) {
        return;
    }

    PhysicalOffset local_point = hit_test_location.Point() - accumulated_offset - inner_editor.GetLayoutObject()->LocalToAncestorPoint(PhysicalOffset(), &box);
...
bool HasValidAvgCharWidth(const Font& font)
{
    ...
    static HashSet<AtomicString>* font_families_with_invalid_char_width_map = nullptr;
    ...
    if (!font_families_with_invalid_char_width_map) {
        ...
        for (size_t i = 0; i < std::size(kFontFamiliesWithInvalidCharWidth); ++i) {
            font_families_with_invalid_char_width_map->insert(AtomicString(kFontFamiliesWithInvalidCharWidth[i]));
        }
    }
