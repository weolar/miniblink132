    ...
    MinMaxSizes RunComputeMinMaxSizes(BlockNode node)
    {
        ConstraintSpace space
            = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(), LayoutUnit()),
                /* stretch_inline_size_if_auto */ true, node.CreatesNewFormattingContext());
...
TEST_F(FieldsetLayoutAlgorithmTest, FieldsetPaddingWithLegend)
{
    ...
    const PhysicalBoxFragment* fragment = BaseLayoutAlgorithmTest::RunFieldsetLayoutAlgorithm(node, space);
...
TEST_F(FieldsetLayoutAlgorithmTest, SimpleFragmentation)
{
    ...
    fragment = BaseLayoutAlgorithmTest::RunFieldsetLayoutAlgorithm(node, space, fragment->GetBreakToken());
    EXPECT_EQ(PhysicalSize(176, 200), fragment->Size());
    ASSERT_TRUE(fragment->GetBreakToken());
...
TEST_F(FieldsetLayoutAlgorithmTest, LegendAndContentFragmentationNegativeMargin)
{
    SetBodyInnerHTML(R"HTML(
      <style>
        #fieldset {
...
TEST_F(FieldsetLayoutAlgorithmTest, LegendBreakInsideAvoid)
{
    ...
    const PhysicalBoxFragment* fragment = BaseLayoutAlgorithmTest::RunBlockLayoutAlgorithm(node, space);
    ASSERT_TRUE(fragment->GetBreakToken());
  ...
  offset:unplaced size:1000x100
    ...
    fragment = BaseLayoutAlgorithmTest::RunBlockLayoutAlgorithm(node, space, fragment->GetBreakToken());
    ASSERT_FALSE(fragment->GetBreakToken());
