...
// margins).
inline LegendBlockAlignment ComputeLegendBlockAlignment(const ComputedStyle& legend_style, const ComputedStyle& fieldset_style)
{
    bool start_auto = legend_style.MarginInlineStartUsing(fieldset_style).IsAuto();
    bool end_auto = legend_style.MarginInlineEndUsing(fieldset_style).IsAuto();
    if (start_auto || end_auto) {
        if (start_auto) {
            return end_auto ? LegendBlockAlignment::kCenter : LegendBlockAlignment::kEnd;
        }
...
FieldsetLayoutAlgorithm::FieldsetLayoutAlgorithm(const LayoutAlgorithmParams& params)
    : LayoutAlgorithm(params)
    , writing_direction_(GetConstraintSpace().GetWritingDirection())
    , consumed_block_size_(GetBreakToken() ? GetBreakToken()->ConsumedBlockSize() : LayoutUnit())
{
...
const LayoutResult* FieldsetLayoutAlgorithm::Layout()
{
    ...
    if (InvolvedInBlockFragmentation(container_builder_)) {
        container_builder_.SetBreakTokenData(MakeGarbageCollected<FieldsetBreakTokenData>(container_builder_.GetBreakTokenData()));
    }
    ...
    intrinsic_block_size_ = ClampIntrinsicBlockSize(
        GetConstraintSpace(), Node(), GetBreakToken(), Borders() + Scrollbar() + Padding(), intrinsic_block_size_ + Borders().block_end);

    // Recompute the block-axis size now that we know our content size.
    border_box_size_.block_size = ComputeBlockSizeForFragment(
        GetConstraintSpace(), Node(), BorderPadding(), intrinsic_block_size_ + consumed_block_size_, border_box_size_.inline_size);
    ...
    if (!Node().ShouldApplyBlockSizeContainment()) {
        border_box_size_.block_size = std::max(border_box_size_.block_size, minimum_border_box_block_size_);
    }
    ...
    if (InvolvedInBlockFragmentation(container_builder_)) [[unlikely]] {
        ...
        if (status == BreakStatus::kNeedsEarlierBreak) {
            ...
            return RelayoutAndBreakEarlier<FieldsetLayoutAlgorithm>(container_builder_.GetEarlyBreak());
...
BreakStatus FieldsetLayoutAlgorithm::LayoutChildren()
{
    ...
    if (const auto* token = GetBreakToken()) {
        ...
        if (base::checked_cast<wtf_size_t>(child_tokens.size())) {
            const BlockBreakToken* child_token = To<BlockBreakToken>(child_tokens[0].Get());
            if (child_token) {
    ...
    LogicalSize adjusted_padding_box_size = ShrinkLogicalSize(border_box_size_, Borders());
    ...
    if (legend) {
        ...
        if (IsBreakInside(GetBreakToken())) {
            const auto* token_data = To<FieldsetBreakTokenData>(GetBreakToken()->TokenData());
        ...
        if (InvolvedInBlockFragmentation(container_builder_)) {
            auto* token_data = To<FieldsetBreakTokenData>(container_builder_.GetBreakTokenData());
        ...
        if (adjusted_padding_box_size.block_size != kIndefiniteSize) {
            ...
            adjusted_padding_box_size.block_size = std::max(adjusted_padding_box_size.block_size - legend_size_contribution, Padding().BlockSum());
        }
        ...
        if (!Node().ShouldApplyBlockSizeContainment()) {
            minimum_border_box_block_size_ = BorderPadding().BlockSum() + legend_size_contribution;
        }
    }
    ...
    if (content_break_token || !has_seen_all_children) {
        ...
        BreakStatus break_status = LayoutFieldsetContent(fieldset_content, content_break_token, adjusted_padding_box_size, !!legend);
        if (break_status == BreakStatus::kNeedsEarlierBreak) {
            return break_status;
        }
    }
...
void FieldsetLayoutAlgorithm::LayoutLegend(BlockNode& legend)
{
    ...
    LogicalSize percentage_size = CalculateChildPercentageSize(GetConstraintSpace(), Node(), ChildAvailableSize());
    BoxStrut legend_margins = ComputeMarginsFor(legend.Style(), percentage_size.inline_size, GetConstraintSpace().GetWritingDirection());
    ...
    LayoutUnit legend_border_box_block_size = LogicalFragment(writing_direction_, physical_fragment).BlockSize();
    LayoutUnit legend_margin_box_block_size = legend_margins.block_start + legend_border_box_block_size + legend_margins.block_end;
    ...
    LayoutUnit legend_margin_end_offset = block_offset + legend_margin_box_block_size - legend_margins.block_start;
    ...
    LayoutUnit legend_border_box_inline_size = LogicalFragment(writing_direction_, result->GetPhysicalFragment()).InlineSize();
    ...
    // that.
    LayoutUnit legend_inline_start = Borders().inline_start + Scrollbar().inline_start + Padding().inline_start + legend_margins.inline_start;

    const LayoutUnit available_space = ChildAvailableSize().inline_size - legend_border_box_inline_size;
...
BreakStatus FieldsetLayoutAlgorithm::LayoutFieldsetContent(
    BlockNode& fieldset_content, const BlockBreakToken* content_break_token, LogicalSize adjusted_padding_box_size, bool has_legend)
{
    const EarlyBreak* early_break_in_child = nullptr;
    if (early_break_) [[unlikely]] {
        if (IsEarlyBreakTarget(*early_break_, container_builder_, fieldset_content)) {
            container_builder_.AddBreakBeforeChild(fieldset_content, kBreakAppealPerfect,
                /* is_forced_break */ false);
            ConsumeRemainingFragmentainerSpace();
            return BreakStatus::kContinue;
        } else {
            early_break_in_child = EnterEarlyBreakInChild(fieldset_content, *early_break_);
        }
    }
    ...
    if (adjusted_padding_box_size.block_size == kIndefiniteSize) {
        max_content_block_size = ResolveInitialMaxBlockLength(GetConstraintSpace(), Style(), BorderPadding(), Style().LogicalMaxHeight());
    }
    ...
    if (!is_past_end || max_content_block_size == LayoutUnit::Max()) {
        auto child_space = CreateConstraintSpaceForFieldsetContent(fieldset_content, adjusted_padding_box_size, intrinsic_block_size_);
        result = fieldset_content.Layout(child_space, content_break_token, early_break_in_child);
    }
    ...
    //   max-block-size.
    if (max_content_block_size != LayoutUnit::Max() && (!result || result->Status() == LayoutResult::kSuccess)) {
        ...
        if (max_content_block_size > Padding().BlockSum()) {
            ...
            max_content_block_size = std::max(max_content_block_size - (intrinsic_block_size_ + Borders().block_end), Padding().BlockSum());
        }

        if (result) {
            const auto& fragment = result->GetPhysicalFragment();
            LayoutUnit total_block_size = LogicalFragment(writing_direction_, fragment).BlockSize();
        ...
        if (!result) {
            ...
            auto adjusted_child_space = CreateConstraintSpaceForFieldsetContent(fieldset_content, adjusted_padding_box_size, intrinsic_block_size_);
            result = fieldset_content.Layout(adjusted_child_space, content_break_token, early_break_in_child);
        }
    }
    DCHECK(result);
    ...
    if (GetConstraintSpace().HasBlockFragmentation() && !early_break_) {
        break_status = BreakBeforeChildIfNeeded(fieldset_content, *result, FragmentainerOffsetForChildren() + intrinsic_block_size_,
    ...
    if (break_status == BreakStatus::kContinue) {
        ...
        if (auto first_baseline = fragment.FirstBaseline()) {
            container_builder_.SetFirstBaseline(offset.block_offset + *first_baseline);
        }
        ...
        intrinsic_block_size_ += LogicalFragment(writing_direction_, fragment).BlockSize();
        container_builder_.SetHasSeenAllChildren();
...
LayoutUnit FieldsetLayoutAlgorithm::FragmentainerSpaceAvailable() const
{
    ...
    return std::max(LayoutUnit(), FragmentainerSpaceLeftForChildren() - intrinsic_block_size_);
}
...
MinMaxSizesResult FieldsetLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    ...
    if (has_inline_size_containment) {
        ...
        std::optional<MinMaxSizesResult> result_without_children = CalculateMinMaxSizesIgnoringChildren(Node(), Borders() + Scrollbar() + Padding());
        if (result_without_children)
            return *result_without_children;
    } else {
        if (BlockNode legend = Node().GetRenderedLegend()) {
            MinMaxConstraintSpaceBuilder builder(GetConstraintSpace(), Style(), legend,
                /* is_new_fc */ true);
            builder.SetAvailableBlockSize(kIndefiniteSize);
            const auto space = builder.ToConstraintSpace();

            result = ComputeMinAndMaxContentContribution(Style(), legend, space);
            result.sizes += ComputeMarginsFor(space, legend.Style(), GetConstraintSpace()).InlineSum();
        }
    }
    ...
    if (!has_inline_size_containment) {
        ...
        MinMaxSizesResult content_result = ComputeMinAndMaxContentContribution(Style(), content, space);
        content_result.sizes += ComputeMarginsFor(space, content.Style(), GetConstraintSpace()).InlineSum();
...
const ConstraintSpace FieldsetLayoutAlgorithm::CreateConstraintSpaceForLegend(BlockNode legend, LogicalSize available_size, LogicalSize percentage_size)
{
    ConstraintSpaceBuilder builder(GetConstraintSpace(), legend.Style().GetWritingDirection(),
        /* is_new_fc */ true);
...
const ConstraintSpace FieldsetLayoutAlgorithm::CreateConstraintSpaceForFieldsetContent(
    BlockNode fieldset_content, LogicalSize padding_box_size, LayoutUnit block_offset)
{
    DCHECK(fieldset_content.CreatesNewFormattingContext());
    ConstraintSpaceBuilder builder(GetConstraintSpace(), fieldset_content.Style().GetWritingDirection(),
        /* is_new_fc */ true);
    ...
    builder.SetPercentageResolutionSize(GetConstraintSpace().PercentageResolutionSize());
    builder.SetIsFixedBlockSize(padding_box_size.block_size != kIndefiniteSize);
    builder.SetBaselineAlgorithmType(GetConstraintSpace().GetBaselineAlgorithmType());

    if (GetConstraintSpace().HasBlockFragmentation()) {
        SetupSpaceBuilderForFragmentation(container_builder_, fieldset_content, block_offset, &builder);
    }
    return builder.ToConstraintSpace();
}
