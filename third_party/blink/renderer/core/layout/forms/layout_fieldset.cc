...
void LayoutFieldset::AddChild(LayoutObject* new_child, LayoutObject* before_child)
{
    if (!new_child->IsText() && !new_child->IsAnonymous()) {
        ...
        Node* before_node = LayoutTreeBuilderTraversal::NextLayoutSibling(*before_child->GetNode());
...
void LayoutFieldset::InsertedIntoTree()
{
    ...
    LayoutBlock* fieldset_content = LayoutBlock::CreateAnonymousWithParentAndDisplay(this, display);
    ...
    fieldset_content->SetCanContainAbsolutePositionObjects(fieldset_content->ComputeIsAbsoluteContainer(fieldset_content->Style()));
    fieldset_content->SetCanContainFixedPositionObjects(fieldset_content->ComputeIsFixedContainer(fieldset_content->Style()));
}

void LayoutFieldset::UpdateAnonymousChildStyle(const LayoutObject*, ComputedStyleBuilder& child_style_builder) const
{
    ...
    child_style_builder.SetBorderBottomLeftRadius(StyleRef().BorderBottomLeftRadius());
    child_style_builder.SetBorderBottomRightRadius(StyleRef().BorderBottomRightRadius());
    child_style_builder.SetBorderTopLeftRadius(StyleRef().BorderTopLeftRadius());
    child_style_builder.SetBorderTopRightRadius(StyleRef().BorderTopRightRadius());
    ...
    child_style_builder.SetColumnRuleColor(GapDataList<StyleColor>(StyleColor(LayoutObject::ResolveColor(StyleRef(), GetCSSPropertyColumnRuleColor()))));
...
void LayoutFieldset::InvalidatePaint(const PaintInvalidatorContext& context) const
{
    ...
    if (legend_box && legend_box->ShouldCheckLayoutForPaintInvalidation()) {
        GetMutableForPainting().SetShouldDoFullPaintInvalidation(PaintInvalidationReason::kLayout);
    }
    LayoutBlockFlow::InvalidatePaint(context);
}

bool LayoutFieldset::BackgroundIsKnownToBeOpaqueInRect(const PhysicalRect& local_rect) const
{
...
LayoutBox* LayoutFieldset::FindInFlowLegend(const LayoutBlock& fieldset)
{
    ...
    for (LayoutObject* legend = fieldset.FirstChild(); legend; legend = legend->NextSibling()) {
