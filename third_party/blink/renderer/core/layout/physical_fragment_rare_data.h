...
class PhysicalFragmentRareData : public GarbageCollected<PhysicalFragmentRareData> {
public:
    explicit PhysicalFragmentRareData(wtf_size_t num_fields);
    PhysicalFragmentRareData(const PhysicalRect* scrollable_overflow, const PhysicalBoxStrut* borders, const PhysicalBoxStrut* scrollbar,
...
private:
    ...
    static_assert(sizeof(RareBitFieldType) * CHAR_BIT > static_cast<unsigned>(FieldId::kMaxValue), "RareBitFieldType is not big enough for FieldId.");
    ...
    struct RareField {
        union {
            ...
            std::unique_ptr<TableFragmentData::CollapsedBordersGeometry> table_collapsed_borders_geometry;
    ...
    {
        return ~(~static_cast<RareBitFieldType>(0) << static_cast<unsigned>(field_id));
    }
    ...
    template <bool allow_overwrite> RareField& EnsureField(FieldId field_id)
    {
    ...
    {
        return EnsureField<false>(field_id);
    }
