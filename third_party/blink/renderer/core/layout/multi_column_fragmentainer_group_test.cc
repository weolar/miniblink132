...
void MultiColumnFragmentainerGroupTest::SetUp()
{
    ...
    flow_thread_ = LayoutMultiColumnFlowThread::CreateAnonymous(GetDocument(), style);
    column_set_ = LayoutMultiColumnSet::CreateAnonymous(*flow_thread_, *flow_thread_->Style());
}
...
int MultiColumnFragmentainerGroupTest::GroupCount(const MultiColumnFragmentainerGroupList& group_list)
{
    int count = 0;
...
TEST_F(MultiColumnFragmentainerGroupTest, LotsOfContent)
{
    ...
    builder.Append("<div id='multicol' style='columns:3; column-gap:1px; width:101px; "
    ...
    const auto& fragmentainer_group = To<LayoutMultiColumnSet>(column_set)->FirstFragmentainerGroup();
