...
bool LayoutOutsideListMarker::NeedsOccupyWholeLine() const
{
    ...
    if (next_sibling && !next_sibling->IsInline() && !next_sibling->IsFloatingOrOutOfFlowPositioned() && next_sibling->GetNode()
        && (IsA<HTMLUListElement>(*next_sibling->GetNode()) || IsA<HTMLOListElement>(*next_sibling->GetNode())))
        return true;

    return false;
}

PositionWithAffinity LayoutOutsideListMarker::PositionForPoint(const PhysicalOffset&) const
{
