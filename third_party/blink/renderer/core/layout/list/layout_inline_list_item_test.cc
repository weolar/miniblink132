...
li {
  list-style: upper-alpha;
    ...
    auto* block_flow = OffsetMapping::GetInlineFormattingContextOf(*GetLayoutObjectByElementId("li3"));
...
<ol style="list-style-type: foo;"><li id="target"></li>)HTML");
    ...
    auto* rule = To<CSSCounterStyleRule>(sheet->cssRules(ASSERT_NO_EXCEPTION)->item(0));
    rule->setPrefix(GetDocument().GetExecutionContext(), "p");
    UpdateAllLifecyclePhasesForTest();

    auto* block_flow = OffsetMapping::GetInlineFormattingContextOf(*GetLayoutObjectByElementId("target"));
    ASSERT_TRUE(block_flow);
    EXPECT_TRUE(InlineNode::GetOffsetMapping(block_flow));
