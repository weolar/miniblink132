...
{
    InsertStyleElement("li { text-combine-upright: all; writing-mode: vertical-rl; }");
    ...
    const auto* const marker_layout_text = LayoutListItem::FindSymbolMarkerLayoutText(target.GetLayoutObject());
    const auto* const text_combine = To<LayoutTextCombine>(marker_layout_text->Parent());
    EXPECT_EQ(marker_layout_text, LayoutListItem::FindSymbolMarkerLayoutText(text_combine));
}
