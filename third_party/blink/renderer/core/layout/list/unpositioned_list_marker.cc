...
LayoutUnit UnpositionedListMarker::InlineOffset(const LayoutUnit marker_inline_size) const
{
    DCHECK(marker_layout_object_);
    LayoutObject* list_item = marker_layout_object_->Marker().ListItem(*marker_layout_object_);
    auto margins = ListMarker::InlineMarginsForOutside(list_item->GetDocument(), marker_layout_object_->StyleRef(), list_item->StyleRef(), marker_inline_size);
    return margins.first;
}

const LayoutResult* UnpositionedListMarker::Layout(const ConstraintSpace& parent_space, const ComputedStyle& parent_style, FontBaseline baseline_type) const
{
    DCHECK(marker_layout_object_);
    ...
    const LayoutResult* marker_layout_result
        = marker_node.LayoutAtomicInline(parent_space, parent_style, parent_space.UseFirstLineStyle(), BaselineAlgorithmType::kDefault);
...
std::optional<LayoutUnit> UnpositionedListMarker::ContentAlignmentBaseline(
    const ConstraintSpace& space, FontBaseline baseline_type, const PhysicalFragment& content) const
{
    ...
    return LogicalBoxFragment(space.GetWritingDirection(), To<PhysicalBoxFragment>(content)).FirstBaseline();
}

void UnpositionedListMarker::AddToBox(const ConstraintSpace& space, FontBaseline baseline_type, const PhysicalFragment& content,
    const BoxStrut& border_scrollbar_padding, const LayoutResult& marker_layout_result, LayoutUnit content_baseline, LayoutUnit* block_offset,
    BoxFragmentBuilder* container_builder) const
{
    const auto& marker_physical_fragment = To<PhysicalBoxFragment>(marker_layout_result.GetPhysicalFragment());

    // Compute the inline offset of the marker.
    LogicalBoxFragment marker_fragment(space.GetWritingDirection(), marker_physical_fragment);
    LogicalOffset marker_offset(InlineOffset(marker_fragment.Size().inline_size), *block_offset);
    ...
    marker_offset.inline_offset += ComputeIntrudedFloatOffset(space, container_builder, border_scrollbar_padding, marker_offset.block_offset);
...
void UnpositionedListMarker::AddToBoxWithoutLineBoxes(const ConstraintSpace& space, FontBaseline baseline_type, const LayoutResult& marker_layout_result,
    BoxFragmentBuilder* container_builder, LayoutUnit* intrinsic_block_size) const
{
    const auto& marker_physical_fragment = To<PhysicalBoxFragment>(marker_layout_result.GetPhysicalFragment());

    // When there are no line boxes, marker is top-aligned to the list item.
    // https://github.com/w3c/csswg-drafts/issues/2417
    LogicalSize marker_size = marker_physical_fragment.Size().ConvertToLogical(space.GetWritingMode());
    ...
    if (container_builder->BfcBlockOffset()) {
        *intrinsic_block_size = std::max(marker_size.block_size, *intrinsic_block_size);
        ...
        container_builder->SetFragmentsTotalBlockSize(std::max(marker_size.block_size, container_builder->Size().block_size));
    }
}
...
LayoutUnit UnpositionedListMarker::ComputeIntrudedFloatOffset(
    const ConstraintSpace& space, const BoxFragmentBuilder* container_builder, const BoxStrut& border_scrollbar_padding, LayoutUnit marker_block_offset) const
{
    DCHECK(container_builder);
    ...
    BfcOffset origin_offset
        = { container_builder->BfcLineOffset() + border_scrollbar_padding.inline_start, *container_builder->BfcBlockOffset() + marker_block_offset };
    const LayoutUnit available_size = container_builder->ChildAvailableSize().inline_size;
    LayoutOpportunity opportunity = space.GetExclusionSpace().FindLayoutOpportunity(origin_offset, available_size);
    DCHECK(marker_layout_object_);
