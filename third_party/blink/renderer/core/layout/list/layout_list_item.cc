...
void LayoutListItem::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    if (old_style) {
        ...
        if (old_list_style_type != new_list_style_type && (!old_list_style_type || !new_list_style_type || *old_list_style_type != *new_list_style_type))
...
{
    if (!StyleRef().ListStyleType() || StyleRef().ListStyleType()->IsCounterStyleReferenceValid(GetDocument())) {
        return;
    }
...
const LayoutObject* LayoutListItem::FindSymbolMarkerLayoutText(const LayoutObject* object)
{
    if (!object)
        return nullptr;
