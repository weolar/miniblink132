...
protected:
    ...
    {
        auto* list_item = To<LayoutListItem>(GetLayoutObjectByElementId(list_item_id));
    ...
    {
        return To<LayoutText>(GetMarker(scope, list_item_id)->SlowFirstChild())->TransformedText();
    }
    ...
    void AddCounterStyle(const AtomicString& name, const String& descriptors)
    {
        ...
        declaration.Append("}");
        Element* sheet = GetDocument().CreateElementForBinding(AtomicString("style"));
...
TEST_F(ListMarkerTest, OverridePredefinedCounterStyle)
{
    ...
    AddCounterStyle(AtomicString("upper-roman"), "system: fixed; symbols: A B C;");
    GetDocument().UpdateStyleAndLayoutTree();
...
TEST_F(ListMarkerTest, ModifyShadowDOMWithOwnCounterStyles)
{
    ...
    ShadowRoot& shadow1 = GetElementById("host1")->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    ...
    // outside.
    ShadowRoot& shadow2 = GetElementById("host2")->AttachShadowRootForTesting(ShadowRootMode::kOpen);
...
TEST_F(ListMarkerTest, WidthOfSymbolForFontSizeZero)
{
    ...
    EXPECT_EQ(LayoutUnit(), ListMarker::WidthOfSymbol(target_layout_object.StyleRef(), target_layout_object.StyleRef().ListStyleType()->GetCounterStyleName()));
}
...
TEST_F(ListMarkerTest, InlineMarginsForOutside)
{
  ...
  ">foo</summary></details>)HTML",
    ...
    auto [start, end] = ListMarker::InlineMarginsForOutside(GetDocument(), marker_object->StyleRef(), item_object->StyleRef(), LayoutUnit::Max());
