...
{
    return const_cast<ListMarker*>(ListMarker::Get(static_cast<const LayoutObject*>(marker)));
}
...
void ListMarker::ListStyleTypeChanged(LayoutObject& marker)
{
    ...
    marker.SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kListStyleTypeChange);
}
...
void ListMarker::CounterStyleChanged(LayoutObject& marker)
{
    ...
    marker_text_type_ = kUnresolved;
    marker.SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kCounterStyleChange);
}
...
void ListMarker::OrdinalValueChanged(LayoutObject& marker)
{
    ...
    if (marker_text_type_ == kOrdinalValue) {
        marker_text_type_ = kUnresolved;
        marker.SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kListValueChange);
    }
}
...
void ListMarker::UpdateMarkerText(LayoutObject& marker)
{
    ...
    marker_text_type_ = MarkerText(marker, &marker_text_builder, kWithPrefixSuffix);
...
ListMarker::MarkerTextType ListMarker::MarkerText(const LayoutObject& marker, StringBuilder* text, MarkerTextFormat format) const
{
    DCHECK_EQ(Get(&marker), this);
    ...
    case ListStyleCategory::kSymbol: {
        const CounterStyle& counter_style = GetCounterStyle(marker.GetDocument(), style);
        switch (format) {
        case kWithPrefixSuffix:
            text->Append(counter_style.GenerateRepresentationWithPrefixAndSuffix(0));
            break;
    ...
    case ListStyleCategory::kLanguage: {
        ...
        switch (format) {
        case kWithPrefixSuffix:
            text->Append(counter_style.GenerateRepresentationWithPrefixAndSuffix(value));
            break;
        case kWithoutPrefixSuffix:
...
String ListMarker::TextAlternative(const LayoutObject& marker) const
{
    ...
    if (marker_text_type_ == kNotText) {
        ...
        if (text_child && !text_child->NextSibling() && IsA<LayoutTextFragment>(text_child)) {
...
void ListMarker::UpdateMarkerContentIfNeeded(LayoutObject& marker)
{
    ...
    if (IsMarkerImage(marker)) {
        ...
        if (child) {
            ...
            if (!child->IsLayoutImage() || To<LayoutImage>(child)->ImageResource()->ImagePtr() != list_style_image->Data()) {
        ...
        if (!child) {
            LayoutListMarkerImage* image = LayoutListMarkerImage::CreateAnonymous(&marker.GetDocument());
            const ComputedStyle* image_style = marker.GetDocument().GetStyleResolver().CreateAnonymousStyleWithDisplay(marker.StyleRef(), EDisplay::kInline);
            image->SetStyle(image_style);
            image->SetImageResource(MakeGarbageCollected<LayoutImageResourceStyleImage>(list_style_image));
    ...
    const ComputedStyle* text_style
        = marker.GetDocument().GetStyleResolver().CreateAnonymousStyleWithDisplay(style_parent.StyleRef(), marker.StyleRef().Display());
    ...
    auto* const new_text = LayoutTextFragment::CreateAnonymous(marker.GetDocument(), StringImpl::empty_, 0, 0);
    new_text->SetStyle(std::move(text_style));
    marker.AddChild(new_text);
    marker_text_type_ = kUnresolved;
}

LayoutObject* ListMarker::SymbolMarkerLayoutText(const LayoutObject& marker) const
{
    DCHECK_EQ(Get(&marker), this);
...
{
    DCHECK_EQ(Get(&marker), this);
    return marker.StyleRef().ContentBehavesAsNormal() && ListItem(marker)->StyleRef().GeneratesMarkerImage();
}

LayoutUnit ListMarker::WidthOfSymbol(const ComputedStyle& style, const AtomicString& list_style)
{
...
std::pair<LayoutUnit, LayoutUnit> ListMarker::InlineMarginsForInside(
    Document& document, const ComputedStyleBuilder& marker_style_builder, const ComputedStyle& list_item_style)
{
    ...
    case ListStyleCategory::kSymbol: {
        const AtomicString& name = list_item_style.ListStyleType()->GetCounterStyleName();
        if (name == keywords::kDisclosureOpen || name == keywords::kDisclosureClosed) {
            return { LayoutUnit(), LayoutUnit(kClosureMarkerMarginEm * marker_style_builder.GetFontDescription().SpecifiedSize()) };
        }
        return { LayoutUnit(-1), LayoutUnit(kCUAMarkerMarginEm * marker_style_builder.GetFontDescription().ComputedSize()) };
    }
    default:
        break;
    }
...
std::pair<LayoutUnit, LayoutUnit> ListMarker::InlineMarginsForOutside(
    Document& document, const ComputedStyle& marker_style, const ComputedStyle& list_item_style, LayoutUnit marker_inline_size)
{
    ...
    if (!marker_style.ContentBehavesAsNormal()) {
        ...
        case ListStyleCategory::kSymbol: {
            ...
            const AtomicString& name = list_item_style.ListStyleType()->GetCounterStyleName();
            LayoutUnit offset = (name == keywords::kDisclosureOpen || name == keywords::kDisclosureClosed) ? DisclosureSymbolSize(marker_style)
...
PhysicalRect ListMarker::RelativeSymbolMarkerRect(const ComputedStyle& style, const AtomicString& list_style, LayoutUnit width)
{
    ...
    if (list_style == keywords::kDisclosureOpen || list_style == keywords::kDisclosureClosed) {
        ...
        relative_rect = LogicalRect(LayoutUnit(), ascent - marker_size, marker_size, marker_size);
    } else {
        LayoutUnit bullet_width = LayoutUnit((ascent * 2 / 3 + 1) / 2);
        relative_rect = LogicalRect(LayoutUnit(1), LayoutUnit(3 * (ascent - ascent * 2 / 3) / 2), bullet_width, bullet_width);
    }
    // TextDirection doesn't matter here.  Passing
    // `relative_rect.size.inline_size` to get a correct result in sideways-lr.
    WritingModeConverter converter({ ToLineWritingMode(style.GetWritingMode()), TextDirection::kLtr }, PhysicalSize(width, relative_rect.size.inline_size));
    return converter.ToPhysical(relative_rect);
}

const CounterStyle& ListMarker::GetCounterStyle(Document& document, const ComputedStyle& style)
{
...
ListMarker::ListStyleCategory ListMarker::GetListStyleCategory(Document& document, const ComputedStyle& style)
{
    ...
    return GetCounterStyle(document, style).IsPredefinedSymbolMarker() ? ListStyleCategory::kSymbol : ListStyleCategory::kLanguage;
}
