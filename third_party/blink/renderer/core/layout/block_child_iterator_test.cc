...
const BlockBreakToken* CreateBreakToken(LayoutInputNode node, const BreakTokenVector* child_break_tokens = nullptr, bool has_seen_all_children = false)
{
    WritingDirectionMode writing_direction(WritingMode::kHorizontalTb, TextDirection::kLtr);
    ConstraintSpaceBuilder space_builder(writing_direction.GetWritingMode(), writing_direction,
        /* is_new_fc */ true);
    BoxFragmentBuilder fragment_builder(node, &node.Style(), space_builder.ToConstraintSpace(), writing_direction,
        /*previous_break_token=*/nullptr);
    DCHECK(!fragment_builder.HasBreakTokenData());
    fragment_builder.SetBreakTokenData(MakeGarbageCollected<BlockBreakTokenData>());
...
TEST_F(BlockChildIteratorTest, BreakTokens)
{
    ...
    const BlockBreakToken* parent_token = CreateBreakToken(container, &child_break_tokens);

    BlockChildIterator iterator(node1, parent_token);
    ASSERT_EQ(BlockChildIterator::Entry(node1, child_token1), iterator.NextChild());
    ASSERT_EQ(BlockChildIterator::Entry(node2, nullptr), iterator.NextChild());
    ASSERT_EQ(BlockChildIterator::Entry(node3, nullptr), iterator.NextChild());
    ...
    ASSERT_EQ(BlockChildIterator::Entry(node1, child_token1), iterator.NextChild());
    ASSERT_EQ(BlockChildIterator::Entry(node2, child_token2), iterator.NextChild());
    ASSERT_EQ(BlockChildIterator::Entry(node3, nullptr), iterator.NextChild());
    ASSERT_EQ(BlockChildIterator::Entry(node4, nullptr), iterator.NextChild());
    ASSERT_EQ(BlockChildIterator::Entry(nullptr, nullptr), iterator.NextChild());
    ...
    iterator = BlockChildIterator(node1, parent_token);
    ASSERT_EQ(BlockChildIterator::Entry(node2, child_token2), iterator.NextChild());
    ASSERT_EQ(BlockChildIterator::Entry(node3, child_token3), iterator.NextChild());
    ASSERT_EQ(BlockChildIterator::Entry(node4, nullptr), iterator.NextChild());
    ASSERT_EQ(BlockChildIterator::Entry(nullptr, nullptr), iterator.NextChild());
...
TEST_F(BlockChildIteratorTest, SeenAllChildren)
{
    ...
    BreakTokenVector child_break_tokens;
    child_break_tokens.push_back(child_token1);
    const BlockBreakToken* parent_token = CreateBreakToken(container, &child_break_tokens, /* has_seen_all_children*/ true);
...
TEST_F(BlockChildIteratorTest, DeleteNodeWhileIteration)
{
    ...
    EXPECT_EQ(Entry(node1, nullptr), iterator.NextChild());
    {
        ...
        base::AutoReset<bool> cq_recalc(&GetDocument().GetStyleEngine().in_container_query_style_recalc_, true);
