...
inline LayoutMultiColumnFlowThread* GetFlowThread(const LayoutBlockFlow* block_flow)
{
    if (!block_flow)
        return nullptr;
...
// inline.
template <typename Algorithm, typename Callback> NOINLINE void CreateAlgorithmAndRun(const LayoutAlgorithmParams& params, const Callback& callback)
{
    Algorithm algorithm(params);
    callback(&algorithm);
}

template <typename Callback> NOINLINE void DetermineMathMLAlgorithmAndRun(const LayoutBox& box, const LayoutAlgorithmParams& params, const Callback& callback)
{
    DCHECK(box.IsMathML());
    ...
    if (element) {
        if (IsA<MathMLSpaceElement>(element)) {
            ...
            return;
        } else if (IsA<MathMLFractionElement>(element) && IsValidMathMLFraction(params.node)) {
            CreateAlgorithmAndRun<MathFractionLayoutAlgorithm>(params, callback);
            return;
        } else if (IsA<MathMLRadicalElement>(element) && IsValidMathMLRadical(params.node)) {
            ...
            return;
        } else if (IsA<MathMLScriptsElement>(element) && IsValidMathMLScript(params.node)) {
...
template <typename Callback> NOINLINE void DetermineAlgorithmAndRun(const LayoutAlgorithmParams& params, const Callback& callback)
{
...
inline const LayoutResult* LayoutWithAlgorithm(const LayoutAlgorithmParams& params)
{
    const LayoutResult* result = nullptr;
    DetermineAlgorithmAndRun(params, [&result]<typename Algorithm>(Algorithm* algorithm) { result = algorithm->Layout(); });
    return result;
}

inline MinMaxSizesResult ComputeMinMaxSizesWithAlgorithm(const LayoutAlgorithmParams& params, const MinMaxSizesFloatInput& float_input)
{
    MinMaxSizesResult result;
    DetermineAlgorithmAndRun(
        params, [&result, &float_input]<typename Algorithm>(Algorithm* algorithm) { result = algorithm->ComputeMinMaxSizes(float_input); });
    return result;
}

bool CanUseCachedIntrinsicInlineSizes(const ConstraintSpace& constraint_space, const MinMaxSizesFloatInput& float_input, const BlockNode& node)
{
    ...
    if (style.MayHavePadding()
        && (style.PaddingTop().HasPercent() || style.PaddingRight().HasPercent() || style.PaddingBottom().HasPercent() || style.PaddingLeft().HasPercent())) {
        return false;
    }

    if (node.IsTableCell() && To<LayoutTableCell>(node.GetLayoutBox())->IntrinsicLogicalWidthsBorderSizes() != constraint_space.TableCellBorders()) {
        return false;
    }
    ...
    if (node.IsGrid()) {
        if (style.LogicalMinWidth().HasPercentOrStretch() || style.LogicalMaxWidth().HasPercentOrStretch()) {
            return false;
        }
        // Also consider transferred min/max sizes.
        if (node.HasAspectRatio() && (style.LogicalMinHeight().HasPercentOrStretch() || style.LogicalMaxHeight().HasPercentOrStretch())) {
            return false;
        }
    }
...
std::optional<LayoutUnit> ContentMinimumInlineSize(const BlockNode& block_node, const BoxStrut& border_padding)
{
    ...
    LayoutUnit inline_size = MinimumValueForLength(main_inline_size, LayoutUnit());
...
void AttachScrollMarkers(LayoutObject& parent, Node::AttachContext& context, bool has_absolute_containment = false, bool has_fixed_containment = false)
{
    ...
    for (LayoutObject* child = parent.SlowFirstChild(); child; child = child->NextSibling()) {
        if ((child->IsFixedPositioned() && !has_fixed_containment) || (child->IsAbsolutePositioned() && !has_absolute_containment)) {
            continue;
        }
        if (auto* element = DynamicTo<Element>(child->GetNode())) {
            if (PseudoElement* marker = element->GetPseudoElement(kPseudoIdScrollMarker)) {
        ...
        if (!child->IsScrollMarkerGroup() && !child->GetScrollMarkerGroup()) {
            AttachScrollMarkers(*child, context, has_absolute_containment, has_fixed_containment);
        }
    }
    ...
    if (parent_box && parent_box->IsFragmentationContextRoot()) {
        if (const ColumnPseudoElementsVector* column_pseudos = To<Element>(parent.EnclosingNode())->GetColumnPseudoElements()) {
            for (const auto& column_pseudo : *column_pseudos) {
                if (PseudoElement* scroll_marker = column_pseudo->GetPseudoElement(kPseudoIdScrollMarker)) {
...
const LayoutResult* BlockNode::Layout(const ConstraintSpace& constraint_space, const BlockBreakToken* break_token, const EarlyBreak* early_break,
    const ColumnSpannerPath* column_spanner_path) const
{
    ...
    if (const LayoutResult* previous_result = box_->GetCachedLayoutResult(break_token)) {
        constraint_space.GetExclusionSpace().PreInitialize(previous_result->GetConstraintSpaceForCaching().GetExclusionSpace());
    }
    ...
    const LayoutResult* layout_result
        = box_->CachedLayoutResult(constraint_space, break_token, early_break, column_spanner_path, &fragment_geometry, &cache_status);

    if ((cache_status == LayoutCacheStatus::kHit || cache_status == LayoutCacheStatus::kNeedsSimplifiedLayout) && needed_layout
        && constraint_space.CacheSlot() == LayoutResultCacheSlot::kLayout && box_->HasBrokenSpine() && !ChildLayoutBlockedByDisplayLock()) {
        ...
        const auto& new_fragment = To<PhysicalBoxFragment>(layout_result->GetPhysicalFragment());
        ...
        // items).
        bool clear_trailing_results = new_fragment.GetBreakToken() && new_fragment.HasItems();
    ...
    if (cache_status == LayoutCacheStatus::kHit) {
        ...
        UpdateMarginPaddingInfoIfNeeded(constraint_space, layout_result->GetPhysicalFragment());
    ...
    if (!fragment_geometry) {
        fragment_geometry = CalculateInitialFragmentGeometry(constraint_space, *this, break_token);
    }
    ...
    if (!IsBreakInside(break_token) && CanMatchSizeContainerQueries()) {
        if (auto* element = DynamicTo<Element>(GetDOMNode())) {
            ...
            BoxStrut scrollbar = fragment_geometry->scrollbar;
            {
                ...
                if (style.IsScrollbarGutterAuto() && style.OverflowBlockDirection() == EOverflow::kAuto) {
            ...
            const LogicalSize available_size = CalculateChildAvailableSize(
                constraint_space, *this, fragment_geometry->border_box_size, fragment_geometry->border + scrollbar + fragment_geometry->padding);
            GetDocument().GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*element, available_size, ContainedAxes());
            ...
            layout_result = box_->CachedLayoutResult(constraint_space, break_token, early_break, column_spanner_path, &fragment_geometry, &cache_status);
        }
    }

    TextAutosizer::NGLayoutScope text_autosizer_layout_scope(box_, fragment_geometry->border_box_size.inline_size);

    PrepareForLayout();

    LayoutAlgorithmParams params(*this, *fragment_geometry, constraint_space, break_token, early_break);
    ...
    if (cache_status == LayoutCacheStatus::kNeedsSimplifiedLayout && (!block_flow || !block_flow->IsFragmentationContextRoot())) {
        DCHECK(layout_result);
#if DCHECK_IS_ON()
...
#if DCHECK_IS_ON()
        if (layout_result) {
            layout_result->CheckSameForSimplifiedLayout(*previous_result, /* check_same_block_size */ !block_flow);
        }
#endif
    ...
    const LayoutUnit inline_size_before = fragment_geometry->border_box_size.inline_size;
    const bool intrinsic_logical_widths_dirty_before = box_->IntrinsicLogicalWidthsDirty();
    ...
    if (layout_result->Status() == LayoutResult::kSuccess && !layout_result->GetPhysicalFragment().GetBreakToken()) {
        optional_old_box_size = box_->Size();
    }

    FinishLayout(block_flow, constraint_space, break_token, layout_result, optional_old_box_size);
    ...
    if (!intrinsic_logical_widths_dirty_before && box_->IntrinsicLogicalWidthsDirty()) {
        fragment_geometry = CalculateInitialFragmentGeometry(constraint_space, *this, break_token);
    }
    ...
    if ((scrollbars_before != scrollbars_after || inline_size_before != fragment_geometry->border_box_size.inline_size)
        && !DisableLayoutSideEffectsScope::IsDisabled() && !IsBreakInside(break_token)) {
        ...
        do {
            ...
            AddScrollbarFreeze(scrollbars_before, scrollbars_after, constraint_space.GetWritingDirection(), &freeze_horizontal, &freeze_vertical);
            scrollbars_before = scrollbars_after;
            PaintLayerScrollableArea::FreezeScrollbarsRootScope freezer(*box_, freeze_horizontal, freeze_vertical);
            ...
            DEFINE_STATIC_LOCAL(
                Persistent<HeapHashSet<WeakMember<LayoutBox>>>, scrollbar_changed, (MakeGarbageCollected<HeapHashSet<WeakMember<LayoutBox>>>()));
            DCHECK(scrollbar_changed->insert(box_.Get()).is_new_entry);
#endif

            // Scrollbar changes are hard to detect. Make sure everyone gets the
            // message.
            box_->SetNeedsLayout(layout_invalidation_reason::kScrollbarChanged, kMarkOnlyThis);
            ...
            fragment_geometry = CalculateInitialFragmentGeometry(constraint_space, *this, break_token);
            layout_result = LayoutWithAlgorithm(params);
            FinishLayout(block_flow, constraint_space, break_token, layout_result, old_box_size);
...
const LayoutResult* BlockNode::SimplifiedLayout(const PhysicalFragment& previous_fragment) const
{
    ...
    DCHECK(box_->NeedsSimplifiedLayoutOnly() || box_->ChildLayoutBlockedByDisplayLock());
    ...
    const auto& old_fragment = To<PhysicalBoxFragment>(previous_result->GetPhysicalFragment());
    const auto& new_fragment = To<PhysicalBoxFragment>(result->GetPhysicalFragment());
...
const LayoutResult* BlockNode::LayoutRepeatableRoot(const ConstraintSpace& constraint_space, const BlockBreakToken* break_token) const
{
    ...
    const BlockBreakToken* outgoing_break_token = BlockBreakToken::CreateRepeated(*this, index);
...
void BlockNode::FinishRepeatableRoot() const
{
    ...
    for (wtf_size_t i = 1; i < fragment_count; i++) {
        const PhysicalBoxFragment& physical_fragment = *box_->GetPhysicalFragment(i);
        bool is_first = i == 1;
...
void BlockNode::FinishLayout(LayoutBlockFlow* block_flow, const ConstraintSpace& constraint_space, const BlockBreakToken* break_token,
    const LayoutResult* layout_result, const std::optional<PhysicalSize>& old_box_size) const
{
    ...
    if (auto* svg_root = DynamicTo<LayoutSVGRoot>(GetLayoutBox())) {
        ...
        if (!svg_root->NeedsLayout()) {
            svg_root->SetNeedsLayout(layout_invalidation_reason::kSizeChanged, kMarkOnlyThis);
        }
    ...
    bool clear_trailing_results = break_token || box_->PhysicalFragmentCount() > 1;
...
void BlockNode::StoreResultInLayoutBox(const LayoutResult* result, const BlockBreakToken* break_token, bool clear_trailing_results) const
{
    const auto& fragment = To<PhysicalBoxFragment>(result->GetPhysicalFragment());
...
MinMaxSizesResult BlockNode::ComputeMinMaxSizes(
    WritingMode container_writing_mode, const SizeType type, const ConstraintSpace& constraint_space, const MinMaxSizesFloatInput float_input) const
{
    // TODO(layoutng) Can UpdateMarkerTextIfNeeded call be moved
    // somewhere else? List items need up-to-date markers before layout.
    if (IsListItem())
    ...
    auto IntrinsicFragmentGeometry = [&]() -> FragmentGeometry& {
        if (!cached_fragment_geometry) {
            cached_fragment_geometry = CalculateInitialFragmentGeometry(constraint_space, *this,
                /* break_token */ nullptr,
                /* is_intrinsic */ true);
        }
    ...
    if (!is_in_perform_layout && (IsGrid() || (IsFlexibleBox() && Style().ResolvedIsColumnFlexDirection()))) {
    ...
    bool is_orthogonal_flow_root = !IsParallelWritingMode(container_writing_mode, Style().GetWritingMode());
    ...
    if (is_orthogonal_flow_root) {
        ...
        sizes = LogicalFragment({ container_writing_mode, TextDirection::kLtr }, layout_result->GetPhysicalFragment()).InlineSize();
        const bool depends_on_block_constraints = Style().LogicalWidth().HasAuto() || Style().LogicalWidth().HasPercentOrStretch()
            || Style().LogicalMinWidth().HasPercentOrStretch() || Style().LogicalMaxWidth().HasPercentOrStretch();
        return MinMaxSizesResult(sizes, depends_on_block_constraints);
    }

    // Returns if we are (directly) dependent on any block constraints.
    auto DependsOnBlockConstraints = [&]() -> bool {
        return Style().LogicalHeight().HasPercentOrStretch() || Style().LogicalMinHeight().HasPercentOrStretch()
            || Style().LogicalMaxHeight().HasPercentOrStretch() || (Style().LogicalHeight().HasAuto() && constraint_space.IsBlockAutoBehaviorStretch());
    };
    ...
    if (has_aspect_ratio && type == SizeType::kContent) {
        ...
        if (fragment_geometry.border_box_size.block_size != kIndefiniteSize) {
            const LayoutUnit inline_size_from_ar = InlineSizeFromAspectRatio(
                border_padding, Style().LogicalAspectRatio(), Style().BoxSizingForAspectRatio(), fragment_geometry.border_box_size.block_size);
            return MinMaxSizesResult({ inline_size_from_ar, inline_size_from_ar }, DependsOnBlockConstraints(),
                /* applied_aspect_ratio */ true);
        }
    }

    bool can_use_cached_intrinsic_inline_sizes = CanUseCachedIntrinsicInlineSizes(constraint_space, float_input, *this);
    ...
    // block constraints.
    if (can_use_cached_intrinsic_inline_sizes && !box_->IntrinsicLogicalWidthsDependsOnBlockConstraints()) {
        result = box_->CachedIndefiniteIntrinsicLogicalWidths();
    }

    // We might still be able to use the cached values for a specific initial
    // block-size.
    if (!result && can_use_cached_intrinsic_inline_sizes && !UseParentPercentageResolutionBlockSizeForChildren()) {
        result = box_->CachedIntrinsicLogicalWidths(IntrinsicFragmentGeometry().border_box_size.block_size);
    }

    if (!result) {
        const FragmentGeometry& fragment_geometry = IntrinsicFragmentGeometry();
        result = ComputeMinMaxSizesWithAlgorithm(LayoutAlgorithmParams(*this, fragment_geometry, constraint_space), float_input);
        ...
        box_->SetIntrinsicLogicalWidths(fragment_geometry.border_box_size.block_size, *result);
        if (IsTableCell()) {
            To<LayoutTableCell>(box_.Get())->SetIntrinsicLogicalWidthsBorderSizes(constraint_space.TableCellBorders());
        }
    }
    ...
    if (has_aspect_ratio) {
        ...
        if (fragment_geometry.border_box_size.block_size == kIndefiniteSize) {
            ...
            const BoxStrut border_padding = fragment_geometry.border + fragment_geometry.padding;
            const MinMaxSizes min_max = ComputeMinMaxInlineSizesFromAspectRatio(constraint_space, *this, border_padding);
    ...
    result->depends_on_block_constraints
        = (DependsOnBlockConstraints() || UseParentPercentageResolutionBlockSizeForChildren()) && (result->depends_on_block_constraints || has_aspect_ratio);
    return *result;
}
...
BlockNode BlockNode::GetRenderedLegend() const
{
    if (!IsFieldsetContainer())
        return nullptr;
    return BlockNode(LayoutFieldset::FindInFlowLegend(*To<LayoutBlock>(box_.Get())));
}
...
BlockNode BlockNode::GetFieldsetContent() const
{
    if (!IsFieldsetContainer())
        return nullptr;
    return BlockNode(To<LayoutFieldset>(box_.Get())->FindAnonymousFieldsetContentBox());
}

LayoutUnit BlockNode::EmptyLineBlockSize(const BlockBreakToken* incoming_break_token) const
{
...
{
    return String::Format("BlockNode: %s", GetLayoutBox()->ToString().Ascii().c_str());
}

void BlockNode::CopyFragmentDataToLayoutBox(
    const ConstraintSpace& constraint_space, const LayoutResult& layout_result, const BlockBreakToken* previous_break_token) const
{
    const auto& physical_fragment = To<PhysicalBoxFragment>(layout_result.GetPhysicalFragment());
    ...
    if (!ChildLayoutBlockedByDisplayLock()) {
        if (flow_thread) [[unlikely]] {
            ...
            if (is_last_fragment) {
                ...
                for (const PhysicalBoxFragment& multicol_fragment : box_->PhysicalFragments()) {
                    PlaceChildrenInFlowThread(flow_thread, constraint_space, multicol_fragment, incoming_break_token);
    ...
    box_->SetNeedsOverflowRecalc(LayoutObject::OverflowRecalcType::kOnlyVisualOverflowRecalc);
...
void BlockNode::PlaceChildrenInLayoutBox(
    const PhysicalBoxFragment& physical_fragment, const BlockBreakToken* previous_break_token, bool needs_invalidation_check) const
{
    for (const auto& child_fragment : physical_fragment.Children()) {
        ...
        if (physical_fragment.IsFragmentainerBox() && child_fragment->IsOutOfFlowPositioned()) [[unlikely]] {
            continue;
        }

        CopyChildFragmentPosition(box_fragment, child_fragment.offset, physical_fragment, previous_break_token, needs_invalidation_check);
    }
}

void BlockNode::PlaceChildrenInFlowThread(LayoutMultiColumnFlowThread* flow_thread, const ConstraintSpace& space, const PhysicalBoxFragment& physical_fragment,
    const BlockBreakToken* previous_container_break_token) const
{
    ...
    WritingModeConverter converter(space.GetWritingDirection(), physical_fragment.Size());
    ...
    if (IsBreakInside(previous_container_break_token)) {
        ...
        const auto& child_break_tokens = previous_container_break_token->ChildBreakTokens();
    ...
    for (const auto& child : physical_fragment.Children()) {
        ...
        if (child_box && child_box != box_) {
            CopyChildFragmentPosition(child_fragment, child.offset, physical_fragment);
            continue;
        }
        ...
        if (const FragmentItems* items = child_fragment.Items()) {
            CopyFragmentItemsToLayoutBox(child_fragment, *items, previous_column_break_token);
        }
...
void BlockNode::CopyChildFragmentPosition(const PhysicalBoxFragment& child_fragment, PhysicalOffset offset, const PhysicalBoxFragment& container_fragment,
    const BlockBreakToken* previous_container_break_token, bool needs_invalidation_check) const
{
    auto* layout_box = To<LayoutBox>(child_fragment.GetMutableLayoutObject());
    if (!layout_box)
        return;

    if (child_fragment.GetBoxType() == PhysicalFragment::kPageContainer || child_fragment.GetBoxType() == PhysicalFragment::kPageBorderBox) {
    ...
    LayoutPoint point = LayoutBoxUtils::ComputeLocation(child_fragment, offset, container_fragment, previous_container_break_token);
...
void BlockNode::MakeRoomForExtraColumns(LayoutUnit block_size) const
{
    ...
    MultiColumnFragmentainerGroup& last_group = block_flow->MultiColumnFlowThread()->LastMultiColumnSet()->LastFragmentainerGroup();
...
void BlockNode::FinishPageContainerLayout(const LayoutResult* result) const
{
    ...
    DCHECK(result->GetPhysicalFragment().GetBoxType() == PhysicalFragment::kPageContainer
        || result->GetPhysicalFragment().GetBoxType() == PhysicalFragment::kPageBorderBox);
...
void BlockNode::CopyFragmentItemsToLayoutBox(
    const PhysicalBoxFragment& container, const FragmentItems& items, const BlockBreakToken* previous_break_token) const
{
    LayoutUnit previously_consumed_block_size;
    if (previous_break_token) {
        previously_consumed_block_size = previous_break_token->ConsumedBlockSizeForLegacy();
    }
        ...
        if (const PhysicalBoxFragment* child = cursor.Current().BoxFragment()) {
            ...
            if (auto* layout_box = DynamicTo<LayoutBox>(layout_object)) {
                ...
                if (initial_container_is_flipped) {
                    maybe_flipped_offset.left = container.Size().width - child->Size().width - maybe_flipped_offset.left;
                }
...
bool BlockNode::IsInlineFormattingContextRoot(InlineNode* first_child_out) const
{
...
bool BlockNode::HasAspectRatio() const
{
    ...
    if (!layout_object->IsImage() && !IsA<LayoutVideo>(layout_object) && !layout_object->IsCanvas() && !layout_object->IsSVGRoot()) {
        return false;
    }
...
LogicalSize BlockNode::GetAspectRatio() const
{
    ...
    if (ar_type == EAspectRatioType::kRatio || (ar_type == EAspectRatioType::kAutoAndRatio && !IsReplaced())) {
        return Style().LogicalAspectRatio();
    }

    if (!ShouldApplySizeContainment()) {
        IntrinsicSizingInfo legacy_sizing_info;
        To<LayoutReplaced>(box_.Get())->ComputeIntrinsicSizingInfo(legacy_sizing_info);
        if (!legacy_sizing_info.aspect_ratio.IsEmpty()) {
            return StyleAspectRatio::LayoutRatioFromSizeF(legacy_sizing_info.aspect_ratio).ConvertToLogical(Style().GetWritingMode());
        }
    }
...
std::optional<gfx::Transform> BlockNode::GetTransformForChildFragment(const PhysicalBoxFragment& child_fragment, PhysicalSize size) const
{
    ...
    if (!child_fragment.IsOnlyForNode()) {
        ...
        child_fragment.Style().ApplyTransform(*fragment_transform, box_, reference_box, ComputedStyle::kIncludeTransformOperations,
            ComputedStyle::kIncludeTransformOrigin, ComputedStyle::kIncludeMotionPath, ComputedStyle::kIncludeIndependentTransformProperties);
    }

    gfx::Transform transform;
    child_layout_object->GetTransformFromContainer(box_, PhysicalOffset(), transform, &size, base::OptionalToPtr(fragment_transform));
...
void BlockNode::HandleScrollMarkerGroup() const
{
...

    {
        ...
        while (LayoutObject* child = group_node.GetLayoutBox()->SlowFirstChild()) {
            ...
            for (LayoutObject* walker = child; walker; walker = walker->NextInPreOrder(child)) {
                if (walker->GetNode() && walker->GetNode()->IsScrollMarkerPseudoElement()) {
        ...
        auto* scroll_marker_group = To<ScrollMarkerGroupPseudoElement>(group_node.GetLayoutBox()->GetNode());
        scroll_marker_group->ClearFocusGroup();
        if (PseudoElement* scroll_next_button = scroll_marker_group->UltimateOriginatingElement()->GetPseudoElement(kPseudoIdScrollNextButton)) {
            To<ScrollButtonPseudoElement>(scroll_next_button)->SetScrollMarkerGroup(scroll_marker_group);
        }
        if (PseudoElement* scroll_prev_button = scroll_marker_group->UltimateOriginatingElement()->GetPseudoElement(kPseudoIdScrollPrevButton)) {
            To<ScrollButtonPseudoElement>(scroll_prev_button)->SetScrollMarkerGroup(scroll_marker_group);
        }
    ...
    const LayoutResult* result = group_node.GetLayoutBox()->GetCachedLayoutResult(nullptr);
    if (!result) {
    ...
    // really "cloning".
    fragment.GetMutableForCloning().ReplaceChildren(To<PhysicalBoxFragment>(new_result->GetPhysicalFragment()));
...
const LayoutResult* BlockNode::LayoutAtomicInline(
    const ConstraintSpace& parent_constraint_space, const ComputedStyle& parent_style, bool use_first_line_style, BaselineAlgorithmType baseline_algorithm_type)
{
    ConstraintSpaceBuilder builder(parent_constraint_space, Style().GetWritingDirection(),
        /* is_new_fc */ true);
    ...
    builder.SetPercentageResolutionSize(parent_constraint_space.PercentageResolutionSize());
    builder.SetReplacedPercentageResolutionSize(parent_constraint_space.ReplacedPercentageResolutionSize());
...
const LayoutResult* BlockNode::RunSimplifiedLayout(const LayoutAlgorithmParams& params, const LayoutResult& previous_result) const
{
    SimplifiedLayoutAlgorithm algorithm(params, previous_result);
    if (const auto* previous_box_fragment = DynamicTo<PhysicalBoxFragment>(&previous_result.GetPhysicalFragment())) {
...
void BlockNode::UpdateMarginPaddingInfoIfNeeded(const ConstraintSpace& space, const PhysicalFragment& fragment) const
{
    ...
    if (Style().MayHaveMargin()) {
        ...
        To<PhysicalBoxFragment>(fragment).GetMutableForContainerLayout().SetMargins(ComputePhysicalMargins(space, Style()));
    ...
    if (Style().MayHaveMargin() || Style().MayHavePadding()) {
        ...
        if (containing_block && containing_block->IsLayoutGrid()) [[unlikely]] {
            box_->SetOverrideContainingBlockContentLogicalWidth(space.MarginPaddingPercentageResolutionSize().inline_size);
        }
    }
}
...
void BlockNode::UpdateShapeOutsideInfoIfNeeded(const LayoutResult& layout_result, const ConstraintSpace& constraint_space) const
{
    ...
    BoxStrut margins = ComputePhysicalMargins(constraint_space, Style()).ConvertToLogical({ writing_mode, TextDirection::kLtr });
    shape_outside->SetReferenceBoxLogicalSize(box_size.ConvertToLogical(writing_mode), LogicalSize(margins.InlineSum(), margins.BlockSum()));
    shape_outside->SetPercentageResolutionInlineSize(constraint_space.PercentageResolutionInlineSize());
}

void BlockNode::StoreColumnSizeAndCount(LayoutUnit inline_size, int count)
{
    LayoutMultiColumnFlowThread* flow_thread = To<LayoutBlockFlow>(box_.Get())->MultiColumnFlowThread();
