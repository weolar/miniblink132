...
public:
    ...
    std::optional<MinMaxSizesResult> Find(LayoutUnit initial_block_size)
    {
        ...
        for (auto it = cache_.rbegin(); it != cache_.rend(); ++it) {
            if (it->initial_block_size == initial_block_size) {
                ...
                auto copy = *it;
                cache_.EraseAt(static_cast<wtf_size_t>(std::distance(it, cache_.rend())) - 1u);
    ...
    void Add(const MinMaxSizes& sizes, LayoutUnit initial_block_size, bool depends_on_block_constraints)
    {
        ...
        cache_.push_back(Entry { sizes, initial_block_size, depends_on_block_constraints });
    }
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::MinMaxSizesCache::Entry)
