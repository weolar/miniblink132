...
public:
    ...
    bool HitTestNoLifecycleUpdate(const HitTestLocation& location, HitTestResult&);
    ...
    int ViewHeight(IncludeScrollbarsInRect scrollbar_inclusion = kExcludeScrollbars) const
    {
        NOT_DESTROYED();
        return GetLayoutSize(scrollbar_inclusion).height();
    }
    int ViewWidth(IncludeScrollbarsInRect scrollbar_inclusion = kExcludeScrollbars) const
    {
        NOT_DESTROYED();
    ...
    PhysicalRect OverflowClipRect(const PhysicalOffset& location, OverlayScrollbarClipBehavior = kIgnoreOverlayScrollbarSize) const override;

    // If either direction has a non-auto mode, the other must as well.
    void SetAutosizeScrollbarModes(mojom::blink::ScrollbarMode h_mode, mojom::blink::ScrollbarMode v_mode);
    ...
    void CalculateScrollbarModes(mojom::blink::ScrollbarMode& h_mode, mojom::blink::ScrollbarMode& v_mode) const;
    ...
    void UpdateHitTestResult(HitTestResult&, const PhysicalOffset&) const override;
    ...
    void MapLocalToAncestor(const LayoutBoxModelObject* ancestor, TransformState&, MapCoordinatesFlags) const override;
    ...
    void RegisterVariableLengthTransformResult(const LayoutText& text, const VariableLengthTransformResult& result);
    void UnregisterVariableLengthTransformResult(const LayoutText& text);
    VariableLengthTransformResult GetVariableLengthTransformResult(const LayoutText& text);
...
private:
    ...
    Member<HeapHashSet<Member<const LayoutObject>>> initial_containing_block_resize_handled_list_;
    ...
    HeapHashMap<WeakMember<const LayoutText>, VariableLengthTransformResult> text_to_variable_length_transform_result_;
...
template <> struct DowncastTraits<LayoutView> {
