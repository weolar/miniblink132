...
const LayoutObject* PositionAnchorObject(const LayoutBox& box)
{
    ...
    return style.PositionAnchor() ? box.FindTargetAnchor(*style.PositionAnchor()) : box.AcceptableImplicitAnchor();
}

const HeapVector<NonOverflowingScrollRange>* GetNonOverflowingScrollRanges(const LayoutObject* layout_object)
{
...
std::pair<bool, bool> CheckHasDefaultAnchorReferences(const LayoutObject* layout_object)
{
    if (!layout_object || !layout_object->IsOutOfFlowPositioned()) {
    ...
    return std::make_pair(box->NeedsAnchorPositionScrollAdjustmentInX(), box->NeedsAnchorPositionScrollAdjustmentInY());
}
...
gfx::Vector2dF AnchorPositionScrollData::TotalOffset(const LayoutObject& anchor_object) const
{
...
AnchorPositionScrollData::AdjustmentData AnchorPositionScrollData::ComputeAdjustmentContainersData(const LayoutObject& anchor) const
{
    CHECK(anchored_element_->GetLayoutObject());
    AnchorPositionScrollData::AdjustmentData result;

    auto container_ignore_layout_view_for_fixed_pos = [](const LayoutObject& o) -> const LayoutObject* {
    ...
    const auto* bounding_container = container_ignore_layout_view_for_fixed_pos(*anchored_element_->GetLayoutObject());

    if (bounding_container && bounding_container->IsScrollContainer()) {
        result.anchored_element_container_scroll_offset = To<LayoutBox>(bounding_container)->GetScrollableArea()->GetScrollOffset();
    }

    for (const auto* container = &anchor; container && container != bounding_container; container = container_ignore_layout_view_for_fixed_pos(*container)) {
        if (container->IsScrollContainer()) {
            const PaintLayerScrollableArea* scrollable_area = To<LayoutBox>(container)->GetScrollableArea();
            if (container != anchor && container != bounding_container &&
                ...
                result.adjustment_container_ids.push_back(scrollable_area->GetScrollElementId());
                result.accumulated_adjustment += scrollable_area->GetScrollOffset();
                result.accumulated_adjustment_scroll_origin += scrollable_area->ScrollOrigin().OffsetFromOrigin();
        ...
        if (const auto* box_model = DynamicTo<LayoutBoxModelObject>(container)) {
            if (box_model->StickyConstraints()) {
                result.adjustment_container_ids.push_back(
                    CompositorElementIdFromUniqueObjectId(box_model->UniqueId(), CompositorElementIdNamespace::kStickyTranslation));
                result.accumulated_adjustment -= gfx::Vector2dF(box_model->StickyPositionOffset());
            }
        }
        if (const auto* box = DynamicTo<LayoutBox>(container)) {
            if (auto* data = box->GetAnchorPositionScrollData()) {
                ...
                if (data->NeedsScrollAdjustment()) {
                    ...
                    result.adjustment_container_ids.push_back(
                        CompositorElementIdFromUniqueObjectId(box->UniqueId(), CompositorElementIdNamespace::kAnchorPositionScrollTranslation));
                    result.accumulated_adjustment += gfx::Vector2dF(data->ComputeDefaultAnchorAdjustmentData().accumulated_adjustment);
                }
            }
        }
    }
...
AnchorPositionScrollData::AdjustmentData AnchorPositionScrollData::ComputeDefaultAnchorAdjustmentData() const
{
    const LayoutObject* layout_object = anchored_element_->GetLayoutObject();
    auto [needs_scroll_adjustment_in_x, needs_scroll_adjustment_in_y] = CheckHasDefaultAnchorReferences(layout_object);
    if (!needs_scroll_adjustment_in_x && !needs_scroll_adjustment_in_y) {
        return AdjustmentData();
    }

    const LayoutObject* anchor_default_object = PositionAnchorObject(To<LayoutBox>(*layout_object));
...
AnchorPositionScrollData::SnapshotDiff AnchorPositionScrollData::TakeAndCompareSnapshot(bool update)
{
    DCHECK(IsActive());
    ...
    if (default_anchor_adjustment_data_.anchor_object != new_adjustment_data.anchor_object
        || AdjustmentContainerIds() != new_adjustment_data.adjustment_container_ids || !IsFallbackPositionValid(new_adjustment_data)) {
        diff = SnapshotDiff::kScrollersOrFallbackPosition;
    } else if (NeedsScrollAdjustmentInX() != new_adjustment_data.needs_scroll_adjustment_in_x
        || NeedsScrollAdjustmentInY() != new_adjustment_data.needs_scroll_adjustment_in_y
        || default_anchor_adjustment_data_.TotalOffset() != new_adjustment_data.TotalOffset()
        || AccumulatedAdjustmentScrollOrigin() != new_adjustment_data.accumulated_adjustment_scroll_origin) {
...
bool AnchorPositionScrollData::IsFallbackPositionValid(const AdjustmentData& new_adjustment_data) const
{
    const HeapVector<NonOverflowingScrollRange>* non_overflowing_scroll_ranges = GetNonOverflowingScrollRanges(anchored_element_->GetLayoutObject());
    if (!non_overflowing_scroll_ranges || non_overflowing_scroll_ranges->empty()) {
        return true;
    }

    for (const NonOverflowingScrollRange& range : *non_overflowing_scroll_ranges) {
        if (range.anchor_object != new_adjustment_data.anchor_object) {
            ...
            if (range.Contains(default_anchor_adjustment_data_.TotalOffset()) != range.Contains(new_adjustment_data.TotalOffset())) {
                return false;
            }
        }
    }
...
{
    return IsActive() && TakeAndCompareSnapshot(false /*update*/) != SnapshotDiff::kNone;
}

AnchorPositionVisibilityObserver& AnchorPositionScrollData::EnsureAnchorPositionVisibilityObserver()
{
    if (!position_visibility_observer_) {
        position_visibility_observer_ = MakeGarbageCollected<AnchorPositionVisibilityObserver>(*anchored_element_);
    }
...
void AnchorPositionScrollData::InvalidateLayoutAndPaint()
{
    ...
    anchored_element_->GetLayoutObject()->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kAnchorPositioning);
