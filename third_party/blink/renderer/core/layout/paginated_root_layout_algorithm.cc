...
PaginatedRootLayoutAlgorithm::PaginatedRootLayoutAlgorithm(const LayoutAlgorithmParams& params)
...
const LayoutResult* PaginatedRootLayoutAlgorithm::Layout()
{
    ...
    WritingModeConverter converter(GetConstraintSpace().GetWritingDirection(), container_builder_.Size());
    ...
    do {
        PageContainerResult result = LayoutPageContainer(page_index, total_page_count, page_name, counters_context.DeepClone(), page_area_params);
        ...
        if (page_name != result.fragment->PageName()) {
            ...
            result = LayoutPageContainer(page_index, total_page_count, page_name, counters_context, page_area_params);
        ...
        LogicalOffset origin = converter.ToLogical(PhysicalOffset(), result.fragment->Size());
    ...
    LayoutUnit block_size = ComputeBlockSizeForFragment(GetConstraintSpace(), Node(), /*border_padding=*/BoxStrut(),
    ...
    if (needs_total_page_count || oof_part.AdditionalPagesWereAdded()) {
        ...
        for (const LogicalFragmentLink& old_container : old_page_containers) {
            const auto& old_fragment = *To<PhysicalBoxFragment>(old_container.fragment.Get());

            // At least this time we know the page name up-front.
            const AtomicString& name = old_fragment.PageName();

            PageContainerResult result = LayoutPageContainer(page_index, total_page_count, name, counters_context, page_area_params, &old_fragment);
...
const PhysicalBoxFragment& PaginatedRootLayoutAlgorithm::CreateEmptyPage(const BlockNode& node, const ConstraintSpace& parent_space, wtf_size_t page_index,
    const PhysicalBoxFragment& previous_fragmentainer, bool* needs_total_page_count)
{
    const BlockBreakToken* break_token = previous_fragmentainer.GetBreakToken();
    PageAreaLayoutParams page_area_params = { .break_token = break_token, .template_fragmentainer = &previous_fragmentainer };
    ...
    PageContainerResult result = LayoutPageContainer(
        node, parent_space, page_index, /*total_page_count=*/0, previous_fragmentainer.PageName(), dummy_counters_context, page_area_params);
    *needs_total_page_count = result.needs_total_page_count;
    return *result.fragment;
}

PaginatedRootLayoutAlgorithm::PageContainerResult PaginatedRootLayoutAlgorithm::LayoutPageContainer(const BlockNode& root_node,
    const ConstraintSpace& parent_space, wtf_size_t page_index, wtf_size_t total_page_count, const AtomicString& page_name,
    const CountersAttachmentContext& counters_context, const PageAreaLayoutParams& page_area_params, const PhysicalBoxFragment* existing_page_container)
{
    Document& document = root_node.GetDocument();
    const ComputedStyle* page_container_style = document.GetStyleResolver().StyleForPage(page_index, page_name);

    LayoutBlockFlow* page_container = document.View()->GetPaginationState()->CreateAnonymousPageLayoutObject(document, *page_container_style);
    ...
    BoxStrut margins;
    LogicalSize page_containing_block_size = DesiredPageContainingBlockSize(document, *page_container_style);
    ResolvePageBoxGeometry(page_container_node, page_containing_block_size, &geometry, &margins);

    // Check if the resulting page area size is usable.
    LogicalSize desired_page_area_size = geometry.border_box_size - geometry.border - geometry.padding;
    bool ignore_author_page_style = false;
    if (desired_page_area_size.inline_size < LayoutUnit(1) || desired_page_area_size.block_size < LayoutUnit(1)) {
        ...
        page_container_style = document.GetStyleResolver().StyleForPage(page_index, page_name, 1.0, ignore_author_page_style);
        page_container->SetStyle(page_container_style, LayoutObject::ApplyStyleChanges::kNo);
        page_containing_block_size = DesiredPageContainingBlockSize(document, *page_container_style);
        ResolvePageBoxGeometry(page_container_node, page_containing_block_size, &geometry, &margins);
    }
    ...
    LogicalSize page_container_size = FittedPageContainerSize(document, page_container_node.Style(), margin_box_size);

    ConstraintSpaceBuilder space_builder(parent_space, page_container_style->GetWritingDirection(),
        /*is_new_fc=*/true);
    ...
    FragmentGeometry margin_box_geometry = { .border_box_size = page_container_size };

    LayoutAlgorithmParams params(page_container_node, margin_box_geometry, child_space, /*break_token=*/nullptr);
    PageContainerLayoutAlgorithm child_algorithm(
        params, page_index, total_page_count, page_name, root_node, counters_context, page_area_params, ignore_author_page_style, existing_page_container);
    ...
    return PageContainerResult(To<PhysicalBoxFragment>(result->GetPhysicalFragment()), child_algorithm.FragmentainerBreakToken(),
        child_algorithm.GetCountersContext(), child_algorithm.NeedsTotalPageCount());
}
