...
public:
    LayoutAlgorithmParams(BlockNode node, const FragmentGeometry& fragment_geometry, const ConstraintSpace& space, const BlockBreakToken* break_token = nullptr,
        const EarlyBreak* early_break = nullptr, const HeapVector<Member<EarlyBreak>>* additional_early_breaks = nullptr)
        : node(node)
...
template <typename InputNodeType, typename BoxFragmentBuilderType, typename BreakTokenType> class CORE_EXPORT LayoutAlgorithm {
    STACK_ALLOCATED();

public:
    LayoutAlgorithm(InputNodeType node, const ComputedStyle* style, const ConstraintSpace& space, TextDirection direction, const BreakTokenType* break_token)
        : node_(node)
        , container_builder_(node, style, space, { space.GetWritingMode(), direction }, break_token)
    {
    }
    ...
    explicit LayoutAlgorithm(const LayoutAlgorithmParams& params)
        ...
        , container_builder_(params.node, &params.node.Style(), params.space, { params.space.GetWritingMode(), params.space.Direction() }, params.break_token)
...
protected:
    ...
    BfcOffset ContainerBfcOffset() const
    {
        ...
        return { container_builder_.BfcLineOffset(), *container_builder_.BfcBlockOffset() };
    }

    const InputNodeType& Node() const
    {
        return node_;
    }
    ...
    {
        return container_builder_.Borders();
    }
    const BoxStrut& Scrollbar() const
    {
        return container_builder_.Scrollbar();
    }
    const BoxStrut& Padding() const
    {
        return container_builder_.Padding();
    }
    ...
    BreakStatus BreakBeforeChildIfNeeded(
        LayoutInputNode child, const LayoutResult& layout_result, LayoutUnit fragmentainer_block_offset, bool has_container_separation)
    {
        return ::blink::BreakBeforeChildIfNeeded(GetConstraintSpace(), child, layout_result, fragmentainer_block_offset, FragmentainerCapacityForChildren(),
            has_container_separation, &container_builder_);
    }

    bool MovePastBreakpoint(LayoutInputNode child, const LayoutResult& layout_result, LayoutUnit fragmentainer_block_offset, BreakAppeal appeal_before)
    {
    ...
    bool MovePastBreakpoint(const LayoutResult& layout_result, LayoutUnit fragmentainer_block_offset, BreakAppeal appeal_before)
    {
        return ::blink::MovePastBreakpoint(
            GetConstraintSpace(), layout_result, fragmentainer_block_offset, FragmentainerCapacityForChildren(), appeal_before, &container_builder_);
    }
    ...
    void SetupRelayoutData(const LayoutAlgorithm& previous_algorithm, RelayoutType relayout_type)
    {
        if (relayout_mode_ & kRelayoutForEarlyBreak) {
            ...
            container_builder_.PropagateSpaceShortage(previous_algorithm.container_builder_.MinimalSpaceShortage());
        }
        ...
        container_builder_.SetBoxType(previous_algorithm.container_builder_.GetBoxType());
    }

    // Re-run this layout algorithm for the specified type of relayout.
    template <typename Algorithm>
    const LayoutResult* Relayout(
        RelayoutType relayout_type, const EarlyBreak* breakpoint = nullptr, const HeapVector<Member<EarlyBreak>>* additional_early_breaks = nullptr) const
    {
        ...
        LayoutAlgorithmParams params(Node(), container_builder_.InitialFragmentGeometry(), new_space, GetBreakToken(), breakpoint, additional_early_breaks);

        Algorithm relayout_algorithm(params);
        relayout_algorithm.relayout_mode_ = new_relayout_mode;
        relayout_algorithm.SetupRelayoutData(*static_cast<const Algorithm*>(this), relayout_type);
    ...
    template <typename Algorithm>
    const LayoutResult* RelayoutAndBreakEarlier(const EarlyBreak& breakpoint, const HeapVector<Member<EarlyBreak>>* additional_early_breaks = nullptr)
    {
        // Not allowed to recurse!
        DCHECK(!early_break_);
        DCHECK(!additional_early_breaks_ || additional_early_breaks_->empty());

        return Relayout<Algorithm>(kRelayoutForEarlyBreak, &breakpoint, additional_early_breaks);
    }
    ...
    template <typename Algorithm> const LayoutResult* RelayoutWithoutFragmentation()
    {
