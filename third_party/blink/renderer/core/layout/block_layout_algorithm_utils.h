...
LayoutUnit CalculateOutOfFlowStaticInlineLevelOffset(
    const ComputedStyle& container_style, const BfcOffset& origin_bfc_offset, const ExclusionSpace&, LayoutUnit child_available_inline_size);
...
enum class BlockContentAlignment { kStart, kBaseline, kSafeCenter, kUnsafeCenter, kSafeEnd, kUnsafeEnd };
BlockContentAlignment ComputeContentAlignmentForBlock(const ComputedStyle& style, UseCounter* use_counter = nullptr);
BlockContentAlignment ComputeContentAlignmentForTableCell(const ComputedStyle& style, UseCounter* use_counter = nullptr);
