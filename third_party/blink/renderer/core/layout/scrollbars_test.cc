...
public:
    ...
    void Paint(cc::PaintCanvas*, Part part, State, const gfx::Rect&, const ExtraParams*, mojom::blink::ColorScheme color_scheme, bool in_forced_colors,
        const ui::ColorProvider* color_provider, const std::optional<SkColor>& accent_color) override
    {
    ...
    SkColor4f GetScrollbarThumbColor(State, const ExtraParams*, const ui::ColorProvider*) const override
    {
        return SkColors::kRed;
    }

private:
    std::array<mojom::blink::ColorScheme, kPartProgressBar + 1> painted_color_scheme_;
};
...
public:
    ...
    {
        old_theme_ = WebThemeEngineHelper::SwapNativeThemeEngineForTesting(std::make_unique<StubWebThemeEngine>());
    }

    ~ScopedStubThemeEngine()
    {
        WebThemeEngineHelper::SwapNativeThemeEngineForTesting(std::move(old_theme_));
    }
...
public:
    void SetUp() override
    {
        ...
        mock_overlay_scrollbars_ = std::make_unique<ScopedMockOverlayScrollbars>(false);
        original_overlay_scrollbars_enabled_ = ScrollbarThemeSettings::OverlayScrollbarsEnabled();
    }
    ...
    {
        return WebView().MainFrameViewWidget()->CoreHitTestResultAt(gfx::PointF(x, y));
    }
    ...
    {
        WebMouseEvent event(WebInputEvent::Type::kMouseMove, gfx::PointF(x, y), gfx::PointF(x, y), WebPointerProperties::Button::kNoButton, 0,
            WebInputEvent::kNoModifiers, base::TimeTicks::Now());
        event.SetFrameScale(1);
        GetEventHandler().HandleMouseMoveEvent(event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    }

    void HandleMousePressEvent(int x, int y, WebPointerProperties::Button button = WebPointerProperties::Button::kLeft)
    {
        WebMouseEvent event(WebInputEvent::Type::kMouseDown, gfx::PointF(x, y), gfx::PointF(x, y), button, 0, WebInputEvent::Modifiers::kLeftButtonDown,
            base::TimeTicks::Now());
        event.SetFrameScale(1);
    ...
    {
        WebMouseEvent event(WebInputEvent::Type::kMouseDown, gfx::PointF(x, y), gfx::PointF(x, y), WebPointerProperties::Button::kNoButton, 0,
            WebInputEvent::Modifiers::kNoModifiers, base::TimeTicks::Now());
        event.SetFrameScale(1);
        GetEventHandler().SendContextMenuEvent(event);
    }

    void HandleMouseReleaseEvent(int x, int y, WebPointerProperties::Button button = WebPointerProperties::Button::kLeft)
    {
        WebMouseEvent event(
            WebInputEvent::Type::kMouseUp, gfx::PointF(x, y), gfx::PointF(x, y), button, 0, WebInputEvent::Modifiers::kNoModifiers, base::TimeTicks::Now());
        event.SetFrameScale(1);
    ...
    {
        WebMouseEvent event(WebInputEvent::Type::kMouseDown, gfx::PointF(x, y), gfx::PointF(x, y), WebPointerProperties::Button::kMiddle, 0,
    ...
    {
        WebMouseEvent event(WebInputEvent::Type::kMouseUp, gfx::PointF(x, y), gfx::PointF(x, y), WebPointerProperties::Button::kMiddle, 0,
            WebInputEvent::Modifiers::kMiddleButtonDown, base::TimeTicks::Now());
        event.SetFrameScale(1);
        GetEventHandler().HandleMouseReleaseEvent(event);
    }
    ...
    {
        WebMouseEvent event(WebInputEvent::Type::kMouseLeave, gfx::PointF(1, 1), gfx::PointF(1, 1), WebPointerProperties::Button::kLeft, 0,
            WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
        event.SetFrameScale(1);
        GetEventHandler().HandleMouseLeaveEvent(event);
    }

    WebGestureEvent GenerateWheelGestureEvent(WebInputEvent::Type type, const gfx::Point& position, ScrollOffset offset = ScrollOffset())
    {
        return GenerateGestureEvent(type, WebGestureDevice::kTouchpad, position, offset);
    }

    WebCoalescedInputEvent GenerateTouchGestureEvent(WebInputEvent::Type type, const gfx::Point& position, ScrollOffset offset = ScrollOffset())
    {
        return WebCoalescedInputEvent(GenerateGestureEvent(type, WebGestureDevice::kTouchscreen, position, offset), ui::LatencyInfo());
    }

    ui::mojom::blink::CursorType CursorType()
    {
        return GetDocument().GetFrame()->GetChromeClient().LastSetCursorForTesting().type();
    }
...
protected:
    WebGestureEvent GenerateGestureEvent(WebInputEvent::Type type, WebGestureDevice device, const gfx::Point& position, ScrollOffset offset)
    {
        WebGestureEvent event(type, WebInputEvent::kNoModifiers, base::TimeTicks::Now(), device);
...
public:
    ...
    void TimeAdvance()
    {
        GetVirtualTimeController()->SetVirtualTimePolicy(VirtualTimeController::VirtualTimePolicy::kAdvance);
    }

    void StopVirtualTimeAndExitRunLoop(base::OnceClosure quit_closure)
    {
        GetVirtualTimeController()->SetVirtualTimePolicy(VirtualTimeController::VirtualTimePolicy::kPause);
    ...
    void RunTasksForPeriod(base::TimeDelta delay)
    {
        ...
        scheduler::GetSingleThreadTaskRunnerForTesting()->PostDelayedTask(
            FROM_HERE, WTF::BindOnce(&ScrollbarsTestWithVirtualTimer::StopVirtualTimeAndExitRunLoop, WTF::Unretained(this), loop.QuitClosure()), delay);
        loop.Run();
    }
...
TEST_P(ScrollbarsTest, DocumentStyleRecalcPreservesScrollbars)
{
    ...
    ASSERT_TRUE(layout_viewport->VerticalScrollbar() && layout_viewport->HorizontalScrollbar());

    // Forces recalc of LayoutView's computed style in Document::updateStyle,
    // without invalidating layout.
    MainFrame().ExecuteScriptAndReturnValue(WebScriptSource("document.querySelector('style').sheet.insertRule('body {}', 1);"));
...
TEST(ScrollbarsTestWithOwnWebViewHelper, ScrollbarSizeF)
{
    ...
    frame_test_helpers::LoadHTMLString(web_view_impl->MainFrameImpl(),
        ...
        "</body>",
        base_url);
    web_view_impl->MainFrameViewWidget()->UpdateAllLifecyclePhases(DocumentUpdateReason::kTest);
    ...
    int horizontal_scrollbar = visual_viewport.LayerForHorizontalScrollbar()->bounds().height();
    int vertical_scrollbar = visual_viewport.LayerForVerticalScrollbar()->bounds().width();

    const float device_scale = 3.5f;
    web_view_impl->MainFrameViewWidget()->SetDeviceScaleFactorForTesting(device_scale);
    web_view_impl->MainFrameViewWidget()->Resize(gfx::Size(400, 300));

    EXPECT_EQ(ClampTo<int>(std::floor(horizontal_scrollbar * device_scale)), visual_viewport.LayerForHorizontalScrollbar()->bounds().height());
    EXPECT_EQ(ClampTo<int>(std::floor(vertical_scrollbar * device_scale)), visual_viewport.LayerForVerticalScrollbar()->bounds().width());

    web_view_impl->MainFrameViewWidget()->SetDeviceScaleFactorForTesting(1.f);
    web_view_impl->MainFrameViewWidget()->Resize(gfx::Size(800, 600));

    EXPECT_EQ(horizontal_scrollbar, visual_viewport.LayerForHorizontalScrollbar()->bounds().height());
    EXPECT_EQ(vertical_scrollbar, visual_viewport.LayerForVerticalScrollbar()->bounds().width());
}
...
TEST_P(ScrollbarsTest, CustomScrollbarsCauseLayoutOnExistenceChange)
{
    ...
    MainFrame().ExecuteScript(WebScriptSource("document.getElementById('box').className = 'box transformed';"));
    Compositor().BeginFrame();
...
TEST_P(ScrollbarsTest, TransparentBackgroundUsesLightOverlayColorScheme)
{
    ...
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, layout_viewport->GetOverlayScrollbarColorScheme());
}
...
TEST_P(ScrollbarsTest, BodyBackgroundChangesOverlayColorTheme)
{
    ...
    MainFrame().ExecuteScriptAndReturnValue(WebScriptSource("document.body.style.backgroundColor = 'black';"));

    Compositor().BeginFrame();
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, layout_viewport->GetOverlayScrollbarColorScheme());
}
...
TEST_P(ScrollbarsTest, OverlayScrollbarChangeToDisplayNoneDynamically)
{
    ...
    document.body()->setAttribute(html_names::kClassAttr, AtomicString("noscrollbars"));
    Compositor().BeginFrame();
...
TEST_P(ScrollbarsTest, HidingScrollbarsOnScrollableAreaDisablesScrollbars)
{
    ...
    EXPECT_FALSE(frame_scroller_area->HorizontalScrollbar()->ShouldParticipateInHitTesting());
    EXPECT_FALSE(frame_scroller_area->VerticalScrollbar()->ShouldParticipateInHitTesting());

    EXPECT_TRUE(scroller_area->ScrollbarsHiddenIfOverlay());
    EXPECT_FALSE(scroller_area->HorizontalScrollbar()->ShouldParticipateInHitTesting());
    EXPECT_FALSE(scroller_area->VerticalScrollbar()->ShouldParticipateInHitTesting());

    frame_scroller_area->SetScrollbarsHiddenForTesting(false);
    EXPECT_TRUE(frame_scroller_area->HorizontalScrollbar()->ShouldParticipateInHitTesting());
    EXPECT_TRUE(frame_scroller_area->VerticalScrollbar()->ShouldParticipateInHitTesting());
    frame_scroller_area->SetScrollbarsHiddenForTesting(true);
    EXPECT_FALSE(frame_scroller_area->HorizontalScrollbar()->ShouldParticipateInHitTesting());
    EXPECT_FALSE(frame_scroller_area->VerticalScrollbar()->ShouldParticipateInHitTesting());

    scroller_area->SetScrollbarsHiddenForTesting(false);
    EXPECT_TRUE(scroller_area->HorizontalScrollbar()->ShouldParticipateInHitTesting());
    EXPECT_TRUE(scroller_area->VerticalScrollbar()->ShouldParticipateInHitTesting());
    scroller_area->SetScrollbarsHiddenForTesting(true);
    EXPECT_FALSE(scroller_area->HorizontalScrollbar()->ShouldParticipateInHitTesting());
    EXPECT_FALSE(scroller_area->VerticalScrollbar()->ShouldParticipateInHitTesting());
}
...
TEST_P(ScrollbarsTest, MouseOverCustomScrollbarTrackPieceBothTrackAndTrackPieceWithoutStyle)
{
    // Skip this test if scrollbars don't allow hit testing on the platform.
    if (!WebView().GetPage()->GetScrollbarTheme().AllowsHitTest()) {
        return;
...
TEST_P(ScrollbarsTest, MouseOverCustomScrollbarCornerFrame)
{
    ...
    Document& iframe_document = *To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("iframe")))->contentDocument();
...
TEST_P(ScrollbarsTest, MouseOverLinkAndOverlayScrollbar)
{
    ...
    WebView().MainFrameImpl()->GetFrameView()->LayoutViewport()->SetScrollbarsHiddenForTesting(false);
    ...
    // active hover element.
    WebView().MainFrameImpl()->GetFrameView()->LayoutViewport()->SetScrollbarsHiddenForTesting(true);
...
TEST_P(ScrollbarsTest, MouseOverCustomScrollbar)
{
    ...
    EXPECT_EQ(document.HoverElement(), scrollbar_div);
    EXPECT_EQ(hit_test_result.GetScrollbar()->HoveredPart(), ScrollbarPart::kThumbPart);
}
...
TEST_P(ScrollbarsTest, CustomScrollbarInOverlayScrollbarThemeWillNotCauseDCHECKFails)
{
    WebView().MainFrameViewWidget()->Resize(gfx::Size(200, 200));
...
TEST_P(ScrollbarsTestWithVirtualTimer, DISABLED_TestNonCompositedOverlayScrollbarsFade)
{
#else
TEST_P(ScrollbarsTestWithVirtualTimer, TestNonCompositedOverlayScrollbarsFade)
{
    ...
    ScrollbarThemeOverlayMock& mock_overlay_theme = static_cast<ScrollbarThemeOverlayMock&>(theme);
    ...
    scrollable_area->SetScrollOffset(ScrollOffset(10, 10), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kInstant);

    EXPECT_FALSE(scrollable_area->ScrollbarsHiddenIfOverlay());
    RunTasksForPeriod(kMockOverlayFadeOutDelay);
    EXPECT_TRUE(scrollable_area->ScrollbarsHiddenIfOverlay());

    MainFrame().ExecuteScript(WebScriptSource("document.getElementById('space').style.height = '500px';"));
    Compositor().BeginFrame();

    EXPECT_TRUE(scrollable_area->ScrollbarsHiddenIfOverlay());

    MainFrame().ExecuteScript(WebScriptSource("document.getElementById('container').style.height = '300px';"));
    Compositor().BeginFrame();
    ...
    EXPECT_TRUE(scrollable_area->VerticalScrollbar());
    scrollable_area->SetScrollOffset(ScrollOffset(20, 20), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kInstant);
    EXPECT_FALSE(scrollable_area->ScrollbarsHiddenIfOverlay());
...
INSTANTIATE_TEST_SUITE_P(All, ScrollbarAppearanceTest, ::testing::Values(0, kUseOverlayScrollbars));

// Make sure native scrollbar can change by Emulator.
// Disable on Android since Android always enable OverlayScrollbar.
#if BUILDFLAG(IS_ANDROID)
TEST_P(ScrollbarAppearanceTest, DISABLED_NativeScrollbarChangeToMobileByEmulator)
{
#else
TEST_P(ScrollbarAppearanceTest, NativeScrollbarChangeToMobileByEmulator)
{
    ...
    DCHECK(root_scrollable->VerticalScrollbar());
    DCHECK(!root_scrollable->VerticalScrollbar()->IsCustomScrollbar());
    DCHECK_EQ(UsesOverlayScrollbars(), root_scrollable->VerticalScrollbar()->IsOverlayScrollbar());
    DCHECK(!root_scrollable->VerticalScrollbar()->GetTheme().IsMockTheme());
    ...
    DCHECK(div_scrollable->VerticalScrollbar());
    DCHECK(!div_scrollable->VerticalScrollbar()->IsCustomScrollbar());
    DCHECK_EQ(UsesOverlayScrollbars(), div_scrollable->VerticalScrollbar()->IsOverlayScrollbar());
    DCHECK(!div_scrollable->VerticalScrollbar()->GetTheme().IsMockTheme());
...
TEST_P(ScrollbarAppearanceTest, ThemeEngineDefinesMinimumThumbLength)
{
    ...
    EXPECT_EQ(StubWebThemeEngine::kMinimumHorizontalLength, theme.ThumbLength(*scrollable_area->HorizontalScrollbar()));
    EXPECT_EQ(StubWebThemeEngine::kMinimumVerticalLength, theme.ThumbLength(*scrollable_area->VerticalScrollbar()));
}
...
TEST_P(ScrollbarAppearanceTest, HugeScrollingThumbPosition)
{
    ...
    scrollable_area->SetScrollOffset(ScrollOffset(0, 10000000), mojom::blink::ScrollType::kProgrammatic);
    ...
    int max_thumb_position = WebView().MainFrameViewWidget()->Size().height() - StubWebThemeEngine::kMinimumVerticalLength;
    max_thumb_position -= scrollbar->GetTheme().ScrollbarMargin(scrollbar->ScaleFromDIP(), EScrollbarWidth::kAuto) * 2;

    EXPECT_EQ(max_thumb_position, scrollbar->GetTheme().ThumbPosition(*scrollbar));
}
#endif
...
TEST_P(ScrollbarsTest, AutosizeExpandingContentScrollable)
{
    ...
    GetDocument().getElementById(AtomicString("spacer"))->setAttribute(html_names::kStyleAttr, AtomicString("height: 900px"));
    Compositor().BeginFrame();
...
TEST_P(ScrollbarsTest, HideTheOverlayScrollbarNotCrashAfterPLSADisposedPaintLayer)
{
    // This test is specifically checking the behavior when overlay scrollbars
    // are enabled.
    ENABLE_OVERLAY_SCROLLBARS(true);
...
TEST_P(ScrollbarsTest, OverlayScrollbarHitTest)
{
    ...
    iframe_element->contentDocument()->View()->LayoutViewport()->SetScrollbarsHiddenForTesting(false);
...
TEST_P(ScrollbarsTest, RecorderedOverlayScrollbarHitTest)
{
    ...
    auto* target = GetDocument().getElementById(AtomicString("target"))->GetLayoutBox();
    ...
    EXPECT_EQ(GetDocument().getElementById(AtomicString("stacked")), result.InnerNode());
}

TEST_P(ScrollbarsTest, AllowMiddleButtonPressOnScrollbarWhenDisableMiddleClickAutoScroll)
{
    ...
    ScrollableArea* scrollable_area = WebView().MainFrameImpl()->GetFrameView()->LayoutViewport();
...
TEST_P(ScrollbarsTest, NotAllowMiddleButtonPressOnScrollbarWhenEnableMiddleClickAutoScroll)
{
...
TEST_P(ScrollbarsTest, MiddleDownShouldNotAffectScrollbarPress)
{
    ...
    WebMouseEvent event(WebInputEvent::Type::kMouseMove, gfx::PointF(5, 5), gfx::PointF(5, 5), WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
    event.SetFrameScale(1);
    GetEventHandler().HandleMouseMoveEvent(event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    EXPECT_EQ(scrollbar->PressedPart(), ScrollbarPart::kThumbPart);
...
TEST_P(ScrollbarsTest, UseCounterNegativeWhenThumbIsNotScrolledWithMouse)
{
    ...
    widget.DispatchThroughCcInputHandler(GenerateWheelGestureEvent(WebInputEvent::Type::kGestureScrollBegin, gfx::Point(100, 100), ScrollOffset(0, -100)));
    widget.DispatchThroughCcInputHandler(GenerateWheelGestureEvent(WebInputEvent::Type::kGestureScrollUpdate, gfx::Point(100, 100), ScrollOffset(0, -100)));
    widget.DispatchThroughCcInputHandler(GenerateWheelGestureEvent(WebInputEvent::Type::kGestureScrollEnd, gfx::Point(100, 100)));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kVerticalScrollbarThumbScrollingWithMouse));
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kHorizontalScrollbarThumbScrollingWithMouse));

    // Clicking on the vertical scrollbar won't trigger the UseCounter.
    HandleMousePressEvent(195, 175);
    EXPECT_EQ(vertical_scrollbar->PressedPart(), ScrollbarPart::kForwardTrackPart);
    ...
    EXPECT_EQ(horizontal_scrollbar->PressedPart(), ScrollbarPart::kForwardTrackPart);
...
TEST_P(ScrollbarsTest, UseCounterPositiveWhenThumbIsScrolledWithMouse)
{
    ...
    HandleMouseReleaseEvent(195, 5);
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kVerticalScrollbarThumbScrollingWithMouse));
    ...
    HandleMouseReleaseEvent(5, 195);
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kHorizontalScrollbarThumbScrollingWithMouse));
}
...
TEST_P(ScrollbarsTest, UseCounterNegativeWhenThumbIsNotScrolledWithTouch)
{
    ...
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapDown, gfx::Point(195, 175)));
    EXPECT_EQ(vertical_scrollbar->PressedPart(), ScrollbarPart::kForwardTrackPart);
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapCancel, gfx::Point(195, 175)));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kVerticalScrollbarThumbScrollingWithTouch));

    // Tapping on the horizontal scrollbar won't trigger the UseCounter.
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapDown, gfx::Point(175, 195)));
    EXPECT_EQ(horizontal_scrollbar->PressedPart(), ScrollbarPart::kForwardTrackPart);
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapCancel, gfx::Point(175, 195)));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kHorizontalScrollbarThumbScrollingWithTouch));

    // Tapping outside the scrollbar and then releasing over the thumb of the
    // vertical scrollbar won't trigger the UseCounter.
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapDown, gfx::Point(50, 50)));
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapCancel, gfx::Point(195, 5)));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kVerticalScrollbarThumbScrollingWithTouch));

    // Tapping outside the scrollbar and then releasing over the thumb of the
    // horizontal scrollbar won't trigger the UseCounter.
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapDown, gfx::Point(50, 50)));
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapCancel, gfx::Point(5, 195)));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kHorizontalScrollbarThumbScrollingWithTouch));
}
...
TEST_P(ScrollbarsTest, UseCounterPositiveWhenThumbIsScrolledWithTouch)
{
    ...
    // Clicking the thumb on the vertical scrollbar will trigger the UseCounter.
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapDown, gfx::Point(195, 5)));
    EXPECT_EQ(vertical_scrollbar->PressedPart(), ScrollbarPart::kThumbPart);
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapCancel, gfx::Point(195, 5)));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kVerticalScrollbarThumbScrollingWithTouch));

    // Clicking the thumb on the horizontal scrollbar will trigger the UseCounter.
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapDown, gfx::Point(5, 195)));
    EXPECT_EQ(horizontal_scrollbar->PressedPart(), ScrollbarPart::kThumbPart);
    WebView().MainFrameViewWidget()->HandleInputEvent(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureTapCancel, gfx::Point(5, 195)));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kHorizontalScrollbarThumbScrollingWithTouch));
}
...
TEST_P(ScrollbarsTest, UseCounterCustomScrollbarPercentSize)
{
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCustomScrollbarPercentThickness));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCustomScrollbarPartPercentLength));
    ...
    child->setAttribute(html_names::kStyleAttr, AtomicString("width: 50px; height: 200px"));
    Compositor().BeginFrame();
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCustomScrollbarPercentThickness));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCustomScrollbarPartPercentLength));
    ...
    child->setAttribute(html_names::kStyleAttr, AtomicString("width: 200px; height: 50px"));
    Compositor().BeginFrame();
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCustomScrollbarPercentThickness));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCustomScrollbarPartPercentLength));
}
...
TEST_P(ScrollbarsTest, NoNeedsBeginFrameForCustomScrollbarAfterBeginFrame)
{
    ...
    auto* target = GetDocument().getElementById(AtomicString("target"));
    auto* scrollbar = To<CustomScrollbar>(target->GetLayoutBox()->GetScrollableArea()->HorizontalScrollbar());
...
TEST_P(ScrollbarsTest, CustomScrollbarHypotheticalThickness)
{
    ...
    EXPECT_EQ(33, CustomScrollbar::HypotheticalScrollbarThickness(scrollable_area1, kHorizontalScrollbar, target1->GetLayoutBox()));
    EXPECT_EQ(22, CustomScrollbar::HypotheticalScrollbarThickness(scrollable_area1, kVerticalScrollbar, target1->GetLayoutBox()));

    auto* target2 = GetDocument().getElementById(AtomicString("target2"));
    auto* scrollable_area2 = target2->GetLayoutBox()->GetScrollableArea();
    EXPECT_EQ(13, CustomScrollbar::HypotheticalScrollbarThickness(scrollable_area2, kHorizontalScrollbar, target2->GetLayoutBox()));
    EXPECT_EQ(21, CustomScrollbar::HypotheticalScrollbarThickness(scrollable_area2, kVerticalScrollbar, target2->GetLayoutBox()));
}
...
// http://crbug.com/633321
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_IOS)
TEST_P(ScrollbarsTestWithVirtualTimer, DISABLED_PressScrollbarButtonOnInfiniteScrolling)
{
#else
TEST_P(ScrollbarsTestWithVirtualTimer, PressScrollbarButtonOnInfiniteScrolling)
{
#endif
    TimeAdvance();
    ...
    // Scroll to bottom.
    scrollable_area->SetScrollOffset(ScrollOffset(0, 400), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kInstant);
    ...
    RunTasksForPeriod(base::Milliseconds(1000));
    // Change #big size.
    MainFrame().ExecuteScript(WebScriptSource("document.getElementById('big').style.height = '1000px';"));
    Compositor().BeginFrame();
...
public:
    void PrepareTest(const String& track_style)
    {
      ...
      <div id='d1'/>
        ...
        CustomScrollbar* horizontal_scrollbar = To<CustomScrollbar>(div_scrollable->HorizontalScrollbar());
        ...
        CustomScrollbar* vertical_scrollbar = To<CustomScrollbar>(div_scrollable->VerticalScrollbar());
...
TEST_P(ScrollbarTrackMarginsTest, CustomScrollbarFractionalMarginsWillNotCauseDCHECKFailure)
{
    PrepareTest(R"CSS(
    ::-webkit-scrollbar-track {
      margin-left: 10.2px;
...
TEST_P(ScrollbarTrackMarginsTest, CustomScrollbarScaledMarginsWillNotCauseDCHECKFailure)
{
...
INSTANTIATE_TEST_SUITE_P(NonOverlay, ScrollbarColorSchemeTest, testing::Values(false));
...
TEST_P(ScrollbarColorSchemeTest, ThemeEnginePaint)
{
    ...
    auto* theme_engine = static_cast<StubWebThemeEngine*>(WebThemeEngineHelper::GetNativeThemeEngine());
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, theme_engine->GetPaintedPartColorScheme(WebThemeEngine::kPartScrollbarHorizontalThumb));
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, theme_engine->GetPaintedPartColorScheme(WebThemeEngine::kPartScrollbarVerticalThumb));
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, theme_engine->GetPaintedPartColorScheme(WebThemeEngine::kPartScrollbarCorner));
}
...
TEST_P(ScrollbarsTest, ScrollbarGutterWithHorizontalTextAndClassicScrollbars)
{
    ...
    auto* stable_both_edges = GetDocument().getElementById(AtomicString("stable_both_edges"));
    ...
    PhysicalBoxStrut box_stable_both_edges_scrollbars = box_stable_both_edges->ComputeScrollbars();
...
TEST_P(ScrollbarsTest, ScrollbarGutterBothEdgesKeywordWithClassicScrollbars)
{
    ...
    // Scroll down.
    auto& widget = GetWebFrameWidget();
    widget.DispatchThroughCcInputHandler(GenerateWheelGestureEvent(WebInputEvent::Type::kGestureScrollBegin, gfx::Point(5, 5), ScrollOffset(0, -100)));
    widget.DispatchThroughCcInputHandler(GenerateWheelGestureEvent(WebInputEvent::Type::kGestureScrollUpdate, gfx::Point(5, 5), ScrollOffset(0, -100)));
    widget.DispatchThroughCcInputHandler(GenerateWheelGestureEvent(WebInputEvent::Type::kGestureScrollEnd, gfx::Point(5, 5)));
    ...
    widget.DispatchThroughCcInputHandler(GenerateWheelGestureEvent(WebInputEvent::Type::kGestureScrollBegin, gfx::Point(5, 5), ScrollOffset(0, 100)));
    widget.DispatchThroughCcInputHandler(GenerateWheelGestureEvent(WebInputEvent::Type::kGestureScrollUpdate, gfx::Point(5, 5), ScrollOffset(0, 100)));
    widget.DispatchThroughCcInputHandler(GenerateWheelGestureEvent(WebInputEvent::Type::kGestureScrollEnd, gfx::Point(195, 5)));
...
TEST_P(ScrollbarsTest, ScrollbarsRestoredAfterCapturePaintPreview)
{
    ...
    HTMLElement* content_div = To<HTMLElement>(document.getElementById(AtomicString("content")));
    ...
    ASSERT_FALSE(layout_viewport->VerticalScrollbar() || layout_viewport->HorizontalScrollbar());
...
TEST_P(ScrollbarsTest, ScrollbarsUsedColorSchemeFollowsOverlayTheme)
{
    ...
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, layout_viewport->GetOverlayScrollbarColorScheme());
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, layout_viewport->VerticalScrollbar()->UsedColorScheme());
    ...
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, layout_viewport->GetOverlayScrollbarColorScheme());
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, layout_viewport->VerticalScrollbar()->UsedColorScheme());
}
