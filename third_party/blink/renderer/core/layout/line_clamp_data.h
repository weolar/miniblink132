...
struct LineClampData {
    ...
    {
        if (state == kClampByLines || (show_measured_lines && state == kMeasureLinesUntilBfcOffset)) {
    ...
    bool operator==(const LineClampData& other) const
    {
        ...
        case kMeasureLinesUntilBfcOffset:
            return lines_until_clamp == other.lines_until_clamp && clamp_bfc_offset == other.clamp_bfc_offset;
        default:
            return true;
        }
    }
