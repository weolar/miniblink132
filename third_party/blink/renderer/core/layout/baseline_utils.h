...
inline WritingMode DetermineBaselineWritingMode(
    const WritingDirectionMode container_writing_direction, const WritingMode child_writing_mode, bool is_parallel_context)
{
    ...
    const auto orthogonal_writing_mode = (is_parallel_context)
        ? container_writing_direction.GetWritingMode()
        : ((child_writing_mode == WritingMode::kHorizontalTb) ? (container_writing_direction.IsLtr() ? WritingMode::kVerticalLr : WritingMode::kVerticalRl)
                                                              : WritingMode::kHorizontalTb);
    const bool is_parallel = IsParallelWritingMode(container_writing_direction.GetWritingMode(), child_writing_mode);
...
inline BaselineGroup DetermineBaselineGroup(const WritingDirectionMode container_writing_direction, const WritingMode baseline_writing_mode,
    bool is_parallel_context, bool is_last_baseline, bool is_flipped = false)
{
    const auto container_writing_mode = container_writing_direction.GetWritingMode();
    ...
    if (is_parallel_context) {
        DCHECK(IsParallelWritingMode(container_writing_mode, baseline_writing_mode));
        return (baseline_writing_mode == container_writing_mode) ? start_group : end_group;
    }
