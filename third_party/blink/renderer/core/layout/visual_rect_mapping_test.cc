...
inline PaintLayerScrollableArea* GetScrollableArea(const LayoutBlock* container)
{
    return To<Element>(container->GetNode())->GetLayoutBoxForScrolling()->GetScrollableArea();
}

} // namespace

class VisualRectMappingTest : public PaintTestConfigurations, public RenderingTest {
public:
...
protected:
    ...
    void CheckPaintInvalidationVisualRect(
        const LayoutObject& object, const LayoutBoxModelObject& ancestor, const PhysicalRect& expected_visual_rect_in_ancestor)
    {
        CheckVisualRect(object, ancestor, LocalVisualRect(object), expected_visual_rect_in_ancestor);
    }

    void CheckVisualRect(const LayoutObject& object, const LayoutBoxModelObject& ancestor, const PhysicalRect& local_rect,
        const PhysicalRect& expected_visual_rect_in_ancestor, unsigned flags = 0)
    {
        ...
        if (fragment_data.HasLocalBorderBoxProperties()) {
            ...
            object.MapToVisualRectInAncestorSpace(&ancestor, local_rect_copy, kUseGeometryMapper);
        ...
        if (flags & kContainsToEnclosingRect) {
            EXPECT_TRUE(ToEnclosingRect(slow_map_rect).Contains(ToEnclosingRect(expected_visual_rect_in_ancestor)));

            if (object.FirstFragment().HasLocalBorderBoxProperties()) {
                EXPECT_TRUE(gfx::ToEnclosingRect(geometry_mapper_rect.Rect()).Contains(ToEnclosingRect(expected_visual_rect_in_ancestor)));
            }
        } else {
            EXPECT_EQ(expected_visual_rect_in_ancestor, slow_map_rect);
            if (object.FirstFragment().HasLocalBorderBoxProperties()) {
                EXPECT_EQ(expected_visual_rect_in_ancestor, PhysicalRect::EnclosingRect(geometry_mapper_rect.Rect()));
            }
        }
    }
    ...
    // geometry mapper.
    void CheckMapToVisualRectInAncestorSpace(const PhysicalRect& rect, const PhysicalRect& expected, const LayoutObject* object,
        const LayoutBoxModelObject* ancestor, VisualRectFlags flags, bool expected_retval)
    {
        PhysicalRect result = rect;
        EXPECT_EQ(expected_retval, object->MapToVisualRectInAncestorSpace(ancestor, result, flags));
        EXPECT_EQ(result, expected);
        result = rect;
        EXPECT_EQ(expected_retval, object->MapToVisualRectInAncestorSpace(ancestor, result, static_cast<VisualRectFlags>(flags | kUseGeometryMapper)));
        EXPECT_EQ(result, expected);
    }
};
...
TEST_P(VisualRectMappingTest, LayoutText)
{
    ...
    EXPECT_TRUE(text->MapToVisualRectInAncestorSpace(container, rect, kEdgeInclusive));
    rect.Move(-PhysicalOffset(container->ScrolledContentOffset()));
...
TEST_P(VisualRectMappingTest, LayoutInline)
{
    ...
    rect = PhysicalRect(0, 60, 80, 0);
    EXPECT_TRUE(leaf->MapToVisualRectInAncestorSpace(container, rect, kEdgeInclusive));
    rect.Move(-PhysicalOffset(container->ScrolledContentOffset()));
    EXPECT_EQ(rect, PhysicalRect(0, 10, 80, 0));
}
...
TEST_P(VisualRectMappingTest, LayoutView)
{
    ...
    auto* frame_container = To<LayoutBlock>(GetLayoutObjectByElementId("frameContainer"));
    ...
    ChildDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 47), mojom::blink::ScrollType::kProgrammatic);
    UpdateAllLifecyclePhasesForTest();
    ...
    rect = original_rect;
    EXPECT_TRUE(frame_text->MapToVisualRectInAncestorSpace(&GetLayoutView(), rect));
    EXPECT_EQ(rect, PhysicalRect(4, 13, 20, 37));

    CheckPaintInvalidationVisualRect(*frame_text, GetLayoutView(), PhysicalRect());

    rect = PhysicalRect(4, 60, 0, 80);
    EXPECT_TRUE(frame_text->MapToVisualRectInAncestorSpace(frame_container, rect, kEdgeInclusive));
...
TEST_P(VisualRectMappingTest, LayoutViewSubpixelRounding)
{
    ...
    auto* frame_container = To<LayoutBlock>(GetLayoutObjectByElementId("frameContainer"));
    LayoutObject* target = ChildDocument().getElementById(AtomicString("target"))->GetLayoutObject();
    ...
    // frameContainer.
    EXPECT_EQ(PhysicalRect(LayoutUnit(0.5), LayoutUnit(), LayoutUnit(101), LayoutUnit(100)), rect);
}
...
TEST_P(VisualRectMappingTest, SelfFlippedWritingMode)
{
    ...
    CheckPaintInvalidationVisualRect(*target, GetLayoutView(), PhysicalRect(222, 111, 140, 70));
}
...
TEST_P(VisualRectMappingTest, ContainerFlippedWritingMode)
{
    ...
    CheckPaintInvalidationVisualRect(*target, GetLayoutView(), PhysicalRect(322, 111, 140, 110));
    ...
    rect = container_local_visual_rect;
    EXPECT_TRUE(container->MapToVisualRectInAncestorSpace(&GetLayoutView(), rect));
...
TEST_P(VisualRectMappingTest, ContainerOverflowScroll)
{
    ...
    // (222, 111).
    CheckPaintInvalidationVisualRect(*target, GetLayoutView(), PhysicalRect(232, 121, 50, 80));
    ...
    CheckPaintInvalidationVisualRect(*container, GetLayoutView(), PhysicalRect(222, 111, 70, 100));
}
...
TEST_P(VisualRectMappingTest, DifferentPaintInvalidaitionContainerForAbsolutePosition)
{
    ...
    auto* normal_flow = To<LayoutBlock>(GetLayoutObjectByElementId("normal-flow"));
    ...
    auto* stacking_context = To<LayoutBlock>(GetLayoutObjectByElementId("stacking-context"));
    ...
    CheckPaintInvalidationVisualRect(*absolute, *stacking_context, PhysicalRect(222, 111, 50, 50));
}

TEST_P(VisualRectMappingTest, ContainerOfAbsoluteAbovePaintInvalidationContainer)
{
    SetPreferCompositingToLCDText(true);

    SetBodyInnerHTML("<div id='container' style='position: absolute; top: 88px; left: 99px'>"
    ...
    CheckPaintInvalidationVisualRect(*absolute, GetLayoutView(), PhysicalRect(149, 138, 50, 50));
}
...
TEST_P(VisualRectMappingTest, CSSClip)
{
    ...
    CheckPaintInvalidationVisualRect(*target, GetLayoutView(), PhysicalRect(0, 0, 200, 200));
}
...
TEST_P(VisualRectMappingTest, ShouldAccountForPreserve3d)
{
    ...
    * { margin: 0; }
    ...
    PhysicalRect output = PhysicalRect::EnclosingRect(matrix.MapRect(gfx::RectF(original_rect)));

    CheckVisualRect(*target, *target->View(), original_rect, output, kContainsToEnclosingRect);
}
...
TEST_P(VisualRectMappingTest, PerspectivePlusScroll)
{
    ...
    * { margin: 0; }
    ...
    target->GetTransformFromContainer(container, target->OffsetFromContainer(container), transform);
    transform.Flatten();

    PhysicalRect output = PhysicalRect::EnclosingRect(transform.MapRect(gfx::RectF(originalRect)));
...
TEST_P(VisualRectMappingTest, FixedContentsInIframe)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    auto* fixed = ChildDocument().getElementById(AtomicString("fixed"))->GetLayoutObject();
    auto* root_view = fixed->View();
    while (root_view->GetFrame()->OwnerLayoutObject())
        root_view = root_view->GetFrame()->OwnerLayoutObject()->View();

    CheckMapToVisualRectInAncestorSpace(PhysicalRect(0, 0, 400, 300), PhysicalRect(0, 0, 400, 300), fixed, root_view, kDefaultVisualRectFlags, true);

    ChildDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 50), mojom::blink::ScrollType::kProgrammatic);
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(VisualRectMappingTest, FixedContentsWithScrollOffset)
{
    ...
    CheckMapToVisualRectInAncestorSpace(PhysicalRect(0, 0, 400, 300), PhysicalRect(0, -10, 400, 300), fixed, ancestor, kDefaultVisualRectFlags, true);

    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 50), mojom::blink::ScrollType::kProgrammatic);
    UpdateAllLifecyclePhasesForTest();

    // The fixed element does not scroll but the ancestor does which changes the
    // visual rect.
    CheckMapToVisualRectInAncestorSpace(PhysicalRect(0, 0, 400, 300), PhysicalRect(0, 40, 400, 300), fixed, ancestor, kDefaultVisualRectFlags, true);
}
...
TEST_P(VisualRectMappingTest, FixedContentsUnderViewWithScrollOffset)
{
    ...
    CheckMapToVisualRectInAncestorSpace(PhysicalRect(0, 0, 400, 300), PhysicalRect(0, 0, 400, 300), fixed, fixed->View(), kDefaultVisualRectFlags, true);
    ...
    CheckMapToVisualRectInAncestorSpace(PhysicalRect(0, 0, 400, 300), PhysicalRect(0, 50, 400, 300), fixed, fixed->View(), kDefaultVisualRectFlags, true);
}
...
TEST_P(VisualRectMappingTest, InclusiveIntersect)
{
    ...
    CheckMapToVisualRectInAncestorSpace(PhysicalRect(0, 0, 10, 10), PhysicalRect(50, 0, 0, 10), child, ancestor, kEdgeInclusive, true);

    CheckMapToVisualRectInAncestorSpace(PhysicalRect(1, 1, 10, 10), PhysicalRect(), child, ancestor, kEdgeInclusive, false);

    CheckMapToVisualRectInAncestorSpace(PhysicalRect(1, 1, 10, 10), PhysicalRect(1, 1, 10, 10), child, child, kEdgeInclusive, true);

    CheckMapToVisualRectInAncestorSpace(PhysicalRect(0, 0, 10, 10), PhysicalRect(), child, ancestor, kDefaultVisualRectFlags, false);
}
...
TEST_P(VisualRectMappingTest, AnchorPositionScroll)
{
    ...
    CheckVisualRect(anchored, ancestor, PhysicalRect(0, 0, 50, 50), PhysicalRect());

    auto* scrollable_area = GetScrollableArea(To<LayoutBlock>(GetLayoutBoxByElementId("scroller")));
    scrollable_area->ScrollToAbsolutePosition(gfx::PointF(400, 0));

    // Simulates a frame to update snapshotted scroll offset.
    GetPage().Animator().ServiceScriptedAnimations(GetAnimationClock().CurrentTime() + base::Milliseconds(100));
    UpdateAllLifecyclePhasesForTest();

    // #anchored is moved into view and should have a non-empty visual rect
    CheckVisualRect(anchored, ancestor, PhysicalRect(0, 0, 50, 50), PhysicalRect(100, 50, 50, 50));
}
...
TEST_P(VisualRectMappingTest, IgnoreFilters)
{
    ...
    CheckMapToVisualRectInAncestorSpace(input, expected_without_filter, child, filter, kDefaultVisualRectFlags, true);
    CheckMapToVisualRectInAncestorSpace(input, expected_without_filter, child, filter, kIgnoreFilters, true);
    CheckMapToVisualRectInAncestorSpace(input, expected_with_filter, child, parent, kDefaultVisualRectFlags, true);
    CheckMapToVisualRectInAncestorSpace(input, expected_without_filter, child, parent, kIgnoreFilters, true);
    CheckMapToVisualRectInAncestorSpace(input, expected_with_filter, filter, parent, kDefaultVisualRectFlags, true);
    CheckMapToVisualRectInAncestorSpace(input, expected_without_filter, filter, parent, kIgnoreFilters, true);
}
