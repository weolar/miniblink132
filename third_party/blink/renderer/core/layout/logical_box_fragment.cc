...
FontHeight LogicalBoxFragment::BaselineMetrics(const LineBoxStrut& margins, FontBaseline baseline_type) const
{
    ...
    case EBaselineSource::kAuto: {
        baseline = fragment.UseLastBaselineForInlineBaseline() ? LastBaseline() : FirstBaseline();
    ...
    if (baseline) {
        FontHeight metrics
            = writing_direction_.IsFlippedLines() ? FontHeight(BlockSize() - *baseline, *baseline) : FontHeight(*baseline, BlockSize() - *baseline);
    ...
    const auto SynthesizeMetrics = [&](LayoutUnit size) -> FontHeight {
        return baseline_type == kAlphabeticBaseline ? FontHeight(size, LayoutUnit()) : FontHeight(size - size / 2, size / 2);
    };
    ...
    case EInlineBlockBaselineEdge::kContentBox: {
        const LineBoxStrut border_scrollbar_padding(Borders() + Scrollbar() + Padding(), writing_direction_.IsFlippedLines());
        const LayoutUnit content_size = (BlockSize() - border_scrollbar_padding.BlockSum()).ClampNegativeToZero();
...
LayoutUnit LogicalBoxFragment::BlockEndScrollableOverflow() const
{
    ...
    LogicalRect overflow = converter.ToLogical(GetPhysicalBoxFragment().ScrollableOverflow());
