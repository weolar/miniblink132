...
class AnchorPositionVisibilityObserver final : public GarbageCollected<AnchorPositionVisibilityObserver> {
public:
    ...
    static void UpdateForChainedAnchorVisibility(const HeapHashSet<WeakMember<ScrollSnapshotClient>>&);
...
private:
    ...
    void OnIntersectionVisibilityChanged(const HeapVector<Member<IntersectionObserverEntry>>&);
