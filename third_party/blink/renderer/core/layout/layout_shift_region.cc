...
private:
    ...
    HashMap<int64_t, unsigned, IntWithZeroKeyHashTraits<int64_t>> endpoint_to_index_;
...
{
    DCHECK_HAS_INDEX(true);
    return endpoints_[segment.last_interval + 1] - endpoints_[segment.first_interval];
}
...
private:
    ...
    void Visit(unsigned node_index, Segment node_segment, Segment query_segment, int refcount_delta);
...
inline unsigned SegmentTree::ComputeActiveLength(unsigned node_index, Segment node_segment) const
{
    ...
    if (node_segment.last_interval > node_segment.first_interval) {
        return nodes_[LeftChild(node_index)].active_length + nodes_[RightChild(node_index)].active_length;
    }
    return 0;
}

void SegmentTree::Visit(unsigned node_index, Segment node_segment, Segment query_segment, int refcount_delta)
{
    ...
    if (node_low == query_low && node_high == query_high) {
        ...
        if (query_low <= lower_mid) {
            Visit(LeftChild(node_index), { node_low, lower_mid }, { query_low, std::min(query_high, lower_mid) }, refcount_delta);
        }
        if (query_high >= upper_mid) {
            Visit(RightChild(node_index), { upper_mid, node_high }, { std::max(query_low, upper_mid), query_high }, refcount_delta);
        }
    }
...
void Sweeper::InitEventQueue(Vector<SweepEvent>& events, const BasicIntervals& y_vals) const
{
    ...
    std::sort(events.begin(), events.end(), [](const SweepEvent& e1, const SweepEvent& e2) -> bool { return e1.x < e2.x; });
}

uint64_t Sweeper::SweepImpl(SegmentTree& tree, const Vector<SweepEvent>& events) const
{
    uint64_t area = 0;
