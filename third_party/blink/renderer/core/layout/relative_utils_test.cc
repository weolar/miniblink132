...
protected:
    ...
    const ComputedStyle* CreateStyle(LayoutUnit top, LayoutUnit right, LayoutUnit bottom, LayoutUnit left)
    {
        ...
        builder.SetRight(right == kAuto ? Length::Auto() : Length::Fixed(right.ToInt()));
        builder.SetBottom(bottom == kAuto ? Length::Auto() : Length::Fixed(bottom.ToInt()));
        builder.SetLeft(left == kAuto ? Length::Auto() : Length::Fixed(left.ToInt()));
...
TEST_F(RelativeUtilsTest, HorizontalTB)
{
    ...
    offset = ComputeRelativeOffset(*style, { WritingMode::kHorizontalTb, TextDirection::kLtr }, container_size_);
    ...
    // kRtl
    offset = ComputeRelativeOffset(*style, { WritingMode::kHorizontalTb, TextDirection::kRtl }, container_size_);
...
TEST_F(RelativeUtilsTest, VerticalRightLeft)
{
    ...
    // kLtr
    offset = ComputeRelativeOffset(*style, { WritingMode::kVerticalRl, TextDirection::kLtr }, container_size_);
    EXPECT_EQ(offset.inline_offset, kTop);
    EXPECT_EQ(offset.block_offset, kRight);

    // kRtl
    offset = ComputeRelativeOffset(*style, { WritingMode::kVerticalRl, TextDirection::kRtl }, container_size_);
...
TEST_F(RelativeUtilsTest, VerticalLeftRight)
{
    ...
    // kLtr
    offset = ComputeRelativeOffset(*style, { WritingMode::kVerticalLr, TextDirection::kLtr }, container_size_);
    EXPECT_EQ(offset.inline_offset, kTop);
    EXPECT_EQ(offset.block_offset, kLeft);

    // kRtl
    offset = ComputeRelativeOffset(*style, { WritingMode::kVerticalLr, TextDirection::kRtl }, container_size_);
    EXPECT_EQ(offset.inline_offset, kBottom);
    EXPECT_EQ(offset.block_offset, kLeft);
