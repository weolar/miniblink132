...
protected:
    bool ForegroundIsKnownToBeOpaqueInRect(const LayoutBox& box, const PhysicalRect& rect)
    {
...
TEST_F(LayoutBoxTest, ForegroundIsKnownToBeOpaqueInRect)
{
    ...
    EXPECT_TRUE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(0, 0, 10, 10)));
    // Covered by the first child of the second child is translucent.
    EXPECT_FALSE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(0, 10, 10, 10)));
    // Though covered by the second child of the second child which is opaque,
    // we ignore child layers.
    EXPECT_FALSE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(20, 20, 10, 10)));
    // Not covered by any child.
    EXPECT_FALSE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(0, 20, 10, 10)));
    // Partly covered by opaque children.
    EXPECT_FALSE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(0, 0, 30, 30)));
    EXPECT_FALSE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(0, 0, 10, 30)));
}
...
TEST_F(LayoutBoxTest, ForegroundIsKnownToBeOpaqueInRectVerticalRL)
{
    ...
    auto& target = *GetLayoutBoxByElementId("target");
    // Covered by the first child which is opaque.
    EXPECT_TRUE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(20, 0, 10, 10)));
    // Covered by the first child of the second child is translucent.
    EXPECT_FALSE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(10, 0, 10, 10)));
    ...
    EXPECT_FALSE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(0, 20, 10, 10)));
    // Not covered by any child.
    EXPECT_FALSE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(0, 0, 10, 10)));
    // Partly covered by opaque children.
    EXPECT_FALSE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(0, 0, 30, 30)));
    EXPECT_FALSE(ForegroundIsKnownToBeOpaqueInRect(target, PhysicalRect(20, 0, 30, 10)));
}
...
TEST_F(LayoutBoxTest, BackgroundRect)
{
    ...
    EXPECT_EQ(PhysicalRect(20, 20, 100, 100), layout_box->PhysicalBackgroundRect(kBackgroundKnownOpaqueRect));
    // The opaque border covers the translucent image outside of the padding box.
    EXPECT_EQ(PhysicalRect(10, 10, 120, 120), layout_box->PhysicalBackgroundRect(kBackgroundPaintedExtent));

    // #target1a is the same as #target1 except that the border is not opaque.
    layout_box = GetLayoutBoxByElementId("target1a");
    EXPECT_EQ(PhysicalRect(20, 20, 100, 100), layout_box->PhysicalBackgroundRect(kBackgroundKnownOpaqueRect));
    EXPECT_EQ(PhysicalRect(0, 0, 140, 140), layout_box->PhysicalBackgroundRect(kBackgroundPaintedExtent));

    // #target1b is the same as #target1 except no background color.
    layout_box = GetLayoutBoxByElementId("target1b");
    EXPECT_TRUE(layout_box->PhysicalBackgroundRect(kBackgroundKnownOpaqueRect).IsEmpty());
    EXPECT_EQ(PhysicalRect(10, 10, 120, 120), layout_box->PhysicalBackgroundRect(kBackgroundPaintedExtent));
    ...
    EXPECT_EQ(PhysicalRect(10, 10, 120, 120), layout_box->PhysicalBackgroundRect(kBackgroundKnownOpaqueRect));
    EXPECT_EQ(PhysicalRect(10, 10, 120, 120), layout_box->PhysicalBackgroundRect(kBackgroundPaintedExtent));

    // #target2b is the same as #target2 except no background color.
    layout_box = GetLayoutBoxByElementId("target2b");
    EXPECT_TRUE(layout_box->PhysicalBackgroundRect(kBackgroundKnownOpaqueRect).IsEmpty());
    EXPECT_EQ(PhysicalRect(20, 20, 100, 100), layout_box->PhysicalBackgroundRect(kBackgroundPaintedExtent));
    ...
    EXPECT_TRUE(layout_box->PhysicalBackgroundRect(kBackgroundKnownOpaqueRect).IsEmpty());
    EXPECT_TRUE(layout_box->PhysicalBackgroundRect(kBackgroundPaintedExtent).IsEmpty());
...
TEST_F(LayoutBoxTest, LocationContainerOfSVG)
{
    ...
    EXPECT_EQ(GetDocument().body()->GetLayoutObject(), svg_root->LocationContainer());
...
#define EXPECT_CONTENTS_VISUAL_OVERFLOW(rect, layout_box)                                                                                                      \
    if (layout_box->CanUseFragmentsForVisualOverflow()) {                                                                                                      \
        EXPECT_EQ(UnionRect(rect, layout_box->PhysicalBorderBoxRect()), layout_box->ContentsVisualOverflowRect());                                             \
    } else {                                                                                                                                                   \
...
TEST_F(LayoutBoxTest, ContentsVisualOverflowPropagation)
{
    ...
    EXPECT_CONTENTS_VISUAL_OVERFLOW(PhysicalRect(kCContentsLeft, 20, 100, 100), c);
    ...
    EXPECT_CONTENTS_VISUAL_OVERFLOW(PhysicalRect(kBContentsLeft, 40, 200, 120), b);
...
TEST_F(LayoutBoxTest, OverflowRectsContainPaint)
{
    ...
    EXPECT_EQ(PhysicalRect(10, 10, 300, 400), container->ScrollableOverflowRect());
    ...
    EXPECT_EQ(PhysicalRect(10, 10, 100, 200), container->OverflowClipRect(PhysicalOffset()));
}
...
TEST_F(LayoutBoxTest, DelayedInvalidation)
{
    ...
    // animated images.
    auto* image = ImageResourceContent::CreateLoaded(base::AdoptRef(new AnimatedImage()));
    ...
    EXPECT_EQ(obj->PaintInvalidationReasonForPrePaint(), PaintInvalidationReason::kNone);
    ...
    EXPECT_FALSE(obj->ShouldDoFullPaintInvalidation());
    EXPECT_EQ(obj->PaintInvalidationReasonForPrePaint(), PaintInvalidationReason::kImage);
...
TEST_F(LayoutBoxTest, DelayedInvalidationLayoutViewScrolled)
{
    ...
    // it from there.
    auto* background_image = layout_view->StyleRef().BackgroundLayers().GetImage();
...
TEST_F(LayoutBoxTest, MarkerContainerScrollableOverflowRect)
{
    ...
    auto* marker_container = To<LayoutBox>(GetLayoutObjectByElementId("target")->SlowFirstChild());
    EXPECT_GE(marker_container->PhysicalLocation().top + marker_container->ScrollableOverflowRect().Bottom(), LayoutUnit(50));
}
...
TEST_F(LayoutBoxTest, LocationOfAbsoluteChildWithContainerScrollbarsDifferentWritingModes)
{
    SetBodyInnerHTML(CommonStyleForGeometryWithScrollbarTests() + R"HTML(
    <style>
      .container { position: relative; }
...
TEST_F(LayoutBoxTest, LocationOfAbsoluteAutoTopLeftChildWithContainerScrollbars)
{
    SetBodyInnerHTML(CommonStyleForGeometryWithScrollbarTests() + R"HTML(
    <style>
      .container { position: relative; }
...
TEST_F(LayoutBoxTest, LocationOfAbsoluteAutoTopLeftGrandChildWithContainerScrollbars)
{
    SetBodyInnerHTML(CommonStyleForGeometryWithScrollbarTests() + R"HTML(
    <style>
      .container { position: relative; }
...
TEST_F(LayoutBoxTest, GeometriesWithScrollbarsScrollable)
{
    ...
    EXPECT_EQ(gfx::Vector2d(1615, 716), scrollable_area->MaximumScrollOffsetInt());
    EXPECT_EQ(gfx::Vector2d(), scrollable_area->MinimumScrollOffsetInt());
    EXPECT_EQ(gfx::Point(), scrollable_area->ScrollOrigin());
    EXPECT_EQ(gfx::PointF(), scrollable_area->ScrollPosition());
    ...
    EXPECT_EQ(gfx::Vector2d(), rtl_vlr->OriginAdjustmentForScrollbars());
    EXPECT_EQ(gfx::Vector2d(), scrollable_area->ScrollOffsetInt());
    EXPECT_EQ(PhysicalRect(50, -696, 2060, 1040), rtl_vlr->ScrollableOverflowRect());
    ...
    EXPECT_EQ(gfx::Vector2d(), scrollable_area->ScrollOffsetInt());
    // Same as "vlr" except for flipping.
    EXPECT_EQ(PhysicalRect(-1565, -696, 2060, 1040), rtl_vrl->ScrollableOverflowRect());
    EXPECT_EQ(gfx::Vector2d(), scrollable_area->MaximumScrollOffsetInt());
    EXPECT_EQ(gfx::Vector2d(-1615, -716), scrollable_area->MinimumScrollOffsetInt());
...
TEST_F(LayoutBoxTest, ThickScrollbarSubpixelSizeMarginNoDirtyLayoutAfterLayout)
{
...
TEST_F(LayoutBoxTest, HasReflection)
{
    ...
    auto check_has_layer_and_reflection = [&](const char* element_id, bool expected) {
...
TEST_F(LayoutBoxTest, PhysicalVisualOverflowRectIncludingFilters)
{
    ...
    EXPECT_EQ(PhysicalRect(-12, -12, 224, 424), GetLayoutBoxByElementId("target")->VisualOverflowRectIncludingFilters());
}
...
TEST_F(LayoutBoxTest, HitTestResizerWithTextAreaChild)
{
    ...
    EXPECT_TRUE(HitTest(1, 1)->IsDescendantOrShadowDescendantOf(GetElementById("textarea")));
}
...
TEST_F(LayoutBoxTest, AnchorInFragmentedContainingBlock)
{
    ...
    EXPECT_EQ(GetLayoutObjectByElementId("anchor"), target->FindTargetAnchor(*MakeGarbageCollected<ScopedCSSName>(AtomicString("--a"), &GetDocument())));
    EXPECT_EQ(GetLayoutObjectByElementId("anchor"), target->AcceptableImplicitAnchor());
}
...
TEST_F(LayoutBoxTest, AnchorInInlineContainingBlockWithNameConflicts)
{
    ...
    const ScopedCSSName& anchor_name = *MakeGarbageCollected<ScopedCSSName>(AtomicString("--a"), &GetDocument());

    const LayoutBox* target1 = To<LayoutBox>(GetLayoutObjectByElementId("target1"));
    EXPECT_EQ(GetLayoutObjectByElementId("anchor1"), target1->FindTargetAnchor(anchor_name));

    const LayoutBox* target2 = To<LayoutBox>(GetLayoutObjectByElementId("target2"));
    EXPECT_EQ(GetLayoutObjectByElementId("anchor2"), target2->FindTargetAnchor(anchor_name));

    const LayoutBox* target3 = To<LayoutBox>(GetLayoutObjectByElementId("target3"));
    EXPECT_EQ(GetLayoutObjectByElementId("anchor3"), target3->FindTargetAnchor(anchor_name));
}
...
TEST_F(LayoutBoxTest, IsUserScrollable)
{
    ...
    target_element->setAttribute(html_names::kStyleAttr, AtomicString("overflow: hidden"));
    UpdateAllLifecyclePhasesForTest();
    ...
    GetElementById("content")->setAttribute(html_names::kStyleAttr, AtomicString("height: 0"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(target->ScrollsOverflow());
    EXPECT_FALSE(target->IsUserScrollable());
}
...
class LayoutBoxBackgroundPaintLocationTest : public RenderingTest, public PaintTestConfigurations {
protected:
    void SetUp() override
    {
        EnableCompositing();
...
TEST_P(LayoutBoxBackgroundPaintLocationTest, ContentBoxClipZeroPadding)
{
    ...
    EXPECT_EQ(kBackgroundPaintInBorderBoxSpace, ScrollerBackgroundPaintLocation());
}

TEST_P(LayoutBoxBackgroundPaintLocationTest, AttachmentLocalContentBoxClipNonZeroPadding)
{
    SetBodyInnerHTML(kCommonStyle + R"HTML(
    <div id='scroller'
...
TEST_P(LayoutBoxBackgroundPaintLocationTest, LocalImageAndNonLocalClipPaddingColor)
{
    SetBodyInnerHTML(kCommonStyle + R"HTML(
    <div id='scroller'
        style='background: url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUg)
...
TEST_P(LayoutBoxBackgroundPaintLocationTest, LocalImageAndNonLocalClipContentColorNonZeroPadding)
{
    SetBodyInnerHTML(kCommonStyle + R"HTML(
    <div id='scroller'
        style='background: url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUg)
...
TEST_P(LayoutBoxBackgroundPaintLocationTest, BorderBoxClipColorTranslucentBorder)
{
    SetBodyInnerHTML(kCommonStyle + R"HTML(
    <div id='scroller'
...
TEST_P(LayoutBoxBackgroundPaintLocationTest, TranslucentColorAndTranslucentBorder)
{
    SetBodyInnerHTML(kCommonStyle + R"HTML(
    <div id='scroller'
...
TEST_P(LayoutBoxBackgroundPaintLocationTest, LocalImageTranslucentColorAndTransparentBorder)
{
    SetBodyInnerHTML(kCommonStyle + R"HTML(
    <div id='scroller'
