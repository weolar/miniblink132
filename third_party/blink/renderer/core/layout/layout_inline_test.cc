...
protected:
    bool HitTestAllPhases(LayoutObject& object, HitTestResult& result, const HitTestLocation& location, const PhysicalOffset& offset)
    {
        if (!object.IsBox()) {
        ...
        return BoxFragmentPainter(fragment).HitTestAllPhases(result, location, offset);
    }
};
...
TEST_F(LayoutInlineTest, PhysicalLinesBoundingBox)
{
    ...
    EXPECT_EQ(PhysicalRect(0, 0, 30, 20), To<LayoutInline>(GetLayoutObjectByElementId("ltr1"))->PhysicalLinesBoundingBox());
    EXPECT_EQ(PhysicalRect(0, 0, 110, 10), To<LayoutInline>(GetLayoutObjectByElementId("ltr2"))->PhysicalLinesBoundingBox());
    EXPECT_EQ(PhysicalRect(270, 0, 30, 20), To<LayoutInline>(GetLayoutObjectByElementId("rtl1"))->PhysicalLinesBoundingBox());
    EXPECT_EQ(PhysicalRect(190, 0, 110, 10), To<LayoutInline>(GetLayoutObjectByElementId("rtl2"))->PhysicalLinesBoundingBox());
    EXPECT_EQ(PhysicalRect(280, 0, 20, 30), To<LayoutInline>(GetLayoutObjectByElementId("vertical"))->PhysicalLinesBoundingBox());
}

TEST_F(LayoutInlineTest, SimpleContinuation)
{
    SetBodyInnerHTML("<span id='splitInline'>"
                     "<i id='before'></i>"
                     "<h1 id='blockChild'></h1>"
                     "<i id='after'></i>"
                     "</span>");

    auto* split_inline_part1 = To<LayoutInline>(GetLayoutObjectByElementId("splitInline"));
...
TEST_F(LayoutInlineTest, BlockInInlineRemove)
{
    ...
    for (const auto* child = span->SlowFirstChild(); child; child = child->NextSibling()) {
    ...
    Element* block2_element = document.CreateElementForBinding(AtomicString("div"));
...
TEST_F(LayoutInlineTest, RegionHitTest)
{
    ...
    auto* lots_of_boxes = To<LayoutInline>(GetLayoutObjectByElementId("lotsOfBoxes"));
    ASSERT_TRUE(lots_of_boxes);

    HitTestRequest hit_request(HitTestRequest::kTouchEvent | HitTestRequest::kListBased);
    ...
    for (cursor.MoveToFirstLine(); cursor; cursor.MoveToNextLine()) {
        ...
        bool hit_outcome = lots_of_boxes->HitTestCulledInline(hit_result, location, hit_offset, line_cursor);
...
{
    LoadAhem();
    SetBodyInnerHTML("<div style='font: 10px/10px Ahem'>"
                     "  <span style='position: relative'>XXX</span>"
                     "</div>");

    HitTestRequest hit_request(HitTestRequest::kReadOnly | HitTestRequest::kActive);
    ...
    // Shouldn't hit anything in SPAN as it's in another paint layer
    {
        ...
        HitTestResult hit_result(hit_request, location);
        bool hit_outcome = HitTestAllPhases(*layout_div, hit_result, location, container_offset);
    ...
    // the SPAN itself.
    {
        ...
        HitTestResult hit_result(hit_request, location);
        bool hit_outcome = HitTestAllPhases(*layout_span, hit_result, location, container_offset);
        EXPECT_TRUE(hit_outcome);
...
{
    LoadAhem();
    SetBodyInnerHTML("<div style='font: 10px/10px Ahem; width: 30px'>"
    ...
    HitTestRequest hit_request(HitTestRequest::kReadOnly | HitTestRequest::kActive | HitTestRequest::kIgnorePointerEventsNone);
    const PhysicalOffset container_offset(8, 8);
    ...
    // Hit test first line
    {
        ...
        bool layout_view_hit_outcome = GetLayoutView().HitTest(location, layout_view_hit_result);
...
TEST_F(LayoutInlineTest, DISABLED_FocusRingRecursiveContinuations)
{
    ...
    auto rects = GetLayoutObjectByElementId("target")->OutlineRects(nullptr, PhysicalOffset(), OutlineType::kIncludeBlockInkOverflow);

    EXPECT_THAT(rects,
        UnorderedElementsAre(PhysicalRect(0, 0, 100, 20), // 'SPAN0'
...
TEST_F(LayoutInlineTest, DISABLED_FocusRingRecursiveInlinesVerticalRL)
{
    ...
    auto rects = target->OutlineRects(nullptr, target->FirstFragment().PaintOffset(), OutlineType::kIncludeBlockInkOverflow);
    EXPECT_THAT(rects,
        UnorderedElementsAre(PhysicalRect(180, 0, 20, 120), // 'INLINE'
...
TEST_F(LayoutInlineTest, DISABLED_FocusRingRecursiveContinuationsVerticalRL)
{
    ...
    auto rects = target->OutlineRects(nullptr, target->FirstFragment().PaintOffset(), OutlineType::kIncludeBlockInkOverflow);
    EXPECT_THAT(rects,
        UnorderedElementsAre(PhysicalRect(180, 0, 20, 100), // 'SPAN0'
...
TEST_F(LayoutInlineTest, DISABLED_FocusRingRecursiveInlines)
{
    ...
    EXPECT_THAT(rects,
        UnorderedElementsAre(PhysicalRect(0, 0, 120, 20), // 'INLINE'
...
TEST_F(LayoutInlineTest, AbsoluteBoundingBoxRectHandlingEmptyInline)
{
    ...
    EXPECT_EQ(PhysicalRect(50, 70, 0, 0), GetLayoutObjectByElementId("target1")->AbsoluteBoundingBoxRectHandlingEmptyInline());
    EXPECT_EQ(PhysicalRect(50, 90, 0, 0), GetLayoutObjectByElementId("target2")->AbsoluteBoundingBoxRectHandlingEmptyInline());
    EXPECT_EQ(PhysicalRect(50, 110, 0, 0), GetLayoutObjectByElementId("target3")->AbsoluteBoundingBoxRectHandlingEmptyInline());
    EXPECT_EQ(PhysicalRect(50, 130, 0, 0), GetLayoutObjectByElementId("target4")->AbsoluteBoundingBoxRectHandlingEmptyInline());
    EXPECT_EQ(PhysicalRect(50, 150, 0, 0), GetLayoutObjectByElementId("target5")->AbsoluteBoundingBoxRectHandlingEmptyInline());
    // This rect covers the overflowing images and continuations.
    // 168 = (30 + 4) * 2 + 100. 4 is the descent of the font.
    const int width = 400;
    EXPECT_EQ(PhysicalRect(50, 170, width, 168), GetLayoutObjectByElementId("target6")->AbsoluteBoundingBoxRectHandlingEmptyInline());
}
...
TEST_F(LayoutInlineTest, AbsoluteBoundingBoxRectHandlingEmptyInlineVerticalRL)
{
    ...
    EXPECT_EQ(PhysicalRect(630, 70, 0, 0), GetLayoutObjectByElementId("target1")->AbsoluteBoundingBoxRectHandlingEmptyInline());
    EXPECT_EQ(PhysicalRect(610, 70, 0, 0), GetLayoutObjectByElementId("target2")->AbsoluteBoundingBoxRectHandlingEmptyInline());
    EXPECT_EQ(PhysicalRect(590, 70, 0, 0), GetLayoutObjectByElementId("target3")->AbsoluteBoundingBoxRectHandlingEmptyInline());
    EXPECT_EQ(PhysicalRect(570, 70, 0, 0), GetLayoutObjectByElementId("target4")->AbsoluteBoundingBoxRectHandlingEmptyInline());
    EXPECT_EQ(PhysicalRect(550, 70, 0, 0), GetLayoutObjectByElementId("target5")->AbsoluteBoundingBoxRectHandlingEmptyInline());
    // This rect covers the overflowing images and continuations.
    const int height = 400;
    EXPECT_EQ(PhysicalRect(390, 70, 160, height), GetLayoutObjectByElementId("target6")->AbsoluteBoundingBoxRectHandlingEmptyInline());
}
...
TEST_F(LayoutInlineTest, VisualOverflowRecalcLegacyLayout)
{
    ...
    span_element->setAttribute(html_names::kStyleAttr, AtomicString("outline: 50px solid red"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(PhysicalRect(-50, -50, 200, 120), span->VisualOverflowRect());

    span_element->setAttribute(html_names::kStyleAttr, g_empty_atom);
    span2_element->setAttribute(html_names::kStyleAttr, AtomicString("outline: 50px solid red"));
    UpdateAllLifecyclePhasesForTest();
