...
LayoutTextCombine* LayoutTextCombine::CreateAnonymous(LayoutText* text_child)
{
    ...
    ComputedStyleBuilder new_style_builder = document.GetStyleResolver().CreateAnonymousStyleBuilderWithDisplay(text_child->StyleRef(), EDisplay::kInlineBlock);
...
void LayoutTextCombine::AssertStyleIsValid(const ComputedStyle& style)
{
    ...
    DCHECK_EQ(style.GetFont().GetFontDescription().Orientation(), FontOrientation::kHorizontal);
#endif
}

float LayoutTextCombine::DesiredWidth() const
{
    DCHECK_EQ(StyleRef().GetFont().GetFontDescription().Orientation(), FontOrientation::kHorizontal);
    const float one_em = StyleRef().ComputedFontSize();
    if (EnumHasFlags(Parent()->StyleRef().TextDecorationsInEffect(), TextDecorationLine::kUnderline | TextDecorationLine::kOverline)) {
        return one_em;
    }
...
PhysicalOffset LayoutTextCombine::ApplyScaleX(const PhysicalOffset& offset) const
{
    DCHECK(scale_x_.has_value());
    const float spacing = ComputeInlineSpacing();
    return PhysicalOffset(LayoutUnit(offset.left * *scale_x_ + spacing), offset.top);
}
...
PhysicalOffset LayoutTextCombine::UnapplyScaleX(const PhysicalOffset& offset) const
{
    DCHECK(scale_x_.has_value());
    const float spacing = ComputeInlineSpacing();
    return PhysicalOffset(LayoutUnit((offset.left - spacing) / *scale_x_), offset.top);
}

PhysicalOffset LayoutTextCombine::AdjustOffsetForHitTest(const PhysicalOffset& offset_in_container) const
{
    if (!scale_x_) {
...
PhysicalOffset LayoutTextCombine::AdjustOffsetForLocalCaretRect(const PhysicalOffset& offset_in_container) const
{
    if (!scale_x_) {
        return offset_in_container;
    }
...
PhysicalRect LayoutTextCombine::AdjustRectForBoundingBox(const PhysicalRect& rect) const
{
    if (!scale_x_) {
        return rect;
    }
...
PhysicalRect LayoutTextCombine::ComputeTextBoundsRectForHitTest(const FragmentItem& text_item, const PhysicalOffset& inline_root_offset) const
{
...
LayoutUnit LayoutTextCombine::AdjustTextLeftForPaint(LayoutUnit position) const
{
    if (!scale_x_) {
        return position;
    }
...
AffineTransform LayoutTextCombine::ComputeAffineTransformForPaint(const PhysicalOffset& paint_offset) const
{
...
LineRelativeRect LayoutTextCombine::ComputeTextFrameRect(const PhysicalOffset paint_offset) const
{
...
PhysicalRect LayoutTextCombine::RecalcContentsInkOverflow(const InlineCursor& cursor) const
{
    const ComputedStyle& style = Parent()->StyleRef();
    DCHECK(style.GetFont().GetFontDescription().IsVerticalBaseline());

    const LineRelativeRect line_relative_text_rect = ComputeTextFrameRect(PhysicalOffset());
    ...
    // requires it.
    const PhysicalRect text_rect { PhysicalOffset(), PhysicalSize { line_relative_text_rect.size.inline_size, line_relative_text_rect.size.block_size } };
    ...
    if (style.HasAppliedTextDecorations()) {
        ...
        const LogicalRect decoration_rect = InkOverflow::ComputeDecorationOverflow(cursor, style, style.GetFont(),
    ...
    if (style.GetTextEmphasisMark() != TextEmphasisMark::kNone) {
        ink_overflow = InkOverflow::ComputeEmphasisMarkOverflow(style, text_rect.size, ink_overflow);
    }

    if (const ShadowList* text_shadow = style.TextShadow()) {
        InkOverflow::ExpandForShadowOverflow(ink_overflow, *text_shadow, writing_mode);
    }

    PhysicalRect local_ink_overflow = WritingModeConverter({ writing_mode, TextDirection::kLtr }, text_rect.size).ToPhysical(ink_overflow);
    local_ink_overflow.ExpandEdgesToPixelBoundaries();
    return local_ink_overflow;
}

gfx::Rect LayoutTextCombine::VisualRectForPaint(const PhysicalOffset& paint_offset) const
{
...
{
    return Parent()->StyleRef().GetFont().GetFontDescription().IsSyntheticOblique();
}
