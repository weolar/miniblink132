...
public:
    ...
    OofContainingBlock(OffsetType offset, OffsetType relative_offset, const PhysicalFragment* fragment,
        std::optional<LayoutUnit> clipped_container_block_offset, bool is_inside_column_spanner)
        : offset_(offset)
        , relative_offset_(relative_offset)
        , fragment_(std::move(fragment))
        , clipped_container_block_offset_(clipped_container_block_offset.value_or(LayoutUnit::Min()))
    ...
    bool IsInsideColumnSpanner() const
    {
        return is_inside_column_spanner_;
    }
...
template <typename OffsetType> struct OofInlineContainer {
    DISALLOW_NEW();
...
template <typename OffsetType> struct MulticolWithPendingOofs : public GarbageCollected<MulticolWithPendingOofs<OffsetType>> {
public:
    ...
    MulticolWithPendingOofs(
        OffsetType multicol_offset, OofContainingBlock<OffsetType> fixedpos_containing_block, OofInlineContainer<OffsetType> fixedpos_inline_container)
...
public:
    ...
    PhysicalOofPositionedNode(BlockNode node, PhysicalStaticPosition static_position, bool requires_content_before_breaking, bool is_hidden_for_paint,
        OofInlineContainer<PhysicalOffset> inline_container = {})
    ...
    {
        return { static_position, GetStaticPositionHorizontalEdge(), GetStaticPositionVerticalEdge() };
    }
...
public:
    ...
    LogicalOofPositionedNode(BlockNode node, LogicalStaticPosition static_position, bool requires_content_before_breaking, bool is_hidden_for_paint,
        OofInlineContainer<LogicalOffset> inline_container = {})
        : box(node.GetLayoutBox())
...
//
// This is struct is allowed to be stored/persisted.
struct CORE_EXPORT PhysicalOofNodeForFragmentation final : public PhysicalOofPositionedNode {
    DISALLOW_NEW();
...
public:
    ...
    PhysicalOofNodeForFragmentation(BlockNode node, PhysicalStaticPosition static_position, bool requires_content_before_breaking, bool is_hidden_for_paint,
        OofInlineContainer<PhysicalOffset> inline_container = {}, OofContainingBlock<PhysicalOffset> containing_block = {},
        OofContainingBlock<PhysicalOffset> fixedpos_containing_block = {}, OofInlineContainer<PhysicalOffset> fixedpos_inline_container = {})
        : PhysicalOofPositionedNode(node, static_position, requires_content_before_breaking, is_hidden_for_paint, inline_container)
...
// It is *only* used within an algorithm pass, (it is temporary, and should not
// be stored/persisted).
struct CORE_EXPORT LogicalOofNodeForFragmentation final : public LogicalOofPositionedNode {
    DISALLOW_NEW();
...
public:
    ...
    LogicalOofNodeForFragmentation(BlockNode node, LogicalStaticPosition static_position, bool requires_content_before_breaking, bool is_hidden_for_paint,
        OofInlineContainer<LogicalOffset> inline_container = {}, OofContainingBlock<LogicalOffset> containing_block = {},
        OofContainingBlock<LogicalOffset> fixedpos_containing_block = {}, OofInlineContainer<LogicalOffset> fixedpos_inline_container = {})
        : LogicalOofPositionedNode(node, static_position, requires_content_before_breaking, is_hidden_for_paint, inline_container)
        , containing_block(containing_block)
        , fixedpos_containing_block(fixedpos_containing_block)
        , fixedpos_inline_container(fixedpos_inline_container)
    {
    ...
    explicit LogicalOofNodeForFragmentation(const LogicalOofPositionedNode& oof_node)
        : LogicalOofPositionedNode(
            oof_node.Node(), oof_node.static_position, oof_node.requires_content_before_breaking, oof_node.is_hidden_for_paint, oof_node.inline_container)
    {
        is_for_fragmentation = true;
    }

    const LayoutObject* CssContainingBlock() const
    {
...
template <> struct DowncastTraits<LogicalOofNodeForFragmentation> {
...
struct FragmentedOofData final : PhysicalFragment::OofData {
    using MulticolCollection = HeapHashMap<Member<LayoutBox>, Member<MulticolWithPendingOofs<PhysicalOffset>>>;

    static bool HasOutOfFlowPositionedFragmentainerDescendants(const PhysicalFragment& fragment)
    {
        const auto* oof_data = fragment.GetFragmentedOofData();
        return oof_data && !oof_data->oof_positioned_fragmentainer_descendants.empty();
    }

    bool NeedsOOFPositionedInfoPropagation() const
    {
        return !oof_positioned_fragmentainer_descendants.empty() || !multicols_with_pending_oofs.empty();
    }

    static base::span<PhysicalOofNodeForFragmentation> OutOfFlowPositionedFragmentainerDescendants(const PhysicalFragment& fragment)
    {
        const auto* oof_data = fragment.GetFragmentedOofData();
        ...
        HeapVector<PhysicalOofNodeForFragmentation>& descendants
            = const_cast<HeapVector<PhysicalOofNodeForFragmentation>&>(oof_data->oof_positioned_fragmentainer_descendants);
    ...
    HeapVector<PhysicalOofNodeForFragmentation> oof_positioned_fragmentainer_descendants;
    MulticolCollection multicols_with_pending_oofs;
};

inline PhysicalOffset RelativeInsetToPhysical(LogicalOffset relative_inset, WritingDirectionMode writing_direction)
{
    return relative_inset.ConvertToPhysical(writing_direction, PhysicalSize(), PhysicalSize());
}

inline LogicalOffset RelativeInsetToLogical(PhysicalOffset relative_inset, WritingDirectionMode writing_direction)
{
    return relative_inset.ConvertToLogical(writing_direction, PhysicalSize(), PhysicalSize());
}

} // namespace blink

WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::PhysicalOofPositionedNode)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::PhysicalOofNodeForFragmentation)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::LogicalOofPositionedNode)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::LogicalOofNodeForFragmentation)
