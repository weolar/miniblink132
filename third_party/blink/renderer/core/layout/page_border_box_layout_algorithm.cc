...
PageBorderBoxLayoutAlgorithm::PageBorderBoxLayoutAlgorithm(
    const LayoutAlgorithmParams& params, const BlockNode& content_node, const PageAreaLayoutParams& page_area_params)
...
const LayoutResult* PageBorderBoxLayoutAlgorithm::Layout()
{
    ...
    FragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(fragmentainer_space, content_node_, /*break_token=*/nullptr);
    LayoutAlgorithmParams params(content_node_, fragment_geometry, fragmentainer_space, page_area_params_.break_token, /*early_break=*/nullptr);
    ...
    if (page_area_params_.template_fragmentainer) {
        ...
        SimplifiedOofLayoutAlgorithm algorithm(params, *page_area_params_.template_fragmentainer);
    ...
    LogicalOffset origin = converter.ToLogical(PhysicalOffset(), result->GetPhysicalFragment().Size());
    container_builder_.AddResult(*result, origin, /*margins=*/std::nullopt);

    return container_builder_.ToBoxFragment();
}

ConstraintSpace PageBorderBoxLayoutAlgorithm::CreateConstraintSpaceForPageArea() const
{
    ...
    // document contents.
    ConstraintSpaceBuilder space_builder(GetConstraintSpace(), content_node_.Style().GetWritingDirection(),
        /*is_new_fc=*/true);
