...
{
    NOT_DESTROYED();
    if (IsInline() || IsFloatingOrOutOfFlowPositioned() || IsScrollContainer() || IsFlexItem() || IsCustomItem() || IsDocumentElement() || IsGridItem()
        || IsWritingModeRoot() || IsMathItem() || StyleRef().Display() == EDisplay::kFlowRoot || StyleRef().Display() == EDisplay::kFlowRootListItem
        || ShouldApplyPaintContainment() || ShouldApplyLayoutContainment() || StyleRef().HasLineClamp() || StyleRef().SpecifiesColumns()
        || StyleRef().GetColumnSpan() == EColumnSpan::kAll) {
        // The specs require this object to establish a new formatting context.
        return true;
    }

    if (RuntimeEnabledFeatures::CanvasPlaceElementEnabled() && Parent()->IsCanvas()) {
        return true;
    }
    ...
    if (StyleRef().AlignContent().GetPosition() != ContentPosition::kNormal || StyleRef().AlignContent().Distribution() != ContentDistributionType::kDefault) {
        return true;
    }
...
DISABLE_CFI_PERF
void LayoutBlockFlow::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    if (old_style) {
        ...
        if (old_style->CanGeneratePseudoElement(kPseudoIdColumn) != StyleRef().CanGeneratePseudoElement(kPseudoIdColumn)) {
    ...
    if (diff.NeedsReshape()) {
        ...
        if (old_style->InitialLetter().IsNormal() != new_style.InitialLetter().IsNormal()) [[unlikely]] {
