...
{
    Locale& locale = node ? To<Element>(node)->GetLocale() : Locale::DefaultLocale();
    switch (availability) {
...
void LayoutEmbeddedObject::SetPluginAvailability(PluginAvailability availability)
{
    ...
    unavailable_plugin_replacement_text_ = LocalizedUnavailablePluginReplacementText(GetNode(), availability);
...
void LayoutEmbeddedObject::PaintReplaced(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
...
void LayoutEmbeddedObject::ComputeIntrinsicSizingInfo(IntrinsicSizingInfo& intrinsic_sizing_info) const
{
    NOT_DESTROYED();
