    ...
    , autosize_v_scrollbar_mode_(mojom::blink::ScrollbarMode::kAuto)
{
    ...
    if (GetDocument().GetRootScrollerController().EffectiveRootScroller() == GetDocument()) {
...
bool LayoutView::HitTest(const HitTestLocation& location, HitTestResult& result)
{
    NOT_DESTROYED();
    ...
    if (!GetFrameView()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kHitTest))
        return false;
...
bool LayoutView::HitTestNoLifecycleUpdate(const HitTestLocation& location, HitTestResult& result)
{
    NOT_DESTROYED();
    ...
    if (hit_test_cache_->LookupCachedResult(location, cache_result, dom_tree_version)) {
        ...
        if (frame_view) {
            ...
            if (result.GetHitTestRequest().IgnoreClipping()) {
                hit_test_area.Unite(frame_view->DocumentToFrame(PhysicalRect(DocumentRect())));
            }
        }
    ...
    TRACE_EVENT_END1("blink,devtools.timeline", "HitTest", "endData",
        [&](perfetto::TracedValue context) { inspector_hit_test_event::EndData(std::move(context), result.GetHitTestRequest(), location, result); });
    return hit_layer;
}
...
LayoutUnit LayoutView::ComputeMinimumWidth()
{
    ...
    ConstraintSpaceBuilder builder(mode, style.GetWritingDirection(),
        /* is_new_fc */ true);
    return BlockNode(this).ComputeMinMaxSizes(mode, SizeType::kIntrinsic, builder.ToConstraintSpace()).sizes.min_size;
}
...
void LayoutView::AddChild(LayoutObject* new_child, LayoutObject* before_child)
{
    if (new_child->StyleRef().StyleType() == kPseudoIdViewTransition) {
        ...
        LayoutViewTransitionRoot* snapshot_containing_block = MakeGarbageCollected<LayoutViewTransitionRoot>(GetDocument());
        LayoutBlockFlow::AddChild(snapshot_containing_block,
            /*before_child=*/nullptr);
        snapshot_containing_block->AddChild(new_child);

        ViewTransition* transition = ViewTransitionUtils::GetTransition(GetDocument());
        CHECK(transition);
...
bool LayoutView::CanHaveChildren() const
{
    ...
    if (IsA<PluginDocument>(GetDocument()) || GetDocument().IsForExternalHandler())
        return true;
...
bool LayoutView::ShouldPlaceBlockDirectionScrollbarOnLogicalLeft() const
{
    ...
    if (Element* body = GetDocument().body()) {
        if (LayoutObject* body_layout_object = body->GetLayoutObject()) {
            return body_layout_object->StyleRef().ShouldPlaceBlockDirectionScrollbarOnLogicalLeft();
        }
    }
    return false;
}

void LayoutView::MapLocalToAncestor(const LayoutBoxModelObject* ancestor, TransformState& transform_state, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    if (!ancestor && !(mode & kIgnoreTransforms) && ShouldUseTransformFromContainer(nullptr)) {
        gfx::Transform t;
    ...
    if (mode & kTraverseDocumentBoundaries) {
        ...
        if (parent_doc_layout_object) {
            transform_state.Move(parent_doc_layout_object->PhysicalContentBoxOffset());
            parent_doc_layout_object->MapLocalToAncestor(ancestor, transform_state, mode);
        } else {
            DCHECK(!ancestor);
...
void LayoutView::MapAncestorToLocal(const LayoutBoxModelObject* ancestor, TransformState& transform_state, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    if (this != ancestor && (mode & kTraverseDocumentBoundaries)) {
        if (auto* parent_doc_layout_object = GetFrame()->OwnerLayoutObject()) {
            ...
            parent_doc_layout_object->MapAncestorToLocal(ancestor, transform_state, mode);
            ...
            if ((mode & kApplyRemoteMainFrameTransform) && GetFrame()->IsLocalRoot()) {
                GetFrameView()->MapLocalToRemoteMainFrame(transform_state);
            }
        }
    } else {
...
{
    return LogicalSize(LayoutUnit(ViewLogicalWidthForBoxSizing()), LayoutUnit(ViewLogicalHeightForBoxSizing()));
}
...
void LayoutView::RegisterVariableLengthTransformResult(const LayoutText& text, const VariableLengthTransformResult& result)
{
    CHECK(text.HasVariableLengthTransform());
    text_to_variable_length_transform_result_.Set(&text, result);
}

void LayoutView::UnregisterVariableLengthTransformResult(const LayoutText& text)
{
    text_to_variable_length_transform_result_.erase(&text);
}

VariableLengthTransformResult LayoutView::GetVariableLengthTransformResult(const LayoutText& text)
{
    CHECK(text.HasVariableLengthTransform());
    ...
    const LayoutBoxModelObject* ancestor, TransformState& transform_state, VisualRectFlags visual_rect_flags) const
{
    ...
    if (Layer() && Layer()->Transform()) {
        transform_state.ApplyTransform(Layer()->CurrentTransform(), TransformState::kFlattenTransform);
    }
    ...
    if (!owner) {
        ...
        bool retval = GetFrameView()->MapToVisualRectInRemoteRootFrame(rect, !(visual_rect_flags & kDontApplyMainFrameOverflowClip));
    ...
    if (LayoutBox* obj = owner->GetLayoutBox()) {
        ...
        return obj->MapToVisualRectInAncestorSpaceInternal(ancestor, transform_state, visual_rect_flags);
    }
...
void LayoutView::QuadsInAncestorInternal(Vector<gfx::QuadF>& quads, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    quads.push_back(LocalRectToAncestorQuad(PhysicalRect(PhysicalOffset(), GetScrollableArea()->Size()), ancestor, mode));
}
...
PhysicalRect LayoutView::ViewRect() const
{
    ...
    if (frame_view_->GetFrame().IsOutermostMainFrame()) {
        if (auto* transition = ViewTransitionUtils::GetTransition(GetDocument()); transition && transition->IsRootTransitioning()) {
            ...
            // HTML if this occurs.
            CHECK_GE(transition->GetSnapshotRootSize().width(), frame_view_->Size().width());
            CHECK_GE(transition->GetSnapshotRootSize().height(), frame_view_->Size().height());

            return PhysicalRect(PhysicalOffset(transition->GetFrameToSnapshotRootOffset()), PhysicalSize(transition->GetSnapshotRootSize()));
        }
    }
...
PhysicalRect LayoutView::OverflowClipRect(const PhysicalOffset& location, OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior) const
{
    NOT_DESTROYED();
    PhysicalRect rect = ViewRect();
    if (rect.IsEmpty()) {
        return LayoutBox::OverflowClipRect(location, overlay_scrollbar_clip_behavior);
    }
...
void LayoutView::SetAutosizeScrollbarModes(mojom::blink::ScrollbarMode h_mode, mojom::blink::ScrollbarMode v_mode)
{
    NOT_DESTROYED();
    DCHECK_EQ(v_mode == mojom::blink::ScrollbarMode::kAuto, h_mode == mojom::blink::ScrollbarMode::kAuto);
    autosize_v_scrollbar_mode_ = v_mode;
    autosize_h_scrollbar_mode_ = h_mode;
}

void LayoutView::CalculateScrollbarModes(mojom::blink::ScrollbarMode& h_mode, mojom::blink::ScrollbarMode& v_mode) const
{
    NOT_DESTROYED();
#define RETURN_SCROLLBAR_MODE(mode)                                                                                                                            \
    ...
    if (AutosizeVerticalScrollbarMode() != mojom::blink::ScrollbarMode::kAuto || AutosizeHorizontalScrollbarMode() != mojom::blink::ScrollbarMode::kAuto) {
    ...
    if (!frame->ClipsContent()) {
        ...
        if (Platform::Current()->IsSynchronousCompositingEnabledForAndroidWebView() && !GetDocument().IsPrintingOrPaintingPreview()) {
    ...
    if (viewport->IsSVGRoot()) {
        ...
        if (To<LayoutSVGRoot>(viewport)->IsEmbeddedThroughFrameContainingSVGDocument()) {
            RETURN_SCROLLBAR_MODE(mojom::blink::ScrollbarMode::kAlwaysOff);
        }
    }
    ...
    if (Settings* settings = document.GetSettings()) {
        if (settings->GetIgnoreMainFrameOverflowHiddenQuirk() && frame->IsMainFrame())
...
gfx::Size LayoutView::GetLayoutSize(IncludeScrollbarsInRect scrollbar_inclusion) const
{
    NOT_DESTROYED();
    if (GetDocument().Printing()) {
...
gfx::Size LayoutView::GetNonPrintingLayoutSize(IncludeScrollbarsInRect scrollbar_inclusion) const
{
    NOT_DESTROYED();
    if (!frame_view_)
        return gfx::Size();
    ...
    if (scrollbar_inclusion == kExcludeScrollbars && frame_view_->LayoutViewport()) {
        result = frame_view_->LayoutViewport()->ExcludeScrollbars(result);
    }
    return result;
}

int LayoutView::ViewLogicalWidth(IncludeScrollbarsInRect scrollbar_inclusion) const
{
    NOT_DESTROYED();
    return StyleRef().IsHorizontalWritingMode() ? ViewWidth(scrollbar_inclusion) : ViewHeight(scrollbar_inclusion);
}

int LayoutView::ViewLogicalHeight(IncludeScrollbarsInRect scrollbar_inclusion) const
{
    NOT_DESTROYED();
    return StyleRef().IsHorizontalWritingMode() ? ViewHeight(scrollbar_inclusion) : ViewWidth(scrollbar_inclusion);
}
...
void LayoutView::InvalidateSvgRootsWithRelativeLengthDescendents()
{
    if (GetDocument().SvgExtensions() && !ShouldUsePaginatedLayout()) {
        GetDocument().AccessSVGExtensions().InvalidateSVGRootsWithRelativeLengthDescendents();
    }
}
...
void LayoutView::LayoutRoot()
{
    ...
    FontCache::SetDeviceScaleFactor(chrome_client.GetScreenInfo(frame).device_scale_factor);
#endif

    bool is_resizing_initial_containing_block = LogicalWidth() != ViewLogicalWidthForBoxSizing() || LogicalHeight() != ViewLogicalHeightForBoxSizing();
    ...
    if (is_resizing_initial_containing_block) {
        ...
        initial_containing_block_resize_handled_list_ = MakeGarbageCollected<HeapHashSet<Member<const LayoutObject>>>();
    }
...
void LayoutView::UpdateHitTestResult(HitTestResult& result, const PhysicalOffset& point) const
{
    NOT_DESTROYED();
    if (result.InnerNode())
        return;
...
bool LayoutView::BackgroundIsKnownToBeOpaqueInRect(const PhysicalRect&) const
{
    ...
    return GetFrame()->IsMainFrame() && frame_view_->BaseBackgroundColor().IsOpaque();
}

gfx::SizeF LayoutView::SmallViewportSizeForViewportUnits() const
{
    NOT_DESTROYED();
    return GetFrameView() ? GetFrameView()->SmallViewportSizeForViewportUnits() : gfx::SizeF();
}

gfx::SizeF LayoutView::LargeViewportSizeForViewportUnits() const
{
    NOT_DESTROYED();
    return GetFrameView() ? GetFrameView()->LargeViewportSizeForViewportUnits() : gfx::SizeF();
}

gfx::SizeF LayoutView::DynamicViewportSizeForViewportUnits() const
{
    NOT_DESTROYED();
    return GetFrameView() ? GetFrameView()->DynamicViewportSizeForViewportUnits() : gfx::SizeF();
}

gfx::SizeF LayoutView::DefaultPageAreaSize() const
{
    NOT_DESTROYED();
    const WebPrintPageDescription& default_page_description = frame_view_->GetFrame().GetPrintParams().default_page_description;
    return gfx::SizeF(std::max(.0f, default_page_description.size.width() - (default_page_description.margin_left + default_page_description.margin_right)),
        std::max(.0f, default_page_description.size.height() - (default_page_description.margin_top + default_page_description.margin_bottom)));
}
...
void LayoutView::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    if (frame.IsMainFrame() && visual_viewport.IsActiveViewport()) {
        ...
        // color scheme.
        if (!old_style || old_style->UsedColorScheme() != visual_viewport.UsedColorSchemeScrollbars()) {
            visual_viewport.UsedColorSchemeChanged();
        }
        if (old_style && old_style->ScrollbarThumbColorResolved() != visual_viewport.CSSScrollbarThumbColor()) {
...
{
    NOT_DESTROYED();
    return PhysicalRect(gfx::Rect(0, 0, ViewWidth(kIncludeScrollbars), ViewHeight(kIncludeScrollbars)));
}
...
bool LayoutView::AffectedByResizedInitialContainingBlock(const LayoutResult& layout_result)
{
    NOT_DESTROYED();
    if (!initial_containing_block_resize_handled_list_) {
        return false;
    }
    const LayoutObject* layout_object = layout_result.GetPhysicalFragment().GetLayoutObject();
    DCHECK(layout_object);
    auto add_result = initial_containing_block_resize_handled_list_->insert(layout_object);
...
void LayoutView::UpdateCountersAfterStyleChange(LayoutObject* container)
{
    ...
    DCHECK(!container || (container->View() == this && container->IsDescendantOf(this) && GetDocument().GetStyleEngine().InContainerQueryStyleRecalc()))
    ...
    LayoutObject* stay_within = container && container->ShouldApplyStyleContainment() ? container : nullptr;

    for (LayoutObject* layout_object = start; layout_object; layout_object = layout_object->NextInPreOrder(stay_within)) {
