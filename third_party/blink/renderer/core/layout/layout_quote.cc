...
void LayoutQuote::WillBeDestroyed()
{
    NOT_DESTROYED();
    if (scope_) {
        GetDocument().GetStyleEngine().EnsureStyleContainmentScopeTree().UpdateOutermostQuotesDirtyScope(scope_);
...
void LayoutQuote::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
...
{
    DEFINE_STATIC_REF(QuotesData, static_basic_quotes, (QuotesData::Create(0x201c, 0x201d, 0x2018, 0x2019)));
...
void LayoutQuote::UpdateText()
{
    ...
    if (fragment) {
        fragment->SetStyle(IsA<LayoutTextCombine>(fragment->Parent()) ? fragment->Parent()->Style() : Style());
