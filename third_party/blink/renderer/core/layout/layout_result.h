...
public:
    ...
    static const LayoutResult* CloneWithPostLayoutFragments(const LayoutResult& other);
    ...
    LayoutResult(const LayoutResult& other, const ConstraintSpace& new_space, const MarginStrut& new_end_margin_strut, LayoutUnit bfc_line_offset,
    ...
    LayoutResult(const LayoutResult& other, const PhysicalFragment* physical_fragment);

    // Delegate constructor that sets up what it can, based on the builder.
    LayoutResult(const PhysicalFragment* physical_fragment, FragmentBuilder* builder);
    ...
    LogicalOffset OutOfFlowPositionedOffset() const
    {
        ...
        return rare_data_ && rare_data_->oof_positioned_offset_is_set() ? rare_data_->OutOfFlowPositionedOffset()
    ...
    {
        return rare_data_ && rare_data_->needs_anchor_position_scroll_adjustment_in_x();
    }
    bool NeedsAnchorPositionScrollAdjustmentInY() const
    {
        return rare_data_ && rare_data_->needs_anchor_position_scroll_adjustment_in_y();
    }
    ...
    std::optional<LayoutUnit> LineBoxBfcBlockOffset() const
    {
        ...
        if (rare_data_) {
            if (std::optional<LayoutUnit> offset = rare_data_->LineBoxBfcBlockOffset()) {
                return offset;
            }
        }
    ...
    {
        if (!rare_data_ || space_.IsInitialColumnBalancingPass() || rare_data_->minimal_space_shortage == kIndefiniteSize) {
            return std::nullopt;
        }
    ...
    {
        if (!rare_data_ || !space_.IsInitialColumnBalancingPass() || rare_data_->tallest_unbreakable_block_size == kIndefiniteSize) {
            return LayoutUnit();
        }
    ...
    bool HasForcedBreak() const
    {
        return bitfields_.has_forced_break;
    }

    // Returns true if the fragment should be considered empty for margin
    // collapsing purposes (e.g. margins "collapse through").
    bool IsSelfCollapsing() const
    {
        return bitfields_.is_self_collapsing;
    }
    ...
    {
        return bitfields_.is_pushed_by_floats;
    }
    ...
    protected:
        ...
        void SetOutOfFlowInsetsForGetComputedStyle(const BoxStrut& insets)
        {
            ...
            DCHECK_EQ(layout_result_->BfcBlockOffset().value_or(LayoutUnit()), LayoutUnit());
        ...
        void SetOutOfFlowPositionedOffset(const LogicalOffset& offset)
        {
            ...
            if ((layout_result_->rare_data_ && layout_result_->rare_data_->oof_positioned_offset_is_set())
                || offset != layout_result_->oof_insets_for_get_computed_style_.StartOffset()) {
                layout_result_->EnsureRareData()->SetOutOfFlowPositionedOffset(offset);
            }
        }

        void SetNeedsScrollAdjustment(bool needs_scroll_adjustment_in_x, bool needs_scroll_adjustment_in_y)
        {
            if (!needs_scroll_adjustment_in_x && !needs_scroll_adjustment_in_y) {
                return;
            }
            layout_result_->EnsureRareData()->set_needs_anchor_position_scroll_adjustment_in_x(needs_scroll_adjustment_in_x);
            layout_result_->EnsureRareData()->set_needs_anchor_position_scroll_adjustment_in_y(needs_scroll_adjustment_in_y);
        }

        void SetNonOverflowingScrollRanges(const HeapVector<NonOverflowingScrollRange>& non_overflowing_ranges)
        {
            if (layout_result_->rare_data_ || !non_overflowing_ranges.empty()) {
                layout_result_->EnsureRareData()->SetNonOverflowingScrollRanges(non_overflowing_ranges);
            }
        }

        void SetAccessibilityAnchor(Element* anchor);

        void SetDisplayLocksAffectedByAnchors(HeapHashSet<Member<Element>>* display_locks);
...
#if DCHECK_IS_ON()
    void CheckSameForSimplifiedLayout(const LayoutResult&, bool check_same_block_size = true, bool check_no_fragmentation = true) const;
#endif
    ...
    LayoutResult(BoxFragmentBuilderPassKey, const PhysicalFragment* physical_fragment, BoxFragmentBuilder*);

    using LineBoxFragmentBuilderPassKey = base::PassKey<LineBoxFragmentBuilder>;
    // This constructor requires a non-null fragment and sets a success status.
    LayoutResult(LineBoxFragmentBuilderPassKey, const PhysicalFragment* physical_fragment, LineBoxFragmentBuilder*);
...
private:
    ...
    public:
        ...
        using OutOfFlowPositionedOffsetIsSetFlag = LineBoxBfcBlockOffsetIsSetFlag::DefineNextValue<bool, 1>;
        using NeedsAnchorPositionScrollAdjustmentInXFlag = OutOfFlowPositionedOffsetIsSetFlag::DefineNextValue<bool, 1>;
        using NeedsAnchorPositionScrollAdjustmentInYFlag = NeedsAnchorPositionScrollAdjustmentInXFlag::DefineNextValue<bool, 1>;
        using DataUnionTypeValue = NeedsAnchorPositionScrollAdjustmentInYFlag::DefineNextValue<uint8_t, 3>;
        using IsBlockEndTrimmableLineFlag = DataUnionTypeValue::DefineNextValue<bool, 1>;

        struct FlexData {
            FlexData() = default;
            FlexData(const FlexData& other)
            {
                flex_layout_data = std::make_unique<DevtoolsFlexInfo>(*other.flex_layout_data);
            }
        ...
        struct GridData {
            ...
            {
                grid_layout_data = std::make_unique<GridLayoutData>(*other.grid_layout_data);
            }
        ...
        {
            return static_cast<DataUnionType>(bit_field.get_concurrently<DataUnionTypeValue>());
        }
        ...
        template <typename DataType> DataType* EnsureData(DataType* address, DataUnionType data_type)
        {
        ...
        template <typename DataType> const DataType* GetData(const DataType* address, DataUnionType data_type) const
        {
        ...
        RareData()
            : bit_field(DataUnionTypeValue::encode(kNone))
        {
        }
        ...
        RareData(const RareData& rare_data)
            ...
            tallest_unbreakable_block_size(rare_data.tallest_unbreakable_block_size)
            ...
            , non_overflowing_scroll_ranges(rare_data.non_overflowing_scroll_ranges)
    ...
    public:
        ...
        Bitfields(bool is_self_collapsing, bool is_pushed_by_floats, AdjoiningObjectTypes adjoining_object_types,
            bool has_descendant_that_depends_on_percentage_block_size, bool subtree_modified_margin_strut)
