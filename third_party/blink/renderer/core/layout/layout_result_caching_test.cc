...
protected:
    ...
    const LayoutResult* TestCachedLayoutResultWithBreakToken(LayoutBox* box, const ConstraintSpace& constraint_space, const BlockBreakToken* break_token)
    {
        std::optional<FragmentGeometry> fragment_geometry;
        LayoutCacheStatus cache_status;
        return box->CachedLayoutResult(constraint_space, break_token, nullptr, nullptr, &fragment_geometry, &cache_status);
    }

    const LayoutResult* TestCachedLayoutResult(LayoutBox* box, const ConstraintSpace& constraint_space, LayoutCacheStatus* out_cache_status = nullptr)
    {
        std::optional<FragmentGeometry> fragment_geometry;
        LayoutCacheStatus cache_status;
        const LayoutResult* result = box->CachedLayoutResult(constraint_space, nullptr, nullptr, nullptr, &fragment_geometry, &cache_status);
        if (out_cache_status) {
...
TEST_F(LayoutResultCachingTest, HitDifferentExclusionSpace)
{
    ...
    LayoutCacheStatus cache_status;
    const ConstraintSpace& space = src->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
    const LayoutResult* result = TestCachedLayoutResult(test, space, &cache_status);
...
TEST_F(LayoutResultCachingTest, HitDifferentBFCOffset)
{
    ...
    LayoutOpportunityVector opportunities = result->GetExclusionSpace().AllLayoutOpportunities(
        /* offset */ { LayoutUnit(), LayoutUnit() },
        /* available_inline_size */ LayoutUnit(100));

    EXPECT_EQ(opportunities.size(), 3u);

    EXPECT_EQ(opportunities[0].rect.start_offset, BfcOffset(LayoutUnit(50), LayoutUnit()));
    EXPECT_EQ(opportunities[0].rect.end_offset, BfcOffset(LayoutUnit(100), LayoutUnit::Max()));

    EXPECT_EQ(opportunities[1].rect.start_offset, BfcOffset(LayoutUnit(), LayoutUnit(20)));
    EXPECT_EQ(opportunities[1].rect.end_offset, BfcOffset(LayoutUnit(100), LayoutUnit(45)));

    EXPECT_EQ(opportunities[2].rect.start_offset, BfcOffset(LayoutUnit(), LayoutUnit(65)));
    EXPECT_EQ(opportunities[2].rect.end_offset, BfcOffset(LayoutUnit(100), LayoutUnit::Max()));
}
...
TEST_F(LayoutResultCachingTest, HitShrinkToFit)
{
    ...
    LayoutCacheStatus cache_status;
    ConstraintSpace space = src1->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
    const LayoutResult* result = TestCachedLayoutResult(test1, space, &cache_status);
...
TEST_F(LayoutResultCachingTest, OptimisticFloatPlacementNoRelayout)
{
    ...
    ConstraintSpace space = empty->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
...
TEST_F(LayoutResultCachingTest, HitFlexDefiniteChange)
{
    ...
    const LayoutResult* measure1 = target1->GetSingleCachedMeasureResultForTesting();
    EXPECT_EQ(measure1->IntrinsicBlockSize(), 100);
    EXPECT_EQ(result1->GetPhysicalFragment().Size().height, 200);

    EXPECT_EQ(result1->GetConstraintSpaceForCaching().CacheSlot(), LayoutResultCacheSlot::kMeasure);
...
TEST_F(LayoutResultCachingTest, HitOrthogonalRoot)
{
    ...
    LayoutCacheStatus cache_status;
    const ConstraintSpace& space = target->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
    const LayoutResult* result = TestCachedLayoutResult(target, space, &cache_status);
...
TEST_F(LayoutResultCachingTest, SimpleTable)
{
    ...
    const LayoutResult* measure2 = target2->GetSingleCachedMeasureResultForTesting();
    EXPECT_EQ(result2->GetConstraintSpaceForCaching().CacheSlot(), LayoutResultCacheSlot::kMeasure);
...
TEST_F(LayoutResultCachingTest, MissTableCellMiddleAlignment)
{
    ...
    const LayoutResult* measure = target->GetSingleCachedMeasureResultForTesting();
    EXPECT_NE(measure, nullptr);
    EXPECT_NE(result, nullptr);
    EXPECT_EQ(measure->GetConstraintSpaceForCaching().CacheSlot(), LayoutResultCacheSlot::kMeasure);
    EXPECT_EQ(result->GetConstraintSpaceForCaching().CacheSlot(), LayoutResultCacheSlot::kLayout);
...
TEST_F(LayoutResultCachingTest, HitTableCellBaselineAlignment)
{
    ...
    const LayoutResult* result = target->GetSingleCachedLayoutResult();
    const LayoutResult* measure = target->GetSingleCachedMeasureResultForTesting();
    EXPECT_EQ(result->GetConstraintSpaceForCaching().CacheSlot(), LayoutResultCacheSlot::kMeasure);
    EXPECT_NE(result, nullptr);
...
TEST_F(LayoutResultCachingTest, FragmentainerSizeChange)
{
    ...
    const ConstraintSpace& test_space1 = test_result1->GetConstraintSpaceForCaching();
    const auto* test_break_token1 = To<BlockBreakToken>(test_result1->GetPhysicalFragment().GetBreakToken());
    ASSERT_TRUE(test_break_token1);
    const LayoutResult* test_result2 = test->GetCachedLayoutResult(test_break_token1);
    ASSERT_TRUE(test_result2);
    const ConstraintSpace& test_space2 = test_result2->GetConstraintSpaceForCaching();
    const auto* test_break_token2 = To<BlockBreakToken>(test_result2->GetPhysicalFragment().GetBreakToken());
    ASSERT_TRUE(test_break_token2);
    const LayoutResult* test_result3 = test->GetCachedLayoutResult(test_break_token2);
    ASSERT_TRUE(test_result3);
    const ConstraintSpace& test_space3 = test_result3->GetConstraintSpaceForCaching();
    ...
    const ConstraintSpace& src_space1 = src_result1->GetConstraintSpaceForCaching();
    const auto* src_break_token1 = To<BlockBreakToken>(src_result1->GetPhysicalFragment().GetBreakToken());
    ASSERT_TRUE(src_break_token1);
    const LayoutResult* src_result2 = src->GetCachedLayoutResult(src_break_token1);
    ASSERT_TRUE(src_result2);
    const ConstraintSpace& src_space2 = src_result2->GetConstraintSpaceForCaching();
    const auto* src_break_token2 = To<BlockBreakToken>(src_result2->GetPhysicalFragment().GetBreakToken());
    ASSERT_TRUE(src_break_token2);
    const LayoutResult* src_result3 = src->GetCachedLayoutResult(src_break_token2);
    ASSERT_TRUE(src_result3);
    const ConstraintSpace& src_space3 = src_result3->GetConstraintSpaceForCaching();
    ...
    EXPECT_TRUE(TestCachedLayoutResultWithBreakToken(src, src_space2, src_break_token1));
    EXPECT_TRUE(TestCachedLayoutResultWithBreakToken(src, src_space3, src_break_token2));
    ...
    EXPECT_FALSE(TestCachedLayoutResultWithBreakToken(src, test_space2, test_break_token1));
    EXPECT_FALSE(TestCachedLayoutResultWithBreakToken(src, test_space3, test_break_token2));
}
...
TEST_F(LayoutResultCachingTest, BlockOffsetChangeInFragmentainer)
{
    ...
    const ConstraintSpace& test1_space = test1->GetCachedLayoutResult(nullptr)->GetConstraintSpaceForCaching();
    const ConstraintSpace& test2_space = test2->GetCachedLayoutResult(nullptr)->GetConstraintSpaceForCaching();
    const ConstraintSpace& test3_space = test3->GetCachedLayoutResult(nullptr)->GetConstraintSpaceForCaching();
...
TEST_F(LayoutResultCachingTest, MissMonolithicChangeInFragmentainer)
{
    ...
    auto* test = To<LayoutBlockFlow>(GetLayoutObjectByElementId("test"));
    auto* src = To<LayoutBlockFlow>(GetLayoutObjectByElementId("src"));
    const ConstraintSpace& src_space = src->GetCachedLayoutResult(nullptr)->GetConstraintSpaceForCaching();
    const ConstraintSpace& test_space = test->GetCachedLayoutResult(nullptr)->GetConstraintSpaceForCaching();
...
TEST_F(LayoutResultCachingTest, MissGridIncorrectIntrinsicSize)
{
    ...
    LayoutCacheStatus cache_status;
    ConstraintSpace space = src->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
    const LayoutResult* result = TestCachedLayoutResult(test, space, &cache_status);
