...
public:
    BoxFragmentBuilder(LayoutInputNode node, const ComputedStyle* style, const ConstraintSpace& space, WritingDirectionMode writing_direction,
    ...
    BoxFragmentBuilder(LayoutObject* layout_object, const ComputedStyle* style, const ConstraintSpace& space, WritingDirectionMode writing_direction)
        : FragmentBuilder(/*node=*/nullptr, std::move(style), space, writing_direction,
    ...
    void SetInitialFragmentGeometry(const FragmentGeometry& initial_fragment_geometry)
    {
        ...
        border_padding_ = initial_fragment_geometry.border + initial_fragment_geometry.padding;

        // Box decorations don't take up layout space in table rows / sections.
        if (!node_ || (!node_.IsTableSection() && !node_.IsTableRow())) {
            border_scrollbar_padding_ = initial_fragment_geometry.border + initial_fragment_geometry.scrollbar;
        ...
        original_border_scrollbar_padding_block_start_ = border_scrollbar_padding_.block_start;
        if (node_) {
            child_available_size_ = CalculateChildAvailableSize(space_, To<BlockNode>(node_), size_, border_padding_ + initial_fragment_geometry.scrollbar);
        }
    }
    ...
    {
#if DCHECK_IS_ON()
        DCHECK(!block_size_is_for_all_fragments_ || !has_block_fragmentation_ || IsInitialColumnBalancingPass());
        DCHECK(size_.block_size != kIndefiniteSize);
#endif
        return size_.block_size;
    }
    ...
    LayoutUnit IntrinsicBlockSize() const
    {
    ...
    BoxStrut ExcludedSidesTruncated(const BoxStrut& strut) const
    {
        ...
        return BoxStrut(strut.inline_start, strut.inline_end, sides_to_include_.block_start ? strut.block_start : LayoutUnit(),
    ...
    void AddBreakBeforeChild(LayoutInputNode child, std::optional<BreakAppeal> appeal, bool is_forced_break);
    ...
    // non-atomic inline.
    void AddResult(const LayoutResult&, const LogicalOffset, std::optional<const BoxStrut> margins, std::optional<LogicalOffset> relative_offset = std::nullopt,
        const OofInlineContainer<LogicalOffset>* inline_container = nullptr);
    // AddResult() with the default margin computation.
    void AddResult(const LayoutResult& child_layout_result, const LogicalOffset offset);
    ...
    void AddChild(const PhysicalFragment&, const LogicalOffset&, const MarginStrut* margin_strut = nullptr, bool is_self_collapsing = false,
        std::optional<LogicalOffset> relative_offset = std::nullopt, const OofInlineContainer<LogicalOffset>* inline_container = nullptr);
    ...
    void SetIsFirstForNode(bool is_first)
    {
        is_first_for_node_ = is_first;
    }
    ...
    void ReserveSpaceForMonolithicOverflow(LayoutUnit monolithic_overflow)
    {
        ...
        data->monolithic_overflow = std::max(data->monolithic_overflow, monolithic_overflow);
    }
    ...
    bool DidBreakSelf() const
    {
        return did_break_self_;
    }
    void SetDidBreakSelf()
    {
        did_break_self_ = true;
    }
    ...
    bool HasInsertedChildBreak() const
    {
        ...
        for (auto& child_token : child_break_tokens_) {
            const auto* block_child_token = DynamicTo<BlockBreakToken>(child_token.Get());
    ...
    EBreakBetween PreviousBreakAfter() const
    {
        return previous_break_after_;
    }
    ...
    // left to discover.
    void SetHasSeenAllChildren()
    {
        has_seen_all_children_ = true;
    }
    bool HasSeenAllChildren()
    {
    ...
    {
        has_subsequent_children_ = true;
    }

    void SetIsAtBlockEnd()
    {
        is_at_block_end_ = true;
    }
    ...
    void SetIsMathMLFraction()
    {
        is_math_fraction_ = true;
    }
    void SetIsMathMLOperator()
    {
        is_math_operator_ = true;
    }
    ...
    void SetFirstBaseline(LayoutUnit baseline)
    {
    ...
    void SetTableColumnGeometries(const TableFragmentData::ColumnGeometries& table_column_geometries)
    {
    ...
    void SetTableCollapsedBordersGeometry(std::unique_ptr<TableFragmentData::CollapsedBordersGeometry> table_collapsed_borders_geometry)
    {
        table_collapsed_borders_geometry_ = std::move(table_collapsed_borders_geometry);
    }
    ...
    void SetTableSectionCollapsedBordersGeometry(wtf_size_t start_row_index, Vector<LayoutUnit>&& row_offsets)
    {
        table_section_start_row_index_ = start_row_index;
        table_section_row_offsets_ = std::move(row_offsets);
    }

    void TransferGridLayoutData(std::unique_ptr<GridLayoutData> grid_layout_data)
    {
        grid_layout_data_ = std::move(grid_layout_data);
    }
    void TransferFlexLayoutData(std::unique_ptr<DevtoolsFlexInfo> flex_layout_data)
    {
    ...
    BlockBreakTokenData* GetBreakTokenData()
    {
        return break_token_data_;
    }
    ...
    void AdjustFragmentainerDescendant(LogicalOofNodeForFragmentation& descendant, bool only_fixedpos_containing_block = false);
    ...
    bool ShouldCalculateScrollableOverflow() const
    {
        ...
        return node_ && !node_.IsReplaced() && (!node_.IsPaginatedRoot() || IsFragmentainerBoxType());
    }
...
private:
    ...
    std::unique_ptr<TableFragmentData::CollapsedBordersGeometry> table_collapsed_borders_geometry_;
