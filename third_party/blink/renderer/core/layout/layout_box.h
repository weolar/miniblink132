...
enum ShouldIncludeScrollbarGutter { kExcludeScrollbarGutter, kIncludeScrollbarGutter };
...
public:
    ...
    bool BackgroundIsKnownToBeOpaqueInRect(const PhysicalRect& local_rect) const override;
    ...
    {
        NOT_DESTROYED();
        return PhysicalRect(ClientLeft(), ClientTop(), ClientWidth(), ClientHeight());
    }
    ...
    {
        NOT_DESTROYED();
        return PhysicalRect(ContentLeft(), ContentTop(), ContentWidth(), ContentHeight());
    }
    // TODO(crbug.com/877518): Some callers of this method may actually want
    // "physical coordinates in flipped block-flow direction".
    ...
    {
        NOT_DESTROYED();
        return PhysicalRect(BorderLeft() + ComputedCSSPaddingLeft(), BorderTop() + ComputedCSSPaddingTop(),
            ClientWidth() - ComputedCSSPaddingLeft() - ComputedCSSPaddingRight(), ClientHeight() - ComputedCSSPaddingTop() - ComputedCSSPaddingBottom());
    }

    void AddOutlineRects(OutlineRectCollector&, OutlineInfo*, const PhysicalOffset& additional_offset, OutlineType) const override;
    ...
    PhysicalRect ScrollableOverflowRect() const
    {
        ...
        return ScrollableOverflowIsSet() ? overflow_->scrollable_overflow->ScrollableOverflowRect() : NoOverflowRect();
    }
    ...
    {
        NOT_DESTROYED();
        return VisualOverflowIsSet() ? overflow_->visual_overflow->SelfVisualOverflowRect() : PhysicalBorderBoxRect();
    }
    PhysicalRect ContentsVisualOverflowRect() const
    {
        NOT_DESTROYED();
        return VisualOverflowIsSet() ? overflow_->visual_overflow->ContentsVisualOverflowRect() : PhysicalRect();
    }
    ...
    DISABLE_CFI_PERF LayoutUnit ContentWidth() const
    {
        ...
        return (ClientWidth() - PaddingLeft() - PaddingRight()).ClampNegativeToZero();
    }
    DISABLE_CFI_PERF LayoutUnit ContentHeight() const
    {
        ...
        // subtract two padding values from that result, which is another source of
        // saturated arithmetic.
        return (ClientHeight() - PaddingTop() - PaddingBottom()).ClampNegativeToZero();
    }
    ...
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? ContentWidth() : ContentHeight();
    }
    LayoutUnit ContentLogicalHeight() const
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? ContentHeight() : ContentWidth();
    }
    ...
    void QuadsInAncestorInternal(Vector<gfx::QuadF>&, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags) const override;
    ...
    bool HitTestAllPhases(HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset) final;
    bool NodeAtPoint(HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset, HitTestPhase) override;
    bool HasHitTestableOverflow() const;
    // Fast check if |NodeAtPoint| may find a hit.
    bool MayIntersect(const HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset) const;
    ...
    const LayoutResult* GetCachedMeasureResult(const ConstraintSpace&, std::optional<FragmentGeometry>* fragment_geometry) const;
    ...
    const LayoutResult* CachedLayoutResult(const ConstraintSpace&, const BlockBreakToken*, const EarlyBreak*, const ColumnSpannerPath*,
        std::optional<FragmentGeometry>* initial_fragment_geometry, LayoutCacheStatus* out_cache_status);
    ...
    public:
        ...
        {
            return Iterator(layout_results_.begin());
        }
        Iterator end() const
        {
            return Iterator(layout_results_.end());
        }
    ...
    const FragmentData* FragmentDataFromPhysicalFragment(const PhysicalBoxFragment&) const;
    ...
    bool MapToVisualRectInAncestorSpaceInternal(
        const LayoutBoxModelObject* ancestor, TransformState&, VisualRectFlags = kDefaultVisualRectFlags) const override;
    ...
    inline BoxStrut ComputeLogicalScrollbars() const
    {
        ...
        if (CanSkipComputeScrollbars()) {
            ...
            return ComputeScrollbarsInternal().ConvertToLogical(StyleRef().GetWritingDirection());
        }
    }

    bool IsUserScrollable() const;
    virtual void Autoscroll(const PhysicalOffset&);
    PhysicalOffset CalculateAutoscrollDirection(const gfx::PointF& point_in_root_frame) const;
    static LayoutBox* FindAutoscrollable(LayoutObject*, bool is_middle_click_autoscroll);
    ...
    virtual PhysicalRect OverflowClipRect(const PhysicalOffset& location, OverlayScrollbarClipBehavior = kIgnoreOverlayScrollbarSize) const;
    ...
    {
        NOT_DESTROYED();
        return !Parent() || Parent()->StyleRef().GetWritingMode() != StyleRef().GetWritingMode();
    }
    ...
    {
        NOT_DESTROYED();
        return !IsInline() && !IsOutOfFlowPositioned() && Parent() && Parent()->IsFlexibleBox();
    }
    ...
    [[nodiscard]] LayoutUnit FlipForWritingMode(LayoutUnit position, LayoutUnit width = LayoutUnit()) const
    {
        NOT_DESTROYED();
    ...
    PhysicalOffset PhysicalLocation(const LayoutBox* flipped_blocks_container = nullptr) const
    {
        NOT_DESTROYED();
        return PhysicalLocationInternal(flipped_blocks_container ? flipped_blocks_container : LocationContainer());
    }

    bool HasSelfVisualOverflow() const
    {
        NOT_DESTROYED();
        return VisualOverflowIsSet() && !PhysicalBorderBoxRect().Contains(overflow_->visual_overflow->SelfVisualOverflowRect());
    }
    ...
    bool MapContentsRectToBoxSpace(
        TransformState&, TransformState::TransformAccumulation, const LayoutObject& contents, VisualRectFlags = kDefaultVisualRectFlags) const;
    ...
    bool ApplyBoxClips(TransformState&, TransformState::TransformAccumulation, VisualRectFlags) const;
    ...
    // perspective).
    bool MapVisualRectToContainer(
        const LayoutObject* container_object, const PhysicalOffset& container_offset, const LayoutObject* ancestor, VisualRectFlags, TransformState&) const;
    ...
    bool HitTestClippedOutByBorder(const HitTestLocation&, const PhysicalOffset& border_box_location) const;

    bool HitTestOverflowControl(HitTestResult&, const HitTestLocation&, const PhysicalOffset&) const;
    ...
    public:
        ...
        void ClearPreviousSize()
        {
            GetLayoutBox().previous_size_ = PhysicalSize();
        }
        ...
        void SavePreviousContentBoxRect()
        {
            ...
            rare_data.previous_physical_content_box_rect_ = GetLayoutBox().PhysicalContentBoxRect();
        }
        ...
        // original LayoutBox.
        void SetPreviousGeometryForLayoutShiftTracking(const PhysicalOffset& paint_offset, const PhysicalSize& size, const PhysicalRect& visual_overflow_rect);
    ...
    {
        NOT_DESTROYED();
        return rare_data_ && rare_data_->has_previous_content_box_rect_ ? rare_data_->previous_physical_content_box_rect_
                                                                        : PhysicalRect(PhysicalOffset(), PreviousSize());
    }
    PhysicalRect PreviousVisualOverflowRect() const
    {
        NOT_DESTROYED();
        return overflow_ && overflow_->previous_overflow_data ? overflow_->previous_overflow_data->previous_visual_overflow_rect
                                                              : PhysicalRect(PhysicalOffset(), PreviousSize());
    }
    PhysicalRect PreviousScrollableOverflowRect() const
    {
        NOT_DESTROYED();
        return overflow_ && overflow_->previous_overflow_data ? overflow_->previous_overflow_data->previous_scrollable_overflow_rect
                                                              : PhysicalRect(PhysicalOffset(), PreviousSize());
    }
    PhysicalRect PreviousSelfVisualOverflowRect() const
    {
        NOT_DESTROYED();
        return overflow_ && overflow_->previous_overflow_data ? overflow_->previous_overflow_data->previous_self_visual_overflow_rect
                                                              : PhysicalRect(PhysicalOffset(), PreviousSize());
    }
    ...
    bool MayHaveFragmentItems() const
    {
        ...
        return (ChildrenInline() || NeedsLayout()) && PhysicalFragments().MayHaveFragmentItems();
    }
    bool HasFragmentItems() const
    {
        NOT_DESTROYED();
        // See `MayHaveFragmentItems()`.
        return (ChildrenInline() || NeedsLayout()) && PhysicalFragments().HasFragmentItems();
    }
    ...
    bool IsFixedToView(const LayoutObject* container_for_fixed_position = nullptr) const;
    ...
    const HeapVector<NonOverflowingScrollRange>* NonOverflowingScrollRanges() const;
    ...
    void NotifyContainingDisplayLocksForAnchorPositioning(const HeapHashSet<Member<Element>>* past_display_locks_affected_by_anchors,
        const HeapHashSet<Member<Element>>* display_locks_affected_by_anchors) const;
...
protected:
    ...
    virtual ItemPosition SelfAlignmentNormalBehavior(const LayoutBox* child = nullptr) const
    {
        NOT_DESTROYED();
        DCHECK(!child);
    ...
    virtual bool ForegroundIsKnownToBeOpaqueInRect(const PhysicalRect& local_rect, unsigned max_depth_to_test) const;
    virtual bool ComputeBackgroundIsKnownToBeObscured() const;
    bool ComputeCanCompositeBackgroundAttachmentFixed() const override;

    virtual bool HitTestChildren(HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset, HitTestPhase);

    void InvalidatePaint(const PaintInvalidatorContext&) const override;

    void ExcludeScrollbars(
        PhysicalRect&, OverlayScrollbarClipBehavior = kIgnoreOverlayScrollbarSize, ShouldIncludeScrollbarGutter = kIncludeScrollbarGutter) const;

    LayoutUnit ContainingBlockLogicalHeightForPositioned(const LayoutBoxModelObject* containing_block) const;

    static bool SkipContainingBlockForPercentHeightCalculation(const LayoutBox* containing_block);
    ...
    PhysicalOffset OffsetFromContainerInternal(const LayoutObject*, MapCoordinatesFlags mode) const override;
...
private:
    ...
    void SetVisualOverflow(const PhysicalRect& self, const PhysicalRect& contents);
    void CopyVisualOverflowFromFragmentsWithoutInvalidations();

    void UpdateShapeOutsideInfoAfterStyleChange(const ComputedStyle&, const ComputedStyle* old_style);
    ...
    void InflateVisualRectForFilterUnderContainer(TransformState&, const LayoutObject& container, const LayoutBoxModelObject* ancestor_to_stop_at) const;
    ...
    {
        NOT_DESTROYED();
        return (StyleRef().IsOverflowVisibleAlongBothAxes() || !HasNonVisibleOverflow()
                   || (GetScrollableArea() && !GetScrollableArea()->HasHorizontalScrollbar() && !GetScrollableArea()->HasVerticalScrollbar()))
            && StyleRef().IsScrollbarGutterAuto();
    }

    PhysicalBoxStrut ComputeScrollbarsInternal(ShouldClampToContentBox = kDoNotClampToContentBox, OverlayScrollbarClipBehavior = kIgnoreOverlayScrollbarSize,
        ShouldIncludeScrollbarGutter = kIncludeScrollbarGutter) const;

    LayoutUnit FlipForWritingModeInternal(LayoutUnit position, LayoutUnit width, const LayoutBox* box_for_flipping) const final
    {
        NOT_DESTROYED();
    ...
    PhysicalOffset PhysicalLocationInternal(const LayoutBox* container_box) const
    {
        NOT_DESTROYED();
        DCHECK_EQ(container_box, LocationContainer());
        LayoutPoint location = LocationInternal();
        if (!container_box || !container_box->HasFlippedBlocksWritingMode()) [[likely]] {
            return PhysicalOffset(location);
        }

        return PhysicalOffset(container_box->Size().width - Size().width - location.X(), location.Y());
    }
...
template <> struct DowncastTraits<LayoutBox> {
