...
public:
    void CreateAndSetImage(const char* id, int width, int height)
    {
        ...
        ImageResourceContent* image_content = ImageResourceContent::CreateLoaded(UnacceleratedStaticBitmapImage::Create(image).get());

        // Set image to video
        auto* video = To<HTMLVideoElement>(GetElementById(id));
        auto* layout_image = To<LayoutImage>(video->GetLayoutObject());
        video->setAttribute(html_names::kPosterAttr, AtomicString("http://example.com/foo.jpg"));
...
TEST_F(LayoutVideoTest, PosterSizeWithNormal)
{
    ...
    int width = To<LayoutBox>(GetLayoutObjectByElementId("video"))->AbsoluteBoundingBoxRect().width();
    EXPECT_EQ(width, 10);
}
...
TEST_F(LayoutVideoTest, DefaultPosterImageSize)
{
    ...
    GetDocument().GetSettings()->SetDefaultVideoPosterURL("https://www.example.com/foo.jpg");
