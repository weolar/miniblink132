...
#if DCHECK_IS_ON()
void AppendSubtreeToString(const BlockNode&, const LayoutInputNode* target, StringBuilder*, unsigned indent);

void IndentForDump(const LayoutInputNode& node, const LayoutInputNode* target, StringBuilder* string_builder, unsigned indent)
{
...
void AppendNodeToString(const LayoutInputNode& node, const LayoutInputNode* target, StringBuilder* string_builder, unsigned indent = 2)
{
    if (!node)
        return;
...
void AppendSubtreeToString(const BlockNode& node, const LayoutInputNode* target, StringBuilder* string_builder, unsigned indent)
{
    LayoutInputNode first_child = node.FirstChild();
    for (LayoutInputNode node_runner = first_child; node_runner; node_runner = node_runner.NextSibling()) {
...
{
    return box_->IsTableSection() && To<LayoutTableSection>(box_.Get())->IsEmpty();
}
...
void LayoutInputNode::IntrinsicSize(std::optional<LayoutUnit>* computed_inline_size, std::optional<LayoutUnit>* computed_block_size) const
{
    DCHECK(IsReplaced());
    ...
    std::optional<LayoutUnit> intrinsic_inline_size
        = legacy_sizing_info.has_width ? std::make_optional(LayoutUnit::FromFloatRound(legacy_sizing_info.size.width())) : std::nullopt;
    std::optional<LayoutUnit> intrinsic_block_size
        = legacy_sizing_info.has_height ? std::make_optional(LayoutUnit::FromFloatRound(legacy_sizing_info.size.height())) : std::nullopt;
...
{
    gfx::Size icb_size = GetDocument().GetLayoutView()->GetLayoutSize(kIncludeScrollbars);
...
{
    auto* inline_node = DynamicTo<InlineNode>(this);
    return inline_node ? inline_node->ToString() : To<BlockNode>(*this).ToString();
}
...
void LayoutInputNode::GetOverrideIntrinsicSize(std::optional<LayoutUnit>* computed_inline_size, std::optional<LayoutUnit>* computed_block_size) const
{
    DCHECK(IsReplaced());
