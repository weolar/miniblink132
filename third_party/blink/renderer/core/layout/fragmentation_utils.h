...
template <typename Property> bool IsAvoidBreakValue(const ConstraintSpace&, Property);
...
{
    return builder.GetConstraintSpace().HasBlockFragmentation() || IsBreakInside(builder.PreviousBreakToken());
}
...
// breakpoint.
EBreakBetween CalculateBreakBetweenValue(LayoutInputNode child, const LayoutResult&, const BoxFragmentBuilder&);
...
BreakAppeal CalculateBreakAppealBefore(
    const ConstraintSpace&, LayoutInputNode child, const LayoutResult&, const BoxFragmentBuilder&, bool has_container_separation);
BreakAppeal CalculateBreakAppealBefore(const ConstraintSpace&, LayoutResult::EStatus layout_result_status, EBreakBetween break_between,
...
BreakAppeal CalculateBreakAppealInside(const ConstraintSpace& space, const LayoutResult&, std::optional<BreakAppeal> hypothetical_appeal = std::nullopt);
...
inline LayoutUnit ClampedToValidFragmentainerCapacity(const BoxFragmentBuilder& builder, LayoutUnit length, bool is_for_children)
{
...
inline LayoutUnit FragmentainerCapacity(const BoxFragmentBuilder& builder, bool is_for_children)
{
...
// of them.
inline LayoutUnit FragmentainerOffset(const BoxFragmentBuilder& builder, bool is_for_children = true)
{
    const ConstraintSpace& space = builder.GetConstraintSpace();
...
// are included. Such box decorations will otherwise be subtracted, since
// children should steer clear of them.
inline LayoutUnit FragmentainerSpaceLeft(const BoxFragmentBuilder& builder, bool is_for_children)
{
    const ConstraintSpace& space = builder.GetConstraintSpace();
    if (!space.HasKnownFragmentainerBlockSize())
        return kIndefiniteSize;
...
{
    return FragmentainerOffset(builder) - builder.GetConstraintSpace().ExpectedBfcBlockOffset();
}
...
// ignored.
inline void AdjustMarginsForFragmentation(const BlockBreakToken* break_token, BoxStrut* box_strut)
{
    if (!break_token)
        return;
    ...
    if (!break_token->IsBreakBefore() || (!break_token->IsForcedBreak() && !break_token->InputNode().IsFloating()))
...
LogicalOffset GetFragmentainerProgression(const BoxFragmentBuilder&, FragmentationType);
...
void SetupSpaceBuilderForFragmentation(const ConstraintSpace& parent_space, const LayoutInputNode& child, LayoutUnit fragmentainer_offset,
    LayoutUnit fragmentainer_block_size, bool requires_content_before_breaking, ConstraintSpaceBuilder*);
...
void SetupSpaceBuilderForFragmentation(
    const BoxFragmentBuilder& parent_fragment_builder, const LayoutInputNode& child, LayoutUnit fragmentainer_offset_delta, ConstraintSpaceBuilder*);

// Set up a node's fragment builder for block fragmentation. To be done at the
// beginning of layout.
void SetupFragmentBuilderForFragmentation(const ConstraintSpace&, const LayoutInputNode&, const BlockBreakToken* previous_break_token, BoxFragmentBuilder*);
...
inline LayoutUnit ClonedBlockStartDecoration(const BoxFragmentBuilder& builder)
{
    const BlockBreakToken* break_token = builder.PreviousBreakToken();
    if (builder.Style().BoxDecorationBreak() == EBoxDecorationBreak::kClone && IsBreakInside(break_token) && !break_token->IsAtBlockEnd()) {
...
// the column level.
BreakStatus BreakBeforeChildIfNeeded(const ConstraintSpace&, LayoutInputNode child, const LayoutResult&, LayoutUnit fragmentainer_block_offset,
    LayoutUnit fragmentainer_block_size, bool has_container_separation, BoxFragmentBuilder*, bool is_row_item = false,
...
void BreakBeforeChild(const ConstraintSpace&, LayoutInputNode child, const LayoutResult*, LayoutUnit fragmentainer_block_offset,
    LayoutUnit fragmentainer_block_size, std::optional<BreakAppeal> appeal, bool is_forced_break, BoxFragmentBuilder*,
...
// possible.
inline void PropagateUnbreakableBlockSize(LayoutUnit block_size, LayoutUnit fragmentainer_block_offset, BoxFragmentBuilder* builder)
{
...
// layout result.
void PropagateSpaceShortage(const ConstraintSpace&, const LayoutResult*, LayoutUnit fragmentainer_block_offset, LayoutUnit fragmentainer_block_size,
    FragmentBuilder*, std::optional<LayoutUnit> block_size_override = std::nullopt);
...
// when you wish to propagate a different block-size than that of the provided
// layout result.
LayoutUnit CalculateSpaceShortage(const ConstraintSpace&, const LayoutResult*, LayoutUnit fragmentainer_block_offset, LayoutUnit fragmentainer_block_size,
    std::optional<LayoutUnit> block_size_override = std::nullopt);
// Update |minimal_space_shortage| based on the current |space_shortage|.
void UpdateMinimalSpaceShortage(std::optional<LayoutUnit> space_shortage, LayoutUnit* minimal_space_shortage);
...
bool MovePastBreakpoint(const ConstraintSpace& space, LayoutInputNode child, const LayoutResult& layout_result, LayoutUnit fragmentainer_block_offset,
    LayoutUnit fragmentainer_block_size, BreakAppeal appeal_before, BoxFragmentBuilder* builder, bool is_row_item = false,
    FlexColumnBreakInfo* flex_column_break_info = nullptr);
...
// details on |flex_column_break_info|.
void UpdateEarlyBreakAtBlockChild(const ConstraintSpace&, BlockNode child, const LayoutResult&, BreakAppeal appeal_before, BoxFragmentBuilder*,
    FlexColumnBreakInfo* flex_column_break_info = nullptr);
...
bool AttemptSoftBreak(const ConstraintSpace&, LayoutInputNode child, const LayoutResult*, LayoutUnit fragmentainer_block_offset,
    LayoutUnit fragmentainer_block_size, BreakAppeal appeal_before, BoxFragmentBuilder*, std::optional<LayoutUnit> block_size_override = std::nullopt,
    FlexColumnBreakInfo* flex_column_break_info = nullptr);
...
const EarlyBreak* EnterEarlyBreakInChild(const BlockNode& child, const EarlyBreak&);

// Return true if this is the child that we had previously determined to break
// before.
bool IsEarlyBreakTarget(const EarlyBreak&, const BoxFragmentBuilder&, const LayoutInputNode& child);
...
inline const ColumnSpannerPath* FollowColumnSpannerPath(const ColumnSpannerPath* path, const BlockNode& child)
{
    if (!path)
        return nullptr;
...
inline LayoutUnit AdjustedMarginAfterFinalChildFragment(const BoxFragmentBuilder& builder, LayoutUnit block_offset, LayoutUnit block_end_margin)
{
    LayoutUnit space_left = FragmentainerSpaceLeft(builder, /*is_for_children=*/true) - block_offset;
...
PhysicalOffset OffsetInStitchedFragments(const PhysicalBoxFragment&, PhysicalSize* out_stitched_fragments_size = nullptr);
...
LayoutUnit BlockSizeForFragmentation(const LayoutResult&, WritingDirectionMode container_writing_direction);
