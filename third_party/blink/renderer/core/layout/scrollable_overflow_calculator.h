...
public:
    static PhysicalRect RecalculateScrollableOverflowForFragment(const PhysicalBoxFragment&, bool has_block_fragmentation);

    ScrollableOverflowCalculator(const BlockNode&, bool is_css_box, bool has_block_fragmentation, const PhysicalBoxStrut& borders,
        const PhysicalBoxStrut& scrollbar, const PhysicalBoxStrut& padding, PhysicalSize size, WritingDirectionMode);
    ...
    void AddItems(const LayoutObject*, const FragmentItemsBuilder::ItemWithOffsetList&);

    void AddTableSelfRect();

private:
    template <typename Items> void AddItemsInternal(const LayoutObject* layout_object, const Items& items);

    PhysicalRect AdjustOverflowForHanging(const PhysicalRect& line_box_rect, PhysicalRect overflow);
