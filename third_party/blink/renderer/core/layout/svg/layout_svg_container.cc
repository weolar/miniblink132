...
SVGLayoutResult LayoutSVGContainer::UpdateSVGLayout(const SVGLayoutInfo& layout_info)
{
    NOT_DESTROYED();
    DCHECK(NeedsLayout());
    ...
    child_layout_info.scale_factor_changed |= transform_change == SVGTransformChange::kFull;
    ...
    if (UpdateAfterSVGLayout(layout_info, transform_change, content_result.bounds_changed)) {
        result.bounds_changed = true;
    }
...
bool LayoutSVGContainer::UpdateAfterSVGLayout(const SVGLayoutInfo& layout_info, SVGTransformChange transform_change, bool bbox_changed)
{
    ...
    if (needs_transform_update_) {
        ...
        transform_change = std::max(UpdateLocalTransform(reference_box), transform_change);
        needs_transform_update_ = false;
    }

    // Reset the viewport dependency flag based on the state for this container.
    TransformHelper::UpdateReferenceBoxDependency(*this, transform_uses_reference_box_);
    ...
    if (!IsSVGHiddenContainer()) {
        ...
        for (auto* child = FirstChild(); child; child = child->NextSibling()) {
            ...
            if (child->StyleRef().HasCurrentTransformRelatedAnimation() || child->SVGDescendantMayHaveTransformRelatedAnimation()) {
...
void LayoutSVGContainer::AddChild(LayoutObject* child, LayoutObject* before_child)
{
    NOT_DESTROYED();
...
void LayoutSVGContainer::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    const bool had_isolation = old_style && SVGLayoutSupport::WillIsolateBlendingDescendantsForStyle(*old_style);
    const bool will_isolate_blending_descendants = SVGLayoutSupport::WillIsolateBlendingDescendantsForStyle(StyleRef());
    const bool isolation_changed = had_isolation != will_isolate_blending_descendants;
    ...
    if (isolation_changed) {
        ...
        if (Parent() && HasNonIsolatedBlendingDescendants()) {
            Parent()->DescendantIsolationRequirementsChanged(
                will_isolate_blending_descendants ? kDescendantIsolationNeedsUpdate : kDescendantIsolationRequired);
        }
    }
}
...
void LayoutSVGContainer::DescendantIsolationRequirementsChanged(DescendantIsolationState state)
{
    ...
    if (!IsSVGHiddenContainer() && SVGLayoutSupport::WillIsolateBlendingDescendantsForStyle(StyleRef())) {
        SetNeedsPaintPropertyUpdate();
        return;
    }
    if (Parent())
    ...
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    if (!ChildPaintBlockedByDisplayLock() && content_.HitTest(result, *local_location, phase))
        return true;
    ...
    if (StyleRef().UsedPointerEvents() == EPointerEvents::kBoundingBox) {
        ...
        // containers.
        if (IsObjectBoundingBoxValid() && local_location->Intersects(ObjectBoundingBox())) {
...
SVGTransformChange LayoutSVGContainer::UpdateLocalTransform(const gfx::RectF& reference_box)
{
    NOT_DESTROYED();
