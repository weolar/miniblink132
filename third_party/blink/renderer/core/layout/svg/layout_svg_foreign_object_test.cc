...
TEST_F(LayoutSVGForeignObjectTest, DivInForeignObject)
{
    ...
    EXPECT_EQ(PhysicalOffset(150, 150), div.LocalToAncestorPoint(PhysicalOffset(), &GetLayoutView(), kTraverseDocumentBoundaries));

    // MapAncestorToLocal
    EXPECT_EQ(PhysicalOffset(-150, -150), div.AncestorToLocalPoint(&GetLayoutView(), PhysicalOffset(), kTraverseDocumentBoundaries));
...
TEST_F(LayoutSVGForeignObjectTest, IframeInForeignObject)
{
    ...
    const auto& div = *ChildDocument().getElementById(AtomicString("div"))->GetLayoutObject();
    ...
    // LocalToAncestorPoint
    EXPECT_EQ(PhysicalOffset(200, 200), div.LocalToAncestorPoint(PhysicalOffset(), &GetLayoutView(), kTraverseDocumentBoundaries));

    // AncestorToLocalPoint
    EXPECT_EQ(PhysicalOffset(-200, -200), div.AncestorToLocalPoint(&GetLayoutView(), PhysicalOffset(), kTraverseDocumentBoundaries));
    ...
    for (auto result : results) {
        Node* node = result.Get();
        if (node == svg || node == div.GetNode() || node == foreign || node == iframe) {
            count++;
        }
    }
    EXPECT_EQ(4, count);
...
TEST_F(LayoutSVGForeignObjectTest, HitTestZoomedForeignObject)
{
    ...
    // MapToVisualRectInAncestorSpace
    PhysicalRect div_rect(0, 0, 100, 50);
    EXPECT_TRUE(div.GetLayoutObject()->MapToVisualRectInAncestorSpace(&GetLayoutView(), div_rect));
    ...
    // LocalToAncestorPoint
    EXPECT_EQ(div_offset, div.GetLayoutObject()->LocalToAncestorPoint(PhysicalOffset(), &GetLayoutView(), kTraverseDocumentBoundaries));

    // AncestorToLocalPoint
    EXPECT_EQ(PhysicalOffset(), div.GetLayoutObject()->AncestorToLocalPoint(&GetLayoutView(), div_offset, kTraverseDocumentBoundaries));
...
TEST_F(LayoutSVGForeignObjectTest, HitTestViewBoxForeignObject)
{
    ...
    // LocalToAncestorPoint
    EXPECT_EQ(PhysicalOffset(128, 128), div.GetLayoutObject()->LocalToAncestorPoint(PhysicalOffset(), &GetLayoutView(), kTraverseDocumentBoundaries));

    // AncestorToLocalPoint
    EXPECT_EQ(PhysicalOffset(), div.GetLayoutObject()->AncestorToLocalPoint(&GetLayoutView(), PhysicalOffset(128, 128), kTraverseDocumentBoundaries));
...
TEST_F(LayoutSVGForeignObjectTest, HitTestUnderClippedPositionedForeignObjectDescendant)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      * {
...
TEST_F(LayoutSVGForeignObjectTest, HitTestUnderTransformedForeignObjectDescendant)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      * {
...
TEST_F(LayoutSVGForeignObjectTest, SetNeedsCollectInlines)
{
    ...
    target->setAttribute(svg_names::kUnicodeBidiAttr, AtomicString("bidi-override"));
...
<svg><pattern id="pat"></pattern>
    ...
    UpdateAllLifecyclePhasesForTest();
    GetElementById("in-foreign")->setAttribute(svg_names::kStyleAttr, AtomicString("display: inline-block"));
    UpdateAllLifecyclePhasesForTest();
    GetElementById("pat")->setAttribute(svg_names::kViewBoxAttr, AtomicString("972 815 1088 675"));
    UpdateAllLifecyclePhasesForTest();
    GetElementById("sibling-div")->setAttribute(svg_names::kStyleAttr, AtomicString("display: none"));
    UpdateAllLifecyclePhasesForTest();
    // Pass if no crashes.
}
...
TEST_F(LayoutSVGForeignObjectTest, ZoomChangesInvalidatePaintProperties)
{
    ...
    const TransformPaintPropertyNode* svg_replaced_contents = svg->FirstFragment().PaintProperties()->ReplacedContentTransform();
    ...
    GetDocument().documentElement()->setAttribute(svg_names::kStyleAttr, AtomicString("zoom: 2"));
    ...
    const TransformPaintPropertyNode* foreign_transform = foreign->FirstFragment().PaintProperties()->Transform();
...
<div id="target">b</div>
    ...
    EXPECT_NE(target->LocalToAbsolutePoint(PhysicalOffset()), target->LocalToAncestorPoint(PhysicalOffset(), foreign));
}
