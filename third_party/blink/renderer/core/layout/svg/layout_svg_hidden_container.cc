...
SVGLayoutResult LayoutSVGHiddenContainer::UpdateSVGLayout(const SVGLayoutInfo& layout_info)
{
    NOT_DESTROYED();
    DCHECK(NeedsLayout());
    ...
    child_layout_info.viewport_changed = layout_info.viewport_changed && GetElement()->HasRelativeLengths();
...
bool LayoutSVGHiddenContainer::NodeAtPoint(HitTestResult&, const HitTestLocation&, const PhysicalOffset&, HitTestPhase)
{
    NOT_DESTROYED();
    return false;
}
