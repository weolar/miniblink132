...
AffineTransform DeprecatedCalculateTransformToLayer(const LayoutObject* layout_object)
{
...
gfx::RectF SVGLayoutSupport::LocalVisualRect(const LayoutObject& object)
{
    ...
    if (int outset = OutlinePainter::OutlineOutsetExtent(object.StyleRef(), LayoutObject::OutlineInfo::GetUnzoomedFromStyle(object.StyleRef()))) {
        visual_rect.Outset(outset);
    }
    return visual_rect;
}

PhysicalRect SVGLayoutSupport::VisualRectInAncestorSpace(const LayoutObject& object, const LayoutBoxModelObject& ancestor, VisualRectFlags flags)
{
    PhysicalRect rect;
    MapToVisualRectInAncestorSpace(object, &ancestor, LocalVisualRect(object), rect, flags);
    return rect;
}

static gfx::RectF MapToSVGRootIncludingFilter(const LayoutObject& object, const gfx::RectF& local_visual_rect)
{
    ...
    return To<LayoutSVGRoot>(*parent).LocalToBorderBoxTransform().MapRect(visual_rect);
}

static const LayoutSVGRoot& ComputeTransformToSVGRoot(const LayoutObject& object, AffineTransform& root_border_box_transform, bool* filter_skipped)
{
    DCHECK(object.IsSVGChild());
...
bool SVGLayoutSupport::MapToVisualRectInAncestorSpace(const LayoutObject& object, const LayoutBoxModelObject* ancestor, const gfx::RectF& local_visual_rect,
    PhysicalRect& result_rect, VisualRectFlags visual_rect_flags)
{
    AffineTransform root_border_box_transform;
    bool filter_skipped = false;
    const LayoutSVGRoot& svg_root = ComputeTransformToSVGRoot(object, root_border_box_transform, &filter_skipped);
    ...
    return svg_root.MapToVisualRectInAncestorSpace(ancestor, result_rect, visual_rect_flags);
}

void SVGLayoutSupport::MapLocalToAncestor(
    const LayoutObject* object, const LayoutBoxModelObject* ancestor, TransformState& transform_state, MapCoordinatesFlags flags)
{
    if (object == ancestor) {
        return;
    }
...
void SVGLayoutSupport::MapAncestorToLocal(
    const LayoutObject& object, const LayoutBoxModelObject* ancestor, TransformState& transform_state, MapCoordinatesFlags flags)
{
    ...
    DCHECK(object.IsSVGContainer() || object.IsSVGShape() || object.IsSVGImage() || object.IsSVGForeignObject());
    AffineTransform local_to_svg_root;
    const LayoutSVGRoot& svg_root = ComputeTransformToSVGRoot(object, local_to_svg_root, nullptr);
...
{
    return style.OverflowX() == EOverflow::kHidden || style.OverflowX() == EOverflow::kClip || style.OverflowX() == EOverflow::kScroll;
}

void SVGLayoutSupport::AdjustWithClipPathAndMask(const LayoutObject& layout_object, const gfx::RectF& object_bounding_box, gfx::RectF& visual_rect)
{
    SVGResourceClient* client = SVGResources::GetClient(layout_object);
    if (!client)
        return;
    const ComputedStyle& style = layout_object.StyleRef();
    if (LayoutSVGResourceClipper* clipper = GetSVGResourceAsType(*client, style.ClipPath()))
        visual_rect.Intersect(clipper->ResourceBoundingBox(object_bounding_box));
    if (auto mask_bbox = CSSMaskPainter::MaskBoundingBox(layout_object, PhysicalOffset())) {
        visual_rect.Intersect(*mask_bbox);
    }
}

gfx::RectF SVGLayoutSupport::ExtendTextBBoxWithStroke(const LayoutObject& layout_object, const gfx::RectF& text_bounds)
{
...
gfx::RectF SVGLayoutSupport::ComputeVisualRectForText(const LayoutObject& layout_object, const gfx::RectF& text_bounds)
{
    DCHECK(layout_object.IsSVGText() || layout_object.IsSVGInline());
...
DashArray SVGLayoutSupport::ResolveSVGDashArray(const SVGDashArray& svg_dash_array, const ComputedStyle& style, const SVGViewportResolver& viewport_resolver)
{
...
void SVGLayoutSupport::ApplyStrokeStyleToStrokeData(StrokeData& stroke_data, const ComputedStyle& style, const LayoutObject& object, float dash_scale_factor)
{
    DCHECK(object.GetNode());
    DCHECK(object.GetNode()->IsSVGElement());

    const SVGViewportResolver viewport_resolver(object);
    stroke_data.SetThickness(ValueForLength(style.StrokeWidth(), viewport_resolver));
    ...
    DashArray dash_array = ResolveSVGDashArray(*style.StrokeDashArray(), style, viewport_resolver);
    float dash_offset = ValueForLength(style.StrokeDashOffset(), viewport_resolver, style);
...
SubtreeContentTransformScope::SubtreeContentTransformScope(const AffineTransform& subtree_content_transformation)
...
float SVGLayoutSupport::CalculateScreenFontSizeScalingFactor(const LayoutObject* layout_object)
{
    ...
    AffineTransform ctm = DeprecatedCalculateTransformToLayer(layout_object) * SubtreeContentTransformScope::CurrentContentTransformation();
...
static inline double DistanceToChildLayoutObject(LayoutObject* child, const gfx::PointF& point)
{
    const AffineTransform& local_to_parent_transform = child->LocalToSVGParentTransform();
    if (!local_to_parent_transform.IsInvertible())
        return std::numeric_limits<float>::max();
    gfx::PointF child_local_point = local_to_parent_transform.Inverse().MapPoint(point);
    return (child->ObjectBoundingBox().ClosestPoint(child_local_point) - child_local_point).LengthSquared();
}

static SearchCandidate SearchTreeForFindClosestLayoutSVGText(const LayoutObject* layout_object, const gfx::PointF& point)
{
    ...
    for (LayoutObject* child = layout_object->SlowLastChild(); child; child = child->PreviousSibling()) {
        if (child->IsSVGText()) {
    ...
    for (const SearchCandidate& search_candidate : candidates) {
        ...
        gfx::PointF candidate_local_point = candidate_layout_object->LocalToSVGParentTransform().Inverse().MapPoint(point);

        SearchCandidate candidate_text = SearchTreeForFindClosestLayoutSVGText(candidate_layout_object, candidate_local_point);
...
LayoutObject* SVGLayoutSupport::FindClosestLayoutSVGText(const LayoutObject* layout_object, const gfx::PointF& point)
{
    return SearchTreeForFindClosestLayoutSVGText(layout_object, point).layout_object;
}
