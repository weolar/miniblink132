...
SVGLayoutResult LayoutSVGResourceMarker::UpdateSVGLayout(const SVGLayoutInfo& layout_info)
{
    NOT_DESTROYED();
    DCHECK(NeedsLayout());
    if (is_in_layout_)
...
gfx::RectF LayoutSVGResourceMarker::MarkerBoundaries(const AffineTransform& marker_transformation) const
{
    NOT_DESTROYED();
    gfx::RectF coordinates = LayoutSVGContainer::VisualRectInLocalSVGCoordinates();
...
gfx::PointF LayoutSVGResourceMarker::ReferencePoint() const
{
    ...
    return gfx::PointF(marker->refX()->CurrentValue()->Value(length_context), marker->refY()->CurrentValue()->Value(length_context));
}

float LayoutSVGResourceMarker::Angle() const
{
    NOT_DESTROYED();
    return To<SVGMarkerElement>(GetElement())->orientAngle()->CurrentValue()->Value();
}
...
AffineTransform LayoutSVGResourceMarker::MarkerTransformation(const MarkerPosition& position, float stroke_width) const
{
    NOT_DESTROYED();
    // Apply scaling according to markerUnits ('strokeWidth' or 'userSpaceOnUse'.)
    float marker_scale = MarkerUnits() == kSVGMarkerUnitsStrokeWidth ? stroke_width : 1;
    ...
    } else if (position.type == kStartMarker && orient_type == kSVGMarkerOrientAutoStartReverse) {
        computed_angle += 180;
    }
    ...
    gfx::PointF mapped_reference_point = LocalToSVGParentTransform().MapPoint(ReferencePoint());
...
bool LayoutSVGResourceMarker::ShouldPaint() const
{
    ...
    auto* marker = To<SVGMarkerElement>(GetElement());
    DCHECK(marker);
    return !marker->HasValidViewBox() || !marker->viewBox()->CurrentValue()->Rect().IsEmpty();
}
...
SVGTransformChange LayoutSVGResourceMarker::UpdateLocalTransform(const gfx::RectF& reference_box)
{
    NOT_DESTROYED();
    auto* marker = To<SVGMarkerElement>(GetElement());
    DCHECK(marker);
