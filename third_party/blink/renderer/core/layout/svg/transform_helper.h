...
public:
    ...
    static void UpdateReferenceBoxDependency(LayoutObject&, bool transform_uses_reference_box);

    // Check if ComputedStyle has changed in a way that could be reflected in the
    // transform reference box.
    static bool CheckReferenceBoxDependencies(const ComputedStyle& old_style, const ComputedStyle& style);
    ...
    static AffineTransform ComputeTransform(UseCounter&, const ComputedStyle&, const gfx::RectF& reference_box, ComputedStyle::ApplyTransformOrigin);
    ...
    static AffineTransform ComputeTransformIncludingMotion(const SVGElement& element, const gfx::RectF& reference_box);

    // Like the above, but also computes the reference box.
    static AffineTransform ComputeTransformIncludingMotion(const SVGElement& element);

    static gfx::PointF ComputeTransformOrigin(const ComputedStyle&, const gfx::RectF& reference_box);
};
