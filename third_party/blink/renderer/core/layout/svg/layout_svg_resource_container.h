...
public:
    ...
    bool IsSVGPaintServer() const
    {
        ...
        return resource_type == kPatternResourceType || resource_type == kLinearGradientResourceType || resource_type == kRadialGradientResourceType;
    }

    bool FindCycle() const;

    static void InvalidateDependentElements(LayoutObject&, bool needs_layout);
    static void InvalidateAncestorChainResources(LayoutObject&, bool needs_layout);
    static void MarkForLayoutAndParentResourceInvalidation(LayoutObject&, bool needs_layout = true);
    ...
    static gfx::RectF ResolveRectangle(const SVGViewportResolver&, const SVGLengthConversionData&, SVGUnitTypes::SVGUnitType type,
        const gfx::RectF& reference_box, const SVGLength& x, const SVGLength& y, const SVGLength& width, const SVGLength& height,
        const std::optional<gfx::SizeF>& override_viewport = std::nullopt);
    static gfx::RectF ResolveRectangle(const SVGElement& context, SVGUnitTypes::SVGUnitType type, const gfx::RectF& reference_box, const SVGLength& x,
        const SVGLength& y, const SVGLength& width, const SVGLength& height, const std::optional<gfx::SizeF>& override_viewport = std::nullopt);
    ...
    static gfx::RectF ResolveRectangle(
        const T& context, SVGUnitTypes::SVGUnitType type, const gfx::RectF& reference_box, const std::optional<gfx::SizeF>& override_viewport = std::nullopt)
    {
        return ResolveRectangle(context, type, reference_box, *context.x()->CurrentValue(), *context.y()->CurrentValue(), *context.width()->CurrentValue(),
            *context.height()->CurrentValue(), override_viewport);
    }

    gfx::RectF ResolveRectangle(SVGUnitTypes::SVGUnitType type, const gfx::RectF& reference_box, const SVGLength& x, const SVGLength& y, const SVGLength& width,
...
protected:
    ...
    enum InvalidationMode {
        ...
        kInvalidateAll
        = kLayoutInvalidation | kBoundariesInvalidation | kPaintInvalidation | kPaintPropertiesInvalidation | kClipCacheInvalidation | kFilterCacheInvalidation,
    };
...
template <> struct DowncastTraits<LayoutSVGResourceContainer> {
...
template <typename ContainerType> inline ContainerType* GetSVGResourceAsType(SVGResourceClient& client, const SVGResource* resource)
{
    if (!resource) {
        return nullptr;
    }
...
template <typename ContainerType> inline ContainerType* GetSVGResourceAsType(SVGResourceClient& client, const StyleSVGResource* style_resource)
{
    if (!style_resource) {
        return nullptr;
    }
    return GetSVGResourceAsType<ContainerType>(client, style_resource->Resource());
}
