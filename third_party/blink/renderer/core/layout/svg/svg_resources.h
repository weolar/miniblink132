...
public:
    ...
    static gfx::RectF ReferenceBoxForEffects(
        const LayoutObject&, GeometryBox geometry_box = GeometryBox::kFillBox, ForeignObjectQuirk foreign_object_quirk = ForeignObjectQuirk::kEnabled);

    static void UpdateEffects(LayoutObject&, StyleDifference, const ComputedStyle* old_style);
    static void ClearEffects(const LayoutObject&);
    static void UpdatePaints(const LayoutObject&, const ComputedStyle* old_style, const ComputedStyle& style);
    static void ClearPaints(const LayoutObject&, const ComputedStyle* style);
    static void UpdateMarkers(const LayoutObject&, const ComputedStyle* old_style);
...
class SVGElementResourceClient final : public GarbageCollected<SVGElementResourceClient>, public SVGResourceClient {
public:
    explicit SVGElementResourceClient(SVGElement*);

    void ResourceContentChanged(SVGResource*) override;

    void FilterPrimitiveChanged(SVGResource*, SVGFilterPrimitiveStandardAttributes& primitive, const QualifiedName& attribute) override;
...
private:
    ...
    static FilterData* CreateFilterDataWithNodeMap(FilterEffectBuilder&, const ReferenceFilterOperation&);
