...
public:
    ...
    // space.
    gfx::RectF MarkerBoundaries(const AffineTransform& marker_transformation) const;
    AffineTransform MarkerTransformation(const MarkerPosition&, float stroke_width) const;
...
private:
    ...
    SVGTransformChange UpdateLocalTransform(const gfx::RectF& reference_box) final;
...
template <> struct DowncastTraits<LayoutSVGResourceMarker> {
