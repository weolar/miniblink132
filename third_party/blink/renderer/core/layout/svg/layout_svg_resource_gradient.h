...
public:
    ...
    bool ApplyShader(const SVGResourceClient&, const gfx::RectF& reference_box, const AffineTransform* additional_transform, const AutoDarkMode& auto_dark_mode,
        cc::PaintFlags&) final;
...
protected:
    ...
    gfx::PointF ResolvePoint(SVGUnitTypes::SVGUnitType, const SVGLength& x, const SVGLength& y) const;
...
private:
    std::unique_ptr<GradientData> BuildGradientData(const gfx::RectF& object_bounding_box) const;

    using GradientMap = HeapHashMap<Member<const SVGResourceClient>, std::unique_ptr<GradientData>>;
