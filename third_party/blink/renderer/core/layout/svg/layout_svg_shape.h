...
protected:
    ...
    virtual void UpdateMarkerBounds()
    {
        NOT_DESTROYED();
    }
    virtual bool ShapeDependentStrokeContains(const HitTestLocation&);
    virtual bool ShapeDependentFillContains(const HitTestLocation&, const WindRule) const;
...
private:
    ...
    bool FillContains(const HitTestLocation&, bool requires_fill = true, const WindRule fill_rule = RULE_NONZERO);
    ...
    bool HitTestShape(const HitTestRequest&, const HitTestLocation&, PointerEventsHitRules);
...
template <> struct DowncastTraits<LayoutSVGShape> {
