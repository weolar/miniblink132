...
public:
    ...
    PhysicalSize Layout(const String& ifc_text_content, FragmentItemsBuilder::ItemWithOffsetList& items);

private:
    // Returns false if we should skip the following steps.
    bool Setup(wtf_size_t approximate_count);
    void SetFlags(const String& ifc_text_content, const FragmentItemsBuilder::ItemWithOffsetList& items);
    void AdjustPositionsDxDy(const FragmentItemsBuilder::ItemWithOffsetList& items);
    void ApplyTextLengthAttribute(const FragmentItemsBuilder::ItemWithOffsetList& items);
    ...
    PhysicalSize WriteBackToFragmentItems(FragmentItemsBuilder::ItemWithOffsetList& items);
    ...
    float ScalingFactorAt(const FragmentItemsBuilder::ItemWithOffsetList& items, wtf_size_t addressable_index) const;
