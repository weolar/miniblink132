...
const GradientAttributes& LayoutSVGResourceRadialGradient::EnsureAttributes() const
{
    NOT_DESTROYED();
    DCHECK(GetElement());
    if (should_collect_gradient_attributes_) {
        attributes_ = To<SVGRadialGradientElement>(*GetElement()).CollectGradientAttributes();
        should_collect_gradient_attributes_ = false;
    }
    return attributes_;
}

gfx::PointF LayoutSVGResourceRadialGradient::CenterPoint(const RadialGradientAttributes& attributes) const
{
    NOT_DESTROYED();
    return ResolvePoint(attributes.GradientUnits(), *attributes.Cx(), *attributes.Cy());
}

gfx::PointF LayoutSVGResourceRadialGradient::FocalPoint(const RadialGradientAttributes& attributes) const
{
    NOT_DESTROYED();
    return ResolvePoint(attributes.GradientUnits(), *attributes.Fx(), *attributes.Fy());
}
...
scoped_refptr<Gradient> LayoutSVGResourceRadialGradient::BuildGradient() const
{
    ...
    return Gradient::CreateRadial(FocalPoint(attributes_), FocalRadius(attributes_), CenterPoint(attributes_), Radius(attributes_), 1,
        PlatformSpreadMethodFromSVGType(attributes_.SpreadMethod()), Gradient::ColorInterpolation::kUnpremultiplied, Gradient::DegenerateHandling::kAllow);
}
