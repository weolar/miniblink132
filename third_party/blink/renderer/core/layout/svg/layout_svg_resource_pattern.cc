...
void LayoutSVGResourcePattern::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
...
bool LayoutSVGResourcePattern::RemoveClientFromCache(SVGResourceClient& client)
{
    NOT_DESTROYED();
...
const PatternAttributes& LayoutSVGResourcePattern::EnsureAttributes() const
{
    ...
    if (should_collect_pattern_attributes_) {
        attributes_ = To<SVGPatternElement>(*GetElement()).CollectPatternAttributes();
...
std::unique_ptr<PatternData> LayoutSVGResourcePattern::BuildPatternData(const gfx::RectF& object_bounding_box)
{
    NOT_DESTROYED();
    ...
    if (attributes.PatternUnits() == SVGUnitTypes::kSvgUnitTypeObjectboundingbox && object_bounding_box.IsEmpty())
        return pattern_data;

    // Compute tile metrics.
    gfx::RectF tile_bounds
        = ResolveRectangle(attributes.PatternUnits(), object_bounding_box, *attributes.X(), *attributes.Y(), *attributes.Width(), *attributes.Height());
    ...
    if (attributes.HasViewBox()) {
        ...
        if (attributes.ViewBox().IsEmpty())
            return pattern_data;
        tile_transform = SVGFitToViewBox::ViewBoxToViewTransform(attributes.ViewBox(), attributes.PreserveAspectRatio(), tile_bounds.size());
    } else {
        ...
        if (attributes.PatternContentUnits() == SVGUnitTypes::kSvgUnitTypeObjectboundingbox) {
            tile_transform.Scale(object_bounding_box.width(), object_bounding_box.height());
        }
    }
    ...
    pattern_data->pattern = Pattern::CreatePaintRecordPattern(AsPaintRecord(tile_transform), gfx::RectF(tile_bounds.size()));
...
bool LayoutSVGResourcePattern::ApplyShader(
    const SVGResourceClient& client, const gfx::RectF& reference_box, const AffineTransform* additional_transform, const AutoDarkMode&, cc::PaintFlags& flags)
{
    NOT_DESTROYED();
    ClearInvalidationMask();

    std::unique_ptr<PatternData>& pattern_data = pattern_map_.insert(&client, nullptr).stored_value->value;
    if (!pattern_data)
    ...
    pattern_data->pattern->ApplyToFlags(flags, AffineTransformToSkMatrix(transform));
    flags.setFilterQuality(cc::PaintFlags::FilterQuality::kLow);
    return true;
}

PaintRecord LayoutSVGResourcePattern::AsPaintRecord(const AffineTransform& tile_transform) const
{
    NOT_DESTROYED();
    ...
    if (DisplayLockUtilities::LockedInclusiveAncestorPreventingPaint(*pattern_content_element)) {
        return paint_recorder.finishRecordingAsPicture();
    }

    const auto* pattern_layout_object = To<LayoutSVGResourceContainer>(pattern_content_element->GetLayoutObject());
    ...
    for (LayoutObject* child = pattern_layout_object->FirstChild(); child; child = child->NextSibling()) {
