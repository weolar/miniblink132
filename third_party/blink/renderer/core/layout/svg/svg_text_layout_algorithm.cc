...
SvgTextLayoutAlgorithm::SvgTextLayoutAlgorithm(InlineNode node, WritingMode writing_mode)
    : inline_node_(node)
    ,
    ...
    , inline_direction_(WritingDirectionMode(writing_mode, TextDirection::kLtr).InlineEnd())
{
    DCHECK(node.IsSvgText());
}

PhysicalSize SvgTextLayoutAlgorithm::Layout(const String& ifc_text_content, FragmentItemsBuilder::ItemWithOffsetList& items)
{
    ...
    ResolvedTextLayoutAttributesIterator iterator(inline_node_.SvgCharacterDataList());
...
void SvgTextLayoutAlgorithm::SetFlags(const String& ifc_text_content, const FragmentItemsBuilder::ItemWithOffsetList& items)
{
    ...
    if (inline_node_.IsBidiEnabled()) {
        base::ranges::sort(sorted_item_indexes, [&](wtf_size_t a, wtf_size_t b) { return items[a]->StartOffset() < items[b]->StartOffset(); });
    }
    ...
    for (wtf_size_t i : sorted_item_indexes) {
        ...
        if (const auto* font_data = item.ScaledFont().PrimaryFont()) {
            ascent = font_data->GetFontMetrics().FixedAscent(item.Style().GetFontBaseline());
        }
        gfx::PointF offset;
        if (IsHorizontal()) {
            offset.SetPoint(logical_offset.inline_offset, logical_offset.block_offset + ascent);
        } else if (IsVerticalDownward()) {
            offset.SetPoint(-(logical_offset.block_offset + ascent), logical_offset.inline_offset);
        } else {
            offset.SetPoint(logical_offset.block_offset + ascent, -logical_offset.inline_offset);
        }
        ...
        StringView item_string(ifc_text_content, item.StartOffset(), item.TextLength());
...
void SvgTextLayoutAlgorithm::AdjustPositionsDxDy(const FragmentItemsBuilder::ItemWithOffsetList& items)
{
    ...
    for (wtf_size_t i = 0; i < addressable_count_; ++i) {
        ...
        // https://github.com/w3c/svgwg/issues/271
        shift.Offset(resolve.HasDx() ? resolve.dx : 0.0f, resolve.HasDy() ? resolve.dy : 0.0f);
        ...
        result_[i].x = ClampTo<float>(css_positions_[i].x() + shift.x() * scaling_factor);
        result_[i].y = ClampTo<float>(css_positions_[i].y() + shift.y() * scaling_factor);
    }
}

void SvgTextLayoutAlgorithm::ApplyTextLengthAttribute(const FragmentItemsBuilder::ItemWithOffsetList& items)
{
...
void SvgTextLayoutAlgorithm::ResolveTextLength(
    const FragmentItemsBuilder::ItemWithOffsetList& items, const SvgTextContentRange& range, Vector<wtf_size_t>& resolved_descendant_node_starts)
{
    ...
    const float text_length = ClampTo<float>(element->textLength()->CurrentValue()->Value(SVGLengthContext(element)) * ScalingFactorAt(items, i));
    const SVGLengthAdjustType length_adjust = element->lengthAdjust()->CurrentEnumValue();
    ...
    for (wtf_size_t k = i; k < j_plus_1; ++k) {
        ...
        float min_char_pos = IsHorizontal() ? *result_[k].x : IsVerticalDownward() ? *result_[k].y : -*result_[k].y;
    ...
    if (length_adjust == kSVGLengthAdjustSpacingAndGlyphs) {
        ...
        auto n
            = base::ranges::count_if(base::span(result_).subspan(i, j_plus_1 - i), [](const auto& info) { return !info.middle && !info.text_length_resolved; });
        // 2.4.3. Let n = n + number of resolved descendant nodes − 1.
        n += base::ranges::count_if(resolved_descendant_node_starts, [i, j_plus_1](const auto& start_index) {
        ...
        if (inline_node_.IsBidiEnabled()) {
            std::sort(visual_indexes.begin(), visual_indexes.end(), [&](wtf_size_t a, wtf_size_t b) { return result_[a].item_index < result_[b].item_index; });
        }
        ...
        for (wtf_size_t k : visual_indexes) {
            ...
            if (!info.middle && (base::Contains(resolved_descendant_node_starts, k) || !info.text_length_resolved)) {
    ...
    auto new_end = std::remove_if(resolved_descendant_node_starts.begin(), resolved_descendant_node_starts.end(),
        [i, j_plus_1](const auto& start_index) { return i <= start_index && start_index < j_plus_1; });
    resolved_descendant_node_starts.erase(new_end, resolved_descendant_node_starts.end());
    resolved_descendant_node_starts.push_back(i);
}

void SvgTextLayoutAlgorithm::AdjustPositionsXY(const FragmentItemsBuilder::ItemWithOffsetList& items)
{
    ...
    for (wtf_size_t i = 0; i < result_.size(); ++i) {
        ...
        if (resolve.HasX()) {
            shift.set_x(resolve.x * scaling_factor - css_positions_[i].x() - result_[i].text_length_shift_x);
        ...
        // https://github.com/w3c/svgwg/issues/845
        if (resolve.HasY()) {
            shift.set_y(resolve.y * scaling_factor - css_positions_[i].y() - result_[i].text_length_shift_y);
            // Take into account of baseline-shift.
            if (horizontal_) {
...
void SvgTextLayoutAlgorithm::ApplyAnchoring(const FragmentItemsBuilder::ItemWithOffsetList& items)
{
    ...
    while (i < result_.size()) {
        ...
        auto next_anchor = base::ranges::find_if(result_range, [](const auto& info) { return info.anchored_chunk; });
        wtf_size_t j = start_index + static_cast<wtf_size_t>(std::distance(result_range.begin(), next_anchor) - 1);

        const auto& text_path_ranges = inline_node_.SvgTextPathRangeList();
        const auto text_path_iter = base::ranges::find_if(text_path_ranges, [i](const auto& range) { return range.start_index <= i && i <= range.end_index; });
        ...
        for (wtf_size_t k = i; k <= j; ++k) {
            ...
            const float min_char_pos = IsHorizontal() ? *result_[k].x : IsVerticalDownward() ? *result_[k].y : -*result_[k].y;
        ...
        if (min_position != std::numeric_limits<float>::infinity()) {
            ...
            float shift = IsHorizontal() ? *result_[i].x : IsVerticalDownward() ? *result_[i].y : -*result_[i].y;
...
void SvgTextLayoutAlgorithm::PositionOnPath(const FragmentItemsBuilder::ItemWithOffsetList& items)
{
    ...
    for (unsigned index = 0; index < result_.size(); ++index) {
        ...
        if (range_index < ranges.size() && index >= ranges[range_index].start_index && index <= ranges[range_index].end_index) {
            if (!in_path || in_path_index != range_index) {
                path_mapper = To<LayoutSVGTextPath>(ranges[range_index].layout_object.Get())->LayoutPath();
            }
            ...
            if (!info.middle) {
                ...
                if (!path_mapper) {
                    ...
                    const float char_offset = IsHorizontal() ? *info.x : IsVerticalDownward() ? *info.y : -*info.y;
                    ...
                    if (!info.hidden) {
                        ...
                        PathPositionMapper::PositionType position_type = path_mapper->PointAndNormalAtLength(mid, point_tangent);
                        ...
                        if (*info.rotate == 0.0f) {
                            if (IsHorizontal()) {
                                ...
                                info.y = point_tangent.point.y() * scaling_factor - info.inline_size / 2;
                            } else {
                                info.x = point_tangent.point.x() * scaling_factor + *info.x;
                                info.y = point_tangent.point.y() * scaling_factor + info.inline_size / 2;
                            }
                            ...
                            info.baseline_shift = IsHorizontal() ? *info.y : IsVerticalDownward() ? *info.x : -*info.x;
            ...
            if (in_path) {
                ...
                if (path_mapper) {
                    ...
                    const auto iter = base::ranges::find_if(reverse_result_range, [](const auto& info) { return !info.hidden && !info.middle; });
...
PhysicalSize SvgTextLayoutAlgorithm::WriteBackToFragmentItems(FragmentItemsBuilder::ItemWithOffsetList& items)
{
    ...
    for (const SvgPerCharacterInfo& info : result_) {
        ...
        const auto* layout_object = To<LayoutSVGInlineText>(item->GetLayoutObject());
        LayoutUnit ascent;
        LayoutUnit descent;
        ...
        gfx::RectF scaled_rect(ClampTo<float>(x), ClampTo<float>(y), ClampTo<float>(width), ClampTo<float>(height));
        ...
        item.item.SetSvgFragmentData(data, PhysicalRect::EnclosingRect(unscaled_rect), info.hidden);

        gfx::RectF transformd_rect = scaled_rect;
        if (item.item.HasSvgTransformForBoundingBox()) {
            transformd_rect = item.item.BuildSvgTransformForBoundingBox().MapRect(transformd_rect);
        }
        transformd_rect.Scale(1 / scaling_factor);
        unscaled_visual_rect.Union(transformd_rect);
    }
    if (items[0]->Type() == FragmentItem::kLine) {
        items[0].item.SetSvgLineLocalRect(PhysicalRect(gfx::ToEnclosingRect(unscaled_visual_rect)));
    }
    return { LayoutUnit(unscaled_visual_rect.right()), LayoutUnit(unscaled_visual_rect.bottom()) };
}

float SvgTextLayoutAlgorithm::ScalingFactorAt(const FragmentItemsBuilder::ItemWithOffsetList& items, wtf_size_t addressable_index) const
{
    return items[result_[addressable_index].item_index]->SvgScalingFactor();
}

bool SvgTextLayoutAlgorithm::IsFirstCharacterInTextPath(wtf_size_t index) const
{
    ...
    return base::Contains(inline_node_.SvgTextPathRangeList(), index, &SvgTextContentRange::start_index);
}
