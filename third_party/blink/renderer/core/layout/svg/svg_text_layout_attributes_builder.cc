...
class LayoutAttributesIterator final : public GarbageCollected<LayoutAttributesIterator> {
public:
    ...
    bool HasX() const
    {
        return consumed_ < x_->length();
    }
    bool HasY() const
    {
        return consumed_ < y_->length();
    }
    bool HasDx() const
    {
        return consumed_ < dx_->length();
    }
    bool HasDy() const
    {
        return consumed_ < dy_->length();
    }
    ...
    bool InTextPath() const
    {
        return in_text_path_;
    }
...
public:
    ...
    ~LayoutAttributesStack()
    {
        DCHECK_EQ(stack_.size(), 0u);
    }
    ...
    {
        stack_.push_back(MakeGarbageCollected<LayoutAttributesIterator>(layout_object, in_text_path));
    }
    void Pop()
    {
        stack_.pop_back();
    ...
    float X() const
    {
        auto it = base::ranges::find_if(base::Reversed(stack_), &LayoutAttributesIterator::HasX);
        return it != stack_.rend() ? (*it)->X() : SvgCharacterData::EmptyValue();
    }
    float Y() const
    {
        auto it = base::ranges::find_if(base::Reversed(stack_), &LayoutAttributesIterator::HasY);
        return it != stack_.rend() ? (*it)->Y() : SvgCharacterData::EmptyValue();
    }
    float Dx() const
    {
        auto it = base::ranges::find_if(base::Reversed(stack_), &LayoutAttributesIterator::HasDx);
        return it != stack_.rend() ? (*it)->Dx() : SvgCharacterData::EmptyValue();
    }
    float Dy() const
    {
        auto it = base::ranges::find_if(base::Reversed(stack_), &LayoutAttributesIterator::HasDy);
    ...
    bool ShouldStartAnchoredChunk(bool horizontal) const
    {
        ...
        if (!stack_.back()->InTextPath()) {
            return !SvgCharacterData::IsEmptyValue(X()) || !SvgCharacterData::IsEmptyValue(Y());
        }
...
{
    return data.HasX() || data.HasY() || data.HasDx() || data.HasDy() || data.HasRotate() || data.anchored_chunk;
}

bool HasValidTextLength(const LayoutObject& layout_object)
{
    if (auto* element = DynamicTo<SVGTextContentElement>(layout_object.GetNode())) {
        if (element->TextLengthIsSpecifiedByUser()) {
            float text_length = element->textLength()->CurrentValue()->Value(SVGLengthContext(element));
...
void SvgTextLayoutAttributesBuilder::Build(const String& ifc_text_content, const HeapVector<InlineItem>& items)
{
    ...
    if (HasValidTextLength(*block_flow_)) {
        text_length_stack.push_back(SVGTextLengthContext { block_flow_, addressable_index });
    }
    for (const auto& item : items) {
        ...
        if (item.Type() == InlineItem::kOpenTag) {
            ...
            if (HasValidTextLength(*object)) {
                text_length_stack.push_back(SVGTextLengthContext { object, addressable_index });
            }
            ...
            if (object->IsSVGTSpan()) {
                ...
                if (addressable_index != *text_path_start) {
                    text_path_range_list_.push_back(SvgTextContentRange { object, *text_path_start, addressable_index - 1 });
                }
                text_path_start.reset();
            }
            if (text_length_stack.size() > 0u && text_length_stack.back().layout_object == object) {
                if (text_length_stack.back().start_index != addressable_index) {
                    text_length_range_list_.push_back(SvgTextContentRange { object, text_length_stack.back().start_index, addressable_index - 1 });
                }
    ...
    if (text_length_stack.size() > 0u) {
        ...
        text_length_range_list_.push_back(SvgTextContentRange { block_flow_, 0u, addressable_index - 1 });
        text_length_stack.pop_back();
    }
    attr_stack.Pop();
...
unsigned SvgTextLayoutAttributesBuilder::IfcTextContentOffsetAt(wtf_size_t index)
{
