...
SVGTransformChange LayoutSVGTransformableContainer::UpdateLocalTransform(const gfx::RectF& reference_box)
{
    NOT_DESTROYED();
    ...
    if (IsA<SVGUseElement>(element)) {
        ...
        additional_translation_ = VectorForLengthPair(style.X(), style.Y(), viewport_resolver, style);
    }
    ...
    local_transform_.Translate(additional_translation_.x(), additional_translation_.y());
    return change_detector.ComputeChange(local_transform_);
}

void LayoutSVGTransformableContainer::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    if (old_style && IsA<SVGUseElement>(element)) {
        ...
        if (old_style->FillPaint() != style.FillPaint() || old_style->StrokePaint() != style.StrokePaint()) {
            SetSubtreeShouldDoFullPaintInvalidation(PaintInvalidationReason::kSVGResource);
        }
    }
    ...
    SetTransformUsesReferenceBox(TransformHelper::UpdateReferenceBoxDependency(*this));
}
