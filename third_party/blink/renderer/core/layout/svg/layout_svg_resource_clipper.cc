...
ClipStrategy ModifyStrategyForClipPath(const ComputedStyle& style, ClipStrategy strategy)
{
...
ClipStrategy DetermineClipStrategy(const SVGGraphicsElement& element)
{
    ...
    if (style.Display() == EDisplay::kNone || style.Visibility() != EVisibility::kVisible) {
        return ClipStrategy::kNone;
    }
...
ClipStrategy DetermineClipStrategy(const SVGElement& element)
{
    ...
    if (auto* svg_use_element = DynamicTo<SVGUseElement>(element)) {
        ...
        if (!use_layout_object)
            return ClipStrategy::kNone;
        if (DisplayLockUtilities::LockedAncestorPreventingLayout(*use_layout_object))
            return ClipStrategy::kNone;
        if (use_layout_object->StyleRef().Display() == EDisplay::kNone)
            return ClipStrategy::kNone;
        const SVGGraphicsElement* shape_element = svg_use_element->VisibleTargetGraphicsElementForClipping();
        if (!shape_element)
            return ClipStrategy::kNone;
        ClipStrategy shape_strategy = DetermineClipStrategy(*shape_element);
        return ModifyStrategyForClipPath(use_layout_object->StyleRef(), shape_strategy);
    }
...
std::optional<Path> LayoutSVGResourceClipper::AsPath()
{
    ...
    for (const SVGElement& child_element : Traversal<SVGElement>::ChildrenOf(*GetElement())) {
        ...
        if (clip_path_builder) {
            clip_path_builder->add(PathFromElement(child_element).GetSkPath(), kUnion_SkPathOp);
...
PaintRecord LayoutSVGResourceClipper::CreatePaintRecord()
{
    ...
    PaintInfo info(builder.Context(), CullRect::Infinite(), PaintPhase::kForeground, ChildPaintBlockedByDisplayLock(),
...
void LayoutSVGResourceClipper::CalculateLocalClipBounds()
{
    ...
    for (const SVGElement& child_element : Traversal<SVGElement>::ChildrenOf(*GetElement())) {
        if (!ContributesToClip(child_element))
            continue;
        const LayoutObject* layout_object = child_element.GetLayoutObject();
        local_clip_bounds_.Union(layout_object->LocalToSVGParentTransform().MapRect(layout_object->VisualRectInLocalSVGCoordinates()));
    }
}
...
{
    NOT_DESTROYED();
    return To<SVGClipPathElement>(GetElement())->clipPathUnits()->CurrentEnumValue();
}

AffineTransform LayoutSVGResourceClipper::CalculateClipTransform(const gfx::RectF& reference_box) const
{
    NOT_DESTROYED();
    AffineTransform transform = To<SVGClipPathElement>(GetElement())->CalculateTransform(SVGElement::kIncludeMotionTransform);
...
bool LayoutSVGResourceClipper::HitTestClipContent(
    const gfx::RectF& reference_box, const LayoutObject& reference_box_object, const HitTestLocation& location) const
{
    NOT_DESTROYED();
    if (HasClipPath() && !ClipPathClipper::HitTest(*this, reference_box, reference_box_object, location)) {
        return false;
    }

    TransformedHitTestLocation local_location(location, CalculateClipTransform(reference_box));
    if (!local_location)
        return false;
    ...
    for (const SVGElement& child_element : Traversal<SVGElement>::ChildrenOf(*GetElement())) {
        ...
        DCHECK(!layout_object->IsBoxModelObject() || !To<LayoutBoxModelObject>(layout_object)->HasSelfPaintingLayer());

        if (layout_object->NodeAtPoint(result, *local_location, PhysicalOffset(), HitTestPhase::kForeground))
            return true;
    }
    return false;
}

gfx::RectF LayoutSVGResourceClipper::ResourceBoundingBox(const gfx::RectF& reference_box)
{
    NOT_DESTROYED();
...
bool LayoutSVGResourceClipper::FindCycleFromSelf() const
{
    ...
    if (auto* reference_clip = DynamicTo<ReferenceClipPathOperation>(StyleRef().ClipPath())) {
...
void LayoutSVGResourceClipper::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
