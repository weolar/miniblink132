...
gfx::SizeF MakeViewport(const SVGViewportResolver& viewport_resolver, const LengthPoint& point, SVGUnitTypes::SVGUnitType type)
{
...
float MakeViewportDimension(const SVGViewportResolver& viewport_resolver, const Length& radius, SVGUnitTypes::SVGUnitType type)
{
...
bool LayoutSVGResourceGradient::RemoveClientFromCache(SVGResourceClient& client)
{
    NOT_DESTROYED();
...
std::unique_ptr<GradientData> LayoutSVGResourceGradient::BuildGradientData(const gfx::RectF& object_bounding_box) const
{
    NOT_DESTROYED();
    ...
    if (attributes.GradientUnits() == SVGUnitTypes::kSvgUnitTypeObjectboundingbox) {
        ...
        gradient_data->userspace_transform.Translate(object_bounding_box.x(), object_bounding_box.y());
        gradient_data->userspace_transform.ScaleNonUniform(object_bounding_box.width(), object_bounding_box.height());
    }
    ...
    if (RuntimeEnabledFeatures::SvgGradientColorInterpolationLinearRgbSupportEnabled()) {
        gradient_data->gradient->SetColorInterpolationSpace(
            StyleRef().ColorInterpolation() == EColorInterpolation::kLinearrgb ? Color::ColorSpace::kSRGBLinear : Color::ColorSpace::kNone,
...
bool LayoutSVGResourceGradient::ApplyShader(const SVGResourceClient& client, const gfx::RectF& reference_box, const AffineTransform* additional_transform,
    const AutoDarkMode& auto_dark_mode, cc::PaintFlags& flags)
{
    NOT_DESTROYED();
    ClearInvalidationMask();

    std::unique_ptr<GradientData>& gradient_data = gradient_map_.insert(&client, nullptr).stored_value->value;
    if (!gradient_data)
    ...
    draw_options.apply_dark_mode = auto_dark_mode.enabled && StyleRef().ForceDark();
    gradient_data->gradient->ApplyToFlags(flags, AffineTransformToSkMatrix(transform), draw_options);
    return true;
}
...
gfx::PointF LayoutSVGResourceGradient::ResolvePoint(SVGUnitTypes::SVGUnitType type, const SVGLength& x, const SVGLength& y) const
{
    NOT_DESTROYED();
    ...
    return PointForLengthPoint(point, MakeViewport(viewport_resolver, point, type));
}

float LayoutSVGResourceGradient::ResolveRadius(SVGUnitTypes::SVGUnitType type, const SVGLength& r) const
{
    NOT_DESTROYED();
    const SVGViewportResolver viewport_resolver(*this);
    const SVGLengthConversionData conversion_data(*this);
    const Length& radius = r.ConvertToLength(conversion_data);
    return FloatValueForLength(radius, MakeViewportDimension(viewport_resolver, radius, type));
}
