...
TEST_F(LayoutSVGContainerTest, TransformAffectsVectorEffect)
{
    ...
    rect_element->setAttribute(svg_names::kVectorEffectAttr, AtomicString("non-scaling-stroke"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(g->TransformAffectsVectorEffect());
    EXPECT_TRUE(rect->TransformAffectsVectorEffect());
    EXPECT_FALSE(text->TransformAffectsVectorEffect());
...
TEST_F(LayoutSVGContainerTest, TransformAffectsVectorEffectHiddenContainerAndUse)
{
    SetBodyInnerHTML(R"HTML(
    <svg id="svg" width="300" height="300">
      <g id="g0">
    ...
    EXPECT_FALSE(GetLayoutObjectByElementId("svg")->TransformAffectsVectorEffect());
    EXPECT_FALSE(GetLayoutObjectByElementId("g0")->TransformAffectsVectorEffect());
    EXPECT_TRUE(GetLayoutObjectByElementId("rect")->TransformAffectsVectorEffect());
...
TEST_F(LayoutSVGContainerTest, PatternWithContentVisibility)
{
    ...
    pattern->setAttribute(svg_names::kStyleAttr, AtomicString("contain: strict; content-visibility: hidden"));
