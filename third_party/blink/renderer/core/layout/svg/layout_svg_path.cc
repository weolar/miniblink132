...
{
    return element.HasTagName(svg_names::kLineTag) || element.HasTagName(svg_names::kPathTag) || element.HasTagName(svg_names::kPolygonTag)
...
bool PathGeometryChanged(const ComputedStyle& old_style, const ComputedStyle& new_style)
{
...
void LayoutSVGPath::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    if (old_style) {
        ...
        if (!base::ValuesEquivalent(old_style->MarkerStartResource(), style.MarkerStartResource())
            || !base::ValuesEquivalent(old_style->MarkerMidResource(), style.MarkerMidResource())
            || !base::ValuesEquivalent(old_style->MarkerEndResource(), style.MarkerEndResource())) {
...
void LayoutSVGPath::UpdateMarkerBounds()
{
    ...
    for (const auto& position : marker_positions_) {
        if (LayoutSVGResourceMarker* marker = position.SelectMarker(marker_start, marker_mid, marker_end)) {
            boundaries.Union(marker->MarkerBoundaries(marker->MarkerTransformation(position, stroke_width)));
        }
    }
