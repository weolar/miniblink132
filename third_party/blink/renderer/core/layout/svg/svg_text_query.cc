...
{
    return (U16_IS_TRAIL(string[offset]) && offset > 0 && U16_IS_LEAD(string[offset - 1])) ? offset - 1 : offset;
}

unsigned AdjustCodeUnitEndOffset(StringView string, unsigned offset)
{
    return (offset < string.length() && U16_IS_TRAIL(string[offset]) && offset > 0 && U16_IS_LEAD(string[offset - 1])) ? offset + 1 : offset;
}

std::tuple<Vector<const FragmentItem*>, const FragmentItems*> FragmentItemsInVisualOrder(const LayoutObject& query_root)
{
...
std::tuple<Vector<const FragmentItem*>, const FragmentItems*> FragmentItemsInLogicalOrder(const LayoutObject& query_root)
{
    ...
    std::sort(item_list.begin(), item_list.end(), [](const FragmentItem* a, const FragmentItem* b) { return a->StartOffset() < b->StartOffset(); });
    return items_tuple;
}

// Returns a tuple of FragmentItem, Item text, IFC text offset for |index|,
// and the next IFC text offset.
std::tuple<const FragmentItem*, StringView, unsigned, unsigned> FindFragmentItemForAddressableCodeUnitIndex(const LayoutObject& query_root, unsigned index)
{
    ...
    for (const auto* item : item_list) {
        ...
        return { item, item_text, item->StartOffset() + i, item->StartOffset() + item_text.NextCodePointOffset(i) };
    }
    return { nullptr, StringView(), WTF::kNotFound, WTF::kNotFound };
}

void GetCanvasRotation(void* context, unsigned, Glyph, gfx::Vector2dF, float, bool, CanvasRotationInVertical rotation, const SimpleFontData*)
{
    auto* canvas_rotation = static_cast<CanvasRotationInVertical*>(context);
    *canvas_rotation = rotation;
}

float InlineSize(const FragmentItem& item, StringView item_text, unsigned start_code_unit_offset, unsigned end_code_unit_offset)
{
    unsigned start_ifc_offset = item.StartOffset() + AdjustCodeUnitStartOffset(item_text, start_code_unit_offset);
    unsigned end_ifc_offset = item.StartOffset() + AdjustCodeUnitEndOffset(item_text, end_code_unit_offset);
    PhysicalRect r = item.LocalRect(item_text, start_ifc_offset, end_ifc_offset);
    return (item.IsHorizontal() ? r.Width() : r.Height()) * item.GetSvgFragmentData()->length_adjust_scale / item.SvgScalingFactor();
}

std::tuple<const FragmentItem*, gfx::RectF> ScaledCharacterRectInContainer(const LayoutObject& query_root, unsigned code_unit_index)
{
    auto [item, item_text, start_ifc_offset, end_ifc_offset] = FindFragmentItemForAddressableCodeUnitIndex(query_root, code_unit_index);
    DCHECK(item);
    DCHECK(item->IsSvgText());
    if (item->IsHiddenForPaint()) {
        return { item, gfx::RectF() };
    }
    auto char_rect = gfx::RectF(item->LocalRect(item_text, start_ifc_offset, end_ifc_offset));
    char_rect.Offset(item->GetSvgFragmentData()->rect.OffsetFromOrigin());
    return { item, char_rect };
}

enum class QueryPosition { kStart, kEnd };
gfx::PointF StartOrEndPosition(const LayoutObject& query_root, unsigned index, QueryPosition pos)
{
    ...
    const float ascent = font ? font->GetFontMetrics().FixedAscent(item->Style().GetFontBaseline()) : 0.0f;
    const bool is_reversed = IsLtr(item->ResolvedDirection()) != (pos == QueryPosition::kStart);
    gfx::PointF point;
...
float SvgTextQuery::SubStringLength(unsigned start_index, unsigned length) const
{
    if (length <= 0) {
        return 0.0f;
    }
    ...
    for (const auto* item : item_list) {
        ...
        if ((character_index <= start_index && start_index < next_character_index) || (character_index < end_index && end_index <= next_character_index)
            || (start_index < character_index && next_character_index < end_index)) {
            total_length += InlineSize(*item, item_text, start_index < character_index ? 0 : start_index - character_index,
...
{
    auto [item, item_text, start_ifc_offset, end_ifc_offset] = FindFragmentItemForAddressableCodeUnitIndex(query_root_, index);
    DCHECK(item);
    DCHECK(item->IsSvgText());
    if (item->IsHiddenForPaint()) {
        return 0.0f;
    ...
    item->TextShapeResult()->ForEachGlyph(0, start_ifc_offset, end_ifc_offset, 0, GetCanvasRotation, &canvas_rotation);
...
int SvgTextQuery::CharacterNumberAtPosition(const gfx::PointF& position) const
{
    ...
    PhysicalOffset transformed_point = hit_item->MapPointInContainer(PhysicalOffset::FromPointFRound(position)) - hit_item->OffsetInContainerFragment();
    // FragmentItem::TextOffsetForPoint() is not suitable here because it
    // returns an offset for the nearest glyph edge.
    LayoutUnit inline_offset
        = WritingModeConverter({ hit_item->GetWritingMode(), TextDirection::kLtr }, hit_item->Size()).ToLogical(transformed_point, {}).inline_offset;
    unsigned offset_in_item
        = hit_item->TextShapeResult()->CreateShapeResult()->OffsetForPosition(hit_item->ScaleInlineOffset(inline_offset), BreakGlyphsOption(true));
