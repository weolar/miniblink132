...
SVGLayoutResult LayoutSVGForeignObject::UpdateSVGLayout(const SVGLayoutInfo& layout_info)
{
    NOT_DESTROYED();
    DCHECK(NeedsLayout());
    ...
    viewport_.set_origin(PointForLengthPair(style.X(), style.Y(), viewport_resolver, style));
    ...
    LogicalSize zoomed_size
        = PhysicalSize(LayoutUnit(viewport_.width() * zoom), LayoutUnit(viewport_.height() * zoom)).ConvertToLogical(style.GetWritingMode());
    ...
    const auto* content_result = BlockNode(this).Layout(builder.ToConstraintSpace());

    // Any propagated sticky-descendants may have invalid sticky-constraints.
    // Clear them now.
    if (const auto* sticky_descendants = content_result->GetPhysicalFragment().PropagatedStickyDescendants()) {
...
bool LayoutSVGForeignObject::UpdateAfterSVGLayout(const SVGLayoutInfo& layout_info, bool bounds_changed)
{
...
void LayoutSVGForeignObject::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    LayoutSVGBlock::StyleDidChange(diff, old_style);

    float old_zoom = old_style ? old_style->EffectiveZoom() : ComputedStyleInitialValues::InitialZoom();
...
bool LayoutSVGForeignObject::NodeAtPointFromSVG(
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase)
{
    NOT_DESTROYED();
    ...
    bool retval = Layer()->HitTest(local_without_offset, layer_result, PhysicalRect(InfiniteIntRect()));
    ...
    PhysicalOffset original_point_in_inner_node_frame = result.PointInInnerNodeFrame();
