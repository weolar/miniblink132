...
void LayoutSVGResourceMasker::RemoveAllClientsFromCache()
{
    ...
    MarkAllClientsForInvalidation(kPaintPropertiesInvalidation | kPaintInvalidation);
}
...
PaintRecord LayoutSVGResourceMasker::CreatePaintRecord()
{
    ...
    for (const SVGElement& child_element : Traversal<SVGElement>::ChildrenOf(*GetElement())) {
        ...
        if (!layout_object)
            continue;
        if (DisplayLockUtilities::LockedAncestorPreventingLayout(*layout_object) || layout_object->StyleRef().Display() == EDisplay::kNone)
            continue;
        SVGObjectPainter(*layout_object, nullptr).PaintResourceSubtree(builder.Context(), PaintFlag::kPaintingSVGMask);
    }
...
{
    NOT_DESTROYED();
    return To<SVGMaskElement>(GetElement())->maskContentUnits()->CurrentEnumValue();
}

gfx::RectF LayoutSVGResourceMasker::ResourceBoundingBox(const gfx::RectF& reference_box, float reference_box_zoom)
{
    NOT_DESTROYED();
    ...
    gfx::RectF mask_boundaries = ResolveRectangle(mask_units, reference_box, *mask_element->x()->CurrentValue(), *mask_element->y()->CurrentValue(),
        *mask_element->width()->CurrentValue(), *mask_element->height()->CurrentValue());
