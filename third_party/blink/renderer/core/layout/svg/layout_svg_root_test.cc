...
TEST_P(LayoutSVGRootTest, VisualRectMappingWithoutViewportClipWithBorder)
{
    ...
    const auto& svg_rect = *To<LayoutSVGShape>(GetLayoutObjectByElementId("rect"));
    ...
    auto root_visual_rect = LocalVisualRect(static_cast<const LayoutObject&>(root));
...
TEST_P(LayoutSVGRootTest, VisualOverflowExpandsLayer)
{
    ...
    auto* layer = CcLayersByDOMElementId(GetDocument().View()->RootCcLayer(), "root")[0];
    EXPECT_EQ(gfx::Size(100, 100), layer->bounds());

    GetElementById("rect")->setAttribute(svg_names::kHeightAttr, AtomicString("200"));
...
TEST_P(LayoutSVGRootTest, PaintLayerType)
{
    ...
    GetElementById("rect")->setAttribute(svg_names::kStyleAttr, AtomicString("will-change: transform"));
    UpdateAllLifecyclePhasesForTest();
    ASSERT_TRUE(root.Layer());
    EXPECT_FALSE(root.Layer()->IsSelfPaintingLayer());
