...
void LayoutSVGInlineText::TextDidChange()
{
    ...
    LayoutSVGText::NotifySubtreeStructureChanged(this, layout_invalidation_reason::kTextChanged);

    if (StyleRef().UsedUserModify() != EUserModify::kReadOnly)
        UseCounter::Count(GetDocument(), WebFeature::kSVGTextEdited);
}

void LayoutSVGInlineText::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    if (auto* ng_text = LayoutSVGText::LocateLayoutSVGTextAncestor(this)) {
        ...
        ng_text->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kStyleChange);
    }
}
...
void LayoutSVGInlineText::InvalidateSubtreeLayoutForFontUpdates()
{
    ...
    if (!IsFontFallbackValid()) {
        LayoutSVGText::NotifySubtreeStructureChanged(this, layout_invalidation_reason::kFontsChanged);
    }
...
PositionWithAffinity LayoutSVGInlineText::PositionForPoint(const PhysicalOffset& point) const
{
    NOT_DESTROYED();
    ...
    for (; cursor; cursor.MoveToNextForSameLayoutObject()) {
        PhysicalOffset transformed_point = cursor.CurrentItem()->MapPointInContainer(point);
        PhysicalRect item_rect = cursor.Current().RectInContainerFragment();
        LayoutUnit distance;
        if (!item_rect.Contains(transformed_point) || !cursor.PositionForPointInChild(transformed_point)) {
    ...
    if (last_hit_cursor) {
        auto position_with_affinity = last_hit_cursor.PositionForPointInChild(last_hit_transformed_point);
...
void LayoutSVGInlineText::ComputeNewScaledFontForStyle(const LayoutObject& layout_object, float& scaling_factor, Font& scaled_font)
{
    ...
    scaling_factor = SVGLayoutSupport::CalculateScreenFontSizeScalingFactor(&layout_object);
    if (!scaling_factor) {
        scaling_factor = 1;
    ...
    float scaled_font_size = FontSizeFunctions::GetComputedSizeFromSpecifiedSize(
        &document, scaling_factor, unscaled_font_description.IsAbsoluteSize(), unscaled_font_description.SpecifiedSize(), kDoNotApplyMinimumForFontSize);
    ...
    font_description.SetLetterSpacing(font_description.LetterSpacing() * scaling_factor / zoom);
    font_description.SetWordSpacing(font_description.WordSpacing() * scaling_factor / zoom);

    scaled_font = Font(font_description, document.GetStyleEngine().GetFontSelector());
}
