...
public:
    explicit ResolvedTextLayoutAttributesIterator(const Vector<std::pair<unsigned, SvgCharacterData>>& resolved)
        : resolved_(resolved)
    {
    }
    ResolvedTextLayoutAttributesIterator(const ResolvedTextLayoutAttributesIterator&) = delete;
    ResolvedTextLayoutAttributesIterator& operator=(const ResolvedTextLayoutAttributesIterator&) = delete;
    ...
    const SvgCharacterData& AdvanceTo(unsigned addressable_index)
    {
        ...
        auto resolved_range = base::span(resolved_.data(), resolved_.size()).subspan(index_);
        auto it = base::ranges::find_if(resolved_range, [addressable_index](const auto& pair) { return addressable_index <= pair.first; });
        index_ += static_cast<wtf_size_t>(std::distance(resolved_range.begin(), it));
