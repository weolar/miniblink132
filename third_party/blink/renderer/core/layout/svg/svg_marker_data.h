...
struct MarkerPosition {
    DISALLOW_NEW();
    MarkerPosition(SVGMarkerType use_type, const gfx::PointF& use_origin, float use_angle)
        : type(use_type)
        , origin(use_origin)
        , angle(use_angle)
    {
    ...
    LayoutSVGResourceMarker* SelectMarker(LayoutSVGResourceMarker* marker_start, LayoutSVGResourceMarker* marker_mid, LayoutSVGResourceMarker* marker_end) const
    {
        switch (type) {
        case kStartMarker:
            return marker_start;
...
private:
    ...
    static void ComputeQuadTangents(SegmentData&, const gfx::PointF& start, const gfx::PointF& control, const gfx::PointF& end);
