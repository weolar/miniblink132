...
SVGElementResourceClient& SVGResources::EnsureClient(const LayoutObject& object)
{
    return To<SVGElement>(object.GetNode())->EnsureSVGResourceClient();
}

gfx::RectF SVGResources::ReferenceBoxForEffects(const LayoutObject& layout_object, GeometryBox geometry_box, ForeignObjectQuirk foreign_object_quirk)
{
    ...
    if (layout_object.IsSVGInline()) {
        obb_layout_object = LayoutSVGText::LocateLayoutSVGTextAncestor(&layout_object);
    }
    ...
    if (foreign_object_quirk == ForeignObjectQuirk::kEnabled && obb_layout_object->IsSVGForeignObject()) {
...
void SVGResources::UpdateEffects(LayoutObject& object, StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    if (diff.FilterChanged() && (style.HasFilter() || (old_style && old_style->HasFilter()))) {
    ...
    if (auto* old_reference_clip = DynamicTo<ReferenceClipPathOperation>(old_style->ClipPath())) {
...
void SVGResources::ClearEffects(const LayoutObject& object)
{
    ...
    if (!client)
        return;
    if (auto* old_reference_clip = DynamicTo<ReferenceClipPathOperation>(style->ClipPath())) {
        old_reference_clip->RemoveClient(*client);
    }
    if (style->HasFilter()) {
...
void SVGResources::UpdatePaints(const LayoutObject& object, const ComputedStyle* old_style, const ComputedStyle& style)
{
    const bool had_client = GetClient(object);
...
void SVGResources::ClearPaints(const LayoutObject& object, const ComputedStyle* style)
{
    if (!style)
        return;
...
void SVGResources::UpdateMarkers(const LayoutObject& object, const ComputedStyle* old_style)
{
    const bool had_client = GetClient(object);
    const ComputedStyle& style = object.StyleRef();
...
void SVGResources::ClearMarkers(const LayoutObject& object, const ComputedStyle* style)
{
    if (!style)
        return;
    SVGResourceClient* client = GetClient(object);
...
class SVGElementResourceClient::FilterData final : public GarbageCollected<SVGElementResourceClient::FilterData> {
public:
    ...
    {
        return last_effect_ != nullptr;
    }
    sk_sp<PaintFilter> BuildPaintFilter()
    {
        return paint_filter_builder::Build(last_effect_.Get(), kInterpolationSpaceSRGB);
    }
    ...
    bool Invalidate(SVGFilterPrimitiveStandardAttributes& primitive, const QualifiedName& attribute)
    {
...
template <typename ContainerType> bool ContainsResource(const ContainerType* container, SVGResource* resource)
{
    return container && container->Resource() == resource;
}

bool ContainsResource(const FilterOperations& operations, SVGResource* resource)
{
    return base::ranges::any_of(operations.Operations(),
        [resource](const FilterOperation* operation) { return ContainsResource(DynamicTo<ReferenceFilterOperation>(operation), resource); });
}
...
void SVGElementResourceClient::ResourceContentChanged(SVGResource* resource)
{
    ...
    if (ContainsResource(style.FillPaint().Resource(), resource) || ContainsResource(style.StrokePaint().Resource(), resource)) {
        ...
        layout_object->SetSubtreeShouldDoFullPaintInvalidation(PaintInvalidationReason::kSVGResource);
    }

    bool needs_layout = false;
    if (ContainsResource(style.MarkerStartResource(), resource) || ContainsResource(style.MarkerMidResource(), resource)
        || ContainsResource(style.MarkerEndResource(), resource)) {
        needs_layout = true;
        layout_object->SetNeedsBoundariesUpdate();
    }

    const auto* clip_reference = DynamicTo<ReferenceClipPathOperation>(style.ClipPath());
    ...
    LayoutSVGResourceContainer::MarkForLayoutAndParentResourceInvalidation(*layout_object, needs_layout);
}

void SVGElementResourceClient::FilterPrimitiveChanged(SVGResource* resource, SVGFilterPrimitiveStandardAttributes& primitive, const QualifiedName& attribute)
{
    ...
    layout_object->SetNeedsPaintPropertyUpdate();
    MarkFilterDataDirty();
    LayoutSVGResourceContainer::InvalidateDependentElements(*layout_object, false);
    LayoutSVGResourceContainer::InvalidateAncestorChainResources(*layout_object, false);
}

SVGElementResourceClient::FilterData* SVGElementResourceClient::CreateFilterDataWithNodeMap(
    FilterEffectBuilder& builder, const ReferenceFilterOperation& reference_filter)
{
    auto* node_map = MakeGarbageCollected<SVGFilterGraphNodeMap>();
    Filter* filter = builder.BuildReferenceFilter(reference_filter, nullptr, node_map);
    if (!filter)
        return nullptr;
    paint_filter_builder::PopulateSourceGraphicImageFilters(filter->GetSourceGraphic(), kInterpolationSpaceSRGB);
    return MakeGarbageCollected<FilterData>(filter->LastEffect(), node_map);
}

void SVGElementResourceClient::UpdateFilterData(CompositorFilterOperations& operations)
{
    ...
    if (!operations.IsEmpty() && !filter_data_dirty_ && reference_box == operations.ReferenceBox())
        return;
    const ComputedStyle& style = object.StyleRef();
    FilterEffectBuilder builder(reference_box, std::nullopt, 1, style.VisitedDependentColor(GetCSSPropertyColor()), style.UsedColorScheme());
    ...
    const bool is_single_reference_filter = filter.size() == 1 && IsA<ReferenceFilterOperation>(*filter.at(0));
    if (is_single_reference_filter) {
        if (!filter_data_) {
            filter_data_ = CreateFilterDataWithNodeMap(builder, To<ReferenceFilterOperation>(*filter.at(0)));
        }
        operations.Clear();
        if (filter_data_) {
...
void SVGResourceInvalidator::InvalidatePaints()
{
    ...
    const ComputedStyle& style = object_.StyleRef();
    if (auto* fill = GetSVGResourceAsType<LayoutSVGResourcePaintServer>(*client, style.FillPaint().Resource())) {
        fill->RemoveClientFromCache(*client);
        needs_invalidation = true;
    }
    if (auto* stroke = GetSVGResourceAsType<LayoutSVGResourcePaintServer>(*client, style.StrokePaint().Resource())) {
        stroke->RemoveClientFromCache(*client);
        needs_invalidation = true;
    }
    if (!needs_invalidation)
        return;
    object_.SetSubtreeShouldDoFullPaintInvalidation(PaintInvalidationReason::kSVGResource);
}
