...
void LayoutSVGText::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
...
void LayoutSVGText::SubtreeStructureChanged(LayoutInvalidationReasonForTracing)
{
    NOT_DESTROYED();
...
void LayoutSVGText::UpdateTransformAffectsVectorEffect()
{
    ...
    for (LayoutObject* descendant = FirstChild(); descendant; descendant = descendant->NextInPreOrder(this)) {
        if (descendant->IsSVGInline() && descendant->StyleRef().VectorEffect() == EVectorEffect::kNonScalingStroke) {
            SetTransformAffectsVectorEffect(true);
            break;
        }
    }
...
{
    if (paint_info.phase != PaintPhase::kForeground && paint_info.phase != PaintPhase::kForcedColorsModeBackplate
...
SVGLayoutResult LayoutSVGText::UpdateSVGLayout(const SVGLayoutInfo& layout_info)
{
    NOT_DESTROYED();
...
bool LayoutSVGText::UpdateAfterSVGLayout(const SVGLayoutInfo& layout_info, bool bounds_changed)
{
    if (bounds_changed) {
...
void LayoutSVGText::QuadsInAncestorInternal(Vector<gfx::QuadF>& quads, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    quads.push_back(LocalToAncestorQuad(gfx::QuadF(DecoratedBoundingBox()), ancestor, mode));
}
...
bool LayoutSVGText::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    return LayoutSVGBlock::NodeAtPoint(result, *local_location, accumulated_offset, phase);
}

PositionWithAffinity LayoutSVGText::PositionForPoint(const PhysicalOffset& point_in_contents) const
{
    NOT_DESTROYED();
    ...
    for (const LayoutObject* descendant = FirstChild(); descendant; descendant = descendant->NextInPreOrder(this)) {
        const auto* text = DynamicTo<LayoutSVGInlineText>(descendant);
        if (!text) {
            continue;
        }
        float distance = (descendant->ObjectBoundingBox().ClosestPoint(point) - point).LengthSquared();
...
// static
void LayoutSVGText::NotifySubtreeStructureChanged(LayoutObject* object, LayoutInvalidationReasonForTracing reason)
{
