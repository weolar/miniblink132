...
TEST_F(LayoutSVGTextTest, TransformAffectsVectorEffect)
{
    ...
    GetElementById("tspan1")->setAttribute(svg_names::kVectorEffectAttr, AtomicString("non-scaling-stroke"));
...
<text x="240" y="50" font-size="16" id="t2">bar</text>
    ...
    GetElementById("t")->setAttribute(svg_names::kTransformAttr, AtomicString("scale(0.5)"));
...
<text id="t">foo</text>
    ...
    GetElementById("d")->setAttribute(html_names::kStyleAttr, AtomicString("position:absolute;"));
    UpdateAllLifecyclePhasesForTest();

    // The <text> should be unregistered from all of ancestors.
    GetElementById("svg")->remove();
    GetElementById("to_be_skipped")->setAttribute(html_names::kStyleAttr, AtomicString("transform:rotate(20deg)"));
    UpdateAllLifecyclePhasesForTest();
}
