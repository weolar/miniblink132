...
void UpdateSVGLayoutIfNeeded(LayoutObject* child, const SVGLayoutInfo& layout_info)
{
...
void LayoutMarkerResourcesIfNeeded(LayoutObject& layout_object, const SVGLayoutInfo& layout_info)
{
    SVGElementResourceClient* client = SVGResources::GetClient(layout_object);
    if (!client)
        return;
    const ComputedStyle& style = layout_object.StyleRef();
    if (auto* marker = GetSVGResourceAsType<LayoutSVGResourceMarker>(*client, style.MarkerStartResource()))
        UpdateSVGLayoutIfNeeded(marker, layout_info);
    if (auto* marker = GetSVGResourceAsType<LayoutSVGResourceMarker>(*client, style.MarkerMidResource()))
        UpdateSVGLayoutIfNeeded(marker, layout_info);
    if (auto* marker = GetSVGResourceAsType<LayoutSVGResourceMarker>(*client, style.MarkerEndResource()))
        UpdateSVGLayoutIfNeeded(marker, layout_info);
}

// Update a bounding box taking into account the validity of the other bounding
// box.
inline void UpdateObjectBoundingBox(gfx::RectF& object_bounding_box, bool& object_bounding_box_valid, const gfx::RectF& other_bounding_box)
{
...
bool HasValidBoundingBoxForContainer(const LayoutObject& object)
{
    ...
    if (auto* svg_container = DynamicTo<LayoutSVGContainer>(object)) {
        return svg_container->IsObjectBoundingBoxValid() && !svg_container->IsSVGHiddenContainer();
    }
...
SVGLayoutResult SVGContentContainer::Layout(const SVGLayoutInfo& layout_info)
{
    ...
    result.bounds_changed = std::exchange(bounds_dirty_from_removed_child_, false);

    for (LayoutObject* child = children_.FirstChild(); child; child = child->NextSibling()) {
            ...
            if (auto* element = DynamicTo<SVGElement>(child->GetNode())) {
                if (element->HasRelativeLengths()) {
                    ...
                    if (auto* shape = DynamicTo<LayoutSVGShape>(*child)) {
                        ...
                        ng_text->SetNeedsTextMetricsUpdate();
                    } else if (auto* container = DynamicTo<LayoutSVGTransformableContainer>(*child)) {
                        container->SetNeedsTransformUpdate();
                    }

                    force_child_layout = true;
                }
                if (!child->NeedsLayout() && child->SVGSelfOrDescendantHasViewportDependency()) {
                    force_child_layout = true;
                }
            }
        }
        ...
        if (force_child_layout) {
            child->SetNeedsLayout(layout_invalidation_reason::kSvgChanged, kMarkOnlyThis);
        }
...
bool SVGContentContainer::HitTest(HitTestResult& result, const HitTestLocation& location, HitTestPhase phase) const
{
    PhysicalOffset accumulated_offset;
    for (LayoutObject* child = children_.LastChild(); child; child = child->PreviousSibling()) {
        if (auto* foreign_object = DynamicTo<LayoutSVGForeignObject>(child)) {
            if (foreign_object->NodeAtPointFromSVG(result, location, accumulated_offset, phase)) {
                return true;
            }
        } else {
...
bool SVGContentContainer::UpdateBoundingBoxes()
{
    ...
    for (LayoutObject* current = children_.FirstChild(); current; current = current->NextSibling()) {
        ...
        UpdateObjectBoundingBox(object_bounding_box, object_bounding_box_valid_, transform.MapRect(ObjectBoundsForPropagation(*current)));
        decorated_bounding_box.Union(transform.MapRect(current->DecoratedBoundingBox()));
    }
...
bool SVGContentContainer::ComputeHasNonIsolatedBlendingDescendants() const
{
    for (LayoutObject* child = children_.FirstChild(); child; child = child->NextSibling()) {
        if (child->IsBlendingAllowed() && child->StyleRef().HasBlendMode())
            return true;
        if (child->HasNonIsolatedBlendingDescendants() && !SVGLayoutSupport::WillIsolateBlendingDescendantsForObject(child))
            return true;
    }
    return false;
}
