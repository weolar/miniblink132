...
void LayoutSVGRoot::UnscaledIntrinsicSizingInfo(const SVGRect* override_viewbox, IntrinsicSizingInfo& intrinsic_sizing_info) const
{
    NOT_DESTROYED();
    ...
    intrinsic_sizing_info.size = gfx::SizeF(intrinsic_width.value_or(0), intrinsic_height.value_or(0));
    ...
    if (!intrinsic_sizing_info.size.IsEmpty()) {
        ...
        const SVGRect& view_box = override_viewbox ? *override_viewbox : svg->CurrentViewBox();
...
void LayoutSVGRoot::ComputeIntrinsicSizingInfo(IntrinsicSizingInfo& intrinsic_sizing_info) const
{
    NOT_DESTROYED();
...
double LayoutSVGRoot::LogicalSizeScaleFactorForPercentageLengths() const
{
    ...
    if (!GetDocument().IsInOutermostMainFrame() || GetDocument().GetLayoutView()->ShouldUsePaginatedLayout()) {
        return 1;
    }
...
void LayoutSVGRoot::LayoutRoot(const PhysicalRect& content_rect)
{
    ...
    base::AutoReset<const PhysicalSize*> reset(&new_content_size_, &content_rect.size, nullptr);
    ...
    // any of these.
    SVGTransformChange transform_change = BuildLocalToBorderBoxTransform(content_rect);

    // The scale factor from the local-to-border-box transform is all that our
    // scale-dependent descendants care about.
    const bool screen_scale_factor_changed = transform_change == SVGTransformChange::kFull;

    // selfNeedsLayout() will cover changes to one (or more) of viewBox,
    // current{Scale,Translate}, decorations and 'overflow'.
    const bool viewport_may_have_changed = SelfNeedsFullLayout() || old_content_size != content_rect.size;
    ...
    if (transform_change != SVGTransformChange::kNone || viewport_may_have_changed) {
...
PhysicalRect LayoutSVGRoot::ComputeContentsVisualOverflow() const
{
    ...
    content_visual_rect = local_to_border_box_transform_.MapRect(content_visual_rect);
    ...
    return Intersection(PhysicalRect::EnclosingRect(content_visual_rect), PhysicalRect(InfiniteIntRect()));
}

void LayoutSVGRoot::PaintReplaced(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
...
bool LayoutSVGRoot::IntrinsicSizeIsFontMetricsDependent() const
{
    ...
    return svg.width()->CurrentValue()->IsFontRelative() || svg.height()->CurrentValue()->IsFontRelative();
}

bool LayoutSVGRoot::StyleChangeAffectsIntrinsicSize(const ComputedStyle& old_style) const
{
    NOT_DESTROYED();
    ...
    // dimensions.
    if (IntrinsicSizeIsFontMetricsDependent() && old_style.GetFont() != style.GetFont())
        return true;
    return false;
}
...
void LayoutSVGRoot::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    if (diff.TransformChanged()) {
        for (auto& svg_text : text_set_) {
            svg_text->SetNeedsLayout(layout_invalidation_reason::kStyleChange, kMarkContainerChain);
...
void LayoutSVGRoot::AddChild(LayoutObject* child, LayoutObject* before_child)
{
    ...
    bool should_isolate_descendants = (child->IsBlendingAllowed() && child->StyleRef().HasBlendMode()) || child->HasNonIsolatedBlendingDescendants();
...
void LayoutSVGRoot::RemoveChild(LayoutObject* child)
{
    ...
    bool had_non_isolated_descendants = (child->IsBlendingAllowed() && child->StyleRef().HasBlendMode()) || child->HasNonIsolatedBlendingDescendants();
...
bool LayoutSVGRoot::HasNonIsolatedBlendingDescendants() const
{
    ...
    if (has_non_isolated_blending_descendants_dirty_) {
        has_non_isolated_blending_descendants_ = content_.ComputeHasNonIsolatedBlendingDescendants();
        has_non_isolated_blending_descendants_dirty_ = false;
    }
...
PositionWithAffinity LayoutSVGRoot::PositionForPoint(const PhysicalOffset& point) const
{
    NOT_DESTROYED();
    gfx::PointF absolute_point(point);
    absolute_point = local_to_border_box_transform_.Inverse().MapPoint(absolute_point);
    LayoutObject* closest_descendant = SVGLayoutSupport::FindClosestLayoutSVGText(this, absolute_point);
    ...
    return closest_descendant->PositionForPoint(PhysicalOffset::FromPointFRound(absolute_point));
}

// LayoutBox methods will expect coordinates w/o any transforms in coordinates
// relative to our borderBox origin.  This method gives us exactly that.
SVGTransformChange LayoutSVGRoot::BuildLocalToBorderBoxTransform(const PhysicalRect& content_rect)
{
    NOT_DESTROYED();
    ...
    gfx::SizeF content_size(content_rect.size.width / scale, content_rect.size.height / scale);
    local_to_border_box_transform_ = svg->ViewBoxToViewTransform(content_size);

    gfx::Vector2dF translate = svg->CurrentTranslate();
    AffineTransform view_to_border_box_transform(scale, 0, 0, scale, content_rect.offset.left + translate.x(), content_rect.offset.top + translate.y());
...
AffineTransform LayoutSVGRoot::LocalToSVGParentTransform() const
{
    ...
    return AffineTransform::Translation(RoundToInt(location.left), RoundToInt(location.top)) * local_to_border_box_transform_;
}
...
{
    const PhysicalSize& viewport_size = new_content_size_ ? *new_content_size_ : PhysicalContentBoxSize();
...
// CSS box coordinates.
void LayoutSVGRoot::MapLocalToAncestor(const LayoutBoxModelObject* ancestor, TransformState& transform_state, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    LayoutReplaced::MapLocalToAncestor(ancestor, transform_state, mode);
}

bool LayoutSVGRoot::HitTestChildren(
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    HitTestLocation local_border_box_location(hit_test_location, -accumulated_offset);
    TransformedHitTestLocation local_location(local_border_box_location, LocalToBorderBoxTransform());
    if (!local_location) {
        return false;
    }
...
void LayoutSVGRoot::IntersectChildren(HitTestResult& result, const HitTestLocation& location) const
{
...
OverflowClipAxes LayoutSVGRoot::ComputeOverflowClipAxes() const
{
    ...
    if (StyleRef().OverflowX() == EOverflow::kHidden || StyleRef().OverflowX() == EOverflow::kAuto || StyleRef().OverflowX() == EOverflow::kScroll
