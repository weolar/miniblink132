...
LocalSVGResource* ResourceForContainer(const LayoutSVGResourceContainer& resource_container)
{
    const SVGElement& element = *resource_container.GetElement();
    return element.GetTreeScope().EnsureSVGTreeScopedResources().ExistingResourceForId(element.GetIdAttribute());
}

float ObjectBoundingBoxUnitToUserUnits(const Length& length, float ref_dimension)
{
...
SVGLayoutResult LayoutSVGResourceContainer::UpdateSVGLayout(const SVGLayoutInfo& layout_info)
{
    NOT_DESTROYED();
...
gfx::RectF LayoutSVGResourceContainer::ResolveRectangle(const SVGViewportResolver& viewport_resolver, const SVGLengthConversionData& conversion_data,
    SVGUnitTypes::SVGUnitType type, const gfx::RectF& reference_box, const SVGLength& x, const SVGLength& y, const SVGLength& width, const SVGLength& height,
    const std::optional<gfx::SizeF>& override_viewport)
{
    // Convert SVGLengths to Lengths (preserves percentages).
    const LengthPoint point(x.ConvertToLength(conversion_data), y.ConvertToLength(conversion_data));
    const LengthSize size(width.ConvertToLength(conversion_data), height.ConvertToLength(conversion_data));
    ...
    if (type == SVGUnitTypes::kSvgUnitTypeObjectboundingbox) {
        ...
        resolved_rect = gfx::RectF(ObjectBoundingBoxUnitToUserUnits(point.X(), reference_box.width()),
            ObjectBoundingBoxUnitToUserUnits(point.Y(), reference_box.height()), ObjectBoundingBoxUnitToUserUnits(size.Width(), reference_box.width()),
            ObjectBoundingBoxUnitToUserUnits(size.Height(), reference_box.height()));
        ...
        if (size.Width().MayHavePercentDependence() || size.Height().MayHavePercentDependence() || point.X().HasPercent() || point.Y().HasPercent()) {
            viewport_size_for_resolve = override_viewport.value_or(viewport_resolver.ResolveViewport());
        }
        // Resolve the Lengths to user units.
        resolved_rect = gfx::RectF(PointForLengthPoint(point, viewport_size_for_resolve), SizeForLengthSize(size, viewport_size_for_resolve));
    }
    return resolved_rect;
}

gfx::RectF LayoutSVGResourceContainer::ResolveRectangle(const SVGElement& context_element, SVGUnitTypes::SVGUnitType type, const gfx::RectF& reference_box,
    const SVGLength& x, const SVGLength& y, const SVGLength& width, const SVGLength& height, const std::optional<gfx::SizeF>& override_viewport)
{
    const ComputedStyle* style = SVGLengthContext::ComputedStyleForLengthResolving(context_element);
    if (!style) {
    ...
    return ResolveRectangle(viewport_resolver, conversion_data, type, reference_box, x, y, width, height, override_viewport);
}

gfx::RectF LayoutSVGResourceContainer::ResolveRectangle(SVGUnitTypes::SVGUnitType type, const gfx::RectF& reference_box, const SVGLength& x, const SVGLength& y,
    const SVGLength& width, const SVGLength& height) const
{
    const SVGViewportResolver viewport_resolver(*this);
    const SVGLengthConversionData conversion_data(*this);
    return ResolveRectangle(viewport_resolver, conversion_data, type, reference_box, x, y, width, height);
}
...
void LayoutSVGResourceContainer::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
...
static HeapVector<Member<SVGResource>> CollectResources(const LayoutObject& layout_object)
{
    ...
    for (const auto& operation : style.Filter().Operations()) {
        if (auto* reference_operation = DynamicTo<ReferenceFilterOperation>(*operation))
...
bool LayoutSVGResourceContainer::FindCycleInResources(const LayoutObject& layout_object)
{
    ...
    for (const FillLayer* layer = &layout_object.StyleRef().MaskLayers(); layer; layer = layer->Next()) {
        const auto* mask_source = DynamicTo<StyleMaskSourceImage>(layer->GetImage());
        if (!mask_source) {
            continue;
        }
        const SVGResource* svg_resource = mask_source->GetSVGResource();
        SVGResourceClient* client = mask_source->GetSVGResourceClient(layout_object);
...
bool LayoutSVGResourceContainer::FindCycleInDescendants(const LayoutObject& root)
{
...
bool LayoutSVGResourceContainer::FindCycleInSubtree(const LayoutObject& root)
{
...
void LayoutSVGResourceContainer::MarkAllClientsForInvalidation(InvalidationModeMask invalidation_mask)
{
    NOT_DESTROYED();
    if (is_invalidating_)
        return;
    ...
    if (document.InStyleRecalc() || document.GetStyleEngine().InDetachLayoutTree()) {
...
static inline void RemoveFromCacheAndInvalidateDependencies(LayoutObject& object, bool needs_layout)
{
...
void LayoutSVGResourceContainer::InvalidateDependentElements(LayoutObject& object, bool needs_layout)
{
    ...
    element->NotifyIncomingReferences([needs_layout](SVGElement& element) {
        ...
        LayoutSVGResourceContainer::MarkForLayoutAndParentResourceInvalidation(*element.GetLayoutObject(), needs_layout);
    });
}

void LayoutSVGResourceContainer::InvalidateAncestorChainResources(LayoutObject& object, bool needs_layout)
{
...
void LayoutSVGResourceContainer::MarkForLayoutAndParentResourceInvalidation(LayoutObject& object, bool needs_layout)
{
    DCHECK(object.GetNode());

    if (needs_layout && !object.DocumentBeingDestroyed()) {
        object.SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kSvgResourceInvalidated);
    }
...
static inline bool IsLayoutObjectOfResourceContainer(const LayoutObject& layout_object)
{
...
void LayoutSVGResourceContainer::StyleChanged(LayoutObject& object, StyleDifference diff)
{
    ...
    bool needs_layout = diff.NeedsNormalPaintInvalidation() && IsLayoutObjectOfResourceContainer(object);
