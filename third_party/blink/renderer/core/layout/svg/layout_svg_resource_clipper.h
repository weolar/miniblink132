...
public:
    ...
    bool HitTestClipContent(const gfx::RectF& reference_box, const LayoutObject& reference_box_object, const HitTestLocation&) const;
...
template <> struct DowncastTraits<LayoutSVGResourceClipper> {
...
inline LayoutSVGResourceClipper* GetSVGResourceAsType(SVGResourceClient& client, const ReferenceClipPathOperation& reference_clip)
{
    return GetSVGResourceAsType<LayoutSVGResourceClipper>(client, reference_clip.Resource());
}

inline LayoutSVGResourceClipper* GetSVGResourceAsType(SVGResourceClient& client, const ClipPathOperation* clip_path_operation)
{
    const auto* reference_clip = DynamicTo<ReferenceClipPathOperation>(clip_path_operation);
    if (!reference_clip)
        return nullptr;
