...
PathPositionMapper::PathPositionMapper(const Path& path, float computed_path_length, float start_offset)
...
PathPositionMapper::PositionType PathPositionMapper::PointAndNormalAtLength(float length, PointAndTangent& point_and_tangent)
{
    if (length < 0)
        return kBeforePath;
...
std::unique_ptr<PathPositionMapper> LayoutSVGTextPath::LayoutPath() const
{
    ...
    Element* target_element = SVGURIReference::TargetElementFromIRIString(text_path_element.HrefString(), text_path_element.OriginatingTreeScope());
    ...
    path_data.Transform(path_element->CalculateTransform(SVGElement::kIncludeMotionTransform));
    ...
    if (!std::isnan(author_path_length)) {
        offset_scale = SVGGeometryElement::PathLengthScaleFactor(computed_path_length, author_path_length);
    } else {
        author_path_length = computed_path_length;
    }

    const SVGLengthConversionData conversion_data(*this);
    float path_start_offset = text_path_element.startOffset()->CurrentValue()->Value(conversion_data, author_path_length);
    path_start_offset *= offset_scale;

    return std::make_unique<PathPositionMapper>(path_data, computed_path_length, path_start_offset);
}
