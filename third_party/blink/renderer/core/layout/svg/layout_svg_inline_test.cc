...
<tspan id="container">abc<a id="target">foo</a></tspan>
    ...
    LayoutSVGInline* container = To<LayoutSVGInline>(GetLayoutObjectByElementId("container"));
    EXPECT_NE(target->LocalToAbsolutePoint(PhysicalOffset()), target->LocalToAncestorPoint(PhysicalOffset(), container));
}
