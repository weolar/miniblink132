...
void LayoutSVGImage::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
...
gfx::SizeF LayoutSVGImage::CalculateObjectSize() const
{
    ...
    gfx::Vector2dF style_size = VectorForLengthPair(StyleRef().Width(), StyleRef().Height(), viewport_resolver, StyleRef());
    ...
    const gfx::SizeF kDefaultObjectSize(LayoutReplaced::kDefaultWidth, LayoutReplaced::kDefaultHeight);
    ...
    const gfx::SizeF concrete_object_size = ConcreteObjectSize(sizing_info, kDefaultObjectSize);
    if (width_is_auto && height_is_auto) {
    ...
    if (height_is_auto) {
        if (has_intrinsic_ratio) {
            return gfx::SizeF(style_size.x(), ResolveHeightForRatio(style_size.x(), sizing_info.aspect_ratio));
        }
    ...
    if (has_intrinsic_ratio) {
        return gfx::SizeF(ResolveWidthForRatio(style_size.y(), sizing_info.aspect_ratio), style_size.y());
    }
...
bool LayoutSVGImage::UpdateBoundingBox()
{
    ...
    object_bounding_box_.set_origin(PointForLengthPair(style.X(), style.Y(), viewport_resolver, style));
    object_bounding_box_.set_size(CalculateObjectSize());

    return old_object_bounding_box != object_bounding_box_;
}

SVGLayoutResult LayoutSVGImage::UpdateSVGLayout(const SVGLayoutInfo& layout_info)
{
    NOT_DESTROYED();
    DCHECK(NeedsLayout());
...
bool LayoutSVGImage::UpdateAfterSVGLayout(const SVGLayoutInfo& layout_info, bool bbox_changed)
{
    if (bbox_changed) {
...
bool LayoutSVGImage::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    ...
    const ComputedStyle& style = StyleRef();
    PointerEventsHitRules hit_rules(PointerEventsHitRules::kSvgImageHitTesting, result.GetHitTestRequest(), style.UsedPointerEvents());
