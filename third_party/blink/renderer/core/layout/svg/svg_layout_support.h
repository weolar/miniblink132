...
public:
    ...
    static void AdjustWithClipPathAndMask(const LayoutObject& layout_object, const gfx::RectF& object_bounding_box, gfx::RectF& visual_rect);

    // Add any contribution from 'stroke' to a text content bounding rect.
    static gfx::RectF ExtendTextBBoxWithStroke(const LayoutObject&, const gfx::RectF& text_bounds);

    // Compute the visual rect for the a text content LayoutObject.
    static gfx::RectF ComputeVisualRectForText(const LayoutObject&, const gfx::RectF& text_bounds);
    ...
    static PhysicalRect VisualRectInAncestorSpace(const LayoutObject&, const LayoutBoxModelObject& ancestor, VisualRectFlags = kDefaultVisualRectFlags);
    static bool MapToVisualRectInAncestorSpace(const LayoutObject&, const LayoutBoxModelObject* ancestor, const gfx::RectF& local_visual_rect,
        PhysicalRect& result_rect, VisualRectFlags = kDefaultVisualRectFlags);
    static void MapLocalToAncestor(const LayoutObject*, const LayoutBoxModelObject* ancestor, TransformState&, MapCoordinatesFlags);
    static void MapAncestorToLocal(const LayoutObject&, const LayoutBoxModelObject* ancestor, TransformState&, MapCoordinatesFlags);

    // Shared between SVG layoutObjects and resources.
    static void ApplyStrokeStyleToStrokeData(StrokeData&, const ComputedStyle&, const LayoutObject&, float dash_scale_factor);

    static DashArray ResolveSVGDashArray(const SVGDashArray&, const ComputedStyle&, const SVGViewportResolver&);
    ...
    static LayoutObject* FindClosestLayoutSVGText(const LayoutObject*, const gfx::PointF&);
};
