...
void LayoutSVGShape::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    transform_uses_reference_box_ = TransformHelper::UpdateReferenceBoxDependency(*this);
    ...
    if (old_style) {
        ...
        if (!diff.NeedsFullLayout() && stroke_path_cache_) {
            if (old_style->StrokeDashOffset() != style.StrokeDashOffset() || *old_style->StrokeDashArray() != *style.StrokeDashArray()) {
...
bool CanUseSimpleStrokeApproximation(LayoutSVGShape::GeometryType geometry_type)
{
    return geometry_type == LayoutSVGShape::GeometryType::kRectangle || geometry_type == LayoutSVGShape::GeometryType::kRoundedRectangle
        || geometry_type == LayoutSVGShape::GeometryType::kEllipse || geometry_type == LayoutSVGShape::GeometryType::kCircle;
}
...
{
    return geometry_type == LayoutSVGShape::GeometryType::kPath || geometry_type == LayoutSVGShape::GeometryType::kLine;
}

} // namespace

gfx::RectF LayoutSVGShape::ApproximateStrokeBoundingBox(const gfx::RectF& shape_bounds) const
{
    NOT_DESTROYED();
...
gfx::RectF LayoutSVGShape::StrokeBoundingBox() const
{
    ...
    SVGLayoutSupport::ApplyStrokeStyleToStrokeData(stroke_data, StyleRef(), *this, DashScaleFactor());
...
bool LayoutSVGShape::ShapeDependentStrokeContains(const HitTestLocation& location)
{
    NOT_DESTROYED();
    if (!stroke_path_cache_) {
        ...
        if (HasNonScalingStroke()) {
            ...
            root_transform.Scale(StyleRef().EffectiveZoom()).PreConcat(NonScalingStrokeTransform());
        ...
        stroke_path_cache_ = std::make_unique<Path>(path->StrokePath(stroke_data, root_transform));
    }
    DCHECK(stroke_path_cache_);
    ...
    TransformedHitTestLocation host_space_location(location, host_space_transform, TransformedHitTestLocation::kDontComputeInverse);
    DCHECK(host_space_location);
    return host_space_location->Intersects(*stroke_path_cache_, RULE_NONZERO);
}

bool LayoutSVGShape::ShapeDependentFillContains(const HitTestLocation& location, const WindRule fill_rule) const
{
    NOT_DESTROYED();
...
static bool HasPaintServer(const LayoutObject& object, const SVGPaint& paint)
{
    ...
    if (paint.HasUrl()) {
        ...
        if (GetSVGResourceAsType<LayoutSVGResourcePaintServer>(*client, paint.Resource()))
            return true;
    }
    return false;
}

bool LayoutSVGShape::FillContains(const HitTestLocation& location, bool requires_fill, const WindRule fill_rule)
{
    NOT_DESTROYED();
...
bool LayoutSVGShape::StrokeContains(const HitTestLocation& location, bool requires_stroke)
{
    NOT_DESTROYED();
...
SVGLayoutResult LayoutSVGShape::UpdateSVGLayout(const SVGLayoutInfo& layout_info)
{
    NOT_DESTROYED();
...
bool LayoutSVGShape::UpdateAfterSVGLayout(const SVGLayoutInfo& layout_info, bool bbox_changed)
{
    if (bbox_changed) {
    ...
    if (!needs_transform_update_ && transform_uses_reference_box_) {
        needs_transform_update_ = CheckForImplicitTransformChange(layout_info, bbox_changed);
        if (needs_transform_update_)
            SetNeedsPaintPropertyUpdate();
    }
    bool updated_transform = false;
    if (needs_transform_update_) {
        local_transform_ = TransformHelper::ComputeTransformIncludingMotion(*GetElement());
...
AffineTransform LayoutSVGShape::ComputeRootTransform() const
{
    ...
    while (root && !root->IsSVGRoot())
        root = root->Parent();
    return AffineTransform::FromTransform(LocalToAncestorTransform(To<LayoutSVGRoot>(root)));
}
...
AffineTransform LayoutSVGShape::ComputeNonScalingStrokeTransform() const
{
    ...
    host_transform.Scale(1 / StyleRef().EffectiveZoom()).PreConcat(ComputeRootTransform());
...
bool LayoutSVGShape::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    ...
    if (IsShapeEmpty())
        return false;
    const ComputedStyle& style = StyleRef();
    const PointerEventsHitRules hit_rules(PointerEventsHitRules::kSvgGeometryHitTesting, result.GetHitTestRequest(), style.UsedPointerEvents());
    if (hit_rules.require_visible && style.Visibility() != EVisibility::kVisible) {
        return false;
    }

    TransformedHitTestLocation local_location(hit_test_location, LocalToSVGParentTransform());
    if (!local_location)
        return false;
    ...
    if (HitTestShape(result.GetHitTestRequest(), *local_location, hit_rules)) {
        UpdateHitTestResult(result, PhysicalOffset::FromPointFRound(local_location->TransformedPoint()));
        if (result.AddNodeToListBasedTestResult(GetElement(), *local_location) == kStopHitTesting)
            return true;
    }
...
bool LayoutSVGShape::HitTestShape(const HitTestRequest& request, const HitTestLocation& local_location, PointerEventsHitRules hit_rules)
{
    NOT_DESTROYED();
    if (hit_rules.can_hit_bounding_box && local_location.Intersects(ObjectBoundingBox()))
        return true;

    // TODO(chrishtr): support rect-based intersections in the cases below.
    const ComputedStyle& style = StyleRef();
    if (hit_rules.can_hit_stroke && (style.HasStroke() || !hit_rules.require_stroke) && StrokeContains(local_location, hit_rules.require_stroke))
        return true;
    WindRule fill_rule = style.FillRule();
    if (request.SvgClipContent())
        fill_rule = style.ClipRule();
    if (hit_rules.can_hit_fill && (style.HasFill() || !hit_rules.require_fill) && FillContains(local_location, hit_rules.require_fill, fill_rule))
        return true;
    return false;
}
...
gfx::RectF LayoutSVGShape::CalculateNonScalingStrokeBoundingBox() const
{
    ...
    if (non_scaling_transform.IsInvertible()) {
        gfx::RectF stroke_bounding_rect = ApproximateStrokeBoundingBox(NonScalingStrokePath().BoundingRect());
        stroke_bounding_rect = non_scaling_transform.Inverse().MapRect(stroke_bounding_rect);
...
float LayoutSVGShape::StrokeWidthForMarkerUnits() const
{
    ...
    if (HasNonScalingStroke()) {
        ...
        if (!non_scaling_transform.IsInvertible())
            return 0;
        float scale_factor = ClampTo<float>(sqrt((non_scaling_transform.XScaleSquared() + non_scaling_transform.YScaleSquared()) / 2));
