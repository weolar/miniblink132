...
bool StrokeBoundingBoxMayHaveChanged(const ComputedStyle& old_style, const ComputedStyle& style)
{
    return old_style.StrokeWidth() != style.StrokeWidth() || old_style.CapStyle() != style.CapStyle()
        || old_style.StrokeMiterLimit() != style.StrokeMiterLimit() || old_style.JoinStyle() != style.JoinStyle();
}
...
static inline bool TransformOriginIsFixed(const ComputedStyle& style)
{
    ...
    return style.TransformBox() == ETransformBox::kViewBox && style.GetTransformOrigin().X().IsFixed() && style.GetTransformOrigin().Y().IsFixed();
}

// static
void TransformHelper::UpdateOffsetPath(SVGElement& element, const ComputedStyle* old_style)
{
...
bool TransformHelper::DependsOnReferenceBox(const ComputedStyle& style)
{
    ...
    // explicitly later.
    if (!TransformOriginIsFixed(style) && style.RequireTransformOrigin(ComputedStyle::kIncludeTransformOrigin, ComputedStyle::kExcludeMotionPath))
        return true;
...
bool TransformHelper::UpdateReferenceBoxDependency(LayoutObject& layout_object)
{
    const bool transform_uses_reference_box = DependsOnReferenceBox(layout_object.StyleRef());
    UpdateReferenceBoxDependency(layout_object, transform_uses_reference_box);
    return transform_uses_reference_box;
}

void TransformHelper::UpdateReferenceBoxDependency(LayoutObject& layout_object, bool transform_uses_reference_box)
{
    if (transform_uses_reference_box && layout_object.StyleRef().TransformBox() == ETransformBox::kViewBox) {
...
bool TransformHelper::CheckReferenceBoxDependencies(const ComputedStyle& old_style, const ComputedStyle& style)
{
    const ETransformBox transform_box = style.UsedTransformBox(ComputedStyle::TransformBoxContext::kSvg);
...
gfx::RectF TransformHelper::ComputeReferenceBox(const LayoutObject& layout_object)
{
...
AffineTransform TransformHelper::ComputeTransform(
    UseCounter& use_counter, const ComputedStyle& style, const gfx::RectF& reference_box, ComputedStyle::ApplyTransformOrigin apply_transform_origin)
{
    ...
    style.ApplyTransform(transform, nullptr, reference_box, ComputedStyle::kIncludeTransformOperations, apply_transform_origin,
...
AffineTransform TransformHelper::ComputeTransformIncludingMotion(const SVGElement& element, const gfx::RectF& reference_box)
{
    const LayoutObject& layout_object = *element.GetLayoutObject();
    if (layout_object.HasTransform() || element.HasMotionTransform()) {
        AffineTransform matrix = ComputeTransform(element.GetDocument(), layout_object.StyleRef(), reference_box, ComputedStyle::kIncludeTransformOrigin);
...
AffineTransform TransformHelper::ComputeTransformIncludingMotion(const SVGElement& element)
{
    const LayoutObject& layout_object = *element.GetLayoutObject();
...
gfx::PointF TransformHelper::ComputeTransformOrigin(const ComputedStyle& style, const gfx::RectF& reference_box)
{
    gfx::PointF origin(FloatValueForLength(style.GetTransformOrigin().X(), reference_box.width()) + reference_box.x(),
        FloatValueForLength(style.GetTransformOrigin().Y(), reference_box.height()) + reference_box.y());
