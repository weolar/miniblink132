...
public:
    ...
    void SetupSvgHitTest()
    {
        ...
        html.AppendFormat("<svg width='%.2f' height='%.2f' viewBox='0 0 %.2f %.2f' "
                          "style='cursor: auto;'>",
        ...
        for (size_t row = 0; row < num_rows; ++row) {
            for (size_t column = 0; column < num_columns; ++column) {
                ...
                html.AppendFormat("<rect id='leaf_%zu' x='%.2f' y='%.2f' width='%.2f' height='%.2f' "
                                  "/>",
                    column * row, column * column_stride, row * row_stride, column_stride, row_stride);
    ...
    void RunTest(const std::string& story, base::RepeatingCallback<void()> test_case)
    {
    ...
    RunTest("HandleMouseMoveEvent",
        WTF::BindRepeating(
            [](EventHandler* event_handler) {
                WebMouseEvent mouse_move_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(1, 1), gfx::PointF(1, 1), WebPointerProperties::Button::kNoButton,
                    0, WebInputEvent::Modifiers::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
                mouse_move_event.SetFrameScale(1);
                event_handler->HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
            },
...
TEST_F(SvgHitTestPerfTest, IntersectsClipPath)
{
    ...
    PhysicalOffset document_point = event_handling_util::ContentPointFromRootFrame(GetDocument().GetFrame(), gfx::PointF(1, 1));

    TransformedHitTestLocation local_location(HitTestLocation(document_point), container->LocalToSVGParentTransform());
    ASSERT_TRUE(local_location);

    RunTest("IntersectsClipPath",
        WTF::BindRepeating(
            [](const LayoutObject* container, TransformedHitTestLocation& local_location) {
                container->HasClipPath() && ClipPathClipper::HitTest(*container, *local_location);
            },
