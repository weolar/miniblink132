...
class CORE_EXPORT ColumnLayoutAlgorithm : public LayoutAlgorithm<BlockNode, BoxFragmentBuilder, BlockBreakToken> {
public:
    ...
    static const PhysicalBoxFragment& CreateEmptyColumn(const BlockNode&, const ConstraintSpace& parent_space, const PhysicalBoxFragment& previous_column);

private:
    MinMaxSizesResult ComputeSpannersMinMaxSizes(const BlockNode& search_parent) const;
    ...
    const LayoutResult* LayoutRow(const BlockBreakToken* next_column_token, LayoutUnit miminum_column_block_size, MarginStrut*);
    ...
    // columns.
    BreakStatus LayoutSpanner(BlockNode spanner_node, const BlockBreakToken* break_token, MarginStrut*);
    ...
    void AttemptToPositionListMarker(const PhysicalBoxFragment& child_fragment, LayoutUnit block_offset);
    ...
    void PropagateBaselineFromChild(const PhysicalBoxFragment& child, LayoutUnit block_offset);
    ...
    LayoutUnit ResolveColumnAutoBlockSize(const LogicalSize& column_size, LayoutUnit row_offset, LayoutUnit available_outer_space,
        const BlockBreakToken* child_break_token, bool balance_columns);

    LayoutUnit ResolveColumnAutoBlockSizeInternal(const LogicalSize& column_size, LayoutUnit row_offset, LayoutUnit available_outer_space,
        const BlockBreakToken* child_break_token, bool balance_columns);

    LayoutUnit ConstrainColumnBlockSize(LayoutUnit size, LayoutUnit row_offset, LayoutUnit available_outer_space) const;
    ...
    ConstraintSpace CreateConstraintSpaceForBalancing(const LogicalSize& column_size) const;
    ConstraintSpace CreateConstraintSpaceForSpanner(const BlockNode& spanner, LayoutUnit block_offset) const;
