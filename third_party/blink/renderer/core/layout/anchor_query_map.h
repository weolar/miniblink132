...
public:
    ...
    LogicalAnchorQueryMap(const LayoutBox& root_box, const LogicalFragmentLinkVector& children, WritingDirectionMode writing_direction);

    bool IsEmpty() const
    {
        return !has_anchor_queries_;
    }
    ...
    const LogicalAnchorQuery& AnchorQuery(const LayoutObject& containing_block) const;
...
private:
    ...
    mutable HeapHashMap<Member<const LayoutObject>, Member<LogicalAnchorQuery>> queries_;
