...
public:
    ...
    PhysicalRect ReplacedContentRectFrom(const PhysicalRect& base_content_rect) const final;
...
private:
    ...
    bool NodeAtPointOverEmbeddedContentView(HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset, HitTestPhase);

    bool PointOverResizer(const HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset) const;

    void PropagateZoomFactor(double zoom_factor);
};

template <> struct DowncastTraits<LayoutEmbeddedContent> {
