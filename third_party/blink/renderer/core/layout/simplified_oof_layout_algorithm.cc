...
SimplifiedOofLayoutAlgorithm::SimplifiedOofLayoutAlgorithm(const LayoutAlgorithmParams& params, const PhysicalBoxFragment& last_fragmentainer)
    : LayoutAlgorithm(params)
{
    ...
    container_builder_.SetFragmentBlockSize(params.space.FragmentainerBlockSize());
    container_builder_.SetHasOutOfFlowFragmentChild(true);
}

void SimplifiedOofLayoutAlgorithm::ResumeColumnLayout(const BlockBreakToken* old_fragment_break_token)
{
    if (!old_fragment_break_token || !old_fragment_break_token->IsCausedByColumnSpanner()) {
        return;
    }
    ...
    for (const auto& child_break_token : old_fragment_break_token->ChildBreakTokens()) {
...
void SimplifiedOofLayoutAlgorithm::AppendOutOfFlowResult(const LayoutResult* result)
{
