    ...
    : LayoutBlockFlow(nullptr)
{
    ...
    ComputedStyleBuilder new_style_builder
        = GetDocument().GetStyleResolver().CreateAnonymousStyleBuilderWithDisplay(GetDocument().GetLayoutView()->StyleRef(), EDisplay::kBlock);
    SetStyle(new_style_builder.TakeStyle());
}

LayoutViewTransitionRoot::~LayoutViewTransitionRoot() = default;

void LayoutViewTransitionRoot::UpdateSnapshotStyle(const ViewTransitionStyleTracker& style_tracker)
{
    PhysicalRect snapshot_containing_block_rect(
        PhysicalOffset(style_tracker.GetFixedToSnapshotRootOffset()), PhysicalSize(style_tracker.GetSnapshotRootSize()));
    ...
    new_style_builder.SetWidth(Length::Fixed(snapshot_containing_block_rect.Width()));
    new_style_builder.SetHeight(Length::Fixed(snapshot_containing_block_rect.Height()));
