...
protected:
    ...
    void SimulateInput()
    {
        GetLayoutShiftTracker().NotifyInput(WebMouseEvent(WebInputEvent::Type::kMouseDown, gfx::PointF(), gfx::PointF(), WebPointerProperties::Button::kLeft, 0,
...
TEST_F(LayoutShiftTrackerTest, IgnoreAfterInput)
{
    ...
    GetElementById("j")->setAttribute(html_names::kStyleAttr, AtomicString("top: 60px"));
    SimulateInput();
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(0.0, GetLayoutShiftTracker().Score());
    EXPECT_TRUE(GetLayoutShiftTracker().ObservedInputOrScroll());
    EXPECT_TRUE(GetLayoutShiftTracker().MostRecentInputTimestamp().since_origin().InSecondsF() > 0.0);
}
...
TEST_F(LayoutShiftTrackerTest, CompositedShiftBeforeFirstPaint)
{
    ...
    GetFrameView().UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest);
    GetElementById("A")->setAttribute(html_names::kClassAttr, AtomicString("hide"));
    UpdateAllLifecyclePhasesForTest();
}
...
TEST_F(LayoutShiftTrackerTest, IgnoreSVG)
{
    ...
    GetDocument().QuerySelector(AtomicString("circle"))->setAttribute(svg_names::kCxAttr, AtomicString("100"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(LayoutShiftTrackerSimTest, SubframeWeighting)
{
    ...
    WebLocalFrameImpl& child_frame = To<WebLocalFrameImpl>(*MainFrame().FirstChild());

    Element* div = child_frame.GetFrame()->GetDocument()->getElementById(AtomicString("j"));
    ...
    LayoutShiftTracker& layout_shift_tracker = child_frame.GetFrameView()->GetLayoutShiftTracker();
    ...
    GetDocument().getElementById(AtomicString("i"))->setAttribute(html_names::kStyleAttr, AtomicString("left: 600px"));
...
TEST_F(LayoutShiftTrackerSimTest, ViewportSizeChange)
{
    ...
    LayoutShiftTracker& layout_shift_tracker = MainFrame().GetFrameView()->GetLayoutShiftTracker();
...
void LayoutShiftTrackerNavigationTest::RunTest(bool is_browser_initiated)
{
    ...
    Persistent<HistoryItem> item1 = main_frame->Loader().GetDocumentLoader()->GetHistoryItem();

    WebMouseEvent event1(WebInputEvent::Type::kMouseDown, gfx::PointF(), gfx::PointF(), WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
    WebMouseEvent event2(WebInputEvent::Type::kMouseUp, gfx::PointF(), gfx::PointF(), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
    ...
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(event1, ui::LatencyInfo()));
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(event2, ui::LatencyInfo()));
    ...
    Persistent<HistoryItem> item2 = main_frame->Loader().GetDocumentLoader()->GetHistoryItem();

    main_frame->Loader().GetDocumentLoader()->CommitSameDocumentNavigation(item1->Url(), WebFrameLoadType::kBackForward, item1.Get(),
    ...
    auto entries = perf.getBufferedEntriesByType(performance_entry_names::kLayoutShift);
    ...
    const double expected_cls_score = is_browser_initiated ? 0 : expected_shift_value;
...
TEST_F(LayoutShiftTrackerNavigationTest, BrowserInitiatedSameDocumentHistoryNavigation)
{
    RunTest(true /* is_browser_initiated */);
}

TEST_F(LayoutShiftTrackerNavigationTest, RendererInitiatedSameDocumentHistoryNavigation)
{
...
void LayoutShiftTrackerPointerdownTest::RunTest(WebInputEvent::Type completion_type, bool expect_exclusion)
{
    SimRequest main_resource("https://example.com/", "text/html");
    LoadURL("https://example.com/");
    main_resource.Complete(R"HTML(
    ...
    WebPointerProperties pointer_properties
        = WebPointerProperties(1 /* PointerId */, WebPointerProperties::PointerType::kTouch, WebPointerProperties::Button::kLeft);

    WebPointerEvent event1(WebInputEvent::Type::kPointerDown, pointer_properties, 5, 5);
    ...
    EXPECT_EQ(0u, perf.getBufferedEntriesByType(performance_entry_names::kLayoutShift).size());
...
{
    RunTest(WebInputEvent::Type::kPointerCausedUaAction, false /* expect_exclusion */);
}
...
TEST_F(LayoutShiftTrackerSimTest, MouseMoveDraggingAction)
{
    ...
    WebMouseEvent event1(WebInputEvent::Type::kMouseDown, gfx::PointF(), gfx::PointF(), WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
    WebMouseEvent event2(WebInputEvent::Type::kMouseMove, gfx::PointF(), gfx::PointF(), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
    WebMouseEvent event3(WebInputEvent::Type::kMouseUp, gfx::PointF(), gfx::PointF(), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
    ...
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(event3, ui::LatencyInfo()));
...
TEST_F(LayoutShiftTrackerSimTest, TouchDraggingAction)
{
    ...
    WebPointerEvent event1(WebInputEvent::Type::kPointerDown, pointer_properties, 5, 5);
    WebPointerEvent event2(WebInputEvent::Type::kPointerMove, pointer_properties, 5, 5);
    WebPointerEvent event3(WebInputEvent::Type::kPointerUp, pointer_properties, 5, 5);
...
TEST_F(LayoutShiftTrackerSimTest, TouchScrollingAction)
{
    ...
    WebPointerEvent event1(WebInputEvent::Type::kPointerDown, pointer_properties, 5, 5);
    WebPointerEvent event2(WebInputEvent::Type::kPointerMove, pointer_properties, 5, 5);
    WebPointerEvent event3(WebInputEvent::Type::kPointerCancel, pointer_properties, 5, 5);
    WebPointerEvent event4(WebInputEvent::Type::kPointerMove, pointer_properties, 5, 5);
    ...
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(event4, ui::LatencyInfo()));
    ...
    entries = perf.getBufferedEntriesByType(performance_entry_names::kLayoutShift);
    EXPECT_EQ(2u, entries.size());
...
TEST_F(LayoutShiftTrackerSimTest, MultiplePointerDownUps)
{
    ...
    WebPointerEvent event1(WebInputEvent::Type::kPointerDown, pointer_properties, 5, 5);
    WebPointerEvent event2(WebInputEvent::Type::kPointerDown, pointer_properties, 5, 5);
    WebPointerEvent event3(WebInputEvent::Type::kPointerUp, pointer_properties, 5, 5);
    WebPointerEvent event4(WebInputEvent::Type::kPointerUp, pointer_properties, 5, 5);
...
TEST_F(LayoutShiftTrackerTest, ContentVisibilityAutoOffscreenAfterScrollFirstPaint)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      .auto {
...
TEST_F(LayoutShiftTrackerTest, ContentVisibilityAutoOnscreenAndOffscreenAfterScrollFirstPaint)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      .auto {
    ...
    // report layout shift.
    To<Element>(offscreen->GetNode())->setAttribute(html_names::kStyleAttr, AtomicString("position: relative; top: 100100px"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(LayoutShiftTrackerTest, ClipByVisualViewport)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    // The visual viewport.
    EXPECT_EQ(gfx::Rect(0, 100, 200, 500), GetDocument().View()->GetScrollableArea()->VisibleContentRect());
    // The layout viewport .
    EXPECT_EQ(gfx::Rect(0, 0, 800, 600), GetDocument().View()->LayoutViewport()->VisibleContentRect());
    EXPECT_FLOAT_EQ(0, GetLayoutShiftTracker().Score());
    ...
    EXPECT_FLOAT_EQ(50.0 * (100.0 + 100.0) / (200.0 * 500.0) * (100.0 / 500.0), GetLayoutShiftTracker().Score());
}
...
TEST_F(LayoutShiftTrackerTest, NeedsToTrack)
{
    ...
    EXPECT_TRUE(tracker.NeedsToTrack(*GetLayoutObjectByElementId("empty-parent")));
    EXPECT_FALSE(tracker.NeedsToTrack(*GetLayoutObjectByElementId("empty")));
    EXPECT_TRUE(tracker.NeedsToTrack(*GetLayoutObjectByElementId("hidden-parent")));
    EXPECT_FALSE(tracker.NeedsToTrack(*GetLayoutObjectByElementId("hidden")));
    EXPECT_TRUE(tracker.NeedsToTrack(*GetLayoutObjectByElementId("visible-under-hidden")));
    ...
    {
        LayoutShiftTracker::ContainingBlockScope scope(PhysicalSize(1, 2), PhysicalSize(2, 3), PhysicalRect(1, 2, 3, 4), PhysicalRect(2, 3, 4, 5));
...
TEST_F(LayoutShiftTrackerTest, AnimatingTransformCreatesLayoutShiftRoot)
{
    ...
    GetElementById("animation")->setAttribute(html_names::kStyleAttr, AtomicString("top: 400px"));
    ...
    GetElementById("child")->setAttribute(html_names::kStyleAttr, AtomicString("top: 2px"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FLOAT_EQ(0, GetLayoutShiftTracker().Score());
}
