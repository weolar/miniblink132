...
public:
    ...
    int ComputeUnderlineOffsetForUnder(const Length& style_underline_offset, float computed_font_size, const SimpleFontData* font_data,
        float text_decoration_thickness, FontVerticalPositionType) const;

    int ComputeUnderlineOffset(ResolvedUnderlinePosition, float computed_font_size, const SimpleFontData* font_data, const Length& style_underline_offset,
        float text_decoration_thickness) const;

private:
    static float StyleUnderlineOffsetToPixels(const Length& style_underline_offset, float font_size);
