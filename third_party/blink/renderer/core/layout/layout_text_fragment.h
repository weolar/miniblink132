...
public:
    ...
    static LayoutTextFragment* Create(Node*, const String&, int start_offset, int length);
    static LayoutTextFragment* CreateAnonymous(Document&, const String&);
    static LayoutTextFragment* CreateAnonymous(Document&, const String&, unsigned start, unsigned length);

    void Trace(Visitor*) const override;

    Position PositionForCaretOffset(unsigned) const override;
    std::optional<unsigned> CaretOffsetForPosition(const Position&) const override;
...
template <> struct DowncastTraits<LayoutTextFragment> {
