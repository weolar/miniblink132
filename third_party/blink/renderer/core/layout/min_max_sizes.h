...
struct CORE_EXPORT MinMaxSizes {
    ...
    bool IsEmpty() const
    {
        return !min_size && max_size == LayoutUnit::Max();
    }
    ...
    void operator=(LayoutUnit value)
    {
...
struct MinMaxSizesResult {
    ...
    MinMaxSizesResult(MinMaxSizes sizes, bool depends_on_block_constraints, bool applied_aspect_ratio)
        : sizes(sizes)
        , depends_on_block_constraints(depends_on_block_constraints)
