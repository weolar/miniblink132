...
bool HasImageSourceURL(const Node& node)
{
    ...
    if (IsA<HTMLImageElement>(node) || (html_input_element && html_input_element->FormControlType() == FormControlType::kInputImage)) {
        return true;
    }
    ...
    if (layout_object->IsImage() && (IsA<HTMLEmbedElement>(node) || IsA<HTMLObjectElement>(node))) {
        return true;
    }
...
HitTestResult::HitTestResult(const HitTestRequest& other_request, const HitTestLocation& location)
    ...
    , is_over_scroll_corner_(other.is_over_scroll_corner_)
{
    ...
    list_based_test_result_ = other.list_based_test_result_ ? MakeGarbageCollected<NodeSet>(*other.list_based_test_result_) : nullptr;
}
...
{
    return hit_test_request_.EqualForCacheability(other.hit_test_request_) && inner_node_ == other.InnerNode() && inner_element_ == other.InnerElement()
        && inner_possibly_pseudo_node_ == other.InnerPossiblyPseudoNode() && point_in_inner_node_frame_ == other.point_in_inner_node_frame_
        && local_point_ == other.LocalPoint() && inner_url_element_ == other.URLElement() && scrollbar_ == other.GetScrollbar()
        && is_over_embedded_content_view_ == other.IsOverEmbeddedContentView();
}

void HitTestResult::CacheValues(const HitTestResult& other)
{
    hit_test_request_ = other.hit_test_request_.GetType() & ~HitTestRequest::kAvoidCache;
}
...
void HitTestResult::SetNodeAndPosition(Node* node, const PhysicalBoxFragment* box_fragment, const PhysicalOffset& position)
{
    if (box_fragment) {
...
void HitTestResult::OverrideNodeAndPosition(Node* node, PhysicalOffset position)
{
    local_point_ = position;
    SetInnerNode(node);
}
...
PositionWithAffinity HitTestResult::GetPosition() const
{
    ...
    DCHECK_GE(node->GetDocument().Lifecycle().GetState(), DocumentLifecycle::kPrePaintClean);
    ...
    if (node->IsPseudoElement() && node->GetPseudoId() == kPseudoIdBefore) {
        return PositionWithAffinity(MostForwardCaretPosition(Position::FirstPositionInNode(*inner_node_)));
    }
...
PositionWithAffinity HitTestResult::GetPositionForInnerNodeOrImageMapImage() const
{
...
CompositorElementId HitTestResult::GetScrollableContainer() const
{
    ...
    while (cur_box) {
        if (cur_box->IsGlobalRootScroller() || (cur_box->IsScrollContainer() && cur_box->GetScrollableArea()->ScrollsOverflow())) {
    ...
    return InnerNode()->GetDocument().GetPage()->GetVisualViewport().GetScrollElementId();
}
...
HTMLAreaElement* HitTestResult::ImageAreaForImage() const
{
    ...
    if (!image_element && inner_node_->IsInShadowTree()) {
        if (inner_node_->ContainingShadowRoot()->IsUserAgent()) {
            image_element = DynamicTo<HTMLImageElement>(inner_node_->OwnerShadowHost());
        }
    }

    if (!image_element || !image_element->GetLayoutObject() || !image_element->GetLayoutObject()->IsBox())
        return nullptr;

    HTMLMapElement* map = image_element->GetTreeScope().GetImageMap(image_element->FastGetAttribute(html_names::kUsemapAttr));
    if (!map)
        return nullptr;
...
String HitTestResult::Title(TextDirection& dir) const
{
    ...
    // using it.
    for (Node* title_node = inner_node_.Get(); title_node; title_node = FlatTreeTraversal::Parent(*title_node)) {
...
Image* HitTestResult::GetImage(const Node* node)
{
    ...
    const ImageResourceContent* image_content = layout_image_resource ? layout_image_resource->CachedImage() : nullptr;
...
gfx::Rect HitTestResult::ImageRect() const
{
    if (!GetImage())
        return gfx::Rect();
    return gfx::ToEnclosingRect(InnerNodeOrImageMapImage()->GetLayoutBox()->AbsoluteContentQuad().BoundingBox());
}
...
KURL HitTestResult::AbsoluteImageURL(const Node* node)
{
    ...
    return node->GetDocument().CompleteURL(StripLeadingAndTrailingHTMLSpaces(url_string));
}
...
HTMLMediaElement* HitTestResult::MediaElement() const
{
    ...
    if (!(inner_node_->GetLayoutObject() && inner_node_->GetLayoutObject()->IsMedia()))
        return nullptr;
...
std::tuple<bool, ListBasedHitTestBehavior> HitTestResult::AddNodeToListBasedTestResultInternal(Node* node, const HitTestLocation& location)
{
    ...
    if (GetHitTestRequest().PenetratingList()) {
        ...
        if (GetHitTestRequest().UseHitNodeCb()) {
            LocalFrameView::InvalidationDisallowedScope invalidation_disallowed(*node->GetDocument().View());
...
ListBasedHitTestBehavior HitTestResult::AddNodeToListBasedTestResult(Node* node, const HitTestLocation& location, const PhysicalRect& rect)
{
    bool should_check_containment;
    ListBasedHitTestBehavior behavior;
    std::tie(should_check_containment, behavior) = AddNodeToListBasedTestResultInternal(node, location);
    if (!should_check_containment)
        return behavior;
    return rect.Contains(location.BoundingBox()) ? kStopHitTesting : kContinueHitTesting;
}

ListBasedHitTestBehavior HitTestResult::AddNodeToListBasedTestResult(Node* node, const HitTestLocation& location, const gfx::QuadF& quad)
{
    bool should_check_containment;
    ListBasedHitTestBehavior behavior;
    std::tie(should_check_containment, behavior) = AddNodeToListBasedTestResultInternal(node, location);
    if (!should_check_containment)
        return behavior;
    return quad.ContainsQuad(gfx::QuadF(gfx::RectF(location.BoundingBox()))) ? kStopHitTesting : kContinueHitTesting;
}

ListBasedHitTestBehavior HitTestResult::AddNodeToListBasedTestResult(Node* node, const HitTestLocation& location, const cc::Region& region)
{
    bool should_check_containment;
    ListBasedHitTestBehavior behavior;
    std::tie(should_check_containment, behavior) = AddNodeToListBasedTestResultInternal(node, location);
    if (!should_check_containment)
        return behavior;
    return region.Contains(location.ToEnclosingRect()) ? kStopHitTesting : kContinueHitTesting;
}
...
void HitTestResult::Append(const HitTestResult& other)
{
    ...
    if (other.list_based_test_result_) {
        ...
        for (NodeSet::const_iterator it = other.list_based_test_result_->begin(), last = other.list_based_test_result_->end(); it != last; ++it)
            set.insert(it->Get());
    }
}
...
HitTestLocation HitTestResult::ResolveRectBasedTest(Node* resolved_inner_node, const PhysicalOffset& resolved_point_in_main_frame)
{
