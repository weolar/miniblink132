...
// intrinsic-sizes.
class CustomLayoutWorkTask final : public GarbageCollected<CustomLayoutWorkTask> {
public:
    enum TaskType {
        kLayoutFragment,
        kIntrinsicSizes,
    ...
    CustomLayoutWorkTask(CustomLayoutChild*, CustomLayoutToken*, ScriptPromiseResolverBase*, const TaskType type);

    // Used when resolving a promise with a fragment.
    CustomLayoutWorkTask(CustomLayoutChild*, CustomLayoutToken*, ScriptPromiseResolverBase*, const CustomLayoutConstraintsOptions*,
        scoped_refptr<SerializedScriptValue> constraint_data, const TaskType type);
    ~CustomLayoutWorkTask();
    void Trace(Visitor*) const;

    // Runs this work task.
    void Run(const ConstraintSpace& parent_space, const ComputedStyle& parent_style, const LayoutUnit child_available_block_size,
...
private:
    ...
    void RunLayoutFragmentTask(const ConstraintSpace& parent_space, const ComputedStyle& parent_style, LayoutInputNode child);
    void RunIntrinsicSizesTask(const ConstraintSpace& parent_space, const ComputedStyle& parent_style, const LayoutUnit child_available_block_size,
        LayoutInputNode child, bool* child_depends_on_block_constraints);
};
