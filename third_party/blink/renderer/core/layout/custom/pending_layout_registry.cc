    ...
    if (it != pending_layouts_.end()) {
        for (const auto& node : *it->value) {
            ...
            if (node && node->GetLayoutObject()) {
                ...
                if (style.IsDisplayLayoutCustomBox() && style.DisplayLayoutCustomName() == name)
...
void PendingLayoutRegistry::AddPendingLayout(const AtomicString& name, Node* node)
{
    Member<PendingSet>& set = pending_layouts_.insert(name, nullptr).stored_value->value;
    if (!set)
