...
CustomLayoutAlgorithm::CustomLayoutAlgorithm(const LayoutAlgorithmParams& params)
...
MinMaxSizesResult CustomLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput& input)
{
    ...
    LogicalSize border_box_size { container_builder_.InlineSize(),
        ComputeBlockSizeForFragment(GetConstraintSpace(), Node(), BorderPadding(),
            CalculateDefaultBlockSize(GetConstraintSpace(), Node(), GetBreakToken(), BorderScrollbarPadding()), container_builder_.InlineSize()) };
    if (!instance->IntrinsicSizes(GetConstraintSpace(), document, Node(), border_box_size, BorderScrollbarPadding(), ChildAvailableSize().block_size, &scope,
    ...
    MinMaxSizes sizes;
    sizes.max_size = LayoutUnit::FromDoubleRound(intrinsic_sizes_result_options->maxContentSize());
    sizes.min_size = std::min(sizes.max_size, LayoutUnit::FromDoubleRound(intrinsic_sizes_result_options->minContentSize()));
...
const LayoutResult* CustomLayoutAlgorithm::Layout()
{
    ...
    LogicalSize border_box_size { container_builder_.InlineSize(),
        ComputeBlockSizeForFragment(GetConstraintSpace(), Node(), BorderPadding(),
            CalculateDefaultBlockSize(GetConstraintSpace(), Node(), GetBreakToken(), BorderScrollbarPadding()), container_builder_.InlineSize()) };
    if (!instance->Layout(
            GetConstraintSpace(), document, Node(), border_box_size, BorderScrollbarPadding(), &scope, fragment_result_options, &fragment_result_data)) {
        // TODO(ikilpatrick): Report this error to the developer.
        return FallbackLayout();
    }

    const HeapVector<Member<CustomLayoutFragment>>& child_fragments = fragment_result_options->childFragments();
    ...
    for (auto fragment : child_fragments) {
        ...
        LayoutUnit inline_offset = LayoutUnit::FromDoubleRound(fragment->inlineOffset());
        LayoutUnit block_offset = LayoutUnit::FromDoubleRound(fragment->blockOffset());
        container_builder_.AddResult(fragment->GetLayoutResult(), { inline_offset, block_offset });
    ...
    LayoutUnit auto_block_size = std::max(BorderScrollbarPadding().BlockSum(), LayoutUnit::FromDoubleRound(fragment_result_options->autoBlockSize()));
    LayoutUnit block_size
        = ComputeBlockSizeForFragment(GetConstraintSpace(), Node(), BorderPadding(), auto_block_size, container_builder_.InitialBorderBoxSize().inline_size);

    // TODO(ikilpatrick): Allow setting both the first/last baseline instead of a
    // general baseline.
    if (fragment_result_options->hasBaseline()) {
        LayoutUnit baseline = LayoutUnit::FromDoubleRound(effective_zoom * fragment_result_options->baseline());
...
void CustomLayoutAlgorithm::AddAnyOutOfFlowPositionedChildren(LayoutInputNode* child)
{
    DCHECK(child);
...
MinMaxSizesResult CustomLayoutAlgorithm::FallbackMinMaxSizes(const MinMaxSizesFloatInput& input) const
{
