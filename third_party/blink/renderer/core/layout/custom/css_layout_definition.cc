...
void GatherChildren(const BlockNode& node, CustomLayoutScope* custom_layout_scope, HeapVector<Member<CustomLayoutChild>>* children)
{
...
CSSLayoutDefinition::CSSLayoutDefinition(ScriptState* script_state, V8NoArgumentConstructor* constructor, V8IntrinsicSizesCallback* intrinsic_sizes,
    V8LayoutCallback* layout, const Vector<CSSPropertyID>& native_invalidation_properties, const Vector<AtomicString>& custom_invalidation_properties,
    const Vector<CSSPropertyID>& child_native_invalidation_properties, const Vector<AtomicString>& child_custom_invalidation_properties)
...
CSSLayoutDefinition::Instance::Instance(CSSLayoutDefinition* definition, v8::Local<v8::Value> instance)
    : definition_(definition)
    , instance_(definition->GetScriptState()->GetIsolate(), instance)
{
}

bool CSSLayoutDefinition::Instance::Layout(const ConstraintSpace& space, const Document& document, const BlockNode& node, const LogicalSize& border_box_size,
    const BoxStrut& border_scrollbar_padding, CustomLayoutScope* custom_layout_scope, FragmentResultOptions*& fragment_result_options,
    scoped_refptr<SerializedScriptValue>* fragment_result_data)
{
    ...
    CustomLayoutEdges* edges = MakeGarbageCollected<CustomLayoutEdges>(border_scrollbar_padding);

    CustomLayoutConstraints* constraints = MakeGarbageCollected<CustomLayoutConstraints>(border_box_size, space.CustomLayoutData(), isolate);
    ...
    StylePropertyMapReadOnly* style_map = MakeGarbageCollected<PrepopulatedComputedStylePropertyMap>(
        document, node.Style(), definition_->native_invalidation_properties_, definition_->custom_invalidation_properties_);
    ...
    ScriptValue return_value;
    {
        ...
        if (!definition_->layout_->Invoke(instance_.Get(isolate), children, edges, constraints, style_map).To(&return_value)) {
            return false;
        }
    }
    ...
    if (v8_return_value.IsEmpty() || !v8_return_value->IsPromise()) {
        execution_context->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kInfo,
    ...
    v8::Local<v8::Promise> v8_result_promise = v8::Local<v8::Promise>::Cast(v8_return_value);
    ...
    fragment_result_options = NativeValueTraits<FragmentResultOptions>::NativeValue(isolate, inner_value, PassThroughException(isolate));
    ...
    if (fragment_result_options->hasData()) {
        ...
        *fragment_result_data = SerializedScriptValue::Serialize(isolate, fragment_result_options->data().V8Value(),
            SerializedScriptValue::SerializeOptions(SerializedScriptValue::kForStorage), PassThroughException(isolate));
    }
...
bool CSSLayoutDefinition::Instance::IntrinsicSizes(const ConstraintSpace& space, const Document& document, const BlockNode& node,
    const LogicalSize& border_box_size, const BoxStrut& border_scrollbar_padding, const LayoutUnit child_available_block_size,
    CustomLayoutScope* custom_layout_scope, IntrinsicSizesResultOptions** intrinsic_sizes_result_options, bool* child_depends_on_block_constraints)
{
    ScriptState* script_state = definition_->GetScriptState();
    v8::Isolate* isolate = script_state->GetIsolate();
    ...
    {
        v8::MicrotasksScope microtasks_scope(isolate, microtask_queue, v8::MicrotasksScope::kRunMicrotasks);
        if (!definition_->intrinsic_sizes_->Invoke(instance_.Get(isolate), children, edges, style_map).To(&return_value)) {
            return false;
        }
    }
    ...
    while (!queue.empty()) {
        {
            ...
            for (wtf_size_t index = 0; index < queue.size(); ++index) {
                auto task = queue[index];
                task->Run(space, node.Style(), child_available_block_size, child_depends_on_block_constraints);
            }
            queue.clear();
        }
        microtask_queue->PerformCheckpoint(isolate);
    ...
    // Attempt to convert the result.
    v8::TryCatch try_catch(isolate);
    *intrinsic_sizes_result_options = NativeValueTraits<IntrinsicSizesResultOptions>::NativeValue(isolate, inner_value, PassThroughException(isolate));
