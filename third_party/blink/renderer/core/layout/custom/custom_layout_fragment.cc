...
CustomLayoutFragment::CustomLayoutFragment(CustomLayoutChild* child, CustomLayoutToken* token, const LayoutResult* layout_result, const LogicalSize& size,
    const std::optional<LayoutUnit> baseline, v8::Isolate* isolate)
    : child_(child)
    , token_(token)
...
ScriptValue CustomLayoutFragment::data(ScriptState* script_state) const
{
    ...
    return ScriptValue(script_state->GetIsolate(), layout_worklet_world_v8_data_.Get(script_state->GetIsolate()));
}
