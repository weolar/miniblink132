...
CustomLayoutChild::CustomLayoutChild(const CSSLayoutDefinition& definition, LayoutInputNode node)
    : node_(node)
    , style_map_(MakeGarbageCollected<PrepopulatedComputedStylePropertyMap>(
          node.GetDocument(), node.Style(), definition.ChildNativeInvalidationProperties(), definition.ChildCustomInvalidationProperties()))
{
}

ScriptPromise<CustomIntrinsicSizes> CustomLayoutChild::intrinsicSizes(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    if (!node_ || !token_->IsValid()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, kInvalidLayoutChild);
        return EmptyPromise();
    }

    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<CustomIntrinsicSizes>>(script_state, exception_state.GetContext());
    CustomLayoutScope::Current()->Queue()->emplace_back(
        MakeGarbageCollected<CustomLayoutWorkTask>(this, token_, resolver, CustomLayoutWorkTask::TaskType::kIntrinsicSizes));
    return resolver->Promise();
}

ScriptPromise<CustomLayoutFragment> CustomLayoutChild::layoutNextFragment(
    ScriptState* script_state, const CustomLayoutConstraintsOptions* options, ExceptionState& exception_state)
{
    // A layout child may be invalid if it has been removed from the tree (it is
    // possible for a web developer to hold onto a LayoutChild object after its
    // underlying LayoutObject has been destroyed).
    ...
    if (options->hasData()) {
        ...
        if (!data->IsNullOrUndefined()) {
            ...
            constraint_data = SerializedScriptValue::Serialize(
                script_state->GetIsolate(), data, SerializedScriptValue::SerializeOptions(SerializedScriptValue::kForStorage), exception_state);
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<CustomLayoutFragment>>(script_state, exception_state.GetContext());
    CustomLayoutScope::Current()->Queue()->emplace_back(MakeGarbageCollected<CustomLayoutWorkTask>(
        this, token_, resolver, options, std::move(constraint_data), CustomLayoutWorkTask::TaskType::kLayoutFragment));
    return resolver->Promise();
}
