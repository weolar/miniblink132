...
{
    LayoutWorklet* supplement = Supplement<LocalDOMWindow>::From<LayoutWorklet>(window);
...
WorkletGlobalScopeProxy* LayoutWorklet::CreateGlobalScope()
{
    ...
    return MakeGarbageCollected<LayoutWorkletGlobalScopeProxy>(
        To<LocalDOMWindow>(GetExecutionContext())->GetFrame(), ModuleResponsesMap(), pending_layout_registry_, GetNumberOfGlobalScopes() + 1);
}
