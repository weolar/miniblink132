...
public:
    void SetUp() override
    {
        ...
        layout_worklet_ = MakeGarbageCollected<LayoutWorklet>(*GetDocument().domWindow());
    ...
    ScriptEvaluationResult EvaluateScriptModule(const String& source_code)
    {
        ...
        v8::MicrotasksScope microtasks_scope(script_state->GetIsolate(), ToMicrotaskQueue(script_state), v8::MicrotasksScope::kDoNotRunMicrotasks);
        ...
        return JSModuleScript::CreateForTest(Modulator::From(script_state), module, js_url)->RunScriptOnScriptStateAndReturnValue(script_state);
    }
...
TEST_F(LayoutWorkletTest, ParseProperties)
{
    ...
    CSSLayoutDefinition* definition = global_scope->FindDefinition(AtomicString("foo"));
    EXPECT_NE(nullptr, definition);

    Vector<CSSPropertyID> native_invalidation_properties = { CSSPropertyID::kFlexBasis };
    Vector<AtomicString> custom_invalidation_properties = { AtomicString("--prop") };
    Vector<CSSPropertyID> child_native_invalidation_properties = { CSSPropertyID::kMarginTop };
    Vector<AtomicString> child_custom_invalidation_properties = { AtomicString("--child-prop") };

    EXPECT_EQ(native_invalidation_properties, definition->NativeInvalidationProperties());
    EXPECT_EQ(custom_invalidation_properties, definition->CustomInvalidationProperties());
    EXPECT_EQ(child_native_invalidation_properties, definition->ChildNativeInvalidationProperties());
    EXPECT_EQ(child_custom_invalidation_properties, definition->ChildCustomInvalidationProperties());
}
