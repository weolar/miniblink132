...
void LayoutBlock::StyleWillChange(StyleDifference diff, const ComputedStyle& new_style)
{
    NOT_DESTROYED();
...
static double ComputeSquaredLocalFontSizeScalingFactor(const gfx::Transform* transform)
{
    if (!transform)
        return 1;
...
void LayoutBlock::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    if (Layer() && diff.TransformChanged() && has_svg_text_descendants_) {
        old_squared_scale = ComputeSquaredLocalFontSizeScalingFactor(Layer()->Transform());
    }
    ...
    if (old_style && Parent()) {
        if (old_style->GetPosition() != new_style.GetPosition() && new_style.GetPosition() != EPosition::kStatic) {
    ...
    if (diff.TransformChanged() && has_svg_text_descendants_) {
        const double new_squared_scale = ComputeSquaredLocalFontSizeScalingFactor(Layer() ? Layer()->Transform() : nullptr);
        // Compare local scale before and after.
        if (old_squared_scale != new_squared_scale) {
            bool stacking_context_changed = old_style && (IsStackingContext(*old_style) != IsStackingContext(new_style));
            for (LayoutBox* box : *View()->SvgTextDescendantsMap().at(this)) {
                To<LayoutSVGText>(box)->SetNeedsTextMetricsUpdate();
                if (GetNode() == GetDocument().documentElement() || stacking_context_changed) {
...
void LayoutBlock::AddChildBeforeDescendant(LayoutObject* new_child, LayoutObject* before_descendant)
{
    NOT_DESTROYED();
    ...
    if (before_descendant_container->IsAnonymousBlock()) {
        ...
        if ((new_child->IsInline() && !new_child->IsLayoutOutsideListMarker())
            || (new_child->IsFloatingOrOutOfFlowPositioned() && (StyleRef().IsDeprecatedFlexboxUsingFlexLayout() || (!IsFlexibleBox() && !IsLayoutGrid())))
...
void LayoutBlock::AddChild(LayoutObject* new_child, LayoutObject* before_child)
{
    ...
    if (new_child->IsInline()
        ...
        LayoutObject* after_child = before_child ? before_child->PreviousSibling() : LastChild();
...
void LayoutBlock::Paint(const PaintInfo& paint_info) const
{
    ...
    DCHECK(IsMonolithic() || !CanTraversePhysicalFragments() || !Parent()->CanTraversePhysicalFragments());
    // We may get here in multiple-fragment cases if the object is repeated
    // (inside table headers and footers, for instance).
    DCHECK(PhysicalFragmentCount() <= 1u || GetPhysicalFragment(0)->GetBreakToken()->IsRepeated());
...
void LayoutBlock::InvalidatePaint(const PaintInvalidatorContext& context) const
{
    NOT_DESTROYED();
    BlockPaintInvalidator(*this).InvalidatePaint(context);
}

void LayoutBlock::ImageChanged(WrappedImagePtr image, CanDeferInvalidation defer)
{
    NOT_DESTROYED();
    ...
    const auto* first_line_style = StyleRef().GetCachedPseudoElementStyle(kPseudoIdFirstLine);
    if (!first_line_style)
        return;
    if (auto* first_line_container = NearestInnerBlockWithFirstLine()) {
        for (const auto* layer = &first_line_style->BackgroundLayers(); layer; layer = layer->Next()) {
...
void LayoutBlock::RemovePositionedObjects(LayoutObject* stay_within)
{
    ...
    auto ProcessPositionedObjectRemoval = [&](LayoutObject* positioned_object) {
        if (stay_within && (!positioned_object->IsDescendantOf(stay_within) || stay_within == positioned_object)) {
            return false;
        }
...
void LayoutBlock::AddSvgTextDescendant(LayoutBox& svg_text)
{
    ...
    if (result.is_new_entry) {
        result.stored_value->value = MakeGarbageCollected<TrackedLayoutBoxLinkedHashSet>();
    }
...
bool LayoutBlock::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
...
bool LayoutBlock::HitTestChildren(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    DCHECK(!ChildrenInline());
    ...
    for (LayoutBox* child = LastChildBox(); child; child = child->PreviousSiblingBox()) {
        ...
        PhysicalOffset child_accumulated_offset = scrolled_offset + child->PhysicalLocation(this);
        bool did_hit;
        if (child->IsFloating()) {
            ...
            did_hit = child->HitTestAllPhases(result, hit_test_location, child_accumulated_offset);
        } else {
            did_hit = child->NodeAtPoint(result, hit_test_location, child_accumulated_offset, child_hit_test);
        }
        if (did_hit) {
...
PositionWithAffinity LayoutBlock::PositionForPointIfOutsideAtomicInlineLevel(const PhysicalOffset& point) const
{
    NOT_DESTROYED();
    DCHECK(IsAtomicInlineLevel());
    LogicalOffset logical_offset = point.ConvertToLogical({ StyleRef().GetWritingMode(), ResolvedDirection() }, PhysicalSize(Size()), PhysicalSize());
...
PositionWithAffinity LayoutBlock::PositionForPoint(const PhysicalOffset& point) const
{
    NOT_DESTROYED();
...
std::optional<LayoutUnit> LayoutBlock::BaselineForEmptyLine() const
{
    ...
    if (RuntimeEnabledFeatures::SidewaysWritingModesEnabled()) {
        int ascent_or_descent = IsFlippedLinesWritingMode(style->GetWritingMode()) ? font_metrics.Descent(baseline_type) : font_metrics.Ascent(baseline_type);
        return LayoutUnit((ascent_or_descent + (line_height - font_metrics.Height()) / 2 + BorderAndPaddingBlockStart()).ToInt());
    }
    const LayoutUnit border_padding = style->IsHorizontalWritingMode() ? BorderTop() + PaddingTop() : BorderRight() + PaddingRight();
    return LayoutUnit((font_metrics.Ascent(baseline_type) + (line_height - font_metrics.Height()) / 2 + border_padding).ToInt());
}
...
LayoutBlockFlow* LayoutBlock::NearestInnerBlockWithFirstLine()
{
    ...
    for (LayoutObject* child = FirstChild(); child && !child->IsFloatingOrOutOfFlowPositioned() && child->IsLayoutBlockFlow();
...
{
    NOT_DESTROYED();
    return IsInline() && IsAtomicInlineLevel() && !Size().IsEmpty() && GetNode() && EditingIgnoresContent(*GetNode());
}
...
PhysicalRect LayoutBlock::LocalCaretRect(int caret_offset) const
{
    ...
    LogicalRect caret_rect = LocalCaretRectForEmptyElement(inline_size, TextIndentOffset());
    ...
    OutlineRectCollector& collector, OutlineInfo* info, const PhysicalOffset& additional_offset, OutlineType include_block_overflows) const
{
    ...
    if (ShouldIncludeBlockInkOverflow(include_block_overflows) && !HasNonVisibleOverflow() && !HasControlClip()) {
...
LayoutBlock* LayoutBlock::CreateAnonymousWithParentAndDisplay(const LayoutObject* parent, EDisplay display)
{
    ...
    ComputedStyleBuilder new_style_builder = parent->GetDocument().GetStyleResolver().CreateAnonymousStyleBuilderWithDisplay(parent->StyleRef(), new_display);
    ...
    if (new_display == EDisplay::kFlex) {
        ...
        DCHECK(new_display == EDisplay::kBlock || new_display == EDisplay::kFlowRoot);
