...
bool LayoutBoxUtils::SkipContainingBlockForPercentHeightCalculation(const LayoutBlock* cb)
{
...
LayoutUnit LayoutBoxUtils::InlineSize(const LayoutBox& box)
{
    ...
    // size.
    return box.GetPhysicalFragment(0u)->Size().ConvertToLogical(box.StyleRef().GetWritingMode()).inline_size;
}
...
LayoutUnit LayoutBoxUtils::TotalBlockSize(const LayoutBox& box)
{
    ...
    while (num_fragments > 0) {
        LayoutUnit block_size = box.GetPhysicalFragment(num_fragments - 1)->Size().ConvertToLogical(box.StyleRef().GetWritingMode()).block_size;
    ...
    if (num_fragments > 1) {
        total_block_size += box.GetPhysicalFragment(num_fragments - 2)->GetBreakToken()->ConsumedBlockSize();
    }
    return total_block_size;
}

// static
LayoutPoint LayoutBoxUtils::ComputeLocation(const PhysicalBoxFragment& child_fragment, PhysicalOffset offset, const PhysicalBoxFragment& container_fragment,
    const BlockBreakToken* previous_container_break_token)
{
    if (container_fragment.Style().IsFlippedBlocksWritingMode()) [[unlikely]] {
        ...
        offset.left = container_fragment.Size().width - offset.left - child_fragment.Size().width;
    }
    ...
    if (previous_container_break_token) [[unlikely]] {
        ...
        LayoutUnit consumed = previous_container_break_token->ConsumedBlockSizeForLegacy();
