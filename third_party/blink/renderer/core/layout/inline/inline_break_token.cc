...
// static
InlineBreakToken* InlineBreakToken::Create(InlineNode node, const ComputedStyle* style, const InlineItemTextIndex& start,
    unsigned flags /* InlineBreakTokenFlags */, const BlockBreakToken* sub_break_token, const RubyBreakTokenData* ruby_data)
{
    ...
    return MakeGarbageCollected<InlineBreakToken>(AdditionalBytes(size), PassKey(), node, style, start, flags, sub_break_token, ruby_data);
}

// static
InlineBreakToken* InlineBreakToken::CreateForParallelBlockFlow(InlineNode node, const InlineItemTextIndex& start, const BlockBreakToken& child_break_token)
{
    return Create(node, &node.Style(), start, kIsInParallelBlockFlow, &child_break_token);
}

InlineBreakToken::InlineBreakToken(PassKey key, InlineNode node, const ComputedStyle* style, const InlineItemTextIndex& start,
    unsigned flags /* InlineBreakTokenFlags */, const BlockBreakToken* sub_break_token, const RubyBreakTokenData* ruby_data)
    : BreakToken(kInlineBreakToken, node, flags)
    , style_(style)
    , start_(start)
{
    if (sub_break_token || ruby_data) [[unlikely]] {
...
String InlineBreakToken::ToString() const
{
    ...
    string_builder.Append(String::Format("InlineBreakToken index:%u offset:%u", StartItemIndex(), StartTextOffset()));
