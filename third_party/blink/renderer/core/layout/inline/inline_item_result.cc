...
InlineItemResult::InlineItemResult(const InlineItem* item, unsigned index, const TextOffsetRange& text_offset, bool break_anywhere_if_overflow,
    bool should_create_line_box, bool has_unpositioned_floats)
    : item(item)
    , item_index(index)
...
String InlineItemResult::ToString(const String& ifc_text_content, const String& indent) const
{
    ...
    builder.Append(" ");
    if (item->Type() == InlineItem::kText) {
        builder.Append(ifc_text_content.Substring(TextOffset().start, TextOffset().Length()).EncodeForDebugging());
