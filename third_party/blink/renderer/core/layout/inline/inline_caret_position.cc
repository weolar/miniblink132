...
bool CanResolveInlineCaretPositionBeforeFragment(const InlineCursor& cursor, TextAffinity affinity)
{
...
bool CanResolveInlineCaretPositionAfterFragment(const InlineCursor& cursor, TextAffinity affinity)
{
...
// on |affinity|.
InlineCaretPositionResolution TryResolveInlineCaretPositionInTextFragment(const InlineCursor& cursor, unsigned offset, TextAffinity affinity)
{
    if (cursor.Current().IsGeneratedText()) {
        return InlineCaretPositionResolution();
    }

    const OffsetMapping& mapping = *OffsetMapping::GetFor(cursor.Current().GetLayoutObject());
    ...
    if (offset < start_offset && !mapping.HasBidiControlCharactersOnly(offset, start_offset)) {
        return InlineCaretPositionResolution();
    }
    if (affinity == TextAffinity::kUpstream && offset == current_offset.end + 1 && cursor.Current().Style().NeedsTrailingSpace()
        && cursor.Current().Style().IsCollapsibleWhiteSpace(mapping.GetText()[offset - 1])) {
        // |offset| is after soft line wrap, e.g. "abc |xyz".
        // See http://crbug.com/1183269 and |AdjustForSoftLineWrap()|
        return { ResolutionType::kResolved, { cursor, InlineCaretPositionType::kAtTextOffset, offset - 1 } };
    }
    if (offset > current_offset.end && !mapping.HasBidiControlCharactersOnly(end_offset, offset)) {
        return InlineCaretPositionResolution();
    }

    offset = std::max(offset, start_offset);
    offset = std::min(offset, end_offset);
    InlineCaretPosition candidate = { cursor, InlineCaretPositionType::kAtTextOffset, offset };
    ...
    if (offset == start_offset && CanResolveInlineCaretPositionBeforeFragment(cursor, affinity)) {
        return { ResolutionType::kResolved, candidate };
    }

    if (offset == end_offset && !cursor.Current().IsLineBreak() && CanResolveInlineCaretPositionAfterFragment(cursor, affinity)) {
        return { ResolutionType::kResolved, candidate };
    }
...
unsigned GetTextOffsetBefore(const Node& node)
{
    ...
    std::optional<unsigned> maybe_offset_before = OffsetMapping::GetFor(before_node)->GetTextContentOffset(before_node);
...
InlineCaretPositionResolution TryResolveInlineCaretPositionByBoxFragmentSide(const InlineCursor& cursor, unsigned offset, TextAffinity affinity)
{
    ...
    const InlineCaretPositionType position_type = offset == offset_before ? InlineCaretPositionType::kBeforeBox : InlineCaretPositionType::kAfterBox;
    InlineCaretPosition candidate { cursor, position_type, std::nullopt };

    if (offset == offset_before && CanResolveInlineCaretPositionBeforeFragment(cursor, affinity)) {
        return { ResolutionType::kResolved, candidate };
    }

    if (offset == offset_after && CanResolveInlineCaretPositionAfterFragment(cursor, affinity)) {
        return { ResolutionType::kResolved, candidate };
    }
...
InlineCaretPositionResolution TryResolveInlineCaretPositionWithFragment(const InlineCursor& cursor, unsigned offset, TextAffinity affinity)
{
    if (cursor.Current().IsText()) {
        return TryResolveInlineCaretPositionInTextFragment(cursor, offset, affinity);
    }
    if (cursor.Current().IsAtomicInline()) {
        return TryResolveInlineCaretPositionByBoxFragmentSide(cursor, offset, affinity);
    }
    return InlineCaretPositionResolution();
}
...
bool NeedsBidiAdjustment(const InlineCaretPosition& caret_position)
{
    ...
    return *caret_position.text_offset == start_offset || *caret_position.text_offset == end_offset;
}

InlineCaretPosition AdjustInlineCaretPositionForBidiText(const InlineCaretPosition& caret_position)
{
...
InlineCaretPosition BetterCandidateBetween(const InlineCaretPosition& current, const InlineCaretPosition& other, unsigned offset)
{
    DCHECK(!other.IsNull());
    if (current.IsNull()) {
        return other;
    ...
    DCHECK(current.ToPositionInDOMTreeWithAffinity().Affinity() == TextAffinity::kUpstream
        || other.ToPositionInDOMTreeWithAffinity().Affinity() == TextAffinity::kUpstream);
...
InlineCaretPosition ComputeInlineCaretPositionAfterInline(const PositionWithAffinity& position_with_affinity)
{
    const Position& position = position_with_affinity.GetPosition();
    const LayoutInline& layout_inline = *To<LayoutInline>(position.AnchorNode()->GetLayoutObject());
    ...
    if (cursor.Current().IsText()) {
        const unsigned offset
            = line.Current().BaseDirection() == cursor.Current().ResolvedDirection() ? cursor.Current()->EndOffset() : cursor.Current()->StartOffset();
        return AdjustInlineCaretPositionForBidiText({ cursor, InlineCaretPositionType::kAtTextOffset, offset });
    }

    if (cursor.Current().IsAtomicInline()) {
        const InlineCaretPositionType type
            = line.Current().BaseDirection() == cursor.Current().ResolvedDirection() ? InlineCaretPositionType::kAfterBox : InlineCaretPositionType::kBeforeBox;
        return AdjustInlineCaretPositionForBidiText({ cursor, type, std::nullopt });
    }

    return AdjustInlineCaretPositionForBidiText({ cursor, InlineCaretPositionType::kAfterBox, std::nullopt });
}
InlineCaretPosition ComputeInlineCaretPositionBeforeInline(const PositionWithAffinity& position_with_affinity)
{
    const Position& position = position_with_affinity.GetPosition();
    const LayoutInline& layout_inline = *To<LayoutInline>(position.AnchorNode()->GetLayoutObject());
    ...
    if (cursor.Current().IsText()) {
        const unsigned offset
            = line.Current().BaseDirection() == cursor.Current().ResolvedDirection() ? cursor.Current()->StartOffset() : cursor.Current()->EndOffset();
        return AdjustInlineCaretPositionForBidiText({ cursor, InlineCaretPositionType::kAtTextOffset, offset });
    }

    if (cursor.Current().IsAtomicInline()) {
        const InlineCaretPositionType type
            = line.Current().BaseDirection() == cursor.Current().ResolvedDirection() ? InlineCaretPositionType::kBeforeBox : InlineCaretPositionType::kAfterBox;
        return AdjustInlineCaretPositionForBidiText({ cursor, type, std::nullopt });
    }

    return AdjustInlineCaretPositionForBidiText({ cursor, InlineCaretPositionType::kBeforeBox, std::nullopt });
}
...
InlineCaretPosition ComputeInlineCaretPosition(const LayoutBlockFlow& context, unsigned offset, TextAffinity affinity, const LayoutText* layout_text)
{
    ...
    for (; cursor; cursor.MoveToNextIncludingFragmentainer()) {
        const InlineCaretPositionResolution resolution = TryResolveInlineCaretPositionWithFragment(cursor, offset, affinity);
        ...
        if (resolution.type == ResolutionType::kResolved) {
            ...
            if (!layout_text || candidate.cursor.Current().GetLayoutObject() == layout_text) {
        ...
        candidate = BetterCandidateBetween(candidate, resolution.caret_position, offset);
    }

    return AdjustInlineCaretPositionForBidiText(candidate);
}

InlineCaretPosition ComputeInlineCaretPosition(const PositionWithAffinity& position_with_affinity)
{
    const Position& position = position_with_affinity.GetPosition();
    ...
    if (layout_object->IsLayoutInline()) {
        ...
        NOTREACHED() << "Caller should not pass a position inside inline: " << position;
    }
    ...
    const std::optional<unsigned> maybe_offset = mapping->GetTextContentOffset(position);
    ...
    const LayoutText* const layout_text = position.IsOffsetInAnchor() && IsA<Text>(position.AnchorNode())
        ? To<LayoutText>(AssociatedLayoutObjectOf(*position.AnchorNode(), position.OffsetInContainerNode()))
        : nullptr;
    ...
    const unsigned adjusted_offset
        = affinity == TextAffinity::kUpstream && offset && mapping->GetText()[offset - 1] == kZeroWidthSpaceCharacter ? offset - 1 : offset;
    return ComputeInlineCaretPosition(*context, adjusted_offset, affinity, layout_text);
}
...
PositionWithAffinity InlineCaretPosition::ToPositionInDOMTreeWithAffinity() const
{
    if (IsNull()) {
    ...
    case InlineCaretPositionType::kBeforeBox:
        if (const Node* node = cursor.Current().GetNode()) {
            return PositionWithAffinity(Position::BeforeNode(*node), TextAffinity::kDownstream);
        }
        return PositionWithAffinity();
    case InlineCaretPositionType::kAfterBox:
        if (const Node* node = cursor.Current().GetNode()) {
            return PositionWithAffinity(Position::AfterNode(*node), TextAffinity::kUpstreamIfPossible);
        }
        return PositionWithAffinity();
    case InlineCaretPositionType::kAtTextOffset:
        ...
        if (!mapping) {
            ...
            DUMP_WILL_BE_NOTREACHED() << cursor << " " << cursor.Current().GetLayoutObject();
            return PositionWithAffinity();
        }
        const TextAffinity affinity = *text_offset == cursor.Current().TextEndOffset() ? TextAffinity::kUpstreamIfPossible : TextAffinity::kDownstream;
        const Position position = affinity == TextAffinity::kDownstream ? mapping->GetLastPosition(*text_offset) : mapping->GetFirstPosition(*text_offset);
        if (position.IsNull()) {
            return PositionWithAffinity();
        }
