...
public:
    LineBreakStrategy(InlineChildLayoutContext* context, const InlineNode& node, const ComputedStyle& block_style, const InlineBreakToken* break_token,
        const ColumnSpannerPath* column_spanner_path)
    {
        if (!column_spanner_path) {
            ...
            if (text_wrap == TextWrapStyle::kBalance) [[unlikely]] {
                ...
                if (initiate_balancing_) {
                    DCHECK(!score_line_break_context_ || score_line_break_context_->IsActive());
                    use_score_line_break_ = score_line_break_context_;
                }
            } else if (text_wrap == TextWrapStyle::kPretty) [[unlikely]] {
                score_line_break_context_ = context->GetScoreLineBreakContext();
                use_score_line_break_ = score_line_break_context_ && score_line_break_context_->IsActive();
            } else {
        ...
        DCHECK(!context->GetScoreLineBreakContext() || !node.IsScoreLineBreakDisabled());
    ...
    void Prepare(InlineChildLayoutContext* context, const InlineNode& node, const ConstraintSpace& space, base::span<const LayoutOpportunity> opportunities,
        const LineLayoutOpportunity& line_opportunity, const LeadingFloats& leading_floats, const InlineBreakToken* break_token,
        ExclusionSpace* exclusion_space)
    {
        if (initiate_balancing_) {
            Balance(context, node, space, opportunities, line_opportunity, leading_floats, break_token, exclusion_space);
        } else if (use_score_line_break_) {
            Optimize(node, space, opportunities, leading_floats, break_token, exclusion_space);
        }
    }

    void SetupLineBreaker(InlineChildLayoutContext* context, LineBreaker& line_breaker)
    {
        if (const std::optional<LayoutUnit>& balanced_available_width = context->BalancedAvailableWidth()) [[unlikely]] {
            DCHECK(!score_line_break_context_ || !score_line_break_context_->CurrentLineBreakPoint());
            line_breaker.OverrideAvailableWidth(*balanced_available_width);
        } else if (score_line_break_context_) [[unlikely]] {
            if (const LineBreakPoint* break_point = score_line_break_context_->CurrentLineBreakPoint()) {
...
private:
    void Balance(InlineChildLayoutContext* context, const InlineNode& node, const ConstraintSpace& space,
        const base::span<const LayoutOpportunity>& opportunities, const LineLayoutOpportunity& line_opportunity, const LeadingFloats& leading_floats,
        const InlineBreakToken* break_token, ExclusionSpace* exclusion_space)
    {
        ...
        if (use_score_line_break_ && score_line_break_context_->IsActive()) {
            ...
            if (line_widths.Set(node, opportunities)) {
                ScoreLineBreaker optimizer(node, space, line_widths, break_token, exclusion_space);
                optimizer.BalanceBreakPoints(leading_floats, *score_line_break_context_);
        ...
        if (opportunities.size() == 1 && line_opportunity.AvailableInlineSize() > LayoutUnit()) {
            if (const std::optional<LayoutUnit> balanced_available_width = ParagraphLineBreaker::AttemptParagraphBalancing(node, space, line_opportunity)) {
    ...
    void Optimize(const InlineNode& node, const ConstraintSpace& space, const base::span<const LayoutOpportunity>& opportunities,
        const LeadingFloats& leading_floats, const InlineBreakToken* break_token, ExclusionSpace* exclusion_space)
    {
        DCHECK(score_line_break_context_->GetLineBreakPoints().empty());
        DCHECK_EQ(score_line_break_context_->LineBreakPointsIndex(), 0u);
...
void PlaceRelativePositionedItems(const ConstraintSpace& constraint_space, LogicalLineItems* line_box)
{
    for (auto& child : *line_box) {
        ...
        child.rect.offset += ComputeRelativeOffsetForInline(constraint_space, physical_fragment->Style());
    }
}

} // namespace

InlineLayoutAlgorithm::InlineLayoutAlgorithm(InlineNode inline_node, const ConstraintSpace& space, const InlineBreakToken* break_token,
    const ColumnSpannerPath* column_spanner_path, InlineChildLayoutContext* context)
    : LayoutAlgorithm(inline_node, &inline_node.Style(), space,
        // Use LTR direction since inline layout handles bidi by
        // itself and lays out in visual order.
        TextDirection::kLtr, break_token)
    , box_states_(nullptr)
    , context_(context)
...
void InlineLayoutAlgorithm::PrepareBoxStates(const LineInfo& line_info, const InlineBreakToken* break_token)
{
#if EXPENSIVE_DCHECKS_ARE_ON()
    ...
    if (!break_token->UseFirstLineStyle()) {
        box_states_ = context_->BoxStatesIfValidForItemIndex(items, break_token->StartItemIndex());
        if (box_states_) {
#if EXPENSIVE_DCHECKS_ARE_ON()
    ...
    box_states_ = context_->ResetBoxStates();
    LogicalLineBuilder(Node(), GetConstraintSpace(), nullptr, box_states_, context_).RebuildBoxStates(line_info, 0u, break_token->StartItemIndex());
}

static LayoutUnit AdjustLineOffsetForHanging(LineInfo* line_info, LayoutUnit& line_offset)
{
...
void InlineLayoutAlgorithm::CheckBoxStates(const LineInfo& line_info) const
{
    ...
    LogicalLineBuilder(Node(), GetConstraintSpace(), nullptr, &rebuilt, context_).RebuildBoxStates(line_info, 0u, GetBreakToken()->StartItemIndex());
    LogicalLineItems& line_box = context_->AcquireTempLogicalLineItems();
    rebuilt.OnBeginPlaceItems(Node(), line_info.LineStyle(), baseline_type_, quirks_mode_, &line_box);
    DCHECK(box_states_);
...
ALWAYS_INLINE InlineLayoutAlgorithm::LineClampState InlineLayoutAlgorithm::GetLineClampState(const LineInfo* line_info, LayoutUnit line_box_height) const
{
    ...
    } else if (!line_info->IsBlockInInline() && line_info->HasOverflow() && node_.GetLayoutBlockFlow()->ShouldTruncateOverflowingText()) {
        return LineClampState::kEllipsize;
    }
...
void InlineLayoutAlgorithm::CreateLine(const LineLayoutOpportunity& opportunity, LineInfo* line_info, LogicalLineContainer* line_container)
{
    ...
    LogicalLineBuilder line_builder(Node(), GetConstraintSpace(), GetBreakToken(), box_states_, context_);
    line_builder.CreateLine(line_info, line_box, this);

    const LayoutUnit hang_width = line_info->HangWidth();
    const LayoutUnit position = AdjustLineOffsetForHanging(line_info, line_offset_for_text_align);
    LayoutUnit inline_size = box_states_->ComputeInlinePositions(line_box, position, line_info->IsBlockInInline());
    ...
    if (line_info->HasLineEvenIfEmpty() || !box_states_->RubyColumnList().empty()) [[unlikely]] {
        box_states_->LineBoxState().EnsureTextMetrics(line_info->LineStyle(), *box_states_->LineBoxState().font, baseline_type_);
    } else if (line_builder.InitialLetterItemResult() && box_states_->LineBoxState().metrics.IsEmpty()) [[unlikely]] {
    ...
    if (Node().HasRuby() && !line_info->IsEmptyLine()) [[unlikely]] {
        ...
        if (!box_states_->RubyColumnList().empty()) {
            HeapVector<Member<LogicalRubyColumn>>& column_list = box_states_->RubyColumnList();
            UpdateRubyColumnInlinePositions(*line_box, inline_size, column_list);
            RubyBlockPositionCalculator calculator;
            calculator.GroupLines(column_list).PlaceLines(*line_box, line_box_metrics).AddLinesTo(*line_container);
            annotation_metrics = calculator.AnnotationMetrics();
        }
        line_info->SetAnnotationBlockStartAdjustment(SetAnnotationOverflow(*line_info, *line_box, line_box_metrics, annotation_metrics));
    }
    ...
    const LineClampState line_clamp_state = GetLineClampState(line_info, line_box_metrics.LineHeight());
    if (line_clamp_state == LineClampState::kEllipsize) [[unlikely]] {
        DCHECK(!line_info->IsBlockInInline());
        LineTruncator truncator(*line_info);
        auto* input = DynamicTo<HTMLInputElement>(node_.GetLayoutBlockFlow()->GetNode());
        if (input && input->ShouldApplyMiddleEllipsis()) {
            inline_size = truncator.TruncateLineInTheMiddle(inline_size, line_box, box_states_);
        } else {
    ...
    if (line_info->IsBlockInInline()) {
        container_builder_.SetBfcLineOffset(GetConstraintSpace().GetBfcOffset().line_offset);
    } else {
        ...
        LayoutUnit bfc_line_offset = line_info->GetBfcOffset().line_offset + line_offset_for_text_align;
    ...
    if (line_builder.InitialLetterItemResult()) [[unlikely]] {
        ...
        const ExclusionArea* exclusion = PostPlaceInitialLetterBox(line_box_metrics,
            BoxStrut(line_builder.InitialLetterItemResult()->margins, line_info->LineStyle().IsFlippedLinesWritingMode()), line_box,
            BfcOffset(container_builder_.BfcLineOffset(), line_info->GetBfcOffset().block_offset), line_info);
    ...
    if (line_builder.HasFloatingItems()) {
        ...
        PlaceFloatingObjects(line_box_metrics, opportunity, line_info->ComputeBlockStartAdjustment(), line_info, line_box);
    }
    ...
    for (auto annotation_line : line_container->AnnotationLineList()) {
        PlaceRelativePositionedItems(GetConstraintSpace(), annotation_line.line_items);
    }

    // Apply any relative positioned offsets to any boxes (and their children).
    box_states_->ApplyRelativePositioning(GetConstraintSpace(), line_box, nullptr);
    ...
    box_states_->CreateBoxFragments(GetConstraintSpace(), line_box, line_info->IsBlockInInline());
    box_states_->ClearRubyColumnList();

    // Update item index of the box states in the context.
    context_->SetItemIndex(line_info->ItemsData().items, line_info->EndItemIndex());
    ...
    const ConstraintSpace& space = GetConstraintSpace();
    if (space.ShouldTextBoxTrimNodeStart() || space.ShouldTextBoxTrimNodeEnd() || space.ShouldTextBoxTrimFragmentainerStart()
        || space.ShouldTextBoxTrimFragmentainerEnd() || space.ShouldTextBoxTrimInsideWhenLineClamp()) [[unlikely]] {
        bool is_truncated = line_clamp_state == LineClampState::kEllipsize || space.GetLineClampData().state == LineClampData::kMeasureLinesUntilBfcOffset;
    ...
    if (Node().IsTextCombine()) [[unlikely]] {
        ...
        const FontHeight& adjusted_metrics = AdjustInitialLetterInTextPosition(line_box_metrics, line_box);
...
void InlineLayoutAlgorithm::ApplyTextBoxTrim(LineInfo& line_info, bool is_truncated)
{
    const ConstraintSpace& space = GetConstraintSpace();
    ...
    const bool should_apply_start = (space.ShouldTextBoxTrimNodeStart() && line_info.IsFirstFormattedLine()) || space.ShouldTextBoxTrimFragmentainerStart();
    const bool should_apply_end = (space.ShouldTextBoxTrimNodeEnd() && !line_info.GetBreakToken())
        || (space.ShouldTextBoxTrimInsideWhenLineClamp() && is_truncated) || space.ShouldForceTextBoxTrimEnd();
    ...
    InlineBoxState::AdjustEdges(space.EffectiveTextBoxEdge(), line_style.GetFont(), baseline_type_, should_apply_over, should_apply_under, intrinsic_metrics);

    if (should_apply_start) {
        // Apply `text-box-trim: start` if this is the first formatted line.
        LayoutUnit offset_for_trimming_box;
        if (is_flipped_line) [[unlikely]] {
            offset_for_trimming_box = intrinsic_metrics.descent - line_box_metrics.descent;
        } else {
            offset_for_trimming_box = intrinsic_metrics.ascent - line_box_metrics.ascent;
        }
        container_builder_.SetLineBoxBfcBlockOffset(container_builder_.LineBoxBfcBlockOffset()
                ? offset_for_trimming_box + container_builder_.LineBoxBfcBlockOffset().value()
    ...
    if (should_apply_end) {
        ...
        if (is_flipped_line) [[unlikely]] {
            block_end_to_be_trimmed = line_box_metrics.ascent - intrinsic_metrics.ascent;
        } else {
            block_end_to_be_trimmed = line_box_metrics.descent - intrinsic_metrics.descent;
        }
        container_builder_.SetTrimBlockEndBy(block_end_to_be_trimmed);
    }
}

void InlineLayoutAlgorithm::PlaceBlockInInline(const InlineItem& item, InlineItemResult* item_result, LogicalLineItems* line_box)
{
    ...
    LogicalBoxFragment fragment(GetConstraintSpace().GetWritingDirection(), box_fragment);
...
void InlineLayoutAlgorithm::PlaceOutOfFlowObjects(const LineInfo& line_info, const FontHeight& line_box_metrics, LogicalLineItems* line_box)
{
    DCHECK(line_info.IsEmptyLine() || !line_box_metrics.IsEmpty()) << "Non-empty lines must have a valid set of linebox metrics.";
    ...
    LayoutUnit baseline_adjustment = line_info.IsEmptyLine() ? LayoutUnit() : -line_box_metrics.ascent;

    LayoutUnit line_height = line_info.IsEmptyLine() ? LayoutUnit() : line_box_metrics.LineHeight();
    ...
    LayoutUnit block_level_line_location = IsLtr(GetConstraintSpace().Direction()) ? LayoutUnit() : GetConstraintSpace().AvailableSize().inline_size;
    ...
    LayoutUnit block_level_inline_offset = block_level_line_location - (container_builder_.BfcLineOffset() - GetConstraintSpace().GetBfcOffset().line_offset);
    ...
    if (has_rtl_block_level_out_of_flow_objects) [[unlikely]] {
        ...
        for (LogicalLineItem& child : base::Reversed(*line_box)) {
            ...
            if (has_preceding_inline_level_content && !box->StyleRef().IsOriginalDisplayInlineType()) {
...
void InlineLayoutAlgorithm::PlaceFloatingObjects(const FontHeight& line_box_metrics, const LineLayoutOpportunity& opportunity,
    LayoutUnit ruby_block_start_adjust, LineInfo* line_info, LogicalLineItems* line_box)
{
    DCHECK(line_info->IsEmptyLine() || !line_box_metrics.IsEmpty()) << "Non-empty lines must have a valid set of linebox metrics.";
    ...
    LayoutUnit origin_bfc_block_offset = opportunity.bfc_block_offset + line_height;

    LayoutUnit bfc_line_offset = container_builder_.BfcLineOffset();
    LayoutUnit bfc_block_offset
        = line_info->IsEmptyLine() ? GetConstraintSpace().ExpectedBfcBlockOffset() : line_info->GetBfcOffset().block_offset + ruby_block_start_adjust;
    ...
    for (LogicalLineItem& child : *line_box) {
        ...
        if (child.unpositioned_float) {
            ...
            PositionedFloat positioned_float = PositionFloat(origin_bfc_block_offset, child.unpositioned_float, &GetExclusionSpace());
            const BlockBreakToken* break_token = positioned_float.BreakToken();
            if (break_token) {
                const auto* parallel_token = InlineBreakToken::CreateForParallelBlockFlow(node_, child.item_index, *break_token);
                line_info->PropagateParallelFlowBreakToken(parallel_token);
                if (positioned_float.minimum_space_shortage) {
                    line_info->PropagateMinimumSpaceShortage(positioned_float.minimum_space_shortage);
                }
            }
        ...
        if (!child.layout_result || !child.layout_result->GetPhysicalFragment().IsFloating()) {
            continue;
        }

        LayoutUnit block_offset = child.bfc_offset.block_offset - bfc_block_offset + baseline_adjustment;
        ...
        if (IsFlippedLinesWritingMode(GetConstraintSpace().GetWritingMode())) {
            LogicalFragment fragment(GetConstraintSpace().GetWritingDirection(), child.layout_result->GetPhysicalFragment());

            block_offset = -fragment.BlockSize() - block_offset;
        }

        child.rect.offset = { child.bfc_offset.line_offset - bfc_line_offset, block_offset };
    }
}
...
LayoutUnit InlineLayoutAlgorithm::ApplyTextAlign(LineInfo* line_info)
{
    ...
    LayoutUnit space = line_info->AvailableWidth() - line_info->WidthForAlignment();
    ...
    if (text_align == ETextAlign::kJustify) {
        ...
        std::optional<LayoutUnit> offset = ApplyJustification(space, target, line_info);
        if (offset)
            return *offset;
...
LayoutUnit InlineLayoutAlgorithm::SetAnnotationOverflow(
    const LineInfo& line_info, const LogicalLineItems& line_box, const FontHeight& line_box_metrics, std::optional<FontHeight> annotation_font_height)
{
    AnnotationMetrics annotation_metrics = ComputeAnnotationOverflow(line_box, line_box_metrics, line_info.LineStyle(), annotation_font_height);
    ...
    if (GetConstraintSpace().BlockStartAnnotationSpace() < LayoutUnit() && annotation_space_block_start) {
        const LayoutUnit overflow = -GetConstraintSpace().BlockStartAnnotationSpace();
    ...
    if (annotation_overflow_block_start && GetConstraintSpace().BlockStartAnnotationSpace() > LayoutUnit()) {
        block_offset_shift = (annotation_overflow_block_start - GetConstraintSpace().BlockStartAnnotationSpace()).ClampNegativeToZero();
    }
...
bool InlineLayoutAlgorithm::AddAnyClearanceAfterLine(const LineInfo& line_info)
{
    ...
    const LayoutUnit content_size = container_builder_.LineHeight() - container_builder_.TrimBlockEndBy().value_or(LayoutUnit());

    // layout_object may be null in certain cases, e.g. if it's a kBidiControl.
    if (layout_object && layout_object->IsBR()) {
        const LayoutUnit line_box_bfc_block_offset = *container_builder_.LineBoxBfcBlockOffset();
        BfcOffset bfc_offset = { LayoutUnit(), line_box_bfc_block_offset + content_size };
        ...
        if (clear_type != EClear::kNone) {
            AdjustToClearance(GetExclusionSpace().ClearanceOffset(clear_type), &bfc_offset);
            ...
            container_builder_.SetClearanceAfterLine(bfc_offset.block_offset - block_end_offset_without_clearence);
        }

        if (GetConstraintSpace().HasBlockFragmentation() && GetExclusionSpace().NeedsClearancePastFragmentainer(clear_type)) {
            return false;
        }
    }
    return true;
...
const LayoutResult* InlineLayoutAlgorithm::Layout()
{
    ...
    container_builder_.SetAdjoiningObjectTypes(constraint_space.GetAdjoiningObjectTypes());
    ...
    LayoutUnit bfc_block_offset
        = constraint_space.ForcedBfcBlockOffset().value_or(constraint_space.GetBfcOffset().block_offset + constraint_space.GetMarginStrut().Sum());
    ...
    if (!context_->ItemsBuilder()->Size()) {
        ...
        const LayoutUnit initial_letter_clearance = constraint_space.GetExclusionSpace().InitialLetterClearanceOffset(clear_type);
    ...
    LayoutOpportunityVector opportunities = initial_exclusion_space.AllLayoutOpportunities(
        { constraint_space.GetBfcOffset().line_offset, bfc_block_offset }, constraint_space.AvailableSize().inline_size);
    ...
    LogicalLineContainer* const line_container = items_builder->AcquireLogicalLineContainer();
    DCHECK(line_container);
    // Determine which line breaker to use.
    LineBreakStrategy line_break_strategy(context_, Node(), Style(), break_token, column_spanner_path_);
    ...
    while (opportunities_it != opportunities.end()) {
        ...
        if (opportunities_it + 1 == opportunities.end()) {
            ...
            if (opportunity.rect.InlineSize() != LayoutUnit::Max()) {
                DCHECK_EQ(opportunity.rect.InlineSize(), constraint_space.AvailableSize().inline_size);
            }
        ...
        LineLayoutOpportunity line_opportunity = opportunity.ComputeLineLayoutOpportunity(constraint_space, line_block_size, block_delta);
        if (line_break_strategy.NeedsToPrepare()) [[unlikely]] {
            line_break_strategy.Prepare(context_, Node(), constraint_space, base::make_span(opportunities_it, opportunities.end()), line_opportunity,
                leading_floats, break_token, &GetExclusionSpace());
        }
        bool is_line_info_cached = false;
        LineInfo& line_info = context_->GetLineInfo(break_token, is_line_info_cached);
        if (is_line_info_cached) [[unlikely]] {
            // Update the BFC offset because it was not known when the `line_info` was
            // cached.
            line_info.SetBfcOffset({ line_opportunity.line_left_offset, line_opportunity.bfc_block_offset });
        } else {
            LineBreaker line_breaker(
                Node(), LineBreakerMode::kContent, constraint_space, line_opportunity, leading_floats, break_token, column_spanner_path_, &GetExclusionSpace());
        ...
        if (Node().IsInitialLetterBox()) [[unlikely]] {
            ...
            line_info.SetWidth(line_info.AvailableWidth(), CalculateInitialLetterBoxInlineSize(line_info));
        }

        const auto* block_in_inline_result = line_info.BlockInInlineLayoutResult();
        if (block_in_inline_result) {
            if (block_in_inline_result->Status() != LayoutResult::kSuccess) [[unlikely]] {
        ...
        if (!line_info.IsEmptyLine()) {
            if (block_in_inline_result && block_in_inline_result->BfcBlockOffset()) {
                const auto result_bfc_block_offset = block_in_inline_result->BfcBlockOffset();
                ...
                container_builder_.SetLineBoxBfcBlockOffset(line_info.GetBfcOffset().block_offset);
                if (is_pushed_by_floats)
                    container_builder_.SetIsPushedByFloats();
            }

            // Abort if something before needs to know the correct BFC block-offset.
            if (container_builder_.GetAdjoiningObjectTypes() && bfc_block_offset != constraint_space.ExpectedBfcBlockOffset()) {
                items_builder->ReleaseCurrentLogicalLineContainer();
        ...
        if (line_info.HasOverflow() && !line_opportunity.IsEqualToAvailableFloatInlineSize(constraint_space.AvailableSize().inline_size)
            ...
            if (opportunity.HasShapeExclusions() && block_delta < opportunity.rect.BlockSize() && !opportunity.IsBlockDeltaBelowShapes(block_delta))
                [[unlikely]] {
        ...
        const LayoutUnit block_start_adjust = line_info.ComputeBlockStartAdjustment();
        if (block_start_adjust) [[unlikely]] {
            DCHECK(container_builder_.BfcBlockOffset());
            DCHECK(container_builder_.LineBoxBfcBlockOffset());
            DCHECK(!line_info.IsEmptyLine());
            container_builder_.SetLineBoxBfcBlockOffset(line_info.GetBfcOffset().block_offset + block_start_adjust);
            container_builder_.SetAnnotationBlockOffsetAdjustment(line_info.ComputeAnnotationBlockOffsetAdjustment());
        }
        ...
        const LayoutUnit total_block_size
            = line_info.ComputeTotalBlockSize(container_builder_.LineHeight(), container_builder_.AnnotationOverflow().ClampNegativeToZero());
        ...
        // this logic.
        if (opportunity.HasShapeExclusions() && !line_info.IsEmptyLine()) [[unlikely]] {
            LineLayoutOpportunity line_opportunity_with_height = opportunity.ComputeLineLayoutOpportunity(constraint_space, total_block_size, block_delta);

            if (line_opportunity_with_height.AvailableInlineSize() != line_opportunity.AvailableInlineSize()) {
        ...
        for (const InlineBreakToken* parallel_token : line_info.ParallelFlowBreakTokens()) {
            DCHECK(parallel_token->IsInParallelBlockFlow());
            context_->PropagateParallelFlowBreakToken(parallel_token);
        }
        if (std::optional<LayoutUnit> minimum_space_shortage = line_info.MinimumSpaceShortage()) {
        ...
        if (line_info.IsEmptyLine()) {
            ...
            if (auto forced_bfc_block_offset = constraint_space.ForcedBfcBlockOffset()) {
            ...
            if (!line_info.IsBlockInInline()) {
                ...
                if (lines_until_clamp_) {
                    if (constraint_space.GetLineClampData().state == LineClampData::kClampByLines) {
                        *lines_until_clamp_ = *lines_until_clamp_ - 1;
                    } else {
                        DCHECK_EQ(constraint_space.GetLineClampData().state, LineClampData::kMeasureLinesUntilBfcOffset);
                        *lines_until_clamp_ = *lines_until_clamp_ + 1;
                    }
                }
            }
    ...
    items_builder->AssociateLogicalLineContainer(line_container, layout_result->GetPhysicalFragment());
...
void InlineLayoutAlgorithm::PositionLeadingFloats(ExclusionSpace& exclusion_space, LeadingFloats& leading_floats)
{
    ...
    const HeapVector<InlineItem>& items = Node().ItemsData(/* is_first_line */ false).items;
    ...
    for (; index < items.size(); ++index) {
        ...
        container_builder_.AddAdjoiningObjectTypes(
            item.GetLayoutObject()->StyleRef().Floating(GetConstraintSpace().Direction()) == EFloat::kLeft ? kAdjoiningFloatLeft : kAdjoiningFloatRight);

        // Place any floats at the "expected" BFC block-offset, this may be an
        // optimistic guess.
        const LayoutUnit origin_bfc_block_offset = GetConstraintSpace().ExpectedBfcBlockOffset();
        PositionedFloat positioned_float = PositionFloat(origin_bfc_block_offset, item.GetLayoutObject(), &exclusion_space);
        ...
        if (GetConstraintSpace().HasBlockFragmentation()) {
            ...
            if (const auto* float_break_token = positioned_float.BreakToken()) {
                const auto* parallel_token = InlineBreakToken::CreateForParallelBlockFlow(node_, { index, item.StartOffset() }, *float_break_token);
                context_->PropagateParallelFlowBreakToken(parallel_token);
            }
        }
...
PositionedFloat InlineLayoutAlgorithm::PositionFloat(LayoutUnit origin_bfc_block_offset, LayoutObject* floating_object, ExclusionSpace* exclusion_space)
{
    const auto& space = GetConstraintSpace();
    BfcOffset origin_bfc_offset = { space.GetBfcOffset().line_offset, origin_bfc_block_offset };
    ...
    bool is_hidden_for_paint = GetConstraintSpace().GetLineClampData().ShouldHideForPaint();
    UnpositionedFloat unpositioned_float(BlockNode(To<LayoutBox>(floating_object)),
        /* break_token */ nullptr, space.AvailableSize(), space.PercentageResolutionSize(), space.ReplacedPercentageResolutionSize(), origin_bfc_offset, space,
        Style(), space.FragmentainerBlockSize(), space.FragmentainerOffset(), is_hidden_for_paint);

    PositionedFloat positioned_float = ::blink::PositionFloat(&unpositioned_float, exclusion_space);
