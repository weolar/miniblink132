...
public:
    ...
    {
        return std::accumulate(lines_.begin(), lines_.end(), LayoutUnit(), [](LayoutUnit acc, const LineBreakResult& item) { return acc + item.width; });
    }
    const InlineBreakToken* BreakToken() const
    {
        return break_token_;
    }
    ...
    Status BreakLines(const LayoutUnit available_width, wtf_size_t max_lines, const InlineBreakToken* stop_at = nullptr)
    {
        DCHECK(lines_.empty());
        ...
        LineInfo line_info;
        for (;;) {
            LineBreaker line_breaker(node_, LineBreakerMode::kContent, space_, line_opportunity, leading_floats, break_token_,
            ...
            if (!break_token_ || (stop_at && break_token_->Start() >= stop_at->Start())) {
    ...
    LayoutUnit BisectAvailableWidth(const LayoutUnit max_available_width, const LayoutUnit min_available_width, const LayoutUnit epsilon,
        const wtf_size_t num_lines, const InlineBreakToken* stop_at = nullptr)
    {
...
wtf_size_t EstimateNumLines(const String& text_content, const SimpleFontData* font, LayoutUnit available_width)
{
...
std::optional<LayoutUnit> ParagraphLineBreaker::AttemptParagraphBalancing(
    const InlineNode& node, const ConstraintSpace& space, const LineLayoutOpportunity& line_opportunity)
{
    ...
    const int lines_until_clamp = space.GetLineClampData().LinesUntilClamp().value_or(0);
    if (lines_until_clamp > 0 && static_cast<unsigned>(lines_until_clamp) <= max_lines) {
        if (lines_until_clamp == 1) {
            return std::nullopt; // Balancing not needed for single line paragraphs.
        }

        const LineBreakResults::Status status = normal_lines.BreakLines(available_width, lines_until_clamp);
        ...
        const wtf_size_t estimated_num_lines
            = EstimateNumLines(items_data.text_content, block_style.GetFont().PrimaryFont(), line_opportunity.AvailableInlineSize());
        if (estimated_num_lines > max_lines * 2) {
            return std::nullopt;
        }

        const LineBreakResults::Status status = normal_lines.BreakLines(available_width, max_lines);
    ...
    const LayoutUnit min_available_width = LayoutUnit::FromFloatRound(avg_line_width * .8f);
    return balanced_lines.BisectAvailableWidth(available_width, min_available_width, epsilon, num_lines, normal_lines.BreakToken());
}
