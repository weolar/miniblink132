...
public:
    ...
    InlineItemResult(const InlineItem*, unsigned index, const TextOffsetRange& text_offset, bool break_anywhere_if_overflow, bool should_create_line_box,
        bool has_unpositioned_floats);

    const TextOffsetRange& TextOffset() const
    {
        return text_offset;
    }
    wtf_size_t StartOffset() const
    {
        return text_offset.start;
    }
    wtf_size_t EndOffset() const
    {
        return text_offset.end;
    }
    ...
    InlineItemTextIndex Start() const
    {
        return { item_index, StartOffset() };
    }
    InlineItemTextIndex End() const
    {
        return { item_index, EndOffset() };
    }
    ...
    String ToString(const String& ifc_text_content, const String& indent = "") const;
