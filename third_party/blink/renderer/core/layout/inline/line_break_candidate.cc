...
void LineBreakCandidateContext::Append(
    State new_state, InlineItemTextIndex offset, InlineItemTextIndex end, float pos_no_break, float pos_if_break, float penalty, bool is_hyphenated)
{
    ...
    switch (state_) {
    case State::kBreak:
        candidates_.emplace_back(offset, end, pos_no_break, pos_if_break, penalty, is_hyphenated);
        break;
    case State::kMidWord: {
...
void LineBreakCandidateContext::Append(State new_state, const InlineItemTextIndex& offset, float position)
{
    Append(new_state, offset, offset, position, position);
}

void LineBreakCandidateContext::AppendTrailingSpaces(State new_state, const InlineItemTextIndex& offset, float pos_no_break)
{
...
bool LineBreakCandidateContext::AppendLine(const LineInfo& line_info, LineBreaker& line_breaker)
{
    ...
    for (const InlineItemResult& item_result : line_info.Results()) {
        ...
        case InlineItem::kControl:
            AppendTrailingSpaces(item_result.can_break_after ? kBreak : kMidWord, { item_result.item_index, item_result.EndOffset() },
        ...
        default: {
            ...
            const InlineItemTextIndex offset { item_result.item_index + 1, item_result.EndOffset() };
            ...
            if (!item.Length()) {
                ...
                const LineBreakCandidate& last_candidate = candidates_.back();
                Append(new_state, offset, last_candidate.end, end_position, last_candidate.pos_if_break);
            } else {
...
void LineBreakCandidateContext::EnsureFirstSentinel(const LineInfo& first_line_info)
{
    DCHECK(candidates_.empty());
...
void LineBreakCandidateContext::EnsureLastSentinel(const LineInfo& last_line_info)
{
#if EXPENSIVE_DCHECKS_ARE_ON()
    ...
    DCHECK(candidates_.back().offset == last_item_result.End() || candidates_.back().offset == last_line_info.End());
#endif // EXPENSIVE_DCHECKS_ARE_ON()
}
...
void LineBreakCandidateContext::CheckConsistency() const
{
    for (wtf_size_t i = 1; i < candidates_.size(); ++i) {
        ...
        if (std::isnan(candidate.pos_no_break) || candidate.pos_no_break >= LayoutUnit::NearlyMax()) {
            continue;
        }
...
std::ostream& operator<<(std::ostream& ostream, const LineBreakCandidate& candidate)
{
    return ostream << candidate.offset << "/" << candidate.end << " " << candidate.pos_no_break << "/" << candidate.pos_if_break
                   << " penalty=" << candidate.penalty << (candidate.is_hyphenated ? " (hyphenated)" : "");
}
