...
public:
    LineBoxFragmentBuilder(
        InlineNode node, const ComputedStyle* style, const ConstraintSpace& space, WritingDirectionMode writing_direction, const InlineBreakToken* break_token)
        : FragmentBuilder(node, style, space,
            // Always use LTR because line items are in visual order.
            { writing_direction.GetWritingMode(), TextDirection::kLtr }, break_token)
    ...
    const FontHeight& Metrics() const
    {
        return metrics_;
    }
    void SetMetrics(const FontHeight& metrics)
    {
        metrics_ = metrics;
    }
