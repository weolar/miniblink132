...
PhysicalRect ComputeLocalCaretRectByBoxSide(const InlineCursor& cursor, InlineCaretPositionType position_type)
{
    ...
    WritingModeConverter converter(
        { cursor.Current().Style().GetWritingMode(), TextDirection::kLtr }, is_atomic_inline ? cursor.Current().Size() : cursor.ContainerFragment().Size());
    LogicalRect line_rect = converter.ToLogical(line_box.Current().RectInContainerFragment());
    LogicalRect item_rect = converter.ToLogical(cursor.Current().RectInContainerFragment());
    ...
    const LocalFrameView* frame_view = cursor.Current().GetLayoutObject()->GetDocument().View();
    ...
    if (is_ltr != (position_type == InlineCaretPositionType::kBeforeBox)) {
        caret_rect.offset.inline_offset += item_rect.size.inline_size - caret_rect.size.inline_size;
    }
...
PhysicalRect ComputeLocalCaretRectAtTextOffset(const InlineCursor& cursor, unsigned offset)
{
    ...
    WritingModeConverter converter({ style.GetWritingMode(), TextDirection::kLtr }, cursor.Current().Size());
    LogicalRect caret_rect;
    caret_rect.size.inline_size = caret_width;
    caret_rect.size.block_size = converter.ToLogical(cursor.Current().Size()).block_size;
    ...
    PhysicalOffset caret_location = physical_caret_rect.offset + cursor.Current().OffsetInContainerFragment();
    const auto* const text_combine = DynamicTo<LayoutTextCombine>(cursor.Current().GetLayoutObject()->Parent());
    if (text_combine) [[unlikely]] {
        caret_location = text_combine->AdjustOffsetForLocalCaretRect(caret_location);
    }

    const PhysicalBoxFragment& fragment = cursor.ContainerFragment();
    InlineCursor line_box(cursor);
    line_box.MoveToContainingLine();
    const PhysicalOffset line_box_offset = line_box.Current().OffsetInContainerFragment();
    ...
    bool should_align_caret_right = ShouldAlignCaretRight(block_style.GetTextAlign(is_last_line), line_box.Current().BaseDirection());
    ...
    if (is_horizontal) {
        if (should_align_caret_right) {
            ...
            caret_location.left = ClampAndRound(caret_location.left, left_edge, right_limit);
        } else {
            const LayoutUnit right_limit = std::max(fragment.Size().width, line_box_rect.Right()) - caret_width;
            caret_location.left = ClampAndRound(caret_location.left, line_box_rect.X(), right_limit);
        }
    ...
    const LayoutUnit bottom_limit = std::max(fragment.Size().height, line_box_rect.Bottom()) - caret_width;
...
LocalCaretRect ComputeLocalCaretRect(const InlineCaretPosition& caret_position)
{
    ...
    const LayoutObject* const layout_object = caret_position.cursor.Current().GetLayoutObject();
    const PhysicalBoxFragment& container_fragment = caret_position.cursor.ContainerFragment();
    ...
    case InlineCaretPositionType::kAfterBox: {
        ...
        const PhysicalRect fragment_local_rect = ComputeLocalCaretRectByBoxSide(caret_position.cursor, caret_position.position_type);
        return { layout_object, fragment_local_rect, &container_fragment };
    }
    case InlineCaretPositionType::kAtTextOffset: {
        DCHECK(caret_position.cursor.Current().IsText());
        DCHECK(caret_position.text_offset.has_value());
        const PhysicalRect caret_rect = ComputeLocalCaretRectAtTextOffset(caret_position.cursor, *caret_position.text_offset);
...
LocalCaretRect ComputeLocalSelectionRect(const InlineCaretPosition& caret_position)
{
    ...
    return { caret_rect.layout_object, rect, &caret_position.cursor.ContainerFragment() };
}
