...
CORE_EXPORT InlineCaretPosition ComputeInlineCaretPosition(
    const LayoutBlockFlow& context, unsigned offset, TextAffinity affinity, const LayoutText* layout_text = nullptr);

// Shorthand of the above when the input is a position instead of a
// (context, offset) pair.
CORE_EXPORT InlineCaretPosition ComputeInlineCaretPosition(const PositionWithAffinity&);
