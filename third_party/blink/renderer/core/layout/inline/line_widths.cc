...
bool LineWidths::Set(const InlineNode& node, base::span<const LayoutOpportunity> opportunities, const InlineBreakToken* break_token)
{
    ...
    base::span<const InlineItem> items(items_data.items.data(), items_data.items.size());
    ...
    for (const InlineItem& item : items) {
        ...
        case InlineItem::kText: {
            ...
            if (shape_result->HasFallbackFonts(primary_font)) {
                ...
                text_box.ComputeTextMetrics(item_style, item_style.GetFont(), baseline_type);
    ...
    const LayoutUnit exclusion_block_size = last_opportunity.rect.BlockStartOffset() - first_opportunity.rect.BlockStartOffset();
    ...
    const float num_excluded_lines = ceil(exclusion_block_size.ToFloat() / line_height.ToFloat());
