...
public:
    ...
    LineBreakCandidate(const InlineItemTextIndex& offset, const InlineItemTextIndex& end, float pos_no_break, float pos_if_break, float penalty = .0f,
    ...
    {
        return LineBreakPoint::operator==(other) && pos_no_break == other.pos_no_break && pos_if_break == other.pos_if_break && penalty == other.penalty;
    }
...
CORE_EXPORT std::ostream& operator<<(std::ostream& ostream, const LineBreakCandidate& candidate);

//
// A vector of `LineBreakCandidate`.
//
using LineBreakCandidates = Vector<LineBreakCandidate, LineBreakCandidate::kInlineCapacity>;
...
public:
    ...
    float HyphenPenalty() const
    {
        return hyphen_penalty_;
    }
    void SetHyphenPenalty(float penalty)
    {
        hyphen_penalty_ = penalty;
    }
    ...
    const LineBreakCandidates& Candidates() const
    {
        return candidates_;
    }

    const InlineItem* LastItem() const
    {
        return last_item_;
    }
    wtf_size_t LastEndOffset() const
    {
        return last_end_offset_;
    }
    ...
    void Append(State new_state, InlineItemTextIndex offset, InlineItemTextIndex end, float pos_no_break, float pos_if_break, float penalty = .0f,
        bool is_hyphenated = false);
    void Append(State new_state, const InlineItemTextIndex& offset, float position);
    void AppendTrailingSpaces(State new_state, const InlineItemTextIndex& offset, float pos_no_break);
