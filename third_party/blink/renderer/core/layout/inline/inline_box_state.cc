...
FontHeight ComputeEmphasisMarkOutsets(const ComputedStyle& style, const Font& font)
{
    ...
    return style.GetTextEmphasisLineLogicalSide() == LineLogicalSide::kOver ? FontHeight(emphasis_mark_height, LayoutUnit())
...
void InlineBoxState::ResetStyle(const ComputedStyle& style_ref, bool is_svg, const LayoutObject& layout_object)
{
    style = &style_ref;
    is_svg_text = is_svg;
    if (!is_svg_text) {
    ...
    LayoutSVGInlineText::ComputeNewScaledFontForStyle(layout_object, scaling_factor, scaled_font);
    font = &scaled_font;
...
{
    const auto baseline_type = styleref.CssDominantBaseline() == EDominantBaseline::kAuto ? ifc_baseline : styleref.GetFontBaseline();
    if (const SimpleFontData* font_data = fontref.PrimaryFont()) {
        if (is_svg_text) {
            text_metrics = font_data->GetFontMetrics().GetFloatFontHeight(baseline_type);
        } else {
    ...
    FontHeight leading_space = CalculateLeadingSpace(styleref.ComputedLineHeightAsFixed(fontref), text_metrics);
...
void InlineBoxState::AdjustEdges(
    const TextBoxEdge text_box_edge, const Font& font, FontBaseline baseline_type, bool should_apply_over, bool should_apply_under, FontHeight& metrics)
{
...
void InlineBoxState::AccumulateUsedFonts(const ShapeResultView* shape_result)
{
    ...
    HeapHashSet<Member<const SimpleFontData>> used_fonts = shape_result->UsedFonts();
...
bool InlineBoxState::CanAddTextOfStyle(const ComputedStyle& text_style) const
{
    if (text_style.VerticalAlign() != EVerticalAlign::kBaseline)
        return false;
    DCHECK(style);
    if (style == &text_style || &style->GetFont() == &text_style.GetFont() || style->GetFont().PrimaryFont() == text_style.GetFont().PrimaryFont())
        return true;
    return false;
}
...
InlineBoxState* InlineLayoutStateStack::OnBeginPlaceItems(
    const InlineNode node, const ComputedStyle& line_style, FontBaseline baseline_type, bool line_height_quirk, LogicalLineItems* line_box)
{
    ...
    if (stack_.empty()) {
        ...
        for (InlineBoxState& box : stack_) {
            ...
            if (box.has_start_edge) {
                ...
                box.has_start_edge = box.needs_box_fragment && box.style->BoxDecorationBreak() == EBoxDecorationBreak::kClone;
            }
    ...
    if (line_box_state.style != &line_style) {
        line_box_state.ResetStyle(line_style, node.IsSvgText(), *node.GetLayoutBox());
...
InlineBoxState* InlineLayoutStateStack::OnOpenTag(
    const ConstraintSpace& space, const InlineItem& item, const InlineItemResult& item_result, FontBaseline baseline_type, LogicalLineItems* line_box)
{
    InlineBoxState* box = OnOpenTag(space, item, item_result, baseline_type, *line_box);
...
InlineBoxState* InlineLayoutStateStack::OnOpenTag(
    const ConstraintSpace& space, const InlineItem& item, const InlineItemResult& item_result, FontBaseline baseline_type, const LogicalLineItems& line_box)
{
    DCHECK(item.Style());
...
InlineBoxState* InlineLayoutStateStack::OnCloseTag(const ConstraintSpace& space, LogicalLineItems* line_box, InlineBoxState* box, FontBaseline baseline_type)
{
...
void InlineLayoutStateStack::OnEndPlaceItems(const ConstraintSpace& space, LogicalLineItems* line_box, FontBaseline baseline_type)
{
    for (auto& box : base::Reversed(stack_)) {
        if (!box.has_end_edge && box.needs_box_fragment && box.style->BoxDecorationBreak() == EBoxDecorationBreak::kClone)
            box.has_end_edge = true;
        EndBoxState(space, &box, line_box, baseline_type);
    }
...
void InlineLayoutStateStack::EndBoxState(const ConstraintSpace& space, InlineBoxState* box, LogicalLineItems* line_box, FontBaseline baseline_type)
{
    if (box->needs_box_fragment)
...
void InlineLayoutStateStack::OnBlockInInline(const FontHeight& metrics, LogicalLineItems* line_box)
{
...
// from placeholders.
void InlineLayoutStateStack::AddBoxFragmentPlaceholder(InlineBoxState* box, LogicalLineItems* line_box, FontBaseline baseline_type)
{
    DCHECK(box != stack_.data() && box->item->Type() != InlineItem::kAtomicInline);
    ...
    if (!is_empty_line_) {
        ...
        if (const auto* font_data = box->font->PrimaryFont()) {
            metrics = is_svg_text_ ? font_data->GetFontMetrics().GetFloatFontHeight(baseline_type) : font_data->GetFontMetrics().GetFontHeight(baseline_type);
        }
        ...
        block_offset = -metrics.ascent - (box->borders.line_over + box->padding.line_over);
        block_size = metrics.LineHeight() + box->borders.BlockSum() + box->padding.BlockSum();
    }
...
void InlineLayoutStateStack::AddBoxData(const ConstraintSpace& space, InlineBoxState* box, LogicalLineItems* line_box)
{
    ...
    DCHECK(box->item);
    BoxData& box_data = box_data_list_.emplace_back(box->fragment_start, fragment_end, box->item, placeholder.Size());
    ...
    if (box->has_start_edge) {
        ...
        box_data.margin_border_padding_line_left = box->margins.inline_start + box->borders.inline_start + box->padding.inline_start;
    } else {
    ...
    if (box->has_end_edge) {
        ...
        box_data.margin_border_padding_line_right = box->margins.inline_end + box->borders.inline_end + box->padding.inline_end;
    } else {
    ...
    if (IsRtl(style.Direction())) {
        ...
        std::swap(box_data.margin_border_padding_line_left, box_data.margin_border_padding_line_right);
    }
    ...
    for (const auto& logical_column : ruby_column_list_) {
        ...
        if (box->fragment_start <= logical_column->start_index && logical_column->EndIndex() <= fragment_end) {
            if (!box_data.ruby_column_list) {
                box_data.ruby_column_list = MakeGarbageCollected<HeapVector<Member<LogicalRubyColumn>>>();
            }
    ...
    placeholder.rect.offset += ComputeRelativeOffsetForInline(space, *box_data.item->Style());
    LayoutUnit advance = box_data.margin_border_padding_line_left + box_data.margin_border_padding_line_right;
    box_data.rect.size.inline_size = advance - box_data.margin_line_left - box_data.margin_line_right;
...
std::optional<std::pair<LayoutUnit, LayoutUnit>> InlineLayoutStateStack::AnnotationBoxBlockAxisMargins() const
{
    if (!HasBoxFragments() || box_data_list_[0].fragment_start != 0) {
        return std::nullopt;
    }
    const BoxData& data = box_data_list_[0];
    if (data.padding.BlockSum() == LayoutUnit() && data.borders.BlockSum() == LayoutUnit() && data.margin_line_over == LayoutUnit()
...
unsigned InlineLayoutStateStack::UpdateBoxDataFragmentRange(LogicalLineItems* line_box, unsigned index, HeapVector<BoxData>* fragmented_boxes)
{
    ...
    for (; index < line_box->size(); index++) {
        ...
        start->box_data_index = box_data_list_[box_data_index - 1].parent_box_data_index;
        ...
        for (index++; index < line_box->size(); index++) {
            ...
            if (box_data_index != end->box_data_index)
                break;
            end->box_data_index = box_data_list_[box_data_index - 1].parent_box_data_index;
        }
        ...
        if (!box_data_list_[box_data_index - 1].fragment_end) {
            ...
            // fragmented range.
            BoxData& fragmented_box = fragmented_boxes->emplace_back(box_data_list_[box_data_index - 1], start_index, index);
...
void InlineLayoutStateStack::UpdateFragmentedBoxDataEdges(HeapVector<BoxData>* fragmented_boxes)
{
    ...
    std::sort(fragmented_boxes->begin(), fragmented_boxes->end(), [](const BoxData& a, const BoxData& b) {
        if (a.fragmented_box_data_index != b.fragmented_box_data_index) {
            return a.fragmented_box_data_index < b.fragmented_box_data_index;
        }
...
void InlineLayoutStateStack::BoxData::UpdateFragmentEdges(HeapVector<BoxData, 4>& list)
{
    ...
    if (has_line_right_edge) {
        ...
        margin_line_right = margin_border_padding_line_right = padding.inline_end = LayoutUnit();
    }
}

LayoutUnit InlineLayoutStateStack::ComputeInlinePositions(LogicalLineItems* line_box, LayoutUnit position, bool ignore_box_margin_border_padding)
{
    ...
    if (!ignore_box_margin_border_padding) {
        ...
        for (BoxData& box_data : box_data_list_) {
            ...
            if (box_data.margin_border_padding_line_left) {
                line_box->MoveInInlineDirection(box_data.margin_border_padding_line_left, start, line_box->size());
                position += box_data.margin_border_padding_line_left;
            }

            if (box_data.margin_border_padding_line_right) {
                line_box->MoveInInlineDirection(box_data.margin_border_padding_line_right, end, line_box->size());
    ...
    for (BoxData& box_data : box_data_list_) {
        ...
        LayoutUnit line_left_offset = start_child.rect.offset.inline_offset - start_child.margin_line_left;
        ...
        LayoutUnit line_right_offset = last_child.rect.offset.inline_offset - last_child.margin_line_left + last_child.inline_size;
...
void InlineLayoutStateStack::ApplyRelativePositioning(const ConstraintSpace& space, LogicalLineItems* line_box, const LogicalOffset* parent_offset)
{
    ...
    for (BoxData& box_data : box_data_list_) {
        unsigned start = box_data.fragment_start;
        unsigned end = box_data.fragment_end;
        const LogicalOffset relative_offset = ComputeRelativeOffsetForInline(space, *box_data.item->Style());
    ...
    for (auto& logical_column : ruby_column_list_) {
        logical_column->state_stack.ApplyRelativePositioning(space, logical_column->annotation_items, &accumulated_offsets[logical_column->start_index]);
    }
}

void InlineLayoutStateStack::CreateBoxFragments(const ConstraintSpace& space, LogicalLineItems* line_box, bool is_opaque)
{
    for (auto& logical_column : ruby_column_list_) {
        logical_column->state_stack.CreateBoxFragments(space, logical_column->annotation_items, /* is_opaque */ false);
    }
    ...
    for (BoxData& box_data : box_data_list_) {
        ...
        const LayoutResult* box_fragment = box_data.CreateBoxFragment(space, line_box, is_opaque);
        ...
        line_box->InsertChild(start, std::move(box_fragment), box_data.rect, end - start + 1);
...
const LayoutResult* InlineLayoutStateStack::BoxData::CreateBoxFragment(const ConstraintSpace& space, LogicalLineItems* line_box, bool is_opaque)
{
    DCHECK(item);
    DCHECK(item->Style());
    const ComputedStyle& style = *item->Style();
    ...
    fragment_geometry.border_box_size = { rect.size.inline_size.ClampNegativeToZero(), rect.size.block_size };
    fragment_geometry.border = BoxStrut(borders, IsFlippedLinesWritingMode(style.GetWritingMode()));
    fragment_geometry.padding = BoxStrut(padding, IsFlippedLinesWritingMode(style.GetWritingMode()));
    ...
    BoxFragmentBuilder box(item->GetLayoutObject(), &style, space, { style.GetWritingMode(), TextDirection::kLtr });
    ...
    auto handle_box_child = [&](LogicalLineItem& child) {
        if (child.out_of_flow_positioned_box) {
            ...
            box.AddOutOfFlowInlineChildCandidate(oof_box, static_offset, child.container_direction, child.is_hidden_for_paint);
        ...
        if (child.layout_result) {
            ...
            box.PropagateFromLayoutResultAndFragment(*child.layout_result, child.rect.offset - rect.offset - ComputeRelativeOffsetForInline(space, child_style),
...
InlineLayoutStateStack::PositionPending InlineLayoutStateStack::ApplyBaselineShift(InlineBoxState* box, LogicalLineItems* line_box, FontBaseline baseline_type)
{
    ...
    if (!box->pending_descendants.empty()) {
        ...
        for (PendingPositions& child : box->pending_descendants) {
            ...
            line_box->MoveInBlockDirection(baseline_shift, child.fragment_start, child.fragment_end);
        }
    ...
    if (box->item && IsA<LayoutTextCombine>(box->item->GetLayoutObject())) [[unlikely]] {
    ...
    if (is_svg_text_) {
        ...
        case EBaselineShiftType::kLength: {
            ...
            const float computed_font_size = box->font->GetFontDescription().ComputedPixelSize() / box->scaling_factor;
            baseline_shift = LayoutUnit(-ValueForLength(length, style, computed_font_size) * box->scaling_factor);
            break;
        }
        case EBaselineShiftType::kSub:
            if (const auto* font_data = box->font->PrimaryFont()) {
                baseline_shift = LayoutUnit(font_data->GetFontMetrics().FloatHeight() / 2);
            }
            break;
        ...
        line_box->MoveInBlockDirection(baseline_shift, box->fragment_start, fragment_end);
        return kPositionNotPending;
    }
    ...
    case EVerticalAlign::kLength: {
        ...
        LayoutUnit line_height = length.HasPercent() ? style.ComputedLineHeightAsFixed() : box->text_metrics.LineHeight();
    ...
    case EVerticalAlign::kBottom: {
        ...
        for (; ancestor != stack_.data(); --ancestor) {
            if (ancestor->style->VerticalAlign() == EVerticalAlign::kTop || ancestor->style->VerticalAlign() == EVerticalAlign::kBottom)
                break;
        }
        ancestor->pending_descendants.push_back(PendingPositions { box->fragment_start, fragment_end, box->metrics, vertical_align });
        return kPositionPending;
    }
    default:
        // Other values require the layout size of the parent box.
        parent_box.pending_descendants.push_back(PendingPositions { box->fragment_start, fragment_end, box->metrics, vertical_align });
        return kPositionPending;
    }
    if (!box->metrics.IsEmpty())
        box->metrics.Move(baseline_shift);
...
LayoutUnit InlineLayoutStateStack::ComputeAlignmentBaselineShift(const InlineBoxState* box)
{
    LayoutUnit result;
    if (const auto* font_data = box->font->PrimaryFont()) {
        const FontMetrics& metrics = font_data->GetFontMetrics();
        result = metrics.FixedAscent(box->style->GetFontBaseline()) - metrics.FixedAscent(box->alignment_type);
    }
    ...
    if (const auto* font_data = box[-1].font->PrimaryFont()) {
        ...
        result -= parent_metrics.FixedAscent(box[-1].style->GetFontBaseline()) - parent_metrics.FixedAscent(box[-1].alignment_type);
    }

    return result;
}

FontHeight InlineLayoutStateStack::MetricsForTopAndBottomAlign(const InlineBoxState& box, const LogicalLineItems& line_box) const
{
    DCHECK(!box.pending_descendants.empty());
    ...
    for (const BoxData& box_data : box_data_list_) {
        ...
        FontHeight box_metrics(box_ascent, box_data.rect.size.block_size - box_ascent);
        ...
        FontHeight leading_space = CalculateLeadingSpace(style.ComputedLineHeightAsFixed(), box_metrics);
...
#if DCHECK_IS_ON()
void InlineLayoutStateStack::CheckSame(const InlineLayoutStateStack& other) const
{
