...
wtf_size_t GlyphCount(const InlineItemResult& item_result)
{
    if (item_result.shape_result) {
        ...
        wtf_size_t count = 0;
        for (const auto& nested_result : item_result.ruby_column->base_line.Results()) {
...
void LineInfo::SetLineStyle(const InlineNode& node, const InlineItemsData& items_data, bool use_first_line_style)
{
...
ETextAlign LineInfo::GetTextAlign(bool is_last_line) const
{
    ...
    if (is_ruby_text_) {
        ...
        if ((ruby_align == ERubyAlign::kSpaceAround && (text_align == ComputedStyleInitialValues::InitialTextAlign() || text_align == ETextAlign::kJustify))
...
bool LineInfo::ComputeNeedsAccurateEndPosition() const
{
    ...
    } else if (is_ruby_text_ && align_last == ComputedStyleInitialValues::InitialTextAlignLast()) {
...
unsigned LineInfo::InflowStartOffset() const
{
    for (const auto& item_result : Results()) {
        ...
        if ((item.Type() == InlineItem::kText || item.Type() == InlineItem::kControl || item.Type() == InlineItem::kAtomicInline) && item.Length() > 0) {
...
unsigned LineInfo::InflowEndOffsetInternal(bool skip_forced_break) const
{
    for (const auto& item_result : base::Reversed(Results())) {
        ...
        if (skip_forced_break) {
            if (item.Type() == InlineItem::kControl && ItemsData().text_content[item.StartOffset()] == kNewlineCharacter) {
                continue;
            } else if (item.Type() == InlineItem::kText && item.Length() == 0) {
                continue;
            }
        }
        if (item.Type() == InlineItem::kText || item.Type() == InlineItem::kControl || item.Type() == InlineItem::kAtomicInline) {
            return item_result.EndOffset();
        } else if (item_result.IsRubyColumn()) {
            const LineInfo& base_line = item_result.ruby_column->base_line;
            unsigned end_offset = base_line.InflowEndOffsetInternal(skip_forced_break);
...
{
    if (RuntimeEnabledFeatures::HangingWhitespaceDoesNotDependOnAlignmentEnabled()) {
        return true;
    }
...
LayoutUnit LineInfo::ComputeTrailingSpaceWidth(unsigned* end_offset_out) const
{
    ...
    for (const auto& item_result : base::Reversed(Results())) {
        ...
        DCHECK(item.Type() != InlineItem::kFloating && item.Type() != InlineItem::kOutOfFlowPositioned && item.Type() != InlineItem::kBidiControl);
        ...
        if (item.Type() == InlineItem::kControl || item_result.has_only_pre_wrap_trailing_spaces) {
            ...
            if (end_offset && IsHangingSpace(text[end_offset - 1])) {
                do {
                    --end_offset;
                } while (end_offset > item_result.StartOffset() && IsHangingSpace(text[end_offset - 1]));
                ...
                if (end_offset == item_result.StartOffset()) {
                    ...
                    float end_position = shape_result->PositionForOffset(end_offset - shape_result->StartIndex());
                    if (IsRtl(BaseDirection())) {
                        trailing_item_width = LayoutUnit(end_position);
                    } else {
                        trailing_item_width = LayoutUnit(shape_result->Width() - end_position);
                    }
                }
            }
        }

        if (trailing_item_width && RuntimeEnabledFeatures::HangingWhitespaceDoesNotDependOnAlignmentEnabled()) {
            ...
            case WhiteSpaceCollapse::kPreserve:
                if (item.Style()->ShouldWrapLine()) {
                    if (!trailing_spaces_width && (HasForcedBreak() || IsLastLine())) {
                        ...
                        LayoutUnit actual_hang_width = std::min(trailing_item_width, item_end - available_width_).ClampNegativeToZero();
...
LayoutUnit LineInfo::ComputeAnnotationBlockOffsetAdjustment() const
{
    if (annotation_block_start_adjustment_ < 0) {
        ...
        return annotation_block_start_adjustment_ + initial_letter_box_block_start_adjustment_;
    }
    // The raise/sunken initial letter may cover annotations[2].
    // [2]
    // https://wpt.live/css/css-inline/initial-letter/initial-letter-block-position-raise-over-ruby.html
    return std::max(annotation_block_start_adjustment_ - initial_letter_box_block_start_adjustment_, LayoutUnit());
}
...
LayoutUnit LineInfo::ComputeBlockStartAdjustment() const
{
    ...
    return std::max(annotation_block_start_adjustment_, initial_letter_box_block_start_adjustment_);
}
...
LayoutUnit LineInfo::ComputeInitialLetterBoxBlockStartAdjustment() const
{
    if (!annotation_block_start_adjustment_)
        return LayoutUnit();
    if (annotation_block_start_adjustment_ < 0) {
        return std::min(initial_letter_box_block_start_adjustment_ + annotation_block_start_adjustment_, LayoutUnit());
    }
    return std::max(annotation_block_start_adjustment_ - initial_letter_box_block_start_adjustment_, LayoutUnit());
}

LayoutUnit LineInfo::ComputeTotalBlockSize(LayoutUnit line_height, LayoutUnit annotation_overflow_block_end) const
{
    DCHECK_GE(annotation_overflow_block_end, LayoutUnit());
    const LayoutUnit line_height_with_annotation = line_height + annotation_block_start_adjustment_ + annotation_overflow_block_end;
...
void LineInfo::RemoveParallelFlowBreakToken(unsigned item_index)
{
    ...
    DCHECK(std::is_sorted(parallel_flow_break_tokens_.begin(), parallel_flow_break_tokens_.end(),
        [](const auto& a, const auto& b) { return a->StartItemIndex() < b->StartItemIndex(); }));
#endif //  EXPENSIVE_DCHECKS_ARE_ON()
    for (auto iter = parallel_flow_break_tokens_.begin(); iter != parallel_flow_break_tokens_.end(); ++iter) {
        const InlineBreakToken* break_token = *iter;
        DCHECK(break_token->IsInParallelBlockFlow());
        if (break_token->StartItemIndex() >= item_index) {
            const wtf_size_t index = static_cast<wtf_size_t>(iter - parallel_flow_break_tokens_.begin());
...
std::ostream& operator<<(std::ostream& ostream, const LineInfo& line_info)
{
    ...
    ostream << "LineInfo available_width_=" << line_info.AvailableWidth() << " width_=" << line_info.Width() << " Results=[\n";
