...
FragmentItem::FragmentItem(
    const InlineItem& inline_item, const ShapeResultView* shape_result, const TextOffsetRange& text_offset, const PhysicalSize& size, bool is_hidden_for_paint)
...
FragmentItem::FragmentItem(const LayoutObject& layout_object, TextItemType text_type, StyleVariant style_variant, TextDirection direction,
    const ShapeResultView* shape_result, const String& text_content, const PhysicalSize& size, bool is_hidden_for_paint)
...
FragmentItem::FragmentItem(
    const InlineItem& inline_item, const ShapeResultView* shape_result, const String& text_content, const PhysicalSize& size, bool is_hidden_for_paint)
    : FragmentItem(*inline_item.GetLayoutObject(), inline_item.TextType(), inline_item.GetStyleVariant(), inline_item.Direction(), shape_result, text_content,
        size, is_hidden_for_paint)
{
}
...
FragmentItem::FragmentItem(const PhysicalSize& size, const PhysicalLineBoxFragment& base_line)
    : line_({ nullptr, /* descendants_count */ 1 })
    , rect_({ PhysicalOffset(), size })
    , layout_object_(base_line.ContainerLayoutObject())
    , const_type_(kLine)
    , sub_type_(static_cast<unsigned>(FragmentItem::LineBoxType::kNormalLineBox))
...
FragmentItem::FragmentItem(const PhysicalBoxFragment& box, TextDirection resolved_direction)
...
// this one.
FragmentItem::FragmentItem(LogicalLineItem&& line_item, WritingMode writing_mode)
    : const_type_(kInvalid)
{
    ...
    if (line_item.inline_item) {
        if (line_item.text_content) [[unlikely]] {
            new (this) FragmentItem(*line_item.inline_item, std::move(line_item.shape_result), line_item.text_content,
                ToPhysicalSize(line_item.MarginSize(), writing_mode), line_item.is_hidden_for_paint);
            has_over_annotation_ = line_item.has_over_annotation;
            has_under_annotation_ = line_item.has_under_annotation;
            return;
        }

        new (this) FragmentItem(*line_item.inline_item, std::move(line_item.shape_result), line_item.text_offset,
            ToPhysicalSize(line_item.MarginSize(), writing_mode), line_item.is_hidden_for_paint);
        has_over_annotation_ = line_item.has_over_annotation;
        has_under_annotation_ = line_item.has_under_annotation;
        return;
    ...
    if (line_item.layout_result) {
        const auto& box_fragment = To<PhysicalBoxFragment>(line_item.layout_result->GetPhysicalFragment());
    ...
    if (line_item.layout_object) {
        ...
        new (this) FragmentItem(*line_item.layout_object, TextItemType::kLayoutGenerated, line_item.style_variant, direction, std::move(line_item.shape_result),
            line_item.text_content, ToPhysicalSize(line_item.MarginSize(), writing_mode), line_item.is_hidden_for_paint);
        return;
    }
...
FragmentItem::FragmentItem(const FragmentItem& source)
    ...
    , delta_to_next_for_same_layout_object_(source.delta_to_next_for_same_layout_object_)
    ...
    , is_last_for_node_(source.is_last_for_node_)
{
    ...
    if (source.IsInkOverflowComputed()) {
        ...
        new (&ink_overflow_) InkOverflow(source.InkOverflowType(), source.ink_overflow_);
    }
}
    ...
    , is_last_for_node_(source.is_last_for_node_)
{
    ...
    case kText:
        ...
        break;
    case kGeneratedText:
        new (&generated_text_) GeneratedTextItem(std::move(source.generated_text_));
        break;
    case kLine:
...
void FragmentItem::SetSvgFragmentData(const SvgFragmentData* data, const PhysicalRect& unscaled_rect, bool is_hidden)
{
...
gfx::RectF FragmentItem::ObjectBoundingBox(const FragmentItems& items) const
{
    ...
    case WritingMode::kSidewaysRl:
        ink_bounds = gfx::RectF(scaled_rect.width() - ink_bounds.bottom(), ink_bounds.x(), ink_bounds.height(), ink_bounds.width());
        break;
    case WritingMode::kSidewaysLr:
        ink_bounds = gfx::RectF(ink_bounds.y(), scaled_rect.height() - ink_bounds.right(), ink_bounds.height(), ink_bounds.width());
        break;
    }
...
{
    DCHECK(IsSvgText());
    gfx::QuadF quad = BuildSvgTransformForBoundingBox().MapQuad(gfx::QuadF(GetSvgFragmentData()->rect));
    const float scaling_factor = SvgScalingFactor();
    quad.Scale(1 / scaling_factor, 1 / scaling_factor);
    return quad;
}

PhysicalOffset FragmentItem::MapPointInContainer(const PhysicalOffset& point) const
{
    if (IsSvgText() && HasSvgTransformForBoundingBox()) {
        const float scaling_factor = SvgScalingFactor();
        return PhysicalOffset::FromPointFRound(
            gfx::ScalePoint(BuildSvgTransformForBoundingBox().Inverse().MapPoint(gfx::ScalePoint(gfx::PointF(point), scaling_factor)), scaling_factor));
    }
    return point;
}
...
float FragmentItem::ScaleInlineOffset(LayoutUnit inline_offset) const
{
    if (const SvgFragmentData* svg_data = GetSvgFragmentData()) {
        return inline_offset.ToFloat() * SvgScalingFactor() / svg_data->length_adjust_scale;
    }
...
bool FragmentItem::InclusiveContains(const gfx::PointF& position) const
{
    ...
    return BuildSvgTransformForBoundingBox().MapQuad(gfx::QuadF(item_rect)).Contains(scaled_position);
}
...
FragmentItem::BoxItem::BoxItem(const PhysicalBoxFragment* box_fragment, wtf_size_t descendants_count)
...
{
    if (Type() == kBox) {
        return DynamicTo<LayoutBox>(const_cast<LayoutObject*>(layout_object_.Get()));
    }
    return nullptr;
}
...
unsigned FragmentItem::StartOffsetInContainer(const InlineCursor& container) const
{
...
{
    if (Type() == kText) {
        return StringView(items.Text(UsesFirstLineStyle()), text_.text_offset.start, text_.text_offset.Length());
    }
    if (Type() == kGeneratedText)
...
TextFragmentPaintInfo FragmentItem::TextPaintInfo(const FragmentItems& items) const
{
    if (Type() == kText) {
        return { items.Text(UsesFirstLineStyle()), text_.text_offset.start, text_.text_offset.end, text_.shape_result.Get() };
    }
    if (Type() == kGeneratedText) {
        return { generated_text_.text, 0, generated_text_.text.length(), generated_text_.shape_result.Get() };
    }
    NOTREACHED();
}
...
AffineTransform FragmentItem::BuildSvgTransformForLengthAdjust() const
{
    ...
    if (scale != 1.0f) {
        ...
        // rotation.
        const bool with_text_path_transform = svg_data.in_text_path && svg_data.angle != 0.0f;
        // We'd like to scale only inline-size without moving inline position.
        if (is_horizontal) {
            float x = svg_data.rect.x();
            scale_transform.SetMatrix(scale, 0, 0, 1, with_text_path_transform ? 0 : x - scale * x, 0);
        } else {
            ...
            scale_transform.SetMatrix(1, 0, 0, scale, 0, with_text_path_transform ? 0 : y - scale * y);
        }
    }
    return scale_transform;
}

AffineTransform FragmentItem::BuildSvgTransformForTextPath(const AffineTransform& length_adjust) const
{
    DCHECK(IsSvgText());
    const SvgFragmentData& svg_data = *text_.svg_data;
...
AffineTransform FragmentItem::BuildSvgTransformForBoundingBox() const
{
    ...
    float ascent = font_data ? font_data->GetFontMetrics().FixedAscent().ToFloat() : 0.0f;
...
{
    const auto* svg_inline_text = DynamicTo<LayoutSVGInlineText>(GetLayoutObject());
    if (!svg_inline_text)
        return 1.0f;
...
{
    if (const auto* svg_inline_text = DynamicTo<LayoutSVGInlineText>(GetLayoutObject()))
...
String FragmentItem::ToString() const
{
    ...
    case FragmentItem::kText: {
        ...
        if (const LayoutBlockFlow* block_flow = layout_object_->FragmentItemsContainer()) {
            for (unsigned i = 0; i < block_flow->PhysicalFragmentCount(); ++i) {
                const PhysicalBoxFragment* containing_fragment = block_flow->GetPhysicalFragment(i);
...
PhysicalRect FragmentItem::LocalVisualRectFor(const LayoutObject& layout_object)
{
    DCHECK(layout_object.IsInLayoutNGInlineFormattingContext());

    PhysicalRect visual_rect;
    InlineCursor cursor;
    for (cursor.MoveTo(layout_object); cursor; cursor.MoveToNextForSameLayoutObject()) {
...
{
    ink_overflow_type_ = static_cast<unsigned>(ink_overflow_.Invalidate(InkOverflowType()));
}

PhysicalRect FragmentItem::RecalcInkOverflowForCursor(InlineCursor* cursor, InlinePaintContext* inline_context)
{
    DCHECK(cursor);
    ...
    for (; *cursor; cursor->MoveToNextSkippingChildren()) {
        ...
        item->GetMutableForPainting().RecalcInkOverflow(*cursor, inline_context, &child_rect);
...
void FragmentItem::RecalcInkOverflow(const InlineCursor& cursor, InlinePaintContext* inline_context, PhysicalRect* self_and_contents_rect_out)
{
    ...
    if (IsText()) {
        ...
        if (paint_info.shape_result) {
            if (const SvgFragmentData* svg_data = GetSvgFragmentData()) {
                ink_overflow_type_ = static_cast<unsigned>(ink_overflow_.SetSvgTextInkOverflow(InkOverflowType(), cursor, paint_info, Style(), ScaledFont(),
                    svg_data->rect, SvgScalingFactor(), svg_data->length_adjust_scale, BuildSvgTransformForBoundingBox(), self_and_contents_rect_out));
                return;
            }
            // Create |ScopedInlineItem| here because the decoration box is not
            // supported for SVG.
            InlinePaintContext::ScopedInlineItem scoped_inline_item(*this, inline_context);
            ink_overflow_type_ = static_cast<unsigned>(ink_overflow_.SetTextInkOverflow(
                InkOverflowType(), cursor, paint_info, Style(), RectInContainerFragment(), inline_context, self_and_contents_rect_out));
            return;
        }

        ink_overflow_type_ = static_cast<unsigned>(ink_overflow_.Reset(InkOverflowType()));
    ...
    if (Type() == kBox) {
        ...
        InlinePaintContext::ScopedInlineItem scoped_inline_item(*this, inline_context);
        const PhysicalRect contents_rect = RecalcInkOverflowForDescendantsOf(cursor, inline_context);
    ...
    if (Type() == kLine) {
        ...
        PhysicalRect contents_rect = RecalcInkOverflowForDescendantsOf(cursor, inline_context);
        ...
        *self_and_contents_rect_out = UnionRect(LocalRect(), contents_rect);
        ink_overflow_type_ = static_cast<unsigned>(ink_overflow_.SetContents(InkOverflowType(), contents_rect, Size()));
        return;
    }
...
PhysicalRect FragmentItem::RecalcInkOverflowForDescendantsOf(const InlineCursor& cursor, InlinePaintContext* inline_context) const
{
    ...
    PhysicalRect contents_rect = RecalcInkOverflowForCursor(&descendants_cursor, inline_context);
...
LayoutUnit FragmentItem::CaretInlinePositionForOffset(StringView text, unsigned offset) const
{
    ...
    if (TextShapeResult()) {
        ...
        return LayoutUnit::FromFloatRound(TextShapeResult()->CreateShapeResult()->CaretPositionForOffset(offset, text, AdjustMidCluster::kToEnd));
    }
    ...
    if (const SvgFragmentData* svg_data = GetSvgFragmentData()) {
        return LayoutUnit(IsHorizontal() ? svg_data->rect.width() : svg_data->rect.height());
    }
    return IsHorizontal() ? Size().width : Size().height;
}

std::pair<LayoutUnit, LayoutUnit> FragmentItem::LineLeftAndRightForOffsets(StringView text, unsigned start_offset, unsigned end_offset) const
{
    ...
    if (TextShapeResult()) {
        ...
        float unrounded_start_position = shape_result->CaretPositionForOffset(start_offset, text, AdjustMidCluster::kToStart);
        float unrounded_end_position = shape_result->CaretPositionForOffset(end_offset, text, AdjustMidCluster::kToEnd);
        ...
        if (!start_offset) {
            ...
            start_position = LayoutUnit(IsHorizontal() ? GetSvgFragmentData()->rect.width() : GetSvgFragmentData()->rect.height());
        } else {
        ...
        if (!end_offset) {
            ...
            end_position = LayoutUnit(IsHorizontal() ? GetSvgFragmentData()->rect.width() : GetSvgFragmentData()->rect.height());
        } else {
...
PhysicalRect FragmentItem::LocalRect(StringView text, unsigned start_offset, unsigned end_offset) const
{
    LayoutUnit width = Size().width;
    LayoutUnit height = Size().height;
    if (const SvgFragmentData* svg_data = GetSvgFragmentData()) {
        if (IsHorizontal()) {
            width = LayoutUnit(svg_data->rect.size().width() / svg_data->length_adjust_scale);
            height = LayoutUnit(svg_data->rect.size().height());
        } else {
            width = LayoutUnit(svg_data->rect.size().width());
            height = LayoutUnit(svg_data->rect.size().height() / svg_data->length_adjust_scale);
        }
    }
    ...
    std::tie(start_position, end_position) = LineLeftAndRightForOffsets(text, start_offset, end_offset);
...
PhysicalRect FragmentItem::ComputeTextBoundsRectForHitTest(const PhysicalOffset& inline_root_offset, bool is_occlusion_test) const
{
    DCHECK(IsText());
    const PhysicalOffset offset = inline_root_offset + OffsetInContainerFragment();
...
PositionWithAffinity FragmentItem::PositionForPointInText(const PhysicalOffset& point, const InlineCursor& cursor) const
{
    DCHECK_EQ(Type(), kText);
    DCHECK_EQ(cursor.CurrentItem(), this);
    if (IsGeneratedText())
        return PositionWithAffinity();
    return PositionForPointInText(TextOffsetForPoint(point, cursor.Items()), cursor);
}

PositionWithAffinity FragmentItem::PositionForPointInText(unsigned text_offset, const InlineCursor& cursor) const
{
    DCHECK_EQ(Type(), kText);
    DCHECK_EQ(cursor.CurrentItem(), this);
    DCHECK(!IsGeneratedText());
    DCHECK_LE(text_offset, EndOffset());
    const InlineCaretPosition unadjusted_position { cursor, InlineCaretPositionType::kAtTextOffset, text_offset };
    if (RuntimeEnabledFeatures::BidiCaretAffinityEnabled())
        return unadjusted_position.ToPositionInDOMTreeWithAffinity();
    if (text_offset > StartOffset() && text_offset < EndOffset())
        return unadjusted_position.ToPositionInDOMTreeWithAffinity();
    return BidiAdjustment::AdjustForHitTest(unadjusted_position).ToPositionInDOMTreeWithAffinity();
}

unsigned FragmentItem::TextOffsetForPoint(const PhysicalOffset& point, const FragmentItems& items) const
{
    DCHECK_EQ(Type(), kText);
    WritingModeConverter converter({ GetWritingMode(), TextDirection::kLtr }, Size());
    const LayoutUnit point_in_line_direction = converter.ToLogical(point, PhysicalSize()).inline_offset;
    if (const ShapeResultView* shape_result = TextShapeResult()) {
        ...
        return shape_result->CreateShapeResult()->CaretOffsetForHitTest(scaled_offset, Text(items), BreakGlyphsOption(true)) + StartOffset();
    }
    ...
    LayoutUnit inline_offset = IsLtr(ResolvedDirection()) ? point_in_line_direction : size.inline_size - point_in_line_direction;
...
std::ostream& operator<<(std::ostream& ostream, const FragmentItem& item)
{
    ...
    case FragmentItem::kText:
        ostream << "Text " << item.StartOffset() << "-" << item.EndOffset() << " " << (IsLtr(item.ResolvedDirection()) ? "LTR" : "RTL");
        break;
    case FragmentItem::kGeneratedText:
        ostream << "GeneratedText \"" << item.GeneratedText() << "\"";
        break;
    case FragmentItem::kLine:
        ostream << "Line #descendants=" << item.DescendantsCount() << " " << (IsLtr(item.BaseDirection()) ? "LTR" : "RTL");
        break;
    case FragmentItem::kBox:
        ostream << "Box #descendants=" << item.DescendantsCount();
        if (item.IsAtomicInline()) {
            ostream << " AtomicInline" << (IsLtr(item.ResolvedDirection()) ? "LTR" : "RTL");
        }
        break;
    }
    ostream << " ";
