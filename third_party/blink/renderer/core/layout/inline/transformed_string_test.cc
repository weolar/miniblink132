...
TEST(TransformedStringTest, CreateLengthMap)
{
    ...
    for (const auto& data : kTestData) {
        ...
        String transformed = CaseMap(AtomicString(data.locale)).ToUpper(source, &offset_map);
        EXPECT_EQ(String(data.expected_string), transformed);
        EXPECT_EQ(data.expected_map, TransformedString::CreateLengthMap(source.length(), transformed.length(), offset_map));
    }
}
...
TEST(TransformedStringTest, CreateLengthMapCombiningMark)
{
    ...
    Vector<unsigned> length_map = TransformedString::CreateLengthMap(1000u, 1u, offset_map);
