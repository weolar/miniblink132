...
class InlineTextAutoSpaceTest : public RenderingTest, ScopedCSSTextAutoSpaceForTest {
public:
    explicit InlineTextAutoSpaceTest()
        : ScopedCSSTextAutoSpaceForTest(true)
    {
    }

    LayoutBlockFlow* PreparePageLayoutBlock(String html, String container_css = String())
    {
        html = String(R"HTML(
      <style>
      #container {
    ...
    Vector<wtf_size_t> AutoSpaceOffsets(String html, String container_css = String())
    {
        const LayoutBlockFlow* container = PreparePageLayoutBlock(html, container_css);
...
TEST_F(InlineTextAutoSpaceTest, NonHanIdeograph)
{
    ...
    for (UChar ch = TextAutoSpace::kNonHanIdeographMin - 1; ch <= TextAutoSpace::kNonHanIdeographMax + 1; ++ch) {
...
TEST_F(InlineTextAutoSpaceTest, InsertSpacing)
{
    ...
    for (wtf_size_t i = 0; i < final_ranges.size(); i++) {
        EXPECT_NEAR(final_ranges[i].start, expected_result_start[i], 1e-6) << "unexpected width at position i of " << i;
    }
}
...
struct HtmlData {
    ...
    { u"あ<span style='text-orientation: upright'>1</span>あ", {}, "writing-mode: vertical-rl" },
...
class HtmlTest : public InlineTextAutoSpaceTest, public testing::WithParamInterface<HtmlData> { };
INSTANTIATE_TEST_SUITE_P(InlineTextAutoSpaceTest, HtmlTest, testing::ValuesIn(g_html_data));
