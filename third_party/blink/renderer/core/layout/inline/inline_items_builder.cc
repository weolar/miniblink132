...
InlineItemsBuilderTemplate<MappingBuilder>::InlineItemsBuilderTemplate(
    LayoutBlockFlow* block_flow, HeapVector<InlineItem>* items, const String& previous_text_content, const SvgTextChunkOffsets* chunk_offsets)
...
template <typename MappingBuilder> bool InlineItemsBuilderTemplate<MappingBuilder>::NeedsBoxInfo()
{
    return !std::is_same<OffsetMappingBuilder, MappingBuilder>::value;
}

template <typename MappingBuilder> InlineItemsBuilderTemplate<MappingBuilder>::~InlineItemsBuilderTemplate()
{
    DCHECK_EQ(0u, bidi_context_.size());
    DCHECK_EQ(text_.length(), items_->empty() ? 0 : items_->back().EndOffset());
}

template <typename MappingBuilder> String InlineItemsBuilderTemplate<MappingBuilder>::ToString()
{
...
{
    UEastAsianWidth eaw = static_cast<UEastAsianWidth>(u_getIntPropertyValue(c, UCHAR_EAST_ASIAN_WIDTH));
    return eaw == U_EA_WIDE || eaw == U_EA_FULLWIDTH || eaw == U_EA_HALFWIDTH || (eaw == U_EA_AMBIGUOUS && style && IsAmbiguosEastAsianWidthWide(style));
}
#endif
...
bool ShouldRemoveNewlineSlow(
    const StringBuilder& before, unsigned space_index, const ComputedStyle* before_style, const StringView& after, const ComputedStyle* after_style)
{
    // Remove if either before/after the newline is zeroWidthSpaceCharacter.
    UChar32 last = 0;
    DCHECK(space_index == before.length() || (space_index < before.length() && before[space_index] == ' '));
    if (space_index) {
...
bool ShouldRemoveNewline(
    const StringBuilder& before, unsigned space_index, const ComputedStyle* before_style, const StringView& after, const ComputedStyle* after_style)
{
    // All characters before/after removable newline are 16 bits.
    return (!before.Is8Bit() || !after.Is8Bit()) && ShouldRemoveNewlineSlow(before, space_index, before_style, after, after_style);
}

inline InlineItem& AppendItem(HeapVector<InlineItem>* items, InlineItem::InlineItemType type, unsigned start, unsigned end, LayoutObject* layout_object)
{
...
template <typename MappingBuilder>
InlineItemsBuilderTemplate<MappingBuilder>::BoxInfo::BoxInfo(unsigned item_index, const InlineItem& item)
    : style(*item.Style())
...
template <typename MappingBuilder> bool InlineItemsBuilderTemplate<MappingBuilder>::BoxInfo::ShouldCreateBoxFragmentForChild(const BoxInfo& child) const
{
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::BoxInfo::SetShouldCreateBoxFragment(HeapVector<InlineItem>* items)
{
...
template <typename MappingBuilder>
void InlineItemsBuilderTemplate<MappingBuilder>::AppendTextItem(const TransformedString& transformed, LayoutText* layout_object)
{
...
InlineItem& InlineItemsBuilderTemplate<MappingBuilder>::AppendTextItem(
    InlineItem::InlineItemType type, const TransformedString& transformed, LayoutText* layout_object)
{
    DCHECK(layout_object);
    unsigned start_offset = text_.length();
    AppendTransformedString(transformed, *layout_object);
    InlineItem& item = AppendItem(items_, type, start_offset, text_.length(), layout_object);
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendEmptyTextItem(LayoutText* layout_object)
{
    DCHECK(layout_object);
    unsigned offset = text_.length();
    InlineItem& item = AppendItem(items_, InlineItem::kText, offset, offset, layout_object);
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendGeneratedBreakOpportunity(LayoutObject* layout_object)
{
...
template <typename MappingBuilder> inline void InlineItemsBuilderTemplate<MappingBuilder>::DidAppendForcedBreak()
{
...
template <typename MappingBuilder> inline void InlineItemsBuilderTemplate<MappingBuilder>::DidAppendTextReusing(const InlineItem& item)
{
...
template <typename MappingBuilder>
bool InlineItemsBuilderTemplate<MappingBuilder>::AppendTextReusing(const InlineNodeData& original_data, LayoutText* layout_text)
{
    DCHECK(layout_text);
        ...
        if (collapse_spaces) {
            ...
            case InlineItem::kCollapsible:
                ...
                if (last_item->IsEndCollapsibleNewline()) {
                    const StringView old_item0_view(original_string, old_item0.StartOffset(), old_item0.Length());
                    if (ShouldRemoveNewline(text_, last_item->EndOffset() - 1, last_item->Style(), old_item0_view, &new_style)) {
                        return false;
                    }
                }
                break;
            case InlineItem::kNotCollapsible: {
                ...
                if (source_text.length() && Character::IsCollapsibleSpace(source_text[0])) {
                    ...
                    bool contains_newline = MoveToEndOfCollapsibleSpaces(source_text, &offset, &c);
                    if (contains_newline && ShouldRemoveNewline(text_, text_.length(), last_item->Style(), StringView(source_text, offset), &new_style)) {
                        return false;
                    }
                }
                break;
    ...
    if (old_item0.StartOffset() > 0 && ShouldInsertBreakOpportunityAfterLeadingPreservedSpaces(layout_text->TransformedText(), new_style)) [[unlikely]] {
    ...
    for (const InlineItem& item : items) {
        ...
        has_non_orc_16bit_ = has_non_orc_16bit_ || original_data.HasNonOrc16BitCharacters();
        ...
        InlineItem& adjusted_item = items_->emplace_back(item, start, end, adjusted_shape_result);
#if DCHECK_IS_ON()
...
template <> bool InlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendTextReusing(const InlineNodeData&, LayoutText*)
{
    NOTREACHED();
}

template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendText(LayoutText* layout_text, const InlineNodeData* previous_data)
{
    ...
    if (layout_text->IsWordBreak()) [[unlikely]] {
        typename MappingBuilder::SourceNodeScope scope(&mapping_builder_, layout_text);
    ...
    auto [original_length, offset_map] = layout_text->GetVariableLengthTransformResult();
    String transformed = layout_text->TransformedText();
    const Vector<unsigned> length_map = TransformedString::CreateLengthMap(original_length, transformed.length(), offset_map);
    CHECK(transformed.length() == length_map.size() || length_map.size() == 0);
    AppendText(TransformedString(transformed, { length_map.data(), length_map.size() }), *layout_text);
}

template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendText(const String& string, LayoutText* layout_object)
{
    AppendText(TransformedString(string), *layout_object);
}

template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendText(const TransformedString& transformed, LayoutText& layout_object)
{
    ...
    if (estimated_length > text_.Capacity()) {
        ...
        const wtf_size_t new_capacity = std::max(estimated_length, text_.Capacity() * 2);
        if (string.Is8Bit())
    ...
    typename MappingBuilder::SourceNodeScope scope(&mapping_builder_, &layout_object);

    const ComputedStyle& style = layout_object.StyleRef();
    bool should_not_preserve_newline;
    if (layout_object.IsSVGInlineText() || is_text_combine_ || ruby_text_nesting_level_ > 0) [[unlikely]] {
...
template <typename MappingBuilder>
bool InlineItemsBuilderTemplate<MappingBuilder>::AppendTextChunks(const TransformedString& transformed, LayoutText& layout_text)
{
    ...
    for (unsigned offset : iter->value) {
        ...
        if (start < offset) {
            if (!should_collapse_space) {
                AppendPreserveWhitespace(transformed.Substring(start, offset - start), &style, &layout_text);
            } else {
                AppendCollapseWhitespace(transformed.Substring(start, offset - start), &style, &layout_text);
            }
        }
    ...
    if (!should_collapse_space) {
        AppendPreserveWhitespace(transformed.Substring(start), &style, &layout_text);
    } else {
        AppendCollapseWhitespace(transformed.Substring(start), &style, &layout_text);
    }
    return true;
}

template <typename MappingBuilder>
void InlineItemsBuilderTemplate<MappingBuilder>::AppendTransformedString(const TransformedString& transformed, const LayoutText& layout_text)
{
...
void InlineItemsBuilderTemplate<MappingBuilder>::AppendCollapseWhitespace(
    const TransformedString& transformed, const ComputedStyle* style, LayoutText* layout_object)
{
    StringView string = transformed.View();
    DCHECK(!string.empty());
    ...
    if (Character::IsCollapsibleSpace(c)) {
        ...
        if (space_run_has_newline && string.length() == 1 && layout_object && layout_object->IsBR()) [[unlikely]] {
        ...
        if (InlineItem* item = LastItemToCollapseWith(items_)) {
            if (item->EndCollapseType() == InlineItem::kNotCollapsible) {
                ...
                if ((space_run_has_newline || item->IsEndCollapsibleNewline()) && item->Type() == InlineItem::kText
                    && ShouldRemoveNewline(text_, item->EndOffset() - 1, item->Style(), StringView(string, i), style)) {
                    RemoveTrailingCollapsibleSpace(item);
                    space_run_has_newline = false;
                } else if (!item->Style()->ShouldWrapLine() && style->ShouldWrapLine()) {
                    ...
                    if (item->Type() != InlineItem::kControl || text_[item->StartOffset()] != kNewlineCharacter) {
        ...
        if (space_run_has_newline && ShouldRemoveNewline(text_, text_.length(), style, StringView(string, i), style)) {
        ...
        // last item.
        if (InlineItem* item = LastItemToCollapseWith(items_)) {
            if (item->EndCollapseType() == InlineItem::kCollapsible && item->IsEndCollapsibleNewline()
                && ShouldRemoveNewline(text_, item->EndOffset() - 1, item->Style(), string, style)) {
                RemoveTrailingCollapsibleSpace(item);
            }
        }
    ...
    if (i < string.length()) {
        while (true) {
            ...
            AppendTransformedString(transformed.Substring(start_of_non_space, i - start_of_non_space), *layout_object);
            ...
            // If this space run contains a newline, apply segment break rules.
            bool remove_newline = space_run_has_newline && ShouldRemoveNewline(text_, text_.length(), style, StringView(string, i), style);
    ...
    InlineItem& item = AppendItem(items_, InlineItem::kText, start_offset, text_.length(), layout_object);
...
template <typename MappingBuilder>
bool InlineItemsBuilderTemplate<MappingBuilder>::ShouldInsertBreakOpportunityAfterLeadingPreservedSpaces(
    StringView string, const ComputedStyle& style, unsigned index) const
{
    ...
    if (style.ShouldCollapseWhiteSpaces() || !style.ShouldWrapLine() || !string.length() || index >= string.length() || string[index] != kSpaceCharacter) {
        return false;
    }
...
template <typename MappingBuilder>
void InlineItemsBuilderTemplate<MappingBuilder>::InsertBreakOpportunityAfterLeadingPreservedSpaces(
    const TransformedString& transformed, const ComputedStyle& style, LayoutText* layout_object, unsigned* start)
{
    DCHECK(start);
    StringView string = transformed.View();
    ...
    const TransformedString& transformed, const ComputedStyle* style, LayoutText* layout_object)
{
    ...
    while (start < length) {
        ...
        case kTabulationCharacter: {
            wtf_size_t tab_end = transformed_view.Find([](UChar c) { return c != kTabulationCharacter; }, start + 1);
            if (tab_end == kNotFound) {
                tab_end = length;
            }
            InlineItem& item = AppendTextItem(InlineItem::kControl, transformed.Substring(start, tab_end - start), layout_object);
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendForcedBreak(LayoutObject* layout_object)
{
    DCHECK(layout_object);
    ...
    InlineItem& item = Append(InlineItem::kControl, kNewlineCharacter, layout_object);
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendForcedBreakCollapseWhitespace(LayoutObject* layout_object)
{
...
template <typename MappingBuilder> InlineItem& InlineItemsBuilderTemplate<MappingBuilder>::AppendBreakOpportunity(LayoutObject* layout_object)
{
    DCHECK(layout_object);
    InlineItem& item = AppendOpaque(InlineItem::kControl, kZeroWidthSpaceCharacter, layout_object);
    item.SetTextType(TextItemType::kFlowControl);
    return item;
}

// The logic is similar to AppendForcedBreak().
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::ExitAndEnterSvgTextChunk(LayoutText& layout_text)
{
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::EnterSvgTextChunk(const ComputedStyle* style)
{
    if (!block_flow_->IsSVGText() || !text_chunk_offsets_) [[likely]] {
        return;
    }
    EnterBidiContext(nullptr, style, kLeftToRightIsolateCharacter, kRightToLeftIsolateCharacter, kPopDirectionalIsolateCharacter);
    // This context is automatically popped by Exit(nullptr) in ExitBlock().
}

template <typename MappingBuilder>
InlineItem& InlineItemsBuilderTemplate<MappingBuilder>::Append(InlineItem::InlineItemType type, UChar character, LayoutObject* layout_object)
{
    ...
    InlineItem& item = AppendItem(items_, type, end_offset - 1, end_offset, layout_object);
    is_block_level_ &= item.IsBlockLevel();
    return item;
}

template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendAtomicInline(LayoutObject* layout_object)
{
    DCHECK(layout_object);
    typename MappingBuilder::SourceNodeScope scope(&mapping_builder_, layout_object);
    RestoreTrailingCollapsibleSpaceIfRemoved();
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendBlockInInline(LayoutObject* layout_object)
{
    DCHECK(layout_object);
    // Before a block-in-inline is like after a forced break.
    RemoveTrailingCollapsibleSpaceIfExists();
    InlineItem& item = Append(InlineItem::kBlockInInline, kObjectReplacementCharacter, layout_object);
    ...
    if (ShouldUpdateLayoutObject()) {
        ...
        DCHECK(!layout_object->Parent() || IsA<LayoutInline>(layout_object->Parent()));
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendFloating(LayoutObject* layout_object)
{
    AppendOpaque(InlineItem::kFloating, kObjectReplacementCharacter, layout_object);
    has_floats_ = true;
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendOutOfFlowPositioned(LayoutObject* layout_object)
{
    AppendOpaque(InlineItem::kOutOfFlowPositioned, kObjectReplacementCharacter, layout_object);
}

template <typename MappingBuilder>
InlineItem& InlineItemsBuilderTemplate<MappingBuilder>::AppendOpaque(InlineItem::InlineItemType type, UChar character, LayoutObject* layout_object)
{
    has_non_orc_16bit_ = has_non_orc_16bit_ || IsNonOrc16BitCharacter(character);
    text_.Append(character);
    mapping_builder_.AppendIdentityMapping(1);
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::AppendOpaque(InlineItem::InlineItemType type, LayoutObject* layout_object)
{
    unsigned end_offset = text_.length();
    InlineItem& item = AppendItem(items_, type, end_offset, end_offset, layout_object);
    item.SetEndCollapseType(InlineItem::kOpaqueToCollapsing);
    is_block_level_ &= item.IsBlockLevel();
}

// Removes the collapsible space at the end of |text_| if exists.
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::RemoveTrailingCollapsibleSpaceIfExists()
{
    if (InlineItem* item = LastItemToCollapseWith(items_)) {
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::RemoveTrailingCollapsibleSpace(InlineItem* item)
{
    DCHECK(item);
    ...
    if (item->Type() != InlineItem::kText) {
        DCHECK(item->Type() == InlineItem::kControl || item->Type() == InlineItem::kBlockInInline);
        return;
    }
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::RestoreTrailingCollapsibleSpaceIfRemoved()
{
    if (InlineItem* last_item = LastItemToCollapseWith(items_)) {
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::RestoreTrailingCollapsibleSpace(InlineItem* item)
{
    DCHECK(item);
    DCHECK(item->EndCollapseType() == InlineItem::kCollapsed);

    mapping_builder_.RestoreTrailingCollapsibleSpace(To<LayoutText>(*item->GetLayoutObject()), item->EndOffset());
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::EnterBidiContext(LayoutObject* node, UChar enter, UChar exit)
{
...
template <typename MappingBuilder>
void InlineItemsBuilderTemplate<MappingBuilder>::EnterBidiContext(LayoutObject* node, const ComputedStyle* style, UChar ltr_enter, UChar rtl_enter, UChar exit)
{
    EnterBidiContext(node, IsLtr(style->Direction()) ? ltr_enter : rtl_enter, exit);
}

template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::EnterBlock(const ComputedStyle* style)
{
    ...
    if (style->RtlOrdering() == EOrder::kLogical) {
        ...
        case UnicodeBidi::kIsolateOverride:
            EnterBidiContext(nullptr, style, kLeftToRightOverrideCharacter, kRightToLeftOverrideCharacter, kPopDirectionalFormattingCharacter);
            break;
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::EnterInline(LayoutInline* node)
{
    DCHECK(node);
    ...
    if (style->RtlOrdering() == EOrder::kLogical) {
        ...
        case UnicodeBidi::kNormal:
            break;
        case UnicodeBidi::kEmbed:
            EnterBidiContext(node, style, kLeftToRightEmbedCharacter, kRightToLeftEmbedCharacter, kPopDirectionalFormattingCharacter);
            break;
        case UnicodeBidi::kBidiOverride:
            EnterBidiContext(node, style, kLeftToRightOverrideCharacter, kRightToLeftOverrideCharacter, kPopDirectionalFormattingCharacter);
            break;
        case UnicodeBidi::kIsolate:
            EnterBidiContext(node, style, kLeftToRightIsolateCharacter, kRightToLeftIsolateCharacter, kPopDirectionalIsolateCharacter);
            break;
        case UnicodeBidi::kPlaintext:
            has_unicode_bidi_plain_text_ = true;
            EnterBidiContext(node, kFirstStrongIsolateCharacter, kPopDirectionalIsolateCharacter);
            break;
        case UnicodeBidi::kIsolateOverride:
            EnterBidiContext(node, kFirstStrongIsolateCharacter, kPopDirectionalIsolateCharacter);
            EnterBidiContext(node, style, kLeftToRightOverrideCharacter, kRightToLeftOverrideCharacter, kPopDirectionalFormattingCharacter);
    ...
    if (node->IsInlineRubyText()) {
        ...
        if (!node->Parent()->IsInlineRuby()) {
            ...
            AppendOpaque(InlineItem::kOpenRubyColumn, IsLtr(style->Direction()) ? kLeftToRightIsolateCharacter : kRightToLeftIsolateCharacter, nullptr);
    ...
    if (NeedsBoxInfo()) {
        ...
        BoxInfo* current_box = &boxes_.emplace_back(items_->size() - 1, items_->back());
        if (boxes_.size() > 1) {
            BoxInfo* parent_box = std::prev(current_box);
            if (!parent_box->should_create_box_fragment && parent_box->ShouldCreateBoxFragmentForChild(*current_box)) {
    ...
    if (node->IsInlineRuby()) {
        AppendOpaque(InlineItem::kOpenRubyColumn, IsLtr(style->Direction()) ? kLeftToRightIsolateCharacter : kRightToLeftIsolateCharacter, node);
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::ExitBlock()
{
    Exit(nullptr);
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::ExitInline(LayoutObject* node)
{
    DCHECK(node);
    ...
    if (node->IsInlineRuby()) {
        ...
        if (size >= 3 && items_->at(size - 3).Type() == InlineItem::kCloseRubyColumn && items_->at(size - 2).Type() == InlineItem::kOpenRubyColumn
            ...
            AppendOpaque(InlineItem::kCloseRubyColumn, kPopDirectionalIsolateCharacter, node);
        }
    } else if (node->IsInlineRubyText()) {
        typename MappingBuilder::SourceNodeScope scope(&mapping_builder_, nullptr);
        AppendOpaque(InlineItem::kRubyLinePlaceholder, node);
    ...
    if (node->IsInlineRubyText()) {
        ...
        if (node->Parent()->IsInlineRuby()) {
            ...
            AppendOpaque(InlineItem::kCloseRubyColumn, kPopDirectionalIsolateCharacter, ruby_container);
            // This produces almost-empty ruby-columns if </ruby> follows.
            // The beginning part of this function removes such ruby-columns.
            AppendOpaque(InlineItem::kOpenRubyColumn, IsLtr(node->Parent()->Style()->Direction()) ? kLeftToRightIsolateCharacter : kRightToLeftIsolateCharacter,
                ruby_container);
            AppendOpaque(InlineItem::kRubyLinePlaceholder, node);
        } else {
            AppendOpaque(InlineItem::kCloseRubyColumn, kPopDirectionalIsolateCharacter, nullptr);
        }
    }
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::Exit(LayoutObject* node)
{
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::DidFinishCollectInlines(InlineNodeData* data)
{
    ...
    data->is_bidi_enabled_ = HasBidiControls() || (has_non_orc_16bit_ && Character::MaybeBidiRtl(data->text_content));
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::SetHasInititialLetterBox()
{
    DCHECK(!items_->empty());
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::SetIsSymbolMarker()
{
    DCHECK(!items_->empty());
    items_->back().SetIsSymbolMarker();
}

template <typename MappingBuilder> bool InlineItemsBuilderTemplate<MappingBuilder>::ShouldUpdateLayoutObject() const
{
    return true;
}
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::ClearInlineFragment(LayoutObject* object)
{
    object->SetIsInLayoutNGInlineFormattingContext(true);
}

template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::ClearNeedsLayout(LayoutObject* object)
{
...
template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::UpdateShouldCreateBoxFragment(LayoutInline* object)
{
    object->UpdateShouldCreateBoxFragment();
}

// |OffsetMappingBuilder| doesn't change states of |LayoutObject|
template <> bool InlineItemsBuilderTemplate<OffsetMappingBuilder>::ShouldUpdateLayoutObject() const
{
    return false;
}

// |OffsetMappingBuilder| doesn't change states of |LayoutObject|
template <> void InlineItemsBuilderTemplate<OffsetMappingBuilder>::ClearNeedsLayout(LayoutObject* object)
{
}

// |OffsetMappingBuilder| doesn't change states of |LayoutObject|
template <> void InlineItemsBuilderTemplate<OffsetMappingBuilder>::ClearInlineFragment(LayoutObject*)
{
}

// |OffsetMappingBuilder| doesn't change states of |LayoutInline|
template <> void InlineItemsBuilderTemplate<OffsetMappingBuilder>::UpdateShouldCreateBoxFragment(LayoutInline*)
{
}

template <typename MappingBuilder> void InlineItemsBuilderTemplate<MappingBuilder>::BidiContext::Trace(Visitor* visitor) const
{
    visitor->Trace(node);
}

template class CORE_TEMPLATE_EXPORT InlineItemsBuilderTemplate<EmptyOffsetMappingBuilder>;
template class CORE_TEMPLATE_EXPORT InlineItemsBuilderTemplate<OffsetMappingBuilder>;
