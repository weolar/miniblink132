...
public:
    ...
    const FragmentItem* Item() const
    {
        return item_;
    }
    const FragmentItem* operator->() const
    {
        return item_;
    }
    const FragmentItem& operator*() const
    ...
    void RecalcInkOverflow(const InlineCursor& cursor, InlinePaintContext* inline_context) const;
    ...
    {
        return TextOffset().start;
    }
    wtf_size_t TextEndOffset() const
    {
        return TextOffset().end;
    }
...
public:
    ...
    InlineCursor(const PhysicalBoxFragment& box_fragment, const FragmentItems& items);
    ...
    wtf_size_t ContainerFragmentIndex() const
    {
    ...
    //
    const InlineCursorPosition& Current() const
    {
        return current_;
    }
    ...
    const FragmentItem* CurrentItem() const
    {
        return Current().Item();
    }
    ...
    PhysicalRect CurrentLocalRect(unsigned start_offset, unsigned end_offset) const;
    PhysicalRect CurrentLocalSelectionRectForText(const LayoutSelectionStatus& selection_status) const;
    ...
    {
        DCHECK_EQ(Current().OffsetInContainerFragment(), Current().RectInContainerFragment().offset);
    ...
    PositionWithAffinity PositionForPointInInlineFormattingContext(const PhysicalOffset& point, const PhysicalBoxFragment& container);
    ...
    // direction.
    PositionWithAffinity PositionForPointInInlineBox(const PhysicalOffset& point) const;
    ...
    PositionWithAffinity PositionForPointInChild(const PhysicalOffset& point_in_container) const;
    ...
    bool IsBlockFragmented() const
    {
        return max_fragment_index_ > 0;
    }
...
private:
    InlineCursor(const PhysicalBoxFragment& box_fragment, const FragmentItems& fragment_items, ItemsSpan items);
    ...
    void SetRoot(const PhysicalBoxFragment& box_fragment, const FragmentItems& items);
    void SetRoot(const PhysicalBoxFragment& box_fragment, const FragmentItems& fragment_items, ItemsSpan items);
    ...
    public:
        ...
        const LayoutInline* GetLayoutInline() const
        {
            return layout_inline_;
        }
        ...
        bool UseFragmentTree() const
        {
            return use_fragment_tree_;
        }
