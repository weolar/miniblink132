...
LogicalRect ExpandedSelectionRectForSoftLineBreakIfNeeded(const LogicalRect& rect, const InlineCursor& cursor, const LayoutSelectionStatus& selection_status)
{
    ...
    return { rect.offset, { rect.size.inline_size + space_width, rect.size.block_size } };
}

// Expands selection height so that the selection rect fills entire line.
LogicalRect ExpandSelectionRectToLineHeight(const LogicalRect& rect, const LogicalRect& line_logical_rect)
{
    // Unite the rect only in the block direction.
    const LayoutUnit selection_top = std::min(rect.offset.block_offset, line_logical_rect.offset.block_offset);
    const LayoutUnit selection_bottom = std::max(rect.BlockEndOffset(), line_logical_rect.BlockEndOffset());
    return { { rect.offset.inline_offset, selection_top }, { rect.size.inline_size, selection_bottom - selection_top } };
}

LogicalRect ExpandSelectionRectToLineHeight(const LogicalRect& rect, const InlineCursor& cursor)
{
    InlineCursor line(cursor);
    line.MoveToContainingLine();
    const PhysicalRect line_physical_rect(line.Current().OffsetInContainerFragment() - cursor.Current().OffsetInContainerFragment(), line.Current().Size());
    return ExpandSelectionRectToLineHeight(rect, cursor.Current().ConvertChildToLogical(line_physical_rect));
}
...
void InlineCursor::SetRoot(const PhysicalBoxFragment& box_fragment, const FragmentItems& fragment_items, ItemsSpan items)
{
...
void InlineCursor::SetRoot(const PhysicalBoxFragment& box_fragment, const FragmentItems& items)
{
...
bool InlineCursor::TrySetRootFragmentItems()
{
    ...
    for (; fragment_index_ <= max_fragment_index_; IncrementFragmentIndex()) {
        const PhysicalBoxFragment* fragment = root_block_flow_->GetPhysicalFragment(fragment_index_);
        DCHECK(fragment);
...
InlineCursor::InlineCursor(const PhysicalBoxFragment& box_fragment, const FragmentItems& fragment_items, ItemsSpan items)
{
    SetRoot(box_fragment, fragment_items, items);
}

InlineCursor::InlineCursor(const PhysicalBoxFragment& box_fragment, const FragmentItems& items)
{
...
const LayoutBlockFlow* InlineCursor::GetLayoutBlockFlow() const
{
    ...
    if (root_box_fragment_) {
        const LayoutObject* layout_object = root_box_fragment_->GetSelfOrContainerLayoutObject();
...
InlineCursor InlineCursor::CursorForDescendants() const
{
    if (current_.item_) {
        ...
        if (descendants_count > 1) {
            ...
            return InlineCursor(*root_box_fragment_, *fragment_items_, ItemsSpan(&*(current_.item_iter_ + 1), descendants_count - 1));
        }
        return InlineCursor();
    }
    NOTREACHED();
...
{
    if (fragment_items_) {
        const unsigned index_diff = base::checked_cast<unsigned>(items_.data() - fragment_items_->Items().data());
        DCHECK_LT(index_diff, fragment_items_->Items().size());
        const unsigned item_index = base::checked_cast<unsigned>(current_.item_iter_ - items_.begin());
...
bool InlineCursorPosition::IsPartOfCulledInlineBox(const LayoutInline& layout_inline) const
{
    ...
    for (const LayoutObject* parent = layout_object->Parent(); parent; parent = parent->Parent()) {
...
{
    if (item_) {
        return item_->Type() == FragmentItem::kLine || (item_->Type() == FragmentItem::kBox && !item_->IsAtomicInline());
    }
    NOTREACHED();
}
...
UBiDiLevel InlineCursorPosition::BidiLevel() const
{
    if (IsText()) {
        ...
        auto* const item
            = base::ranges::find_if(*items, [offset](const InlineItem& item) { return item.StartOffset() <= offset.start && item.EndOffset() >= offset.end; });
    ...
    if (IsAtomicInline()) {
        ...
        const LayoutBlockFlow& block_flow = *GetLayoutObject()->FragmentItemsContainer();
        const auto& items = block_flow.GetInlineNodeData()->ItemsData(UsesFirstLineStyle()).items;
        const auto item = base::ranges::find(items, GetLayoutObject(), &InlineItem::GetLayoutObject);
        CHECK(item != items.end(), base::NotFatalUntil::M130) << this;
        return item->BidiLevel();
    }
...
const DisplayItemClient* InlineCursorPosition::GetSelectionDisplayItemClient() const
{
...
gfx::RectF InlineCursorPosition::ObjectBoundingBox(const InlineCursor& cursor) const
{
    return item_->ObjectBoundingBox(cursor.Items());
}

void InlineCursorPosition::RecalcInkOverflow(const InlineCursor& cursor, InlinePaintContext* inline_context) const
{
    DCHECK(item_);
    DCHECK_EQ(item_, cursor.Current().Item());
    PhysicalRect self_and_contents_rect;
    item_->GetMutableForPainting().RecalcInkOverflow(cursor, inline_context, &self_and_contents_rect);
}
...
PhysicalRect InlineCursor::CurrentLocalRect(unsigned start_offset, unsigned end_offset) const
{
    DCHECK(Current().IsText());
    if (current_.item_) {
        return current_.item_->LocalRect(current_.item_->Text(*fragment_items_), start_offset, end_offset);
    }
    NOTREACHED();
}

PhysicalRect InlineCursor::CurrentLocalSelectionRectForText(const LayoutSelectionStatus& selection_status) const
{
    const PhysicalRect selection_rect = CurrentLocalRect(selection_status.start, selection_status.end);
    ...
    if (selection_status.start != selection_status.end && Current().IsLineBreak() &&
        // This is for old compatible that old doesn't paint last br in a page.
        !IsLastBRInPage(*Current().GetLayoutObject())) {
        logical_rect.size.inline_size = LayoutUnit(Current().Style().GetFont().SpaceWidth());
    }
    const LogicalRect line_break_extended_rect
        = Current().IsLineBreak() ? logical_rect : ExpandedSelectionRectForSoftLineBreakIfNeeded(logical_rect, *this, selection_status);
    const LogicalRect line_height_expanded_rect = ExpandSelectionRectToLineHeight(line_break_extended_rect, *this);
    const PhysicalRect physical_rect = Current().ConvertChildToPhysical(line_height_expanded_rect);
...
PhysicalRect InlineCursor::CurrentLocalSelectionRectForReplaced() const
{
    ...
    LogicalRect logical_rect = Current().ConvertChildToLogical(selection_rect);
    const LogicalRect line_height_expanded_rect = ExpandSelectionRectToLineHeight(logical_rect, *this);
    const PhysicalRect physical_rect = Current().ConvertChildToPhysical(line_height_expanded_rect);
    return physical_rect;
}
...
PhysicalRect InlineCursor::CurrentRectInBlockFlow() const
{
    ...
    case WritingMode::kVerticalRl: {
        ...
        const LayoutBlock* containing_block = Current().GetLayoutObject()->ContainingBlock();
        DCHECK_EQ(containing_block->StyleRef().GetWritingDirection(), ContainerFragment().Style().GetWritingDirection());
        LogicalOffset logical_offset = rect.offset.ConvertToLogical(writing_direction, ContainerFragment().Size(), rect.size);
        LogicalOffset logical_offset_in_flow_thread(logical_offset.inline_offset, logical_offset.block_offset + previously_consumed_block_size_);
        rect.offset = logical_offset_in_flow_thread.ConvertToPhysical(writing_direction, PhysicalSize(containing_block->Size()), rect.size);
        break;
    }
    };
    return rect;
...
LayoutUnit InlineCursor::CaretInlinePositionForOffset(unsigned offset) const
{
    DCHECK(Current().IsText());
    if (current_.item_) {
        return current_.item_->CaretInlinePositionForOffset(current_.item_->Text(*fragment_items_), offset);
    }
    NOTREACHED();
}

LogicalRect InlineCursorPosition::ConvertChildToLogical(const PhysicalRect& physical_rect) const
{
    return WritingModeConverter({ Style().GetWritingMode(), ResolvedOrBaseDirection() }, Size()).ToLogical(physical_rect);
}

PhysicalRect InlineCursorPosition::ConvertChildToPhysical(const LogicalRect& logical_rect) const
{
    return WritingModeConverter({ Style().GetWritingMode(), ResolvedOrBaseDirection() }, Size()).ToPhysical(logical_rect);
}

PositionWithAffinity InlineCursor::PositionForPointInInlineFormattingContext(const PhysicalOffset& point, const PhysicalBoxFragment& container)
{
    DCHECK(HasRoot());
    const auto writing_direction = container.Style().GetWritingDirection();
    const PhysicalSize& container_size = container.Size();
    const LayoutUnit point_block_offset = point
    ...
    while (*this) {
        ...
        if (child_item->Type() == FragmentItem::kLine) {
            ...
            const LayoutUnit child_block_offset
                = child_item->OffsetInContainerFragment().ConvertToLogical(writing_direction, container_size, child_item->Size()).block_offset;
            ...
            const LayoutUnit child_block_end_offset = child_block_offset + child_item->Size().ConvertToLogical(writing_direction.GetWritingMode()).block_size;
            ...
            if (const PositionWithAffinity child_position = PositionForPointInInlineBox(point))
    ...
    if (closest_line_before) {
        ...
        const bool move_caret_to_boundary = To<LayoutBlockFlow>(Current().GetLayoutObject())->ShouldMoveCaretToHorizontalBoundaryWhenPastTopOrBottom();
        ...
        } else if (const PositionWithAffinity child_position = PositionForPointInInlineBox(point))
            return child_position;
    }
...
PositionWithAffinity InlineCursor::PositionForPointInInlineBox(const PhysicalOffset& point_in) const
{
    const FragmentItem* container = CurrentItem();
    DCHECK(container);
    DCHECK(container->Type() == FragmentItem::kLine || container->Type() == FragmentItem::kBox);
    const auto* const text_combine = DynamicTo<LayoutTextCombine>(container->GetLayoutObject());
    ...
    const auto writing_direction = container->Style().GetWritingDirection();
    const PhysicalSize& container_size = container->Size();
    const LayoutUnit point_inline_offset = point
                                               .ConvertToLogical(writing_direction, container_size,
                                                   // |point| is actually a pixel with size 1x1.
                                                   PhysicalSize(LayoutUnit(1), LayoutUnit(1)))
                                               .inline_offset;
    ...
    for (; descendants; descendants.MoveToNext()) {
        ...
        const LayoutUnit child_inline_offset
            = child_item->OffsetInContainerFragment().ConvertToLogical(writing_direction, container_size, child_item->Size()).inline_offset;
        ...
        const LayoutUnit child_inline_end_offset = child_inline_offset + child_item->Size().ConvertToLogical(writing_direction.GetWritingMode()).inline_size;
        ...
        if (const PositionWithAffinity child_position = descendants.PositionForPointInChild(point))
            return child_position;
    }
    ...
    if (closest_child_after) {
        ...
        if (closest_child_after->BoxFragment()) {
            ...
            if (const PositionWithAffinity child_position = descendants.PositionForPointInInlineBox(point))
                return child_position;
        }
    }
...
PositionWithAffinity InlineCursor::PositionForPointInChild(const PhysicalOffset& point_in_container) const
{
    DCHECK(CurrentItem());
    const FragmentItem& child_item = *CurrentItem();
    switch (child_item.Type()) {
    case FragmentItem::kText:
        return child_item.PositionForPointInText(point_in_container - child_item.OffsetInContainerFragment(), *this);
    case FragmentItem::kGeneratedText:
        break;
    case FragmentItem::kBox:
        if (const PhysicalBoxFragment* box_fragment = child_item.BoxFragment()) {
            if (!box_fragment->IsInlineBox()) {
                ...
                return child_item.GetLayoutObject()->PositionForPoint(point_in_container - child_item.OffsetInContainerFragment());
            }
        } else {
...
PositionWithAffinity InlineCursor::PositionForPointInText(unsigned text_offset) const
{
...
PositionWithAffinity InlineCursor::PositionForStartOfLine() const
{
    ...
    if (!first_leaf)
        return PositionWithAffinity();
    const auto& layout_object = first_leaf.Current()->IsBlockInInline() ? first_leaf.Current()->BlockInInline() : *first_leaf.Current().GetLayoutObject();
    ...
    const unsigned text_offset = Current().BaseDirection() == first_leaf.Current().ResolvedDirection() ? first_leaf.Current().TextOffset().start
...
PositionWithAffinity InlineCursor::PositionForEndOfLine() const
{
    ...
    if (!last_leaf)
        return PositionWithAffinity();
    const auto& layout_object = last_leaf.Current()->IsBlockInInline() ? last_leaf.Current()->BlockInInline() : *last_leaf.Current().GetLayoutObject();
    Node* const node = layout_object.NonPseudoNode();
    if (!node) {
...
inline wtf_size_t InlineCursor::GetTextOffsetForEndOfLine(InlineCursor& last_leaf) const
{
    wtf_size_t text_offset = last_leaf.Current().TextOffset().start;
    if (Current().BaseDirection() == last_leaf.Current().ResolvedDirection() && !last_leaf.Current().IsLineBreak()) {
...
inline wtf_size_t InlineCursor::SpanBeginItemIndex() const
{
    ...
    DCHECK(!items_.empty());
    DCHECK(fragment_items_->IsSubSpan(items_));
    const wtf_size_t delta = base::checked_cast<wtf_size_t>(items_.data() - fragment_items_->Items().data());
...
inline wtf_size_t InlineCursor::SpanIndexFromItemIndex(unsigned index) const
{
    ...
    if (items_.data() == fragment_items_->Items().data())
        return index;
    const wtf_size_t span_index = base::checked_cast<wtf_size_t>(fragment_items_->Items().data() - items_.data() + index);
...
{
    if (HasRoot()) {
        auto iter = base::ranges::find(items_, FragmentItem::kLine, &FragmentItem::Type);
        if (iter != items_.end()) {
            MoveToItem(iter);
            return;
        }
...
{
    DCHECK(HasRoot());
    auto iter = base::ranges::find(base::Reversed(items_), FragmentItem::kLine, &FragmentItem::Type);
    if (iter != items_.rend())
...
void InlineCursor::MoveToNextInlineLeafOnLine()
{
    ...
    DCHECK(!cursor.Current() || cursor.Current().IsInlineLeaf())
        << "Must return an empty or inline leaf position, returned: " << cursor.CurrentMutableLayoutObject();
}
...
void InlineCursor::SlowMoveToNextForSameLayoutObject(const LayoutObject& layout_object)
{
    MoveToNextIncludingFragmentainer();
    SlowMoveToForIfNeeded(layout_object);
}
...
void InlineCursor::MoveTo(const LayoutObject& layout_object)
{
    ...
    if (root_block_flow_) {
        ...
        check_cursor.SlowMoveToFirstFor(layout_object);
        DCHECK_EQ(check_cursor.Current().Item(), &fragment_items_->Items()[item_index]);
#endif
    } else {
        ...
        if (fragment_items_->HasItemIndex(item_index)) {
            ...
            InlineCursor cursor;
            for (cursor.MoveTo(layout_object);; cursor.MoveToNextForSameLayoutObject()) {
                if (!cursor || cursor.fragment_items_->SizeOfEarlierFragments() > fragment_items_->SizeOfEarlierFragments()) {
                    MakeNull();
                    return;
                }
                if (cursor.fragment_items_ == fragment_items_) {
                    DCHECK_GE(cursor.Current().Item(), fragment_items_->Items().data());
                    item_index = base::checked_cast<wtf_size_t>(cursor.Current().Item() - fragment_items_->Items().data());
                    break;
                }
            }
        }
...
void InlineCursor::MoveToNextForSameLayoutObjectExceptCulledInline()
{
    ...
    if (wtf_size_t delta = current_.item_->DeltaToNextForSameLayoutObject()) {
        while (true) {
            ...
            const wtf_size_t delta_to_end = base::checked_cast<wtf_size_t>(items_.end() - current_.item_iter_);
...
const LayoutObject* InlineCursor::CulledInlineTraversal::Find(const LayoutObject* child) const
{
    while (child) {
        if (child->IsText())
            return child;
...
void InlineCursor::CulledInlineTraversal::SetUseFragmentTree(const LayoutInline& layout_inline)
{
    layout_inline_ = &layout_inline;
    use_fragment_tree_ = true;
}

const LayoutObject* InlineCursor::CulledInlineTraversal::MoveToFirstFor(const LayoutInline& layout_inline)
{
    layout_inline_ = &layout_inline;
...
const LayoutObject* InlineCursor::CulledInlineTraversal::MoveToNext()
{
    if (!current_object_)
        return nullptr;
    current_object_ = Find(current_object_->NextInPreOrderAfterChildren(layout_inline_));
    return current_object_;
}

void InlineCursor::MoveToFirstForCulledInline(const LayoutInline& layout_inline)
{
    ...
    if (const LayoutObject* layout_object = culled_inline_.MoveToFirstFor(layout_inline)) {
...
void InlineCursor::DecrementFragmentIndex()
{
    ...
    const auto& root_box_fragment = *root_block_flow_->GetPhysicalFragment(fragment_index_ - 1);
...
void InlineCursor::IncrementFragmentIndex()
{
    ...
    if (!root_box_fragment_)
        return;
    if (const BlockBreakToken* break_token = root_box_fragment_->GetBreakToken()) {
        previously_consumed_block_size_ = break_token->ConsumedBlockSize();
    }
}

void InlineCursor::MoveToIncludingCulledInline(const LayoutObject& layout_object)
{
...
void InlineCursor::MoveToVisualFirstOrLastForCulledInline(bool last)
{
    ...
    for (; Current(); MoveToNextForSameLayoutObject()) {
        ...
        if (!found_index || (last && index > *found_index) || (!last && index < *found_index)) {
            ...
            if ((last && index == fragment_items_->Size() - 1) || (!last && index == 0))
                break;
        }
    }
    ...
    : cursor_(cursor)
{
    if (cursor.HasRoot()) {
        ...
        for (InlineCursor sibling(cursor); sibling; sibling.MoveToNextSkippingChildren()) {
...
void InlineCursor::CheckValid(const InlineCursorPosition& position) const
{
    if (position.Item()) {
        ...
        const unsigned index = base::checked_cast<unsigned>(position.item_iter_ - items_.begin());
