...
class CORE_EXPORT InlineLayoutAlgorithm final : public LayoutAlgorithm<InlineNode, LineBoxFragmentBuilder, InlineBreakToken> {
public:
    InlineLayoutAlgorithm(InlineNode, const ConstraintSpace&, const InlineBreakToken*, const ColumnSpannerPath*, InlineChildLayoutContext* context);
    ~InlineLayoutAlgorithm();

    void CreateLine(const LineLayoutOpportunity&, LineInfo*, LogicalLineContainer* line_container);
...
#endif
    void PlaceBlockInInline(const InlineItem&, InlineItemResult*, LogicalLineItems* line_box);

private:
    friend class LineWidthsTest;

    void PositionLeadingFloats(ExclusionSpace&, LeadingFloats&);
    PositionedFloat PositionFloat(LayoutUnit origin_block_bfc_offset, LayoutObject* floating_object, ExclusionSpace*);

    void PrepareBoxStates(const LineInfo&, const InlineBreakToken*);

    void PlaceOutOfFlowObjects(const LineInfo&, const FontHeight&, LogicalLineItems* line_box);
    void PlaceFloatingObjects(const FontHeight&, const LineLayoutOpportunity&, LayoutUnit ruby_block_start_adjust, LineInfo*, LogicalLineItems* line_box);
    ...
    LineClampState GetLineClampState(const LineInfo*, LayoutUnit line_box_height) const;
