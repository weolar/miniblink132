...
inline bool MaybeIdeograph(UScriptCode script, StringView text)
{
    ...
    return std::any_of(text.Characters16(), text.Characters16() + text.length(),
        [](const UChar ch) { return ch >= TextAutoSpace::kNonHanIdeographMin && ch <= TextAutoSpace::kNonHanIdeographMax; });
}
...
public:
    void SetSpacing(const Vector<wtf_size_t, 16>& offsets, const InlineItem* current_item, const ComputedStyle& style)
    {
        ...
        if (!offsets.empty() && *offset == current_item->StartOffset()) {
            ...
            if (last_item_->Direction() == TextDirection::kLtr) {
                ...
                offsets_with_spacing_.emplace_back(OffsetWithSpacing({ .offset = *offset, .spacing = spacing }));
                ++offset;
            } else {
...
void InlineTextAutoSpace::Initialize(const InlineItemsData& data)
{
    ...
    if (!data.segments) {
        ...
        if (std::none_of(ranges_.begin(), ranges_.end(), [&text](const RunSegmenter::RunSegmenterRange& range) {
                return MaybeIdeograph(range.script, StringView(text, range.start, range.end - range.start));
            })) {
            ranges_.clear();
            return;
        }
...
void InlineTextAutoSpace::Apply(InlineItemsData& data, Vector<wtf_size_t>* offsets_out)
{
    const String& text = data.text_content;
    DCHECK(!text.Is8Bit());
    ...
    for (const InlineItem& item : data.items) {
        ...
        if (style->GetFontDescription().Orientation() == FontOrientation::kVerticalUpright) [[unlikely]] {
            applier.SetSpacing(offsets, &item, *style);
        ...
        do {
            ...
            if (!MaybeIdeograph(range->script, StringView(text, offset, end_offset - offset))) {
                if (last_type == kIdeograph) {
                    ...
                    } else if (last_direction == TextDirection::kLtr && item.Direction() == TextDirection::kRtl) [[unlikely]] {
            ...
            while (offset < end_offset) {
                const wtf_size_t saved_offset = offset;
                const CharType type = GetTypeAndNext(text, offset);
                if (((type == kIdeograph && last_type == kLetterOrNumeral) || (last_type == kIdeograph && type == kLetterOrNumeral))) {
                    if (last_direction == item.Direction()) {
                        offsets.push_back(saved_offset);
                    } else if (last_direction == TextDirection::kRtl && item.Direction() == TextDirection::kLtr) [[unlikely]] {
