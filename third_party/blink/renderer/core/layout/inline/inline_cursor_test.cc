...
protected:
    InlineCursor SetupCursor(const String& html)
    {
        ...
        const LayoutBlockFlow& block_flow = *To<LayoutBlockFlow>(GetLayoutObjectByElementId("root"));
    ...
    {
        Vector<String> list;
        for (InlineCursor cursor(start); cursor; cursor.MoveToNextSkippingChildren()) {
            list.push_back(ToDebugString(cursor));
        }
        return list;
    }
    ...
    void TestPrevoiusSibling(const InlineCursor& start)
    {
        ...
        for (InlineBackwardCursor cursor(start); cursor; cursor.MoveToPreviousSibling()) {
...
TEST_F(InlineCursorTest, BidiLevelInlineBoxLTR)
{
    ...
    InlineCursor cursor = SetupCursor("<div id=root dir=ltr>"
                                      "abc<b id=def>def</b><bdo dir=rtl><b id=ghi>GHI</b></bdo>jkl</div>");
    Vector<String> list = ToDebugStringListWithBidiLevel(cursor);
    EXPECT_THAT(list, ElementsAre("#linebox", "abc:0", "#def:0", "#ghi:3", "jkl:0"));
}

TEST_F(InlineCursorTest, BidiLevelInlineBoxRTL)
{
    InsertStyleElement("b { display: inline-block; }");
    InlineCursor cursor = SetupCursor("<div id=root dir=rtl>"
                                      "abc<b id=def>def</b><bdo dir=rtl><b id=ghi>GHI</b></bdo>jkl</div>");
    Vector<String> list = ToDebugStringListWithBidiLevel(cursor);
    EXPECT_THAT(list, ElementsAre("#linebox", "abc:2", "#def:2", "#ghi:3", "jkl:2"));
}
...
TEST_F(InlineCursorTest, BidiLevelSimpleLTR)
{
    ...
    Vector<String> list = ToDebugStringListWithBidiLevel(cursor);
    EXPECT_THAT(list, ElementsAre("#linebox", "DEF:3", "abc:6", "GHI:3", ":0", "#linebox", "123, jkl:0", "MNO:3"));
}
...
TEST_F(InlineCursorTest, BidiLevelSimpleRTL)
{
    InlineCursor cursor = SetupCursor("<div id=root dir=rtl>"
                                      ...
                                      "123, jkl <bdo dir=rtl>MNO</bdo></div>");
    Vector<String> list = ToDebugStringListWithBidiLevel(cursor);
    EXPECT_THAT(list, ElementsAre("#linebox", ":0", "DEF:3", "abc:6", "GHI:3", "#linebox", "MNO:3", ":1", "jkl:2", ",:1", "123:2"));
}
...
TEST_F(InlineCursorTest, ContainingLine)
{
    ...
    InlineCursor cursor = SetupCursor("<div id=root>abc<a id=target>def</a>ghi<br>xyz</div>");
...
TEST_F(InlineCursorTest, CulledInlineWithAtomicInline)
{
    ...
    EXPECT_THAT(LayoutObjectToDebugStringList(cursor), ElementsAre("abc", "ABC", "", "XYZ", "xyz"));
}
...
TEST_F(InlineCursorTest, CulledInlineBlockChild)
{
    ...
    InlineCursor cursor;
    cursor.MoveToIncludingCulledInline(*GetLayoutObjectByElementId("culled"));
    EXPECT_THAT(LayoutObjectToDebugStringList(cursor), ElementsAre("#culled", "#culled", "#culled"));
}
...
{
    LoadAhem();
    InsertStyleElement("div { font: 10px/15px Ahem; column-count: 2; width: 20ch; }");
...
TEST_F(InlineCursorTest, FirstChild)
{
    // TDOO(yosin): Remove <style> once FragmentItem don't do culled inline.
    InsertStyleElement("a, b { background: gray; }");
    InlineCursor cursor = SetupCursor("<div id=root>abc<a>DEF<b>GHI</b></a>xyz</div>");
...
TEST_F(InlineCursorTest, FirstChild2)
{
    // TDOO(yosin): Remove <style> once FragmentItem don't do culled inline.
    InsertStyleElement("a, b { background: gray; }");
    InlineCursor cursor = SetupCursor("<div id=root><b id=first>abc</b><a>DEF<b>GHI</b></a><a "
                                      "id=last>xyz</a></div>");
    cursor.MoveToFirstChild();
...
TEST_F(InlineCursorTest, FirstLastLogicalLeafInSimpleText)
{
    ...
    InlineCursor cursor = SetupCursor("<div id=root><b>first</b><b>middle</b><b>last</b></div>");
...
TEST_F(InlineCursorTest, FirstLastLogicalLeafInRtlText)
{
    // TDOO(yosin): Remove <style> once FragmentItem don't do culled inline.
    InsertStyleElement("b { background: gray; }");
    InlineCursor cursor = SetupCursor("<bdo id=root dir=rtl style=display:block>"
...
TEST_F(InlineCursorTest, FirstLastLogicalLeafInTextAsDeepDescendants)
{
    // TDOO(yosin): Remove <style> once FragmentItem don't do culled inline.
    InsertStyleElement("b { background: gray; }");
    InlineCursor cursor = SetupCursor("<div id=root>"
                                      "<b><b>first</b>ABC</b>"
                                      "<b>middle</b>"
                                      "<b>DEF<b>last</b></b>"
                                      "</div>");
...
{
    SetBodyContent("<textarea rows=\"3\" cols=\"50\">foo&#10;&#10;bar</textarea>");
    const auto& textarea = ToTextControl(*GetDocument().QuerySelector(AtomicString("textarea")));
    const LayoutObject* textarea_layout = textarea.InnerEditorElement()->GetLayoutObject();
    ...
    const PositionWithAffinity end_position = move_to_end_of_line.PositionForEndOfLine();
    EXPECT_EQ(4, end_position.GetPosition().OffsetInContainerNode());
}

TEST_F(InlineCursorTest, MoveToEndOfLineWithNoCharsRtl)
{
    SetBodyContent("<textarea rows=\"3\" cols=\"50\" "
                   "dir=\"rtl\">foo&#10;&#10;bar</textarea>");
    const auto& textarea = ToTextControl(*GetDocument().QuerySelector(AtomicString("textarea")));
    const LayoutObject* textarea_layout = textarea.InnerEditorElement()->GetLayoutObject();
    const LayoutBlockFlow& block_flow = *To<LayoutBlockFlow>(textarea_layout);
...
TEST_F(InlineCursorTest, FirstLastLogicalLeafWithInlineBlock)
{
    ...
    InlineCursor first_logical_leaf(cursor);
    first_logical_leaf.MoveToFirstLogicalLeaf();
    EXPECT_EQ("#first", ToDebugString(first_logical_leaf)) << "stop at inline-block";

    InlineCursor last_logical_leaf(cursor);
    last_logical_leaf.MoveToLastLogicalLeaf();
    EXPECT_EQ("#last", ToDebugString(last_logical_leaf)) << "stop at inline-block";
}

TEST_F(InlineCursorTest, FirstLastLogicalLeafWithImages)
{
    InlineCursor cursor = SetupCursor("<div id=root><img id=first>middle<img id=last></div>");
...
TEST_F(InlineCursorTest, FirstNonPseudoLeafWithBlockImage)
{
    ...
    //   +--LayoutText #text ""
    const auto& target = *To<LayoutImage>(GetElementById("target")->GetLayoutObject());
...
TEST_F(InlineCursorTest, IsEmptyLineBox)
{
    ...
    EXPECT_FALSE(cursor.Current().IsEmptyLineBox()) << "'abc\\n' is in non-empty line box.";
    cursor.MoveToNextLine();
    EXPECT_TRUE(cursor.Current().IsEmptyLineBox()) << "<b></b> with margin produces empty line box.";
}
...
TEST_F(InlineCursorTest, LastChild2)
{
    // TDOO(yosin): Remove <style> once FragmentItem don't do culled inline.
    InsertStyleElement("a, b { background: gray; }");
    InlineCursor cursor = SetupCursor("<div id=root><b id=first>abc</b><a>DEF<b>GHI</b></a>"
                                      "<a id=last>xyz</a></div>");
    cursor.MoveToLastChild();
...
TEST_F(InlineCursorTest, Next)
{
    ...
    LayoutBlockFlow* block_flow = To<LayoutBlockFlow>(GetLayoutObjectByElementId("root"));
    InlineCursor cursor(*block_flow);
    Vector<String> list = ToDebugStringList(cursor);
    EXPECT_THAT(list, ElementsAre("#linebox", "text1", "#span1", "text2", "#span2", "text3", "text4", "text5"));
}
...
TEST_F(InlineCursorTest, NextIncludingFragmentainer)
{
    ...
    EXPECT_THAT(list, ElementsAre("abc", "", "#linebox", "def", "", "#linebox", "LayoutInline B", "ghi", "", "#linebox", "jkl"));
}

TEST_F(InlineCursorTest, NextWithEllipsis)
{
    LoadAhem();
    InsertStyleElement("#root {"
                       "font: 10px/10px Ahem;"
                       "width: 5ch;"
                       "overflow-x: hidden;"
...
TEST_F(InlineCursorTest, NextWithListItem)
{
    ...
    Vector<String> list = ToDebugStringList(cursor);
    EXPECT_THAT(list, ElementsAre("LayoutOutsideListMarker (inline, children-inline) ::marker", "#linebox", "abc"));
...
TEST_F(InlineCursorTest, NextWithSoftHyphens)
{
    ...
    Vector<String> list = ToDebugStringList(cursor);
    EXPECT_THAT(list, ElementsAre("#linebox", u"abc\u00AD", u"#'\u2010'", "#linebox", "def"));
}
...
TEST_F(InlineCursorTest, NextInlineLeafOnLineFromLayoutInline)
{
    ...
    EXPECT_THAT(list, ElementsAre("#start", "def", "")) << "we don't have 'abc' and items in second line.";
}
...
TEST_F(InlineCursorTest, NextInlineLeafOnLineFromNestedLayoutInline)
{
    ...
    InlineCursor cursor = SetupCursor("<div id=root>"
                                      ...
                                      "<span>next</span>"
                                      "</div>");
    cursor.MoveToIncludingCulledInline(*GetElementById("start")->GetLayoutObject());
    Vector<String> list;
    while (cursor) {
        list.push_back(ToDebugString(cursor));
        cursor.MoveToNextInlineLeafOnLine();
    }
    EXPECT_THAT(list, ElementsAre("#start", "next")) << "next on line doesn't return descendant.";
}
...
TEST_F(InlineCursorTest, NextInlineLeafOnLineFromLayoutText)
{
    ...
    EXPECT_THAT(list, ElementsAre("abc", "def", "")) << "We don't have items from second line.";
}
...
{
    InlineCursor cursor = SetupCursor("<div id=root style='width:3ch'>abc&shy;def</div>");
    Vector<String> list;
    while (cursor) {
        list.push_back(ToDebugString(cursor));
        cursor.MoveToNextInlineLeaf();
...
{
    InsertStyleElement("b { display: inline-block; }");
    InlineCursor cursor = SetupCursor("<div id=root>abc<b id=ib>def</b>xyz</div>");
    Vector<String> list = ToDebugStringList(cursor);
...
TEST_F(InlineCursorTest, NextForSameLayoutObject)
{
    ...
    EXPECT_THAT(LayoutObjectToDebugStringList(cursor), ElementsAre("abc", "", "def", "", "ghi"));
}
...
TEST_F(InlineCursorTest, NextForSameLayoutObjectWithRange)
{
    ...
    LayoutBlockFlow* root = To<LayoutBlockFlow>(GetLayoutObjectByElementId("root"));
...
TEST_F(InlineCursorTest, Sibling2)
{
    // TDOO(yosin): Remove <style> once FragmentItem don't do culled inline.
    InsertStyleElement("a, b { background: gray; }");
    InlineCursor cursor = SetupCursor("<div id=root><a>abc<b>def</b>xyz</a></div>");
...
TEST_F(InlineCursorTest, PositionForPointInChildHorizontalLTR)
{
    ...
    EXPECT_EQ(PhysicalRect(PhysicalOffset(10, 10), PhysicalSize(20, 20)), cursor.Current().RectInContainerFragment());

    cursor.MoveTo(*text.GetLayoutObject());
    EXPECT_EQ(PhysicalRect(PhysicalOffset(10, 15), PhysicalSize(20, 10)), cursor.Current().RectInContainerFragment());
    const PhysicalOffset left_top = cursor.Current().OffsetInContainerFragment();

    EXPECT_EQ(PositionWithAffinity(Position(text, 0)), cursor.PositionForPointInChild(left_top + PhysicalOffset(-5, 0)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 0)), cursor.PositionForPointInChild(left_top));
    EXPECT_EQ(PositionWithAffinity(Position(text, 0)), cursor.PositionForPointInChild(left_top + PhysicalOffset(5, 0)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 1)), cursor.PositionForPointInChild(left_top + PhysicalOffset(10, 0)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 1)), cursor.PositionForPointInChild(left_top + PhysicalOffset(15, 0)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 2), TextAffinity::kUpstream), cursor.PositionForPointInChild(left_top + PhysicalOffset(20, 0)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 2), TextAffinity::kUpstream), cursor.PositionForPointInChild(left_top + PhysicalOffset(25, 0)));
}
...
TEST_F(InlineCursorTest, PositionForPointInChildHorizontalRTL)
{
    ...
    const auto& text = *To<Text>(GetElementById("root")->firstChild()->firstChild());
    ASSERT_TRUE(cursor.Current().IsLineBox());
    EXPECT_EQ(PhysicalRect(PhysicalOffset(754, 10), PhysicalSize(20, 20)), cursor.Current().RectInContainerFragment());

    cursor.MoveTo(*text.GetLayoutObject());
    EXPECT_EQ(PhysicalRect(PhysicalOffset(754, 15), PhysicalSize(20, 10)), cursor.Current().RectInContainerFragment());
    const PhysicalOffset left_top = cursor.Current().OffsetInContainerFragment();

    EXPECT_EQ(PositionWithAffinity(Position(text, 2), TextAffinity::kUpstream), cursor.PositionForPointInChild(left_top + PhysicalOffset(-5, 0)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 2), TextAffinity::kUpstream), cursor.PositionForPointInChild(left_top));
    EXPECT_EQ(PositionWithAffinity(Position(text, 2), TextAffinity::kUpstream), cursor.PositionForPointInChild(left_top + PhysicalOffset(5, 0)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 1)), cursor.PositionForPointInChild(left_top + PhysicalOffset(10, 0)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 1)), cursor.PositionForPointInChild(left_top + PhysicalOffset(15, 0)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 0)), cursor.PositionForPointInChild(left_top + PhysicalOffset(20, 0)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 0)), cursor.PositionForPointInChild(left_top + PhysicalOffset(25, 0)));
}
...
TEST_F(InlineCursorTest, PositionForPointInChildVerticalLTR)
{
    ...
    cursor.MoveTo(*text.GetLayoutObject());
    EXPECT_EQ(PhysicalRect(PhysicalOffset(15, 10), PhysicalSize(10, 20)), cursor.Current().RectInContainerFragment());
    const PhysicalOffset left_top = cursor.Current().OffsetInContainerFragment();

    EXPECT_EQ(PositionWithAffinity(Position(text, 0)), cursor.PositionForPointInChild(left_top + PhysicalOffset(0, -5)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 0)), cursor.PositionForPointInChild(left_top));
    EXPECT_EQ(PositionWithAffinity(Position(text, 0)), cursor.PositionForPointInChild(left_top + PhysicalOffset(0, 5)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 1)), cursor.PositionForPointInChild(left_top + PhysicalOffset(0, 10)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 1)), cursor.PositionForPointInChild(left_top + PhysicalOffset(0, 15)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 2), TextAffinity::kUpstream), cursor.PositionForPointInChild(left_top + PhysicalOffset(0, 20)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 2), TextAffinity::kUpstream), cursor.PositionForPointInChild(left_top + PhysicalOffset(0, 25)));
}
...
TEST_F(InlineCursorTest, PositionForPointInChildVerticalRTL)
{
    ...
    EXPECT_EQ(PositionWithAffinity(Position(text, 1)), cursor.PositionForPointInChild(left_top + PhysicalOffset(0, 10)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 1)), cursor.PositionForPointInChild(left_top + PhysicalOffset(0, 15)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 0)), cursor.PositionForPointInChild(left_top + PhysicalOffset(0, 20)));
    EXPECT_EQ(PositionWithAffinity(Position(text, 0)), cursor.PositionForPointInChild(left_top + PhysicalOffset(0, 25)));
}
...
TEST_F(InlineCursorTest, PositionForPointInChildBlockChildren)
{
    ...
    InlineCursor cursor = SetupCursor("<div id=root>a<b id=target><div>x</div></b></div>");
    const Element& target = *GetElementById("target");
    cursor.MoveTo(*target.GetLayoutObject());
    EXPECT_EQ(PositionWithAffinity(Position::FirstPositionInNode(target)), cursor.PositionForPointInChild(PhysicalOffset()));
}
...
TEST_F(InlineCursorTest, Previous)
{
    ...
    EXPECT_THAT(list, ElementsAre("xyz", "#linebox", "", "DEF", "LayoutInline B", "abc", "#linebox"));
}
...
TEST_F(InlineCursorTest, PreviousIncludingFragmentainer)
{
    ...
    EXPECT_THAT(list, ElementsAre("jkl", "#linebox", "", "ghi", "LayoutInline B", "#linebox", "", "def", "#linebox", "", "abc", "#linebox"));
}
...
TEST_F(InlineCursorTest, PreviousInlineLeafOnLineFromLayoutInline)
{
    ...
    EXPECT_THAT(list, ElementsAre("#start", "", "ABC")) << "We don't have 'DEF' and items in first line.";
}
...
TEST_F(InlineCursorTest, PreviousInlineLeafOnLineFromNestedLayoutInline)
{
    ...
    EXPECT_THAT(list, ElementsAre("#start", "previous")) << "previous on line doesn't return descendant.";
}
...
TEST_F(InlineCursorTest, PreviousInlineLeafOnLineFromLayoutText)
{
    ...
    EXPECT_THAT(list, ElementsAre("DEF", "", "ABC")) << "We don't have items in first line.";
}
...
TEST_F(InlineCursorTest, CursorForDescendants)
{
    ...
    EXPECT_THAT(ToDebugStringList(cursor.CursorForDescendants()), ElementsAre("text2", "#span2", "text3", "text4"));
    cursor.MoveToNext();
    EXPECT_EQ(ToDebugString(cursor), "text2");
    EXPECT_THAT(ToDebugStringList(cursor.CursorForDescendants()), ElementsAre());
    cursor.MoveToNext();
    EXPECT_EQ(ToDebugString(cursor), "#span2");
    EXPECT_THAT(ToDebugStringList(cursor.CursorForDescendants()), ElementsAre("text3"));
}
...
TEST_F(InlineCursorBlockFragmentationTest, MoveToLayoutObject)
{
    ...
    // Enumerate all fragments for |LayoutText|.
    {
        InlineCursor cursor;
        cursor.MoveTo(*text1);
        EXPECT_THAT(LayoutObjectToDebugStringList(cursor), ElementsAre("1111", "22"));
    }
    {
        InlineCursor cursor;
        cursor.MoveTo(*text2);
        EXPECT_THAT(LayoutObjectToDebugStringList(cursor), ElementsAre("33", "4444"));
    }
