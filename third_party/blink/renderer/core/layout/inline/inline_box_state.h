...
public:
    ...
    // |alignment_type|.
    void ResetStyle(const ComputedStyle& style_ref, bool is_svg, const LayoutObject& layout_object);
    ...
    void ComputeTextMetrics(const ComputedStyle&, const Font& fontref, FontBaseline ifc_baseline);
    void EnsureTextMetrics(const ComputedStyle&, const Font& fontref, FontBaseline ifc_baseline);
    ...
    static void AdjustEdges(
        TextBoxEdge text_box_edge, const Font& font, FontBaseline baseline_type, bool should_apply_over, bool should_apply_under, FontHeight& metrics);
...
public:
    ...
    InlineBoxState& LineBoxState()
    {
        return stack_.front();
    }

    void SetIsEmptyLine(bool is_empty_line)
    {
        is_empty_line_ = is_empty_line;
    }

    // Initialize the box state stack for a new line.
    // @return The initial box state for the line.
    InlineBoxState* OnBeginPlaceItems(const InlineNode node, const ComputedStyle&, FontBaseline, bool line_height_quirk, LogicalLineItems* line_box);

    // Push a box state stack.
    InlineBoxState* OnOpenTag(const ConstraintSpace&, const InlineItem&, const InlineItemResult&, FontBaseline baseline_type, const LogicalLineItems&);
    // This variation adds a box placeholder to |line_box|.
    InlineBoxState* OnOpenTag(const ConstraintSpace&, const InlineItem&, const InlineItemResult&, FontBaseline baseline_type, LogicalLineItems* line_box);

    // Pop a box state stack.
    InlineBoxState* OnCloseTag(const ConstraintSpace& space, LogicalLineItems*, InlineBoxState*, FontBaseline);

    // Compute all the pending positioning at the end of a line.
    void OnEndPlaceItems(const ConstraintSpace& space, LogicalLineItems*, FontBaseline);
    ...
    void ClearRubyColumnList()
    {
        ruby_column_list_.Shrink(0);
    }

    bool HasBoxFragments() const
    {
        return !box_data_list_.empty();
    }
    ...
    std::optional<std::pair<LayoutUnit, LayoutUnit>> AnnotationBoxBlockAxisMargins() const;
    ...
    LayoutUnit ComputeInlinePositions(LogicalLineItems*, LayoutUnit position, bool ignore_box_margin_border_padding);
    ...
    void ApplyRelativePositioning(const ConstraintSpace&, LogicalLineItems*, const LogicalOffset* parent_offset);
    // Create box fragments. This function turns a flat list of children into
    // a box tree.
    void CreateBoxFragments(const ConstraintSpace&, LogicalLineItems*, bool is_opaque);
...
private:
    ...
    // end of a line.
    void EndBoxState(const ConstraintSpace&, InlineBoxState*, LogicalLineItems*, FontBaseline);

    void AddBoxFragmentPlaceholder(InlineBoxState*, LogicalLineItems*, FontBaseline);
    ...
    // |pending_descendants|.
    FontHeight MetricsForTopAndBottomAlign(const InlineBoxState&, const LogicalLineItems&) const;
...
public:
    ...
    struct BoxData {
        DISALLOW_NEW();
        BoxData(unsigned start, unsigned end, const InlineItem* item, LogicalSize size)
        ...
        const LayoutResult* CreateBoxFragment(const ConstraintSpace&, LogicalLineItems*, bool is_opaque = false);
        void Trace(Visitor* visitor) const;
    };
...
private:
    ...
    // this function.
    unsigned UpdateBoxDataFragmentRange(LogicalLineItems*, unsigned index, HeapVector<BoxData>* fragmented_boxes);
...
struct CORE_EXPORT LogicalRubyColumn : public GarbageCollected<LogicalRubyColumn> {
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::InlineLayoutStateStack::BoxData)
