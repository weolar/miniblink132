...
{
    const PhysicalBoxFragment* fragment = node.GetLayoutBox()->GetPhysicalFragment(0);
...
void TestLinesAreContiguous(const LineInfoList& line_info_list)
{
    for (wtf_size_t i = 1; i < line_info_list.Size(); ++i) {
        EXPECT_EQ(line_info_list[i].Start(), line_info_list[i - 1].GetBreakToken()->Start());
    }
}
...
public:
    void RunUntilSuspended(ScoreLineBreaker& breaker, ScoreLineBreakContext& context)
    {
        ...
        for (;;) {
            ...
            if (!context.IsActive() || !breaker.BreakToken() || line_info_list.IsEmpty()) {
                break;
            }
...
class BlockInInlineTest : public ScoreLineBreakerTest, public testing::WithParamInterface<int> { };
INSTANTIATE_TEST_SUITE_P(ScoreLineBreakerTest, BlockInInlineTest, testing::Range(0, 4));
...
TEST_P(BlockInInlineTest, BeforeAfter)
{
    ...
    <div id="target">
      <span>%s<div>
        ...
        has_before ? "Before 89 1234 6789 1234 6789 1234 6789 12" : "", has_after ? "After 789 1234 6789 1234 6789 1234 6789 12" : ""));
    const InlineNode node = GetInlineNodeByElementId("target");
    const LayoutUnit width = FragmentWidth(node);
    ConstraintSpace space = ConstraintSpaceForAvailableSize(width);
    LineWidths line_widths(width);
...
class DisabledByLineBreakerTest : public ScoreLineBreakerTest, public testing::WithParamInterface<DisabledByLineBreakerData> { };

INSTANTIATE_TEST_SUITE_P(ScoreLineBreakerTest, DisabledByLineBreakerTest, testing::ValuesIn(disabled_by_line_breaker_data));
