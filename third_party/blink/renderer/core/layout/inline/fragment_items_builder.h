...
public:
    ...
    FragmentItemsBuilder(const InlineNode& node, WritingDirectionMode writing_direction, bool is_block_fragmented);
    ...
    const LogicalLineItems& GetLogicalLineItems(const PhysicalLineBoxFragment&) const;
    void AssociateLogicalLineContainer(LogicalLineContainer* line_container, const PhysicalFragment& line_fragment);
    void AddLine(const PhysicalLineBoxFragment& line, const LogicalOffset& offset);

    // Add a list marker to the current line.
    void AddListMarker(const PhysicalBoxFragment& marker_fragment, const LogicalOffset& offset);
    ...
    AddPreviousItemsResult AddPreviousItems(const PhysicalBoxFragment& container, const FragmentItems& items, BoxFragmentBuilder* container_builder = nullptr,
        const FragmentItem* end_item = nullptr, wtf_size_t max_lines = 0);
    ...
    // SVG <text>.
    std::optional<PhysicalSize> ToFragmentItems(const PhysicalSize& outer_size, void* data);
...
private:
    ...
    HeapHashMap<Member<const PhysicalFragment>, Member<LogicalLineContainer>> line_container_map_;
    LogicalLineContainer* const line_container_pool_ = MakeGarbageCollected<LogicalLineContainer>();
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::FragmentItemsBuilder::ItemWithOffset)
