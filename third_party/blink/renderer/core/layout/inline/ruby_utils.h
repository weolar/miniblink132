...
[[nodiscard]] std::pair<LayoutUnit, LayoutUnit> ApplyRubyAlign(LayoutUnit available_line_size, bool on_start_edge, bool on_end_edge, LineInfo& line_info);
...
public:
    ...
    public:
        ...
        const Vector<wtf_size_t>& BaseIndexList() const
        {
            return base_index_list_;
        }
    ...
    RubyBlockPositionCalculator& GroupLines(const HeapVector<Member<LogicalRubyColumn>>& column_list);

    // Update block offset values of annotation LogicalRubyColumns. This must be
    // called after GroupLines().
    RubyBlockPositionCalculator& PlaceLines(const LogicalLineItems& base_line_items, const FontHeight& line_box_metrics);
...
private:
    void HandleRubyLine(const RubyLine& current_ruby_line, const HeapVector<Member<LogicalRubyColumn>>& column_list);
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::RubyBlockPositionCalculator::AnnotationDepth)
