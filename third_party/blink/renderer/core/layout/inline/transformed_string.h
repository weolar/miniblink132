...
public:
    ...
    explicit TransformedString(StringView view)
        : view_(view)
    {
    }
    ...
    static CORE_EXPORT Vector<unsigned> CreateLengthMap(unsigned dom_length, unsigned transformed_length, const TextOffsetMap& offset_map);

    const StringView& View() const
    {
        return view_;
    }
    bool HasLengthMap() const
    {
        return !length_map_.empty();
    }
    const base::span<Length>& LengthMap() const
    {
        return length_map_;
    }
