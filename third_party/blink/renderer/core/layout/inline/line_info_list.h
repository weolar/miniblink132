...
inline LineInfo& LineInfoList::Get(const InlineBreakToken* break_token, bool& is_cached_out)
{
    ...
    if (break_token ? line_info.Start() == break_token->Start() : line_info.Start().IsZero()) {
        RemoveFront();
        is_cached_out = true;
        return line_info;
    }
...
//
template <wtf_size_t max_lines> class LineInfoListOf : public LineInfoList {
public:
    LineInfoListOf()
        : LineInfoList(line_infos_instance_, max_lines)
    {
    }
