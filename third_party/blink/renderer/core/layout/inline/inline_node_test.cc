...
public:
    ...
    {
        return InlineNode::IsNGShapeCacheAllowed(text_content, override_font, items, spacing);
    }
    ...
    void Append(const String& text, LayoutObject* layout_object)
    {
        ...
        data->items.push_back(InlineItem(InlineItem::kText, start, start + text.length(), layout_object));
    }
    ...
    void Append(UChar character)
    {
        ...
        data->items.push_back(InlineItem(InlineItem::kBidiControl, end - 1, end, nullptr));
    ...
    void CollectInlines()
    {
        InlineNode::CollectInlines(MutableData());
    }
    void ShapeText()
    {
        InlineNode::ShapeText(MutableData());
    }
};
...
protected:
    ...
    InlineNodeForTest CreateInlineNode(LayoutBlockFlow* layout_block_flow = nullptr)
    {
    ...
    {
        const auto space = ConstraintSpaceBuilder(node.Style().GetWritingMode(), node.Style().GetWritingDirection(),
            /* is_new_fc */ false)
                               .ToConstraintSpace();

        return node.ComputeMinMaxSizes(node.Style().GetWritingMode(), space, MinMaxSizesFloatInput()).sizes;
    }
    ...
    void ForceLayout()
    {
        GetDocument().body()->OffsetTop();
    }

    Vector<unsigned> ToEndOffsetList(InlineItemSegments::const_iterator segments)
    {
    ...
    void LoadGoogleSans()
    {
        LoadFontFromFile(GetFrame(), test::CoreTestDataPath("GoogleSans-Regular.ttf"), AtomicString("Google Sans"));
    }
...
TEST_F(InlineNodeTest, CollectInlinesFloat)
{
    ...
    InlineNodeForTest node = CreateInlineNode();
    node.CollectInlines();
    EXPECT_EQ("abc\uFFFCghi\uFFFCmno", node.Text()) << "floats are appeared as an object replacement character";
    HeapVector<InlineItem>& items = node.Items();
...
TEST_F(InlineNodeTest, CollectInlinesInlineBlock)
{
    ...
    InlineNodeForTest node = CreateInlineNode();
    node.CollectInlines();
    EXPECT_EQ("abc\uFFFCjkl", node.Text()) << "inline-block is appeared as an object replacement character";
    HeapVector<InlineItem>& items = node.Items();
...
{
    InsertStyleElement("#t { text-combine-upright: all; writing-mode: vertical-rl; }");
    SetupHtml("t", u"<div id=t>a<br>z</div>");
    InlineNodeForTest node = CreateInlineNode(To<LayoutBlockFlow>(layout_object_.Get()));
    node.CollectInlines();
...
TEST_F(InlineNodeTest, CollectInlinesTextCombineListItemMarker)
{
    ...
    //   LayoutTextCombine (anonymous)
    //     LayoutText {#text} "a"
    InlineNodeForTest node = CreateInlineNode(To<LayoutTextCombine>(layout_object_->SlowFirstChild()));
    node.CollectInlines();
    EXPECT_EQ("\u2022", node.Text());
    HeapVector<InlineItem>& items = node.Items();
...
static InlineNodeForTest CreateBidiIsolateNode(InlineNodeForTest node, LayoutObject* layout_object)
{
...
struct MinMaxData {
    ...
    { "M):\n<span>    </span>p", { 10, 90 }, "white-space: pre-wrap; word-break: break-word;", "span { font-size: 200%; }" },
    // Tabulation characters with `break-word`.
    { "&#9;&#9;<span>X</span>", { 10, 170 }, "white-space: pre-wrap; word-break: break-word;" },
    // Soft Hyphens.
    ...
    // Floats.
    { "XXX <div id=left></div> XXXX", { 50, 130 }, "", "#left { float: left; width: 50px; }" },
    // Floats with clearances.
    { "XXX <div id=left></div><div id=right></div><div id=left></div> XXX", { 50, 160 }, "",
...
class MinMaxTest : public InlineNodeTest, public testing::WithParamInterface<MinMaxData> { };

INSTANTIATE_TEST_SUITE_P(InlineNodeTest, MinMaxTest, testing::ValuesIn(min_max_data));
...
TEST_P(MinMaxTest, Data)
{
    ...
    if (data.lang) {
        ...
        LayoutLocale::SetHyphenationForTesting(AtomicString(data.lang), MockHyphenation::Create());
    }
    html.AppendFormat(">%s</div>", data.content);
    SetupHtml("target", html.ToString());
    InlineNodeForTest node = CreateInlineNode();
    const MinMaxSizes actual_sizes = ComputeMinMaxSizes(node);
    const MinMaxSizes expected_sizezs { LayoutUnit(data.min_max[0]), LayoutUnit(data.min_max[1]) };
...
{
    SetBodyInnerHTML("<pre id=t style='-webkit-rtl-ordering:visual'>ab\nde</pre>");
    auto* const layout_text = To<LayoutText>(GetElementById("t")->firstChild()->GetLayoutObject());
...
struct StyleChangeData {
    ...
    { "#parent.after { text-decoration-line: underline; }", StyleChangeData::kNone, false, true },
    ...
    { "#parent.after { position: absolute; }", StyleChangeData::kContainer, true },
    ...
    // |CollectInlines()|.
    { "#parent.after { unicode-bidi: bidi-override; }", StyleChangeData::kParentAndAbove, true },
    { "#container.after { unicode-bidi: bidi-override; }", StyleChangeData::kContainer, false },
};
...
class StyleChangeTest : public InlineNodeTest, public testing::WithParamInterface<StyleChangeData> { };

INSTANTIATE_TEST_SUITE_P(InlineNodeTest, StyleChangeTest, testing::ValuesIn(style_change_data));
...
TEST_P(StyleChangeTest, NeedsCollectInlinesOnStyle)
{
    ...
    if (data.is_line_dirty) {
        TestAnyItemsAreDirty(*To<LayoutBlockFlow>(container->GetLayoutObject()), *data.is_line_dirty);
    }

    if (data.invalidate_ink_overflow) {
        const LayoutObject* parent_layout_object = parent->GetLayoutObject();
        for (const LayoutObject* child = parent_layout_object->SlowFirstChild(); child; child = child->NextInPreOrder(parent_layout_object)) {
            if (child->IsText()) {
                InlineCursor cursor;
                for (cursor.MoveTo(*child); cursor; cursor.MoveToNextForSameLayoutObject()) {
                    const FragmentItem* item = cursor.CurrentItem();
                    EXPECT_EQ(item->IsInkOverflowComputed(), !*data.invalidate_ink_overflow);
                }
            }
        }
    }
...
static CreateNode node_creators[] = { [](Document& document) -> Node* { return document.createTextNode("new"); },
    [](Document& document) -> Node* { return document.CreateRawElement(html_names::kSpanTag); },
...
class NodeInsertTest : public InlineNodeTest, public testing::WithParamInterface<CreateNode> { };

INSTANTIATE_TEST_SUITE_P(InlineNodeTest, NodeInsertTest, testing::ValuesIn(node_creators));
...
class NodeRemoveTest : public InlineNodeTest, public testing::WithParamInterface<const char*> { };

INSTANTIATE_TEST_SUITE_P(InlineNodeTest, NodeRemoveTest, testing::Values(nullptr, "span", "abspos", "float", "inline-block", "img"));
...
TEST_F(InlineNodeTest, SegmentBidiChangeSetsNeedsLayout)
{
    ...
    // up to |PrepareLayout|.
    GetDocument().UpdateStyleAndLayoutTree();
    LayoutBlockFlow* container = To<LayoutBlockFlow>(GetLayoutObjectByElementId("container"));
...
{
    SetupHtml("container", "<div id=container><b id=blockify><i>foo</i></b></div>");
    ASSERT_TRUE(layout_block_flow_->GetInlineNodeData());

    GetElementById("blockify")->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kBlock);
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(InlineNodeTest, ClearFirstInlineFragmentOnSplitFlow)
{
    ...
    EXPECT_EQ(inner_span_cb_cursor.Current().GetLayoutObject(), outer_span->GetLayoutObject());
}
...
{
    SetupHtml("container", "<pre id=container>foo<span dir=rtl>\nbar</span></pre>");
    EXPECT_EQ(String(u"foo\u2067\u2069\n\u2067bar\u2069"), GetText());

    GetDocument().QuerySelector(AtomicString("span"))->removeAttribute(html_names::kDirAttr);
    UpdateAllLifecyclePhasesForTest();
...
{
    SetupHtml("container", "<pre id=container>foo<span dir=ltr>\nbar</span></pre>");
    EXPECT_EQ(String(u"foo\u2066\u2069\n\u2066bar\u2069"), GetText());
...
TEST_F(InlineNodeTest, CollapsibleSpaceFollowingBRWithNoWrapStyle)
{
    ...
    GetDocument().QuerySelector(AtomicString("span"))->removeAttribute(html_names::kStyleAttr);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ("\n", GetText());
}
...
TEST_F(InlineNodeTest, ReuseFirstNonSafe)
{
    ...
    EXPECT_EQ(StringView(data->text_content, item_v.StartOffset(), item_v.Length()), "V");
...
TEST_F(InlineNodeTest, ShouldNotResueLigature)
{
    ...
    const LayoutText& layout_text = *To<Text>(sample.firstChild())->GetLayoutObject();
    const ShapeResult& shape_result_before = *layout_text.InlineItems().begin()->TextShapeResult();
    ASSERT_EQ(3u, shape_result_before.NumGlyphs());

    const LayoutText& layout_text_i = *To<Text>(sample.lastChild()->firstChild())->GetLayoutObject();
    const ShapeResult& shape_result_i = *layout_text_i.InlineItems().begin()->TextShapeResult();
    ...
    const ShapeResult& shape_result_after = *layout_text.InlineItems().begin()->TextShapeResult();
...
{
    LoadAhem();
    InsertStyleElement("p { font: 20px/24px Ahem; }"
    ...
    auto& initial_letter_box = *To<LayoutBlockFlow>(sample.GetPseudoElement(kPseudoIdFirstLetter)->GetLayoutObject());
...
TEST_F(InlineNodeTest, TextCombineUsesScalingX)
{
    ...
    EXPECT_TRUE(To<LayoutTextCombine>(GetLayoutObjectByElementId("t1")->SlowFirstChild())->UsesScaleX())
        << "We paint combined text '0123456789' with scaling in X-axis.";
    EXPECT_FALSE(To<LayoutTextCombine>(GetLayoutObjectByElementId("t2")->SlowFirstChild())->UsesScaleX())
...
TEST_F(InlineNodeTest, TextCombineWordSpacing)
{
    ...
    const auto& text = *To<Text>(GetElementById("t1")->firstChild())->GetLayoutObject();
...
{
    SetBodyInnerHTML("<svg><text id='text' xml:space='preserve'>"
...
TEST_F(InlineNodeTest, ShapeCacheDisabled)
{
    ...
    EXPECT_FALSE(node.IsNGShapeCacheAllowed(text_content, nullptr, items, spacing));
}
