...
LogicalLineBuilder::LogicalLineBuilder(InlineNode node, const ConstraintSpace& constraint_space, const InlineBreakToken* break_token,
    InlineLayoutStateStack* state_stack, InlineChildLayoutContext* context)
    : node_(node)
...
void LogicalLineBuilder::CreateLine(LineInfo* line_info, LogicalLineItems* line_box, InlineLayoutAlgorithm* main_line_helper)
{
    ...
    InlineBoxState* box = box_states_->OnBeginPlaceItems(node_, line_style, baseline_type_, quirks_mode_, line_box);
...
#endif
    box = HandleItemResults(*line_info, *line_items, line_box, main_line_helper, box);
    ...
    if (node_.IsBidiEnabled()) [[unlikely]] {
        ...
        BidiReorder(line_info->BaseDirection(), line_box, box_states_->RubyColumnList());
    ...
    for (auto& logical_column : box_states_->RubyColumnList()) {
        ...
        ApplyLeftAndRightExpansion(insets.first, insets.second, base::span(*line_box).subspan(logical_column->start_index, logical_column->size));
    }
}

InlineBoxState* LogicalLineBuilder::HandleItemResults(
    const LineInfo& line_info, InlineItemResults& line_items, LogicalLineItems* line_box, InlineLayoutAlgorithm* main_line_helper, InlineBoxState* box)
{
    for (InlineItemResult& item_result : line_items) {
        ...
        if (item.Type() == InlineItem::kText) {
            ...
            DCHECK(item.GetLayoutObject()->IsText() || item.GetLayoutObject()->IsLayoutListItem());
            ...
            DCHECK(item.TextType() == TextItemType::kNormal || item.TextType() == TextItemType::kSymbolMarker);
            if (item_result.is_hyphenated) [[unlikely]] {
                ...
                line_box->AddChild(item, item_result, item_result.TextOffset(), box->text_top, item_result.inline_size - hyphen_inline_size, box->text_height,
                    item.BidiLevel());
                ...
                line_box->AddChild(item, item_result, item_result.TextOffset(), text_top, item_result.inline_size, text_height, item.BidiLevel());
            } else {
                line_box->AddChild(item, item_result, item_result.TextOffset(), box->text_top, item_result.inline_size, box->text_height, item.BidiLevel());
            }
            ...
            PlaceControlItem(item, line_info.ItemsData().text_content, &item_result, line_box, box);
            ...
            has_relative_positioned_items_ |= item.Style()->GetPosition() == EPosition::kRelative;
            ...
            line_box->AddChild(item, item_result, item_result.TextOffset(),
                /* block_offset */ LayoutUnit(), item_result.inline_size + start_overhang + end_overhang,
                /* text_height */ LayoutUnit(), item.BidiLevel());
            (*line_box)[line_box->size() - 1].rect.offset.inline_offset = start_overhang;
            ...
            TextDirection direction = item.GetLayoutObject()->StyleRef().IsOriginalDisplayInlineType() ? item.Direction() : constraint_space_.Direction();
            ...
            if (item_result.positioned_float) {
                if (!item_result.positioned_float->break_before_token) {
                    ...
                    line_box->AddChild(item_result.positioned_float->layout_result, item_result.positioned_float->bfc_offset, item.BidiLevel());
                }
            } else {
                line_box->AddChild(item.GetLayoutObject(), item.BidiLevel(), item_result.Start());
            }
...
InlineBoxState* LogicalLineBuilder::HandleOpenTag(const InlineItem& item, const InlineItemResult& item_result, LogicalLineItems* line_box)
{
    InlineBoxState* box = box_states_->OnOpenTag(constraint_space_, item, item_result, baseline_type_, line_box);
...
InlineBoxState* LogicalLineBuilder::HandleCloseTag(const InlineItem& item, const InlineItemResult& item_result, LogicalLineItems* line_box, InlineBoxState* box)
{
    if (quirks_mode_ && !item.IsEmptyItem()) [[unlikely]] {
        box->EnsureTextMetrics(*item.Style(), *box->font, baseline_type_);
    }
    box = box_states_->OnCloseTag(constraint_space_, line_box, box, baseline_type_);
...
void LogicalLineBuilder::PlaceControlItem(
    const InlineItem& item, const String& text_content, InlineItemResult* item_result, LogicalLineItems* line_box, InlineBoxState* box)
{
    ...
    line_box->AddChild(
        item, std::move(item_result->shape_result), item_result->TextOffset(), box->text_top, item_result->inline_size, box->text_height, item.BidiLevel());
}

void LogicalLineBuilder::PlaceHyphen(const InlineItemResult& item_result, LayoutUnit hyphen_inline_size, LogicalLineItems* line_box, InlineBoxState* box)
{
    DCHECK(item_result.item);
    ...
    const InlineItem& item = *item_result.item;
    line_box->AddChild(item, ShapeResultView::Create(&item_result.hyphen.GetShapeResult()), item_result.hyphen.Text(), box->text_top, hyphen_inline_size,
        box->text_height, item.BidiLevel());
}

InlineBoxState* LogicalLineBuilder::PlaceAtomicInline(const InlineItem& item, InlineItemResult* item_result, LogicalLineItems* line_box)
{
    ...
    if (!IsA<LayoutTextCombine>(layout_object)) [[likely]] {
        PlaceLayoutResult(item_result, line_box, box, box->margins.inline_start + item_result->spacing_before);
    } else {
        ...
        const LayoutUnit inline_offset = box->margins.inline_start + item_result->spacing_before;
        line_box->AddChild(std::move(item_result->layout_result), LogicalOffset { inline_offset, box->text_top }, item_result->inline_size,
            /* children_count */ 0, item.BidiLevel());
    }
    return box_states_->OnCloseTag(constraint_space_, line_box, box, baseline_type_);
}

// Place a LayoutResult into the line box.
void LogicalLineBuilder::PlaceLayoutResult(InlineItemResult* item_result, LogicalLineItems* line_box, InlineBoxState* box, LayoutUnit inline_offset)
{
    DCHECK(item_result->layout_result);
    DCHECK(item_result->item);
    const InlineItem& item = *item_result->item;
    DCHECK(item.Style());
    FontHeight metrics = LogicalBoxFragment(constraint_space_.GetWritingDirection(), To<PhysicalBoxFragment>(item_result->layout_result->GetPhysicalFragment()))
    ...
    line_box->AddChild(
        std::move(item_result->layout_result), LogicalOffset { inline_offset, line_top }, item_result->inline_size, /* children_count */ 0, item.BidiLevel());
}

void LogicalLineBuilder::PlaceInitialLetterBox(const InlineItem& item, InlineItemResult* item_result, LogicalLineItems* line_box)
{
    DCHECK(item_result->layout_result);
    ...
    line_box->AddChild(std::move(item_result->layout_result), LogicalOffset { item_result->margins.inline_start, LayoutUnit() }, item_result->inline_size,
        /* children_count */ 0, item.BidiLevel());
}

InlineBoxState* LogicalLineBuilder::PlaceRubyColumn(const LineInfo& line_info, InlineItemResult& item_result, LogicalLineItems& line_box, InlineBoxState* box)
{
    ...
    if (!node_.IsBidiEnabled() && !line_info.IsRubyBase() && !line_info.IsRubyText()
        && (line_info.TextAlign() == ETextAlign::kJustify
            || (line_info.IsLastLine() && line_info.LineStyle().GetTextAlign(/* is_last_line */ false) == ETextAlign::kJustify))) {
        on_start_edge = ruby_column.base_line.InflowStartOffset() == line_info.InflowStartOffset();
        if (line_info.TextAlign() == ETextAlign::kJustify) {
            ...
            if (on_start_edge && on_end_edge && item_result.inline_size > ruby_column.base_line.Width()) {
                line_available_size = line_info.AvailableWidth();
            }
        }
    }
    std::pair<LayoutUnit, LayoutUnit> base_insets
        = ApplyRubyAlign(line_available_size.value_or(item_result.inline_size), on_start_edge, on_end_edge, ruby_column.base_line);
    ...
    box = HandleItemResults(line_info, *ruby_column.base_line.MutableResults(), &line_box,
    ...
    for (wtf_size_t i = 0; i < ruby_column.annotation_line_list.size(); ++i) {
        LogicalRubyColumn& logical_column = box_states_->RubyColumnAt(ruby_column_start_index + i);
        if (!ruby_column.annotation_line_list[i].IsEmptyLine()) {
            if (!line_box[start_index].has_over_annotation && logical_column.ruby_position == RubyPosition::kOver) {
                for (wtf_size_t j = start_index; j < line_box.size(); ++j) {
                    line_box[j].has_over_annotation = true;
                }
            }
            if (!line_box[start_index].has_under_annotation && logical_column.ruby_position == RubyPosition::kUnder) {
                for (wtf_size_t j = start_index; j < line_box.size(); ++j) {
        ...
        PlaceRubyAnnotation(item_result, line_available_size, i, ruby_column.annotation_line_list[i], logical_column);
    }

    return box;
}

void LogicalLineBuilder::PlaceRubyAnnotation(InlineItemResult& item_result, std::optional<LayoutUnit> line_available_size, wtf_size_t index,
    LineInfo& annotation_line, LogicalRubyColumn& logical_column)
{
    std::pair<LayoutUnit, LayoutUnit> insets
        = ApplyRubyAlign(line_available_size.value_or(item_result.inline_size - item_result.ruby_column->last_base_glyph_spacing),
            /* on_start_edge */ false,
            /* on_end_edge */ false, annotation_line);

    auto* line_items = MakeGarbageCollected<LogicalLineItems>();
    LogicalLineBuilder annotation_builder(node_, constraint_space_, nullptr, &logical_column.state_stack, context_);
    if (item_result.ruby_column->is_continuation && !annotation_line.Results().empty()) {
        CHECK(break_token_->RubyData());
        annotation_builder.RebuildBoxStates(
            annotation_line, break_token_->RubyData()->annotation_data[index].start_item_index, annotation_line.Results()[0].item_index);
    }
    annotation_builder.CreateLine(&annotation_line, line_items,
        /* main_line_helper */ nullptr);
    ApplyLeftAndRightExpansion(insets.first, insets.second, base::span(*line_items));

    logical_column.state_stack.ComputeInlinePositions(line_items, LayoutUnit(), /* ignore_box_margin_border_padding */ false);

    logical_column.annotation_items = line_items;
}

// Place a list marker.
void LogicalLineBuilder::PlaceListMarker(const InlineItem& item, InlineItemResult* item_result)
{
    if (quirks_mode_) [[unlikely]] {
        box_states_->LineBoxState().EnsureTextMetrics(*item.Style(), item.Style()->GetFont(), baseline_type_);
    }
}

void LogicalLineBuilder::BidiReorder(TextDirection base_direction, LogicalLineItems* line_box, HeapVector<Member<LogicalRubyColumn>>& column_list)
{
    if (line_box->IsEmpty()) {
        return;
    }
    ...
    if (column_list.size() > 0) {
        ...
        for (auto& column : column_list) {
            ...
            column->start_index = *base::ranges::min_element(base::span(logical_to_visual).subspan(column->start_index, column->size));
        }
        // The order is important for RubyBlockPositionCalculator::HandleRubyLine().
        std::stable_sort(column_list.begin(), column_list.end(), [](const auto& column1, const auto& column2) {
...
void LogicalLineBuilder::RebuildBoxStates(const LineInfo& line_info, wtf_size_t start_item_index, wtf_size_t end_item_index)
{
    // Compute which tags are not closed at the beginning of this line.
    InlineItemsData::OpenTagItems open_items;
    line_info.ItemsData().GetOpenTagItems(start_item_index, end_item_index - start_item_index, &open_items);

    // Create box states for tags that are not closed yet.
    LogicalLineItems& line_box = context_->AcquireTempLogicalLineItems();
    box_states_->OnBeginPlaceItems(node_, line_info.LineStyle(), baseline_type_, quirks_mode_, &line_box);
    for (const InlineItem* item : open_items) {
        InlineItemResult item_result;
        LineBreaker::ComputeOpenTagResult(*item, constraint_space_, node_.IsSvgText(), &item_result);
