...
public:
    ...
    LineInfo& GetLineInfo(const InlineBreakToken* break_token, bool& is_cached_out);
    ...
    bool HasBoxStates() const
    {
        return box_states_.has_value();
    }
    InlineLayoutStateStack* BoxStates()
    {
        return &*box_states_;
    }
    InlineLayoutStateStack* ResetBoxStates()
    {
        return &box_states_.emplace();
    }
    ...
    // current line.
    InlineLayoutStateStack* BoxStatesIfValidForItemIndex(const HeapVector<InlineItem>& items, unsigned item_index);
...
protected:
    InlineChildLayoutContext(const InlineNode& node, BoxFragmentBuilder* container_builder, LineInfo* line_info);
    InlineChildLayoutContext(const InlineNode& node, BoxFragmentBuilder* container_builder, ScoreLineBreakContext* score_line_break_context);
...
// only one `LineInfo`.
class CORE_EXPORT SimpleInlineChildLayoutContext : public InlineChildLayoutContext {
public:
    SimpleInlineChildLayoutContext(const InlineNode& node, BoxFragmentBuilder* container_builder)
...
template <wtf_size_t max_lines> class CORE_EXPORT OptimalInlineChildLayoutContext : public InlineChildLayoutContext {
public:
    OptimalInlineChildLayoutContext(const InlineNode& node, BoxFragmentBuilder* container_builder)
        : InlineChildLayoutContext(node, container_builder, &score_line_break_context_instance_)
    {
    }
...
inline LineInfo& InlineChildLayoutContext::GetLineInfo(const InlineBreakToken* break_token, bool& is_cached_out)
{
    ...
    return score_line_break_context_->GetLineInfoList().Get(break_token, is_cached_out);
}

inline LogicalLineItems& InlineChildLayoutContext::AcquireTempLogicalLineItems()
{
...
inline void InlineChildLayoutContext::ReleaseTempLogicalLineItems(LogicalLineItems& line_items)
{
    DCHECK(&line_items);
    line_items.clear();
