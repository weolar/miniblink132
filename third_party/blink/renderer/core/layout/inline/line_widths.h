...
public:
    ...
    explicit LineWidths(LayoutUnit width)
        : default_width_(width)
    {
    }

    LayoutUnit Default() const
    {
        return default_width_;
    }
    bool HasExclusions() const
    {
        return num_excluded_lines_;
    }
    ...
    bool Set(const InlineNode& node, base::span<const LayoutOpportunity> opportunities, const InlineBreakToken* break_token = nullptr);
