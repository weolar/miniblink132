...
struct LogicalLineItem {
    ...
    explicit LogicalLineItem(UBiDiLevel bidi_level)
        : bidi_level(bidi_level)
    {
    }
    // Create an in-flow |LayoutResult|.
    LogicalLineItem(const LayoutResult* layout_result, const LogicalRect& rect, unsigned children_count, UBiDiLevel bidi_level)
    ...
    LogicalLineItem(const LayoutResult* layout_result, LogicalOffset offset, LayoutUnit inline_size, unsigned children_count, UBiDiLevel bidi_level)
        : layout_result(layout_result)
    ...
    LogicalLineItem(const InlineItem& inline_item, InlineItemResult& item_result, const TextOffsetRange& text_offset, LayoutUnit block_offset,
        LayoutUnit inline_size, LayoutUnit text_height, UBiDiLevel bidi_level)
    ...
    LogicalLineItem(const InlineItem& inline_item, const ShapeResultView* shape_result, const TextOffsetRange& text_offset, LayoutUnit block_offset,
        LayoutUnit inline_size, LayoutUnit text_height, UBiDiLevel bidi_level)
        : inline_item(&inline_item)
    ...
    LogicalLineItem(const InlineItem& inline_item, const ShapeResultView* shape_result, const String& text_content, LayoutUnit block_offset,
        LayoutUnit inline_size, LayoutUnit text_height, UBiDiLevel bidi_level)
        : inline_item(&inline_item)
        , shape_result(shape_result)
    ...
    LogicalLineItem(const LayoutObject& layout_object, StyleVariant style_variant, const ShapeResultView* shape_result, const String& text_content,
        const LogicalRect& rect, UBiDiLevel bidi_level)
        : shape_result(shape_result)
        , text_offset({ shape_result->StartIndex(), shape_result->EndIndex() })
        , text_content(text_content)
    ...
    LogicalLineItem(const LogicalLineItem& source_item, const ShapeResultView* shape_result, const TextOffsetRange& text_offset)
    ...
    LogicalLineItem(LayoutObject* out_of_flow_positioned_box, UBiDiLevel bidi_level, TextDirection container_direction)
    ...
    LogicalLineItem(LayoutObject* unpositioned_float, UBiDiLevel bidi_level, InlineItemTextIndex item_index)
    ...
    LogicalLineItem(const LayoutResult* layout_result, BfcOffset bfc_offset, UBiDiLevel bidi_level)
        : layout_result(layout_result)
    ...
    {
        return layout_result && layout_result->GetPhysicalFragment().IsInitialLetterBox();
    }
    ...
    {
        return (inline_item && inline_item->Type() != InlineItem::kRubyLinePlaceholder)
            || (layout_result && !layout_result->GetPhysicalFragment().IsFloating());
    }
    bool HasInFlowOrFloatingFragment() const
    {
        return (inline_item && inline_item->Type() != InlineItem::kRubyLinePlaceholder) || layout_result || layout_object;
    }
    ...
    bool CanCreateFragmentItem() const
    {
        return HasInFlowOrFloatingFragment();
    }
    bool HasBidiLevel() const
    {
        return bidi_level != 0xff;
    }
    bool IsPlaceholder() const
    {
        return !HasFragment() && !HasBidiLevel();
    }
    bool IsOpaqueToBidiReordering() const
    {
        ...
        if (layout_result) {
            ...
            if (layout_result->GetPhysicalFragment().GetLayoutObject()->IsLayoutInline()) {
                return true;
            }
        }
        return false;
    ...
    {
        return rect.offset.inline_offset;
    }
    LayoutUnit BlockOffset() const
    {
        return rect.offset.block_offset;
    }
    LayoutUnit BlockEndOffset() const
    {
        return rect.BlockEndOffset();
    }
    const LogicalSize& Size() const
    {
        return rect.size;
    }
    LogicalSize MarginSize() const
    {
        return { inline_size, Size().block_size };
    }
    ...
    TextDirection ResolvedDirection() const
    {
        ...
        return HasBidiLevel() ? DirectionFromLevel(bidi_level) : TextDirection::kLtr;
    }
...
CORE_EXPORT std::ostream& operator<<(std::ostream& stream, const LogicalLineItem& item);
...
public:
    ...
    LogicalLineItem& operator[](wtf_size_t i)
    {
        return children_[i];
    }
    const LogicalLineItem& operator[](wtf_size_t i) const
    {
        return children_[i];
    }
    ...
    {
        children_.Shrink(size);
    }
    void swap(LogicalLineItems& other)
    {
    ...
    using const_reverse_iterator = HeapVector<LogicalLineItem, 16>::const_reverse_iterator;
    ...
    template <class... Args> void AddChild(Args&&... args)
    {
    ...
    void InsertChild(unsigned index, const LayoutResult* layout_result, const LogicalRect& rect, unsigned children_count)
    {
        WillInsertChild(index);
        children_.insert(index,
            LogicalLineItem(std::move(layout_result), rect, children_count,
                /* bidi_level */ 0));
    }
