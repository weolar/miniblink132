...
void LineBoxFragmentBuilder::PropagateChildrenData(LogicalLineContainer& container)
{
...
void LineBoxFragmentBuilder::PropagateChildrenDataFromLineItems(LogicalLineItems& children)
{
    for (unsigned index = 0; index < children.size(); ++index) {
        ...
        if (child.layout_result) {
            ...
            PropagateFromLayoutResultAndFragment(*child.layout_result, child.Offset() - ComputeRelativeOffsetForInline(GetConstraintSpace(), child_style),
                ComputeRelativeOffsetForOOFInInline(GetConstraintSpace(), child_style));
        ...
        if (child.out_of_flow_positioned_box) {
            AddOutOfFlowInlineChildCandidate(
                BlockNode(To<LayoutBox>(child.out_of_flow_positioned_box.Get())), child.Offset(), child.container_direction, child.is_hidden_for_paint);
...
const LayoutResult* LineBoxFragmentBuilder::ToLineBoxFragment()
{
    ...
    return MakeGarbageCollected<LayoutResult>(LayoutResult::LineBoxFragmentBuilderPassKey(), std::move(fragment), this);
}
