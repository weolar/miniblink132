...
const PhysicalLineBoxFragment* PhysicalLineBoxFragment::Create(LineBoxFragmentBuilder* builder)
{
    DCHECK_EQ(builder->children_.size(), 0u);
    return MakeGarbageCollected<PhysicalLineBoxFragment>(PassKey(), builder);
}

const PhysicalLineBoxFragment* PhysicalLineBoxFragment::Clone(const PhysicalLineBoxFragment& other)
{
    return MakeGarbageCollected<PhysicalLineBoxFragment>(PassKey(), other);
}

PhysicalLineBoxFragment::PhysicalLineBoxFragment(PassKey key, LineBoxFragmentBuilder* builder)
    : PhysicalFragment(builder, builder->GetWritingMode(), kFragmentLineBox, builder->line_box_type_)
    , metrics_(builder->metrics_)
{
    ...
    has_propagated_descendants_ = has_floating_descendants_for_paint_ || HasOutOfFlowPositionedDescendants() || builder->unpositioned_list_marker_;
}

PhysicalLineBoxFragment::PhysicalLineBoxFragment(PassKey key, const PhysicalLineBoxFragment& other)
