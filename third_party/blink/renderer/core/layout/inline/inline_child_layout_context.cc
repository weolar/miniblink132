...
static_assert(sizeof(InlineChildLayoutContext) == sizeof(SameSizeAsInlineChildLayoutContext),
...
bool IsBlockFragmented(const BoxFragmentBuilder& fragment_builder)
{
    ...
    return space.HasBlockFragmentation() && space.HasKnownFragmentainerBlockSize();
}

} // namespace

InlineChildLayoutContext::InlineChildLayoutContext(const InlineNode& node, BoxFragmentBuilder* container_builder, LineInfo* line_info)
    : container_builder_(container_builder)
    , items_builder_(node, container_builder->GetWritingDirection(), IsBlockFragmented(*container_builder))
...
InlineChildLayoutContext::InlineChildLayoutContext(
    const InlineNode& node, BoxFragmentBuilder* container_builder, ScoreLineBreakContext* score_line_break_context)
    : container_builder_(container_builder)
    , items_builder_(node, container_builder->GetWritingDirection(), IsBlockFragmented(*container_builder))
...
InlineLayoutStateStack* InlineChildLayoutContext::BoxStatesIfValidForItemIndex(const HeapVector<InlineItem>& items, unsigned item_index)
{
...
void InlineChildLayoutContext::PropagateParallelFlowBreakToken(const BreakToken* token)
{
