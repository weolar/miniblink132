...
public:
    ...
    RunSegmenter::RunSegmenterRange ToRunSegmenterRange(unsigned start_offset, unsigned end_offset) const;
    RunSegmenter::RunSegmenterRange ToRunSegmenterRange(unsigned start_offset) const
    {
    ...
    static constexpr unsigned kScriptBits = std::bit_width(static_cast<unsigned>(USCRIPT_CODE_LIMIT));
    static constexpr unsigned kFontFallbackPriorityBits = std::bit_width(static_cast<unsigned>(FontFallbackPriority::kMaxEnumValue));
    static constexpr unsigned kRenderOrientationBits = std::bit_width(static_cast<unsigned>(OrientationIterator::RenderOrientation::kMaxEnumValue));
    static constexpr unsigned kSegmentDataBits = kScriptBits + kFontFallbackPriorityBits + kRenderOrientationBits;

    static unsigned PackSegmentData(const RunSegmenter::RunSegmenterRange& range);
    static RunSegmenter::RunSegmenterRange UnpackSegmentData(unsigned start_offset, unsigned end_offset, unsigned value);
...
public:
    ...
    unsigned EndOffset() const
    {
        return segments_.back().EndOffset();
    }
    ...
    void ComputeSegments(RunSegmenter* segmenter, RunSegmenter::RunSegmenterRange* range);
    ...
    // fonts.
    unsigned AppendMixedFontOrientation(const String& text_content, unsigned start_offset, unsigned end_offset, unsigned segment_index);
    ...
    public:
        Iterator(unsigned start_offset, unsigned end_offset, const InlineItemSegment* segment);

        bool IsDone() const
        {
            return range_.start == end_offset_;
        }
        ...
        const RunSegmenter::RunSegmenterRange& operator*() const
        {
            return range_;
        }
        void operator++();
    ...
    const_iterator Ranges(unsigned start_offset, unsigned end_offset, unsigned item_index) const;

    // Shape runs in the range and return the concatenated |ShapeResult|.
    ShapeResult* ShapeText(const HarfBuzzShaper* shaper, const Font* font, TextDirection direction, unsigned start_offset, unsigned end_offset,
        unsigned item_index, ShapeOptions = ShapeOptions()) const;

private:
    unsigned PopulateItemsFromFontOrientation(unsigned start_offset, unsigned end_offset, OrientationIterator::RenderOrientation, unsigned segment_index);
...
inline InlineItemSegments::Iterator::Iterator(unsigned start_offset, unsigned end_offset, const InlineItemSegment* segment)
    : segment_(segment)
