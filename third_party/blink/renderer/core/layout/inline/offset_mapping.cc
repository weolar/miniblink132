...
Position CreatePositionForOffsetMapping(const Node& node, unsigned dom_offset)
{
    if (auto* text_node = DynamicTo<Text>(node)) {
        ...
        if (node.GetLayoutObject()->StyleRef().TextTransform() == ETextTransform::kNone) {
...
{
    LayoutBlockFlow* block_flow = OffsetMapping::GetInlineFormattingContextOf(position);
...
// static
LayoutBlockFlow* OffsetMapping::GetInlineFormattingContextOf(const Position& position)
{
    if (!AcceptsPosition(position))
        return nullptr;
    const auto node_offset_pair = ToNodeOffsetPair(position);
    const LayoutObject* layout_object = AssociatedLayoutObjectOf(node_offset_pair.first, node_offset_pair.second);
    if (!layout_object)
        return nullptr;
    return GetInlineFormattingContextOf(*layout_object);
}

OffsetMappingUnit::OffsetMappingUnit(
    OffsetMappingUnitType type, const LayoutObject& layout_object, unsigned dom_start, unsigned dom_end, unsigned text_content_start, unsigned text_content_end)
    : type_(type)
    ,
    // Use atomic construction to allow for concurrently marking
    // OffsetMappingUnit.
    layout_object_(&layout_object, Member<const LayoutObject>::AtomicInitializerTag {})
...
void OffsetMappingUnit::AssertValid() const
{
    ...
    SECURITY_DCHECK(text_content_start_ <= text_content_end_) << text_content_start_ << " vs. " << text_content_end_;
    if (layout_object_->IsText() && !To<LayoutText>(*layout_object_).IsWordBreak()) {
        const auto& layout_text = To<LayoutText>(*layout_object_);
        const unsigned text_start = AssociatedNode() ? layout_text.TextStartOffset() : 0;
        SECURITY_DCHECK(dom_end_ >= text_start) << dom_end_ << " vs. " << text_start;
    } else {
...
{
    if (const auto* text_fragment = DynamicTo<LayoutTextFragment>(layout_object_.Get()))
...
bool OffsetMappingUnit::Concatenate(const OffsetMappingUnit& other)
{
    ...
    if (const auto* text_fragment = DynamicTo<LayoutTextFragment>(layout_object_.Get())) {
        ...
        if (text_fragment->IsRemainingTextLayoutObject() && other.dom_start_ == text_fragment->TextStartOffset())
            return false;
    }
...
bool OffsetMapping::AcceptsPosition(const Position& position)
{
    ...
    if (position.AnchorNode()->IsTextNode()) {
        ...
        DCHECK(position.IsOffsetInAnchor() || position.IsBeforeAnchor() || position.IsAfterAnchor());
        return true;
    }
...
const OffsetMapping* OffsetMapping::ForceGetFor(const Position& position)
{
    ...
    LayoutBlockFlow* context = OffsetMapping::GetInlineFormattingContextOf(position);
    if (!context)
        return nullptr;
...
// static
LayoutBlockFlow* OffsetMapping::GetInlineFormattingContextOf(const LayoutObject& object)
{
    for (LayoutObject* runner = object.Parent(); runner; runner = runner->Parent()) {
    ...
    , text_(text)
{
    ...
    for (const auto& unit : units_) {
        SECURITY_DCHECK(unit.TextContentStart() <= text.length()) << unit.TextContentStart() << "<=" << text.length();
        SECURITY_DCHECK(unit.TextContentEnd() <= text.length()) << unit.TextContentEnd() << "<=" << text.length();
        unit.AssertValid();
    }
    for (const auto& pair : ranges_) {
        SECURITY_DCHECK(pair.value.first < units_.size()) << pair.value.first << "<" << units_.size();
        SECURITY_DCHECK(pair.value.second <= units_.size()) << pair.value.second << "<=" << units_.size();
    }
#endif
}
...
const OffsetMappingUnit* OffsetMapping::GetMappingUnitForPosition(const Position& position) const
{
    ...
    std::tie(range_start, range_end) = it != ranges_.end() ? it->value : std::pair<unsigned, unsigned>(0, 0);
    if (range_start == range_end || units_[range_start].DOMStart() > offset)
        return nullptr;
    // Find the last unit where unit.dom_start <= offset
    auto unit = std::prev(std::upper_bound(units_.begin() + range_start, units_.begin() + range_end, offset,
        [](unsigned offset, const OffsetMappingUnit& unit) { return offset < unit.DOMStart(); }));
    if (unit->DOMEnd() < offset)
        return nullptr;
    return &*unit;
}

OffsetMapping::UnitVector OffsetMapping::GetMappingUnitsForDOMRange(const EphemeralRange& range) const
{
    DCHECK(OffsetMapping::AcceptsPosition(range.StartPosition()));
    DCHECK(OffsetMapping::AcceptsPosition(range.EndPosition()));
    DCHECK_EQ(range.StartPosition().AnchorNode(), range.EndPosition().AnchorNode());
    ...
    if (range_start == range_end || units_[range_start].DOMStart() > end_offset || units_[range_end - 1].DOMEnd() < start_offset)
        return UnitVector();

    // Find the first unit where unit.dom_end >= start_offset
    auto result_begin = std::lower_bound(units_.begin() + range_start, units_.begin() + range_end, start_offset,
        [](const OffsetMappingUnit& unit, unsigned offset) { return unit.DOMEnd() < offset; });

    // Find the next of the last unit where unit.dom_start <= end_offset
    auto result_end = std::upper_bound(
        result_begin, units_.begin() + range_end, end_offset, [](unsigned offset, const OffsetMappingUnit& unit) { return offset < unit.DOMStart(); });
    ...
    for (const auto& unit : base::make_span(result_begin, result_end)) {
        ...
        result.emplace_back(unit.GetType(), unit.GetLayoutObject(), clamped_start, clamped_end, clamped_text_content_start, clamped_text_content_end);
    }
    return result;
}

base::span<const OffsetMappingUnit> OffsetMapping::GetMappingUnitsForNode(const Node& node) const
{
    const auto it = ranges_.find(&node);
    if (it == ranges_.end()) {
        return {};
    }
    return base::make_span(units_.begin() + it->value.first, units_.begin() + it->value.second);
}

base::span<const OffsetMappingUnit> OffsetMapping::GetMappingUnitsForLayoutObject(const LayoutObject& layout_object) const
{
    const auto begin = base::ranges::find(units_, layout_object, &OffsetMappingUnit::GetLayoutObject);
    CHECK_NE(begin, units_.end());
    const auto end
        = std::find_if(std::next(begin), units_.end(), [&layout_object](const OffsetMappingUnit& unit) { return unit.GetLayoutObject() != layout_object; });
    DCHECK_LT(begin, end);
    return base::make_span(begin, end);
}

base::span<const OffsetMappingUnit> OffsetMapping::GetMappingUnitsForTextContentOffsetRange(unsigned start, unsigned end) const
{
    DCHECK_LE(start, end);
    if (units_.front().TextContentStart() >= end || units_.back().TextContentEnd() <= start)
        return {};

    // Find the first unit where unit.text_content_end > start
    auto result_begin
        = std::lower_bound(units_.begin(), units_.end(), start, [](const OffsetMappingUnit& unit, unsigned offset) { return unit.TextContentEnd() <= offset; });
    ...
    auto result_end
        = std::upper_bound(units_.begin(), units_.end(), end, [](unsigned offset, const OffsetMappingUnit& unit) { return offset <= unit.TextContentStart(); });
    return base::make_span(result_begin, result_end);
}

std::optional<unsigned> OffsetMapping::GetTextContentOffset(const Position& position) const
{
    DCHECK(OffsetMapping::AcceptsPosition(position)) << position;
...
Position OffsetMapping::StartOfNextNonCollapsedContent(const Position& position) const
{
    DCHECK(OffsetMapping::AcceptsPosition(position)) << position;
    const OffsetMappingUnit* unit = GetMappingUnitForPosition(position);
    if (!unit)
    ...
    const Node& node = node_and_offset.first;
    const unsigned offset = node_and_offset.second;
    while (unit != units_.data() + units_.size() && unit->AssociatedNode() == node) {
        if (unit->DOMEnd() > offset && unit->GetType() != OffsetMappingUnitType::kCollapsed) {
...
Position OffsetMapping::EndOfLastNonCollapsedContent(const Position& position) const
{
    DCHECK(OffsetMapping::AcceptsPosition(position)) << position;
    const OffsetMappingUnit* unit = GetMappingUnitForPosition(position);
    if (!unit)
    ...
    while (unit->AssociatedNode() == node) {
        if (unit->DOMStart() < offset && unit->GetType() != OffsetMappingUnitType::kCollapsed) {
...
bool OffsetMapping::IsBeforeNonCollapsedContent(const Position& position) const
{
    DCHECK(OffsetMapping::AcceptsPosition(position));
    const OffsetMappingUnit* unit = GetMappingUnitForPosition(position);
    const unsigned offset = ToNodeOffsetPair(position).second;
    return unit && offset < unit->DOMEnd() && unit->GetType() != OffsetMappingUnitType::kCollapsed;
}
...
bool OffsetMapping::IsAfterNonCollapsedContent(const Position& position) const
{
    ...
    const OffsetMappingUnit* unit = GetMappingUnitForPosition(CreatePositionForOffsetMapping(node, offset - 1));
    return unit && offset > unit->DOMStart() && unit->GetType() != OffsetMappingUnitType::kCollapsed;
}

std::optional<UChar> OffsetMapping::GetCharacterBefore(const Position& position) const
{
    DCHECK(OffsetMapping::AcceptsPosition(position));
...
Position OffsetMapping::GetFirstPosition(unsigned offset) const
{
    ...
    if (units_.empty() || units_.back().TextContentEnd() < offset)
        return {};
    auto result
        = std::lower_bound(units_.begin(), units_.end(), offset, [](const OffsetMappingUnit& unit, unsigned offset) { return unit.TextContentEnd() < offset; });
...
const OffsetMappingUnit* OffsetMapping::GetLastMappingUnit(unsigned offset) const
{
    ...
    auto result = std::upper_bound(
        units_.begin(), units_.end(), offset, [](unsigned offset, const OffsetMappingUnit& unit) { return offset < unit.TextContentStart(); });
...
unsigned OffsetMapping::LayoutObjectConverter::TextContentOffset(unsigned offset) const
{
    auto iter = offset >= last_offset_ ? last_unit_ : units_.begin();
    if (offset >= iter->DOMEnd()) {
        iter = base::ranges::find_if(
            iter, units_.end(), [offset](const OffsetMappingUnit& unit) { return unit.DOMStart() <= offset && offset < unit.DOMEnd(); });
    }
