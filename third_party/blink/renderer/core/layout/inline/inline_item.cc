...
bool IsInlineBoxStartEmpty(const ComputedStyle& style, const LayoutObject& layout_object)
{
    ...
    if (!style.MarginInlineStart().IsZero() && !layout_object.GetDocument().InLineHeightQuirksMode()) {
        return false;
    }
...
// as non-empty.
bool IsInlineBoxEndEmpty(const ComputedStyle& style, const LayoutObject& layout_object)
{
    ...
    // Non-zero margin can prevent "empty" only in non-quirks mode.
    // https://quirks.spec.whatwg.org/#the-line-height-calculation-quirk
    if (!style.MarginInlineEnd().IsZero() && !layout_object.GetDocument().InLineHeightQuirksMode()) {
        return false;
    }
...
InlineItem::InlineItem(InlineItemType type, unsigned start, unsigned end, LayoutObject* layout_object)
    : start_offset_(start)
    , end_offset_(end)
    ,
    // Use atomic construction to allow for concurrently marking InlineItem.
    layout_object_(layout_object, Member<LayoutObject>::AtomicInitializerTag {})
    , type_(type)
{
    DCHECK_GE(end, start);
...
InlineItem::InlineItem(const InlineItem& other, unsigned start, unsigned end, const ShapeResult* shape_result)
    : start_offset_(start)
    , end_offset_(end)
    ,
    // Use atomic construction to allow for concurrently marking InlineItem.
    shape_result_(shape_result, Member<ShapeResult>::AtomicInitializerTag {})
    , layout_object_(other.layout_object_, Member<LayoutObject>::AtomicInitializerTag {})
    , type_(other.type_)
...
InlineItem::InlineItem(const InlineItem& other)
    : InlineItem(other, other.start_offset_, other.end_offset_, other.shape_result_.Get())
{
}
...
void InlineItem::ComputeBoxProperties()
{
    ...
    if (type_ == InlineItem::kText || type_ == InlineItem::kAtomicInline || type_ == InlineItem::kControl) {
        return;
    }
...
void InlineItem::SetSegmentData(const RunSegmenter::RunSegmenterRange& range, HeapVector<InlineItem>* items)
{
...
unsigned InlineItem::SetBidiLevel(HeapVector<InlineItem>& items, unsigned index, unsigned end_offset, UBiDiLevel level)
{
    ...
    if (item->end_offset_ == end_offset) {
        ...
        while (index + 1 < items.size() && items[index + 1].Type() == InlineItem::kCloseTag) {
            items[++index].SetBidiLevel(level);
        }
    } else {
...
{
    if (const auto* svg_text = DynamicTo<LayoutSVGInlineText>(layout_object_.Get())) {
...
String InlineItem::ToString() const
{
    ...
    return String::Format("InlineItem %s. %s", InlineItemTypeToString(Type()), object_info.Ascii().c_str());
}
...
void InlineItem::Split(HeapVector<InlineItem>& items, unsigned index, unsigned offset)
{
...
void InlineItem::CheckTextType(const String& text_content) const
{
    ...
    default:
        ...
        DCHECK(TextType() == TextItemType::kNormal || TextType() == TextItemType::kSymbolMarker);
        break;
    }
}
#endif
