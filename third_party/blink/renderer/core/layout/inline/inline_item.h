...
public:
    ...
    InlineItem(InlineItemType type, unsigned start, unsigned end, LayoutObject* layout_object);
    ~InlineItem();

    // Copy constructor adjusting start/end and shape results.
    InlineItem(const InlineItem&, unsigned adjusted_start, unsigned adjusted_end, const ShapeResult*);
    InlineItem(const InlineItem&);

    InlineItemType Type() const
    {
        return type_;
    }
    ...
    {
        return is_empty_item_;
    }
    void SetIsEmptyItem(bool value)
    {
        is_empty_item_ = value;
    }
    ...
    void SetIsBlockLevel(bool value)
    {
        is_block_level_ = value;
    }
    ...
    {
        return static_cast<UBiDiLevel>(bidi_level_);
    }
    ...
    void SetEndCollapseType(CollapseType type)
    {
        ...
        DCHECK(Type() == InlineItem::kText ||
            ...
            ((Type() == InlineItem::kControl || Type() == InlineItem::kBlockInInline) && type == kCollapsible) ||
    ...
    {
        return Type() == InlineItem::kText && end_collapse_type_ == kCollapsible && Length() == 1u;
    }
    ...
    // fragments.
    bool IsGeneratedForLineBreak() const
    {
        return is_generated_for_line_break_;
    }
    void SetIsGeneratedForLineBreak()
    {
        is_generated_for_line_break_ = true;
    }

    // Whether the end collapsible space run contains a newline.
    // Valid only when kCollapsible or kCollapsed.
    bool IsEndCollapsibleNewline() const
    {
        return is_end_collapsible_newline_;
    }
    ...
    unsigned SegmentData() const
    {
        return segment_data_;
    }
    static void SetSegmentData(const RunSegmenter::RunSegmenterRange& range, HeapVector<InlineItem>* items);
    ...
    RunSegmenter::RunSegmenterRange CreateRunSegmenterRange() const
    {
        ...
        return InlineItemSegment::UnpackSegmentData(start_offset_, end_offset_, segment_data_);
    }
    ...
    static unsigned SetBidiLevel(HeapVector<InlineItem>&, unsigned index, unsigned end_offset, UBiDiLevel);
...
private:
    ...
    Member<const ShapeResult> shape_result_ { nullptr, Member<const ShapeResult>::AtomicInitializerTag {} };
...
{
    return (offset >= start_offset_ && offset < end_offset_) || (start_offset_ == end_offset_ && offset == start_offset_);
}
...
template <> struct VectorTraits<blink::InlineItem> : VectorTraitsBase<blink::InlineItem> {
