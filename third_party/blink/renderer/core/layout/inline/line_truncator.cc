...
void LineTruncator::SetupEllipsis()
{
    ...
    ellipsis_text_ = ellipsis_font_data_ && ellipsis_font_data_->GlyphForCharacter(kHorizontalEllipsisCharacter) ? String(&kHorizontalEllipsisCharacter, 1u)
                                                                                                                 : String(u"...");
    HarfBuzzShaper shaper(ellipsis_text_);
    ellipsis_shape_result_ = ShapeResultView::Create(shaper.Shape(&font, line_direction_));
    ellipsis_width_ = ellipsis_shape_result_->SnappedWidth();
}

LayoutUnit LineTruncator::PlaceEllipsisNextTo(LogicalLineItems* line_box, LogicalLineItem* ellipsized_child)
{
    // Create the ellipsis, associating it with the ellipsized child.
    DCHECK(ellipsized_child->HasInFlowFragment());
    const LayoutObject* ellipsized_layout_object = ellipsized_child->GetMutableLayoutObject();
    DCHECK(ellipsized_layout_object);
    DCHECK(ellipsized_layout_object->IsInline());
    DCHECK(ellipsized_layout_object->IsText() || ellipsized_layout_object->IsAtomicInlineLevel());

    // Now the offset of the ellpisis is determined. Place the ellpisis into the
    // line box.
    LayoutUnit ellipsis_inline_offset
        = IsLtr(line_direction_) ? ellipsized_child->InlineOffset() + ellipsized_child->inline_size : ellipsized_child->InlineOffset() - ellipsis_width_;
    FontHeight ellipsis_metrics;
    DCHECK(ellipsis_font_data_);
    if (ellipsis_font_data_) {
        ellipsis_metrics = ellipsis_font_data_->GetFontMetrics().GetFontHeight(line_style_->GetFontBaseline());
    }

    DCHECK(ellipsis_text_);
    DCHECK(ellipsis_shape_result_);
    line_box->AddChild(*ellipsized_layout_object, use_first_line_style_ ? StyleVariant::kFirstLineEllipsis : StyleVariant::kStandardEllipsis,
        ellipsis_shape_result_, ellipsis_text_, LogicalRect(ellipsis_inline_offset, -ellipsis_metrics.ascent, ellipsis_width_, ellipsis_metrics.LineHeight()),
        /* bidi_level */ 0);
    return ellipsis_inline_offset;
}

wtf_size_t LineTruncator::AddTruncatedChild(
    wtf_size_t source_index, bool leave_one_character, LayoutUnit position, TextDirection edge, LogicalLineItems* line_box, InlineLayoutStateStack* box_states)
{
    LogicalLineItems& line = *line_box;
    const LogicalLineItem& source_item = line[source_index];
    DCHECK(source_item.shape_result);
    const ShapeResult* shape_result = source_item.shape_result->CreateShapeResult();
    unsigned text_offset = shape_result->OffsetToFit(position, edge);
    if (IsLtr(edge) ? IsLeftMostOffset(*shape_result, text_offset) : IsRightMostOffset(*shape_result, text_offset)) {
        if (!leave_one_character)
            return kDidNotAddChild;
        text_offset
            = shape_result->OffsetToFit(shape_result->PositionForOffset(IsRtl(edge) == shape_result->IsRtl() ? 1 : shape_result->NumCharacters() - 1), edge);
    }
...
LayoutUnit LineTruncator::TruncateLine(LayoutUnit line_width, LogicalLineItems* line_box, InlineLayoutStateStack* box_states)
{
    ...
    if (IsLtr(line_direction_)) {
        ...
        for (auto& child : base::Reversed(*line_box)) {
            if (EllipsizeChild(line_width, ellipsis_width_, &child == first_child, &child, &truncated_child)) {
    ...
    if (truncated_child) {
        ...
        unsigned child_index_to_truncate = base::checked_cast<unsigned>(ellipsized_child - &*line_box->begin());
        line_box->InsertChild(child_index_to_truncate + 1, std::move(*truncated_child));
        ...
        if (IsRtl(line_direction_)) [[unlikely]] {
            ellipsized_child->rect.offset.inline_offset += child_to_truncate->inline_size - ellipsized_child->inline_size;
        }
    }

    // Create the ellipsis, associating it with the ellipsized child.
    LayoutUnit ellipsis_inline_offset = PlaceEllipsisNextTo(line_box, ellipsized_child);
...
LayoutUnit LineTruncator::TruncateLineInTheMiddle(LayoutUnit line_width, LogicalLineItems* line_box, InlineLayoutStateStack* box_states)
{
    // Shape the ellipsis and compute its inline size.
    SetupEllipsis();
    ...
    const LayoutUnit available_width = available_width_ - static_width_left - static_width_right;
    ...
    if (IsLtr(line_direction_)) {
        ...
        wtf_size_t new_index = AddTruncatedChild(index_left, index_left == initial_index_left, available_width_left, TextDirection::kLtr, line_box, box_states);
        if (new_index == kDidNotAddChild) {
            ...
            available_width_right += available_width_left - line[new_index].inline_size.ClampNegativeToZero();
        }
        ...
        LayoutUnit new_modified_right_offset = line[line.size() - 1].InlineOffset() + ellipsis_width_;
        DCHECK_LE(index_left, index_right);
        DCHECK(!line[index_right].IsPlaceholder());
        if (available_width_right > 0) {
            new_index = AddTruncatedChild(index_right, false, line[index_right].inline_size - available_width_right, TextDirection::kRtl, line_box, box_states);
            if (new_index != kDidNotAddChild) {
        ...
        // web-exposed.
        LayoutUnit offset_diff = line[index_right].InlineOffset() + line[index_right].inline_size - new_modified_right_offset;
        ...
        wtf_size_t new_index = AddTruncatedChild(
            index_right, index_right == initial_index_right, line[index_right].inline_size - available_width_right, TextDirection::kRtl, line_box, box_states);
        if (new_index == kDidNotAddChild) {
            ...
            line[new_index].rect.offset.inline_offset += line[index_right].inline_size - line[new_index].inline_size;
            PlaceEllipsisNextTo(line_box, &line[new_index]);
            available_width_left += available_width_right - line[new_index].inline_size.ClampNegativeToZero();
        }
        ...
        if (available_width_left > 0) {
            new_index = AddTruncatedChild(index_left, false, available_width_left, TextDirection::kLtr, line_box, box_states);
            if (new_index != kDidNotAddChild) {
                line[new_index].rect.offset.inline_offset = ellipsis_offset - line[new_index].inline_size;
            }
        }
        ...
        // It's ok to modify existing children's offsets because they are not
        // web-exposed.
        LayoutUnit offset_diff = line[line.size() - 1].InlineOffset() - line[index_left].InlineOffset();
...
bool LineTruncator::EllipsizeChild(
    LayoutUnit line_width, LayoutUnit ellipsis_width, bool is_first_child, LogicalLineItem* child, std::optional<LogicalLineItem>* truncated_child)
{
    ...
    LayoutUnit child_inline_offset = IsLtr(line_direction_) ? child->InlineOffset() : line_width - (child->InlineOffset() + child->inline_size);
...
bool LineTruncator::TruncateChild(
    LayoutUnit space_for_child, bool is_first_child, const LogicalLineItem& child, std::optional<LogicalLineItem>* truncated_child)
{
    DCHECK(truncated_child && !*truncated_child);
    ...
    unsigned offset_to_fit = shape_result->OffsetToFit(IsLtr(line_direction_) ? space_for_child : shape_result->Width() - space_for_child, line_direction_);
    ...
    *truncated_child = TruncateText(child, *shape_result, offset_to_fit, line_direction_);
    return true;
}

LogicalLineItem LineTruncator::TruncateText(const LogicalLineItem& item, const ShapeResult& shape_result, unsigned offset_to_fit, TextDirection direction)
{
    const TextOffsetRange new_text_offset = direction == shape_result.Direction() ? TextOffsetRange(item.StartOffset(), item.StartOffset() + offset_to_fit)
                                                                                  : TextOffsetRange(item.StartOffset() + offset_to_fit, item.EndOffset());
    const ShapeResultView* new_shape_result = ShapeResultView::Create(&shape_result, new_text_offset.start, new_text_offset.end);
