...
const PhysicalLineBoxFragment* FindBlockInInlineLineBoxFragment(Element* container)
{
    InlineCursor cursor(*To<LayoutBlockFlow>(container->GetLayoutObject()));
    for (cursor.MoveToFirstLine(); cursor; cursor.MoveToNextLine()) {
        const PhysicalLineBoxFragment* fragment = cursor.Current()->LineBoxFragment();
        DCHECK(fragment);
...
protected:
    ...
    PhysicalRect TextAreaFirstLineRect(const char* id)
    {
        ...
        InlineCursor cursor(*To<LayoutBlockFlow>(textarea->InnerEditorElement()->GetLayoutObject()));
        cursor.MoveToFirstLine();
        EXPECT_TRUE(cursor.IsNotNull());

        return PhysicalRect(cursor.Current().OffsetInContainerFragment(), cursor.Current().Size());
    }
};
...
TEST_F(InlineLayoutAlgorithmTest, Types)
{
    ...
    InlineCursor normal(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("normal")));
...
TEST_F(InlineLayoutAlgorithmTest, TypesForFirstLine)
{
    ...
    EXPECT_EQ(normal.Current()->LineBoxFragment()->GetStyleVariant(), StyleVariant::kFirstLine);

    InlineCursor empty(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("empty")));
    empty.MoveToFirstLine();
    EXPECT_TRUE(empty.Current()->LineBoxFragment()->IsEmptyLineBox());
    EXPECT_EQ(empty.Current().GetStyleVariant(), StyleVariant::kFirstLine);
    EXPECT_EQ(empty.Current()->LineBoxFragment()->GetStyleVariant(), StyleVariant::kFirstLine);
}
...
TEST_F(InlineLayoutAlgorithmTest, TypesForBlockInInline)
{
    ...
    InlineCursor block_in_inline(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("block-in-inline")));
    ...
    int block_count = 0;
    for (InlineCursor children = block_in_inline.CursorForDescendants(); children; children.MoveToNext()) {
        if (children.Current()->BoxFragment() && children.Current()->BoxFragment()->IsBlockInInline())
            ++block_count;
    }
    ...
    InlineCursor block_in_inline_empty(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("block-in-inline-empty")));
    block_in_inline_empty.MoveToFirstLine();
    block_in_inline_empty.MoveToNextLine();
    EXPECT_TRUE(block_in_inline_empty.Current()->LineBoxFragment()->IsEmptyLineBox());
    EXPECT_TRUE(block_in_inline_empty.Current()->LineBoxFragment()->IsBlockInInline());

    // Test empty but non-self-collapsing block in an inline box.
    InlineCursor block_in_inline_height(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("block-in-inline-height")));
    block_in_inline_height.MoveToFirstLine();
    block_in_inline_height.MoveToNextLine();
    EXPECT_FALSE(block_in_inline_height.Current()->LineBoxFragment()->IsEmptyLineBox());
    EXPECT_TRUE(block_in_inline_height.Current()->LineBoxFragment()->IsBlockInInline());
}
...
TEST_F(InlineLayoutAlgorithmTest, BreakToken)
{
    ...
    // Perform 1st Layout.
    auto* block_flow = To<LayoutBlockFlow>(GetLayoutObjectByElementId("container"));
    ...
    BoxFragmentBuilder container_builder(block_flow, block_flow->Style(), constraint_space, block_flow->Style()->GetWritingDirection());
    SimpleInlineChildLayoutContext context(inline_node, &container_builder);
    const LayoutResult* layout_result = inline_node.Layout(constraint_space, nullptr, nullptr, &context);
    ...
    const LayoutResult* layout_result2 = inline_node.Layout(constraint_space, line1.GetBreakToken(), nullptr, &context);
    ...
    const LayoutResult* layout_result3 = inline_node.Layout(constraint_space, line2.GetBreakToken(), nullptr, &context);
...
// out-of-flow object.
TEST_F(InlineLayoutAlgorithmTest, EmptyLineWithOutOfFlowInInlineContainingBlock)
{
    SetBodyInnerHTML(R"HTML(
    <!DOCTYPE html>
    <style>
    ...
    ASSERT_EQ(GetLayoutObjectByElementId("target"), off_container.Current().GetLayoutObject());
...
TEST_F(InlineLayoutAlgorithmTest, BoxForEndMargin)
{
    ...
    cursor.MoveToNext();
    ASSERT_TRUE(cursor);
    EXPECT_EQ(GetLayoutObjectByElementId("span"), cursor.Current().GetLayoutObject());
...
TEST_F(InlineLayoutAlgorithmTest, InlineBoxBorderPadding)
{
    ...
    int borders_and_padding1[] = { borders1.left.ToInt(), borders1.top.ToInt(), borders1.right.ToInt(), borders1.bottom.ToInt(), padding1.left.ToInt(),
        padding1.top.ToInt(), padding1.right.ToInt(), padding1.bottom.ToInt() };
    EXPECT_THAT(borders_and_padding1, testing::ElementsAre(1, 2, 0, 4, 5, 6, 0, 8));
    EXPECT_EQ(box1->ContentOffset(), PhysicalOffset(6, 8));
    EXPECT_EQ(item1.ContentOffsetInContainerFragment(), item1.OffsetInContainerFragment() + box1->ContentOffset());
    ...
    int borders_and_padding2[] = { borders2.left.ToInt(), borders2.top.ToInt(), borders2.right.ToInt(), borders2.bottom.ToInt(), padding2.left.ToInt(),
        padding2.top.ToInt(), padding2.right.ToInt(), padding2.bottom.ToInt() };
    EXPECT_THAT(borders_and_padding2, testing::ElementsAre(0, 2, 3, 4, 0, 6, 7, 8));
    EXPECT_EQ(box2->ContentOffset(), PhysicalOffset(0, 8));
    EXPECT_EQ(item2.ContentOffsetInContainerFragment(), item2.OffsetInContainerFragment() + box2->ContentOffset());
}
...
TEST_F(InlineLayoutAlgorithmTest, ContainerBorderPadding)
{
    ...
    const auto* layout_result = GetLayoutBoxByElementId("container")->GetSingleCachedLayoutResult();
...
TEST_F(InlineLayoutAlgorithmTest, MAYBE_VerticalAlignBottomReplaced)
{
    ...
    cursor.MoveToNext();
    ASSERT_TRUE(cursor);
    EXPECT_EQ(LayoutUnit(0), cursor.Current().OffsetInContainerFragment().top) << "Offset top of <img> should be zero.";
}
...
TEST_F(InlineLayoutAlgorithmTest, TextFloatsAroundFloatsBefore)
{
    ...
    const auto& html_fragment = To<LayoutBox>(GetDocument().getElementsByTagName(AtomicString("html"))->item(0)->GetLayoutObject())
...
TEST_F(InlineLayoutAlgorithmTest, TextFloatsAroundInlineFloatThatDoesNotFitOnLine)
{
    SetBodyInnerHTML(R"HTML(
    <!DOCTYPE html>
    <style>
...
// alignment rule.
TEST_F(InlineLayoutAlgorithmTest, FloatsArePositionedWithRespectToTopEdgeAlignmentRule)
{
    SetBodyInnerHTML(R"HTML(
    <!DOCTYPE html>
    <style>
...
TEST_F(InlineLayoutAlgorithmTest, BlockInInlineAppend)
{
    ...
    const PhysicalLineBoxFragment* before_append = FindBlockInInlineLineBoxFragment(container_element);
    ASSERT_TRUE(before_append);

    Document& doc = GetDocument();
    container_element->appendChild(doc.createTextNode("12345678"));
    UpdateAllLifecyclePhasesForTest();
    const PhysicalLineBoxFragment* after_append = FindBlockInInlineLineBoxFragment(container_element);
...
{Text 2-3 LTR Standard} "5,200 100x100"
        ...
        AsFragmentItemsString(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("root"))));

    EXPECT_EQ(R"DUMP(
{Line #descendants=2 LTR Standard} "0,0 100x100"
{Text 0-5 LTR Standard} "0,0 500x100"
)DUMP",
        AsFragmentItemsString(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("target")->SlowFirstChild())));
}
...
{Text 0-1 LTR Standard} "0,5 100x100"
        ...
        AsFragmentItemsString(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("target"))));
}
...
{Box #descendants=1 Standard} "0,0 0x0"
    ...
    EXPECT_EQ(expected, AsFragmentItemsString(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("sample"))));
}
...
TEST_F(InlineLayoutAlgorithmTest, InitialLetterWithEmptyInline)
{
    ...
    LoadAhem();
    InsertStyleElement("body { font: 20px/24px Ahem; }"
...
TEST_F(InlineLayoutAlgorithmTest, TextBoxTrimConstraintSpace)
{
    ...
    const TextBoxTrimResult empty_before { *GetLayoutBlockFlowByElementId("empty_before") };
    EXPECT_TRUE(empty_before.should_trim_start);
    EXPECT_FALSE(empty_before.should_trim_end);

    for (const char* id : { "nested_empty_before", "nested_empty_before_child", "middle", "nested_empty_after", "nested_empty_after_child" }) {
        const TextBoxTrimResult result { *GetLayoutBlockFlowByElementId(id) };
        EXPECT_FALSE(result.should_trim_start) << id;
        EXPECT_FALSE(result.should_trim_end) << id;
    }
    ...
    const TextBoxTrimResult empty_after { *GetLayoutBlockFlowByElementId("empty_after") };
