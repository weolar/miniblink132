...
void InlineItemsData::GetOpenTagItems(wtf_size_t start_index, wtf_size_t size, OpenTagItems* open_items) const
{
    DCHECK_LE(size, items.size());
    for (const InlineItem& item : base::make_span(items.data(), items.size()).subspan(start_index, size)) {
