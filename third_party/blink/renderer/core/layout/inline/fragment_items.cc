    ...
    , first_line_text_content_(std::move(builder->first_line_text_content_))
{
    ...
    std::transform(builder->items_.begin(), builder->items_.end(), std::back_inserter(items_), [](auto& item) { return std::move(item.item); });
}
    ...
    , items_(other.items_)
{
    for (const auto& other_item : other.items_) {
        ...
        // |FragmentItem|.
        if (auto* layout_text = DynamicTo<LayoutText>(other_item.GetMutableLayoutObject()))
...
{
    return span.empty() || (span.data() >= ItemsData() && !items_.empty() && &span.back() <= &items_.back());
}

void FragmentItems::FinalizeAfterLayout(const HeapVector<Member<const LayoutResult>, 1>& results, LayoutBlockFlow& container)
{
    struct LastItem {
    ...
    ClearCollectionScope<HeapHashMap<Member<const LayoutObject>, LastItem>> clear_scope(&last_items);
    ...
    for (const auto& result : results) {
        ...
        for (const FragmentItem& item : items) {
            ...
            const bool use_break_token = layout_object->IsFloating() || !layout_object->IsInline();
            ...
            const auto last_item_result = last_items.insert(layout_object, LastItem { &item, 0, item_index });
...
void FragmentItems::ClearAssociatedFragments(LayoutObject* container)
{
    ...
    for (LayoutObject* child = container->SlowFirstChild(); child; child = child->NextSibling()) {
        if (!child->IsInLayoutNGInlineFormattingContext() || child->IsOutOfFlowPositioned()) [[unlikely]] {
            continue;
        }
...
const FragmentItem* FragmentItems::EndOfReusableItems(const PhysicalBoxFragment& container) const
{
...
bool FragmentItems::IsContainerForCulledInline(
    const LayoutInline& layout_inline, bool* is_first_container, bool* is_last_container, bool* child_has_any_child_items) const
{
    ...
    *child_has_any_child_items = false;
    for (const LayoutObject* descendant = layout_inline.FirstChild(); descendant; descendant = next_descendant) {
        ...
        *child_has_any_child_items = true;
        ...
        if (item_idx < start_idx) {
            ...
            *is_first_container = false;
            InlineCursor cursor;
            for (cursor.MoveTo(*descendant); cursor.Current() && item_idx < end_idx; cursor.MoveToNextForSameLayoutObject()) {
...
// static
bool FragmentItems::TryDirtyFirstLineFor(const LayoutObject& layout_object, const LayoutBlockFlow& container)
{
...
// static
bool FragmentItems::TryDirtyLastLineFor(const LayoutObject& layout_object, const LayoutBlockFlow& container)
{
    DCHECK(layout_object.IsDescendantOf(&container));
    InlineCursor cursor(container);
    cursor.MoveTo(layout_object);
...
// static
void FragmentItems::DirtyLinesFromChangedChild(const LayoutObject& child, const LayoutBlockFlow& container)
{
    if (child.IsInLayoutNGInlineFormattingContext() && !child.IsFloatingOrOutOfFlowPositioned()) {
...
// static
void FragmentItems::DirtyLinesFromNeedsLayout(const LayoutBlockFlow& container)
{
    DCHECK(base::ranges::any_of(container.PhysicalFragments(), [](const PhysicalBoxFragment& fragment) { return fragment.HasItems(); }));
    ...
    for (LayoutObject* child = container.FirstChild(); child; child = child->NextSibling()) {
        ...
        // the container.
        if (child->NeedsLayout() || !IsParallelWritingMode(writing_mode, child->StyleRef().GetWritingMode())) {
...
bool FragmentItems::ReplaceBoxFragment(
    const PhysicalBoxFragment& old_fragment, const PhysicalBoxFragment& new_fragment, const PhysicalBoxFragment& containing_fragment)
{
...
// static
void FragmentItems::LayoutObjectWillBeDestroyed(const LayoutObject& layout_object)
{
    InlineCursor cursor;
    cursor.MoveTo(layout_object);
    for (; cursor; cursor.MoveToNextForSameLayoutObject()) {
