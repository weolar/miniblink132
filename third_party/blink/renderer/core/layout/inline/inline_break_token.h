...
struct RubyBreakTokenData : GarbageCollected<RubyBreakTokenData> {
    ...
    RubyBreakTokenData(wtf_size_t open_column_index, wtf_size_t base_end_index, const Vector<AnnotationBreakTokenData, 1>& annotations)
...
public:
    ...
    static InlineBreakToken* Create(InlineNode node, const ComputedStyle* style, const InlineItemTextIndex& start, unsigned flags /* InlineBreakTokenFlags */,
        const BlockBreakToken* sub_break_token = nullptr, const RubyBreakTokenData* ruby_data = nullptr);
    ...
    static InlineBreakToken* CreateForParallelBlockFlow(InlineNode node, const InlineItemTextIndex& start, const BlockBreakToken& child_break_token);
    ...
    wtf_size_t StartItemIndex() const
    {
        return start_.item_index;
    }
    wtf_size_t StartTextOffset() const
    {
    ...
    bool IsInParallelBlockFlow() const
    {
        return flags_ & kIsInParallelBlockFlow;
    }
    ...
    InlineBreakToken(PassKey, InlineNode node, const ComputedStyle*, const InlineItemTextIndex& start, unsigned flags /* InlineBreakTokenFlags */,
        const BlockBreakToken* sub_break_token, const RubyBreakTokenData* ruby_data);
...
inline bool InlineBreakToken::IsStartEqual(const InlineBreakToken* lhs, const InlineBreakToken* rhs)
{
    if (!lhs) {
        return !rhs;
    }
...
template <> struct DowncastTraits<InlineBreakToken> {
