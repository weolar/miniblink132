...
OffsetMappingBuilder::SourceNodeScope::SourceNodeScope(OffsetMappingBuilder* builder, const LayoutObject* node)
    : builder_(builder)
    , layout_object_auto_reset_(&builder->current_layout_object_, node)
    , appended_length_auto_reset_(&builder->current_offset_, GetAssociatedStartOffset(node))
{
...
void OffsetMappingBuilder::AppendIdentityMapping(unsigned length)
{
    ...
    mapping_units_.emplace_back(OffsetMappingUnitType::kIdentity, *current_layout_object_, dom_start, dom_end, text_content_start, text_content_end);
    has_open_unit_ = true;
}
...
void OffsetMappingBuilder::AppendCollapsedMapping(unsigned length)
{
    ...
    if (has_open_unit_ && mapping_units_.back().GetType() == OffsetMappingUnitType::kCollapsed) {
        DCHECK_EQ(mapping_units_.back().GetLayoutObject(), current_layout_object_);
        DCHECK_EQ(mapping_units_.back().DOMEnd(), dom_start);
        mapping_units_.back().dom_end_ += length;
        return;
    ...
    mapping_units_.emplace_back(OffsetMappingUnitType::kCollapsed, *current_layout_object_, dom_start, dom_end, text_content_start, text_content_end);
    has_open_unit_ = true;
}
...
void OffsetMappingBuilder::AppendVariableMapping(unsigned dom_length, unsigned text_content_length)
{
    ...
    mapping_units_.emplace_back(OffsetMappingUnitType::kVariable, *current_layout_object_, dom_start, dom_end, text_content_start, text_content_end);
...
void OffsetMappingBuilder::CollapseTrailingSpace(unsigned space_offset)
{
    ...
    if (offset_to_collapse) {
        new_units.emplace_back(OffsetMappingUnitType::kIdentity, layout_object, dom_offset, dom_offset + offset_to_collapse, text_content_offset,
            text_content_offset + offset_to_collapse);
        dom_offset += offset_to_collapse;
        text_content_offset += offset_to_collapse;
    }
    new_units.emplace_back(OffsetMappingUnitType::kCollapsed, layout_object, dom_offset, dom_offset + 1, text_content_offset, text_content_offset);
    ++dom_offset;
    if (dom_offset < container_unit->DOMEnd()) {
        new_units.emplace_back(
            OffsetMappingUnitType::kIdentity, layout_object, dom_offset, container_unit->DOMEnd(), text_content_offset, container_unit->TextContentEnd() - 1);
    }

    // TODO(xiaochengh): Optimize if this becomes performance bottleneck.
    wtf_size_t position = base::checked_cast<wtf_size_t>(std::distance(mapping_units_.data(), container_unit));
    ...
    while (new_unit_end && new_unit_end < mapping_units_.size() && mapping_units_[new_unit_end - 1].Concatenate(mapping_units_[new_unit_end])) {
        mapping_units_.EraseAt(new_unit_end);
    }
    while (position && position < mapping_units_.size() && mapping_units_[position - 1].Concatenate(mapping_units_[position])) {
        mapping_units_.EraseAt(position);
    }
}

void OffsetMappingBuilder::RestoreTrailingCollapsibleSpace(const LayoutText& layout_text, unsigned offset)
{
    ++destination_length_;
    for (auto& unit : base::Reversed(mapping_units_)) {
        ...
        if (unit.text_content_start_ != offset || unit.text_content_end_ != offset || unit.layout_object_ != layout_text) {
            ++unit.text_content_start_;
            ++unit.text_content_end_;
            continue;
        }
        ...
        mapping_units_.insert(base::checked_cast<wtf_size_t>(std::distance(mapping_units_.data(), &unit) + 1),
            OffsetMappingUnit(OffsetMappingUnitType::kCollapsed, layout_text, unit.dom_end_, original_dom_end, unit.text_content_end_, unit.text_content_end_));
        return;
    }
    NOTREACHED();
}
...
OffsetMapping* OffsetMappingBuilder::Build()
{
    ...
    for (unsigned range_start = 0; range_start < mapping_units_.size();) {
        const LayoutObject& layout_object = mapping_units_[range_start].GetLayoutObject();
        ...
        if (node) {
            ...
            while (range_end < mapping_units_.size() && mapping_units_[range_end].GetLayoutObject() == layout_object)
                ++range_end;
        }
        range_start = range_end;
    }

    return MakeGarbageCollected<OffsetMapping>(std::move(mapping_units_), std::move(unit_ranges_), destination_string_);
}
