...
template <typename Span1, typename Span2> unsigned MismatchInternal(const Span1& span1, const Span2& span2)
{
...
template <typename Span1, typename Span2> unsigned MismatchFromEnd(const Span1& span1, const Span2& span2)
{
    const auto old_new = base::ranges::mismatch(base::Reversed(span1), base::Reversed(span2));
...
{
    return std::accumulate(data.items.begin(), data.items.end(), 0.0f, [](float sum, const InlineItem& item) {
        DCHECK(item.Type() == InlineItem::kText || item.Type() == InlineItem::kBidiControl || item.Type() == InlineItem::kControl) << item.Type();
...
{
    unsigned count = 0;
    for (LayoutObject* child = block.FirstChild(); child; child = child->NextSibling()) {
        ++count;
    }
    return count * 4;
}
...
public:
    ReusingTextShaper(InlineItemsData* data, const HeapVector<InlineItem>* reusable_items, const bool allow_shape_cache)
        : data_(*data)
    ...
    void SetOptions(ShapeOptions options)
    {
        options_ = options;
    }

    const ShapeResult* Shape(const InlineItem& start_item, const Font& font, unsigned end_offset)
    {
        auto ShapeFunc = [&]() -> const ShapeResult* { return ShapeWithoutCache(start_item, font, end_offset); };
        if (allow_shape_cache_) {
            DCHECK(RuntimeEnabledFeatures::LayoutNGShapeCacheEnabled());
            return font.GetNGShapeCache().GetOrCreate(shaper_.GetText(), start_item.Direction(), ShapeFunc);
        }
        return ShapeFunc();
    }

private:
    const ShapeResult* ShapeWithoutCache(const InlineItem& start_item, const Font& font, unsigned end_offset)
    {
        ...
        HeapVector<Member<const ShapeResult>> reusable_shape_results = CollectReusableShapeResults(start_offset, end_offset, font, start_item.Direction());
        ClearCollectionScope clear_scope(&reusable_shape_results);

        if (reusable_shape_results.empty())
            return Reshape(start_item, font, start_offset, end_offset);

        ShapeResult* shape_result = ShapeResult::CreateEmpty(*reusable_shape_results.front());
        ...
        for (const ShapeResult* reusable_shape_result : reusable_shape_results) {
            ...
            if (offset < reusable_shape_result->StartIndex()) {
                AppendShapeResult(*Reshape(start_item, font, offset, reusable_shape_result->StartIndex()), shape_result);
                offset = shape_result->EndIndex();
                options_.han_kerning_start = false;
            }
            DCHECK_LT(offset, reusable_shape_result->EndIndex());
            DCHECK(shape_result->NumCharacters() == 0 || shape_result->EndIndex() == offset);
            reusable_shape_result->CopyRange(offset, std::min(reusable_shape_result->EndIndex(), end_offset), shape_result);
            offset = shape_result->EndIndex();
            if (offset == end_offset)
                return shape_result;
        }
        DCHECK_LT(offset, end_offset);
        AppendShapeResult(*Reshape(start_item, font, offset, end_offset), shape_result);
        return shape_result;
    }

    void AppendShapeResult(const ShapeResult& shape_result, ShapeResult* target)
    {
        DCHECK(target->NumCharacters() == 0 || target->EndIndex() == shape_result.StartIndex());
        shape_result.CopyRange(shape_result.StartIndex(), shape_result.EndIndex(), target);
    }

    HeapVector<Member<const ShapeResult>> CollectReusableShapeResults(unsigned start_offset, unsigned end_offset, const Font& font, TextDirection direction)
    {
        DCHECK_LT(start_offset, end_offset);
        HeapVector<Member<const ShapeResult>> shape_results;
        if (!reusable_items_)
            return shape_results;
        for (auto item = std::lower_bound(reusable_items_->begin(), reusable_items_->end(), start_offset,
                 [](const InlineItem& item, unsigned offset) { return item.EndOffset() <= offset; });
    ...
    const ShapeResult* Reshape(const InlineItem& start_item, const Font& font, unsigned start_offset, unsigned end_offset)
    {
        DCHECK_LT(start_offset, end_offset);
        const TextDirection direction = start_item.Direction();
        if (data_.segments) {
            return data_.segments->ShapeText(&shaper_, &font, direction, start_offset, end_offset, data_.ToItemIndex(start_item), options_);
        }
        RunSegmenter::RunSegmenterRange range = start_item.CreateRunSegmenterRange();
        range.end = end_offset;
        return shaper_.Shape(&font, direction, start_offset, end_offset, range, options_);
    }
...
template <typename ItemsBuilder> void CollectInlinesInternal(ItemsBuilder* builder, const InlineNodeData* previous_data)
{
    LayoutBlockFlow* const block = builder->GetLayoutBlockFlow();
    builder->EnterBlock(block->Style());
    LayoutObject* node = GetLayoutObjectForFirstChildNode(block);

    const LayoutObject* symbol = LayoutListItem::FindSymbolMarkerLayoutText(block);
    ...
    while (node) {
        if (auto* counter = DynamicTo<LayoutCounter>(node)) {
            ...
            if (node->IsLayoutOutsideListMarker()) {
                ...
                builder->AppendOpaque(InlineItem::kInitialLetterBox, kObjectReplacementCharacter, node);
            ...
            if (auto* inline_list_item = DynamicTo<LayoutInlineListItem>(node)) {
                ...
                inline_list_item_marker = LayoutListItem::FindSymbolMarkerLayoutText(inline_list_item);
            }
...
inline bool ShouldBreakShapingBeforeText(
    const InlineItem& item, const InlineItem& start_item, const ComputedStyle& start_style, const Font& start_font, TextDirection start_direction)
{
    ...
    return item.Direction() != start_direction || !item.EqualsRunSegment(start_item);
}
...
inline bool ShouldBreakShapingBeforeBox(const InlineItem& item)
{
    ...
    if ((style.MayHavePadding() && !style.PaddingInlineStart().IsZero()) || (style.MayHaveMargin() && !style.MarginInlineStart().IsZero())
        || style.BorderInlineStartWidth() || style.VerticalAlign() != EVerticalAlign::kBaseline) {
        return true;
    }
...
inline bool ShouldBreakShapingAfterBox(const InlineItem& item)
{
    ...
    // These properties values must break shaping.
    // https://drafts.csswg.org/css-text-3/#boundary-shaping
    if ((style.MayHavePadding() && !style.PaddingInlineEnd().IsZero()) || (style.MayHaveMargin() && !style.MarginInlineEnd().IsZero())
        || style.BorderInlineEndWidth() || style.VerticalAlign() != EVerticalAlign::kBaseline) {
        return true;
    }
...
bool FirstLineNeedsReshape(const ComputedStyle& first_line_style, const ComputedStyle& base_style)
{
...
bool SetParagraphTo(const String& text, const ComputedStyle& block_style, BidiParagraph& bidi)
{
...
{
    const InlineNodeData* data = To<LayoutBlockFlow>(box_.Get())->GetInlineNodeData();
...
void InlineNode::PrepareLayout(InlineNodeData* previous_data) const
{
    ...
    ShapeTextIncludingFirstLine(data, previous_data ? &previous_data->text_content : nullptr, nullptr);
...
public:
    ...
    {
        if (!block_flow_ || block_flow_->NeedsCollectInlines() || block_flow_->NeedsLayout() || block_flow_->GetDocument().NeedsLayoutTreeUpdate()
            || !block_flow_->GetInlineNodeData() || block_flow_->GetInlineNodeData()->text_content.IsNull()
        ...
        const OffsetMapping* const offset_mapping = InlineNode::GetOffsetMapping(block_flow_);
    ...
    void Run()
    {
        ...
        const auto [start_offset, end_match_length] = MatchedLengths(old_text, new_text);
        ...
        while (it != end) {
            ...
            if (it->start_offset_ < start_offset) {
                ...
                if (new_item.EndOffset() < start_offset) {
                    items.push_back(InlineItem(*it, new_item.EndOffset(), start_offset, nullptr));
                }
            }
            ...
            if (end_offset < it->end_offset_) {
                ...
                if (end_offset < new_item.StartOffset()) {
                    items.push_back(InlineItem(*it, end_offset, new_item.StartOffset(), nullptr));
        ...
        if (items.empty()) {
            items.push_back(InlineItem(data_->items.front(), 0, new_data.text_content.length(), nullptr));
        } else if (items.back().end_offset_ < new_data.text_content.length()) {
            items.push_back(InlineItem(data_->items.back(), items.back().end_offset_, new_data.text_content.length(), nullptr));
        }
...
private:
    ...
    // and from the end.
    std::pair<unsigned, unsigned> MatchedLengths(const String& old_text, const String& new_text) const
    {
        ...
        const unsigned old_length = old_text.length();
        const unsigned new_length = new_text.length();
        const unsigned max_end_length = std::min(old_length - start_match_length, new_length - start_match_length);
        const unsigned end_match_length = MismatchFromEnd(StringView(old_text, old_length - max_end_length), StringView(new_text, new_length - max_end_length));
    ...
    InlineItem CopyItemAfter(const InlineItem& item, unsigned start_offset) const
    {
        ...
        return InlineItem(item, safe_start_offset, end_offset, item.shape_result_->SubRange(safe_start_offset, end_offset));
    }
    ...
    InlineItem CopyItemBefore(const InlineItem& item, unsigned end_offset) const
    {
        ...
        return InlineItem(item, start_offset, safe_end_offset, item.shape_result_->SubRange(start_offset, safe_end_offset));
    }

    // See also |GetLastSafeToReuse()|.
    unsigned GetFirstSafeToReuse(const InlineItem& item, unsigned start_offset) const
    {
        DCHECK_LE(item.start_offset_, start_offset);
        ...
        if (!item.shape_result_ || item.shape_result_->IsAppliedSpacing() || start_offset + skip >= end_offset)
            return end_offset;
    ...
    unsigned GetLastSafeToReuse(const InlineItem& item, unsigned end_offset) const
    {
        DCHECK_LT(item.start_offset_, end_offset);
        DCHECK_LE(end_offset, item.end_offset_);
        const unsigned start_offset = item.start_offset_;
        ...
        const unsigned skip = max_context - 1;
        if (!item.shape_result_ || item.shape_result_->IsAppliedSpacing() || end_offset <= start_offset + skip)
            return start_offset;
        item.shape_result_->EnsurePositionData();
        // TODO(yosin): It is better to utilize OpenType |usMaxContext|.
        ...
        return item.shape_result_->CachedPreviousSafeToBreakOffset(end_offset - skip);
    }
    ...
    static void ShiftItem(InlineItem* item, int delta)
    {
        ...
        if (!item->shape_result_)
            return;
        item->shape_result_ = item->shape_result_->CopyAdjustedOffset(item->start_offset_);
    }
    ...
    void VerifyItems(const HeapVector<InlineItem>& items) const
    {
        ...
        DCHECK_EQ(last_offset, block_flow_->GetInlineNodeData()->text_content.length());
#endif
    }
...
// static
bool InlineNode::SetTextWithOffset(LayoutText* layout_text, String new_text, const TextDiffRange& diff)
{
    if (!layout_text->HasValidInlineItems() || !layout_text->IsInLayoutNGInlineFormattingContext())
        return false;
    ...
    InlineItemsBuilder builder(editor.GetLayoutBlockFlow(), &data->items, previous_data ? previous_data->text_content : String());
    ...
    node.ShapeTextIncludingFirstLine(data, &previous_data->text_content, &previous_data->items);
...
{
#if DCHECK_IS_ON()
    DCHECK(!GetLayoutBlockFlow()->GetDocument().NeedsLayoutTreeUpdate() || GetLayoutBlockFlow()->IsInDetachedNonDomTree());
#endif
...
void InlineNode::ComputeOffsetMapping(LayoutBlockFlow* layout_block_flow, InlineNodeData* data)
{
#if DCHECK_IS_ON()
    DCHECK(!data->offset_mapping);
    DCHECK(!layout_block_flow->GetDocument().NeedsLayoutTreeUpdate() || layout_block_flow->IsInDetachedNonDomTree());
#endif
    ...
    InlineItemsBuilderForOffsetMapping builder(layout_block_flow, &items, data->text_content, chunk_offsets);
    builder.GetOffsetMappingBuilder().ReserveCapacity(EstimateOffsetMappingItemsCount(*layout_block_flow));
...
const OffsetMapping* InlineNode::GetOffsetMapping(LayoutBlockFlow* layout_block_flow)
{
...
void InlineNode::CollectInlines(InlineNodeData* data, InlineNodeData* previous_data) const
{
    DCHECK(data->text_content.IsNull());
    ...
    if (block->IsSVGText()) {
        ...
        bool empty_or_one_char = !block->FirstChild() || (layout_text && !layout_text->NextSibling() && layout_text->TransformedTextLength() <= 1);
        if (!empty_or_one_char)
            chunk_offsets = FindSvgTextChunks(*block, *data);
    }

    data->items.reserve(EstimateInlineItemsCount(*block));
    InlineItemsBuilder builder(block, &data->items, previous_data ? previous_data->text_content : String(), chunk_offsets);
    CollectInlinesInternal(&builder, previous_data);
...
const SvgTextChunkOffsets* InlineNode::FindSvgTextChunks(LayoutBlockFlow& block, InlineNodeData& data) const
{
    ...
    OffsetMappingBuilder& mapping_builder = items_builder.GetOffsetMappingBuilder();
    ...
    for (wtf_size_t i = 0; i < data.svg_node_data_->character_data_list.size(); ++i) {
        const std::pair<unsigned, SvgCharacterData>& char_data = data.svg_node_data_->character_data_list[i];
        ...
        DCHECK(unit);
        auto result = data.svg_node_data_->chunk_offsets.insert(To<LayoutText>(&unit->GetLayoutObject()), Vector<unsigned>());
        result.stored_value->value.push_back(unit->ConvertTextContentToFirstDOMOffset(text_content_offset));
    }
    return data.svg_node_data_->chunk_offsets.size() > 0 ? &data.svg_node_data_->chunk_offsets : nullptr;
}

void InlineNode::SegmentText(InlineNodeData* data, InlineNodeData* previous_data) const
{
...
void InlineNode::SegmentScriptRuns(InlineNodeData* data, InlineNodeData* previous_data) const
{
    ...
    if (previous_data && text_content == previous_data->text_content) {
        if (!previous_data->segments) {
            const auto it = base::ranges::find_if(previous_data->items, [](const auto& item) { return item.Type() == InlineItem::kText; });
    ...
    if ((text_content.Is8Bit() || !data->HasNonOrc16BitCharacters()) && !data->is_bidi_enabled_) {
        if (data->items.size()) {
            RunSegmenter::RunSegmenterRange range
                = { 0u, data->text_content.length(), USCRIPT_LATIN, OrientationIterator::kOrientationKeep, FontFallbackPriority::kText };
...
void InlineNode::SegmentFontOrientation(InlineNodeData* data) const
{
    ...
    for (const InlineItem& item : items) {
        if (item.Type() == InlineItem::kText && item.Length()
            && item.Style()->GetFont().GetFontDescription().Orientation() == FontOrientation::kVerticalMixed) {
            if (!segments) {
                data->segments = std::make_unique<InlineItemSegments>();
            ...
            segment_index = segments->AppendMixedFontOrientation(text_content, item.StartOffset(), item.EndOffset(), segment_index);
        }
    }
}
...
bool InlineNode::IsNGShapeCacheAllowed(
    const String& text_content, const Font* override_font, const HeapVector<InlineItem>& items, ShapeResultSpacing<String>& spacing) const
{
    ...
    if (!(single_item.Type() == InlineItem::kText && single_item.StartOffset() == 0 && single_item.EndOffset() == text_content.length())) {
        return false;
    }
    const Font& font = override_font ? *override_font : single_item.FontWithSvgScaling();
    return !spacing.SetSpacing(font.GetFontDescription());
}

void InlineNode::ShapeText(InlineItemsData* data, const String* previous_text, const HeapVector<InlineItem>* previous_items, const Font* override_font) const
{
    ...
    base::ScopedClosureRunner scoped_closure_runner(WTF::BindOnce(
        ...
        base::ElapsedTimer(), WrapWeakPersistent(GetLayoutBox() ? &GetLayoutBox()->GetDocument() : nullptr)));
    ...
    const bool allow_shape_cache = IsNGShapeCacheAllowed(text_content, override_font, *items, spacing) && !auto_space.MayApply();
    ...
    DCHECK(!data->segments || data->segments->EndOffset() == text_content.length());
    ...
    for (unsigned index = 0; index < items->size();) {
        ...
        const Font& font = override_font ? *override_font : start_item.FontWithSvgScaling();
#if DCHECK_IS_ON()
        if (!IsTextCombine()) {
            DCHECK(!override_font);
        } else {
            DCHECK_EQ(font.GetFontDescription().Orientation(), FontOrientation::kHorizontal);
            LayoutTextCombine::AssertStyleIsValid(start_style);
            DCHECK(!override_font || font.GetFontDescription().WidthVariant() != kRegularWidth);
        }
#endif
        shaper.SetOptions({
            .is_line_start = is_next_start_of_paragraph,
            .han_kerning_start = is_next_start_of_paragraph && ShouldTrimStartOfParagraph(font.GetFontDescription().GetTextSpacingTrim())
                && Character::MaybeHanKerningOpen(text_content[start_item.StartOffset()]),
        });
        ...
        if (start_item.IsSymbolMarker()) [[unlikely]] {
            LayoutUnit symbol_width = ListMarker::WidthOfSymbol(start_style, LayoutCounter::ListStyle(start_item.GetLayoutObject(), start_style));
            DCHECK_GE(symbol_width, 0);
            start_item.shape_result_ = ShapeResult::CreateForSpaces(&font, direction, start_item.StartOffset(), start_item.Length(), symbol_width);
            index++;
            continue;
        }
        ...
        for (; end_index < items->size(); end_index++) {
            ...
            if (item.Type() == InlineItem::kText) {
                ...
                if (ShouldBreakShapingBeforeText(item, start_item, start_style, font, direction)) {
                    break;
                }
        ...
        if (previous_text && end_offset == start_item.EndOffset() && !NeedsShaping(start_item)) {
            if (!IsTextCombine()) [[likely]] {
                DCHECK_EQ(start_item.StartOffset(), start_item.TextShapeResult()->StartIndex());
                DCHECK_EQ(start_item.EndOffset(), start_item.TextShapeResult()->EndIndex());
                index++;
                continue;
            }
        }
        ...
        if (previous_text) {
            ...
            if (has_valid_shape_results && previous_text && end_offset <= previous_text->length()
                && StringView(text_content, text_start, text_length) == StringView(*previous_text, text_start, text_length)) {
                index = end_index;
                continue;
            }
        }
        ...
        if (end_offset == start_item.EndOffset()) {
            ...
            DCHECK_EQ(start_item.TextShapeResult()->StartIndex(), start_item.StartOffset());
            DCHECK_EQ(start_item.TextShapeResult()->EndIndex(), start_item.EndOffset());
            index++;
            continue;
        }
        ...
        const bool has_ligatures = shape_result->NumGlyphs() < shape_result->NumCharacters();
        if (has_ligatures) {
        ...
        for (; index < end_index; index++) {
            ...
            text_item_ranges.emplace_back(item.StartOffset(), item.EndOffset(), item_result);
            if (has_ligatures && item.EndOffset() < shape_result->EndIndex()
                && shape_result->CachedNextSafeToBreakOffset(item.EndOffset()) != item.EndOffset()) {
...
void InlineNode::ShapeTextIncludingFirstLine(InlineNodeData* data, const String* previous_text, const HeapVector<InlineItem>* previous_items) const
{
...
const LayoutResult* InlineNode::Layout(const ConstraintSpace& constraint_space, const BreakToken* break_token, const ColumnSpannerPath* column_spanner_path,
    InlineChildLayoutContext* context) const
{
    PrepareLayoutIfNeeded();

    const auto* inline_break_token = To<InlineBreakToken>(break_token);
    InlineLayoutAlgorithm algorithm(*this, constraint_space, inline_break_token, column_spanner_path, context);
    return algorithm.Layout();
}

namespace {

template <typename CharType> String CreateTextContentForStickyImagesQuirk(const CharType* text, unsigned length, base::span<const InlineItem> items)
{
...
// static
String InlineNode::TextContentForStickyImagesQuirk(const InlineItemsData& items_data)
{
    ...
    for (unsigned i = 0; i < items_data.items.size(); ++i) {
        ...
        if (item.Type() == InlineItem::kAtomicInline && item.IsImage()) {
            auto item_span = base::span(items_data.items.data(), items_data.items.size()).subspan(i);
            if (text_content.Is8Bit()) {
                return CreateTextContentForStickyImagesQuirk(text_content.Characters8(), text_content.length(), item_span);
            }
            return CreateTextContentForStickyImagesQuirk(text_content.Characters16(), text_content.length(), item_span);
        }
    }
    return text_content;
}

static LayoutUnit ComputeContentSize(InlineNode node, WritingMode container_writing_mode, const ConstraintSpace& space,
    const MinMaxSizesFloatInput& float_input, LineBreakerMode mode, LineBreaker::MaxSizeCache* max_size_cache, std::optional<LayoutUnit>* max_size_out,
    bool* depends_on_block_constraints_out)
{
    const ComputedStyle& style = node.Style();
    LayoutUnit available_inline_size = mode == LineBreakerMode::kMaxContent ? LayoutUnit::Max() : LayoutUnit();
    ...
    LayoutUnit result;
    LineBreaker line_breaker(node, mode, space, line_opportunity, empty_leading_floats,
        /* break_token */ nullptr,
    ...
    public:
        explicit FloatsMaxSize(const MinMaxSizesFloatInput& float_input)
            : floats_inline_size_(float_input.float_left_inline_size + float_input.float_right_inline_size)
        {
            DCHECK_GE(floats_inline_size_, 0);
        }

        void AddFloat(const ComputedStyle& float_style, const ComputedStyle& style, LayoutUnit float_inline_max_size_with_margin)
        {
            floating_objects_.push_back(InlineNode::FloatingObject { float_style, style, float_inline_max_size_with_margin });
        }

        LayoutUnit ComputeMaxSizeForLine(LayoutUnit line_inline_size, LayoutUnit max_inline_size)
        {
            ...
            for (const auto& floating_object : floating_objects_) {
                const EClear float_clear = floating_object.float_style->Clear(*floating_object.style);
                ...
                if ((previous_float_type == EFloat::kLeft && (float_clear == EClear::kBoth || float_clear == EClear::kLeft))
                    || (previous_float_type == EFloat::kRight && (float_clear == EClear::kBoth || float_clear == EClear::kRight))) {
                    max_inline_size = std::max(max_inline_size, line_inline_size + floats_inline_size_);
                ...
                floats_inline_size_ += floating_object.float_inline_max_size_with_margin.ClampNegativeToZero();
                previous_float_type = floating_object.float_style->Floating(*floating_object.style);
            }
            max_inline_size = std::max(max_inline_size, line_inline_size + floats_inline_size_);
            floats_inline_size_ = LayoutUnit();
    ...
    public:
        ...
        explicit MaxSizeFromMinSize(const InlineItemsData& items_data, const LineBreaker::MaxSizeCache& max_size_cache, FloatsMaxSize* floats)
        ...
        void AddTextUntil(ItemIterator end)
        {
            for (; next_item != end; ++next_item) {
                if (next_item->Type() == InlineItem::kOpenTag && next_item->GetLayoutObject()->IsInlineRubyText()) {
                    ++annotation_nesting_level;
                } else if (next_item->Type() == InlineItem::kCloseTag && next_item->GetLayoutObject()->IsInlineRubyText()) {
                    --annotation_nesting_level;
                } else if (next_item->Type() == InlineItem::kText && next_item->Length() && annotation_nesting_level == 0) {
        ...
        void ForceLineBreak(const LineInfo& line_info)
        {
            ...
            max_size = floats->ComputeMaxSizeForLine(position.ClampNegativeToZero(), max_size);
            position = LayoutUnit();
            is_after_break = true;
        }
        ...
        void AddTabulationCharacters(const InlineItem& item, unsigned length)
        {
            ...
            TextRunLayoutUnit glyph_advance = TextRunLayoutUnit::FromFloatRound(font.TabWidth(font_data, tab_size, position));
            InlineLayoutUnit run_advance = glyph_advance;
            DCHECK_GE(length, 1u);
            if (length > 1u) {
                glyph_advance = TextRunLayoutUnit::FromFloatRound(font.TabWidth(font_data, tab_size));
        ...
        {
            AddTextUntil(end);
            return floats->ComputeMaxSizeForLine(position.ClampNegativeToZero(), max_size);
        }
...
#endif
                if (item.Type() == InlineItem::kAtomicInline || item.Type() == InlineItem::kBlockInInline) {
    ...
    if (node.IsInitialLetterBox()) [[unlikely]] {
        ...
        do {
            ...
            if (line_info.Results().empty())
                break;
            inline_size = std::max(CalculateInitialLetterBoxInlineSize(line_info), inline_size);
    ...
    MaxSizeFromMinSize max_size_from_min_size(items_data, *max_size_cache, &floats_max_size);
    ...
    do {
        ...
        for (const InlineItemResult& item_result : line_info.Results()) {
            ...
            builder.SetPercentageResolutionBlockSize(space.PercentageResolutionBlockSize());
            builder.SetReplacedPercentageResolutionBlockSize(space.ReplacedPercentageResolutionBlockSize());
            const auto float_space = builder.ToConstraintSpace();

            const MinMaxSizesResult child_result = ComputeMinAndMaxContentContribution(style, float_node, float_space);
            LayoutUnit child_inline_margins = ComputeMarginsFor(float_space, float_node.Style(), space).InlineSum();
            ...
            if (mode == LineBreakerMode::kMinContent) {
                result = std::max(result, child_result.sizes.min_size + child_inline_margins);
            }
            floats_max_size.AddFloat(float_node.Style(), style, child_result.sizes.max_size + child_inline_margins);
        }
        ...
        if (mode == LineBreakerMode::kMinContent) {
            ...
            can_compute_max_size_from_min_size = can_compute_max_size_from_min_size &&
                ...
                // min-content.
                !line_breaker.HasClonedBoxDecorations() && !line_info.MayHaveRubyOverhang();
    ...
    if (mode == LineBreakerMode::kMinContent && can_compute_max_size_from_min_size) {
        ...
        *max_size_out = max_size_from_min_size.Finish(items_data.items.end());
        ...
        LayoutUnit content_size
            = ComputeContentSize(node, container_writing_mode, space, float_input, LineBreakerMode::kMaxContent, max_size_cache, nullptr, nullptr);
        bool values_might_be_saturated = (*max_size_out)->MightBeSaturated() || content_size.MightBeSaturated();
        if (!values_might_be_saturated) {
            DCHECK_EQ((*max_size_out)->Round(), content_size.Round()) << node.GetLayoutBox();
        }
#endif
    }
...
MinMaxSizesResult InlineNode::ComputeMinMaxSizes(
    WritingMode container_writing_mode, const ConstraintSpace& space, const MinMaxSizesFloatInput& float_input) const
{
    PrepareLayoutIfNeeded();
    ...
    sizes.min_size = ComputeContentSize(
        *this, container_writing_mode, space, float_input, LineBreakerMode::kMinContent, &max_size_cache, &max_size, &depends_on_block_constraints);
    if (max_size) {
        sizes.max_size = *max_size;
    } else {
        sizes.max_size = ComputeContentSize(*this, container_writing_mode, space, float_input, LineBreakerMode::kMaxContent, &max_size_cache, nullptr, nullptr);
    }
...
{
    return GetLayoutBox() && GetLayoutBox()->GetDocument().GetStyleEngine().UsesFirstLineRules();
}
...
void InlineNode::CheckConsistency() const
{
    ...
    for (const InlineItem& item : items) {
        DCHECK(!item.GetLayoutObject() || !item.Style() || item.Style() == item.GetLayoutObject()->Style());
    }
#endif
}

const Vector<std::pair<unsigned, SvgCharacterData>>& InlineNode::SvgCharacterDataList() const
{
    DCHECK(IsSvgText());
    return Data().svg_node_data_->character_data_list;
}

const HeapVector<SvgTextContentRange>& InlineNode::SvgTextLengthRangeList() const
{
    DCHECK(IsSvgText());
    return Data().svg_node_data_->text_length_range_list;
}

const HeapVector<SvgTextContentRange>& InlineNode::SvgTextPathRangeList() const
{
    DCHECK(IsSvgText());
