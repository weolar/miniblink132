...
using LineBoxPair = std::pair<const PhysicalLineBoxFragment*, const PhysicalLineBoxFragment*>;
...
void GatherInlineContainerFragmentsFromItems(const Items& items, const PhysicalOffset& box_offset,
    InlineContainingBlockUtils::InlineContainingBlockMap* inline_containing_block_map,
    HeapHashMap<Member<const LayoutObject>, LineBoxPair>* containing_linebox_map, const WritingModeConverter* fragment_converter = nullptr,
    const WritingModeConverter* containing_block_converter = nullptr)
{
    ...
    for (const auto& item : items) {
        ...
        if (const PhysicalLineBoxFragment* current_linebox = item->LineBoxFragment()) {
        ...
        std::optional<InlineContainingBlockUtils::InlineContainingBlockGeometry>& containing_block_geometry = it->value;
        LineBoxPair& containing_lineboxes = containing_linebox_map->insert(key, LineBoxPair { nullptr, nullptr }).stored_value->value;
        DCHECK(containing_block_geometry.has_value() || !containing_lineboxes.first);
        ...
        if (fragment_converter) {
            ...
            fragment_rect.offset
                = containing_block_converter->ToPhysical(fragment_converter->ToLogical(fragment_rect.offset, fragment_rect.size), fragment_rect.size);
        }
        ...
        if (containing_lineboxes.first == linebox) {
            ...
            containing_block_geometry
                = InlineContainingBlockUtils::InlineContainingBlockGeometry { fragment_rect, fragment_rect, containing_block_geometry->relative_offset };
        }
...
void InlineContainingBlockUtils::ComputeInlineContainerGeometry(InlineContainingBlockMap* inline_containing_block_map, BoxFragmentBuilder* container_builder)
{
    ...
    if (container_builder->ItemsBuilder()) {
        ...
        // coordinate space.
        DCHECK_EQ(container_builder->ItemsBuilder()->GetWritingMode(), container_builder->GetWritingMode());
        DCHECK_EQ(container_builder->ItemsBuilder()->Direction(), container_builder->Direction());
        GatherInlineContainerFragmentsFromItems(
            container_builder->ItemsBuilder()->Items(ToPhysicalSize(container_builder->Size(), container_builder->GetWritingMode())), PhysicalOffset(),
    ...
    for (const auto& child : container_builder->Children()) {
        ...
        const PhysicalOffset child_offset = child.offset.ConvertToPhysical(
            container_builder->GetWritingDirection(), ToPhysicalSize(container_builder->Size(), container_builder->GetWritingMode()), child_fragment.Size());
        GatherInlineContainerFragmentsFromItems(items->Items(), child_offset, inline_containing_block_map, &containing_linebox_map);
    }
}

void InlineContainingBlockUtils::ComputeInlineContainerGeometryForFragmentainer(
    const LayoutBox* box, PhysicalSize accumulated_containing_block_size, InlineContainingBlockMap* inline_containing_block_map)
{
    if (inline_containing_block_map->empty())
        return;

    WritingDirectionMode writing_direction = box->StyleRef().GetWritingDirection();
    WritingModeConverter containing_block_converter = WritingModeConverter(writing_direction, accumulated_containing_block_size);
    ...
    for (auto& physical_fragment : box->PhysicalFragments()) {
        ...
        PhysicalOffset offset = containing_block_converter.ToPhysical(logical_offset, accumulated_containing_block_size);

        WritingModeConverter current_fragment_converter = WritingModeConverter(writing_direction, physical_fragment.Size());
        if (physical_fragment.HasItems()) {
            GatherInlineContainerFragmentsFromItems(physical_fragment.Items()->Items(), offset, inline_containing_block_map, &containing_linebox_map,
            ...
            for (const auto& child : physical_fragment.Children()) {
                ...
                GatherInlineContainerFragmentsFromItems(child_fragment.Items()->Items(), child.offset + offset, inline_containing_block_map,
                    &containing_linebox_map, &current_fragment_converter, &containing_block_converter);
            }
        }
        if (const BlockBreakToken* break_token = physical_fragment.GetBreakToken()) {
