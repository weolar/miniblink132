...
TEST_F(FragmentItemsTest, IsContainerForCulledInline)
{
    ...
    const auto* culled1 = DynamicTo<LayoutInline>(GetLayoutObjectByElementId("culled1"));
    const auto* culled2 = DynamicTo<LayoutInline>(GetLayoutObjectByElementId("culled2"));
    const auto* area = DynamicTo<LayoutInline>(GetLayoutObjectByElementId("area"));
    ...
    EXPECT_FALSE(fragment->Items()->IsContainerForCulledInline(*culled1, &is_first, &is_last, &has_any_child));
    EXPECT_TRUE(has_any_child);
    EXPECT_FALSE(fragment->Items()->IsContainerForCulledInline(*culled2, &is_first, &is_last, &has_any_child));
    EXPECT_TRUE(has_any_child);
    EXPECT_FALSE(fragment->Items()->IsContainerForCulledInline(*area, &is_first, &is_last, &has_any_child));
    EXPECT_FALSE(has_any_child);

    fragment = container->GetPhysicalFragment(1);
    ASSERT_TRUE(fragment->Items());
    EXPECT_TRUE(fragment->Items()->IsContainerForCulledInline(*culled1, &is_first, &is_last, &has_any_child));
    ...
    EXPECT_TRUE(is_last);
    EXPECT_TRUE(has_any_child);
    EXPECT_TRUE(fragment->Items()->IsContainerForCulledInline(*culled2, &is_first, &is_last, &has_any_child));
    EXPECT_TRUE(is_first);
    EXPECT_FALSE(is_last);
    EXPECT_TRUE(has_any_child);
    EXPECT_FALSE(fragment->Items()->IsContainerForCulledInline(*area, &is_first, &is_last, &has_any_child));
