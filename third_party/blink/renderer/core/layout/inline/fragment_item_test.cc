...
public:
    ...
    wtf_size_t IndexOf(const Vector<InlineCursorPosition>& items, const FragmentItem* target)
    {
        wtf_size_t index = 0;
    ...
    void TestFirstDirtyLineIndex(const char* id, wtf_size_t expected_index)
    {
        ...
        const FragmentItem* end_reusable_item = items->EndOfReusableItems(*fragment);
    ...
    Vector<const FragmentItem*> ItemsForAsVector(const LayoutObject& layout_object)
    {
...
TEST_F(FragmentItemTest, CopyMove)
{
    ...
    move_of_line.ink_overflow_type_
        = static_cast<int>(move_of_line.ink_overflow_.SetContents(move_of_line.InkOverflowType(), not_small_ink_overflow_rect, line_item->Size()));
...
TEST_F(FragmentItemTest, BasicInlineBox)
{
    ...
    EXPECT_EQ(PhysicalOffset(40, 0), items_for_span1[0]->OffsetInContainerFragment());
    ...
    EXPECT_EQ(PhysicalOffset(0, 10), items_for_span1[1]->OffsetInContainerFragment());
    ...
    EXPECT_EQ(PhysicalOffset(0, 20), items_for_span2[0]->OffsetInContainerFragment());
...
TEST_F(FragmentItemTest, SelfPaintingInlineBox)
{
    ...
    auto* self_painting_inline_box = To<LayoutInline>(GetLayoutObjectByElementId("self_painting_inline_box"));
...
TEST_F(FragmentItemTest, LineFragmentId)
{
    ...
    for (cursor.MoveToFirstLine(); cursor; cursor.MoveToNextLineIncludingFragmentainer(), ++line_index) {
        EXPECT_EQ(cursor.Current()->FragmentId(), line_index + FragmentItem::kInitialLineFragmentId);
    }
...
TEST_F(FragmentItemTest, Outline)
{
    ...
    auto* target = To<LayoutBlockFlow>(GetLayoutObjectByElementId("target"));
    Vector<PhysicalRect> rects = target->OutlineRects(nullptr, PhysicalOffset(), OutlineType::kIncludeBlockInkOverflow);
    EXPECT_THAT(rects,
        testing::ElementsAre(PhysicalRect(0, 0, 200, 10), // <div id="target">
...
class FragmentItemInsertTest : public FragmentItemTest, public testing::WithParamInterface<CreateNode> { };

INSTANTIATE_TEST_SUITE_P(FragmentItemTest, FragmentItemInsertTest, testing::ValuesIn(node_creators));

// Various nodes/elements to test removals.
class FragmentItemRemoveTest : public FragmentItemTest, public testing::WithParamInterface<const char*> { };

INSTANTIATE_TEST_SUITE_P(FragmentItemTest, FragmentItemRemoveTest,
    testing::Values("text", "<span>span</span>", "<span>1234 12345678</span>", "<span style='display: inline-block'>box</span>", "<img>",
        "<div style='float: left'>float</div>", "<div style='position: absolute'>abs</div>"));
...
TEST_F(FragmentItemTest, MarkLineBoxesDirtyByInsertAtStart)
{
    ...
    Element& target = *GetElementById("target");
    target.parentNode()->insertBefore(Text::Create(GetDocument(), "XYZ"), &target);
...
TEST_F(FragmentItemTest, MarkLineBoxesDirtyByInsertAtMiddle)
{
    ...
    Element& target = *GetElementById("target");
    target.parentNode()->insertBefore(Text::Create(GetDocument(), "XYZ"), target.nextSibling());
    GetDocument().UpdateStyleAndLayoutTree();
    // TODO(kojii): This can be optimized more.
    TestFirstDirtyLineIndex("container", 0);
}
