...
public:
    LineBreaker(InlineNode, LineBreakerMode, const ConstraintSpace&, const LineLayoutOpportunity&, const LeadingFloats& leading_floats, const InlineBreakToken*,
        const ColumnSpannerPath*, ExclusionSpace*);
    ~LineBreaker();
    ...
    {
        return has_cloned_box_decorations_;
    }
    ...
    {
        return current_.item_index >= Items().size();
    }
    ...
    bool ShouldDisableScoreLineBreak() const
    {
        return disable_score_line_break_;
    }
    ...
    void SetIntrinsicSizeOutputs(MaxSizeCache* max_size_cache, bool* depends_on_block_constraints_out);

    // Compute InlineItemResult for an open tag item.
    // Returns true if this item has edge and may have non-zero inline size.
    static bool ComputeOpenTagResult(const InlineItem&, const ConstraintSpace&, bool is_in_svg_text, InlineItemResult*);
    ...
    void AppendCandidates(const InlineItemResult& item_result, const LineInfo& line_info, LineBreakCandidateContext& context);
    ...
    void SetInputRange(InlineItemTextIndex start, wtf_size_t end_item_index, WhitespaceState initial_whitespace_state, const LineBreaker* parent);
...
private:
    ...
    const HeapVector<InlineItem>& Items() const
    {
        return items_data_->items;
    }
    ...
    BreakResult BreakText(
        InlineItemResult*, const InlineItem&, const ShapeResult&, LayoutUnit available_width, LayoutUnit available_width_with_hyphens, LineInfo*);
    bool BreakTextAt(InlineItemResult*, const InlineItem&, ShapingLineBreaker& breaker, LineInfo*);
    bool BreakTextAtPreviousBreakOpportunity(InlineItemResult* item_result);
    bool HandleTextForFastMinContent(InlineItemResult*, const InlineItem&, const ShapeResult&, LineInfo*);
    void HandleEmptyText(const InlineItem& item, LineInfo*);

    const ShapeResultView* TruncateLineEndResult(const LineInfo&, const InlineItemResult&, unsigned end_offset);
    void UpdateShapeResult(const LineInfo&, InlineItemResult*);
    const ShapeResult* ShapeText(const InlineItem&, unsigned start, unsigned end, ShapeOptions = ShapeOptions());
    ...
    void HandleBlockInInline(const InlineItem&, const BlockBreakToken*, LineInfo*);
    void ComputeMinMaxContentSizeForBlockChild(const InlineItem&, InlineItemResult*);
    ...
    NOINLINE bool HandleRuby(LineInfo* line_info, LayoutUnit retry_size = kIndefiniteSize);
    bool IsMonolithicRuby(const LineInfo& base_line, const HeapVector<LineInfo, 1>& annotation_line_list) const;
    ...
    InlineItemResult* AddRubyColumnResult(const InlineItem& item, const LineInfo& base_line_info, const HeapVector<LineInfo, 1>& annotation_line_list,
    ...
    bool CanBreakAfterRubyColumn(const InlineItemResult& column_result, wtf_size_t column_end_item_index) const;
    ...
    void HandleFloat(const InlineItem&, const BlockBreakToken* float_break_token, LineInfo*);
    ...
    const ComputedStyle& ComputeCurrentStyle(unsigned item_result_index, LineInfo*) const;
    ...
    bool IsAtEnd() const
    {
        return current_.item_index >= end_item_index_;
    }
    ...
    {
        return (parent_breaker_ && !auto_wrap_) || position_ <= AvailableWidthToFit();
    }
    ...
    {
        return hyphen_index_.has_value();
    }
    LayoutUnit AddHyphen(InlineItemResults* item_results, wtf_size_t index, InlineItemResult* item_result);
    LayoutUnit AddHyphen(InlineItemResults* item_results, wtf_size_t index);
    LayoutUnit AddHyphen(InlineItemResults* item_results, InlineItemResult* item_result);
