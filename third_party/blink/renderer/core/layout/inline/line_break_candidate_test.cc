...
public:
    bool ComputeCandidates(const InlineNode& node, LayoutUnit available_width, LineBreakCandidates& candidates)
    {
...
TEST_F(LineBreakCandidateTest, Text)
{
    ...
    for (int width : { 800, 50, 10 }) {
        ...
        EXPECT_THAT(
            candidates, testing::ElementsAre(LineBreakCandidate({ 0, 0 }, 0), LineBreakCandidate({ 0, 3 }, { 0, 2 }, 30, 20), LineBreakCandidate({ 0, 6 }, 60)))
...
TEST_F(LineBreakCandidateTest, SoftHyphen)
{
    ...
    for (int width : { 800, 70, 60, 50, 10 }) {
        ...
        EXPECT_THAT(candidates,
            testing::ElementsAre(LineBreakCandidate({ 0, 0 }, 0), LineBreakCandidate({ 0, 3 }, { 0, 3 }, 20, 30, 0, true),
                LineBreakCandidate({ 0, 7 }, { 0, 7 }, 50, 60, 0, true), LineBreakCandidate({ 0, 12 }, 90)))
            << String::Format("Width=%d", width);
    }
}
...
TEST_F(LineBreakCandidateTest, SoftHyphenDisabled)
{
    ...
    for (int width : { 800, 60, 10 }) {
        LineBreakCandidates candidates;
        EXPECT_TRUE(ComputeCandidates(target, LayoutUnit(width), candidates));
        EXPECT_THAT(candidates, testing::ElementsAre(LineBreakCandidate({ 0, 0 }, 0), LineBreakCandidate({ 0, 11 }, 90))) << String::Format("Width=%d", width);
    }
}
...
TEST_F(LineBreakCandidateTest, Span)
{
    ...
    for (const int width : { 800, 60, 50, 10 }) {
        ...
        EXPECT_TRUE(ComputeCandidates(target, LayoutUnit(width), candidates));
        EXPECT_THAT(candidates,
            testing::ElementsAre(LineBreakCandidate({ 0, 0 }, 0), LineBreakCandidate({ 0, 3 }, { 0, 2 }, 30, 20),
                LineBreakCandidate({ 4, 7 }, { 2, 6 }, 70, 60), LineBreakCandidate({ 4, 11 }, 110)))
            << String::Format("Width=%d", width);
    }
}
...
TEST_F(LineBreakCandidateTest, SpanMidWord)
{
    ...
    for (const int width : { 800, 80, 10 }) {
        ...
        EXPECT_TRUE(ComputeCandidates(target, LayoutUnit(width), candidates));
        EXPECT_THAT(candidates,
            testing::ElementsAre(LineBreakCandidate({ 0, 0 }, 0), LineBreakCandidate({ 4, 7 }, { 4, 6 }, 70, 60), LineBreakCandidate({ 4, 11 }, 110)))
            << String::Format("Width=%d", width);
    }
}
...
TEST_F(LineBreakCandidateTest, TrailingSpacesCollapsed)
{
    ...
    for (const int width : { 800, 50, 10 }) {
        ...
        if (width < 70) {
            EXPECT_THAT(candidates,
                testing::ElementsAre(LineBreakCandidate({ 0, 0 }, 0), LineBreakCandidate({ 0, 4 }, { 0, 3 }, 40, 30),
                    LineBreakCandidate({ 4, 4 }, { 0, 3 }, 40, 30), LineBreakCandidate({ 4, 7 }, 70)))
                << String::Format("Width=%d", width);
            continue;
        }
        EXPECT_THAT(
            candidates, testing::ElementsAre(LineBreakCandidate({ 0, 0 }, 0), LineBreakCandidate({ 0, 4 }, { 0, 3 }, 40, 30), LineBreakCandidate({ 4, 7 }, 70)))
            << String::Format("Width=%d", width);
    }
}
...
TEST_F(LineBreakCandidateTest, AtomicInline)
{
    ...
    for (const int width : { 800, 10 }) {
        LineBreakCandidates candidates;
        EXPECT_TRUE(ComputeCandidates(target, LayoutUnit(width), candidates));
        EXPECT_THAT(candidates, testing::ElementsAre(LineBreakCandidate({ 0, 0 }, 0), LineBreakCandidate({ 1, 1 }, 10), LineBreakCandidate({ 2, 2 }, 20)))
            << String::Format("Width=%d", width);
    }
}
...
TEST_F(LineBreakCandidateTest, AtomicInlineBr)
{
    ...
    for (const int width : { 800, 10 }) {
        LineBreakCandidates candidates;
        EXPECT_TRUE(ComputeCandidates(target, LayoutUnit(width), candidates));
        EXPECT_THAT(candidates, testing::ElementsAre(LineBreakCandidate({ 0, 0 }, 0), LineBreakCandidate({ 2, 2 }, { 1, 1 }, 10, 10)))
            << String::Format("Width=%d", width);
    }
}
    ...
    for (const int width : { 800, 10 }) {
        ...
        EXPECT_THAT(candidates,
            testing::ElementsAre(LineBreakCandidate({ 0, 0 }, 0),
                ...
                LineBreakCandidate({ 3, 2 }, { 2, 1 }, 20, 10), LineBreakCandidate({ 7, 4 }, { 5, 4 }, 40, 40)))
            << String::Format("Width=%d", width);
    }
}
...
TEST_F(LineBreakCandidateTest, ForcedBreak)
{
    ...
    for (const int width : { 800, 40, 10 }) {
        ...
        EXPECT_THAT(candidates,
            testing::ElementsAre(LineBreakCandidate({ 0, 0 }, 0), LineBreakCandidate({ 0, 3 }, { 0, 2 }, 30, 20),
                LineBreakCandidate({ 1, 7 }, { 0, 6 }, 60, 60), LineBreakCandidate({ 2, 10 }, { 2, 9 }, 90, 80),
