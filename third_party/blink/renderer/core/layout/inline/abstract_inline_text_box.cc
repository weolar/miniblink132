...
{
    return static_cast<wtf_size_t>(cursor.CurrentItem() - &cursor.Items().front());
}
...
private:
    AbstractInlineTextBoxCache()
        : map_(MakeGarbageCollected<MapType>())
    {
    }
    ...
    AbstractInlineTextBox* GetOrCreateInternal(const InlineCursor& cursor)
    {
        ...
        auto* const layout_text = To<LayoutText>(cursor.CurrentMutableLayoutObject());
        if (it != map_->end()) {
...
AbstractInlineTextBox* AbstractInlineTextBox::GetOrCreate(const InlineCursor& cursor)
{
    if (!cursor)
        return nullptr;
...
bool AbstractInlineTextBox::NeedsTrailingSpace() const
{
    ...
    if (text_content[end_offset] != ' ')
        return false;
    const InlineBreakToken* break_token = line_box.Current().GetInlineBreakToken();
    ...
    if (!mapping)
        return false;
    const base::span<const OffsetMappingUnit> mapping_units = mapping->GetMappingUnitsForTextContentOffsetRange(end_offset, end_offset + 1);
...
unsigned AbstractInlineTextBox::TextOffsetInFormattingContext(unsigned offset) const
{
    const InlineCursor& cursor = GetCursor();
    if (!cursor)
        return 0;
...
PhysicalDirection AbstractInlineTextBox::GetDirection() const
{
    ...
    return WritingDirectionMode(GetLayoutText()->Style()->GetWritingMode(), cursor.Current().ResolvedDirection()).InlineEnd();
}
...
{
    return layout_text_ ? layout_text_->GetDocument().ExistingAXObjectCache() : nullptr;
}
...
void AbstractInlineTextBox::GetWordBoundaries(Vector<WordBoundaries>& words) const
{
...
// static
void AbstractInlineTextBox::GetWordBoundariesForText(Vector<WordBoundaries>& words, const String& text)
{
    if (!text.length()) {
        return;
    }
    ...
    for (int offset = 0; offset != kTextBreakDone && offset < static_cast<int>(text.length()); offset = it->following(offset)) {
        ...
        if (WTF::unicode::IsPunct(text[offset]) || U16_IS_SURROGATE(text[offset])) {
            ...
            if (offset == 0 || WTF::unicode::IsAlphanumeric(text[offset - 1]) || !IsWordBreak(text[offset - 1])) {
                if (word_start) {
            ...
            if (IsWordBreak(prev_character) || WTF::unicode::IsPunct(prev_character) || U16_IS_SURROGATE(prev_character)) {
                if (word_start) {
                    words.emplace_back(*word_start, offset);
