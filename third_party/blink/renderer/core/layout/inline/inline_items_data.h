...
public:
    ...
    HeapVector<InlineItem>::const_iterator ToItemIterator(const InlineItem& item) const
    {
        // SAFETY: ToItemIndex() ensures the index is valid.
        return UNSAFE_BUFFERS(items.begin() + ToItemIndex(item));
    }
    ...
    void GetOpenTagItems(wtf_size_t start_index, wtf_size_t size, OpenTagItems* open_items) const;
