...
FragmentItemsBuilder::FragmentItemsBuilder(WritingDirectionMode writing_direction)
    : node_(nullptr)
    , writing_direction_(writing_direction)
{
}

FragmentItemsBuilder::FragmentItemsBuilder(const InlineNode& node, WritingDirectionMode writing_direction, bool is_block_fragmented)
    : node_(node)
    , writing_direction_(writing_direction)
{
...
const LogicalLineItems& FragmentItemsBuilder::GetLogicalLineItems(const PhysicalLineBoxFragment& line_fragment) const
{
    if (&line_fragment == current_line_fragment_) {
        DCHECK(current_line_container_);
        return current_line_container_->BaseLine();
    }
    const LogicalLineContainer* container = line_container_map_.at(&line_fragment);
    DCHECK(container);
    return container->BaseLine();
}

void FragmentItemsBuilder::AssociateLogicalLineContainer(LogicalLineContainer* line_container, const PhysicalFragment& line_fragment)
{
...
void FragmentItemsBuilder::AddLine(const PhysicalLineBoxFragment& line_fragment, const LogicalOffset& offset)
{
    ...
    const wtf_size_t estimated_size = size_before + line_container->EstimatedFragmentItemCount();
    ...
    for (auto& annotation_line : line_container->AnnotationLineList()) {
        ...
        LayoutUnit line_inline_size = annotation_line->LastInFlowChild()->rect.InlineEndOffset() - line_offset.inline_offset;
        PhysicalSize size
            = IsHorizontalWritingMode(GetWritingMode()) ? PhysicalSize(line_inline_size, line_height) : PhysicalSize(line_height, line_inline_size);
        ...
        items_[annotation_line_start_index].item.SetDescendantsCount(items_.size() - annotation_line_start_index);
    }
...
void FragmentItemsBuilder::AddListMarker(const PhysicalBoxFragment& marker_fragment, const LogicalOffset& offset)
{
    DCHECK(!is_converted_to_physical_);
...
FragmentItemsBuilder::AddPreviousItemsResult FragmentItemsBuilder::AddPreviousItems(
    const PhysicalBoxFragment& container, const FragmentItems& items, BoxFragmentBuilder* container_builder, const FragmentItem* end_item, wtf_size_t max_lines)
{
    if (end_item) {
        DCHECK(node_);
    ...
    const wtf_size_t estimated_size = base::checked_cast<wtf_size_t>(source_items.size());
    ...
    const WritingMode writing_mode = GetWritingMode();
    WritingModeConverter line_converter({ ToLineWritingMode(writing_mode), TextDirection::kLtr });
    ...
    for (InlineCursor cursor(container, items); cursor;) {
        ...
        const LogicalOffset item_offset = converter.ToLogical(item.OffsetInContainerFragment(), item.Size());
        ...
        if (item.Type() == FragmentItem::kLine) {
            ...
            if (end_item) {
                ...
                const auto* break_token = To<InlineBreakToken>(line_fragment->GetBreakToken());
                DCHECK(break_token);
            ...
            for (InlineCursor line = cursor.CursorForDescendants(); line; line.MoveToNext()) {
                ...
                // layout.
                std::optional<PhysicalBoxFragment::AllowPostLayoutScope> allow_post_layout;
                if (line_child.IsRelayoutBoundary())
                    allow_post_layout.emplace();
#endif
                items_.emplace_back(line_converter.ToLogical(line_child.OffsetInContainerFragment() - line_box_bounds.offset, line_child.Size()), line_child);
    ...
    if (end_item && last_break_token) {
        ...
        return AddPreviousItemsResult { last_break_token, used_block_size, line_count, true };
    }
    return AddPreviousItemsResult();
}

const FragmentItemsBuilder::ItemWithOffsetList& FragmentItemsBuilder::Items(const PhysicalSize& outer_size)
{
...
void FragmentItemsBuilder::ConvertToPhysical(const PhysicalSize& outer_size)
{
    ...
    WritingModeConverter line_converter({ ToLineWritingMode(GetWritingMode()), TextDirection::kLtr });
        ...
        if (item->Type() == FragmentItem::kLine) {
            ...
            if (descendants_count) {
                ...
                while (--descendants_count) {
                    ...
                    item = &iter->item;
                    item->SetOffset(line_converter.ToPhysical(iter->offset, item->Size()) + line_box_bounds.offset);
                }
            }
        }
    }
...
std::optional<PhysicalSize> FragmentItemsBuilder::ToFragmentItems(const PhysicalSize& outer_size, void* data)
{
    DCHECK(text_content_);
    ConvertToPhysical(outer_size);
    std::optional<PhysicalSize> new_size;
    if (node_.IsSvgText()) {
        new_size = SvgTextLayoutAlgorithm(node_, GetWritingMode()).Layout(TextContent(false), items_);
    }
