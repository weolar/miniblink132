...
ALWAYS_INLINE bool ShouldOptimize(const LineInfoList& line_info_list, LineBreaker& line_breaker)
{
    ...
    if (num_lines >= kNumLastHyphenatedLines + 1 && line_info_list[num_lines - 2].IsHyphenated() && line_info_list[num_lines - 3].IsHyphenated()) {
        return true;
    }
...
void ScoreLineBreaker::OptimalBreakPoints(const LeadingFloats& leading_floats, ScoreLineBreakContext& context)
{
    ...
    LineBreaker line_breaker(node_, LineBreakerMode::kContent, GetConstraintSpace(), LineLayoutOpportunity(line_width), leading_floats, break_token_,
        /* column_spanner_path */ nullptr, exclusion_space_);
    const int lines_until_clamp = space_.GetLineClampData().LinesUntilClamp().value_or(0);
    for (;;) {
        ...
        if (line_info.IsEndParagraph() || [&] {
                if (lines_until_clamp > 0 && line_info_list.Size() == static_cast<wtf_size_t>(lines_until_clamp)) [[unlikely]] {
                    return true;
                }
                return false;
            }()) {
...
void ScoreLineBreaker::BalanceBreakPoints(const LeadingFloats& leading_floats, ScoreLineBreakContext& context)
{
    is_balanced_ = true;
    OptimalBreakPoints(leading_floats, context);
}

bool ScoreLineBreaker::Optimize(const LineInfoList& line_info_list, LineBreaker& line_breaker, LineBreakPoints& break_points)
{
    DCHECK(break_points.empty());
    ...
    if (candidates.size() >= 4) {
        ...
        const auto candidates_span = base::span(candidates).first(candidates.size() - 1);
...
bool ScoreLineBreaker::ComputeCandidates(const LineInfoList& line_info_list, LineBreaker& line_breaker, LineBreakCandidates& candidates)
{
...
void ScoreLineBreaker::SetupParameters()
{
    ...
    const LayoutUnit available_width = line_widths_.Default().ClampNegativeToZero();
    ...
    const float width_times_font_size = available_width.ToFloat() * font_size / zoom_;
...
void ScoreLineBreaker::ComputeScores(const LineBreakCandidates& candidates, LineBreakScores& scores)
{
    DCHECK_GE(candidates.size(), 2u);
    DCHECK(scores.empty());
    ...
    for (wtf_size_t end = 1; end < candidates.size(); ++end) {
        ...
        scores.push_back(LineBreakScore { best + end_candidate.penalty + line_penalty_, best_prev_index, scores[best_prev_index].line_index + 1 });
    }
}

void ScoreLineBreaker::ComputeBreakPoints(const LineBreakCandidates& candidates, const LineBreakScores& scores, LineBreakPoints& break_points)
{
    ...
    for (wtf_size_t i = scores.size() - 1, prev_index; i > 0; i = prev_index) {
        ...
        const LayoutUnit line_width = LayoutUnit::FromFloatCeil(candidate.pos_if_break - prev_candidate.pos_no_break);
