...
LayoutUnit ComputeInitialLetterBoxBlockOffset(const PhysicalBoxFragment& initial_letter_box_fragment, const LayoutUnit block_size,
    const ComputedStyle& initial_letter_box_style, const ComputedStyle& paragraph_style, LayoutUnit* initial_letter_block_start_adjust)
{
    ...
    const int size = std::ceil(RuntimeEnabledFeatures::InitialLetterRaiseBySpecifiedEnabled() ? initial_letter.Size() : block_size / line_height.ToFloat());
    const int sink = initial_letter.IsRaise() || initial_letter.IsIntegerSink() ? initial_letter.Sink() : size;
    ...
    *initial_letter_block_start_adjust = line_height * (size - sink);
    ...
    if (paragraph_style.IsHorizontalTypographicMode() || initial_letter_box_style.GetTextOrientation() == ETextOrientation::kSideways) {
        // `writing-mode: horizontal-tb` or `text-orientation: sideways`
        // `baseline` is ascent for not `vertical-lr`, descent for `vertical-lr`.
        const LayoutUnit baseline = initial_letter_box_fragment.FirstBaseline().value_or(LayoutUnit());
        const LayoutUnit ascent = paragraph_style.IsFlippedLinesWritingMode() ? block_size - baseline : baseline;
        const LayoutUnit block_offset = LayoutUnit(line_height * initial_letter.Size()) - ascent;
        const FontHeight text_metrics = paragraph_style.GetFontHeight();
        FontHeight line_metrics = text_metrics;
        FontHeight leading_space = CalculateLeadingSpace(paragraph_style.ComputedLineHeightAsFixed(), line_metrics);
...
LogicalRect ComputeTextInkBounds(const ShapeResultView& shape_result, const ComputedStyle& style, LayoutUnit* out_baseline = nullptr)
{
    ...
    const LogicalRect text_ink_bounds = LogicalRect::EnclosingRect(text_ink_float_bounds);

    // Calculation of `ascent` should be as same as
    // `NGTextFragmentPainter::Paint()`
    const FontMetrics& font_metrics = style.GetFont().PrimaryFont()->GetFontMetrics();
...
const ExclusionArea* CreateExclusionSpaceForInitialLetterBox(EFloat float_type, BfcOffset origin, const BfcOffset& border_box_offset,
    const LogicalSize& border_box_size, const BoxStrut& margins, bool is_hidden_for_paint)
{
    ...
    const BfcOffset local_start_offset(border_box_offset.line_offset - margins.inline_start, border_box_offset.block_offset - margins.block_start);
    ...
    const LogicalSize margin_box_size(
        (border_box_size.inline_size + margins.InlineSum()).ClampNegativeToZero(), (border_box_size.block_size + margins.BlockSum()).ClampNegativeToZero());
    ...
    //  |    *    line 6
    //  V         line 7
    const BfcOffset start_offset(float_type == EFloat::kLeft ? origin.line_offset + local_start_offset.line_offset
                                                             : origin.line_offset - margin_box_size.inline_size + margins.inline_end,
        origin.block_offset + std::min(local_start_offset.block_offset, LayoutUnit()));

    const BfcOffset end_offset(
        start_offset.line_offset + margin_box_size.inline_size, origin.block_offset + local_start_offset.block_offset + margin_box_size.block_size);

    return ExclusionArea::CreateForInitialLetterBox(BfcRect(start_offset, end_offset), float_type, is_hidden_for_paint);
}

} // namespace

FontHeight AdjustInitialLetterInTextPosition(const FontHeight& line_box_metrics, LogicalLineItems* line_box)
{
    ...
    for (LogicalLineItem& line_item : *line_box) {
        ...
        if (!shape_result || !line_item.inline_item || line_item.inline_item->Type() != InlineItem::kText) {
            continue;
        }

        LayoutUnit baseline;
        const ComputedStyle& style = *line_item.Style();
        const LogicalRect text_ink_bounds = ComputeTextInkBounds(*shape_result, style, &baseline);
        ...
        if (style.IsHorizontalTypographicMode() || style.GetTextOrientation() == ETextOrientation::kSideways) {
...
LayoutUnit CalculateInitialLetterBoxInlineSize(const LineInfo& line_info)
{
    ...
    for (const InlineItemResult& item_result : line_info.Results()) {
        ...
        const LogicalRect text_ink_bounds = ComputeTextInkBounds(*shape_result, style);
...
const ExclusionArea* PostPlaceInitialLetterBox(const FontHeight& line_box_metrics, const BoxStrut& initial_letter_box_margins, LogicalLineItems* line_box,
    const BfcOffset& line_origin, LineInfo* line_info)
{
    auto initial_letter_line_item = std::find_if(line_box->begin(), line_box->end(), [](const auto& line_item) { return line_item.IsInitialLetterBox(); });

    const auto& initial_letter_box_fragment = *To<PhysicalBoxFragment>(initial_letter_line_item->GetPhysicalFragment());
    ...
    const WritingDirectionMode writing_direction_mode = line_style.GetWritingDirection();

    const LogicalSize initial_letter_box_size = LogicalFragment(writing_direction_mode, initial_letter_box_fragment).Size();

    LayoutUnit initial_letter_block_start_adjust;
    const LayoutUnit initial_letter_border_box_block_offset
        = ComputeInitialLetterBoxBlockOffset(initial_letter_box_fragment, initial_letter_box_size.block_size, *initial_letter_line_item->Style(), line_style,
              &initial_letter_block_start_adjust)
    ...
    if (writing_direction_mode.IsFlippedLines()) {
        ...
        adjusted_block_offset = line_style.ComputedLineHeightAsFixed() + -adjusted_block_offset + -initial_letter_box_size.block_size;
    }

    // Convert to baseline origin block offset.
    initial_letter_line_item->rect.offset.block_offset = adjusted_block_offset - line_box_metrics.ascent;

    const LayoutUnit initial_letter_border_box_inline_offset = initial_letter_line_item->rect.offset.inline_offset;

    const BfcOffset initial_letter_box_origin(writing_direction_mode.IsLtr()
            ? line_origin.line_offset
            : line_origin.line_offset + initial_letter_border_box_inline_offset + initial_letter_box_size.inline_size,
        line_origin.block_offset);

    const ExclusionArea* exclusion = CreateExclusionSpaceForInitialLetterBox(writing_direction_mode.IsLtr() ? EFloat::kLeft : EFloat::kRight,
        initial_letter_box_origin,
        BfcOffset(initial_letter_border_box_inline_offset, initial_letter_border_box_block_offset + line_info->ComputeInitialLetterBoxBlockStartAdjustment()),
        initial_letter_box_size, initial_letter_box_margins, initial_letter_box_fragment.IsHiddenForPaint());
