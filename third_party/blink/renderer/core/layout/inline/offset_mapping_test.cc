...
HeapVector<OffsetMappingUnit> ToVector(const base::span<const OffsetMappingUnit>& range)
{
...
{
    return unit.GetType() == other.GetType() && unit.GetLayoutObject() == other.GetLayoutObject() && unit.DOMStart() == other.DOMStart()
        && unit.DOMEnd() == other.DOMEnd() && unit.TextContentStart() == other.TextContentStart() && unit.TextContentEnd() == other.TextContentEnd();
}
...
{
    static const std::array<const char*, 3> kTypeNames = { "Identity", "Collapsed", "Expanded" };
    *ostream << "{" << kTypeNames[static_cast<unsigned>(unit.GetType())] << " " << unit.GetLayoutObject() << " dom=" << unit.DOMStart() << "-" << unit.DOMEnd()
             << " tc=" << unit.TextContentStart() << "-" << unit.TextContentEnd() << "}";
}

void PrintTo(const HeapVector<OffsetMappingUnit>& units, std::ostream* ostream)
{
    *ostream << "[";
    const char* comma = "";
...
void PrintTo(const base::span<const OffsetMappingUnit>& range, std::ostream* ostream)
{
...
protected:
    ...
    {
        const OffsetMapping* map = InlineNode(layout_block_flow_).ComputeOffsetMappingIfNeeded();
        CHECK(map);
        return *map;
    }
    ...
    String GetCollapsedIndexes() const
    {
        ...
        const EphemeralRange block_range = EphemeralRange::RangeOfContents(*layout_block_flow_->GetNode());
        ...
        for (const Node& node : block_range.Nodes()) {
            ...
            for (const auto& unit : mapping.GetMappingUnitsForDOMRange(EphemeralRange::RangeOfContents(node))) {
    ...
    HeapVector<OffsetMappingUnit> GetFirstLast(const std::string& caret_text)
    {
        ...
        return { *GetOffsetMapping().GetFirstMappingUnit(offset), *GetOffsetMapping().GetLastMappingUnit(offset) };
    }
...
TEST_F(OffsetMappingTest, CollapseSpaces)
{
    ...
    EXPECT_EQ("{10, 16, 17}", TestCollapsingWithCSSWhiteSpace(input, "-webkit-nowrap"));
...
TEST_F(OffsetMappingTest, CollapseNewLines)
{
    ...
    EXPECT_EQ("{10, 11, 17}", TestCollapsingWithCSSWhiteSpace(input, "-webkit-nowrap"));
...
{
    EXPECT_EQ("{}{0}", TestCollapsing("text ", " text")) << "Spaces are collapsed even when across elements.";
}
...
{
    EXPECT_EQ("{4, 5, 7, 8}", TestCollapsingWithCSSWhiteSpace("text  \n  text", "pre-line")) << "Spaces before and after newline are removed.";
}

TEST_F(OffsetMappingTest, CollapsibleSpaceAfterNonCollapsibleSpaceAcrossElements)
{
    SetupHtml("t",
        "<div id=t>"
        ...
        " text"
        "</div>");
    EXPECT_EQ("{}{}", GetCollapsedIndexes()) << "The whitespace in constructions like '<span style=\"white-space: "
                                                "pre-wrap\">text <span><span> text</span>' does not collapse.";
}

TEST_F(OffsetMappingTest, CollapseZeroWidthSpaces)
{
    EXPECT_EQ("{5}", TestCollapsing(u"text\u200B\ntext")) << "Newline is removed if the character before is ZWS.";
    EXPECT_EQ("{4}", TestCollapsing(u"text\n\u200Btext")) << "Newline is removed if the character after is ZWS.";
    EXPECT_EQ("{5}", TestCollapsing(u"text\u200B\n\u200Btext")) << "Newline is removed if the character before/after is ZWS.";

    EXPECT_EQ("{4}{}", TestCollapsing(u"text\n", u"\u200Btext")) << "Newline is removed if the character after across elements is ZWS.";
    EXPECT_EQ("{}{0}", TestCollapsing(u"text\u200B", u"\ntext")) << "Newline is removed if the character before is ZWS even across "
                                                                    "elements.";

    EXPECT_EQ("{4, 5}{}", TestCollapsing(u"text \n", u"\u200Btext")) << "Collapsible space before newline does not affect the result.";
    EXPECT_EQ("{5}{}", TestCollapsing(u"text\u200B\n", u" text")) << "Collapsible space after newline is removed even when the "
                                                                     "newline was removed.";
    EXPECT_EQ("{5}{0}", TestCollapsing(u"text\u200B ", u"\ntext")) << "A white space sequence containing a segment break before or after "
...
{
    EXPECT_EQ("{1}", TestCollapsing(u"\u4E00\n\u4E00")) << "Newline is removed when both sides are Wide.";

    EXPECT_EQ("{}", TestCollapsing(u"\u4E00\nA")) << "Newline is not removed when after is Narrow.";
    EXPECT_EQ("{}", TestCollapsing(u"A\n\u4E00")) << "Newline is not removed when before is Narrow.";

    EXPECT_EQ("{1}{}", TestCollapsing(u"\u4E00\n", u"\u4E00")) << "Newline at the end of elements is removed when both sides are Wide.";
    EXPECT_EQ("{}{0}", TestCollapsing(u"\u4E00", u"\n\u4E00")) << "Newline at the beginning of elements is removed "
                                                                  "when both sides are Wide.";
}
#endif

#define TEST_UNIT(unit, type, owner, dom_start, dom_end, text_content_start, text_content_end)                                                                 \
...
TEST_F(OffsetMappingTest, NGInlineFormattingContextOf)
{
    ...
    EXPECT_EQ(nullptr, NGInlineFormattingContextOf(Position::BeforeNode(*container)));
    EXPECT_EQ(nullptr, NGInlineFormattingContextOf(Position::AfterNode(*container)));
    ...
    EXPECT_EQ(container_object, NGInlineFormattingContextOf(Position::BeforeNode(*inline_block)));
    EXPECT_EQ(container_object, NGInlineFormattingContextOf(Position::AfterNode(*inline_block)));

    const LayoutObject* inline_block_object = inline_block->GetLayoutObject();
    EXPECT_EQ(inline_block_object, NGInlineFormattingContextOf(Position(blah, 0)));
}
...
TEST_F(OffsetMappingTest, OneTextNode)
{
    ...
    TEST_UNIT(result.GetUnits()[0], OffsetMappingUnitType::kIdentity, foo_node, 0u, 3u, 0u, 3u);
    ...
    EXPECT_EQ(Position(foo_node, 0), StartOfNextNonCollapsedContent(Position(foo_node, 0)));
    EXPECT_EQ(Position(foo_node, 1), StartOfNextNonCollapsedContent(Position(foo_node, 1)));
    EXPECT_EQ(Position(foo_node, 2), StartOfNextNonCollapsedContent(Position(foo_node, 2)));
    ...
    EXPECT_EQ(Position(foo_node, 1), EndOfLastNonCollapsedContent(Position(foo_node, 1)));
    EXPECT_EQ(Position(foo_node, 2), EndOfLastNonCollapsedContent(Position(foo_node, 2)));
    EXPECT_EQ(Position(foo_node, 3), EndOfLastNonCollapsedContent(Position(foo_node, 3)));
    ...
    EXPECT_FALSE(IsBeforeNonCollapsedContent(Position(foo_node, 3))); // false at node end
...
TEST_F(OffsetMappingTest, TwoTextNodes)
{
    ...
    TEST_UNIT(result.GetUnits()[0], OffsetMappingUnitType::kIdentity, foo_node, 0u, 3u, 0u, 3u);
    TEST_UNIT(result.GetUnits()[1], OffsetMappingUnitType::kIdentity, bar_node, 0u, 3u, 3u, 6u);
    ...
    EXPECT_FALSE(IsBeforeNonCollapsedContent(Position(bar_node, 3))); // false at node end
...
TEST_F(OffsetMappingTest, BRBetweenTextNodes)
{
    ...
    TEST_UNIT(result.GetUnits()[0], OffsetMappingUnitType::kIdentity, foo_node, 0u, 3u, 0u, 3u);
    TEST_UNIT(result.GetUnits()[1], OffsetMappingUnitType::kIdentity, br_node, 0u, 1u, 3u, 4u);
    TEST_UNIT(result.GetUnits()[2], OffsetMappingUnitType::kIdentity, bar_node, 0u, 3u, 4u, 7u);
    ...
    EXPECT_EQ(&result.GetUnits()[0], GetUnitForPosition(Position(foo_node, 2)));
    EXPECT_EQ(&result.GetUnits()[0], GetUnitForPosition(Position(foo_node, 3)));
    EXPECT_EQ(&result.GetUnits()[1], GetUnitForPosition(Position::BeforeNode(*br_node)));
    EXPECT_EQ(&result.GetUnits()[1], GetUnitForPosition(Position::AfterNode(*br_node)));
...
TEST_F(OffsetMappingTest, OneTextNodeWithCollapsedSpace)
{
    ...
    ASSERT_EQ(3u, result.GetUnits().size());
    TEST_UNIT(result.GetUnits()[0], OffsetMappingUnitType::kIdentity, node, 0u, 4u, 0u, 4u);
    TEST_UNIT(result.GetUnits()[1], OffsetMappingUnitType::kCollapsed, node, 4u, 5u, 4u, 4u);
    TEST_UNIT(result.GetUnits()[2], OffsetMappingUnitType::kIdentity, node, 5u, 8u, 4u, 7u);
    ...
    EXPECT_EQ(Position(node, 3), StartOfNextNonCollapsedContent(Position(node, 3)));
    EXPECT_EQ(Position(node, 5), StartOfNextNonCollapsedContent(Position(node, 4)));
    EXPECT_EQ(Position(node, 5), StartOfNextNonCollapsedContent(Position(node, 5)));
...
TEST_F(OffsetMappingTest, FullyCollapsedWhiteSpaceNode)
{
    ...
    ASSERT_EQ(3u, result.GetUnits().size());
    TEST_UNIT(result.GetUnits()[0], OffsetMappingUnitType::kIdentity, foo_node, 0u, 4u, 0u, 4u);
    TEST_UNIT(result.GetUnits()[1], OffsetMappingUnitType::kCollapsed, space_node, 0u, 1u, 4u, 4u);
    TEST_UNIT(result.GetUnits()[2], OffsetMappingUnitType::kIdentity, bar_node, 0u, 3u, 4u, 7u);
    ...
    EXPECT_TRUE(StartOfNextNonCollapsedContent(Position(space_node, 0u)).IsNull());
}
...
TEST_F(OffsetMappingTest, ReplacedElement)
{
    ...
    ASSERT_EQ(3u, result.GetUnits().size());
    TEST_UNIT(result.GetUnits()[0], OffsetMappingUnitType::kIdentity, foo_node, 0u, 4u, 0u, 4u);
    TEST_UNIT(result.GetUnits()[1], OffsetMappingUnitType::kIdentity, img_node, 0u, 1u, 4u, 5u);
    TEST_UNIT(result.GetUnits()[2], OffsetMappingUnitType::kIdentity, bar_node, 0u, 4u, 5u, 9u);
    ...
    EXPECT_EQ(&result.GetUnits()[0], GetUnitForPosition(Position(foo_node, 3)));
    EXPECT_EQ(&result.GetUnits()[0], GetUnitForPosition(Position(foo_node, 4)));
    EXPECT_EQ(&result.GetUnits()[1], GetUnitForPosition(Position::BeforeNode(*img_node)));
    EXPECT_EQ(&result.GetUnits()[1], GetUnitForPosition(Position::AfterNode(*img_node)));
    EXPECT_EQ(&result.GetUnits()[2], GetUnitForPosition(Position(bar_node, 0)));
    EXPECT_EQ(&result.GetUnits()[2], GetUnitForPosition(Position(bar_node, 1)));
    EXPECT_EQ(&result.GetUnits()[2], GetUnitForPosition(Position(bar_node, 2)));
    EXPECT_EQ(&result.GetUnits()[2], GetUnitForPosition(Position(bar_node, 3)));
...
TEST_F(OffsetMappingTest, FirstLetter)
{
    ...
    ASSERT_EQ(2u, result.GetUnits().size());
    TEST_UNIT(result.GetUnits()[0], OffsetMappingUnitType::kIdentity, foo_node, 0u, 1u, 0u, 1u);
    // first leter and remaining text are always in different mapping units.
    TEST_UNIT(result.GetUnits()[1], OffsetMappingUnitType::kIdentity, foo_node, 1u, 3u, 1u, 3u);
...
TEST_F(OffsetMappingTest, FirstLetterWithLeadingSpace)
{
    ...
    ASSERT_EQ(3u, result.GetUnits().size());
    TEST_UNIT(result.GetUnits()[0], OffsetMappingUnitType::kCollapsed, foo_node, 0u, 2u, 0u, 0u);
    TEST_UNIT(result.GetUnits()[1], OffsetMappingUnitType::kIdentity, foo_node, 2u, 3u, 0u, 1u);
    // first leter and remaining text are always in different mapping units.
    TEST_UNIT(result.GetUnits()[2], OffsetMappingUnitType::kIdentity, foo_node, 3u, 5u, 1u, 3u);
...
TEST_F(OffsetMappingTest, FirstLetterWithoutRemainingText)
{
    ...
    ASSERT_EQ(2u, result.GetUnits().size());
    TEST_UNIT(result.GetUnits()[0], OffsetMappingUnitType::kCollapsed, text_node, 0u, 2u, 0u, 0u);
    TEST_UNIT(result.GetUnits()[1], OffsetMappingUnitType::kIdentity, text_node, 2u, 3u, 0u, 1u);
...
{
    SetupHtml("t", "<style>:first-letter{float:right}</style><div id=t>foo</div>");
    Element* div = GetElementById("t");
    const Node* text_node = div->firstChild();
    ...
    TEST_UNIT(first_letter_result.GetUnits()[0], OffsetMappingUnitType::kIdentity, text_node, 0u, 1u, 0u, 1u);
    ...
    TEST_UNIT(remaining_text_result.GetUnits()[0], OffsetMappingUnitType::kIdentity, text_node, 1u, 3u, 1u, 3u);
    ASSERT_EQ(1u, remaining_text_result.GetRanges().size());
    TEST_RANGE(remaining_text_result.GetRanges(), text_node, 0u, 1u);

    EXPECT_EQ(&first_letter_result.GetUnits()[0], first_letter_result.GetMappingUnitForPosition(Position(text_node, 0)));
    EXPECT_EQ(&remaining_text_result.GetUnits()[0], remaining_text_result.GetMappingUnitForPosition(Position(text_node, 1)));
    EXPECT_EQ(&remaining_text_result.GetUnits()[0], remaining_text_result.GetMappingUnitForPosition(Position(text_node, 2)));
    EXPECT_EQ(&remaining_text_result.GetUnits()[0], remaining_text_result.GetMappingUnitForPosition(Position(text_node, 3)));

    EXPECT_EQ(0u, *first_letter_result.GetTextContentOffset(Position(text_node, 0)));
    EXPECT_EQ(1u, *remaining_text_result.GetTextContentOffset(Position(text_node, 1)));
    EXPECT_EQ(2u, *remaining_text_result.GetTextContentOffset(Position(text_node, 2)));
    EXPECT_EQ(3u, *remaining_text_result.GetTextContentOffset(Position(text_node, 3)));
...
TEST_F(OffsetMappingTest, ContainerWithGeneratedContent)
{
    ...
    const LayoutObject& before = *span->GetPseudoElement(kPseudoIdBefore)->GetLayoutObject()->SlowFirstChild();
    const LayoutObject& after = *span->GetPseudoElement(kPseudoIdAfter)->GetLayoutObject()->SlowFirstChild();
    const OffsetMapping& result = GetOffsetMapping();

    EXPECT_EQ((HeapVector<OffsetMappingUnit> { OffsetMappingUnit(kIdentity, before, 0u, 3u, 0u, 3u),
                  OffsetMappingUnit(kIdentity, *text->GetLayoutObject(), 0u, 3u, 3u, 6u), OffsetMappingUnit(kIdentity, after, 0u, 3u, 6u, 9u) }),
        result.GetUnits());

    // Verify |GetMappingUnitsForLayoutObject()| for ::before and ::after
    EXPECT_EQ((HeapVector<OffsetMappingUnit> { OffsetMappingUnit(kIdentity, before, 0u, 3u, 0u, 3u) }), result.GetMappingUnitsForLayoutObject(before));
    EXPECT_EQ((HeapVector<OffsetMappingUnit> { OffsetMappingUnit(kIdentity, after, 0u, 3u, 6u, 9u) }), result.GetMappingUnitsForLayoutObject(after));
}

TEST_F(OffsetMappingTest, ContainerWithGeneratedContentWithCollapsedWhitespace)
{
    SetupHtml("t",
        "<style>"
    ...
    const auto& remaining_part = *To<LayoutText>(target.GetPseudoElement(kPseudoIdBefore)->GetLayoutObject()->SlowLastChild());
    const LayoutObject& first_letter_part = *remaining_part.GetFirstLetterPart();
    const OffsetMapping& result = GetOffsetMapping();
    const auto& target_text = To<LayoutText>(*target.firstChild()->GetLayoutObject());

    EXPECT_EQ((HeapVector<OffsetMappingUnit> { OffsetMappingUnit(kCollapsed, first_letter_part, 0u, 2u, 0u, 0u),
                  OffsetMappingUnit(kIdentity, first_letter_part, 2u, 3u, 0u, 1u), OffsetMappingUnit(kIdentity, remaining_part, 0u, 1u, 1u, 2u),
                  OffsetMappingUnit(kCollapsed, remaining_part, 1u, 3u, 2u, 2u), OffsetMappingUnit(kIdentity, remaining_part, 3u, 5u, 2u, 4u),
    ...
    EXPECT_EQ((HeapVector<OffsetMappingUnit> {
                  OffsetMappingUnit(kCollapsed, first_letter_part, 0u, 2u, 0u, 0u), OffsetMappingUnit(kIdentity, first_letter_part, 2u, 3u, 0u, 1u) }),
        result.GetMappingUnitsForLayoutObject(first_letter_part));
    EXPECT_EQ((HeapVector<OffsetMappingUnit> { OffsetMappingUnit(kIdentity, remaining_part, 0u, 1u, 1u, 2u),
                  OffsetMappingUnit(kCollapsed, remaining_part, 1u, 3u, 2u, 2u), OffsetMappingUnit(kIdentity, remaining_part, 3u, 5u, 2u, 4u) }),
...
TEST_F(OffsetMappingTest, Table)
{
    ...
    ASSERT_EQ(3u, result.GetUnits().size());
    TEST_UNIT(result.GetUnits()[0], OffsetMappingUnitType::kCollapsed, foo_node, 0u, 2u, 0u, 0u);
    TEST_UNIT(result.GetUnits()[1], OffsetMappingUnitType::kIdentity, foo_node, 2u, 5u, 0u, 3u);
    TEST_UNIT(result.GetUnits()[2], OffsetMappingUnitType::kCollapsed, foo_node, 5u, 7u, 3u, 3u);
...
TEST_F(OffsetMappingTest, GetMappingForInlineBlock)
{
    ...
    const OffsetMapping* div_mapping = OffsetMapping::GetFor(Position(div->firstChild(), 0));
    const OffsetMapping* span_mapping = OffsetMapping::GetFor(Position(span->firstChild(), 0));
    ...
    const OffsetMapping* span_before_mapping = OffsetMapping::GetFor(Position::BeforeNode(*span));
    EXPECT_EQ(div_mapping, span_before_mapping);
    EXPECT_NE(span_mapping, span_before_mapping);

    const OffsetMapping* span_after_mapping = OffsetMapping::GetFor(Position::AfterNode(*span));
...
TEST_F(OffsetMappingTest, NoWrapSpaceAndCollapsibleSpace)
{
    ...
    TEST_UNIT(mapping.GetUnits()[0], OffsetMappingUnitType::kIdentity, foo, 0u, 4u, 0u, 4u);
    TEST_UNIT(mapping.GetUnits()[1], OffsetMappingUnitType::kCollapsed, bar, 0u, 1u, 5u, 5u);
    TEST_UNIT(mapping.GetUnits()[2], OffsetMappingUnitType::kIdentity, bar, 1u, 4u, 5u, 8u);
...
TEST_F(OffsetMappingTest, PreLine)
{
    ...
    EXPECT_EQ((HeapVector<OffsetMappingUnit> { OffsetMappingUnit(kIdentity, text_ab_n_cd, 0u, 2u, 0u, 2u),
                  OffsetMappingUnit(kCollapsed, text_ab_n_cd, 2u, 3u, 2u, 2u), OffsetMappingUnit(kIdentity, text_ab_n_cd, 3u, 4u, 2u, 3u),
                  OffsetMappingUnit(kCollapsed, text_ab_n_cd, 4u, 5u, 3u, 3u), OffsetMappingUnit(kIdentity, text_ab_n_cd, 5u, 7u, 3u, 5u) }),
        result.GetUnits());
...
TEST_F(OffsetMappingTest, BiDiAroundForcedBreakInPreLine)
{
    ...
    ASSERT_EQ(3u, mapping.GetUnits().size());
    TEST_UNIT(mapping.GetUnits()[0], OffsetMappingUnitType::kIdentity, text, 0u, 3u, 2u, 5u); // "foo"
    TEST_UNIT(mapping.GetUnits()[1], OffsetMappingUnitType::kIdentity, text, 3u, 4u, 7u, 8u); // "\n"
    TEST_UNIT(mapping.GetUnits()[2], OffsetMappingUnitType::kIdentity, text, 4u, 7u, 10u, 13u); // "bar"
...
{
    LoadAhem();
    SetupHtml("t", "<div id=t style='font: 10px/10px Ahem; width: 40px'>abc&shy;def</div>");
    ...
    TEST_UNIT(mapping.GetUnits()[0], OffsetMappingUnitType::kIdentity, text, 0u, 7u, 0u, 7u);
...
TEST_F(OffsetMappingTest, PreWrapAndReusing)
{
    ...
    const OffsetMapping& mapping = GetOffsetMapping();
    EXPECT_EQ(String(u" \u200Bxyz"), mapping.GetText()) << "We have ZWS after leading preserved space.";
    EXPECT_EQ((HeapVector<OffsetMappingUnit> {
...
TEST_F(OffsetMappingTest, RestoreTrailingCollapsibleSpaceReplace)
{
    ...
    EXPECT_EQ((HeapVector<OffsetMappingUnit> {
                  OffsetMappingUnit(kIdentity, *layout_object_a.SlowFirstChild(), 0u, 1u, 0u, 1u),
                  OffsetMappingUnit(kCollapsed, *layout_object_b.SlowFirstChild(), 0u, 1u, 2u, 2u),
                  OffsetMappingUnit(kCollapsed, newline, 0u, 1u, 2u, 2u),
                  OffsetMappingUnit(kCollapsed, *layout_object_i.SlowFirstChild(), 0u, 1u, 2u, 2u),
              }),
        result.GetUnits());
}
...
TEST_F(OffsetMappingTest, RestoreTrailingCollapsibleSpaceReplaceKeep)
{
    ...
    EXPECT_EQ((HeapVector<OffsetMappingUnit> {
                  OffsetMappingUnit(kIdentity, *layout_object_a.SlowFirstChild(), 0u, 1u, 0u, 1u),
                  OffsetMappingUnit(kIdentity, *layout_object_b.SlowFirstChild(), 0u, 1u, 2u, 3u),
                  OffsetMappingUnit(kCollapsed, newline, 0u, 1u, 3u, 3u),
                  OffsetMappingUnit(kIdentity, *layout_object_i.SlowFirstChild(), 0u, 1u, 3u, 4u),
              }),
        result.GetUnits());
}
...
TEST_F(OffsetMappingTest, RestoreTrailingCollapsibleSpaceNone)
{
    ...
    EXPECT_EQ((HeapVector<OffsetMappingUnit> {
                  ...
                  OffsetMappingUnit(kIdentity, *layout_object_b.SlowFirstChild(), 0u, 1u, 1u, 2u),
                  OffsetMappingUnit(kCollapsed, *layout_object_b.SlowFirstChild(), 1u, 3u, 2u, 2u),
                  OffsetMappingUnit(kCollapsed, newline, 0u, 1u, 2u, 2u),
                  OffsetMappingUnit(kIdentity, *layout_object_i.SlowFirstChild(), 0u, 1u, 2u, 3u),
              }),
        result.GetUnits());
}
...
TEST_F(OffsetMappingTest, RestoreTrailingCollapsibleSpaceSplit)
{
    ...
    EXPECT_EQ((HeapVector<OffsetMappingUnit> {
                  OffsetMappingUnit(kIdentity, *layout_object_a.SlowFirstChild(), 0u, 1u, 0u, 1u),
                  OffsetMappingUnit(kCollapsed, *layout_object_b.SlowFirstChild(), 0u, 3u, 2u, 2u),
                  OffsetMappingUnit(kCollapsed, newline, 0u, 1u, 2u, 2u),
                  OffsetMappingUnit(kCollapsed, *layout_object_i.SlowFirstChild(), 0u, 1u, 2u, 2u),
              }),
        result.GetUnits());
...
TEST_F(OffsetMappingTest, RestoreTrailingCollapsibleSpaceSplitKeep)
{
    ...
    EXPECT_EQ((HeapVector<OffsetMappingUnit> {
                  OffsetMappingUnit(kIdentity, *layout_object_a.SlowFirstChild(), 0u, 1u, 0u, 1u),
                  OffsetMappingUnit(kIdentity, *layout_object_b.SlowFirstChild(), 0u, 1u, 2u, 3u),
                  OffsetMappingUnit(kCollapsed, *layout_object_b.SlowFirstChild(), 1u, 3u, 3u, 3u),
                  OffsetMappingUnit(kCollapsed, newline, 0u, 1u, 3u, 3u),
                  OffsetMappingUnit(kIdentity, *layout_object_i.SlowFirstChild(), 0u, 1u, 3u, 4u),
              }),
        result.GetUnits());
...
TEST_F(OffsetMappingTest, TextOverflowEllipsis)
{
    ...
    ASSERT_EQ(1u, mapping.GetUnits().size());
    TEST_UNIT(mapping.GetUnits()[0], OffsetMappingUnitType::kIdentity, text, 0u, 6u, 0u, 6u);
    TEST_RANGE(mapping.GetRanges(), text, 0u, 1u);
}
...
TEST_F(OffsetMappingTest, StartOfNextNonCollapsedContentWithPseudo)
{
    ...
    EXPECT_EQ(Position(), GetOffsetMapping().StartOfNextNonCollapsedContent(position));
}
...
TEST_F(OffsetMappingTest, EndOfLastNonCollapsedContentWithPseudo)
{
    ...
    EXPECT_EQ(Position(), GetOffsetMapping().EndOfLastNonCollapsedContent(position));
}
...
TEST_F(OffsetMappingTest, WordBreak)
{
    ...
    EXPECT_EQ((HeapVector<OffsetMappingUnit> { OffsetMappingUnit(kIdentity, text_a, 0u, 1u, 0u, 1u), OffsetMappingUnit(kIdentity, wbr, 0u, 1u, 1u, 2u),
                  OffsetMappingUnit(kIdentity, text_b, 0u, 1u, 2u, 3u) }),
        result.GetUnits());

    EXPECT_EQ((HeapVector<OffsetMappingUnit> { OffsetMappingUnit(kIdentity, wbr, 0u, 1u, 1u, 2u) }), result.GetMappingUnitsForLayoutObject(wbr));
}
...
TEST_F(OffsetMappingGetterTest, Get)
{
    ...
    auto* layout_block_flow = To<LayoutBlockFlow>(GetLayoutObjectByElementId("container"));
    DCHECK(layout_block_flow->ChildrenInline());

    const OffsetMapping* mapping = InlineNode::GetOffsetMapping(layout_block_flow);
    EXPECT_TRUE(mapping);
...
TEST_F(OffsetMappingTest, LayoutObjectConverter)
{
    ...
    ASSERT_TRUE(s1);
    OffsetMapping::LayoutObjectConverter converter1 { mapping, *s1->SlowFirstChild() };
    ...
    ASSERT_TRUE(s2);
    OffsetMapping::LayoutObjectConverter converter2 { mapping, *s2->SlowFirstChild() };
