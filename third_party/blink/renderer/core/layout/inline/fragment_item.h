...
public:
    ...
    public:
        void Trace(Visitor* visitor) const
        {
            visitor->Trace(line_box_fragment);
        }
    ...
    // Create a box item.
    FragmentItem(const PhysicalBoxFragment& box, TextDirection resolved_direction);
    // Create a line item.
    explicit FragmentItem(const PhysicalLineBoxFragment& line);
    // Create an annotation line item.
    FragmentItem(const PhysicalSize& size, const PhysicalLineBoxFragment& base_line);
    ...
    ItemType Type() const
    {
        return static_cast<ItemType>(const_type_);
    }

    bool IsText() const
    {
        return Type() == kText || Type() == kGeneratedText;
    }
    bool IsContainer() const
    {
        return Type() == kBox || Type() == kLine;
    }
    ...
    {
        return is_hidden_for_paint_;
    }
    bool IsListMarker() const;

    bool IsSvgText() const
    {
        return Type() == kText && text_.svg_data;
    }

    void SetSvgFragmentData(const SvgFragmentData* data, const PhysicalRect& unscaled_rect, bool is_hidden);
    ...
    void SetIsLastForNode(bool is_last) const
    {
        is_last_for_node_ = is_last;
    }
    ...
    void SetOffset(const PhysicalOffset& offset)
    {
        rect_.offset = offset;
    }
    ...
    static PhysicalRect RecalcInkOverflowForCursor(InlineCursor* cursor, InlinePaintContext* inline_context);
    ...
    public:
        ...
        {
            return item_.InvalidateInkOverflow();
        }
        void RecalcInkOverflow(const InlineCursor& cursor, InlinePaintContext* inline_context, PhysicalRect* self_and_contents_rect_out)
        {
    ...
    {
        return blink::IsEllipsis(GetStyleVariant());
    }
    ...
    wtf_size_t TextLength() const
    {
        return TextOffset().Length();
    }
    ...
    LayoutUnit CaretInlinePositionForOffset(StringView text, unsigned offset) const;

    // Compute line-relative coordinates for given offsets, this is not
    // flow-relative:
    // This returns scaled values for kSVGText type.
    // https://drafts.csswg.org/css-writing-modes-3/#line-directions
    std::pair<LayoutUnit, LayoutUnit> LineLeftAndRightForOffsets(StringView text, unsigned start_offset, unsigned end_offset) const;
    ...
    PhysicalRect LocalRect(StringView text, unsigned start_offset, unsigned end_offset) const;
    ...
    PhysicalRect ComputeTextBoundsRectForHitTest(const PhysicalOffset& inline_root_offset, bool is_occlusion_test) const;

    // Converts the given point, relative to the fragment itself, into a position
    // in DOM tree.
    PositionWithAffinity PositionForPointInText(const PhysicalOffset& point, const InlineCursor& cursor) const;
    PositionWithAffinity PositionForPointInText(unsigned text_offset, const InlineCursor& cursor) const;
    unsigned TextOffsetForPoint(const PhysicalOffset& point, const FragmentItems& items) const;

    // True if this item is associated with over/under ruby annotations.
    // These functions are valid only if IsText() is true.
    bool HasOverAnnotation() const
    {
        return has_over_annotation_;
    }
    bool HasUnderAnnotation() const
    {
        return has_under_annotation_;
    }
...
private:
    ...
    // Create a text item.
    FragmentItem(const InlineItem& inline_item, const ShapeResultView* shape_result, const TextOffsetRange& text_offset, const PhysicalSize& size,
    ...
    FragmentItem(const LayoutObject& layout_object, TextItemType text_type, StyleVariant style_variant, TextDirection direction,
    ...
    void RecalcInkOverflow(const InlineCursor& cursor, InlinePaintContext* inline_context, PhysicalRect* self_and_contents_rect_out);
    PhysicalRect RecalcInkOverflowForDescendantsOf(const InlineCursor& cursor, InlinePaintContext* inline_context) const;

    AffineTransform BuildSvgTransformForTextPath(const AffineTransform& length_adjust) const;
...
namespace WTF {
template <> struct VectorTraits<blink::FragmentItem> : VectorTraitsBase<blink::FragmentItem> {
