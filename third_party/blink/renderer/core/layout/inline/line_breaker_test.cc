...
String ToString(InlineItemResults line, InlineNode node)
{
    ...
    for (const auto& item_result : line) {
        builder.Append(StringView(text, item_result.StartOffset(), item_result.Length()));
    }
...
protected:
    ...
    Vector<std::pair<String, unsigned>> BreakLines(
        InlineNode node, LayoutUnit available_width, void (*callback)(const LineBreaker&, const LineInfo&) = nullptr, bool fill_first_space_ = false)
    {
        DCHECK(node);
        ...
        LineInfo line_info;
        do {
            LineBreaker line_breaker(node, LineBreakerMode::kContent, space, line_opportunity, leading_floats, break_token,
                /* column_spanner_path */ nullptr, &exclusion_space);
            line_breaker.NextLine(&line_info);
            if (callback)
                callback(line_breaker, line_info);
            trailing_whitespaces_.push_back(line_breaker.TrailingWhitespaceForTesting());
            ...
            lines.push_back(std::make_pair(ToString(line_info.Results(), node), line_info.Results().back().item_index));
        } while (break_token);

        return lines;
    }

    wtf_size_t BreakLinesAt(InlineNode node, LayoutUnit available_width, base::span<LineBreakPoint> break_points, base::span<LineInfo> line_info_list)
    {
        DCHECK(node);
        node.PrepareLayoutIfNeeded();
        ConstraintSpace space = ConstraintSpaceForAvailableSize(available_width);
    ...
    wtf_size_t BreakLines(InlineNode node, LayoutUnit available_width, base::span<LineInfo> line_info_list)
    {
...
TEST_F(LineBreakerTest, FitWithEpsilon)
{
    ...
    auto lines = BreakLines(node, LayoutUnit::FromFloatRound(50 - LayoutUnit::Epsilon()),
        [](const LineBreaker& line_breaker, const LineInfo& line_info) { EXPECT_FALSE(line_info.HasOverflow()); });
...
{
    LoadAhem();
    InsertStyleElement("#container {"
                       "  font: 10px/2 Ahem;"
                       "  writing-mode: vertical-lr;"
                       "}"
                       "tcy { text-combine-upright: all }");
    InlineNode node = CreateInlineNode("<div id=container>"
...
TEST_F(LineBreakerTest, IdeographicTrailingSpaces)
{
    ...
    for (LayoutUnit width : { LayoutUnit(60), LayoutUnit(90) }) {
        ...
        EXPECT_EQ(LineBreaker::WhitespaceState::kPreserved, trailing_whitespaces_[0]);
        EXPECT_EQ(LineBreaker::WhitespaceState::kPreserved, trailing_whitespaces_[1]);
    }
}
...
struct WhitespaceStateTestData {
    ...
    { "12 34<span>56</span>", "normal", LineBreaker::WhitespaceState::kCollapsed },
    { "12 34<span>56</span>", "pre-wrap", LineBreaker::WhitespaceState::kPreserved },
    // Atomic inlines.
    { "12 <span style='display: inline-block'></span>", "normal", LineBreaker::WhitespaceState::kNone },
...
std::ostream& operator<<(std::ostream& os, const WhitespaceStateTestData& data)
{
    return os << static_cast<int>(data.expected) << " for '" << data.html << "' with 'white-space: " << data.white_space << "'";
}

class WhitespaceStateTest : public LineBreakerTest, public testing::WithParamInterface<WhitespaceStateTestData> { };

INSTANTIATE_TEST_SUITE_P(LineBreakerTest, WhitespaceStateTest, testing::ValuesIn(whitespace_state_test_data));
...
} trailing_space_width_test_data[] = { { " ", "pre", 0 }, { "   ", "pre", 0 }, { "1 ", "pre", 0 }, { "1  ", "pre", 0 }, { "1<span> </span>", "pre", 0 },
    { "<span>1 </span> ", "pre", 0 }, { "1<span> </span> ", "pre", 0 }, { "1 <span> </span> ", "pre", 0 }, { "1 \t", "pre", 0 }, { "1  \n", "pre", 0 },
    { "1  <br>", "pre", 0 },

    { " ", "pre-wrap", 0 }, { "   ", "pre-wrap", 0 }, { "1 ", "pre-wrap", 0 }, { "1  ", "pre-wrap", 0 }, { "1<span> </span>", "pre-wrap", 0 },
    { "<span>1 </span> ", "pre-wrap", 0 }, { "1<span> </span> ", "pre-wrap", 0 }, { "1 <span> </span> ", "pre-wrap", 0 }, { "1 \t", "pre-wrap", 0 },
    { "1  <br>", "pre-wrap", 0 }, { "12 1234", "pre-wrap", 1 }, { "12  1234", "pre-wrap", 2 }, { "12  <br>1234", "pre-wrap", 0 }, { "12    ", "pre-wrap", 1 } };

class TrailingSpaceWidthTest : public LineBreakerTest, public testing::WithParamInterface<TrailingSpaceWidthTestData> { };

INSTANTIATE_TEST_SUITE_P(LineBreakerTest, TrailingSpaceWidthTest, testing::ValuesIn(trailing_space_width_test_data));
...
TEST_F(LineBreakerTest, SplitTextZero)
{
    ...
    To<Text>(GetElementById("target")->firstChild())->splitText(0, ASSERT_NO_EXCEPTION);
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(LineBreakerTest, ForcedBreakFollowedByCloseTag)
{
    ...
    for (const LayoutObject* child = container->SlowFirstChild(); child; child = child->NextSibling()) {
...
{
    InlineNode node = CreateInlineNode(uR"HTML(
      <!DOCTYPE html>
      <svg viewBox="0 0 800 600">
      <text id="container" rotate="1" style="font-family:Times">AV)HTML"
                                       u"\U0001F197\u05E2\u05B4\u05D1\u05E8\u05B4\u05D9\u05EA</text></svg>)");
    BreakLines(node, LayoutUnit::Max(), [](const LineBreaker& line_breaker, const LineInfo& line_info) {
...
TEST_F(LineBreakerTest, IdeographicSpaceBeforeEndBracket)
{
    ...
    InlineNode node1 = CreateInlineNode(uR"HTML(
<!DOCTYPE html>
<style>
...
全角空白の前では、変な行末があります。　]
    ...
    InlineNode node2 = CreateInlineNode(uR"HTML(
<!DOCTYPE html>
<style>
body { margin: 0; padding: 0; font: 10px/10px Ahem; }
</style>
...
TEST_F(LineBreakerTest, BreakAt)
{
    ...
    LineBreakPoint break_points[] { LineBreakPoint { { 0, 2 } }, LineBreakPoint { { 1, 6 } }, LineBreakPoint { { 2, 7 } } };
    LineInfo line_info_list[4];
    const wtf_size_t num_lines = BreakLinesAt(target, LayoutUnit(800), break_points, line_info_list);
...
TEST_F(LineBreakerTest, WideContentInRuby)
{
    ...
    LineBreaker line_breaker(
        node, LineBreakerMode::kContent, space, LineLayoutOpportunity(LayoutUnit::Max()), leading_floats, nullptr, nullptr, &exclusion_space);
    LineInfo line_info;
    line_breaker.NextLine(&line_info);
    EXPECT_EQ(InlineItem::kOpenRubyColumn, line_info.Results()[1].item->Type());
    // The base result should contain both <div>s.
    const auto& base_results = line_info.Results()[1].ruby_column->base_line.Results();
...
TEST_F(LineBreakerTest, SetInputRange)
{
    ...
    ExclusionSpace exclusion_space;
    LeadingFloats leading_floats;
    LineBreaker line_breaker(node, LineBreakerMode::kContent, ConstraintSpaceForAvailableSize(LayoutUnit::Max()), LineLayoutOpportunity(LayoutUnit::Max()),
        leading_floats, nullptr, nullptr, &exclusion_space);
    // <span> to just after </span>.
    line_breaker.SetInputRange({ 1, 6 }, 4, LineBreaker::WhitespaceState::kLeading, nullptr);
    LineInfo line_info;
    line_breaker.NextLine(&line_info);
...
TEST_F(LineBreakerTest, CreateSubLineInfoAvailableWidth)
{
    ...
    LineBreaker line_breaker(node, LineBreakerMode::kContent, space, LineLayoutOpportunity(width), leading_floats, nullptr, nullptr, &exclusion_space);
    LineInfo line_info;
    line_breaker.NextLine(&line_info);
...
class CanBreakInsideTest : public LineBreakerTest, public testing::WithParamInterface<CanBreakInsideTestData> { };
INSTANTIATE_TEST_SUITE_P(LineBreakerTest, CanBreakInsideTest, testing::ValuesIn(can_break_inside_test_data));
...
TEST_P(CanBreakInsideTest, Data)
{
    ...
    const wtf_size_t num_lines = BreakLines(target, available_width, line_info_list);
    ...
    LeadingFloats leading_floats;
    LineLayoutOpportunity line_opportunity(available_width);
    LineBreaker line_breaker(target, LineBreakerMode::kContent, space, line_opportunity, leading_floats, break_token,
        /* column_spanner_path */ nullptr, &exclusion_space);
    EXPECT_EQ(line_breaker.CanBreakInside(line_info_list[0]), data.can_break_insde);
}
