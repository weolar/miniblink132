...
public:
    ScoreLineBreaker(const InlineNode& node, const ConstraintSpace& space, const LineWidths& line_widths, const InlineBreakToken* break_token,
    ...
    // comment.
    void OptimalBreakPoints(const LeadingFloats& leading_floats, ScoreLineBreakContext& context);
    ...
    void BalanceBreakPoints(const LeadingFloats& leading_floats, ScoreLineBreakContext& context);
...
private:
    ...
    using LineBreakScores = Vector<LineBreakScore, LineBreakCandidate::kInlineCapacity>;

    const InlineNode& Node() const
    {
        return node_;
    }
    ...
    bool Optimize(const LineInfoList& line_info_list, LineBreaker& line_breaker, LineBreakPoints& break_points);
    bool ComputeCandidates(const LineInfoList& line_info_list, LineBreaker& line_breaker, LineBreakCandidates& candidates);
    void SetupParameters();
    void ComputeLineWidths(const LineInfoList& line_info_list);
    void ComputeScores(const LineBreakCandidates& candidates, LineBreakScores& scores);
    void ComputeBreakPoints(const LineBreakCandidates& candidates, const LineBreakScores& scores, LineBreakPoints& break_points);
