...
public:
    std::optional<LineWidths> ComputeLineWidths(InlineNode node)
    {
        ...
        BoxFragmentBuilder container_builder(node, &style, space, style.GetWritingDirection(),
        ...
        const LayoutOpportunityVector& opportunities = exclusion_space.AllLayoutOpportunities({ space.GetBfcOffset().line_offset,
...
class LineWidthsDataTest : public LineWidthsTest, public testing::WithParamInterface<LineWidthsData> { };
INSTANTIATE_TEST_SUITE_P(LineWidthsTest, LineWidthsDataTest, testing::ValuesIn(line_widths_data));
