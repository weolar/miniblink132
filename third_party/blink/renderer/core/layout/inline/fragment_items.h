...
public:
    ...
    Span Items() const
    {
        return base::span(items_.data(), items_.size());
    }
    ...
    static void FinalizeAfterLayout(const HeapVector<Member<const LayoutResult>, 1>& results, LayoutBlockFlow& container);
    ...
    const FragmentItem* EndOfReusableItems(const PhysicalBoxFragment& container) const;
    ...
    bool IsContainerForCulledInline(const LayoutInline&, bool* is_first_container, bool* is_last_container, bool* child_has_any_child_items) const;

    // Mark items dirty when |child| is removed from the tree.
    static void DirtyLinesFromChangedChild(const LayoutObject& child, const LayoutBlockFlow& container);
...
private:
    const FragmentItem* ItemsData() const
    {
        return items_.data();
    }

    static bool CanReuseAll(InlineCursor* cursor);
    static bool TryDirtyFirstLineFor(const LayoutObject& layout_object, const LayoutBlockFlow& container);
    static bool TryDirtyLastLineFor(const LayoutObject& layout_object, const LayoutBlockFlow& container);
