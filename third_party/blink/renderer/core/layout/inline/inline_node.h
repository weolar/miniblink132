...
public:
    ...
    const LayoutResult* Layout(const ConstraintSpace&, const BreakToken*, const ColumnSpannerPath*, InlineChildLayoutContext* context) const;

    // Computes the value of min-content and max-content for this anonymous block
    // box. min-content is the inline size when lines wrap at every break
    // opportunity, and max-content is when lines do not wrap at all.
    MinMaxSizesResult ComputeMinMaxSizes(WritingMode container_writing_mode, const ConstraintSpace&, const MinMaxSizesFloatInput&) const;
    ...
    static bool SetTextWithOffset(LayoutText* layout_text, String new_text, const TextDiffRange&);
    ...
    static const OffsetMapping* GetOffsetMapping(LayoutBlockFlow* layout_block_flow);

    bool IsBidiEnabled() const
    {
        return Data().is_bidi_enabled_;
    }
    ...
    {
        return Data().HasFloats();
    }
    bool HasInitialLetterBox() const
    {
        return Data().has_initial_letter_box_;
    }
    bool HasRuby() const
    {
        return Data().has_ruby_;
    }

    bool IsBlockLevel()
    {
        return EnsureData().is_block_level_;
    }
    ...
    const Vector<std::pair<unsigned, SvgCharacterData>>& SvgCharacterDataList() const;
    // This function is available after PrepareLayout(), only for SVG <text>.
...
protected:
    ...
    void CollectInlines(InlineNodeData*, InlineNodeData* previous_data = nullptr) const;
    const SvgTextChunkOffsets* FindSvgTextChunks(LayoutBlockFlow& block, InlineNodeData& data) const;
    ...
    void ShapeText(InlineItemsData*, const String* previous_text = nullptr, const HeapVector<InlineItem>* previous_items = nullptr,
        const Font* override_font = nullptr) const;
    void ShapeTextForFirstLineIfNeeded(InlineNodeData*) const;
    void ShapeTextIncludingFirstLine(InlineNodeData* data, const String* previous_text, const HeapVector<InlineItem>* previous_items) const;
    void AssociateItemsWithInlines(InlineNodeData*) const;
    bool IsNGShapeCacheAllowed(const String&, const Font*, const HeapVector<InlineItem>&, ShapeResultSpacing<String>&) const;
    ...
    static void ComputeOffsetMapping(LayoutBlockFlow* layout_block_flow, InlineNodeData* data);
    ...
    if (GetDocument().InQuirksMode()) [[unlikely]] {
        ...
        if (style.Display() == EDisplay::kTableCell) [[unlikely]] {
            ...
            if (!RuntimeEnabledFeatures::StricterCellWidthContentSizeQuirkEnabled()
                && (style.LogicalWidth().HasAutoOrContentOrIntrinsic() || style.LogicalWidth().HasStretch())) {
                return true;
            }
        }
    }
...
template <> struct DowncastTraits<InlineNode> {
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::InlineNode::FloatingObject)
