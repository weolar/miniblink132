...
const LayoutResult* LogicalLineItems::BlockInInlineLayoutResult() const
{
    for (const LogicalLineItem& item : *this) {
        if (item.layout_result && item.layout_result->GetPhysicalFragment().IsBlockInInline()) {
...
void LogicalLineItems::MoveInInlineDirection(LayoutUnit delta, unsigned start, unsigned end)
{
...
void LogicalLineItems::MoveInBlockDirection(LayoutUnit delta, unsigned start, unsigned end)
{
    for (unsigned index = start; index < end; index++)
