...
LineBoxStrut ComputeNegativeSideBearings(const PhysicalBoxFragment& box_fragment)
{
    const auto get_shape_result = [](const InlineCursor cursor) -> const ShapeResultView* {
        if (!cursor)
            return nullptr;
        const FragmentItem& item = *cursor.CurrentItem();
        if (item.Type() != FragmentItem::kText && item.Type() != FragmentItem::kGeneratedText) {
            return nullptr;
        }
    ...
    for (InlineCursor cursor(box_fragment); cursor; cursor.MoveToNextLine()) {
        ...
        if (auto* shape_result = get_shape_result(child_at_left_edge)) {
            const LayoutUnit left_bearing = LogicalRect::EnclosingRect(shape_result->ComputeInkBounds()).offset.inline_offset;
            side_bearing.inline_start = std::min(side_bearing.inline_start, left_bearing);
        }
        ...
        if (auto* shape_result = get_shape_result(child_at_right_edge)) {
            ...
            const LogicalRect text_ink_bounds = LogicalRect::EnclosingRect(shape_result->ComputeInkBounds());
            const LayoutUnit right_bearing = width - text_ink_bounds.InlineEndOffset();
            side_bearing.inline_end = std::min(side_bearing.inline_end, right_bearing);
        }
    }
...
bool ShouldApplyInlineKerning(const PhysicalBoxFragment& box_fragment)
{
    ...
    return style.IsHorizontalWritingMode() || style.GetTextOrientation() == ETextOrientation::kSideways;
}
...
inline bool IsAllBreakableSpaces(const String& string, unsigned start, unsigned end)
{
    DCHECK_GE(end, start);
    return StringView(string, start, end - start).IsAllSpecialCharacters<IsBreakableSpace>();
}
...
inline LayoutUnit HyphenAdvance(const ComputedStyle& style, bool is_ltr, const HyphenResult& hyphen_result, std::optional<LayoutUnit>& cache)
{
    if (cache) {
        return *cache;
    }
    const LayoutUnit size = hyphen_result ? hyphen_result.InlineSize() : HyphenResult(style).InlineSize();
...
{
    return type != InlineItem::kAtomicInline && type != InlineItem::kOutOfFlowPositioned && type != InlineItem::kInitialLetterBox
...
LayoutUnit ComputeInlineEndSize(const ConstraintSpace& space, const ComputedStyle* style)
{
    DCHECK(style);
...
{
    DCHECK(line_end_item.Type() == InlineItem::kText || line_end_item.Type() == InlineItem::kControl);
    DCHECK(line_end_item.Style());
    const ComputedStyle& line_end_style = *line_end_item.Style();
    return line_end_style.HasBoxDecorationBackground() || line_end_style.HasAppliedTextDecorations();
}

inline bool NeedsAccurateEndPosition(const LineInfo& line_info, const InlineItem& line_end_item)
{
    return line_info.NeedsAccurateEndPosition() || NeedsAccurateEndPosition(line_end_item);
}

inline void ComputeCanBreakAfter(InlineItemResult* item_result, bool auto_wrap, const LazyLineBreakIterator& break_iterator)
{
...
LayoutUnit ComputeFloatAncestorInlineEndSize(const ConstraintSpace& space, const HeapVector<InlineItem>& items, wtf_size_t item_index)
{
    LayoutUnit inline_end_size;
    for (const InlineItem *cur = items.data() + item_index, *end = items.data() + items.size(); cur != end; ++cur) {
...
void CollectCharIndex(void* context, unsigned char_index, Glyph, gfx::Vector2dF, float, bool, CanvasRotationInVertical, const SimpleFontData*)
{
...
LayoutUnit MaxLineWidth(const LineInfo& base_line, const HeapVector<LineInfo, 1>& annotation_lines)
{
...
public:
    ...
    void Add(const ShapeResult& shape_result, unsigned start_offset, unsigned end_offset, bool has_hyphen, InlineItemResult& item_result)
    {
    ...
    void AddHyphenated(const ShapeResult& shape_result, unsigned start_offset, unsigned end_offset, bool has_hyphen, InlineItemResult& item_result,
        const Hyphenation& hyphenation, const StringView& word)
    {
        ...
        for (const wtf_size_t location : locations) {
            ...
            LayoutUnit part_width = shape_result.CachedWidth(part_start_offset, end_offset);
            if (has_hyphen) {
...
void LineBreaker::UpdateAvailableWidth()
{
    ...
    available_width = std::max(available_width, cloned_box_decorations_initial_size_);
...
LineBreaker::LineBreaker(InlineNode node, LineBreakerMode mode, const ConstraintSpace& space, const LineLayoutOpportunity& line_opportunity,
    const LeadingFloats& leading_floats, const InlineBreakToken* break_token, const ColumnSpannerPath* column_spanner_path, ExclusionSpace* exclusion_space)
    ...
    , is_first_formatted_line_((!break_token || !break_token->IsPastFirstFormattedLine()) && node.CanContainFirstFormattedLine())
    , use_first_line_style_(is_first_formatted_line_ && node.UseFirstLineStyle())
    , sticky_images_quirk_(mode != LineBreakerMode::kContent && node.IsStickyImagesQuirkForContentSize())
    , items_data_(&node.ItemsData(use_first_line_style_))
    , end_item_index_(items_data_->items.size())
    , text_content_(!sticky_images_quirk_ ? items_data_->text_content : InlineNode::TextContentForStickyImagesQuirk(*items_data_))
    ...
    , base_direction_(node_.BaseDirection())
{
    ...
    if (is_svg_text_) {
        ...
        if (node_.SvgTextPathRangeList().empty() && node_.SvgTextLengthRangeList().empty()
            && (char_data_list.empty() || (char_data_list.size() == 1 && char_data_list[0].first == 0))) {
            needs_svg_segmentation_ = false;
        } else {
            needs_svg_segmentation_ = true;
            svg_resolved_iterator_ = std::make_unique<ResolvedTextLayoutAttributesIterator>(char_data_list);
        }
    }
    ...
    disallow_auto_wrap_ = is_svg_text_ || is_text_combine_ || is_initial_letter_box_;
...
void LineBreaker::SetLineOpportunity(const LineLayoutOpportunity& line_opportunity)
{
    line_opportunity_ = line_opportunity;
    UpdateAvailableWidth();
}
...
inline InlineItemResult* LineBreaker::AddItem(const InlineItem& item, unsigned end_offset, LineInfo* line_info)
{
    ...
    InlineItemResults* item_results = line_info->MutableResults();
    return &item_results->emplace_back(&item, current_.item_index, TextOffsetRange(current_.text_offset, end_offset), break_anywhere_if_overflow_,
        ShouldCreateLineBox(*item_results), HasUnpositionedFloats(*item_results));
}

inline InlineItemResult* LineBreaker::AddItem(const InlineItem& item, LineInfo* line_info)
{
    return AddItem(item, item.EndOffset(), line_info);
}

InlineItemResult* LineBreaker::AddEmptyItem(const InlineItem& item, LineInfo* line_info)
{
    InlineItemResult* item_result = AddItem(item, current_.text_offset, line_info);
...
void LineBreaker::SetIntrinsicSizeOutputs(MaxSizeCache* max_size_cache, bool* depends_on_block_constraints_out)
{
...
void LineBreaker::ComputeBaseDirection()
{
    ...
    if (previous_line_had_forced_break_) {
        ...
        start_offset = text.ReverseFind(kNewlineCharacter, current_.text_offset - 1);
    ...
    base_direction_ = BidiParagraph::BaseDirectionForStringOrLtr(StringView(text, start_offset),
...
void LineBreaker::RecalcClonedBoxDecorations()
{
    ...
    for (const InlineItem* item : open_items) {
        if (item->Style()->BoxDecorationBreak() == EBoxDecorationBreak::kClone) {
            ...
            ComputeOpenTagResult(*item, constraint_space_, is_svg_text_, &item_result);
            cloned_box_decorations_initial_size_ += item_result.inline_size;
            cloned_box_decorations_end_size_ += item_result.margins.inline_end + item_result.borders.inline_end + item_result.padding.inline_end;
        }
    }
...
// |position_|
LayoutUnit LineBreaker::AddHyphen(InlineItemResults* item_results, wtf_size_t index, InlineItemResult* item_result)
{
    DCHECK(!HasHyphen());
...
LayoutUnit LineBreaker::AddHyphen(InlineItemResults* item_results, wtf_size_t index)
{
...
LayoutUnit LineBreaker::AddHyphen(InlineItemResults* item_results, InlineItemResult* item_result)
{
    return AddHyphen(item_results, base::checked_cast<wtf_size_t>(item_result - item_results->data()), item_result);
}
...
void LineBreaker::RestoreLastHyphen(InlineItemResults* item_results)
{
    ...
    for (InlineItemResult& item_result : base::Reversed(*item_results)) {
        ...
        if (item.Type() == InlineItem::kText || item.Type() == InlineItem::kAtomicInline) {
            return;
        }
    }
}
...
void LineBreaker::PrepareNextLine(LineInfo* line_info)
{
    ...
    if (parent_breaker_) {
        previous_line_had_forced_break_ = parent_breaker_->previous_line_had_forced_break_;
        ...
        // is concerned).
        if (!RuntimeEnabledFeatures::LineBoxBelowLeadingFloatsEnabled() || !break_token_ || current_ != break_token_->Start()) {
    ...
    has_cloned_box_decorations_ = false;
    if ((break_token_ && break_token_->HasClonedBoxDecorations()) || cloned_box_decorations_count_) [[unlikely]] {
...
void LineBreaker::NextLine(LineInfo* line_info)
{
    ...
    if (break_token_ && break_token_->IsInParallelBlockFlow()) {
        ...
        DCHECK_EQ(item.GetLayoutObject(), block_break_token->InputNode().GetLayoutBox());
    ...
    const bool should_create_line_box
        = ShouldCreateLineBox(item_results) || (force_non_empty_if_last_line_ && line_info->IsLastLine()) || mode_ != LineBreakerMode::kContent;
...
void LineBreaker::BreakLine(LineInfo* line_info)
{
    ...
    while (state_ != LineBreakState::kDone) {
        ...
        const InlineItemResults& item_results = line_info->Results();
        if (state_ == LineBreakState::kOverflow && CanBreakAfterLast(item_results)) [[unlikely]] {
        ...
        if (item.Type() == InlineItem::kBlockInInline) {
            const BlockBreakToken* block_break_token = break_token_ ? break_token_->GetBlockBreakToken() : nullptr;
            HandleBlockInInline(item, block_break_token, line_info);
            continue;
        }
        if (item.Type() == InlineItem::kCloseRubyColumn || item.Type() == InlineItem::kRubyLinePlaceholder) {
        ...
        if (item.Type() == InlineItem::kOpenRubyColumn) {
            ...
            if (items[i + 1].Type() == InlineItem::kRubyLinePlaceholder
                && (items[i + 2].Type() == InlineItem::kCloseRubyColumn
                    || (items[i + 2].Type() == InlineItem::kRubyLinePlaceholder && items[i + 3].Type() == InlineItem::kCloseRubyColumn))) {
                AddItem(item, line_info);
                MoveToNextOf(item);
                continue;
            }
...
void LineBreaker::ComputeLineLocation(LineInfo* line_info) const
{
    ...
    line_info->SetWidth(available_width, position_ + cloned_box_decorations_end_size_);
    line_info->SetBfcOffset({ line_opportunity_.line_left_offset, line_opportunity_.bfc_block_offset });
    if (mode_ == LineBreakerMode::kContent) {
...
{
    DCHECK(item.Type() == InlineItem::kAtomicInline || item.Type() == InlineItem::kInitialLetterBox);
    if (!auto_wrap_) {
        return false;
    }
    ...
    if (auto* next_text_combine = MayBeTextCombine(atomic_inline_item)) {
        ...
        text_content.Append(StringView(Text(), item.EndOffset(), Text().length() - item.EndOffset()));
    }

    DCHECK_EQ(Text(), break_iterator_.GetString());
    LazyLineBreakIterator break_iterator(break_iterator_, text_content.ReleaseString());
...
bool LineBreaker::CanBreakAfter(const InlineItem& item) const
{
    ...
    if (ignorable_bidi_length > 0u) {
        return break_iterator_.IsBreakable(item.EndOffset() + ignorable_bidi_length);
    }
    auto* const atomic_inline_item = TryGetAtomicInlineItemAfter(item);
    ...
    // We can not break before sticky images quirk was applied.
    if (Text()[atomic_inline_item->StartOffset()] == kNoBreakSpaceCharacter) [[unlikely]] {
...
const InlineItem* LineBreaker::TryGetAtomicInlineItemAfter(const InlineItem& item) const
{
    DCHECK(auto_wrap_);
    const String& text = Text();
    if (item.EndOffset() == text.length())
    ...
    for (const auto& next_item : base::span(Items().data(), Items().size()).subspan(items_data_->ToItemIndex(item) + 1)) {
...
unsigned LineBreaker::IgnorableBidiControlLength(const InlineItem& item) const
{
    ...
    for (wtf_size_t i = base::checked_cast<wtf_size_t>(std::distance(items, &item)) + 1; i < end_item_index_; ++i) {
        if (items[i].Length() == 0u) {
            continue;
        }
        if (items[i].Type() != InlineItem::kOpenRubyColumn && items[i].Type() != InlineItem::kCloseRubyColumn) {
            return items[i].StartOffset() - item.EndOffset();
        }
    }
    return (end_item_index_ >= Items().size() ? Text().length() : items[end_item_index_].StartOffset()) - item.EndOffset();
}

void LineBreaker::HandleText(const InlineItem& item, const ShapeResult& shape_result, LineInfo* line_info)
{
    DCHECK(item.Type() == InlineItem::kText || (item.Type() == InlineItem::kControl && Text()[item.StartOffset()] == kTabulationCharacter));
    DCHECK(&shape_result);
    ...
    if (trailing_whitespace_ == WhitespaceState::kLeading) {
        if (item.Style()->ShouldCollapseWhiteSpaces() && Text()[current_.text_offset] == kSpaceCharacter) {
    ...
    if (auto_wrap_) {
        ...
        if (mode_ == LineBreakerMode::kMinContent && !parent_breaker_ && HandleTextForFastMinContent(item_result, item, shape_result, line_info)) {
            return;
        }

        // Try to break inside of this text item.
        const LayoutUnit available_width = RemainingAvailableWidth();
        BreakResult break_result = BreakText(item_result, item, shape_result, available_width, available_width, line_info);
        DCHECK(item_result->shape_result || !item_result->TextOffset().Length()
            || (break_result == kOverflow && break_anywhere_if_overflow_ && !override_break_anywhere_));
        ...
        if (item_result->has_only_pre_wrap_trailing_spaces) {
            state_ = LineBreakState::kTrailing;
            if (item_result->item->Style()->ShouldPreserveWhiteSpaces() && IsBreakableSpace(Text()[item_result->EndOffset() - 1])) {
                unsigned end_index = base::checked_cast<unsigned>(item_result - line_info->Results().data());
        ...
        if (IsAllBreakableSpaces(Text(), item_result->StartOffset(), item_result->EndOffset()))
            return;
    ...
    if (item_result->StartOffset() == item.StartOffset()) {
        ...
        DCHECK(trailing_whitespace_ == WhitespaceState::kLeading && item_result->StartOffset() >= item.StartOffset());
        item_result->shape_result = ShapeResultView::Create(&shape_result, item_result->StartOffset(), item_result->EndOffset());
        item_result->inline_size = item_result->shape_result->SnappedWidth().ClampNegativeToZero();
    }
...
void LineBreaker::SplitTextIntoSegments(const InlineItem& item, LineInfo* line_info)
{
    DCHECK(is_svg_text_);
    ...
    for (wtf_size_t i = 0; i < size; ++i) {
        ...
        for (; glyph_start < glyph_end; glyph_start = text_view.NextCodePointOffset(glyph_start)) {
        ...
        result->should_create_line_box = true;
        auto* shape_result_view = ShapeResultView::Create(&shape, current_.text_offset, glyph_end);
...
bool LineBreaker::ShouldCreateNewSvgSegment() const
{
    ...
    for (const auto& range : node_.SvgTextPathRangeList()) {
        if (range.start_index <= svg_addressable_offset_ && svg_addressable_offset_ <= range.end_index)
            return true;
    }
    for (const auto& range : node_.SvgTextLengthRangeList()) {
        if (To<SVGTextContentElement>(range.layout_object->GetNode())->lengthAdjust()->CurrentEnumValue() == kSVGLengthAdjustSpacingAndGlyphs)
            continue;
        if (range.start_index <= svg_addressable_offset_ && svg_addressable_offset_ <= range.end_index)
            return true;
    }
    const SvgCharacterData& char_data = svg_resolved_iterator_->AdvanceTo(svg_addressable_offset_);
    return char_data.HasRotate() || char_data.HasX() || char_data.HasY() || char_data.HasDx() || char_data.HasDy();
}

LineBreaker::BreakResult LineBreaker::BreakText(InlineItemResult* item_result, const InlineItem& item, const ShapeResult& item_shape_result,
    LayoutUnit available_width, LayoutUnit available_width_with_hyphens, LineInfo* line_info)
{
    DCHECK(item.Type() == InlineItem::kText || (item.Type() == InlineItem::kControl && Text()[item.StartOffset()] == kTabulationCharacter));
    ...
    public:
        ShapingLineBreakerImpl(LineBreaker* line_breaker, const InlineItem* item, const ShapeResult* result)
            : ShapingLineBreaker(result, &line_breaker->break_iterator_, line_breaker->hyphenation_, &item->Style()->GetFont())
    ...
    protected:
        const ShapeResult* Shape(unsigned start, unsigned end, ShapeOptions options) final
        {
    ...
    while (true) {
...
#endif
        const ShapeResultView* shape_result = breaker.ShapeLine(item_result->StartOffset(), available_width.ClampNegativeToZero(), &result);
        ...
        DCHECK_EQ(shape_result->NumCharacters(), result.break_offset - item_result->StartOffset());
        ...
        if (result.is_hyphenated) [[unlikely]] {
            InlineItemResults* item_results = line_info->MutableResults();
            const LayoutUnit hyphen_inline_size = AddHyphen(item_results, item_result);
            // If the hyphen overflows, retry with the reduced available width.
            if (!result.is_overflow && inline_size <= available_width) {
                const LayoutUnit space_for_hyphen = available_width_with_hyphens - inline_size;
    ...
    if (item_result->EndOffset() < item.EndOffset()) {
        ...
        if (break_iterator_.BreakType() == LineBreakType::kBreakCharacter) [[unlikely]] {
            trailing_whitespace_ = WhitespaceState::kUnknown;
        } else {
...
bool LineBreaker::BreakTextAt(InlineItemResult* item_result, const InlineItem& item, ShapingLineBreaker& breaker, LineInfo* line_info)
{
    DCHECK(break_at_);
    ...
    if (item_result->Length()) {
        const ShapeResultView* shape_result = breaker.ShapeLineAt(item_result->StartOffset(), item_result->EndOffset());
        item_result->inline_size = shape_result->SnappedWidth().ClampNegativeToZero();
        item_result->shape_result = shape_result;
...
// opportunities.
bool LineBreaker::BreakTextAtPreviousBreakOpportunity(InlineItemResult* item_result)
{
    DCHECK(item_result->item);
    DCHECK(item_result->may_break_inside);
    const InlineItem& item = *item_result->item;
    ...
    unsigned break_opportunity = break_iterator_.PreviousBreakOpportunity(item_result->EndOffset() - 1, item_result->StartOffset());
    if (break_opportunity <= item_result->StartOffset())
        return false;
    item_result->text_offset.end = break_opportunity;
    item_result->text_offset.AssertNotEmpty();
    item_result->shape_result = ShapeResultView::Create(item.TextShapeResult(), item_result->StartOffset(), item_result->EndOffset());
    item_result->inline_size = item_result->shape_result->SnappedWidth().ClampNegativeToZero();
    item_result->can_break_after = true;

    if (trailing_collapsible_space_.has_value() && trailing_collapsible_space_->item_result == item_result) {
...
// previous/next item.
bool LineBreaker::HandleTextForFastMinContent(InlineItemResult* item_result, const InlineItem& item, const ShapeResult& shape_result, LineInfo* line_info)
{
    ...
    if (indent) [[unlikely]] {
        ...
        DCHECK_EQ(should_break_at_first_opportunity, is_at_mid_word || (has_cloned_box_decorations_ && cloned_box_decorations_initial_size_ > LayoutUnit()));
#endif // EXPENSIVE_DCHECKS_ARE_ON()
    }
    ...
    while (start_offset < end_offset) {
        ...
        next_break = break_iterator_.NextBreakOpportunity(start_offset + 1, std::min(item_end_offset + 1, text.length()));
        ...
        if (!should_break_spaces) {
            while (non_hangable_run_end > start_offset && IsBreakableSpace(text[non_hangable_run_end - 1])) {
        ...
        if (const wtf_size_t word_len = non_hangable_run_end - start_offset) {
            bool has_hyphen = can_break_after && text[non_hangable_run_end - 1] == kSoftHyphenCharacter;
            if (hyphenation_) [[unlikely]] {
                const StringView word(text, start_offset, word_len);
                if (should_break_at_first_opportunity) [[unlikely]] {
                    if (const wtf_size_t location = hyphenation_->FirstHyphenLocation(word, 0)) {
                        next_break = non_hangable_run_end = start_offset + location;
                        has_hyphen = can_break_after = true;
                    }
                    context.Add(shape_result, start_offset, non_hangable_run_end, has_hyphen, *item_result);
                } else {
                    context.AddHyphenated(shape_result, start_offset, non_hangable_run_end, has_hyphen, *item_result, *hyphenation_, word);
                }
            } else {
                context.Add(shape_result, start_offset, non_hangable_run_end, has_hyphen, *item_result);
            }
    ...
    if (item_style.ShouldCollapseWhiteSpaces()) {
        ...
        trailing_whitespace_ = non_hangable_run_end != next_break ? WhitespaceState::kCollapsed : WhitespaceState::kNone;
    } else {
        item_result->text_offset.end = next_break;
        trailing_whitespace_ = non_hangable_run_end != next_break ? WhitespaceState::kPreserved : WhitespaceState::kNone;
    }
    item_result->text_offset.AssertValid();
...
const ShapeResult* LineBreaker::ShapeText(const InlineItem& item, unsigned start, unsigned end, ShapeOptions options)
{
    ShapeResult* shape_result = nullptr;
    if (!items_data_->segments) {
        RunSegmenter::RunSegmenterRange segment_range = InlineItemSegment::UnpackSegmentData(start, end, item.SegmentData());
        shape_result = shaper_.Shape(&item.Style()->GetFont(), item.Direction(), start, end, segment_range, options);
    } else {
        shape_result = items_data_->segments->ShapeText(
            &shaper_, &item.Style()->GetFont(), item.Direction(), start, end, base::checked_cast<unsigned>(&item - items_data_->items.data()), options);
    }
...
void LineBreaker::AppendCandidates(const InlineItemResult& item_result, const LineInfo& line_info, LineBreakCandidateContext& context)
{
    DCHECK(item_result.item);
    const InlineItem& item = *item_result.item;
    const wtf_size_t item_index = item_result.item_index;
    DCHECK(context.GetState() == LineBreakCandidateContext::kBreak || !context.Candidates().empty());

    DCHECK_EQ(item.Type(), InlineItem::kText);
    if (!item.Length()) {
        // Fully collapsed spaces don't have break opportunities.
        context.AppendTrailingSpaces(item_result.can_break_after ? LineBreakCandidateContext::kBreak : context.GetState(),
    ...
    public:
        ...
        float PositionForOffset(unsigned offset) const
        {
            ...
            const float position = shape_result->CachedPositionForOffset(offset - shape_result_start_index);
    ...
    offset.end = std::max(offset.end, std::min(item.EndOffset(), line_info.EndTextOffset()));
    ...
    if (context.LastItem()) {
        ...
        if (IsBreakableSpace(text_content[offset.start])) {
            ...
            do {
                ++offset.start;
            } while (offset.start < offset.end && IsBreakableSpace(text_content[offset.start]));
            const float end_position = shape_result.PositionForOffset(offset.start);
            if (!offset.Length()) {
                context.AppendTrailingSpaces(item_result.can_break_after ? LineBreakCandidateContext::kBreak : LineBreakCandidateContext::kMidWord,
                    { item_index, offset.start }, end_position);
                context.SetLast(&item, offset.end);
                return;
            }
            context.AppendTrailingSpaces(
                auto_wrap_ ? LineBreakCandidateContext::kBreak : LineBreakCandidateContext::kMidWord, { item_index, offset.start }, end_position);
        }
    } else {
        shape_result.SetBasePosition(offset.start, context.Position());
    }
    ...
    for (;;) {
        ...
        float next_position;
        float end_position;
        LineBreakCandidateContext::State next_state = LineBreakCandidateContext::kBreak;
        float penalty = 0;
        ...
        if (next_offset > offset.end) {
            ...
            end_position = next_position = shape_result.PositionForOffset(next_offset);
            ...
            if (hyphenation_) [[unlikely]] {
                const LayoutUnit hyphen_advance = HyphenAdvance(*current_style_, shape_result.IsLtr(), item_result.hyphen, hyphen_advance_cache);
                DCHECK_GT(end_offset, offset.start);
                ...
                for (const wtf_size_t location : base::Reversed(locations)) {
                    ...
                    const float position = shape_result.PositionForOffset(hyphen_offset.text_offset);
                    context.Append(LineBreakCandidateContext::kBreak, hyphen_offset, hyphen_offset, position, position + hyphen_advance, hyphen_penalty,
                        /*is_hyphenated*/ true);
                }
            }
            ...
            case LineBreakCandidateContext::kBreak:
                ...
                if (end_safe_offset < offset.start) {
                    DCHECK_EQ(context.Candidates().back().offset.text_offset, offset.start);
            ...
            if (end_safe_offset == end_offset) {
                ...
                const ShapeResult* end_shape_result = ShapeText(item, end_safe_offset, end_offset);
            ...
            } else if (last_ch == kSoftHyphenCharacter && next_state == LineBreakCandidateContext::kBreak) [[unlikely]] {
                is_hyphenated = true;
            }
            if (is_hyphenated) {
                end_position += HyphenAdvance(*current_style_, shape_result.IsLtr(), item_result.hyphen, hyphen_advance_cache);
                penalty = context.HyphenPenalty();
            }
        }

        context.Append(next_state, { item_index, next_offset }, { item_index, end_offset }, next_position, end_position, penalty, is_hyphenated);
        if (next_offset >= offset.end) {
            break;
        }
...
{
    const InlineItemResults& item_results = line_info.Results();
    for (const InlineItemResult& item_result : base::make_span(item_results.begin(), item_results.size() - 1)) {
        if (item_result.can_break_after) {
            return true;
        }
    }
...
bool LineBreaker::CanBreakInside(const InlineItemResult& item_result)
{
    ...
    const wtf_size_t next_offset = break_iterator_.NextBreakOpportunity(offset.start + 1);
...
const ShapeResultView* LineBreaker::TruncateLineEndResult(const LineInfo& line_info, const InlineItemResult& item_result, unsigned end_offset)
{
    DCHECK(item_result.item);
    const InlineItem& item = *item_result.item;
    ...
    DCHECK(start_offset > source_result->StartIndex() || end_offset < source_result->EndIndex());
    ...
    const ShapeResult* end_result = ShapeText(item, std::max(last_safe, start_offset), end_offset);
...
void LineBreaker::UpdateShapeResult(const LineInfo& line_info, InlineItemResult* item_result)
{
    DCHECK(item_result);
    item_result->shape_result = TruncateLineEndResult(line_info, *item_result, item_result->EndOffset());
    DCHECK(item_result->shape_result);
    item_result->inline_size = item_result->shape_result->SnappedWidth();
}

inline void LineBreaker::HandleTrailingSpaces(const InlineItem& item, LineInfo* line_info)
{
...
void LineBreaker::HandleTrailingSpaces(const InlineItem& item, const ShapeResult* shape_result, LineInfo* line_info)
{
    DCHECK(item.Type() == InlineItem::kText || (item.Type() == InlineItem::kControl && Text()[item.StartOffset()] == kTabulationCharacter));
    bool is_control_tab = item.Type() == InlineItem::kControl && Text()[item.StartOffset()] == kTabulationCharacter;
    ...
    if (style.ShouldCollapseWhiteSpaces() && !Character::IsOtherSpaceSeparator(text[current_.text_offset])) {
        if (text[current_.text_offset] != kSpaceCharacter) {
            if (current_.text_offset > 0 && IsBreakableSpace(text[current_.text_offset - 1])) {
        ...
        DCHECK(style.ShouldBreakOnlyAfterWhiteSpace() || Character::IsOtherSpaceSeparator(text[current_.text_offset]));
        unsigned end = current_.text_offset;
        while (end < item.EndOffset() && IsBreakableSpaceOrOtherSeparator(text[end]))
            end++;
        ...
        item_result->shape_result = ShapeResultView::Create(shape_result);
        if (item_result->StartOffset() == item.StartOffset() && item_result->EndOffset() == item.EndOffset()) {
            item_result->inline_size
                = item_result->shape_result && mode_ != LineBreakerMode::kMinContent ? item_result->shape_result->SnappedWidth() : LayoutUnit();
        } else {
        ...
        position_ += item_result->inline_size;
        item_result->can_break_after = end < text.length() && !IsBreakableSpaceOrOtherSeparator(text[end]);
...
void LineBreaker::RewindTrailingOpenTags(LineInfo* line_info)
{
    ...
    for (const InlineItemResult& item_result : base::Reversed(item_results)) {
        ...
        if (item_result.item->Type() != InlineItem::kOpenTag) {
            unsigned end_index = base::checked_cast<unsigned>(&item_result - item_results.data() + 1);
...
void LineBreaker::RemoveTrailingCollapsibleSpace(LineInfo* line_info)
{
    ...
    position_ -= item_result->inline_size;
    if (const ShapeResultView* collapsed_shape_result = trailing_collapsible_space_->collapsed_shape_result) {
...
{
    if (trailing_whitespace_ == WhitespaceState::kLeading || trailing_whitespace_ == WhitespaceState::kNone
        || trailing_whitespace_ == WhitespaceState::kCollapsed || trailing_whitespace_ == WhitespaceState::kPreserved) {
        trailing_collapsible_space_.reset();
        return;
    }
    DCHECK(trailing_whitespace_ == WhitespaceState::kUnknown || trailing_whitespace_ == WhitespaceState::kCollapsible);
        ...
        if (item.Type() == InlineItem::kText) {
            ...
            if (!trailing_collapsible_space_.has_value() || trailing_collapsible_space_->item_result != &item_result) {
                ...
                if (item_result.EndOffset() - 1 > item_result.StartOffset()) {
                    trailing_collapsible_space_->collapsed_shape_result = TruncateLineEndResult(*line_info, item_result, item_result.EndOffset() - 1);
                }
            }
            trailing_whitespace_ = WhitespaceState::kCollapsible;
            return;
...
{
    DCHECK(trailing_whitespace_ == WhitespaceState::kLeading || trailing_whitespace_ == WhitespaceState::kNone
        || trailing_whitespace_ == WhitespaceState::kCollapsed || trailing_whitespace_ == WhitespaceState::kPreserved);
    ...
    for (auto& item_result : base::Reversed(*line_info->MutableResults())) {
        ...
        if (item_result.has_only_bidi_trailing_spaces || item.EndCollapseType() == InlineItem::kOpaqueToCollapsing
            || item.TextType() == TextItemType::kForcedLineBreak) {
            continue;
        }

        if (item.Type() != InlineItem::kText && item.Type() != InlineItem::kControl) {
            return;
        }
        ...
        for (; i > item_result.StartOffset() && (IsBreakableSpace(text[i - 1]) || IsBidiTrailingSpace(text[i - 1])); i--) {
        }
        ...
        if (i == item_result.StartOffset()) {
            ...
            if (item.BidiLevel() != (UBiDiLevel)base_direction_) {
                const ShapeResultView* source_shape_result = item_result.shape_result.Get();
                ...
                item_result.shape_result = ShapeResultView::Create(source_shape_result, start, i);
                item_result.inline_size = item_result.shape_result->SnappedWidth();
                DCHECK_LE(item_result.inline_size, prev_inline_size);

                InlineItemResult spaces_result(&item, item_result.item_index, TextOffsetRange(i, end), item_result.break_anywhere_if_overflow,
                    item_result.should_create_line_box, item_result.has_unpositioned_floats);
                spaces_result.has_only_bidi_trailing_spaces = true;
                spaces_result.shape_result = ShapeResultView::Create(source_shape_result, i, end);
                spaces_result.inline_size = prev_inline_size - item_result.inline_size;

                line_info->MutableResults()->insert(result_index + 1, std::move(spaces_result));
            }
            break;
        }
    }
...
void LineBreaker::HandleForcedLineBreak(const InlineItem* item, LineInfo* line_info)
{
    ...
    if (item) {
        ...
        if (constraint_space_.HasBlockFragmentation() && item->GetLayoutObject() && item->GetLayoutObject()->IsBR()
            && exclusion_space_->NeedsClearancePastFragmentainer(item->Style()->Clear(*current_style_))) {
...
void LineBreaker::HandleControlItem(const InlineItem& item, LineInfo* line_info)
{
    ...
    case kTabulationCharacter: {
        ...
        const ShapeResult* shape_result
            = ShapeResult::CreateForTabulationCharacters(&style.GetFont(), item.Direction(), style.GetTabSize(), position_, item.StartOffset(), item.Length());
...
void LineBreaker::HandleBidiControlItem(const InlineItem& item, LineInfo* line_info)
{
...
void LineBreaker::HandleAtomicInline(const InlineItem& item, LineInfo* line_info)
{
    DCHECK(item.Type() == InlineItem::kAtomicInline || item.Type() == InlineItem::kInitialLetterBox);
    DCHECK(item.Style());
    const ComputedStyle& style = *item.Style();
    ...
    if (state_ == LineBreakState::kContinue && remaining_width < 0 && (!parent_breaker_ || auto_wrap_)) {
    ...
    // the current line.
    InlineItemResult* item_result = AddItem(item, line_info);
    item_result->margins = ComputeLineMarginsForVisualContainer(constraint_space_, style);
    ...
    const bool is_initial_letter_box = item.Type() == InlineItem::kInitialLetterBox;
    ...
    if (mode_ == LineBreakerMode::kContent || [&] {
        ...
        BaselineAlgorithmType baseline_algorithm_type
            = style.BaselineSource() == EBaselineSource::kAuto ? BaselineAlgorithmType::kInlineBlock : BaselineAlgorithmType::kDefault;
        ...
        item_result->layout_result = BlockNode(To<LayoutBox>(item.GetLayoutObject()))
                                         .LayoutAtomicInline(constraint_space_, node_.Style(),
                                             /* use_first_line_style */ false, baseline_algorithm_type);
        // Ensure `NeedsCollectInlines` isn't set, or it may cause security risks.
        CHECK(!node_.GetLayoutBox()->NeedsCollectInlines());

        const auto& physical_box_fragment = To<PhysicalBoxFragment>(item_result->layout_result->GetPhysicalFragment());
        item_result->inline_size = LogicalFragment(constraint_space_.GetWritingDirection(), physical_box_fragment).InlineSize();

        if (is_initial_letter_box && ShouldApplyInlineKerning(physical_box_fragment)) [[unlikely]] {
            // Apply "Inline Kerning" to the initial letter box[1].
            // [1] https://drafts.csswg.org/css-inline/#initial-letter-inline-position
            const LineBoxStrut side_bearing = ComputeNegativeSideBearings(physical_box_fragment);
        ...
        DCHECK(mode_ == LineBreakerMode::kMaxContent || mode_ == LineBreakerMode::kMinContent);
...
void LineBreaker::ComputeMinMaxContentSizeForBlockChild(const InlineItem& item, InlineItemResult* item_result)
{
    DCHECK(mode_ == LineBreakerMode::kMaxContent || mode_ == LineBreakerMode::kMinContent);
    if (mode_ == LineBreakerMode::kMaxContent && max_size_cache_) {
        const unsigned item_index = base::checked_cast<unsigned>(&item - Items().data());
    ...
    builder.SetPercentageResolutionBlockSize(constraint_space_.PercentageResolutionBlockSize());
    builder.SetReplacedPercentageResolutionBlockSize(constraint_space_.ReplacedPercentageResolutionBlockSize());
    const auto space = builder.ToConstraintSpace();

    const MinMaxSizesResult result = ComputeMinAndMaxContentContribution(node_.Style(), child, space);
    // Ensure `NeedsCollectInlines` isn't set, or it may cause security risks.
    CHECK(!node_.GetLayoutBox()->NeedsCollectInlines());
...
void LineBreaker::HandleBlockInInline(const InlineItem& item, const BlockBreakToken* block_break_token, LineInfo* line_info)
{
    DCHECK_EQ(item.Type(), InlineItem::kBlockInInline);
    DCHECK(!block_break_token || block_break_token->InputNode().GetLayoutBox() == item.GetLayoutObject());
    ...
    if (mode_ == LineBreakerMode::kContent) {
        ...
        constraint_space_.GetExclusionSpace().MoveAndUpdateDerivedGeometry(*exclusion_space_);

        BlockNode block_node(To<LayoutBox>(item.GetLayoutObject()));
        std::optional<ConstraintSpace> modified_space;
        const ConstraintSpace& child_space = constraint_space_.CloneForBlockInInlineIfNeeded(modified_space);
        const ColumnSpannerPath* spanner_path_for_child = FollowColumnSpannerPath(column_spanner_path_, block_node);
        const LayoutResult* layout_result = block_node.Layout(child_space, block_break_token,
        ...
        item_result->inline_size = LogicalFragment(constraint_space_.GetWritingDirection(), fragment).InlineSize();

        item_result->should_create_line_box = !layout_result->IsSelfCollapsing();
        item_result->layout_result = layout_result;

        if (const auto* outgoing_block_break_token = To<BlockBreakToken>(layout_result->GetPhysicalFragment().GetBreakToken())) {
            ...
            if (outgoing_block_break_token->IsAtBlockEnd()) {
                const auto* parallel_token = InlineBreakToken::CreateForParallelBlockFlow(node_, current_, *outgoing_block_break_token);
...
bool LineBreaker::HandleRuby(LineInfo* line_info, LayoutUnit retry_size)
{
    ...
    if (!ruby_token) {
        ...
        RubyItemIndexes ruby_indexes = ParseRubyInInlineItems(Items(), current_.item_index);
        ...
        base_start = { current_.item_index + 1, Items()[current_.item_index].EndOffset() };

        wtf_size_t start = ruby_indexes.annotation_start;
        annotation_data.push_back(AnnotationBreakTokenData { { start, Items()[start].StartOffset() }, start, ruby_indexes.column_end });
    } else {
    ...
    LineInfo base_line_info = CreateSubLineInfo(base_start, base_end_index, LineBreakerMode::kMaxContent, kIndefiniteSize, trailing_whitespace_);
    ...
    for (const auto& data : annotation_data) {
        annotation_line_list.push_back(
            CreateSubLineInfo(data.start, data.end_item_index, LineBreakerMode::kMaxContent, kIndefiniteSize, WhitespaceState::kLeading));
        annotation_line_list.back().OverrideLineStyle(Items()[data.start_item_index].GetLayoutObject()->StyleRef());
    }
    ...
    AnnotationOverhang overhang = GetOverhang(ruby_size, base_line_info, annotation_line_list);
    if (!CanApplyStartOverhang(*line_info, line_info->Results().size(), *current_style_, overhang.start)) {
        overhang.start = LayoutUnit();
    }
    bool is_monolithic = IsMonolithicRuby(base_line_info, annotation_line_list);
    if ((retry_size == kIndefiniteSize && ruby_size <= available + overhang.start) || is_monolithic) {
        if (mode_ == LineBreakerMode::kContent) {
            // Recreate lines because lines created with LineBreakerMode::kMaxContent
            // are not usable in InlineLayoutAlgorithm.
            base_line_info = CreateSubLineInfo(base_start, base_end_index, LineBreakerMode::kContent, kIndefiniteSize, trailing_whitespace_);
            for (wtf_size_t i = 0; i < annotation_data.size(); ++i) {
                annotation_line_list[i] = CreateSubLineInfo(
                    annotation_data[i].start, annotation_data[i].end_item_index, LineBreakerMode::kContent, kIndefiniteSize, WhitespaceState::kLeading);
            }
        }

        InlineItemResult* result = AddRubyColumnResult(item, base_line_info, annotation_line_list, annotation_data, ruby_size, ruby_token, *line_info);
    ...
    LayoutUnit base_target = retry_size == kIndefiniteSize ? (available * base_intrinsic_size / ruby_size) : retry_size - 1;
    base_line_info = CreateSubLineInfo(base_start, base_end_index, mode_, base_target, trailing_whitespace_);
    ...
    for (wtf_size_t i = 0; i < number_of_annotations; ++i) {
        ...
        line = CreateSubLineInfo(annotation_data[i].start, annotation_data[i].end_item_index, mode, limit, WhitespaceState::kLeading);
    ...
    for (wtf_size_t i = 0; i < number_of_annotations; ++i) {
        breaks.push_back(AnnotationBreakTokenData { annotation_line_list[i].End(), annotation_data[i].start_item_index, annotation_data[i].end_item_index });
    }
    result->ruby_column->end_ruby_break_token = MakeGarbageCollected<RubyBreakTokenData>(open_column_item_index, base_end_index, breaks);
...
bool LineBreaker::IsMonolithicRuby(const LineInfo& base_line, const HeapVector<LineInfo, 1>& annotation_line_list) const
{
    ...
    if (!base_line.GlyphCountIsGreaterThan(kBaseLetterLimit)) {
        auto iter = std::find_if(annotation_line_list.begin(), annotation_line_list.end(),
            [](const LineInfo& line) { return line.GlyphCountIsGreaterThan(kAnnotationLetterLimit); });
...
LineInfo LineBreaker::CreateSubLineInfo(
    InlineItemTextIndex start, wtf_size_t end_item_index, LineBreakerMode mode, LayoutUnit limit, WhitespaceState initial_whitespace_state)
{
    ...
    LineBreaker sub_line_breaker(node_, mode, constraint_space_, LineLayoutOpportunity(limit), empty_leading_floats,
        /* break_token */ nullptr,
        /* column_spanner_path */ nullptr, &empty_exclusion_space);
    sub_line_breaker.disallow_auto_wrap_ = disallow_auto_wrap;
    sub_line_breaker.SetInputRange(start, end_item_index, initial_whitespace_state, this);
...
InlineItemResult* LineBreaker::AddRubyColumnResult(const InlineItem& item, const LineInfo& base_line_info, const HeapVector<LineInfo, 1>& annotation_line_list,
    const Vector<AnnotationBreakTokenData, 1>& annotation_data_list, LayoutUnit ruby_size, bool is_continuation, LineInfo& line_info)
{
    ...
    for (wtf_size_t i = 0; i < annotation_line_list.size(); ++i) {
        LayoutObject& annotation_object = *Items()[annotation_data_list[i].start_item_index].GetLayoutObject();
        ...
        data->position_list.push_back(parent->IsInlineRuby() ? parent->Style(use_first_line_style_)->GetRubyPosition() : RubyPosition::kOver);
    }
    ...
    column_result->can_break_after = CanBreakAfterRubyColumn(*column_result, annotation_data_list[0].end_item_index);
    ...
    if (base_line_info.Width() < ruby_size) {
        ...
        if (CanApplyStartOverhang(line_info, line_info.Results().size() - 1,
                column_result->item->GetLayoutObject() ? *column_result->item->Style() : *current_style_, overhang.start)) {
            DCHECK_EQ(column_result->margins.inline_start, LayoutUnit());
            DCHECK_EQ((*column_result->ruby_column->base_line.MutableResults())[0].item->Type(), InlineItem::kRubyLinePlaceholder);
            (*column_result->ruby_column->base_line.MutableResults())[0].margins.inline_start = -overhang.start;
...
bool LineBreaker::CanBreakAfterRubyColumn(const InlineItemResult& column_result, wtf_size_t column_end_item_index) const
{
    ...
    // after `column_result`.
    StringBuilder text_content;
    unsigned base_text_length = base_line.EndTextOffset() - base_line.StartOffset();
    text_content.Append(StringView(Text(), base_line.StartOffset(), base_text_length));
    ...
    text_content.Append(StringView(Text(), next_item.StartOffset() + ignorable_bidi_length));
    LazyLineBreakIterator break_iterator(break_iterator_, text_content.ReleaseString());
...
// attempt to place it.
bool LineBreaker::ShouldPushFloatAfterLine(UnpositionedFloat* unpositioned_float, LineInfo* line_info)
{
    if (unpositioned_float->token) {
        return false;
    }

    LayoutUnit inline_margin_size = ComputeMarginBoxInlineSizeForUnpositionedFloat(unpositioned_float);

    LayoutUnit used_size = position_ + inline_margin_size + ComputeFloatAncestorInlineEndSize(constraint_space_, Items(), current_.item_index);
    bool can_fit_float = used_size <= line_opportunity_.AvailableFloatInlineSize().AddEpsilon();
    if (!can_fit_float) {
        ...
        can_fit_float = used_size - TrailingCollapsibleSpaceWidth(line_info) <= line_opportunity_.AvailableFloatInlineSize().AddEpsilon();
    }

    LayoutUnit bfc_block_offset = unpositioned_float->origin_bfc_offset.block_offset;
    ...
    return !can_fit_float || exclusion_space_->LastFloatBlockStart() > bfc_block_offset
        || exclusion_space_->ClearanceOffset(unpositioned_float->ClearType(constraint_space_.Direction())) > bfc_block_offset;
}
...
// in the document.
void LineBreaker::HandleFloat(const InlineItem& item, const BlockBreakToken* float_break_token, LineInfo* line_info)
{
    ...
    if (current_.item_index <= leading_floats_.handled_index && !leading_floats_.floats.empty()) {
        DCHECK_LT(leading_floats_index_, leading_floats_.floats.size());
        item_result->positioned_float = leading_floats_.floats[leading_floats_index_++];

        // Save a backup copy of `exclusion_space_` even if leading floats don't
        // modify it. See `RewindFloat`.
        DCHECK(exclusion_space_);
        item_result->exclusion_space_before_position_float.CopyFrom(*exclusion_space_);
    ...
    bool is_hidden_for_paint = constraint_space_.GetLineClampData().ShouldHideForPaint();
    UnpositionedFloat unpositioned_float(BlockNode(To<LayoutBox>(item.GetLayoutObject())), float_break_token, constraint_space_.AvailableSize(),
        constraint_space_.PercentageResolutionSize(), constraint_space_.ReplacedPercentageResolutionSize(),
        { constraint_space_.GetBfcOffset().line_offset, bfc_block_offset }, constraint_space_, node_.Style(), constraint_space_.FragmentainerBlockSize(),
        constraint_space_.FragmentainerOffset(), is_hidden_for_paint);

    bool float_after_line = ShouldPushFloatAfterLine(&unpositioned_float, line_info);
    ...
    item_result->positioned_float = PositionFloat(&unpositioned_float, exclusion_space_);
    // Ensure `NeedsCollectInlines` isn't set, or it may cause security risks.
    CHECK(!node_.GetLayoutBox()->NeedsCollectInlines());

    if (constraint_space_.HasBlockFragmentation()) {
        if (const auto* break_token = item_result->positioned_float->BreakToken()) {
            const auto* parallel_token = InlineBreakToken::CreateForParallelBlockFlow(node_, index_before_float, *break_token);
            line_info->PropagateParallelFlowBreakToken(parallel_token);
            if (item_result->positioned_float->minimum_space_shortage) {
                line_info->PropagateMinimumSpaceShortage(item_result->positioned_float->minimum_space_shortage);
            }
...
void LineBreaker::UpdateLineOpportunity()
{
    ...
    LayoutOpportunity opportunity = exclusion_space_->FindLayoutOpportunity(
        { constraint_space_.GetBfcOffset().line_offset, bfc_block_offset }, constraint_space_.AvailableSize().inline_size);

    DCHECK_EQ(bfc_block_offset, opportunity.rect.BlockStartOffset());

    line_opportunity_ = opportunity.ComputeLineLayoutOpportunity(constraint_space_, line_opportunity_.line_block_size, LayoutUnit());
    UpdateAvailableWidth();
...
void LineBreaker::RewindFloats(unsigned new_end, LineInfo& line_info, InlineItemResults& item_results)
{
    for (const InlineItemResult& item_result : base::make_span(item_results).subspan(new_end)) {
        if (item_result.positioned_float) {
            ...
            if (item_index < leading_floats_.handled_index) {
                for (unsigned i = 0; i < leading_floats_.floats.size(); ++i) {
                    if (leading_floats_.floats[i].layout_result == item_result.positioned_float->layout_result) {
...
void LineBreaker::HandleInitialLetter(const InlineItem& item, LineInfo* line_info)
{
...
void LineBreaker::HandleOutOfFlowPositioned(const InlineItem& item, LineInfo* line_info)
{
...
bool LineBreaker::ComputeOpenTagResult(const InlineItem& item, const ConstraintSpace& constraint_space, bool is_in_svg_text, InlineItemResult* item_result)
{
    DCHECK_EQ(item.Type(), InlineItem::kOpenTag);
    DCHECK(item.Style());
    const ComputedStyle& style = *item.Style();
    if (!is_in_svg_text && item.ShouldCreateBoxFragment() && (style.HasBorder() || style.MayHavePadding() || style.MayHaveMargin())) {
        ...
        item_result->inline_size = item_result->margins.inline_start + item_result->borders.inline_start + item_result->padding.inline_start;
        return true;
    }
    return false;
}
...
void LineBreaker::HandleOpenTag(const InlineItem& item, LineInfo* line_info)
{
    ...
    DCHECK(item.Style());
    const ComputedStyle& style = *item.Style();
    if (ComputeOpenTagResult(item, constraint_space_, is_svg_text_, item_result)) {
        // Negative margins on open tags may bring the position back. Update
        // |state_| if that happens.
        if (item_result->inline_size < 0 && state_ == LineBreakState::kTrailing) [[unlikely]] {
            LayoutUnit available_width = AvailableWidthToFit();
            if (position_ > available_width && position_ + item_result->inline_size <= available_width) {
                state_ = LineBreakState::kContinue;
            }
        }
...
void LineBreaker::HandleCloseTag(const InlineItem& item, LineInfo* line_info)
{
    ...
    if (item_results.size() >= 2) {
        ...
        if (was_auto_wrap) {
            ...
            bool preceded_by_breakable_space = item_result->EndOffset() > 0 && IsBreakableSpace(Text()[item_result->EndOffset() - 1]);
            item_result->can_break_after
                = IsBreakableSpace(Text()[item_result->EndOffset()]) && (!current_style_->ShouldBreakOnlyAfterWhiteSpace() || preceded_by_breakable_space);
            return;
        }
...
void LineBreaker::HandleOverflow(LineInfo* line_info)
{
    ...
    for (unsigned i = item_results->size(); i;) {
        ...
        if (item.Type() == InlineItem::kText) {
            ...
            DCHECK(item_result->shape_result || (item_result->break_anywhere_if_overflow && !override_break_anywhere_) ||
                // |HandleTextForFastMinContent| can produce an item without
                // |ShapeResult|. In this case, it is not breakable.
                (mode_ == LineBreakerMode::kMinContent && !item_result->may_break_inside));
            ...
            if (width_to_rewind < 0 && item_result->may_break_inside) {
                ...
                BreakText(item_result, item, *item.TextShapeResult(), std::min(item_available_width, min_available_width), item_available_width, line_info);
                ...
                if (item_result->can_break_after && item_result->inline_size <= item_available_width
                    ...
                    if (new_end == item_results->size()) {
                        position_ = available_width + width_to_rewind + item_result->inline_size;
            ...
            if (width_to_rewind < 0 && item_result->may_break_inside) {
                ...
                const LineInfo& new_base_line = line_info->Results().back().ruby_column->base_line;
    ...
    if (applied_text_indent_ && width_to_rewind > LayoutUnit() && is_first_formatted_line_ && !leading_floats_.floats.empty()
    ...
    if (break_iterator_.BreakType() == LineBreakType::kPhrase && !disable_phrase_ && mode_ == LineBreakerMode::kContent) {
    ...
    DCHECK(base::ranges::all_of(*item_results, [](const InlineItemResult& item_result) { return !item_result.can_break_after; }));
    state_ = LineBreakState::kOverflow;
}

void LineBreaker::RetryAfterOverflow(LineInfo* line_info, InlineItemResults* item_results)
{
    ...
    for (unsigned index = new_end; index < item_results.size(); index++) {
        ...
        if (item.Type() == InlineItem::kText) {
            ...
            if (item_result.shape_result || // kNoResultIfOverflow if 'break-word'
                ...
                DCHECK(item.Style());
                const ComputedStyle& style = *item.Style();
                if (style.ShouldWrapLine() && !style.ShouldBreakSpaces() && IsBreakableSpace(text[item_result.StartOffset()])) {
                    // If all characters are trailable spaces, check the next item.
                    if (item_result.shape_result && IsAllBreakableSpaces(text, item_result.StartOffset() + 1, item_result.EndOffset())) {
                        continue;
                    }
...
void LineBreaker::Rewind(unsigned new_end, LineInfo* line_info)
{
    ...
    if (last_rewind_) {
        ...
        if (current_.item_index == last_rewind_->from_item_index && new_end == last_rewind_->to_index) {
            NOTREACHED();
        }
    ...
    if (new_end) {
        ...
        const HeapVector<InlineItem>& items = Items();
        while (!IsAtEnd() && items[current_.item_index].Type() == InlineItem::kText && !items[current_.item_index].Length()) {
        ...
        if (!item_results.empty() && item_results.front().IsRubyColumn()) {
            ruby_break_token_ = item_results.front().ruby_column->start_ruby_break_token;
        }
...
const ComputedStyle& LineBreaker::ComputeCurrentStyle(unsigned item_result_index, LineInfo* line_info) const
{
    const InlineItemResults& item_results = line_info->Results();
    ...
    DCHECK(item);
    if (item->Type() == InlineItem::kText || item->Type() == InlineItem::kCloseTag) {
        DCHECK(item->Style());
        return *item->Style();
    }
    ...
    while (item_result_index) {
        ...
        if (item->Type() == InlineItem::kText || item->Type() == InlineItem::kOpenTag) {
            DCHECK(item->Style());
            return *item->Style();
        }
        if (item->Type() == InlineItem::kCloseTag) {
    ...
    if (&style == current_style_) {
        ...
        if (auto_wrap_) {
            DCHECK_EQ(break_iterator_.IsSoftHyphenEnabled(),
                style.GetHyphens() != Hyphens::kNone && (disable_phrase_ || style.WordBreak() != EWordBreak::kAutoPhrase));
        ...
        if (line_break == LineBreak::kAnywhere) [[unlikely]] {
            ...
            if (!break_anywhere_if_overflow_) {
                ...
                break_anywhere_if_overflow_
                    = overflow_wrap == EOverflowWrap::kAnywhere || (overflow_wrap == EOverflowWrap::kBreakWord && mode_ == LineBreakerMode::kContent);
            }
...
{
    return current_.item_index > 0 ? Items().at(current_.item_index - 1).Type() == type : false;
}
...
void LineBreaker::SetInputRange(InlineItemTextIndex start, wtf_size_t end_item_index, WhitespaceState initial_whitespace_state, const LineBreaker* parent)
{
    current_ = start;
...
const InlineBreakToken* LineBreaker::CreateBreakToken(const LineInfo& line_info)
{
#if DCHECK_IS_ON()
    ...
    if (resume_block_in_inline_in_same_flow_) {
        ...
        const auto& block_in_inline_fragment = To<PhysicalBoxFragment>(block_in_inline->GetPhysicalFragment());
        sub_break_token = block_in_inline_fragment.GetBreakToken();
    }

    bool is_past_first_formatted_line = !is_first_formatted_line_ || !line_info.IsEmptyLine();

    DCHECK_EQ(line_info.HasForcedBreak(), is_forced_break_);
    unsigned flags = (is_forced_break_ ? InlineBreakToken::kIsForcedBreak : 0) | (line_info.UseFirstLineStyle() ? InlineBreakToken::kUseFirstLineStyle : 0)
        | (cloned_box_decorations_count_ ? InlineBreakToken::kHasClonedBoxDecorations : 0)
        | (is_past_first_formatted_line ? InlineBreakToken::kIsPastFirstFormattedLine : 0);

    return InlineBreakToken::Create(node_, current_style_, current_, flags, sub_break_token, ruby_break_token_);
}
