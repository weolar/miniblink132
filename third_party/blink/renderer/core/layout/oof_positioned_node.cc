...
void PhysicalOofPositionedNode::Trace(Visitor* visitor) const
{
    if (is_for_fragmentation) {
        static_cast<const PhysicalOofNodeForFragmentation*>(this)->TraceAfterDispatch(visitor);
    } else {
...
{
    if (is_for_fragmentation) {
        static_cast<const LogicalOofNodeForFragmentation*>(this)->TraceAfterDispatch(visitor);
    } else {
        TraceAfterDispatch(visitor);
    }
}
...
void PhysicalOofNodeForFragmentation::TraceAfterDispatch(Visitor* visitor) const
{
...
void LogicalOofNodeForFragmentation::TraceAfterDispatch(Visitor* visitor) const
{
