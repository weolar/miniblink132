...
public:
    ...
    ConstraintSpaceBuilder(
        const ConstraintSpace& parent_space, WritingDirectionMode writing_direction, bool is_new_fc, bool adjust_inline_size_if_needed = true)
        : ConstraintSpaceBuilder(parent_space.GetWritingMode(), writing_direction, is_new_fc,
            /* force_orthogonal_writing_mode_root */ false, adjust_inline_size_if_needed)
    {
    ...
    // unless we force it.
    ConstraintSpaceBuilder(WritingMode parent_writing_mode, WritingDirectionMode writing_direction, bool is_new_fc,
        bool force_orthogonal_writing_mode_root = false, bool adjust_inline_size_if_needed = true)
        : space_(writing_direction)
        , is_in_parallel_flow_(IsParallelWritingMode(parent_writing_mode, writing_direction.GetWritingMode()))
        ...
        , adjust_inline_size_if_needed_(adjust_inline_size_if_needed)
    {
        ...
        space_.bitfields_.is_orthogonal_writing_mode_root = !is_in_parallel_flow_ || force_orthogonal_writing_mode_root_;
    }
    ...
    void SetAvailableSize(LogicalSize available_size)
    {
        ...
        if (is_in_parallel_flow_) [[likely]] {
            ...
            space_.available_size_ = { available_size.block_size, available_size.inline_size };
    ...
    void SetReplacedPercentageResolutionSize(LogicalSize replaced_percentage_resolution_size);
    ...
    void ReserveSpaceInFragmentainer(LayoutUnit space)
    {
        ...
        space_.rare_data_->fragmentainer_block_size = space_.rare_data_->fragmentainer_block_size.ClampNegativeToZero();
    }
    ...
    void SetShouldRepeat(bool b)
    {
        space_.EnsureRareData()->should_repeat = b;
    }
    ...
    {
        space_.DisableFurtherFragmentation();
    }
    ...
    {
        if (is_in_parallel_flow_ || !force_orthogonal_writing_mode_root_) [[likely]] {
    ...
    {
        if (is_in_parallel_flow_) [[likely]] {
            space_.bitfields_.inline_auto_behavior = static_cast<unsigned>(auto_behavior);
        } else {
            space_.bitfields_.block_auto_behavior = static_cast<unsigned>(auto_behavior);
        }
    }
    ...
    void SetFragmentationType(FragmentationType fragmentation_type)
    {
        ...
        if (fragmentation_type != FragmentationType::kFragmentNone) {
            space_.EnsureRareData()->block_direction_fragmentation_type = fragmentation_type;
        }
    }
    ...
    void SetIsInColumnBfc()
    {
        space_.EnsureRareData()->is_in_column_bfc = true;
    }

    void SetIsPastBreak()
    {
        space_.EnsureRareData()->is_past_break = true;
    }

    void SetMinBlockSizeShouldEncompassIntrinsicSize()
    {
        space_.EnsureRareData()->min_block_size_should_encompass_intrinsic_size = true;
    }
    ...
    void SetShouldPropagateChildBreakValues(bool propagate_child_break_values = true)
    {
        // Don't create rare data if `propagate_child_break_values` is already
        // false.
        if (!space_.HasRareData() && !propagate_child_break_values)
            return;
        space_.EnsureRareData()->propagate_child_break_values = propagate_child_break_values;
    }
    ...
    {
        if (!is_new_fc_) {
            space_.bitfields_.adjoining_object_types = static_cast<unsigned>(adjoining_object_types);
        }
    }
    ...
    void SetOptimisticBfcBlockOffset(LayoutUnit optimistic_bfc_block_offset)
    {
        ...
        if (!is_new_fc_) [[likely]] {
            space_.EnsureRareData()->SetOptimisticBfcBlockOffset(optimistic_bfc_block_offset);
        }
    }
    ...
    void SetTableCellBorders(const BoxStrut& table_cell_borders, WritingDirectionMode cell_writing_direction, WritingDirectionMode table_writing_direction)
    {
#if DCHECK_IS_ON()
        ...
        if (table_cell_borders != BoxStrut()) {
            space_.EnsureRareData()->SetTableCellBorders(
                table_cell_borders.ConvertToPhysical(table_writing_direction).ConvertToLogical(cell_writing_direction));
        }
    }

    void SetTableCellAlignmentBaseline(const std::optional<LayoutUnit>& table_cell_alignment_baseline)
    {
#if DCHECK_IS_ON()
        ...
        if (is_in_parallel_flow_ && table_cell_alignment_baseline) {
            space_.EnsureRareData()->SetTableCellAlignmentBaseline(*table_cell_alignment_baseline);
        }
    }
    ...
    void SetIsTableCellWithCollapsedBorders(bool has_collapsed_borders)
    {
        ...
        if (has_collapsed_borders) {
            space_.EnsureRareData()->SetIsTableCellWithCollapsedBorders(has_collapsed_borders);
        }
    }
    ...
    void SetCustomLayoutData(scoped_refptr<SerializedScriptValue> custom_layout_data)
    {
        ...
        if (custom_layout_data) {
            space_.EnsureRareData()->SetCustomLayoutData(std::move(custom_layout_data));
        }
    }

    void SetTableRowData(const TableConstraintSpaceData* table_data, wtf_size_t row_index)
    {
#if DCHECK_IS_ON()
...
#endif
        space_.EnsureRareData()->SetTableSectionData(std::move(table_data), section_index);
    }
    ...
    {
        space_.SetShouldForceTextBoxTrimEnd();
    }
    ...
    void SetDecorationPercentageResolutionType(DecorationPercentageResolutionType type)
    {
        space_.EnsureRareData()->decoration_percentage_resolution_type = static_cast<unsigned>(type);
    }
    ...
    void SetTargetStretchInlineSize(LayoutUnit target_stretch_inline_size)
    {
        ...
        space_.EnsureRareData()->SetTargetStretchInlineSize(target_stretch_inline_size);
    }

    void SetTargetStretchBlockSizes(ConstraintSpace::MathTargetStretchBlockSizes target_stretch_block_sizes)
    {
        DCHECK_GE(target_stretch_block_sizes.ascent, LayoutUnit());
        DCHECK_GE(target_stretch_block_sizes.descent, LayoutUnit());
        space_.EnsureRareData()->SetTargetStretchBlockSizes(target_stretch_block_sizes);
    }
...
#endif
        space_.EnsureRareData()->SetGridLayoutSubtree(std::move(grid_layout_subtree));
    }
    ...
    {
#if DCHECK_IS_ON()
        DCHECK(!to_constraint_space_called_) << "ToConstraintSpace should only be called once.";
        to_constraint_space_called_ = true;
#endif

        DCHECK(!is_new_fc_ || !space_.bitfields_.adjoining_object_types);
        DCHECK_EQ(space_.bitfields_.is_orthogonal_writing_mode_root, !is_in_parallel_flow_ || force_orthogonal_writing_mode_root_);

        DCHECK(!force_orthogonal_writing_mode_root_ || is_in_parallel_flow_) << "Forced and inferred orthogonal writing mode shouldn't happen "
...
public:
    MinMaxConstraintSpaceBuilder(const ConstraintSpace& parent_space, const ComputedStyle& parent_style, const LayoutInputNode& child, bool is_new_fc)
        : delegate_(parent_space, child.Style().GetWritingDirection(), is_new_fc)
    {
    ...
    {
        delegate_.SetReplacedPercentageResolutionSize({ kIndefiniteSize, block_size });
    }
