...
TEST_F(OutOfFlowLayoutPartTest, FixedInsideAbs)
{
    ...
    EXPECT_TRUE(result);
    EXPECT_EQ(result->GetPhysicalFragment().OutOfFlowPositionedDescendants().size(), 2u);
...
TEST_F(OutOfFlowLayoutPartTest, PositionedFragmentationWithNegativeTopPropertyAndNewEmptyColumn)
{
    SetBodyInnerHTML(
        R"HTML(
      <style>
...
// spanner.
TEST_F(OutOfFlowLayoutPartTest, PositionedFragmentationWithNewColumnsAndSpanners)
{
    SetBodyInnerHTML(
        R"HTML(
      <style>
...
// spanner.
TEST_F(OutOfFlowLayoutPartTest, PositionedFragmentationWithNewColumnsAfterSpanner)
{
    SetBodyInnerHTML(
        R"HTML(
      <style>
...
TEST_F(OutOfFlowLayoutPartTest, PositionedFragmentationWithBottomPropertyAndHeightAuto)
{
    SetBodyInnerHTML(
        R"HTML(
      <style>
...
static void CheckMulticolumnPositionedObjects(const LayoutBox* multicol, const LayoutBox* abspos)
{
    for (const PhysicalBoxFragment& fragmentation_root : multicol->PhysicalFragments()) {
        EXPECT_TRUE(fragmentation_root.IsFragmentationContextRoot());
        EXPECT_FALSE(fragmentation_root.HasOutOfFlowFragmentChild());
        for (const PhysicalFragmentLink& fragmentainer : fragmentation_root.Children()) {
...
TEST_F(OutOfFlowLayoutPartTest, PositionedObjectsInMulticol)
{
    ...
    CheckMulticolumnPositionedObjects(GetLayoutBoxByElementId("outer"), GetLayoutBoxByElementId("abs1"));
    CheckMulticolumnPositionedObjects(GetLayoutBoxByElementId("inner"), GetLayoutBoxByElementId("abs2"));
}
...
TEST_F(OutOfFlowLayoutPartTest, FragmentainerBreakTokens)
{
    ...
    const PhysicalBoxFragment* multicol_fragment = multicol->GetPhysicalFragment(0);
...
TEST_F(OutOfFlowLayoutPartTest, RelayoutNestedMulticolWithOOF)
{
    ...
    auto GetInnerFragmentainer = [&inner]() -> const PhysicalBoxFragment* {
        ...
        if (inner->GetPhysicalFragment(0)->Children().size() != 1u)
            return nullptr;
        return To<PhysicalBoxFragment>(inner->GetPhysicalFragment(0)->Children()[0].fragment.Get());
    };
...
TEST_F(OutOfFlowLayoutPartTest, UseCountOutOfFlowNoInsets)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kOutOfFlowJustifySelfNoInsets));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kOutOfFlowAlignSelfNoInsets));
}
...
TEST_F(OutOfFlowLayoutPartTest, UseCountOutOfFlowSingleInset)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kOutOfFlowJustifySelfSingleInset));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kOutOfFlowAlignSelfSingleInset));
}
...
TEST_F(OutOfFlowLayoutPartTest, UseCountOutOfFlowBothInsets)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kOutOfFlowJustifySelfBothInsets));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kOutOfFlowAlignSelfBothInsets));
}
