...
public:
    ...
    // point vs LayoutUnit).
    explicit HitTestLocation(const gfx::PointF& point, const PhysicalRect& bounding_box);
    ...
    gfx::Point RoundedPoint() const
    {
        return ToRoundedPoint(point_);
    }
    ...
    {
        return is_rect_based_;
    }
    bool IsRectilinear() const
    {
        return is_rectilinear_;
    }
    const PhysicalRect& BoundingBox() const
    {
    ...
    bool IntersectsEllipse(const gfx::PointF& center, const gfx::SizeF& radii) const;
    bool IntersectsCircleStroke(const gfx::PointF& center, float radius, float stroke_width) const;

    const gfx::PointF& TransformedPoint() const
    {
        return transformed_point_;
    }
    const gfx::QuadF& TransformedRect() const
    {
