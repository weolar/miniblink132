...
public:
    ...
    void SetLogicalTop(LayoutUnit logical_top)
    {
    ...
    bool IsLogicalHeightKnown() const
    {
        return is_logical_height_known_;
    }
    ...
    LayoutUnit LogicalHeightInFlowThread() const
    {
        ...
        return (logical_bottom_in_flow_thread_ - logical_top_in_flow_thread_).ClampNegativeToZero();
    }
    ...
    PhysicalOffset FlowThreadTranslationAtOffset(LayoutUnit, LayoutBox::PageBoundaryRule) const;

    LogicalOffset VisualPointToFlowThreadPoint(const LogicalOffset& visual_point) const;

    PhysicalRect FragmentsBoundingBox(const PhysicalRect& bounding_box_in_flow_thread) const;
    ...
    unsigned ColumnIndexAtOffset(LayoutUnit offset_in_flow_thread, LayoutBox::PageBoundaryRule) const;
    ...
    unsigned ConstrainedColumnIndexAtOffset(LayoutUnit offset_in_flow_thread, LayoutBox::PageBoundaryRule) const;
...
public:
    ...
    MultiColumnFragmentainerGroup& First()
    {
    ...
    MultiColumnFragmentainerGroup& Last()
    {
        return groups_.back();
    }
    ...
    typedef HeapVector<MultiColumnFragmentainerGroup, 1>::const_iterator const_iterator;
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::MultiColumnFragmentainerGroup)
