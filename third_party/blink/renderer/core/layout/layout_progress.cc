...
LayoutProgress::LayoutProgress(HTMLProgressElement& node)
    ...
    , animating_(false)
    , animation_timer_(node.GetDocument().GetTaskRunner(TaskType::kInternalDefault), this, &LayoutProgress::AnimationTimerFired)
{
}
...
double LayoutProgress::AnimationProgress() const
{
    ...
    if (!animating_)
        return 0;
    const base::TimeDelta elapsed = base::TimeTicks::Now() - animation_start_time_;
...
{
    NOT_DESTROYED();
    return (HTMLProgressElement::kIndeterminatePosition != GetPosition() && HTMLProgressElement::kInvalidPosition != GetPosition());
}
