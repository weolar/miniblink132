...
public:
        ...
        , child_token_idx_(0)
    {
        ...
        // process.
        if (IsBreakInside(parent_break_token_) && !current_.break_token && parent_break_token_->HasSeenAllChildren())
            is_finished_ = true;
    }
...
void MulticolPartWalker::MoveToSpanner(BlockNode spanner, const BlockBreakToken* next_column_token)
{
...
void MulticolPartWalker::AddNextColumnBreakToken(const BlockBreakToken& next_column_token)
{
    *this = MulticolPartWalker(multicol_container_, nullptr);
    next_column_token_ = &next_column_token;
    UpdateCurrent();
...
void MulticolPartWalker::UpdateNextColumnBreakToken(const FragmentBuilder::ChildrenVector& children)
{
    if (children.empty())
        return;
    const blink::PhysicalFragment* last_child = children[children.size() - 1].fragment;
    if (!last_child->IsColumnBox())
        return;
    const auto* child_break_token = To<BlockBreakToken>(last_child->GetBreakToken());
...
void MulticolPartWalker::UpdateCurrent()
{
    ...
    if (parent_break_token_) {
        ...
        if (child_token_idx_ < child_break_tokens.size()) {
            const auto* child_break_token = To<BlockBreakToken>(child_break_tokens[child_token_idx_].Get());
    ...
    : LayoutAlgorithm(params)
{
    ...
    if (const BlockNode marker_node = Node().ListMarkerBlockNodeIfListItem()) {
        if (!marker_node.ListMarkerOccupiesWholeLine() && (!GetBreakToken() || GetBreakToken()->HasUnpositionedListMarker())) {
...
const LayoutResult* ColumnLayoutAlgorithm::Layout()
{
    ...
    column_inline_size_ = ResolveUsedColumnInlineSize(ChildAvailableSize().inline_size, Style());

    column_inline_progression_ = column_inline_size_ + ResolveUsedColumnGap(ChildAvailableSize().inline_size, Style());
    used_column_count_ = ResolveUsedColumnCount(ChildAvailableSize().inline_size, Style());
    ...
    is_constrained_by_outer_fragmentation_context_ = GetConstraintSpace().HasKnownFragmentainerBlockSize();
    ...
    if (break_status == BreakStatus::kNeedsEarlierBreak) {
        ...
        return RelayoutAndBreakEarlier<ColumnLayoutAlgorithm>(container_builder_.GetEarlyBreak());
    }
    DCHECK_EQ(break_status, BreakStatus::kContinue);

    intrinsic_block_size_ = std::max(intrinsic_block_size_, BorderScrollbarPadding().block_start);
    ...
    intrinsic_block_size_ = ClampIntrinsicBlockSize(GetConstraintSpace(), Node(), GetBreakToken(), BorderScrollbarPadding(), intrinsic_block_size_);

    LayoutUnit block_size = ComputeBlockSizeForFragment(
        GetConstraintSpace(), Node(), BorderPadding(), previously_consumed_block_size + intrinsic_block_size_, border_box_size.inline_size);

    container_builder_.SetFragmentsTotalBlockSize(block_size);
    container_builder_.SetIntrinsicBlockSize(intrinsic_block_size_);
    container_builder_.SetBlockOffsetForAdditionalColumns(CurrentContentBlockOffset(intrinsic_block_size_));
    ...
    if (GetConstraintSpace().IsTableCell()) {
        FinalizeTableCellLayout(unconstrained_intrinsic_block_size, &container_builder_);
    } else {
...
MinMaxSizesResult ColumnLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    ...
    if (override_intrinsic_inline_size != kIndefiniteSize) {
        const LayoutUnit size = BorderScrollbarPadding().InlineSum() + override_intrinsic_inline_size;
    ...
    FragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(space, Node(), /* break_token */ nullptr, /* is_intrinsic */ true);
    BlockLayoutAlgorithm algorithm({ Node(), fragment_geometry, space });
    MinMaxSizesResult result = algorithm.ComputeMinMaxSizes(MinMaxSizesFloatInput());
    ...
    if (!Style().HasAutoColumnWidth()) {
        ...
        result.sizes.max_size = std::max(result.sizes.max_size, result.sizes.min_size);
    }
...
const PhysicalBoxFragment& ColumnLayoutAlgorithm::CreateEmptyColumn(
    const BlockNode& node, const ConstraintSpace& parent_space, const PhysicalBoxFragment& previous_column)
{
    ...
    LogicalSize column_size = previous_column.Size().ConvertToLogical(writing_mode);
    ConstraintSpace child_space = CreateConstraintSpaceForFragmentainer(parent_space, kFragmentColumn, column_size,
        /*percentage_resolution_size=*/column_size, /*balance_columns=*/false, kBreakAppealLastResort);
    FragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(child_space, node, break_token);
    LayoutAlgorithmParams params(node, fragment_geometry, child_space, break_token);
    SimplifiedOofLayoutAlgorithm child_algorithm(params, previous_column);
    child_algorithm.ResumeColumnLayout(break_token);
    return To<PhysicalBoxFragment>(child_algorithm.Layout()->GetPhysicalFragment());
}

MinMaxSizesResult ColumnLayoutAlgorithm::ComputeSpannersMinMaxSizes(const BlockNode& search_parent) const
{
    MinMaxSizesResult result;
    for (LayoutInputNode child = search_parent.FirstChild(); child; child = child.NextSibling()) {
        ...
        if (!child_block->IsColumnSpanAll()) {
            ...
            MinMaxConstraintSpaceBuilder builder(GetConstraintSpace(), Style(), *child_block, /* is_new_fc */ true);
            builder.SetAvailableBlockSize(ChildAvailableSize().block_size);
            const ConstraintSpace child_space = builder.ToConstraintSpace();
            child_result = ComputeMinAndMaxContentContribution(Style(), *child_block, child_space);
        }
...
BreakStatus ColumnLayoutAlgorithm::LayoutChildren()
{
    ...
    while (!walker.IsFinished()) {
        ...
        // spanners.
        if (!entry.spanner) {
            const LayoutResult* result = LayoutRow(child_break_token, LayoutUnit(), &margin_strut);
        ...
        if (early_break_ && IsEarlyBreakTarget(*early_break_, container_builder_, spanner_node))
            break;
        ...
        BreakStatus break_status = LayoutSpanner(spanner_node, child_break_token, &margin_strut);
        ...
        if (break_status == BreakStatus::kBrokeBefore || container_builder_.HasInflowChildBreakInside()) {
            break;
        }
    }
    ...
    if (!walker.IsFinished() || container_builder_.HasInflowChildBreakInside()) {
        ...
        // remaining space.
        intrinsic_block_size_ = std::max(intrinsic_block_size_, FragmentainerSpaceLeftForChildren());
        ...
        for (; !walker.IsFinished(); walker.Next()) {
            ...
            if (entry.break_token) {
                ...
                // either).
                container_builder_.AddBreakBeforeChild(entry.spanner, kBreakAppealPerfect, /* is_forced_break */ false);
            }
        }
    } else {
...
const LayoutResult* ColumnLayoutAlgorithm::LayoutRow(const BlockBreakToken* next_column_token, LayoutUnit minimum_column_block_size, MarginStrut* margin_strut)
{
    ...
    if (is_constrained_by_outer_fragmentation_context_) {
        available_outer_space = std::max(minimum_column_block_size, FragmentainerSpaceLeftForChildren() - row_offset);
        ...
        if (column_size.block_size == kIndefiniteSize || column_size.block_size > available_outer_space)
    ...
    bool balance_columns = Style().GetColumnFill() == EColumnFill::kBalance
        || (GetConstraintSpace().HasBlockFragmentation() && !GetConstraintSpace().HasKnownFragmentainerBlockSize());
    ...
    bool has_content_based_block_size = balance_columns || (column_size.block_size == kIndefiniteSize && !is_constrained_by_outer_fragmentation_context_);

    if (has_content_based_block_size) {
        column_size.block_size = ResolveColumnAutoBlockSize(column_size, row_offset, available_outer_space, next_column_token, balance_columns);
        ...
        // context.
        if (column_size.block_size > available_outer_space || column_size.block_size == kIndefiniteSize) {
    ...
    if (may_resume_in_next_outer_fragmentainer && !IsBreakInside(GetBreakToken())) {
    ...
    do {
        ...
        do {
            ...
            ConstraintSpace child_space = CreateConstraintSpaceForFragmentainer(GetConstraintSpace(), kFragmentColumn, column_size,
                ColumnPercentageResolutionSize(), balance_columns, min_break_appeal.value_or(kBreakAppealLastResort));

            FragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(child_space, Node(), GetBreakToken());

            LayoutAlgorithmParams params(Node(), fragment_geometry, child_space, column_break_token);
            ...
            const auto& column = To<PhysicalBoxFragment>(result->GetPhysicalFragment());
            ...
            if (shrink_to_fit_column_block_size) {
                ...
                intrinsic_block_size_contribution = std::min(intrinsic_block_size_contribution, result->BlockSizeForFragmentation());
                shrink_to_fit_column_block_size = false;
            }

            if (!has_oof_fragmentainer_descendants && balance_columns && FragmentedOofData::HasOutOfFlowPositionedFragmentainerDescendants(column)) {
            ...
            if (may_resume_in_next_outer_fragmentainer && column_break_token && actual_column_count >= used_column_count_)
                break;
            ...
            if (may_have_more_space_in_next_outer_fragmentainer) {
                ...
                // fragmentainer.
                min_break_appeal = std::min(min_break_appeal.value_or(kBreakAppealPerfect), result->GetBreakAppeal());

                LayoutUnit block_end_overflow = LogicalBoxFragment(GetConstraintSpace().GetWritingDirection(), column).BlockEndScrollableOverflow();
                if (row_offset + block_end_overflow > FragmentainerSpaceLeftForChildren()) {
                    if (GetConstraintSpace().IsInsideBalancedColumns() && !container_builder_.IsInitialColumnBalancingPass()) {
                    ...
                    if (!minimum_column_block_size && block_end_overflow > column_size.block_size) {
                        ...
                        return LayoutRow(next_column_token, minimum_column_block_size, margin_strut);
                    }
                }
            }
        ...
        if (has_oof_fragmentainer_descendants) {
            ...
            for (wtf_size_t i = 0; i < new_columns.size(); i++) {
                ...
                columns.push_back(LogicalFragmentLink { &new_column.Fragment(), new_column.offset });
                ...
                // builder).
                container_builder_.PropagateOOFFragmentainerDescendants(new_column.Fragment(), new_column.offset,
                    /* relative_offset */ LogicalOffset(), containing_block_adjustment,
                    /* containing_block */ nullptr,
                    /* fixedpos_containing_block */ nullptr, &column_balancing_info.out_of_flow_fragmentainer_descendants);
            }
            ...
            if (column_balancing_info.minimal_space_shortage > LayoutUnit()) {
                UpdateMinimalSpaceShortage(column_balancing_info.minimal_space_shortage, &minimal_space_shortage);
            }
        ...
        // afterwards.
        if (!has_violating_break && actual_column_count <= used_column_count_ && (!column_break_token || result->GetColumnSpannerPath())) {
            break;
        }
        ...
        new_column_block_size = ConstrainColumnBlockSize(new_column_block_size, row_offset, available_outer_space);
    ...
    if (GetConstraintSpace().HasBlockFragmentation() && row_offset > LayoutUnit()) {
        // If we have container separation, breaking before this row is fine.
        LayoutUnit fragmentainer_block_offset = FragmentainerOffsetForChildren() + row_offset;
        ...
        if (!MovePastBreakpoint(*result, fragmentainer_block_offset, kBreakAppealPerfect)) {
    ...
    bool is_empty = !column_size.block_size && new_columns.size() == 1 && (new_columns[0].Fragment().Children().empty() || is_empty_spanner_parent);
    ...
    StyleEngine::AttachScrollMarkersScope scope(Node().GetDocument().GetStyleEngine());
    ...
    for (auto result_with_offset : new_columns) {
        ...
        const WritingModeConverter converter(GetConstraintSpace().GetWritingDirection(), LogicalSize(ChildAvailableSize().inline_size, column_block_size_));
        ColumnPseudoElement* column_pseudo = element->CreateColumnPseudoElementIfNeeded(converter.ToPhysical(column_logical_rect));
        if (column_pseudo && column_pseudo->GetComputedStyle()->GetScrollSnapAlign() != cc::ScrollSnapAlign()) {
...
BreakStatus ColumnLayoutAlgorithm::LayoutSpanner(BlockNode spanner_node, const BlockBreakToken* break_token, MarginStrut* margin_strut)
{
    spanner_path_ = nullptr;
    const ComputedStyle& spanner_style = spanner_node.Style();
    BoxStrut margins = ComputeMarginsFor(spanner_style, ChildAvailableSize().inline_size, GetConstraintSpace().GetWritingDirection());
    ...
    auto spanner_space = CreateConstraintSpaceForSpanner(spanner_node, block_offset);
    ...
    auto* result = spanner_node.Layout(spanner_space, break_token, early_break_in_child);
    ...
    if (GetConstraintSpace().HasBlockFragmentation() && !early_break_) {
        ...
        LayoutUnit fragmentainer_block_offset = FragmentainerOffsetForChildren() + block_offset;

        BreakStatus break_status = BreakBeforeChildIfNeeded(spanner_node, *result, fragmentainer_block_offset, has_processed_first_child_);
    ...
    const auto& spanner_fragment = To<PhysicalBoxFragment>(result->GetPhysicalFragment());
    LogicalFragment logical_fragment(GetConstraintSpace().GetWritingDirection(), spanner_fragment);

    ResolveInlineAutoMargins(spanner_style, Style(), ChildAvailableSize().inline_size, logical_fragment.InlineSize(), &margins);

    LogicalOffset offset(BorderScrollbarPadding().inline_start + margins.inline_start, block_offset);
...
void ColumnLayoutAlgorithm::AttemptToPositionListMarker(const PhysicalBoxFragment& child_fragment, LayoutUnit block_offset)
{
    ...
    marker.AddToBox(
        GetConstraintSpace(), baseline_type, child_fragment, BorderScrollbarPadding(), *layout_result, *baseline, &block_offset, &container_builder_);
...
void ColumnLayoutAlgorithm::PositionAnyUnclaimedListMarker()
{
    ...
    marker.AddToBoxWithoutLineBoxes(GetConstraintSpace(), baseline_type, *layout_result, &container_builder_, &intrinsic_block_size_);
    container_builder_.ClearUnpositionedListMarker();
}

void ColumnLayoutAlgorithm::PropagateBaselineFromChild(const PhysicalBoxFragment& child, LayoutUnit block_offset)
{
    ...
    if (auto first_baseline = fragment.FirstBaseline()) {
        LayoutUnit baseline = std::min(block_offset + *first_baseline, container_builder_.FirstBaseline().value_or(LayoutUnit::Max()));
    ...
    if (auto last_baseline = fragment.LastBaseline()) {
        LayoutUnit baseline = std::max(block_offset + *last_baseline, container_builder_.LastBaseline().value_or(LayoutUnit::Min()));
...
LayoutUnit ColumnLayoutAlgorithm::ResolveColumnAutoBlockSize(
    const LogicalSize& column_size, LayoutUnit row_offset, LayoutUnit available_outer_space, const BlockBreakToken* child_break_token, bool balance_columns)
{
    spanner_path_ = nullptr;
    return ResolveColumnAutoBlockSizeInternal(column_size, row_offset, available_outer_space, child_break_token, balance_columns);
}
    ...
    const LogicalSize& column_size, LayoutUnit row_offset, LayoutUnit available_outer_space, const BlockBreakToken* child_break_token, bool balance_columns)
{
    ...
    FragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(space, Node(), GetBreakToken());
    ...
    struct ContentRun {
        ...
        LayoutUnit ColumnBlockSize() const
        {
            ...
            return LayoutUnit::FromFloatCeil(float(content_block_size) / float(implicit_breaks_assumed_count + 1));
        }
    ...
    public:
        ...
        void AddRun(LayoutUnit content_block_size)
        {
            ...
            tallest_content_block_size_ = std::max(tallest_content_block_size_, content_block_size);
        }

        void DistributeImplicitBreaks(int used_column_count)
        {
            for (int columns_found = runs_.size(); columns_found < used_column_count; ++columns_found) {
    ...
    private:
        ContentRun* TallestRun() const
        {
            ...
            auto const it = std::max_element(
                runs_.begin(), runs_.end(), [](const ContentRun& run1, const ContentRun& run2) { return run1.ColumnBlockSize() < run2.ColumnBlockSize(); });
    ...
    do {
        ...
        if (forced_break_count < used_column_count_) {
            LayoutUnit column_block_size = BlockSizeForFragmentation(*result, GetConstraintSpace().GetWritingDirection());
            ...
            LogicalFragment logical_fragment(GetConstraintSpace().GetWritingDirection(), fragment);
            column_block_size = std::max(column_block_size, logical_fragment.BlockSize());
            content_runs.AddRun(column_block_size);
        }

        tallest_unbreakable_block_size_ = std::max(tallest_unbreakable_block_size_, result->TallestUnbreakableBlockSize());
    ...
    if (GetConstraintSpace().IsInitialColumnBalancingPass()) {
        ...
        // block-size.
        container_builder_.PropagateTallestUnbreakableBlockSize(tallest_unbreakable_block_size_);
    }
    ...
    if (tallest_unbreakable_block_size_ >= content_runs.TallestContentBlockSize()) {
        return ConstrainColumnBlockSize(tallest_unbreakable_block_size_, row_offset, available_outer_space);
    }
    ...
    return ConstrainColumnBlockSize(content_runs.TallestColumnBlockSize(), row_offset, available_outer_space);
}

// Constrain a balanced column block size to not overflow the multicol
// container.
LayoutUnit ColumnLayoutAlgorithm::ConstrainColumnBlockSize(LayoutUnit size, LayoutUnit row_offset, LayoutUnit available_outer_space) const
{
    ...
    LayoutUnit max = ResolveInitialMaxBlockLength(space, style, BorderPadding(), style.LogicalMaxHeight());
    LayoutUnit extent = kIndefiniteSize;

    const Length& block_length = style.LogicalHeight();
    const Length& auto_length = space.IsBlockAutoBehaviorStretch() ? Length::Stretch() : Length::FitContent();

    extent = ResolveMainBlockLength(space, style, BorderPadding(), block_length, &auto_length, kIndefiniteSize);
    ...
    LayoutUnit min = ResolveInitialMinBlockLength(space, style, BorderPadding(), style.LogicalMinHeight());
    max = std::max(max, min);
...
ConstraintSpace ColumnLayoutAlgorithm::CreateConstraintSpaceForBalancing(const LogicalSize& column_size) const
{
    ConstraintSpaceBuilder space_builder(GetConstraintSpace(), Style().GetWritingDirection(),
        /* is_new_fc */ true);
...
ConstraintSpace ColumnLayoutAlgorithm::CreateConstraintSpaceForSpanner(const BlockNode& spanner, LayoutUnit block_offset) const
{
    auto child_writing_direction = spanner.Style().GetWritingDirection();
    ConstraintSpaceBuilder space_builder(GetConstraintSpace(), child_writing_direction, /* is_new_fc */ true);
    if (!IsParallelWritingMode(GetConstraintSpace().GetWritingMode(), child_writing_direction.GetWritingMode())) {
    ...
    space_builder.SetBaselineAlgorithmType(GetConstraintSpace().GetBaselineAlgorithmType());

    if (GetConstraintSpace().HasBlockFragmentation()) {
        SetupSpaceBuilderForFragmentation(container_builder_, spanner, block_offset, &space_builder);
    }
...
LayoutUnit ColumnLayoutAlgorithm::TotalColumnBlockSize() const
{
    ...
    for (auto& child : container_builder_.Children()) {
        if (child.fragment->IsFragmentainerBox()) {
            LayoutUnit fragmentainer_block_size = child.fragment->Size().ConvertToLogical(writing_mode).block_size;
            total_block_size += ClampedToValidFragmentainerCapacity(fragmentainer_block_size);
        }
    }
