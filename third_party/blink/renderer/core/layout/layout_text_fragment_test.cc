...
protected:
    ...
    {
        RenderingTest::SetUp();
        GetDocument().head()->setInnerHTML("<style>#target::first-letter{color:red}</style>");
    }
    ...
    {
        return To<LayoutTextFragment>(GetElementById("target")->firstChild()->GetLayoutObject());
    }

    const LayoutTextFragment* GetFirstLetter() const
    {
        return To<LayoutTextFragment>(AssociatedLayoutObjectOf(*GetElementById("target")->firstChild(), 0));
    }
};
...
TEST_F(LayoutTextFragmentTest, IsBeforeAfterNonCollapsedCharacterNoLineWrap)
{
    ...
    SetBasicBody("f   bar");
    EXPECT_TRUE(GetRemainingText()->IsBeforeNonCollapsedCharacter(0)); // "f|   bar"
    EXPECT_FALSE(GetRemainingText()->IsBeforeNonCollapsedCharacter(1)); // "f |  bar"
    EXPECT_FALSE(GetRemainingText()->IsBeforeNonCollapsedCharacter(2)); // "f  | bar"
    EXPECT_TRUE(GetRemainingText()->IsAfterNonCollapsedCharacter(1)); // "f |  bar"
    EXPECT_FALSE(GetRemainingText()->IsAfterNonCollapsedCharacter(2)); // "f  | bar"
    EXPECT_FALSE(GetRemainingText()->IsAfterNonCollapsedCharacter(3)); // "f   |bar"
    ...
    SetBasicBody("foo  ");
    EXPECT_FALSE(GetRemainingText()->IsBeforeNonCollapsedCharacter(2)); // "foo|  "
    EXPECT_FALSE(GetRemainingText()->IsBeforeNonCollapsedCharacter(3)); // "foo | "
    EXPECT_FALSE(GetRemainingText()->IsAfterNonCollapsedCharacter(3)); // "foo | "
    ...
    EXPECT_TRUE(GetRemainingText()->IsBeforeNonCollapsedCharacter(2)); // "foo| <span>bar</span>"
    EXPECT_TRUE(GetRemainingText()->IsAfterNonCollapsedCharacter(3)); // "foo |<span>bar</span>"

    // Non-collapsed space as remaining text
    SetBasicBody("f <span>bar</span>");
    EXPECT_TRUE(GetRemainingText()->IsBeforeNonCollapsedCharacter(0)); // "f| <span>bar</span>"
    EXPECT_TRUE(GetRemainingText()->IsAfterNonCollapsedCharacter(1)); // "f |<span>bar</span>"

    // Collapsed space as remaining text
    SetBasicBody("f <br>");
    EXPECT_FALSE(GetRemainingText()->IsBeforeNonCollapsedCharacter(0)); // "f| <br>"
    EXPECT_FALSE(GetRemainingText()->IsAfterNonCollapsedCharacter(1)); // "f |<br>"
}
...
TEST_F(LayoutTextFragmentTest, IsBeforeAfterNonCollapsedLineWrapSpace)
{
    ...
    SetAhemBody("xx xx", 2);
    EXPECT_TRUE(GetRemainingText()->IsBeforeNonCollapsedCharacter(1)); // "xx| xx"
    EXPECT_TRUE(GetRemainingText()->IsAfterNonCollapsedCharacter(2)); // "xx |xx"

    // Line wrapping at remaining text start
    SetAhemBody("(x xx", 2);
    EXPECT_TRUE(GetRemainingText()->IsBeforeNonCollapsedCharacter(0)); // "(x| xx"
    ...
    EXPECT_TRUE(GetRemainingText()->IsBeforeNonCollapsedCharacter(1)); // "xx| <span>xx</span>"
    EXPECT_TRUE(GetRemainingText()->IsAfterNonCollapsedCharacter(2)); // "xx |<span>xx</span>"

    // Entire remaining text as line wrapping
    SetAhemBody("(x <span>xx</span>", 2);
    EXPECT_TRUE(GetRemainingText()->IsBeforeNonCollapsedCharacter(0)); // "(x| <span>xx</span>"
    EXPECT_TRUE(GetRemainingText()->IsAfterNonCollapsedCharacter(1)); // "(x |<span>xx</span>"
}
...
TEST_F(LayoutTextFragmentTest, SetTextWithFirstLetter)
{
    ...
    SetBodyInnerHTML("<style>div::first-letter {color: red;}</style>"
                     "<div id=sample>a</div>");
    ...
    EXPECT_TRUE(To<LayoutTextFragment>(letter_x.GetLayoutObject())->IsRemainingTextLayoutObject());
    ASSERT_TRUE(letter_x.GetLayoutObject()->GetFirstLetterPart());
    EXPECT_EQ("a", letter_x.GetLayoutObject()->GetFirstLetterPart()->TransformedText());

    // Make <div>"" "a"</div>
    Text& letter_a = *letter_x.splitText(0, ASSERT_NO_EXCEPTION);
    UpdateAllLifecyclePhasesForTest();

    EXPECT_TRUE(letter_a.GetLayoutObject()->IsTextFragment()) << "'a' is still first-letter";
    EXPECT_TRUE(letter_a.GetLayoutObject()->GetFirstLetterPart());
    EXPECT_TRUE(To<LayoutTextFragment>(letter_a.GetLayoutObject())->IsRemainingTextLayoutObject());
    ASSERT_TRUE(letter_a.GetLayoutObject()->GetFirstLetterPart());
    EXPECT_EQ("a", letter_a.GetLayoutObject()->GetFirstLetterPart()->TransformedText());
    EXPECT_FALSE(letter_x.GetLayoutObject()) << "We don't have layout text for empty Text node.";
    ...
    EXPECT_TRUE(letter_a.GetLayoutObject()->IsTextFragment()) << "We still use LayoutTextFragment for 'a'";
    EXPECT_FALSE(letter_a.GetLayoutObject()->GetFirstLetterPart());
    EXPECT_FALSE(To<LayoutTextFragment>(letter_a.GetLayoutObject())->IsRemainingTextLayoutObject());
    EXPECT_FALSE(To<LayoutTextFragment>(letter_a.GetLayoutObject())->GetFirstLetterPseudoElement());
    ASSERT_TRUE(letter_x.GetLayoutObject()->IsTextFragment()) << "'x' is first letter-part";
    EXPECT_TRUE(To<LayoutTextFragment>(letter_x.GetLayoutObject())->IsRemainingTextLayoutObject());
    ASSERT_TRUE(letter_x.GetLayoutObject()->GetFirstLetterPart());
    EXPECT_EQ("x", letter_x.GetLayoutObject()->GetFirstLetterPart()->TransformedText());
}
...
TEST_F(LayoutTextFragmentTest, SplitTextWithZero)
{
    ...
    FirstLetterPseudoElement& first_letter_element = *To<LayoutTextFragment>(xy.GetLayoutObject())->GetFirstLetterPseudoElement();
    EXPECT_EQ(first_letter_element.GetLayoutObject(), xy.GetLayoutObject()->PreviousSibling())
