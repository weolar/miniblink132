...
typedef HeapHashMap<WeakMember<const LayoutText>, Member<SecureTextTimer>> SecureTextTimerMap;
static SecureTextTimerMap& GetSecureTextTimers()
{
    DEFINE_STATIC_LOCAL(const Persistent<SecureTextTimerMap>, map, (MakeGarbageCollected<SecureTextTimerMap>()));
    return *map;
}

class SecureTextTimer final : public GarbageCollected<SecureTextTimer>, public TimerBase {
public:
    explicit SecureTextTimer(LayoutText* layout_text)
        : TimerBase(layout_text->GetDocument().GetTaskRunner(TaskType::kUserInteraction))
    ...
    void RestartWithNewText(unsigned last_typed_character_offset)
    {
        ...
        if (Settings* settings = layout_text_->GetDocument().GetSettings()) {
            StartOneShot(base::Seconds(settings->GetPasswordEchoDurationInSeconds()), FROM_HERE);
        }
    }
    void Invalidate()
    {
    ...
    {
        return last_typed_character_offset_;
    }
...
class SelectionDisplayItemClient : public GarbageCollected<SelectionDisplayItemClient>, public DisplayItemClient {
public:
...
using SelectionDisplayItemClientMap = HeapHashMap<WeakMember<const LayoutText>, Member<SelectionDisplayItemClient>>;
SelectionDisplayItemClientMap& GetSelectionDisplayItemClientMap()
{
    DEFINE_STATIC_LOCAL(Persistent<SelectionDisplayItemClientMap>, map, (MakeGarbageCollected<SelectionDisplayItemClientMap>()));
    return *map;
}
...
LayoutText* LayoutText::CreateEmptyAnonymous(Document& doc, const ComputedStyle* style)
{
...
void LayoutText::StyleWillChange(StyleDifference diff, const ComputedStyle& new_style)
{
    NOT_DESTROYED();

    if (const ComputedStyle* current_style = Style()) {
        // Process accessibility for style changes that affect text.
        if (current_style->Visibility() != new_style.Visibility() || current_style->IsInert() != new_style.IsInert()) {
...
void LayoutText::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    ...
    ETextTransform old_transform = old_style ? old_style->TextTransform() : ETextTransform::kNone;
    ETextSecurity old_security = old_style ? old_style->TextSecurity() : ETextSecurity::kNone;
    if (old_transform != new_style.TextTransform() || old_security != new_style.TextSecurity()) {
        TransformAndSecureOriginalText();
    } else if (old_transform == new_style.TextTransform() && new_style.TextTransform() != ETextTransform::kNone && old_style->Locale() != new_style.Locale()) {
        TransformAndSecureOriginalText();
    }
...
Vector<LayoutText::TextBoxInfo> LayoutText::GetTextBoxInfo() const
{
    ...
    if (const OffsetMapping* mapping = GetOffsetMapping()) {
        ...
        for (; cursor; cursor.MoveToNextForSameLayoutObject()) {
            ...
            for (const OffsetMappingUnit& unit : mapping->GetMappingUnitsForTextContentOffsetRange(offset.start, offset.end)) {
                ...
                const unsigned clamped_start = std::max(unit.TextContentStart(), offset.start);
                const unsigned clamped_end = std::min(unit.TextContentEnd(), offset.end);
                ...
                if (unit.AssociatedNode()) {
                    ...
                    if (const std::optional<unsigned> box_start = CaretOffsetForPosition(mapping->GetLastPosition(clamped_start))) {
                ...
                const OffsetMappingUnit* const mapping_unit = mapping->GetLastMappingUnit(clamped_start);
                DCHECK(mapping_unit) << this << " at " << clamped_start;
                const unsigned dom_offset = mapping_unit->ConvertTextContentToLastDOMOffset(clamped_start);
...
String LayoutText::PlainText() const
{
    ...
    if (GetNode()) {
        if (const OffsetMapping* mapping = GetOffsetMapping()) {
            ...
            for (const OffsetMappingUnit& unit : mapping->GetMappingUnitsForNode(*GetNode())) {
                result.Append(StringView(mapping->GetText(), unit.TextContentStart(), unit.TextContentEnd() - unit.TextContentStart()));
            }
    ...
    for (const auto& text_box : GetTextBoxInfo()) {
        ...
        if (last_end_offset && text_box.dom_start_offset > last_end_offset && !IsASCIISpace(text_[end_offset - 1])) {
            plain_text_builder.Append(kSpaceCharacter);
        }
        last_end_offset = end_offset;

        String text = text_.Substring(text_box.dom_start_offset, text_box.dom_length).SimplifyWhiteSpace(WTF::kDoNotStripWhiteSpace);
        plain_text_builder.Append(text);
    }
    return plain_text_builder.ToString();
}

template <typename PhysicalRectCollector> void LayoutText::CollectLineBoxRects(const PhysicalRectCollector& yield, ClippingOption option) const
{
    NOT_DESTROYED();
    if (IsInLayoutNGInlineFormattingContext()) {
        InlineCursor cursor;
...
void LayoutText::QuadsInAncestorInternal(Vector<gfx::QuadF>& quads, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    CollectLineBoxRects([this, &quads, ancestor, mode](const PhysicalRect& r) { quads.push_back(LocalRectToAncestorQuad(r, ancestor, mode)); });
}

bool LayoutText::MapDOMOffsetToTextContentOffset(const OffsetMapping& mapping, unsigned* start, unsigned* end) const
{
    NOT_DESTROYED();
    DCHECK_LE(*start, *end);

    // Adjust |start| to the next non-collapsed offset if |start| is collapsed.
    Position start_position = PositionForCaretOffset(std::min(*start, OriginalTextLength()));
    Position non_collapsed_start_position = mapping.StartOfNextNonCollapsedContent(start_position);

    // If all characters after |start| are collapsed, adjust to the last
    // non-collapsed offset.
    if (non_collapsed_start_position.IsNull()) {
        non_collapsed_start_position = mapping.EndOfLastNonCollapsedContent(start_position);
    ...
    *start = mapping.GetTextContentOffset(non_collapsed_start_position).value();
    ...
    Position end_position = PositionForCaretOffset(std::min(*end, OriginalTextLength()));
    Position non_collpased_end_position = mapping.EndOfLastNonCollapsedContent(end_position);

    // Note: `non_collpased_{start,end}_position}` can be position before/after
    // non-`Text` node. See http://crbug.com/1389193
    if (non_collpased_end_position.IsNull() || non_collpased_end_position <= non_collapsed_start_position) {
...
void LayoutText::AbsoluteQuadsForRange(Vector<gfx::QuadF>& quads, unsigned start, unsigned end) const
{
    NOT_DESTROYED();
    ...
    if (auto* mapping = GetOffsetMapping()) {
        ...
        for (cursor.MoveTo(*this); cursor; cursor.MoveToNextForSameLayoutObject()) {
            ...
            if (const SvgFragmentData* svg_data = item.GetSvgFragmentData()) {
                ...
                quad = item.BuildSvgTransformForBoundingBox().MapQuad(gfx::QuadF(float_rect));
...
gfx::RectF LayoutText::LocalBoundingBoxRectForAccessibility() const
{
    NOT_DESTROYED();
    gfx::RectF result;
    CollectLineBoxRects([&result](const PhysicalRect& rect) { result.Union(gfx::RectF(rect)); }, kClipToEllipsis);
    return result;
}

PositionWithAffinity LayoutText::PositionForPoint(const PhysicalOffset& point) const
{
    NOT_DESTROYED();
    ...
    if (IsInLayoutNGInlineFormattingContext()) {
        ...
        if (containing_block_flow->IsScrollContainer()) {
            point_in_contents += PhysicalOffset(containing_block_flow->PixelSnappedScrolledContentOffset());
        }
        const auto* const text_combine = DynamicTo<LayoutTextCombine>(Parent());
        ...
        for (; cursor; cursor.MoveToNextForSameLayoutObject()) {
            ...
            if (container_fragment != &cursor.ContainerFragment()) {
                ...
                point_in_container_fragment = point_in_contents - container_fragment->OffsetFromOwnerLayoutBox();
                if (text_combine) [[unlikely]] {
                    point_in_container_fragment = text_combine->AdjustOffsetForHitTest(point_in_container_fragment);
                }
            }
            if (!ToEnclosingRect(cursor.Current().RectInContainerFragment()).Contains(ToFlooredPoint(point_in_container_fragment)))
                continue;
            if (auto position_with_affinity = cursor.PositionForPointInChild(point_in_container_fragment)) {
...
PhysicalOffset LayoutText::FirstLineBoxTopLeft() const
{
    ...
    if (IsInLayoutNGInlineFormattingContext()) {
        ...
        InlineCursor cursor;
        cursor.MoveTo(*this);
        return cursor ? cursor.Current().OffsetInContainerFragment() : PhysicalOffset();
    }
    return PhysicalOffset();
}

void LayoutText::LogicalStartingPointAndHeight(LogicalOffset& logical_starting_point, LayoutUnit& logical_height) const
{
    NOT_DESTROYED();
    if (IsInLayoutNGInlineFormattingContext()) {
        InlineCursor cursor;
        ...
        if (!cursor)
            return;
        PhysicalOffset physical_offset = cursor.Current().OffsetInContainerFragment();
        if (StyleRef().GetWritingDirection().IsHorizontalLtr()) {
            cursor.MoveToLastForSameLayoutObject();
            logical_height = cursor.Current().RectInContainerFragment().Bottom() - physical_offset.top;
            logical_starting_point = { physical_offset.left, physical_offset.top };
            return;
        }
        PhysicalSize outer_size = ContainingBlock()->Size();
        logical_starting_point = physical_offset.ConvertToLogical(StyleRef().GetWritingDirection(), outer_size, cursor.Current().Size());
        cursor.MoveToLastForSameLayoutObject();
        PhysicalRect last_physical_rect = cursor.Current().RectInContainerFragment();
        LogicalOffset logical_ending_point = WritingModeConverter(StyleRef().GetWritingDirection(), outer_size).ToLogical(last_physical_rect).EndOffset();
        logical_height = logical_ending_point.block_offset - logical_starting_point.block_offset;
    }
}
...
UChar LayoutText::PreviousCharacter() const
{
    ...
    if (previous_text && previous_text->IsText()) {
        if (const String& previous_string = To<LayoutText>(previous_text)->TransformedText()) {
...
void LayoutText::SetTextInternal(String text)
{
    ...
    DCHECK(text_);
    DCHECK(!IsBR() || (TransformedTextLength() == 1 && text_[0] == kNewlineCharacter));
}

String LayoutText::TransformAndSecureText(const String& original, TextOffsetMap& offset_map) const
{
    NOT_DESTROYED();
    if (const ComputedStyle* style = Style()) {
        String transformed = style->ApplyTextTransform(original, PreviousCharacter(), &offset_map);
        ...
        if (!secure_map.IsEmpty()) {
            offset_map = TextOffsetMap(offset_map, secure_map, RuntimeEnabledFeatures::TextTransformAndSecurityFixEnabled());
        }
        return masked;
    }
    return original;
...
std::pair<String, TextOffsetMap> LayoutText::SecureText(const String& plain, UChar mask) const
{
    NOT_DESTROYED();
    if (!plain.length()) {
    ...
    if (auto* secure_text_timer = SecureTextTimer::ActiveInstanceFor(this)) {
        last_typed_character_offset_to_reveal = secure_text_timer->LastTypedCharacterOffset();
    }
    ...
    for (unsigned offset = 0; offset < plain.length();) {
        ...
        if (last_typed_character_offset_to_reveal >= 0) {
            unsigned last_typed_offset = base::checked_cast<unsigned>(last_typed_character_offset_to_reveal);
...
void LayoutText::SetVariableLengthTransformResult(wtf_size_t original_length, const TextOffsetMap& offset_map)
{
    ...
    View()->RegisterVariableLengthTransformResult(*this, { original_length, offset_map });
}

VariableLengthTransformResult LayoutText::GetVariableLengthTransformResult() const
{
...
void LayoutText::SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(LayoutInvalidationReasonForTracing reason)
{
    ...
    if (text_combine) [[unlikely]] {
        ...
        text_combine->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(reason);
        return;
    }
    LayoutObject::SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(reason);
}
...
void LayoutText::TextDidChange()
{
    ...
    if (Parent()) {
        SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kTextChanged);
    }
...
PhysicalRect LayoutText::PhysicalLinesBoundingBox() const
{
    NOT_DESTROYED();
    PhysicalRect result;
    CollectLineBoxRects([&result](const PhysicalRect& r) { result.UniteIfNonZero(r); });
...
PhysicalRect LayoutText::LocalSelectionVisualRect() const
{
    ...
    if (IsInLayoutNGInlineFormattingContext()) {
        ...
        float scaling_factor = svg_inline_text ? svg_inline_text->ScalingFactor() : 1.0f;
        PhysicalRect rect;
        InlineCursor cursor(*FragmentItemsContainer());
        for (cursor.MoveTo(*this); cursor; cursor.MoveToNextForSameLayoutObject()) {
            if (cursor.Current().IsHiddenForPaint())
                continue;
            const LayoutSelectionStatus status = frame_selection.ComputeLayoutSelectionStatus(cursor);
            ...
            if (svg_inline_text) {
                ...
                if (item.HasSvgTransformForBoundingBox()) {
                    float_rect = item.BuildSvgTransformForBoundingBox().MapRect(float_rect);
                }
...
std::optional<unsigned> LayoutText::CaretOffsetForPosition(const Position& position) const
{
    NOT_DESTROYED();
    // ::first-letter handling should be done by LayoutTextFragment override.
    DCHECK(!IsTextFragment());
    ...
    DCHECK_LE(position.OffsetInContainerNode(), static_cast<int>(OriginalTextLength())) << position;
...
int LayoutText::CaretMinOffset() const
{
    ...
    if (auto* mapping = GetOffsetMapping()) {
        ...
        if (first_position.IsNull())
            return 0;
        std::optional<unsigned> candidate = CaretOffsetForPosition(mapping->StartOfNextNonCollapsedContent(first_position));
        // Align with the legacy behavior that 0 is returned if the entire node
        // contains only collapsed whitespaces.
        const bool fully_collapsed = !candidate || *candidate == TransformedTextLength();
...
int LayoutText::CaretMaxOffset() const
{
    ...
    if (auto* mapping = GetOffsetMapping()) {
        ...
        if (last_position.IsNull())
            return text_length;
        std::optional<unsigned> candidate = CaretOffsetForPosition(mapping->EndOfLastNonCollapsedContent(last_position));
...
unsigned LayoutText::ResolvedTextLength() const
{
    ...
    if (auto* mapping = GetOffsetMapping()) {
        ...
        const Position end_position = PositionForCaretOffset(NonCollapsedCaretMaxOffset());
        if (start_position.IsNull()) {
            DCHECK(end_position.IsNull()) << end_position;
            return 0;
        }
        DCHECK(end_position.IsNotNull()) << start_position;
        std::optional<unsigned> start = mapping->GetTextContentOffset(start_position);
...
bool LayoutText::ContainsCaretOffset(int text_offset) const
{
    ...
    if (auto* mapping = GetOffsetMapping()) {
        ...
        if (text_offset < text_length && mapping->IsBeforeNonCollapsedContent(position)) {
            return true;
        }
...
void LayoutText::MomentarilyRevealLastTypedCharacter(unsigned last_typed_character_offset)
{
    NOT_DESTROYED();
    auto it = GetSecureTextTimers().find(this);
    SecureTextTimer* secure_text_timer = it != GetSecureTextTimers().end() ? it->value : nullptr;
...
void LayoutText::InvalidateDisplayItemClients(PaintInvalidationReason reason) const
{
    NOT_DESTROYED();
    LayoutObject::InvalidateDisplayItemClients(reason);

    if (const auto* selection_client = GetSelectionDisplayItemClient()) {
        ObjectPaintInvalidator(*this).InvalidateDisplayItemClient(*selection_client, reason);
    }
...
const DisplayItemClient* LayoutText::GetSelectionDisplayItemClient() const
{
    ...
    const auto* const text_combine = DynamicTo<LayoutTextCombine>(Parent());
    if (text_combine && text_combine->NeedsAffineTransformInPaint()) [[unlikely]] {
        return text_combine;
    }
    if (!IsSelected())
        return nullptr;
    ...
    if (it != GetSelectionDisplayItemClientMap().end())
        return &*it->value;
    return GetSelectionDisplayItemClientMap().insert(this, MakeGarbageCollected<SelectionDisplayItemClient>()).stored_value->value.Get();
}
...
void LayoutText::SetInlineItems(InlineItemsData* data, wtf_size_t begin, wtf_size_t size)
{
    NOT_DESTROYED();
#if DCHECK_IS_ON()
