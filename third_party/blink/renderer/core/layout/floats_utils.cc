...
BfcOffset AdjustToTopEdgeAlignmentRule(const ExclusionSpace& exclusion_space, const BfcOffset& offset)
{
    BfcOffset adjusted_offset = offset;
    adjusted_offset.block_offset = std::max(adjusted_offset.block_offset, exclusion_space.LastFloatBlockStart());

    return adjusted_offset;
}

LayoutOpportunity FindLayoutOpportunityForFloat(
    const UnpositionedFloat& unpositioned_float, const ExclusionSpace& exclusion_space, const BoxStrut& fragment_margins, LayoutUnit inline_size)
{
    BfcOffset adjusted_origin_point = AdjustToTopEdgeAlignmentRule(exclusion_space, unpositioned_float.origin_bfc_offset);
    ...
    const LayoutUnit clearance_offset = std::max({ exclusion_space.ClearanceOffset(clear_type), exclusion_space.InitialLetterClearanceOffset(float_type) });

    AdjustToClearance(clearance_offset, &adjusted_origin_point);

    return exclusion_space.FindLayoutOpportunity(
        adjusted_origin_point, unpositioned_float.available_size.inline_size, inline_size + fragment_margins.InlineSum() /* minimum_inline_size */);
}
...
ConstraintSpace CreateConstraintSpaceForFloat(
    const UnpositionedFloat& unpositioned_float, std::optional<LayoutUnit> origin_block_offset = std::nullopt, std::optional<BoxStrut> margins = std::nullopt)
{
    ...
    ConstraintSpaceBuilder builder(parent_space, style.GetWritingDirection(),
        /* is_new_fc */ true);
    SetOrthogonalFallbackInlineSizeIfNeeded(unpositioned_float.parent_style, unpositioned_float.node, &builder);
    ...
    if (origin_block_offset) {
        ...
        SetupSpaceBuilderForFragmentation(parent_space, unpositioned_float.node, unpositioned_float.fragmentainer_block_offset + *origin_block_offset,
        ...
        // truncated.
        LayoutUnit margin_edge_offset = unpositioned_float.fragmentainer_block_offset + *origin_block_offset - margins->block_start;
    ...
    builder.SetReplacedPercentageResolutionSize(unpositioned_float.replaced_percentage_size);
    return builder.ToConstraintSpace();
}

ExclusionShapeData* CreateExclusionShapeData(const BoxStrut& margins, const UnpositionedFloat& unpositioned_float)
{
    ...
    BoxStrut new_margins(margins.LineLeft(direction), margins.LineRight(direction), margins.block_start, margins.block_end);
    ...
    case CSSBoxType::kContent:
        const ConstraintSpace space = CreateConstraintSpaceForFloat(unpositioned_float);
        ...
        shape_insets = strut.ConvertToPhysical(style.GetWritingDirection()).ConvertToLogical({ parent_space.GetWritingMode(), TextDirection::kLtr });
        break;
    }

    return MakeGarbageCollected<ExclusionShapeData>(layout_box, new_margins, shape_insets);
}

// Creates an exclusion from the fragment that will be placed in the provided
// layout opportunity.
const ExclusionArea* CreateExclusionArea(const LogicalFragment& fragment, const BfcOffset& float_margin_bfc_offset, const BoxStrut& margins,
    const UnpositionedFloat& unpositioned_float, EFloat type)
{
    BfcOffset start_offset = float_margin_bfc_offset;
    BfcOffset end_offset(start_offset.line_offset + (fragment.InlineSize() + margins.InlineSum()).ClampNegativeToZero(),
        start_offset.block_offset + (fragment.BlockSize() + margins.BlockSum()).ClampNegativeToZero());

    ExclusionShapeData* shape_data
        = unpositioned_float.node.GetLayoutBox()->GetShapeOutsideInfo() ? CreateExclusionShapeData(margins, unpositioned_float) : nullptr;

    return ExclusionArea::Create(BfcRect(start_offset, end_offset), type, unpositioned_float.is_hidden_for_paint, std::move(shape_data));
}
...
void LayoutFloatWithoutFragmentation(UnpositionedFloat* unpositioned_float)
{
    ...
    unpositioned_float->layout_result = unpositioned_float->node.Layout(space, unpositioned_float->token);
    unpositioned_float->margins = ComputeMarginsFor(space, unpositioned_float->node.Style(), unpositioned_float->parent_space);
}

} // namespace

LayoutUnit ComputeMarginBoxInlineSizeForUnpositionedFloat(UnpositionedFloat* unpositioned_float)
{
    ...
    const auto& fragment = unpositioned_float->layout_result->GetPhysicalFragment();
    DCHECK(!fragment.GetBreakToken());

    const ConstraintSpace& parent_space = unpositioned_float->parent_space;

    return (LogicalFragment(parent_space.GetWritingDirection(), fragment).InlineSize() + unpositioned_float->margins.InlineSum()).ClampNegativeToZero();
}

PositionedFloat PositionFloat(UnpositionedFloat* unpositioned_float, ExclusionSpace* exclusion_space)
{
    DCHECK(unpositioned_float);
    const ConstraintSpace& parent_space = unpositioned_float->parent_space;
    BlockNode node = unpositioned_float->node;
    bool is_same_writing_mode = node.Style().GetWritingMode() == parent_space.GetWritingMode();

    bool is_fragmentable = is_same_writing_mode && parent_space.HasBlockFragmentation();

    const LayoutResult* layout_result = nullptr;
    BoxStrut fragment_margins;
    LayoutOpportunity opportunity;
    LayoutUnit fragmentainer_block_size = unpositioned_float->fragmentainer_block_size;
    ...
    if (!is_fragmentable) {
        ...
        LogicalFragment float_fragment(parent_space.GetWritingDirection(), layout_result->GetPhysicalFragment());

        // Find a layout opportunity that will fit our float.
        opportunity = FindLayoutOpportunityForFloat(*unpositioned_float, *exclusion_space, fragment_margins, float_fragment.InlineSize());
    } else {
        fragment_margins = ComputeMarginsFor(node.Style(), unpositioned_float->percentage_size.inline_size, parent_space.GetWritingDirection());
        ...
        if (unpositioned_float->layout_result) {
            ...
            LogicalFragment float_fragment(parent_space.GetWritingDirection(), unpositioned_float->layout_result->GetPhysicalFragment());
            // We can find a layout opportunity and set the fragmentainer offset right
            // away.
            opportunity = FindLayoutOpportunityForFloat(*unpositioned_float, *exclusion_space, fragment_margins, float_fragment.InlineSize());
            fragmentainer_delta = opportunity.rect.start_offset.block_offset + fragment_margins.block_start;
        } else {
            ...
            // lay out again.
            fragmentainer_delta = unpositioned_float->origin_bfc_offset.block_offset + fragment_margins.block_start;
        ...
        do {
            ConstraintSpace space
                = CreateConstraintSpaceForFloat(*unpositioned_float, fragmentainer_delta - parent_space.ExpectedBfcBlockOffset(), fragment_margins);
            ...
            LayoutUnit new_fragmentainer_delta = opportunity.rect.start_offset.block_offset + fragment_margins.block_start;
        ...
        if (!is_at_fragmentainer_start) {
            LayoutUnit fragmentainer_block_offset
                = unpositioned_float->FragmentainerOffsetAtBfc() + opportunity.rect.start_offset.block_offset + fragment_margins.block_start;
            ...
            if (!MovePastBreakpoint(parent_space, node, *layout_result, fragmentainer_block_offset, fragmentainer_block_size, kBreakAppealPerfect,
                    /*builder=*/nullptr)) {
                need_break_before = true;
            } else if (is_at_block_end && parent_space.HasKnownFragmentainerBlockSize()) {
                LogicalFragment float_fragment(parent_space.GetWritingDirection(), layout_result->GetPhysicalFragment());
                LayoutUnit outer_block_end = fragmentainer_block_offset + float_fragment.BlockSize() + fragment_margins.block_end;
                if (outer_block_end > fragmentainer_block_size && !IsBreakInside(unpositioned_float->token)) {
    ...
    LogicalFragment float_fragment(parent_space.GetWritingDirection(), physical_fragment);
    ...
    if (unpositioned_float->IsLineRight(parent_space.Direction())) {
        LayoutUnit float_margin_box_inline_size = float_fragment.InlineSize() + fragment_margins.InlineSum();
        float_margin_bfc_offset.line_offset += (opportunity.rect.InlineSize() - float_margin_box_inline_size);
    }

    if (parent_space.HasBlockFragmentation() && !need_break_before && !IsBreakInside(unpositioned_float->token)
        && exclusion_space->NeedsBreakBeforeFloat(unpositioned_float->ClearType(parent_space.Direction())))
        need_break_before = true;
    ...
    if (need_break_before) {
        ...
        BfcOffset past_everything(LayoutUnit(), unpositioned_float->FragmentainerSpaceLeft() + parent_space.ExpectedBfcBlockOffset());
        const ExclusionArea* exclusion = ExclusionArea::Create(BfcRect(past_everything, past_everything), float_type, unpositioned_float->is_hidden_for_paint);
        ...
        const ExclusionArea* exclusion = CreateExclusionArea(float_fragment, float_margin_bfc_offset, fragment_margins, *unpositioned_float, float_type);
        exclusion_space->Add(std::move(exclusion));
    ...
    BfcOffset float_bfc_offset(float_margin_bfc_offset.line_offset + fragment_margins.LineLeft(parent_space.Direction()),
        float_margin_bfc_offset.block_offset + fragment_margins.block_start);

    const BlockBreakToken* break_before_token = nullptr;
    if (need_break_before) {
        break_before_token = BlockBreakToken::CreateBreakBefore(node, /* is_forced_break */ false);
    }

    LayoutUnit minimum_space_shortage;
    if (break_before_token || physical_fragment.GetBreakToken()) {
        // Broke before or inside the float.
        if (parent_space.HasKnownFragmentainerBlockSize() && parent_space.BlockFragmentationType() == kFragmentColumn) {
            LayoutUnit fragmentainer_block_offset = unpositioned_float->FragmentainerOffsetAtBfc() + float_bfc_offset.block_offset;
            minimum_space_shortage = CalculateSpaceShortage(parent_space, layout_result, fragmentainer_block_offset, fragmentainer_block_size);
        }
    }

    return PositionedFloat(layout_result, break_before_token, float_bfc_offset, minimum_space_shortage);
}
