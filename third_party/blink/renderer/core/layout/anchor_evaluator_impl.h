...
template <typename AnchorReference> class AnchorQueryBase : public GarbageCollectedMixin {
    using NamedAnchorMap = HeapHashMap<Member<const ScopedCSSName>, Member<AnchorReference>>;
    using ImplicitAnchorMap = HeapHashMap<Member<const LayoutObject>, Member<AnchorReference>>;
...
public:
    ...
    {
        if (const ScopedCSSName* const* name = absl::get_if<const ScopedCSSName*>(&key)) {
            return GetAnchorReference(named_anchors_, *name);
        }
        return GetAnchorReference(implicit_anchors_, absl::get<const LayoutObject*>(key));
    }
    ...
    AddResult insert(const AnchorKey& key, AnchorReference* reference)
    {
        ...
        return insert(implicit_anchors_, absl::get<const LayoutObject*>(key), reference);
    }
    ...
    public:
        Iterator(const AnchorQueryBase* anchor_query, typename NamedAnchorMap::const_iterator named_map_iterator,
        ...
        {
            return named_map_iterator_ == other.named_map_iterator_ && implicit_map_iterator_ == other.implicit_map_iterator_;
        }
        bool operator!=(const Iterator& other) const
        {
...
private:
    ...
    template <typename AnchorMapType, typename KeyType> static const AnchorReference* GetAnchorReference(const AnchorMapType& anchors, const KeyType& key)
    {
        auto it = anchors.find(key);
        return it != anchors.end() ? it->value : nullptr;
    }

    template <typename AnchorMapType, typename KeyType> static AddResult insert(AnchorMapType& anchors, const KeyType& key, AnchorReference* reference)
    {
...
struct CORE_EXPORT PhysicalAnchorReference : public GarbageCollected<PhysicalAnchorReference> {
    PhysicalAnchorReference(const LogicalAnchorReference& logical_reference, const WritingModeConverter& converter);
...
class CORE_EXPORT PhysicalAnchorQuery : public AnchorQueryBase<PhysicalAnchorReference> {
public:
    using Base = AnchorQueryBase<PhysicalAnchorReference>;

    const PhysicalAnchorReference* AnchorReference(const LayoutObject& query_object, const AnchorKey&) const;
    const LayoutObject* AnchorLayoutObject(const LayoutObject& query_object, const AnchorKey&) const;

    void SetFromLogical(const LogicalAnchorQuery& logical_query, const WritingModeConverter& converter);
};

struct CORE_EXPORT LogicalAnchorReference : public GarbageCollected<LogicalAnchorReference> {
    LogicalAnchorReference(const LayoutObject& layout_object, const LogicalRect& rect, bool is_out_of_flow, HeapHashSet<Member<Element>>* display_locks)
        : rect(rect)
...
class CORE_EXPORT LogicalAnchorQuery : public GarbageCollected<LogicalAnchorQuery>, public AnchorQueryBase<LogicalAnchorReference> {
public:
    ...
    const LogicalAnchorReference* AnchorReference(const LayoutObject& query_object, const AnchorKey&) const;
    ...
    void Set(const AnchorKey&, const LayoutObject& layout_object, const LogicalRect& rect, SetOptions, Element* element_for_display_lock);
    void Set(const AnchorKey&, LogicalAnchorReference* reference);
    // If the element owning this object has a display lock, the element should be
    // passed as |element_for_display_lock|.
    void SetFromPhysical(const PhysicalAnchorQuery& physical_query, const WritingModeConverter& converter, const LogicalOffset& additional_offset, SetOptions,
    ...
    std::optional<LayoutUnit> EvaluateAnchor(const LogicalAnchorReference& reference, CSSAnchorValue anchor_value, float percentage, LayoutUnit available_size,
        const WritingModeConverter& container_converter, WritingDirectionMode self_writing_direction, const PhysicalOffset& offset_to_padding_box,
        bool is_y_axis, bool is_right_or_bottom) const;
    LayoutUnit EvaluateSize(
...
public:
    ...
    AnchorEvaluatorImpl(const LayoutObject& query_object, const LogicalAnchorQuery& anchor_query, const LayoutObject* implicit_anchor,
        const WritingModeConverter& container_converter, WritingDirectionMode self_writing_direction, const PhysicalOffset& offset_to_padding_box,
        ...
        , display_locks_affected_by_anchors_(MakeGarbageCollected<HeapHashSet<Member<Element>>>())
    {
    ...
    AnchorEvaluatorImpl(const LayoutObject& query_object, const LogicalAnchorQueryMap& anchor_queries, const LayoutObject* implicit_anchor,
        const LayoutObject& containing_block, const WritingModeConverter& container_converter, WritingDirectionMode self_writing_direction,
        const PhysicalOffset& offset_to_padding_box, const PhysicalSize& available_size)
        : query_object_(&query_object)
    ...
    std::optional<LayoutUnit> Evaluate(const class AnchorQuery&, const ScopedCSSName* position_anchor, const std::optional<PositionAreaOffsets>&) override;

    std::optional<PositionAreaOffsets> ComputePositionAreaOffsetsForLayout(const ScopedCSSName* position_anchor, PositionArea position_area) override;
...
private:
    const LogicalAnchorReference* ResolveAnchorReference(const AnchorSpecifierValue& anchor_specifier, const ScopedCSSName* position_anchor) const;
    bool ShouldUseScrollAdjustmentFor(const LayoutObject* anchor, const ScopedCSSName* position_anchor) const;

    std::optional<LayoutUnit> EvaluateAnchor(const AnchorSpecifierValue& anchor_specifier, CSSAnchorValue anchor_value, float percentage,
        const ScopedCSSName* position_anchor, const std::optional<PositionAreaOffsets>&) const;
    ...
    const PaintLayer* DefaultAnchorScrollContainerLayer(const ScopedCSSName* position_anchor) const;
    ...
    LayoutUnit AvailableSizeAlongAxis(const PhysicalRect& position_area_modified_containing_block_rect) const
    {
        return IsYAxis() ? position_area_modified_containing_block_rect.Height() : position_area_modified_containing_block_rect.Width();
    }
    ...
    PhysicalRect PositionAreaModifiedContainingBlock(const std::optional<PositionAreaOffsets>&) const;
    ...
    const WritingModeConverter container_converter_ { { WritingMode::kHorizontalTb, TextDirection::kLtr } };
    WritingDirectionMode self_writing_direction_ { WritingMode::kHorizontalTb, TextDirection::kLtr };
    ...
    template <typename KeyType, typename ValueType> class CachedValue {
        STACK_ALLOCATED();
    ...
    public:
        template <typename CreationFunc> ValueType Get(KeyType key, CreationFunc create)
        {
    ...
    mutable CachedValue<std::optional<PositionAreaOffsets>, PhysicalRect> cached_position_area_modified_containing_block_;

    // Caches most recent result of DefaultAnchor.
    mutable CachedValue<const ScopedCSSName*, const LayoutObject*> cached_default_anchor_;

    // Caches most recent result of DefaultAnchorScrollContainerLayer.
    mutable CachedValue<const ScopedCSSName*, const PaintLayer*> cached_default_anchor_scroll_container_layer_;
    ...
    mutable HeapHashSet<Member<Element>>* display_locks_affected_by_anchors_ = nullptr;
};
