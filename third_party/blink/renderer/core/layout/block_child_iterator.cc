...
BlockChildIterator::BlockChildIterator(LayoutInputNode first_child, const BlockBreakToken* break_token, bool calculate_child_idx)
...
BlockChildIterator::Entry BlockChildIterator::NextChild(const InlineBreakToken* previous_inline_break_token)
{
    if (previous_inline_break_token) {
        DCHECK(!child_idx_);
        return Entry(previous_inline_break_token->InputNode(), previous_inline_break_token, std::nullopt);
    }
        ...
        if (break_token_) {
            ...
            if (child_token_idx_ == child_break_tokens.size()) {
                ...
                if (!break_token_->HasSeenAllChildren()) {
                    AdvanceToNextChild(child_break_tokens[child_token_idx_ - 1]->InputNode());
                }
                break_token_ = nullptr;
            }
