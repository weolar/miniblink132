...
public:
    LogicalBoxFragment(WritingDirectionMode writing_direction, const PhysicalBoxFragment& physical_fragment)
    ...
    {
        return writing_direction_.GetWritingMode() == physical_fragment_.Style().GetWritingMode();
    }

    static LayoutUnit SynthesizedBaseline(FontBaseline baseline_type, bool is_flipped_lines, LayoutUnit block_size)
    {
    ...
    LayoutUnit FirstBaselineOrSynthesize(FontBaseline baseline_type) const
    {
        ...
        return SynthesizedBaseline(baseline_type, writing_direction_.IsFlippedLines(), BlockSize());
    }
    ...
    {
        return GetPhysicalBoxFragment().Borders().ConvertToLogical(writing_direction_);
    }
    BoxStrut Scrollbar() const
    {
        return GetPhysicalBoxFragment().Scrollbar().ConvertToLogical(writing_direction_);
    }
    BoxStrut Padding() const
    {
        return GetPhysicalBoxFragment().Padding().ConvertToLogical(writing_direction_);
    }
