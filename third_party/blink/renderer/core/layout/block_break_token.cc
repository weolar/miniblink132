...
BlockBreakToken* BlockBreakToken::Create(BoxFragmentBuilder* builder)
{
    ...
    return MakeGarbageCollected<BlockBreakToken>(AdditionalBytes(builder->child_break_tokens_.size() * sizeof(Member<BreakToken>)), PassKey(), builder);
}

BlockBreakToken* BlockBreakToken::CreateRepeated(const BlockNode& node, unsigned sequence_number)
{
...
BlockBreakToken* BlockBreakToken::CreateForBreakInRepeatedFragment(
    const BlockNode& node, unsigned sequence_number, LayoutUnit consumed_block_size, bool is_at_block_end)
{
    auto* token = MakeGarbageCollected<BlockBreakToken>(PassKey(), node);
    token->data_ = MakeGarbageCollected<BlockBreakTokenData>();
    token->data_->sequence_number = sequence_number;
    ...
    , const_num_children_(builder->child_break_tokens_.size())
{
    ...
    has_unpositioned_list_marker_ = static_cast<bool>(builder->GetUnpositionedListMarker());
...
const InlineBreakToken* BlockBreakToken::InlineBreakTokenFor(const LayoutInputNode& node) const
{
    DCHECK(node.GetLayoutBox());
    return InlineBreakTokenFor(*node.GetLayoutBox());
}

const InlineBreakToken* BlockBreakToken::InlineBreakTokenFor(const LayoutBox& layout_object) const
{
...
void BlockBreakToken::MutableForOofFragmentation::Merge(const BlockBreakToken& new_break_token)
{
    if (LayoutUnit monolithic_overflow = new_break_token.MonolithicOverflow()) {
        ...
        break_token_.data_->monolithic_overflow = std::max(break_token_.data_->monolithic_overflow, monolithic_overflow);
    }
}
