...
void LayoutImageResource::SetImageResource(ImageResourceContent* new_image)
{
    ...
    if (cached_image_) {
        ...
        if (cached_image_->ErrorOccurred()) {
            layout_object_->ImageChanged(cached_image_.Get(), ImageResourceObserver::CanDeferInvalidation::kNo);
        }
    } else {
        layout_object_->ImageChanged(cached_image_.Get(), ImageResourceObserver::CanDeferInvalidation::kNo);
    }
...
RespectImageOrientationEnum LayoutImageResource::ImageOrientation() const
{
    ...
    return cached_image_->ForceOrientationIfNecessary(layout_object_->StyleRef().ImageOrientation());
}

IntrinsicSizingInfo LayoutImageResource::GetNaturalDimensions(float multiplier) const
{
    if (!cached_image_ || !cached_image_->IsSizeAvailable() || !cached_image_->HasImage()) {
        return IntrinsicSizingInfo::None();
    }
    IntrinsicSizingInfo sizing_info;
    Image& image = *cached_image_->GetImage();
    if (auto* svg_image = DynamicTo<SVGImage>(image)) {
        const SVGImageViewInfo* view_info = SVGImageForContainer::CreateViewInfo(*svg_image, layout_object_->GetNode());
        if (!SVGImageForContainer::GetNaturalDimensions(*svg_image, view_info, sizing_info)) {
...
gfx::SizeF LayoutImageResource::ImageSize(float multiplier) const
{
    if (!cached_image_)
        return gfx::SizeF();
    gfx::SizeF size(cached_image_->IntrinsicSize(layout_object_->StyleRef().ImageOrientation()));
    if (multiplier != 1 && HasIntrinsicSize()) {
...
gfx::SizeF LayoutImageResource::ConcreteObjectSize(float multiplier, const gfx::SizeF& default_object_size) const
{
...
Image* LayoutImageResource::BrokenImage(double device_pixel_ratio)
{
    ...
    if (device_pixel_ratio >= 2) {
        DEFINE_STATIC_REF(Image, broken_image_hi_res, (Image::LoadPlatformResource(IDR_BROKENIMAGE, ui::k200Percent)));
        return broken_image_hi_res;
    }

    DEFINE_STATIC_REF(Image, broken_image_lo_res, (Image::LoadPlatformResource(IDR_BROKENIMAGE)));
...
{
    auto* broken_image = ImageResourceContent::CreateLoaded(BrokenImage(DevicePixelRatio()));
    broken_image->SetIsBroken();
    SetImageResource(broken_image);
}

scoped_refptr<Image> LayoutImageResource::GetImage(const gfx::Size& container_size) const
{
    return GetImage(gfx::SizeF(container_size));
}

scoped_refptr<Image> LayoutImageResource::GetImage(const gfx::SizeF& container_size) const
{
    if (!cached_image_)
    ...
    auto preferred_color_scheme = layout_object_->GetDocument().GetStyleEngine().ResolveColorSchemeForEmbedding(&style);
    const SVGImageViewInfo* view_info = SVGImageForContainer::CreateViewInfo(*svg_image, layout_object_->GetNode());
    return SVGImageForContainer::Create(*svg_image, container_size, style.EffectiveZoom(), view_info, preferred_color_scheme);
}
