...
public:
    ...
    inline static LayoutUnit AdjustLayoutUnit(LayoutUnit value, const ComputedStyle& style)
    {
        return LayoutUnit(value / style.EffectiveZoom());
    }
    inline static LayoutUnit AdjustLayoutUnit(LayoutUnit value, LayoutObject& layout_object)
    {
        return AdjustLayoutUnit(value, layout_object.StyleRef());
    }
    inline static PhysicalSize AdjustPhysicalSize(PhysicalSize size, const ComputedStyle& style)
    {
        return PhysicalSize(AdjustLayoutUnit(size.width, style), AdjustLayoutUnit(size.height, style));
    }

    inline static void AdjustQuadMaybeExcludingCSSZoom(gfx::QuadF& quad, const LayoutObject& layout_object)
    {
        float zoom;
    ...
    inline static void AdjustRectMaybeExcludingCSSZoom(gfx::RectF& rect, const LayoutObject& layout_object)
    {
        float zoom;
        if (layout_object.GetDocument().StandardizedBrowserZoomEnabled()) {
            zoom = layout_object.GetFrame()->LayoutZoomFactor();
    ...
    inline static float AdjustScroll(float scroll_offset, const ComputedStyle& style)
    {
