...
public:
    ...
    void BlockInInlineBecameFloatingOrOutOfFlow(LayoutBlockFlow* anonymous_block_child);
    ...
    {
        NOT_DESTROYED();
        return AlwaysCreateLineBoxesForLayoutInline() && !IsInLayoutNGInlineFormattingContext();
    }
    ...
    bool HitTestCulledInline(HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset, const InlineCursor& parent_cursor);
    ...
    PhysicalRect AbsoluteBoundingBoxRectHandlingEmptyInline(MapCoordinatesFlags = 0) const final;
...
protected:
    ...
    PhysicalOffset OffsetFromContainerInternal(const LayoutObject*, MapCoordinatesFlags mode) const final;

private:
    bool AbsoluteTransformDependsOnPoint(const LayoutObject& object) const;
    void QuadsForSelfInternal(Vector<gfx::QuadF>& quads, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode, bool map_to_ancestor) const;
    ...
    template <typename PhysicalRectCollector> void CollectLineBoxRects(const PhysicalRectCollector&) const;

    void AddChildIgnoringContinuation(LayoutObject* new_child, LayoutObject* before_child = nullptr) final;
    void AddChildAsBlockInInline(LayoutObject* new_child, LayoutObject* before_child);
    ...
    LayoutBox* CreateAnonymousBoxToSplit(const LayoutBox* box_to_split) const final;

    void Paint(const PaintInfo&) const final;

    bool NodeAtPoint(HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset, HitTestPhase) final;
    ...
    bool MapToVisualRectInAncestorSpaceInternal(const LayoutBoxModelObject* ancestor, TransformState&, VisualRectFlags = kDefaultVisualRectFlags) const final;
...
template <> struct DowncastTraits<LayoutInline> {
