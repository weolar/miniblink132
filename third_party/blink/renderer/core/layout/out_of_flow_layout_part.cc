...
bool CalculateNonOverflowingRangeInOneAxis(LayoutUnit margin_box_start, LayoutUnit margin_box_end, LayoutUnit imcb_inset_start, LayoutUnit imcb_inset_end,
    LayoutUnit position_area_start, LayoutUnit position_area_end, bool has_non_auto_inset_start, bool has_non_auto_inset_end,
    std::optional<LayoutUnit>* out_scroll_min, std::optional<LayoutUnit>* out_scroll_max)
{
    ...
    if (*out_scroll_min && *out_scroll_max && out_scroll_min->value() > out_scroll_max->value()) {
        return false;
    }
    return true;
}
...
public:
    explicit OOFCandidateStyleIterator(const LayoutObject& object, AnchorEvaluatorImpl& anchor_evaluator)
    ...
    const ComputedStyle& ActivateBaseStyleForTryAttempt()
    {
        ...
        if (&base_style != &GetStyle()) {
            element_->GetLayoutObject()->SetStyle(&base_style, LayoutObject::ApplyStyleChanges::kNo);
        }
        return base_style;
    }
    ...
    const ComputedStyle& ActivateStyleForChosenFallback()
    {
        ...
        element_->GetLayoutObject()->SetStyle(&style, LayoutObject::ApplyStyleChanges::kNo);
        return style;
    }
    ...
    bool MoveToNextStyle()
    {
        ...
        for (; *try_fallback_index_ < position_try_fallbacks_->GetFallbacks().size(); ++*try_fallback_index_) {
    ...
    std::optional<const CSSPropertyValueSet*> TrySetFromFallback(const PositionTryFallback& fallback)
    {
        if (!fallback.GetPositionArea().IsNone()) {
            ...
            CSSPropertyValue declaration(CSSPropertyName(CSSPropertyID::kPositionArea), *ComputedStyleUtils::ValueForPositionArea(fallback.GetPositionArea()));
            return ImmutableCSSPropertyValueSet::Create(base::span_from_ref(declaration), kHTMLStandardMode);
    ...
    void MoveToChosenTryFallbackIndex(std::optional<wtf_size_t> index)
    {
        ...
        if (index.has_value()) {
            ...
            const PositionTryFallback& fallback = position_try_fallbacks_->GetFallbacks()[*index];
            try_tactics = fallback.GetTryTactic();
            std::optional<const CSSPropertyValueSet*> opt_try_set = TrySetFromFallback(fallback);
            CHECK(opt_try_set.has_value());
            try_set = opt_try_set.value();
            may_invalidate_last_successful
                = element_->EnsureOutOfFlowData().SetPendingSuccessfulPositionFallback(position_try_fallbacks_, try_set, try_tactics, index);
        } else if (OutOfFlowData* out_of_flow_data = element_->GetOutOfFlowData()) {
            may_invalidate_last_successful = out_of_flow_data->SetPendingSuccessfulPositionFallback(position_try_fallbacks_,
                /* try_set */ nullptr, kNoTryTactics, /* index */ std::nullopt);
        }
        if (may_invalidate_last_successful) {
            element_->GetDocument().GetStyleEngine().MarkLastSuccessfulPositionFallbackDirtyForElement(*element_);
        }
...
private:
    ...
    bool ElementStyleDependsOnAnchor(const Element& element, const ComputedStyle& style)
    {
    ...
    const StyleRulePositionTry* GetPositionTryRule(const ScopedCSSName& scoped_name)
    {
        CHECK(element_);
        return element_->GetDocument().GetStyleEngine().GetPositionTryRule(scoped_name);
    }
    ...
    const ComputedStyle* UpdateStyle(wtf_size_t try_fallback_index)
    {
        ...
        CHECK(position_try_fallbacks_);
        CHECK_LE(try_fallback_index, position_try_fallbacks_->GetFallbacks().size());
        const PositionTryFallback& fallback = position_try_fallbacks_->GetFallbacks()[try_fallback_index];
        std::optional<const CSSPropertyValueSet*> try_set = TrySetFromFallback(fallback);
    ...
    const ComputedStyle* UpdateStyle(const CSSPropertyValueSet* try_set, const TryTacticList& tactic_list)
    {
        CHECK(element_);
        element_->GetDocument().GetStyleEngine().UpdateStyleForOutOfFlow(*element_, try_set, tactic_list, &anchor_evaluator_);
...
const Element* GetPositionAnchorElement(const BlockNode& node, const ComputedStyle& style, const LogicalAnchorQuery* anchor_query)
{
    if (!anchor_query) {
        return nullptr;
    }
    if (const ScopedCSSName* specifier = style.PositionAnchor()) {
        if (const LogicalAnchorReference* reference = anchor_query->AnchorReference(*node.GetLayoutBox(), specifier); reference && reference->layout_object) {
...
const LayoutObject* GetPositionAnchorObject(const BlockNode& node, const ComputedStyle& style, const LogicalAnchorQuery* anchor_query)
{
    if (const Element* element = GetPositionAnchorElement(node, style, anchor_query)) {
        return element->GetLayoutObject();
    }
    return nullptr;
}

gfx::Vector2dF GetAnchorOffset(const BlockNode& node, const ComputedStyle& style, const LogicalAnchorQuery* anchor_query)
{
    if (const LayoutObject* anchor_object = GetPositionAnchorObject(node, style, anchor_query)) {
        if (const AnchorPositionScrollData* data = To<Element>(node.GetDOMNode())->GetAnchorPositionScrollData()) {
...
// step.
void UpdatePositionVisibilityAfterLayout(const OutOfFlowLayoutPart::OffsetInfo& offset_info, const BlockNode& node, const LogicalAnchorQuery* anchor_query)
{
    if (!anchor_query) {
        return;
    }
    ...
    CHECK(layer);
    bool has_no_overflow_visibility = node.Style().HasPositionVisibility(PositionVisibility::kNoOverflow);
    layer->SetInvisibleForPositionVisibility(LayerPositionVisibility::kNoOverflow, has_no_overflow_visibility && offset_info.overflows_containing_block);
    ...
    bool is_anchor_positioned = offset_info.needs_scroll_adjustment_in_x || offset_info.needs_scroll_adjustment_in_y;
    bool has_anchors_visible_visibility = node.Style().HasPositionVisibility(PositionVisibility::kAnchorsVisible);
    ...
    const Element* anchor = anchored ? GetPositionAnchorElement(node, node.Style(), anchor_query) : nullptr;
    if (is_anchor_positioned && has_anchors_visible_visibility && anchor) {
        anchored->EnsureAnchorPositionScrollData().EnsureAnchorPositionVisibilityObserver().MonitorAnchor(anchor);
    } else if (anchored) {
...
// static
std::optional<LogicalSize> OutOfFlowLayoutPart::InitialContainingBlockFixedSize(BlockNode container)
{
    if (!container.GetLayoutBox()->IsLayoutView() || container.GetDocument().Printing())
        return std::nullopt;
    const auto* frame_view = container.GetDocument().View();
    DCHECK(frame_view);
    PhysicalSize size(frame_view->LayoutViewport()->ExcludeScrollbars(frame_view->Size()));
...
OutOfFlowLayoutPart::OutOfFlowLayoutPart(BoxFragmentBuilder* container_builder)
    ...
    , has_block_fragmentation_(InvolvedInBlockFragmentation(*container_builder))
{
    ...
    if (!container_builder->HasOutOfFlowPositionedCandidates() && !container_builder->HasOutOfFlowFragmentainerDescendants()
        && !container_builder->HasMulticolsWithPendingOOFs() && !container_builder->IsRoot()) {
        return;
    }
    ...
    const BoxStrut border_scrollbar = container_builder->Borders() + container_builder->Scrollbar();
    default_containing_block_info_for_absolute_.writing_direction = GetConstraintSpace().GetWritingDirection();
    default_containing_block_info_for_fixed_.writing_direction = GetConstraintSpace().GetWritingDirection();
    default_containing_block_info_for_absolute_.is_scroll_container = container_builder_->Node().IsScrollContainer();
    default_containing_block_info_for_fixed_.is_scroll_container = container_builder_->Node().IsScrollContainer();
    if (container_builder_->HasBlockSize()) {
        default_containing_block_info_for_absolute_.rect.size = ShrinkLogicalSize(container_builder_->Size(), border_scrollbar);
        default_containing_block_info_for_fixed_.rect.size
            = InitialContainingBlockFixedSize(container_builder->Node()).value_or(default_containing_block_info_for_absolute_.rect.size);
    }
    LogicalOffset container_offset = { border_scrollbar.inline_start, border_scrollbar.block_start };
...
void OutOfFlowLayoutPart::Run()
{
    ...
    ClearCollectionScope<HeapVector<LogicalOofPositionedNode>> clear_scope(&candidates);
    container_builder_->SwapOutOfFlowPositionedCandidates(&candidates);

    if (!candidates.empty()) {
        LayoutCandidates(&candidates);
    } else {
        container_builder_->AdjustFixedposContainingBlockForFragmentainerDescendants();
    ...
    for (LayoutInputNode child = node.FirstChild(); child; child = child.NextSibling()) {
        if (!child.IsBlock()) {
            continue;
        }
        BlockNode block_child = To<BlockNode>(child);
        if (!block_child.IsInTopOrViewTransitionLayer() || !block_child.IsOutOfFlowPositioned()) {
            continue;
        }

        // https://drafts.csswg.org/css-position-4/#top-styling
        // The static position for top-layer elements is just 0x0.
        container_builder_->AddOutOfFlowChildCandidate(block_child, LogicalOffset(), LogicalStaticPosition::InlineEdge::kInlineStart,
...
void OutOfFlowLayoutPart::PropagateOOFsFromPageAreas()
{
    ...
    for (wtf_size_t i = 0; i < ChildCount(); i++) {
        ...
        if (fragmentainer.NeedsOOFPositionedInfoPropagation()) {
            container_builder_->PropagateOOFPositionedInfo(fragmentainer, LogicalOffset(), LogicalOffset(), offset_adjustment);
        }
...
void OutOfFlowLayoutPart::HandleFragmentation()
{
    ...
    if (!column_balancing_info_ && (!container_builder_->IsBlockFragmentationContextRoot() || has_block_fragmentation_)) {
        return;
    }

    if (container_builder_->Node().IsPaginatedRoot()) {
        HeapVector<LogicalOofPositionedNode> candidates;
        ClearCollectionScope<HeapVector<LogicalOofPositionedNode>> scope(&candidates);
        container_builder_->SwapOutOfFlowPositionedCandidates(&candidates);
    ...
    DCHECK(!column_balancing_info_ || !column_balancing_info_->out_of_flow_fragmentainer_descendants.empty());

    auto ShouldContinue = [&]() -> bool {
        if (column_balancing_info_)
            return column_balancing_info_->HasOutOfFlowFragmentainerDescendants();
        return container_builder_->HasOutOfFlowFragmentainerDescendants() || container_builder_->HasMulticolsWithPendingOOFs();
    };

    while (ShouldContinue()) {
        HeapVector<LogicalOofNodeForFragmentation> fragmentainer_descendants;
        ClearCollectionScope<HeapVector<LogicalOofNodeForFragmentation>> scope(&fragmentainer_descendants);
        if (column_balancing_info_) {
            column_balancing_info_->SwapOutOfFlowFragmentainerDescendants(&fragmentainer_descendants);
            ...
            if (container_builder_->HasOutOfFlowFragmentainerDescendants()) {
                container_builder_->SwapOutOfFlowFragmentainerDescendants(&fragmentainer_descendants);
                DCHECK(!fragmentainer_descendants.empty());
            }
        }
        if (!fragmentainer_descendants.empty()) {
            LogicalOffset fragmentainer_progression = GetFragmentainerProgression(*container_builder_, GetFragmentainerType());
            LayoutFragmentainerDescendants(&fragmentainer_descendants, fragmentainer_progression);
        }
    }
    if (!column_balancing_info_) {
...
OutOfFlowLayoutPart::ContainingBlockInfo OutOfFlowLayoutPart::ApplyPositionAreaOffsets(
    const PositionAreaOffsets& offsets, const OutOfFlowLayoutPart::ContainingBlockInfo& container_info) const
{
    ContainingBlockInfo adjusted_container_info(container_info);
    PhysicalToLogical converter(container_info.writing_direction, offsets.top.value_or(LayoutUnit()), offsets.right.value_or(LayoutUnit()),
        offsets.bottom.value_or(LayoutUnit()), offsets.left.value_or(LayoutUnit()));

    // Reduce the container size and adjust the offset based on the position-area.
    adjusted_container_info.rect.ContractEdges(converter.BlockStart(), converter.InlineEnd(), converter.BlockEnd(), converter.InlineStart());
    ...
    if (adjusted_container_info.rect.size.inline_size < LayoutUnit()) {
        DCHECK(converter.InlineStart() == LayoutUnit() || converter.InlineEnd() == LayoutUnit()) << "If aligned to both anchor edges, the size should never be "
                                                                                                    "negative.";
        ...
        if (converter.InlineStart() == LayoutUnit()) {
            ...
            adjusted_container_info.rect.offset.inline_offset += adjusted_container_info.rect.size.inline_size;
        }
        adjusted_container_info.rect.size.inline_size = LayoutUnit();
    }
    if (adjusted_container_info.rect.size.block_size < LayoutUnit()) {
        DCHECK(converter.BlockStart() == LayoutUnit() || converter.BlockEnd() == LayoutUnit()) << "If aligned to both anchor edges, the size should never be "
                                                                                                  "negative.";
        ...
        if (converter.BlockStart() == LayoutUnit()) {
            ...
            adjusted_container_info.rect.offset.block_offset += adjusted_container_info.rect.size.block_size;
        }
...
const OutOfFlowLayoutPart::ContainingBlockInfo OutOfFlowLayoutPart::GetContainingBlockInfo(const LogicalOofPositionedNode& candidate)
{
    ...
    auto IsPlacedWithinGridArea = [&](const auto* containing_block) {
        ...
        return !node_style.GridColumnStart().IsAuto() || !node_style.GridColumnEnd().IsAuto() || !node_style.GridRowStart().IsAuto()
            || !node_style.GridRowEnd().IsAuto();
    };

    auto GridAreaContainingBlockInfo = [&](const LayoutGrid& containing_grid, const GridLayoutData& layout_data, const BoxStrut& borders,
        ...
        return { .writing_direction = grid_style.GetWritingDirection(),
            .rect = GridLayoutAlgorithm::ComputeOutOfFlowItemContainingRect(
                containing_grid.CachedPlacementData(), layout_data, grid_style, borders, size, &grid_item) };
    };

    if (candidate.inline_container.container) {
        const auto it = containing_blocks_map_.find(candidate.inline_container.container);
        CHECK(it != containing_blocks_map_.end(), base::NotFatalUntil::M130);
        return it->value;
    }

    if (candidate.is_for_fragmentation) {
        LogicalOofNodeForFragmentation fragmentainer_descendant = To<LogicalOofNodeForFragmentation>(candidate);
        if (fragmentainer_descendant.containing_block.Fragment()) {
            DCHECK(container_builder_->IsBlockFragmentationContextRoot());

            const PhysicalFragment* containing_block_fragment = fragmentainer_descendant.containing_block.Fragment();
            const LayoutObject* containing_block = containing_block_fragment->GetLayoutObject();
            DCHECK(containing_block);

            bool is_placed_within_grid_area = IsPlacedWithinGridArea(containing_block);
            auto it = containing_blocks_map_.find(containing_block);
            if (it != containing_blocks_map_.end() && !is_placed_within_grid_area)
                return it->value;

            const auto writing_direction = containing_block->StyleRef().GetWritingDirection();
            LogicalSize size = containing_block_fragment->Size().ConvertToLogical(writing_direction.GetWritingMode());
            size.block_size = LayoutBoxUtils::TotalBlockSize(*To<LayoutBox>(containing_block));

            // TODO(1079031): This should eventually include scrollbar and border.
            BoxStrut border = To<PhysicalBoxFragment>(containing_block_fragment)->Borders().ConvertToLogical(writing_direction);

            if (is_placed_within_grid_area) {
                return GridAreaContainingBlockInfo(*To<LayoutGrid>(containing_block), *To<LayoutGrid>(containing_block)->LayoutData(), border, size);
            }

            LogicalSize content_size = ShrinkLogicalSize(size, border);
            LogicalOffset container_offset = LogicalOffset(border.inline_start, border.block_start);
            container_offset += fragmentainer_descendant.containing_block.Offset();

            ContainingBlockInfo containing_block_info { writing_direction, containing_block_fragment->IsScrollContainer(),
                LogicalRect(container_offset, content_size), fragmentainer_descendant.containing_block.RelativeOffset(),
                fragmentainer_descendant.containing_block.Offset() };

            return containing_blocks_map_.insert(containing_block, containing_block_info).stored_value->value;
        }
    }

    if (IsPlacedWithinGridArea(container_object)) {
        return GridAreaContainingBlockInfo(*To<LayoutGrid>(container_object), container_builder_->GetGridLayoutData(), container_builder_->Borders(),
            { container_builder_->InlineSize(), container_builder_->FragmentBlockSize() });
    }

    return node_style.GetPosition() == EPosition::kAbsolute ? default_containing_block_info_for_absolute_ : default_containing_block_info_for_fixed_;
}

void OutOfFlowLayoutPart::ComputeInlineContainingBlocks(const HeapVector<LogicalOofPositionedNode>& candidates)
{
    InlineContainingBlockUtils::InlineContainingBlockMap inline_container_fragments;

    for (auto& candidate : candidates) {
        if (candidate.inline_container.container && !inline_container_fragments.Contains(candidate.inline_container.container)) {
            InlineContainingBlockUtils::InlineContainingBlockGeometry inline_geometry = {};
            inline_container_fragments.insert(candidate.inline_container.container.Get(), inline_geometry);
        }
    }

    // Fetch the inline start/end fragment geometry.
    InlineContainingBlockUtils::ComputeInlineContainerGeometry(&inline_container_fragments, container_builder_);

    LogicalSize container_builder_size = container_builder_->Size();
    PhysicalSize container_builder_physical_size = ToPhysicalSize(container_builder_size, GetConstraintSpace().GetWritingMode());
    AddInlineContainingBlockInfo(inline_container_fragments, default_containing_block_info_for_absolute_.writing_direction, container_builder_physical_size);
}

void OutOfFlowLayoutPart::ComputeInlineContainingBlocksForFragmentainer(const HeapVector<LogicalOofNodeForFragmentation>& descendants)
{
    ...
    HeapHashMap<Member<const LayoutBox>, InlineContainingBlockInfo> inline_containg_blocks;
    ...
    for (auto& descendant : descendants) {
        if (descendant.inline_container.container) {
            ...
            const LayoutBox* containing_block = To<LayoutBox>(descendant.containing_block.Fragment()->GetLayoutObject());

            InlineContainingBlockUtils::InlineContainingBlockGeometry inline_geometry = {};
            inline_geometry.relative_offset = descendant.inline_container.relative_offset;
            ...
            if (it != inline_containg_blocks.end()) {
                if (!it->value.map.Contains(descendant.inline_container.container)) {
                    it->value.map.insert(descendant.inline_container.container.Get(), inline_geometry);
                }
                continue;
            }
            InlineContainingBlockUtils::InlineContainingBlockMap inline_containers;
            inline_containers.insert(descendant.inline_container.container.Get(), inline_geometry);
            InlineContainingBlockInfo inline_info { inline_containers, descendant.containing_block.RelativeOffset(), descendant.containing_block.Offset() };
    ...
    for (auto& inline_containg_block : inline_containg_blocks) {
        ...
        LogicalSize size(LayoutBoxUtils::InlineSize(*containing_block), LayoutBoxUtils::TotalBlockSize(*containing_block));
        PhysicalSize container_builder_physical_size = ToPhysicalSize(size, containing_block->StyleRef().GetWritingMode());

        // Fetch the inline start/end fragment geometry.
        InlineContainingBlockUtils::ComputeInlineContainerGeometryForFragmentainer(containing_block, container_builder_physical_size, &inline_info.map);

        AddInlineContainingBlockInfo(inline_info.map, containing_block->StyleRef().GetWritingDirection(), container_builder_physical_size,
            inline_info.relative_offset, inline_info.offset_to_fragmentation_context,
            /* adjust_for_fragmentation */ true);
    }
}

void OutOfFlowLayoutPart::AddInlineContainingBlockInfo(const InlineContainingBlockUtils::InlineContainingBlockMap& inline_container_fragments,
    const WritingDirectionMode container_writing_direction, PhysicalSize container_builder_size, LogicalOffset containing_block_relative_offset,
    LogicalOffset containing_block_offset, bool adjust_for_fragmentation)
{
    ...
    for (const auto& block_info : inline_container_fragments) {
        ...
        DCHECK_EQ(container_writing_direction.GetWritingMode(), inline_writing_direction.GetWritingMode());

        bool is_same_direction = container_writing_direction == inline_writing_direction;

        // Step 1 - determine the start_offset.
        const PhysicalRect& start_rect = block_info.value->start_fragment_union_rect;
        LogicalOffset start_offset = start_rect.offset.ConvertToLogical(container_writing_direction, container_builder_size, start_rect.size);
        ...
        LogicalOffset end_offset = end_rect.offset.ConvertToLogical(container_writing_direction, container_builder_size, end_rect.size);

        // Add in the size of the fragment to get the logical end of the fragment.
        end_offset += end_rect.size.ConvertToLogical(container_writing_direction.GetWritingMode());
        ...
        end_offset.inline_offset = std::max(end_offset.inline_offset, start_offset.inline_offset);
        end_offset.block_offset = std::max(end_offset.block_offset, start_offset.block_offset);

        // Step 3 - determine the logical rectangle.

        // Determine the logical size of the containing block.
        LogicalSize inline_cb_size = { end_offset.inline_offset - start_offset.inline_offset, end_offset.block_offset - start_offset.block_offset };
        ...
        if (adjust_for_fragmentation) {
            ...
            PhysicalSize physical_size = ToPhysicalSize(inline_cb_size, GetConstraintSpace().GetWritingMode());
            start_offset = start_offset.ConvertToPhysical(container_writing_direction, container_builder_size, physical_size)
                               .ConvertToLogical(GetConstraintSpace().GetWritingDirection(), container_builder_size, physical_size);
        }
        ...
        // descendants.
        DCHECK((block_info.value->relative_offset == LogicalOffset() && containing_block_relative_offset == LogicalOffset()
                   && containing_block_offset == LogicalOffset())
            || container_builder_->IsBlockFragmentationContextRoot());
        LogicalOffset container_offset = start_offset - block_info.value->relative_offset;
        LogicalOffset total_relative_offset = containing_block_relative_offset + block_info.value->relative_offset;
        ...
        containing_blocks_map_.insert(block_info.key.Get(),
            ContainingBlockInfo { inline_writing_direction,
                /* is_scroll_container */ false, LogicalRect(container_offset, inline_cb_size), total_relative_offset,
                containing_block_offset - block_info.value->relative_offset });
    }
}

void OutOfFlowLayoutPart::LayoutCandidates(HeapVector<LogicalOofPositionedNode>* candidates)
{
    while (candidates->size() > 0) {
        if (!has_block_fragmentation_ || container_builder_->IsInitialColumnBalancingPass()) {
            ...
            if (IsContainingBlockForCandidate(candidate)) {
                if (has_block_fragmentation_) {
                    ...
                    if (!container_builder_->IsInitialColumnBalancingPass()) {
                        ...
                        container_builder_->AdjustFragmentainerDescendant(fragmentainer_descendant);
                        container_builder_->AdjustFixedposContainingBlockForInnerMulticols();
                        container_builder_->AddOutOfFlowFragmentainerDescendant(fragmentainer_descendant);
                        continue;
                    }
                }
                ...
                PhysicalBoxStrut physical_margins
                    = node_to_layout.offset_info.node_dimensions.margins.ConvertToPhysical(node_info.node.Style().GetWritingDirection());
                BoxStrut margins = physical_margins.ConvertToLogical(container_builder_->GetWritingDirection());
                container_builder_->AddResult(*result, result->OutOfFlowPositionedOffset(), margins,
                    /* relative_offset */ std::nullopt, &candidate.inline_container);
                container_builder_->SetHasOutOfFlowFragmentChild(true);
                if (container_builder_->IsInitialColumnBalancingPass()) {
                    container_builder_->PropagateTallestUnbreakableBlockSize(result->TallestUnbreakableBlockSize());
                }
            } else {
...
void OutOfFlowLayoutPart::HandleMulticolsWithPendingOOFs(BoxFragmentBuilder* container_builder)
{
...
void OutOfFlowLayoutPart::LayoutOOFsInMulticol(const BlockNode& multicol, const MulticolWithPendingOofs<LogicalOffset>* multicol_info)
{
    HeapVector<LogicalOofNodeForFragmentation> oof_nodes_to_layout;
    ClearCollectionScope<HeapVector<LogicalOofNodeForFragmentation>> oof_nodes_scope(&oof_nodes_to_layout);
    HeapVector<MulticolChildInfo> multicol_children;
    ClearCollectionScope<HeapVector<MulticolChildInfo>> multicol_scope(&multicol_children);
    ...
    ConstraintSpace limited_multicol_constraint_space = CreateConstraintSpaceForMulticol(multicol);
    FragmentGeometry limited_fragment_geometry = CalculateInitialFragmentGeometry(limited_multicol_constraint_space, multicol, /* break_token */ nullptr);
    BoxFragmentBuilder limited_multicol_container_builder
        = CreateContainerBuilderForMulticol(multicol, limited_multicol_constraint_space, limited_fragment_geometry);
    ...
    WritingDirectionMode writing_direction = multicol.Style().GetWritingDirection();
    ...
    for (auto& multicol_fragment : multicol.GetLayoutBox()->PhysicalFragments()) {
        const auto* multicol_box_fragment = To<PhysicalBoxFragment>(&multicol_fragment);

        const ComputedStyle& style = multicol_box_fragment->Style();
        const WritingModeConverter converter(writing_direction, multicol_box_fragment->Size());
        ...
        if (column_inline_progression == kIndefiniteSize) {
            ...
            BoxStrut border_padding
                = multicol_box_fragment->Borders().ConvertToLogical(writing_direction) + multicol_box_fragment->Padding().ConvertToLogical(writing_direction);
            LayoutUnit available_inline_size
                = multicol_box_fragment->Size().ConvertToLogical(writing_direction.GetWritingMode()).inline_size - border_padding.InlineSum();
            column_inline_progression = ColumnInlineProgression(available_inline_size, style);
        }

        // Collect the children of the multicol fragments.
        for (auto& child : multicol_box_fragment->GetMutableChildrenForOutOfFlow().Children()) {
            const auto* fragment = child.get();
            LogicalOffset offset = converter.ToLogical(child.Offset(), fragment->Size());
        ...
        if (current_column_index != kNotFound && break_token && break_token->ChildBreakTokens().size()) {
            ...
            const BlockBreakToken* child_token = To<BlockBreakToken>(children[children.size() - 1].Get());
        ...
        limited_multicol_container_builder.SwapOutOfFlowFragmentainerDescendants(&oof_fragmentainer_descendants);
        for (const auto& descendant : oof_fragmentainer_descendants) {
            if (oof_nodes_to_layout.empty() && multicol_info->fixedpos_containing_block.Fragment() && previous_multicol_break_token) {
                ...
                // multicol fragment.
                multicol_offset.block_offset -= previous_multicol_break_token->ConsumedBlockSize();
            }
            ...
            if (!descendant.containing_block.Fragment() || descendant.containing_block.IsInsideColumnSpanner()) {
                continue;
            }
    ...
    DCHECK(!limited_multicol_container_builder.HasOutOfFlowFragmentainerDescendants());
    ...
    wtf_size_t old_fragment_count = limited_multicol_container_builder.Children().size();

    LogicalOffset fragmentainer_progression(column_inline_progression, LayoutUnit());
    ...
    inner_part.outer_container_builder_ = outer_container_builder_ ? outer_container_builder_ : container_builder_;
    inner_part.LayoutFragmentainerDescendants(
        &oof_nodes_to_layout, fragmentainer_progression, multicol_info->fixedpos_containing_block.Fragment(), &multicol_children);

    wtf_size_t new_fragment_count = limited_multicol_container_builder.Children().size();
    ...
    if (old_fragment_count != new_fragment_count) {
        ...
        do {
            ...
            if (&old_result->GetPhysicalFragment() == last_fragment_with_fragmentainer) {
                break;
            }
        ...
        const auto& existing_fragment = To<PhysicalBoxFragment>(old_result->GetPhysicalFragment());
        WritingModeConverter converter(existing_fragment.Style().GetWritingDirection(), existing_fragment.Size());
        ...
        for (wtf_size_t i = old_fragment_count; i < new_fragment_count; i++) {
            const LogicalFragmentLink& child = limited_multicol_container_builder.Children()[i];
            fragment_mutator.AddChildFragmentainer(*To<PhysicalBoxFragment>(child.get()), child.offset);
            additional_column_block_size += converter.ToLogical(child.fragment->Size()).block_size;
        }
    ...
    DCHECK(!limited_multicol_container_builder.HasOutOfFlowPositionedDescendants());
    DCHECK(!limited_multicol_container_builder.HasOutOfFlowFragmentainerDescendants());
    limited_multicol_container_builder.TransferOutOfFlowCandidates(container_builder_, multicol_offset, multicol_info);
    ...
    FragmentBuilder::MulticolCollection multicols_with_pending_oofs;
    limited_multicol_container_builder.SwapMulticolsWithPendingOOFs(&multicols_with_pending_oofs);
    for (auto& descendant : multicols_with_pending_oofs) {
        container_builder_->AddMulticolWithPendingOOFs(BlockNode(descendant.key), descendant.value);
    }
}

void OutOfFlowLayoutPart::LayoutFragmentainerDescendants(HeapVector<LogicalOofNodeForFragmentation>* descendants, LogicalOffset fragmentainer_progression,
    bool outer_context_has_fixedpos_container, HeapVector<MulticolChildInfo>* multicol_children)
{
    ...
    LogicalAnchorQueryMap stitched_anchor_queries(
        *builder_for_anchor_query->Node().GetLayoutBox(), builder_for_anchor_query->Children(), builder_for_anchor_query->GetWritingDirection());

    const bool may_have_anchors_on_oof
        = std::any_of(descendants->begin(), descendants->end(), [](const LogicalOofPositionedNode& node) { return node.box->MayHaveAnchorQuery(); });

    HeapVector<HeapVector<NodeToLayout>> descendants_to_layout;
    ClearCollectionScope<HeapVector<HeapVector<NodeToLayout>>> descendants_to_layout_scope(&descendants_to_layout);
    ...
    ClearCollectionScope<HeapVector<NodeToLayout>> repeated_fixedpos_descendants_scope(&repeated_fixedpos_descendants);
    ...
    while (descendants->size() > 0) {
        ...
        base::span<LogicalOofNodeForFragmentation> descendants_span = base::make_span(*descendants);
        for (;;) {
            ...
            for (auto& descendant : descendants_span) {
                if (GetFragmentainerType() == kFragmentColumn) {
                    auto* containing_block = To<LayoutBox>(descendant.containing_block.Fragment()->GetLayoutObject());
                    DCHECK(containing_block);
                ...
                if (!stitched_anchor_queries.IsEmpty() || may_have_anchors_on_oof) {
                    const LayoutObject* css_containing_block = descendant.box->Container();
                    ...
                    if (css_containing_block != last_css_containing_block) {
                        ...
                        if (last_css_containing_block && (last_css_containing_block->MayHaveAnchorQuery() || may_have_anchors_on_oof)) {
                            has_new_descendants_span = true;
                            descendants_span = descendants_span.subspan(&descendant - descendants_span.data());
                            break;
                        }
                ...
                NodeToLayout node_to_layout = { node_info, CalculateOffset(node_info, &stitched_anchor_queries) };
                node_to_layout.containing_block_fragment = descendant.containing_block.Fragment();
                ...
                ComputeStartFragmentIndexAndRelativeOffset(node_info.default_writing_direction.GetWritingMode(), *node_to_layout.offset_info.block_estimate,
                    node_info.containing_block.ClippedContainerBlockOffset(), &start_index, &node_to_layout.offset_info.offset);
            ...
            ClearCollectionScope<HeapVector<NodeToLayout>> fragmented_descendants_scope(&fragmented_descendants);
            ...
            for (wtf_size_t index = 0; index < descendants_to_layout.size(); index++) {
                ...
                if (!fragment || fragment->IsFragmentainerBox()) {
                    HeapVector<NodeToLayout>& pending_descendants = descendants_to_layout[index];

                    if (!repeated_fixedpos_descendants.empty() && index == previous_repeaded_fixedpos_resume_idx) {
                    ...
                    bool has_oofs_in_later_fragmentainer = index + 1 < descendants_to_layout.size();
                    last_fragmentainer_has_break_inside = false;
                    LayoutOOFsInFragmentainer(pending_descendants, index, fragmentainer_progression, has_oofs_in_later_fragmentainer, &monolithic_overflow,
                    ...
                    if (!column_balancing_info_) {
                        fragment = &GetChildFragment(index);
                        fragmentainer_consumed_block_size_ += fragment->Size().ConvertToLogical(container_builder_->Style().GetWritingMode()).block_size;
                    }
                }
                ...
                if (index == descendants_to_layout.size() - 1
                    && (last_fragmentainer_has_break_inside || monolithic_overflow > LayoutUnit()
            ...
            if (!fragmented_descendants.empty()) {
                ...
                DCHECK(previous_repeaded_fixedpos_resume_idx == WTF::kNotFound || previous_repeaded_fixedpos_resume_idx <= descendants_to_layout.size());
            ...
            if (may_have_anchors_on_oof) {
                stitched_anchor_queries.SetChildren(builder_for_anchor_query->Children());
            }
        }
...
AnchorEvaluatorImpl OutOfFlowLayoutPart::CreateAnchorEvaluator(
    const ContainingBlockInfo& container_info, const BlockNode& candidate, const LogicalAnchorQueryMap* anchor_queries) const
{
    const LayoutObject* implicit_anchor = nullptr;
    const LayoutBox& candidate_layout_box = *candidate.GetLayoutBox();
    if (const Element* element = DynamicTo<Element>(candidate_layout_box.GetNode())) {
        if (const Element* implicit_anchor_element = element->ImplicitAnchorElement()) {
            implicit_anchor = implicit_anchor_element->GetLayoutObject();
        }
    }

    LogicalSize container_content_size = container_info.rect.size;
    PhysicalSize container_physical_content_size = ToPhysicalSize(container_content_size, GetConstraintSpace().GetWritingMode());
    WritingDirectionMode self_writing_direction = candidate.Style().GetWritingDirection();
    const WritingModeConverter container_converter(container_info.writing_direction, container_physical_content_size);
    if (anchor_queries) {
        ...
        return AnchorEvaluatorImpl(candidate_layout_box, *anchor_queries, implicit_anchor, *css_containing_block, container_converter, self_writing_direction,
            container_converter.ToPhysical(container_info.rect).offset, container_physical_content_size);
    }
    if (const LogicalAnchorQuery* anchor_query = container_builder_->AnchorQuery()) {
        // Otherwise the |container_builder_| is the containing block.
        return AnchorEvaluatorImpl(candidate_layout_box, *anchor_query, implicit_anchor, container_converter, self_writing_direction,
            container_converter.ToPhysical(container_info.rect).offset, container_physical_content_size);
    }
    return AnchorEvaluatorImpl();
}

OutOfFlowLayoutPart::NodeInfo OutOfFlowLayoutPart::SetupNodeInfo(const LogicalOofPositionedNode& oof_node)
{
    BlockNode node = oof_node.Node();
    const PhysicalFragment* containing_block_fragment
        = oof_node.is_for_fragmentation ? To<LogicalOofNodeForFragmentation>(oof_node).containing_block.Fragment() : nullptr;

#if DCHECK_IS_ON()
    const LayoutObject* container = containing_block_fragment ? containing_block_fragment->GetLayoutObject() : container_builder_->GetLayoutObject();
    ...
    if (container) {
        ...
        DCHECK(container == node.GetLayoutBox()->ContainingBlock() || node.GetLayoutBox()->ContainingBlock()->IsTable());
    } else {
        ...
        DCHECK_EQ(containing_block_fragment->GetBoxType(), PhysicalFragment::kPageArea);
        DCHECK_EQ(node.GetLayoutBox()->ContainingBlock(), node.GetLayoutBox()->View());
    }
#endif

    const ContainingBlockInfo base_container_info = GetContainingBlockInfo(oof_node);
    ...
    if (containing_block_fragment) {
        containing_block = To<LogicalOofNodeForFragmentation>(oof_node).containing_block;
        fixedpos_containing_block = To<LogicalOofNodeForFragmentation>(oof_node).fixedpos_containing_block;
        fixedpos_inline_container = To<LogicalOofNodeForFragmentation>(oof_node).fixedpos_inline_container;
    }

    return NodeInfo(node, oof_node.static_position, base_container_info, GetConstraintSpace().GetWritingDirection(),
        /* is_fragmentainer_descendant */ containing_block_fragment, containing_block, fixedpos_containing_block, fixedpos_inline_container,
...
const LayoutResult* OutOfFlowLayoutPart::LayoutOOFNode(
    NodeToLayout& oof_node_to_layout, const ConstraintSpace* fragmentainer_constraint_space, bool is_last_fragmentainer_so_far)
{
    ...
    const LayoutResult* layout_result = Layout(oof_node_to_layout, fragmentainer_constraint_space, is_last_fragmentainer_so_far);
    ...
    if (node_info.node.GetLayoutBox()->IntrinsicLogicalWidthsDirty() && offset_info.inline_size_depends_on_min_max_sizes) {
        WritingDirectionMode writing_mode_direction = node_info.node.Style().GetWritingDirection();
        bool freeze_horizontal = false, freeze_vertical = false;
        BoxStrut scrollbars_after = ComputeScrollbarsForNonAnonymous(node_info.node);
        bool ignore_first_inline_freeze = scrollbars_after.InlineSum() && scrollbars_after.BlockSum();
        ...
        do {
            ...
            AddScrollbarFreeze(scrollbars_before, scrollbars_after, writing_mode_direction, &freeze_horizontal, &freeze_vertical);
            ...
            PaintLayerScrollableArea::FreezeScrollbarsRootScope freezer(*node_info.node.GetLayoutBox(), freeze_horizontal, freeze_vertical);
            ...
            layout_result = Layout(oof_node_to_layout, fragmentainer_constraint_space, is_last_fragmentainer_so_far);

            scrollbars_after = ComputeScrollbarsForNonAnonymous(node_info.node);
            DCHECK(!freeze_horizontal || !freeze_vertical || scrollbars_after == scrollbars_before);
        } while (scrollbars_after != scrollbars_before);
    }

    auto& state = oof_node_to_layout.node_info.node.GetLayoutBox()->GetDocument().GetDisplayLockDocumentState();

    if (state.DisplayLockCount() > state.DisplayLockBlockingAllActivationCount()) {
        if (auto* box = oof_node_to_layout.node_info.node.GetLayoutBox()) {
            box->NotifyContainingDisplayLocksForAnchorPositioning(past_display_lock_elements, offset_info.display_locks_affected_by_anchors);
        }
    }
...
EPositionTryOrder ToLogicalPositionTryOrder(EPositionTryOrder position_try_order, WritingDirectionMode writing_direction)
{
    ...
    case EPositionTryOrder::kMostWidth:
        return writing_direction.IsHorizontal() ? EPositionTryOrder::kMostInlineSize : EPositionTryOrder::kMostBlockSize;
    case EPositionTryOrder::kMostHeight:
        return writing_direction.IsHorizontal() ? EPositionTryOrder::kMostBlockSize : EPositionTryOrder::kMostInlineSize;
    }
}
...
void SortNonOverflowingCandidates(
    EPositionTryOrder position_try_order, WritingDirectionMode writing_direction, HeapVector<NonOverflowingCandidate, kMaxTryAttempts>& candidates)
{
    EPositionTryOrder logical_position_try_order = ToLogicalPositionTryOrder(position_try_order, writing_direction);
    ...
    std::stable_sort(candidates.begin(), candidates.end(), [logical_position_try_order](const NonOverflowingCandidate& a, const NonOverflowingCandidate& b) {
        switch (logical_position_try_order) {
        case EPositionTryOrder::kMostBlockSize:
            return a.offset_info.imcb_for_position_order->BlockSize() > b.offset_info.imcb_for_position_order->BlockSize();
        case EPositionTryOrder::kMostInlineSize:
            return a.offset_info.imcb_for_position_order->InlineSize() > b.offset_info.imcb_for_position_order->InlineSize();
        case EPositionTryOrder::kNormal:
...
OutOfFlowLayoutPart::OffsetInfo OutOfFlowLayoutPart::CalculateOffset(const NodeInfo& node_info, const LogicalAnchorQueryMap* anchor_queries)
{
    ...
    AnchorEvaluatorImpl anchor_evaluator = CreateAnchorEvaluator(node_info.base_container_info, node_info.node, anchor_queries);

    OOFCandidateStyleIterator iter(*node_info.node.GetLayoutBox(), anchor_evaluator);
    ...
    bool has_no_overflow_visibility = node_info.node.Style().HasPositionVisibility(PositionVisibility::kNoOverflow);
    ...
    bool try_fit_available_space = has_try_fallbacks || has_no_overflow_visibility;
    ...
    HeapVector<NonOverflowingCandidate, kMaxTryAttempts> non_overflowing_candidates;
    do {
        ...
        std::optional<OffsetInfo> offset_info = TryCalculateOffset(node_info, style, anchor_evaluator, try_fit_available_space, &non_overflowing_range);
        ...
        if (offset_info) {
            if (try_fit_available_space) {
                ...
                if (!non_overflowing_range.Contains(GetAnchorOffset(node_info.node, style, anchor_evaluator.AnchorQuery()))) {
                    continue;
                }
            }
            non_overflowing_candidates.push_back(NonOverflowingCandidate { iter.TryFallbackIndex(), *offset_info });
        }
    } while ((non_overflowing_candidates.empty() || position_try_order != EPositionTryOrder::kNormal) && --attempts_left != 0 && has_try_fallbacks
        && iter.MoveToNextStyle());

    // https://drafts.csswg.org/css-anchor-position-1/#position-try-order-property
    SortNonOverflowingCandidates(position_try_order, node_info.base_container_info.writing_direction, non_overflowing_candidates);

    std::optional<OffsetInfo> offset_info = non_overflowing_candidates.empty() ? std::optional<OffsetInfo>() : non_overflowing_candidates.front().offset_info;
    ...
    if (try_fit_available_space) {
        ...
        if (non_overflowing_candidates.empty()) {
            ...
            iter.MoveToChosenTryFallbackIndex(non_overflowing_candidates.front().try_fallback_index);
        }
    ...
    if (try_fit_available_space) {
        offset_info->non_overflowing_scroll_ranges = std::move(non_overflowing_scroll_ranges);
    } else {
    ...
    offset_info->display_locks_affected_by_anchors = anchor_evaluator.GetDisplayLocksAffectedByAnchors();

    return *offset_info;
}

std::optional<OutOfFlowLayoutPart::OffsetInfo> OutOfFlowLayoutPart::TryCalculateOffset(const NodeInfo& node_info, const ComputedStyle& candidate_style,
    AnchorEvaluatorImpl& anchor_evaluator, bool try_fit_available_space, NonOverflowingScrollRange* out_non_overflowing_range)
{
    ...
    DCHECK_EQ(node_info.node.Style().GetWritingDirection(), candidate_style.GetWritingDirection());
    DCHECK(base::ValuesEquivalent(node_info.node.Style().PositionAnchor(), candidate_style.PositionAnchor()));
    ...
    const ContainingBlockInfo container_info = ([&]() -> ContainingBlockInfo {
        ...
        if (const std::optional<PositionAreaOffsets> offsets = candidate_style.PositionAreaOffsets()) {
            container_info = ApplyPositionAreaOffsets(offsets.value(), container_info);
        }
        return container_info;
    })();

    const WritingDirectionMode candidate_writing_direction = candidate_style.GetWritingDirection();
    ...
    const PhysicalSize container_physical_content_size = ToPhysicalSize(container_rect.size, node_info.default_writing_direction.GetWritingMode());
    ...
    const BoxStrut container_insets = ([&]() -> BoxStrut {
        ...
        const BoxStrut insets(container_rect.offset.inline_offset - base_rect.offset.inline_offset,
            base_rect.InlineEndOffset() - container_rect.InlineEndOffset(), container_rect.offset.block_offset - base_rect.offset.block_offset,
            base_rect.BlockEndOffset() - container_rect.BlockEndOffset());

        // Convert into the candidate writing-direction.
        return insets.ConvertToPhysical(node_info.default_writing_direction).ConvertToLogical(candidate_writing_direction);
    })();

    // Create a constraint space to resolve border/padding/insets.
    const ConstraintSpace space = ([&]() -> ConstraintSpace {
        ConstraintSpaceBuilder builder(GetConstraintSpace(), candidate_writing_direction,
            /* is_new_fc */ true);
        ...
        if (container_builder_->IsInitialColumnBalancingPass()) {
            ...
            SetupSpaceBuilderForFragmentation(GetConstraintSpace(), node_info.node,
                /*fragmentainer_offset_delta=*/LayoutUnit(), GetConstraintSpace().FragmentainerBlockSize(),
    ...
    const LogicalAlignment alignment
        = ComputeAlignment(candidate_style, container_info.is_scroll_container, container_writing_direction, candidate_writing_direction);

    const LogicalOofInsets insets = ComputeOutOfFlowInsets(candidate_style, space.AvailableSize(), alignment, candidate_writing_direction);
    ...
    static_position = static_position.ConvertToPhysical({ node_info.default_writing_direction, container_physical_content_size })
                          .ConvertToLogical({ candidate_writing_direction, container_physical_content_size });

    const InsetModifiedContainingBlock imcb = ComputeInsetModifiedContainingBlock(
        node_info.node, space.AvailableSize(), alignment, insets, static_position, container_writing_direction, candidate_writing_direction);

    {
        ...
        if (alignment.inline_alignment.GetPosition() != ItemPosition::kNormal) {
            if (insets.inline_start && insets.inline_end) {
                UseCounter::Count(document, WebFeature::kOutOfFlowJustifySelfBothInsets);
            } else if (insets.inline_start || insets.inline_end) {
                UseCounter::Count(document, WebFeature::kOutOfFlowJustifySelfSingleInset);
            } else {
    ...
    const BoxStrut border_padding = ComputeBorders(space, node_info.node) + ComputePadding(space, candidate_style);
    ...
    if (node_info.node.IsReplaced()) {
        ...
        ConstraintSpaceBuilder builder(candidate_style.GetWritingMode(), candidate_style.GetWritingDirection(),
            /* is_new_fc */ true);
        builder.SetAvailableSize(imcb.Size());
        builder.SetPercentageResolutionSize(space.PercentageResolutionSize());
        builder.SetReplacedPercentageResolutionSize(space.PercentageResolutionSize());

        const bool is_parallel = IsParallelWritingMode(container_writing_direction.GetWritingMode(), candidate_writing_direction.GetWritingMode());
        const ItemPosition inline_position = (is_parallel ? candidate_style.JustifySelf() : candidate_style.AlignSelf()).GetPosition();
        const bool is_inline_stretch = !imcb.has_auto_inline_inset && inline_position == ItemPosition::kStretch;
        if (is_inline_stretch) {
            builder.SetInlineAutoBehavior(AutoSizeBehavior::kStretchExplicit);
        }
        const ItemPosition block_position = (is_parallel ? candidate_style.AlignSelf() : candidate_style.JustifySelf()).GetPosition();
        const bool is_block_stretch = !imcb.has_auto_block_inset && block_position == ItemPosition::kStretch;
        if (is_block_stretch) {
            builder.SetBlockAutoBehavior(AutoSizeBehavior::kStretchExplicit);
        }

        replaced_size = ComputeReplacedSize(node_info.node, builder.ToConstraintSpace(), border_padding, ReplacedSizeMode::kNormal);
    }

    const LogicalAnchorCenterPosition anchor_center_position
        = ComputeAnchorCenterPosition(candidate_style, alignment, candidate_writing_direction, space.AvailableSize());

    OffsetInfo offset_info;
    LogicalOofDimensions& node_dimensions = offset_info.node_dimensions;
    offset_info.inline_size_depends_on_min_max_sizes = ComputeOofInlineDimensions(node_info.node, candidate_style, space, imcb, anchor_center_position,
        alignment, border_padding, replaced_size, container_insets, container_writing_direction, &node_dimensions);

    PhysicalToLogicalGetter has_non_auto_inset(candidate_writing_direction, candidate_style, &ComputedStyle::IsTopInsetNonAuto,
        &ComputedStyle::IsRightInsetNonAuto, &ComputedStyle::IsBottomInsetNonAuto, &ComputedStyle::IsLeftInsetNonAuto);
    ...
    if (try_fit_available_space) {
        imcb_for_position_fallback = ComputeIMCBForPositionFallback(
            space.AvailableSize(), alignment, insets, static_position, candidate_style, container_writing_direction, candidate_writing_direction);
        offset_info.imcb_for_position_order = imcb_for_position_fallback;
        if (!CalculateNonOverflowingRangeInOneAxis(node_dimensions.MarginBoxInlineStart(), node_dimensions.MarginBoxInlineEnd(),
                imcb_for_position_fallback->inline_start, imcb_for_position_fallback->InlineEndOffset(), container_insets.inline_start,
                container_insets.inline_end, has_non_auto_inset.InlineStart(), has_non_auto_inset.InlineEnd(), &inline_scroll_min, &inline_scroll_max)) {
            return std::nullopt;
        }
    }
    ...
    if (node_dimensions.size.block_size == kIndefiniteSize) {
        offset_info.initial_layout_result = ComputeOofBlockDimensions(node_info.node, candidate_style, space, imcb, anchor_center_position, alignment,
            border_padding, replaced_size, container_insets, container_writing_direction, &node_dimensions);
    }
    ...
    if (try_fit_available_space) {
        if (!CalculateNonOverflowingRangeInOneAxis(node_dimensions.MarginBoxBlockStart(), node_dimensions.MarginBoxBlockEnd(),
                imcb_for_position_fallback->block_start, imcb_for_position_fallback->BlockEndOffset(), container_insets.block_start, container_insets.block_end,
                has_non_auto_inset.BlockStart(), has_non_auto_inset.BlockEnd(), &block_scroll_min, &block_scroll_max)) {
            return std::nullopt;
        }
    }

    offset_info.block_estimate = node_dimensions.size.block_size;
    offset_info.container_content_size = container_physical_content_size.ConvertToLogical(candidate_writing_direction.GetWritingMode());

    // Calculate the offsets.
    const BoxStrut inset = node_dimensions.inset.ConvertToPhysical(candidate_writing_direction).ConvertToLogical(node_info.default_writing_direction);
    ...
    insets_to_store.inline_start = insets.inline_start.value_or(used_insets.inline_start);
    insets_to_store.inline_end = insets.inline_end.value_or(used_insets.inline_end);
    insets_to_store.block_start = insets.block_start.value_or(used_insets.block_start);
    insets_to_store.block_end = insets.block_end.value_or(used_insets.block_end);
    offset_info.insets_for_get_computed_style
        = insets_to_store.ConvertToPhysical(candidate_writing_direction).ConvertToLogical(node_info.default_writing_direction);

    if (try_fit_available_space) {
        out_non_overflowing_range->containing_block_range
            = LogicalScrollRange { inline_scroll_min, inline_scroll_max, block_scroll_min, block_scroll_max }.ToPhysical(candidate_writing_direction);
        out_non_overflowing_range->anchor_object = GetPositionAnchorObject(node_info.node, candidate_style, anchor_evaluator.AnchorQuery());
    }
    ...
    offset_info.needs_scroll_adjustment_in_x = anchor_center_x || anchor_evaluator.NeedsScrollAdjustmentInX();
    offset_info.needs_scroll_adjustment_in_y = anchor_center_y || anchor_evaluator.NeedsScrollAdjustmentInY();

    return offset_info;
}

const LayoutResult* OutOfFlowLayoutPart::Layout(
    const NodeToLayout& oof_node_to_layout, const ConstraintSpace* fragmentainer_constraint_space, bool is_last_fragmentainer_so_far)
{
    ...
    if (fragmentainer_constraint_space || GetConstraintSpace().IsInsideRepeatableContent()) {
    ...
    if (!layout_result) {
        layout_result = GenerateFragment(oof_node_to_layout, fragmentainer_constraint_space, is_last_fragmentainer_so_far);
    }

    DCHECK_EQ(layout_result->Status(), LayoutResult::kSuccess);

    layout_result->GetMutableForOutOfFlow().SetOutOfFlowInsetsForGetComputedStyle(offset_info.insets_for_get_computed_style);

    layout_result->GetMutableForOutOfFlow().SetOutOfFlowPositionedOffset(offset_info.offset);

    layout_result->GetMutableForOutOfFlow().SetNeedsScrollAdjustment(offset_info.needs_scroll_adjustment_in_x, offset_info.needs_scroll_adjustment_in_y);

    layout_result->GetMutableForOutOfFlow().SetNonOverflowingScrollRanges(offset_info.non_overflowing_scroll_ranges);

    layout_result->GetMutableForOutOfFlow().SetAccessibilityAnchor(offset_info.accessibility_anchor);

    layout_result->GetMutableForOutOfFlow().SetDisplayLocksAffectedByAnchors(offset_info.display_locks_affected_by_anchors);

    const BlockNode& node = oof_node_to_layout.node_info.node;

    UpdatePositionVisibilityAfterLayout(offset_info, node, container_builder_->AnchorQuery());
...
bool OutOfFlowLayoutPart::IsContainingBlockForCandidate(const LogicalOofPositionedNode& candidate)
{
    ...
    if (candidate.inline_container.container) {
        DCHECK(candidate.inline_container.container->CanContainOutOfFlowPositionedElement(position));
        return container_builder_->GetLayoutObject() == candidate.box->ContainingBlock();
    }
    return (is_absolute_container_ && position == EPosition::kAbsolute) || (is_fixed_container_ && position == EPosition::kFixed);
}
    ...
    const NodeToLayout& oof_node_to_layout, const ConstraintSpace* fragmentainer_constraint_space, bool is_last_fragmentainer_so_far)
{
    ...
    LayoutUnit block_size = offset_info.block_estimate.value_or(offset_info.container_content_size.block_size);
    ...
    // builder expects.
    PhysicalSize physical_size = ToPhysicalSize(logical_size, style.GetWritingMode());
    LogicalSize available_size = physical_size.ConvertToLogical(GetConstraintSpace().GetWritingMode());
    bool is_repeatable = false;

    ConstraintSpaceBuilder builder(GetConstraintSpace(), style.GetWritingDirection(),
        /* is_new_fc */ true);
    ...
    if (offset_info.block_estimate.has_value() && (!fragmentainer_constraint_space || !offset_info.initial_layout_result)) {
        builder.SetIsFixedBlockSize(true);
    }

    if (fragmentainer_constraint_space) {
        if (container_builder_->Node().IsPaginatedRoot() && style.GetPosition() == EPosition::kFixed && !oof_node_to_layout.containing_block_fragment) {
            ...
            SetupSpaceBuilderForFragmentation(*fragmentainer_constraint_space, node, fragmentainer_constraint_space->FragmentainerOffset() + block_offset,
                fragmentainer_constraint_space->FragmentainerBlockSize(), node_info.requires_content_before_breaking, &builder);
        ...
        SetupSpaceBuilderForFragmentation(GetConstraintSpace(), node, GetConstraintSpace().FragmentainerOffset() + block_offset,
...
void OutOfFlowLayoutPart::LayoutOOFsInFragmentainer(HeapVector<NodeToLayout>& pending_descendants, wtf_size_t index, LogicalOffset fragmentainer_progression,
    bool has_oofs_in_later_fragmentainer, LayoutUnit* monolithic_overflow, bool* has_actual_break_inside, HeapVector<NodeToLayout>* fragmented_descendants)
{
    ...
    ClearCollectionScope<HeapVector<NodeToLayout>> descendants_continued_scope(&descendants_continued);
    ...
    if (pending_descendants.empty() && descendants_continued.empty() && *monolithic_overflow <= LayoutUnit() && !is_new_fragment
    ...
    while (last_fragmentainer_index >= num_children || !GetChildFragment(last_fragmentainer_index).IsFragmentainerBox()) {
        DCHECK_GT(num_children, 0u);
        last_fragmentainer_index--;
    }

    const LogicalFragmentLink& container_link = FragmentationContextChildren()[last_fragmentainer_index];
    const BlockNode& node = container_builder_->Node();
    ...
    if (is_new_fragment) {
        ...
        const PhysicalBoxFragment& previous_fragmentainer = GetChildFragment(last_fragmentainer_index);
        const PhysicalBoxFragment* new_fragmentainer;
        if (node.IsPaginatedRoot()) {
            bool needs_total_page_count;
            new_fragmentainer
                = &PaginatedRootLayoutAlgorithm::CreateEmptyPage(node, GetConstraintSpace(), index, previous_fragmentainer, &needs_total_page_count);
            needs_total_page_count_ |= needs_total_page_count;
            additional_pages_were_added_ = true;
        } else {
            new_fragmentainer = &ColumnLayoutAlgorithm::CreateEmptyColumn(node, GetConstraintSpace(), previous_fragmentainer);
        }
    ...
    FragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(space, node, /* break_token */ nullptr);
    LayoutAlgorithmParams params(node, fragment_geometry, space, PreviousFragmentainerBreakToken(index));
    ...
    for (auto& descendant : descendants_continued) {
        AddOOFToFragmentainer(
            descendant, &space, fragmentainer_offset, index, is_last_fragmentainer_so_far, has_actual_break_inside, &algorithm, fragmented_descendants);
    }
    ...
    const auto& new_fragmentainer = To<PhysicalBoxFragment>(fragmentainer_result->GetPhysicalFragment());
...
void OutOfFlowLayoutPart::AddOOFToFragmentainer(NodeToLayout& descendant, const ConstraintSpace* fragmentainer_space, LogicalOffset fragmentainer_offset,
    wtf_size_t index, bool is_last_fragmentainer_so_far, bool* has_actual_break_inside, SimplifiedOofLayoutAlgorithm* algorithm,
    HeapVector<NodeToLayout>* fragmented_descendants)
{
    const LayoutResult* result = LayoutOOFNode(descendant, fragmentainer_space, is_last_fragmentainer_so_far);
    ...
    LogicalOffset relative_offset = descendant.node_info.base_container_info.relative_offset;
    ...
    result->GetMutableForOutOfFlow().SetOutOfFlowPositionedOffset(adjusted_offset);

    LogicalOffset additional_fixedpos_offset;
    if (descendant.node_info.fixedpos_containing_block.Fragment()) {
        additional_fixedpos_offset = descendant.offset_info.original_offset - descendant.node_info.fixedpos_containing_block.Offset();
        ...
        if (descendant.break_token) {
            additional_fixedpos_offset.block_offset += descendant.break_token->ConsumedBlockSize();
        }
        ...
        for (wtf_size_t i = std::min(index, multicol_children_->size()); i > 0u; i--) {
            MulticolChildInfo& column_info = (*multicol_children_)[i - 1];
            if (column_info.parent_break_token) {
                additional_fixedpos_offset.block_offset += column_info.parent_break_token->ConsumedBlockSize();
                break;
            }
        }
    }
    ...
    if (break_token) {
        ...
        if (!break_token->IsRepeated()) {
            ...
            // that.
            fragmented_descendant.offset_info.offset.block_offset = break_token->MonolithicOverflow();
    ...
    if (column_balancing_info_) {
        LayoutUnit space_shortage
            = CalculateSpaceShortage(*fragmentainer_space, result, oof_offset.block_offset, fragmentainer_space->FragmentainerBlockSize());
    ...
    container_builder_->PropagateChildAnchors(physical_fragment, oof_offset + relative_offset + offset_adjustment);
    container_builder_->PropagateStickyDescendants(physical_fragment);
    LayoutUnit containing_block_adjustment = container_builder_->BlockOffsetAdjustmentForFragmentainer(fragmentainer_consumed_block_size_);
    if (result->GetPhysicalFragment().NeedsOOFPositionedInfoPropagation()) {
        container_builder_->PropagateOOFPositionedInfo(result->GetPhysicalFragment(), oof_offset, relative_offset, offset_adjustment,
            /* inline_container */ nullptr, containing_block_adjustment, &descendant.node_info.containing_block,
            &descendant.node_info.fixedpos_containing_block, &descendant.node_info.fixedpos_inline_container, additional_fixedpos_offset);
    }
    ...
    // added to.
    if (!descendant.break_token) {
        const auto* container = To<PhysicalBoxFragment>(descendant.containing_block_fragment.Get());
        ...
        LogicalOffset legacy_offset = descendant.offset_info.original_offset - descendant.node_info.base_container_info.offset_to_border_box;
        descendant.node_info.node.CopyChildFragmentPosition(physical_fragment,
            legacy_offset.ConvertToPhysical(container->Style().GetWritingDirection(), container->Size(), physical_fragment.Size()), *container,
            /* previous_container_break_token */ nullptr);
    }
}

ConstraintSpace OutOfFlowLayoutPart::GetFragmentainerConstraintSpace(wtf_size_t index)
{
    DCHECK_LT(index, ChildCount());
    const PhysicalBoxFragment& fragment = GetChildFragment(index);
    DCHECK(fragment.IsFragmentainerBox());
    const WritingMode container_writing_mode = container_builder_->Style().GetWritingMode();
    LogicalSize fragmentainer_size = fragment.Size().ConvertToLogical(container_writing_mode);
    LogicalSize percentage_resolution_size = LogicalSize(fragmentainer_size.inline_size, container_builder_->ChildAvailableSize().block_size);
    ...
    return CreateConstraintSpaceForFragmentainer(
        GetConstraintSpace(), GetFragmentainerType(), fragmentainer_size, percentage_resolution_size, /* balance_columns */ false, min_break_appeal);
}
...
void OutOfFlowLayoutPart::ComputeStartFragmentIndexAndRelativeOffset(WritingMode default_writing_mode, LayoutUnit block_estimate,
    std::optional<LayoutUnit> clipped_container_block_offset, wtf_size_t* start_index, LogicalOffset* offset) const
{
    ...
    if (clipped_container_block_offset && container_builder_->Node().IsPaginatedRoot()) {
        ...
        // fragments.
        target_block_offset = std::max(target_block_offset, *clipped_container_block_offset);
    }
    ...
    for (; child_index < ChildCount(); child_index++) {
        ...
        if (child_fragment.IsFragmentainerBox()) {
            fragmentainer_block_size = child_fragment.Size().ConvertToLogical(default_writing_mode).block_size;
            fragmentainer_block_size = ClampedToValidFragmentainerCapacity(fragmentainer_block_size);
            ...
            if (target_block_offset < current_max_block_size || (target_block_offset == current_max_block_size && block_estimate == 0)) {
    ...
    wtf_size_t additional_fragment_count = int(floorf(remaining_block_offset / fragmentainer_block_size));
    *start_index = child_index + additional_fragment_count;
    offset->block_offset = remaining_block_offset - additional_fragment_count * fragmentainer_block_size;
}

void OutOfFlowLayoutPart::SaveStaticPositionOnPaintLayer(LayoutBox* layout_box, const LogicalStaticPosition& position) const
{
    const LayoutObject* parent = GetLayoutObjectForParentNode<const LayoutObject*>(layout_box);
    const LayoutObject* container = container_builder_->GetLayoutObject();
    if (parent == container || (parent->IsLayoutInline() && parent->ContainingBlock() == container)) {
        DCHECK(layout_box->Layer());
        layout_box->Layer()->SetStaticPositionFromNG(ToStaticPositionForLegacy(position));
    }
}

LogicalStaticPosition OutOfFlowLayoutPart::ToStaticPositionForLegacy(LogicalStaticPosition position) const
{
...
const PhysicalBoxFragment& OutOfFlowLayoutPart::GetChildFragment(wtf_size_t index) const
{
...
const BlockBreakToken* OutOfFlowLayoutPart::PreviousFragmentainerBreakToken(wtf_size_t index) const
{
...
void OutOfFlowLayoutPart::ColumnBalancingInfo::PropagateSpaceShortage(LayoutUnit space_shortage)
{
