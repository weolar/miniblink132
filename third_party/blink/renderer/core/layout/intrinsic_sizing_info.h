...
inline float ResolveWidthForRatio(float height, const gfx::SizeF& natural_ratio)
{
    return height * natural_ratio.width() / natural_ratio.height();
}

inline float ResolveHeightForRatio(float width, const gfx::SizeF& natural_ratio)
{
...
CORE_EXPORT gfx::SizeF ConcreteObjectSize(const IntrinsicSizingInfo& sizing_info, const gfx::SizeF& default_object_size);
