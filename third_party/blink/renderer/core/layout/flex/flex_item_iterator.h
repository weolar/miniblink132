...
public:
    FlexItemIterator(const HeapVector<NGFlexLine>& flex_lines, const BlockBreakToken* break_token, bool is_column);
...
public:
    Entry(NGFlexItem* flex_item, wtf_size_t flex_item_idx, wtf_size_t flex_line_idx, const BlockBreakToken* token)
        : flex_item(flex_item)
    ...
    {
        return flex_item == other.flex_item && flex_item_idx == other.flex_item_idx && flex_line_idx == other.flex_line_idx && token == other.token;
    }
};
