...
FlexChildIterator::FlexChildIterator(const BlockNode node)
{
    ...
    int initial_order = is_deprecated_webkit_box ? ComputedStyleInitialValues::InitialBoxOrdinalGroup() : ComputedStyleInitialValues::InitialOrder();
    ...
    for (LayoutInputNode child = node.FirstChild(); child; child = child.NextSibling()) {
        int order = is_deprecated_webkit_box ? child.Style().BoxOrdinalGroup() : child.Style().Order();
    ...
    // -webkit-box-ordinal-group/order property.
    if (needs_sort) {
        std::stable_sort(children_.begin(), children_.end(), [](const ChildWithOrder& c1, const ChildWithOrder& c2) { return c1.order < c2.order; });
    }
}
