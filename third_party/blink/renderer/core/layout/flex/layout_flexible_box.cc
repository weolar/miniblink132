...
LogicalToPhysical<bool> GetOverflowConverter(const ComputedStyle& style)
{
    ...
    return LogicalToPhysical(style.GetWritingDirection(), inline_start, inline_end, block_start, block_end);
}
...
void MergeAnonymousFlexItems(LayoutObject* remove_child)
{
    ...
    if (!next || !next->IsAnonymousBlock())
        return;
    To<LayoutBoxModelObject>(next)->MoveAllChildrenTo(To<LayoutBoxModelObject>(prev));
    next->Destroy();
}

} // namespace

bool LayoutFlexibleBox::IsChildAllowed(LayoutObject* object, const ComputedStyle& style) const
{
    ...
    if (select && select->UsesMenuList()) [[unlikely]] {
        if (select->IsAppearanceBaseButton()) {
            ...
            if (IsA<HTMLOptionElement>(object->GetNode()) || IsA<HTMLOptGroupElement>(object->GetNode()) || IsA<HTMLHRElement>(object->GetNode())) {
...
{
    if (!DocumentBeingDestroyed() && !StyleRef().IsDeprecatedFlexboxUsingFlexLayout())
