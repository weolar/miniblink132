...
class CORE_EXPORT FlexLayoutAlgorithm : public LayoutAlgorithm<BlockNode, BoxFragmentBuilder, BlockBreakToken> {
public:
    explicit FlexLayoutAlgorithm(const LayoutAlgorithmParams& params, const HashMap<wtf_size_t, LayoutUnit>* cross_size_adjustments = nullptr);
...
private:
    ...
    void PlaceFlexItems(
        HeapVector<NGFlexLine>* flex_line_outputs, HeapVector<Member<LayoutBox>>* oof_children, bool is_computing_multiline_column_intrinsic_size = false);
    ...
    ConstraintSpace BuildSpaceForIntrinsicInlineSize(const BlockNode& flex_item) const;
    ConstraintSpace BuildSpaceForFlexBasis(const BlockNode& flex_item) const;
    ConstraintSpace BuildSpaceForIntrinsicBlockSize(const BlockNode& flex_item, std::optional<LayoutUnit> override_inline_size) const;
    ...
    ConstraintSpace BuildSpaceForLayout(const BlockNode& flex_item_node, LayoutUnit item_main_axis_final_size, bool is_initial_block_size_indefinite,
        std::optional<LayoutUnit> override_inline_size = std::nullopt, std::optional<LayoutUnit> line_cross_size_for_stretch = std::nullopt,
        std::optional<LayoutUnit> block_offset_for_fragmentation = std::nullopt, bool min_block_size_should_encompass_intrinsic_size = false) const;

    void ConstructAndAppendFlexItems(Phase phase, HeapVector<Member<LayoutBox>>* oof_children = nullptr);
    void ApplyReversals(HeapVector<NGFlexLine>* flex_line_outputs);
    LayoutResult::EStatus GiveItemsFinalPositionAndSize(HeapVector<NGFlexLine>* flex_line_outputs, Vector<EBreakBetween>* row_break_between_outputs);
    ...
    LayoutResult::EStatus PropagateFlexItemInfo(FlexItem* flex_item, wtf_size_t flex_line_idx, LogicalOffset offset, PhysicalSize fragment_size);
    ...
    void HandleOutOfFlowPositionedItems(HeapVector<Member<LayoutBox>>& oof_children);
    ...
    void ConsumeRemainingFragmentainerSpace(
        LayoutUnit offset_in_stitched_container, NGFlexLine* flex_line, const FlexColumnBreakInfo* column_break_info = nullptr);

    BreakStatus BreakBeforeChildIfNeeded(LayoutInputNode child, const LayoutResult& layout_result, LayoutUnit fragmentainer_block_offset,
        bool has_container_separation, bool is_row_item, FlexColumnBreakInfo* flex_column_break_info)
    {
        return ::blink::BreakBeforeChildIfNeeded(GetConstraintSpace(), child, layout_result, fragmentainer_block_offset, FragmentainerCapacityForChildren(),
            has_container_separation, &container_builder_, is_row_item, flex_column_break_info);
    }
    ...
    BreakStatus BreakBeforeRowIfNeeded(const NGFlexLine& row, LayoutUnit row_block_offset, EBreakBetween row_break_between, wtf_size_t row_index,
        LayoutInputNode child, bool has_container_separation, bool is_first_for_row);
    ...
    // more documentation.
    bool MovePastRowBreakPoint(BreakAppeal appeal_before, LayoutUnit fragmentainer_block_offset, LayoutUnit row_block_size, wtf_size_t row_index,
    ...
    void AdjustOffsetForNextLine(HeapVector<NGFlexLine>* flex_line_outputs, wtf_size_t flex_line_idx, LayoutUnit item_expansion) const;
