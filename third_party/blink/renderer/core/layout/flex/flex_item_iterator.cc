...
FlexItemIterator::FlexItemIterator(const HeapVector<NGFlexLine>& flex_lines, const BlockBreakToken* break_token, bool is_column)
    ...
    , is_column_(is_column)
{
    if (flex_lines_.size()) {
        ...
        next_unstarted_item_ = const_cast<NGFlexItem*>(&flex_lines_[0].line_items[0]);
        flex_item_idx_++;
    }
    if (break_token_) {
...
FlexItemIterator::Entry FlexItemIterator::NextItem(bool broke_before_row)
{
    ...
    if (break_token_) {
        ...
        if (child_token_idx_ < child_break_tokens.size()) {
            current_child_break_token = To<BlockBreakToken>(child_break_tokens[child_token_idx_++].Get());
            ...
            if (child_token_idx_ == child_break_tokens.size()) {
                ...
                if (!is_column_ && (current_item_idx != 0 || !current_child_break_token->IsBreakBefore() || !broke_before_row)) {
    ...
    return Entry(current_item, current_item_idx, current_line_idx, current_child_break_token);
}

NGFlexItem* FlexItemIterator::FindNextItem(const BlockBreakToken* item_break_token)
{
    while (flex_line_idx_ < flex_lines_.size()) {
        ...
        if (!flex_line.has_seen_all_children || item_break_token) {
            while (flex_item_idx_ < flex_line.line_items.size()) {
                NGFlexItem* flex_item = const_cast<NGFlexItem*>(&flex_line.line_items[flex_item_idx_++]);
                if (!item_break_token || flex_item->ng_input_node == item_break_token->InputNode())
                    return flex_item;
            }
        }
        ...
        if (is_column_ && !item_break_token && flex_line_idx_ == next_item_idx_for_line_.size() - 1)
            break;
