...
public:
    TableTypes::Column MakeColumn(int min_width, int max_width, std::optional<float> percent = std::nullopt, bool is_constrained = false)
    {
        return { LayoutUnit(min_width), LayoutUnit(max_width), percent,
            /* border_padding */ LayoutUnit(), is_constrained,
            /* is_collapsed */ false,
            /* is_table_fixed */ false,
            /* is_mergeable */ false };
    }

    TableTypes::Row MakeRow(int block_size, bool is_constrained = false, bool has_rowspan_start = false, std::optional<float> percent = std::nullopt)
    {
        return { LayoutUnit(block_size), 0, 0, std::nullopt, percent, is_constrained, has_rowspan_start, false };
    }

    TableTypes::Section MakeSection(TableTypes::Rows* rows, int block_size, wtf_size_t rowspan = 1, std::optional<float> percent = std::nullopt)
    {
        ...
        bool is_tbody = true;
        return TableTypes::Section { start_row, rowspan, LayoutUnit(block_size), percent, is_constrained, is_tbody, false };
    }
};

TEST_F(TableLayoutUtilsTest, DistributeColspanAutoPercent)
{
    TableTypes::ColspanCell colspan_cell(TableTypes::CellInlineConstraint(), 0, 3);
    ...
    DistributeColspanCellsToColumns(colspan_cells, LayoutUnit(), false, column_constraints.get());
...
TEST_F(TableLayoutUtilsTest, DistributeColspanAutoExactMaxSize)
{
    ...
    LayoutUnit column_widths[] = { LayoutUnit(0.1), LayoutUnit(22.123456), LayoutUnit(33.789012), LayoutUnit(2000.345678) };
    scoped_refptr<TableTypes::Columns> column_constraints = base::MakeRefCounted<TableTypes::Columns>();
    column_constraints->data.Shrink(0);
    column_constraints->data.push_back(TableTypes::Column { LayoutUnit(0), column_widths[0], std::nullopt, LayoutUnit(), false, false, false, false });
    column_constraints->data.push_back(TableTypes::Column { LayoutUnit(3.33333), column_widths[1], std::nullopt, LayoutUnit(), false, false, false, false });
    column_constraints->data.push_back(TableTypes::Column { LayoutUnit(3.33333), column_widths[2], std::nullopt, LayoutUnit(), false, false, false, false });
    column_constraints->data.push_back(TableTypes::Column { LayoutUnit(0), column_widths[3], std::nullopt, LayoutUnit(), false, false, false, false });

    LayoutUnit assignable_table_inline_size = column_widths[0] + column_widths[1] + column_widths[2] + column_widths[3];
    Vector<LayoutUnit> column_sizes = SynchronizeAssignableTableInlineSizeAndColumns(assignable_table_inline_size, false, *column_constraints);
...
TEST_F(TableLayoutUtilsTest, ComputeGridInlineMinMax)
{
    ...
    MinMaxSizes minmax = ComputeGridInlineMinMax(node, *column_constraints, undistributable_space, is_fixed_layout, is_layout_pass);
    ...
    column_constraints->data.push_back(MakeColumn(10, 10));
    minmax = ComputeGridInlineMinMax(node, *column_constraints, undistributable_space, is_fixed_layout, is_layout_pass);
...
TEST_F(TableLayoutUtilsTest, DistributeSectionFixedBlockSizeToRows)
{
    ...
    rows.push_back(MakeRow(100));
    DistributeSectionFixedBlockSizeToRows(0, 3, LayoutUnit(1000), LayoutUnit(), LayoutUnit(1000), &rows);
...
TEST_F(TableLayoutUtilsTest, DistributeTableBlockSizeToSections)
{
    ...
    DistributeTableBlockSizeToSections(LayoutUnit(), LayoutUnit(500), &sections, &rows);
    ...
    sections.push_back(MakeSection(&rows, 100));
    DistributeTableBlockSizeToSections(LayoutUnit(), LayoutUnit(1000), &sections, &rows);
