...
public:
    static constexpr LayoutUnit kTableMaxInlineSize = LayoutUnit(static_cast<uint64_t>(1000000));
    ...
    struct ColspanCell {
        ...
        wtf_size_t span;
        ColspanCell(const CellInlineConstraint& cell_inline_constraint, unsigned start_column, unsigned span)
    ...
    struct Column {
        DISALLOW_NEW();
        Column(const std::optional<LayoutUnit>& min_inline_size, const std::optional<LayoutUnit>& max_inline_size, const std::optional<float>& percent,
            LayoutUnit percent_border_padding, bool is_constrained, bool is_collapsed, bool is_table_fixed, bool is_mergeable)
        ...
        {
            return min_inline_size == other.min_inline_size && max_inline_size == other.max_inline_size && percent == other.percent
                && percent_border_padding == other.percent_border_padding && is_constrained == other.is_constrained && is_collapsed == other.is_collapsed
                && is_table_fixed == other.is_table_fixed && is_mergeable == other.is_mergeable;
        }
        bool operator!=(const Column& other) const
        {
            return !(*this == other);
        ...
        LayoutUnit ResolvePercentInlineSize(LayoutUnit percentage_resolution_inline_size) const
        {
            return std::max(min_inline_size.value_or(LayoutUnit()), LayoutUnit(*percent * percentage_resolution_inline_size / 100) + percent_border_padding);
        }
        bool IsFixed() const
        {
    ...
    struct CellBlockConstraint {
        ...
        CellBlockConstraint(LayoutUnit min_block_size, BoxStrut borders, wtf_size_t column_index, wtf_size_t effective_rowspan, bool is_constrained,
            bool has_descendant_that_depends_on_percentage_block_size)
    ...
    struct RowspanCell {
        ...
        wtf_size_t effective_rowspan;
        LayoutUnit min_block_size;
        RowspanCell(wtf_size_t start_row, wtf_size_t effective_rowspan, LayoutUnit min_block_size)
            : start_row(start_row)
            , effective_rowspan(effective_rowspan)
            , min_block_size(min_block_size)
        {
        ...
        bool operator<(const TableTypes::RowspanCell& rhs) const
        {
            ...
            // |RowspanCell|.
            auto IsEnclosed = [](const TableTypes::RowspanCell& c1, const TableTypes::RowspanCell& c2) {
                return (c1.start_row >= c2.start_row) && (c1.start_row + c1.effective_rowspan) <= (c2.start_row + c2.effective_rowspan);
            };

            // If cells span the same rows, the bigger cell is distributed first.
            if (start_row == rhs.start_row && effective_rowspan == rhs.effective_rowspan) {
    ...
    static Column CreateColumn(const ComputedStyle&, std::optional<LayoutUnit> default_inline_size, bool is_table_fixed);

    static CellInlineConstraint CreateCellInlineConstraint(
        const BlockNode&, WritingDirectionMode table_writing_direction, bool is_fixed_layout, const BoxStrut& cell_border, const BoxStrut& cell_padding);

    static Section CreateSection(const LayoutInputNode&, wtf_size_t start_row, wtf_size_t row_count, LayoutUnit block_size, bool treat_as_tbody);
...
public:
    explicit TableGroupedChildrenIterator(const TableGroupedChildren& grouped_children, bool is_end = false);
    ...
    bool TreatAsTBody() const
    {
        return current_section_ == kBody;
    }
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::TableTypes::CellInlineConstraint)
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::TableTypes::ColspanCell)
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::TableTypes::Column)
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::TableTypes::CellBlockConstraint)
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::TableTypes::RowspanCell)
