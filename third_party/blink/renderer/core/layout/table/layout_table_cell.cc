...
{
    const ComputedStyle* new_style = parent.GetDocument().GetStyleResolver().CreateAnonymousStyleWithDisplay(parent.StyleRef(), EDisplay::kTableCell);
...
void LayoutTableCell::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
...
void LayoutTableCell::ColSpanOrRowSpanChanged()
{
    ...
    UpdateColAndRowSpanFlags();
    if (LayoutTable* table = Table()) {
        table->SetNeedsLayoutAndIntrinsicWidthsRecalc(layout_invalidation_reason::kTableChanged);
        table->TableGridStructureChanged();
    }
}

LayoutBox* LayoutTableCell::CreateAnonymousBoxWithSameTypeAs(const LayoutObject* parent) const
{
    NOT_DESTROYED();
...
void LayoutTableCell::InvalidatePaint(const PaintInvalidatorContext& context) const
{
    NOT_DESTROYED();
    TableCellPaintInvalidator(*this, context).InvalidatePaint();
}

bool LayoutTableCell::BackgroundIsKnownToBeOpaqueInRect(const PhysicalRect& local_rect) const
{
    NOT_DESTROYED();
...
unsigned LayoutTableCell::ParseColSpanFromDOM() const
{
    ...
    if (const auto* cell_element = DynamicTo<HTMLTableCellElement>(GetNode())) {
        ...
        return span;
    } else if (const auto* mathml_cell_element = DynamicTo<MathMLTableCellElement>(GetNode())) {
