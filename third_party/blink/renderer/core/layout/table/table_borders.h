...
public:
    ...
    static LayoutUnit BorderWidth(const ComputedStyle* style, EdgeSide edge_side)
    {
        if (!style)
            return LayoutUnit();
        switch (edge_side) {
    ...
    static EBorderStyle BorderStyle(const ComputedStyle* style, EdgeSide edge_side)
    {
        if (!style)
    ...
    static bool HasBorder(const ComputedStyle* style)
    {
        if (!style)
            return false;
        return style->BorderLeftStyle() != EBorderStyle::kNone || style->BorderRightStyle() != EBorderStyle::kNone
            || style->BorderTopStyle() != EBorderStyle::kNone || style->BorderBottomStyle() != EBorderStyle::kNone;
    }

    LayoutUnit BorderWidth(wtf_size_t edge_index) const
    {
        return BorderWidth(edges_[edge_index].style.Get(), edges_[edge_index].edge_side);
    }

    EBorderStyle BorderStyle(wtf_size_t edge_index) const
    {
        return BorderStyle(edges_[edge_index].style.Get(), edges_[edge_index].edge_side);
    }

    Color BorderColor(wtf_size_t edge_index) const
    {
        return BorderColor(edges_[edge_index].style.Get(), edges_[edge_index].edge_side);
    }
    ...
    wtf_size_t EdgesPerRow() const
    {
        return edges_per_row_;
    }

    const BoxStrut& TableBorder() const
    {
        return table_border_;
    }

    BoxStrut CellBorder(const BlockNode& cell, wtf_size_t row, wtf_size_t column, wtf_size_t section, WritingDirectionMode table_writing_direction) const;

    BoxStrut CellPaddingForMeasure(const ComputedStyle& cell_style, WritingDirectionMode table_writing_direction) const;

    void UpdateTableBorder(wtf_size_t table_row_count, wtf_size_t table_column_count);

    // |section_index| is only used to clamp rowspan. Only needed for
    // cells with rowspan > 1.
    void MergeBorders(wtf_size_t start_row, wtf_size_t start_column, wtf_size_t rowspan, wtf_size_t colspan, const ComputedStyle& source_style,
        EdgeSource source, wtf_size_t box_order, WritingDirectionMode table_writing_direction, wtf_size_t section_index = kNotFound);
    ...
    Edges::const_iterator begin() const
    {
        return edges_.begin();
    }

    Edges::const_iterator end() const
    {
        return edges_.end();
    }

    wtf_size_t EdgeCount() const
    {
        return edges_.size();
    }
    ...
    {
        return (index_offset >= 0 || (index_offset < 0 && edge_index >= static_cast<wtf_size_t>(abs(index_offset))))
            && edge_index + index_offset < edges_.size() && edges_[edge_index + index_offset].style;
    }
...
private:
    ...
    wtf_size_t ClampRowspan(wtf_size_t section_index, wtf_size_t table_row_index, wtf_size_t rowspan) const
    {
        if (rowspan <= 1)
            return rowspan;
        DCHECK_LT(section_index, sections_.size());
        return std::min(rowspan, sections_[section_index].row_count - (table_row_index - sections_[section_index].start_row));
    }

    BoxStrut GetCellBorders(wtf_size_t row, wtf_size_t column, wtf_size_t rowspan, wtf_size_t colspan) const;

    void EnsureCellColumnFits(wtf_size_t cell_column);

    void EnsureCellRowFits(wtf_size_t cell_row);

    void MergeRowAxisBorder(
        wtf_size_t start_row, wtf_size_t start_column, wtf_size_t colspan, const ComputedStyle& source_style, wtf_size_t box_order, EdgeSide side);

    void MergeColumnAxisBorder(
        wtf_size_t start_row, wtf_size_t start_column, wtf_size_t rowspan, const ComputedStyle& source_style, wtf_size_t box_order, EdgeSide side);

    void MarkInnerBordersAsDoNotFill(wtf_size_t start_row, wtf_size_t start_column, wtf_size_t rowspan, wtf_size_t colspan);
