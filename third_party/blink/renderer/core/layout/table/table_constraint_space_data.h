...
public:
    ...
    struct Row {
        Row(LayoutUnit block_size, wtf_size_t start_cell_index, wtf_size_t cell_count, std::optional<LayoutUnit> baseline, bool is_collapsed)
        ...
        bool MaySkipLayout(const Row& other) const
        {
            ...
            return block_size == other.block_size && cell_count == other.cell_count && baseline == other.baseline && is_collapsed == other.is_collapsed;
        }
    ...
    struct Cell {
        Cell(BoxStrut borders, LayoutUnit rowspan_block_size, wtf_size_t start_column, bool is_initial_block_size_indefinite,
        ...
        {
            return borders == other.borders && rowspan_block_size == other.rowspan_block_size && start_column == other.start_column
                && is_initial_block_size_indefinite == other.is_initial_block_size_indefinite
                && has_descendant_that_depends_on_percentage_block_size == other.has_descendant_that_depends_on_percentage_block_size;
        }
        bool operator!=(const Cell& other) const
        {
            return !(*this == other);
    ...
    {
        return column_locations == other.column_locations && table_writing_direction == other.table_writing_direction
            && table_border_spacing == other.table_border_spacing && is_table_block_size_specified == other.is_table_block_size_specified
            && has_collapsed_borders == other.has_collapsed_borders;
    }

    bool MaySkipRowLayout(const TableConstraintSpaceData& other, wtf_size_t new_row_index, wtf_size_t old_row_index) const
    {
        ...
        const wtf_size_t new_end_cell_index = new_start_cell_index + new_row.cell_count;
        const wtf_size_t old_end_cell_index = old_start_cell_index + old_row.cell_count;

        for (wtf_size_t new_cell_index = new_start_cell_index, old_cell_index = old_start_cell_index;
             new_cell_index < new_end_cell_index && old_cell_index < old_end_cell_index; ++new_cell_index, ++old_cell_index) {
    ...
    bool MaySkipSectionLayout(const TableConstraintSpaceData& other, wtf_size_t new_section_index, wtf_size_t old_section_index) const
    {
        ...
        const wtf_size_t new_end_row_index = new_start_row_index + new_section.row_count;
        const wtf_size_t old_end_row_index = old_start_row_index + old_section.row_count;

        for (wtf_size_t new_row_index = new_start_row_index, old_row_index = old_start_row_index;
             new_row_index < new_end_row_index && old_row_index < old_end_row_index; ++new_row_index, ++old_row_index) {
    ...
    Vector<Row> rows;
    Vector<Cell> cells;
    WritingDirectionMode table_writing_direction = WritingDirectionMode(WritingMode::kHorizontalTb, TextDirection::kLtr);
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::TableConstraintSpaceData::Section)
WTF_ALLOW_MOVE_AND_INIT_WITH_MEM_FUNCTIONS(blink::TableConstraintSpaceData::Row)
WTF_ALLOW_MOVE_AND_INIT_WITH_MEM_FUNCTIONS(blink::TableConstraintSpaceData::Cell)
