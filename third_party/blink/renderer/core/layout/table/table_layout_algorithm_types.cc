...
inline void InlineSizesFromStyle(const ComputedStyle& style, LayoutUnit inline_border_padding, bool is_parallel, std::optional<LayoutUnit>* inline_size,
    std::optional<LayoutUnit>* min_inline_size, std::optional<LayoutUnit>* max_inline_size, std::optional<float>* percentage_inline_size)
{
    const Length& length = is_parallel ? style.LogicalWidth() : style.LogicalHeight();
    const Length& min_length = is_parallel ? style.LogicalMinWidth() : style.LogicalMinHeight();
    const Length& max_length = is_parallel ? style.LogicalMaxWidth() : style.LogicalMaxHeight();
    ...
    if (*percentage_inline_size && max_length.IsPercent()) {
        *percentage_inline_size = std::min(**percentage_inline_size, max_length.Percent());
    }
...
TableTypes::Column TableTypes::CreateColumn(const ComputedStyle& style, std::optional<LayoutUnit> default_inline_size, bool is_table_fixed)
{
    ...
    InlineSizesFromStyle(style, /* inline_border_padding */ LayoutUnit(),
        /* is_parallel */ true, &inline_size, &min_inline_size, &max_inline_size, &percentage_inline_size);
    bool is_mergeable;
    if (!inline_size)
    ...
    if (is_table_fixed) {
        ...
        is_mergeable = (inline_size.value_or(LayoutUnit()) == LayoutUnit()) && (percentage_inline_size.value_or(0.0f) == 0.0f);
    }
    return Column(min_inline_size.value_or(LayoutUnit()), inline_size, percentage_inline_size, LayoutUnit() /* percent_border_padding */, is_constrained,
...
TableTypes::CellInlineConstraint TableTypes::CreateCellInlineConstraint(
    const BlockNode& node, WritingDirectionMode table_writing_direction, bool is_fixed_layout, const BoxStrut& cell_border, const BoxStrut& cell_padding)
{
    ...
    const bool is_parallel = IsParallelWritingMode(table_writing_mode, style.GetWritingMode());
    ...
    auto MinMaxSizesFunc = [&]() -> MinMaxSizes {
        if (!cached_min_max_sizes) {
            ...
            ConstraintSpaceBuilder builder(table_writing_mode, cell_writing_direction,
                /* is_new_fc */ true);
            builder.SetTableCellBorders(cell_border, cell_writing_direction, table_writing_direction);
            ...
            if (!is_parallel) {
                ...
                builder.SetOrthogonalFallbackInlineSize(IsHorizontalWritingMode(table_writing_mode) ? icb_size.height : icb_size.width);
            }
            builder.SetAvailableSize({ kIndefiniteSize, kIndefiniteSize });
            const auto space = builder.ToConstraintSpace();

            cached_min_max_sizes = node.ComputeMinMaxSizes(table_writing_mode, SizeType::kIntrinsic, space).sizes;
        }

        return *cached_min_max_sizes;
    };

    InlineSizesFromStyle(style, (cell_border + cell_padding).InlineSum(), is_parallel, &css_inline_size, &css_min_inline_size, &css_max_inline_size,
        &css_percentage_inline_size);

    // Compute the resolved min inline-size.
    LayoutUnit resolved_min_inline_size;
    if (!is_fixed_layout) {
        resolved_min_inline_size = std::max(MinMaxSizesFunc().min_size, css_min_inline_size.value_or(LayoutUnit()));
        // https://quirks.spec.whatwg.org/#the-table-cell-nowrap-minimum-width-calculation-quirk
        bool has_nowrap_attribute = node.GetDOMNode() && To<Element>(node.GetDOMNode())->FastHasAttribute(html_names::kNowrapAttr);
        if (css_inline_size && node.GetDocument().InQuirksMode() && has_nowrap_attribute) {
            resolved_min_inline_size = std::max(resolved_min_inline_size, *css_inline_size);
        }
    }
    ...
    if (css_max_inline_size) {
        ...
        resolved_min_inline_size = std::min(resolved_min_inline_size, *css_max_inline_size);
    }
    LayoutUnit resolved_max_inline_size = std::max(resolved_min_inline_size, content_max);
    ...
    if (is_fixed_layout && css_percentage_inline_size && style.BoxSizing() == EBoxSizing::kContentBox)
        percent_border_padding = (cell_border + cell_padding).InlineSum();

    DCHECK_GE(resolved_max_inline_size, percent_border_padding);
    return TableTypes::CellInlineConstraint { resolved_min_inline_size, resolved_max_inline_size, css_percentage_inline_size, percent_border_padding,
        is_constrained };
}

TableTypes::Section TableTypes::CreateSection(
    const LayoutInputNode& section, wtf_size_t start_row, wtf_size_t row_count, LayoutUnit block_size, bool treat_as_tbody)
{
    ...
    bool is_constrained = section_css_block_size.IsFixed() || section_css_block_size.IsPercent();
    std::optional<float> percent;
    if (section_css_block_size.IsPercent())
        percent = section_css_block_size.Percent();
    return Section { start_row, row_count, block_size, percent, is_constrained, treat_as_tbody,
        /* needs_redistribution */ false };
}

void TableTypes::CellInlineConstraint::Encompass(const TableTypes::CellInlineConstraint& other)
{
    // Standard says:
...
void TableTypes::Column::Encompass(const std::optional<TableTypes::CellInlineConstraint>& cell)
{
    if (!cell)
        return;
    ...
    if (min_inline_size) {
        ...
        if (is_constrained) {
            ...
            max_inline_size = std::max(max_inline_size.value_or(LayoutUnit()), cell->max_inline_size);
        }
    } else {
        min_inline_size = cell->min_inline_size;
...
TableGroupedChildren::TableGroupedChildren(const BlockNode& table)
    : header(BlockNode(nullptr))
    , footer(BlockNode(nullptr))
{
    for (LayoutInputNode child = table.FirstChild(); child; child = child.NextSibling()) {
...
TableGroupedChildrenIterator::TableGroupedChildrenIterator(const TableGroupedChildren& grouped_children, bool is_end)
...
bool TableGroupedChildrenIterator::operator==(const TableGroupedChildrenIterator& rhs) const
{
