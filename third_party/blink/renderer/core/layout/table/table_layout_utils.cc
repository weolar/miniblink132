...
// non-meargeable.
void EnsureDistributableColumnExists(wtf_size_t start_column_index, wtf_size_t span, TableTypes::Columns* column_constraints)
{
    DCHECK_LT(start_column_index, column_constraints->data.size());
    DCHECK_GT(span, 1u);

    wtf_size_t effective_span = std::min(span, column_constraints->data.size() - start_column_index);
    TableTypes::Column* start_column = &column_constraints->data[start_column_index];
    TableTypes::Column* end_column = start_column + effective_span;
    for (TableTypes::Column* column = start_column; column != end_column; ++column) {
...
void ApplyCellConstraintsToColumnConstraints(const TableTypes::CellInlineConstraints& cell_constraints, LayoutUnit inline_border_spacing, bool is_fixed_layout,
    TableTypes::ColspanCells* colspan_cell_constraints, TableTypes::Columns* column_constraints)
{
    ...
    if (column_constraints->data.size() < cell_constraints.size()) {
        ...
        wtf_size_t column_count = cell_constraints.size() - column_constraints->data.size();
        ...
        while (last_non_merged_column + 1 > cell_constraints.size() && column_constraints->data[last_non_merged_column].is_mergeable) {
    ...
    for (const TableTypes::ColspanCell& colspan_cell : *colspan_cell_constraints) {
        EnsureDistributableColumnExists(colspan_cell.start_column, colspan_cell.span, column_constraints);
    }
    ...
    auto colspan_cell_less_than = [](const TableTypes::ColspanCell& lhs, const TableTypes::ColspanCell& rhs) {
        if (lhs.span == rhs.span)
            return lhs.start_column < rhs.start_column;
        return lhs.span < rhs.span;
    };
    std::stable_sort(colspan_cell_constraints->begin(), colspan_cell_constraints->end(), colspan_cell_less_than);

    DistributeColspanCellsToColumns(*colspan_cell_constraints, inline_border_spacing, is_fixed_layout, column_constraints);
...
TableTypes::Row ComputeMinimumRowBlockSize(const RowCountFunc& row_count_func, const BlockNode& row, const LayoutUnit cell_percentage_inline_size,
    const bool is_table_block_size_specified, const Vector<TableColumnLocation>& column_locations, const TableBorders& table_borders,
    wtf_size_t start_row_index, wtf_size_t row_index, wtf_size_t section_index, bool is_section_collapsed,
    TableTypes::CellBlockConstraints* cell_block_constraints, TableTypes::RowspanCells* rowspan_cells, ColspanCellTabulator* colspan_cell_tabulator)
{
    const WritingDirectionMode table_writing_direction = row.Style().GetWritingDirection();
    ...
    for (BlockNode cell = To<BlockNode>(row.FirstChild()); cell; cell = To<BlockNode>(cell.NextSibling())) {
        ...
        const BoxStrut cell_borders
            = table_borders.CellBorder(cell, row_index, colspan_cell_tabulator->CurrentColumn(), section_index, table_writing_direction);
        ...
        if (effective_rowspan > 1) {
            const wtf_size_t max_rows = row_count_func() - (row_index - start_row_index);
            effective_rowspan = std::min(max_rows, effective_rowspan);
        }

        ConstraintSpaceBuilder space_builder(table_writing_direction.GetWritingMode(), cell_writing_direction,
            /* is_new_fc */ true);

        // We want these values to match the "layout" pass as close as possible.
        SetupTableCellConstraintSpaceBuilder(table_writing_direction, cell, cell_borders, column_locations,
            /* cell_block_size */ kIndefiniteSize, cell_percentage_inline_size,
            /* alignment_baseline */ std::nullopt, colspan_cell_tabulator->CurrentColumn(),
            /* is_initial_block_size_indefinite */ true, is_table_block_size_specified, has_collapsed_borders, LayoutResultCacheSlot::kMeasure, &space_builder);

        const auto cell_space = space_builder.ToConstraintSpace();
        const LayoutResult* layout_result = cell.Layout(cell_space);

        const LogicalBoxFragment fragment(table_writing_direction, To<PhysicalBoxFragment>(layout_result->GetPhysicalFragment()));
        const Length& cell_specified_block_length = IsParallelWritingMode(table_writing_direction.GetWritingMode(), cell_style.GetWritingMode())
            ? cell_style.LogicalHeight()
            : cell_style.LogicalWidth();

        bool has_descendant_that_depends_on_percentage_block_size = layout_result->HasDescendantThatDependsOnPercentageBlockSize();
        bool has_effective_rowspan = effective_rowspan > 1;

        TableTypes::CellBlockConstraint cell_block_constraint = { fragment.BlockSize(), cell_borders, colspan_cell_tabulator->CurrentColumn(),
            effective_rowspan, cell_specified_block_length.IsFixed(), has_descendant_that_depends_on_percentage_block_size };
        colspan_cell_tabulator->ProcessCell(cell);
        cell_block_constraints->push_back(cell_block_constraint);
        is_constrained |= cell_block_constraint.is_constrained && !has_effective_rowspan;
        row_baseline_tabulator.ProcessCell(
            fragment, ComputeContentAlignmentForTableCell(cell_style), has_effective_rowspan, has_descendant_that_depends_on_percentage_block_size);
        ...
        if (cell_specified_block_length.IsPercent()) {
            ...
            if (cell.GetDocument().InQuirksMode() || cell_style.BoxSizing() == EBoxSizing::kBorderBox) {
                cell_css_block_size = std::max(border_padding.BlockSum(), LayoutUnit(cell_specified_block_length.Value()));
            } else {
                cell_css_block_size = border_padding.BlockSum() + LayoutUnit(cell_specified_block_length.Value());
            }
        }
        ...
        if (!has_effective_rowspan) {
            ...
            max_cell_block_size = std::max({ max_cell_block_size, cell_block_constraint.min_block_size, cell_css_block_size.value_or(LayoutUnit()) });
        } else {
            ...
            rowspan_cells->push_back(TableTypes::RowspanCell { row_index, effective_rowspan, min_block_size });
        }
    }
    ...
    if (row_specified_block_length.IsPercent()) {
        is_constrained = true;
        row_percent = std::max(row_percent.value_or(0), row_specified_block_length.Percent());
    } else if (row_specified_block_length.IsFixed()) {
        is_constrained = true;
        max_cell_block_size = std::max(LayoutUnit(row_specified_block_length.Value()), max_cell_block_size);
    }

    const LayoutUnit row_block_size = row_baseline_tabulator.ComputeRowBlockSize(max_cell_block_size);
    ...
    return TableTypes::Row { row_block_size, start_cell_index, cell_block_constraints->size() - start_cell_index, row_baseline, row_percent, is_constrained,
        has_rowspan_start,
        /* is_collapsed */ is_section_collapsed || row.Style().Visibility() == EVisibility::kCollapse };
}

// Computes inline constraints for COLGROUP/COLs.
class ColumnConstraintsBuilder {
public:
    void VisitCol(const LayoutInputNode& column, wtf_size_t start_column_index, wtf_size_t span)
    {
        // COL creates SPAN constraints. Its width is col css width, or enclosing
        // colgroup css width.
        TableTypes::Column col_constraint = TableTypes::CreateColumn(
            column.Style(), !is_fixed_layout_ && colgroup_constraint_ ? colgroup_constraint_->max_inline_size : std::nullopt, is_fixed_layout_);
        for (wtf_size_t i = 0; i < span; ++i)
            column_constraints_->data.push_back(col_constraint);
        column.GetLayoutBox()->ClearNeedsLayout();
    }

    void EnterColgroup(const LayoutInputNode& colgroup, wtf_size_t start_column_index)
    {
        colgroup_constraint_ = TableTypes::CreateColumn(colgroup.Style(), std::nullopt, is_fixed_layout_);
    }

    void LeaveColgroup(const LayoutInputNode& colgroup, wtf_size_t start_column_index, wtf_size_t span, bool has_children)
    {
        if (!has_children) {
            for (wtf_size_t i = 0; i < span; ++i)
        ...
        To<LayoutTableColumn>(colgroup.GetLayoutBox())->ClearNeedsLayoutForChildren();
    }

    ColumnConstraintsBuilder(TableTypes::Columns* column_constraints, bool is_fixed_layout)
...
void ComputeColumnElementConstraints(const HeapVector<BlockNode>& columns, bool is_fixed_layout, TableTypes::Columns* column_constraints)
{
    ColumnConstraintsBuilder constraints_builder(column_constraints, is_fixed_layout);
...
void ComputeSectionInlineConstraints(const BlockNode& section, bool is_fixed_layout, bool is_first_section, WritingDirectionMode table_writing_direction,
    const TableBorders& table_borders, wtf_size_t section_index, wtf_size_t* row_index, TableTypes::CellInlineConstraints* cell_inline_constraints,
    TableTypes::ColspanCells* colspan_cell_inline_constraints)
{
    ColspanCellTabulator colspan_cell_tabulator;
    bool is_first_row = true;
    for (BlockNode row = To<BlockNode>(section.FirstChild()); row; row = To<BlockNode>(row.NextSibling())) {
        ...
        for (BlockNode cell = To<BlockNode>(row.FirstChild()); cell; cell = To<BlockNode>(cell.NextSibling())) {
            ...
            bool ignore_because_of_fixed_layout = is_fixed_layout && (!is_first_section || !is_first_row);

            wtf_size_t max_column = ComputeMaxColumn(colspan_cell_tabulator.CurrentColumn(), colspan, is_fixed_layout);
            if (max_column >= cell_inline_constraints->size())
                cell_inline_constraints->Grow(max_column);
            if (!ignore_because_of_fixed_layout) {
                BoxStrut cell_border
                    = table_borders.CellBorder(cell, *row_index, colspan_cell_tabulator.CurrentColumn(), section_index, table_writing_direction);
                BoxStrut cell_padding = table_borders.CellPaddingForMeasure(cell.Style(), table_writing_direction);
                TableTypes::CellInlineConstraint cell_constraint
                    = TableTypes::CreateCellInlineConstraint(cell, table_writing_direction, is_fixed_layout, cell_border, cell_padding);
                if (colspan == 1) {
                    std::optional<TableTypes::CellInlineConstraint>& constraint = (*cell_inline_constraints)[colspan_cell_tabulator.CurrentColumn()];
                    ...
                    colspan_cell_inline_constraints->emplace_back(cell_constraint, colspan_cell_tabulator.CurrentColumn(), colspan);
                }
            }
            colspan_cell_tabulator.ProcessCell(cell);
        }
...
Vector<LayoutUnit> DistributeInlineSizeToComputedInlineSizeAuto(
    LayoutUnit target_inline_size, const TableTypes::Column* start_column, const TableTypes::Column* end_column, const bool treat_target_size_as_constrained)
{
    ...
    for (const TableTypes::Column* column = start_column; column != end_column; ++column) {
        all_columns_count++;
        ...
        if (column->percent) {
            ...
            LayoutUnit percent_inline_size = column->ResolvePercentInlineSize(target_inline_size);
            ...
            guess_size_total_increases[kPercentageGuess] += percent_inline_size - *column->min_inline_size;
            ...
            guess_size_total_increases[kSpecifiedGuess] += *column->max_inline_size - *column->min_inline_size;
        } else { // Auto column
            auto_columns_count++;
            ...
            guess_sizes[kMaxGuess] += *column->max_inline_size;
            guess_size_total_increases[kMaxGuess] += *column->max_inline_size - *column->min_inline_size;
        }
    }
    ...
    case kMinGuess: {
        ...
        for (const TableTypes::Column* column = start_column; column != end_column; ++column, ++computed_size) {
            if (column->is_mergeable) {
                continue;
            }
    ...
    case kPercentageGuess: {
        ...
        LayoutUnit percent_inline_size_increase = guess_size_total_increases[kPercentageGuess];
        LayoutUnit distributable_inline_size = target_inline_size - guess_sizes[kMinGuess];
        ...
        for (const TableTypes::Column* column = start_column; column != end_column; ++column, ++computed_size) {
            ...
            if (column->percent) {
                ...
                LayoutUnit percent_inline_size = column->ResolvePercentInlineSize(target_inline_size);
                LayoutUnit column_inline_size_increase = percent_inline_size - *column->min_inline_size;
                LayoutUnit delta;
                if (percent_inline_size_increase > LayoutUnit()) {
                    delta = distributable_inline_size.MulDiv(column_inline_size_increase, percent_inline_size_increase);
                } else {
    ...
    case kSpecifiedGuess: {
        ...
        LayoutUnit fixed_inline_size_increase = guess_size_total_increases[kSpecifiedGuess];
        LayoutUnit distributable_inline_size = target_inline_size - guess_sizes[kPercentageGuess];
        LayoutUnit remaining_deficit = distributable_inline_size;
        LayoutUnit* last_computed_size = nullptr;
        LayoutUnit* computed_size = computed_sizes.data();
        for (const TableTypes::Column* column = start_column; column != end_column; ++column, ++computed_size) {
            ...
            if (column->percent) {
                *computed_size = column->ResolvePercentInlineSize(target_inline_size);
                ...
                last_computed_size = computed_size;
                LayoutUnit column_inline_size_increase = *column->max_inline_size - *column->min_inline_size;
                LayoutUnit delta;
                if (fixed_inline_size_increase > LayoutUnit()) {
                    delta = distributable_inline_size.MulDiv(column_inline_size_increase, fixed_inline_size_increase);
                } else {
    ...
    case kMaxGuess: {
        ...
        LayoutUnit auto_inline_size_increase = guess_size_total_increases[kMaxGuess];
        LayoutUnit distributable_inline_size = target_inline_size - guess_sizes[kSpecifiedGuess];
        ...
        LayoutUnit remaining_deficit = is_exact_match ? LayoutUnit() : distributable_inline_size;
        LayoutUnit* last_computed_size = nullptr;
        LayoutUnit* computed_size = computed_sizes.data();
        for (const TableTypes::Column* column = start_column; column != end_column; ++column, ++computed_size) {
            if (column->is_mergeable) {
            ...
            if (column->percent) {
                ...
                *computed_size = *column->max_inline_size;
                ...
                if (auto_inline_size_increase > LayoutUnit()) {
                    delta = distributable_inline_size.MulDiv(column_inline_size_increase, auto_inline_size_increase);
                } else {
    ...
    case kAboveMax: {
        LayoutUnit distributable_inline_size = target_inline_size - guess_sizes[kMaxGuess];
        if (auto_columns_count > 0) {
                ...
                if (column->percent) {
                    ...
                    *computed_size = *column->max_inline_size;
                    ...
                    if (total_auto_max_inline_size > LayoutUnit()) {
                        delta = distributable_inline_size.MulDiv(*column->max_inline_size, total_auto_max_inline_size);
                    } else {
                        delta = distributable_inline_size / auto_columns_count;
                    }
                    remaining_deficit -= delta;
                ...
                if (column->percent) {
                    *computed_size = column->ResolvePercentInlineSize(target_inline_size);
                    ...
                    if (total_fixed_max_inline_size > LayoutUnit()) {
                        delta = distributable_inline_size.MulDiv(*column->max_inline_size, total_fixed_max_inline_size);
                    } else {
                        delta = distributable_inline_size / fixed_columns_count;
                    }
                    remaining_deficit -= delta;
            ...
            for (const TableTypes::Column* column = start_column; column != end_column; ++column, ++computed_size) {
                ...
                if (total_percent != 0.0f) {
                    delta = LayoutUnit(distributable_inline_size * *column->percent / total_percent);
                } else {
                    delta = distributable_inline_size / percent_columns_count;
                }
                remaining_deficit -= delta;
...
Vector<LayoutUnit> SynchronizeAssignableTableInlineSizeAndColumnsFixed(LayoutUnit target_inline_size, const TableTypes::Columns& column_constraints)
{
    unsigned all_columns_count = 0;
    unsigned percent_columns_count = 0;
    unsigned auto_columns_count = 0;
    ...
    for (const TableTypes::Column& column : column_constraints.data) {
        ...
        if (column.percent) {
            percent_columns_count++;
            total_percent_inline_size += column.ResolvePercentInlineSize(target_inline_size);
            ...
            auto_columns_count++;
            total_auto_max_inline_size += column.max_inline_size.value_or(LayoutUnit());
        }
    }
    ...
    if (fixed_columns_count > 0) {
        ...
        LayoutUnit target_fixed_size = (target_inline_size - total_percent_inline_size).ClampNegativeToZero();
        bool scale_up = total_fixed_inline_size < target_fixed_size && auto_columns_count == 0;
        ...
        for (auto column = column_constraints.data.begin(); column != column_constraints.data.end(); ++column, ++column_size) {
            if (!TreatAsFixed(*column)) {
                continue;
            }
            last_column_size = column_size;
            if (scale_available) {
                *column_size = LayoutUnit(scale * column->max_inline_size.value_or(LayoutUnit()));
            } else {
                DCHECK_EQ(fixed_columns_count, all_columns_count);
                *column_size = LayoutUnit(target_inline_size.ToFloat() / fixed_columns_count);
            }
    ...
    if (percent_columns_count > 0) {
        ...
        bool scale_up = total_percent_inline_size < (target_inline_size - assigned_inline_size) && auto_columns_count == 0;
        bool scale_down = total_percent_inline_size > (target_inline_size - assigned_inline_size);
        if (scale_up || scale_down) {
            if (total_percent_inline_size != LayoutUnit()) {
                scale = (target_inline_size - assigned_inline_size).ToFloat() / total_percent_inline_size;
            } else {
                scale_available = false;
            }
        }
        ...
        for (auto column = column_constraints.data.begin(); column != column_constraints.data.end(); ++column, ++column_size) {
            ...
            last_column_size = column_size;
            if (scale_available) {
                *column_size = LayoutUnit(scale * column->ResolvePercentInlineSize(target_inline_size));
            } else {
                *column_size = LayoutUnit((target_inline_size - assigned_inline_size).ToFloat() / percent_columns_count);
            }
            assigned_inline_size += *column_size;
        }
    }
    // Distribute to auto, and zero inline size columns.
    LayoutUnit distributing_inline_size = target_inline_size - assigned_inline_size;
    LayoutUnit* column_size = column_sizes.data();

    bool distribute_zero_inline_size = zero_inline_size_constrained_colums_count == all_columns_count;
    ...
    for (auto column = column_constraints.data.begin(); column != column_constraints.data.end(); ++column, ++column_size) {
        ...
        last_column_size = column_size;
        *column_size
            = LayoutUnit(distributing_inline_size / float(distribute_zero_inline_size ? zero_inline_size_constrained_colums_count : auto_columns_count));
        assigned_inline_size += *column_size;
    }
...
void DistributeColspanCellToColumnsFixed(const TableTypes::ColspanCell& colspan_cell, LayoutUnit inline_border_spacing, TableTypes::Columns* column_constraints)
{
    // Fixed layout does not merge columns.
    DCHECK_LE(colspan_cell.span, column_constraints->data.size() - colspan_cell.start_column);
    TableTypes::Column* start_column = &column_constraints->data[colspan_cell.start_column];
    ...
    if (colspan_cell.cell_inline_constraint.is_constrained) {
        colspan_cell_min_inline_size = (colspan_cell.cell_inline_constraint.min_inline_size - total_inner_border_spacing).ClampNegativeToZero();
    }
    colspan_cell_max_inline_size = (colspan_cell.cell_inline_constraint.max_inline_size - total_inner_border_spacing).ClampNegativeToZero();
    ...
    LayoutUnit new_min_size = LayoutUnit(colspan_cell_min_inline_size / static_cast<float>(effective_span));
    LayoutUnit new_max_size = LayoutUnit(colspan_cell_max_inline_size / static_cast<float>(effective_span));
    ...
    for (TableTypes::Column* column = start_column; column < end_column; ++column) {
        ...
        if (!column->min_inline_size) {
            column->is_constrained |= colspan_cell.cell_inline_constraint.is_constrained;
    ...
    DCHECK(last_column);
    last_column->min_inline_size = *last_column->min_inline_size + rounding_error_min_inline_size;
    last_column->max_inline_size = *last_column->max_inline_size + rounding_error_max_inline_size;
}

void DistributeColspanCellToColumnsAuto(const TableTypes::ColspanCell& colspan_cell, LayoutUnit inline_border_spacing, TableTypes::Columns* column_constraints)
{
    if (column_constraints->data.empty()) {
        return;
    }
    unsigned effective_span = std::min(colspan_cell.span, column_constraints->data.size() - colspan_cell.start_column);
    TableTypes::Column* start_column = &column_constraints->data[colspan_cell.start_column];
    TableTypes::Column* end_column = start_column + effective_span;
    ...
    LayoutUnit colspan_cell_min_inline_size = (colspan_cell.cell_inline_constraint.min_inline_size - total_inner_border_spacing).ClampNegativeToZero();
    LayoutUnit colspan_cell_max_inline_size = (colspan_cell.cell_inline_constraint.max_inline_size - total_inner_border_spacing).ClampNegativeToZero();
    std::optional<float> colspan_cell_percent = colspan_cell.cell_inline_constraint.percent;
        ...
        if (surplus_percent > 0.0f && all_columns_count > percent_columns_count) {
            ...
            for (TableTypes::Column* column = start_column; column != end_column; ++column) {
                ...
                if (nonpercent_columns_max_inline_size != LayoutUnit()) {
                    ...
                    column_percent = surplus_percent * column->max_inline_size.value_or(LayoutUnit()) / nonpercent_columns_max_inline_size;
                } else {
    ...
    Vector<LayoutUnit> computed_sizes = DistributeInlineSizeToComputedInlineSizeAuto(colspan_cell_min_inline_size, start_column, end_column, true);
    LayoutUnit* computed_size = computed_sizes.data();
    for (TableTypes::Column* column = start_column; column != end_column; ++column, ++computed_size) {
        column->min_inline_size = std::max(*column->min_inline_size, *computed_size);
    }
    computed_sizes = DistributeInlineSizeToComputedInlineSizeAuto(colspan_cell_max_inline_size, start_column, end_column, /* treat_target_size_as_constrained */
        colspan_cell.cell_inline_constraint.is_constrained);
    computed_size = computed_sizes.data();
    for (TableTypes::Column* column = start_column; column != end_column; ++column, ++computed_size) {
        column->max_inline_size = std::max(std::max(*column->min_inline_size, *column->max_inline_size), *computed_size);
    }
}
...
void DistributeExcessBlockSizeToRows(const wtf_size_t start_row_index, const wtf_size_t row_count, LayoutUnit desired_block_size, bool is_rowspan_distribution,
    LayoutUnit border_block_spacing, LayoutUnit percentage_resolution_block_size, TableTypes::Rows* rows)
{
    ...
    auto RowBlockSizeDeficit = [&percentage_resolution_block_size](const TableTypes::Row& row) {
        DCHECK_NE(percentage_resolution_block_size, kIndefiniteSize);
        DCHECK(row.percent);
        return (LayoutUnit(*row.percent * percentage_resolution_block_size / 100) - row.block_size).ClampNegativeToZero();
    };
    ...
    for (auto index = start_row_index; index < end_row_index; ++index) {
        ...
        bool is_row_with_originating_rowspan = is_rowspan_distribution && index != start_row_index && row.has_rowspan_start;
        ...
        if (row.percent && *row.percent != 0 && percentage_resolution_block_size != kIndefiniteSize) {
        ...
        const bool is_row_constrained = row.is_constrained && (!row.percent || percentage_resolution_block_size != kIndefiniteSize);
    ...
    LayoutUnit distributable_block_size = (desired_block_size - border_block_spacing * (row_count - 1)) - total_block_size;
    ...
    if (!percent_rows_with_deficit.empty()) {
        ...
        LayoutUnit percent_distributable_block_size = std::min(percent_block_size_deficit, distributable_block_size);
        ...
        for (auto& index : percent_rows_with_deficit) {
            ...
            LayoutUnit delta = percent_distributable_block_size.MulDiv(RowBlockSizeDeficit(row), percent_block_size_deficit);
    ...
    if (!rows_with_originating_rowspan.empty()) {
        ...
        for (auto& index : rows_with_originating_rowspan) {
            auto& row = rows->at(index);
            LayoutUnit delta = distributable_block_size / rows_with_originating_rowspan.size();
            row.block_size += delta;
            remaining_deficit -= delta;
        }
    ...
    if (!unconstrained_non_empty_rows.empty()) {
        ...
        for (auto& index : unconstrained_non_empty_rows) {
            auto& row = rows->at(index);
            LayoutUnit delta = distributable_block_size.MulDiv(row.block_size, unconstrained_non_empty_row_block_size);
            row.block_size += delta;
            remaining_deficit -= delta;
        }
    ...
    if (!empty_rows.empty()) {
        ...
        if (is_rowspan_distribution) {
        ...
        } else if (has_only_empty_rows || (empty_rows.size() + constrained_non_empty_row_count == row_count)) {
            ...
            Vector<wtf_size_t>& rows_to_grow = !unconstrained_empty_rows.empty() ? unconstrained_empty_rows : empty_rows;
    ...
    if (!non_empty_rows.empty()) {
        ...
        for (auto& index : non_empty_rows) {
            auto& row = rows->at(index);
            LayoutUnit delta = distributable_block_size.MulDiv(row.block_size, total_block_size);
            row.block_size += delta;
            remaining_deficit -= delta;
        }
...
CellBlockSizeData ComputeCellBlockSize(const TableTypes::CellBlockConstraint& cell_block_constraint, const TableTypes::Rows& rows, wtf_size_t row_index,
    const LogicalSize& border_spacing, bool is_table_block_size_specified)
{
    ...
    bool is_initial_block_size_definite = cell_block_constraint.is_constrained || (has_grown && is_table_block_size_specified);

    return { cell_block_size, !is_initial_block_size_definite };
}

void SetupTableCellConstraintSpaceBuilder(const WritingDirectionMode table_writing_direction, const BlockNode cell, const BoxStrut& cell_borders,
    const Vector<TableColumnLocation>& column_locations, LayoutUnit cell_block_size, LayoutUnit percentage_inline_size,
    std::optional<LayoutUnit> alignment_baseline, wtf_size_t start_column, bool is_initial_block_size_indefinite, bool is_table_block_size_specified,
    bool has_collapsed_borders, LayoutResultCacheSlot cache_slot, ConstraintSpaceBuilder* builder)
{
    const auto& cell_style = cell.Style();
    const auto table_writing_mode = table_writing_direction.GetWritingMode();
    const wtf_size_t end_column = std::min(start_column + cell.TableCellColspan() - 1, column_locations.size() - 1);
    const LayoutUnit cell_inline_size = column_locations[end_column].offset + column_locations[end_column].size - column_locations[start_column].offset;
    ...
    if (!IsParallelWritingMode(table_writing_mode, cell_style.GetWritingMode())) {
        ...
        builder->SetOrthogonalFallbackInlineSize(table_writing_direction.IsHorizontal() ? icb_size.height : icb_size.width);
    }
    ...
    builder->SetPercentageResolutionSize({ percentage_inline_size, kIndefiniteSize });

    builder->SetTableCellBorders(cell_borders, cell_style.GetWritingDirection(), table_writing_direction);
    builder->SetTableCellAlignmentBaseline(alignment_baseline);
    builder->SetTableCellColumnIndex(start_column);
    builder->SetIsRestrictedBlockSizeTableCell(is_table_block_size_specified || cell_style.LogicalHeight().IsFixed());
    builder->SetIsHiddenForPaint(is_hidden_for_paint);
    builder->SetIsTableCellWithCollapsedBorders(has_collapsed_borders);
    builder->SetHideTableCellIfEmpty(!has_collapsed_borders && cell_style.EmptyCells() == EEmptyCells::kHide);
    builder->SetCacheSlot(cache_slot);
}

// Computes maximum possible number of non-mergeable columns.
wtf_size_t ComputeMaximumNonMergeableColumnCount(const HeapVector<BlockNode>& columns, bool is_fixed_layout)
{
    // Build column constraints.
    scoped_refptr<TableTypes::Columns> column_constraints = base::MakeRefCounted<TableTypes::Columns>();
    ColumnConstraintsBuilder constraints_builder(column_constraints.get(), is_fixed_layout);
    VisitLayoutTableColumn(columns, UINT_MAX, &constraints_builder);
    // Find last non-mergeable column.
    if (column_constraints->data.size() == 0)
        return 0;
    wtf_size_t column_index = column_constraints->data.size() - 1;
    while (column_index > 0 && column_constraints->data[column_index].is_mergeable) {
        --column_index;
    }
...
scoped_refptr<TableTypes::Columns> ComputeColumnConstraints(
    const BlockNode& table, const TableGroupedChildren& grouped_children, const TableBorders& table_borders, const BoxStrut& border_padding)
{
    ...
    scoped_refptr<TableTypes::Columns> column_constraints = base::MakeRefCounted<TableTypes::Columns>();
    ComputeColumnElementConstraints(grouped_children.columns, is_fixed_layout, column_constraints.get());
    ...
    for (BlockNode section : grouped_children) {
        if (!section.IsEmptyTableSection()) {
            ComputeSectionInlineConstraints(section, is_fixed_layout, is_first_section, table_style.GetWritingDirection(), table_borders, section_index,
    ...
    ApplyCellConstraintsToColumnConstraints(
        cell_inline_constraints, table_style.TableBorderSpacing().inline_size, is_fixed_layout, &colspan_cell_constraints, column_constraints.get());

    return column_constraints;
}

void ComputeSectionMinimumRowBlockSizes(const BlockNode& section, const LayoutUnit cell_percentage_inline_size, const bool is_table_block_size_specified,
    const Vector<TableColumnLocation>& column_locations, const TableBorders& table_borders, const LayoutUnit block_border_spacing, wtf_size_t section_index,
    bool treat_section_as_tbody, TableTypes::Sections* sections, TableTypes::Rows* rows, TableTypes::CellBlockConstraints* cell_block_constraints)
{
    ...
    for (BlockNode row = To<BlockNode>(section.FirstChild()); row; row = To<BlockNode>(row.NextSibling())) {
        colspan_cell_tabulator.StartRow();
        TableTypes::Row row_constraint = ComputeMinimumRowBlockSize(RowCountFunc, row, cell_percentage_inline_size, is_table_block_size_specified,
            column_locations, table_borders, start_row, current_row++, section_index,
            /* is_section_collapsed */ section.Style().Visibility() == EVisibility::kCollapse, cell_block_constraints, &rowspan_cells, &colspan_cell_tabulator);
        if (row_constraint.percent.has_value()) {
            row_constraint.percent = std::min(100.0f - total_row_percent, *row_constraint.percent);
    ...
    const wtf_size_t block_spacing_count = current_row == start_row ? 0 : current_row - start_row - 1;
    const LayoutUnit border_spacing_total = block_border_spacing * block_spacing_count;
    section_block_size += border_spacing_total;

    // Redistribute section's css block size.
    const Length& section_specified_block_length = section.Style().LogicalHeight();
    // TODO(1105272) Handle section_specified_block_length.IsCalculated()
    if (section_specified_block_length.IsFixed()) {
        LayoutUnit section_fixed_block_size = LayoutUnit(section_specified_block_length.Value());
        if (section_fixed_block_size > section_block_size) {
            DistributeSectionFixedBlockSizeToRows(
                start_row, current_row - start_row, section_fixed_block_size, block_border_spacing, section_fixed_block_size, rows);
            section_block_size = section_fixed_block_size;
        }
    }
    sections->push_back(TableTypes::CreateSection(section, start_row, current_row - start_row, section_block_size, treat_section_as_tbody));
}

void FinalizeTableCellLayout(LayoutUnit unconstrained_intrinsic_block_size, BoxFragmentBuilder* builder)
{
    ...
    if (!space.IsHiddenForPaint()) {
        builder->SetIsHiddenForPaint(space.HideTableCellIfEmpty() && !has_inflow_children);
    }
    ...
    LayoutUnit free_space = builder->FragmentBlockSize() - unconstrained_intrinsic_block_size;
    BlockContentAlignment alignment = ComputeContentAlignmentForTableCell(builder->Style(), &builder->Node().GetDocument());
    if (alignment == BlockContentAlignment::kSafeCenter || alignment == BlockContentAlignment::kSafeEnd) {
    ...
    case BlockContentAlignment::kBaseline:
        ...
        if (!builder->FirstBaseline() || node.ShouldApplyLayoutContainment()) {
            builder->SetBaselines(unconstrained_intrinsic_block_size - builder->BorderScrollbarPadding().block_end);
        }
        ...
        if (has_inflow_children) {
            if (auto alignment_baseline = space.TableCellAlignmentBaseline()) {
                builder->MoveChildrenInBlockDirection(*alignment_baseline - *builder->FirstBaseline());
            }
        }
        break;
...
void ColspanCellTabulator::EndRow()
{
    ...
    std::sort(colspanned_cells_.begin(), colspanned_cells_.end(),
        [](const ColspanCellTabulator::Cell& a, const ColspanCellTabulator::Cell& b) { return a.column_start < b.column_start; });
}
...
void ColspanCellTabulator::FindNextFreeColumn()
{
    for (const Cell& colspanned_cell : colspanned_cells_) {
        if (colspanned_cell.column_start <= current_column_ && colspanned_cell.column_start + colspanned_cell.span > current_column_) {
...
void RowBaselineTabulator::ProcessCell(
    const LogicalBoxFragment& fragment, BlockContentAlignment align, const bool is_rowspanned, const bool descendant_depends_on_percentage_block_size)
{
    if (align == BlockContentAlignment::kBaseline && fragment.HasDescendantsForTablePart() && fragment.FirstBaseline()) {
        max_cell_baseline_depends_on_percentage_block_descendant_ |= descendant_depends_on_percentage_block_size;
        const LayoutUnit cell_baseline = *fragment.FirstBaseline();
        max_cell_ascent_ = std::max(max_cell_ascent_.value_or(LayoutUnit::Min()), cell_baseline);
        if (is_rowspanned) {
            if (!max_cell_descent_)
                max_cell_descent_ = LayoutUnit();
        } else {
            max_cell_descent_ = std::max(max_cell_descent_.value_or(LayoutUnit::Min()), fragment.BlockSize() - cell_baseline);
        }
    }
    ...
    // the cell box."
    if (!max_cell_ascent_) {
        fallback_cell_depends_on_percentage_block_descendant_ |= descendant_depends_on_percentage_block_size;
        const LayoutUnit cell_block_end_border_padding = fragment.Padding().block_end + fragment.Borders().block_end;
        fallback_cell_descent_ = std::min(fallback_cell_descent_.value_or(LayoutUnit::Max()), cell_block_end_border_padding);
    }
}

LayoutUnit RowBaselineTabulator::ComputeRowBlockSize(const LayoutUnit max_cell_block_size)
{
    if (max_cell_ascent_) {
        return std::max(max_cell_block_size, *max_cell_ascent_ + *max_cell_descent_);
    }
    return max_cell_block_size;
}

LayoutUnit RowBaselineTabulator::ComputeBaseline(const LayoutUnit row_block_size)
{
    if (max_cell_ascent_)
...
MinMaxSizes ComputeGridInlineMinMax(
    const TableNode& node, const TableTypes::Columns& column_constraints, LayoutUnit undistributable_space, bool is_fixed_layout, bool is_layout_pass)
{
    MinMaxSizes min_max;
        ...
        if (column.min_inline_size) {
            ...
            if (column.percent && *column.percent > 0) {
                if (*column.max_inline_size > LayoutUnit()) {
                    LayoutUnit estimate = LayoutUnit(100 / *column.percent * (*column.max_inline_size - column.percent_border_padding));
                    percent_max_size_estimate = std::max(percent_max_size_estimate, estimate);
                }
            } else {
    ...
    if (percent_sum > 0 && node.AllowColumnPercentages(is_layout_pass)) {
        ...
        if (non_percent_max_size_sum != LayoutUnit()) {
            if (percent_sum == 100.0f) {
                ...
                size_from_percent_and_fixed = LayoutUnit((100 / (100 - percent_sum)) * non_percent_max_size_sum);
            }
        }
...
void DistributeColspanCellsToColumns(
    const TableTypes::ColspanCells& colspan_cells, LayoutUnit inline_border_spacing, bool is_fixed_layout, TableTypes::Columns* column_constraints)
{
    for (const TableTypes::ColspanCell& colspan_cell : colspan_cells) {
        ...
        if (is_fixed_layout) {
            DistributeColspanCellToColumnsFixed(colspan_cell, inline_border_spacing, column_constraints);
        } else {
            DistributeColspanCellToColumnsAuto(colspan_cell, inline_border_spacing, column_constraints);
        }
    }
}
...
Vector<LayoutUnit> SynchronizeAssignableTableInlineSizeAndColumns(
    LayoutUnit assignable_table_inline_size, bool is_fixed_layout, const TableTypes::Columns& column_constraints)
{
    if (column_constraints.data.empty()) {
        return Vector<LayoutUnit>();
    }
    if (is_fixed_layout) {
        return SynchronizeAssignableTableInlineSizeAndColumnsFixed(assignable_table_inline_size, column_constraints);
    } else {
        const TableTypes::Column* start_column = &column_constraints.data[0];
        const TableTypes::Column* end_column = start_column + column_constraints.data.size();
        return DistributeInlineSizeToComputedInlineSizeAuto(assignable_table_inline_size, start_column, end_column,
            /* treat_target_size_as_constrained */ true);
    }
}

void DistributeRowspanCellToRows(const TableTypes::RowspanCell& rowspan_cell, LayoutUnit border_block_spacing, TableTypes::Rows* rows)
{
    DCHECK_GT(rowspan_cell.effective_rowspan, 1u);
    DistributeExcessBlockSizeToRows(rowspan_cell.start_row, rowspan_cell.effective_rowspan, rowspan_cell.min_block_size,
        /* is_rowspan_distribution */ true, border_block_spacing, kIndefiniteSize, rows);
}

// Legacy code ignores section block size.
void DistributeSectionFixedBlockSizeToRows(const wtf_size_t start_row, const wtf_size_t rowspan, LayoutUnit section_fixed_block_size,
    LayoutUnit border_block_spacing, LayoutUnit percentage_resolution_block_size, TableTypes::Rows* rows)
{
    DistributeExcessBlockSizeToRows(start_row, rowspan, section_fixed_block_size,
        /* is_rowspan_distribution */ false, border_block_spacing, percentage_resolution_block_size, rows);
}

void DistributeTableBlockSizeToSections(LayoutUnit border_block_spacing, LayoutUnit table_block_size, TableTypes::Sections* sections, TableTypes::Rows* rows)
{
    if (sections->empty()) {
        return;
    }
    ...
    const LayoutUnit undistributable_space = (sections->size() + 1) * border_block_spacing;
    const LayoutUnit distributable_table_block_size = (table_block_size - undistributable_space).ClampNegativeToZero();

    auto ComputePercentageSize = [&distributable_table_block_size](auto& section) {
        DCHECK(section.percent.has_value());
        return std::max(section.block_size, LayoutUnit(*section.percent * distributable_table_block_size / 100));
    };
    ...
    for (wtf_size_t index = 0u; index < sections->size(); ++index) {
        ...
        percent_size_guess += section.percent ? ComputePercentageSize(section) : section.block_size;
    ...
    if (!percent_sections.empty() && percent_size_guess > minimum_size_guess) {
        const LayoutUnit distributable_size = std::min(percent_size_guess, distributable_table_block_size) - minimum_size_guess;
        DCHECK_GE(distributable_size, LayoutUnit());
        const LayoutUnit percent_minimum_difference = percent_size_guess - minimum_size_guess;

        LayoutUnit remaining_deficit = distributable_size;
        for (auto& index : percent_sections) {
            auto& section = sections->at(index);
            LayoutUnit delta = distributable_size.MulDiv(ComputePercentageSize(section) - section.block_size, percent_minimum_difference);
    ...
    LayoutUnit distributable_size = distributable_table_block_size - minimum_size_guess;
    ...
    for (TableTypes::Section& section : *sections) {
        ...
        DistributeExcessBlockSizeToRows(section.start_row, section.row_count, section.block_size,
            /* is_rowspan_distribution */ false, border_block_spacing, section.block_size, rows);
    }
}
