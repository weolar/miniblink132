...
TEST_F(LayoutTableTest, OverflowViaOutline)
{
    ...
    To<Element>(target->GetNode())->setAttribute(html_names::kStyleAttr, AtomicString("outline: 2px solid black"));

    auto* child = GetTableByElementId("child");
    To<Element>(child->GetNode())->setAttribute(html_names::kStyleAttr, AtomicString("outline: 2px solid black"));
...
TEST_F(LayoutTableTest, OverflowWithCollapsedBorders)
{
    ...
    expected_border_box_rect.ExpandEdges(LayoutUnit(2), LayoutUnit(10), LayoutUnit(0), LayoutUnit(10));
    ...
    expected_self_visual_overflow.ExpandEdges(LayoutUnit(2), LayoutUnit(10), LayoutUnit(0), LayoutUnit(10));
    ...
    expected_visual_overflow.ExpandEdges(LayoutUnit(6), LayoutUnit(10), LayoutUnit(8), LayoutUnit(10));
    EXPECT_EQ(expected_visual_overflow, table->VisualOverflowRect());
}

TEST_F(LayoutTableTest, CollapsedBorders)
{
    SetBodyInnerHTML("<style>table { border-collapse: collapse }</style>"
                     "<table id='table1'"
...
TEST_F(LayoutTableTest, OutOfOrderHeadAndBody)
{
    ...
    auto* table = GetTableByElementId("table");
    auto* body_section = To<LayoutTableSection>(GetLayoutObjectByElementId("body"));
    ASSERT_TRUE(table);
...
TEST_F(LayoutTableTest, OutOfOrderHeadFootAndBody)
{
    ...
    auto* table = GetTableByElementId("table");
    auto* head_section = To<LayoutTableSection>(GetLayoutObjectByElementId("head"));
    auto* body_section = To<LayoutTableSection>(GetLayoutObjectByElementId("body"));
    auto* foot_section = To<LayoutTableSection>(GetLayoutObjectByElementId("foot"));
    ASSERT_TRUE(table);
...
TEST_F(LayoutTableTest, VisualOverflowCleared)
{
    ...
    To<Element>(table->GetNode())->setAttribute(html_names::kStyleAttr, AtomicString("box-shadow: initial"));
    UpdateAllLifecyclePhasesForTest();
