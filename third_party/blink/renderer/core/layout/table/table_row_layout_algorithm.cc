...
TableRowLayoutAlgorithm::TableRowLayoutAlgorithm(const LayoutAlgorithmParams& params)
...
{
    const TableConstraintSpaceData& table_data = *GetConstraintSpace().TableData();
    const auto& row = table_data.rows[GetConstraintSpace().TableRowIndex()];

    auto CreateCellConstraintSpace
        = [this, &table_data](BlockNode cell, const BlockBreakToken* cell_break_token, const TableConstraintSpaceData::Cell& cell_data,
              LayoutUnit row_block_size, std::optional<LayoutUnit> row_baseline, bool min_block_size_should_encompass_intrinsic_size) {
              bool has_rowspan = cell_data.rowspan_block_size != kIndefiniteSize;
              LayoutUnit cell_block_size = has_rowspan ? cell_data.rowspan_block_size : row_block_size;

              if (IsBreakInside(cell_break_token) && IsBreakInside(GetBreakToken()) && !has_rowspan) {
                  ...
                  cell_block_size -= GetBreakToken()->ConsumedBlockSize() - cell_break_token->ConsumedBlockSize();
              }

              DCHECK_EQ(table_data.table_writing_direction.GetWritingMode(), GetConstraintSpace().GetWritingMode());

              ConstraintSpaceBuilder builder(GetConstraintSpace(), cell.Style().GetWritingDirection(),
                  /* is_new_fc */ true);

              SetupTableCellConstraintSpaceBuilder(table_data.table_writing_direction, cell, cell_data.borders, table_data.column_locations, cell_block_size,
                  container_builder_.InlineSize(), row_baseline, cell_data.start_column, cell_data.is_initial_block_size_indefinite,
                  table_data.is_table_block_size_specified, table_data.has_collapsed_borders, LayoutResultCacheSlot::kLayout, &builder);

              if (GetConstraintSpace().HasBlockFragmentation()) {
                  SetupSpaceBuilderForFragmentation(container_builder_, cell,
    ...
    bool should_propagate_child_break_values = GetConstraintSpace().ShouldPropagateChildBreakValues();

    auto MinBlockSizeShouldEncompassIntrinsicSize = [&](const BlockNode& cell, const TableConstraintSpaceData::Cell& cell_data) -> bool {
    ...
    auto PlaceCells = [&](LayoutUnit row_block_size, std::optional<LayoutUnit> row_baseline) {
        // Reset our state.
        ...
        for (auto entry = child_iterator.NextChild(); BlockNode cell = To<BlockNode>(entry.node); entry = child_iterator.NextChild()) {
            ...
            const TableConstraintSpaceData::Cell& cell_data = table_data.cells[cell_index];

            bool min_block_size_should_encompass_intrinsic_size = MinBlockSizeShouldEncompassIntrinsicSize(cell, cell_data);

            const auto cell_space
                = CreateCellConstraintSpace(cell, cell_break_token, cell_data, row_block_size, row_baseline, min_block_size_should_encompass_intrinsic_size);
            const LayoutResult* cell_result = cell.Layout(cell_space, cell_break_token);
            DCHECK_EQ(cell_result->Status(), LayoutResult::kSuccess);

            const LogicalOffset offset(table_data.column_locations[cell_data.start_column].offset - table_data.table_border_spacing.inline_size, LayoutUnit());
            ...
            if (should_propagate_child_break_values) {
                auto cell_break_before = JoinFragmentainerBreakValues(cell_style.BreakBefore(), cell_result->InitialBreakBefore());
                auto cell_break_after = JoinFragmentainerBreakValues(cell_style.BreakAfter(), cell_result->FinalBreakAfter());
                row_break_before = JoinFragmentainerBreakValues(row_break_before, cell_break_before);
                row_break_after = JoinFragmentainerBreakValues(row_break_after, cell_break_after);
            }

            bool has_rowspan = cell_data.rowspan_block_size != kIndefiniteSize;
            const auto& physical_fragment = To<PhysicalBoxFragment>(cell_result->GetPhysicalFragment());
            const LogicalBoxFragment fragment(table_data.table_writing_direction, physical_fragment);
            row_baseline_tabulator.ProcessCell(
                fragment, ComputeContentAlignmentForTableCell(cell_style), has_rowspan, cell_data.has_descendant_that_depends_on_percentage_block_size);
            if (min_block_size_should_encompass_intrinsic_size) {
                max_cell_block_size = std::max(max_cell_block_size, fragment.BlockSize());
            }

            if (const auto* outgoing_break_token = physical_fragment.GetBreakToken(); outgoing_break_token && !has_inflow_break_inside && !has_rowspan) {
    ...
    if (IsBreakInside(GetBreakToken())) {
        const auto* table_row_data = To<TableRowBreakTokenData>(GetBreakToken()->TokenData());
        previous_consumed_row_block_size = table_row_data->previous_consumed_row_block_size;
    }
    ...
    LayoutUnit row_block_size = max_cell_block_size + previous_consumed_row_block_size;
    ...
    if (InvolvedInBlockFragmentation(container_builder_)) [[unlikely]] {
        ...
        container_builder_.SetBreakTokenData(MakeGarbageCollected<TableRowBreakTokenData>(
            container_builder_.GetBreakTokenData(), previous_consumed_row_block_size + container_builder_.FragmentBlockSize()));
    }

    // NOTE: When we support "align-content: last baseline" for tables there may
    // be two baseline alignment contexts.
    container_builder_.SetBaselines(row_baseline_tabulator.ComputeBaseline(container_builder_.FragmentBlockSize()));
