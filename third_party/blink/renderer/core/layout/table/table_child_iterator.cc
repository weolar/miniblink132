...
TableChildIterator::TableChildIterator(const TableGroupedChildren& grouped_children, const BlockBreakToken* break_token)
    ...
    , break_token_(break_token)
{
    ...
    if (grouped_children_->captions.size()) {
        ...
        while (caption_idx_ < grouped_children_->captions.size()) {
            if (grouped_children_->captions[caption_idx_].Style().CaptionSide() == ECaptionSide::kTop)
                return;
            caption_idx_++;
        }
...
BlockNode TableChildIterator::CurrentChild() const
{
    ...
    if (!section_iterator_) {
        ...
        DCHECK_EQ(grouped_children_->captions[caption_idx_].Style().CaptionSide(), ECaptionSide::kTop);
    ...
    if (caption_idx_ < grouped_children_->captions.size()) {
        ...
        DCHECK_EQ(grouped_children_->captions[caption_idx_].Style().CaptionSide(), ECaptionSide::kBottom);
        return grouped_children_->captions[caption_idx_];
    }
...
void TableChildIterator::AdvanceChild()
{
    ...
    while (caption_idx_ < grouped_children_->captions.size()) {
        if (grouped_children_->captions[caption_idx_].Style().CaptionSide() == ECaptionSide::kBottom)
            return;
        caption_idx_++;
    }
}
