...
TableTypes::Caption ComputeCaptionConstraint(const ConstraintSpace& table_space, const ComputedStyle& table_style, const TableGroupedChildren& grouped_children)
{
    ...
    for (const BlockNode& caption : grouped_children.captions) {
        ...
        MinMaxSizes min_max_sizes = ComputeMinAndMaxContentContribution(table_style, caption, space).sizes;
        min_max_sizes += ComputeMarginsFor(space, caption.Style(), table_space).InlineSum();
        caption_min_max.Encompass(min_max_sizes);
    }
    return caption_min_max;
}

ConstraintSpace CreateCaptionConstraintSpace(const ConstraintSpace& table_constraint_space, const ComputedStyle& table_style, const BlockNode& caption,
    LogicalSize available_size, std::optional<LayoutUnit> block_offset = std::nullopt)
{
    ConstraintSpaceBuilder builder(table_constraint_space, caption.Style().GetWritingDirection(),
        /* is_new_fc */ true);
    ...
    if (block_offset && table_constraint_space.HasBlockFragmentation()) {
        SetupSpaceBuilderForFragmentation(table_constraint_space, caption, table_constraint_space.FragmentainerOffset() + *block_offset,
...
TableLayoutAlgorithm::CaptionResult LayoutCaption(const ConstraintSpace& table_constraint_space, const ComputedStyle& table_style, LayoutUnit table_inline_size,
    const ConstraintSpace& caption_constraint_space, const BlockNode& caption, BoxStrut margins, const BlockBreakToken* break_token = nullptr,
    const EarlyBreak* early_break = nullptr)
{
    const LayoutResult* layout_result = caption.Layout(caption_constraint_space, break_token, early_break);
    DCHECK_EQ(layout_result->Status(), LayoutResult::kSuccess);

    LogicalFragment fragment(table_constraint_space.GetWritingDirection(), layout_result->GetPhysicalFragment());
    ResolveInlineAutoMargins(caption.Style(), table_style, table_inline_size, fragment.InlineSize(), &margins);
...
// percentages.
BoxStrut ComputeCaptionMargins(const ConstraintSpace& table_constraint_space, const BlockNode& caption, LayoutUnit table_border_box_inline_size,
    const BlockBreakToken* caption_break_token = nullptr)
{
    BoxStrut margins = ComputeMarginsFor(caption.Style(), table_border_box_inline_size, table_constraint_space.GetWritingDirection());
    AdjustMarginsForFragmentation(caption_break_token, &margins);
    return margins;
}

void ComputeCaptionFragments(const BoxFragmentBuilder& table_builder, const ComputedStyle& table_style, const TableGroupedChildren& grouped_children,
    HeapVector<TableLayoutAlgorithm::CaptionResult>* captions, LayoutUnit& captions_block_size)
{
    const ConstraintSpace& table_constraint_space = table_builder.GetConstraintSpace();
    ...
    for (BlockNode caption : grouped_children.captions) {
        BoxStrut margins = ComputeCaptionMargins(table_constraint_space, caption, table_inline_size);
        ConstraintSpace caption_constraint_space = CreateCaptionConstraintSpace(table_constraint_space, table_style, caption, available_size);
        ...
        if ((!captions && !caption.GetLayoutBox()->NeedsLayout()) || InvolvedInBlockFragmentation(table_builder)) {
            disable_side_effects.emplace();
        }

        TableLayoutAlgorithm::CaptionResult caption_result
            = LayoutCaption(table_constraint_space, table_style, table_inline_size, caption_constraint_space, caption, margins);
        LogicalFragment fragment(table_constraint_space.GetWritingDirection(), caption_result.layout_result->GetPhysicalFragment());
        captions_block_size += fragment.BlockSize() + caption_result.margins.BlockSum();
        if (captions)
...
LayoutUnit ComputeUndistributableTableSpace(
    const TableTypes::Columns& column_constraints, LayoutUnit inline_table_border_padding, LayoutUnit inline_border_spacing)
{
    ...
    return inline_table_border_padding + inline_space_count * inline_border_spacing;
}

// Empty table sizes have been a source of many inconsistencies
// between browsers.
LayoutUnit ComputeEmptyTableInlineSize(const ConstraintSpace& space, const ComputedStyle& table_style, const LayoutUnit assignable_table_inline_size,
    const LayoutUnit undistributable_space, const TableTypes::Caption& caption_constraint, const BoxStrut& table_border_padding,
    const bool has_collapsed_borders)
{
    ...
    if (space.IsFixedInlineSize() || space.IsInlineAutoBehaviorStretch() || !table_style.LogicalWidth().IsAuto() || !table_style.LogicalMinWidth().IsAuto()) {
    ...
    if (caption_constraint.min_size) {
        return std::max(caption_constraint.min_size, table_border_padding.InlineSum());
    }
...
LayoutUnit ComputeAssignableTableInlineSize(const TableNode& table, const ConstraintSpace& space, const TableTypes::Columns& column_constraints,
    const TableTypes::Caption& caption_constraint, const LayoutUnit undistributable_space, const BoxStrut& table_border_padding, const bool is_fixed_layout)
{
    if (space.IsFixedInlineSize()) {
        return (space.AvailableSize().inline_size - undistributable_space).ClampNegativeToZero();
    }

    const MinMaxSizes grid_min_max = ComputeGridInlineMinMax(table, column_constraints, undistributable_space, is_fixed_layout,
        /* is_layout_pass */ true);

    // Standard: "used width of the table".
    LayoutUnit used_table_inline_size = ComputeUsedInlineSizeForTableFragment(space, table, table_border_padding, grid_min_max);
    ...
    used_table_inline_size = std::max(used_table_inline_size, caption_constraint.min_size);

    // Standard: The assignable table width is the "used width of the table"
    // minus the total horizontal border spacing.
    const LayoutUnit assignable_table_inline_size = used_table_inline_size - undistributable_space;
...
void ComputeLocationsFromColumns(const TableTypes::Columns& column_constraints, const Vector<LayoutUnit>& column_sizes, LayoutUnit inline_border_spacing,
    bool shrink_collapsed, Vector<TableColumnLocation>* column_locations, bool* has_collapsed_columns)
{
    *has_collapsed_columns = false;
    ...
    for (wtf_size_t i = 0; i < column_constraints.data.size(); ++i) {
        ...
        *has_collapsed_columns |= column_constraint.is_collapsed;
        if (column_constraints.data[i].is_mergeable && (column_sizes[i] == kIndefiniteSize || column_sizes[i] == LayoutUnit())) {
            ...
            column_location.offset = column_offset;
            column_location.size = column_sizes[i] != kIndefiniteSize ? column_sizes[i] : LayoutUnit();
...
scoped_refptr<const TableConstraintSpaceData> CreateConstraintSpaceData(const ComputedStyle& style, const Vector<TableColumnLocation>& column_locations,
    const TableTypes::Sections& sections, const TableTypes::Rows& rows, const TableTypes::CellBlockConstraints& cell_block_constraints,
    const LogicalSize& border_spacing)
{
    ...
    scoped_refptr<TableConstraintSpaceData> data = base::MakeRefCounted<TableConstraintSpaceData>();
    ...
    data->has_collapsed_borders = style.BorderCollapse() == EBorderCollapse::kCollapse;
    ...
    for (const auto& row : rows) {
        data->rows.emplace_back(row.block_size, row.start_cell_index, row.cell_count, row.baseline, row.is_collapsed);
    }
    ...
    for (const auto& section : sections) {
        for (wtf_size_t row_index = section.start_row; row_index < section.start_row + section.row_count; ++row_index) {
            const auto& row = rows[row_index];
            for (wtf_size_t cell_index = row.start_cell_index; cell_index < row.start_cell_index + row.cell_count; ++cell_index) {
                ...
                const auto [cell_block_size, is_initial_block_size_indefinite]
                    = ComputeCellBlockSize(cell_block_constraint, rows, row_index, border_spacing, is_table_block_size_specified);

                LayoutUnit rowspan_block_size = cell_block_constraint.effective_rowspan > 1 ? cell_block_size : kIndefiniteSize;

                data->cells.emplace_back(cell_block_constraint.borders, rowspan_block_size, cell_block_constraint.column_index,
                    is_initial_block_size_indefinite, cell_block_constraint.has_descendant_that_depends_on_percentage_block_size);
            }
        }
    }
    return data;
...
public:
    void VisitCol(const LayoutInputNode& col, wtf_size_t start_column_index, wtf_size_t span)
    {
        ...
        LayoutUnit column_inline_size
            = column_locations[end_column_index].offset + column_locations[end_column_index].size - column_locations[start_column_index].offset;

        column_geometries.emplace_back(
            start_column_index, span, column_locations[start_column_index].offset - column_locations[0].offset, column_inline_size, col);
    }
    ...
    void LeaveColgroup(const LayoutInputNode& colgroup, wtf_size_t start_column_index, wtf_size_t span, bool has_children)
    {
        ...
        LayoutUnit colgroup_size
            = column_locations[last_column_index].offset + column_locations[last_column_index].size - column_locations[start_column_index].offset;

        column_geometries.emplace_back(
            start_column_index, span, column_locations[start_column_index].offset - column_locations[0].offset, colgroup_size, colgroup);
    }
    ...
    void Sort()
    {
        ...
        std::sort(
            column_geometries.begin(), column_geometries.end(), [](const TableFragmentData::ColumnGeometry& a, const TableFragmentData::ColumnGeometry& b) {
        ...
        for (const auto& col : column_geometries) {
            To<LayoutTableColumn>(col.node.GetLayoutBox())->SetColumnIndex(column_idx);
            column_idx++;
        }
    }

    ColumnGeometriesBuilder(const Vector<TableColumnLocation>& column_locations, LayoutUnit table_column_block_size)
...
LayoutUnit ComputeTableSizeFromColumns(
    const Vector<TableColumnLocation>& column_locations, const BoxStrut& table_border_padding, const LogicalSize& border_spacing)
{
    return column_locations.back().offset + column_locations.back().size + table_border_padding.InlineSum() + border_spacing.inline_size;
}
...
TableBoxExtent BeginTableBoxLayout(LayoutUnit block_start_border_edge, LayoutUnit table_border_padding_block_start)
{
    return { block_start_border_edge, block_start_border_edge + table_border_padding_block_start };
}

// Call when done with layout of the table box (typically right after having
// laid out the last table section).
LayoutUnit EndTableBoxLayout(LayoutUnit table_border_padding_block_end, LayoutUnit border_spacing_after_last_section, LayoutUnit minimal_table_grid_block_size,
    TableBoxExtent* extent, LayoutUnit* grid_block_size_inflation)
{
    DCHECK_LE(extent->start, extent->end);
    extent->end += border_spacing_after_last_section + table_border_padding_block_end;
    LayoutUnit sections_total_size = extent->end - extent->start;
    LayoutUnit grid_block_size = std::max(sections_total_size, minimal_table_grid_block_size);
...
LayoutUnit TableLayoutAlgorithm::ComputeTableInlineSize(const TableNode& table, const ConstraintSpace& space, const BoxStrut& table_border_padding)
{
    ...
    const scoped_refptr<const TableTypes::Columns> column_constraints = table.GetColumnConstraints(grouped_children, table_border_padding);

    const TableTypes::Caption caption_constraint = ComputeCaptionConstraint(space, table.Style(), grouped_children);

    const LayoutUnit undistributable_space
        = ComputeUndistributableTableSpace(*column_constraints, table_border_padding.InlineSum(), border_spacing.inline_size);

    const LayoutUnit assignable_table_inline_size
        = ComputeAssignableTableInlineSize(table, space, *column_constraints, caption_constraint, undistributable_space, table_border_padding, is_fixed_layout);
    if (column_constraints->data.empty()) {
        return ComputeEmptyTableInlineSize(
            space, table.Style(), assignable_table_inline_size, undistributable_space, caption_constraint, table_border_padding, table_borders->IsCollapsed());
    }

    const Vector<LayoutUnit> column_sizes = SynchronizeAssignableTableInlineSizeAndColumns(assignable_table_inline_size, is_fixed_layout, *column_constraints);
    ...
    ComputeLocationsFromColumns(*column_constraints, column_sizes, border_spacing.inline_size,
        /* collapse_columns */ true, &column_locations, &has_collapsed_columns);
    return std::max(ComputeTableSizeFromColumns(column_locations, table_border_padding, border_spacing), caption_constraint.min_size);
}
...
void TableLayoutAlgorithm::SetupRelayoutData(const TableLayoutAlgorithm& previous, RelayoutType relayout_type)
{
...
const LayoutResult* TableLayoutAlgorithm::Layout()
{
    ...
    // - Generate fragment.
    const scoped_refptr<const TableTypes::Columns> column_constraints = Node().GetColumnConstraints(grouped_children, border_padding);
    const TableTypes::Caption caption_constraint = ComputeCaptionConstraint(GetConstraintSpace(), Style(), grouped_children);
    // Compute assignable table inline size.
    // Standard: https://www.w3.org/TR/css-tables-3/#width-distribution
    const LayoutUnit undistributable_space = ComputeUndistributableTableSpace(*column_constraints, border_padding.InlineSum(), border_spacing.inline_size);

    const LayoutUnit assignable_table_inline_size = ComputeAssignableTableInlineSize(
        Node(), GetConstraintSpace(), *column_constraints, caption_constraint, undistributable_space, border_padding, is_fixed_layout);
    ...
    if (is_grid_empty) {
        table_inline_size_before_collapse = ComputeEmptyTableInlineSize(GetConstraintSpace(), Style(), assignable_table_inline_size, undistributable_space,
            caption_constraint, border_padding, table_borders->IsCollapsed());
    } else {
        table_inline_size_before_collapse = ComputeTableSizeFromColumns(column_locations, border_padding, border_spacing);
    }
    ...
    LayoutUnit captions_block_size;
    ComputeCaptionFragments(container_builder_, Style(), grouped_children, &captions, captions_block_size);
    ...
    ComputeRows(table_inline_size_before_collapse - border_padding.InlineSum(), grouped_children, column_locations, *table_borders, border_spacing,
        border_padding, captions_block_size, &rows, &cell_block_constraints, &sections, &minimal_table_grid_block_size);
    ...
    if (!has_collapsed_columns) {
        ...
        DCHECK_LT((table_inline_size_before_collapse - container_builder_.InlineSize()).Abs(), LayoutUnit(1));
        ...
        LayoutUnit table_inline_size = std::max(ComputeTableSizeFromColumns(column_locations, border_padding, border_spacing), caption_constraint.min_size);
        DCHECK_LE(table_inline_size, container_builder_.InlineSize());
    } else {
        LayoutUnit table_inline_size = std::max(ComputeTableSizeFromColumns(column_locations, border_padding, border_spacing), caption_constraint.min_size);
        DCHECK_LT((table_inline_size - container_builder_.InlineSize()).Abs(), LayoutUnit(1));
    }
#endif

    const LayoutResult* result = GenerateFragment(container_builder_.InlineSize(), minimal_table_grid_block_size, grouped_children, column_locations, rows,
        cell_block_constraints, sections, captions, *table_borders, is_grid_empty ? LogicalSize() : border_spacing);
    ...
    if (result->Status() == LayoutResult::kNeedsEarlierBreak) {
        ...
        return RelayoutAndBreakEarlier<TableLayoutAlgorithm>(*result->GetEarlyBreak());
    }

    return result;
}

MinMaxSizesResult TableLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    const bool is_fixed_layout = Style().IsFixedTableLayout();
    // Tables need autosizer.
    std::optional<TextAutosizer::TableLayoutScope> text_autosizer;
    ...
    const MinMaxSizes grid_min_max = ComputeGridInlineMinMax(Node(), *column_constraints, undistributable_space, is_fixed_layout,
        /* is_layout_pass */ false);

    MinMaxSizes min_max { std::max(grid_min_max.min_size, caption_constraint.min_size), std::max(grid_min_max.max_size, caption_constraint.min_size) };
...
void TableLayoutAlgorithm::ComputeRows(const LayoutUnit table_grid_inline_size, const TableGroupedChildren& grouped_children,
    const Vector<TableColumnLocation>& column_locations, const TableBorders& table_borders, const LogicalSize& border_spacing,
    const BoxStrut& table_border_padding, const LayoutUnit captions_block_size, TableTypes::Rows* rows,
    TableTypes::CellBlockConstraints* cell_block_constraints, TableTypes::Sections* sections, LayoutUnit* minimal_table_grid_block_size)
{
    ...
    if (GetBreakToken()) {
        table_break_data = DynamicTo<TableBreakTokenData>(GetBreakToken()->TokenData());
    }
    if (table_break_data) {
        ...
        *sections = table_break_data->sections;
        ...
        for (auto it = grouped_children.begin(); it != grouped_children.end(); ++it) {
            ComputeSectionMinimumRowBlockSizes(*it, table_grid_inline_size, is_table_block_size_specified, column_locations, table_borders,
                border_spacing.block_size, section_index++, it.TreatAsTBody(), sections, rows, cell_block_constraints);
    ...
    if (space.IsInitialBlockSizeIndefinite() && !space.IsFixedBlockSize()) {
        ...
        const LayoutUnit intrinsic_block_size = min_length.HasAuto()
                || ResolveMainBlockLength(space, Style(), table_border_padding, min_length, /* auto_length */ nullptr, kIndefiniteSize) == kIndefiniteSize
            ? kIndefiniteSize
        ...
        if (space.AvailableSize().block_size != kIndefiniteSize) {
            override_available_block_size = (space.AvailableSize().block_size - captions_block_size).ClampNegativeToZero();
        }

        css_table_block_size
            = ComputeBlockSizeForFragment(space, Node(), table_border_padding, intrinsic_block_size, table_grid_inline_size, override_available_block_size);
    }
    // In quirks mode, empty tables ignore any specified block-size.
    const bool is_empty_quirks_mode_table = Node().GetDocument().InQuirksMode() && grouped_children.begin() == grouped_children.end();
    ...
    if (css_table_block_size != kIndefiniteSize && !is_empty_quirks_mode_table) {
        *minimal_table_grid_block_size = css_table_block_size;
        LayoutUnit distributable_block_size = std::max(LayoutUnit(), css_table_block_size - table_border_padding.BlockSum());
        if (distributable_block_size > total_table_min_block_size_) {
            DistributeTableBlockSizeToSections(border_spacing.block_size, distributable_block_size, sections, rows);
        }
    }
...
void TableLayoutAlgorithm::ComputeTableSpecificFragmentData(const TableGroupedChildren& grouped_children, const Vector<TableColumnLocation>& column_locations,
    const TableTypes::Rows& rows, const TableBorders& table_borders, const LogicalRect& table_grid_rect, const LayoutUnit table_grid_block_size)
{
    ...
    if (grouped_children.columns.size() > 0) {
        ColumnGeometriesBuilder geometry_builder(column_locations, table_grid_block_size);
        VisitLayoutTableColumn(grouped_children.columns, column_locations.size(), &geometry_builder);
        geometry_builder.Sort();
        container_builder_.SetTableColumnGeometries(geometry_builder.column_geometries);
    }
    // Collapsed borders.
    if (!table_borders.IsEmpty()) {
        std::unique_ptr<TableFragmentData::CollapsedBordersGeometry> fragment_borders_geometry
        ...
        fragment_borders_geometry->columns.push_back(column_locations.back().offset + column_locations.back().size);

        // Ensure the dimensions of table_borders and fragment_borders_geometry are
        // consistent.
        DCHECK_LE(table_borders.EdgesPerRow() / 2, fragment_borders_geometry->columns.size());

        container_builder_.SetTableCollapsedBorders(table_borders);
        container_builder_.SetTableCollapsedBordersGeometry(std::move(fragment_borders_geometry));
    }
}
...
// +--------------------------------+
const LayoutResult* TableLayoutAlgorithm::GenerateFragment(const LayoutUnit table_inline_size, LayoutUnit minimal_table_grid_block_size,
    const TableGroupedChildren& grouped_children, const Vector<TableColumnLocation>& column_locations, const TableTypes::Rows& rows,
    const TableTypes::CellBlockConstraints& cell_block_constraints, const TableTypes::Sections& sections, const HeapVector<CaptionResult>& captions,
    const TableBorders& table_borders, const LogicalSize& border_spacing)
{
    const TableBreakTokenData* incoming_table_break_data = nullptr;
    LogicalBoxSides border_padding_sides_to_include;
    const auto& constraint_space = GetConstraintSpace();
    const LayoutUnit fragmentainer_space_at_start = FragmentainerSpaceLeftForChildren();
    ...
    if (GetBreakToken()) {
        ...
        incoming_table_break_data = DynamicTo<TableBreakTokenData>(GetBreakToken()->TokenData());
        if (incoming_table_break_data) {
            previously_consumed_table_box_block_size = incoming_table_break_data->consumed_table_box_block_size;
            minimal_table_grid_block_size -= incoming_table_break_data->consumed_table_box_block_size;
            ...
            if (incoming_table_break_data->has_entered_table_box) {
                ...
                if (Style().BoxDecorationBreak() == EBoxDecorationBreak::kSlice || is_past_table_box) {
    ...
    scoped_refptr<const TableConstraintSpaceData> constraint_space_data
        = CreateConstraintSpaceData(Style(), column_locations, sections, rows, cell_block_constraints, border_spacing);
    ...
    auto AddCaptionResult = [&](const CaptionResult& caption, LayoutUnit* block_offset) -> void {
        *block_offset += caption.margins.block_start;
        container_builder_.AddResult(*caption.layout_result, LogicalOffset(caption.margins.inline_start, *block_offset), caption.margins);

        *block_offset += LogicalFragment(table_writing_direction, caption.layout_result->GetPhysicalFragment()).BlockSize() + caption.margins.block_end;
    };
    ...
    // resume properly.
    const bool relayout_captions = InvolvedInBlockFragmentation(container_builder_);
    ...
    // Section setup.
    const LayoutUnit section_available_inline_size = (table_inline_size - border_padding.InlineSum() - border_spacing.inline_size * 2).ClampNegativeToZero();

    enum ESectionRepeatMode { kNotRepeated, kMayRepeatAgain, kRepeatedLast };

    auto CreateSectionConstraintSpace
        = [&table_writing_direction, &section_available_inline_size, &constraint_space_data, &sections, this](const BlockNode& section,
              LayoutUnit fragmentainer_block_offset, wtf_size_t section_index, LayoutUnit reserved_space, ESectionRepeatMode repeat_mode) {
              ConstraintSpaceBuilder section_space_builder(GetConstraintSpace(), table_writing_direction, /* is_new_fc */ true);

              LogicalSize available_size = { section_available_inline_size, kIndefiniteSize };
              ...
              section_space_builder.SetPercentageResolutionSize({ section_available_inline_size, kIndefiniteSize });
              section_space_builder.SetTableSectionData(constraint_space_data, section_index);
              ...
              if (repeat_mode != kNotRepeated) {
                  ...
                  // doesn't fit).
                  SetupSpaceBuilderForFragmentation(container_builder_, section, fragmentainer_block_offset, &section_space_builder);
    ...
    auto BlockStartBorderPadding = [&border_padding, &border_padding_sides_to_include]() {
        if (border_padding_sides_to_include.block_start)
            return border_padding.block_start;
        return LayoutUnit();
    };

    const LayoutUnit section_inline_offset = border_padding.inline_start + border_spacing.inline_size;
    ...
    if (!GetConstraintSpace().IsInsideRepeatableContent() && !DisableLayoutSideEffectsScope::IsDisabled()
        && (grouped_children.header || grouped_children.footer)) {
        LayoutUnit max_section_block_size = GetConstraintSpace().FragmentainerBlockSize() / 4;
        TableChildIterator child_iterator(grouped_children, GetBreakToken());
        for (auto entry = child_iterator.NextChild(); BlockNode child = entry.GetNode(); entry = child_iterator.NextChild()) {
            ...
            if (!child_break_token || !child_break_token->IsRepeated()) {
                ...
                if (incoming_table_break_data && incoming_table_break_data->has_entered_table_box)
                    continue;
                ...
                if (!GetConstraintSpace().HasKnownFragmentainerBlockSize() || block_size > max_section_block_size) {
                    continue;
                }

                if (!IsAvoidBreakValue(GetConstraintSpace(), child.Style().BreakInside())) {
                    continue;
                }
            }
            ...
            if (child == grouped_children.header) {
                ...
                // fragmentainer.
                repeated_footer_block_size = block_size + border_spacing.block_size + (has_collapsed_borders ? border_padding.block_end : LayoutUnit());
            }
        }
    }
    ...
    for (auto entry = child_iterator.NextChild(); BlockNode child = entry.GetNode(); entry = child_iterator.NextChild()) {
        ...
        if (child.IsTableCaption()) {
            if (!relayout_captions)
                continue;
            if (child.Style().CaptionSide() == ECaptionSide::kBottom && !is_past_table_box) {
                ...
                if (!table_box_extent) {
                    ...
                    table_box_extent = BeginTableBoxLayout(child_block_offset, BlockStartBorderPadding());
                }

                child_block_offset = EndTableBoxLayout(border_padding.block_end, border_spacing_after_last_section, minimal_table_grid_block_size,
                    &(*table_box_extent), &grid_block_size_inflation);
                has_ended_table_box_layout = true;

                // We're done with the table box if it fits inside the fragmentainer.
                is_past_table_box = !constraint_space.HasKnownFragmentainerBlockSize() || table_box_extent->end <= fragmentainer_space_at_start;
            }

            LogicalSize available_size(container_builder_.InlineSize(), kIndefiniteSize);
            BoxStrut margins = ComputeCaptionMargins(constraint_space, child, container_builder_.InlineSize(), child_break_token);
            child_block_start_margin = margins.block_start;
            child_block_end_margin = margins.block_end;

            ConstraintSpace child_space
                = CreateCaptionConstraintSpace(constraint_space, Style(), child, available_size, child_block_offset + child_block_start_margin);
            CaptionResult caption = LayoutCaption(
                constraint_space, Style(), container_builder_.InlineSize(), child_space, child, margins, child_break_token, early_break_in_child);
            ...
            if (table_box_extent) {
                ...
                // box" starts.
                new_table_box_extent = BeginTableBoxLayout(child_block_offset, BlockStartBorderPadding());
            ...
            if (child == grouped_children.header) {
                if (has_repeated_header) {
                    ...
                    if (has_collapsed_borders && !border_padding_sides_to_include.block_start)
                ...
                if (has_pending_repeated_footer) {
                    ...
                    // relayout).
                    has_pending_repeated_footer = false;
                    may_repeat_again = !is_known_to_be_last_table_box_ && has_repeated_header;
                }
            }
            ...
            if (repeat_mode == kNotRepeated) {
                reserved_space = repeated_header_block_size + repeated_footer_block_size;
            }

            ConstraintSpace child_space
                = CreateSectionConstraintSpace(child, child_block_offset - repeated_header_block_size, entry.GetSectionIndex(), reserved_space, repeat_mode);
            if (is_repeated_section) {
                child_result = child.LayoutRepeatableRoot(child_space, child_break_token);
            } else {
                child_result = child.Layout(child_space, child_break_token, early_break_in_child);
            }
            child_inline_offset = section_inline_offset;

            border_spacing_after_last_section = border_spacing.block_size;
            if (To<PhysicalBoxFragment>(child_result->GetPhysicalFragment()).HasDescendantsForTablePart()) {
        ...
        if (constraint_space.HasBlockFragmentation() && (!child_break_token || !is_repeated_section)) {
            LayoutUnit fragmentainer_block_offset
                = FragmentainerOffsetForChildren() + child_block_start_margin + child_block_offset - repeated_header_block_size;
            BreakStatus break_status = BreakBeforeChildIfNeeded(child, *child_result, fragmentainer_block_offset, has_container_separation);
        ...
        const auto& physical_fragment = To<PhysicalBoxFragment>(child_result->GetPhysicalFragment());
        ...
        container_builder_.AddResult(*child_result, LogicalOffset(child_inline_offset, child_block_offset));
        child_block_offset += fragment.BlockSize() + child_block_end_margin;

        if (child.IsTableSection()) {
            if (offset_before_repeated_header) {
                repeated_header_block_size = child_block_offset - *offset_before_repeated_header + border_spacing.block_size;
                ...
                if (has_overlapping_repeated_header) {
                    ...
                    child_block_offset = std::max(child_block_offset, *offset_before_repeated_header);
                }
            }
    ...
    bool table_box_will_continue = table_box_extent && !is_past_table_box && broke_inside;
    ...
    if (has_pending_repeated_footer && table_box_extent) {
        ...
        for (; BlockNode child = entry.GetNode(); entry = child_iterator.NextChild()) {
            if (child == grouped_children.footer)
                break;

            auto* token = BlockBreakToken::CreateBreakBefore(child, /* is_forced_break */ false);
        ...
        if (has_entered_non_repeated_section) {
            ...
            LayoutUnit footer_offset_at_end_of_page
                = FragmentainerCapacityForChildren() - GetConstraintSpace().FragmentainerOffset() - repeated_footer_block_size;
            adjusted_child_block_offset = std::min(adjusted_child_block_offset, footer_offset_at_end_of_page);
        }

        LogicalOffset offset(section_inline_offset, adjusted_child_block_offset);
        ConstraintSpace child_space = CreateSectionConstraintSpace(grouped_children.footer, offset.block_offset, entry.GetSectionIndex(),
            /* reserved_space */ LayoutUnit(), kMayRepeatAgain);
        const LayoutResult* result = grouped_children.footer.LayoutRepeatableRoot(child_space, entry.GetBreakToken());
        ...
        if (!entry.GetBreakToken() || entry.GetBreakToken()->IsBreakBefore()) {
            ...
            // necessary.
            LayoutUnit fragmentainer_block_offset = FragmentainerOffsetForChildren() + offset.block_offset;
            break_status = BreakBeforeChildIfNeeded(grouped_children.footer, *result, fragmentainer_block_offset, has_container_separation);
        }
    ...
    if (table_box_extent && !is_past_table_box) {
        ...
        if (broke_inside) {
            if (constraint_space.HasKnownFragmentainerBlockSize()) {
                table_box_extent->end = std::max(table_box_extent->end, fragmentainer_space_at_start);
            }
            border_spacing_after_last_section = LayoutUnit();
            ...
            LayoutUnit new_border_spacing_after_last_section
                = std::min(border_spacing_after_last_section, fragmentainer_space_at_start - child_block_offset - border_padding.block_end);
            new_border_spacing_after_last_section = new_border_spacing_after_last_section.ClampNegativeToZero();
            if (border_spacing_after_last_section != new_border_spacing_after_last_section) {
                container_builder_.SetIsTruncatedByFragmentationLine();
                border_spacing_after_last_section = new_border_spacing_after_last_section;
            }
        }

        if (!has_ended_table_box_layout) {
            child_block_offset = EndTableBoxLayout(
                border_padding.block_end, border_spacing_after_last_section, minimal_table_grid_block_size, &(*table_box_extent), &grid_block_size_inflation);
    ...
    if (!table_box_extent)
        border_padding_sides_to_include.block_start = false;
    if (!is_past_table_box && (!container_builder_.ShouldCloneBoxEndDecorations() || !table_box_extent)) {
        border_padding_sides_to_include.block_end = false;
    }
    ...
    if (!has_ended_table_box_layout && !is_past_table_box) {
        ...
        LayoutUnit table_block_end_fluff = border_padding.block_end + border_spacing_after_last_section;
    ...
    if (constraint_space.IsFixedBlockSize()) {
        ...
        if (constraint_space.MinBlockSizeShouldEncompassIntrinsicSize()) {
            block_size = std::max(block_size, previously_consumed_block_size + intrinsic_block_size);
        }
    }
    container_builder_.SetFragmentsTotalBlockSize(block_size);

    if (Node().GetDOMNode() && Node().GetDOMNode()->HasTagName(mathml_names::kMtableTag)) {
        container_builder_.SetBaselines(MathTableBaseline(Style(), child_block_offset));
    } else {
        if (first_baseline)
    ...
    if (InvolvedInBlockFragmentation(container_builder_)) [[unlikely]] {
        ...
        if (table_box_extent) {
            table_box_extent->end = std::min(table_box_extent->end, container_builder_.FragmentBlockSize());
        }
    }
    ...
    if (table_box_extent) {
        ...
        if (!table_box_will_continue) {
            ...
            column_block_size = previously_consumed_table_box_block_size + grid_block_size;
            // Subtract first and last border-spacing, and table border/padding.
            column_block_size -= border_spacing.block_size * 2 + border_padding.BlockSum();
        }

        table_grid_rect = LogicalRect(LayoutUnit(), table_box_extent->start, container_builder_.InlineSize(), grid_block_size);
    }

    bool has_entered_table_box = false;
    if (InvolvedInBlockFragmentation(container_builder_)) {
        LayoutUnit consumed_table_box_block_size = previously_consumed_table_box_block_size;
        if (incoming_table_break_data)
        ...
        container_builder_.SetBreakTokenData(MakeGarbageCollected<TableBreakTokenData>(container_builder_.GetBreakTokenData(), rows, cell_block_constraints,
            sections, total_table_min_block_size_, consumed_table_box_block_size, has_entered_table_box, is_past_table_box));
    }

    ComputeTableSpecificFragmentData(grouped_children, column_locations, rows, table_borders, table_grid_rect, column_block_size);

    container_builder_.HandleOofsAndSpecialDescendants();

    if ((has_repeated_header || container_builder_.ShouldCloneBoxEndDecorations()) && has_entered_table_box && !table_box_will_continue
