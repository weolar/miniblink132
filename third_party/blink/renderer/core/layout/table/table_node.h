...
public:
    explicit TableNode(LayoutBox* box)
        : BlockNode(box)
    {
    }
    ...
    scoped_refptr<const TableTypes::Columns> GetColumnConstraints(const TableGroupedChildren&, const BoxStrut& border_padding) const;

    LayoutUnit ComputeTableInlineSize(const ConstraintSpace&, const BoxStrut& border_padding) const;
...
template <> struct DowncastTraits<TableNode> {
