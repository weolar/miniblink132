...
//                      bool has_children);
// }
template <typename Visitor> void VisitLayoutTableColumn(const HeapVector<BlockNode>& columns, wtf_size_t table_column_count, Visitor* visitor)
{
    ...
    for (const BlockNode& table_column : columns) {
        ...
        if (col_child) {
            ...
            colgroup_span = std::min(table_column.TableColumnSpan(), table_column_count - current_column_index);
            current_column_index += colgroup_span;
        }
        visitor->LeaveColgroup(table_column, colgroup_start_index, colgroup_span, has_children);
    }
}
