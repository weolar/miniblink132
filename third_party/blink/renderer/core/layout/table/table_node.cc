...
scoped_refptr<const TableTypes::Columns> TableNode::GetColumnConstraints(const TableGroupedChildren& grouped_children, const BoxStrut& border_padding) const
{
    auto* layout_table = To<LayoutTable>(box_.Get());
    scoped_refptr<const TableTypes::Columns> column_constraints = layout_table->GetCachedTableColumnConstraints();
    if (!column_constraints) {
        column_constraints = ComputeColumnConstraints(*this, grouped_children, *GetTableBorders(), border_padding);
        layout_table->SetCachedTableColumnConstraints(column_constraints.get());
    }
    return column_constraints;
}

LayoutUnit TableNode::ComputeTableInlineSize(const ConstraintSpace& space, const BoxStrut& border_padding) const
{
    return TableLayoutAlgorithm::ComputeTableInlineSize(*this, space, border_padding);
}

LayoutUnit TableNode::ComputeCaptionBlockSize(const ConstraintSpace& space) const
{
    FragmentGeometry geometry = CalculateInitialFragmentGeometry(space, *this, /* break_token */ nullptr);
...
bool TableNode::AllowColumnPercentages(bool is_layout_pass) const
{
    ...
    while (!block->IsLayoutView()) {
        if (block->IsTableCell() || block->IsFlexibleBox() || block->IsLayoutGrid()) {
            return false;
        }
