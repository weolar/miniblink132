...
public:
    ...
    void SetCachedTableColumnConstraints(scoped_refptr<const TableTypes::Columns>);
    ...
    PhysicalRect OverflowClipRect(const PhysicalOffset&, OverlayScrollbarClipBehavior) const override;
    ...
    bool ForegroundIsKnownToBeOpaqueInRect(const PhysicalRect& local_rect, unsigned max_depth_to_test) const override
    {
        NOT_DESTROYED();
        return false;
    }
    ...
    unsigned AbsoluteColumnToEffectiveColumn(unsigned absolute_column_index) const;
...
template <> struct DowncastTraits<LayoutTable> {
