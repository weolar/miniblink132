...
TableSectionLayoutAlgorithm::TableSectionLayoutAlgorithm(const LayoutAlgorithmParams& params)
...
const LayoutResult* TableSectionLayoutAlgorithm::Layout()
{
    ...
    const auto& section = table_data.sections[constraint_space.TableSectionIndex()];
    const wtf_size_t start_row_index = section.start_row_index;
    const LogicalSize available_size = { container_builder_.InlineSize(), kIndefiniteSize };
    ...
    for (auto entry = child_iterator.NextChild(); BlockNode row = To<BlockNode>(entry.node); entry = child_iterator.NextChild()) {
        ...
        if (early_break_ && IsEarlyBreakTarget(*early_break_, container_builder_, row)) [[unlikely]] {
        ...
        DCHECK_EQ(table_data.table_writing_direction.GetWritingMode(), constraint_space.GetWritingMode());

        ConstraintSpaceBuilder row_space_builder(constraint_space, table_data.table_writing_direction,
            /* is_new_fc */ true);
        ...
        if (constraint_space.HasBlockFragmentation()) {
            SetupSpaceBuilderForFragmentation(container_builder_, row, offset.block_offset, &row_space_builder);
        }
        ...
        if (constraint_space.HasBlockFragmentation()) {
            ...
            BreakStatus break_status = BreakBeforeChildIfNeeded(row, *row_result, fragmentainer_block_offset, !is_first_non_collapsed_row);
            if (break_status == BreakStatus::kNeedsEarlierBreak) {
                return RelayoutAndBreakEarlier<TableSectionLayoutAlgorithm>(container_builder_.GetEarlyBreak());
            }
        ...
        const auto& physical_fragment = To<PhysicalBoxFragment>(row_result->GetPhysicalFragment());
        ...
        if (table_data.has_collapsed_borders && (!row_break_token || !row_break_token->IsAtBlockEnd())) {
    ...
    if (table_data.has_collapsed_borders && row_offsets.size() > 1u) {
        container_builder_.SetTableSectionCollapsedBordersGeometry(actual_start_row_index, std::move(row_offsets));
    }
