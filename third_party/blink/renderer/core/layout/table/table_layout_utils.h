...
void SetupTableCellConstraintSpaceBuilder(const WritingDirectionMode table_writing_direction, const BlockNode cell, const BoxStrut& cell_borders,
    ...
    std::optional<LayoutUnit> alignment_baseline, wtf_size_t start_column, bool is_initial_block_size_indefinite, bool is_restricted_block_size_table,
    bool has_collapsed_borders, LayoutResultCacheSlot, ConstraintSpaceBuilder*);
...
scoped_refptr<TableTypes::Columns> ComputeColumnConstraints(
    const BlockNode& table, const TableGroupedChildren&, const TableBorders& table_borders, const BoxStrut& border_padding);

void ComputeSectionMinimumRowBlockSizes(const BlockNode& section, const LayoutUnit cell_percentage_resolution_inline_size,
    ...
    const LayoutUnit block_border_spacing, wtf_size_t section_index, bool treat_section_as_tbody, TableTypes::Sections* sections, TableTypes::Rows* rows,
...
void FinalizeTableCellLayout(LayoutUnit unconstrained_intrinsic_block_size, BoxFragmentBuilder*);
...
public:
    void ProcessCell(const LogicalBoxFragment& fragment, BlockContentAlignment align, bool is_rowspanned, bool descendant_depends_on_percentage_block_size);
...
// and its colspan.
constexpr wtf_size_t ComputeMaxColumn(wtf_size_t current_column, wtf_size_t colspan, bool is_fixed_table_layout)
{
...
CORE_EXPORT MinMaxSizes ComputeGridInlineMinMax(
...
CORE_EXPORT void DistributeRowspanCellToRows(const TableTypes::RowspanCell& rowspan_cell, LayoutUnit border_block_spacing, TableTypes::Rows* rows);

CORE_EXPORT void DistributeSectionFixedBlockSizeToRows(const wtf_size_t start_row, const wtf_size_t end_row, LayoutUnit section_fixed_block_size,
...
CORE_EXPORT void DistributeTableBlockSizeToSections(
    LayoutUnit border_block_spacing, LayoutUnit table_block_size, TableTypes::Sections* sections, TableTypes::Rows* rows);

} // namespace blink

WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::ColspanCellTabulator::Cell)
