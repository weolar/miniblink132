...
inline bool NeedsTableSection(const LayoutObject& object)
{
    ...
    return display != EDisplay::kTableCaption && display != EDisplay::kTableColumnGroup && display != EDisplay::kTableColumn;
}
...
LayoutTable* LayoutTable::CreateAnonymousWithParent(const LayoutObject& parent)
{
    ...
    const ComputedStyle* new_style = parent.GetDocument().GetStyleResolver().CreateAnonymousStyleWithDisplay(
        parent_style, ShouldCreateInlineAnonymous(parent) ? EDisplay::kInlineTable : EDisplay::kTable);
...
bool LayoutTable::IsFirstCell(const LayoutTableCell& cell) const
{
    ...
    TableGroupedChildren grouped_children(BlockNode(const_cast<LayoutTable*>(this)));
    auto first_section = grouped_children.begin();
    return first_section != grouped_children.end() && (*first_section).GetLayoutBox() == section;
}
...
LayoutTableSection* LayoutTable::FirstSection() const
{
    ...
    if (first_section != grouped_children.end()) {
        auto* section_object = To<LayoutTableSection>((*first_section).GetLayoutBox());
...
LayoutTableSection* LayoutTable::LastSection() const
{
    ...
    if (last_section != grouped_children.end()) {
        auto* section_object = To<LayoutTableSection>((*last_section).GetLayoutBox());
...
LayoutTableSection* LayoutTable::NextSection(const LayoutTableSection* current) const
{
    NOT_DESTROYED();
    TableGroupedChildren grouped_children(BlockNode(const_cast<LayoutTable*>(this)));
    bool found = false;
...
LayoutTableSection* LayoutTable::PreviousSection(const LayoutTableSection* current) const
{
    NOT_DESTROYED();
    TableGroupedChildren grouped_children(BlockNode(const_cast<LayoutTable*>(this)));
...
void LayoutTable::SetCachedTableColumnConstraints(scoped_refptr<const TableTypes::Columns> columns)
{
    NOT_DESTROYED();
...
void LayoutTable::GridBordersChanged()
{
    ...
    if (StyleRef().BorderCollapse() == EBorderCollapse::kCollapse) {
        ...
        SetNeedsLayoutAndIntrinsicWidthsRecalc(layout_invalidation_reason::kTableChanged);
    }
}
...
bool LayoutTable::HasBackgroundForPaint() const
{
    ...
    const TableFragmentData::ColumnGeometries* column_geometries = GetPhysicalFragment(0)->TableColumnGeometries();
...
void LayoutTable::AddChild(LayoutObject* child, LayoutObject* before_child)
{
    ...
    DCHECK(child->IsLayoutNGObject() || (!child->IsTableCaption() && !child->IsLayoutTableCol() && !child->IsTableSection()));
    bool wrap_in_anonymous_section = !child->IsTableCaption() && !child->IsLayoutTableCol() && !child->IsTableSection();
    ...
    if (!before_child && LastChild() && LastChild()->IsTableSection() && LastChild()->IsAnonymous() && !LastChild()->IsBeforeContent()) {
        LastChild()->AddChild(child);
        return;
    }

    if (before_child && !before_child->IsAnonymous() && before_child->Parent() == this) {
        auto* section = DynamicTo<LayoutTableSection>(before_child->PreviousSibling());
    ...
    while (last_box && last_box->Parent()->IsAnonymous() && !last_box->IsTableSection() && NeedsTableSection(*last_box))
        last_box = last_box->Parent();
    if (last_box && last_box->IsAnonymous() && last_box->IsTablePart() && !IsAfterContent(last_box)) {
    ...
    if (before_child && !before_child->IsTableSection() && NeedsTableSection(*before_child))
...
void LayoutTable::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    // StyleDifference handles changes in table-layout, border-spacing.
    if (old_style) {
        bool borders_changed = !old_style->BorderVisuallyEqual(StyleRef()) || old_style->GetWritingDirection() != StyleRef().GetWritingDirection()
            || old_style->IsFixedTableLayout() != StyleRef().IsFixedTableLayout() || old_style->EmptyCells() != StyleRef().EmptyCells();
        bool collapse_changed = StyleRef().BorderCollapse() != old_style->BorderCollapse();
...
LayoutBox* LayoutTable::CreateAnonymousBoxWithSameTypeAs(const LayoutObject* parent) const
{
    NOT_DESTROYED();
    return CreateAnonymousWithParent(*parent);
}

PhysicalRect LayoutTable::OverflowClipRect(const PhysicalOffset& location, OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior) const
{
    NOT_DESTROYED();
    ...
    if (StyleRef().BorderCollapse() == EBorderCollapse::kCollapse) {
        ...
        clip_rect = LayoutBlock::OverflowClipRect(location, overlay_scrollbar_clip_behavior);
    }
    // TODO(1142929)
...
LayoutUnit LayoutTable::BorderLeft() const
{
    ...
    if (HasCollapsedBorders() && cached_table_borders_) {
        return cached_table_borders_->TableBorder().ConvertToPhysical(Style()->GetWritingDirection()).left;
    }
...
LayoutUnit LayoutTable::BorderRight() const
{
    ...
    // ScrollAnchoring fails this DCHECK.
    if (HasCollapsedBorders() && cached_table_borders_) {
        return cached_table_borders_->TableBorder().ConvertToPhysical(Style()->GetWritingDirection()).right;
    }
...
LayoutUnit LayoutTable::BorderTop() const
{
    ...
    // ScrollAnchoring fails this DCHECK.
    if (HasCollapsedBorders() && cached_table_borders_) {
        return cached_table_borders_->TableBorder().ConvertToPhysical(Style()->GetWritingDirection()).top;
    }
...
unsigned LayoutTable::AbsoluteColumnToEffectiveColumn(unsigned absolute_column_index) const
{
    NOT_DESTROYED();
    ...
    for (unsigned current_column_index = 0; current_column_index < column_count; ++current_column_index) {
        if (current_column_index != 0 && !cached_table_columns_.get()->data[current_column_index].is_mergeable)
