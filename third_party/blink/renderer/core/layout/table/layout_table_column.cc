...
void LayoutTableColumn::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
            ...
            if (diff.NeedsLayout()) {
                ...
                if (old_style
                    && TableTypes::CreateColumn(*old_style,
                           /* default_inline_size */ std::nullopt, table->StyleRef().IsFixedTableLayout())
                        != TableTypes::CreateColumn(StyleRef(), /* default_inline_size */ std::nullopt, table->StyleRef().IsFixedTableLayout())) {
                    table->GridBordersChanged();
                }
            }
        }
...
void LayoutTableColumn::ImageChanged(WrappedImagePtr, CanDeferInvalidation)
{
    NOT_DESTROYED();
    if (LayoutTable* table = Table()) {
        table->SetShouldDoFullPaintInvalidationWithoutLayoutChange(PaintInvalidationReason::kImage);
    }
}
...
bool LayoutTableColumn::IsChildAllowed(LayoutObject* child, const ComputedStyle& style) const
{
    NOT_DESTROYED();
...
void LayoutTableColumn::UpdateFromElement()
{
    ...
    if (span_ != old_span && Style() && Parent()) {
        SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kAttributeChanged);
        if (LayoutTable* table = Table()) {
            table->GridBordersChanged();
...
PhysicalSize LayoutTableColumn::Size() const
{
    ...
    for (auto& fragment : table->PhysicalFragments()) {
        ...
        size.block_size += fragment.TableGridRect().size.block_size
            - (fragment.Padding().ConvertToLogical(direction).BlockSum() + fragment.Borders().ConvertToLogical(direction).BlockSum());
    }
...
LayoutPoint LayoutTableColumn::LocationInternal() const
{
    ...
    for (auto& fragment : table->PhysicalFragments()) {
        BoxStrut decorations = (fragment.Padding() + fragment.Borders()).ConvertToLogical(direction);
        if (!found_geometries && fragment.TableColumnGeometries()) {
            // If there was a table relayout, and this column box doesn't have a
            // corresponding column in the table anymore, the column_idx_ will not
            // have been updated. Therefore if it is greater or equal to the number of
            ...
            if (parent_colgroup) {
                const auto& parent_geometry = (*fragment.TableColumnGeometries())[parent_colgroup->column_idx_];
            ...
            if (!parent_colgroup) {
                offset.inline_offset += decorations.inline_start + table_border_spacing.inline_size;
                offset.block_offset += decorations.block_start + table_border_spacing.block_size + fragment.TableGridRect().offset.block_offset;
            }
        }

        size.block_size += fragment.TableGridRect().size.block_size - decorations.BlockSum();
    }
    ...
    if (!parent_colgroup) {
        ...
        outer_size = ToPhysicalSize(LogicalSize(parent_colgroup_inline_size, size.block_size), direction.GetWritingMode());
    }
    PhysicalSize inner_size = ToPhysicalSize(size, direction.GetWritingMode());
    return offset.ConvertToPhysical(direction, outer_size, inner_size).ToLayoutPoint();
}
