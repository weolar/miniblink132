...
public:
    UnpositionedFloat(BlockNode node, const BlockBreakToken* token, const LogicalSize available_size, const LogicalSize percentage_size,
        const LogicalSize replaced_percentage_size, const BfcOffset& origin_bfc_offset, const ConstraintSpace& parent_space, const ComputedStyle& parent_style,
        LayoutUnit fragmentainer_block_size, LayoutUnit fragmentainer_block_offset, bool is_hidden_for_paint)
        : node(node)
        , token(token)
