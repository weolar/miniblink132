...
public:
    ...
    void QuadsInAncestorInternal(Vector<gfx::QuadF>&, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags) const final;
    void AbsoluteQuadsForRange(Vector<gfx::QuadF>&, unsigned start_offset = 0, unsigned end_offset = INT_MAX) const;
    gfx::RectF LocalBoundingBoxRectForAccessibility() const final;

    enum ClippingOption { kNoClipping, kClipToEllipsis };
    void LocalQuadsInFlippedBlocksDirection(Vector<gfx::QuadF>&, ClippingOption = kNoClipping) const;
    ...
    String TransformAndSecureText(const String& original, TextOffsetMap& offset_map) const;
    ...
    bool MapDOMOffsetToTextContentOffset(const OffsetMapping&, unsigned* start, unsigned* end) const;
    ...
    // of the LayoutText.
    void LogicalStartingPointAndHeight(LogicalOffset& logical_starting_point, LayoutUnit& logical_height) const;
...
private:
    ...
    template <typename PhysicalRectCollector> void CollectLineBoxRects(const PhysicalRectCollector&, ClippingOption option = kNoClipping) const;
    ...
    bool NodeAtPoint(HitTestResult&, const HitTestLocation&, const PhysicalOffset&, HitTestPhase) final
    {
        NOT_DESTROYED();
        NOTREACHED();
    }
    ...
    std::pair<String, TextOffsetMap> SecureText(const String& plain, UChar mask) const;
    void SetVariableLengthTransformResult(wtf_size_t original_length, const TextOffsetMap& offset_map);
...
private:
    ...
    mutable LogicalOffset previous_logical_starting_point_ = UninitializedLogicalStartingPoint();
...
template <> struct DowncastTraits<LayoutText> {
