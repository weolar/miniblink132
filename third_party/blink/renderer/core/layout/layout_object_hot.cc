...
LayoutObject* LayoutObject::Container(AncestorSkipInfo* skip_info) const
{
    ...
    if (IsColumnSpanAll()) {
        ...
        if (skip_info) {
            ...
            for (LayoutObject* walker = Parent(); walker && walker != multicol_container; walker = walker->Parent())
...
void LayoutObject::SetNeedsOverflowRecalc(OverflowRecalcType overflow_recalc_type)
{
    NOT_DESTROYED();
    ...
    bool mark_container_chain_scrollable_overflow_recalc = !SelfNeedsScrollableOverflowRecalc();

    if (overflow_recalc_type == OverflowRecalcType::kLayoutAndVisualOverflowRecalc) {
        SetSelfNeedsScrollableOverflowRecalc();
    }

    DCHECK(overflow_recalc_type == OverflowRecalcType::kOnlyVisualOverflowRecalc || overflow_recalc_type == OverflowRecalcType::kLayoutAndVisualOverflowRecalc);
    SetShouldCheckForPaintInvalidation();
    MarkSelfPaintingLayerForVisualOverflowRecalc();

    if (mark_container_chain_scrollable_overflow_recalc) {
        MarkContainerChainForOverflowRecalcIfNeeded(overflow_recalc_type == OverflowRecalcType::kLayoutAndVisualOverflowRecalc);
    }
...
void LayoutObject::PropagateStyleToAnonymousChildren()
{
    ...
    for (LayoutObject* child = SlowFirstChild(); child; child = child->NextSibling()) {
        ...
        ComputedStyleBuilder new_style_builder
            = GetDocument().GetStyleResolver().CreateAnonymousStyleBuilderWithDisplay(StyleRef(), child->StyleRef().Display());
        ...
        if (IsA<LayoutTextCombine>(child)) [[unlikely]] {
            if (blink::IsHorizontalWritingMode(new_style_builder.GetWritingMode())) {
                ...
                DCHECK(child->SlowFirstChild()->IsBR() || To<LayoutText>(child->SlowFirstChild())->IsWordBreak()
...
void LayoutObject::UpdateImageObservers(const ComputedStyle* old_style, const ComputedStyle* new_style)
{
    NOT_DESTROYED();
    DCHECK(old_style || new_style);
    DCHECK(!IsText());

    UpdateFillImages(old_style ? &old_style->BackgroundLayers() : nullptr, new_style ? &new_style->BackgroundLayers() : nullptr);
    UpdateFillImages(old_style ? &old_style->MaskLayers() : nullptr, new_style ? &new_style->MaskLayers() : nullptr);

    UpdateImage(old_style ? old_style->BorderImage().GetImage() : nullptr, new_style ? new_style->BorderImage().GetImage() : nullptr);
    UpdateImage(old_style ? old_style->MaskBoxImage().GetImage() : nullptr, new_style ? new_style->MaskBoxImage().GetImage() : nullptr);

    StyleImage* old_content_image = old_style && old_style->GetContentData() && old_style->GetContentData()->IsImage()
        ? To<ImageContentData>(old_style->GetContentData())->GetImage()
        : nullptr;
    StyleImage* new_content_image = new_style && new_style->GetContentData() && new_style->GetContentData()->IsImage()
        ? To<ImageContentData>(new_style->GetContentData())->GetImage()
        : nullptr;
    UpdateImage(old_content_image, new_content_image);

    StyleImage* old_box_reflect_mask_image = old_style && old_style->BoxReflect() ? old_style->BoxReflect()->Mask().GetImage() : nullptr;
    StyleImage* new_box_reflect_mask_image = new_style && new_style->BoxReflect() ? new_style->BoxReflect()->Mask().GetImage() : nullptr;
    UpdateImage(old_box_reflect_mask_image, new_box_reflect_mask_image);

    UpdateShapeImage(old_style ? old_style->ShapeOutside() : nullptr, new_style ? new_style->ShapeOutside() : nullptr);
    UpdateCursorImages(old_style ? old_style->Cursors() : nullptr, new_style ? new_style->Cursors() : nullptr);
...
LayoutBlock* LayoutObject::ContainingBlock(AncestorSkipInfo* skip_info) const
{
    ...
    if (IsColumnSpanAll()) {
        ...
        if (!object && IsLayoutCustomScrollbarPart()) {
            object = To<LayoutCustomScrollbarPart>(this)->GetScrollableArea()->GetLayoutBox();
        }
