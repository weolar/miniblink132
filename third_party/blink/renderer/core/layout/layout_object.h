...
enum {
    ...
    kBackgroundPaintInBothSpaces = kBackgroundPaintInBorderBoxSpace | kBackgroundPaintInContentsSpace,
};
...
class CORE_EXPORT LayoutObject : public GarbageCollected<LayoutObject>, public ImageResourceObserver, public DisplayItemClient {
    friend class LayoutObjectChildList;
    FRIEND_TEST_ALL_PREFIXES(LayoutObjectTest, MutableForPaintingClearPaintFlags);
    FRIEND_TEST_ALL_PREFIXES(LayoutObjectTest, ContainingBlockAbsoluteLayoutObjectShouldBeNonStaticallyPositionedBlockAncestor);
    FRIEND_TEST_ALL_PREFIXES(LayoutObjectTest, ContainingBlockFixedLayoutObjectInTransformedDiv);
    FRIEND_TEST_ALL_PREFIXES(LayoutObjectTest, ContainingBlockFixedLayoutObjectInTransformedDiv);
    FRIEND_TEST_ALL_PREFIXES(LayoutObjectTest, ContainingBlockFixedLayoutObjectInBody);
    FRIEND_TEST_ALL_PREFIXES(LayoutObjectTest, ContainingBlockAbsoluteLayoutObjectInBody);
    FRIEND_TEST_ALL_PREFIXES(LayoutObjectTest, ContainingBlockAbsoluteLayoutObjectShouldNotBeNonStaticallyPositionedInlineAncestor);
...
public:
    ...
    LayoutObject* NextInPreOrderAfterChildren(const LayoutObject* stay_within) const;
    ...
    LayoutObject* PreviousInPostOrderBeforeChildren(const LayoutObject* stay_within) const;
    ...
    LayoutObject* CommonAncestor(const LayoutObject& other, CommonAncestorData* data = nullptr) const;
    ...
    PaintLayer* FindNextLayer(PaintLayer* parent_layer, LayoutObject* start_point, bool check_parent = true);
    ...
    {
        NOT_DESTROYED();
        for (const LayoutObject* layout_object = this; layout_object; layout_object
             = layout_object->ChildLayoutBlockedByDisplayLock() ? layout_object->NextInPreOrderAfterChildren(this) : layout_object->NextInPreOrder(this)) {
    ...
    void AssertSubtreeClearedPaintInvalidationFlags() const
    {
        NOT_DESTROYED();
        for (const LayoutObject* layout_object = this; layout_object; layout_object
             = layout_object->ChildPrePaintBlockedByDisplayLock() ? layout_object->NextInPreOrderAfterChildren(this) : layout_object->NextInPreOrder(this)) {
    ...
    virtual void AddChildIgnoringContinuation(LayoutObject* new_child, LayoutObject* before_child = nullptr)
    {
        NOT_DESTROYED();
    ...
    inline bool ShouldApplyOverflowClipMargin() const
    {
        ...
        if (IsLayoutReplaced()) {
            is_overflow_clip = style.OverflowX() != EOverflow::kVisible && style.OverflowY() != EOverflow::kVisible;
        } else {
            is_overflow_clip = style.OverflowX() == EOverflow::kClip && style.OverflowY() == EOverflow::kClip;
        }
        ...
        return (is_overflow_clip || ShouldApplyPaintContainment()) && RespectsCSSOverflow();
    }

    inline bool IsEligibleForPaintOrLayoutContainment() const
    {
        NOT_DESTROYED();
        return (!IsInline() || IsAtomicInlineLevel()) && (!IsTablePart() || IsLayoutBlockFlow());
    }
    ...
    {
        NOT_DESTROYED();
        return (!IsInline() || IsAtomicInlineLevel()) && (!IsTablePart() || IsTableCaption()) && !IsTable();
    }
    ...
    {
        NOT_DESTROYED();
        return ShouldApplyStyleContainment() && ShouldApplyPaintContainment() && ShouldApplyLayoutContainment();
    }
    inline bool ShouldApplyStrictContainment() const
    {
        NOT_DESTROYED();
        return ShouldApplyStyleContainment() && ShouldApplyPaintContainment() && ShouldApplyLayoutContainment() && ShouldApplySizeContainment();
    }
    inline bool ShouldApplyAnyContainment() const
    {
        NOT_DESTROYED();
        return ShouldApplyPaintContainment() || ShouldApplyLayoutContainment() || ShouldApplyStyleContainment() || ShouldApplyBlockSizeContainment()
    ...
    inline bool IsStackingContext(const ComputedStyle& style) const
    {
        ...
        return style.IsStackingContextWithoutContainment()
            || ((style.ContainsLayout() || style.ContainsPaint()) && (!IsInline() || IsAtomicInlineLevel()) && (!IsTablePart() || IsLayoutBlockFlow()));
    }
    ...
    {
        NOT_DESTROYED();
        return style.GetPosition() != EPosition::kStatic || IsStackingContext(style);
    }
...
public:
    ...
    void DumpLayoutObject(StringBuilder&, bool dump_address, unsigned show_tree_character_offset) const;
    ...
    // label.
    void DumpLayoutTreeAndMark(StringBuilder&, const LayoutObject* marked_object1 = nullptr, const char* marked_label1 = nullptr,
        const LayoutObject* marked_object2 = nullptr, const char* marked_label2 = nullptr, unsigned depth = 0) const;
#endif // DCHECK_IS_ON()
    ...
    static LayoutBlockFlow* CreateBlockFlowOrListItem(Element* element, const ComputedStyle& style);
    ...
    void SetAlwaysCreateLineBoxesForLayoutInline(bool always_create_line_boxes)
    {
        NOT_DESTROYED();
        DCHECK(IsLayoutInline());
        bitfields_.SetAlwaysCreateLineBoxesForLayoutInline(always_create_line_boxes);
    }
    ...
    {
        NOT_DESTROYED();
        return IsInsideFlowThread() || (GetDocument().Printing() && !IsLayoutView());
    }
    ...
    {
        NOT_DESTROYED();
        return !IsSVG() || IsSVGShape() || IsSVGImage() || IsSVGInline() || IsSVGRoot() || IsSVGForeignObject() || IsSVGText() ||
    ...
    bool IsAnonymousBlock() const
    {
        ...
        return IsAnonymous() && (StyleRef().Display() == EDisplay::kBlock || StyleRef().Display() == EDisplay::kWebkitBox)
            && StyleRef().StyleType() == kPseudoIdNone && IsLayoutBlock() && !IsLayoutFlowThread() && !IsLayoutMultiColumnSet();
    }
    ...
    {
        NOT_DESTROYED();
        return IsOutOfFlowPositioned() && StyleRef().GetPosition() == EPosition::kFixed;
    }
    bool IsAbsolutePositioned() const
    {
        NOT_DESTROYED();
        return IsOutOfFlowPositioned() && StyleRef().GetPosition() == EPosition::kAbsolute;
    }
    ...
    {
        NOT_DESTROYED();
        return IsAnonymous() && !IsInline() && !IsFloatingOrOutOfFlowPositioned() && Parent() && Parent()->IsLayoutInline();
    }
    ...
    {
        NOT_DESTROYED();
        return IsHorizontalWritingMode() || StyleRef().IsHorizontalTypographicMode();
    }
    ...
    [[nodiscard]] LayoutPoint FlipForWritingMode(const PhysicalOffset& p, const LayoutBox* box_for_flipping = nullptr) const
    {
        NOT_DESTROYED();
        if (!HasFlippedBlocksWritingMode()) [[likely]] {
            return p.ToLayoutPoint();
        }
        return { FlipForWritingModeInternal(p.left, LayoutUnit(), box_for_flipping), p.top };
    }
    ...
    {
        NOT_DESTROYED();
        return bitfields_.SelfNeedsFullLayout() || bitfields_.ChildNeedsFullLayout() || bitfields_.NeedsSimplifiedLayout();
    }

    bool NeedsSimplifiedLayoutOnly() const
    {
        NOT_DESTROYED();
        return bitfields_.NeedsSimplifiedLayout() && !bitfields_.SelfNeedsFullLayout() && !bitfields_.ChildNeedsFullLayout();
    }
    ...
    {
        NOT_DESTROYED();
        return bitfields_.SelfNeedsScrollableOverflowRecalc() || bitfields_.ChildNeedsScrollableOverflowRecalc();
    }
    ...
    {
        NOT_DESTROYED();
        return HasTransformRelatedProperty() && StyleRef().Preserves3D() && !IsSVGChild();
    }
    ...
    bool IsRenderedLegendCandidate() const
    {
        ...
        // that it is floating.
        return IsHTMLLegendElement() && !IsOutOfFlowPositioned() && !Style()->IsFloating();
    }
    ...
    {
        NOT_DESTROYED();
        return StyleRef().GetColumnSpan() == EColumnSpan::kAll && SpannerPlaceholder();
    }
    ...
    public:
        AncestorSkipInfo(const LayoutObject* ancestor, bool check_for_filters = false)
            : ancestor_(ancestor)
        ...
        bool AncestorSkipped() const
        {
            return ancestor_skipped_;
        }
    ...
    bool CanContainOutOfFlowPositionedElement(EPosition position) const
    {
        ...
        return (position == EPosition::kAbsolute && CanContainAbsolutePositionObjects()) || (position == EPosition::kFixed && CanContainFixedPositionObjects());
    }
    ...
    const LayoutBoxModelObject* FindFirstStickyContainer(const LayoutBox* below) const;
    ...
    void SetNeedsLayoutAndFullPaintInvalidation(LayoutInvalidationReasonForTracing, MarkingBehavior = kMarkContainerChain);
    ...
    void SetNeedsLayoutAndIntrinsicWidthsRecalc(LayoutInvalidationReasonForTracing reason)
    {
        NOT_DESTROYED();
        SetNeedsLayout(reason);
        SetIntrinsicLogicalWidthsDirty();
    }
    void SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(LayoutInvalidationReasonForTracing reason)
    {
        NOT_DESTROYED();
...
public:
    ...
    {
        NOT_DESTROYED();
        DCHECK((position != EPosition::kAbsolute && position != EPosition::kFixed) || IsBox());
    ...
    virtual bool HitTestAllPhases(HitTestResult&, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset);
    ...
    // |accumulated_offset|.
    virtual bool NodeAtPoint(HitTestResult&, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase);

    virtual PositionWithAffinity PositionForPoint(const PhysicalOffset&) const;
    PositionWithAffinity CreatePositionWithAffinity(int offset, TextAffinity) const;
    ...
    void SetStyle(const ComputedStyle*, ApplyStyleChanges = ApplyStyleChanges::kYes);

    // Set the style of the object if it's generated content.
    void SetPseudoElementStyle(const LayoutObject& owner, bool match_parent_size = false);
    ...
    void SetModifiedStyleOutsideStyleRecalc(const ComputedStyle*, ApplyStyleChanges);
    ...
    const LayoutBox* ContainingScrollContainer(bool ignore_layout_view_for_fixed_pos = false) const;
    const PaintLayer* ContainingScrollContainerLayer(bool ignore_layout_view_for_fixed_pos = false) const;
    ...
    PhysicalRect AncestorToLocalRect(const LayoutBoxModelObject* ancestor, const PhysicalRect& rect, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return PhysicalRect::EnclosingRect(AncestorToLocalQuad(ancestor, gfx::QuadF(gfx::RectF(rect)), mode).BoundingBox());
    }
    gfx::QuadF AncestorToLocalQuad(const LayoutBoxModelObject*, const gfx::QuadF&, MapCoordinatesFlags mode = 0) const;
    PhysicalOffset AncestorToLocalPoint(const LayoutBoxModelObject* ancestor, const PhysicalOffset& p, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return PhysicalOffset::FromPointFRound(AncestorToLocalPoint(ancestor, gfx::PointF(p), mode));
    }
    gfx::PointF AncestorToLocalPoint(const LayoutBoxModelObject* ancestor, const gfx::PointF& p, MapCoordinatesFlags = 0) const;
    ...
    PhysicalRect LocalToAncestorRect(const PhysicalRect& rect, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode = 0) const;
    gfx::QuadF LocalRectToAncestorQuad(const PhysicalRect& rect, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return LocalToAncestorQuad(gfx::QuadF(gfx::RectF(rect)), ancestor, mode);
    }
    gfx::QuadF LocalToAncestorQuad(const gfx::QuadF&, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags = 0) const;
    PhysicalOffset LocalToAncestorPoint(const PhysicalOffset& p, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return PhysicalOffset::FromPointFRound(LocalToAncestorPoint(gfx::PointF(p), ancestor, mode));
    }
    gfx::PointF LocalToAncestorPoint(const gfx::PointF&, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags = 0) const;
    void LocalToAncestorRects(
        Vector<PhysicalRect>&, const LayoutBoxModelObject* ancestor, const PhysicalOffset& pre_offset, const PhysicalOffset& post_offset) const;
    ...
    gfx::Transform LocalToAncestorTransform(const LayoutBoxModelObject* ancestor, MapCoordinatesFlags = 0) const;
    ...
    // This method supports kUseGeometryMapperMode.
    PhysicalRect LocalToAbsoluteRect(const PhysicalRect& rect, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return LocalToAncestorRect(rect, nullptr, mode);
    }
    gfx::QuadF LocalRectToAbsoluteQuad(const PhysicalRect& rect, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return LocalRectToAncestorQuad(rect, nullptr, mode);
    }
    gfx::QuadF LocalToAbsoluteQuad(const gfx::QuadF& quad, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return LocalToAncestorQuad(quad, nullptr, mode);
    }
    PhysicalOffset LocalToAbsolutePoint(const PhysicalOffset& p, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return LocalToAncestorPoint(p, nullptr, mode);
    }
    gfx::PointF LocalToAbsolutePoint(const gfx::PointF& p, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return LocalToAncestorPoint(p, nullptr, mode);
    }
    PhysicalRect AbsoluteToLocalRect(const PhysicalRect& rect, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return AncestorToLocalRect(nullptr, rect, mode);
    }
    gfx::QuadF AbsoluteToLocalQuad(const gfx::QuadF& quad, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return AncestorToLocalQuad(nullptr, quad, mode);
    }
    PhysicalOffset AbsoluteToLocalPoint(const PhysicalOffset& p, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return AncestorToLocalPoint(nullptr, p, mode);
    }
    gfx::PointF AbsoluteToLocalPoint(const gfx::PointF& p, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
        return AncestorToLocalPoint(nullptr, p, mode);
    }
    ...
    // and multicol).
    PhysicalOffset OffsetFromContainer(const LayoutObject*, MapCoordinatesFlags = 0) const;
    ...
    virtual PhysicalRect AbsoluteBoundingBoxRectHandlingEmptyInline(MapCoordinatesFlags flags = 0) const;
    ...
    void QuadsInAncestor(Vector<gfx::QuadF>& quads, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
    ...
    void AbsoluteQuads(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
    ...
    {
        NOT_DESTROYED();
        return style_variant == StyleVariant::kStandard ? StyleRef() : SlowEffectiveStyle(style_variant);
    }

    static inline Color ResolveColor(const ComputedStyle& style_to_use, const Longhand& color_property)
    {
    ...
    template <class Property> static inline Color ResolveColorFast(const ComputedStyle& style_to_use, const Property& color_property)
    {
        return style_to_use.VisitedDependentColorFast(color_property);
    }

    template <class Property> inline Color ResolveColorFast(const Property& color_property) const
    {
        NOT_DESTROYED();
    ...
    // information.
    bool MapToVisualRectInAncestorSpace(const LayoutBoxModelObject* ancestor, PhysicalRect&, VisualRectFlags = kDefaultVisualRectFlags) const;

    bool MapToVisualRectInAncestorSpace(const LayoutBoxModelObject* ancestor, gfx::RectF&, VisualRectFlags = kDefaultVisualRectFlags) const;

    // Do not call this method directly. Call mapToVisualRectInAncestorSpace
    // instead.
    virtual bool MapToVisualRectInAncestorSpaceInternal(const LayoutBoxModelObject* ancestor, TransformState&, VisualRectFlags = kDefaultVisualRectFlags) const;
    ...
    const LayoutObject* GetPropertyContainer(AncestorSkipInfo*, PropertyTreeStateOrAlias* = nullptr, VisualRectFlags = kDefaultVisualRectFlags) const;
   ...
   */
    ...
    bool VisibleToHitTestRequest(const HitTestRequest& request) const
    {
        ...
        return StyleRef().Visibility() == EVisibility::kVisible
            && (request.IgnorePointerEventsNone() || StyleRef().UsedPointerEvents() != EPointerEvents::kNone);
    }
    ...
    virtual void MapLocalToAncestor(const LayoutBoxModelObject* ancestor, TransformState&, MapCoordinatesFlags) const;
    ...
    virtual void MapAncestorToLocal(const LayoutBoxModelObject*, TransformState&, MapCoordinatesFlags) const;
    ...
    // fragmented).
    void GetTransformFromContainer(const LayoutObject* container, const PhysicalOffset& offset_in_container, gfx::Transform&,
        const PhysicalSize* size = nullptr, const gfx::Transform* fragment_transform = nullptr) const;
    ...
    Vector<PhysicalRect> CollectOutlineRectsAndAdvance(OutlineType, AccompaniedFragmentIterator& iterator) const;
    ...
    struct OutlineInfo {
        ...
        {
            return { static_cast<int>(getUnzoomedWidth(style)), static_cast<int>(std::floor(style.OutlineOffset() / style.EffectiveZoom())) };
        }
    };

    // OutlineInfo, if specified, is filled in with the outline width and offset
    // in the same space as the physical rects returned.
    Vector<PhysicalRect> OutlineRects(OutlineInfo*, const PhysicalOffset& additional_offset, OutlineType) const;
    ...
    virtual void AddOutlineRects(OutlineRectCollector&, OutlineInfo*, const PhysicalOffset& additional_offset, OutlineType) const
    {
        NOT_DESTROYED();
    }
    ...
    {
        NOT_DESTROYED();
        return static_cast<PaintInvalidationReason>(paint_invalidation_reason_for_pre_paint_);
    }
    ...
    void SetShouldDoFullPaintInvalidation(PaintInvalidationReason = PaintInvalidationReason::kLayout);
    void SetShouldDoFullPaintInvalidationWithoutLayoutChange(PaintInvalidationReason reason);

    void SetShouldInvalidatePaintForHitTest();
    bool ShouldInvalidatePaintForHitTestOnly() const
    {
        NOT_DESTROYED();
        return PaintInvalidationReasonForPrePaint() == PaintInvalidationReason::kHitTest;
    }
    ...
    void SetSubtreeShouldDoFullPaintInvalidation(PaintInvalidationReason reason = PaintInvalidationReason::kSubtree);
    bool SubtreeShouldDoFullPaintInvalidation() const
    {
        NOT_DESTROYED();
        DCHECK(!bitfields_.SubtreeShouldDoFullPaintInvalidation() || ShouldDoFullPaintInvalidation());
    ...
    virtual void UpdateAnonymousChildStyle(const LayoutObject* child, ComputedStyleBuilder&) const
    {
        NOT_DESTROYED();
    }
    ...
    void SetNeedsOverflowRecalc(OverflowRecalcType = OverflowRecalcType::kLayoutAndVisualOverflowRecalc);
    ...
    public:
        ...
        {
            layout_object_.ClearPaintFlags();
        }
        ...
        void SetShouldCheckForPaintInvalidation()
        {
            DCHECK_EQ(layout_object_.GetDocument().Lifecycle().GetState(), DocumentLifecycle::kInPrePaint);
        ...
        void SetShouldDoFullPaintInvalidationWithoutLayoutChange(PaintInvalidationReason reason)
        {
            ...
            layout_object_.bitfields_.SetShouldCheckForPaintInvalidation(true);
            layout_object_.SetShouldDoFullPaintInvalidationWithoutLayoutChangeInternal(reason);
        }
        ...
        {
            layout_object_.bitfields_.SetPreviousVisibilityVisible(layout_object_.StyleRef().Visibility() == EVisibility::kVisible);
        }
        ...
        void AddSubtreePaintPropertyUpdateReason(SubtreePaintPropertyUpdateReason reason)
        {
        ...
        {
            layout_object_.SetShouldAssumePaintOffsetTranslationForLayoutShiftTracking(b);
        }
        ...
        void EnsureId()
        {
            layout_object_.fragment_->EnsureId();
        }
    ...
    protected:
        ...
        FRIEND_TEST_ALL_PREFIXES(AnimationCompositorAnimationsTest, canStartElementOnCompositorTransformCAP);
        FRIEND_TEST_ALL_PREFIXES(AnimationCompositorAnimationsTest, canStartElementOnCompositorEffectCAP);
        FRIEND_TEST_ALL_PREFIXES(PrePaintTreeWalkTest, ClipRects);
        FRIEND_TEST_ALL_PREFIXES(LayoutObjectTest, VisualRect);
        FRIEND_TEST_ALL_PREFIXES(BoxPaintInvalidatorTest, ComputePaintInvalidationReasonBasic);
    ...
    {
        NOT_DESTROYED();
        DCHECK_LE(static_cast<unsigned>(reason), 1u << (kSubtreePaintPropertyUpdateReasonsBitfieldWidth - 1));
    ...
    {
        NOT_DESTROYED();
        return !ShouldDelayFullPaintInvalidation() && bitfields_.BackgroundNeedsFullPaintInvalidation();
    }
    void SetBackgroundNeedsFullPaintInvalidation()
    {
        NOT_DESTROYED();
        SetShouldDoFullPaintInvalidationWithoutLayoutChangeInternal(PaintInvalidationReason::kBackground);
    ...
    void SetIsDetachedNonDomRoot(bool)
    {
        NOT_DESTROYED();
    }
#endif // DCHECK_IS_ON()
    ...
    {
        NOT_DESTROYED();
        return bitfields_.ShouldAssumePaintOffsetTranslationForLayoutShiftTracking();
    }
    void SetShouldAssumePaintOffsetTranslationForLayoutShiftTracking(bool b)
    {
        NOT_DESTROYED();
...
protected:
    ...
    bool MapToVisualRectInAncestorSpaceInternalFastPath(const LayoutBoxModelObject* ancestor, gfx::RectF&, VisualRectFlags, bool& intersects) const;
    ...
    virtual PhysicalOffset OffsetFromContainerInternal(const LayoutObject*, MapCoordinatesFlags mode) const;
    PhysicalOffset OffsetFromScrollableContainer(const LayoutObject*, bool ignore_scroll_offset) const;

    virtual void QuadsInAncestorInternal(Vector<gfx::QuadF>&, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags) const
    {
        NOT_DESTROYED();
    }
    ...
    {
        NOT_DESTROYED();
        DCHECK_GE(GetDocument().Lifecycle().GetState(), DocumentLifecycle::kInPrePaint);
    ...
    LayoutBlock* ContainingBlockForAbsolutePosition(AncestorSkipInfo* = nullptr) const;
    // Returns ContainerForFixedPosition() if it's a LayoutBlock, or the
    // containing LayoutBlock of it.
    LayoutBlock* ContainingBlockForFixedPosition(AncestorSkipInfo* = nullptr) const;
...
private:
    gfx::QuadF LocalToAncestorQuadInternal(const gfx::QuadF&, const LayoutBoxModelObject* ancestor, MapCoordinatesFlags = 0) const;
    ...
    void UpdateFillImages(const FillLayer* old_layers, const FillLayer* new_layers);
    void UpdateCursorImages(const CursorList* old_cursors, const CursorList* new_cursors);
    ...
    inline void MarkContainerChainForOverflowRecalcIfNeeded(bool mark_container_chain_scrollable_overflow_recalc);
    ...
    void UpdateImageObservers(const ComputedStyle* old_style, const ComputedStyle* new_style);
    ...
    virtual LayoutUnit FlipForWritingModeInternal(LayoutUnit position, LayoutUnit width, const LayoutBox* box_for_flipping) const;

    void MarkSelfPaintingLayerForVisualOverflowRecalc();

    void SetShouldDoFullPaintInvalidationWithoutLayoutChangeInternal(PaintInvalidationReason);
    ...
    unsigned subtree_paint_property_update_reasons_ : kSubtreePaintPropertyUpdateReasonsBitfieldWidth;
...
private:                                                                                                                                                       \
    ...
    public:
        ...
        ADD_BOOLEAN_BITFIELD(self_needs_scrollable_overflow_recalc_, SelfNeedsScrollableOverflowRecalc);

        ADD_BOOLEAN_BITFIELD(child_needs_scrollable_overflow_recalc_, ChildNeedsScrollableOverflowRecalc);
        ...
        // widths.
        ADD_BOOLEAN_BITFIELD(intrinsic_logical_widths_dirty_, IntrinsicLogicalWidthsDirty);
        ...
        ADD_BOOLEAN_BITFIELD(intrinsic_logical_widths_depends_on_block_constraints_, IntrinsicLogicalWidthsDependsOnBlockConstraints);

        // Indicates if the indefinite min/max sizes cache slot is dirty.
        ADD_BOOLEAN_BITFIELD(indefinite_intrinsic_logical_widths_dirty_, IndefiniteIntrinsicLogicalWidthsDirty);

        // Indicates if the definite min/max sizes cache slots are dirty.
        ADD_BOOLEAN_BITFIELD(definite_intrinsic_logical_widths_dirty_, DefiniteIntrinsicLogicalWidthsDirty);
        ...
        ADD_BOOLEAN_BITFIELD(should_check_for_paint_invalidation_, ShouldCheckForPaintInvalidation);
        ADD_BOOLEAN_BITFIELD(subtree_should_check_for_paint_invalidation_, SubtreeShouldCheckForPaintInvalidation);
        ADD_BOOLEAN_BITFIELD(should_delay_full_paint_invalidation_, ShouldDelayFullPaintInvalidation);
        ADD_BOOLEAN_BITFIELD(subtree_should_do_full_paint_invalidation_, SubtreeShouldDoFullPaintInvalidation);
        ADD_BOOLEAN_BITFIELD(may_need_paint_invalidation_animated_background_image_, MayNeedPaintInvalidationAnimatedBackgroundImage);
        ADD_BOOLEAN_BITFIELD(should_invalidate_selection_, ShouldInvalidateSelection);
        ADD_BOOLEAN_BITFIELD(should_check_layout_for_paint_invalidation_, ShouldCheckLayoutForPaintInvalidation);
        ADD_BOOLEAN_BITFIELD(descendant_should_check_layout_for_paint_invalidation_, DescendantShouldCheckLayoutForPaintInvalidation);
        // Whether the paint properties need to be updated. For more details, see
        // LayoutObject::NeedsPaintPropertyUpdate().
        ADD_BOOLEAN_BITFIELD(needs_paint_property_update_, NeedsPaintPropertyUpdate);
        ...
        ADD_BOOLEAN_BITFIELD(descendant_needs_paint_property_update_, DescendantNeedsPaintPropertyUpdate);
        ...
        // legacy.
        ADD_BOOLEAN_BITFIELD(is_in_layout_ng_inline_formatting_context_, IsInLayoutNGInlineFormattingContext);
        ...
        // LayoutInline).
        ADD_BOOLEAN_BITFIELD(has_transform_related_property_, HasTransformRelatedProperty);
        ...
        ADD_BOOLEAN_BITFIELD(can_contain_absolute_position_objects_, CanContainAbsolutePositionObjects);
        // This boolean is used to know if this LayoutObject is a container for
        // fixed position descendants.
        ADD_BOOLEAN_BITFIELD(can_contain_fixed_position_objects_, CanContainFixedPositionObjects);

        ADD_BOOLEAN_BITFIELD(ever_had_layout_, EverHadLayout);

        ADD_BOOLEAN_BITFIELD(is_inside_flow_thread_, IsInsideFlowThread);

        ADD_BOOLEAN_BITFIELD(subtree_change_listener_registered_, SubtreeChangeListenerRegistered);
        ADD_BOOLEAN_BITFIELD(notified_of_subtree_change_, NotifiedOfSubtreeChange);
        ADD_BOOLEAN_BITFIELD(consumes_subtree_change_notification_, ConsumesSubtreeChangeNotification);

        // from LayoutBlock
        ADD_BOOLEAN_BITFIELD(children_inline_, ChildrenInline);

        // from LayoutInline
        ADD_BOOLEAN_BITFIELD(always_create_line_boxes_for_layout_inline_, AlwaysCreateLineBoxesForLayoutInline);

        // For LayoutBox to cache the result of LayoutBox::
        // ComputeBackgroundIsKnownToBeObscured(). It's updated during PrePaint.
        ADD_BOOLEAN_BITFIELD(background_is_known_to_be_obscured_, BackgroundIsKnownToBeObscured);

        ADD_BOOLEAN_BITFIELD(is_background_attachment_fixed_object_, IsBackgroundAttachmentFixedObject);
        ADD_BOOLEAN_BITFIELD(can_composite_background_attachment_fixed_, CanCompositeBackgroundAttachmentFixed);
        ...
        ADD_BOOLEAN_BITFIELD(scroll_anchor_disabling_style_changed_, ScrollAnchorDisablingStyleChanged);

        ADD_BOOLEAN_BITFIELD(should_skip_layout_cache_, ShouldSkipLayoutCache);

        ADD_BOOLEAN_BITFIELD(has_box_decoration_background_, HasBoxDecorationBackground);

        ADD_BOOLEAN_BITFIELD(background_needs_full_paint_invalidation_, BackgroundNeedsFullPaintInvalidation);
        ...
        ADD_BOOLEAN_BITFIELD(outline_may_be_affected_by_descendants_, OutlineMayBeAffectedByDescendants);
        // The outlineMayBeAffectedByDescendants status of the last paint
        // invalidation.
        ADD_BOOLEAN_BITFIELD(previous_outline_may_be_affected_by_descendants_, PreviousOutlineMayBeAffectedByDescendants);
        // CSS visibility : visible status of the last paint invalidation.
        ADD_BOOLEAN_BITFIELD(previous_visibility_visible_, PreviousVisibilityVisible);

        ADD_BOOLEAN_BITFIELD(is_truncated_, IsTruncated);

        // Whether this object's Node has a blocking touch event handler on itself
        // or an ancestor. This is updated during the PrePaint phase.
        ADD_BOOLEAN_BITFIELD(inside_blocking_touch_event_handler_, InsideBlockingTouchEventHandler);
        ...
        ADD_BOOLEAN_BITFIELD(effective_allowed_touch_action_changed_, EffectiveAllowedTouchActionChanged);
        ...
        ADD_BOOLEAN_BITFIELD(descendant_effective_allowed_touch_action_changed_, DescendantEffectiveAllowedTouchActionChanged);

        // Whether this object's Node has a blocking wheel event handler on itself
        // or an ancestor. This is updated during the PrePaint phase.
        ADD_BOOLEAN_BITFIELD(inside_blocking_wheel_event_handler_, InsideBlockingWheelEventHandler);
        ...
        ADD_BOOLEAN_BITFIELD(blocking_wheel_event_handler_changed_, BlockingWheelEventHandlerChanged);

        // Set when a descendant's |InsideBlockingWheelEventHandler| changes. This
        // is used to ensure the PrePaint tree walk processes objects with
        // |blocking_wheel_event_handler_changed_|.
        ADD_BOOLEAN_BITFIELD(descendant_blocking_wheel_event_handler_changed_, DescendantBlockingWheelEventHandlerChanged);
        ...
        // observer.
        ADD_BOOLEAN_BITFIELD(registered_as_first_line_image_observer_, RegisteredAsFirstLineImageObserver);
        ...
        ADD_BOOLEAN_BITFIELD(is_table_column_constraints_dirty_, IsTableColumnsConstraintsDirty);
        ...
        ADD_BOOLEAN_BITFIELD(is_subgrid_min_max_sizes_cache_dirty_, IsSubgridMinMaxSizesCacheDirty);
        ...
        // included).
        ADD_BOOLEAN_BITFIELD(transform_affects_vector_effect_, TransformAffectsVectorEffect);
        ...
        ADD_BOOLEAN_BITFIELD(svg_descendant_may_have_transform_related_animation_, SVGDescendantMayHaveTransformRelatedAnimation);

        // For SVG objects, indicates if this object or any descendant depends on
        // the dimensions of the viewport.
        ADD_BOOLEAN_BITFIELD(svg_self_or_descendant_has_viewport_dependency_, SVGSelfOrDescendantHasViewportDependency);

        // Whether to skip layout shift tracking in the next paint invalidation.
        // See PaintInvalidator::UpdateLayoutShiftTracking().
        ADD_BOOLEAN_BITFIELD(should_skip_next_layout_shift_tracking_, ShouldSkipNextLayoutShiftTracking);
        ...
        ADD_BOOLEAN_BITFIELD(should_assume_paint_offset_translation_for_layout_shift_tracking_, ShouldAssumePaintOffsetTranslationForLayoutShiftTracking);
        ...
        ADD_BOOLEAN_BITFIELD(might_traverse_physical_fragments_, MightTraversePhysicalFragments);
        ...
        ADD_BOOLEAN_BITFIELD(whitespace_children_may_change_, WhitespaceChildrenMayChange);
        ...
        // its scrollable area.
        ADD_BOOLEAN_BITFIELD(scrollable_area_size_changed_, ScrollableAreaSizeChanged);
    };
...
// identical.
inline void LayoutObject::SetNeedsLayout(LayoutInvalidationReasonForTracing reason, MarkingBehavior mark_parents)
{
#if DCHECK_IS_ON()
    ...
    if (!already_needed_layout) {
        DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT_WITH_CATEGORIES(TRACE_DISABLED_BY_DEFAULT("devtools.timeline.invalidationTracking"), "LayoutInvalidationTracking",
...
inline void LayoutObject::SetNeedsLayoutAndFullPaintInvalidation(LayoutInvalidationReasonForTracing reason, MarkingBehavior mark_parents)
{
...
inline void LayoutObject::SetIsInLayoutNGInlineFormattingContext(bool new_value)
{
    DCHECK(!GetDocument().InvalidationDisallowed());
...
enum class LayoutObjectSide { kRemainingTextIfOnBoundary, kFirstLetterIfOnBoundary };
CORE_EXPORT const LayoutObject* AssociatedLayoutObjectOf(const Node&, int offset_in_node, LayoutObjectSide = LayoutObjectSide::kRemainingTextIfOnBoundary);
...
CORE_EXPORT void ShowLayoutTree(const blink::LayoutObject* object1, const blink::LayoutObject* object2);

#endif

namespace cppgc {
// Assign LayoutObject to be allocated on custom LayoutObjectSpace.
template <typename T> struct SpaceTrait<T, std::enable_if_t<std::is_base_of<blink::LayoutObject, T>::value>> {
