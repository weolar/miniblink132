...
const LayoutResult& BoxFragmentBuilder::LayoutResultForPropagation(const LayoutResult& layout_result) const
{
...
void BoxFragmentBuilder::AddBreakBeforeChild(LayoutInputNode child, std::optional<BreakAppeal> appeal, bool is_forced_break)
{
        ...
        if (!last_inline_break_token_) {
            ...
            if (PreviousBreakToken()) {
                ...
                if (child_tokens.size()) {
                    ...
                    // child.
                    last_inline_break_token_ = DynamicTo<InlineBreakToken>(child_tokens.back().Get());
                    if (last_inline_break_token_)
                        return;
                }
            }
            ...
            last_inline_break_token_ = InlineBreakToken::Create(*child_inline_node, /* style */ nullptr, InlineItemTextIndex(), InlineBreakToken::kDefault);
        }
        return;
    }
...
void BoxFragmentBuilder::AddResult(const LayoutResult& child_layout_result, const LogicalOffset offset, std::optional<const BoxStrut> margins,
    std::optional<LogicalOffset> relative_offset, const OofInlineContainer<LogicalOffset>* inline_container)
{
    ...
    AddChild(fragment, offset, &end_margin_strut, child_layout_result.IsSelfCollapsing(), relative_offset, inline_container);
    if (margins) {
        const auto& box_fragment = To<PhysicalBoxFragment>(fragment);
        if (!margins->IsEmpty() || !box_fragment.Margins().IsZero()) {
            box_fragment.GetMutableForContainerLayout().SetMargins(margins->ConvertToPhysical(GetWritingDirection()));
        }
    }
...
void BoxFragmentBuilder::AddResult(const LayoutResult& child_layout_result, const LogicalOffset offset)
{
    AddResult(child_layout_result, offset, std::nullopt, std::nullopt, nullptr);
}

void BoxFragmentBuilder::AddChild(const PhysicalFragment& child, const LogicalOffset& child_offset, const MarginStrut* margin_strut, bool is_self_collapsing,
    std::optional<LogicalOffset> relative_offset, const OofInlineContainer<LogicalOffset>* inline_container)
{
#if DCHECK_IS_ON()
    needs_inflow_bounds_explicitly_set_ = !!relative_offset;
    needs_may_have_descendant_above_block_start_explicitly_set_ = !!relative_offset;
#endif
        ...
        if (box_type_ != PhysicalFragment::BoxType::kInlineBox) {
            if (child.IsLineBox()) {
                ...
                if (box_child.Style().GetPosition() == EPosition::kRelative) {
                    relative_offset = ComputeRelativeOffsetForBoxFragment(box_child, GetWritingDirection(), child_available_size_);
                }
                ...
                if ((child_offset.block_offset < LayoutUnit() && !box_child.IsOutOfFlowPositioned())
                    || (!box_child.IsFormattingContextRoot() && box_child.MayHaveDescendantAboveBlockStart()))
                    may_have_descendant_above_block_start_ = true;
            }
            ...
            if (Node().IsScrollContainer() && !IsFragmentainerBoxType() && !child.IsOutOfFlowPositioned()) {
                BoxStrut margins;
                if (child.IsCSSBox()) {
                    margins = ComputeMarginsFor(child.Style(), child_available_size_.inline_size, GetWritingDirection());
                }
                ...
                if (margin_strut) {
                    ...
                    margins.block_end = is_self_collapsing ? end_margin_strut.Sum() - margin_strut->Sum() : end_margin_strut.Sum();
                }
                ...
                if (!margins.IsEmpty()) {
                    ...
                    PhysicalToLogical<bool> converter(GetWritingDirection(), has_top_overflow, !has_left_overflow, !has_top_overflow, has_left_overflow);

                    if (converter.InlineStart()) {
                        margins.inline_end = margins.inline_end.ClampNegativeToZero();
                        margins.inline_start = std::max(margins.inline_start, -fragment.InlineSize());
                    } else {
                        margins.inline_start = margins.inline_start.ClampNegativeToZero();
                        margins.inline_end = std::max(margins.inline_end, -fragment.InlineSize());
                    }
                    if (converter.BlockStart()) {
                        margins.block_end = margins.block_end.ClampNegativeToZero();
                        margins.block_start = std::max(margins.block_start, -fragment.BlockSize());
                    } else {
                        margins.block_start = margins.block_start.ClampNegativeToZero();
                        margins.block_end = std::max(margins.block_end, -fragment.BlockSize());
                    }
...
void BoxFragmentBuilder::AddBreakToken(const BreakToken* token, bool is_in_parallel_flow)
{
    // If there's a pre-set break token, we shouldn't be here.
    DCHECK(!break_token_);
...
EBreakBetween BoxFragmentBuilder::JoinedBreakBetweenValue(EBreakBetween break_before) const
{
...
void BoxFragmentBuilder::PropagateBreakInfo(const LayoutResult& child_layout_result, LogicalOffset offset)
{
    DCHECK(has_block_fragmentation_);

    // Include the bounds of this child (in the block direction).
    LayoutUnit block_end_in_container
        = offset.block_offset - child_layout_result.AnnotationBlockOffsetAdjustment() + BlockSizeForFragmentation(child_layout_result, writing_direction_);

    block_size_for_fragmentation_ = std::max(block_size_for_fragmentation_, block_end_in_container);
    ...
    const auto* child_box_fragment = DynamicTo<PhysicalBoxFragment>(child_fragment);
    const BlockBreakToken* token = child_box_fragment ? child_box_fragment->GetBreakToken() : nullptr;
    ...
    bool child_is_in_same_flow
        = ((!token || !token->IsAtBlockEnd()) && !child_fragment.IsFloatingOrOutOfFlowPositioned()) || child_layout_result.ShouldForceSameFragmentationFlow();
    ...
    // makes any sense.
    if (GetConstraintSpace().IsPaginated() && ((child_is_in_same_flow && !IsFragmentainerBoxType()) || Node().IsPaginatedRoot())
        && (!child_box_fragment || !child_box_fragment->IsMonolithicOverflowPropagationDisabled())) {
        ...
        if (Node().IsPaginatedRoot() && !child_fragment.HasNonVisibleBlockOverflow()) {
            // The root node is guaranteed to be block-level, so there should be a
            // child box fragment here.
            DCHECK(child_box_fragment);

            LogicalBoxFragment logical_fragment(child_box_fragment->Style().GetWritingDirection(), *child_box_fragment);
            block_size = logical_fragment.BlockEndScrollableOverflow();
        } else {
            LogicalFragment logical_fragment(child_fragment.Style().GetWritingDirection(), child_fragment);
            block_size = logical_fragment.BlockSize();
        }
        LayoutUnit fragment_block_end = offset.block_offset + block_size;
        LayoutUnit fragmentainer_overflow = fragment_block_end - FragmentainerSpaceLeft(*this, /*is_for_children=*/false);
    ...
    if (IsBreakInside(token)) {
        ...
        BreakAppeal appeal_inside = CalculateBreakAppealInside(GetConstraintSpace(), child_layout_result);
        ClampBreakAppeal(appeal_inside);
    }

    if (IsInitialColumnBalancingPass()) {
        PropagateTallestUnbreakableBlockSize(child_layout_result.TallestUnbreakableBlockSize());
    }
    ...
    if (GetConstraintSpace().IsInColumnBfc()) [[unlikely]] {
        if (const auto* child_spanner_path = child_layout_result.GetColumnSpannerPath()) {
            DCHECK(HasInflowChildBreakInside() || !child_layout_result.GetPhysicalFragment().IsBox());
            const auto* spanner_path = MakeGarbageCollected<ColumnSpannerPath>(Node(), child_spanner_path);
    ...
    if (!child_box_fragment->IsFragmentainerBox() && !HasOutOfFlowInFragmentainerSubtree()) {
        SetHasOutOfFlowInFragmentainerSubtree(child_box_fragment->HasOutOfFlowInFragmentainerSubtree());
    }
}

void BoxFragmentBuilder::PropagateChildBreakValues(const LayoutResult& child_layout_result)
{
    ...
    // page names).
    const auto& fragment = child_layout_result.GetPhysicalFragment();
    if (fragment.IsInline() || !fragment.IsBox() || fragment.IsColumnBox() || fragment.IsFloatingOrOutOfFlowPositioned()) {
        return;
    }
    ...
    EBreakBetween break_before = JoinFragmentainerBreakValues(child_layout_result.InitialBreakBefore(), child_style.BreakBefore());
    ...
    EBreakBetween break_after = JoinFragmentainerBreakValues(child_layout_result.FinalBreakAfter(), child_style.BreakAfter());
...
void BoxFragmentBuilder::HandleOofsAndSpecialDescendants()
{
    ...
    if (Style().ScrollMarkerGroup() != EScrollMarkerGroup::kNone && !GetConstraintSpace().IsAnonymous()) {
...
const LayoutResult* BoxFragmentBuilder::ToBoxFragment(WritingMode block_or_line_writing_mode)
{
    ...
    if (box_type_ == PhysicalFragment::kNormalBox && node_ && node_.IsBlockInInline()) [[unlikely]] {
        SetIsBlockInInline();
    }
    ...
    if (has_block_fragmentation_ && node_) [[unlikely]] {
        ...
        if (!PhysicalFragment::IsFragmentainerBoxType(box_type_)) {
            OverflowClipAxes block_axis = GetWritingDirection().IsHorizontal() ? kOverflowClipY : kOverflowClipX;
            if ((To<BlockNode>(node_).GetOverflowClipAxes() & block_axis) || is_block_size_for_fragmentation_clamped_) {
                ...
                block_size_for_fragmentation_ = std::max(block_size_for_fragmentation_, FragmentBlockSize());
            }
    ...
    const PhysicalBoxFragment* fragment = PhysicalBoxFragment::Create(this, block_or_line_writing_mode);
    fragment->CheckType();

    return MakeGarbageCollected<LayoutResult>(LayoutResult::BoxFragmentBuilderPassKey(), std::move(fragment), this);
}

void BoxFragmentBuilder::AdjustFragmentainerDescendant(LogicalOofNodeForFragmentation& descendant, bool only_fixedpos_containing_block)
{
    ...
    // offset.
    if (!only_fixedpos_containing_block && !descendant.containing_block.Fragment()) {
        descendant.containing_block.IncreaseBlockOffset(-previous_consumed_block_size);
        descendant.static_position.offset.block_offset += previous_consumed_block_size;
    }
    ...
    if (!descendant.fixedpos_containing_block.Fragment() && (node_.IsFixedContainer() || descendant.fixedpos_inline_container.container)) {
        descendant.fixedpos_containing_block.IncreaseBlockOffset(-previous_consumed_block_size);
    }
}

void BoxFragmentBuilder::AdjustFixedposContainingBlockForFragmentainerDescendants()
{
...
void BoxFragmentBuilder::AdjustFixedposContainingBlockForInnerMulticols()
{
    ...
    // containing block.
    LayoutUnit previous_consumed_block_size = PreviousBreakToken()->ConsumedBlockSize();
    for (auto& multicol : multicols_with_pending_oofs_) {
        MulticolWithPendingOofs<LogicalOffset>& value = *multicol.value;
        if (!value.fixedpos_containing_block.Fragment() && (node_.IsFixedContainer() || value.fixedpos_inline_container.container)) {
            value.fixedpos_containing_block.IncreaseBlockOffset(-previous_consumed_block_size);
