...
// on paper).
class CORE_EXPORT PageContainerLayoutAlgorithm : public LayoutAlgorithm<BlockNode, BoxFragmentBuilder, BlockBreakToken> {
public:
    PageContainerLayoutAlgorithm(const LayoutAlgorithmParams& params, wtf_size_t page_index, wtf_size_t total_page_count, const AtomicString& page_name,
        const BlockNode& content_node, const CountersAttachmentContext&, const PageAreaLayoutParams&, bool ignore_author_page_style,
    ...
    bool NeedsTotalPageCount() const
    {
        return needs_total_page_count_;
    }

private:
    enum ProgressionDirection { LeftToRight, TopToBottom, RightToLeft, BottomToTop };
    ...
    public:
        ...
        LayoutUnit MinLength() const
        {
            return min_max.min_size + margin_sum;
        }
        LayoutUnit MaxLength() const
        {
            return min_max.max_size + margin_sum;
        }
    ...
    void LayoutPageBorderBox(LogicalSize containing_block_size, LogicalOffset target_offset);
    ...
    bool IsAtTopEdge(EdgeAdjacency mask) const
    {
        return mask & TopEdge;
    }
    bool IsAtLeftEdge(EdgeAdjacency mask) const
    {
        return mask & LeftEdge;
    }
    ...
    void LayoutCornerMarginNode(const ComputedStyle* corner_style, const PhysicalRect&, EdgeAdjacency);
    ...
    void LayoutEdgeMarginNodes(const ComputedStyle* start_box_style, const ComputedStyle* center_box_style, const ComputedStyle* end_box_style,
        const PhysicalRect& edge_rect, EdgeAdjacency);
    ...
    PreferredSizeInfo EdgeMarginNodePreferredSize(const BlockNode& child, LogicalSize containing_block_size, ProgressionDirection) const;
    ...
    void CalculateEdgeMarginBoxSizes(
        PhysicalSize available_physical_size, const BlockNode nodes[3], ProgressionDirection, LayoutUnit final_main_axis_sizes[3]) const;
    ...
    // be non-auto.
    static void ResolveTwoEdgeMarginBoxLengths(const PreferredSizeInfo preferred_main_axis_sizes[3], LayoutUnit available_main_axis_size,
    ...
    void LayoutEdgeMarginNode(
        const BlockNode& child, const PhysicalRect& edge_rect, LayoutUnit main_axis_size, EdgeMarginType, EdgeAdjacency, ProgressionDirection);

    // Resolve margins after layout. This includes adding auto margins to resolve
    // any overconstrainedness.
    PhysicalBoxStrut ResolveMargins(
        const ConstraintSpace& child_space, const ComputedStyle& child_style, PhysicalSize child_size, PhysicalSize available_size, EdgeAdjacency) const;
