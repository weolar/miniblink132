...
public:
    ...
    static BlockBreakToken* CreateBreakBefore(LayoutInputNode node, bool is_forced_break)
    {
    ...
    static BlockBreakToken* CreateRepeated(const BlockNode&, unsigned sequence_number);
    ...
    static BlockBreakToken* CreateForBreakInRepeatedFragment(const BlockNode&, unsigned sequence_number, LayoutUnit consumed_block_size, bool is_at_block_end);
...
#endif
        DCHECK(data_);
        return data_->consumed_block_size + data_->consumed_block_size_legacy_adjustment;
    }
...
private:
    const base::span<const Member<const BreakToken>> ChildBreakTokensInternal() const
    {
...
template <> struct DowncastTraits<BlockBreakToken> {
