...
public:
    ...
    PhysicalOffset MapLocalToAncestor(const LayoutObject*, const LayoutBoxModelObject* ancestor, PhysicalOffset, MapCoordinatesFlags = 0) const;
    gfx::QuadF MapLocalToAncestor(const LayoutObject*, const LayoutBoxModelObject* ancestor, gfx::QuadF, MapCoordinatesFlags = 0) const;
    PhysicalOffset MapAncestorToLocal(const LayoutObject*, const LayoutBoxModelObject* ancestor, PhysicalOffset, MapCoordinatesFlags = 0) const;
    gfx::QuadF MapAncestorToLocal(const LayoutObject*, const LayoutBoxModelObject* ancestor, gfx::QuadF, MapCoordinatesFlags = 0) const;
...
PhysicalOffset MapCoordinatesTest::AdjustForFrameScroll(const PhysicalOffset& point) const
{
...
PhysicalOffset MapCoordinatesTest::MapLocalToAncestor(
    const LayoutObject* object, const LayoutBoxModelObject* ancestor, PhysicalOffset point, MapCoordinatesFlags mode) const
{
    return object->LocalToAncestorPoint(point, ancestor, mode);
}

gfx::QuadF MapCoordinatesTest::MapLocalToAncestor(
    const LayoutObject* object, const LayoutBoxModelObject* ancestor, gfx::QuadF quad, MapCoordinatesFlags mode) const
{
...
TEST_F(MapCoordinatesTest, SimpleText)
{
    ...
    PhysicalOffset mapped_point = MapLocalToAncestor(text, container, PhysicalOffset(10, 30));
...
TEST_F(MapCoordinatesTest, SimpleInline)
{
    ...
    PhysicalOffset mapped_point = MapLocalToAncestor(target, To<LayoutBoxModelObject>(target->Parent()), PhysicalOffset(10, 10));
    EXPECT_EQ(PhysicalOffset(10, 10), mapped_point);
    mapped_point = MapAncestorToLocal(target, To<LayoutBoxModelObject>(target->Parent()), mapped_point);
    EXPECT_EQ(PhysicalOffset(10, 10), mapped_point);
}
...
TEST_F(MapCoordinatesTest, SimpleBlock)
{
    ...
    LayoutObject* target = GetLayoutObjectByElementId("target");
    PhysicalOffset mapped_point = MapLocalToAncestor(target, To<LayoutBoxModelObject>(target->Parent()), PhysicalOffset(100, 100));
...
TEST_F(MapCoordinatesTest, OverflowClip)
{
    ...
    To<Element>(overflow->GetNode())->GetLayoutBoxForScrolling()->GetScrollableArea()->ScrollToAbsolutePosition(gfx::PointF(32, 54));
...
{
    SetBodyInnerHTML("<div><span style='position:relative; left:7px; top:4px;'><br "
                     "id='sibling'>text</span></div>");

    LayoutObject* br = GetLayoutObjectByElementId("sibling");
    LayoutObject* text = br->NextSibling();
    ASSERT_TRUE(text->IsText());
    PhysicalOffset mapped_point = MapLocalToAncestor(text, text->ContainingBlock(), PhysicalOffset(10, 30));
    EXPECT_EQ(PhysicalOffset(10, 30), mapped_point);
...
{
    SetBodyInnerHTML("<span id='target' style='position:relative; left:50px; "
                     "top:100px;'>text</span>");
...
TEST_F(MapCoordinatesTest, RelposInlineInRelposInline)
{
    ...
    PhysicalOffset mapped_point = MapLocalToAncestor(target, containing_block, PhysicalOffset(20, 10));
...
TEST_F(MapCoordinatesTest, RelPosBlock)
{
    ...
    PhysicalOffset mapped_point = MapLocalToAncestor(target, container, PhysicalOffset());
    EXPECT_EQ(PhysicalOffset(106, 106), mapped_point);
    mapped_point = MapAncestorToLocal(target, container, PhysicalOffset(110, 110));
...
TEST_F(MapCoordinatesTest, FixedPos)
{
    ...
    PhysicalOffset mapped_point = MapLocalToAncestor(target, view, PhysicalOffset());
...
TEST_F(MapCoordinatesTest, FixedPosAuto)
{
    ...
    PhysicalOffset mapped_point = MapLocalToAncestor(target, target->ContainingBlock(), PhysicalOffset());
    EXPECT_EQ(PhysicalOffset(36, 61), mapped_point);
    mapped_point = MapAncestorToLocal(target, target->ContainingBlock(), PhysicalOffset(36, 61));
    EXPECT_EQ(PhysicalOffset(), mapped_point);
...
TEST_F(MapCoordinatesTest, FixedPosInFixedPosScrollView)
{
    ...
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0.0, 50), mojom::blink::ScrollType::kProgrammatic);
    ...
    EXPECT_EQ(PhysicalOffset(100, 100), mapped_point);
    mapped_point = MapAncestorToLocal(target, container, PhysicalOffset(100, 100));
    EXPECT_EQ(PhysicalOffset(), mapped_point);
}
...
TEST_F(MapCoordinatesTest, FixedPosInAbsolutePosScrollView)
{
    ...
    mapped_point = MapAncestorToLocal(target, container, PhysicalOffset(100, 150));
    EXPECT_EQ(PhysicalOffset(), mapped_point);
}
...
TEST_F(MapCoordinatesTest, FixedPosInIFrameWhenMainFrameScrolled)
{
    ...
    SetChildFrameHTML("<style>body { margin: 0; } #target { width: 200px; height: 200px; "
                      "position:fixed}</style><div id=target></div>");

    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0.0, 1000), mojom::blink::ScrollType::kProgrammatic);
    UpdateAllLifecyclePhasesForTest();

    Element* target = ChildDocument().getElementById(AtomicString("target"));
    ASSERT_TRUE(target);
    PhysicalOffset mapped_point = MapAncestorToLocal(target->GetLayoutObject(), nullptr, PhysicalOffset(10, 70), kTraverseDocumentBoundaries);
...
TEST_F(MapCoordinatesTest, IFrameTransformed)
{
    ...
    SetChildFrameHTML("<style>body { margin: 0; } #target { width: 200px; "
                      "height: 8000px}</style><div id=target></div>");

    UpdateAllLifecyclePhasesForTest();

    ChildDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0.0, 1000), mojom::blink::ScrollType::kProgrammatic);
    ChildDocument().View()->UpdateAllLifecyclePhasesForTest();

    Element* target = ChildDocument().getElementById(AtomicString("target"));
    ASSERT_TRUE(target);
    PhysicalOffset mapped_point = MapAncestorToLocal(target->GetLayoutObject(), nullptr, PhysicalOffset(200, 200), kTraverseDocumentBoundaries);
...
TEST_F(MapCoordinatesTest, FixedPosInScrolledIFrameWithTransform)
{
    ...
    Element* target = ChildDocument().getElementById(AtomicString("target"));
    ASSERT_TRUE(target);
    PhysicalOffset mapped_point = MapAncestorToLocal(target->GetLayoutObject(), nullptr, PhysicalOffset(0, 0), kTraverseDocumentBoundaries);
...
TEST_F(MapCoordinatesTest, MulticolWithText)
{
    ...
    auto* const multicol = To<LayoutBlockFlow>(GetLayoutBoxByElementId("multicol"));
    ...
    PhysicalOffset mapped_point = MapLocalToAncestor(target, flow_thread, PhysicalOffset(10, 70));
    EXPECT_EQ(PhysicalOffset(10, 70), mapped_point);
    mapped_point = MapAncestorToLocal(target, flow_thread, mapped_point);
    EXPECT_EQ(PhysicalOffset(10, 70), mapped_point);

    mapped_point = MapLocalToAncestor(flow_thread, multicol, PhysicalOffset(10, 70));
...
TEST_F(MapCoordinatesTest, MulticolWithBlock)
{
    ...
    mapped_point = MapLocalToAncestor(flow_thread, container, PhysicalOffset(10, 120));
    EXPECT_EQ(PhysicalOffset(125, 35), mapped_point);
...
TEST_F(MapCoordinatesTest, MulticolWithBlockAbove)
{
    ...
    mapped_point = MapLocalToAncestor(flow_thread, container, PhysicalOffset(0, -50));
    EXPECT_EQ(PhysicalOffset(0, -50), mapped_point);
    mapped_point = MapAncestorToLocal(flow_thread, container, mapped_point);
    EXPECT_EQ(PhysicalOffset(0, -50), mapped_point);
}
...
TEST_F(MapCoordinatesTest, NestedMulticolWithBlock)
{
    ...
    PhysicalOffset mapped_point = MapLocalToAncestor(target, outer_multicol, PhysicalOffset());
    EXPECT_EQ(PhysicalOffset(435, 115), mapped_point);
    mapped_point = MapAncestorToLocal(target, outer_multicol, mapped_point);
    EXPECT_EQ(PhysicalOffset(), mapped_point);

    // Walk each ancestor in the chain separately, to verify each step on the way.
    mapped_point = MapLocalToAncestor(target, inner_flow_thread, PhysicalOffset());
    EXPECT_EQ(PhysicalOffset(0, 630), mapped_point);
    mapped_point = MapAncestorToLocal(target, inner_flow_thread, mapped_point);
    EXPECT_EQ(PhysicalOffset(), mapped_point);

    mapped_point = MapLocalToAncestor(inner_flow_thread, inner_multicol, PhysicalOffset(0, 630));
    EXPECT_EQ(PhysicalOffset(140, 305), mapped_point);
    mapped_point = MapAncestorToLocal(inner_flow_thread, inner_multicol, mapped_point);
    EXPECT_EQ(PhysicalOffset(0, 630), mapped_point);

    mapped_point = MapLocalToAncestor(inner_multicol, outer_flow_thread, PhysicalOffset(140, 305));
    EXPECT_EQ(PhysicalOffset(140, 315), mapped_point);
    mapped_point = MapAncestorToLocal(inner_multicol, outer_flow_thread, mapped_point);
    EXPECT_EQ(PhysicalOffset(140, 305), mapped_point);

    mapped_point = MapLocalToAncestor(outer_flow_thread, outer_multicol, PhysicalOffset(140, 315));
    EXPECT_EQ(PhysicalOffset(435, 115), mapped_point);
    mapped_point = MapAncestorToLocal(outer_flow_thread, outer_multicol, mapped_point);
    EXPECT_EQ(PhysicalOffset(140, 315), mapped_point);
}
...
TEST_F(MapCoordinatesTest, MulticolWithAbsPosInRelPos)
{
    ...
    PhysicalOffset mapped_point = MapLocalToAncestor(target, multicol, PhysicalOffset());
    ...
    mapped_point = MapLocalToAncestor(relpos, flow_thread, PhysicalOffset(25, 25));
    EXPECT_EQ(PhysicalOffset(29, 139), mapped_point);
    mapped_point = MapAncestorToLocal(relpos, flow_thread, mapped_point);
    EXPECT_EQ(PhysicalOffset(25, 25), mapped_point);

    mapped_point = MapLocalToAncestor(flow_thread, multicol, PhysicalOffset(29, 139));
    EXPECT_EQ(PhysicalOffset(144, 54), mapped_point);
    mapped_point = MapAncestorToLocal(flow_thread, multicol, mapped_point);
    EXPECT_EQ(PhysicalOffset(29, 139), mapped_point);
}
...
TEST_F(MapCoordinatesTest, MulticolWithAbsPosInInlineRelPos)
{
    ...
    mapped_point = MapLocalToAncestor(container, flow_thread, PhysicalOffset(25, 25));
...
TEST_F(MapCoordinatesTest, MulticolRtl)
{
    ...
    mapped_point = MapLocalToAncestor(flow_thread, container, PhysicalOffset(0, 200));
    EXPECT_EQ(PhysicalOffset(100, 0), mapped_point);
    mapped_point = MapAncestorToLocal(flow_thread, container, mapped_point);
    EXPECT_EQ(PhysicalOffset(0, 200), mapped_point);
}
...
TEST_F(MapCoordinatesTest, FlippedBlocksWritingModeWithText)
{
    ...
    PhysicalOffset mapped_point = MapLocalToAncestor(text, text->ContainingBlock(), PhysicalOffset(75, 10));
    EXPECT_EQ(PhysicalOffset(75, 10), mapped_point);
    mapped_point = MapAncestorToLocal(text, text->ContainingBlock(), mapped_point);
    EXPECT_EQ(PhysicalOffset(75, 10), mapped_point);

    // Map to a container further up in the tree.
    mapped_point = MapLocalToAncestor(text, text->ContainingBlock()->ContainingBlock(), PhysicalOffset(75, 10));
    EXPECT_EQ(PhysicalOffset(80, 10), mapped_point);
    mapped_point = MapAncestorToLocal(text, text->ContainingBlock()->ContainingBlock(), mapped_point);
    EXPECT_EQ(PhysicalOffset(75, 10), mapped_point);
}
...
TEST_F(MapCoordinatesTest, FlippedBlocksWritingModeWithInline)
{
    ...
    PhysicalOffset mapped_point = MapLocalToAncestor(target, To<LayoutBoxModelObject>(target->Parent()), PhysicalOffset(75, 10));
    EXPECT_EQ(PhysicalOffset(75, 10), mapped_point);
    mapped_point = MapAncestorToLocal(target, To<LayoutBoxModelObject>(target->Parent()), mapped_point);
    EXPECT_EQ(PhysicalOffset(75, 10), mapped_point);
    ...
    // everything is in physical coordinates.
    mapped_point = MapLocalToAncestor(To<LayoutBoxModelObject>(target->Parent()), target->ContainingBlock(), PhysicalOffset(75, 10));
    EXPECT_EQ(PhysicalOffset(75, 10), mapped_point);
    mapped_point = MapAncestorToLocal(To<LayoutBoxModelObject>(target->Parent()), target->ContainingBlock(), mapped_point);
    EXPECT_EQ(PhysicalOffset(75, 10), mapped_point);

    // Now map from the innermost inline to the nearest container in one go.
    mapped_point = MapLocalToAncestor(target, target->ContainingBlock(), PhysicalOffset(75, 10));
    EXPECT_EQ(PhysicalOffset(75, 10), mapped_point);
    mapped_point = MapAncestorToLocal(target, target->ContainingBlock(), mapped_point);
    EXPECT_EQ(PhysicalOffset(75, 10), mapped_point);

    // Map to a container further up in the tree.
    mapped_point = MapLocalToAncestor(target, target->ContainingBlock()->ContainingBlock(), PhysicalOffset(75, 10));
    EXPECT_EQ(PhysicalOffset(82, 10), mapped_point);
    mapped_point = MapAncestorToLocal(target, target->ContainingBlock()->ContainingBlock(), mapped_point);
    EXPECT_EQ(PhysicalOffset(75, 10), mapped_point);
}
...
static bool QuadsAlmostEqual(const gfx::QuadF& expected, const gfx::QuadF& actual)
{
    return FloatValuesAlmostEqual(expected.p1().x(), actual.p1().x()) && FloatValuesAlmostEqual(expected.p1().y(), actual.p1().y())
        && FloatValuesAlmostEqual(expected.p2().x(), actual.p2().x()) && FloatValuesAlmostEqual(expected.p2().y(), actual.p2().y())
        && FloatValuesAlmostEqual(expected.p3().x(), actual.p3().x()) && FloatValuesAlmostEqual(expected.p3().y(), actual.p3().y())
        && FloatValuesAlmostEqual(expected.p4().x(), actual.p4().x()) && FloatValuesAlmostEqual(expected.p4().y(), actual.p4().y());
}
...
TEST_F(MapCoordinatesTest, Transforms)
{
    ...
    gfx::QuadF initial_quad(gfx::PointF(0, 0), gfx::PointF(200, 0), gfx::PointF(200, 200), gfx::PointF(0, 200));
    gfx::QuadF mapped_quad = MapLocalToAncestor(target, container, initial_quad);
    EXPECT_QUADF_EQ(gfx::QuadF(gfx::PointF(200, 0), gfx::PointF(200, 200), gfx::PointF(0, 200), gfx::PointF(0, 0)), mapped_quad);
    ...
    EXPECT_QUADF_EQ(gfx::QuadF(gfx::PointF(0, 0), gfx::PointF(200, 0), gfx::PointF(200, 200), gfx::PointF(0, 200)), mapped_quad);
    mapped_quad = MapAncestorToLocal(target, inner_transform, mapped_quad);
    EXPECT_QUADF_EQ(initial_quad, mapped_quad);

    initial_quad = gfx::QuadF(gfx::PointF(0, 0), gfx::PointF(200, 0), gfx::PointF(200, 200), gfx::PointF(0, 200));
    mapped_quad = MapLocalToAncestor(inner_transform, outer_transform, initial_quad);
    // Clockwise rotation by 45 degrees.
    EXPECT_QUADF_EQ(gfx::QuadF(gfx::PointF(100, -41.42), gfx::PointF(241.42, 100), gfx::PointF(100, 241.42), gfx::PointF(-41.42, 100)), mapped_quad);
    mapped_quad = MapAncestorToLocal(inner_transform, outer_transform, mapped_quad);
    EXPECT_QUADF_EQ(initial_quad, mapped_quad);

    initial_quad = gfx::QuadF(gfx::PointF(100, -41.42), gfx::PointF(241.42, 100), gfx::PointF(100, 241.42), gfx::PointF(-41.42, 100));
...
TEST_F(MapCoordinatesTest, SVGForeignObject)
{
    ...
    // <svg>
    mapped_point = MapAncestorToLocal(target->Parent()->Parent()->Parent(), container, PhysicalOffset(250, 50));
    EXPECT_EQ(PhysicalOffset(250, 50), mapped_point);
    // <g>
    mapped_point = MapAncestorToLocal(target->Parent()->Parent(), container, PhysicalOffset(250, 50));
    EXPECT_EQ(PhysicalOffset(25, -25), mapped_point);
    // <foreignObject>
    mapped_point = MapAncestorToLocal(target->Parent(), container, PhysicalOffset(250, 50));
...
TEST_F(MapCoordinatesTest, LocalToAbsoluteTransform)
{
    ...
    auto* container = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("container"));
    ...
    EXPECT_EQ(gfx::PointF(20.0f, 40.0f), child_matrix.ProjectPoint(gfx::PointF(10.0f, 20.0f)));
}
...
TEST_F(MapCoordinatesTest, LocalToAncestorTransform)
{
    ...
    auto* container = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("container"));
    auto* rotate1 = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("rotate1"));
    auto* rotate2 = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("rotate2"));
    LayoutObject* child = GetLayoutObjectByElementId("child");
    ...
    EXPECT_NEAR(0.0, matrix.ProjectPoint(gfx::PointF(100.0, 0.0)).x(), LayoutUnit::Epsilon());
    EXPECT_NEAR(100.0, matrix.ProjectPoint(gfx::PointF(100.0, 0.0)).y(), LayoutUnit::Epsilon());

    // Rotate (100, 0) 135 degrees to (-70.7, 70.7)
    matrix = child->LocalToAncestorTransform(container);
    EXPECT_FALSE(matrix.IsIdentity());
    EXPECT_TRUE(matrix.Is2dTransform());
    EXPECT_NEAR(-100.0 * sqrt(2.0) / 2.0, matrix.ProjectPoint(gfx::PointF(100.0, 0.0)).x(), LayoutUnit::Epsilon());
    EXPECT_NEAR(100.0 * sqrt(2.0) / 2.0, matrix.ProjectPoint(gfx::PointF(100.0, 0.0)).y(), LayoutUnit::Epsilon());
}
...
TEST_F(MapCoordinatesTest, LocalToAbsoluteTransformFlattens)
{
    ...
    // themselves.
    EXPECT_NEAR(100.0, matrix.MapPoint(gfx::PointF(100.0, 50.0)).x(), LayoutUnit::Epsilon());
    EXPECT_NEAR(50.0, matrix.MapPoint(gfx::PointF(100.0, 50.0)).y(), LayoutUnit::Epsilon());
    EXPECT_NEAR(50.0, matrix.MapPoint(gfx::PointF(50.0, 100.0)).x(), LayoutUnit::Epsilon());
    EXPECT_NEAR(100.0, matrix.MapPoint(gfx::PointF(50.0, 100.0)).y(), LayoutUnit::Epsilon());
    ...
    EXPECT_NEAR(50.0, matrix.MapPoint(gfx::PointF(100.0, 50.0)).x(), LayoutUnit::Epsilon());
    EXPECT_NEAR(50.0, matrix.MapPoint(gfx::PointF(100.0, 50.0)).y(), LayoutUnit::Epsilon());
    EXPECT_NEAR(25.0, matrix.MapPoint(gfx::PointF(50.0, 100.0)).x(), LayoutUnit::Epsilon());
    EXPECT_NEAR(100.0, matrix.MapPoint(gfx::PointF(50.0, 100.0)).y(), LayoutUnit::Epsilon());
}
...
TEST_F(MapCoordinatesTest, Transform3DWithOffset)
{
    ...
    EXPECT_EQ(gfx::QuadF(gfx::RectF(0, 100, 100, 100)), MapLocalToAncestor(target, nullptr, gfx::QuadF(gfx::RectF(0, 0, 100, 100))));
}
...
TEST_F(MapCoordinatesTest, Transform3DWithOffset2)
{
    ...
    auto* target = GetLayoutObjectByElementId("target");
    EXPECT_EQ(gfx::QuadF(gfx::RectF(0, 200, 200, 200)), MapLocalToAncestor(target, nullptr, gfx::QuadF(gfx::RectF(0, 0, 100, 100))));
}
...
TEST_F(MapCoordinatesTest, IgnoreScrollOffset)
{
    ...
    EXPECT_EQ(PhysicalOffset(0, 10), MapLocalToAncestor(box, scroller, PhysicalOffset()));
    EXPECT_EQ(PhysicalOffset(0, 10), MapLocalToAncestor(box, scroller, PhysicalOffset(), kIgnoreScrollOffset));

    To<Element>(scroller->GetNode())->GetLayoutBoxForScrolling()->GetScrollableArea()->ScrollToAbsolutePosition(gfx::PointF(0, 50));

    EXPECT_EQ(PhysicalOffset(0, -40), MapLocalToAncestor(box, scroller, PhysicalOffset()));
    EXPECT_EQ(PhysicalOffset(0, 10), MapLocalToAncestor(box, scroller, PhysicalOffset(), kIgnoreScrollOffset));
}
...
TEST_F(MapCoordinatesTest, IgnoreScrollOffsetForInline)
{
    ...
    EXPECT_EQ(PhysicalOffset(0, 10), MapLocalToAncestor(box, scroller, PhysicalOffset()));
    EXPECT_EQ(PhysicalOffset(0, 60), MapLocalToAncestor(box, scroller, PhysicalOffset(), kIgnoreScrollOffset));
}
...
TEST_F(MapCoordinatesTest, IgnoreScrollOffsetWithWritingModes)
{
    ...
    EXPECT_EQ(PhysicalOffset(90, 10), MapLocalToAncestor(box, scroller, PhysicalOffset()));
    EXPECT_EQ(PhysicalOffset(1990, 10), MapLocalToAncestor(box, scroller, PhysicalOffset(), kIgnoreScrollOffset));

    scroll_element->GetLayoutBoxForScrolling()->GetScrollableArea()->ScrollToAbsolutePosition(gfx::PointF(0, 50));

    EXPECT_EQ(PhysicalOffset(1990, -40), MapLocalToAncestor(box, scroller, PhysicalOffset()));
    EXPECT_EQ(PhysicalOffset(1990, 10), MapLocalToAncestor(box, scroller, PhysicalOffset(), kIgnoreScrollOffset));

    scroll_element->GetLayoutBoxForScrolling()->GetScrollableArea()->ScrollToAbsolutePosition(gfx::PointF(1900, 50));

    EXPECT_EQ(PhysicalOffset(90, -40), MapLocalToAncestor(box, scroller, PhysicalOffset()));
    EXPECT_EQ(PhysicalOffset(1990, 10), MapLocalToAncestor(box, scroller, PhysicalOffset(), kIgnoreScrollOffset));
}
...
TEST_F(MapCoordinatesTest, FixedPositionWithScrollOffset)
{
    ...
    EXPECT_EQ(expected, MapLocalToAncestor(target, &GetLayoutView(), PhysicalOffset()));
    EXPECT_EQ(expected, MapLocalToAncestor(target, nullptr, PhysicalOffset(), kIgnoreScrollOffset));
    EXPECT_EQ(expected, MapLocalToAncestor(target, &GetLayoutView(), PhysicalOffset(), kIgnoreScrollOffset));

    // Scroll offset doesn't affect MapLocalToAncestor(), regardless of
    // kIgnoreScrollOffset.
    GetLayoutView().GetScrollableArea()->ScrollToAbsolutePosition(gfx::PointF(0, 400));
    EXPECT_EQ(expected, MapLocalToAncestor(target, nullptr, PhysicalOffset()));
    EXPECT_EQ(expected, MapLocalToAncestor(target, &GetLayoutView(), PhysicalOffset()));
    EXPECT_EQ(expected, MapLocalToAncestor(target, nullptr, PhysicalOffset(), kIgnoreScrollOffset));
    EXPECT_EQ(expected, MapLocalToAncestor(target, &GetLayoutView(), PhysicalOffset(), kIgnoreScrollOffset));
...
TEST_F(MapCoordinatesTest, FixedPositionWithScrollOffsetVerticalRL)
{
    ...
    // Scroll offset doesn't affect MapLocalToAncestor(), regardless of
    // kIgnoreScrollOffset.
    GetLayoutView().GetScrollableArea()->ScrollToAbsolutePosition(gfx::PointF(400, 0));
    EXPECT_EQ(expected, MapLocalToAncestor(target, nullptr, PhysicalOffset()));
    EXPECT_EQ(expected, MapLocalToAncestor(target, &GetLayoutView(), PhysicalOffset()));
    EXPECT_EQ(expected, MapLocalToAncestor(target, nullptr, PhysicalOffset(), kIgnoreScrollOffset));
    EXPECT_EQ(expected, MapLocalToAncestor(target, &GetLayoutView(), PhysicalOffset(), kIgnoreScrollOffset));
...
TEST_F(MapCoordinatesTest, FixedPositionUnderTransformWithScrollOffset)
{
    ...
    EXPECT_EQ(expected_scrolled, MapLocalToAncestor(target, nullptr, PhysicalOffset()));
    EXPECT_EQ(expected_scrolled, MapLocalToAncestor(target, &GetLayoutView(), PhysicalOffset()));
    EXPECT_EQ(expected, MapLocalToAncestor(target, nullptr, PhysicalOffset(), kIgnoreScrollOffset));
    EXPECT_EQ(expected, MapLocalToAncestor(target, &GetLayoutView(), PhysicalOffset(), kIgnoreScrollOffset));
}
...
TEST_F(MapCoordinatesTest, IgnoreScrollOffsetWithWritingModesAndNonOverlayScrollbar)
{
    ...
    EXPECT_EQ(PhysicalOffset(75, 10), MapLocalToAncestor(box, scroller, PhysicalOffset()));
    EXPECT_EQ(PhysicalOffset(1990, 10), MapLocalToAncestor(box, scroller, PhysicalOffset(), kIgnoreScrollOffset));

    To<Element>(scroller->GetNode())->GetLayoutBoxForScrolling()->GetScrollableArea()->ScrollToAbsolutePosition(gfx::PointF(0, 0));
    ...
    EXPECT_EQ(PhysicalOffset(1990, 10), MapLocalToAncestor(box, scroller, PhysicalOffset()));
    EXPECT_EQ(PhysicalOffset(1990, 10), MapLocalToAncestor(box, scroller, PhysicalOffset(), kIgnoreScrollOffset));
}
