...
TEST_F(LayoutRubyBaseTest, AddChildNoBlockChildren)
{
    ...
    EXPECT_EQ(EDisplay::kInlineTable, first_child->NextSibling()->StyleRef().Display());
}
...
TEST_F(LayoutRubyBaseTest, AddSpecialWithTableInternalDisplayNoBlockChildren)
{
    ...
    input->setAttribute(html_names::kStyleAttr, AtomicString("display:table-column; appearance:none"));
...
TEST_F(LayoutRubyBaseTest, ChangeToRubyNoBlockChildren)
{
    ...
    GetElementById("target")->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kRuby);
    UpdateAllLifecyclePhasesForTest();
