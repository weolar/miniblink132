...
public:
    ...
    const PhysicalOffset& LocalPoint() const
    {
        return local_point_;
    }
    ...
    void SetNodeAndPosition(Node*, const PhysicalBoxFragment*, const PhysicalOffset&);
    ...
    bool IsOverResizer() const
    {
        return is_over_resizer_;
    }
    ...
    bool IsOverScrollCorner() const
    {
        return is_over_scroll_corner_;
    }
    ...
    ListBasedHitTestBehavior AddNodeToListBasedTestResult(Node*, const HitTestLocation&, const PhysicalRect& = PhysicalRect());
    ListBasedHitTestBehavior AddNodeToListBasedTestResult(Node*, const HitTestLocation&, const gfx::QuadF& quad);
    ListBasedHitTestBehavior AddNodeToListBasedTestResult(Node*, const HitTestLocation&, const cc::Region&);
    ...
    // location.
    HitTestLocation ResolveRectBasedTest(Node* resolved_inner_node, const PhysicalOffset& resolved_point_in_main_frame);

private:
    NodeSet& MutableListBasedTestResult(); // See above.
    HTMLMediaElement* MediaElement() const;
    std::tuple<bool, ListBasedHitTestBehavior> AddNodeToListBasedTestResultInternal(Node* node, const HitTestLocation& location);
