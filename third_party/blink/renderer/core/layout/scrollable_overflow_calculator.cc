...
// static
PhysicalRect ScrollableOverflowCalculator::RecalculateScrollableOverflowForFragment(const PhysicalBoxFragment& fragment, bool has_block_fragmentation)
{
    const BlockNode node(const_cast<LayoutBox*>(To<LayoutBox>(fragment.GetSelfOrContainerLayoutObject())));
    DCHECK(!node.IsReplaced() || node.IsMedia());

    const WritingDirectionMode writing_direction = node.Style().GetWritingDirection();

    ScrollableOverflowCalculator calculator(
        node, fragment.IsCSSBox(), has_block_fragmentation, fragment.Borders(), fragment.Scrollbar(), fragment.Padding(), fragment.Size(), writing_direction);
    ...
    for (const auto& child : fragment.PostLayoutChildren()) {
        ...
        if (box_fragment->IsFragmentainerBox()) {
            ...
            // directly.
            PhysicalRect child_overflow = RecalculateScrollableOverflowForFragment(*box_fragment, has_block_fragmentation);
...
ScrollableOverflowCalculator::ScrollableOverflowCalculator(const BlockNode& node, bool is_css_box, bool has_block_fragmentation,
    const PhysicalBoxStrut& borders, const PhysicalBoxStrut& scrollbar, const PhysicalBoxStrut& padding, PhysicalSize size,
    ...
    , size_(size)
{
    ...
    padding_rect_ = { PhysicalOffset(border_scrollbar.left, border_scrollbar.top),
        PhysicalSize(
            (size_.width - border_scrollbar.HorizontalSum()).ClampNegativeToZero(), (size_.height - border_scrollbar.VerticalSum()).ClampNegativeToZero()) };
    scrollable_overflow_ = padding_rect_;
}

const PhysicalRect ScrollableOverflowCalculator::Result(const std::optional<PhysicalRect> inflow_bounds)
{
...
template <typename Items> void ScrollableOverflowCalculator::AddItemsInternal(const LayoutObject* layout_object, const Items& items)
{
...
void ScrollableOverflowCalculator::AddItems(const LayoutObject* layout_object, const FragmentItemsBuilder::ItemWithOffsetList& items)
{
    AddItemsInternal(layout_object, items);
}

void ScrollableOverflowCalculator::AddItems(const PhysicalBoxFragment& box_fragment, const FragmentItems& items)
{
    AddItemsInternal(box_fragment.GetLayoutObject(), items.Items());
}

PhysicalRect ScrollableOverflowCalculator::AdjustOverflowForHanging(const PhysicalRect& line_rect, PhysicalRect overflow)
{
...
PhysicalRect ScrollableOverflowCalculator::AdjustOverflowForScrollOrigin(const PhysicalRect& overflow)
{
    LayoutUnit left_offset
        = has_left_overflow_ ? std::min(padding_rect_.Right(), overflow.offset.left) : std::max(padding_rect_.offset.left, overflow.offset.left);

    LayoutUnit right_offset = has_left_overflow_ ? std::min(padding_rect_.Right(), overflow.Right()) : std::max(padding_rect_.offset.left, overflow.Right());

    LayoutUnit top_offset = has_top_overflow_ ? std::min(padding_rect_.Bottom(), overflow.offset.top) : std::max(padding_rect_.offset.top, overflow.offset.top);

    LayoutUnit bottom_offset = has_top_overflow_ ? std::min(padding_rect_.Bottom(), overflow.Bottom()) : std::max(padding_rect_.offset.top, overflow.Bottom());

    return { PhysicalOffset(left_offset, top_offset), PhysicalSize(right_offset - left_offset, bottom_offset - top_offset) };
}

PhysicalRect ScrollableOverflowCalculator::ScrollableOverflowForPropagation(const PhysicalBoxFragment& child_fragment)
{
    ...
    bool ignore_scrollable_overflow = child_fragment.ShouldApplyLayoutContainment() || child_fragment.IsInlineBox()
        || (child_fragment.ShouldClipOverflowAlongBothAxis() && !child_fragment.ShouldApplyOverflowClipMargin());
    ...
    if (std::optional<gfx::Transform> transform = node_.GetTransformForChildFragment(child_fragment, size_)) {
    ...
    if (has_block_fragmentation_ && child_fragment.IsOutOfFlowPositioned()) {
        ...
        for (const LayoutObject* walker = child_fragment.GetLayoutObject()->ContainingBlock(&skip_info);
             walker != node_.GetLayoutBox() && !skip_info.AncestorSkipped(); walker = walker->ContainingBlock(&skip_info)) {
