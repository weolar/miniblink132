...
#define EXPECT_INVALID_EXCLUDED_INTERVAL(shape, top, height)                                                                                                   \
    do {                                                                                                                                                       \
        auto segment = shape.GetExcludedInterval(LayoutUnit(top), LayoutUnit(height));                                                                         \
        ASSERT_FALSE(segment.is_valid);                                                                                                                        \
        EXPECT_FLOAT_EQ(0, segment.logical_left);                                                                                                              \
        EXPECT_FLOAT_EQ(0, segment.logical_right);                                                                                                             \
    } while (false)

#define EXPECT_EXCLUDED_INTERVAL(shape, top, height, expected_left, expected_right)                                                                            \
    do {                                                                                                                                                       \
        auto segment = shape.GetExcludedInterval(LayoutUnit(top), LayoutUnit(height));                                                                         \
...
{
    test::TaskEnvironment task_environment;
    EXPECT_EQ(LogicalRect(LayoutUnit(-2.25f), LayoutUnit(-2.125f), LayoutUnit(7), LayoutUnit(9.75f)),
        EllipseShape(gfx::PointF(1.25f, 2.75f), 3.5f, 4.875f).ShapeMarginLogicalBoundingBox());
    EXPECT_EQ(LogicalRect(LayoutUnit::Min(), LayoutUnit(), LayoutUnit::Max(), LayoutUnit()),
        EllipseShape(gfx::PointF(), 1e20f, 1e-20f).ShapeMarginLogicalBoundingBox());
}
