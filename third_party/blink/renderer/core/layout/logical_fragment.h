...
public:
    LogicalFragment(WritingDirectionMode writing_direction, const PhysicalFragment& physical_fragment)
    ...
    {
        return writing_direction_.IsHorizontal() ? physical_fragment_.Size().height : physical_fragment_.Size().width;
    }
    LogicalSize Size() const
    {
        return physical_fragment_.Size().ConvertToLogical(writing_direction_.GetWritingMode());
    }
