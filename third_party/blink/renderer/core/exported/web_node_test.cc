...
protected:
    ...
    WebNode Root()
    {
        return WebNode(GetDocument().documentElement());
    }
};
...
{
    SetInnerHTML("<body><LABEL></LABEL><svg "
...
TEST_F(WebNodeTest, CannotFindTextInElementThatIsNotAContainer)
{
    ...
    EXPECT_FALSE(element.IsNull());
    EXPECT_TRUE(element.FindTextInElementWith("Hello world", [](const WebString&) { return true; }).IsEmpty());
}
...
TEST_F(WebNodeTest, CanFindTextInElementThatIsAContainer)
{
    ...
    EXPECT_FALSE(element.IsNull());
    EXPECT_EQ(WebString(" Hello world! "), element.FindTextInElementWith("Hello world", [](const WebString&) { return true; }));
}
...
TEST_F(WebNodeTest, CanFindTextNodesThatAreContainers)
{
    ...
    WebVector<WebNode> nodes = element.FindAllTextNodesMatchingRegex("^ Hello world! $");
...
TEST_F(WebNodeTest, CannotFindTextInElementIfValidatorRejectsIt)
{
    ...
    EXPECT_FALSE(element.IsNull());
    EXPECT_TRUE(element.FindTextInElementWith("Hello world", [](const WebString&) { return false; }).IsEmpty());
}
...
TEST_F(WebNodeTest, CannotFindTextNodesInNonTextInputElement)
{
    ...
    EXPECT_FALSE(element.IsNull());
    EXPECT_TRUE(element.FindAllTextNodesMatchingRegex("^ HeLLo WoRLd! $").empty());
}
...
TEST_F(WebNodeTest, AddEventListener)
{
    ...
    auto set_caret = [&](int caret_position) {
        ...
        AddScript(String(base::StringPrintf("document.getElementById('field').setSelectionRange(%d, %d)", caret_position, caret_position)));
        task_environment().RunUntilIdle();
    };

    focus();
    {
        auto remove_listener = Root().AddEventListener(WebNode::EventType::kSelectionchange, handler.Get());
        set_caret(1);
        set_caret(2);
