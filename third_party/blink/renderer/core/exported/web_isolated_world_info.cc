...
void SetIsolatedWorldInfo(int32_t world_id, const WebIsolatedWorldInfo& info)
{
    ...
    scoped_refptr<SecurityOrigin> security_origin = info.security_origin.Get() ? info.security_origin.Get()->IsolatedCopy() : nullptr;
    ...
    DOMWrapperWorld::SetNonMainWorldHumanReadableName(world_id, info.human_readable_name);
    IsolatedWorldCSP::Get().SetContentSecurityPolicy(world_id, info.content_security_policy, security_origin);
}
