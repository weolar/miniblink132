...
TEST(WebDragDataTest, items)
{
    ...
    // Native file.
    data_object->Add(MakeGarbageCollected<File>(&context.GetExecutionContext(), "/native/path"));
    // Blob file.
    data_object->Add(MakeGarbageCollected<File>("name", base::Time::UnixEpoch(), BlobDataHandle::Create()));

    // User visible snapshot file.
    {
        FileMetadata metadata;
        metadata.platform_path = "/native/visible/snapshot";
        data_object->Add(File::CreateForFileSystemFile(&context.GetExecutionContext(), "name", metadata, File::kIsUserVisible));
    }
    ...
    // Not user visible snapshot file.
    {
        ...
        data_object->Add(File::CreateForFileSystemFile(&context.GetExecutionContext(), "name", metadata, File::kIsNotUserVisible));
    }
    ...
    // User visible file system URL file.
    {
        ...
        data_object->Add(File::CreateForFileSystemFile(url, metadata, File::kIsUserVisible, BlobDataHandle::Create()));
    }
    ...
    // Not user visible file system URL file.
    {
        ...
        data_object->Add(File::CreateForFileSystemFile(url, metadata, File::kIsNotUserVisible, BlobDataHandle::Create()));
    }
...

    {
        ...
        ASSERT_TRUE(item != nullptr);
        EXPECT_EQ("filesystem:http://example.com/isolated/hash/visible-non-native-file", item->url);
...

    {
        ...
        ASSERT_TRUE(item != nullptr);
        EXPECT_EQ("filesystem:http://example.com/isolated/hash/"
                  "not-visible-non-native-file",
            item->url);
        EXPECT_EQ(1234, item->size);
    }
