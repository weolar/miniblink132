...
TEST_F(WebDisallowTransitionScopeTest, TestDisallowTransition)
{
...

    {
        ...
        EXPECT_DEATH_IF_SUPPORTED(core_doc->Lifecycle().EnsureStateAtMost(DocumentLifecycle::kVisualUpdatePending), "Cannot rewind document lifecycle");
    }

    // Legal transition.
    core_doc->Lifecycle().EnsureStateAtMost(DocumentLifecycle::kVisualUpdatePending);
}
#endif
