...
{
    return CONTAINS_STRING(arg, "Effective playback rate changed from " + std::string(old_rate_string) + " to " + std::string(new_rate_string));
}
...
public:
    ...
    MOCK_METHOD6(DidMediaMetadataChange, void(bool, bool, media::AudioCodec, media::VideoCodec, media::MediaContentType, bool));
    MOCK_METHOD4(DidPlayerMediaPositionStateChange, void(double, base::TimeDelta, base::TimeDelta position, bool end_of_media));
...
class MockWebMediaPlayerEncryptedMediaClient : public WebMediaPlayerEncryptedMediaClient {
public:
    MockWebMediaPlayerEncryptedMediaClient() = default;

    MockWebMediaPlayerEncryptedMediaClient(const MockWebMediaPlayerEncryptedMediaClient&) = delete;
    MockWebMediaPlayerEncryptedMediaClient& operator=(const MockWebMediaPlayerEncryptedMediaClient&) = delete;

    MOCK_METHOD3(Encrypted, void(media::EmeInitDataType, const unsigned char*, unsigned));
...
public:
    ...
    MOCK_METHOD4(DidMediaMetadataChange, void(int, bool, bool, media::MediaContentType));
    ...
    void SetIdleForTesting(bool is_idle)
    {
        is_idle_ = is_idle;
    }
...
public:
    MockVideoFrameCompositor(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner)
    ...
    MOCK_METHOD0(GetLastPresentedFrameMetadata, std::unique_ptr<WebMediaPlayer::VideoFramePresentationMetadata>());
    MOCK_METHOD0(GetCurrentFrameOnAnyThread, scoped_refptr<media::VideoFrame>());
    MOCK_METHOD1(UpdateCurrentFrameIfStale, void(VideoFrameCompositor::UpdateType));
    MOCK_METHOD3(EnableSubmission, void(const viz::SurfaceId&, media::VideoTransformation, bool));
};

} // namespace

class WebMediaPlayerImplTest : public testing::Test, private WebTestingSupport::WebScopedMockScrollbars {
public:
    WebMediaPlayerImplTest()
        : media_thread_("MediaThreadForTest")
        , context_provider_(viz::TestContextProvider::Create())
        , audio_parameters_(media::TestAudioParameters::Normal())
        , memory_dump_manager_(base::trace_event::MemoryDumpManager::CreateInstanceForTesting())
    {
    ...
    {
        surface_layer_bridge_ = std::make_unique<NiceMock<MockSurfaceLayerBridge>>();
...
protected:
    void InitializeWebMediaPlayerImpl(std::unique_ptr<media::Demuxer> demuxer_override = nullptr)
    {
        ...
        factory_selector->AddBaseFactory(media::RendererType::kRendererImpl,
            std::make_unique<media::RendererImplFactory>(media_log.get(), decoder_factory_.get(), media::RendererImplFactory::GetGpuFactoriesCB(), player_id));
        factory_selector->StartRequestRemotePlayStateCB(base::DoNothing());
#else
        factory_selector->AddBaseFactory(media::RendererType::kRendererImpl,
            std::make_unique<media::RendererImplFactory>(
                media_log.get(), decoder_factory_.get(), media::RendererImplFactory::GetGpuFactoriesCB(), player_id, nullptr));
#endif

        mojo::Remote<media::mojom::MediaMetricsProvider> provider;
        media::MediaMetricsProvider::Create(media::MediaMetricsProvider::BrowsingMode::kNormal, media::MediaMetricsProvider::FrameStatus::kNotTopFrame,
            ukm::kInvalidSourceId, media::learning::FeatureValue(0), media::VideoDecodePerfHistory::SaveCallback(),
            media::MediaMetricsProvider::GetLearningSessionCallback(), WTF::BindRepeating(&WebMediaPlayerImplTest::IsShuttingDown, WTF::Unretained(this)),
        ...
        provider->Initialize(false, media::mojom::MediaURLScheme::kHttp, media::mojom::MediaStreamType::kNone);

        audio_sink_ = base::WrapRefCounted(new NiceMock<media::MockAudioRendererSink>());

        url_index_ = std::make_unique<UrlIndex>(&mock_resource_fetch_context_, media_thread_.task_runner());

        auto compositor = std::make_unique<NiceMock<MockVideoFrameCompositor>>(media_thread_.task_runner());
        compositor_ = compositor.get();

        wmpi_ = std::make_unique<WebMediaPlayerImpl>(GetWebLocalFrame(), &client_, &encrypted_client_, &delegate_, std::move(factory_selector),
            url_index_.get(), std::move(compositor), std::move(media_log), player_id, WebMediaPlayerBuilder::DeferLoadCB(), audio_sink_,
            media_thread_.task_runner(), media_thread_.task_runner(), media_thread_.task_runner(), media_thread_.task_runner(), nullptr,
            media::RequestRoutingTokenCallback(), mock_observer_.AsWeakPtr(), false, false, provider.Unbind(),
            WTF::BindOnce(&WebMediaPlayerImplTest::CreateMockSurfaceLayerBridge, base::Unretained(this)), viz::TestContextProvider::Create(),
            /*use_surface_layer=*/true, is_background_suspend_enabled_, is_background_video_playback_enabled_, true, std::move(demuxer_override), nullptr);
    }

    std::unique_ptr<WebSurfaceLayerBridge> CreateMockSurfaceLayerBridge(WebSurfaceLayerBridgeObserver*, cc::UpdateSubmissionStateCB)
    {
    ...
    {
        wmpi_->paused_ = is_paused;
    }
    void SetSeeking(bool is_seeking)
    {
        wmpi_->seeking_ = is_seeking;
    }
    void SetEnded(bool is_ended)
    {
        wmpi_->ended_ = is_ended;
    }
    ...
    void SetMetadata(bool has_audio, bool has_video)
    {
        ...
        if (has_video) {
            wmpi_->pipeline_metadata_.video_decoder_config = TestVideoConfig::Normal();
        }

        if (has_audio) {
            wmpi_->pipeline_metadata_.audio_decoder_config = TestAudioConfig::Normal();
        }
    }
    ...
    {
        return wmpi_->UpdatePlayState_ComputePlayState(false, true, false, false, false);
    }

    WebMediaPlayerImpl::PlayState ComputePlayState_FrameHidden()
    {
        return wmpi_->UpdatePlayState_ComputePlayState(false, true, false, true, false);
    }

    WebMediaPlayerImpl::PlayState ComputePlayState_Suspended()
    {
        return wmpi_->UpdatePlayState_ComputePlayState(false, true, true, false, false);
    }

    WebMediaPlayerImpl::PlayState ComputePlayState_Flinging()
    {
        return wmpi_->UpdatePlayState_ComputePlayState(true, true, false, false, false);
    }

    WebMediaPlayerImpl::PlayState ComputePlayState_BackgroundedStreaming()
    {
        return wmpi_->UpdatePlayState_ComputePlayState(false, false, false, true, false);
    }

    WebMediaPlayerImpl::PlayState ComputePlayState_FrameHiddenPictureInPicture()
    {
        return wmpi_->UpdatePlayState_ComputePlayState(false, true, false, true, true);
    }

    bool IsSuspended()
    {
        return wmpi_->pipeline_controller_->IsSuspended();
    }

    bool IsStreaming() const
    {
        return wmpi_->IsStreaming();
    }
    ...
    {
        base::RunLoop loop;
        EXPECT_CALL(*compositor_, SetIsPageVisible(false)).WillOnce(RunClosure(loop.QuitClosure()));
    ...
    {
        base::RunLoop loop;
        EXPECT_CALL(*compositor_, SetIsPageVisible(true)).WillOnce(RunClosure(loop.QuitClosure()));
    ...
    {
        wmpi_->ScheduleIdlePauseTimer();
    }
    void FireIdlePauseTimer()
    {
        wmpi_->background_pause_timer_.FireNow();
    }
    ...
    bool IsVideoTrackDisabled() const
    {
        return wmpi_->video_track_disabled_;
    }
    ...
    bool ShouldCancelUponDefer() const
    {
        ...
        return static_cast<const MultiBufferDataSource*>(ds)->cancel_on_defer_for_testing();
    }
    ...
    {
        gfx::Size size(8, 8);
        return media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_I420, size, gfx::Rect(size), size, base::TimeDelta());
    }

    void RequestVideoFrameCallback()
    {
        wmpi_->RequestVideoFrameCallback();
    }
    void GetVideoFramePresentationMetadata()
    {
        wmpi_->GetVideoFramePresentationMetadata();
    }
    void UpdateFrameIfStale()
    {
        wmpi_->UpdateFrameIfStale();
    }

    void OnNewFramePresentedCallback()
    {
        wmpi_->OnNewFramePresentedCallback();
    }
    ...
    void Load(std::string data_file, LoadType load_type = LoadType::kFullyBuffered)
    {
        ...
        // underlying demuxer.
        const KURL kTestURL(String::FromUTF8(std::string(is_streaming ? "http" : "file") + "://example.com/sample.webm"));
        ...
        const bool should_have_client = !wmpi_->demuxer_manager_->HasDemuxerOverride();
        WebAssociatedURLLoaderClient* client = nullptr;
        if (should_have_client) {
            EXPECT_CALL(mock_resource_fetch_context_, CreateUrlLoader(_)).WillRepeatedly(Invoke([&client](const WebAssociatedURLLoaderOptions&) {
                auto a = std::make_unique<NiceMock<MockWebAssociatedURLLoader>>();
                EXPECT_CALL(*a, LoadAsynchronously(_, _)).WillRepeatedly(testing::SaveArg<1>(&client));
                return a;
            }));
        }

        wmpi_->Load(WebMediaPlayer::kLoadTypeURL, WebMediaPlayerSource(WebURL(kTestURL)), WebMediaPlayer::kCorsModeUnspecified,
        ...
        scoped_refptr<media::DecoderBuffer> data = media::ReadTestDataFile(data_file);
        ...
        response.SetHttpHeaderField(WebString::FromUTF8("Content-Length"), WebString::FromUTF8(is_streaming ? "-1" : base::NumberToString(data->size())));
    ...
    void LoadAndWaitForReadyState(std::string data_file, WebMediaPlayer::ReadyState ready_state)
    {
        Load(data_file);
        while (wmpi_->GetReadyState() < ready_state) {
            base::RunLoop loop;
            EXPECT_CALL(client_, ReadyStateChanged()).WillRepeatedly(RunClosure(loop.QuitClosure()));
            loop.Run();
    ...
    {
        LoadAndWaitForReadyState(data_file, WebMediaPlayer::kReadyStateHaveCurrentData);
    }
    ...
    void OnCdmCreated(base::RepeatingClosure quit_closure, std::unique_ptr<WebContentDecryptionModule> cdm, media::CreateCdmStatus status)
    {
        LOG_IF(ERROR, status != media::CreateCdmStatus::kSuccess) << "status = " << static_cast<int>(status);
        EXPECT_TRUE(cdm);
    ...
    void CreateCdm()
    {
        ...
        media::CdmConfig cdm_config = { media::kClearKeyKeySystem, false, false, false };
        auto test_origin = WebSecurityOrigin::CreateFromString(WebString::FromUTF8("https://test.origin"));

        if (!key_systems_) {
            key_systems_ = std::make_unique<media::KeySystemsImpl>(base::NullCallback());
        }
        base::RunLoop run_loop;
        WebContentDecryptionModuleImpl::Create(&mock_cdm_factory_, key_systems_.get(), test_origin, cdm_config,
            WTF::BindOnce(&WebMediaPlayerImplTest::OnCdmCreated, WTF::Unretained(this), run_loop.QuitClosure()));
        run_loop.Run();
    ...
    {
        DCHECK(web_cdm_);
        EXPECT_CALL(*mock_cdm_, GetCdmContext()).WillRepeatedly(Return(&mock_cdm_context_));
    ...
    scoped_refptr<media::MockCdm> mock_cdm_ = base::MakeRefCounted<media::MockCdm>();
    ...
    viz::LocalSurfaceId local_surface_id_ = viz::LocalSurfaceId(11, base::UnguessableToken::CreateForTesting(0x111111, 0));
    viz::SurfaceId surface_id_ = viz::SurfaceId(frame_sink_id_, local_surface_id_);
...
TEST_F(WebMediaPlayerImplTest, LoadAndDestroy)
{
    ...
    EXPECT_GT(task_environment_.isolate()->AdjustAmountOfExternalAllocatedMemory(0), data_source_size);
}
...
TEST_F(WebMediaPlayerImplTest, LoadAndDestroyDataUrl)
{
    ...
    const KURL kMp3DataUrl("data://audio/mp3;base64,SUQzAwAAAAAAFlRFTkMAAAAMAAAAQW1hZGV1cyBQcm//"
    ...
    wmpi_->Load(WebMediaPlayer::kLoadTypeURL, WebMediaPlayerSource(WebURL(kMp3DataUrl)), WebMediaPlayer::kCorsModeUnspecified,
        /*is_cache_disabled=*/false);
...
TEST_F(WebMediaPlayerImplTest, LoadPreloadMetadataSuspend)
{
    ...
    LoadAndWaitForReadyState(kAudioOnlyTestFile, WebMediaPlayer::kReadyStateHaveMetadata);
    testing::Mock::VerifyAndClearExpectations(&client_);
    ...
    const int64_t data_source_size = GetDataSourceMemoryUsage();
    EXPECT_GT(data_source_size, 0);
    EXPECT_EQ(task_environment_.isolate()->AdjustAmountOfExternalAllocatedMemory(0), data_source_size);
}
...
TEST_F(WebMediaPlayerImplTest, LazyLoadPreloadMetadataSuspend)
{
    ...
    LoadAndWaitForReadyState(kVideoOnlyTestFile, WebMediaPlayer::kReadyStateHaveMetadata);
    testing::Mock::VerifyAndClearExpectations(&client_);
    EXPECT_CALL(client_, ReadyStateChanged()).Times(AnyNumber());
    CycleThreads();
    EXPECT_TRUE(IsSuspended());
...
TEST_F(WebMediaPlayerImplTest, IdleSuspendIsDisabledIfLoadingProgressedRecently)
{
    InitializeWebMediaPlayerImpl();
...
TEST_F(WebMediaPlayerImplTest, UpdateFrameIfStale)
{
    ...
    base::RunLoop loop;
    EXPECT_CALL(*compositor_, UpdateCurrentFrameIfStale(VideoFrameCompositor::UpdateType::kBypassClient)).WillOnce(RunClosure(loop.QuitClosure()));
...
{
    std::unique_ptr<media::MockDemuxer> demuxer = std::make_unique<NiceMock<media::MockDemuxer>>();
...
{
    InitializeWebMediaPlayerImpl();
    LoadAndWaitForReadyState(kAudioOnlyTestFile, WebMediaPlayer::kReadyStateHaveFutureData);
    wmpi_->SetRate(1.0);
    Play();

    EXPECT_CALL(client_,
        DidPlayerMediaPositionStateChange(1.0, kAudioOnlyTestFileDuration, base::TimeDelta(),
            /*end_of_media=*/false));
    wmpi_->OnTimeUpdate();
}
...
TEST_F(WebMediaPlayerImplTest, MediaPositionState_Paused)
{
    ...
    EXPECT_CALL(client_,
        DidPlayerMediaPositionStateChange(0.0, kAudioOnlyTestFileDuration, base::TimeDelta(),
            /*end_of_media=*/false));
    wmpi_->OnTimeUpdate();
}
...
TEST_F(WebMediaPlayerImplTest, MediaPositionState_PositionChange)
{
    ...
    EXPECT_CALL(client_,
        DidPlayerMediaPositionStateChange(0.0, kAudioOnlyTestFileDuration, base::Seconds(0.1),
            /*end_of_media=*/false))
        .InSequence(sequence);
    wmpi_->Seek(0.1);
    wmpi_->OnTimeUpdate();
    ...
    EXPECT_CALL(client_,
        DidPlayerMediaPositionStateChange(0.5, kAudioOnlyTestFileDuration, base::Seconds(0.1),
            /*end_of_media=*/false))
        .InSequence(sequence);
    SetReadyState(WebMediaPlayer::kReadyStateHaveFutureData);
    wmpi_->OnTimeUpdate();
...
TEST_F(WebMediaPlayerImplTest, MediaPositionState_InfiniteCurrentTime)
{
    ...
    EXPECT_CALL(client_,
        DidPlayerMediaPositionStateChange(0.0, media::kInfiniteDuration, media::kInfiniteDuration,
            /*end_of_media=*/false));
...
TEST_F(WebMediaPlayerImplTest, Encrypted)
{
    ...
    {
        base::RunLoop run_loop;
        EXPECT_CALL(encrypted_client_, Encrypted(media::EmeInitDataType::WEBM, NotNull(), Gt(0u)));
        EXPECT_CALL(encrypted_client_, DidBlockPlaybackWaitingForKey());
        EXPECT_CALL(encrypted_client_, DidResumePlaybackBlockedForKey()).WillRepeatedly(RunClosure(run_loop.QuitClosure()));
    ...
    EXPECT_CALL(mock_cdm_context_, GetDecryptor()).Times(AnyNumber()).WillRepeatedly(Return(nullptr));
...
TEST_F(WebMediaPlayerImplTest, Waiting_SecureSurfaceLost)
{
    ...
    LoadAndWaitForReadyState(kVideoOnlyTestFile, WebMediaPlayer::kReadyStateHaveFutureData);
    wmpi_->SetRate(1.0);
    Play();
...
{
    arg0->OnBufferingStateChange(media::BUFFERING_HAVE_ENOUGH, media::BUFFERING_CHANGE_REASON_UNKNOWN);
}
...
TEST_F(WebMediaPlayerImplTest, FallbackToMediaFoundationRenderer)
{
    ...
    EXPECT_CALL(*mock_renderer_factory, CreateRenderer(_, _, _, _, _, _)).WillOnce(testing::WithoutArgs(Invoke([]() {
        auto mock_renderer = std::make_unique<NiceMock<media::MockRenderer>>();
        EXPECT_CALL(*mock_renderer, OnSetCdm(_, _)).WillOnce(RunOnceCallback<1>(true));
        EXPECT_CALL(*mock_renderer, OnInitialize(_, _, _)).WillOnce(DoAll(RunOnceCallback<2>(media::PIPELINE_OK), WithArg<1>(ReportHaveEnough())));
        return mock_renderer;
    })));

    renderer_factory_selector_->AddFactory(media::RendererType::kMediaFoundation, std::move(mock_renderer_factory));

    // Create and set CDM. The CDM doesn't support a Decryptor and requires Media
    // Foundation Renderer.
    EXPECT_CALL(mock_cdm_context_, GetDecryptor()).WillRepeatedly(Return(nullptr));
    EXPECT_CALL(mock_cdm_context_, RequiresMediaFoundationRenderer()).WillRepeatedly(Return(true));
    ...
    EXPECT_CALL(client_, DidUseAudioServiceChange(/*uses_audio_service=*/false)).WillOnce(RunClosure(run_loop.QuitClosure()));
    Load(kEncryptedVideoOnlyTestFile);
    run_loop.Run();
}
    ...
    EXPECT_CALL(*mock_renderer_factory, CreateRenderer(_, _, _, _, _, _))
        .WillOnce(testing::WithoutArgs(Invoke([]() {
            ...
            EXPECT_CALL(*mock_renderer, OnInitialize(_, _, _))
                .WillOnce(DoAll(RunOnceCallback<2>(media::PIPELINE_OK), WithArg<1>(ReportHardwareContextReset())));
            return mock_renderer;
        })))
        .WillOnce(testing::WithoutArgs(Invoke([&]() {
            auto mock_renderer = std::make_unique<NiceMock<media::MockRenderer>>();
            EXPECT_CALL(*mock_renderer, OnInitialize(_, _, _)).WillOnce(DoAll(RunOnceCallback<2>(media::PIPELINE_OK), RunClosure(run_loop.QuitClosure())));
            return mock_renderer;
        })));

    renderer_factory_selector_->AddFactory(media::RendererType::kTest, std::move(mock_renderer_factory));
...
TEST_F(WebMediaPlayerImplTest, VideoConfigChange)
{
    ...
    media::PipelineMetadata metadata;
    metadata.has_video = true;
    metadata.video_decoder_config = TestVideoConfig::NormalCodecProfile(media::VideoCodec::kVP9, media::VP9PROFILE_PROFILE0);
    ...
    auto new_profile_config = TestVideoConfig::NormalCodecProfile(media::VideoCodec::kVP9, media::VP9PROFILE_PROFILE1);
    ...
    auto new_codec_config = TestVideoConfig::NormalCodecProfile(media::VideoCodec::kVP8);
    ...
    // recreation of the reporter
    media::VideoDecoderConfig new_color_config = TestVideoConfig::NormalWithColorSpace(media::VideoCodec::kVP8, media::VideoColorSpace::REC709());
...
TEST_F(WebMediaPlayerImplTest, NaturalSizeChange)
{
    ...
    media::PipelineMetadata metadata;
    metadata.has_video = true;
    metadata.video_decoder_config = TestVideoConfig::NormalCodecProfile(media::VideoCodec::kVP8, media::VP8PROFILE_MIN);
    metadata.natural_size = gfx::Size(320, 240);
    ...
    ASSERT_TRUE(orig_stats_reporter->MatchesBucketedNaturalSize(gfx::Size(320, 240)));
    ...
    ASSERT_TRUE(GetVideoStatsReporter()->MatchesBucketedNaturalSize(gfx::Size(1920, 1080)));
...
TEST_F(WebMediaPlayerImplTest, NaturalSizeChange_Rotated)
{
    ...
    media::PipelineMetadata metadata;
    metadata.has_video = true;
    metadata.video_decoder_config = TestVideoConfig::NormalRotated(media::VIDEO_ROTATION_90);
    metadata.natural_size = gfx::Size(320, 240);
    ...
    // New natural size triggers new reporter to be created.
    ASSERT_NE(orig_stats_reporter, GetVideoStatsReporter());
    ASSERT_TRUE(GetVideoStatsReporter()->MatchesBucketedNaturalSize(gfx::Size(1080, 1920)));
...
TEST_F(WebMediaPlayerImplTest, VideoLockedWhenPausedWhenHidden)
{
    ...
    GetWebLocalFrame()->NotifyUserActivation(mojom::UserActivationNotificationType::kTest);
    Play();
    EXPECT_FALSE(IsVideoLockedWhenPausedWhenHidden());
...
TEST_F(WebMediaPlayerImplTest, PageEventsHasNoEffectIfPausedDueToFrameVisibility)
{
...
TEST_F(WebMediaPlayerImplTest, FrameVisibilityEventsHavePrecedenceOverPageEvents)
{
    // Adding a demuxer and loading a media is necessary to make sure that the
    // pipeline will start and that `WebMediaPlayerImpl::PauseVideoIfNeeded` won't
    // return early.
...
TEST_F(WebMediaPlayerImplTest, BackgroundIdlePauseTimerDependsOnAudio)
{
    ...
    EXPECT_CALL(client_, PausePlayback(MediaPlayerClient::PauseReason::kSuspendedPlayerIdleTimeout));
    FireIdlePauseTimer();
    base::RunLoop().RunUntilIdle();
}
...
TEST_F(WebMediaPlayerImplTest, SetContentsLayerGetsWebLayerFromBridge)
{
    ...
    EXPECT_CALL(*surface_layer_bridge_ptr_, GetSurfaceId()).WillOnce(ReturnRef(surface_id_));
    ...
    EXPECT_CALL(*surface_layer_bridge_ptr_, GetCcLayer()).WillRepeatedly(Return(layer.get()));
...
TEST_F(WebMediaPlayerImplTest, PictureInPictureStateChange)
{
    ...
    EXPECT_CALL(*surface_layer_bridge_ptr_, CreateSurfaceLayer());
    EXPECT_CALL(*surface_layer_bridge_ptr_, GetSurfaceId()).WillRepeatedly(ReturnRef(surface_id_));
    EXPECT_CALL(*compositor_, EnableSubmission(_, _, _));
    EXPECT_CALL(*surface_layer_bridge_ptr_, SetContentsOpaque(false));
    ...
    EXPECT_CALL(client_, GetDisplayType()).WillRepeatedly(Return(DisplayType::kPictureInPicture));
...
TEST_F(WebMediaPlayerImplTest, OnProgressClearsStale)
{
    ...
    for (auto rs = WebMediaPlayer::kReadyStateHaveNothing; rs <= WebMediaPlayer::kReadyStateHaveEnoughData;
         rs = static_cast<WebMediaPlayer::ReadyState>(static_cast<int>(rs) + 1)) {
        SetReadyState(rs);
        delegate_.SetStaleForTesting(true);
        OnProgress();
        EXPECT_EQ(delegate_.IsStale(delegate_.player_id()), rs >= WebMediaPlayer::kReadyStateHaveFutureData);
    }
}
...
TEST_F(WebMediaPlayerImplTest, MemDumpReporting)
{
    ...
    base::trace_event::MemoryDumpRequestArgs args
        = { 1 /* dump_guid*/, base::trace_event::MemoryDumpType::kExplicitlyTriggered, base::trace_event::MemoryDumpLevelOfDetail::kDetailed };

    int32_t id = media::GetNextMediaPlayerLoggingID() - 1;
    int dump_count = 0;

    auto on_memory_dump_done = base::BindLambdaForTesting([&](bool success, uint64_t dump_guid, std::unique_ptr<base::trace_event::ProcessMemoryDump> pmd) {
        ASSERT_TRUE(success);
        const auto& dumps = pmd->allocator_dumps();

        std::vector<const char*> allocations = { "audio", "video", "data_source", "demuxer" };

        for (const char* name : allocations) {
            auto it = dumps.find(base::StringPrintf("media/webmediaplayer/%s/player_0x%x", name, id));
            ASSERT_NE(dumps.end(), it) << name;
            ASSERT_GT(it->second->GetSizeInternal(), 0u) << name;
        }

        auto it = dumps.find(base::StringPrintf("media/webmediaplayer/player_0x%x", id));
        ...
        if (args.level_of_detail == base::trace_event::MemoryDumpLevelOfDetail::kDetailed) {
            ASSERT_TRUE(base::ranges::any_of(entries, [](const auto& e) { return e.name == "player_state" && !e.value_string.empty(); }));
        }
        dump_count++;
    });
    ...
    args.level_of_detail = base::trace_event::MemoryDumpLevelOfDetail::kBackground;
    args.dump_guid++;
    dump_manager->CreateProcessDump(args, on_memory_dump_done);
...
TEST_F(WebMediaPlayerImplTest, DISABLED_DemuxerOverride)
{
    ...
    EXPECT_CALL(*demuxer.get(), OnInitialize(_, _)).WillOnce(RunOnceCallback<1>(media::PIPELINE_OK));
    ...
    EXPECT_FALSE(IsSuspended());
    wmpi_->Load(WebMediaPlayer::kLoadTypeURL, WebMediaPlayerSource(WebURL(KURL("data://test"))), WebMediaPlayer::kCorsModeUnspecified,
        /*is_cache_disabled=*/false);
    base::RunLoop().RunUntilIdle();
    EXPECT_TRUE(IsSuspended());
}

class WebMediaPlayerImplBackgroundBehaviorTest : public WebMediaPlayerImplTest,
                                                 public WebAudioSourceProviderClient,
                                                 public ::testing::WithParamInterface<std::tuple<bool, int, int, bool, bool, bool, bool, bool, bool>> {
public:
    ...
    void SetUp() override
    {
        ...
        SetVideoKeyframeDistanceAverage(base::Seconds(GetAverageKeyframeDistanceSec()));
    ...
    {
        return std::get<kDurationSec>(GetParam());
    }
    ...
    {
        return WebMediaPlayerImpl::kMaxKeyframeDistanceToDisableBackgroundVideo.InSeconds();
    }
    ...
    {
        return background_type_ == BackgroundBehaviorType::Frame && GetShouldPauseWhenFrameIsHidden();
    }

    std::string PrintValues()
    {
        std::stringstream stream;
        stream << "is_media_suspend_enabled=" << IsMediaSuspendOn() << ", duration_sec=" << GetDurationSec()
               << ", average_keyframe_distance_sec=" << GetAverageKeyframeDistanceSec()
               << ", is_resume_background_video_enabled=" << IsResumeBackgroundVideoEnabled() << ", is_picture_in_picture=" << IsPictureInPictureOn()
               << ", is_background_video_playback_enabled=" << IsBackgroundVideoPlaybackEnabled() << ", is_video_being_captured=" << IsVideoBeingCaptured()
               << ", should_pause_when_frame_is_hidden=" << GetShouldPauseWhenFrameIsHidden()
               << ", should_hide_page=" << (background_type_ == BackgroundBehaviorType::Page);
        return stream.str();
    }
...
TEST_P(WebMediaPlayerImplBackgroundBehaviorTest, VideoOnly)
{
    ...
    bool matches_requirements = !IsPictureInPictureOn() && !IsVideoBeingCaptured();
    ...
    if (IsFrameHiddenAndShouldPauseWhenHidden()) {
        ...
        bool should_pause = (!IsBackgroundVideoPlaybackEnabled() || IsMediaSuspendOn() || matches_requirements) && !IsPictureInPictureOn();
...
TEST_P(WebMediaPlayerImplBackgroundBehaviorTest, AudioVideo)
{
    ...
    bool always_pause = (!IsBackgroundVideoPlaybackEnabled() || (IsMediaSuspendOn() && IsResumeBackgroundVideoEnabled())) && !IsPictureInPictureOn();

    bool should_pause = !IsPictureInPictureOn() && (!IsBackgroundVideoPlaybackEnabled() || IsMediaSuspendOn() || !IsVideoBeingCaptured());
    ...
    bool matches_requirements = !IsPictureInPictureOn() && !IsVideoBeingCaptured()
        && ((GetDurationSec() < GetMaxKeyframeDistanceSec()) || (GetAverageKeyframeDistanceSec() < GetMaxKeyframeDistanceSec()));

    EXPECT_EQ(matches_requirements, ShouldDisableVideoWhenHidden());

    if (!matches_requirements || !ShouldDisableVideoWhenHidden() || IsMediaSuspendOn()) {
        return;
    }
...
INSTANTIATE_TEST_SUITE_P(BackgroundBehaviorTestInstances, WebMediaPlayerImplBackgroundBehaviorTest,
    ::testing::Combine(::testing::Bool(), ::testing::Values(WebMediaPlayerImpl::kMaxKeyframeDistanceToDisableBackgroundVideo.InSeconds() - 1, 300),
        ::testing::Values(WebMediaPlayerImpl::kMaxKeyframeDistanceToDisableBackgroundVideo.InSeconds() - 1, 100), ::testing::Bool(), ::testing::Bool(),
