...
void WebV8Features::EnableMojoJS(v8::Local<v8::Context> context, bool enable)
{
    ...
    ContextFeatureSettings::From(ExecutionContext::From(script_state), ContextFeatureSettings::CreationMode::kCreateIfNotExists)->EnableMojoJS(enable);
}

// static
void WebV8Features::EnableMojoJSAndUseBroker(v8::Local<v8::Context> context, CrossVariantMojoRemote<mojom::BrowserInterfaceBrokerInterfaceBase> broker_remote)
{
    ...
    blink::ExecutionContext::From(context)->SetMojoJSInterfaceBroker(std::move(broker_remote));
}

// static
void WebV8Features::EnableMojoJSFileSystemAccessHelper(v8::Local<v8::Context> context, bool enable)
{
    if (enable) {
        // If the code is trying to enable mojo JS but mojo JS is not allowed for
        // the process, as determined by the protected memory bool value, then it
    ...
    auto* context_feature_settings
        = ContextFeatureSettings::From(ExecutionContext::From(script_state), ContextFeatureSettings::CreationMode::kCreateIfNotExists);
...
bool WebV8Features::IsMojoJSEnabledForTesting(v8::Local<v8::Context> context)
{
    ...
    ContextFeatureSettings* settings
        = ContextFeatureSettings::From(ExecutionContext::From(script_state), ContextFeatureSettings::CreationMode::kDontCreateIfNotExists);
    return settings && settings->isMojoJSEnabled();
}

// static
void WebV8Features::EnableMojoJSWithoutSecurityChecksForTesting(v8::Local<v8::Context> context)
{
    v8::Isolate* isolate = context->GetIsolate();
    ScriptState* script_state = ScriptState::From(isolate, context);
    DCHECK(script_state->World().IsMainWorld());
    ContextFeatureSettings::From(ExecutionContext::From(script_state), ContextFeatureSettings::CreationMode::kCreateIfNotExists)->EnableMojoJS(true);
}
...
void WebV8Features::SetIsolatePriority(base::Process::Priority priority)
{
    ...
    Thread::MainThread()->Scheduler()->ToMainThreadScheduler()->ForEachMainThreadIsolate(
        WTF::BindRepeating([](v8::Isolate::Priority priority, v8::Isolate* isolate) { isolate->SetPriority(priority); }, isolate_priority));
