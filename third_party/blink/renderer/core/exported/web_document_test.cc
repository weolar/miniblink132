...
void WebDocumentTest::SetUpTestSuite()
{
    url_test_helpers::RegisterMockedURLLoad(
        ToKURL(std::string(kDefaultOrigin) + kNoOriginTrialTestFilePath), test::CoreTestDataPath(kNoOriginTrialTestFilePath));
    url_test_helpers::RegisterMockedURLLoad(ToKURL(std::string(kDefaultOrigin) + kOriginTrialTestFilePath), test::CoreTestDataPath(kOriginTrialTestFilePath));
}
...
{
    return To<LocalFrame>(web_view_helper_.GetWebView()->GetPage()->MainFrame())->GetDocument();
}
...
TEST_F(WebDocumentTest, InsertAndRemoveStyleSheet)
{
    ...
    WebStyleSheetKey style_sheet_key = web_doc.InsertStyleSheet("body { color: green }");

    // Check insertStyleSheet did not cause a synchronous style recalc.
    unsigned element_count = core_doc->GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(0U, element_count);

    HTMLElement* body_element = core_doc->body();
    DCHECK(body_element);

    const ComputedStyle& style_before_insertion = body_element->ComputedStyleRef();

    // Inserted style sheet not yet applied.
    ASSERT_EQ(Color(0, 0, 0), style_before_insertion.VisitedDependentColor(GetCSSPropertyColor()));
    ...
    ASSERT_EQ(Color(0, 128, 0), style_after_insertion.VisitedDependentColor(GetCSSPropertyColor()));
    ...
    element_count = core_doc->GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(0U, element_count);

    const ComputedStyle& style_before_removing = body_element->ComputedStyleRef();

    // Removed style sheet not yet applied.
    ASSERT_EQ(Color(0, 128, 0), style_before_removing.VisitedDependentColor(GetCSSPropertyColor()));

    // Apply removed style sheet.
    core_doc->UpdateStyleAndLayoutTree();

    const ComputedStyle& style_after_removing = body_element->ComputedStyleRef();
    ASSERT_EQ(Color(0, 0, 0), style_after_removing.VisitedDependentColor(GetCSSPropertyColor()));
}
...
TEST_F(WebDocumentTest, OriginTrialDisabled)
{
    ...
    EXPECT_FALSE(blink::WebRuntimeFeatures::IsOriginTrialsSampleAPIEnabledByRuntimeFlag());
}
...
TEST_F(WebDocumentTest, OriginTrialEnabled)
{
    ...
    // also enabled.
    EXPECT_FALSE(WebOriginTrials::IsOriginTrialsSampleAPIDeprecationEnabled(&web_doc));
    EXPECT_FALSE(WebRuntimeFeatures::IsOriginTrialsSampleAPIDeprecationEnabledByRuntimeFlag());
}
...
const char* g_nested_origin_a_in_origin_a = "first_party/nested-originA-in-originA.html";
const char* g_nested_origin_a_in_origin_b = "first_party/nested-originA-in-originB.html";
const char* g_nested_origin_b = "first_party/nested-originB.html";
const char* g_nested_origin_b_in_origin_a = "first_party/nested-originB-in-originA.html";
const char* g_nested_origin_b_in_origin_b = "first_party/nested-originB-in-originB.html";
...
void WebDocumentFirstPartyTest::SetUpTestSuite()
{
    ...
    RegisterMockedURLLoad(ToOriginA(g_nested_origin_secure_a), g_nested_origin_secure_a);
    RegisterMockedURLLoad(ToOriginA(g_nested_origin_a_in_origin_a), g_nested_origin_a_in_origin_a);
    RegisterMockedURLLoad(ToOriginA(g_nested_origin_a_in_origin_b), g_nested_origin_a_in_origin_b);
    RegisterMockedURLLoad(ToOriginA(g_nested_origin_b), g_nested_origin_b);
    RegisterMockedURLLoad(ToOriginA(g_nested_origin_b_in_origin_a), g_nested_origin_b_in_origin_a);
    RegisterMockedURLLoad(ToOriginA(g_nested_origin_b_in_origin_b), g_nested_origin_b_in_origin_b);
...
{
    return To<LocalFrame>(web_view_helper_.GetWebView()->GetPage()->MainFrame()->Tree().FirstChild())->GetDocument();
}

Document* WebDocumentFirstPartyTest::NestedNestedDocument() const
{
    return To<LocalFrame>(web_view_helper_.GetWebView()->GetPage()->MainFrame()->Tree().FirstChild()->Tree().FirstChild())->GetDocument();
}

bool OriginsEqual(const char* path, scoped_refptr<const SecurityOrigin> origin)
{
    return SecurityOrigin::Create(ToOriginA(path))->IsSameOriginWith(origin.get());
}

bool SiteForCookiesEqual(const char* path, const net::SiteForCookies& site_for_cookies)
{
    KURL ref_url = ToOriginA(path);
    ref_url.SetPort(80); // url::Origin takes exception with :0.
    return net::SiteForCookies::FromUrl(GURL(ref_url)).IsEquivalent(site_for_cookies);
}
...
TEST_F(WebDocumentFirstPartyTest, Empty)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_empty_file, TopDocument()->SiteForCookies()));
...
TEST_F(WebDocumentFirstPartyTest, EmptySandbox)
{
    ...
    auto params = WebNavigationParams::CreateWithEmptyHTMLForTesting(KURL("https://a.com"));
    MockPolicyContainerHost mock_policy_container_host;
    params->policy_container
        = std::make_unique<blink::WebPolicyContainer>(blink::WebPolicyContainerPolicies(), mock_policy_container_host.BindNewEndpointAndPassDedicatedRemote());
    ...
    ASSERT_TRUE(TopDocument()->TopFrameOrigin()->IsOpaque()) << TopDocument()->TopFrameOrigin()->ToUrlOrigin().GetDebugString();
...
TEST_F(WebDocumentFirstPartyTest, NestedOriginA)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_a, TopDocument()->SiteForCookies()));
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_a, NestedDocument()->SiteForCookies()));

    ASSERT_TRUE(OriginsEqual(g_nested_origin_a, TopDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_origin_a, NestedDocument()->TopFrameOrigin()));
}
...
TEST_F(WebDocumentFirstPartyTest, NestedOriginSubA)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_sub_a, TopDocument()->SiteForCookies()));
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_sub_a, NestedDocument()->SiteForCookies()));

    ASSERT_TRUE(OriginsEqual(g_nested_origin_sub_a, TopDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_origin_sub_a, NestedDocument()->TopFrameOrigin()));
}
...
TEST_F(WebDocumentFirstPartyTest, NestedOriginSecureA)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_secure_a, TopDocument()->SiteForCookies()));
    // Since NestedDocument is secure, and the parent is insecure, its
    // SiteForCookies will be null and therefore will not match.
    ASSERT_FALSE(SiteForCookiesEqual(g_nested_origin_secure_a, NestedDocument()->SiteForCookies()));
    // However its site shouldn't be opaque
    ASSERT_FALSE(NestedDocument()->SiteForCookies().site().opaque());

    ASSERT_TRUE(OriginsEqual(g_nested_origin_secure_a, TopDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_origin_secure_a, NestedDocument()->TopFrameOrigin()));
}
...
TEST_F(WebDocumentFirstPartyTest, NestedOriginAInOriginA)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_a_in_origin_a, TopDocument()->SiteForCookies()));
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_a_in_origin_a, NestedDocument()->SiteForCookies()));
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_a_in_origin_a, NestedNestedDocument()->SiteForCookies()));

    ASSERT_TRUE(OriginsEqual(g_nested_origin_a_in_origin_a, TopDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_origin_a_in_origin_a, NestedDocument()->TopFrameOrigin()));
}
...
TEST_F(WebDocumentFirstPartyTest, NestedOriginAInOriginB)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_a_in_origin_b, TopDocument()->SiteForCookies()));
    ASSERT_TRUE(NestedDocument()->SiteForCookies().IsNull());
    ASSERT_TRUE(NestedNestedDocument()->SiteForCookies().IsNull());

    ASSERT_TRUE(OriginsEqual(g_nested_origin_a_in_origin_b, TopDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_origin_a_in_origin_b, NestedDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_origin_a_in_origin_b, NestedNestedDocument()->TopFrameOrigin()));
}
...
TEST_F(WebDocumentFirstPartyTest, NestedOriginB)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_b, TopDocument()->SiteForCookies()));
    ASSERT_TRUE(NestedDocument()->SiteForCookies().IsNull());

    ASSERT_TRUE(OriginsEqual(g_nested_origin_b, TopDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_origin_b, NestedDocument()->TopFrameOrigin()));
}
...
TEST_F(WebDocumentFirstPartyTest, NestedOriginBInOriginA)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_b_in_origin_a, TopDocument()->SiteForCookies()));
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_b_in_origin_a, NestedDocument()->SiteForCookies()));
    ASSERT_TRUE(NestedNestedDocument()->SiteForCookies().IsNull());

    ASSERT_TRUE(OriginsEqual(g_nested_origin_b_in_origin_a, TopDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_origin_b_in_origin_a, NestedDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_origin_b_in_origin_a, NestedNestedDocument()->TopFrameOrigin()));
}
...
TEST_F(WebDocumentFirstPartyTest, NestedOriginBInOriginB)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_b_in_origin_b, TopDocument()->SiteForCookies()));
    ASSERT_TRUE(NestedDocument()->SiteForCookies().IsNull());
    ASSERT_TRUE(NestedNestedDocument()->SiteForCookies().IsNull());

    ASSERT_TRUE(OriginsEqual(g_nested_origin_b_in_origin_b, TopDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_origin_b_in_origin_b, NestedDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_origin_b_in_origin_b, NestedNestedDocument()->TopFrameOrigin()));
}
...
TEST_F(WebDocumentFirstPartyTest, NestedSrcdoc)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_src_doc, TopDocument()->SiteForCookies()));
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_src_doc, NestedDocument()->SiteForCookies()));

    ASSERT_TRUE(OriginsEqual(g_nested_src_doc, TopDocument()->TopFrameOrigin()));
    ASSERT_TRUE(OriginsEqual(g_nested_src_doc, NestedDocument()->TopFrameOrigin()));
}
...
TEST_F(WebDocumentFirstPartyTest, NestedData)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_data, TopDocument()->SiteForCookies()));
    ASSERT_TRUE(NestedDocument()->SiteForCookies().IsNull());
...
TEST_F(WebDocumentFirstPartyTest, NestedOriginAInOriginBWithFirstPartyOverride)
{
    ...
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_a_in_origin_b, TopDocument()->SiteForCookies()));
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_a_in_origin_b, NestedDocument()->SiteForCookies()));
    ASSERT_TRUE(SiteForCookiesEqual(g_nested_origin_a_in_origin_b, NestedNestedDocument()->SiteForCookies()));
...
{
    web_view_helper_.InitializeAndLoad(std::string("file:///") + g_nested_origin_a);
