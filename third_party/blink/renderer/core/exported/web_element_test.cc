...
TEST_F(WebElementTest, IsAutonomousCustomElement)
{
    ...
    EXPECT_TRUE(WebElement(To<Element>(v1autonomous)).IsAutonomousCustomElement());
}
...
TEST_F(WebElementTest, SelectedTextOfTextArea)
{
    InsertHTML(
        R"(<div>Foo</div>
         ...
         <div>Bar</div>)");
    auto* element = blink::To<HTMLTextAreaElement>(GetDocument().getElementById(AtomicString("testElement")));
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    element->Focus();
...
{
    InsertHTML("<div id=testElement contenteditable>Some <b>rich text</b> here.</div>"
               "<textarea>Some plain text here.</textarea>");
    auto* element = GetDocument().getElementById(AtomicString("testElement"));
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    Selection().SelectSubString(*element->firstElementChild(), 0, 9);
...
{
    InsertHTML("<div contenteditable>Some <b>rich text</b> here.</div>"
               "<textarea id=testElement>Some plain text here.</textarea>");
    auto* element = blink::To<HTMLTextAreaElement>(GetDocument().getElementById(AtomicString("testElement")));
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    element->Focus();
    ...
    ASSERT_EQ(element->Value().Substring(element->selectionStart(), element->selectionEnd() - element->selectionStart()), String("plain text"));
    // Paste and replace selection.
...
{
    InsertHTML("<input id=testElement>");
    EXPECT_TRUE(TestElement().ShadowRoot().IsNull()) << "ShadowRoot() should not return a UA ShadowRoot.";

    {
        InsertHTML("<span id=testElement></span>");
        EXPECT_TRUE(TestElement().ShadowRoot().IsNull()) << "No ShadowRoot initially.";
        auto* element = GetDocument().getElementById(AtomicString("testElement"));
        element->AttachShadowRootForTesting(ShadowRootMode::kOpen);
        EXPECT_FALSE(TestElement().ShadowRoot().IsNull()) << "Should return V1 open ShadowRoot.";
    }

    {
        ...
        EXPECT_FALSE(TestElement().ShadowRoot().IsNull()) << "Should return V1 closed ShadowRoot.";
    }
}
...
TEST_F(WebElementTest, ComputedStyleProperties)
{
    ...
    element.GetDocument().InsertStyleSheet("body { font-size: 16px; text-decoration: underline; color: blue;}");
    // font-size
    {
    ...
    // text-decoration
    {
        EXPECT_EQ(element.GetComputedValue("text-decoration"), "none solid rgb(0, 0, 255)");
        ...
        EXPECT_EQ(element.GetComputedValue("-Webkit-text-decorations-in-effect"), "underline line-through");
    }
