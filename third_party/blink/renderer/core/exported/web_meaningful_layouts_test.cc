...
TEST_F(WebMeaningfulLayoutsTest, VisuallyNonEmptyMissingPump)
{
    ...
    EXPECT_TRUE(GetDocument().Lifecycle().GetState() >= DocumentLifecycle::kLayoutClean);
...
TEST_F(WebMeaningfulLayoutsTest, NoOverflowInIncrementVisuallyNonEmptyPixelCount)
{
    ...
    SimSubresourceRequest svg_resource("https://example.com/test.svg", "image/svg+xml");
    ...
    svg_resource.Start();
    svg_resource.Write("<svg xmlns=\"http://www.w3.org/2000/svg\" height=\"65536\" "
                       "width=\"65536\"></svg>");
    svg_resource.Finish();
    Compositor().BeginFrame();
    EXPECT_EQ(1, WebFrameClient().VisuallyNonEmptyLayoutCount());
...
{
    SimRequest main_resource("https://example.com/index.html", "text/html");
    SimSubresourceRequest style_resource("https://example.com/style.css", "text/css");

    LoadURL("https://example.com/index.html");

    main_resource.Complete("<html><head>"
