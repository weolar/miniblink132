...
bool HasNoTranslate(const Document& document)
{
    ...
    for (const HTMLMetaElement& meta_element : Traversal<HTMLMetaElement>::ChildrenOf(*head_element)) {
...
void MatchTargetLanguageWithAcceptLanguages(
    const Document& document, const AtomicString& target_language, bool is_xml_lang, const std::string& language_histogram_name)
{
    ...
    // here.
    bool is_accept_language_dirty = document.domWindow()->navigator()->IsLanguagesDirty();
    const Vector<String>& accept_languages = document.domWindow()->navigator()->languages();
    ...
    if (GetLanguageCode(accept_languages.front()) == GetLanguageCode(target_language)) {
        base::UmaHistogramEnumeration(language_histogram_name,
            is_xml_lang ? AcceptLanguageAndXmlHtmlLangUsage::kXmlLangMatchesPrimaryAcceptLanguage
                        : AcceptLanguageAndXmlHtmlLangUsage::kHtmlLangMatchesPrimaryAcceptLanguage);
    } else if (base::Contains(accept_languages, target_language, &GetLanguageCode)) {
        base::UmaHistogramEnumeration(language_histogram_name,
            is_xml_lang ? AcceptLanguageAndXmlHtmlLangUsage::kXmlLangMatchesAnyNonPrimayAcceptLanguage
                        : AcceptLanguageAndXmlHtmlLangUsage::kHtmlLangMatchesAnyNonPrimayAcceptLanguage);
    }
...
WebLanguageDetectionDetails WebLanguageDetectionDetails::CollectLanguageDetectionDetails(const WebDocument& web_document)
{
...
void WebLanguageDetectionDetails::RecordAcceptLanguageAndXmlHtmlLangMetric(const WebDocument& web_document)
{
    ...
    constexpr const char language_histogram_name[] = "LanguageUsage.AcceptLanguageAndXmlHtmlLangUsage";

    // Spec: xml:lang takes precedence -- http://www.w3.org/TR/xhtml1/#C_7
    const AtomicString& xml_language = DocumentXmlLanguage(*document);
    if (xml_language) {
        if (xml_language.empty()) {
            base::UmaHistogramEnumeration(language_histogram_name, AcceptLanguageAndXmlHtmlLangUsage::kXmlLangEmpty);
            return;
        }

        if (xml_language == "*") {
            base::UmaHistogramEnumeration(language_histogram_name, AcceptLanguageAndXmlHtmlLangUsage::kXmlLangWildcard);
            return;
        }

        MatchTargetLanguageWithAcceptLanguages(*document, xml_language, true, language_histogram_name);
        return;
    }
    ...
    if (!html_language || html_language.empty()) {
        base::UmaHistogramEnumeration(language_histogram_name, AcceptLanguageAndXmlHtmlLangUsage::kHtmlLangEmpty);
        return;
    }

    if (html_language == "*") {
        base::UmaHistogramEnumeration(language_histogram_name, AcceptLanguageAndXmlHtmlLangUsage::kHtmlLangWildcard);
        return;
    }

    MatchTargetLanguageWithAcceptLanguages(*document, html_language, false, language_histogram_name);
}
