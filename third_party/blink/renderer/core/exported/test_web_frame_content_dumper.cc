...
WebString TestWebFrameContentDumper::DumpWebViewAsText(WebView* web_view, size_t max_chars)
{
    DCHECK(web_view);
    ...
    StringBuilder text;
    FrameContentAsText(static_cast<wtf_size_t>(max_chars), To<WebLocalFrameImpl>(frame)->GetFrame(), text);
...
WebString TestWebFrameContentDumper::DumpLayoutTreeAsText(WebLocalFrame* frame, LayoutAsTextControls to_show)
{
    ...
    if (to_show & kLayoutAsTextDebug) {
        behavior |= kLayoutAsTextShowAddresses | kLayoutAsTextShowIDAndClass | kLayoutAsTextShowLayerNesting;
    }

    if (to_show & kLayoutAsTextPrinting)
        behavior |= kLayoutAsTextPrintingMode;

    return ExternalRepresentation(To<WebLocalFrameImpl>(frame)->GetFrame(), behavior);
}
