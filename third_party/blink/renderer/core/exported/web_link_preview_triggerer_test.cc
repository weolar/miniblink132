...
public:
    ...
    int LastKeyEventModifiers() const
    {
        return last_key_event_modifiers_;
    }

    const WebElement& HoverElement() const
    {
        return hover_element_;
    }

    const WebElement& MouseEventElement() const
    {
        return mouse_event_element_;
    }
    ...
    void DidAnchorElementReceiveMouseDownEvent(blink::WebElement anchor_element, blink::WebMouseEvent::Button button, int click_count) override
    {
...
protected:
    void Initialize()
    {
        ...
        local_frame->SetLinkPreviewTriggererForTesting(std::make_unique<MockWebLinkPreviewTriggerer>());
    }
...
TEST_F(WebLinkPreviewTriggererTest, MaybeChangedKeyEventModifierCalled)
{
    ...
    MockWebLinkPreviewTriggerer* triggerer = static_cast<MockWebLinkPreviewTriggerer*>(GetDocument().GetFrame()->GetOrCreateLinkPreviewTriggerer());

    EXPECT_EQ(WebInputEvent::kNoModifiers, triggerer->LastKeyEventModifiers());

    WebKeyboardEvent e0 { WebInputEvent::Type::kRawKeyDown, WebInputEvent::kAltKey, WebInputEvent::GetStaticTimeStampForTests() };
    ...
    WebKeyboardEvent e1 { WebInputEvent::Type::kKeyUp, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests() };
...
TEST_F(WebLinkPreviewTriggererTest, MaybeChangedKeyEventModifierCalledWithNoModifiersOnMouseLeave)
{
    Initialize();
    SetHtmlInnerHTML("<div></div>");
    MockWebLinkPreviewTriggerer* triggerer = static_cast<MockWebLinkPreviewTriggerer*>(GetDocument().GetFrame()->GetOrCreateLinkPreviewTriggerer());
    ...
    WebMouseEvent e1(WebMouseEvent::Type::kMouseLeave, gfx::PointF(262, 29), gfx::PointF(329, 67), WebPointerProperties::Button::kNoButton, 1,
...
TEST_F(WebLinkPreviewTriggererTest, DidChangeHoverElementCalledOnHoverChanged)
{
...

    {
        ...
        WebMouseEvent mouse_move_event(WebInputEvent::Type::kMouseMove, point, point, WebPointerProperties::Button::kNoButton, 0,
            WebInputEvent::Modifiers::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
        ...
        EXPECT_EQ("https://example.com", triggerer->HoverElement().GetAttribute("href"));
    }
...
TEST_F(WebLinkPreviewTriggererTest, DidAnchorElementReceiveMouseDownEventCalledOnMousePress)
{
    Initialize();
    SetHtmlInnerHTML("<style>"
                     "  body { margin:0px; }"
    ...
    gfx::PointF point(50, 50);
    WebMouseEvent mouse_down_event(WebInputEvent::Type::kMouseDown, point, point, WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
    ...
    EXPECT_EQ("https://example.com", triggerer->MouseEventElement().GetAttribute("href"));
