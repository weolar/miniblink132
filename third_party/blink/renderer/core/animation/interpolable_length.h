...
public:
    ...
    explicit InterpolableLength(CSSValueID keyword, std::optional<EInterpolateSize> interpolate_size);
    ...
    static bool CanMergeValues(const InterpolableValue* start, const InterpolableValue* end);

    static PairwiseInterpolationValue MaybeMergeSingles(InterpolableValue* start, InterpolableValue* end);

    Length CreateLength(const CSSToLengthConversionData& conversion_data, Length::ValueRange range) const;
    ...
    InterpolableLength* Clone() const
    {
        return RawClone();
    }
    InterpolableLength* CloneAndZero() const
    {
...
private:
    ...
    {
        return type_ == Type::kRestrictedKeyword || type_ == Type::kFullyInterpolableKeyword || type_ == Type::kUnknownKeyword;
    }
    bool IsLengthArray() const
    {
        return type_ == Type::kLengthArray;
    }
    ...
    void SetKeyword(CSSValueID keyword, std::optional<EInterpolateSize> interpolate_size);
    ...
    enum class Type : unsigned char { kLengthArray, kExpression, kRestrictedKeyword, kFullyInterpolableKeyword, kUnknownKeyword };
...
template <> struct DowncastTraits<InterpolableLength> {
