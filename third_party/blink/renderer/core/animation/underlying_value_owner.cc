...
void UnderlyingValueOwner::SetInterpolableValue(InterpolableValue* interpolable_value)
{
    DCHECK(type_);
    MutableValue().interpolable_value = interpolable_value;
}

const NonInterpolableValue* UnderlyingValueOwner::GetNonInterpolableValue() const
{
    DCHECK(value_);
    return value_->non_interpolable_value.get();
}

void UnderlyingValueOwner::SetNonInterpolableValue(scoped_refptr<const NonInterpolableValue> non_interpolable_value)
{
