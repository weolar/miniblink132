...
protected:
    double InterpolateNumbers(int a, int b, double progress)
    {
        ...
        TransitionInterpolation* i
            = MakeGarbageCollected<TransitionInterpolation>(property_handle, interpolation_type, std::move(start), std::move(end), nullptr, nullptr);
        ...
        return To<InterpolableNumber>(interpolated_value->GetInterpolableValue()).Value(length_resolver);
    }
    ...
    InterpolableValue* InterpolateLists(InterpolableValue* list_a, InterpolableValue* list_b, double progress)
    {
...
TEST_F(AnimationInterpolableValueTest, SimpleList)
{
    ...
    InterpolableValue* interpolated_value = InterpolateLists(std::move(list_a), std::move(list_b), 0.3);
    const auto& out_list = To<InterpolableList>(*interpolated_value);

    CSSToLengthConversionData length_resolver;
    EXPECT_FLOAT_EQ(30, To<InterpolableNumber>(out_list.Get(0))->Value(length_resolver));
    EXPECT_FLOAT_EQ(-30.6f, To<InterpolableNumber>(out_list.Get(1))->Value(length_resolver));
    EXPECT_FLOAT_EQ(104.35f, To<InterpolableNumber>(out_list.Get(2))->Value(length_resolver));
}
...
TEST_F(AnimationInterpolableValueTest, NestedList)
{
    ...
    CSSToLengthConversionData length_resolver;
    EXPECT_FLOAT_EQ(50, To<InterpolableNumber>(out_list.Get(0))->Value(length_resolver));
    EXPECT_FLOAT_EQ(75, To<InterpolableNumber>(To<InterpolableList>(out_list.Get(1))->Get(0))->Value(length_resolver));
    EXPECT_FLOAT_EQ(0.5, To<InterpolableNumber>(out_list.Get(2))->Value(length_resolver));
}
...
TEST_F(AnimationInterpolableValueTest, ScaleAndAddLists)
{
    ...
    CSSToLengthConversionData length_resolver;
    EXPECT_FLOAT_EQ(11, To<InterpolableNumber>(base_list->Get(0))->Value(length_resolver));
    EXPECT_FLOAT_EQ(22, To<InterpolableNumber>(base_list->Get(1))->Value(length_resolver));
    EXPECT_FLOAT_EQ(33, To<InterpolableNumber>(base_list->Get(2))->Value(length_resolver));
}
...
TEST_F(AnimationInterpolableValueTest, InterpolableNumberAsExpression)
{
    const struct TestCase {
        ...
        { "progress(11em from 1rem to 110px) * 10", 10.0, 10.0, 5.0, "progress(11em from 1rem to 110px) * 11", 11.0, 0.5, 10.5 },
        { "10deg", 10.0, 10.0, 5.0, "progress(11em from 1rem to 110px) * 11deg", 11.0, 0.5, 10.5 },
        { "progress(11em from 1rem to 110px) * 10deg", 10.0, 10.0, 5.0, "11deg", 11.0, 0.5, 10.5 },
    };
    ...
    for (const auto& test_case : test_cases) {
        ...
        const CSSMathExpressionNode* expression
            = CSSMathExpressionNode::ParseMathFunction(CSSValueID::kCalc, stream, *context, Flags({ AllowPercent }), kCSSAnchorQueryTypesNone);
        InterpolableNumber* number = nullptr;
        if (auto* numeric_literal = DynamicTo<CSSMathExpressionNumericLiteral>(expression)) {
            number = MakeGarbageCollected<InterpolableNumber>(numeric_literal->DoubleValue(), numeric_literal->ResolvedUnitType());
        } else {
        ...
        EXPECT_EQ(number_copy->Value(length_resolver), test_case.scale_value * test_case.output);
        number_copy->Add(*MakeGarbageCollected<InterpolableNumber>(test_case.add_value, expression->ResolvedUnitType()));
        EXPECT_EQ(number_copy->Value(length_resolver), test_case.scale_value * test_case.output + test_case.add_value);
        number_copy = number->Clone();
        number_copy->ScaleAndAdd(test_case.scale_value, *MakeGarbageCollected<InterpolableNumber>(test_case.add_value, expression->ResolvedUnitType()));
        EXPECT_EQ(number_copy->Value(length_resolver), test_case.scale_value * test_case.output + test_case.add_value);
        ...
        const CSSMathExpressionNode* target_expression
            = CSSMathExpressionNode::ParseMathFunction(CSSValueID::kCalc, target_stream, *context, Flags({ AllowPercent }), kCSSAnchorQueryTypesNone);
        InterpolableNumber* target = nullptr;
        if (auto* numeric_literal = DynamicTo<CSSMathExpressionNumericLiteral>(target_expression)) {
            target = MakeGarbageCollected<InterpolableNumber>(numeric_literal->DoubleValue(), numeric_literal->ResolvedUnitType());
        } else {
        ...
        number->Interpolate(*target, test_case.interpolation_fraction, *interpolation_result);
        EXPECT_EQ(interpolation_result->Value(length_resolver), test_case.interpolation_result);
    }
}
