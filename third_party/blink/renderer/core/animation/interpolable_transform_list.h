...
public:
    InterpolableTransformList(TransformOperations&& operations, TransformOperations::BoxSizeDependentMatrixBlending box_size_dependent)
        : operations_(std::move(operations))
        , box_size_dependent_(box_size_dependent)
    {
    }

    static InterpolableTransformList* ConvertCSSValue(const CSSValue&, const CSSToLengthConversionData&, TransformOperations::BoxSizeDependentMatrixBlending);
    ...
    TransformOperations operations() const
    {
        return operations_;
    }
    ...
    bool IsTransformList() const final
    {
        return true;
    }
...
private:
    ...
    {
        return MakeGarbageCollected<InterpolableTransformList>(TransformOperations(operations_), box_size_dependent_);
    }
