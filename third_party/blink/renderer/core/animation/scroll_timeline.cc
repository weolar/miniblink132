...
ScrollOrientation ToPhysicalScrollOrientation(ScrollAxis axis, const LayoutBox& source_box)
{
...
ScrollTimeline* ScrollTimeline::Create(Document& document, ScrollTimelineOptions* options, ExceptionState& exception_state)
{
    std::optional<Element*> source = options->hasSource() ? std::make_optional(options->source()) : std::nullopt;
    ...
    return Create(&document, source.value_or(document.ScrollingElementNoLayout()), axis);
}

ScrollTimeline* ScrollTimeline::Create(Document* document, Element* source, ScrollAxis axis)
{
    ScrollTimeline* scroll_timeline = MakeGarbageCollected<ScrollTimeline>(document, ReferenceType::kSource, source, axis);
    scroll_timeline->UpdateSnapshot();

    return scroll_timeline;
}

ScrollTimeline::ScrollTimeline(Document* document, ReferenceType reference_type, Element* reference, ScrollAxis axis)
...
ScrollTimeline::TimelineState ScrollTimeline::ComputeTimelineState() const
{
    ...
    // - min_offset).
    PaintLayerScrollableArea* scrollable_area = scroll_container->GetScrollableArea();
    ...
    DCHECK(scrollable_area->MaximumScrollOffset().y() == 0 || scrollable_area->MinimumScrollOffset().y() == 0);
    DCHECK(scrollable_area->MaximumScrollOffset().x() == 0 || scrollable_area->MinimumScrollOffset().x() == 0);

    ScrollOffset scroll_offset = scrollable_area->GetScrollOffset();
    auto physical_orientation = ToPhysicalScrollOrientation(GetAxis(), *scroll_container);
    double current_offset = (physical_orientation == kHorizontalScroll) ? scroll_offset.x() : scroll_offset.y();
    ...
    if (state.scroll_offsets->end - state.scroll_offsets->start > 0) {
        ...
        double duration_in_microseconds = range * kScrollTimelineMicrosecondsPerPixel;
        state.duration = std::make_optional(ANIMATION_TIME_DELTA_FROM_MILLISECONDS(duration_in_microseconds / 1000));
        state.current_time = base::Microseconds(offset * kScrollTimelineMicrosecondsPerPixel);
    }
    return state;
}

void ScrollTimeline::CalculateOffsets(PaintLayerScrollableArea* scrollable_area, ScrollOrientation physical_orientation, TimelineState* state) const
{
    ScrollOffset scroll_dimensions = scrollable_area->MaximumScrollOffset() - scrollable_area->MinimumScrollOffset();
    double end_offset = physical_orientation == kHorizontalScroll ? scroll_dimensions.x() : scroll_dimensions.y();
...
{
    if (reference_type_ == ReferenceType::kNearestAncestor && reference_element_) {
        reference_element_->GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kJavaScript);
    }
...
Element* ScrollTimeline::ComputeSourceNoLayout() const
{
    ...
    const LayoutBox* scroll_container = layout_object->ContainingScrollContainer();
    if (!scroll_container) {
...
bool ScrollTimeline::Matches(ReferenceType reference_type, Element* reference_element, ScrollAxis axis) const
{
    return (reference_type_ == reference_type) && (reference_element_ == reference_element) && (axis_ == axis);
}
...
std::optional<double> ScrollTimeline::GetMaximumScrollPosition() const
{
    ...
    ScrollOffset scroll_dimensions = scrollable_area->MaximumScrollOffset() - scrollable_area->MinimumScrollOffset();
    auto physical_orientation = ToPhysicalScrollOrientation(GetAxis(), *scroll_container);
    return physical_orientation == kHorizontalScroll ? scroll_dimensions.x() : scroll_dimensions.y();
}
