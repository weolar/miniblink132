...
KeyframeEffect* CreateSimpleKeyframeEffectForTest(Element* target, CSSPropertyID property, String value_start, String value_end)
{
    Timing timing;
    timing.iteration_duration = ANIMATION_TIME_DELTA_FROM_SECONDS(1000);

    auto* model = CreateSimpleKeyframeEffectModelForTest(property, value_start, value_end);
    return MakeGarbageCollected<KeyframeEffect>(target, model, timing);
}

KeyframeEffectModelBase* CreateSimpleKeyframeEffectModelForTest(CSSPropertyID property, String value_start, String value_end)
{
    StringKeyframe* start_keyframe = MakeGarbageCollected<StringKeyframe>();
    start_keyframe->SetOffset(0.0);
    start_keyframe->SetCSSPropertyValue(property, value_start, SecureContextMode::kSecureContext, nullptr);

    StringKeyframe* end_keyframe = MakeGarbageCollected<StringKeyframe>();
    end_keyframe->SetOffset(1.0);
    end_keyframe->SetCSSPropertyValue(property, value_end, SecureContextMode::kSecureContext, nullptr);
...
void EnsureInterpolatedValueCached(ActiveInterpolations* interpolations, Document& document, Element* element)
{
    ...
    const ComputedStyle& initial_style = document.GetStyleResolver().InitialStyle();
    StyleResolverState state(document, *element, nullptr /* StyleRecalcContext */, StyleRequest(&initial_style));
