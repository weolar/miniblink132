...
class InheritedScaleChecker : public CSSInterpolationType::CSSConversionChecker {
public:
...
private:
    bool IsValid(const StyleResolverState& state, const InterpolationValue&) const final
    {
        if (state.ParentStyle()->Scale()) {
            return state.ParentStyle()->Scale()->X() != scales_[0] && state.ParentStyle()->Scale()->Y() != scales_[1]
...
public:
    ...
    static scoped_refptr<CSSScaleNonInterpolableValue> Create(const InterpolableList& list)
    {
        return base::AdoptRef(new CSSScaleNonInterpolableValue(list, list, false, false));
    }

    static scoped_refptr<CSSScaleNonInterpolableValue> CreateAdditive(const CSSScaleNonInterpolableValue& other)
    {
        const bool is_additive = true;
        return base::AdoptRef(new CSSScaleNonInterpolableValue(*other.start_, *other.end_, is_additive, is_additive));
    }

    static scoped_refptr<CSSScaleNonInterpolableValue> Merge(const CSSScaleNonInterpolableValue& start, const CSSScaleNonInterpolableValue& end)
    {
        return base::AdoptRef(new CSSScaleNonInterpolableValue(start.Start(), end.end(), start.IsStartAdditive(), end.IsEndAdditive()));
    }

    const InterpolableList& Start() const
    {
        return *start_;
    }
    const InterpolableList& end() const
    {
        return *end_;
    }
    bool IsStartAdditive() const
    {
        return is_start_additive_;
    }
    bool IsEndAdditive() const
    {
        return is_end_additive_;
    }

    DECLARE_NON_INTERPOLABLE_VALUE_TYPE();

private:
    CSSScaleNonInterpolableValue(const InterpolableList& start, const InterpolableList& end, bool is_start_additive, bool is_end_additive)
        : start_(start.Clone())
        , end_(end.Clone())
...
template <> struct DowncastTraits<CSSScaleNonInterpolableValue> {
...
{
    if (!op) {
        return InterpolationValue(MakeGarbageCollected<InterpolableList>(0), CSSScaleNonInterpolableValue::Create(*MakeGarbageCollected<InterpolableList>(0)));
    }
...
InterpolationValue CreateInterpolationValue(std::array<InterpolableNumber*, 3> a)
{
    auto* list = MakeGarbageCollected<InterpolableList>(3);
    list->Set(0, a[0]);
    list->Set(1, a[1]);
...
InterpolationValue CreateInterpolationValue()
{
    ...
    return InterpolationValue(MakeGarbageCollected<InterpolableList>(0), CSSScaleNonInterpolableValue::Create(*list));
}
...
InterpolationValue CSSScaleInterpolationType::MaybeConvertInherit(const StyleResolverState& state, ConversionCheckers& conversion_checkers) const
{
    ScaleTransformOperation* op = state.ParentStyle()->Scale();
    double x = op ? op->X() : 1.0;
    double y = op ? op->Y() : 1.0;
    double z = op ? op->Z() : 1.0;
    conversion_checkers.push_back(MakeGarbageCollected<InheritedScaleChecker>(!op, std::array<double, 3>({ x, y, z })));
...
InterpolationValue CSSScaleInterpolationType::MaybeConvertValue(const CSSValue& value, const StyleResolverState*, ConversionCheckers&) const
{
    ...
    if (list.length() == 1) {
        ...
        return CreateInterpolationValue({ scale, scale, MakeGarbageCollected<InterpolableNumber>(1.0) });
        ...
        return CreateInterpolationValue({ x_scale, y_scale, MakeGarbageCollected<InterpolableNumber>(1.0) });
    } else {
        InterpolableNumber* x_scale = CSSValueToInterpolableNumber(list.Item(0));
        InterpolableNumber* y_scale = CSSValueToInterpolableNumber(list.Item(1));
...
{
    value.non_interpolable_value = CSSScaleNonInterpolableValue::CreateAdditive(To<CSSScaleNonInterpolableValue>(*value.non_interpolable_value));
    return value;
}
...
{
    wtf_size_t start_list_length = To<InterpolableList>(*start.interpolable_value).length();
    wtf_size_t end_list_length = To<InterpolableList>(*end.interpolable_value).length();
    ...
    return PairwiseInterpolationValue(std::move(start.interpolable_value), std::move(end.interpolable_value),
        CSSScaleNonInterpolableValue::Merge(
            To<CSSScaleNonInterpolableValue>(*start.non_interpolable_value), To<CSSScaleNonInterpolableValue>(*end.non_interpolable_value)));
}

InterpolationValue CSSScaleInterpolationType::MaybeConvertStandardPropertyUnderlyingValue(const ComputedStyle& style) const
{
...
{
    if (To<InterpolableList>(*underlying_value_owner.MutableValue().interpolable_value).length() == 0) {
        underlying_value_owner.MutableValue().interpolable_value = CreateScaleIdentity();
    }

    const auto& metadata = To<CSSScaleNonInterpolableValue>(*value.non_interpolable_value);
    ...
    for (wtf_size_t i = 0; i < 3; i++) {
        ...
        InterpolableNumber& start_number = metadata.IsStartAdditive() ? *underlying.Clone() : *MakeGarbageCollected<InterpolableNumber>(1.0);
        start_number.Scale(*To<InterpolableNumber>(metadata.Start().Get(i)));
        InterpolableNumber& end_number = metadata.IsEndAdditive() ? *underlying.Clone() : *MakeGarbageCollected<InterpolableNumber>(1.0);
    ...
    const InterpolableValue& interpolable_value, const NonInterpolableValue*, StyleResolverState& state) const
{
    ...
    state.StyleBuilder().SetScale(MakeGarbageCollected<ScaleTransformOperation>(To<InterpolableNumber>(list.Get(0))->Value(state.CssToLengthConversionData()),
        To<InterpolableNumber>(list.Get(1))->Value(state.CssToLengthConversionData()),
        To<InterpolableNumber>(list.Get(2))->Value(state.CssToLengthConversionData()), TransformOperation::kScale3D));
}
