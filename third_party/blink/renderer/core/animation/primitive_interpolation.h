...
public:
    ...
    virtual void InterpolateValue(double fraction, Member<TypedInterpolationValue>& result) const = 0;
...
public:
    PairwisePrimitiveInterpolation(
        const InterpolationType& type, InterpolableValue* start, InterpolableValue* end, scoped_refptr<const NonInterpolableValue> non_interpolable_value)
        : type_(type)
        , start_(start)
        , end_(end)
    ...
    {
        return MakeGarbageCollected<TypedInterpolationValue>(type_, start_->Clone(), non_interpolable_value_);
    }
...
private:
    void InterpolateValue(double fraction, Member<TypedInterpolationValue>& result) const final
    {
        DCHECK(result);
        ...
        start_->Interpolate(*end_, fraction, *result->MutableValue().interpolable_value);
    }

    double InterpolateUnderlyingFraction(double start, double end, double fraction) const final
    {
...
public:
    FlipPrimitiveInterpolation(TypedInterpolationValue* start, TypedInterpolationValue* end)
        : start_(start)
        , end_(end)
...
private:
    void InterpolateValue(double fraction, Member<TypedInterpolationValue>& result) const final
    {
        if (!std::isnan(last_fraction_) && (fraction < 0.5) == (last_fraction_ < 0.5)) {
            return;
        }
        const TypedInterpolationValue* side = ((fraction < 0.5) ? start_ : end_).Get();
