...
public:
    ...
    public:
        ...
        {
            return IsValid(To<CSSInterpolationEnvironment>(environment).GetState(), underlying);
        }

    protected:
        virtual bool IsValid(const StyleResolverState&, const InterpolationValue& underlying) const = 0;
    };
    ...
    virtual const CSSValue* CreateCSSValue(const InterpolableValue&, const NonInterpolableValue*, const StyleResolverState&) const
    {
...
protected:
    explicit CSSInterpolationType(PropertyHandle, const PropertyRegistration* = nullptr);
    ...
    InterpolationValue MaybeConvertSingle(
        const PropertySpecificKeyframe&, const InterpolationEnvironment&, const InterpolationValue& underlying, ConversionCheckers&) const final;

    InterpolationValue MaybeConvertUnderlyingValue(const InterpolationEnvironment&) const override;
    virtual InterpolationValue MaybeConvertStandardPropertyUnderlyingValue(const ComputedStyle&) const = 0;

    void Apply(const InterpolableValue&, const NonInterpolableValue*, InterpolationEnvironment&) const final;
    virtual void ApplyStandardPropertyValue(const InterpolableValue&, const NonInterpolableValue*, StyleResolverState&) const = 0;

private:
    InterpolationValue MaybeConvertSingleInternal(
        const PropertySpecificKeyframe&, const InterpolationEnvironment&, const InterpolationValue& underlying, ConversionCheckers&) const;

    InterpolationValue MaybeConvertCustomPropertyDeclaration(const CSSValue&, const InterpolationEnvironment&, ConversionCheckers&) const;
    ...
    void ApplyCustomPropertyValue(const InterpolableValue&, const NonInterpolableValue*, StyleResolverState&) const;
