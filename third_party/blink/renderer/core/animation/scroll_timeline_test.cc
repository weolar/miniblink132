...
#define EXPECT_CURRENT_TIME_AS_PERCENT_NEAR(expected, animation)                                                                                               \
    EXPECT_NEAR(                                                                                                                                               \
        expected, (animation->CurrentTimeInternal()->InMillisecondsF() / animation->timeline()->GetDuration()->InMillisecondsF()) * 100, percent_precision);

Animation* CreateTestAnimation(AnimationTimeline* timeline)
{
    Timing timing;
    timing.iteration_duration = ANIMATION_TIME_DELTA_FROM_SECONDS(0.1);
    return Animation::Create(MakeGarbageCollected<KeyframeEffect>(nullptr, MakeGarbageCollected<StringKeyframeEffectModel>(StringKeyframeVector()), timing),
        timeline, ASSERT_NO_EXCEPTION);
}

Animation* CreateCompositableTestAnimation(Element* target, AnimationTimeline* timeline)
{
    KeyframeEffect* effect = animation_test_helpers::CreateSimpleKeyframeEffectForTest(target, CSSPropertyID::kTranslate, "50px", "100px");
    effect->Model()->SnapshotAllCompositorKeyframesIfNecessary(*target, target->GetDocument().GetStyleResolver().InitialStyle(),
        /* parent_style */ nullptr);
    return MakeGarbageCollected<Animation>(timeline->GetDocument()->GetExecutionContext(), timeline, effect);
}
...
public:
    ...
    {
        return GetDocument().GetDocumentAnimations().GetTimelinesForTesting().size();
    }

    wtf_size_t AnimationsCount() const
    {
        wtf_size_t count = 0;
        for (auto timeline : GetDocument().GetDocumentAnimations().GetTimelinesForTesting()) {
...
public:
    TestScrollTimeline(Document* document, Element* source, bool snapshot = true)
        : ScrollTimeline(document, ScrollTimeline::ReferenceType::kSource, source, ScrollAxis::kY)
    {
        if (snapshot) {
            UpdateSnapshot();
        }
    ...
    void UpdateSnapshotForTesting()
    {
        UpdateSnapshot();
    }

    AnimationTimeDelta CalculateIntrinsicIterationDurationForTest(
        const std::optional<TimelineOffset>& range_start, const std::optional<TimelineOffset>& range_end)
    {
        Timing timing;
        timing.iteration_count = 1;
        TimelineRange timeline_range = GetTimelineRange();
        return CalculateIntrinsicIterationDuration(timeline_range, range_start, range_end, timing);
    }
};
...
TEST_F(ScrollTimelineTest, UsingDocumentScrollingElementShouldCorrectlyResolveToDocument)
{
    SetBodyInnerHTML(R"HTML(
    <style>
...
TEST_F(ScrollTimelineTest, ChangingDocumentScrollingElementShouldNotImpactScrollTimeline)
{
    SetBodyInnerHTML(R"HTML(
    <style>
...
TEST_F(ScrollTimelineTest, AttachOrDetachAnimationWithNullSource)
{
    ...
    Persistent<ScrollTimeline> scroll_timeline = ScrollTimeline::Create(&GetDocument(), scroll_source, ScrollTimeline::ScrollAxis::kBlock);
    ...
    Animation* animation
        = Animation::Create(MakeGarbageCollected<KeyframeEffect>(nullptr, MakeGarbageCollected<StringKeyframeEffectModel>(StringKeyframeVector()), timing),
...
TEST_F(ScrollTimelineTest, AnimationIsGarbageCollectedWhenScrollerIsRemoved)
{
    ...
    TestScrollTimeline* scroll_timeline = MakeGarbageCollected<TestScrollTimeline>(&GetDocument(), GetElementById("scroller"));
    NonThrowableExceptionState exception_state;
    Timing timing;
    timing.iteration_duration = ANIMATION_TIME_DELTA_FROM_SECONDS(30);
    Animation* animation
...
TEST_F(ScrollTimelineTest, AnimationPersistsWhenFinished)
{
    ...
    Persistent<TestScrollTimeline> scroll_timeline = MakeGarbageCollected<TestScrollTimeline>(&GetDocument(), GetElementById("scroller"));
    ...
    scrollable_area->SetScrollOffset(offset, mojom::blink::ScrollType::kProgrammatic);
    SimulateFrame();
...
TEST_F(ScrollTimelineTest, AnimationPersistsWhenSourceBecomesNonScrollable)
{
    ...
    scroller->GetScrollableArea()->SetScrollOffset(offset_50, mojom::blink::ScrollType::kProgrammatic);
    SimulateFrame();
    EXPECT_CURRENT_TIME_AS_PERCENT_NEAR(50.0, animation);
    ...
    scroller->GetScrollableArea()->SetScrollOffset(offset_40, mojom::blink::ScrollType::kProgrammatic);
    SimulateFrame();
...
TEST_F(ScrollTimelineTest, ScheduleFrameOnlyWhenScrollOffsetChanges)
{
    ...
    TestScrollTimeline* scroll_timeline = MakeGarbageCollected<TestScrollTimeline>(&GetDocument(), scroller_element);
    ...
    GetChromeClient().UnsetAnimationScheduled();
    scrollable_area->SetScrollOffset(ScrollOffset(0, 30), mojom::blink::ScrollType::kProgrammatic);
    GetFrame().ScheduleNextServiceForScrollSnapshotClients();
...
TEST_F(ScrollTimelineTest, ScheduleFrameWhenScrollerLayoutChanges)
{
    ...
    spacer_element->setAttribute(html_names::kStyleAttr, AtomicString("height:1000px;"));
    GetChromeClient().UnsetAnimationScheduled();
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(GetChromeClient().AnimationScheduled());

    // Also test changing the scroller height, which also affect the max offset.
    GetElementById("scroller")->setAttribute(html_names::kStyleAttr, AtomicString("height: 200px"));
    GetChromeClient().UnsetAnimationScheduled();
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(GetChromeClient().AnimationScheduled());
}

TEST_F(ScrollTimelineTest, TimelineInvalidationWhenScrollerDisplayPropertyChanges)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #scroller { overflow: scroll; width: 100px; height: 100px; }
    ...
    scroller_element->setAttribute(html_names::kStyleAttr, AtomicString("display:table-cell;"));
    GetChromeClient().UnsetAnimationScheduled();
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(GetChromeClient().AnimationScheduled());
}
...
TEST_F(ScrollTimelineTest, CurrentTimeUpdateAfterNewAnimationFrame)
{
    ...
    scrollable_area->SetScrollOffset(ScrollOffset(0, 5), mojom::blink::ScrollType::kProgrammatic);

    ScrollTimeline* scroll_timeline = ScrollTimeline::Create(GetDocument(), options, ASSERT_NO_EXCEPTION);

    double time_before = scroll_timeline->CurrentTimeSeconds().value();

    scrollable_area->SetScrollOffset(ScrollOffset(0, 10), mojom::blink::ScrollType::kProgrammatic);
...
TEST_F(ScrollTimelineTest, FinishedAnimationPlaysOnReversedScrolling)
{
    ...
    // current time.
    SimulateFrame();
    ASSERT_EQ(V8AnimationPlayState::Enum::kFinished, scroll_animation->playState());
    ...
    EXPECT_EQ(V8AnimationPlayState::Enum::kRunning, scroll_animation->playState());
}
...
TEST_F(ScrollTimelineTest, FiringAnimationEventsByFinishedAnimationOnReversedScrolling)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #scroller { overflow: scroll; width: 100px; height: 100px; }
    ...
    // Scroll back.
    scrollable_area->SetScrollOffset(ScrollOffset(0, 80), mojom::blink::ScrollType::kProgrammatic);
    SimulateFrame();
    ...
    scrollable_area->SetScrollOffset(ScrollOffset(0, 95), mojom::blink::ScrollType::kProgrammatic);
    SimulateFrame();
...
TEST_F(ScrollTimelineTest, WeakViewTimelines)
{
    ...
    StaticElementList* list = GetDocument().QuerySelectorAll(AtomicString("#scroller > div"));
    ASSERT_TRUE(list);
    ...
    for (wtf_size_t i = 0; i < list->length(); ++i) {
        ...
        Animation* animation = CreateTestAnimation(MakeGarbageCollected<TestViewTimeline>(&GetDocument(), element));
        animation->play();
...
TEST_F(ScrollTimelineTest, ScrollTimelineOffsetZoom)
{
    ...
    // zoom = 1
    {
        auto* timeline = MakeGarbageCollected<TestScrollTimeline>(&GetDocument(), GetElementById("scroller"));
        std::optional<ScrollOffsets> scroll_offsets = timeline->GetResolvedScrollOffsets();
...
TEST_F(ScrollTimelineTest, ViewTimelineOffsetZoom)
{
    ...
    // zoom = 1
    {
        auto* timeline = MakeGarbageCollected<TestViewTimeline>(&GetDocument(), GetElementById("subject"));
        std::optional<ScrollOffsets> scroll_offsets = timeline->GetResolvedScrollOffsets();
        ASSERT_TRUE(scroll_offsets.has_value());
...
TEST_F(ScrollTimelineTest, ScrollTimelineGetTimelineRange)
{
    ...
    auto* timeline = MakeGarbageCollected<TestScrollTimeline>(&GetDocument(), GetElementById("scroller"), /* snapshot */ false);

    // GetTimelineRange before taking a snapshot.
    EXPECT_TRUE(timeline->GetTimelineRange().IsEmpty());

    timeline->UpdateSnapshotForTesting();
    EXPECT_EQ(TimelineRange(TimelineRange::ScrollOffsets(0, 300), TimelineRange::ViewOffsets(0, 0)), timeline->GetTimelineRange());
}
...
TEST_F(ScrollTimelineTest, ViewTimelineGetTimelineRange)
{
    ...
    auto* timeline = MakeGarbageCollected<TestViewTimeline>(&GetDocument(), GetElementById("subject"), /* snapshot */ false);

    // GetTimelineRange before taking a snapshot.
    EXPECT_TRUE(timeline->GetTimelineRange().IsEmpty());

    timeline->UpdateSnapshotForTesting();
    EXPECT_EQ(TimelineRange(TimelineRange::ScrollOffsets(100, 300), TimelineRange::ViewOffsets(100, 100)), timeline->GetTimelineRange());
}
...
TEST_F(ScrollTimelineTest, ScrollTimelineCalculateIntrinsicIterationDuration)
{
    ...
    // [0, 300]
    EXPECT_TRUE(TimingCalculations::IsWithinAnimationTimeTolerance(duration,
    ...
    EXPECT_TRUE(TimingCalculations::IsWithinAnimationTimeTolerance(duration,
        timeline->CalculateIntrinsicIterationDurationForTest(
            ...
            /* range_end */ TimelineOffset(NamedRange::kNone, Length::Fixed(300)))));

    // [50, 200]
    EXPECT_TRUE(TimingCalculations::IsWithinAnimationTimeTolerance(duration / 2.0,
        timeline->CalculateIntrinsicIterationDurationForTest(
            /* range_start */
            TimelineOffset(NamedRange::kNone, Length::Fixed(50)),
            /* range_end */ TimelineOffset(NamedRange::kNone, Length::Fixed(200)))));
    ...
    // [50, 50]
    EXPECT_TRUE(TimingCalculations::IsWithinAnimationTimeTolerance(AnimationTimeDelta(),
        timeline->CalculateIntrinsicIterationDurationForTest(
            /* range_start */
            TimelineOffset(NamedRange::kNone, Length::Fixed(50)),
            /* range_end */ TimelineOffset(NamedRange::kNone, Length::Fixed(50)))));
}
...
TEST_F(ScrollTimelineTest, CompositedDeferredTimelineReattachment)
{
    ...
    TestScrollTimeline* scroll_timeline = MakeGarbageCollected<TestScrollTimeline>(&GetDocument(), GetElementById("scroller"));
    TestDeferredTimeline* deferred_timeline = MakeGarbageCollected<TestDeferredTimeline>(&GetDocument());

    deferred_timeline->AttachTimeline(scroll_timeline);

    Animation* animation = CreateCompositableTestAnimation(GetElementById("target"), deferred_timeline);
    ...
    deferred_timeline->AttachTimeline(MakeGarbageCollected<TestScrollTimeline>(&GetDocument(), GetElementById("scroller")));
