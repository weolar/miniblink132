...
std::tuple<double, double, double, double> AddPremultipliedColor(
    double param0, double param1, double param2, double alpha, double fraction, Color color, Color::ColorSpace color_space)
{
    DCHECK(color_space == Color::ColorSpace::kSRGBLegacy || color_space == Color::ColorSpace::kOklab);
    color.ConvertToColorSpace(color_space);
    return std::make_tuple(param0 + fraction * color.Param0() * color.Alpha(), param1 + fraction * color.Param1() * color.Alpha(),
        param2 + fraction * color.Param2() * color.Alpha(), alpha + fraction * color.Alpha());
}

// Convert color parameters back to unpremultiplied form (not scaled by alpha)
// suitable for the Color constructor.
std::tuple<double, double, double> UnpremultiplyColor(double param0, double param1, double param2, double alpha)
{
...
InterpolableColor* InterpolableColor::Create(CSSValueID keyword, mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider)
{
    switch (keyword) {
    ...
    default:
        ...
        // from Document.
        return Create(StyleColor::ColorFromKeyword(keyword, color_scheme, color_provider,
            /*is_in_web_app_scope=*/false));
    }
}

InterpolableColor::InterpolableColor(InlinedInterpolableDouble param0, InlinedInterpolableDouble param1, InlinedInterpolableDouble param2,
    InlinedInterpolableDouble alpha, InlinedInterpolableDouble current_color, InlinedInterpolableDouble webkit_active_link,
    InlinedInterpolableDouble webkit_link, InlinedInterpolableDouble quirk_inherit, Color::ColorSpace color_space)
...
InterpolableColor* InterpolableColor::RawClone() const
{
    return MakeGarbageCollected<InterpolableColor>(
        param0_, param1_, param2_, alpha_, current_color_, webkit_active_link_, webkit_link_, quirk_inherit_, color_space_);
}

InterpolableColor* InterpolableColor::RawCloneAndZero() const
{
    return MakeGarbageCollected<InterpolableColor>(InlinedInterpolableDouble(0), InlinedInterpolableDouble(0), InlinedInterpolableDouble(0),
        InlinedInterpolableDouble(0), InlinedInterpolableDouble(0), InlinedInterpolableDouble(0), InlinedInterpolableDouble(0), InlinedInterpolableDouble(0),
        color_space_);
}
...
void InterpolableColor::AssertCanInterpolateWith(const InterpolableValue& other) const
{
...
bool InterpolableColor::IsKeywordColor() const
{
    ...
    return current_color_.Value() || webkit_active_link_.Value() || webkit_link_.Value() || quirk_inherit_.Value();
}
...
// static
void InterpolableColor::SetupColorInterpolationSpaces(InterpolableColor& to, InterpolableColor& from)
{
    ...
    DCHECK(from.color_space_ == Color::ColorSpace::kOklab || to.color_space_ == Color::ColorSpace::kOklab);
...
Color InterpolableColor::Resolve(
    const Color& current_color, const Color& active_link_color, const Color& link_color, const Color& text_color, mojom::blink::ColorScheme color_scheme) const
{
    ...
    if (double currentcolor_fraction = current_color_.Value()) {
        std::tie(param0, param1, param2, alpha) = AddPremultipliedColor(param0, param1, param2, alpha, currentcolor_fraction, current_color, color_space_);
    }
    if (double webkit_activelink_fraction = webkit_active_link_.Value()) {
        std::tie(param0, param1, param2, alpha)
            = AddPremultipliedColor(param0, param1, param2, alpha, webkit_activelink_fraction, active_link_color, color_space_);
    }
    if (double webkit_link_fraction = webkit_link_.Value()) {
        std::tie(param0, param1, param2, alpha) = AddPremultipliedColor(param0, param1, param2, alpha, webkit_link_fraction, link_color, color_space_);
    }
    if (double quirk_inherit_fraction = quirk_inherit_.Value()) {
        std::tie(param0, param1, param2, alpha) = AddPremultipliedColor(param0, param1, param2, alpha, quirk_inherit_fraction, text_color, color_space_);
    }
    ...
    std::tie(param0, param1, param2) = UnpremultiplyColor(param0, param1, param2, alpha);
...
void InterpolableColor::Interpolate(const InterpolableValue& to, const double progress, InterpolableValue& result) const
{
    ...
    result_color.param0_.Set(param0_.Interpolate(to_color.param0_.Value(), progress));
    result_color.param1_.Set(param1_.Interpolate(to_color.param1_.Value(), progress));
    result_color.param2_.Set(param2_.Interpolate(to_color.param2_.Value(), progress));
    result_color.alpha_.Set(alpha_.Interpolate(to_color.alpha_.Value(), progress));

    result_color.current_color_.Set(current_color_.Interpolate(to_color.current_color_.Value(), progress));
    result_color.webkit_active_link_.Set(webkit_active_link_.Interpolate(to_color.webkit_active_link_.Value(), progress));
    result_color.webkit_link_.Set(webkit_link_.Interpolate(to_color.webkit_link_.Value(), progress));
    result_color.quirk_inherit_.Set(quirk_inherit_.Interpolate(to_color.quirk_inherit_.Value(), progress));
}

void InterpolableColor::Composite(const BaseInterpolableColor& value, double fraction)
{
