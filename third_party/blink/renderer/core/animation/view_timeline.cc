...
using InsetValueSequence = const HeapVector<Member<V8UnionCSSNumericValueOrString>>;
...
TimelineInset ResolveAuto(const TimelineInset& inset, Element& source, ViewTimeline::ScrollAxis axis)
{
    ...
    if (IsBlockDirection(axis, style->GetWritingMode())) {
        return TimelineInset(start.IsAuto() ? style->ScrollPaddingBlockStart() : start, end.IsAuto() ? style->ScrollPaddingBlockEnd() : end);
    }
    return TimelineInset(start.IsAuto() ? style->ScrollPaddingInlineStart() : start, end.IsAuto() ? style->ScrollPaddingInlineEnd() : end);
}
...
const CSSValue* ParseInset(const InsetValueSequence& array, wtf_size_t index, ExceptionState& exception_state)
{
    if (index >= array.size())
        return nullptr;
    ...
    const CSSPrimitiveValue* css_value = DynamicTo<CSSPrimitiveValue>(numeric_value->ToCSSValue());
...
{
    const CSSValue* value = CSSParser::ParseSingleValue(CSSPropertyID::kViewTimelineInset, str_value, document.ElementSheet().Contents()->ParserContext());
...
bool IsStyleDependent(const CSSValue* value)
{
    ...
    if (const CSSPrimitiveValue* css_primitive_value = DynamicTo<CSSPrimitiveValue>(value)) {
...
Length InsetValueToLength(const CSSValue* inset_value, Element* subject, Length default_value)
{
    if (!inset_value)
    ...
    if (inset_value->IsIdentifierValue()) {
        DCHECK_EQ(To<CSSIdentifierValue>(inset_value)->GetValueID(), CSSValueID::kAuto);
        return Length(Length::Type::kAuto);
    }
    ...
    if (!subject->GetComputedStyle()) {
        if (const CSSNumericLiteralValue* literal_value = DynamicTo<CSSNumericLiteralValue>(inset_value)) {
    ...
    if (inset_value->IsPrimitiveValue()) {
        ...
        CSSToLengthConversionData length_conversion_data(subject->ComputedStyleRef(), element_resolve_context.ParentStyle(),
            element_resolve_context.RootElementStyle(), CSSToLengthConversionData::ViewportSize(document.GetLayoutView()),
            CSSToLengthConversionData::ContainerSizes(subject), CSSToLengthConversionData::AnchorData(), subject->GetComputedStyle()->EffectiveZoom(),
            ignored_flags);

        return DynamicTo<CSSPrimitiveValue>(inset_value)->ConvertToLength(length_conversion_data);
    }

    NOTREACHED();
}

enum class StickinessRange { kBeforeEntry, kDuringEntry, kWhileContained, kWhileCovering, kDuringExit, kAfterExit };

StickinessRange ComputeStickinessRange(
    LayoutUnit sticky_box_stuck_pos_in_viewport, LayoutUnit sticky_box_static_pos, double viewport_size, double target_size, double target_pos)
{
    // Need to know: when the sticky box is stuck, where is the view-timeline
    // target in relation to the scroller's viewport?
    double target_pos_in_viewport = sticky_box_stuck_pos_in_viewport + target_pos - sticky_box_static_pos.ToDouble();

    if (target_pos_in_viewport < 0 && target_pos_in_viewport + target_size > viewport_size) {
...
ViewTimeline* ViewTimeline::Create(Document& document, ViewTimelineOptions* options, ExceptionState& exception_state)
{
    Element* subject = options->hasSubject() ? options->subject() : nullptr;

    ScrollAxis axis = options->hasAxis() ? options->axis().AsEnum() : ScrollAxis::kBlock;

    if (subject) {
        // This ensures that Client[Left,Top]NoLayout (reached via SnapshotState)
        // returns up-to-date information.
        document.UpdateStyleAndLayoutForNode(subject, DocumentUpdateReason::kJavaScript);
    }

    // Parse insets.
    const V8UnionCSSNumericValueOrStringSequenceOrString* v8_inset = options->inset();
    ...
    if (v8_inset && v8_inset->IsCSSNumericValueOrStringSequence()) {
        const InsetValueSequence inset_array = v8_inset->GetAsCSSNumericValueOrStringSequence();
        ...
        const CSSValuePair* value_pair = ParseInsetPair(document, v8_inset->GetAsString());
        if (!value_pair) {
            exception_state.ThrowTypeError("Invalid inset");
            return nullptr;
        }
    ...
    Length inset_start_side = InsetValueToLength(start_inset_value.value_or(nullptr), subject, Length(Length::Type::kFixed));
    Length inset_end_side = InsetValueToLength(end_inset_value.value_or(nullptr), subject, inset_start_side);

    ViewTimeline* view_timeline = MakeGarbageCollected<ViewTimeline>(&document, subject, axis, TimelineInset(inset_start_side, inset_end_side));
...
ViewTimeline::ViewTimeline(Document* document, Element* subject, ScrollAxis axis, TimelineInset inset)
    : ScrollTimeline(document, ReferenceType::kNearestAncestor,
        /* reference_element */ subject, axis)
    , inset_(inset)
{
}

void ViewTimeline::CalculateOffsets(PaintLayerScrollableArea* scrollable_area, ScrollOrientation physical_orientation, TimelineState* state) const
{
    ...
    std::optional<gfx::PointF> subject_position = SubjectPosition(scroll_container);
    DCHECK(subject_position);

    // TODO(crbug.com/1448801): Handle nested sticky elements.
    double target_offset = physical_orientation == kHorizontalScroll ? subject_position->x() : subject_position->y();
    double target_size;
    ...
    if (style_dependant_start_inset_ || style_dependant_end_inset_) {
        ...
        if (style_dependant_start_inset_) {
            updated_start = InsetValueToLength(style_dependant_start_inset_, subject(), Length::Fixed());
        }
        if (style_dependant_end_inset_) {
            updated_end = InsetValueToLength(style_dependant_end_inset_, subject(), Length::Fixed());
        }
    ...
    ScrollOffsets scroll_offsets = { target_offset - viewport_size_double + end_side_inset, target_offset + target_size - start_side_inset };
    ViewOffsets view_offsets = { target_size, target_size };
    ApplyStickyAdjustments(scroll_offsets, view_offsets, viewport_size_double, target_size, target_offset, physical_orientation, scroll_container);

    state->scroll_offsets = scroll_offsets;
    state->view_offsets = view_offsets;
}

void ViewTimeline::ApplyStickyAdjustments(ScrollOffsets& scroll_offsets, ViewOffsets& view_offsets, double viewport_size, double target_size,
    double target_offset, ScrollOrientation orientation, LayoutBox* scroll_container) const
{
    if (!subject()) {
        return;
    }
    ...
    const LayoutBoxModelObject* sticky_container = subject_layout_box->FindFirstStickyContainer(scroll_container);
    if (!sticky_container) {
        return;
    }

    StickyPositionScrollingConstraints* constraints = sticky_container->StickyConstraints();
    if (!constraints) {
        return;
    }

    const PhysicalRect& container = constraints->scroll_container_relative_containing_block_rect;
    const PhysicalRect& sticky_rect = constraints->scroll_container_relative_sticky_box_rect;
    ...
    if (is_horizontal) {
        if (constraints->left_inset) {
            ...
            forward_stickiness = ComputeStickinessRange(*constraints->left_inset, sticky_rect.X(), viewport_size, target_size, target_offset);
        }
        if (constraints->right_inset) {
            ...
            backward_stickiness = ComputeStickinessRange(
                LayoutUnit(viewport_size) - *constraints->right_inset - sticky_rect.Width(), sticky_rect.X(), viewport_size, target_size, target_offset);
        }
    } else { // Vertical.
        if (constraints->top_inset) {
            max_forward_adjust = (container.Bottom() - sticky_rect.Bottom()).ToDouble();
            forward_stickiness = ComputeStickinessRange(*constraints->top_inset, sticky_rect.Y(), viewport_size, target_size, target_offset);
        }
        if (constraints->bottom_inset) {
            max_backward_adjust = (container.Y() - sticky_rect.Y()).ToDouble();
            backward_stickiness = ComputeStickinessRange(
                LayoutUnit(viewport_size) - *constraints->bottom_inset - sticky_rect.Height(), sticky_rect.Y(), viewport_size, target_size, target_offset);
        }
    }
    ...
    if (forward_stickiness == StickinessRange::kDuringEntry || forward_stickiness == StickinessRange::kWhileCovering) {
        view_offsets.entry_crossing_distance += max_forward_adjust;
    }
    if (backward_stickiness == StickinessRange::kDuringEntry || backward_stickiness == StickinessRange::kWhileCovering) {
        view_offsets.entry_crossing_distance -= max_backward_adjust;
    }

    if (forward_stickiness == StickinessRange::kDuringExit || forward_stickiness == StickinessRange::kWhileCovering) {
        view_offsets.exit_crossing_distance += max_forward_adjust;
    }
    if (backward_stickiness == StickinessRange::kDuringExit || backward_stickiness == StickinessRange::kWhileCovering) {
...
std::optional<gfx::SizeF> ViewTimeline::SubjectSize() const
{
    ...
    if (subject_layout_object->IsSVGChild()) {
        ...
        const gfx::QuadF local_bounds(subject_layout_object->DecoratedBoundingBox());
        return subject_layout_object->LocalToAncestorQuad(local_bounds, To<LayoutSVGRoot>(svg_root)).BoundingBox().size();
    }
...
std::optional<gfx::PointF> ViewTimeline::SubjectPosition(LayoutBox* scroll_container) const
{
    ...
    MapCoordinatesFlags flags = kIgnoreScrollOffset | kIgnoreStickyOffset | kIgnoreTransforms;
    gfx::PointF subject_pos = subject_layout_object->LocalToAncestorPoint(gfx::PointF(), scroll_container, flags);
    ...
    return gfx::PointF(subject_pos.x() - scroll_container->ClientLeft().ToDouble(), subject_pos.y() - scroll_container->ClientTop().ToDouble());
}
...
CSSNumericValue* ViewTimeline::getCurrentTime(const String& rangeName)
{
    ...
    double timeline_progress = CurrentPhaseAndTime().time.value().InMillisecondsF() / GetDuration().value().InMillisecondsF();

    double named_range_progress = (timeline_progress - relative_start_offset) / range;
...
bool ViewTimeline::Matches(Element* subject, ScrollAxis axis, const TimelineInset& inset) const
{
...
double ViewTimeline::ToFractionalOffset(const TimelineOffset& timeline_offset) const
{
