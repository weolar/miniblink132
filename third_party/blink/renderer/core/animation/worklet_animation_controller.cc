...
void WorkletAnimationController::AttachAnimation(WorkletAnimationBase& animation)
{
    DCHECK(IsMainThread());
...
void WorkletAnimationController::DetachAnimation(WorkletAnimationBase& animation)
{
    DCHECK(IsMainThread());
...
void WorkletAnimationController::InvalidateAnimation(WorkletAnimationBase& animation)
{
    DCHECK(IsMainThread());
    pending_animations_.insert(&animation);
    if (LocalFrameView* view = animation.GetDocument()->View())
...
void WorkletAnimationController::UpdateAnimationTimings(TimingUpdateReason reason)
{
    DCHECK(IsMainThread());
...
base::WeakPtr<AnimationWorkletMutatorDispatcherImpl> WorkletAnimationController::EnsureMainThreadMutatorDispatcher(
    scoped_refptr<base::SingleThreadTaskRunner> mutator_task_runner)
{
    base::WeakPtr<AnimationWorkletMutatorDispatcherImpl> mutator_dispatcher;
    if (!main_thread_mutator_client_) {
        main_thread_mutator_client_ = AnimationWorkletMutatorDispatcherImpl::CreateMainThreadClient(mutator_dispatcher, std::move(mutator_task_runner));
...
void WorkletAnimationController::SynchronizeAnimatorName(const String& animator_name)
{
    animator_names_.insert(animator_name);
}

bool WorkletAnimationController::IsAnimatorRegistered(const String& animator_name) const
{
    return animator_names_.Contains(animator_name);
}

void WorkletAnimationController::SetMutationUpdate(std::unique_ptr<AnimationWorkletOutput> output_state)
{
    if (!output_state)
        return;
...
void WorkletAnimationController::MutateAnimations()
{
    ...
    main_thread_mutator_client_->Mutator()->MutateSynchronously(CollectAnimationStates());
}

std::unique_ptr<AnimationWorkletDispatcherInput> WorkletAnimationController::CollectAnimationStates()
{
    std::unique_ptr<AnimationWorkletDispatcherInput> result = std::make_unique<AnimationWorkletDispatcherInput>();
...
void WorkletAnimationController::ApplyAnimationTimings(TimingUpdateReason reason)
{
