...
public:
    InterpolableGridTrackSize(InterpolableValue* min_value, InterpolableValue* max_value, const GridTrackSizeType type);
    static InterpolableGridTrackSize* Create(const GridTrackSize& grid_track_size, const CSSProperty& property, float zoom);

    GridTrackSize CreateTrackSize(const CSSToLengthConversionData& conversion_data) const;
    ...
    bool IsGridTrackSize() const final
    {
        return true;
    }
...
template <> struct DowncastTraits<InterpolableGridTrackSize> {
