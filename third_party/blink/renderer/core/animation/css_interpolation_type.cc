...
public:
    ResolvedValueChecker(const PropertyHandle& property, const CSSValue* unresolved_value, const CSSValue* resolved_value)
        : property_(property)
...
private:
    bool IsValid(const InterpolationEnvironment& environment, const InterpolationValue&) const final
    {
        ...
        const CSSValue* resolved_value = css_environment.Resolve(property_, unresolved_value_);
...
public:
    ResolvedVariableChecker(CSSPropertyID property, const CSSValue* variable_reference, const CSSValue* resolved_value)
        : property_(property)
...
private:
    bool IsValid(const InterpolationEnvironment& environment, const InterpolationValue&) const final
    {
        ...
        const CSSValue* resolved_value = css_environment.Resolve(PropertyHandle(CSSProperty::Get(property_)), variable_reference_);
        return base::ValuesEquivalent(resolved_value_.Get(), resolved_value);
    }
...
class InheritedCustomPropertyChecker : public CSSInterpolationType::CSSConversionChecker {
public:
    InheritedCustomPropertyChecker(const AtomicString& name, bool is_inherited_property, const CSSValue* inherited_value, const CSSValue* initial_value)
        : name_(name)
...
private:
    ...
    {
        const CSSValue* inherited_value = state.ParentStyle()->GetVariableValue(name_, is_inherited_property_);
        if (!inherited_value) {
...
class ResolvedRegisteredCustomPropertyChecker : public InterpolationType::ConversionChecker {
public:
    ResolvedRegisteredCustomPropertyChecker(const PropertyHandle& property, const CSSValue& value, CSSVariableData* resolved_tokens)
        : property_(property)
        , value_(value)
        , resolved_tokens_(resolved_tokens)
    {
    }
...
template <typename RevertValueType> class RevertChecker : public CSSInterpolationType::ConversionChecker {
public:
    static_assert(std::is_same<RevertValueType, cssvalue::CSSRevertValue>::value || std::is_same<RevertValueType, cssvalue::CSSRevertLayerValue>::value,
        "RevertCheck only accepts CSSRevertValue and CSSRevertLayerValue");

    RevertChecker(const PropertyHandle& property_handle, const CSSValue* resolved_value)
...
private:
    ...
    {
        const auto& css_environment = To<CSSInterpolationEnvironment>(environment);
        const CSSValue* current_resolved_value = css_environment.Resolve(property_handle_, RevertValueType::Create());
        return base::ValuesEquivalent(resolved_value_.Get(), current_resolved_value);
    }
...
CSSInterpolationType::CSSInterpolationType(PropertyHandle property, const PropertyRegistration* registration)
...
InterpolationValue CSSInterpolationType::MaybeConvertSingle(const PropertySpecificKeyframe& keyframe, const InterpolationEnvironment& environment,
    const InterpolationValue& underlying, ConversionCheckers& conversion_checkers) const
{
    InterpolationValue result = MaybeConvertSingleInternal(keyframe, environment, underlying, conversion_checkers);
    if (result && keyframe.Composite() != EffectModel::CompositeOperation::kCompositeReplace) {
        return PreInterpolationCompositeIfNeeded(std::move(result), underlying, keyframe.Composite(), conversion_checkers);
    }
    return result;
}

InterpolationValue CSSInterpolationType::MaybeConvertSingleInternal(const PropertySpecificKeyframe& keyframe, const InterpolationEnvironment& environment,
    const InterpolationValue& underlying, ConversionCheckers& conversion_checkers) const
{
    ...
    if (GetProperty().IsCSSCustomProperty()) {
        return MaybeConvertCustomPropertyDeclaration(*value, environment, conversion_checkers);
    }

    if (value->IsUnparsedDeclaration() || value->IsPendingSubstitutionValue()) {
        const CSSValue* resolved_value = css_environment.Resolve(GetProperty(), value);

        DCHECK(resolved_value);
        conversion_checkers.push_back(MakeGarbageCollected<ResolvedVariableChecker>(CssProperty().PropertyID(), value, resolved_value));
    ...
    if (value->IsMathFunctionValue()) {
        ...
        const CSSValue* resolved_value = css_environment.Resolve(GetProperty(), value);
        DCHECK(resolved_value);
        conversion_checkers.push_back(MakeGarbageCollected<ResolvedValueChecker>(GetProperty(), /* unresolved_value */ value, resolved_value));
        value = resolved_value;
    }
    ...
    if (value->IsRevertValue()) {
        ...
        DCHECK(value);
        conversion_checkers.push_back(MakeGarbageCollected<RevertChecker<cssvalue::CSSRevertValue>>(GetProperty(), value));
    }

    if (value->IsRevertLayerValue()) {
        value = css_environment.Resolve(GetProperty(), value);
        DCHECK(value);
        conversion_checkers.push_back(MakeGarbageCollected<RevertChecker<cssvalue::CSSRevertLayerValue>>(GetProperty(), value));
    }
...
InterpolationValue CSSInterpolationType::MaybeConvertCustomPropertyDeclaration(
    const CSSValue& declaration, const InterpolationEnvironment& environment, ConversionCheckers& conversion_checkers) const
{
    const auto& css_environment = To<CSSInterpolationEnvironment>(environment);
    const StyleResolverState& state = css_environment.GetState();
    ...
    if (const auto* resolved_declaration = DynamicTo<CSSUnparsedDeclarationValue>(value)) {
        ...
        if (resolved_declaration != &declaration) {
            conversion_checkers.push_back(
                MakeGarbageCollected<ResolvedRegisteredCustomPropertyChecker>(GetProperty(), declaration, resolved_declaration->VariableDataValue()));
        }
    }
    ...
    if (value->IsInitialValue() || (value->IsUnsetValue() && !is_inherited)) {
        value = initial_value;
    } else if (value->IsInheritedValue() || (value->IsUnsetValue() && is_inherited)) {
        value = state.ParentStyle()->GetVariableValue(name, is_inherited);
        if (!value) {
            value = initial_value;
        }
        conversion_checkers.push_back(MakeGarbageCollected<InheritedCustomPropertyChecker>(name, is_inherited, value, initial_value));
    }

    if (const auto* resolved_declaration = DynamicTo<CSSUnparsedDeclarationValue>(value)) {
        DCHECK(!resolved_declaration->VariableDataValue()->NeedsVariableResolution());
        value = resolved_declaration->VariableDataValue()->ParseForSyntax(
            registration_->Syntax(), state.GetDocument().GetExecutionContext()->GetSecureContextMode());
        if (!value)
            return nullptr;
    }
...
InterpolationValue CSSInterpolationType::MaybeConvertUnderlyingValue(const InterpolationEnvironment& environment) const
{
    const auto& css_environment = To<CSSInterpolationEnvironment>(environment);
    ...
    const CSSValue* underlying_value = style.GetVariableValue(name, Registration().Inherits());
    if (!underlying_value)
        return nullptr;
    // TODO(alancutter): Remove the need for passing in conversion checkers.
    ConversionCheckers dummy_conversion_checkers;
    return MaybeConvertValue(*underlying_value, css_environment.GetOptionalState(), dummy_conversion_checkers);
}

void CSSInterpolationType::Apply(
    const InterpolableValue& interpolable_value, const NonInterpolableValue* non_interpolable_value, InterpolationEnvironment& environment) const
{
    StyleResolverState& state = To<CSSInterpolationEnvironment>(environment).GetState();
    ...
    AnchorScope anchor_scope(CssProperty().PropertyID(), state.CssToLengthConversionData().GetAnchorEvaluator());
    ...
    const InterpolableValue& interpolable_value, const NonInterpolableValue* non_interpolable_value, StyleResolverState& state) const
{
    ...
    const CSSValue* css_value = CreateCSSValue(interpolable_value, non_interpolable_value, state);
    DCHECK(!css_value->IsUnparsedDeclaration());
    StyleBuilder::ApplyProperty(GetProperty().GetCSSPropertyName(), state, *css_value, StyleBuilder::ValueMode::kAnimated);
}
