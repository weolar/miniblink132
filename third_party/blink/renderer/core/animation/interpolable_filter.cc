...
InterpolableNumber* ConvertFilterOperationToInterpolableNumber(CSSValueID type, const CSSValue& item)
{
    switch (type) {
    ...
    case CSSValueID::kOpacity: {
        ...
        if (value.IsPercentage()) {
            return MakeGarbageCollected<InterpolableNumber>(*value.ConvertLiteralsFromPercentageToNumber());
        }
        return MakeGarbageCollected<InterpolableNumber>(value);
    }
    case CSSValueID::kHueRotate: {
        return MakeGarbageCollected<InterpolableNumber>(To<CSSPrimitiveValue>(item));
    }
    default:
...
InterpolableNumber* CreateDefaultValue(CSSValueID type)
{
    ...
    case CSSValueID::kHueRotate:
        return MakeGarbageCollected<InterpolableNumber>(0, CSSPrimitiveValue::UnitType::kDegrees);
    default:
        NOTREACHED();
    }
}
...
InterpolableFilter* InterpolableFilter::MaybeCreate(
    const FilterOperation& filter, const CSSProperty& property, double zoom, mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider)
{
    ...
    case FilterOperation::OperationType::kSaturate:
    case FilterOperation::OperationType::kSepia:
        value = MakeGarbageCollected<InterpolableNumber>(To<BasicColorMatrixFilterOperation>(filter).Amount(), CSSPrimitiveValue::UnitType::kNumber);
        break;

    case FilterOperation::OperationType::kHueRotate:
        value = MakeGarbageCollected<InterpolableNumber>(To<BasicColorMatrixFilterOperation>(filter).Amount(), CSSPrimitiveValue::UnitType::kDegrees);
        break;

    case FilterOperation::OperationType::kBrightness:
        value = MakeGarbageCollected<InterpolableNumber>(To<BasicComponentTransferFilterOperation>(filter).Amount());
        break;

    case FilterOperation::OperationType::kContrast:
    case FilterOperation::OperationType::kInvert:
    case FilterOperation::OperationType::kOpacity:
        value = MakeGarbageCollected<InterpolableNumber>(To<BasicComponentTransferFilterOperation>(filter).Amount(), CSSPrimitiveValue::UnitType::kNumber);
        break;

    case FilterOperation::OperationType::kBlur:
        value = InterpolableLength::MaybeConvertLength(To<BlurFilterOperation>(filter).StdDeviation(), property, zoom,
            /*interpolate_size=*/std::nullopt);
        break;

    case FilterOperation::OperationType::kDropShadow:
        value = InterpolableShadow::Create(To<DropShadowFilterOperation>(filter).Shadow(), zoom, color_scheme, color_provider);
        break;
...
InterpolableFilter* InterpolableFilter::MaybeConvertCSSValue(
    const CSSValue& css_value, mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider)
{
    ...
    case FilterOperation::OperationType::kSepia:
    case FilterOperation::OperationType::kHueRotate:
        value = filter.length() > 0 ? ConvertFilterOperationToInterpolableNumber(filter.FunctionType(), filter.Item(0))
                                    : CreateDefaultValue(filter.FunctionType());
        break;

    case FilterOperation::OperationType::kBlur:
        value = filter.length() > 0 ? InterpolableLength::MaybeConvertCSSValue(filter.Item(0)) : InterpolableLength::CreateNeutral();
        break;

    case FilterOperation::OperationType::kDropShadow:
        value = InterpolableShadow::MaybeConvertCSSValue(filter.Item(0), color_scheme, color_provider);
        break;
...
// static
InterpolableFilter* InterpolableFilter::CreateInitialValue(FilterOperation::OperationType type)
{
    // See https://www.w3.org/TR/filter-effects-1/#filter-functions for the
    ...
    case FilterOperation::OperationType::kHueRotate:
        value = MakeGarbageCollected<InterpolableNumber>(0, CSSPrimitiveValue::UnitType::kDegrees);
        break;
...
FilterOperation* InterpolableFilter::CreateFilterOperation(const StyleResolverState& state) const
{
    switch (type_) {
    case FilterOperation::OperationType::kGrayscale:
    case FilterOperation::OperationType::kHueRotate:
    case FilterOperation::OperationType::kSaturate:
    case FilterOperation::OperationType::kSepia: {
        double value = ClampParameter(To<InterpolableNumber>(*value_).Value(state.CssToLengthConversionData()), type_);
        return MakeGarbageCollected<BasicColorMatrixFilterOperation>(value, type_);
    }
    ...
    case FilterOperation::OperationType::kOpacity: {
        double value = ClampParameter(To<InterpolableNumber>(*value_).Value(state.CssToLengthConversionData()), type_);
        return MakeGarbageCollected<BasicComponentTransferFilterOperation>(value, type_);
    }

    case FilterOperation::OperationType::kBlur: {
        Length std_deviation = To<InterpolableLength>(*value_).CreateLength(state.CssToLengthConversionData(), Length::ValueRange::kNonNegative);
        return MakeGarbageCollected<BlurFilterOperation>(std_deviation);
    }

    case FilterOperation::OperationType::kDropShadow: {
        ShadowData shadow_data = To<InterpolableShadow>(*value_).CreateShadowData(state);
...
void InterpolableFilter::AssertCanInterpolateWith(const InterpolableValue& other) const
{
...
void InterpolableFilter::Interpolate(const InterpolableValue& to, const double progress, InterpolableValue& result) const
{
