...
InterpolableColor* MaybeConvertColor(const CSSValue* value, mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider)
{
    if (value) {
        return CSSColorInterpolationType::MaybeCreateInterpolableColor(*value, color_scheme, color_provider);
    }
    return CSSColorInterpolationType::CreateInterpolableColor(StyleColor::CurrentColor(), color_scheme, color_provider);
}
} // namespace

InterpolableShadow::InterpolableShadow(
    InterpolableLength* x, InterpolableLength* y, InterpolableLength* blur, InterpolableLength* spread, InterpolableColor* color, ShadowStyle shadow_style)
    : x_(x)
    , y_(y)
    , blur_(blur)
    , spread_(spread)
...
InterpolableShadow* InterpolableShadow::Create(
    const ShadowData& shadow_data, double zoom, mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider)
{
    return MakeGarbageCollected<InterpolableShadow>(InterpolableLength::CreatePixels(shadow_data.X() / zoom),
        InterpolableLength::CreatePixels(shadow_data.Y() / zoom), InterpolableLength::CreatePixels(shadow_data.Blur() / zoom),
        InterpolableLength::CreatePixels(shadow_data.Spread() / zoom),
        CSSColorInterpolationType::CreateInterpolableColor(shadow_data.GetColor(), color_scheme, color_provider), shadow_data.Style());
}
    ...
    const CSSValue& value, mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider)
{
    ...
    InterpolableColor* color = MaybeConvertColor(shadow->color, color_scheme, color_provider);
    ...
    return MakeGarbageCollected<InterpolableShadow>(x, y, blur, spread, color, shadow_style);
}

// static
PairwiseInterpolationValue InterpolableShadow::MaybeMergeSingles(InterpolableValue* start, InterpolableValue* end)
{
    ...
    // necessary.
    InterpolableColor::SetupColorInterpolationSpaces(*start_shadow->color_, *end_shadow->color_);

    return PairwiseInterpolationValue(start, end);
}

//  static
bool InterpolableShadow::CompatibleForCompositing(const InterpolableValue* from, const InterpolableValue* to)
{
    return To<InterpolableShadow>(from)->shadow_style_ == To<InterpolableShadow>(to)->shadow_style_;
}

// static
void InterpolableShadow::Composite(
...
{
    InterpolableShadow& underlying_shadow = To<InterpolableShadow>(underlying_value.MutableInterpolableValue());
    const InterpolableShadow& interpolable_shadow = To<InterpolableShadow>(interpolable_value);
...
ShadowData InterpolableShadow::CreateShadowData(const StyleResolverState& state) const
{
    const CSSToLengthConversionData& conversion_data = state.CssToLengthConversionData();
    ...
    Length shadow_blur = blur_->CreateLength(conversion_data, Length::ValueRange::kNonNegative);
    Length shadow_spread = spread_->CreateLength(conversion_data, Length::ValueRange::kAll);
    DCHECK(shadow_x.IsFixed());
    DCHECK(shadow_y.IsFixed());
    DCHECK(shadow_blur.IsFixed());
    DCHECK(shadow_spread.IsFixed());
    return ShadowData(gfx::Vector2dF(shadow_x.Value(), shadow_y.Value()), shadow_blur.Value(), shadow_spread.Value(), shadow_style_,
        StyleColor(CSSColorInterpolationType::ResolveInterpolableColor(*color_, state)));
}

InterpolableShadow* InterpolableShadow::RawClone() const
{
    return MakeGarbageCollected<InterpolableShadow>(x_->Clone(), y_->Clone(), blur_->Clone(), spread_->Clone(), color_->Clone(), shadow_style_);
}

InterpolableShadow* InterpolableShadow::RawCloneAndZero() const
{
    return MakeGarbageCollected<InterpolableShadow>(
        x_->CloneAndZero(), y_->CloneAndZero(), blur_->CloneAndZero(), spread_->CloneAndZero(), color_->CloneAndZero(), shadow_style_);
}
...
void InterpolableShadow::AssertCanInterpolateWith(const InterpolableValue& other) const
{
    const InterpolableShadow& other_shadow = To<InterpolableShadow>(other);
...
void InterpolableShadow::Interpolate(const InterpolableValue& to, const double progress, InterpolableValue& result) const
{
