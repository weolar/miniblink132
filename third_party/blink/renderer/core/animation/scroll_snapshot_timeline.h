...
// See ScrollTimeline.
class CORE_EXPORT ScrollSnapshotTimeline : public AnimationTimeline, public ScrollSnapshotClient {
public:
    ...
    bool IsScrollSnapshotTimeline() const override
    {
        return true;
    }
    ...
    float GetResolvedZoom() const
    {
        return timeline_state_snapshotted_.zoom;
    }
    ...
    std::optional<AnimationTimeDelta> GetDuration() const final
    {
        ...
        return timeline_state_snapshotted_.duration.has_value() ? timeline_state_snapshotted_.duration
...
protected:
    ...
    AnimationTimeDelta CalculateIntrinsicIterationDuration(
        const TimelineRange&, const std::optional<TimelineOffset>& range_start, const std::optional<TimelineOffset>& range_end, const Timing&) override;
    ...
    public:
        ...
        {
            return scroll_offsets == other.scroll_offsets && zoom == other.zoom && view_offsets == other.view_offsets;
        }

        bool operator==(const TimelineState& other) const
        {
            return phase == other.phase && current_time == other.current_time && scroll_offsets == other.scroll_offsets && zoom == other.zoom
                && view_offsets == other.view_offsets && resolved_source == other.resolved_source;
        }
...
template <> struct DowncastTraits<ScrollSnapshotTimeline> {
