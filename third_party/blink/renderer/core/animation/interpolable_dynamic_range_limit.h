...
class CORE_EXPORT InterpolableDynamicRangeLimit final : public InterpolableValue {
public:
    explicit InterpolableDynamicRangeLimit(DynamicRangeLimit mix_value);

    static InterpolableDynamicRangeLimit* Create(DynamicRangeLimit dynamic_range_limit);
    ...
    bool IsDynamicRangeLimit() const final
    {
        return true;
    }
    ...
    {
        dynamic_range_limit_ = To<InterpolableDynamicRangeLimit>(other).dynamic_range_limit_;
    }
...
template <> struct DowncastTraits<InterpolableDynamicRangeLimit> {
