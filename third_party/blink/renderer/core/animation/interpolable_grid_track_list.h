...
public:
    ...
    static InterpolableGridTrackList* MaybeCreate(const NGGridTrackList& track_list, const CSSProperty& property, float zoom);

    NGGridTrackList CreateNGGridTrackList(const CSSToLengthConversionData& conversion_data) const;
    ...
    bool IsGridTrackList() const final
    {
        return true;
    }
...
template <> struct DowncastTraits<InterpolableGridTrackList> {
