...
InterpolationValue SVGNumberOptionalNumberInterpolationType::MaybeConvertSVGValue(const SVGPropertyBase& svg_value) const
{
    ...
    auto* result = MakeGarbageCollected<InterpolableList>(2);
    result->Set(0, MakeGarbageCollected<InterpolableNumber>(number_optional_number.FirstNumber()->Value()));
    result->Set(1, MakeGarbageCollected<InterpolableNumber>(number_optional_number.SecondNumber()->Value()));
    return InterpolationValue(result);
}

SVGPropertyBase* SVGNumberOptionalNumberInterpolationType::AppliedSVGValue(const InterpolableValue& interpolable_value, const NonInterpolableValue*) const
{
    ...
    return MakeGarbageCollected<SVGNumberOptionalNumber>(MakeGarbageCollected<SVGNumber>(To<InterpolableNumber>(list.Get(0))->Value(length_resolver)),
        MakeGarbageCollected<SVGNumber>(To<InterpolableNumber>(list.Get(1))->Value(length_resolver)));
}
