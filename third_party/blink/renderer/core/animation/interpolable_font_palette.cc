...
InterpolableFontPalette::InterpolableFontPalette(scoped_refptr<const FontPalette> font_palette)
...
// static
InterpolableFontPalette* InterpolableFontPalette::Create(scoped_refptr<const FontPalette> font_palette)
{
    return MakeGarbageCollected<InterpolableFontPalette>(font_palette);
}

scoped_refptr<const FontPalette> InterpolableFontPalette::GetFontPalette() const
{
...
{
    const InterpolableFontPalette& other_palette = To<InterpolableFontPalette>(other);
    return *font_palette_ == *other_palette.font_palette_;
}

void InterpolableFontPalette::AssertCanInterpolateWith(const InterpolableValue& other) const
{
    DCHECK(other.IsFontPalette());
}

void InterpolableFontPalette::Interpolate(const InterpolableValue& to, const double progress, InterpolableValue& result) const
{
    ...
    if (normalized_progress == 0 || *font_palette_.get() == *to_palette.font_palette_.get()) {
        ...
        FontPalette::NonNormalizedPercentages percentages = FontPalette::ComputeEndpointPercentagesFromNormalized(normalized_progress);
        ...
        result_palette.font_palette_ = FontPalette::Mix(
            font_palette_, to_palette.font_palette_, percentages.start, percentages.end, normalized_progress, 1.0, Color::ColorSpace::kOklab, std::nullopt);
    }
}
