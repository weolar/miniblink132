...
{
    return TimingCalculations::LessThanOrEqualToWithinEpsilon(offsets_.end - offsets_.start, 0.0);
}

double TimelineRange::ToFractionalOffset(const TimelineOffset& timeline_offset) const
{
    if (IsEmpty()) {
    ...
    double offset = range.start + MinimumValueForLength(timeline_offset.offset, LayoutUnit(range.end - range.start));
    return (offset - offsets_.start) / full_range_size;
}

TimelineRange::ScrollOffsets TimelineRange::ConvertNamedRange(NamedRange named_range) const
{
    // https://drafts.csswg.org/scroll-animations-1/#view-timelines-ranges
    double align_subject_start_view_end = offsets_.start;
    double align_subject_end_view_start = offsets_.end;
    double align_subject_start_view_start = align_subject_end_view_start - view_offsets_.exit_crossing_distance;
    double align_subject_end_view_end = align_subject_start_view_end + view_offsets_.entry_crossing_distance;
    ...
    case TimelineOffset::NamedRange::kContain:
        ...
        //   2. the end border edge of the element’s principal box coincides with
        //      the end edge of its view progress visibility range.
        return { std::min(align_subject_start_view_start, align_subject_end_view_end), std::max(align_subject_start_view_start, align_subject_end_view_end) };
    ...
    case TimelineOffset::NamedRange::kEntry:
        ...
        return { align_subject_start_view_end, std::min(align_subject_start_view_start, align_subject_end_view_end) };
    ...
    case TimelineOffset::NamedRange::kExit:
        ...
        return { std::max(align_subject_start_view_start, align_subject_end_view_end), align_subject_end_view_start };
