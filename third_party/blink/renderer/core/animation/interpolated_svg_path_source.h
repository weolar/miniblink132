...
public:
    InterpolatedSVGPathSource(const InterpolableList& list_value, const Vector<SVGPathSegType>& path_seg_types)
        : current_index_(0)
    ...
    InterpolatedSVGPathSource& operator=(const InterpolatedSVGPathSource&) = delete;
...
PathSegmentData InterpolatedSVGPathSource::ParseSegment()
{
    PathSegmentData segment = SVGPathSegInterpolationFunctions::ConsumeInterpolablePathSeg(
        *interpolable_path_segs_.Get(current_index_), path_seg_types_.at(current_index_), current_coordinates_);
    current_index_++;
    return segment;
}
