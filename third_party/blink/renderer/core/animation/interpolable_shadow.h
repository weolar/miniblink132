...
public:
    InterpolableShadow(
        InterpolableLength* x, InterpolableLength* y, InterpolableLength* blur, InterpolableLength* spread, InterpolableColor* color, ShadowStyle);

    static InterpolableShadow* Create(const ShadowData&, double zoom, mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider);
    static InterpolableShadow* CreateNeutral();

    static InterpolableShadow* MaybeConvertCSSValue(const CSSValue&, mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider);
    ...
    static bool CompatibleForCompositing(const InterpolableValue*, const InterpolableValue*);
    static void Composite(UnderlyingValue&, double underlying_fraction, const InterpolableValue&, const NonInterpolableValue*);
    ShadowStyle GetShadowStyle() const
    {
        return shadow_style_;
    }
...
template <> struct DowncastTraits<InterpolableShadow> {
