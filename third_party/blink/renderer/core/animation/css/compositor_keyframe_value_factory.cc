...
static CompositorKeyframeValue* CreateFromTransformProperties(TransformOperation* transform, double zoom, TransformOperation* initial_transform)
{
    ...
    return MakeGarbageCollected<CompositorKeyframeTransform>(operation, transform ? zoom : 1);
}

CompositorKeyframeValue* CompositorKeyframeValueFactory::Create(const PropertyHandle& property, const ComputedStyle& style, double offset)
{
    ...
    case CSSPropertyID::kFilter:
        return MakeGarbageCollected<CompositorKeyframeFilterOperations>(style.Filter());
    case CSSPropertyID::kBackdropFilter:
        return MakeGarbageCollected<CompositorKeyframeFilterOperations>(style.BackdropFilter());
    case CSSPropertyID::kTransform:
        return MakeGarbageCollected<CompositorKeyframeTransform>(style.Transform(), style.EffectiveZoom());
    case CSSPropertyID::kTranslate: {
        return CreateFromTransformProperties(style.Translate(), style.EffectiveZoom(), nullptr);
    }
    case CSSPropertyID::kRotate: {
        return CreateFromTransformProperties(style.Rotate(), style.EffectiveZoom(), nullptr);
    }
    case CSSPropertyID::kScale: {
        return CreateFromTransformProperties(style.Scale(), style.EffectiveZoom(), nullptr);
    }
    case CSSPropertyID::kVariable: {
        ...
        if (primitive_value && primitive_value->IsNumber()) {
            return MakeGarbageCollected<CompositorKeyframeDouble>(primitive_value->GetFloatValue());
        }
        ...
        if (const auto* color_value = DynamicTo<cssvalue::CSSColor>(value)) {
            ...
            return MakeGarbageCollected<CompositorKeyframeColor>(SkColorSetARGB(color.AlphaAsInteger(), color.Red(), color.Green(), color.Blue()));
        }
