...
public:
    explicit CssKeyframeEffectModel(const KeyframeVector& keyframes, CompositeOperation composite = kCompositeReplace,
        scoped_refptr<TimingFunction> default_keyframe_easing = nullptr, bool has_named_range_keyframes = false)
        : StringKeyframeEffectModel(keyframes, composite, std::move(default_keyframe_easing), has_named_range_keyframes)
    {
    }
    ...
    // physical properties.
    KeyframeEffectModelBase::KeyframeVector GetComputedKeyframes(Element* element) override;

    bool IsCssKeyframeEffectModel() override
    {
        return true;
    }
};
