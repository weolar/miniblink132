...
void ResolveUnderlyingPropertyValues(Element& element, const PropertyHandleSet& properties, MissingPropertyValueMap& map)
{
    ...
    AnimationUtils::ForEachInterpolatedPropertyValue(&element, properties, empty_interpolations_map, [&map](PropertyHandle property, const CSSValue* value) {
...
void AddMissingProperties(const MissingPropertyValueMap& property_map, const PropertyHandleSet& all_properties, const PropertyHandleSet& keyframe_properties,
    StringKeyframe* keyframe)
{
    for (const auto& property : all_properties) {
        ...
        if (property_map.Contains(property_name)) {
            ...
            keyframe->SetCSSPropertyValue(property.GetCSSProperty().PropertyID(), value, SecureContextMode::kInsecureContext, nullptr);
        }
    }
}
...
void ResolveComputedValues(Element* element, StringKeyframe* keyframe)
{
    ...
    for (const auto& property : keyframe->Properties()) {
        if (property.IsCSSCustomProperty()) {
            ...
            const CSSValue* computed_value = StyleResolver::ComputeValue(element, property_name, value);
            if (computed_value)
...
StringKeyframe* CreateKeyframe(double offset, scoped_refptr<TimingFunction> easing)
{
...
StringKeyframe* FindOrInsertKeyframe(KeyframeEffectModelBase::KeyframeVector& keyframes, double target_offset, scoped_refptr<TimingFunction> default_easing)
{
    for (wtf_size_t i = 0; i < keyframes.size(); i++) {
        ...
        if (offset == target_offset && (keyframe->Easing() == *default_easing)
            && (!keyframe->Composite() || keyframe->Composite() == EffectModel::kCompositeReplace)) {
            return keyframe;
        }
        if (offset > target_offset) {
            StringKeyframe* missing_keyframe = CreateKeyframe(target_offset, default_easing);
...
KeyframeEffectModelBase::KeyframeVector CssKeyframeEffectModel::GetComputedKeyframes(Element* element)
{
    ...
    if (from_properties.size() < all_properties.size() || to_properties.size() < all_properties.size()) {
        ResolveUnderlyingPropertyValues(*element, all_properties, missing_property_value_map);
    }
    ...
    if (from_properties.size() < all_properties.size() && !computed_keyframes.empty()) {
        StringKeyframe* keyframe = FindOrInsertKeyframe(computed_keyframes, 0, default_keyframe_easing_);
        AddMissingProperties(missing_property_value_map, all_properties, from_properties, keyframe);
    }
    if (to_properties.size() < all_properties.size() && !computed_keyframes.empty()) {
        StringKeyframe* keyframe = FindOrInsertKeyframe(computed_keyframes, 1, default_keyframe_easing_);
        AddMissingProperties(missing_property_value_map, all_properties, to_properties, keyframe);
    }
