...
public:
    CSSAnimation(ExecutionContext*, AnimationTimeline*, AnimationEffect*, wtf_size_t animation_index, const String& animation_name);

    bool IsCSSAnimation() const final
    {
        return true;
    }

    void ClearOwningElement() final
    {
        owning_element_ = nullptr;
    }
    Element* OwningElement() const override
    {
        return owning_element_.Get();
    }
    ...
    wtf_size_t AnimationIndex() const
    {
    ...
    void setStartTime(const V8CSSNumberish* start_time, ExceptionState& exception_state) override;
    void setRangeStart(const RangeBoundary* range_start, ExceptionState& exception_state) override;
    void setRangeEnd(const RangeBoundary* range_end, ExceptionState& exception_state) override;
    ...
    void SetRange(const std::optional<TimelineOffset>& range_start, const std::optional<TimelineOffset>& range_end) override;
    ...
    bool GetIgnoreCSSPlayState()
    {
        return ignore_css_play_state_;
    }
    void ResetIgnoreCSSPlayState()
    {
        ignore_css_play_state_ = false;
    }
    bool GetIgnoreCSSTimeline() const
    {
        return ignore_css_timeline_;
    }
    void ResetIgnoreCSSTimeline()
    {
        ignore_css_timeline_ = false;
    }
    bool GetIgnoreCSSRangeStart()
    {
        return ignore_css_range_start_;
    }
    void ResetIgnoreCSSRangeStart()
    {
        ignore_css_range_start_ = false;
    }
    bool GetIgnoreCSSRangeEnd()
    {
        return ignore_css_range_end_;
    }
    void ResetIgnoreCSSRangeEnd()
    {
        ignore_css_range_end_ = false;
    }
    ...
    void FlushPendingUpdates() const override
    {
        FlushStyles();
    }

protected:
    AnimationEffect::EventDelegate* CreateEventDelegate(Element* target, const AnimationEffect::EventDelegate* old_event_delegate) override;
...
template <> struct DowncastTraits<CSSAnimation> {
