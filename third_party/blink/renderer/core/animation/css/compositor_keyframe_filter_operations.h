...
class CompositorKeyframeFilterOperations final : public CompositorKeyframeValue {
public:
    CompositorKeyframeFilterOperations(const FilterOperations& operations)
        : operation_wrapper_(MakeGarbageCollected<FilterOperationsWrapper>(operations))
    {
    }
...
private:
    ...
    {
        return Type::kFilterOperations;
    }

    Member<FilterOperationsWrapper> operation_wrapper_;
};

template <> struct DowncastTraits<CompositorKeyframeFilterOperations> {
