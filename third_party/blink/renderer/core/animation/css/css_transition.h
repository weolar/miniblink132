...
public:
    CSSTransition(ExecutionContext*, AnimationTimeline*, AnimationEffect*, uint64_t transition_generation, const PropertyHandle& transition_property);

    bool IsCSSTransition() const final
    {
        return true;
    }
...
template <> struct DowncastTraits<CSSTransition> {
