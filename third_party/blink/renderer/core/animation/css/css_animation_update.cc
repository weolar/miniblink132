...
void CSSAnimationUpdate::Copy(const CSSAnimationUpdate& update)
{
    ...
    active_interpolations_for_animations_ = update.ActiveInterpolationsForAnimations();
    active_interpolations_for_transitions_ = update.ActiveInterpolationsForTransitions();
    cancelled_animation_indices_ = update.CancelledAnimationIndices();
    animation_indices_with_pause_toggled_ = update.AnimationIndicesWithPauseToggled();
...
void CSSAnimationUpdate::StartTransition(const PropertyHandle& property, const ComputedStyle* from, const ComputedStyle* to,
    const ComputedStyle* reversing_adjusted_start_value, double reversing_shortening_factor, const InertEffect& effect)
{
    NewTransition* new_transition
        = MakeGarbageCollected<NewTransition>(property, from, to, reversing_adjusted_start_value, reversing_shortening_factor, &effect);
