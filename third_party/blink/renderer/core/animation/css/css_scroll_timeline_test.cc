...
class CSSScrollTimelineTest : public PageTestBase, private ScopedScrollTimelineForTest {
public:
    CSSScrollTimelineTest()
        : ScopedScrollTimelineForTest(true)
    {
    }
...
TEST_F(CSSScrollTimelineTest, SharedTimelines)
{
    ...
    EXPECT_EQ(animations1[0]->TimelineInternal(), animations1[1]->TimelineInternal());
    EXPECT_EQ(animations1[1]->TimelineInternal(), animations2[0]->TimelineInternal());
    EXPECT_EQ(animations2[0]->TimelineInternal(), animations2[1]->TimelineInternal());

    // The animation associated with anim3 uses a different timeline
    // from anim1/2.
    EXPECT_EQ(animations1[2]->TimelineInternal(), animations2[2]->TimelineInternal());

    EXPECT_NE(animations2[2]->TimelineInternal(), animations1[0]->TimelineInternal());
    EXPECT_NE(animations2[2]->TimelineInternal(), animations1[1]->TimelineInternal());
}
...
TEST_F(CSSScrollTimelineTest, MultipleLifecyclePasses)
{
    ...
    EXPECT_EQ(Color::FromRGB(0, 128, 0), element->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
}
...
public:
    ...
    {
        scroller_element_->SetInlineStyleProperty(CSSPropertyID::kScrollTimeline, "--timeline");
    }
...
TEST_F(CSSScrollTimelineTest, ResizeObserverTriggeredTimelines)
{
    ...
    ResizeObserver* observer = ResizeObserver::Create(GetDocument().domWindow(), delegate);
...
std::optional<ScrollTimeline::ScrollAxis> GetTimelineAxis(const Animation& animation)
{
    if (auto* scroll_timeline = DynamicTo<ScrollTimeline>(animation.TimelineInternal())) {
...
TEST_F(CSSScrollTimelineTest, ViewTimelineHost)
{
    ...
    ASSERT_EQ(ScrollTimeline::ScrollAxis::kX, GetTimelineAxis(*animations[0])) << "Outer animation can not see view timeline defined by :host";
}
...
TEST_F(CSSScrollTimelineTest, ViewTimelineSlotted)
{
    ...
    HeapVector<Member<Animation>> animations = target->getAnimations();
    ASSERT_EQ(1u, animations.size());
    ASSERT_EQ(ScrollTimeline::ScrollAxis::kX, GetTimelineAxis(*animations[0])) << "Outer animation can not see view timeline defined by ::slotted";
}
...
TEST_F(CSSScrollTimelineTest, ViewTimelinePart)
{
    ...
    Element* target = host->GetShadowRoot()->QuerySelector(AtomicString(".target"));
    ASSERT_TRUE(target);
    HeapVector<Member<Animation>> animations = target->getAnimations();
    ASSERT_EQ(1u, animations.size());
    ASSERT_EQ(ScrollTimeline::ScrollAxis::kX, GetTimelineAxis(*animations[0])) << "Inner animation can see view timeline defined by ::part";
}
...
TEST_F(CSSScrollTimelineTest, ScrollTimelineHost)
{
    ...
    HeapVector<Member<Animation>> animations = target->getAnimations();
    ASSERT_EQ(1u, animations.size());
    ASSERT_EQ(ScrollTimeline::ScrollAxis::kX, GetTimelineAxis(*animations[0])) << "Outer animation can not see scroll timeline defined by :host";
}
...
TEST_F(CSSScrollTimelineTest, ScrollTimelineSlotted)
{
    ...
    HeapVector<Member<Animation>> animations = target->getAnimations();
    ASSERT_EQ(1u, animations.size());
    ASSERT_EQ(ScrollTimeline::ScrollAxis::kX, GetTimelineAxis(*animations[0])) << "Outer animation can not see scroll timeline defined by ::slotted";
}
...
TEST_F(CSSScrollTimelineTest, ScrollTimelinePart)
{
    ...
    HeapVector<Member<Animation>> animations = target->getAnimations();
    ASSERT_EQ(1u, animations.size());
    ASSERT_EQ(ScrollTimeline::ScrollAxis::kX, GetTimelineAxis(*animations[0])) << "Inner animation can see scroll timeline defined by ::part";
}
