...
public:
    NewCSSAnimation(AtomicString name, size_t name_index, wtf_size_t position_index, const InertEffect& effect, Timing timing, StyleRuleKeyframes* style_rule,
        AnimationTimeline* timeline, const Vector<EAnimPlayState>& play_state_list, const std::optional<TimelineOffset>& range_start,
...
public:
    UpdatedCSSAnimation(wtf_size_t index, Animation* animation, const InertEffect& effect, Timing specified_timing, StyleRuleKeyframes* style_rule,
        AnimationTimeline* timeline, const Vector<EAnimPlayState>& play_state_list, const std::optional<TimelineOffset>& range_start,
        const std::optional<TimelineOffset>& range_end)
...
public:
    ...
    void StartAnimation(const AtomicString& animation_name, size_t name_index, wtf_size_t position_index, const InertEffect& effect, const Timing& timing,
        StyleRuleKeyframes* style_rule, AnimationTimeline* timeline, const Vector<EAnimPlayState>& play_state_list,
        const std::optional<TimelineOffset>& range_start, const std::optional<TimelineOffset>& range_end)
    {
        new_animations_.push_back(
            NewCSSAnimation(animation_name, name_index, position_index, effect, timing, style_rule, timeline, play_state_list, range_start, range_end));
    }
    ...
    void UpdateAnimation(wtf_size_t index, Animation* animation, const InertEffect& effect, const Timing& specified_timing, StyleRuleKeyframes* style_rule,
        AnimationTimeline* timeline, const Vector<EAnimPlayState>& play_state_list, const std::optional<TimelineOffset>& range_start,
        const std::optional<TimelineOffset>& range_end)
    {
        animations_with_updates_.push_back(
            UpdatedCSSAnimation(index, animation, effect, specified_timing, style_rule, timeline, play_state_list, range_start, range_end));
        suppressed_animations_.insert(animation);
    }
    ...
    void StartTransition(const PropertyHandle&, const ComputedStyle* from, const ComputedStyle* to, const ComputedStyle* reversing_adjusted_start_value,
        double reversing_shortening_factor, const InertEffect&);
    ...
    public:
        NewTransition(const PropertyHandle& property, const ComputedStyle* from, const ComputedStyle* to, const ComputedStyle* reversing_adjusted_start_value,
            double reversing_shortening_factor, const InertEffect* effect)
            : property(property)
            , from(from)
            , to(to)
    ...
    const NewTransitionMap& NewTransitions() const
    {
        return new_transitions_;
    }
    ...
    void AdoptActiveInterpolationsForAnimations(ActiveInterpolationsMap& new_map)
    {
        new_map.swap(active_interpolations_for_animations_);
    }
    void AdoptActiveInterpolationsForTransitions(ActiveInterpolationsMap& new_map)
    {
    ...
    bool IsEmpty() const
    {
        return !HasUpdates() && !HasActiveInterpolations();
    }

    bool HasUpdates() const
    {
        return !new_animations_.empty() || !cancelled_animation_indices_.empty() || !suppressed_animations_.empty()
            || !animation_indices_with_pause_toggled_.empty() || !animations_with_updates_.empty() || !new_transitions_.empty()
            || !cancelled_transitions_.empty() || !finished_transitions_.empty() || !updated_compositor_keyframes_.empty() || !changed_scroll_timelines_.empty()
            || !changed_view_timelines_.empty() || !changed_deferred_timelines_.empty() || !changed_timeline_attachments_.empty();
    }
...
private:
    ...
    {
        return !active_interpolations_for_animations_.empty() || !active_interpolations_for_transitions_.empty();
    }
