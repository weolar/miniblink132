...
class CORE_EXPORT InterpolableGridTrackRepeater final : public InterpolableValue {
public:
    InterpolableGridTrackRepeater(InterpolableList* values, const NGGridTrackRepeater& repeater);
    ...
    Vector<GridTrackSize, 1> CreateTrackSizes(const CSSToLengthConversionData& conversion_data) const;

    wtf_size_t RepeatSize() const
    {
        return repeater_.repeat_size;
    }
    wtf_size_t RepeatCount() const
    {
        return repeater_.repeat_count;
    }
    ...
    bool IsGridTrackRepeater() const final
    {
        return true;
    }
...
template <> struct DowncastTraits<InterpolableGridTrackRepeater> {
