...
Length::ValueRange LengthPropertyFunctions::GetValueRange(const CSSProperty& property)
{
...
bool LengthPropertyFunctions::CanAnimateKeyword(const CSSProperty& property, CSSValueID value_id)
{
...
bool LengthPropertyFunctions::GetPixelsForKeyword(const CSSProperty& property, CSSValueID value_id, double& result)
{
    switch (property.PropertyID()) {
...
bool LengthPropertyFunctions::GetInitialLength(const CSSProperty& property, const ComputedStyle& initial_style, Length& result)
{
    switch (property.PropertyID()) {
    ...
    case CSSPropertyID::kOutlineWidth:
        ...
        return true;
    case CSSPropertyID::kColumnRuleWidth:
        result = Length::Fixed(ComputedStyleInitialValues::InitialColumnRuleWidth());
        return true;
...
bool LengthPropertyFunctions::GetLength(const CSSProperty& property, const ComputedStyle& style, Length& result_param)
{
    bool success = false;
    Length result;
    switch (property.PropertyID()) {
    ...
    if (success) {
        CSSValueID id = InterpolableLength::LengthTypeToCSSValueID(result.GetType());
...
bool LengthPropertyFunctions::SetLength(const CSSProperty& property, ComputedStyleBuilder& builder, const Length& value)
{
    switch (property.PropertyID()) {
