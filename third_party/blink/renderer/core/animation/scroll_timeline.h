...
public:
    ...
    static constexpr double kScrollTimelineMicrosecondsPerPixel = cc::ScrollTimeline::kScrollTimelineMicrosecondsPerPixel;

    static ScrollTimeline* Create(Document&, ScrollTimelineOptions*, ExceptionState&);

    static ScrollTimeline* Create(Document* document, Element* source, ScrollAxis axis);
    ...
    ScrollTimeline(Document*, ReferenceType reference_type, Element* reference, ScrollAxis axis);
    ...
    const V8ScrollAxis axis() const
    {
        return V8ScrollAxis(GetAxis());
    }
...
protected:
    ...
    virtual void CalculateOffsets(PaintLayerScrollableArea* scrollable_area, ScrollOrientation physical_orientation, TimelineState* state) const;
    ...
    Element* GetReferenceElement() const
    {
...
template <> struct DowncastTraits<ScrollTimeline> {
