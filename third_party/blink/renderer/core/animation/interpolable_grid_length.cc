...
InterpolableGridLength::InterpolableGridLengthType GetInterpolableGridLengthType(const Length& length)
{
...
Length CreateContentSizedLength(const InterpolableGridLength::InterpolableGridLengthType& type)
{
    switch (type) {
...
InterpolableGridLength::InterpolableGridLength(InterpolableValue* value, InterpolableGridLengthType type)
    : value_(value)
    , type_(type)
{
...
// static
InterpolableGridLength* InterpolableGridLength::Create(const Length& length, const CSSProperty& property, float zoom)
{
    ...
    if (length.IsFlex()) {
        ...
        value = InterpolableLength::MaybeConvertLength(length, property, zoom,
            /*interpolate_size=*/std::nullopt);
    }
    return MakeGarbageCollected<InterpolableGridLength>(std::move(value), type);
}

Length InterpolableGridLength::CreateGridLength(const CSSToLengthConversionData& conversion_data) const
{
    if (IsContentSized()) {
    ...
    return To<InterpolableLength>(*value_).CreateLength(conversion_data, Length::ValueRange::kNonNegative);
}
...
bool InterpolableGridLength::IsCompatibleWith(const InterpolableGridLength& other) const
{
...
{
    return MakeGarbageCollected<InterpolableGridLength>(value_ ? value_->Clone() : nullptr, type_);
}

InterpolableGridLength* InterpolableGridLength::RawCloneAndZero() const
{
    return MakeGarbageCollected<InterpolableGridLength>(value_ ? value_->CloneAndZero() : nullptr, type_);
}
...
{
    const InterpolableGridLength& other_interpolable_grid_length = To<InterpolableGridLength>(other);
    ...
    if (IsCompatibleWith(other_interpolable_grid_length)) {
        ...
        value_ = other_interpolable_grid_length.value_ ? other_interpolable_grid_length.value_->Clone() : nullptr;
    }
}

void InterpolableGridLength::AssertCanInterpolateWith(const InterpolableValue& other) const
{
    const InterpolableGridLength& other_interpolable_grid_length = To<InterpolableGridLength>(other);
    ...
    if ((type_ == kLength && other_interpolable_grid_length.type_ == kLength) || (type_ == kFlex && other_interpolable_grid_length.type_ == kFlex)) {
        DCHECK(value_ && other_interpolable_grid_length.value_);
...
void InterpolableGridLength::Interpolate(const InterpolableValue& to, const double progress, InterpolableValue& result) const
{
    const InterpolableGridLength& grid_length_to = To<InterpolableGridLength>(to);
    InterpolableGridLength& grid_length_result = To<InterpolableGridLength>(result);
    if (!IsCompatibleWith(grid_length_to)) {
        if (progress < 0.5) {
            ...
            grid_length_result.value_ = grid_length_to.value_ ? grid_length_to.value_->Clone() : nullptr;
        }
        return;
    }
    value_->Interpolate(*grid_length_to.value_, progress, *grid_length_result.value_);
}
