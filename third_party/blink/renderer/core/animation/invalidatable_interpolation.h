...
public:
    InvalidatableInterpolation(const PropertyHandle& property, PropertySpecificKeyframe* start_keyframe, PropertySpecificKeyframe* end_keyframe)
        : Interpolation()
        , property_(property)
    ...
    const PropertyHandle& GetProperty() const final
    {
        return property_;
    }
    ...
    bool IsInvalidatableInterpolation() const override
    {
        return true;
    }
...
private:
    ...
    TypedInterpolationValue* MaybeConvertUnderlyingValue(const InterpolationEnvironment&) const;
    const TypedInterpolationValue* EnsureValidConversion(InterpolationEnvironment&, const UnderlyingValueOwner&) const;
    ...
    bool IsConversionCacheValid(const InterpolationEnvironment&, const UnderlyingValueOwner&) const;
    bool IsNeutralKeyframeActive() const;
    PairwisePrimitiveInterpolation* MaybeConvertPairwise(const InterpolationEnvironment&, const UnderlyingValueOwner&) const;
    TypedInterpolationValue* ConvertSingleKeyframe(const PropertySpecificKeyframe&, const InterpolationEnvironment&, const UnderlyingValueOwner&) const;
...
template <> struct DowncastTraits<InvalidatableInterpolation> {
