...
public:
    ...
    TimelinePhase Phase()
    {
        return CurrentPhaseAndTime().phase;
    }
    ...
    virtual bool IsScrollSnapshotTimeline() const
    {
        return false;
    }
    ...
    virtual AnimationTimeline* ExposedTimeline()
    {
        return this;
    }
    ...
    {
        return IsDocumentTimeline();
    }
    // https://drafts.csswg.org/web-animations-2/#progress-based-timeline
    bool IsProgressBased() const
    {
        return IsScrollSnapshotTimeline();
    }
    ...
    AnimationTimeDelta CalculateIntrinsicIterationDuration(const Animation* animation, const Timing& timing)
    {
        return CalculateIntrinsicIterationDuration(animation->GetRangeStartInternal(), animation->GetRangeEndInternal(), timing);
    }
    ...
    {
        return CalculateIntrinsicIterationDuration(GetTimelineRange(), range_start, range_end, timing);
    }

    // See class TimelineRange.
    virtual TimelineRange GetTimelineRange() const
    {
    ...
    virtual Animation* Play(AnimationEffect*, ExceptionState& = ASSERT_NO_EXCEPTION);
    ...
    using ReplaceableAnimationsMap = HeapHashMap<Member<Element>, Member<HeapVector<Member<Animation>>>>;
    void getReplaceableAnimations(ReplaceableAnimationsMap* replaceable_animation_set);
...
protected:
    ...
    virtual AnimationTimeDelta CalculateIntrinsicIterationDuration(
        const TimelineRange&, const std::optional<TimelineOffset>& range_start, const std::optional<TimelineOffset>& range_end, const Timing&)
    {
