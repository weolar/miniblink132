...
public:
    explicit PropertyHandle(const CSSProperty& property, bool is_presentation_attribute = false)
        : handle_type_(is_presentation_attribute ? kHandlePresentationAttribute : kHandleCSSProperty)
    ...
    explicit PropertyHandle(const CSSPropertyName& property_name)
        ...
        , css_property_(property_name.IsCustomProperty() ? &GetCSSPropertyVariable() : &CSSProperty::Get(property_name.Id()))
    ...
    explicit PropertyHandle(const QualifiedName& attribute_name)
        : handle_type_(kHandleSVGAttribute)
        , svg_attribute_(&attribute_name)
    {
    }
    ...
    bool IsSVGAttribute() const
    {
        return handle_type_ == kHandleSVGAttribute;
    }
...
template <> struct HashTraits<blink::PropertyHandle> : SimpleClassHashTraits<blink::PropertyHandle> {
    ...
    {
        new (NotNullTag::kNotNull, &slot) blink::PropertyHandle(blink::PropertyHandle::DeletedValueForHashTraits());
    }
