...
std::optional<ScrollOffsets> ScrollSnapshotTimeline::GetResolvedScrollOffsets() const
{
    return timeline_state_snapshotted_.scroll_offsets;
}

std::optional<ScrollSnapshotTimeline::ViewOffsets> ScrollSnapshotTimeline::GetResolvedViewOffsets() const
{
...
{
    return { timeline_state_snapshotted_.phase, timeline_state_snapshotted_.current_time };
}

V8CSSNumberish* ScrollSnapshotTimeline::ConvertTimeToProgress(AnimationTimeDelta time) const
{
    return MakeGarbageCollected<V8CSSNumberish>(CSSUnitValues::percent((time / GetDuration().value()) * 100));
}
...
std::optional<base::TimeDelta> ScrollSnapshotTimeline::InitialStartTimeForAnimations()
{
    return base::TimeDelta();
}

AnimationTimeDelta ScrollSnapshotTimeline::CalculateIntrinsicIterationDuration(
    const TimelineRange& timeline_range, const std::optional<TimelineOffset>& range_start, const std::optional<TimelineOffset>& range_end, const Timing& timing)
{
    ...
    if (duration && timing.iteration_count > 0) {
        ...
        double start = range_start ? timeline_range.ToFractionalOffset(range_start.value()) : 0;
        double end = range_end ? timeline_range.ToFractionalOffset(range_end.value()) : 1;
...
TimelineRange ScrollSnapshotTimeline::GetTimelineRange() const
{
    ...
    return TimelineRange(scroll_offsets.value(), view_offsets.has_value() ? view_offsets.value() : ViewOffsets());
}
...
void ScrollSnapshotTimeline::ServiceAnimations(TimingUpdateReason reason)
{
    ...
    bool was_active = last_current_phase_and_time_ && last_current_phase_and_time_.value().phase == TimelinePhase::kActive;
...
LayoutBox* ScrollSnapshotTimeline::ComputeScrollContainer(Node* resolved_source)
{
    if (!resolved_source) {
        return nullptr;
    }
...
bool ScrollSnapshotTimeline::ValidateSnapshot()
{
    ...
    bool state_changed = !timeline_state_snapshotted_.HasConsistentLayout(new_state);
...
void ScrollSnapshotTimeline::UpdateCompositorTimeline()
{
    ...
    ToScrollTimeline(compositor_timeline_.get())
        ->UpdateScrollerIdAndScrollOffsets(scroll_timeline_util::GetCompositorScrollElementId(ResolvedSource()), GetResolvedScrollOffsets());
}
