...
CSSLengthInterpolationType::CSSLengthInterpolationType(PropertyHandle property, const PropertyRegistration* registration)
    : CSSInterpolationType(property, registration)
    , value_range_(LengthPropertyFunctions::GetValueRange(CssProperty()))
    , is_zoomed_length_(LengthPropertyFunctions::IsZoomedLength(CssProperty()))
{
}

class InheritedLengthChecker : public CSSInterpolationType::CSSConversionChecker {
public:
    InheritedLengthChecker(const CSSProperty& property, bool get_length_success, const Length& length)
        : property_(property)
...
private:
    bool IsValid(const StyleResolverState& state, const InterpolationValue& underlying) const final
    {
        ...
        bool success = LengthPropertyFunctions::GetLength(property_, *state.ParentStyle(), parent_length);
...
InterpolationValue CSSLengthInterpolationType::MaybeConvertInitial(const StyleResolverState& state, ConversionCheckers& conversion_checkers) const
{
    Length initial_length;
    if (!LengthPropertyFunctions::GetInitialLength(CssProperty(), state.GetDocument().GetStyleResolver().InitialStyle(), initial_length))
        return nullptr;
    return InterpolationValue(InterpolableLength::MaybeConvertLength(initial_length, CssProperty(), 1, state.StyleBuilder().InterpolateSize()));
}

InterpolationValue CSSLengthInterpolationType::MaybeConvertInherit(const StyleResolverState& state, ConversionCheckers& conversion_checkers) const
{
    if (!state.ParentStyle())
        return nullptr;
    Length inherited_length;
    bool success = LengthPropertyFunctions::GetLength(CssProperty(), *state.ParentStyle(), inherited_length);
    conversion_checkers.push_back(MakeGarbageCollected<InheritedLengthChecker>(CssProperty(), success, inherited_length));
    if (!success) {
    ...
    return InterpolationValue(InterpolableLength::MaybeConvertLength(
        inherited_length, CssProperty(), EffectiveZoom(state.ParentStyle()->EffectiveZoom()), state.StyleBuilder().InterpolateSize()));
}
    ...
    if (auto* identifier_value = DynamicTo<CSSIdentifierValue>(value)) {
        ...
        if (LengthPropertyFunctions::CanAnimateKeyword(CssProperty(), value_id)) {
            return InterpolationValue(
                MakeGarbageCollected<InterpolableLength>(value_id, state ? std::make_optional(state->StyleBuilder().InterpolateSize()) : std::nullopt));
        }

        double pixels;
        if (!LengthPropertyFunctions::GetPixelsForKeyword(CssProperty(), value_id, pixels))
            return nullptr;
...
{
    InterpolationValue result = CSSInterpolationType::MaybeConvertUnderlyingValue(environment);
    ...
    if (auto* length = To<InterpolableLength>(result.interpolable_value.Get())) {
        ...
        length->SetInterpolateSize(css_environment.AnimationControlsStyle().InterpolateSize());
    }
...
InterpolationValue CSSLengthInterpolationType::PreInterpolationCompositeIfNeeded(
    InterpolationValue value, const InterpolationValue& underlying, EffectModel::CompositeOperation composite, ConversionCheckers& conversion_checkers) const
{
    ...
    InterpolableLength& length = To<InterpolableLength>(*value.interpolable_value);
    const InterpolableLength* underlying_length = DynamicTo<InterpolableLength>(underlying.interpolable_value.Get());
...
{
    return InterpolableLength::MaybeMergeSingles(std::move(start.interpolable_value), std::move(end.interpolable_value));
}

InterpolationValue CSSLengthInterpolationType::MaybeConvertStandardPropertyUnderlyingValue(const ComputedStyle& style) const
{
    Length underlying_length;
    if (!LengthPropertyFunctions::GetLength(CssProperty(), style, underlying_length))
        return nullptr;
    return InterpolationValue(
        InterpolableLength::MaybeConvertLength(underlying_length, CssProperty(), EffectiveZoom(style.EffectiveZoom()), style.InterpolateSize()));
}

const CSSValue* CSSLengthInterpolationType::CreateCSSValue(
    const InterpolableValue& interpolable_value, const NonInterpolableValue*, const StyleResolverState&) const
{
    return To<InterpolableLength>(interpolable_value).CreateCSSValue(value_range_);
}
    ...
    const InterpolableValue& interpolable_value, const NonInterpolableValue* non_interpolable_value, StyleResolverState& state) const
{
    ...
    CSSToLengthConversionData conversion_data = state.CssToLengthConversionData().CopyWithAdjustedZoom(zoom);
    Length length = To<InterpolableLength>(interpolable_value).CreateLength(conversion_data, value_range_);
    if (LengthPropertyFunctions::SetLength(CssProperty(), builder, length)) {
        ...
        Length before;
        Length after;
        DCHECK(LengthPropertyFunctions::GetLength(CssProperty(), *before_style, before));
        StyleBuilder::ApplyProperty(GetProperty().GetCSSProperty(), state, *CSSValue::Create(length, zoom));
        const ComputedStyle* after_style = builder.CloneStyle();
        DCHECK(LengthPropertyFunctions::GetLength(CssProperty(), *after_style, after));
        if (before.IsSpecified() && after.IsSpecified()) {
            ...
            // (crbug.com/1349686).
            if (std::isfinite(before_length) && std::isfinite(after_length) && std::abs(before_length) < LayoutUnit::kIntMax) {
                // Test relative difference for large values to avoid floating point
                // inaccuracies tripping the check.
                const float delta = std::abs(before_length) < kSlack ? after_length - before_length : (after_length - before_length) / before_length;
