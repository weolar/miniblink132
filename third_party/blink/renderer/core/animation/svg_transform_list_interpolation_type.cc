...
public:
    ...
    static scoped_refptr<SVGTransformNonInterpolableValue> Create(Vector<SVGTransformType>& transform_types)
    {
        return base::AdoptRef(new SVGTransformNonInterpolableValue(transform_types));
    }
...
template <> struct DowncastTraits<SVGTransformNonInterpolableValue> {
...
SVGTransform* TranslateFromInterpolableValue(const InterpolableValue& value)
{
    ...
    auto* transform = MakeGarbageCollected<SVGTransform>(SVGTransformType::kTranslate);
    transform->SetTranslate(To<InterpolableNumber>(list.Get(0))->Value(length_resolver), To<InterpolableNumber>(list.Get(1))->Value(length_resolver));
    return transform;
}
...
SVGTransform* ScaleFromInterpolableValue(const InterpolableValue& value)
{
    ...
    CSSToLengthConversionData length_resolver;
    auto* transform = MakeGarbageCollected<SVGTransform>(SVGTransformType::kScale);
    transform->SetScale(To<InterpolableNumber>(list.Get(0))->Value(length_resolver), To<InterpolableNumber>(list.Get(1))->Value(length_resolver));
    return transform;
}
...
SVGTransform* RotateFromInterpolableValue(const InterpolableValue& value)
{
    ...
    CSSToLengthConversionData length_resolver;
    auto* transform = MakeGarbageCollected<SVGTransform>(SVGTransformType::kRotate);
    transform->SetRotate(To<InterpolableNumber>(list.Get(0))->Value(length_resolver), To<InterpolableNumber>(list.Get(1))->Value(length_resolver),
...
{
    auto* transform = MakeGarbageCollected<SVGTransform>(SVGTransformType::kSkewx);
    transform->SetSkewX(To<InterpolableNumber>(value).Value(CSSToLengthConversionData()));
    return transform;
}
...
{
    auto* transform = MakeGarbageCollected<SVGTransform>(SVGTransformType::kSkewy);
    transform->SetSkewY(To<InterpolableNumber>(value).Value(CSSToLengthConversionData()));
    return transform;
}

InterpolableValue* ToInterpolableValue(SVGTransform* transform, SVGTransformType transform_type)
{
...
SVGTransform* FromInterpolableValue(const InterpolableValue& value, SVGTransformType transform_type)
{
    switch (transform_type) {
    case SVGTransformType::kTranslate:
...
{
    return To<SVGTransformNonInterpolableValue>(*value.non_interpolable_value).TransformTypes();
}
...
public:
    ...
    bool IsValid(const InterpolationEnvironment&, const InterpolationValue& underlying) const final
    {
        ...
        return underlying_->underlying().interpolable_value->Equals(*underlying.interpolable_value)
            && GetTransformTypes(underlying_->underlying()) == GetTransformTypes(underlying);
    }
...
InterpolationValue SVGTransformListInterpolationType::MaybeConvertSVGValue(const SVGPropertyBase& svg_value) const
{
    ...
    return InterpolationValue(result, SVGTransformNonInterpolableValue::Create(transform_types));
}

InterpolationValue SVGTransformListInterpolationType::MaybeConvertSingle(const PropertySpecificKeyframe& keyframe, const InterpolationEnvironment& environment,
    const InterpolationValue& underlying, ConversionCheckers& conversion_checkers) const
{
    ...
    if (keyframe.Composite() == EffectModel::kCompositeAdd) {
        ...
        conversion_checkers.push_back(MakeGarbageCollected<SVGTransformListChecker>(underlying));
    } else {
        DCHECK(!keyframe.IsNeutral());
    }

    if (!keyframe.IsNeutral()) {
        auto* svg_value = To<SVGInterpolationEnvironment>(environment).SvgBaseValue().CloneForAnimation(To<SVGPropertySpecificKeyframe>(keyframe).Value());
    ...
    auto* interpolable_list = MakeGarbageCollected<InterpolableList>(types.size());
    ...
    for (auto& part : interpolable_parts) {
        ...
        for (wtf_size_t i = 0; i < list.length(); ++i) {
            interpolable_list->Set(interpolable_list_index, std::move(list.GetMutable(i)));
            ++interpolable_list_index;
        }
    }

    return InterpolationValue(interpolable_list, SVGTransformNonInterpolableValue::Create(types));
}

SVGPropertyBase* SVGTransformListInterpolationType::AppliedSVGValue(
    const InterpolableValue& interpolable_value, const NonInterpolableValue* non_interpolable_value) const
{
    auto* result = MakeGarbageCollected<SVGTransformList>();
    const auto& list = To<InterpolableList>(interpolable_value);
    const Vector<SVGTransformType>& transform_types = To<SVGTransformNonInterpolableValue>(non_interpolable_value)->TransformTypes();
    for (wtf_size_t i = 0; i < list.length(); ++i) {
