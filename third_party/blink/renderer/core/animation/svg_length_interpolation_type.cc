...
InterpolationValue SVGLengthInterpolationType::MaybeConvertSVGLength(const SVGLength& length)
{
    ...
    return InterpolationValue(InterpolableLength::MaybeConvertCSSValue(length.AsCSSPrimitiveValue()));
}

SVGLength* SVGLengthInterpolationType::ResolveInterpolableSVGLength(
    const InterpolableValue& interpolable_value, SVGLengthMode unit_mode, bool negative_values_forbidden)
{
    const InterpolableLength& length = To<InterpolableLength>(interpolable_value);
    const CSSPrimitiveValue* primitive_value = length.CreateCSSValue(negative_values_forbidden ? Length::ValueRange::kNonNegative : Length::ValueRange::kAll);
...
InterpolationValue SVGLengthInterpolationType::MaybeConvertSVGValue(const SVGPropertyBase& svg_value) const
{
...
SVGPropertyBase* SVGLengthInterpolationType::AppliedSVGValue(const InterpolableValue& interpolable_value, const NonInterpolableValue*) const
{
    return ResolveInterpolableSVGLength(interpolable_value, unit_mode_, negative_values_forbidden_);
}
