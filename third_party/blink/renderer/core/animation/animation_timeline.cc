...
bool CompareAnimations(const Member<Animation>& left, const Member<Animation>& right)
{
    ...
    return Animation::HasLowerCompositeOrdering(left.Get(), right.Get(), Animation::CompareAnimationsOrdering::kPointerOrder);
}
...
std::optional<AnimationTimeDelta> AnimationTimeline::CurrentTime()
{
    ...
    return result ? std::make_optional(AnimationTimeDelta(result.value())) : std::nullopt;
}
...
wtf_size_t AnimationTimeline::AnimationsNeedingUpdateCount() const
{
    ...
    for (const auto& animation : animations_needing_update_) {
        ...
        if ((!animation->CompositorPending() && !animation->Playing() && !IsProgressBased()) || animation->AnimationHasNoEffect()) {
            continue;
        }
        count++;
    }
...
void AnimationTimeline::ServiceAnimations(TimingUpdateReason reason)
{
    ...
    if (IsProgressBased() && last_current_phase_and_time_ != current_phase_and_time) {
...
void AnimationTimeline::getReplaceableAnimations(AnimationTimeline::ReplaceableAnimationsMap* replaceable_animations_map)
{
    for (Animation* animation : animations_) {
        ...
        if (inserted.is_new_entry) {
            inserted.stored_value->value = MakeGarbageCollected<HeapVector<Member<Animation>>>();
        }
...
void AnimationTimeline::SetOutdatedAnimation(Animation* animation)
{
    ...
    if (IsActive() && document_->GetPage() && !document_->GetPage()->Animator().IsServicingAnimations()) {
...
Animation* AnimationTimeline::Play(AnimationEffect* child, ExceptionState& exception_state)
{
...
void AnimationTimeline::MarkAnimationsCompositorPending(bool source_changed)
{
    Animation::CompositorPendingReason reason
        = source_changed ? Animation::CompositorPendingReason::kPendingEffectChange : Animation::CompositorPendingReason::kPendingUpdate;
    for (const auto& animation : animations_) {
        animation->SetCompositorPending(reason);
    }
}

void AnimationTimeline::MarkPendingIfCompositorPropertyAnimationChanges(const PaintArtifactCompositor* paint_artifact_compositor)
{
    for (const auto& animation : animations_) {
        animation->MarkPendingIfCompositorPropertyAnimationChanges(paint_artifact_compositor);
    }
}
