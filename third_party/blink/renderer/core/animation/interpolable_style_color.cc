...
void SetupColorInterpolationSpaces(Color& first, Color& second)
{
    ...
    if (first.GetColorSpace() == second.GetColorSpace()) {
        DCHECK(first.GetColorSpace() == Color::ColorSpace::kSRGBLegacy || first.GetColorSpace() == Color::ColorSpace::kOklab);
        return;
    }
    DCHECK(first.GetColorSpace() == Color::ColorSpace::kOklab || second.GetColorSpace() == Color::ColorSpace::kOklab);
...
void InterpolableStyleColor::Interpolate(const InterpolableValue& to, const double progress, InterpolableValue& result) const
{
...
/* static */
void InterpolableStyleColor::Interpolate(const InterpolableValue& from, const InterpolableValue& to, double progress, InterpolableValue& result)
{
    const InterpolableStyleColor& from_color = from.IsStyleColor() ? To<InterpolableStyleColor>(from) : *InterpolableStyleColor::Create(from);
    const InterpolableStyleColor& to_color = to.IsStyleColor() ? To<InterpolableStyleColor>(to) : *InterpolableStyleColor::Create(to);
    from_color.Interpolate(to_color, progress, result);
}

void InterpolableStyleColor::Composite(const BaseInterpolableColor& other, double fraction)
{
    ...
    const Color& current_color, const Color& active_link_color, const Color& link_color, const Color& text_color, mojom::blink::ColorScheme color_scheme) const
{
    if (blend_op_ == BlendOp::kBase) {
        ...
        if (from_color_) {
            ...
            return from_color_->Resolve(current_color, active_link_color, link_color, text_color, color_scheme);
        }
    ...
    Color first = from_color_->Resolve(current_color, active_link_color, link_color, text_color, color_scheme);
    Color second = to_color_->Resolve(current_color, active_link_color, link_color, text_color, color_scheme);

    if (blend_op_ == BlendOp::kInterpolate) {
        SetupColorInterpolationSpaces(first, second);
        return Color::InterpolateColors(first.GetColorSpace(), std::nullopt, first, second, fraction_);
    }
    ...
    return Color::FromColorSpace(first.GetColorSpace(), p0 / alpha, p1 / alpha, p2 / alpha, alpha);
}
