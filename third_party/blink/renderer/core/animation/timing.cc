...
EffectTiming* Timing::ConvertToEffectTiming() const
{
    ...
    if (iteration_duration) {
        duration = MakeGarbageCollected<V8UnionCSSNumericValueOrStringOrUnrestrictedDouble>(iteration_duration->InMillisecondsF());
    } else {
...
V8CSSNumberish* Timing::ToComputedValue(std::optional<AnimationTimeDelta> time, std::optional<AnimationTimeDelta> max_time) const
{
    if (time) {
...
ComputedEffectTiming* Timing::getComputedTiming(
    const CalculatedTiming& calculated_timing, const NormalizedTiming& normalized_timing, bool is_keyframe_effect) const
{
    ...
    computed_timing->setActiveDuration(ToComputedValue(normalized_timing.active_duration, normalized_timing.timeline_duration));
    ...
    computed_timing->setFill(Timing::FillModeEnum(ResolvedFillMode(is_keyframe_effect)));
    computed_timing->setIterationStart(iteration_start);
    computed_timing->setIterations(iteration_count);

    V8CSSNumberish* computed_duration = ToComputedValue(normalized_timing.iteration_duration, normalized_timing.timeline_duration);
    if (computed_duration->IsCSSNumericValue()) {
        if (normalized_timing.timeline_duration) {
            computed_timing->setDuration(MakeGarbageCollected<V8UnionCSSNumericValueOrStringOrUnrestrictedDouble>(computed_duration->GetAsCSSNumericValue()));
        }
    } else {
        computed_timing->setDuration(MakeGarbageCollected<V8UnionCSSNumericValueOrStringOrUnrestrictedDouble>(computed_duration->GetAsDouble()));
    }
...
Timing::CalculatedTiming Timing::CalculateTimings(std::optional<AnimationTimeDelta> local_time, bool is_idle, const NormalizedTiming& normalized_timing,
    AnimationDirection animation_direction, bool is_keyframe_effect, std::optional<double> playback_rate) const
{
    ...
    const std::optional<AnimationTimeDelta> active_time
        = TimingCalculations::CalculateActiveTime(normalized_timing, ResolvedFillMode(is_keyframe_effect), local_time, current_phase);

    std::optional<double> progress;

    const std::optional<double> overall_progress
        = TimingCalculations::CalculateOverallProgress(current_phase, active_time, duration, iteration_count, iteration_start);
    const std::optional<double> simple_iteration_progress
        = TimingCalculations::CalculateSimpleIterationProgress(current_phase, overall_progress, iteration_start, active_time, active_duration, iteration_count);
    const std::optional<double> current_iteration
        = TimingCalculations::CalculateCurrentIteration(current_phase, active_time, iteration_count, overall_progress, simple_iteration_progress);
    const bool current_direction_is_forwards = TimingCalculations::IsCurrentDirectionForwards(current_iteration, direction);
    const std::optional<double> directed_progress = TimingCalculations::CalculateDirectedProgress(simple_iteration_progress, current_iteration, direction);

    progress = TimingCalculations::CalculateTransformedProgress(current_phase, directed_progress, current_direction_is_forwards, timing_function);
    ...
    if (!duration.is_zero()) {
        ...
        const std::optional<AnimationTimeDelta> offset_active_time = TimingCalculations::CalculateOffsetActiveTime(active_duration, active_time, start_offset);
        const std::optional<AnimationTimeDelta> iteration_time
            = TimingCalculations::CalculateIterationTime(duration, active_duration, offset_active_time, start_offset, current_phase, *this);
        if (iteration_time) {
    ...
    DCHECK(!calculated.is_in_effect || (current_iteration.has_value() && progress.has_value()));
    calculated.is_in_play = calculated.phase == Timing::kPhaseActive;

    // https://w3.org/TR/web-animations-1/#current
    calculated.is_current = calculated.is_in_play || (playback_rate.has_value() && playback_rate > 0 && calculated.phase == Timing::kPhaseBefore)
        || (playback_rate.has_value() && playback_rate < 0 && calculated.phase == Timing::kPhaseAfter) || (!is_idle && normalized_timing.timeline_duration);
