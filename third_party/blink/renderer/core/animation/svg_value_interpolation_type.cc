...
public:
    ...
    static scoped_refptr<SVGValueNonInterpolableValue> Create(SVGPropertyBase* svg_value)
    {
        return base::AdoptRef(new SVGValueNonInterpolableValue(svg_value));
    }

    SVGPropertyBase* SvgValue() const
    {
        return svg_value_;
    }
...
template <> struct DowncastTraits<SVGValueNonInterpolableValue> {
...
{
    SVGPropertyBase* referenced_value = const_cast<SVGPropertyBase*>(&value); // Take ref.
    return InterpolationValue(MakeGarbageCollected<InterpolableList>(0), SVGValueNonInterpolableValue::Create(referenced_value));
}

SVGPropertyBase* SVGValueInterpolationType::AppliedSVGValue(const InterpolableValue&, const NonInterpolableValue* non_interpolable_value) const
{
