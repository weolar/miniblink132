...
TEST(InertEffectTest, IsCurrent)
{
    ...
    auto* opacity_model = CreateSimpleKeyframeEffectModelForTest(CSSPropertyID::kOpacity, "0", "1");

    {
        Timing timing;
        timing.iteration_duration = ANIMATION_TIME_DELTA_FROM_SECONDS(1000);

        auto* inert_effect = MakeGarbageCollected<InertEffect>(opacity_model, timing, animation_test_helpers::TestAnimationProxy());
...

    {
        ...
        auto* inert_effect = MakeGarbageCollected<InertEffect>(opacity_model, timing, proxy);
...
TEST(InertEffectTest, Affects)
{
    test::TaskEnvironment task_environment;
    auto* opacity_model = CreateSimpleKeyframeEffectModelForTest(CSSPropertyID::kOpacity, "0", "1");
    auto* color_model = CreateSimpleKeyframeEffectModelForTest(CSSPropertyID::kColor, "red", "green");

    Timing timing;

    auto* opacity_effect = MakeGarbageCollected<InertEffect>(opacity_model, timing, animation_test_helpers::TestAnimationProxy());

    auto* color_effect = MakeGarbageCollected<InertEffect>(color_model, timing, animation_test_helpers::TestAnimationProxy());
