...
TEST_F(ScrollTimelineUtilTest, ToCompositorScrollTimeline)
{
    ...
    std::optional<CompositorElementId> element_id = GetCompositorScrollElementId(scroller);
    ASSERT_TRUE(element_id.has_value());

    ScrollTimelineOptions* options = ScrollTimelineOptions::Create();
    options->setSource(scroller);
    options->setAxis("block");
    ScrollTimeline* timeline = ScrollTimeline::Create(GetDocument(), options, ASSERT_NO_EXCEPTION);

    scoped_refptr<CompositorScrollTimeline> compositor_timeline = ToCompositorScrollTimeline(timeline);
    EXPECT_EQ(compositor_timeline->GetActiveIdForTest(), std::nullopt);
    EXPECT_EQ(compositor_timeline->GetPendingIdForTest(), element_id);
    EXPECT_EQ(compositor_timeline->GetDirectionForTest(), CompositorScrollTimeline::ScrollDown);
}
...
TEST_F(ScrollTimelineUtilTest, ToCompositorScrollTimelineDocumentTimelineParameter)
{
    ScopedNullExecutionContext execution_context;
    DocumentTimeline* timeline = MakeGarbageCollected<DocumentTimeline>(Document::CreateForTest(execution_context.GetExecutionContext()));
...
TEST_F(ScrollTimelineUtilTest, ToCompositorScrollTimelineNullSource)
{
    ...
    ScrollTimeline* timeline = ScrollTimeline::Create(&GetDocument(), source, ScrollTimeline::ScrollAxis::kBlock);
...
TEST_F(ScrollTimelineUtilTest, ConvertOrientationPhysicalCases)
{
    ...
    // sure it doesn't.
    Vector<WritingMode> writing_modes = { WritingMode::kHorizontalTb, WritingMode::kVerticalLr, WritingMode::kVerticalRl };
    ...
    for (const WritingMode& writing_mode : writing_modes) {
        for (const TextDirection& direction : directions) {
            ComputedStyleBuilder style_builder = GetDocument().GetStyleResolver().CreateComputedStyleBuilder();
            ...
            EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kY, style), CompositorScrollTimeline::ScrollDown);
            EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kX, style), CompositorScrollTimeline::ScrollRight);
        }
    }
}
...
TEST_F(ScrollTimelineUtilTest, ConvertOrientationLogical)
{
    ...
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kBlock, style), CompositorScrollTimeline::ScrollDown);
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kInline, style), CompositorScrollTimeline::ScrollRight);
    ...
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kBlock, style), CompositorScrollTimeline::ScrollRight);
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kInline, style), CompositorScrollTimeline::ScrollDown);
    ...
    builder.SetDirection(TextDirection::kLtr);
    style = builder.TakeStyle();
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kBlock, style), CompositorScrollTimeline::ScrollLeft);
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kInline, style), CompositorScrollTimeline::ScrollDown);
    ...
    style = builder.TakeStyle();
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kBlock, style), CompositorScrollTimeline::ScrollDown);
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kInline, style), CompositorScrollTimeline::ScrollLeft);
    ...
    style = builder.TakeStyle();
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kBlock, style), CompositorScrollTimeline::ScrollRight);
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kInline, style), CompositorScrollTimeline::ScrollUp);
...
TEST_F(ScrollTimelineUtilTest, ConvertOrientationNullStyle)
{
    ...
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kY, nullptr), CompositorScrollTimeline::ScrollDown);
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kX, nullptr), CompositorScrollTimeline::ScrollRight);
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kBlock, nullptr), CompositorScrollTimeline::ScrollDown);
    EXPECT_EQ(ConvertOrientation(ScrollTimeline::ScrollAxis::kInline, nullptr), CompositorScrollTimeline::ScrollRight);
}
