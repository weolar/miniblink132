...
public:
    ...
    InterpolableGridLength(InterpolableValue* value, InterpolableGridLengthType type);
    static InterpolableGridLength* Create(const Length& grid_length, const CSSProperty& property, float zoom);

    Length CreateGridLength(const CSSToLengthConversionData& conversion_data) const;
...
template <> struct DowncastTraits<InterpolableGridLength> {
