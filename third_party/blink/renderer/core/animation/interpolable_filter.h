...
public:
    InterpolableFilter(InterpolableValue* value, FilterOperation::OperationType type)
        : value_(value)
        , type_(type)
    {
        static_assert(std::is_trivially_destructible_v<InterpolableFilter>, "Require trivial destruction for faster sweeping");
        DCHECK(value_);
    }

    static InterpolableFilter* MaybeCreate(
        const FilterOperation&, const CSSProperty& property, double zoom, mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider);
    static InterpolableFilter* MaybeConvertCSSValue(const CSSValue&, mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider);
    ...
    FilterOperation::OperationType GetType() const
    {
        return type_;
    }
...
private:
    ...
    {
        return MakeGarbageCollected<InterpolableFilter>(value_->CloneAndZero(), type_);
    }
...
template <> struct DowncastTraits<InterpolableFilter> {
