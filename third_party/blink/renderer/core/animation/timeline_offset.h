...
struct TimelineOffset {
    ...
    TimelineOffset(NamedRange name, Length offset, std::optional<String> style_dependent_offset = std::nullopt)
        : name(name)
        , offset(offset)
    ...
    {
        return name == other.name && offset == other.offset && style_dependent_offset == other.style_dependent_offset;
    }
    ...
    static std::optional<TimelineOffset> Create(Element* element, String value, double default_percent, ExceptionState& exception_state);
