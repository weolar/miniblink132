...
public:
    CSSInterpolationTypesMap(const PropertyRegistry* registry, const Document& document);

    const InterpolationTypes& Get(const PropertyHandle&) const final;
    size_t Version() const final;

    static InterpolationTypes CreateInterpolationTypesForCSSSyntax(const AtomicString& property_name, const CSSSyntaxDefinition&, const PropertyRegistration&);
