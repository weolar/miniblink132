...
private:
    explicit TestNonInterpolableValue(int value)
        : value_(value)
    {
    }
...
inline const TestNonInterpolableValue& ToTestNonInterpolableValue(const NonInterpolableValue& value)
{
...
InterpolationValue CreateInterpolableList(const Vector<std::pair<double, int>>& values)
{
    return ListInterpolationFunctions::CreateList(values.size(), [&values](wtf_size_t i) {
        return InterpolationValue(MakeGarbageCollected<InterpolableNumber>(values[i].first), TestNonInterpolableValue::Create(values[i].second));
    });
}
...
InterpolationValue CreateInterpolableList(const Vector<double>& values)
{
    return ListInterpolationFunctions::CreateList(
        values.size(), [&values](wtf_size_t i) { return InterpolationValue(MakeGarbageCollected<InterpolableNumber>(values[i]), nullptr); });
}
...
{
    return ListInterpolationFunctions::CreateList(values.size(),
        [&values](wtf_size_t i) { return InterpolationValue(MakeGarbageCollected<InterpolableNumber>(0), TestNonInterpolableValue::Create(values[i])); });
}
...
bool NonInterpolableValuesAreCompatible(const NonInterpolableValue* a, const NonInterpolableValue* b)
{
    return (a ? ToTestNonInterpolableValue(*a).GetValue() : 0) == (b ? ToTestNonInterpolableValue(*b).GetValue() : 0);
}

PairwiseInterpolationValue MaybeMergeSingles(InterpolationValue&& start, InterpolationValue&& end)
{
...
void Composite(UnderlyingValue& underlying_value, double underlying_fraction, const InterpolableValue& interpolable_value,
    const NonInterpolableValue* non_interpolable_value)
{
    DCHECK(NonInterpolableValuesAreCompatible(underlying_value.GetNonInterpolableValue(), non_interpolable_value));
...
TEST(ListInterpolationFunctionsTest, EqualMergeSinglesSameLengths)
{
    ...
    auto pairwise = ListInterpolationFunctions::MaybeMergeSingles(
        std::move(list1), std::move(list2), ListInterpolationFunctions::LengthMatchingStrategy::kEqual, MaybeMergeSingles);
...
TEST(ListInterpolationFunctionsTest, EqualCompositeSameLengths)
{
    ...
    ListInterpolationFunctions::Composite(owner, 1.0, interpolation_type, list2, ListInterpolationFunctions::LengthMatchingStrategy::kEqual,
        ListInterpolationFunctions::InterpolableValuesKnownCompatible, NonInterpolableValuesAreCompatible, Composite);
...
TEST(ListInterpolationFunctionsTest, EqualCompositeIncompatibleInterpolableValues)
{
    auto list1 = CreateInterpolableList({ 1.0, 2.0, 3.0 });
    auto list2 = CreateInterpolableList({ 4.0, 5.0, 6.0 });

    InterpolableValuesCompatibilityHelper compatibility_helper({ true, false, true });
    ...
    ListInterpolationFunctions::Composite(
        owner, 1.0, interpolation_type, list2, ListInterpolationFunctions::LengthMatchingStrategy::kEqual,
        [&compatibility_helper](const InterpolableValue* a, const InterpolableValue* b) { return compatibility_helper.AreCompatible(a, b); },
...
// If one (or more) of the element pairs are incompatible, the list as a whole
// is non-interpolable. We expect the underlying value to be replaced.
TEST(ListInterpolationFunctionsTest, EqualCompositeIncompatibleNonInterpolableValues)
{
    auto list1 = CreateInterpolableList({ { 1.0, 1 }, { 2.0, 2 }, { 3.0, 3 } });
