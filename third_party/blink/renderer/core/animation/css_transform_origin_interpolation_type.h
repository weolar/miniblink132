...
private:
    InterpolationValue MaybeConvertValue(const CSSValue& value, const StyleResolverState*, ConversionCheckers&) const final
    {
        ...
        return ListInterpolationFunctions::CreateList(3, [&list](wtf_size_t index) {
            if (index == list.length()) {
                return InterpolationValue(InterpolableLength::MaybeConvertCSSValue(*CSSNumericLiteralValue::Create(0, CSSPrimitiveValue::UnitType::kPixels)));
            }
