...
public:
    ...
    virtual bool IsStyleColor() const
    {
        return false;
    }
    virtual bool IsScrollbarColor() const
    {
        return false;
    }
    ...
    virtual bool IsLength() const
    {
        return false;
    }
    virtual bool IsAspectRatio() const
    {
        return false;
    }
    ...
    virtual bool IsTransformList() const
    {
        return false;
    }
    virtual bool IsGridLength() const
    {
        return false;
    }
    virtual bool IsGridTrackList() const
    {
        return false;
    }
    virtual bool IsGridTrackRepeater() const
    {
        return false;
    }
    virtual bool IsGridTrackSize() const
    {
        return false;
    }
    virtual bool IsFontPalette() const
    {
        return false;
    }
    virtual bool IsDynamicRangeLimit() const
    {
        return false;
    }
    ...
    InterpolableValue* Clone() const
    {
        return RawClone();
    }
    InterpolableValue* CloneAndZero() const
    {
        return RawCloneAndZero();
    }
...
public:
    ...
    explicit InlinedInterpolableDouble(double d)
        : value_(d)
    {
    }
...
public:
    ...
    explicit InterpolableNumber(double value, CSSPrimitiveValue::UnitType unit_type = CSSPrimitiveValue::UnitType::kNumber);
    ...
    InterpolableNumber* Clone() const
    {
        return RawClone();
    }
...
private:
    ...
    {
        if (IsDoubleValue()) {
            return MakeGarbageCollected<InterpolableNumber>(value_.Value(), unit_type_);
        }
...
static_assert(std::is_trivially_destructible_v<InterpolableNumber>, "Require trivial destruction for faster sweeping");

class CORE_EXPORT InterpolableList final : public InterpolableValue {
public:
    explicit InterpolableList(wtf_size_t size)
        : values_(size)
    {
        static_assert(std::is_trivially_destructible_v<InterpolableList>, "Require trivial destruction for faster sweeping");
    }
...
template <> struct DowncastTraits<InterpolableNumber> {
    static bool AllowFrom(const InterpolableValue& value)
    {
        return value.IsNumber();
    }
};
template <> struct DowncastTraits<InterpolableList> {
    static bool AllowFrom(const InterpolableValue& value)
    {
        return value.IsList();
    }
