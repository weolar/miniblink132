...
const double Keyframe::kNullComputedOffset = std::numeric_limits<double>::quiet_NaN();

Keyframe::PropertySpecificKeyframe::PropertySpecificKeyframe(double offset, scoped_refptr<TimingFunction> easing, EffectModel::CompositeOperation composite)
    : offset_(offset)
...
Interpolation* Keyframe::PropertySpecificKeyframe::CreateInterpolation(
    const PropertyHandle& property_handle, const Keyframe::PropertySpecificKeyframe& end) const
{
...
void Keyframe::AddKeyframePropertiesToV8Object(V8ObjectBuilder& object_builder, Element* element) const
{
    ...
    if (timeline_offset_) {
        ...
        timeline_range_offset->setOffset(CSSUnitValue::Create(timeline_offset_->offset.Value(), CSSPrimitiveValue::UnitType::kPercentage));
    ...
    if (composite_) {
        object_builder.AddString("composite", V8CompositeOperation(EffectModel::CompositeOperationToEnum(composite_.value())).AsCStr());
    } else {
        object_builder.AddString("composite", "auto");
    }
}

bool Keyframe::ResolveTimelineOffset(const TimelineRange& timeline_range, double range_start, double range_end)
{
    if (!timeline_offset_) {
        return false;
    }

    double relative_offset = timeline_range.ToFractionalOffset(timeline_offset_.value());
...
{
    std::optional first = a->ComputedOffset().has_value() ? a->ComputedOffset() : a->Offset();
    std::optional second = b->ComputedOffset().has_value() ? b->ComputedOffset() : b->Offset();
