...
public:
    template <typename CreateItemCallback> static InterpolationValue CreateList(wtf_size_t length, CreateItemCallback);
    static InterpolationValue CreateEmptyList()
    {
        return InterpolationValue(MakeGarbageCollected<InterpolableList>(0));
    }

    enum class LengthMatchingStrategy { kEqual, kLowestCommonMultiple, kPadToLargest };

    using MergeSingleItemConversionsCallback = base::FunctionRef<PairwiseInterpolationValue(InterpolationValue&&, InterpolationValue&&)>;

    static PairwiseInterpolationValue MaybeMergeSingles(
        InterpolationValue&& start, InterpolationValue&& end, LengthMatchingStrategy, MergeSingleItemConversionsCallback);
    ...
    using InterpolableValuesAreCompatibleCallback = base::FunctionRef<bool(const InterpolableValue*, const InterpolableValue*)>;
    using NonInterpolableValuesAreCompatibleCallback = base::FunctionRef<bool(const NonInterpolableValue*, const NonInterpolableValue*)>;
    using CompositeItemCallback = base::FunctionRef<void(UnderlyingValue&, double underlying_fraction, const InterpolableValue&, const NonInterpolableValue*)>;
    static void Composite(UnderlyingValueOwner&, double underlying_fraction, const InterpolationType&, const InterpolationValue&, LengthMatchingStrategy,
        InterpolableValuesAreCompatibleCallback, NonInterpolableValuesAreCompatibleCallback, CompositeItemCallback);

    // Used when the interpolable values are known to always be compatible.
    static bool InterpolableValuesKnownCompatible(const InterpolableValue* a, const InterpolableValue* b)
    {
        return true;
    }
    ...
    static bool VerifyNoNonInterpolableValues(const NonInterpolableValue* a, const NonInterpolableValue* b);
};
...
public:
    ...
    static scoped_refptr<NonInterpolableList> Create(Vector<scoped_refptr<const NonInterpolableValue>>&& list)
    {
...
template <> struct DowncastTraits<NonInterpolableList> {
...
template <typename CreateItemCallback> InterpolationValue ListInterpolationFunctions::CreateList(wtf_size_t length, CreateItemCallback create_item)
{
    if (length == 0)
    ...
    return InterpolationValue(interpolable_list, NonInterpolableList::Create(std::move(non_interpolable_values)));
}
