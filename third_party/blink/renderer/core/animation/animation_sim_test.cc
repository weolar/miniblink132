...
TEST_F(AnimationSimTest, CustomPropertyBaseComputedStyle)
{
    ...
    //   inherits: false
    // })
    css_test_helpers::RegisterProperty(GetDocument(), "--x", "<percentage>", "0%", false);

    DummyExceptionStateForTesting exception_state;
    // target.style.setProperty('--x', '100%');
    target->style()->setProperty(GetDocument().GetExecutionContext(), "--x", "100%", g_empty_string, exception_state);
    EXPECT_THAT(exception_state, HadNoException());

    // target.animate({'--x': '100%'}, 1000);
    auto* keyframe = MakeGarbageCollected<StringKeyframe>();
    keyframe->SetCSSPropertyValue(AtomicString("--x"), "100%", Window().GetSecureContextMode(), GetDocument().ElementSheet().Contents());
    StringKeyframeVector keyframes;
    keyframes.push_back(keyframe);
    Timing timing;
    timing.iteration_duration = ANIMATION_TIME_DELTA_FROM_SECONDS(1);

    auto* keyframe_effect = MakeGarbageCollected<KeyframeEffect>(target, MakeGarbageCollected<StringKeyframeEffectModel>(keyframes), timing);
    ...
    target->style()->setProperty(GetDocument().GetExecutionContext(), "--x", "0%", g_empty_string, exception_state);
    EXPECT_THAT(exception_state, HadNoException());
    ...
    keyframe_effect = MakeGarbageCollected<KeyframeEffect>(target, MakeGarbageCollected<StringKeyframeEffectModel>(keyframes), timing);
    target->GetDocument().Timeline().Play(keyframe_effect);
