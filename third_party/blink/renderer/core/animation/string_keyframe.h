...
public:
    explicit StringKeyframe(const TreeScope* tree_scope = nullptr)
        ...
        , presentation_attribute_map_(MakeGarbageCollected<MutableCSSPropertyValueSet>(kHTMLStandardMode))
    {
    }
    StringKeyframe(const StringKeyframe& copy_from);

    MutableCSSPropertyValueSet::SetResult SetCSSPropertyValue(
        const AtomicString& custom_property_name, const String& value, SecureContextMode, StyleSheetContents*);
    MutableCSSPropertyValueSet::SetResult SetCSSPropertyValue(CSSPropertyID, const String& value, SecureContextMode, StyleSheetContents*);
    void SetCSSPropertyValue(const CSSPropertyName&, const CSSValue&);
    void RemoveCustomCSSProperty(const PropertyHandle& property);

    void SetPresentationAttributeValue(const CSSProperty&, const String& value, SecureContextMode, StyleSheetContents*);
    ...
    const CSSValue& CssPropertyValue(const PropertyHandle& property) const
    {
        ...
        if (property.IsCSSCustomProperty()) {
            index = css_property_map_->FindPropertyIndex(property.CustomPropertyName());
        } else {
            DCHECK(!property.GetCSSProperty().IsShorthand());
            index = css_property_map_->FindPropertyIndex(property.GetCSSProperty().PropertyID());
        }
        CHECK_GE(index, 0);
        return css_property_map_->PropertyAt(static_cast<unsigned>(index)).Value().EnsureScopedValue(tree_scope_.Get());
    }

    const CSSValue& PresentationAttributeValue(const CSSProperty& property) const
    {
        int index = presentation_attribute_map_->FindPropertyIndex(property.PropertyID());
        CHECK_GE(index, 0);
        return presentation_attribute_map_->PropertyAt(static_cast<unsigned>(index)).Value().EnsureScopedValue(tree_scope_.Get());
        ;
    }
    ...
    void AddKeyframePropertiesToV8Object(V8ObjectBuilder&, Element*) const override;

    Keyframe* Clone() const override;

    bool HasLogicalProperty()
    {
        return has_logical_property_;
    }
    ...
    public:
        CSSPropertySpecificKeyframe(double offset, scoped_refptr<TimingFunction> easing, const CSSValue* value, EffectModel::CompositeOperation composite)
        ...
        bool PopulateCompositorKeyframeValue(const PropertyHandle&, Element&, const ComputedStyle& base_style, const ComputedStyle* parent_style) const final;
        ...
        Keyframe::PropertySpecificKeyframe* NeutralKeyframe(double offset, scoped_refptr<TimingFunction> easing) const final;

        void Trace(Visitor*) const override;

    private:
        Keyframe::PropertySpecificKeyframe* CloneWithOffset(double offset) const override;
        bool IsCSSPropertySpecificKeyframe() const override
        {
            return true;
        }
    ...
    public:
        SVGPropertySpecificKeyframe(double offset, scoped_refptr<TimingFunction> easing, const String& value, EffectModel::CompositeOperation composite)
            : Keyframe::PropertySpecificKeyframe(offset, std::move(easing), composite)
            , value_(value)
        {
        }
        ...
        PropertySpecificKeyframe* NeutralKeyframe(double offset, scoped_refptr<TimingFunction> easing) const final;

    private:
        bool IsSVGPropertySpecificKeyframe() const override
        {
            return true;
        }
    ...
    public:
        ...
        // constructor.
        PropertyResolver(const CSSProperty& property, const MutableCSSPropertyValueSet* property_value_set, bool is_logical);

        static PropertyResolver* CreateCustomVariableResolver(const CSSValue& css_value);

        bool IsValid() const;

        const CSSValue* CssValue();

        void AppendTo(MutableCSSPropertyValueSet* property_value_set, WritingDirectionMode writing_direction);
        ...
        static bool HasLowerPriority(PropertyResolver* first, PropertyResolver* second);
        ...
        bool IsShorthand()
        {
            return css_property_value_set_ != nullptr;
        }
        unsigned ExpansionCount()
        {
            return css_property_value_set_ ? css_property_value_set_->PropertyCount() : 1;
        }
...
private:
    Keyframe::PropertySpecificKeyframe* CreatePropertySpecificKeyframe(
        const PropertyHandle&, EffectModel::CompositeOperation effect_composite, double offset) const override;

    void InvalidateCssPropertyMap()
    {
        css_property_map_ = nullptr;
    }
    void EnsureCssPropertyMap() const;

    bool IsStringKeyframe() const override
    {
        return true;
    }
    ...
    // properties.
    WritingDirectionMode writing_direction_ { WritingMode::kHorizontalTb, TextDirection::kLtr };
};
