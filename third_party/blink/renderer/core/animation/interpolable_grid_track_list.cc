...
InterpolableGridTrackList::InterpolableGridTrackList(InterpolableList* values, double progress)
    : values_(values)
    , progress_(progress)
{
    DCHECK(values_);
...
// static
InterpolableGridTrackList* InterpolableGridTrackList::MaybeCreate(const NGGridTrackList& track_list, const CSSProperty& property, float zoom)
{
    ...
    for (wtf_size_t i = 0; i < repeater_count; ++i) {
        ...
        const NGGridTrackRepeater repeater(
            track_list.RepeatIndex(i), track_list.RepeatSize(i), track_list.RepeatCount(i, 0), track_list.LineNameIndicesCount(i), track_list.RepeatType(i));
        InterpolableGridTrackRepeater* result = InterpolableGridTrackRepeater::Create(repeater, repeater_track_sizes, property, zoom);
        DCHECK(result);
        values->Set(i, result);
    }
...
NGGridTrackList InterpolableGridTrackList::CreateNGGridTrackList(const CSSToLengthConversionData& conversion_data) const
{
    NGGridTrackList new_track_list;
    for (wtf_size_t i = 0; i < values_->length(); ++i) {
        const InterpolableGridTrackRepeater& repeater = To<InterpolableGridTrackRepeater>(*values_->Get(i));
        new_track_list.AddRepeater(repeater.CreateTrackSizes(conversion_data), repeater.RepeatType(), repeater.RepeatCount());
    }
...
InterpolableGridTrackList* InterpolableGridTrackList::RawClone() const
{
    ...
    return MakeGarbageCollected<InterpolableGridTrackList>(std::move(values), progress_);
}
...
{
    return IsCompatibleWith(other) && values_->Equals(*(To<InterpolableGridTrackList>(other).values_));
}
...
void InterpolableGridTrackList::Add(const InterpolableValue& other)
{
    ...
    const InterpolableGridTrackList& other_track_list = To<InterpolableGridTrackList>(other);
    values_->Add(*other_track_list.values_);
    progress_ = other_track_list.progress_;
}

bool InterpolableGridTrackList::IsCompatibleWith(const InterpolableValue& other) const
{
    const InterpolableGridTrackList& other_track_list = To<InterpolableGridTrackList>(other);
...
void InterpolableGridTrackList::AssertCanInterpolateWith(const InterpolableValue& other) const
{
    const InterpolableGridTrackList& other_track_list = To<InterpolableGridTrackList>(other);
...
void InterpolableGridTrackList::Interpolate(const InterpolableValue& to, const double progress, InterpolableValue& result) const
{
    const InterpolableGridTrackList& grid_track_list_to = To<InterpolableGridTrackList>(to);
    InterpolableGridTrackList& grid_track_list_result = To<InterpolableGridTrackList>(result);
    values_->Interpolate(*grid_track_list_to.values_, progress, *grid_track_list_result.values_);
