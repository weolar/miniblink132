...
{
    return CSSMathExpressionNumericLiteral::Create(CSSNumericLiteralValue::Create(number, unit_type));
}
...
{
    if (IsDoubleValue()) {
        return value_.Value() * CSSPrimitiveValue::ConversionToCanonicalUnitsScaleFactor(unit_type_);
    }
    std::optional<double> result = expression_->ComputeValueInCanonicalUnit(length_resolver);
...
bool InterpolableNumber::Equals(const InterpolableValue& other) const
{
    ...
    if (IsDoubleValue() && other_number.IsDoubleValue() && unit_type_ == other_number.unit_type_) {
...
void InterpolableNumber::Interpolate(const InterpolableValue& to, const double progress, InterpolableValue& result) const
{
    ...
    if (IsDoubleValue() && to_number.IsDoubleValue() && unit_type_ == to_number.unit_type_) {
    ...
    CSSMathExpressionNode* blended_to
        = CSSMathExpressionOperation::CreateArithmeticOperationAndSimplifyCalcSize(&to_number.AsExpression(), NumberNode(progress), CSSMathOperator::kMultiply);
...
void InterpolableList::Interpolate(const InterpolableValue& to, const double progress, InterpolableValue& result) const
{
    ...
    for (wtf_size_t i = 0; i < length(); i++) {
        ...
        if (values_[i]->IsStyleColor() || to_list.values_[i]->IsStyleColor() || result_list.values_[i]->IsStyleColor()) {
            ...
            InterpolableStyleColor::Interpolate(*values_[i], *(to_list.values_[i]), progress, *(result_list.values_[i]));
            continue;
        }
...
void InterpolableNumber::Scale(double scale)
{
    ...
    SetExpression(*CSSMathExpressionOperation::CreateArithmeticOperationAndSimplifyCalcSize(&AsExpression(), NumberNode(scale), CSSMathOperator::kMultiply));
}

void InterpolableNumber::Scale(const InterpolableNumber& other)
{
    if (IsDoubleValue() && other.IsDoubleValue()
        && (unit_type_ == CSSPrimitiveValue::UnitType::kNumber || other.unit_type_ == CSSPrimitiveValue::UnitType::kNumber)) {
        SetDouble(value_.Value() * other.Value(), (unit_type_ == CSSPrimitiveValue::UnitType::kNumber ? other.unit_type_ : unit_type_));
        return;
    }
    SetExpression(
        *CSSMathExpressionOperation::CreateArithmeticOperationAndSimplifyCalcSize(&AsExpression(), &other.AsExpression(), CSSMathOperator::kMultiply));
}
...
void InterpolableNumber::Add(const InterpolableValue& other)
{
    ...
    CSSMathExpressionNode* result
        = CSSMathExpressionOperation::CreateArithmeticOperationAndSimplifyCalcSize(&AsExpression(), &other_number.AsExpression(), CSSMathOperator::kAdd);
