...
public:
    ...
    {
        DCHECK(property.GetCSSProperty().PropertyID() == CSSPropertyID::kContainIntrinsicWidth
            || property.GetCSSProperty().PropertyID() == CSSPropertyID::kContainIntrinsicHeight);
    }
    ...
    static InterpolableValue* CreateInterpolableIntrinsicDimension(const StyleIntrinsicLength&);

private:
    StyleIntrinsicLength GetIntrinsicDimension(const ComputedStyle&) const;
    void SetIntrinsicDimension(ComputedStyleBuilder&, const StyleIntrinsicLength& dimension) const;
