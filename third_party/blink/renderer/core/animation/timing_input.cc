...
Timing::PlaybackDirection ConvertPlaybackDirection(V8PlaybackDirection::Enum direction)
{
    switch (direction) {
...
std::optional<AnimationTimeDelta> ConvertIterationDuration(const V8UnionCSSNumericValueOrStringOrUnrestrictedDouble* duration)
{
    if (duration->IsUnrestrictedDouble()) {
        return ANIMATION_TIME_DELTA_FROM_MILLISECONDS(duration->GetAsUnrestrictedDouble());
    }
    return std::nullopt;
}

Timing::Delay ConvertDelay(const Timing::V8Delay* delay, double default_percent, ExceptionState& exception_state)
{
    Timing::Delay result;
    if (delay->IsDouble()) {
        ...
        CSSUnitValue* unit_value = numeric_value->to(CSSPrimitiveValue::UnitType::kPercentage);
        if (!unit_value) {
            exception_state.ThrowTypeError("Delay must be a finite double or percentage for animation delay.");
            return result;
        }
...
Timing ConvertEffectTiming(const EffectTiming* timing_input, Document* document, ExceptionState& exception_state)
{
...
template <class V> bool UpdateValueIfChanged(V& lhs, const V& rhs)
{
    if (lhs != rhs) {
        lhs = rhs;
        return true;
...
Timing TimingInput::Convert(const V8UnionKeyframeEffectOptionsOrUnrestrictedDouble* options, Document* document, ExceptionState& exception_state)
{
    if (!options) {
        return Timing();
    }
    ...
    case V8UnionKeyframeEffectOptionsOrUnrestrictedDouble::ContentType::kKeyframeEffectOptions:
        return ConvertEffectTiming(options->GetAsKeyframeEffectOptions(), document, exception_state);
    case V8UnionKeyframeEffectOptionsOrUnrestrictedDouble::ContentType::kUnrestrictedDouble: {
        ...
        timing_input->setDuration(MakeGarbageCollected<V8UnionCSSNumericValueOrStringOrUnrestrictedDouble>(options->GetAsUnrestrictedDouble()));
...
Timing TimingInput::Convert(const V8UnionKeyframeAnimationOptionsOrUnrestrictedDouble* options, Document* document, ExceptionState& exception_state)
{
    if (!options) {
        return Timing();
    }
    ...
    switch (options->GetContentType()) {
    case V8UnionKeyframeAnimationOptionsOrUnrestrictedDouble::ContentType::kKeyframeAnimationOptions:
        return ConvertEffectTiming(options->GetAsKeyframeAnimationOptions(), document, exception_state);
    case V8UnionKeyframeAnimationOptionsOrUnrestrictedDouble::ContentType::kUnrestrictedDouble: {
        // https://drafts.csswg.org/web-animations-1/#dom-keyframeeffect-keyframeeffect
        // If options is a double,
        //   Let timing input be a new EffectTiming object with all members set to
        //   their default values and duration set to options.
...
template <class InputTiming> bool TimingInput::Update(Timing& timing, const InputTiming* input, Document* document, ExceptionState& exception_state)
{
    ...
    if (input->hasIterations() && (std::isnan(input->iterations()) || input->iterations() < 0)) {
    ...
    if (input->hasDuration()) {
        ...
        case V8UnionCSSNumericValueOrStringOrUnrestrictedDouble::ContentType::kCSSNumericValue:
            exception_state.ThrowTypeError("Setting duration using CSSNumericValue is not supported.");
            return false;
        case V8UnionCSSNumericValueOrStringOrUnrestrictedDouble::ContentType::kString:
            if (input->duration()->GetAsString() != "auto") {
                exception_state.ThrowTypeError(error_message);
                return false;
            }
            break;
        case V8UnionCSSNumericValueOrStringOrUnrestrictedDouble::ContentType::kUnrestrictedDouble: {
    ...
    if (input->hasEasing()) {
        timing_function = AnimationInputHelpers::ParseTimingFunction(input->easing(), document, exception_state);
        if (!timing_function) {
    ...
    bool changed = false;
    if (input->hasDelay()) {
        changed |= UpdateValueIfChanged(timing.start_delay, ConvertDelay(input->delay(), 0, exception_state));
        timing.SetTimingOverride(Timing::kOverrideStartDelay);
    }
    if (input->hasEndDelay()) {
        changed |= UpdateValueIfChanged(timing.end_delay, ConvertDelay(input->endDelay(), 100, exception_state));
        timing.SetTimingOverride(Timing::kOverrideEndDelay);
    }
    if (input->hasFill()) {
        changed |= UpdateValueIfChanged(timing.fill_mode, Timing::EnumToFillMode(input->fill().AsEnum()));
        timing.SetTimingOverride(Timing::kOverideFillMode);
    }
    if (input->hasIterationStart()) {
        changed |= UpdateValueIfChanged(timing.iteration_start, input->iterationStart());
    ...
    if (input->hasDuration()) {
        changed |= UpdateValueIfChanged(timing.iteration_duration, ConvertIterationDuration(input->duration()));
        timing.SetTimingOverride(Timing::kOverrideDuration);
    }
    if (input->hasDirection()) {
        changed |= UpdateValueIfChanged(timing.direction, ConvertPlaybackDirection(input->direction().AsEnum()));
...
template CORE_EXPORT bool TimingInput::Update(Timing&, const OptionalEffectTiming*, Document*, ExceptionState&);
