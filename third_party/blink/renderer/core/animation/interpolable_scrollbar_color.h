...
public:
    ...
    InterpolableScrollbarColor(InterpolableColor* thumb_color, InterpolableColor* track_color);

    static InterpolableScrollbarColor* Create(const StyleScrollbarColor&);
    bool IsScrollbarColor() const final
    {
        return true;
    }
    ...
    InterpolableScrollbarColor* Clone() const
    {
        return RawClone();
    }
...
template <> struct DowncastTraits<InterpolableScrollbarColor> {
