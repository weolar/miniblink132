...
InterpolationValue ConvertTransform(TransformOperations&& transform)
{
    return InterpolationValue(
        MakeGarbageCollected<InterpolableTransformList>(std::move(transform), TransformOperations::BoxSizeDependentMatrixBlending::kAllow));
}
...
class InheritedTransformChecker : public CSSInterpolationType::CSSConversionChecker {
public:
...
InterpolationValue CSSTransformInterpolationType::MaybeConvertInitial(const StyleResolverState& state, ConversionCheckers&) const
{
    return ConvertTransform(state.GetDocument().GetStyleResolver().InitialStyle().Transform());
}

InterpolationValue CSSTransformInterpolationType::MaybeConvertInherit(const StyleResolverState& state, ConversionCheckers& conversion_checkers) const
{
    const TransformOperations& inherited_transform = state.ParentStyle()->Transform();
    conversion_checkers.push_back(MakeGarbageCollected<InheritedTransformChecker>(inherited_transform));
    ...
    const CSSValue& value, const StyleResolverState* state, ConversionCheckers& conversion_checkers) const
{
    ...
    if (auto* list_value = DynamicTo<CSSValueList>(value)) {
        ...
        for (const CSSValue* item : *list_value) {
            ...
            if (transform_function.FunctionType() == CSSValueID::kMatrix || transform_function.FunctionType() == CSSValueID::kMatrix3d) {
            ...
            for (const CSSValue* argument : transform_function) {
                ...
                DCHECK(primitive_value || (transform_function.FunctionType() == CSSValueID::kPerspective && argument->IsIdentifierValue()));
                if (!primitive_value || (!primitive_value->IsLength() && !primitive_value->IsCalculatedPercentageWithLength())) {
                    continue;
                }
        ...
        if (InterpolationType::ConversionChecker* length_units_checker = LengthUnitsChecker::MaybeCreate(types, *state)) {
            conversion_checkers.push_back(length_units_checker);
        }
    }

    return InterpolationValue(
        InterpolableTransformList::ConvertCSSValue(value, state->CssToLengthConversionData(), TransformOperations::BoxSizeDependentMatrixBlending::kAllow));
}

InterpolationValue CSSTransformInterpolationType::PreInterpolationCompositeIfNeeded(
    InterpolationValue value, const InterpolationValue& underlying, EffectModel::CompositeOperation composite, ConversionCheckers& conversion_checkers) const
{
    ...
    InterpolableTransformList& transform_list = To<InterpolableTransformList>(*value.interpolable_value);
    const InterpolableTransformList& underlying_transform_list = To<InterpolableTransformList>(*underlying.interpolable_value);
...
InterpolationValue CSSTransformInterpolationType::MaybeConvertStandardPropertyUnderlyingValue(const ComputedStyle& style) const
{
...
{
    state.StyleBuilder().SetTransform(To<InterpolableTransformList>(interpolable_value).operations());
}
