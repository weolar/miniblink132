...
public:
    CSSInterpolationEnvironment(const InterpolationTypesMap& map, StyleResolverState& state, StyleCascade* cascade, CascadeResolver* cascade_resolver)
    ...
    CSSInterpolationEnvironment(const InterpolationTypesMap& map, StyleResolverState& state)
        : InterpolationEnvironment(map)
        , state_(&state)
    {
    }

    CSSInterpolationEnvironment(const InterpolationTypesMap& map, const ComputedStyle& base_style, const ComputedStyle& animation_controls_style)
        : InterpolationEnvironment(map)
    ...
    StyleResolverState* GetOptionalState()
    {
        return state_;
    }
    const StyleResolverState* GetOptionalState() const
    {
        return state_;
    }
...
template <> struct DowncastTraits<CSSInterpolationEnvironment> {
