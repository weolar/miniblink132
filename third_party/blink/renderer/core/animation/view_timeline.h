...
public:
    ...
    bool IsViewTimeline() const override
    {
        return true;
    }
...
protected:
    void CalculateOffsets(PaintLayerScrollableArea* scrollable_area, ScrollOrientation physical_orientation, TimelineState* state) const override;
...
private:
    ...
    void ApplyStickyAdjustments(ScrollOffsets& scroll_offsets, ViewOffsets& view_offsets, double viewport_size, double target_size, double target_offset,
        ScrollOrientation orientation, LayoutBox* scroll_container) const;
...
template <> struct DowncastTraits<ViewTimeline> {
