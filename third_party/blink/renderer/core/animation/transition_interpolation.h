...
public:
    TransitionInterpolation(const PropertyHandle& property, const InterpolationType& type, InterpolationValue&& start, InterpolationValue&& end,
        CompositorKeyframeValue* compositor_start, CompositorKeyframeValue* compositor_end)
        : property_(property)
        , type_(type)
        ...
        , compositor_end_(compositor_end)
    {
        ...
        DCHECK_EQ(compositor_start_ && compositor_end_,
            ...
            property_.GetCSSProperty().IsCompositableProperty() && CompositorAnimations::CompositedPropertyRequiresSnapshot(property_));
    }

    void Apply(InterpolationEnvironment&) const;

    bool IsTransitionInterpolation() const final
    {
        return true;
    }
...
template <> struct DowncastTraits<TransitionInterpolation> {
