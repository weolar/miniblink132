...
const FillLayer* GetFillLayerForPosition(const CSSProperty& property, const ComputedStyle& style)
{
...
FillLayer* AccessFillLayerForPosition(const CSSProperty& property, ComputedStyleBuilder& builder)
{
    switch (property.PropertyID()) {
    case CSSPropertyID::kBackgroundPositionX:
    case CSSPropertyID::kBackgroundPositionY:
...
Length::ValueRange LengthListPropertyFunctions::GetValueRange(const CSSProperty& property)
{
    switch (property.PropertyID()) {
    case CSSPropertyID::kBackgroundPositionX:
    case CSSPropertyID::kBackgroundPositionY:
...
bool LengthListPropertyFunctions::GetInitialLengthList(const CSSProperty& property, const ComputedStyle& initial_style, Vector<Length>& result)
{
...
static bool AppendToVector(const TransformOrigin& transform_origin, Vector<Length>& result)
{
...
bool LengthListPropertyFunctions::GetLengthList(const CSSProperty& property, const ComputedStyle& style, Vector<Length>& result)
{
    DCHECK(result.empty());
...
void LengthListPropertyFunctions::SetLengthList(const CSSProperty& property, ComputedStyleBuilder& builder, Vector<Length>&& length_list)
{
    switch (property.PropertyID()) {
    case CSSPropertyID::kStrokeDasharray:
        builder.SetStrokeDashArray(length_list.empty() ? nullptr : base::MakeRefCounted<SVGDashArray>(std::move(length_list)));
        return;
