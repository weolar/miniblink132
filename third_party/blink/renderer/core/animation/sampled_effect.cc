...
void SampledEffect::RemoveReplacedInterpolations(const HashSet<PropertyHandle>& replaced_properties)
{
    auto new_end = std::remove_if(
        interpolations_.begin(), interpolations_.end(), [&](const auto& interpolation) { return replaced_properties.Contains(interpolation->GetProperty()); });
    interpolations_.Shrink(static_cast<wtf_size_t>(new_end - interpolations_.begin()));
}

void SampledEffect::UpdateReplacedProperties(HashSet<PropertyHandle>& replaced_properties)
{
