...
AtomicString ColorSchemeToString(mojom::blink::PreferredColorScheme colorScheme)
{
    switch (colorScheme) {
...
PreferenceObject::PreferenceObject(ExecutionContext* executionContext, AtomicString name)
    : ExecutionContextLifecycleObserver(executionContext)
    , name_(name)
{
...
std::optional<AtomicString> PreferenceObject::override(ScriptState* script_state)
{
    ...
    const PreferenceOverrides* overrides = window->GetFrame()->GetPage()->GetPreferenceOverrides();
    ...
    if (name_ == preference_names::kColorScheme) {
        std::optional<mojom::blink::PreferredColorScheme> color_scheme = overrides->GetPreferredColorScheme();
        ...
        std::optional<mojom::blink::PreferredContrast> contrast = overrides->GetPreferredContrast();
        ...
        return std::make_optional(reduced_motion.value() ? preference_values::kReduce : preference_values::kNoPreference);
    } else if (name_ == preference_names::kReducedTransparency) {
        std::optional<bool> reduced_transparency = overrides->GetPrefersReducedTransparency();
        if (!reduced_transparency.has_value()) {
            return std::nullopt;
        }

        return std::make_optional(reduced_transparency.value() ? preference_values::kReduce : preference_values::kNoPreference);
        ...
        return std::make_optional(reduced_data ? preference_values::kReduce : preference_values::kNoPreference);
    } else {
        NOTREACHED();
    }
}
...
AtomicString PreferenceObject::value(ScriptState* script_state)
{
    ...
    if (name_ == preference_names::kColorScheme) {
        ...
        return prefers_reduced_motion_ ? preference_values::kReduce : preference_values::kNoPreference;
    } else if (name_ == preference_names::kReducedTransparency) {
        return prefers_reduced_transparency_ ? preference_values::kReduce : preference_values::kNoPreference;
    } else if (name_ == preference_names::kReducedData) {
        return prefers_reduced_data_ ? preference_values::kReduce : preference_values::kNoPreference;
    } else {
        NOTREACHED();
    }
}
...
void PreferenceObject::clearOverride(ScriptState* script_state)
{
    ...
    if (name_ == preference_names::kColorScheme) {
        ...
        window->GetFrame()->GetPage()->SetPreferenceOverride(media_feature_names::kPrefersColorSchemeMediaFeature, String());
        value_unchanged = (color_scheme.value() == media_values_->GetPreferredColorScheme());
    } else if (name_ == preference_names::kContrast) {
        std::optional<mojom::blink::PreferredContrast> contrast = overrides->GetPreferredContrast();
        if (!contrast.has_value()) {
            return;
        ...
        window->GetFrame()->GetPage()->SetPreferenceOverride(media_feature_names::kPrefersContrastMediaFeature, String());
        value_unchanged = (contrast.value() == media_values_->GetPreferredContrast());
    } else if (name_ == preference_names::kReducedMotion) {
        std::optional<bool> reduced_motion = overrides->GetPrefersReducedMotion();
        if (!reduced_motion.has_value()) {
            return;
        ...
        window->GetFrame()->GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedMotionMediaFeature, String());
        value_unchanged = (reduced_motion.value() == media_values_->PrefersReducedMotion());
    } else if (name_ == preference_names::kReducedTransparency) {
        std::optional<bool> reduced_transparency = overrides->GetPrefersReducedTransparency();
        if (!reduced_transparency.has_value()) {
            return;
        ...
        window->GetFrame()->GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedTransparencyMediaFeature, String());
        value_unchanged = (reduced_transparency.value() == media_values_->PrefersReducedTransparency());
    } else if (name_ == preference_names::kReducedData) {
        std::optional<bool> reduced_data = overrides->GetPrefersReducedData();
        if (!reduced_data.has_value()) {
            return;
        ...
        window->GetFrame()->GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedDataMediaFeature, String());
        value_unchanged = (reduced_data.value() == media_values_->PrefersReducedData());
    } else {
        NOTREACHED();
    }
    if (value_unchanged) {
...
ScriptPromise<IDLUndefined> PreferenceObject::requestOverride(ScriptState* script_state, std::optional<AtomicString> value)
{
    CHECK(RuntimeEnabledFeatures::WebPreferencesEnabled());
    if (!script_state || !script_state->ContextIsValid()) {
        return EmptyPromise();
    ...
    if (name_ == preference_names::kColorScheme) {
        ...
        if (overrides) {
            ...
            if (override.has_value()) {
                has_existing_override = true;
                if ((new_value == preference_values::kReduce && override.value()) || (new_value == preference_values::kNoPreference && !override.value())) {
                    value_same_as_existing_override = true;
                }
            }
        }
        existing_value = prefers_reduced_motion_ ? preference_values::kReduce : preference_values::kNoPreference;
    } else if (name_ == preference_names::kReducedTransparency) {
        ...
        existing_value = prefers_reduced_transparency_ ? preference_values::kReduce : preference_values::kNoPreference;
    } else if (name_ == preference_names::kReducedData) {
        ...
        existing_value = prefers_reduced_data_ ? preference_values::kReduce : preference_values::kNoPreference;
    } else {
        NOTREACHED();
    }
    ...
    if (new_value.empty()) {
        return ScriptPromise<IDLUndefined>::RejectWithDOMException(
            script_state, MakeGarbageCollected<DOMException>(DOMExceptionCode::kTypeMismatchError, value.value() + " is not a valid value."));
    }

    if (!value_same_as_existing_override) {
        window->GetFrame()->GetPage()->SetPreferenceOverride(feature_name, new_value.GetString());
    }
...
const FrozenArray<IDLString>& PreferenceObject::validValues()
{
    ...
    valid_values_ = MakeGarbageCollected<FrozenArray<IDLString>>(std::move(valid_values));
    return *valid_values_.Get();
}
...
void PreferenceObject::PreferenceMaybeChanged()
{
    ...
    if (name_ == preference_names::kColorScheme) {
        ...
        if (prefers_reduced_transparency_ == media_values_->PrefersReducedTransparency()) {
            return;
        }
    } else if (name_ == preference_names::kReducedData) {
