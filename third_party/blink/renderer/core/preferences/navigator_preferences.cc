...
{
    NavigatorPreferences* supplement = Supplement<Navigator>::From<NavigatorPreferences>(navigator);
    if (!supplement) {
...
{
    preference_manager_ = MakeGarbageCollected<PreferenceManager>(navigator.GetExecutionContext());
}
