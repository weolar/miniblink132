...
class CORE_EXPORT ImageBitmap final : public ScriptWrappable, public CanvasImageSource, public ImageBitmapSource {
    DEFINE_WRAPPERTYPEINFO();

public:
    // Expects the ImageElementBase to return/have an SVGImage.
    static ScriptPromise<ImageBitmap> CreateAsync(ImageElementBase*, std::optional<gfx::Rect>, ScriptState*,
        scoped_refptr<base::SequencedTaskRunner> task_runner, mojom::blink::PreferredColorScheme, ExceptionState&,
        const ImageBitmapOptions* = ImageBitmapOptions::Create());
    static sk_sp<SkImage> GetSkImageFromDecoder(std::unique_ptr<ImageDecoder>);

    ImageBitmap(ImageElementBase*, std::optional<gfx::Rect>, const ImageBitmapOptions* = ImageBitmapOptions::Create());
    ImageBitmap(HTMLVideoElement*, std::optional<gfx::Rect>, const ImageBitmapOptions* = ImageBitmapOptions::Create());
    ImageBitmap(HTMLCanvasElement*, std::optional<gfx::Rect>, const ImageBitmapOptions* = ImageBitmapOptions::Create());
    ImageBitmap(OffscreenCanvas*, std::optional<gfx::Rect>, const ImageBitmapOptions* = ImageBitmapOptions::Create());
    ImageBitmap(ImageData*, std::optional<gfx::Rect>, const ImageBitmapOptions* = ImageBitmapOptions::Create());
    ImageBitmap(ImageBitmap*, std::optional<gfx::Rect>, const ImageBitmapOptions* = ImageBitmapOptions::Create());
    ImageBitmap(scoped_refptr<StaticBitmapImage>);
    ImageBitmap(scoped_refptr<StaticBitmapImage>, std::optional<gfx::Rect>, const ImageBitmapOptions* = ImageBitmapOptions::Create());
    ...
    // origin clean or not.
    ImageBitmap(const SkPixmap& pixmap, bool is_image_bitmap_origin_clean, ImageOrientationEnum);
    ...
    scoped_refptr<StaticBitmapImage> BitmapImage() const
    {
        return image_;
    }
    ...
    Vector<uint8_t> CopyBitmapData(const SkImageInfo& info, bool apply_orientation);
    unsigned width() const;
    unsigned height() const;
    gfx::Size Size() const;

    bool IsNeutered() const override
    {
        return is_neutered_;
    }
    ...
    bool IsImageBitmap() const override
    {
        return true;
    }
    ...
    ScriptPromise<ImageBitmap> CreateImageBitmap(ScriptState*, std::optional<gfx::Rect>, const ImageBitmapOptions*, ExceptionState&) override;
    ...
    struct ParsedOptions {
        ...
        gfx::Rect crop_rect;
        cc::PaintFlags::FilterQuality resize_quality = cc::PaintFlags::FilterQuality::kLow;
        ...
        // output size.
        void ComputeSubsetParameters(SkIRect& source_skrect, SkIRect& source_skrect_valid, SkISize& dest_sksize) const;
    };

private:
    void UpdateImageBitmapMemoryUsage();
    static void ResolvePromiseOnOriginalThread(
        ScriptPromiseResolver<ImageBitmap>*, bool origin_clean, std::unique_ptr<ParsedOptions>, sk_sp<SkImage>, const ImageOrientationEnum);
    static void RasterizeImageOnBackgroundThread(PaintRecord, const gfx::Rect&, scoped_refptr<base::SequencedTaskRunner>,
