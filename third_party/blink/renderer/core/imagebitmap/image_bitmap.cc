...
gfx::Size ParseDstSize(const ImageBitmapOptions* options, const gfx::Rect& src_rect)
{
    int resize_width = 0;
    int resize_height = 0;
    if (!options->hasResizeWidth() && !options->hasResizeHeight()) {
        ...
        resize_width = options->resizeWidth();
        resize_height = ClampTo<unsigned>(ceil(static_cast<float>(options->resizeWidth()) / src_rect.width() * src_rect.height()));
    } else {
        resize_height = options->resizeHeight();
        resize_width = ClampTo<unsigned>(ceil(static_cast<float>(options->resizeHeight()) / src_rect.height() * src_rect.width()));
    }
    return gfx::Size(resize_width, resize_height);
}

ImageBitmap::ParsedOptions ParseOptions(
    const ImageBitmapOptions* options, std::optional<gfx::Rect> crop_rect, gfx::Size source_size, ImageOrientation source_orientation, bool source_is_unpremul)
{
    ...
    if (options->imageOrientation() == kImageOrientationFlipY) {
        ...
        DCHECK(options->imageOrientation() == kImageOrientationFromImage || options->imageOrientation() == kImageBitmapOptionNone);
        parsed_options.flip_y = false;
        parsed_options.orientation_from_image = true;
        parsed_options.source_orientation = source_orientation;
        if (base::FeatureList::IsEnabled(features::kCreateImageBitmapOrientationNone) && options->imageOrientation() == kImageBitmapOptionNone) {
    ...
    if (options->premultiplyAlpha() == kImageBitmapOptionNone) {
        ...
        DCHECK(options->premultiplyAlpha() == kImageBitmapOptionDefault || options->premultiplyAlpha() == kImageBitmapOptionPremultiply);
    }

    parsed_options.has_color_space_conversion = (options->colorSpaceConversion() != kImageBitmapOptionNone);
    if (options->colorSpaceConversion() != kImageBitmapOptionNone && options->colorSpaceConversion() != kImageBitmapOptionDefault) {
        NOTREACHED() << "Invalid ImageBitmap creation attribute colorSpaceConversion: " << IDLEnumAsString(options->colorSpaceConversion());
    }

    parsed_options.source_size = parsed_options.source_orientation.UsesWidthAsHeight() ? gfx::TransposeSize(source_size) : source_size;
    if (!crop_rect) {
    ...
    if (static_cast<int>(parsed_options.resize_width) == parsed_options.crop_rect.width()
        && static_cast<int>(parsed_options.resize_height) == parsed_options.crop_rect.height()) {
    ...
    parsed_options.sampling = cc::PaintFlags::FilterQualityToSkSamplingOptions(parsed_options.resize_quality);
    return parsed_options;
}

ImageBitmap::ParsedOptions ParseOptions(const ImageBitmapOptions* options, std::optional<gfx::Rect> crop_rect, scoped_refptr<Image> input)
{
    const auto info = input->PaintImageForCurrentFrame().GetSkImageInfo();
    return ParseOptions(
        options, crop_rect, gfx::Size(info.width(), info.height()), input->CurrentFrameOrientation(), info.alphaType() == kUnpremul_SkAlphaType);
}

ImageBitmap::ParsedOptions ParseOptions(const ImageBitmapOptions* options, std::optional<gfx::Rect> crop_rect, scoped_refptr<StaticBitmapImage> input)
{
...
scoped_refptr<StaticBitmapImage> ApplyTransformsFromOptions(
    scoped_refptr<StaticBitmapImage> source, const ImageBitmap::ParsedOptions& options, bool force_copy = false)
{
    ...
    return StaticBitmapImageTransform::Apply(FlushReason::kCreateImageBitmap, source, params);
}

scoped_refptr<StaticBitmapImage> MakeBlankImage(const ImageBitmap::ParsedOptions& parsed_options)
{
    SkImageInfo info = SkImageInfo::Make(
        parsed_options.crop_rect.width(), parsed_options.crop_rect.height(), kN32_SkColorType, kPremul_SkAlphaType, SkColorSpace::MakeSRGB());
    if (parsed_options.should_scale_input) {
        info = info.makeWH(parsed_options.resize_width, parsed_options.resize_height);
    }
...
ImageBitmap::ImageBitmap(ImageElementBase* image, std::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options)
{
    ...
    if (input->IsBitmapImage()) {
        ...
        std::unique_ptr<ImageDecoder> decoder(ImageDecoder::Create(input->Data(), data_complete,
            parsed_options.premultiply_alpha ? ImageDecoder::kAlphaPremultiplied : ImageDecoder::kAlphaNotPremultiplied,
            paint_image.GetColorType() == kRGBA_F16_SkColorType ? ImageDecoder::kHighBitDepthToHalfFloat : ImageDecoder::kDefaultBitDepth,
            parsed_options.has_color_space_conversion ? ColorBehavior::kTag : ColorBehavior::kIgnore, cc::AuxImage::kDefault,
        ...
        paint_image = PaintImageBuilder::WithDefault()
                          ...
                          .set_image(std::move(skia_image), paint_image.GetContentIdForFrame(0u))
                          .TakePaintImage();

        // Update source alpha states after redecoding.
        parsed_options.source_is_unpremul = paint_image.GetAlphaType() == kUnpremul_SkAlphaType;
        ...
        paint_image = PaintImageBuilder::WithDefault()
                          .set_id(paint_image.stable_id())
                          .set_image(SkImages::RasterFromBitmap(bitmap), paint_image.GetContentIdForFrame(0u))
                          .TakePaintImage();
    }

    auto static_input = UnacceleratedStaticBitmapImage::Create(std::move(paint_image), input->CurrentFrameOrientation());
...
ImageBitmap::ImageBitmap(HTMLVideoElement* video, std::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options)
{
    ...
    const bool allow_accelerated_images = !options->hasResizeWidth() && !options->hasResizeHeight();
    const bool reinterpret_as_srgb = (options->colorSpaceConversion() == kImageBitmapOptionNone);
    auto input = video->CreateStaticBitmapImage(allow_accelerated_images, /*size=*/std::nullopt, reinterpret_as_srgb);
    if (!input)
        return;
...
ImageBitmap::ImageBitmap(HTMLCanvasElement* canvas, std::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options)
{
    SourceImageStatus status;
    scoped_refptr<Image> image_input = canvas->GetSourceImageForCanvas(FlushReason::kCreateImageBitmap, &status, gfx::SizeF(), kPremultiplyAlpha);
    if (status != kNormalSourceImageStatus)
        return;
    DCHECK(IsA<StaticBitmapImage>(image_input.get()));
    scoped_refptr<StaticBitmapImage> input = static_cast<StaticBitmapImage*>(image_input.get());
...
ImageBitmap::ImageBitmap(OffscreenCanvas* offscreen_canvas, std::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options)
{
    SourceImageStatus status;
    scoped_refptr<Image> raw_input = offscreen_canvas->GetSourceImageForCanvas(FlushReason::kCreateImageBitmap, &status, gfx::SizeF(offscreen_canvas->Size()));
    DCHECK(IsA<StaticBitmapImage>(raw_input.get()));
    scoped_refptr<StaticBitmapImage> input = static_cast<StaticBitmapImage*>(raw_input.get());
    raw_input = nullptr;
...
ImageBitmap::ImageBitmap(const SkPixmap& pixmap, bool is_image_bitmap_origin_clean, ImageOrientationEnum image_orientation)
{
...
ImageBitmap::ImageBitmap(ImageData* data, std::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options)
{
    const ParsedOptions parsed_options = ParseOptions(options, crop_rect, data->BitmapSourceSize(), ImageOrientationEnum::kOriginTopLeft,
    ...
    auto image = StaticBitmapImage::Create(sk_data, pm.info(), ImageOrientationEnum::kOriginTopLeft);

    // Force a copy of the data during the transformation (so that we do not
    // reference ImageData's mutable data).
    image_ = ApplyTransformsFromOptions(image, parsed_options, /*force_copy=*/true);
    if (!image_)
        return;
...
ImageBitmap::ImageBitmap(ImageBitmap* bitmap, std::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options)
{
...
ImageBitmap::ImageBitmap(scoped_refptr<StaticBitmapImage> image, std::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options)
{
...
scoped_refptr<StaticBitmapImage> ImageBitmap::Transfer()
{
    ...
    if (!image_->HasOneRef()) {
        ...
        // NotifyWillTransfer
        const auto content_id = image_->PaintImageForCurrentFrame().GetContentIdForFrame(0);
        ...
        // rare edge cases.
        if (!image_->HasOneRef()) {
            auto copy = StaticBitmapImageTransform::Clone(FlushReason::kCreateImageBitmap, image_);
            if (!copy) {
                return nullptr;
            }
...
void ImageBitmap::UpdateImageBitmapMemoryUsage()
{
    ...
    if (!is_neutered_ && image_) {
        ...
        new_memory_usage = memory_usage_checked.ValueOrDefault(std::numeric_limits<int32_t>::max());
    }

    external_memory_accounter_.Update(v8::Isolate::GetCurrent(), new_memory_usage - memory_usage_);
...
void ImageBitmap::ResolvePromiseOnOriginalThread(ScriptPromiseResolver<ImageBitmap>* resolver, bool origin_clean, std::unique_ptr<ParsedOptions> parsed_options,
    sk_sp<SkImage> skia_image, const ImageOrientationEnum orientation)
{
    if (!skia_image) {
        resolver->Reject(v8::Null(resolver->GetScriptState()->GetIsolate()));
        return;
    }
    scoped_refptr<StaticBitmapImage> image = UnacceleratedStaticBitmapImage::Create(std::move(skia_image), orientation);
    DCHECK(IsMainThread());
    if (!image) {
        resolver->Reject(v8::Null(resolver->GetScriptState()->GetIsolate()));
        return;
...
void ImageBitmap::RasterizeImageOnBackgroundThread(PaintRecord paint_record, const gfx::Rect& dst_rect, scoped_refptr<base::SequencedTaskRunner> task_runner,
    WTF::CrossThreadOnceFunction<void(sk_sp<SkImage>, const ImageOrientationEnum)> callback)
{
    DCHECK(!IsMainThread());
    SkImageInfo info = SkImageInfo::MakeN32Premul(dst_rect.width(), dst_rect.height());
    ...
    PostCrossThreadTask(*task_runner, FROM_HERE, CrossThreadBindOnce(std::move(callback), std::move(skia_image), ImageOrientationEnum::kDefault));
}

ScriptPromise<ImageBitmap> ImageBitmap::CreateAsync(ImageElementBase* image, std::optional<gfx::Rect> crop_rect, ScriptState* script_state,
    scoped_refptr<base::SequencedTaskRunner> task_runner, mojom::blink::PreferredColorScheme preferred_color_scheme, ExceptionState& exception_state,
    const ImageBitmapOptions* options)
{
    ...
    const ParsedOptions parsed_options = ParseOptions(options, crop_rect, input);
    if (DstBufferSizeHasOverflow(parsed_options)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The ImageBitmap could not be allocated.");
        return EmptyPromise();
    }
    ...
    if (!parsed_options.crop_rect.Intersects(input_rect)) {
        ImageBitmap* bitmap = MakeGarbageCollected<ImageBitmap>(MakeBlankImage(parsed_options));
    ...
    gfx::Rect draw_dst_rect(0, 0, parsed_options.resize_width, parsed_options.resize_height);
    ...
    if (!parsed_options.orientation_from_image && input->CurrentFrameOrientation() != ImageOrientationEnum::kOriginTopLeft) {
        auto affineTransform = input->CurrentFrameOrientation().TransformFromDefault(gfx::SizeF(draw_dst_rect.size()));
    ...
    SVGImageForContainer::Create(To<SVGImage>(*input), gfx::SizeF(input_rect.size()), 1, nullptr, preferred_color_scheme)
        ->Draw(canvas, cc::PaintFlags(), gfx::RectF(draw_dst_rect), gfx::RectF(draw_src_rect), ImageDrawOptions());
    PaintRecord paint_record = recorder.finishRecordingAsPicture();

    std::unique_ptr<ParsedOptions> passed_parsed_options = std::make_unique<ParsedOptions>(parsed_options);
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<ImageBitmap>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();

    worker_pool::PostTask(FROM_HERE,
        CrossThreadBindOnce(&RasterizeImageOnBackgroundThread, std::move(paint_record), draw_dst_rect, std::move(task_runner),
            CrossThreadBindOnce(
                &ResolvePromiseOnOriginalThread, MakeUnwrappingCrossThreadHandle(resolver), !image->WouldTaintOrigin(), std::move(passed_parsed_options))));
    return promise;
}
...
// static
ImageBitmap* ImageBitmap::Take(ScriptPromiseResolverBase*, sk_sp<SkImage> image)
{
    return MakeGarbageCollected<ImageBitmap>(UnacceleratedStaticBitmapImage::Create(std::move(image)));
}
...
Vector<uint8_t> ImageBitmap::CopyBitmapData(const SkImageInfo& info, bool apply_orientation)
{
...
gfx::SizeF ImageBitmap::ElementSize(const gfx::SizeF&, const RespectImageOrientationEnum respect_orientation) const
{
