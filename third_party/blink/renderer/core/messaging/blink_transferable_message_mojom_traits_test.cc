...
scoped_refptr<SerializedScriptValue> BuildSerializedScriptValue(v8::Isolate* isolate, v8::Local<v8::Value> value, Transferables& transferables)
{
    SerializedScriptValue::SerializeOptions options;
    options.transferables = &transferables;
    ExceptionState exceptionState(isolate, v8::ExceptionContext::kOperation, "MessageChannel", "postMessage");
    return SerializedScriptValue::Serialize(isolate, value, options, exceptionState);
}

TEST(BlinkTransferableMessageStructTraitsTest, ArrayBufferTransferOutOfScopeSucceeds)
{
    ...
    mojo::Message mojo_message;
    {
        ...
        v8::Local<v8::ArrayBuffer> v8_buffer = v8::ArrayBuffer::New(isolate, num_elements);
        ...
        DOMArrayBuffer* array_buffer = NativeValueTraits<DOMArrayBuffer>::NativeValue(isolate, v8_buffer, scope.GetExceptionState());
        ...
        msg.message = BuildSerializedScriptValue(scope.GetIsolate(), v8_buffer, transferables);
        mojo_message = mojom::blink::TransferableMessage::SerializeAsMessage(&msg);
    }

    BlinkTransferableMessage out;
    ASSERT_TRUE(mojom::blink::TransferableMessage::DeserializeFromMessage(std::move(mojo_message), &out));
    ASSERT_EQ(out.message->GetArrayBufferContentsArray().size(), 1U);
    ArrayBufferContents& deserialized_contents = out.message->GetArrayBufferContentsArray()[0];
    Vector<uint8_t> deserialized_data;
    deserialized_data.Append(static_cast<uint8_t*>(deserialized_contents.Data()), 8);
...
TEST(BlinkTransferableMessageStructTraitsTest, ArrayBufferContentsLazySerializationSucceeds)
{
    // More exhaustive tests in web_tests/. This is a sanity check.
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    ...
    DOMArrayBuffer* original_array_buffer = NativeValueTraits<DOMArrayBuffer>::NativeValue(isolate, v8_buffer, scope.GetExceptionState());
    Transferables transferables;
...
{
    sk_sp<SkSurface> surface = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(8, 4));
    surface->getCanvas()->clear(SK_ColorRED);
    return MakeGarbageCollected<ImageBitmap>(UnacceleratedStaticBitmapImage::Create(surface->makeImageSnapshot()));
}

TEST(BlinkTransferableMessageStructTraitsTest, BitmapTransferOutOfScopeSucceeds)
{
    // More exhaustive tests in web_tests/. This is a sanity check.
    ...
    mojo::Message mojo_message;
    {
        ...
        msg.sender_origin = SecurityOrigin::CreateUniqueOpaque();
        msg.sender_agent_cluster_id = base::UnguessableToken::Create();
        msg.message = BuildSerializedScriptValue(scope.GetIsolate(), wrapper, transferables);
        mojo_message = mojom::blink::TransferableMessage::SerializeAsMessage(&msg);
    };
...
TEST(BlinkTransferableMessageStructTraitsTest, BitmapLazySerializationSucceeds)
{
    // More exhaustive tests in web_tests/. This is a sanity check.
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    ...
    scoped_refptr<SharedBuffer> original_bitmap_data = original_bitmap->BitmapImage()->Data();
    v8::Local<v8::Value> wrapper = ToV8Traits<ImageBitmap>::ToV8(scope.GetScriptState(), original_bitmap);
    Transferables transferables;
    ...
    ASSERT_TRUE(mojom::blink::TransferableMessage::DeserializeFromMessage(std::move(mojo_message), &out));
    ASSERT_EQ(out.message->GetImageBitmapContentsArray().size(), 1U);
    scoped_refptr<blink::StaticBitmapImage> deserialized_bitmap_contents = out.message->GetImageBitmapContentsArray()[0];
    auto* deserialized_bitmap = MakeGarbageCollected<ImageBitmap>(std::move(deserialized_bitmap_contents));
...
public:
    ...
    {
        gpu::SyncToken token;
        token.Set(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(64), id);
    ...
    ImageBitmap* CreateAcceleratedStaticImageBitmap()
    {
        ...
        return MakeGarbageCollected<ImageBitmap>(AcceleratedStaticBitmapImage::CreateFromCanvasSharedImage(std::move(client_si), GenTestSyncToken(100), 0,
            SkImageInfo::MakeN32Premul(100, 100), GL_TEXTURE_2D, true, SharedGpuContext::ContextProviderWrapper(), base::PlatformThread::CurrentRef(),
            base::MakeRefCounted<base::NullTaskRunner>(),
            WTF::BindOnce(&BlinkTransferableMessageStructTraitsWithFakeGpuTest::OnImageDestroyed, WTF::Unretained(this)),
...
TEST_F(BlinkTransferableMessageStructTraitsWithFakeGpuTest, AcceleratedImageTransferSuccess)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scope.GetExecutionContext()->GetTaskRunner(TaskType::kInternalTest)->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
        ...
        msg.sender_agent_cluster_id = base::UnguessableToken::Create();
        msg.message = BuildSerializedScriptValue(scope.GetIsolate(), wrapper, transferables);
        mojo::Message mojo_message = mojom::blink::TransferableMessage::SerializeAsMessage(&msg);
...
TEST_F(BlinkTransferableMessageStructTraitsWithFakeGpuTest, AcceleratedImageTransferReceiverCrash)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scope.GetExecutionContext()->GetTaskRunner(TaskType::kInternalTest)->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
