...
DOMHighResTimeStamp ComputeIntersectionsContext::GetTimeStamp(const IntersectionObserver& observer)
{
    ...
    DOMHighResTimeStamp timestamp = DOMWindowPerformance::performance(To<LocalDOMWindow>(*context))->MonotonicTimeToDOMHighResTimeStamp(GetMonotonicTime());
...
std::optional<IntersectionGeometry::RootGeometry>& ComputeIntersectionsContext::GetRootGeometry(const IntersectionObserver& observer, unsigned flags)
{
    if (observer.RootIsImplicit()) {
        if (&observer != implicit_root_geometry_observer_) {
            implicit_root_geometry_observer_ = &observer;
            if (implicit_root_geometry_) {
                implicit_root_geometry_->UpdateMargin(flags & IntersectionGeometry::kShouldReportRootBounds ? observer.RootMargin() : Vector<Length>());
            }
        }
...
IntersectionObserverController::IntersectionObserverController(ExecutionContext* context)
...
void IntersectionObserverController::PostTaskToDeliverNotifications()
{
    ...
    GetExecutionContext()
        ...
        ->PostTask(FROM_HERE,
            WTF::BindOnce(&IntersectionObserverController::DeliverNotifications, WrapWeakPersistent(this), IntersectionObserver::kPostTaskToDeliver));
}
...
void IntersectionObserverController::ScheduleIntersectionObserverForDelivery(IntersectionObserver& observer)
{
    ...
    if (observer.GetDeliveryBehavior() == IntersectionObserver::kPostTaskToDeliver)
        PostTaskToDeliverNotifications();
}

void IntersectionObserverController::DeliverNotifications(IntersectionObserver::DeliveryBehavior behavior)
{
...
bool IntersectionObserverController::ComputeIntersections(
    unsigned flags, LocalFrameView& frame_view, gfx::Vector2dF accumulated_scroll_delta_since_last_update, ComputeIntersectionsContext& context)
{
    ...
    auto compute_observer_intersections = [&](IntersectionObserver& observer, const auto& observations) {
        ...
        for (auto& observation : observations) {
            count += observation->ComputeIntersection(flags, accumulated_scroll_delta_since_last_update, context);
        }
    ...
    if (metrics_aggregator) {
        metrics_aggregator->RecordCountSample(LocalFrameUkmAggregator::kIntersectionObservationInternalCount, internal_observation_count);
        metrics_aggregator->RecordCountSample(LocalFrameUkmAggregator::kIntersectionObservationJavascriptCount, javascript_observation_count);
    }
...
void IntersectionObserverController::AddTrackedObservation(IntersectionObservation& observation)
{
    ...
    if (!observer->RootIsImplicit())
        return;
    tracked_implicit_root_observations_.insert(observer, HeapHashSet<Member<IntersectionObservation>>()).stored_value->value.insert(&observation);
    if (observer->trackVisibility()) {
        needs_occlusion_tracking_ = true;
...
wtf_size_t IntersectionObserverController::GetTrackedObservationCountForTesting() const
{
    wtf_size_t count = 0;
