...
class CORE_EXPORT IntersectionObserver final : public ScriptWrappable, public ActiveScriptWrappable<IntersectionObserver>, public ExecutionContextClient {
    DEFINE_WRAPPERTYPEINFO();

public:
    using EventCallback = base::RepeatingCallback<void(const HeapVector<Member<IntersectionObserverEntry>>&)>;
    ...
    // number of pixels.
    static constexpr float kMinimumThreshold = IntersectionGeometry::kMinimumThreshold;
    ...
    static IntersectionObserver* Create(const IntersectionObserverInit*, IntersectionObserverDelegate&,
        std::optional<LocalFrameUkmAggregator::MetricId> ukm_metric_id, ExceptionState& = ASSERT_NO_EXCEPTION);
    static IntersectionObserver* Create(ScriptState*, V8IntersectionObserverCallback*, const IntersectionObserverInit*, ExceptionState& = ASSERT_NO_EXCEPTION);
    ...
    IntersectionObserver(IntersectionObserverDelegate& delegate, std::optional<LocalFrameUkmAggregator::MetricId> ukm_metric_id, Params&& params);
    ...
    HeapVector<Member<IntersectionObserverEntry>> takeRecords(ExceptionState& = ASSERT_NO_EXCEPTION);
    ...
    {
        return delay_.InMilliseconds();
    }
    bool trackVisibility() const
    {
        return track_visibility_;
    }
    bool trackFractionOfRoot() const
    {
        return track_fraction_of_root_;
    }
    ...
    bool AlwaysReportRootBounds() const
    {
        return always_report_root_bounds_;
    }
    ...
    Vector<Length> ScrollMargin() const
    {
        return scroll_margin_;
    }
    ...
    bool UseOverflowClipEdge() const
    {
        return use_overflow_clip_edge_ == 1;
    }
