...
public:
    ...
    static constexpr gfx::Vector2dF kInfiniteScrollDelta { std::numeric_limits<float>::max(), std::numeric_limits<float>::max() };
    ...
    static const LayoutObject* GetTargetLayoutObject(const Element& target_element);
    ...
    // first call.
    IntersectionGeometry(const Node* root, const Element& target, const Vector<Length>& root_margin, const Vector<float>& thresholds,
        const Vector<Length>& target_margin, const Vector<Length>& scroll_margin, unsigned flags, std::optional<RootGeometry>& root_geometry,
    ...
    gfx::RectF TargetRect() const
    {
        return target_rect_;
    }
    gfx::RectF IntersectionRect() const
    {
    ...
    wtf_size_t ThresholdIndex() const
    {
        return threshold_index_;
    }

    bool DidComputeGeometry() const
    {
        return flags_ & kDidComputeGeometry;
    }
    bool IsIntersecting() const
    {
        return threshold_index_ > 0;
    }
    bool IsVisible() const
    {
        return flags_ & kIsVisible;
    }
    DOMNodeId occluder_node_id() const
    {
        return occluder_node_id_;
    }

    bool CanUseCachedRectsForTesting() const
    {
        return ShouldUseCachedRects();
    }

private:
    bool RootIsImplicit() const
    {
        return flags_ & kRootIsImplicit;
    }
    bool ShouldUseCachedRects() const
    {
        return flags_ & kShouldUseCachedRects;
    }
    bool ShouldRespectFilters() const
    {
        return flags_ & kRespectFilters;
    }
    ...
    public:
        RootAndTarget(const Node* root_node, const Element& target_element, bool has_target_margin, bool has_scroll_margin);
    ...
    private:
        ...
        void ComputeRelationship(bool root_is_implicit, bool has_target_margin, bool has_scroll_margin);
    };

    void UpdateShouldUseCachedRects(const RootAndTarget& root_and_target, CachedRects* cached_rects);

    void ComputeGeometry(const RootGeometry& root_geometry, const RootAndTarget& root_and_target, const Vector<float>& thresholds,
    ...
    bool ClipToRoot(const RootAndTarget& root_and_target, const gfx::RectF& root_rect, gfx::RectF& unclipped_intersection_rect, gfx::RectF& intersection_rect,
        const Vector<Length>& scroll_margin, CachedRects* cached_rects);
    bool ApplyClip(const LayoutObject* target, const LayoutBox* local_ancestor, const LayoutObject* root, const gfx::RectF& root_rect,
    ...
    unsigned FirstThresholdGreaterThan(float ratio, const Vector<float>& thresholds) const;

    gfx::Vector2dF ComputeMinScrollDeltaToUpdate(const RootAndTarget& root_and_target, const gfx::Transform& target_to_view_transform,
