...
int64_t IntersectionObservation::ComputeIntersection(
    unsigned compute_flags, gfx::Vector2dF accumulated_scroll_delta_since_last_update, ComputeIntersectionsContext& context)
{
    DCHECK(Observer());
    cached_rects_.min_scroll_delta_to_update -= accumulated_scroll_delta_since_last_update;
    ...
    bool is_post_layout_delivery_observer = Observer()->GetDeliveryBehavior() == IntersectionObserver::kDeliverDuringPostLayoutSteps;
    if (post_layout_delivery_only != is_post_layout_delivery_observer) {
        return 0;
    }

    bool has_pending_update = needs_update_;
    if (compute_flags & (observer_->RootIsImplicit() ? kImplicitRootObserversNeedUpdate : kExplicitRootObserversNeedUpdate)) {
        needs_update_ = true;
    }
...
#endif
    if (!has_pending_update && (compute_flags & kScrollAndVisibilityOnly) && cached_rects_.min_scroll_delta_to_update.x() > 0
    ...
    bool honor_margins = geometry_flags & IntersectionGeometry::kShouldReportRootBounds;
    Vector<Length> empty_margin;
    IntersectionGeometry geometry(observer_->root(), *Target(), honor_margins ? observer_->RootMargin() : empty_margin, observer_->thresholds(),
        honor_margins ? observer_->TargetMargin() : empty_margin, honor_margins ? observer_->ScrollMargin() : empty_margin, geometry_flags,
...
void IntersectionObservation::ComputeIntersectionImmediately(ComputeIntersectionsContext& context)
{
    ComputeIntersection(
        kImplicitRootObserversNeedUpdate | kExplicitRootObserversNeedUpdate | kIgnoreDelay, IntersectionGeometry::kInfiniteScrollDelta, context);
}
...
void IntersectionObservation::TakeRecords(HeapVector<Member<IntersectionObserverEntry>>& entries)
{
...
void IntersectionObservation::Disconnect()
{
    ...
    if (target_) {
        ...
        ElementIntersectionObserverData* observer_data = target_->IntersectionObserverData();
        observer_data->RemoveObservation(*this);
        if (target_->isConnected()) {
            IntersectionObserverController* controller = target_->GetDocument().GetIntersectionObserverController();
            if (controller)
...
bool IntersectionObservation::CanUseCachedRectsForTesting(bool scroll_and_visibility_only) const
{
    ...
    IntersectionGeometry geometry(observer_->root(), *target_,
        ...
        /* thresholds */ { 0 },
        /* target_margin */ {},
        /* scroll_margin */ {}, scroll_and_visibility_only ? IntersectionGeometry::kScrollAndVisibilityOnly : 0, root_geometry, &cached_rects_copy);
...
bool IntersectionObservation::ShouldCompute(unsigned flags) const
{
    ...
    if (target_->isConnected() && target_->GetDocument().GetFrame() && Observer()->trackVisibility()) {
        mojom::blink::FrameOcclusionState occlusion_state = target_->GetDocument().GetFrame()->GetOcclusionState();
        ...
        if (!last_is_visible_ && occlusion_state == mojom::blink::FrameOcclusionState::kUnknown) {
            return false;
        }
    }
    return true;
...
bool IntersectionObservation::MaybeDelayAndReschedule(unsigned flags, ComputeIntersectionsContext& context)
{
    ...
    base::TimeDelta delay = observer_->GetEffectiveDelay() - (context.GetMonotonicTime() - last_run_time_);
...
unsigned IntersectionObservation::GetIntersectionGeometryFlags(unsigned compute_flags) const
{
    bool report_root_bounds = observer_->AlwaysReportRootBounds() || (compute_flags & kReportImplicitRootBounds) || !observer_->RootIsImplicit();
...
void IntersectionObservation::ProcessIntersectionGeometry(const IntersectionGeometry& geometry, ComputeIntersectionsContext& context)
{
    CHECK_LT(geometry.ThresholdIndex(), kNotFound);

    if (last_threshold_index_ != geometry.ThresholdIndex() || last_is_visible_ != geometry.IsVisible()) {
        entries_.push_back(MakeGarbageCollected<IntersectionObserverEntry>(geometry, context.GetTimeStamp(*Observer()), Target()));
