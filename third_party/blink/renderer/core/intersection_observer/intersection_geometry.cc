...
LayoutUnit ComputeMargin(const Length& length, float reference_length, float zoom)
{
    if (length.IsPercent()) {
        return LayoutUnit(static_cast<int>(reference_length * length.Percent() / 100.0));
    }
    DCHECK(length.IsFixed());
    return LayoutUnit(length.Value() * zoom);
}

PhysicalBoxStrut ResolveMargin(const Vector<Length>& margin, const gfx::SizeF& reference_size, float zoom)
{
    DCHECK_EQ(margin.size(), 4u);

    return PhysicalBoxStrut(ComputeMargin(margin[0], reference_size.height(), zoom), ComputeMargin(margin[1], reference_size.width(), zoom),
        ComputeMargin(margin[2], reference_size.height(), zoom), ComputeMargin(margin[3], reference_size.width(), zoom));
}

// Expand rect by the given margin values.
void ApplyMargin(gfx::RectF& expand_rect, const Vector<Length>& margin, float zoom, const gfx::SizeF& reference_size)
{
    if (margin.empty()) {
        return;
    }
    expand_rect.Outset(gfx::OutsetsF(ResolveMargin(margin, reference_size, zoom)));
}
...
gfx::RectF InitializeTargetRect(const LayoutObject* target, unsigned flags)
{
    ...
    if (auto* layout_box = DynamicTo<LayoutBox>(target)) {
        return GetBoxBounds(layout_box, flags & IntersectionGeometry::kUseOverflowClipEdge);
    }
...
VisibilityInfo ComputeVisibilityInfo(const LayoutObject* target, const PhysicalRect& rect, unsigned flags)
{
    if (!target->GetDocument().GetFrame()
        || target->GetDocument().GetFrame()->LocalFrameRoot().GetOcclusionState() != mojom::blink::FrameOcclusionState::kGuaranteedNotOccluded) {
    ...
    bool should_expose_occluder_id = flags & IntersectionGeometry::kShouldExposeOccluderNodeId;
    ...
    if (target->IsLayoutInline()) {
        ...
        return { is_visible, (!is_visible && should_expose_occluder_id) ? hit_node->GetDomNodeId() : kInvalidDOMNodeId };
    }
    return { false, should_expose_occluder_id ? hit_node->GetDomNodeId() : kInvalidDOMNodeId };
}
...
gfx::Transform ObjectToViewTransform(const LayoutObject& object)
{
    ...
    PropertyTreeStateOrAlias container_properties(PropertyTreeState::kUninitialized);
    const LayoutObject* property_container
        = IntersectionGeometry::CanUseGeometryMapper(object) ? object.GetPropertyContainer(nullptr, &container_properties) : nullptr;
    if (property_container) {
        gfx::Transform transform = GeometryMapper::SourceToDestinationProjection(
            container_properties.Transform(), object.View()->FirstFragment().LocalBorderBoxProperties().Transform());
...
static const unsigned kConstructorFlagsMask = IntersectionGeometry::kShouldReportRootBounds | IntersectionGeometry::kShouldComputeVisibility
    | IntersectionGeometry::kShouldTrackFractionOfRoot | IntersectionGeometry::kForFrameViewportIntersection | IntersectionGeometry::kShouldConvertToCSSPixels
    | IntersectionGeometry::kUseOverflowClipEdge | IntersectionGeometry::kRespectFilters | IntersectionGeometry::kScrollAndVisibilityOnly
    | IntersectionGeometry::kShouldExposeOccluderNodeId;

} // namespace

IntersectionGeometry::RootGeometry::RootGeometry(const LayoutObject* root, const Vector<Length>& margin)
{
...
void IntersectionGeometry::RootGeometry::UpdateMargin(const Vector<Length>& margin)
{
    local_root_rect = pre_margin_local_root_rect;
    ApplyMargin(local_root_rect, margin, zoom, pre_margin_local_root_rect.size());
}

bool IntersectionGeometry::RootGeometry::operator==(const RootGeometry& other) const
{
    return zoom == other.zoom && local_root_rect == other.local_root_rect && root_to_view_transform == other.root_to_view_transform;
}

const LayoutObject* IntersectionGeometry::GetExplicitRootLayoutObject(const Node& root_node)
{
...
IntersectionGeometry::IntersectionGeometry(const Node* root_node, const Element& target_element, const Vector<Length>& root_margin,
    const Vector<float>& thresholds, const Vector<Length>& target_margin, const Vector<Length>& scroll_margin, unsigned flags,
    std::optional<RootGeometry>& root_geometry, CachedRects* cached_rects)
    : flags_(flags & kConstructorFlagsMask)
{
    ...
    RootAndTarget root_and_target(root_node, target_element, !target_margin.empty(), !scroll_margin.empty());
    ...
    ComputeGeometry(*root_geometry, root_and_target, thresholds, target_margin, scroll_margin, cached_rects);
}

IntersectionGeometry::RootAndTarget::RootAndTarget(const Node* root_node, const Element& target_element, bool has_target_margin, bool has_scroll_margin)
...
const LayoutObject* IntersectionGeometry::GetTargetLayoutObject(const Element& target_element)
{
    ...
    if (target->GetFrameView()->IsDisplayLocked() || DisplayLockUtilities::IsInLockedSubtreeCrossingFrames(target_element)) [[unlikely]] {
        return nullptr;
    }
...
const LayoutObject* IntersectionGeometry::RootAndTarget::GetRootLayoutObject(const Node* root_node) const
{
    if (root_node) {
...
void IntersectionGeometry::RootAndTarget::ComputeRelationship(bool root_is_implicit, bool has_target_margin, bool has_scroll_margin)
{
    ...
    while (container != root) {
        has_filter |= !have_crossed_frame_boundary && container->HasFilterInducingProperty();
        ...
        if (!container) {
            ...
            auto* owner = previous_container->GetFrameView()->GetFrame().OwnerLayoutObject();
            if (!owner) {
                return;
            }
        ...
        if (!has_intermediate_clippers && !have_crossed_frame_boundary && container != root && ClipsContents(*container)) {
            has_intermediate_clippers = true;
        }

        if (container != root && has_scroll_margin && container->IsScrollContainer()) {
    ...
    root_scrolls_target
        = root->IsScrollContainer() && To<LayoutBox>(root)->HasScrollableOverflow() && !(root->IsLayoutView() && previous_container->IsFixedPositioned());
...
bool IntersectionGeometry::CanUseGeometryMapper(const LayoutObject& object)
{
    ...
    return layout_view && !layout_view->NeedsPaintPropertyUpdate() && !layout_view->DescendantNeedsPaintPropertyUpdate();
}

void IntersectionGeometry::UpdateShouldUseCachedRects(const RootAndTarget& root_and_target, CachedRects* cached_rects)
{
    ...
    if (root_and_target.relationship != RootAndTarget::kNotScrollable && root_and_target.relationship != RootAndTarget::kScrollableByRootOnly) {
        return;
    }
...
void IntersectionGeometry::ComputeGeometry(const RootGeometry& root_geometry, const RootAndTarget& root_and_target, const Vector<float>& thresholds,
    const Vector<Length>& target_margin, const Vector<Length>& scroll_margin, CachedRects* cached_rects)
{
    ...
    if (ShouldUseCachedRects()) {
        ...
        pre_margin_target_rect_is_empty = cached_rects->pre_margin_target_rect_is_empty;
        ...
        // been applied.
        unclipped_intersection_rect_ = cached_rects->unscrolled_unclipped_intersection_rect;
    } else {
        target_rect_ = InitializeTargetRect(target, flags_);
        pre_margin_target_rect_is_empty = target_rect_.IsEmpty();
        ApplyMargin(target_rect_, target_margin, root_geometry.zoom, root_geometry.pre_margin_local_root_rect.size());
    ...
    if (cached_rects) {
        ...
        cached_rects->pre_margin_target_rect_is_empty = pre_margin_target_rect_is_empty;
    }
    root_rect_ = root_geometry.local_root_rect;

    bool does_intersect = ClipToRoot(root_and_target, root_rect_, unclipped_intersection_rect_, intersection_rect_, scroll_margin, cached_rects);
    ...
    if (does_intersect) {
        ...
        if (RootIsImplicit()) {
            ...
            TransformState implicit_root_to_target_document_transform(TransformState::kUnapplyInverseTransformDirection);
            target->View()->MapAncestorToLocal(
                nullptr, implicit_root_to_target_document_transform, kTraverseDocumentBoundaries | kApplyRemoteMainFrameTransform);
            gfx::Transform matrix = implicit_root_to_target_document_transform.AccumulatedTransform().InverseOrIdentity();
            intersection_rect_ = matrix.ProjectQuad(gfx::QuadF(intersection_rect_)).BoundingBox();
            unclipped_intersection_rect = matrix.ProjectQuad(gfx::QuadF(unclipped_intersection_rect_)).BoundingBox();
        } else {
            ...
            intersection_rect_ = root_geometry.root_to_view_transform.MapRect(intersection_rect_);
            unclipped_intersection_rect = root_geometry.root_to_view_transform.MapRect(unclipped_intersection_rect);
        }
    ...
    root_rect_ = root_geometry.root_to_view_transform.MapRect(gfx::RectF(root_rect_));
    ...
    if (does_intersect) {
        const gfx::RectF& comparison_rect = ShouldTrackFractionOfRoot() ? root_rect_ : target_rect_;
        ...
        bool empty_override = !ShouldTrackFractionOfRoot() && pre_margin_target_rect_is_empty;
        ...
        threshold_index_ = FirstThresholdGreaterThan(intersection_ratio_, thresholds);
    } else {
        intersection_ratio_ = 0;
        threshold_index_ = 0;
    }
    if (IsIntersecting() && ShouldComputeVisibility()) {
        auto visiblity_info = ComputeVisibilityInfo(target, PhysicalRect::FastAndLossyFromRectF(target_rect_), flags_);
    ...
    if (cached_rects) {
        cached_rects->min_scroll_delta_to_update
            = ComputeMinScrollDeltaToUpdate(root_and_target, target_to_view_transform, root_geometry.root_to_view_transform, thresholds, scroll_margin);
    ...
    if (flags_ & kShouldConvertToCSSPixels) {
        AdjustForAbsoluteZoom::AdjustRectMaybeExcludingCSSZoom(target_rect_, *target);
        AdjustForAbsoluteZoom::AdjustRectMaybeExcludingCSSZoom(intersection_rect_, *target);
        AdjustForAbsoluteZoom::AdjustRectMaybeExcludingCSSZoom(root_rect_, *root);
    }
}

bool IntersectionGeometry::ClipToRoot(const RootAndTarget& root_and_target, const gfx::RectF& root_rect, gfx::RectF& unclipped_intersection_rect,
    gfx::RectF& intersection_rect, const Vector<Length>& scroll_margin, CachedRects* cached_rects)
{
    const LayoutObject* root = root_and_target.root;
    // TODO(crbug.com/1456208): Support inline root.
    if (!root->IsBox()) {
    ...
    if (!scroll_margin.empty()) {
        ...
        for (const LayoutBox* scroller : root_and_target.intermediate_scrollers) {
            gfx::RectF scroller_rect = gfx::RectF(scroller->OverflowClipRect(PhysicalOffset()));
            if (std::optional<gfx::RectF> clip_path_box = ClipPathClipper::LocalClipPathBoundingBox(*scroller)) {
                scroller_rect.Intersect(*clip_path_box);
            }

            local_ancestor = To<LayoutBox>(scroller);
            if (!ApplyClip(target, local_ancestor, scroller, scroller_rect, unclipped_intersection_rect, intersection_rect, scroll_margin,
                    ignore_local_clip_path,
    ...
    if (!RootIsImplicit() || root->GetDocument().GetFrame()->IsOutermostMainFrame()) {
        local_ancestor = To<LayoutBox>(root);
    }

    return ApplyClip(target, local_ancestor, root_and_target.root, root_rect, unclipped_intersection_rect, intersection_rect, scroll_margin,
        ignore_local_clip_path, root_and_target.root_scrolls_target, cached_rects);
}

bool IntersectionGeometry::ApplyClip(const LayoutObject* target, const LayoutBox* local_ancestor, const LayoutObject* root, const gfx::RectF& root_rect,
    gfx::RectF& unclipped_intersection_rect, gfx::RectF& intersection_rect, const Vector<Length>& scroll_margin, bool ignore_local_clip_path,
    bool root_scrolls_target, CachedRects* cached_rects)
{
    unsigned flags = kDefaultVisualRectFlags | kEdgeInclusive | kDontApplyMainFrameOverflowClip;
    ...
    if (ShouldUseCachedRects()) {
        ...
        does_intersect = target->MapToVisualRectInAncestorSpace(local_ancestor, unclipped_intersection_rect, static_cast<VisualRectFlags>(flags));
        if (local_ancestor && local_ancestor->IsScrollContainer() && !root_scrolls_target) {
            ...
            // root.
            ScrollingContentsToBorderBoxSpace(local_ancestor, unclipped_intersection_rect);
        }
    }
    if (cached_rects) {
        cached_rects->unscrolled_unclipped_intersection_rect = unclipped_intersection_rect;
    ...
    if (does_intersect) {
        if (local_ancestor) {
            if (root_scrolls_target) {
                ...
                if (auto* properties = local_ancestor->FirstFragment().PaintProperties()) {
                    if (auto* replaced_transform = properties->ReplacedContentTransform()) {
                        gfx::Transform invert_replaced_transform
                            = GeometryMapper::SourceToDestinationProjection(*replaced_transform, *replaced_transform->Parent());
                        unclipped_intersection_rect = invert_replaced_transform.MapRect(unclipped_intersection_rect);
                    }
                }
            }
            ...
            if (!scroll_margin.empty() && root->IsScrollContainer()) {
                ...
                // root_clip_rect.
                ApplyMargin(root_clip_rect, scroll_margin, root->StyleRef().EffectiveZoom(), root_clip_rect.size());
            }
            ...
            if (clip_rect.IsEmpty()) {
                ...
                clip_rect = ToPixelSnappedRect(local_root_frame->ContentLayoutObject()->LocalToAncestorRect(
                    PhysicalRect(clip_rect), nullptr, kTraverseDocumentBoundaries | kApplyRemoteMainFrameTransform));
                intersection_rect = unclipped_intersection_rect;
                does_intersect &= intersection_rect.InclusiveIntersect(gfx::RectF(clip_rect));
            }
        }
    }
...
wtf_size_t IntersectionGeometry::FirstThresholdGreaterThan(float ratio, const Vector<float>& thresholds) const
{
    wtf_size_t result = 0;
...
gfx::Vector2dF IntersectionGeometry::ComputeMinScrollDeltaToUpdate(const RootAndTarget& root_and_target, const gfx::Transform& target_to_view_transform,
    const gfx::Transform& root_to_view_transform, const Vector<float>& thresholds, const Vector<Length>& scroll_margin) const
{
    if (!scroll_margin.empty()) {
    ...
    if (!target_to_view_transform.IsIdentityOr2dTranslation() || !root_to_view_transform.IsIdentityOr2dTranslation()) {
        return gfx::Vector2dF();
    }
    CHECK_GE(thresholds.size(), 1u);
    if (thresholds[0] == 1) {
        if (ShouldTrackFractionOfRoot()) {
            if (root_rect_.width() > target_rect_.width() || root_rect_.height() > target_rect_.height()) {
                // The intersection rect (which is contained by target_rect_) can never
                // cover root_rect_ 100%.
                return kInfiniteScrollDelta;
            }
            if (target_rect_.Contains(root_rect_) && root_and_target.relationship == RootAndTarget::kHasIntermediateClippers) {
            ...
            if (target_rect_.width() > root_rect_.width() || target_rect_.height() > root_rect_.height()) {
                // The intersection rect (which is contained by root_rect_) can never
                // cover target_rect_ 100%.
                return kInfiniteScrollDelta;
            }
            if (root_rect_.Contains(target_rect_) && root_and_target.relationship == RootAndTarget::kHasIntermediateClippers) {
        ...
        return gfx::Vector2dF(std::min(std::abs(root_rect_.x() - target_rect_.x()), std::abs(root_rect_.right() - target_rect_.right())),
            std::min(std::abs(root_rect_.y() - target_rect_.y()), std::abs(root_rect_.bottom() - target_rect_.bottom())));
    }
    ...
    bool inclusively_intersects = root_target_intersection_rect.InclusiveIntersect(target_rect_);
    if (inclusively_intersects
        && (thresholds.size() != 1 || thresholds[0] > kMinimumThreshold || root_and_target.relationship == RootAndTarget::kHasIntermediateClippers
    ...
    return gfx::Vector2dF(std::min(std::abs(root_rect_.right() - target_rect_.x()), std::abs(target_rect_.right() - root_rect_.x())),
        std::min(std::abs(root_rect_.bottom() - target_rect_.y()), std::abs(target_rect_.bottom() - root_rect_.y())));
}
