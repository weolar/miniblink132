...
protected:
    void TestScrollMargin(int scroll_margin, bool is_intersecting, double intersectionRatio)
    {
    ...
    <div id=scroller>
      ...
      <div id=target></div>
        ...
        TestIntersectionObserverDelegate* scroll_margin_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());

        IntersectionObserver* scroll_margin_observer
            = MakeGarbageCollected<IntersectionObserver>(*scroll_margin_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver,
                IntersectionObserver::Params { .margin = { Length::Fixed(10) },
        ...
        EXPECT_EQ(is_intersecting, scroll_margin_delegate->LastEntry()->isIntersecting());
        EXPECT_NEAR(intersectionRatio, scroll_margin_delegate->LastEntry()->intersectionRatio(), 0.001);
    }

    void TestScrollMarginNested(int scroll_margin, bool is_intersecting, double intersectionRatio)
    {
        WebView().MainFrameViewWidget()->Resize(gfx::Size(200, 200));
        ...
        IntersectionObserver* scroll_margin_observer
            = MakeGarbageCollected<IntersectionObserver>(*scroll_margin_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver,
                IntersectionObserver::Params {
                    .margin = { Length::Fixed(10) }, .scroll_margin = { Length::Fixed(scroll_margin) }, .thresholds = { std::numeric_limits<float>::min() } });
    ...
    void TestMinScrollDeltaToUpdateWithIntermediateClip()
    {
        ...
        DummyExceptionStateForTesting exception_state;
        TestIntersectionObserverDelegate* observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
        IntersectionObserver* observer
            = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver, exception_state);
        ASSERT_FALSE(exception_state.HadException());
        observer->observe(target, exception_state);
        ASSERT_FALSE(exception_state.HadException());
        const IntersectionObservation* observation = target->IntersectionObserverData()->GetObservationFor(*observer);
        EXPECT_EQ(gfx::Vector2dF(), observation->MinScrollDeltaToUpdate());
        EXPECT_EQ(LocalFrameView::kRequired, frame_view->GetIntersectionObservationStateForTesting());
        ...
        root->scrollTo(0, 50);
        EXPECT_EQ(gfx::Vector2dF(50, 100), observation->MinScrollDeltaToUpdate());
        EXPECT_EQ(LocalFrameView::kScrollAndVisibilityOnly, frame_view->GetIntersectionObservationStateForTesting());
        Compositor().BeginFrame();
        test::RunPendingTasks();
        EXPECT_EQ(observer_delegate->CallCount(), 1);
        EXPECT_EQ(observer_delegate->EntryCount(), 1);
    ...
    bool CanUseCachedRects(const IntersectionObservation& observation)
    {
        return observation.CanUseCachedRectsForTesting(
            GetDocument().View()->GetIntersectionObservationStateForTesting() <= LocalFrameView::kScrollAndVisibilityOnly);
    }
};
...
TEST_F(IntersectionObserverTest, DocumentRootClips)
{
    ...
    Document* iframe_document = To<WebLocalFrameImpl>(MainFrame().FirstChild())->GetFrame()->GetDocument();
    IntersectionObserverInit* observer_init = IntersectionObserverInit::Create();
    observer_init->setRoot(MakeGarbageCollected<V8UnionDocumentOrElement>(iframe_document));
    DummyExceptionStateForTesting exception_state;
    TestIntersectionObserverDelegate* observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    IntersectionObserver* observer
        = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver, exception_state);
    ...
    iframe_document->View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 1000), mojom::blink::ScrollType::kProgrammatic);
    Compositor().BeginFrame();
    test::RunPendingTasks();
    EXPECT_EQ(observer_delegate->CallCount(), 2);
    EXPECT_EQ(observer_delegate->EntryCount(), 2);
...
TEST_F(IntersectionObserverTest, ReportsFractionOfTargetOrRoot)
{
    ...
    TestIntersectionObserverDelegate* target_observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());

    IntersectionObserver* target_observer
        = MakeGarbageCollected<IntersectionObserver>(*target_observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver,
            IntersectionObserver::Params {
    ...
    TestIntersectionObserverDelegate* root_observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());

    IntersectionObserver* root_observer
        = MakeGarbageCollected<IntersectionObserver>(*root_observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver,
            IntersectionObserver::Params { .thresholds = { kExpectedFractionOfRoot / 2 }, .semantics = IntersectionObserver::kFractionOfRoot });
    ...
    EXPECT_NEAR(kExpectedFractionOfTarget, target_observer_delegate->LastEntry()->intersectionRatio(), 1e-6);
    ...
    EXPECT_NEAR(kExpectedFractionOfRoot, root_observer_delegate->LastEntry()->intersectionRatio(), 1e-6);
}
...
TEST_F(IntersectionObserverTest, DirectlyUpdateTransform)
{
    ...
    container->SetInlineStyleProperty(CSSPropertyID::kTransform, "translateX(300px)");
    ...
    EXPECT_EQ(LocalFrameView::kDesired, GetDocument().View()->GetIntersectionObservationStateForTesting());
    Compositor().BeginFrame();
    test::RunPendingTasks();
    ASSERT_FALSE(Compositor().NeedsBeginFrame());
    ...
    GetDocument().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest);
    EXPECT_EQ(LocalFrameView::kNotNeeded, GetDocument().View()->GetIntersectionObservationStateForTesting());
    Compositor().BeginFrame();
    test::RunPendingTasks();
    ASSERT_FALSE(Compositor().NeedsBeginFrame());
...
TEST_F(IntersectionObserverTest, ResumePostsTask)
{
    ...
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 300), mojom::blink::ScrollType::kProgrammatic);
    Compositor().BeginFrame();
    EXPECT_EQ(observer_delegate->CallCount(), 1);
    test::RunPendingTasks();
    EXPECT_EQ(observer_delegate->CallCount(), 2);
    ...
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 0), mojom::blink::ScrollType::kProgrammatic);
    Compositor().BeginFrame();
    EXPECT_EQ(observer_delegate->CallCount(), 2);
    test::RunPendingTasks();
    EXPECT_EQ(observer_delegate->CallCount(), 2);
...
TEST_F(IntersectionObserverTest, HitTestAfterMutation)
{
    ...
    HitTestResult result(HitTestRequest(HitTestRequest::kReadOnly | HitTestRequest::kActive | HitTestRequest::kAllowChildFrameContent), location);
...
TEST_F(IntersectionObserverTest, DisconnectClearsNotifications)
{
    ...
    Element* target = GetDocument().getElementById(AtomicString("target"));
    ASSERT_TRUE(target);
    IntersectionObserverController& controller = GetDocument().EnsureIntersectionObserverController();
    observer->observe(target, exception_state);
...
TEST_F(IntersectionObserverTest, RootIntersectionWithForceZeroLayoutHeight)
{
    ...
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 600), mojom::blink::ScrollType::kProgrammatic);
    Compositor().BeginFrame();
    test::RunPendingTasks();
    ASSERT_EQ(observer_delegate->CallCount(), 2);
    EXPECT_FALSE(observer_delegate->LastIntersectionRect().IsEmpty());
    EXPECT_EQ(gfx::RectF(200, 400, 100, 100), observer_delegate->LastIntersectionRect());

    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 1200), mojom::blink::ScrollType::kProgrammatic);
    Compositor().BeginFrame();
    test::RunPendingTasks();
...
TEST_F(IntersectionObserverTest, TrackedTargetBookkeeping)
{
    ...
    IntersectionObserverInit* observer_init = IntersectionObserverInit::Create();
    TestIntersectionObserverDelegate* observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    IntersectionObserver* observer1 = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver);
    observer1->observe(target);
    IntersectionObserver* observer2 = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver);
    observer2->observe(target);

    ElementIntersectionObserverData* target_data = target->IntersectionObserverData();
...
TEST_F(IntersectionObserverTest, TrackedRootBookkeeping)
{
    ...
    Persistent<Element> target = GetDocument().getElementById(AtomicString("target1"));
    Persistent<IntersectionObserverInit> observer_init = IntersectionObserverInit::Create();
    observer_init->setRoot(MakeGarbageCollected<V8UnionDocumentOrElement>(root));
    Persistent<TestIntersectionObserverDelegate> observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    Persistent<IntersectionObserver> observer
        = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver);
    ...
    observer = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver);
    observer->observe(target);
...
TEST_F(IntersectionObserverTest, InaccessibleTarget)
{
    ...
    Persistent<TestIntersectionObserverDelegate> observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    Persistent<IntersectionObserver> observer
        = IntersectionObserver::Create(IntersectionObserverInit::Create(), *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver);

    Persistent<Element> target = GetDocument().getElementById(AtomicString("target"));
    ...
    WeakPersistent<TestIntersectionObserverDelegate> observer_delegate_weak = observer_delegate.Get();
...
TEST_F(IntersectionObserverTest, RootMarginDevicePixelRatio)
{
    ...
    EXPECT_EQ(observer_delegate->EntryCount(), 1);
    EXPECT_FALSE(observer_delegate->LastEntry()->isIntersecting());
    EXPECT_RECTF_NEAR(observer_delegate->LastEntry()->GetGeometry().RootRect(), gfx::RectF(0, 31, 800, 600 - 31), 0.0001);
}
...
TEST_F(IntersectionObserverTest, CachedRectsWithScrollers)
{
    ...
    IntersectionObservation* observation1 = target1->IntersectionObserverData()->GetObservationFor(*observer);
    EXPECT_FALSE(CanUseCachedRects(*observation1));
    IntersectionObservation* observation2 = target2->IntersectionObserverData()->GetObservationFor(*observer);
    EXPECT_FALSE(CanUseCachedRects(*observation2));
    IntersectionObservation* observation3 = target3->IntersectionObserverData()->GetObservationFor(*observer);
    ...
    target1->parentElement()->SetInlineStyleProperty(CSSPropertyID::kMarginLeft, "10px");
...
TEST_F(IntersectionObserverTest, CachedRectsWithoutIntermediateScrollable)
{
    ...
    IntersectionObservation* observation3 = target3->IntersectionObserverData()->GetObservationFor(*observer);
    EXPECT_FALSE(CanUseCachedRects(*observation3));
    IntersectionObservation* observation4 = target4->IntersectionObserverData()->GetObservationFor(*observer);
...
TEST_F(IntersectionObserverTest, CachedRectsWithPaintPropertyChange)
{
    ...
    IntersectionObservation* observation = target->IntersectionObserverData()->GetObservationFor(*observer);
    GetDocument().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest);
    ...
    GetDocument().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest);
    EXPECT_TRUE(CanUseCachedRects(*observation));
    container->SetInlineStyleProperty(CSSPropertyID::kTransform, "translateY(20px)");
    GetDocument().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest);
    EXPECT_FALSE(CanUseCachedRects(*observation));
}
...
TEST_F(IntersectionObserverTest, CachedRectsWithFixedPosition)
{
    ...
    IntersectionObservation* observation1 = fixed->IntersectionObserverData()->GetObservationFor(*observer);
    EXPECT_FALSE(CanUseCachedRects(*observation1));
    IntersectionObservation* observation2 = child->IntersectionObserverData()->GetObservationFor(*observer);
    EXPECT_FALSE(CanUseCachedRects(*observation2));
...
TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateNotScrollable)
{
    ...
    EXPECT_EQ(observer_delegate->EntryCount(), 1);
    EXPECT_FALSE(observer_delegate->LastEntry()->isIntersecting());
    EXPECT_EQ(IntersectionGeometry::kInfiniteScrollDelta, observation->MinScrollDeltaToUpdate());
    EXPECT_EQ(LocalFrameView::kNotNeeded, frame_view->GetIntersectionObservationStateForTesting());
}

TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateNotScrollableToScrollable)
{
    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest main_resource("https://example.com/", "text/html");
    LoadURL("https://example.com/");
...
TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateThresholdZeroIntermediateClip)
{
    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest main_resource("https://example.com/", "text/html");
    LoadURL("https://example.com/");
...
TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateThresholdZeroIntermediateClipPath)
{
    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest main_resource("https://example.com/", "text/html");
    LoadURL("https://example.com/");
...
TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateThresholdZeroClipPathOnTarget)
{
    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest main_resource("https://example.com/", "text/html");
    LoadURL("https://example.com/");
...
TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateMinimumThreshold)
{
    ...
    IntersectionObserverInit* observer_init = IntersectionObserverInit::Create();
    observer_init->setRoot(MakeGarbageCollected<V8UnionDocumentOrElement>(root));
    observer_init->setThreshold(MakeGarbageCollected<V8UnionDoubleOrDoubleSequence>(IntersectionObserver::kMinimumThreshold));
    DummyExceptionStateForTesting exception_state;
    TestIntersectionObserverDelegate* observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    IntersectionObserver* observer
        = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver, exception_state);
...
TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateThreshold0_5)
{
    ...
    IntersectionObserverInit* observer_init = IntersectionObserverInit::Create();
    observer_init->setRoot(MakeGarbageCollected<V8UnionDocumentOrElement>(root));
    observer_init->setThreshold(MakeGarbageCollected<V8UnionDoubleOrDoubleSequence>(0.5));
    DummyExceptionStateForTesting exception_state;
    TestIntersectionObserverDelegate* observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    IntersectionObserver* observer
        = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver, exception_state);
...
TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateThresholdOne)
{
    ...
    IntersectionObserverInit* observer_init = IntersectionObserverInit::Create();
    observer_init->setRoot(MakeGarbageCollected<V8UnionDocumentOrElement>(root));
    observer_init->setThreshold(MakeGarbageCollected<V8UnionDoubleOrDoubleSequence>(1));
    DummyExceptionStateForTesting exception_state;
    TestIntersectionObserverDelegate* observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    IntersectionObserver* observer
        = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver, exception_state);
...
TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateThresholdOneOfRoot)
{
    ...
    IntersectionObserver* observer = MakeGarbageCollected<IntersectionObserver>(*observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver,
        IntersectionObserver::Params {
            .root = root,
            .thresholds = { 1 },
...
TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateThresholdFilterOnTarget)
{
    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest main_resource("https://example.com/", "text/html");
    LoadURL("https://example.com/");
    ...
    TestIntersectionObserverDelegate* observer_delegate_js = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    TestIntersectionObserverDelegate* observer_delegate_display_lock = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    IntersectionObserverInit* observer_init = IntersectionObserverInit::Create();
    observer_init->setRoot(MakeGarbageCollected<V8UnionDocumentOrElement>(root));
    DummyExceptionStateForTesting exception_state;
    IntersectionObserver* observer_js
        = IntersectionObserver::Create(observer_init, *observer_delegate_js, LocalFrameUkmAggregator::kJavascriptIntersectionObserver, exception_state);
    IntersectionObserver* observer_display_lock = IntersectionObserver::Create(
        observer_init, *observer_delegate_display_lock, LocalFrameUkmAggregator::kDisplayLockIntersectionObserver, exception_state);
    ASSERT_FALSE(exception_state.HadException());
    observer_js->observe(target, exception_state);
    ASSERT_FALSE(exception_state.HadException());
    observer_display_lock->observe(target, exception_state);
    ASSERT_FALSE(exception_state.HadException());
    const IntersectionObservation* observation_js = target->IntersectionObserverData()->GetObservationFor(*observer_js);
    EXPECT_EQ(gfx::Vector2dF(), observation_js->MinScrollDeltaToUpdate());
    const IntersectionObservation* observation_display_lock = target->IntersectionObserverData()->GetObservationFor(*observer_display_lock);
    EXPECT_EQ(gfx::Vector2dF(), observation_display_lock->MinScrollDeltaToUpdate());
    EXPECT_EQ(LocalFrameView::kRequired, frame_view->GetIntersectionObservationStateForTesting());
...
TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateThresholdFilterOnIntermediateContainer)
{
    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest main_resource("https://example.com/", "text/html");
    LoadURL("https://example.com/");
    ...
    DummyExceptionStateForTesting exception_state;
    IntersectionObserver* observer
        = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kDisplayLockIntersectionObserver, exception_state);
    ASSERT_FALSE(exception_state.HadException());
    observer->observe(target, exception_state);
    ASSERT_FALSE(exception_state.HadException());
    const IntersectionObservation* observation = target->IntersectionObserverData()->GetObservationFor(*observer);
...
TEST_F(IntersectionObserverTest, MinScrollDeltaToUpdateThresholdFilterOnIntermediateNonContainer)
{
    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest main_resource("https://example.com/", "text/html");
    LoadURL("https://example.com/");
...
TEST_F(IntersectionObserverV2Test, TrackVisibilityInit)
{
    ...
    TestIntersectionObserverDelegate* observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    IntersectionObserver* observer
        = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver, ASSERT_NO_EXCEPTION);
    ...
    // This should fail because no delay is set.
    {
        ...
        observer = IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver, exception_state);
...
TEST_F(IntersectionObserverV2Test, BasicOpacity)
{
    ...
    ASSERT_FALSE(exception_state.HadException());
    Element* target = GetDocument().getElementById(AtomicString("target"));
    Element* transparent = GetDocument().getElementById(AtomicString("transparent"));
    ASSERT_TRUE(target);
...
TEST_F(IntersectionObserverV2Test, BasicTransform)
{
    ...
    ASSERT_FALSE(exception_state.HadException());
    Element* target = GetDocument().getElementById(AtomicString("target"));
    Element* transformed = GetDocument().getElementById(AtomicString("transformed"));
    ASSERT_TRUE(target);
    ...
    transformed->SetInlineStyleProperty(CSSPropertyID::kTransform, "translateX(10px) translateY(20px) scale(2)");
    Compositor().BeginFrame();
    test::RunPendingTasks();
    ASSERT_FALSE(Compositor().NeedsBeginFrame());
    EXPECT_EQ(observer_delegate->CallCount(), 1);
    ...
    transformed->SetInlineStyleProperty(CSSPropertyID::kTransform, "skewX(10deg)");
    Compositor().BeginFrame();
    test::RunPendingTasks();
    ASSERT_FALSE(Compositor().NeedsBeginFrame());
    EXPECT_EQ(observer_delegate->CallCount(), 2);
...
TEST_F(IntersectionObserverTest, ApplyMarginToTarget)
{
    ...
    TestIntersectionObserverDelegate* root_margin_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    IntersectionObserver* root_margin_observer
        = MakeGarbageCollected<IntersectionObserver>(*root_margin_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver,
            IntersectionObserver::Params {
    ...
    TestIntersectionObserverDelegate* target_margin_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    // Same parameters as above except that margin is applied to target.
    IntersectionObserver* target_margin_observer
        = MakeGarbageCollected<IntersectionObserver>(*target_margin_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver,
            IntersectionObserver::Params {
                .margin = { Length::Fixed(10) },
...
TEST_F(IntersectionObserverTest, ScrollMarginIntersectingNonScrollingRoot)
{
    ...
    EXPECT_NEAR(1.0, scroll_margin_delegate->LastEntry()->intersectionRatio(), 0.001);
}
...
TEST_F(IntersectionObserverTest, ParseMarginExtraText)
{
    ...
    EXPECT_EQ(exception_state.Message(), "Extra text found at the end of rootMargin.");
}
...
TEST_F(IntersectionObserverTest, ParseMarginUnsupportedUnitType)
{
    ...
    IntersectionObserver::Create(observer_init, *observer_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver, exception_state);
    ASSERT_TRUE(exception_state.HadException());
    EXPECT_EQ(exception_state.Message(), "rootMargin must be specified in pixels or percent.");
}
