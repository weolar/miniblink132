...
class IntersectionObserverDelegateImpl final : public IntersectionObserverDelegate {
public:
    IntersectionObserverDelegateImpl(ExecutionContext* context, IntersectionObserver::EventCallback callback,
        IntersectionObserver::DeliveryBehavior delivery_behavior, bool needs_initial_observation_with_detached_target)
        : context_(context)
        , callback_(std::move(callback))
        , delivery_behavior_(delivery_behavior)
        , needs_initial_observation_with_detached_target_(needs_initial_observation_with_detached_target)
    {
    }
    IntersectionObserverDelegateImpl(const IntersectionObserverDelegateImpl&) = delete;
    IntersectionObserverDelegateImpl& operator=(const IntersectionObserverDelegateImpl&) = delete;
    ...
    void Deliver(const HeapVector<Member<IntersectionObserverEntry>>& entries, IntersectionObserver& observer) override
    {
...
void ParseMargin(const String& margin_parameter, Vector<Length>& margin, ExceptionState& exception_state, const String& marginName)
{
    ...
    while (stream.Peek().GetType() != kEOFToken && !exception_state.HadException()) {
        if (margin.size() == 4) {
            exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Extra text found at the end of " + marginName + "Margin.");
            break;
        }
        ...
        case kDimensionToken:
            ...
            case CSSPrimitiveValue::UnitType::kPixels:
                margin.push_back(Length::Fixed(static_cast<int>(floor(token.NumericValue()))));
                break;
            case CSSPrimitiveValue::UnitType::kPercentage:
                margin.push_back(Length::Percent(token.NumericValue()));
                break;
            default:
                exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, marginName + "Margin must be specified in pixels or percent.");
            }
            stream.ConsumeIncludingWhitespace();
            break;
        default:
...
void ParseThresholds(const V8UnionDoubleOrDoubleSequence* threshold_parameter, Vector<float>& thresholds, ExceptionState& exception_state)
{
    switch (threshold_parameter->GetContentType()) {
    case V8UnionDoubleOrDoubleSequence::ContentType::kDouble:
        thresholds.push_back(base::MakeClampedNum<float>(threshold_parameter->GetAsDouble()));
        break;
    ...
    for (auto threshold_value : thresholds) {
        if (std::isnan(threshold_value) || threshold_value < 0.0 || threshold_value > 1.0) {
            exception_state.ThrowRangeError("Threshold values must be numbers between 0 and 1");
            break;
        }
    }
...
Vector<Length> NormalizeMargins(const Vector<Length>& margins)
{
    ...
    case 0:
        break;
    case 1:
        normalized_margins[0] = normalized_margins[1] = normalized_margins[2] = normalized_margins[3] = margins[0];
        break;
    case 2:
...
Vector<Length> NormalizeScrollMargins(const Vector<Length>& margins)
{
    ...
    if (std::all_of(normalized_margins.begin(), normalized_margins.end(), [](const auto& m) { return m.IsZero(); })) {
...
IntersectionObserver* IntersectionObserver::Create(const IntersectionObserverInit* observer_init, IntersectionObserverDelegate& delegate,
    std::optional<LocalFrameUkmAggregator::MetricId> ukm_metric_id, ExceptionState& exception_state)
{
    Node* root = nullptr;
    ...
    ParseMargin(observer_init->rootMargin(), params.margin, exception_state, "root");
    if (exception_state.HadException()) {
        return nullptr;
    }

    if (RuntimeEnabledFeatures::IntersectionObserverScrollMarginEnabled()) {
        ParseMargin(observer_init->scrollMargin(), params.scroll_margin, exception_state, "scroll");
        if (exception_state.HadException()) {
            return nullptr;
        }
    }

    ParseThresholds(observer_init->threshold(), params.thresholds, exception_state);
    if (exception_state.HadException()) {
        return nullptr;
    }

    return MakeGarbageCollected<IntersectionObserver>(delegate, ukm_metric_id, std::move(params));
}

IntersectionObserver* IntersectionObserver::Create(
    ScriptState* script_state, V8IntersectionObserverCallback* callback, const IntersectionObserverInit* observer_init, ExceptionState& exception_state)
{
    V8IntersectionObserverDelegate* delegate = MakeGarbageCollected<V8IntersectionObserverDelegate>(callback, script_state);
    if (observer_init && observer_init->trackVisibility()) {
        UseCounter::Count(delegate->GetExecutionContext(), WebFeature::kIntersectionObserverV2);
    }
    return Create(observer_init, *delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver, exception_state);
}

IntersectionObserver* IntersectionObserver::Create(
    const Document& document, EventCallback callback, std::optional<LocalFrameUkmAggregator::MetricId> ukm_metric_id, Params&& params)
{
    IntersectionObserverDelegateImpl* intersection_observer_delegate = MakeGarbageCollected<IntersectionObserverDelegateImpl>(
        document.GetExecutionContext(), std::move(callback), params.behavior, params.needs_initial_observation_with_detached_target);
    return MakeGarbageCollected<IntersectionObserver>(*intersection_observer_delegate, ukm_metric_id, std::move(params));
}

IntersectionObserver::IntersectionObserver(
    IntersectionObserverDelegate& delegate, std::optional<LocalFrameUkmAggregator::MetricId> ukm_metric_id, Params&& params)
    ...
    , expose_occluder_id_(params.expose_occluder_id)
{
    if (params.root) {
        if (params.root->IsDocumentNode()) {
            To<Document>(params.root)->EnsureDocumentExplicitRootIntersectionObserverData().AddObserver(*this);
        } else {
            DCHECK(params.root->IsElementNode());
            To<Element>(params.root)->EnsureIntersectionObserverData().AddObserver(*this);
        }
    }
}
...
void IntersectionObserver::observe(Element* target, ExceptionState& exception_state)
{
    ...
    IntersectionObservation* observation = MakeGarbageCollected<IntersectionObservation>(*this, *target);
    target->EnsureIntersectionObserverData().AddObservation(*observation);
    observations_.insert(observation);
    if (root() && root()->isConnected()) {
        root()->GetDocument().EnsureIntersectionObserverController().AddTrackedObserver(*this);
    }
    if (target->isConnected()) {
        target->GetDocument().EnsureIntersectionObserverController().AddTrackedObservation(*observation);
...
void IntersectionObserver::unobserve(Element* target, ExceptionState& exception_state)
{
    ...
    IntersectionObservation* observation = target->IntersectionObserverData()->GetObservationFor(*this);
    if (!observation)
        return;
...
HeapVector<Member<IntersectionObserverEntry>> IntersectionObserver::takeRecords(ExceptionState& exception_state)
{
...
{
    return !GetUkmMetricId() || GetUkmMetricId() != LocalFrameUkmAggregator::kJavascriptIntersectionObserver;
}
...
void IntersectionObserver::ReportUpdates(IntersectionObservation& observation)
{
    ...
    if (needs_scheduling) {
        To<LocalDOMWindow>(GetExecutionContext())->document()->EnsureIntersectionObserverController().ScheduleIntersectionObserverForDelivery(*this);
    }
}
...
{
    visitor->template RegisterWeakCallbackMethod<IntersectionObserver, &IntersectionObserver::ProcessCustomWeakness>(this);
