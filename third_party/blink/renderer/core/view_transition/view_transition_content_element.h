...
class CORE_EXPORT ViewTransitionContentElement : public ViewTransitionPseudoElementBase {
public:
    explicit ViewTransitionContentElement(Element* parent, PseudoId, const AtomicString& view_transition_name, viz::ViewTransitionElementResourceId,
        bool is_live_content_element, const ViewTransitionStyleTracker* style_tracker);
    ~ViewTransitionContentElement() override;

    void SetIntrinsicSize(const gfx::RectF& captured_rect, const gfx::RectF& border_box_rect, bool propagates_max_extents_rect);
    const gfx::RectF& captured_rect() const
    {
        return captured_rect_;
    }
    const gfx::RectF& border_box_rect() const
    {
        return border_box_rect_;
    }
    const viz::ViewTransitionElementResourceId& resource_id() const
    {
        return resource_id_;
    }
    bool is_live_content_element() const
    {
        return is_live_content_element_;
    }

    bool propagate_max_extent_rect() const
    {
        return propagate_max_extent_rect_;
    }
