...
class ViewTransitionTypeIterationSource : public ViewTransitionTypeSet::IterationSource {
public:
    ...
    bool FetchNextItem(ScriptState*, String& out_value, ExceptionState&) override
    {
...
ViewTransitionTypeSet::ViewTransitionTypeSet(ViewTransition* view_transition, const Vector<String>& initial_values)
{
...
void ViewTransitionTypeSet::add(const String& value, ExceptionState& exception_state)
{
    AddInternal(value);
}
...
void ViewTransitionTypeSet::InvalidateStyle()
{
    ...
    if (ViewTransitionSupplement::From(*document)->GetTransition() != view_transition_) {
        return;
    }

    if (Element* document_element = document->documentElement()) {
        document_element->PseudoStateChanged(CSSSelector::kPseudoActiveViewTransitionType);
    }
}
...
bool ViewTransitionTypeSet::deleteForBinding(ScriptState*, const String& value, ExceptionState&)
{
...
ViewTransitionTypeSet::IterationSource* ViewTransitionTypeSet::CreateIterationSource(ScriptState*, ExceptionState&)
{
    auto* iterator = MakeGarbageCollected<ViewTransitionTypeIterationSource>(*this);
