...
const char* kDuplicateTagBaseError = "Unexpected duplicate view-transition-name: ";
...
template <typename K, typename V> class FlatMapBuilder {
public:
    explicit FlatMapBuilder(size_t reserve = 0)
    {
...
private:
    std::vector<std::pair<K, V>> data_ ALLOW_DISCOURAGED_TYPE("flat_map underlying type");
};
...
CSSPropertyID FromTransitionPropertyId(mojom::blink::ViewTransitionPropertyId id)
{
    switch (id) {
...
{
    DEFINE_STATIC_LOCAL(String, kStaticUAStyles, (UncompressResourceAsASCIIString(IDR_UASTYLE_TRANSITION_CSS)));
    return kStaticUAStyles;
}

const String& AnimationUAStyles()
{
    DEFINE_STATIC_LOCAL(String, kAnimationUAStyles, (UncompressResourceAsASCIIString(IDR_UASTYLE_TRANSITION_ANIMATIONS_CSS)));
...
float ComputeStartForSide(float start, float end, int snapshot_root_dimension, int max_capture_size)
{
    DCHECK_GT((end - start), max_capture_size) << "Side must be larger than max texture size";
    DCHECK_GE(max_capture_size, snapshot_root_dimension) << "Snapshot root bounds must be a subset of max texture size";
    ...
    const float delta_to_distribute_per_side = (max_capture_size - snapshot_root_dimension) / 2;
    const float delta_on_end_side = end - snapshot_root_dimension;
    const float delta_for_start_side = delta_to_distribute_per_side + std::max(0.f, (delta_to_distribute_per_side - delta_on_end_side));
...
std::optional<gfx::RectF> ComputeCaptureRect(const int max_capture_size, const PhysicalRect& ink_overflow_rect_in_border_box_space,
    const gfx::Transform& element_to_snapshot_root, const gfx::Size& snapshot_root_size)
{
    if (ink_overflow_rect_in_border_box_space.Width() <= max_capture_size && ink_overflow_rect_in_border_box_space.Height() <= max_capture_size) {
        return std::nullopt;
    }
    ...
    if (!element_to_snapshot_root.GetInverse(&snapshot_root_to_element)) {
        ...
        return gfx::RectF(gfx::PointF(ink_overflow_rect_in_border_box_space.offset), size);
    }

    const gfx::RectF ink_overflow_rect_in_snapshot_root_space = element_to_snapshot_root.MapRect(gfx::RectF(ink_overflow_rect_in_border_box_space));
    gfx::RectF captured_ink_overflow_subrect_in_snapshot_root_space = ink_overflow_rect_in_snapshot_root_space;

    if (ink_overflow_rect_in_snapshot_root_space.width() > max_capture_size) {
        captured_ink_overflow_subrect_in_snapshot_root_space.set_x(ComputeStartForSide(
            ink_overflow_rect_in_snapshot_root_space.x(), ink_overflow_rect_in_snapshot_root_space.right(), snapshot_root_size.width(), max_capture_size));
        captured_ink_overflow_subrect_in_snapshot_root_space.set_width(max_capture_size);
    }

    if (ink_overflow_rect_in_snapshot_root_space.height() > max_capture_size) {
        captured_ink_overflow_subrect_in_snapshot_root_space.set_y(ComputeStartForSide(
            ink_overflow_rect_in_snapshot_root_space.y(), ink_overflow_rect_in_snapshot_root_space.bottom(), snapshot_root_size.height(), max_capture_size));
        captured_ink_overflow_subrect_in_snapshot_root_space.set_height(max_capture_size);
    }

    return snapshot_root_to_element.MapRect(captured_ink_overflow_subrect_in_snapshot_root_space);
}

int ComputeMaxCaptureSize(Document& document, std::optional<int> max_texture_size, const gfx::Size& snapshot_root_size)
{
    ...
    // computed).
    const float min_page_scale_factor = document.GetPage()->GetPageScaleConstraintsSet().FinalConstraints().minimum_scale;
    const int max_texture_size_in_layout = static_cast<int>(std::ceil(*max_texture_size / min_page_scale_factor));

    LOG_IF(WARNING, snapshot_root_size.width() > max_texture_size_in_layout || snapshot_root_size.height() > max_texture_size_in_layout)
    ...
    // additional content.
    const int max_bounds_based_on_viewport = 2 * std::max(snapshot_root_size.width(), snapshot_root_size.height());
...
gfx::Transform ComputeViewportTransform(const LayoutObject& object)
{
    ...
    DCHECK(ToRoundedPoint(first_fragment.PaintOffset()).IsOrigin()) << first_fragment.PaintOffset();
    ...
    auto transform = GeometryMapper::SourceToDestinationProjection(paint_properties.Transform(), root_properties.Transform());
    if (auto* layout_inline = DynamicTo<LayoutInline>(object)) {
        ...
        transform.Translate(gfx::Vector2dF(layout_inline->PhysicalLinesBoundingBox().offset));
    }
...
gfx::Transform ConvertFromTopLeftToCenter(const gfx::Transform& transform_from_top_left, const PhysicalSize& box_size)
{
...
float DevicePixelRatioFromDocument(Document& document)
{
    ...
    if (document.documentElement() && document.documentElement()->GetLayoutObject()) {
        return document.documentElement()->GetLayoutObject()->StyleRef().EffectiveZoom();
    }

    if (!document.GetPage() || !document.GetFrame()) {
        return 0.f;
    }
    return document.GetPage()->GetChromeClient().GetScreenInfo(*document.GetFrame()).device_scale_factor;
}
...
Vector<AtomicString> GetDocumentScopedClassList(Element* element)
{
    ...
    if (!class_list || class_list->GetNames().empty() || class_list->GetNames().front()->GetTreeScope() != element->GetDocument().GetTreeScope()) {
...
class ViewTransitionStyleTracker::ImageWrapperPseudoElement : public ViewTransitionPseudoElementBase {
public:
    ImageWrapperPseudoElement(Element* parent, PseudoId pseudo_id, const AtomicString& view_transition_name, const ViewTransitionStyleTracker* style_tracker)
        : ViewTransitionPseudoElementBase(parent, pseudo_id, view_transition_name, style_tracker)
    {
    }
...
ViewTransitionStyleTracker::ViewTransitionStyleTracker(Document& document, const blink::ViewTransitionToken& transition_token)
    : document_(document)
...
ViewTransitionStyleTracker::ViewTransitionStyleTracker(Document& document, ViewTransitionState transition_state)
    : document_(document)
    ...
    , deserialized_(true)
{
    ...
    CHECK(supplement);
    supplement->InitializeResourceIdSequence(transition_state.next_element_resource_id);
    ...
    snapshot_root_layout_size_at_capture_ = transition_state.snapshot_root_size_at_capture;
    ...
    for (const auto& transition_state_element : transition_state.elements) {
        auto name = AtomicString::FromUTF8(transition_state_element.tag_name.c_str());
        transition_names.push_back(name);

        DCHECK(!element_data_map_.Contains(name));
        auto* element_data = MakeGarbageCollected<ElementData>();

        element_data->container_properties
            = ContainerProperties { PhysicalRect::EnclosingRect(transition_state_element.border_box_rect_in_enclosing_layer_css_space),
        ...
        set_element_sequence_id_ = std::max(set_element_sequence_id_, transition_state_element.paint_order);

        element_data->visual_overflow_rect_in_layout_space = PhysicalRect::EnclosingRect(transition_state_element.overflow_rect_in_layout_space);
        element_data->captured_rect_in_layout_space = transition_state_element.captured_rect_in_layout_space;

        CHECK_LE(transition_state_element.captured_css_properties.size(), std::size(kPropertiesToCapture) + std::size(kLayeredCaptureProperties));

        FlatMapBuilder<CSSPropertyID, String> css_property_builder(transition_state_element.captured_css_properties.size());
        for (const auto& [id, value] : transition_state_element.captured_css_properties) {
            css_property_builder.Insert(FromTransitionPropertyId(id), String::FromUTF8(value));
        }
        element_data->captured_css_properties = std::move(css_property_builder).Finish();

        for (const auto& class_name : transition_state_element.class_list) {
            element_data->class_list.push_back(AtomicString::FromUTF8(class_name.c_str()));
        }

        element_data->containing_group_name = transition_state_element.containing_group_name.empty()
            ? AtomicString()
            : AtomicString::FromUTF8(transition_state_element.containing_group_name.c_str());
    ...
    if (transition_state.subframe_snapshot_id.IsValid()) {
        subframe_snapshot_layer_ = cc::ViewTransitionContentLayer::Create(transition_state.subframe_snapshot_id, /*is_live_content_layer=*/false);
    }
...
void ViewTransitionStyleTracker::AddConsoleError(String message, Vector<DOMNodeId> related_nodes)
{
    auto* console_message
        = MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kError, std::move(message));
...
void ViewTransitionStyleTracker::AddTransitionElement(
    Element* element, const AtomicString& name, const AtomicString& nearest_containing_group, const AtomicString& nearest_group_with_contain)
{
    DCHECK(element);

    // Insert an empty hash set for the element if it doesn't exist, or get it if
    // it does.
    auto& value = pending_transition_element_names_.insert(element, HashSet<std::pair<AtomicString, int>>()).stored_value->value;

    if (nearest_containing_group) {
        group_state_map_.Set(name,
            AncestorGroupNames {
...
bool ViewTransitionStyleTracker::MatchForOnlyChild(PseudoId pseudo_id, const AtomicString& view_transition_name) const
{
    switch (pseudo_id) {
...
void ViewTransitionStyleTracker::AddTransitionElementsFromCSS()
{
    ...
    DCHECK_GE(document_->Lifecycle().GetState(), DocumentLifecycle::kCompositingInputsClean);

    Vector<AtomicString> containing_group_stack;

    AddTransitionElementsFromCSSRecursive(
        document_->GetLayoutView()->PaintingLayer(), document_.Get(), containing_group_stack, /*nearest_group_with_contain=*/g_null_atom);
}

AtomicString ViewTransitionStyleTracker::GenerateAutoName(Element& element, const TreeScope* scope)
{
...
void ViewTransitionStyleTracker::AddTransitionElementsFromCSSRecursive(
    PaintLayer* root, const TreeScope* tree_scope, Vector<AtomicString>& containing_group_stack, const AtomicString& nearest_group_with_contain)
{
    ...
    if (view_transition_name && !root_object.IsFragmented()) {
        ...
        auto* relevant_tree_scope
            = RuntimeEnabledFeatures::ViewTransitionTreeScopedNamesEnabled() ? view_transition_name->GetTreeScope() : &node->GetTreeScope();

        if (relevant_tree_scope == tree_scope || !relevant_tree_scope) {
            current_name = view_transition_name->IsAuto() ? GenerateAutoName(*To<Element>(node), tree_scope) : view_transition_name->CustomName();
            AddTransitionElement(DynamicTo<Element>(node), current_name, containing_group_stack.empty() ? g_null_atom : containing_group_stack.back(),
                nearest_group_with_contain);
        }
    }
    ...
    while (auto* child = child_iterator.Next()) {
        AddTransitionElementsFromCSSRecursive(
            child, tree_scope, containing_group_stack, root_style.ViewTransitionGroup().IsNormal() ? nearest_group_with_contain : current_name);
    }
...
bool ViewTransitionStyleTracker::FlattenAndVerifyElements(VectorOf<Element>& elements, VectorOf<AtomicString>& transition_names)
{
    ...
    for (auto& [element, names] : pending_transition_element_names_) {
        ...
        for (auto& name_pair : names) {
            flat_list.push_back(MakeGarbageCollected<FlatData>(element, name_pair.first, name_pair.second));
        }
    }

    // Sort it.
    std::sort(flat_list.begin(), flat_list.end(), [](const FlatData* a, const FlatData* b) { return a->ordering < b->ordering; });
...
AtomicString ViewTransitionStyleTracker::ComputeContainingGroupName(const AtomicString& name, const StyleViewTransitionGroup& group) const
{
...
bool ViewTransitionStyleTracker::Capture(bool snap_browser_controls)
{
    ...
    HeapHashMap<Member<Element>, viz::ViewTransitionElementResourceId> element_snapshot_ids;
    int next_index = 0;
    for (int i = 0; i < captured_name_count_; ++i) {
        ...
        auto& snapshot_id = element_snapshot_ids.insert(element, viz::ViewTransitionElementResourceId()).stored_value->value;
        ...
        // already sorted.
        element_data->containing_group_name = ComputeContainingGroupName(name, element->ComputedStyleRef().ViewTransitionGroup());
        element_data_map_.insert(name, std::move(element_data));
    ...
    for (wtf_size_t i = 0; i < transition_names.size(); ++i) {
        DCHECK_EQ(transition_names.Find(transition_names[i]), i) << " Duplicate transition name: " << transition_names[i];
    }
#endif
    ...
    if (RuntimeEnabledFeatures::PaintHoldingForLocalIframesEnabled() && !document_->GetFrame()->IsLocalRoot()) {
        subframe_snapshot_layer_ = cc::ViewTransitionContentLayer::Create(GenerateResourceId(), /*is_live_content_layer=*/true);
        capture_resource_ids_.push_back(subframe_snapshot_layer_->ViewTransitionResourceId());
    }

    return true;
}

void ViewTransitionStyleTracker::SetCaptureRectsFromCompositor(const std::unordered_map<viz::ViewTransitionElementResourceId, gfx::RectF>& rects)
{
    ...
    for (auto& entry : element_data_map_) {
        ...
        if (!rects.contains(element_data->old_snapshot_id) || !element_data->ShouldPropagateVisualOverflowRectAsMaxExtentsRect()) {
            continue;
        }
        ...
        auto captured_rect = PhysicalRect(gfx::ToEnclosedRect(rect_from_compositor));
        ...
        element_data->cached_visual_overflow_rect_in_layout_space = element_data->visual_overflow_rect_in_layout_space = captured_rect;

        // This rect no longer matters.
        element_data->cached_captured_rect_in_layout_space.reset();

        if (auto* pseudo_element = document_->documentElement()->GetStyledPseudoElement(PseudoId::kPseudoIdViewTransitionOld, entry.key)) {
...
VectorOf<Element> ViewTransitionStyleTracker::GetTransitioningElements() const
{
    ...
    for (auto& entry : element_data_map_) {
        if (entry.value->target_element && !entry.value->target_element->IsDocumentElement()) {
...
const Vector<AtomicString>& ViewTransitionStyleTracker::GetViewTransitionClassList(const AtomicString& name) const
{
    CHECK(element_data_map_.Contains(name));
    return element_data_map_.at(name)->class_list;
}

AtomicString ViewTransitionStyleTracker::GetContainingGroupName(const AtomicString& name) const
{
    if (!RuntimeEnabledFeatures::NestedViewTransitionEnabled() || state_ != State::kStarted) {
        return g_null_atom;
    }
...
bool ViewTransitionStyleTracker::Start()
{
    ...
    if (found_new_names) {
        ...
        std::sort(new_name_pairs.begin(), new_name_pairs.end(),
            [](const std::pair<AtomicString, int>& left, const std::pair<AtomicString, int>& right) { return left.second < right.second; });
        ...
        for (wtf_size_t i = 0; i < new_names.size(); ++i) {
            DCHECK_EQ(new_names.Find(new_names[i]), i) << " Duplicate transition name: " << new_names[i];
        }
#endif
    ...
    DCHECK_GE(document_->Lifecycle().GetState(), DocumentLifecycle::kPrePaintClean);
...
void ViewTransitionStyleTracker::DidThrottleLocalSubframeRendering()
{
    ...
    if (subframe_snapshot_layer_) {
        ...
        subframe_snapshot_layer_ = cc::ViewTransitionContentLayer::Create(resource_id, /*is_live_content_layer=*/false);
    }
}
...
viz::ViewTransitionElementResourceId ViewTransitionStyleTracker::GetSnapshotId(const Element& element) const
{
    ...
    for (const auto& entry : element_data_map_) {
        ...
        if (entry.value->target_element == element) {
            const auto& snapshot_id = HasLiveNewContent() ? entry.value->new_snapshot_id : entry.value->old_snapshot_id;
...
const scoped_refptr<cc::ViewTransitionContentLayer>& ViewTransitionStyleTracker::GetSubframeSnapshotLayer() const
{
    return subframe_snapshot_layer_;
}

PseudoElement* ViewTransitionStyleTracker::CreatePseudoElement(Element* parent, PseudoId pseudo_id, const AtomicString& view_transition_name) const
{
    ...
    switch (pseudo_id) {
    case kPseudoIdViewTransition:
        return MakeGarbageCollected<ViewTransitionTransitionElement>(parent, this);

    case kPseudoIdViewTransitionGroup: {
        return MakeGarbageCollected<ViewTransitionPseudoElementBase>(parent, pseudo_id, view_transition_name, this);
    }
    case kPseudoIdViewTransitionImagePair:
        return MakeGarbageCollected<ImageWrapperPseudoElement>(parent, pseudo_id, view_transition_name, this);

    case kPseudoIdViewTransitionOld: {
        DCHECK(view_transition_name);
        const auto& element_data = element_data_map_.find(view_transition_name)->value;
        ...
        auto border_box_rect = element_data->GetBorderBoxRect(use_cached_data, device_pixel_ratio_);
        ...
        auto* pseudo_element = MakeGarbageCollected<ViewTransitionContentElement>(parent, pseudo_id, view_transition_name, snapshot_id,
            /*is_live_content_element=*/false, this);
        pseudo_element->SetIntrinsicSize(captured_rect, border_box_rect, element_data->ShouldPropagateVisualOverflowRectAsMaxExtentsRect());
...
bool ViewTransitionStyleTracker::RunPostPrePaintSteps()
{
    ...
    DCHECK(document_->documentElement() && document_->documentElement()->GetLayoutObject());
    ...
    const int max_capture_size_in_layout = ComputeMaxCaptureSize(
        *document_, document_->GetPage()->GetChromeClient().GetMaxRenderBufferBounds(*document_->GetFrame()), *snapshot_root_layout_size_at_capture_);

    if (snapshot_root_layout_size_at_capture_->width() > max_capture_size_in_layout
        || snapshot_root_layout_size_at_capture_->height() > max_capture_size_in_layout) {
    ...
    for (auto& entry : element_data_map_) {
        ...
        if (element_data->target_element->IsDocumentElement()) {
            ...
            container_properties = ContainerProperties { PhysicalRect(PhysicalOffset(), layout_view_size_in_css_space), gfx::Transform(), gfx::Vector2dF() };
            visual_overflow_rect_in_layout_space.size = layout_view_size;
        } else {
            ComputeLiveElementGeometry(
                max_capture_size_in_layout, *layout_object, container_properties, visual_overflow_rect_in_layout_space, captured_rect_in_layout_space);
        }

        FlatMapBuilder<CSSPropertyID, String> css_property_builder(std::size(kPropertiesToCapture));

        auto capture_property = [&](CSSPropertyID id) {
            if (const CSSValue* css_value = CSSProperty::Get(id).CSSValueFromComputedStyle(layout_object->StyleRef(),
        ...
        if (element_data->container_properties == container_properties
            && visual_overflow_rect_in_layout_space == element_data->visual_overflow_rect_in_layout_space
            && captured_rect_in_layout_space == element_data->captured_rect_in_layout_space && css_properties == element_data->captured_css_properties) {
            continue;
        }

        element_data->container_properties = container_properties;

        element_data->visual_overflow_rect_in_layout_space = visual_overflow_rect_in_layout_space;
        ...
        PseudoId live_content_element = HasLiveNewContent() ? kPseudoIdViewTransitionNew : kPseudoIdViewTransitionOld;
        DCHECK(document_->documentElement());
        if (auto* pseudo_element = document_->documentElement()->GetStyledPseudoElement(live_content_element, entry.key)) {
            ...
            auto border_box_rect = element_data->GetBorderBoxRect(use_cached_data, device_pixel_ratio_);
            static_cast<ViewTransitionContentElement*>(pseudo_element)
                ->SetIntrinsicSize(captured_rect, border_box_rect, element_data->ShouldPropagateVisualOverflowRectAsMaxExtentsRect());
        }
    ...
    if (LayoutViewTransitionRoot* snapshot_containing_block = document_->GetLayoutView()->GetViewTransitionRoot()) {
...
void ViewTransitionStyleTracker::ComputeLiveElementGeometry(int max_capture_size, LayoutObject& layout_object, ContainerProperties& container_properties,
    PhysicalRect& visual_overflow_rect_in_layout_space, std::optional<gfx::RectF>& captured_rect_in_layout_space) const
{
    ...
    auto snapshot_matrix_in_layout_space = ComputeViewportTransform(layout_object);
    ...
    gfx::Vector2d fixed_to_frame = -document_->GetLayoutView()->OriginAdjustmentForScrollbars();
    ...
    if (layout_object.IsSVGChild() || IsA<LayoutBox>(layout_object)) {
        ...
        auto* resize_observer_entry = MakeGarbageCollected<ResizeObserverEntry>(To<Element>(layout_object.GetNode()));
        auto entry_size = resize_observer_entry->borderBoxSize()[0];
        // ResizeObserver gives us CSS space pixels.
        border_box_size_in_css_space = layout_object.IsHorizontalWritingMode()
            ? PhysicalSize(LayoutUnit(entry_size->inlineSize()), LayoutUnit(entry_size->blockSize()))
            : PhysicalSize(LayoutUnit(entry_size->blockSize()), LayoutUnit(entry_size->inlineSize()));
    } else if (auto* layout_inline = DynamicTo<LayoutInline>(layout_object)) {
        border_box_size_in_css_space = layout_inline->PhysicalLinesBoundingBox().size;
    ...
    if (float effective_zoom = layout_object.StyleRef().EffectiveZoom();
        std::abs(effective_zoom - device_pixel_ratio_) >= std::numeric_limits<float>::epsilon()) {
        border_box_size_in_css_space.Scale(effective_zoom / device_pixel_ratio_);
    }

    snapshot_matrix_in_css_space = ConvertFromTopLeftToCenter(snapshot_matrix_in_css_space, border_box_size_in_css_space);
    ...
    captured_rect_in_layout_space
        = ComputeCaptureRect(max_capture_size, visual_overflow_rect_in_layout_space, snapshot_matrix_in_layout_space, *snapshot_root_layout_size_at_capture_);

    container_properties = ContainerProperties { .border_box_rect_in_css_space = PhysicalRect(offset_in_css_space, border_box_size_in_css_space),
...
bool ViewTransitionStyleTracker::HasActiveAnimations() const
{
    auto pseudo_has_animation = [](PseudoElement* pseudo_element) {
        ...
        for (auto& animation_pair : animations->Animations()) {
            auto animation_play_state = animation_pair.key->CalculateAnimationPlayState();
            if (animation_play_state == V8AnimationPlayState::Enum::kRunning || animation_play_state == V8AnimationPlayState::Enum::kPaused) {
                return true;
            }
        }
        return false;
...
PaintPropertyChangeType ViewTransitionStyleTracker::UpdateCaptureClip(
    const Element& element, const ClipPaintPropertyNodeOrAlias* current_clip, const TransformPaintPropertyNodeOrAlias* current_transform)
{
    for (auto& entry : element_data_map_) {
        auto& element_data = entry.value;
        ...
        ClipPaintPropertyNode::State state(
            *current_transform, *element_data->captured_rect_in_layout_space, FloatRoundedRect(*element_data->captured_rect_in_layout_space));

        if (!element_data->clip_node) {
            element_data->clip_node = ClipPaintPropertyNode::Create(*current_clip, std::move(state));
#if DCHECK_IS_ON()
            element_data->clip_node->SetDebugName(element.DebugName() + "ViewTransition");
#endif
...
const ClipPaintPropertyNode* ViewTransitionStyleTracker::GetCaptureClip(const Element& element) const
{
    for (auto& entry : element_data_map_) {
        auto& element_data = entry.value;
        if (element_data->target_element != &element) {
...
bool ViewTransitionStyleTracker::IsTransitionElement(const Element& element) const
{
...
bool ViewTransitionStyleTracker::NeedsCaptureClipNode(const Element& node) const
{
    if (state_ == State::kIdle || state_ == State::kCaptured) {
        return false;
    }
    ...
    for (auto& entry : element_data_map_) {
        ...
        DCHECK(!entry.value->captured_rect_in_layout_space.has_value() || !entry.value->target_element->IsDocumentElement())
...
StyleRequest::RulesToInclude ViewTransitionStyleTracker::StyleRulesToInclude() const
{
    switch (state_) {
    case State::kIdle:
...
gfx::Outsets GetFixedToSnapshotViewportOutsets(Document& document)
{
    ...
    if (document.GetFrame()->IsOutermostMainFrame()) {
        ...
        bottom += document.GetFrame()->GetWidgetForLocalRoot()->GetVirtualKeyboardResizeHeight();
    }

    PhysicalBoxStrut scrollbar_strut = document.GetLayoutView()->ComputeScrollbars();
...
gfx::Rect ViewTransitionStyleTracker::GetSnapshotRootInFixedViewport() const
{
    ...
    gfx::Rect snapshot_viewport_rect = document_->GetSettings()->GetViewportEnabled()
        ? gfx::Rect(frame_view.Size().width(), frame_view.Size().height())
        : gfx::Rect(layout_view.ClientWidth().ToInt(), layout_view.ClientHeight().ToInt());
...
gfx::Vector2d ViewTransitionStyleTracker::GetFrameToSnapshotRootOffset() const
{
    ...
    gfx::Vector2d frame_to_snapshot = fixed_to_snapshot + document_->GetLayoutView()->OriginAdjustmentForScrollbars();
...
ViewTransitionState ViewTransitionStyleTracker::GetViewTransitionState() const
{
    ...
    transition_state.snapshot_root_size_at_capture = *snapshot_root_layout_size_at_capture_;
    ...
    for (const auto& entry : element_data_map_) {
        ...
        element.border_box_rect_in_enclosing_layer_css_space = gfx::RectF(element_data->container_properties->border_box_rect_in_css_space);
        element.viewport_matrix = element_data->container_properties->snapshot_matrix;
        element.overflow_rect_in_layout_space = gfx::RectF(element_data->visual_overflow_rect_in_layout_space);

        element.snapshot_id = element_data->old_snapshot_id;
        element.paint_order = element_data->element_index;
        element.captured_rect_in_layout_space = element_data->captured_rect_in_layout_space;

        FlatMapBuilder<mojom::blink::ViewTransitionPropertyId, std::string> css_property_builder(element_data->captured_css_properties.size());
        ...
        element.containing_group_name = element_data->containing_group_name ? element_data->containing_group_name.Utf8() : "";
    }
    ...
    if (subframe_snapshot_layer_) {
        transition_state.subframe_snapshot_id = subframe_snapshot_layer_->ViewTransitionResourceId();
    }
...
bool ViewTransitionStyleTracker::SnapshotRootDidChangeSize() const
{
    ...
    if (std::abs(snapshot_root_layout_size_at_capture_->width() - current_size.width()) <= 1
        && std::abs(snapshot_root_layout_size_at_capture_->height() - current_size.height()) <= 1) {
        return false;
    }
...
void ViewTransitionStyleTracker::InvalidateStyle()
{
    ...
    if (auto* originating_element = document_->documentElement()) {
        originating_element->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kViewTransition));
    }

    auto invalidate_style = [](PseudoElement* pseudo_element) {
        pseudo_element->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kViewTransition));
    };
    ...
    for (auto& entry : element_data_map_) {
        ...
        // changed.
        entry.value->target_element->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kViewTransition));
    ...
    document_->GetDisplayLockDocumentState().NotifyViewTransitionPseudoTreeChanged();
}
...
CSSStyleSheet& ViewTransitionStyleTracker::UAStyleSheet()
{
    ...
    for (auto& entry : element_data_map_) {
        ...
        if (element_data->containing_group_name && HasLiveNewContent()) {
            ...
            const auto& containing_group_data = element_data_map_.at(element_data->containing_group_name);
            old_parent_inverse_transform = containing_group_data->cached_container_properties.snapshot_matrix.InverseOrIdentity();

            old_parent_inverse_transform.Translate(-containing_group_data->cached_container_properties.border_offset);
            ...
            if (containing_group_data->container_properties) {
                ...
                new_parent_inverse_transform.Translate(-new_container_properties.border_offset);
            }
        }
        ...
        builder.AddContainerStyles(
            view_transition_name, *element_data->container_properties, element_data->captured_css_properties, new_parent_inverse_transform);
        ...
        if (add_animations) {
            CHECK(element_data->old_snapshot_id.IsValid() || element_data->new_snapshot_id.IsValid());
            ...
            builder.AddAnimations(type, view_transition_name, element_data->cached_container_properties, element_data->cached_animated_css_properties,
    ...
    auto* ua_parser_context = MakeGarbageCollected<CSSParserContext>(kUASheetMode, SecureContextMode::kInsecureContext);

    auto* ua_parser_context_with_base_url = MakeGarbageCollected<CSSParserContext>(
        ua_parser_context, document_->BaseURL(), ua_parser_context->IsOriginClean(), ua_parser_context->GetReferrer(), ua_parser_context->Charset(), nullptr);

    auto* sheet = MakeGarbageCollected<StyleSheetContents>(ua_parser_context_with_base_url);
...
gfx::RectF ViewTransitionStyleTracker::ElementData::GetInkOverflowRect(bool use_cached_data) const
{
    return gfx::RectF(use_cached_data ? cached_visual_overflow_rect_in_layout_space : visual_overflow_rect_in_layout_space);
}

gfx::RectF ViewTransitionStyleTracker::ElementData::GetCapturedSubrect(bool use_cached_data) const
{
    if (RuntimeEnabledFeatures::ViewTransitionOverflowRectFromSurfaceEnabled() && use_cached_data) {
        return GetInkOverflowRect(true);
    }
    auto captured_rect = use_cached_data ? cached_captured_rect_in_layout_space : captured_rect_in_layout_space;
    return captured_rect.value_or(GetInkOverflowRect(use_cached_data));
}

gfx::RectF ViewTransitionStyleTracker::ElementData::GetBorderBoxRect(bool use_cached_data, float device_scale_factor) const
{
    ...
    auto border_box_rect_in_layout_space
        = use_cached_data ? cached_container_properties.border_box_rect_in_css_space : container_properties->border_box_rect_in_css_space;
    border_box_rect_in_layout_space.Scale(device_scale_factor);
    return gfx::RectF(border_box_rect_in_layout_space);
}

bool ViewTransitionStyleTracker::ElementData::ShouldPropagateVisualOverflowRectAsMaxExtentsRect() const
{
    return RuntimeEnabledFeatures::ViewTransitionOverflowRectFromSurfaceEnabled() && target_element && !target_element->IsDocumentElement();
}
...
void ViewTransitionStyleTracker::ElementData::CacheStateForOldSnapshot()
{
    ...
    cached_visual_overflow_rect_in_layout_space = visual_overflow_rect_in_layout_space;
    cached_captured_rect_in_layout_space = captured_rect_in_layout_space;

    FlatMapBuilder<CSSPropertyID, String> builder(std::size(kPropertiesToAnimate));
...
PhysicalRect ViewTransitionStyleTracker::ComputeVisualOverflowRect(LayoutBoxModelObject& box, const LayoutBoxModelObject* ancestor) const
{
    ...
    if (RuntimeEnabledFeatures::ViewTransitionOverflowRectFromSurfaceEnabled()) {
        ...
        // coordinate space.
        auto rect = box.EnclosingLayer()->LocalBoundingBoxIncludingSelfPaintingDescendants();
    ...
    if (ancestor) {
        if (auto* element = DynamicTo<Element>(box.GetNode()); element && IsTransitionElement(*element)) {
            return {};
        }
    }

    const bool visible = box.StyleRef().Visibility() == EVisibility::kVisible || !box.VisualRectRespectsVisibility();
    const bool layered_effects_contribute_to_visual_overflow = ancestor || !ViewTransitionUtils::UseLayeredCapture(box.StyleRef());
    PhysicalRect result;
    ...
    if (layered_effects_contribute_to_visual_overflow) {
        if (auto clip_path_bounds = ClipPathClipper::LocalClipPathBoundingBox(box)) {
            ...
            if (visible) {
                ...
                if (ancestor) {
                    box.MapToVisualRectInAncestorSpace(ancestor, result, kUseGeometryMapper);
                }
            }
    ...
    if (!paint_layer || (!box.ChildPaintBlockedByDisplayLock() && !paint_layer->KnownToClipSubtreeToPaddingBox())) {
        const LayoutBoxModelObject* ancestor_for_recursion = ancestor ? ancestor : &box;
        for (auto* child = box.SlowFirstChild(); child; child = child->NextSibling()) {
            ...
            if (auto* child_box = DynamicTo<LayoutBoxModelObject>(child)) {
                PhysicalRect mapped_overflow_rect = ComputeVisualOverflowRect(*child_box, ancestor_for_recursion);
                ...
                const bool child_visible = child_text->StyleRef().Visibility() == EVisibility::kVisible || !child_text->VisualRectRespectsVisibility();
                if (!child_visible) {
                    continue;
                }

                auto overflow_rect = child_text->VisualOverflowRect();
                child_text->MapToVisualRectInAncestorSpace(ancestor_for_recursion, overflow_rect, kUseGeometryMapper);
    ...
    if (visible) {
        if (auto* layout_box = DynamicTo<LayoutBox>(box)) {
            ...
            if (layout_box->StyleRef().HasVisualOverflowingEffect()) {
                PhysicalBoxStrut outsets = layout_box->ComputeVisualEffectOverflowOutsets();
    ...
    if (ancestor) {
        ...
        // and the ancestor.
        if (visible) {
            box.MapToVisualRectInAncestorSpace(ancestor, overflow_rect, kUseGeometryMapper);
            result.Unite(overflow_rect);
        }
    } else {
        ...
        if (auto* layout_box = DynamicTo<LayoutBox>(&box); layout_box && layout_box->ShouldClipOverflowAlongEitherAxis()) {
...
viz::ViewTransitionElementResourceId ViewTransitionStyleTracker::GenerateResourceId() const
{
...
void ViewTransitionStyleTracker::SnapBrowserControlsToFullyShown()
{
    ...
    if (root_scroller.ScrollPosition().y()) {
        ...
        root_scroller.ScrollBy(ScrollOffset(0, counter_scroll), mojom::blink::ScrollType::kCompositor);
