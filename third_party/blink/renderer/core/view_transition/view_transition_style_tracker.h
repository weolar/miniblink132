...
class ViewTransitionStyleTracker : public GarbageCollected<ViewTransitionStyleTracker> {
public:
    ...
    explicit ViewTransitionStyleTracker(Document& document, const blink::ViewTransitionToken& transition_token);
    ...
    void SetCaptureRectsFromCompositor(const std::unordered_map<viz::ViewTransitionElementResourceId, gfx::RectF>&);
    ...
    PseudoElement* CreatePseudoElement(Element* parent, PseudoId pseudo_id, const AtomicString& view_transition_name) const;
    ...
    int CapturedTagCount() const
    {
        return captured_name_count_;
    }
    ...
    const Vector<AtomicString>& GetViewTransitionClassList(const AtomicString& name) const;
...
private:
    ...
    AtomicString ComputeContainingGroupName(const AtomicString& name, const StyleViewTransitionGroup& group) const;
    ...
    struct ElementData : public GarbageCollected<ElementData> {
        ...
        gfx::RectF GetBorderBoxRect(bool use_cached_data, float device_scale_factor) const;
    ...
    void AddTransitionElement(
        Element*, const AtomicString& name, const AtomicString& nearest_containing_group, const AtomicString& current_containing_group_name);
    bool FlattenAndVerifyElements(VectorOf<Element>&, VectorOf<AtomicString>&);

    void AddTransitionElementsFromCSSRecursive(
        PaintLayer*, const TreeScope*, Vector<AtomicString>& containing_group_stack, const AtomicString& current_containing_group_name);
    ...
    PhysicalRect ComputeVisualOverflowRect(LayoutBoxModelObject& box, const LayoutBoxModelObject* ancestor = nullptr) const;
    ...
    void ComputeLiveElementGeometry(int max_capture_size, LayoutObject& layout_object, ContainerProperties&, PhysicalRect& visual_overflow_rect_in_layout_space,
    ...
    HeapHashMap<Member<Element>, HashSet<std::pair<AtomicString, int>>> pending_transition_element_names_;
    ...
    std::vector<viz::ViewTransitionElementResourceId> capture_resource_ids_ ALLOW_DISCOURAGED_TYPE("cc API uses STL types");
