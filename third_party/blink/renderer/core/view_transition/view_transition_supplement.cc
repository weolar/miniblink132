...
bool HasActiveTransitionInAncestorFrame(LocalFrame* frame)
{
    ...
    while (parent && parent->IsLocalFrame()) {
        if (To<LocalFrame>(parent)->GetDocument() && ViewTransitionUtils::GetTransition(*To<LocalFrame>(parent)->GetDocument())) {
            return true;
        }
...
void SkipTransitionInAllLocalFrames(LocalFrame* curr_frame)
{
    ...
    root_view->ForAllChildLocalFrameViews([curr_frame](LocalFrameView& child) {
        ...
        auto* transition = document ? ViewTransitionUtils::GetTransition(*document) : nullptr;
        if (!transition)
            return;
...
// static
ViewTransitionSupplement* ViewTransitionSupplement::FromIfExists(const Document& document)
{
...
// static
DOMViewTransition* ViewTransitionSupplement::StartViewTransitionInternal(ScriptState* script_state, Document& document, V8ViewTransitionCallback* callback,
    const std::optional<Vector<String>>& types, ExceptionState& exception_state)
{
    DCHECK(script_state);
    ...
    return supplement->StartTransition(document, callback, types, exception_state);
}

DOMViewTransition* ViewTransitionSupplement::startViewTransition(
    ScriptState* script_state, Document& document, V8ViewTransitionCallback* callback, ExceptionState& exception_state)
{
    return StartViewTransitionInternal(script_state, document, callback, std::nullopt, exception_state);
}

DOMViewTransition* ViewTransitionSupplement::startViewTransition(
    ScriptState* script_state, Document& document, ViewTransitionOptions* options, ExceptionState& exception_state)
{
    CHECK(!options || (options->hasUpdate() && options->hasTypes()));
    return StartViewTransitionInternal(
        script_state, document, options ? options->update() : nullptr, options ? options->types() : std::nullopt, exception_state);
}

DOMViewTransition* ViewTransitionSupplement::startViewTransition(ScriptState* script_state, Document& document, ExceptionState& exception_state)
{
    return StartViewTransitionInternal(script_state, document, static_cast<V8ViewTransitionCallback*>(nullptr), std::nullopt, exception_state);
}

DOMViewTransition* ViewTransitionSupplement::StartTransition(
    Document& document, V8ViewTransitionCallback* callback, const std::optional<Vector<String>>& types, ExceptionState& exception_state)
{
    // Disallow script initiated transitions during a navigation initiated
    // transition.
    if (transition_ && !transition_->IsCreatedViaScriptAPI()) {
        return ViewTransition::CreateSkipped(&document, callback)->GetScriptDelegate();
    }
    ...
    DCHECK(!transition_) << "SkipTransition() should finish existing |transition_|";
    ...
    transition_ = ViewTransition::CreateFromScript(&document, callback, types, this);

    if (document.hidden()) {
        auto skipped_transition = transition_;
        skipped_transition->SkipTransition(ViewTransition::PromiseResponse::kRejectInvalidState);
    ...
    if (!RuntimeEnabledFeatures::ConcurrentViewTransitionsSPAEnabled() && HasActiveTransitionInAncestorFrame(document.GetFrame())) {
        auto skipped_transition = transition_;
...
void ViewTransitionSupplement::DidChangeVisibilityState()
{
    if (GetSupplementable()->hidden() && transition_) {
        transition_->SkipTransition(ViewTransition::PromiseResponse::kRejectInvalidState);
    }
...
void ViewTransitionSupplement::SendOptInStatusToHost()
{
    ...
    document->GetFrame()->GetLocalFrameHostRemote().OnViewTransitionOptInChanged(
        (document->domWindow()->HasBeenRevealed() && !document->hidden()) ? cross_document_opt_in_ : mojom::blink::ViewTransitionSameOriginOptIn::kDisabled);
}

void ViewTransitionSupplement::SetCrossDocumentOptIn(mojom::blink::ViewTransitionSameOriginOptIn cross_document_opt_in)
{
...
// static
void ViewTransitionSupplement::SnapshotDocumentForNavigation(Document& document, const blink::ViewTransitionToken& navigation_id,
    mojom::blink::PageSwapEventParamsPtr params, ViewTransition::ViewTransitionStateCallback callback)
{
    DCHECK(RuntimeEnabledFeatures::ViewTransitionOnNavigationEnabled());
    auto* supplement = From(document);
    supplement->StartTransition(document, navigation_id, std::move(params), std::move(callback));
}

void ViewTransitionSupplement::StartTransition(Document& document, const blink::ViewTransitionToken& navigation_id, mojom::blink::PageSwapEventParamsPtr params,
    ViewTransition::ViewTransitionStateCallback callback)
{
    ...
    DCHECK(!transition_) << "SkipTransition() should finish existing |transition_|";
    transition_ = ViewTransition::CreateForSnapshotForNavigation(&document, navigation_id, std::move(callback), cross_document_types_, this);

    auto* page_swap_event = MakeGarbageCollected<PageSwapEvent>(document, std::move(params), transition_->GetScriptDelegate());
    document.domWindow()->DispatchEvent(*page_swap_event);
}

// static
void ViewTransitionSupplement::CreateFromSnapshotForNavigation(Document& document, ViewTransitionState transition_state)
{
    DCHECK(RuntimeEnabledFeatures::ViewTransitionOnNavigationEnabled());
    auto* supplement = From(document);
...
void ViewTransitionSupplement::StartTransition(Document& document, ViewTransitionState transition_state)
{
    DCHECK(!transition_) << "Existing transition on new Document";
    transition_ = ViewTransition::CreateFromSnapshotForNavigation(&document, std::move(transition_state), this);
}

void ViewTransitionSupplement::OnTransitionFinished(ViewTransition* transition)
{
    CHECK(transition);
...
void ViewTransitionSupplement::AddPendingRequest(std::unique_ptr<ViewTransitionRequest> request)
{
...
VectorOf<std::unique_ptr<ViewTransitionRequest>> ViewTransitionSupplement::TakePendingRequests()
{
    return std::move(pending_requests_);
}

void ViewTransitionSupplement::OnViewTransitionsStyleUpdated(bool cross_document_enabled, const Vector<String>& types)
{
    ...
    SetCrossDocumentOptIn(
        cross_document_enabled ? mojom::blink::ViewTransitionSameOriginOptIn::kEnabled : mojom::blink::ViewTransitionSameOriginOptIn::kDisabled);
...
DOMViewTransition* ViewTransitionSupplement::ResolveCrossDocumentViewTransition()
{
    if (!transition_ || !transition_->IsForNavigationOnNewDocument()) {
        return nullptr;
    }
    ...
    if (cross_document_opt_in_ == mojom::blink::ViewTransitionSameOriginOptIn::kDisabled) {
        transition_->SkipTransition();
...
viz::ViewTransitionElementResourceId ViewTransitionSupplement::GenerateResourceId(const blink::ViewTransitionToken& transition_token)
{
    return viz::ViewTransitionElementResourceId(transition_token, ++resource_local_id_sequence_);
}

void ViewTransitionSupplement::InitializeResourceIdSequence(uint32_t next_local_id)
{
    CHECK_GT(next_local_id, viz::ViewTransitionElementResourceId::kInvalidLocalId);
    resource_local_id_sequence_ = std::max(next_local_id - 1, resource_local_id_sequence_);
}
