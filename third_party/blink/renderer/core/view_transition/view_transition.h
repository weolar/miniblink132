...
class CORE_EXPORT ViewTransition : public GarbageCollected<ViewTransition>, public ExecutionContextLifecycleObserver, public ChromeClient::CommitObserver {
public:
    ...
    // script API.
    static ViewTransition* CreateFromScript(Document*, V8ViewTransitionCallback*, const std::optional<Vector<String>>& types, Delegate*);
    ...
    using ViewTransitionStateCallback = base::OnceCallback<void(const ViewTransitionState&)>;
    static ViewTransition* CreateForSnapshotForNavigation(
        Document*, const ViewTransitionToken& transition_token, ViewTransitionStateCallback, const Vector<String>& types, Delegate*);
    ...
    static ViewTransition* CreateFromSnapshotForNavigation(Document*, ViewTransitionState, Delegate*);

    // Script-based constructor.
    ViewTransition(PassKey, Document*, V8ViewTransitionCallback*, const std::optional<Vector<String>>& types, Delegate*);
    ...
    ViewTransition(PassKey, Document*, const ViewTransitionToken& transition_token, ViewTransitionStateCallback, const Vector<String>& types, Delegate*);
    // Navigation-initiated from-snapshot constructor.
    ViewTransition(PassKey, Document*, ViewTransitionState, Delegate*);

    DOMViewTransition* GetScriptDelegate()
    {
        return script_delegate_.Get();
    }
    ...
    // is the only child.
    bool MatchForOnlyChild(PseudoId pseudo_id, const AtomicString& view_transition_name) const;
    ...
    // the given types.
    bool MatchForActiveViewTransitionType(const Vector<AtomicString>& pseudo_types);
    ...
    // transition.
    viz::ViewTransitionElementResourceId GetSnapshotId(const LayoutObject& object) const;

    // The layer used to paint the old Document rendered in a LocalFrame subframe
    // until the new Document can start rendering.
    const scoped_refptr<cc::ViewTransitionContentLayer>& GetSubframeSnapshotLayer() const;
    ...
    PseudoElement* CreatePseudoElement(Element* parent, PseudoId pseudo_id, const AtomicString& view_transition_name);
    ...
    {
        return style_tracker_ ? style_tracker_->GetTransitioningElements() : VectorOf<Element> {};
    }

    bool IsRootTransitioning() const
    {
        return style_tracker_ && document_->documentElement() && style_tracker_->IsTransitionElement(*document_->documentElement());
    }
    ...
    enum class PromiseResponse { kResolve, kRejectAbort, kRejectInvalidState, kRejectTimeout };
    ...
    bool IsGeneratingPseudo(const ViewTransitionPseudoElementBase& pseudo_element) const;
...
private:
    ...
    void NotifyCaptureFinished(const std::unordered_map<viz::ViewTransitionElementResourceId, gfx::RectF>&);
