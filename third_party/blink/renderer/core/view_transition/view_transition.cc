...
ViewTransition::ScopedPauseRendering::ScopedPauseRendering(const Document& document)
{
...
{
    LayoutViewTransitionRoot* transition_root = document_->GetLayoutView()->GetViewTransitionRoot();
...
ViewTransition* ViewTransition::CreateFromScript(
    Document* document, V8ViewTransitionCallback* callback, const std::optional<Vector<String>>& types, Delegate* delegate)
{
    CHECK(document->GetExecutionContext());
    return MakeGarbageCollected<ViewTransition>(PassKey(), document, callback, types, delegate);
}

ViewTransition* ViewTransition::CreateSkipped(Document* document, V8ViewTransitionCallback* callback)
{
    return MakeGarbageCollected<ViewTransition>(PassKey(), document, callback);
}

ViewTransition::ViewTransition(
    PassKey, Document* document, V8ViewTransitionCallback* update_dom_callback, const std::optional<Vector<String>>& types, Delegate* delegate)
    : ExecutionContextLifecycleObserver(document->GetExecutionContext())
    , creation_type_(CreationType::kScript)
    , document_(document)
    , delegate_(delegate)
    , style_tracker_(MakeGarbageCollected<ViewTransitionStyleTracker>(*document_, transition_token_))
    , script_delegate_(MakeGarbageCollected<DOMViewTransition>(*document->GetExecutionContext(), *this, update_dom_callback))
{
...
ViewTransition::ViewTransition(PassKey, Document* document, V8ViewTransitionCallback* update_dom_callback)
    : ExecutionContextLifecycleObserver(document->GetExecutionContext())
    , creation_type_(CreationType::kScript)
    , document_(document)
    , script_delegate_(MakeGarbageCollected<DOMViewTransition>(*document->GetExecutionContext(), *this, update_dom_callback))
...
ViewTransition* ViewTransition::CreateForSnapshotForNavigation(
    Document* document, const ViewTransitionToken& transition_token, ViewTransitionStateCallback callback, const Vector<String>& types, Delegate* delegate)
{
    return MakeGarbageCollected<ViewTransition>(PassKey(), document, transition_token, std::move(callback), types, delegate);
}

ViewTransition::ViewTransition(PassKey, Document* document, const ViewTransitionToken& transition_token, ViewTransitionStateCallback callback,
    const Vector<String>& types, Delegate* delegate)
    : ExecutionContextLifecycleObserver(document->GetExecutionContext())
    ...
    , script_delegate_(MakeGarbageCollected<DOMViewTransition>(*document_->GetExecutionContext(), *this))
{
...
// static
ViewTransition* ViewTransition::CreateFromSnapshotForNavigation(Document* document, ViewTransitionState transition_state, Delegate* delegate)
{
    return MakeGarbageCollected<ViewTransition>(PassKey(), document, std::move(transition_state), delegate);
}

ViewTransition::ViewTransition(PassKey, Document* document, ViewTransitionState transition_state, Delegate* delegate)
    : ExecutionContextLifecycleObserver(document->GetExecutionContext())
    , creation_type_(CreationType::kFromSnapshot)
    , document_(document)
    , delegate_(delegate)
    , transition_token_(transition_state.transition_token)
    , style_tracker_(MakeGarbageCollected<ViewTransitionStyleTracker>(*document_, std::move(transition_state)))
    , script_delegate_(MakeGarbageCollected<DOMViewTransition>(*document_->GetExecutionContext(), *this))
{
    TRACE_EVENT0("blink", "ViewTransition::ViewTransition - CreatingFromSnapshot");
...
void ViewTransition::SkipTransition(PromiseResponse response)
{
    ...
    if (static_cast<int>(state_) > static_cast<int>(State::kCaptureTagDiscovery) && creation_type_ != CreationType::kForSnapshot) {
        delegate_->AddPendingRequest(ViewTransitionRequest::CreateRelease(transition_token_, MaybeCrossFrameSink()));
    }
...
{
    DCHECK(CanAdvanceTo(state)) << "Current state " << static_cast<int>(state_) << " new state " << static_cast<int>(state);
    ...
    if (StateRunsInViewTransitionStepsDuringMainFrame(state_) != in_main_lifecycle_update_) {
...
bool ViewTransition::CanAdvanceTo(State state) const
{
    ...
    switch (state_) {
    case State::kInitial:
        return state == State::kCaptureTagDiscovery || state == State::kWaitForRenderBlock || state == State::kAborted;
    case State::kCaptureTagDiscovery:
    ...
    case State::kCaptured:
        return state == State::kDOMCallbackRunning || state == State::kDOMCallbackFinished || state == State::kAborted
...
// static
bool ViewTransition::StateRunsInViewTransitionStepsDuringMainFrame(State state)
{
    switch (state) {
    case State::kInitial:
        return false;
...
{
    return state == State::kCapturing || state == State::kDOMCallbackRunning || state == State::kWaitForRenderBlock
        || state == State::kTransitionStateCallbackDispatched;
}
...
{
    return state == State::kFinished || state == State::kAborted || state == State::kTimedOut;
}
...
void ViewTransition::ProcessCurrentState()
{
    ...
    while (process_next_state) {
        DCHECK_EQ(in_main_lifecycle_update_, StateRunsInViewTransitionStepsDuringMainFrame(state_));
        TRACE_EVENT1("blink", "ViewTransition::ProcessCurrentState", "state", StateToString(state_));
        ...
        case State::kCaptureRequestPending: {
            ...
            bool snap_browser_controls = document_->GetFrame()->IsOutermostMainFrame()
                && (!RuntimeEnabledFeatures::ViewTransitionDisableSnapBrowserControlsOnHiddenEnabled()
                    || document_->GetPage()->GetBrowserControls().PermittedState() != cc::BrowserControlsState::kHidden)
                && creation_type_ == CreationType::kForSnapshot;
            ...
            delegate_->AddPendingRequest(
                ViewTransitionRequest::CreateCapture(transition_token_, MaybeCrossFrameSink(), style_tracker_->TakeCaptureResourceIds(),
                    ConvertToBaseOnceCallback(CrossThreadBindOnce(&ViewTransition::NotifyCaptureFinished, MakeUnwrappingCrossThreadHandle(this)))));
            ...
            if (document_->GetFrame()->IsLocalRoot()) {
                ...
                document_->GetPage()->GetChromeClient().StopDeferringCommits(*document_->GetFrame(), cc::PaintHoldingCommitTrigger::kViewTransition);
            }
        ...
        case State::kCaptured: {
            ...
            if (creation_type_ == CreationType::kForSnapshot) {
                DCHECK(transition_state_callback_);
                ViewTransitionState view_transition_state = style_tracker_->GetViewTransitionState();
                CHECK_EQ(view_transition_state.transition_token, transition_token_);

                process_next_state = AdvanceTo(State::kTransitionStateCallbackDispatched);
                DCHECK(process_next_state);
        ...
        case State::kDOMCallbackFinished:
            ...
            if (RuntimeEnabledFeatures::SerializeViewTransitionStateInSPAEnabled()) {
                style_tracker_ = MakeGarbageCollected<ViewTransitionStyleTracker>(*document_, style_tracker_->GetViewTransitionState());
            }
        ...
        case State::kAnimateTagDiscovery:
            DCHECK(!in_main_lifecycle_update_);
            document_->View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kViewTransition);
        ...
        case State::kAnimateRequestPending:
            ...
            delegate_->AddPendingRequest(ViewTransitionRequest::CreateAnimateRenderer(transition_token_, MaybeCrossFrameSink()));
...
bool ViewTransition::MatchForOnlyChild(PseudoId pseudo_id, const AtomicString& view_transition_name) const
{
    if (!style_tracker_)
        return false;
...
bool ViewTransition::MatchForActiveViewTransitionType(const Vector<AtomicString>& pseudo_types)
{
    CHECK(RuntimeEnabledFeatures::ViewTransitionTypesEnabled());
    if (IsTerminalState(state_)) {
        return false;
    ...
    return base::ranges::any_of(
        pseudo_types, [&](const String& pseudo_type) { return ViewTransitionTypeSet::IsValidType(pseudo_type) && types_->Contains(pseudo_type); });
}
...
void ViewTransition::NotifyCaptureFinished(const std::unordered_map<viz::ViewTransitionElementResourceId, gfx::RectF>& capture_rects)
{
...
bool ViewTransition::NeedsViewTransitionEffectNode(const LayoutObject& object) const
{
...
bool ViewTransition::NeedsViewTransitionClipNode(const LayoutObject& object) const
{
    ...
    auto* element = DynamicTo<Element>(object.GetNode());
    return element && style_tracker_ && style_tracker_->NeedsCaptureClipNode(*element);
}

bool ViewTransition::IsRepresentedViaPseudoElements(const LayoutObject& object) const
{
    if (IsTerminalState(state_)) {
        return false;
    }
    ...
    if (IsA<LayoutView>(object)) {
        return document_->documentElement() && style_tracker_->IsTransitionElement(*document_->documentElement());
    }
...
bool ViewTransition::IsTransitionElementExcludingRoot(const Element& node) const
{
    if (IsTerminalState(state_)) {
        return false;
    }
...
viz::ViewTransitionElementResourceId ViewTransition::GetSnapshotId(const LayoutObject& object) const
{
...
const scoped_refptr<cc::ViewTransitionContentLayer>& ViewTransition::GetSubframeSnapshotLayer() const
{
    return style_tracker_->GetSubframeSnapshotLayer();
}

PaintPropertyChangeType ViewTransition::UpdateCaptureClip(
    const LayoutObject& object, const ClipPaintPropertyNodeOrAlias* current_clip, const TransformPaintPropertyNodeOrAlias* current_transform)
{
    ...
    auto* element = DynamicTo<Element>(object.GetNode());
    DCHECK(element);
    return style_tracker_->UpdateCaptureClip(*element, current_clip, current_transform);
}

const ClipPaintPropertyNode* ViewTransition::GetCaptureClip(const LayoutObject& object) const
{
    DCHECK(NeedsViewTransitionClipNode(object));
...
{
    DCHECK(document_->Lifecycle().GetState() >= DocumentLifecycle::kPrePaintClean);
    DCHECK(!in_main_lifecycle_update_);

    if (state_ == State::kAnimating && style_tracker_ && !style_tracker_->RunPostPrePaintSteps()) {
        SkipTransition(PromiseResponse::kRejectInvalidState);
    }
}
...
void ViewTransition::RunViewTransitionStepsDuringMainFrame()
{
    ...
    if (style_tracker_ && document_->Lifecycle().GetState() >= DocumentLifecycle::kPrePaintClean && !style_tracker_->RunPostPrePaintSteps()) {
        SkipTransition(PromiseResponse::kRejectInvalidState);
    }
}
...
{
    return state_ == State::kCaptureTagDiscovery || state_ == State::kAnimateTagDiscovery;
}

PseudoElement* ViewTransition::CreatePseudoElement(Element* parent, PseudoId pseudo_id, const AtomicString& view_transition_name)
{
    DCHECK(style_tracker_);

    return style_tracker_->CreatePseudoElement(parent, pseudo_id, view_transition_name);
}
...
void ViewTransition::PauseRendering()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("blink", "ViewTransition::PauseRendering", this);
    static const base::TimeDelta timeout_delay
        = RuntimeEnabledFeatures::ViewTransitionLongCallbackTimeoutForTestingEnabled() ? base::Seconds(15) : base::Seconds(4);
    document_->GetTaskRunner(TaskType::kInternalFrameLifecycleControl)
        ->PostDelayedTask(FROM_HERE, WTF::BindOnce(&ViewTransition::OnRenderingPausedTimeout, WrapWeakPersistent(this)), timeout_delay);
}
...
void ViewTransition::ResumeRendering()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END0("blink", "ViewTransition::PauseRendering", this);
    if (rendering_paused_scope_->ShouldThrottleRendering() && document_->View()) {
...
void ViewTransition::ActivateFromSnapshot()
{
    ...
    v8::MicrotasksScope microtasks_scope(window->GetIsolate(), ToMicrotaskQueue(window), v8::MicrotasksScope::Type::kRunMicrotasks);
...
bool ViewTransition::IsGeneratingPseudo(const ViewTransitionPseudoElementBase& pseudo_element) const
{
