...
ViewTransitionPseudoElementBase::ViewTransitionPseudoElementBase(
    Element* parent, PseudoId pseudo_id, const AtomicString& view_transition_name, const ViewTransitionStyleTracker* style_tracker)
...
bool ViewTransitionPseudoElementBase::CanGeneratePseudoElement(PseudoId pseudo_id) const
{
    switch (GetPseudoId()) {
    ...
    case kPseudoIdViewTransitionGroup:
        return pseudo_id == kPseudoIdViewTransitionImagePair
            || (pseudo_id == kPseudoIdViewTransitionGroup && RuntimeEnabledFeatures::NestedViewTransitionEnabled());
    case kPseudoIdViewTransitionImagePair:
        return pseudo_id == kPseudoIdViewTransitionOld || pseudo_id == kPseudoIdViewTransitionNew;
...
const ComputedStyle* ViewTransitionPseudoElementBase::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
    // Set the parent style to the style of our parent. There is no use
    // for an originating element for a view transition pseudo.
    StyleRequest style_request(GetPseudoId(), ParentOrShadowHostElement()->GetComputedStyle(),
    ...
    if (GetPseudoId() != kPseudoIdViewTransition) {
        style_request.pseudo_ident_list = style_tracker_->GetViewTransitionClassList(view_transition_name());
    }
    ...
    // elements.
    return GetDocument().documentElement()->StyleForPseudoElement(style_recalc_context, style_request);
}
...
bool ViewTransitionPseudoElementBase::IsBoundTo(const blink::ViewTransitionStyleTracker* tracker) const
{
