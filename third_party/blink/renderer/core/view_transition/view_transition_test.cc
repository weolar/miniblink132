...
class ViewTransitionTest : public testing::Test, public PaintTestConfigurations, private ScopedViewTransitionOnNavigationForTest {
public:
    ViewTransitionTest()
        : ScopedViewTransitionOnNavigationForTest(true)
    {
    }
    ...
    {
        return ToScriptStateForMainWorld(web_view_helper_->GetWebView()->MainFrameImpl()->GetFrame());
    }

    Document& GetDocument()
    {
        return *web_view_helper_->GetWebView()->MainFrameImpl()->GetFrame()->GetDocument();
    }
    ...
    void UpdateAllLifecyclePhasesAndFinishDirectives()
    {
        ...
        for (auto& callback : LayerTreeHost()->TakeViewTransitionCallbacksForTesting()) {
    ...
    bool ShouldCompositeForViewTransition(Element* e)
    {
        ...
        auto* transition = ViewTransitionUtils::GetTransition(GetDocument());
        return layout_object && transition && transition->NeedsViewTransitionEffectNode(*layout_object);
    }

    void ValidatePseudoElementTree(const Vector<WTF::AtomicString>& view_transition_names, bool has_incoming_image)
    {
        auto* transition_pseudo = GetDocument().documentElement()->GetPseudoElement(kPseudoIdViewTransition);
        ...
        for (const auto& view_transition_name : view_transition_names) {
            ...
            auto* container_pseudo = transition_pseudo->GetPseudoElement(kPseudoIdViewTransitionGroup, view_transition_name);
            ...
            if (previous_container) {
                EXPECT_EQ(LayoutTreeBuilderTraversal::NextSibling(*previous_container), container_pseudo);
            }
            previous_container = container_pseudo;

            auto* image_wrapper_pseudo = container_pseudo->GetPseudoElement(kPseudoIdViewTransitionImagePair, view_transition_name);

            auto* outgoing_image = image_wrapper_pseudo->GetPseudoElement(kPseudoIdViewTransitionOld, view_transition_name);
            ASSERT_TRUE(outgoing_image);
            EXPECT_TRUE(outgoing_image->GetComputedStyle());
            EXPECT_TRUE(outgoing_image->GetLayoutObject());

            auto* incoming_image = image_wrapper_pseudo->GetPseudoElement(kPseudoIdViewTransitionNew, view_transition_name);
...
TEST_P(ViewTransitionTest, LayoutShift)
{
    ...
    auto* view_transition_callback = V8ViewTransitionCallback::Create(funcs.ExpectCall()->ToV8Function(script_state));

    auto* transition = ViewTransitionSupplement::startViewTransition(script_state, GetDocument(), view_transition_callback, IGNORE_EXCEPTION_FOR_TESTING);

    ScriptPromiseTester finished_tester(script_state, transition->finished(script_state));
    ...
    auto start_requests = ViewTransitionSupplement::From(GetDocument())->TakePendingRequests();
    ...
    auto* transition_pseudo = GetDocument().documentElement()->GetPseudoElement(kPseudoIdViewTransition);
    ASSERT_TRUE(transition_pseudo);
    auto* container_pseudo = transition_pseudo->GetPseudoElement(kPseudoIdViewTransitionGroup, AtomicString("shared"));
    ASSERT_TRUE(container_pseudo);
    ...
    auto* target = To<LayoutBox>(GetDocument().getElementById(AtomicString("target"))->GetLayoutObject());
...
TEST_P(ViewTransitionTest, TransitionCreatesNewObject)
{
    ...
    MockFunctionScope funcs(script_state);
    auto* first_callback = V8ViewTransitionCallback::Create(funcs.ExpectCall()->ToV8Function(script_state));
    auto* second_callback = V8ViewTransitionCallback::Create(funcs.ExpectCall()->ToV8Function(script_state));

    auto* first_transition = ViewTransitionSupplement::startViewTransition(script_state, GetDocument(), first_callback, IGNORE_EXCEPTION_FOR_TESTING);
    auto* second_transition = ViewTransitionSupplement::startViewTransition(script_state, GetDocument(), second_callback, IGNORE_EXCEPTION_FOR_TESTING);
...
TEST_P(ViewTransitionTest, TransitionReadyPromiseResolves)
{
    ...
    ScriptPromiseTester promise_tester(script_state, transition->ready(script_state));
...
TEST_P(ViewTransitionTest, StartTransitionElementsWantToBeComposited)
{
    ...
    e1->setAttribute(html_names::kStyleAttr, AtomicString("view-transition-name: e1"));
    e3->setAttribute(html_names::kStyleAttr, AtomicString("view-transition-name: e3"));

    struct Data {
        STACK_ALLOCATED();

    public:
        Data(Document& document, ScriptState* script_state, ExceptionState& exception_state, Element* e1, Element* e2)
            : document(document)
    ...
    auto start_setup_lambda = [](const v8::FunctionCallbackInfo<v8::Value>& info) {
        auto* data = static_cast<Data*>(info.Data().As<v8::External>()->Value());
        data->document.getElementById(AtomicString("e1"))->setAttribute(html_names::kStyleAttr, g_empty_atom);
        data->document.getElementById(AtomicString("e3"))->setAttribute(html_names::kStyleAttr, g_empty_atom);
        data->e1->setAttribute(html_names::kStyleAttr, AtomicString("view-transition-name: e1"));
        data->e2->setAttribute(html_names::kStyleAttr, AtomicString("view-transition-name: e2"));
    };
    auto start_setup_callback
        = v8::Function::New(script_state->GetContext(), start_setup_lambda, v8::External::New(script_state->GetIsolate(), &data)).ToLocalChecked();

    ViewTransitionSupplement::startViewTransition(script_state, GetDocument(), V8ViewTransitionCallback::Create(start_setup_callback), exception_state);
...
TEST_P(ViewTransitionTest, TransitionCleanedUpBeforePromiseResolution)
{
    ...
    auto* transition = ViewTransitionSupplement::startViewTransition(script_state, GetDocument(), view_transition_callback, IGNORE_EXCEPTION_FOR_TESTING);
    ScriptPromiseTester promise_tester(script_state, transition->finished(script_state));
...
TEST_P(ViewTransitionTest, ViewTransitionPseudoTree)
{
    ...
    public:
        Data(ScriptState* script_state, ExceptionState& exception_state, Document& document)
            : script_state(script_state)
            , exception_state(exception_state)
            , document(document)
        {
    ...
    auto* transition = ViewTransitionSupplement::startViewTransition(
        script_state, GetDocument(), V8ViewTransitionCallback::Create(start_setup_callback), ASSERT_NO_EXCEPTION);

    UpdateAllLifecyclePhasesForTest();

    // The prepare phase should generate the pseudo tree.
    const Vector<AtomicString> view_transition_names = { AtomicString("root"), AtomicString("e1"), AtomicString("e2"), AtomicString("e3") };
    ...
    FinishTransition();
    UpdateAllLifecyclePhasesAndFinishDirectives();
    EXPECT_FALSE(GetDocument().documentElement()->GetPseudoElement(kPseudoIdViewTransition));
}
...
TEST_P(ViewTransitionTest, ViewTransitionElementInvalidation)
{
    ...
    // This callback sets the elements for the start phase of the transition.
    auto start_setup_callback = v8::Function::New(script_state->GetContext(), start_setup_lambda, {}).ToLocalChecked();
...
namespace {
void AssertOnlyViewTransitionElementsInvalidated(PaintArtifactCompositor* compositor)
{
    const char kViewTransition[] = "view-transition";
    const char kLayoutViewTransition[] = "ViewTransition";
    compositor->ForAllContentLayersForTesting([&](ContentLayerClientImpl* client) {
        if (::testing::Matcher<std::string>(::testing::ContainsRegex(kViewTransition)).Matches(client->Layer().DebugName())) {
            return;
        }
        if (::testing::Matcher<std::string>(::testing::ContainsRegex(kLayoutViewTransition)).Matches(client->Layer().DebugName())) {
            return;
        }
        auto* tracking = client->GetRasterInvalidator().GetTracking();
        EXPECT_FALSE(tracking->HasInvalidations()) << client->Layer().DebugName();
...
TEST_P(ViewTransitionTest, InspectorStyleResolver)
{
    ...
    TestCase test_cases[] = { { kPseudoIdViewTransition, false, "::view-transition { background-color: red; }" },
        { kPseudoIdViewTransitionGroup, true, "::view-transition-group(foo) { background-color: blue; }" },
        { kPseudoIdViewTransitionImagePair, true, "::view-transition-image-pair(foo) { background-color: lightblue; }" },
        { kPseudoIdViewTransitionNew, true, "::view-transition-new(foo) { background-color: black; }" },
        { kPseudoIdViewTransitionOld, true, "::view-transition-old(foo) { background-color: grey; }" } };

    for (const auto& test_case : test_cases) {
        InspectorStyleResolver resolver(GetDocument().documentElement(), test_case.pseudo_id, test_case.uses_tags ? AtomicString("foo") : g_null_atom);
        ...
        EXPECT_EQ(pseudo_element_rules->back().first->cssText(), test_case.user_rule);
    }

    InspectorStyleResolver parent_resolver(GetDocument().documentElement(), kPseudoIdNone, g_null_atom);
    for (const auto& test_case : test_cases) {
        ...
        EXPECT_EQ(matched_rules_for_pseudo->view_transition_name, test_case.uses_tags ? AtomicString("foo") : g_null_atom);
...
TEST_P(ViewTransitionTest, VirtualKeyboardDoesntAffectSnapshotSize)
{
    ...
    gfx::Size new_size = gfx::Size(original_size.width(), original_size.height() - kVirtualKeyboardHeight);

    web_view_helper_->Resize(new_size);
    web_view_helper_->LocalMainFrame()->FrameWidgetImpl()->SetVirtualKeyboardResizeHeightForTesting(kVirtualKeyboardHeight);
    ...
    EXPECT_EQ(transition->GetViewTransitionForTest()->GetSnapshotRootSize(), original_size);

    // The height of the ::view-transition should come from the snapshot root
    // rect.
    {
        auto* transition_pseudo = GetDocument().documentElement()->GetPseudoElement(kPseudoIdViewTransition);
        int height = To<LayoutBox>(transition_pseudo->GetLayoutObject())->GetPhysicalFragment(0)->Size().height.ToInt();
    ...
    web_view_helper_->LocalMainFrame()->FrameWidgetImpl()->SetVirtualKeyboardResizeHeightForTesting(0);
...
{
    auto* document = Document::CreateForTest(*GetDocument().GetExecutionContext());
    ...
    DOMViewTransition* transition = ViewTransitionSupplement::startViewTransition(
        script_state, *document, V8ViewTransitionCallback::Create(start_setup_callback), IGNORE_EXCEPTION_FOR_TESTING);
...
TEST_P(ViewTransitionTest, RootEffectLifetime)
{
    ...
    EXPECT_TRUE(transition->GetViewTransitionForTest()->NeedsViewTransitionEffectNode(*GetDocument().GetLayoutView()));
}
...
TEST_P(ViewTransitionTest, PseudoAwareChildTraversal)
{
    ...
    auto start_setup_lambda = [](const v8::FunctionCallbackInfo<v8::Value>& info) {
        auto* document = static_cast<Document*>(info.Data().As<v8::External>()->Value());
        document->documentElement()->classList().Add(AtomicString("transitioned"));
    };

    // This callback sets the elements for the start phase of the transition.
    auto start_setup_callback
        = v8::Function::New(script_state->GetContext(), start_setup_lambda, v8::External::New(script_state->GetIsolate(), &GetDocument())).ToLocalChecked();

    ViewTransitionSupplement::startViewTransition(script_state, GetDocument(), V8ViewTransitionCallback::Create(start_setup_callback), ASSERT_NO_EXCEPTION);
    ...
    EXPECT_EQ(GetDocument().documentElement()->PseudoAwareFirstChild(), static_cast<Node*>(GetDocument().head()));
    EXPECT_EQ(GetDocument().documentElement()->PseudoAwareLastChild(), transition_pseudo);

    // Root is last since it doesn't appear until encountered in the new view.
    auto* foo_group_pseudo = transition_pseudo->GetPseudoElement(kPseudoIdViewTransitionGroup, AtomicString("foo"));
    auto* bar_group_pseudo = transition_pseudo->GetPseudoElement(kPseudoIdViewTransitionGroup, AtomicString("bar"));
    auto* root_group_pseudo = transition_pseudo->GetPseudoElement(kPseudoIdViewTransitionGroup, AtomicString("root"));
    ...
    auto* root_image_pair_pseudo = root_group_pseudo->GetPseudoElement(kPseudoIdViewTransitionImagePair, AtomicString("root"));
    auto* foo_image_pair_pseudo = foo_group_pseudo->GetPseudoElement(kPseudoIdViewTransitionImagePair, AtomicString("foo"));
    auto* bar_image_pair_pseudo = bar_group_pseudo->GetPseudoElement(kPseudoIdViewTransitionImagePair, AtomicString("bar"));
    ...
    auto* foo_old_pseudo = foo_image_pair_pseudo->GetPseudoElement(kPseudoIdViewTransitionOld, AtomicString("foo"));
    auto* foo_new_pseudo = foo_image_pair_pseudo->GetPseudoElement(kPseudoIdViewTransitionNew, AtomicString("foo"));
    ...
    auto* bar_old_pseudo = bar_image_pair_pseudo->GetPseudoElement(kPseudoIdViewTransitionOld, AtomicString("bar"));
    ...
    auto* root_new_pseudo = root_image_pair_pseudo->GetPseudoElement(kPseudoIdViewTransitionNew, AtomicString("root"));
...
TEST_P(ViewTransitionTest, PseudoAwareSiblingTraversal)
{
    ...
    EXPECT_EQ(transition_pseudo->PseudoAwarePreviousSibling(), GetDocument().QuerySelector(AtomicString("body")));
...
TEST_P(ViewTransitionTest, IncludingPseudoTraversal)
{
  ...
  <div id="foo"></div>
    ...
    PseudoElement* vt_group = vt->GetPseudoElement(kPseudoIdViewTransitionGroup, AtomicString("root"));
    PseudoElement* vt_image_pair = vt_group->GetPseudoElement(kPseudoIdViewTransitionImagePair, AtomicString("root"));
    PseudoElement* vt_old = vt_image_pair->GetPseudoElement(kPseudoIdViewTransitionOld, AtomicString("root"));
    PseudoElement* vt_new = vt_image_pair->GetPseudoElement(kPseudoIdViewTransitionNew, AtomicString("root"));
    ...
    HeapVector<Member<Node>> preorder_traversal = { root, html, marker, before, head, body, style, foo, after, vt, vt_group, vt_image_pair, vt_old, vt_new };

    HeapVector<Member<Node>> forward_traversal;
    for (Node* cur = preorder_traversal.front(); cur; cur = NodeTraversal::NextIncludingPseudo(*cur)) {
    ...
    for (Node* cur = preorder_traversal.back(); cur; cur = NodeTraversal::PreviousIncludingPseudo(*cur)) {
        if (cur->IsTextNode()) {
            continue;
        }
...
{
    GetDocument().domWindow()->GetSecurityContext().SetSecurityOriginForTesting(SecurityOrigin::Create(KURL("http://test.com")));
    GetDocument().domWindow()->GetFrame()->Loader().SetIsNotOnInitialEmptyDocument();

    auto* current_item = MakeGarbageCollected<HistoryItem>();
    current_item->SetURL(KURL("http://test.com"));
    GetDocument().domWindow()->navigation()->UpdateCurrentEntryForTesting(*current_item);
    ...
    page_swap_params->navigation_type = mojom::blink::NavigationTypeForNavigationApi::kPush;
    ViewTransitionSupplement::SnapshotDocumentForNavigation(
        GetDocument(), blink::ViewTransitionToken(), std::move(page_swap_params), base::BindOnce([](const ViewTransitionState&) {}));
    ...
    // This callback sets the elements for the start phase of the transition.
    auto start_setup_lambda = [](const v8::FunctionCallbackInfo<v8::Value>& info) {
        auto* callback_issued = static_cast<bool*>(info.Data().As<v8::External>()->Value());
        *callback_issued = true;
    };
    auto start_setup_callback
        = v8::Function::New(script_state->GetContext(), start_setup_lambda, v8::External::New(script_state->GetIsolate(), &callback_issued)).ToLocalChecked();
    DOMViewTransition* script_transition = ViewTransitionSupplement::startViewTransition(
        script_state, GetDocument(), V8ViewTransitionCallback::Create(start_setup_callback), IGNORE_EXCEPTION_FOR_TESTING);
...
TEST_P(ViewTransitionTest, NoEffectOnIframe)
{
    ...
    auto& child_document = *To<LocalFrame>(GetDocument().GetFrame()->Tree().FirstChild())->GetDocument();
    ViewTransitionSupplement::startViewTransition(script_state, child_document, V8ViewTransitionCallback::Create(start_setup_callback), ASSERT_NO_EXCEPTION);

    UpdateAllLifecyclePhasesForTest();
    auto* paint_properties = child_document.GetLayoutView()->FirstFragment().PaintProperties();
