...
class CORE_EXPORT ViewTransitionTransitionElement : public ViewTransitionPseudoElementBase {
public:
    ViewTransitionTransitionElement(Element* parent, const ViewTransitionStyleTracker* style_tracker);
    ~ViewTransitionTransitionElement() override = default;

    PseudoElement* FindViewTransitionGroupPseudoElement(const AtomicString& view_transition_name);
    PseudoElement* FindViewTransitionGroupPseudoElementParent(const AtomicString& view_transition_name);
};

template <> struct DowncastTraits<ViewTransitionTransitionElement> {
