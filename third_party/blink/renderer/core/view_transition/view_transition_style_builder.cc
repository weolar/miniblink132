...
void ViewTransitionStyleBuilder::AddSelector(const String& name, const String& tag)
{
...
void ViewTransitionStyleBuilder::AddRules(const String& selector, const String& tag, const String& rules)
{
...
void ViewTransitionStyleBuilder::AddAnimations(AnimationType type, const String& tag, const ContainerProperties& source_properties,
    const CapturedCssProperties& animated_css_properties, const gfx::Transform& parent_inverse_transform)
{
    switch (type) {
    case AnimationType::kOldOnly:
        AddRules(kOldImageTagName, tag, "animation-name: -ua-view-transition-fade-out");
        break;

    case AnimationType::kNewOnly:
        AddRules(kNewImageTagName, tag, "animation-name: -ua-view-transition-fade-in");
        break;
    ...
    case AnimationType::kBoth:
        ...
        const String& animation_name = AddKeyframes(tag, source_properties, animated_css_properties, parent_inverse_transform);
...
namespace {
std::string GetTransformString(const ViewTransitionStyleBuilder::ContainerProperties& properties, const gfx::Transform& parent_inverse_transform)
{
    gfx::Transform applied_transform(parent_inverse_transform);
    applied_transform.PreConcat(properties.snapshot_matrix);
    return ComputedStyleUtils::ValueForTransform(applied_transform, 1, false)->CssText().Utf8();
}
} // namespace

String ViewTransitionStyleBuilder::AddKeyframes(const String& tag, const ContainerProperties& source_properties,
    const CapturedCssProperties& animated_css_properties, const gfx::Transform& parent_inverse_transform)
{
    ...
    builder_.AppendFormat(
        ...
        GetTransformString(source_properties, parent_inverse_transform).c_str(), source_properties.border_box_rect_in_css_space.Width().ToFloat(),
        source_properties.border_box_rect_in_css_space.Height().ToFloat());

    for (const auto& [id, value] : animated_css_properties) {
        builder_.AppendFormat("%s: %s;\n", CSSProperty::Get(id).GetPropertyNameAtomicString().Utf8().c_str(), value.Utf8().c_str());
    }
    builder_.Append("}}");
    return keyframe_name;
}

void ViewTransitionStyleBuilder::AddContainerStyles(const String& tag, const ContainerProperties& properties,
    const CapturedCssProperties& captured_css_properties, const gfx::Transform& parent_inverse_transform)
{
    StringBuilder rule_builder;
    rule_builder.AppendFormat(
        ...
        properties.border_box_rect_in_css_space.Width().ToFloat(), properties.border_box_rect_in_css_space.Height().ToFloat(),
        GetTransformString(properties, parent_inverse_transform).c_str());
    for (const auto& [id, value] : captured_css_properties) {
        rule_builder.AppendFormat("%s: %s;\n", CSSProperty::Get(id).GetPropertyNameAtomicString().Utf8().c_str(), value.Utf8().c_str());
    }
