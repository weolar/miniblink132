...
V8NavigationType::Enum TypeToEnum(mojom::blink::NavigationTypeForNavigationApi type)
{
    switch (type) {
...
PageSwapEvent::PageSwapEvent(Document& document, mojom::blink::PageSwapEventParamsPtr page_swap_event_params, DOMViewTransition* view_transition)
    ...
    , dom_view_transition_(view_transition)
{
    ...
    CHECK(!view_transition || RuntimeEnabledFeatures::ViewTransitionOnNavigationEnabled());
    ...
    if (page_swap_event_params) {
        ...
        case mojom::blink::NavigationTypeForNavigationApi::kTraverse: {
            ...
            Member<HistoryItem> destination_item = HistoryItem::Create(PageState::CreateFromEncodedData(page_swap_event_params->page_state));
            entry = navigation->GetExistingEntryFor(destination_item->GetNavigationApiKey(), destination_item->GetNavigationApiId());
        } break;
        case mojom::blink::NavigationTypeForNavigationApi::kPush:
        case mojom::blink::NavigationTypeForNavigationApi::kReplace:
            entry = MakeGarbageCollected<NavigationHistoryEntry>(document.domWindow(),
        ...
        activation_->Update(entry, from, TypeToEnum(page_swap_event_params->navigation_type));
    }
}

PageSwapEvent::PageSwapEvent(const AtomicString& type, const PageSwapEventInit* initializer)
    : Event(type, initializer)
    , activation_(initializer ? initializer->activation() : nullptr)
    , dom_view_transition_(initializer ? initializer->viewTransition() : nullptr)
{
}
