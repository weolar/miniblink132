...
const char kInvalidStateMessage[] = "Transition was aborted because of invalid state";
const char kTimeoutMessage[] = "Transition was aborted because of timeout in DOM update";

} // namespace

DOMViewTransition::DOMViewTransition(ExecutionContext& execution_context, ViewTransition& view_transition)
    : DOMViewTransition(execution_context, view_transition,
...
DOMViewTransition::DOMViewTransition(ExecutionContext& execution_context, ViewTransition& view_transition, V8ViewTransitionCallback* update_dom_callback)
    : ExecutionContextLifecycleObserver(&execution_context)
    , execution_context_(&execution_context)
    , view_transition_ { &view_transition }
    , update_dom_callback_(update_dom_callback)
    , finished_promise_property_(MakeGarbageCollected<PromiseProperty>(execution_context_))
    , ready_promise_property_(MakeGarbageCollected<PromiseProperty>(execution_context_))
    , dom_updated_promise_property_(MakeGarbageCollected<PromiseProperty>(execution_context_))
{
...
ScriptPromise<IDLUndefined> DOMViewTransition::finished(ScriptState* script_state) const
{
    return finished_promise_property_->Promise(script_state->World());
}

ScriptPromise<IDLUndefined> DOMViewTransition::ready(ScriptState* script_state) const
{
    return ready_promise_property_->Promise(script_state->World());
}

ScriptPromise<IDLUndefined> DOMViewTransition::updateCallbackDone(ScriptState* script_state) const
{
    return dom_updated_promise_property_->Promise(script_state->World());
}

void DOMViewTransition::DidSkipTransition(ViewTransition::PromiseResponse response)
{
    ...
    if (dom_callback_result_ == DOMCallbackResult::kNotInvoked) {
        execution_context_->GetTaskRunner(TaskType::kMiscPlatformAPI)
            ->PostTask(FROM_HERE, WTF::BindOnce(&DOMViewTransition::InvokeDOMChangeCallback, WrapPersistent(this)));
        ...
        CHECK_EQ(finished_promise_property_->GetState(), PromiseProperty::State::kRejected);
        ...
        AtMicrotask(ViewTransition::PromiseResponse::kResolve, finished_promise_property_);
    }
}
...
{
    AtMicrotask(ViewTransition::PromiseResponse::kResolve, ready_promise_property_);
}
...
class DOMChangeFinishedCallback : public ThenCallable<IDLUndefined, DOMChangeFinishedCallback> {
public:
...
class DOMChangeRejectedCallback : public ThenCallable<IDLAny, DOMChangeRejectedCallback> {
public:
...
{
    CHECK_EQ(dom_callback_result_, DOMCallbackResult::kNotInvoked) << "UpdateDOM callback invoked multiple times.";
    ...
    ScriptState* script_state = update_dom_callback_ ? update_dom_callback_->CallbackRelevantScriptState() : ToScriptStateForMainWorld(execution_context_);
    ScriptState::Scope scope(script_state);

    if (update_dom_callback_) {
        v8::Maybe<ScriptPromise<IDLUndefined>> maybe_result = update_dom_callback_->Invoke(nullptr);
        ...
        if (maybe_result.IsNothing()) {
            result = ScriptPromise<IDLUndefined>::RejectWithDOMException(
                script_state, MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, kAbortedMessage));
        } else {
    ...
    result.React(script_state, MakeGarbageCollected<DOMChangeFinishedCallback>(*this), MakeGarbageCollected<DOMChangeRejectedCallback>(*this));
}
...
void DOMViewTransition::AtMicrotask(ViewTransition::PromiseResponse response, PromiseProperty* property)
{
    if (!execution_context_) {
        return;
    }
    execution_context_->GetAgent()->event_loop()->EnqueueMicrotask(
        WTF::BindOnce(&DOMViewTransition::HandlePromise, WrapPersistent(this), response, WrapPersistent(property)));
}

void DOMViewTransition::HandlePromise(ViewTransition::PromiseResponse response, PromiseProperty* property)
{
    if (!execution_context_) {
        return;
    }
    ...
    ScriptState* main_world_script_state = ToScriptStateForMainWorld(execution_context_);
    ...
    case ViewTransition::PromiseResponse::kRejectAbort: {
        ...
        auto value = ScriptValue::From(main_world_script_state, MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, kAbortedMessage));
        property->Reject(value);
        break;
    }
    case ViewTransition::PromiseResponse::kRejectInvalidState: {
        ScriptState::Scope scope(main_world_script_state);
        auto value = ScriptValue::From(main_world_script_state, MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, kInvalidStateMessage));
        property->Reject(value);
        break;
    }
    case ViewTransition::PromiseResponse::kRejectTimeout: {
        ScriptState::Scope scope(main_world_script_state);
        auto value = ScriptValue::From(main_world_script_state, MakeGarbageCollected<DOMException>(DOMExceptionCode::kTimeoutError, kTimeoutMessage));
        property->Reject(value);
        break;
    }
    }
