...
public:
    template <typename Functor> static void ForEachTransitionPseudo(Document& document, Functor& func)
    {
        if (!document.documentElement()) {
            return;
        }

        auto* transition_pseudo = document.documentElement()->GetPseudoElement(kPseudoIdViewTransition);
        if (!transition_pseudo)
            return;

        func(transition_pseudo);

        for (const auto& view_transition_name : document.GetStyleEngine().ViewTransitionTags()) {
            ...
            if (auto* content = wrapper_pseudo->GetPseudoElement(kPseudoIdViewTransitionOld, view_transition_name)) {
                func(content);
            }

            if (auto* content = wrapper_pseudo->GetPseudoElement(kPseudoIdViewTransitionNew, view_transition_name)) {
                func(content);
            }
        }
    }

    template <typename Functor> static PseudoElement* FindPseudoIf(const Document& document, const Functor& condition)
    {
        if (!document.documentElement()) {
            return nullptr;
        }
        ...
        for (const auto& view_transition_name : document.GetStyleEngine().ViewTransitionTags()) {
            ...
            if (auto* content = wrapper_pseudo->GetPseudoElement(kPseudoIdViewTransitionOld, view_transition_name); content && condition(content)) {
                return content;
            }

            if (auto* content = wrapper_pseudo->GetPseudoElement(kPseudoIdViewTransitionNew, view_transition_name); content && condition(content)) {
                return content;
            }
        }
    ...
    template <typename Functor> static void ForEachDirectTransitionPseudo(const Element* element, Functor& func)
    {
        ...
        case kPseudoIdViewTransition:
            for (auto name : element->GetDocument().GetStyleEngine().ViewTransitionTags()) {
                if (auto* pseudo = element->GetPseudoElement(kPseudoIdViewTransitionGroup, name)) {
                    func(pseudo);
                }
            }
            break;
        case kPseudoIdViewTransitionGroup:
            if (auto* pseudo = element->GetPseudoElement(kPseudoIdViewTransitionImagePair)) {
                func(pseudo);
            }
            break;
    ...
    static ViewTransition* GetIncomingCrossDocumentTransition(const Document& document);

    // Return the outgoing cross-document view transition, if one exists.
    static ViewTransition* GetOutgoingCrossDocumentTransition(const Document& document);
    ...
    // null).
    static DOMViewTransition* GetTransitionScriptDelegate(const Document& document);
    ...
    static VectorOf<std::unique_ptr<ViewTransitionRequest>> GetPendingRequests(const Document& document);
    ...
    static bool IsViewTransitionElementExcludingRootFromSupplement(const Element& element);
    ...
    static bool IsViewTransitionParticipantFromSupplement(const LayoutObject& object);
    static bool UseLayeredCapture(const ComputedStyle& style)
    {
        return RuntimeEnabledFeatures::ViewTransitionLayeredCaptureEnabled() && style.ViewTransitionCaptureMode() == StyleViewTransitionCaptureMode::kLayered;
    }
    static bool ShouldDelegateEffectsAndBoxDecorationsToViewTransitionGroup(const LayoutObject&);
};
