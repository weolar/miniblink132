...
class ReadableStreamBYOBReader::BYOBReaderReadIntoRequest final : public ReadIntoRequest {
public:
    explicit BYOBReaderReadIntoRequest(ScriptPromiseResolver<ReadableStreamReadResult>* resolver)
        : resolver_(resolver)
    {
    }

    void ChunkSteps(ScriptState* script_state, DOMArrayBufferView* chunk, ExceptionState& exception_state) const override
    {
        auto* read_result = ReadableStreamReadResult::Create();
        read_result->setValue(ScriptValue(script_state->GetIsolate(), ToV8Traits<DOMArrayBufferView>::ToV8(script_state, chunk)));
    ...
    {
        auto* read_result = ReadableStreamReadResult::Create();
        read_result->setValue(ScriptValue(script_state->GetIsolate(),
            chunk ? ToV8Traits<DOMArrayBufferView>::ToV8(script_state, chunk) : static_cast<v8::Local<v8::Value>>(v8::Undefined(script_state->GetIsolate()))));
...
ReadableStreamBYOBReader* ReadableStreamBYOBReader::Create(ScriptState* script_state, ReadableStream* stream, ExceptionState& exception_state)
{
...
ReadableStreamBYOBReader::ReadableStreamBYOBReader(ScriptState* script_state, ReadableStream* stream, ExceptionState& exception_state)
{
...
ScriptPromise<ReadableStreamReadResult> ReadableStreamBYOBReader::read(
    ScriptState* script_state, NotShared<DOMArrayBufferView> view, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#byob-reader-read
    // 1. If view.[[ByteLength]] is 0, return a promise rejected with a TypeError
    // exception.
    if (view->byteLength() == 0) {
        exception_state.ThrowTypeError("This readable stream reader cannot be used to read as the view has "
    ...
    if (view->buffer()->ByteLength() == 0) {
        exception_state.ThrowTypeError("This readable stream reader cannot be used to read as the viewed "
    ...
    if (!owner_readable_stream_) {
        exception_state.ThrowTypeError("This readable stream reader has been released and cannot be used to "
                                       "read from its previous owner stream");
        return EmptyPromise();
    }

    // 5. Let promise be a new promise.
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<ReadableStreamReadResult>>(script_state, exception_state.GetContext());
    ...
    auto* read_into_request = MakeGarbageCollected<BYOBReaderReadIntoRequest>(resolver);
...
void ReadableStreamBYOBReader::Read(ScriptState* script_state, ReadableStreamBYOBReader* reader, NotShared<DOMArrayBufferView> view,
    ReadIntoRequest* read_into_request, ExceptionState& exception_state)
{
    ...
    if (stream->state_ == ReadableStream::kErrored) {
        read_into_request->ErrorSteps(script_state, stream->GetStoredError(script_state->GetIsolate()));
    } else {
        // 5. Otherwise, perform !
        ...
        ReadableByteStreamController::PullInto(script_state, To<ReadableByteStreamController>(controller), view, read_into_request, exception_state);
    }
}

void ReadableStreamBYOBReader::ErrorReadIntoRequests(ScriptState* script_state, ReadableStreamBYOBReader* reader, v8::Local<v8::Value> e)
{
...
void ReadableStreamBYOBReader::Release(ScriptState* script_state, ReadableStreamBYOBReader* reader)
{
    ...
    v8::Local<v8::Value> e = V8ThrowException::CreateTypeError(script_state->GetIsolate(), "Releasing BYOB reader");

    // 3. Perform ! ReadableStreamBYOBReaderErrorReadIntoRequests(reader, e).
    ErrorReadIntoRequests(script_state, reader, e);
}

void ReadableStreamBYOBReader::releaseLock(ScriptState* script_state, ExceptionState& exception_state)
{
...
void ReadableStreamBYOBReader::SetUpBYOBReader(
    ScriptState* script_state, ReadableStreamBYOBReader* reader, ReadableStream* stream, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#set-up-readable-stream-byob-reader
    // If ! IsReadableStreamLocked(stream) is true, throw a TypeError exception.
    if (ReadableStream::IsLocked(stream)) {
        exception_state.ThrowTypeError("ReadableStreamBYOBReader constructor can only accept readable streams "
    ...
    if (!stream->readable_stream_controller_->IsByteStreamController()) {
        exception_state.ThrowTypeError("Cannot use a BYOB reader with a non-byte stream");
        return;
    }
