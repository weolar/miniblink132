...
public:
    ...
    virtual ScriptPromise<IDLUndefined> start(ScriptState*, WritableStreamDefaultController*, ExceptionState&) = 0;
    virtual ScriptPromise<IDLUndefined> write(ScriptState*, ScriptValue chunk, WritableStreamDefaultController*, ExceptionState&) = 0;
    virtual ScriptPromise<IDLUndefined> close(ScriptState*, ExceptionState&) = 0;
    virtual ScriptPromise<IDLUndefined> abort(ScriptState*, ScriptValue reason, ExceptionState&) = 0;

    ScriptPromise<IDLUndefined> start(ScriptState*, ScriptValue controller, ExceptionState&);

    ScriptPromise<IDLUndefined> write(ScriptState* script_state, ScriptValue chunk, ScriptValue controller, ExceptionState& exception_state)
    {
        DCHECK(controller_);
