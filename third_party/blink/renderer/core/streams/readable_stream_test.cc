...
public:
    ...
    std::optional<String> ReadAll(V8TestingScope& scope, ReadableStream* stream)
    {
        ...
        v8::Local<v8::Value> v8_stream = ToV8Traits<ReadableStream>::ToV8(script_state, stream);
        v8::Local<v8::Object> global = context->Global();
        bool set_result = false;
        if (!global->Set(context, V8String(isolate, "stream"), v8_stream).To(&set_result)) {
            ADD_FAILURE();
            return std::nullopt;
        }
...
class TestTransferringOptimizer final : public ReadableStreamTransferringOptimizer {
...
TEST_F(ReadableStreamTest, CreateWithoutArguments)
{
    ...
    ReadableStream* stream = ReadableStream::Create(scope.GetScriptState(), scope.GetExceptionState());
    ASSERT_TRUE(stream);
...
TEST_F(ReadableStreamTest, CreateWithUnderlyingSourceOnly)
{
    V8TestingScope scope;
    ScriptState* script_state = scope.GetScriptState();
    auto* underlying_source = MakeGarbageCollected<TestUnderlyingSource>(script_state);
    EXPECT_FALSE(underlying_source->IsStartCalled());

    ReadableStream* stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, underlying_source, 0);
...
TEST_F(ReadableStreamTest, CreateWithPathologicalStrategy)
{
    ...
    ScriptValue js_pathological_strategy = EvalWithPrintingError(&scope, "({get size() { throw Error('e'); }})");
    ASSERT_FALSE(js_pathological_strategy.IsEmpty());

    ReadableStream* stream
        = ReadableStream::Create(scope.GetScriptState(), ScriptValue(isolate, v8::Undefined(isolate)), js_pathological_strategy, scope.GetExceptionState());
    ASSERT_FALSE(stream);
...
TEST_F(ReadableStreamTest, GetReader)
{
    ...
    ReadableStreamDefaultReader* reader = stream->GetDefaultReaderForTesting(script_state, ASSERT_NO_EXCEPTION);
...
{
    V8TestingScope scope;
    ScriptValue byte_stream = EvalWithPrintingError(&scope, "new ReadableStream({type: 'b' + 'ytes'})");
...
TEST_F(ReadableStreamTest, GetBYOBReader)
{
    ...
    ScriptValue byte_stream = EvalWithPrintingError(&scope, "new ReadableStream({type: 'bytes'})");
    ReadableStream* stream { V8ReadableStream::ToWrappable(isolate, byte_stream.V8Value()) };
    ASSERT_TRUE(stream);
    ...
    if (const auto* result = stream->getReader(script_state, options, ASSERT_NO_EXCEPTION)) {
...
TEST_F(ReadableStreamTest, CancelWithNull)
{
    ...
    stream->cancel(script_state, ScriptValue(isolate, v8::Null(isolate)), ASSERT_NO_EXCEPTION);
...
TEST_F(ReadableStreamTest, Close)
{
    ...
    auto* underlying_source = MakeGarbageCollected<TestUnderlyingSource>(script_state);
    auto* stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, underlying_source, 0);
...
TEST_F(ReadableStreamTest, Serialize)
{
    ...
    auto* channel = MakeGarbageCollected<MessageChannel>(scope.GetExecutionContext());

    stream->Serialize(script_state, channel->port1(), ASSERT_NO_EXCEPTION);
    EXPECT_TRUE(stream->IsLocked());

    auto* transferred = ReadableStream::Deserialize(script_state, channel->port2(),
    ...
    EXPECT_EQ(ReadAll(scope, transferred), std::make_optional<String>("hello, bye"));
}
...
TEST_F(ReadableStreamTest, DeserializeWithNullOptimizer)
{
    ...
    auto* transferred = ReadableStream::Deserialize(script_state, channel->port2(), std::move(optimizer), ASSERT_NO_EXCEPTION);
    ASSERT_TRUE(transferred);
...
TEST_F(ReadableStreamTest, DeserializeWithTestOptimizer)
{
    ...
    EXPECT_EQ(ReadAll(scope, transferred), std::make_optional<String>("hello, byefoo, bar"));
}
...
TEST_F(ReadableStreamTest, GarbageCollectJavaScriptUnderlyingSource)
{
...

    {
        ...
        ReadableStream::Create(scope.GetScriptState(), ScriptValue(isolate, underlying_source), ASSERT_NO_EXCEPTION);
...
TEST_F(ReadableStreamTest, GarbageCollectCPlusPlusUnderlyingSource)
{
    ...
    {
        v8::HandleScope handle_scope(isolate);
        auto* underlying_source = MakeGarbageCollected<NoopUnderlyingSource>(scope.GetScriptState());
        weak_underlying_source = underlying_source;
        ReadableStream::CreateWithCountQueueingStrategy(scope.GetScriptState(), underlying_source, 0);
    }
...
public:
    ...
    void Init(ScriptState* script_state, UnderlyingByteSourceBase* underlying_byte_source)
    {
        stream_ = ReadableStream::CreateByteStream(script_state, underlying_byte_source);
    }
    ...
    void CopyStreamToGlobal(const V8TestingScope& scope)
    {
        ...
        EXPECT_TRUE(global->Set(scope.GetContext(), V8String(scope.GetIsolate(), "stream"), ToV8Traits<ReadableStream>::ToV8(script_state, stream)).IsJust());
    }
...
public:
    ...
    MOCK_METHOD2(Pull, ScriptPromise<IDLUndefined>(ReadableByteStreamController*, ExceptionState&));
    MOCK_METHOD0(Cancel, ScriptPromise<IDLUndefined>());
    MOCK_METHOD1(Cancel, ScriptPromise<IDLUndefined>(v8::Local<v8::Value> reason));
...
{
    V8TestingScope scope;
    Init(scope.GetScriptState(), MakeGarbageCollected<TestUnderlyingByteSource>(scope.GetScriptState()));
    EXPECT_TRUE(Stream());
}
...
{
    V8TestingScope scope;
    auto* mock = MakeGarbageCollected<MockUnderlyingByteSource>(scope.GetScriptState());
    Init(scope.GetScriptState(), mock);
    // Need to run microtasks so the startAlgorithm promise resolves.
    scope.PerformMicrotaskCheckpoint();
    CopyStreamToGlobal(scope);

    EXPECT_CALL(*mock, Pull(_, _)).WillOnce(Return(ByMove(ToResolvedUndefinedPromise(scope.GetScriptState()))));

    EvalWithPrintingError(&scope, "stream.getReader({ mode: 'byob' }).read(new Uint8Array(1));\n");
...
TEST_F(ReadableByteStreamTest, CancelIsCalled)
{
    ...
    EXPECT_CALL(*mock, Cancel(_)).WillOnce(Return(ByMove(ToResolvedUndefinedPromise(scope.GetScriptState()))));
...
TEST_F(ReadableByteStreamTest, ThrowFromPull)
{
    ...
    class ThrowFromPullUnderlyingByteSource final : public TestUnderlyingByteSource {
    public:
        ...
        void PullVoid(ReadableByteStreamController*, ExceptionState& exception_state) override
        {
    ...
    V8TestingScope scope;
    auto* script_state = scope.GetScriptState();
    Init(script_state, MakeGarbageCollected<ThrowFromPullUnderlyingByteSource>(script_state));

    auto* reader = Stream()->GetBYOBReaderForTesting(script_state, ASSERT_NO_EXCEPTION);
    NotShared<DOMArrayBufferView> view = NotShared<DOMUint8Array>(DOMUint8Array::Create(1));
    ScriptPromiseTester read_tester(script_state, reader->read(script_state, view, ASSERT_NO_EXCEPTION));
...
TEST_F(ReadableByteStreamTest, ThrowFromCancel)
{
    ...
    class ThrowFromCancelUnderlyingByteSource final : public TestUnderlyingByteSource {
    public:
        ...
        {
            V8ThrowException::ThrowTypeError(GetScriptState()->GetIsolate(), kMessage);
        }
    };

    V8TestingScope scope;
    auto* script_state = scope.GetScriptState();
    Init(script_state, MakeGarbageCollected<ThrowFromCancelUnderlyingByteSource>(script_state));

    auto* reader = Stream()->GetBYOBReaderForTesting(script_state, ASSERT_NO_EXCEPTION);
    ScriptPromiseTester read_tester(script_state, reader->cancel(script_state, ASSERT_NO_EXCEPTION));
    read_tester.WaitUntilSettled();
    EXPECT_TRUE(read_tester.IsRejected());
    EXPECT_TRUE(IsTypeError(script_state, read_tester.Value(), kMessage));
}
...
TEST_F(ReadableByteStreamTest, CloseStream)
{
    V8TestingScope scope;
    auto* script_state = scope.GetScriptState();
    Init(script_state, MakeGarbageCollected<TestUnderlyingByteSource>(script_state));
    EXPECT_TRUE(Stream());
