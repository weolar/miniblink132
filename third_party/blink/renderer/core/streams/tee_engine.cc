...
v8::MaybeLocal<v8::Value> TeeEngine::StructuredClone(ScriptState* script_state, v8::Local<v8::Value> chunk, ExceptionState& exception_state)
{
    ...
    scoped_refptr<SerializedScriptValue> serialized
        = SerializedScriptValue::Serialize(isolate, chunk, SerializedScriptValue::SerializeOptions(SerializedScriptValue::kNotForStorage), exception_state);
    if (exception_state.HadException()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "chunk could not be cloned.");
...
public:
    explicit PullAlgorithm(TeeEngine* engine)
        : engine_(engine)
    {
    }

    v8::Local<v8::Promise> Run(ScriptState* script_state, int, v8::Local<v8::Value>[]) override
    {
        ...
        ReadableStreamDefaultReader::Read(script_state, engine_->reader_, read_request, exception_state);
...
private:
    ...
    public:
        explicit TeeReadRequest(TeeEngine* engine)
            : engine_(engine)
        {
        }
        ...
        void ChunkSteps(ScriptState* script_state, v8::Local<v8::Value> chunk, ExceptionState&) const override
        {
            ...
            event_loop->EnqueueMicrotask(WTF::BindOnce(&TeeReadRequest::ChunkStepsBody, WrapPersistent(this), WrapPersistent(script_state), std::move(value)));
        }
        ...
        void CloseSteps(ScriptState* script_state) const override
        {
            ...
            for (int branch = 0; branch < 2; ++branch) {
                if (!engine_->canceled_[branch] && ReadableStreamDefaultController::CanCloseOrEnqueue(engine_->controller_[branch])) {
                    ReadableStreamDefaultController::Close(script_state, engine_->controller_[branch]);
                }
            }
    ...
    private:
        void ChunkStepsBody(ScriptState* script_state, v8::Global<v8::Value> value) const
        {
            ...
            if (!engine_->canceled_[1] && engine_->clone_for_branch2_) {
                ...
                v8::MaybeLocal<v8::Value> clone_result_maybe = engine_->StructuredClone(script_state, chunk[1], PassThroughException(isolate));
                ...
                if (!clone_result_maybe.ToLocal(&clone_result)) {
                    ...
                    //     cloneResult.[[Value]]).
                    ReadableStreamDefaultController::Error(script_state, engine_->controller_[0], exception);
                    //     ii. Perform !
                    //     ReadableStreamDefaultControllerError(branch2.[[controller]],
                    //     cloneResult.[[Value]]).
                    ReadableStreamDefaultController::Error(script_state, engine_->controller_[1], exception);
                    //     iii. Resolve cancelPromise with !
                    //     ReadableStreamCancel(stream, cloneResult.[[Value]]).
                    engine_->cancel_promise_->Resolve(ReadableStream::Cancel(script_state, engine_->stream_, exception));
                    //     iv. Return.
                    return;
                } else {
            ...
            for (int branch = 0; branch < 2; ++branch) {
                if (!engine_->canceled_[branch] && ReadableStreamDefaultController::CanCloseOrEnqueue(engine_->controller_[branch])) {
                    ReadableStreamDefaultController::Enqueue(script_state, engine_->controller_[branch], chunk[branch], PassThroughException(isolate));
                    if (try_catch.HasCaught()) {
                        ...
                        ReadableStreamDefaultController::Error(script_state, engine_->controller_[branch], try_catch.Exception());
                        return;
                    }
                }
            }
...
void TeeEngine::Start(ScriptState* script_state, ReadableStream* stream, bool clone_for_branch2, ExceptionState& exception_state)
{
    ...
    //   cancel1Algorithm).
    branch_[0] = ReadableStream::Create(script_state, start_algorithm, pull_algorithm, cancel1_algorithm, 1.0, size_algorithm, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    //   cancel2Algorithm).
    branch_[1] = ReadableStream::Create(script_state, start_algorithm, pull_algorithm, cancel2_algorithm, 1.0, size_algorithm, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    public:
        explicit RejectFunction(TeeEngine* engine)
            : engine_(engine)
        {
        }
        ...
        void React(ScriptState* script_state, ScriptValue r)
        {
            ...
            ReadableStreamDefaultController::Error(script_state, engine_->controller_[0], r.V8Value());

            //   b. Perform ! ReadableStreamDefaultControllerError(branch2.
            //      [[readableStreamController]], r).
            ReadableStreamDefaultController::Error(script_state, engine_->controller_[1], r.V8Value());
    ...
    reader_->closed(script_state).Catch(script_state, MakeGarbageCollected<RejectFunction>(this));
