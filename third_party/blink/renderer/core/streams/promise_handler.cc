...
v8::Local<v8::Promise> AttemptToReturnDummyPromise(v8::Local<v8::Context> context, v8::Local<v8::Promise> original_promise)
{
...
void PromiseHandler::CallRaw(ScriptState* script_state, const v8::FunctionCallbackInfo<v8::Value>& args)
{
...
void PromiseHandlerWithValue::CallRaw(ScriptState* script_state, const v8::FunctionCallbackInfo<v8::Value>& args)
{
    DCHECK_EQ(args.Length(), 1);
...
v8::Local<v8::Promise> StreamThenPromise(ScriptState* script_state, v8::Local<v8::Promise> promise, ScriptFunction* on_fulfilled, ScriptFunction* on_rejected)
{
    ...
    if (!on_fulfilled) {
        ...
        result_maybe = promise->Then(context, noop, on_rejected->ToV8Function(script_state));
    } else if (on_rejected) {
        result_maybe = promise->Then(context, on_fulfilled->ToV8Function(script_state), on_rejected->ToV8Function(script_state));
    } else {
        result_maybe = promise->Then(context, on_fulfilled->ToV8Function(script_state));
    }

    v8::Local<v8::Promise> result;
    if (!result_maybe.ToLocal(&result)) {
        DVLOG(3) << "assuming that failure of promise->Then() is caused by shutdown and"
                    "ignoring it";
