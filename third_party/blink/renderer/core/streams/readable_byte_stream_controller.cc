...
template <typename DOMType> DOMArrayBufferView* CreateAsArrayBufferView(DOMArrayBuffer* buffer, size_t byte_offset, size_t length)
{
...
ReadableByteStreamController::QueueEntry::QueueEntry(DOMArrayBuffer* buffer, size_t byte_offset, size_t byte_length)
    : buffer(buffer)
...
ReadableByteStreamController::PullIntoDescriptor::PullIntoDescriptor(DOMArrayBuffer* buffer, size_t buffer_byte_length, size_t byte_offset, size_t byte_length,
    size_t bytes_filled, size_t element_size, ViewConstructorType view_constructor, ReaderType reader_type)
    : buffer(buffer)
...
void ReadableByteStreamController::PullIntoDescriptor::Trace(Visitor* visitor) const
{
    visitor->Trace(buffer);
}
...
ReadableStreamBYOBRequest* ReadableByteStreamController::GetBYOBRequest(ReadableByteStreamController* controller)
{
    ...
    if (!controller->byob_request_ && !controller->pending_pull_intos_.empty()) {
        ...
        const PullIntoDescriptor* first_descriptor = controller->pending_pull_intos_[0];
        ...
        DOMUint8Array* const view = DOMUint8Array::Create(first_descriptor->buffer, first_descriptor->byte_offset + first_descriptor->bytes_filled,
        ...
        controller->byob_request_ = MakeGarbageCollected<ReadableStreamBYOBRequest>(controller, NotShared<DOMUint8Array>(view));
    }
...
std::optional<double> ReadableByteStreamController::GetDesiredSize(ReadableByteStreamController* controller)
{
    ...
    case ReadableStream::kReadable:
        ...
        return controller->strategy_high_water_mark_ - controller->queue_total_size_;
    }
}

void ReadableByteStreamController::close(ScriptState* script_state, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#rbs-controller-close
    // 1. If this.[[closeRequested]] is true, throw a TypeError exception.
    if (close_requested_) {
        exception_state.ThrowTypeError("Cannot close a readable stream that has already been requested "
                                       "to be closed");
        return;
    }
    ...
    if (controlled_readable_stream_->state_ != ReadableStream::kReadable) {
        exception_state.ThrowTypeError("Cannot close a readable stream that is not readable");
        return;
    }
...
void ReadableByteStreamController::enqueue(ScriptState* script_state, NotShared<DOMArrayBufferView> chunk, ExceptionState& exception_state)
{
...
{
    error(script_state, ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())));
}

void ReadableByteStreamController::error(ScriptState* script_state, const ScriptValue& e)
{
...
void ReadableByteStreamController::Close(ScriptState* script_state, ReadableByteStreamController* controller)
{
    ...
    // "readable", return.
    if (controller->close_requested_ || stream->state_ != ReadableStream::kReadable) {
        return;
    }
    ...
    if (!controller->pending_pull_intos_.empty()) {
        ...
        const PullIntoDescriptor* first_pending_pull_into = controller->pending_pull_intos_[0];
        //   b. If firstPendingPullInto’s bytes filled > 0,
        if (first_pending_pull_into->bytes_filled > 0) {
            //     i. Let e be a new TypeError exception.
            v8::Local<v8::Value> e = V8ThrowException::CreateTypeError(script_state->GetIsolate(), "Cannot close while responding");
...
void ReadableByteStreamController::Error(ScriptState* script_state, ReadableByteStreamController* controller, v8::Local<v8::Value> e)
{
...
void ReadableByteStreamController::Enqueue(
    ScriptState* script_state, ReadableByteStreamController* controller, NotShared<DOMArrayBufferView> chunk, ExceptionState& exception_state)
{
    ...
    DOMArrayBuffer* const transferred_buffer = TransferArrayBuffer(script_state, buffer, exception_state);
    ...
    if (!controller->pending_pull_intos_.empty()) {
        //     a. Let firstPendingPullInto be controller.[[pendingPullIntos]][0].
        PullIntoDescriptor* first_pending_pull_into = controller->pending_pull_intos_[0];
        ...
        first_pending_pull_into->buffer = TransferArrayBuffer(script_state, first_pending_pull_into->buffer, exception_state);
    ...
    if (ReadableStream::HasDefaultReader(stream)) {
        ...
        if (ReadableStream::GetNumReadRequests(stream) == 0) {
            ...
            EnqueueChunkToQueue(controller, transferred_buffer, byte_offset, byte_length);
        } else {
            // c. Otherwise,
            ...
            if (!controller->pending_pull_intos_.empty()) {
                ...
                //        "default".
                DCHECK_EQ(controller->pending_pull_intos_[0]->reader_type, ReaderType::kDefault);
            ...
            //     transferredBuffer, byteOffset, byteLength »).
            v8::Local<v8::Value> const transferred_view
                = v8::Uint8Array::New(ToV8Traits<DOMArrayBuffer>::ToV8(script_state, transferred_buffer).As<v8::ArrayBuffer>(), byte_offset, byte_length);
            //     iv. Perform ! ReadableStreamFulfillReadRequest(stream,
            //     transferredView, false).
            ReadableStream::FulfillReadRequest(script_state, stream, transferred_view, false, exception_state);
        }
    }
...
void ReadableByteStreamController::EnqueueChunkToQueue(ReadableByteStreamController* controller, DOMArrayBuffer* buffer, size_t byte_offset, size_t byte_length)
{
    ...
    QueueEntry* const entry = MakeGarbageCollected<QueueEntry>(buffer, byte_offset, byte_length);
...
void ReadableByteStreamController::EnqueueClonedChunkToQueue(
    ReadableByteStreamController* controller, DOMArrayBuffer* buffer, size_t byte_offset, size_t byte_length)
{
    ...
    // %ArrayBuffer%).
    DOMArrayBuffer* const clone_result = DOMArrayBuffer::Create(buffer->ByteSpan().subspan(byte_offset, byte_length));
...
void ReadableByteStreamController::EnqueueDetachedPullIntoToQueue(ReadableByteStreamController* controller, PullIntoDescriptor* pull_into_descriptor)
{
    ...
    if (pull_into_descriptor->bytes_filled > 0) {
        EnqueueClonedChunkToQueue(controller, pull_into_descriptor->buffer, pull_into_descriptor->byte_offset, pull_into_descriptor->bytes_filled);
    }
...
void ReadableByteStreamController::ProcessPullIntoDescriptorsUsingQueue(ScriptState* script_state, ReadableByteStreamController* controller)
{
    ...
    while (!controller->pending_pull_intos_.empty()) {
        ...
        PullIntoDescriptor* const pull_into_descriptor = controller->pending_pull_intos_[0];
        //   c. If ! ReadableByteStreamControllerFillPullIntoDescriptorFromQueue(
        //   controller, pullIntoDescriptor) is true,
        if (FillPullIntoDescriptorFromQueue(controller, pull_into_descriptor, PassThroughException(isolate))) {
            //     i. Perform !
            //     ReadableByteStreamControllerShiftPendingPullInto(controller).
            ShiftPendingPullInto(controller);
            //     ii. Perform ! ReadableByteStreamControllerCommitPullIntoDescriptor(
            //     controller.[[stream]], pullIntoDescriptor).
            CommitPullIntoDescriptor(script_state, controller->controlled_readable_stream_, pull_into_descriptor, PassThroughException(isolate));
        ...
        if (try_catch.HasCaught()) {
            ...
            ReadableByteStreamController::Error(script_state, controller, try_catch.Exception());
            return;
        }
    }
}
...
void ReadableByteStreamController::ProcessReadRequestsUsingQueue(
    ScriptState* script_state, ReadableByteStreamController* controller, ExceptionState& exception_state)
{
    ...
    ReadableStreamGenericReader* reader = controller->controlled_readable_stream_->reader_;
    // 2. Assert: reader implements ReadableStreamDefaultReader.
    DCHECK(reader->IsDefaultReader());
    ReadableStreamDefaultReader* default_reader = To<ReadableStreamDefaultReader>(reader);
    ...
    while (!default_reader->read_requests_.empty()) {
        ...
        //   readRequest).
        FillReadRequestFromQueue(script_state, controller, read_request, exception_state);
    }
}

void ReadableByteStreamController::CallPullIfNeeded(ScriptState* script_state, ReadableByteStreamController* controller)
{
    ...
    auto pull_promise = controller->pull_algorithm_->Run(script_state, 0, nullptr);
    ...
    public:
        ...
        void CallWithLocal(ScriptState* script_state, v8::Local<v8::Value>) override
        {
    ...
    public:
        ...
        void CallWithLocal(ScriptState* script_state, v8::Local<v8::Value> e) override
        {
    ...
    StreamThenPromise(script_state, pull_promise, MakeGarbageCollected<ResolveFunction>(controller), MakeGarbageCollected<RejectFunction>(controller));
}

ReadableByteStreamController::PullIntoDescriptor* ReadableByteStreamController::ShiftPendingPullInto(ReadableByteStreamController* controller)
{
...
bool ReadableByteStreamController::ShouldCallPull(ReadableByteStreamController* controller)
{
    ...
    if (ReadableStream::HasDefaultReader(stream) && ReadableStream::GetNumReadRequests(stream) > 0) {
        return true;
    }
    // 6. If ! ReadableStreamHasBYOBReader(stream) is true and !
    // ReadableStreamGetNumReadIntoRequests(stream) > 0, return true.
    if (ReadableStream::HasBYOBReader(stream) && ReadableStream::GetNumReadIntoRequests(stream) > 0) {
        return true;
    }
...
void ReadableByteStreamController::CommitPullIntoDescriptor(
    ScriptState* script_state, ReadableStream* stream, PullIntoDescriptor* pull_into_descriptor, ExceptionState& exception_state)
{
    ...
    auto* filled_view = ConvertPullIntoDescriptor(script_state, pull_into_descriptor, exception_state);
    DCHECK(!exception_state.HadException());
    ...
    if (pull_into_descriptor->reader_type == ReaderType::kDefault) {
        ...
        //   done).
        ReadableStream::FulfillReadRequest(script_state, stream, ToV8Traits<DOMArrayBufferView>::ToV8(script_state, filled_view), done, exception_state);
    } else {
        // 7. Otherwise,
        ...
        //   done).
        ReadableStream::FulfillReadIntoRequest(script_state, stream, filled_view, done, exception_state);
    }
}

DOMArrayBufferView* ReadableByteStreamController::ConvertPullIntoDescriptor(
    ScriptState* script_state, PullIntoDescriptor* pull_into_descriptor, ExceptionState& exception_state)
{
    ...
    DOMArrayBuffer* const buffer = TransferArrayBuffer(script_state, pull_into_descriptor->buffer, exception_state);
    // 6. Return ! Construct(pullIntoDescriptor’s view constructor, « buffer,
    // pullIntoDescriptor’s byte offset, bytesFilled ÷ elementSize »).
    return pull_into_descriptor->view_constructor(buffer, pull_into_descriptor->byte_offset, (bytes_filled / element_size));
}

void ReadableByteStreamController::ClearPendingPullIntos(ReadableByteStreamController* controller)
{
...
void ReadableByteStreamController::ClearAlgorithms(ReadableByteStreamController* controller)
{
...
void ReadableByteStreamController::InvalidateBYOBRequest(ReadableByteStreamController* controller)
{
...
void ReadableByteStreamController::SetUp(ScriptState* script_state, ReadableStream* stream, ReadableByteStreamController* controller,
    StreamStartAlgorithm* start_algorithm, StreamAlgorithm* pull_algorithm, StreamAlgorithm* cancel_algorithm, double high_water_mark,
    size_t auto_allocate_chunk_size, ExceptionState& exception_state)
{
    ...
    if (!start_algorithm->Run(script_state, exception_state).ToLocal(&start_promise)) {
        if (!exception_state.HadException()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "start algorithm failed with no exception thrown");
        }
        return;
    }
    DCHECK(!exception_state.HadException());
    ...
    public:
        ...
        void CallWithLocal(ScriptState* script_state, v8::Local<v8::Value> r) override
        {
    ...
    StreamThenPromise(script_state, start_promise, MakeGarbageCollected<ResolveFunction>(controller), MakeGarbageCollected<RejectFunction>(controller));
}

void ReadableByteStreamController::SetUpFromUnderlyingSource(ScriptState* script_state, ReadableStream* stream, v8::Local<v8::Object> underlying_source,
    UnderlyingSource* underlying_source_dict, double high_water_mark, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#set-up-readable-byte-stream-controller-from-underlying-source
    // 1. Let controller be a new ReadableByteStreamController.
    ReadableByteStreamController* controller = MakeGarbageCollected<ReadableByteStreamController>();
    ...
    const auto controller_value = ToV8Traits<ReadableByteStreamController>::ToV8(script_state, controller);
    ...
    if (underlying_source_dict->hasStart()) {
        start_algorithm = CreateByteStreamStartAlgorithm(script_state, underlying_source,
            ToV8Traits<V8UnderlyingSourceStartCallback>::ToV8(script_state, underlying_source_dict->start()), controller_value);
    }
    ...
    if (underlying_source_dict->hasPull()) {
        pull_algorithm = CreateAlgorithmFromResolvedMethod(
            script_state, underlying_source, ToV8Traits<V8UnderlyingSourcePullCallback>::ToV8(script_state, underlying_source_dict->pull()), controller_value);
    }
    ...
    if (underlying_source_dict->hasCancel()) {
        cancel_algorithm = CreateAlgorithmFromResolvedMethod(script_state, underlying_source,
            ToV8Traits<V8UnderlyingSourceCancelCallback>::ToV8(script_state, underlying_source_dict->cancel()), controller_value);
    }
    ...
    size_t auto_allocate_chunk_size
        = underlying_source_dict->hasAutoAllocateChunkSize() ? static_cast<size_t>(underlying_source_dict->autoAllocateChunkSize()) : 0u;
    // 9. If autoAllocateChunkSize is 0, then throw a TypeError exception.
    if (underlying_source_dict->hasAutoAllocateChunkSize() && auto_allocate_chunk_size == 0) {
    ...
    SetUp(script_state, stream, controller, start_algorithm, pull_algorithm, cancel_algorithm, high_water_mark, auto_allocate_chunk_size, exception_state);
}

void ReadableByteStreamController::FillHeadPullIntoDescriptor(ReadableByteStreamController* controller, size_t size, PullIntoDescriptor* pull_into_descriptor)
{
    ...
    pull_into_descriptor->bytes_filled = base::CheckAdd(pull_into_descriptor->bytes_filled, size).ValueOrDie();
}

bool ReadableByteStreamController::FillPullIntoDescriptorFromQueue(
    ReadableByteStreamController* controller, PullIntoDescriptor* pull_into_descriptor, ExceptionState& exception_state)
{
    ...
    const size_t current_aligned_bytes = pull_into_descriptor->bytes_filled - (pull_into_descriptor->bytes_filled % element_size);
    ...
    const size_t max_bytes_to_copy
        = std::min(static_cast<size_t>(controller->queue_total_size_), pull_into_descriptor->byte_length - pull_into_descriptor->bytes_filled);
    ...
    const size_t max_bytes_filled = pull_into_descriptor->bytes_filled + max_bytes_to_copy;
    ...
    const size_t max_aligned_bytes = max_bytes_filled - (max_bytes_filled % element_size);
    ...
    if (max_aligned_bytes > current_aligned_bytes) {
        ...
        // pullIntoDescriptor’s bytes filled.
        total_bytes_to_copy_remaining = base::CheckSub(max_aligned_bytes, pull_into_descriptor->bytes_filled).ValueOrDie();
        // b. Set ready to true.
        ready = true;
    }
    ...
    while (total_bytes_to_copy_remaining > 0) {
        ...
        size_t bytes_to_copy = std::min(total_bytes_to_copy_remaining, head_of_queue->byte_length);
        ...
        // size_t.
        size_t dest_start = pull_into_descriptor->byte_offset + pull_into_descriptor->bytes_filled;
        ...
        auto copy_destination = pull_into_descriptor->buffer->ByteSpan().subspan(dest_start, bytes_to_copy);
        auto copy_source = head_of_queue->buffer->ByteSpan().subspan(head_of_queue->byte_offset, bytes_to_copy);
        ...
        if (head_of_queue->byte_length == bytes_to_copy) {
            ...
            //   bytesToCopy.
            head_of_queue->byte_offset = base::CheckAdd(head_of_queue->byte_offset, bytes_to_copy).ValueOrDie();
            //   ii. Set headOfQueue’s byte length to headOfQueue’s byte
            //   length − bytesToCopy.
            head_of_queue->byte_length = base::CheckSub(head_of_queue->byte_length, bytes_to_copy).ValueOrDie();
        }
        // g. Set controller.[[queueTotalSize]] to controller.[[queueTotalSize]] −
        // bytesToCopy.
        controller->queue_total_size_ = base::CheckSub(controller->queue_total_size_, bytes_to_copy).ValueOrDie();
        // h. Perform !
    ...
    if (!ready) {
        ...
        // element size.
        DCHECK_LT(pull_into_descriptor->bytes_filled, pull_into_descriptor->element_size);
    }
    // 12. Return ready.
    return ready;
}
...
void ReadableByteStreamController::FillReadRequestFromQueue(
    ScriptState* script_state, ReadableByteStreamController* controller, ReadRequest* read_request, ExceptionState& exception_state)
{
    ...
    DOMUint8Array* view = DOMUint8Array::Create(entry->buffer, entry->byte_offset, entry->byte_length);
    // 7. Perform readRequest’s chunk steps, given view.
    read_request->ChunkSteps(script_state, ToV8Traits<DOMUint8Array>::ToV8(script_state, view), exception_state);
}

void ReadableByteStreamController::PullInto(ScriptState* script_state, ReadableByteStreamController* controller, NotShared<DOMArrayBufferView> view,
    ReadIntoRequest* read_into_request, ExceptionState& exception_state)
{
    ...
    PullIntoDescriptor* pull_into_descriptor
        = MakeGarbageCollected<PullIntoDescriptor>(buffer, buffer->ByteLength(), byte_offset, byte_length, 0, element_size, ctor, ReaderType::kBYOB);
    ...
    if (stream->state_ == ReadableStream::kClosed) {
        ...
        DOMArrayBufferView* emptyView = ctor(pull_into_descriptor->buffer, pull_into_descriptor->byte_offset, 0);
    ...
    if (controller->queue_total_size_ > 0) {
        ...
        v8::TryCatch try_catch(script_state->GetIsolate());
        if (FillPullIntoDescriptorFromQueue(controller, pull_into_descriptor, PassThroughException(script_state->GetIsolate()))) {
            //     i. Let filledView be !
            //     ReadableByteStreamControllerConvertPullIntoDescriptor(pullIntoDescriptor).
            DOMArrayBufferView* filled_view = ConvertPullIntoDescriptor(script_state, pull_into_descriptor, PassThroughException(script_state->GetIsolate()));
            DCHECK(!try_catch.HasCaught());
            //     ii. Perform !
        ...
        if (controller->close_requested_) {
            ...
            v8::Local<v8::Value> e = V8ThrowException::CreateTypeError(script_state->GetIsolate(), "close requested");
            //     ii. Perform ! ReadableByteStreamControllerError(controller, e).
...
void ReadableByteStreamController::HandleQueueDrain(ScriptState* script_state, ReadableByteStreamController* controller)
{
    // https://streams.spec.whatwg.org/#readable-byte-stream-controller-handle-queue-drain
    // 1. Assert: controller.[[stream]].[[state]] is "readable".
    DCHECK_EQ(controller->controlled_readable_stream_->state_, ReadableStream::kReadable);
    ...
    if (!controller->queue_total_size_ && controller->close_requested_) {
        ...
        ReadableStream::Close(script_state, controller->controlled_readable_stream_);
    } else {
        // 3. Otherwise,
...
void ReadableByteStreamController::ResetQueue(ReadableByteStreamController* controller)
{
...
void ReadableByteStreamController::Respond(
    ScriptState* script_state, ReadableByteStreamController* controller, size_t bytes_written, ExceptionState& exception_state)
{
    ...
    const ReadableStream::State state = controller->controlled_readable_stream_->state_;
    ...
    if (state == ReadableStream::kClosed) {
        ...
        if (base::ClampAdd(first_descriptor->bytes_filled, bytes_written) > first_descriptor->byte_length) {
            exception_state.ThrowRangeError("available read buffer is too small for specified number of bytes");
            return;
        }
    }
    // 6. Set firstDescriptor's buffer to ! TransferArrayBuffer(firstDescriptor's
    // buffer).
    first_descriptor->buffer = TransferArrayBuffer(script_state, first_descriptor->buffer, exception_state);
...
void ReadableByteStreamController::RespondInClosedState(
    ScriptState* script_state, ReadableByteStreamController* controller, PullIntoDescriptor* first_descriptor, ExceptionState& exception_state)
{
    ...
    if (ReadableStream::HasBYOBReader(stream)) {
        ...
        while (ReadableStream::GetNumReadIntoRequests(stream) > 0) {
            ...
            //     ReadableByteStreamControllerShiftPendingPullInto(controller).
            PullIntoDescriptor* pull_into_descriptor = ShiftPendingPullInto(controller);
            //     ii. Perform !
            //     ReadableByteStreamControllerCommitPullIntoDescriptor(stream,
            //     pullIntoDescriptor).
            CommitPullIntoDescriptor(script_state, stream, pull_into_descriptor, exception_state);
            DCHECK(!exception_state.HadException());
        }
    }
}

void ReadableByteStreamController::RespondInReadableState(ScriptState* script_state, ReadableByteStreamController* controller, size_t bytes_written,
    PullIntoDescriptor* pull_into_descriptor, ExceptionState& exception_state)
{
    ...
    DCHECK_LE(pull_into_descriptor->bytes_filled + bytes_written, pull_into_descriptor->byte_length);
    // 2. Perform !
    // ReadableByteStreamControllerFillHeadPullIntoDescriptor(controller,
    ...
    const size_t remainder_size = pull_into_descriptor->bytes_filled % pull_into_descriptor->element_size;
    ...
    if (remainder_size > 0) {
        ...
        //   refer to actually allocated memory, so together they cannot exceed
        //   size_t.
        size_t end = pull_into_descriptor->byte_offset + pull_into_descriptor->bytes_filled;
        //   b. Perform ?
        //   ReadableByteStreamControllerEnqueueClonedChunkToQueue(controller,
        //   pullIntoDescriptor’s buffer, end − remainderSize, remainderSize).
        EnqueueClonedChunkToQueue(controller, pull_into_descriptor->buffer, end - remainder_size, remainder_size);
    }
    // 8. Set pullIntoDescriptor’s bytes filled to pullIntoDescriptor’s bytes
    // filled − remainderSize.
    pull_into_descriptor->bytes_filled = pull_into_descriptor->bytes_filled - remainder_size;
    // 9. Perform !
    // ReadableByteStreamControllerCommitPullIntoDescriptor(controller.[[stream]],
    // pullIntoDescriptor).
    CommitPullIntoDescriptor(script_state, controller->controlled_readable_stream_, pull_into_descriptor, exception_state);
    DCHECK(!exception_state.HadException());
    // 10. Perform !
    // ReadableByteStreamControllerProcessPullIntoDescriptorsUsingQueue(controller).
    ProcessPullIntoDescriptorsUsingQueue(script_state, controller);
    ...
    ScriptState* script_state, ReadableByteStreamController* controller, size_t bytes_written, ExceptionState& exception_state)
{
    ...
    PullIntoDescriptor* const first_descriptor = controller->pending_pull_intos_[0];
    ...
    if (state == ReadableStream::kClosed) {
        ...
        //   firstDescriptor).
        RespondInClosedState(script_state, controller, first_descriptor, exception_state);
    } else {
        // 6. Otherwise,
        //   a. Assert: state is "readable".
        DCHECK_EQ(state, ReadableStream::kReadable);
        ...
        RespondInReadableState(script_state, controller, bytes_written, first_descriptor, exception_state);
    }
...
void ReadableByteStreamController::RespondWithNewView(
    ScriptState* script_state, ReadableByteStreamController* controller, NotShared<DOMArrayBufferView> view, ExceptionState& exception_state)
{
    ...
    // equal.
    if (first_descriptor->byte_offset + first_descriptor->bytes_filled != view->byteOffset()) {
        exception_state.ThrowRangeError("supplied view's byte offset doesn't match the expected value");
        return;
    }
    ...
    if (base::ClampAdd(first_descriptor->bytes_filled, view->byteLength()) > first_descriptor->byte_length) {
        exception_state.ThrowRangeError("supplied view is too large for the read buffer");
        return;
    }
    ...
    first_descriptor->buffer = TransferArrayBuffer(script_state, view->buffer(), exception_state);
    if (exception_state.HadException()) {
        return;
    }
...
bool ReadableByteStreamController::CanTransferArrayBuffer(DOMArrayBuffer* buffer)
{
    return !buffer->IsDetached();
}

DOMArrayBuffer* ReadableByteStreamController::TransferArrayBuffer(ScriptState* script_state, DOMArrayBuffer* buffer, ExceptionState& exception_state)
{
    ...
    if (!buffer->Transfer(script_state->GetIsolate(), contents, exception_state)) {
        return nullptr;
    }
...
v8::Local<v8::Promise> ReadableByteStreamController::CancelSteps(ScriptState* script_state, v8::Local<v8::Value> reason)
{
...
void ReadableByteStreamController::PullSteps(ScriptState* script_state, ReadRequest* read_request, ExceptionState& exception_state)
{
    ...
    if (auto_allocate_chunk_size) {
        ...
        PullIntoDescriptor* pull_into_descriptor
            = MakeGarbageCollected<PullIntoDescriptor>(buffer, auto_allocate_chunk_size, 0, auto_allocate_chunk_size, 0, 1, ctor, ReaderType::kDefault);
