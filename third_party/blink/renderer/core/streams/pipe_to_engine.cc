...
public:
    ...
    void Run() override
    {
        engine_->AbortAlgorithm(signal_);
    }
...
public:
    explicit PipeToReadRequest(PipeToEngine* instance)
        : instance_(instance)
    {
    }
    ...
    void ChunkSteps(ScriptState* script_state, v8::Local<v8::Value> chunk, ExceptionState&) const override
    {
        ...
        event_loop->EnqueueMicrotask(
            WTF::BindOnce(&PipeToEngine::ReadRequestChunkStepsBody, WrapPersistent(instance_.Get()), WrapPersistent(script_state), std::move(value)));
    }
    ...
    void ErrorSteps(ScriptState* script_state, v8::Local<v8::Value> e) const override
    {
        ...
        instance_->ReadableError(instance_->Readable()->GetStoredError(script_state->GetIsolate()));
    }
...
class PipeToEngine::WrappedPromiseReaction final : public PromiseHandlerWithValue {
public:
    ...
    v8::Local<v8::Value> CallWithLocal(ScriptState* script_state, v8::Local<v8::Value> value) override
    {
...
ScriptPromise<IDLUndefined> PipeToEngine::Start(ReadableStream* readable, WritableStream* destination, ExceptionState& exception_state)
{
    ...
    reader_ = ReadableStream::AcquireDefaultReader(script_state_, readable, exception_state);
    DCHECK(!exception_state.HadException());

    // 10. Let writer be ! AcquireWritableStreamDefaultWriter(dest).
    writer_ = WritableStream::AcquireDefaultWriter(script_state_, destination, exception_state);
    DCHECK(!exception_state.HadException());
    ...
    promise_ = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state_);
    ...
    if (auto* signal = pipe_options_->Signal()) {
        ...
        abort_handle_ = signal->AddAlgorithm(MakeGarbageCollected<PipeToAbortAlgorithm>(this, signal));
    }
    ...
    if (CheckInitialState()) {
        ...
        //        becomes "closed", ...
        ThenPromise(reader_->closed(script_state_).V8Promise(), &PipeToEngine::OnReaderClosed, &PipeToEngine::ReadableError);
        ...
        ThenPromise(writer_->closed(script_state_).V8Promise(), nullptr, &PipeToEngine::WritableError);
...
v8::Local<v8::Promise> PipeToEngine::AbortAlgorithmAction()
{
    ...
    if (!pipe_options_->PreventAbort() && Destination()->IsWritable()) {
        actions.push_back(WritableStream::Abort(script_state_, Destination(), error));
    }
    ...
    //      2. Otherwise, return a promise resolved with undefined.
    if (!pipe_options_->PreventCancel() && ReadableStream::IsReadable(Readable())) {
        actions.push_back(ReadableStream::Cancel(script_state_, Readable(), error));
    }

    return PromiseAll<IDLUndefined>::Create(script_state_.Get(), actions).V8Promise();
}
...
v8::Local<v8::Value> PipeToEngine::HandleNextEvent(v8::Local<v8::Value>)
{
    ...
    if (desired_size.value() <= 0) {
        ...
        ThenPromise(writer_->ready(script_state_).V8Promise(), &PipeToEngine::HandleNextEvent, &PipeToEngine::WritableError);
        return Undefined();
    }
    ...
    ReadableStreamDefaultReader::Read(script_state_, reader_, read_request, exception_state);
    return Undefined();
}

void PipeToEngine::ReadRequestChunkStepsBody(ScriptState* script_state, v8::Global<v8::Value> chunk)
{
    ...
    is_reading_ = false;
    const auto write = WritableStreamDefaultWriter::Write(script_state, writer_, chunk.Get(script_state->GetIsolate()), exception_state)->V8Promise();
...
v8::Local<v8::Value> PipeToEngine::ReadableError(v8::Local<v8::Value> error)
{
    ...
    //    source.[[storedError]].
    DCHECK(error->SameValue(Readable()->GetStoredError(script_state_->GetIsolate())));
...
v8::Local<v8::Value> PipeToEngine::WritableError(v8::Local<v8::Value> error)
{
    ...
    //    dest.[[storedError]].
    DCHECK(error->SameValue(Destination()->GetStoredError(script_state_->GetIsolate())));
...
void PipeToEngine::ReadableClosed()
{
    ...
    if (!pipe_options_->PreventClose()) {
        ShutdownWithAction(&PipeToEngine::WritableStreamDefaultWriterCloseWithErrorPropagationAction, v8::MaybeLocal<v8::Value>());
    } else {
...
void PipeToEngine::WritableStartedClosed()
{
    ...
    const auto dest_closed = v8::Exception::TypeError(V8String(script_state_->GetIsolate(), "Destination stream closed"));
...
void PipeToEngine::ShutdownWithAction(Action action, v8::MaybeLocal<v8::Value> original_error)
{
    ...
    ThenPromise(p, &PipeToEngine::FinalizeWithOriginalErrorIfSet, &PipeToEngine::FinalizeWithNewError);
}
...
void PipeToEngine::Shutdown(v8::MaybeLocal<v8::Value> error_maybe)
{
    ...
    if (ShouldWriteQueuedChunks()) {
        ...
        ThenPromise(WriteQueuedChunks(), &PipeToEngine::FinalizeWithOriginalErrorIfSet);
    } else {
        // d. Finalize, passing along error if it was given.
...
v8::Local<v8::Value> PipeToEngine::FinalizeWithOriginalErrorIfSet(v8::Local<v8::Value>)
{
...
v8::Local<v8::Value> PipeToEngine::FinalizeWithNewError(v8::Local<v8::Value> new_error)
{
    Finalize(new_error);
    return Undefined();
}
...
bool PipeToEngine::ShouldWriteQueuedChunks() const
{
    ...
    // WritableStreamCloseQueuedOrInFlight(dest) is false"
    return Destination()->IsWritable() && !WritableStream::CloseQueuedOrInFlight(Destination());
}
...
v8::Local<v8::Promise> PipeToEngine::WriteQueuedChunks()
{
    if (!last_write_.IsEmpty()) {
        ...
        // resolution.
        return ThenPromise(last_write_.Get(script_state_->GetIsolate()), nullptr, &PipeToEngine::IgnoreErrors);
    }
...
{
    return WritableStream::Abort(script_state_, Destination(), ShutdownError()).V8Promise();
}

v8::Local<v8::Promise> PipeToEngine::ReadableStreamCancelAction()
{
    return ReadableStream::Cancel(script_state_, Readable(), ShutdownError()).V8Promise();
}

v8::Local<v8::Promise> PipeToEngine::WritableStreamDefaultWriterCloseWithErrorPropagationAction()
{
    return WritableStreamDefaultWriter::CloseWithErrorPropagation(script_state_, writer_).V8Promise();
}
...
v8::Local<v8::Promise> PipeToEngine::ThenPromise(v8::Local<v8::Promise> promise, PromiseReaction on_fulfilled, PromiseReaction on_rejected)
{
    return StreamThenPromise(script_state_, promise, on_fulfilled ? MakeGarbageCollected<WrappedPromiseReaction>(this, on_fulfilled) : nullptr,
        on_rejected ? MakeGarbageCollected<WrappedPromiseReaction>(this, on_rejected) : nullptr);
}
