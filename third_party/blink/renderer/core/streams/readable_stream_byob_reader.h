...
class CORE_EXPORT ReadableStreamBYOBReader : public ReadableStreamGenericReader {
    DEFINE_WRAPPERTYPEINFO();

public:
    static ReadableStreamBYOBReader* Create(ScriptState*, ReadableStream* stream, ExceptionState&);

    // https://streams.spec.whatwg.org/#byob-reader-constructor
    ReadableStreamBYOBReader(ScriptState*, ReadableStream* stream, ExceptionState&);
    ~ReadableStreamBYOBReader() override;

    bool IsDefaultReader() const override
    {
        return false;
    }
    bool IsBYOBReader() const override
    {
        return true;
    }
    ...
    ScriptPromise<ReadableStreamReadResult> read(ScriptState*, NotShared<DOMArrayBufferView>, ExceptionState&);

    // https://streams.spec.whatwg.org/#byob-reader-release-lock
    void releaseLock(ScriptState*, ExceptionState&);

    // https://streams.spec.whatwg.org/#set-up-readable-stream-byob-reader
    static void SetUpBYOBReader(ScriptState*, ReadableStreamBYOBReader* reader, ReadableStream* stream, ExceptionState&);
...
private:
    ...
    static void Read(ScriptState*, ReadableStreamBYOBReader*, NotShared<DOMArrayBufferView> view, ReadIntoRequest*, ExceptionState&);

    // https://streams.spec.whatwg.org/#abstract-opdef-readablestreambyobreadererrorreadintorequests
    static void ErrorReadIntoRequests(ScriptState*, ReadableStreamBYOBReader*, v8::Local<v8::Value> e);
...
template <> struct DowncastTraits<ReadableStreamBYOBReader> {
