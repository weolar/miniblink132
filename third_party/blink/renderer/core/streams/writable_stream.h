...
public:
    ...
    static WritableStream* Create(ScriptState*, ScriptValue underlying_sink, ExceptionState&);
    static WritableStream* Create(ScriptState*, ScriptValue raw_underlying_sink, ScriptValue raw_strategy, ExceptionState&);

    // https://streams.spec.whatwg.org/#create-writable-stream
    // Unlike in the standard, |high_water_mark| and |size_algorithm| are
    // required parameters.
    static WritableStream* Create(ScriptState*, StreamStartAlgorithm* start_algorithm, StreamAlgorithm* write_algorithm, StreamAlgorithm* close_algorithm,
        StreamAlgorithm* abort_algorithm, double high_water_mark, StrategySizeAlgorithm* size_algorithm, ExceptionState&);

    static WritableStream* CreateWithCountQueueingStrategy(ScriptState*, UnderlyingSinkBase*, size_t high_water_mark);
    static WritableStream* CreateWithCountQueueingStrategy(
        ScriptState*, UnderlyingSinkBase*, size_t high_water_mark, std::unique_ptr<WritableStreamTransferringOptimizer> optimizer);
    ...
    void InitWithCountQueueingStrategy(
        ScriptState*, UnderlyingSinkBase*, size_t high_water_mark, std::unique_ptr<WritableStreamTransferringOptimizer>, ExceptionState&);
    ...
    ScriptPromise<IDLUndefined> abort(ScriptState*, ScriptValue reason, ExceptionState&);
    ...
    static WritableStream* Deserialize(ScriptState*, MessagePort*, std::unique_ptr<WritableStreamTransferringOptimizer> optimizer, ExceptionState&);
    ...
    static WritableStreamDefaultWriter* AcquireDefaultWriter(ScriptState*, WritableStream*, ExceptionState&);
    ...
    static ScriptPromise<IDLUndefined> Abort(ScriptState*, WritableStream*, v8::Local<v8::Value> reason);

    // https://streams.spec.whatwg.org/#writable-stream-add-write-request
    static void AddWriteRequest(WritableStream*, ScriptPromiseResolver<IDLUndefined>*);
    ...
    static void DealWithRejection(ScriptState*, WritableStream*, v8::Local<v8::Value> error);

    // https://streams.spec.whatwg.org/#writable-stream-start-erroring
    static void StartErroring(ScriptState*, WritableStream*, v8::Local<v8::Value> reason);
    ...
    static void FinishInFlightWriteWithError(ScriptState*, WritableStream*, v8::Local<v8::Value> error);
    ...
    static void FinishInFlightCloseWithError(ScriptState*, WritableStream*, v8::Local<v8::Value> error);
    ...
    static void UpdateBackpressure(ScriptState*, WritableStream*, bool backpressure);
    ...
    {
        return state_ == kErrored;
    }
    bool IsErroring() const
    {
        return state_ == kErroring;
    }
    bool IsWritable() const
    {
        return state_ == kWritable;
    }

    bool HasBackpressure() const
    {
        return has_backpressure_;
    }

    bool HasInFlightWriteRequest() const
    {
    ...
    const WritableStreamDefaultWriter* Writer() const
    {
        return writer_.Get();
    }
    ...
    std::unique_ptr<WritableStreamTransferringOptimizer> TakeTransferringOptimizer();
    ...
    static v8::Local<v8::Value> CreateCannotActionOnStateStreamException(v8::Isolate*, const char* action, State);
...
protected:
    ...
    void InitInternal(ScriptState*, ScriptValue raw_underlying_sink, ScriptValue raw_strategy, ExceptionState&);
...
private:
    ...
    static void RejectCloseAndClosedPromiseIfNeeded(ScriptState*, WritableStream*);

    // Functions that don't appear in the standard.

    // Rejects all the promises in |queue| with value |e|.
    static void RejectPromises(ScriptState*, PromiseQueue* queue, v8::Local<v8::Value> e);
