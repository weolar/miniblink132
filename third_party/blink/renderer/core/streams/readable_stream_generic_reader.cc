...
ScriptPromise<IDLUndefined> ReadableStreamGenericReader::closed(ScriptState*) const
{
...
ScriptPromise<IDLUndefined> ReadableStreamGenericReader::cancel(ScriptState* script_state, ExceptionState& exception_state)
{
    return cancel(script_state, ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())), exception_state);
}

ScriptPromise<IDLUndefined> ReadableStreamGenericReader::cancel(ScriptState* script_state, ScriptValue reason, ExceptionState& exception_state)
{
...
void ReadableStreamGenericReader::GenericRelease(ScriptState* script_state, ReadableStreamGenericReader* reader)
{
    ...
    if (stream->state_ != ReadableStream::kReadable) {
        reader->closed_resolver_ = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state);
        reader->closed_promise_ = reader->closed_resolver_->Promise();
    }
    reader->closed_promise_.MarkAsSilent();
    reader->closed_resolver_->Reject(v8::Exception::TypeError(V8String(isolate,
...
ScriptPromise<IDLUndefined> ReadableStreamGenericReader::GenericCancel(
    ScriptState* script_state, ReadableStreamGenericReader* reader, v8::Local<v8::Value> reason)
{
...
void ReadableStreamGenericReader::GenericInitialize(ScriptState* script_state, ReadableStreamGenericReader* reader, ReadableStream* stream)
{
    auto* isolate = script_state->GetIsolate();
