...
public:
    ...
    v8::Local<v8::Promise> Run(ScriptState* script_state, int argc, v8::Local<v8::Value> argv[]) override
    {
        ...
        if (!transformer_script_state->ContextIsValid()) {
            return PromiseReject(script_state, V8ThrowException::CreateTypeError(isolate, "invalid realm"));
        }
...
public:
    ...
    v8::Local<v8::Promise> Run(ScriptState* script_state, int argc, v8::Local<v8::Value> argv[]) override
    {
        ...
        v8::TryCatch try_catch(isolate);
        auto promise = transformer_->Transform(argv[0], controller_, PassThroughException(isolate));
        if (try_catch.HasCaught()) {
            return PromiseReject(script_state, try_catch.Exception());
        }
...
TransformStream* TransformStream::Create(ScriptState* script_state, ExceptionState& exception_state)
{
    ScriptValue undefined(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate()));
    return Create(script_state, undefined, undefined, undefined, exception_state);
}

TransformStream* TransformStream::Create(ScriptState* script_state, ScriptValue transform_stream_transformer, ExceptionState& exception_state)
{
    ScriptValue undefined(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate()));
    return Create(script_state, transform_stream_transformer, undefined, undefined, exception_state);
}

TransformStream* TransformStream::Create(
    ScriptState* script_state, ScriptValue transform_stream_transformer, ScriptValue writable_strategy, ExceptionState& exception_state)
{
    ScriptValue undefined(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate()));
    return Create(script_state, transform_stream_transformer, writable_strategy, undefined, exception_state);
}

TransformStream* TransformStream::Create(
    ScriptState* script_state, ScriptValue transformer, ScriptValue writable_strategy, ScriptValue readable_strategy, ExceptionState& exception_state)
{
    auto* ts = MakeGarbageCollected<TransformStream>();

    ts->InitInternal(script_state, transformer, writable_strategy, readable_strategy, exception_state);
...
// static
TransformStream* TransformStream::Create(ScriptState* script_state, TransformStreamTransformer* transformer, ExceptionState& exception_state)
{
    auto* transform_algorithm = MakeGarbageCollected<TransformAlgorithm>(transformer);
    auto* flush_algorithm = MakeGarbageCollected<FlushAlgorithm>(transformer);
    auto* size_algorithm = CreateDefaultSizeAlgorithm();
    auto* stream
        = Create(script_state, CreateTrivialStartAlgorithm(), transform_algorithm, flush_algorithm, 1, size_algorithm, 0, size_algorithm, exception_state);
    DCHECK(stream);
    DCHECK(!exception_state.HadException());
    TransformStreamDefaultController* controller = stream->transform_stream_controller_;
    transform_algorithm->SetController(controller);
    flush_algorithm->SetController(controller);
    return stream;
}

TransformStream* TransformStream::Create(ScriptState* script_state, StreamStartAlgorithm* start_algorithm, StreamAlgorithm* transform_algorithm,
    StreamAlgorithm* flush_algorithm, double writable_high_water_mark, StrategySizeAlgorithm* writable_size_algorithm, double readable_high_water_mark,
    StrategySizeAlgorithm* readable_size_algorithm, ExceptionState& exception_state)
{
    ...
    auto* start_promise = MakeGarbageCollected<ScriptPromiseResolver<IDLAny>>(script_state);

    // 9. Perform ! InitializeTransformStream(stream, startPromise,
    //    writableHighWaterMark, writableSizeAlgorithm, readableHighWaterMark,
    //    readableSizeAlgorithm).
    Initialize(script_state, stream, start_promise, writable_high_water_mark, writable_size_algorithm, readable_high_water_mark, readable_size_algorithm,
        exception_state);
    ...
    TransformStreamDefaultController::SetUp(script_state, stream, controller, transform_algorithm, flush_algorithm);

    // 12. Let startResult be the result of performing startAlgorithm. (This may
    //     throw an exception.)
    v8::MaybeLocal<v8::Promise> start_result_maybe = start_algorithm->Run(script_state, exception_state);
...
TransformStream::TransformStream(ReadableStream* readable, WritableStream* writable)
    : readable_(readable)
    , writable_(writable)
{
}
...
class TransformStream::ReturnStartPromiseAlgorithm final : public StreamStartAlgorithm {
public:
    explicit ReturnStartPromiseAlgorithm(ScriptPromiseResolver<IDLAny>* start_promise)
...
//
class TransformStream::DefaultSinkWriteAlgorithm final : public StreamAlgorithm {
public:
    ...
    v8::Local<v8::Promise> Run(ScriptState* script_state, int argc, v8::Local<v8::Value> argv[]) override
    {
        ...
        TransformStreamDefaultController* controller = stream_->transform_stream_controller_;
        ...
        if (stream_->had_backpressure_) {
            ...
            auto* backpressure_change_promise = stream_->backpressure_change_promise_.Get();
            ...
            public:
                ResponseFunction(ScriptState* script_state, TransformStream* stream, v8::Local<v8::Value> chunk)
                    : stream_(stream)
                    , chunk_(script_state->GetIsolate(), chunk)
                {
                }

                v8::Local<v8::Value> CallWithLocal(ScriptState* script_state, v8::Local<v8::Value>) override
                {
                    ...
                    if (writable->IsErroring()) {
                        return PromiseReject(script_state, writable->GetStoredError(isolate));
                    }
                    ...
                    //    controller, chunk).
                    return TransformStreamDefaultController::PerformTransform(script_state, stream_->transform_stream_controller_, chunk_.Get(isolate));
                }
            ...
            return StreamThenPromise(
                script_state, backpressure_change_promise->V8Promise(), MakeGarbageCollected<ResponseFunction>(script_state, stream_, chunk));
        }

        //  4. Return ! TransformStreamDefaultControllerPerformTransform(controller,
        //     chunk).
        return TransformStreamDefaultController::PerformTransform(script_state, controller, chunk);
    }
...
class TransformStream::DefaultSinkAbortAlgorithm final : public StreamAlgorithm {
public:
...
class TransformStream::DefaultSinkCloseAlgorithm final : public StreamAlgorithm {
public:
    ...
    v8::Local<v8::Promise> Run(ScriptState* script_state, int argc, v8::Local<v8::Value> argv[]) override
    {
        ...
        auto flush_promise = controller->flush_algorithm_->Run(script_state, 0, nullptr);
        ...
        public:
            explicit ResolveFunction(TransformStream* stream)
                : stream_(stream)
            {
            }
            ...
            v8::Local<v8::Value> CallWithLocal(ScriptState* script_state, v8::Local<v8::Value>) override
            {
                ...
                if (ReadableStream::IsErrored(stream_->readable_)) {
                    ...
                    return PromiseReject(script_state, stream_->readable_->GetStoredError(script_state->GetIsolate()));
                }
        ...
        public:
            explicit RejectFunction(TransformStream* stream)
                : stream_(stream)
            {
            }

            v8::Local<v8::Value> CallWithLocal(ScriptState* script_state, v8::Local<v8::Value> r) override
            {
        ...
        return StreamThenPromise(script_state, flush_promise, MakeGarbageCollected<ResolveFunction>(stream_), MakeGarbageCollected<RejectFunction>(stream_));
    }
...
class TransformStream::DefaultSourcePullAlgorithm final : public StreamAlgorithm {
public:
...
class TransformStream::DefaultSourceCancelAlgorithm final : public StreamAlgorithm {
public:
...
void TransformStream::InitInternal(ScriptState* script_state, ScriptValue raw_transformer, ScriptValue raw_writable_strategy, ScriptValue raw_readable_strategy,
    ExceptionState& exception_state)
{
    // TODO(ricea): Move this to IDL.
    UseCounter::Count(ExecutionContext::From(script_state), WebFeature::kTransformStreamConstructor);
    ...
    ScriptValueToObject(script_state, raw_transformer, &transformer, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    StrategyUnpacker writable_strategy_unpacker(script_state, raw_writable_strategy, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    StrategyUnpacker readable_strategy_unpacker(script_state, raw_readable_strategy, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    if (!transformer->Get(context, V8AtomicString(isolate, "writableType")).ToLocal(&writable_type)) {
        return;
    }
    ...
    auto* writable_size_algorithm = writable_strategy_unpacker.MakeSizeAlgorithm(script_state, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    double writable_high_water_mark = writable_strategy_unpacker.GetHighWaterMark(script_state, 1, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    if (!transformer->Get(context, V8AtomicString(isolate, "readableType")).ToLocal(&readable_type)) {
        return;
    }
    ...
    auto* readable_size_algorithm = readable_strategy_unpacker.MakeSizeAlgorithm(script_state, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    double readable_high_water_mark = readable_strategy_unpacker.GetHighWaterMark(script_state, 0, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    //     writableHighWaterMark, writableSizeAlgorithm, readableHighWaterMark,
    //     readableSizeAlgorithm).
    Initialize(script_state, this, start_promise, writable_high_water_mark, writable_size_algorithm, readable_high_water_mark, readable_size_algorithm,
        exception_state);

    // 17. Perform ? SetUpTransformStreamDefaultControllerFromTransformer(this,
    //     transformer).
    const auto controller_value = TransformStreamDefaultController::SetUpFromTransformer(script_state, this, transformer, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    v8::MaybeLocal<v8::Value> start_result_maybe = CallOrNoop1(script_state, transformer, "start", "transformer.start", controller_value, exception_state);
...
void TransformStream::Initialize(ScriptState* script_state, TransformStream* stream, ScriptPromiseResolver<IDLAny>* start_promise,
    double writable_high_water_mark, StrategySizeAlgorithm* writable_size_algorithm, double readable_high_water_mark,
    StrategySizeAlgorithm* readable_size_algorithm, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#initialize-transform-stream
    // 1. Let startAlgorithm be an algorithm that returns startPromise.
    auto* start_algorithm = MakeGarbageCollected<ReturnStartPromiseAlgorithm>(start_promise);
    ...
    auto* write_algorithm = MakeGarbageCollected<DefaultSinkWriteAlgorithm>(stream);
    ...
    auto* abort_algorithm = MakeGarbageCollected<DefaultSinkAbortAlgorithm>(stream);

    // 4. Let closeAlgorithm be the following steps:
    //    a. Return ! TransformStreamDefaultSinkCloseAlgorithm(stream).
    auto* close_algorithm = MakeGarbageCollected<DefaultSinkCloseAlgorithm>(stream);
    ...
    stream->writable_ = WritableStream::Create(
        script_state, start_algorithm, write_algorithm, close_algorithm, abort_algorithm, writable_high_water_mark, writable_size_algorithm, exception_state);
    DCHECK(!exception_state.HadException());

    // 6. Let pullAlgorithm be the following steps:
    //    a. Return ! TransformStreamDefaultSourcePullAlgorithm(stream).
    auto* pull_algorithm = MakeGarbageCollected<DefaultSourcePullAlgorithm>(stream);

    // 7. Let cancelAlgorithm be the following steps, taking a reason argument:
    //    a. Perform ! TransformStreamErrorWritableAndUnblockWrite(stream,
    //       reason).
    //    b. Return a promise resolved with undefined.
    auto* cancel_algorithm = MakeGarbageCollected<DefaultSourceCancelAlgorithm>(stream);
    ...
    stream->readable_ = ReadableStream::Create(
        script_state, start_algorithm, pull_algorithm, cancel_algorithm, readable_high_water_mark, readable_size_algorithm, exception_state);
    DCHECK(!exception_state.HadException());
    ...
    stream->backpressure_change_promise_ = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state);
...
void TransformStream::Error(ScriptState* script_state, TransformStream* stream, v8::Local<v8::Value> e)
{
    // https://streams.spec.whatwg.org/#transform-stream-error
    // 1. Perform ! ReadableStreamDefaultControllerError(stream.[[readable]].
    //    [[readableStreamController]], e).
    ReadableStreamDefaultController::Error(script_state, stream->GetReadableController(), e);

    // 2. Perform ! TransformStreamErrorWritableAndUnblockWrite(stream, e).
    ErrorWritableAndUnblockWrite(script_state, stream, e);
}

void TransformStream::ErrorWritableAndUnblockWrite(ScriptState* script_state, TransformStream* stream, v8::Local<v8::Value> e)
{
    // https://streams.spec.whatwg.org/#transform-stream-error-writable-and-unblock-write
    // 1. Perform ! TransformStreamDefaultControllerClearAlgorithms(stream.
    //    [[transformStreamController]]).
    TransformStreamDefaultController::ClearAlgorithms(stream->transform_stream_controller_);

    // 2. Perform !
    //    WritableStreamDefaultControllerErrorIfNeeded(stream.[[writable]].
    //    [[writableStreamController]], e).
    WritableStreamDefaultController::ErrorIfNeeded(script_state, stream->writable_->Controller(), e);
...
void TransformStream::SetBackpressure(ScriptState* script_state, TransformStream* stream, bool backpressure)
{
