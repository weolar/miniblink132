...
public:
    ...
    static TransformStream* Create(ScriptState*, ScriptValue transformer, ExceptionState&);
    static TransformStream* Create(ScriptState*, ScriptValue transformer, ScriptValue writable_strategy, ExceptionState&);
    static TransformStream* Create(ScriptState*, ScriptValue transformer, ScriptValue writable_strategy, ScriptValue readable_strategy, ExceptionState&);

    // Creates a TransformStream from a C++ object.
    static TransformStream* Create(ScriptState*, TransformStreamTransformer*, ExceptionState&);

    // https://streams.spec.whatwg.org/#create-transform-stream
    static TransformStream* Create(ScriptState*, StreamStartAlgorithm* start_algorithm, StreamAlgorithm* transform_algorithm, StreamAlgorithm* flush_algorithm,
        double writable_high_water_mark, StrategySizeAlgorithm* writable_size_algorithm, double readable_high_water_mark,
        StrategySizeAlgorithm* readable_size_algorithm, ExceptionState&);
...
private:
    ...
    void InitInternal(ScriptState*, ScriptValue raw_transformer, ScriptValue raw_writable_strategy, ScriptValue raw_readable_strategy, ExceptionState&);

    // https://streams.spec.whatwg.org/#initialize-transform-stream
    static void Initialize(ScriptState*, TransformStream*, ScriptPromiseResolver<IDLAny>* start_promise, double writable_high_water_mark,
        StrategySizeAlgorithm* writable_size_algorithm, double readable_high_water_mark, StrategySizeAlgorithm* readable_size_algorithm, ExceptionState&);

    // https://streams.spec.whatwg.org/#transform-stream-error
    static void Error(ScriptState*, TransformStream*, v8::Local<v8::Value> e);

    // https://streams.spec.whatwg.org/#transform-stream-error-writable-and-unblock-write
    static void ErrorWritableAndUnblockWrite(ScriptState*, TransformStream*, v8::Local<v8::Value> e);

    // https://streams.spec.whatwg.org/#transform-stream-set-backpressure
    static void SetBackpressure(ScriptState*, TransformStream*, bool backpressure);
