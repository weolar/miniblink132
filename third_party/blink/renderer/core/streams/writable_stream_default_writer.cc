...
{
    return String::Format("This writable stream writer has been released and cannot be %s", verbed);
}

v8::Local<v8::Value> CreateWriterLockReleasedException(v8::Isolate* isolate, const char* verbed)
{
    return v8::Exception::TypeError(V8String(isolate, CreateWriterLockReleasedMessage(verbed)));
}

} // namespace

WritableStreamDefaultWriter* WritableStreamDefaultWriter::Create(ScriptState* script_state, WritableStream* stream, ExceptionState& exception_state)
{
    auto* writer = MakeGarbageCollected<WritableStreamDefaultWriter>(script_state, static_cast<WritableStream*>(stream), exception_state);
...
WritableStreamDefaultWriter::WritableStreamDefaultWriter(ScriptState* script_state, WritableStream* stream, ExceptionState& exception_state)
    //  3. Set this.[[ownerWritableStream]] to stream.
    : owner_writable_stream_(stream)
    , closed_resolver_(MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state))
    , ready_resolver_(MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state))
{
    ...
    if (WritableStream::IsLocked(stream)) {
        exception_state.ThrowTypeError("Cannot create writer when WritableStream is locked");
        return;
    }
    ...
    case WritableStream::kWritable: {
        ...
        if (WritableStream::CloseQueuedOrInFlight(stream) || !stream->HasBackpressure()) {
    ...
    case WritableStream::kErrored: {
        ...
        const auto stored_error = ScriptValue(isolate, stream->GetStoredError(isolate));
...
ScriptPromise<IDLUndefined> WritableStreamDefaultWriter::closed(ScriptState* script_state) const
{
...
ScriptValue WritableStreamDefaultWriter::desiredSize(ScriptState* script_state, ExceptionState& exception_state) const
{
    auto* isolate = script_state->GetIsolate();
    ...
    if (!owner_writable_stream_) {
        exception_state.ThrowTypeError(CreateWriterLockReleasedMessage("used to get the desiredSize"));
        return ScriptValue();
    }
...
ScriptPromise<IDLUndefined> WritableStreamDefaultWriter::ready(ScriptState* script_state) const
{
...
ScriptPromise<IDLUndefined> WritableStreamDefaultWriter::abort(ScriptState* script_state, ExceptionState& exception_state)
{
    return abort(script_state, ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())), exception_state);
}

ScriptPromise<IDLUndefined> WritableStreamDefaultWriter::abort(ScriptState* script_state, ScriptValue reason, ExceptionState& exception_state)
{
...
ScriptPromise<IDLUndefined> WritableStreamDefaultWriter::close(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    if (WritableStream::CloseQueuedOrInFlight(stream)) {
        exception_state.ThrowTypeError("Cannot close a writable stream that has "
...
ScriptPromise<IDLUndefined> WritableStreamDefaultWriter::write(ScriptState* script_state, ExceptionState& exception_state)
{
    return write(script_state, ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())), exception_state);
}

ScriptPromise<IDLUndefined> WritableStreamDefaultWriter::write(ScriptState* script_state, ScriptValue chunk, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#default-writer-write
    //  2. If this.[[ownerWritableStream]] is undefined, return a promise rejected
    //     with a TypeError exception.
    if (!owner_writable_stream_) {
        exception_state.ThrowTypeError(CreateWriterLockReleasedMessage("written to"));
        return EmptyPromise();
    }
...
void WritableStreamDefaultWriter::EnsureReadyPromiseRejected(ScriptState* script_state, WritableStreamDefaultWriter* writer, v8::Local<v8::Value> error)
{
    if (!script_state->ContextIsValid()) {
        return;
    }
    ...
    if (writer->ready_resolver_->Promise().V8Promise()->State() != v8::Promise::kPending) {
        //  2. Otherwise, set writer.[[readyPromise]] to a promise rejected with
        //     error.
        writer->ready_resolver_ = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state);
    }
...
ScriptPromise<IDLUndefined> WritableStreamDefaultWriter::CloseWithErrorPropagation(ScriptState* script_state, WritableStreamDefaultWriter* writer)
{
    ...
    if (WritableStream::CloseQueuedOrInFlight(stream) || state == WritableStream::kClosed) {
...
void WritableStreamDefaultWriter::Release(ScriptState* script_state, WritableStreamDefaultWriter* writer)
{
    ...
    const auto released_error = v8::Exception::TypeError(V8String(script_state->GetIsolate(),
...
ScriptPromiseResolver<IDLUndefined>* WritableStreamDefaultWriter::Write(
    ScriptState* script_state, WritableStreamDefaultWriter* writer, v8::Local<v8::Value> chunk, ExceptionState& exception_state)
{
    ...
    double chunk_size = WritableStreamDefaultController::GetChunkSize(script_state, controller, chunk);
    ...
    if (WritableStream::CloseQueuedOrInFlight(stream)) {
        resolver->Reject(v8::Exception::TypeError(WritableStream::CreateCannotActionOnStateStreamMessage(isolate, "write to", "closing")));
        return resolver;
    }
    if (state == WritableStream::kClosed) {
        resolver->Reject(WritableStream::CreateCannotActionOnStateStreamException(isolate, "write to", WritableStream::kClosed));
        return resolver;
    }
    ...
    //     chunkSize).
    WritableStreamDefaultController::Write(script_state, controller, chunk, chunk_size, exception_state);

    // 13. Return promise.
    return resolver;
}

std::optional<double> WritableStreamDefaultWriter::GetDesiredSizeInternal() const
{
    ...
    default:
        ...
        return WritableStreamDefaultController::GetDesiredSize(stream->Controller());
    }
}
...
ScriptPromise<IDLUndefined> WritableStreamDefaultWriter::Abort(ScriptState* script_state, WritableStreamDefaultWriter* writer, v8::Local<v8::Value> reason)
{
...
ScriptPromise<IDLUndefined> WritableStreamDefaultWriter::Close(ScriptState* script_state, WritableStreamDefaultWriter* writer)
{
...
void WritableStreamDefaultWriter::EnsureClosedPromiseRejected(ScriptState* script_state, WritableStreamDefaultWriter* writer, v8::Local<v8::Value> error)
{
    if (!script_state->ContextIsValid()) {
        return;
    }
    ...
    if (writer->closed_resolver_->Promise().V8Promise()->State() != v8::Promise::kPending) {
        //  2. Otherwise, set writer.[[closedPromise]] to a promise rejected with
        //     error.
        writer->closed_resolver_ = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state);
    }
...
v8::Local<v8::Value> WritableStreamDefaultWriter::GetDesiredSize(v8::Isolate* isolate, const WritableStreamDefaultWriter* writer)
{
    // https://streams.spec.whatwg.org/#writable-stream-default-writer-get-desired-size
    //  1. Let stream be writer.[[ownerWritableStream]].
    //  2. Let state be stream.[[state]].
