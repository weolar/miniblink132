...
ReadableStreamDefaultController* TransformStreamDefaultController::GetDefaultController(TransformStream* stream)
{
    // The TransformStreamDefaultController will always use a
    // ReadableStreamDefaultController. Hence, it is safe to down-cast here.
    return To<ReadableStreamDefaultController>(stream->readable_->GetController());
}
...
std::optional<double> TransformStreamDefaultController::desiredSize() const
{
    ...
    const auto* readable_controller = GetDefaultController(controlled_transform_stream_);
...
void TransformStreamDefaultController::enqueue(ScriptState* script_state, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#ts-default-controller-enqueue
    // 2. Perform ? TransformStreamDefaultControllerEnqueue(this, chunk).
    Enqueue(script_state, this, v8::Undefined(script_state->GetIsolate()), exception_state);
}

void TransformStreamDefaultController::enqueue(ScriptState* script_state, ScriptValue chunk, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#ts-default-controller-enqueue
    // 2. Perform ? TransformStreamDefaultControllerEnqueue(this, chunk).
...
void TransformStreamDefaultController::error(ScriptState* script_state, ScriptValue reason)
{
    // https://streams.spec.whatwg.org/#ts-default-controller-error
    // 2. Perform ! TransformStreamDefaultControllerError(this, reason).
...
// the standard.
class TransformStreamDefaultController::DefaultTransformAlgorithm final : public StreamAlgorithm {
public:
    explicit DefaultTransformAlgorithm(TransformStreamDefaultController* controller)
...
void TransformStreamDefaultController::SetUp(ScriptState* script_state, TransformStream* stream, TransformStreamDefaultController* controller,
    StreamAlgorithm* transform_algorithm, StreamAlgorithm* flush_algorithm)
{
    ...
    controller->reject_function_ = MakeGarbageCollected<PerformTransformRejectFunction>(controller->controlled_transform_stream_);
}

v8::Local<v8::Value> TransformStreamDefaultController::SetUpFromTransformer(
    ScriptState* script_state, TransformStream* stream, v8::Local<v8::Object> transformer, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#set-up-transform-stream-default-controller-from-transformer
    ...
    // not crash.
    auto controller_value = ToV8Traits<TransformStreamDefaultController>::ToV8(script_state, controller);
    ...
    v8::MaybeLocal<v8::Value> method_maybe = ResolveMethod(script_state, transformer, "transform", "transformer.transform", exception_state);
    ...
    if (transform_method->IsUndefined()) {
        ...
        transform_algorithm = MakeGarbageCollected<DefaultTransformAlgorithm>(controller);
    } else {
        ...
        //          controller »).
        transform_algorithm = CreateAlgorithmFromResolvedMethod(script_state, transformer, transform_method, controller_value);
    }

    // 6. Let flushAlgorithm be ? CreateAlgorithmFromUnderlyingMethod(transformer,
    //    "flush", 0, « controller »).
    auto* flush_algorithm = CreateAlgorithmFromUnderlyingMethod(script_state, transformer, "flush", "transformer.flush", controller_value, exception_state);
...
void TransformStreamDefaultController::ClearAlgorithms(TransformStreamDefaultController* controller)
{
...
void TransformStreamDefaultController::Enqueue(
    ScriptState* script_state, TransformStreamDefaultController* controller, v8::Local<v8::Value> chunk, ExceptionState& exception_state)
{
    ...
    if (!ReadableStreamDefaultController::CanCloseOrEnqueue(readable_controller)) {
        exception_state.ThrowTypeError(ReadableStreamDefaultController::EnqueueExceptionMessage(readable_controller));
        return;
    }
    ...
    ReadableStreamDefaultController::Enqueue(script_state, readable_controller, chunk, PassThroughException(isolate));
    ...
    if (rethrow_scope.HasCaught()) {
        ...
        //    enqueueResult.[[Value]]).
        TransformStream::ErrorWritableAndUnblockWrite(script_state, stream, rethrow_scope.GetException());

        // b. Throw stream.[[readable]].[[storedError]].
        V8ThrowException::ThrowException(isolate, stream->readable_->GetStoredError(isolate));
        return;
    }

    // 6. Let backpressure be ! ReadableStreamDefaultControllerHasBackpressure(
    //    readableController).
    bool backpressure = ReadableStreamDefaultController::HasBackpressure(readable_controller);
...
void TransformStreamDefaultController::Error(ScriptState* script_state, TransformStreamDefaultController* controller, v8::Local<v8::Value> e)
{
    ...
    //    e).
    TransformStream::Error(script_state, controller->controlled_transform_stream_, e);
}

v8::Local<v8::Promise> TransformStreamDefaultController::PerformTransform(
    ScriptState* script_state, TransformStreamDefaultController* controller, v8::Local<v8::Value> chunk)
{
    if (!script_state->ContextIsValid()) {
        v8::Local<v8::Value> error = V8ThrowException::CreateTypeError(script_state->GetIsolate(), "invalid realm");
        v8::MaybeLocal<v8::Value> result_maybe
            = controller->reject_function_->ToV8Function(script_state)->Call(script_state->GetContext(), v8::Undefined(script_state->GetIsolate()), 1, &error);
    ...
    auto transform_promise = controller->transform_algorithm_->Run(script_state, 1, &chunk);
    DCHECK(!transform_promise.IsEmpty());

    // 2. Return the result of transforming transformPromise ...
    v8::Local<v8::Promise> streamed_promise = StreamThenPromise(script_state, transform_promise, nullptr, controller->reject_function_);
    v8::Local<v8::Value> escapable_streamed_promise = scope.Escape(streamed_promise);
    return escapable_streamed_promise.As<v8::Promise>();
}

void TransformStreamDefaultController::Terminate(ScriptState* script_state, TransformStreamDefaultController* controller)
{
    ...
    // 2. Let readableController be
    //    stream.[[readable]].[[readableStreamController]].
    ReadableStreamDefaultController* readable_controller = GetDefaultController(stream);
    ...
    //    terminated.
    const auto error = v8::Exception::TypeError(V8String(script_state->GetIsolate(), "The transform stream has been terminated"));
