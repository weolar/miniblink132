...
WritableStreamDefaultController* WritableStreamDefaultController::From(ScriptState* script_state, ScriptValue controller)
{
    CHECK(controller.IsObject());
    auto* controller_impl = V8WritableStreamDefaultController::ToWrappable(script_state->GetIsolate(), controller.V8Value().As<v8::Object>());
...
void WritableStreamDefaultController::error(ScriptState* script_state, ScriptValue e)
{
...
v8::Local<v8::Promise> WritableStreamDefaultController::AbortSteps(ScriptState* script_state, v8::Local<v8::Value> reason)
{
...
void WritableStreamDefaultController::SetUp(ScriptState* script_state, WritableStream* stream, WritableStreamDefaultController* controller,
    StreamStartAlgorithm* start_algorithm, StreamAlgorithm* write_algorithm, StreamAlgorithm* close_algorithm, StreamAlgorithm* abort_algorithm,
    double high_water_mark, StrategySizeAlgorithm* size_algorithm, ExceptionState& exception_state)
{
    ...
    public:
        explicit ResolvePromiseFunction(WritableStream* stream)
            : stream_(stream)
        {
        }
        ...
        void CallWithLocal(ScriptState* script_state, v8::Local<v8::Value>) override
        {
            ...
            CHECK(state == WritableStream::kWritable || state == WritableStream::kErroring);
            ...
            //         controller).
            WritableStreamDefaultController::AdvanceQueueIfNeeded(script_state, controller);
        }
    ...
    public:
        explicit RejectPromiseFunction(WritableStream* stream)
            : stream_(stream)
        {
        }
    ...
    StreamThenPromise(script_state, start_promise, MakeGarbageCollected<ResolvePromiseFunction>(stream), MakeGarbageCollected<RejectPromiseFunction>(stream));

    class ProcessWriteResolveFunction final : public PromiseHandler {
    public:
        ProcessWriteResolveFunction(WritableStream* stream, WritableStreamDefaultController* controller)
            : stream_(stream)
        ...
        void CallWithLocal(ScriptState* script_state, v8::Local<v8::Value>) override
        {
            ...
            //         state is "writable",
            if (!WritableStream::CloseQueuedOrInFlight(stream_) && state == WritableStream::kWritable) {
                //          i. Let backpressure be !
                //             WritableStreamDefaultControllerGetBackpressure(
                //             controller).
                const bool backpressure = WritableStreamDefaultController::GetBackpressure(controller_);
            ...
            //         controller).
            WritableStreamDefaultController::AdvanceQueueIfNeeded(script_state, controller_);
        }
    ...
    public:
        ProcessWriteRejectFunction(WritableStream* stream, WritableStreamDefaultController* controller)
            : stream_(stream)
            , controller_(controller)
        {
        }
        ...
        void CallWithLocal(ScriptState* script_state, v8::Local<v8::Value> reason) override
        {
            ...
            //         reason).
            WritableStream::FinishInFlightWriteWithError(script_state, stream_, reason);
        }
    ...
    controller->resolve_function_ = MakeGarbageCollected<ProcessWriteResolveFunction>(controller->controlled_writable_stream_, controller);
    controller->reject_function_ = MakeGarbageCollected<ProcessWriteRejectFunction>(controller->controlled_writable_stream_, controller);
}

// TODO(ricea): Should this be a constructor?
void WritableStreamDefaultController::SetUpFromUnderlyingSink(ScriptState* script_state, WritableStream* stream, v8::Local<v8::Object> underlying_sink,
    double high_water_mark, StrategySizeAlgorithm* size_algorithm, ExceptionState& exception_state)
{
    ...
    // not crash.
    auto controller_value = ToV8Traits<WritableStreamDefaultController>::ToV8(script_state, controller);

    //  3. Let startAlgorithm be the following steps:
    //      a. Return ? InvokeOrNoop(underlyingSink, "start", « controller »).
    auto* start_algorithm = CreateStartAlgorithm(script_state, underlying_sink, "underlyingSink.start", controller_value);

    //  4. Let writeAlgorithm be ? CreateAlgorithmFromUnderlyingMethod(
    //     underlyingSink, "write", 1, « controller »).
    auto* write_algorithm
        = CreateAlgorithmFromUnderlyingMethod(script_state, underlying_sink, "write", "underlyingSink.write", controller_value, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    auto* close_algorithm
        = CreateAlgorithmFromUnderlyingMethod(script_state, underlying_sink, "close", "underlyingSink.close", v8::MaybeLocal<v8::Value>(), exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    auto* abort_algorithm
        = CreateAlgorithmFromUnderlyingMethod(script_state, underlying_sink, "abort", "underlyingSink.abort", v8::MaybeLocal<v8::Value>(), exception_state);
    if (exception_state.HadException()) {
        return;
    }
...
void WritableStreamDefaultController::Close(ScriptState* script_state, WritableStreamDefaultController* controller)
{
...
double WritableStreamDefaultController::GetChunkSize(ScriptState* script_state, WritableStreamDefaultController* controller, v8::Local<v8::Value> chunk)
{
    if (!controller->strategy_size_algorithm_) {
        DCHECK_NE(controller->controlled_writable_stream_->GetState(), WritableStream::kWritable);
    ...
    auto return_value = controller->strategy_size_algorithm_->Run(script_state, chunk);
...
double WritableStreamDefaultController::GetDesiredSize(const WritableStreamDefaultController* controller)
{
    // https://streams.spec.whatwg.org/#writable-stream-default-controller-get-desired-size
    //  1. Return controller.[[strategyHWM]] − controller.[[queueTotalSize]].
    return controller->strategy_high_water_mark_ - controller->queue_->TotalSize();
}

void WritableStreamDefaultController::Write(
    ScriptState* script_state, WritableStreamDefaultController* controller, v8::Local<v8::Value> chunk, double chunk_size, ExceptionState& exception_state)
{
    ...
    //  1. Let writeRecord be Record {[[chunk]]: chunk}.
    {
        ...
        controller->queue_->EnqueueValueWithSize(isolate, chunk, chunk_size, PassThroughException(isolate));
    ...
    if (!WritableStream::CloseQueuedOrInFlight(stream) && stream->GetState() == WritableStream::kWritable) {
...
void WritableStreamDefaultController::ErrorIfNeeded(ScriptState* script_state, WritableStreamDefaultController* controller, v8::Local<v8::Value> error)
{
...
void WritableStreamDefaultController::ClearAlgorithms(WritableStreamDefaultController* controller)
{
...
void WritableStreamDefaultController::AdvanceQueueIfNeeded(ScriptState* script_state, WritableStreamDefaultController* controller)
{
    ...
    const auto chunk = controller->queue_->PeekQueueValue(script_state->GetIsolate());
    ...
    WritableStreamDefaultController::ProcessWrite(script_state, controller, chunk);
}

void WritableStreamDefaultController::ProcessClose(ScriptState* script_state, WritableStreamDefaultController* controller)
{
    ...
    const auto sinkClosePromise = controller->close_algorithm_->Run(script_state, 0, nullptr);
    ...
    public:
        explicit ResolveFunction(WritableStream* stream)
            : stream_(stream)
        {
        }
    ...
    public:
        explicit RejectFunction(WritableStream* stream)
            : stream_(stream)
        {
        }
        ...
        void CallWithLocal(ScriptState* script_state, v8::Local<v8::Value> reason) override
        {
            ...
            //         reason).
            WritableStream::FinishInFlightCloseWithError(script_state, stream_, reason);
        }
    ...
    StreamThenPromise(script_state, sinkClosePromise, MakeGarbageCollected<ResolveFunction>(stream), MakeGarbageCollected<RejectFunction>(stream));
}

void WritableStreamDefaultController::ProcessWrite(ScriptState* script_state, WritableStreamDefaultController* controller, v8::Local<v8::Value> chunk)
{
    // https://streams.spec.whatwg.org/#writable-stream-default-controller-process-write
    //  1. Let stream be controller.[[controlledWritableStream]].
    WritableStream* stream = controller->controlled_writable_stream_;
    ...
    const auto sinkWritePromise = controller->write_algorithm_->Run(script_state, 1, &chunk);

    StreamThenPromise(script_state, sinkWritePromise, controller->resolve_function_, controller->reject_function_);
}

bool WritableStreamDefaultController::GetBackpressure(const WritableStreamDefaultController* controller)
{
...
void WritableStreamDefaultController::Error(ScriptState* script_state, WritableStreamDefaultController* controller, v8::Local<v8::Value> error)
{
...
void WritableStreamDefaultController::Abort(ScriptState* script_state, ScriptValue reason)
{
