...
class ReadableStreamDefaultReader::DefaultReaderReadRequest final : public ReadRequest {
public:
    explicit DefaultReaderReadRequest(ScriptPromiseResolver<ReadableStreamReadResult>* resolver)
        : resolver_(resolver)
    {
    }
    ...
    {
        auto* read_result = ReadableStreamReadResult::Create();
        read_result->setValue(ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())));
...
ReadableStreamDefaultReader* ReadableStreamDefaultReader::Create(ScriptState* script_state, ReadableStream* stream, ExceptionState& exception_state)
{
...
ReadableStreamDefaultReader::ReadableStreamDefaultReader(ScriptState* script_state, ReadableStream* stream, ExceptionState& exception_state)
...
ScriptPromise<ReadableStreamReadResult> ReadableStreamDefaultReader::read(ScriptState* script_state, ExceptionState& exception_state)
{
...
void ReadableStreamDefaultReader::Read(
    ScriptState* script_state, ReadableStreamDefaultReader* reader, ReadRequest* read_request, ExceptionState& exception_state)
{
    ...
    case ReadableStream::kReadable:
        ...
        stream->GetController()->PullSteps(script_state, read_request, exception_state);
        break;
    }
}

void ReadableStreamDefaultReader::ErrorReadRequests(ScriptState* script_state, ReadableStreamDefaultReader* reader, v8::Local<v8::Value> e)
{
...
void ReadableStreamDefaultReader::Release(ScriptState* script_state, ReadableStreamDefaultReader* reader)
{
    ...
    v8::Local<v8::Value> e = V8ThrowException::CreateTypeError(script_state->GetIsolate(), "Releasing Default reader");

    // 3. Perform ! ReadableStreamDefaultReaderErrorReadRequests(reader, e).
    ErrorReadRequests(script_state, reader, e);
}

void ReadableStreamDefaultReader::releaseLock(ScriptState* script_state, ExceptionState& exception_state)
{
...
void ReadableStreamDefaultReader::SetUpDefaultReader(
    ScriptState* script_state, ReadableStreamDefaultReader* reader, ReadableStream* stream, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#set-up-readable-stream-default-reader
    // 1. If ! IsReadableStreamLocked(stream) is true, throw a TypeError
    //    exception.
    if (ReadableStream::IsLocked(stream)) {
        exception_state.ThrowTypeError("ReadableStreamDefaultReader constructor can only accept readable "
                                       "streams "
