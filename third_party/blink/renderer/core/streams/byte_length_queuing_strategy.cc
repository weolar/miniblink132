...
static const V8PrivateProperty::SymbolKey kByteLengthQueuingStrategySizeFunction;
...
public:
    ...
    void CallRaw(ScriptState* script_state, const v8::FunctionCallbackInfo<v8::Value>& args) override
    {
        ...
        if (!chunk_as_object->Get(context, V8AtomicString(isolate, "byteLength")).ToLocal(&byte_length)) {
...
ByteLengthQueuingStrategy* ByteLengthQueuingStrategy::Create(ScriptState* script_state, const QueuingStrategyInit* init)
{
    return MakeGarbageCollected<ByteLengthQueuingStrategy>(script_state, init);
}

ByteLengthQueuingStrategy::ByteLengthQueuingStrategy(ScriptState* script_state, const QueuingStrategyInit* init)
...
ScriptValue ByteLengthQueuingStrategy::size(ScriptState* script_state) const
{
    ...
    return GetCachedSizeFunction(script_state, kByteLengthQueuingStrategySizeFunction, &ByteLengthQueuingStrategySizeFunction::CreateFunction);
}
