...
ReadableStreamBYOBRequest::ReadableStreamBYOBRequest(ReadableByteStreamController* controller, NotShared<DOMUint8Array> view)
...
void ReadableStreamBYOBRequest::respond(ScriptState* script_state, uint64_t bytes_written, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#rs-byob-request-respond
    // 1. If this.[[controller]] is undefined, throw a TypeError exception.
    if (!controller_) {
        exception_state.ThrowTypeError("Cannot respond to an invalidated ReadableStreamBYOBRequest");
        return;
    }
    ...
    // bytesWritten).
    ReadableByteStreamController::Respond(script_state, controller_, static_cast<size_t>(bytes_written), exception_state);
}

void ReadableStreamBYOBRequest::respondWithNewView(ScriptState* script_state, NotShared<DOMArrayBufferView> view, ExceptionState& exception_state)
{
    ...
    ReadableByteStreamController::RespondWithNewView(script_state, controller_, view, exception_state);
}
