...
TEST(QueueWithSizesTest, EnqueueIncreasesTotalSize)
{
    ...
    queue->EnqueueValueWithSize(isolate, v8::Undefined(isolate), 4.5, ASSERT_NO_EXCEPTION);
...
TEST(QueueWithSizesTest, EnqueueAddsSize)
{
    ...
    auto* isolate = scope.GetIsolate();
    queue->EnqueueValueWithSize(isolate, v8::Undefined(isolate), 4.5, ASSERT_NO_EXCEPTION);
    queue->EnqueueValueWithSize(isolate, v8::Undefined(isolate), 2.0, ASSERT_NO_EXCEPTION);
    queue->EnqueueValueWithSize(isolate, v8::Undefined(isolate), 1.25, ASSERT_NO_EXCEPTION);
...
TEST_P(QueueWithSizesBadSizeTest, BadSizeThrowsException)
{
    ...
    queue->EnqueueValueWithSize(isolate, v8::Undefined(isolate), GetParam(), exception_state);
    EXPECT_TRUE(exception_state.HadException());
    EXPECT_TRUE(queue->IsEmpty());
}

INSTANTIATE_TEST_SUITE_P(All, QueueWithSizesBadSizeTest, Values(-1, std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::infinity()));
...
TEST(QueueWithSizesTest, DequeueSubtractsSize)
{
    ...
    auto* queue = MakeGarbageCollected<QueueWithSizes>();
    auto* isolate = scope.GetIsolate();
    queue->EnqueueValueWithSize(isolate, v8::Undefined(isolate), 1, ASSERT_NO_EXCEPTION);
...
TEST(QueueWithSizesTest, UsesDoubleArithmetic)
{
    ...
    auto* queue = MakeGarbageCollected<QueueWithSizes>();
    auto* isolate = scope.GetIsolate();
    queue->EnqueueValueWithSize(isolate, v8::Undefined(isolate), 1e-15, ASSERT_NO_EXCEPTION);
    queue->EnqueueValueWithSize(isolate, v8::Undefined(isolate), 1, ASSERT_NO_EXCEPTION);
    // 1e-15 + 1 can be represented in a double.
    EXPECT_EQ(queue->TotalSize(), 1.000000000000001);
    queue->DequeueValue(isolate);
    EXPECT_EQ(queue->TotalSize(), 1.0);
    queue->EnqueueValueWithSize(isolate, v8::Undefined(isolate), 1e-16, ASSERT_NO_EXCEPTION);
