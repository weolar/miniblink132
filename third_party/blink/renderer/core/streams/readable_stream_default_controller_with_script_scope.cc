...
ReadableStreamDefaultControllerWithScriptScope::ReadableStreamDefaultControllerWithScriptScope(
    ScriptState* script_state, ReadableStreamDefaultController* controller)
...
void ReadableStreamDefaultControllerWithScriptScope::Enqueue(v8::Local<v8::Value> js_chunk) const
{
    if (!controller_)
        return;
    ...
    ReadableStreamDefaultController::Enqueue(script_state_, controller_, js_chunk, IGNORE_EXCEPTION);
}

void ReadableStreamDefaultControllerWithScriptScope::Error(v8::Local<v8::Value> js_error)
{
    if (!controller_)
        return;
...
void ReadableStreamDefaultControllerWithScriptScope::Trace(Visitor* visitor) const
{
