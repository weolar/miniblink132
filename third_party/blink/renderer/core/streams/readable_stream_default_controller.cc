...
ReadableStreamDefaultController::ReadableStreamDefaultController(ScriptState* script_state)
    : queue_(MakeGarbageCollected<QueueWithSizes>())
{
    class CallPullIfNeededResolveFunction final : public PromiseHandler {
    public:
        explicit CallPullIfNeededResolveFunction(ReadableStreamDefaultController* controller)
    ...
    public:
        explicit CallPullIfNeededRejectFunction(ReadableStreamDefaultController* controller)
            : controller_(controller)
        {
        }
    ...
    resolve_function_ = MakeGarbageCollected<CallPullIfNeededResolveFunction>(this);
    reject_function_ = MakeGarbageCollected<CallPullIfNeededRejectFunction>(this);
}

void ReadableStreamDefaultController::close(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    if (!CanCloseOrEnqueue(this)) {
        ...
        if (is_close_requested_) {
            errorDescription = "Cannot close a readable stream that has already been requested to "
                               "be closed";
        } else {
...
void ReadableStreamDefaultController::enqueue(ScriptState* script_state, ExceptionState& exception_state)
{
    enqueue(script_state, ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())), exception_state);
}

void ReadableStreamDefaultController::enqueue(ScriptState* script_state, ScriptValue chunk, ExceptionState& exception_state)
{
...
void ReadableStreamDefaultController::error(ScriptState* script_state, ScriptValue e)
{
...
void ReadableStreamDefaultController::Close(ScriptState* script_state, ReadableStreamDefaultController* controller)
{
...
void ReadableStreamDefaultController::Enqueue(
    ScriptState* script_state, ReadableStreamDefaultController* controller, v8::Local<v8::Value> chunk, ExceptionState& exception_state)
{
    ...
    if (ReadableStream::IsLocked(stream) && ReadableStream::GetNumReadRequests(stream) > 0) {
        ReadableStream::FulfillReadRequest(script_state, stream, chunk, false, exception_state);
    } else {
        // 4. Otherwise,
        ...
        std::optional<double> chunk_size = controller->strategy_size_algorithm_->Run(script_state, chunk);
        ...
        //     chunkSize).
        controller->queue_->EnqueueValueWithSize(script_state->GetIsolate(), chunk, chunk_size.value(), PassThroughException(script_state->GetIsolate()));
...
void ReadableStreamDefaultController::Error(ScriptState* script_state, ReadableStreamDefaultController* controller, v8::Local<v8::Value> e)
{
...
bool ReadableStreamDefaultController::CanCloseOrEnqueue(const ReadableStreamDefaultController* controller)
{
...
bool ReadableStreamDefaultController::HasBackpressure(const ReadableStreamDefaultController* controller)
{
...
const char* ReadableStreamDefaultController::EnqueueExceptionMessage(const ReadableStreamDefaultController* controller)
{
...
v8::Local<v8::Promise> ReadableStreamDefaultController::CancelSteps(ScriptState* script_state, v8::Local<v8::Value> reason)
{
...
void ReadableStreamDefaultController::PullSteps(ScriptState* script_state, ReadRequest* read_request, ExceptionState& exception_state)
{
...
void ReadableStreamDefaultController::CallPullIfNeeded(ScriptState* script_state, ReadableStreamDefaultController* controller)
{
    // https://streams.spec.whatwg.org/#readable-stream-default-controller-call-pull-if-needed
    ...
    StreamThenPromise(script_state, pull_promise, controller->resolve_function_, controller->reject_function_);
}

bool ReadableStreamDefaultController::ShouldCallPull(const ReadableStreamDefaultController* controller)
{
...
void ReadableStreamDefaultController::ClearAlgorithms(ReadableStreamDefaultController* controller)
{
...
void ReadableStreamDefaultController::SetUp(ScriptState* script_state, ReadableStream* stream, ReadableStreamDefaultController* controller,
    ...
    StrategySizeAlgorithm* size_algorithm, ExceptionState& exception_state)
{
...
void ReadableStreamDefaultController::SetUpFromUnderlyingSource(ScriptState* script_state, ReadableStream* stream, v8::Local<v8::Object> underlying_source,
    double high_water_mark, StrategySizeAlgorithm* size_algorithm, ExceptionState& exception_state)
{
    ...
    // not crash.
    auto controller_value = ToV8Traits<ReadableStreamDefaultController>::ToV8(script_state, controller);

    // 3. Let startAlgorithm be the following steps:
    //   a. Return ? InvokeOrNoop(underlyingSource, "start", « controller »).
    auto* start_algorithm = CreateStartAlgorithm(script_state, underlying_source, "underlyingSource.start", controller_value);

    // 4. Let pullAlgorithm be ? CreateAlgorithmFromUnderlyingMethod
    //    (underlyingSource, "pull", 0, « controller »).
    auto* pull_algorithm
        = CreateAlgorithmFromUnderlyingMethod(script_state, underlying_source, "pull", "underlyingSource.pull", controller_value, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    auto* cancel_algorithm
        = CreateAlgorithmFromUnderlyingMethod(script_state, underlying_source, "cancel", "underlyingSource.cancel", controller_value, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    //    sizeAlgorithm).
    SetUp(script_state, stream, controller, start_algorithm, pull_algorithm, cancel_algorithm, high_water_mark, size_algorithm, exception_state);
}
