...
public:
    ...
    v8::Local<v8::Promise> Run(ScriptState* script_state, int argc, v8::Local<v8::Value> argv[]) override
    {
        ...
        ReadableStreamBYOBRequest* byob_request = ReadableByteStreamController::GetBYOBRequest(engine_->controller_[branch_]);
        ...
        if (!byob_request) {
            ...
            //   and false.
            engine_->PullWithBYOBReader(script_state, byob_request->view(), branch_, exception_state);
        }
...
public:
    ...
    v8::Local<v8::Promise> Run(ScriptState* script_state, int argc, v8::Local<v8::Value> argv[]) override
    {
        ...
        if (engine_->canceled_[other_branch]) {
            ...
            //     »).
            v8::Local<v8::Value> reason[] = { engine_->reason_[0].Get(isolate), engine_->reason_[1].Get(isolate) };
            v8::Local<v8::Value> composite_reason = v8::Array::New(script_state->GetIsolate(), reason, 2);
            //     ii. Let cancelResult be ! ReadableStreamCancel(stream,
            //     compositeReason).
            auto cancel_result = ReadableStream::Cancel(script_state, engine_->stream_, composite_reason);
...
public:
    ...
    {
        scoped_refptr<scheduler::EventLoop> event_loop = ExecutionContext::From(script_state)->GetAgent()->event_loop();
        v8::Global<v8::Value> value(script_state->GetIsolate(), chunk);
        event_loop->EnqueueMicrotask(WTF::BindOnce(&ByteTeeReadRequest::ChunkStepsBody, WrapPersistent(this), WrapPersistent(script_state), std::move(value)));
    }
    ...
    void CloseSteps(ScriptState* script_state) const override
    {
        ...
        for (int branch = 0; branch < 2; ++branch) {
            if (!engine_->canceled_[branch]) {
                engine_->controller_[branch]->Close(script_state, engine_->controller_[branch]);
                if (try_catch.HasCaught()) {
                    ...
                    ReadableByteStreamController::Error(script_state, engine_->controller_[branch], try_catch.Exception());
                    return;
                }
            }
        }
        ...
        for (int branch = 0; branch < 2; ++branch) {
            if (!engine_->controller_[branch]->pending_pull_intos_.empty()) {
                ReadableByteStreamController::Respond(script_state, engine_->controller_[branch], 0, PassThroughException(isolate));
                if (try_catch.HasCaught()) {
                    // Instead of returning a rejection, which is inconvenient here,
                    // call ControllerError(). The only difference this makes is that it
                    // happens synchronously, but that should not be observable.
...
private:
    void ChunkStepsBody(ScriptState* script_state, v8::Global<v8::Value> value) const
    {
        ...
        NotShared<DOMUint8Array> buffer_view = NativeValueTraits<NotShared<DOMUint8Array>>::NativeValue(isolate, value.Get(isolate), exception_state);
        ...
        for (int branch = 0; branch < 2; ++branch) {
            if (!engine_->canceled_[branch]) {
                v8::TryCatch try_catch(isolate);
                ReadableByteStreamController::Enqueue(script_state, engine_->controller_[branch], chunk[branch], PassThroughException(isolate));
                if (try_catch.HasCaught()) {
                    // Instead of returning a rejection, which is inconvenient here,
                    // call ControllerError(). The only difference this makes is that it
                    // happens synchronously, but that should not be observable.
...
class ByteStreamTeeEngine::ByteTeeReadIntoRequest final : public ReadIntoRequest {
public:
    explicit ByteTeeReadIntoRequest(ByteStreamTeeEngine* engine, ReadableStream* byob_branch, ReadableStream* other_branch, bool for_branch_2)
        : engine_(engine)
    ...
    void ChunkSteps(ScriptState* script_state, DOMArrayBufferView* chunk, ExceptionState&) const override
    {
        scoped_refptr<scheduler::EventLoop> event_loop = ExecutionContext::From(script_state)->GetAgent()->event_loop();
        event_loop->EnqueueMicrotask(
            WTF::BindOnce(&ByteTeeReadIntoRequest::ChunkStepsBody, WrapPersistent(this), WrapPersistent(script_state), WrapPersistent(chunk)));
    }

    void CloseSteps(ScriptState* script_state, DOMArrayBufferView* chunk) const override
    {
        // 1. Set reading to false.
        engine_->reading_ = false;
        // 2. Let byobCanceled be canceled2 if forBranch2 is true, and canceled1
        //    otherwise.
        auto byob_canceled = for_branch_2_ ? engine_->canceled_[1] : engine_->canceled_[0];
        // 3. Let otherCanceled be canceled2 if forBranch2 is false, and canceled1
        //    otherwise.
        auto other_canceled = !for_branch_2_ ? engine_->canceled_[1] : engine_->canceled_[0];
        // 4. If byobCanceled is false, perform !
        //    ReadableByteStreamControllerClose(byobBranch.[[controller]]).
        if (!byob_canceled) {
            ReadableStreamController* controller = byob_branch_->readable_stream_controller_;
            ReadableByteStreamController* byte_controller = To<ReadableByteStreamController>(controller);
        ...
        if (!other_canceled) {
            ReadableStreamController* controller = other_branch_->readable_stream_controller_;
            ReadableByteStreamController* byte_controller = To<ReadableByteStreamController>(controller);
            byte_controller->Close(script_state, byte_controller);
        }
        ...
        if (chunk) {
            ...
            if (!byob_canceled) {
                ...
                ReadableByteStreamController::RespondWithNewView(
                    script_state, To<ReadableByteStreamController>(controller), NotShared<DOMArrayBufferView>(chunk), exception_state);
            ...
            //      0).
            ReadableStreamController* controller = other_branch_->readable_stream_controller_;
            if (!other_canceled && !To<ReadableByteStreamController>(controller)->pending_pull_intos_.empty()) {
                ReadableByteStreamController::Respond(script_state, To<ReadableByteStreamController>(controller), 0, exception_state);
                DCHECK(!exception_state.HadException());
            }
        }
...
private:
    void ChunkStepsBody(ScriptState* script_state, DOMArrayBufferView* chunk) const
    {
        ...
        if (!other_canceled) {
            ...
            NotShared<DOMArrayBufferView> cloned_chunk = NotShared<DOMArrayBufferView>(clone_result);

            //   d. If byobCanceled is false, perform !
            //   ReadableByteStreamControllerRespondWithNewView(byobBranch.[[controller]],
            //   chunk).
            if (!byob_canceled) {
                ReadableStreamController* byob_controller = byob_branch_->readable_stream_controller_;
                ReadableByteStreamController::RespondWithNewView(
                    script_state, To<ReadableByteStreamController>(byob_controller), NotShared<DOMArrayBufferView>(chunk), exception_state);
                DCHECK(!exception_state.HadException());
            }
            //   e. Perform !
            //   ReadableByteStreamControllerEnqueue(otherBranch.[[controller]],
            //   clonedChunk).
            ReadableStreamController* other_controller = other_branch_->readable_stream_controller_;
            ReadableByteStreamController::Enqueue(script_state, To<ReadableByteStreamController>(other_controller), cloned_chunk, exception_state);
            DCHECK(!exception_state.HadException());
...
void ByteStreamTeeEngine::ForwardReaderError(ScriptState* script_state, ReadableStreamGenericReader* this_reader)
{
    ...
    public:
        explicit RejectFunction(ByteStreamTeeEngine* engine, ReadableStreamGenericReader* reader)
            : engine_(engine)
            , reader_(reader)
        {
        }
        ...
        void React(ScriptState* script_state, ScriptValue r)
        {
            ...
            ReadableByteStreamController::Error(script_state, engine_->controller_[0], r.V8Value());
            //     iii. Perform !
            //     ReadableByteStreamControllerError(branch2.[[controller]], r).
            ReadableByteStreamController::Error(script_state, engine_->controller_[1], r.V8Value());
    ...
    this_reader->closed(script_state).Catch(script_state, MakeGarbageCollected<RejectFunction>(this, this_reader));
}

void ByteStreamTeeEngine::PullWithDefaultReader(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    if (reader_->IsBYOBReader()) {
        ...
        reader_ = ReadableStream::AcquireDefaultReader(script_state, stream_, exception_state);
        DCHECK(!exception_state.HadException());
    ...
    ReadableStreamGenericReader* reader = reader_;
    ReadableStreamDefaultReader::Read(script_state, To<ReadableStreamDefaultReader>(reader), read_request, exception_state);
}

void ByteStreamTeeEngine::PullWithBYOBReader(ScriptState* script_state, NotShared<DOMArrayBufferView> view, bool for_branch_2, ExceptionState& exception_state)
{
    ...
    if (reader_->IsDefaultReader()) {
        ...
        reader_ = ReadableStream::AcquireBYOBReader(script_state, stream_, exception_state);
        DCHECK(!exception_state.HadException());
        //     iv. Perform forwardReaderError, given reader.
        ForwardReaderError(script_state, reader_);
    }
    ...
    auto* read_into_request = MakeGarbageCollected<ByteTeeReadIntoRequest>(this, byob_branch, other_branch, for_branch_2);
...
DOMUint8Array* ByteStreamTeeEngine::CloneAsUint8Array(DOMArrayBufferView* chunk)
{
    auto* cloned_buffer = DOMArrayBuffer::Create(chunk->ByteSpan());
    return DOMUint8Array::Create(cloned_buffer, 0, chunk->byteLength());
}

void ByteStreamTeeEngine::Start(ScriptState* script_state, ReadableStream* stream, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#abstract-opdef-readablebytestreamtee
    ...
    reader_ = ReadableStream::AcquireDefaultReader(script_state, stream, exception_state);
    ...
    cancel_promise_ = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state);
    ...
    branch_[0] = ReadableStream::CreateByteStream(script_state, start_algorithm, pull1_algorithm, cancel1_algorithm, exception_state);
    ...
    branch_[1] = ReadableStream::CreateByteStream(script_state, start_algorithm, pull2_algorithm, cancel2_algorithm, exception_state);
    if (exception_state.HadException()) {
        return;
    }

    for (int branch = 0; branch < 2; ++branch) {
        ReadableStreamController* controller = branch_[branch]->readable_stream_controller_;
