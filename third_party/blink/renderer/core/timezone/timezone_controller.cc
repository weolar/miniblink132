...
BASE_FEATURE(kLazyBlinkTimezoneInit, "LazyBlinkTimezoneInit", base::FEATURE_DISABLED_BY_DEFAULT);
...
void NotifyTimezoneChangeOnWorkerThread(WorkerThread* worker_thread)
{
    ...
    if (RuntimeEnabledFeatures::TimeZoneChangeEventEnabled() && worker_thread->GlobalScope()->IsWorkerGlobalScope()) {
        worker_thread->GlobalScope()->DispatchEvent(*Event::Create(event_type_names::kTimezonechange));
    }
}
    ...
    for (const Page* page : Page::OrdinaryPages()) {
        for (Frame* frame = page->MainFrame(); frame; frame = frame->Tree().TraverseNext()) {
            if (auto* main_local_frame = DynamicTo<LocalFrame>(frame)) {
                main_local_frame->DomWindow()->EnqueueWindowEvent(*Event::Create(event_type_names::kTimezonechange), TaskType::kMiscPlatformAPI);
            }
        }
    }
}
...
bool SetIcuTimeZoneAndNotifyV8(const String& timezone_id)
{
    ...
    std::unique_ptr<icu::TimeZone> timezone(icu::TimeZone::createTimeZone(icu::UnicodeString(timezone_id.Ascii().data(), -1, US_INV)));
    CHECK(timezone);
    ...
    Thread::MainThread()->Scheduler()->ToMainThreadScheduler()->ForEachMainThreadIsolate(
        WTF::BindRepeating([](v8::Isolate* isolate) { NotifyTimezoneChangeToV8(isolate); }));
    WorkerThread::CallOnAllWorkerThreads(&NotifyTimezoneChangeOnWorkerThread, TaskType::kInternalDefault);
...
void TimeZoneController::Init()
{
    ...
    Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(monitor.BindNewPipeAndPassReceiver());
...
bool CanonicalEquals(const String& time_zone_a, const String& time_zone_b)
{
    ...
    UBool dummy;
    icu::TimeZone::getCanonicalID(icu::UnicodeString(time_zone_a.Ascii().data(), -1, US_INV), canonical_a, dummy, status);
    icu::TimeZone::getCanonicalID(icu::UnicodeString(time_zone_b.Ascii().data(), -1, US_INV), canonical_b, dummy, status);
    if (U_FAILURE(status)) {
        return false;
    }
...
// static
std::unique_ptr<TimeZoneController::TimeZoneOverride> TimeZoneController::SetTimeZoneOverride(const String& timezone_id)
{
    DCHECK(!timezone_id.empty());
...
void TimeZoneController::ClearTimeZoneOverride()
{
    ...
    if (!CanonicalEquals(instance().GetHostTimezoneId(), instance().override_timezone_id_)) {
