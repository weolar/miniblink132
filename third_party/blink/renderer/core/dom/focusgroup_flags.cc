...
FocusgroupFlags ParseFocusgroup(const Element* element, const AtomicString& input)
{
    DCHECK(element);
    ...
    if (!invalid_tokens.empty()) {
        element->GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther,
            mojom::blink::ConsoleMessageLevel::kError, "Unrecognized focusgroup attribute values: " + invalid_tokens.ReleaseString()));
    }
    ...
    if (has_grid) {
        ...
        if (has_flow) {
            if (flags & FocusgroupFlags::kWrapInline || flags & FocusgroupFlags::kWrapBlock) {
                element->GetDocument().AddConsoleMessage(
                    MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kError,
            ...
            if (flags & FocusgroupFlags::kRowFlow && flags & FocusgroupFlags::kColFlow) {
                element->GetDocument().AddConsoleMessage(
                    MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kWarning,
    ...
    if (has_wrap) {
        if (flags & FocusgroupFlags::kExtend) {
            bool extends_inline = flags & FocusgroupFlags::kInline && ancestor_flags & FocusgroupFlags::kInline;
            if (!extends_inline && flags & FocusgroupFlags::kInline) {
                flags |= FocusgroupFlags::kWrapInline;
            }
            bool extends_block = flags & FocusgroupFlags::kBlock && ancestor_flags & FocusgroupFlags::kBlock;
    ...
    if (flags & FocusgroupFlags::kExtend) {
        ...
        if ((flags & FocusgroupFlags::kWrapInline) == (ancestor_flags & FocusgroupFlags::kWrapInline)
            && (flags & FocusgroupFlags::kWrapBlock) == (ancestor_flags & FocusgroupFlags::kWrapBlock)) {
            element->GetDocument().AddConsoleMessage(
                MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kWarning,
