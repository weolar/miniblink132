...
{
    return node ? WeakIdentifierMap<Node, DOMNodeId>::ExistingIdentifier(node) : kInvalidDOMNodeId;
}
...
{
    return node ? WeakIdentifierMap<Node, DOMNodeId>::Identifier(node) : kInvalidDOMNodeId;
}

// static
Node* DOMNodeIds::NodeForId(DOMNodeId id)
{
    return id == kInvalidDOMNodeId ? nullptr : WeakIdentifierMap<Node, DOMNodeId>::Lookup(id);
}
