...
TEST_F(ElementTest, FocusableDesignMode)
{
    ...
    EXPECT_TRUE(document.documentElement()->IsFocusable()) << "<html> with designMode=on should be focusable.";
}
...
TEST_F(ElementTest, GetBoundingClientRectCorrectForStickyElementsAfterInsertion)
{
    ...
    EXPECT_EQ(DocumentLifecycle::kVisualUpdatePending, document.Lifecycle().GetState());
...
TEST_F(ElementTest, StickySubtreesAreTrackedCorrectly)
{
    ...
    LayoutObject* ancestor = document.getElementById(AtomicString("ancestor"))->GetLayoutObject();
    LayoutObject* outer_sticky = document.getElementById(AtomicString("outerSticky"))->GetLayoutObject();
    LayoutObject* child = document.getElementById(AtomicString("child"))->GetLayoutObject();
    LayoutObject* grandchild = document.getElementById(AtomicString("grandchild"))->GetLayoutObject();
    LayoutObject* inner_sticky = document.getElementById(AtomicString("innerSticky"))->GetLayoutObject();
    LayoutObject* great_grandchild = document.getElementById(AtomicString("greatGrandchild"))->GetLayoutObject();
    ...
    document.getElementById(AtomicString("child"))->SetInlineStyleProperty(CSSPropertyID::kWebkitRubyPosition, CSSValueID::kAfter);
    UpdateAllLifecyclePhasesForTest();
    ...
    EXPECT_EQ(RubyPosition::kUnder, great_grandchild->StyleRef().GetRubyPosition());
    ...
    document.getElementById(AtomicString("outerSticky"))->SetInlineStyleProperty(CSSPropertyID::kPosition, CSSValueID::kStatic);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(DocumentLifecycle::kPaintClean, document.Lifecycle().GetState());
...
TEST_F(ElementTest, GetBoundingClientRectForSVG)
{
    ...
    Element* stroke_transformed = document.getElementById(AtomicString("stroke_transformed"));
    DOMRect* stroke_transformedbounding_client_rect = stroke_transformed->GetBoundingClientRect();
    ...
    Element* foreign_transformed = document.getElementById(AtomicString("foreign_transformed"));
    DOMRect* foreign_transformed_bounding_client_rect = foreign_transformed->GetBoundingClientRect();
    ...
    DOMRect* svg_stroke_bounding_client_rect = svg_stroke->GetBoundingClientRect();
...
TEST_F(ElementTest, PartAttribute)
{
    ...
    Element* has_two_parts = document.getElementById(AtomicString("has_two_parts"));
...
TEST_F(ElementTest, ExportpartsAttribute)
{
    ...
    Element* has_one_mapping = document.getElementById(AtomicString("has_one_mapping"));
    Element* has_two_mappings = document.getElementById(AtomicString("has_two_mappings"));
    Element* has_no_mapping = document.getElementById(AtomicString("has_no_mapping"));
...

    {
        ...
        ASSERT_EQ("partname2", part_names_map->Get(AtomicString("partname1"))->SerializeToString());
    }

    {
        ...
        ASSERT_EQ("partname2", part_names_map->Get(AtomicString("partname1"))->SerializeToString());
        ASSERT_EQ("partname4", part_names_map->Get(AtomicString("partname3"))->SerializeToString());
    }

    {
        ...
        // Now update the attribute value and make sure it's reflected.
        has_no_mapping->setAttribute(AtomicString("exportparts"), AtomicString("partname1: partname2"));
...
TEST_F(ElementTest, OptionElementDisplayNoneComputedStyle)
{
    ...
    EXPECT_FALSE(document.getElementById(AtomicString("group"))->GetComputedStyle());
    EXPECT_FALSE(document.getElementById(AtomicString("option"))->GetComputedStyle());
    EXPECT_FALSE(document.getElementById(AtomicString("inner-group"))->GetComputedStyle());
    EXPECT_FALSE(document.getElementById(AtomicString("inner-option"))->GetComputedStyle());
}

// A fake plugin which will assert that script is allowed in Destroy.
class ScriptOnDestroyPlugin : public GarbageCollected<ScriptOnDestroyPlugin>, public WebPlugin {
public:
    ...
    WebPluginContainer* Container() const override
    {
        return container_;
    }
    ...
    void UpdateGeometry(const gfx::Rect&, const gfx::Rect&, const gfx::Rect&, bool) override
    {
    }
    ...
    WebInputEventResult HandleInputEvent(const WebCoalescedInputEvent&, ui::Cursor*) override
    {
        return {};
    }
...
TEST_F(ElementTest, CreateAndAttachShadowRootSuspendsPluginDisposal)
{
    ...
    auto* plugin_element = DynamicTo<HTMLPlugInElement>(document.getElementById(AtomicString("plugin")));
    ASSERT_TRUE(plugin_element);

    auto* plugin = MakeGarbageCollected<ScriptOnDestroyPlugin>();
    auto* plugin_container = MakeGarbageCollected<WebPluginContainerImpl>(*plugin_element, plugin);
...
TEST_F(ElementTest, ParseFocusgroupAttrExtendCorrectly)
{
    ...
    auto* fg4 = fg4_container->GetShadowRoot()->getElementById(AtomicString("fg4"));
    ASSERT_TRUE(fg4);
...
TEST_F(ElementTest, ParseFocusgroupAttrGrid)
{
    ...
    ASSERT_EQ(e3_flags, (FocusgroupFlags::kGrid | FocusgroupFlags::kWrapInline | FocusgroupFlags::kWrapBlock));
    ...
    ASSERT_EQ(e6_flags, (FocusgroupFlags::kGrid | FocusgroupFlags::kWrapInline | FocusgroupFlags::kWrapBlock));
    ASSERT_EQ(e7_flags, (FocusgroupFlags::kGrid | FocusgroupFlags::kRowFlow | FocusgroupFlags::kColFlow));
    ...
    ASSERT_EQ(e10_flags, (FocusgroupFlags::kGrid | FocusgroupFlags::kRowFlow | FocusgroupFlags::kColFlow));
    ASSERT_EQ(e11_flags, (FocusgroupFlags::kGrid | FocusgroupFlags::kWrapInline));
    ASSERT_EQ(e12_flags, (FocusgroupFlags::kGrid | FocusgroupFlags::kWrapInline | FocusgroupFlags::kColFlow));
    ASSERT_EQ(e13_flags, (FocusgroupFlags::kGrid | FocusgroupFlags::kWrapBlock));
    ASSERT_EQ(e14_flags, (FocusgroupFlags::kGrid | FocusgroupFlags::kWrapBlock | FocusgroupFlags::kRowFlow));
...
TEST_F(ElementTest, MixStyleAttributeAndCSSOMChanges)
{
    ...
    elmt->style()->setProperty(GetDocument().GetExecutionContext(), "color", "red", String(), ASSERT_NO_EXCEPTION);
...
TEST_F(ElementTest, GetPseudoElement)
{
    ...
    struct {
        ...
        { "marker1", false, false, true },
        { "marker2", false, false, true },
        { "marker3", false, false, true },
        { "marker4", false, false, true },
    };
...
TEST_F(ElementTest, ColumnPseudoElements)
{
    ...
    PseudoElement* first_column_pseudo_element = element->CreateColumnPseudoElementIfNeeded(dummy_column_rect);
    ASSERT_TRUE(first_column_pseudo_element);
    EXPECT_EQ(first_column_pseudo_element->GetComputedStyle()->Opacity(), 0.5f);
    ASSERT_TRUE(first_column_pseudo_element->GetPseudoElement(kPseudoIdScrollMarker));
    EXPECT_EQ(first_column_pseudo_element->GetPseudoElement(kPseudoIdScrollMarker)->GetComputedStyle()->Opacity(), 0.3f);

    PseudoElement* second_column_pseudo_element = element->CreateColumnPseudoElementIfNeeded(dummy_column_rect);
    ASSERT_TRUE(second_column_pseudo_element);
    EXPECT_EQ(second_column_pseudo_element->GetComputedStyle()->Opacity(), 0.5f);
    ASSERT_TRUE(second_column_pseudo_element->GetPseudoElement(kPseudoIdScrollMarker));
    EXPECT_EQ(second_column_pseudo_element->GetPseudoElement(kPseudoIdScrollMarker)->GetComputedStyle()->Opacity(), 0.3f);

    PseudoElement* third_column_pseudo_element = element->CreateColumnPseudoElementIfNeeded(dummy_column_rect);
    ASSERT_TRUE(third_column_pseudo_element);
    EXPECT_EQ(third_column_pseudo_element->GetComputedStyle()->Opacity(), 0.5f);
    ASSERT_TRUE(third_column_pseudo_element->GetPseudoElement(kPseudoIdScrollMarker));
    EXPECT_EQ(third_column_pseudo_element->GetPseudoElement(kPseudoIdScrollMarker)->GetComputedStyle()->Opacity(), 0.3f);
