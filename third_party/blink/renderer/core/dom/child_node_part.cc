...
// static
ChildNodePart* ChildNodePart::Create(
    PartRootUnion* root_union, Node* previous_sibling, Node* next_sibling, const PartInit* init, ExceptionState& exception_state)
{
    if (!IsAcceptableNodeType(*previous_sibling) || !IsAcceptableNodeType(*next_sibling)) {
    ...
    return MakeGarbageCollected<ChildNodePart>(*GetPartRootFromUnion(root_union), *previous_sibling, *next_sibling, init);
}

ChildNodePart::ChildNodePart(PartRoot& root, Node& previous_sibling, Node& next_sibling, Vector<String> metadata)
...
PartRootUnion* ChildNodePart::clone(ExceptionState& exception_state)
{
    ...
    NodeCloningData data { RuntimeEnabledFeatures::DOMPartsAPIMinimalEnabled() ? CloneOption::kPreserveDOMPartsMinimalAPI : CloneOption::kPreserveDOMParts };
    auto& fragment_part_root = fragment->getPartRoot();
    data.PushPartRoot(fragment_part_root);
    ContainerNode* new_parent = To<ContainerNode>(parentNode()->Clone(document, data, fragment, exception_state));
...
void ChildNodePart::replaceChildren(const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes, ExceptionState& exception_state)
{
    if (!IsValid()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError,
            "This ChildNodePart is not in a valid state. It must have "
    ...
    if (RuntimeEnabledFeatures::SkipTemporaryDocumentFragmentEnabled()) {
        // Insert new contents.
        VectorOf<Node> node_vector = Node::ConvertNodeUnionsIntoNodes(parent, nodes, parent->GetDocument(), "replaceChildren", exception_state);
        if (exception_state.HadException()) {
            return;
        }
        parent->InsertBefore(node_vector, next_sibling_, exception_state);
    } else {
        Node* nodes_as_node = Node::ConvertNodeUnionsIntoNode(parent, nodes, parent->GetDocument(), "replaceChildren", exception_state);
        if (exception_state.HadException()) {
            return;
        }
...
{
    DCHECK(IsValid());
    ChildNodePart* clone = MakeGarbageCollected<ChildNodePart>(data.CurrentPartRoot(), node_clone, node_clone, metadata().AsVector());
