...
TEST_F(ContainerNodeTest, CannotFindTextInElementWithoutDescendants)
{
    ...
    String text = GetDocument().FindTextInElementWith(AtomicString("anything"), [](const String&) { return true; });
...
TEST_F(ContainerNodeTest, CannotFindTextInElementWithNonTextDescendants)
{
    ...
    String text = GetDocument().FindTextInElementWith(AtomicString("Hello"), [](const String&) { return true; });
...
TEST_F(ContainerNodeTest, CanFindTextNodesWithBreakInBetween)
{
    ...
    EXPECT_EQ(nodes->item(0), GetDocument().getElementById(AtomicString("id"))->firstChild());
    EXPECT_EQ(nodes->item(1), GetDocument().getElementById(AtomicString("id"))->lastChild());
}
...
TEST_F(ContainerNodeTest, CannotFindTextInElementWithoutMatchingSubtring)
{
    ...
    String text = GetDocument().FindTextInElementWith(AtomicString("Goodbye"), [](const String&) { return true; });
...
{
    SetBodyContent(R"HTML(<body><span id="id"> Find me please </span></body>)HTML");

    String text = GetDocument().FindTextInElementWith(AtomicString("me"), [](const String&) { return true; });
...
TEST_F(ContainerNodeTest, CannotFindTextIfTheValidatorRejectsIt)
{
    ...
    String text = GetDocument().FindTextInElementWith(AtomicString("me"), [](const String&) { return false; });
...
{
    SetBodyContent(R"HTML(<body><span id="id"> Don't find me please </span></body>)HTML");

    StaticNodeList* nodes = GetDocument().FindAllTextNodesMatchingRegex("not present");
...
TEST_F(ContainerNodeTest, CanFindAllTextNodes)
{
    ...
    EXPECT_EQ(To<Text>(nodes->item(0))->data(), String("\n            Text number 1\n          "));
    EXPECT_EQ(To<Text>(nodes->item(1))->data(), String("\n            Text number 2\n            "));
    EXPECT_EQ(To<Text>(nodes->item(2))->data(), String(" Text number 3"));
    EXPECT_EQ(To<Text>(nodes->item(3))->data(), String("\n            Text number 4\n          "));
    EXPECT_EQ(To<Text>(nodes->item(4))->data(), String("\n            Text number 5\n          "));
    EXPECT_EQ(To<Text>(nodes->item(5))->data(), String("\n          Text number 6\n        "));
}
...
TEST_F(ContainerNodeTest, CanFindOnlyTextNodesThatMatch)
{
    ...
    StaticNodeList* nodes = GetDocument().FindAllTextNodesMatchingRegex("(\\s|\n)*(Text number 3|Text number 4)(\\s|\n)*");
    ASSERT_EQ(nodes->length(), 2U);
    EXPECT_EQ(nodes->item(0), GetDocument().getElementById(AtomicString("id_1"))->firstChild());
    EXPECT_EQ(nodes->item(1), GetDocument().getElementById(AtomicString("id_2"))->firstChild());
}
...
TEST_F(ContainerNodeTest, FindTextInElementWithFirstMatch)
{
    ...
    String text = GetDocument().FindTextInElementWith(AtomicString(" match "), [](const String&) { return true; });
...
TEST_F(ContainerNodeTest, FindTextInElementWithValidatorApprovingTheSecond)
{
    ...
    String text
        = GetDocument().FindTextInElementWith(AtomicString(" match "), [](const String& potential_match) { return potential_match == " Text match #2 "; });
...
TEST_F(ContainerNodeTest, FindTextInElementWithSubstringIgnoresComments)
{
    ...
    String text = GetDocument().FindTextInElementWith(AtomicString("comment"), [](const String&) { return true; });
...
TEST_F(ContainerNodeTest, FindTextInElementWithSubstringIgnoresAsciiCase)
{
    ...
    String text = GetDocument().FindTextInElementWith(AtomicString("magic"), [](const String&) { return true; });
...
TEST_F(ContainerNodeTest, FindTextInTheValueOfTheReadonlyInputFirst)
{
    ...
    String text = GetDocument().FindTextInElementWith(AtomicString("lookup"), [](const String&) { return true; });
