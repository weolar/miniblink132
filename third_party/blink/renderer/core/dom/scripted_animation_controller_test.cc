...
protected:
    ...
    ScriptedAnimationController& Controller()
    {
        return *controller_;
    }
...
void ScriptedAnimationControllerTest::SetUp()
{
    ...
    // instance.
    controller_ = WrapPersistent(MakeGarbageCollected<ScriptedAnimationController>(dummy_page_holder_->GetFrame().DomWindow()));
}
...
public:
    ...
    {
        return WTF::BindRepeating(&TaskOrderObserver::RunTask, WTF::Unretained(this), id);
    }
...
TEST_F(ScriptedAnimationControllerTest, EnqueueOneTask)
{
    ...
    PageAnimator::ServiceScriptedAnimations(base::TimeTicks(), { { Controller(), false } });
...
void EnqueueTask(ScriptedAnimationController* controller, TaskOrderObserver* observer, int id)
{
...
TEST_F(ScriptedAnimationControllerTest, EnqueueWithinTask)
{
    ...
    Controller().EnqueueTask(observer.CreateTask(1));
    Controller().EnqueueTask(WTF::BindOnce(&EnqueueTask, WrapPersistent(&Controller()), WTF::Unretained(&observer), 2));
...
TEST_F(ScriptedAnimationControllerTest, EnqueueTaskAndEvent)
{
    ...
    Controller().EnqueueTask(observer.CreateTask(1));
    GetDocument().addEventListener(AtomicString("test"), MakeGarbageCollected<RunTaskEventListener>(observer.CreateTask(2)));
...
public:
    RunTaskCallback(base::RepeatingClosure task)
        : task_(std::move(task))
    {
    }
...
TEST_F(ScriptedAnimationControllerTest, RegisterCallbackAndEnqueueTask)
{
    ...
    Controller().RegisterFrameCallback(MakeGarbageCollected<RunTaskCallback>(observer.CreateTask(1)));
    Controller().EnqueueTask(observer.CreateTask(2));
    EXPECT_EQ(0u, observer.Order().size());
...
TEST_F(ScriptedAnimationControllerTest, TestHasCallback)
{
    ...
    Controller().RegisterFrameCallback(MakeGarbageCollected<RunTaskCallback>(observer.CreateTask(1)));
    Controller().RegisterFrameCallback(MakeGarbageCollected<RunTaskCallback>(observer.CreateTask(2)));
    EXPECT_TRUE(Controller().HasFrameCallback());
...
TEST_F(ScriptedAnimationControllerTest, TestIsInRequestAnimationFrame)
{
    ...
    Controller().RegisterFrameCallback(MakeGarbageCollected<RunTaskCallback>(WTF::BindRepeating(
        [](ScriptedAnimationController* controller, bool* ran_callback) {
            EXPECT_TRUE(controller->GetExecutionContext()->IsInRequestAnimationFrame());
            *ran_callback = true;
        },
