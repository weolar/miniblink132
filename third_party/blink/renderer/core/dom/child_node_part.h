...
public:
    static ChildNodePart* Create(PartRootUnion* root_union, Node* previous_sibling, Node* next_sibling, const PartInit* init, ExceptionState& exception_state);
    ChildNodePart(PartRoot& root, Node& previous_sibling, Node& next_sibling, const PartInit* init)
        : ChildNodePart(root, previous_sibling, next_sibling, init && init->hasMetadata() ? init->metadata() : Vector<String>())
    {
    }
    ChildNodePart(PartRoot& root, Node& previous_sibling, Node& next_sibling, Vector<String> metadata);
    ...
    bool IsDocumentPartRoot() const override
    {
        return false;
    }
...
inline bool ChildNodePart::IsValid() const
{
    ...
    DCHECK(!RuntimeEnabledFeatures::DOMPartsAPIMinimalEnabled() || (previous_sibling_->HasNodePart() && next_sibling_->HasNodePart()));
