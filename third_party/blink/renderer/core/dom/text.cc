...
Node* Text::MergeNextSiblingNodesIfPossible()
{
    ...
    while (Node* next_sibling = nextSibling()) {
        ...
        UpdateTextLayoutObject(TextDiffRange::Insert(old_text_data.length(), next_text_data.length()));
        ...
        next_text->UpdateTextLayoutObject(TextDiffRange::Delete(0, next_text_data.length()));

        // Restore nextText for mutation event.
        next_text->SetDataWithoutUpdate(next_text_data);
        next_text->UpdateTextLayoutObject(TextDiffRange::Insert(0, next_text_data.length()));
...
Text* Text::splitText(unsigned offset, ExceptionState& exception_state)
{
    ...
    if (offset > length()) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kIndexSizeError, "The offset " + String::Number(offset) + " is larger than the Text node's length.");
        return nullptr;
    }

    EventQueueScope scope;
    String old_str = data();
    Text* new_text = To<Text>(CloneWithData(GetDocument(), old_str.Substring(offset)));
    ...
    if (LayoutText* layout_text = GetLayoutObject()) {
        if (RuntimeEnabledFeatures::TextDiffSplitFixEnabled()) {
            ...
            if (ContainsOnlyWhitespaceOrEmpty()) {
                ...
                layout_text->SetTextWithOffset(data(), TextDiffRange::Delete(offset, old_str.length() - offset));
            }
        } else {
            layout_text->SetTextWithOffset(data(), TextDiffRange::Delete(0, old_str.length()));
            if (ContainsOnlyWhitespaceOrEmpty()) {
                // To avoid |LayoutText| has empty text, we rebuild layout tree.
                SetForceReattachLayoutTree();
            }
    ...
    // does not exist yet.
    DCHECK(DOMDataStore::GetWrapper(GetDocument().GetAgent().isolate(), new_text).IsEmpty());
...
String Text::wholeText() const
{
    ...
    for (const Node* n = start_text; n != one_past_end_text; n = n->nextSibling()) {
        auto* text_node = DynamicTo<Text>(n);
        if (!text_node)
            continue;
        const String& data = text_node->data();
        CHECK_GE(std::numeric_limits<unsigned>::max() - data.length(), result_length);
...
Text* Text::ReplaceWholeText(const String& new_text)
{
    ...
    for (Node* n = start_text; n && n != this && n->IsTextNode() && n->parentNode() == parent;) {
    ...
    if (this != end_text) {
        Node* one_past_end_text = end_text->nextSibling();
        for (Node* n = nextSibling(); n && n != one_past_end_text && n->IsTextNode() && n->parentNode() == parent;) {
            Node* node_to_remove = n;
            n = node_to_remove->nextSibling();
            parent->RemoveChild(node_to_remove, IGNORE_EXCEPTION_FOR_TESTING);
        }
...
static inline bool CanHaveWhitespaceChildren(const ComputedStyle& style, const Text::AttachContext& context)
{
    const LayoutObject& parent = *context.parent;
    if (parent.IsTable() || parent.IsTableRow() || parent.IsTableSection() || parent.IsLayoutTableCol() || parent.IsFrameSet() || parent.IsFlexibleBox()
        || parent.IsLayoutGrid() || parent.IsSVGRoot() || parent.IsSVGContainer() || parent.IsSVGImage() || parent.IsSVGShape()) {
        if (!context.use_previous_in_flow || !context.previous_in_flow || !context.previous_in_flow->IsText())
            return false;

        return style.ShouldPreserveBreaks() || !EndsWithWhitespace(To<LayoutText>(context.previous_in_flow)->TransformedText());
    }
    return true;
}

bool Text::TextLayoutObjectIsNeeded(const AttachContext& context, const ComputedStyle& style) const
{
    const LayoutObject& parent = *context.parent;
    ...
    if (style.ShouldPreserveWhiteSpaces() && style.ShouldWrapLine() && parent.IsSVG()) {
        return false;
    }
    ...
    if (context.previous_in_flow->IsText()) {
        return !EndsWithWhitespace(To<LayoutText>(context.previous_in_flow)->TransformedText());
    }

    return context.previous_in_flow->IsInline() && !context.previous_in_flow->IsBR();
}
...
static bool IsSVGText(Text* text)
{
    ...
    return parent_or_shadow_host_node->IsSVGElement() && !IsA<SVGForeignObjectElement>(*parent_or_shadow_host_node);
}
...
{
    if (context.parent) {
        if (Element* style_parent = LayoutTreeBuilderTraversal::ParentElement(*this)) {
            const ComputedStyle* const style = IsA<HTMLHtmlElement>(style_parent) && style_parent->GetLayoutObject() ? style_parent->GetLayoutObject()->Style()
...
void Text::ReattachLayoutTreeIfNeeded(AttachContext& context)
{
    ...
    if (layout_object_is_needed) {
        ...
        LayoutTreeBuilderForText(*this, context, style_parent->GetComputedStyle()).CreateLayoutObject();
    } else {
...
{
    const ComputedStyle* new_style = GetDocument().GetStyleResolver().StyleForText(this);
    if (LayoutText* layout_text = GetLayoutObject()) {
        const ComputedStyle* layout_parent_style = GetLayoutObject()->Parent()->Style();
        if (!new_style || GetForceReattachLayoutTree() || (new_style != layout_parent_style && !new_style->InheritedEqual(*layout_parent_style))) {
    ...
    } else if (new_style && (NeedsStyleRecalc() || change.ReattachLayoutTree() || GetForceReattachLayoutTree() || NeedsWhitespaceLayoutObject(*new_style))) {
        SetNeedsReattachLayoutTree();
    }
...
static bool ShouldUpdateLayoutByReattaching(const Text& text_node, LayoutText* text_layout_object)
{
    ...
    if (!text_node.TextLayoutObjectIsNeeded(context, *text_layout_object->Style())) {
        return true;
    }
    if (text_layout_object->IsTextFragment()) {
        ...
        const auto& text_fragment_layout_object = *To<LayoutTextFragment>(text_layout_object);
        return text_fragment_layout_object.GetFirstLetterPseudoElement() || !text_fragment_layout_object.IsRemainingTextLayoutObject();
    }
    ...
    FirstLetterPseudoElement::Punctuation punctuation1 = FirstLetterPseudoElement::Punctuation::kNotSeen;
    FirstLetterPseudoElement::Punctuation punctuation2 = FirstLetterPseudoElement::Punctuation::kNotSeen;
    bool preserve_breaks = ShouldPreserveBreaks(text_layout_object->StyleRef().GetWhiteSpaceCollapse());
    if (!FirstLetterPseudoElement::FirstLetterLength(text_layout_object->TransformedText(), preserve_breaks, punctuation1)
        && FirstLetterPseudoElement::FirstLetterLength(text_node.data(), preserve_breaks, punctuation2)) {
...
CharacterData* Text::CloneWithData(Document& factory, const String& data) const
{
