...
// given document.
class DocumentParserTiming final : public GarbageCollected<DocumentParserTiming>, public Supplement<Document> {
public:
    ...
    // load.
    void RecordParserBlockedOnScriptLoadDuration(base::TimeDelta duration, bool script_inserted_via_document_write);
    ...
    void RecordParserBlockedOnScriptExecutionDuration(base::TimeDelta duration, bool script_inserted_via_document_write);

    // The getters below return monotonically-increasing time, or zero if the
    // given parser event has not yet occurred.

    base::TimeTicks ParserStart() const
    {
        return parser_start_;
    }
    base::TimeTicks ParserStop() const
    {
        return parser_stop_;
    }
    ...
    // crbug/600711 for details.
    base::TimeDelta ParserBlockedOnScriptExecutionFromDocumentWriteDuration() const
    {
...
private:
    ...
    base::TimeDelta parser_blocked_on_script_execution_from_document_write_duration_;
