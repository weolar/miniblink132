...
template <typename IDLType> class FrozenArray;
...
using AttributeNamesView = bindings::TransformedView<AttributeCollection, AttributeToNameTransform>;

using ColumnPseudoElementsVector = HeapVector<Member<ColumnPseudoElement>>;

enum SpellcheckAttributeState { kSpellcheckAttributeTrue, kSpellcheckAttributeFalse, kSpellcheckAttributeDefault };
...
public:
    Element(const QualifiedName& tag_name, Document*, ConstructionType = kCreateElement);
    ...
    void SetAttributeWithoutValidation(const QualifiedName&, const AtomicString& value);

    // Set an attribute with Trusted Type validation. Passing g_null_atom
    // is the same as removing the attribute.
    void SetAttributeWithValidation(const QualifiedName&, const AtomicString& value, ExceptionState&);
    ...
    int GetIntegralAttribute(const QualifiedName& attribute_name, int default_value) const;
    unsigned int GetUnsignedIntegralAttribute(const QualifiedName& attribute_name) const;
    void SetIntegralAttribute(const QualifiedName& attribute_name, int value);
    void SetUnsignedIntegralAttribute(const QualifiedName& attribute_name, unsigned value, unsigned default_value = 0);
    double GetFloatingPointAttribute(const QualifiedName& attribute_name, double fallback_value = std::numeric_limits<double>::quiet_NaN()) const;
    void SetFloatingPointAttribute(const QualifiedName& attribute_name, double value);
    ...
    const Element* GetShadowReferenceTargetOrSelf(const QualifiedName& name) const;
    ...
    HeapLinkedHashSet<WeakMember<Element>>* GetExplicitlySetElementsForAttr(const QualifiedName& name) const;
    Element* GetElementAttribute(const QualifiedName& name) const;
    Element* GetElementAttributeResolvingReferenceTarget(const QualifiedName& name) const;
    void SetElementAttribute(const QualifiedName&, Element*);
    HeapVector<Member<Element>>* GetAttrAssociatedElements(const QualifiedName& name, bool resolve_reference_target) const;
    ...
    // instead.
    Element* getElementByIdIncludingDisconnected(const Element& treescope_element, const AtomicString& id) const;
    ...
    bool hasAttributeNS(const AtomicString& namespace_uri, const AtomicString& local_name) const;
    ...
    const AtomicString& getAttributeNS(const AtomicString& namespace_uri, const AtomicString& local_name) const;

    void setAttribute(AtomicString name, String value, ExceptionState& exception_state = ASSERT_NO_EXCEPTION)
    {
        WTF::AtomicStringTable::WeakResult weak_lowercase_name = WeakLowercaseIfNecessary(name);
        SetAttributeHinted(std::move(name), weak_lowercase_name, std::move(value), exception_state);
    }

    // Trusted Types variant for explicit setAttribute() use.
    void setAttribute(AtomicString name, const V8TrustedType* trusted_string, ExceptionState& exception_state)
    {
        WTF::AtomicStringTable::WeakResult weak_lowercase_name = WeakLowercaseIfNecessary(name);
        SetAttributeHinted(std::move(name), weak_lowercase_name, trusted_string, exception_state);
    }

    // Returns attributes that should be checked against Trusted Types
    virtual const AttrNameToTrustedType& GetCheckedAttributeTypes() const;

    static std::optional<QualifiedName> ParseAttributeName(const AtomicString& namespace_uri, const AtomicString& qualified_name, ExceptionState&);
    void setAttributeNS(const AtomicString& namespace_uri, const AtomicString& qualified_name, String value, ExceptionState& exception_state);
    void setAttributeNS(
    ...
    WTF::AtomicStringTable::WeakResult WeakLowercaseIfNecessary(const AtomicString&) const;
    ...
    Vector<gfx::Rect> OutlineRectsInWidget(DocumentUpdateReason reason = DocumentUpdateReason::kUnknown) const;
    ...
    void ariaNotify(const String& announcement, const AriaNotificationOptions* options);
    ...
    void removeAttributeNS(const AtomicString& namespace_uri, const AtomicString& local_name);

    Attr* DetachAttribute(wtf_size_t index);

    Attr* getAttributeNode(const AtomicString& name);
    Attr* getAttributeNodeNS(const AtomicString& namespace_uri, const AtomicString& local_name);
    ...
    const AtomicString& LocateNamespacePrefix(const AtomicString& namespace_uri) const;

    String nodeName() const override;

    Element& CloneWithChildren(NodeCloningData& data, Document*, ContainerNode*, ExceptionState& = ASSERT_NO_EXCEPTION) const;
    Element& CloneWithoutChildren(NodeCloningData& data, Document* = nullptr) const;
    ...
    void SetInlineStyleProperty(const CSSPropertyName&, const CSSValue&, bool important = false);
    ...
    virtual void CollectStyleForPresentationAttribute(const QualifiedName&, const AtomicString&, MutableCSSPropertyValueSet*)
    {
    }
    ...
    virtual void CollectExtraStyleForPresentationAttribute(MutableCSSPropertyValueSet*)
    {
    }
    ...
    enum class AttributeModificationReason { kDirectly, kByParser, kByCloning, kByMoveToNewDocument, kBySynchronizationOfLazyAttribute };
    struct AttributeModificationParams {
        STACK_ALLOCATED();

    public:
        AttributeModificationParams(
            const QualifiedName& qname, const AtomicString& old_value, const AtomicString& new_value, AttributeModificationReason reason)
            : name(qname)
            , old_value(old_value)
            , new_value(new_value)
            , reason(reason)
    ...
    virtual void CloneNonAttributePropertiesFrom(const Element&, NodeCloningData&)
    {
    }
    ...
    bool NeedsRebuildChildLayoutTrees(const WhitespaceAttacher& whitespace_attacher) const
    {
        return ChildNeedsReattachLayoutTree() || NeedsWhitespaceChildrenUpdate()
            || (whitespace_attacher.TraverseIntoDisplayContents() && HasDisplayContentsStyle());
    }
    bool NeedsRebuildLayoutTree(const WhitespaceAttacher& whitespace_attacher) const
    {
        return NeedsReattachLayoutTree() || NeedsRebuildChildLayoutTrees(whitespace_attacher) || NeedsLayoutSubtreeUpdate();
    }
    ...
    bool AttachDeclarativeShadowRoot(
        HTMLTemplateElement&, String, FocusDelegation, SlotAssignmentMode, bool serializable, bool clonable, const AtomicString& reference_target);

    ShadowRoot& CreateUserAgentShadowRoot(SlotAssignmentMode = SlotAssignmentMode::kNamed);
    ShadowRoot& AttachShadowRootInternal(
        ShadowRootMode, FocusDelegation, SlotAssignmentMode, CustomElementRegistry*, bool serializable, bool clonable, const AtomicString& reference_target);
    ...
    {
        return AttachShadowRootInternal(type, FocusDelegation::kNone, SlotAssignmentMode::kNamed,
            /*registry*/ nullptr,
            /*serializable*/ false,
            /*clonable*/ false,
    ...
    ShadowRoot& EnsureUserAgentShadowRoot(SlotAssignmentMode = SlotAssignmentMode::kNamed);
    ...
    // display none.
    const ComputedStyle* EnsureComputedStyle(PseudoId = kPseudoIdNone, const AtomicString& pseudo_argument = g_null_atom);
    ...
    bool IsDefined() const
    {
        ...
        return GetCustomElementState() == CustomElementState::kUncustomized || GetCustomElementState() == CustomElementState::kCustom;
    }
    ...
    virtual bool IsSVGAnimationAttributeSettingJavaScriptURL(const Attribute&) const
    {
        return false;
    }
    ...
    void SetHasFocusWithinUpToAncestor(bool, Element* ancestor, bool need_snap_container_search = false);
    ...
    virtual void UpdateSelectionOnFocus(SelectionBehaviorOnFocus, const FocusOptions*);
    virtual void blur();
    ...
    bool IsFocusable(UpdateBehavior update_behavior = UpdateBehavior::kStyleAndLayout) const;
    bool IsMouseFocusable(UpdateBehavior update_behavior = UpdateBehavior::kStyleAndLayout) const;
    virtual bool IsKeyboardFocusable(UpdateBehavior update_behavior = UpdateBehavior::kStyleAndLayout) const;
    ...
    virtual bool DispatchFocusEvent(Element* old_focused_element, mojom::blink::FocusType, InputDeviceCapabilities* source_capabilities = nullptr);
    virtual void DispatchBlurEvent(Element* new_focused_element, mojom::blink::FocusType, InputDeviceCapabilities* source_capabilities = nullptr);
    virtual void DispatchFocusInEvent(
        const AtomicString& event_type, Element* old_focused_element, mojom::blink::FocusType, InputDeviceCapabilities* source_capabilities = nullptr);
    void DispatchFocusOutEvent(const AtomicString& event_type, Element* new_focused_element, InputDeviceCapabilities* source_capabilities = nullptr);
    ...
    virtual bool IsValidBuiltinCommand(HTMLElement& invoker, CommandEventType command)
    {
        return false;
    }
    virtual bool HandleCommandInternal(HTMLElement& invoker, CommandEventType command)
    {
        CHECK(command != CommandEventType::kCustom && command != CommandEventType::kNone);
        return false;
    }

    void InterestGained();

    virtual Element* interestTargetElement()
    {
        return nullptr;
    }
    virtual AtomicString interestAction() const
    {
        return g_null_atom;
    }
    ...
    Element* insertAdjacentElement(const String& where, Element* new_child, ExceptionState&);
    ...
    PseudoElement* GetPseudoElement(PseudoId, const AtomicString& view_transition_name = g_null_atom) const;
    ...
    const ComputedStyle* CachedStyleForPseudoElement(PseudoId, const AtomicString& pseudo_argument = g_null_atom);
    ...
    const ComputedStyle* StyleForPseudoElement(const StyleRecalcContext&, const StyleRequest&);

    // These are used by ResolveStyle with Highlight Inheritance when caching
    // is not used.
    const ComputedStyle* StyleForHighlightPseudoElement(const StyleRecalcContext& style_recalc_context, const ComputedStyle* highlight_parent,
        const ComputedStyle& originating_style, const PseudoId pseudo_id, const AtomicString& pseudo_argument = g_null_atom);
    const ComputedStyle* StyleForSearchTextPseudoElement(const StyleRecalcContext& style_recalc_context, const ComputedStyle* highlight_parent,
    ...
    virtual bool IsVTTCueBox() const
    {
        return false;
    }
    ...
    virtual bool IsOutputElement() const
    {
        return false;
    }
    ...
    ScriptPromise<IDLUndefined> requestPointerLock(ScriptState* script_state, const PointerLockOptions* options, ExceptionState& exception_state);
    ...
    {
        SynchronizeAttributeHinted(local_name, WeakLowercaseIfNecessary(local_name));
    }
    ...
    void LogUpdateAttributeIfIsolatedWorldAndInDocument(const char element[], const AttributeModificationParams&);
    ...
    // elements.
    ColumnPseudoElement* CreateColumnPseudoElementIfNeeded(const PhysicalRect& column_rect);
    ...
    Element* GetStyledPseudoElement(PseudoId pseudo_id, const AtomicString& view_transition_name) const;

    void RecalcTransitionPseudoTreeStyle(const Vector<AtomicString>& view_transition_names);
    void RebuildTransitionPseudoLayoutTree(const Vector<AtomicString>& view_transition_names);
    ...
    virtual CascadeFilter GetCascadeFilter() const
    {
...
protected:
    ...
    {
        return static_cast<bool>(element_data_);
    }
    ...
    void RecordScrollbarSizeForStudy(int measurement, bool is_width, bool is_offset);

    void AddPropertyToPresentationAttributeStyle(MutableCSSPropertyValueSet*, CSSPropertyID, CSSValueID identifier);
    void AddPropertyToPresentationAttributeStyle(MutableCSSPropertyValueSet*, CSSPropertyID, double value, CSSPrimitiveValue::UnitType);
    void AddPropertyToPresentationAttributeStyle(MutableCSSPropertyValueSet*, CSSPropertyID, const String& value);
    void AddPropertyToPresentationAttributeStyle(MutableCSSPropertyValueSet*, CSSPropertyID, const CSSValue&);
    void MapLanguageAttributeToLocale(const AtomicString&, MutableCSSPropertyValueSet*);
    ...
    virtual const ComputedStyle* CustomStyleForLayoutObject(const StyleRecalcContext&);
    ...
    virtual bool IsFocusableStyle(UpdateBehavior update_behavior = UpdateBehavior::kStyleAndLayout) const;
    ...
    void UpdateClassList(const AtomicString& old_class_string, const AtomicString& new_class_string);
...
private:
    ...
    template <typename Functor> bool PseudoElementStylesDependOnFunc(Functor& func) const;
    ...
    void ClearPseudoElement(PseudoId, const AtomicString& view_transition_name = g_null_atom);
    ...
    const char* ErrorMessageForAttachShadow(String mode_string, bool for_declarative, ShadowRootMode& mode_out) const;

    void StyleAttributeChanged(const AtomicString& new_style_string, AttributeModificationReason);
    ...
    const ComputedStyle* EnsureOwnComputedStyle(const StyleRecalcContext&, PseudoId, const AtomicString& pseudo_argument = g_null_atom);
    ...
    bool ShouldRecalcHighlightPseudoStyle(HighlightRecalc highlight_recalc, const ComputedStyle* highlight_parent, const ComputedStyle& originating_style,
        const Element* originating_container) const;

    // Recalc those custom highlights that require it.
    void RecalcCustomHighlightPseudoStyle(const StyleRecalcContext&, HighlightRecalc, ComputedStyleBuilder&, const StyleHighlightData*, const ComputedStyle&);

    // Recalculate the ComputedStyle for this element and return a
    // StyleRecalcChange for propagation/traversal into child nodes.
    StyleRecalcChange RecalcOwnStyle(const StyleRecalcChange, const StyleRecalcContext&);
    ...
    bool SkipStyleRecalcForContainer(const ComputedStyle& style, const StyleRecalcChange& child_change, const StyleRecalcContext&);
    ...
    inline void CheckForEmptyStyleChange(const Node* node_before_change, const Node* node_after_change);

    void UpdateColumnPseudoElements(const StyleRecalcChange, const StyleRecalcContext&);
    PseudoElement* UpdatePseudoElement(PseudoId, const StyleRecalcChange, const StyleRecalcContext&, const AtomicString& view_transition_name = g_null_atom);
    ...
    void UpdateBackdropPseudoElement(const StyleRecalcChange, const StyleRecalcContext&);

    void UpdateFirstLetterPseudoElement(StyleUpdatePhase, const StyleRecalcContext&);
    ...
    inline PseudoElement* CreatePseudoElementIfNeeded(PseudoId, const StyleRecalcContext&, const AtomicString& view_transition_name = g_null_atom);
    ...
    ShadowRoot& CreateAndAttachShadowRoot(ShadowRootMode, SlotAssignmentMode = SlotAssignmentMode::kNamed);
    ...
    void WillModifyAttribute(const QualifiedName&, const AtomicString& old_value, const AtomicString& new_value);
    void DidModifyAttribute(const QualifiedName&, const AtomicString& old_value, const AtomicString& new_value, AttributeModificationReason reason);
    ...
    void UpdateId(TreeScope&, const AtomicString& old_id, const AtomicString& new_id);
    ...
    void SetAttributeInternal(wtf_size_t index, const QualifiedName&, const AtomicString& value, AttributeModificationReason);
    void AppendAttributeInternal(const QualifiedName&, const AtomicString& value, AttributeModificationReason);
    void RemoveAttributeInternal(wtf_size_t index, AttributeModificationReason);
    SpecificTrustedType ExpectedTrustedTypeForAttribute(const QualifiedName&) const;
    ...
    const AtomicString& GetAttributeHinted(const AtomicString& name, WTF::AtomicStringTable::WeakResult hint) const;
    void RemoveAttributeHinted(const AtomicString& name, WTF::AtomicStringTable::WeakResult hint);
    void SynchronizeAttributeHinted(const AtomicString& name, WTF::AtomicStringTable::WeakResult hint) const;
    void SetAttributeHinted(AtomicString name, WTF::AtomicStringTable::WeakResult hint, String value, ExceptionState& = ASSERT_NO_EXCEPTION);
    void SetAttributeHinted(AtomicString name, WTF::AtomicStringTable::WeakResult hint, const V8TrustedType* trusted_string, ExceptionState& exception_state);
    std::pair<wtf_size_t, const QualifiedName> LookupAttributeQNameHinted(AtomicString name, WTF::AtomicStringTable::WeakResult hint) const;
    wtf_size_t ValidateAttributeIndex(wtf_size_t index, const QualifiedName& qname) const;

    void CancelSelectionAfterLayout();
    virtual int DefaultTabIndex() const;

    inline void UpdateCallbackSelectors(const ComputedStyle* old_style, const ComputedStyle* new_style);
    ...
    Node* Clone(Document& factory, NodeCloningData& data, ContainerNode* append_to, ExceptionState& append_exception_state) const override;

    virtual Element& CloneWithoutAttributesAndChildren(Document& factory) const;

    void UpdateNamedItemRegistration(NamedItemType, const AtomicString& old_name, const AtomicString& new_name);
    void UpdateIdNamedItemRegistration(NamedItemType, const AtomicString& old_name, const AtomicString& new_name);

    void CreateUniqueElementData();

    bool ShouldInvalidateDistributionWhenAttributeChanged(ShadowRoot&, const QualifiedName&, const AtomicString&);

    void SetInnerHTMLInternal(const String&, ParseDeclarativeShadowRoots parse_declarative_shadows, ForceHtml force_html_over_xml, SanitizeHtml sanitize_html,
        SetHTMLOptions* set_html_options, ExceptionState&);
    ...
    void SynchronizeContentAttributeAndElementReference(const QualifiedName& name);

    DisplayLockContext* GetDisplayLockContextFromRareData() const;

    void PseudoStateChanged(CSSSelector::PseudoType pseudo, AffectedByPseudoStateChange&&);
    ...
    HighlightRecalc CalculateHighlightRecalc(const ComputedStyle* old_style, const ComputedStyle& new_style, const ComputedStyle* parent_style) const;
    ...
    // If UpdateBehavior::kNoneForAccessibility argument is passed, which should
    // only be used by a11y code, layout updates will never be performed.
    bool CanBeKeyboardFocusableScroller(UpdateBehavior update_behavior = UpdateBehavior::kStyleAndLayout) const;
    ...
    // If UpdateBehavior::kNoneForAccessibility argument is passed, which should
    // only be used by a11y code, layout updates will never be performed.
    bool IsKeyboardFocusableScroller(UpdateBehavior update_behavior = UpdateBehavior::kStyleAndLayout) const;

    FrozenArray<Element>* GetElementArrayAttribute(const QualifiedName& name);
    void SetElementArrayAttribute(const QualifiedName& name, const HeapVector<Member<Element>>* given_elements);
...
template <> struct DowncastTraits<Element> {
...
{
#if DCHECK_IS_ON()
    DCHECK(FastAttributeLookupAllowed(name)) << TagQName().ToString().Utf8() << "/@" << name.ToString().Utf8();
#endif
...
inline void Element::SetTagNameForCreateElementNS(const QualifiedName& tag_name)
{
