...
// static
void PartRoot::CloneParts(const Node& source_node, Node& destination_node, NodeCloningData& data)
{
    ...
    if (auto* parts = source_node.GetDOMParts()) {
        for (Part* part : *parts) {
            ...
            DCHECK_EQ(static_cast<ChildNodePart*>(part)->nextSibling(), source_node) << "This should be the next sibling node";
            ...
            ChildNodePart& child_node_part = static_cast<ChildNodePart&>(data.CurrentPartRoot());
...
void PartRoot::RebuildPartsList()
{
    ...
    enum class NestedPartRoot { kNone, kAtStart, kAtEnd } nested_part_root = NestedPartRoot::kNone;
    while (node != end_node) {
        ...
        if (auto* parts = node->GetDOMParts()) {
            ...
            // end.
            nested_part_root = nested_part_root == NestedPartRoot::kAtStart ? NestedPartRoot::kAtEnd : NestedPartRoot::kNone;
            for (Part* part : *parts) {
                if (!part->IsValid()) {
                    continue;
                }
                if (PartRoot* part_root = part->GetAsPartRoot()) {
                    ...
                    DCHECK_EQ(part_root->LastIncludedChildNode()->parentNode(), node->parentNode());
...
// caching.
void BuildPartsList(PartRoot& part_root, PartRoot::PartList* part_list, PartRoot::PartNodeList* node_part_nodes, PartRoot::PartNodeList* child_node_part_nodes)
{
    DCHECK(RuntimeEnabledFeatures::DOMPartsAPIMinimalEnabled());
    ...
    while (node != end_node) {
        if (node->HasNodePart()) {
            if (Comment* start_comment = DynamicTo<Comment>(node); start_comment && start_comment->data() == kChildNodePartStartCommentData) {
                ...
                while (node->HasNextSibling() && ((node = node->nextSibling()) != end_node)) {
                    ...
                    if (end_comment.data() == kChildNodePartEndCommentData) [[likely]] {
                        if (!nested_child_node_part_count) [[likely]] {
                            ...
                            if (part_list) {
                                part_list->push_back(MakeGarbageCollected<ChildNodePart>(part_root, *start_comment, end_comment, Vector<String>()));
                            }
                ...
                if (part_list) {
                    part_list->push_back(MakeGarbageCollected<NodePart>(part_root, *node, Vector<String>()));
                }
                if (node_part_nodes) {
...
PartRootUnion* PartRoot::GetUnionFromPartRoot(PartRoot* root)
{
    ...
    if (root->IsDocumentPartRoot()) {
        return MakeGarbageCollected<PartRootUnion>(static_cast<DocumentPartRoot*>(root));
    }
