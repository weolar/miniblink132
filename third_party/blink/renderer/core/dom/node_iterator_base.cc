...
NodeIteratorBase::NodeIteratorBase(Node* root_node, unsigned what_to_show, V8NodeFilter* node_filter)
    : root_(root_node)
...
unsigned NodeIteratorBase::AcceptNode(Node* node, ExceptionState& exception_state)
{
    // DOM 6. Traversal
    ...
    if (active_flag_) {
        ...
        // DOMException.
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Filter function can't be recursive");
    ...
    UseCounter::Count(ExecutionContext::From(filter_->CallbackRelevantScriptState()),
