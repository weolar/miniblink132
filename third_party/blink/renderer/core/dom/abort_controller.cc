...
{
    return MakeGarbageCollected<AbortController>(MakeGarbageCollected<AbortSignal>(ExecutionContext::From(script_state), AbortSignal::SignalType::kController));
}

AbortController::AbortController(AbortSignal* signal)
    : signal_(signal)
{
}
