...
{
    EXPECT_EQ(GetDocument(), GetDocument()->CommonAncestorTreeScope(*GetDocument()));
    ShadowRoot& shadow_root = GetBody()->AttachShadowRootForTesting(ShadowRootMode::kOpen);
...
TEST_F(TreeScopeTest, CommonAncestorOfSiblingTrees)
{
    ...
    ShadowRoot& shadow_root_a = div_a->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    ShadowRoot& shadow_root_b = div_b->AttachShadowRootForTesting(ShadowRootMode::kOpen);

    EXPECT_EQ(GetDocument(), shadow_root_a.CommonAncestorTreeScope(shadow_root_b));
    EXPECT_EQ(GetDocument(), shadow_root_b.CommonAncestorTreeScope(shadow_root_a));
}
...
TEST_F(TreeScopeTest, CommonAncestorOfTreesAtDifferentDepths)
{
    ...
    ShadowRoot& shadow_root_y = div_y->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    ShadowRoot& shadow_root_b = div_b->AttachShadowRootForTesting(ShadowRootMode::kOpen);

    Element* div_in_y = GetDocument()->CreateRawElement(html_names::kDivTag);
    shadow_root_y.AppendChild(div_in_y);
    ShadowRoot& shadow_root_a = div_in_y->AttachShadowRootForTesting(ShadowRootMode::kOpen);
