...
public:
    ...
    SpaceSplitString(const SpaceSplitString& other)
        : data_(other.data_)
    {
        if (data_) {
            data_->MarkShared();
    ...
    const AtomicString& operator[](wtf_size_t i) const
    {
        return (*data_)[i];
    }
    Vector<AtomicString, 4>::const_iterator begin() const
    {
        return data_ ? data_->begin() : Vector<AtomicString, 4>::const_iterator(nullptr);
    }
    Vector<AtomicString, 4>::const_iterator end() const
    {
        return data_ ? data_->end() : Vector<AtomicString, 4>::const_iterator(nullptr);
    }
...
private:
    ...
    public:
        ...
        bool MightBeShared() const
        {
            return might_be_shared_;
        }
        wtf_size_t size() const
        {
            return vector_.size();
        ...
        AtomicString& operator[](wtf_size_t i)
        {
            return vector_[i];
        }
    ...
    private:
        ...
        template <typename CharacterType> inline void CreateVector(const AtomicString&, base::span<const CharacterType>);
