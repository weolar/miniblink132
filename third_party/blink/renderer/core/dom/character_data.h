...
protected:
    ...
    {
        DCHECK(
            type == kCreateComment || type == kCreateText || type == kCreateCdataSection || type == kCreateProcessingInstruction || type == kCreateEditingText);
    }
...
private:
    ...
    bool IsCharacterDataNode() const final
    {
        return true;
    }
    void SetDataAndUpdate(const String&, const TextDiffRange&, UpdateSource = kUpdateFromNonParser);
...
template <> struct DowncastTraits<CharacterData> {
