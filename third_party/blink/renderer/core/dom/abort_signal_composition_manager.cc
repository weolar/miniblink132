...
AbortSignalCompositionManager::AbortSignalCompositionManager(AbortSignal& signal, AbortSignalCompositionType type)
    : signal_(signal)
...
DependentSignalCompositionManager::DependentSignalCompositionManager(
    AbortSignal& managed_signal, AbortSignalCompositionType type, const HeapVector<Member<AbortSignal>>& source_signals)
    : AbortSignalCompositionManager(managed_signal, type)
{
    ...
    for (auto& source : source_signals) {
        if (source->IsCompositeSignal()) {
            auto* source_manager = To<DependentSignalCompositionManager>(source->GetCompositionManager(GetCompositionType()));
...
DependentSignalCompositionManager::~DependentSignalCompositionManager() = default;
...
{
    auto* source_manager = To<SourceSignalCompositionManager>(source.GetCompositionManager(GetCompositionType()));
    DCHECK(source_manager);
...
void DependentSignalCompositionManager::OnSourceSettled(SourceSignalCompositionManager& source_manager)
{
    DCHECK(GetSignal().IsCompositeSignal());
    DCHECK(!IsSettled());
...
SourceSignalCompositionManager::SourceSignalCompositionManager(AbortSignal& signal, AbortSignalCompositionType composition_type)
...
void SourceSignalCompositionManager::AddDependentSignal(DependentSignalCompositionManager& dependent_manager)
{
    DCHECK(!IsSettled());
    ...
    DCHECK(GetCompositionType() != AbortSignalCompositionType::kAbort || !GetSignal().aborted());
...
void SourceSignalCompositionManager::Settle()
{
    ...
    for (auto& signal : dependent_signals_) {
        auto* manager = To<DependentSignalCompositionManager>(signal->GetCompositionManager(GetCompositionType()));
        DCHECK(manager);
