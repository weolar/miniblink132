...
TEST_F(WhitespaceAttacherTest, WhitespaceAfterReattachedWhitespace)
{
    ...
    auto* second_whitespace = To<Text>(first_whitespace->nextSibling()->nextSibling());
...
{
    GetDocument().body()->setInnerHTML("<div id=block></div><span style='display:contents'> </span>");
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(WhitespaceAttacherTest, WhitespaceInDisplayContentsAfterReattachedInline)
{
    GetDocument().body()->setInnerHTML("<span id=inline></span><span style='display:contents'> </span>");
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(WhitespaceAttacherTest, WhitespaceAfterEmptyDisplayContentsAfterReattachedBlock)
{
    GetDocument().body()->setInnerHTML("<div id=block></div><span style='display:contents'></span> ");
    UpdateAllLifecyclePhasesForTest();
...
{
    GetDocument().body()->setInnerHTML("<div id=block></div><span style='display:contents'>"
...
{
    GetDocument().body()->setInnerHTML("<span id=inline></span><span style='display:contents'>"
    ...
    Element* span = GetDocument().getElementById(AtomicString("inline"));
    auto* contents = To<Element>(span->nextSibling());
    auto* text = To<Text>(GetDocument().getElementById(AtomicString("inner"))->firstChild());
    EXPECT_TRUE(text->GetLayoutObject());
...
TEST_F(WhitespaceAttacherTest, SlottedWhitespaceInsideDisplayContents)
{
    ...
    ShadowRoot& shadow_root = host->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    shadow_root.setInnerHTML("<span id=inline></span>"
...
{
    GetDocument().body()->setInnerHTML("<span id=inline></span><div id=float style='float:right'></div> ");
    UpdateAllLifecyclePhasesForTest();
...
{
    GetDocument().body()->setInnerHTML("<span>x</span> <span id=test></span> <span>y</span>"
                                       "<style>"
    ...
    Node* scroll_marker_group = LayoutTreeBuilderTraversal::NextLayoutSibling(*first_space);
    ...
    Node* scroller = LayoutTreeBuilderTraversal::NextLayoutSibling(*scroll_marker_group);
...
TEST_F(WhitespaceAttacherTest, RemoveSpaceForScrollMarkerGroup)
{
    ...
    ASSERT_TRUE(scroller);
    Node* scroll_marker_group = LayoutTreeBuilderTraversal::NextLayoutSibling(*scroller);
    ASSERT_TRUE(scroll_marker_group);
    EXPECT_TRUE(scroll_marker_group->IsScrollMarkerGroupAfterPseudoElement());
    EXPECT_TRUE(scroll_marker_group->GetLayoutObject());

    Node* space2 = LayoutTreeBuilderTraversal::NextLayoutSibling(*scroll_marker_group);
    ASSERT_TRUE(space2);
    EXPECT_TRUE(space2->IsTextNode());
    EXPECT_TRUE(space2->GetLayoutObject());

    To<Element>(scroller)->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kFlex);
    UpdateAllLifecyclePhasesForTest();
...
{
    GetDocument().body()->setInnerHTML("<style>div { display: contents }</style>"
    ...
    Node* space = GetDocument().getElementById(AtomicString("innerdiv"))->firstChild();
    ASSERT_TRUE(space);
