...
// static
NodePart* NodePart::Create(PartRootUnion* root_union, Node* node, const PartInit* init, ExceptionState& exception_state)
{
    if (!IsAcceptableNodeType(*node)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidNodeTypeError, "The provided node is not a valid node for a NodePart.");
        return nullptr;
    }
    return MakeGarbageCollected<NodePart>(*PartRoot::GetPartRootFromUnion(root_union), *node, init);
}
...
{
    DCHECK(IsValid());
    return MakeGarbageCollected<NodePart>(data.CurrentPartRoot(), node_clone, metadata().AsVector());
}
