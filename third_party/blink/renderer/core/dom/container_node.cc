...
public:
    ...
    bool NeedsRecheck()
    {
        ...
        if (node_document_ == parent_document_)
            return true;
        return parent_document_->DomTreeVersion() == original_parent_document_version_;
    }
...
inline bool CheckReferenceChildParent(const Node& parent, const Node* next, const Node* old_child, ExceptionState& exception_state)
{
    ...
    if (old_child && old_child->parentNode() != &parent) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotFoundError, "The node to be replaced is not a child of this node.");
        return false;
    }
    return true;
}
...
static inline bool CollectChildrenAndRemoveFromOldParent(Node& node, NodeVector& nodes, ExceptionState& exception_state)
{
...
bool ContainerNode::IsHostIncludingInclusiveAncestorOfThis(const Node& new_child, ExceptionState& exception_state) const
{
    ...
    if (child_contains_parent) {
        exception_state.ThrowDOMException(DOMExceptionCode::kHierarchyRequestError, "The new child element contains the parent.");
    }
    ...
    const Node* new_child, const VectorOf<Node>* new_children, const Node* next, const Node* old_child, ExceptionState& exception_state) const
{
    ...
    if (new_child && (new_child->IsElementNode() || new_child->IsTextNode()) && IsElementNode()) {
    ...
    if (auto* document = DynamicTo<Document>(this)) {
        ...
        if (!CheckReferenceChildParent(*this, next, old_child, exception_state))
            return false;
        // Step 4-6.
        return document->CanAcceptChild(new_child, new_children, next, old_child, exception_state);
    }
    ...
    auto is_child_allowed = [&](const Node* child) -> bool {
        if (!ChildTypeAllowed(child->getNodeType())) {
            exception_state.ThrowDOMException(DOMExceptionCode::kHierarchyRequestError,
                "Nodes of type '" + child->nodeName() + "' may not be inserted inside nodes of type '" + nodeName() + "'.");
            return false;
        }
        return true;
    };
...
// trees.
bool ContainerNode::RecheckNodeInsertionStructuralPrereq(const NodeVector& new_children, const Node* next, ExceptionState& exception_state)
{
...
void ContainerNode::InsertNodeVector(const NodeVector& targets, Node* next, const Functor& mutator, NodeVector& post_insertion_notification_targets)
{
...
void ContainerNode::DidInsertNodeVector(const NodeVector& targets, Node* next, const NodeVector& post_insertion_notification_targets)
{
...
void ContainerNode::InsertBefore(const VectorOf<Node>& new_children, Node* ref_child, ExceptionState& exception_state)
{
    ...
    if (!EnsurePreInsertionValidity(/*new_child*/ nullptr, &new_children, ref_child, /*old_child*/ nullptr, exception_state)) {
        return;
    }
    ...
    if (new_children.size() == 1u) {
        ...
        if (!detector.NeedsRecheck() && !RecheckNodeInsertionStructuralPrereq(new_children, ref_child, exception_state)) {
            return;
        }
    }
    ...
    NodeVector post_insertion_notification_targets;
    {
        ...
        InsertNodeVector(new_children, ref_child, AdoptAndInsertBefore(), post_insertion_notification_targets);
    }
    DidInsertNodeVector(new_children, ref_child, post_insertion_notification_targets);
}
...
Node* ContainerNode::InsertBefore(Node* new_child, Node* ref_child, ExceptionState& exception_state)
{
    ...
    if (!EnsurePreInsertionValidity(new_child, /*new_children*/ nullptr, ref_child, /*old_child*/ nullptr, exception_state)) {
        return new_child;
    }
    ...
    if (!detector.NeedsRecheck()) {
        if (!RecheckNodeInsertionStructuralPrereq(targets, ref_child, exception_state))
            return new_child;
    }
...
bool ContainerNode::CheckParserAcceptChild(const Node& new_child) const
{
    ...
    return document->CanAcceptChild(&new_child, /*new_children*/ nullptr,
        /*next*/ nullptr, /*old_child*/ nullptr, IGNORE_EXCEPTION_FOR_TESTING);
}

void ContainerNode::ParserInsertBefore(Node* new_child, Node& next_child)
{
    DCHECK(new_child);
    DCHECK(next_child.parentNode() == this || (DynamicTo<DocumentFragment>(this) && DynamicTo<DocumentFragment>(this)->IsTemplateContent()));
...
void ContainerNode::ReplaceChild(const VectorOf<Node>& new_children, Node* old_child, ExceptionState& exception_state)
{
    // https://dom.spec.whatwg.org/#concept-node-replace
    if (!old_child) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotFoundError, "The node to be replaced is null.");
        return;
    }

    if (!EnsurePreInsertionValidity(/*new_child*/ nullptr, &new_children,
            /*next*/ nullptr, old_child, exception_state)) {
        return;
    }
    ...
    NodeVector post_insertion_notification_targets;
    {
        ...
        if (needs_recheck && !RecheckNodeInsertionStructuralPrereq(new_children, next, exception_state)) {
            return;
        }
        ...
        // observers flag set.
        if (next) {
            InsertNodeVector(new_children, next, AdoptAndInsertBefore(), post_insertion_notification_targets);
        } else {
            InsertNodeVector(new_children, nullptr, AdoptAndAppendChild(), post_insertion_notification_targets);
        }
    }
...
void ContainerNode::WillRemoveChild(Node& child)
{
    ...
    if (!GetDocument().StatePreservingAtomicMoveInProgress()) {
        ...
        ChildFrameDisconnector(child, ChildFrameDisconnector::DisconnectReason::kDisconnectSelf).Disconnect();
    }
...
void ContainerNode::WillRemoveChildren()
{
    ...
    // the tree, instead of atomically re-inserting it.
    if (!GetDocument().StatePreservingAtomicMoveInProgress()) {
        ChildFrameDisconnector(*this, ChildFrameDisconnector::DisconnectReason::kDisconnectSelf).Disconnect(ChildFrameDisconnector::kDescendantsOnly);
    }
}
...
static bool ShouldMergeCombinedTextAfterRemoval(const Node& old_child)
{
    ...
    if (!next_sibling)
        return false;
    if (IsA<LayoutTextCombine>(previous_sibling) && IsA<LayoutTextCombine>(next_sibling)) [[unlikely]] {
        return true;
    }

    // Request to merge combined texts in anonymous block.
    // See http://crbug.com/1233432
    if (!previous_sibling->IsAnonymousBlock() || !next_sibling->IsAnonymousBlock())
        return false;

    if (IsA<LayoutTextCombine>(previous_sibling->SlowLastChild()) && IsA<LayoutTextCombine>(next_sibling->SlowFirstChild())) [[unlikely]] {
        return true;
    }
    return false;
}
...
Node* ContainerNode::RemoveChild(Node* old_child, ExceptionState& exception_state)
{
    ...
    if (!old_child || old_child->parentNode() != this || old_child->IsPseudoElement()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotFoundError, "The node to be removed is not a child of this node.");
        return nullptr;
    }
    ...
    if (!GetForceReattachLayoutTree() && ShouldMergeCombinedTextAfterRemoval(*child)) [[unlikely]] {
...
void ContainerNode::RemoveBetween(Node* previous_child, Node* next_child, Node& old_child)
{
    ...
    if (InActiveDocument() && !GetDocument().StatePreservingAtomicMoveInProgress()) {
...
void ContainerNode::ParserRemoveChild(Node& old_child)
{
    ...
    if (old_child.ConnectedSubframeCount()) {
        ChildFrameDisconnector(old_child, ChildFrameDisconnector::DisconnectReason::kDisconnectSelf).Disconnect();
    }
...
void ContainerNode::RemoveChildren(SubtreeModificationAction action)
{
    ...
    const bool children_changed = ChildrenChangedAllChildrenRemovedNeedsList();
    {
        ...
        ChildrenChange change = { .type = ChildrenChangeType::kAllChildrenRemoved,
...
void ContainerNode::AppendChildren(const VectorOf<Node>& new_children, ExceptionState& exception_state)
{
    if (!EnsurePreInsertionValidity(/*new_child*/ nullptr, &new_children,
            /*next*/ nullptr, /*old_child*/ nullptr, exception_state)) {
        return;
    }
    ...
    if (new_children.size() == 1u) {
        ...
        if (!detector.NeedsRecheck() && !RecheckNodeInsertionStructuralPrereq(new_children, nullptr, exception_state)) {
            return;
        }
    }
    ...
    DidInsertNodeVector(new_children, nullptr, post_insertion_notification_targets);
}
...
Node* ContainerNode::AppendChild(Node* new_child, ExceptionState& exception_state)
{
    ...
    if (!CollectChildrenAndRemoveFromOldParent(*new_child, targets, exception_state))
        return new_child;
    if (!detector.NeedsRecheck()) {
        if (!RecheckNodeInsertionStructuralPrereq(targets, nullptr, exception_state))
            return new_child;
    }
...
void ContainerNode::ParserAppendChild(Node* new_child)
{
    ...
    RUNTIME_CALL_TIMER_SCOPE(GetDocument().GetAgent().isolate(), RuntimeCallStats::CounterId::kParserAppendChild);
...
void ContainerNode::ParserFinishedBuildingDocumentFragment()
{
    ...
    for (Node& node : NodeTraversal::DescendantsOf(*this)) {
        NotifyNodeAtEndOfBuildingFragmentTree(node, change, may_contain_shadow_roots);
    }
...
void ContainerNode::NotifyNodeAtEndOfBuildingFragmentTree(Node& node, const ChildrenChange& change, bool may_contain_shadow_roots)
{
    ...
    if (ShadowRoot* shadow_root = node.GetShadowRoot()) {
        for (Node& shadow_node : NodeTraversal::InclusiveDescendantsOf(*shadow_root)) {
            NotifyNodeAtEndOfBuildingFragmentTree(shadow_node, change, may_contain_shadow_roots);
        }
    }
...
{
    const bool is_state_preserving_atomic_insert = GetDocument().StatePreservingAtomicMoveInProgress();
    EventDispatchForbiddenScope assert_no_event_dispatch;
    ScriptForbiddenScope forbid_script;
    ...
    for (Node& node : NodeTraversal::InclusiveDescendantsOf(root)) {
        ...
        // Parts attached.
        if (!isConnected() && !IsInShadowTree() && !node.IsContainerNode() && !node.GetDOMParts()) {
            continue;
        }
        ...
        if (Node::kInsertionShouldCallDidNotifySubtreeInsertions == node.InsertedInto(*this) && !is_state_preserving_atomic_insert) {
            post_insertion_notification_targets.push_back(&node);
        }
        if (ShadowRoot* shadow_root = node.GetShadowRoot()) {
            NotifyNodeInsertedInternal(*shadow_root, post_insertion_notification_targets);
        }
    }
}
...
void ContainerNode::NotifyNodeRemoved(Node& root)
{
    ...
    for (Node& node : NodeTraversal::InclusiveDescendantsOf(root)) {
        ...
        // needed.
        if (!node.IsContainerNode() && !node.IsInTreeScope() && !node.GetDOMParts()) {
            continue;
        }
...
void ContainerNode::CloneChildNodesFrom(const ContainerNode& node, NodeCloningData& data)
{
...
StaticElementList* ContainerNode::querySelectorAll(const AtomicString& selectors, ExceptionState& exception_state)
{
...
StaticElementList* ContainerNode::QuerySelectorAll(const AtomicString& selectors)
{
...
static void DispatchChildInsertionEvents(Node& child)
{
    ...
    if (c->parentNode() && document.HasListenerType(Document::kDOMNodeInsertedListener)) {
        c->DispatchScopedEvent(*MutationEvent::Create(event_type_names::kDOMNodeInserted, Event::Bubbles::kYes, c->parentNode()));
    }

    // dispatch the DOMNodeInsertedIntoDocument event to all descendants
    if (c->isConnected() && document.HasListenerType(Document::kDOMNodeInsertedIntoDocumentListener)) {
        for (; c; c = NodeTraversal::Next(*c, &child)) {
            c->DispatchScopedEvent(*MutationEvent::Create(event_type_names::kDOMNodeInsertedIntoDocument, Event::Bubbles::kNo));
        }
    }
}
...
static void DispatchChildRemovalEvents(Node& child)
{
    ...
    if (c->parentNode() && document.HasListenerType(Document::kDOMNodeRemovedListener)) {
        NodeChildRemovalTracker scope(child);
        c->DispatchScopedEvent(*MutationEvent::Create(event_type_names::kDOMNodeRemoved, Event::Bubbles::kYes, c->parentNode()));
    }
    ...
    if (c->isConnected() && document.HasListenerType(Document::kDOMNodeRemovedFromDocumentListener)) {
        NodeChildRemovalTracker scope(child);
        for (; c; c = NodeTraversal::Next(*c, &child)) {
            c->DispatchScopedEvent(*MutationEvent::Create(event_type_names::kDOMNodeRemovedFromDocument, Event::Bubbles::kNo));
        }
    }
}
...
void ContainerNode::RecalcDescendantStyles(const StyleRecalcChange change, const StyleRecalcContext& style_recalc_context)
{
...
void ContainerNode::RebuildLayoutTreeForChild(Node* child, WhitespaceAttacher& whitespace_attacher)
{
    if (auto* child_text_node = DynamicTo<Text>(child)) {
...
void ContainerNode::RebuildChildrenLayoutTrees(WhitespaceAttacher& whitespace_attacher)
{
...
void ContainerNode::CheckForSiblingStyleChanges(SiblingCheckType change_type, Element* changed_element, Node* node_before_change, Node* node_after_change)
{
    if (!InActiveDocument() || GetDocument().HasPendingForcedStyleRecalc() || GetStyleChangeType() == kSubtreeStyleChange)
        return;
    ...
    if ((ChildrenAffectedByForwardPositionalRules() && element_after_change) || (ChildrenAffectedByBackwardPositionalRules() && element_before_change)) {
        GetDocument().GetStyleEngine().ScheduleNthPseudoInvalidations(*this);
    }

    if (ChildrenAffectedByFirstChildRules() && !element_before_change && element_after_change && element_after_change->AffectedByFirstChildRules()) {
    ...
    if (ChildrenAffectedByLastChildRules() && !element_after_change && element_before_change && element_before_change->AffectedByLastChildRules()) {
    ...
    if (change_type == kSiblingElementInserted) {
        GetDocument().GetStyleEngine().ScheduleInvalidationsForInsertedSibling(element_before_change, *changed_element);
        return;
    }

    DCHECK(change_type == kSiblingElementRemoved);
    GetDocument().GetStyleEngine().ScheduleInvalidationsForRemovedSibling(element_before_change, *changed_element, *element_after_change);
}

void ContainerNode::InvalidateNodeListCachesInAncestors(const QualifiedName* attr_name, Element* attribute_owner_element, const ChildrenChange* change)
{
...
HTMLCollection* ContainerNode::getElementsByTagName(const AtomicString& qualified_name)
{
    DCHECK(!qualified_name.IsNull());

    if (IsA<HTMLDocument>(GetDocument())) {
        return EnsureCachedCollection<HTMLTagCollection>(kHTMLTagCollectionType, qualified_name);
    }
    return EnsureCachedCollection<TagCollection>(kTagCollectionType, qualified_name);
}

HTMLCollection* ContainerNode::getElementsByTagNameNS(const AtomicString& namespace_uri, const AtomicString& local_name)
{
    return EnsureCachedCollection<TagCollectionNS>(kTagCollectionNSType, namespace_uri.empty() ? g_null_atom : namespace_uri, local_name);
}
...
RadioNodeList* ContainerNode::GetRadioNodeList(const AtomicString& name, bool only_match_img_elements)
{
    DCHECK(IsA<HTMLFormElement>(this) || IsA<HTMLFieldSetElement>(this));
    CollectionType type = only_match_img_elements ? kRadioImgNodeListType : kRadioNodeListType;
    return EnsureCachedCollection<RadioNodeList>(type, name);
}

String ContainerNode::FindTextInElementWith(const AtomicString& substring, base::FunctionRef<bool(const String&)> validity_checker) const
{
    for (Element& element : ElementTraversal::DescendantsOf(*this)) {
        String text;
        if (element.HasTagName(html_names::kInputTag) && element.FastHasAttribute(html_names::kReadonlyAttr)
            && EqualIgnoringASCIICase(element.FastGetAttribute(html_names::kTypeAttr), "text")
        ...
        if (text.FindIgnoringASCIICase(substring) != WTF::kNotFound && validity_checker(text)) {
            return text;
        }
    }
...
StaticNodeList* ContainerNode::FindAllTextNodesMatchingRegex(const String& regex) const
{
    blink::HeapVector<Member<Node>> nodes_matching_regex;
    Node* node = FlatTreeTraversal::FirstWithin(*this);
    ScriptRegexp* raw_regexp = MakeGarbageCollected<ScriptRegexp>(GetDocument().GetAgent().isolate(), regex, kTextCaseASCIIInsensitive);
    while (node) {
        if (node->IsTextNode()) {
...
Element* ContainerNode::GetAutofocusDelegate() const
{
    ...
    while (element) {
        ...
        Element* focusable_area = element->IsFocusable() ? element : element->GetFocusableArea();
        if (!focusable_area) {
            element = ElementTraversal::Next(*element, this);
            continue;
        }
...
void ContainerNode::ReplaceChildren(Node* new_child, ExceptionState& exception_state)
{
...
// https://dom.spec.whatwg.org/#dom-parentnode-replacechildren
void ContainerNode::ReplaceChildren(const VectorOf<Node>& nodes, ExceptionState& exception_state)
{
...
void ContainerNode::CheckSoftNavigationHeuristicsTracking(const Document& document, Node& inserted_node)
{
...
String ContainerNode::getInnerHTML(const GetInnerHTMLOptions* options) const
{
    ...
    ShadowRootInclusion shadow_root_inclusion { options->includeShadowRoots() ? ShadowRootInclusion::Behavior::kIncludeAllOpenShadowRoots
    ...
    return CreateMarkup(this, kChildrenOnly, kDoNotResolveURLs, shadow_root_inclusion);
}

String ContainerNode::getHTML(const GetHTMLOptions* options, ExceptionState& exception_state) const
{
    DCHECK(options && options->hasSerializableShadowRoots()) << "Should have IDL default";
    DCHECK(options->hasShadowRoots()) << "Should have IDL default";
    DCHECK(IsShadowRoot() || IsElementNode());
    ShadowRootInclusion shadow_root_inclusion { options->serializableShadowRoots() ? ShadowRootInclusion::Behavior::kIncludeAnySerializableShadowRoots
                                                                                   : ShadowRootInclusion::Behavior::kOnlyProvidedShadowRoots };
