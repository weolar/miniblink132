...
public:
    NthIndexData(
        ContainerNode&, const CSSSelectorList* filter, const SelectorChecker* selector_checker, const SelectorChecker::SelectorCheckingContext* context);
    ...
    unsigned NthIndex(
        Element&, const CSSSelectorList* filter, const SelectorChecker* selector_checker, const SelectorChecker::SelectorCheckingContext* context) const;
...
private:
    ...
    struct Key : public GarbageCollected<Key> {
        ...
        Key(Node* parent_arg, String child_tag_name_arg)
            : parent(parent_arg)
            , child_tag_name(child_tag_name_arg)
        {
        }
        ...
        bool operator==(const Key& other) const
        {
            ...
            // is simpler.
            return parent == other.parent && filter == other.filter && child_tag_name == other.child_tag_name;
        }
    };
    ...
    struct KeyHashTraits : WTF::MemberHashTraits<Key> {
        static unsigned GetHash(const Member<Key>& key)
        {
            return key->GetHash();
        }
    ...
    static bool MatchesFilter(
        Element* element, const CSSSelectorList* filter, const SelectorChecker* selector_checker, const SelectorChecker::SelectorCheckingContext* context);
    static unsigned UncachedNthChildIndex(Element& element, const CSSSelectorList* filter, const SelectorChecker* selector_checker,
        const SelectorChecker::SelectorCheckingContext* context, unsigned& sibling_count);
    static unsigned UncachedNthLastChildIndex(Element& element, const CSSSelectorList* filter, const SelectorChecker* selector_checker,
        const SelectorChecker::SelectorCheckingContext* context, unsigned& sibling_count);
    ...
    HeapHashMap<Member<Key>, Member<NthIndexData>, KeyHashTraits>* cache_ = nullptr;
