...
ScrollMarkerGroupPseudoElement::ScrollMarkerGroupPseudoElement(Element* originating_element, PseudoId pseudo_id)
...
void ScrollMarkerGroupPseudoElement::AddToFocusGroup(ScrollMarkerPseudoElement& scroll_marker)
{
    scroll_marker.SetScrollMarkerGroup(this);
    focus_group_.push_back(scroll_marker);
}

ScrollMarkerPseudoElement* ScrollMarkerGroupPseudoElement::FindNextScrollMarker(const Element& current)
{
...
ScrollMarkerPseudoElement* ScrollMarkerGroupPseudoElement::FindPreviousScrollMarker(const Element& current)
{
    if (wtf_size_t index = focus_group_.Find(current); index != kNotFound) {
...
void ScrollMarkerGroupPseudoElement::RemoveFromFocusGroup(const ScrollMarkerPseudoElement& scroll_marker)
{
...
{
    ActivateScrollMarker(&ScrollMarkerGroupPseudoElement::FindPreviousScrollMarker);
}

void ScrollMarkerGroupPseudoElement::ActivateScrollMarker(ScrollMarkerPseudoElement* (ScrollMarkerGroupPseudoElement::*find_scroll_marker_func)(const Element&))
{
    if (!selected_marker_) {
        return;
    }
    ScrollMarkerPseudoElement* scroll_marker = (this->*find_scroll_marker_func)(*Selected());
    ...
    mojom::blink::ScrollIntoViewParamsPtr params = scroll_into_view_util::CreateScrollIntoViewParams(*scroll_marker->parentElement()->GetComputedStyle());
    scroll_marker->ScrollIntoViewNoVisualUpdate(std::move(params));
    GetDocument().SetFocusedElement(scroll_marker,
        FocusParams(SelectionBehaviorOnFocus::kNone, mojom::blink::FocusType::kNone,
            /*capabilities=*/nullptr));
    SetSelected(*scroll_marker);
}

bool ScrollMarkerGroupPseudoElement::SetSelected(ScrollMarkerPseudoElement& scroll_marker)
{
    if (selected_marker_ == scroll_marker) {
        return false;
    }
...
{
    HeapVector<Member<ScrollMarkerPseudoElement>> focus_group = std::move(focus_group_);
...
bool ScrollMarkerGroupPseudoElement::UpdateSelectedScrollMarker(const ScrollOffset& offset)
{
    ...
    for (ScrollMarkerPseudoElement* scroll_marker : ScrollMarkers()) {
        ...
        const LayoutBox* target_box = scroll_marker->UltimateOriginatingElement()->GetLayoutBox();
        if (!target_box) {
            continue;
        }
        const LayoutBox* scroll_marker_box = scroll_marker->GetLayoutBox();
        CHECK(scroll_marker_box);
        PhysicalBoxStrut scroll_margin = target_box->Style() ? target_box->Style()->ScrollMarginStrut() : PhysicalBoxStrut();
        ...
        PhysicalRect rect_to_scroll = scroller->AbsoluteToLocalRect(scroll_marker_box->AbsoluteBoundingBoxRectForScrollIntoView(), flag);
        rect_to_scroll.Expand(scroll_margin);
        ScrollOffset target_scroll_offset = scroll_into_view_util::GetScrollOffsetToExpose(*scrollable_area, rect_to_scroll, scroll_margin,
            scroll_into_view_util::PhysicalAlignmentFromSnapAlignStyle(*target_box, kHorizontalScroll),
            scroll_into_view_util::PhysicalAlignmentFromSnapAlignStyle(*target_box, kVerticalScroll));
        PhysicalOffset target_offset(LayoutUnit(target_scroll_offset.x()), LayoutUnit(target_scroll_offset.y()));
        ...
        if ((target_offset.left.Abs() <= scroll_offset.left.Abs() || max_offset.x() == min_offset.x())
            && (target_offset.top.Abs() <= scroll_offset.top.Abs() || max_offset.y() == min_offset.y())) {
            selected = scroll_marker;
        }
    }
    if (selected) {
