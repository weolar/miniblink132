...
public:
    ...
    static TextDiffRange Replace(wtf_size_t offset, wtf_size_t old_size, wtf_size_t new_size)
    {
        return TextDiffRange { offset, old_size, new_size };
    }

    inline wtf_size_t OldEndOffset() const
    {
        return offset + old_size;
    }
    inline wtf_size_t NewEndOffset() const
    {
        return offset + new_size;
    }

    // Check if text outside of the diff are not changed.
    void CheckValid(const WTF::String& old_text, const WTF::String& new_text) const;
...
inline void TextDiffRange::CheckValid(const WTF::String& old_text, const WTF::String& new_text) const
{
}
