...
// arrow keys.
class ScrollMarkerGroupPseudoElement : public PseudoElement, public ScrollSnapshotClient {
public:
    // pseudo_id is needed, as ::scroll-marker-group can be after or before.
    ScrollMarkerGroupPseudoElement(Element* originating_element, PseudoId pseudo_id);

    bool IsScrollMarkerGroupPseudoElement() const final
    {
        return true;
    }
    ...
    ScrollMarkerPseudoElement* Selected()
    {
...
private:
    void ActivateScrollMarker(ScrollMarkerPseudoElement* (ScrollMarkerGroupPseudoElement::*find_scroll_marker_func)(const Element&));
...
template <> struct DowncastTraits<ScrollMarkerGroupPseudoElement> {
