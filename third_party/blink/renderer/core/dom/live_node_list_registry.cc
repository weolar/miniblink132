...
{
    visitor->RegisterWeakCallbackMethod<LiveNodeListRegistry, &LiveNodeListRegistry::ProcessCustomWeakness>(this);
}
...
{
    auto it = std::remove_if(data_.begin(), data_.end(), [info](Entry entry) { return !info.IsHeapObjectAlive(entry.first); });
    if (it == data_.end())
        return;
