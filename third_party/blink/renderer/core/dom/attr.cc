...
Attr::Attr(Document& document, const QualifiedName& name, const AtomicString& standalone_value)
...
const QualifiedName Attr::GetQualifiedName() const
{
    if (element_ && !standalone_value_or_attached_local_name_.IsNull()) {
        ...
        return QualifiedName(name_.Prefix(), standalone_value_or_attached_local_name_, name_.NamespaceURI());
    }
...
void Attr::setValue(const AtomicString& value, ExceptionState& exception_state)
{
    // Element::setAttribute will remove the attribute if value is null.
    DCHECK(!value.IsNull());
    if (element_) {
        element_->SetAttributeWithValidation(GetQualifiedName(), value, exception_state);
    } else {
...
void Attr::setTextContentForBinding(const V8UnionStringOrTrustedScript* value, ExceptionState& exception_state)
{
    String string_value;
    if (value) {
        if (value->IsString())
...
Node* Attr::Clone(Document& factory, NodeCloningData&, ContainerNode* append_to, ExceptionState& append_exception_state) const
{
...
void Attr::AttachToElement(Element* element, const AtomicString& attached_local_name)
{
    DCHECK(!element_);
    element_ = element;
