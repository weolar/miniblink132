...
Node* NodeTraversal::PreviousIncludingPseudo(const Node& current, const Node* stay_within)
{
...
Node* NodeTraversal::NextIncludingPseudo(const Node& current, const Node* stay_within)
{
...
Node* NodeTraversal::NextIncludingPseudoSkippingChildren(const Node& current, const Node* stay_within)
{
    for (Node& node : InclusiveAncestorsOf(current)) {
        if (node == stay_within)
            return nullptr;
...
Node* NodeTraversal::PreviousAbsoluteSiblingIncludingPseudo(const Node& current, const Node* stay_within)
{
