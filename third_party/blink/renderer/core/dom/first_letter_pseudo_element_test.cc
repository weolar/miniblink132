...
TEST_F(FirstLetterPseudoElementTest, DoesNotBreakEmoji)
{
    ...
    EXPECT_EQ(2u, FirstLetterPseudoElement::FirstLetterLength(emoji, preserve_breaks, punctuation));
}
...
TEST_F(FirstLetterPseudoElementTest, EmptySpanOnly)
{
    ...
    const PseudoElement* const first_letter = sample.GetPseudoElement(kPseudoIdFirstLetter);
...
TEST_F(FirstLetterPseudoElementTest, InitialLetter)
{
    ...
    const auto& initial_letter_box = *sample.GetPseudoElement(kPseudoIdFirstLetter)->GetLayoutObject();
    const auto& initial_letter_text1 = *To<LayoutTextFragment>(initial_letter_box.SlowFirstChild());
    ...
    EXPECT_EQ(sample.GetLayoutObject()->StyleRef().GetFont(), initial_letter_box.StyleRef().GetFont()) << "initial letter box should have a specified font.";

    const auto& initial_letter_text_style1 = initial_letter_text1.StyleRef();
    EXPECT_EQ(EVerticalAlign::kBaseline, initial_letter_text_style1.VerticalAlign());
    EXPECT_EQ(LayoutUnit(80), initial_letter_text_style1.ComputedLineHeightAsFixed());
    EXPECT_EQ(FontHeight(LayoutUnit(64), LayoutUnit(16)), initial_letter_text_style1.GetFontHeight()) << "initial letter box should have a cap font.";

    // Changing paragraph style should be distributed to initial letter text.
    sample.style()->setProperty(GetDocument().GetExecutionContext(), "font-size", "30px", String(), ASSERT_NO_EXCEPTION);
    sample.style()->setProperty(GetDocument().GetExecutionContext(), "line-height", "34px", String(), ASSERT_NO_EXCEPTION);
    UpdateAllLifecyclePhasesForTest();

    const auto& initial_letter_text2 = *To<LayoutTextFragment>(initial_letter_box.SlowFirstChild());
    EXPECT_EQ(&initial_letter_text2, &initial_letter_text1) << "font-size and line-height changes don't build new first-letter tree.";

    const auto& initial_letter_text_style2 = initial_letter_text2.StyleRef();
    EXPECT_EQ(EVerticalAlign::kBaseline, initial_letter_text_style2.VerticalAlign());
    EXPECT_EQ(LayoutUnit(115), initial_letter_text_style2.ComputedLineHeightAsFixed());
    EXPECT_EQ(FontHeight(LayoutUnit(92), LayoutUnit(23)), initial_letter_text_style2.GetFontHeight()) << "initial letter box should have a cap font.";
}

TEST_F(FirstLetterPseudoElementTest, UnicodePairBreaking)
{
    const UChar test_string[] = { 0xD800, 0xDD00, 'A', 0xD800, 0xDD00, 0xD800, 0xDD00, 'B', 0 };
    const bool preserve_breaks = false;
    FirstLetterPseudoElement::Punctuation punctuation = FirstLetterPseudoElement::Punctuation::kNotSeen;
    EXPECT_EQ(7u, FirstLetterPseudoElement::FirstLetterLength(test_string, preserve_breaks, punctuation));
}
...
class FirstLetterTextTest : public FirstLetterPseudoElementTest, public ::testing::WithParamInterface<struct FirstLetterLayoutTextTestCase> { };

INSTANTIATE_TEST_SUITE_P(FirstLetterPseudoElemenTest, FirstLetterTextTest, testing::ValuesIn(first_letter_layout_text_cases));
...
TEST_P(FirstLetterTextTest, All)
{
    ...
    builder.SetPseudoElementStyles(1 << (kPseudoIdFirstLetter - kFirstPublicPseudoId));
    layout_body->SetStyle(builder.TakeStyle(), LayoutObject::ApplyStyleChanges::kNo);

    LayoutText* layout_text = FirstLetterPseudoElement::FirstLetterTextLayoutObject(*GetDocument().body());
