...
void DocumentStatisticsCollectorTest::SetHtmlInnerHTML(const String& html_content)
{
...
TEST_F(DocumentStatisticsCollectorTest, HasOpenGraphArticle)
{
    SetHtmlInnerHTML("<head>"
                     ...
                     "</head>");
    WebDistillabilityFeatures features = DocumentStatisticsCollector::CollectStatistics(GetDocument());
...
{
    SetHtmlInnerHTML("<p class='menu' id='article'>1</p>" // textContentLength = 1
    ...
    EXPECT_DOUBLE_EQ(features.moz_score_all_sqrt, 1 + sqrt(144) + sqrt(9) + sqrt(12));
...
TEST_F(DocumentStatisticsCollectorTest, CountScoreSaturation)
{
    ...
    double error = 1e-5;
    EXPECT_NEAR(features.moz_score, 6 * sqrt(kTextContentLengthSaturation - kParagraphLengthThreshold), error);
    EXPECT_NEAR(features.moz_score_all_sqrt, 6 * sqrt(kTextContentLengthSaturation), error);
    EXPECT_NEAR(features.moz_score_all_linear, 6 * kTextContentLengthSaturation, error);
}
