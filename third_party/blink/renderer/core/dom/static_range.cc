...
StaticRange::StaticRange(Document& document, Node* start_container, unsigned start_offset, Node* end_container, unsigned end_offset)
...
{
    DCHECK(!range.IsNull());
    return MakeGarbageCollected<StaticRange>(range.GetDocument(), range.StartPosition().ComputeContainerNode(),
        range.StartPosition().ComputeOffsetInContainerNode(), range.EndPosition().ComputeContainerNode(), range.EndPosition().ComputeOffsetInContainerNode());
}

StaticRange* StaticRange::Create(Document& document, const StaticRangeInit* static_range_init, ExceptionState& exception_state)
{
    DCHECK(static_range_init);

    if (static_range_init->startContainer()->IsDocumentTypeNode() || static_range_init->startContainer()->IsAttributeNode()
        || static_range_init->endContainer()->IsDocumentTypeNode() || static_range_init->endContainer()->IsAttributeNode()) {
    ...
    return MakeGarbageCollected<StaticRange>(
        document, static_range_init->startContainer(), static_range_init->startOffset(), static_range_init->endContainer(), static_range_init->endOffset());
}
...
bool StaticRange::IsValid() const
{
    ...
    is_valid_ = start_offset_ <= AbstractRange::LengthOfContents(start_container_) && end_offset_ <= AbstractRange::LengthOfContents(end_container_)
        && !HasDifferentRootContainer(start_container_, end_container_)
        && ComparePositionsInDOMTree(start_container_, start_offset_, end_container_, end_offset_) <= 0;
