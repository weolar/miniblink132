...
TEST_F(RangeTest, extractContentsWithDOMMutationEvent)
{
    ...
    script_element->setTextContent("let count = 0;"
    ...
    Element* const span_element = GetDocument().QuerySelector(AtomicString("span"));
    auto* const range = MakeGarbageCollected<Range>(GetDocument(), span_element, 0, span_element, 1);
    ...
    EXPECT_EQ("<b>abc</b>", result->innerHTML()) << "DOM mutation event handler should not affect result.";
    EXPECT_EQ("<span>DEF</span>", span_element->outerHTML()) << "DOM mutation event handler should be executed.";
}
...
TEST_F(RangeTest, SplitTextNodeRangeWithinText)
{
    ...
    auto* range04 = MakeGarbageCollected<Range>(GetDocument(), old_text, 0, old_text, 4);
    auto* range02 = MakeGarbageCollected<Range>(GetDocument(), old_text, 0, old_text, 2);
    auto* range22 = MakeGarbageCollected<Range>(GetDocument(), old_text, 2, old_text, 2);
    auto* range24 = MakeGarbageCollected<Range>(GetDocument(), old_text, 2, old_text, 4);
...
TEST_F(RangeTest, SplitTextNodeRangeOutsideText)
{
    ...
    GetDocument().body()->setInnerHTML("<span id=\"outer\">0<span id=\"inner-left\">1</span>SPLITME<span "
                                       "id=\"inner-right\">2</span>3</span>");

    Element* outer = GetDocument().getElementById(AtomicString::FromUTF8("outer"));
    Element* inner_left = GetDocument().getElementById(AtomicString::FromUTF8("inner-left"));
    Element* inner_right = GetDocument().getElementById(AtomicString::FromUTF8("inner-right"));
    auto* old_text = To<Text>(outer->childNodes()->item(2));

    auto* range_outer_outside = MakeGarbageCollected<Range>(GetDocument(), outer, 0, outer, 5);
    auto* range_outer_inside = MakeGarbageCollected<Range>(GetDocument(), outer, 1, outer, 4);
    auto* range_outer_surrounding_text = MakeGarbageCollected<Range>(GetDocument(), outer, 2, outer, 3);
    auto* range_inner_left = MakeGarbageCollected<Range>(GetDocument(), inner_left, 0, inner_left, 1);
    auto* range_inner_right = MakeGarbageCollected<Range>(GetDocument(), inner_right, 0, inner_right, 1);
    auto* range_from_text_to_middle_of_element = MakeGarbageCollected<Range>(GetDocument(), old_text, 6, outer, 3);
    ...
    EXPECT_EQ(6u,
        range_outer_outside->endOffset()); // Increased by 1 since a new node is inserted.
...
TEST_F(RangeTest, updateOwnerDocumentIfNeeded)
{
    ...
    auto* range = MakeGarbageCollected<Range>(GetDocument(), Position(bar, 0), Position(foo, 1));

    ScopedNullExecutionContext execution_context;
    auto* another_document = Document::CreateForTest(execution_context.GetExecutionContext());
...
{
    GetDocument().body()->setInnerHTML("<div><span id=span1>foo</span>bar<span id=span2>baz</span></div>");
...
{
    GetDocument().body()->setInnerHTML("<div><span id=span1>foofoo</span>bar<span id=span2>baz</span></div>");
...
{
    LoadAhem();
    GetDocument().body()->setInnerHTML("<div style='font: Ahem; width: 2em;letter-spacing: 5px;'>xx xx </div>");
    Node* const div = GetDocument().QuerySelector(AtomicString("div"));
    // "x^x
    //  x|x "
    auto* const range = MakeGarbageCollected<Range>(GetDocument(), div->firstChild(), 1, div->firstChild(), 4);
...
static Vector<gfx::QuadF> GetBorderAndTextQuads(const Position& start, const Position& end)
{
    DCHECK_LE(start, end);
    auto* const range = MakeGarbageCollected<Range>(*start.GetDocument(), start, end);
    Vector<gfx::QuadF> quads;
    range->GetBorderAndTextQuads(quads);
    return quads;
}
...
{
    LoadAhem();
    InsertStyleElement("body { font: 20px/25px Ahem; margin: 0px; }"
    ...
    EXPECT_THAT(GetBorderAndTextQuads(Position(text1, 0), Position(text1, 1)), ElementsAre(gfx::QuadF(gfx::RectF(3, 0, 20, 20))));
    EXPECT_THAT(GetBorderAndTextQuads(Position(text2, 0), Position(text2, 2)), ElementsAre(gfx::QuadF(gfx::RectF(2, 20, 22, 20))));
    EXPECT_THAT(GetBorderAndTextQuads(Position(text3, 0), Position(text3, 3)), ElementsAre(gfx::QuadF(gfx::RectF(2, 40, 22, 20))));
    EXPECT_THAT(GetBorderAndTextQuads(Position(text4, 0), Position(text4, 4)), ElementsAre(gfx::QuadF(gfx::RectF(2, 60, 22, 20))));
}
...
TEST_F(RangeTest, GetBorderAndTextQuadsWithFirstLetterOne)
{
    ...
    Element* const expected = GetDocument().getElementById(AtomicString("expected"));
    Element* const sample = GetDocument().getElementById(AtomicString("sample"));

    const Vector<gfx::QuadF> expected_quads = GetBorderAndTextQuads(Position(expected, 0), Position(expected, 2));
    const Vector<gfx::QuadF> sample_quads = GetBorderAndTextQuads(Position(sample, 0), Position(sample, 1));
    ASSERT_EQ(2u, sample_quads.size());
    ASSERT_EQ(3u, expected_quads.size()) << "expected_quads has SPAN, SPAN.firstChild and P.lastChild";
    EXPECT_EQ(gfx::ToEnclosingRect(expected_quads[0].BoundingBox()).size(), gfx::ToEnclosingRect(sample_quads[0].BoundingBox()).size())
        << "Check size of first-letter part";
    EXPECT_EQ(gfx::ToEnclosingRect(expected_quads[2].BoundingBox()).size(), gfx::ToEnclosingRect(sample_quads[1].BoundingBox()).size())
        << "Check size of first-letter part";

    EXPECT_EQ(ComputeSizesOfQuads(GetBorderAndTextQuads(Position(expected->firstChild(), 0), Position(expected->firstChild(), 1))),
        ComputeSizesOfQuads(GetBorderAndTextQuads(Position(sample->firstChild(), 0), Position(sample->firstChild(), 1))))
        << "All first-letter part";

    EXPECT_EQ(ComputeSizesOfQuads(GetBorderAndTextQuads(Position(expected->lastChild(), 0), Position(expected->lastChild(), 2))),
        ComputeSizesOfQuads(GetBorderAndTextQuads(Position(sample->firstChild(), 1), Position(sample->firstChild(), 3))))
        << "All remaining part";
}
...
TEST_F(RangeTest, GetBorderAndTextQuadsWithFirstLetterThree)
{
    ...
    EXPECT_EQ(ComputeSizesOfQuads(GetBorderAndTextQuads(Position(expected->firstChild(), 0), Position(expected->firstChild(), 1))),
        ComputeSizesOfQuads(GetBorderAndTextQuads(Position(sample->firstChild(), 0), Position(sample->firstChild(), 3))))
        << "All first-letter part";

    EXPECT_EQ(ComputeSizesOfQuads(GetBorderAndTextQuads(Position(expected->lastChild(), 0), Position(expected->lastChild(), 2))),
        ComputeSizesOfQuads(GetBorderAndTextQuads(Position(sample->firstChild(), 3), Position(sample->firstChild(), 5))))
        << "All remaining part";

    EXPECT_EQ(ComputeSizesOfQuads(GetBorderAndTextQuads(Position(expected->firstChild()->firstChild(), 1), Position(expected->firstChild()->firstChild(), 2))),
        ComputeSizesOfQuads(GetBorderAndTextQuads(Position(sample->firstChild(), 1), Position(sample->firstChild(), 2))))
        << "Partial first-letter part";

    EXPECT_EQ(ComputeSizesOfQuads(GetBorderAndTextQuads(Position(expected->firstChild()->firstChild(), 1), Position(expected->lastChild(), 1))),
        ComputeSizesOfQuads(GetBorderAndTextQuads(Position(sample->firstChild(), 1), Position(sample->firstChild(), 4))))
...
TEST_F(RangeTest, CollapsedRangeGetBorderAndTextQuadsWithFirstLetter)
{
    ...
    EXPECT_EQ(ComputeSizesOfQuads(GetBorderAndTextQuads(Position(expected->firstChild()->firstChild(), 0), Position(expected->firstChild()->firstChild(), 0))),
        ComputeSizesOfQuads(GetBorderAndTextQuads(Position(sample->firstChild(), 0), Position(sample->firstChild(), 0))))
        << "Collapsed range before first-letter part";

    EXPECT_EQ(ComputeSizesOfQuads(GetBorderAndTextQuads(Position(expected->firstChild()->firstChild(), 1), Position(expected->firstChild()->firstChild(), 1))),
        ComputeSizesOfQuads(GetBorderAndTextQuads(Position(sample->firstChild(), 1), Position(sample->firstChild(), 1))))
        << "Collapsed range after first-letter part";

    EXPECT_EQ(
        ComputeSizesOfQuads(GetBorderAndTextQuads(Position(expected->firstChild()->nextSibling(), 1), Position(expected->firstChild()->nextSibling(), 1))),
        ComputeSizesOfQuads(GetBorderAndTextQuads(Position(sample->firstChild(), 2), Position(sample->firstChild(), 2))))
...
TEST_F(RangeTest, ContainerNodeRemoval)
{
    ...
    auto* rangea0a2 = MakeGarbageCollected<Range>(GetDocument(), text_a, 0, text_a, 2);
    auto* rangea2a4 = MakeGarbageCollected<Range>(GetDocument(), text_a, 2, text_a, 4);
    auto* rangea2b2 = MakeGarbageCollected<Range>(GetDocument(), text_a, 0, text_b, 2);
    auto* rangeb2b6 = MakeGarbageCollected<Range>(GetDocument(), text_b, 2, text_b, 6);
...
TEST_F(RangeTest, ContainerNodeRemovalWithSequentialFocusNavigationStartingPoint)
{
    SetBodyContent("<input value='text inside input'>");
    const auto& input = ToTextControl(*GetDocument().QuerySelector(AtomicString("input")));
    ...
    Range* sequential_focus_navigation_starting_point = GetDocument().sequential_focus_navigation_starting_point_;

    EXPECT_TRUE(sequential_focus_navigation_starting_point->BoundaryPointsValid());
    EXPECT_EQ(GetDocument().body(), sequential_focus_navigation_starting_point->startContainer());
    EXPECT_EQ(0u, sequential_focus_navigation_starting_point->startOffset());
    EXPECT_EQ(GetDocument().body(), sequential_focus_navigation_starting_point->endContainer());
