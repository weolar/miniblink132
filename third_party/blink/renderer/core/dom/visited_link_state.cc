...
static inline const SecurityOrigin* CalculateFrameOrigin(const Document& document)
{
...
static inline LinkHash UnpartitionedLinkHashForElement(const Element& element, const AtomicString& attribute)
{
    ...
    return VisitedLinkHash(element.GetDocument().BaseURL(), attribute.IsNull() ? LinkAttribute(element) : attribute);
}

static inline LinkHash PartitionedLinkHashForElement(const Element& element, const AtomicString& attribute)
{
    // TODO(crbug.com/369219144): Should this be DynamicTo<HTMLAnchorElementBase>?
    if (auto* anchor = DynamicTo<HTMLAnchorElement>(element)) {
    ...
    const AtomicString relative_link_url = attribute.IsNull() ? LinkAttribute(element) : attribute;
    // (2) Top-Level Site.
    // NOTE: for all Documents which have a valid VisitedLinkState, we should not
    // ever encounter an invalid GetFrame() or an invalid TopFrameOrigin().
    DCHECK(element.GetDocument().TopFrameOrigin());
    const net::SchemefulSite top_level_site(element.GetDocument().TopFrameOrigin()->ToUrlOrigin());
    // (3) Frame Origin.
    const SecurityOrigin* frame_origin = CalculateFrameOrigin(element.GetDocument());
    ...
    return PartitionedVisitedLinkFingerprint(base_link_url, relative_link_url, top_level_site, frame_origin);
}
...
static inline LinkHash LinkHashForElement(const Element& element, const AtomicString& attribute = AtomicString())
{
    ...
    return base::FeatureList::IsEnabled(blink::features::kPartitionVisitedLinkDatabase)
...
static void InvalidateStyleForAllLinksRecursively(Node& root_node, bool invalidate_visited_link_hashes)
{
    for (Node& node : NodeTraversal::StartsAt(root_node)) {
        ...
        if (ShadowRoot* root = node.GetShadowRoot()) {
            InvalidateStyleForAllLinksRecursively(*root, invalidate_visited_link_hashes);
        }
    }
}

void VisitedLinkState::InvalidateStyleForAllLinks(bool invalidate_visited_link_hashes)
{
    if (!links_checked_for_visited_state_.empty() && GetDocument().firstChild())
        InvalidateStyleForAllLinksRecursively(*GetDocument().firstChild(), invalidate_visited_link_hashes);
}

static void InvalidateStyleForLinkRecursively(Node& root_node, LinkHash link_hash)
{
    for (Node& node : NodeTraversal::StartsAt(root_node)) {
...
{
    if (links_checked_for_visited_state_.Contains(link_hash) && GetDocument().firstChild())
...
void VisitedLinkState::UpdateSalt(uint64_t visited_link_salt)
{
    ...
    Platform::Current()->AddOrUpdateVisitedLinkSalt(CalculateFrameOrigin(GetDocument())->ToUrlOrigin(), visited_link_salt);
}
...
EInsideLink VisitedLinkState::DetermineLinkStateSlowCase(const Element& element)
{
    ...
    static const bool are_partitioned_visited_links_enabled = base::FeatureList::IsEnabled(blink::features::kPartitionVisitedLinkDatabase)
        || base::FeatureList::IsEnabled(blink::features::kPartitionVisitedLinkDatabaseWithSelfLinks);
    ...
    if (are_partitioned_visited_links_enabled) {
        ...
        if (GetDocument().GetExecutionContext()->GetPolicyContainer()->GetPolicies().is_credentialless) {
        ...
        if (GetDocument().GetFrame()->IsInFencedFrameTree()) {
            UMA_HISTOGRAM_BOOLEAN("Blink.History.VisitedLinks.InFencedFrameTree", true);
            return EInsideLink::kNotInsideLink;
        }
        // Record in our histogram that we are not in or a child of a Fenced Frame.
        UMA_HISTOGRAM_BOOLEAN("Blink.History.VisitedLinks.InFencedFrameTree", false);
    }
    ...
    if (attribute.empty()) {
        base::UmaHistogramBoolean("Blink.History.VisitedLinks.IsLinkStyledAsVisited", true);
    ...
    base::UmaHistogramBoolean("Blink.History.VisitedLinks.IsLinkStyledAsVisited", false);
