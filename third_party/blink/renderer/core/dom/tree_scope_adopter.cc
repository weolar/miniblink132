...
void TreeScopeAdopter::MoveTreeToNewScope(Node& root) const
{
    ...
    bool is_document_unmodified_and_uninteracted = IsDocumentEligibleForFastAdoption(old_document);

    for (Node& node : NodeTraversal::InclusiveDescendantsOf(root)) {
        UpdateTreeScope(node);

        if (will_move_to_new_document) {
            MoveNodeToNewDocument(node, old_document, is_document_unmodified_and_uninteracted);
        ...
        if (ShadowRoot* shadow = element->GetShadowRoot()) {
            ...
            if (will_move_to_new_document) {
                MoveShadowTreeToNewDocument(*shadow, old_document, new_document, is_document_unmodified_and_uninteracted);
            }
        }
    }
}
...
void TreeScopeAdopter::MoveShadowTreeToNewDocument(
    ShadowRoot& shadow_root, Document& old_document, Document& new_document, bool is_document_unmodified_and_uninteracted) const
{
    DCHECK_NE(old_document, new_document);
    if (old_document.TemplateDocumentHost() != &new_document && new_document.TemplateDocumentHost() != &old_document) {
    ...
    MoveTreeToNewDocument(shadow_root, old_document, new_document, is_document_unmodified_and_uninteracted);
}

void TreeScopeAdopter::MoveTreeToNewDocument(Node& root, Document& old_document, Document& new_document, bool is_document_unmodified_and_uninteracted) const
{
    DCHECK_NE(old_document, new_document);
    for (Node& node : NodeTraversal::InclusiveDescendantsOf(root)) {
        MoveNodeToNewDocument(node, old_document, is_document_unmodified_and_uninteracted);
        ...
        if (HeapVector<Member<Attr>>* attrs = element->GetAttrNodeList()) {
            for (const auto& attr : *attrs) {
                MoveTreeToNewDocument(*attr, old_document, new_document, is_document_unmodified_and_uninteracted);
            }
        }

        if (ShadowRoot* shadow_root = element->GetShadowRoot()) {
            MoveShadowTreeToNewDocument(*shadow_root, old_document, new_document, is_document_unmodified_and_uninteracted);
        }
    }
}
...
static Document* g_old_document_did_move_to_new_document_was_called_with = nullptr;

void TreeScopeAdopter::EnsureDidMoveToNewDocumentWasCalled(Document& old_document)
{
...
inline void TreeScopeAdopter::MoveNodeToNewDocument(Node& node, Document& old_document, bool is_document_unmodified_and_uninteracted) const
{
    ...
    if (!is_document_unmodified_and_uninteracted) {
        ...
        if (auto* element = DynamicTo<Element>(node)) {
            if (old_document.HasExplicitlySetAttrElements()) {
                old_document.MoveElementExplicitlySetAttrElementsMapToNewDocument(element, new_document);
            }
            if (old_document.HasCachedAttrAssociatedElements()) {
                old_document.MoveElementCachedAttrAssociatedElementsMapToNewDocument(element, new_document);
            }
        }
    ...
    if (node.GetCustomElementState() == CustomElementState::kCustom) {
        CustomElement::EnqueueAdoptedCallback(To<Element>(node), old_document, new_document);
    }
...
inline bool TreeScopeAdopter::IsDocumentEligibleForFastAdoption(Document& old_document) const
{
    return !old_document.HasNodeIterators() && !old_document.HasRanges() && !old_document.HasAnyNodeWithEventListeners() && !old_document.HasMutationObservers()
        && !old_document.ShouldInvalidateNodeListCaches() && !old_document.HasExplicitlySetAttrElements() && !old_document.HasCachedAttrAssociatedElements();
}
