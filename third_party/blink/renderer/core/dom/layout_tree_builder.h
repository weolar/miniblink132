...
template <typename NodeType> class LayoutTreeBuilder {
    STACK_ALLOCATED();

protected:
    LayoutTreeBuilder(NodeType& node, Node::AttachContext& context, const ComputedStyle* style)
        : node_(&node)
        , context_(context)
        , style_(style)
    {
        DCHECK(!node.GetLayoutObject());
        DCHECK(node.GetDocument().InStyleRecalc() || node.GetDocument().GetStyleEngine().InScrollMarkersAttachment());
    ...
    LayoutObject* NextLayoutObject() const
    {
        if (!context_.next_sibling_valid) {
            context_.next_sibling = LayoutTreeBuilderTraversal::NextSiblingLayoutObject(*node_);
    ...
    {
        return layout_object && layout_object->IsAnonymous() && layout_object->IsInline();
    }
...
public:
    LayoutTreeBuilderForElement(Element&, Node::AttachContext&, const ComputedStyle*);
...
public:
    LayoutTreeBuilderForText(Text& text, Node::AttachContext& context, const ComputedStyle* style_from_parent)
...
private:
    const ComputedStyle* CreateInlineWrapperStyleForDisplayContentsIfNeeded() const;
    LayoutObject* CreateInlineWrapperForDisplayContentsIfNeeded(const ComputedStyle* wrapper_style) const;
};
