...
LayoutTreeBuilderForElement::LayoutTreeBuilderForElement(Element& element, Node::AttachContext& context, const ComputedStyle* style)
...
LayoutObject* LayoutTreeBuilderForElement::NextLayoutObject() const
{
    ...
    if (style_->IsRenderedInTopLayer(*node_)) {
        if (LayoutObject* next_in_top_layer = LayoutTreeBuilderTraversal::NextInTopLayer(*node_)) {
...
LayoutObject* LayoutTreeBuilderForElement::ParentLayoutObject() const
{
    ...
    if (node_->IsScrollMarkerGroupPseudoElement()) {
        Element* originating_element = To<PseudoElement>(node_)->UltimateOriginatingElement();
        ContainerNode* parent_element = LayoutTreeBuilderTraversal::LayoutParent(*originating_element);
...
void LayoutTreeBuilderForElement::CreateLayoutObject()
{
    ...
    if (style_->IsRenderedInTopLayer(*node_) && context_.parent && !context_.parent->CanHaveChildren() && node_->GetPseudoId() != kPseudoIdBackdrop) {
        return;
    }

    if (node_->IsPseudoElement() && !CanHaveGeneratedChildren(*parent_layout_object))
        return;
    ...
    // fail.
    new_layout_object->SetIsInsideFlowThread(parent_layout_object->IsInsideFlowThread());
...
const ComputedStyle* LayoutTreeBuilderForText::CreateInlineWrapperStyleForDisplayContentsIfNeeded() const
{
    ...
    return node_->GetDocument().GetStyleResolver().CreateInheritedDisplayContentsStyleIfNeeded(*style_, context_.parent->StyleRef());
}

LayoutObject* LayoutTreeBuilderForText::CreateInlineWrapperForDisplayContentsIfNeeded(const ComputedStyle* wrapper_style) const
{
    if (!wrapper_style)
        return nullptr;
    ...
    LayoutObject* inline_wrapper = LayoutInline::CreateAnonymous(&node_->GetDocument());
...
void LayoutTreeBuilderForText::CreateLayoutObject()
{
    ...
    LayoutObject* next_layout_object = NextLayoutObject();
    const ComputedStyle* nullable_wrapper_style = CreateInlineWrapperStyleForDisplayContentsIfNeeded();
    if (LayoutObject* wrapper = CreateInlineWrapperForDisplayContentsIfNeeded(nullable_wrapper_style)) {
    ...
    // using IsInsideFlowThread() in the StyleWillChange and StyleDidChange will
    // fail.
    new_layout_object->SetIsInsideFlowThread(context_.parent->IsInsideFlowThread());
