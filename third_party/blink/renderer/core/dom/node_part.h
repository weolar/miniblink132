...
public:
    static NodePart* Create(PartRootUnion* root_union, Node* node, const PartInit* init, ExceptionState& exception_state);
    NodePart(PartRoot& root, Node& node, const PartInit* init)
        : NodePart(root, node, init && init->hasMetadata() ? init->metadata() : Vector<String>())
    {
    }
