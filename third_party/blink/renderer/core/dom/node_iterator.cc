...
NodeIterator::NodeIterator(Node* root_node, unsigned what_to_show, V8NodeFilter* filter)
...
Node* NodeIterator::nextNode(ExceptionState& exception_state)
{
    ...
    while (candidate_node_.MoveToNext(root())) {
        ...
        bool node_was_accepted = AcceptNode(provisional_result, exception_state) == V8NodeFilter::FILTER_ACCEPT;
...
void NodeIterator::UpdateForNodeRemoval(Node& removed_node, NodePointer& reference_node) const
{
    ...
    bool will_remove_reference_node_ancestor = reference_node.node && reference_node.node->IsDescendantOf(&removed_node);
