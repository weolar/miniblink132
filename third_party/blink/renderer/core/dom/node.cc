...
static_assert(sizeof(Node) <= sizeof(NotSmallerThanNode), "members of node should be reordered for better packing");

#if DUMP_NODE_STATISTICS
using WeakNodeSet = HeapHashSet<WeakMember<Node>>;
static WeakNodeSet& LiveNodeSet()
{
    DEFINE_STATIC_LOCAL(Persistent<WeakNodeSet>, set, (MakeGarbageCollected<WeakNodeSet>()));
    return *set;
}
...
void Node::DumpStatistics()
{
    ...
    for (const auto& entry : per_tag_count) {
        per_tag_stream << "  Number of <" << entry.key.Utf8().data() << "> tags: " << entry.value << "\n";
    }
    ...
    LOG(INFO) << "\n"
              ...
              << "  Number of Elements with attribute storage: " << elements_with_attribute_storage << " x " << sizeof(ElementData) << "Bytes\n"
              << "  Number of Elements with RareData: " << elements_with_rare_data << " x " << sizeof(ElementRareData) << "Bytes\n"
              << "  Number of Elements with NamedNodeMap: " << elements_with_named_node_map << " x " << sizeof(NamedNodeMap) << "Bytes";
}
#endif
...
Node* Node::PseudoAwarePreviousSibling() const
{
    ...
    case kPseudoIdViewTransition:
        if (Node* previous = parent->GetPseudoElement(kPseudoIdScrollMarkerGroupAfter)) {
            return previous;
        }
        [[fallthrough]];
    case kPseudoIdScrollNextButton:
        if (Node* next = parent->GetPseudoElement(kPseudoIdScrollMarkerGroupAfter)) {
            return next;
        }
        [[fallthrough]];
    ...
    case kPseudoIdMarker:
        if (Node* next = parent->GetPseudoElement(kPseudoIdScrollMarkerGroupBefore)) {
            return next;
        }
        [[fallthrough]];
    ...
    case kPseudoIdViewTransitionNew:
        ...
        return parent->GetPseudoElement(kPseudoIdViewTransitionOld, To<PseudoElement>(this)->view_transition_name());
    case kPseudoIdViewTransitionGroup: {
        const Vector<AtomicString>& names = GetDocument().GetStyleEngine().ViewTransitionTags();
        wtf_size_t found_index = names.Find(To<PseudoElement>(this)->view_transition_name());
        ...
        return parent->GetPseudoElement(kPseudoIdViewTransitionGroup, names[found_index - 1]);
    }
...
Node* Node::PseudoAwareNextSibling() const
{
    ...
    case kPseudoIdViewTransitionOld:
        CHECK_EQ(parent->GetPseudoId(), kPseudoIdViewTransitionImagePair);
        return parent->GetPseudoElement(kPseudoIdViewTransitionNew, To<PseudoElement>(this)->view_transition_name());
    case kPseudoIdViewTransitionGroup: {
        const Vector<AtomicString>& names = GetDocument().GetStyleEngine().ViewTransitionTags();
        wtf_size_t found_index = names.Find(To<PseudoElement>(this)->view_transition_name());
        CHECK_NE(found_index, kNotFound);
...
Node* Node::PseudoAwareFirstChild() const
{
    if (const auto* current_element = DynamicTo<Element>(this)) {
        ...
        if (GetPseudoId() == kPseudoIdViewTransition) {
            ...
            return current_element->GetPseudoElement(kPseudoIdViewTransitionGroup, names.front());
        }
        if (GetPseudoId() == kPseudoIdViewTransitionGroup) {
            return current_element->GetPseudoElement(kPseudoIdViewTransitionImagePair, To<PseudoElement>(this)->view_transition_name());
        }
        if (GetPseudoId() == kPseudoIdViewTransitionImagePair) {
            const AtomicString& name = To<PseudoElement>(this)->view_transition_name();
            if (Node* first = current_element->GetPseudoElement(kPseudoIdViewTransitionOld, name)) {
                return first;
            }

            return current_element->GetPseudoElement(kPseudoIdViewTransitionNew, name);
        }
        if (Node* first = current_element->GetPseudoElement(kPseudoIdScrollPrevButton)) {
            return first;
        }
        if (Node* first = current_element->GetPseudoElement(kPseudoIdScrollMarkerGroupBefore)) {
            return first;
        }
        ...
        if (Node* first = current_element->GetPseudoElement(kPseudoIdScrollMarkerGroupAfter)) {
            return first;
        }
        if (Node* first = current_element->GetPseudoElement(kPseudoIdScrollNextButton)) {
            return first;
        }
...
Node* Node::PseudoAwareLastChild() const
{
    if (const auto* current_element = DynamicTo<Element>(this)) {
        ...
        if (GetPseudoId() == kPseudoIdViewTransition) {
            ...
            return current_element->GetPseudoElement(kPseudoIdViewTransitionGroup, names.back());
        }
        if (GetPseudoId() == kPseudoIdViewTransitionGroup) {
            return current_element->GetPseudoElement(kPseudoIdViewTransitionImagePair, To<PseudoElement>(this)->view_transition_name());
        }
        if (GetPseudoId() == kPseudoIdViewTransitionImagePair) {
            const AtomicString& name = To<PseudoElement>(this)->view_transition_name();
            if (Node* last = current_element->GetPseudoElement(kPseudoIdViewTransitionNew, name)) {
                return last;
            }

            return current_element->GetPseudoElement(kPseudoIdViewTransitionOld, name);
        }
        if (Node* last = current_element->GetPseudoElement(kPseudoIdViewTransition)) {
            return last;
        }
        if (Node* last = current_element->GetPseudoElement(kPseudoIdScrollNextButton)) {
            return last;
        }
        if (Node* last = current_element->GetPseudoElement(kPseudoIdScrollMarkerGroupAfter)) {
            return last;
        }
        ...
        if (Node* last = current_element->GetPseudoElement(kPseudoIdScrollMarkerGroupBefore)) {
            return last;
        }
...
Node* Node::insertBefore(Node* new_child, Node* ref_child, ExceptionState& exception_state)
{
    ...
    exception_state.ThrowDOMException(DOMExceptionCode::kHierarchyRequestError, "This node type does not support this method.");
    return nullptr;
}
...
Node* Node::moveBefore(Node* new_child, Node* ref_child, ExceptionState& exception_state)
{
    DCHECK(new_child);
...
Node* Node::removeChild(Node* old_child, ExceptionState& exception_state)
{
    ...
    exception_state.ThrowDOMException(DOMExceptionCode::kNotFoundError, "This node type does not support this method.");
    return nullptr;
}
...
static bool IsNodeInNodes(const Node* const node, const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes)
{
...
static Node* FindViablePreviousSibling(const Node& node, const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes)
{
...
static Node* FindViableNextSibling(const Node& node, const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes)
{
...
static Node* NodeOrStringToNode(const V8UnionNodeOrStringOrTrustedScript* node_or_string, Document& document, bool needs_trusted_types_check,
    const char* property_name, ExceptionState& exception_state)
{
    if (!needs_trusted_types_check) {
        ...
        case V8UnionNodeOrStringOrTrustedScript::ContentType::kTrustedScript:
            return Text::Create(document, node_or_string->GetAsTrustedScript()->toString());
        }
        NOTREACHED();
    }
    ...
    String string_value = node_or_string->IsString() ? node_or_string->GetAsString() : node_or_string->GetAsNode()->textContent();

    string_value = TrustedTypesCheckForScript(string_value, document.GetExecutionContext(), "Node", property_name, exception_state);
...
// static
Node* Node::ConvertNodeUnionsIntoNode(const ContainerNode* parent, const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& node_unions,
    Document& document, const char* property_name, ExceptionState& exception_state)
{
    DCHECK(!RuntimeEnabledFeatures::SkipTemporaryDocumentFragmentEnabled());

    bool needs_check = IsA<HTMLScriptElement>(parent) && document.GetExecutionContext() && document.GetExecutionContext()->RequireTrustedTypes();
    VectorOf<Node> nodes;
    for (const auto& node_union : node_unions) {
        Node* node = NodeOrStringToNode(node_union, document, needs_check, property_name, exception_state);
        if (exception_state.HadException()) {
            return nullptr;
        }
        if (node) {
...
// Returns nullptr if an exception was thrown.
// static
VectorOf<Node> Node::ConvertNodeUnionsIntoNodes(const ContainerNode* parent, const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& node_unions,
    Document& document, const char* property_name, ExceptionState& exception_state)
{
    DCHECK(RuntimeEnabledFeatures::SkipTemporaryDocumentFragmentEnabled());
...
void Node::prepend(const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes, ExceptionState& exception_state)
{
    auto* this_node = DynamicTo<ContainerNode>(this);
    if (!this_node) {
        exception_state.ThrowDOMException(DOMExceptionCode::kHierarchyRequestError, "This node type does not support this method.");
    ...
    if (RuntimeEnabledFeatures::SkipTemporaryDocumentFragmentEnabled()) {
        VectorOf<Node> node_vector = ConvertNodeUnionsIntoNodes(this_node, nodes, GetDocument(), "prepend", exception_state);
        if (exception_state.HadException()) {
            return;
        }
        this_node->InsertBefore(node_vector, this_node->firstChild(), exception_state);
    } else {
        if (Node* node = ConvertNodeUnionsIntoNode(this_node, nodes, GetDocument(), "prepend", exception_state)) {
            this_node->InsertBefore(node, this_node->firstChild(), exception_state);
        }
    }
}

void Node::append(const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes, ExceptionState& exception_state)
{
    auto* this_node = DynamicTo<ContainerNode>(this);
    if (!this_node) {
        exception_state.ThrowDOMException(DOMExceptionCode::kHierarchyRequestError, "This node type does not support this method.");
    ...
    if (RuntimeEnabledFeatures::SkipTemporaryDocumentFragmentEnabled()) {
        VectorOf<Node> node_vector = ConvertNodeUnionsIntoNodes(this_node, nodes, GetDocument(), "append", exception_state);
        if (exception_state.HadException()) {
            return;
        }
        this_node->AppendChildren(node_vector, exception_state);
    } else {
        if (Node* node = ConvertNodeUnionsIntoNode(this_node, nodes, GetDocument(), "append", exception_state)) {
            this_node->AppendChild(node, exception_state);
        }
    }
}

void Node::before(const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes, ExceptionState& exception_state)
{
    ...
    if (RuntimeEnabledFeatures::SkipTemporaryDocumentFragmentEnabled()) {
        VectorOf<Node> node_vector = ConvertNodeUnionsIntoNodes(parent, nodes, GetDocument(), "before", exception_state);
        if (exception_state.HadException()) {
            return;
        }
        parent->InsertBefore(node_vector, viable_previous_sibling ? viable_previous_sibling->nextSibling() : parent->firstChild(), exception_state);
    } else {
        if (Node* node = ConvertNodeUnionsIntoNode(parent, nodes, GetDocument(), "before", exception_state)) {
            parent->InsertBefore(node, viable_previous_sibling ? viable_previous_sibling->nextSibling() : parent->firstChild(), exception_state);
        }
    }
}

void Node::after(const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes, ExceptionState& exception_state)
{
    ContainerNode* parent = parentNode();
    if (!parent)
        return;
    ...
    if (RuntimeEnabledFeatures::SkipTemporaryDocumentFragmentEnabled()) {
        VectorOf<Node> node_vector = ConvertNodeUnionsIntoNodes(parent, nodes, GetDocument(), "after", exception_state);
        if (exception_state.HadException()) {
            return;
        }
        parent->InsertBefore(node_vector, viable_next_sibling, exception_state);
    } else {
        if (Node* node = ConvertNodeUnionsIntoNode(parent, nodes, GetDocument(), "after", exception_state)) {
            parent->InsertBefore(node, viable_next_sibling, exception_state);
        }
    }
}

void Node::replaceWith(const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes, ExceptionState& exception_state)
{
    ContainerNode* parent = parentNode();
    if (!parent)
        return;
    Node* viable_next_sibling = FindViableNextSibling(*this, nodes);
    if (RuntimeEnabledFeatures::SkipTemporaryDocumentFragmentEnabled()) {
        VectorOf<Node> node_vector = ConvertNodeUnionsIntoNodes(parent, nodes, GetDocument(), "replaceWith", exception_state);
        if (exception_state.HadException()) {
            return;
        }
        ...
        Node* node = ConvertNodeUnionsIntoNode(parent, nodes, GetDocument(), "replaceWith", exception_state);
        if (exception_state.HadException()) {
            return;
        }
        if (parent == parentNode()) {
...
void Node::replaceChildren(const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& node_unions, ExceptionState& exception_state)
{
    auto* this_node = DynamicTo<ContainerNode>(this);
    if (!this_node) {
        exception_state.ThrowDOMException(DOMExceptionCode::kHierarchyRequestError, "This node type does not support this method.");
    ...
    if (RuntimeEnabledFeatures::SkipTemporaryDocumentFragmentEnabled()) {
        VectorOf<Node> nodes = ConvertNodeUnionsIntoNodes(this_node, node_unions, GetDocument(), "replace", exception_state);
        if (exception_state.HadException()) {
            return;
        }
        this_node->ReplaceChildren(nodes, exception_state);
    } else {
        Node* node = ConvertNodeUnionsIntoNode(this_node, node_unions, GetDocument(), "replace", exception_state);
        if (!exception_state.HadException()) {
...
Node* Node::cloneNode(bool deep, ExceptionState& exception_state) const
{
    ...
    if (IsShadowRoot()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "ShadowRoot nodes are not clonable.");
        return nullptr;
    }
...
bool Node::IsClosedShadowHiddenFrom(const Node& other) const
{
    ...
    for (TreeScope* other_scope = &other.GetTreeScope(); other_scope; other_scope = other_scope->ParentTreeScope()) {
...
void Node::MarkSubtreeNeedsStyleRecalcForFontUpdates()
{
    ...
    if (auto* element = DynamicTo<Element>(this)) {
        ...
        if (!NeedsStyleRecalc()) {
            if (style->DependsOnFontMetrics() || element->PseudoElementStylesDependOnFontMetrics()) {
                SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kFonts));
            }
        }
...
void Node::MarkAncestorsWithChildNeedsStyleRecalc()
{
    ...
    if (GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount() > 0) {
        for (Element* ancestor_copy = ancestor; ancestor_copy; ancestor_copy = ancestor_copy->GetStyleRecalcParent()) {
...
void Node::MarkAncestorsWithChildNeedsReattachLayoutTree()
{
    ...
    DCHECK(!ancestor || !ChildNeedsReattachLayoutTree() || !ancestor->ChildNeedsReattachLayoutTree() || NeedsReattachLayoutTree())
        << "If both this and the parent are already marked with "
           ...
           "ancestors.";
    for (; ancestor && !ancestor->ChildNeedsReattachLayoutTree(); ancestor = ancestor->GetReattachParent()) {
    ...
    if (GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount() > 0) {
        for (Element* ancestor_copy = ancestor; ancestor_copy; ancestor_copy = ancestor_copy->GetReattachParent()) {
            if (ancestor_copy->ChildStyleRecalcBlockedByDisplayLock())
                return;
        }
    }
...
void Node::SetNeedsStyleRecalc(StyleChangeType change_type, const StyleChangeReasonForTracing& reason)
{
    ...
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT_WITH_CATEGORIES(TRACE_DISABLED_BY_DEFAULT("devtools.timeline.invalidationTracking"),
        "StyleRecalcInvalidationTracking", inspector_style_recalc_invalidation_tracking_event::Data, this, change_type, reason);
    ...
    if (auto* this_element = DynamicTo<Element>(this)) {
        ...
        if (this_element->IsDocumentElement()) {
            ...
            ViewTransitionUtils::ForEachTransitionPseudo(GetDocument(), update_style_change);
        }
    }
...
void Node::FocusabilityLost()
{
    if (IsA<HTMLFormElement>(this) || IsA<HTMLFormControlElement>(this)) {
        GetDocument().DidChangeFormRelatedElementDynamically(DynamicTo<HTMLElement>(this), WebFormRelatedChangeType::kHide);
    }
}
...
{
    if (FlatTreeNodeData* data = GetFlatTreeNodeData()) {
        if (data->AssignedSlot() && data->AssignedSlot()->OwnerShadowHost() != &parent) {
            data->Clear();
        }
    }
}
...
{
    DCHECK(GetDocument().InStyleRecalc() || IsDocumentNode() || GetDocument().GetStyleEngine().InScrollMarkersAttachment());
    DCHECK(!GetDocument().Lifecycle().InDetach());
    DCHECK(!context.performing_reattach || GetDocument().GetStyleEngine().InRebuildLayoutTree());

    LayoutObject* layout_object = GetLayoutObject();
    DCHECK(!layout_object || (layout_object->Style() && (layout_object->Parent() || IsA<LayoutView>(layout_object))));
...
void Node::DetachLayoutTree(bool performing_reattach)
{
    ...
    // DetachLayoutTree().
    DCHECK(GetDocument().Lifecycle().StateAllowsDetach() || GetDocument().GetStyleEngine().InContainerQueryStyleRecalc()
        || GetDocument().GetStyleEngine().InScrollMarkersAttachment()
        || (GetDocument().GetStyleEngine().InPositionTryStyleRecalc() && IsPseudoElement() && !GetLayoutObject()));
    DCHECK(!performing_reattach || GetDocument().GetStyleEngine().InRebuildLayoutTree() || GetDocument().GetStyleEngine().InScrollMarkersAttachment());
...
{
    if (const auto* layout_object = GetLayoutObject()) {
        return layout_object->WhitespaceChildrenMayChange() || layout_object->WasNotifiedOfSubtreeChange();
    }
    return false;
}
...
bool Node::CanStartSelection() const
{
    if (DisplayLockUtilities::LockedAncestorPreventingPaint(*this)) {
        if (const Element* element = FlatTreeTraversal::InclusiveParentElement(*this)) {
            GetDocument().UpdateStyleAndLayoutTreeForElement(element, DocumentUpdateReason::kSelection);
        }
    }
    if (IsEditable(*this)) {
        return true;
    ...
    if (GetLayoutObject()) {
        ...
        if (style.UserDrag() != EUserDrag::kElement && (user_select == EUserSelect::kText || user_select == EUserSelect::kAll))
            return true;
    }
...
AtomicString Node::SlotName() const
{
    DCHECK(IsSlotable());
    if (IsElementNode()) {
        return HTMLSlotElement::NormalizeSlotName(To<Element>(*this).FastGetAttribute(html_names::kSlotAttr));
    }
    DCHECK(IsTextNode());
    return g_empty_atom;
}
...
bool Node::isDefaultNamespace(const AtomicString& namespace_uri_maybe_empty) const
{
    // https://dom.spec.whatwg.org/#dom-node-isdefaultnamespace

    // 1. If namespace is the empty string, then set it to null.
    const AtomicString& namespace_uri = namespace_uri_maybe_empty.empty() ? g_null_atom : namespace_uri_maybe_empty;
...
const AtomicString& Node::lookupPrefix(const AtomicString& namespace_uri) const
{
...
const AtomicString& Node::lookupNamespaceURI(const String& specified_prefix) const
{
    // Implemented according to
...
String Node::textContent(bool convert_brs_to_newlines, TextVisitor* visitor, unsigned int max_length) const
{
...
void Node::setTextContentForBinding(const V8UnionStringOrTrustedScript* value, ExceptionState& exception_state)
{
    if (!value)
...
uint16_t Node::compareDocumentPosition(const Node* other_node, ShadowTreesTreatment treatment) const
{
    if (other_node == this)
    ...
    if (!start1 || !start2) {
        uint16_t direction = (this > other_node) ? kDocumentPositionPreceding : kDocumentPositionFollowing;
    ...
    if (start1->isConnected() != start2->isConnected() || (treatment == kTreatShadowTreesAsDisconnected && start1->GetTreeScope() != start2->GetTreeScope())) {
        uint16_t direction = (this > other_node) ? kDocumentPositionPreceding : kDocumentPositionFollowing;
        return kDocumentPositionDisconnected | kDocumentPositionImplementationSpecific | direction;
    }
    ...
    unsigned connection = start1->GetTreeScope() != start2->GetTreeScope() ? kDocumentPositionDisconnected | kDocumentPositionImplementationSpecific : 0;
    ...
    for (unsigned i = std::min(index1, index2); i; --i) {
        ...
        if (child1 != child2) {
            ...
            for (const Node* child = child2->PseudoAwarePreviousSibling(); child; child = child->PseudoAwarePreviousSibling()) {
                if (child == child1)
                    return kDocumentPositionFollowing | connection;
            }
            return kDocumentPositionPreceding | connection;
    ...
    return index1 < index2 ? kDocumentPositionFollowing | kDocumentPositionContainedBy | connection
...
Node::InsertionNotificationRequest Node::InsertedInto(ContainerNode& insertion_point)
{
    ...
    DCHECK(insertion_point.isConnected() || insertion_point.IsInShadowTree() || IsContainerNode() || GetDOMParts());
...
String Node::DebugName() const
{
    ...
    if (const auto* vt_pseudo = DynamicTo<ViewTransitionPseudoElementBase>(this)) {
        name.Append("(");
...
static void DumpAttributeDesc(const Node& node, const QualifiedName& name, StringBuilder& builder)
{
...
String Node::ToString() const
{
    ...
    } else if (const auto* vt_pseudo = DynamicTo<ViewTransitionPseudoElementBase>(this)) {
        builder.Append("(");
    ...
    for (unsigned index = chain.size(); index > 0; --index) {
        ...
        case kElementNode: {
            ...
            if (node->previousSibling() || node->nextSibling()) {
                ...
                if (has_id_attr)
                    stream << "[@id=\"" << idattr.Utf8() << "\" and position()=" << count << "]";
                else
                    stream << "[" << count << "]";
            } else if (has_id_attr) {
...
static void AppendMarkedTree(const String& base_indent, const Node* root_node, const Node* marked_node1, const char* marked_label1, const Node* marked_node2,
    const char* marked_label2, StringBuilder& builder)
{
    for (const Node& node : NodeTraversal::InclusiveDescendantsOf(*root_node)) {
        ...
        for (const Node* tmp_node = &node; tmp_node && tmp_node != root_node; tmp_node = tmp_node->ParentOrShadowHostNode())
            indent.Append('\t');
        ...
        if (const auto* element = DynamicTo<Element>(node)) {
            if (Element* pseudo = element->GetPseudoElement(kPseudoIdScrollMarkerGroupBefore)) {
                AppendMarkedTree(indent_string, pseudo, marked_node1, marked_label1, marked_node2, marked_label2, builder);
            }
            ...
            if (Element* pseudo = element->GetPseudoElement(kPseudoIdScrollMarkerGroupAfter)) {
                AppendMarkedTree(indent_string, pseudo, marked_node1, marked_label1, marked_node2, marked_label2, builder);
            }
        ...
        if (ShadowRoot* shadow_root = node.GetShadowRoot()) {
            AppendMarkedTree(indent_string, shadow_root, marked_node1, marked_label1, marked_node2, marked_label2, builder);
        }
    }
}

static void AppendMarkedFlatTree(const String& base_indent, const Node* root_node, const Node* marked_node1, const char* marked_label1,
    const Node* marked_node2, const char* marked_label2, StringBuilder& builder)
{
    for (const Node* node = root_node; node; node = FlatTreeTraversal::NextSibling(*node)) {
        StringBuilder indent;
        if (node == marked_node1)
            indent.Append(marked_label1);
        if (node == marked_node2)
        ...
        if (Node* child = FlatTreeTraversal::FirstChild(*node))
            AppendMarkedFlatTree(indent.ReleaseString(), child, marked_node1, marked_label1, marked_node2, marked_label2, builder);
    }
}

String Node::ToMarkedTreeString(const Node* marked_node1, const char* marked_label1, const Node* marked_node2, const char* marked_label2) const
{
    ...
    AppendMarkedTree(starting_indent, root_node, marked_node1, marked_label1, marked_node2, marked_label2, builder);
    return builder.ReleaseString();
}

String Node::ToMarkedFlatTreeString(const Node* marked_node1, const char* marked_label1, const Node* marked_node2, const char* marked_label2) const
{
    const Node* root_node;
    const Node* node = this;
    while (node->ParentOrShadowHostNode() && !IsA<HTMLBodyElement>(*node))
    ...
    StringBuilder builder;
    String starting_indent;
    AppendMarkedFlatTree(starting_indent, root_node, marked_node1, marked_label1, marked_node2, marked_label2, builder);
    return builder.ReleaseString();
}
...
static void PrintSubTreeAcrossFrame(const Node* node, const Node* marked_node, const String& indent, std::ostream& stream)
{
    if (node == marked_node)
        stream << "*";
    stream << indent.Utf8() << *node << "\n";
    if (auto* frame_owner_element = DynamicTo<HTMLFrameOwnerElement>(node)) {
        PrintSubTreeAcrossFrame(frame_owner_element->contentDocument(), marked_node, indent + "\t", stream);
    }
    if (ShadowRoot* shadow_root = node->GetShadowRoot())
...
void Node::WillMoveToNewDocument(Document& new_document)
{
    ...
    if (!old_document.GetPage() || old_document.GetPage() == new_document.GetPage())
        return;

    old_document.GetFrame()->GetEventHandlerRegistry().DidMoveOutOfPage(*this);

    if (auto* this_element = DynamicTo<Element>(this)) {
        StylePropertyMapReadOnly* computed_style_map_item = old_document.RemoveComputedStyleMapItem(this_element);
        if (computed_style_map_item) {
            new_document.AddComputedStyleMapItem(this_element, computed_style_map_item);
        }
    }
}
...
void Node::AddedEventListener(const AtomicString& event_type, RegisteredEventListener& registered_listener)
{
...
void Node::RemovedEventListener(const AtomicString& event_type, const RegisteredEventListener& registered_listener)
{
...
void Node::RemoveAllEventListeners()
{
    ...
    if (HasEventListeners()) {
        GetEventTargetData()->event_listener_map.ForAllEventListenerTypes(
            [&document](const AtomicString& event_type, uint32_t count) { document.DidRemoveEventListeners(count); });

        if (document.GetPage()) {
            document.GetFrame()->GetEventHandlerRegistry().DidRemoveAllEventHandlers(*this);
        }
    }
...
void Node::MoveEventListenersToNewDocument(Document& old_document, Document& new_document)
{
    DCHECK_EQ(&new_document, &GetDocument());
    if (const EventTargetData* event_target_data = GetEventTargetData()) {
        const EventListenerMap& listener_map = event_target_data->event_listener_map;
        if (!listener_map.IsEmpty()) {
            listener_map.ForAllEventListenerTypes([this, &old_document, &new_document](const AtomicString& event_type, uint32_t count) {
    ...
    if (new_document.GetPage() && new_document.GetPage() != old_document.GetPage()) {
        new_document.GetFrame()->GetEventHandlerRegistry().DidMoveIntoPage(*this);
    }
}

const HeapVector<Member<MutationObserverRegistration>>* Node::MutationObserverRegistry()
{
    if (!data_) {
        return nullptr;
    }
...
const HeapHashSet<Member<MutationObserverRegistration>>* Node::TransientMutationObserverRegistry()
{
    if (!data_) {
        return nullptr;
    }
...
{
    DCHECK_EQ(&new_document, &GetDocument());
    if (const HeapVector<Member<MutationObserverRegistration>>* registry = MutationObserverRegistry()) {
        for (const auto& registration : *registry) {
            new_document.AddMutationObserverTypes(registration->MutationTypes());
        }
    }

    if (const HeapHashSet<Member<MutationObserverRegistration>>* transient_registry = TransientMutationObserverRegistry()) {
...
static inline void CollectMatchingObserversForMutation(HeapHashMap<Member<MutationObserver>, MutationRecordDeliveryOptions>& observers, Registry* registry,
    Node& target, MutationType type, const QualifiedName* attribute_name)
{
    if (!registry)
        return;
    ...
    for (const auto& registration : *registry) {
        if (registration->ShouldReceiveMutationFrom(target, type, attribute_name)) {
            ...
            HeapHashMap<Member<MutationObserver>, MutationRecordDeliveryOptions>::AddResult result
...
void Node::GetRegisteredMutationObserversOfType(
    HeapHashMap<Member<MutationObserver>, MutationRecordDeliveryOptions>& observers, MutationType type, const QualifiedName* attribute_name)
{
    DCHECK((type == kMutationTypeAttributes && attribute_name) || !attribute_name);
    ...
    for (Node* node = parentNode(); node; node = node->parentNode()) {
        ...
        CollectMatchingObserversForMutation(observers, node->TransientMutationObserverRegistry(), *this, type, attribute_name);
    }
}

void Node::RegisterMutationObserver(MutationObserver& observer, MutationObserverOptions options, const HashSet<AtomicString>& attribute_filter)
{
...
{
    const HeapVector<Member<MutationObserverRegistration>>* registry = MutationObserverRegistry();
    DCHECK(registry);
    if (!registry)
        return;
...
void Node::NotifyMutationObserversNodeWillDetach()
{
    ...
    ScriptForbiddenScope forbid_script_during_raw_iteration;
    for (Node* node = parentNode(); node; node = node->parentNode()) {
        if (const HeapVector<Member<MutationObserverRegistration>>* registry = node->MutationObserverRegistry()) {
            for (const auto& registration : *registry)
                registration->ObservedSubtreeNodeWillDetach(*this);
        }

        if (const HeapHashSet<Member<MutationObserverRegistration>>* transient_registry = node->TransientMutationObserverRegistry()) {
...
void Node::DispatchSubtreeModifiedEvent()
{
    ...
    DispatchScopedEvent(*MutationEvent::Create(event_type_names::kDOMSubtreeModified, Event::Bubbles::kYes));
}

DispatchEventResult Node::DispatchDOMActivateEvent(int detail, Event& underlying_event)
{
#if DCHECK_IS_ON()
    DCHECK(!EventDispatchForbiddenScope::IsEventDispatchForbidden());
#endif
    UIEvent& event = *UIEvent::Create();
    event.initUIEvent(event_type_names::kDOMActivate, true, true, GetDocument().domWindow(), detail);
...
void Node::DispatchSimulatedClick(const Event* underlying_event, SimulatedClickCreationScope scope)
{
    if (auto* element = IsElementNode() ? To<Element>(this) : parentElement()) {
        element->ActivateDisplayLockIfNeeded(DisplayLockActivationReason::kSimulatedClick);
    }
...
void Node::DefaultEventHandler(Event& event)
{
    ...
    if (event_type == event_type_names::kKeydown || event_type == event_type_names::kKeypress || event_type == event_type_names::kKeyup) {
        ...
        if (DispatchDOMActivateEvent(detail, event) != DispatchEventResult::kNotCanceled)
            event.SetDefaultHandled();
    } else if (event_type == event_type_names::kContextmenu && IsA<MouseEvent>(event)) {
        if (Page* page = GetDocument().GetPage()) {
            page->GetContextMenuController().HandleContextMenuEvent(To<MouseEvent>(&event));
        }
    } else if (event_type == event_type_names::kTextInput) {
        if (event.HasInterface(event_interface_names::kTextEvent)) {
            if (LocalFrame* frame = GetDocument().GetFrame()) {
                frame->GetEventHandler().DefaultTextInputEventHandler(To<TextEvent>(&event));
            }
        }
    } else if (RuntimeEnabledFeatures::MiddleClickAutoscrollEnabled() && event_type == event_type_names::kMousedown && IsA<MouseEvent>(event)) {
        auto& mouse_event = To<MouseEvent>(event);
        if (mouse_event.button() == static_cast<int16_t>(WebPointerProperties::Button::kMiddle)) {
...
bool Node::WillRespondToMouseMoveEvents() const
{
    if (IsDisabledFormControl(this))
        return false;
    return HasEventListeners(event_type_names::kMousemove) || HasEventListeners(event_type_names::kMouseover) || HasEventListeners(event_type_names::kMouseout);
}
...
bool Node::WillRespondToMouseClickEvents()
{
    ...
    return IsEditable(*this) || HasAnyEventListeners(event_util::MouseButtonEventTypes());
}
...
HTMLSlotElement* Node::AssignedSlot() const
{
    ...
    if (FlatTreeNodeData* data = GetFlatTreeNodeData()) {
        ...
        if (root->IsNamedSlotting()) {
            DCHECK_EQ(root->AssignedSlotFor(*this), data->AssignedSlot()) << "Assigned slot mismatch for node " << this;
        }
#endif
        return data->AssignedSlot();
    }
...
void Node::SetCustomElementState(CustomElementState new_state)
{
    ...
    case CustomElementState::kCustom:
        DCHECK(old_state == CustomElementState::kUndefined || old_state == CustomElementState::kFailed || old_state == CustomElementState::kPreCustomized);
        break;
...
void Node::CheckSlotChange(SlotChangeType slot_change_type)
{
    ...
    if (ShadowRoot* root = ShadowRootOfParent()) {
        ...
        if (auto* parent_slot = DynamicTo<HTMLSlotElement>(parentElement())) {
            ...
            if (parent_slot->SupportsAssignment() && !parent_slot->HasAssignedNodesSlow())
...
{
    return GetLayoutObject() ? GetLayoutObject()->IsEffectiveRootScroller() : false;
}
...
{
    DCHECK(isConnected());
    const ComputedStyle* style = IsElementNode() ? To<Element>(this)->GetComputedStyle() : nullptr;
    bool detach = false;
    if (ShouldSkipMarkingStyleDirty()) {
        ...
        // flags are cleared.
        detach = IsDirtyForStyleRecalc() || ChildNeedsStyleRecalc() || style || GetLayoutObject();
    }
    if (!detach) {
    ...
    if (!ShouldSkipMarkingStyleDirty()) {
        if (NeedsStyleRecalc()) {
            ...
            SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kFlatTreeChange));
        }
...
HeapHashSet<Member<TreeScope>> Node::GetAncestorTreeScopes() const
{
    ...
    for (TreeScope* scope = &GetTreeScope(); scope; scope = scope->ParentTreeScope()) {
...
void Node::AddConsoleMessage(mojom::blink::ConsoleMessageSource source, mojom::blink::ConsoleMessageLevel level, const String& message)
{
    auto* console_message = MakeGarbageCollected<ConsoleMessage>(source, level, message);
