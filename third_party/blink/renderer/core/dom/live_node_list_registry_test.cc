...
protected:
    ...
    {
        return MakeGarbageCollected<NameNodeList>(GetDocument(), kNameNodeListType, g_empty_atom);
    }
};
...
TEST_F(LiveNodeListRegistryTest, Add)
{
    ...
    EXPECT_FALSE(registry.ContainsInvalidationType(kInvalidateOnIdNameAttrChange));
...
struct LiveNodeListRegistryWrapper final : public GarbageCollected<LiveNodeListRegistryWrapper> {
    LiveNodeListRegistry registry;
...
{
    auto wrapper = WrapPersistent(MakeGarbageCollected<LiveNodeListRegistryWrapper>());
