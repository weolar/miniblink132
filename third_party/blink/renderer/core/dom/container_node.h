...
enum class DynamicRestyleFlags {
    ...
    kChildrenAffectedByStructuralRules = kChildrenAffectedByFirstChildRules | kChildrenAffectedByLastChildRules | kChildrenAffectedByDirectAdjacentRules
        | kChildrenAffectedByIndirectAdjacentRules | kChildrenAffectedByForwardPositionalRules | kChildrenAffectedByBackwardPositionalRules
};

enum SubtreeModificationAction { kDispatchSubtreeModifiedEvent, kOmitSubtreeModifiedEvent };
...
public:
    ...
    {
        return last_child_.Get();
    }
    bool hasChildren() const
    {
        return static_cast<bool>(first_child_);
    }
    bool HasChildren() const
    {
        return static_cast<bool>(first_child_);
    ...
    void InsertBefore(const VectorOf<Node>& new_children, Node* ref_child, ExceptionState&);
    Node* InsertBefore(Node* new_child, Node* ref_child, ExceptionState&);
    Node* InsertBefore(Node* new_child, Node* ref_child);
    void ReplaceChild(const VectorOf<Node>& new_children, Node* old_child, ExceptionState&);
    ...
    bool EnsurePreInsertionValidity(const Node* new_child, const VectorOf<Node>* new_children, const Node* next, const Node* old_child, ExceptionState&) const;

    Element* getElementById(const AtomicString& id) const;
    HTMLCollection* getElementsByTagName(const AtomicString&);
    HTMLCollection* getElementsByTagNameNS(const AtomicString& namespace_uri, const AtomicString& local_name);
    ...
    RadioNodeList* GetRadioNodeList(const AtomicString&, bool only_match_img_elements = false);
    ...
    String FindTextInElementWith(const AtomicString& substring, base::FunctionRef<bool(const String&)> validity_checker) const;
    ...
    void RemoveChildren(SubtreeModificationAction = kDispatchSubtreeModifiedEvent);
    ...
    {
        return HasRestyleFlag(DynamicRestyleFlags::kChildrenOrSiblingsAffectedByFocus);
    }
    ...
    {
        return HasRestyleFlag(DynamicRestyleFlags::kChildrenOrSiblingsAffectedByFocusVisible);
    }
    void SetChildrenOrSiblingsAffectedByFocusVisible()
    {
        SetRestyleFlag(DynamicRestyleFlags::kChildrenOrSiblingsAffectedByFocusVisible);
    }

    bool ChildrenOrSiblingsAffectedByFocusWithin() const
    {
        return HasRestyleFlag(DynamicRestyleFlags::kChildrenOrSiblingsAffectedByFocusWithin);
    }
    ...
    {
        return HasRestyleFlag(DynamicRestyleFlags::kChildrenOrSiblingsAffectedByHover);
    }
    ...
    {
        return HasRestyleFlag(DynamicRestyleFlags::kChildrenOrSiblingsAffectedByActive);
    }
    ...
    {
        return HasRestyleFlag(DynamicRestyleFlags::kChildrenAffectedByFirstChildRules);
    }
    ...
    void CheckForSiblingStyleChanges(SiblingCheckType, Element* changed_element, Node* node_before_change, Node* node_after_change);
    void RecalcDescendantStyles(const StyleRecalcChange, const StyleRecalcContext&);
    ...
    public:
        ...
        static ChildrenChange ForInsertion(Node& node, Node* unchanged_previous, Node* unchanged_next, ChildrenChangeSource by_parser)
        {
            ChildrenChange change = {
                .type = node.IsElementNode() ? ChildrenChangeType::kElementInserted : ChildrenChangeType::kNonElementInserted,
                .by_parser = by_parser,
                .affects_elements = node.IsElementNode() ? ChildrenChangeAffectsElements::kYes : ChildrenChangeAffectsElements::kNo,
        ...
        static ChildrenChange ForRemoval(Node& node, Node* previous_sibling, Node* next_sibling, ChildrenChangeSource by_parser)
        {
            ChildrenChange change = {
        ...
        bool ByParser() const
        {
            return by_parser == ChildrenChangeSource::kParser;
        }
    ...
    virtual bool ChildrenCanHaveStyle() const
    {
        return true;
    }
    ...
    void ReplaceChildren(const VectorOf<Node>& nodes, ExceptionState& exception_state);
...
protected:
    ...
    void InvalidateNodeListCachesInAncestors(const QualifiedName* attr_name, Element* attribute_owner_element, const ChildrenChange*);
    ...
    template <typename Collection> Collection* EnsureCachedCollection(CollectionType, const AtomicString& namespace_uri, const AtomicString& local_name);
...
private:
    ...
    // |next| may be nullptr.
    template <typename Functor> void InsertNodeVector(const NodeVector&, Node* next, const Functor&, NodeVector& post_insertion_notification_targets);
    ...
    bool RecheckNodeInsertionStructuralPrereq(const NodeVector&, const Node* next, ExceptionState&);
    ...
    void CheckSoftNavigationHeuristicsTracking(const Document& document, Node& inserted_node);

    Member<Node> first_child_;
    Member<Node> last_child_;
};

template <> struct DowncastTraits<ContainerNode> {
