...
class TransitionPseudoElementData final : public GarbageCollected<TransitionPseudoElementData> {
public:
    TransitionPseudoElementData() = default;
    TransitionPseudoElementData(const TransitionPseudoElementData&) = delete;
    TransitionPseudoElementData& operator=(const TransitionPseudoElementData&) = delete;

    void SetPseudoElement(PseudoId, PseudoElement*, const AtomicString& view_transition_name = g_null_atom);
...
{
    return transition_ || transition_outgoing_image_ || transition_incoming_image_ || transition_image_wrapper_ || !transition_containers_.empty();
}

inline void TransitionPseudoElementData::ClearPseudoElements()
{
    SetPseudoElement(kPseudoIdViewTransition, nullptr);
    SetPseudoElement(kPseudoIdViewTransitionImagePair, nullptr, transition_image_wrapper_ ? transition_image_wrapper_->view_transition_name() : g_null_atom);
    SetPseudoElement(kPseudoIdViewTransitionOld, nullptr, transition_outgoing_image_ ? transition_outgoing_image_->view_transition_name() : g_null_atom);
    SetPseudoElement(kPseudoIdViewTransitionNew, nullptr, transition_incoming_image_ ? transition_incoming_image_->view_transition_name() : g_null_atom);
...
inline void TransitionPseudoElementData::SetPseudoElement(PseudoId pseudo_id, PseudoElement* element, const AtomicString& view_transition_name)
{
    PseudoElement* previous_element = GetPseudoElement(pseudo_id, view_transition_name);
    switch (pseudo_id) {
    case kPseudoIdViewTransition:
        transition_ = element;
        break;
    case kPseudoIdViewTransitionImagePair:
        DCHECK(!element || element->view_transition_name() == view_transition_name);
...
inline PseudoElement* TransitionPseudoElementData::GetPseudoElement(PseudoId pseudo_id, const AtomicString& view_transition_name) const
{
    switch (pseudo_id) {
    case kPseudoIdViewTransition:
        return transition_.Get();
    case kPseudoIdViewTransitionImagePair:
        DCHECK(!transition_image_wrapper_ || !view_transition_name || transition_image_wrapper_->view_transition_name() == view_transition_name);
        return transition_image_wrapper_.Get();
    case kPseudoIdViewTransitionOld:
        DCHECK(!transition_outgoing_image_ || !view_transition_name || transition_outgoing_image_->view_transition_name() == view_transition_name);
        return transition_outgoing_image_.Get();
    case kPseudoIdViewTransitionNew:
        DCHECK(!transition_incoming_image_ || !view_transition_name || transition_incoming_image_->view_transition_name() == view_transition_name);
...
inline void TransitionPseudoElementData::AddPseudoElements(HeapVector<Member<PseudoElement>, 2>* result) const
{
    if (transition_)
