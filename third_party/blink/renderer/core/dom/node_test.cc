...
public:
    FakeMediaControlElement(Document& document)
        : HTMLDivElement(document)
    {
    }

    bool IsMediaControlElement() const override
    {
        return true;
    }
};
...
public:
    FakeMediaControls(Document& document)
        : HTMLDivElement(document)
    {
    }

    bool IsMediaControls() const override
    {
        return true;
    }
};
...
{
    const char* body_content = "<a id=one href='http://www.msn.com'>one</a><b id=two>two</b>";
...
TEST_F(NodeTest, AttachContext_PreviousInFlow_Slotted)
{
    ...
    shadow_root.setInnerHTML("<div id=root style='display:contents'><span></span><slot></slot></div>");
...
{
    FakeMediaControlElement* node = MakeGarbageCollected<FakeMediaControlElement>(GetDocument());
...
{
    FakeMediaControls* node = MakeGarbageCollected<FakeMediaControls>(GetDocument());
    EXPECT_TRUE(node->HasMediaControlAncestor());
    EXPECT_TRUE(InitializeUserAgentShadowTree(node)->HasMediaControlAncestor());
}
...
TEST_F(NodeTest, appendChildProcessingInstructionNoStyleRecalc)
{
    ...
    auto* pi = MakeGarbageCollected<ProcessingInstruction>(GetDocument(), "A", "B");
...
TEST_F(NodeTest, MutationOutsideFlatTreeStyleDirty)
{
    ...
    GetDocument().getElementById(AtomicString("nonslotted"))->setAttribute(html_names::kStyleAttr, AtomicString("color:green"));
    EXPECT_FALSE(GetDocument().NeedsLayoutTreeUpdate());
}
...
TEST_F(NodeTest, SkipStyleDirtyHostChild)
{
    ...
    To<Element>(host->firstChild())->setAttribute(html_names::kStyleAttr, AtomicString("color:green"));
    EXPECT_FALSE(GetDocument().NeedsLayoutTreeUpdate());
}
...
{
    SetBodyContent("<style>#a::before{content:'aaa';}</style>"
                   "<div id=a></div>");
    Element* a = GetDocument().getElementById(AtomicString("a"));
...
TEST_F(NodeTest, UpdateChildDirtyAncestorsOnSlotAssignment)
{
    ...
    Element* host = GetDocument().getElementById(AtomicString("host"));
    ShadowRoot& shadow_root = host->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    shadow_root.setInnerHTML("<div><slot></slot></div><div id='child-dirty'><slot "
...
TEST_F(NodeTest, FlatTreeParentForChildDirty)
{
    ...
    Element* host = GetDocument().getElementById(AtomicString("host"));
    Element* slotted = GetDocument().getElementById(AtomicString("slotted"));
    Element* not_slotted = GetDocument().getElementById(AtomicString("not_slotted"));
