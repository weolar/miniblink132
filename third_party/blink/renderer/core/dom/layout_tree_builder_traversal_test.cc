...
{
    const char* const kHtml = "<div></div>"
    ...
    EXPECT_EQ(last->GetLayoutObject(), LayoutTreeBuilderTraversal::NextSiblingLayoutObject(*first));
}
...
TEST_F(LayoutTreeBuilderTraversalTest, displayContentsChildren)
{
    ...
    EXPECT_EQ(inner->GetLayoutObject(), LayoutTreeBuilderTraversal::NextSiblingLayoutObject(*first));
    EXPECT_EQ(first->GetLayoutObject(), LayoutTreeBuilderTraversal::PreviousSiblingLayoutObject(*inner));

    EXPECT_EQ(last->GetLayoutObject(), LayoutTreeBuilderTraversal::NextSiblingLayoutObject(*inner));
    EXPECT_EQ(inner->GetLayoutObject(), LayoutTreeBuilderTraversal::PreviousSiblingLayoutObject(*last));
}
...
TEST_F(LayoutTreeBuilderTraversalTest, displayContentsChildrenNested)
{
    ...
    Element* first = GetDocument().QuerySelector(AtomicString("div"));
    Element* inner = GetDocument().QuerySelector(AtomicString("#inner"));
    Element* sibling = GetDocument().QuerySelector(AtomicString("#inner-sibling"));
    Element* last = GetDocument().QuerySelector(AtomicString("#last"));
    ...
    EXPECT_EQ(sibling->GetLayoutObject(), LayoutTreeBuilderTraversal::NextSiblingLayoutObject(*inner));
    EXPECT_EQ(inner->GetLayoutObject(), LayoutTreeBuilderTraversal::PreviousSiblingLayoutObject(*sibling));

    EXPECT_EQ(last->GetLayoutObject(), LayoutTreeBuilderTraversal::NextSiblingLayoutObject(*sibling));
    EXPECT_EQ(sibling->GetLayoutObject(), LayoutTreeBuilderTraversal::PreviousSiblingLayoutObject(*last));
}
...
TEST_F(LayoutTreeBuilderTraversalTest, limits)
{
    ...
    EXPECT_TRUE(first->GetLayoutObject());
    LayoutObject* next_sibling = LayoutTreeBuilderTraversal::NextSiblingLayoutObject(*first, 2);
