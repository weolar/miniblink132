...
TreeWalker::TreeWalker(Node* root_node, unsigned what_to_show, V8NodeFilter* filter)
...
template <typename Strategy> Node* TreeWalker::TraverseSiblings(ExceptionState& exception_state)
{
...
Children:
    ...
    while (Node* next_sibling = NodeTraversal::NextSkippingChildren(*node, root())) {
        node = next_sibling;
        unsigned accept_node_result = AcceptNode(node, exception_state);
        if (exception_state.HadException())
            return nullptr;
