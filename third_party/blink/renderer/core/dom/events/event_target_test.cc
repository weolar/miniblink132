...
{
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kPassiveTouchEventListener));
    GetDocument().GetSettings()->SetScriptEnabled(true);
    ClassicScript::CreateUnspecifiedScript("window.addEventListener('touchstart', function() {}, "
                                           "{passive: true});")
        ->RunScript(GetDocument().domWindow());
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kPassiveTouchEventListener));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kNonPassiveTouchEventListener));
}
...
TEST_F(EventTargetTest, UseCountNonPassiveTouchEventListener)
{
    ...
    ClassicScript::CreateUnspecifiedScript("window.addEventListener('touchstart', function() {}, "
                                           "{passive: false});")
        ->RunScript(GetDocument().domWindow());
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kNonPassiveTouchEventListener));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kPassiveTouchEventListener));
}
...
TEST_F(EventTargetTest, UseCountPassiveTouchEventListenerPassiveNotSpecified)
{
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kPassiveTouchEventListener));
    GetDocument().GetSettings()->SetScriptEnabled(true);
    ClassicScript::CreateUnspecifiedScript("window.addEventListener('touchstart', function() {});")->RunScript(GetDocument().domWindow());
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kPassiveTouchEventListener));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kNonPassiveTouchEventListener));
}
...
{
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kBeforematchHandlerRegistered));
    GetDocument().GetSettings()->SetScriptEnabled(true);
    ClassicScript::CreateUnspecifiedScript(R"HTML(
        ...
        ->RunScript(GetDocument().domWindow());
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kBeforematchHandlerRegistered));
}

TEST_F(EventTargetTest, UseCountAbortSignal)
{
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kAddEventListenerWithAbortSignal));
    GetDocument().GetSettings()->SetScriptEnabled(true);
    ClassicScript::CreateUnspecifiedScript(R"HTML(
                       const element = document.createElement('div');
        ...
        ->RunScript(GetDocument().domWindow());
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kAddEventListenerWithAbortSignal));
}
...
TEST_F(EventTargetTest, EventTargetWithAbortSignalDestroyed)
{
    ...
    Persistent<AbortController> controller = AbortController::Create(scope.GetScriptState());
    Persistent<EventListener> listener = JSEventListener::CreateOrNull(V8EventListener::Create(scope.GetContext()->Global()));
    {
        ...
        event_target->addEventListener(AtomicString("test"), listener.Get(), MakeGarbageCollected<AddEventListenerOptionsResolved>(options));
...
// listener is removed.
TEST_F(EventTargetTest, ObservableSubscriptionBecomingInactiveRemovesEventListener)
{
    V8TestingScope scope;
    EventTarget* event_target = EventTarget::Create(scope.GetScriptState());
    Observable* observable = event_target->when(AtomicString("test"), MakeGarbageCollected<ObservableEventListenerOptions>());
    ...
    V8UnionObserverOrObserverCallback* observer_union = MakeGarbageCollected<V8UnionObserverOrObserverCallback>(observer);
