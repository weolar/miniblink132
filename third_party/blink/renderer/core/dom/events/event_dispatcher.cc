...
void EventDispatcher::DispatchSimulatedClick(Node& node, const Event* underlying_event, SimulatedClickCreationScope creation_scope)
{
    ...
    DEFINE_STATIC_LOCAL(Persistent<HeapHashSet<Member<Node>>>, nodes_dispatching_simulated_clicks, (MakeGarbageCollected<HeapHashSet<Member<Node>>>()));
    ...
    if (creation_scope == SimulatedClickCreationScope::kFromAccessibility) {
        DispatchEventResult dispatch_result
            = EventDispatcher(node, *SimulatedEventUtil::CreateEvent(event_type_names::kPointerdown, node, underlying_event, creation_scope)).Dispatch();
        prevent_mouse_events = dispatch_result == DispatchEventResult::kCanceledByEventHandler;
        if (!prevent_mouse_events) {
            EventDispatcher(node, *SimulatedEventUtil::CreateEvent(event_type_names::kMousedown, node, underlying_event, creation_scope)).Dispatch();
        }
        if (element)
            element->SetActive(true);
        EventDispatcher(node, *SimulatedEventUtil::CreateEvent(event_type_names::kPointerup, node, underlying_event, creation_scope)).Dispatch();
        if (!prevent_mouse_events) {
            EventDispatcher(node, *SimulatedEventUtil::CreateEvent(event_type_names::kMouseup, node, underlying_event, creation_scope)).Dispatch();
        }
    }
    ...
    // Always send click.
    EventDispatcher(node, *SimulatedEventUtil::CreateEvent(event_type_names::kClick, node, underlying_event, creation_scope)).Dispatch();

    nodes_dispatching_simulated_clicks->erase(&node);
}

void EventDispatcher::DispatchSimulatedEnterEvent(HTMLInputElement& input_element)
{
    LocalDOMWindow* local_dom_window = input_element.GetDocument().domWindow();
    for (auto type : { WebInputEvent::Type::kRawKeyDown, WebInputEvent::Type::kChar, WebInputEvent::Type::kKeyUp }) {
        WebKeyboardEvent enter { type, WebInputEvent::kNoModifiers, base::TimeTicks::Now() };
        ...
        KeyboardEvent* event = blink::KeyboardEvent::Create(enter, local_dom_window, true);
...
DispatchEventResult EventDispatcher::Dispatch()
{
    ...
    if (event_->type() == event_type_names::kChange && event_->isTrusted() && view_) {
        view_->GetLayoutShiftTracker().NotifyChangeEvent();
    }
    event_->GetEventPath().EnsureWindowEventContext();

    const bool is_click = event_->IsMouseEvent() && event_->type() == event_type_names::kClick;
    ...
    if (window) {
        if (auto* heuristics = SoftNavigationHeuristics::From(*window)) {
            soft_navigation_scope = heuristics->MaybeCreateEventScopeForEvent(*event_);
        }
    }
    ...
    if (is_click && event_->isTrusted() && frame) {
        ...
        DCHECK(!frame->GetAdTracker() || !frame->GetAdTracker()->IsAdScriptInStack(AdTracker::StackType::kBottomAndTop));
        if (frame->IsAdFrame()) {
    ...
    const bool is_activation_event = is_click || event_->type() == event_type_names::kTextInput;

    // 7. Let activationTarget be target, if isActivationEvent is true and target
    // has activation behavior, and null otherwise.
    Node* activation_target = is_activation_event && node_->HasActivationBehavior() ? node_ : nullptr;
    ...
    DEVTOOLS_TIMELINE_TRACE_EVENT("EventDispatch", inspector_event_dispatch_event::Data, *event_, document.GetAgent().isolate());
    EventDispatchHandlingState* pre_dispatch_event_handler_result = nullptr;
    if (DispatchEventPreProcess(activation_target, pre_dispatch_event_handler_result) == kContinueDispatching) {
        if (DispatchEventAtCapturing() == kContinueDispatching) {
            DispatchEventAtBubbling();
        }
    }
    DispatchEventPostProcess(activation_target, pre_dispatch_event_handler_result);
...
inline EventDispatchContinuation EventDispatcher::DispatchEventPreProcess(
    Node* activation_target, EventDispatchHandlingState*& pre_dispatch_event_handler_result)
{
    ...
    if (activation_target) {
        pre_dispatch_event_handler_result = activation_target->PreDispatchEventHandler(*event_);
    }

    return (event_->GetEventPath().IsEmpty() || event_->PropagationStopped()) ? kDoneDispatching : kContinueDispatching;
}
...
inline EventDispatchContinuation EventDispatcher::DispatchEventAtCapturing()
{
    ...
    if (event_->GetEventPath().GetWindowEventContext().HandleLocalEvents(*event_) && event_->PropagationStopped())
...
inline void EventDispatcher::DispatchEventPostProcess(Node* activation_target, EventDispatchHandlingState* pre_dispatch_event_handler_result)
{
    ...
    bool is_click = mouse_event && mouse_event->type() == event_type_names::kClick;
    if (is_click) {
        ...
        if (activation_target) {
            activation_target->PostDispatchEventHandler(*event_, pre_dispatch_event_handler_result);
        }
    ...
    if (!is_trusted_or_click && event_->IsMouseEvent() && event_->type() == event_type_names::kMousedown && IsA<HTMLSelectElement>(*node_)) {
    ...
    if (!event_->defaultPrevented() && !event_->DefaultHandled() && is_trusted_or_click) {
        ...
        if (!event_->DefaultHandled() && !event_->defaultPrevented() && event_->bubbles()) {
            wtf_size_t size = event_->GetEventPath().size();
            for (wtf_size_t i = 1; i < size; ++i) {
    ...
    if (Page* page = node_->GetDocument().GetPage()) {
        if (page->GetSettings().GetSpatialNavigationEnabled() && is_trusted_or_click && keyboard_event && keyboard_event->key() == keywords::kCapitalEnter
    ...
    if (!event_->isTrusted() && event_->IsMouseEvent() && event_->type() == event_type_names::kMousedown && IsA<HTMLSelectElement>(*node_)) {
        UseCounter::Count(node_->GetDocument(), WebFeature::kUntrustedMouseDownEventDispatchedToSelect);
    }
