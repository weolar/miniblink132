...
public:
    ...
    enum class PhaseType : uint8_t { kNone = 0, kCapturingPhase = 1, kAtTarget = 2, kBubblingPhase = 3 };
    ...
    static Event* Create()
    {
        return MakeGarbageCollected<Event>();
    }
    ...
    {
        return MakeGarbageCollected<Event>(type, Bubbles::kYes, Cancelable::kYes, base::TimeTicks::UnixEpoch());
    }

    Event();
    Event(const AtomicString& type, Bubbles, Cancelable, ComposedMode, base::TimeTicks platform_time_stamp);
    Event(const AtomicString& type, Bubbles, Cancelable, base::TimeTicks platform_time_stamp);
    Event(const AtomicString& type, Bubbles, Cancelable, ComposedMode = ComposedMode::kScoped);
    Event(const AtomicString& type, const EventInit*, base::TimeTicks platform_time_stamp);
    ...
    void initEvent(const AtomicString& event_type_arg, bool bubbles_arg, bool cancelable_arg, EventTarget* related_target);
    ...
    void SetEventPhase(PhaseType event_phase)
    {
    ...
    base::TimeTicks PlatformTimeStamp() const
    {
        return platform_time_stamp_;
    }
    ...
    {
        return default_handled_;
    }
    void SetDefaultHandled()
    {
        default_handled_ = true;
    }
    ...
    const Event* UnderlyingEvent() const
    {
        return underlying_event_.Get();
    }
    void SetUnderlyingEvent(const Event*);

    bool HasEventPath() const
    {
        return static_cast<bool>(event_path_);
    }
    ...
    {
        return eventPhase() != PhaseType::kNone;
    }
...
protected:
    ...
    void SetBubbles(bool bubble)
    {
        bubbles_ = bubble;
    }

    PassiveMode HandlingPassive() const
    {
        return handling_passive_;
    }
