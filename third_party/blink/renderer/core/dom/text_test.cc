...
{
    SetBodyContent("<div id=block></div>Ends with whitespace "
                   "<span id=inline></span>Nospace<br id=br>");
    UpdateAllLifecyclePhasesForTest();

    LayoutObject* block = GetDocument().getElementById(AtomicString("block"))->GetLayoutObject();
    LayoutObject* in_line = GetDocument().getElementById(AtomicString("inline"))->GetLayoutObject();
    LayoutObject* space_at_end = GetDocument().getElementById(AtomicString("block"))->nextSibling()->GetLayoutObject();
    LayoutObject* no_space = GetDocument().getElementById(AtomicString("inline"))->nextSibling()->GetLayoutObject();
    LayoutObject* br = GetDocument().getElementById(AtomicString("br"))->GetLayoutObject();
    ASSERT_TRUE(block);
    ASSERT_TRUE(in_line);
    ...
    EXPECT_FALSE(whitespace->TextLayoutObjectIsNeeded(context, block->StyleRef()));
    context.parent = in_line;
    EXPECT_FALSE(whitespace->TextLayoutObjectIsNeeded(context, in_line->StyleRef()));
