...
public:
    static AttributePart* Create(PartRootUnion* root_union, Node* node, AtomicString local_name, const PartInit* init, ExceptionState& exception_state);
    AttributePart(PartRoot& root, Element& element, AtomicString local_name, const PartInit* init)
        : AttributePart(root, element, local_name, init && init->hasMetadata() ? init->metadata() : Vector<String>())
    {
    }
    AttributePart(PartRoot& root, Element& element, AtomicString local_name, Vector<String> metadata);
