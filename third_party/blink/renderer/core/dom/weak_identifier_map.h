...
template <typename T, typename IdentifierType = int> class WeakIdentifierMap final : public GarbageCollected<WeakIdentifierMap<T, IdentifierType>> {
public:
    ...
    static IdentifierType ExistingIdentifier(T* object)
    {
        ...
        return it_result != Instance().object_to_identifier_.end() ? it_result->value : 0;
    }
    ...
    static void SetLastIdForTesting(IdentifierType i)
    {
        LastIdRef() = i;
    }
...
private:
    ...
    static IdentifierType Next()
    {
        ...
        if (LastIdRef() == std::numeric_limits<IdentifierType>::max()) [[unlikely]] {
            LastIdRef() = 0;
        }
        return ++LastIdRef();
    }
    ...
    {
        if (!identifier_to_object_.insert(identifier, object).is_new_entry) [[unlikely]] {
            return false;
        }
...
#define DEFINE_WEAK_IDENTIFIER_MAP(T, ...)                                                                                                                     \
    ...
    {                                                                                                                                                          \
        ...
        DEFINE_STATIC_LOCAL(Persistent<RefType>, map_instance, (MakeGarbageCollected<WeakIdentifierMap<T, ##__VA_ARGS__>>()));                                 \
        return *map_instance;                                                                                                                                  \
    }
