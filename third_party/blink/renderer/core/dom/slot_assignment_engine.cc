...
void SlotAssignmentEngine::RemoveShadowRootNeedingRecalc(ShadowRoot& shadow_root)
{
    DCHECK(shadow_root.isConnected());
    DCHECK(!shadow_root.NeedsSlotAssignmentRecalc());
...
void SlotAssignmentEngine::RecalcSlotAssignments()
{
    ...
    for (auto& shadow_root : HeapHashSet<WeakMember<ShadowRoot>>(shadow_roots_needing_recalc_)) {
        DCHECK(shadow_root->isConnected());
        DCHECK(shadow_root->NeedsSlotAssignmentRecalc());
