...
template <class Traversal> class TraversalIteratorBase {
    STACK_ALLOCATED();
...
template <class Traversal> class TraversalIterator : public TraversalIteratorBase<Traversal> {
    STACK_ALLOCATED();
...
template <class Traversal> class TraversalDescendantIterator : public TraversalIteratorBase<Traversal> {
    STACK_ALLOCATED();
...
public:
    ...
    explicit TraversalDescendantIterator(const StartNodeType* start)
        : TraversalIteratorBase<Traversal>(start ? Traversal::FirstWithin(*start) : nullptr)
        , root_(start)
    {
    }
...
template <class Traversal> class TraversalInclusiveDescendantIterator : public TraversalIteratorBase<Traversal> {
    STACK_ALLOCATED();
...
template <class Traversal> class TraversalParent {
public:
...
template <class Traversal> class TraversalSibling {
public:
    using TraversalNodeType = typename Traversal::TraversalNodeType;
    static TraversalNodeType* Next(const TraversalNodeType& node)
    {
...
template <class T> using TraversalNextRange = TraversalRange<TraversalIterator<T>>;

template <class T> using TraversalAncestorRange = TraversalRange<TraversalIterator<TraversalParent<T>>>;

template <class T> using TraversalSiblingRange = TraversalRange<TraversalIterator<TraversalSibling<T>>>;

template <class T> using TraversalDescendantRange = TraversalRange<TraversalDescendantIterator<T>>;

template <class T> using TraversalInclusiveDescendantRange = TraversalRange<TraversalInclusiveDescendantIterator<T>>;
