...
public:
    ...
    Range(Document&, Node* start_container, unsigned start_offset, Node* end_container, unsigned end_offset);
    ...
    static Node* commonAncestorContainer(const Node* container_a, const Node* container_b);
    void setStart(Node* container, unsigned offset, ExceptionState& = ASSERT_NO_EXCEPTION);
    void setEnd(Node* container, unsigned offset, ExceptionState& = ASSERT_NO_EXCEPTION);
    ...
    int16_t compareBoundaryPoints(unsigned how, const Range* source_range, ExceptionState&) const;
    static int16_t compareBoundaryPoints(Node* container_a, unsigned offset_a, Node* container_b, unsigned offset_b, ExceptionState&);
    static int16_t compareBoundaryPoints(const RangeBoundaryPoint& boundary_a, const RangeBoundaryPoint& boundary_b, ExceptionState&);
    ...
    {
        return start_.ToPosition();
    }
    const Position EndPosition() const
    {
        return end_.ToPosition();
    }
...
private:
    ...
    static Node* ProcessContentsBetweenOffsets(ActionType, DocumentFragment*, Node*, unsigned start_offset, unsigned end_offset, ExceptionState&);
    static void ProcessNodes(ActionType, NodeVector&, Node* old_container, Node* new_container, ExceptionState&);
    enum ContentsProcessDirection { kProcessContentsForward, kProcessContentsBackward };
    static Node* ProcessAncestorsAndTheirSiblings(
        ActionType, Node* container, ContentsProcessDirection, Node* cloned_container, Node* common_root, ExceptionState&);
