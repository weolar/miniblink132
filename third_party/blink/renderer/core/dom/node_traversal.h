...
public:
    ...
    static Node* PreviousIncludingPseudo(const Node&, const Node* stay_within = nullptr);
    static Node* NextIncludingPseudo(const Node&, const Node* stay_within = nullptr);
    // See comment for |FlatTreeTraversal::PreviousAbsoluteSibling| for details.
    static Node* PreviousAbsoluteSiblingIncludingPseudo(const Node&, const Node* stay_within = nullptr);
    static Node* NextIncludingPseudoSkippingChildren(const Node&, const Node* stay_within = nullptr);
    ...
    static bool HasChildren(const Node& parent)
    {
    ...
    static Node* FirstChild(const Node& parent)
    {
        return parent.firstChild();
    }
    static Node* LastChild(const Node& parent)
    {
    ...
    static ContainerNode* Parent(const Node& node)
    {
        return node.parentNode();
    }
    static Node* CommonAncestor(const Node& node_a, const Node& node_b);
    static unsigned Index(const Node& node)
    {
...
private:
    template <class NodeType> static Node* TraverseNextTemplate(NodeType&);
    template <class NodeType> static Node* TraverseNextTemplate(NodeType&, const Node* stay_within);
    template <class NodeType> static Node* ChildAtTemplate(NodeType&, unsigned);
...
template <class NodeType> inline Node* NodeTraversal::TraverseNextTemplate(NodeType& current)
{
...
template <class NodeType> inline Node* NodeTraversal::TraverseNextTemplate(NodeType& current, const Node* stay_within)
{
    if (current.hasChildren())
        return current.firstChild();
...
template <class NodeType> inline Node* NodeTraversal::ChildAtTemplate(NodeType& parent, unsigned index)
{
