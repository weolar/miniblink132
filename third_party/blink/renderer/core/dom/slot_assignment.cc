...
void SlotAssignment::DidAddSlot(HTMLSlotElement& slot)
{
    ...
    DCHECK(!slot_map_->Contains(slot.GetName()) || GetCachedFirstSlotWithoutAccessingNodeTree(slot.GetName()));
...
void SlotAssignment::DidAddSlotInternal(HTMLSlotElement& slot)
{
    ...
    HTMLSlotElement* old_active = GetCachedFirstSlotWithoutAccessingNodeTree(slot_name);
...
void SlotAssignment::DidRemoveSlotInternal(HTMLSlotElement& slot, const AtomicString& slot_name, SlotMutationType slot_mutation_type)
{
...
bool SlotAssignment::FindHostChildBySlotName(const AtomicString& slot_name) const
{
...
void SlotAssignment::DidRenameSlot(const AtomicString& old_slot_name, HTMLSlotElement& slot)
{
...
void SlotAssignment::DidChangeHostChildSlotName(const AtomicString& old_value, const AtomicString& new_value)
{
    if (HTMLSlotElement* slot = FindSlotByName(HTMLSlotElement::NormalizeSlotName(old_value))) {
        slot->DidSlotChange(SlotChangeType::kSignalSlotChangeEvent);
    }
    if (HTMLSlotElement* slot = FindSlotByName(HTMLSlotElement::NormalizeSlotName(new_value))) {
        slot->DidSlotChange(SlotChangeType::kSignalSlotChangeEvent);
    }
}
...
void SlotAssignment::SetNeedsAssignmentRecalc()
{
    ...
    if (owner_->isConnected()) {
        owner_->GetDocument().GetSlotAssignmentEngine().AddShadowRootNeedingRecalc(*owner_);
...
void SlotAssignment::RecalcAssignment()
{
    if (!needs_assignment_recalc_)
        return;
    {
        NestingLevelIncrementer slot_assignment_recalc_depth(owner_->GetDocument().SlotAssignmentRecalcDepth());
        ...
        FlatTreeTraversalForbiddenScope forbid_flat_tree_traversal(owner_->GetDocument());
        ...
        if (owner_->IsManualSlotting()) {
            ...
            for (Member<HTMLSlotElement> slot : Slots()) {
                for (Node* slottable : slot->ManuallyAssignedNodes()) {
                    ...
                    if (slottable && slottable->IsChildOfShadowHost() && slottable->parentElement() == owner_->host()) {
        ...
        if (owner_->isConnected()) {
            owner_->GetDocument().GetSlotAssignmentEngine().RemoveShadowRootNeedingRecalc(*owner_);
        }
        ...
        for (auto& slot : Slots()) {
            ...
            slot->DidRecalcAssignedNodes(!!DisplayLockUtilities::LockedInclusiveAncestorPreventingStyleWithinTreeScope(*slot));
        }
    }
...
HTMLSlotElement* SlotAssignment::FindSlot(const Node& node)
{
    if (!node.IsSlotable())
        return nullptr;
    return owner_->IsManualSlotting() ? FindSlotInManualSlotting(const_cast<Node&>(node)) : FindSlotByName(node.SlotName());
}

HTMLSlotElement* SlotAssignment::FindSlotByName(const AtomicString& slot_name) const
{
...
HTMLSlotElement* SlotAssignment::FindSlotInManualSlotting(Node& node)
{
    ...
    if (slot && slot->ContainingShadowRoot() == owner_ && node.IsChildOfShadowHost() && node.parentElement() == owner_->host())
        return slot;
...
void SlotAssignment::CollectSlots()
{
    ...
    for (HTMLSlotElement& slot : Traversal<HTMLSlotElement>::DescendantsOf(*owner_)) {
...
HTMLSlotElement* SlotAssignment::GetCachedFirstSlotWithoutAccessingNodeTree(const AtomicString& slot_name)
{
    if (Element* slot = slot_map_->GetCachedFirstElementWithoutAccessingNodeTree(slot_name)) {
