...
void WhitespaceAttacher::DidReattachElement(Element* element, LayoutObject* prev_in_flow)
{
    DCHECK(element);
...
void WhitespaceAttacher::ReattachWhitespaceSiblings(LayoutObject* previous_in_flow)
{
    DCHECK(!last_display_contents_);
    ...
    context.parent = LayoutTreeBuilderTraversal::ParentLayoutObject(*last_text_node_);

    for (Node* sibling = last_text_node_; sibling; sibling = LayoutTreeBuilderTraversal::NextLayoutSibling(*sibling)) {
        ...
        } else if (sibling_layout_object && sibling_layout_object->AffectsWhitespaceSiblings()) {
            break;
        }
...
void WhitespaceAttacher::UpdateLastTextNodeFromDisplayContents()
{
    ...
    Node* sibling = LayoutTreeBuilderTraversal::FirstLayoutChild(*contents_element);
    ...
    for (; sibling && sibling != last_text_node_; sibling = LayoutTreeBuilderTraversal::NextLayoutSibling(*sibling)) {
