...
template <class T> HeapVector<Member<Node>> CollectFromIterable(T iterable)
{
...
{
    for (Node* descendant : CollectFromIterable(NodeTraversal::InclusiveDescendantsOf(container))) {
...
TEST_F(NodeTraversalTest, ChildAt)
{
    ...
    for (unsigned index = 0; index < kNumberOfChildNodes; ++index) {
        ...
        EXPECT_EQ(index, NodeTraversal::Index(*child)) << "NodeTraversal::index(NodeTraversal(*body, " << index << "))";
        EXPECT_TRUE(NodeTraversal::IsDescendantOf(*child, *body)) << "NodeTraversal::isDescendantOf(*NodeTraversal(*body, " << index << "), *body)";
        EXPECT_EQ(expected_child_nodes[index], child) << "NodeTraversal::childAt(*body, " << index << ")";
    }
    EXPECT_EQ(nullptr, NodeTraversal::ChildAt(*body, kNumberOfChildNodes + 1)) << "Out of bounds childAt() returns nullptr.";
...
TEST_F(NodeTraversalTest, AncestorsOf)
{
    ...
    for (Node* parent = NodeTraversal::Parent(*child); parent; parent = NodeTraversal::Parent(*parent)) {
...
TEST_F(NodeTraversalTest, ChildrenOf)
{
    ...
    HeapVector<Member<Node>> expected_nodes;
    for (Node* child = NodeTraversal::FirstChild(*body); child; child = NodeTraversal::NextSibling(*child)) {
...
TEST_F(NodeTraversalTest, DescendantsOf)
{
    ...
    HeapVector<Member<Node>> expected_nodes;
    for (Node* child = NodeTraversal::FirstChild(*body); child; child = NodeTraversal::Next(*child)) {
        expected_nodes.push_back(child);
    }
