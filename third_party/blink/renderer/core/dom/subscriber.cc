...
class Subscriber::CloseSubscriptionAlgorithm final : public AbortSignal::Algorithm {
public:
    explicit CloseSubscriptionAlgorithm(Subscriber* subscriber, AbortSignal* signal, ScriptState* script_state)
    ...
    void Run() override
    {
        subscriber_->CloseSubscription(script_state_, signal_->reason(script_state_));
    }
...
Subscriber::Subscriber(base::PassKey<Observable>, ScriptState* script_state, ObservableInternalObserver* internal_observer, SubscribeOptions* options)
    ...
    , subscription_controller_(AbortController::Create(script_state))
{
    ...
    if (options->hasSignal()) {
        ...
        if (downstream_signal->aborted()) {
            CloseSubscription(script_state,
                /*abort_reason=*/downstream_signal->reason(script_state));
        } else {
            close_subscription_algorithm_handle_
                = downstream_signal->AddAlgorithm(MakeGarbageCollected<CloseSubscriptionAlgorithm>(this, downstream_signal, script_state));
        }
    }
}
...
void Subscriber::error(ScriptState* script_state, ScriptValue error_value)
{
    if (!active_) {
        ...
        V8ScriptRunner::ReportException(script_state->GetIsolate(), error_value.V8Value());
        return;
    }
...
void Subscriber::CloseSubscription(ScriptState* script_state, std::optional<ScriptValue> abort_reason)
{
