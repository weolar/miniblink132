...
{
    DCHECK(
        IsSrcdocDocument() || IsAboutBlankDocument() || IsInitialEmptyDocument() || is_for_javascript_url_ || is_for_discard_ || fallback_base_url_.IsEmpty())
        << " url = " << url_ << ", fallback_base_url = " << fallback_base_url_;
    return fallback_base_url_;
}

DocumentInit& DocumentInit::WithWindow(LocalDOMWindow* window, Document* owner_document)
{
    DCHECK(!window_);
    DCHECK(!execution_context_);
    DCHECK(!agent_);
...
// static
DocumentInit::Type DocumentInit::ComputeDocumentType(LocalFrame* frame, const String& mime_type, bool* is_for_external_handler)
{
    ...
    if (MIMETypeRegistry::IsSupportedImageResourceMIMEType(mime_type) || mime_type == "multipart/x-mixed-replace") {
        return Type::kImage;
    }
    ...
    if (frame && frame->GetPage() && frame->Loader().AllowPlugins()) {
        ...
        // case.
        if (mime_type != "text/plain" && plugin_data && plugin_data->SupportsMimeType(mime_type)) {
...
DocumentInit& DocumentInit::WithTypeFrom(const String& mime_type)
{
    ...
    type_ = ComputeDocumentType(window_ ? window_->GetFrame() : nullptr, mime_type_, &is_for_external_handler_);
    return *this;
}

DocumentInit& DocumentInit::WithExecutionContext(ExecutionContext* execution_context)
{
    DCHECK(!execution_context_);
    DCHECK(!window_);
    DCHECK(!agent_);
...
{
    const KURL& cookie_url = owner_document_ ? owner_document_->CookieURL() : url_;
...
DocumentInit& DocumentInit::WithBaseAuctionNonce(base::Uuid base_auction_nonce)
{
...
Document* DocumentInit::CreateDocument() const
{
    ...
    case Type::kPlugin: {
        DCHECK(window_);
        if (window_->IsSandboxed(network::mojom::blink::WebSandboxFlags::kPlugins)) {
