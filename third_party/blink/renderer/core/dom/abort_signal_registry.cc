...
{
    AbortSignalRegistry* registry = Supplement<ExecutionContext>::From<AbortSignalRegistry>(context);
    if (!registry) {
...
void AbortSignalRegistry::RegisterAbortAlgorithm(EventListener* listener, AbortSignal::AlgorithmHandle* handle)
{
...
void AbortSignalRegistry::RegisterSignal(const AbortSignal& signal, AbortSignalCompositionType type)
{
    if (!GetExecutionContext() || GetExecutionContext()->IsContextDestroyed()) {
        return;
    }
...
void AbortSignalRegistry::UnregisterSignal(const AbortSignal& signal, AbortSignalCompositionType type)
{
    if (!GetExecutionContext() || GetExecutionContext()->IsContextDestroyed()) {
        return;
    }
