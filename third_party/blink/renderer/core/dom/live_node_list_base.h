...
public:
    LiveNodeListBase(ContainerNode& owner_node, NodeListSearchRoot search_root, NodeListInvalidationType invalidation_type, CollectionType collection_type)
...
protected:
    ...
    {
        return owner_node_->GetDocument();
    }

    template <typename MatchFunc>
    static Element* TraverseMatchingElementsForwardToOffset(
        Element& current_element, const ContainerNode* stay_within, unsigned offset, unsigned& current_offset, MatchFunc);
    template <typename MatchFunc>
...
ALWAYS_INLINE bool LiveNodeListBase::ShouldInvalidateTypeOnAttributeChange(NodeListInvalidationType type, const QualifiedName& attr_name)
{
    switch (type) {
    ...
    case kInvalidateForFormControls:
        return attr_name == html_names::kNameAttr || attr_name == html_names::kIdAttr || attr_name == html_names::kForAttr || attr_name == html_names::kFormAttr
    ...
    case kInvalidateOnPopoverInvokerAttrChange:
        return attr_name == html_names::kPopoverAttr || attr_name == html_names::kPopovertargetAttr || attr_name == html_names::kPopovertargetactionAttr;
...
Element* LiveNodeListBase::TraverseMatchingElementsForwardToOffset(
    Element& current_element, const ContainerNode* stay_within, unsigned offset, unsigned& current_offset, MatchFunc is_match)
{
    DCHECK_LT(current_offset, offset);
    for (Element* next = ElementTraversal::Next(current_element, stay_within, is_match); next; next = ElementTraversal::Next(*next, stay_within, is_match)) {
