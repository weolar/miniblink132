...
using PopoverHoverShowMap = HeapHashMap<WeakMember<const HTMLFormControlElement>, TaskHandle>;

class PopoverData final : public GarbageCollected<PopoverData>, public ElementRareDataField {
public:
    PopoverData() = default;
    PopoverData(const PopoverData&) = delete;
    PopoverData& operator=(const PopoverData&) = delete;

    PopoverVisibilityState visibilityState() const
    {
    ...
    {
        type_ = type;
        DCHECK_NE(type, PopoverValueType::kNone) << "Remove PopoverData rather than setting kNone type";
    }
    ...
    void setInvoker(Element* element)
    {
        invoker_ = element;
    }
    ...
    void cancelPendingToggleEventTask()
    {
        pending_toggle_event_task_.Cancel();
    }
    ...
    public:
        explicit ScopedStartShowingOrHiding(const Element& popover, bool show_warning = true)
            : popover_(popover)
    ...
    PopoverHoverShowMap& hoverShowTasks()
    {
    ...
    {
        return implicit_anchor_.Get();
    }
    void setImplicitAnchor(Element* element)
    {
        implicit_anchor_ = element;
    }

    CloseWatcher* closeWatcher()
    {
        return close_watcher_.Get();
    }
