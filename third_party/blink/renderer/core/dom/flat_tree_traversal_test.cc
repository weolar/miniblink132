...
protected:
    ...
    void SetupSampleHTML(std::string_view main_html, std::string_view shadow_html, unsigned index);

    void SetupDocumentTree(std::string_view main_html);

    void AttachOpenShadowRoot(Element& shadow_host, std::string_view shadow_inner_html);
};

void FlatTreeTraversalTest::SetupSampleHTML(std::string_view main_html, std::string_view shadow_html, unsigned index)
{
...
void FlatTreeTraversalTest::AttachOpenShadowRoot(Element& shadow_host, std::string_view shadow_inner_html)
{
    ShadowRoot& shadow_root = shadow_host.AttachShadowRootForTesting(ShadowRootMode::kOpen);
    shadow_root.setInnerHTML(String::FromUTF8(shadow_inner_html));
}

namespace {

void TestCommonAncestor(Node* expected_result, const Node& node_a, const Node& node_b)
{
    Node* result1 = FlatTreeTraversal::CommonAncestor(node_a, node_b);
    EXPECT_EQ(expected_result, result1) << "commonAncestor(" << node_a.textContent() << "," << node_b.textContent() << ")";
    Node* result2 = FlatTreeTraversal::CommonAncestor(node_b, node_a);
    EXPECT_EQ(expected_result, result2) << "commonAncestor(" << node_b.textContent() << "," << node_a.textContent() << ")";
}
...
{
    const char* main_html = "<div id='m0'>"
                            "<span slot='#m00' id='m00'>m00</span>"
                            "<span slot='#m01' id='m01'>m01</span>"
                            "</div>";
    const char* shadow_html = "<a id='s00'>s00</a>"
                              "<slot name='#m01'></slot>"
                              "<a id='s02'>s02</a>"
    ...
    ASSERT_EQ(kNumberOfChildNodes, FlatTreeTraversal::CountChildren(*shadow_host));
    ...
    for (unsigned index = 0; index < kNumberOfChildNodes; ++index) {
        ...
        EXPECT_EQ(index, FlatTreeTraversal::Index(*child)) << "FlatTreeTraversal::index(FlatTreeTraversal(*shadowHost, " << index << "))";
        EXPECT_TRUE(FlatTreeTraversal::IsDescendantOf(*child, *shadow_host)) << "FlatTreeTraversal::isDescendantOf(*FlatTreeTraversal(*"
                                                                                "shadowHost, "
                                                                             << index << "), *shadowHost)";
        ...
        EXPECT_EQ(expected_child_nodes[index], child) << "FlatTreeTraversal::childAt(*shadowHost, " << index << ")";
        EXPECT_EQ(is_slot_element ? 0 : index, FlatTreeTraversal::Index(*child)) << "FlatTreeTraversal::index(FlatTreeTraversal(*shadowHost, " << index << "))";
    }
    EXPECT_EQ(nullptr, FlatTreeTraversal::ChildAt(*shadow_host, kNumberOfChildNodes + 1)) << "Out of bounds childAt() returns nullptr.";

    // Distributed node |m00| is child of slot in shadow tree |s03|.
    EXPECT_EQ(m00, FlatTreeTraversal::FirstChild(*FlatTreeTraversal::FirstChild(*s03)));
}
...
TEST_F(FlatTreeTraversalTest, DescendantsOf)
{
...

    {
        ...
        for (Node* child = FlatTreeTraversal::FirstChild(*body); child; child = FlatTreeTraversal::Next(*child)) {
            expected_nodes.push_back(child);
        }
        EXPECT_EQ(expected_nodes, GatherFromTraversalRange(FlatTreeTraversal::DescendantsOf(*body)));

        expected_nodes.push_front(body);
        EXPECT_EQ(expected_nodes, GatherFromTraversalRange(FlatTreeTraversal::InclusiveDescendantsOf(*body)));
    }
    ...
    {
        HeapVector<Member<Node>> expected_nodes;
        for (Node* child = FlatTreeTraversal::FirstChild(*s03); child; child = FlatTreeTraversal::Next(*child, /*stay_within=*/s03)) {
            expected_nodes.push_back(child);
        }
        EXPECT_EQ(expected_nodes, GatherFromTraversalRange(FlatTreeTraversal::DescendantsOf(*s03)));

        expected_nodes.push_front(s03);
        EXPECT_EQ(expected_nodes, GatherFromTraversalRange(FlatTreeTraversal::InclusiveDescendantsOf(*s03)));
    }
}
...
TEST_F(FlatTreeTraversalTest, StartsAtOrAfter)
{
    ...
    HeapVector<Member<Node>> expected_nodes;
    for (Node* child = FlatTreeTraversal::Next(*s03); child; child = FlatTreeTraversal::Next(*child)) {
        expected_nodes.push_back(child);
    }
    EXPECT_EQ(expected_nodes, GatherFromTraversalRange(FlatTreeTraversal::StartsAfter(*s03)));

    expected_nodes.push_front(*s03);
    EXPECT_EQ(expected_nodes, GatherFromTraversalRange(FlatTreeTraversal::StartsAt(*s03)));
}

TEST_F(FlatTreeTraversalTest, ChildrenOf)
{
    SetupSampleHTML("<p id=sample>ZERO<span slot=three>three</b><span "
    ...
    HeapVector<Member<Node>> expected_nodes;
    for (Node* runner = FlatTreeTraversal::FirstChild(*sample); runner; runner = FlatTreeTraversal::NextSibling(*runner)) {
...
TEST_F(FlatTreeTraversalTest, commonAncestor)
{
    ...
    const char* main_html = "<a id='m0'><b id='m00'>m00</b><b id='m01'>m01</b></a>"
                            "<div id='m1'>"
    ...
    const char* shadow_html = "<a id='s10'>s10</a>"
...
{
    const char* main_html = "<div id='m0'>m0</div>"
                            "<div id='m1'>"
                            "<span slot='#m10' id='m10'>m10</span>"
                            "<span slot='#m11' id='m11'>m11</span>"
                            "</div>"
                            "<div id='m2'>m2</div>";
    const char* shadow_html = "<slot name='#m11'></slot>"
                              "<a id='s11'>s11</a>"
                              "<a id='s12'>"
                              "<b id='s120'>s120</b>"
                              "<slot name='#m10'></slot>"
...
TEST_F(FlatTreeTraversalTest, AncestorsOf)
{
    ...
    HeapVector<Member<Node>> expected_nodes;
    for (Node* parent = FlatTreeTraversal::Parent(*sample); parent; parent = FlatTreeTraversal::Parent(*parent)) {
...
TEST_F(FlatTreeTraversalTest, InclusiveAncestorsOf)
{
    ...
    HeapVector<Member<Node>> expected_nodes;
    for (Node* parent = sample; parent; parent = FlatTreeTraversal::Parent(*parent)) {
        expected_nodes.push_back(parent);
    }
...
TEST_F(FlatTreeTraversalTest, v1Simple)
{
    const char* main_html = "<div id='host'>"
                            ...
                            "</div>";
    const char* shadow_html = "<div id='shadow-child1'></div>"
    ...
    Element* shadow_child1 = shadow_root->QuerySelector(AtomicString("#shadow-child1"));
    Element* shadow_child2 = shadow_root->QuerySelector(AtomicString("#shadow-child2"));
...
TEST_F(FlatTreeTraversalTest, v1Redistribution)
{
    ...
    //     ├── slot name=d1-1-s2
    //     └── d1-1-2
    const char* main_html = "<div id='d1'>"
                            ...
                            "</div>"
                            "<div id='d6'></div>";
    const char* shadow_html1 = "<div id='d1-1'>"
                               "<div id='d1-2'></div>"
                               "<slot id='d1-s0'></slot>"
                               ...
                               "</div>";
    const char* shadow_html2 = "<div id='d1-1-1'></div>"
...
{
    const char* main_html = "<div id='parent'>"
                            "<slot>"
                            "<div id='child1'></div>"
                            "<div id='child2'></div>"
                            "</slot>"
...
TEST_F(FlatTreeTraversalTest, v1FallbackContent)
{
    ...
    const char* shadow_html = "<div id='before'></div>"
...
{
    const char* main_html = "<div id='d1'></div>";
    const char* shadow_html = "<slot name='a'><p id='x'>fallback content X</p></slot>"
