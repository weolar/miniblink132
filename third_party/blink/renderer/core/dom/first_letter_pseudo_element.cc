...
inline bool IsPunctuationForFirstLetter(UChar32 c)
{
    ...
    return char_category == WTF::unicode::kPunctuation_Open || char_category == WTF::unicode::kPunctuation_Close
        || char_category == WTF::unicode::kPunctuation_InitialQuote || char_category == WTF::unicode::kPunctuation_FinalQuote
...
{
    return (preserve_breaks ? IsSpace(c) : IsSpaceOrNewline(c)) || c == WTF::unicode::kNoBreakSpaceCharacter;
}
...
unsigned FirstLetterPseudoElement::FirstLetterLength(const String& text, bool preserve_breaks, Punctuation& punctuation)
{
    ...
    if (punctuation == Punctuation::kNotSeen) {
        while (length < text_length && IsSpaceForFirstLetter(text[length], preserve_breaks)) {
            length++;
        }
    ...
    if (IsSpaceForFirstLetter(text[length], preserve_breaks) || IsNewLine(text[length])) {
        return 0;
    }
...
LayoutText* FirstLetterPseudoElement::FirstLetterTextLayoutObject(const Element& element)
{
    ...
    if (element.IsFirstLetterPseudoElement()) {
        ...
        parent_layout_object = element.ParentOrShadowHostElement()->GetLayoutObject();
    } else {
        parent_layout_object = element.GetLayoutObject();
    }

    if (!parent_layout_object || !parent_layout_object->StyleRef().HasPseudoElementStyle(kPseudoIdFirstLetter)
        || !CanHaveGeneratedChildren(*parent_layout_object) || !parent_layout_object->BehavesLikeBlockContainer()) {
        // This element can not have a styleable ::first-letter.
        return nullptr;
    }

    LayoutObject* inline_child = FirstInFlowInlineDescendantForFirstLetter(*parent_layout_object);
    if (!inline_child) {
        return nullptr;
    }
    ...
    while (inline_child) {
        if (inline_child->StyleRef().StyleType() == kPseudoIdFirstLetter) {
            ...
            if (auto* layout_text = DynamicTo<LayoutText>(inline_child)) {
                ...
                String str = layout_text->IsTextFragment() ? To<LayoutTextFragment>(inline_child)->CompleteText() : layout_text->OriginalText();
                bool preserve_breaks = ShouldPreserveBreaks(inline_child->StyleRef().GetWhiteSpaceCollapse());
            ...
            } else if (inline_child->IsAtomicInlineLevel() || inline_child->IsMenuList()) {
                return nullptr;
            }
...
void FirstLetterPseudoElement::UpdateTextFragments()
{
    ...
    bool preserve_breaks = ShouldPreserveBreaks(remaining_text_layout_object_->StyleRef().GetWhiteSpaceCollapse());
    FirstLetterPseudoElement::Punctuation punctuation = FirstLetterPseudoElement::Punctuation::kNotSeen;
    unsigned length = FirstLetterPseudoElement::FirstLetterLength(old_text, preserve_breaks, punctuation);
    remaining_text_layout_object_->SetTextFragment(old_text.Impl()->Substring(length, old_text.length()), length, old_text.length() - length);
    remaining_text_layout_object_->InvalidateInlineItems();

    for (auto* child = GetLayoutObject()->SlowFirstChild(); child; child = child->NextSibling()) {
        ...
        child_fragment->SetTextFragment(old_text.Impl()->Substring(0, length), 0, length);
        ...
        child_fragment->SetNeedsLayoutAndIntrinsicWidthsRecalc(layout_invalidation_reason::kTextChanged);
        break;
    }
}
...
{
    LayoutText* first_letter_text = FirstLetterPseudoElement::FirstLetterTextLayoutObject(*this);
...
void FirstLetterPseudoElement::DetachLayoutTree(bool performing_reattach)
{
    if (remaining_text_layout_object_) {
        if (remaining_text_layout_object_->GetNode() && GetDocument().IsActive()) {
            ...
            remaining_text_layout_object_->SetTextFragment(text_node->data(), 0, text_node->data().length());
        }
...
LayoutObject* FirstLetterPseudoElement::CreateLayoutObject(const ComputedStyle& style)
{
...
const ComputedStyle* FirstLetterPseudoElement::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
    LayoutObject* first_letter_text = FirstLetterPseudoElement::FirstLetterTextLayoutObject(*this);
    if (!first_letter_text)
        return nullptr;
    DCHECK(first_letter_text->Parent());
    return ParentOrShadowHostElement()->StyleForPseudoElement(style_recalc_context, StyleRequest(GetPseudoId(), first_letter_text->Parent()->FirstLineStyle()));
}
...
void FirstLetterPseudoElement::AttachFirstLetterTextLayoutObjects(LayoutText* first_letter_text)
{
    ...
    // applied to it.
    String old_text = first_letter_text->IsTextFragment() ? To<LayoutTextFragment>(first_letter_text)->CompleteText() : first_letter_text->OriginalText();
    DCHECK(old_text.Impl());

    // FIXME: This would already have been calculated in firstLetterLayoutObject.
    // Can we pass the length through?
    bool preserve_breaks = ShouldPreserveBreaks(first_letter_text->StyleRef().GetWhiteSpaceCollapse());
    FirstLetterPseudoElement::Punctuation punctuation = FirstLetterPseudoElement::Punctuation::kNotSeen;
    unsigned length = FirstLetterPseudoElement::FirstLetterLength(old_text, preserve_breaks, punctuation);

    // In case of inline level content made of punctuation, we use
    // the whole text length instead of FirstLetterLength.
    if (IsParentInlineLayoutObject(first_letter_text) && length == 0 && old_text.length()) {
    ...
    if (first_letter_text->GetNode()) {
        remaining_text = LayoutTextFragment::Create(first_letter_text->GetNode(), old_text.Impl(), length, remaining_length);
    } else {
        remaining_text = LayoutTextFragment::CreateAnonymous(GetDocument(), old_text.Impl(), length, remaining_length);
    }
    ...
    LayoutTextFragment* letter = LayoutTextFragment::CreateAnonymous(GetDocument(), old_text.Impl(), 0, length);
    ...
    if (GetLayoutObject()->IsInitialLetterBox()) [[unlikely]] {
        ...
        const ComputedStyle& paragraph_style = paragraph.EffectiveStyle(StyleVariant::kFirstLine);
        const ComputedStyle* initial_letter_text_style = GetDocument().GetStyleResolver().StyleForInitialLetterText(*letter_style, paragraph_style);
