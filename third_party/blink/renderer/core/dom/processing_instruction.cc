...
ProcessingInstruction::ProcessingInstruction(Document& document, const String& target, const String& data)
...
CharacterData* ProcessingInstruction::CloneWithData(Document& factory, const String& data) const
{
...
{
    if (target_ != "xml-stylesheet" || !GetDocument().GetFrame() || parentNode() != GetDocument())
        return false;
    ...
    is_xsl_ = (type == "text/xml" || type == "text/xsl" || type == "application/xml" || type == "application/xhtml+xml" || type == "application/rss+xml"
...
void ProcessingInstruction::Process(const String& href, const String& charset)
{
    if (href.length() > 1 && href[0] == '#') {
        ...
        if (is_xsl_) {
            ...
            sheet_ = MakeGarbageCollected<XSLStyleSheet>(this, final_url.GetString(), final_url, true);
            loading_ = false;
        }
        return;
    }
    ...
    options.initiator_info.name = fetch_initiator_type_names::kProcessinginstruction;
    FetchParameters params(ResourceRequest(GetDocument().CompleteURL(href)), options);
    loading_ = true;
    if (is_xsl_) {
        ...
        params.SetCharset(charset.empty() ? GetDocument().Encoding() : WTF::TextEncoding(charset));
        GetDocument().GetStyleEngine().AddPendingBlockingSheet(*this, PendingSheetType::kBlocking);
...
void ProcessingInstruction::NotifyFinished(Resource* resource)
{
    ...
    std::unique_ptr<IncrementLoadEventDelayCount> delay = is_xsl_ ? std::make_unique<IncrementLoadEventDelayCount>(GetDocument()) : nullptr;
    if (is_xsl_) {
        sheet_ = MakeGarbageCollected<XSLStyleSheet>(this, resource->Url(), resource->GetResponse().ResponseUrl(), false);
        To<XSLStyleSheet>(sheet_.Get())->ParseString(To<XSLStyleSheetResource>(resource)->Sheet());
    } else {
        DCHECK(is_css_);
        auto* style_resource = To<CSSStyleSheetResource>(resource);
        auto* parser_context = MakeGarbageCollected<CSSParserContext>(GetDocument(), style_resource->GetResponse().ResponseUrl(),
            style_resource->GetResponse().IsCorsSameOrigin(), Referrer(style_resource->GetResponse().ResponseUrl(), style_resource->GetReferrerPolicy()),
            style_resource->Encoding());
        if (style_resource->GetResourceRequest().IsAdResource())
            parser_context->SetIsAdRelated();

        auto* new_sheet = MakeGarbageCollected<StyleSheetContents>(parser_context, style_resource->Url());
        ...
        css_sheet->SetMediaQueries(MediaQuerySet::Create(media_, GetExecutionContext()));
        sheet_ = css_sheet;
        // We don't need the cross-origin security check here because we are
        // getting the sheet text in "strict" mode. This enforces a valid CSS MIME
        // type.
        css_sheet->Contents()->ParseString(style_resource->SheetText(parser_context));
    }
...
Node::InsertionNotificationRequest ProcessingInstruction::InsertedInto(ContainerNode& insertion_point)
{
    ...
    if (!DocumentXSLT::ProcessingInstructionInsertedIntoDocument(GetDocument(), this))
...
void ProcessingInstruction::RemovedFrom(ContainerNode& insertion_point)
{
    ...
    if (!DocumentXSLT::ProcessingInstructionRemovedFromDocument(GetDocument(), this)) {
...
void ProcessingInstruction::RemovePendingSheet()
{
    if (is_xsl_)
        return;
    GetDocument().GetStyleEngine().RemovePendingBlockingSheet(*this, PendingSheetType::kBlocking);
}
