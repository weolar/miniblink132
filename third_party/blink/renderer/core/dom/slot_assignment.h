...
public:
    ...
    bool NeedsAssignmentRecalc() const
    {
        return needs_assignment_recalc_;
    }
...
private:
    ...
    void CollectSlots();
    HTMLSlotElement* GetCachedFirstSlotWithoutAccessingNodeTree(const AtomicString& slot_name);

    void DidAddSlotInternal(HTMLSlotElement&);
    void DidRemoveSlotInternal(HTMLSlotElement&, const AtomicString& slot_name, SlotMutationType);
