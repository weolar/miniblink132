...
void NamesMap::Add(const AtomicString& key, const AtomicString& value)
{
    ...
    if (add_result.is_new_entry) {
        add_result.stored_value->value = MakeGarbageCollected<SpaceSplitStringWrapper>();
    }
...
template <typename CharacterType> void NamesMap::Set(base::span<const CharacterType> characters)
{
    Clear();
    ...
    while (cur < characters.size()) {
        ...
        case kValue:
            ...
            } else if (IsHTMLSpace<CharacterType>(current_char) || IsColon<CharacterType>(current_char)) {
                value = AtomicString(characters.subspan(start, cur - start));
                state = kPostValue;
                // A colon is invalid.
            } else if (IsColon<CharacterType>(current_char)) {
