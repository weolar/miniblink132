...
ALWAYS_INLINE void SpaceSplitString::Data::CreateVector(const AtomicString& source, base::span<const CharacterType> characters)
{
    DCHECK(vector_.empty());
    HashSet<AtomicString> token_set;
    size_t start = 0;
    while (true) {
        while (start < characters.size() && IsHTMLSpace<CharacterType>(characters[start])) {
            ++start;
        }
        ...
        size_t end = start + 1;
        while (end < characters.size() && IsNotHTMLSpace<CharacterType>(characters[end])) {
            ++end;
        }
...
{
    WTF::VisitCharacters(string, [&](auto chars) { CreateVector(string, chars); });
}
...
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(ThreadSpecific<Persistent<DataMap>>, static_map_holder, {});
...
SpaceSplitString::Data* SpaceSplitString::Data::Create(const AtomicString& string)
{
...
SpaceSplitString::Data* SpaceSplitString::Data::CreateUnique(const Data& other)
{
