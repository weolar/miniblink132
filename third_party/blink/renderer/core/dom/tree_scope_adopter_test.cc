...
TEST(TreeScopeAdopterTest, MoveNestedShadowRoots)
{
    ...
    ShadowRoot& outer_shadow = outer_div->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    ...
    ASSERT_TRUE(middle_target->addEventListener(event_type_names::kMousewheel, listener, false));

    ShadowRoot& middle_shadow = middle_div->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    ...
    // side-effect too.
    ASSERT_TRUE(inner_div->addEventListener(event_type_names::kMousewheel, listener, false));
