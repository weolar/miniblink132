...
public:
    ...
    public:
        static scoped_refptr<QualifiedNameImpl> Create(StringImpl* prefix, StringImpl* local_name, StringImpl* namespace_uri, bool is_static)
        {
            return base::AdoptRef(new QualifiedNameImpl(prefix, local_name, namespace_uri, is_static));
        }
    ...
    private:
        QualifiedNameImpl(StringImpl* prefix, StringImpl* local_name, StringImpl* namespace_uri, bool is_static)
            : existing_hash_(0)
    ...
    {
        return impl_->prefix_ != g_null_atom;
    }
    ...
    bool IsDefinedName() const
    {
        return impl_ && impl_->is_static_;
    }

    String ToString() const;

    QualifiedNameImpl* Impl() const
    {
        return impl_.get();
    }
    ...
    static void CreateStatic(void* target_address, StringImpl* name, const AtomicString& name_namespace);
...
private:
    ...
    QualifiedName(const AtomicString& prefix, const AtomicString& local_name, const AtomicString& namespace_uri, bool is_static);
...
template <> struct HashTraits<blink::QualifiedName::QualifiedNameImpl*> : GenericHashTraits<blink::QualifiedName::QualifiedNameImpl*> {
...
template <> struct HashTraits<blink::QualifiedName> : GenericHashTraits<blink::QualifiedName> {
    ...
    {
        return HashTraits<scoped_refptr<QualifiedNameImpl>>::IsDeletedValue(value.impl_);
    }
    static void ConstructDeletedValue(blink::QualifiedName& slot)
    {
        HashTraits<scoped_refptr<QualifiedNameImpl>>::ConstructDeletedValue(slot.impl_);
    }
};
