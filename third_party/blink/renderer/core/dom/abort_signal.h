...
public:
    ...
    AbortSignal(ScriptState*, const HeapVector<Member<AbortSignal>>& source_signals);
    ...
    static AbortSignal* any(ScriptState*, HeapVector<Member<AbortSignal>> signals);
    static AbortSignal* timeout(ScriptState*, uint64_t milliseconds);
    ScriptValue reason(ScriptState*) const;
    bool aborted() const
    {
        return !abort_reason_.IsEmpty();
    }
    ...
    virtual bool IsTaskSignal() const
    {
        return false;
    }
    ...
    virtual AbortSignalCompositionManager* GetCompositionManager(AbortSignalCompositionType);
...
private:
    void InitializeCompositeSignal(const HeapVector<Member<AbortSignal>>& source_signals);

    void AbortTimeoutFired(ScriptState*);

    enum class AddRemoveType { kAdded, kRemoved };
    void OnEventListenerAddedOrRemoved(const AtomicString& event_type, AddRemoveType);
