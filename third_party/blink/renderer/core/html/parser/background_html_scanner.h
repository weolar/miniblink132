...
public:
    ...
    public:
        static std::unique_ptr<ScriptTokenScanner> Create(ScriptableDocumentParser* parser);

        ScriptTokenScanner(ScriptableDocumentParser* parser, scoped_refptr<base::SequencedTaskRunner> task_runner, wtf_size_t min_script_size);

        void ScanToken(const HTMLToken& token);

        void set_first_script_in_scan(bool value)
        {
            first_script_in_scan_ = value;
        }
    ...
    static WTF::SequenceBound<BackgroundHTMLScanner> Create(const HTMLParserOptions& options, ScriptableDocumentParser* parser);
    BackgroundHTMLScanner(std::unique_ptr<HTMLTokenizer> tokenizer, std::unique_ptr<ScriptTokenScanner> token_scanner);
