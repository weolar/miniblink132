...
TEST(AtomicHTMLTokenTest, EmptyAttributeValueFromHTMLToken)
{
    ...
    const blink::Attribute* attribute_b = atoken.GetAttributeItem(QualifiedName(AtomicString(), AtomicString("b"), AtomicString()));
    ASSERT_TRUE(attribute_b);
    EXPECT_FALSE(attribute_b->Value().IsNull());
    EXPECT_TRUE(attribute_b->Value().empty());

    const blink::Attribute* attribute_c = atoken.GetAttributeItem(QualifiedName(AtomicString(), AtomicString("c"), AtomicString()));
    ASSERT_TRUE(attribute_c);
    EXPECT_FALSE(attribute_c->Value().IsNull());
    EXPECT_TRUE(attribute_c->Value().empty());

    const blink::Attribute* attribute_d = atoken.GetAttributeItem(QualifiedName(AtomicString(), AtomicString("d"), AtomicString()));
