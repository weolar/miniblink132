...
{
    static const base::FeatureParam<CompileStrategy>::Option kCompileStrategyOptions[] = {
        { CompileStrategy::kLazy, "lazy" },
        { CompileStrategy::kFirstScriptLazy, "first-script-lazy" },
        { CompileStrategy::kEager, "eager" },
    };

    static const base::FeatureParam<CompileStrategy> kCompileStrategyParam { &features::kPrecompileInlineScripts, "compile-strategy", CompileStrategy::kLazy,
    ...
    case CompileStrategy::kFirstScriptLazy:
        return first_script_in_scan ? CompileOptions::kNoCompileOptions : CompileOptions::kEagerCompile;
...
{
    static const base::FeatureParam<bool> kCompileInParallelParam { &features::kPrecompileInlineScripts, "compile-in-parallel", false };
...
{
    static const base::FeatureParam<int> kMinimumScriptSizeParam { &features::kPrecompileInlineScripts, "minimum-script-size", 0 };
    // Cache the value to avoid parsing the param string more than once.
    static const wtf_size_t kMinimumScriptSizeValue = static_cast<wtf_size_t>(kMinimumScriptSizeParam.Get());
...
bool ShouldPrecompileFrame(bool is_main_frame)
{
    ...
    static const base::FeatureParam<bool> kPrecompileMainFrameOnlyParam { &features::kPrecompileInlineScripts, "precompile-main-frame-only", true };
    // Cache the value to avoid parsing the param string more than once.
    static const bool kPrecompileMainFrameOnlyValue = kPrecompileMainFrameOnlyParam.Get();
...
// static
WTF::SequenceBound<BackgroundHTMLScanner> BackgroundHTMLScanner::Create(const HTMLParserOptions& options, ScriptableDocumentParser* parser)
{
    ...
    return WTF::SequenceBound<BackgroundHTMLScanner>(
        worker_pool::CreateSequencedTaskRunner({ base::TaskPriority::USER_BLOCKING }), std::make_unique<HTMLTokenizer>(options), std::move(token_scanner));
}

BackgroundHTMLScanner::BackgroundHTMLScanner(std::unique_ptr<HTMLTokenizer> tokenizer, std::unique_ptr<ScriptTokenScanner> token_scanner)
...
std::unique_ptr<BackgroundHTMLScanner::ScriptTokenScanner> BackgroundHTMLScanner::ScriptTokenScanner::Create(ScriptableDocumentParser* parser)
{
    bool is_main_frame = parser->GetDocument() && parser->GetDocument()->IsInOutermostMainFrame();
    bool precompile_scripts = ShouldPrecompileFrame(is_main_frame);
    if (!precompile_scripts) {
        return nullptr;
    }
    return std::make_unique<ScriptTokenScanner>(parser, GetCompileTaskRunner(), GetMinimumScriptSize());
}

BackgroundHTMLScanner::ScriptTokenScanner::ScriptTokenScanner(
    ScriptableDocumentParser* parser, scoped_refptr<base::SequencedTaskRunner> task_runner, wtf_size_t min_script_size)
...
void BackgroundHTMLScanner::ScriptTokenScanner::ScanToken(const HTMLToken& token)
{
    ...
    case HTMLToken::kEndTag: {
        if (Match(TagImplFor(token.Data()), html_names::kScriptTag)) {
            ...
            auto streamer = base::MakeRefCounted<BackgroundInlineScriptStreamer>(isolate_, script_text, GetCompileOptions(first_script_in_scan_));
            ...
            if (task_runner_) {
                PostCrossThreadTask(*task_runner_, FROM_HERE, CrossThreadBindOnce(&BackgroundInlineScriptStreamer::Run, std::move(streamer)));
            } else {
                worker_pool::PostTask(
                    FROM_HERE, { base::TaskPriority::USER_BLOCKING }, CrossThreadBindOnce(&BackgroundInlineScriptStreamer::Run, std::move(streamer)));
            }
        }
        return;
    }
