...
protected:
    void SetUp() override
    {
        ...
        GetFrame().SetPrescientNetworkingForTesting(std::make_unique<PreloaderNetworkHintsMock>());
        mock_network_hints_ = static_cast<PreloaderNetworkHintsMock*>(GetFrame().PrescientNetworking());
    }
    ...
    void Test(HTMLResourcePreconnectTestCase test_case)
    {
        ...
        // beyond preconnect.
        auto preload_request = PreloadRequest::CreateIfNeeded(String(), test_case.url, KURL(test_case.base_url), ResourceType::kImage,
            network::mojom::ReferrerPolicy(), ResourceFetcher::kImageNotImageSet, nullptr /* exclusion_info */, std::nullopt /* resource_width */,
            std::nullopt /* resource_height */, PreloadRequest::kRequestTypePreconnect);
        DCHECK(preload_request);
        if (test_case.is_cors)
            preload_request->SetCrossOrigin(kCrossOriginAttributeAnonymous);
        auto* preloader = MakeGarbageCollected<HTMLResourcePreloader>(GetDocument());
