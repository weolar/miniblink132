...
PreloadRequest::ExclusionInfo::ExclusionInfo(const KURL& document_url, HashSet<KURL> scopes, HashSet<KURL> resources)
...
bool PreloadRequest::ExclusionInfo::ShouldExclude(const KURL& base_url, const String& resource_url) const
{
...
KURL PreloadRequest::CompleteURL(Document* document)
{
    if (!base_url_.IsEmpty()) {
        return document->CompleteURLWithOverride(resource_url_, base_url_, Document::kIsPreload);
    }
    return document->CompleteURL(resource_url_, Document::kIsPreload);
}

// static
std::unique_ptr<PreloadRequest> PreloadRequest::CreateIfNeeded(const String& initiator_name, const String& resource_url, const KURL& base_url,
    ResourceType resource_type, const network::mojom::ReferrerPolicy referrer_policy, ResourceFetcher::IsImageSet is_image_set,
    const ExclusionInfo* exclusion_info, std::optional<float> resource_width, std::optional<float> resource_height, RequestType request_type)
{
    ...
    if (resource_url.empty() || resource_url.StartsWith("#") || ProtocolIs(resource_url, "data")) {
        return nullptr;
    }
    ...
    return base::WrapUnique(new PreloadRequest(
        initiator_name, resource_url, base_url, resource_type, resource_width, resource_height, request_type, referrer_policy, is_image_set));
}

Resource* PreloadRequest::Start(Document* document)
{
    DCHECK(document->domWindow());
    base::UmaHistogramTimes("Blink.PreloadRequestWaitTime", base::TimeTicks::Now() - creation_time_);
    ...
    resource_request.SetRequestContext(ResourceFetcher::DetermineRequestContext(resource_type_, is_image_set_));
    resource_request.SetRequestDestination(ResourceFetcher::DetermineRequestDestination(resource_type_));
    ...
    if (is_attribution_reporting_eligible_img_or_script_
        && document->domWindow()->GetFrame()->GetAttributionSrcLoader()->CanRegister(url, /*element=*/nullptr,
    ...
    bool shared_storage_writable_opted_in = shared_storage_writable_opted_in_ && RuntimeEnabledFeatures::SharedStorageAPIM118Enabled(document->domWindow())
        && document->domWindow()->IsSecureContext() && !document->domWindow()->GetSecurityOrigin()->IsOpaque();
    ...
    if (script_type_ == mojom::blink::ScriptType::kModule) {
        ...
        params.SetCrossOriginAccessControl(origin, ScriptLoader::ModuleScriptCredentialsMode(cross_origin_));
    ...
    } else if (resource_type_ == ResourceType::kScript || resource_type_ == ResourceType::kCSSStyleSheet) {
        params.SetCharset(charset_.empty() ? document->Encoding() : WTF::TextEncoding(charset_));
    }
    FetchParameters::SpeculativePreloadType speculative_preload_type = FetchParameters::SpeculativePreloadType::kInDocument;
    if (from_insertion_scanner_) {
        speculative_preload_type = FetchParameters::SpeculativePreloadType::kInserted;
    }
