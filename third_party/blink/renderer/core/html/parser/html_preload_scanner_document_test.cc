...
public:
    bool DidDnsPrefetch() const
    {
        return did_dns_prefetch_;
    }
    bool DidPreconnect() const
    {
        return did_preconnect_;
    }

private:
    void PrefetchDNS(const WebURL&) override
    {
        did_dns_prefetch_ = true;
    }
    void Preconnect(const WebURL&, bool) override
    {
...
private:
    void SetUp() override
    {
        ...
        frame->SetPrescientNetworkingForTesting(std::make_unique<MockPrescientNetworking>());
        mock_network_hints_ = static_cast<MockPrescientNetworking*>(frame->PrescientNetworking());
...
TEST_F(HTMLPreloadScannerDocumentTest, SetsClientHintsPreferencesOnFrameDelegateCH)
{
    // Create a prefetch only document since that will ensure only the preload
    // scanner runs.
    ProvideNoStatePrefetchClientTo(*GetDocument().GetPage(), MakeGarbageCollected<MockNoStatePrefetchClient>(*GetDocument().GetPage()));
    EXPECT_TRUE(GetDocument().IsPrefetchOnly());
    main_resource_->Complete(R"(<meta http-equiv="Delegate-CH" content="sec-ch-dpr">)");
    EXPECT_TRUE(GetDocument().GetFrame()->GetClientHintsPreferences().ShouldSend(network::mojom::WebClientHintsType::kDpr));
}

TEST_F(HTMLPreloadScannerDocumentTest, SetsClientHintsPreferencesOnFrameAcceptCH)
{
    // Create a prefetch only document since that will ensure only the preload
    // scanner runs.
    ProvideNoStatePrefetchClientTo(*GetDocument().GetPage(), MakeGarbageCollected<MockNoStatePrefetchClient>(*GetDocument().GetPage()));
    EXPECT_TRUE(GetDocument().IsPrefetchOnly());
    main_resource_->Complete(R"(<meta http-equiv="Accept-CH" content="sec-ch-dpr">)");
    EXPECT_TRUE(GetDocument().GetFrame()->GetClientHintsPreferences().ShouldSend(network::mojom::WebClientHintsType::kDpr));
}
