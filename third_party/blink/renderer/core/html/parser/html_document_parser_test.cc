...
private:
    ...
    {
        return is_prefetch_only_;
    }

    bool is_prefetch_only_;
};

class HTMLDocumentParserTest : public PageTestBase, public testing::WithParamInterface<ParserSynchronizationPolicy> {
protected:
    HTMLDocumentParserTest()
        : original_force_synchronous_parsing_for_testing_(Document::ForceSynchronousParsingForTesting())
    {
        Document::SetForceSynchronousParsingForTesting(Policy() == kForceSynchronousParsing);
    }
    ~HTMLDocumentParserTest() override
    {
        ...
        Document::SetForceSynchronousParsingForTesting(original_force_synchronous_parsing_for_testing_);
    }
    ...
    {
        auto* parser = MakeGarbageCollected<HTMLDocumentParser>(document, GetParam());
        std::unique_ptr<TextResourceDecoder> decoder(BuildTextResourceDecoder(document.GetFrame(), document.Url(), AtomicString("text/html"), g_null_atom));
...
public:
    explicit ScopedParserDetacher(DocumentParser* parser)
        : parser_(parser)
    {
    }
...
INSTANTIATE_TEST_SUITE_P(HTMLDocumentParserTest, HTMLDocumentParserTest, testing::Values(kForceSynchronousParsing, kAllowDeferredParsing));
...
{
    auto& document = To<HTMLDocument>(GetDocument());
    ProvideNoStatePrefetchClientTo(*document.GetPage(), MakeGarbageCollected<MockNoStatePrefetchClient>(*document.GetPage(), true));
    ...
    HTMLParserScriptRunnerHost* script_runner_host = parser->AsHTMLParserScriptRunnerHostForTesting();
...
TEST_P(HTMLDocumentParserTest, AppendNoPrefetch)
{
    ...
    HTMLParserScriptRunnerHost* script_runner_host = parser->AsHTMLParserScriptRunnerHostForTesting();
    EXPECT_EQ(script_runner_host->HasPreloadScanner(), GetParam() == kAllowDeferredParsing);
...
protected:
    ...
    {
        scoped_feature_list_.InitWithFeatures({ features::kThreadedPreloadScanner, features::kPrecompileInlineScripts }, {});
    ...
    HTMLDocumentParser* CreateParser(HTMLDocument& document)
    {
        return MakeGarbageCollected<HTMLDocumentParser>(document, kAllowDeferredParsing);
    }
...
TEST_F(HTMLDocumentParserThreadedPreloadScannerTest, TakeBackgroundScanCallback)
{
    auto& document = To<HTMLDocument>(GetDocument());
    HTMLDocumentParser* parser = CreateParser(document);
    ScopedParserDetacher detacher(parser);
    ...
    auto callback = static_cast<DocumentParser*>(parser)->TakeBackgroundScanCallback();
...
protected:
    ...
    {
        auto* parser = MakeGarbageCollected<HTMLDocumentParser>(document, kAllowDeferredParsing);
        std::unique_ptr<TextResourceDecoder> decoder(BuildTextResourceDecoder(document.GetFrame(), document.Url(), AtomicString("text/html"), g_null_atom));
        parser->SetDecoder(std::move(decoder));
        return parser;
    }

    static HTMLDocumentParser* ConfigureWebViewHelperForChildFrameAndCreateParser(frame_test_helpers::WebViewHelper& web_view_helper)
    {
        ...
        return child_document ? CreateParser(To<HTMLDocument>(*child_document)) : nullptr;
    }
};
...
TEST_F(HTMLDocumentParserProcessImmediatelyTest, FirstChunk)
{
    ...
    scoped_feature_list.InitAndEnableFeatureWithParameters(features::kProcessHtmlDataImmediately,
        { { features::kProcessHtmlDataImmediatelyFirstChunk.name, "true" }, { features::kProcessHtmlDataImmediatelyMainFrame.name, "true" } });
    auto& document = To<HTMLDocument>(GetDocument());
    HTMLDocumentParser* parser = CreateParser(document);
    ScopedParserDetacher detacher(parser);
    parser->AppendBytes(base::byte_span_from_cstring("<htttttt"));
...
TEST_F(HTMLDocumentParserProcessImmediatelyTest, SecondChunk)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeatureWithParameters(features::kProcessHtmlDataImmediately,
        { { features::kProcessHtmlDataImmediatelySubsequentChunks.name, "true" }, { features::kProcessHtmlDataImmediatelyMainFrame.name, "true" } });
    auto& document = To<HTMLDocument>(GetDocument());
    HTMLDocumentParser* parser = CreateParser(document);
    ScopedParserDetacher detacher(parser);
...
TEST_F(HTMLDocumentParserProcessImmediatelyTest, FirstChunkChildFrame)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeatureWithParameters(features::kProcessHtmlDataImmediately,
        { { features::kProcessHtmlDataImmediatelyChildFrame.name, "true" }, { features::kProcessHtmlDataImmediatelyFirstChunk.name, "true" } });
    frame_test_helpers::WebViewHelper web_view_helper;
    HTMLDocumentParser* parser = ConfigureWebViewHelperForChildFrameAndCreateParser(web_view_helper);
    ASSERT_TRUE(parser);
    ScopedParserDetacher detacher(parser);
...
TEST_F(HTMLDocumentParserProcessImmediatelyTest, FirstChunkDelayedChildFrame)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeatureWithParameters(features::kProcessHtmlDataImmediately,
        { { features::kProcessHtmlDataImmediatelyChildFrame.name, "true" }, { features::kProcessHtmlDataImmediatelyFirstChunk.name, "false" } });
    frame_test_helpers::WebViewHelper web_view_helper;
    HTMLDocumentParser* parser = ConfigureWebViewHelperForChildFrameAndCreateParser(web_view_helper);
    ASSERT_TRUE(parser);
    ScopedParserDetacher detacher(parser);
