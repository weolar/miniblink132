...
// UCharLiteralBuffer.
template <typename T, wtf_size_t kInlineSize> class LiteralBufferBase {
    static_assert(std::is_same<LChar, T>::value || std::is_same<UChar, T>::value, "T must be a character type");
...
protected:
    ...
    template <typename OtherT, wtf_size_t kOtherSize> void AppendLiteralImpl(const LiteralBufferBase<OtherT, kOtherSize>& val)
    {
        static_assert(sizeof(T) >= sizeof(OtherT), "T is not big enough to contain OtherT");
    ...
    template <wtf_size_t kOtherInlineSize> void Copy(const LiteralBufferBase<T, kOtherInlineSize>& other)
    {
        ...
        if (capacity() < other_size) {
            ...
            if (!is_stored_inline())
                WTF::Partitions::BufferFree(begin_);
            begin_ = static_cast<T*>(WTF::Partitions::BufferMalloc(AllocationSize(other_size), "LiteralBufferBase"));
...
private:
    ...
    size_t RoundUpToPowerOfTwo(size_t value)
    {
        ...
        static_assert(digits == 32 || digits == 64, "size_t must be either 32 or 64 bits");
    ...
    NOINLINE T* Grow(size_t min_capacity)
    {
        ...
        size_t new_capacity = RoundUpToPowerOfTwo(std::max(min_capacity, 2 * capacity()));
        T* new_storage = static_cast<T*>(WTF::Partitions::BufferMalloc(AllocationSize(new_capacity), "LiteralBufferBase"));
...
template <wtf_size_t kInlineSize> class LCharLiteralBuffer : public LiteralBufferBase<LChar, kInlineSize> {
public:
    LCharLiteralBuffer() = default;
    LCharLiteralBuffer(const LCharLiteralBuffer& other)
    {
        *this = other;
    }
    LCharLiteralBuffer(LCharLiteralBuffer&& other)
    {
        *this = std::move(other);
    }
    ...
    template <wtf_size_t kOtherInlineSize> LCharLiteralBuffer& operator=(const LCharLiteralBuffer<kOtherInlineSize>& other)
    {
    ...
    ALWAYS_INLINE void AddChar(LChar val)
    {
...
template <wtf_size_t kInlineSize> class UCharLiteralBuffer : public LiteralBufferBase<UChar, kInlineSize> {
public:
    UCharLiteralBuffer() = default;
    UCharLiteralBuffer(const UCharLiteralBuffer& other)
    {
        *this = other;
    }
    UCharLiteralBuffer(UCharLiteralBuffer&& other)
    {
        *this = std::move(other);
    }
    ...
    template <wtf_size_t kOtherInlineSize> UCharLiteralBuffer& operator=(const UCharLiteralBuffer<kOtherInlineSize>& other)
    {
        if (this->data() == other.data())
            return *this;
        this->Copy(other);
    ...
    template <wtf_size_t kOtherSize> void AppendLiteral(const LCharLiteralBuffer<kOtherSize>& val)
    {
    ...
    {
        if (Is8Bit()) {
            return String::Make8BitFrom16BitSource(base::span<const UChar>(this->data(), this->size()));
        }
    ...
    {
        return AtomicString(
            base::span<const UChar>(this->data(), this->size()), Is8Bit() ? WTF::AtomicStringUCharEncoding::kIs8Bit : WTF::AtomicStringUCharEncoding::kIs16Bit);
    }
...
private:
    // Needed for operator=.
    template <wtf_size_t kOtherInlineSize> friend class UCharLiteralBuffer;
