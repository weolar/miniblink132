...
public:
    HTMLMetricsTest()
    {
        ...
        helper_.LocalMainFrame()->GetFrame()->GetDocument()->GetSettings()->SetDoHtmlPreloadScanning(false);
    }
    ...
    {
        frame_test_helpers::LoadHTMLString(helper_.GetWebView()->MainFrameImpl(), html, url_test_helpers::ToKURL("https://www.foo.com/"));
    }
...
TEST_F(HTMLMetricsTest, DISABLED_ReportSingleChunk)
{
    ...
    histogram_tester.ExpectTotalCount("Blink.HTMLParsing.TokensParsedAverage4", 1);
    ...
    histogram_tester.ExpectUniqueSample("Blink.HTMLParsing.TokensParsedMax4", 5, 1);
    histogram_tester.ExpectUniqueSample("Blink.HTMLParsing.TokensParsedMin4", 5, 1);
    histogram_tester.ExpectUniqueSample("Blink.HTMLParsing.TokensParsedAverage4", 5, 1);
    histogram_tester.ExpectUniqueSample("Blink.HTMLParsing.TokensParsedTotal4", 5, 1);

    // Expect that the times have moved from the default and the max and min
    // and total are all the same (within the same bucket)
    std::vector<base::Bucket> parsing_time_max_buckets = histogram_tester.GetAllSamples("Blink.HTMLParsing.ParsingTimeMax4");
    std::vector<base::Bucket> parsing_time_min_buckets = histogram_tester.GetAllSamples("Blink.HTMLParsing.ParsingTimeMin4");
    std::vector<base::Bucket> parsing_time_total_buckets = histogram_tester.GetAllSamples("Blink.HTMLParsing.ParsingTimeTotal4");
    ...
    histogram_tester.ExpectUniqueSample("Blink.HTMLParsing.InputCharacterCount4", 19, 1);
}
...
TEST_F(HTMLMetricsTest, DISABLED_HistogramReportsTwoChunks)
{
    ...
    histogram_tester.ExpectUniqueSample("Blink.HTMLParsing.ChunkCount4", 1, 1);
    histogram_tester.ExpectUniqueSample("Blink.HTMLParsing.TokensParsedMax4", 258, 1);
    histogram_tester.ExpectUniqueSample("Blink.HTMLParsing.TokensParsedMin4", 268, 1);
    histogram_tester.ExpectUniqueSample("Blink.HTMLParsing.TokensParsedAverage4", 258, 1);
    histogram_tester.ExpectUniqueSample("Blink.HTMLParsing.TokensParsedTotal4", 203, 1);
    ...
    std::vector<base::Bucket> yield_time_max_buckets = histogram_tester.GetAllSamples("Blink.HTMLParsing.YieldedTimeMax4");
    std::vector<base::Bucket> yield_time_min_buckets = histogram_tester.GetAllSamples("Blink.HTMLParsing.YieldedTimeMin4");
    std::vector<base::Bucket> yield_time_average_buckets = histogram_tester.GetAllSamples("Blink.HTMLParsing.YieldedTimeAverage4");
    ...
    histogram_tester.ExpectUniqueSample("Blink.HTMLParsing.InputCharacterCount4", 1447, 1);
}
...
TEST_F(HTMLMetricsTest, UkmStoresValuesCorrectly)
{
    ...
    const int64_t* metric_value = ukm::TestUkmRecorder::GetEntryMetric(entry, "ChunkCount");
    ...
    metric_value = ukm::TestUkmRecorder::GetEntryMetric(entry, "ParsingTimeTotal");
    EXPECT_EQ(*metric_value, (first_parse_time + second_parse_time + third_parse_time).InMicroseconds());
    ...
    EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entry, "TokensParsedAverage"));
    metric_value = ukm::TestUkmRecorder::GetEntryMetric(entry, "TokensParsedAverage");
    EXPECT_EQ(*metric_value, (first_tokens_parsed + second_tokens_parsed + third_tokens_parsed) / 3);

    EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entry, "TokensParsedTotal"));
    metric_value = ukm::TestUkmRecorder::GetEntryMetric(entry, "TokensParsedTotal");
    EXPECT_EQ(*metric_value, first_tokens_parsed + second_tokens_parsed + third_tokens_parsed);
    ...
    EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entry, "YieldedTimeAverage"));
    metric_value = ukm::TestUkmRecorder::GetEntryMetric(entry, "YieldedTimeAverage");
    EXPECT_EQ(*metric_value, ((first_yield_time + second_yield_time) / 2).InMicroseconds());
}
