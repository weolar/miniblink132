...
using HTMLFragmentParsingBehaviorSet
    = base::EnumSet<HTMLFragmentParsingBehavior, HTMLFragmentParsingBehavior::kMinValue, HTMLFragmentParsingBehavior::kMaxValue>;

// If this fails because of an unsupported tag and
// `failed_because_unsupported_tag` is non-null, then it is set to true.
CORE_EXPORT bool TryParsingHTMLFragment(const WTF::String& source, Document& document, ContainerNode& root_node, Element& context_element,
    ParserContentPolicy policy, HTMLFragmentParsingBehaviorSet behavior, bool* failed_because_unsupported_tag = nullptr);
