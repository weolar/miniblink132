...
String DecodeByteByByte(TextResourceDecoder& decoder, base::span<const uint8_t> data)
{
    String decoded;
...
TEST(TextResourceDecoderTest, UTF8Decode)
{
    ...
    std::unique_ptr<TextResourceDecoder> decoder = std::make_unique<TextResourceDecoder>(TextResourceDecoderOptions::CreateUTF8Decode());
...
{
    test::TaskEnvironment task_environment;
    std::unique_ptr<TextResourceDecoder> decoder = std::make_unique<TextResourceDecoder>(TextResourceDecoderOptions::CreateUTF8DecodeWithoutBOM());
    const unsigned char kFooUTF8WithBOM[] = { 0xef, 0xbb, 0xbf, 0x66, 0x6f, 0x6f };
    WTF::String decoded = decoder->Decode(base::span(kFooUTF8WithBOM));
    decoded = decoded + decoder->Flush();
    EXPECT_EQ(WTF::UTF8Encoding(), decoder->Encoding());
...
TEST(TextResourceDecoderTest, BasicUTF16)
{
    ...
    std::unique_ptr<TextResourceDecoder> decoder
        = std::make_unique<TextResourceDecoder>(TextResourceDecoderOptions(TextResourceDecoderOptions::kPlainTextContent));
    WTF::String decoded;

    const unsigned char kFooLE[] = { 0xff, 0xfe, 0x66, 0x00, 0x6f, 0x00, 0x6f, 0x00 };
    decoded = decoder->Decode(base::span(kFooLE));
    decoded = decoded + decoder->Flush();
    EXPECT_EQ("foo", decoded);

    decoder = std::make_unique<TextResourceDecoder>(TextResourceDecoderOptions(TextResourceDecoderOptions::kPlainTextContent));
    const unsigned char kFooBE[] = { 0xfe, 0xff, 0x00, 0x66, 0x00, 0x6f, 0x00, 0x6f };
...
{
    test::TaskEnvironment task_environment;
    std::unique_ptr<TextResourceDecoder> decoder = std::make_unique<TextResourceDecoder>(TextResourceDecoderOptions(TextResourceDecoderOptions::kHTMLContent));

    String decoded = DecodeByteByByte(*decoder, base::byte_span_from_cstring("<?xml encoding='utf-8'?>foo"));
    EXPECT_EQ(UTF8Encoding(), decoder->Encoding());
...
{
    test::TaskEnvironment task_environment;
    std::unique_ptr<TextResourceDecoder> decoder = std::make_unique<TextResourceDecoder>(TextResourceDecoderOptions(TextResourceDecoderOptions::kCSSContent));

    String decoded = DecodeByteByByte(*decoder, base::byte_span_from_cstring("@charset \"utf-8\";\n:root{}"));
    EXPECT_EQ(UTF8Encoding(), decoder->Encoding());
...
TEST(TextResourceDecoderTest, ContentSniffingStopsAfterSuccess)
{
    ...
    std::unique_ptr<TextResourceDecoder> decoder = std::make_unique<TextResourceDecoder>(
        TextResourceDecoderOptions::CreateWithAutoDetection(TextResourceDecoderOptions::kPlainTextContent, WTF::UTF8Encoding(), WTF::UTF8Encoding(), KURL("")));

    std::string utf8_bytes = "tnegirjji gosa gii beare s\xC3\xA1htt\xC3\xA1 \xC4\x8D\xC3"
                             "\xA1llit artihkkaliid. Maid don s\xC3\xA1ht\xC3\xA1t dievasmah";

    std::string eucjp_bytes = "<TITLE>"
