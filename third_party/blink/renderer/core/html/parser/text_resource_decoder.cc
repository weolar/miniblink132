...
template <typename... Bytes> static inline bool BytesEqual(base::span<const char> bytes, Bytes... bytes_sequence)
{
...
{
    const wtf_size_t length = base::checked_cast<wtf_size_t>(encoding_name.size());
    Vector<char, 64> buffer(length + 1);
    base::span(buffer).copy_prefix_from(base::span<const char>(encoding_name.data(), encoding_name.size()));
    buffer[length] = '\0';
    return WTF::TextEncoding(buffer.data());
}

const WTF::TextEncoding& TextResourceDecoder::DefaultEncoding(
    TextResourceDecoderOptions::ContentType content_type, const WTF::TextEncoding& specified_default_encoding)
{
    ...
    if (content_type == TextResourceDecoderOptions::kXMLContent || content_type == TextResourceDecoderOptions::kJSONContent)
        return UTF8Encoding();
...
TextResourceDecoder::TextResourceDecoder(const TextResourceDecoderOptions& options)
    : options_(options)
    , encoding_(DefaultEncoding(options_.GetContentType(), options_.DefaultEncoding()))
    ...
    , detection_completed_(false)
{
    ...
    if (options_.GetEncodingDetectionOption() == TextResourceDecoderOptions::kAlwaysUseUTF8ForText) {
        DCHECK_EQ(options_.GetContentType(), TextResourceDecoderOptions::kPlainTextContent);
...
void TextResourceDecoder::SetEncoding(const WTF::TextEncoding& encoding, EncodingSource source)
{
    ...
    if (source == kEncodingFromMetaTag && WTF::EqualIgnoringASCIICase(encoding.GetName(), "x-user-defined"))
        encoding_ = WTF::TextEncoding("windows-1252");
    else if (source == kEncodingFromMetaTag || source == kEncodingFromXMLHeader || source == kEncodingFromCSSCharset)
...
wtf_size_t TextResourceDecoder::CheckForBOM(base::span<const char> data)
{
    ...
    if (c1 == 0xEF && c2 == 0xBB && c3 == 0xBF) {
        ...
        length_of_bom = 3;
    } else if (options_.GetEncodingDetectionOption() != TextResourceDecoderOptions::kAlwaysUseUTF8ForText) {
        if (c1 == 0xFE && c2 == 0xFF) {
...
bool TextResourceDecoder::CheckForCSSCharset(base::span<const char> data)
{
    ...
    if (BytesEqual(data, '@', 'c', 'h', 'a', 'r', 's', 'e', 't', ' ', '"')) {
        ...
        if (*it == ';') {
            const auto encoding_name = base::as_string_view(data.first(encoding_name_length));
...
bool TextResourceDecoder::CheckForXMLCharset(base::span<const char> data)
{
    ...
    if (BytesEqual(data, '<', '?', 'x', 'm', 'l')) {
        ...
        const std::string_view encoding_name = FindXMLEncoding(base::as_string_view(data.first(search_length)));
...
//   in the first place.
void TextResourceDecoder::AutoDetectEncodingIfAllowed(base::span<const char> data)
{
    if (options_.GetEncodingDetectionOption() != TextResourceDecoderOptions::kUseAllAutoDetection || detection_completed_)
        return;
    ...
    if (!(source_ == kDefaultEncoding || (source_ == kEncodingFromParentFrame && options_.HintEncoding())))
        return;

    WTF::TextEncoding detected_encoding;
    if (DetectTextEncoding(base::as_bytes(data), options_.HintEncoding().Utf8().c_str(), options_.HintURL(), options_.HintLanguage(), &detected_encoding)) {
...
String TextResourceDecoder::Decode(base::span<const char> data)
{
    ...
    if (options_.GetContentType() == TextResourceDecoderOptions::kCSSContent && !checked_for_css_charset_) {
    ...
    if ((options_.GetContentType() == TextResourceDecoderOptions::kHTMLContent || options_.GetContentType() == TextResourceDecoderOptions::kXMLContent)
    ...
    if (options_.GetContentType() == TextResourceDecoderOptions::kHTMLContent && !checked_for_meta_charset_)
    ...
    String result = codec_->Decode(base::as_bytes(data_for_decode), WTF::FlushBehavior::kDoNotFlush,
        options_.GetContentType() == TextResourceDecoderOptions::kXMLContent && !options_.GetUseLenientXMLDecoding(), saw_error_);
...
String TextResourceDecoder::Flush()
{
    ...
    if (buffer_.size()
        && ((!checked_for_xml_charset_
                && (options_.GetContentType() == TextResourceDecoderOptions::kHTMLContent
                    || options_.GetContentType() == TextResourceDecoderOptions::kXMLContent))
            || (!checked_for_css_charset_ && (options_.GetContentType() == TextResourceDecoderOptions::kCSSContent)))) {
        AutoDetectEncodingIfAllowed(base::span<const char>(buffer_.data(), buffer_.size()));
    }

    if (!codec_)
        codec_ = NewTextCodec(encoding_);

    String result = codec_->Decode(base::span<const uint8_t>((const uint8_t*)buffer_.data(), buffer_.size()), WTF::FlushBehavior::kFetchEOF,
        options_.GetContentType() == TextResourceDecoderOptions::kXMLContent && !options_.GetUseLenientXMLDecoding(), saw_error_);
    buffer_.clear();
    codec_.reset();
...
WebEncodingData TextResourceDecoder::GetEncodingData() const
{
    return WebEncodingData {
        .encoding = encoding_.GetName(), .was_detected_heuristically = EncodingWasDetectedHeuristically(), .saw_decoding_error = SawError()
    };
}
