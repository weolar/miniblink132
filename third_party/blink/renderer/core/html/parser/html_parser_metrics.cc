...
void HTMLParserMetrics::AddChunk(base::TimeDelta elapsed_time, unsigned tokens_parsed)
{
...
void HTMLParserMetrics::ReportUMAs()
{
    ...
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Blink.HTMLParsing.ParsingTimeMax4", max_parsing_time_, base::Microseconds(1), base::Seconds(100), 1000);
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Blink.HTMLParsing.ParsingTimeMin4", min_parsing_time_, base::Microseconds(1), base::Seconds(1), 100);
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Blink.HTMLParsing.ParsingTimeTotal4", accumulated_parsing_time_, base::Microseconds(1), base::Seconds(100), 1000);
    UMA_HISTOGRAM_COUNTS_1M("Blink.HTMLParsing.TokensParsedMax4", max_tokens_parsed_);
    UMA_HISTOGRAM_COUNTS_10000("Blink.HTMLParsing.TokensParsedMin4", min_tokens_parsed_);
    UMA_HISTOGRAM_COUNTS_1M("Blink.HTMLParsing.TokensParsedAverage4", total_tokens_parsed_ / chunk_count_);
    UMA_HISTOGRAM_COUNTS_10M("Blink.HTMLParsing.TokensParsedTotal4", total_tokens_parsed_);
    UMA_HISTOGRAM_COUNTS_1000("Blink.HTMLParsing.PreloadRequestCount", total_preload_request_count_);

    // Only report yield data if we actually yielded.
    if (max_yield_interval_ != base::TimeDelta()) {
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Blink.HTMLParsing.YieldedTimeMax4", max_yield_interval_, base::Microseconds(1), base::Seconds(100), 1000);
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Blink.HTMLParsing.YieldedTimeMin4", min_yield_interval_, base::Microseconds(1), base::Seconds(10), 100);
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
            "Blink.HTMLParsing.YieldedTimeAverage4", accumulated_yield_intervals_ / yield_count_, base::Microseconds(1), base::Seconds(10), 100);
    }

    UMA_HISTOGRAM_COUNTS_10M("Blink.HTMLParsing.InputCharacterCount4", input_character_count_);
}
...
void HTMLParserMetrics::ReportMetricsAtParseEnd()
{
    ...
    if (accumulated_yield_intervals_ != base::TimeDelta()) {
        ...
        builder.SetYieldedTimeAverage(accumulated_yield_intervals_.InMicroseconds() / yield_count_);
    }
    if (fetch_queued_preloads_time_ > 0 || preload_time_ > 0 || prepare_to_stop_parsing_time_ > 0 || pump_tokenizer_time_ > 0 || scan_time_ > 0
        || scan_and_preload_time_ > 0) {
        builder.SetFetchQueuedPreloadsTime(ukm::GetExponentialBucketMinForUserTiming(fetch_queued_preloads_time_));
        builder.SetPreloadTime(ukm::GetExponentialBucketMinForUserTiming(preload_time_));
        builder.SetPrepareToStopParsingTime(ukm::GetExponentialBucketMinForUserTiming(prepare_to_stop_parsing_time_));
        builder.SetPumpTokenizerTime(ukm::GetExponentialBucketMinForUserTiming(pump_tokenizer_time_));
        builder.SetScanAndPreloadTime(ukm::GetExponentialBucketMinForUserTiming(scan_and_preload_time_));
