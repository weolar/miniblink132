...
TEST(HTMLViewSourceParserTest, DetachThenFinish_ShouldNotCrash)
{
    ...
    auto* document
        = MakeGarbageCollected<HTMLViewSourceDocument>(DocumentInit::Create().ForTest(execution_context.GetExecutionContext()).WithTypeFrom(mime_type));
    auto* parser = MakeGarbageCollected<HTMLViewSourceParser>(*document, mime_type);
