...
public:
    ...
    public:
        ExclusionInfo(const KURL& document_url, HashSet<KURL> scopes, HashSet<KURL> resources);
        ...
        const KURL& document_url() const
        {
            return document_url_;
        }
        const HashSet<KURL>& scopes() const
        {
            return scopes_;
        }
        const HashSet<KURL>& resources() const
        {
            return resources_;
        }
    ...
    enum RequestType { kRequestTypePreload, kRequestTypePreconnect, kRequestTypeLinkRelPreload };

    static std::unique_ptr<PreloadRequest> CreateIfNeeded(const String& initiator_name, const String& resource_url, const KURL& base_url,
        ResourceType resource_type, const network::mojom::ReferrerPolicy referrer_policy, ResourceFetcher::IsImageSet is_image_set,
        const ExclusionInfo* exclusion_info, const std::optional<float> resource_width = std::nullopt,
        const std::optional<float> resource_height = std::nullopt, RequestType request_type = kRequestTypePreload);
    ...
    FetchParameters::DeferOption DeferOption() const
    {
        return defer_;
    }
    ...
    const String& ResourceURL() const
    {
    ...
    {
        return request_type_ == kRequestTypePreconnect;
    }
    ...
    void SetRenderBlockingBehavior(RenderBlockingBehavior render_blocking_behavior)
    {
    ...
    bool IsPotentiallyLCPElement() const
    {
        return is_potentially_lcp_element_;
    }
    ...
    std::optional<float> GetResourceHeight() const
    {
        return resource_height_;
    }

private:
    PreloadRequest(const String& initiator_name, const String& resource_url, const KURL& base_url, ResourceType resource_type,
        const std::optional<float> resource_width, const std::optional<float> resource_height, RequestType request_type,
        const network::mojom::ReferrerPolicy referrer_policy, ResourceFetcher::IsImageSet is_image_set)
