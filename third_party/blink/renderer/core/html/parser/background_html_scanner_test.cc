...
public:
    BackgroundHTMLScannerTest()
        : task_runner_(worker_pool::CreateSequencedTaskRunner({ base::TaskPriority::USER_BLOCKING }))
    {
    }

protected:
    std::unique_ptr<BackgroundHTMLScanner> CreateScanner(TestParser* parser, wtf_size_t min_script_size = 0u)
    {
        return std::make_unique<BackgroundHTMLScanner>(std::make_unique<HTMLTokenizer>(HTMLParserOptions()),
            std::make_unique<BackgroundHTMLScanner::ScriptTokenScanner>(parser, task_runner_, min_script_size));
    }
...
TEST_F(BackgroundHTMLScannerTest, InsideHTMLPreloadScanner)
{
    ...
    HTMLPreloadScanner preload_scanner(std::make_unique<HTMLTokenizer>(HTMLParserOptions()), GetDocument().Url(),
        std::make_unique<CachedDocumentParameters>(&GetDocument()), std::make_unique<MediaValuesCached::MediaValuesCachedData>(GetDocument()),
        TokenPreloadScanner::ScannerType::kMainDocument, std::make_unique<BackgroundHTMLScanner::ScriptTokenScanner>(parser, task_runner_, 0),
        CrossThreadBindRepeating([](std::unique_ptr<PendingPreloadData>) {}));
    preload_scanner.ScanInBackground("<script>foo</script>", GetDocument().ValidBaseElementURL());
    FlushTaskRunner();
    EXPECT_NE(parser->TakeInlineScriptStreamer("foo"), nullptr);
}
