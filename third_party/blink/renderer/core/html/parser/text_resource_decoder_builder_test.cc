...
static const WTF::TextEncoding DefaultEncodingForUrlAndContentType(const char* url, const char* content_type)
{
    auto page_holder = std::make_unique<DummyPageHolder>(gfx::Size(0, 0));
    Document& document = page_holder->GetDocument();
    document.SetURL(KURL(NullURL(), url));
    return BuildTextResourceDecoder(document.GetFrame(), document.Url(), AtomicString(content_type), g_null_atom)->Encoding();
}
...
TEST(TextResourceDecoderBuilderTest, defaultEncodingForJsonIsUTF8)
{
    ...
    EXPECT_EQ(WTF::TextEncoding("UTF-8"), DefaultEncodingForUrlAndContentType("https://udarenieru.ru/1.2/dealers/", "application/json"));
}
