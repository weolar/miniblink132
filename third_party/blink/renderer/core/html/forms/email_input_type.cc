...
const char kLocalPartCharacters[] = "abcdefghijklmnopqrstuvwxyz0123456789!#$%&'*+/=?^_`{|}~.-";
const char kEmailPattern[] = "[a-z0-9!#$%&'*+/=?^_`{|}~.-]+" // local part
                             "@"
...
{
    return MakeGarbageCollected<ScriptRegexp>(isolate, kEmailPattern, kTextCaseUnicodeInsensitive);
}
...
String EmailInputType::ConvertEmailAddressToASCII(const ScriptRegexp& regexp, const String& address)
{
    ...
    static const icu::IDNA* const idna = icu::IDNA::createUTS46Instance(kIdnaConversionOption, error_code);
    DCHECK(idna);
    icu::IDNAInfo idna_info;
    idna->nameToASCII(idn_domain_name, domain_name, idna_info, error_code);
    if (U_FAILURE(error_code) || idna_info.hasErrors() || domain_name.length() > kMaximumDomainNameLength)
        return address;
...
String EmailInputType::ConvertEmailAddressToUnicode(const String& address) const
{
    ...
    String unicode_host = Platform::Current()->ConvertIDNToUnicode(address.Substring(at_position + 1));
    StringBuilder builder;
    builder.Append(address, 0, at_position + 1);
...
void EmailInputType::CountUsage()
{
    ...
    bool has_max_length = GetElement().FastHasAttribute(html_names::kMaxlengthAttr);
    if (has_max_length)
...
String EmailInputType::FindInvalidAddress(const String& value) const
{
    if (value.empty())
        return String();
    if (!GetElement().Multiple()) {
        return IsValidEmailAddress(GetElement().GetDocument().EnsureEmailRegexp(), value) ? String() : value;
    }
    Vector<String> addresses = ParseMultipleValues(value);
    for (const auto& address : addresses) {
        String stripped = StripLeadingAndTrailingHTMLSpaces(address);
        if (!IsValidEmailAddress(GetElement().GetDocument().EnsureEmailRegexp(), stripped))
            return stripped;
    }
    return String();
}
...
String EmailInputType::TypeMismatchText() const
{
    ...
    if (invalid_address.empty()) {
        return GetLocale().QueryString(IDS_FORM_VALIDATION_TYPE_MISMATCH_EMAIL_EMPTY);
    }
    String at_sign = String("@");
    wtf_size_t at_index = invalid_address.find('@');
    if (at_index == kNotFound)
        return GetLocale().QueryString(IDS_FORM_VALIDATION_TYPE_MISMATCH_EMAIL_NO_AT_SIGN, at_sign, invalid_address);
    ...
    if (local_part.empty())
        return GetLocale().QueryString(IDS_FORM_VALIDATION_TYPE_MISMATCH_EMAIL_EMPTY_LOCAL, at_sign, unicode_address);
    if (domain.empty())
        return GetLocale().QueryString(IDS_FORM_VALIDATION_TYPE_MISMATCH_EMAIL_EMPTY_DOMAIN, at_sign, unicode_address);
    ...
    if (invalid_char_index != kNotFound) {
        ...
        return GetLocale().QueryString(IDS_FORM_VALIDATION_TYPE_MISMATCH_EMAIL_INVALID_LOCAL, at_sign, local_part.Substring(invalid_char_index, char_length));
    }
    invalid_char_index = domain.Find(IsInvalidDomainCharacter);
    if (invalid_char_index != kNotFound) {
        unsigned char_length = U_IS_LEAD(domain[invalid_char_index]) ? 2 : 1;
        return GetLocale().QueryString(IDS_FORM_VALIDATION_TYPE_MISMATCH_EMAIL_INVALID_DOMAIN, at_sign, domain.Substring(invalid_char_index, char_length));
    }
    if (!CheckValidDotUsage(domain)) {
        wtf_size_t at_index_in_unicode = unicode_address.find('@');
        DCHECK_NE(at_index_in_unicode, kNotFound);
        return GetLocale().QueryString(IDS_FORM_VALIDATION_TYPE_MISMATCH_EMAIL_INVALID_DOTS, String("."), unicode_address.Substring(at_index_in_unicode + 1));
    }
    if (GetElement().Multiple()) {
        return GetLocale().QueryString(IDS_FORM_VALIDATION_TYPE_MISMATCH_MULTIPLE_EMAIL);
    }
...
String EmailInputType::ConvertFromVisibleValue(const String& visible_value) const
{
    String sanitized_value = SanitizeValue(visible_value);
    if (!GetElement().Multiple()) {
        return ConvertEmailAddressToASCII(GetElement().GetDocument().EnsureEmailRegexp(), sanitized_value);
    }
    ...
    for (wtf_size_t i = 0; i < addresses.size(); ++i) {
        if (i > 0)
            builder.Append(',');
        builder.Append(ConvertEmailAddressToASCII(GetElement().GetDocument().EnsureEmailRegexp(), addresses[i]));
    }
    return builder.ToString();
}
