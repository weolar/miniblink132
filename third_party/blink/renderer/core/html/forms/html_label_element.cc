...
HTMLLabelElement::HTMLLabelElement(Document& document)
    : HTMLElement(html_names::kLabelTag, document)
        ...
        for (HTMLElement& element : Traversal<HTMLElement>::DescendantsOf(*this)) {
            if (element.IsLabelable()) {
                if (!element.IsFormControlElement()) {
                    UseCounter::Count(GetDocument(), WebFeature::kHTMLLabelElementControlForNonFormAssociatedElement);
                }
                return &element;
            }
        }
...
HTMLElement* HTMLLabelElement::Control() const
{
    ...
    if (auto* reference_target = control->GetShadowReferenceTarget(html_names::kForAttr)) {
    ...
    if (evt.type() == event_type_names::kClick && !processing_click_) {
        ...
        if (mouse_event && mouse_event->HasPosition()) {
            if (LocalFrame* frame = GetDocument().GetFrame()) {
                ...
                // selection.
                if (GetLayoutObject() && GetLayoutObject()->IsSelectable() && frame->Selection().ComputeVisibleSelectionInDOMTree().IsRange()
                    && !frame->GetEventHandler().GetSelectionController().MouseDownWasSingleClickInSelection() && target_node->CanStartSelection()) {
        ...
        if (element->IsMouseFocusable() || (element->IsShadowHostWithDelegatesFocus() && RuntimeEnabledFeatures::LabelAndDelegatesFocusNewHandlingEnabled())) {
            ...
            if (!is_label_text_selected) {
                ...
                if (RuntimeEnabledFeatures::CustomizableSelectEnabled() && select && select->IsAppearanceBaseButton() && select->SlottedButton()) {
                    ...
                    select->SlottedButton()->Focus(
                        FocusParams(SelectionBehaviorOnFocus::kRestore, mojom::blink::FocusType::kMouse, nullptr, FocusOptions::Create()));
                } else {
                    element->Focus(FocusParams(SelectionBehaviorOnFocus::kRestore, mojom::blink::FocusType::kMouse, nullptr, FocusOptions::Create()));
                }
            }
        }
...
void HTMLLabelElement::Focus(const FocusParams& params)
{
    ...
    if (HTMLElement* element = Control()) {
        element->Focus(FocusParams(SelectionBehaviorOnFocus::kRestore, params.type, params.source_capabilities, params.options, params.focus_trigger));
    }
}

void HTMLLabelElement::AccessKeyAction(SimulatedClickCreationScope creation_scope)
{
    if (HTMLElement* element = Control()) {
