...
{
    for (const Node* child = LayoutTreeBuilderTraversal::FirstChild(parent); child; child = LayoutTreeBuilderTraversal::NextSibling(*child)) {
        ...
        if (const ComputedStyle* style = element->GetComputedStyle()) {
            if (style->Display() == EDisplay::kContents && WillReattachChildLayoutObject(*element)) {
                return true;
            }
        }
    }
...
bool HTMLFieldSetElement::IsValidElement()
{
    for (Element* element : *elements()) {
        if (auto* html_form_element = DynamicTo<HTMLFormControlElement>(element)) {
            ...
            if (html_element->IsFormAssociatedCustomElement() && !element->EnsureElementInternals().IsNotCandidateOrValid())
                return false;
        }
    }
    return true;
...
Element* HTMLFieldSetElement::InvalidateDescendantDisabledStateAndFindFocusedOne(Element& base)
{
...
void HTMLFieldSetElement::DisabledAttributeChanged()
{
    ...
    if (Element* focused_element = InvalidateDescendantDisabledStateAndFindFocusedOne(*this))
...
FocusableState HTMLFieldSetElement::SupportsFocus(UpdateBehavior update_behavior) const
{
