...
public:
    ...
    public:
        ...
        const ComputedStyle* CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context) override;
    ...
    {
        return uses_menu_list_;
    }

    void add(const V8UnionHTMLOptGroupElementOrHTMLOptionElement* element, const V8UnionHTMLElementOrLong* before, ExceptionState& exception_state);

    using Node::remove;
    void remove(int index);

    String Value() const;
    void SetValue(const String&, bool send_events = false, WebAutofillState = WebAutofillState::kNotFilled);
    ...
    IndexedPropertySetterResult AnonymousIndexedSetter(unsigned, HTMLOptionElement*, ExceptionState&);
    ...
    void SelectedContentElementInserted(HTMLSelectedContentElement* selectedcontent);
    void SelectedContentElementRemoved(HTMLSelectedContentElement* selectedcontent);
...
private:
    ...
    void DispatchBlurEvent(Element* new_focused_element, mojom::blink::FocusType, InputDeviceCapabilities* source_capabilities) override;
    ...
    void SelectOption(HTMLOptionElement*, SelectOptionFlags, WebAutofillState = WebAutofillState::kNotFilled);
    bool DeselectItemsWithoutValidation(HTMLOptionElement* element_to_exclude = nullptr);
    void ParseMultipleAttribute(const AtomicString&);
    HTMLOptionElement* LastSelectedOption() const;
    wtf_size_t SearchOptionsForValue(const String&, wtf_size_t list_index_start, wtf_size_t list_index_end) const;
