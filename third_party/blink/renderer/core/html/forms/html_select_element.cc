        ...
        for (const auto& record : records) {
            if (record->type() == "childList") {
                for (unsigned i = 0; i < record->addedNodes()->length(); ++i) {
                    ...
                    if (descendant->IsTextNode() && descendant->textContent().ContainsOnlyWhitespaceOrEmpty()) {
                        continue;
                    }
...
private:
    ...
    {
        for (Element* current_element = ElementTraversal::FirstWithin(*select_); current_element;) {
    ...
    void AddWarningToElement(HTMLElement* html_element)
    {
        ...
        html_element->AddConsoleMessage(mojom::blink::ConsoleMessageSource::kRecommendation, mojom::blink::ConsoleMessageLevel::kError,
    ...
    bool IsDescendantAllowed(HTMLElement* element)
    {
        ...
        return IsA<HTMLOptionElement>(element) || IsA<HTMLOptGroupElement>(element) || IsA<HTMLHRElement>(element) || IsA<HTMLScriptElement>(element)
            || IsA<HTMLTemplateElement>(element) || IsA<HTMLNoScriptElement>(element) || IsA<HTMLButtonElement>(element) || IsA<HTMLDivElement>(element)
            || IsA<HTMLSpanElement>(element) || IsA<HTMLSelectedContentElement>(element);
    }
...
{
    return is_multiple_ ? FormControlType::kSelectMultiple : FormControlType::kSelectOne;
}
...
bool HTMLSelectElement::ValueMissing() const
{
    ...
    return first_selection_index < 0 || (!first_selection_index && HasPlaceholderLabelOption());
}
...
void HTMLSelectElement::SelectMultipleOptionsByPopup(const Vector<int>& list_indices)
{
    DCHECK(UsesMenuList());
    DCHECK(IsMultiple());
...
void HTMLSelectElement::add(
    const V8UnionHTMLOptGroupElementOrHTMLOptionElement* element, const V8UnionHTMLElementOrLong* before, ExceptionState& exception_state)
{
    DCHECK(element);

    HTMLElement* element_to_insert = nullptr;
    switch (element->GetContentType()) {
    case V8UnionHTMLOptGroupElementOrHTMLOptionElement::ContentType::kHTMLOptGroupElement:
        element_to_insert = element->GetAsHTMLOptGroupElement();
        break;
    case V8UnionHTMLOptGroupElementOrHTMLOptionElement::ContentType::kHTMLOptionElement:
    ...
    target_container->InsertBefore(element_to_insert, before_element, exception_state);
    SetNeedsValidityCheck();
}
...
void HTMLSelectElement::setValueForBinding(const String& value)
{
    ...
    SetValue(value, false, was_autofilled && !value_changed ? WebAutofillState::kAutofilled : WebAutofillState::kNotFilled);
...
void HTMLSelectElement::SetValue(const String& value, bool send_events, WebAutofillState autofill_state)
{
...
void HTMLSelectElement::SetAutofillValue(const String& value, WebAutofillState autofill_state)
{
...
void HTMLSelectElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kSizeAttr) {
        ...
        if (RuntimeEnabledFeatures::CustomizableSelectEnabled()) {
            HTMLSelectedContentElement* old_selectedcontent
                = DynamicTo<HTMLSelectedContentElement>(getElementByIdIncludingDisconnected(*this, params.old_value));
            HTMLSelectedContentElement* new_selectedcontent
                = DynamicTo<HTMLSelectedContentElement>(getElementByIdIncludingDisconnected(*this, params.new_value));
...
LayoutObject* HTMLSelectElement::CreateLayoutObject(const ComputedStyle& style)
{
    if (UsesMenuList()) {
...
void HTMLSelectElement::AccessKeyAction(SimulatedClickCreationScope creation_scope)
{
...
void HTMLSelectElement::SetOption(unsigned index, HTMLOptionElement* option, ExceptionState& exception_state)
{
    int diff = index - length();
    // If we are adding options, we should check |index > maxListItems| first to
    // avoid integer overflow.
    if (index > length() && (index >= kMaxListItems || GetListItems().size() + diff + 1 > kMaxListItems)) {
        GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kWarning,
            String::Format("Unable to expand the option list and set an option at index=%u. "
                           "The maximum allowed list length is %u.",
                index, kMaxListItems)));
        return;
    }
    auto* element = MakeGarbageCollected<V8UnionHTMLOptGroupElementOrHTMLOptionElement>(option);
...
void HTMLSelectElement::setLength(unsigned new_len, ExceptionState& exception_state)
{
    // If we are adding options, we should check |index > maxListItems| first to
    // avoid integer overflow.
    if (new_len > length() && (new_len > kMaxListItems || GetListItems().size() + new_len - length() > kMaxListItems)) {
        GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kWarning,
    ...
    if (diff < 0) { // Add dummy elements.
        do {
            AppendChild(MakeGarbageCollected<HTMLOptionElement>(GetDocument()), exception_state);
            if (exception_state.HadException())
                break;
        } while (++diff);
    } else {
...
{
    if (HTMLCollection* collection = CachedCollection<HTMLCollection>(kSelectedOptions))
...
void HTMLSelectElement::SetRecalcListItems()
{
    ...
    if (!isConnected()) {
        if (HTMLOptionsCollection* collection = CachedCollection<HTMLOptionsCollection>(kSelectOptions))
            collection->InvalidateCache();
...
void HTMLSelectElement::RecalcListItems() const
{
    ...
    for (Element* current_element = ElementTraversal::FirstWithin(*this); current_element && list_items_.size() < kMaxListItems;) {
        auto* current_html_element = DynamicTo<HTMLElement>(current_element);
        if (!current_html_element) {
            current_element = RuntimeEnabledFeatures::SelectParserRelaxationEnabled() ? ElementTraversal::Next(*current_element, this)
        ...
        if (IsA<HTMLSelectElement>(current_html_element)) {
            current_element = ElementTraversal::NextSkippingChildren(*current_element, this);
            continue;
        }
        ...
        if (RuntimeEnabledFeatures::SelectParserRelaxationEnabled()) {
            ...
            if (auto* current_optgroup = DynamicTo<HTMLOptGroupElement>(*current_html_element)) {
            ...
            } else if (IsA<HTMLOptionElement>(*current_html_element) || IsA<HTMLHRElement>(*current_html_element)) {
                list_items_.push_back(current_html_element);
            }

            Element* (*next_element_fn)(const Node&, const Node*) = &ElementTraversal::Next;
            if (skip_children) {
            ...
            if (current_ancestor_optgroup) {
                ...
                auto* next_within_optgroup = next_element_fn(*current_element, current_ancestor_optgroup);
        ...
        if (IsA<HTMLOptGroupElement>(*current_html_element)) {
            if (current_html_element->parentNode() != this) {
                current_element = ElementTraversal::NextSkippingChildren(*current_html_element, this);
                continue;
            }
            list_items_.push_back(current_html_element);
            if (Element* next_element = ElementTraversal::FirstWithin(*current_html_element)) {
...
void HTMLSelectElement::ResetToDefaultSelection(ResetReason reason)
{
    ...
    if (!last_selected_option && size_ <= 1 && (!first_enabled_option || (first_enabled_option && !first_enabled_option->Selected()))) {
        SelectOption(first_enabled_option, reason == kResetReasonSelectedOptionRemoved ? 0 : kDeselectOtherOptionsFlag);
...
void HTMLSelectElement::SetSuggestedOption(HTMLOptionElement* option)
{
    if (suggested_option_ == option)
        return;
    SetAutofillState(option ? WebAutofillState::kPreviewed : WebAutofillState::kNotFilled);
    suggested_option_ = option;

    select_type_->DidSetSuggestedOption(option);
}

void HTMLSelectElement::OptionSelectionStateChanged(HTMLOptionElement* option, bool option_is_selected)
{
...
void HTMLSelectElement::ChildrenChanged(const ChildrenChange& change)
{
    ...
    if (change.type == ChildrenChangeType::kFinishedBuildingDocumentFragmentTree) {
        ...
        } else if (auto* optgroup = DynamicTo<HTMLOptGroupElement>(change.sibling_changed)) {
            for (auto& child_option : Traversal<HTMLOptionElement>::ChildrenOf(*optgroup))
    ...
    if (invalidate_button && RuntimeEnabledFeatures::CustomizableSelectEnabled()) {
        GetShadowRoot()->SetDelegatesFocus(IsAppearanceBaseButton() && SlottedButton());
...
void HTMLSelectElement::OptionInserted(HTMLOptionElement& option, bool option_is_selected)
{
    DCHECK_EQ(option.OwnerSelectElement(), this);
    ...
    GetDocument().GetFrame()->GetPage()->GetChromeClient().SelectFieldOptionsChanged(*this);
}
...
void HTMLSelectElement::SelectOption(HTMLOptionElement* element, SelectOptionFlags flags, WebAutofillState autofill_state)
{
    ...
    if (LocalFrame::HasTransientUserActivation(GetDocument().GetFrame()) && GetDocument().IsActive()) {
        GetDocument().GetPage()->GetChromeClient().DidChangeSelectionInSelectControl(*this);
    }
...
bool HTMLSelectElement::DispatchFocusEvent(Element* old_focused_element, mojom::blink::FocusType type, InputDeviceCapabilities* source_capabilities)
{
    // Save the selection so it can be compared to the new selection when
    // dispatching change events during blur event dispatch.
    if (UsesMenuList())
        select_type_->SaveLastSelection();
    return HTMLFormControlElementWithState::DispatchFocusEvent(old_focused_element, type, source_capabilities);
}

void HTMLSelectElement::DispatchBlurEvent(Element* new_focused_element, mojom::blink::FocusType type, InputDeviceCapabilities* source_capabilities)
{
    type_ahead_.ResetSession();
    select_type_->DidBlur();
    HTMLFormControlElementWithState::DispatchBlurEvent(new_focused_element, type, source_capabilities);
}

// Returns true if selection state of any OPTIONs is changed.
bool HTMLSelectElement::DeselectItemsWithoutValidation(HTMLOptionElement* exclude_element)
{
    if (!IsMultiple() && UsesMenuList() && last_on_change_option_ && last_on_change_option_ != exclude_element) {
...
wtf_size_t HTMLSelectElement::SearchOptionsForValue(const String& value, wtf_size_t list_index_start, wtf_size_t list_index_end) const
{
    const ListItems& items = GetListItems();
...
void HTMLSelectElement::RestoreFormControlState(const FormControlState& state)
{
    ...
    if (!IsMultiple()) {
        ...
        auto* option_element = index < items_size ? DynamicTo<HTMLOptionElement>(items[index].Get()) : nullptr;
        if (option_element && option_element->value() == state[0]) {
            ...
            if (found_index != kNotFound) {
                auto* found_option_element = To<HTMLOptionElement>(items[found_index].Get());
        ...
        for (wtf_size_t i = 0; i < state.ValueSize(); i += 2) {
            ...
            if (option_element && option_element->value() == value) {
                ...
                wtf_size_t found_index = SearchOptionsForValue(value, start_index, items_size);
                if (found_index == kNotFound)
...
void HTMLSelectElement::UpdateMutationObserver()
{
    ...
    if (UsesMenuList() && isConnected() && IsAppearanceBasePicker()) {
        if (!descendants_observer_) {
            descendants_observer_ = MakeGarbageCollected<SelectDescendantsObserver>(*this);
        }
...
void HTMLSelectElement::DefaultEventHandler(Event& event)
{
    ...
    if (event.type() == event_type_names::kClick || event.type() == event_type_names::kChange || event.type() == event_type_names::kKeydown) {
    ...
    if (event.type() == event_type_names::kKeypress && keyboard_event) {
        if (!keyboard_event->ctrlKey() && !keyboard_event->altKey() && !keyboard_event->metaKey()
...
{
    int index = type_ahead_.HandleEvent(event, event.charCode(), TypeAhead::kMatchPrefix | TypeAhead::kCycleFirstChar);
    if (index < 0) {
        return;
    }
    ...
    if (RuntimeEnabledFeatures::CustomizableSelectEnabled() && select_type_->IsAppearanceBasePicker() && select_type_->PopupIsVisible()) {
        option_at_index->Focus(FocusParams(FocusTrigger::kScript));
        return;
    }

    SelectOption(option_at_index, kDeselectOtherOptionsFlag | kMakeOptionDirtyFlag | kDispatchInputAndChangeEventFlag);
...
void HTMLSelectElement::SelectOptionByAccessKey(HTMLOptionElement* option)
{
    ...
    // selected index.
    SelectOptionFlags flags = kDispatchInputAndChangeEventFlag | (IsMultiple() ? 0 : kDeselectOtherOptionsFlag);
    if (option->Selected()) {
        if (UsesMenuList())
...
IndexedPropertySetterResult HTMLSelectElement::AnonymousIndexedSetter(unsigned index, HTMLOptionElement* value, ExceptionState& exception_state)
{
...
{
    return element.GetComputedStyle() ? element.GetComputedStyle() : element.EnsureComputedStyle();
}
...
LayoutUnit HTMLSelectElement::ClientPaddingLeft() const
{
    ...
    int inner_padding
        = style.IsLeftToRightDirection() ? theme.PopupInternalPaddingStart(style) : theme.PopupInternalPaddingEnd(GetDocument().GetFrame(), style);
...
LayoutUnit HTMLSelectElement::ClientPaddingRight() const
{
    ...
    const ComputedStyle& style = this_box->StyleRef();
    int inner_padding
        = style.IsLeftToRightDirection() ? theme.PopupInternalPaddingEnd(GetDocument().GetFrame(), style) : theme.PopupInternalPaddingStart(style);
...
void HTMLSelectElement::SelectOptionByPopup(HTMLOptionElement* option)
{
    ...
    if (option == SelectedOption())
        return;
    SelectOption(option, kDeselectOtherOptionsFlag | kMakeOptionDirtyFlag | kDispatchInputAndChangeEventFlag);
}
...
void HTMLSelectElement::DidRecalcStyle(const StyleRecalcChange change)
{
    ...
    if (auto* style = GetComputedStyle()) {
        if (style->EffectiveAppearance() == ControlPart::kNoControlPart) {
            UseCounter::Count(GetDocument(), WebFeature::kSelectElementAppearanceNone);
        }
    }
...
void HTMLSelectElement::CloneNonAttributePropertiesFrom(const Element& source, NodeCloningData& data)
{
...
void HTMLSelectElement::ChangeRendering()
{
    ...
    SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kControl));
    UpdateMutationObserver();
}
...
void HTMLSelectElement::showPicker(ExceptionState& exception_state)
{
    ...
    if (!LocalFrame::HasTransientUserActivation(frame)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "showPicker() requires a user gesture.");
        return;
    }

    document.UpdateStyleAndLayout(DocumentUpdateReason::kJavaScript);
    if (DisplayLockUtilities::LockedAncestorPreventingPaint(*this) || !GetLayoutBox()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "showPicker() requires the select is rendered.");
        return;
    }
...
bool HTMLSelectElement::IsValidBuiltinCommand(HTMLElement& invoker, CommandEventType command)
{
...
bool HTMLSelectElement::HandleCommandInternal(HTMLElement& invoker, CommandEventType command)
{
...
bool HTMLSelectElement::IsPopoverForAppearanceBase(const Element* element)
{
    if (auto* root = DynamicTo<ShadowRoot>(element->parentNode())) {
        return IsA<HTMLSelectElement>(root->host()) && element->FastHasAttribute(html_names::kPopoverAttr);
    }
    return false;
}
...
void HTMLSelectElement::SelectedContentElementInserted(HTMLSelectedContentElement* selectedcontent)
{
    descendant_selectedcontents_.insert(selectedcontent);
    selectedcontent->CloneContentsFromOptionElement(SelectedOption());
}

void HTMLSelectElement::SelectedContentElementRemoved(HTMLSelectedContentElement* selectedcontent)
{
    descendant_selectedcontents_.erase(selectedcontent);
    selectedcontent->CloneContentsFromOptionElement(nullptr);
}

FocusableState HTMLSelectElement::SupportsFocus(UpdateBehavior update_behavior) const
{
...
HTMLSelectElement::SelectAutofillPreviewElement* HTMLSelectElement::GetAutofillPreviewElement() const
{
    return select_type_->GetAutofillPreviewElement();
}

HTMLSelectElement::SelectAutofillPreviewElement::SelectAutofillPreviewElement(Document& document, HTMLSelectElement* select)
...
const ComputedStyle* HTMLSelectElement::SelectAutofillPreviewElement::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
    ...
    const ComputedStyle* original_style = OriginalStyleForLayoutObject(style_recalc_context);
    ComputedStyleBuilder style_builder(*original_style);
    if (button_style.HasAuthorBorderRadius()) {
        style_builder.SetBorderBottomLeftRadius(button_style.BorderBottomLeftRadius());
        style_builder.SetBorderBottomRightRadius(button_style.BorderBottomRightRadius());
...
Node::InsertionNotificationRequest HTMLSelectElement::SelectAutofillPreviewElement::InsertedInto(ContainerNode& container)
{
    select_->IncrementImplicitlyAnchoredElementCount();
    return HTMLDivElement::InsertedInto(container);
}

void HTMLSelectElement::SelectAutofillPreviewElement::RemovedFrom(ContainerNode& container)
{
    HTMLDivElement::RemovedFrom(container);
    select_->DecrementImplicitlyAnchoredElementCount();
}

void HTMLSelectElement::SelectAutofillPreviewElement::Trace(Visitor* visitor) const
{
    visitor->Trace(select_);
...
HTMLSelectedContentElement* HTMLSelectElement::selectedContentElement() const
{
    ...
    return DynamicTo<HTMLSelectedContentElement>(GetElementAttribute(html_names::kSelectedcontentelementAttr));
}

void HTMLSelectElement::setSelectedContentElement(HTMLSelectedContentElement* new_selectedcontent)
{
    CHECK(RuntimeEnabledFeatures::CustomizableSelectEnabled());
    auto* old_selectedcontent = selectedContentElement();
    SetElementAttribute(html_names::kSelectedcontentelementAttr, new_selectedcontent);
...
void HTMLSelectElement::UpdateAllSelectedcontents()
{
    ...
    // while iterating.
    for (auto& selectedcontent : VectorOf<HTMLSelectedContentElement>(descendant_selectedcontents_)) {
