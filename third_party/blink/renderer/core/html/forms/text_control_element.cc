...
TextControlElement::TextControlElement(const QualifiedName& tag_name, Document& doc)
...
{
    cached_selection_direction_ = doc.GetFrame() && doc.GetFrame()->GetEditor().Behavior().ShouldConsiderSelectionAsDirectional()
...
bool TextControlElement::DispatchFocusEvent(Element* old_focused_element, mojom::blink::FocusType type, InputDeviceCapabilities* source_capabilities)
{
...
void TextControlElement::DispatchBlurEvent(Element* new_focused_element, mojom::blink::FocusType type, InputDeviceCapabilities* source_capabilities)
{
    if (SupportsPlaceholder())
        UpdatePlaceholderVisibility();
    HandleBlurEvent();
...
{
    if (event.type() == event_type_names::kWebkitEditableContentChanged && GetLayoutObject() && GetLayoutObject()->IsTextControl()) {
        ...
        if (IsFocused()) {
            ...
            CacheSelection(computed_selection.start, computed_selection.end, computed_selection.direction);
        }
...
{
    if (event.type() == event_type_names::kBlur || event.type() == event_type_names::kFocus)
        return;
...
String TextControlElement::StrippedPlaceholder() const
{
    ...
    // the attribute value.
    const AtomicString& attribute_value = FastGetAttribute(html_names::kPlaceholderAttr);
    if (!attribute_value.Contains(kNewlineCharacter) && !attribute_value.Contains(kCarriageReturnCharacter))
...
{
    return SuggestedValue().empty() && SupportsPlaceholder() && FastHasAttribute(html_names::kPlaceholderAttr) && IsInnerEditorValueEmpty();
}
...
void TextControlElement::UpdatePlaceholderVisibility()
{
    ...
    if (!placeholder) {
        if (RuntimeEnabledFeatures::CreateInputShadowTreeDuringLayoutEnabled() && !InnerEditorElement()) {
    ...
    if (placeholder) {
        placeholder->SetInlineStyleProperty(CSSPropertyID::kDisplay,
            ...
            IsPlaceholderVisible() || !SuggestedValue().IsNull() ? CSSValueID::kBlock : CSSValueID::kNone, true);
    }

    // If there was a visibility change not caused by the suggested value, set
    // that the pseudo state changed.
    if (place_holder_was_visible != IsPlaceholderVisible() && SuggestedValue().empty()) {
        PseudoStateChanged(CSSSelector::kPseudoPlaceholderShown);
    }
}

void TextControlElement::UpdatePlaceholderShadowPseudoId(HTMLElement& placeholder)
{
    if (suggested_value_.empty()) {
        // Reset the pseudo-id for placeholders to use the appropriated style
        placeholder.SetShadowPseudoId(shadow_element_names::kPseudoInputPlaceholder);
    } else {
        // Set the pseudo-id for suggested values to use the appropriated style.
        placeholder.SetShadowPseudoId(shadow_element_names::kPseudoInternalInputSuggested);
    }
}
...
void TextControlElement::select()
{
    ...
    // the selection.
    Focus(FocusParams(SelectionBehaviorOnFocus::kNone, mojom::blink::FocusType::kScript, nullptr, FocusOptions::Create()));
...
void TextControlElement::SetFocused(bool flag, mojom::blink::FocusType focus_type)
{
...
{
    if (!value_before_first_user_edit_.IsNull() && !EqualIgnoringNullity(value_before_first_user_edit_, Value())) {
        ClearValueBeforeFirstUserEdit();
        DispatchChangeEvent();
    } else {
        ClearValueBeforeFirstUserEdit();
...
void TextControlElement::setRangeText(const String& replacement, ExceptionState& exception_state)
{
    setRangeText(replacement, selectionStart(), selectionEnd(), V8SelectionMode(V8SelectionMode::Enum::kPreserve), exception_state);
}

void TextControlElement::setRangeText(
    const String& replacement, unsigned start, unsigned end, const V8SelectionMode& selection_mode, ExceptionState& exception_state)
{
    ...
    SetValue(text.ToString(), TextFieldEventBehavior::kDispatchNoEvent, TextControlSetValueSelection::kDoNotSet);
...
void TextControlElement::setSelectionRangeForBinding(unsigned start, unsigned end, const String& direction_string)
{
...
unsigned TextControlElement::IndexForPosition(HTMLElement* inner_editor, const Position& passed_position)
{
    if (!inner_editor || !inner_editor->contains(passed_position.AnchorNode()) || passed_position.IsNull())
        return 0;
    ...
    for (Node* node = start_node; node; node = NodeTraversal::Previous(*node, inner_editor)) {
        ...
        } else if (node->HasTagName(html_names::kBrTag) && node != inner_editor->lastChild()) {
            ++index;
        }
    }
...
bool TextControlElement::SetSelectionRange(unsigned start, unsigned end, TextFieldSelectionDirection direction)
{
    ...
    if (direction == kSelectionHasNoDirection && frame && frame->GetEditor().Behavior().ShouldConsiderSelectionAsDirectional())
        direction = kSelectionHasForwardDirection;
    ...
    Position end_position = start == end ? start_position : PositionForIndex(inner_editor, end);
    ...
    frame->Selection().SetSelection(SelectionInDOMTree::Builder()
                                        .Collapse(direction == kSelectionHasBackwardDirection ? end_position : start_position)
                                        .Extend(direction == kSelectionHasBackwardDirection ? start_position : end_position)
                                        .Build(),
...
bool TextControlElement::CacheSelection(unsigned start, unsigned end, TextFieldSelectionDirection direction)
{
    DCHECK_LE(start, end);
    bool did_change = cached_selection_start_ != start || cached_selection_end_ != end || cached_selection_direction_ != direction;
...
void TextControlElement::ComputeSelection(uint32_t flags, ComputedSelection& computed_selection) const
{
    ...
    const SelectionInDOMTree& selection = frame->Selection().GetSelectionInDOMTree();
    if (flags & kStart) {
        computed_selection.start = IndexForPosition(InnerEditorElement(), selection.ComputeStartPosition());
    }
    if (flags & kEnd) {
        if (flags & kStart && !selection.IsRange()) {
            computed_selection.end = computed_selection.start;
        } else {
            computed_selection.end = IndexForPosition(InnerEditorElement(), selection.ComputeEndPosition());
        }
    }
    if (flags & kDirection && frame->Selection().IsDirectional()) {
        computed_selection.direction = (selection.IsAnchorFirst()) ? kSelectionHasForwardDirection : kSelectionHasBackwardDirection;
    }
}
...
static inline void SetContainerAndOffsetForRange(Node* node, int offset, Node*& container_node, int& offset_in_container)
{
    if (node->IsTextNode()) {
        container_node = node;
...
SelectionInDOMTree TextControlElement::Selection() const
{
    ...
    if (!inner_text->HasChildren()) {
        return SelectionInDOMTree::Builder().Collapse(Position(inner_text, 0)).Build();
    }
    ...
    if (GetDocument().FocusedElement() == this && GetDocument().GetFrame()) {
        const SelectionInDOMTree& selection = GetDocument().GetFrame()->Selection().GetSelectionInDOMTree();
        affinity = selection.Affinity();
    }

    return SelectionInDOMTree::Builder().SetBaseAndExtent(Position(start_node, start), Position(end_node, end)).SetAffinity(affinity).Build();
}
...
void TextControlElement::setMaxLength(int new_value, ExceptionState& exception_state)
{
    int min = minLength();
    if (new_value < 0) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError, "The value provided (" + String::Number(new_value) + ") is not positive or 0.");
    } else if (min >= 0 && new_value < min) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError, ExceptionMessages::IndexExceedsMinimumBound("maxLength", new_value, min));
    } else {
        SetIntegralAttribute(html_names::kMaxlengthAttr, new_value);
    }
}

void TextControlElement::setMinLength(int new_value, ExceptionState& exception_state)
{
    int max = maxLength();
    if (new_value < 0) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError, "The value provided (" + String::Number(new_value) + ") is not positive or 0.");
    } else if (max >= 0 && new_value > max) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError, ExceptionMessages::IndexExceedsMaximumBound("minLength", new_value, max));
    } else {
...
{
    if (SetSelectionRange(cached_selection_start_, cached_selection_end_, cached_selection_direction_))
        ScheduleSelectEvent();
}
...
void TextControlElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kPlaceholderAttr) {
        UpdatePlaceholderText();
        UpdatePlaceholderVisibility();
        UseCounter::Count(GetDocument(), WebFeature::kPlaceholderAttribute);
    } else if (params.name == html_names::kReadonlyAttr || params.name == html_names::kDisabledAttr) {
        ...
        if (HTMLElement* inner_editor = InnerEditorElement()) {
            if (auto* frame = GetDocument().GetFrame()) {
                frame->GetSpellChecker().RespondToChangedEnablement(*inner_editor, IsSpellCheckingEnabled());
            }
        }
    } else {
        HTMLFormControlElementWithState::ParseAttribute(params);
...
void TextControlElement::DisabledOrReadonlyAttributeChanged(const QualifiedName& attr)
{
    if (Element* inner_editor = InnerEditorElement()) {
        inner_editor->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::FromAttribute(attr));
    }
}
...
void TextControlElement::AddPlaceholderBreakElementIfNecessary()
{
    ...
    if (inner_editor->GetLayoutObject() && inner_editor->GetLayoutObject()->Style()->ShouldCollapseBreaks()) {
        return;
    }
...
String TextControlElement::InnerEditorValue() const
{
    ...
    if (auto* first_child_text_node = DynamicTo<Text>(first_child)) {
        ...
        if (!second_child || (!second_child->nextSibling() && IsA<HTMLBRElement>(*second_child)))
    ...
    if (layout_object->IsLayoutNGObject()) {
        ...
        for (Node& node : NodeTraversal::DescendantsOf(*inner_text)) {
            if (IsA<HTMLBRElement>(node)) {
                ...
                unsigned position = 0;
                while (break_position.AnchorNode() == node && static_cast<unsigned>(break_position.OffsetInContainerNode()) <= length) {
...
{
    DCHECK(position.IsNull() || position.IsOffsetInAnchor() || position.ComputeContainerNode() || !position.AnchorNode()->OwnerShadowHost()
...
TextControlElement* EnclosingTextControl(const Node* container)
{
    ...
    return ancestor && IsTextControl(*ancestor) && container->ContainingShadowRoot()->IsUserAgent() ? ToTextControl(ancestor) : nullptr;
}
...
String TextControlElement::DirectionForFormData() const
{
    for (const HTMLElement* element = this; element; element = Traversal<HTMLElement>::FirstAncestor(*element)) {
        const AtomicString& dir_attribute_value = element->FastGetAttribute(html_names::kDirAttr);
        ...
        if (EqualIgnoringASCIICase(dir_attribute_value, "rtl") || EqualIgnoringASCIICase(dir_attribute_value, "ltr"))
            return dir_attribute_value;

        if (EqualIgnoringASCIICase(dir_attribute_value, "auto")) {
            return element->CachedDirectionality() == TextDirection::kRtl ? "rtl" : "ltr";
        }
    }
...
void TextControlElement::SetAutofillValue(const String& value, WebAutofillState autofill_state)
{
    // Set the value trimmed to the max length of the field and dispatch the input
    // and change events.
    SetValue(value.Substring(0, maxLength()), TextFieldEventBehavior::kDispatchInputAndChangeEvent, TextControlSetValueSelection::kSetSelectionToEnd,
...
void TextControlElement::SetSuggestedValue(const String& value)
{
    ...
    const String new_suggested_value = value.empty() ? value : value.Substring(0, maxLength());
...
HTMLElement* TextControlElement::CreateInnerEditorElement()
{
    ...
    inner_editor_ = MakeGarbageCollected<TextControlInnerEditorElement>(GetDocument());
...
void TextControlElement::ScheduleSelectionchangeEvent()
{
    if (RuntimeEnabledFeatures::CoalesceSelectionchangeEventEnabled()) {
        ...
        EnqueueEvent(*Event::CreateBubble(event_type_names::kSelectionchange), TaskType::kMiscPlatformAPI);
    } else {
        EnqueueEvent(*Event::CreateBubble(event_type_names::kSelectionchange), TaskType::kMiscPlatformAPI);
    }
}
...
void TextControlElement::CloneNonAttributePropertiesFrom(const Element& source, NodeCloningData& data)
{
    const TextControlElement& source_element = static_cast<const TextControlElement&>(source);
